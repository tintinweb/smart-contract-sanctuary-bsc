...
interface IERC20 {
	 ...
	 * This allows applications to reconstruct the allowance for all Shibaroo just
	...
	library SafeMath {
		...
		function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
			...
			// benefit is lost if 'b' is also Shibarooed.
	...
	// File: contracts/ShibarooBEP20PayingToken.sol
	...
	// File: contracts/Shibaroo.sol

	// 

	pragma solidity ^0.8.0;

	contract Shibaroo is ERC20, Ownable {
		using SafeMath for uint256;
		...
		address payable public devFundingWallet = payable(msg.sender); 

		
        // Transaction Limit
		uint256 public maxTxAmount = 10000000 * (10**9); 
		uint256 public maxWalletToken = 30000000 * (10**9);
		...
		event excludeMultipleAccountFromRewards(address[] Shibaroo, bool isExcluded);
		...
		constructor() ERC20("Shibaroo", "SHIROO"){
			uint256 _liquidityBuyTax = 2;
			uint256 _marketingBuyTax = 6;
			uint256 _devBuyTax = 2;
			uint256 _liquiditySellTax = 2;
			uint256 _marketingSellTax = 6;
			uint256 _devSellTax = 2;
			uint256 _extraSellTax = 2;
			...
			totalBuyTax = _liquidityBuyTax + _marketingBuyTax + _devBuyTax;
			totalSellTax = _liquiditySellTax + _marketingSellTax + _devSellTax + _extraSellTax;
		...
		function setIsFeeExempt(address account, bool excluded) public onlyOwner {
			require(_isExcludedFromFees[account] != excluded, "Shibaroo: Account is already the value of 'excluded'");
			_isExcludedFromFees[account] = excluded;
		}
        
        function setIsTxLimitExempt(address account, bool excluded) public onlyOwner {
			require(_isTxLimit[account] != excluded, "Shibaroo: Account is already the value of 'excluded'");
		...
		function setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
			require(pair != pancakeswapV2Pair, "Shibaroo: The pancakeswap pair cannot be removed from automatedMarketMakerPairs");

			_setAutomatedMarketMakerPair(pair, value);
		}

		function _setAutomatedMarketMakerPair(address pair, bool value) private {
			require(automatedMarketMakerPairs[pair] != value, "Shibaroo: Automated market maker pair is already set to that value");
		...
		function updateLiquidityWallet(address newLiquidityWallet) private onlyOwner {
			require(newLiquidityWallet != liquidityWallet, "Shibaroo: The liquidity wallet is already this address");
		...
		function setDistributorSettings(uint256 gas) private onlyOwner {
			require(gas >= 200000 && gas <= 500000, "Shibaroo: distributorGas must be between 200,000 and 500,000");
			require(gas != distributorGas, "Shibaroo: Cannot update distributorGas to same value");
		...
		function setFundingWallet(address payable _marketingFundingWallet, address payable _devFundingWallet) external onlyOwner() {
		...
		// Set our buy Taxes
        function setBuyTaxes (uint256 _liquidityBuyTax, uint256 _marketingBuyTax, uint256 _devBuyTax) external onlyOwner {
            liquidityBuyTax = _liquidityBuyTax;
            marketingBuyTax = _marketingBuyTax;
			devBuyTax = _devBuyTax;
			totalBuyTax = _liquidityBuyTax + _marketingBuyTax + _devBuyTax;
		 ...
		 // Set our sell fees
		function setSellTaxes ( uint256 _liquiditySellTax, uint256 _marketingSellTax, uint256 _devSellTax, uint256 _extraSellTax) external onlyOwner {
			liquiditySellTax = _liquiditySellTax;
            marketingSellTax = _marketingSellTax;
			devSellTax = _devSellTax;
			extraSellTax = _extraSellTax;
		...
		function _transfer(
			...
			if(!tradingOpen) {
				require(canTransferBeforeswapEnabled[from], "Shibaroo: This account cannot send tokens until trading is enabled");
			}
