...
contract TESTTOKENTest is ERC20, Ownable {
    ...
    constructor() ERC20("TESTLANDS", "TESTTOKENTEST") {
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        ...
        require(newDividendTracker.owner() == address(this), "TESTTOKEN: The new dividend tracker must be owned by the TESTLANDS contract");
    ...
    function setrewardFees(uint256 value) external onlyOwner{
        ...
        require(totalFees <= 25, "TESTTOKEN: Maximum allowed fees are 25%");
    }
    ...
    function setExtraSellFee(uint256 value) external onlyOwner{
        ...
        require(totalSellFees <= 25, "TESTTOKEN: Maximum allowed fees are 25%");
    }
    ...
    function limitMaxWallet(uint256 maxWalletPercent) external onlyOwner {
         ...
         require(maxWalletPercent >= 1, "TESTTOKEN: Can not limit Max Wallet to less than 1%");
    }
    ...
    function _transfer(
        ...
        if (from != owner() &&
            ...
            require(balanceOf(to) + amount <= maxWalletTokens, "TESTTOKEN: Exceeds maximum wallet token amount.");
        }
        if(amount == 0) {
        ...
        if(takeFee) {
            require(tradingOpen,"TESTTOKEN: Trading is not open yet");
...
contract TESTTOKENDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor() DividendPayingToken("TESTTOKEN_Dividen_Tracker", "TESTTOKEN_Dividend_Tracker") {
    	claimWait = 3600;
...
// Written by @MrGreenCrypto for TESTLANDS