...
contract XHashToken is BEP20, MinterRole {
    ...
    address public constant XMINE = address(0xb30B27aDb3B0A45e88385eB2BB144Fad9120A420); //XMINE
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. XBXs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that XBXs distribution occurs.
        uint256 accXhashPerShare; // Accumulated XBXs per share, times 1e12. See below.
    ...
    // Update the given pool's XBX allocation point. Can only be called by the owner.
    ...
    // View function to see pending XBXs on frontend.
    function pendingXhash(uint256 _pid, address _user) public view returns (uint256) {
        ...
        if (address(pool.lpToken) == address(xblox)) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accXhashPerShare = accXhashPerShare.add(xbloxReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 xbloxReward = multiplier.mul(xbloxPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        xblox.mint(devaddr, xbloxReward.div(10));
        xblox.mint(address(this), xbloxReward);
        pool.accXhashPerShare = pool.accXhashPerShare.add(xbloxReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for XBX allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (pool.depositFeeBP > 0) {
                ...
                pool.amount = pool.amount.add(_amount).sub(depositFeeAmount);
            } else {
    ...
    // Safe xblox transfer function, just in case if rounding error causes pool to not have enough XBXs.
    ...
    // Updates fee addresses.
    function fee(address _feeAddress, address _buyBackAddress) public onlyOwner {
    ...
    // updateEmissionRate
    function updateEmissionRate(uint256 _xbloxPerBlock) public onlyOwner {
        massUpdatePools();
        emit EmissionRateUpdated(msg.sender, xbloxPerBlock, _xbloxPerBlock);
        xbloxPerBlock = _xbloxPerBlock;
    }
}