...
contract HankHillCoin is Context, IERC20, Ownable {
    ...
    address payable private ecosystemWallet = payable(0x98fD650F36765dF04635Be4e6A2a81236e959E06); // Ecosystem Wallet

    address payable private lotteryWallet = payable(0x9d16A9eB8a1D0fc8b70d48eE4EE283f2ca77888E); // Lottery Wallet
    ...
    string private _name = "Hank Hill Coin";

    string private _symbol = "HankHill";



    uint256 public _maxWalletToken = _tTotal.div(1000).mul(10); //1.0% for first few mins



    uint256 public _buyLiquidityFee = 0;   //1%

    uint256 public _buyLotteryFee = 480;     //2% 

    uint256 public _buyMarketingFee = 480;   //2%

    uint256 public _buyReflectionFee = 0;   //0%



    uint256 public _sellLiquidityFee = 10; //2%

    uint256 public _sellMarketingFee = 50;  //5%

    uint256 public _sellLotteryFee = 50;   //5%

    uint256 public _sellReflectionFee = 0; //0%

    

    uint256 private ecosystemFee = 2;   //2% same for buys and sells
    ...
    function setTaxesBuy(uint256 _reflectionFee, uint256 _liquidityFee, uint256 _marketingFee,uint256 _lotteryFee,uint256 _ecosystemFee) external onlyOwner {
        ...
        require(totalFee.add(_buyReflectionFee) <= 990, "Must keep taxes below 99%");
    ...
    function setTaxesSell(uint256 _reflectionFee,uint256 _liquidityFee, uint256 _marketingFee,uint256 _lotteryFee) external onlyOwner {
        ...
        require(_sellLiquidityFee.add(_sellMarketingFee).add(_sellLotteryFee).add(_sellReflectionFee).add(ecosystemFee) <= 990, "Must keep taxes below 99%");
