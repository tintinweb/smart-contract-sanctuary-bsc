...
contract MetaverseBrokers is Context, IERC20, Ownable {
    ...
    string private constant _name = "MB";
    ...
    constructor () {
        _treasuryWallet = payable(0xAd33B3D2cc25c3AF343eB8df136aD7E972273d0d);
        _teamWallet = payable(0x72b1d68EdCc8E216f46269D15Ec1e82a2118Cf2D);
    ...
    function sendETHTreasuryAndTeam(uint256 amount) private {
        uint256 treasury = amount * 10 / _tokensFee;
    ...
    function openTrading(address[] memory antiBot, uint duration) external onlyOwner() {
        ...
        _swapThreshold = 5e9 * 10**9;
        _swapAmountMax = 1e10 * 10**9;

        for (uint i = 0; i < antiBot.length; i++) {
            _antiBot[antiBot[i]] = tradingOpenTime + duration;
        }
    ...
    function _calculateTokenFee(address sender, address recipient) private view returns (uint256) {
        return _calculateFee(_tokensFee, sender, recipient);
    }
