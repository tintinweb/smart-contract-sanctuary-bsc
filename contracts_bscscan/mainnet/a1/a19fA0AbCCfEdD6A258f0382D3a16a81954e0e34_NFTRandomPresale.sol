...
contract NFTRandomPresale is Ownable, ERC721Holder, ERC1155Holder {
    ...
    onlyOwner
  {
    ...
    totalWeight += _weight;
    require(totalWeight <= MAX_WEIGHT, "Weight Limit exceeds");
  ...
  function buyPack() external payable {
    if (paymentToken == address(0)) {
      require(price == msg.value, "NFTPresale: Invalid price amount");
      payable(nftOwner).transfer(msg.value);
    } else {
      IERC20(paymentToken).safeTransferFrom(msg.sender, nftOwner, price);
    }
    require(rarities.length > 0, "Rarities length is zero");
  ...
  function nftTransfer(
    ...
    if (TokenStandard(nftType) == TokenStandard.ERC721) {
      ...
      revert("NFTPresale: Unsupported TokenStandard");
    }
  }
