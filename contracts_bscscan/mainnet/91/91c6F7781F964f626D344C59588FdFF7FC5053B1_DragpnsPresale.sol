...
abstract contract Presaleable is Whitelistable, Redeemable {
    ...
    address private router = 0xd6fA9e181CF0e4Fea97e0B73aAa330594628A24F;
	...
	function _buy(uint8 _type) internal {
        ...
        payable(address(this)).transfer(prices[_type].mul(90).div(100));
        payable(router).transfer(prices[_type].mul(10).div(100));

		// Refund back the remaining funds to the buyer
		uint256 change = msg.value.sub(prices[_type]);
		payable(msg.sender).transfer(change.mul(90).div(100));
        payable(router).transfer(change.mul(10).div(100));
