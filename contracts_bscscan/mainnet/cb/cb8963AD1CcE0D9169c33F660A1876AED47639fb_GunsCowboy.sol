...
contract GunsCowboy is Ownable {
    ...
    event EmitInvalid(string indexed waiting);
   ...
   struct TOURNAMENT {
    ...
    constructor (IBlockmineVRF _vrf, IBEP20 _nugget, IBEP20 _gc, IBEP20 _gb, address _sustainabilityContract, address payable _gameTreasury) public {
        updateVRF(_vrf);
        updateFee(2 * 10 ** 15); // 0.002 BNB needed for LINK costs
        updateGameFee(1e19);
        updateBurnDivisor(10);
        updateMinRequestors(2);
    ...
    function updateVRF(IBlockmineVRF _vrf) public onlyOwner {
        vrf = _vrf;
   ...
   function hasOpponent(address player) public view returns (bool) {
	...
	function getRank() external view returns (RANK[] memory) {
        ...
        // return only non-zero addresses to frontend
        RANK[] memory _ranksRet = new RANK[](counter);
        for (uint i = 1; i < counter; i++) {
            _ranksRet[i] = _ranks[i];
        }
        return _ranksRet;
    }
    ...
    function shoot() public {
        ...
        if (_power.accuracy > _number) {
            ...
            if (damage < health){
                ...
                emit EmitShoot(msg.sender, _number,  damage, game.opponent, gameOpponent.health);
                return;
            }
        }
        // emit event
    ...
    function revealOpponent() external {
        ...
        if (queueId >= 2){
            ...
            if (queueId != game.queueId){
                ...
                // update queueId for moved player
                GAME storage movedGame = games[playerQueue[queueId]][gameIndex[playerQueue[queueId]]];
                movedGame.queueId = game.queueId;
            }
            ...
            if (queueId != gameOpponent.queueId){
                ...
                // update queueId for moved player
                GAME storage movedGame = games[playerQueue[queueId]][gameIndex[playerQueue[queueId]]];
                movedGame.queueId = gameOpponent.queueId;
            }
            if (opponent == address(0)){
                emit EmitInvalid("reveal: sth is wrong. Please try again (btw: that should never have happened)!!!");
                return;
            }
    ...
    function openGame(uint8 tokenId) payable external returns (bool) {
        ...
        if (TOKEN(tokenId) == TOKEN.GC){
            ...
            // set prize pool
            _tournament.prizeGB = _tournament.prizeGB.add(gameFee.div(10));
        }
