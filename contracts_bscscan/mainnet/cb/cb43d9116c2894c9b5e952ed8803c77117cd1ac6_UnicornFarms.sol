...
// UnicornFarms UNIFAR
//Contract made and deployed by Oliver Clarke
//Approved by CEO Kevin Blake
...
contract UnicornFarmsDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("UnicornFarms_Dividend_Tracker", "UnicornFarms_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "UnicornFarms_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "UnicornFarms_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main UnicornFarms contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "UnicornFarms_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "UnicornFarms: Cannot update claimWait to same value");
...
contract UnicornFarms is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    UnicornFarmsDividendTracker public dividendTracker;
    ...
    function setExtraFeeOnSell(uint256 _extraFeeOnSell) public onlyOwner {
        extraFeeOnSell = _extraFeeOnSell; // extra fee on sell for ecosystem
    }
    ...
    constructor() ERC20("UnicornFarms", "UNIFAR") {
        BNBRewardsFee = 5;
        liquidityFee = 3;
        extraFeeOnSell = 1; // extra fee on sell for ecosystem
        marketingFee = 3;
        marketingWallet = payable(0xeF58c7487F33cFd69221d4C80e8171d2F035519E);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new UnicornFarmsDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "UnicornFarms: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "UnicornFarms: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "UnicornFarms: The PanUnicornFarmsSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "UnicornFarms: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "UnicornFarms: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "UnicornFarms: Cannot update gasForProcessing to same value");
