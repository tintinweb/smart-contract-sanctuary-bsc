...
contract DOGEDEAD_Reward_Token is RewardPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() RewardPayingToken("DOGEDEAD_Reward_Tracker", "DOGEDEAD_Reward_Tracker") {
        rewardInterval = 43200;
        minimumTokenBalanceForRewards =  100_000_000 * (10**18);
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "DOGEDEAD_Reward_Tracker: No transfers allowed");
    }

    function withdrawReward() public pure override {
        require(false, "DOGEDEAD_Reward_Tracker: withdraw disabled. Use the 'claim' function on the main DOGEDEAD contract.");
    }
    ...
    function updateRewardInterval(uint256 newRewardInterval) external onlyOwner {
        require(newRewardInterval >= 3600 && newRewardInterval <= 86400, "DOGEDEAD_Reward_Tracker: RewardInterval must be updated to between 1 and 24 hours");	
        require(newRewardInterval != rewardInterval, "DOGEDEAD_Reward_Tracker: Cannot update RewardInterval to same value");
    ...
    function setMinimumTokenBalanceForRewards(uint256 _minimumTokenBalanceWei) public onlyOwner {
        require(_minimumTokenBalanceWei <=  100_000_000 * (10**18), "minimumTokenBalanceForRewards can't be more than initially established");
...
contract DOGEDEAD is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    DOGEDEAD_Reward_Token public rewardTracker;

    uint256 public maxSellTransactionAmount = 10_000_000_000 * (10**18);//1_000_000_000_000_0
    ...
    constructor() ERC20("DogeDead", "DOGEDEAD") {
        BNBRewardsFee = 5; // rewards will be sent to holders in bnb
        buybackFee = 2; //will be sent to buyback address in bnb. 
        extraFeeOnSell = 5; // extra fee on sell to contract address then to reward_token in bnb
        marketingFee = 3; //will be send to marketing address in bnb form. 
        
        marketingWallet = payable(0x64c851F870C3cC03801fBb23d3C1f397A5fF7065);
        buybackWallet = payable(0xA2962b1d33fe118248A3a4565b6479a16bD3faF6);
        
        totalFees = BNBRewardsFee.add(buybackFee).add(marketingFee); // total fees transfer and buy

    	rewardTracker = new DOGEDEAD_Reward_Token();
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DOGEDEAD: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "DOGEDEAD: gasForProcessing must be between 200,000 and 500,000");
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    {
        require(newAmount <= 1_000_000 * (10**18), "swapTokensAtAmount can't to be more than 1.000.000 Tokens");
        require(newAmount >= 1_000 * (10**18), "swapTokensAtAmount can't to be less than 1.000 Tokens");
    ...
    {
        require(newAmount >= 1_000_000_000 * (10**18), "MaxSellTransactionAmount has to be over 0.1% of the total supply");
    ...
    function burnTokens(uint256 burn_amount_wei) public onlyOwner
    {
        super._burn(owner(), burn_amount_wei);
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // swap tokens for ETH
        swapTokensForBnb(contractTokenBalance, address(this)); //   swap when swap+liquify is triggered
        ...
        if(success) {
   	 		emit SendRewards(contractTokenBalance, rewards);
        }
    }
