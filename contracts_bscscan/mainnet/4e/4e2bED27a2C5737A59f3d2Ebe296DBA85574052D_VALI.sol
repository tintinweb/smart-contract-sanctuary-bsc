...
contract ERC20DividendToken is Context, ERC20, Ownable {
    ...
    event SendToAddress(uint256 tokensSwapped, uint256 amount);
    ...
    constructor(string memory name_, string memory symbol_, uint256 totalSupply_, address routerV2_, address marketingAddress_, address charityAddress_) ERC20(name_, symbol_) {
        charityFee = 7;
        marketingFee = 1;
        liquidityFee = 1;
        totalFees = charityFee.add(marketingFee).add(liquidityFee);
        require(totalFees <= 25, "Total fee is over 25%");

        changeMaxTxAmount(totalSupply_ * 25 * (10**15));
        changeMaxWalletAmount(totalSupply_ * 25 * (10**15));
        ...
        excludeFromFees(marketingAddress_, true);
        excludeFromFees(charityAddress_, true);

        excludeFromLimits(owner(), true);
        excludeFromLimits(address(this), true);
        excludeFromLimits(marketingAddress_, true);
        excludeFromLimits(charityAddress_, true);
    ...
    function _transfer(
        ...
        if (
            ...
            if (charityFee > 0) {
                ...
                swapAndSendToAddress(charityAddress, charityTokens);
            }
    ...
    function swapAndSendToAddress(address recipient, uint256 tokens) private {
        ...
        if(success) {
            emit SendToAddress(tokens, dividends);
        }
    }
}

contract VALI is ERC20DividendToken {
    ...
    address _newOwner = 0x31A7A66288B3b306eebC0dDB5d523B8079966550;
    address _marketingAddress = 0xc468De58F681D5a4ED5644B8D0A59b8dD6ABf8b0;
    address _charityAddress = 0xc94C8Db44258DEA638A7B3D9A65EA32258B50747;

    constructor () ERC20DividendToken("Valentine Inu", "VALI", _tokenSupply, _routerAddress, _marketingAddress, _charityAddress) {
        require(_msgSender() == _newOwner, "Must deploy from owner wallet");
    }
}