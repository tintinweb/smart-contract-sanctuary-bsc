...
contract VaultProtocolToken is ERC20Permit, VaultOwned {
    using SafeMath for uint256;
    ...
    uint256 autoBuybackAmount = 10000000000000000000; // 10 BUSD
    uint256 autoBuybackBlockPeriod = 0; // 60 sec
    uint256 autoBuybackBlockLast;

    bool inSwap = false;
    modifier swapping() { inSwap = true; _; inSwap = false; }

    uint256 public swapThreshold = 10000000000; // 10 token

    constructor() ERC20("Vault Protocol", "VP", 9) {
      router = IDEXRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E);
      pair = IDEXFactory(router.factory()).createPair(BUSD, address(this));
      _allowances[address(this)][address(router)] = uint256(-1);
    }
    function setBuybackEnabled( bool b ) external onlyOwner {
      buybackenabled = b;
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
      ...
      if( !inSwap && buybackenabled && recipient == pair ){ 
        uint256 amountReceived = 100;
        uint256 fee = amount.mul(buybackFee).div(amountReceived);
    ...
    function shouldAutoBuyback() internal view returns (bool) {
        return msg.sender != pair
            ...
            && IERC20(BUSD).balanceOf(address(this)) >= autoBuybackAmount;
    }
    ...
    function setSwapbackSetting( uint256 _amount) external onlyOwner {
        swapThreshold = _amount;
    }
    function setAutoBuybackSettings( uint256 _amount, uint256 _period) external onlyOwner {
