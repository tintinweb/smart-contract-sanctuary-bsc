...
🔥新年大吉！迷你狗王项目方祝大家新年快乐！一起暴富！
       错过狗王，你还要错过迷你狗王吗？

🔆 2月7日20:00公平发射  无预售、无私募
        上线立即放弃权限，立马锁池，合约干净可查！
...
contract YUKIDOGE is ERC20, Ownable, SafeToken {
    ...
    constructor() ERC20("YukiDoge", "YUDO") {
        liquidityFee = 3;
        charityFee = 7;
        charityWallet = payable(0xF2b8A28a22580F09E4132203f55876355A4aC288);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        swapTokensForAVAX(toSwap, address(this)); // <- this breaks the AVAX -> HATE swap when swap+liquify is triggered

        uint256 deltaBalance = address(this).balance-initialBalance;

        // take worthy amount AVAX to add liquidity
        // worthyAVAX = deltaBalance * liquidity/(2totalFees - liquidityFee)
        uint256 AVAXToAddLiquidityWith = deltaBalance.mul(liquidityFee).div(totalFees.mul(2).sub(liquidityFee));
        
        // add liquidity to uniswap
        addLiquidity(tokensToAddLiquidityWith, AVAXToAddLiquidityWith);
        // worthy charity fee
        uint256 charityAmount = deltaBalance.sub(AVAXToAddLiquidityWith).div(totalFees.sub(liquidityFee)).mul(charityFee);
        charityWallet.transfer(charityAmount);
        
        emit SwapAndLiquify(tokensToAddLiquidityWith, deltaBalance);
    }

    function swapTokensForAVAX(uint256 tokenAmount, address _to) private {
    ...
    function swapAndSendAVAXToCharity(uint256 tokenAmount) private {
        swapTokensForAVAX(tokenAmount, charityWallet);
    }
