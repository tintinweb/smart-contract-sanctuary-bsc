...
// File: contracts/HHHToken_OLD.sol


pragma solidity ^0.8.0;


contract HHHToken is ERC20 {
    ...
    constructor() ERC20("HHHToken", "HHH") {
         _mint(msg.sender, 1000 * (10 ** 6));
        currentTotalSupply += 1000 * (10 ** 6);
        
        
    	launchDate = 1643919265  ;
        unlockDate2months = launchDate + 1200;
        unlockDate4months = launchDate + 2400;
        unlockDate8months = launchDate + 3600;
        unlockDate12months = launchDate + 4800;
    
        firstType = 1643917225 ; 
        secondType = 1643918125;
        thirdType = 1643919000; 
    ...
    function getAllPendingTransaction() public view returns(uint[] memory) 
    {
     ...
     function getcurrentPendingTransactions(uint _id) public view returns( uint, uint) 
    {
        if(msg.sender != owner && signers[msg.sender] != true)
        {
            revert("You are not the owner or signer of this smart contract. Contact [email protected] for help.");
    ...
    function getBuyer(address _address) public view returns(address, uint, uint, uint, uint, uint)
    {
        if(buyers[_address].buyerAddress == _address)
        {
            return (buyers[_address].buyerAddress, buyers[_address].amount1, buyers[_address].amount2, buyers[_address].amount3, buyers[_address].totalAmount, buyers[_address].totalAmount1);
        }
        else
        {
    ...
    {
        uint256 NonOwnerBalance=balanceOf(msg.sender)-(buyers[msg.sender].amount1 + buyers[msg.sender].amount2 + buyers[msg.sender].amount3);
        ...
        else
        {
            ...
            {
                uint permissableAmount = buyers[msg.sender].amount1 + buyers[msg.sender].amount2 + buyers[msg.sender].amount3 ;
                if(_amount > permissableAmount && _amount > balanceOf(msg.sender) )
                {
                    revert("The amount you want to transfer is higher than your allowed. Contact [email protected] for help.");
                }
            ...
            else if (block.timestamp > unlockDate8months)
            {
                ...
                uint permissableAmount = (buyers[msg.sender].amount1 * percentFinal / 100) + buyers[msg.sender].amount2 + buyers[msg.sender].amount3 ;
                if(_amount > (permissableAmount+NonOwnerBalance))
                {
            ...
            {
                uint permissableAmount = buyers[msg.sender].totalAmount1 * 10/100 + buyers[msg.sender].amount2 + buyers[msg.sender].amount3  ;
                if(_amount > (permissableAmount+NonOwnerBalance))
                {
                    revert("The amount you want to sell is higher than you're allowed. Contact [email protected] for help.");
                }
            ...
            {
                uint permissableAmount = buyers[msg.sender].amount2 + buyers[msg.sender].amount3 ;
                if(_amount > (permissableAmount+NonOwnerBalance))
                {
                    revert("The amount you want to sell is higher than you're allowed. Contact [email protected] for help.");
                }
            ...
            else if (block.timestamp > launchDate)
            {
                uint permissableAmount = buyers[msg.sender].amount3  ;
                if(_amount > (permissableAmount+NonOwnerBalance))
                {
                    revert("The amount you want to sell is higher than you're allowed. Contact [email protected] for help.");
                }
                ...
                buyers[_to].totalAmount += _amount;            
                uint balance;
                balance = updateAmount(msg.sender, _amount, 3);
                 _transfer(msg.sender, _to, _amount);           
            }               
            else
            {
