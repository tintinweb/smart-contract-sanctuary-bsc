...
contract SaveWithEscrow is Context, ERC20, Ownable {
    using SafeMath for uint256;
   ...
   SaveWithEscrowDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("ETHCROW", "ETHCROW") {
        _buyRewardsFee = 12;
        _buyLiquidityFee = 3;
		_buyMarketingFee = 6;
        _buyDevFee = 1;
    	...
    	dividendTracker = new SaveWithEscrowDividendTracker();

    	liquidityWallet = owner();
		
		marketingWallet = payable(0xb8bF1Def9D5E7a18128B80D21F47599A08602A66);
        _devWallet = payable(0x913289C5d8D4775ed8Fef7d2d202965e6Ce06B9b);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "ETHCROW: The dividend tracker already has that address");

        SaveWithEscrowDividendTracker newDividendTracker = SaveWithEscrowDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "ETHCROW: The new dividend tracker must be owned by the ETHCROW token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "ETHCROW: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "ETHCROW: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ETHCROW: Account is already the value of 'excluded'");
    ...
    function addToBlacklist(address account, bool blacklisted) public onlyOwner {
        require(_isBlacklisted[account] != blacklisted, "ETHCROW: Account is already the value of 'blacklisted'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ETHCROW: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ETHCROW: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "ETHCROW: The liquidity wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "ETHCROW: The marketing wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ETHCROW: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ETHCROW: Cannot update gasForProcessing to same value");
...
contract SaveWithEscrowDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function withdrawDividend() public pure override {
        require(false, "Escrow_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ETHCROW contract.");
    }
