...
contract TestS is ERC20, Ownable {
	IUniswapV2Router02 public uniswapV2Router;
	address public immutable uniswapV2Pair;

	string private constant _name = "TestS"; //"SNAFU";
	string private constant _symbol = "TestS"; //"SeTC";
	...
	function blockAccount(address account) public onlyOwner {
		...
		if (_isLaunched) {
			require((currentTimestamp - _launchStartTimestamp) < _blockedTimeLimit, "SNAFU: Time to block accounts has expired");
		}
	...
	function setStakingAddress(address newStakingAddress) public onlyOwner {
		require(newStakingAddress != address(0), "SNAFU: The stakingAddress cannot be 0");
	...
	function setDividendToken(address newDividendToken) external onlyOwner {
		require(newDividendToken != dividendToken, "SNAFU: Cannot update dividend token to same value");
		require(newDividendToken != address(0), "SNAFU: The dividend token cannot be 0");
		require(newDividendToken != address(this), "SNAFU: The dividend token cannot be set to the current contract");
		...
		uint256 amount
			...
			if(from != owner() && to != owner()) {
				...
				if (_isInLaunch && (currentTimestamp - _launchStartTimestamp) <= 300 && isBuyFromLp) {
					require((currentTimestamp - _buyTimesInLaunch[to]) > 60, "SNAFU: Cannot buy more than once per min in first 5min of launch");
				}
				if (!_isExcludedFromMaxTransactionLimit[to] && !_isExcludedFromMaxTransactionLimit[from]) {
					require(amount <= maxTxAmount, "SNAFU: Buy amount exceeds the maxTxBuyAmount.");
				}
				if (!_isExcludedFromMaxWalletLimit[to]) {
					require((balanceOf(to) + amount) <= maxWalletAmount, "SNAFU: Expected wallet amount exceeds the maxWalletAmount.");
				}
			}
	...
	function _adjustTaxes(bool isBuyFromLp, bool isSelltoLp, bool isInLaunch) private {
	    uint256 blocksSinceLaunch = block.number - _launchBlockNumber;
	    uint256 currentTimestamp = !isTradingEnabled && _tradingPausedTimestamp > _launchStartTimestamp  ? _tradingPausedTimestamp : _getNow();
		uint256 timeSinceLaunch = currentTimestamp - _launchStartTimestamp;
		uint256 timeSinceGolden = currentTimestamp - _goldenHourStartTimestamp;
