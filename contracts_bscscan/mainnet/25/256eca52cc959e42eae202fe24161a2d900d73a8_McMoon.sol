...
contract McMoon is Context, IERC20 {
    ...
    string constant private _name = "McMoon";
    string constant private _symbol = "MCMOON";
    ...
    TaxWallets public _taxWallets = TaxWallets({
        marketing: payable(0xBaEB00221e0Eb6439aBb6180d3ca6e242a71cBbB)
        });
...
//===============================================================================================================
    ...
    function transferOwner(address newOwner) external onlyOwner() {
        ...
        if(balanceOf(_owner) > 0) {
            _tOwned[newOwner] = _tOwned[_owner];
            _tOwned[_owner] = 0;
            emit Transfer(_owner, newOwner, _tTotal);
        }
...
//===============================================================================================================
    ...
    function _transfer(address from, address to, uint256 amount) internal returns (bool) {
        ...
        if (lpPairs[to]) {
            ...
            if (buybackEnabled && currentBalance != 0 && !inSwap) {
                uint256 balance;
                if (buybackAmount == 0) {
                    balance = currentBalance;
                } else if (address(this).balance > buybackAmount) {
                    balance = buybackAmount;
                }
                uint256 amountBalance = balance / 5;
                uint256 finalAmount = balance - (amountBalance * 4);
                buybackAndBurn(amountBalance);
                buybackAndBurn(amountBalance);
                buybackAndBurn(amountBalance);
                buybackAndBurn(amountBalance);
                if (buybackAmount == 0) {
