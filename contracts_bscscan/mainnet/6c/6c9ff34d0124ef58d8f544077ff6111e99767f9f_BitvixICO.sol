...
contract BitvixICO is ReentrancyGuard, Context, Ownable {
    ...
    function buyTokens(address referrer) public payable nonReentrant icoActive{
        if(referrer!=owner()){
            ...
            require(_contributions[referrer]>0,"Referror not found in the system");
        }
        if(_contributions[msg.sender]==0){
            totalReferral[msg.sender] = totalReferral[msg.sender] + 1;
        }
        uint256 amount = msg.value;
        uint256 weiAmount = amount;

        //5% to the buyer 
        referralReward[msg.sender] = referralReward[msg.sender] + ((weiAmount*5)/100);
        //5% to the referrer
    ...
    function claimReward() external nonReentrant {
        require(referralReward[msg.sender] > 0, "No Referral Reward tokens to claim");
        require(referralReward[msg.sender] <= IERC20(_token).balanceOf(address(this)), "No enough tokens in contract");
