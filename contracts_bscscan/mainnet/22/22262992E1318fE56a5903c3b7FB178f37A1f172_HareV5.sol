...
contract HareV5 is ContextUpgradeable, IBEP20, OwnableUpgradeable {
    ...
    function initialize(
        ...
        maxWalletAmount = TOTAL_COIN * 10**uint8(_decimals);
        ...
        inSwap = false;
        isSwapEnabled = false;
        _maxTxAmount = TOTAL_COIN * 10**uint8(_decimals);
    ...
    function sendETHToFee(uint256 amount) private {
        uint256 _tempTotalFee = _marketingFee + _developmentFee;

        _developmentWalletAddress.transfer(
            amount.mul(_tempTotalFee).div(_companyTotalFees)
        );
