...
contract LuzToken is IBEP20, LockToken
{
    using SafeMath for uint256;
    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;
    uint256 public _totalSupply = 20_000_000 * 10**18;
    string public _name = "Luz Token";
    string public _symbol = "Luz";
    ...
    uint256 public _walletMaxHoldingLimit =  _totalSupply.div(10000).mul(3); //0.03%
    ...
    function excludeWalletsFromWhales() private 
    {
        ...
        _excludedFromWhale[address(0)]=true;
    }
    ...
    private view
    {
        ...
        if(from==poolPair && !_excludedFromWhale[to]) 
        { 
            require(newBalance <= _walletMaxHoldingLimit, "Exceeding max tokens limit in the wallet"); 
        } 
    }
    ...
    function  setExcludeFromWhale(address _account, bool _value) public onlyOwner 
    {
        _excludedFromWhale[_account] = _value; // true for excluded, false for not excluded. 
    }
    ...
    uint256 public coolDownSpan = 120;

    function enableCooldown(bool _enable) external onlyOwner 
    {
        cooldownEnabled = _enable;
    }

    function checkForCoolDown(address from, address to) private
    {
        if(cooldownEnabled)
        {
            if(!_excludedFromWhale[from] && !_excludedFromWhale[to] && from != poolPair && to != poolPair)
            {
                uint256 coolDownTimestamp = _lastTxnTimeStamp[from].add(coolDownSpan);
                require(coolDownTimestamp < block.timestamp, "Cool Down Wait");
                _lastTxnTimeStamp[from] = block.timestamp;
            }

            if((from == poolPair && !_excludedFromWhale[to]) ||  (to == poolPair && !_excludedFromWhale[from]))
            {
                uint256 coolDownTimestamp = _lastTxnTimeStamp[from].add(coolDownSpan);
                require(coolDownTimestamp < block.timestamp, "Cool Down Wait");
                _lastTxnTimeStamp[from] = block.timestamp;
    ...
    function setCoolDownSpan(uint256 _seconds) external onlyOwner 
    {
        coolDownSpan = _seconds;
    }
