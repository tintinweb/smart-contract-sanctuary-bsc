...
contract SevenCoinToken is Authorized, ERC20 {
  ...
  uint256 public feeMarketingWallet = 300; // 3%
  uint256 public feeOperationWallet = 200; // 2%
  ...
  function getFeeTotal() public view returns(uint256) { return feeAdministrationWallet + feeMarketingWallet + feeOperationWallet + feePool; }
  ...
  function setFeeMarketingWallet(uint256 feeAmount) external isAuthorized(1) { feeMarketingWallet = feeAmount; }
  function setFeeOperationWallet(uint256 feeAmount) external isAuthorized(1) { feeOperationWallet = feeAmount; }
  ...
  function setOperationWallet(address account) public isAuthorized(0) { operationWallet = account; }
  ...
  constructor()ERC20(_name, _symbol) {
    ...
    administrationWallet = 0x23F645f859A9abe3cF894131D1d5e4A6fF791eF4;
    marketingWallet = 0x0918e8DF56f555169aadbDB11a38dA02bd0A2a9f;
    operationWallet = 0x235c26fBd77a839cB67f09715f0Bb61c3cE7Dd65;
    ...
    exemptTxLimit[operationWallet] = true;
    exemptAmountLimit[operationWallet] = true;
  ...
  function exchangeFeeParts(uint256 incomingFeeTokenAmount) private returns (bool){
    ...
    // Deep Stack problem avoid
    {
      ...
      if (feeMarketingWallet > 0) tokenTransfer(busdAddress, marketingWallet, (busdAmount * feeMarketingWallet) / totalFee);
      if (feeOperationWallet > 0) tokenTransfer(busdAddress, operationWallet, (busdAmount * feeOperationWallet) / totalFee);
    }
    return true;
  }
