...
contract FPAssetsNFTURI {
	...
	//--Get token URI uint--//
    function GettokenURI(uint _typeID, uint _rarityID) public view returns(string memory){
        string memory _typeIDSTR = uint2str(_typeID);
        return GettokenSTRURI(_typeIDSTR, _rarityID);
    }

	//--Get token URI string--//
    function GettokenSTRURI(string memory _typeIDSTR, uint _rarityID) public view returns(string memory){
        string memory preURI = strConcat(tokenURI[_rarityID], _typeIDSTR);
