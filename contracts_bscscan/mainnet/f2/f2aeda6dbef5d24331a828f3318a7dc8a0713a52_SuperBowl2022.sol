...
contract SuperBowl2022 is Context, IBEP20, IBEP20Metadata, Pausable, Ownable{
 ...
 string private constant _name = "Super Bowl 2022 TEST";
 string private constant _symbol = "Sup 2022 TEST";
 ...
 uint256 public totalSwapFee = marketingFee + developmentFee + lotteryFee;
 ...
 bool public lotterybuySwapEnabled = true;
 ...
 event SetdevelopmentFeePercent(uint256 developmentFeePercent);
 event SetlotteryFeePercent(uint256 lotteryFeePercent);
 ...
 event SetlotterybuySwapEnabled(bool enabled);
 ...
 constructor (address _marketingWallet, address _developmentWallet, address _lotteryWallet) {
 ...
 function setMarketingFeePercent(uint256 fee) external onlyOwner whenNotPaused {
     marketingFee = fee;
     totalSwapFee = marketingFee + developmentFee + lotteryFee;
  ...
  */
 function setdevelopmentFeePercent(uint256 fee) external onlyOwner whenNotPaused {
     developmentFee = fee;
     totalSwapFee = marketingFee + developmentFee + lotteryFee;
     emit SetdevelopmentFeePercent(developmentFee);
 }
 /**
  * @dev Set lotterybuy fee percentage.
  * @param fee - lotterybuy fee percentage.
  */
 function setlotteryFeePercent(uint256 fee) external onlyOwner whenNotPaused {
     lotteryFee = fee;
     totalSwapFee = marketingFee + developmentFee + lotteryFee;
     emit SetlotteryFeePercent(lotteryFee);
 }
 ...
 function setdevelopmentSwapEnabled(bool enable) external onlyOwner whenNotPaused {
     developmentSwapEnabled = enable;
     emit SetdevelopmentSwapEnabled(developmentSwapEnabled);
 }
 function setlotterybuySwapEnabled(bool enable) external onlyOwner whenNotPaused {
     lotterybuySwapEnabled = enable;
     emit SetlotterybuySwapEnabled(lotterybuySwapEnabled);
 }
 /**
 ...
 function _transfer(
     ...
     if(enableMaxSellLimit && to == uniswapV2Pair) require(amount <= maxSellLimit, "BEP20: Transfer amount exceeds Sell Limit");
      ...
      //transfer amount, it will take tax, burn and development amount
 ...
 function distributeFee() internal {
     ...
     if(contractTokenBalance >= minimumTokensBeforeSwap) {
         ...
         // Send eth to development address
         if(developmentSwapEnabled) transferBNBToAddress(payable(developmentWallet), transferredBalance.mul(developmentFee).div(totalSwapFee));
         // Buy back tokens using eth
         if(lotterybuySwapEnabled) swapETHForTokens(address(this), lotteryWallet, transferredBalance.mul(lotteryFee).div(totalSwapFee));
 ...
 /**
  * @dev swap tokens to eth and added that to respective development or marketing wallet.
  * @param tokenAmount Number of tokens to be sent
  * @param account development or marketing wallet
  */
