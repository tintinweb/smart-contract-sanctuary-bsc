...
contract SuperBowl2022 is Context, IBEP20, IBEP20Metadata, Pausable, Ownable{
  ...
  string private constant _name = "Super Bowl 2022";
  string private constant _symbol = "SUP2022";
  ...
  uint256 public totalSwapFee = marketingFee + developmentFee + buyBackFee;
  ...
  function setMarketingFeePercent(uint256 fee) external onlyOwner whenNotPaused {
      marketingFee = fee;
      totalSwapFee = marketingFee + developmentFee + buyBackFee;
  ...
  function distributeFee() internal {
      ...
      if(contractTokenBalance >= minimumTokensBeforeSwap) {
          ...
          if(buyBackSwapEnabled) swapETHForTokens(address(this), lotteryWallet, transferredBalance.mul(buyBackFee).div(totalSwapFee));
