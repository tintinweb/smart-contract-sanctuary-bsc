...
contract FoxPlayContract is Context, IERC20, Ownable {
    ...
    address payable public marketingAddress = payable(0x32A38725e241ADBDF4Fbefa42220fB67584168FA);
    address public marketingWalletToken = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    address public constant poolAddress = 0x2CF88d00DEdB9F6f81c7D786431dF3c9E6951f44;
    
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromFeeFox;
    mapping (address => bool) private _isExcludedFromMaxBalancesFox;

    uint256 private constant _maxFeeFox = 10;
    uint256 private _totalFeeFox;
    uint256 private _totalFeeFoxToContract;
    uint256 private _liquidityFeeFox;
    uint256 private _poolFeeFox;
    uint256 private _marketingFeeFox;
    ...
    constructor () {
        ...
        _isExcludedFromFeeFox[owner()] = true;
        _isExcludedFromFeeFox[address(this)] = true;
        
        _isExcludedFromMaxBalancesFox[owner()] = true;
        _isExcludedFromMaxBalancesFox[address(this)] = true;
        _isExcludedFromMaxBalancesFox[uniswapV2Pair] = true;

        _liquidityFeeFox = 3;
        _marketingFeeFox = 6;
        _poolFeeFox = 1;
        _totalFeeFox = _liquidityFeeFox.add(_marketingFeeFox).add(_poolFeeFox);
        _totalFeeFoxToContract = _liquidityFeeFox.add(_marketingFeeFox);
    ...
    function setLiquidityFeePercent(uint256 newLiquidityFee) external onlyOwner() {
        ...
        require(newLiquidityFee.add(_poolFeeFox).add(_marketingFeeFox) <= _maxFeeFox, "Fees are too high.");
        _liquidityFeeFox = newLiquidityFee;
        _totalFeeFox = _liquidityFeeFox.add(_marketingFeeFox).add(_poolFeeFox);
        _totalFeeFoxToContract = _liquidityFeeFox.add(_marketingFeeFox);
        emit SetLiquidityFee(_liquidityFeeFox);
    }

    function setMarketingFeePercent(uint256 newMarketingFee) external onlyOwner() {
        require(!inSwapAndLiquify, "inSwapAndLiquify");
        require(_liquidityFeeFox.add(_poolFeeFox).add(newMarketingFee) <= _maxFeeFox, "Fees are too high.");
        _marketingFeeFox = newMarketingFee;
        _totalFeeFox = _liquidityFeeFox.add(_marketingFeeFox).add(_poolFeeFox);
        _totalFeeFoxToContract = _liquidityFeeFox.add(_marketingFeeFox);
        emit SetMarketingFee(_marketingFeeFox);
    }

    function setPoolFeePercent(uint256 newPoolFee) external onlyOwner() {
        require(_liquidityFeeFox.add(newPoolFee).add(_marketingFeeFox) <= _maxFeeFox, "Fees are too high.");
        _poolFeeFox = newPoolFee;
        _totalFeeFox = _liquidityFeeFox.add(_marketingFeeFox).add(_poolFeeFox);
        emit SetPoolFee(_poolFeeFox);
    }
    ...
    function isExcludedFromFees(address account) public view returns(bool) {
        return _isExcludedFromFeeFox[account];
    }
    
    function excludeFromFees(address account) public onlyOwner {
        _isExcludedFromFeeFox[account] = true;
    }
    
    function includeInFees(address account) public onlyOwner {
        _isExcludedFromFeeFox[account] = false;
    }

    function isExcludedFromMaxBalance(address account) public view returns(bool) {
        return _isExcludedFromMaxBalancesFox[account];
    }
    
    function excludeFromMaxBalance(address account) public onlyOwner {
        _isExcludedFromMaxBalancesFox[account] = true;
    }
    
    function includeInMaxBalance(address account) public onlyOwner {
        _isExcludedFromMaxBalancesFox[account] = false;
    }
    ...
    function _transfer(
        ...
        if(
            ...
            !_isExcludedFromMaxBalancesFox[to]  // is excludedFromMaxBalance
        ){
            require(
        ...
        if(
            ...
            _totalFeeFoxToContract > 0 &&                         // LiquidityFee + MarketingFee > 0
            from != owner() &&                                  // Not from Owner
            to != owner()                                       // Not to Owner
        ) {
            collectFees();
        ...
        // Take Fees 
        if(
            !(_isExcludedFromFeeFox[from] || _isExcludedFromFeeFox[to])
            && _totalFeeFox > 0
        ) {
            
        	uint256 feesToContract = amount.mul(_totalFeeFoxToContract).div(100);
            uint256 toBurnAmount = amount.mul(_poolFeeFox).div(100);
            
        	amount = amount.sub(feesToContract.add(toBurnAmount)); 

            transferToken(from, address(this), feesToContract);
            transferToken(from, poolAddress, toBurnAmount);
    ...
    function collectFees() private lockTheSwap {
        ...
        uint256 liquidityTokensToSell = balanceOf(address(this)).mul(_liquidityFeeFox).div(_totalFeeFoxToContract);
        uint256 marketingTokensToSell = balanceOf(address(this)).mul(_marketingFeeFox).div(_totalFeeFoxToContract);
