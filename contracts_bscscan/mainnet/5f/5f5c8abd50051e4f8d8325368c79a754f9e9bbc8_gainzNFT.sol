...
contract gainzNFT is ERC721Enumerable, Ownable {
    ...
    function mint(uint256 _amount) internal {
        ...
        require(totalSupply() + _amount <= 2000, "exceeds max supply");
    ...
    function airdropMint(address [] memory _addresses) external onlyOwner {
        uint256 size = _addresses.length;
        require(totalSupply() + size <= MAX_SUPPLY, "exceeds max supply");
        require(airdropped + size <= 100, "exceeds airdrop supply");       

        uint256 _tokenId = 2001 + airdropped;
    ...
    function changeValues(uint256 _price, uint256 _walletLimit) public onlyOwner {
        price = _price;
