...
contract Trade {
    ...
    event RoyaltyUpdated(address indexed nftAddress, uint256 indexed tokenId, address indexed royaltyReciever, uint256 royaltyFee);
    ...
    mapping(address => mapping(uint256 => uint256)) private royaltyFeeForImport;
    mapping(address => mapping(uint256 => address)) private royaltyRecieverForImport;
    struct Fee {
        uint platformFee;
        uint assetFee;
        uint royaltyFee;
    ...
    function setSigner(address newSigner) public onlySigner returns(bool){
    ...
    function verifySignerSign(address nftAddress, uint256 tokenId, address royaltyAddress, uint256 royaltyFee, Sign memory sign) internal view {
        bytes32 hash = keccak256(abi.encodePacked(nftAddress, tokenId, royaltyAddress, royaltyFee, sign.nonce));
        require(signer == getSigner(hash, sign), "signer sign verification failed");
    }
    ...
    function getFees(Order memory order) internal view returns(Fee memory){
        ...
        if(order.nftType == BuyingAssetType.ERC721 && !order.isImport) {
        ...
        if(order.nftType == BuyingAssetType.ERC1155 && !order.isImport)  {
        ...
        if(order.isImport) {
            royaltyPermille = royaltyFeeForImport[order.nftAddress][order.tokenId];
            tokenCreator = royaltyRecieverForImport[order.nftAddress][order.tokenId];
        }
    ...
    function setRoyalty(address nftAddress, uint256 tokenId, address royaltyReciever, uint256 royaltyFee, Sign memory sign) public returns(bool) {
        require(nftAddress != address(0),"Asset Address in zero");
        require(royaltyReciever != address(0) && royaltyFee != 0 ,"royalty Receiever should not be zero or royaltyFee should not be zero ");
        verifySignerSign(nftAddress, tokenId, royaltyReciever, royaltyFee, sign);
        royaltyFeeForImport[nftAddress][tokenId] = royaltyFee;
        royaltyRecieverForImport[nftAddress][tokenId] = royaltyReciever;
        emit RoyaltyUpdated(nftAddress, tokenId, royaltyReciever, royaltyFee);
        return true;
    }
}