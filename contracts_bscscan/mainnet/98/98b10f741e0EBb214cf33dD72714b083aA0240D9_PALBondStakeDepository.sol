...
contract PALBondStakeDepository is Ownable {
    ...
    address public immutable PAL; // intermediate reward token from treasury
    address public immutable sPAL; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints PAL when receives principle
    ...
    struct Bond {
        uint gonsPayout; // sPAL gons remaining to be paid
        uint PALPayout; //PAL amount at the moment of bond
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 PAL ( underflow protection )
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) PAL
         */
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            IERC20( PAL ).safeTransfer( DAO, fee ); 
        }
    ...
    function redeem( address _recipient, bool _stake) external returns ( uint ) {        
        ...
        uint _amount = ISPAL(sPAL).balanceForGons(info.gonsPayout);
        emit BondRedeemed( _recipient, _amount, 0 ); // emit bond data
        IERC20( sPAL ).transfer( _recipient, _amount ); // pay user everything due
        return _amount;
    }
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( PAL ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    /**
     *  @notice return bond info with latest sPAL balance calculated from gons
     *  @param _depositor address
     *  @return payout uint
     *  @return vesting uint
    ...
    function bondInfo(address _depositor) public view returns ( uint payout,uint vesting,uint lastBlock,uint pricePaid ) {
        ...
        payout=ISPAL(sPAL).balanceForGons(info.gonsPayout);
    ...
    /**
     *  @notice calculate current ratio of debt to PAL supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = IERC20( PAL ).totalSupply();
    ...
    /**
     *  @notice calculate amount of PAL available for claim by depositor
     *  @param _depositor address
     *  @return pendingPayout_ uint
     */
    function pendingPayoutFor( address _depositor ) external view returns ( uint pendingPayout_ ) {
        uint percentVested = percentVestedFor( _depositor );
        uint payout = ISPAL(sPAL).balanceForGons(_bondInfo[ _depositor ].gonsPayout);
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or PAL) to the DAO
     *  @return bool
     */
