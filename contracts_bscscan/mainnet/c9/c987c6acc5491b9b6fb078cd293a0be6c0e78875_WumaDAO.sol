...
contract WumaDAO is IERC20, Ownable {
    ...
    constructor(address tokenOwner) {
        Deeper = IERC20(0xA0A2eE912CAF7921eaAbC866c6ef6FEc8f7E90A4);
        _name = "WumaDAO";
        _symbol = "WumaDAO";
        _decimals = 18;
        _tTotal = 7557 * 10**_decimals;
        uint256 leftAmount = 1000 * 10**_decimals;
    ...
    function _splitOtherToken() public {
        ...
        if(thisAmount >= 2 * 10**17){
			if(thisAmount > 2 * 10**18){
				thisAmount = 2 * 10**18;
			}
            uint256 buySize = buyUser.length;
            if(buySize>0 && Deeper.balanceOf(address(this)) > thisAmount.mul(10)){
                address user;
                uint256 startIndex;
                uint256 totalAmount;
                if(buySize >10){
                ...
                if(buySize >10){
                    for(uint256 i=0;i<10;i++){
                        ...
                        if(rate>0){
                            Deeper.transfer(user,thisAmount.mul(rate).div(1000));
                        }
                    }
                }else{
                    for(uint256 i=0;i<buySize;i++){
    ...
    function getInfo() public view returns (uint256[] memory) {
        uint256 thisAmount = balanceOf(address(this));
        uint256[] memory listr = new uint256[](2);
        listr[0] = thisAmount;
        if(thisAmount >= 10**18){
            require(Deeper.balanceOf(address(this)) > thisAmount.mul(10),"token not enough");
            uint256 buySize = buyUser.length;
            listr[1] = buySize;
            if(buySize>0){
                address user;
                ...
                if(buySize >10){
                    ...
                    uint256[] memory list = new uint256[](buySize);
                    for(uint256 i=0;i<buySize;i++){
                        user = buyUser[i];
                        rate = balanceOf(user).mul(10000).div(totalAmount);
