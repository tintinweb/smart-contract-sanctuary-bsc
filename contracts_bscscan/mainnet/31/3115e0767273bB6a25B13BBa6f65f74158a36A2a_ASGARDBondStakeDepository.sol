...
contract ASGARDBondStakeDepository is Ownable {
    ...
    address public immutable ASGARD; // intermediate reward token from treasury
    address public immutable sASGARD; // token given as payment for bond
    ...
    struct Bond {
        uint gonsPayout; // sASGARD gons remaining to be paid
        uint ASGARDPayout; //ASGARD amount at the moment of bond
    ...
    function deposit( 
        ...
        IERC20( ASGARD ).approve( staking, payout );
        IStaking( staking ).stake( payout, address(this) );
        IStaking( staking ).claim( address(this) );
        uint stakeGons=ISASGARD(sASGARD).gonsForBalance(payout);

        // depositor info is stored
        _bondInfo[ _depositor ] = Bond({ 
            gonsPayout: _bondInfo[ _depositor ].gonsPayout.add( stakeGons ),
            ASGARDPayout: _bondInfo[ _depositor ].ASGARDPayout.add( payout ),
    ...
    function redeem( address _recipient, bool _stake) external returns ( uint ) {        
        ...
        uint _amount = ISASGARD(sASGARD).balanceForGons(info.gonsPayout);
        emit BondRedeemed( _recipient, _amount, 0 ); // emit bond data
        IERC20( sASGARD ).transfer( _recipient, _amount ); // pay user everything due
        return _amount;
    }
    ...
    /**
     *  @notice return bond info with latest sASGARD balance calculated from gons
     *  @param _depositor address
     *  @return payout uint
     *  @return vesting uint
    ...
    function bondInfo(address _depositor) public view returns ( uint payout,uint vesting,uint lastBlock,uint pricePaid ) {
        ...
        payout=ISASGARD(sASGARD).balanceForGons(info.gonsPayout);
    ...
    function pendingPayoutFor( address _depositor ) external view returns ( uint pendingPayout_ ) {
        ...
        uint payout = ISASGARD(sASGARD).balanceForGons(_bondInfo[ _depositor ].gonsPayout);
