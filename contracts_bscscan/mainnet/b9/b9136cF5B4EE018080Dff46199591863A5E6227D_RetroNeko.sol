...
interface ILiquidityLocker {
    function lock(address owner, address token, bool isLpToken, uint256 amount, uint256 unlockDate) external payable returns (uint256 id);
    function unlock(uint256 lockId) external;
    function editLock(uint256 lockId, uint256 newAmount, uint256 newUnlockDate) external payable;
}
...
contract RetroNeko is Initializable, ERC20Upgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function _swapSendFeesAndLiquify() private swapping {
        ...
        uint256 liquidityEthAmount = ethAmount - teamEthAmount - marketingEthAmount - warEthBalance;
...
contract RetroNeko is Initializable, ERC20Upgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if (sender == presaleAddress) {
            uint256 totalFeesPerTenThousand = 1000; // 10%
        	uint256 feesAmount = amount * totalFeesPerTenThousand / 10_000;
        	amount -= feesAmount;
...
contract RetroNeko is Initializable, ERC20Upgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        require(_msgSender() == owner() || _tradingEnabled, "RetroNeko: Trading is not enabled");
        require(sender != address(0), "RetroNeko: Transfer from the zero address");
        require(recipient != address(0), "RetroNeko: Transfer to the zero address");
        require(amount > 0, "RetroNeko: Transfer zero token");
        require(!_isBlacklisted[sender] && !_isBlacklisted[recipient], 'RetroNeko: Blacklisted address');
...
contract Staking is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function withdrawSilver(uint8 stakeIndex) external {
        ...
        require(block.timestamp >= stake.since + _silverLockPeriod, "Staking: Silver slot is locked for 90 days");

        _withdrawPack(stakeIndex);
    }
    
    function withdrawGold(uint8 stakeIndex) external {
        require(stakeIndex < 3, "Staking: Gold has only 3 slots");
        Stake memory stake = _stakeholders[_stakeholderIndexes[msg.sender]].stakes[stakeIndex];
        require(block.timestamp >= stake.since + _silverLockPeriod, "Staking: Gold slot is locked for 180 days");

        _withdrawPack(3 + stakeIndex);
    }
    
    function withdrawDiamond(uint8 stakeIndex) external {
        require(stakeIndex < 3, "Staking: Diamond has only 3 slots");
        Stake memory stake = _stakeholders[_stakeholderIndexes[msg.sender]].stakes[stakeIndex];
        require(block.timestamp >= stake.since + _silverLockPeriod, "Staking: Diamond slot is locked for 270 days");

        _withdrawPack(6 + stakeIndex);
    }
    ...
    function _withdrawPack(uint8 stakeIndex) private {
        require(stakeIndex < 9, "Staking: There is only 9 slots (3 of each)");
        Stake memory stake = _stakeholders[_stakeholderIndexes[msg.sender]].stakes[stakeIndex];
        require(stake.amount > 0, "Staking: Slot is empty");

        if (block.timestamp >= stake.since + _silverLockPeriod) {
            uint256 amountWithRewards = _withdraw(stakeIndex, stake.amount);
            ...
            require(stake.amount <= _token.balanceOf(address(this)), "Staking: Staking contract does not own enough token");

            _token.transfer(msg.sender, stake.amount);
        }

        _totalStakedAmount -= stake.amount;
    }
    
    function _withdraw(uint8 stakeIndex, uint256 amount) private returns (uint256) {
        uint256 stakeholderIndex = _stakeholderIndexes[msg.sender];
        ...
        uint256 rewards = _computeRewards(stakeIndex, currentStake);
