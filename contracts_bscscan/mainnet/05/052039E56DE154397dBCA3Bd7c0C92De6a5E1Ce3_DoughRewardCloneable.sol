...
/** 
 *  SourceUnit: c:\Users\Ed\Documents\Crypto\Git-Contracts\Doughpad-BackEnd\contracts\Tokens\DoughRewardCloneable\DoughRewardCloneable.sol
*/
...
////import "./IDividendTrackerCloneable.sol";
...
contract DoughRewardCloneable is DoughTokenCloneable  {
    ...
    string private constant MIN_BALANCE_ISSUE = ">0.001% supply";
    ...
    function __DoughToken_init (SharedStructures.TokenInfo memory _tokenInfo) internal override {
        require (_tokenInfo.decimals == 18, createErrorMessage ("18 decimals only"));
        __Versioned_init ("0.4.2");
        address nativeRewardTracker;
        uint256 length = otherFeeInfo.length;
        ISwapRouter02 router = ISwapRouter02(addressInfo.router);

        require (length > 0 && length <= 3, createErrorMessage ("Need >0 & <=3 rewards"));
            
        uint256 minBalanceForDividends = otherFeeInfo[0].minTokensForDividendsWithoutDecimals * 10**_tokenInfo.decimals;
        
        require (minBalanceForDividends <= totalSupply() / 100_000, createErrorMessage (MIN_BALANCE_ISSUE));
    ...
    function updateMinimumBalanceForDividends (uint256 newMinBalanceWithoutDecimals) external onlyOwner {
        uint256 newMinBalance = newMinBalanceWithoutDecimals * 10**decimals();
        require (newMinBalance <= totalSupply() / 100_000, createErrorMessage (MIN_BALANCE_ISSUE));
        
        emit MinBalanceForDividendsChanged (getMinimumBalanceForDividends(), newMinBalance);

        uint256 length = otherFeeInfo.length;
        
        for (uint256 i; i < length; i++) {
            IDividendTrackerCloneable(otherFeeInfo[i].feeRecipient).updateMinimumBalanceForDividends (newMinBalance);
            otherFeeInfo[i].minTokensForDividendsWithoutDecimals = newMinBalanceWithoutDecimals;
        }
    }
    ...
    function sendOtherEth (uint256 tokensToSwap, uint256 ethAvailable, uint256[] memory scaledOtherFeeTokens) internal override {
        uint256 length = otherFeeInfo.length;

        for (uint256 i; i < length; i++) {
            if (tokensToSwap > 0 && otherFeeInfo[i].rewardToken != address(this)) {
                uint256 ethForRewards = (ethAvailable * scaledOtherFeeTokens[i]) / tokensToSwap;
