...
contract McValentines is IERC20, Ownable, BaseToken {
    ...
    constructor(
        ...
        require(devFeeBps_ >= 0, "Invalid dev fee");
        ...
        require(devFeeBps_ + liquidityFeeBps_ + marketingFeeBps_ <= 10**4 / 4, "Total fee is over 25%");
        ...
        _totalFee = _liquidityFee + _marketingFee + _devFee;
        
        _maxTxAmount = totalSupply_.mul(1).div(10**2); // 1%
    ...
    function setDevFeePercent(uint256 devFeeBps) external onlyOwner {
        _devFee = devFeeBps;
        require(_devFee + _liquidityFee + _marketingFee <= 10**4 / 4, "Total fee is over 25%");
    }
    ...
    function removeAllFee() private {
        if (_devFee == 0 && _liquidityFee == 0 && _marketingFee == 0) return;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 amountBNBDev = newBalance.mul(_devFee).div(totalBNBFee);
        ...
        if (amountBNBDev > 0) {
            _devAddress.transfer(amountBNBDev);
        }

        emit SwapAndLiquify(amountToLiquify, amountBNBLiquidity, amountToLiquify);
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 tTransferAmount, uint256 tDev, uint256 tLiquidity, uint256 tMarketing) = _getValues(tAmount);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tDev, uint256 tLiquidity, uint256 tMarketing) = _getTValues(tAmount);
        return (tTransferAmount, tDev, tLiquidity, tMarketing);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 tTransferAmount = tAmount.sub(tDevFee).sub(tLiquidity).sub(tMarketingFee);
        return (tTransferAmount, tDevFee, tLiquidity, tMarketingFee);
    }
