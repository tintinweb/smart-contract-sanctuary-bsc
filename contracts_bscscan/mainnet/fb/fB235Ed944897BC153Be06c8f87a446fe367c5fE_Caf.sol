...
contract Caf is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Initial distribution for the day 2-5 CAF-WETH LP -> CAF pool
    uint256 public constant INITIAL_CAF_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the CAF ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Caffeine Fund", "CAF") {
        // Mints 1 CAF to contract creator for initial pool setup
    ...
    function _getCafPrice() internal view returns (uint256 _cafPrice) {
        try IOracle(cafOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Caf: failed to fetch CAF price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _cafPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_cafPrice >= taxTiersTwaps[tierId]) {
    ...
    function setCafOracle(address _cafOracle) public onlyOperatorOrTaxOffice {
        require(_cafOracle != address(0), "oracle address cannot be 0 address");
        cafOracle = _cafOracle;
    }
    ...
    /**
     * @notice Operator mints CAF to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of CAF to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentCafPrice = _getCafPrice();
            currentTaxRate = _updateTaxRate(currentCafPrice);
            if (currentCafPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_cafPool != address(0), "!_cafPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_cafPool, INITIAL_CAF_POOL_DISTRIBUTION);
