...
contract zsbToken is ERC20("Stabilize Strategy Trader BSC ETH Token", "zstb-ETH"), Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // Variables
    uint256 constant DIVISION_FACTOR = 100000;
    uint256 private _maxOracleLag = 12 hours; // Maximum amount of lag the oracle can have before reverting the price
    bool public convertToMainToken = true; // This means by default, the program will convert USDT into wBTC
    uint256 public withdrawMaxSlippage = 2000; // Max price difference between chainlink and spot price

    address constant ETH_PRICE_ORACLE = address(0x9ef1B8c0E4F7dc8bF5719Ea496883DC6401d5b2e);
    ...
    // This wrapper takes into account the value of eth compared to the collateral
    // Using the Chainlink price oracle, we convert USDC in the strategy to ETH equivalent to determine the balance
    ...
    // Token information
    // This wrapped token accepts a couple coins
    // ETH, USDC
    struct TokenInfo {
    ...
    function getETHUSDPrice() public view returns (uint256) {
        AggregatorV3Interface priceOracle = AggregatorV3Interface(ETH_PRICE_ORACLE);
        ( , int256 intPrice, , uint256 lastTime, ) = priceOracle.latestRoundData(); // We only want the answer 
        require(block.timestamp.sub(lastTime) < _maxOracleLag, "Price data is too old to use"); // Prevent use of stale oracle data
        return uint256(intPrice).mul(10**10); // Give it 10^18 decimals
    }
    
    function usdcInETHUnits(uint256 _amount) internal view returns (uint256) {
        if(_amount == 0){return 0;}
        uint256 ethPrice = getETHUSDPrice();
        _amount = _amount.mul(10**tokenList[0].decimals).div(10**tokenList[1].decimals); // Convert to eth decimals
        _amount = _amount.mul(1e18).div(ethPrice);
        return _amount;
    }
    
    function ethInUSDCUnits(uint256 _amount) internal view returns (uint256) {
        if(_amount == 0){return 0;}
        uint256 ethPrice = getETHUSDPrice();
        _amount = _amount.mul(10**tokenList[1].decimals).div(10**tokenList[0].decimals); // Convert to usdc decimals
        _amount = _amount.mul(ethPrice).div(1e18);
        return _amount;
    }
    ...
    function getNormalizedTotalBalance(address _address) public view returns (uint256) {
        // Get eth at address and usdc in eth units based on price oracle
        uint256 _balance = 0;
        for(uint256 i = 0; i < tokenList.length; i++){
            uint256 _bal = tokenList[i].token.balanceOf(_address);
            if(i > 0){
                _bal = usdcInETHUnits(_bal); // Convert to eth units
            }
    ...
    function deposit(uint256 amount) public nonReentrant {        
        ...
        _mint(_msgSender(),mintAmount); // Now mint new zst-token to the depositor
    ...
    function redeem(uint256 share) public nonReentrant {
        ...
        if(currentStrategy != StabilizeStrategy(address(0))){
            ...
            if(share < tokenTotal){
                ...
                uint256 _myBalance = _balance.mul(share).div(tokenTotal); // This is in wbtc units normalized
                withdrawPerOrderWithOracleSwap(_msgSender(), _myBalance, false, convertToMainToken); // This will withdraw based on token balanace
                withdrawAmount = _myBalance;
            }else{
                // We are all shares, transfer all
                uint256 _balance = getNormalizedTotalBalance(address(this)); // This is in wbtc units normalized
                withdrawPerOrderWithOracleSwap(_msgSender(), _balance, true, convertToMainToken);
    ...
    function pancakeSwapUSDCForETH(uint256 amount, address receiver) internal {
        // Since BSC tx can be frontran, must account for slippage
        uint256 maxETH = usdcInETHUnits(amount); // Amount without slippage
        uint256 minETHWithSlip = maxETH.mul(DIVISION_FACTOR.sub(withdrawMaxSlippage)).div(DIVISION_FACTOR);
        ...
        path[1] = address(tokenList[0].token);
        {
            ...
            estimates = router.getAmountsOut(amount, path);
            uint256 expectedAmount = estimates[estimates.length - 1]; // This is the amount of ETH
            require(expectedAmount >= minETHWithSlip, "Price difference too high between spot and chainlink, try smaller amount");
        }
        tokenList[1].token.safeApprove(address(router), 0);
        tokenList[1].token.safeApprove(address(router), amount);
        router.swapExactTokensForTokens(amount, minETHWithSlip, path, receiver, now.add(60));
    }
    ...
    function withdrawPerOrderWithOracleSwap(address _receiver, uint256 _withdrawAmount, bool _takeAll, bool convert) internal {
                ...
                if(_bal > 0){
                    if(i == 0){
                        ...
                        if(convert == false){
                            ...
                            pancakeSwapUSDCForETH(_bal, _receiver); // Will convert USDC to ETH
                        }
                    }
        ...
        for(uint256 i = 0; i < length; i++){
            ...
            if(i == 0){
                _normalizedBalance = _normalizedBalance.mul(1e18).div(10**tokenList[0].decimals); // Convert eth to normalized
            }else if(i == 1){
                // USDT
                _normalizedBalance = usdcInETHUnits(_normalizedBalance); // Convert USDC to eth units
            ...
            if(_normalizedBalance <= _withdrawAmount){
                ...
                if(_normalizedBalance > 0){
                    ...
                    if(i == 0){
                        ...
                        if(convert == false){
                            ...
                            // Convert whatever is left to ETH
                            pancakeSwapUSDCForETH(tokenList[i].token.balanceOf(address(this)), _receiver); // Will convert USDC to ETH
                        }
                    }              
                }
            }else{
                ...
                if(_withdrawAmount > 0){
                    ...
                    if(i == 1){
                        ...
                        _balance = ethInUSDCUnits(_balance);
                    }
                    _withdrawAmount = 0;
                    if(i == 0){
                        ...
                        if(convert == false){
                            ...
                            pancakeSwapUSDCForETH(_balance, _receiver); // Will convert USDC to ETH
                        }
                    }
                }
    ...
    // Change the option to swap upon withdraw
    // --------------------
    function startChangeSwapTokens(bool _swapable) external onlyGovernance {
        _timelockStart = now;
        _timelockType = 3;
        _timelock_bool = _swapable;
    }
    
    function finishChangeSwapTokens() external onlyGovernance timelockConditionsMet(3) {
        convertToMainToken = _timelock_bool;
    }
    // -------------------
    ...
    function finishChangeOracleLag() external onlyGovernance timelockConditionsMet(4) {
        _maxOracleLag = _timelock_data;
    }
    // -------------------

    // Change the amount of max lag for the oracle
    // --------------------
    function startChangeWithdrawSlippage(uint256 _slip) external onlyGovernance {
        _timelockStart = now;
        _timelockType = 5;
        _timelock_data = _slip;
    }
    
    function finishChangeWithdrawSlippage() external onlyGovernance timelockConditionsMet(5) {
        withdrawMaxSlippage = _timelock_data;
    }
    // -------------------
