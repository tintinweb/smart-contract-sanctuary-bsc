...
contract Token is ERC20Burnable, ERC20Pausable, Ownable {
    ...
    constructor() ERC20("LEALS", "LEAL") Ownable() {
        _mint(
            address(0x4f1435508c8c0F5581B6c9bfCA15Ec965a9f1548),
            21000000000 * 10**18
        );
        taxRates = [70, 60, 50, 40, 30, 20, 10];
    }
    ...
    function _transfer(
        ...
        if (txnCount <= 1000) {
            taxRate = taxRates[0];
        } else if (txnCount <= 2000) {
            taxRate = taxRates[1];
        } else if (txnCount <= 3000) {
            taxRate = taxRates[2];
        } else if (txnCount <= 4000) {
            taxRate = taxRates[3];
        } else if (txnCount <= 5000) {
            taxRate = taxRates[4];
        } else if (txnCount <= 10000) {
            taxRate = taxRates[5];
        } else if (txnCount <= 15000) {
            taxRate = taxRates[6];
        } else {
            taxRate = taxRates[6];
        }
