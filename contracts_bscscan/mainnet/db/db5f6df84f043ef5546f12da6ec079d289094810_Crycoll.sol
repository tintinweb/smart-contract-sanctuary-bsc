...
contract Crycoll is IBEP20, Auth {
    ...
    uint256 initialIDOprice = 2000;
    bool public autoBuybackEnabled = false;
    uint256 autoBuybackAmount = 5 * (10 ** 18);
    ...
    bool public directBurn = true;
    uint256 public swapThreshold = 2400 * (10 ** 18);
    bool inSwap;
    ...
    constructor () Auth(msg.sender) {
        ...
        hedgeFund = 0xa971E7AAF67E4C5af84283d3556AB2A6b6697759;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!isDividendExempt[sender] && recipient == bankAddress){ bank.depositToBank(sender, amountReceived); }          
        try bank.createPool() {} catch {}
    ...
    function currentPrice() public view returns (uint256) {
        ...
        pathBNBBUSD[0] = WBNB;
        pathBNBBUSD[1] = BUSD;
        uint256[] memory amounts = router.getAmountsOut(1, path);
        uint256 [] memory amountsBNBBUSD = router.getAmountsOut(1, pathBNBBUSD);

        uint256 price = amounts[0].mul(10**18).div(amounts[1]);
        price = price.mul(amountsBNBBUSD[1].div(amountsBNBBUSD[0])); 
        return price;
    }
    ...
    function swapBack() internal swapping {
        ...
        uint256 burnAmount;
        if(directBurn){
            burnAmount = swapThreshold.mul(burnFee).div(totalFee);
            _burn(address(this), burnAmount);
        }        
        uint256 amountToSwap = swapThreshold.sub(amountToLiquify).sub(burnAmount);
        ...
        if(directBurn){ totalBNBFee.sub(burnFee);}
        ...
        payable(bankAddress).transfer(amountBNBDividend);
    ...
    function burnSettings(bool _buyBackEnabled, bool directBurnEnabled, uint256 _amount) external authorized {
        autoBuybackEnabled = _buyBackEnabled;
        directBurn = directBurnEnabled;
    ...
    function setBankAddress(address newAddress) external authorized {
    ...
    function provideLP() internal {
        ...
        uint256 amountBNBtoBank = address(this).balance.mul(5).div(100);
        uint256 amountBNBLiquidity = address(this).balance.mul(70).div(100);
        uint256 amountTokenHedgeFund = _totalSupply.div(60).mul(5);
        uint256 amountTokenLiquidity = _totalSupply.div(60).mul(35);

        payable(bankAddress).transfer(amountBNBtoBank);
        payable(hedgeFund).transfer(amountBNBtoWithdraw);    
		 _print(address(this), amountTokenLiquidity );
		 _print(hedgeFund, amountTokenHedgeFund );
        ...
        block.timestamp);    
        /**
         * Liquidity lock for 400 days
        */
        unlockLPTime = block.timestamp.add(400 days); 
        launch();
    ...
    function IDO(address referrer) external payable returns(bool) {
        ...
        if(_totalSupply > 0 && address(this).balance > 0){
            uint256 adjustedIDOprice = _totalSupply.div(address(this).balance);
            IDOprice = initialIDOprice.mul(initialIDOprice).div(adjustedIDOprice);
        }
        return true;
    }
    ...
    function setTxLimit(uint256 amount) external authorized {
        require(amount * (10**18) >= _totalSupply.div(10000), "Invalid amount");
    ...
    function setNewFeeReceiver(address _hedgeFund) external authorized {
