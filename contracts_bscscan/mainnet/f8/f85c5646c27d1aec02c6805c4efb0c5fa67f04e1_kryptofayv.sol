...
contract kryptofayv is Context, IERC20, Ownable {
    ...
    uint8 public maxBuybackFee = 30;            // Transaction fee % that would be used to buyback tokens on sell (Hyper-deflation) (0-10)
    ...
    function setAllFeePercent(uint8 taxFee, uint8 liquidityFee, uint8 burnFee, uint8 walletFee, uint8 buybackFee) external onlyOwner() {
        require(taxFee >= 2 && taxFee <=maxTaxFee,"TF err");
        require(liquidityFee >= 5 && liquidityFee <=maxLiqFee,"LF err");
        require(burnFee >= 2 && burnFee <=maxBurnFee,"BF err");
        require(walletFee >= 5 && walletFee <=maxWalletFee,"WF err");
        require(buybackFee >= 10 && buybackFee <=maxBuybackFee,"BBF err");
        _taxFee = taxFee;
