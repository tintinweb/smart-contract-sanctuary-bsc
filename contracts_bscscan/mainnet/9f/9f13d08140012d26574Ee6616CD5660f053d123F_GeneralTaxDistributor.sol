...
contract GeneralTaxDistributor is IGeneralTaxDistributor, WithAdmin {
    ...
    enum TargetType {NotSet, Burn, Address, DefaultRewardPool, CustomRewardPool}
    struct TokenInfo {
        uint248 bufferSize;
    ...
    function turnRandimation(uint8 off) external onlyAdmin {
		...
		function setPoolRouting(
			...
			address msgSender,
			address poolId) external onlyAdmin {
    ...
    function _distributeTax(
        ...
        return distributeToTarget(idx, ti.tokenSpecificConfig, token, origin, balance);
    }
