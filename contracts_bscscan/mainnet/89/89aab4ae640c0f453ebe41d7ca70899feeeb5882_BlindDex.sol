...
contract BlindDex is Ownable, DexStorage {
 ...
 function addSupportNFT(address _addr, bool isRoyaltyAllowed) public {
  ...
  function mintBlindbox(address collection, address to, uint256 quantity, address from, uint256 royalty, string memory ownerId) public returns(uint256 fromIndex,uint256 toIndex) {
        ...
        for(uint256 i = fromIndex; i<= toIndex; i++){
            if(to == nonCryptoNFTVault){
                _nonCryptoOwners[i][collection] = ownerId;
                 emit MintWithTokenURINonCrypto(to, ownerId, "", collection);
