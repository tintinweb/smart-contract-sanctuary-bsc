...
contract Astronaut is Context, IBEP20, Ownable {
    ...
    string private _symbol = "Astronaut";
    uint8 private _decimals = 18;
    uint256 public _BNBPoolFee = 6;
    ...
    event UpdateFees(uint256 BNBPoolFee, uint256 LpFee);
    ...
    function setFees(uint256 BNBPoolFee, uint256 LpFee) external onlyOwner() {
        require(_BNBPoolFee != BNBPoolFee || _LpFee != LpFee);
        require(BNBPoolFee + LpFee <= 8);
        _BNBPoolFee = BNBPoolFee;
        _LpFee = LpFee;
        emit UpdateFees(BNBPoolFee, LpFee);
    }
    ...
    function _getFeeValues(uint256 tAmount) private view returns (uint256) {
        uint256 fee = tAmount.mul(_BNBPoolFee + _LpFee).div(10**2);
        uint256 tTransferAmount = tAmount.sub(fee);
        return tTransferAmount;
    }

    function removeAllFee() private {
        if(_BNBPoolFee == 0 && _LpFee == 0) return;

        _previousBNBPoolFee = _BNBPoolFee;
    ...
    function restoreAllFee() private {
        _BNBPoolFee = _previousBNBPoolFee;
    ...
    function swapAndCharge() private lockTheSwap {
        ...
        uint256 totalFee = _BNBPoolFee + _LpFee;
        payable(bnbPoolAddress).transfer(bnbBalance.mul(_BNBPoolFee).div(totalFee));
