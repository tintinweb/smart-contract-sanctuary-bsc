...
contract DeuxSaleLock is Context, Ownable, ReentrancyGuard {
    ...
    mapping(address => mapping(address => bool)) public bannedAccounts;
    ...
    mapping(address => bool) public tokenSaleStatusList;
    mapping(address => bool) public tokenLockStatusList;
    mapping(address => bool) public tokenSaleWhitelistingStatusList;

    uint256[] public lockTimestampList;
    uint256[] public lockPercentList;
    ...
    // Add one account to Whitelisting List
    ...
    // Remove one account from Whitelisting List
    ...
    // Add accounts to Whitelisting List
    ...
    // Remove accounts from Whitelisting List
    ...
    // Set multiple accounts buy limit
    ...
    // Remove multiple accounts buy limit on list
    ...
    // Add single account buy limit to list
    ...
    // Remove single account buy limit on list
    ...
    function addSingleAccountToBannedList(address _token, address _addr)
        ...
        onlyOwner
    {
        bannedAccounts[_token][_addr] = true;
    }

    // Remove single account banned
    function removeSingleAccountFromBannedList(address _token, address _addr)
        external
        onlyOwner
    {
        bannedAccounts[_token][_addr] = false;
    }

    // Get account ban status
    function getAccountBanStatus(address _token, address _addr)
        external
        view
        returns (bool)
    {
        return (bannedAccounts[_token][_addr]);
    }
        ...
        onlyOwner
    {
        ...
        // Set LockPercentList
        delete lockPercentList;
        for (uint256 i = 0; i < _lockPercents.length; i++) {
            lockPercentList.push(_lockPercents[i]);
        }
    }
    ...
    function setLockTimeList(uint256[] memory _lockTimestampList)
        ...
        onlyOwner
    {
        for (uint256 i = 0; i < _lockTimestampList.length; i++) {
            require(
                _lockTimestampList[i] > block.timestamp,
                "DEUX : unlock timestamp should be higher than current time"
            );
        }
        delete lockTimestampList;
        for (uint256 i = 0; i < _lockTimestampList.length; i++) {
            lockTimestampList.push(_lockTimestampList[i]);
        }
    }
    ...
    // Contract sale status for token via crowedsale active or passive
    function setSaleStatus(address _token, bool _saleActive)
        external
        onlyOwner
    {
        tokenSaleStatusList[_token] = _saleActive;
    }
    ...
    function setLockStatus(address _token, bool _lockActive)
        external
        onlyOwner
    {
    ...
    // Here G<et Liquidity function for call inside
        ...
        )
    {
        ...
        uint256[] memory lockAmounts = new uint256[](lockPercentList.length);
        ...
        for (uint256 i = 0; i < lockPercentList.length; i++) {
            uint256 lockPercent = lockPercentList[i];
    ...
    function withdrawTokens(uint256 _id) external {
        require(
            !bannedAccounts[lockedToken[_id].tokenAddress][_msgSender()],
            "DEUX: sender account banned to claim for this token"
        );
        require(
    ...
    // Get available sale limit on active pair for an address
    function getWalletBuyLimit(address _addr)
        external
        view
        shouldSaleActive
        shouldPairDefined
