...
library SafeMath {
		...
		function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
			...
			// benefit is lost if 'b' is also ValentinesInued.
...
abstract contract Context {
	 ...
	 * This allows applications to reconstruct the allowance for all ValentinesInu just
	...
	// File: contracts/ValentinesInu.sol

	// 

	pragma solidity ^0.8.0;

	contract ValentinesInu is ERC20, Ownable {
		using SafeMath for uint256;
        ...
        address payable public marketingWallet = payable(0x23A6106917ADb781BC2948FfaF42123876FF2181); 
		address payable public devWallet = payable(0x621A8D5304060C0a5E5c4Ac480bC36cA008C59d9); 
		address payable public buyBackWallet = payable(0x621A8D5304060C0a5E5c4Ac480bC36cA008C59d9); 
		...
		uint256 public tokenThreshold = 10000000000 * (10**18); 
		
	    mapping (address => bool) private _isBlacklisted;
        mapping (address => bool) private _isTxLimit;
		mapping (address => bool) private _isExcludedFromFees;
		mapping (address => bool) private canTransferBeforetadingOpen;


		bool public tradingOpen = false;
		bool public blacklistMode = false;
		uint256 private _buyBackThreshold = 200000 * (10**18);
		...
		event excludeMultipleAccountFromRewards(address[] ValentinesInu, bool isExcluded);

		event SetAutomatedMarketMakerPair(address indexed pair, bool indexed value);

		event LiquidityWalletUpdated(address indexed newLiquidityWallet, address indexed oldLiquidityWallet);
		
		uint256 private _swapThreshold = 1000000000000 * (10**18); 
		...
		constructor() ERC20("Valentines Inu", "VINU"){
			...
			totalFee =  _liquidityFee + _marketingFee + _devFee + _buyBackFee + _burnFee;
			...
			canTransferBeforetadingOpen[owner()] = true;
			/*
		...
		function setIsFeeExempt(address account, bool excluded) public onlyOwner {
			require(_isExcludedFromFees[account] != excluded, "ValentinesInu: Account is already the value of 'excluded'");
			_isExcludedFromFees[account] = excluded;
		}
        
        function setIsTxLimitExempt(address account, bool excluded) public onlyOwner {
			require(_isTxLimit[account] != excluded, "ValentinesInu: Account is already the value of 'excluded'");
		...
		function setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
			require(pair != pancakeswapV2Pair, "ValentinesInu: The pancakeswap pair cannot be removed from automatedMarketMakerPairs");

			_setAutomatedMarketMakerPair(pair, value);
		}

		function _setAutomatedMarketMakerPair(address pair, bool value) private {
			require(automatedMarketMakerPairs[pair] != value, "ValentinesInu: Automated market maker pair is already set to that value");
		...
		function updateLiquidityWallet(address newLiquidityWallet) private onlyOwner {
			require(newLiquidityWallet != liquidityWallet, "ValentinesInu: The liquidity wallet is already this address");
		...
		function setDistributorSettings(uint256 gas) public onlyOwner {
			require(gas >= 200000 && gas <= 500000, "ValentinesInu: distributorGas must be between 200,000 and 500,000");
			require(gas != distributorGas, "ValentinesInu: Cannot update distributorGas to same value");
		...
		function setSwapThreshold(uint256 amount) external onlyOwner() {
            _swapThreshold = amount	 * 10**18;
			
		}
		function setFeeReceiver(address payable _marketingWallet, address payable _devWallet, address payable _buyBackWallet) external onlyOwner() {
		...
		function setFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _devFee, uint256 _buyBackFee, uint256 _burnFee) external onlyOwner {
			marketingFee = _marketingFee;
			liquidityFee = _liquidityFee;
			devFee = _devFee;
			buyBackFee = _buyBackFee;
		...
		function setBlacklistMode(bool _enabled, uint256 _antiSnipeBlock) public onlyOwner {
			blacklistMode = _enabled;
			antiSnipe = _antiSnipeBlock;
		
		}
			function blacklistAddress(address _account, bool enabled) external onlyOwner {
            _isBlacklisted[_account] = enabled;
		...
		function clearBuybacMultiplier() external onlyOwner() {
		...
		function _transfer(
			...
			if (blacklistMode)
			if (block.number <= launchBlock + antiSnipe ) {
			...
			if(!tradingOpen) {
				require(canTransferBeforetadingOpen[from], "ValentinesInu: This account cannot send tokens until trading is enabled");
			}
			...
			if( 
				...
				require(amount <= _swapThreshold, "address 100%.");
			}

			uint256 contractTokenBalance = balanceOf(address(this));
			
			bool canSwap = contractTokenBalance >= _buyBackThreshold;
			...
			if(takeFee) {
				...
				if(automatedMarketMakerPairs[to]) {
					fees = fees.mul(marketingFee).div(100);
				}
