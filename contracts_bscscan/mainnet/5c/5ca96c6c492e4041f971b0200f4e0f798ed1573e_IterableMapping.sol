...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  function updateDividendUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "SBSE: The router already has that address");
...
contract SBSE is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    SBSEDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Starbase", "SBSE", 9) {

        dividendTracker = new SBSEDividendTracker();
    ...
    function setWhiteListAMM(address ammAddress, bool isWhiteListed) external onlyOwner {
      require(isContract(ammAddress), "SBSE: setWhiteListAMM:: AMM is a wallet, not a contract");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "SBSE: The dividend tracker already has that address");

        SBSEDividendTracker newDividendTracker = SBSEDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "SBSE: The new dividend tracker must be owned by the SBSE token contract");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "SBSE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) external onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "SBSE: The liquidity wallet is already this address");
    ...
    function updateOperationsWallet(address newOperationsWallet) external onlyOwner {
        require(newOperationsWallet != operationsWallet, "SBSE: The operations wallet is already this address");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) external onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "SBSE: The buyback wallet is already this address");
    ...
    function updateFees(uint256 bnbRewardPerc, uint256 liquidityPerc, uint256 operationsPerc, uint256 buyBackPerc) external onlyOwner {
        require (operationsPerc.add(buyBackPerc) <= liquidityPerc, "SBSE: updateFees:: Liquidity Perc must be equal to or higher than operations and buyback combined.");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SBSE: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SBSE: Cannot update gasForProcessing to same value");
    ...
    function setRewardToken(address rewardTokenAddress) public returns (bool) {
        require(isContract(rewardTokenAddress), "SBSE: setRewardToken:: Address is a wallet, not a contract.");
        require(rewardTokenAddress != address(this), "SBSE: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
        require(!isIgnoredToken(rewardTokenAddress), "SBSE: setRewardToken:: Reward Token is ignored from being used as rewards.");
    ...
    function setRewardTokenWithCustomAMM(address rewardTokenAddress, address ammContractAddress) public returns (bool) {
        require(isContract(rewardTokenAddress), "SBSE: setRewardToken:: Address is a wallet, not a contract.");
        require(ammContractAddress != address(uniswapV2Router), "SBSE: setRewardToken:: Use setRewardToken to use default Router");
        require(rewardTokenAddress != address(this), "SBSE: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
        require(!isIgnoredToken(rewardTokenAddress), "SBSE: setRewardToken:: Reward Token is ignored from being used as rewards.");
        require(isAMMWhitelisted(ammContractAddress) == true, "SBSE: setRewardToken:: AMM is not whitelisted!");
    ...
    function buyBackTokensWithNoFees() external payable returns (bool) {
        ...
        require(userRawBNBDividends >= holderBNBUsedForBuyBacks[msg.sender].add(msg.value), "SBSE: buyBackTokensWithNoFees:: Cannot Spend more than earned.");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SBSE: Automated market maker pair is already set to that value");
    ...
    function _transfer(
        ...
        require(!_isIgnoredAddress[to] || !_isIgnoredAddress[from], "SBSE: To/from address is ignored");
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        if(liquidityFee > 0){ // dividing by 0 is not fun.
            ...
            if(operationsFee > 0){
                ...
                require(success, "SBSE: SwapAndLiquify:: Unable to send BNB to Operations Wallet");
            }
            ...
            if(buyBackFee > 0){
                ...
                require(success, "SBSE: SwapAndLiquify:: Unable to send BNB to BuyBack Wallet");
            }
...
contract SBSEDividendTracker is DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("SBSE_Dividend_Tracker", "SBSE_Dividend_Tracker", 9) {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10 * 10 ** 9 * (10**9); //must hold 10,000,000,000+ tokens to get divs
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "SBSE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "SBSE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SBSE contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SBSE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SBSE_Dividend_Tracker: Cannot update claimWait to same value");
