...
contract GaziniV1 is Context, IERC20, Ownable, Stakeable {
    ...
    uint256 private _tTotal = 10 * 10**8 * _decimalFactor; // Tokens total
    ...
    uint256 public _previousDonationFee = _donationFee;
    ...
    constructor() {
        ...
        // Fixed Developer wallet
        address devAddr =0x30FBf4dFE1df1e951A1eBC7A1439Fcf2af45b79f;
        address payable devWallet = payable(devAddr);

        // Fixed Marketing wallet
        address marketingAddr =0xC54089350afa7ecCEb6d09c9Bf09ff4282E63328;
        address payable marketingWallet = payable(marketingAddr);

        // Fixed Burn wallet
        address burnWallet =0x6E39484653598f17904fBeA9113768c041c1D1dA;
        address payable burnAddress = payable(burnWallet);

        // Fixed Donation wallet
        address donationWallet = 0x94B8F733612e487852E30D6039bE782218A0373F;
        address payable donationAddress = payable(donationWallet);


        setDeveloperAddress(devWallet);
        setMarketingAddress(marketingWallet);
        setBurnAddress(burnAddress);
        setDonationAddress(donationAddress);
    }
//
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "GaziniV1: approve from the zero address");
        require(spender != address(0), "GaziniV1: approve to the zero address");
    ...
    function _transfer(address from, address to, uint256 amount) private {
        require(from != address(0), "GaziniV1: transfer from the zero address");
        require(to != address(0), "GaziniV1: transfer to the zero address");
        ...
        if (_marketingAddress == address(0) || _developerAddress == address(0) || _donationAddress == address(0)) {
            takeFee = false;
        }
        ...
        if (
            ...
            uint256 totalFee = _developerFee.add(_marketingFee).add(_liqFee).add(_donationFee);
    ...
    function _takeFees(uint256 amount, address from) private lockTakeFees {
        // Calculate the wallet fees to take (partnership, marketing and donations) - don't
        // take reflect tax or liquidity here
        uint256 walletFee = _developerFee.add(_marketingFee);
        walletFee.add(_donationFee);
        ...
        uint256 donationETHPortion = swappedETH.div(totalFeeToBeSwappedForETHMul10).mul(_donationFee.mul(10));
        ...
        totalETHPortionForWallets.add(donationETHPortion);
        ...
        (sent, data) = _donationAddress.call{value: donationETHPortion}("");
        require(sent, 'ETH was not sent to marketing');
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256) {
        ...
        uint256 tFee = taxFee + calculateDonationFee(tAmount);
    ...
    function removeAllFees() private {
        if(_donationFee == 0 && _marketingFee == 0 && _developerFee == 0 && _liqFee == 0) return;
    ...
    /* Public setters */
    function setMaxTxPercent(uint256 maxTxPercent) private {
  ...
  function setTaxFeePercent(uint256 taxFee) external onlyOwner {
    ...
    function setDonationFeePercent(uint256 taxFee) private{
        _previousDonationFee = _donationFee;
        _donationFee = taxFee;
    }

    function setLiqFeePercent(uint256 liqFee) private  {
        _previousLiqFee = _liqFee;
        _liqFee = liqFee;
    }

    function setDeveloperFeePercent(uint256 developerFee) private {
        _previousDeveloperFee = _developerFee;
    ...
    function setBurnAddress(address payable addr) private  {
        _burnAddress = addr;
    }

    function setDonationAddress(address payable addr) private  {
    ...
    require(account != address(0), "GaziniV1: cannot mint to zero address");
  ...
  */
  function _burn(address account, uint256 amount) internal {
    require(account != address(0), "GaziniV1: cannot burn from zero address");
    require(_balances[account] >= amount, "GaziniV1: Cannot burn more than the account owns");
  ...
  *
  ...
  function burn(address account, uint256 amount) public onlyOwner returns(bool) {
   ...
   function burnGlobalWallet(uint256 amount) public onlyOwner returns(bool) {
  ...
  function mint(address account, uint256 amount) public onlyOwner returns(bool){
    ...
    *
    ...
    function stake(uint256 _amount) public {
      ...
      require(_amount < _balances[msg.sender], "GaziniV1: Cannot stake more than you own");
