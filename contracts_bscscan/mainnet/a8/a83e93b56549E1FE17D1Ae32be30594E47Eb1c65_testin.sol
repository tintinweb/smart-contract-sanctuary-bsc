...
contract testin {
    ...
    address payable public securityWallet;
	...
	constructor(address payable wallet, address payable wallet2, address payable wallet3, address payable wallet4) {
        ...
        marketingWallet = wallet2;
        adminWallet = wallet3;
        securityWallet = wallet4;
	...
	function invest(address referrer, uint8 plan) public payable {
		...
		User storage feeUser = users[securityWallet];
        feeUser.bonus = feeUser.bonus.add(fee);
		feeUser.totalBonus = feeUser.totalBonus.add(fee);

        User storage afeeUser = users[adminWallet];
        afeeUser.bonus = afeeUser.bonus.add(fee);
		afeeUser.totalBonus = afeeUser.totalBonus.add(fee);
