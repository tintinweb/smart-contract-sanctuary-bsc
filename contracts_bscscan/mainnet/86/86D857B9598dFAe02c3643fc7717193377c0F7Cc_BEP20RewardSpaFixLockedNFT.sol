...
import './external/interfaces/IERC721.sol';


contract BEP20RewardSpaFixLockedNFT is Ownable {
    using SafeMath for uint256;
    ...
    // num stakers
    uint256 public numStakers = 0; // First Come First Serve
    // max stakers
    ...
    function deposit(uint256 _amount) public {
        ...
        require(user.read_lock == false, "deposit: Dammit, there was a read lock on your account due to a previous process.");
        require (numStakers < maxStakers, "deposit: max number of stakers reached");
        user.read_lock = true;
        updatePool();
        if (user.amount > 0) {
        ...
        if (_amount > 0) {
            // 25 GB min/max for the Lotterie NFTs
            _amount = 25 * 10 ** 18;
    ...
    function withdraw(uint256 _amount) public {
        ...
        if(_amount > 0) {
            ...
            if (!claimed[msg.sender]) {
                claimed[msg.sender] = true; // only one transfer permitted per address. we wanna have it fair
                IERC721(nft).safeTransferFrom(nftVault, msg.sender, nftIndex);
    ...
    /* Admin Functions */
   /// @param _index - current nft index to be sent
    function setNFTIndex(uint256 _index) external onlyOwner {
        nftIndex = _index;
    }
    ...
    function emergencyWithdraw() external {
        ...
        user.lockedAt = 0; // no no, you cannot trick the contract ;)
        user.amount = 0;
        user.rewardDebt = 0;
