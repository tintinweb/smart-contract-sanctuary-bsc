...
contract BABYTOKEN is ERC20, Ownable, BaseToken {
    ...
    constructor(
        ...
        address[5] memory addrs, // reward, router, marketing wallet, dividendTracker,dev wallet
        ...
        _devWalletAddress = payable(addrs[4]);
        require(
        ...
        developmentFee = feeSettings[3];
        totalFees = tokenRewardsFee.add(liquidityFee).add(marketingFee).add(developmentFee);
    ...
    function _transfer(
        ...
        if (
            ...
            .mul(marketingFee.add(developmentFee))
            .div(totalFees);
            swapToken(marketingTokens);
    ...
    function swapToken(uint256 contractTokenBalance) private  {
        uint256 swapFees = developmentFee.add(marketingFee);
        uint256 initialBalance = address(this).balance;
        swapTokensForEth(contractTokenBalance);
        uint256 transferredBalance = address(this).balance.sub(initialBalance);

        //Send to Marketing address
        transferToAddressETH(_marketingWalletAddress, transferredBalance.mul(marketingFee).div(swapFees));
        transferToAddressETH(_devWalletAddress, transferredBalance.mul(developmentFee).div(swapFees));
