...
abstract contract Data is Events {
    ...
    uint256 public DAILY_SUPPLY = 1E25;  // 10M tokens per day 10000000x1E18
    
    uint256 public MIN_DONATE = 10E18; //10 BUSD
    uint256 public MAX_DONATE = 25000E18; //25K BUSD
    uint256 public MAX_NUM_PARTICIPANTS = 1000;
    
    uint256 public MAX_DONATION_DAYS = 273;
    uint256 public LAST_CONTRACT_DAY = 320;

    uint256 public CurrentAuctionDay = 1;

    bool public ARE_REFERRALS_ACTIVE = true;
...
abstract contract PoolMgmt is Helper {
    ...
    function _fillLiquidityPool(uint256 poolTokenAvg, uint256 poolBusdAvg) internal {
        ...
        //the auction contract may have some remaining EQNOX tokens that it wasn't able to transfer to the pool
...
/**
 * @dev Abstract contract that contains all functions related to updating a day, generating random busd rewards, and functions for claiming purchased EQNOX and rewarded BUSD.
 */
abstract contract DistributionMgmt is PoolMgmt {
    ...
    /**
     * @dev Internal function to update the day. Advances the auction day until it reaches the CurrentTokenDay on the EQNOX contract.
     */
    function _updateDay() internal {
        ...
        while(CurrentAuctionDay < currentTokenDay) {
            ...
            //update the auctioned supply
            //every day passed, the daily auctioned EQNOX is reduced by 0.5%
            DAILY_SUPPLY = DAILY_SUPPLY.sub(DAILY_SUPPLY.div(200));
    ...
    /**
     * @dev Private function to register the daily distribution ratio of EQNOX to BUSD.
     */
...
abstract contract DonationMgmt is DistributionMgmt {
    ...
    /**
     * @dev External function to donate busd for an auction day. Users can donate BUSD multiple times and as such get a greater stake of the EQNOX to distribute.
    ...
    function _processDonatedBusd(address sender, uint256 tokenDay, uint256 amountToDonateBusd, address referral) private {
        ...
        if(referrals[sender] != address(0x0) && ARE_REFERRALS_ACTIVE) {
...
abstract contract ContractStateReader is DonationMgmt {
    ...
    /**
     * @dev External function to read how many EQNOX a user can claim
     */
 ...
 */
contract EQNoxAuctions is ContractStateReader {
    address public TOKEN_DEFINER;
    address public BID_MANAGER;
    address public AUCTION_MANAGER;
    address public REFERRALS_MANAGER;
    ...
    modifier onlyAuctionManager() {
        require(
            msg.sender == AUCTION_MANAGER,
            'Wrong sender.'
        );
        _;
    }

    modifier onlyReferralsManager() {
        require(
            msg.sender == REFERRALS_MANAGER,
            'Wrong sender.'
        );
        _;
    }
    ...
    /**
     * @dev External function accessible only by the bidManager to forever deny special access for the bid manager. This operation is irreversible.
     */
    function revokeAccessToBidManager() external onlyBidManager {
        BID_MANAGER = address(0x0);
    }

    /**
     * @dev External function accessible only by the auctionManager to forever deny special access for the auction manager. This operation is irreversible.
     */
    function revokeAccessToAuctionManager() external onlyAuctionManager {
        AUCTION_MANAGER = address(0x0);
    }


    /**
     * @dev External function accessible only by the referralsManager to forever deny special access for the referrals manager. This operation is irreversible.
     */
    function revokeAccessToReferralsManager() external onlyReferralsManager {
        REFERRALS_MANAGER = address(0x0);
    }
    ...
    function updateMinBid(uint256 minBidValue) external onlyBidManager {
        MIN_DONATE = minBidValue;
    }

    function updateMaxBid(uint256 maxBidValue) external onlyBidManager {
        MAX_DONATE = maxBidValue;
    }

    function updateMaxParticipants(uint256 maxParticipants) external onlyBidManager {
        MAX_NUM_PARTICIPANTS = maxParticipants;
    }

    function updateAuctionEndDate(uint256 maxDonationDays, uint256 lastContractDay) external onlyAuctionManager {
        MAX_DONATION_DAYS = maxDonationDays;
        LAST_CONTRACT_DAY = lastContractDay;
    }

    function updateReferralsActiveState(bool areReferralsActive) external onlyReferralsManager {
        ARE_REFERRALS_ACTIVE = areReferralsActive;
    }
