// MetaCryptoCatsNFT
// website: https://www.metacryptocats.com/
// telegram: https://t.me/metacryptocats
...
contract MetaCryptoCatsNFT is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    event liquidityReceiverWalletUpdated(address indexed newWallet, address indexed oldWallet);
    ...
    constructor() ERC20("MetaCryptoCatsNFT", "MCC") {
        address newOwner = address(0x25e555C72Fdc168E0EdeccaCDe849253429A7956); // update to 0x25e555C72Fdc168E0EdeccaCDe849253429A7956

        uint256 totalSupply = 1000 * 1e9 * 1e18;

        maxTransactionAmount = totalSupply; // 0.5% maxTransactionAmountTxn (  * 5 / 1000 )
        swapTokensAtAmount = totalSupply * 5 / 100000; // 0.05% swap tokens amount


        rewardsSellFee = 0;
        marketingSellFee = 7; // 5% marketing&dev 2% buyback&burn
        developerSellFee = 2;
        liquiditySellFee = 3;
        charitySellFee = 1;
        totalSellFees = rewardsSellFee + marketingSellFee + developerSellFee + liquiditySellFee + charitySellFee;

        rewardsBuyFee = 0;
        marketingBuyFee = 7; // 5% marketing&dev 2% buyback&burn
        developerBuyFee = 2;
        liquidityBuyFee = 3;
        charityBuyFee = 1;
        totalBuyFees = rewardsBuyFee + marketingBuyFee + developerBuyFee + liquidityBuyFee + charityBuyFee;

    	dividendTracker = new DividendTracker();

    	marketingWallet = address(0x3F14118568b07164814D1C9269E39099a36F351B); // set as marketing wallet
      developerWallet = address(0x7cf834608B0588bb69350ebA30421069095Fd330); // set as developer wallet
      charityWallet = address(0x6D872Ff96c9eeb509A24a178b971197dF56f7E38); // set as charity wallet

      autoLiquidityReceiver = address(0x3F14118568b07164814D1C9269E39099a36F351B);
    ...
    function updateMaxAmount(uint256 newNum) external onlyOwner {
        require(newNum > (totalSupply() * 5 / 10000)/1e18, "Cannot set maxTransactionAmount lower than 0.5%");
        maxTransactionAmount = newNum * (10**18);
    }

    function updateBuyFees(uint256 _marketingFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _developerFee, uint256 _charityFee) external onlyOwner {
        marketingBuyFee = _marketingFee;
        rewardsBuyFee = _rewardsFee;
        liquidityBuyFee = _liquidityFee;
        developerBuyFee = _developerFee;
        charityBuyFee = _charityFee;
        totalBuyFees = marketingBuyFee + rewardsBuyFee + liquidityBuyFee + developerBuyFee + charityBuyFee;
        require(totalBuyFees <= 20, "Must keep fees at 20% or less");
    }

    function updateSellFees(uint256 _marketingFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _developerFee, uint256 _charityFee) external onlyOwner {
        marketingSellFee = _marketingFee;
        rewardsSellFee = _rewardsFee;
        liquiditySellFee = _liquidityFee;
        developerSellFee = _developerFee;
        charitySellFee = _charityFee;
        totalSellFees = marketingSellFee + rewardsSellFee + liquiditySellFee + developerSellFee + charitySellFee;
    ...
    function updateDeveloperWallet(address newDeveloperWallet) external onlyOwner {
        excludeFromFees(newDeveloperWallet, true);
    ...
    function updateLiquidityReceiver(address newLiquidityReceiver) external onlyOwner {
        excludeFromFees(newLiquidityReceiver, true);
        emit liquidityReceiverWalletUpdated(newLiquidityReceiver, autoLiquidityReceiver);
    ...
    function _transfer(
        ...
        // block transfer if blacklisted
        require(!_isBlacklisted[from] || !_isBlacklisted[to], "Recipient is backlisted");
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForLiquidity + tokensForMarketing + tokensForRewards + tokensForDeveloper + tokensForCharity;
        ...
        uint256 ethForLiquidity = ethBalance - ethForMarketing - ethForRewards - ethForDeveloper - ethForCharity;
        ...
        (bool successTwo,) = address(developerWallet).call{value: ethForDeveloper}("");
        (bool successThree,) = address(charityWallet).call{value: ethForCharity}("");
        ...
        success = IERC20(token).transfer(address(autoLiquidityReceiver), tokenBalance);
    ...
    function removeFromBlacklist(address _user) external onlyOwner {
        require(_isBlacklisted[_user], "user already removed from blacklist");
