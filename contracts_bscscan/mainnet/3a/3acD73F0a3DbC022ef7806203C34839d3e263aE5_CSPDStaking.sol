...
contract CSPDStaking is Ownable {
    ...
    */
    ...
    /*
        Get amount of staked token per user
    */
    function allStakesInfo() external view returns (address[] memory, uint256[] memory){
        uint256 len = 0;
        for(uint256 i = 0; i < stakers.length; i ++){
            if(stakerInfos[stakers[i]].stake > 0){
                len ++;
            }
        }
    ...
    */
    function adminAllHistoriesOfStakes() external view onlyOwner returns(address[] memory, uint256[] memory, uint256[] memory, bool[] memory, uint256[] memory){
        ...
        for(uint256 i = 0; i < stakingHistories.length; i ++){
            users[i] = stakingHistories[i].user;
        ...
        return (users, times, amounts, status, fees);
    }

    /*
        Get all logs of harvest
    */
    function adminAllHistoriesOfHarvest() public view onlyOwner returns(address[] memory, uint256[] memory, uint256[] memory, bool[] memory, uint256[] memory){
        ...
        for(uint256 i = 0; i < harvestingHistories.length; i ++){
            users[i] = harvestingHistories[i].user;
