...
// CHANGE
contract OMEGACAKE is IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string constant _name = "OMEGACAKE";
    string constant _symbol = "$OMECAKE";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 100000000000 * (10 ** _decimals); // 100,000,000,000
    
    // LIMITTER
    uint256 public _maxBuyTxAmount = (_totalSupply * 20) / 1000; // 2% MAX 2,000,000,000
    uint256 public _maxSellTxAmount = (_totalSupply * 10) / 1000; // 1% MAX 1,000,000,000
    uint256 public _maxWalletSize = (_totalSupply * 40) / 1000; // 4% MAX 4,000,000,000
    ...
    uint256 buyLockTime = 5; // 5 seconds
    uint256 sellLockTime = 5; // 5 seconds
    ...
    constructor () {
        ...
        devFeeReceiver = address(0xDEb21e77b23AE6CCCd23e2c4e1BB98FE6B5eDBCc);
    ...
    function setDevFee(uint256 fee) external onlyOwner {
        ...
        uint256 simulatedFee = fee.add(liquidityFee).add(buybackFee).add(reflectionFee);
    ...
    function setBuybackFee(uint256 fee) external onlyOwner {
        // total fee should not be more than 20%;
        uint256 simulatedFee = fee.add(liquidityFee).add(devFee).add(reflectionFee);
        require(simulatedFee <= 2000, "Fees too high !!");
        buybackFee = fee;
        totalFee = simulatedFee;
    }
    function setLpFee(uint256 fee) external onlyOwner {
        // total fee should not be more than 20%;
        uint256 simulatedFee = fee.add(devFee).add(buybackFee).add(reflectionFee);
        require(simulatedFee <= 2000, "Fees too high !!");
        liquidityFee = fee;
        totalFee = simulatedFee;
    }
    function setReflectionFee(uint256 fee) external onlyOwner {
        // total fee should not be more than 20%;
        uint256 simulatedFee = fee.add(devFee).add(buybackFee).add(liquidityFee);
        require(simulatedFee < 2000, "Fees too high !!");
        reflectionFee = fee;
        totalFee = simulatedFee;
    }
