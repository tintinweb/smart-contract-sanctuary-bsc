...
contract xBADGE {
  string public name = "xBADGE Token";
  string public symbol = "xBADGE";
  ...
  modifier onlyOwner() {
    require(owners[msg.sender], "xBADGE Token: No owner privilege");
    _;
  }

  modifier onlyMinter() {
    require(minters[msg.sender], "xBADGE Token: No minter privilege");
    _;
  }

  modifier onlyOperator() {
    require(operators[msg.sender], "xBADGE Token: No operator privilege");
    _;
  }

  modifier Transferable(address _account) {
    require(allUnfrozen || !frozen[_account], "xBADGE Token: Account is frozen");
     _;
  }
  ...
  function removeOwner(address _owner) external onlyOwner returns (bool) {
    require(_owner != msg.sender, "xBADGE Token: Cannot remove yourself");
    owners[_owner] = false;
    return true;
  }
  ...
  function transferFrom(address _from, address _to, uint256 _value) external Transferable(_from) returns (bool) {
    require(_value <= allowed[_from][msg.sender], 'xBADGE Token: insufficient allowance');
  ...
  function _transfer(address _from, address _to, uint256 _amount) private returns (bool) {
    ...
    require(_amount <= balances[_from], 'xBADGE Token: insufficient balance');
