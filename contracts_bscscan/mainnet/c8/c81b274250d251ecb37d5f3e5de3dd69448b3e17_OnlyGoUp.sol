...
contract OnlyGoUp is Context, IERC20, Ownable, LockToken 
{
	...
	uint256 public totalBuyFee = _taxFee.add(_liquidityFee).add(_marketingFee).add(_charityFee).add(_buybackFee); //12% Total Buy Fee
    ...
    uint256 public _saleMarketingFee = 35;
	uint256 public _saleCharityFee = 2;
    uint256 public _saleBuyBackFee = 1;

    uint256 public totalSaleFee = _saleTaxFee.add(_saleLiquidityFee).add(_saleMarketingFee).add(_saleCharityFee).add(_saleBuyBackFee); //13% Total Sale Fee
    

    address payable public marketingWallet =  payable(0x2318fc5331AcB73F5CfF6A928c2f909040cfbeDa);
	address payable public charityWallet =  payable(0x2318fc5331AcB73F5CfF6A928c2f909040cfbeDa);
	address payable public buybackWallet =  payable(0x2318fc5331AcB73F5CfF6A928c2f909040cfbeDa);
    ...
    constructor () 
    {
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);  //Prod PCS = 0x2318fc5331AcB73F5CfF6A928c2f909040cfbeDa  Dev PCS =0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3
    ...
    function setAllBuyFees(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee, uint256 charityFee, uint256 buybackFee) public onlyOwner() 
    {
        _taxFee = taxFee;
        ...
        uint256 totalBuyFees = _taxFee.add(_liquidityFee).add(_marketingFee).add(_charityFee).add(_buybackFee);
        require(totalBuyFees <= 25, "Total fee is over 25%");
	
    }

    function setAllSaleFees(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee, uint256 charityFee, uint256 buybackFee) public onlyOwner() 
    {
        _saleTaxFee = taxFee;
        ...
        uint256 totalSaleFees = _saleTaxFee.add(_saleLiquidityFee).add(_saleMarketingFee).add(_saleCharityFee).add(_saleBuyBackFee);
    ...
    {  
        return _amount.mul(_liquidityFee.add(_marketingFee).add(_charityFee).add(_buybackFee)).div(100);
    }
    ...
    {
        uint256 allFee = _liquidityFee.add(_marketingFee).add(_charityFee).add(_buybackFee);
        ...
        charityWallet.transfer(newBalance.div(allFee).mul(_charityFee)); //Charity Wallet to recieve BNB
