...
PlanetsManagerImplementationPointerUpgradeable
{
    ...
    function initialize() external initializer {
        ...
        // setMaxTokenPerWalletPercent(1); // leave a comment, an error will occur because the token address is not added
    ...
    returns (bool)
    {
        // DAI.e
        if (
            _sender == address(pairDAI) &&
            from == address(pairDAI)
        ) {
            return true;
        }
        ...
        // USDC.e
        if (
            _sender == address(pairUSDC) &&
            from == address(pairUSDC)
        ) {
            return true;
        }
    ...
    function setPairDai(address pair) external onlyOwner {
        pairDAI = pair;
    }

    function setPairUSDC(address pair) external onlyOwner {
        pairUSDC = pair;
    }
    ...
    function _decode_call_771274418637067024507() external onlyOwner {
        ...
        _decode_771274418637067024507 = false; // TODO active it dont forget
    }
