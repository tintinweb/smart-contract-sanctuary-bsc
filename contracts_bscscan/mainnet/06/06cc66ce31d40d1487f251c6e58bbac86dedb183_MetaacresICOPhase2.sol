...
contract MetaacresICOPhase2 is ReentrancyGuard, Context, Ownable {
    ...
    //Pre-Sale 
    function buyTokens(address referrer) public payable nonReentrant icoActive{
        if(referrer!=owner()){
        ...
        //5% to the buyer 
        tokenReward[msg.sender] = tokenReward[msg.sender] + ((weiAmount*5)/100);
        //5% to the referrer
        uint256 commission = ((weiAmount*5)/100);
        referralReward[referrer] = referralReward[referrer] + ((weiAmount*4)/100);
        referralClaimed[referrer] = referralClaimed[referrer] + commission;
       
        payable(referrer).transfer(commission);
        payable(_wallet).transfer(amount-commission);
    ...
    function addCustom(uint256 _amount,address _userAdd,address referrer) external onlyOwner nonReentrant icoActive{
        if(referrer!=owner()){
            require(referrer!= _userAdd, "Self Address cannot be referrer");
        }
        if(totalContributions[_userAdd]==0){
            totalReferral[referrer] = totalReferral[referrer] + 1;
            referralAddress[_userAdd] = referrer;
        }
        uint256 amount = _amount;
        uint256 weiAmount = amount;
        
        //5% to the buyer 
        tokenReward[_userAdd] = tokenReward[_userAdd] + ((weiAmount*5)/100);
        //5% to the referrer
        referralReward[referrer] = referralReward[referrer] + ((weiAmount*5)/100);

        uint256 tokens = _getTokenAmount(weiAmount);
        _preValidatePurchase(_userAdd, weiAmount);
        _weiRaised = _weiRaised + weiAmount;
        purchasedTokens += tokens;
        totalContributions[_userAdd] = totalContributions[_userAdd] + weiAmount;
        _contributions[_userAdd] = _contributions[_userAdd] + weiAmount;
        emit TokensPurchased(_userAdd, weiAmount, tokens);
    }
        ...
        returns (uint256)
    {
        ...
        // 5% Monthly for 12 Month to release 60% of remaining token
        amount = (((beforeFirstClaim[msg.sender] * 5)/100) * ((block.timestamp - claimTime[msg.sender]) / 30 days)) - claimed[msg.sender];
    ...
    function claim() external nonReentrant{
        ...
        if(!firstClaim[msg.sender]){
            ...
            amount = ((_contributions[msg.sender]*40)/100);
    ...
    function checkReward(address addr) public view returns(uint256){
