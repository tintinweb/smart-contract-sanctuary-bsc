...
contract ApeSmol is Context, ERC20, Ownable {
    using SafeMath for uint256;
   ...
   ApeSmolDividendTracker public dividendTracker;
    ...
    constructor() ERC20 (unicode"A🅿️e Smol", unicode"🅱️uy di🅿️") {
        _buyRewardsFee = 2;
        _buyLiquidityFee = 3;
		_buyMarketingFee = 6;
        _buyDevFee = 1;
    	...
    	dividendTracker = new ApeSmolDividendTracker();

    	liquidityWallet = owner();
		
		marketingWallet = payable(0x7d80825b471cFc340b489467A2d1affAbB24CAf3);
        _devWallet = payable(0xd1c665CBB8AbFC848bfD4C680229A584bFfb9d61);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "ape: The dividend tracker already has that address");

        ApeSmolDividendTracker newDividendTracker = ApeSmolDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "ape: The new dividend tracker must be owned by the ape token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "ape: The router already has that address");
    ...
    function addToBlacklist(address account, bool blacklisted) public onlyOwner {
        require(_isBlacklisted[account] != blacklisted, "ape: Account is already the value of 'blacklisted'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ape: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "ape: The liquidity wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "ape: The marketing wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ape: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ape: Cannot update gasForProcessing to same value");
...
contract ApeSmolDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("ApeSmol_Dividend_Tracker", "ApeSmol_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function withdrawDividend() public pure override {
        require(false, "ApeSmol_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ape contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 0 && newClaimWait <= 86400, "ApeSmol_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "ApeSmol_Dividend_Tracker: Cannot update claimWait to same value");
