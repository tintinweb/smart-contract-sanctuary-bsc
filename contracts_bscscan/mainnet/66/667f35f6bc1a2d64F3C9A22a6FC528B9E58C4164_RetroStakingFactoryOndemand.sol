...
contract RetroStakingLock is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    function poolHIsLock() public view returns (bool) {
        bool statusPool;
        if (hasPoolHL) {
            statusPool = bonusEndBlock >= block.number;
    ...
    function payOrLockupPending() internal {
        UserInfo storage user = userInfo[msg.sender];
...
contract RetroStakingFactoryOndemand is Ownable {
    event NewRetroStakingLockContract(address indexed retrostaking);
    ...
    function deployPool(
        ...
        bytes memory bytecode = type(RetroStakingLock).creationCode;
        ...
        RetroStakingLock(retrostakingAddress).initialize(
            _stakedToken,
            _rewardToken,
            _rewardPerBlock,
            _startBlock,
        ...
        emit NewRetroStakingLockContract(retrostakingAddress);
    }
}