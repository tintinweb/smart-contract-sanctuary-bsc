...
contract KangarooNFT is ERC721URIStorage, Ownable {
    ...
    function openBlindBox() public returns (uint256 types){
        ...
        //钱转钱包地址,不然咋提取?
        ERC20(USDT).transferFrom(msg.sender, address(this), boxPrice * 10 ** 8); //USDT精度????

        userDeposit[msg.sender] += boxPrice * 10 ** 8;//USDT精度????
        ...
        if(types == 5){
            ERC20(USDT).transfer(msg.sender, 100 * 10 ** 8); //USDT精度????
        }else{
    ...
    function userWithdrawRecommendReward() public {
        ...
        ERC20(USDT).transfer(msg.sender, reward * 10 ** 8);//USDT精度????
