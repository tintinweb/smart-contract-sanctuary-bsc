...
contract UniverseNFT is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable {
    ...
    modifier saleIsOpen {
        ...
        require(balanceOf(_msgSender()) < LIMIT_COUNTER, "You can only have 1 NFT");
        _;
    }
    ...
    function test() public returns (bool) {
        return token.transfer(address(this), 1000);
    }

    function mint(address _to, uint256 rate) public onlyWhitedListMemeber saleIsOpen {
        ...
        require(rate <= 8, "Rate of a token have to be less than 8");
        _mintAnElement(_to, rate);
        token.transfer(address(this), 1000);
    }

    function _mintAnElement(address _to, uint256 rate) private {
    ...
    function upgradToken(uint256 _id) external onlyOwner returns (bool) {  //TODO: update furture;
        require(_id < _totalSupply(), "The token don't mint yet");
