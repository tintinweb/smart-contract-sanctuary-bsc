...
contract Trade {
    ...
    mapping(address => mapping(uint256 => uint256)) private royaltyFeeForImported;
    mapping(address => mapping(uint256 => address)) private creatorListForImported;
    struct Fee {
        uint platformFee;
        uint assetFee;
        uint royaltyFee;
    ...
    function verifySignerSign(address nftAddress, uint256 tokenId, address royaltyAddress, uint256 royaltyFee, Sign memory signerSign) internal view {
        bytes32 hash = keccak256(abi.encodePacked(nftAddress, tokenId, royaltyAddress, royaltyFee, signerSign.nonce));
        require(signer == getSigner(hash, signerSign), "signer sign verification failed");
    }
    ...
    function getFees(Order memory order) internal view returns(Fee memory){
        ...
        if(order.nftType == BuyingAssetType.ERC721) {
            if(!order.isImport) {
                ...
                royaltyPermille = royaltyFeeForImported[order.nftAddress][order.tokenId];
                tokenCreator = creatorListForImported[order.nftAddress][order.tokenId];
            }
        }
    ...
    function setRoyalty(address nftAddress, uint256 tokenId, address royaltyAddress, uint256 royaltyFee, Sign memory signerSign) public returns(bool) {
        require(nftAddress != address(0),"Asset Address in zero");
        require(royaltyAddress != address(0),"royalty Address in zero");
        verifySignerSign(nftAddress, tokenId, royaltyAddress, royaltyFee, signerSign);
        royaltyFeeForImported[nftAddress][tokenId] = royaltyFee;
        creatorListForImported[nftAddress][tokenId] = royaltyAddress;
        return true;
    }
}