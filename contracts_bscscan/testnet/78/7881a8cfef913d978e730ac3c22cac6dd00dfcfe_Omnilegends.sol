...
contract Omnilegends is Initializable, ERC721Upgradeable , ERC721EnumerableUpgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    struct Treasure {
        uint256 price; // price of treasure
        uint256 totalLegends; //total legends for this treasure
        uint256 totalLegendsRemaining;
        uint256[] rarities; // rarities for this treasure
        uint256[] total; // totals for each rarities
        uint256[] remaining; // remainings for each rarities
    }

    mapping(uint256=>Treasure) episodeTreasure;
    ...
    function getLegendTypeFromRandomSeed(uint256 seed, uint256 episode_) private view returns (uint256 _legendType) {
        uint256 remainingLegend = _episodes[episode_].remainingLegend; // total remaining
        uint256 range = seed.mod(remainingLegend).add(1); // range
        uint256[] memory charTypesPerEpisode = _episodesCharacterTypes[episode_]; // types of rarity for episode
        uint256 runningTotal = 0;
        ...
        for(uint256 idx = 0 ; idx < charTypesPerEpisode.length ; idx++){
            ...
            uint256 cremaining = _charactersPerEpisode[episode_][(ctype)].remaining; //total remainig for each rarity for specific episode
    ...
    * @dev summon legend for specific episode
    ...
    // 
    // 850
    // buy lunar new year lucky ninja treasure
    function buyTreasure(address buyer, uint256 episodeNumber, uint256 seed) external onlySummoner whenNotPaused returns (uint256)  {
        // address buyer = _msgSender();
        Treasure memory treasure = episodeTreasure[episodeNumber];

        require(treasure.totalLegendsRemaining > 0,"No legends available");
        
        uint256 rarityGet =  getRarityFromRandomSeed(seed, episodeNumber); // rarity
        treasure.totalLegendsRemaining = treasure.totalLegendsRemaining.sub(1);

        for(uint256 idx = 0; idx < treasure.rarities.length ; idx++){
            if(rarityGet == treasure.rarities[idx]){
                treasure.remaining[idx] = treasure.remaining[idx].sub(1);
                break;
            }
        }

        uint256 legendId = _summonLegendOnEpisode(buyer, rarityGet, true, episodeNumber);

        emit TreasureSold(buyer, treasure.price, legendId, episodeNumber);

        return legendId;
    }

    function createTreasure(uint256 episodeNumber, uint256 price, uint256 total, uint256[] calldata rarities, uint256[] calldata totals) external onlyOwner {
        require(price>0,"invalid price");
        require(total>0,"invalid totals");
        require(_currentEpisode>=episodeNumber,"invalid totals");
        episodeTreasure[episodeNumber] = Treasure(price, total,total,rarities, totals, totals);
    }

    function updateEpisodePriceTreasure(uint256 episodeNumber , uint256 price) external onlyOwner {
        require(price>0,"invalid price");
        episodeTreasure[episodeNumber].price = price;
    }

    function getEpisodeTreasureInfo(uint256 episodeNumber) external view returns (Treasure memory) {
        return episodeTreasure[episodeNumber];
    }

    function getEpisodeTreasurePrice(uint256 episodeNumber) external view returns (uint256) {
        return episodeTreasure[episodeNumber].price;
    }
    
    function getRarityFromRandomSeed(uint256 seed, uint256 episodeNumber) private view returns (uint256 _legendType) {
        uint256 remainingLegend = episodeTreasure[episodeNumber].totalLegendsRemaining; 
        uint256 range = seed.mod(remainingLegend).add(1); // range
        uint256[] memory charTypesPerEpisode = episodeTreasure[episodeNumber].rarities;
        uint256 runningTotal = 0;
        uint256 currentMin = 0;
        uint256 currentMax = 0;
        for(uint256 idx = 0 ; idx < charTypesPerEpisode.length ; idx++){
            uint256 ctype = charTypesPerEpisode[idx]; // rarity
            uint256 cremaining = episodeTreasure[episodeNumber].remaining[idx]; //total remainig for each rarity for specific episode
            currentMin = runningTotal.add(1);
            currentMax = runningTotal.add(cremaining);
            runningTotal = runningTotal.add(cremaining);
            if( range >= currentMin && range <= currentMax) {
