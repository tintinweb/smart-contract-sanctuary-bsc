...
contract PrivateSale is Auth {
  ...
  function joinPool() public payable {
    ...
    require(msg.value >= min_buy_amount_estimate.div(percDenominator).mul(etherDenominator), "PrivateSale:joinPool - Can't be under min_buy_amount");
    require(msg.value <= max_buy_amount_estimate.div(percDenominator).mul(etherDenominator), "PrivateSale:joinPool - Can't be over max_buy_amount");
    require(isWhitelisted(msg.sender), 'PrivateSale:joinPool - User is not whitelisted');

    uint256 amountToBuy = (msg.value).div(rateDenominator).mul(rateMAMPperBNB);
