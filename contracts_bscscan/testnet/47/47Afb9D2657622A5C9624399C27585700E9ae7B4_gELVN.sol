...
contract gELVN is IERC20, Ownable {
    ...
    function setOriginalToken(address payable _originalToken) external onlyOwner {
     
        _ensureNotZeroAddress(address(_originalToken));
        require(address(elvnToken) == address(0), "ERR_ORIGINAL_TOKEN_ALREADY_SET");
       // require(_originalToken.decimals() == decimals(), "ERR_DECIMALS_MISMATCH");

        elvnToken = ELVN(_originalToken);
    ...
    function swapToOriginal(uint256 _amount) external {
        ...
        require(balanceOf(_msgSender()) >= _amount, "Insufficient gELVN balance");
        require(elvnToken.balanceOf(address(this)) >= _amount, "Insufficient ELVN balance on contract");
        ...
        elvnToken.transfer(_msgSender(), _amount - feeAmount);
    ...
    function swapFromOriginal(uint256 _amount) external {
        _ensureOriginalTokenSet();
        require(_amount != 0, "ERR_ZERO_SWAP_AMOUNT");
        require(elvnToken.balanceOf(_msgSender()) >= _amount, "Insufficient balance");
        require(balanceOf(address(this)) >= _amount, "Insufficient gELVN balance on frozen account");

        address msgSender = _msgSender();

        elvnToken.transferFrom(msgSender, address(this), _amount);
    ...
    function _ensureOriginalTokenSet() private view {
        require(address(elvnToken) != address(0), "ERR_ORIGINAL_TOKEN_NOT_SET");
    }
