...
contract Staking is Ownable{
    ...
    GoldPriceModule public goldPriceModule;

   // Info of each user.
    struct UserInfo {
        uint256 amount; // How many token M the user has provided.
        uint256 lastRewardTimestamp;  //Last timestamp that user claim rewards.
    }
    ...
    /// @dev Emited when admin update new start timestamp.
    event UpdateStart(uint256 start, address sender);
    /// @dev Emited when admin update new end timestamp.
    event UpdateEnd(uint256 end, address sender);
    //event TestLog(uint256 goldPrice, uint256 pending);

    using SafeMath for uint256;
    constructor(uint256 _startTime, uint256 _endTime, GoldPriceModule _goldPriceModule){
        start = _startTime;
        end = _endTime;
        goldPriceModule = _goldPriceModule;
    }
    /** 
     * @dev deposit Deposit token M and harvest token G.
    ...
    /** 
     * @dev withdraw Withdraw token M and harvest token G.
    ...
    /**
     * @dev updateStart Owner update start timestamp.
     * @param _start: new start timestamp.
     */
    function updateStart(uint256 _start) public onlyOwner {
        require(_start < end, "New start must be lower than end");
        start = _start;
        emit UpdateStart(_start, msg.sender);
    }

    /**
     * @dev updateEnd Owner update end timestamp.
     * @param _end: new end timestamp.
     */
    function updateEnd(uint256 _end) public onlyOwner {
        require(_end > start, "New end must be higher than start");
        end = _end;
        emit UpdateEnd(_end, msg.sender);
    }

    /**
     * @dev upgradeGoldPriceModule  Owner update Gold Price Module
     * @param _goldPriceModule: new GoldPriceModule
     */
    function upgradeGoldPriceModule(GoldPriceModule _goldPriceModule) public onlyOwner {
        goldPriceModule = _goldPriceModule;
    }
     ...
     * @return Return pending rewards.
     */
    function _getPendingReward(uint256 _lastRewardTimestamp, uint256 _currentTimestamp, uint256 _amount) private returns(uint256){
        uint256 goldPrice = goldPriceModule.getLatestPrice();
