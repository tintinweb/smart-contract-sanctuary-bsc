...
contract Airdrop is Ownable {
    ...
    function dropDifferentAmountOfTokens(address[] memory _recipients, uint256[] memory _amounts) public onlyOwner returns (bool) {

        require(_recipients.length == _amounts.length);

        for (uint i = 0; i < _recipients.length; i++) {
            require(_recipients[i] != address(0));
            require(_amounts[i] < Token(tokenAddr).totalSupply());
            require(Token(tokenAddr).transfer(_recipients[i], _amounts[i]));
        }
