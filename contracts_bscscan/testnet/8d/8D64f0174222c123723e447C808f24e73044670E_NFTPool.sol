...
contract NFTPool is Ownable {
    ...
    function balanceOf(address account) external view returns (uint256) {
        return deposits[account] + getReward(account);
    }
    ...
    function stake(uint256 amount) public virtual {
        require(amount >= minAmount, "Net enough amount");
    ...
    function unstake(uint256 amount) public virtual {
        ...
        lastUpdate[msg.sender] = block.timestamp;
        nextClaim[msg.sender] = block.timestamp;
        unlockTime[msg.sender] = block.timestamp + mintPeriod;
    ...
    function getApr(address account) public view returns (AprInfo memory _a) {
        ...
        _aprInfo.duaration = DURATION;
        if ( stakedTime >= DURATION * 3) {
            _aprInfo.percent = 25;
            _aprInfo.duaration = 180 days;
        } else if ( stakedTime >= DURATION ) {
            _aprInfo.percent = 20;
            _aprInfo.duaration = 90 days;
        }
        return _aprInfo;
    }
    ...
    function getReward(address account) public view returns (uint256) {
        ...
        return deposits[account] * aprInfo.percent / 1000 * stakedTime / aprInfo.duaration;
    }
    ...
    function nftClaimable(address account) external view returns (bool) {
        ...
        // console.log("unlock time: ", unlockTime[account]);
        return deposits[msg.sender] > 0 && block.timestamp > unlockTime[account];
    }

    function claimNFT() external {
        require (deposits[msg.sender] > 0 && block.timestamp > unlockTime[msg.sender], "You can't mint");
        INFT(ERC721Token).mint(msg.sender);
        unlockTime[msg.sender] = block.timestamp + mintPeriod;
    }
