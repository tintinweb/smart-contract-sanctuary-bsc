...
contract ROI {
    ...
    uint8 public MAX_TEAM_MEMBERS;
    uint8 public MAX_LENGTH_TEAM_NAME;
    ...
    uint256 public MAX_PAYOUT_PROJECT;

    uint256 public MAX_PAYOUT_TOKENS;
    uint256 public AIRDROP_MINAMOUNT;
    bool public TOPSPONSOR_POOL_ENABLED;
    uint8 public TOPSPONSOR_POOL_PERCENT;
    ...
    function initialize(address erctokenAddress, address payable ownerAddress, address payable projectAddress, address payable marketingAddress) external {
        require(!isContract(ownerAddress) && !isContract(projectAddress) && !isContract(marketingAddress));
        erctoken = erctokenAddress;
		owner = ownerAddress;
		project = projectAddress;
		marketing = marketingAddress;
        token_PSTN = IERC20(erctoken);

        REFERRAL = 50;
        PROJECT = 70;
        MARKETING = 230;
        AIRDROP = 50; // 5% Sustainability Tax on airdrop
        REINVEST_BONUS = 50;
        MAX_PAYOUT = 3650;
        BASE_PERCENT = 10;
        TIME_STEP = 1 days;
        ...
        MAX_TEAM_MEMBERS = 15;
        MAX_LENGTH_TEAM_NAME = 10;
        MAX_LENGTH_NICKNAME = 10;
        AIRDROP_MINAMOUNT = 1;
		pool_last_draw = block.timestamp;
		total_users = 1;
		
	    MIN_INVEST = 1 ether; //1 PSTN
		MAX_WALLET_DEPOSIT = 5000 ether; //5,000 PSTN
		MAX_PAYOUT_PROJECT = 100000;
        MAX_PAYOUT_TOKENS = 100000; // if this is reached, new deposits and reinvest are not possible

        TOPSPONSOR_POOL_ENABLED = true;
        TOPSPONSOR_POOL_PERCENT = 3;
    ...
    function depositWithNickname(string calldata _nickname, uint256 _amount) external {
        require(started, "Contract not yet started.");
    ...
    function _deposit(address _addr, uint256 _amount) private {
        ...
        require(users[_addr].payouts <= MAX_PAYOUT_TOKENS, "Max payout reached.");
        ...
        //transfer amount from investor to contract.
        token_PSTN.transferFrom(address(_addr), address(this), _amount);
    ...
    function _poolDeposits(address _addr, uint256 _amount) private {
        if(!TOPSPONSOR_POOL_ENABLED){
            return;
        }

        pool_balance += _amount.mul(TOPSPONSOR_POOL_PERCENT).div(100);
    ...
    function withdraw() external {
        ...
        // Pool payout
        if( TOPSPONSOR_POOL_ENABLED && users[msg.sender].payouts < max_payout && users[msg.sender].pool_bonus > 0) {
        ...
        token_PSTN.transfer(msg.sender, payout);
    ...
    function reinvest() external {
        ...
        require(users[msg.sender].payouts <= MAX_PAYOUT_TOKENS, "Max payout reached.");
		...
		users[msg.sender].total_payouts += to_reinvest; // new in V2
    ...
    function airdrop(address payable _to, uint256 _amount) external {

        address _addr = msg.sender;

        require(_amount >= AIRDROP_MINAMOUNT, "minimum not reached");

        // transfer to recipient        
        uint256 sustainability_tax = _amount.mul(AIRDROP).div(PERCENTS_DIVIDER); // tax on airdrop if enabled
        ...
        //transfer amount from address to contract.
        token_PSTN.transferFrom(address(_addr), address(this), _amount);

        //Fund to airdrop_bonus (not a transfer). user is able to withdraw or reinvest
    ...
    function teamAirdrop(uint256 teamId, uint256 _amount, bool excludeOwner) payable external {
    ...
    function payoutToReinvest(address _addr) view external returns(uint256 payout) {
        ...
        uint256 max_payout = ((_addr == project || _addr == marketing || _addr == owner)  
                ? this.maxPayoutP(users[_addr].deposit_amount) 
                : this.maxPayoutOf(users[_addr].deposit_amount));
    ...
    function fees(uint256 amount) internal returns(uint256){
        ...
        token_PSTN.transfer(project, proj);
		token_PSTN.transfer(marketing, market);
        return proj.add(market);
    }

    function withdrawalTaxPercentage(uint256 to_payout) view external returns(uint256 finalPayout) {
      uint256 contractBalance = token_PSTN.balanceOf(address(this));
   ...
   function changeTeamName(uint256 teamId, string memory name) external {
    ...
    function userInfo2(address _addr) view external returns(uint256 last_airdrop, uint8 teams_counter, TeamInfo[] memory member_of_teams, string memory nickname, uint256 last_airdrop_received) {

        return (airdrops[_addr].last_airdrop, user_teams_counter[_addr], user_teams[_addr], nicknames[_addr], airdrops[_addr].last_airdrop_received);
    }
...
*/
    ...
    function SET_BASE_PERCENT(uint8 value) external{
        require(msg.sender == owner, "Admin use only");
        require(value >= 10);
        BASE_PERCENT = value;
    }

    function SET_AIRDROP_MINAMOUNT(uint8 value) external{
        require(msg.sender == owner, "Admin use only");
        AIRDROP_MINAMOUNT = value;
    }

    function SET_ERC20TOKEN(address value) external{
        require(msg.sender == owner, "Admin use only");
        erctoken = value;
        token_PSTN = token_PSTN = IERC20(erctoken);
    }
    
    function SET_TOPSPONSOR_POOL_ENABLED(bool value) external{
        require(msg.sender == owner, "Admin use only");
        TOPSPONSOR_POOL_ENABLED = value;
    }

    function SET_TOPSPONSOR_POOL_PERCENT(uint8 value) external{
        require(msg.sender == owner, "Admin use only");
        TOPSPONSOR_POOL_PERCENT = value;
    }

    function SET_MAX_PAYOUT_TOKENS(uint256 value) external{
        require(msg.sender == owner, "Admin use only");
        MAX_PAYOUT_TOKENS = value;
    }
