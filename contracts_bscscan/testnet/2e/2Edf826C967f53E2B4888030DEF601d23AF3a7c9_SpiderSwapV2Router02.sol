...
// File: contracts/swap/interfaces/ISpiderSwapV2Pair.sol
...
// File: contracts/swap/libraries/SpiderSwapV2Library.sol
...
library SpiderSwapV2Library {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'8ea7229054ef0dafecb90df58d1ebef781ca946087b3e1873adc67bf5c2497be' // init code hash
...
// File: contracts/swap/SpiderSwapV2Router02.sol
...
contract SpiderSwapV2Router02 is ISpiderSwapV2Router02 {
    ...
    constructor(address _factory, address _WETH) public {
        factory = _factory;
        WETH = _WETH;
        ISpiderSwapV2Factory(_factory).setRouter(address(this));
     }
    ...
    function removeLiquidity(
        ...
        if(amountAMin > 0){
            amountAMin = amountAMin.sub(amountAMin.mul(ISpiderSwapV2Factory(factory).InOutTotalFee()).div(ISpiderSwapV2Factory(factory).PERCENT100()));
        }
        if(amountBMin > 0){
            amountBMin = amountBMin.sub(amountBMin.mul(ISpiderSwapV2Factory(factory).InOutTotalFee()).div(ISpiderSwapV2Factory(factory).PERCENT100()));
        }
        ...
        returns (uint[] memory amounts)
    {
        ...
        amounts = SpiderSwapV2Library.getAmountsOut(factory, msgvalue, path);        
        require(amounts[amounts.length - 1] >= amountOutMin, 'SpiderSwapV2Router: INSUFFICIENT_OUTPUT_AMOUNT');      
    ...
    function takeAddLiquidityFee(address _token0, address _token1, uint256 _amount0, uint256 _amount1, bool isEth) internal returns(uint256, uint256){
       if(ISpiderSwapV2Factory(factory).pause()== false){
            ...
            TransferHelper.safeTransferFrom(_token0, msg.sender, converter, _totalFees0);
            if(!isEth){
                TransferHelper.safeTransferFrom(_token1, msg.sender, converter, _totalFees1);
            }else{
                TransferHelper.safeTransfer(_token1, converter, _totalFees1);
            }
   ...
   function takeSwapFee(address token, uint256 amount, bool isEth) internal returns(uint256){
        if(ISpiderSwapV2Factory(factory).pause() == false){
            ...
            uint256 totalFees = amount.mul(ISpiderSwapV2Factory(factory).swapTax()).div(PERCENT100);
            if(isEth){
                TransferHelper.safeTransfer(token, ISpiderSwapV2Factory(factory).SwapTaxConverter(), totalFees);
            }else {
                TransferHelper.safeTransferFrom(token, msg.sender,ISpiderSwapV2Factory(factory).SwapTaxConverter(), totalFees);
            }
