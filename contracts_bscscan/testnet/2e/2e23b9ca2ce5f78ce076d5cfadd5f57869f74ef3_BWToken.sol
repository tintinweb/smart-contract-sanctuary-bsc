...
// BW Token.
contract BWToken is ERC20("BWToken", "BW"), Ownable {
    uint256 private _cap = 1 * (10 ** 9) * (10 ** 18);
    ...
    modifier onlyMintOperator() {
        require(mintOperators[msg.sender] == true || msg.sender == owner(), "!mintOperators");
        _;
    }
    ...
    /// @notice Creates `_amount` token to `_to`. Must only be called by the mint operator.
    function mint(address _to, uint256 _amount) public onlyMintOperator {
        _mint(_to, _amount);
    }
