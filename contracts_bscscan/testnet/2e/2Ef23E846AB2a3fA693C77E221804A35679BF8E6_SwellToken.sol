...
contract Token is ERC20 {
    ...
    constructor() public {
        _name = "Swell_Test_Name";
        _symbol = "Swell_Test";
        _decimals = 18;
        _limitSupply = 10000000 ether;
    }
...
contract SwellToken is Token {
    ...
    uint public MIN_TOKEN_STAKE_AMT = 5 ether;
    ...
    function stakeToken(uint tokenAmount) public {
        ...
        //configuration: must meet min stake requirement
        require (tokenAmount >= MIN_TOKEN_STAKE_AMT, concatenate("Must meet minimum token balance: ", uintToStr(MIN_TOKEN_STAKE_AMT)));
...
// SET Functions
    ...
    function SET_MIN_INVEST_AMOUNT(uint256 value) external {
        ...
        require(value >= MIN_INVEST_AMOUNT);
        MIN_INVEST_AMOUNT = value * 1 ether;
    }

    function SET_MIN_TOKEN_STAKE_AMOUNT(uint256 value) external {
        require(msg.sender == ADMIN, "Admin use only");
        require(value >=  MIN_TOKEN_STAKE_AMT);
        MIN_TOKEN_STAKE_AMT = value * 1 ether;
    }

    function SET_SELL_LIMIT(uint256 value) external {
        require(msg.sender == ADMIN, "Admin use only");
        require(value >= SELL_LIMIT);
