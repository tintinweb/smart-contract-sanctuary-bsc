...
Manual Task Approval
    getManualTaskListForApprovalCount(uint task)
    getManualTaskListItemForApproval(uint task, uint index)
    setManualTaskApproval(uint task, address player, uint manualTaskListIndex, uint value)
    requestManualTaskApproval(uint task)
    isWaitingForManualTaskApproval(uint task, address player)
    getManualTaskIsApproved(uint task, address player)
*/
...
contract CommunityTasks
{
    ...
    *  *****************************/
    ...
    function updateLeaderboardScore(address player) private
    {
        ...
        for (uint ix = 0; ix < leaderboardLength; ix++) 
        {
            ...
            if ( playerClaimedAmount[currentUser] < playerClaimedAmount[msg.sender]) 
            {
                // shift leaderboard
    ...
    *  *****************************/
    ...
    function isWaitingForManualTaskApproval(uint task, address player) public view returns (uint)
    {
