...
contract DogeChampionsMarketplace is ReentrancyGuard, ERC1155Holder
{
    ...
    // 10K from first mint, 10K from final mint, and 500 giveaway in total
    uint256 public maximumPossibleNFTSale = 20500;
...
contract DogeChampionsNFT is ERC721URIStorage, ERC721Enumerable
{
    ...
    mapping(address => bool) public initialMintDiscountWhitelist;
    mapping(address => bool) public initialMintGiveawayWhitelist;
    mapping(address => bool) public finalMintDiscountWhitelist;
    mapping(address => bool) public finalMintGiveawayWhitelist;
    ...
    function normalMint(uint256 amount) public payable returns (uint256)
    {
        ...
        if(mintPhase.current() < 2)
        {
            userToNumberOfFirstPhaseMints[msg.sender] = userToNumberOfFirstPhaseMints[msg.sender] + 1;
        }
        else
        {
            userToNumberOfSecondPhaseMints[msg.sender] = userToNumberOfSecondPhaseMints[msg.sender] + 1;
        }
     ...
     */
    function discountMint() public payable returns (uint256)
    {
        require(!ignoreDiscountWhitelist, "Discount period ended.");
        if(mintPhase.current() < 2)
        {
            require(initialMintDiscountWhitelist[msg.sender] == true, "You are not in whitelist.");
            initialMintDiscountWhitelist[msg.sender] = false;
        }  
        else
        {
            require(finalMintDiscountWhitelist[msg.sender] == true, "You are not in whitelist.");
            finalMintDiscountWhitelist[msg.sender] = false;
        }
        require(isPresaleAvailable == true, "Presale is not started yet.");
        require(msg.value == discountedMintPrice, "Please submit the exact price to mint a Doge Champion.");
        require(numberOfDiscountMints.current() < discountMintLimit, "We are out of stock!");
    ...
    function giveawayMint() public returns (uint256)
    {
        if(mintPhase.current() < 2)
        {
            require(initialMintGiveawayWhitelist[msg.sender] == true, "You are not in whitelist.");
            initialMintGiveawayWhitelist[msg.sender] = false;
        }  
        else
        {
            require(finalMintGiveawayWhitelist[msg.sender] == true, "You are not in whitelist.");
            finalMintGiveawayWhitelist[msg.sender] = false;
        }
        require(isMintAvailable == true, "Minting is not started yet.");
     ...
     */
    function getCurrentNormalMintCount() public view returns (uint256)
    {
        return numberOfNormalMints.current();
    }
     ...
     */
    function setAddressToDiscountWhitelist(address walletAddress) public onlyOwner
    {
        if(mintPhase.current() < 2){
            initialMintDiscountWhitelist[walletAddress] = true;
        }else {
            finalMintDiscountWhitelist[walletAddress] = true;
        }
    }
     ...
     */
    function setAddressToGiveawayWhitelist(address walletAddress) public onlyOwner
    {
        if(mintPhase.current() < 2){
            initialMintGiveawayWhitelist[walletAddress] = true;
        }else {
            finalMintGiveawayWhitelist[walletAddress] = true;
        }
    }
}
