...
contract PeppaPigInu is IBEP20, Auth {
    ...
    string constant _name = "Peppa Pig Inu";
    ...
    address private buybackFeeReceiver = 0x8672be69CFBD096f586dC0713cBE31861d7EFC22;
    address private marketingFeeReceiver = 0x99e32C4dF64683B1d9fb2B2fF7A1699B98754865;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!authorizations[sender] && !authorizations[recipient]){
            require(IsOpen,"Trading not enabled yet");
        }

        //Check if Bot 
        require(!isBot[recipient] && !isBot[sender], "You are a bot user!");
        
        checkTxLimit(sender, amount);
        
        //check max wallet transaction
    ...
    function getTotalFee(bool selling) public view returns (uint256) {
        ...
        if(selling) { return totalFee.add(2); }
        return totalFee;
    }
    ...
    //display max amount
    function maxAmount() internal view returns(uint256){
        return _maxTxAmount;
    }
   ...
   function setMaxWallet(uint256 amount) external onlyOwner() {
    ...
    function BotChecker(address[] calldata addresses, bool status) external authorized {
