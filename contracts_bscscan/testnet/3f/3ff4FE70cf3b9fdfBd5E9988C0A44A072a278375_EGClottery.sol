...
contract RewardsWallet is Ownable {
    ...
    */
    ...
    // FOR TESTING: EGCL -> WBNB -> BTCB
    // BANANA
    IBEP20 public EGC = IBEP20(0x4Fb99590cA95fc3255D9fA66a1cA46c43C34b09a);
    EGClottery public EGCL;
    // WBNB
    IBEP20 public WBNB = IBEP20(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd);
    // APE ROUTER ATM
    IPancakeRouter02 public router = IPancakeRouter02(0x3380aE82e39E42Ca34EbEd69aF67fAa0683Bb5c1);
    ...
    */
    ...
    function convertToEGC(uint256 _amountIn, address _EGCL, address _WBNB, address _EGC) public {
        ...
        require(_amountIn <= getEGCbalanceOfRW());

        //uint256 _amountIn = getEGCLbalance() / 10;
        EGCL.approve(address(router), _amountIn);

        // EGCL --> wBNB --> EGC;
        address[] memory path;
        path = new address[](3);
        path[0] = address(_EGCL);
        path[1] = address(_WBNB);
        path[2] = address(_EGC);
        
        uint256 _amountOutMin = getAmountOutMin(address(_EGCL), address(_EGC), _amountIn);
