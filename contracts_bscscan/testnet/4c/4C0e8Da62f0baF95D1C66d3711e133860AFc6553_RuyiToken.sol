...
contract RuyiToken is Context, IERC20, Ownable, ITokensRecoverable {
    using SafeMath for uint256;
    using Address for address;
    ...
    string private _name = "RuyiToken";
    string private _symbol = "RYB";
    ...
    bool public initialAdd = true;
    uint256 private maxTxAmt = 1000000000000000;
    bool public pauseTrading = false;
    ...
    constructor(
        ...
        _busd = IERC20(BUSD);
        _busd.approve(address(router), uint256(-1));
    ...
    function _transfer(
        ...
        require(!pauseTrading, "Trading is paused");
        ...
        if ((block.timestamp < lockTime) && (!antiSnipeWhitelist[recipient])) {
            require(sender != uniswapV2Pair, "Liquidity is locked or limited");
        }
        //if lock is active AND recipient IS whitelisted, recipient can buy up to 1M
        if ((block.timestamp < lockTime) && (antiSnipeWhitelist[recipient])) {
            require(amount <= maxTxAmt, "Can't buy more than 1Million");
        }
        ...
        if (
            ...
            //tokens to receiver
            _tOwned[sender] = senderBalance.sub(amount);
    ...
    // Integers that make up the Tax on transfers.
    ...
    function setLiquidityBuyPoolTax(uint256 _liquidityPoolTax)
        external
        onlyOwner
    {
    ...
    function setLiquiditySellPoolTax(uint256 _liquidityPoolTax)
        external
        onlyOwner
    {
    ...
    function _tokenTransferStandard(
        ...
        if (sender == uniswapV2Pair) {
            StakingPoolAmount = amount.mul(StakingBuyPoolTax).div(100);
            marketingAmount = amount.mul(MarketingBuyTax).div(100);
            AddToLiquidityAmount = amount.mul(LiquidityBuyPoolTax).div(100);
        } else {
            StakingPoolAmount = amount.mul(StakingSellPoolTax).div(100);
            marketingAmount = amount.mul(MarketingSellTax).div(100);
            AddToLiquidityAmount = amount.mul(LiquiditySellPoolTax).div(100);
        }

        //calculate amount to send to receiver
        amount = amount.sub(StakingPoolAmount).sub(marketingAmount).sub(
            AddToLiquidityAmount
        );

        //tokens to StakingPool
        _tOwned[sender] = _tOwned[sender].sub(StakingPoolAmount);
        _tOwned[StakingPool] = _tOwned[StakingPool].add(StakingPoolAmount);

        //Swap and Liquify to marketingWallet when a buy or sell
        if (sender == uniswapV2Pair || recipient == uniswapV2Pair) {
            swapTokensForBUSD(marketingAmount, MarketingWallet);
        ...
        _tOwned[sender] = _tOwned[sender].sub(marketingAmount);

        // add tokens to contract for future liquification
        _tOwned[sender] = _tOwned[sender].sub(AddToLiquidityAmount);
        _tOwned[LiquifyWallet] = _tOwned[LiquifyWallet].add(
            AddToLiquidityAmount
        );
    ...
    bool public isCooldownTimerActive = false;
    uint256 internal Timer;

    function timeLeftOnCooldown() external view returns (uint256) {
    ...
    function addInitialLiquidity(uint256 tokenAmount, uint256 busdAmount)
        public
        onlyOwner
    {
        _approve(address(this), address(uniswapV2Router), tokenAmount);
        _approve(address(this), address(BUSD), busdAmount);
        ...
        if (initialAdd) {
            initialAdd = false;
            lockTime = block.timestamp + 45 seconds;
        }
    }
