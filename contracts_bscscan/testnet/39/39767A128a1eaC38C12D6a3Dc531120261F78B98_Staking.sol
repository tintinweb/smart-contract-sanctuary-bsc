...
contract Staking is Ownable, ReentrancyGuard {
  ...
  // Top staking
  mapping(address => UserStaking) public _topStaking;
  ...
  constructor(address _token) {
    ...
    endStakingAt = block.timestamp + 14 days;
  }
  ...
  function stake(uint256 _amount) external nonReentrant {
    ...
    require(totalDNLStaked.add(_amount) <= maxPool, "The pool is full");
    ...
    private
  {
    ...
    uint256 lastDepositeAmount = _topStaking[_address].lastDepositeAmount;
    uint256 lastDepositeAt = _topStaking[_address].lastDepositeAt;
    uint256 lastDepositePoint = _topStaking[_address].lastDepositePoint;
    // first time staking
    ...
    uint256 amount = _depositeAmount.mul(10).div(100);
    lastDepositePoint = lastDepositePoint.add(durationInSeconds.mul(amount));
    _topStaking[_address] = UserStaking(lastDepositeAt, _depositeAmount, lastDepositePoint);
  }
  ...
  function withdraw(uint256 _id) external nonReentrant onlyStakeholder(_id) {
    ...
    totalDNLStaked = totalDNLStaked.sub(principal);
    emit Withdraw(
      msg.sender,
      _id,
      term,
