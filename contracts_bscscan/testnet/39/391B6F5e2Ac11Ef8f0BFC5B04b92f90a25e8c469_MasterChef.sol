...
contract MasterChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 totalBalance;            // pool token balance, allow multiple pools with same token
        uint256 startTime;
        uint256 endTime;
    ...
    constructor(
        ...
        uint256 _st2_05 = 1643990400; //2022-02-05
        uint256 _et2_11 = 1644508800;
        uint256 _et2_15 = 1644854400;
        uint256 _et2_20 = 1645286400;
        uint256 _et2_25 = 1645718400;
        uint256 _et3_10 = 1646841600;
        // staking pool
    ...
    function set(uint256 _pid, uint256 _apr, uint256 _withdrawLockPeriod, uint256 _totalBalance, uint256 _startTime, uint256 _endTime) public onlyOwner {
        require(_endTime > _startTime, "time set error");
        PoolInfo storage pool = poolInfo[_pid];
        pool.apr = _apr;
        pool.withdrawLockPeriod = _withdrawLockPeriod;
        pool.totalBalance = _totalBalance;
    ...
    function add(IBEP20 _lpToken, uint256 _apr, uint256 _withdrawLockPeriod, uint256 _totalBalance, uint256 _startTime, uint256 _endTime) public onlyOwner {
    ...
    function getMultiplier(uint256 _from, uint256 _to, uint256 _lock) public pure returns (uint256) {
        uint256 delta = _to.sub(_from);
        if (delta >= _lock){
            return 1e12;
        }
        return delta.mul(1e12).div(_lock);
    }
    ...
    function pendingPeace(uint256 _pid, address _user) public view returns (uint256) {
        ...
        uint256 multiplier = getMultiplier(pool.endTime, block.timestamp, pool.withdrawLockPeriod);
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        require (canStake(_pid, msg.sender), "Not Staking Period");
        require (pool.balance.add(_amount) <= pool.totalBalance, "Overflow TotalBalance of Pool");
    ...
    function canStake(uint256 _pid, address _user) public view returns (bool) {
        PoolInfo storage pool = poolInfo[_pid];
        return (block.timestamp <= pool.endTime) && (block.timestamp >= pool.startTime) && (pool.balance < pool.totalBalance);
    }

    function canHarvest(uint256 _pid, address _user) public view returns (bool) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        return (user.amount > 0) && (block.timestamp >= pool.endTime.add(pool.withdrawLockPeriod));
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        PeaceToken.safeTransferFrom(rewardaddr, msg.sender, pending.sub(feeAmount));
