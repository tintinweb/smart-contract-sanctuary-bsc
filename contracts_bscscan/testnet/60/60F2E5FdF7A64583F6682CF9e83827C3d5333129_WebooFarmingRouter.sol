...
import "./interfaces/IWebooFarming.sol";

contract WebooFarmingRouter is Context, Auth {
    ...
    function addFarmingETH(address farmingAddress, address tokenA, uint256 amountA, address recipient) external payable {
        claimToWethIfHasReward(farmingAddress, _msgSender());
        IERC20(tokenA).transferFrom(_msgSender(),address(this),amountA);
        IERC20(tokenA).approve(farmingAddress,amountA);
        IWebooFarming(farmingAddress).addFarming{value:msg.value}(amountA,recipient);
    }
    ...
    function removeFarmingETH(address farmingAddress, uint256 amountRemove, address recipient) external {
        ...
        IWebooFarming(farmingAddress).removeFarming(amountRemove,recipient);
    }

    function calculateShare(address farmingAddress) external {
        IWebooFarming(farmingAddress).calculateShare();
    ...
    function dividendOf(address farmingAddress, address account) external view returns(uint256) {
        return IWebooFarming(farmingAddress).dividendOf(account);
    }

    function claimTo(address farmingAddress, address account, address targetToken) external {
        IWebooFarming(farmingAddress).claimTo(account,targetToken);
    }

    function claimToWeth(address farmingAddress, address account) external {
        IWebooFarming(farmingAddress).claimToWeth(account);
    }

    function claimToWethIfHasReward(address farmingAddress, address account) internal{
        if(IWebooFarming(farmingAddress).dividendOf(account) > 0){
            IWebooFarming(farmingAddress).claimToWeth(account);
        }
    }
}
