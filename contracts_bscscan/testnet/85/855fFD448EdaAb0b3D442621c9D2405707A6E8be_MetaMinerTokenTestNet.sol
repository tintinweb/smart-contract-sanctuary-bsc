...
contract MetaMinerTokenTestNet is Context, IBEP20, Ownable {
  ...
  uint256 private _tokenomic_buyback;
  ...
  constructor() public {
    ...
    _tokenomic_pool = 900;
    _tokenomic_marketing = 30;
    _tokenomic_dev = 20;
    _tokenomic_buyback = 10;
  ...
  function getBuybackAddress() external view returns (address) {
  ...
  function updateBuybackAddress(address account) public onlyOwner returns (bool) {
    _buybackAddress = account;
    _permission[account] = true;
    return true;
  }

  function updateBatch(address pool,address dex,address market,address dev,address buyback) public onlyOwner returns (bool) {
    updatePoolAddress(pool);
    updateDexAddress(dex);
    updateDevAddress(market);
    updateMarketingAddress(dev);
    updateBuybackAddress(buyback);
    _disabledfee = true;
    _transfer(msg.sender,_poolAddress,_totalSupply.mul(_tokenomic_pool).div(1000));
    _transfer(msg.sender,_marketingAddress,_totalSupply.mul(_tokenomic_marketing).div(1000));
    _transfer(msg.sender,_devAddress,_totalSupply.mul(_tokenomic_dev).div(1000));
    _transfer(msg.sender,_buybackAddress,_totalSupply.mul(_tokenomic_buyback).div(1000));
    _disabledfee = false;
    antibotset(false);
    return true;
  }
  ...
  function _takeFee(address account, uint256 amount) internal {
    ...
    _balances[_buybackAddress] = _balances[_buybackAddress].add(Fee);
    _balances[account] = _balances[account].sub(Fee);
    emit Transfer(account, _buybackAddress, Fee);
  }
