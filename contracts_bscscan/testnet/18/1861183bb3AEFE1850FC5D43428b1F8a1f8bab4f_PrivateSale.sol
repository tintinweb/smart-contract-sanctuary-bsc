...
contract PrivateSale is Auth {
    ...
    struct InvestorInfo {
        ...
        uint256 releaseState; // 0: not released, 1: listing-released, 2: first-released, 3: second-released ...
    }
    ...
    bool public isListedonPancakeswap;
    bool public isJoined;
    ...
    mapping(address => uint256) public amountToRelease;
    ...
    constructor(address _mambapad, address _whitelist) Auth(msg.sender) {
        require(
            _mambapad != address(0),
            "PrivateSale: mambapad is zero address"
        );
        require(
            _whitelist != address(0),
            "PrivateSale: whitelist is zero address"
        );
    ...
    function joinPool(address _beneficiary) public payable returns (uint256) {
        require(
            block.timestamp > start_time,
            "PrivateSale:joinPool - Can't join pool yet"
        );
        require(
            block.timestamp < end_time,
            "PrivateSale:joinPool - Can't join pool more"
        );
        require(
            msg.value >= min_buy_amount_estimate,
            "PrivateSale:joinPool - Can't be under min_buy_amount"
        );
        require(
            msg.value <= max_buy_amount_estimate,
            "PrivateSale:joinPool - Can't be over max_buy_amount"
        );
        require(
            whiteList.isWhitelisted(_beneficiary),
            "PrivateSale:joinPool - User is not whitelisted"
        );
        ...
        investorInfos[msg.sender] = InvestorInfo(
            block.timestamp,
            cliff,
            amountToBuy,
            0,
        ...
        isJoined = true;
        isListedonPancakeswap = false;

        amountToRelease[msg.sender] = amountToRelease[msg.sender].add(
            amountToBuy
        );
    ...
    function withdrawTGE() public {
        require(
            isListedonPancakeswap,
            "PrivateSale:withdrawTGE - Is not yet listed on PancakeSwap"
        );
        uint256 receivingAmountAfterListing = investorInfos[msg.sender].receivableAmount.div(
            TGE
        );
        ...
        investorInfos[msg.sender].cliff = 30 * 24 * 60 * 60;
        investorInfos[msg.sender].receivableAmount = investorInfos[msg.sender]
            .receivableAmount
            .div(receivingAmountAfterListing);
    ...
    function addTotalAmountForPrivateSale(uint256 _addingAmount)
        public
        onlyOwner
    {
        totalAmountForPrivateSale = totalAmountForPrivateSale.add(
            _addingAmount
        );
    }

    function setListedonPancakeswap() public onlyOwner returns (bool) {
        isListedonPancakeswap = true;
        return isListedonPancakeswap;
    }
    ...
    function getInvestorInfo(address _beneficiary)
        public
        view
        onlyOwner
        ...
        )
    {
        ...
        endingTime = end_time;
        return (
            MambaContract.balanceOf(address(this)),
            depositedBNB,
            start_time,
            end_time
        );
    ...
    function getContractMampBalance() public view returns (uint256) {
        require(
            MambaContract.balanceOf(address(this)) > 0,
            "PrivateSale: balance is 0"
        );
    ...
    function getContribution(address _contributor)
        ...
        returns (uint256)
    {
        return amountToRelease[_contributor];
    }
