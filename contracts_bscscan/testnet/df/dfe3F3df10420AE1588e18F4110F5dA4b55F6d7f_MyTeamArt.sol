...
contract MyTeamArt is Ownable, ReentrancyGuard {
    ...
    function placeItemForSale(address nftContract, uint256 tokenId, uint256 _itemId, uint256 price) external nonReentrant {
    ...
    function buyMarketItem(address nftContract, uint256 itemId) external nonReentrant {
        uint256 price = idToMarketItem[itemId].price;
        uint256 tokenId = idToMarketItem[itemId].tokenId;
        uint256 calculatedFee = calculateFee(price);

        require(idToMarketItem[itemId].sold != true, "This item is sold");
    ...
    function setMarketItemPrice(uint256 itemId, uint256 price) external nonReentrant {
    ...
    function fetchMarketItems(uint256 page, uint256 limit) public view returns (MarketItem[] memory) {
        ...
        for (uint256 i = limit.mul(page).sub(limit).add(1); i <= limit.mul(page); i++) {
            if (idToMarketItem[i].itemId > 0 && idToMarketItem[i].isUnlisted != true) {
    ...
    function fetchMyOwnedItems(uint256 page, uint256 limit) public view returns (MarketItem[] memory) {
        require(page > 0, "Page must be more than 0");
        require(limit > 0, "Limit must be more than 0");
        require(limit <= 100, "Max limit reached");
        ...
        uint256 marketItemCount = 0;
        for (uint256 i = limit.mul(page).sub(limit).add(1); i <= limit.mul(page); i++) {
            if (idToMarketItem[i].itemId > 0 && idToMarketItem[i].owner == msg.sender) {
                marketItemCount = marketItemCount.add(1);
            }
        }
    ...
    function fetchMyItemsOnSale(uint256 page, uint256 limit) public view returns (MarketItem[] memory) {
        require(page > 0, "Page must be more than 0");
        require(limit > 0, "Limit must be more than 0");
        require(limit <= 100, "Max limit reached");
        ...
        uint256 marketItemCount = 0;
        for (uint256 i = limit.mul(page).sub(limit).add(1); i <= limit.mul(page); i++) {
            if (idToMarketItem[i].itemId > 0 && idToMarketItem[i].isUnlisted != true && idToMarketItem[i].seller == msg.sender) {
                marketItemCount = marketItemCount.add(1);
            }
        }
