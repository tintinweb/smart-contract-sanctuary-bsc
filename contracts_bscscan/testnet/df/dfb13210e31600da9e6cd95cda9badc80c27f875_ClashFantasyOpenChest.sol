...
contract ClashFantasyOpenChest is Initializable, OwnableUpgradeable {
        ...
        returns(uint256)
    {
        ...
        uint256 purchasenumber =  uint(keccak256(abi.encodePacked(block.timestamp, block.difficulty, _tokenId, _aditional, msg.sender))) % 100;        
    ...
    function returnRandomCards(uint _tokenId, uint _aditional, uint256 amountCards) 
        ...
        returns(uint256)
    {
        uint256 choose =  uint(keccak256(abi.encodePacked(block.timestamp, block.difficulty, _tokenId, _aditional, msg.sender))) % amountCards;        
        return choose;
    }
