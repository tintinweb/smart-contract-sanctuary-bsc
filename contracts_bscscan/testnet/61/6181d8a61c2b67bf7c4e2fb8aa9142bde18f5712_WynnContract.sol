...
contract WynnContract is IERC20, ERC20Ownable {
    using SafeMath for uint256;

    address dead = 0x000000000000000000000000000000000000dEaD;
    address zero = address(0);

    uint8 private maxLiqFee = 5;
    uint8 private maxTaxFee = 5;
    uint8 private maxBurnFee = 5;
    uint8 private maxWalletFee = 5;
    uint8 private maxBuybackFee = 5;
    ...
    uint8 public _holdersFee = 0; // Fee for Reflection
    uint8 private _previousTaxFee = _holdersFee;
    ...
    constructor(
        ...
        _symbol = "TRC";
        _decimals = 9;
        _tTotal =   50000000 * 10**9; 
        _rTotal = (MAX - (MAX % _tTotal));

        buyBackUpperLimit = 10 * 10**9;

        _rOwned[_msgSender()] = _rTotal;

        feeWallet = payable(0x182F027139ba41B9d98de589c74EF8C77b44fb53);

        numTokensSellToAddToLiquidity = 10 * 10**9;
    ...
    function removeAllFee() private {
        if (_holdersFee == 0 && _liquidityFee == 0 && _burnFee == 0 && _walletFee == 0 && _buybackFee == 0) return;
