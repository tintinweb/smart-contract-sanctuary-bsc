...
contract RevoStaking is IRevoStaking, Ownable {
    /// @dev is paused is true then staking/redemption will be banned.
    bool public paused;

    /// @dev represents the pools-created
    uint256 public poolCount;

    /// @dev represents the cliff-time
    ...
    /// @dev see {IRevoStaking-toggleStakingStatus}
    function toggleStakingStatus(bool status)
        public
        virtual
        override
        onlyOwner
    ...
    function stakeToken(uint256 poolId, uint256 amount)
        ...
        returns (bool)
    {
        require(!paused, Error.LE_STAKING_PAUSED);
        require(poolId <= poolCount, Error.VE_INVALID_POOLID);
        ...
        returns (bool)
    {
        ...
        PoolInfo storage p = pool[poolId];
        StakingInfo storage s = stake[_msgSender()][poolId];
        uint256 releaseable = unclaimed - p.totalClaimed;
        p.totalClaimed = p.totalClaimed + unclaimed;
        ...
        IERC20(p.rewardToken).transfer(_msgSender(), releaseable);
    ...
    {
        StakingInfo storage s = stake[user][poolId];

        if (block.timestamp > s.stakeTime + cliff) {
        ...
        returns (uint256)
    {
        ...
        totalReward = (totalReward / 31536000000);
        return totalReward;
    }

    /// @dev see {IRevoStaking-fetchPool}
    function fetchPool(uint256 poolId)
        public
        view
        virtual
        override
