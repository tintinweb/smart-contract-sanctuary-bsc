...
contract BitfarmsNodesV1 is ERC20, Ownable, PaymentSplitter {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        uint256 swapAmount,
        address Router
    ) ERC20("BitFarms", "BFN") PaymentSplitter(payees, shares) {
    ...
    function updatePancakeRouter(address newAddress) public onlyOwner {
        require(newAddress != address(PancakeRouter), "TKN: The router already has that address");
    ...
    function _transfer(
		...
		if (from != owner() && to != owner() && from != address(this) && to != address(this) && !_isSuper[from] && !_isSuper[to] && to != PancakePair && to != address(PancakeRouter)) {
			if (tradingOpen) {
				uint256 walletBalance = balanceOf(address(to));
				require(amount.add(walletBalance) <= maxTx, "Transfer amount too high");
			}
			if (!tradingOpen && from == PancakePair) {
