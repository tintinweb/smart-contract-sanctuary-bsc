...
contract BSI is Ownable {
   ...
   uint256 bets;
   uint256 num;
   uint256 count; // This is the count for the array of winners
...
// The address of the player and => the user info
   ...
   function() public payable {}
    ...
    function allocatePrizes(uint _gameId, uint16 teamWinner) public onlyAgent {
        ...
        for(j=0; j < players.length; j++){
            if(players[j] != address(0)){
                delete playerInfo[playerAddress].amountBet;
                delete playerInfo[playerAddress].teamSelected;
    ...
    // if(_pendingBalance[msg.sender] > 0 ){
    // token.transfer(msg.sender, _pendingBalance[winner[j]]);
    // _pendingBalance[msg.sender] = 0;
