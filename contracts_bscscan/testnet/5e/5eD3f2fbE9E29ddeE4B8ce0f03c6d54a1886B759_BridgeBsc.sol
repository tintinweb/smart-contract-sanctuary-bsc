...
abstract contract BridgeBase is Multiownable {
    ...
    function sendTokens(uint256 amount) public {
        ...
        require(ERC20(_tokenAddress).balanceOf(msg.sender) >= amount,"Not enough balance");
    ...
    function receiveTokens(uint256[] memory commissions) public payable {
        if (_tokensReceivedButNotSent[msg.sender] != 0) {
            require(commissions.length == _owners.length, "The number of commissions and owners does not match");
            uint256 sum;
            ...
            require(msg.value >= _owners.length * 150000 * 10**9, "Not enough BNB (The amount of BNB is less than the internal commission.)");
    ...
    function withdrawTokens(uint256 amount, address receiver) public onlyAllOwners {
        ...
        require(ERC20(_tokenAddress).balanceOf(address(this)) >= amount,"Not enough balance");
