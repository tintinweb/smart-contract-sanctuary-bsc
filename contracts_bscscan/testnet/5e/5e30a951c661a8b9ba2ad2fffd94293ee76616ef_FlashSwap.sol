...
abstract contract PancakeBuySell{
function swapTokenForToken(uint amountToken,address[] calldata pair,address[] calldata sellpair) external virtual;
}
...
contract FlashSwap is IPancakeCallee {
    ...
    address private  _factory=0xB7926C0430Afb07AA7DEfDE6DA862aE0Bde767bc;
    ...
    address private buycontract;
    uint private prevbal;

    uint256 constant public MAX_INT = 2**256 - 1;

    event tokenBalance(uint _value);
    event tokenAddress(address indexed _from);
...
function updatePbs(address _t) public OnlyManager {
        pbs = PancakeBuySell(_t);
        buycontract = _t;
    }
 

    function startFlashLoan(address[] calldata path, uint amountIn,address[] calldata buypath,address[] calldata sellpath ) external {
       ...
       address pairAddress = PancakeLibrary.pairFor(_factory, path[0], path[1]);
       require(pairAddress != address(0), 'This pool does not exist');
        // Save the path to use it in uniswapV2Call
         
       IERC20 wbnbToken = IERC20(path[1]);
       prevbal = wbnbToken.balanceOf(address(this));
       emit tokenAddress(path[1]);
       emit tokenBalance(prevbal);

        
        address token0 = IPancakePair(pairAddress).token0();
        address token1 = IPancakePair(pairAddress).token1();

        uint amount0 = path[1] == token0 ? amountIn : 0;
        uint amount1 = path[1] == token1 ? amountIn : 0;
    ...
    function pancakeCall(address sender, uint amount0, uint amount1, bytes calldata data) external override 
    {
        ...
        // uint amountReceived = IPancakeRouter02(_router).swapExactTokensForTokens(
        //     amountToken,
        //     0,
        //     path,
        //     address(this),
        ...
        token.transfer(buycontract, amountToken);
        pbs.swapTokenForToken(amountToken,_buypath,_sellpath);
        // otherToken.transfer(msg.sender, amountRequired);
        // otherToken.transfer(owner, amountReceived.sub(amountRequired));
        uint amountOut = amountToken.mul(1000).div(997).add(1);
...
function tokenApprove(IERC20 token) external OnlyManager {
require(token.approve(_router, MAX_INT), 'approve failed.');
}
