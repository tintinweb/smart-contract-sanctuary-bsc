...
contract ResearchLab is OwnableUpgradeable, ReentrancyGuard {
    ...
    function getTotalPoolValue() public view returns (uint256 totalPoolAlloc) {
        for (uint256 i=0; i < poolToken.length; i++) { 
            ...
            totalPoolAlloc = totalPoolAlloc.add(pool.lpToken.balanceOf(address(this)));
        }
    }
    ...
    function pendingRewards(uint256 _pid, address _user) public view returns (uint256, uint256 ) {
        ...
        if (currentEra <= numberOfEras) {
          for (uint256 i=lastRewardEra; i<=currentEra;i++) {
            endBlock = startBlock.add(i.mul(blocksPerEra)).sub(1);
            availableUnlockedRewards = endBlock.sub(lastRewardBlock).mul(rewardAllocated[i].div(blocksPerEra)).mul(i.mul(5)).div(100);
            availableLockedRewards = endBlock.sub(lastRewardBlock).mul(rewardAllocated[i].div(blocksPerEra)).mul(100-(i.mul(5))).div(100);
