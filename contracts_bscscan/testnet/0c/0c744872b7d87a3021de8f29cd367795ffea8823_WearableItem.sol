...
contract WearableItem is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    event itemGenerated(uint256 indexed _id, address owner, string name);
    ...
    function mintToken(string memory _name) public onlyOwner {     
        uint nextId = minted;
        minted += 1;
        _safeMint(_msgSender(), nextId); 
        fullName[nextId] = _name;
        LATEST_CLAIMED_ID = minted - 1;
        emit itemGenerated(nextId, _msgSender(), _name);
    }
