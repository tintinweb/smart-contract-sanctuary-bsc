...
contract LIFEGAMES is Context, IERC20, Ownable {
    ...
    function doBuyBack() private lockTheSwap {
        ...
        swapExactTokensForTokens(amountToSwapForBUSD);
        ...
        payable(buybackAndBurnAddress).transfer(busdForBuybackAndBurnPool);

        if (tokensForLiquidity > 0 && busdForLiquidity > 0) {
            swapAndSendToAddress(liquidityAddress, tokensForLiquidity);
            payable(liquidityAddress).transfer(busdForLiquidity);
        }
    }
