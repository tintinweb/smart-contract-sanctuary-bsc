   ...
   // Info of each user.
    ...
    function deposit(uint256 _amount) public {
        ...
        if(user.amount > 0 && block.timestamp > user.lastRewardTimestamp) {
            uint256 pending = _getPendingReward(user.lastRewardTimestamp, user.amount);
            if(pending > 0) {
                _mintReward(msg.sender, pending);
                user.lastRewardTimestamp = block.timestamp;
                emit Harvest(msg.sender, msg.sender, pending);
            }
        }
        if(_amount > 0) {
    ...
    function pendingReward(address _user) external view returns (uint256) {
        UserInfo storage user = userInfo[_user];
        return _getPendingReward(user.lastRewardTimestamp, user.amount);
    }
     ...
     * @return Return pending rewards.
     */
    function _getPendingReward(uint256 _lastRewardTimestamp, uint256 _amount) private view returns(uint256){
        uint256 goldPrice = goldPriceModule.getLatestPrice();
        uint256 multiplier = _getMultiplier(_lastRewardTimestamp, block.timestamp);
