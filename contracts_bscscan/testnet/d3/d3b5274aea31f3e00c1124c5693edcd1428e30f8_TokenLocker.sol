...
//mulai whitelist dari sini
    ...
    function setWhiteListAccess(address _target, bool _allowed) public {
        ...
        //token idm to lock - testnet
        //uint256 amount = IERC20(0xF35D45f57e00623149098a8befBBB5F589556Fba).balanceOf(_target)/(10**9);
        //require(amount >= 50000);

        //if(_allowed == true && amount >= 50000 ) {
        if(_allowed == true ) {        
            list[_target] = 1;
        } else {
            list[_target] = 0;
    ...
    //used by single user
    function setBlackListAccess(address _target, bool _allowed) public {
        //require(_target != 0x0);
    ...
    //used by admin
    function setWhiteListAccessMod(address _target, bool _allowed) public {
        //require(_target != 0x0);


        if(_allowed == true && jalurkhusus == true) {        
            list[_target] = 1;
        } else {
            list[_target] = 0;
        }
    ...
    //use this by admin
    function setWhiteListAccessByList(address[] memory _backers, bool[] memory _allows) public {
        ...
        for( uint backerIndex = 0; backerIndex < _backers.length; backerIndex++) {
            setWhiteListAccessMod(_backers[backerIndex], _allows[backerIndex]);

        }
    }

    function addWhiteListAccessByList(address[] memory _backers) public {
        for( uint backerIndex = 0; backerIndex < _backers.length; backerIndex++) {
    ...
    function imInWhiteList() public view returns (bool) {
    ...
    function LockToken(
        ...
        unlockTime = block.timestamp + 300; //1209600 = 14 days, 600 = 10 menit untuk tes saja
    ...
    function UnlockToken(address token) public {
        ...
        lockedBalance = 0;
        setBlackListAccess(msg.sender, false);   
