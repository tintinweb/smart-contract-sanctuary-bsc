...
contract HVS is HVSBase {
  ...
  function claimReward() isHuman nonReentrant external {
    ...
    function bnbRewardClaimedAsHVS(address byAddress) public view returns (uint256) {
    ...
    function totalBNBClaimedAsHVS() public view returns (uint256) {
	...
	function reimburseAfterHVSClaimFailure() public view returns(bool) {
	...
	function setBlacklistAccount(address account) public onlyOwner {
		require(!_isBlacklisted[account], "HVS: Account is already blacklisted");
		_isBlacklisted[account] = true;
		emit BlacklistChange(account, true);
	}

	function unsetBlacklistAccount(address account) public onlyOwner {
