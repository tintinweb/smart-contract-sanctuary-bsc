...
contract TickALock is Ownable, IBEP20 {
    ...
    event TokensToBurnAdd(uint256 tokensToBurn);
    event TokensToBurnSubtract(uint256 tokensToBurn);
    ...
    function _sellTokens(address sender, uint256 amount) private {
        ...
        if (burnTax > 0) {
            ...
            tokensToBurnAdd(taxedTokensBurn);
        }
        if (_marketingTax > 0) {
            // Amount of tokens to be sent to contract
            taxedTokensMarketing = (amount * _marketingTax) / 10000;
    ...
    function burnTokenContractTokens() private {
        ...
        tokensToBurnSubtract(_tokensToBurn);
    }

    function totalAllocatedCrosswordPotContractAdd(uint256 amount) private {
        _totalAllocatedCrosswordPotContract += amount;
    ...
    function totalAllocatedMarketingContractAdd(uint256 amount) private {
        _totalAllocatedMarketingContract += amount;
    ...
    function tokensToBurnAdd(uint256 amount) private {
        _tokensToBurn += amount;
        emit TokensToBurnAdd(amount);
    }

    function tokensToBurnSubtract(uint256 amount) private {
        require(_tokensToBurn >= amount, "Not enough tokens to subtract from tokens to burn.");
        _tokensToBurn -= amount;
        emit TokensToBurnSubtract(amount);
    }
}