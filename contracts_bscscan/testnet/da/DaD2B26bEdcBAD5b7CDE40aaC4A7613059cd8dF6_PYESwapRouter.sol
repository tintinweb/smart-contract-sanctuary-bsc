...
contract PYESwapRouter is IPYESwapRouter {
    ...
    function _calculateAmounts(address input, address output, address token0) internal view returns (uint amountInput, uint amountOutput) {
        ...
        amountOutput = PYESwapLibrary.getAmountOut(amountInput, reserveInput, reserveOutput, baseToken != address(0));
    }
    ...
    {
        return PYESwapLibrary.getAmountOut(amountIn, reserveIn, reserveOut, false);
    }
    ...
    {
        return PYESwapLibrary.getAmountIn(amountOut, reserveIn, reserveOut, false);
    }
...
library PYESwapLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'7e4b8858964794f2e94b38733da0eb2df79b375b3e042ea15c8c902a54933506' // init code hash
            ))));
    }
    ...
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut, bool tokenFee) internal pure returns (uint amountOut) {
        ...
        uint amountInMultiplier = tokenFee ? 8575 : 9975;
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut, bool tokenFee) internal pure returns (uint amountIn) {
        require(amountOut > 0, 'PYESwapLibrary: INSUFFICIENT_OUTPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'PYESwapLibrary: INSUFFICIENT_LIQUIDITY');
        uint amountOutMultiplier = tokenFee ? 8575 : 9975;
    ...
    function getAmountsOut(address factory, uint amountIn, address[] memory path) internal view returns (uint[] memory amounts) {
        ...
        amounts[0] = amountIn;
        for (uint i; i < path.length - 1; i++) {
            address baseToken = IPYESwapPair(pairFor(factory, path[i], path[i + 1])).baseToken();
            (uint reserveIn, uint reserveOut) = getReserves(factory, path[i], path[i + 1]);
            amounts[i + 1] = getAmountOut(amounts[i], reserveIn, reserveOut, baseToken != address(0));
        }
    }
    ...
    function getAmountsIn(address factory, uint amountOut, address[] memory path) internal view returns (uint[] memory amounts) {
        ...
        for (uint i = path.length - 1; i > 0; i--) {
            address baseToken = IPYESwapPair(pairFor(factory, path[i - 1], path[i])).baseToken();
            (uint reserveIn, uint reserveOut) = getReserves(factory, path[i - 1], path[i]);
            amounts[i - 1] = getAmountIn(amounts[i], reserveIn, reserveOut, baseToken != address(0));
        }
    }
}
