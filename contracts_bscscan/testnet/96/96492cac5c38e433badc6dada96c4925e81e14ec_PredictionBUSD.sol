 ...
 */
contract PredictionBUSD is Ownable, Pausable, ReentrancyGuard {
    ...
    //new
    IERC20 public tokenAddress;//address of token to bet
     ...
     * @param epoch: epoch
     */
    function betBear(uint256 epoch, uint256 amount) public whenNotPaused nonReentrant notContract {
     ...
     * @param epoch: epoch
     */
    function betBull(uint256 epoch, uint256 amount) public whenNotPaused nonReentrant notContract {
        require(epoch == currentEpoch, "Bet is too early/late");
        require(_bettable(epoch), "Round not bettable");
        require(amount >= minBetAmount, "Bet amount must be greater than minBetAmount");
        require(ledger[epoch][msg.sender].amount == 0, "Can only bet once per round");
    ...
    function setNewTokenAddress(IERC20 newTokenAddress) external onlyOwner{
            tokenAddress = IERC20(newTokenAddress);
