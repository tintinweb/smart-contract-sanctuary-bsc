...
contract CustomBond is Initializable, Policy {
    ...
    address public subsidyRouter; // pays subsidy in OHM to custom treasury
    ...
    function initialize(
        ...
        payoutToken = IERC20Metadata(ICustomTreasury(_customTreasury).payoutToken());
    ...
    function deposit(
        ...
        if(feeInPayout) {
            ...
            if(fee != 0) { // if fee, send to Olympus treasury
                principalToken.safeTransferFrom(msg.sender, treasury, fee);
            }
        }
