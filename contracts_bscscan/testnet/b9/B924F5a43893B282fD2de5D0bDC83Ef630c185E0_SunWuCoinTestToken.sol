...
contract SunWuCoinTestToken is ERC20, RoleControle  {

    constructor( )
    ERC20("Sun WuCoin Test", "SWCT")
    {
        _mint(owner(), 10000000 * 10 ** decimals());
    }

    function MintByCEO (address _toAddr, uint _amount ) external onlyCEO {
        _mint(_toAddr, _amount);
    }

    function BurnByCEO (address _formAddr, uint _amount) external onlyCEO {
        _burn(_formAddr, _amount);
    }
