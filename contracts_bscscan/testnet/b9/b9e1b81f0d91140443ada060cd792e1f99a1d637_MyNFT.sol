...
contract MyNFT is ERC721Enumerable, Ownable {
    ...
    function buyAndMint(uint256 _tokenId, string memory _tokenURI, uint256 _price) external payable {
        require(msg.value >= _price, "Error, the amount sent is lower than price");
        require(!_exists(_tokenId), "Error, token already exists");
        
        (bool sent,) = _contractOwner.call{value: _price}("");
        require(sent, "Failed to send money");

        mint(_tokenId, _tokenURI, msg.sender, _price);
        listedMap[_tokenId] = false;
        
        emit NftListStatus(msg.sender, _tokenId, false);
        emit Purchase(address(0), msg.sender, _price, _tokenId, _tokenURI);
    }
    ...
    function _trade(uint256 _id) internal {
        ...
        emit NftListStatus(_owner, _id, false);
