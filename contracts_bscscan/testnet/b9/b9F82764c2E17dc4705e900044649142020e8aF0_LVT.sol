...
// File: Louverture/library/Address.sol
...
contract LVT is ERC20, Ownable, PaymentSplitter {
    ...
    constructor(
        ...
        totalBuyFee = buyLpFee.add(buyMarketingFee);
    ...
    function updateBuyFee(uint256 lpFee, uint256 marketingFee) external onlyOwner {
        buyLpFee = lpFee;
        buyMarketingFee = marketingFee;
        totalBuyFee = buyLpFee.add(buyMarketingFee);
    }

    function updateSellFee(uint256 lpFee, uint256 marketingFee, uint256 devFee) external onlyOwner {
        sellLpFee = lpFee;
    ...
    function _transfer(
        ...
        if (from == uniswapV2Pair && !_isWhitelisted[to] && to != address(this)) {          // buy
            uint256 feeAmount = amount.mul(totalBuyFee).div(100);
            uint256 marketingFeeAmount = amount.mul(buyMarketingFee).div(100);
            amount = amount.sub(feeAmount);

            super._transfer(from, marketingPool, marketingFeeAmount);
        }
        
        if (to == uniswapV2Pair && !_isWhitelisted[from] && from != address(this)) {     // sell
            ...
            uint256 feeAmount = amount.mul(totalSellFee).div(100);
            amount = amount.sub(feeAmount);

            uint256 initTokenBalance = balanceOf(address(this));
            super._transfer(from, address(this), feeAmount);
            uint256 newTokenBalance = balanceOf(address(this));
            uint256 feeTokenBalance = newTokenBalance.sub(initTokenBalance);

            uint256 sellLpFeeAmount = feeTokenBalance.mul(sellLpFee).div(totalSellFee);
            uint256 sellMarketingFeeAmount = feeTokenBalance.mul(sellMarketingFee).div(totalSellFee);
            uint256 sellDevFeeAmount = feeTokenBalance.sub(sellLpFeeAmount).sub(sellMarketingFeeAmount);
            
            if (!swapping) {                
                swapping = true;
                swapAndLiquify(sellLpFeeAmount);
                swapAndSendToFee(marketingPool, sellMarketingFeeAmount);
                swapAndSendToFee(devPool, sellDevFeeAmount);
                swapping = false; 
            }
