...
/*! IswapRouterV1bnb.sol | SPDX-License-Identifier: MIT License */
...
library IswapV1Library {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'191909a20952393c6bc9e6b1c2565e0a3e9f33517fba777587dc34fd643222f1' // init code hash
            ))));
    }
...
contract IswapV1Router02 is IIswapV1Router02 {
    ...
    function addLiquidityETH(
        ...
        address pair = IswapV1Library.pairFor(factory, token, WETH);
    ...
    function swapExactETHForTokens(uint amountOutMin, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WETH, 'IswapV1Router: INVALID_PATH');
        amounts = IswapV1Library.getAmountsOut(factory, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'IswapV1Router: INSUFFICIENT_OUTPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(IswapV1Library.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        external
        ...
        returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WETH, 'IswapV1Router: INVALID_PATH');
        amounts = IswapV1Library.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'IswapV1Router: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, IswapV1Library.pairFor(factory, path[0], path[1]), amounts[0]
        ...
        ensure(deadline)
    {
        ...
        assert(IWETH(WETH).transfer(IswapV1Library.pairFor(factory, path[0], path[1]), amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
