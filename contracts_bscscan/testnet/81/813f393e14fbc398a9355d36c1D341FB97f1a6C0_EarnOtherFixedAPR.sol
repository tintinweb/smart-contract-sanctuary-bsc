  ...
  function withdraw() external nonReentrant {
    ...
    if (user.amount > 0) {
      ...
      if(endBlock >= block.number) {
        uint256 _reward = _calculateReward(_withdrawAmount, user.lastRewardBlock);
        uint256 blockLeft = endBlock.sub(block.number);
        rewardDebt = rewardDebt.add(_reward).sub(
          _withdrawAmount.mul(blockLeft).mul(rewardPerBlock).div(rewardPerBlockPrecisionFactor)
        );
      }
