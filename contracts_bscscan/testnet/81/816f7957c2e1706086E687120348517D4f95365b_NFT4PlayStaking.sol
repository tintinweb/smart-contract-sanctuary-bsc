...
contract NFT4PlayStaking is IStakingRewards, RewardsDistributionRecipient, ReentrancyGuard, Pausable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    /* ========== STATE VARIABLES ========== */
   
    IERC20 public NFT4PlayToken;
    ...
    uint256 public minstake = 200000*10**18;
    ...
    constructor(
        ...
        address _wallet,
        address _NFT4PlayToken
    ) {
        NFT4PlayToken = IERC20(_NFT4PlayToken);
        rewardsDistribution = _rewardsDistribution;
        wallet = _wallet;
    }
    ...
    /* ========== MUTATIVE FUNCTIONS ========== */
    function stakeRewards(uint256 amount) external nonReentrant whenNotPaused updateReward(msg.sender) {
    ...
    function stake(uint256 amount) external override nonReentrant whenNotPaused updateReward(msg.sender) {
        ...
        _totalSupply = _totalSupply.add(amount);
        _balances[msg.sender] = _balances[msg.sender].add(amount);
        NFT4PlayToken.safeTransferFrom(msg.sender, address(this), amount);
        emit Staked(msg.sender, amount);
    }                                                  
    function withdraw(uint256 amount, bool payingCharges) public override nonReentrant updateReward(msg.sender)  {
        ...
        if(payingCharges == true){
            ...
            NFT4PlayToken.safeTransfer(wallet, fee);
            amount = amount.sub(fee);
            NFT4PlayToken.safeTransfer(msg.sender, amount);
            if(flag == 0){
    ...
    function getReward() public override nonReentrant updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            NFT4PlayToken.safeTransfer(msg.sender, reward);
    ...
    function notifyRewardAmount(uint256 reward) external override onlyRewardsDistribution updateReward(address(0)) {
        ...
        uint balance = NFT4PlayToken.balanceOf(address(this)).sub(_totalSupply);
    ...
    function recoverERC20(address tokenAddress, uint256 tokenAmount) external onlyOwner {
        ...
        require(
            tokenAddress != address(NFT4PlayToken),
