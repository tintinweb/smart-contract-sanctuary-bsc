...
contract NonFungibleToken is ERC721URIStorage, Ownable, Pausable {
    ...
    uint256 public maxAmountWhiteListPerUser;               // maximum number of NFTs that can be minted per whitelisted user
    uint256 public startingTime;                            // UTC timestamp when minting starts
    uint256 public price;                                   // price for minting one NFT
    address internal _signer;                               // Signer of whitelisted signatures
    ...
    mapping(address => uint256) internal _whitelistMinted;  // number of NFT minted per address during the whitelist period
    ...
    constructor(string memory name_,
                ...
                uint256 maxAmountWhiteListPerUser_,
        ...
        maxAmountWhiteListPerUser = maxAmountWhiteListPerUser_;
    ...
    function setMaxAmountWhiteListPerUser(uint256 _maxAmountWhiteListPerUser) external onlyOwner {
        require(_maxAmountWhiteListPerUser <= COLLECTION_MAX_ITEMS, "max whitelist per user cannot exceed max supply");
        maxAmountWhiteListPerUser = _maxAmountWhiteListPerUser;
    }
    ...
    function whitelistMint(bytes memory _signature) external payable whenNotPaused {
        require(_verify(_signature, msg.sender), "invalid arguments or not whitelisted");
        uint256 _numToMint = _getNumToMint(price);
        ...
        _whitelistMinted[msg.sender] += _numToMint;

        require(totalWhitelistMinted <= maxWhitelistMint, "over whitelist limit");
        require(_whitelistMinted[msg.sender] <= maxAmountWhiteListPerUser, "over account whitelist limit");
    }
	...
	function _verify(bytes memory signature, address target) internal view returns (bool) {
		uint8 v;
		bytes32 r;
		bytes32 s;
		...
		return (_signer == address(ecrecover(senderHash, v, r, s)));
	}
