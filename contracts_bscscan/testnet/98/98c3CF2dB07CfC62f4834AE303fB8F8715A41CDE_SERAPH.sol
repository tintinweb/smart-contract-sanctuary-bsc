...
contract SERAPH is ERC20, Ownable {
    ...
    constructor() ERC20("SERAPH HOLDINGS", "SERAPH") {
    	shibaDividendTracker = new ShibaDividendTracker();
    	busdDividendTracker = new BusdDividendTracker();

    	marketingWallet = 0x0ab124c5b276a01c5256114A407318F5E01DaDfd;
    	teamWallet = 0x419e915ba9dd0a6121959fA7257E916bf983EEB9;
    	...
    	IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1); //0x401ed259a80aF9174611f48E8eF0e82182C00900
    ...
    receive() external payable {
  	...
  	function updateTeamWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != teamWallet, "SERAPH: The team wallet is already this address");
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "SERAPH: The marketing wallet is already this address");
        excludeFromFees(_newWallet, true);
    ...
    function setBuyBackAndLiquifyEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = buyBackAndLiquidityFee.add(marketingFee).add(busdDividendRewardsFee).add(shibaDividendRewardsFee);
            buyBackAndLiquifyEnabled = _enabled;
        }
    ...
    function setShibaDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = shibaDividendRewardsFee.add(marketingFee).add(busdDividendRewardsFee).add(buyBackAndLiquidityFee);
            shibaDividendEnabled = _enabled;
        }
    ...
    function setBusdDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = busdDividendRewardsFee.add(marketingFee).add(shibaDividendRewardsFee).add(buyBackAndLiquidityFee);
            busdDividendEnabled = _enabled;
        }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(busdDividendRewardsFee).add(shibaDividendRewardsFee).add(buyBackAndLiquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updateShibaDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(shibaDividendTracker), "SERAPH: The dividend tracker already has that address");

        ShibaDividendTracker newShibaDividendTracker = ShibaDividendTracker(payable(newAddress));

        require(newShibaDividendTracker.owner() == address(this), "SERAPH: The new dividend tracker must be owned by the SERAPH token contract");
    ...
    function updateBusdDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(busdDividendTracker), "SERAPH: The dividend tracker already has that address");

        BusdDividendTracker newBusdDividendTracker = BusdDividendTracker(payable(newAddress));

        require(newBusdDividendTracker.owner() == address(this), "SERAPH: The new dividend tracker must be owned by the SERAPH token contract");
    ...
    function updateBusdDividendRewardFee(uint8 newFee) external onlyOwner {
        ...
        totalFees = busdDividendRewardsFee.add(shibaDividendRewardsFee).add(marketingFee).add(buyBackAndLiquidityFee);
    }
    
    function updateMarketingFee(uint8 newFee) external onlyOwner {
        
        marketingFee = newFee;
        totalFees = marketingFee.add(shibaDividendRewardsFee).add(busdDividendRewardsFee).add(buyBackAndLiquidityFee);
    }
    
    function updateBuyBackAndLiquidityFee(uint8 newFee) external onlyOwner {
        
        buyBackAndLiquidityFee = newFee;
        totalFees = buyBackAndLiquidityFee.add(shibaDividendRewardsFee).add(busdDividendRewardsFee).add(marketingFee);
    }

    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "SERAPH: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "SERAPH: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SERAPH: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "SERAPH: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "SERAPH: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "SERAPH: Trading has not started yet");
        ...
        if (
            ...
            if (!swapping && canSwap) {
                ...
                if (shibaDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(shibaDividendRewardsFee.add(busdDividendRewardsFee)).mul(shibaDividendRewardsFee);
                    swapAndSendShibaDividends(sellTokens.div(10**2).mul(rand()));
                }
                
                if (busdDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(shibaDividendRewardsFee.add(busdDividendRewardsFee)).mul(busdDividendRewardsFee);
...
contract ShibaDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("SERAPH_Shibaereum_Dividend_Tracker", "SERAPH_Shibaereum_Dividend_Tracker", 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82) {
    	claimWait = 60;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "SERAPH_Shibaereum_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "SERAPH_Shibaereum_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SERAPH contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait != claimWait, "SERAPH_Shibaereum_Dividend_Tracker: Cannot update claimWait to same value");
...
contract BusdDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("SERAPH_Busd_Dividend_Tracker", "SERAPH_Busd_Dividend_Tracker", 0xE02dF9e3e622DeBdD69fb838bB799E3F168902c5) {
    	claimWait = 60;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "SERAPH_Busd_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "SERAPH_Busd_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SERAPH contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait != claimWait, "SERAPH_Busd_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }
