...
contract BossyMartian is ERC721Enumerable, Ownable {
    ...
    uint256 public  PRICE = 125 * 10**15; // 0.125 BNB
    uint256 public  PRESALE_PRICE = 75 * 10**15; //0.075 BNB
    uint256 public constant MAX_BY_MINT = 20;
    uint256 public constant MAX_BY_MINT_WHITELIST = 20;
    uint256 public constant MAX_WALLET = 20;
    uint256 public MAX_RESERVE_COUNT = 20;
    ...
    string private notRevealedtokenUri =
        "https://marvin.mypinata.cloud/ipfs/QmdbGcRUtUvCpcnfy7HJWVVUYVnQvd2wXADKkMoXSwY1xB";
    mapping(address => bool) private _whiteList;
    mapping(address => uint256) private _whiteListClaimed;
    uint256 private _reservedCount = 0;
    uint256 private _reserveAtATime = 20;
    mapping(uint256 => bool) private _winners;
    mapping(uint256 => bool) private _redeemStatus;

    string public baseTokenURI;

    event CreateBossyMartian(uint256 indexed id);

    constructor(string memory baseURI) ERC721("The Bossy Martian", "BOSSY") {
        setBaseURI(baseURI);
    ...
    function mint(uint256 _count) public payable saleIsOpen {
        ...
        require(total <= MAX_ELEMENTS, "All Bossy Martian NFT's are sold out");
    ...
    function setLotteryWinners(uint256 number, bool isWinner)
        external
        onlyOwner
    {
        require(
            !isSaleOpen,
            "Sale has started you can not change winners"
        );
        require(
            !isPresaleOpen,
            "Presale has started you cannot change winners"
        );
        require(!isRevealed);
        _winners[number] = isWinner;
    }

    function isLotteryWinner(uint256 number) public view returns (bool) {
        if (isRevealed) return _winners[number];
        else return false;
    }

    function redeemLottery(uint256 number) external {
        require(isRevealed, "Lottery winners are not announced");
        require(_winners[number], "You are not a winner");
        require(!_redeemStatus[number], "You have already redeemed the prize.");
        require(ownerOf(number) == address(_msgSender()), "You do not own this NFT");
        require(address(this).balance >= 1 ether , "Prize fund does not have enough funds. Contact project admin.");
        payable(address(this)).transfer(1 ether);
        _redeemStatus[number] = true;
    }

    function isRedeemed(uint256 number) external view returns(bool){
        return _redeemStatus[number];
    }
