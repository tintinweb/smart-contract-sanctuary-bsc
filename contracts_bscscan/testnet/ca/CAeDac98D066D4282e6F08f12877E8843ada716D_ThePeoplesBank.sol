...
contract ThePeoplesBank {
        
    // call it DefiBank
    string public name = "ThePeoplesBank";
    // Bank interest rate
    uint public interestRate = 12;
    //Owner of the bank
    ...
    function deposit(uint BUSD) public {

        // Transfer usdc tokens to contract
        IERC20(busd).transferFrom(msg.sender, address(this), BUSD);

        // Update the account balance in map
        stakingBalance[msg.sender] = stakingBalance[msg.sender] + BUSD;
    ...
    function setInterestRate(uint _rate)  public {
        require(owner == msg.sender, "Your are not the current owner");
        require(_rate >= 1 && _rate <= 12, "rate should be between 1 and 15");
        interestRate = _rate;
    } 
