...
contract TransferTool  {
   ...
   function withdraw() public {
    ...
    function transferTokensAvg(address _tokenAddr,address[] memory _tos,uint v) public onlyOwner returns (bool){
    ...
    function transferTokens(address _tokenAddr,address[] memory _tos,uint[] memory values) public onlyOwner returns (bool){
        ...
        for(uint i=0;i<_tos.length;i++){
            ...
            abi.encodeWithSignature("transferFrom(address,address,uint256)", msg.sender,_tos[i],values[i]));
