...
contract LiquidClaiming is Ownable {
    using SafeMath for uint256;
    ...
    function addProgram(
        ...
        /**
     * @dev Imports multiple new programs in a batch order.
     */
    function addMultiplePrograms(
        ...
        _programs[_owners[i]] = Program({ amount: _amounts[i], start: start, cliff: cliff, end: end, claimed: 0 });

        _totalVesting = _totalVesting.add(_amounts[i]);

        emit ProgramCreated(_owners[i], _amounts[i]);
        }
    }
