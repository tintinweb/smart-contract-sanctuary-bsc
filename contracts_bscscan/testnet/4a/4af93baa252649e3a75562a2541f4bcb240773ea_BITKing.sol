...
contract BITKing is ERC20, Ownable {
    ...
    address public immutable BIT = address(0xF9f93cF501BFaDB6494589Cb4b4C15dE49E85D0e);

    uint256 public swapTokensAtAmount = 2000000 * (10**18);
    uint256 public _maxWalletTotal = 1000000000* 10**18;
    mapping(address => bool) public _isBlacklisted;

    uint256 public BITRewardsFee = 8;
    uint256 public liquidityFee = 1;
    uint256 public marketingFee = 1;
    uint256 public totalFees = BITRewardsFee.add(liquidityFee).add(marketingFee);
    ...
    constructor() public ERC20("BITKing", "BITKing") {
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BIT: Account is already the value of 'excluded'");
    ...
    function setBITRewardsFee(uint256 value) external onlyOwner{
        BITRewardsFee = value;
        totalFees = BITRewardsFee.add(liquidityFee).add(marketingFee);
    }
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BIT: gasForProcessing must be between 200,000 and 500,000");
    ...
    function swapAndSendToFee(uint256 tokens) private  {
        uint256 initialBITBalance = IERC20(BIT).balanceOf(address(this));
        swapTokensForBIT(tokens);
        uint256 newBalance = (IERC20(BIT).balanceOf(address(this))).sub(initialBITBalance);
        IERC20(BIT).transfer(_liquidityWalletAddress, newBalance);
    }
    ...
    function swapTokensForBIT(uint256 tokenAmount) private {
    ...
    function swapAndSendDividends(uint256 tokens) private{
        swapTokensForBIT(tokens);
        uint256 dividends = IERC20(BIT).balanceOf(address(this));
        bool success = IERC20(BIT).transfer(address(dividendTracker), dividends);

        if (success) {
            dividendTracker.distributeBITDividends(dividends);
...
contract BITDividendTracker is Ownable, DividendPayingToken {
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BIT_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
