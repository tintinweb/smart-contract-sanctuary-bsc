...
contract Staking is ERC20 {
        ...
        function staking(uint256 _amount, uint256 _time) public {
                ...
                _stake.roleTreasure = _amount >= 500 ? block.timestamp + 30 minutes: MAX_INT;
                _stake.roleStructure = _amount >= 5000 ? block.timestamp + 180 minutes: MAX_INT;
                ERC20(token).transferFrom(msg.sender, address(this), _amount);
        }

        function withdraw(uint256 _amount) public {
                stake storage _stake = stakes[msg.sender];
                require(_stake.amount - _amount >= 0, "STAKE: no funds");
                _stake.amount -= _amount;
                _stake.roleTreasure = _stake.amount >= 500 ? _stake.roleTreasure : MAX_INT;
                _stake.roleStructure = _stake.amount >= 5000 ? _stake.roleTreasure : MAX_INT;
