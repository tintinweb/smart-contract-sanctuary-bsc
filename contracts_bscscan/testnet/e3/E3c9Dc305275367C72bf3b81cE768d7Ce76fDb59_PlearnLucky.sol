...
contract PlearnLucky is IPlearnLucky, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    function checkLuckyForTicketId(uint256 _luckyId, uint256 _ticketId) public view returns (bool) {
        ...
        if (_lucks[_luckyId].status != Status.Open) {
            if (_ticketId >= _lucks[_luckyId].firstTicketId && _ticketId < _lucks[_luckyId].firstTicketIdNextLottery) {
    ...
    function checkLuckyForTicketNumber(uint256 _luckyId, uint256 _ticketNumber) public view returns (bool) {
        bool isLucky = false;
        if (_lucks[_luckyId].status != Status.Open) {
            for (uint32 i = 0; i < _lucks[_luckyId].luckyNumbers.length; i++) {
    ...
    function getTicketNumber(uint256 _ticketId) external view returns (uint32) {
    ...
    function _getTickettIdFromTicketNumber(uint256 _luckyId, uint256 _ticketNumber) internal view returns (uint256) {
        uint256 firstTicketId = _lucks[_luckyId].firstTicketId;
        (uint32 fistTicketNUmber, uint32 lastTicketNumber) = _getFirstAndLastTicketNumberInRound(_luckyId);
        require(_ticketNumber >= fistTicketNUmber, 'Ticket number too low');
        require(_ticketNumber <= lastTicketNumber, 'Ticket number too high');
