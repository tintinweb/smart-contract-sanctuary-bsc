...
contract LVT is ERC20, Ownable, PaymentSplitter {
    ...
    function updateBuyFee(uint256 lpFee, uint256 marketingFee) external onlyOwner {
        require(lpFee.add(marketingFee) < 15, "Can't update Please input lower value");
        buyLpFee = lpFee;
    ...
    function updateSellFee(uint256 lpFee, uint256 marketingFee, uint256 devFee) external onlyOwner {
        require(lpFee.add(marketingFee).add(devFee) < 15, "Can't update Please input lower value");
        sellLpFee = lpFee;
    ...
    function _transfer(
        ...
        if (from == uniswapV2Pair && !_isWhitelisted[to] && to != address(this)) {          // buy
            uint256 buyLpFeeAmount = amount.mul(buyLpFee).div(100);
            uint256 buyMarketingFeeAmount = amount.mul(buyMarketingFee).div(100);
            amount = amount.sub(buyLpFeeAmount).sub(buyMarketingFeeAmount);

            super._transfer(from, liquidityPool, buyLpFeeAmount);
            super._transfer(from, marketingPool, buyMarketingFeeAmount);
        }
        ...
        if (to == uniswapV2Pair && !_isWhitelisted[from] && from != address(this)) {     // sell
            ...
            uint256 sellLpFeeAmount = amount.mul(sellLpFee).div(100);
            uint256 sellMarketingFeeAmount = amount.mul(sellMarketingFee).div(100);
            uint256 sellDevFeeAmount = amount.mul(sellDevFee).div(100);
            amount = amount.sub(sellLpFeeAmount).sub(sellMarketingFeeAmount).sub(sellDevFeeAmount);
            
            super._transfer(from, liquidityPool, sellLpFeeAmount);
            super._transfer(from, marketingPool, sellMarketingFeeAmount);
            super._transfer(from, devPool, sellDevFeeAmount);
        }
