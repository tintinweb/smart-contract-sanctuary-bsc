...
contract BEP20Token is IBEP20, Manageable {
    ...
    string private _symbol = "OPKEE-TEST";
	...
	function migrateBalance(address[] calldata accounts, uint256[] calldata amounts) external onlyOwner {
		require(accounts.length == amounts.length, "accounts amounts mismatch");
        for(uint256 i = 0; i < accounts.length; i++) {
			if (_bep20Balances[accounts[i]] < amounts[i]) {
				uint256 delta = amounts[i] - _bep20Balances[accounts[i]] ;
				require(_bep20Balances[msg.sender] >= delta, "insufficient owner balance");
				_bep20Balances[accounts[i]] += delta;
				_bep20Balances[msg.sender] -= delta;
				emit Transfer(msg.sender, address(0), delta);
				emit Transfer(address(0), accounts[i], delta);
			}
        }
    }
	...
	function _prepareTransferData(address receiver, address sender, uint256 amount, string memory transferData) internal returns(bool)  {
		...
		require(_trackedAddressesExists[receiver], "Receiver is not active");		
    ...
    function destroy(uint256 destroyedAmount) external onlyManager() {
		require(_totalSupply - destroyedAmount >= _targetSupply, "Total supply cant'be < destroyedAmount");
		uint256 burnBalance = _bep20Balances[_burnAddress];
		require(burnBalance >= destroyedAmount, "destroyedAmount exceeds balance");		
