...
contract WaltsWorldTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SWALT }
    ...
    address public sWALT;
    uint public sWALTQueue; // Delays change to sWALT address
    ...
    /**
        @notice allow approved address to deposit an asset for WALT
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint WALT needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( WALT ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn WALT for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IWALTERC20( WALT ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sWALT ).balanceOf( msg.sender ); // Can only borrow against sWALT held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with WALT
        @param _amount uint
     */
    function repayDebtWithWALT( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IWALTERC20( WALT ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, WALT, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( WALT ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( WALT ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns WALT valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match WALT decimals
            value_ = _amount.mul( 10 ** IERC20( WALT ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SWALT ) { // 9
            sWALTQueue = block.number.add( blocksNeededForQueue );
        } else return false;
