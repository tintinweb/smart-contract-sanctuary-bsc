...
Anti-Whale System:
	1 week period for sells
		Sell Total Fees x2 for second sell
		Sell Total Fees x3 for third sell
		Sell Total Fees x4 for fourth sell
...
contract DexteraTest is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    string private _name = "DexteraTest";
    string private _symbol = "DXTAT";
    uint8 private _decimals = 18;

    address payable public marketingWalletAddress = payable(0xA747bbc59611049A137b4b1c8132D8e3B14980C1); 
    ...
    uint256 public _secondSellMultiplier = 2;
    uint256 public _thirdSellMultiplier = 3;
    uint256 public _fourthSellMultiplier = 4;

    uint256 private _totalSupply = 10000000000 *  10**_decimals;
    uint256 public _maxBuyTxAmount = 5000000 *  10**_decimals;
    ...
    function setSecondSellMultiplier(uint256 multiplier) external onlyOwner() {
        _secondSellMultiplier = multiplier;
    }

    function setThirdSellMultiplier(uint256 multiplier) external onlyOwner() {
        _thirdSellMultiplier = multiplier;
    }

    function setFourthSellMultiplier(uint256 multiplier) external onlyOwner() {
        _fourthSellMultiplier = multiplier;
    }
            ...
            if(isMarketPair[recipient]){
                ...
                if (delta > 0 && delta < _whaleSellTimer && _lastSellTime[sender] != 0 && _numberOfSells[sender] != 0) {
                    if (_numberOfSells[sender]==1) {
                        _totalTaxIfSelling = previousTotalTaxIfSelling.mul(_secondSellMultiplier);
                        _sellMarketingFee = _totalTaxIfSelling.sub(_sellLiquidityFee);
                    } else if (_numberOfSells[sender]==2) {
                        _totalTaxIfSelling = previousTotalTaxIfSelling.mul(_thirdSellMultiplier);
                        _sellMarketingFee = _totalTaxIfSelling.sub(_sellLiquidityFee);
                    } else if (_numberOfSells[sender]>=3) {
                        _totalTaxIfSelling = previousTotalTaxIfSelling.mul(_fourthSellMultiplier);
                        _sellMarketingFee = _totalTaxIfSelling.sub(_sellLiquidityFee);
                    }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 tokensForLP = tAmount.mul(_sellLiquidityFee.add(_buyLiquidityFee)).div(_totalTaxIfSelling.add(_totalTaxIfBuying));
        uint256 tokensForMarketing = tAmount.mul(_sellMarketingFee.add(_buyMarketingFee)).div(_totalTaxIfSelling.add(_totalTaxIfBuying));
        uint256 tokensForSwap = (tokensForLP.div(2)).add(tokensForMarketing);
        uint256 initialBalance = address(this).balance;
        swapTokensForEth(tokensForSwap);
        uint256 amountReceived = (address(this).balance).sub(initialBalance);
        uint256 amountBNBLiquidity = amountReceived.mul(tokensForLP.div(2)).div(tokensForSwap);
        ...
        if(amountBNBLiquidity > 0 && tokensForLP > 0)
            addLiquidity(tokensForLP.div(2), amountBNBLiquidity);
    }
