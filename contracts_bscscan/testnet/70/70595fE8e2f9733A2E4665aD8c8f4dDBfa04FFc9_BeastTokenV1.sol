...
contract BeastTokenV1 is TokenBasic {

    address public developer;
    address public marketing;

    uint16 public developerPercentage; // Using 2 decimals
    uint16 public marketingPercentage; // Using 2 decimals
    uint16 public burnPercentage; // Using 2 decimals

    event DeveloperUpdated(address newDeveloperAddress);
    event MarketingUpdated(address newMarketingAddress);
    event PercentageUpdated(uint16 developerPercentage, uint16 marketingPercentage, uint16 burnPercentage);

    function setDeveloper(address newAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
        developer = newAddress;
        emit DeveloperUpdated(newAddress);
    }
    ...
    function setMarketing(address newAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
     ...
     function setPercentage(uint16 developerPrctg, uint16 marketingPrctg, uint16 burnPrctg) external onlyRole(DEFAULT_ADMIN_ROLE) {            
        developerPercentage = developerPrctg;
        marketingPercentage = marketingPrctg;
        burnPercentage = burnPrctg;
        emit PercentageUpdated(developerPercentage, marketingPercentage, burnPercentage);
    }
    ...
    */
    ...
    function _taxDeduction(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if(!(_isExcludedFromFees[sender] || _isExcludedFromFees[recipient])) {        
            // Tax Calculation
            // NOTE : Who will pay the tax?
            // If Tax is deduct by the amount of transfer, then tax taken from from to's address 
            // (Recomended, avoid problem on Dex bcz buyer not have the token yet)
            // If Tax is deduct not by the amount of transfer a.k.a exclude of amount, then tax taken from from's address 
            // (Not Recommended, will raise problem on Dex bcz will deduct money of dex)       
            require(developer != address(0) && marketing != address(0), "Tax : Address not set correctly");
            require(developerPercentage + marketingPercentage + burnPercentage != 0, "Tax : Percentage not set correctly");

            // Calculate amount
            uint256 developerAmount = uint256(developerPercentage) * amount / 100_00;
            uint256 marketingAmount = uint256(marketingPercentage) * amount / 100_00;
            uint256 burnAmount = uint256(burnPercentage) * amount / 100_00;

            // Transfer token        
            _transfer(recipient, developer, developerAmount);
    ...
    PausableUpgradeable
{
    ...
    function _beforeTokenTransfer(
        ...
        // Only applicable on _transfer function
