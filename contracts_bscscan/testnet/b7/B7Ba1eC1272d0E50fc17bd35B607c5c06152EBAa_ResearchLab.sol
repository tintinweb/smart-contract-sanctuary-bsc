...
contract ResearchLab is OwnableUpgradeable, ReentrancyGuard {
    ...
    // Info of each era for the pool
    struct EraInfo {
        uint256 eraStartBlock;      // Block number at the start of the era
        uint256 eraRewardAlloc;     // Reward allocation per era
        uint256 eraRewardPerBlock;  // Reward allocation per block
        uint256 eraLockPercentage;  // Lock percentage by era
    }
    ...
    // Info of each era for the pool
    mapping(uint256 => EraInfo) public eraInfo;
    ...
    // Set the rewards by era
    function setEraRewards() private {
        require(numberOfEras > 0, "numberOfEras: Not set");

        for (uint256 i=0; i<(numberOfEras+2); i++) {
            EraInfo storage era = eraInfo[i];
            era.eraRewardAlloc = rewardAllocated[i];
            era.eraRewardPerBlock = era.eraRewardAlloc.div(blocksPerEra);
            if (i>0 && i<=numberOfEras) {
                era.eraLockPercentage = 100-(i*5);
            }
        }
    }
    ...
    function startPool() public onlyOwner {
        ...
        for (uint256 i=1; i<(numberOfEras+2); i++) {
            EraInfo storage era = eraInfo[i];
            era.eraStartBlock = startBlock + ((i-1)*blocksPerEra);
        }
    ...
    // Update the era
    function updateEra() private {
      uint256 blockNumber = block.number;

      if (startTime > 0 && currentEra < (numberOfEras+1)) {    
          currentEra = blockNumber.sub(startBlock).div(blocksPerEra).add(1);
      }

      if (currentEra > (numberOfEras + 1)) {
        currentEra = (numberOfEras + 1);
      }
    ...
    function pendingRewards(uint256 _pid, address _user) public view returns (uint256 unlockedRewards, uint256 lockedRewards) {
        ...
        if (currentEra <= numberOfEras) {
          for (uint256 i=lastRewardBlock; i<blockNumber;i++) {
            eraValue = i.sub(startBlock).div(blocksPerEra).add(1);
            EraInfo storage era = eraInfo[eraValue];
            availableUnlockedRewards = availableUnlockedRewards.add(era.eraRewardPerBlock.mul((100-era.eraLockPercentage)).div(100));
            availableLockedRewards = availableLockedRewards.add(era.eraRewardPerBlock.mul(era.eraLockPercentage).div(100));
          }

          unlockedRewards = user.amount.mul(availableUnlockedRewards).div(totalPoolValue);
          lockedRewards = user.amount.mul(availableLockedRewards).div(totalPoolValue);
        } else {
          EraInfo storage eraCheck = eraInfo[numberOfEras+1];

          if (lastRewardBlock < eraCheck.eraStartBlock) {
            for (uint256 i=lastRewardBlock; i<eraCheck.eraStartBlock;i++) {
              eraValue = i.sub(startBlock).div(blocksPerEra).add(1);
              EraInfo storage era = eraInfo[eraValue];
              availableUnlockedRewards = availableUnlockedRewards.add(era.eraRewardPerBlock.mul((100-era.eraLockPercentage)).div(100));
              availableLockedRewards = availableLockedRewards.add(era.eraRewardPerBlock.mul(era.eraLockPercentage).div(100));
          ...
          uint256 blockStart = eraCheck.eraStartBlock < lastRewardBlock ? lastRewardBlock : eraCheck.eraStartBlock;
          uint256 blockMax = eraCheck.eraStartBlock.add(blocksPerVesting-1) < blockNumber ? eraCheck.eraStartBlock.add(blocksPerVesting-1) : blockNumber;

          for (uint256 i=blockStart; i<blockMax;i++) {
            availableUnlockedRewards = availableUnlockedRewards.add(rewardPerBlock);
          }

          unlockedRewards = availableUnlockedRewards;
          lockedRewards = lockedRewards.add(user.lockedAmount.sub(unlockedRewards));
        }
