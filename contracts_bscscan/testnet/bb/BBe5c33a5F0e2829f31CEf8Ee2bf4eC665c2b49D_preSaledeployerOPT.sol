...
contract preSaledeployerOPT {
    ...
    constructor() {
        ...
        projectTokenFee = 3;
        userFee = 10;
        routerAddress = (0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3); // 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3
    }
...
contract preSale {
    ...
    uint256 public projectCoinFee;
    uint256 public projectTokenFee;
    uint256 public adminFeeCounterPublic;
    uint256 public adminFeeCounterPrivate;
    uint256 public adminFeeCounterSeed;
...
*/
    ...
    function initialize(
        ...
        require(msg.sender == deployer, "FORBIDDEN"); // sufficient check
    ...
    function buyToken(uint256 _type, uint256 _amount) public allowed {
        require(block.timestamp < preSaleEndTime, "Time over"); // time check
        ...
        if (_type == 1) {
            ...
            adminFeeCounterPublic = adminFeeCounterPublic.add(_fee);
            ...
            require(whiteListPrivate[msg.sender], "Not whiteListed");
            ...
            adminFeeCounterPrivate = adminFeeCounterPrivate.add(_fee);
            ...
            require(whiteListSeed[msg.sender], "Not whiteListed");
            ...
            adminFeeCounterSeed = adminFeeCounterSeed.add(_fee);
    ...
    function claim() public allowed {
        require(block.timestamp > preSaleEndTime, "Presale time not over");
        require(canClaim, "Wait for the owner to end preSale");
        require(
            tokenBalancePublic[msg.sender]
                .add(tokenBalancePrivate[msg.sender])
                .add(tokenBalanceSeed[msg.sender]) > 0,
            "No claim able balance"
        );
        require(
            claimCount[msg.sender] <= currentClaimCycle,
            "you have already claimed in this vesting"
        );
    ...
    function endPreSale() public onlyTokenOwner allowed {
        require(!presaleEnd, "Presale already ended");
        if (voteUp < voteDown) {
        ...
        if (!profitClaim) {
            ...
            canClaim = true;
            _adminTokenFee = adminFeeCounterPublic
                .add(adminFeeCounterPrivate)
                .add(adminFeeCounterSeed);
            if (_adminTokenFee > 0) token.transfer(admin, _adminTokenFee);
            adminFeeCounterPublic = 0;
            adminFeeCounterPrivate = 0;
            adminFeeCounterSeed = 0;
        }
    ...
    function vote(bool _vote) public {
        require(token.balanceOf(msg.sender) > 0, "!Holder");
        require(
            !usersVoting[msg.sender][currentVotingCycle].voteCasted,
            "Voted"
        );
        require(votingStatus, "!Allowed");
        require(
    ...
    function startVoting(uint256 _endTime) external onlyAdmin {
        require(!votingStatus, "Started");
        require(block.timestamp > preSaleEndTime.add(vestingTime), "Ended");
        votingStatus = true;
        voteUp = 0;
        voteDown = 0;
