...
contract HearnToken is ERC20("HEARN", "HEARN") {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if (contractTokenBalance > 0) {
            ...
            uint256 busdAmount = swapTokensForBusd(half, address(this)); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered

            // add liquidity to pancakeswap
            if (otherHalf > 0 && busdAmount > 0) {
                addLiquidityAndBurn(otherHalf, busdAmount);
