...
contract SP1 is Context, IERC20, Ownable {
    ...
    uint256 public swapTokensAtBNBAmount;
    ...
    constructor () {
        ...
        // Swap tokens(for reward, liquidity, other fees) when contract balance is 'swapTokensAtBNBAmount' worth
        swapTokensAtBNBAmount = 2 ether; // 1 BNB
    ...
    function updateSwapTokensBNBLimit(uint256 _limit) external onlyOwner {
        require(_limit > 0, "Invalid swap tokens limit");
        swapTokensAtBNBAmount = _limit;
    }

    function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
        require(taxFee<1500, "Tax higher 15 leads to several issues and is not allowed");
        _taxFee = taxFee;
    ...
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
        require(liquidityFee<1500, "Liquidity fee higher 15 leads to several issues and is not allowed");
    ...
    function _sendTax(uint256 rFee, uint256 tFee) private{
        ...
        if(tMarketingTax > 0 && marketingWallet!= address(0)){
            ...
            if(marketingTaxinBNB > 0){
                ...
                //transferTo(marketingWallet, marketingTaxinBNB);
                if(sent){
                    initialBalance = 0;
                }
            }*/
            ...
            _marketingTaxToRedistr += tMarketingTax; // to calculate the redistribution amount
        }

        if(tDevTax > 0 && devWallet!= address(0)){
            ...
            _devTaxToRedistr += tDevTax; // to calculate the redistribution amount
        }
    ...
    function _transfer(
        ...
        bool overMinTokenBalance = contractTokenBalance >= getAmountTokensOut(swapTokensAtBNBAmount);
        if (
            tradingEnabled &&
            overMinTokenBalance &&
            !inSwapAndLiquify &&
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 half = ((contractTokenBalance.sub(_marketingTaxToRedistr).sub(_devTaxToRedistr)).div(2)).add(_marketingTaxToRedistr).add(_devTaxToRedistr);
        ...
        // calculate marketing bnb
        uint256 marketingBNB = getAmountBNBOut(_marketingTaxToRedistr);
        // calculate dev bnb
        uint256 devBNB = getAmountBNBOut(_devTaxToRedistr);

        // swap tokens for ETH
        swapTokensForEth(half); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered // including the tax for marketing and dev

        // redistribution marketing
        (sent, data) = address(marketingWallet).call{value: marketingBNB}("");
        if(sent){
            marketingBNB = 0;
        ...
        // redistribution dev
        (sent, data) = address(devWallet).call{value: devBNB}("");
        if(sent){
            devBNB = 0;
            _devTaxToRedistr = 0;
        }
        require(sent, "Failed to send to Dev Wallet");
    ...
    function calculateSellTax(address seller, uint256 amount) private {
        ...
        if (length > 0) {
            for (uint256 i = length; i > 0; i--) {
                if (i >= 1  && (_sellData[seller][i].timeStamp - _sellData[seller][i-1].timeStamp) <= maxSellAllowanceTime){
        ...
        if (totalBNB == maxSellAllowanceBeforeExtra && bnbAmount < maxSellAllowanceBeforeExtra) {
        ...
        _taxFee = (
            (regularAmount.mul(_sellTaxFee).div(10000)).add(
                extraAmount.mul(_sellExtraTaxFee).div(10000)
            )
        ).div(bnbAmount);
        marketingTax = (
            (regularAmount.mul(sellMarketingTax).div(10000)).add(
                extraAmount.mul(sellExtraMarketingTax).div(10000)
            )
        ).div(bnbAmount);
        devTax = (
            (regularAmount.mul(sellDevTax).div(10000)).add(
                extraAmount.mul(sellExtraDevTax).div(10000)
            )
        ).div(bnbAmount);*/
        if(extraAmount > 0 && regularAmount == 0){
            _liquidityFee = _sellLiquidityFee.add(_sellExtraLiquidityFee);
            _taxFee = _sellTaxFee.add(_sellExtraTaxFee);
            devTax = sellDevTax.add(sellExtraDevTax);
            marketingTax = sellMarketingTax.add(sellExtraMarketingTax);
            ...
            // example calculation (1*1300/10000+((4*600/10000)-(1*1300/10000))/5)*10000
            _liquidityFee = (
                (regularAmount.mul(_sellLiquidityFee).div(10000)).add(
                    extraAmount.mul(_sellExtraLiquidityFee).div(10000)
                )
            ).div(bnbAmount)*10000;
            _taxFee = (
                (regularAmount.mul(_sellTaxFee).div(10000)).add(
