...
contract MetaDracarys is Context, IERC20, Ownable {
    ...
    uint256 public _maxTxAmount = 21000000000000 * 1 / 100; // Whale protection
    ...
    function setDevFeePercent(uint256 devFee) external ownerOrDevAccessOnly {
        _devFee = devFee;
    }
    ...
    function setDevelopment(address _address) public onlyOwner {
        _development = _address;
    }
    ...
    function _updateDev(uint256 tDev) private {
        uint256 currentRate =  _getRate();
        uint256 rDev = tDev.mul(currentRate);
        _rOwned[_development] = _rOwned[_development].add(rDev);
        if(_isExcluded[_development])
            _tOwned[_development] = _tOwned[_development].add(tDev);
    }
    ...
    function clearStuckBalance(uint256 amountPercentage) external ownerOrDevAccessOnly {
        ...
        payable(_development).transfer(amountBNB * amountPercentage / 100);
    }
    ...
    function claimTokens() public {
        ...
        setClaimableAllowances(msg.sender, 0);
    }
}