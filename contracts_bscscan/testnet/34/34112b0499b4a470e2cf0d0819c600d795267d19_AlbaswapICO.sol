...
contract AlbaswapICO {
    ...
    uint256 public tokenLaunch;
     ...
     function deposit(uint256 _amount) public {
      ...
      function pendingAlba() public view returns (uint256) {
          UserInfo storage user = userInfo[msg.sender];
          uint256 locked = user.totalToken + user.referralbonus;
          return  locked -  user.claimToken ;
       }
    ...
    function claimAlba() public {
           ...
           user.claimToken += available;
             if (available > bgtBal) {
                albaToken.transfer(msg.sender, bgtBal);
            } else {
                albaToken.transfer(msg.sender, available);
            }
      }
