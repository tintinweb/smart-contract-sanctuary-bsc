...
contract Crowdsale {
    ...
    function buyTokens(address _beneficiary) public payable {
        ...
        require(weiAmount >= minWeiAmount, "BNB amount is too small");
    ...
    function _deliverTokens(address _beneficiary, uint256 _tokenAmount) internal virtual {
        IERC20(token).safeTransfer(_beneficiary, _tokenAmount);
    }
    ...
    function getTokenAmountAvailable() public view virtual returns (uint256) {
        return IERC20(token).balanceOf(address(this));
    }

    function getMaxReceivingTokenToPurchase() public view returns (uint256) {
        return getTokenAmountAvailable() / rate;
    }
...
contract TokenPlaySaleByBNB is Crowdsale, Ownable {
    using SafeERC20 for IERC20;
    ...
    event TokenPurchaseWithRef(address indexed purchaser, address indexed beneficiary, address referrer, uint256 value, uint256 amount);
    ...
    constructor(
        ...
        address payable _wallet
    ) Crowdsale(_rate, _minWeiAmount, _wallet, address(0)) {
        cap = _cap;
    ...
    function _preValidatePurchase(address _beneficiary, uint256 _weiAmount) internal override {
        ...
        require(individualCap == 0 || walletPurchased[_beneficiary] + _weiAmount <= individualCap, "Over cap");
    }

    function _updatePurchasingState(address _beneficiary, uint256 _weiAmount) internal override {
        super._updatePurchasingState(_beneficiary, _weiAmount);
        walletPurchased[_beneficiary] += _weiAmount;
    }

    function buyTokensSelf() public payable {
        buyTokens(msg.sender);
    }

    function buyTokensSelfWithRef(address _referrer) external payable {
        address _beneficiary = msg.sender;
        uint256 weiAmount = msg.value;
        uint256 tokens = _getTokenAmount(weiAmount);
        emit TokenPurchaseWithRef(_beneficiary, _beneficiary, _referrer, weiAmount, tokens);
        buyTokensSelf();
    }

    function setMinWeiAmount(uint256 _minWeiAmount) external onlyAdmin {
        require(_minWeiAmount > 0);
        minWeiAmount = _minWeiAmount;
    }

    function setIndividualCap(uint256 _individualCap) external onlyOwner {
        individualCap = _individualCap;
    }

    function getTokenAmountAvailable() public view override returns (uint256) {
        return cap;
    }

    function getMaxBnbToPurchase() external view returns (uint256) {
        return getTokenAmountAvailable() / rate;
    }

    function _deliverTokens(address _beneficiary, uint256 _tokenAmount) internal override {
        balanceOf[_beneficiary] += _tokenAmount;
        cap -= _tokenAmount;
    }
