...
contract AirDropLottery is Manager, math{
	...
	address public BNBWinner;
	address[] public RestCabinList;
	address[] public OxygenMaskList;
	address[] public GravityShoesList;
	address[] public LimitedBeginnerGiftList;
	address[] public PreAlphaList;
	...
	function checkBNBWinner() public view returns(address){
        return BNBWinner;
    }

	function checkPreAlphaList(uint sort) public view returns(address){
        return PreAlphaList[sort];
    }
	...
	function checkLimitedBeginnerGiftList(uint sort) public view returns(address){
		return LimitedBeginnerGiftList[sort];
	}
    ...
    function getPreAlphaListLength() public view returns (uint256) {
        return PreAlphaList.length;
    }
	...
	function getLimitedBeginnerGiftListLength() public view returns (uint256) {
		return LimitedBeginnerGiftList.length;
	}
	...
	//--Draw PreAlpha Manager only--//
    function drawPreAlpha(uint _drawTimes) public onlyManager{
    ...
    function drawWinner() public onlyManager{
        uint winnerLength = winnerList.length;
		require(winnerLength < 69, "AirDropNFTLottery : Length error.");
        uint _WhitelistAmount = getWhitelistLength();
        bytes memory seed = abi.encodePacked(_WhitelistAmount);
        uint drawNO = rand(seed, 0, _WhitelistAmount.sub(1));
		...
		if(winnerLength == 0){
			BNBWinner = _winner;
		}else if(winnerLength > 0 && winnerLength <= 3){
			RestCabinList.push(_winner);
		}else if(winnerLength > 3 && winnerLength <= 8){
			OxygenMaskList.push(_winner);
		}else if(winnerLength > 8 && winnerLength <= 18){
			GravityShoesList.push(_winner);
		}else if(winnerLength > 18 && winnerLength <= 68){
			LimitedBeginnerGiftList.push(_winner);
		}
