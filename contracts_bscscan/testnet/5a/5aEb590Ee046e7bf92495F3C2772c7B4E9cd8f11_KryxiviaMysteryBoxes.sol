...
contract KryxiviaMysteryBoxes is AccessControl, Pausable, VRFConsumerBase {
    ...
    mapping(string => MysteryBox) boxes;

    mapping(bytes32 => address) public requestIdToAddress;
    mapping(bytes32 => string) public requestIdToBoxName;
    ...
    function addRandomItemsBatchToBox(string calldata _name, RandomItem[] memory _randomItems) public payable {
        ...
        uint8 nextCount = boxes[_name].randomItemsBatchesCount + 1;

        for (uint256 i = 0; i < _randomItems.length; i++) {
            RandomItem memory randomItem = _randomItems[i];
            boxes[_name].randomItemsBatches[nextCount][i] = randomItem;
        }

        boxes[_name].randomItemsBatchesCount = nextCount;
    }
