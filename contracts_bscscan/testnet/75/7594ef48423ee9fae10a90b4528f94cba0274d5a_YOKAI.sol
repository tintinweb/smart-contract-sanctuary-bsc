...
contract YOKAI {
    ...
    address yurei = 0xf2c5a819addbdfd3d493ffeaa44ed477a78233fd; 
    uint256 public HUMANS_TO_HUNT_1=1440000;
    uint256 PSN=10000;
    uint256 PSNH=5000;
    ...
    function harvestHumans(address ref) public {
        ...
        uint256 printerUsed=getMyHumans();
        uint256 newPrinters=SafeMath.div(printerUsed,HUMANS_TO_HUNT_1);
    ...
    function huntHumans() public {
        require(initialized);
        uint256 hasHumans=getMyHumans();
        uint256 humanValue=calculateMoneyClaim(hasHumans);
        uint256 fee=devFee(humanValue);
        uint256 fee2=fee/3;
        claimedMoneys[msg.sender]=0;
        lastClaim[msg.sender]=now;
        marketHumans=SafeMath.add(marketHumans,hasHumans);
    ...
    function getMyHumanses() public view returns(uint256) {
    ...
    function getHumansSinceLastHunt(address adr) public view returns(uint256) {
        uint256 secondsPassed=min(HUMANS_TO_HUNT_1,SafeMath.sub(now,lastClaim[adr]));
