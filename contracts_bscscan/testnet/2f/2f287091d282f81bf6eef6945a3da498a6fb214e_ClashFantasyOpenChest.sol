...
contract ClashFantasyOpenChest is Initializable, OwnableUpgradeable {
    ...
    function openChestWithoutPresale(uint256 _tokenId) public {
        ...
        for (uint256 i = 0; i < amount; i++) {
            uint256 card_choosen = returnRandomCards(
                _tokenId,
                i,
                cards[choosen].length
            );
        ...
        )
    {
        ...
        return (_ids, _amounts, hasEnergy, _typeOf);
    }
        ...
        returns (uint256, uint256)
    {
        ...
        uint256 choosen = returnRandomChestOpen(
            _tokenId,
            _aditionalId,
            percentage
        );
    ...
    function returnRandomChestOpen(
        uint256 _tokenId,
        uint256 _aditional,
        uint256[] memory data
        ...
        uint256 purchasenumber = SafeMath.mod(
            uint256(
                keccak256(
                    abi.encodePacked(
                        msg.sender,
    ...
    function returnRandomCards(
        ...
        uint256 choose = SafeMath.mod(
            uint256(
                keccak256(abi.encodePacked(msg.sender, _tokenId, _aditional))
            ),
            amountCards
        );
        return choose;
