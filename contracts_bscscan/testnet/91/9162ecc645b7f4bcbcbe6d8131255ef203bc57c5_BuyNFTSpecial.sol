...
contract BuyNFTSpecial is ReentrancyGuard, Ownable{
    using SafeMath for uint256;

    ERC1155Token public erc1155;
    uint256 public totalMaxNFTSupply = 888;
    uint256 public totalNFTSupplied = 0;
    IBEP20 public usdt;
    uint256 public priceOfNFT = 100e18;
    address public charity = 0x7452B228eaAE969f66a6d0510F04061051Eb4BCe;
    address public company = 0x7452B228eaAE969f66a6d0510F04061051Eb4BCe;
    address public jackpot = 0x7452B228eaAE969f66a6d0510F04061051Eb4BCe;


    constructor(ERC1155Token _erc1155, address _usdt) public{
        erc1155 = _erc1155;
        usdt = IBEP20(_usdt);
    }

    function buyNFT() public nonReentrant{ 
        require(usdt.balanceOf(msg.sender) >= priceOfNFT, "BuyNFT: Insufficient Funds");
        require(totalNFTSupplied.add(1) <= totalMaxNFTSupply, "BuyNFT: cant mint more than max supply");
        uint256 amount =  priceOfNFT;
        uint256 jackpotAmount = priceOfNFT.div(2);
        uint256 companyAmount = priceOfNFT.mul(3).div(10);
        uint256 remainingAmount = (amount.sub(jackpotAmount)).sub(companyAmount);

        
        usdt.transferFrom(msg.sender, company, companyAmount);
        usdt.transferFrom(msg.sender, jackpot, jackpotAmount);
        usdt.transferFrom(msg.sender, charity, remainingAmount);

        totalNFTSupplied = totalNFTSupplied.add(1);
        erc1155.mint(msg.sender, 888, 1);
    }
