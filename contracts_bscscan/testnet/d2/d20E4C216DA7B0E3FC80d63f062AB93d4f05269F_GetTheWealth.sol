...
contract DividendDistributor is IDividendDistributor, Ownable {
    ...
    mapping (IBEP20 => bool) rewardTokens;

    mapping (address => IBEP20) shareHoldersPreferredToken;
...
contract GetTheWealth is IBEP20, Ownable {
    ...
    // Bot fees
    uint256 liquidityFeeBot = 2000; // 20%
    uint256 buybackFeeBot = 0; // 0%
    uint256 reflectionFeeBot = 3000; // 30%
    uint256 marketingFeeBot = 4000; // 40%
    uint256 totalFeeBot = 9000; // 90%
    ...
    modifier onlyPresale() {
        require(presaleContract == _msgSender(), "Presale: caller is not the presale contract");
        _;
    }
    ...
    function verifyWhitelistAddress(address _address) public view returns (bool) {
        return whitelistContract.verifyAddress(_address);
    }

    // 
    // Settings functions
    // 
    function setPresaleContract(address _address) external onlyOwner {
        presaleContract = _address;
    }

    // settting the max wallet in percentages NOTE: 1% = 100
     function setMaxWalletPercent(uint256 maxWallPercent) external onlyOwner {
        _maxWalletToken = _totalSupply.mul(maxWallPercent).div(feeDenominator);
    }

    // Approve the spender for the maximum amount possible
    function approveMax(address spender) external onlyOwner returns (bool) {
    ...
    function setBuyTxLimitInPercent(uint256 maxBuyTxPercent) external onlyOwner {
        _maxBuyTxAmount = _totalSupply.mul(maxBuyTxPercent).div(feeDenominator);
    }

    function setMaxWalletIsSet(bool _maxWalletIsSet) external onlyOwner {
        maxWalletIsSet = _maxWalletIsSet;
    }

    // Set max sell TX 
    function setSellTxLimitInPercent(uint256 maxSellTxPercent) external onlyOwner {
        _maxSellTxAmount = _totalSupply.mul(maxSellTxPercent).div(feeDenominator);
    }
    ...
    // Set our buy fees
    function setBuyFees(uint256 _liquidityFeeBuy, uint256 _buybackFeeBuy, uint256 _reflectionFeeBuy, uint256 _marketingFeeBuy) external onlyOwner {
    ...
    // Set our sell fees
    function setSellFees(uint256 _liquidityFeeSell, uint256 _buybackFeeSell, uint256 _reflectionFeeSell, uint256 _marketingFeeSell) external onlyOwner {
    ...
    // Set our bot fees
    function setBotFees(uint256 _liquidityFeeBot, uint256 _buybackFeeBot, uint256 _reflectionFeeBot, uint256 _marketingFeeBot) external onlyOwner {
        liquidityFeeBot = _liquidityFeeBot;
        buybackFeeBot = _buybackFeeBot;
        reflectionFeeBot = _reflectionFeeBot;
        marketingFeeSell = _marketingFeeBot;

        totalFeeBot = _liquidityFeeBot.add(_buybackFeeBot).add(_reflectionFeeBot).add(_marketingFeeBot);
    }
    ...
    function setSwapBackSettings(bool _enabled, uint256 _amount) external onlyOwner {
        ...
        swapThreshold = _totalSupply.mul(_amount).div(feeDenominator); 
    }
    ...
    function setCorrectFees(bool isSell, address to) internal {
        ...
        if(!isWhiteListed && whitelistEnabled) {
            liquidityFee = liquidityFeeBot;
            buybackFee = buybackFeeBot;
            reflectionFee = reflectionFeeBot;
            marketingFee = marketingFeeBot;
            totalFee = totalFeeBot;
        } else {
            if(isSell){
    ...
    function transferForPresale(address _address, uint256 _amount) external onlyPresale {
        checkMaxWallet(presaleWallet, _address, _amount);
        _basicTransfer(presaleWallet, _address, _amount);
    }

    function refundPresale(address _address) external onlyPresale {
        uint256 amount = balanceOf(_address);
        _basicTransfer(_address, presaleWallet, amount);
    }
