...
contract Wenabisv2 is  ERC20, Ownable,Pausable,ERC20Burnable {
    ...
    constructor ( uint256 _decimals, uint256 _supply, uint256 _txFee,uint256 _burnFee,uint256 _charityFee,address _FeeAddress,address tokenOwner,address service) ERC20("Wn8", "WN8") payable   {
		_DECIMALS = _decimals;
    ...
    function setMaxTxPercent(uint8 percent) external onlyOwner {
    ...
    function setMaxWalletPercent(uint8 percent) external onlyOwner {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override  {
        ...
        require(!isBlacklisted(msg.sender), "Wenabisv2: sender en lista negra");
        require(!isBlacklisted(recipient), "Wenabisv2n: receptor en  blacklisted");
        require(!isBlacklisted(tx.origin), "Wenabisv2: sender en  blacklisted");

        if(!_isExcluded[sender] && owner() != sender) {
            require(amount < _maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
        }

        if(!_isExcluded[recipient] && owner() != recipient && address(this) != recipient && deadAddress != recipient) {
            require((balanceOf(recipient) + amount) < _maxWalletAmount, "you are violating anti-whale rules no more 5% from total supply");
        }
    ...
    function updateFee(uint256 _txFee,uint256 _burnFee,uint256 _charityFee) public onlyOwner{
