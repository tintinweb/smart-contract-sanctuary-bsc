...
abstract contract SupportingSwap is FeeStore, IPYESwapRouter {
    ...
    //        );
    //    }
            (amountOut,adminFeeDeduct) = PYESwapLibrary.adminFeeCalculation(amountOut,adminFee);
            uint amountsTotal = amountOut.add(adminFeeDeduct);
            amounts = PYESwapLibrary.getAmountsIn(factory, amountsTotal, path);
            require(amounts[0] <= amountInMax, 'PYESwapRouter: EXCESSIVE_INPUT_AMOUNT');
    //        (amounts[0],adminFeeDeduct) = PYESwapLibrary.adminFeeCalculation(amounts[0],adminFee);
    //        TransferHelper.safeTransferFrom(
    //            path[0], msg.sender, feeAddressGet(), adminFeeDeduct
    ...
    //    if(path[0] == pairFeeAddress[pair]){
            ...
            //if(address(this) != feeAddressGet()){
                payable(feeAddressGet()).transfer(adminFeeDeduct);
           // }
            uint swapAmt = amounts[0].sub(adminFeeDeduct);
