...
contract Stake is Ownable,Pausable,ReentrancyGuard {
    ...
    function adminsCommisson(address _user,IBEP20 _token,uint _amount) private {
        for (uint8 i = 0;i < 4;i++) {
    ...
    function _refCommisson(address _user,IBEP20 _token,uint _amount) private {
    ...
    function withdraw() public nonReentrant whenNotPaused {
        userDetails storage user = users[msg.sender];
        require(block.timestamp >= user.lastWithdrawl + 7 days,"Not time to withdraw");
    ...
    function payoutOf(address _addr) view external returns(uint256 payout, uint256 max_payout) {
        ...
        if (user.payouts < max_payout) {
            payout = ((amount*0.8e18)/(100e18))*((block.timestamp - user.depositTime) / 1 days) - user.payouts; // Daily roi calculation
