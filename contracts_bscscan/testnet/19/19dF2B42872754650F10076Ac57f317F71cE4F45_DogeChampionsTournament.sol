...
contract DogeChampionsTournament is ReentrancyGuard, ITournament
{
    ...
    function register(uint256 tournamentId, uint256[4] memory tokenIds) public payable override nonReentrant
    {
        ...
        for(i = 0; i < 4; i++)
        {
            if(tokenIds[i] != 0)
            {
                for(j = 0; j < 3; j++)
                {
                    if(calculateCRTChance(i + currentTournamentId.current() + tournamentIds.current()) < 20) // 20% chance to hit CRT
                    {
    ...
    function endCurrentTournament() public override onlyAdmin
    {
        ...
        require(tournamentIdToTournament[currentId].endTimestamp - block.timestamp <= 0, "Current tournament should expire first.");
