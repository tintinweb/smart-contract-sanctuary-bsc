...
contract ArkarusTestFee4 is Context, IBEP20, Ownable {
  ...
  */
  ...
  function transfer(address recipient, uint256 amount) override external returns (bool) {
    if(_msgSender() == pancakeLPAddress && buyFee > 0) {
  ...
  function transferFrom(address sender, address recipient, uint256 amount) override external returns (bool) {
    ...
    if(recipient == pancakeLPAddress && _sellFee > 0) {
      _transfer(sender, feeReceiveAddress, amount * _sellFee / 100);
      _transfer(sender, recipient, amount * (100 - _sellFee)/100);
    }
    else {
