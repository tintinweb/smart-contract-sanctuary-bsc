...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    uint256 public startingRange;
    uint256 public endingRange;
    ...
    // }
    constructor(
        string memory name_, // name of collection
        string memory symbol_, // Symbol of collection
        string memory baseURI_, // baseURI of collection
        uint256 _royality, // Royality for collection
        uint256 _startingRange, // Hiding NFT starting range
        uint256 _endingRange, // Hiding NFT ending range
        uint256 _maxSupply, // Number of NFTs
        string memory _placeholderURI, // PlaceHolder URI for hidden NFTs.
        ...
        royality = _royality;
        startingRange = _startingRange;
        endingRange = _endingRange;
        placeholderURI = _placeholderURI;
        collectionCreator = payable(creator);
        _balances[creator] = maxSupply;
    ...
    function ownerOf(uint256 tokenId) public view override returns (address) {
        require(_exists(tokenId), "Collection: quering non existing token.");
        if (
            (!isBurned[tokenId] &&
    ...
    function _exists(uint256 tokenId) internal view returns (bool) {
        if (
            ((_owners[tokenId] == address(0) && !isBurned[tokenId]) ||
                _owners[tokenId] != address(0)) && tokenId <= maxSupply
        ) {
            return true;
        }
        return false;
    ...
    function _mint(address to, uint256 tokenId) internal {
        ...
        // _balances[to] += 1; // Disable this as NFT's are already minted.
        _owners[tokenId] = to;
    ...
    function _transfer(
        ...
        // mint NFT is being transfered first time.
        if(_owners[tokenId] == address(0) && !isBurned[tokenId]){
            _mint(collectionCreator, tokenId);
        }
    ...
    function setOnSale(bool _onSale, uint256 _price) external onlyOwner {
        onSale = _onSale;
        salePrice = _price;
    }
    ...
    function mint(uint256 tokenId) public payable {
        require(onSale, "Collection: Collection is not on sale.");
        require(msg.value >= salePrice, "Collection: Insufficient fund.");
        require(
            ownerOf(tokenId) == collectionCreator,
            "Collection: for initial Minting only."
        );

        _mint(collectionCreator, tokenId);
        _transfer(collectionCreator, msg.sender, tokenId);
    }
    ...
    function batchTransfer(
        ...
        require(
            tokenIds.length == toAddresses.length,
            "Collection: tokenIds and toAddresses must have same length"
        );
...
contract CollectionFactory is Ownable {
     ...
     * @param _name Display name for  collection contract
     * @param _symbol Symbol for collection contract
     * @param _baseURI prefix for tokenURI of NFT contract
     */
    function createCollection(
        string memory _name, // name of collection
        string memory _symbol, // Symbol of collection
        string memory _baseURI, // baseURI of collection
        uint256 _royality, // Royality for collection
        uint256 _startingRange, // Hiding NFT starting range
        uint256 _endingRange, // Hiding NFT ending range
        uint256 _maxSupply, // Number of NFTs
    ...
    function removeCollection(address collection) public {
        ...
        require(
            _userCollections[msg.sender].exists(collection),
            "Collection doesn't exists"
        );
        Ownable _collection = Ownable(collection);
        require(
            _collection.owner() == address(0),
            "renounceOwnership of contract required"
        );
