...
contract xpndStaking is Ownable, ReentrancyGuard, AccessControl {
    ...
    struct PoolInfo {
        ...
        uint256 poolReward;
        uint16 poolInstance;
        mapping(address => uint16[]) poolStakeByAddress;
    }
        ...
        for (uint8 poolType = 0; poolType < poolTypes.length; ++poolType) {
            if (listOfAllPoolIds[poolType].length == 0) {
                ...
                if (
                    ...
                    // latest poolType pool has ended, reward should be collected
                    // in default bucket and added to the new instance of poolType when it starts
                    defaultRewardBuckets[poolType] +=
                        (_amount *
                            poolTypes[poolType].proportionalRewardShare) /
                        totalRewardPercent;
    ...
    // Deposit tokens to pool of _poolType for Reward allocation.
    ...
    function withdrawPoolInstance(uint16 _poolInstance) external nonReentrant {
        ...
        emit WithdrawPoolInstance(msg.sender, _poolInstance, rewardAmount);
    }

    function withdrawPool(uint8 _poolType) external nonReentrant {
        require(_poolType < poolTypes.length, "Invalid poolInstance");
        ...
        emit WithdrawPool(msg.sender, _poolType, rewardAmount);
    }
