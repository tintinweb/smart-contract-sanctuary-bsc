...
//  𝗦𝗼𝗹𝘂𝘁𝗶𝗼𝗻 𝗣𝗿𝗼𝘃𝗶𝗱𝗲𝗿: HyperchainX Technology
    ...
    contract ZOUTBREAK_AUDIT is Context, IERC20, Ownable {
        ...
        string private _name = "Z OutBreak (Audit)";
        ...
        function F15__Set_Product_Development_Fee_Portion(uint256 fee_percent) external onlyCEO {
            ...
            // IMPORTANT: 
            // pdf + mf + bsf + rsvf = 100
            uint256 Total_All_Portions =  fee_percent + mf + bsf + rsvf;
        ...
        function F16__Set_Marketing_Fee_Portion(uint256 fee_percent) external onlyMarketingManager {
            ...
            // IMPORTANT: 
            // pdf + mf + bsf + rsvf = 100
            uint256 Total_All_Portions =  pdf + fee_percent + bsf + rsvf;
            require(Total_All_Portions <= 100, 
            "The sum of all fees portions must be less or equal 100");
            mf = fee_percent;
        }
        function F17__Set_BlockchainSupport_Fee_Portion(uint256 fee_percent) external onlyBlockchainManager {
            ...
            // IMPORTANT: 
            // pdf + mf + bsf + rsvf = 100
            uint256 Total_All_Portions =  pdf + mf + fee_percent + rsvf;
            require(Total_All_Portions <= 100,
            "The sum of all fees portions must be less or equal 100");
            bsf = fee_percent;
        }
        function F18__Set_Reserva_Fee_Portion(uint256 fee_percent) external onlySecurityManager {
            ...
            // IMPORTANT: 
            // pdf + mf + bsf + rsvf = 100   
            uint256 Total_All_Portions =  pdf + mf + bsf + fee_percent;        
            require(Total_All_Portions <= 100, 
            "The sum of all fees portions must be less or equal 100");
            rsvf = fee_percent;
        }
