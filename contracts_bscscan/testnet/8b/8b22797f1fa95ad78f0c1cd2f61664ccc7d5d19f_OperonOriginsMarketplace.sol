...
contract OperonOriginsMarketplace {
    ...
    uint256 private constant defaultRoy = 5;
    ...
    function placeOffering(
        ...
        blockedNFTBalances[compositeKey] += tokenBalance;
        ...
        offeringRegistry[offeringId].amount = tokenBalance;
        ...
        payable
    {
        ...
        require(tokenBalanceNFT >= amount, "Error: Insufficient balance");
        ...
        if (
            ...
            tokenContract.transfer(adminAddress, _adminfee);

            tokenContract.transfer(nftCreator, roy);
        ...
        offeringRegistry[_offeringId].amount = tokenBalanceNFT - amount;

        if (tokenBalanceNFT - amount == 0) {
        ...
        blockedNFTBalances[compositeKey] -= amount;
     ...
     *   @param _offeringId identifier of offering
     */
    function purchaseOffering3(bytes32 _offeringId, uint256 amount)
        external
        payable
    {
        require(
    ...
    function mint(
        ...
        OperonOriginsNFT hostContract = OperonOriginsNFT(_hostContract);
        _royal[id] = royalty * (10**uint256(decimal));
    ...
    function transfer(
        ...
        require(tokenBalance >= amount, "Insufficient balance");
        hostContract.safeTransferFrom(from, to, id, amount, data);
    ...
    function percentageCalc(uint256 value1, uint256 value2)
        internal
        pure
        returns (uint256)
    {
        uint256 result = (value1 * value2) / 10**uint256(10);
        return (result);
    }
    ...
    {
        uint256 fee = percentageCalc(amount, _serviceValue);
        uint256 roy = percentageCalc(amount, royal);
        uint256 netamount = amount - roy;
