...
contract SAITAMAMA is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 10 * 10**11 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "ELEKTRON DEFI";
    ...
    address public marketingWallet = 0x422B0755EABeA90Cc2C5674F8Bba65C861962fdD;
    
   
    uint256 public _developmentFee = 30;
    address public sustainableAddress = 0x422B0755EABeA90Cc2C5674F8Bba65C861962fdD;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, (amount.sub(marketingAmt).sub(sustainableAmt)));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, (amount.sub(marketingAmt).sub(sustainableAmt)));
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender, recipient, (amount.sub(marketingAmt).sub(sustainableAmt)));
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, (amount.sub(marketingAmt).sub(sustainableAmt)));
        } else {
            _transferStandard(sender, recipient, (amount.sub(marketingAmt).sub(sustainableAmt)));
        }
