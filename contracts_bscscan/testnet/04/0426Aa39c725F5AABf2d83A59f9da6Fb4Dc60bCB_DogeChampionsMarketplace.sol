...
import "@openzeppelin/contracts/security/ReentrancyGuard.sol"; // to prevent reentry attacks
...
contract DogeChampionsMarketplace is ReentrancyGuard, ERC1155Holder
{
    event MarketItemCreated(uint256 itemId);
    event MarketItemSold(uint256 itemId);
    event MarketItemCancelled(uint256 itemId);

    event ConsumableMarketItemCreated(uint256 itemId);
    event ConsumableMarketItemSold(uint256 itemId);
    event ConsumableMarketItemCancelled(uint256 itemId);

    using Counters for Counters.Counter;
    Counters.Counter public _itemIds;
    Counters.Counter public _itemsSold;
    Counters.Counter public _numberOfItemsOnSale;

    Counters.Counter public _consumableItemIds;
    Counters.Counter public _consumablesSold;
    Counters.Counter public _numberOfConsumablesOnSale;

    address payable public owner;

    // NFTs that are rewarded from tournaments are ignored.
    // 10K from first mint, 10K from final mint, and 800 giveaway in total
    uint256 public maximumPossibleNFTSale = 20800;

    uint256 public dividendPerNFT;
    mapping(address => uint256) public minterProfits;
    mapping(uint256 => uint256) public paidDividendToNFTs;

    mapping(uint256 => uint256[]) public saleHistory;
    mapping(uint256 => uint256[]) public saleHistoryDate;
    
    mapping(uint256 => MarketItem) public idToMarketItem;
    mapping(uint256 => ConsumableMarketItem) public idToConsumableMarketItem;

    mapping(uint256 => uint256) public tokenIdToMarketItemId;
    mapping(address => uint256[]) public userToOnSaleConsumableMarketItemIds;

    mapping(address => bool) public availableNFTContractAddresses;
    mapping(address => bool) public availableSemiFungibleTokenContractAddresses;
    
    uint256 public totalNFTVolume = 0;
    uint256 public totalConsumableVolume = 0;
    ...
    // boolean flags to stop market sales, cancelations and purchases if needed
    bool public isMarketSaleOpen = true;
    bool public isMarketOperationsOpen = true;
    ...
    /*
     * @author data model for market items
     */
    struct MarketItem
    {
        uint256 itemId;
    ...
    /*
     * @author data model for consumable market items
     */
    struct ConsumableMarketItem
    {
        uint256 itemId;
        address nftContract;
        uint256 tokenId;
    ...
    /*
     * @author creates an NFT market item on marketplace
     */
    function createMarketItem(address nftContract, uint256 tokenId, uint256 price) public nonReentrant
    {
        require(availableNFTContractAddresses[nftContract] == true, "Can't sell NFTs from this contract.");
        require(isMarketOperationsOpen, "Creating market items is closed for now. Please contact to owner.");
        ...
        _numberOfItemsOnSale.increment();

        emit MarketItemCreated(itemId);
    }

    /*
     * @author creates a consumable market item on marketplace
     */
    function createConsumableMarketItem(address semiFungibleContract, uint256 tokenId, uint256 price, uint256 amount) public nonReentrant
    {
        require(availableSemiFungibleTokenContractAddresses[semiFungibleContract] == true, "Can't sell Semi Fungible Tokens from this contract.");
        require(isMarketOperationsOpen, "Creating market items is closed for now. Please contact to owner.");
        require(price > 0, "Price must be at least 1 wei");

        _consumableItemIds.increment();
        uint256 itemId = _consumableItemIds.current();

        idToConsumableMarketItem[itemId] = ConsumableMarketItem(
            itemId,
            semiFungibleContract,
            tokenId,
            payable(msg.sender),
            payable(address(0)),
            price,
        ...
        userToOnSaleConsumableMarketItemIds[msg.sender].push(itemId);

        IERC1155(semiFungibleContract).safeTransferFrom(msg.sender, address(this), tokenId, amount, "");

        _numberOfConsumablesOnSale.increment();

        emit ConsumableMarketItemCreated(itemId);
    }

    /*
     * @author creates a marketsale for NFTs and transfers price value between shareholders
     */
    function createMarketSale(address nftContract, uint256 itemId) public payable nonReentrant
    {
        require(isMarketSaleOpen, "Sales are closed for now. Please contact to owner.");
        require(!idToMarketItem[itemId].sold, "This item have been sold already.");
        require(!idToMarketItem[itemId].cancelled, "This item have been cancelled already.");
        require(idToMarketItem[itemId].price > 0, "There is no such item on sale.");
        uint256 price = idToMarketItem[itemId].price;
        uint256 tokenId = idToMarketItem[itemId].tokenId;
        address payable seller = idToMarketItem[itemId].seller;
        require(msg.value == price, "Please submit the asking price in order to complete the purchase");

        saleHistory[tokenId].push(msg.value);
        saleHistoryDate[tokenId].push(block.timestamp);

        uint256 percentAmount = msg.value / 100;
        address minter = dogeChampionsNFT.getMinterOfToken(tokenId);

        dividendPerNFT += (2 * percentAmount) / maximumPossibleNFTSale / 10 ** 9; // stored in gwei
        minterProfits[minter] = minterProfits[minter] + (3 * percentAmount) / 10 ** 9; // stored in gwei

        seller.transfer(94 * percentAmount);
        owner.transfer(1 * percentAmount);
        IERC721(nftContract).transferFrom(address(this), msg.sender, tokenId);
        idToMarketItem[itemId].owner = payable(msg.sender);
        idToMarketItem[itemId].sold = true;
        _itemsSold.increment();

        totalNFTVolume += price;

        dogeChampionsNFT.removeFromPlayableIds(seller, tokenId);
        dogeChampionsNFT.pushToPlayableIds(msg.sender, tokenId);

        tokenIdToMarketItemId[tokenId] = 0;

        _numberOfItemsOnSale.decrement();

        emit MarketItemSold(itemId);
    }

    /*
     * @author creates a marketsale for consumables and transfers price value between shareholders
     */
    function createConsumableMarketSale(address semiFungibleContract, uint256 itemId) public payable nonReentrant
    {
        require(isMarketSaleOpen, "Sales are closed for now. Please contact to owner.");
        require(!idToConsumableMarketItem[itemId].sold, "This item have been sold already.");
        require(!idToConsumableMarketItem[itemId].cancelled, "This item have been cancelled already.");
        require(idToConsumableMarketItem[itemId].price > 0, "There is no such item on sale.");
        uint256 price = idToConsumableMarketItem[itemId].price;
        uint256 amount = idToConsumableMarketItem[itemId].amount;
        uint256 tokenId = idToConsumableMarketItem[itemId].tokenId;
        address payable seller = idToConsumableMarketItem[itemId].seller;
        require(msg.value == price, "Please submit the asking price in order to complete the purchase");

        seller.transfer(msg.value);
        IERC1155(semiFungibleContract).safeTransferFrom(address(this), msg.sender, tokenId, amount, "");
        idToConsumableMarketItem[itemId].owner = payable(msg.sender);
        idToConsumableMarketItem[itemId].sold = true;
        _consumablesSold.increment();

        totalConsumableVolume += price;

        _numberOfConsumablesOnSale.decrement();

        emit ConsumableMarketItemSold(itemId);
    }

    /*
     * @author cancels an active NFT market item's listing
     */
    function cancelMarketItem(address nftContract, uint256 itemId) public nonReentrant
    {
        require(isMarketOperationsOpen, "Canceling sales is closed for now. Please contact to owner.");
        require(!idToMarketItem[itemId].sold, "This item have been sold already.");
        require(!idToMarketItem[itemId].cancelled, "This item have been cancelled already.");
        require(msg.sender == idToMarketItem[itemId].seller, "You are not seller of this item.");
    ...
    /*
     * @author cancels an active consumable market item's listing
     */
    function cancelConsumableMarketItem(address semiFungibleContract, uint256 itemId) public nonReentrant
    {
        require(isMarketOperationsOpen, "Canceling sales is closed for now. Please contact to owner.");
        require(!idToConsumableMarketItem[itemId].sold, "This item have been sold already.");
        require(!idToConsumableMarketItem[itemId].cancelled, "This item have been cancelled already.");
        require(msg.sender == idToConsumableMarketItem[itemId].seller, "You are not seller of this item.");

        idToConsumableMarketItem[itemId].cancelled = true;
        uint256 tokenId = idToConsumableMarketItem[itemId].tokenId;
        uint256 amount = idToConsumableMarketItem[itemId].amount;
        ...
        emit ConsumableMarketItemCancelled(itemId);
    }

    /*
     * @author returns single NFT market item for given itemId
     */
    function fetchMarketItem(uint256 itemId) public view returns(MarketItem memory)
    {
        require(itemId > 0 && itemId <= _itemIds.current(), "There is no such item on sale with given itemId.");
        return idToMarketItem[itemId];
    }

    /*
     * @author returns all unsold NFT market items
     */
    ...
    /*
     * @author returns all NFT market items including sold and cancelled ones
     */
    ...
    /*
     * @author returns single consumable market item for given itemId
     */
    function fetchConsumableMarketItem(uint256 itemId) public view returns(ConsumableMarketItem memory)
    {
        require(itemId > 0 && itemId <= _consumableItemIds.current(), "There is no such item on sale with given itemId.");
        return idToConsumableMarketItem[itemId];
    }

    /*
     * @author returns all unsold consumable market items
     */
    function fetchConsumableMarketItems() public view returns(ConsumableMarketItem[] memory)
    {
        uint256 itemCount = _consumableItemIds.current();
        uint256 unsoldItemCount = _consumableItemIds.current() - _consumablesSold.current();
        uint256 currentIndex = 0;

        ConsumableMarketItem[] memory items = new ConsumableMarketItem[](unsoldItemCount);
        for (uint256 i = 0; i < itemCount; i++)
        {
            if (idToConsumableMarketItem[i + 1].owner == address(0))
            {
                uint256 currentId = i + 1;
                items[currentIndex] = idToConsumableMarketItem[currentId];
                currentIndex += 1;
            }
        }
        return items;
    ...
    function withdrawRewards() public
    {
        ...
        {
            if (holdings[i] != 0 && !dogeChampionsNFT.isNFTDividendIgnored(holdings[i]))
            {
        ...
        balance += minterProfits[msg.sender];
        minterProfits[msg.sender] = 0;

        payable(msg.sender).transfer(balance * 10 ** 9); // rewards in wei
    }

    // @author Following region is for setter functions

    /*
     * @author sets sell availability state of given Semi Fungible Token contract address
     */
    function setSemiFungibleTokenContractState(address contractAddress, bool isAvailable) public onlyOwner
    {
        availableSemiFungibleTokenContractAddresses[contractAddress] = isAvailable;
    }

    /*
     * @author sets sell availability state of given NFT contract address
     */
    function setNFTContractState(address contractAddress, bool isAvailable) public onlyOwner
    {
        availableNFTContractAddresses[contractAddress] = isAvailable;
    }

    /*
     * @author sets sale and operations state of marketplace. This will be used if we need to pause marketplace
     */
    function setMarketplaceState(bool isSaleOpen, bool isOperationsOpen) public onlyOwner
    {
        isMarketSaleOpen = isSaleOpen;
        isMarketOperationsOpen = isOperationsOpen;
    }
    ...
    /*
     * @author returns market itemId of given token
     */
    function getMarketItemId(uint256 tokenId) public view returns (uint256)
    {
        return tokenIdToMarketItemId[tokenId];
    }

    /*
     * @author returns market item values of given token
     */
    function getMarketItemValues(uint256 tokenId) public view returns (uint256 marketId, uint256 price, address seller, address tokenOwner)
    {
        marketId = tokenIdToMarketItemId[tokenId];
        price = idToMarketItem[marketId].price;
        seller = idToMarketItem[marketId].seller;
        tokenOwner = idToMarketItem[marketId].owner;
        return (marketId, price, seller, tokenOwner);
    }

    /*
     * @author returns consumable market itemIds user has
     */
    function getOnSaleConsumableMarketItems() public view returns (uint256[] memory)
    {
        return userToOnSaleConsumableMarketItemIds[msg.sender];
    }

    /*
     * @author returns number of active NFT market items on sale
     */
    function getNumberOfItemsOnSale() public view returns (uint256)
    {
        return _numberOfItemsOnSale.current();
    }

    /*
     * @author returns number of active NFT market items on sale
     */
    function getNumberOfConsumablesOnSale() public view returns (uint256)
    {
        return _numberOfConsumablesOnSale.current();
    }

    /*
     * @author returns number of created NFT market items
     */
    function getNumberOfMarketItems() public view returns (uint256)
    {
        return _itemIds.current();
    }

    /*
     * @author returns number of created consumable market items
     */
    function getNumberOfConsumableMarketItems() public view returns (uint256)
    {
        return _consumableItemIds.current();
    }

    /*
     * @author returns total NFT transaction volume of the marketplace since deployment
     */
    function getTotalNFTVolume() public view returns (uint256)
    {
        return totalNFTVolume;
    }

    /*
     * @author returns total consumable transaction volume of the marketplace since deployment
     */
    function getTotalConsumableVolume() public view returns (uint256)
    {
        return totalConsumableVolume;
    }

    /*
     * @author returns total NFT transaction volume of the marketplace per day
     */
    function getDailyNFTVolume() public view returns (uint256)
    {
        uint256 daysPassed = (block.timestamp - contractDeploymentTimestamp) / 86400; // 86400 is 1 day in epoch

        // zero check to prevent undefined division error
        if(daysPassed == 0)
            daysPassed = 1;

        return totalNFTVolume / daysPassed;
    }

    /*
     * @author returns total consumable transaction volume of the marketplace per day
     */
    function getDailyConsumableVolume() public view returns (uint256)
    {
        uint256 daysPassed = (block.timestamp - contractDeploymentTimestamp) / 86400; // 86400 is 1 day in epoch
        ...
        return totalConsumableVolume / daysPassed;
    }

    /*
     * @author returns latest 10 sale history of given tokenId
     */
    function getSaleHistory(uint256 tokenId) public view returns(uint256[] memory, uint256[] memory)
    {
        uint256[] memory saleHistoryArray = saleHistory[tokenId];
        uint256[] memory saleHistoryDateArray = saleHistoryDate[tokenId];
        uint256 length = saleHistoryArray.length;

        if(length < 10)
        {
            uint256[] memory history = new uint256[](length);
            uint256[] memory historyDate = new uint256[](length);
            for(uint256 i = 0; i < length; i++)
            {
                history[i] = saleHistoryArray[i];
                historyDate[i] = saleHistoryDateArray[i];
            }
        ...
        else
        {
            ...
            uint256[] memory historyDate = new uint256[](10);
            uint256 historyIndex = length - 10;
            for(uint256 i = 0; i < 10; i++)
            {
                history[i] = saleHistoryArray[historyIndex];
                historyDate[i] = saleHistoryDateArray[historyIndex];
                historyIndex++;
            }
            return (history, historyDate);
        }
    ...
    function getRewardBalance(address walletAddress) public view returns(uint256)
    {
        ...
        balance += minterProfits[walletAddress];

        return balance * 10 ** 9; // rewards in wei
    }
}
...
contract DogeChampionsNFT is ERC721URIStorage, ERC721Enumerable
{
    ...
    /*
     * @auhor very big and ugly array of bytes that will be used for getting bytes4 values of randomly determined layer index.
     * This solution has been choosen to lower the gast price without having unnecessary string to bytes conversions during the mint.
     * This is solidity, fancy programming patterns doesn't apply here. Everything consumes gas :)
     */
    bytes4[] private hashmap = [bytes4('0'), bytes4('1'), bytes4('2'), bytes4('3'), bytes4('4'), bytes4('5'), bytes4('6'), bytes4('7'), bytes4('8'), bytes4('9'),
                               bytes4('10'), bytes4('11'), bytes4('12'), bytes4('13'), bytes4('14'), bytes4('15'), bytes4('16'), bytes4('17'), bytes4('18'), bytes4('19'),
                               bytes4('20'), bytes4('21'), bytes4('22'), bytes4('23'), bytes4('24'), bytes4('25'), bytes4('26'), bytes4('27'), bytes4('28'), bytes4('29'),
                               bytes4('30'), bytes4('31'), bytes4('32'), bytes4('33'), bytes4('34'), bytes4('35'), bytes4('36'), bytes4('37'), bytes4('38'), bytes4('39'),
                               bytes4('40'), bytes4('41'), bytes4('42'), bytes4('43'), bytes4('44'), bytes4('45'), bytes4('46'), bytes4('47'), bytes4('48'), bytes4('49'),
                               bytes4('50'), bytes4('51'), bytes4('52'), bytes4('53'), bytes4('54'), bytes4('55'), bytes4('56'), bytes4('57'), bytes4('58'), bytes4('59'),
                               bytes4('60'), bytes4('61'), bytes4('62'), bytes4('63'), bytes4('64'), bytes4('65'), bytes4('66'), bytes4('67'), bytes4('68'), bytes4('69'),
                               bytes4('70'), bytes4('71'), bytes4('72'), bytes4('73'), bytes4('74'), bytes4('75'), bytes4('76'), bytes4('77'), bytes4('78'), bytes4('79'),
                               bytes4('80'), bytes4('81'), bytes4('82'), bytes4('83'), bytes4('84'), bytes4('85'), bytes4('86'), bytes4('87'), bytes4('88'), bytes4('89'),
                               bytes4('90'), bytes4('91'), bytes4('92'), bytes4('93'), bytes4('94'), bytes4('95'), bytes4('96'), bytes4('97'), bytes4('98'), bytes4('99'),
                               bytes4('100'), bytes4('101'), bytes4('102'), bytes4('103'), bytes4('104'), bytes4('105'), bytes4('106'), bytes4('107'), bytes4('108'), bytes4('109'),
                               bytes4('110'), bytes4('111'), bytes4('112'), bytes4('113'), bytes4('114')];

    string private baseURIOverride = "ipfs://CustomCIDHashToBeSet/";
    ...
    function discountMint(uint256 amount) public payable returns (uint256)
    {
        ...
        if(mintPhase.current() < 2)
        {
            require(initialMintDiscountWhitelist[msg.sender] >= amount, "You don't have anough WL credit.");
            initialMintDiscountWhitelist[msg.sender] = initialMintDiscountWhitelist[msg.sender] - amount;
            require(userToNumberOfFirstPhaseMints[msg.sender] + amount <= 20, "You hit mint limit.");
        }
        else
        {
            require(finalMintDiscountWhitelist[msg.sender] >= amount, "You don't have anough WL credit.");
    ...
    {
        return calculateRandomness(114);
    }
...
contract DogeChampionsConsumable is ERC1155
{
     ...
     * @author plain good old constructor
     */
    // TODO change consturctor URI before mainnet deploy
    constructor() ERC1155("ipfs://CustomCIDToBeSet/{id}.json")
    {
        owner = payable(msg.sender);
