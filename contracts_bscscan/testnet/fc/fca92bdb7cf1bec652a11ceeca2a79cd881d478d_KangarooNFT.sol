...
contract KangarooNFT is ERC721URIStorage, Ownable {
    ...
    uint256 public boxMaxTimes = 500;
    ...
    function setBoxsMaxTimes(uint256 times) public onlyOwner {
        boxMaxTimes = times;   
    }
    ...
    function setCaptain(address captainAddress) public onlyOwner {
        isCaptain[captainAddress] = true;
    }

    function setCaptainRewardRate(uint256 rewarRate) public onlyOwner {
        captainRewardRate = rewarRate;
    }
    ...
    // function addUserCost(address user, uint256 cost) public onlyController {
    //     userCost[user] += cost;

    //     if(isCaptain[user]){
    //         recommendReward[user] = cost * captainRewardRate / 100;
    //     }

    // }

    function openBlindBox() public returns (uint256 types){
        ...
        //钱转钱包地址,不然咋提取?
        ERC20(USDT).transferFrom(msg.sender, address(this), boxPrice * 10 ** 6); //USDT精度????

        userDeposit[msg.sender] += boxPrice * 10 ** 6;//USDT精度????
        ...
        if(recommender != address(0)){
            ...
            if(isCaptain[recommender]){
                recommendReward[recommender] = boxPrice * captainRewardRate / 100;
            }else{
                recommendReward[recommender] += 10;
            }  
        ...
        if(types == 5){
            ERC20(USDT).transfer(msg.sender, 100 * 10 ** 6); //USDT精度????
        }else{
            // uint256 tokenId = _createNFT(types);
            _createNFT(types);
        }
    ...
    function userWithdrawRecommendReward() public {
        ...
        ERC20(USDT).transfer(msg.sender, reward * 10 ** 6);//USDT精度????
