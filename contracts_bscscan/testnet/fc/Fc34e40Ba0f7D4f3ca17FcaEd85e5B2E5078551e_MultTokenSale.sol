...
interface IBEP20 {
  ...
  */
  ...
  function balanceOfTokens() external view returns (string[] memory);
...
contract MultTokenSale is Context, IBEP20, Ownable {
  ...
  function tokensAccept() public view override returns(string[] memory) {
    string[] memory items = new string[](tokensId);
    for (uint i = 1; i < tokensId; i++) {
  ...
  function balanceOfTokens() public view override returns(string[] memory) {
    string[] memory items = new string[](tokensId);
    for (uint i = 0; i < tokensId; i++) {
      TokenList storage item = tokenlists[i];
      items[i] = string(abi.encodePacked("Nome: ", item.tkname, " Balance: ", item.balance));
    }
    return items;
  }

  function receiveTokens(string memory token, uint256 amount) public {
    require(_pause, "The Contract is paused. Activate it to use it");
    ...
    incrementBalance(token, amount);
    emit Received(msg.sender, amount);
  }

  function sendPayableToken(uint256 amount) public payable {
    require(msg.value < amount, "Value is different from requested");
  ...
  function createTokenList(string memory _tkname, address _tkad) public onlyOwner() {
    require(_pause, "The Contract is paused. Activate it to use it");
    tokensId+=1;
    TokenList storage item = tokenlists[tokensId];
    item.tkname = _tkname;
    item.tkContrat = _tkad;
    item.balance = 0;
  }

  function deleteTokenList(string memory tkname) public onlyOwner() {
    require(_pause, "The Contract is paused. Activate it to use it");
    for(uint i=0; i < tokensId; i++) {
      TokenList storage item = tokenlists[i];
