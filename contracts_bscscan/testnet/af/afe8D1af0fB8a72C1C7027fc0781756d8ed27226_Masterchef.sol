...
contract Masterchef is Ownable {
    ...
    function add(uint256 _allocPoint, IBEP20 _lpToken, uint16 _depositFeeBP, bool _withUpdate,RewardToken _rwdToken,uint256 _lockedPeriod,bool _isFixedAPR,bool onlyowner) public onlyOwner {
       ...
       //_lockedPeriod = _lockedPeriod.mul(1 days);
        ...
        poolInfo.push(PoolInfo({
            ...
            isFixedAPR:_isFixedAPR,
            onlyOwneraccess:onlyowner
        }));
    }
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        // condition to check fixed or variable Approval
        if(!pool.isFixedAPR){
        ...
        tostReward = (user.amount.mul(RewardPerBlock).div(10000)).mul(multiplier).mul(pool.allocPoint.div(100)); // TostPerBlock = 20 i.e 0.2 %
        }
        return (tostReward);
        }
    }
    ...
    function updatePool(uint256 _pid) public {
       ...
       uint256 depositedAmount = pool.depositedAmount;
        ...
        uint256 currentTime = !user.isClaimed ? (block.timestamp > user.userLockedPeriod && ( user.isClaimed || user.lastRewardBlock < user.userLockedPeriod)  ? user.userLockedPeriod : block.timestamp) : user.lastRewardBlock;
        uint256 multiplier =  (getMultiplier(user.lastRewardBlock, currentTime)).div(intervalBlock); // mul(3)  difference in seconds and converted into day
        uint256 tostReward = (user.amount.mul(RewardPerBlock).div(10000)).mul(multiplier).mul(pool.allocPoint.div(100)); // TostPerBlock = 20 i.e 0.2 %
        //rwd.transfer(devaddr,tostReward.div(10));
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        pending = (tostReward);
        if(pending > 0) {
            safeTostTransfer(msg.sender, pending,_pid);
            user.lastRewardBlock= block.timestamp;
        }
        if(_amount > 0) {
            user.amount = user.amount.sub(_amount);
    ...
    function claim(uint256 pid) public {
        ...
        if(claimAmount > 0) {
            safeTostTransfer(msg.sender, claimAmount,pid);
            user.lastRewardBlock= block.timestamp;
            emit Claim(msg.sender, pid, claimAmount);
        }
        user.pendingRewards = 0;
        user.isClaimed = true;
    ...
    function safeTostTransfer(address _to, uint256 _amount,uint _pid) internal {
         PoolInfo memory pool = poolInfo[_pid];
        uint256 tostBal = rwd.balanceOf(address(this));
        if (_amount > tostBal) {
            pool.rwdToken.transfer(_to, tostBal);
            //rwd.transfer(_to, tostBal);
        } else {
