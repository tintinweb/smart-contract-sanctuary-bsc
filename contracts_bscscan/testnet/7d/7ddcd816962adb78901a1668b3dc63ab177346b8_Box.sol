    ...
    AccessControlUpgradeable
{
    ...
    event MaxBuyEpicLimit(address _operator, uint256 _buyEpicLimit);
     ...
     */
    function buyEpicBox(uint256 _amount) public whenNotPaused {
        address sender = msg.sender;
        require(_amount <= buyEpicLimit, "AMOUNT_OVER_LIMIT");
        ...
        emit BuyBox(sender, boxIds, boxEpicPrice, boughtAt, EPIC, boxHashes);
    }
    ...
    function buyLegendBox() public whenNotPaused {
        ...
        uint256[] memory boxIds = new uint256[](1);
        boxIds[0] = boxId;

        bytes32[] memory boxHashes = new bytes32[](1);
        boxHashes[0] = BoxDetails.hash(boxDetail, sender);
    ...
    function openBox(
        ...
        for (uint256 i = 0; i < _amount; i++) {
            ...
            require(_isBoxOwner(sender, _boxId), "NOT_OWNER_BOX");
    ...
    function setMaxBuyEpicLimit(uint256 _buyEpicLimit)
        public
        onlyRole(OPERATOR_ROLE)
    {
        buyEpicLimit = _buyEpicLimit;
        emit MaxBuyEpicLimit(msg.sender, _buyEpicLimit);
    }
    ...
    function _isBoxOwner(address _user, uint256 _boxId) public view returns (bool) {
        require(_user != address(0), "ZERO_ADDRESS");
        uint256[] memory boxIds = boxByAddress[_user];
        require(boxIds.length > 0, "BOX_NOT_FOUND");
        
        for (uint256 i = 0; i < boxIds.length; i++)
            if (boxIds[i] == _boxId) return true;
    ...
    function getHashById(uint256 _boxId, address _sender) public view returns (bytes32) {
        return BoxDetails.hash(boxDetails[_boxId], _sender);
    }
