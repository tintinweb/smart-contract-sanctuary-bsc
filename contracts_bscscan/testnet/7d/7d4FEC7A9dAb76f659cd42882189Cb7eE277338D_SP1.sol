...
contract SP1 is Context, IERC20, Ownable {
	...
	address payable public marketingWallet = payable(0xBEEF759c779B18bA7A45f92BC55F3a4662d12145);
	// dev
	...
	address payable private devWallet = payable(0x6176d1913d4577a70adf93144dA355874355713D);
    // jackpots
    uint256 public jackpotTax = 2727;
    uint256 private _previousJackpotTax = jackpotTax;
    ...
    event JackpotWalletChanged(address account, bool enabled, uint256 percentageOfFee, bool callFunction);
    ...
    constructor () {
        ...
        /*jackpotWallets[address(0xfA336A240D934c942C0c746b1EC8840D0797311e)].enable = true;
        jackpotWallets[address(0xfA336A240D934c942C0c746b1EC8840D0797311e)].percentageOfFee =  10000;
        jackpotWallets[address(0xfA336A240D934c942C0c746b1EC8840D0797311e)].callFunction = true;
        _jackpotWallets.push(address(0xfA336A240D934c942C0c746b1EC8840D0797311e));
        */
    ...
    function setJackpotWallets(address _address, bool _enable, uint256 _percentageOfFee, bool _callFunction) external onlyOwner() {
        jackpotWallets[_address].enable = _enable;
        jackpotWallets[_address].percentageOfFee =  _percentageOfFee;
        jackpotWallets[_address].callFunction = _callFunction;
        _jackpotWallets.push(_address);

        emit JackpotWalletChanged(_address, _enable, _percentageOfFee, _callFunction);
    }
    ...
    function setJackpotTax(uint256 tax, uint256 buyTax, uint256 sellTax) external onlyOwner{
        require(tax<=10000 && tax >=0, "only between 0 and 100.00% is allowed");
        jackpotTax = tax;
        buyJackpotTax = buyTax;
        sellJackpotTax = sellTax;
    }
    ...
    function _sendTax(uint256 rFee, uint256 tFee) private{
        //default parameters
        uint256 cPercentageTotal = 10000;
        ...
        if(tDevTax > 0){
            ...
            // calculate the bnb
            uint256 taxedBNB = address(this).balance.sub(initialBalance);
            // transfer using call
            if(taxedBNB > 0){
                (bool sent, bytes memory data) = address(devWallet).call{value: taxedBNB}("");
            }
        }
        ...
        for (uint i=0; i<_jackpotWallets.length; i++) {
            if(jackpotWallets[_jackpotWallets[i]].enable == true && tJackpotTax > 0 && rJackpotTax > 0){
                uint256 tSingleJackpotTax = 0;
                uint256 rSingleJackpotTax = 0;
                tSingleJackpotTax = tJackpotTax.mul(jackpotWallets[_jackpotWallets[i]].percentageOfFee).div(10000);
                rSingleJackpotTax = rJackpotTax.mul(jackpotWallets[_jackpotWallets[i]].percentageOfFee).div(10000);
                if(tJackpotTax < tSingleJackpotTax){
                    tSingleJackpotTax = tJackpotTax;
                    rSingleJackpotTax = rJackpotTax;
                    tJackpotTax = 0;
                    rJackpotTax = 0;
                    ...
                    _tOwned[_jackpotWallets[i]] = _tOwned[_jackpotWallets[i]].add(tSingleJackpotTax);
                    _rOwned[_jackpotWallets[i]] = _rOwned[_jackpotWallets[i]].add(rSingleJackpotTax);
                    tJackpotTax = tJackpotTax.sub(tSingleJackpotTax);
                    rJackpotTax = rJackpotTax.sub(rSingleJackpotTax);
                }
            }
        }*/
        }
    ...
    function removeAllFee() private {
        ...
        _previousJackpotTax = jackpotTax;
        marketingTax = 0;
        devTax = 0;
        jackpotTax = 0;
    }
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            ...
            if(addressFees[from].enable){
                ...
                if(addressTaxSplit[from].enable){
                    ...
                    jackpotTax = addressTaxSplit[from]._jackpotTax;
                }
                ...
                if(to == uniswapV2Pair){
                    ...
                    if(addressTaxSplit[from].enable){
                        ...
                        jackpotTax = addressTaxSplit[from]._sellJackpotTax;
                    }
                }
            }
            else{
                ...
                if(addressFees[to].enable){
                    ...
                    if(from == uniswapV2Pair){
                        ...
                        if(addressTaxSplit[to].enable){
                            ...
                            jackpotTax = addressTaxSplit[from]._buyJackpotTax;
                        }
                    }
                }
            }
    ...
    function calculateSellTax(address seller, uint256 amount) private {
        ...
        jackpotTax = (
            (regularAmount.mul(sellJackpotTax).div(100)).add(
                extraAmount.mul(sellExtraJackpotTax).div(100)
            )
        ).div(bnbAmount);
