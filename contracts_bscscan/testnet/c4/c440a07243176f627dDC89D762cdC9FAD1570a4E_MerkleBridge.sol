...
contract MerkleBridge is Ownable, MerkleProof {
    ...
    function getInfo(address user, uint256 epoch)
        public
        view
    ...
    function userInfo(address user, uint256 last) public view returns (Info[] memory) {
        uint256 length = getEpoch();
        last = last == 0 ? length : last;

        Info[] memory infos = new Info[](last);
        for (uint256 i = 0; i < last; i++) infos[i] = getInfo(user, length - (last - i));
    ...
    /// @dev adds roots from another chains
    /// @param roots_ array of roots from another chain
    function addRoots(Root[] memory roots_) external onlyOracle {
    ...
    /// @dev adds root from another chain
    /// @param root adding root
    function addRoot(Root memory root) external onlyOracle {
