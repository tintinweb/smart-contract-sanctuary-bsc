...
import "./SEAXToken.sol";
import "./SEAXSplitBar.sol";
...
contract MasterSeaDEX is Ownable {
    ...
    // The BANANA TOKEN!
    SEAXToken public cake;
    // The BANANA SPLIT TOKEN!
    SEAXSplitBar public syrup;
    // Dev address.
...
// IDDXToken with Governance.
contract SEAXToken is BEP20('SeaDEX Token', 'SEAX') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterSeaDEX).
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "IDDX::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "IDDX::delegateBySig: invalid nonce");
        require(now <= expiry, "IDDX::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "IDDX::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying IDDXs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "IDDX::_writeCheckpoint: block number exceeds 32 bits");
...
// SEAXSplitBar with Governance.
contract SEAXSplitBar is BEP20('SEAXSplitBar Token', 'SEAXSPLIT') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterSeaDEX).
    function mint(address _to, uint256 _amount) external onlyOwner {
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
    ...
    // Safe cake transfer function, just in case if rounding error causes pool to not have enough IDDXs.
    ...
    */
        ...
        external
    {
        ...
        address signatory = ecrecover(digest, v, r, s);
        require(signatory != address(0), "SEAX::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "SEAX::delegateBySig: invalid nonce");
        require(now <= expiry, "SEAX::delegateBySig: signature expired");
        return _delegate(signatory, delegatee);
    }
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "SEAX::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "SEAX::_writeCheckpoint: block number exceeds 32 bits");
