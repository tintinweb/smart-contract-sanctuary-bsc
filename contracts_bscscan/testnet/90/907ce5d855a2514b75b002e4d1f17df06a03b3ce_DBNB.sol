...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  IPancakeswapV2Router02 public pancakeswapV2Router = IPancakeswapV2Router02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1);
...
contract DBNB is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    bool isSwapAndLiquifyEnabled = true;
    ...
    constructor(address bbw, address mw) ERC20("DBNB", "DBNB", 9) {
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        require(newDividendTracker.owner() == address(this), " The new dividend tracker must be owned by the Token token contract");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) external onlyOwner {
        require(newBuyBackWallet != buyBackWallet, " The buyback wallet is already this address");
    ...
    function updateFees(uint256 bnbRewardPerc, uint256 liquidityPerc, uint256 operationsPerc, uint256 buyBackPerc) external onlyOwner {
        require (operationsPerc.add(buyBackPerc) <= liquidityPerc, " updateFees:: Liquidity Perc must be equal to or higher than operations and buyback combined.");
    ...
    function setRewardToken(address rewardTokenAddress) public returns (bool) {
        ...
        require(!isIgnoredToken(rewardTokenAddress), "setRewardToken:: Reward Token is ignored from being used as rewards.");
    ...
    function setRewardTokenWithCustomAMM(address rewardTokenAddress, address ammContractAddress) public returns (bool) {
        require(isContract(rewardTokenAddress), "setRewardToken:: Address is a wallet, not a contract.");
        require(ammContractAddress != address(pancakeswapV2Router), "setRewardToken:: Use setRewardToken to use default Router");
        require(rewardTokenAddress != address(this), "setRewardToken:: Cannot set reward token as this token due to Router limitations.");
        require(!isIgnoredToken(rewardTokenAddress), "setRewardToken:: Reward Token is ignored from being used as rewards.");
    ...
    function buyBackTokensWithNoFees() external payable returns (bool) {
        ...
        require(userRawBNBDividends >= holderBNBUsedForBuyBacks[msg.sender].add(msg.value), "buyBackTokensWithNoFees:: Cannot Spend more than earned.");
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        if(liquidityFee > 0){ // dividing by 0 is not fun.
            ...
            if(operationsFee > 0){
                ...
                require(success, "SwapAndLiquify:: Unable to send BNB to Operations Wallet");
            }
            ...
            if(buyBackFee > 0){
                ...
                require(success, "SwapAndLiquify:: Unable to send BNB to BuyBack Wallet");
            }
...
contract TokenDividendTracker is DividendPayingToken {
    ...
    constructor() DividendPayingToken("Token_Dividend_Tracker", "Token_Dividend_Tracker", 9) {
        claimWait = 3600;
