...
/**
 * @dev An ERC20 token for GweiDao.
 ...
 */
contract GweiDaoToken is ERC20, ERC20Permit, ERC20Votes, Ownable {
    ...
    uint16 private transferTaxRate = 100;    // 1%
    uint16 private burnRate = 0;
    ...
    function startVest(address tokenLockAddress) public onlyOwner {
        require(!vestStarted, "GweiDao: Vest has already started.");
        vestStarted = true;
        _approve(address(this), tokenLockAddress, devSupply);
        GweiDaoTokenLock(tokenLockAddress).lock(0x73ea708dC6e7A629AE3c89322320F1107537e200, 150_000_000_000e18);
    }
    ...
    function claimTokens(uint256 amount, bytes32[] calldata merkleProof) public {
        ...
        require(valid, "GweiDao: Valid proof required.");
        require(!claimed[msg.sender], "GweiDao: Tokens already claimed.");
    ...
    function sweep(address dest) public onlyOwner {
        require(block.timestamp > claimPeriodEnds, "GweiDao: Claim period not yet ended");
    ...
    /**
     * @dev Withdraw tokens from taxBalance to any. Only owner can call.
     * @param account The address to receive.
     * @param amount The amount to withdraw.
     */
    function withdrawTax(address account, uint256 amount) public onlyOwner {
        require(amount <= taxBalance, "GweiDao: Invalid tax balance");
        super._transfer(address(this), account, amount);
        taxBalance -= amount;
    }

    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOwner {
        require(_transferTaxRate <= 1000, "GweiDao: Transfer tax rate must not exceed the maximum rate.");
        // emit TransferTaxRateUpdated(msg.sender, transferTaxRate, _transferTaxRate);
        transferTaxRate = _transferTaxRate;
    }
    
    function getTransferTaxRate() external view onlyOwner returns (uint16) {
        return transferTaxRate;
    }

    function updateBurnRate(uint16 _burnRate) public onlyOwner {
        require(_burnRate <= 100, "GweiDao: Burn rate must not exceed the maximum rate.");
        // emit BurnRateUpdated(msg.sender, burnRate, _burnRate);
        burnRate = _burnRate;
    }

    function getBurnRate() external view onlyOwner returns (uint16) {
        return burnRate;
    }
    ...
    function _transfer(
        ...
        if (transferTaxRate == 0) {
            ...
            if (sender == owner() || address(sender) == address(this)) {
                taxAmount = 0;
                burnAmount = 0;
                feeAmount = 0;
            }
