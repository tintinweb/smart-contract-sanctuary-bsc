...
// fixed supply
// ----------------------------------------------------------------------------
contract KINGLION is ERC20Interface, Owned {
    ...
    address public LION = address(0x1e33833a035069f42d68d1f53b341643de1c018d);
    ...
    function initialize(address _router, address _LION, address _devPool, address _gaur_dao, address _treasury, address _supe) external onlyOwner{
        ...
        treasury = _treasury;
        supe_token = _supe;
        emit Initialize(_router, _LION, swapFactory);
    }
    ...
    function transfer(address to, uint tokens) public returns (bool success) {
        ...
        address pair = swapFactory.getPair(address(this), LION);
        ...
        if(force1stStep || _in1stWhiteList(msg.sender, to) || pair == address(0) || IERC20(address(this)).balanceOf(pair) == 0 || IERC20(LION).balanceOf(pair) == 0){
            balances[msg.sender] = balances[msg.sender].sub(tokens);
    ...
    function transferFrom(address from, address to, uint tokens) public returns (bool success) {
        ...
        if(force1stStep || _in1stWhiteList(from, to) || pair == address(0) || IERC20(address(this)).balanceOf(pair) == 0 || IERC20(LION).balanceOf(pair) == 0){
            balances[from] = balances[from].sub(tokens);
            allowed[from][msg.sender] = allowed[from][msg.sender].sub(tokens);
            toBlackHole = tokens.div(1000);
            balances[blackHole] = balances[blackHole].add(toBlackHole);
    ...
    function _swapAndLiquify(uint256 tokens) private lockTheSwap {
        ...
        uint256 initialBalance = IERC20(address(LION)).balanceOf(address(this));//address(this).balance;

        // swap tokens for ETH
        _swapTokensForLION(half); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered

        // how much ETH did we just swap into?
        uint256 newBalance = IERC20(address(LION)).balanceOf(address(this)).sub(initialBalance);
        ...
        _safeTransferLION(treasury, toTreasury);
                
        emit SwapAndLiquify(half, toLiquidityBalance, otherHalf);
    }

    function _addLiquidity(uint256 tokenAmount, uint256 lionAmount) internal {
        // approve token transfer to cover all possible scenarios
        IERC20(address(this)).approve(address(router), 0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff);
        IERC20(LION).approve(address(router), 0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff);
        // add the liquidity
        router.addLiquidity(
            address(this),
            LION,
    ...
    function _swapTokensForLION(uint256 tokenAmount) internal {
        IERC20(address(this)).approve(address(router), 0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff);
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
    ...
    function _safeTransferLION(address to, uint value) returns (bool success) {
        return ERC20Interface(LION).transfer(to, value);
    }
