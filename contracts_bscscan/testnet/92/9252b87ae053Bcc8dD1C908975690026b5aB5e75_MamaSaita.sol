...
contract MamaSaita is Context, IERC20, ERC20Ownable {
    using SafeMath for uint256;

    string private constant _nomenclature = "MamaSaita";
    string private constant _symbol = "MAMS";
    ...
    constructor() {
        ...
        devAddress = payable(0x1C1C5095B781e4036585F8DEdb506Af737b24717);
        marketingAddress = payable(0x1C1C5095B781e4036585F8DEdb506Af737b24717);
    ...
    function removeAllFee() private {
        if (_refTax == 0 && _liqTax == 0 && _marTax == 0 && _devTax == 0)
            return;
    ...
    function _transfer(
        ...
        require(
            ...
            "Snipers can't transfer tokens to sell cheaper DM a Mod."
        );
        if (_maxWalletOn == true && !_isMaxWalletExclude[to]) {
            require(
                balanceOf(to) + amount <= _maxWalletSize,
                "Max amount of tokens for wallet reached"
            );
        }
        if (_buyLimits == true && from == uniV2Pair) {
            require(
                amount <= 400000000000002 * 10**9,
                "Limits are in place, please lower buying amount"
            );
        }
        if (_initateLiqTrans == true) {
            ...
            uniV2Pair = IUniswapV2Factory(_uniV2Router.factory()).getPair(
                address(this),
                _uniV2Router.WETH()
            );
    ...
    function swapTokens() private lockTheSwap {
        ...
        uint256 totalTokensToSwap = tokensForLiquidityToSwap +
            tokensForMarketingToSwap +
            tokensForDevToSwap;
        ...
        uint256 ethForMarketing = ethBalance.mul(tokensForMarketingToSwap).div(
            totalTokensToSwap
        );
        uint256 ethForDev = ethBalance.mul(tokensForDevToSwap).div(
            totalTokensToSwap
        );
        uint256 ethForLiquidity = ethBalance.sub(ethForMarketing).sub(
            ethForDev
        );
