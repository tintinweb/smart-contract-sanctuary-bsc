...
contract Votable is Ownable {
    ...
    function addVote() external virtual onlyTeam whenNotVoted returns (bool) {
        totalVotes.add(1);
        return true;
    }
    ...
    modifier onlyTeam() {
        require(
            _ReedAdministrativeTeam[_msgSender()] == _TRUE,
            "Votable: Only team mebers can vote"
        );
        _;
    }

    modifier whenNotVoted() {
        _;
    }

    modifier withTeamVote() {
        require(totalVotes != 0, "Votable: No voting yet");
        require(
            countVotes() >= countTeam(),
            "Votable: All Team members must Vote"
        );
        _;
        clearVote();
    }

    function addTeam(address teamMEmber) external virtual onlyOwner {
        require(
            _ReedAdministrativeTeam[teamMEmber] == _TRUE,
            "Votable: Team member already added"
        );
        _ReedAdministrativeTeam[teamMEmber] = _TRUE;
        teamCount += 1;
    }

    function removeTeam(address teamMEmber) external virtual onlyOwner {
        require(teamCount != 0, "Votable: Team count is Zero");
        require(
            _ReedAdministrativeTeam[teamMEmber] == _FALSE,
            "Votable: Team member already removed"
        );
        _ReedAdministrativeTeam[teamMEmber] = _FALSE;
        teamCount -= 1;
    }

    function countTeam() public view returns (uint256) {
        return teamCount;
    }
...
contract ReedBricks is Context, Ownable, ERC20Burnable, ERC20Pausable, Votable {
    ...
    constructor(
        ...
        // Add the Owner to the voting team //
        _ReedAdministrativeTeam[owner()] = _TRUE;
