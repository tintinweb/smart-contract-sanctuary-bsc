...
contract ValentinesDoge is Context, IERC20, Ownable {
    ...
    address dev = 0x4Ca85B8224a689dAFf55101bC129C3787649E5B3;
   
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 200000 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 public maxWalletBalance = 10000 * 10**18;
    bool public tradingEnabled;
    uint256 private _tFeeTotal;

    string private _name = "ValentinesDOGE";
    string private _symbol = "ValentinesDOGE";
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            ...
            devfee = amount.div(50);
            amount = amount.sub(devfee);
            _transferStandard(sender, dev, devfee);
            _transferToExcluded(sender, recipient, amount);
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            require(tradingEnabled);
            devfee = amount.div(50);
            amount = amount.sub(devfee);
            require(balanceOf(recipient).add(amount) <= maxWalletBalance, "Balance is exceeding maxWalletBalance");
            _transferStandard(sender, dev, devfee);
