...
contract preSaledeployerOPT {
    ...
    event SetData(address indexed admin, address indexed router);

    constructor() {
        admin = payable(0xC2a5ea1d4406EC5fdd5eDFE0E13F59124C7e9803); //0x9fCbfe41188E5852b147549F3a2eAf5881aE748f
        projectCoinFee = 3;
        projectTokenFee = 3;
        userFee = 10;
...
library Address {
    function sendValue(address payable recipient, uint256 amount) internal {
        require(address(this).balance >= amount, "Addr: bal<amount");

        // solhint-disable-next-line avoid-low-level-calls, avoid-call-value
        (bool success, ) = recipient.call{value: amount}("");
        require(success, "Addr: error");
    }
}
...
*/
    ...
    function claim() public allowed nonReentrant {
        require(block.timestamp > preSaleEndTime, "PS: Presale time not over");
    ...
    function endPreSale() public onlyTokenOwner allowed {
        ...
        if (!profitClaim) {
            ...
            canClaim = true;
            if (adminFeeCounter > 0 && f == true)
                token.transfer(admin, adminFeeCounter);
            adminFeeCounter = 0;
        }
    }

    function vote(bool _vote) public {
        require(token.balanceOf(msg.sender) > 0, "VT: Voter must be a holder");
        require(
