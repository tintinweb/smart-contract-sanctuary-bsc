...
contract AngelAndDemonGame is Ownable {
    ...
    function addMonster(
        ...
        uint _monsterId = angelAndDemonToken.safeMint(string(abi.encodePacked(_baseURL, cardURL)));
        require(angelAndDemonToken.ownerOf(_monsterId) == msg.sender);
        monsters[msg.sender][_monsterId] = Monster(attackPoint, defensePoint, manaCost, healthPoint);
    ...
    function addSpell(
        ...
        uint _spellId = angelAndDemonToken.safeMint(string(abi.encodePacked(_baseURL, cardURL)));
    ...
    function addEquip(
        ...
        uint _equipId = angelAndDemonToken.safeMint(string(abi.encodePacked(_baseURL, cardURL)));
    ...
    function addCardsToPlayer(
        ...
        string[] memory _equipURLs
    ) public {
        for (uint i = 0; i < _monsterCount; i ++) {
            addMonster(_monsters[i].attackPoint, _monsters[i].defensePoint, _monsters[i].manaCost, _monsters[i].healthPoint, _monsterURLs[i]);
        }
