...
contract INFP_StakePool_1 is Ownable{
    ...
    uint256 public fullMaturityTime = 30 minutes; 
    uint256 public coolingPeriod = 5 minutes;
        ...
        if(block.timestamp < users[msg.sender].pool_deposit_time + fullMaturityTime){
            if(users[msg.sender].coolingTime == 0){
                ...
                if(block.timestamp > users[msg.sender].coolingTime + coolingPeriod){
                    if (block.timestamp >= users[msg.sender].pool_deposit_time + 21 minutes) {
                        penality = amount*penalityFees[1]/100e18;
                        _update(msg.sender,amount-penality,calculatedRewards,penality);
                    }
                    else if (block.timestamp >= users[msg.sender].pool_deposit_time + 14 minutes)  {
                        penality = amount*penalityFees[2]/100e18;
                        _update(msg.sender,amount-penality,calculatedRewards,penality);
                    }
                    else if (block.timestamp >= users[msg.sender].pool_deposit_time + 7 minutes) {
