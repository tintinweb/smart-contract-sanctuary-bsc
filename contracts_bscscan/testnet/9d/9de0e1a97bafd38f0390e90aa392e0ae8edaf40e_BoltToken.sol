    ...
    ERC20CappedUpgradeable
{
    ...
    function initialize() public initializer {
        __ERC20_init("Bolt Token", "BOLT");
        __ERC20Burnable_init();
        __Pausable_init();
        __AccessControl_init();
        __ERC20Permit_init("Bolt Token");
...
abstract contract LockableUpgradeable is Initializable {
    event AccountLocked(address account, uint256 releaseTime);

    mapping(address => uint256) private _lockedAccounts;

    function __LockableUpgradeable_init_unchained() internal onlyInitializing {}

    modifier whenNotLocked(address account) {
        require(!locked(account), "Lockable: locked");
        _;
    }

    function _lock(address account, uint256 releaseTime) internal virtual {
        _lockedAccounts[account] = releaseTime;
        emit AccountLocked(account, releaseTime);
    }

    function locked(address account) public view virtual returns (bool) {
        return _lockedAccounts[account] > block.timestamp;
    }
