...
contract BNBInvestFeb09{
    ...
    //address payable public autopoolusernewuser;
			...
			for (uint256 i = 0; i < 1; i++) {
				if (upline != address(0)) {
                    ...
                    if(getUserTotalReferrals(referrer)==1){
                        if(getAutoPoolUser(referrer)==1){
                            if(getAutoPoolUser(getUserReferrer(referrer))==1){
                                autopooltransfer(getUserReferrer(referrer),ceo);
                            }else{
	...
	function getUserInfo(address userAddress) public view returns(uint256 checkpoint, uint256 totalDeposit, uint256 totalWithdrawn, uint256 totalReferrals, uint256 rankid, uint256 userautopool) {
		return(getUserCheckpoint(userAddress), getUserTotalDeposits(userAddress), getUserTotalWithdrawn(userAddress), getUserTotalReferrals(userAddress), getUserRankID(userAddress), getAutoPoolUser(userAddress));
	}
