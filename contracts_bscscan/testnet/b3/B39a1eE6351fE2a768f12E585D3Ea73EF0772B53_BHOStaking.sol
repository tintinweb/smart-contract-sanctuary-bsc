...
contract BHOStaking is OwnableUpgradeable {
    ...
    function initialize(IERC20 _bho) public initializer {
        ...
        stakingPackages.push(StakingPackage("Basic", 7 days, 10, ""));
        stakingPackages.push(StakingPackage("Vip", 21 days, 30, "black"));

        levels.push(Level("change", 10000, 100));
        levels.push(Level("obsidian", 20000, 300));
        ...
        onlyOwner
    {
        ...
        for (uint256 i = 0; i < _stakingPackages.length; i++) {
            require(_stakingPackages[i].apy > 0, "APY must be greate than 0");
            require(
    ...
    function updateLevels(Level[] memory _levels) external onlyOwner {
        ...
        for (uint256 i = 0; i < _levels.length; i++) {
            ...
            require(
                ...
                "Slot of next level must be greater than previous level"
            );
        ...
        )
    {
        ...
        for (uint256 i = 0; i < levels.length; i++) {
            Level memory level = levels[i];
            if (userInfo.amount >= level.minAmount) {
                index = i;
            }
        }
        Level memory level = levels[index];
        return (level.id, level.minAmount, level.slot);
    }
