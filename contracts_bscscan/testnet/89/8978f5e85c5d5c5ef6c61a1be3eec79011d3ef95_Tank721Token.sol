...
contract Tank721Token is ERC721Tradable{
    ...
    function setLevel(uint256 tokenId, uint8 level) public onlyMint returns (bool) {
    ...
    function mintWithTokenURI(address to, uint256 tokenId, uint8 level, string memory tokenURI) public onlyMint returns (bool) {
        _safeMint(to, tokenId);
        levels[tokenId] = level;
        _setTokenURI(tokenId, tokenURI);
        return true;
