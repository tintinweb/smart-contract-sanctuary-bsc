...
contract BSC_Defi {
    ...
    uint256 public EGGS_TO_HIRE_1MINERS_COMPOUND = 1200000; /** 7% **/
    uint256 public REFERRAL = 40;
    uint256 public PERCENTS_DIVIDER = 1000;
    uint256 public MAINTENANCE = 10;
    uint256 public COMMUNITY = 40;
    ...
    uint256 public MIN_INVEST = 10 * 1e15; /** 0.01 BNB  **/
	uint256 public WITHDRAW_COOLDOWN = 6 * 60 * 60; /** 6 hours  **/

    /* addresses */
    address payable public owner;
    address payable public maintenance;
    ...
    constructor(address payable _owner, address payable _maintenance, address payable _community) {
		require(!isContract(_owner) && !isContract(_maintenance) && !isContract(_community));
        owner = _owner;
    ...
    function payFees(uint256 eggValue) internal returns(uint256){
        (uint256 maintenanceFee, uint256 communityFee) = getFees(eggValue);
        maintenance.transfer(maintenanceFee);
        community.transfer(communityFee);
        return maintenanceFee.add(communityFee);
    }

    function getFees(uint256 eggValue) public view returns(uint256 _maintenance, uint256 _community) {
        _maintenance = eggValue.mul(MAINTENANCE).div(PERCENTS_DIVIDER);
        _community = eggValue.mul(COMMUNITY).div(PERCENTS_DIVIDER);
    }
    ...
    function CHANGE_MAINTENANCE(address value) external {
        require(msg.sender == owner, "Admin use only.");
        maintenance = payable(value);
    }

    function CHANGE_COMMUITY(address value) external {
        require(msg.sender == owner, "Admin use only.");
    ...
    **/
    ...
    function PRC_MAINTENANCE(uint256 value) external {
        require(msg.sender == owner, "Admin use only.");
    ...
    function PRC_COMMUNITY(uint256 value) external {
        require(msg.sender == owner, "Admin use only.");
        require(value >= 10 && value <= 100); /** 5% max **/
        COMMUNITY = value;
    }
