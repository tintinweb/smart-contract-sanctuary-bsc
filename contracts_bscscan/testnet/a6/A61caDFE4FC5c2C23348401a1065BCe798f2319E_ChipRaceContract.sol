...
contract ChipRaceContract is Ownable {
    ...
    function depositeFunds(uint256 _amount) external onlyOwner {
        ...
        g_craceToken.transferFrom(msg.sender, address(this), _amount * (10**18));
        for(uint i = 1; i < 6; i++)
            g_poolRewardsAmount[i] = _amount * (10**18) / 5;
    }
    function depositePoolFunds(uint256 _poolId, uint256 _amount) external onlyOwner {
        require(_amount > 0, "No funds");
        g_craceToken.transferFrom(msg.sender, address(this), _amount * (10**18));
        g_poolRewardsAmount[_poolId] = g_poolRewardsAmount[_poolId] + (_amount * (10**18));
    }
    
    function withdrawPoolFunds(uint256 _poolId, uint256 _amount) external onlyOwner {
        require(_amount > 0, "No funds");
        require((_amount * 10 ** 18) < g_poolRewardsAmount[_poolId], "No enough funds");
        g_craceToken.transfer(msg.sender, _amount * (10**18));
        g_poolRewardsAmount[_poolId] = g_poolRewardsAmount[_poolId] - (_amount * (10**18));
    }
}
