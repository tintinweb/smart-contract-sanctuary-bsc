...
contract BEP20_TestToken2 is Context, IBEP20, Ownable {
    using SafeMath for uint256;

    IPancakeRouter02 private pancakeV2Router;
    address public pancakeswapPair;

    string private constant _name = "BSCTestToken-3";
    string private constant _symbol = "BSCTESTTOKEN-3";
    uint8 private constant _decimals = 9;
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 100000000000000000 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 public _tFeeTotal;
    uint256 public _BSCTestToken2Burned;
    ...
    uint256 private _BSCTestToken2Burn = 2;
        ...
        if (from != owner() && to != owner() && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            if (from == pancakeswapPair && to != address(pancakeV2Router)) {
                ...
                if(_cooldownEnabled) {
                    ...
                    // 1mins cooldown to SELL after a BUY to ban front-runner bots
                    cooldown[to].sell = block.timestamp + (60 seconds);
                }
    ...
    function removeAllFee() private {
        if (_reflection == 0 && _contractFee == 0 && _BSCTestToken2Burn == 0) return;
        _reflection = 0;
        _contractFee = 0;
        _BSCTestToken2Burn = 0;
    }

    function restoreAllFee() private {
        _reflection = 3;
        _contractFee = 10;
        _BSCTestToken2Burn = 2;
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 amount) private {
        (uint256 tAmount, uint256 tBurn) = _BSCTestToken2EthBurn(amount);
    ...
    function _BSCTestToken2EthBurn(uint amount) private returns (uint, uint) {
        uint orgAmount = amount;
        uint256 currentRate = _getRate();
        uint256 tBurn = amount.mul(_BSCTestToken2Burn).div(100);
        uint256 rBurn = tBurn.mul(currentRate);
        _tTotal = _tTotal.sub(tBurn);
        _rTotal = _rTotal.sub(rBurn);
        _BSCTestToken2Burned = _BSCTestToken2Burned.add(tBurn);
