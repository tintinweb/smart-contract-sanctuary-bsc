 ...
 */
contract ArivaLottery is ReentrancyGuard, IArivaLottery, Ownable {
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInArv;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] arvPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInArv;
        uint32 finalNumber;
    }
     ...
     * @param _arvTokenAddress: address of the ARIVA token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _arvTokenAddress, address _randomGeneratorAddress) {
        arvToken = IERC20(_arvTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of ARIVA to this contract
        uint256 amountArvToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInArv,
            _ticketNumbers.length
        );

        // Transfer ariva tokens to this contract
        arvToken.safeTransferFrom(address(msg.sender), address(this), amountArvToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInArv += amountArvToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInArvToTransfer
        uint256 rewardInArvToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInArvToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        arvToken.safeTransfer(msg.sender, rewardInArvToTransfer);

        emit TicketsClaim(msg.sender, rewardInArvToTransfer, _lotteryId, _ticketIds.length);
    }
    ...
    /**
     * @notice Draw the final number, calculate reward in ARIVA per group, and make lottery claimable
     * @param _lotteryId: lottery id
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInArv) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in ARIVA for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].arvPerBracket[j] =
                ...
                // A. No ARIVA to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].arvPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInArv - amountToShareToWinners);

        // Transfer ARIVA to treasury address
        arvToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in ARIVA token
     * @dev Callable by owner or injector address
     */
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        require(_lotteries[_lotteryId].status == Status.Open, "Lottery not open");

        arvToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInArv += _amount;
     ...
     * @param _priceTicketInArv: price of a ticket in ARIVA
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInArv >= minPriceTicketInArv) && (_priceTicketInArv <= maxPriceTicketInArv),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInArv: _priceTicketInArv,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            arvPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInArv: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(arvToken), "Cannot be ARIVA token");
    ...
    /**
     * @notice Set ARIVA price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInArv: minimum price of a ticket in ARIVA
     * @param _maxPriceTicketInArv: maximum price of a ticket in ARIVA
     */
    function setMinAndMaxTicketPriceInArv(uint256 _minPriceTicketInArv, uint256 _maxPriceTicketInArv)
        external
        onlyOwner
    {
        require(_minPriceTicketInArv <= _maxPriceTicketInArv, "minPrice must be < maxPrice");

        minPriceTicketInArv = _minPriceTicketInArv;
        maxPriceTicketInArv = _maxPriceTicketInArv;
    }
     ...
     * @param _priceTicket price of a ticket (in ARIVA)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].arvPerBracket[_bracket];
        } else {
            return 0;
        }
    }
