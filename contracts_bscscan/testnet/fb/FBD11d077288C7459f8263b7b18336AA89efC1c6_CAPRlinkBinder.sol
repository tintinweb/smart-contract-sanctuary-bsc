...
contract CAPRlinkBinder is CfoTakeable,ReentrancyGuard {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    address public immutable rlt;
    address public immutable rlinkRelation;
    INameMapping public immutable nameMapping;
    address public immutable rewardsToken;

    mapping(address => bool) public exists;
    uint256 public recordedUserCount;

    uint256 public constant rewardAmount = 100 * 1e18;
    uint256 public constant parentRate = 15 * 1e16;
    uint256 public constant grandpaRate = 5 * 1e16;
    ...
    constructor(
        ...
        require(_rlt != address(0),"_rlt can not be address 0");
        require(_rlinkRelation != address(0),"_rlinkRelation can not be address 0");
        require(_nameNft != address(0),"_nameNft can not be address 0");
        require(_rewardsToken != address(0),"_rewardsToken can not be address 0");

        rlt = _rlt;
        rlinkRelation = _rlinkRelation;
        nameMapping = INameMapping(_nameNft);
        rewardsToken = _rewardsToken;
        
        IERC20(_rlt).safeApprove(_rlinkRelation,type(uint256).max);
        IERC20(_rewardsToken).safeApprove(_rlinkRelation, type(uint256).max);
    }
    ...
    function addRelation(address child,address parent) external payable nonReentrant {
        ...
        uint rltReward = IRlinkCore(rlinkRelation).addRelation(child, parent);

        _afterAddedRelation(child,parent,rltReward);
    }

    function addRelationByName(string memory parentName) external payable nonReentrant {
        address parent = nameMapping.getNameMapTo(parentName);
        require(msg.value >= addRelationBNBFee,"value too low");

        uint rltReward = IRlinkCore(rlinkRelation).addRelation(msg.sender, parent);

        _afterAddedRelation(msg.sender,parent,rltReward);       
    }

    function _afterAddedRelation(address child,address parent,uint rltRewardAmount) internal {
        uint recordedUserCount_ = recordedUserCount;
        uint incr=0;
        if(!exists[child]){
            incr += 1;
            exists[child] = true;
        ...
        if(incr > 0){
            recordedUserCount = recordedUserCount_ + incr;
        }

        if(rltRewardAmount > 0){
            require(IRlinkCore(rlinkRelation).distribute(rlt, msg.sender, rltRewardAmount, 0, rltRewardAmount.mul(parentRate).div(1e18), rltRewardAmount.mul(grandpaRate).div(1e18)) > 0,"distribute rlt failed");
        }

        if(rewardAmount > 0 && IERC20(rewardsToken).balanceOf(address(this)) >= rewardAmount){
            require(IRlinkCore(rlinkRelation).distribute(rewardsToken, msg.sender, rewardAmount, 0,rewardAmount.mul(parentRate).div(1e18), rewardAmount.mul(grandpaRate).div(1e18)) > 0,"distribute rewards token failed");
        }
    }

    function setAddRelationBNBFee(uint _fee) external onlyOwner {
        addRelationBNBFee = _fee;
    }

    // function setRewards(uint _rewardAmount,uint _parentAmount,uint _grandpaAmount) external onlyOwner {
    //     require(_parentAmount + _grandpaAmount <= _rewardAmount,"parentAmount + grandpaAmount too high");
    //     rewardAmount = _rewardAmount;
    //     parentAmount = _parentAmount;
    //     grandpaAmount = _grandpaAmount;
    // }
}