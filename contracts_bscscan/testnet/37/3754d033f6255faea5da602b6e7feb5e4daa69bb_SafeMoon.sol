...
contract SafeMoon is Context, IERC20, Ownable {
    ...
    constructor() {
        ...
        _reflectionBalances[msg.sender] = _reflectionTotal;
    ...
    function _transfer(
        ...
        require(balanceOf(from)>=amount, "Insufficient Balances");
        bool takeFee = true;
    ...
    function _transferToExcluded(
        ...
        _tokenBalances[recipient] = _tokenBalances[recipient].add(
            tTransferAmount
        );
        _reflectionBalances[recipient] = _reflectionBalances[recipient].add(
            rTransferAmount
        );
        _takeLiquidity(tLiquidity);
        _reflectFee(rFee, tFee);
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        ...
        _reflectionBalances[address(this)] =
            _reflectionBalances[address(this)] +
            rLiquidity;
        if (_isExcludedFromReward[address(this)])
            _tokenBalances[address(this)] =
                _tokenBalances[address(this)] +
                tLiquidity;
    }
    ...
    function setNumTokensSellToAddToLiquidity(
        uint256 numTokensSellToAddToLiquidityPercent
    ) external onlyOwner {
        numTokensSellToAddToLiquidity = _totalSupply
            .mul(numTokensSellToAddToLiquidityPercent)
            .div(10**2);
        emit SetNumTokensSellToAddLiquidity(
            numTokensSellToAddToLiquidityPercent
        );
    }
