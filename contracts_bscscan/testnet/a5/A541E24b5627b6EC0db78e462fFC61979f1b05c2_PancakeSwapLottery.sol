...
contract PancakeSwapLottery is ReentrancyGuard, IPancakeSwapLottery, Ownable {
    ...
    uint256 public minPriceTicketInCake = 1e7 wei;
    ...
    function setDevFee(uint256 _devFee) external onlyOwner {
        require(_devFee < 2000, "devFee too high");
        devFee = _devFee;
    }
        ...
        nonReentrant
    {
        ...
        uint256 balBef = cakeToken.balanceOf(address(this));
        ...
        uint256 balAft = cakeToken.balanceOf(address(this));

        // Increment the total amount collected for the lottery round
        // _lotteries[_lotteryId].amountCollectedInCake += amountCakeToTransfer;
        _lotteries[_lotteryId].amountCollectedInCake += (balAft - balBef);
        // _lotteries[_lotteryId].amountCollectedInCake += (amountCakeToTransfer * 9000 ) / 10000;
    ...
    function drawFinalNumberAndMakeLotteryClaimable(
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInCake) *
                (10000 - (_lotteries[_lotteryId].treasuryFee + devFee)))
        ) / 10000;
        ...
        uint256 devFeeAmt = (devFee *
            _lotteries[_lotteryId].amountCollectedInCake) / 10000;
        ...
        // Transfer dev fee to funding wallet
        cakeToken.safeTransfer(devWalletAddress, devFeeAmt);
    ...
    function startLottery(
        ...
        require(
            ...
            "Lottery less than min length"
        );
        require(
            (_endTime - block.timestamp) < MAX_LENGTH_LOTTERY,
            "Lottery more than max length"
        );
    ...
    function setOperatorAndTreasuryAndInjectorAddresses(
        ...
        require(_devWalletAddress != address(0), "Cannot be zero address");
