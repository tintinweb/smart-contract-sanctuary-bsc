...
contract RyanCoin is IERC20, Ownable, BaseToken {
    ...
    constructor(
      //  string memory name_,
       ...
       // address serviceFeeReceiver_,
        ...
        require(developmentFeeBps_ >= 0, "Invalid development fee");
        require(teamFeeBps_ >= 0, "Invalid team fee");
        ...
        if (developmentAddress_ == address(0)) {
            require(
                developmentFeeBps_ == 0,
                "Cant set both development address to address 0 and development percent more than 0"
            );
        }
        ...
        require(
            taxFeeBps_ + liquidityFeeBps_ + marketingFeeBps_ + developmentFeeBps_ + teamFeeBps_ <= 10**4 / 4,
            "Total fee is over 25%"
        );

        _name = "Fart";//name_;
        _symbol = "FRTFRT";//symbol_;
        _decimals = 9;

        _tTotal = 100000000 *10**_decimals;//totalSupply_;
...
/////////////////////////////////////////////////////
        ...
        _developmentFee = developmentFeeBps_;
        _previousDevelopmentFee = _developmentFee;


        _teamAddress = teamAddress_;
        _teamFee = teamFeeBps_;
  ...
  //      _takeTeamFee(tTeam);
    ...
    function setTaxFeePercent(uint256 taxFeeBps) external onlyOwner {
        _taxFee = taxFeeBps;
        require(
            _taxFee + _liquidityFee + _marketingFee + _developmentFee + _teamFee <= 10**4 / 4,
            "Total fee is over 25%"
        );
    }
        ...
        
    {
     ...
     //   uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tMarketingFee).sub(tDevelopmentFee).sub(tTeamFee);
          uint256 tTransferAmount = tAmount.sub(tMarketingFee).sub(tDevelopmentFee);
       
        return (tTransferAmount, tMarketingFee,tDevelopmentFee );
       //   return (tTransferAmount, tFee, tLiquidity, tMarketingFee,tDevelopmentFee, tTeamFee );
   ...
   //     uint256 rLiquidity = 0;
      ...
      //  uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity).sub(rMarketing).sub(rDevelopment).sub(rTeam);
        uint256 rTransferAmount = rAmount.sub(rMarketing).sub(rDevelopment);
    ...
    function _takeDevelopmentFee(uint256 tDevelopment) private {
        if (tDevelopment > 0) {
            ...
            _rOwned[_developmentAddress] = _rOwned[_developmentAddress].add(rDevelopment);
            if (_isExcluded[_developmentAddress])
                _tOwned[_developmentAddress] = _tOwned[_developmentAddress].add(
                    tDevelopment
                );
            emit Transfer(_msgSender(), _developmentAddress, tDevelopment);
    ...
    function calculateDevelopmentFee(uint256 _amount)
        ...
        returns (uint256)
    {
        if (_developmentAddress == address(0)) return 0;
  ...
  function calculateTeamFee(uint256 _amount)
    ...
    function removeAllFee() private {
        if (_taxFee == 0 && _liquidityFee == 0 && _marketingFee == 0 && _developmentFee == 0 && _teamFee == 0) return;
