...
contract MAMASHIBA is ERC20, Ownable, SafeToken, LockToken {
    ...
    function setFee(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _MarketingFee, uint256 _GameFee,uint256 _BurnFee) public onlyOwner {
        ...
        totalFees = BNBRewardsFee.add(liquidityFee).add(MarketingFee).add(BurnFee).add(GameFee); // total fee transfer and buy
    }
    ...
    constructor() ERC20("MAMASHIBA", "MAMA") {
        BNBRewardsFee = 5;
        liquidityFee = 3;
        ...
        GameFee = 1;
        BurnFee = 1;
        MarketingWallet = payable(0x77837AF661B188a73d46da412817643F20aB609F); 
        GameWallet = payable(0x7187b4287d9B5387d0F6309e1E3769545272049f);
        DeadWallet = payable(0x000000000000000000000000000000000000dEaD);
        totalFees = BNBRewardsFee.add(liquidityFee).add(MarketingFee).add(GameFee).add(BurnFee); // total fee transfer and buy
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // worthy Game fee
        uint256 GameAmount = deltaBalance.sub(bnbToAddLiquidityWith).div(totalFees.sub(liquidityFee).sub(MarketingFee)).mul(GameFee);
        GameWallet.transfer(GameAmount);
        // worthy Game fee
        uint256 BurnAmount = deltaBalance.sub(bnbToAddLiquidityWith).div(totalFees.sub(liquidityFee).sub(MarketingFee).sub(GameFee)).mul(BurnFee);
        DeadWallet.transfer(BurnAmount);
