...
contract TokenClaimV3 is Ownable {
    ...
    ClaimInfo[] addedTokensAndPhases;
    ...
    function updateAdminAddress(
        ...
        onlyOwner
        {
            require(newAdminAddress != address(0),"Ownable: new owner is the zero address");     
            adminAddress=newAdminAddress;
            emit AdminChanged(newAdminAddress);
        }
    ...
    function updateUserTokens(
        ...
        external
        _hasAllowance(adminAddress, totalReward, tokenAddress)
        onlyOwner
        returns (bool)
    {
        ...
        ERC20Interface.safeTransferFrom(adminAddress, address(this), totalReward);
        emit RewardAdded(tokenAddress, phaseNo, totalReward);
        addedTokensAndPhases.push(ClaimInfo(tokenAddress, ERC20Interface.name(), ERC20Interface.symbol(),  phaseNo));
        return true;
    }
    ...
    function claimMulti(address[] memory tokenAddresses, uint256[] memory phases)
        external
        returns (bool)
    {
        require (tokenAddresses.length==phases.length, "Invalid data lengths");

       for(uint256 i=0;i<tokenAddresses.length;i++){
           this.claim(tokenAddresses[i],phases[i]);
       }
        return true;
    }

    function getClaimList() public view returns (ClaimInfo[] memory)
    {
        return addedTokensAndPhases;
    }
}