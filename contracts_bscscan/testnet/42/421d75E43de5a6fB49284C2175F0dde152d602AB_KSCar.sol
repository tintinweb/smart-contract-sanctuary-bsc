    ...
    PermissionExtension
{
    ...
    constructor() ERC721("KingSpeed Car", "KSCar") {}
    ...
    function setToken(IERC20 kscToken, IERC20 busdToken) public onlyOwner {
        _token = kscToken;
        _busdToken = busdToken;
    }

    function awardItem(address to) private returns (uint256) {
    ...
    function awardItemFromFactory(address to)
        public
        onlyOperator
        returns (uint256)
    {
    ...
    function getNFTs(address queryAddress)
        public
        view
        returns (uint256[] memory _ids)
    {
        uint256 balance = balanceOf(queryAddress);
        uint256[] memory ids = new uint256[](balance);

        for (uint256 i = 0; i < balance; i++) {
            ids[i] = tokenOfOwnerByIndex(queryAddress, i);
        }
        return (ids);
    }
