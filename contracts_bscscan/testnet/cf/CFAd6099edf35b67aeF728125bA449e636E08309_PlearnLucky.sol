...
contract PlearnLucky is IPlearnLucky, ReentrancyGuard, Ownable {
    ...
    /**
     *@notice Remove this function when deploy to aminnet
     */
    ...
    event LuckyClose(uint256 indexed luckyId, uint32 currentTicketNumber);
    event LuckyOpen(
    ...
    event TicketsPurchase(address indexed buyer, uint256 indexed luckyId, uint32 numberTickets);
     ...
     */
    function buyTickets(uint256 _luckyId, uint32 _numberTickets) public notContract nonReentrant {
        ...
        require(
            (_userTicketNumbersPerRound[msg.sender][_luckyId].length + _numberTickets) <= luckyInfo.maxTicketsPerUser,
            'Limit tickets per user each a round'
        );
        require(
            (_getCountTicketsInRound(_luckyId) + _numberTickets) <= luckyInfo.maxTicketsPerRound,
            'Ticket not enough'
        );
        ...
        uint32 thisTicketNumber = luckyInfo.currentTicketNumber;
        ...
        luckyInfo.currentTicketNumber = thisTicketNumber;
     ...
     */
    function calculateTotalPriceForTickets(uint256 _priceTicket, uint32 _numberTickets) public pure returns (uint256) {
     ...
     */
    function closeLucky(uint256 _luckyId) public onlyOperatorOrOwner nonReentrant {
        Lucky storage luckyInfo = _lucks[_luckyId];
        ...
        emit LuckyClose(_luckyId, luckyInfo.currentTicketNumber);
    }
    ...
    function confirmTickets(
        ...
        for (uint256 i = 0; i < _ticketNumbers.length; i++) {
            ...
            require(_confirmBracket.getBracket(_luckyId, _ticketNumbers[i]) == 0, 'Ticket is confirmed');
            ...
            _confirmBracket.setBracket(_luckyId, _ticketNumbers[i], _brackets[i]);
        }
        ...
        nonReentrant
    {
        ...
        uint32[] memory randomResults = randomGenerator.getRandomResults();
        uint32 countLuckyNumbersPerRound = _getCountLuckyNumbersPerRound(_luckyId);
        if (randomResults.length < countLuckyNumbersPerRound) {
            countLuckyNumbersPerRound = uint32(randomResults.length);
        }
        uint32 countReservePerBracket = uint32(randomResults.length) - countLuckyNumbersPerRound;

        _lucks[_luckyId].luckyNumbers = new uint32[](countLuckyNumbersPerRound);
        if (countReservePerBracket != 0) {
            _lucks[_luckyId].reserveNumbers = new uint32[](countReservePerBracket);
        }

        for (uint32 i = 0; i < randomResults.length; i++) {
            if (i < countLuckyNumbersPerRound) {
                _lucks[_luckyId].luckyNumbers[i] = randomResults[i];
            } else {
                _lucks[_luckyId].reserveNumbers[i - countLuckyNumbersPerRound] = randomResults[i];
            }
        }
        ...
        if (_autoAllocateBracket) {
            ...
            for (uint256 i = 0; i < randomResults.length; i++) {
                uint32 luckyNumber = randomResults[i];
                _confirmBracket.setBracket(_luckyId, luckyNumber, (bracketIndex + 1));
                count++;
    ...
    function enableConfirmationForReserve(uint256 _luckyId) external onlyOperatorOrOwner {
        require(_lucks[_luckyId].status == Status.Claimable, 'Lucky not claimable');
        ...
        returns (uint32[] memory countLuckyNumbersPerBracket, uint32[] memory countAvailablesPerBracket)
    {
        ...
        for (uint256 i = 0; i < _lucks[_luckyId].luckyNumbers.length; i++) {
            uint32 bracket = _confirmBracket.getBracket(_luckyId, _lucks[_luckyId].luckyNumbers[i]);
            if (bracket != 0) {
    ...
    function getCountTicketsInRound(uint256 _luckyId) external view override returns (uint32) {
        ...
        )
    {
        ...
        bracket = _confirmBracket.getBracket(_luckyId, _ticketNumber);
        ...
        )
    {
        ...
        for (uint256 i = 0; i < _size; i++) {
            ...
            brackets[i] = _confirmBracket.getBracket(_luckyId, ticketNumbers[i]);
        }
    ...
    function startLucky(
        ...
        _lucks[_currentLuckyId] = Lucky({
            ...
            firstTicketNumber: startNumber,
            currentTicketNumber: startNumber,
    ...
    function tranferTickets(
        ...
        uint256 _luckyId,
        uint32[] calldata _ticketNumbers
    ) public notContract nonReentrant {
    ...
    function updateLuckyStatus(uint256 _luckyId, Status _status) external notContract nonReentrant onlyOperatorOrOwner {
    ...
    function _getCountTicketsInRound(uint256 _luckyId) internal view returns (uint32) {
        Lucky memory luckyInfo = _lucks[_luckyId];
        if (luckyInfo.currentTicketNumber > luckyInfo.firstTicketNumber) {
            return luckyInfo.currentTicketNumber - luckyInfo.firstTicketNumber;
        } else {
            return 0;
        }
    }
    ...
    function _getFirstAndLastTicketNumberInRound(uint256 _luckyId) internal view returns (uint32, uint32) {
        Lucky memory luckyInfo = _lucks[_luckyId];
        if (luckyInfo.currentTicketNumber > luckyInfo.firstTicketNumber) {
            return (luckyInfo.firstTicketNumber, luckyInfo.currentTicketNumber - 1);
        } else {
            return (0, 0);
        }
    }
...
interface IConfirmBracket {
    function getBracket(uint256 _luckyId, uint32 _ticketNumbers) external view returns (uint32);
...
interface IPlearnLucky {
     ...
     * @notice Return count of lucky numbers and reserve each a round
     */
    function getCountRandomsPerRound(uint256 _luckyId) external view returns (uint32);

    function getCountTicketsInRound(uint256 _luckyId) external view returns (uint32);
