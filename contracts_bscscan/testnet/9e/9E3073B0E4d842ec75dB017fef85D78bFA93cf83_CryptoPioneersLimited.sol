...
contract CryptoPioneersLimited is NFT, PriceFeed {
  ...
  {
    uint256 totalPrice = price.mul(getPrice('USDT', 'BNB')).mul(qty);
  ...
  function getWhiteListTotalPrice(uint256 qty)
    ...
    returns (uint256, uint256)
  {
    uint256 totalPrice = whiteListPrice.mul(getPrice('USDT', 'BNB')).mul(qty);
    uint256 totalPriceWithSlippageTolerance = totalPrice
      .mul((10**slippageToleranceDecimals).sub(slippageTolerance))
      .div(10**slippageToleranceDecimals);
    return (totalPrice, totalPriceWithSlippageTolerance);
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    ...
    if (tokenId < 200) {
      return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, 'tiger.json')) : "";
    } else {
      return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, 'snake.json')) : "";
    }
  }
...
contract NFT is ERC721, ERC721Enumerable, Ownable {
  ...
  string private _baseURIExtended = 'ipfs://QmUCJSsgaXeryM9Fh2VmBVdpi9bDatquMEZ7mds6472wsV/';
