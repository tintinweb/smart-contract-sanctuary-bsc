...
contract BHOLaunchpad is Ownable, Pausable, Whitelist, SharedConstants {
    ...
    mapping(address => bool) private registrationList;
    ...
    constructor(
        ...
        require(_launchpadData.allocation.startAt <= _launchpadData.allocation.endAt, "Allocation time invalid");
        require(_launchpadData.register.startAt <= _launchpadData.register.endAt, "Register time invalid");
        require(_launchpadData.fcfs.startAt <= _launchpadData.fcfs.endAt, "FCFS time invalid");
        ...
        registrationList[FACTORY_ADDRESS] = true;
    ...
    function finalize() public onlyOwner {
        ...
        // Get 98% BUSD to FACTORY_ADDRESS
    ...
    function register() public {
        ...
        require(block.timestamp > launchpadData.register.startAt, "Register time dont start yet");
        require(block.timestamp < launchpadData.register.endAt, "Register time expired");
        address user = _msgSender();
        registrationList[user] = true;
    }
    ...
    function buyTokenAllocation(uint256 _amount) external checkWhoCanBuy {
        ...
        require(block.timestamp > launchpadData.allocation.startAt, "Allocation round dont start yet");
        require(block.timestamp < launchpadData.allocation.endAt, "Allocation round expired");

        // max BUSD user can buy
        address buyer = _msgSender();

        (, , uint256 poolWeight) = staking.levelOf(buyer);
        uint256 maxBuy = launchpadData.baseAllocation.mul(poolWeight);
    ...
    function buyTokenFCFS(uint256 _amount) external checkWhoCanBuy {
        ...
        require(block.timestamp > launchpadData.fcfs.startAt, "FCFS round dont start yet");
        require(block.timestamp < launchpadData.fcfs.endAt, "FCFS round expired");
