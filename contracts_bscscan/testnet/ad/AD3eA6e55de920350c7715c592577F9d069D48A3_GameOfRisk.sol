...
contract GameOfRisk is IGameOfRisk, ERC721Enumerable, Ownable, Pauseable {
    // mint price
...
interface IBattle {
    ...
    function totalThroneEarned() external view returns (uint256);

    function lastClaimTimestamp() external view returns (uint256);

    function setOldTokenInfo(uint256 _tokenId) external;

    function pack(uint256, uint256) external view returns (Battle.Stake memory);
...
contract Battle is Ownable, IERC721Receiver, Pauseable {
