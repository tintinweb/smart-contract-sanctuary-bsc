...
contract SpaceWorms001 is ERC721, Ownable {
    ...
    uint public MAX_TOKENS = 20;
    uint public MAXTOKENS_PERUSER = 2;
    ...
    {
		require(numberOfTokens <= MAXTOKENS_PERUSER, "Cada address solo puede tener hasta dos NFT");
		require((balanceOf(msg.sender) + numberOfTokens) <= MAXTOKENS_PERUSER, "Cada address solo puede tener hasta dos NFT");
        require((_tokenIdCounter.current() + numberOfTokens) <= MAX_TOKENS, "Se han minteado todos los NFT");
		require(msg.value == (price * numberOfTokens), "No tiene suficiente BNB");
