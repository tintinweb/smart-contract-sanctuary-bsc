...
contract NFTTNNSPRO1155v3 is ERC1155 {
    ...
    */
    ...
    function mint(string memory uri, uint256 supply, address[] memory royaltyAddress, uint256 royaltyFee, Sign memory sign)  public {
        require(!usedNonce[sign.nonce],"Nonce : Invalid Nonce");
        usedNonce[sign.nonce] = true;
        
        require(royaltyAddress.length > 0 , "Royalty Cannot be Empty");
        verifySign(uri, msg.sender, sign);

        uint[] memory royaltyFeeArr = new uint[](royaltyAddress.length);
        uint256 royaltyPerUser = royaltyFee / royaltyAddress.length ;
        for (uint i=0; i<royaltyAddress.length; i++){
            royaltyFeeArr[i] = royaltyPerUser;
        }
        
        _mint(newItemId, supply, uri, royaltyAddress, royaltyFeeArr);
