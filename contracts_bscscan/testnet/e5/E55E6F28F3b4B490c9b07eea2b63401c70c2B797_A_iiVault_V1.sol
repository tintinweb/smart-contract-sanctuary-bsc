 ...
 */
contract A_iiVault_V1 is ERC20, Ownable, ReentrancyGuard, Pausable {
    ...
    */
    constructor () ERC20("iiVaultCAKE-BNB-LP Token", "iiVaultCAKE-BNB-LP") {
        strategy = IStrategy(address(0));
        approvalDelay = 60;//21600;
    ...
    function getData() public view returns (            
        ...
        TokensHoldData memory tokensHoldData,
        StrategyData memory strategyData
        ) {
        
        bool isStrategy = address(strategy) != address(0);  
        
        vaultData.productionMode = productionMode;
        vaultData.paused = paused();
        vaultData.whiteListOnly = whiteListOnly;
        vaultData.balance = isStrategy ? balance() : 0;
        vaultData.available = isStrategy ? available() : 0;      
        vaultData.pricePerFullShare = isStrategy ? getPricePerFullShare() : 0;   

        tokensHoldData.tokenToHold = tokenToHold;
        tokensHoldData.minTokenToHold = minTokenToHold;  

        strategyData.strategy = address(strategy);
        strategyData.want = isStrategy ? address(strategy.want()) : address(0);
        strategyData.paused = isStrategy ? strategy.paused() : false;        
        strategyData.balanceOf = isStrategy ? strategy.balanceOf() : 0;
        strategyData.balanceOfWant = isStrategy ? strategy.balanceOfWant() : 0;
        strategyData.balanceOfPool = isStrategy ? strategy.balanceOfPool() : 0;        
    }
    ...
    //
	function setStrategy(address _strategy) public onlyOperator {        
