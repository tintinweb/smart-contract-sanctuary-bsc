...
contract LICStake is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address payable public marketingAddress = payable(0xd013844024829de0B9f84c7AB41AFFdaFA31532A);
    ...
    constructor() ERC20("LICStake", "$LICS") {
    ...
    function updatePayoutToken(address token) public {
        require(balanceOf(msg.sender) > 0, "You must own more than zero LICStake tokens to switch your payout token!");
...
contract BeDogeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor(address payable mainContract)
        DividendPayingToken(
            "LICStake_Dividend_Tracker",
            "LICStake_Dividend_Tracker"
        )
    {
        beDogeDealer = LICStake(mainContract);
    ...
    function _transfer(
        ...
        require(false, "LICStake_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "LICStake_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main LICStake contract."
        );
    }
