...
contract LuckyBallGameV7 is ERC1155Holder, AccessControl {
    struct Round {
        uint32 startAt;
        uint32 endAt;
        uint32 totalBall;
    ...
    function claimReward(bytes32 _transactionId, uint256[] memory _ballIds) external {
        ...
        uint256 _totalBallInRound = uint256(_round.totalBall) * 10 ** 18;
        ...
        for (uint256 i = 0; i < _totalBallOfPlayer; i++) {
            ...
            } else if (_totalBallInRound > 5 ether && _isJackpot) {
                uint256 _remainBalls = (_totalBallInRound - 5 ether) / 2;
