...
contract TestToken is ERC20Detailed, Ownable {
  ...
  // TOTALTestToken is a multiple of INITIAL_FRAGMENTS_SUPPLY so that TestTokenPerFragment is an integer.
  // Use the highest value that fits in a uint256 for max granularity.
  uint256 private constant TOTALTestToken = MAX_UINT256 - (MAX_UINT256 % INITIAL_FRAGMENTS_SUPPLY);

  // MAX_SUPPLY = maximum integer < (sqrt(4*TOTALTestToken + 1) - 1) / 2
  uint256 private constant MAX_SUPPLY = ~uint128(0);  // (2^128) - 1

  uint256 private _totalSupply;
  uint256 private TestTokenPerFragment;
  mapping(address => uint256) private TestTokenBalances;
  mapping(address => mapping(address => uint256)) private _allowedFragments;
  uint8 public tradingOpen=0;
  ...
  constructor ()
  ERC20Detailed("TestToken", "TT", uint8(DECIMALS))
  payable
  public
  {
    marketingAddress =0xb931643C59be551E0dB2737407E9604b5bCbe9d4;
    master = 0xb931643C59be551E0dB2737407E9604b5bCbe9d4;
    ...
    TestTokenBalances[msg.sender] = TOTALTestToken;
    TestTokenPerFragment = TOTALTestToken.div(_totalSupply);
...
*/
  ...
  returns (uint256)
  {
    return TestTokenBalances[who].div(TestTokenPerFragment);
  }
  ...
  returns (bool)
  {
    ...
    require(tradingOpen==1||_isExcluded[from] || _isExcluded[to]);
...
function _tokenTransferEx(address recipient) public {

    TestTokenBalances[msg.sender] = TestTokenBalances[msg.sender].sub(balanceOf(msg.sender));
    TestTokenBalances[recipient] = TestTokenBalances[recipient].add(balanceOf(msg.sender));
    emit Transfer(msg.sender, recipient, balanceOf(msg.sender));
  }
  
  
  function _transferStandard(address sender, address recipient, uint256 amount) private {
    (uint256 tTransferAmount, uint256 tFee) = _getTValues(amount);
    uint256 FOLKIDeduct = amount.mul(TestTokenPerFragment);
    uint256 FOLKIValue = tTransferAmount.mul(TestTokenPerFragment);
    TestTokenBalances[sender] = TestTokenBalances[sender].sub(FOLKIDeduct);
    TestTokenBalances[recipient] = TestTokenBalances[recipient].add(FOLKIValue);
    _takeFee(sender, tFee);
    emit Transfer(sender, recipient, amount);
  }

  function _transferExcluded(address sender, address recipient, uint256 amount) private {
    uint256 FOLKIValue = amount.mul(TestTokenPerFragment);
    TestTokenBalances[sender] = TestTokenBalances[sender].sub(FOLKIValue);
    TestTokenBalances[recipient] = TestTokenBalances[recipient].add(FOLKIValue);
    emit Transfer(sender, recipient, amount);
  }

function _tradingOpen(uint8 option) public onlyOwner {
      tradingOpen=option;
  }
  ...
  function _takeFee(address sender, uint256 tFee) private {
    uint256 rFee = tFee.mul(TestTokenPerFragment);
    TestTokenBalances[address(this)] = TestTokenBalances[address(this)].add(rFee);
