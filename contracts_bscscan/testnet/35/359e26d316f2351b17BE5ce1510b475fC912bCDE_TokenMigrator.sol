...
// Migrate at a default ratio, lower that ratio if tokens are locked, unlock on a vesting scheadule
contract TokenMigrator is Ownable, Blacklist, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for ERC20PresetMinterPauser;

    struct TokenRatio {
        uint256 multiplier; // 1 * multiplier is how many tokens for this setting
        uint256 vestPoints; // // lockDuration / vestPoints = each vesting period
        uint256 lockDuration; // how long you need to lock for this setting
        uint256 totalConverted; // total number of the input token taken at this setting
    }
    ...
    // V1 Contract
    ERC20PresetMinterPauser public tokenV1;

    // new token
    ERC20PresetMinterPauser public tokenV2;
    ...
    // track total migration amounts
    uint256 public migratedTokensIn;
    uint256 public migratedTokensOut;

    // The burn address
    address public constant burnAddress = address(0xdead);

    // array of options for each ratio/lock setting
    mapping(uint256 => TokenRatio) public tokenRatios;
    uint256 public totalSettings;

    // max locks 1 account can ever have
    uint256 maxLocks = 100;
    ...
    event Claimed(address account, uint256 settingId, uint256 amount);
    event Migrated(address account, uint256 amountIn, uint256 amountOut, uint256 lockEnd);

    constructor(
        ERC20PresetMinterPauser _tokenV1,
        ERC20PresetMinterPauser _tokenV2,
        Vault _vault,
        uint256 _defaultMultiplier,
        uint256 _maxLocks
    ) {
        tokenV1 = _tokenV1;
        tokenV2 = _tokenV2;
        ...
        tokenRatios[totalSettings] = TokenRatio({
        ...
        totalSettings++;
     //   tokenRatios[0].multiplier = _defaultMultiplier;
     ...
     * we pass in the ratio option
     * basded on the settings we do the following:
     * - multiply the old token input by the ratio multiplier
     * - we burn the old token
     * - we mint the new token at the adjusted amount
     * - we lock the token if the option isn't 0 
     * - 
    */

     function migrate(uint256 _settingId, uint256 _amount) public noBlacklistAddress nonReentrant {
        // check if we are still migrating
        require(migrationActive, "ERC20: Migration not active");
        require(tokenV1.balanceOf(msg.sender) >= _amount, "ERC20: Low balance");
        require(_settingId == 0 || tokenRatios[_settingId].lockDuration > 0, "ERC20: Setting Not Found");
        require(maxLocks > userTotalLocks[msg.sender], "ERC20: Max Migrations");

        bool burnSuccess = false;

        uint256 newBalance = _amount.mul(tokenRatios[_settingId].multiplier).div(1 ether);

        

        // keep track of migrated amount 
        migratedTokensIn = migratedTokensIn.add(_amount);
        migratedTokensOut = migratedTokensOut.add(newBalance);

        address _to;
        uint256 unlockTime = block.timestamp;
        if(_settingId > 0){

            // give them vault shares.
            // users should keep their share points while the tokens are locked
            // the token migrator needs to be set as a systemAddress for the token
            vault.giveShares(msg.sender,newBalance);


            // we need to lock
            // send the mint to the contract
            _to = address(this);

            unlockTime = block.timestamp.add(tokenRatios[_settingId].lockDuration);
            
            // set the lock data and vesting periods
            uint256 _idx = userTotalLocks[msg.sender];
            
            userLocks[msg.sender][_idx].tokenAmount = newBalance;
            userLocks[msg.sender][_idx].vestPoints = tokenRatios[_settingId].vestPoints;
            userLocks[msg.sender][_idx].vestShare = newBalance.div(tokenRatios[_settingId].vestPoints);
            userLocks[msg.sender][_idx].vestPeriod = tokenRatios[_settingId].lockDuration.div(tokenRatios[_settingId].vestPoints);
            userLocks[msg.sender][_idx].startTime = block.timestamp;
            userLocks[msg.sender][_idx].endTime = unlockTime;

/*
            userLocks[msg.sender][userTotalLocks[msg.sender]] = UserLock({
                tokenAmount: newBalance,
                claimedAmount: 0,
                vestPoints: tokenRatios[_settingId].vestPoints,
                vestShare: newBalance.div(tokenRatios[_settingId].vestPoints),
                vestPeriod: tokenRatios[_settingId].lockDuration.div(tokenRatios[_settingId].vestPoints),
                startTime: block.timestamp,
                endTime: unlockTime
            });
            
         */   // update the total locks for this user
            userTotalLocks[msg.sender] = userTotalLocks[msg.sender].add(1);

        } else {
            _to = msg.sender;
            // no lock, default migration
        }

        // burn and requre it was a success
        burnSuccess = tokenV1.transferFrom(msg.sender, burnAddress, _amount);
        require(burnSuccess, "ERC20: Burn failed");

        
        // mint the new tokens
        tokenV2.mint(_to, newBalance);

        emit Migrated(msg.sender,_amount,newBalance,unlockTime);

    }


    function claimLock(uint256 _lockId) public nonReentrant {
        require(userLocks[msg.sender][_lockId].tokenAmount > 0, 'Not Locked');
        require( (userLocks[msg.sender][_lockId].tokenAmount.sub(userLocks[msg.sender][_lockId].claimedAmount) > 0 ), 'Nothing to claim');

        // see how many vest points they have hit
        uint256 vested;
        for(uint256 i = 1; i <= userLocks[msg.sender][_lockId].vestPoints; ++i){
            if(block.timestamp >= userLocks[msg.sender][_lockId].startTime.add(userLocks[msg.sender][_lockId].vestPeriod.mul(i))){    
                vested++;
            }
        }

        uint256 totalVested = userLocks[msg.sender][_lockId].vestShare.mul(vested);

        // get the amount owed to them based on previous claims and current vesting period
        uint256 toClaim = totalVested.sub(userLocks[msg.sender][_lockId].claimedAmount);

        require(toClaim > 0, 'Nothing to claim.');

        userLocks[msg.sender][_lockId].claimedAmount = userLocks[msg.sender][_lockId].claimedAmount.add(toClaim);

        // move the tokens
        tokenV2.safeTransfer(address(msg.sender), toClaim);
        emit Claimed(msg.sender, _lockId, toClaim);
    }
    ...
    function addRatioSetting(uint256 _multiplier, uint256 _vestPoints, uint256 _lockDuration) public onlyOwner {
        tokenRatios[totalSettings] = TokenRatio({
            multiplier: _multiplier,
            vestPoints: _vestPoints,
    ...
    // set lock duration to 0 to disable.
    // update the default setting passing in 0 for the _settingId 
    function updateRatioSetting(uint256 _settingId, uint256 _multiplier, uint256 _vestPoints, uint256 _lockDuration) public onlyOwner {
        tokenRatios[_settingId] = TokenRatio({
            multiplier: _multiplier,
            vestPoints: _vestPoints,
            lockDuration: _lockDuration,
            totalConverted: 0
    ...
    function setMaxLocks(uint256 _maxLocks) public onlyOwner {
        maxLocks = _maxLocks;
    }
   
    function setV1Contract(ERC20PresetMinterPauser _tokenV1) public onlyOwner {
        tokenV1 = _tokenV1;
    }

    function setV2Contract(ERC20PresetMinterPauser _tokenV2) public onlyOwner {
        tokenV2 = _tokenV2;
    }
