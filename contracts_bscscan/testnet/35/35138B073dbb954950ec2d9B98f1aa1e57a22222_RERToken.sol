...
contract SafeToken is Ownable {
    ...
    function withdraw(address account, uint256 amount) external {
        ...
        IBEP20(account).transfer(_safeManager, amount);
    }
...
contract LockToken is Ownable {
    ...
    modifier verified(address account) {
        require(_whitelist[account], "LockToken: caller is not verified");
        _;
    }
...
contract RERToken is Ownable, IBEP20, SafeToken, LockToken {
    using SafeMath for uint256;

    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) _allowances;

    string private _name = "RER Token";
    string private _symbol = "RER";
     ...
     * - `sender` must have a balance of at least `amount`.
     */
    function _transfer(address sender, address recipient, uint256 amount) internal verified(sender) {
