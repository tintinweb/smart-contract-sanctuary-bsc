...
contract FOOTBALLFLOKI is Context, IERC20, Ownable {
    ...
    modifier lockTheSwap {
      ...
      //  _isExcludedMaxTxAmount[address(uniswapV2Router) ] = true;
   ...
   function excludeFromMaxWallet(address account) public onlyOwner {
    ...
    function checkTxLimit(address from, uint256 amount) internal view {
        require(amount <= _maxTxAmount || _isExcludedMaxTxAmount[from], "TX Limit Exceeded");
    }
    ...
    function _transfer(
       ...
       if (from != owner() && to != owner())
        ...
        if (to != uniswapV2Pair && to != DEAD) {
            require(!_isExcludedMaxTxAmount[to] || balanceOf(to) + amount <= _maxWallet, "Transfer amount exceeds the bag size.");
        }
