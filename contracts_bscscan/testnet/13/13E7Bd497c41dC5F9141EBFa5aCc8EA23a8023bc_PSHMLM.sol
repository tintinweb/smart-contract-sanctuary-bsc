...
contract PSHMLM is Ownable, HSHRewarder {
  ...
  event TransferEvent(uint256 sender, uint256 recipient, uint256 amount);
  ...
  constructor(
  ...
  ) HSHRewarder(rewardToken, rewardAmount) {
    ...
    subscriptionDuration = 30 minutes;
    subscriptionRenewalOffset = 5 minutes;
  ...
  function registerUser(uint256 referrer) public solvent {
    ...
    returns (uint256)
  {
    ...
    console.log(
      "ID: %s, depth: %s, count: %s",
      recipientID,
      depth + 1,
      referrals[recipientID].length
    );
  ...
  function renewSubscription(uint256 _userid) private {
    ...
    emit SubscriptionRenewalEvent(msg.sender, block.timestamp, _userid);
  ...
  function payForSubscription(uint256 _userid) private {
    ...
    console.log("Transfer from %s", _userid);
    for (uint256 i = 0; i < recipients.length; i++) {
      console.log("to: %s", recipients[i]);
      paymentToken.safeTransferFrom(
        msg.sender,
        users[recipients[i]].wallet,
        amount
      );
      emit TransferEvent(_userid, recipients[i], amount);
    }
  }
    ...
    returns (uint256)
  {
    ...
    console.log("id: %s lvl: %s mq: %s", userid, level, maxQualified);
    return maxQualified;
  }
...
abstract contract HSHRewarder is Ownable {
  using SafeERC20 for IERC20;
  ...
  function sendReward(address recipient) internal {
    if(rewardSupply < rewardAmount) return;
    rewardToken.safeTransfer(recipient, rewardAmount);
    rewardSupply = rewardSupply - rewardAmount;
  }

  function increaseRewardSupply(uint256 amount) external onlyOwner {
    require(
      amount > 0
      && rewardToken.balanceOf(address(this)) >= (rewardSupply + amount),
      "incorrect reward amount"
    );

    rewardSupply = rewardSupply + amount;
  }
