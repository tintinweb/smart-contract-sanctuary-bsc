...
contract TickALock is Ownable, IBEP20 {
    ...
    uint256 public _totalSupply2 = _initialSupply * 200 / 10000;
    //get 24% of the total supply
    uint256 public _totalSupply24 = _initialSupply * 2400 / 10000;
    /* Trading */
    ...
    /* Balance & Sell Limits */
    uint256 public _maxWalletSize = getTotalSupply() / 100; // 1 % of total supply
    uint256 public _maxSellSize = getTotalSupply() / 100;
    /* PancakeSwap */
    ...
    event TotalBurnedTokensAdd(uint256 totalBurnedTokens);
    ...
    function setMaxWalletSize(uint256 maxWalletSize) external onlyDev {
        ...
        require(maxWalletSize >= getTotalSupply() / 100, "maxWalletSize must be greater than or equal to 1% of the total supply");
        require(maxWalletSize <= getTotalSupply() / 100 * 3, "maxWalletSize must be less than or equal to 3% of the total supply");
    ...
    function calculateBurnTax(address sender) public view returns (uint256) {
        ...
        uint256 percentageOfHoldings = (senderBalance * 10000) / getTotalSupply();
    ...
    //totalSupply() pure function returns _initialSupply minus burned tokens
    function totalSupply() external view override returns (uint256) {
        return _initialSupply - _totalBurnedTokens;
    }
    ...
    function setPuzzleSolved(
        ...
        //if _addressThatSolved is the burn wallet, set those tokens as burned
        if(_addressThatSolved == _burnWallet) {
            totalBurnedTokensAdd(prizeAllotment);
        }
    ...
    function burnTokenContractTokens() private {
        ...
        totalBurnedTokensAdd(_tokensToBurn);
    }
    ...
    function totalBurnedTokensAdd(uint256 amount) private {
        _totalBurnedTokens += amount;
        emit TotalBurnedTokensAdd(amount);
    }
}