...
contract MultTokenSale is Context, IBEP20, Ownable {
  ...
  function tokensAccept() external view returns(string[] memory) {
     ...
     */
  function getpriceBNB(uint256 amount) public view virtual returns (uint price) {
    uint256 resPrice = 0;

    if(getContract("BNB") == _bnbBusdPair.token0()){
      (uint256 ResBNB, uint256 ResBusd,) = _bnbBusdPair.getReserves();
      uint256 totBNB = ResBNB.mul(ResBusd);
      uint256 totBusd = amount.mul(_DECIMALFACTOR);
      resPrice = totBusd.div(totBNB);
    }else{
      (, uint256 ResBNB,) = _bnbBusdPair.getReserves();
      uint256 totBusd = amount.mul(_DECIMALFACTOR);
      resPrice = ResBNB.mul(totBusd);
    }
