...
//  uint constant secondInADay = 86400; // 24 * 60 * 60
  uint constant secondInADay = 1440; // 24 * 60 // todo
  ...
  mapping(uint => uint) dailyTotalPoint;
  ...
  function depositFOTA(uint _amount) external initStartTime {
    ...
    dailyTotalPoint[getDaysPassed()] += _amount;
  ...
  function depositLP(uint _amount) external initStartTime {
    ...
    farmer.point += point;
    dailyTotalPoint[getDaysPassed()] += point;
  ...
  function withdraw() external {
    ...
    dailyTotalPoint[getDaysPassed()] -= farmers[msg.sender].point;
  ...
  function getTodayReward() public view returns (uint) {
    uint dayToReward = getDaysPassed() - 1;
    if (dailyTotalPoint[dayToReward] == 0) {
      return 0;
    }
    return dailyReward[dayToReward] * dailyCheckinPoint[dayToReward] / dailyTotalPoint[dayToReward];
  }
  ...
  function _calculateUserTodayReward() private view returns (uint) {
    uint dayPassed = getDaysPassed();
    uint dayToReward = dayPassed - 1;
    return farmers[msg.sender].point * dailyReward[dayToReward] / dailyTotalPoint[dayToReward];
  }
