...
// File: contracts/Farm/ETHXBank.sol
...
contract ETHXBank is Ownable {
    ...
    event AddReward(uint256 blockTimestamp, uint256 reward);
    event Claim(address indexed user, uint256 blockeTimestamp, uint256 reward);
    event Paused();
    event UnPaused();
    ...
    constructor(IERC20 _staking, IERC20 reward) public {
        token = _staking;
        rewardToken = reward;
    }
    ...
    function addReward(uint256 amount) public {
        ...
        accPerShare = accPerShare.add(amount.mul(1e12).div(lpSupply));

        emit AddReward(block.timestamp, amount);
    }
    ...
    function claim() public isPaused {
        ...
        if (user.amount > 0) {
            ...
            safeRewardTransfer(rewardToken, _userAddr, pendingReward);
        }
