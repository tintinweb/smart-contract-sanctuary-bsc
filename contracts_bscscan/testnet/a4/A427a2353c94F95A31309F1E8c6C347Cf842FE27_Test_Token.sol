...
contract Test_Token is Context, IERC20, Ownable {
    ...
    uint private startingSupply = 1000000000; //1 Billion Total token supply 
    ...
    string private _name = "TESTTOKEN"; //Token Name
    string private _symbol = "TSTKN"; // token symbol
    
    // ALL FEES ARE DIVIDED BY 10**5 FOR MORE ACCURACY
    // EXAMPLE: 400 / 10**5 = 4%

    //This is the redistribution rewards for holders
    uint256 private _reflectFee = 0; // Set dynamically
    uint256 public _buyReflectFee = 100;
    uint256 public _sellReflectFee = 100;
    uint256 private maxReflectFee = 700;

    uint256 private _devFee = 0; // Set dynamically
    uint256 public _buyDevFee = 100;
    uint256 public _sellDevFee = 200;
    uint256 private maxDevFee = 500;
    ...
    //pancakeswap v2 router mainnet: 0x10ED43C718714eb63d5aA57B78B54704E256024E
        ...
        //testnet 360                   : 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3
    address private _routerAddress = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;



    //Set your marketing/dev wallet address here
    
    address payable public _marketingWallet = payable(0x000000000000000000000000000000000000dEaD);
    address payable public _devWallet = payable(0x000000000000000000000000000000000000dEaD);
    
    bool inSwapAndLiquify;
    bool public swapAndLiquifyEnabled = false;
    
    // Max TX amount is 2.5% of the total supply.
    uint256 private maxTxPercent = 25; // Less fields to edit
    ...
    // Maximum wallet size is 5% of the total supply.
    uint256 private maxWalletPercent = 50; // Less fields to edit
    ...
    // 1% of Total Supply
    uint256 private numTokensSellToAddToLiquidity = (_tTotal * 1) / 100;

   //Don't change this address
    address public burnAddress = 0x000000000000000000000000000000000000dEaD;// All the burnt tokens will come to this address
    ...
    function setBuyTaxes(uint256 reflectFee, uint256 liquidityFee, uint256 marketingFee, uint256 devFee) external onlyOwner() {
        ...
        require(devFee <= maxDevFee); // Prevents owner from abusing fees.
    ...
    function setSellTaxes(uint256 reflectFee, uint256 liquidityFee, uint256 marketingFee, uint256 devFee) external onlyOwner() {
        require(reflectFee <= maxReflectFee); // Prevents owner from abusing fees.
        require(liquidityFee <= maxLiquidityFee); // Prevents owner from abusing fees.
        require(marketingFee <= maxMarketingFee); // Prevents owner from abusing fees.
        require(devFee <= maxDevFee); // Prevents owner from abusing fees.
    ...
    function setDevWallet(address payable newDWallet) external onlyOwner {
        require(_devWallet != newDWallet, "Wallet already set!");
        _devWallet = payable(newDWallet);
    }
...
,        address to,
    ...
    function _takeDev(address sender, uint256 tDev) private {
        ...
        emit Transfer(sender, _devWallet, tDev); // Transparency is the key to success.
    }
    ...
    function calculateDevFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_devFee).div(masterTaxDivisor);
    }
