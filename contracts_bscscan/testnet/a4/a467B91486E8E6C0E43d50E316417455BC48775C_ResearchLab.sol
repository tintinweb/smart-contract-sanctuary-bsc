...
contract ResearchLab is OwnableUpgradeable, ReentrancyGuard {
    ...
    function getTotalPoolValueWeighted() public view returns (uint256 totalPoolAlloc) {
      for (uint256 i=0; i < poolToken.length; i++) { 
        totalPoolAlloc = totalPoolAlloc.add(poolToken[i].lpToken.balanceOf(address(this)).mul(1000).div(1000-(poolToken[i].poolWeight)));
      }
    }
    ...
    function createUser(uint256 _pid, address _userAddress, uint256 _amount, uint256 _oDepositTime, uint256 _oDepositBlock, uint256 _oLastRewardBlock) public nonReentrant {
    ...
    // Deposit LP tokens into the pool
    /*function depositBridge(uint256 _pid, uint256 _amount, address _oAddress, uint256 _oDepositTime, uint256 _oDepositBlock, uint256 _oLastRewardBlock) private nonReentrant {
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        bridgedValue[_pid] = bridgedValue[_pid].add(_amount);
    ...
    function pendingRewardsCalc(uint256 _pid, address _user, uint256 _blockNumber) private {
        ...
        if (user.lastRewardBlock < startBlock.add(numberOfEras.mul(blocksPerEra)).add(blocksPerVesting)) {
          (rewardValueUnlocked, rewardValueLocked, adjVested) = pendingRewards(_pid, _user);
        }
