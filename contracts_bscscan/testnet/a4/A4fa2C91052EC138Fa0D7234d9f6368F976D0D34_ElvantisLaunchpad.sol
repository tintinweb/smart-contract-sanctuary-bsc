...
contract ElvantisLaunchpad is Ownable {
    ...
    function contribute() external payable {
        require(
            contributionEnabled,
            "ElvantisLaunchpad: Contribution is disabled"
        );
        require(roundLimits[currentRound] >= totalContributions[currentRound] + msg.value, "ElvantisLaunchpad: Hardcap for current round is filled");
        ...
        require(
            ...
            "ElvantisLaunchpad: Minimun Contribution not met"
        );
        require(
            userContribution[msg.sender] <= maxContribution,
            "ElvantisLaunchpad: Maximum Contribution exceeded"
        );
    ...
    function claim() external {
        require(claimEnabled, "ElvantisLaunchpad: Claim disabled");
        uint256 _contributions = userContribution[msg.sender] - userClaim[msg.sender];
        require(_contributions > 0,"ElvantisLaunchpad: Only contributors are allowed to claim");
    ...
    function moveToNextRound() external onlyOwner {
        require(totalRounds > currentRound, "ElvantisLaunchpad: Rounds are finished");
        currentRound++;
    ...
    function drainAccidentallySentEth(
        address recipient,
        uint256 amount
    ) external onlyOwner {
