...
contract OperonOriginsMarketplace {
    ...
    //     if (royaltyAmount == 0) {
    //         royaltyAmount = defaultRoy * 10**uint256(decimal);
    //     }

    //     uint256 _adminfee = (offeringRegistry[_offeringId].price *
    //         serviceValue) / 10**uint256(20);

    //     uint256 roy = (offeringRegistry[_offeringId].price * royaltyAmount) /
    //         10**uint256(20);

    //     uint256 netamount = offeringRegistry[_offeringId].price - roy;
    ...
    //         tokenContract.transfer(maintainer, _adminfee);
