...
contract MerkleAirdrop is Ownable {
    ...
    mapping(uint256 => uint256) public trancheBalance;
    ...
    constructor(ERC20 _token) {
        ...
        TOKEN_DECIMALS = 10**_token.decimals();
    }
        ...
        returns (uint256 trancheId)
    {
        ...
        trancheBalance[trancheId] = _totalAllocation;
        tranches = tranches.add(1);

        token.safeTransferFrom(
            msg.sender,
            address(this),
            _totalAllocation * TOKEN_DECIMALS
        );
    ...
    function verifyClaim(
        ...
        return _verifyClaim(_LP, _tranche, _balance, _merkleProof);
    }
    ...
    function _claim(
        ...
        require(
            token.balanceOf(address(this)) >= _balance * TOKEN_DECIMALS,
            "LP insufficient"
        );
        require(
            _verifyClaim(msg.sender, _tranche, _balance, _merkleProof),
    ...
    function _verifyClaim(
        ...
        bytes32 leaf = keccak256(abi.encodePacked(_tranche, _LP, _balance));
        valid = MerkleProof.verify(_merkleProof, merkleRoots[_tranche], leaf);
    }

    function _disburse(uint256 _balance, uint256 _trancheId) private {
        require(_balance > 0);
        require(trancheBalance[_trancheId] >= _balance);

        trancheBalance[_trancheId] -= _balance;
        token.safeTransfer(msg.sender, _balance * TOKEN_DECIMALS);
    }

    function devClaim(uint256 _trancheId) external onlyOwner {
        require(token.balanceOf(address(this)) >= trancheBalance[_trancheId]);
        require(trancheBalance[_trancheId] > 0);
        trancheBalance[_trancheId] = 0;
        merkleRoots[_trancheId] = bytes32(0);
        token.safeTransfer(devWallet, trancheBalance[_trancheId]);
    }
