...
contract TokenRewardRate {

    uint[] internal rates = [10,10,10,10,10,10,10,10,10,10,7,7,7,7,6,6,6,6,6,6,6,6,6,6,6,6,7,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,7,6,6,7,6,6,6,
            6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,7,6,6,7,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,6,10,6,6];
    function removeIteamInArray(uint index) internal returns(bool) {
        require(index >= rates.length);

        for (uint i = index; i<rates.length-1; i++){
            rates[i] = rates[i+1];
        }
        delete rates[rates.length-1];
        return true;
    }

    function setRate(uint[] memory _rates) public {
...
contract UniverseNFT is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable, TokenRewardRate {
    ...
    uint256 public constant LIMIT_COUNTER = 2;
    uint256 public PRICE = 50; //TODO: update price
    address public marketingAddress = 0x8A1eAA7f43D44D06ac1b7677FD6B979538EBc652;
    ...
    modifier saleIsOpen(address _address) {

        require(_totalSupply() <= MAX_ELEMENTS, "Sale end");
        require(balanceOf(_address) < LIMIT_COUNTER, "You can only have 1 NFT");
        _;
    }
    ...
    function setWhitedList(address[] memory row) external onlyOwner {
        require(row.length<101, "Input array length have to be less than 100.");
        require(_whitedList.length <= 100, "100 limit");
        for (uint i=0; i < row.length; i++) {
            _whitedList.push(row[i]);
            if(_whitedList.length > 100){
                break;
            }
        }
    }
    ...
    function mint(address _to) public onlyWhitedListMemeber saleIsOpen(_msgSender()) {
        require(getAllowance(msg.sender) >= price(), "Your amount is low than price");
        require(token.balanceOf(msg.sender) >= price(), "Your amount is low than price");
    ...
    function upgradToken(uint256 _id) external onlyOwner {  //TODO: update furture;
        require(rates[_id] <= 8, "maximum upgrade");
        _burn(_id);
        _tokenIdTracker.decrement();
        removeIteamInArray(_id);
        _mint(ownerOf(_id), _id.add(MAX_ELEMENTS));
    }

    function rewardAmount(uint256 _rate, uint256 _amount) internal view returns(uint256) {
        uint256 _rewardAmount = _rate.mul(10).mul(_amount).div(totalSupply());  //_rate.mul(1000).div(100).div(_totalSupply())
