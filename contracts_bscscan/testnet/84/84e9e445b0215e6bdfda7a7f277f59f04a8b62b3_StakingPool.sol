...
contract LiquidifyHelper is Ownable {
    ...
    constructor() {
        _swapRouter = IUniswapV2Router02(
            address(0xBeD1633949Be8997B17cBFd3D418dB5EaA8FA55A)
        );
    }
...
contract StakingPool is Ownable, ReentrancyGuard {
    ...
    function handleDeposits(
        ...
        if (_from != _referrer && _referrer != address(0)) {
            UserInfo storage user = userInfo[_from];
            if (user.referrer != _referrer) {
            ...
            if (uplineAmount > 0) {
                stakedToken.safeTransfer(_referrer, uplineAmount);
        ...
        if (buybackAmount > 0) {
            swapStakeTokenForRewardToken(buybackAmount, DEAD);
