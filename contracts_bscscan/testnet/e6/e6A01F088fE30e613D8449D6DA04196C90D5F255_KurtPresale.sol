...
contract KurtPresale {
    ...
    uint256 public totalTokensToSell =  5000000 * 10**18;          // 81000000000 WFT tokens for sell
    uint256 public WFTPerBnb = 30000 * 10**18;             // 1 BNB = 30000000 WFT
    ...
    mapping(address => uint256) public WFTPerAddresses;
    ...
    function buyWithBNB(uint256 buyAmount) public payable checkSaleRequirements(buyAmount) {
        ...
        uint256 sumSoFar = WFTPerAddresses[msg.sender].add(buyAmount);
        require(sumSoFar <= maxPerUser, 'Greater than the maximum purchase limit');

        WFTPerAddresses[msg.sender] = sumSoFar;
        totalSold = totalSold.add(buyAmount);
        
        WFT.transfer(msg.sender, buyAmount);
    ...
    function setTokenPricePerBNB(uint256 _WFTPerBnb) public {
        require(msg.sender == owner);
        require(_WFTPerBnb > 0, "Invalid WFT price per BNB");
        WFTPerBnb = _WFTPerBnb;
    }
    ...
    function endSale() public {
        ...
        uint256 tokenAmount = WFT.balanceOf(address(this));
    ...
    function withdrawUnsoldTokens() public {
        ...
        WFT.transfer(owner, remainedTokens);
    }
    ...
    function unsoldTokens() public view returns (uint256) {
        ...
        return WFT.balanceOf(address(this));
    }
    ...
    function calculateWFTAmount(uint256 bnbAmount) public view returns (uint256) {
        uint256 WFTAmount = WFTPerBnb.mul(bnbAmount).div(10**18);
        return WFTAmount;
    }

    function calculateBNBAmount(uint256 WFTAmount) public view returns (uint256) {
        require(WFTPerBnb > 0, "WFT price per BNB should be greater than 0");
        uint256 bnbAmount = WFTAmount.mul(10**18).div(WFTPerBnb);
        return bnbAmount;
    }
    ...
    function setRouter(address _router) public {
        ...
        if (IUniswapV2Factory(_newPancakeRouter.factory()).getPair(address(WFT), _newPancakeRouter.WETH()) == address(0)) {
            uniswapV2Pair = IUniswapV2Factory(_newPancakeRouter.factory()).createPair(address(WFT), _newPancakeRouter.WETH());
        } else {
            uniswapV2Pair = IUniswapV2Factory(_newPancakeRouter.factory()).getPair(address(WFT), _newPancakeRouter.WETH());
        }
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        WFT.approve(address(uniswapV2Router), tokenAmount);
        uint256 allowance = WFT.allowance(address(this), address(uniswapV2Router));
