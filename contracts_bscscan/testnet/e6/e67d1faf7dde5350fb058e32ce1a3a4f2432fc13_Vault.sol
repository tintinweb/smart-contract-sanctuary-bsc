 ...
 */
  ...
  function claim(
    ...
    (bool isExists, ) = LacTokenUtils.isNumberExists(
      fundReceiversList,
      _receiverId
    );
    ...
    require(
      _amount > 0 &&
        _amount <= fundReceivers[_receiverId].totalAccumulatedFunds,
      "Vault: INSUFFICIENT_AMOUNT"
    );
    require(
      _verify(
        _hash(_amount, _receiverId, userNonce[msg.sender], _referenceNumber),
        _signature
      ),
   ...
   * @param _shares - indicates the list of shares of respective receivers
   */
  function addFundReceivers(
    string[] memory _fundReceivers,
   ...
   */
  function removeFundReceiver(uint256 _receiverId)
    external
    virtual
    onlyAdmin
    whenPaused
  ...
  function updateVaultParams(
    ...
    whenPaused
    onlyValidReleaseRates(
      _newInitialReleaseRate,
      _newfinalReleaseRate,
      _newPercentage
    )
  {
    ...
    onlyAdmin
  {
    ...
    uint256 tokenAmount = IERC20Upgradeable(_tokenAddress).balanceOf(
      address(this)
    );
   ...
   */
  function getFundReceiverShare(uint256 _receiver)
    public
    view
    virtual
    returns (uint256)
    ...
    )
  {
    ...
    if (_isPeriodCompleted()) {
      ...
      // get correct release rate according to periods
      (
        _currentReleaseRatePerPeriod,
        _currentReleaseRatePerBlock
      ) = _getReleaseRateValues(int256(currentReleaseRatePerPeriod));
      ...
      if (totalPeriodsCompleted > 0) {
        ...
        do {
          ...
          (
            _currentReleaseRatePerPeriod,
            _currentReleaseRatePerBlock
          ) = _getReleaseRateValues(int256(currentPerPeriodRate));
    ...
    returns (uint256 perPeriodReleaseRate, uint256 perBlockReleaseRate)
  {
    ...
    int256 increaseAmount = (_currentPerPeriodReleaseRate * changePercentage) /
      10000;

    if (increaseAmount < 0) {
      if (
        (_currentPerPeriodReleaseRate + increaseAmount) <
        int256(finalReleaseRatePerPeriod)
      ) {
