...
contract DragonPool is DateWrapper {
    ...
    function stake(uint256 usdtAmoiunt, uint256 tokenAmount)
        ...
        checkStart
    {
        require(usdtAmoiunt >= minStakeUsdt, "not usdt amount");

        uint256[] memory price = router.getAmountsOut(tokenAmount, tokenPath);
        require(usdtAmoiunt >= price[0], "not usdt amount");
    ...
    function getReward() public updateReward(msg.sender) checkHalve checkStart {
        ...
        uint256[] memory price = router.getAmountsOut(reward, dbrPath);
        uint256 rewardMax = userRewardMax[msg.sender].sub(
            userAleaReward[msg.sender]
        );
        if (price[0] > rewardMax) {
            reward = router.getAmountsIn(rewardMax, dbrPath)[0];
            price[0] = rewardMax;
        }
        userAleaReward[msg.sender] = userAleaReward[msg.sender].add(price[0]);
        _balances[msg.sender] = _balances[msg.sender].sub(price[0].div(3));

        rewards[msg.sender] = 0;
        dbr.safeTransfer(msg.sender, reward);
    ...
    function getActiveReward() public {
        ...
        uint256[] memory price = router.getAmountsOut(active, dbrPath);
        uint256 rewardMax = userRewardMax[msg.sender].sub(
            userAleaReward[msg.sender]
        );
        if (price[0] > rewardMax) {
            active = router.getAmountsIn(rewardMax, dbrPath)[0];
            price[0] = rewardMax;
        }
    ...
    //         address[] memory inviters = userMap[_account];
