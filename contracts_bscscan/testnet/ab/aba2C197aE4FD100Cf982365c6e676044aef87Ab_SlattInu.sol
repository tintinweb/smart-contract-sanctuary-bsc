...
contract SlattInu is ERC20, ERC20Burnable, AccessControl {
    ...
    constructor(address _router, address _treasury,uint256 _treasuryTax,address _team,uint256 _teamTax) ERC20("SlattInu", "SLATT") {
        treasury = _treasury;
        treasuryTax = _treasuryTax;
        team = _team;
        teamTax = _teamTax;

        _mint(_msgSender(), 108679 * (10**uint256(18)));
    ...
    function setState(bool newState) public virtual onlyRole(OWNER_ROLE){
        state = newState;
    }


    function isTaxed(address wallet) public view returns (bool){
        return _isTaxed[wallet];
    }

    function _taxTransfer(address sender,address recipient,uint256 amount) internal returns (uint256) {
