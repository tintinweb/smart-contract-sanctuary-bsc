...
contract BnbBettingContract is Ownable {
    ...
    uint256 public feeRemainder = 94;
    uint256 public potOfFees = 0;
    uint256 public stakingPot = 0;
    ...
    function placeBet(uint256 teamToBetOn) public payable {  
        ...
        // Make sure the user is not changing who he is placing bet on, or has to pay a fee
        if (roundUserBettedOn[msg.sender] == currentRound) {
            if (teamToBetOn != teamUserBettedOn[currentRound][msg.sender]) {
                uint256 changingFee = amountToDistribute.mul(10).div(100);
                amountToBet = amountToDistribute.mul(90).div(100);
    ...
    function takeOutBet(uint256 amount) public payable{
        ...
        getPayment(msg.sender, welchersFee);
    ...
    function amountBetOnTeam(uint256 team, uint256 round) public view returns (uint256) {
    ...
    function payToSoapStakingContract() public{
        if(stakingPot >= 100000000000000000){
    ...
    function payToDevWallet() public{
        if(potOfFees >= 100000000000000000) {
