...
// Single Mindfullness token

contract SingleMindfullness is ERC721 {
    ...
    constructor() ERC721("SingleMindfullness","SingleMindfullness") {
        factory = msg.sender;
    }
...
// Single Mindfullness Fixed Price Marketplace contract

interface ISingleMindfullness {
...
contract SingleMindfullnessFixed is Ownable, ERC721Holder {
    using SafeMath for uint256;
	using EnumerableSet for EnumerableSet.AddressSet;
	...
	function createCollection(string memory _name, string memory _uri, bool bPublic) public returns(address collection) {
		bytes memory bytecode = type(SingleMindfullness).creationCode;
        ...
        ISingleMindfullness(collection).initialize(_name, _uri, msg.sender, bPublic);
    ...
    function singleList(address _collection, uint256 _tokenId, address _tokenAdr, uint256 _price) OnlyItemOwner(_collection,_tokenId) public {
		...
		ISingleMindfullness nft = ISingleMindfullness(_collection);        
    ...
    function singleDelist(uint256 _id) external {        
        ...
        ISingleMindfullness(pairs[_id].collection).safeTransferFrom(address(this), msg.sender, pairs[_id].tokenId);        
    ...
    function singleBuy(uint256 _id) external payable {
		...
		ISingleMindfullness(pairs[_id].collection).safeTransferFrom(address(this), msg.sender, pair.tokenId);
	...
	modifier OnlyItemOwner(address tokenAddress, uint256 tokenId){
        ISingleMindfullness tokenContract = ISingleMindfullness(tokenAddress);
