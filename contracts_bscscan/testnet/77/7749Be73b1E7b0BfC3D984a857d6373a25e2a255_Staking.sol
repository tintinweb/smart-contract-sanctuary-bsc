...
contract Staking is IStaking, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function addPool(
        ...
        pool.lastBlockWithReward = _stakingLimit.add(_blocksAmount).sub(1);
        ...
        updateReward(_pid, msg.sender)
    {
        ...
        require(block.number < pool.lastBlockWithReward, "Stake: staking  period is over");

        pool.totalStaked = pool.totalStaked.add(_amount);
        pool.staked[msg.sender] = pool.staked[msg.sender].add(_amount);
    ...
    function blocksWithRewardsPassed(uint256 _pid) public view override returns (uint256) {
        ...
        uint256 to = MathUpgradeable.min(block.number, pool.lastBlockWithReward);
    ...
    function rewardPerToken(uint256 _pid) public view override returns (uint256) {
        ...
        uint256 accumulatedReward = blocksWithRewardsPassed(pid).mul(pool.rewardPerBlock).mul(1e18).div(
            pool.totalStaked
        );
    ...
    function _calculateBlocksLeft(uint256 _pid) internal view returns (uint256) {
        ...
        uint256 _to = pool.lastBlockWithReward;
