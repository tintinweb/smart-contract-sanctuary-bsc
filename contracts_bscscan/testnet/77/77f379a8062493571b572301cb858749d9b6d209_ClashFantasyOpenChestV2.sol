...
interface IClashFantasyCards {
    ...
    // function mint(address to,uint256[] memory ids,uint256[] memory amounts, uint hasEnergy) external;
}
...
contract ClashFantasyOpenChestV2 is Initializable, OwnableUpgradeable {
    ...
    // mapping(address => mapping(uint256 =>  UserCard[])) public _userCards;
        ...
        public
    {
        ...
        // if(arrayOpenChest[_tokenId].exists == false) {
        ...
        if(validatePreSale == true) {
            ...
            openChestWithoutPresale(_tokenId, aditionalId );
        }
    }

    function openChestWithoutPresale(uint256 _tokenId, uint256 aditionalId) 
        public
    { 
        ...
        uint256 choosen = returnRandomChestOpen(_tokenId, aditionalId, percentage);

        uint256[] memory _ids = new uint256[](amount[choosen]);
        uint256[] memory _amounts = new uint256[](amount[choosen]);
        for (uint256 i = 0; i < 6; i++) {
            uint256 card_choosen = returnRandomCards(i, aditionalId, cards[_typeOf].length );
            
            if(i < amount[choosen]) {
                _ids[i] = card_choosen;
                _amounts[i] = amount[choosen];
            }
        }
        contractCards.mint(msg.sender, _amounts, _ids, hasEnergy, _typeOf);
        contractBase.burn(msg.sender, _tokenId, 1 , aditionalId);

        // arrayOpenChest[_tokenId].delivered = true;
    }

    function testOpen(uint256 _tokenId, uint256 aditionalId) 
        public
        view
    ...
    { 
        (uint256 hasEnergy,) = contractBaseChest.getChestHasEnergyByChestId(aditionalId);
        uint256 _typeOf = getTypeOf(aditionalId);
        ( uint256[] memory percentage,) = getLootChestArray(_typeOf);
        uint256 choosen = returnRandomChestOpen(_tokenId, aditionalId, percentage);
        return(hasEnergy, _typeOf, choosen, percentage);
    }
        ...
        returns(uint256)
    {
        ...
        uint purchasenumber = uint8(uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty, _tokenId, _aditional, msg.sender)))%100);
    ...
    function returnRandomCards(uint _tokenId, uint _aditional, uint256 amountCards) 
        ...
        returns(uint256)
    {
        uint256 choose =  uint8(uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty, _tokenId, _aditional, msg.sender)))% amountCards);
        return choose;
    }
