...
contract CocktailNFT is ERC721, OwnableAndWhitelistble {
    ...
    constructor() ERC721(_name, _symbol) {
        ...
        setSpecialCocktailInfo('Gold', 5, 50, 100, 0);
        setSpecialCocktailInfo('Silver', 5, 50, 100, 0);
        setSpecialCocktailInfo('Bronze', 5, 50, 100, 0);
    }
...
contract Lottery is VRFConsumerBase, Ownable {
    ...
    function convert(uint256 inStable) public view returns (uint256) {
        ...
        // // return tokenConverter.convertChained(path, inStable);
        // return router.getAmountsOut(inStable, path)[2];
        return inStable * 1_000;
    }
    ...
    function spin(uint256[] memory tokenIds) external payable {
        ...
        if (!game.claimed && game.bet > 0) {
            payable(msg.sender).transfer(msg.value);
            claimReward();
            return;
        }

        if (msg.value > 0) _feeManagment();
    ...
    // TODO: replace public on internal
    function _getRandomNumber() public returns (bytes32) {
        require(enough(), 'Not enough LINK');
