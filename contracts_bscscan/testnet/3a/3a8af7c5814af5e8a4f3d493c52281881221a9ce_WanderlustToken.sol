...
// 2% TOKENS SENT TO wallet X ( Stake conttract ) ----------------------- GOOD UHHHHH
...
// 3% Tokens sent to wallet X ( stake contract ) ----------------------- GOOD UHHHHH
...
contract WanderlustToken is Context, IERC20, Ownable {
    ...
    address payable public stakeAddress =
        payable(0x000000000000000000000000000000000000dEaD); // Stake Address
    ...
    uint256 public _marketingFee = 2; // CONFIRMED?
    uint256 private _previousMarketingFee = _marketingFee;

    uint256 public _stakeFee = 2; // CONFIRMED?
    ...
    //     require(!_isExcluded[account], "Account is already whitelisted");
    //     // if (_rOwned[account] > 0) {
    //     //     _tOwned[account] = tokenFromReflection(_rOwned[account]);
    //     // }
    ...
    //     require(_isExcluded[account], "Account is not whitelisted");
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 tbal = transferredBalance.mul(marketingDivisor).div(100);
        uint256 mt = tbal.mul(_marketingFee).div(100);
        uint256 st = tbal - mt; // Essentially the rest of the fee which should correspond to _stakeFee
        ...
        // Send to Stake address
        transferToAddressETH(
            marketingAddress,
            st
        ); // FIXME: SEND SOME TO MARKETING ADDRESS, SEND SOME TO STAKE ADDRESS
    }
    ...
    function removeAllFee() private {
        if (_taxFee == 0 && _marketingFee == 0 && _stakeFee == 0 && _liquidityFee == 0) return;
