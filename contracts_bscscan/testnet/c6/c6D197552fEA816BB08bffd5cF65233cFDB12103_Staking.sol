...
/// @title Staking for non-fungible tokens.
...
contract Staking is Ownable {
 ...
 // whitelist admin with role
  mapping(address => bool) private _whitelistAdmin;
  ...
  // Min Amount token staking each time
  uint256 public minStakingAmount = 150 * 10**18;
  ...
  modifier onlyAdminOrOwner() {
    require(msg.sender == owner() || _whitelistAdmin[msg.sender] == true, "Owner has no permission to perform action");
    _;
  }
  ...
  function updateWhitelistAdmin (address _address, bool _isAdmin) external onlyOwner {
    _whitelistAdmin[_address] = _isAdmin;
  }
  ...
  /// @dev Update min amount staking 
  function updateMinStakingAmount(uint256 _amount) external onlyOwner {
  ...
  /// @dev Staking 
  function stake(uint256 _amount) external returns(address) {
    require(block.timestamp >= startStakingAt && block.timestamp <=  endStakingAt, "Staking is disabled");
    require(_amount >= minStakingAmount, "amount must be greater than minStakingAmount");
  ...
  function getEstimatedInterest(uint256 _id)
    public
    view
    onlyStakeholder(_id)
    returns (uint256)
  {
    ...
    uint256 currentInterest = stakeAmount.mul(currentStakeDuration).mul(rate).div(term).div(
      10000
    );
    ...
    returns (uint256)
  {
    ...
    return idToBalance[_id].mul(idToInterestRate[_id]).div(1000);
  }
  ...
  function getTotalStakeholders() public view
    returns (address[] memory, uint256[] memory)
  {
    address[] memory stakeholders = new address[]( _addresses.length);
    uint256[] memory totalStaked = new uint[]( _addresses.length);
    for (uint256 i = 0; i < _addresses.length; ++i) {
      uint256 staked = 0;
      uint256[] memory ids = _addressToIds[_addresses[i]];
      for (uint256 j = 0; j < ids.length; ++j) {
