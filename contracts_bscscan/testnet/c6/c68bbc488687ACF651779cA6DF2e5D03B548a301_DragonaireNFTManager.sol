    ...
    ItemFactoryManager
{
    ...
    address private itemFactory_ = 0x883427733fD8C30C6619cFED646d3cFEa5E3a1FB;
    address private randomGenerator_ = 0x9Fc5587e81fc720162Cb840Ad5ef68320cddb8C8;
    uint256 private randomFee_ = 100000000000000000;
    ...
    {
        nftCore = new DragonaireNFTCore();

        _supportedBoxTypes.add(1); // 属性1
        _supportedBoxTypes.add(2); // 属性2
        _supportedBoxTypes.add(3); // 属性3
        _supportedBoxTypes.add(4); // 属性4

        _addComboType(101); // 
        _addBoxTypeToCombo(101, 1); // 属性1的属性值101
        _addBoxTypeToCombo(101, 2); // 属性2的属性值101
        _addBoxTypeToCombo(101, 3); // 属性3的属性值101

        _addComboType(102); // Combo characters, cars
        _addBoxTypeToCombo(102, 1); // 属性1的属性值102
        _addBoxTypeToCombo(102, 2); // 属性2的属性值102
    ...
    function updateItemFactory(address _itemFactory_) public onlyRole(CONTRACT_UPGRADER) {
        require(
            _itemFactory_ != address(0),
    ...
    function updateRandomGenerator(address _randomGenerator_) public onlyRole(CONTRACT_UPGRADER) {
        require(
            _randomGenerator_ != address(0),
            "DragonaireNFT: randomGenerator_ is the zero address"
        );

        _updateRandomGenerator(_randomGenerator_);
    }

    function updateRandomFee(uint256 _randomFee_) public onlyRole(MANAGER_ROLE) {
        _updateRandomFee(_randomFee_);
    }
    ...
    function mintBatch(address to_, uint256 boxType_, uint256 amount_) external 
        onlyRole(MINTER_ROLE) onlySupportedBoxType(boxType_) {
        require(amount_ > 0, "DragonaireNFT: amount_ is 0");
        require(
    ...
    function unbox(uint256 tokenId_) public payable
        onlyExistedToken(tokenId_) onlyTokenOwner(tokenId_) onlyMysteryBox(tokenId_) {
