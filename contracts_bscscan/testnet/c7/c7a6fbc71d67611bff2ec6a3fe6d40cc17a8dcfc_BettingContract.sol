...
contract BettingContract is Ownable {
    ...
    address public soapAddress = 0x7cf54bcE8A0434ad635B80E85F6739E86ECD2C9D;
 ...
 // Distribute is useless, unless someone sends BNB directly to the contract, which is not meant to happen
    ...
    function placeBet(uint256 amount, uint256 teamToBetOn) public {  
        ...
        if (roundUserBettedOn[msg.sender] == currentRound) {
            if (teamToBetOn != teamUserBettedOn[currentRound][msg.sender]) {
                amountToBet = amount.mul(90).div(100);
                uint256 changingFee = amount.mul(10).div(100);
                soapContr.feelessTransfer(msg.sender, owner(), changingFee.mul(MULTIPLIER));
            }
        }
        // Contract transfers tokens from user to itself
        soapContr.feelessTransfer(msg.sender, address(this), amountToBet.mul(MULTIPLIER));
    ...
    function takeOutBet(uint256 amount) public {
        ...
        soapContr.feelessTransfer(address(this), owner(), welchersFee.mul(MULTIPLIER));
        soapContr.feelessTransfer(address(this), msg.sender, amountToTakeOut.mul(MULTIPLIER));
    ...
    function pullBetWithFailsafe() public {
        ...
        soapContr.feelessTransfer(address(this), msg.sender, amountUserPutIn[currentRound][msg.sender].mul(MULTIPLIER));
    ...
    function getSoap() public { //this function is only for test purposes
        ...
        soapContr.feelessTransfer(address(this), msg.sender, 10000000 * MULTIPLIER);
    ...
    function withdrawWinnings(uint256 round) public {
        ...
        if(winnings > 0) {
            soapContr.feelessTransfer(address(this), msg.sender, winnings.mul(MULTIPLIER));
        }
