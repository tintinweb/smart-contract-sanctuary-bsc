            ...
            if (
                ...
                if (transferDelayEnabled) {
                    if (to != owner() && to != address(uniswapV2Router) && to != address(uniswapV2Pair)) {
                        require(_holderLastTransferTimestamp[tx.origin] < block.number * intervalDelayInterval, "_transfer:: Transfer Delay enabled.  Only one purchase per block allowed.");
...
function doBuyBack() private lockTheSwap {
        ...
        uint256 totalTokensToSwap = _liquidityTokensToSwap + _buybackAndBurnToSwap;
        ...
        // El token con el que se respalda liquidez
        swapTokensForAVAX(amountToSwapForAVAX);
        uint256 avaxBalance = address(this).balance.sub(initialAVAXBalance);

        uint256 avaxForBuybackAndBurnPool = avaxBalance.mul(_buybackAndBurnToSwap).div(totalTokensToSwap);
        uint256 avaxForLiquidity = avaxBalance - avaxForBuybackAndBurnPool;

        _liquidityTokensToSwap = 0;
        _buybackAndBurnToSwap = 0;

         payable(buybackAndBurnAddress).transfer(avaxForBuybackAndBurnPool);

        if (tokensForLiquidity > 0 && avaxForLiquidity > 0) {
            swapAndSendToAddress(liquidityAddress, tokensForLiquidity);
            payable(liquidityAddress).transfer(avaxForLiquidity);
        }
    }
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        ...
        if (buyOrSellSwitch == BUY) {
            ...
            _buybackAndBurnToSwap += tLiquidity * _buyBuybackAndBurnFee / _liquidityFee;
        } else if (buyOrSellSwitch == SELL) {
            _liquidityTokensToSwap += tLiquidity * _sellLiquidityFee / _liquidityFee;
            _buybackAndBurnToSwap += tLiquidity * _sellBuybackAndBurnFee / _liquidityFee;
        }
    ...
    function calculateDistributionHoldersFee(uint256 _amount) private view returns (uint256) {
    ...
    function setBuyFee(uint256 buyDistributionHoldersFee, uint256 buyLiquidityFee, uint256 buyBuybackAndBurnFee)
    external
    onlyOwner
    {
        require(_buyDistributionHoldersFee + _buyLiquidityFee + _buyBuybackAndBurnFee <= 30, "Must keep buy taxes below 3%");

        _buyDistributionHoldersFee = buyDistributionHoldersFee;
        _buyLiquidityFee = buyLiquidityFee;
        _buyBuybackAndBurnFee = buyBuybackAndBurnFee;

        emit SetBuyFee(buyDistributionHoldersFee, buyLiquidityFee, buyBuybackAndBurnFee);
    }

    function setSellFee(uint256 sellDistributionHoldersFee, uint256 sellLiquidityFee, uint256 sellBuybackAndBurnFee)
    external
    onlyOwner
    {
        require(_sellDistributionHoldersFee + _sellLiquidityFee + _sellBuybackAndBurnFee <= 30, "Must keep buy taxes below 3%");
        
        _sellDistributionHoldersFee = sellDistributionHoldersFee;
        _sellLiquidityFee = sellLiquidityFee;
        _sellBuybackAndBurnFee = sellBuybackAndBurnFee;

        emit SetSellFee(sellBuybackAndBurnFee, sellLiquidityFee, sellDistributionHoldersFee);
    }
    ...
    function setBuybackAndBurnAddress(address _buybackAndBurnAddress) public onlyOwner {
        require(_buybackAndBurnAddress != address(0), "_buybackAndBurnAddress address cannot be 0");
    ...
    function updateSwapLiquidityTokensThreshold(uint256 value) external onlyOwner {
    ...
    function launch(address routerAddress, address busdAddress) external onlyOwner {
        ...
        removeAllFee();
            ...
            addLiquidity(address(this),busdAddress, balanceOf(address(this)),balanceOf(busdAddress),0,0,address(uniswapV2Router));
        restoreAllFee();
    ...
    function addLiquidity(address tokenA, address tokenB, uint256 amountADesired, uint256 amountBDesired, uint256 amountAMin, uint256 amountBMin, address to) private {
