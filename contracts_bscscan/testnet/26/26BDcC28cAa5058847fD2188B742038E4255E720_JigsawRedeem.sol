...
import "./MooJigsaw.sol";

contract JigsawRedeem is Context, Ownable, Pausable {
  using SafeMath for uint256;

  string public name;
  MooJigsaw public jigsaw;
  uint256 public resultId;

  uint256[] public ids;
  uint256[] public amounts;
  uint256 public totalRedeem;

  event JigsawRedeemed(address user, uint256 totalRedeem);
  ...
  modifier onlyEOA() {
    require(tx.origin == _msgSender(), "JigsawRedeem: onlyEOA");
    _;
  }
  ...
  function redeem() external onlyEOA whenNotPaused {
    jigsaw.burnBatch(_msgSender(), ids, amounts);
    jigsaw.mint(_msgSender(), resultId, 1, "0x");
    totalRedeem += 1;

    emit JigsawRedeemed(_msgSender(), totalRedeem);
  }
}
