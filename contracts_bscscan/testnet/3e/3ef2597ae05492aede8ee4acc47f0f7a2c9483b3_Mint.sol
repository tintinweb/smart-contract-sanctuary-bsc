...
contract Mint is Initializable, AccessControlUpgradeable {
    ...
    function mintMetaLoot(string calldata _domain, string calldata _node) external payable {
        metaLootName.mintRoute{value: msg.value}(_domain, _node, _msgSender());
    }
    ...
    function mintLand(string calldata _domain) external {
        return landName.mintRoute(_domain, _msgSender());
    }

    function mintNode(string calldata _domain) external {
        return mintNodeName.mintRoute(_domain, _msgSender());
    }

    function  tokenURIByDomainOfLand(string memory _domain) public view returns (string memory) {
        return landName.tokenURIByDomain(_domain);
    }
    ...
    function  tokenURIByDomainOfNode(string memory _domain) public view returns (string memory) {
        return mintNodeName.tokenURIByDomain(_domain);
    }

    function  tokenURIByDomainOfMetaLoot(string memory _domain) public view returns (string memory) {
        return metaLootName.tokenURIByDomain(_domain);
    }
}
...
contract SketchURI is Initializable, AccessControlUpgradeable {
    ...
    function getFullName(uint256 tokenId) public view returns (string memory fullName) {
        return string(abi.encodePacked(names[getRandMod("names", tokenId, 31)], '.', lands[getRandMod("lands", tokenId, 4)]));
    }
    ...
    function image(uint256 tokenId) external view returns (string memory) {
        ...
        parts[2] = string(abi.encodePacked('<text x="138" y="22" font-weight="bold" font-size="16" text-anchor="middle" fill="white">', getFullName(tokenId), '</text>'));
...
contract MintNodeName is ReentrancyGuardUpgradeable, AccessControlUpgradeable, BaseRegistrarImplementation{
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "', names[tokenId % 31], ".node", '", "description": "meta ID is.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
    ...
    function current(uint256 _tokenId) public view returns (uint256) {
        return _metaLootCounter[_tokenId]._value;
    }
...
contract MetaLootName is ReentrancyGuardUpgradeable, AccessControlUpgradeable, BaseRegistrarImplementation {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "', skin.getFullName(tokenId), '", "description": "meta loot is.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
        output = string(abi.encodePacked('data:application/json;base64,', json));
    ...
    function mint(string calldata _domain, string calldata _node) public payable {
        ...
        uint256 _currentPrice = price(totalSupply());
        require(msg.value >= _currentPrice, "Not enough payments"); 

        transfer(alchemistDAO, _currentPrice * 30 / 100);
        transfer(lootLp, _currentPrice * 20 / 100);
        uint256 nodeTokenId = uint256(getSubNode(_node));
        address mintNodeOwner = _mintNode.ownerOf(nodeTokenId);
        payable(mintNodeOwner).transfer(_currentPrice * 45 / 100);
        ...
        payable(landOwner).transfer(_currentPrice * 5 / 100);
    ...
    function _create(string calldata _domain, string calldata _node) internal nonReentrant {
        ...
        _landName.increment(landTokenId);
        _mintNode.increment(nodeTokenId);
    }
    ...
    function mintRoute(string calldata _domain, string calldata _node, address owner) public payable {
        ...
        payable(landOwner).transfer(_currentPrice * 5 / 100);
        if(msg.value > _currentPrice){
            payable(owner).transfer(msg.value - _currentPrice);
        }
...
contract Alchemist is ReentrancyGuardUpgradeable, AccessControlUpgradeable, ERC721EnumerableUpgradeable {
    ...
    function setId() public nonReentrant {
        next_id = 1000;
    }
}
