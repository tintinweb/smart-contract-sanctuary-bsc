...
contract DONOTBUY is Context, IBEP20, Ownable {
    ...
    uint256 public numTokensSellToAddToLiquidity = _tTotal * 5 / 2000;                  // contract balance to trigger swap & liquify + wallet transfer.                                                                                                   
   ...
   function isExcludedFromMaxWallet(address account) public view returns(bool) {
    ...
    function _transfer(
        ...
        require(_isExcludedFromFees[from] || _isExcludedFromFees[to] || isTokenLive, "DNB is not live yet");
        //when buy
       if (automatedMarketMakerPairs[from] && !_isExcludedMaxTransactionAmount[to]) {
       require(amount <= maxTransactionAmount, "Buy transfer amount exceeds the maxTransactionAmount.");
     }
              //SORRY JEETS NOT ON OUR COMMUNITY ;)
     if (automatedMarketMakerPairs[from] && !_isExcludedMaxTransactionAmount[to]) {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // swap tokens for BNB
        swapTokensForBNB(liquifyHalves[0]); // <- this breaks the BNB -> DNB swap when swap+liquify is triggered
