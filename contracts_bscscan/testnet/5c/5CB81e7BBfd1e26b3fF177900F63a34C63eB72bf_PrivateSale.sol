...
abstract contract WhiteList is Auth {
...
contract PrivateSale is WhiteList {
  ...
  constructor(address _mambapad) WhiteList() {
  ...
  function withdrawPerMonth() public {
    ...
    require(
      investorInfos[msg.sender].cliff < block.timestamp,
    ...
    uint256 amountToReceivePerMonth = investorInfos[msg.sender].totalReceivableAmount.div(PercPerMonth).mul(
      percDenominator
    );
