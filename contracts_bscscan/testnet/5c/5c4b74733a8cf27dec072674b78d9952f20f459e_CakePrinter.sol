...
BSC Defi - CAKE Printer
  ...
  ~ 50 tickets max per user. 50 CAKE = 50 tickets.
  ~ 1 CAKE deposit = 1 per ticket. based on user deposit/compound.
  ...
  ~ 750 CAKE max withdrawal.
  ~ 3,750 CAKE max deposits per wallet.
...
contract CakePrinter {
    using SafeMath for uint256;

    IERC20 public token_CAKE;
	address erctoken = 0xF9f93cF501BFaDB6494589Cb4b4C15dE49E85D0e; /** mainnet CAKE **/
    ...
    uint256 public MIN_INVEST = 1 ether; /** 1 CAKE  **/
	uint256 public WITHDRAW_COOLDOWN = 12 * 60 * 60; /** 12 hours  **/
    uint256 public WITHDRAW_LIMIT = 750 ether; /** 750 CAKE  **/
    uint256 public WALLET_DEPOSIT_LIMIT = 3750 ether; /** 3,750 CAKE  **/
    ...
    function sellEggs() public{
        ...
        token_CAKE.transfer(msg.sender, eggsPayout);
    ...
    function buyEggs(address ref, uint256 amount) public payable{
        ...
        token_CAKE.transferFrom(address(msg.sender), address(this), amount);
        ...
        if (user.referrer != address(0)) {
            ...
            if (upline != address(0)) {
                /** referral rewards will be in CAKE **/
                ...
                token_CAKE.transfer(upline, refRewards);
    ...
    function payFees(uint256 eggValue) internal returns(uint256){
        ...
        token_CAKE.transfer(project, projectFee);
		token_CAKE.transfer(partner1, partnerFee);
		token_CAKE.transfer(partner2, partnerFee);
        token_CAKE.transfer(buyback, buybackFee);
    ...
    function payFeesSell(uint256 eggValue) internal returns(uint256){
        ...
        token_CAKE.transfer(project, prj);
		token_CAKE.transfer(marketing, mkt);
        token_CAKE.transfer(buyback, bbk);
   ...
   /** will auto execute, when condition is met. buy, hatch and sell, can be triggered manually by admin if theres no user action. **/
        ...
        if(participants != 0){
            ...
            for(uint256 i = 0; i < participants; i++){
                if((random >= init_range[i]) && (random <= end_range[i])){
                    ...
                    token_CAKE.transfer(buyback, back);
    ...
    function getBalance() public view returns (uint256) {
        return token_CAKE.balanceOf(address(this));
	}
