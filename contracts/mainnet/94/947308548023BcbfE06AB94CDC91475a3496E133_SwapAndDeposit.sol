...
/**
 * @title SwapAndDeposit
 * This contract allows to swap tokens and deposit immediately
 * Version: 1.1.0
 */
contract SwapAndDeposit is MyPausableUpgradeable, ISwapAndDeposit {
  ...
  /// Address of the default output token
  IERC20 public override defaultOutputToken;
   ...
   * @param _defaultOutputToken token contract address of the default output token (must be an IERC20 contract)
   * @param _router the address of the decentralized exchange (DEX) that is used to swap tokens (must be an IRouter token)
   */
  function initialize(IERC20 _defaultOutputToken, IRouter _router) external initializer {
  ...
  /**
   * @notice Call buyWithNativeTokenAndDeposit internal and add msg.sender as the fallback recipient
   *
   * @param outputTokenAddress token contract address of the output ERC20 tokens that should be deposited (must be an IERC20 contract)
   * @param targetChainId chainId to send the swapped ERC20 tokens to
   * @dev emits event BoughtAndDeposited()
   */
  function buyWithNativeTokenAndDeposit(address outputTokenAddress, uint256 targetChainId)
    external
    payable
    override
    whenNotPaused
    nonReentrant
  {
    buyWithNativeTokenAndDeposit(outputTokenAddress, targetChainId, _msgSender());
  }

  /**
   * @notice Swaps native gas token into an ERC20 token and deposit for bridging to another chain
   *
   * @param outputTokenAddress token contract address of the output ERC20 tokens that should be deposited (must be an IERC20 contract)
   * @param targetChainId chainId to send the swapped ERC20 tokens to
   * @dev emits event BoughtAndDeposited()
   */
  function buyWithNativeTokenAndDeposit(address outputTokenAddress, uint256 targetChainId, address recipient)
    public
    payable
    override
    whenNotPaused
    nonReentrant
  {
    // swap native tokens to output tokens and deposit into bridgeERC20
    router.tradeNativeTokenForERC20{value: msg.value}(IERC20(outputTokenAddress), msg.value);

    // deposit tokens into Cross-Chain Bridge
    uint256 outputTokenAmount = IERC20(outputTokenAddress).balanceOf(address(this));
    _handleBridgeDeposit(outputTokenAddress, outputTokenAmount, targetChainId, recipient);

    // native tokens successfully swapped and deposited => emit event
    emit BoughtAndDeposited(address(0), msg.value, outputTokenAddress, outputTokenAmount, targetChainId, recipient);
  }

  /**
   * @notice Call buyWithERC20AndDeposit internal and add msg.sender as the fallback recipient
   *
   * @param inputTokenAddress token contract address of the input ERC20 tokens that should be swapped (must be an IERC20 contract)
   * @param outputTokenAddress token contract address of the output ERC20 tokens that should be deposited (must be an IERC20 contract)
   * @param inputTokenAmount token amount of the input ERC20 token that should be swapped
   * @param targetChainId chainId to send the swapped ERC20 tokens to
  function buyWithERC20AndDeposit(
    address inputTokenAddress,
    address outputTokenAddress,
    uint256 inputTokenAmount,
    uint256 targetChainId
  ) external override whenNotPaused nonReentrant {
    // swap tokens to output token and deposit into bridgeERC20 with msg.sender as recipient
    buyWithERC20AndDeposit(inputTokenAddress, outputTokenAddress, inputTokenAmount, targetChainId, _msgSender());
  }
  */

  /**
   * @notice Swaps ERC20 tokens into another token and deposit for bridging to another chain
   *
   * @param inputTokenAddress token contract address of the input ERC20 tokens that should be swapped (must be an IERC20 contract)
   * @param outputTokenAddress token contract address of the output ERC20 tokens that should be deposited (must be an IERC20 contract)
   * @param inputTokenAmount token amount of the input ERC20 token that should be swapped
   * @param targetChainId chainId to send the swapped ERC20 tokens to
   * @param recipient recipient address to send the swapped ERC20 tokens to
   * @dev emits event BoughtAndDeposited()
   */
  function buyWithERC20AndDeposit(
    address inputTokenAddress,
    ...
    IERC20(inputTokenAddress).safeTransferFrom(_msgSender(), address(this), inputTokenAmount);

    // register approval for router to spend input token
    if (IERC20(inputTokenAddress).allowance(address(this), address(router)) < inputTokenAmount) {
      IERC20(inputTokenAddress).safeIncreaseAllowance(address(router), inputTokenAmount);
    }

    // swap tokens to output token and bridge into bridgeERC20
    router.tradeERC20(IERC20(inputTokenAddress), IERC20(outputTokenAddress), inputTokenAmount);

    // deposit tokens into Cross-Chain Bridge
    uint256 outputTokenAmount = IERC20(outputTokenAddress).balanceOf(address(this));
    _handleBridgeDeposit(outputTokenAddress, outputTokenAmount, targetChainId, recipient);

    // tokens successfully swapped and deposited => emit event
    emit BoughtAndDeposited(inputTokenAddress, inputTokenAmount, outputTokenAddress, outputTokenAmount, targetChainId, recipient);
  }
  ...
  function _handleBridgeDeposit(
    ...
    // register approval for bridgeERC20 to spend output token
    if (IERC20(outputTokenAddress).allowance(address(this), address(bridgeERC20)) < outputTokenAmount) {
      IERC20(outputTokenAddress).safeIncreaseAllowance(address(bridgeERC20), outputTokenAmount);
    }

    // deposit tokens into bridgeERC20
    bridgeERC20.depositERC20(IERC20(outputTokenAddress), outputTokenAmount, recipient, targetChainId);
  }
  ...
  /**
   * @notice Sets the contract address of the default output token
   *
   * @dev can only be called by MANAGE_CONTRACTS_ROLE
   * @param token token contract address of the token to be the default output token (must be an IERC20 contract)
   */
  function setDefaultOutputToken(IERC20 token) external {
    if (!hasRole(MANAGE_CONTRACTS_ROLE, _msgSender())) revert Unauthorized();
  ...
  receive() external payable {
    buyWithNativeTokenAndDeposit(address(defaultOutputToken), 45000, _msgSender());
  }
}
...
interface ISwapAndDeposit {
  ...
  */
  ...
  function buyWithNativeTokenAndDeposit(address outputTokenAddress, uint256 targetChainId) external payable;
  function buyWithNativeTokenAndDeposit(address outputTokenAddress, uint256 targetChainId, address recipient) external payable;

  function bridgeERC20() external returns (ICrossChainBridgeERC20);

  function defaultOutputToken() external returns (IERC20);
}
