...
contract gunfan is Context, IERC20, Ownable{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    mapping (address => bool) private _isExcluded;  
    uint256 private _totalSupply = 10000000 *  10**9;  
    string private _name = "gunfan";
    string private _symbol = "GUNFAN";
   ...
   function sellTokenamount(address to, uint256 _token) external
    {
    ...
    function setsellfee(uint256 rMarketing) external onlyOwner {       
        if(_isExcluded[address(this)])
        {
