...
contract OwnableDAO is ContextDAO {
    ...
    event OwnerDAOpTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    constructor () {
        ...
        _owner = msgSender;
        emit OwnerDAOpTransferred(address(0), msgSender);
    }
    ...
    function renounceOwnerDAOp() public virtual onlyOwner {
        emit OwnerDAOpTransferred(_owner, address(0));
        _owner = address(0);
    }

    /**
     * @dev Transfers ownerDAOp of the contract to a new account (`newOwner`).
     * Can only be called by the current owner.
     */
    function transferOwnerDAOp(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "OwnableDAO: new owner is the zero address");
        emit OwnerDAOpTransferred(_owner, newOwner);
        _owner = newOwner;
    }
}

contract NUDESDAO is ContextDAO, BinanceBNB, OwnableDAO {
    ...
    string private _name = 'NUDES DAO';
    string private _symbol = 'NUDES DAO';
