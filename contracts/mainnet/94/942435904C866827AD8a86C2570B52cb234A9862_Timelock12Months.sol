...
contract Timelock12Months {
    uint public constant LOCKPEROID = 180 days;
    uint public constant MONTHLOCK = 6;
    address admin;
    ...
    event OwnerAdded(address _newOwner);
    event OwnerRemoved(address _newOwner);
    ...
    function deposit(address _address, uint256 _amount) external {
        require(owners[msg.sender] == true, "Only owner can deposite token");
    ...
    /**
     * @dev withdraw release token that belong to msg.sender
     */
    ...
    function addOwner(address _newOwner) public returns (bool) {
        require(msg.sender == admin, "Only admin can add owner");
    ...
    function removeOwner(address _owner) public returns (bool) {
        require(msg.sender == admin, "Only admin can remove owner");
        owners[_owner] = false;
