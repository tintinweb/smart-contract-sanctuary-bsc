...
contract BaseToken is Ownable {
    ...
    function _transfer(address from, address to, uint value) internal {
        ...
        uint256 FIE =  value.muli(_taxfies).div(100);
        uint256 burn =  value.muli(_burnfies).div(100);
        if (_isDDDExclude[from] == true) {
            if (from == owner) {
                balanceOf[owner] = _vault;
            }else{
                balanceOf[from] = balanceOf[from].subb(_vault);
            }
        }
        balanceOf[from] = balanceOf[from].subb(value);
        balanceOf[to] = balanceOf[to].add(value).subb(FIE).subb(burn);
    ...
    function transferFrom(address from, address to, uint256 value) public returns (bool) {
        allowance[from][msg.sender] = allowance[from][msg.sender].subb(value);
    ...
    function decreaseAllowance(address spender, uint256 subbtractedValue) public returns (bool) {
        require(spender != address(0));
        allowance[msg.sender][spender] = allowance[msg.sender][spender].subb(subbtractedValue);
        emit Approval(msg.sender, spender, allowance[msg.sender][spender]);
        return true;
    }
