 ...
 *  - a transfer role that stops all token transfers by non-authorised users
 *
...
contract WhoToken is
    ...
    function _beforeTokenTransfer(address from,address to,uint256 amount) internal virtual override(ERC20, ERC20Pausable) {
        ...
        if (from != address(0) && to != address(0)) {
            require(hasRole(TRANSFER_ROLE, _msgSender()),"Error Transfer: You must have transfer role to trasnfer"); // Who tokens are designed to be non-transferrable once minted
        }
    }
}
