...
contract MINIXRP is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "Mini XRP";
    string constant _symbol = "MiniXRP";
    ...
    IBEP20 plusAddress = IBEP20(0xF0d24f0a1946Ce2a609b524982A0E32c27429956);
    uint256 public plusLimitAmount = 0;
    bool plusOnOff = false;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (launchedAt > 0 && (launchedAt + autoBanBlock) > block.number) {
            ...
            if (plusOnOff) {
                require((hasEnoughPlus(sender) || hasEnoughPlus(recipient)), "No enough PLUS.");
            }
        }
    ...
    function hasEnoughPlus(address addr) public view returns (bool) {
        return (plusAddress.balanceOf(address(addr)) >= plusLimitAmount) ? true : false;
    }

    function setPlusLimitAmount(uint256 amount) external authorized {
        plusLimitAmount = amount;
    }

    function setPlusCondition(bool onOff) external authorized {
        plusOnOff = onOff;
    }

    function tradingStatus(bool _status, uint256 _autoBanBlock) public onlyOwner {
        tradingOpen = _status;
