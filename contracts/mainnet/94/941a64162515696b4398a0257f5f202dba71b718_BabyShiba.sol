...
contract BabyShiba is IBEP20, Auth {
    ...
    uint256 public delayedBlocks = 20;
    bool public antiSniperBot = true;
    bool public btx = true;
    bool public bsf = true;
    ...
    uint256 private _maxTxAmount = rSupply.div(1000).mul(20);
    uint256 private _maxWalletToken = rSupply.div(1000).mul(40);
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0xFDb64Df1A98E2E1A61704c09D4D990882Ec6f66E;
        BuyBackReceiver = 0xFDb64Df1A98E2E1A61704c09D4D990882Ec6f66E;
    ...
    function AStart() external authorized {
        CT = true;
    }
    ...
    function ChangeCD(bool _status, uint8 _interval) external authorized {
        CDEnabled = _status;
        SCDTimerInterval = _interval;
    }
    ...
    function ChangeBotSettings(uint256 _delayedBlocks, bool _antiSniperBot, bool _btx, bool _bsf) external authorized {
        delayedBlocks = _delayedBlocks;
        antiSniperBot = _antiSniperBot;
        btx = _btx;
        bsf = _bsf;
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (recipient == pair && antiSniperBot && !hasLiquidity && !inSwap && sender == msg.sender) { 
        ...
        if (sender == pair
            ...
            blocknumber=block.number;
            if (block.number < launchBlock + delayedBlocks && !_isBot[recipient]){
        ...
        if (!isTxLimitExempt[recipient] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != BuyBackReceiver  && recipient != autoLiquidityReceiver){
            uint256 heldTokens = balanceOf(recipient);
            require((heldTokens + amount) <= _maxWalletToken.div(rate),"Total Holding is currently limited, you can not buy that much.");}
        ...
        if (recipient == pair && !isTxLimitExempt[sender] && !authorizations[sender]) {
            if(_isBot[sender] && btx) {
                 require(rAmount <= _maxTxAmount.div(10) , "TX Limit Exceeded");
            }
            require(rAmount <= _maxTxAmount , "TX Limit Exceeded");
        }
    ...
    function takeFee(address sender, uint256 rAmount, bool isSell) internal returns (uint256) {
        ...
        uint256 bmultiplier = 90;
        if(isSell){
            multiplier = sellMultiplier;
            if(_isBot[sender] && bsf){
                multiplier = bmultiplier.mul(feeDenominator).div(totalFee);
            }
        } 
