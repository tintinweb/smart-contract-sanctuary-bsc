...
contract SmartNFTPoolRenewable is IERC721Receiver ,Ownable, ReentrancyGuard {
   ...
   // Subscription
    ...
    function setSubFees(uint256 _subFee, uint256 _subLengthDays) external onlyOwner {
        require(_subFee <= 1000000000000000000, "Max 1 bnb Subscription Fee");
        subFee = _subFee;
    ...
    function prevPendingRewards(address _user) public view returns (address[] memory _rwdToken, uint256[] memory _amount, bool hasRewards) {
        ...
        for (uint i = 0; i < rewardTokenCount; i++) {
            if( i != currentRound){
                ...
                _amount[i] = user.amount.mul(round.accTokenPerShare).div(round.PRECISION_FACTOR).sub(usersRewardDebt[_user][i]);
...
contract NFTPoolFactory is Ownable {
    ...
    address public treasury = 0x1A81EAB2AFed37f35FB561b4867985b87993a1C7;
