...
contract WPToken is ERC20, Ownable {
    ...
    uint256 public buyBonousFee;
    uint256 public sellLiquidityFee;
    uint256 public sellMarketingFee;
    uint256 public sellBonousFee;
    uint256 public transferLiquidityFee;
    uint256 public transferMarketingFee;
    uint256 public transferBonousFee;
    ...
    function setBonousWalletAddress(address payable wallet) external onlyOwner{
    ...
    function setSellFee(uint256[3] memory feeSetting_) public onlyOwner {
        ...
        sellBonousFee = feeSetting_[2];
    }

    function setTransferFee(uint256[3] memory feeSetting_) public onlyOwner {
        transferLiquidityFee = feeSetting_[0];
        transferMarketingFee = feeSetting_[1];
        transferBonousFee = feeSetting_[2];
    }
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            if(automatedMarketMakerPairs[from]){
                ...
                DFee = amount.mul(buyBonousFee).div(100);
            }else if(automatedMarketMakerPairs[to]){
                LFee = amount.mul(sellLiquidityFee).div(100);
                MFee = amount.mul(sellMarketingFee).div(100);
                DFee = amount.mul(sellBonousFee).div(100);
            }else{
                LFee = amount.mul(transferLiquidityFee).div(100);
                MFee = amount.mul(transferMarketingFee).div(100);
                DFee = amount.mul(transferBonousFee).div(100);
            }
        
            if(LFee > 0) super._transfer(from, address(this), LFee);
            if(MFee > 0) super._transfer(from, marketingWalletAddress, MFee);
            if(DFee > 0) super._transfer(from, bonousWalletAddress, DFee);
            amount = amount.sub(LFee).sub(MFee).sub(DFee);
        }
