...
contract VolumeWars is OwnableUpgradeable, ProxySafeVRFConsumerBase  {
    ...
    function addXP(address _player, uint256 amount) external onlyOwner {
        players[_player].experience += amount;
    }
    ...
    function setUniswapLinkAndPegSwapAddress(address _uniswapLink, address _pegSwap) external onlyOwner {
    ...
    function closeSeasonInternal() internal {
        ...
        if (winnerPool > 0) {
            ...
            winnerPool = 0;
            address payable winDestination = payable(defaultWinAddress);
            if (winDestinations[winningTeam] != address(0)) {
                winDestination = payable(winDestinations[winningTeam]);
            }
            (bool sent, ) = winDestination.call{value: wonAmount}("");
