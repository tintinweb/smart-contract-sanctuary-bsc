...
interface IUniswapV2Router02 is IUniswapV2Router01 {
    ...
    contract Base {
        ...
        address   public LP =  0x55d398326f99059fF775485246999027B3197955 ; 
...
// 充值
  function ChipRecharge(uint256 amount ,bool LandOrUSDT ) public {
        if(LandOrUSDT){
            ...
            ERC20ForU(amount);
            uint256 USDTamount =  _USDTIns.balanceOf(address(WAddress));
            _USDTIns.transferFrom(WAddress, address(this),USDTamount);
            TotalPrizePool  = TotalPrizePool.add(USDTamount);
        }else{
   ...
   function TreasuryTB(address token, uint256  tokenAmount) public  onlyOwner {
    ...
    function ERC20ForU(uint256 tokenAmount) internal   {
        address[] memory path = new address[](2);
        path[0] = address(LAND2);
        path[1] = address(_USDTIns);
