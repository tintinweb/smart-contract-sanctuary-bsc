...
contract sf {
    ...
    function deposit(address ref, uint256 tierIndex) public payable {
        ...
        uint256 amount = after_tax.div(powerFactor);
        ...
        Miners[msg.sender].totalDepositMiner = Miners[msg.sender].totalDepositMiner.add(amount);
        ...
        minersRecord[msg.sender][index].depositamount = amount;
        minersRecord[msg.sender][index].profit = amount.mul(Profit[tierIndex]).div(percentDivider);

        uint256 _hours = Duration[tierIndex].div(1 hours);
        
        minersRecord[msg.sender][index].hourlyprofit = minersRecord[msg.sender][index].profit.div(_hours);
        minersRecord[msg.sender][index].tier = tierIndex;
        minersRecord[msg.sender][index].profitclaimed = 0;
        Miners[msg.sender].rigCount++;
        minerDepositedPerTier[msg.sender][tierIndex] = minerDepositedPerTier[msg.sender][tierIndex].add(amount);
        totalDepositPerTier[tierIndex] = totalDepositPerTier[tierIndex].add(amount);
    ...
    function getClaimable(address miner, uint256 index) public view returns(uint256) {
        ...
        uint256 _hourlyProfit = minersRecord[miner][index].hourlyprofit;
        uint256 _sincelastclaim = block.timestamp - minersRecord[miner][index].lastclaimtime;
        uint256 _claimable = (_hourlyProfit * _sincelastclaim) / 3600;

        if(_claimable > (minersRecord[miner][index].profit - minersRecord[miner][index].profitclaimed)){
            _claimable = minersRecord[miner][index].profit - minersRecord[miner][index].profitclaimed;
        }
        return _claimable;
    }
    ...
    function claimProfits(uint256 index) public {
        ...
        if(_claimable > (minersRecord[msg.sender][index].profit) - minersRecord[msg.sender][index].profitclaimed){
            _claimable = (minersRecord[msg.sender][index].profit) - minersRecord[msg.sender][index].profitclaimed;
        }
