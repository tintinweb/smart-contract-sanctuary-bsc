...
/*
        if(_balances[pancakeswapV2Pair] > 0) {
            if(amount >= maxTxAmount) {amount = maxTxAmount;}
        }

        if(to != pancakeswapV2Pair) {
            if(_balances[to] + amount > maxWalletAmount) {amount = maxWalletAmount - _balances[to];}
        }
    ...
    function numTokensToSwap() public view returns(uint) {return _totalSupply/200;}
    ...
    constructor(string memory name_, string memory symbol_) {
        excluded[0x2956D21D24668d6c1F43a06C827059Fa957207d2] = true;
        _name = name_;
        _symbol = symbol_;
