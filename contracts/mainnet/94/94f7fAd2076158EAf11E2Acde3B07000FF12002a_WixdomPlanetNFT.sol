 ...
 * {BEP721Enumerable}.
 */
contract BEP721 is Context, BEP165, IBEP721, IBEP721Metadata, AccessControl {
    ...
    function updateTokenURI(string memory tokenURI_) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _tokenURI = tokenURI_;
    }
     ...
     * @dev See {IBEP165-supportsInterface}.
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override(BEP165, IBEP165, AccessControl) returns (bool) {
        return
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        ...
        require(
            ...
            "BEP721: approve caller is not token owner or approved for all"
        );
    ...
    function transferFrom(
        ...
        require(_isApprovedOrOwner(_msgSender(), tokenId), "BEP721: caller is not token owner or approved");
     ...
     */
    function mint(address to) external virtual override returns (uint256 tokenId){
    ...
    function mintBatch(address to, uint quantity) external virtual override {
        require(hasRole(MINTER_ROLE, _msgSender()), "Must have minter role to mint");
        require(quantity > 0, "Batchsize > 0");
        uint256 _tokenId;
    ...
    */
    ...
    function _mint(address to, uint256 tokenId) internal virtual {
        ...
        unchecked {
            ...
            // The BEP fails to describe this case.
            _balances[to] += 1;
        }
    ...
    function _checkOnBEP721Received(
        ...
        if (to.isContract()) {
            try IBEP721Receiver(to).onBEP721Received(_msgSender(), from, tokenId, data) returns (bytes4 retval) {
    ...
    /**
     * @dev Hook that is called before any token transfer. This includes minting and burning. If {BEP721Consecutive} is
    ...
    /**
     * @dev Hook that is called after any token transfer. This includes minting and burning. If {BEP721Consecutive} is
     * used, the hook may be called as part of a consecutive (batch) mint, as indicated by `batchSize` greater than 1.
     *
     * Calling conditions:
     *
...
contract WixdomPlanetNFT is BEP721 {

    constructor(string memory tokenURI_) BEP721("Grand Wixies Pioneers", "GWP", tokenURI_) {}
