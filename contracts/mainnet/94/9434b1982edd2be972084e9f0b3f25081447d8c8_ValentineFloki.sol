...
contract ValentineFloki is Context, IERC20, Ownable {
    ...
    uint256 public maxTXlimit = 100000000 * 10**_decimals;
    uint256 public maxWalletLimit = 100000000 * 10**_decimals;
    
    address public marketingWallet = 0x76d6cD93D48808b2C493049531D09FFe473932d4;
    address public bbWallet = 0x76d6cD93D48808b2C493049531D09FFe473932d4;

    string private constant _name = "ValentineFloki";
    string private constant _symbol = "ValentineFloki";
    ...
    function setTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _bb) public onlyOwner {
       taxes = Taxes(_rfi,_marketing,_liquidity,_bb);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _bb) public onlyOwner {
       sellTaxes = Taxes(_rfi,_marketing,_liquidity,_bb);
        emit FeesChanged();
    }
    ...
    function SwapEnabled(bool _enabled) external onlyOwner{
        swapEnabled = _enabled;
    }
    
    function Isbots(address account, bool state) external onlyOwner{
        _isBlacklisted[account] = state;
    }
    
    function AllowedTransfer(address account, bool state) external onlyOwner{
        allowedTransfer[account] = state;
    }
    
    function MaxTxLimit(uint256 maxTX) external onlyOwner{
        maxTXlimit = maxTX * 10**_decimals;
    }
    
    function MaxWalletlimit(uint256 amount) external onlyOwner{
        maxWalletLimit = amount * 10**_decimals;
    }

    function Router(address newRouter, address newPair) external onlyOwner{
