...
contract FUTUREBSCCARD is IERC20 {
    ...
    string constant private _name = "FUTURE BSC CARD";
    string constant private _symbol = "FBC";
    ...
    struct TaxWallets {
        address payable nftPrize;
        address payable liquidity;
    }

    TaxWallets public _taxWallets = TaxWallets({
        nftPrize: payable(0xd88dD401937BC1253A3B9E34017778B067f4c191),
        liquidity: payable(0x6429ab369Cbd6300B9eA7D36e93aF76915e2b3c2)
    });
...
//===============================================================================================================
    ...
    function setRatios(uint16 liquidity, uint16 nftPrize, uint16 burn) external onlyOwner {
        _ratios.liquidity = liquidity;
        _ratios.nftPrize = nftPrize;
        _ratios.burn = burn;
        _ratios.totalSwap = liquidity + nftPrize;
        uint256 total = _taxRates.buyFee + _taxRates.sellFee;
        require(_ratios.totalSwap + _ratios.burn <= total, "Cannot exceed sum of buy and sell fees.");
    }

    function setWallets(address payable nftPrize, address payable liquidity) external onlyOwner {
        _taxWallets.nftPrize = payable(nftPrize);
    ...
    function setLiquiditySwitch(bool enabled) external onlyOwner {
        liquiditySwitch = enabled;
    }
    ...
    function contractSwap(uint256 contractTokenBalance) internal lockTheSwap {
        ...
        if (liquiditySwitch) {
            liquidityBalance = (amtBalance * toLiquify) / swapAmt;
        ...
        bool success;
        uint256 nftPrizeBalance;
        if (!liquiditySwitch) {
            liquidityBalance = (amtBalance * ratios.liquidity) / ratios.totalSwap;
            nftPrizeBalance = amtBalance - liquidityBalance;
        } else {
            nftPrizeBalance = amtBalance;
        }
        if (ratios.nftPrize > 0) {
            (success,) = _taxWallets.nftPrize.call{value: nftPrizeBalance, gas: 35000}("");
        }
        if (ratios.liquidity > 0 && !liquiditySwitch) {
    ...
    function multiSendTokens(address[] memory accounts, uint256[] memory amounts) external onlyOwner {
        // Approval not required, function is owner-only and only sends from the transaction sender.
