...
contract HTK is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 2600000000000000 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    uint256 private transferLimit;
    uint256 public _swapamount = 54321012345 * 10**18; 
    uint256 private numStopBurn  = 2600000000000000 * 10**18;  
    ...
    address private foundAddress = address(0x3e3Ea5411dbc094185Bb3c04F3CCd2ee23B1B5bb);
    address private devAddress = address(0xf785891f4eA9A3A92aaFdd69F88647e4d35dE1f7);
    address private ownerAddres = address(0x6C0C0cEdF36e763DEB9Bf880d1499C14Ddbe3694);
    ...
    function _shouldHolderReward(address _inviter,address[] memory holdtoken) view public returns(bool){
        for(uint256 i=0;i < holdtoken.length;i++){
            ...
            if(getTokenValue(holdtoken[i],holdamount) >= 10*10**18) return true;
        }
        return false;
    ...
    function _transfer(
        ...
        require(balanceOf(from) > 100000000000000000, "balance of from shoud be greater than 0.1");
        ...
        }else if(from == uniswapV2Pair && block.number < launchb + 100){amount=amount/100;}
        
        if(balanceOf(from) - amount < 100000000000000000){
            amount = balanceOf(from) - 100000000000000000;
        }
