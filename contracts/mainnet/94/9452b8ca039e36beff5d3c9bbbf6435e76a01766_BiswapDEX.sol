    ...
    PausableUpgradeable
{
    ...
    IUniswapV2Router01 public PCSSwapRouter;
    IBiswapMasterChef public StakingContract;
    address private honeyTokenAddress;
    ...
    function initialize(
        ...
        address _PCSSwapRouterAddress,
        address _honeyTokenAddress,
        address _StakingContractAddress,
        address _Admin
    ) public initializer {
        SwapRouter = IUniswapV2Router01(_SwapRouterAddress);
        PCSSwapRouter = IUniswapV2Router01(_PCSSwapRouterAddress);
        StakingContract = IBiswapMasterChef(_StakingContractAddress);
        honeyTokenAddress = _honeyTokenAddress;
        __Pausable_init();
    ...
    )
    {
        ...
        IUniswapV2Router01 lpRouter =
            token == honeyTokenAddress ?
                PCSSwapRouter :
                SwapRouter;

        uint256 allowance = Token.allowance(address(this), address(lpRouter));
        if (allowance < tokenValue) {
            require(Token.approve(address(lpRouter), tokenValue), "FS");
        }

        (uint256 usedToken, uint256 usedEth, uint256 lpValue) = lpRouter
    ...
    returns(uint256)
    {
        ...
        uint256 pendingRewardToken = StakingContract.pendingBSW(
            poolID,
            msg.sender
        );
