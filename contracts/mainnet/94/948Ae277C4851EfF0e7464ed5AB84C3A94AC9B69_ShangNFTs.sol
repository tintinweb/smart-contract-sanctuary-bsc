...
contract ShangNFTs is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  // Supply
  uint256 public maxSupply = 8816;
  uint256 public lastSupply = maxSupply;
  uint256 public maxMintAmount = 10;

  // Lists 
  uint256[8816] public remainingIds;
  ...
  address private _Artist = 0x856F02c86ACf6F7CA6e4B6bd30A494EfA60aed68;
  address private _Dapp = 0x0eE730cd633F1cbD0Dc410Cf4f37512C9587bE11;
  ...
  function mint(uint256 _mintAmount) public payable {
    // Checks
    require(!paused, "ShangNFTs: minting has not started yet");
    require(_mintAmount > 0, "ShangNFTs: you have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "ShangNFTs: you can only mint 10 at a time");
    require(lastSupply >= _mintAmount,  "ShangNFTs: the collection is sold out");
    if (msg.sender != owner()) {
      require(msg.value >= cost * _mintAmount, "ShangNFTs: total cost doesn't match");
    }
    // Minting
  ...
  function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
    // Checks
    require(_winner != address(0), "ShangNFTs: zero address");
    require(_amount > 0, "ShangNFTs: you have to mint at least one");
    require(lastSupply != 0,  "ShangNFTs: the collection is sold out");
  ...
  function tokenURI(uint256 _id) public view virtual override returns (string memory){
    require(_exists(_id),"ShangNFTs: URI query for nonexistent token");
  ...
  */
  ...
  function handleMintReward(uint256 _amount) private {
      // Split payment
      uint256 dappShare = _amount.div(20);
      uint256 artistShare = _amount.div(20);
      uint256 devShare = _amount.sub(artistShare).sub(dappShare);
