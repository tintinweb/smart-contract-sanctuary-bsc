...
contract M1VerseStake {
    ...
    function stake(uint256 amount, uint256 timeperiod) public {
        ...
        uint256 rewardtokenPrice = (amount.mul(M1VERSEVAL)).div(1e9);
    ...
    function initiateUnstake(uint256 index) external {
        require(!stakersRecord[msg.sender][index].initiateUnstake, "already initiateUnstaked");
        ...
        require(
            ...
            "cannot initiate unstake before lock duration"
        );
        stakersRecord[msg.sender][index].initiateUnstake = true;
        stakersRecord[msg.sender][index].initiateUnstakeTime = block.timestamp;
        
    }

    function unstake(uint256 index) external {
        require(stakersRecord[msg.sender][index].initiateUnstake, "initiate Unstake First");
        require(!stakersRecord[msg.sender][index].unstaked, "already unstaked");
        require(
            (stakersRecord[msg.sender][index].initiateUnstakeTime +  (7*oneDay)) < block.timestamp,
            "cannot unstake before cool down duration"
        );
