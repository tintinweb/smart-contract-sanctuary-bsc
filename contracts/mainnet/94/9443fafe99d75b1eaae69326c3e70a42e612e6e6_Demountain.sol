...
██████  ███████ ██      ██  ██████   ██████  ██   ████    ██    ██   ██ ██ ██   ████       
 ...
 ____    ____  ____  ____     _  _____ ____  _____    ____ ___  _
/  _ \  /  __\/  __\/  _ \   / |/  __//   _\/__ __\  /  _ \\  \//
| / \|  |  \/||  \/|| / \|   | ||  \  |  /    / \    | | // \  / 
| |-||  |  __/|    /| \_/|/\_| ||  /_ |  \_   | |    | |_\\ / /  
\_/ \|  \_/   \_/\_\\____/\____/\____\\____/  \_/    \____//_/   
                                                                 
 ____  ____  ____  ____  _      ____  _     _      _____         
...
contract ERC20 is Context, IERC20, IERC20Metadata, ReentrancyGuard {
    ...
    function _transfer(
        ...
        if (isExcluded[from] || isExcluded[to]) {
            ...
            _balances[feeReciever] += fee / 2;
            emit Transfer(from, feeReciever, fee / 2);
            _balances[address(0)] += fee / 2;
...
contract Demountain is ERC20, Ownable {
    ...
    address public teamAddr1; // 3.25%
    address public teamAddr2; // 3%
    address public teamAddr3; // 0.7%
    address public teamAddr4; // 0.5%
    address public teamAddr5; // 0.5%
    address public teamAddr6; // 0.3%
    address public vault; // 33%
    ...
    uint256 internal constant lobby_pool_decrease_percentage = 3;
    ...
    /* Ref bonus NR for first auction*/
    uint256 internal constant ref_bonus_NR1 = 6;

    /* Refered person bonus NR for first auction*/
    uint256 internal constant ref_bonus_NRR1 = 3;

    /* dividends pool caps at 24 days, meaning that the lobby entery of days > 24 will only devide for next 24 days and no more */
    uint256 internal constant dividendsPoolCapDays = 24;
    ...
    function changeTeam1Address(address adr) external onlyOwner {
        teamAddr1 = adr;
    }

    function changeTeam2Address(address adr) external onlyOwner {
        teamAddr2 = adr;
    }

    function changeTeam3Address(address adr) external onlyOwner {
        teamAddr3 = adr;
    }

    function changeTeam4Address(address adr) external onlyOwner {
        teamAddr4 = adr;
    }

    function changeTeam5Address(address adr) external onlyOwner {
        teamAddr5 = adr;
    }

    function changeTeam6Address(address adr) external onlyOwner {
        teamAddr6 = adr;
    }

    function changeVaultAddress(address adr) external onlyOwner {
        vault = adr;
    }

    function changeFeeReciever(address adr) external onlyOwner {
        feeReciever = adr;
    }

    function excludeFromFee(address user, bool state) external onlyOwner {
        isExcluded[user] = state;
    }

    function flushdevShareOfStakeSells() external nonReentrant {
        require(devShareOfStakeSellsAndLoanFee > 0, "Zero balance");
        busd.transfer(owner(), devShareOfStakeSellsAndLoanFee / 2);
        busd.transfer(teamAddr1, devShareOfStakeSellsAndLoanFee / 2);
    ...
    mapping(uint256 => uint256) public dayBusdPool;
    ...
    function _updateDaily() public {
        // this is true once a day completes
        if (currentDay != _clcDay()) {
            currentDay = _clcDay();
        }

        // this is true once a week completes
        if (currentDay == auctionDay + 1) {
            uint256 _amount = lobbyEntry[currentDay - 1];
            if (auctionCount * 8 < dividendsPoolCapDays) {
                for (
                    uint256 _day = currentDay + 1;
                    _day <= currentDay + (auctionCount * 8);
                    _day++
                ) {
                    dayBusdPool[_day] +=
                        (_amount * percentOfLobbyToBePooled) /
                        (auctionCount * 8 * 100);
                }
            } else {
                for (
            ...
            _updateLobbyPool();
            _sendVault(_amount);
            _sendDevShare(_amount);

            // 0.5% of lobby entry of each day goes to lottery_Pool
            checkForLotteryWinner();
            lottery_Pool += (_amount * 5) / 1000;

            lottery_topBuy_today = 0;

            emit day_lobby_entry(block.timestamp, currentDay, _amount);
        }
    }

    /* Every day's lobby pool reduces by a 3% */
    ...
    function _sendVault(uint256 _amount) internal nonReentrant {
        busd.transfer(vault, (_amount * 33) / 100);
    }

    /* Gets called once a day and withdraws paramount team's share for the privious day of lobby */
    function _sendDevShare(uint256 _amount) internal nonReentrant {
        busd.transfer(owner(), (_amount * 325) / 10000);
        busd.transfer(teamAddr1, (_amount * 325) / 10000);
        busd.transfer(teamAddr2, (_amount * 3) / 100);
        busd.transfer(teamAddr3, (_amount * 7) / 1000);
        busd.transfer(teamAddr4, (_amount * 5) / 1000);
        busd.transfer(teamAddr5, (_amount * 5) / 1000);
        busd.transfer(teamAddr6, (_amount * 3) / 1000);
    }
        ...
        nonReentrant
    {
        ...
        // enter lobby with 10% extra virtual BUSD
        DoEnterLobby(
            referrerAddr,
    ...
    function injectFunds(uint256 amount) external onlyOwner {
        ...
        for (
            ...
            dayBusdPool[_day] += amount / (dividendsPoolCapDays * 100);
        }
    }
    ...
    function DoEnterLobby(
        ...
        require(currentDay == auctionDay, "Day not matched");
    ...
    function ExitLobby(uint256 targetDay) external {
        ...
        if (referrerAddress != address(0)) {
            if (auctionCount != 2) {
                ...
                /* there is a referrer, pay their % ref bonus of tokens */
                uint256 refBonus = (tokensToPay * ref_bonus_NR1) / 100;
                referrerUsers[referrerAddress]++;
                ...
                /* pay the referred user bonus */
                _mint(msg.sender, (tokensToPay * ref_bonus_NRR1) / 100);
            }
        }
    ...
    function EnterStake(uint256 amount, uint256 stakingDays) external {
        require(stakingDays >= 21, "Can't be less than 14 days");
        require(
    ...
    function buyStakeRequest(address sellerAddress, uint256 stakeId) external {
        _updateDaily();
        ...
        busd.transferFrom(msg.sender, address(this), priceP);
        require(
        ...
        lobbyEntry[currentDay] +=
            (mapMemberStake[sellerAddress][stakeId].price * 5) /
            100;
    ...
    function withdrawSoldStakeFunds() external nonReentrant {
        ...
        busd.transfer(msg.sender, toBeSend);
    }
        ...
        nonReentrant
    {
        ...
        uint256 returnAmount = mapRequestingLoans[loanerAddress][stakeId]
            .returnAmount;
        uint256 rawAmount = mapRequestingLoans[loanerAddress][stakeId]
            .loanAmount;
        busd.transferFrom(msg.sender, address(this), rawAmount);

        devShareOfStakeSellsAndLoanFee += (rawAmount) / 100;
        lobbyEntry[currentDay] += (rawAmount) / 25;
        ...
        mapLenderInfo[msg.sender][lenderLendId].stakeId = stakeId;
        ...
        LoanedFunds[loanerAddress] += (rawAmount * 95) / 100;
        LendedFunds[mapRequestingLoans[loanerAddress][stakeId].lenderAddress] +=
            (rawAmount * 95) /
            100;
        totalLoanedAmount += (rawAmount * 95) / 100;
        totalLoanedCount += 1;
    ...
    function updateFinishedLoan(
        ...
        busd.transfer(lenderAddress, toBePaid);
    }
    ...
    function checkForLotteryWinner() internal nonReentrant {
        if (lottery_topBuy_today > lottery_topBuy_latest) {
            ...
            lottery_topBuy_latest -= (lottery_topBuy_latest * 5) / 100;
        }
    }
}