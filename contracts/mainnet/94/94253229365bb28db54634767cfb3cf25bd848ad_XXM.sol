...
contract XXM is ERC20 {
	...
	address private _fundAddress = address(0x6512E7CFA9708ce24C46E975437de56177DAB32a);
	...
	function changeSwapWarp(XXMWarp _warp) public onlyOwner {
        warp = _warp;
	...
	function getNowPrice() public view returns(uint256){
        uint256 poolwbnb = USDT.balanceOf(uniswapV2Pair);
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx) {
			...
			if(from == uniswapV2Pair){
                require(balanceOf(to).add(amount) <= 10**18, "max one");
        ...
        if (takeFee) {
			...
			super._transfer(from, address(warp), amount.div(200));
	...
	function swapAndLiquifyUsdt() public {
		...
		if(allAmount > 10**16){
			...
			if(allusdtAmount > oldusdtAmount){
                uint256 newusdtAmount = allusdtAmount.sub(oldusdtAmount);
				USDT.transfer(_fundAddress,newusdtAmount.div(11).mul(4));
			}
		}
    }
	...
	function _splitOtherToken() private {
        if(usdtPerAmount == 0){
            usdtPerAmount = USDT.balanceOf(address(this));
        }
        if(usdtPerAmount > 0){
	...
	function isLdxUser(address user) public view returns(bool){
