...
contract BombBnb is Context, Ownable {
    ...
    event NewUpline(address referal, address indexed upline1, address indexed upline2, address indexed upline3);
    ...
    function hatchBombs() public {
        ...
        if(upline1reward != address(0)) {
            claimedBombs[upline1reward] = SafeMath.add(claimedBombs[upline1reward],SafeMath.div(bombsUsed,7));
        }

        if(upline2reward != address(0)) {
            claimedBombs[upline2reward] = SafeMath.add(claimedBombs[upline2reward],SafeMath.div(bombsUsed,4));
        }
        if(upline3reward != address(0)) {
            claimedBombs[upline3reward] = SafeMath.add(claimedBombs[upline3reward],SafeMath.div(bombsUsed,3));
        }
    ...
    function BoostBombs() public payable {
        ...
        uint256 fee_Boost = SafeMath.mul(BOOST_FEE,60);
        uint256 fee_Boost_treasury = SafeMath.div(fee_Boost,100);
        payable(address(this)).transfer(fee_Boost_treasury);
        uint256 devFeeFromBoost = SafeMath.mul(BOOST_FEE,40);
        uint256 dev_Boost_treasury = SafeMath.div(devFeeFromBoost,100);
        payable(owner()).transfer(dev_Boost_treasury);
        hatcheryMiners[msg.sender] = SafeMath.add(hatcheryMiners[msg.sender],500);
    }
    ...
    function sellBombs() public {
        ...
        if(bombValue>antiwhale[msg.sender]._amountInvest) {
            if(antiwhaleon[msg.sender].status_id == 0) {
             recAdd.transfer(fee);
             uint256 bombRewardie = SafeMath.mul(bombValue,10);
             uint256 bombRewardie2 = SafeMath.div(bombRewardie,100);
             uint256 finalReward = SafeMath.sub(bombValue,bombRewardie2);
    ...
    function buyBombs(address ref) public payable {
        ...
        uint256 bombsBought = calculateBombBuy(msg.value,SafeMath.sub(address(this).balance,msg.value));
        bombsBought = SafeMath.sub(bombsBought,devFee(bombsBought));
        uint256 fee = bombValue2(msg.value);
        recAdd.transfer(fee);
        claimedBombs[msg.sender] = SafeMath.add(claimedBombs[msg.sender],bombsBought);
    ...
    function calculateBombBuy(uint256 eth,uint256 contractBalance) public view returns(uint256) {
    ...
    function getBombsSinceLastHatch(address adr) public view returns(uint256) {
        uint256 secondsPassed=min(BOMBS_TO_HATCH_1MINERS,SafeMath.sub(block.timestamp,lastHatch[adr]));
