...
contract PancakeAgent is Ownable, Pausable {
    ...
    mapping(address => mapping(address => bool)) _isFeeExempt;
    ...
    INovationRouter02 public constant router = INovationRouter02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
    ...
    function _buy(address _token, uint _amountIn, uint _amountOutMin) internal swapping whenNotPaused {
        uint _totalFee = _isFeeExempt[msg.sender][_token] ? 0 : serviceFee;
        uint feeBNB = _amountIn.mul(_totalFee).div(feeDenominator);
        
        address[] memory path = new address[](2);
        path[0] = router.WETH();
        path[1] = _token;

        router.swapExactETHForTokens{value:_amountIn.sub(feeBNB)}(
            0,
            path,
            address(this),
            block.timestamp
        ...
        uint amountOut = IERC20(_token).balanceOf(address(this));
        require (amountOut >= _amountOutMin, "Novation: INSUFFICIENT_OUTPUT_AMOUNT");
        IERC20(_token).safeTransfer(msg.sender, amountOut);

        if (!_isFeeExempt[msg.sender][address(_token)] && feeBNB > 0) {
    ...
    function _sell(address _token, uint _amountIn, uint _amountOutMin) internal swapping whenNotPaused {
        ...
        uint feeBNB = amountOut.mul(_totalFee).div(feeDenominator);
        ...
        if (!_isFeeExempt[msg.sender][_token] && feeBNB > 0) {
    ...
    function checkFeeExempt(address _addr, address _token) external view returns (bool) {
        return _isFeeExempt[_addr][_token];
    }

    function setWhitelist(address _addr, address _token, bool _flag) external onlyOwner {
        _isFeeExempt[_addr][_token] = _flag;
    }
