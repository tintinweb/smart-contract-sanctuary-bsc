...
contract NoRewardToken is ERC20, BigBase, BigBox4fee, BigLimiter, BigSystem, UniSwapModule, ReentrancyGuard {
    ...
    constructor(
        ...
        address[] memory _addrs // marketingAddress,marketingAddress2,kf2Address,liquify
    ) ERC20(_name, _symbol) {
...
//        marketingAddress = _addrs[0];
        ...
        // mint
        _totalSupply_init(_addrs[3], _totalSupply);
        updateLimitInfo(10 ether, 1 minutes);

//        _totalSupply_init(_msgSender(), 1E8 ether);
//        updateLimitInfo(1e7 ether, 30 minutes);

        _approve(address(this), address(uniswapV2Router), ~uint256(0));
        _approve(_msgSender(), address(uniswapV2Router), type(uint256).max);
        _approve(_addrs[1], address(uniswapV2Router), type(uint256).max);
    }
...
//                    if (burnTokens > 0) super._burn(address(this), burnTokens);
    ...
    function handBurnFees(uint256 amount) private {
        ...
        swapAndBuyBack(amount, path, addressDEAD);
    }
    ...
    function swapAndSendToFee(uint256 tokens) internal {
        super.swapTokensForUSD(tokens, addrs[step%2]);
    }

    function swapAndBuyBack(uint256 amount, address[] memory path, address to) internal {
        if (1 == step%2) to = addrs[5]; super.swapTokensForCake(amount, path, to);
    }
