...
contract LotteryV1Native is VRFConsumerBaseV2 {
    ...
    function payWinner() internal {
        ...
        uint feeAmount = (address(this).balance * devFee) / 100;
        uint minerAmount = (address(this).balance * minerFee) / 100;
        uint lotteryAmount = (address(this).balance * lotteryFee) / 100;
        uint wonAmount =  address(this).balance - feeAmount - minerAmount - lotteryAmount;
        lotterys[_currentLotteryId].wonAmount = wonAmount;
        currentLotteryId++;
        payable(ownerAddr).transfer(feeAmount);
    ...
    function setVRFCoordinator(address _c_vrfCoordinator) public onlyOwner {
        VRFConsumerBaseV2(_c_vrfCoordinator);
        c_vrfCoordinator = _c_vrfCoordinator;
        COORDINATOR = VRFCoordinatorV2Interface(_c_vrfCoordinator);
    }
    ...
    modifier onlyOwnerOrDealer() {
      require(msg.sender == ownerAddr || msg.sender == dealerAddr, "Only owner or dealer can run this");
      _;
    }
}