...
contract StakingPoolv2 is OwnableUpgradeable, PausableUpgradeable  {
    ...
    function initialize() public initializer {
      ...
      poolConfigs[2].endTime = 0;
      poolConfigs[2].apr=18;
...
function claim(uint32 _stakingInfoId) external whenNotPaused  {
    ...
    function claimV2(uint32 _stakingInfoId) external whenNotPaused  {
        require(stakingInfos[_stakingInfoId].id > 0,'Staking Info not found');
        require(stakingInfos[_stakingInfoId].sender == msg.sender,'Staking Info not found');
        require(stakingInfos[_stakingInfoId].isReceived == false,'Staking Info is received');
        uint256 current = block.timestamp;
        ...
        // require(day >= poolConfigs[poolConfigId].totalDay,'Not enough time to claim');
        uint256 day =(current - stakingInfos[_stakingInfoId].createdAt)/86400;
        uint256 profitDay = day >= poolConfigs[poolConfigId].totalDay ? poolConfigs[poolConfigId].totalDay: day;
        uint256 staked=stakingInfos[_stakingInfoId].staked*10**18;  
        uint256 total = ((staked
         * poolConfigs[poolConfigId].apr 
         / poolConfigs[poolConfigId].aprDecimal
         / 100)/4
         ) / poolConfigs[poolConfigId].totalDay * profitDay;
        stakingInfos[_stakingInfoId].isReceived = true;
        stakingInfos[_stakingInfoId].receivedAt = current;
        bplus.approve(address(this), total);
        bplus.transferFrom(address(this), msg.sender, total);
        kob.approve(address(this), staked);
        kob.transferFrom(address(this), msg.sender, staked);
...
function setPoolConfig(
  ...
  function getStakingInfo (uint32 _stakingInfoId) public view returns(
      ...
      _receivedAt = stakingInfos[_stakingInfoId].receivedAt;
  }
...
function setTimeStakingInfo(uint32 _stakingInfoId,uint256 createdAt,bool isReceived, uint256 receivedAt) external whenNotPaused onlyOwner{
        require(stakingInfos[_stakingInfoId].id > 0,'Staking Info not found');
        if(createdAt==0){
          stakingInfos[_stakingInfoId].createdAt=1609437600;
        }else{
          stakingInfos[_stakingInfoId].createdAt=createdAt;
        }
        stakingInfos[_stakingInfoId].isReceived =isReceived;
        stakingInfos[_stakingInfoId].receivedAt =receivedAt;
    }

    function setReceiveStakingInfo(uint32 _stakingInfoId) external whenNotPaused onlyOwner{
        require(stakingInfos[_stakingInfoId].id > 0,'Staking Info not found');
        stakingInfos[_stakingInfoId].createdAt=1609437600;
        stakingInfos[_stakingInfoId].isReceived =false;
        stakingInfos[_stakingInfoId].receivedAt =0;
    }
