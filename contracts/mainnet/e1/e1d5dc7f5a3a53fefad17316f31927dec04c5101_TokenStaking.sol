...
// File: contracts/MetaWeb3Pad.sol
 ...
 */
contract MetaWeb3Pad is Context, IERC20, IERC20Metadata {
...
// File: contracts/MetaWeb3Staking.sol
...
contract TokenStaking is Ownable, ReEntrancyGuard {
    string public name = "MetaWeb3Pad Staking";
    IERC20 public token;

    //declaring APY for custom staking ( default 01.87% daily or 684% APY yearly)
    uint256 public customAPY = 1870;

    //declaring total staked
    uint256 public totalStaked;
    uint256 public customTotalStaked;

    uint8 public stakingFee;
    ...
    constructor(MetaWeb3Pad _token, uint8 _stakingFee, address _feeAddress) {
        token = IERC20(_token);
    ...
    function customStaking(uint256 _amount) public {
        ...
        //require(whitelistedForCustomAPY[staker], "You are not whitelisted for custom staking.");

        uint256 stakedAmount = _amount - (_amount*stakingFee/100);
        uint256 feeTaken = _amount- stakedAmount;

        token.transferFrom(msg.sender, address(this), stakedAmount);
        token.transferFrom(msg.sender, feeAddress, feeTaken);

        customTotalStaked = customTotalStaked + stakedAmount;
    ...
    function customUnstake() public noReentrant{
    ...
    //customAPY airdrop
    function _distributeCustomRewards() external onlyOwner {
        ...
        for (uint256 i = 0; i < customStakers.length; i++) {
            ...
            if (balance > 0) {
                token.transferFrom(distributor, recipient, balance);
            }
        }
    }
    ...
    function pendingCustomRewards(address _staker) public view returns(uint256) {
        uint256 rewardBalance = customStakingBalance[_staker] * customAPY / 100000;
