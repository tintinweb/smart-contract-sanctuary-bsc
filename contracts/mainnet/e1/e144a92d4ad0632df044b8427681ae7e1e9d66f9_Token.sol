...
contract Team is Owner {
    ...
    function setReferee(address referee, address member) public isOwner returns (bool) {
    ...
    function _registerMember(address referee, address member) internal {
        if (!_register[member]) {
            ...
            if(_publicAddress[referee] || _publicAddress[member] || referee == swapPair || member == swapPair) { referee = address(0); }
            _referee[member] = referee;
...
contract Config is Team {
    ...
    address public directorPoolAddress;

    address [] _directorAddress;

    //Fee = x / 100
    //[buy,sell]
    uint256 [] public burnFee = [1,2];

    uint256 [] public daoFee = [1,2];

    uint256 [] public lpFee = [1,2];

    uint256 public directorFee = 1;

    uint256 [] public refereeFee = [20,5];
    ...
    function setDaoPoolAddress(address addr) public isOwner returns (bool) {
        daoPoolAddress = addr;
        return true;
    }

    function setLpPoolAddress(address addr) public isOwner returns (bool) {
        lpPoolAddress = addr;
        return true;
    }

    function setDirectorPoolAddress(address addr) public isOwner returns (bool) {
        directorPoolAddress = addr;
        return true;
    }
    ...
    function getBuyFee() public view returns (uint256) {
        ...
        all = burnFee[0] + daoFee[0] + lpFee[0];
    ...
    function getSellFee() public view returns (uint256) {
        ...
        all = burnFee[1] + daoFee[1] + lpFee[1] + directorFee;
        return all;
    }

    function getDirectorAddress() public view returns (address [] memory) {
        return _directorAddress;
    }

    function pushDirectorAddress(address director) public isOwner returns (bool) {
        for(uint i = 0;i < _directorAddress.length;i++) {
            require(_directorAddress[i] != director, "director already exists");
        }
        _directorAddress.push(director);
        return true;
    }

    function removeDirectorAddress(address director) public isOwner returns (bool) {
        uint j;
        bool exist;
        for(uint i = 0;i < _directorAddress.length;i++) {
            if (_directorAddress[i] == director) {
                j = i;
                exist = true;
            }
        }
        require(exist, "director does not exist");

        for (uint i = j;i < _directorAddress.length - 1;i++) {
            _directorAddress[i] = _directorAddress[i + 1];
        }
        _directorAddress.pop();
...
abstract contract ERC20 is IERC20, Config, ERC20Detailed {
    ...
    */
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        uint256 daoValue;
        uint256 lpValue;
        uint256 directorValue;

        if (sender == swapPair) {  //buy
            
            _bonus(sender, recipient, amount);
            burnValue = amount * burnFee[0] / 100;
            daoValue = amount * daoFee[0] / 100;
            lpValue = amount * lpFee[0] / 100;
            amount = amount.sub(amount.mul(getBuyFee()).div(1000));
        } else if (recipient == swapPair) {    //sell

            burnValue = amount * burnFee[1] / 100;
            daoValue = amount * daoFee[1] / 100;
            lpValue = amount * lpFee[1] / 100;
            directorValue = amount * directorFee / 100;
            amount = amount.sub(amount.mul(getSellFee()).div(1000));
        }

        _tokenBurn(sender, burnValue);
        _tokenTransfer(sender, daoPoolAddress, daoValue);
        _tokenTransfer(sender, lpPoolAddress, lpValue);
        _tokenTransfer(sender, directorPoolAddress, directorValue);
    ...
    function _bonus(address sender, address recipient, uint256 value) internal {
        ...
        for(uint i=0;i<refereeFee.length;i++) {
            member = _referee[member] == address(0) ? daoPoolAddress : _referee[member];
            _tokenTransfer(sender, member, value.mul(refereeFee[i]).div(1000));
        }
    }
...
contract Token is ERC20 {
    ...
    */
    constructor () ERC20Detailed("ALL EMBRACING", "ACH", 18) {
