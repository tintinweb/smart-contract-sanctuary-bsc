...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    function _baseURI() internal pure virtual returns (string memory) {
        return "https://backend.cryptoplants.club/api/nft/land/";
    }
    ...
    function _mintLand(address to,uint256 tokenId,uint256 landId) internal virtual {
        ...
        Land memory _land = Land({landId: landId, birthTime: uint64(now)});
...
contract cpcLandsContract is ERC721Pausable, AccessControl, Ownable {
    ...
    function mintLand(uint256 num, address to) external onlyOwner {
        uint256 supply = totalSupply();
        require(to != address(0));
        require(rangeOfId.length > 0, "INSUFFFICIENT LAND ID");
        require( num > 0, "You cant mint 0 land");
        require( num <= 110, "You can mint a maximum of 100 lands");
        require( supply + num < 110 - _reserved, "Your selected amount exceeds the maximum land supply.");


        for(uint256 i = 1; i <= num; i++) {
            uint256 tokenId = nextTokenId;
            uint256 index = _randomlandId();
            _mintLand(to, tokenId, supply + i);
            _remove(index);
            nextTokenId++;
        }
    }
    ...
    function getLand(uint256 _id)
        public
        view
        returns (uint256 birthTime, uint256 landId)
    {
        Land storage land = lands[_id - 1];

        birthTime = uint256(land.birthTime);
        landId = land.landId;
    }
