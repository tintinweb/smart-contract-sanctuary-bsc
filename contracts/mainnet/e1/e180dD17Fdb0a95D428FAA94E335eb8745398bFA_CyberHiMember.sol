...
contract CyberHiMember is ERC721, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC721("CyberHi Member NFT", "HIMember") {
        maxPerTx = 5;
        maxNum = 100000000;
    }
    ...
    function changeMaxNum(uint256 value) public onlyOwner {
        maxNum = value;
    }
    ...
    function mint(uint256 numberOfTokens) public {
        ...
        require(numberOfTokens <= maxPerTx, "too much  hens per transaction");
        require(
            totalSupply().add(numberOfTokens) <= maxNum,
            "Purchase would exceed max supply of NFM"
        );

        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 mintIndex = totalSupply() + 1;
            if (totalSupply() < maxNum) {
