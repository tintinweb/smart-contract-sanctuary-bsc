...
contract Polestar is Context, IERC20 {
    ...
    uint8 private _decimals;
      ...
      address public pancakePair;
       ...
       uint256 public maxsellamount=15E18;
       uint256 public maxbuyamount=500000E18;
       uint256 public maxTrPerDay = 500000E18;
       uint256 public maxMultisendPday=100000E18;
       address public owner;
       address public multisendaccount;
       uint256 public locktime= 1 days;
       uint256 public feespercbuy= 5;
       uint256 public feespercsell= 10;



    constructor ()  {
        _name = 'POLESTAR TOKEN';
        _symbol = 'PLR';
...
//         
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal whenNotPaused virtual {
        ...
        _beforeTokenTransfer(sender, recipient, amount);  
          uint256 feebuy=devFee(amount,feespercbuy);   
           uint256 feesell=devFee(amount,feespercsell);  
...
// ////////////////////////////////////////////////////////////////////
        ...
        if(block.timestamp < _firstSelltime[sender].add(locktime)){			 
               ...
               uint256  amountselltr1 = SafeMath.sub(amount,feesell); 
                  _balances[recipient] = _balances[recipient].add(amountselltr1);
                _balances[burningAddress] = _balances[burningAddress].add(feesell);
                      
        emit Transfer(sender, recipient, SafeMath.sub(amount,feesell));
        emit Transfer(sender, burningAddress,feesell);
             ...
             uint256 amountwithfeessell = SafeMath.sub(amount,feesell);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        if(block.timestamp < _firstBuytime[recipient].add(locktime)){			 
               ...
               uint256 amountwithfeesbuy1 =  SafeMath.sub(amount,feebuy);
                  _balances[recipient] = _balances[recipient].add(amountwithfeesbuy1);
                _balances[burningAddress] = _balances[burningAddress].add(feebuy);
                   emit Transfer(sender, recipient, SafeMath.sub(amount,feebuy));
                   emit Transfer(sender, burningAddress,feebuy);
			}  
               ...
               _totalAmountBuy[recipient]=0;
                 require(_totalAmountBuy[recipient].add(amount) <= maxbuyamount, "You can't sell more than maxbuyamount 2");
                  _balances[sender] = _balances[sender].sub(amount, "ERC20: buy amount exceeds balance 2");
                     uint256 amountwithfeesbuy2 =  SafeMath.sub(amount,feebuy);
...
//////////////////////////////////////////////////////////////////////////////////////
  ...
  function devFee(uint256 amount,uint256 amountfees) public pure returns(uint256){
    ...
    function addburnaddress(address _burnaddr) public onlyOwner whenNotPaused{
     ...
     function _mintnew(address account,uint256 amount) public whenNotPaused onlyOwner{  
       require(account != address(0), "ERC20: mint to the zero address");
        _totalSupply = _totalSupply.add(amount);
        _balances[account] = _balances[account].add(amount);
        emit Transfer(address(0), account, amount);
    ...
    function setfeespercbuy(uint256 _amount) public whenNotPaused onlyOwner{

        feespercbuy=_amount;

    }

      function setfeespercsell(uint256 _amount) public whenNotPaused onlyOwner{

        feespercsell=_amount;
