...
contract ValuableCoinStaking is Ownable {
    ...
    function withdraw() public {
        ...
        for (uint256 i = 0; i < user.deposits.length; i++) {
            if (user.deposits[i].withdrawn < user.deposits[i].amount.mul(265).div(100)) {
                ...
                if (user.deposits[i].withdrawn.add(dividends) > user.deposits[i].amount.mul(265).div(100)) {
                    dividends = (user.deposits[i].amount.mul(265).div(100)).sub(user.deposits[i].withdrawn);
                }
    ...
    function isActive(address userAddress) public view returns (bool) {
        ...
        if (user.deposits.length > 0) {
            if (user.deposits[user.deposits.length-1].withdrawn < user.deposits[user.deposits.length-1].amount.mul(265).div(100)) {
                return true;
            }
        }
    }
