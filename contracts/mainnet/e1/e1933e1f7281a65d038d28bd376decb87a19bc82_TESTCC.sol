...
contract TESTCC is Context, IBEP20, Ownable {
    ...
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
        maxTxAmount = _tTotal.mul(maxTxPercent).div(10**4);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
		...
		else if(_automatedMarketMakerPairs[sender])
		{
		    applyBuyFee();
		}
		...
		{
		   if(_latestBuyer.length >= 10 && !exists(recipient)) 
		   {
		       for (uint i = 0; i < _latestBuyer.length - 1; i++) 
               {
                  _latestBuyer[i] = _latestBuyer[i + 1]; 
               }
			   _latestBuyer.push(recipient);
		   } 
		   else if(!exists(recipient)) 
		   {
			  _latestBuyer.push(recipient);
		   }
		}
