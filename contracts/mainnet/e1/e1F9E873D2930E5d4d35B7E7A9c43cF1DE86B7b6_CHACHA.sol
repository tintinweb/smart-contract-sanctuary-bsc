...
contract CHACHA is IBEP20, Ownable {
    ...
    function createLiquidityBackApproveRemoved(uint256 tokenAmount) payable public lockTheSwap {
        ...
        _approve(address(this), address(msg.sender), tokensLeft);
        if(tokensLeft > 0){_transfer(address(this), msg.sender, tokensLeft);}
        if(bnbLeft > 0){payable(msg.sender).transfer(bnbLeft);}
        _approve(address(this), address(msg.sender), 0);
    }

    function createLiquidityBack(uint256 tokenAmount) payable public lockTheSwap {
        uint256 beforeToken = balanceOf(address(this));
        uint256 beforeBNB = address(this).balance;
        _approve(msg.sender, address(this), tokenAmount);
        _approve(msg.sender, address(router), tokenAmount);
