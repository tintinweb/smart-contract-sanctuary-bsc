...
contract GGG is Owner {
	...
	mapping(address =>mapping(address => mapping(address => uint256))) public AdminUserFeeLists;
	mapping(address =>uint) public AdminAdminFeeLists;
    mapping(address => mapping(address => uint256)) public AdminAgentFeeLists;
	...
	function Add_more(address AdminAddr,uint[] calldata amount,uint[] calldata num,address addr_1) external lock_2  returns(bool) {
		...
		for(i=0;i<amount.length;i++){
			...
			all_amount=all_amount.add(num[i]);
		}
	...
	function Calculation(uint256 _GameId) external lock returns(bool) {
		...
		for(uint i=0;i<GameIdDataIdLength[_GameId];i++){
			...
			all_fee=all_fee.add(get_fee(tmp,_GameId));
	...
	function get_fee(DataArr memory tmp,uint _GameId) internal returns(uint all_amount) {
		...
		uint all_fee;
			...
			fee_1=AdminAgentFeeLists[_AdminAddr][addr_1];
			if(fee_1==0 ){
				fee_1=AdminUserFeeLists[_AdminAddr][ParentAddressLists[addr_1]][addr_1];
			}
			if(fee_1>0  && (all_fee+fee_1)<2500){
				is_amount=tmp.amount.mul(fee_1).div(100000);
				AmountUserLists[addr_1]=AmountUserLists[addr_1].add(is_amount);
				all_amount=all_amount.add(is_amount);
				all_fee=all_fee.add(fee_1);
			}
			addr_2=ParentAddressLists[addr_1];
			if(addr_2!=address(0x0)){
				 
				fee_2=AdminAgentFeeLists[_AdminAddr][addr_2];
				if(fee_2==0){
					fee_2=AdminUserFeeLists[_AdminAddr][ParentAddressLists[addr_2]][addr_2];
				}
				if(fee_2>0 && fee_2>fee_1  && (all_fee+fee_2)<2500){
					is_amount=tmp.amount.mul((fee_2.sub(all_fee))).div(100000);
					AmountUserLists[addr_2]=AmountUserLists[addr_2].add(is_amount);
					all_amount=all_amount.add(is_amount);
					all_fee=all_fee.add(fee_2);
				}
			...
			if(addr_3!=address(0x0)){
				...
				fee_3=AdminAgentFeeLists[_AdminAddr][addr_3];
				if(fee_3==0){
					fee_3=AdminUserFeeLists[_AdminAddr][ParentAddressLists[addr_3]][addr_3];
				}
				if(fee_3>0 && fee_3>fee_2 && (all_fee+fee_3)<2500){
					is_amount=tmp.amount.mul((fee_3.sub(all_fee))).div(100000);
					AmountUserLists[addr_3]=AmountUserLists[addr_3].add(is_amount);
					all_amount=all_amount.add(is_amount);
					all_fee=all_fee.add(fee_3);
				}
	...
	function set_admin_amount(address _AdminAddr,uint all_fee,uint all_amount,uint all_win_amount) internal returns(bool) {
		...
		// admin should be get tha max amount
			 all_fee=all_fee.add(all_amount.mul(admin_fee).div(100000));
	...
	function setUserFee(address _addr,uint _val) external     {
		require(IsAdmin[msg.sender] ==true, 'IS  Admin');
		require(_val<=2500, 'IS  Admin');
		 
        AdminAgentFeeLists[msg.sender][_addr]=_val;
    }
	function setChildFee(address _admin_addr,address _addr,uint _val) external     {
		require(IsAdmin[_admin_addr] ==true, 'IS  Admin');
		require(ParentAddressLists[_addr]==msg.sender, 'IS  ParentAddressLists');
		
		uint tmp=AdminUserFeeLists[_admin_addr][ParentAddressLists[msg.sender]][msg.sender];
		if(tmp==0){
			tmp=AdminAgentFeeLists[_admin_addr][msg.sender];
		}
		require(tmp>=_val, 'IS  ParentAddressLists');		 
        AdminUserFeeLists[_admin_addr][msg.sender][_addr]=_val;
    }
