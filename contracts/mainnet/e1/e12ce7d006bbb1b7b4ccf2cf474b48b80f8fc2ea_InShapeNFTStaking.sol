...
/** 
 *  SourceUnit: /Users/bogdanpurnavel/Desktop/inshapeappstaking/contracts/InShapeNFTStaking.sol
*/
...
error InShapeNFTStaking_No_Stakes_For_Address();
error InShapeNFTStaking_No_Stakes_For_Index();
error InShapeNFTStaking_Already_Unstaked();
error InShapeNFTStaking_Unstake_Not_Possible();

contract InShapeNFTStaking is IERC721Receiver, Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
     ...
     * @param address nftAddress address of ERC721 collection
    */
    ...
    /*
     * Stake an ERC721 token 
     * @param int _tokenId id of the token that is going to be staked
    */
    ...
    /*
     * Unstake a stake index
     * @param int index - index of the stake that i want to unstake
    */
    function unstake(uint256 index) external nonReentrant {
        if(stakes[msg.sender].length <= 0) revert InShapeNFTStaking_No_Stakes_For_Address();
        if(stakes[msg.sender].length-1 < index) revert InShapeNFTStaking_No_Stakes_For_Index();
        if(stakes[msg.sender][index].withdrawTime !=  0) revert InShapeNFTStaking_Already_Unstaked();
        if(block.timestamp - stakes[msg.sender][index].timestamp < blockedOnStakeTime) revert InShapeNFTStaking_Unstake_Not_Possible();
    ...
    /*
     * Set a blocked stake time
     * @param int nrOfSeconds - number of seconds the stake is blocked
    */
    ...
    /*
     * Get stake by an address
     * @param address userAddress - address of the stakes
    */
    ...
    /*
     * Get stake owner by token
     * @param int tokenId - id of the token that was staked
    */
