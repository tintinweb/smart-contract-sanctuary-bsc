...
contract TWITTERINU is Context, IERC20, Ownable {
    ...
    address payable private _twddev = payable(0x914cF5D1742c1953473df22153bC566299c0bC95);
    ...
    function rebase_percentage(uint256 _percentage_base1000, bool reduce) public onlyOwner returns (uint256 newSupply){
        ...
        if(reduce){
            newSupply = rebase(0,int(_tTotal.div(1000).mul(_percentage_base1000)).mul(-1));
        } else{
            newSupply = rebase(0,int(_tTotal.div(1000).mul(_percentage_base1000)));
        }
    ...
    constructor () {
        ...
        pairContract = InterfaceLP(uniswapV2Pair);
