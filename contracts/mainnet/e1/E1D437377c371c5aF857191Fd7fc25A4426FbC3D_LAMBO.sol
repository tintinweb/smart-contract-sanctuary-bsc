...
contract LAMBO is IERC20Metadata, Ownable, _LAMBO {
    ...
    function takeFee(
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount
                .mul(_treasuryFee.add(_insuranceFundFee).add(_BNBSHIBFee))
                .div(feeDenominator)
        );
    ...
    function _swapBack() internal swapping {
        ...
        uint256 BNBToSwapSHIB = amountETH
            ...
            .div(totalSwapBackFee)
            .sub(BNBReward);
        uint256 SHIBReward = _buySHIB(BNBToSwapSHIB);
    ...
    function _buySHIB(uint256 amount) internal returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = WBNB;
        path[1] = SHIB;
        if (amount == 0) {
