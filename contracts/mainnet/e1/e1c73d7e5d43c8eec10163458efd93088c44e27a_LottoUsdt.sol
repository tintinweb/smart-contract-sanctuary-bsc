...
contract LottoUsdt is ReentrancyGuard{
    ...
    address private usdtminer = 0x2c9571De16B18Efc973146C187470156836A7236; //USDTminer addres
    address private dev = 0x1c72cAAc86BFef423CFC229459eFD03279127462;
...
for (uint i=0; i<arrayLength2; i++) {
  ...
  if (participants[i]==ad){
    ...
    function pickwinner() internal {
        ...
        uint256 minerUsdtFee = SafeMath.div(SafeMath.mul(contractBalance,26),100);
        uint256 devFee = SafeMath.div(SafeMath.mul(contractBalance,4),100);
      
        UsdtInterface.transfer(usdtminer,minerUsdtFee);
        uint256 winnerAmount = SafeMath.sub(contractBalance,minerUsdtFee);
        winnerAmount -= devFee;
