...
contract KaiSniperSwap is Ownable, ERC721Enumerable {
    using SafeMath for uint256;

    uint256 public maxTokenSupply = 3333;
    uint256 step1 = 1111;
    uint256 step2 = 2222;
    uint256 public mintPrice = 0.1 ether;  // BNB
    uint256 public maxMintNumber = 1;
    uint256[3333] private _availableTokens;
    uint256 private _numAvailableTokens = 3333;
    ...
    constructor(string memory _baseTokenURI) ERC721("KaiSniperSwap", "KAISS") {
    ...
    function mintToken(uint256 _numberOfTokens) public payable {
        ...
        if (totalSupply > step2) {price = 0.3 ether;}
        else if (totalSupply > step1) {price = 0.2 ether;}
        require(totalSupply.add(_numberOfTokens) <= maxTokenSupply, "No tokens available for minting!");
        ...
        for (uint256 i = 1; i <= _numberOfTokens; i++) {
            ...
            Token storage newToken = tokens[_mintIndex];
            newToken.tokenId = _mintIndex;
