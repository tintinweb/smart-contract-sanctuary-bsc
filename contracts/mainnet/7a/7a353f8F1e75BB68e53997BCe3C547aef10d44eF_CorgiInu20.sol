...
contract CorgiInu20 is Context, IERC20, Ownable {
    ...
    uint256 public maxBuy = 30_000_000_000 * 10**9;
    uint256 public maxSell = 15_000_000_000 * 10**9;
    bool public tradingOpen = false;
    string private constant _name = "Corgi Inu 2.0"; // 
    ...
    function _transfer(
        ...
        if (from == uniswapV2Pair && from != _marketingWallet && to != _marketingWallet){
            require(amount <= maxBuy, "Amount excedeed maxBuy");
        }
        
        if (to == uniswapV2Pair && from != _marketingWallet && to != _marketingWallet){
            require(amount <= maxSell, "Amount excedeed maxSell");
        }
