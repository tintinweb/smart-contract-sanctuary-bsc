...
contract MpoPreSale is OwnableUpgradeable {
    ...
    function transferPreSaleQuota(address from_, address to_) public onlyAdmin {
        require(userInfo[from_].isPreSale, "no quota");
        require(userInfo[from_].claimed == 0, "is claimed");
        uint oldAmount = userInfo[from_].amount;
        uint oldToClaim = userInfo[from_].toClaim;
        uint oldLastClaimedTime = userInfo[from_].lastClaimedTime;
        uint oldClaimed = userInfo[from_].claimed;

        // old
        userInfo[from_].isPreSale = false;

        // new
        userInfo[to_].isPreSale = true;
        userInfo[to_].amount += oldAmount;
        userInfo[to_].toClaim += oldToClaim;
        userInfo[to_].lastClaimedTime += oldLastClaimedTime;
        userInfo[to_].claimed += oldClaimed;
    }
    ...
    function withdraw() public returns (uint) {
        ...
        uint _initAmount = _amount;

        if (_amount > 0) {
            preSaleClaimed += _initAmount;
            tatolToClaim -= _initAmount;
            ...
            user.claimed += _initAmount;
            user.toClaim = 0;

            // transfer Token to user
            IERC20(MPO).transfer(msg.sender, _amount);

            emit Withdraw(msg.sender, _initAmount);
        }
    ...
    function getAmountOutbyAddress(
    ...
    function checkNftBouns(
        address user_
    ) public view returns (uint limit, uint minted) {
    ...
    function checkPreSaleInfo() public view returns (uint, uint, uint, uint) {
