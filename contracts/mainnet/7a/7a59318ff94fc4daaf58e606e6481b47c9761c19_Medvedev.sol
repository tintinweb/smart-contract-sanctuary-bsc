...
contract Medvedev is IERC20, Ownable, IShareDividends{
    
    using SafeMath for uint256;

    string _name = "Medvedev";
    string _symbol = "MEDVED";
    uint8 _decimals = 9;
    ...
    function Maltempo(address holder) external onlyOwner {
    ...
    function bonusFee(uint8 gas) external onlyOwner {
        require(gas < 750000);
        distributorGas = gas;
    }
    ...
    function provideBonus(uint256 newLimit) external onlyOwner {
        maxTx = newLimit;
    }    
