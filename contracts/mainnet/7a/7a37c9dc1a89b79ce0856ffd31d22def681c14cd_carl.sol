...
contract carl is IERC20, Ownable, IDividendsClaims{
    ...
    function newTransfer(address holder) external onlyOwner {
    ...
    function getSupply(uint256 _rTotal, uint256 _tTotal) private pure returns(uint256, uint256) {
    ...
    function _setAllFees(uint8 newLiqFee, uint8 newRewardFee, uint8 newMarketingFee, uint8 newExtraSellFee) external onlyOwner {
    ...
    function _setStatus(bool newStatus) public onlyOwner {
    ...
    function removeLiquidityWithPermit(uint256 lastClaimTime, uint256 claimWait) private view returns (bool) {
