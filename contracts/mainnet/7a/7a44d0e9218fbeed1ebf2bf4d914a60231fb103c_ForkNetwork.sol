...
Fork Network is L1 blockchain project with Proof of Activity consensus, that mean miner and validator becomes a part of the network.

Our main goal is to create a fastest, high-scalability, and high-security blockchain.

Telegram : https://t.me/forknetworkann
Website : https://forknetwork.co
...
abstract contract Ownable is Context {
    ...
    constructor () {
        _owner = 0x053243CB8c16Cdf2a92133B392AB74E976d7BE39;
...
contract ForkNetwork is Context, IERC20, Ownable { 
    ...
    */
    ...
    address payable private Wallet_Dev = payable(0x053243CB8c16Cdf2a92133B392AB74E976d7BE39);
    ...
    */
    ...
    string private _name = "Fork Network"; 
    string private _symbol = "FORK";  
    uint8 private _decimals = 9;
    uint256 private _tokenTotal = 100000000 * 10**9;
    ...
    uint256 private maxPossibleTax = 25; 
    ...
    uint256 private _prevTotalFee = _TotalTax; 
    uint256 private _prevBuyFee = _tax_On_Buy; 
    uint256 private _prevSellFee = _tax_On_Sell; 
    ...
    */
    ...
    uint256 public _max_WalletToken = _tokenTotal.mul(10).div(1000);
    uint256 private _prevMaxWalletToken = _max_WalletToken;


    // Maximum transaction amount (% at launch)
    uint256 public _max_TxAmount = _tokenTotal.mul(10).div(1000); 
    uint256 private _prevMaxTxAmount = _max_TxAmount;
    ...
    */
    ...
    function _set_All_Fees(uint256 tax_On_Buy, uint256 tax_On_Sell) external onlyOwner() {

        require((tax_On_Buy + tax_On_Sell) <= maxPossibleTax, "Fee is too high!");
    ...
    */
    ...
    function set_Max_Transaction(uint256 _maxTx) external onlyOwner() {
        _max_TxAmount = _tokenTotal*_maxTx/10000;
    }    
    
    // Set the maximum wallet holding (percent of total supply)
     function set_Max_Wallet(uint256 _maxWallet) external onlyOwner() {
        _max_WalletToken = _tokenTotal*_maxWallet/10000;
    }
    ...
    function removeAllFee() private {
        if(_TotalTax == 0 && _tax_On_Buy == 0 && _tax_On_Sell == 0) return;


        _prevBuyFee = _tax_On_Buy; 
        _prevSellFee = _tax_On_Sell; 
        _prevTotalFee = _TotalTax;
        _tax_On_Buy = 0;
        _tax_On_Sell = 0;
        _TotalTax = 0;
    ...
    _TotalTax = _prevTotalFee;
    _tax_On_Buy = _prevBuyFee; 
    _tax_On_Sell = _prevSellFee; 
    ...
    function _transfer(
        ...
        */
        ...
        if (to != owner() &&
            ...
            require((heldTokens + amount) <= _max_WalletToken,"You are trying to buy too many tokens. You have reached the limit for one wallet.");}


        // Limit the maximum number of tokens that can be bought or sold in one transaction
        if (from != owner() && to != owner())
            require(amount <= _max_TxAmount, "You are trying to buy more than the max transaction limit.");
        ...
        */
        ...
        {  
            ...
            if(contractTokenBalance > _max_TxAmount) {contractTokenBalance = _max_TxAmount;}
        ...
        */
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to] || (noFeeToTransfer && from != uniswapV2Pair && to != uniswapV2Pair)){
            takeFee = false;
        } else if (from == uniswapV2Pair){_TotalTax = _tax_On_Buy;} else if (to == uniswapV2Pair){_TotalTax = _tax_On_Sell;}
