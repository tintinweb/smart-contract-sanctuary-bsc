...
contract GIZMO is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public maxBuyTranscationAmount = 25000000 * (10**18); // 2.5%
    uint256 public maxSellTransactionAmount = 25000000 * (10**18);
    uint256 public swapTokensAtAmount = 20000 * (10**18);
    uint256 public _maxWalletToken = 45000000 * (10**18); // 4.5% of total supply
    uint256 public treasuryFee = 10;
    uint256 public _burnFee = 2;
    uint256 public _maxTax = 12;
    ...
    event TreasuryUpdated(address indexed newLiquidityWallet, address indexed oldLiquidityWallet);
    event FixedSaleBuy(address indexed account, uint256 indexed amount, bool indexed earlyParticipant, uint256 numberOfBuyers);

    constructor() public ERC20("Gizmo", "GIZMO") {
    	treasury = owner();
        ...
        // enable owner and fixed-sale wallet to send tokens before trading enabled
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "GIZMO: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "GIZMO: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "GIZMO: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "GIZMO: Automated market maker pair is already set to that value");
        automatedMarketMakerPairs[pair] = value;
        emit SetAutomatedMarketMakerPair(pair, value);
    }

    function updateTreasury(address newtreasury) public onlyOwner {
        require(newtreasury != treasury, "GIZMO: The marketing wallet is already this address");
        excludeFromFees(newtreasury, true);
        treasury = newtreasury;
        emit TreasuryUpdated(newtreasury, treasury);
    }
    ...
    function _getSwapTokensAtAmount() private view returns (uint256) {
    ...
    function setBurnFee(uint256 newBurnFee) public onlyOwner {
        uint256 newTotal = newBurnFee + treasuryFee;
        require(newTotal <= _maxTax, "Max Tax is 12%");
        _burnFee = newBurnFee;
    }

    function setTreasuryFee(uint256 newTreasuryFee) public onlyOwner {
        uint256 newTotal = newTreasuryFee + _burnFee;
        require(newTotal <= _maxTax, "Max Tax is 12%");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "GIZMO: This account cannot send tokens until trading is enabled");
        }
        ...
        if(
            ...
            swapping = true;
            swapTokensForUSDT(contractTokenBalance, treasury);
            swapping = false;
        }
        ...
        if(takeFee) {
        	uint256 fees = amount.mul(treasuryFee).div(100);
        	amount = amount.sub(fees);

            uint256 toBurn = amount.mul(_burnFee).div(100);
            _burn(from, toBurn);
