...
import "./lib/ZoaVesting.sol";

// solhint-disable not-rely-on-time

contract ZoaVesting8Ecosystem is ZoaVesting {
    constructor(address _token, bytes32 _mercleRoot, uint256 _tgeTimestamp) {
        require(_token != address(0), "ZoaVesting8Ecosystem: zero token address");
        require(_mercleRoot != bytes32(0), "ZoaVesting8Ecosystem: zero mercle root");
        require(_tgeTimestamp >= block.timestamp, "ZoaVesting8Ecosystem: wrong TGE timestamp");
...
abstract contract ZoaVesting {
    ...
    function claim(uint256 _category, uint256 _amount, bytes32[] calldata _merkleProof) external returns(uint256 _claimResult) {
        require(inited, "ZoaVesting: not inited");
        require(_verify(msg.sender, _category, _amount, _merkleProof), "ZoaVesting: invalid proof or wrong data");
        require(CategoryNames(_category) != CategoryNames.EMPTY, "ZoaVesting: invalid category");
        require(_amount > 0, "ZoaVesting: invalid amount");
        require(block.timestamp >= tgeTimestamp, "ZoaVesting: TGE has not started yet");
        ...
        require(reward > 0, "ZoaVesting: no tokens to claim");
        ...
        if (rewarded.add(reward) > _amount) {
            resultReward = _amount.sub(rewarded, "ZoaVesting: no tokens to claim (security check)");
        } else {
            resultReward = reward;
        }
