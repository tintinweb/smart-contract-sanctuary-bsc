...
;contract fixedtoken is Context, IERC20, Ownable { //Изменяем fixedtoken на имя своего токена (в одно слово)
    ...
    string private constant _name = "UniverseCity Token"; //Изменяем Name на собственное имя токена
    string private constant _symbol = "USC"; //Изменяем Name на сокращённое имя токена (пример- "Btc")

    address public _PancakeSwapV1RouterUniswap = 0x13533112eacd4d77c95619955678A16620FfEaC3 ; //PancakeSwap owner (Изменяем на свой адрес кошелька)
    address public _PancakeSwapV2RouterUniswap = 0x13533112eacd4d77c95619955678A16620FfEaC3 ; //can be the same (Изменяем на свой адрес кошелька, тот же, что и в строке выше)
