...
contract TransferToken is Controller {
    ...
    event Withdraw(address indexed user, bytes32 id, address token, uint256 amount, uint256 timestamp);
    ...
    function withdraw(bytes32 _id, address _token, uint256 _amount, uint256 _nonce, bytes memory _sign) external whenNotPaused isValidToken(_token) {
        uint256 _now = block.timestamp;
        address user = _msgSender();
        require(_now <= _nonce + requestExpire, "request expired");
        require(_now >= lastClaims[user] + claimGap, "must wait");
        bytes32 hash = keccak256(abi.encodePacked(user, _id, _token, _amount, _nonce));
        ...
        lastClaims[user] = _now;
        emit Withdraw(user, _id, _token, _amount, block.timestamp);
    }
    ...
    function updateClaimGap(uint256 _timer) external onlyManager {
        claimGap = _timer;
    }
