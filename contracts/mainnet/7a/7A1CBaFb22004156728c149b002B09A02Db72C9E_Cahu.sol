...
contract Cahu is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 3000 ether;
    // Initial distribution for cahu reward pool
    uint256 public constant INITIAL_CAHU_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the CAHU ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Cahu.com", "CAHU") {
        // Mints 1 CAHU to contract creator for initial pool setup
    ...
    function _getCahuPrice() internal view returns (uint256 _cahuPrice) {
        try IOracle(cahuOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Cahu: failed to fetch CAHU price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _cahuPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_cahuPrice >= taxTiersTwaps[tierId]) {
                    require(taxTiersRates[tierId] < 1000, "tax equal or bigger to 10%");
    ...
    function setCahuOracle(address _cahuOracle) public onlyOperatorOrTaxOffice {
        require(_cahuOracle != address(0), "oracle address cannot be 0 address");
        cahuOracle = _cahuOracle;
    }
    ...
    /**
     * @notice Operator mints CAHU to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of CAHU to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentCahuPrice = _getCahuPrice();
            currentTaxRate = _updateTaxRate(currentCahuPrice);
            if (currentCahuPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_cahuPool != address(0), "!_cahuPool");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_cahuPool, INITIAL_CAHU_POOL_DISTRIBUTION);
    }
