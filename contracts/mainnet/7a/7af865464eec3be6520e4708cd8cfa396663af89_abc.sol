...
contract abc is ERC20, Ownable, LockToken {
    ...
    uint256 public maxTransactionAmount = 500000 * (10**9);
    uint256 public swapTokensAtAmount = 500000 * (10**9);
    uint256 public maxWalletToken = 1500000 * (10**9);

    IUniswapV2Router02 public uniswapV2Router;
    address public immutable uniswapV2Pair;

    address payable public marketingWallet = payable(0xC9B10DaaaEF92fa76a310632B7c257d0bb3903FA); 
    address payable public devWallet = payable(0x8675b296cC7a324527eb9C0Fe91953c340f374b1);
    ...
    function _transfer(
        ...
        if(overMinTokenBalance && !inSwapAndLiquify && to==uniswapV2Pair && swapAndLiquifyEnabled) {
            require(sellLiquidityFee.add(sellMarketingFee).add(sellDevFee) > 0);
     ...
     function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 totalFees = sellLiquidityFee.add(sellMarketingFee).add(sellDevFee);
        uint256 tokensForLiquidity = contractTokenBalance.mul(sellLiquidityFee).div(totalFees);
        ...
        devWallet.transfer(address(this).balance.mul(sellDevFee).div(totalFees.sub(sellLiquidityFee)));
    ...
    function setMaxTxAmount(uint256 _maxTx) public onlyOwner {
        ...
        require(maxTransactionAmount >= totalSupply().div(1000), "value too low");
    }
    ...
    function updateBuyFees(uint256 _liquidityFee, uint256 _marketingFee, uint256 _devFee) public onlyOwner {
        require(_liquidityFee.add(_marketingFee).add(_devFee) <= 25, "tax too high");
