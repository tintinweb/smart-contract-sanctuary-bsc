...
contract Metadog is Context, IERC20, Ownable {
    ...
    string private _name = "Metadog Racing Utility Token";
    string private _symbol = "MDOG";
    ...
    function calculateLiquidityPlusFees(uint256 _amount) private view returns (uint256) {
        ...
        return _amount.mul(_liquidityFee + _devFee + _marketingFee + _buyBackFee + _stakingFee + _tokenFee + sellFee).div(
            10**4
        );
    }
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _marketingFee == 0 && _devFee == 0 && _buyBackFee == 0 && _stakingFee == 0 && _tokenFee == 0 && _sellFee == 0) return;
    ...
    function _getFeeInfo() private view returns (uint256 [6] memory fees, address [4] memory feeAddresses) {
        ...
        feeAddresses[3] = _tokenFeeAddress;
    }
