...
contract Geyser is IStaking, Ownable {
    ...
    bool public _stakeEnable = false;
    uint256 public _lockTerm = 0 days;
    ...
    function _stakeFor(address staker, address beneficiary, uint256 amount) private {
        ...
        require(_stakeEnable, 'Stake not enabled yet');
        require(beneficiary != address(0), 'TokenGeyser: beneficiary is zero address');
        require(totalStakingShares == 0 || totalStaked() > 0,
                'TokenGeyser: Invalid state. Staking shares exist, but no staking tokens do');

        uint256 transAmount = amount.mul(100 - _taxFee).div(100);

        uint256 mintedStakingShares = (totalStakingShares > 0)
            ? totalStakingShares.mul(transAmount).div(totalStaked())
            : transAmount.mul(_initialSharesPerToken);
    ...
    function _unstake(uint256 amount) private returns (uint256) {
        ...
        require(block.timestamp.sub(accountStakes[accountStakes.length - 1].timestampSec) >= _lockTerm, 'You need to wait more to unstake.');
    ...
    function updateLockTerm(uint256 lockTerm) external onlyOwner {
        _lockTerm = lockTerm;
    }

    function withdrawToken() external onlyOwner {
        _lockedPool.transfer(msg.sender, totalLocked());
    }
}