...
contract SwapMining is Ownable {
	...
	// Trade Reward Transaction Limits
	uint256 public constant MAX_TRADE_REWARD = 1000 ether;
	uint256 public minTradeReward;

	constructor(address _robustToken, address _factory, address _oracle, address _router, address _targetToken, uint256 _startBlock) public {
    ...
    function swap(address account, address input, address output, uint256 amount) public onlyRouter returns (bool) {
        ...
        if (quantity <= 0 || quantity < minTradeReward) {
    ...
    function getQuantity(address outputToken, uint256 outputAmount, address anchorToken) public view returns (uint256) {
        ...
        } else if (IRobustSwapFactory(factory).getPair(outputToken, anchorToken) != address(0)) {
    ...
    function takerWithdraw() public {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            if (user.quantity > 0) {
                ...
                uint256 userReward = pool.allocRBSAmount.mul(user.quantity).div(pool.quantity);
    ...
    function getUserReward(uint256 _pid, address _user) public view returns (uint256, uint256) {
        ...
        if (user.quantity > 0) {
            ...
            uint256 blockReward = getRbsReward();
            uint256 rbsReward = blockReward.mul(pool.allocPoint).div(totalAllocPoint);
            userSub = userSub.add((pool.allocRBSAmount.add(rbsReward)).mul(user.quantity).div(pool.quantity));
        }
		
        //RBS available to users, User transaction amount
    ...
    function getReward(address _user) public view returns (uint256, uint256) {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            if (user.quantity > 0) {
                ...
                rbsReward = blockReward.mul(pool.allocPoint).div(totalAllocPoint);
                userSub = userSub.add((pool.allocRBSAmount.add(rbsReward)).mul(user.quantity).div(pool.quantity));
            }
        }
	...
	function getPoolInfo(uint256 _pid) public view returns (address, address, uint256, uint256, uint256, uint256) {
		...
		return (token0, token1, rbsAmount, pool.totalQuantity, pool.quantity, pool.allocPoint);
	}
	...
	function setMinTradeReward(uint256 _minTradeReward) external onlyOwner {

		require(_minTradeReward <= MAX_TRADE_REWARD, "SwapMining: Value should be less than MAX");
		minTradeReward = _minTradeReward;
	}
}
