...
/*  bytes32 internal constant _rebaseTime_  = "rebaseTime";
   ...
   /* function migrate(address vault_) external governance {
    ...
    function take(uint makeID, uint volume,string memory link,uint price) virtual external nonReentrant returns (uint takeID, uint vol) {
        (takeID,vol) = ArbitrateLib.takeLib(makes,makeExs,takes,config,biddingN,makeID,volume,link,price);
        takesN++;
...
/*  bytes32 internal constant _rebaseTime_  = "rebaseTime";
    ...
    function takeLib(mapping (uint => SMake) storage makes,mapping(uint =>SMakeEx) storage makeExs,mapping (uint => STake) storage takes,mapping (bytes32 => uint) storage config,mapping (address => uint) storage biddingN,uint makeID, uint volume,string memory link,uint price) virtual external returns (uint takeID, uint vol) {
        DOTC dotc = DOTC(address(this));
        require(makes[makeID].maker != address(0), 'Nonexistent make order');
        require(makes[makeID].remain > 0, 'make.remain should > 0');
        require(makes[makeID].minVol <= volume , 'volume must > minVol');
        ...
        if (makeExs[makeID].tradingMargin>0){//config[_tradingPool_]
            ...
            if(IERC20(address(config[_feeToken_])).allowance(address(this),address(config[_tradingPool_]))<makeExs[makeID].tradingMargin)
                IERC20(address(config[_feeToken_])).approve(address(config[_tradingPool_]),uint(-1));
            ITradingStakePool(address(config[_tradingPool_])).stake(msg.sender,makeExs[makeID].tradingMargin);
        }else if (volume > dotc.getConfigA(_assetFreeLimit_,makes[makeID].asset))
        ...
        if(dataTmp.priceType!=0){
            ...
            if (dataTmp.priceType ==1){
                if (makeExs[makeID].floatVal>=0)
                    dataTmp.realPrice = dataTmp.price.mul(dataTmp.rate).div(uint(dataTmp.decimals)).add(uint(makeExs[makeID].floatVal));//1170e18 *6.71e18
                else
                    dataTmp.realPrice = dataTmp.price.mul(dataTmp.rate).div(uint(dataTmp.decimals)).sub(uint(makeExs[makeID].floatVal));//1170e18 *6.71e18
            }
            else if(dataTmp.priceType ==2){
                if (makeExs[makeID].floatVal>=0)
                    dataTmp.realPrice = dataTmp.price.mul(dataTmp.rate).div(uint(dataTmp.decimals)).mul(uint(1e18).add(uint(makeExs[makeID].floatVal))).div(1e18);//
                else
                    dataTmp.realPrice = dataTmp.price.mul(dataTmp.rate).div(uint(dataTmp.decimals)).mul(uint(1e18).sub(uint(makeExs[makeID].floatVal))).div(1e18);//
            }
            uint diff = dataTmp.realPrice>price? dataTmp.realPrice-price:price-dataTmp.realPrice;
            require(diff.mul(1000)<dataTmp.realPrice.mul(5),"price not match chainlink");
            dataTmp.realPrice = price;
        }else{
            dataTmp.realPrice  = makes[makeID].price;
        }
        takeID = dotc.takesN();
        takes[takeID] = STake(makeID, msg.sender, vol, Status.None, now.add(config[_expiry_]),link,dataTmp.realPrice);
        //takesN++;
        emit Take(takeID, makeID, msg.sender, vol, takes[takeID].expiry,link,dataTmp.realPrice);
    }
    event Take(uint indexed takeID, uint indexed makeID, address indexed taker, uint vol, uint expiry,string link,uint realPrice);
