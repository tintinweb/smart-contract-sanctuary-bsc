...
contract MatchingBonus is Ownable, ReentrancyGuard {
    ...
    function comprarCartela() public payable {
        ...
        require(jogoIniciado, "Nao iniciado");

        ultimaCartelaVendida++;

        ICryptoWorldContract(cryptoWorldContract).alterMatchingBonusActive(msgSender);
    ...
    function iniciaRodada() public onlyOwner nonReentrant {
        require(!jogoIniciado, "Ja iniciado");
        jogoIniciado = true;
        ultimaCartelaVendida = 0;
        rodada++;
    }
    ...
    function partialWithdraw(uint256 value) public onlyOwner nonReentrant {
        address msgSender = _msgSender();
...
interface ICryptoWorldContract {
    function alterMatchingBonusActive(address accountAddress) external;
}
