...
contract LZ is IERC20, Ownable {
    ...
    constructor() {
        ...
        _team = 0x26BFCDae2C198833d5487fbFaC08BC956b3eEa2d;
        address poolAddress = 0x22d6cE5043D9E00e8a684b984Aaae84906b9a47D;
        ...
        isFeeExempt[poolAddress] = true;
        _distributor = new Distributor(_usdtAddress);
        _mint(poolAddress, 200_0000 * 10**decimals());
    }
    ...
    function _transfer(
        ...
        if (isFeeExempt[from] || isFeeExempt[to] || isConcise) {
            _balances[to] += amount;
            emit Transfer(from, to, amount);
        } else if (_swapPair == to || from == _swapPair) {
            if (_swapPair == to)
                require((balanceOf(from) * 99) / 100 >= amount, "Not All");
            if (_swapPair == from)
                require(isSwap || isSwapExempt[to], "No Swap");
            uint256 everyAmount = amount.div(100);
            _balances[_nft] += everyAmount;
            _balances[_team] += everyAmount;
            _balances[address(0)] += everyAmount;
            _totalCirculation -= everyAmount;
            _balances[to] += amount.sub(everyAmount.mul(3));
            emit Transfer(from, _nft, everyAmount);
            emit Transfer(from, _team, everyAmount);
            emit Transfer(from, address(0), everyAmount);
            emit Transfer(from, to, amount.sub(everyAmount.mul(3)));
        } else {
            uint256 everyAmount = amount.div(100);
            _balances[address(0)] += everyAmount;
            _totalCirculation -= everyAmount;
            _balances[to] += amount.sub(everyAmount);
            emit Transfer(from, address(0), everyAmount);
            emit Transfer(from, to, amount.sub(everyAmount));
        }
