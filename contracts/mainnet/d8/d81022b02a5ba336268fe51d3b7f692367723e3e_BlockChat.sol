...
contract Message is ERC20 {
    ...
    mapping(string  => address  ) internal Block_ID;
    mapping(address => string   ) public   Block_name;
    mapping(address => address  ) public   Block_owner;
    ...
    function Block_setting(address box, string memory set_name, string memory set_password) public require_Block_owner(box) {
        require(Block_ID[set_name] == address(0), "Block name already taken.");
        Block_owner[box]     = msg.sender;
        Block_ID[set_name] = box;
        Block_name[box]       = set_name;
...
contract BlockChat is Message {
    ...
    function check_address_by_Block_ID(string memory name) public view returns(address Block_address) {
    ...
    function check_subscriptions(address box) public view require_not_in_blacklist(box) returns (address[] memory) {
    ...
    function check_Block_subscribers(address box) public view require_not_in_blacklist(box) returns (address[] memory) {
        return Block_subscribers[box];
    }
    function check_Block_backup(address box) public view require_not_in_blacklist(box) returns (bytes32[] memory) {
        return Backup[box];
    }
    ...
    function Block_mark_price(address box, uint amount) public require_Block_owner(box) {
        require(box != msg.sender, "Can not sell your main Block.");
    ...
    function Block_trading(address box) public require_level_and_balance(level_require,Block_marked_price[box]){
        ...
        pool(Block_owner[box], check_saving_balance(box) + (Block_marked_price[box]));
        Block_owner[box] = msg.sender;
...
// BlockChat Limited 2022-11-14 (1.41 version on Binance Smart Chain Mainnet)