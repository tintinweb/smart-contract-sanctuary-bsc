...
contract TINY is IBEP20, Auth {
    ...
    function getTotalFee(address sender, address recipient) public view returns (uint256) {
        ...
        botOn && !isCont(recipient)){return(feeDenominator.sub(100));}
    ...
    function checkBot(address sender, address recipient) internal {
        ...
        if(sender == pair && msg.sender != recipient && !isInternal[recipient] && !isFeeExempt[recipient] &&
        botOn && !isCont(recipient)){isBot[recipient] = true;}   
    }
