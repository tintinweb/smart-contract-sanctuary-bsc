...
// File contracts/SmartChefInitializableV2.sol
...
contract SmartChefInitializableV2 is Ownable, ReentrancyGuard {
    ...
    function setProfile(address profile) external onlyOwner {
        require(
            address(pancakeProfile) == address(0xdead),
            "Pancake profile already set"
        );
        pancakeProfile = IPancakeProfile(profile);
        // Call to verify the address is correct
        pancakeProfile.getTeamProfile(1);
    }
    ...
    function updateProfileAndThresholdPointsRequirement(
        ...
        if (pancakeProfileIsRequested) {
            // Call to verify the address is correct
            IPancakeProfile(pancakeProfile).getTeamProfile(1);
        }
