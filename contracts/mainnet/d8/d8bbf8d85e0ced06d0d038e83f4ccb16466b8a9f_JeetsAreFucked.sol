...
contract JeetsAreFucked is IBEP20 {
    string constant _name = "JeetsAreFucked7";
    string constant _symbol = "JAF7";
    ...
    uint256 public timeUntilJeetTaxDecrease = 10 minutes; // will be changed in the final version
    ...
    uint256 private buysToStopEvent = 3; // will be changed in the final version
    uint256 private buysUntilEvent = 3; // will be changed in the final version
    ...
    uint256 private minPartySell = 1000;
    uint256 private minPartyBuy = 1000;
    ...
    event PartyBuy(uint256 amountInBusd, uint256 buysLeftToCrash, uint256 sellsLeft);

    event Buy(uint256 amountInBusd, uint256 buysLeftToParty);
    ...
    function takeJeetTax(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if (sender == pair) {
            ...
            if (letTheJeetsOutEvent){
                ...
                emit PartyBuy(valueInBusd(amount), buysToStopEvent - buys, jeetTax - sells);
                return amount;
            } else {
                buys++; 
    ...
    function stopLettingTheJeetsOut() internal {
        emit PartyReview(partyNumber, buys, sells, jeetTax - sells > 0 ? true : false);
