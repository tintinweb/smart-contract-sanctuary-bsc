...
contract ActsToken is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "ACTs Token";
    string constant _symbol = "ACTS";
    ...
    uint256 public totalBNBProceeds;
...
//        emit Transfer(sender, recipient, amount);
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBProceeds = amountBNB.mul(proceedsFee).div(totalBNBFee);
        ...
        payable(proceedsFeeReceiver).transfer(amountBNBProceeds);
        totalBNBProceeds = totalBNBProceeds.add(amountBNBProceeds);
        payable(buybackFeeReceiver).transfer(amountBNBBuyback);

        if(amountToLiquify > 0 && autoLiquifyEnabled){
    ...
    function setAutoLiquifySettings(bool _enabled) external authorized {
    ...
    function setFees(uint256 _liquidityFee, uint256 _buybackFee, uint256 _reflectionFee, uint256 _proceedsFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        buybackFee = _buybackFee;
        reflectionFee = _reflectionFee;
        proceedsFee = _proceedsFee;
        totalFee = _liquidityFee.add(_buybackFee).add(_reflectionFee).add(_proceedsFee);
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator/4);
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _proceedsFeeReceiver, address _buybackFeeReceiver, address _liquidityFeeReceiver) external authorized {
