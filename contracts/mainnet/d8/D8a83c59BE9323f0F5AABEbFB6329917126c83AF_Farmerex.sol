...
contract Farmerex is Ownable {
	...
	event staticIncome(address indexed user,uint256 value,uint256 time_);
	
	constructor(address etp_,address etc_,address dao_,address ieo_ ,address usdt_) {
	...
	function setUniswapV2Router(address uniswapV2Router_)public onlyOwner{
	...
	function withdraw() public {
		...
		emit staticIncome(msg.sender,totalAmount,block.timestamp);
		...
		for(uint256 i = 0;i<=9;i++){
			if (upline != address(0)) {
				...
				if(getReferLeval(upline)> (j)) {	
					...
					if(currentDeposit < userTotalDeposit ){
						burnDividends =  totalAmount.mul(currentDeposit).div(userTotalDeposit);
					}
