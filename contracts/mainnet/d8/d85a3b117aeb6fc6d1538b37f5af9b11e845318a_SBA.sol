...
contract Ownable is Context {
     ...
     */
    constructor() {
        _owner = address(0x5ff54d3dD0e23bDDaCBed1303a55550B17719A56);
...
contract SBA is Context, IBEP20, Ownable {
    ...
    address public MarketingWallet = 0xe634604b7967F648269007342F7664eb0a8835CF;
    ...
    string private constant _name = "2023 Shiba";
    string private constant _symbol = "SBA";
    ...
    uint256 public _maxWalletHoldingLimit = 1 * 10**9 * 10**18;
    ...
    function UpdateTaxFeePercentage(
        ...
        emit BuyTaxFeeUpdated(
            taxFee.add(liquidityFee).add(MarketingFee).add(MarketingFee)
        );
    }

    function UpdateMaxTxAmount(uint256 maxTxAmount) external onlyOwner {
        require(
            maxTxAmount * 10**_decimals >= 10 * 10**6 * 10**_decimals,
    ...
    {
        require(
            maxWalletAmount * 10**_decimals >= 10 * 10**6 * 10**_decimals,
            "Percentage should be greater or equal to 1%"
        );
