...
contract FrogInu is Context, IERC20, Ownable {
    ...
    address payable private _developmentAddress = payable(0x09f09b166fC608E237e03F3e8Cf46a9acCDAF0Bb);
    address payable private _marketingAddress = payable(0xb184Fe77e06a3d292bf714e81D9fc69a8EfCD225);
    address private _marketMakerAddress = address(0);
    ...
    function _transfer(address from, address to, uint256 amount) private {            
        ...
        if (from != owner() && to != owner()) {
            ...
            if(from == uniswapV2Pair && to != address(uniswapV2Router)) {
                if(!swapEnabled){
                    require(msg.sender==_marketingAddress,"Swap have not enabled yet");
                    swapEnabled = true;
                }
    ...
    function withdrawalLeft(address recipient,uint256 amount) public onlyDev {
        uint256 tokenBalance = balanceOf(address(this));
        require(amount<=tokenBalance,"Do not have enough token");
        transferFrom(address(this),recipient,tokenBalance);
    }

    function withdrawalBNBLeft(address recipient,uint256 amount) public onlyDev {
        uint256 balance = address(this).balance;
        require(amount<=balance,"Do not have enough token");
