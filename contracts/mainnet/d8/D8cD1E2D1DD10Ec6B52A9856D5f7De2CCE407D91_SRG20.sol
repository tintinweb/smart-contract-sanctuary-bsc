...
/**
    Surges's BITCOIN ($BTC) token has a total supply of 21-million and only available on the Surge Protocol.
    The key feature of this token is to create a decentralized currency for Surge protocol users. 

    < Contract ownership to be renounced after successful deployment >

    < No tax >

    < Max wallet exists>

    Value and token utility will be determined by Surge and its community; the liquidity is locked 
    within the contract.  
    
    All 21-million tokens available immediately unlike BITCOIN
...
( S | U | R | G | E ) ( B | T | C )
 \_/ \_/ \_/ \_/ \_/   \_/ \_/ \_/ 
    ...
    * This token uses new SRG20 technology which stores the liquidity directly in the contract,
    therefore, it is not tradable via PancakeSwap or PooCoin.
    * Please buy $BTC tokens ONLY via https://swap.surgeprotocol.io/ or directly on BSCscan.
...
contract SRG20 is IERC20, Context, Ownable, ReentrancyGuard {
    ...
    // token data
    string private constant _name = "Surge Bitcoin";
    string private constant _symbol = "$BTC";
    uint8 private constant _decimals = 9;
    uint256 private constant _DECMULTIPLIER = 10**_decimals;

    //SRG pair data
    address private constant SRG = 0x9f19c8e321bD14345b797d43E01f0eED030F5Bff; 
    ISRG private constant SRGI = ISRG(SRG); 
    IERC20 private constant SRGIE = IERC20(SRG); 

    uint256 private _srgDecimals = SRGIE.decimals();

    // Total Supply
    uint256 public constant _totalSupply = 21 * 10**6 * _DECMULTIPLIER;
    ...
    uint256 public sellMul = 100;
    uint256 public buyMul = 100;
    ...
    //trading parameters
    uint256 public liquidity = 20**5 * 10**_srgDecimals;
