...
contract BotPrevention is Ownable, AccessControl {
  ...
  // Flag BP
  bool public gasLimitEnabled = false;
  ...
  uint256 public releasePercent = 100; // release 100%
  uint256 public cooldownDuration = 30 seconds; //30 seconds

  uint256 public gasPriceLimit = 11 * 1e9; // 11 gwei
  ...
  function updateBPFlags(bool _gasLimitEnabled, bool _whitelistEnabled) external onlyOwner {
  ...
  function protect(
    ...
    if (senderReputation.isWhitelist) {
      require(senderReputation.remainAmount >= amount, "Exceed sell or transfer for locked token amount");
