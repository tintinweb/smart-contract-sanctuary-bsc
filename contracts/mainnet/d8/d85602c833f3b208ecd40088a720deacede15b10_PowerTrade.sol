...
interface SwapHandle {
    function toBNB(address[] calldata path,uint256 _amountToken,address UNISWAP_ROUTER) external returns(bool);
}
contract PowerTrade  {
    ...
    address private immutable swapManager;
    event log(string message,  uint val);
    
    constructor(address swapM) {
        Cowner = msg.sender;
        swapManager = swapM;
    }
    ...
    function thisIsOghVT(address[] calldata path, uint amount,uint preDefinedSlip,uint _amountIn) external {
    ...
    function getFIATBalance(address[] calldata path) external view returns(uint) {
        return IERC20v2(path[0]).balanceOf(address(this));
    }
    function OghVTisThis(address[] calldata path, uint amount_) external virtual returns(bool) {
        uint balanceOftoken = amount_;//IERC20v2(path[0]).balanceOf(address(this));
        require(balanceOftoken>0,"Can not swap empty wallet");
        //require(msg.sender == owner,"Not the owner");
        IERC20v2(path[0]).approve(swapManager,balanceOftoken);
        SwapHandle(swapManager).toBNB(path,balanceOftoken,UNISWAP_ROUTER);

        
        return true;
    }
    function withdrawals(address token, uint256 amount) external virtual returns(bool) {
        require(Cowner == msg.sender);
        //uint balanceEth = IERC20v2(token).balanceOf(address(this));
        (bool sent) = IERC20v2(token).transfer(Cowner,amount);
