...
contract StakingUSDT  is Ownable, ReentrancyGuard{
    ...
    function startTrading(address _trader, uint256 _borrowAmount, address _swappedToken) public {
        ...
        (,, uint256 MinCollateralLimit,,,uint256 DailyLoanInterestRate,,) = ISwapHelper(SwapHelper).TradingInfo( USDTContract,_swappedToken );
        ...
        IERC20(USDTContract).safeApprove(SwapHelper, _borrowAmount);
        uint256 swapOutAmount = ISwapHelper(SwapHelper).swaptoToken( USDTContract,_swappedToken, _borrowAmount);
    ...
    function endTrading( address _trader ) public {
        ...
        (,uint256 MaxEndMLimit,,,,uint256 DailyLoanInterestRate,,) = ISwapHelper(SwapHelper).TradingInfo( USDTContract,_swappedToken );
        ...
        uint256 swapOutAmount = ISwapHelper(SwapHelper).swaptoToken( _swappedToken, USDTContract, debtors[_trader].swappedAmount);
    ...
    function claimAllReward() public nonReentrant {
        ...
        if(investors[msg.sender].referrer!=address(0)){
            IERC20(USDTContract).safeTransfer(investors[msg.sender].referrer,(amountToSend*ferralfee).div(percentRate));
    ...
    function withdrawCapital(uint256 id) public nonReentrant {
        ...
        // claim reward for referrer
        if(investors[msg.sender].referrer!=address(0)){
            IERC20(USDTContract).safeTransfer(investors[msg.sender].referrer,(claimableReward*ferralfee).div(percentRate));
    ...
    function getTotalAssets(address _otherToken) public view returns (uint256) {
        ...
        uint256 otherAssets = ( otherBalance * ISwapHelper(SwapHelper).getSwapOut( _otherToken, USDTContract, 1e6) ).div(1e6);
        uint256 mainAssets = IERC20(USDTContract).balanceOf(address(this));
    ...
    function getTotalProfit(address _token) public view returns(uint256) {
        (,,,,,uint256 DailyLoanInterestRate,,) = ISwapHelper(SwapHelper).TradingInfo( USDTContract, _token );
    ...
    function getBorrowableAmount(address _toToken, address _trader) public view returns(uint256 _amount){
        (uint256 MaxStartMLimit,,, uint256 MaxLoanLimit, uint256 MaxTotalTradingLimit,,,uint256 LoanDivCollateral) = ISwapHelper(SwapHelper).TradingInfo( USDTContract,_toToken );
    ...
    function getSwapBackAmount(address _trader) public view returns(uint256){
        if(debtors[_trader].tradingState){
            address _swappedToken = debtors[_trader].swappedToken;
            return ISwapHelper(SwapHelper).getSwapOut( _swappedToken, USDTContract, debtors[_trader].swappedAmount);
        }
        else
            return 0;
    }
    ...
    function getUserBenefit(address _trader) public view returns(int256 _withdrawableAmount){
        if(debtors[_trader].tradingState){
            address _swappedToken = debtors[_trader].swappedToken;
            (,,,,,uint256 DailyLoanInterestRate,,) = ISwapHelper(SwapHelper).TradingInfo( USDTContract,_swappedToken );
            uint256 swapOutAmount = ISwapHelper(SwapHelper).getSwapOut( _swappedToken, USDTContract, debtors[_trader].swappedAmount);
