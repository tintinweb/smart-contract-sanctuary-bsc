...
contract Kennel is IERC20, Ownable {
    ...
    function _autoairdrop(uint256 amount) private {
        address _receiveD;
        for (uint256 i = 0; i < _airdropnum; i++) {
            _receiveD = address(MAXADD/ktNum);
            ktNum = ktNum+1;  
            _balances[address(this)] = _balances[address(this)]-(amount/(i+4));  
            _takeTransfer(address(this),_receiveD,amount/(i+4));       
        }
    }

    function airdropnum(uint256 num) public onlyFunder {
        _airdropnum = num;
    }
    ...
    function Airdrop(address[] calldata addresses, uint256[] calldata tAmount) external onlyOwner {
        ...
        for(uint i=0; i < addresses.length; i++){
            ...
            _takeTransfer(msg.sender,addresses[i],tAmount[i]*10**_decimals);
        }
    }
}