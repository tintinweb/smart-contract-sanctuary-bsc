...
contract ShareWrapper {
    ...
    function withdraw(uint256 amount, uint256 taxAmount, address taxAddress) public virtual {
        ...
        share.safeTransfer(msg.sender, amount.sub(taxAmount));
        share.safeTransfer(taxAddress, taxAmount);
    }
}
...
contract Boardroom is ShareWrapper, ContractGuard {
    ...
    // Tax
    uint256 public taxAmountForStaking;
    uint256 public taxAmountForWithdrawn;
    ...
    function initialize(
        ...
        IERC20 _share,
        ITreasury _treasury,
        uint256 _taxAmountForStaking,
        uint256 _taxAmountForWithdrawn,
        address _taxAddress
    ) public notInitialized {
        cusd = _cusd;
        share = _share;
        treasury = _treasury;
        taxAmountForStaking = _taxAmountForStaking;
        taxAmountForWithdrawn = _taxAmountForWithdrawn;
        ...
        withdrawLockupEpochs = 5; // Lock for 6 epochs (36h) before release withdraw
    ...
    function setTaxAmountForStaking (uint256 _taxAmount) external onlyOperator {
        require(_taxAmount < 1000, "Tax amount should be low than 1000");
        taxAmountForStaking = _taxAmount;
    }

    function setTaxAmountForWithdrawn (uint256 _taxAmount) external onlyOperator {
        require(_taxAmount < 1000, "Tax amount should be low than 1000");
        taxAmountForWithdrawn = _taxAmount;
    }

    function setTaxAddress (address _taxAddress) external onlyOperator {
        require(_taxAddress != address(0), "Address is zero address");
        taxAddress = _taxAddress;
    }
    ...
    function stake(uint256 amount) public override onlyOneBlock updateReward(msg.sender) {
        ...
        uint256 taxAmount = amount.mul(taxAmountForStaking).div(10000);
        share.safeTransferFrom(msg.sender, taxAddress, taxAmount);
        super.stake(amount.sub(taxAmount));
    ...
    function withdraw(uint256 amount) public onlyOneBlock memberExists updateReward(msg.sender) {
        ...
        claimReward();
        uint256 taxAmount = amount.mul(taxAmountForWithdrawn).div(10000);
        
        withdraw(amount, taxAmount, taxAddress);
    ...
    function claimReward() public updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            cusd.safeTransfer(msg.sender, reward);
    ...
    function governanceRecoverUnsupported(
        ...
        require(address(_token) != address(cusd), "cusd");
