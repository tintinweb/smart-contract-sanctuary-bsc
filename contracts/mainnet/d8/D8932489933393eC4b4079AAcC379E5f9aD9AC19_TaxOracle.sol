...
contract TaxOracle is Ownable {
    ...
    function consult(address _token, uint256 _amountIn) external view returns (uint144 amountOut) {
        require(_token == address(dex), "token needs to be correct");
        uint256 dexBalance = dex.balanceOf(pair);
        uint256 wbnbBalance = wbnb.balanceOf(pair);
        return uint144(dexBalance.mul(_amountIn).div(wbnbBalance));
    }

    function getDexBalance() external view returns (uint256) {
	    return dex.balanceOf(pair);
    }
    ...
    function getPrice() external view returns (uint256) {
        uint256 dexBalance = dex.balanceOf(pair);
        uint256 wbnbBalance = wbnb.balanceOf(pair);
        return dexBalance.mul(1e18).div(wbnbBalance);
    }
