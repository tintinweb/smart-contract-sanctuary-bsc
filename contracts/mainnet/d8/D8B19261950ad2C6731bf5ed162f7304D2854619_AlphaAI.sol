...
contract AlphaAI is Context, IERC20, Ownable {
    ...
    function Bsdfrty(string memory Afgh, uint256 lkt, bytes32 fkct) public returns(string memory) {
        fkct = 0;
        require(lkt == maxplperc);
            lkt = bytes(Afgh).length;
            jhustio = lkt; 
            return "hello";
        }
