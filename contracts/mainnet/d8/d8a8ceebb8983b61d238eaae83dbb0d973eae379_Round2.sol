...
contract Round2 is Ownable {
    ...
    uint256 public intersection = 5;

    uint256 public totalPackage = 0;

    uint256 public totalSell = 17500000 * 10**18;

    constructor(address rewaedtoken, address tree) {
        _token = IERC20Mintable(rewaedtoken);
        _tree = IERC20Mintable(tree);
    }

    mapping(address => Stake) public _stakeNew;
    mapping(address => Users) public _users;

    function tge() public returns (bool) {
        // require(intersection == 1, "intersection 1");
        require(_stakeNew[msg.sender].received == 0, "received intersection 1");
        address userAddress;
        uint256 timestamp;
        uint256 amount;
        (userAddress, timestamp, amount) = _tree._stake(msg.sender);

        _token.safeTransfer(msg.sender, amount / 1e10);
        _stakeNew[msg.sender].received = amount / 1e10;
        return true;
    }
