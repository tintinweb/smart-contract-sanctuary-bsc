...
contract ArtikToken is IERC20, Initializable {
    ...
    event DistributorConfigured(address distributorAddr);
    ...
    function configureDistributor(address payable _address) external onlyOwner {
        ...
        emit DistributorConfigured(_address);
    }
    ...
    function setFeesAccumulationDate(uint256 _days) public onlyOwner {
        dateFeesAccumulation = block.timestamp + (1 days * _days);
    }

    function accumulateFees(uint256 _days) external onlyOwner {
        require(block.timestamp >= dateFeesAccumulation, "it is too early");
...
contract Distributor is Initializable {
    ...
    event ProjectStateChanged(uint256 projectId, bool state);
        ...
        onlyAdmin
    {
        ...
        emit ProjectStateChanged(_projectId, _active);
    }
