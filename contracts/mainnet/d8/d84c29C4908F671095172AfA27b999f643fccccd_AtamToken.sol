...
interface IAtamToken is IERC20Metadata {
...
// Avatar Mining Coin
contract AtamToken is Ownable, IAtamToken {
    ...
    uint256 public constant MAX_SUPPLY = 10**10 * TOKEN_UNIT;   // totalsuppy: 100 trillion
    ...
    string private _name = "Avatar Mining Coin";
    string private _symbol = "ATAM";   
    ...
    function _transfer(address from_, address to_, uint256 amount_) private {
        ...
        require( from_ != address(0), "TOS_AM_T01" );
        require( to_ != address(0), "TOS_AM_T02" );
        require( amount_ > 0, "TOS_AM_T03" );
        
        _tOwned[from_] = _tOwned[from_].sub(amount_, "TOS_AM_T04");
    ...
    function transferFrom(address sender, address recipient, uint256 amount_) public override returns (bool) {
        ...
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount_, "TOS_AM_TF01"));
        return true;
    }
    ...
    function decreaseAllowance(address spender_, uint256 subtractedValue_) public virtual returns (bool) {
        _approve(_msgSender(), spender_, _allowances[_msgSender()][spender_].sub(subtractedValue_, "TOS_AM_DA01"));
        return true;
    }
    ...
    function burnFrom(address from_, uint amount_) public override {
        _approve(from_, _msgSender(), _allowances[from_][_msgSender()].sub(amount_, "TOS_AM_BF01"));
        _burn(from_, amount_);
    }

    function _approve(address owner_, address spender_, uint256 amount_) private {
        require(owner_ != address(0), "TOS_AM_AP01");
        require(spender_ != address(0), "TOS_AM_AP02");
