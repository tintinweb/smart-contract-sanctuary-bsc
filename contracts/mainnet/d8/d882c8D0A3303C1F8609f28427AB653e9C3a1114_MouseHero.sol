...
// File contracts/booster/interfaces/IBooster.sol
...
// File contracts/nft/MouseHero.sol
  ...
  ERC721Pausable
{
  ...
  function unboxBooster(address boosterAddress, uint256 amount) public {
    ...
    if (boosterDecimals > 0) {
      require(amount % (10**boosterDecimals) == 0, "INV_AMOUNT");
    }

    // BurnFrom the booster, fail hard if it can't
    trustedBooster.burnFrom(_msgSender(), amount);

    // calculate how many NFTs will be minted according to booster decimals
    uint256 numberOfNFTsToMint = boosterDecimals == 0 ? amount : amount / (10**boosterDecimals);

    // for each NFT to mint, mint it and emit an event
    for (uint256 i = 0; i < numberOfNFTsToMint; i++) {
