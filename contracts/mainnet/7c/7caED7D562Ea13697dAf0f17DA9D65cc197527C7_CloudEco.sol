...
contract CloudEco is Ownable {
    ...
    ISonixGold public sonixGold;

    address[][] public path = [[0xd2d49560BdCeF79D0F4F86d564Ad9c711FE232Ea,0x55d398326f99059fF775485246999027B3197955],[0x55d398326f99059fF775485246999027B3197955,0xd2d49560BdCeF79D0F4F86d564Ad9c711FE232Ea]];
    ...
    struct PoolInfo {
        ...
        uint256 totalHarvToken;
        uint256 totalHarvSell;
        uint256 totalMint;
        uint256 totalBurn;
    }
    ...
    uint256 public mindailyReward = 90;
    uint256 public maxdailyReward = 110;
    ...
    constructor(IBEP20 _salsa,IReferral _referral, address _pancakeRouter, address _ref,address _sc,IBEP20 _usdtToken,address _sonixGold) public {
        rewardToken = IBEP20(_sonixGold);
        referral = _referral;
        totalStakedAmount = 0;
        ...
        sc = ISmartChef(_sc);
        sonixGold = ISonixGold(_sonixGold);
    ...
    function setMinDailyReward(uint256 _mindailyReward) public onlyOwner{
        mindailyReward = _mindailyReward;
    }

    function setMaxDailyReward(uint256 _maxdailyReward) public onlyOwner{
    ...
    function pendingReward(uint256 _pid,address _user) public view returns (uint256) {
        ...
        rew = rew*10000 * _totalSupply/sonixGold.totalSupply();
        return rew;
    }
    ...
    function harvest(uint256 _pid,uint256 _type) public {
        ...
        harv(_pid,msg.sender,_type);
    }

    function harv(uint256 _pid,address buyer,uint256 _type) internal {
        ...
        if (user.amount > 0) {
            ...
            if(totalpending > 0) {
                if(_type<1){
                    uint256 pending = totalpending.mul(8).div(10)/getPriceToken(0);
                    //mint token
                    //sonixGold.mint(pending);
                    pool.totalMint=pool.totalMint+pending;
                    rewardToken.safeTransfer(address(buyer), pending);
                    pool.totalHarvToken = pool.totalHarvToken + pending;
                }else{
                    uint256 pending = totalpending/getPriceToken(0);
                    //mint token
                    //sonixGold.mint(pending);
                    pool.totalMint=pool.totalMint+pending;
                    fillOrderBuy(pending);
                    pool.totalHarvSell = pool.totalHarvSell + pending;
                }
    ...
    function depo(uint256 _pid,uint256 _amount, address _referrer, address _sender) internal {
        ...
        if(_amount > 0) {
            ...
            //burn
            //sonixGold.burn(_tokenValue/price);
            pool.totalBurn = pool.totalBurn + _tokenValue/price;
        }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        ...
        if(commissionAmount>0){
            ...
            if(tpLeader != address(0)){
                ...
                //mint token
                //sonixGold.mint(commissionAmount);
                pool.totalMint=pool.totalMint+commissionAmount;
            ...
            if(lowlead != address(0)){
                ...
                //mint token
                //sonixGold.mint(commissionAmountLead);
                pool.totalMint=pool.totalMint+commissionAmountLead;
            ...
            if(address(referral) != address(0)){
                ...
                if(_referrallevel1 != address(0)){
                    ...
                    //mint token
                    //sonixGold.mint(commissionAmount1);
                    pool.totalMint=pool.totalMint+commissionAmount1;
                    ...
                    if(_referrallevel2 != address(0)){
                        ...
                        //mint token
                        //sonixGold.mint(commissionAmount2);
                        pool.totalMint=pool.totalMint+commissionAmount2;
                        ...
                        if(_referrallevel3 != address(0)){
                            ...
                            //mint token
                            //sonixGold.mint(commissionAmount3);
                            pool.totalMint=pool.totalMint+commissionAmount3;
    ...
    function saveSellback(uint256 _amount,address _referral) public{
        ...
        //convert 50% to xos
        uint256 _amountUSDT = _amount/2; 
        uint256 amountXOS = _amountUSDT/(getPriceToken(0)*(100+markup)/100);

        fillOrderSell(amountXOS,_referral);
    }

    //make special function to call fillOrderBuy
    function fillOrderBuyWallet(uint256 _totalXOSParam) public onlyMasterChef{
        fillOrderBuy(_totalXOSParam);
    }
    ...
    function resetQueue() internal{
        ...
        for (uint i = 0; i < queuesellback.length-1; i++) {
            if(queuesellback[gap].amountXOS==0){
                gap++;
                continue;
            }else{
                break;
    ...
    function intSaveQueueSellback(address buyerAdd,address sellerAdd,uint256 totalXOSParam,address _referral)  internal{
        P2PQueue memory p2p;
        p2p.buyer = buyerAdd;
        p2p.seller = sellerAdd;
        p2p.amountXOS = totalXOSParam;
        p2p.createDate = block.timestamp;
        p2p.price = getPriceToken(0)*(100+markup)/100;
        p2p.amountUSDT = p2p.amountXOS*p2p.price;
        ...
        for(uint i=0;i<queuesellback.length;i++){
            if(queuesellback[i].amountXOS==0){
                index = i;
                update = true;
                break;
            }   
    ...
    //this call from harvest
    function fillOrderBuy(uint256 totalXOSParam) internal{
        ...
        for(uint i=0;i<p2pArray.length;i++){
            ...
            if(p2p.buyer!=address(0)&&p2p.amountXOS>0){
                if(p2p.amountXOS<=transXOS && transXOS>0){
                    transXOS = transXOS - p2p.amountXOS;
                    completeOrder(p2p,msg.sender,p2p.referral,p2p.amountXOS);
                    delete queuesellback[i];
                }else if(p2p.amountXOS>transXOS){
                    p2p.amountXOS = p2p.amountXOS - transXOS;
                    completeOrder(p2p,msg.sender,p2p.referral,transXOS);
                    transXOS = 0;
                }else{
                    break;
                }
        ...
        if(transXOS>0){
            //add order sell
            intSaveQueueSellback(address(0),address(msg.sender),transXOS, address(0));
        }
        //reset queue
        resetQueue();
    }
    ...
    //call from deposit
    function fillOrderSell(uint256 totalXOSParam,address _referral) internal{
        P2PQueue[] storage p2pArray = queuesellback;
        uint256 transXOS = totalXOSParam; 
        for(uint i=0;i<p2pArray.length;i++){
            P2PQueue storage p2p = queuesellback[i];
            if(p2p.seller!=address(0)&&p2p.amountXOS>0){
                if(p2p.amountXOS<=transXOS && transXOS>0){
                    transXOS = transXOS - p2p.amountXOS;
                    completeOrder(p2p,p2p.seller,_referral,p2p.amountXOS);
                    delete queuesellback[i];
                }else if(p2p.amountXOS>transXOS){
                    p2p.amountXOS = p2p.amountXOS - transXOS;
                    completeOrder(p2p,msg.sender,p2p.referral,transXOS);
        ...
        if(transXOS>0){
            //add order buy
            intSaveQueueSellback(address(msg.sender),address(0),transXOS,address(0));
        }
        //reset queue
        resetQueue();
    }
    ...
    function completeOrder(P2PQueue storage p2p,address _seller,address _referral,uint256 _amountXOS) internal{
        //transfer usdt to seller
        if(p2p.amountXOS>0){
            uint256 amountUSDT = _amountXOS*p2p.price;
            usdtToken.safeTransfer(address(_seller), amountUSDT);
            PoolInfo storage pool = poolInfo[0];
            uint256 priceLp = getLPPrice(address(pool.lpToken));
            uint256 amtcake = amountUSDT*2/priceLp;
            depo(0,amtcake,address(_referral),address(p2p.buyer));
        }   
    }
