...
function setFee(uint256 _feeOne, uint256 _feeTwo) public onlyOwner {
    feeOne = _feeOne;
    feeTwo = _feeTwo;
}
  ...
  function setReceiver(address payable _receiver) public onlyOwner{
    ...
    function mintToken(uint256 amount, string[] memory _tokenURI) public payable {
        require(!paused(),"Sale must be active to mint");
        require(amount == 1 || amount == 3, "Wrong mint amount");
        uint256 fee;
        amount == 1 ? fee = feeOne : fee = feeTwo;

        require(msg.value >= fee, "Insufficient funds.");
        require(_tokenIdCounter.current() + amount <= maxSupply, "Cannot mint more than total supply");
        // if(whitelistedOnly){
        //     require(whiteListed[msg.sender] == true,"You are not whitelisted");
        // }
        for (uint256 i = 0; i < amount; i++){
        internalMint(msg.sender, _tokenURI[i]);
