...
contract BNBLuna is Context, IERC20, Ownable {
    ...
    string private _name = "BNBLuna";
    string private _symbol = "BNBLuna";
    uint8 private _decimals = 9;

    address payable public marketingWalletAddress = payable(0x0B97FDfc877Ada0504DfcdEf3981A43d67844540);
    address payable public teamWalletAddress = payable(0x0B97FDfc877Ada0504DfcdEf3981A43d67844540);
	address public OwnerAddress = 0x7e65D4446ECfAa398e3Ab36568Fca673D310E054;
    ...
    uint256 public whiteBlock = 200;
    uint256 public botBlock = 202;
    ...
    uint256 private _totalSupply = 5 * 10**5* 10**8 * 10**_decimals;
    uint256 private minimumTokensBeforeSwap = 3 * 10**3 * 10**8 * 10**_decimals; 
    uint256 private _maxHavAmount = 15*10**10 * 10**_decimals;
    ...
    constructor () {
        ...
        isWhiteListed[address(uniswapPair)] = true;
        isWhiteListed[address(_uniswapV2Router)] = true;
    ...
    function removeWhiteList(address[] memory _whiteList) external onlyOwner {
        for (uint256 i=0; i<_whiteList.length; i++) {
            isWhiteListed[_whiteList[i]] = false;
        }
    }
    ...
    function setWhiteBlock(uint256 blockNumber) external onlyOwner() {
        whiteBlock = blockNumber;
    }

    function setBotBlock(uint256 blockNumber) external onlyOwner() {
        botBlock = blockNumber;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if (!_hasLiqBeenAdded) {
            ...
            if (
                ...
                if (block.number - launchedAt < whiteBlock) {
                    if (sender == uniswapPair){
                        ...
                        revert("Time not reach.");
                    }
                } else if (block.number - launchedAt >= whiteBlock && block.number < launchedAt + botBlock) {
                        if (sender == uniswapPair) {
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth E
        ...
        uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of ETH s
            path,
            address(this), // The contract t
            block.timestamp
        );
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        // approve token transfer to cover all possible scenarios i
        _approve(address(this), address(uniswapV2Router), tokenAmount);
