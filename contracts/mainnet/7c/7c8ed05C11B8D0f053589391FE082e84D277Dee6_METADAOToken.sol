...
contract METADAOToken is IERC20, Ownable {
    ...
    uint256 partnerGas = 250000;
    uint256 public minPeriod = 5 minutes;
    uint256 public minPeriod2 = 5 minutes;
    uint256 public LPFeefenhong;
    uint256 public Partnerfenhong;
    ...
    uint256 public _tLPFeeTotal;

    uint256 public _tFounderFeeTotal;

    uint256 public _FounderAvgAmount;
    ...
    bool public autoSwapOSK = true;
    bool public autoPartnerFlag = true;
    uint256 public baseFounderFeeTotal = 0;
    ...
    constructor(IERC20 _usdt,IERC20 _osk) {
        usdt = _usdt;
        osk = _osk;
        _tOwned[address(0x37909A68DB966Fe22c4a8B8813add5F3292Fa1Af)] = _tTotal;
        ...
        _isExcludedFromFee[address(0x37909A68DB966Fe22c4a8B8813add5F3292Fa1Af)] = true;
   ...
   function isExcludedFromFee(address account) public view returns (bool) {
    ...
    function _transfer(
        ...
        if(autoSwapOSK && _tFounderFeeTotal>baseFounderFeeTotal){
            uint256 _FounderFeeAmount = _tFounderFeeTotal;
            swapTokensForOther(_FounderFeeAmount, address(this));
            _tFounderFeeTotal=_tFounderFeeTotal.sub(_FounderFeeAmount);
        }
        ...
        fromAddress = from;
        toAddress = to;  
         if(_tOwned[address(this)] >0 && from !=address(this) && LPFeefenhong.add(minPeriod) <= block.timestamp) {
             process(distributorGas) ;
             LPFeefenhong = block.timestamp;
        }
        if((osk.balanceOf(address(this)) >0 && from!=address(this) && Partnerfenhong.add(minPeriod2) <= block.timestamp) && autoPartnerFlag){
            process2(partnerGas);
            Partnerfenhong = block.timestamp;
        }
    }
    ...
    function process(uint256 gas) private {
        ...
        uint256 nowbanance = _tLPFeeTotal;//_tOwned[address(this)];
        uint256 gasUsed = 0;
    ...
    function process2(uint256 gas) private {
        ...
        uint256 nowbanance = _FounderAvgAmount;//_tOwned[address(this)];
        uint256 gasUsed = 0;
        uint256 gasLeft = gasleft();
        ...
        while(gasUsed < gas && iterations < partnerCount) {
            if(partnerIndex >= partnerCount || partnerIndex==0){
                partnerIndex = 0;
                uint256 oskBalance=osk.balanceOf(address(this));
                if(oskBalance<=0){
                    return;
                }
                _FounderAvgAmount=oskBalance/partnerCount;
                nowbanance = _FounderAvgAmount;
            }

          uint256 amount = nowbanance;

           osk.transfer(partners[partnerIndex],amount);
    ...
    //    if (takeFee) require(amount <= balanceOf(sender).div(1000).mul(999), "Sell more than 90%, reduce the selling amount");
   ...
   //     if(_tFeeTotal >= 1 * 10**7 * 10**9)_burnFee = 0;
    ...
    function setAutoSwaposk(bool _autoSwapOSK) public onlyOwner {
        autoSwapOSK=_autoSwapOSK;
    }

    function setAutoPartnerFlag(bool _autoPartnerFlag) public onlyOwner {
        autoPartnerFlag = _autoPartnerFlag;
    }


    function pushPartners(address[] memory newPartners) public onlyOwner {
    ...
    function setBaseFounderFeeTotal(uint256 _baseFounderFeeTotal) public onlyOwner {
        baseFounderFeeTotal = _baseFounderFeeTotal;
    }
}