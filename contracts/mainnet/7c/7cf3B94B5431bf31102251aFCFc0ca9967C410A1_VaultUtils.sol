...
contract VaultUtils is IVaultUtils, Governable {
    ...
    function getPositionFee(
        ...
        return
            _sizeDelta.mul(vault.marginFeeBasisPoints()).div(
    ...
    function getFeeBasisPoints(
        ...
        uint256 initialAmount = vault.usdjAmounts(_token);
        uint256 nextAmount = initialAmount.add(_usdjDelta);
        if (!_increment) {
            nextAmount = _usdjDelta > initialAmount
                ? 0
                : initialAmount.sub(_usdjDelta);
        }

        uint256 targetAmount = vault.getTargetUsdjAmount(_token);
        if (targetAmount == 0) {
            return _feeBasisPoints;
        }
