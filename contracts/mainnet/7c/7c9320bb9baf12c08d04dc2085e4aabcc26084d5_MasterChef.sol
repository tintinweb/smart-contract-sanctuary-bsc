...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    ///
    ///   pending reward = (user share * pool.accWojkPerShare) - user.rewardDebt
    ///
    ///   Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
    ///   1. The pool's `accWojkPerShare` (and `lastRewardBlock`) gets updated.
    ...
    /// `accWojkPerShare` Accumulated WOJKs per share, times 1e12.
    ...
    ///  @notice 40 wojks per block in MC
    uint256 public constant MASTERCHEF_WOJK_PER_BLOCK = 40 * 1e18;
    uint256 public constant ACC_WOJK_PRECISION = 1e18;

    /// @notice The block number when wojks mining starts.
    ...
    /// @notice total wojk rate = toBurn + toRegular + toSpecial
    ...
    uint256 public wojkRateToBurn = 0;
    /// @notice WOJK distribute % for regular farm pool
    uint256 public wojkRateToRegularFarm = 250000000000;
    /// @notice WOJK distribute % for special pools
    uint256 public wojkRateToSpecialFarm = 750000000000;
    ...
    event UpdatePool(uint256 indexed pid, uint256 lastRewardBlock, uint256 lpSupply, uint256 accWojkPerShare);
    event Deposit(address indexed user, uint256 indexed pid, uint256 amount);
    event Withdraw(address indexed user, uint256 indexed pid, uint256 amount);
    event EmergencyWithdraw(address indexed user, uint256 indexed pid, uint256 amount);

    event UpdateWojkRate(uint256 burnRate, uint256 regularFarmRate, uint256 specialFarmRate);
    ...
    /// @param _wojk The WOJK token contract address.
    ...
    function add(
        ...
        require(_lpToken != wojk, "WOJK token can't be added to farm pools");
    ...
    function pendingWojk(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 accWojkPerShare = pool.accWojkPerShare;
        uint256 lpSupply = pool.totalBoostedShare;

        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = block.number.sub(pool.lastRewardBlock);

            uint256 wojkReward = multiplier.mul(wojkPerBlock(pool.isRegular)).mul(pool.allocPoint).div(
                (pool.isRegular ? totalRegularAllocPoint : totalSpecialAllocPoint)
            );
            accWojkPerShare = accWojkPerShare.add(wojkReward.mul(ACC_WOJK_PRECISION).div(lpSupply));
        }

        uint256 boostedAmount = user.amount.mul(getBoostMultiplier(_user, _pid)).div(BOOST_PRECISION);
        return boostedAmount.mul(accWojkPerShare).div(ACC_WOJK_PRECISION).sub(user.rewardDebt);
    }

    /// @notice Update wojk reward for all the active pools. Be careful of gas spending!
    ...
    function wojkPerBlock(bool _isRegular) public view returns (uint256 amount) {
        if (_isRegular) {
            amount = MASTERCHEF_WOJK_PER_BLOCK.mul(wojkRateToRegularFarm).div(WOJK_RATE_TOTAL_PRECISION);
        } else {
            amount = MASTERCHEF_WOJK_PER_BLOCK.mul(wojkRateToSpecialFarm).div(WOJK_RATE_TOTAL_PRECISION);
        }
    }

    /// @notice Calculates and returns the `amount` of WOJK per block to burn.
    function wojkPerBlockToBurn() public view returns (uint256 amount) {
        amount = MASTERCHEF_WOJK_PER_BLOCK.mul(wojkRateToBurn).div(WOJK_RATE_TOTAL_PRECISION);
    }
    ...
    function updatePool(uint256 _pid) public returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (lpSupply > 0 && totalAllocPoint > 0) {
                ...
                pool.accWojkPerShare = pool.accWojkPerShare.add((wojkReward.mul(ACC_WOJK_PRECISION).div(lpSupply)));
                wojk.mint(devaddr, wojkReward.div(10));
                wojk.mint(address(this), wojkReward);
            }
            pool.lastRewardBlock = block.number;
            poolInfo[_pid] = pool;
            emit UpdatePool(_pid, pool.lastRewardBlock, lpSupply, pool.accWojkPerShare);
        }
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            settlePendingWojk(msg.sender, _pid, multiplier);
        }
        ...
        user.rewardDebt = user.amount.mul(multiplier).div(BOOST_PRECISION).mul(pool.accWojkPerShare).div(
            ACC_WOJK_PRECISION
        );
        poolInfo[_pid] = pool;
    ...
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function burnWojk(bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }

        uint256 multiplier = block.number.sub(lastBurnedBlock);
        uint256 pendingWojkToBurn = multiplier.mul(wojkPerBlockToBurn());

        // SafeTransfer WOJK
        _safeTransfer(burnAdmin, pendingWojkToBurn);
    ...
    function updateWojkRate(
        ...
        require(
            ...
            "MasterChef: Wojk rate must be greater than 0"
        );
        require(
        ...
        // burn wojk base on old burn wojk rate
        burnWojk(false);

        wojkRateToBurn = _burnRate;
        wojkRateToRegularFarm = _regularFarmRate;
        wojkRateToSpecialFarm = _specialFarmRate;

        emit UpdateWojkRate(_burnRate, _regularFarmRate, _specialFarmRate);
    }
    ...
    function updateBoostMultiplier(
        ...
        settlePendingWojk(_user, _pid, prevMultiplier);

        user.rewardDebt = user.amount.mul(_newMultiplier).div(BOOST_PRECISION).mul(pool.accWojkPerShare).div(
            ACC_WOJK_PRECISION
        );
    ...
    function settlePendingWojk(
        ...
        uint256 accWojk = boostedAmount.mul(poolInfo[_pid].accWojkPerShare).div(ACC_WOJK_PRECISION);
        uint256 pending = accWojk.sub(user.rewardDebt);
        // SafeTransfer WOJK
        _safeTransfer(_user, pending);
    }
    function dev(address _devaddr) external onlyOwner {
        require(_devaddr != address(0), "dev: not zero address");
        devaddr = _devaddr;
    ...
    function _safeTransfer(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            uint256 balance = wojk.balanceOf(address(this));
            if (balance < _amount) {
                _amount = balance;
            }
            wojk.transfer(_to, _amount);
        }
    }
}