...
contract DinoBusd is Context, Ownable , ReentrancyGuard {
    ...
    function userReward(address _userAddress) public view returns(uint256) {
        ...
        uint256 claimInvestTime = weekly[_userAddress].startTime; //weekly 
        uint256 claimInvestEnd = weekly[_userAddress].deadline; //weekly
   ...
   // uint256 totalAmount = SafeMath.add(amount,aval_withdraw2); // it will add one of his half to total withdraw
    ...
    function unStake() external noReentrant {
        ...
        uint256 UnstakeValueCore = SafeMath.div(UnstakeValue,1); //50% investment, now it is 100%
