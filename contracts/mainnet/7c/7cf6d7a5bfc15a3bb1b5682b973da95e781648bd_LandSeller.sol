...
contract LandSeller is Initializable, OwnableUpgradeable, PausableUpgradeable, AccessControlUpgradeable {
  ...
  /**
    * @dev prices (token => [city price 0, city price 1, ...])
    *
    * token address(0) is ether price
    *
    * if price = 0: city is not sold with that token
    * if price = 1: city price is calc base on reference token price
    * if price > 1: city price is exactly that value
    */
  mapping (IERC20 => uint256[]) private price;

  /**
    * @dev stable coin token address which is used as reference to convert prices
    */
  IERC20 public referenceToken;

  mapping (IERC20 => AggregatorV3Interface) internal priceFeed;
  ...
  /**
    * @dev Changes the `referenceToken` address.
    */
  function setReferenceToken(IERC20 newReferenceToken) public onlyOwner {
    referenceToken = newReferenceToken;
  }

  /**
    * @dev Changes the `priceFeed` address for a token.
    */
  function setPriceFeed(IERC20 token, AggregatorV3Interface newPriceFeed) public onlyOwner {
    priceFeed[token] = newPriceFeed;
  }

  /**
    * Returns the latest price
    */
  function getLatestPrice(IERC20 token) internal view returns (uint256) {
    (
      /*uint80 roundID*/,
      int latestPrice,
      /*uint startedAt*/,
    ...
    uint8 baseDecimals = priceFeed[token].decimals();

    return scalePrice(latestPrice, baseDecimals, 18);
  }
    ...
    returns (uint256)
  {
    if (_priceDecimals < _decimals) {
      return uint256(_price) * (10 ** uint256(_decimals - _priceDecimals));
    }
    if (_priceDecimals > _decimals) {
      return uint256(_price) / (10 ** uint256(_priceDecimals - _decimals));
    }
  ...
  /**
    ...
    * - for ether address(0) is used.
    * - for tokens: sender should allow LandSeller contract to use the total price of desired `token` in order to buy lands.
    */
  function buy(IERC20 token, uint8 amount, uint8 city, uint32 referralCode) public payable whenNotPaused returns (uint256[] memory) {
    require(amount <= 40, "Buy limit exceeded");
    require(amount <= getAvailable(city), "Not enough lands on sale");
    uint256 landPrice = calcLandPrice(token, city);
    require(landPrice > 0, "Reference price not available");
    uint256 totalPrice = landPrice * amount;
    require(address(token) == address(0) ? msg.value == totalPrice : msg.value == 0, "Price does not match");
  ...
  /**
    * @dev calc land price base on reference token price for certain token
    */
  function calcLandPrice(IERC20 token, uint8 city) public view returns (uint256) {
    uint256 landPrice = getPrice(token, city);
    require(landPrice > 0, "Token not valid");

    if (landPrice != 1) return landPrice;

    return (getPrice(referenceToken, city) * 10 ** 18) / getLatestPrice(token);
  }
