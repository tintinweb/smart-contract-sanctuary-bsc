...
contract EscrowFactory is Ownable, ReentrancyGuard {
    ...
    event GigCreated(address _address, address _seller);
    ...
    function purchaseViewList(address _list) public {
        ...
        BUSD.transferFrom(msg.sender, owner(),  8000000000000000000);
        BUSD.transferFrom(msg.sender, receiver, 2000000000000000000);
        viewList[msg.sender].push(_list);
    }

    function getAllViewList(address _user) public returns (address[] memory) {
    ...
    function createGig(uint256 _price, string memory _title, string memory _category, string memory _ipfs) public returns(address) {
        ...
        emit GigCreated(address(newEscrow), msg.sender);
