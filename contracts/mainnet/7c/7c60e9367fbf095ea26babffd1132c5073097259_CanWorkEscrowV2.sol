...
contract CanWorkEscrowV2 {
    ...
    function depositBEP20(address asset, address provider, uint value, uint JOBID, address[] calldata swapPath) external {
        ...
        _swapPath = swapPath;                                           
        _swapPath.push(BUSD);                                           // Append swapPath array with BUSD (final settlement token)

        uint _finalBUSD; 
        if (asset == BUSD) {
            _finalBUSD = value;                                         // Skips the swap if BEP20 token is already = BUSD
        } else {
            bool approved = iBEP20(asset).approve(ROUTER, value);       // Approve Pancake Router to spend the deposited token
            require (approved, "!Aprv");
