...
contract JetverseLockUp is Ownable {
    ...
    //the percentage that is paid during the period 4,16% every month
    ...
    // owner method for bulk adding users 
    function massLockup(address[] memory _targets, uint256[] memory _amounts) public onlyOwner {
        uint256 count = _targets.length;
        for (uint256 i = 0; i < count; i++){

            if (_targets[i] == address(0) || _amounts[i] == 0){
                continue;
            }else if(lockup_record[_targets[i]].amount != 0 && lockup_record[_targets[i]].complete != true) {
                continue;
            }
            ...
            require(_amounts[i] <= balance);

            lockup_record[_targets[i]] = LockUpRecord({
                owner: _targets[i],
                amount: _amounts[i],
                withdrawSumm: 0,
                startTime: block.timestamp,
            ...
            lockupFounds += _amounts[i];
            countHolders++;
            emit NewFounder(_targets[i], _amounts[i]);
        }
    } 
}