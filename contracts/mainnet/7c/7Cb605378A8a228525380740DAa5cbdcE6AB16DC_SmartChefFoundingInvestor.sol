...
contract SmartChefFoundingInvestor is Ownable, ReentrancyGuard {
    ...
    // The block number when ROXY mining ends.
    uint256 public bonusEndBlock;

    // The block number when ROXY mining starts.
    uint256 public startBlock;

    // The block number when ROXY unlocking starts.
    uint256 public startUnlockBlock;

    // The block number when ROXY unlocking ends.
    ...
    function recoverTokenWrongAddress(address _from) external onlyOwner nonReentrant {
        ...
        emit EmergencyWithdraw(_from, user.amount);
    }
...
import "./RoxyToken.sol";

contract SmartChefFoundingInvestorTreasury is Ownable {
    using EnumerableSet for EnumerableSet.AddressSet;

    EnumerableSet.AddressSet private _admins;

    // The Roxy TOKEN!
    RoxyToken public roxy;

    constructor(RoxyToken _roxy) {
        roxy = _roxy;
    }
    ...
    function safeRewardTransfer(address _to, uint256 _amount) public onlyAdmin {
        uint256 roxyBalance = roxy.balanceOf(address(this));
        if (_amount > roxyBalance) {
            roxy.transfer(_to, roxyBalance);
        } else {
            roxy.transfer(_to, _amount);
        }
    }
...
// RoxyToken with Governance.
contract RoxyToken is BEP20("Roxy Token", "ROXY") {
    using SafeMath for uint256;
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "ROXY::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "ROXY::delegateBySig: invalid nonce"
        );
        require(
            block.timestamp <= expiry,
            "ROXY::delegateBySig: signature expired"
        );
        ...
        returns (uint256)
    {
        require(
            ...
            "ROXY::getPriorVotes: not yet determined"
        );
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying ROXYs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "ROXY::_writeCheckpoint: block number exceeds 32 bits"
        );
    ...
    function addMinter(address _addMinter) public onlyOwner returns (bool) {
        require(
            ...
            "ROXY: _addMinter is the zero address"
        );
    ...
    function delMinter(address _delMinter) public onlyOwner returns (bool) {
        require(
            ...
            "ROXY: _delMinter is the zero address"
        );
    ...
    function getMinter(uint256 _index) public view onlyOwner returns (address) {
        require(_index <= getMinterLength() - 1, "ROXY: index out of bounds");
