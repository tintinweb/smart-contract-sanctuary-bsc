...
// File: contracts/tokenMining.sol
...
contract LYMine is Ownable {
    ...
    address public lpToken=0x9c364FEb11d0E17947392f2Bf9406819e64b79Aa;


    address  public lpTokenSnapshot=0x8E5dc7dadbFE21A06d5b8d62D4bD3Ed85f8e424d;
    ...
    function getUserTrueReward(address _lpToken, address _user)
        public
        view
        returns (uint256)
    {
        uint256 pid = getPid(_lpToken);
        return userInfo[pid][_user].trunReward;
    }
    ...
    function newMining(
        ...
        //     "Mining: last mining not over yet"
        // );
        massUpdatePools();
        ...
        // require(
        //     _tokenAmount > 0 && (period * _newPerBlock) <= newRemaining,
        //     "Mining: too small amount"
        // );

        remainingAmount = newRemaining;
        tokenPerBlock = _newPerBlock;
        startBlock = _startBlock;
        endBlock = startBlock.add(remainingAmount.div(_newPerBlock));
        updatePoolLastRewardBlock(_startBlock);
        emit NewMining(msg.sender, _tokenAmount, _startBlock, _newPerBlock);
    }

    function setNewPerBlock( uint256 _newPerBlock)public  onlyOwner {
        massUpdatePools();
        tokenPerBlock = _newPerBlock;
        startBlock = block.number;
        endBlock = startBlock.add(remainingAmount.div(_newPerBlock));
        updatePoolLastRewardBlock(startBlock);
    }
    ...
    function withdrawRemaining() external onlyOwner {
        require(block.number > endBlock, "Mining: mining not over yet");
    ...
    function deposit(address _lpToken, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pendingAmount =
             ...
             if (pendingAmount > 0) {
                address referrer=ylToken(mintTokenAddress).getReferralRelationship(msg.sender); 
                if(referrer!=address(0)){
                    safeTOKENTransfer(msg.sender, pendingAmount.mul(90).div(100), pool.tokenAmount);
                    user.trunReward=user.trunReward.add(pendingAmount.mul(90).div(100));
                    uint256 referrerFee = pendingAmount.mul(10).div(100);
                    
                    referralRewards[referrer] = referralRewards[referrer].add(referrerFee);
        
                    safeTOKENTransfer(referrer, referrerFee,pool.tokenAmount); 
                }else{
                    safeTOKENTransfer(msg.sender, pendingAmount, pool.tokenAmount);
                    user.trunReward=user.trunReward.add(pendingAmount);
                }
                totalClaim=totalClaim.add(pendingAmount);
                } 
        }else{
            totalMinter+=1;
        }
    ...
    function withdraw(address _lpToken,uint256 _amount)public {
        require(_amount>0,"LP amount must >0");
        uint256 pid = getPid(_lpToken);
        PoolInfo storage pool = poolInfos[pid];
        UserInfo storage user = userInfo[pid][msg.sender];
        require(_amount<=user.amount,"out of LP value");
        updatePool(pid);
        uint256 pendingAmount =
            user.amount.mul(pool.accTOKENPerShare).div(1e12).sub(
                user.rewardDebt
