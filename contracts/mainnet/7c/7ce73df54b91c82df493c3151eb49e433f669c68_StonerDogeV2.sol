...
/**
 * Stoner Doge V2 is here now fren... featuring innovative SMOL inspired tokenomics right on Binance Smart Chain!
 * 
 * TOKENOMICS
 *
 * 4% Taxes
 ...
 *
 * Buy: 4%
 * Sell: 4-12% -- 12% default, reduced to 4% when you flip a coin for STOGE (see FLIP A COIN below on reducing sell tax)
 *
 * Brand new innovative features:
 * 1. Nuke STOGE tokens from the liquidity pool on sells. This means over time sells will have less of a
     price impact and keep the price floor lifted, where buys have the full impact.
 * 2. Initial elevated sell tax that you can reduce by "flipping coins" using random number generation
     for your chance to both double your bag IN ADDITION TO remove sell tax elevation.
     See FLIP A COIN below
 * 3. Perpetual BNB biggest buyer reward every single hour, paid out on any transfer after
     that hour is completed, forever
 * 
 * HOURLY BIGGEST BUYER REWARD
 *
 * We keep track of the biggest buyer (defined by the number of STOGE received) every hour, and this buyer
 * will receive a portion of the contract BNB balance on the subsequent trade following the completion of the hour.
 * Keep an eye our in your wallet for BNB to come flowing in if you are the biggest buyer within an hour.
 *
 *
 * FLIP A COIN (DOUBLE YOUR BAG & REDUCE SELL TAX) - Using the Write Contract functions on BSCScan, you can flip a coin and wager a percentage from 20% to 95% of your Stoge Holdings! Double or Nothing! Reduce your sales tax in the process to winning STOGE!
 *
 *
 * Visit our Website: www.stonerdoge.org
 * Join Our Telegram: https://t.me/stonerdoge
 * Enjoy the new wassified Stoner Doge V2!
 * 
 */
...
contract StonerDogeV2 is ERC20, Ownable {
    uint256 private constant ONE_HOUR = 1 hours;
    uint256 private constant PERCENT_DENOMENATOR = 1000;
    uint256 private constant MAX_INT = type(uint256).max;
    address private constant DEAD = address(0xdead);

    uint256 public coinFlipMinBalancePerc = (PERCENT_DENOMENATOR * 20) / 1000; // 20% user's balance
    uint256 public coinFlipWinPercentage = (PERCENT_DENOMENATOR * 95) / 1000; // 95% wager amount
    uint256 public coinFlipForMarketPercentage =
        (PERCENT_DENOMENATOR * 1) / 1000; // 1% of wager amount for market
    ...
    uint256 public biggestBuyRewardPercentage =
        (PERCENT_DENOMENATOR * 50) / 1000; // 50%
    ...
    uint256 public nukePercentPerSell = (PERCENT_DENOMENATOR * 25) / 1000; // 25%
    ...
    uint256 public taxLp = (PERCENT_DENOMENATOR * 2) / 1000; // 2%
    uint256 public taxBuyer = (PERCENT_DENOMENATOR * 2) / 1000; // 2%
    uint256 public taxMarket = (PERCENT_DENOMENATOR * 2) / 1000; // 2%
    ...
    uint256 private _liquifyRate = (PERCENT_DENOMENATOR * 5) / 10000; // 0.5%
    ...
    constructor(address _routerAddress, address _marketAddress)
        ERC20("Stoner Doge", "STOGE")
    {
        marketAddress = _marketAddress;
        _mint(address(this), 420_000_000_000 * 10**18);
    ...
    function flipCoin(uint256 _percent) external returns (bool) {
        ...
        if (_didUserWin) {
            uint256 _amountToWin = (_finalWagerAmount * coinFlipWinPercentage) /
                PERCENT_DENOMENATOR;
            ...
            _burn(msg.sender, _finalWagerAmount);
            coinFlipsLost++;
            coinFlipAmountLost += _finalWagerAmount;
            coinFlipsUserLost[msg.sender]++;
            coinFlipUserAmountLost[msg.sender] += _finalWagerAmount;
            lastCoinFlipWon[msg.sender] = false;
        }

        canSellWithoutElevation[msg.sender] = true;

        emit SettledCoinFlip(msg.sender, _finalWagerAmount, _didUserWin);
