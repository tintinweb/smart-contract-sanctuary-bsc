...
contract PAWZPresale is Ownable {
 ...
 uint256 public cap;
   ...
   receive() external payable {
    ...
    function getVestStage() public view returns(uint256){
        return vestStage[msg.sender];
    }
    ...
    function _getReleasedTokens() internal returns(uint256){
        ...
        released[msg.sender] += available;
        balances[msg.sender] = balances[msg.sender].sub(available);
        vestStage[msg.sender] = 2;
        }
        //Release another 20% after 7 days
         if(block.timestamp >= (launchDate + 7 days) && balances[msg.sender] > 0 && vestStage[msg.sender] == 2){
