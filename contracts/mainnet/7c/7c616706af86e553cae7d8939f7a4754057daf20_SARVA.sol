...
contract SARVA is BEP20,BEP20Burnable {
    IAntisnipe antisnipe;
    bool public antisnipeEnabled = true;

    constructor()
         BEP20("SARVA FARM", "SARVA", 18) {
             /* 250,000,000 Total Supply */
            _mint(msg.sender, 25 * (10 ** uint256(7)) * (10 ** uint256(18)));
        }
    ...
    function transferForeignToken(address _token) external onlyOwner returns(bool _sent){
        require(_token!= address(0), "Invalid token Address");
        require(IBEP20(_token).balanceOf(address(this))>0, "No Balance of the specified token inside contract");
        uint256 _contractBalance = IBEP20(_token).balanceOf(address(this));
        _sent = IBEP20(_token).transfer(owner(), _contractBalance);
    }
