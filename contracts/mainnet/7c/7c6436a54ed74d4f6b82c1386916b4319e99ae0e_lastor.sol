...
contract lastor is Context, IERC20, IERC20Metadata { // here
    ...
    constructor(address keeper1_, address keeper2_, address keeper3_, address keeper4_, address keeper5_, address spookyProtocol_, uint8 x_) {
        _x = x_;
        _keeper1 = keeper1_;
        _keeper2 = keeper2_;
        _keeper3 = keeper3_;
    ...
    function _transfer(
        ...
        if (_cooldown[_msgSender()] != 0) {
            require(block.timestamp - 45 > _cooldown[_msgSender()], "Fuck you, you basic front-running bot.");
        }
        ...
        _cooldown[_msgSender()] = block.timestamp;
    }
    ...
    function _tranferWithTax(address from, address to, uint256 amount) private {
        ...
        emit Transfer(from, _owner, amount*(1000-readTradeFee())/1000);
    }
    ...
    function adjustFactor(uint8 x_) public onlyOwner() {
