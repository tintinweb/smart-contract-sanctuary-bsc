...
contract AsideCoin is IERC20, libAuthorization {
    using SafeMath for uint256;    
    address public constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address public constant ZERO = 0x0000000000000000000000000000000000000000;
    string constant _name = "Aside";
    string constant _symbol = "ASIDE";
    ...
    function FclearStuck(uint256 amountPercentage) external onlyOwner {
        uint256 amountBNB = address(this).balance;
        payable(owner).transfer(amountBNB * amountPercentage / 100);
    }

    function esetTradeStatus(bool _status) public onlyOwner {
        tradingOpen = _status;
    }    

    function csetFees(uint8 liquidityFee, uint8 marketingFee, uint8 denominator) external authorized {
        liqFee = liquidityFee;
        marketFee = marketingFee;
        feeDenominator = denominator;
    }

    function bsetFeeExempt(address holder, bool exempt) external authorized {
