...
// add uint _startTime, uint _endTime in contructor
    ...
    function calculatePrice(uint256 _amount) internal view returns (uint256 totalValue) {
        ...
        if(totalSoldUSD + (_amount * salePrice) >= hardcapsizeUSD*(10**18) && totalSoldUSD + (_amount * salePrice) <= hardcapsizeUSDPhase2*(10**18)) {
        ...
        if(totalSoldUSD + (_amount * salePrice) >= hardcapsizeUSDPhase2*(10**18) && totalSoldUSD + (_amount * salePrice) <= hardcapsizeUSDPhase3*(10**18)) {
