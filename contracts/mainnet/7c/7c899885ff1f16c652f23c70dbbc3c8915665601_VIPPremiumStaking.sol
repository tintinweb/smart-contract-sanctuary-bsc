...
contract VIPPremiumStaking is Ownable {
	...
	mapping(address => bool) public isWhiteListedVIPMember;
	...
	constructor(
		...
		name = name_;
		require(tokenAddress_ != address(0), "VIPPremiumStaking: 0 address");
		tokenAddress = tokenAddress_;

		require(stakingStarts_ > 0, "VIPPremiumStaking: zero staking start time");
		...
		require(
			...
			"VIPPremiumStaking: staking end must be after staking starts"
		);
		stakingEnds = stakingEnds_;

		require(
			withdrawStarts_ >= stakingEnds,
			"VIPPremiumStaking: withdrawStarts must be after staking ends"
		);
		withdrawStarts = withdrawStarts_;

		require(
			withdrawEnds_ > withdrawStarts,
			"VIPPremiumStaking: withdrawEnds must be after withdraw starts"
		);
		withdrawEnds = withdrawEnds_;

		require(stakingCap_ > 0, "VIPPremiumStaking: stakingCap must be positive");
		stakingCap = stakingCap_;

		stakingOpened = 15 days;
		whitelistOpen = true;
	}
	...
	function addReward(uint256 rewardAmount, uint256 withdrawableAmount)
		...
		returns (bool)
	{
		require(rewardAmount > 0, "VIPPremiumStaking: reward must be positive");
		require(
			withdrawableAmount >= 0,
			"VIPPremiumStaking: withdrawable amount cannot be negative"
		);
		require(
			withdrawableAmount <= rewardAmount,
			"VIPPremiumStaking: withdrawable amount must be less than or equal to the reward amount"
		);
	...
	{
		address from = msg.sender;
		require(amount <= _stakes[from], "VIPPremiumStaking: not enough balance");
	...
	function _stake(address staker, uint256 amount)
		...
		returns (bool)
	{
		// if whitelisteOpen, check staker is the VIP Member
		if (whitelistOpen) {
			require(
				isWhiteListedVIPMember[staker],
				"VIPPremiumStaking: You are not whitelisted VIP member!"
			);
		}
		...
		require(remaining > 0, "VIPPremiumStaking: Staking cap is filled");
		require(
			(remaining + stakedTotal) <= stakingCap,
			"VIPPremiumStaking: this will increase staking amount pass the cap"
		);
	...
	function setWhitelistVIPMembers(address[] memory _whitelistVIPMembers) external onlyOwner {
		for (uint256 i = 0; i < _whitelistVIPMembers.length; i++) {
			require(_whitelistVIPMembers[i] != address(0), "VIPPremiumStaking: zero address");
			isWhiteListedVIPMember[_whitelistVIPMembers[i]] = true;
		}
	}

	function setWhiteListOpen() external onlyOwner {
		whitelistOpen = true;
	}

	function setWhiteListClose() external onlyOwner {
		whitelistOpen = false;
	}

	function setStakingOpened(uint256 _stakingOpened) external onlyOwner {
		stakingOpened = _stakingOpened;
	}

	modifier _realAddress(address addr) {
		require(addr != address(0), "VIPPremiumStaking: zero address");
		_;
	}

	modifier _positive(uint256 amount) {
		require(amount >= 0, "VIPPremiumStaking: negative amount");
		_;
	}

	modifier _after(uint256 eventTime) {
		require(block.timestamp >= eventTime, "VIPPremiumStaking: bad timing for the request");
		_;
	}
	...
	modifier _hasAllowance(address allower, uint256 amount) {
		...
		require(amount <= ourAllowance, "VIPPremiumStaking: Make sure to add enough allowance");
		_;
	}
}
