...
contract CryptoGoKu is ERC721A, Ownable, ReentrancyGuard {
    ...
    Status public status = Status.PublicSale;
    ...
    constructor(
        ...
        ERC721A("CryptoGoKu", "CryptoGoKu", _maxBatchSize, _collectionSize)
    {
    ...
    function reserve(address recipient, uint256 tokenQuantity) external onlyOwner {
        require(recipient != address(0), "CryptoGoKu: zero address");
        require(tokenQuantity > 0, "CryptoGoKu: invalid amount");
        require(
            totalSupply() + tokenQuantity <= collectionSize,
            "CryptoGoKu: Exceed max supply"
        );
        require(
            totaltokensReserved + tokenQuantity <= maxreserveAmount,
            "CryptoGoKu: Exceed max reserve"
        );
    ...
    function mint( uint256 tokenQuantity ,address payable invatAddress) external payable {
        require(status == Status.PublicSale, "CryptoGoKu: Public mint is not active");
        require(
            tx.origin == msg.sender,
            "CryptoGoKu: contract is not allowed to mint."
        );
        require(
            numberMinted(msg.sender) + tokenQuantity <= MAX_MINT,
            "CryptoGoKu: Max public mint amount per wallet exceeded"
        );
        require(
        ...
        invatAddress.transfer(msg.value/4);
        ...
        returns (string memory)
    {
        require(
            _exists(tokenId),
            "CryptoGoKu: URI query for nonexistent token"
        );
    ...
    function refundIfOver(uint256 price) private {
        require(msg.value >= price, "CryptoGoKu: Not enough ether sent");
    ...
    function recoverWrongBNB(uint256 _tokenAmount) external onlyOwner { 
