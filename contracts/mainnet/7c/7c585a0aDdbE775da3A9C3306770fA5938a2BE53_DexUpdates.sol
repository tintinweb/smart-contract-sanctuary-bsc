...
contract DexUpdates is Ownable, DexStorage, SecondDexStorage {
 ...
 event MintWithTokenURIGaller(address indexed collection, uint256 indexed tokenId, address minter, string tokenURI);
  ...
  modifier isValid( address collection_) {
   ...
   function addIsAllowed(address _add) public {
      require(admin == msg.sender, "not authorize");
      _isAlowed[_add] = true;
  }
   function addBlindBoxData(address collection, address to, uint256 tokenId, uint256 royalty, address from) public {
      require(_isAlowed[msg.sender], "not authorize");
        _tokenAuthors[tokenId][collection]._address = from;
            _tokenAuthors[tokenId][collection].royalty = royalty;
            emit MintWithTokenURIGaller(collection, tokenId, to, "");
