...
// File: contracts/ONUS-launchpad-v4.sol
...
contract ONUSLaunchpadV4 is AccessControlEnumerable, ReentrancyGuard {
    ...
    // Struct ProjectRefundable
    struct ProjectRefundable {
        bool isRefundable;
        uint256 feePercentage;
        uint256 maxFee;
        uint256 endTimestampRefundable;
        uint256 PRECISION_FACTOR;
    }

    /// @dev Project ID => the stages
    mapping(uint256 => ProjectRefundable) public projectRefundableAtId;
    ...
    /// @dev Project ID => Count amount refund;
    mapping(uint256 => uint256) public userRefundCountAtId;
    ...
    /// @dev Project ID => Is user refuned;
    mapping(uint256 => mapping(address => bool)) public userIsRefundedAtId;
    ...
    event ClaimedRefund(uint256 projectId, address indexed account, uint256 amount, uint256 time);
    ...
    function deposit(uint256 _projectId, uint256 _amount, bytes32[] calldata _proofs) external nonReentrant {
        ...
        if (userDepositAtId[_projectId][msg.sender] == 0) {
    ...
    function claimRefund(uint256 _projectId) external nonReentrant {
        Project memory projectDetail = projectAtId[_projectId];
        ProjectRefundable memory projectRefundDetail = projectRefundableAtId[_projectId];
        require(
        ...
        require(
            userIsRefundedAtId[_projectId][msg.sender] == false,
            "Refund is claimed"
        );
        ...
        if (projectRefundDetail.isRefundable) {
            require(
                block.timestamp <= projectRefundDetail.endTimestampRefundable,
                "Refund time not valid"
            );
            uint256 fee = projectRefundDetail.maxFee.min(userDepositAtId[_projectId][msg.sender].mul(projectRefundDetail.feePercentage).div(projectRefundDetail.PRECISION_FACTOR));
            uint256 userRefundableAmount = userDepositAtId[_projectId][msg.sender].sub(fee);

            if (userRefundableAmount > 0) {
                projectDetail.currencyToken.safeTransfer(msg.sender, userRefundableAmount);
                userRefundCountAtId[_projectId] += userRefundableAmount;
                userIsRefundedAtId[_projectId][msg.sender] = true;
                emit ClaimedRefund(_projectId, msg.sender, userRefundableAmount, block.timestamp);
            }
    ...
    function claimTGE(uint256 _projectId) external nonReentrant {
        ...
        (uint256 userRemaining,uint256 amountClaimTGE) = getAmountClaimTGE(_projectId);
    ...
    function claimStage(uint256 _projectId, uint256 _stageIndex) external nonReentrant {
        ...
        require(
            projectDetail.isPausedProject != true,
             "This contract is pause!"
        );
        require(
            _stageIndex <= claimStagesAtId[_projectId].length - 1, 
            "Not valid stage index"
        );
        require(
            userIsRefundedAtId[_projectId][msg.sender] == false,
            "Refund is claimed"
        ...
        // calc amount claim
        uint256 amountClaimByStage = getAmountClaimStage(_projectId, _stageIndex);
    ...
    // Update project refundable
    function updateProjectRefundable(uint256 _projectId, bool _isRefundable, uint256 _feePercentage, uint256 _maxFee, uint256 _endTimestampRefundable, uint256 _PRECISION_FACTOR)  external onlyOwner {
        Project memory projectDetail = projectAtId[_projectId];
        require(
            block.timestamp < projectDetail.depositCondition.startTimestampDeposit,
            "Launchpad has been running"
        ...
        projectRefundableAtId[_projectId] = ProjectRefundable({
            isRefundable: _isRefundable,
            maxFee: _maxFee,
            feePercentage: _feePercentage,
            endTimestampRefundable: _endTimestampRefundable,
            PRECISION_FACTOR: _PRECISION_FACTOR
        });
    }
    ...
    function getAmountClaimTGE(uint256 _projectId) public view returns (uint256, uint256) {
        Project memory projectDetail = projectAtId[_projectId];
        ...
        uint256 percentageAllocation = (userDepositAtId[_projectId][msg.sender].mul(10**18).div(projectDetail.totalUserDeposit)).min(
            userDepositAtId[_projectId][msg.sender].mul(10**18).mul(10**projectDetail.saleToken.decimals()).mul(10**projectDetail.currencyToken.decimals()).div(projectDetail.totalSaleToken.mul(projectDetail.pricePerSaleToken).mul(10**projectDetail.currencyToken.decimals()))
        );
        uint256 userRemaining = userDepositAtId[_projectId][msg.sender].sub(percentageAllocation.mul(projectDetail.totalSaleToken.mul(projectDetail.pricePerSaleToken).div(10**projectDetail.saleToken.decimals())).div(10**18));
        ...
        return (userRemaining, amountClaimTGE);
    }

    function getAmountClaimStage(uint256 _projectId, uint256 _stageIndex) public view returns (uint256) {
        Project memory projectDetail = projectAtId[_projectId];
        ...
        uint256 amountUser = percentageAllocation.mul(projectDetail.totalSaleToken).div(10**18);
        uint256 amountClaimByStage = amountUser.mul(claimStagesAtId[_projectId][_stageIndex].stageRelease).div(projectDetail.PRECISION_FACTOR);

        return amountClaimByStage;
    }
