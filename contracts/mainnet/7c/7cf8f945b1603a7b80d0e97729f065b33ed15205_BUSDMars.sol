...
contract BUSDMars {
	...
	uint256[] public REFERRAL_PERCENTS 	= [1000, 250, 150, 75, 25];
	uint256[] public MARS_PERCENTS 		= [1000, 800, 600, 400, 200];
	uint256 constant public PROJECT_FEE = 300;
	uint256 constant public MARKETING_FEE = 300;
	uint256 constant public PARTNER_FEE = 300;
	...
	event MarsIncome(address indexed referrer, address indexed referral, uint256 indexed level, uint256 amount);
	event FeePayed(address indexed user, uint256 totalAmount);

	constructor(address payable wallet, address payable _marketingWallet, address payable _partnerWallet) public {
		require(!isContract(wallet));
		commissionWallet = wallet;
		marketingWallet = _marketingWallet;
        partnerWallet = _partnerWallet;

        plans.push(Plan(360, 350));
		plans.push(Plan(130, 400));
		plans.push(Plan(60, 450));
	...
	function invest(address referrer, uint8 plan, uint256 amounts) public  {
		...
		uint256 fee1 = amounts.mul(MARKETING_FEE).div(PERCENTS_DIVIDER);
        token.transfer(marketingWallet,fee1);
		uint256 fee2 = amounts.mul(PARTNER_FEE).div(PERCENTS_DIVIDER);
        token.transfer(partnerWallet,fee2);
		emit FeePayed(msg.sender, fee);
		emit FeePayed(msg.sender, fee1);
		emit FeePayed(msg.sender, fee2);
	...
	function withdraw() public {
		...
		uint256 marsAmount = getcurrentmarsincome(msg.sender);
		...
		totalAmount = totalAmount.add(marsAmount);
		user.withdrawnmars = user.withdrawnmars.add(marsAmount);
	...
	function getUserMarsIncome(address userAddress) public view returns (uint256){
	
		uint256 totalMarsAmount;
		uint256 marsshare;
		...
		for	(uint256 y=1; y<= count; y++)
		{
			...
			for (uint256 i = 0; i < downline.deposits.length; i++) {
				...
				if (downline.deposits[i].start < finish) {
					...
					uint256 to = finish < block.timestamp ? finish : block.timestamp;
					//mars income
                    marsshare = share.mul(MARS_PERCENTS[level-1]).div(PERCENTS_DIVIDER);
					
					if (from < to) {
					
							totalMarsAmount = totalMarsAmount.add(marsshare.mul(to.sub(from)).div(TIME_STEP));	
		...
		return totalMarsAmount;		
	
	} 
	
	
	function getcurrentmarsincome(address userAddress) public view returns (uint256){
	    User storage user = users[userAddress];
	    return (getUserMarsIncome(userAddress).sub(user.withdrawnmars));
	    
	}
	
	function getUserTotalMarsWithdrawn(address userAddress) public view returns (uint256) {
		return users[userAddress].withdrawnmars;
	}
