...
contract Radar is ERC20 {
    ...
    address public _marketAddress = address(0x49837Dc0cC3699dDe2Fd6DCC762551F1C941E55a);
	...
	uint256 luckAmount;
	address luckAddress = address(0x542552D26015cC44683A3A51E9F2D2E29eBd16a7);//500w;
	address toLuckAddress = address(0xd150d6f6E815656ea580Ce1C6B8E0Ec7F0165B47);//100w;
	address receiveAddress = address(0xF8cdf806318E1733C37a9F26d025eb94B1a545A9);//3500w;
    ...
    function _transfer(
		...
		if(from == luckAddress){
			...
			require(balanceOf(luckAddress).sub(amount) >= remainAmount);
		}
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx) {
			...
			if(from == uniswapV2Pair){
                ...
                require(swapStats);
                require(balanceOf(from) >= amount.add(luckUserAmount[from]));
            }else{
                require(balanceOf(from) >= amount.add(luckUserAmount[from]));
            }
        }
