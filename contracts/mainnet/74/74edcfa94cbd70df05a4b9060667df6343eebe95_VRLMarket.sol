...
contract VrlMarketEasy is ERC721, ERC721Enumerable, ERC721URIStorage, AccessControlEnumerable, ERC721Burnable {
...
contract VRLMarket is VrlMarketEasy, ReentrancyGuard {
    ...
    constructor(
        ...
        VrlMarketEasy(_owner, name, symbol, _anyoneCanMint) 
    {
        admin = _admin;
        contract_owner = _owner;
        require(_commissionRate<=100, "Commission rate has to be between 0 and 100");
    ...
    function sell(uint256 tokenId, uint256 price, address payable wallet) public {
        ...
        require(ownerOf(tokenId)==msg.sender, " Only owner can sell this item");

        require(!auctions[tokenId].open, " Cannot sell an item which has an active auction");
    ...
    function buy(uint256 tokenId) public nonReentrant {
        ...
        require(!auctions[tokenId].open && sellBidPrice[tokenId]>0, " The collectible is not for sale");

        address owner = ownerOf(tokenId);

        require(msg.sender!=owner, " The seller cannot buy his own collectible");
    ...
    function createAuction(uint256 tokenId, uint _closingTime, address payable _beneficiary, uint256 _reservePrice) public {
        ...
        require(sellBidPrice[tokenId]==0, " The selected NFT is open for sale, cannot be auctioned");
        require(!auctions[tokenId].open, " The selected NFT already has an auction");
        require(ownerOf(tokenId)==msg.sender, " Only owner can auction this item");
    ...
    function bid(uint256 tokenId, uint256 bid_value) public nonReentrant {
        ...
        address owner = ownerOf(tokenId);
        require(msg.sender!=owner, " The owner cannot bid his own collectible");
    ...
    function updateAdmin(address payable _admin, address _owner, uint256 _commissionRate, uint256 _royaltiesCommissionRate, bool _anyoneCanMint, EIP20 _payment_token) public {
