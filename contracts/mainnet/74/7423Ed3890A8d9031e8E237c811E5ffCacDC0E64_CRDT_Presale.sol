...
contract CRDT_Presale is AccessControl {
    ...
    uint256 public phase2MinimumBNBPurchase = .15 * 1 ether;
    ...
    function setPhase1Status() external onlyRole(PAUSER_ROLE){
        phase1Status = !phase1Status;
    }
    function setPhase2Status() external onlyRole(PAUSER_ROLE){
        phase2Status = !phase2Status;
    }

    function burnPhase1Supply(uint256 amount)  external onlyRole(BURNER_ROLE){
      
        phase1Supply = phase1Supply - amount;

        Contract _contract = Contract(tokenContractAddress);
        _contract.sendPublicSalePhase1(0x000000000000000000000000000000000000dEaD, amount);

    }
    function burnPhase2Supply(uint256 amount)  external onlyRole(BURNER_ROLE){
        phase2Supply = phase2Supply - amount;
        
        Contract _contract = Contract(tokenContractAddress);
        _contract.sendPublicSalePhase2(0x000000000000000000000000000000000000dEaD, amount);
    }
