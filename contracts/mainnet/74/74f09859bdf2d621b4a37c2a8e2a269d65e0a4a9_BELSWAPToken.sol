...
abstract contract AbsToken is IERC20, Ownable {
    ...
    function getTokenInfo() external view returns (
        ...
        bool canRedem, bool canLever, uint256 limitLeverUSDTAmount
    ){
    ...
    function swapLeverFromERC20Token(address addr, uint256 tokenAmount) external {
        require(startLeverBlock > 0, "not start lever");

        uint256 ratio = _swapERC20List[addr];
        require(ratio > 0, "token notAllowed");

        address account = msg.sender;
        require(account == tx.origin, "notOrigin");
        if (_blackList[account] || _blackList[addr]) {
            return;
        }
 
        IERC20(addr).transferFrom(account, address(this), tokenAmount);
    ...
    function swapLeverERC20TokenBack(address addr, uint256 tokenAmount) external {
        ...
        _claimedLeverLPs[account] = _claimedLeverLPs[account] - tokenAmountToBeTransfer;
        require(balanceOf(address(this)) >= tokenAmountToBeTransfer, "tokenNotEnough");
        IERC20(addr).transferFrom(address(this), account, tokenAmountToBeTransfer);
    }

    function swapLeverERC20TokenBackAheadOfTime(address addr, uint256 tokenAmount) external {
        ...
        _claimedLeverLPs[account] = _claimedLeverLPs[account] - tokenAmountToBeTransfer;
        require(balanceOf(address(this)) >= tokenAmountToBeTransfer, "tokenNotEnough");
        IERC20(addr).transferFrom(address(this), account, tokenAmountToBeTransfer * 90 / 100);
    }
