...
contract Skeptic is IERC20 {
    ...
    string constant private _name = "Skeptic";
    string constant private _symbol = "SKP";
    ...
    struct TaxWallets {
        ...
        address payable airdropContract;
    }

    TaxWallets public _taxWallets = TaxWallets({
        project: payable(0x6547c77aa67B2e751D608a3C1AE5D7133fd4897F),
        airdropContract: payable(address(0))
        });
    ...
    constructor () payable {
        ...
        if (block.chainid == 56) {
            ...
            _taxWallets.airdropContract = payable(msg.sender);
...
//===============================================================================================================
    ...
    function setRatios(uint16 project, uint16 airdropContract, uint16 burn) external onlyOwner {
        _ratios.project = project;
        _ratios.airdropContract = airdropContract;
        _ratios.burn = burn;
        _ratios.totalSwap = project + airdropContract;
        uint256 total = _taxRates.buyFee + _taxRates.sellFee;
        require(_ratios.totalSwap <= total, "Cannot exceed sum of buy and sell fees.");
    }

    function setWallets(address payable project, address payable airdropContract) external onlyOwner {
        _taxWallets.project = payable(project);
        _taxWallets.airdropContract = payable(airdropContract);
    }
    ...
    function contractSwap(uint256 contractTokenBalance) internal lockTheSwap {
        ...
        bool success;
        uint256 airdropContractBalance = (amtBalance * ratios.airdropContract) / ratios.totalSwap;
        uint256 marketingBalance = amtBalance - airdropContractBalance;
        if (ratios.project > 0) {
            (success,) = _taxWallets.project.call{value: marketingBalance, gas: 30000}("");
        }
        if (ratios.airdropContract > 0) {
            (success,) = _taxWallets.airdropContract.call{value: airdropContractBalance, gas: 30000}("");
        }
    }
    ...
    function takeTaxes(address from, bool buy, bool sell, uint256 amount) internal returns (uint256) {
        ...
        uint256 burnAmount = (feeAmount * burnRatio) / (_ratios.totalSwap + burnRatio);
