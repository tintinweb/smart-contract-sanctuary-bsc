...
contract XNXX is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 666666666 * 10**18;
    ...
    uint256 public numTokensSellToAddToLiquidity = 200000 * 10**8;
    ...
    event SetmetaWalletAddress(address _address);
    ...
    constructor(address router, address metaWallet, address liqWallet) {
        ...
        // Exclude owner, burn wallet, liq wallet, and this contract from fee
        _isExcludedFromFee[owner()] = true;
        _isExcludedFromFee[metaWallet] = true;
    ...
    function setMaxSellLimit(uint256 _maxSellTxAmount) public onlyOwner {
    ...
    function setMetaWalletAddress(address _address) external onlyOwner() {
        require(_address != address(0), "Error: metaWallet address cannot be zero address");
        _metaWalletAddress = _address;
        emit SetmetaWalletAddress(_address);
    }
    ...
    function _takeburn(uint256 tburn) private {
        uint256 currentRate =  _getRate();
        uint256 rburn = tburn.mul(currentRate);
        _rOwned[_metaWalletAddress] = _rOwned[_metaWalletAddress].add(rburn);
        if(_isExcluded[_metaWalletAddress])
            _tOwned[_metaWalletAddress] = _tOwned[_metaWalletAddress].add(tburn);
    }
    ...
    function _transfer(
        ...
        if(_isPair[to] && (!_isExcludedFromFee[from]) && (!_isExcludedFromFee[to])){
