...
// t.me/BitsXTGroup official telegram. 
...
contract BITSNT is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    BITSNTDividendTracker public dividendTracker;
    ...
    address payable public _marketingWalletAddress = payable(0xf6a90D86220a11E91562E254e0a23839f8cCFa32);
    ...
    constructor() BEP20("BITS-NT", "BTNT") 
    {
        dividendTracker = new BITSNTDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "BITSNT: The dividend tracker already has that address");
        BITSNTDividendTracker newDividendTracker = BITSNTDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "BITSNT: The new dividend tracker must be owned by the BITSNT token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BITSNT: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BITSNT: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BITSNT: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BITSNT: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BITSNT: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BITSNT: Cannot update gasForProcessing to same value");
...
contract BITSNTDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("BITSNT_Dividend_Tracker", "BITSNT_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "BITSNT_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "BITSNT_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BITSNT contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BITSNT_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BITSNT_Dividend_Tracker: Cannot update claimWait to same value");
