...
interface IStakingPlatform {
     ...
     * @dev `totalStaked + amount` must be less than `stakingMin`
...
contract StakingPlatform is IStakingPlatform, Ownable {
    ...
    IERC20 public immutable doge;
    IEVT public immutable EVT;

    uint256 public immutable fixedAPY;

    uint256 public immutable stakingDuration;
    uint256 public stakingMin = 0;
    uint256 public stakingMax = 0;
    ...
    uint256 public _numberOfDaysForFee = 0 days;
    uint256 public limitFee = 0;
    address public DAO;
    address public evtAddress;
    ...
    constructor(
        ...
        fixedAPY = 1;
        evtAddress = _EVTtoken;
        token = IERC20(_EVTtoken);
        doge = IERC20(_Doge);
        EVT = IEVT(_EVTtoken);
        DAO = payable(_dao);
    }
    ...
    function setNumberOfDaysForFee(uint256 _days) external onlyOwner {
        _numberOfDaysForFee = _days;
    }
    ...
    function setMinTokenStake(uint256 _minToken) external onlyOwner {
        stakingMin = _minToken;
    }

    function setMaxTokenStake(uint256 _maxToken) external onlyOwner {
        stakingMax = _maxToken;
    }

    function getWithdrawableDividend() external view onlyOwner returns (uint256) {
        uint256 dividend = IEVT(EVT).withdrawableDividendOf(address(this));
        return dividend;
    }

     function WithdrawDividens() public onlyOwner {
        IEVT(EVT).withdrawDividend();
    }

    function WithdrawDoge() public onlyOwner {
        doge.safeTransfer(msg.sender, doge.balanceOf(address(this)));
    }
    function withdrawOtherTokens(address _tokenContract, uint256 _amount) external onlyOwner {
        require(_tokenContract != evtAddress, "Notice: only can withdraw tokens that are not EVT");
        IERC20 tokenContract = IERC20(_tokenContract);
        // transfer the tokens that are NOT EVT from address of this contract
        tokenContract.transfer(msg.sender, _amount);
    }
    
    function withdrawETH() external onlyOwner {
                // transfer the ETH  from address of this contract
   ...
   /**
    ...
    function deposit(uint256 amount) external override {
        ...
        require(
            amount >= stakingMin && amount <= stakingMax,
            "Amount staked is less then minimun or more than maximun "
        );
    ...
    function _claimRewards() private {
        ...
        uint256 withdrawFee = (rewardsToClaim * limitFee) / 100;
        uint256 restRewards = rewardsToClaim - withdrawFee;

        _rewardsToClaim[_msgSender()] = 0;

        token.safeTransfer(_msgSender(), restRewards);
        token.safeTransfer(DAO, withdrawFee);
        emit Claim(_msgSender(), restRewards);
    }
