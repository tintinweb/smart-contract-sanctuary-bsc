...
contract BNBback is IERC20, Context, Ownable {
    ...
    string private constant _name = "BNB Back";

    uint256 public Bnbb_max_BuyLimit = 21000000 * 10**9;
    uint256 public Bnbb_max_SellLimit = 5000000 * 10**9;

    address private Bnbb_Back_Wallet = 0x1E1B40f30bE68cE56bF39bDda02B6b69a49D55a6;
    address private Bnbb_Team_Wallet = 0x3615FD02f7BF2f768fba4bc1d201ec2AbCD0a18C;

    uint256 public Bnbb_max_WalletLimit = 25000000 * 10**9;
    uint256 public swapTokensAtAmount = 600000 * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[Bnbb_Back_Wallet] = true;
        _isExcludedFromFee[Bnbb_Back_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[Bnbb_Back_Wallet] = true;
        allowedTransfer[Bnbb_Back_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= Bnbb_max_BuyLimit, "You are exceeding Bnbb_max_BuyLimit");
            require(balanceOf(to) + amount <= Bnbb_max_WalletLimit, "You are exceeding Bnbb_max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= Bnbb_max_SellLimit, "You are exceeding Bnbb_max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= Bnbb_max_WalletLimit, "You are exceeding Bnbb_max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(Bnbb_Back_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(Bnbb_Back_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function Doji_Update_MarketingWallet(address newWallet) external onlyOwner{
        Bnbb_Back_Wallet = newWallet;
    }
    
    function Bnbb_Update_TeamWallet(address newWallet) external onlyOwner{
        Bnbb_Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        Bnbb_max_BuyLimit = maxBuy * 10**decimals();
        Bnbb_max_SellLimit = maxSell * 10**decimals();
    }
    
    function Bnbb_Max_Walletlimit(uint256 amount) external onlyOwner{
        Bnbb_max_WalletLimit = amount * 10**decimals();
    }
