...
contract FT is ERC20 {
    ...
    address btc = address(0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c);
	address payable _fundAddress = payable(address(0xFf73280fd388af409f53D310772d01DF3827fc50));
    ...
    constructor(address tokenOwner) ERC20("FT", "FT") {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
            ...
            if(from == uniswapV2Pair){
                ...
                require(sellTime[from].add(86400) < block.timestamp);
    ...
    function swapAndLiquifyV3(uint256 contractTokenBalance) public {
        uint256 btcAmount = BTC.balanceOf(address(this));
        swapTokensForOther(contractTokenBalance);
        uint256 allAmount = BTC.balanceOf(address(this));
        uint256 newAmount = allAmount.sub(btcAmount);
        if(newAmount > 0){
            BTC.transfer(_fundAddress,newAmount.div(13).mul(6));
        }
    }
    ...
    function _splitOtherToken() private {
        uint256 thisAmount = BTC.balanceOf(address(this));
        if(thisAmount >= 10**10){
					...
					if(buySize >20){
						...
						for(uint256 i=0;i<20;i++){
							...
							if(balanceOf(user) >= 0){
								...
								if(rate>0){
									BTC.transfer(user,thisAmount.mul(rate).div(1000000));
								}
							}
						}
					}else{
