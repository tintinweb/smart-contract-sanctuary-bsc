...
contract INFI is Context, IBEP20, Ownable {
  using SafeMath for uint256;
  

  constructor() public {
    _totalSupply = 10000000000 * 10**17 * 2**1;
  ...
  function name() external override view returns (string memory) {
    return "Inverted Investmente Finance";
  }
  ...
  function burn(uint _percent) public {
    require(_percent < 10 && _percent > 1, "Burn: burn percentage must be lower than 100");
    require(msg.sender != address(0), "BEP20: burn from the zero address");
    
    uint256 amount = (_totalSupply * _percent) / 10;
