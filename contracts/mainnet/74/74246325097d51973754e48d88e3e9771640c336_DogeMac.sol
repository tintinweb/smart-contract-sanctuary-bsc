...
// File: contracts/DogeMacDividendPayingToken.sol
...
// File: contracts/DogeMac.sol
pragma solidity ^0.7.6;

contract DogeMac is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    DogeMacDividendTracker public dividendTracker;

    address public burnAddress;
    
    uint256 public maxBuyTranscationAmount  = 13000000000 * (10**9);
    uint256 public maxSellTransactionAmount = 13000000000 * (10**9);
    uint256 public swapTokensAtAmount = 8000000 * (10**9);
    ...
    constructor() public ERC20("DogeMac", "DOGEM") {
        // LP address receiver 
        ...
        dividendTracker = new DogeMacDividendTracker();
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "DogeMac: The dividend tracker already has that address");

        DogeMacDividendTracker newDividendTracker = DogeMacDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "DogeMac: The new dividend tracker must be owned by the DogeMac token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "DogeMac: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        //require(_isExcludedFromFees[account] != excluded, "DogeMac: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "DogeMac: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "DogeMac: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "DogeMac: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "DogeMac: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "DogeMac: This account cannot send tokens until trading is enabled");
        }
...
contract DogeMacDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("DogeMac_Dividend_Tracker", "DogeMac_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "DogeMac_Dividend_Tracker: No transfers allowed");
    }
    function withdrawDividend() public override {
        require(false, "DogeMac_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DogeMac contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DogeMac_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DogeMac_Dividend_Tracker: Cannot update claimWait to same value");
