...
contract DoughTokenCloneable is Initializable, ERC20Upgradeable, OwnableUpgradeable, VersionedUpgradeable, IDoughTokenCloneable  {
    ...
    {        
        ...
        swapContractTokensAtAmount = _tokenInfo.totalSupplyWithoutDecimals * 10**_tokenInfo.decimals / 1000; 
    ...
    /**
      ...
      * 
      * @param tokenAmountWithDecimals The token amount (without decimals) at which to trigger the contract swap
      */
    function setSwapContractTokensAtAmount (uint256 tokenAmountWithDecimals) external onlyOwner {
        uint256 _totalSupply = totalSupply();
        require (tokenAmountWithDecimals >= _totalSupply / 1_000_000 && tokenAmountWithDecimals <= _totalSupply / 1000, createErrorMessage ("Must be >0.0001% & <0.1% supply"));
        emit SwapContractTokensAtAmountChanged (swapContractTokensAtAmount, tokenAmountWithDecimals);
        swapContractTokensAtAmount = tokenAmountWithDecimals;
    }
    ...
    function checkTransferRequirementsAndSwap (address from, address to, uint256 amount) internal returns (uint256 transferAmount, uint8 feeType) {
        ...
        feeType = isSell ? SELL : isBuy ? BUY : EOW_TRANSFER;
        ...
        if (!isExcludedFromFees[from] && !isExcludedFromFees[to]) {
            ...
            if (!swapping) { // If we are swapping we shouldn't ever get here, but leave this check in to be safe
                if (balanceOf (address(this)) >= swapContractTokensAtAmount && !isBuy) {
                    swapping = true;
                    swapRequiredFunds();
                    swapping = false;
                }
    ...
    function swapTokensForEth (uint256 tokenAmount) internal returns (uint256) {
        if (tokenAmount > 0) {
            ...
            address router = addressInfo.router;
            // generate the uniswap pair path of token -> weth
            address[] memory path = new address[](2);
            path[0] = address(this);
            path[1] = ISwapRouter02(router).WETH();
    
            _approve (address(this), router, tokenAmount);
    
            // make the swap
            ISwapRouter02(router).swapExactTokensForETHSupportingFeeOnTransferTokens (
                tokenAmount,
    ...
    function addLiquidity (uint256 tokenAmount, uint256 ethAmount) internal returns (uint256) {
        if (tokenAmount > 0 && ethAmount > 0) {
            ...
            // add the liquidity
            (,uint256 ethFromLiquidity,) = ISwapRouter02(router).addLiquidityETH {value: ethAmount} (
                address(this),
                tokenAmount,
    ...
    /**
      ...
      * @param to The address of the recipient
      * @param amount The amount of tokens being transferred
      * @return continueTransfer Whether there are additional transfer actions to perform
      */
    function preTransferActions (address from, address to, uint256 amount) internal virtual returns (bool continueTransfer) {
