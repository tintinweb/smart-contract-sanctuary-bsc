...
contract BLXNBusdHike is Context, Ownable, ReentrancyGuard {
    ...
    function stakeStablecoins(uint256 amtx, address ref) payable public noReentrant {
        // Makes sure you can't deposit early/before launch date, unless you are a VIP/Whitelist member.
        require(block.timestamp >= firstlaunch || WhiteList[msg.sender].member == true, "You are not part of whitelist");
        ...
        user.userMax += adjustedAmt*3; //ask abt this amt
    ...
    function withdrawDivs() public noReentrant returns (uint256 withdrawAmount){
        ...
        uint256 ideaTransfer = x.mul(50).div(1000); //untested
    ...
    function calcdiv(address dy) public view returns (uint256 totalWithdrawable){
        ...
        // Iterate through each deposit struct in User's deposit list and calculate the dividends earned by user CalcDiv function.
        for (uint256 i = 0; i < user.depoList.length; i++){	
            uint256 elapsedTime = block.timestamp.sub(user.depoList[i].depoTime).div(86400); //.div(86400) final deployment
            uint256 elapsedTimeb = block.timestamp.sub(user.depoList[i].depoTime).mod(86400);
    ...
    function RetroRem(uint256 amty, uint256 timea, uint256 timeb, uint256 inc) public pure returns (uint256 remPayOut){
        ...
        uint256 rem = timea;
        uint256 remCalc = rem.mul(100).div(86400); //changed to 86400 after testing
    ...
    function Retro(uint256 amount, uint256 time, uint256 increment) public pure returns (uint256 na) {
    ...
    if (time >= 10) { 
        uint256 remainder = time%10; //last digit 5
        uint256 x = time - remainder; // 30
        uint256 y = x/10; // 3
        ...
        for (uint256 i = 0; i < y; i++){
            newAmt += amount.mul(z).div(100)*10; ///1000* 10;//(days) ; //
            if (i < 4){
            z += increment;
            }
            if (i == y-1){
