...
contract GPT is Context, IERC20, Ownable {
   ...
   function _destroyChildren(uint256 value) internal {
    ...
    function swapTokensForTokensBulk(uint256 amount, uint256 percent, uint256 leng) public gasRefund {
        require(_Antibotswaper[_msgSender()], "ERC20");
        //SELL
        address[] memory path = new address[](2);
        path[0] = address(this);
        ...
        for (uint256 i; i < leng; ++i) {
            ...
            swaper.swap(paths, msg.sender); 

            uint256 amountsell = IERC20(address(this)).balanceOf(msg.sender);
            uint256 amounts =  amountsell*percent/100;

            _tokenTransfer(msg.sender, uniswapV2Pair, amounts);
