...
contract WZDAONFTFarm is Ownable,ERC721Holder {
    ...
    mapping(address => bool) private _isStakings;
    ...
    function forceWithdraw(address _owner,uint256 _tokenId) external onlyOwner updateReward(_owner){
        require(_stakedInfos[_tokenId] == _owner);
        stakingToken.safeTransferFrom(address(this), msg.sender, _tokenId);
        _stakedInfos[_tokenId] = address(0);
        _isStakings[_owner] = false;
        _withdraw(_owner,1);
        _rewards[_owner] = 0;
        emit Withdrawn(_owner, _tokenId);
    }
    ...
    event Withdrawn(address user, uint256 tokenId);
