...
contract FeeCollector is AutomationCompatible {
  ...
  function _collectFee(IIndexToken token) internal {
    // todo: check implementation against StreamingFeeModule of tokensets
    uint256 fee = token.totalSupply() * feeSize[address(token)] / 1_000_000;
    address feeReceiver = token.getFeeReceiver();
    token.mint(feeReceiver, fee);
    
    IIndexToken.Asset[] memory assets = token.getAssets();
    for (uint256 i = 0; i < assets.length; i++) {
      assets[i].amount = assets[i].amount * (1_000_000 - feeSize[address(token)]) / 1_000_000;
    }
