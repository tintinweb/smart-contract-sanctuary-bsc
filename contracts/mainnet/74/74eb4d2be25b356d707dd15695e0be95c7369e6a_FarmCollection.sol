...
contract FarmCollection  {
    address public MasterChef = 0xEc82cd083f7A0F0F533b82aa3C2f40079914B9a4;                    // change with masterchef
    address public rToken;
    uint256 public rDecimals;
    string public rSymbol;
// FARM CALLS

    constructor() {
        rToken = masterChef(MasterChef).cake();
        rDecimals = token(rToken).decimals();
        rSymbol = token(rToken).symbol();
    }

    function getPoolLength() external view returns (uint256 Length) {
        return masterChef(MasterChef).poolLength();
    }

// V1 calls for original code
    function cakePerBlock() public view returns (uint256 tokenPerBlock) {
        return masterChef(MasterChef).cakePerBlock();
     }
     function getPoolAllocPoint(uint256 pid) external view returns(uint256 allocPoint) {
        (, allocPoint,,) = masterChef(MasterChef).poolInfo(pid);
     }

     function getTotalAllocPoint() external view returns(uint256 totalAllocPoint) {
         return masterChef(MasterChef).totalAllocPoint();
     }

    // poolalloc * mintrate / totalalloc
    function getRewardPerBlock(uint256 poolAllocPoints) public view returns (uint256 rpb){
    ...
    function getFarmTokenInfo(uint256 pid) public view returns ( address token0, uint256 t0Dec, string memory t0Sym,address token1, uint256 t1Dec, string memory t1Sym) {
        address LPToken;
    ...
    function getFarmInfo(uint256 pid) public view returns (address _LPToken, uint256 _rewardPerBlock, uint256 _totalStaked){
        uint256 allocPoints;
    ...
    function getFarmUserInfo(uint256 pid, address user) public view returns (uint256 _amount, uint256 _pending){
        (_amount,) = masterChef(MasterChef).userInfo(pid, user);
        _pending = masterChef(MasterChef).pendingCake(pid, user);
    }

    // get many commands below - External uses

    function FarmUserInfo(uint256[] calldata pid, address user) external view returns (uint256[] memory _amount, uint256[] memory _pending){
    ...
    function FarmInfo(uint256[] calldata pid) external view returns (address[] memory _LPToken,uint256[] memory _rewardPerBlock, uint256[] memory _totalStaked,address[] memory token0, uint256[] memory t0Dec, string[] memory t0Sym, address[] memory token1, uint256[] memory t1Dec, string[] memory t1Sym){
      ...
      t1Sym = new string[](arrl); 
        ...
        for (uint i = 0; i < arrl; i++) {
            ...
            try lpToken(address(_LPToken[i])).token1() {
                (token0[i], t0Dec[i], t0Sym[i], token1[i], t1Dec[i], t1Sym[i]) = getFarmTokenInfo(pid[i]);
            } catch {
                token0[i] = address(0x0);
                t0Dec[i] = 0;
                t0Sym[i] = "no";

                token1[i] = address(0x0);
                t1Dec[i] = 0;
                t1Sym[i] = "no";
            }
        }
    }
    function oneFarmInfo(uint256 pid) external view returns (address _LPToken,uint256 _rewardPerBlock, uint256 _totalStaked,address token0, uint256 t0Dec, string memory t0Sym, address token1, uint256 t1Dec, string memory t1Sym){
      (_LPToken, _rewardPerBlock, _totalStaked) = getFarmInfo(pid);
      (token0, t0Dec, t0Sym, token1, t1Dec, t1Sym) = getFarmTokenInfo(pid);
    }
