...
contract BossWarSpirit {

	event RecvSuccess(address indexed from, string requestId);
    event withdrawApplySuccess(address indexed from, string requestId);
    ...
    function withdrawApply(string memory requestId) public returns (bool) {
		emit withdrawApplySuccess(msg.sender, requestId);
        return true;
	}
}