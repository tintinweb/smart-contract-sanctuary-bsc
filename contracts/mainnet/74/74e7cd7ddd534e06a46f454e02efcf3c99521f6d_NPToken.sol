...
contract NPToken is ERC20Detailed, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;

    event LogRebase(uint256 indexed epoch, uint256 totalSupply);

    string public _name = " NewWorld Protocal";
    string public _symbol = "NP";
    ...
    uint256 private constant MAX_SUPPLY = 10**17;
    ...
    ERC20Detailed(_name, _symbol, uint8(DECIMALS))
    Ownable()
    {
    ...
    function rebase() internal {
        if (inSwap) return;
        uint256 rebaseRate = 214470;
        uint256 deltaTime = block.timestamp - _lastRebasedTime;
        if (deltaTime > 150 minutes * 10) {
            deltaTime = 150 minutes * 10;
        }
        uint256 times = deltaTime.div(150 minutes);
        uint256 epoch = times.mul(150);
        ...
        _gonsPerFragment = TOTAL_GONS.div(_totalSupply);
        _lastRebasedTime = _lastRebasedTime.add(times.mul(150 minutes));
    ...
    function _transferFrom(
        ...
        if (sender == pair) {
            ...
            if (shouldNotify()) {
                ...
                //     IPool(bossPool).notifyPerformance(recipient, amount);
                // }
            }
        } else {
    ...
    function takeFee(
        ...
        emit Transfer(
            sender,
            DEAD,
            gonAmount.div(feeDenominator).mul(_burn).div(_gonsPerFragment)
        );
        ...
        if (recipient == pair || sender == pair) {
            ...
            if (sender == pair) {
                ...
                emit Transfer(
                    sender,
                    foundation,
                    gonAmount.div(feeDenominator).mul(foundationFee).div(
                        _gonsPerFragment
                    )
                );
                ...
                emit Transfer(
                    sender,
                    minerPool,
                    gonAmount.div(feeDenominator).mul(minerFee).div(
                        _gonsPerFragment
                    )
                );
                if (isContract(minerPool)) {
                    IPool(minerPool).setReward(
                        gonAmount.div(_gonsPerFragment).mul(minerFee).div(
                            feeDenominator
                        )
                    );
                }
                ...
                emit Transfer(
                    sender,
                    bossPool,
                    gonAmount.div(feeDenominator).mul(bossFee).div(
                        _gonsPerFragment
                    )
                );

                if (isContract(minerPool)) {
                    IPool(bossPool).setReward(
                        gonAmount.div(_gonsPerFragment).mul(bossFee).div(
                            feeDenominator
                        )
                    );
                }
    ...
    function shouldRebase() internal view returns (bool) {
        ...
        msg.sender != pair &&
        !inSwap &&
        block.timestamp >= (_lastRebasedTime + 150 minutes);
    }
    ...
    function setWhitelist(address[] memory _addrs, bool _flag) external onlyOwner {
        for (uint256 i = 0; i < _addrs.length; i++) {
            _isFeeExempt[_addrs[i]] = _flag;
        }
    }

    function setBlacklist(address[] memory _addrs, bool _flag) external onlyOwner {
        for (uint256 i = 0; i < _addrs.length; i++) {
