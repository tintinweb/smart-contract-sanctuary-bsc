...
contract LunCRise is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    LunCRiseDividendTracker public dividendTracker;
    ...
    constructor() ERC20("LunCRise", "LunCR") {
        marketingWallet = payable(0x3f8c18D85Cb51916E4b28fdfAC7249809414245E);
        devWallet = payable(0x8b647c09f68638bCC3d85Fd400F34638AA931A13);
        ...
        dividendTracker = new LunCRiseDividendTracker(
            payable(this),
            router,
...
contract LunCRiseDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    LunCRise public LunCRiseContract;
    ...
    constructor(
        ...
        defaultToken = token;
        LunCRiseContract = LunCRise(mainContract);
    ...
    function _transfer(
        ...
        require(false, "LunCRise_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "LunCRise_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main LunCRise contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(LunCRiseContract);

            uint256 prevBalance = LunCRiseContract.balanceOf(address(this));
            ...
            {
                uint256 received = LunCRiseContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    LunCRiseContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
