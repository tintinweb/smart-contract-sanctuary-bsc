...
contract SusumiCrowdfunding is Ownable, ReentrancyGuard, ISusumiCrowdfunding {
    ...
    uint256 public totalCampaigns; // Total number of campaigns in the contract
    uint256 public totalAmountRaised; // Total amount in USD donated in the campaign
    uint256 public topCampaign; // Campaign with the top amount raised
    uint256 public receiverFeeSUSU; // receiver fee while a user creates a custom campaign
    ...
    mapping(address => uint256) public userCurrentCampaign; // To check if user has already created a campaign (Only 1 campaign at a time)
    mapping(address => uint256) public userTotalFunds; // Total funds raised by the user in BUSD
    mapping(address => uint256) public userTotalDonations; // Total donation by the user in BUSD
    mapping(uint256 => bool) public isCampaignActive; // A check if the campaign is active or not
    mapping(uint256 => SusuLibrary.Campaign) public campaignInfo; // Mapping to the Campaign struct using index
    mapping(address => mapping(uint256 => SusuLibrary.User)) public userInfo; // Mapping to the User struct using user's address
    mapping(uint256 => SusuLibrary.SusuValueInfo) public susuValueMapping; // Mapping to store Susu Value Info
    ...
    constructor(
        ...
        defaultMaturityPeriod = 1 days; // 24 hours (5 minutes for testing)
        receiverAddress = _receiverAddress;
        receiverFeeSUSU = 100 * (10**susu.decimals()); // Fee in SUSU

        busd = Token(_BUSDAddress);
        uint256 _busdDecimals = uint256(10**(busd.decimals()));

        // For Testing
        setSusuValueInfo(
            1,
            10 * _busdDecimals,
            100 * _busdDecimals, // 100 for testing
            2,
            2,
            8300,
            30 minutes // 30 minutes for live
        ); // For level 1
        setSusuValueInfo(
            2,
            20 * _busdDecimals, // 20
            200 * _busdDecimals, // 200
            2,
            30,
            5200, // 5200
            30 minutes
    ...
    function setSusuValueInfo(
        uint8 _userLevel,
        uint256 _minSusuValue,
        uint256 _maxSusuValue,
        uint32 _minKeyValue,
        uint32 _maxKeyValue,
        uint256 _receiverFee,
    ...
    function campaignInternal(
        ...
        // Susu value staked
        uint256 _susuActivation = calculateSUSUActivation(_susuValue, _slot);
    ...
    function newCampaignInternal(
        ...
        SusuLibrary.Campaign memory newCampaign = SusuLibrary.Campaign({
            ...
            totalRewards: uint256(_susuValue * decimalValue * (10**8)) /
                uint256(price * (10**busd.decimals())),
    ...
    function createCampaignUsingClaim(
        ...
        price =
            price -
            uint32(
                uint256((uint256(price * _pricePercentage)) / ((10**8) * 100))
            );
    ...
    function calculateSUSUActivation(uint256 _susuValue, uint32 _slot)
        public
        view
        override
        returns (uint256)
    {
        return
            uint256(
                (_susuValue * uint256(decimalValue) * (10**8)) /
                    (uint256(_slot) * uint256(price) * (10**busd.decimals()))
            );
    }

    // Percentage of price changed - Turn public for test cases
    function calculatePriceShare(uint256 _amount) public view returns (uint64) {
        return
            uint64(
                uint256((_amount * 10**8 * 100) / susu.balanceOf(address(this)))
            );
    }

    // Sets level of the user, if new and checks susuValue according to level
    function setLevelAndCheckValue(uint256 _susuValue, uint8 _level) internal {
    ...
    function donateToCampaign(uint256 _campaignIndex, string memory _userID)
        external
        override
        nonReentrant
        validateCampaign(_campaignIndex)
    {
        ...
        SusuLibrary.User memory uInfo = SusuLibrary.User({
            ...
            tokenAmount: uint256(
                (
                    uint256(
                        (cInfo.slotValue + _remainingSlotValue) *
                            decimalValue *
                            (10**8)
                    )
                ) / (uint256(cInfo.tokenPrice * 10**busd.decimals()))
            ),
        ...
        if (cInfo.susuValueRaised >= cInfo.susuValue) {
            uint64 _pricePercentage = calculatePriceShare(cInfo.totalRewards); // in 10**8
            price =
                price +
                (
                    uint32(
    ...
    function withdrawRaisedAmount(uint256 _campaignIndex)
        external
        override
        nonReentrant
        validateCampaign(_campaignIndex)
    {
        ...
        uint256 receiverShare = uint256(
            uint256(
                susuValueMapping[campaignInfo[_campaignIndex].levelValue]
                    .receiverFee * cInfo.susuValueRaised
            ) / 10**5
        );
    ...
    function claimRewards(uint256 _campaignIndex)
        external
        override
        validateCampaign(_campaignIndex)
    {
        ...
        require(
            susu.balanceOf(address(this)) >= uInfo.tokenAmount,
            "Contract doesn't have enough tokens for the rewards"
        );
    ...
    function claimDonationAfterMaturity(uint256 _campaignIndex)
        external
        override
    ...
    function claimStakeAfterMaturity(uint256 _campaignIndex)
        external
        override
        isCampaignMaturityOver(_campaignIndex)
    {
    ...
    function restartMaturedCampaign(uint256 _campaignIndex)
        external
        override
        isCampaignMaturityOver(_campaignIndex)
    {
        ...
        require(
            ...
            "Creator needs to wait 1 hour before the fund can be restarted"
        );
    ...
    function updateReceiverFeeUSDT(uint256 _receiverFeeSUSU)
        external
        onlyOwner
    {
    ...
    modifier isCampaignMaturityOver(uint256 _campaignIndex) {
        require(
    ...
    function checkSusuAmount(uint256 _susuActivation, address _userAddress)
        internal
        view
    {
...
library SusuLibrary {
    // Campaign Struct
    struct Campaign {
        uint256 susuValue; // in USDT
        uint256 susuValueRaised; // in USDT
        uint256 slotValue; // in USDT
        uint256 totalRewards; // total rewards that will get distribute among slots
        uint256 susuActivation; // in SUSU
    ...
    struct User {
        uint256 amountDonated; // In USDT
        uint256 tokenAmount; // SUSU tokem amount that will be given to user
...
interface ISusumiCrowdfunding {
    ...
    event CampaignCreated(
        ...
        uint256 susuValue,
        uint256 susuValueRaised,
        uint256 slotValue,
        uint256 totalRewards,
        uint256 susuActivation,
        uint64 tokenPrice,
        uint32 slots,
    ...
    event CampaignComplete(uint256 indexed campaignIndex, bool isActive);

    // Emitted when a custom campaign is requested
    event CustomCampaignRequest(
        uint256 indexed customCampaignIndex,
        uint256 susuValue,
        uint256 susuActivation,
        uint64 receiverFee,
        uint32 slots,
    ...
    event CampaignWithdrawal(uint256 indexed campaignIndex, bool isClaimed);
    ...
    // Returns Susu tokens on particular Susu Value depending on the slots
    function calculateSUSUActivation(uint256 susuValue, uint32 slot)
        external
        view
        returns (uint256);

    // Donation to Campaign
    function donateToCampaign(uint256 campaignIndex, string memory userID)
        external;

    // Only campaign creator can withdraw the funds from contract
    function withdrawRaisedAmount(uint256 campaignIndex) external;

    // users who donated to a particular campaign can claim their rewards
    function claimRewards(uint256 campaignIndex) external;

    // Donors claim back their donation
    function claimDonationAfterMaturity(uint256 campaignIndex) external;

    // Creator can claim their stake after maturity
    function claimStakeAfterMaturity(uint256 campaignIndex) external;

    // Creators can restart their campaign
    function restartMaturedCampaign(uint256 campaignIndex) external;
}