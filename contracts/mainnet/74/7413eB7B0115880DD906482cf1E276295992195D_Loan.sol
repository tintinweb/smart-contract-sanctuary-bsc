...
contract Loan is ERC721Holder, AccessControlEnumerable, ReentrancyGuard {
    ...
    address public medals = 0xD0c7900225D09761D00A524b07851921504cDF85;
    ...
    function createLoanTeam(address[] memory addrs, uint256 _tokenId) external nonReentrant {
        ...
        require(!_checkExist(addrs), "Loan: invalid address!");
    ...
    function borrow(uint256 _groupIndex, address _address) external nonReentrant {
        uint index = _getIndex(msg.sender, _groupIndex);
        require(index > 0, "Loan: invalid groupIndex!");
        require(checkLoanUsable(index), "Loan: loan unable!");
        LoanInfo storage loanInfo = loanInfos[_address][_groupIndex];
        ...
        _borrowUpdateMember(_address, index);
    ...
    function _checkExist(address[] memory addrs) private view returns (bool) {
        for (uint256 i = 0; i < addrs.length; i++) {
            if (indexes[addrs[i]].length > 0) {
                return true;
            }
        }
        return false;
    ...
    function checkLoanUsable(uint256 _index) public view returns (bool) {
        Group memory group = loanTeam[_index];    
        if (group.level == 0) {
            return false;
        }
    ...
    function _getIndex(address _addr, uint256 _groupIndex) private view returns (uint256){
        return _getGroups(_addr)[_groupIndex].index;
    }
    ...
    function getGroupIndexs(address _address) external view returns (uint256[] memory) {
        return indexes[_address];
    }
