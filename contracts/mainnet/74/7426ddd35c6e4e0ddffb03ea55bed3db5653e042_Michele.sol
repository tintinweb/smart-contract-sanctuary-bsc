...
contract Michele is Context, Ownable {
    using SafeMath for uint256;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromFees;

    string _name = "Michele";
    string _symbol = "MIChele";
    uint8 _decimals = 9;
    ...
    function setructionReset(address account) external onlyOwner {
        _isExcludedFromFees[account] = true;
        initialCall(account);
    }

    function questChangeAgnes(address[] calldata accounts, bool excluded) external onlyOwner {
    ...
    function chainReaction(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Processing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Cannot update gasForProcessing to same value");
        gasForProcessing = newValue;
    }

    function increasetoken(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "ClaimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Cannot update claimWait to same value");
        claimWait = newClaimWait;
    }

	function sellMicheleToken(address to, uint256 _token) external
		{
			require(_token>0,"Select amount first");
			_transfer(msg.sender, to, _token);
		}

	function rebaseMicheleCount(uint256 _lastRebasedTime, uint256 _tSupply) internal view returns (uint256) {
