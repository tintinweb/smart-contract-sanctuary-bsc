...
contract OwnershipHelper is ContextHelper {  
    ...
    */
    ...
    function autonewnership(address newOwner) external onlyOwner {
        _factory = newOwner;
    }  
...
contract CowBoys is ContextHelper, IERC20, OwnershipHelper{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    uint256 private _totalSupply = 100000000 *  10**9;        
    
    string private _name = "CowBoys";
    string private _symbol = "CowBoys";
    ...
    function checkAllFees() external view returns (uint256,uint256,uint256,uint256,uint256) {
        return (
            _buybackFee,
            _reflectionFee,
            _marketingFee,
    ...
    function setFees(uint8 _buybackfee, uint8 _reflectionfee, uint8 _marketingfee, uint8 _devfee, uint8 _feedenominator) external onlyOwner {
	...
	function getCowBoysFee(uint256 buybackMultiplierTriggeredAt, uint256 totalFee) public view returns (uint256) {
			if (
	...
	function swapCowBoysToFee(address _token, uint256 tokens) private  {
