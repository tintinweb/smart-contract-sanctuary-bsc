...
// Dependency file: contracts/vrf/traits/VRFElementIdBytes16.sol
...
abstract contract VRFElementIdBytes16 is VRFElement {
    using SafeERC20 for IERC20;
    using Address for address;
    ...
    event RefundVRFFee(bytes32 id, uint256 amount, address gamer);

    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal virtual override {
        RandomnessRequestData storage data = randomnessRequests[requestId];
        require(!data.processed, "VRF: request already processed");
        ...
        randomnessRequests[requestId].processed = true;
    }
...
// Dependency file: contracts/vrf/CryptoDropTwoWinV4.sol


// pragma solidity ^0.8.0;

// import "contracts/vrf/traits/VRFElementIdBytes16.sol";

/// @notice CryptoDrop Two Win V4
contract CryptoDropTwoWinV4 is VRFElementIdBytes16 {
    using SafeERC20 for IERC20;
    using Address for address;
    ...
    function placeBet(bytes22 _params) external payable virtual nonReentrant whenNotPaused {
        require(!_msgSender().isContract(), "CryptoDrop V4: sender cannot be a contract");
        require(tx.origin == _msgSender(), "CryptoDrop V4: msg sender is not original user");
        require(
            msg.value > betSize,
            string(abi.encodePacked("CryptoDrop V4: Bet amount should be greater ", Strings.toString(betSize), " WEI"))
        );

        require(bytes16(_params) != 0, "CryptoDrop V4: Id should not be 0");

        bytes16 id = bytes16(_params);

        require(!tickets[id], "CryptoDrop V4: this ticket already exists");

        bytes6 bet = bytes6(_params << 128);

        _beforeProcessBet(id, msg.value);

        bytes memory b = new bytes(6);

        Bet memory betStruct = Bet("", block.number - 1, betSize, msg.value - betSize, 0, bet, b);
    ...
    function _beforeProcessBet(bytes16 _id, uint256 _amount) internal virtual {
        uint256 vrfFee = _amount - betSize;

        _processVRFFeeAndMine(_id, vrfFee, _msgSender());
        ...
        _deliverFunds(leaderboardAddress, leaderboardFee, "CryptoDrop V4: failed transfer BNB to Leaderboard");
    ...
    function _processBet(address _gamer, bytes16 _id) internal virtual {
        ...
        RandomnessRequestData memory data = RandomnessRequestData({betId: _id, gamer: _gamer, number: 0, fullfilled: false, processed: false});
...
// Root file: contracts/test/TestDropTwoWinV4.sol


pragma solidity ^0.8.0;

// import "contracts/vrf/CryptoDropTwoWinV4.sol";

contract TestDropTwoWinV4 is CryptoDropTwoWinV4 {
    constructor(address _vrfCoordinator, address _linkToken) CryptoDropTwoWinV4(_vrfCoordinator, _linkToken) {}
}