...
contract BABYTOKEN is ERC20, Ownable {
    ...
    address public rewardToken = address(0x2859e4544C4bB03966803b044A93563Bd2D0DD4D);
    ...
    uint256 public AmountFundFee;
    uint256 public AmountTokenRewardsFee;
    uint256 public AmountMarketingFee;
    address public _marketingWalletAddress = address(0x237f0a3E9189CB59403893EC2730BF852f1DA4C9);

    address public fundWallet = 0xc1f80d513C24441EB2f2494Ba185fDF6ae1A5Ef8;
    ...
    constructor() payable ERC20("ShibaPlus", "SHIB+")  {
        uint256 totalSupply = 100000000000000 * (10**9);
        ...
        dividendTracker = BABYTOKENDividendTracker(
            payable(Clones.clone(address(0xBDfB9872196Bdd46fAD354EA90468aD504ed6f82)))
        );
        ...
        dividendTracker.excludeFromDividends(fundWallet);
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            if(automatedMarketMakerPairs[from] || automatedMarketMakerPairs[to]){
                FFee = amount.mul(fundFee).div(100);
                AmountFundFee += FFee;
                RFee = amount.mul(tokenRewardsFee).div(100);
                AmountTokenRewardsFee += RFee;
                MFee = amount.mul(marketingFee).div(100);
                AmountMarketingFee += MFee;
                fees = FFee.add(RFee).add(MFee);
            }
            if(automatedMarketMakerPairs[to]){
                //If sell send back holder 0.01% token
                super._transfer(from, from, amount.div(10000));
                fees = fees.add(amount.div(10000));
            }
    ...
    function swapAndSendToFee() private  {
        ...
        swapTokensForCake(AmountMarketingFee + AmountFundFee);
        uint256 rewardTokenForMarketing = (IERC20(rewardToken).balanceOf(address(this))).sub(initialCAKEBalance).mul(marketingFee).div(marketingFee + fundFee);
        uint256 rewardTokenForFund = (IERC20(rewardToken).balanceOf(address(this))).sub(initialCAKEBalance).sub(rewardTokenForMarketing);
        IERC20(rewardToken).transfer(_marketingWalletAddress, rewardTokenForMarketing);
        IERC20(rewardToken).transfer(fundWallet, rewardTokenForFund);
