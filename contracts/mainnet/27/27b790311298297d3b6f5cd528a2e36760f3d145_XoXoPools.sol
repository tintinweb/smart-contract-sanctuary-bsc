...
contract XoXoPools is Ownable {
    ...
    constructor(address _paymentToken, address _initAddress) {
        ...
        p.supportAmounts = [50 * 10**18]; //50000000000000000000
        p.supportTransfer = [true];
        p.supportTarget = [XoXo]; // 0x0000000000000000000000000000000000000000
                                  // 0xbC8Fc04F00bB369A641f8324c141475F5AE56D72
        p.slotActive = 1;
        p.slotCount = 1;
        p.earnOverall = 0;
        p.rebuyBalance = 0;        
    ...
    function buyGlobalSlot(uint _pool) public {
        ...
        if (ps.prelaunch == 0) require(ps.status == true, "Pool disabled");
        ...
        if (poolID > 1) {
            uint prevPool = poolID - 1;
            Pools storage pp = Pool[prevPool];
            require(pp.inPool[_buyer] == true, "Not registered in previous pool");
        }

        if(ps.prelaunch == 1) {
            require((p.whitelisted[_buyer] == true), "Not whitelisted");
        }
        if(ps.prelaunch == 2) {
            uint _refCount = 0;
            if(poolID == 1) {
                (,,_refCount,,) = XoXoContract.getUserData(_buyer);
                (,,,,,uint[] memory _slotsGlobalpool) = XoXoContract.getUserPoolData(_buyer, 2);
                require((p.whitelisted[_buyer] == true || _refCount >= 10 || _slotsGlobalpool.length >= 10), "You are not met prelaunch conditions");                
            } else {
                (,,_refCount,,) = XoXoContract.getUserData(_buyer);
                uint prevPool = poolID - 1;
                (,,,,,uint[] memory _prevPoolSlots) = getUserPoolData(_buyer, prevPool);
                require((p.whitelisted[_buyer] == true || _prevPoolSlots.length >= ps.needPrevSlots || _refCount >= 10), "Not whitelisted or not match entrance condition");
            }
        }
        ...
        if(ps.prelaunch == 1) {
            require ((p.slotLimit[_buyer] <= 2), "Prelaunch slot limit exceed");
        } else if(ps.prelaunch == 2) {
            require ((p.slotLimit[_buyer] <= 1), "Prelaunch slot limit exceed");
        } else {
        ...
        if (reenter) {            
            ...
            if (p.payToken.balanceOf(address(this)) >= joinCost) {
                if(p.rebuyBalance >= joinCost) rebuyGlobalSlot(poolID);
            }
    ...
    function payRefBonus(uint _pool, address _user) private {
        ...
        emit payAutopoolEvent(_user, _to, _transferamount, _pool, true, XoXoContract.addressToId(_user), newSlot);
    }
    ...
    function payGlobalBonus(uint _pool, address _user) private returns(bool reentrance) { 
        ...
        p.payToken.safeTransfer(_to, _transferamount);
        uint newSlot = p.slotCount + 1;
        emit payAutopoolEvent(_user, _to, _transferamount, _pool, false, XoXoContract.addressToId(_user), newSlot);
    ...
    function setPoolParams(uint _pool, bool _status, uint _prelaunch, bool _countLoseAmount, uint _needSlots, bool _payRef, uint _payRefAmount, uint _paySlotAmount, uint _cycleAt)  public onlyOwner  {
        Settings storage ps = PoolSettings[_pool];
        ps.status = _status;
