...
contract ReachDAO is ERC20, Ownable {
    ...
    uint256 public immutable treasuryFeeOnBuy;
    uint256 public immutable treasuryFeeOnSell;

    uint256 public immutable walletToWalletTransferFee;
    ...
    event TreasuryWalletChanged(address treasuryallet);
    event SwapAndSendTreasury(uint256 tokensSwapped, uint256 bnbSend);
    event SwapTokensAtAmountUpdated(uint256 swapTokensAtAmount);

    constructor () ERC20("Reach DAO", "$READ") 
    {   
        ...
        treasuryWallet = 0xd55eA2C9ef34472B4f1d4e009F67BBe575f90B95;
...
//------------------Transfer------------------//
    function _transfer(address from,address to,uint256 amount) internal  override {
        ...
        if (canSwap &&
            ...
            swapAndSendTreasury(contractTokenBalance);
        ...
        if(takeFee) {
        	uint256 bnbFee   = amount / 100;
            uint256 tokenFee = amount / 100;

            amount -= bnbFee + tokenFee;

            super._transfer(from, address(this),  bnbFee);
            super._transfer(from, treasuryWallet, tokenFee);
        }
