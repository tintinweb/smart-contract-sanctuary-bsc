...
contract CEEKLandGate is ERC165, ReentrancyGuard, Context, IERC721TokenReceiver {
    ...
    function beforeBuyCheck01(uint256 tokenId, bytes memory ceekUserIdParam, EIP20 payment_token) private returns(bool) {
        bool result = false;
        address prevGate1 = address(0xa65d7763Cf8b7b2dF802Ad6Eead35b8bfDBcD02f);
        if (sellBidPrice[tokenId] == 0) {
            bytes memory resultGetPrice1 = Address.functionStaticCall(prevGate1, abi.encodeWithSignature("getPrice(uint256,string,uint256,uint256,uint256)", tokenId, "", 0, 0, 0));
            (uint256 tokenPrice1,,) = abi.decode(resultGetPrice1, (uint256,uint256,Currency));
            if (tokenPrice1 > 0) {
                Address.functionCallWithValue(prevGate1, abi.encodeWithSignature("buy(uint256,bytes,EIP20)", tokenId, ceekUserIdParam, payment_token), msg.value);
                callOptionalReturn(contract_ceekland, abi.encodeWithSelector(contract_ceekland.transferFrom.selector, address(this), msg.sender, tokenId));
                result = true;
            }
        }
    ...
    function beforeBuyCheck02(uint256 tokenId, uint256 price, bytes memory ceekUserIdParam, EIP20 payment_token) private returns(bool) {
        bool result = false;
        address prevGate2 = address(0x1269298d6acC72Dc83b865eFf03b495FB6EeDfD6);
        if (sellBidPrice[tokenId] == 0) {
            bytes memory resultGetPrice2 = Address.functionStaticCall(prevGate2, abi.encodeWithSignature("getPrice(uint256,string,uint256,uint256,uint256)", tokenId, "", 0, 0, 0));
            (uint256 tokenPrice2,,) = abi.decode(resultGetPrice2, (uint256,uint256,Currency));
            if (tokenPrice2 > 0) {
                Address.functionCallWithValue(prevGate2, abi.encodeWithSignature("buy(uint256,uint256,bytes,EIP20)", tokenId, price, ceekUserIdParam, payment_token), msg.value);
                callOptionalReturn(contract_ceekland, abi.encodeWithSelector(contract_ceekland.transferFrom.selector, address(this), msg.sender, tokenId));
                result = true;
            }
        }
    ...
    function buy(uint256 tokenId, uint256 price, bytes memory ceekUserIdParam, EIP20 payment_token) public payable nonReentrant {
        ...
        bool resBeforeBuyCheck0 = beforeBuyCheck01(tokenId, ceekUserIdParam, payment_token);

        if (!resBeforeBuyCheck0) {
            resBeforeBuyCheck0 = beforeBuyCheck02(tokenId, price, ceekUserIdParam, payment_token);
        } 
