...
contract FWIN is ERC20, Ownable {
    ...
    uint256 private constant TREASURY_PERCENTAGE = 6667;
    uint256 private constant MARKETING_WALLET_PERCENTAGE = 3333;
    ...
    uint256 public feePercentage = 600;
    uint256 public accumulatedFeeAmount;
    ...
    function setLiquidityProvider(address liquidityProvider_) external onlyOwner {
        require(
            ...
            "FWIN: invalid liquidity provider address"
        );
        isExcludedFromFees[liquidityProvider_] = true;
    ...
    function nullifyFeePercentage() external onlyOwner {
        feePercentage = 0;
    }
    ...
    function enableTrading() external onlyOwner {
        require(
            !tradingEnabled,
            "FWIN: trading is already enabled"
        );
        tradingEnabled = true;
        ...
        override
    {
        require(
            from_ != address(0), 
            "FWIN: transfer from the zero address"
        );
        require(
            to_ != address(0), 
            "FWIN: transfer to the zero address"
        );
        require(
            amount_ > 0, 
            "FWIN: transfer amount can not be 0"
        );
        ...
        require(
            ...
            "FWIN: transfer from or to blacklisted address"
        );
        bool buy;
        bool sell;
        bool transfer;
        ...
        uint256 amountToReceive = takeFee ? _takeFees(from_, amount_, sell, transfer) : amount_;
    ...
    function _takeFees(address from_, uint256 amount_, bool sell_, bool transfer_) private returns (uint256) {
        uint256 feeAmount;
        if (sell_) {
            feeAmount = amount_ * feePercentage / BASE_PERCENTAGE;
            if (feeAmount > 0) {
                accumulatedFeeAmount += feeAmount;
                _balances[address(this)] += feeAmount;
                emit Transfer(from_, address(this), feeAmount);
                if (accumulatedFeeAmount >= thresholdAmount) {
                    _swap();
                }
            }
        } else if (transfer_) {
    ...
    function _swap() private {
        if (accumulatedFeeAmount != 0) {
            ...
            payable(treasury).sendValue(balance * TREASURY_PERCENTAGE / BASE_PERCENTAGE);
            payable(marketingWallet).sendValue(balance * MARKETING_WALLET_PERCENTAGE / BASE_PERCENTAGE);
            accumulatedFeeAmount = 0;
        }
    }
}
