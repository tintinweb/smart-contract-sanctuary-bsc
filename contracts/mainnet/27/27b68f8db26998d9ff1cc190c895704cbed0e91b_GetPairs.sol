...
contract GetPairs is Ownable {
	...
	function getPairsMint(uint256 start, uint256 end)public returns(pairMintInfo[] memory){
		...
		pairMintInfo[] memory pairs = new pairMintInfo[](length+1);
		...
		for(uint256 i = 0; i <= length; i++){
            ...
            pairs[i].isMint0 = checkGetMint(pairs[i].token0);
			pairs[i].isMint1 = checkGetMint(pairs[i].token1);
	...
	function checkGetMint(address token)public returns(uint256){
