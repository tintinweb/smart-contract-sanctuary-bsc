    ...
    IMagpieBridge
{
    ...
    function deposit(
        ...
        if (bridgeType == BridgeType.Wormhole) {
            ...
            return
                IWormhole(bridgeConfig.tokenBridgeAddress).transferTokens(
                    toAssetAddress,
                    amount,
    ...
    function bridgeIn(
        ...
        uint256 amount,
        address toAssetAddress
    ) external override returns (uint64 coreSequence, uint64 tokenSequence) {
    ...
    function bridgeOut(
        ...
        if (payload.bridgeType == BridgeType.Wormhole) {
            ...
            // fromAssetAddress.transfer(recipient, amount);
        } else {
            if (msg.sender == bridgeConfig.relayerAddress) {
                require(
...
interface IMagpieBridge {
    ...
    function bridgeIn(
        ...
        uint256 amount,
        address toAssetAddress
    ) external returns (uint64 coreSequence, uint64 tokenSequence);
