...
// |  \/  |      / __ \            |__   __|  | |             
// | \  / |_   _| |  | |_      ___ __ | | ___ | | _____ _ __  
// | |\/| | | | | |  | \ \ /\ / / '_ \| |/ _ \| |/ / _ \ '_ \ 
// | |  | | |_| | |__| |\ V  V /| | | | | (_) |   <  __/ | | |
// |_|  |_|\__, |\____/  \_/\_/ |_| |_|_|\___/|_|\_\___|_| |_|
//          __/ |                                             
//         |___/             
//
// www.MyOwnVerse.io
...
// File: contracts/MOTDividendPayingToken.sol
pragma solidity ^0.7.6;
...
// File: contracts MOT.sol
pragma solidity ^0.7.6;
contract MOT is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    MOTDividendTracker public dividendTracker;
    ...
    // maxwallet holding @ 1% of supply
    // maxbuy & maxsell @ 1% of supply
    // swaptokensatamount needs to be adjusted over time
    ...
    // timestamp for when the token can be traded freely on dex
    ...
    // addresses that can make transfers before tradingisenabled, if any
    ...
    constructor() public ERC20("MyOwnToken", "MOT") {
        // LP address
        LPaddress = address(0x000000000000000000000000000000000000dEaD);
        uint256 _processDividendTime = block.timestamp;
        processDividendTime = _processDividendTime;

        dividendTracker = new MOTDividendTracker();
        ...
        // enable owner and fixed-sale wallet to send tokens before tradingisenabled
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "MOT: The dividend tracker already has that address");

        MOTDividendTracker newDividendTracker = MOTDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "MOT: The new dividend tracker must be owned by the MOT token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "MOT: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        //require(_isExcludedFromFees[account] != excluded, "MOT: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "MOT: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MOT: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MOT: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MOT: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        // only whitelisted addresses can make transfers before tradingisenabled
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "MOT: This account cannot send tokens until trading is enabled");
        }
...
contract MOTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("MOT_Dividend_Tracker", "MOT_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "MOT_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "MOT_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MOT contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "MOT_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "MOT_Dividend_Tracker: Cannot update claimWait to same value");
