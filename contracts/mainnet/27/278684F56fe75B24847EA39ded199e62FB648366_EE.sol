...
abstract contract AbsToken is IERC20, Ownable {
    ...
    constructor (
        ...
        holderCondition = 100 * 10 ** Decimals;
    }
    ...
    function _transfer(
        ...
        if (_limitAmount > 0 && startTradeBlock > 0 && !_swapPairList[to] && !_feeWhiteList[to]) {
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            if (_swapPairList[sender]) {//Buy
                ...
                uint256 fundAmount = tAmount * (_buyFundFee + _buyFundFee2 + _buyHoldDividendFee) / 10000;
                if (fundAmount > 0) {
                ...
                uint256 fundAmount = tAmount * (_sellFundFee + _sellFundFee2 + _sellHoldDividendFee) / 10000;
                if (fundAmount > 0) {
                    feeAmount += fundAmount;
                    _takeTransfer(sender, address(this), fundAmount);
                }
    ...
    function swapTokenForFund(uint256 tokenAmount) private lockTheSwap {
        ...
        uint256 totalFee = fundFee + fundFee2 + holdDividendFee;
    ...
    function setBuyFee(uint256 buyDestroyFee, uint256 buyFundFee, uint256 buyFundFee2, uint256 buyHoldDividendFee) external onlyOwner {
    ...
    function setSellFee(uint256 sellDestroyFee, uint256 sellFundFee, uint256 sellFundFee2, uint256 sellHoldDividendFee) external onlyOwner {
...
contract EE is AbsToken {
    ...
    //总量
        100000,
    //营销钱包
        address(0x9FEDA923F62519953f02F30A83b1C45032B3ADfd),
    //营销钱包2
        address(0x9FEDA923F62519953f02F30A83b1C45032B3ADfd),
    //代币接收地址
        address(0x67779aB3D58338ee41786A68F656D44349F742Db),
    //限购数量，0表示不限购
        0,
    //销毁到多少停止销毁
        0
