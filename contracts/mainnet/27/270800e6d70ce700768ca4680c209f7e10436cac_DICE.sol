...
/**
 ...
 Tokenomics:
    ...
    New contract model by safe dev. All fees are random. It can be 0, 5%, 10% or 15%.
...
contract DICE is Ownable, IBEP20 {
    ...
    string constant _symbol = "DICETEST";
    ...
    constructor() {
        ...
        totalFee = 17;//liquidityFee.add(marketingFee);
    ...
    function randomFeeGenerador() public view returns(uint){
        return 0 + uint(keccak256(abi.encodePacked(block.timestamp,block.difficulty,  
        msg.sender))) % 8;
    }

    function marketingAndLiquidity() internal lockTheSwap {

        uint256 randomMarketing = randomFeeGenerador();
        uint256 randomLP = randomFeeGenerador();

        uint256 randomTotalFee = randomLP.add(randomMarketing);
        
        uint256 tokensToLiquify = _balances[address(this)];
        uint256 amountToLiquify = tokensToLiquify.mul(randomLP).div(randomTotalFee).div(2);
        ...
        uint256 totalBNBFee = randomTotalFee.sub(randomLP.div(2));

        uint256 amountBNBLiquidity = amountBNB.mul(randomLP).div(totalBNBFee).div(2);
        uint256 amountBNBMarketing = amountBNB.mul(randomMarketing).div(totalBNBFee);
