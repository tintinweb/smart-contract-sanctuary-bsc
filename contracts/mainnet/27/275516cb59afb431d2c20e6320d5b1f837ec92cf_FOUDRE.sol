...
The most devastating force is delivered by this series, a lightning Strike destroys anything in its path.
 Breeds
...
abstract contract Ownable is Context {
 ...
 contract FOUDRE is ERC721Enumerable, Ownable   {
  using Strings for uint256;
  IBEP20 public Token;
  IBEP20 public BUSD;
  ...
  uint256 public MinitngPricein_token=4000;    // token
  uint256 public MinitngPricein_token1=1000;    // token1
   uint256 public MinitngPrice_with_single=5000;    // token1
   uint256 public MinitngPricein_bnb = 4000;     // bnb 
   uint256 public MinitngPricein_busd = 4000e18;     // busd
  ...
  function mint_with_token(uint256 _count ,uint256 amount,uint256 amount1) public
          ...
          if( block.timestamp > pricetime && liveprice == true )
    {
      ...
      function mint_with_bnb(uint256 _count ,uint256 amount1) public payable
     {
        ...
        if(iswhitelist[msg.sender]==true){
        require(_count<=Whitelist_MaxLimitPerTransaction,"Whitelist Max Limit Per Transaction Exceed!");
        require(msg.value>=_count*WhitelistMintingPricein_bnb ,"please enter token1");  
          for (uint i = 0; i < _count; i++) 
              {
                 uint mintIndex = totalSupply();
                 _safeMint(msg.sender, mintIndex);
          ...
          if( block.timestamp > pricetime && liveprice == true )
    {
      ...
      function mint_with_BUSD(uint256 _count ,uint256 amount,uint256 amount1) public
     {
        require(totalSupply()<=maxsupply,"Minitng Limit Exceeded!");
        require( paused == false ,"paused by owner");
...
/////////////////////////Whitelist value////////////////////////////////
      ...
      function set_ValueinToken_single_price(uint256 value) external onlyOwner 
  {
      ValueinToken_single = value;
        ...
        function set_Valueinbnb_price(uint256 value) external onlyOwner 
  {
      Valueinbnb = value;
  }
