...
contract Onigami is Context, IERC20 { 
    ...
    function EnableTrading() external onlyOwner {
        ...
        for (uint256 i = 0; i < _isExcluded.length; i++) {
            _isExcludedFromMaxWallet[_isExcluded[i]] = true;
        }
    }

    function RemoveLimitTxWallet() public onlyOwner{
        _maxTxAmount = _tTotal * 100 / 100;
        _maxWalletToken = _tTotal * 100 / 100;
    }
           
    function ExcludedFromMaxWallets(address[] memory addrs)
        external
        onlyOwner
        returns (bool)
    {
        for (uint256 i = 0; i < addrs.length; i++) {
            _isExcludedFromMaxWallet[addrs[i]] = false;
        }
        return true;
    }
    
    function setWalletPercent(uint256 newNum) public onlyOwner{
        _maxWalletPercent = newNum;
    }
