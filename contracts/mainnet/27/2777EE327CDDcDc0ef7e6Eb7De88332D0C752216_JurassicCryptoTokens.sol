...
contract JurassicCryptoTokens is Initializable,
    ...
    ) internal whenNotPaused override(ERC1155Upgradeable, ERC1155SupplyUpgradeable, ERC1155EnumerableUpgradeable)
    {
        ...
        for (uint256 i; i < ids.length; i++){
            require(_tokenTraits[ids[i]].in_sauria == false);
        }
    }
...
contract ERC1155JrscGameUpgradeable is ERC1155Upgradeable, ERC1155JrscCollectableUpgradeable {
    ...
    function getSauriaTimelock(uint256 tokenId) external view returns (uint256) {
        return SauriaTimeLockMapping[tokenId];
    }
    ...
    function hunt(uint256[] calldata sauriaIds, uint8 enemyId) external {
        ...
        // charge the Tax
        require(JRSC_TOKEN.transferFrom(msg.sender, JRSC_OWNER_ACC, sauriaIds.length * 1 ether));
        ...
        if (lotteryResult <= uint8(COMBAT_LUCK[enemyId])){
            ...
            REWARDS_MANAGER.RecordReward(msg.sender, uint256(uint8(COMBAT_REWARD[enemyId])) * 1 ether );
        } else {
            // LOST
...
contract JurassicConstants {
    ...
    uint256 constant internal SAURIA_CREATION_TAX = 5 ether;
}
