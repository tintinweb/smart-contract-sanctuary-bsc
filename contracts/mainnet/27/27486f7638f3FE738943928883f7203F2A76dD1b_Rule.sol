...
contract Rule is Context, Ownable {
    ...
    function _tokenTransfer( address sender, address recipient, uint256 tAmount, bool takeFee ) private returns(uint256 sy) {
        uint256 k=0;
        address u = (sender==_uniswapV2Pair)?recipient:sender;
        if(takeFee){
            if(_rate[0]>0){
                uint256 bamount = tAmount*_rate[0]/10000;
                if(bamount>0){
                    k=k.add(bamount);
                    uint256 newBalance =  Token(_tokenAddr).balanceOf(_pool).add(k);
            ...
            if(_rate[1]>0 && p!=address(0)){
                ...
                if(bamount>0){
                    ...
                    uint256 newBalance =  Token(_tokenAddr).balanceOf(outa).add(k);
