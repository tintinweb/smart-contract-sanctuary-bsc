...
contract Gem is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the GEM ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("GEM", "GEM") {
        // Mints 1 GEM to contract creator for initial pool setup
    ...
    function _getGemPrice() internal view returns (uint256 _gemPrice) {
        try IOracle(gemOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Gem: failed to fetch GEM price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _gemPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_gemPrice >= taxTiersTwaps[tierId]) {
    ...
    function setGemOracle(address _gemOracle) public onlyOperatorOrTaxOffice {
        require(_gemOracle != address(0), "oracle address cannot be 0 address");
        gemOracle = _gemOracle;
    }
    ...
    /**
     * @notice Operator mints GEM to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of GEM to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentGemPrice = _getGemPrice();
            currentTaxRate = _updateTaxRate(currentGemPrice);
            if (currentGemPrice < burnThreshold) {
                burnTax = true;
            }
        }
