...
contract DexUpdate is Ownable, DexStorage, SecondDexStorage {
 ...
 function migrateNFT(uint256[] calldata tokenIds, address nftContract, address newCollection) external {
        require(admin == msg.sender, "not authorize");
        uint256 tokenId;
        address owner;
        for(uint256 i =0; i< tokenIds.length; i++){
            tokenId = tokenIds[i];
            if(INFT(nftContract).ownerOf(tokenId) == address(this)){
                if(_saleTokens[tokenId][nftContract].seller != address(0x0)){
            ...
            owner = INFT(nftContract).ownerOf(tokenId);
            BNFT(nftContract).burnAdmin(tokenId);
            (uint256 from,) = BNFT(newCollection).mint(owner, 1);
            if(owner == nonCryptoNFTVault){
                _nonCryptoOwners[from][newCollection] = _nonCryptoOwners[tokenId][nftContract];
                delete _nonCryptoOwners[tokenId][nftContract];
            }
        }
    }
