...
contract GDJTool is Ownable{
    event MultiTransfer(uint256 total,uint8 t);
    ...
    constructor(){
        Token = IERC20(0xfFA841840352C12540a9aBC14A601C75B5bc5cBE);
        Usdt = IERC20(0x55d398326f99059fF775485246999027B3197955);
        Pair = 0x24b1671D354A75e348a65bcEcab385d96cB9a0d3;
        Master = payable(0x3cadEd32394A136e67c90cfBbEdCc8f4eaa52fc7);
    }
    ...
    function multiTransfer(uint8 t,address[] memory addresses, uint256[] memory counts) public returns (bool){
        uint256 total;
        for(uint i = 0; i < addresses.length; i++) {
            if(t == 1){
                require(Usdt.transferFrom(msg.sender, addresses[i], counts[i]));
            }else {
                require(Token.transferFrom(msg.sender, addresses[i], counts[i]));
            }
            total += counts[i];
        }
        emit MultiTransfer(total,t);
        return true;
    }
