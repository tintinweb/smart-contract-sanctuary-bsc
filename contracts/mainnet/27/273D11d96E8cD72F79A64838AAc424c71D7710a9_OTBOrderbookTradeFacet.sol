...
contract OTBOrderbookTradeFacet {
    ...
    function getPair(address _tokenA, address _tokenB) internal returns (bytes32 _pairName) {
        if (hasPair(_tokenA, _tokenB)) {
            bytes32 pairName = bytes32(abi.encodePacked(IOTBSwapFactory(factory).getPair(_tokenA, _tokenB)));
        }
    }
    ...
    function performExactSwap(
        ...
        if (_path[0] == IOTBSwapRouter02(router).WETH()) {
            IOTBSwapRouter02(router).swapExactETHForTokens(_tokenOutAmount, _path, address(this), type(uint256).max);
