...
contract SPINCOIN is Context, ERC20, Ownable {
    ...
    uint256 private constant _tSupply = 1e15 * (10**18);
    ...
    address payable private _treasuryWallet = payable(0x674E77E5c06dE53990f6E244eA34acDF4Ce37fa4);
    address payable private _opsWallet = payable(0x5d254A6127262E0e2D908118835dE21d7237167F);
    ...
    event SetBuyFee(uint256 buyTreauryFee, uint256 buyOpsFee);
	event SetSellFee(uint256 sellTreasuryFee, uint256 sellOpsFee);
    
    constructor () ERC20("SPINCOIN", "SPIN") {
    ...
    function _transfer(address from, address to, uint256 amount) override internal {
        ...
        if (from != owner() && to != owner() && to != ZERO && to != DEAD && !_swapping) {
            ...
            if (from == _pancakeSwapV2Pair && to != address(_pancakeSwapV2Router) && !_isExcludedMaxTransactionAmount[to]) {
            ...
            if (to == _pancakeSwapV2Pair && from != address(_pancakeSwapV2Router) && !_isExcludedMaxTransactionAmount[from]) {
    ...
    function _swapBack(uint256 contractBalance) internal {
        ...
        uint256 bnbForTreasury = bnbBalance.mul(_tokensForTreasury).div(contractBalance);
        
        _tokensForTreasury = 0;
        _tokensForOps = 0;

        (success,) = address(_treasuryWallet).call{value: bnbForTreasury}("");
    ...
    function openTrading(uint256 blocks) external onlyOwner {
        require(!tradingOpen, "Trading is already open");
        require(blocks <= 10, "Invalid blocks count.");
        swapEnabled = true;
        cooldownEnabled = true;
        maxSellAmount = _tSupply.mul(25).div(10000);
        _swapTokensAtAmount = _tSupply.mul(25).div(100000);
        tradingOpen = true;
    ...
    function setTreasuryWallet(address _treasuryWalletAddy) external onlyOwner {
    ...
    function setOpsWallet(address _opsWalletAddy) external onlyOwner {
    ...
    function setBlacklisted(address[] memory accounts, bool exempt) external onlyOwner {
        for (uint i = 0; i < accounts.length; i++) {
            if((accounts[i] != _pancakeSwapV2Pair) && (accounts[i] != address(this)) && (accounts[i] != address(_pancakeSwapV2Router))) _isBlacklisted[accounts[i]] = exempt;
        }
    }

    function setBuyFee(uint256 newBuyTreasuryFee, uint256 newBuyOpsFee) external onlyOwner {
        require(newBuyTreasuryFee.add(newBuyOpsFee) <= 125, "Must keep buy taxes below 12.5%");
        buyTreasuryFee = newBuyTreasuryFee;
        buyOpsFee = newBuyOpsFee;
        emit SetBuyFee(buyTreasuryFee, buyOpsFee);
    }

    function setSellFee(uint256 newSellTreasuryFee, uint256 newSellOpsFee) external onlyOwner {
        require(newSellTreasuryFee.add(newSellOpsFee) <= 125, "Must keep sell taxes below 12.5%");
        sellTreasuryFee = newSellTreasuryFee;
        sellOpsFee = newSellOpsFee;
        emit SetSellFee(sellTreasuryFee, sellOpsFee);
    }
    ...
    function _setBot() internal {
        _treasuryFee = 399;
        _opsFee = 399;
        _totalFees = _treasuryFee.add(_opsFee);
    }
