...
contract VIPNFTPriceDistributor is Ownable, IFeeDistributor {
    ...
    address public vipNFT;// year NFT
    address public svipNFT;
    ...
    modifier onlyVIPNFT() {
        require(msg.sender == vipNFT || msg.sender == svipNFT, "invalid caller");
        _;
    }

    constructor(address _pancakeRouter, address _mos, address _asdic,
        address _usdt, address _lpReceiver) {
        mos = IERC20(_mos);
        asdic = IERC20(_asdic);
        usdt = IERC20(_usdt);
    ...
    function setVIPNft(address _vipNFT) public onlyOwner {
        vipNFT = _vipNFT;
    }
    function setSVIPNft(address _svipNFT) public onlyOwner {
        svipNFT = _svipNFT;
    }
    ...
    function distributeFee(uint fee) public onlyVIPNFT {
        //TODO 买成coin2进入矿池
        addMosNFTPool(fee);
        addMosLPPool(fee);
        addNoriLPStakePool(fee);
        //TODO 30个coin2——15个coin2买成nori+15coin2添加LP，把LP锁死，打入指定地址
        addAsdicBottomPool(fee);
        addVipPermanentPool(fee);
        addVipYearPool(fee);
    }

    function addMosNFTPool(uint fee) internal {
        uint usdtAmount = mosNFTPoolRatio * fee / PRECISION;
        uint coin2Amount = buyMos(usdtAmount);
        mos.safeTransfer(mosNFTPool, coin2Amount);
    }

    function addMosLPPool(uint fee) internal {
        uint amount = mosLPPoolRatio * fee / PRECISION;
        uint mosAmount = buyMos(amount);
        mos.safeTransfer(mosNFTPool, mosAmount);
    }

    function addNoriLPStakePool(uint fee) internal {
        uint amount = asdicLPStakePoolRatio * fee / PRECISION;
        uint noriAmount = buyNori(amount);
        asdic.safeTransfer(asdicLPStakePool, noriAmount);
    }

    function addVipPermanentPool(uint fee) internal {
        uint amount = vipPermanentPoolRatio * fee / PRECISION;
        uint noriAmount = buyNori(amount);
        asdic.safeTransfer(vipPermanentPool, noriAmount);
    }

    function addVipYearPool(uint fee) internal {
        uint amount = vipYearPoolRatio * fee / PRECISION;
        uint noriAmount = buyNori(amount);
        asdic.safeTransfer(vipYearPool, noriAmount);
    }


    function buyMos(uint amount) internal returns (uint){
        address[] memory path = new address[](2);
        path[0] = address(usdt);
        path[1] = address(mos);
        uint coin2BalBefore = mos.balanceOf(address(this));
        usdt.approve(address(pancakeRouter), amount);
        ...
        return coin2BalAfter - coin2BalBefore;
    }

    function buyNori(uint amount) internal returns (uint){
        address[] memory path = new address[](3);
        path[0] = address(usdt);
        path[1] = address(mos);
        path[2] = address(asdic);
        uint noriBalBefore = asdic.balanceOf(address(this));
        usdt.approve(address(pancakeRouter), amount);
        pancakeRouter.swapExactTokensForTokens(amount, 0, path, address(this), block.timestamp);
        uint noriBalAfter = asdic.balanceOf(address(this));
        return noriBalAfter - noriBalBefore;
    }

    function addAsdicBottomPool(uint fee) internal {
        uint noriBottomPoolAmount = asdicBottomPoolRatio * fee / PRECISION;
        uint noriAmount = buyNori(noriBottomPoolAmount / 2);
        uint mosAmount = buyMos(noriBottomPoolAmount - noriBottomPoolAmount / 2);
        asdic.approve(address(pancakeRouter), noriAmount);
        mos.approve(address(pancakeRouter), mosAmount);
        pancakeRouter.addLiquidity(address(asdic), address(mos),
            noriAmount, mosAmount, 0, 0, lpReceiver, block.timestamp);
    }
}