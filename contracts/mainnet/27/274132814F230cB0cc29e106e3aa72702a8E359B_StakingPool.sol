...
contract StakingPool is Ownable ,ReentrancyGuard{
    ...
    struct PoolInfo {
        ...
        uint8 lpTokenType;  //0 indicate normal token, 1 is ptoken
        address  lpTokenFrom;  //if lpTokenType is 1, set the deposit token
    ...
    event PoolAdded(address _lpToken,address _rewardToken, uint256 _timeStart, uint256 _timeEnd , uint256 _bonusPerSecond,uint256 _lockedSeconds,
        uint8 _lpTokenType, address _lpTokenFrom , bool _open, uint256 _depositMin);
    event PoolUpdate(uint256 _pid, uint256 _timeStart, uint256 _timeEnd , uint256 _bonusPerSecond,uint256 _lockedSeconds,
        uint8 _lpTokenType, address _lpTokenFrom , bool _open, uint256 _depositMin);
    event PoolStatusChanged(uint256 _pid, bool _open);
    ...
    function add( address _lpToken,address _rewardToken, uint256 _timeStart, uint256 _timeEnd , uint256 _bonusPerSecond,uint256 _lockedSeconds,
        uint8 _lpTokenType, address _lpTokenFrom , bool _open, uint256 _depositMin) public onlyGovernor {
        require(address(_lpToken) != address(0), "_lpToken is the zero address");
        require(address(_rewardToken) != address(0), "rewardToken is the zero address");
        require( _timeStart > block.timestamp && _timeEnd > _timeStart, "invalid time limit");

        if( _lpTokenType == 1 ){
            require(address(_lpTokenFrom) != address(0), "_lpTokenFrom is the zero address.");
        }
        ...
        emit PoolAdded(_lpToken, _rewardToken, _timeStart, _timeEnd, _bonusPerSecond, _lockedSeconds, _lpTokenType, _lpTokenFrom, _open, _depositMin);
    ...
    function setPoolInfo( uint256 _pid, uint256 _timeStart, uint256 _timeEnd , uint256 _bonusPerSecond,uint256 _lockedSeconds,
        ...
        require( _timeEnd > block.timestamp && _timeEnd > _timeStart, "invalid time limit");
        ...
        emit PoolUpdate(_pid, _timeStart, _timeEnd, _bonusPerSecond, _lockedSeconds, _lpTokenType, _lpTokenFrom, _open, _depositMin);
    }
    ...
    function setPoolStatus( uint256 _pid,  bool _open) public onlyGovernor {
        ...
        emit PoolStatusChanged(_pid, _open);
    }
    ...
    function pendingReward(uint256[] memory _pids, address _user) external view returns (uint256){
        uint256 rewards = 0;
        for(uint256 i = 0; i < _pids.length; i ++ ){
            PoolInfo memory pool = poolInfo[ _pids[i] ];
            if( pool.open == true ){
                rewards = rewards.add( pending( _pids[i], _user ));
            }
        }
    ...
    function claimReward(uint256[] memory _pids) public nonReentrant{
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        require( _amount == 0 || (_amount >= pool.depositMin ) , "deposit amount is invalid.");
    ...
    function withdrawUnlock(uint256 _pid) public {
        ...
        if (unlockAmount > 0  ) {
            ...
            IERC20(pool.lpToken).safeTransfer(address(msg.sender), unlockAmount);
        }
        user.rewardDebt = user.amount.mul(pool.accBonusPerShare).div(1e12);
