...
contract DEXODexOrder is Initializable, OwnableUpgradeable {
    ...
    function initialize (address weth) public initializer{
        WETH = weth;
        NATIVE = address(0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE);
        STAKE_FEE = 2;
        EXECUTOR_FEE = 500000000000000;
        ordersNum = 0;
        stakeAddress = address(0x885Be84DA467aC606bC2F9Ff782a8C0B3D9b0670);
        __Ownable_init();
    }
    ...
    function executeOrder(
        ...
        if(order.orderType == OrderType.TokensForEth) {
            ...
            boughtAmount = IERC20Upgradeable(buyToken).balanceOf(address(this));
        }

        require(IERC20Upgradeable(sellToken).approve(spender, type(uint256).max));
        ...
        if(order.orderType == OrderType.TokensForEth) {
            ...
            boughtAmount = IERC20Upgradeable(buyToken).balanceOf(address(this))-boughtAmount;
