...
contract TOKEN is Context, IERC20, Ownable {
    ...
    uint256 public numTokensSellToAddToLMD;
    ...
    constructor() {
        _name = "Galaxy Quest Token"; 
        _symbol = "GQT";
        _decimals = 18; 
        _tTotal = 100000000 * 10**_decimals;  
        _rTotal = (MAX - (MAX % _tTotal));
        numTokensSellToAddToLMD = 50000 * 10**_decimals;  
        _marketingWalletAddress = 0x8f8fDE0C3cD3820db47c89254df142a1A72EF7da; 
        _devWalletAddress = 0xF5B93bFD659B977fF619990A1514F687B2Bec7C1;
    ...
    function setnumTokensSellToAddToLMD(uint256 amount)
        external
        onlyOwner
    {
        numTokensSellToAddToLMD = amount * 10**_decimals;
    }
        ...
        )
    {
        ...
        uint256 tLMD = calculateLiquidityMarketingAndDevFee(tAmount);
        uint256 tBurn = calculateBurnFee(tAmount);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLMD).sub(
            tBurn
        );
        return (tTransferAmount, tFee, tLMD, tBurn);
    }
        ...
        )
    {
        ...
        uint256 rLMD = tLMD.mul(currentRate);
        uint256 rDev = tBurn.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLMD).sub(
            rDev
        );
    ...
    function _takeLMD(uint256 tLMD)
        private
    {
        uint256 currentRate = _getRate();
        uint256 rLMD = tLMD.mul(
            currentRate
        );
    ...
    function calculateLiquidityMarketingAndDevFee(uint256 _amount)
        private
        view
        returns (uint256)
    {
        return _amount.mul((_liquidityFee.add(_marketingFee)).add(_devFee)).div(10**2);
    }
    ...
    function _transfer(
        ...
        if (
            ...
            contractTokenBalance = numTokensSellToAddToLMD;
        ...
        function swapBack(uint256 contractBalance) private lockTheSwap {
                ...
                uint256 devTokens = contractBalance.mul(_devFee).div(1000);
                ...
                uint256 ethForFloor = ethBalance * devTokens / (totalTokensToSwap - (tokensForLiquidity/2));
                ...
                (success,) = address(_devWalletAddress).call{value: ethForFloor}("");
