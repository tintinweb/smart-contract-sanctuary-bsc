...
contract StiMiningCheck is OwnableUpgradeable, DivestorUpgradeable {
    ...
    function initialize() public initializer {
        ...
        mining = IMining(0xF4e968D404487F81340EdfCCd28B3d8F20d09ad7);
        poolInfo.push(
            PoolInfo({
                maxOpt: 6000000000000000,
                iptToken: IBEP20(0x928C72A8B58A08be10166B19673c336CF88Bd5B9),
                optToken: IBEP20(0x0736FBF90Ac5892135908B30161994646678ef7d),
                rate: 1157407407,
                lastTime: 1656769343,
                debted: 1360160276096962,
                endDebted: 0,
                tvl: 89876194500000000000,
                times: [
                    uint(1656136800),
                    1661320800,
                    1661320800,
    ...
    function viewNewReward(
        uint uDebt_,
        uint uStake_,
        uint toCliam_
    ) public view returns (uint reward) {
        uint debt = coutingDebt(0);
        reward = ((debt - uDebt_) * uStake_) / (ACC * 1e10);
        reward += toCliam_;
    }
    ...
    function viewReward(
        ...
        uint uStake_,
        uint toCliam_
    ) public view returns (uint oldreward, uint newReward) {
        (, , , , uint[7] memory uInfos) = mining.viewInfo(0, account_);

        oldreward = uInfos[1];
        newReward = viewNewReward(uDebt_, uStake_, toCliam_);
    }
}
