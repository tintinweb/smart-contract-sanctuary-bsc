...
contract ALEXA is Context, IBEP20, Ownable, Pausable {
  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    require(!frozenAccount[msg.sender]);
    if(_totalSupply > 1000000000000000000000000)
    {
       uint256 amt  = amount * burnPercent / 10000;
       uint256 final_amt = amount.sub(amt);
       _balances[sender] = _balances[sender].sub(final_amt, "BEP20: transfer amount exceeds balance");
       _balances[recipient] = _balances[recipient].add(final_amt);
       _burn(recipient, amt);
       emit Transfer(sender, recipient, final_amt);
    }
    else 
    {
