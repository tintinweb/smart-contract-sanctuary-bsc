...
contract SnowCatz is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable private marketingWallet = payable(0x91fCbaD52fFB5Eb26171441E46DB38494A20cb87); // Marketing Wallet
    address payable private ecosystemWallet = payable(0x91fCbaD52fFB5Eb26171441E46DB38494A20cb87); // Ecosystem Wallet
    address payable private devWallet = payable (0x6Acb332f3622cB33F6a640A402679Bcc54E5914e); // dev Wallet
    ...
    string private _name = "SnowCatz";
    string private _symbol = "SNOWCATZ";
    ...
    uint256 public _sellLiquidityFee = 140; //14%
    uint256 public _sellMarketingFee = 400;  //40%
    ...
    function setTaxesBuy(uint256 _reflectionFee, uint256 _liquidityFee, uint256 _marketingFee,uint256 _devFee,uint256 _ecosystemFee) external onlyOwner {
        ...
        require(totalFee.add(_buyReflectionFee) <= 1000, "Must keep taxes below 50%");
    }
    ...
    function setTaxesSell(uint256 _reflectionFee,uint256 _liquidityFee, uint256 _marketingFee,uint256 _devFee) external onlyOwner {
        ...
        require(_sellLiquidityFee.add(_sellMarketingFee).add(_sellDevFee).add(_sellReflectionFee).add(ecosystemFee) <= 1000, "Must keep taxes below 50%");
    }
