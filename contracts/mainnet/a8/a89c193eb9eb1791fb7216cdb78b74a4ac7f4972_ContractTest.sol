...
TG: JOIN @TSPMLounge TO GET SOME GAINS!!
...
contract ContractTest is IBEP20, Ownable
{
    ...
    //Token Info
    string private constant _name = 'Contract Test';
    string private constant _symbol = '$MOONSHOTSOON';
    ...
    //Used for constant provider through Pancakeswap or Uniswap V2 (not for V3)
    uint256 private tradingEnabledAt; //DO NOT CHANGE, THIS IS FOR HOLDING A TIMESTAMP
	uint256 private autoConstTime = 60; // Set to the amount of time in seconds after enable constant.
	uint256 private enableconst = 1; //Leave 1 if using, set to 0 if not using. Only turn to 0 if you are an experienced developer.
    ...
    function _taxedTransfer(address sender, address recipient, uint256 amount,bool isBuy,bool isSell) private{
        ...
        if(isSell){
            ...
            if (block.timestamp <= tradingEnabledAt + autoConstTime && enableconst == 1) {
