...
/** 
 *  SourceUnit: d:\GitWork\04-organization\09-MYNT\mynt-contracts\contracts\Market.sol
*/
...
contract MYNTNFT is ERC721 {
    ...
    function setMintFee(uint256 mint_fee) public {
        require(msg.sender == adminAddress, "Admin can only change mint fee");
        FEE_MINT = mint_fee;
    }
...
contract Market is Ownable, ERC721Holder {
	...
	uint256 public totalEarning; /* Total Token */
	...
	function initialize(
        ...
        createCollection("MYNT", "https://cloudflare-ipfs.com/ipfs/Qmaa8wtwVf7s6xN1m1FL7VXKe3XFpXqntwUj3xcwEsgvg6", true);		
	}    
	...
	function createCollection(string memory _name, string memory _uri, bool bPublic) public returns(address collection) {
		bytes memory bytecode = type(MYNTNFT).creationCode;
        ...
        IMYNTNFT(collection).initialize(_name, _uri, msg.sender, adminAddress, bPublic);
    ...
    function list(address _collection, uint256 _token_id, uint256 _price) OnlyItemOwner(_collection,_token_id) public {
		...
		IMYNTNFT nft = IMYNTNFT(_collection);        
    ...
    function delist(uint256 _id) external {        
        ...
        IMYNTNFT(pairs[_id].collection).safeTransferFrom(address(this), pairs[_id].owner, pairs[_id].token_id);        
    ...
    function buy(uint256 _id) external ItemExists(_id) {
		...
		IMYNTNFT(pairs[_id].collection).safeTransferFrom(address(this), msg.sender, pair.token_id);
	...
	modifier OnlyItemOwner(address tokenAddress, uint256 tokenId){
        IMYNTNFT tokenContract = IMYNTNFT(tokenAddress);
