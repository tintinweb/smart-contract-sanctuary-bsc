    ...
    ReentrancyGuardUpgradeable
{
    ...
    bytes32 public constant LAUNCHPAD_TOKEN_MINTER =
        keccak256("LAUNCHPAD_TOKEN_MINTER");
    ...
    function initialize(string memory baseURI) public initializer {
        __ERC721_init("SNAIL DAO", "NFT SNAIL DAO");
    ...
    function setRBTable(uint256[7] calldata rbTable)
        external
    ...
    function setLevelTable(uint256[7] calldata levelTable)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function setLevelUpPercent(uint8 percent)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function setBurnRBPeriod(uint256 newPeriod)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function addBlackListNFT(address[] calldata addresses)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function delBlackListNFT(address[] calldata addresses)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        for (uint256 i = 0; i < addresses.length; i++) {
    ...
    function accrueRB(address user, uint256 amount)
        external
    ...
    function remainRBToNextLevel(uint256[] calldata tokenId)
        public
        view
        ...
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < dayCount; i++) {
            balance[i] = _robiBoost[user][
                (block.timestamp - i * 1 days) / 86400
            ];
        }
        return balance;
    }

    function getRbTotalAmount(uint256 period)
        public
        view
        returns (uint256 amount)
    {
        for (uint256 i = 0; i <= period; i++) {
            amount += _robiBoostTotalAmounts[
                (block.timestamp - i * 1 days) / 86400
            ];
        }
        return amount;
    ...
    function increaseRBToken(
        uint256[] calldata tokenId,
        uint256[] calldata amount
    ) public nonReentrant onlyRole(RB_SETTER_ROLE) {
        _sendRBToToken(tokenId, amount);
    }
    ...
    function sendRBToMaxInTokenLevel(uint256[] calldata tokenId)
        public
        nonReentrant
    {
        require(
    ...
    function _safeMint(address to, uint256 tokenId) internal override {
        ...
        emit TokenMint(
            to,
            tokenId,
            _tokens[tokenId].level,
            _tokens[tokenId].robiBoost
        );
    }
    ...
    function _getRbBalance(address user)
        internal
        view
    ...
    function _remainRBToMaxLevel(uint256 tokenId)
        internal
        view
        returns (uint256)
    {
        return
            _rbTable[uint256(_tokens[tokenId].level)] -
            _tokens[tokenId].robiBoost;
    }
