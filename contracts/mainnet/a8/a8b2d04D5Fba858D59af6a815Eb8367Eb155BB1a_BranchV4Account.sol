...
contract BranchV4Account is AccessControlEnumerable {
    // 管理员配置
    ...
    constructor() {
        ...
        _grantRole(BRANCH_ROLE, _msgSender());
    }

    // 用户/经纪人模块数据
    uint8[] public levelUpFollowCount = [0, 2, 2, 2, 2, 2]; // 代理人升级条件
    uint256 public upToM1DepositTotalMin = 1000e18; // 普通用户(M0)升级到M1代理人最低累计存储金
    ...
    function accountAdd(address addr, address parent) external onlyRole(BRANCH_ROLE) {
        require(accounts[addr].id == 0, "account already active.");
        if (parent != address(0)) {
        ...
        accounts[parent].follows.push(addr);
    }
    ...
    ) external onlyRole(BRANCH_ROLE) {
        require(target > accounts[addr].level, "Already higher level.");
        if (target == 1) {
            if (depositTotal >= upToM1DepositTotalMin) {
                accounts[addr].level = target;
                return;
            }
        } else {
            uint256 count = 0;
            address[] storage follows = accounts[addr].follows;
    ...
    function accountFollow(address addr) external view onlyRole(BRANCH_ROLE) returns (address[] memory) {
        return accounts[addr].follows;
    }

    function accountParent(address addr) external view onlyRole(BRANCH_ROLE) returns (address) {
        return accounts[addr].parent;
    }

    function accountLevel(address addr) external view onlyRole(BRANCH_ROLE) returns (uint8) {
