...
contract AggregatorExecutor is Ownable {
    ...
    function callBytes(address _receiver, bytes calldata _data) external payable {
        ...
        if (_info.selectorFunction == this.swapExactInput.selector) {
            ...
            _swapExactOutput(_steps, _info.amountIn, _info.amountOut, _info.fromToken, _info.toToken, payable(_receiver));
        } else {
...
//    }
    ...
    function _swapExactOutput(SwapStep[] memory _steps, uint256 _amountInMax, uint256 _exactAmountOut, address _fromToken, address _toToken, address payable _receiver) internal {
        uint256 _amountIn = _getAmountIns(_steps, _exactAmountOut);
        require(_amountIn <= _amountInMax, "AggregatorExecutor: amount in");
        (, bool _isToETH) = _payment(_fromToken, _toToken, _amountIn);
    ...
    function _getAmountIns(SwapStep[] memory _steps, uint256 _amountOut) internal view returns (uint256 amountIn) {
        ...
        for (uint256 i = _steps.length; i > 0;) {
            ...
            if (_step.protocol == BAKERY_SWAP || _step.protocol == UNI_SWAP) {
                ...
                revert("AggregatorExecutor: Not support this dex in get exact amount out");
            }
        }
    }
