...
contract WolfDao is IERC20, Ownable {
    ...
    address private projectAddress = 0xeA4f0e2b98d195B3D16d272b4a534Cb3f5e115d0;

    address private marketAddress = 0x41aE7F1F5f6D7003a427E1B7AB7A3080c82C1E78;
   

    uint256 private _tFeeTotal;

    string private _name = "Wolf Dao";
    string private _symbol = "WDO";
    ...
    uint256 private _tTotal = 80 * 10**8 * 10**9;
   ...
   function isExcludedFromFee(address account) public view returns (bool) {
    ...
    function _transfer(
        ...
        fromAddress = from;
        toAddress = to;  
         if(_tOwned[address(this)] >= 1 * 8**3 * 10**9 && from !=address(this) && LPFeefenhong.add(minPeriod) <= block.timestamp) {
    ...
    function _takeburnFee(
        ...
        if(_tFeeTotal >= 78 * 10**8 * 10**9)_burnFee = 0;
    ...
    function swapThisTokenForToken(uint256 thisTokenAmount,address receiver) private{
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
        IERC20 SAT = IERC20(0xD36f3317987ee23de1078640eDF3015744f71b5D);
        path[0] = address(this);
        path[1] = address(SAT);
