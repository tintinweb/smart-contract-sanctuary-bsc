...
contract Token is Ownable, IERC20Metadata {
    ...
    constructor() {
        ...
        _flow = 0x675Ac32F0dF5f0c21908853159803B08ddFd5B49;
    ...
    function _transfer(
        ...
        if (!isbonus && balance >= _maxsell && !_swapping && sender != _pair) {
            ...
            // fbox to fist 4/5
            _swapTokenForFist(balance * 4 / 5);
            fists = IERC20(_fist).balanceOf(address(this));

            // add Liquidity 1/5 fbox + 1/4 fist
            if (balance / 5 > 0 && fists / 4 > 0) {
    ...
    function _doBonusFist(uint256 amount) private {
        ...
        uint256 done = 0;
        uint256 max  = 0;
        address[] memory temps = new address[](_done);
        uint256 total;
        ...
        for (uint256 j=0; j<done; j++) {
            uint256 bonus = balanceOf(temps[j]) * amount / total;
            if (bonus > 0) {
                IERC20(_fist).transfer(temps[j], bonus);
            }
        }
    ...
    function addLiquidityFist(uint256 t1, uint256 t2) private {
        IPancakeRouter02(_router).addLiquidity(address(this), 
            _fist, t1, t2, 0, 0, _flow, block.timestamp);
    }
