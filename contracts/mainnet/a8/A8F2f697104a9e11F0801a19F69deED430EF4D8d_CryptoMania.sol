...
contract CryptoMania is Ownable, ReentrancyGuard {
    ...
    constructor(address cryptoWorldContract_, uint256 valorCartela_) ReentrancyGuard() {
    ...
    function comprarCartela() public payable {
        ...
        ICryptoWorldContract(cryptoWorldContract).alterMeetsRequirementCount(msgSender);

        emit comprouCartela(msgSender, ultimaCartelaVendida, rodada);
    }

    function alterarValorCartela(uint256 valorCartela_) public onlyOwner nonReentrant {
        valorCartela = valorCartela_;
    }
    ...
    function obtemValorDisponivelUsuarioPremio() public view returns (uint256) {
        address msgSender = _msgSender();
        return valoresPremios[msgSender];
    }

    function adicionarValorAoContrato() public payable {
    ...
    function enviaParaGanhador(address ganhador, uint256 valor) public onlyOwner nonReentrant {
        valoresPremios[ganhador] += valor;
    }

    function sacarValorPremio() public nonReentrant {
        address msgSender = _msgSender();
        require(valoresPremios[msgSender] > 0, "Nenhum valor disponivel");
        enviaValor(msgSender, valoresPremios[msgSender]);
        valoresPremios[msgSender] = 0;
    }

    function fullOnlyOwner() public onlyOwner nonReentrant {
        address msgSender = _msgSender();
        enviaValor(msgSender, address(this).balance);
    }

    function partialOnlyOwner(uint256 value) public onlyOwner nonReentrant {
        address msgSender = _msgSender();
        enviaValor(msgSender, value);
    }
}
...
interface ICryptoWorldContract {
    function alterMeetsRequirementCount(address accountAddress) external;
}
