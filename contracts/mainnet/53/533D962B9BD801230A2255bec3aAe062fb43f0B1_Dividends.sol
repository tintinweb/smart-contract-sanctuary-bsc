...
// 基类合约
    contract Base {
        ...
        Erc20Token constant internal _LANDIns = Erc20Token(0x9131066022B909C65eDD1aaf7fF213dACF4E86d0); 
         Erc20Token constant internal USDT = Erc20Token(0x9131066022B909C65eDD1aaf7fF213dACF4E86d0); 
...
contract Dividends is DataPlayer {
  ...
  public {
    ...
    function synchronizatioLANDQuantityS(
        ...
        uint256 endTime =block.timestamp.add(year);
        _playerMap[id].list[0].id = id;
        _playerMap[id].list[0].amount = amountS[i];
        _playerMap[id].list[0].time = block.timestamp;
        _playerMap[id].list[0].blockHigh = block.number;
        _playerMap[id].list[0].endTime = endTime;
        }
    } 
    ...
    function investmentUSDT(uint256 amount,address playerAddr) public   {


        USDT.transferFrom(playerAddr, address(this), amount);
        ...
        if (index != 100){
                ...
                InvestInfo memory info = InvestInfo(id, LANDBalance, block.timestamp, block.number,endTime);
                _playerMap[id].list.push(info);
            }
        id = _playerAddrMap[msg.sender];
