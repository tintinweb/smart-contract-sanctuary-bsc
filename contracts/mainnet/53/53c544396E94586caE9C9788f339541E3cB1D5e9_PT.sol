...
contract PT is Ownable {
    ...
    address private PTMediumAddress;
    ...
    constructor(string memory _name, string memory _symbol) {
        ...
        transferMaxRatio = 90;
        serviceCharge = 2;
        ptFoundationAddress = 0x85fbF9be648Df35D2b2b9c5f086f3e888007723C;
        communityAddress = 0xF74e2b84b411DF6b4964a0A9e95E76eF620238F5;
        ptMax = 3600000 * 10 ** decimals;

        systemList[ptFoundationAddress] = true;
        systemList[communityAddress] = true;

        name = _name;
        symbol = _symbol;
        _mint(0x5178c325D46AD1d9Ec26245e1c1169f4a3e00D0E, totalDestruction);
    }

    function setPTMediumAddress(
        address _address
    ) public onlyOwner returns (bool) {
        PTMediumAddress = _address;
        ...
        if (totalDestruction - endDestruction > destroyNum) {
            if (isPairAddress[sender]) {
                ...
                if (!systemList[sender]) {
                    ...
                    emit Transfer(sender, ptFoundationAddress, foundation_amount);
                }
            }
        }
    ...
    function _takeFee(
        ...
        if (_types == 1) {
            ...
            _transfer(address(this), PTMediumAddress, buy_pt_num);
            uint256 usdt_profit = PTMedium(PTMediumAddress).exchangeUsdt(
                buy_pt_num
            );
