...
contract Banluna is Context, IBEP20, Ownable {
    ...
    string private _name = "BanLUNA";
    string private _symbol = "BLUNA";
    ...
    constructor () {
        ...
        v2pair = IUniswapV2Factory(_V2router.factory())
        ...
        _isExcludedFromAutoLiquidity[v2pair]            = true;
        _isExcludedFromAutoLiquidity[address(V2router)] = true;

        _isExcludedFromAntiWhale[owner()]                  = true;
        _isExcludedFromAntiWhale[address(this)]            = true;
        _isExcludedFromAntiWhale[v2pair]            = true;
        _isExcludedFromAntiWhale[address(V2router)] = true;
    ...
    function setLiquidityPercentages(uint256 teamFee, uint256 varMarketingFee) external onlyOwner {
        _percentageOfLiquidityForTeam        = teamFee;
        _percentageOfLiquidityForMarketing  = varMarketingFee;
    }
    ...
    function setUniswapRouter(address r) external onlyOwner {
        IUniswapV2Router02 _V2router = IUniswapV2Router02(r);
        V2router = _V2router;
    }
    ...
    function _transfer(
        ...
        if (from == v2pair && to != address(V2router)) {
    ...
    function addLiquidity(uint256 tokenAmount, uint256 bnbAmount) private returns (uint256, uint256) {
        ...
        (uint amountToken, uint amountETH, ) = V2router.addLiquidityETH{value: bnbAmount}(
            address(this),
            tokenAmount,
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        bool isBuy  = sender == v2pair && recipient != address(V2router);
        bool isSell = recipient == v2pair;
        ...
        if (!takeFee) {
            ...
            if (block_factor < 2000 || block_factor == 100) {
