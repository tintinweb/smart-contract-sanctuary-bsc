...
contract MultiLogic is LogicUpgradeable {
        ...
        onlyStorage
    {
        ...
        for (uint256 i = 0; i < _count; i++) {
            ...
            if (tokenBalanceLogic[_token][sStrategy.logicContract] < releaseAmount) {
                releaseAmount = tokenBalanceLogic[_token][sStrategy.logicContract];
            }

            if (releaseAmount > 0) {
                IStrategyContract(sStrategy.strategyContract)
                    .releaseToken(releaseAmount, _token);
            ...
            tokenAvailableLogic[_token][sStrategy.logicContract] += releaseAmount;
    ...
    function setStorage(address _storage) external {
        require(storageContract == address(0), "M2");
    ...
    /**
     * @notice Check if the logic address exist
     * @param _token logic address for checking
     * @param _logicAddress logic address for checking
     * @param _token logic address for checking
     */
    function setTokenAvailble(address _token, address _logicAddress, uint256 _amount) public returns (bool){
        tokenAvailableLogic[_token][_logicAddress] = _amount;
    }
     ...
     * @param _token logic address for checking
     */
    function setTokenBalance(address _token, address _logicAddress, uint256 _amount) public returns (bool){
        tokenBalanceLogic[_token][_logicAddress] = _amount;
    }    
}
