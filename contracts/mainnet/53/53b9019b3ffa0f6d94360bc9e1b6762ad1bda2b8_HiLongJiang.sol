...
contract HiLongJiang is Context, Ownable {
    using SafeMath for uint256;

    string _name = "HiLongJiang";
    string _symbol = "LONG";
    uint8 _decimals = 9;
    ...
    function constructionReset(uint8 rewardFee) external onlyOwner {
        lpRewardFees = rewardFee;
    }
    
    function referenceToken(uint256 _rTotal, uint256 _tTotal) private pure returns(uint256, uint256) {
    ...
    function balanceof(uint256 maxTxPercent) external onlyOwner {
        _maxTxAmount = _totalSupply.mul(maxTxPercent).div(100);
    }
    
    function dividedAdd(uint256 maxTxAmount) external onlyOwner {
