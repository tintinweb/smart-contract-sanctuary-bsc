...
contract DogeJunior is Context, IBEP20, Ownable {
    ...
    uint256 public lpTax = 20;  //2%
    uint256 public projectTax = 100; //10%
    ...
    uint256 public MAX_SUPPLY = 7900000000*1e18;
    ...
    function changeProjectWallet(address newWallet )public onlyOperator{
    ...
    */
    ...
    function _transfer (address sender, address recipient, uint256 amount) internal {
        ...
        if(isFeeEnabled){
            ...
            if(whitelist[sender] == true){
                ...
                uint256 totalFee = amount.mul(lpTax.add(projectTax)).div(1000);
        ...
        if (
            ...
            uint256 tokenSpent = swapTokens(contractTokenBalance);

            swapAndLiquify(contractTokenBalance.sub(tokenSpent));
        }
    ...
    function swapTokens(uint256 contractTokenBalance) private  returns(uint256) {
        // split the contract balance into halves
        uint256 projectShare = contractTokenBalance.mul(projectTax).div(projectTax.add(lpTax));

        uint256 initialBalance = address(this).balance;
        swapTokensForEth(projectShare); 
