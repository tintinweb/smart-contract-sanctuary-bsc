...
contract Bullish3 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    Bullish3DividendTracker public dividendTracker;
    ...
    constructor() ERC20("Bullish3", "BULL") {
        ...
        dividendTracker = new Bullish3DividendTracker(
            payable(this),
            router,
        ...
        swapTokensAtAmount = totalTokenSupply / 20; // 0.5%;
    ...
    // set cooldown timer, can only be between 0 and 3600 seconds (1 hour max)
    function setcooldowntimer(uint256 value) external onlyOwner {
        require(value <= 3600, "cooldown timer cannot exceed 1 hour");
        cooldowntimer = value;
    }
    ...
    function updateTransferFee(uint256 newTransferFee) public onlyOwner {
        require (newTransferFee <= 100, "transfer fee cannot exceed 1%");
    ...
    function updateFees(
        ...
        require(totalSellFees <= 99 && totalBuyFees <= 40, "total sell fees cannot exceed 40%");
...
contract Bullish3DividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    Bullish3 public Bullish3Contract;
    ...
    constructor(
        ...
        defaultToken = token;
        Bullish3Contract = Bullish3(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Bullish3_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Bullish3_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Bullish3 contract."
        );
    }
    ...
    function setMinimumTokenBalanceForAutoDividends(uint256 value)
        ...
        onlyOwner
    {
        minimumTokenBalanceForAutoDividends = value * (1**18);
    }
    ...
    function setMinimumTokenBalanceForDividends(uint256 value)
        ...
        onlyOwner
    {
        minimumTokenBalanceForDividends = value * (1**18);
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(Bullish3Contract);

            uint256 prevBalance = Bullish3Contract.balanceOf(address(this));
            ...
            {
                uint256 received = Bullish3Contract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    Bullish3Contract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
