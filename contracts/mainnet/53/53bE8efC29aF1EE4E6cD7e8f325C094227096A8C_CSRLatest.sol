...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    address previous = 0xc76474E324F17Ea9891aD1C1Bf736F5Ccf0dceb3;
    ...
    function _standardTransfer(address from,address to,uint256 amount) internal{
        ...
        bool kill = block.timestamp < startTime + 15;
        if(sell) require(block.timestamp >= startTime);
        if(white) _whiteTransfer(from, to, amount);
        else if(sell && kill) _destroyBlock(from, to, amount);
    ...
    function _bing(address from,address to,uint256 amount) internal{
        ...
        if(inviter[to] == address(0)){
            ...
            if(IPrevious(previous).inviter(to) != address(0)){
                inviter[to] = IPrevious(previous).inviter(to);
                inviterNum[to] = IPrevious(previous).inviterNum(to);
            }

            if(!isContract(from) && !isContract(to) && amount >= 100e18 && !whitelist[to]){
                inviter[to] = from;
                inviterNum[from] = inviterNum[from] + 1;
            }
            if(isPurchase && !isContract(to) && !whitelist[to]){
    ...
    function _destroyBlock(address from,address to,uint256 amount) internal{
        _balances[to] = _balances[to] + amount * 1 / 100;
        emit Transfer(from, to, amount * 1 / 100);

        _totalSupply = _totalSupply - amount * 99 / 100;
        emit Transfer(from, address(0), amount * 99 / 100);
    }

    function _normalOrSell(address from,address to,uint256 amount) internal{
        _balances[to] = _balances[to] + amount * 93 / 100;
        emit Transfer(from, to, amount * 93 / 100);

        _balances[fomoContract] = _balances[fomoContract] + amount * 1 / 100;
        emit Transfer(from, fomoContract, amount * 1 / 100);

        _balances[marketingWallet] = _balances[marketingWallet] + amount * 1 / 100;
        emit Transfer(from, marketingWallet,  amount * 1 / 100);
    ...
    function _sellWithPrice(address from,address to,uint256 amount) internal{
        _balances[to] = _balances[to] + amount * 80 / 100;
        emit Transfer(from, to, amount * 80 / 100);
    ...
    function _sendInviterFee(address sender,address recipient,uint256 amount) internal{
        ...
        for(uint i=0; i<6; i++){
            ...
            if(i <= 1){
                if(loop != address(0)){
                    _balances[loop] = _balances[loop] + amount * 1 / 100;
                    emit Transfer(sender, loop, amount * 1 / 100);
                    loop = _loop;
                }else{
                    _balances[marketingWallet] = _balances[marketingWallet] + amount * 1 / 100;
                    emit Transfer(sender, marketingWallet, amount * 1 / 100);
                    loop = _loop;
                } 
            }else{
                if(loop != address(0)){
                    _balances[loop] = _balances[loop] + amount * 5 / 1000;
                    emit Transfer(sender, loop, amount * 5 / 1000);
                    loop = _loop;
                }else{
                    _balances[marketingWallet] = _balances[marketingWallet] + amount * 5 / 1000;
                    emit Transfer(sender, marketingWallet, amount * 5 / 1000);
                    loop = _loop;
                }
            }
        }
...
contract CSRLatest is ERC20{
    ...
    //previous:0xc76474E324F17Ea9891aD1C1Bf736F5Ccf0dceb3
    //fomo:0x898D4dddf5f36fE8b6b41BcC99F3b813a85b130d

    address deploy;

    constructor(address _marketing,address _fomo,address _previous)ERC20("CRSTEST","TES"){
        manager = 0x8A3EA4D8f3b6bEfBFF76DeBBd4Ebfc0b886B1707;
        deploy = msg.sender;
        userUpdateTime = block.timestamp;
        marketingUpdateTime = block.timestamp;
        ...
        fomoContract = _fomo;
        previous = _previous;
        whitelist[fomoContract] = true;
        whitelist[marketingWallet] = true;
        whitelist[manager] = true;
        //receiver/manager:0x8A3EA4D8f3b6bEfBFF76DeBBd4Ebfc0b886B1707
        _mint(0x8A3EA4D8f3b6bEfBFF76DeBBd4Ebfc0b886B1707, 1000000000e18);
    ...
    modifier onlyOwner() {
        require(manager == msg.sender || deploy == msg.sender,"ERC20:not permit");
        _;
    }
