...
contract CHLL {
  ...
  uint256 constant private REFERRAL_COMMISSION = 30;
    ...
    uint256 _referralEarnings = uint256(info.users[msg.sender].referralEarnings);
    uint256 _totalRewards = _rewards + _referralEarnings;
		...
		info.busd.transfer(msg.sender, _totalRewards);
		emit Withdraw(msg.sender, _totalRewards);
	...
	function rewardsOf(address _user) public view returns (uint256) {
		return (uint256(int256(info.scaledBUSDPerToken * balanceOf(_user)) - info.users[_user].scaledPayout) / BIG_NUMBER) + uint256(info.users[_user].referralEarnings);
	}

	function withdrawnRewardsOf(address _user) public view returns (uint256) {
		return uint256(info.users[_user].scaledPayout) / BIG_NUMBER;
	}

	function allInfoFor(address _user) public view returns (uint256 contractBalance, uint256 totalRewardsEarned, uint256 totalTokenSupply, uint256 truePrice, uint256 mintPrice, uint256 burnValue, uint256 userBUSD, uint256 userAllowance, uint256 userBalance, uint256 userRewards,uint256 userRewardsEarned, uint256 userReferralEarnings, uint256 userLiquidValue) {
		contractBalance = info.busd.balanceOf(address(this));
		totalRewardsEarned = info.totalRewards;
		...
		userLiquidValue = calculateResult(userBalance, false) + userRewards + userReferralEarnings;
		userReferralEarnings = uint256(info.users[_user].referralEarnings);
		userRewardsEarned = withdrawnRewardsOf(_user) + uint256(userRewards + userReferralEarnings);
	}
    ...
    info.users[_referrerAddress].referralEarnings += int256(_tax / 3);
    ...
    info.totalRewards += _tax;
		tokens = calculateResult(_amount, true);
    uint256 _taxAfterReferral = uint256(_tax - (_tax / (100 / REFERRAL_COMMISSION)));
    info.users[info.users[msg.sender].referrer].referralEarnings += int256(_tax / 3);
		info.totalSupply += tokens;
		info.users[msg.sender].balance += tokens;
		info.users[msg.sender].scaledPayout += int256(tokens * info.scaledBUSDPerToken);
		info.scaledBUSDPerToken += _taxAfterReferral * BIG_NUMBER / info.totalSupply;
