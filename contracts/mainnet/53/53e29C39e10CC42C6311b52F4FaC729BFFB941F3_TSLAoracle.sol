...
contract TSLAoracle is Context, Ownable {
    ...
    function Oraclemanagement(address _user, uint256 _amount) internal {
            ...
            if (_nowtime >= resets){
                ...
                topdeposit = 0;
                resets = _nowtime + 7 minutes * 5;
            }

            if (block.timestamp >= smallresets){
                smallTSLAholder = address(0);
                smalltopdeposit = 0;
                smallresets = _nowtime + 3 minutes * 5;
            }
    ...
    function deposit(address _ref, uint256 _amount) public  {
        ...
        //fees whitelist can access 2 minutes * 5 before with 50% discount
        if (nowtimer - WhiteTime <= 2 minutes * 5){
            //update whitelist
            Whitelist_TSLA = _BUSDOracle(BUSDaddress).getWhitelist();
            require (isWhiteListed(msg.sender) == true, "only whitelists allowed for the first 2 minutes * 5");
        ...
        //totalweekly is the amount that users deposit in a 7 minutes * 5 range, unlike the total user deposit, this refreshes weekly
        uint256 totalweekly = weekly_investments[msg.sender].invested + _amount;

        //total minutes * 5 refreshes every 48 hours and it is needed to assign the small oracle, total amount has to be less than 3k BUSD
    ...
    function userReward(address _userAddress) public view returns(uint256) {
        ...
        //daily rewards need to be mutiplied for max claiming deadline (4 minutes * 5)
    ...
    if (weekly[msg.sender].deadline <= block.timestamp){
            weekly[msg.sender].deadline = block.timestamp + 2 minutes * 5; 
        }
        else {
            weekly[msg.sender].deadline += 2 minutes * 5;
        }
    ...
    function claimDailyRewards() public {
        ...
        //Users can claim only after 1 minutes * 5 or keep accumulating rewards up to 4 minutes * 5
        require(claimTime[msg.sender].deadline - 3 minutes * 5 <= block.timestamp, "You cant claim before 24 hours");  
        ...
        uint256 claimTimeEnd = claimTimeStart + 4 minutes * 5;
        claimTime[msg.sender] = claimDaily(msg.sender,claimTimeStart,claimTimeEnd);
    ...
    function signal_market() external {
        ...
        resets = block.timestamp + 8 minutes * 5;
        smallresets = block.timestamp + 3 minutes * 5;
    }

    //Owner can call Oraclemanagement to update the platform (in case there there are few claims to trigger it in the first few minutes * 5 after 1 week)
    function callmanagement() external onlyOwner {
        require(block.timestamp >= resets,"Oracle can reset every 7 minutes * 5 only");
        Oraclemanagement(msg.sender, 0);
    }
    ...
    function MinClaim(address _user) external view returns(uint256) {
            return claimTime[_user].deadline - 3 minutes * 5;
    }
