...
contract OPENAIToken is Context,IERC20,Ownable {
    ...
    // 黑洞地址
    address private blackHole = 0x48c92e76E4F32942e36363DA7863D0fcA866ceAf;
    // 空投地址
  ...
  */
   ...
   */
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if(takeFee && automatedMarketMakerPairs[recipient]){
            ...
            if(xhFee > 0){
                ...
                swapXhTokensForCake(xhFee);
            }

            // 空投
            uint256 ktFee = _chuKtRate.mul(amount).div(1000);
            if(ktFee > 0){
                _balances[ktAddress] = _balances[ktAddress].add(ktFee);
            }
    ...
    function swapLpTokensForCake(uint256 tokenAmount) private {
    ...
    function swapXhTokensForCake(uint256 tokenAmount) private {
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = rewardToken;
        // make the swap
