...
contract DEPToken is Context, IERC20, Ownable {
    ...
    constructor (
        ...
        uniswapPair = IUniswapV2Factory(_uniswapV2Router.factory())
            .createPair(address(this), pair_token2);
        ...
        liquidityWalletAddress = payable(liquidityAddress);
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        bool shouldSetInviter = balanceOf(recipient) == 0 && inviter[recipient] == address(0) && sender != address(uniswapPair);
        if (shouldSetInviter) {
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(isMarketPair[sender]) {
            ...
            liquidityAmount = amount.mul(_buyLiquidityFee).div(100);
            teamAmount = amount.mul(_buyTeamFee).div(100);
            marketAmount = amount.mul(_buyMarketingFee).div(100);
            if(liquidityAmount > 0){
                takeFeeTransfer(sender, liquidityWalletAddress, liquidityAmount);
            }
            if(teamAmount>0){
                takeFeeTransfer(sender, teamWalletAddress, teamAmount);
            }
            if (marketAmount>0){
                takeFeeTransfer(sender, marketingWalletAddress, marketAmount);
            }
        }
            ...
            teamAmount = amount.mul(_sellTeamFee).div(100);
        ...
        return amount.sub(liquidityAmount).sub(destAmount).sub(teamAmount).sub(marketAmount);
    }

    function takeFeeTransfer(address sender,address to, uint256 tAmount) private{
