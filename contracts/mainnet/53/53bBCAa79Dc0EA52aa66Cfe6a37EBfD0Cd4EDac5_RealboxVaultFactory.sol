...
contract RealboxVaultFactory is AccessControlEnumerable {
    ...
    function deployVault(
        RealboxVault.SalesInfo memory _salesInfo,
        RealboxVault.TokenInfo memory _tokenInfo,
        address _ownerAddress
    ) external {
        require(hasRole(CREATOR_ROLE, _msgSender()), 'RealboxVaultFactory: must have creator role to deploy new vault');
        require(
            block.number < _salesInfo.privateStartBlock,
            'RealboxVaultFactory: privateStartBlock must be higher than current block'
        );
        require(
            _salesInfo.privateStartBlock <= _salesInfo.publicStartBlock,
            'RealboxVaultFactory: Private sale parameters incorrect'
        );
        require(
            _salesInfo.publicStartBlock < _salesInfo.endBlock,
            'RealboxVaultFactory: publicStartBlock must be lower than endBlock'
        );
        require(
            _salesInfo.minSupply <= _salesInfo.maxSupply,
            'RealboxVaultFactory: minSupply must not be higher than maxSupply'
        );
        ...
        RealboxVault(vaultAddress).initialize(_vaultIds.current(), realx, _salesInfo, _tokenInfo, _ownerAddress);
