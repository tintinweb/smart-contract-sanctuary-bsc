...
contract GFICrypto is Context, IRankedMintingToken, IStakingToken, IBurnableToken, ERC20("GFI Crypto", "bGFI") {
    using Math for uint256;
    using ABDKMath64x64 for int128;
    using ABDKMath64x64 for uint256;

    // INTERNAL TYPE TO DESCRIBE A GFI MINT INFO
    struct MintInfo {
        address user;
        uint256 term;
        uint256 maturityTs;
    ...
    // INTERNAL TYPE TO DESCRIBE A GFI STAKE
    struct StakeInfo {
        uint256 term;
        uint256 maturityTs;
        uint256 amount;
    ...
    uint256 public constant GFI_MIN_STAKE = 0;

    uint256 public constant GFI_MIN_BURN = 0;

    uint256 public constant GFI_APY_START = 20;
    uint256 public constant GFI_APY_DAYS_STEP = 90;
    uint256 public constant GFI_APY_END = 2;

    string public constant AUTHORS = "@globalfi";
    ...
    uint256 public totalGfiStaked;
    // user address => GFI mint info
    mapping(address => MintInfo) public userMints;
    // user address => GFI stake info
    mapping(address => StakeInfo) public userStakes;
    // user address => GFI burn amount
    ...
    /**
     * @dev calculates GFI Stake Reward
     */
    ...
    function _calculateAPY() private view returns (uint256) {
        uint256 decrease = (block.timestamp - genesisTs) / (SECONDS_IN_DAY * GFI_APY_DAYS_STEP);
        if (GFI_APY_START - GFI_APY_END < decrease) return GFI_APY_END;
        return GFI_APY_START - decrease;
    }
    ...
    /**
     * @dev returns GFI Stake object associated with User account address
     */
    ...
    /**
     * @dev ends minting upon maturity (and within permitted Withdrawal Time Window), gets minted GFI
     */
     ...
     *       mints GFI coins and splits them between User and designated other address
     */
    ...
    /**
     * @dev  ends minting upon maturity (and within permitted Withdrawal time Window)
     *       mints GFI coins and stakes 'pct' of it for 'term'
     */
    function claimMintRewardAndStake(uint256 pct, uint256 term) external {
        ...
        require(stakedReward > GFI_MIN_STAKE, "GFI: Below min stake");
        require(term * SECONDS_IN_DAY > MIN_TERM, "GFI: Below min stake term");
        require(term * SECONDS_IN_DAY < MAX_TERM_END + 1, "GFI: Above max stake term");
        require(userStakes[_msgSender()].amount == 0, "GFI: stake exists");
    ...
    /**
     * @dev initiates GFI Stake in amount for a term (days)
     */
    function stake(uint256 amount, uint256 term) external {
        require(balanceOf(_msgSender()) >= amount, "GFI: not enough balance");
        require(amount > GFI_MIN_STAKE, "GFI: Below min stake");
        require(term * SECONDS_IN_DAY > MIN_TERM, "GFI: Below min stake term");
        require(term * SECONDS_IN_DAY < MAX_TERM_END + 1, "GFI: Above max stake term");
        require(userStakes[_msgSender()].amount == 0, "GFI: stake exists");
    ...
    /**
     * @dev ends GFI Stake and gets reward if the Stake is mature
     */
    function withdraw() external {
        StakeInfo memory userStake = userStakes[_msgSender()];
        require(userStake.amount > 0, "GFI: no stake exists");

        uint256 gfiReward = _calculateStakeReward(
            userStake.amount,
            userStake.term,
        ...
        activeStakes--;
        totalGfiStaked -= userStake.amount;

        // mint staked GFI (+ reward)
        _mint(_msgSender(), userStake.amount + gfiReward);
        emit Withdrawn(_msgSender(), userStake.amount, gfiReward);
        delete userStakes[_msgSender()];
    }

    /**
     * @dev burns GFI tokens and creates Proof-Of-Burn record to be used by connected DeFi services
     */
    function burn(address user, uint256 amount) public {
        require(amount > GFI_MIN_BURN, "Burn: Below min limit");
        require(
