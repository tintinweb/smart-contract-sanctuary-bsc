...
contract VabDirectCom is Pausable, Ownable {
    ...
    function purchase() public whenNotPaused returns (uint) {
        require(token20.balanceOf(msg.sender)>= price, 'not enough balance');
        iMint(msg.sender);
        token20.transferFrom(msg.sender, address(this), price);
        return iMint(msg.sender);
    }
    function purchaseRef(address ref) public whenNotPaused returns (uint) {
    ...
    function appPurchase(string memory _tkn, address _user) public whenNotPaused returns (uint) {
    ...
    function appPurchaseRef(string memory _tkn, address _user ,address ref) public whenNotPaused returns (uint) {
        require(keccak256(abi.encodePacked(_tkn)) == tknHash, "App not allowed");
