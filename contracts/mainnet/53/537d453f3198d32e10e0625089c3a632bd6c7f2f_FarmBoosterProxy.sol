...
contract FarmBoosterProxy is ReentrancyGuard {
     ...
     * @param _SELFToken: the address of the SELF token
     */
    function initialize(
        address _admin,
    ...
    function harvestSELF() internal {
        uint256 SELFBalance = SELFToken.balanceOf(address(this));
        if (SELFBalance > 0) {
            SELFToken.safeTransfer(msg.sender, SELFBalance);
        }
    }
}
...
contract FarmBoosterProxyFactory {
    ...
    address public immutable SELFToken;
    ...
    function createFarmBoosterProxy() external {
        ...
        FarmBoosterProxy(farmBoosterProxyAddress).initialize(msg.sender, Farm_Booster, masterchefV2, SELFToken);
