...
// File: contracts/stakingtst4.sol
...
contract StakingBitscrow {
    ...
    function tokenaavailableforrewards() public view returns(uint256){
        return _tokenaavailableforrewards;
    }

    function placesLeft() public view returns(uint256){
        return _tokenaavailableforrewards * 100 / rewardsrate / Cost;
    }
    ...
    function personalRewards(address user) public view returns(uint){
    ...
    function seeRewardsRate() public view returns(uint256){
        return rewardsrate;
    }
    ...
    function  seeapy()  public view returns(uint){
        return apy;
    }

    function  seeCountdownEnd()  public view returns(uint){
        return countdownEnd;
    }
    ...
    /// @param timeperiodInseconds duration of the stake (in seconds)
    constructor(IERC20 _erc20_contract_address, uint256 timeperiodInseconds, uint _apy) {
        // Set contract owner
        owner = msg.sender;
    ...
    function stakeTokens() public {
        uint amount = Cost;
        PersonalRewards[msg.sender] = amount * rewardsrate / 100;
    ...
    /**
     * @dev allows the fromrewards wallet to wiithdraw the locked funds, note that can e used only if 
     * the corect time haas passed sice the bbbegiinning f the countdoown fo the withdraw, note that even
     * if all the funds in ('_tokenaavailableforrewards') are withdrawn the contract would still retain 
     * enough  funds too pay back all the tokens that ae currently staked and their respective rewards
     */
     /// @param amount amount of tokens to be withdrawn
    function withdrawlockedfunds(uint amount) public returns(bool success){
        require(countdownEnd != 0, "countdoownEnd cannot be zero");
        require(block.timestamp >= countdownEnd, "cannot withdraw yet");
        require(msg.sender == fromrewards, "you arent fromrewards");
    ...
    /**
     * @dev allows the fromrewards wallet to initiate the countdown for the withdrawal pocedure
     */
    function initiateCountdownForWithdraaw() public onlyOwner returns(bool success){
        countdownEnd = block.timestamp + minPeriodBetweenWithdraws;
        return true;
    }
    
    /**
     * @dev allows the fromrewards wallet to lock the tokens in oorde foor themm to be used to pay the rewards
     */
     /// @param amount amount of tokens to be locked
    function lockTokens(uint256 amount) public {
        require(msg.sender == fromrewards);
    ...
    function unstakeTokens() public  {
        ...
        require(block.timestamp >= timestart[msg.sender] + timePeriod, 
    ...
    function restakeTokens() public {
        ...
        uint rewards = PersonalRewards[msg.sender];
        alreadyWithdrawnRewards[msg.sender] += rewards;
        PersonalRewards[msg.sender] = _amount * (rewardsrate + 2) / 100;
     ...
     * this will send the only the tokens orignally stake to the user, resulting in the user losin his rewards
     */
