...
contract SafeFarm is ReentrancyGuard, AccessControl {
    ...
    */
    ...
    function deposit(uint256 _amount) public nonReentrant {
        ...
        _deposit(msg.sender, _amount, 0, route, 0);
    }
    ...
    function depositAuto(
        ...
        depositAuto(from, _amount, _fee, route, 0);
    }
    ...
    function depositAutoLP(
        ...
        _depositLP(from, _amount, _fee, route0, route1);
    }
     ...
     *  and the balance deployed in other contracts as part of the strategy + pending reward.
     */
    function balance() public view returns (uint256) {
        return available() + strategy.balance();
    }

    // it calculates pool balance + pending reward
    function poolBalance() public view returns (uint256) {
        return strategy.balanceOfPool() + strategy.pendingReward();
    }
    ...
    function _deposit(
        ...
        if (_fee > 0) {
            ...
            tokenA.safeTransfer(feeRecipient, _fee);
            _amount-= _fee;
        }
    ...
    function _depositLP(
        ...
        uint256[] memory _amountOutMin = new uint256[](4);
        _amountOutMin[0] = _amountOutMin[1] = 0; // min swap amount
        _amountOutMin[2] = _amountOutMin[3] = 1; // min amounts for addLiquidity

        _depositLP(_depositor, _amount, _fee, _route0, _route1, _amountOutMin);
    }
    ...
    function _depositLP(
        ...
        uint256 shares = (poolBalance() - _pool); // Additional check for deflationary tokens
        if (shares > _amount) {
            shares = _amount;
        }
    ...
    function _swapLPToken(
        ...
        address[] memory route,
        uint256 _amountOutMin
    ) internal returns (LPToken memory)
    {
        address tokenB = route[route.length - 1];
    ...
    function _addLpLiquidity(
        ...
        (uint256 amountA, uint256 amountB, uint256 liquidity) = IUniswapRouterETH(unirouter).addLiquidity(
            lpt0.token,
            lpt1.token,
            lpt0.amount,
            lpt1.amount,
