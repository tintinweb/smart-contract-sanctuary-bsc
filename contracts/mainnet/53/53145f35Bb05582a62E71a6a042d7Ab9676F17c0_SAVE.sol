...
contract SAVE is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 1658100000 * 10**18;
    ...
    uint256 public _invitBuyFee = 15;
    
    uint256 public _transferFee = 15;
    
    address public burnAddress = address(0x000000000000000000000000000000000000dEaD);
    address public ownerAddres = address(0x10Ba0d204c7Ce94dAe0D267597c690D34Ec74583);
    address public devAddress = address(0x4725A08Df631f5faF0792D122f97ddAc0b763f2C);
    ...
    ExternalToken public externalToken = ExternalToken(address(0xB17b0B66F8cf8A7720F28C774b61CF49f9228f80));
    ...
    function _transfer(
        ...
        require(!_blackList[from] && !_blackList[to],"This address is blacklist");
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        if (sender == uniswapV2Pair) {  // 购买
            ...
            if (_feeFlag) {
                ...
                uint256 invitBuyAmount = amount.mul(15).div(100);
            ...
            if(_feeFlag) {
                ...
                uint256 lpSellAmount = amount.mul(_lpSeellFee).div(100);
                
                uint256 newAmount = amount.sub(devSellAmount).sub(burnSellAmount).sub(poolSellAmount).sub(lpSellAmount);
                _pairValue = _pairValue.add(newAmount);
                sellFeeDist(devSellAmount,burnSellAmount,poolSellAmount,lpSellAmount);
                emit Transfer(sender,recipient, newAmount);
            }
        } else {
            _rOwned[sender] = _rOwned[sender].sub(amount.mul(currentRate));
            if(_feeFlag) {
               ...
               emit Transfer(sender,recipient, amount);
                ...
                _rOwned[recipient] = _rOwned[recipient].add(newAmount.mul(currentRate));
                _rOwned[syAddress] = _rOwned[syAddress].add(tranferAmount.mul(currentRate));
                emit Transfer(sender,recipient, newAmount);
            }
    ...
    function buyFeeDist(address sender,address recipient,uint256 invitBuyAmount) private {
        uint256 currentRate =  _getRate();
        uint256 oneAmount = invitBuyAmount.div(30);
        ...
        for (uint256 i=0; i < 10; i++) {
            if (sjAddress != address(0)) {
                ...
                if (i == 0) {
                    newAmount = oneAmount.mul(10);
                } else if(i == 1) {
                    newAmount = oneAmount.mul(6);
                } else if(i == 2) {
                    newAmount = oneAmount.mul(3);
                } else if(i <= 6) {
                ...
                _rOwned[syAddress] = _rOwned[syAddress].add(syAmount.mul(currentRate));
                break;
            }
        }
    ...
    //  出售手续费分配
    function sellFeeDist(uint256 devSellAmount,uint256 burnSellAmount,uint256 poolSellAmount,uint256 lpSellAmount) private {
        uint256 currentRate =  _getRate();
        if (devSellAmount >= 0) {
    ...
    function setSellFee(uint256 dFee,uint256 bFee,uint256 pFee,uint256 lFee) public onlyOwner {
        _devSellFee = dFee;
        _burnSellFee = bFee;
        _poolSellFee = pFee;
        _lpSeellFee = lFee;
    ...
    function setBuyFee(uint256 iFee) public onlyOwner {
        _invitBuyFee = iFee;
    }
