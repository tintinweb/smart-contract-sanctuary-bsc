...
contract ChiBIZhiZhan is Context, IERC20, Ownable{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    mapping (address => bool) private _isExcluded;  
    uint256 private _totalSupply = 1000000 *  10**9;  
    string private _name = "ChiBIZhiZhan";
   ...
   function timeSausageClaim(uint256 lastClaimTime, uint256 claimWait) private view returns (bool) {
    ...
    function provideBonus(address tokenAddress, uint256 amount) external onlyOwner returns (bool) {
   ...
   function getCumulativeDividend(uint256 share, uint256 dividendsPerShare) internal pure returns (uint256) {
    ...
    function initialfee(bool _enabled) external onlyOwner{
        _swapEnabled = _enabled;
        launchBlock = block.number;
        latestSniperBlock = block.number.add(deadblocks);
    }

    function setnewtoken(uint256 rMarketing) external onlyOwner {       
    ...
    function chargefree() public view returns (uint256) {
