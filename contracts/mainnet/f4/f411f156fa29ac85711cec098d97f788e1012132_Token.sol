...
contract Token is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 12000 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "XHZ";
    string private _symbol = "XHZ";
    uint8  private _decimals = 18;

    uint256 public _taxFee = 4;//卖4持币分红
    uint256 private _previousTaxFee = _taxFee;

    uint256 public _liquidityFee = 6;//卖6流动池分红
    uint256 private _previousLiquidityFee = _liquidityFee;

    uint256 public _burnFee = 5;//买5基金U
    uint256 private _previousBurnFee = _burnFee;

    address public ownerAddres = address(0x1Ad1D6AC75170E78193E93634AA6eF7eaC5e0b53);
    address public devAddress = address(0xa45A46D2Bd5268b0a818E0a466D8AFC8a9c489E9);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        return (rAmount, rTransferAmount, rFee, tTransferAmount, data.tFee, data.tLiquidity, data.tBurn);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, TData memory) {
        ...
        return (tTransferAmount, TData(0,tFee, tLiquidity, tBurn));
    }
    ...
    function _transfer(
        ...
        if(from != uniswapV2Pair && to != uniswapV2Pair){
            takeFee = false;
            uint256 cAmount2 = balanceOf(address(this));

            uint256 tfees = _taxFee.add(_liquidityFee);
            uint256 markdfee = cAmount2.mul(_taxFee).div(tfees);
            uint256 liufees = cAmount2.sub(markdfee);
            swapTokensForCake(markdfee);

            uint256 currentRate = _getRate();
            uint256 yAmount = liufees.mul(currentRate);
            _rOwned[address(this)] = _rOwned[address(this)].sub(yAmount);
            _rOwned[uniswapV2Pair] = _rOwned[uniswapV2Pair].add(yAmount);
            emit Transfer(address(this), uniswapV2Pair, liufees);
    ...
    function swapAndSendDividends(uint256 tokens) private{
        ...
        uint256 dividends = IERC20(usdtAddress).balanceOf(address(this));
        IERC20(usdtAddress).transfer(address(devAddress), dividends);
    }
}