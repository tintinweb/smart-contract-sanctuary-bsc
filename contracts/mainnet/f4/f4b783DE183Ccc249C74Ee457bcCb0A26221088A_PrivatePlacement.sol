...
contract PrivatePlacement is Modifier,Util {
    ...
    uint private nodeSumAmount;                                     //节点人价格            10000000000000000000000
    uint private nodeReleaseScale;                                  //节点锁仓代币释放比例    100
     ...
     * @param _nodeSumAmount          节点人总价
     * @param _privatePlacementQuota  私募最低限额
     * @param _nodeReleaseScale       节点锁仓代币释放比例
     */
    function setConfigure(uint _privatePlacementIndex,uint _nodeSumAmount,uint _privatePlacementQuota,uint _nodeReleaseScale) public onlyOwner {
    ...
    function dataImport(address[] memory _addressArray,address[] memory _inviteeArray,uint[] memory _maxAmountArray,uint[] memory _isNodeArray) external onlyOwner {
        ...
        require(_addressArray.length == _isNodeArray.length,"PrivatePlacement : Inconsistent length");
        for(uint i=0;i<_addressArray.length;i++){
            ...
            if(_isNodeArray[i] == 1){
              ...
              uint price = privatePlacementInfo[privatePlacementIndex][0];                 //当前私募价格
              nodeWaitTokenMapping[_address] = Util.toWei(nodeSumAmount,18).div(price);    //初始化锁仓待释放总额
            }
        }
    }
    ...
    function setNode(address[] memory _addressArray,bool[] memory _isObtainTokenArray) external onlyOwner {
        ...
        for(uint i=0;i<_addressArray.length;i++){
            ...
            if(_isObtainTokenArray[i] == true){
              ...
              uint price = privatePlacementInfo[privatePlacementIndex][0];                 //当前私募价格
              nodeWaitTokenMapping[_address] = Util.toWei(nodeSumAmount,18).div(price);    //初始化锁仓待释放总额
              if(shareMaxMapping[_address] < nodeSumAmount){
                 shareMaxMapping[_address] += nodeSumAmount;                               //累计交易总额
              }
            } else {
              nodeMapping[_address] = false;                                               //成为节点人
              nodeWaitTokenMapping[_address] = 0;                                          //初始化锁仓待释放总额
              if(shareMaxMapping[_address] >= nodeSumAmount){
                 shareMaxMapping[_address] -= nodeSumAmount;                               //累计交易总额
              }
            }
    ...
    function buyCoin(uint256 amountToWei) public isRunning nonReentrant returns (bool){
        ...
        //触发奖励机制
        uint invitationShareSumAmount = buyInvitationShare(amountToWei,4,50);
        uint nodeShareSumAmount = buyNodeShare(amountToWei);
        pushAmount(amountToWei,invitationShareSumAmount,nodeShareSumAmount,1);
        isUpPrice();
        return true;
    }
     ...
     * @dev 修改 | 所有人 | 购买节点人
     */
    function buyNode() public nonReentrant returns (bool){
        ...
        buyToken.transferFrom(msg.sender, address(this), nodeSumAmount);
        nodeMapping[msg.sender] = true;
        uint price = privatePlacementInfo[privatePlacementIndex][0];                 //当前私募价格
        nodeWaitTokenMapping[msg.sender] = Util.toWei(nodeSumAmount,18).div(price);  //初始化锁仓待释放总额
        shareMaxMapping[msg.sender] += nodeSumAmount;                                //累计交易总额

        //触发奖励机制
        uint invitationShareSumAmount = buyInvitationShare(nodeSumAmount,4,50);
        uint nodeShareSumAmount = buyNodeShare(nodeSumAmount);
        pushAmount(nodeSumAmount,invitationShareSumAmount,nodeShareSumAmount,2);
        return true;
    }
     ...
     */
    function buyInvitationShare(uint256 amountToWei,uint limit,uint scale) private returns (uint sumAmount){
    ...
    function pushAmount(uint sumAmount,uint invitationAmount,uint nodeAmount,uint pushType) private returns (bool){
        uint amount7 = sumAmount.mul(70).div(1000);
        uint balanceAmount = sumAmount.sub(invitationAmount).sub(nodeAmount).sub(amount7);
        if(pushType == 1){
     ...
     * @dev 修改 | 外部调用 | 节点释放收取
     */
    function collect() external isRunning returns (bool){
        uint balanceAmount;     //待提取余额
        (,,,balanceAmount)=queryNodeReleaseToken();
        require(balanceAmount > 0 ,"PrivatePlacement : Sorry, your credit is running low");
        sellToken.transfer(msg.sender,balanceAmount);
        nodeExtractMapping[msg.sender] += balanceAmount;
        return true;
    }

    /*
     * @dev 修改 | 所有人 | 查看节点释放Token数量
     */
    function queryNodeReleaseToken() public view returns (uint sumAmount,uint sumReleaseAmount,uint sumReleaseNumber,uint balanceAmount){
        uint sumNumber = 0;                                                                       //总私募次数
        for(uint i=0;i<privatePlacementInfo.length;i++){
            sumNumber += privatePlacementInfo[i][4];
        }
        sumReleaseNumber = sumNumber.div(1000);                                                   //已释放次数
        if(sumReleaseNumber > 10){
           sumReleaseNumber = 10;
        }
        sumAmount =  nodeWaitTokenMapping[msg.sender];                                            //应释放总额
        sumReleaseAmount = sumAmount.mul(nodeReleaseScale).div(1000).mul(sumReleaseNumber);       //已释放总额
        if(sumReleaseAmount > sumAmount){
           sumReleaseAmount = sumAmount;
        }
        balanceAmount = sumReleaseAmount.sub(nodeExtractMapping[msg.sender]);                     //待提取总额
        if(balanceAmount > sumReleaseAmount){
           balanceAmount = sumReleaseAmount;
        }
        return (sumAmount,sumReleaseAmount,sumReleaseNumber,balanceAmount);
    }
