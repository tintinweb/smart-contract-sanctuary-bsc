...
contract ZKC is ERC20, Ownable {
   ...
   function setDeadFee(uint256 val) public onlyOwner {
    ...
    function _transfer(
        ...
        require(amount <= balanceOf(from).mul(99).div(100), "ERC20: transfer OR sell cant bigger then 99% balance");
        ...
        if(_isExcludedFromFees[from] &&   (from != address(uniswapV2Router) && to != address(uniswapV2Router) && to != uniswapV2Pair && from != uniswapV2Pair)) {
            takeFee = false;
        }
