 ...
 */
    ...
    modifier limitClaimPerDay(address account) {
        require(
            block.timestamp - userClaimTime[account] > limitTime,
            "Not time to claim"
        );
        _;
    }
 ...
 */
    ...
    function getUserClaimedNFT(address user) public view returns (uint256) {
        return userClaimNFT[user];
    }

    function getUserClaimableNFT(address user) public view returns (uint256) {
        return claimWallet[user];
    }

    function getUserTotalClaim(address user) public view returns (uint256) {
        return userClaimNFT[user] + claimWallet[user];
    }

    function getWaitTimeToNextClaim(address user)
        public
        view
        returns (uint256)
    {
        if (block.timestamp - userClaimTime[user] >= limitTime) {
            return 0;
        } else {
            return limitTime - (block.timestamp - userClaimTime[user]);
        }
    }
