...
// File: contracts/USDTStaking1.sol
...
contract JellyFishStaking is ReentrancyGuard, Ownable {
	...
	uint constant internal WITHDRAW_PER_WEEK = 1000 ether; 
    ...
    uint256 constant internal INVEST_MIN_AMOUNT = 5 ether;
    uint256 constant internal INVEST_MAX_AMOUNT = 1000 ether;
    ...
    function deposit(uint8 _tarif, address _upline, uint256 amount) external nonReentrant whenNotPaused{
        ...
        uint256 w2 = amount.mul(30).div(PERCENT_DIVIDER); // 3%
        uint256 w3 = amount.mul(20).div(PERCENT_DIVIDER); // 2%
        
        
		USDT.safeTransfer(wallet1, w1);
        USDT.safeTransfer(wallet2, w2);
        USDT.safeTransfer(wallet3, w3);
       
		emit FeePayed(msg.sender, w1.add(w2).add(w3));
    ...
    function reinvest(uint8 _tarif, uint256 _amount) public whenNotPaused nonReentrant{
       ...
       require(_amount >= INVEST_MIN_AMOUNT, "Minimum deposit amount incorrect");
                ...
                msg.sender != wallet3 ) 
        {
            ...
            uint256 w1 = amountReinvest.mul(30).div(PERCENT_DIVIDER);  // 3%
            uint256 w2 = amountReinvest.mul(20).div(PERCENT_DIVIDER);  // 2%
    ...
    function withdraw(uint256 _amount) external whenNotPaused {
                ...
                msg.sender != wallet3) 
        {
            ...
            // FEES  amountWithdraw
            uint256 w1 = amountWithdraw.mul(30).div(PERCENT_DIVIDER);  // 3%
            uint256 w2 = amountWithdraw.mul(20).div(PERCENT_DIVIDER);  // 2%
    ...
    function panicButton() external onlyOwner{
        USDT.safeTransfer(wallet1, getContractBalance());
    }
