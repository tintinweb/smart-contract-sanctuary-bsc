...
contract FarmBooster is Ownable {
    ...
    uint256 public BOOST_WEIGHT_PRECISION = 100 * 1e10; // 100%
...
contract IterableMappingTest {
    ...
    function insert(uint256 key, uint256 value) external {
        return itmap.insert(key, value);
    }

    function remove(uint256 key) external {
        return itmap.remove(key);
    }

    function contains(uint256 key) external view returns (bool) {
        return itmap.contains(key);
    }

    function size() external view returns (uint256) {
        return itmap.keys.length;
    }

    function get(uint256 key) external view returns (uint256) {
        return itmap.data[key];
    }

    function iterateGet(uint256 index) external view returns (uint256 key, uint256 value) {
        key = itmap.keys[index];
        value = itmap.data[key];
    }
}
...
contract FarmBoosterProxy {
     ...
     */
    function deposit(uint256 _pid, uint256 _amount) external onlyAdmin {
     ...
     */
    function withdraw(uint256 _pid, uint256 _amount) external onlyAdmin {
        uint256 poolLength = masterchefV2.poolLength();
        require(_pid < poolLength, "Pool is not exist");
     ...
     * @param _pid The id of the pool.
     */
    function emergencyWithdraw(uint256 _pid) external onlyAdmin {
        uint256 poolLength = masterchefV2.poolLength();
        require(_pid < poolLength, "Pool is not exist");
...
contract MockContractUser {
    ...
    function setCakePool(CakePool _cakePool) external {
        cakePool = _cakePool;
    }

    function approve(address _account, uint256 _amount) external {
        cake.approve(_account, _amount);
    }
...
contract MockToken is ERC20 {
    constructor() ERC20("Mock LP", "LP92") {
...
import {CakePool} from "pancake-cake-pool/contracts/CakePool.sol";
import {MockContractUser} from "pancake-cake-pool/contracts/test/MockContractUser.sol";