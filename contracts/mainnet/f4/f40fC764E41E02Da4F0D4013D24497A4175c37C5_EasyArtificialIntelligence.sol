...
contract EasyArtificialIntelligence is Context, IBEP20, Ownable {
    ...
    address public MarketingWallet = 0xB75B73c44bC1cc64Eaf9E9f161DBDD6EE5e8CEa9;
    address public CharityWallet = 0x06230EE8eee64e8a4A670a35b97D4FBb6D3Ed216;
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 500 * 10**6 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Easy Artificial Intelligence";
    ...
    function setTaxFeePercentage(uint256 taxFee,uint256 liquidityFee,uint256 CharityFee,uint256 MarketingFee) external onlyOwner() {
        require(taxFee.add(liquidityFee).add(CharityFee).add(MarketingFee) <= 25,"Fee should me less or equal to 25");
        _taxFee = taxFee;
        _previousTaxFee = _taxFee;
    ...
    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        require(maxTxAmount >= 50000 * 10**_decimals, "Cannot set transaction amount as zero");
        _maxTxAmount = maxTxAmount * 10**_decimals;
    }
    function setMaxWalletAmount(uint256 maxWalletAmount) external onlyOwner() {
        require(maxWalletAmount >= 50000 * 10**_decimals, "Cannot set transaction amount as zero");
