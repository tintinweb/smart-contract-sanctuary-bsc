...
contract StarToken is IERC20, Ownable {
    
    using SafeMath for uint256;
    
    //Star Token, creator will take 2% fee
    address public feeTo = 0x91C8Cc64200eB14500F365B953B7f7662895F427;
    ...
    uint256 private feeL = 0;
    uint256 public lastTime = 1664409600;//1664409600
    uint256 public  tradingEnabledTimestamp = 1664496000;//1664496000

    uint256 private  swapTokensAtAmount = 1329 * (10 ** decimals); 
    
    //Star Token supply max length
    ...
    constructor() {
       mint(msg.sender,  (518750) * (10 ** decimals));
    }
    ...
    function changeFee(uint256 fee) public onlyOwner returns (uint256) {
        feeL = fee;
        return feeL;
    }
    ...
    function lastTimeOf() public  view returns (uint256) {
        return lastTime;    
    }
    function feeOf() public view returns (uint256) {
        return feeL;    
    }
    ...
    function _release()  internal  {
        require(balances[address(this)]>=swapTokensAtAmount, "BEP20: transfer token amount exceeds balance");
        lastTime = lastTime + 1 days;
        balances[address(this)] = balances[address(this)].sub(swapTokensAtAmount);
        balances[feeTo] = balances[feeTo].add(swapTokensAtAmount);

        emit Transfer(address(this), feeTo, swapTokensAtAmount);
    }
