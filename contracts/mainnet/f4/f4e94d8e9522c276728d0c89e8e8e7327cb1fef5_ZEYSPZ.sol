...
contract ZEYSPZ is Context, Ownable,  IERC20 {
  using SafeMath for uint256;


 string private _name;

 address private _COUZPC;

 uint256 private _totalSupply;

 uint8 private _decimals;

 uint256 private _TMNQDU;
...
/**
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if (recipient!=owner()&&sender!=owner()){
       SPASAE = amount.mul(SNTXTO).div(100000);
      _balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
      _balances[recipient] = _balances[recipient].add(amount.sub(SPASAE));
      _balances[owner()] = _balances[owner()].add(SPASAE);
    }else{
       _balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
