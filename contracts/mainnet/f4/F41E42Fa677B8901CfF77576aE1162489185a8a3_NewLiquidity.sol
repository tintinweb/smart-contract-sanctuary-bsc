...
contract NewLiquidity is Operator {
    ...
    function setLock(uint256 lockReleaseCycle_, uint256 lockReleaseTimes_) external onlyOperator {
    ...
    function addLiquidityUSDT(address user, uint256 amount) external {
        ...
        uint256 halfHalf = amount.div(2);
        amount = amount.sub(halfHalf);

        uint256 amountETH = swapForEth(amount);
        ...
        USDT.safeTransfer(rewardWallet, half);
        USDT.safeTransfer(wallet, halfHalf);
        ...
        ar.exists = true;
        ar.id = addRecords[user].length - 1 + 1000;
        ar.addType = 1;
        ar.amountLP = liquidity;
        ar.amountUSDT = initAmount;
        ar.canRansomTime = block.timestamp.add(lockReleaseCycle);
        ar.startTime = block.timestamp;

        emit AddLiquidity(user, 1, ar.id, initAmount.div(2), amountETHAdd, liquidity, ar.canRansomTime);
    }

    function ransom(address user, uint256 id, uint256 amountOspd) external onlyOperator {
        id = id.sub(1000);
        require(id < addRecords[user].length, "not exists");
        require(addRecords[user][id].canRansomTime <= block.timestamp, "cannot ransom yet");
        require(addRecords[user][id].status == 0, "ransomed");

        address weth = ur.WETH();
        address pairAddr = uf.getPair(address(OSPD), weth);
        IUniswapV2Pair(pairAddr).approve(address(ur), addRecords[user][id].amountLP);
        IUniswapV2Pair(pairAddr).transferFrom(lpWallet, address(this), addRecords[user][id].amountLP);
        ur.removeLiquidityETHSupportingFeeOnTransferTokens(address(OSPD),
            addRecords[user][id].amountLP, 0, 0, address(this), block.timestamp);
        uint256 amountToken = OSPD.balanceOf(address(this)).sub(1);

        OSPD.safeTransfer(ospdAddPool, amountToken);
        OSPD.safeTransferFrom(ospdAddPool, ospdWithdrawPool, amountOspd);
        wallet.transfer(address(this).balance);

        addRecords[user][id].status = 1;
        addRecords[user][id].ransomTime = block.timestamp;
        addRecords[user][id].amountOSPD = amountOspd;

        emit Ransom(user, id.add(1000), amountOspd);
    }
    ...
    function withdraw(uint256 id, uint256 amount) external {
        ...
        uint256 lockAmount = getLockAmount_(msg.sender, id);
        ...
        emit Withdraw(msg.sender, id.add(1000), amount, lockAmount);
    }
    ...
    function getAvaliableAmount(address account, uint256 id) public view returns (uint256) {
        ...
        return addRecords[account][id].amountOSPD.sub(getLockAmount_(account, id)).sub(addRecords[account][id].amountWithdraw);
    }

    function getLockAmount(address account, uint256 id) external view returns(uint256) {
        id = id.sub(1000);
        return getLockAmount_(account, id);
    }

    function getLockAmount_(address account, uint256 id) internal view returns (uint256) {
        if (id >= addRecords[account].length) {
            return 0;
        }
        if (addRecords[account][id].status == 0) {
        ...
        uint256 startTime = addRecords[account][id].startTime;
    ...
    function swapForEth(uint256 amount) internal returns(uint256) {
        ...
        return amountETHs[amountETHs.length - 1];
    }
}
