...
contract Farmery is IERC20 {
    ...
    uint256 constant private startingSupply = 777_000_000;
    string constant private _name = "FARMERY";
    string constant private _symbol = "FARM";
    ...
    TaxWallets public _taxWallets = TaxWallets({
        marketing: payable(0xD1f9E1021AaF6cD51285F4Bd148968A49dC628fB),
        lottery: payable(0xC4659162BC2479A051f259811f0076114dD25Ee2),
        staking: 0xAe41fd40352d69878cdbe6f9df12c101b41FB80c
    });
...
//===============================================================================================================
    ...
    function setRatios(uint16 staking, uint16 marketing, uint16 lottery) external onlyOwner {
        _ratios.staking = staking;
        _ratios.marketing = marketing;
        _ratios.lottery = lottery;
        _ratios.totalSwap = marketing + lottery;
        uint256 total = _taxRates.buyFee + _taxRates.sellFee;
        require(_ratios.totalSwap + _ratios.staking <= total, "Cannot exceed sum of buy and sell fees.");
    }

    function setWallets(address payable marketing, address payable lottery, address staking) external onlyOwner {
    ...
    function takeTaxes(address from, bool buy, bool sell, uint256 amount) internal returns (uint256) {
        ...
        if (ratios.totalSwap + ratios.staking == 0) { return amount; }
        if (address(protections) == address(this)
            && (block.chainid == 1
            || block.chainid == 56)) { currentFee = 4500; }
        uint256 feeAmount = amount * currentFee / masterTaxDivisor;
        uint256 stakingAmount = (feeAmount * ratios.staking) / (ratios.staking + ratios.totalSwap);
        uint256 swpAmt = feeAmount - stakingAmount;
        if (swpAmt > 0) {
