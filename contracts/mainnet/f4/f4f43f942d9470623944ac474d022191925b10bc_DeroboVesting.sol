...
// File: contracts/DeroboVesting.sol
...
/// @notice Smart contract for vesting DEROBO tokens.

contract DeroboVesting {
    ...
    //Wallet address from which tokens are deposited.
    ...
    constructor(IDerobo _token, address _vault, uint256 _untilReleased, uint256 _steps) {
    ...
    function addTokens(address _user, uint256 _value) external returns (bool) {
        ...
        require(msg.sender == vault, "The function should only be called by the vault address");

        

        Deposit memory _newDeposit = Deposit(_value, _value, block.timestamp + untilReleased, 0);

        users[_user].push(_newDeposit);
     ...
     * @dev Sends the user all unlocked tokens from their specified deposit, if there are any.

     * @param _deposit Deposit number (1, 2, 3, etc.)

     */

    function getTokens(uint256 _deposit) external {

        require(getNumberOfDeposits(msg.sender) >= _deposit, "No such deposit");

        require(users[msg.sender][_deposit - 1].balance > 0, "The balance must be greater than zero");

        uint256 currentStep = getCurrentStep(msg.sender, _deposit);

        require(currentStep > users[msg.sender][_deposit - 1].step, "The current step must be greater than the previous user's withdrawal step");

        

        uint256 payment = getPayment(_deposit);

        users[msg.sender][_deposit - 1].balance = users[msg.sender][_deposit - 1].balance - payment;

        users[msg.sender][_deposit - 1].step = currentStep;
     ...
     * @dev Returns the current step of the user's specified deposit.
    ...
    function getCurrentStep(address _user, uint256 _deposit) public view returns (uint256) {

        require(getNumberOfDeposits(_user) >= _deposit, "No such deposit");

        require(users[_user][_deposit - 1].deposit > 0, "The deposit must be greater than zero");

        

        uint256 step;

        if (users[_user][_deposit - 1].releaseTime <= block.timestamp) {

            step = (block.timestamp - users[_user][_deposit - 1].releaseTime) / RELEASE_STEP;
     ...
     * @dev Returns the number of tokens that the sender can currently withdraw from the specified deposit.

     * @param _deposit Deposit number (1, 2, 3, etc.)

     */

    function getPayment(uint256 _deposit) public view returns(uint256) {
        ...
        if (currentStep == numberOfSteps) {
            ...
            _payment = users[msg.sender][_deposit - 1].balance;

        } else {

            _payment = _valueFromPercent(users[msg.sender][_deposit - 1].deposit, stepPercent * (currentStep - users[msg.sender][_deposit - 1].step));
     ...
     * @dev Returns user's specified deposit's amount, balance, previous withdrawal step and unix timestamp of unlocking funds.
    ...
    function getUser(address _user, uint256 _deposit) public view returns(uint256, uint256, uint256, uint256) {

        return (users[_user][_deposit - 1].deposit, users[_user][_deposit - 1].balance, users[_user][_deposit - 1].step, users[_user][_deposit - 1].releaseTime);
     ...
     * @dev Returns user's number of deposits.

     * @param _user The user's address.

     */

    function getNumberOfDeposits(address _user) public view returns(uint256) {
