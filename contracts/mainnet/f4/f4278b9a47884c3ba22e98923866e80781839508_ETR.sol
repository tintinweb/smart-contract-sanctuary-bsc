...
contract ETR is ERC20, Ownable {
    ...
    ETRDividendTracker public dividendTracker;
    ...
    uint256 public maxTransactionAmount = 5 * (10**6) * (10**18); //1%

    uint256 public _BuyETHRewardsFee = 4;
    uint256 public _BuyLiquidityFee = 1;
    uint256 public _BuyMarketingFee = 4;
    uint256 public _BuyBackETRFee = 4;
  
    uint256 public _BuyTotalFees = _BuyETHRewardsFee.add(_BuyLiquidityFee).add(_BuyMarketingFee).add(_BuyBackETRFee);
    
    uint256 public _SellETHRewardsFee = 6;
    uint256 public _SellLiquidityFee = 1;
    uint256 public _SellMarketingFee = 4;
    uint256 public _SellBackETRFee = 4;
    uint256 public _SellTotalFees = _SellETHRewardsFee.add(_SellLiquidityFee).add(_SellMarketingFee).add(_SellBackETRFee);
    
    uint256 public maxWalletToken = 5 * (10**6) * (10**18); //1%

    address public _marketingWalletAddress = 0xD12Fd8A49A2b9bF625E296ae9dF9D8e4046D5D13;
    address public _ETRWalletAddress = 0x528Cf9f06596b83665e114b06e5bA157883Fc87D;
    ...
    constructor() ERC20("Ethereal", "$ETR") {

    	dividendTracker = new ETRDividendTracker();
        ...
        dividendTracker.excludeFromDividends(_ETRWalletAddress);
        ...
        excludeOrIncludeFromFees(_ETRWalletAddress, true);
        ...
        excludeOrIncludeFromMaxTx(_ETRWalletAddress, true);
        ...
        excludeOrIncludeFromMaxWallet(_ETRWalletAddress, true);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "ETR: The dividend tracker already has that address");

        ETRDividendTracker newDividendTracker = ETRDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "ETR: The new dividend tracker must be owned by the ETR token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "ETR: The router already has that address");
    ...
    function excludeOrIncludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ETR: Account is already the value of 'excluded'");
    ...
    {
        require(_isExcludedFromMaxTx[account] != excluded, "ETR: Account is already the value of 'excluded'");
    ...
    {
        require(_isExcludedFromMaxWallet[account] != excluded, "ETR: Account is already the value of 'excluded'");
    ...
    function setETRWallet(address payable wallet) external onlyOwner{
         require(wallet != address(0), "Invalid address");
        _ETRWalletAddress = wallet;
    }

      function setBuyFees(uint256 buyEthRewardFee, uint256 buyLiquidityFee, uint256 buyMarketingFee, uint256 buyBackETRFee) external onlyOwner
    {
        _BuyETHRewardsFee = buyEthRewardFee;
        _BuyLiquidityFee = buyLiquidityFee;
        _BuyMarketingFee = buyMarketingFee;
        _BuyBackETRFee = buyBackETRFee;
        _BuyTotalFees = _BuyETHRewardsFee.add(_BuyLiquidityFee).add(_BuyMarketingFee).add(_BuyBackETRFee);

    }

    function setSellFees(uint256 sellEthRewardFee, uint256 sellLiquidityFee, uint256 sellMarketingFee, uint256 sellBackETRFee) external onlyOwner
    {
        _SellETHRewardsFee = sellEthRewardFee;
        _SellLiquidityFee = sellLiquidityFee;
        _SellMarketingFee = sellMarketingFee;
        _SellBackETRFee = sellBackETRFee;
        _SellTotalFees = _SellETHRewardsFee.add(_SellLiquidityFee).add(_SellMarketingFee).add(_SellBackETRFee);

    }
      function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ETR: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 800000, "ETR: gasForProcessing must be between 200,000 and 800,000");
        require(newValue != gasForProcessing, "ETR: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        
        {
            ...
            uint256 walletTokens = contractTokenBalance.mul(_SellMarketingFee.add(_SellBackETRFee)).div(_SellTotalFees);
            uint256 contractBalance = address(this).balance;
            swapTokensForEth(walletTokens);
            uint256 newBalance = address(this).balance.sub(contractBalance);
            uint256 marketingShare = newBalance.mul(_SellMarketingFee).div(_SellBackETRFee.add(_SellMarketingFee));
            uint256 developmentShare = newBalance.sub(marketingShare);
            payable(_marketingWalletAddress).transfer(marketingShare);
            payable(_ETRWalletAddress).transfer(developmentShare);
...
contract ETRDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("ETR_Dividend_Tracker", "ETR_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 200000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "ETR_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "ETR_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ETR contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "ETR_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "ETR_Dividend_Tracker: Cannot update claimWait to same value");
