...
contract CALottery is ReentrancyGuard, ICALottery, Ownable {
    ...
    uint256 public pendingAmountToWinners;
    ...
    function claimTickets(
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInBusdToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        busdToken.safeTransfer(msg.sender, rewardInBusdToTransfer);
        if(pendingAmountToWinners >= rewardInBusdToTransfer)
            pendingAmountToWinners -= rewardInBusdToTransfer;
        else
            pendingAmountToWinners = 0;

        emit TicketsClaim(msg.sender, rewardInBusdToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].busdPerBracket[j] =
     ...
     * @param _priceTicket price of a ticket (in busd)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].busdPerBracket[_bracket];
        } else {
            return 0;
        }
    }
...
interface ICALottery {
    ...
    /**
     * @notice Draw the final number, calculate reward in BPT per group, and make lottery claimable
     * @param _lotteryId: lottery id
     * @param _autoInjection: reinjects funds into next lottery (vs. withdrawing all)
     * @dev Callable by operator
     */
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in BPT token
     * @dev Callable by operator
     */
     ...
     * @dev Callable by operator
     * @param _endTime: endTime of the lottery
     * @param _priceTicketInBpt: price of a ticket in BPT
     * @param _discountDivisor: the divisor to calculate the discount magnitude for bulks
     * @param _rewardsBreakdown: breakdown of rewards per bracket (must sum to 10,000)
     * @param _treasuryFee: treasury fee (10,000 = 100%, 100 = 1%)
     */
