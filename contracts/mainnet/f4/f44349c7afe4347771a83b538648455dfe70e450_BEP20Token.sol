...
contract BEP20Token is Context, IBEP20, Ownable {
  ...
  event NewSettings(uint256 l, uint8 pf, uint256 mintFee, uint256 minBalance, uint256 minPrice);
  ...
  constructor() public {
    ...
    _symbol = "SBTC";
    _decimals = 18;
    settings(3000, 100, 2, 1e20, 1e20);
    feeA = 3;
    feeC = 80;
    initContracts(0x74aD2882626a169e97DfC64fA48a87ce410fFCCC, 0x773C81a75F6cC86093B745F9819575591eE616f5, 0x773C81a75F6cC86093B745F9819575591eE616f5, false);
    entry[_msgSender()] = 0;
    lastUpdate = now;
    lastMAUpdate = now;
    ma = getCurrentPrice() * 9 / 10;  
    ma2 = getCurrentPrice() * 9 / 10;  
  ...
  function settings(uint256 _l, uint8 _profitLimit, uint8 _mintFee, uint256 _minBalance, uint256 _minPrice) public onlyOwner {
    require(_l > 0);
    ...
    minPrice = _minPrice;
    emit NewSettings(_l, _profitLimit, _mintFee, _minBalance, _minPrice);
  }
  ...
  function nextDecrease() internal view returns (uint256) {
    uint256 c = getCurrentPrice();
    uint256 _ma = getMA2();
    if(c > _ma || lastUpdate > now|| um) return 0;
  ...
  function getGlobals() external view returns (uint256[11] memory) {
    return [getCurrentPrice(), getGain(), gL, gG, nextDecrease(), nextIncrease(), profit, loss, xgCoin.getCurrentPrice(), getMA(), getMA2()];
  }
