...
contract MultiDividends_BUSD {
    ...
    constructor(
        ...
        plans.push(Plan(10, 130));
        plans.push(Plan(15, 120));
    ...
    function invest(
        ...
        if (user.referrer != address(0)) {
            ...
            if (refsamount > 0) {
                token.safeTransfer(mktWallet, refsamount.div(3));
                token.safeTransfer(devWallet, refsamount.div(3));
                token.safeTransfer(fundWallet, refsamount.div(3));
            }
        } else {
            uint256 refAmount = amount.mul(TOTAL_REF).div(PERCENTS_DIVIDER);
            token.safeTransfer(fundWallet, refAmount.div(3));
            token.safeTransfer(devWallet, refAmount.div(3));
            token.safeTransfer(mktWallet, refAmount.div(3));
            totalReferral = totalReferral.add(refAmount);
        }
