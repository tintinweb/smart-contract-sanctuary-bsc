...
contract RabitKing is Context, IERC20, Ownable {
    ...
    function _whitelist_release_time_init() internal {
        ...
        whitelistNum = 30;
        require(
            whitelistNum.mul(whitelistPresaleAmount) <= totalPresaleNum,
            "Whitelist: presale nums  execced max amount"
        );

        whitelistReleaseTime[0] = uint256(1672970400);
    ...
    function addWhitelist(address account) external {
        ...
        if (
            whitelistAddress.length < whitelistNum &&
            balanceOf(address(this)) >= whitelistPresaleAmount
        ) {
            _tokenTransfer(
                address(this),
                account,
