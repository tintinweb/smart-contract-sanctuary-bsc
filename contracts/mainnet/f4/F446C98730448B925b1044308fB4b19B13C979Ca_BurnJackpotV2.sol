...
contract BurnJackpotV2 is Ownable, Pausable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    uint256 public jackpotBalance;
    uint256 public priceOfTicket = 0.015 ether;
    uint256 public soldTickets;
    uint256 public percentageToBurn = 50;
    uint256 private howManyWinners = 2;
    ...
    address public tefi = 0xD23a8017B014cB3C461a80D1ED9EC8164c3f7A77;
    ...
    function participate() external payable {
        uint amount = msg.value / priceOfTicket;
        require (amount > 0, "can't buy ticket");
        _participate(amount);
    }

    function _participate(uint amount) internal whenNotPaused nonReentrant {
        require(canParticipateUntil >= block.timestamp, "Too late");
        jackpotBalance += msg.value;
        soldTickets += amount;
        for(uint256 i = 0; i < amount; i++) jackpotPlayers.push(msg.sender);
    }
    ...
    function sendJackpotToWinners() external onlyOwner {
        ...
        uint burnAmount = curBalance.mul(percentageToBurn).div(100);

        address[] memory path = new address[](2);
        path[0] = router.WETH();
        path[1] = tefi;

        router.swapExactETHForTokens{value: burnAmount}(
            0,
            path,
            DEAD,
            block.timestamp
        ...
        for(uint256 i = 0; i < winners.length; i++){
            payable(winners[i]).call{
                value: curBalance.div(winners.length),
                gas: 30000
            }("");
        }
    }

    function newJackpot(uint256 openForHowManyHours) external onlyOwner {
    ...
    function setBurnPercentage(uint _percent) external onlyOwner {
        percentageToBurn = _percent;
    }

    function setWinnersLimit(uint _limit) external onlyOwner {
        howManyWinners = _limit;
    }
