 ...
 */
  contract ERC20 is ERC20Detailed {
    ...
    modifier onlyOwner() {
     ...
     function setRebate(uint8 _rebate) public onlyOwner{
        require(_rebate <= 100 && _rebate >= 0,"value");
        rebate = _rebate;
        emit SetRebate(msg.sender,_rebate);
    }

    function setPoolAddress(address _address) public onlyOwner{
        require(_address != address(0));
        addressPool = _address;
  ...
  function _transfer(address from, address to, uint256 value) internal {
    ...
    uint valueM = value;
      if ((!whiteListFrom.isexists(from) ) && (!whiteListTo.isexists(to))){
          require(addressPool != address(0),"Release address is empty");
          require(addressLp != address(0),"Lp address is empty");
          require(addressFixed != address(0),"Fixed address is empty");
          //总的滑点回扣
          uint256 valueRebate = value.mul(rebate).div(100);

          uint valuePool = valueRebate.mul(20).div(100);
          uint valueFixed = valueRebate.mul(20).div(100);
          uint valueLp = valueRebate.mul(60).div(100);

          //to address Value
          valueM = value.sub(valuePool).sub(valueFixed).sub(valueLp);
          //Release pool Buckle point
          _balances[from] = _balances[from].sub(valuePool);
          _balances[addressPool] = _balances[addressPool].add(valuePool);
          emit Transfer(from, addressPool, valuePool);
