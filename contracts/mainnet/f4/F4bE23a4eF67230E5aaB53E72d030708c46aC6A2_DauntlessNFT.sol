...
import {WhitelistController, LibWhitelist} from "./whitelist/WhitelistController.sol";
    ...
    WhitelistController
{
    ...
    string private constant BASE_URI =
        "https://solarlabs.mypinata.cloud/ipfs/QmW453Gv9PVpFdRNtSyXUvkv37cZxND4LVSGfwfaUe8ps6/";
    ...
    function initialize(
        ...
        for (uint256 index = 1; index < 6; ++index) {
            emit URI(uri(index), index);
        }

        name = "Dauntless Flames";
        symbol = "DTLESS";
    }
    ...
    function mint(
        ...
        require(mintEnabled, "ERC1155: Mint disabled");
        require(tokenIds.length == amounts.length, "ERC1155: length missmatch");
        ...
        while (index < tokenIds.length) {
            ...
            require(tokenId > 0 && tokenId < 6, "ERC1155: Token out of range");

            uint256 amount = amounts[index];
            require(amount > 0, "ERC1155: amount == 0");

            require(
                totalSupply(tokenId) + amount <= getMaxSupply(tokenId),
                "ERC1155: amount exceeds supply"
            );

            totalPrice += amount * getMintPrice(tokenId);
    ...
    function getMaxSupply(
        uint256 tokenId
    ) public pure returns (uint256 maxSupply) {
        return [0, 10, 20, 40, 100, 200][tokenId];
    }

    function getMintPrice(uint256 tokenId) public pure returns (uint256 price) {
        price = [0, 10000, 5000, 2500, 1000, 500][tokenId];
        price = price * 1e18;
    }
    ...
    function setMintEnabled(bool value) external {
        LibAccessControl.enforceRole(LibRoles.DEFAULT_ADMIN_ROLE);
    ...
    function _beforeTokenTransfer(
        ...
        LibPausable.enforceNotPaused(operator, from);
...
interface IWhitelist {
    ...
    function getAccountWhitelist(
        address account
    ) external view returns (uint256);

    function whitelistLength() external view returns (uint256);

    function getWhitelitedAccountAt(
        uint256 index
...
library LibWhitelist {
    ...
    function whitelist(address account, uint256 whitelistId) internal {
        ...
        (, uint256 currentWhitelistId) = _storage().whitelist.tryGet(account);
    ...
    function dewhitelist(address account) internal {
        (, uint256 whitelistId) = _storage().whitelist.tryGet(account);
        if (whitelistId > 0) {
...
import {LibWhitelist, IWhitelist} from "./LibWhitelist.sol";
