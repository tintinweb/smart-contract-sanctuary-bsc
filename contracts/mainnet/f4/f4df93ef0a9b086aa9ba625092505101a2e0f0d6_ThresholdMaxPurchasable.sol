...
contract ThresholdMaxPurchasable is ExternalEligibilityContract {
    ...
    uint public exclusive_buy_deadline_timestamp;               // Expiration of the exclusive buy possibility for NFT pass holders
    uint8 public max_purchasable_nopass;                        // Max number of purchasable items/packages (with same ID) for users not holding the NFT pass
    uint8 public max_purchasable_withpass;                      // Max number of purchasable items/packages (with same ID) for users holding the NFT pass 
    bool public only_whitelisted_mode;                          // True if this smart contract is configured for whitelisted mode (only whitelisted addresses can buy)
    address public whitelist_manager;                           // Other Wallet that can add users to the whitelist (Powermade owner always can do it) 
    mapping(address => bool) public whitelist;                  // Whitelist used only when deployed in whitelisted mode
    ...
    constructor(
        ...
        address _BUSD_Token, 
        address _NFTpass, 
        uint _exclusive_buy_deadline_timestamp, 
        uint8 _max_purchasable_nopass,
        uint8 _max_purchasable_withpass,
    ...
    public {
        ...
        exclusive_buy_deadline_timestamp = _exclusive_buy_deadline_timestamp;
        max_purchasable_nopass = _max_purchasable_nopass;
        max_purchasable_withpass = _max_purchasable_withpass;
    ...
    function changeWhitelistManager(address _whitelist_manager) external onlyPowermadeOwner {
        require(only_whitelisted_mode, "Whitelist not enabled");
    ...
    function isEligibleExtW(address user_address, address, uint16, uint level, uint8 sourceID) public view returns (bool is_eligible) {
        if (sourceID == 1) {        // Case Package Buy
            ...
            if (block.timestamp <= exclusive_buy_deadline_timestamp && !isNFTpass) {
                return false;
            }
            if (!isNFTpass && level < max_purchasable_nopass) {
                return true;
            } else if (isNFTpass && level < max_purchasable_withpass) {
                return true;
            } else {
                return false;
            }
    ...
    function isEligibleExt(address user_address, address, uint16, uint level, uint8 sourceID) external onlyPowermadeContract returns (bool is_eligible) {
        ...
        if (sourceID == 1) {        // Case Package Buy
            ...
            bool isNFTpass = address(NFTpass) == address(0) ? false : NFTpass.balanceOf(user_address) >= 1;
            // Exclusive buy period
            if (block.timestamp <= exclusive_buy_deadline_timestamp) {
            ...
            if (!isNFTpass) {
                if (max_purchasable_nopass == 0) {
                    revert("Buy Disabled");
                }
                require(level < max_purchasable_nopass, "Max Purchasable quantity reached");
            } else {
                if (max_purchasable_withpass == 0) {
                    revert("Buy Disabled");
                }
                require(level < max_purchasable_withpass, "Max Purchasable quantity reached");
            }
            return true;
        } else if (sourceID == 2) {     // Case sponsor commission (direct)
            return true;
