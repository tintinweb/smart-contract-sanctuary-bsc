...
// File: pexlotto_locked_staking.sol
contract PEXLottoStaker is Ownable {
    using SafeMath for uint256;
    ...
    rewardToken: The address of Reward token contract.
    ...
    */ 
    ...
    string constant public lockPeriod = '5 days';  // locked period in text to be shown by the GUI
    uint256 private contractLockPeriod = 432000; // 5 days in seconds 
    address private unusedRewardAddress; // address for withdrawal of the unused rewards # TODO: remove
    uint256 private claimableTime = 0;
    ...
    // TODO: remove emergency withdraw
    ...
    constructor(
        ...
        */ 
        ...
        // initialize 
        unusedRewardAddress = address(0);
        claimableTime = block.timestamp + 259200;// 3 Days = 60*60*24*3
    }
    ...
    */
    function getUnusedRewardAddress() public view onlyOwner returns(address) {
        return unusedRewardAddress;
    }

    /*
    * set wallet address to withdraw unused rewards
    */
    function setUnusedRewardAddress(address _unusedRewardAddress) public onlyOwner {
        unusedRewardAddress = _unusedRewardAddress;
    }

    /*
    * returns total number of staked token
    */
    function getTotalStakedTokenBalance() public view returns(uint256) {
    ...
    */
    function getTotalRewardTokenBalance() public view returns(uint256) {
        return poolInfo.rewardToken.balanceOf(address(this));
    }

    /*
    * returns total reward debt
    */
    function getTotalRewardDebt() public view returns(uint256) {
        return poolInfo.poolTotalRewardDebt;
    }
    ...
    */
    ...
    /*
    * Returns the remaining lock time before claimis activated for all users
    */
    function remainClaimLockTime() public view returns(uint256) {
        uint256 remain;

        if (claimableTime >= block.timestamp) {
            remain = claimableTime.sub(block.timestamp);
        } else {
            remain = 0;
        }
    ...
    * This is necessary on any change to any user blance i.e. (deposite, withdrawal)
    ...
    */
    function _MassUpdate() internal {
        ...
        /* Calculate only when:
        * - totalStaked token is larger than 0
        * - time elapsed since last calculation is larger than 0
        * - #TODO: double check time elapsed condition
        */

        uint256 totalBalance = getTotalStakedTokenBalance();

        if (totalBalance > 0 && block.timestamp.sub(pool.lastMassUpdate) > 0) {
            for (uint256 i = 1; i <= userCount; i++) {
                reward = calculateClaimableReward(userMapping[i], block.timestamp);
            ...
            pool.lastMassUpdate = block.timestamp;
            pool.poolTotalRewardDebt = poolTotalRewardDebt;
        } else if (totalBalance == 0) { // Add To Set lastMassUpdate for First Time
            pool.lastMassUpdate = block.timestamp;
        }
    }
    ...
    */
    function calculateClaimableReward(address _user, uint256 _calculationTime)
        public
        view
        returns (uint256)
    {
        uint256 totalStakedTokenBalance = getTotalStakedTokenBalance();
        require(totalStakedTokenBalance > 0, "Rewards calculation failed due to zero total staked token!");
        ...
        if (_calculationTime > poolInfo.lastMassUpdate) {
            durationSecond = _calculationTime.sub(poolInfo.lastMassUpdate);
            durationCount = durationSecond.div(rewardDuration); 
            rewardTokenCount = durationCount.mul(poolInfo.periodRewardTokenCount); 
        }

        uint256 userOwnershipPercent = user.amount.mul(calculationFactor).div(totalStakedTokenBalance);
        uint256 userReward = userOwnershipPercent.mul(rewardTokenCount).div(calculationFactor);
    ...
    * Triggers _MassUpdate to ensure correct calculation of earned rewards.
    * Important! Make sure to pass amount in wei  (10**18)
    */
    function withdraw(uint256 _amount) public {
        ...
        require(remainLock <= 0, "withdrawal failed: locktime remains!");
        require(user.amount >= _amount, "withdrawal failed: the requested amount exceeds the balance!");
    ...
    */
    function claim() external {
        require(block.timestamp > claimableTime, "claim: The time of claim has not yet come");

        // update rewards for all users before the claim.
        _MassUpdate();

        // make sure there is enough reward token in the contarct
         UserInfo storage user = userInfo[msg.sender];
        require(user.rewardEarned < getTotalRewardTokenBalance(), 'claim: Insufficient reward token in the contract');
        
        // transfer the reward
        if (user.rewardEarned > 0) {
            poolInfo.rewardToken.safeTransfer(msg.sender, user.rewardEarned*10**18);
        }
        user.rewardEarned = 0;
    }
