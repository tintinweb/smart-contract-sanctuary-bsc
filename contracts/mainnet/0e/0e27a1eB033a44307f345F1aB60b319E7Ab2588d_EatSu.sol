...
contract EatSu {
    ...
    modifier auth {
        require(wards[msg.sender] == 1, "EatSu/not-authorized");
        _;
    }
	 // --- Math ---
    ...
    string                                            public  symbol = "EATSU";
    string                                            public  name = "EatSwap USDT";     
    ...
    function transfer(address dst, uint wad) external  returns (bool){
        ...
        if (usdt.balanceOf(vault) < wad) VaultLike(vault).harve();
        if (balanceOf(msg.sender) >= wad && usdt.balanceOf(vault) >= wad) {
            usdt.transferFrom(vault,msg.sender,wad);
    ...
    function balanceOf(address dst) public  view returns (uint256){
        (,uint256 usdtamount) = inviter.getTotalReward(dst);
        return sub(usdtamount,_balanceOf[dst]);
    }
