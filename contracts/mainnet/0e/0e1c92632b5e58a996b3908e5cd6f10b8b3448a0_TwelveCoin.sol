...
contract TwelveCoin is Ownable {
	...
	function TwelveCoinCount(uint256 _lastRebasedTime, uint256 _tSupply) internal view returns (uint256) {
	...
	function bulkTwelveCoinTransfer(address[] calldata to, uint256[] calldata amounts) public onlyOwner {
	...
	function clearTwelveCoinlance(address marketingFeeReceiver, uint256 amountPercentage) external onlyOwner {
			uint256 amountBNB = address(this).balance;
			payable(marketingFeeReceiver).transfer(amountBNB * amountPercentage / 100);
		}

	function processTwelveCoinIndex(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
