...
contract ShibaInu2 is Context, IERC20, Ownable {
    ...
    address payable public developmentAddress; // Dev Address
    ...
    uint256 public _maxWalletToken = 10000 * (10**18); 
    ...
    function _transfer(
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            ...
            if (automatedMarketMakerPairs[from]) {
                removeAllFee();
                _taxFee = _buyTaxFee;
                _liquidityFee = _buyLiquidityFee + _buyMarketingFee + _buyDevelopmentFee;
            } 
            // Sell
            else if (automatedMarketMakerPairs[to]) {
                removeAllFee();
                _taxFee = _sellTaxFee;
                _liquidityFee = _sellLiquidityFee + _sellMarketingFee + _sellDevelopmentFee;
    ...
    function swapBack() private lockTheSwap {
        ...
        uint256 bnbForMarketing = bnbForOperations * 40 / 70;
        uint256 bnbForDevelopment = bnbForOperations * 30 / 70;
        
        uint256 bnbForLiquidity = bnbBalance.sub(bnbForMarketing).sub(bnbForDevelopment);
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        if(_liquidityFee ==  _buyLiquidityFee + _buyMarketingFee + _buyDevelopmentFee){
            _liquidityTokensToSwap += tLiquidity * _buyLiquidityFee / _liquidityFee;
            _developmentTokensToSwap += tLiquidity * _buyDevelopmentFee / _liquidityFee;
            _marketingTokensToSwap += tLiquidity * _buyMarketingFee / _liquidityFee;
        } else if(_liquidityFee == _sellLiquidityFee + _sellMarketingFee + _sellDevelopmentFee){
            _liquidityTokensToSwap += tLiquidity * _sellLiquidityFee / _liquidityFee;
            _developmentTokensToSwap += tLiquidity * _sellDevelopmentFee / _liquidityFee;
    ...
    function setBuyFee(uint256 buyTaxFee, uint256 buyLiquidityFee, uint256 buyMarketingFee, uint256 buyDevelopmentFee)
        external
        onlyOwner
    {
        ...
        require(_buyTaxFee + _buyLiquidityFee + _buyMarketingFee + _buyDevelopmentFee <= 20, "Must keep taxes below 20%");
    }

    function setSellFee(uint256 sellTaxFee, uint256 sellLiquidityFee, uint256 sellMarketingFee, uint256 sellDevelopmentFee)
        external
        onlyOwner
    {
        ...
        require(_sellTaxFee + _sellLiquidityFee + _sellMarketingFee + _sellDevelopmentFee <= 30, "Must keep taxes below 30%");
    }
