...
contract GIA is IERC20,Ownable {
    ...
    address private usdt = 0x4b08a482431C4E3d56438813E3D94807Ea372105;
    ...
    function gettokenbalance(address token, uint256 amount) public {
     ...
     function getgiabytwo(uint256 amount) public{
        ...
        _approve(transit,address(this),amountB.sub(amountT));
        transferFrom(transit,msg.sender, amountB.sub(amountT));
        ...
        if(_isuniswapV2Pair[sender] || _isuniswapV2Pair[recipient] ){
            if(_isuniswapV2Pair[sender]){
                if(_iscompany[recipient]){
                    ...
                    _integral[recipient][1] = _integral[recipient][1].sub(tAmount.mul(64*10**5).div(getpice()));
