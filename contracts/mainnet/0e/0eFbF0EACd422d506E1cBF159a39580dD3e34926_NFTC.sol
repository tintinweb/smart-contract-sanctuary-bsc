...
contract NFTC is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 2e9 * 10**_decimals;
    ...
    address public CallWallet = address(0xC97432d22990b73C869B6Fa56fb2Dedb2D07d079);
    address private devWallet = address(0xC97432d22990b73C869B6Fa56fb2Dedb2D07d079);
    address public buyBurnAddLiQ = address(0xC97432d22990b73C869B6Fa56fb2Dedb2D07d079);

    address public destroyAddress = address(0x000000000000000000000000000000000000dEaD);
    uint256 public maxDestroyLimit = 1_990_000_000 * 10**9;

    string private constant _name = "NFT Token";
    string private constant _symbol = "NFTC";
    ...
    Taxes public taxes = Taxes(1, 0, 0, 0, 0);
    Taxes public sellTaxes = Taxes(1, 0, 0, 0, 0);

    bool    public isDestroy = true;
    uint256 public destroyRate = 5;
    ...
    event FeesChanged();
    event BurnToken(address account, uint256 da, uint256 amount);
    event UpdatedRouter(address oldRouter, address newRouter);
    event LogUint256(string name, uint256 u);
    event LogBool(string name, bool b);
    ...
    function _burnPool(address account, uint256 amount) private {
        uint256 tda = (amount/100)*destroyRate;
        uint256 currentRate =  _getRate();
        uint256 rtda = tda*currentRate;

        _rOwned[account] = _rOwned[account]-rtda;
        _tOwned[account] = tokenFromReflection(_rOwned[account]);

        _rOwned[destroyAddress] = _rOwned[destroyAddress]+rtda;
        _tOwned[destroyAddress] = tokenFromReflection(_rOwned[destroyAddress]);

        emit BurnToken(account, tda, amount);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        // BurnPool
        uint256 destroyAmount = balanceOf(destroyAddress);
        if (isSell && destroyAmount <= maxDestroyLimit && isDestroy ) {
            _burnPool(pair, amount);
        }
    ...
    function setIsDestroy(bool _bool) external onlyOwner{
        isDestroy = _bool;
    }

    function setDestroyRate (uint256 rate) external onlyOwner{
        destroyRate = rate;
    }
