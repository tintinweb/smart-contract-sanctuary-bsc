...
contract TEST is Context, IERC20, Ownable {
    ...
    uint256 public maxBuyLimit = _tTotal * 3 / 100;
    uint256 public maxSellLimit = _tTotal * 3 / 100;
    uint256 public maxWalletLimit = _tTotal * 3 / 66;
    
    uint256 public genesis_block;
    uint256 private deadline;
    
    address public marketingWallet = 0x5ca6c0cEE9bf1213F9413875243aE54Df7b1b0c1;
    address private operationsWallet = 0x5ca6c0cEE9bf1213F9413875243aE54Df7b1b0c1;
    ...
    Taxes public buyTaxes = Taxes(5, 1, 1);
    ...
    function isExcludedFromReward(address account) public view returns (bool) {
     ...
     function EnableTrading(
        ...
        require (_deadline <= 5, "Deadline cannot exceed 5 Blocks"); 
        deadline = _deadline;
        launchtax = Taxes(marketing, liquidity, operations);
    ...
    function setBuyTaxes(uint256 _marketing, uint256 _liquidity, uint256 _operations) public onlyOwner {
        require((_marketing + _liquidity + _operations) <= 20, "Fees can't exceed 20%");
        buyTaxes = Taxes(_marketing,_liquidity,_operations);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _marketing, uint256 _liquidity, uint256 _operations) public onlyOwner {
        require((_marketing + _liquidity + _operations) <= 20, "Fees can't exceed 20%");
        sellTaxes = Taxes(_marketing,_liquidity,_operations);
        emit FeesChanged();
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell, bool useLaunchTax) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.roperations) = _getRValues2(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell, bool useLaunchTax) private view returns (valuesFromGetValues memory s) {
        ...
        s.tTransferAmount = tAmount-s.tMarketing-s.tLiquidity-s.toperations;
        return s;
    }
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount,uint256 rMarketing, uint256 rLiquidity){
        ...
        rTransferAmount =  rAmount-rMarketing-rLiquidity-roperations;
        return (rAmount, rTransferAmount,rMarketing,rLiquidity);
    }
    
    function _getRValues2(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 roperations) {
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        require(maxBuy >= _tTotal * 1 / 1000, "Cannot set max buy amount lower than 0.1% of total supply");
        require(maxSell >= _tTotal * 1 / 1000, "Cannot set max sell amount lower than 0.1% of total supply");
        maxBuyLimit = maxBuy * 10**decimals();
        maxSellLimit = maxSell * 10**decimals();
    }
    
    function updateMaxWalletlimit(uint256 maxWallet) external onlyOwner{
        require(maxWallet >= _tTotal * 1 / 400, "Cannot set max wallet amount lower than 0.25% of total supply");
