...
contract NftFarmV3 is SafeOwnable, ReentrancyGuard, ERC1155Holder {
    ...
    mapping(address => mapping (uint256 => bool)) public disableNftIds;
    ...
    function addNftIds(address _nft, uint[] memory _ids) external onlyOwner {
        ...
        for (uint i = 0; i < _ids.length; i ++) {
            require(!disableNftIds[_nft][_ids[i]], "id already exist");
            disableNftIds[_nft][_ids[i]] = true;
    ...
    function delNftIds(address _nft, uint[] memory _ids) external onlyOwner {
        for (uint i = 0; i < _ids.length; i ++) {
            require(disableNftIds[_nft][_ids[i]], "id not exist");
            delete disableNftIds[_nft][_ids[i]];
    ...
    modifier onlySupportedNftIds(address _nft, uint[] memory _ids) {
        ...
        for (uint i = 0; i < _ids.length; i ++) {
            require(!disableNftIds[_nft][_ids[i]], "nft not supported");
        }
        _;
    }
