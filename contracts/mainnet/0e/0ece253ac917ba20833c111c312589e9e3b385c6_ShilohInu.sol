...
contract ShilohInu is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ShilohInuDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Shiloh Inu", "Shiloh") {
        marketingWallet = payable(0xB6aCf5c8a75b548c75C54bE4bAba4e9b0C2CA121);
        devWallet = payable(0xBbFFad4060E6F2648214a5cf3eAa088b8EC6eA31);
        ...
        dividendTracker = new ShilohInuDividendTracker(
            payable(this),
            router,
        ...
        maxWallet = totalTokenSupply / 10000; // 0.005%
        swapTokensAtAmount = totalTokenSupply / 1000000; // 0.05%;
    ...
    function setmaxWallet(uint256 value) external onlyOwner {
        value = value * (10**18);
        require(value >= _totalSupply / 10000, "max wallet cannot be set to less than 0.05%");
        maxWallet = value;
    }
...
contract ShilohInuDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    ShilohInu public ShilohInuContract;
    ...
    constructor(
        ...
        defaultToken = token;
        ShilohInuContract = ShilohInu(mainContract);
    ...
    function _transfer(
        ...
        require(false, "ShilohInu_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "ShilohInu_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ShilohInucontract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(ShilohInuContract);

            uint256 prevBalance = ShilohInuContract.balanceOf(address(this));
            ...
            {
                uint256 received = ShilohInuContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    ShilohInuContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
