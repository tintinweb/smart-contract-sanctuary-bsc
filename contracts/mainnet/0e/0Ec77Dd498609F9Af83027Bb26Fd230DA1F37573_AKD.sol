...
contract AKD is ERC20Upgradeable, OwnableUpgradeable {
    ...
    function initialize() external initializer {
        __ERC20_init_unchained("AKD Token", "AKD");
        __Context_init_unchained();
        __Ownable_init_unchained();
        claimWait = 3600;
        _mint(msg.sender, 98000000 ether);
        sellFee = 8;
        buyFee = 8;
    ...
    function transferFrom(
        ...
        if (!whiteList[msg.sender] && !whiteList[recipient] && !whiteList[sender]) {
            ...
            if (msg.sender == address(router) || recipient == pair || sender == pair) {
                ...
                _transfer(sender, address(this), temp * 3 / 8);
                SendDividends(temp * 3 / 8);
                amount -= temp;
                uint left = temp * 5 / 8;
                ...
                for (uint i = 0; i < 2; i++) {
                    ...
                    _transfer(sender, tempAddress, temp * feeRate[i] / 8);
                    left -= temp * feeRate[i] / 8;
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        if (!whiteList[msg.sender] && !whiteList[recipient]) {
            if (recipient == pair || msg.sender == pair) {
                ...
                _transfer(msg.sender, fund, temp / 8);
                if (totalSupply() - balanceOf(address(0)) <= 980000 ether) {

                    amount -= amount * (buyFee - 2) / 100;
                } else {
                    _transfer(msg.sender, address(0), temp / 4);
                    amount -= temp;
                }
                ...
                for (uint i = 0; i < 2; i++) {
                    ...
                    _transfer(msg.sender, tempAddress, temp * feeRate[i] / 8);
                    left -= temp * feeRate[i] / 8;
                    tempAddress = invitor[tempAddress];
                }
            } else {
