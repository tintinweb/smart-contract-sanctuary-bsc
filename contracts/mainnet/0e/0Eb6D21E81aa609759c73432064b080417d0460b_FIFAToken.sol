...
contract FIFAToken is Context, IBEP20, Ownable {
    ...
    function setFees(uint256 bnbFee) external onlyOwner() {
        require(_BNBFee != bnbFee);
        _BNBFee = bnbFee;
        emit UpdateFees(bnbFee, 0);
    }
    ...
    function _getFeeValues(uint256 tAmount) private view returns (uint256) {
        uint256 fee = tAmount.mul(_BNBFee).div(10**2);
    ...
    function swapAndCharge(uint256 tokenBalance) private lockTheSwap {
        ...
        emit SwapAndCharged(tokenBalance, 0, address(this).balance, 0);
    }
