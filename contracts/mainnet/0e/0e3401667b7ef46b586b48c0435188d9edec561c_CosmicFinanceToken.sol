...
contract CosmicFinanceToken is ERC20Detailed, Ownable {
    ...
    string public _name = "Cosmik Finance";
    string public _symbol = "COSMIK";
    ...
    uint256 public cosmikTrustFee = 25;
    uint256 public safuuInsuranceFundFee = 50;
    uint256 public sellFee = 20;
    uint256 public firePitFee = 25;
    uint256 public totalFee =
        liquidityFee.add(cosmikTrustFee).add(safuuInsuranceFundFee).add(
            firePitFee
        );
    ...
    address public cosmikTrustReceiver;
    address public cosmicVaultFundReceiver;
    ...
    constructor() ERC20Detailed("Cosmik Finance", "COSMIK", uint8(DECIMALS)) Ownable() {
        ...
        cosmikTrustReceiver = 0x296E755D2622A253D5BaaFA582d40aA7879b87bb; 
        cosmicVaultFundReceiver = 0xd17Bc6123A197642D3944d0DA106342CEFBB7009;
        firePit = DEAD;
        ...
        _isFeeExempt[cosmikTrustReceiver] = true;
        _isFeeExempt[address(this)] = true;

        _transferOwnership(msg.sender);
        emit Transfer(address(0x0), cosmikTrustReceiver, _totalSupply);
    }
    ...
    function takeFee(
        ...
        uint256 _cosmikTrustFee = cosmikTrustFee;
        ...
        if (recipient == pair) {
            if (_isBot[sender]) {
                ...
                _cosmikTrustFee = _cosmikTrustFee.add(botFee);
            }
            _totalFee = _totalFee.add(sellFee);
            _cosmikTrustFee = _cosmikTrustFee.add(sellFee);
        }
        ...
        if (isTaxBracketEnabled) {
            ...
            _cosmikTrustFee = _cosmikTrustFee.add(_dynamicFee);
        }
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_cosmikTrustFee.add(safuuInsuranceFundFee))
        );
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountETHToCosmikTrustAndSIF = address(this).balance.sub(
            balanceBefore
        );

        (bool success, ) = payable(cosmikTrustReceiver).call{
            value: amountETHToCosmikTrustAndSIF.mul(cosmikTrustFee).div(
                cosmikTrustFee.add(safuuInsuranceFundFee)
            ),
            gas: 30000
        }("");
        (success, ) = payable(cosmicVaultFundReceiver).call{
            value: amountETHToCosmikTrustAndSIF.mul(safuuInsuranceFundFee).div(
                cosmikTrustFee.add(safuuInsuranceFundFee)
            ),
            gas: 30000
        }("");
    ...
    function withdrawAllToCosmikTrust() external swapping onlyOwner {
    ...
    {
        if (_isBot[from]){
            require(to == pair, "Cannot Transfer tokens to other address. You can only sell it");
        }
        return 
            (pair == from || pair == to) &&
            !_isFeeExempt[from];
    ...
    function setFeeReceivers(
        ...
        address _cosmikTrustReceiver,
        address _cosmicVaultFundReceiver,
        address _firePit
    ) external onlyOwner {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        cosmikTrustReceiver = _cosmikTrustReceiver;
        cosmicVaultFundReceiver = _cosmicVaultFundReceiver;
        firePit = _firePit;
    }
    ...
    function setCosmikTrustReceiver (address _cosmikTrustReceiver) external onlyOwner {
        cosmikTrustReceiver = _cosmikTrustReceiver;
    }
