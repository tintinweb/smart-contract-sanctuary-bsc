...
𝗧𝗘𝗟𝗘𝗚𝗥𝗔𝗠: https://t.me/GrinchGrow

𝗧𝗪𝗜𝗧𝗧𝗘𝗥: https://twitter.com/GrinchGrow
...
contract GrinchGrow is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    GrinchGrowDividendTracker public dividendTracker;
    ...
    constructor() ERC20("GrinchGrow", "GrinchGrow") {
        MarketingWallet = payable(0x21d65603EDBf20595a47B967F703f3E6DaED8d75);
        TeamWallet = payable(0x90804821dd68Be0aad3C7cAeDFdf5B6b60364381);
        ...
        dividendTracker = new GrinchGrowDividendTracker(
            payable(this),
            router,
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            uint256 totalFees = RewardsFee
                .add(liquidityFee + marketingFees + teamFees);
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        if (buyRewardsFee > 0) {
            ...
            .div(totalAmount)
            .mul(buyRewardsFee)
            .div(buyRewardsFee + buyMarketingFees + buyTeamFee);
        }
        if (sellRewardsFee > 0) {
            ...
            .div(totalAmount)
            .mul(sellRewardsFee)
            .div(sellRewardsFee + sellMarketingFees + sellTeamFee);
        }
        ...
        uint256 _completeFees = sellMarketingFees.add(sellTeamFee) +
            buyMarketingFees.add(buyTeamFee);
        ...
        if (teamPayout > 0) {
            (successOp2, ) = address(TeamWallet).call{value: teamPayout}("");
        }

        emit SendDividends(
            dividends,
            marketingPayout + teamPayout,
...
contract GrinchGrowDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    GrinchGrow public GrinchGrowContract;
    ...
    constructor(
        ...
        defaultToken = token;
        GrinchGrowContract = GrinchGrow(mainContract);
    ...
    function _transfer(
        ...
        require(false, "GrinchGrow_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "GrinchGrow_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main GrinchGrow contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(GrinchGrowContract);

            uint256 prevBalance = GrinchGrowContract.balanceOf(address(this));
            ...
            {
                uint256 received = GrinchGrowContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    GrinchGrowContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
