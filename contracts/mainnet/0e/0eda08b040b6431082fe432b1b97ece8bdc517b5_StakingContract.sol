...
// ========================== Leave Battle Functions ============================================
    ...
    function LeaveBattle(uint256 _battleId) public {
        ...
        if (!battle.joined) {
            if (block.timestamp < (battle.creatorStartingtime + 4 minutes)) //////////////////////48 minutes add
            {
    ...
    function GenWithdraw(uint256 _percentage) external {
        ...
        if (_percentage == 3) {
            ...
            require(genLastTransaction[msg.sender] < block.timestamp,
                "You canot withdraw amount before 16 minutes");
            genLastTransaction[msg.sender] = block.timestamp + 6 minutes; //minutes/////////////////////////
            ...
            require(genLastTransaction[msg.sender] < block.timestamp,
                "You canot withdraw amount before 24 minutes.");
            genLastTransaction[msg.sender] = block.timestamp + 4 minutes; //minutes//////////////////////
            ...
            require(genLastTransaction[msg.sender] < block.timestamp,
                "You canot withdraw amount before 32 minutes");
            genLastTransaction[msg.sender] = block.timestamp + 2 minutes; //minutes/////////////////
...
//================================Areena Functions ============================================
    ...
    function sellAreenaByBusd(uint256 _tokenAmount) external {
        ...
        if ((busdToken.balanceOf(busdWallet) + (90000 * 1e18)) >= uppermileStone) {
            ...
            areenaLastTransactionForBusd[msg.sender] = block.timestamp + 1 minutes; //////////minutes////////////////
...
//=====================================Clain ReferalBonus =========================================
    ...
    function claimReferalBonus(uint256 _battleId) public returns (bool v) {
        ...
        if (msg.sender == referalPerson[_battleId].creatorReferalPerson){
            ...
            require(block.timestamp > referallTime.add(7 minutes),
                "You can not claim bonus before 7 minutes."); //////////Add 7 minutes here /////////
