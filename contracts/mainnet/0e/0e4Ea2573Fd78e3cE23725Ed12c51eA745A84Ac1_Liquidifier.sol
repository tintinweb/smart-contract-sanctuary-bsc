...
contract Liquidifier is AccessControl, ReentrancyGuard, Operations {
    ...
    modifier isLiquidated(address _lp) {
        if (userinvestment[msg.sender][_lp].liqStatus) {
            revert liquidatedPosition();
        }
        _;
    }
    ...
    function increaseLiqETH(
        ...
        uint256 _position,
        address LP
    ) external payable isLiquidated(LP) returns (uint256 lp, uint256 treasury) {
        LiquidifierLocalVars memory vars;
        ...
        emit addETH(msg.sender, _position, vars.liquidity, vars._cake);
        ...
        )
    {
        ...
        return (_loan, _deposit, token1, token2);
    }
