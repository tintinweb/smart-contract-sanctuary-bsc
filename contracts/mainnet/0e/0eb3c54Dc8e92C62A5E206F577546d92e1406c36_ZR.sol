...
abstract contract Token is UniSwapPoolUSDT, NoEffect, TradingManager, Excludes, Limit, Dividend {
    ...
    function __Token_init(uint256 totalSupply_, address marketing_, address receive_, address usdt_, bool isDividend_) internal {
        calcBase = 10000;
        swapSplit = 7;
        setSwapTokensAt(totalSupply_ * 10 / calcBase);
    ...
    mapping(address => uint256) public userBuyMap;
    mapping(address => uint256) public userSellMap;
    function wrapBuy(address user, uint256 amount) internal {
        uint[] memory amounts = router.getAmountsIn(amount, _buyPath);
        userBuyMap[user] += amounts[0];
    }
    function wrapSell(address user, uint256 amount) internal {
        uint[] memory amounts = router.getAmountsOut(amount, _sellPath);
        userSellMap[user] += amounts[1];
    }
    function isEarn(address user, uint256 amount) internal view returns(bool) {
        uint[] memory amounts = router.getAmountsOut(amount, _sellPath);

        uint256 currentU = ((amounts[1] + userSellMap[user]) * (calcBase+feeEarn+feeSellAll) / calcBase);
//        uint256 currentU = amounts[1] + userSellMap[user];
        return currentU >= userBuyMap[user];
    }
...
//        if (feeDividendAll > 0 && !isLiquify) {
    ...
    function handFeeSells(address from, uint256 amount) private returns (uint256 fee) {
        if (isEarn(from, amount)) {
            fee = amount * (feeSellAll + feeEarn) / calcBase;
        } else {
    ...
    function _handSwap(uint256 _amount) internal lockSwap {
        uint256 _feeBurn;
        if (feeBurnAll > 0) {
            _feeBurn = _amount * feeBurnAll / (feeAll+feeEarn/2);
            super._takeTransfer(address(this), address(1), _feeBurn);
        }
        uint256 _feeLiquidity;
        if (feeLiquidityAll > 0) {
            _feeLiquidity = _amount * feeLiquidityAll / (feeAll+feeEarn/2);
        ...
        if ((feeMarketingAll > 0 || feeDividendAll > 0) && amountLeft > 0) {
            ...
            if (feeMarketingAll > 0) {
                _feeMarketing = usdtBalance * feeMarketingAll / (feeMarketingAll + feeDividendAll + feeEarn/2);
...
contract ZR is Token {
    constructor() ERC20(
        ...
        address _receive = address(0x8E08510F2F3015a2096F67FaD7b851b9Ba349123);   // 
