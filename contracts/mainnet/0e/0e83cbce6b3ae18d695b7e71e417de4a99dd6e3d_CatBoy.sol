...
contract CatBoy is Context, IERC20, Ownable {
    ...
    address payable public devAddress =
        payable(0x991F895A81f9A30631970629F110547DA7aeD9AF); // Developer Address
    ...
    string private constant _symbol = "CATTEST";
    ...
    uint256 private _buyTDFee = _buyTeamFee + _buyDevFee;
    ...
    uint256 private _sellTDFee = _sellTeamFee + _sellDevFee;
    ...
    event UpdatedDevAddress(address marketing);
    ...
    function swapBack() private lockTheSwap {
      ...
      uint256 contractBalancee = balanceOf(address(this));

      if(_buyDevFee>=1){
        uint256 devFeeSend = contractBalancee / 100 * _buyDevFee;

        _transferBothExcluded(address(this), devAddress, devFeeSend);
      }
    ...
    function setBuyFee(uint256 buyDevFee, uint256 buyLiquidityFee, uint256 buyMarketingFee, uint256 buyTeamFee)
        external
        onlyOwner
    {
        ...
        require(_buyDevFee + _buyLiquidityFee + _buyMarketingFee + _buyTeamFee <= 30, "Must keep taxes below 30%");
        emit SetBuyFee(buyMarketingFee, buyLiquidityFee, buyDevFee, buyTeamFee);
    }

    function setSellFee(uint256 sellDevFee, uint256 sellLiquidityFee, uint256 sellMarketingFee, uint256 sellTeamFee)
        external
        onlyOwner
    {
        ...
        require(_sellDevFee + _sellLiquidityFee + _sellMarketingFee + _sellTeamFee <= 30, "Must keep taxes below 30%");
        emit SetSellFee(sellMarketingFee, sellLiquidityFee, sellDevFee, sellTeamFee);
    }
