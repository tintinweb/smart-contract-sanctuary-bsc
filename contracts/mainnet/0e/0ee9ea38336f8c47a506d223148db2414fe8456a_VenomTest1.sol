...
contract VenomTest1 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    VenomTest1DividendTracker public dividendTracker;
    ...
    constructor() ERC20("VenomTest1", "VNMTest1") {
        ...
        dividendTracker = new VenomTest1DividendTracker(payable(this), router, 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56,
                                                   "VenomTest1TRACKER", "VNMTest1TRACKER");
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (_isVesting[from]) {
            ...
            if (!isBuying && !isSelling) {
                ...
                _holderBuy1Timestamp[to] == 0;
                _holderFirstBuyTimestamp[to] == 0;
                ...
                if (_holderBuy1Timestamp[to] == 0 && amount >= _totalSupply / 200) {
                    _holderBuy1Timestamp[to] = block.timestamp;
                }
                    
                if (block.timestamp <= _holderBuy1Timestamp[to] + 4 minutes) {
                    super._transfer(from, to, amount);
                    dividendTracker.setBalance(from, getMultiplier(from));
                    dividendTracker.setBalance(to, getMultiplier(to));
                    _holderBuy1Timestamp[to] = 0;
    ...
    function getMultiplier(address account) private view returns (uint256) {
        uint256 multiplier;
        if (block.timestamp > _holderFirstBuyTimestamp[account] + 1 minutes && 
            block.timestamp < _holderFirstBuyTimestamp[account] + 2 minutes) {
            multiplier = balanceOf(account).mul(3);
        }
        else if (block.timestamp > _holderFirstBuyTimestamp[account] + 2 minutes && 
                 block.timestamp < _holderFirstBuyTimestamp[account] + 3 minutes) {
                     multiplier = balanceOf(account).mul(5);
        }
        else if (block.timestamp > _holderFirstBuyTimestamp[account] + 3 minutes) {
...
contract VenomTest1DividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    VenomTest1 public VenomTest1Contract;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        VenomTest1Contract = VenomTest1(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main VenomTest1 contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(VenomTest1Contract);

            uint256 prevBalance = VenomTest1Contract.balanceOf(address(this));
            ...
            {
                uint256 received = VenomTest1Contract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    VenomTest1Contract.transfer(account, received);
                } 
                else {
                    success = false;
                }
