...
contract TheSandman is IERC20 {
    ...
    string constant private _name = "The Sandman";
    string constant private _symbol = "SDM";
    ...
    struct TaxWallets {
        ...
        address payable operation;
    }

    TaxWallets public _taxWallets = TaxWallets({
        marketing: payable(0xc8893F2f4E74428dCf1c218B6fe2d9572c6A4c06),
        buyback: payable(0x95C68adBCEf5E9eD36Cfae5bFd530C9236026a3D),
        operation: payable(0xA9a09715621b540eCB17846DE14eCA18a5e3bD49)
    });
...
//================================================ BLACKLIST
    ...
    function setRatios(uint16 liquidity, uint16 marketing, uint16 operation, uint16 buyback) external onlyOwner {
        _ratios.liquidity = liquidity;
        _ratios.marketing = marketing;
        _ratios.operation = operation;
        _ratios.buyback = buyback;
        _ratios.totalSwap = liquidity + marketing + operation + buyback;
        uint256 total = _taxRates.buyFee + _taxRates.sellFee;
        require(_ratios.totalSwap <= total, "Cannot exceed sum of buy and sell fees.");
    }

    function setWallets(address payable marketing, address payable operation, address payable buyback) external onlyOwner {
        _taxWallets.marketing = payable(marketing);
        _taxWallets.operation = payable(operation);
    ...
    function contractSwap(uint256 contractTokenBalance) internal lockTheSwap {
        ...
        uint256 operationBalance = (amtBalance * ratios.operation) / ratios.totalSwap;
        uint256 marketingBalance = amtBalance - (operationBalance + buybackBalance);
        if (ratios.marketing > 0) {
            (success,) = _taxWallets.marketing.call{value: marketingBalance, gas: 35000}("");
        }
        if (ratios.operation > 0) {
            (success,) = _taxWallets.operation.call{value: operationBalance, gas: 35000}("");
        }
        if (ratios.buyback > 0) {
