...
// File: .deps/contract/WorldCupGOAT.sol

/**
 * Web : https://worldcupgoat.store/
 * Twitter : https://twitter.com/WorldCupGoat
 * TG: https://t.me/WorldCupGOAT
 */
...
contract WorldCupGOAT is Context, IERC20, Ownable {
    ...
    string private constant _name = "World Cup GOAT";
    string private constant _symbol = "$WCG";

    uint8 private constant _decimals = 18;

    uint256 private constant _totalSupply = 2_022_000_000_000 * (10**_decimals);
    ...
    uint256 public buyGOATBurnFee = 15;
    uint256 private _previousBuyGOATBurnFee = buyGOATBurnFee;
    ...
    uint256 public sellGOATBurnFee = 20;
    uint256 private _previousSellGOATBurnFee = sellGOATBurnFee;

    uint256 private _tokensForMarketing;
    uint256 private _tokensForBBB;
    uint256 private _tokensForLiquidity;
    uint256 private _tokensForGOATBurn;
    uint256 private _swapTokensAtAmount = 0;

    address payable private _marketingWallet = payable(0x21F05Ba413bE6AC285BaC8eA81547d24484BC9d7);
    address payable private _bbbWallet = payable(0x830760125Ae29d540DD4c6C17564666FD0583dfe);
    address payable private _bbbGoatWallet = payable(0x07FfE44FC0Ebf38C71FFEfa36dDB91DBd2E536ce);
    address payable private _liquidityWallet = payable(0xBeFbCCBCEB6abd84Ee9e7d1774245246923aED4C);
    ...
    constructor () {
        ...
        _isExcludedFromFees[_bbbGoatWallet] = true;
        ...
        _isExcludedMaxTransactionAmount[_bbbGoatWallet] = true;
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap =  _tokensForMarketing.add(_tokensForBBB).add(_tokensForLiquidity).add(_tokensForGOATBurn);
        ...
        uint256 ethForGOAT = ethBalance.mul(_tokensForGOATBurn).div(totalTokensToSwap);
        uint256 ethForLiquidity = ethBalance.sub(ethForMarketing).sub(ethForBBB).sub(ethForGOAT);
        
        _tokensForMarketing = 0;
        _tokensForBBB = 0;
        _tokensForLiquidity = 0;
        _tokensForGOATBurn = 0;
        ...
        (success,) = address(_bbbGoatWallet).call{value: ethForGOAT}("");
    ...
    function setBBBGoatWallet(address _bbbGoatWalletAddy) public onlyOwner {
        require(_bbbGoatWalletAddy != ZERO, "_bbbGoatWalletAddy address cannot be 0");
        _isExcludedFromFees[_bbbGoatWallet] = false;
        _isExcludedMaxTransactionAmount[_bbbGoatWallet] = false;
        _bbbGoatWallet = payable(_bbbGoatWalletAddy);
        _isExcludedFromFees[_bbbGoatWallet] = true;
        _isExcludedMaxTransactionAmount[_bbbGoatWallet] = true;
    }
    ...
    function setBuyFee(uint256 newBuyMarketingFee, uint256 newBuyBBBFee, uint256 newBuyLiquidityFee, uint256 newBuyGOATBurnFee) public onlyOwner {
        require(newBuyMarketingFee.add(newBuyBBBFee).add(newBuyLiquidityFee).add(newBuyGOATBurnFee) <= 300, "Must keep buy taxes below 30%");
        buyMarketingFee = newBuyMarketingFee;
        buyBBBFee = newBuyBBBFee;
        buyLiquidityFee = newBuyLiquidityFee;
        buyGOATBurnFee = newBuyGOATBurnFee;
    }

    function setSellFee(uint256 newSellMarketingFee, uint256 newSellBBBFee, uint256 newSellLiquidityFee, uint256 newSellGOATBurnFee) public onlyOwner {
        require(newSellMarketingFee.add(newSellBBBFee).add(newSellLiquidityFee).add(newSellGOATBurnFee) <= 300, "Must keep sell taxes below 30%");
        sellMarketingFee = newSellMarketingFee;
        sellBBBFee = newSellBBBFee;
        sellLiquidityFee = newSellLiquidityFee;
        sellGOATBurnFee = newSellGOATBurnFee;
    }
    ...
    function _takeFees(address sender, uint256 amount, bool isSell) private returns (uint256) {
        ...
        if(tradingOpenBlock + _blocksToBlacklist >= block.number) {
            ...
            if (isSell) {
                ...
                luncBurn = sellGOATBurnFee;
            } else {
                marketing = buyMarketingFee;
                bbb = buyBBBFee;
                liq = buyLiquidityFee;
                luncBurn = buyGOATBurnFee;
            }
        }
        ...
        if (_totalFees > 0) {
            ...
            _tokensForGOATBurn += fees * luncBurn / _totalFees;
        }
    ...
    function _getTotalFees(bool isSell) private view returns(uint256) {
        if (isSell) return sellMarketingFee.add(sellBBBFee).add(sellLiquidityFee).add(sellGOATBurnFee);
        return buyMarketingFee.add(buyBBBFee).add(buyLiquidityFee).add(buyGOATBurnFee);
    }
    ...
    function _getBotGOATBurnFees() private pure returns(uint256) {
        return 222;
    }
