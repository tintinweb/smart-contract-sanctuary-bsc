...
contract ABCtransaction is Base  {
    ...
    function SellABCorder(uint256 orderID,uint256 price,uint256 ABC_quantity) public  returns(uint256){
        ...
        if(orderID>0){
            ...
            ABC.transferFrom(msg.sender,listBuy[orderID].orderAddress, listBuy[orderID].ABC_quantity);
            _USDTAddr.transferFrom(msg.sender,listSell[orderID].orderAddress, listSell[orderID].Price.mul(listSell[orderID].ABC_quantity));
  ...
  function removeIndex(address playerAddr,uint256 orderID,bool buyOrSell) private     {

            uint256 index = findIndex(  orderID,  msg.sender,  true );
   ...
   function SellABC(uint256[] calldata orderIDS,uint256 price,uint256 ABC_quantity) public  returns(uint256){
        ...
        if(orderIDS.length>0){
            for (uint256 i = 0; i < orderIDS.length; i++) {
              uint256  orderID = orderIDS[i];
                if(price  <= listBuy[orderID].Price){
    ...
    function BuyABC(uint256[] calldata orderIDS,uint256 price,uint256 ABC_quantity) public  returns(uint256 ){
        ...
        if(orderIDS.length>0){
            for (uint256 i = 0; i < orderIDS.length; i++) {
              uint256  orderID = orderIDS[i];
                if(price  >= listSell[orderID].Price){
