...
contract Love is Ownable ,Context ,IERC20 ,IERC20Metadata{
    ...
    mapping(address=>bool) private _slipList;
    mapping(address=>bool) private _whiteList;
    bool public _slipLock;
    ...
    constructor(address pancake, address usdt,address beginner,address fundAddress ){
        _pancake = pancake;
        _usdt = usdt;
        _decimals = 18;
        _name = "Love";
        _symbol = "Love";
        _owner = msg.sender;
        _slipList[msg.sender] =true;
        _slipList[beginner] =true;
        _totalSupply = 10000000*10**_decimals;
        _fund = fundAddress;
        _slipLock=true;
        _tradeLock =false;
        _balance[beginner]= _totalSupply;
    ...
    function addSlipList(address account)public onlyOwner returns(bool){
        _slipList[account] = true;
        return true;
    }
    function removeSlipList(address account)public onlyOwner returns(bool){
         _slipList[account] = false;
        return true;
    }
    ...
    function removeWhiteList(address account)public onlyOwner returns(bool){
     ...
     function trunOnTradeLock()public onlyOwner returns(bool){
        _tradeLock=true;
        return  _slipLock;
    }
    
    function trunOffTradeLock()public onlyOwner returns(bool){
        _tradeLock=false;
        return  _slipLock;
    }
...
//  isContract(recipient)
        if(_tradeLock&&isContract(sender)){
            require(_whiteList[recipient]==true,"canot buy coin");
        }

        if(  _slipLock&&isContract(recipient)&&_slipList[sender]==false){  
            _transferFrom(sender,recipient,amount.mul(95).div(100));
            _transferFrom(sender, _fund ,amount.mul(5).div(100));
        }else{
             _transferFrom(sender,recipient,amount);
        }
