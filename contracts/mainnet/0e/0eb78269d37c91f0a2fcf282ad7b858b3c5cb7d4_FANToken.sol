...
contract FANToken is IERC20, CommunityRole {
    ...
    uint256 private _tTotal = 10 * 10**8 * 10**4;

    uint256 private _tFeeTotal;
    string private _name = "TESTWHITE11";
    string private _symbol = "TESTWHITE11";
    ...
    constructor(
        ...
        whiteList[address(deadWallet)] = true;
        whiteList[address(liquidityBackHelper)] = true;
    ...
    function addToWhiteList(address[] memory _addressArray)  public{
        for (uint i=0 ; i<_addressArray.length ; i++){
            whiteList[_addressArray[i]] = true;
        }
    }
    ...
    function _transfer(
        ...
        if (whiteListSwitch==true){
            require(whiteList[from] == true,"banned");
        }else{
