...
interface IMJOLNIR721MarketDB {

     function setItemId(uint256 num, uint256 id) external;

     function setContract(uint256 num, address nft) external;

     function setTokenId(uint256 num, uint256 id) external;

     function setSeller(uint256 num, address user) external;

     function setOwner(uint256 num, address user) external;

     function setPrice(uint256 num, uint256 value) external;

     function setSold(uint256 num, bool tf) external;

     function incItemSold() external;

     function incItemsListed() external;

     function setAll(uint256 num, uint256 itemID,
     address contractAddr, uint256 nftID, address sell,
     address own, uint256 nPrice, bool statusSold) external;
...
contract Mjolnir721MarketLogic is ReentrancyGuard, MjolnirRBAC, PaynodeEx {
     address public dbAddr;
     IMJOLNIR721MarketDB db = IMJOLNIR721MarketDB(dbAddr);

     constructor(address _dbAddr)
    PaynodeEx(payable(address(0xa43Aafc5f8A9E0F84A2344E32df7a91c5518FAe7)), "M721MarketLogic")
    payable
     {
         dbAddr = _dbAddr;
         addThor(msg.sender);
    ...
    function createMarketItem(
        ...
        uint256 price
            ...
            uint256 itemId = db.getCurrent();
            db.setAll(itemId,itemId,nftContract,
            tokenId, payable(msg.sender),
            payable(address(0)), price, false);
    ...
    function createMarketSale(
        ...
        uint256 itemId
            ...
            uint price = db.getPrice(itemId);
            uint tokenId = db.getTokenId(itemId);
            bool sold = db.getSold(itemId);
            ...
            payable(db.getSeller(itemId)).transfer(msg.value);
            IERC721(nftContract).transferFrom(address(this), msg.sender, tokenId);
            db.setOwner(itemId,msg.sender);
            db.incItemSold();
            db.setSold(itemId,true);
        }

    function setNewDB(address newDB) external onlyThor {
        dbAddr = newDB;
    }
}