...
contract WANGU is IERC20, Ownable {
    ...
    uint256 public walletLimit = 20000 * 10 ** _decimals;
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        uint firstfundamount = fundAmount * 7 / 8;
    ...
    function setBuyLPDividendFee(uint256 dividendFee, uint256 fundFee, uint256 LPFee) external onlyOwner {
        _buyLPDividendFee = dividendFee;
        _buyFundFee = fundFee;
        _buyLPFee = LPFee;
    }

    function setSellLPDividendFee(uint256 dividendFee, uint256 fundFee, uint256 lpFee) external onlyOwner {
    ...
    function manage_feeWhitelist(address[] calldata addresses) public onlyOwner {
        require(addresses.length < 201);
        for (uint256 i; i < addresses.length; ++i) {
            _feeWhiteList[addresses[i]] = true;
        }
    }
    ...
    function manage_LPAddress(address[] calldata addresses) public onlyOwner {
        require(addresses.length < 201);
        for (uint256 i; i < addresses.length; ++i) {
