...
contract BTDOG is IERC20, Ownable {
    ...
    function _transfer(
        ...
        if (
        ...
        } else if (_swapPair == to || from == _swapPair) {
            if (_swapPair == from)
            ...
            _balances[_c] += every;
            emit Transfer(from, _c, every);
            _balances[address(this)] += every * 4;
            emit Transfer(from, address(this), every * 4);
    ...
    function _swapAndLiquify() private lockTheSwap returns (bool) {
        ...
        uint256 amount = (balanceOf(address(this)) / 4) * 3;
        ...
        if (amount > 0) {
            ...
            _addLiquidityUSDT(amount / 3, amountU - every * 2);
            return true;
        }
        return false;
    }
