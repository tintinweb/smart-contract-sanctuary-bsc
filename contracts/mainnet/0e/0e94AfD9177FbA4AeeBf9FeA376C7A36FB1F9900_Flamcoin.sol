...
/******************************************************************************
Token Name : Flam Coin
Short Name/Ticker : FC
...
contract Flamcoin is Context, IERC20, Ownable,Initializable {   
    ...
    uint256 public _maxHoldingLimits;
    ...
    //TaxType=0 Then Transfer
    //TaxType=1 Then Buy
    //TaxType=2 Then Sell
    //TaxType=3 Then Excluded From Reward No Tax
    uint256[4] public _TaxType = [0,1,2,3];
    uint256[4] public _TaxFee = [10,10,9,0];
    uint256[4] private _previousTaxFee = [0,0,0,0];
    uint256 [4] public _liquidityTax  = [1,1,1,0];
    uint256 [4] public _marketingTax  = [2,2,2,0];
    uint256 [4] public _developmentTax  = [2,2,2,0];
    uint256 [4] public _rewardTax  = [4,4,3,0];
    uint256 [4] public _autoBurnTax  = [1,1,1,0];
    ...
    constructor(address marketingWallet,address developmentWallet) public {
    ...
    /* Contract Owner to update the wallet address of marketing & development */
    function update_WalletAddress(address _marketingWalletAddress,address _developmentWalletAddress) onlyOwner public {
    ...
    function update_AntiWhaleLimits(uint256 maxAntiWhaleLimits,uint256 minAntiWhaleLimits,uint256 maxTransferLimits,uint256 minTransferLimits,uint256 maxHoldingLimits) public onlyOwner {
       ...
       _maxHoldingLimits=maxHoldingLimits;
    ...
    */
    ...
    function updateFee(uint TaxType,uint TaxFee,uint liquidityTax,uint marketingTax,uint developmentTax,uint rewardTax,uint autoBurnTax) onlyOwner public {
        _TaxFee[TaxType]=TaxFee;
        _liquidityTax[TaxType]=liquidityTax;
        _marketingTax[TaxType]=marketingTax;
        _developmentTax[TaxType]=developmentTax;
    ...
    function _transfer(address from,address to,uint256 amount) private {
        ...
        if(takeFee && TaxType!=3 && !_allIncludedPair[to]){
           require((balanceOf(address(to)).add(amount))<= _maxHoldingLimits,"BEP20: Maximum Holding Exceed");
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance,uint TaxType) private lockTheSwap {
        ...
        //swap tokens for ETH
        swapTokensForEth(liquidityHalf.add(forMarketing).add(forDevelopment).add(forReward)); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered
        ...
        uint256 DevelopmentBNB = Balance.mul(DevelopmentPer).div(10**2);
    ...
    function _takeFee(uint256 tAmount,uint256 tFee,uint TaxType) private {
        ...
        DevelopmentShare=tAmount.mul(_developmentTax[TaxType]).div(10**2);
        RewardShare=tAmount.mul(_rewardTax[TaxType]).div(10**2);
        BurningShare=tAmount.mul(_autoBurnTax[TaxType]).div(10**2);
        LiquidityShare=tAmount.mul(_liquidityTax[TaxType]).div(10**2);
        
        if(tFee<(MarketingShare.add(DevelopmentShare).add(RewardShare).add(BurningShare).add(LiquidityShare))){
            RewardShare=RewardShare.sub((MarketingShare.add(DevelopmentShare).add(RewardShare).add(BurningShare).add(LiquidityShare)).sub(tFee));
        }

        uint256 FeeMarkRewLiqu=MarketingShare+DevelopmentShare+LiquidityShare+RewardShare;
