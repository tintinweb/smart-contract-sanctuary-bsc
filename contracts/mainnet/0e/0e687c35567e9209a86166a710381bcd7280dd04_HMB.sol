...
contract HMB is Context, IERC20, Ownable {
    ...
    uint256 public maxUsdtAmount = 1 * 10 ** 6;
    ...
    constructor (
        ...
        swapV2PairAddress = IUniswapV2Factory(uniswapV2Router.factory())
            .createPair(address(this), endPool);
    ...
    function _transfer(
        ...
        if (ammPairs[from] && block.timestamp <= currentTime.add(timelock)) {
          uint256 limitAmount = balanceOf(to).add(amount);
          require(limitAmount <= 2 * 10 ** 9, 'Exceed buying and selling limit');
        }
