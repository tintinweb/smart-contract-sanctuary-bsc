...
interface operation {//konwnsec//IDividends 接口
    ...
    function updateTX(uint256 id, uint256 OutGold,uint256 SEOSQuantity) external;
...
// 提取产出（挖矿）
   function TX() public {
        ...
        if(temp[2] > 0 &&  temp[1]  > 0){
            ...
            uint256 Unum = temp[1].mul(10000000).div(EOSSPrice);
            if(temp[1]  >= Unum){
                _SEOSAddr.transfer(msg.sender, temp[2]);
                OutGold  = temp[2].sub(Unum);
                SEOSQuantity  = 0;
            }else{
                uint256 EOSSnum = temp[1].mul(10000000).div(EOSSPrice);
                _SEOSAddr.transfer(msg.sender, EOSSnum);
                SEOSQuantity  = temp[1].sub(EOSSnum);
                OutGold  = 0;
            }
        }
        diviIns.updateTX(id,OutGold,SEOSQuantity);
    }
