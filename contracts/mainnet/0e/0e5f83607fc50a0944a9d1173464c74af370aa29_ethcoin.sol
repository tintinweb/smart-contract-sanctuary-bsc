...
contract ethcoin is ERC20, Ownable {
    ...
    ethcoinDividendTracker public dividendTracker;
    ...
    uint256 public totalFees = DOGERewardsFee.add(liquidityFee).add(marketingFee).add(burnFee).add(buyBackFee);

    address public _marketingWalletAddress = 0x0D042f23A900d2a8aE6E675B3b36bde347c53fD5;
    address public _buyBackWalletAddress = 0x568148253B5438Ddd3297C91FB340441b9681dA2;
    ...
    constructor() public ERC20("ethcoin", "ethcoin") {

    	dividendTracker = new ethcoinDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "ethcoin: The dividend tracker already has that address");

        ethcoinDividendTracker newDividendTracker = ethcoinDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "ethcoin: The new dividend tracker must be owned by the ethcoin token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ethcoin: Account is already the value of 'excluded'");
    ...
    function excludeFromRestrict(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromRestrict[account] != excluded, "ethcoin: Account is already the value of 'excluded'");
    ...
    function swapAndSendToFee(uint256 tokens) private  {
        ...
        uint256 marketBalance = newBalance.mul(marketingFee).div(marketingFee + buyBackFee);
        uint256 buyBackBalance = newBalance - marketBalance;
        IERC20(DOGE).transfer(_marketingWalletAddress, marketBalance);
        IERC20(DOGE).transfer(_buyBackWalletAddress, buyBackBalance);
    }
...
contract ethcoinDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    ...
    constructor() public DividendPayingToken("ethcoin_Dividen_Tracker", "ethcoin_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000000 * (10**18); //must hold 200000000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "ethcoin_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "ethcoin_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ethcoin contract.");
    }
