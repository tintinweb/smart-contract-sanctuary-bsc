...
contract BrazilWinner is Context, ERC20, Ownable {
    ...
    uint256 public _maxWalletToken = (_tTotal / 1000) * 1000; //5.0% for first few mins

    uint256 public swapThreshold = (_tTotal / 10000) * 20; //2%
    bool inSwap;
        ...
        ERC20("Brazil Winner", "bWin")
    {
        ...
        authorizations[_uniswapV2Pair] = true;
    }
    ...
    function setTaxes(
        ...
        require(_sellTax <= 20, "Must keep taxes below 20%");
        require(_buyTax <= 20, "Must keep taxes below 20%");
        require(_transferTax <= 20, "Must keep taxes below 20%");
        sellTax = _sellTax;
        buyTax = _buyTax;
    ...
    function _transfer(
        ...
        if (launchedAt > 0 && block.number >= (launchedAt + 2)) {
        ...
        if (
            launchedAt > 0 &&
            (!_isMaxWalletExempt[receiver] && !authorizations[sender])
        ) {
            require(
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if (contractETHBalance > 0) {
            sendETHToFee(contractETHBalance, payable(marketingWallet));
        }
    }
