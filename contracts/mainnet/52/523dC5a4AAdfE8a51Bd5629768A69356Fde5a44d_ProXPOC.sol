...
contract ProXPOC is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    address private movieWallet = 0x9D248671163C79B308fF473E5fa01F454276A871;
    address private mktWallet = 0x52D92f6edf3620C771A6a2dCB0abD854fEe57B12;
    address private oldCollectionWallet =
        0x87E165a90Fe7409a66AfB1c72d3445e3845d0806;
    ...
    function setFeeReceiver(address _newMovie, address _newMkt, address _newOld) public onlyOwner {
        movieWallet = _newMovie;
        mktWallet = _newMkt;
        oldCollectionWallet = _newOld;
    }

    // Minting
    function mint(uint256 _mintAmount) public payable {
        // Checks
        require(!paused, "ProXPOC: minting has not started yet");
        require(_mintAmount > 0, "ProXPOC: you have to mint at least one");
        require(
            _mintAmount <= maxMintAmount,
            "ProXPOC: you can only mint 10 at a time"
        );
        require(
            lastSupply >= _mintAmount,
            "ProXPOC: the collection is sold out"
        );
        if (msg.sender != owner()) {
            require(
                msg.value >= cost * _mintAmount,
                "ProXPOC: total cost isn't match"
            );
        }
        // Minting
    ...
    {
        // Checks
        require(_winner != address(0), "ProXPOC: zero address");
        require(_amount > 0, "ProXPOC: you have to mint at least one");
        require(lastSupply != 0, "ProXPOC: the collection is sold out");
    ...
    function tokenURI(uint256 _id)
        ...
        returns (string memory)
    {
        require(_exists(_id), "ProXPOC: URI query for nonexistent token");
  ...
  function handleMintReward(uint256 _amount) private {
      // Split payment
      uint256 mintShare = _amount.div(10);
      uint movieShare = _amount.div(2);
      uint oldCollectionShare = _amount.div(10);
      uint mktShare = _amount.sub(mintShare).sub(movieShare).sub(oldCollectionShare);
      // Save dividend for community
      reflectDividend(mintShare);
      // Send owner share
      payable(oldCollectionWallet).transfer(oldCollectionShare);
      payable(movieWallet).transfer(movieShare);
      payable(mktWallet).transfer(mktShare);
  }
