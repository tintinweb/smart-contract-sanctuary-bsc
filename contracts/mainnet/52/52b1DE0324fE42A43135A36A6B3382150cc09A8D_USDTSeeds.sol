...
contract USDTSeeds is Ownable {
	...
	constructor(address payable wallet) public {
		...
		plans.push(Plan(7, 800));
        plans.push(Plan(10, 400));
        plans.push(Plan(15, 450));
		plans.push(Plan(30, 550));
		plans.push(Plan(60, 650));
	...
	function invest(address referrer, uint8 plan, uint256 amounts) public  {
		...
		user.deposits.push(Deposit(plan, amounts, block.timestamp,0));
	...
	function getUserDividends(address userAddress) public  returns (uint256) {
		...
		for (uint256 i = 0; i < user.deposits.length; i++) {
			...
			if (block.timestamp >= finish && user.deposits[i].stauts==0) {
