...
contract TAnts is IERC20, Ownable {
    ...
    function _transfer(
        ...
        if (from==uniswapV2Pair && isRoute[to]){
            ...
            if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
                require(amount<=2000*10**18,"buy limit");
            }
            takeFee = true;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if (sender==uniswapV2Pair && isRoute[recipient]){
                ...
                _reflectFee(rAmount.div(100).mul(_taxFee.mul(multiple)), tAmount.div(100).mul(_taxFee.mul(multiple)));
