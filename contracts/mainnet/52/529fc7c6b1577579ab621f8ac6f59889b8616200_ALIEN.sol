...
contract ALIEN is ERC721Enumerable, Ownable {
  ...
  uint256 public COLLECTED_MINTFEE;
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    for (uint256 i = 1; i <= _mintAmount; i++) {
      ...
      lastReflections[supply + i] = currentReflection;
    }
    uint256 reflectionShare = (msg.value * reflectionRate) / 100;
    addRewards(reflectionShare);
    COLLECTED_MINTFEE += msg.value;
  }
  ...
  function setReflectionRate(uint256 _reflectionRate) public onlyOwner {
    reflectionRate = _reflectionRate;
  }

  function setHolderRoyaltyRate(uint256 _royaltiesHOLDERS) public onlyOwner {
    royaltiesHOLDERS = _royaltiesHOLDERS;
  }
  function setTeamRoyaltyRate(uint256 _royaltiesTEAM) public onlyOwner {
  ...
  function withdraw() public onlyOwner {
    payable(msg.sender).transfer(COLLECTED_MINTFEE);
    COLLECTED_MINTFEE = 0;
  }
}