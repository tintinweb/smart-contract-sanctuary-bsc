...
contract mmmPlayer is Ownable {
    ...
    constructor(address[] memory _wl,address _market,address _fund,address _main,address _usdtToken) {
        market = _market;
        fund = _fund;
        main = _main;
    ...
    function claim() public {
        ...
        if(isWL[msg.sender]){
            canCliamReward = 2315 * BASE_RADIO;
        }else{
    ...
    function OpenGame(address _WL)public onlyOwner{
        refered[_WL] = true;
        isWL[_WL] = true;
    }
    ...
    function distributionLevelPool() private {
        ...
        if (level4Pool > 0) {
            for (uint256 i = 0; i < level4Num; i++) {
                if (
                    ...
                    !isOrederEndOfPlayer(level4PoolAddress[i])
                ) {
                    trueAddressNum = trueAddressNum + 1;
                }
            }
        ...
        if (trueAddressNum > 0) {
            ...
            for (uint256 i = 0; i < level4Num; i++) {
                ...
                if (!isOrederEndOfPlayer(level4PoolAddress[i])) {
    ...
    function calculateReward(uint256 _amount) private {
