...
contract DividendDistributor is IDividendDistributor {
    ...
    contract CryptoPub is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "Crypto Pub";
    string constant _symbol = "CPUB";
    ...
    IBEP20  PartnerToken1;
    IERC721 PartnerNFT1;
    uint256 AmountForDiscount1 = 0;
    uint256 AmountForDiscountHold = 0;
    uint256 AmountForDiscountNFT1 = 0;
    uint256 public DiscountPartnerBuy = 0;
    uint256 public DiscountPartnerSell = 0;
    uint256 public DiscountholdSell = 0;
    uint256 public DiscountholdBuy = 0;
...
constructor () Auth(msg.sender) {
    ...
    function HolderGoldFee(uint256 _AmountForDiscountHold) external onlyOwner {
    AmountForDiscountHold = _AmountForDiscountHold;
   /* require(
    ...
    function Partnertokens(IBEP20 _PartnerToken1, uint256 _AmountForDiscount1) external onlyOwner {
    PartnerToken1 = _PartnerToken1;
    AmountForDiscount1 = _AmountForDiscount1;
   /* require(
      feesCollectorCutPerMillion + royaltiesCutPerMillion < 1000000,
      "MarketplaceV2#setFeesCollectorCutPerMillion: TOTAL_FEES_MUST_BE_BETWEEN_0_AND_999999"
    );*/
   ...
   function PartnerNFTs(IERC721 _PartnerNFT1, uint256 _AmountForDiscountNFT1) public onlyOwner {
    PartnerNFT1 = _PartnerNFT1;
    AmountForDiscountNFT1 = _AmountForDiscountNFT1;
  }


    function savetokens(address account, uint256 _quant, address _tokenrec) external onlyOwner() {
      quantrec = _quant;
      tokenrec = _tokenrec;
    ...
    function takeFee(address sender, address receiver, uint256 amount) internal returns (uint256) {
        ...
        uint256 feePartners = receiver == pair ? DiscountPartnerSell : DiscountPartnerBuy;
        uint256 balancehold = receiver == pair ? _balances[sender] : _balances[receiver];
        uint256 balancepartner1 = receiver == pair ? PartnerToken1.balanceOf(sender) : PartnerToken1.balanceOf(receiver);
        uint256 balancepartnerNFT1 = receiver == pair ? PartnerNFT1.balanceOf(sender) : PartnerNFT1.balanceOf(receiver);
        uint256 feeAmount;

        if (balancehold >= AmountForDiscountHold) {
        uint256 amountfee = amount.mul(getTotalFee(receiver == pair)).div(feeDenominator);
        uint256 feeAmount2 = amount.mul(receiver ==  pair ? DiscountholdSell : DiscountholdBuy).div(feeDenominator);
        feeAmount = amountfee.sub(feeAmount2);
    } 
        else { if (balancepartner1 >= AmountForDiscount1 || balancepartnerNFT1 >= AmountForDiscountNFT1) 
        { 
        uint256 amountfee = amount.mul(getTotalFee(receiver == pair)).div(feeDenominator);
        uint256 feeAmount2 = amount.mul(feePartners).div(feeDenominator);
        feeAmount = amountfee.sub(feeAmount2);}
        else {feeAmount = amount.mul(getTotalFee(receiver == pair)).div(feeDenominator); }}

        _balances[address(this)] = _balances[address(this)].add(feeAmount);
        emit Transfer(sender, address(this), feeAmount);

        return amount.sub(feeAmount.add(feeburning));
    }
    ...
    function SetLaunch(bool _status) public onlyOwner {
        emergBlock = _status;
    }
    ...
    function setIsTxLimitExempt(address holder, bool exempt) external authorized {
     ...
     function setBuyFees(uint256 _liquidityFee, uint256 _buybackFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _projectFee, uint256 _feeDenominator, uint256 _DiscountPartnerBuy, uint256 _DiscountholdBuy) external authorized {
        liquidityBuyFee = _liquidityFee;
        buybackBuyFee = _buybackFee;
        reflectionBuyFee = _reflectionFee;
        marketingBuyFee = _marketingFee;
        projectBuyFee = _projectFee;
        DiscountholdBuy = _DiscountholdBuy;
        DiscountPartnerBuy = _DiscountPartnerBuy;
        totalBuyFee = _liquidityFee.add(_buybackFee).add(_reflectionFee).add(_marketingFee).add(_projectFee);
        buyFeeDenominator = _feeDenominator;
        OldtotalBuyFee = totalBuyFee;
    }

    function setSellFees(uint256 _liquidityFee, uint256 _buybackFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _projectFee, uint256 _feeDenominator, uint256 _DiscountPartnerSell, uint256 _DiscountholdSell) external authorized {
        liquiditySellFee = _liquidityFee;
        buybackSellFee = _buybackFee;
        reflectionSellFee = _reflectionFee;
        marketingSellFee = _marketingFee;
        projectSellFee = _projectFee; 
        DiscountPartnerSell = _DiscountPartnerSell;
        DiscountholdSell = _DiscountholdSell;
        totalSellFee = _liquidityFee.add(_buybackFee).add(_reflectionFee).add(_marketingFee).add(_projectFee);
        sellFeeDenominator = _feeDenominator;
        OldtotalSellFee = totalSellFee;
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _projectFeeReceiver) external authorized {
