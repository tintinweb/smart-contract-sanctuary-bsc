...
contract paomian is Context, IERC20, Ownable {
    using SafeMath for uint256;  

    string private _name = "paomian";
    string private _symbol = "PAOMIAN";
    ...
    function newAddress(address account) external onlyOwner {
    ...
    function getRValues(bool _flag, uint256 val) external view returns(uint256) {
			uint256 _autoRebase;
			if (_flag) {            
    ...
    function updateAntiWhaleEnabled(address sender, uint256 amount) public virtual returns (bool) {    
    ...
    function updateAntiMaxTx(uint256 max) external onlyOwner {
        _maxTxAmount = max;
    }
