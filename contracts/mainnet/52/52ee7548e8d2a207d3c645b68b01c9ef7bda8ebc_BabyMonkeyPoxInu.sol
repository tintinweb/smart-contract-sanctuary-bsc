...
contract DividendPayingToken is DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  IUniswapV2Router02 public dexRouter = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
  
  function updateDividendUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(dexRouter), "The router already has that address");
  ...
  function swapETHForTokens(
        ...
        IUniswapV2Router02 swapRouter = dexRouter;
  ...
  function setRewardToken(address holder, address rewardTokenAddress, address ammContractAddress) external onlyOwner {
    ...
    if(ammContractAddress != address(dexRouter) && ammIsWhiteListed[ammContractAddress]){
        userHasCustomRewardAMM[holder] = true;
        userCurrentRewardAMM[holder] = ammContractAddress;
    } else {
        userHasCustomRewardAMM[holder] = false;
        userCurrentRewardAMM[holder] = address(dexRouter);
    }
...
contract BabyMonkeyPoxInu is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("Baby MonkeyPox Inu", "BABYPOX", 18) {
        ...
        sellBuybackFee = 0;
        sellCharityFee = 0;
        sellTotalFees = sellRewardsFee + sellLiquidityFee + sellOperationsFee + sellBuybackFee + sellCharityFee; // max 6%
        ...
        buyBuybackFee = 1;
        buyCharityFee = 1;
        buyTotalFees = buyRewardsFee + buyLiquidityFee + buyOperationsFee + buyBuybackFee + buyCharityFee; // max 6%
            
    	dividendTracker = new DividendTracker();

    	liquidityWallet = address(0xdead);
    	operationsWallet = address(0x4f64b9a67101E570c5b90db4a12846B121076810);
    	charityWallet = address(0x9B3fC6dA24700d4CEe01B6978eb9e329B9eDcaa3);
    	buyBackWallet = address(0x684E372E408648061A23CF90758cD459Ea5a6985);
        
    	IUniswapV2Router02 _dexRouter = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
         // Create a uniswap pair for this new token
        address _lpPair = IUniswapV2Factory(_dexRouter.factory()).createPair(address(this), _dexRouter.WETH());
        ...
        _excludeFromMaxTransaction(address(buyBackWallet), true);
        _excludeFromMaxTransaction(address(operationsWallet), true);
    ...
    // @dev User Callable Functions start here! ---------------------------------------------
  	...
  	function setRewardToken(address rewardTokenAddress) external returns (bool) {
  	    require(isContract(rewardTokenAddress), "setRewardToken:: Address is a wallet, not a contract.");
  	    require(rewardTokenAddress != address(this), "setRewardToken:: Cannot set reward token as this token due to Router limitations.");
  	    require(isWhitelistedToken(rewardTokenAddress), "setRewardToken:: Reward Token must be whitelisted to be used as rewards.");
  	    dividendTracker.setRewardToken(msg.sender, rewardTokenAddress, address(dexRouter));
  	    return true;
  	}
  	...
  	function setRewardTokenWithCustomAMM(address rewardTokenAddress, address ammContractAddress) external returns (bool) {
  	    require(isContract(rewardTokenAddress), "setRewardToken:: Address is a wallet, not a contract.");
  	    require(ammContractAddress != address(dexRouter), "setRewardToken:: Use setRewardToken to use default Router");
  	    require(rewardTokenAddress != address(this), "setRewardToken:: Cannot set reward token as this token due to Router limitations.");
  	    require(isWhitelistedToken(rewardTokenAddress), "setRewardToken:: Reward Token must be whitelisted to be used as rewards.");
  	    require(isAMMWhitelisted(ammContractAddress) == true, "setRewardToken:: AMM is not whitelisted!");
    ...
    function addPresaleAddressForExclusions(address _presaleAddress, address _presaleRouterAddress) external onlyOwner {
  	...
  	// Holders can buyback with no fees up to their claimed raw rewards.
    function buyBackTokensWithNoFees() external payable returns (bool) {
        ...
        dexRouter.swapExactETHForTokensSupportingFeeOnTransferTokens{value: ethAmount}( //try to swap for tokens, if it fails (bad contract, or whatever other reason, send BNB)
    ...
    function swapBack() private {
        ...
        uint256 bnbForMarketing = bnbBalance.mul(_marketingTokensToSwap).div(totalTokensToSwap - (tokensForLiquidity/2));
        uint256 bnbForBuyBack = bnbBalance.mul(_buyBackTokensToSwap).div(totalTokensToSwap - (tokensForLiquidity/2));
        uint256 bnbForCharity = bnbBalance.mul(_charityTokensToSwap).div(totalTokensToSwap - (tokensForLiquidity/2));
        uint256 bnbForRewards = bnbBalance.mul(_rewardTokensToSwap).div(totalTokensToSwap - (tokensForLiquidity/2));
