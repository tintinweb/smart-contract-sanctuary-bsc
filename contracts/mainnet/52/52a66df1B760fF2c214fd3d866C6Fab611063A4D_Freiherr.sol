...
contract Reward {
    function getTransferReward(address from, address to, uint amount) public returns (uint256);
}

contract Freiherr is BEP20 {
    using SafeMath for uint256;

    address public owner = msg.sender;
    string public name = "Freiherr";
    ...
    function doTransfer(address from, address to, uint amount) private returns (bool) {
        uint256 reward = Reward(rewardContract).getTransferReward(from, to, amount);
