...
contract StrategyCommonChefLP is StratManager, FeeManager, GasThrottler {
    ...
    address[] public wantToZefiRoute = [want, wbnb, zefi];
    address[] public cakeToZefiRoute = [cake, wbnb, zefi];
    ...
    function chargeFees() internal {
        uint256 toZefi = IERC20(cake).balanceOf(address(this)); // ALL        
        IUniswapRouterETH(unirouter).swapExactTokensForTokens(toZefi, 0, cakeToZefiRoute, address(this), now.add(600));

        uint256 toBurn = IERC20(zefi).balanceOf(address(this)).mul(100).div(1000); // 10% BURN ZEFI
        IERC20(zefi).safeTransfer(dead, toBurn);

        uint256 zefiBal = IERC20(zefi).balanceOf(address(this));
        IERC20(zefi).safeTransfer(vault, zefiBal);
        IZefiBurningVault(vault).zvdepositAll();
    }


    // performance fees
    function chargeFeesOld() internal {
    ...
    function _giveAllowances() internal {
        ...
        // IERC20(zefi).safeApprove(unirouter, 0);
        IERC20(zefi).safeApprove(unirouter, uint256(-1));
        // IERC20(zefi).safeApprove(masterchef, 0); 
        IERC20(zefi).safeApprove(masterchef, uint256(-1));

        // IERC20(cake).safeApprove(zvzefi, 0);
        IERC20(cake).safeApprove(zvzefi, uint256(-1));
        // IERC20(wbnb).safeApprove(zvzefi, 0);
        IERC20(wbnb).safeApprove(zvzefi, uint256(-1));
        // IERC20(zefi).safeApprove(zvzefi, 0);
        IERC20(zefi).safeApprove(zvzefi, uint256(-1));
