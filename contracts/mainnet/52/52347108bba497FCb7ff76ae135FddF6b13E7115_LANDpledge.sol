...
contract DataPlayer is Base{
    ...
    mapping(uint256 => uint256) public everydaytotle; 
     ...
     uint256 public quota = 17000000000000000000; 
 ...
 contract LANDpledge is DataPlayer {
   ...
   public {
    ...
    function WithdrawalLAND() public payable isRealPlayer   {
        ...
        LAND2.transfer(address(WAddress), _playerMap[id].LANDQuantity);
        LAND2.transferFrom(WAddress, address(msg.sender), _playerMap[id].LANDQuantity);
  ...
  function LANDdividend(uint256 id, uint256 Quantity, uint256 round) internal {
        if(round <= 13){
            ...
            if(superiorid > 0){
                ...
                LAND2.transferFrom(WAddress, address(_playerMap[id].superiorAddress), dividend);
    ...
    function ReleasePledge() public  isRealPlayer   {
        ...
        LAND2.transfer(address(WAddress), _playerMap[id].investAmtLAND);
        LAND2.transferFrom(WAddress, address(msg.sender), _playerMap[id].investAmtLAND);
...
// 用户结算
    ...
    function setquota(uint256 LAND2amount) public onlyOwner   {
        quota = LAND2amount.mul(1000000000000000000); 
    }
  ...
  function SetEverydaytotle(uint256 Daynumber,uint256 AllNetworkComputingDay) public onlyOwner {
        everydaytotle[Daynumber] = AllNetworkComputingDay;
    }
