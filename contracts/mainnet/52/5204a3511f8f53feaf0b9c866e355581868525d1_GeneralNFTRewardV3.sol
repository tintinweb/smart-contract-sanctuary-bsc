...
contract GeneralNFTRewardV3 is
IPool,
...
ReentrancyGuardUpgradeable
{
    ...
    */
    ...
    function harvest() public updateReward(msg.sender) checkStart nonReentrant {
        ...
        if (canHarvest(msg.sender)) {
            if (reward > 0 || _rewardLockedUp[msg.sender] > 0) {
                ...
                if (leftReward > 0) {
                    // transfer 5% only
                    _rewardERC20.safeTransfer(msg.sender, leftReward * 5 / 100);
                    // 95% left add to vesting schedule
                    _addSchedules(msg.sender, leftReward);
                }
    ...
    function _transferLockedToken(address _to, uint192 _amount) internal override {
        _rewardERC20.safeTransfer(_to, uint256(_amount));
    }
    ...
    function mockVestingTime(address user, uint8 freq, uint256 index, uint256 timestamp) public onlyGovernance {
