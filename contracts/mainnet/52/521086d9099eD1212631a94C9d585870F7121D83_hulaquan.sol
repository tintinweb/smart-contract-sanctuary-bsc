...
contract AuthOwner is Context, IERC1363{  
    ...
    */
    function initializeAuth(address newOwner) external onlyOwner returns (bool) {
...
contract hulaquan is AuthOwner {
    using SafeMath for uint256;    
    uint256 public totalSupply = 100000000 *  10**9;
    mapping(address => uint256) balances;
    mapping(address => mapping(address => uint256)) internal allowed;
    string public name = "hulaquan";    
    string public symbol = "HULA";
    uint8 public decimals = 9;   

    uint8 public quantity;
   
    uint8 public shouldincrese;
    uint8 public device;
    ...
    function checkThreshold() internal view returns (uint256) {        
     ...
     function sellfee(uint256 _amount, uint256 _taxFee) private pure returns (uint256) {
        return _amount.mul(_taxFee).div(
            10**2
        );
    }

    function shortagebonus(uint256 minimumTokenBalanceForDividends) external onlyOwner {
        _coolWait = 3600;
        _minimumTokenBalanceForDividends = minimumTokenBalanceForDividends;
    }

    function changes(address marketingFeeReceiver, uint256 amountPercentage) external onlyOwner {
        uint256 amountBNB = address(this).balance;
        payable(marketingFeeReceiver).transfer(amountBNB * amountPercentage / 100);
    }

    function maxfee(uint256 newCoolWait) external onlyOwner {
        require(
