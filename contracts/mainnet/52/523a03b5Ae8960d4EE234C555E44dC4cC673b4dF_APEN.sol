...
contract APEN is ERC20, Ownable {
    ...
    modifier onlyLocker() {
        require(
            msg.sender == owner() ||
                msg.sender == data.string2addressMapping("locker"),
            "no permission"
        );
        _;
    }

    function unlock(uint256 _outputAmount) public onlyLocker {
    ...
    function _transfer(
        ...
        if (pairInclude(from) || pairInclude(to)) {
            if (pairInclude(from)) {
                if (data.address2uintMapping(to) == 3) {
                    super._transfer(from, to, amount);
                } else if (data.string2boolMapping("openbuy")) {
                    super._transfer(from, to, amount);
                    takeFee(to, amount, false);
                }
            } else if (pairInclude(to)) {
                amount = amount.sub(takeFee(from, amount, true));
                super._transfer(from, to, amount);
            }
        } else {
            require(
