...
contract FightNight_V1_BUSD_USDT is
    IRewardDistributionRecipient,
    ReentrancyGuard
{
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    //?sourosh az client tarikh bgir be timestamp tabdil kon inja bezar
    //!commenta ro ham pak kon baad
    uint256 public constant endTime = 0; //timestamp
    ...
    event EarningsPaid(
        ...
        uint256 busdEarnings,
        uint256 usdtEarnings
    );
    ...
    function BUSDBet(Fighter fighter, uint256 amount)
        public
        checkStatus
        nonReentrant
    {
    ...
    function getFees()
        ...
        returns (uint256 busdFees, uint256 usdtFees)
    {
    ...
    function earned(address account)
        public
        view
        returns (uint256 busdEarnings, uint256 usdtEarnings)
    {
        if (isFinal) {
            ...
            if (winner == Fighter.Player_1 && _Player_1BUSDBet != 0) {
                winnings = Player_2BUSDPot.mul(_Player_1BUSDBet).div(
                    Player_1BUSDPot
                );
                ...
                winnings = Player_1BUSDPot.mul(_Player_2BUSDBet).div(
                    Player_2BUSDPot
                );
                fee = winnings.mul(1e18).div(1e19);
                winnings = winnings.sub(fee);
    ...
    function getRewards() public nonReentrant {
        ...
        require(!lock[msg.sender], "you have already wotdraw your reward"); //*custome
    ...
    function getPlayerinformationBusd(address sender)
        public
        view
        returns (
            uint256 _Player_1BUSDBet,
            uint256 _Player_2BUSDBet,
            uint256 _Player_1BUSDPot,
            uint256 _Player_2BUSDPot
        )
    {
        _Player_1BUSDBet = Player_1BUSDBet[sender];
        _Player_2BUSDBet = Player_2BUSDBet[sender];
        _Player_1BUSDPot = Player_1BUSDPot;
        _Player_2BUSDPot = Player_2BUSDPot;
    }
