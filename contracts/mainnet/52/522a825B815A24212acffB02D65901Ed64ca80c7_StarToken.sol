...
contract ERC20 is Context, IERC20 {
    ...
    uint256 public maxSupply = 1280000 * 1e18;

    address[5] public taxes = [
        address(0),
        address(0),
        address(0),
        address(0),
    ...
    function _transfer(
        ...
        require(blackNames[sender] == false && blackNames[recipient] == false, "ERC20: black name");

        uint256 tax = 0;
        if(whiteNames[recipient] == false && whiteNames[sender] == false) {
            require(startTrade == true, "ERC20: not start");
            if (pairs[sender] == true) {
                require(whiteNames[recipient] == true, "ERC20: only white names");
            } else if (pairs[recipient] == true) {
                tax = 5;
            }
        }

        uint256 taxAmount = amount.mul(tax).div(HUNDRED);
        ...
        if (tax > 0) {
            uint256 taxAmountPer = taxAmount.div(5);
            for (uint8 i = 0; i < taxes.length; i++) {
                _balances[taxes[i]] = _balances[taxes[i]].add(taxAmountPer);
            }
        }
...
contract StarToken is ERC20, ERC20Detailed {
    ...
    function setTaxes(address[5] memory _taxes) public onlyGovernance {
        taxes = _taxes;
    }
    ...
    function setStart() public onlyGovernance {
        startTrade = true;
    }

    function setStop() public onlyGovernance {
        startTrade = false;
    }
}