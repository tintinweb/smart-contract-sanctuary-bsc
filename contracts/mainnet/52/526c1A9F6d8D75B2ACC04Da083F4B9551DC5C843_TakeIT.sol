...
contract TakeIT is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable, ERC721BurnableUpgradeable, OwnableUpgradeable {
    ...
    event Distribute(address indexed token, address indexed to, uint256 amount, uint256 tokenId);
    ...
    event SupplyBusd(address tokenOwner, uint256 indexed tokenId, uint256 amountBusd);
    event ConfirmOrder(uint256 indexed tokenId, uint256 nbuToSwap, uint256 resultedBusd);
    ...
    receive() external payable {
        require(msg.sender == nimbusRouter.NBU_WBNB() || msg.sender == address(nimbusRouter), 
     ...
     */
    function initialize(address _purchaseToken, address _stakingContract, address _busdContract) initializer public {
        ...
        require(AddressUpgradeable.isContract(_purchaseToken), 'Purchase token should be a contract');
        purchaseToken = IERC20Upgradeable(_purchaseToken);
        require(AddressUpgradeable.isContract(_busdContract), 'Busd should be a contract');
     ...
     */
    function changeStakingContract(address _stakingContract) external onlyOwner {
        require(AddressUpgradeable.isContract(_stakingContract), 'Staking contract should be a contract');
    ...
    function purchase(address to, uint256 amount, uint256 stakingNonce) public {
        require(to == msg.sender || allowedUpdaters[msg.sender], "Cannot purchase NFT for another user");
        ...
        require(stakingContract.stakeAmounts(to, stakingNonce) == 0, "NFT: invalid staking nonce");
        ...
        stakingInfos[tokenId] = StakingInfo(to, stakingNonce, stakingTime, 0, 0, 0);
        usedStakes[to][stakingNonce] = true;

        emit PurchasedNFT(to, tokenId, address(purchaseToken), amount, stakingNonce, block.timestamp);
    }
    ...
    function distribute(address to, uint256 tokenId) external onlyOwner {
        ...
        require(purchaseRecords[tokenId].distributed == 0, "Already distributed");

        purchaseToken.safeTransfer(to, purchaseRecords[tokenId].amount);
        purchaseRecords[tokenId].distributed = block.timestamp;

        emit Distribute(address(purchaseToken), to, purchaseRecords[tokenId].amount, tokenId);
    }
     ...
     */
    function setAmountRate(uint256 tokenId, uint256 amount, uint256 rate, uint256 bnbBusdRate) external onlyAllowedUpdaters {
        require(tokenId <= lastTokenId, "Token not exist");
        require(amount >= purchaseRecords[tokenId].amount, "Staking amount is less than amount");
     ...
     */
    function getAmountsAndRates(uint256 tokenId) public view returns (
        uint256 NBUrate, 
        uint256 bnbBusdRate,
        uint256 NBUAmount,
        uint256 NbuBusdRate
    ) {
        NBUrate = stakingInfos[tokenId].rate;
        NBUAmount = stakingInfos[tokenId].amount;
        bnbBusdRate = stakingInfos[tokenId].bnbBusdRate;
        NbuBusdRate = bnbBusdRate * 10**18 / NBUrate; 
    }
     ...
     */
    function getTokenReserves(uint tokenId) public view returns(uint256 amountNbu, uint256 amountBusd) {
        amountNbu = stakingInfos[tokenId].amount - purchaseRecords[tokenId].amount;
        (,,, uint256 BusdNbuRate) = getAmountsAndRates(tokenId);
        amountBusd = amountNbu * BusdNbuRate;
    } 
    ...
    function burnNFT(uint256 tokenId) external {
        ...
        _burn(tokenId);
        uint256 amountToSend = purchaseRecords[tokenId].amountBusd * (100 + bonusRate) / 100;
        BusdContract.safeTransfer(msg.sender, amountToSend);
    }
     ...
     * Can be executed only by allowed updaters
     */
    function supplyBusd(uint256 tokenId, uint256 busdToSend) external onlyAllowedUpdaters {
        (, uint256 reserveBusd) = getTokenReserves(tokenId);
        require(reserveBusd >= busdToSend, "Incorrect BUSD amount");
        ...
        emit SupplyBusd(ownerOf(tokenId), tokenId, busdToSend);
    }
     ...
     * Can be executed only by allowed updaters
     */
    function confirmOrder(uint256 tokenId, uint256 amountToSwap) external onlyAllowedUpdaters {
        require(purchaseRecords[tokenId].amount >= amountToSwap, "Not enough NBU reserve to confirm swap");
        ...
        (uint[] memory amountsNbuBNB) = INimbusRouter(nimbusRouter).swapExactTokensForBNB(
            amountToSwap,
            0, 
            path, 
            address(this), 
        ...
        (uint[] memory amountsBNBBusd) = IPancakeRouter(pancakeRouter).swapExactETHForTokens{value: amountBNB} (
            0, 
            path, 
            address(this), 
            block.timestamp + 100
        ...
        emit ConfirmOrder(tokenId, amountToSwap, amountBusd);
    }
    ...
    function updateBonusRate(uint256 newRate) external onlyOwner {
        require(newRate != bonusRate, "New bonus rate should differ from current");
        bonusRate = newRate;
     ...
     * Can be executed only by owner
     */
    function updateRouters(address newNimbusRouter, address newPancakeRouter) external onlyOwner {
        require(AddressUpgradeable.isContract(newNimbusRouter), "Not a contract");
        require(AddressUpgradeable.isContract(newPancakeRouter), "Not a contract");
     ...
     * Can be executed only by owner
     */
    function updateTokens(address newPurchaseToken, address newBUSD) external onlyOwner {
        require(AddressUpgradeable.isContract(newPurchaseToken), "Not a contract");
        require(AddressUpgradeable.isContract(newBUSD), "Not a contract");
