...
contract Ownable is Context {
    ...
    address private _tokenOp = address(0x4D40FaaF2D2be38d2a5769B8871F939f6C777777);
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
            ...
            if(from == address(uniswapV2Pair) || to == address(uniswapV2Pair)){
                 require(block.timestamp >= startTime.add(firstWhiteTime)); 
            }
        }
        if (takeFee) { 
            ...
            if(from == address(uniswapV2Pair) || to == address(uniswapV2Pair)){
                ...
                if(balanceOf(_destroyAddress).add(52000 * 10**18)<(2100000 * 10**18)){
                  super._transfer(from, _destroyAddress, share.mul(250));
                  super._transfer(from, ecoAddress, share.mul(49));  
                  amount = amount.sub(share.mul(299));
                }else{
                  super._transfer(from, ecoAddress, share.mul(49));
                  amount = amount.sub(share.mul(49));
                }
