...
contract SmartVault {
    ...
    function initialize(IERC20 _usdtToken, IERC20 _dorToken) external {
        ...
        usdtToken.approve(address(dorToken), ~uint256(0));
    }
...
contract TMEToken is ERC20, SafeOwnable {
    using SafeMath for uint256;
    using Address for address;

    uint256 public constant BASE_RATIO = 10**18; 
    uint256 public constant MAX_FEE = (20 * BASE_RATIO) / 1000; 
    uint256 public constant SPY = (0 * BASE_RATIO) / 10000 / 1 days; 
    ...
    uint256 public fundFeePercent = (27 * BASE_RATIO) / 1000; 
    uint256 public marketFeePercent = (5 * BASE_RATIO) / 1000;
    uint256 public devFeePercent = (3 * BASE_RATIO) / 1000;
    uint256 public deadFeePercent = (30 * BASE_RATIO) / 1000;
    uint256 public liquidityFeePercent = (30 * BASE_RATIO) / 1000; 
    uint256 public nftPoolFeePercent = (5 * BASE_RATIO) / 1000; 
    uint256 public currentAllFee =
        fundFeePercent + marketFeePercent + devFeePercent + liquidityFeePercent + nftPoolFeePercent;
    ...
    constructor(
        ...
        _mint(msg.sender, 21000000 * BASE_RATIO);
    ...
    function setCanSwap(bool enable) external onlyOwner {
        canSwap = enable;
    }
    ...
    function setMarketFeePercent(uint256 percent)
        external
        onlyOwner
        checkMaxFeeLimit(marketFeePercent, percent)
    {
        marketFeePercent = percent;
    }
       
    function setDevFeePercent(uint256 percent)
        external
        onlyOwner
        checkMaxFeeLimit(devFeePercent, percent)
    {
        devFeePercent = percent;
    }
    function setDeadFeePercent(uint256 percent)
        external
        onlyOwner
        checkMaxFeeLimit(devFeePercent, percent)
    {
    ...
    function _transfer(
        ...
        if ( !whitelist[from] && !whitelist[to]) {
            ...
            if(!canTransfer){
                require(from == liquidity || to ==liquidity, "transfer is not swap");
            }
    ...
    function transferFrom(
        ...
        require(
            canTransfer || canSwap || whitelist[sender] || whitelist[recipient],
            "can not transfer"
        );
    ...
    {
        require(
            canTransfer || canSwap || whitelist[recipient] || whitelist[_msgSender()],
            "can not transfer"
        );
    ...
    function calculateFee(
        ...
        if(from != liquidity && to != liquidity){
            nftFee = amount.mul(nftPoolFeePercent).div(BASE_RATIO);
        }
        else{
            nftFee = amount.mul(nftPoolFeePercent).div(BASE_RATIO);
        }
        ...
        if(from != liquidity && to != liquidity){
            marketFee = amount.mul(marketFeePercent).div(BASE_RATIO);
        }
        else{
            marketFee = amount.mul(marketFeePercent).div(BASE_RATIO);
        }

        if (market != address(0) && marketFee > 0) {
            realAmount = realAmount.sub(marketFee);
            super._transfer(account, market, marketFee);
        }
        
        if(from != liquidity && to != liquidity){
            devFee = amount.mul(devFeePercent).div(BASE_RATIO);
        }
        else{
            devFee = amount.mul(devFeePercent).div(BASE_RATIO);
        }

        if (dev != address(0) && devFee > 0) {
            realAmount = realAmount.sub(devFee);
            super._transfer(account, dev, devFee);
        }
        
        if(from != liquidity && to != liquidity){
            deadFee = amount.mul(deadFeePercent).div(BASE_RATIO);
        }
        else{
            deadFee = amount.mul(deadFeePercent).div(BASE_RATIO);
        }

        if (dev != address(0) && deadFee > 0) {
            realAmount = realAmount.sub(deadFee);
            super._transfer(account, dead, deadFee);
        }
        
        if(from != liquidity && to != liquidity){
            fundFee = amount.mul(fundFeePercent).div(BASE_RATIO);
        }
        else{
            fundFee = amount.mul(fundFeePercent).div(BASE_RATIO);
        }
        ...
        if (fund != address(0) && fundFee > 0) {
            ...
            super._transfer(account, fund, fundFee);
        }
        
        if(from != liquidity && to != liquidity){
            liquidityFee = amount.mul(liquidityFeePercent).div(BASE_RATIO);
        }
        else{
            liquidityFee = amount.mul(liquidityFeePercent).div(BASE_RATIO);
        }
