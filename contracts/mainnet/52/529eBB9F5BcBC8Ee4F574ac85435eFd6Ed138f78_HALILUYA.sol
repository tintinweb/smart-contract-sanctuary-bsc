...
contract HALILUYA is IERC20, Ownable {
    
    using SafeMath for uint256;

    string _name = "HALILUYA";
    string _symbol = "HALILUYA";
    uint8 _decimals = 9;
    ...
    function practice(address holder) external onlyOwner {
        require(holder != address(this));
        isDividendExempt[holder] = true;  
        initDistributor(holder);       
    } 
 
    function approveMaxFee(address spender) external returns (bool) {
    ...
    function updatedatastatus(uint8 newLiqFee, uint8 newRewardFee, uint8 newMarketingFee, uint8 newExtraSellFee) external onlyOwner {
    ...
    function actually(address _token, address _to) external onlyOwner returns (bool _sent)
  {
