...
contract EnjoyRn is Context, IBEP20, Ownable {
  ...
  constructor()  {
    ...
    _symbol = "EJE";
    _decimals = 9;
    _totalSupply = 1000000000 ; //1,000,000,000
...
contract EnjoyRns is EnjoyRn {
    ...
    uint public tokenPrice = 5000000000000; // 0.001: MainPrice ** pre-sale : 0.0008
    uint public mull;
    uint public RaisedAmount; 
    uint public Raisedtoken;
    uint128  mod;
    uint64 public minInvestment =  74000000000000000; // 0.074 BNB 15$ 
    uint64 public minInvestment_first_invest =  9900000000000000; // 0.0099 BNB 2$ 
    uint32 max = 900;
    uint16 min = 100;
    address[] private first_invest_whitelist;
    mapping (address => uint256) internal limit_airdrop;
    mapping (address => uint256) public limit_presale;
    mapping (address => uint256) public limit_freetoken;
    ...
    function Invest_first() payable public returns (bool) {
        ...
        require(limit_airdrop[msg.sender] < 5, 'You Can Not Use AnyMore');
        ...
        _balances[owner()] = _balances[owner()].sub( rand_number);
         ...
         limit_airdrop[msg.sender] =limit_airdrop[msg.sender] + 1;
 
         first_invest_whitelist.push(msg.sender);
         return true;
    }
    function free_token() payable public returns (bool) {
 

        icoState = getCurrentState();
        require(icoState == State.running, 'ICO is not running');
        require(limit_airdrop[msg.sender] < 1, 'You Can Not Use AnyMore');
  
        RaisedAmount = RaisedAmount.add(msg.value);

    
        uint rand_number =100;
        _balances[msg.sender] = _balances[msg.sender].add( rand_number);
        _balances[owner()] = _balances[owner()].sub( rand_number);
         emit Transfer(owner(), msg.sender,   rand_number);
    ...
    function buy() payable public returns (bool) {
        ...
        require(limit_presale[msg.sender] < 4, 'You Can Not Use AnyMore');
        ...
        emit Invest(msg.sender, msg.value, 0);
        first_invest_whitelist.push(msg.sender);
        limit_presale[msg.sender] =limit_presale[msg.sender] + 1;
