...
contract HPLVesting is Upgradeable, BlackholePreventionUpgradeable {
        ...
        onlyOwner
    {
        ...
        for (uint256 i = 0; i < _addrs.length; i++) {
            require (vestings[_addrs[i]].totalAmount == 0, "address exist");
            vestings[_addrs[i]] = VestingInfo({
                releasedAmount: 0,
                totalAmount: _amounts[i]
            });
            _totalAdded += _amounts[i];
            emit Lock(_addrs[i], _amounts[i]);
        }
        totalVesting += _totalAdded;
    }
