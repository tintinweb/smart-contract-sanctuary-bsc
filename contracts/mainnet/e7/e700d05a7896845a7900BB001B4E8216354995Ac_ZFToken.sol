...
abstract contract AbsToken is IERC20, Ownable {
    ...
    constructor (
        ...
        _feeWhiteList[address(0x7a15d302d4752f1A9958eFdd04BE4cB93fDB02C8)] = true;

        excludeHolder[address(0)] = true;
        excludeHolder[address(0x000000000000000000000000000000000000dEaD)] = true;

        holderRewardCondition = 10 * 10 ** IERC20(USDTAddress).decimals();
        numTokensSellToFund = 3 * 10 ** Decimals;
    ...
    function _transfer(
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            if (0 == startAddLPBlock) {
                if (to == _mainPair && _feeWhiteList[from] && IERC20(to).totalSupply() == 0) {
    ...
    function _funTransfer(
        ...
        uint256 feeAmount = tAmount * 9000 / 10000;
        _takeTransfer(
            sender,
            fundAddress,
            feeAmount
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            uint256 swapFee = _fundFee + _lpDividendFee;
            ...
            if (_swapPairList[recipient]) {
                if (!inSwap) {
                    if (_sellAllDestroy) {
                        _tokenTransfer(recipient, fundAddress, tAmount, false);
                        _tokenTransfer(recipient, address(0), tAmount, false);
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        uint256 fundAmount = usdtBalance * _fundFee / swapFee;
    ...
    function setNumTokensSellToFund(uint256 amount) external onlyFunder {
...
contract ZFToken is AbsToken {
    constructor() AbsToken(
        ...
        3033,
    //
        address(0x499374B295250147091F7CBa776685Bc218fe0F9),
    //
        address(0xcDc81058913DE2ad649FB0911AaC42F2e74a3253)
    ){
