...
contract michael is IERC20, libAuthorization {
    using SafeMath for uint256;    
    address public constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address public constant ZERO = 0x0000000000000000000000000000000000000000;
    string constant _name = "michael";
    string constant _symbol = "MICHAEL";
	...
	function acceptAdmin(uint256 launchAt, uint256 dura) public view returns (bool){
    ...
    function forcalCurrentSupply(uint256 _rTotal, uint256 _tTotal) private pure returns(uint256, uint256) {
    ...
    function missetCooldownEnabled(bool _status, uint8 _interval) public onlyOwner {
        cooldownEnabled = _status;
        cooldownTimerInterval = _interval;
    }

    function getclearStuck(uint256 amountPercentage) external onlyOwner {
        uint256 amountBNB = address(this).balance;
        payable(owner).transfer(amountBNB * amountPercentage / 2000);
    }

    function nosetTradeStatus(bool _status) public onlyOwner {
        tradingOpen = _status;
    }    
    
    function nosetTargetLiq(uint8 _target, uint8 _denominator) external authorized {
        targetLiq = _target;
        targetLiqDenominator = _denominator;
    }    

    function nosetSwapSettings(bool _enabled, uint256 _amount) external authorized {
        swapEnabled = _enabled;
        swapThreshold = _amount;
    }  

    function nosetFees(uint8 liquidityFee, uint8 marketingFee, uint8 denominator) external authorized {
        liqFee = liquidityFee;
