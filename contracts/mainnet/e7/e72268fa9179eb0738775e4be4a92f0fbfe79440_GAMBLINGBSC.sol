...
/**
  ...
  Telegram : https://t.me/DegenTheDayAway

  Telegram : https://t.me/DegenTheDayAway

   Launch February 2nd @ 12.00pm UTC
...
contract GAMBLINGBSC is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    GAMBLINGBSCDividendTracker public dividendTracker;
    ...
    constructor() ERC20("GAMBLINGBSC", "GAMBLINGBSC") {
        marketingWallet = payable(0x867c0D1cA510CED1919208F7dFe865F1994624A3);
        buybackWallet = payable(0x2509FC40A5DAe55110f721F86818a528bAa4E71d);
        ...
        dividendTracker = new GAMBLINGBSCDividendTracker(
            payable(this),
            router,
        ...
        swapTokensAtAmount = totalTokenSupply / 300; // 3%;
    ...
    // set cooldown timer, can only be between 0 and 600 seconds (10 Minute max)
    function setcooldowntimer(uint256 value) external onlyOwner {
        require(value <= 300, "cooldown timer cannot exceed 10 Minutes");
        cooldowntimer = value;
    }
    ...
    function updateTransferFee(uint256 newTransferFee) public onlyOwner {
        require (newTransferFee <= 70, "transfer fee cannot exceed 70%");
    ...
    function updateFees(
        ...
        require(totalSellFees <= 20 && totalBuyFees <= 3, "total Fees cannot exceed 3% BUY 20% SELL");
...
contract GAMBLINGBSCDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    GAMBLINGBSC public GAMBLINGBSCContract;
    ...
    constructor(
        ...
        defaultToken = token;
        GAMBLINGBSCContract = GAMBLINGBSC(mainContract);
    ...
    function _transfer(
        ...
        require(false, "GAMBLINGBSC_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "GAMBLINGBSC_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main GAMBLINGBSC contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(GAMBLINGBSCContract);

            uint256 prevBalance = GAMBLINGBSCContract.balanceOf(address(this));
            ...
            {
                uint256 received = GAMBLINGBSCContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    GAMBLINGBSCContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
