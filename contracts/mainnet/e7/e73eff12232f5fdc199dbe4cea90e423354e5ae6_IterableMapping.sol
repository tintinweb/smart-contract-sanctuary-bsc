...
contract NFTPROLAND is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public totalFees = BUSDRewardsFee.add(liquidityFee).add(marketingFee).add(recoveryFee);
    ...
    address public _marketingWalletAddress = address(0x0EF86186Ee9c381b5831263021231B5410f8203f); // marketing wallet
    address public _devWalletAddress = address(0xFD0E6E76d86B8169e0EA520285F65340c8aA06e5); // fee for contract development
    address public _recoveryAddress = address(0xAfa89dE37ceca739d3e7a8286a78999dcdC7F19B); //  Recovery Address (finalized)
    ...
    function setBUSDRewardsFee(uint256 value) external onlyOwner{
        ...
        require(BUSDRewardsFee.add(liquidityFee).add(marketingFee).add(recoveryFee) <= 15, "May not set total fees above 15%");
        totalFees = BUSDRewardsFee.add(liquidityFee).add(marketingFee).add(recoveryFee);
    }
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "EA: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
