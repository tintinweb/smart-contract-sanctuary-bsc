...
// File: contracts\SingleOpensaleNft.sol

// Single Opensale token

pragma solidity ^0.8.0;


contract SingleOpensaleNft is ERC721 {
    using SafeMath for uint256;

    uint256 public constant PERCENTS_DIVIDER = 100;
    uint256 public constant FEE_MAX_PERCENT = 20; // 20 %
    uint256 public constant FEE_MIN_PERCENT = 1; // 1 %
    ...
    constructor() ERC721("SingleOpensaleNft", "SingleOpensaleNft") {
        factory = msg.sender;
    }
     ...
     */
    function addItem(string memory _tokenURI, uint256 royalty)
        public
        returns (uint256)
    {
...
// File: contracts\SingleOpensaleNftFixed.sol

// Single Opensale Fixed Price Marketplace contract
...
contract SingleOpensaleNftFixed is Ownable, ERC721Holder {
    using SafeMath for uint256;
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    function initialize(address _feeAddress) external onlyOwner {
        ...
        createCollection(
            "Global",
            "https://opensale.mypinata.cloud/ipfs/QmbNwqMGawhsf7Kni9g2VSMCdfzQFKFJf197RfFVK1tjm6",
            true
        );
    }
    ...
    function createCollection(
        ...
        bytes memory bytecode = type(SingleOpensaleNft).creationCode;
        ...
        ISingleOpensaleNft(collection).initialize(
            _name,
            _uri,
            msg.sender,
            bPublic
    ...
    function singleList(
        ...
        ISingleOpensaleNft nft = ISingleOpensaleNft(_collection);
    ...
    function singleDelist(uint256 _id) external {
        ...
        ISingleOpensaleNft(pairs[_id].collection).safeTransferFrom(
            address(this),
            pairs[_id].owner,
            pairs[_id].tokenId
        );
    ...
    function singleBuy(uint256 _id) external payable {
        ...
        uint256 createrAmount = totalAmount.mul(pair.creatorFee).div(
            PERCENTS_DIVIDER
        );
    ...
    modifier OnlyItemOwner(address tokenAddress, uint256 tokenId) {
        ISingleOpensaleNft tokenContract = ISingleOpensaleNft(tokenAddress);
    ...
    modifier ItemExists(uint256 id) {
        require(
            id <= currentPairId && pairs[id].pairId == id,
            "Could not find item"
        );
        _;
    }
