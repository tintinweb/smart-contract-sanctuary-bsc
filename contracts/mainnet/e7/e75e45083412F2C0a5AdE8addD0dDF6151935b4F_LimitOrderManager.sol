...
contract LimitOrderManager is Base, IBiswapAddLimOrderCallback {
    ...
    // callback data passed through BiswapPoolV3#addLimOrderWithX(Y) to the callback
    ...
    function getEarnX(address pool, bytes32 key) private view returns(uint256, uint128, uint128) {
        (uint256 lastAccEarn, , , uint128 earn, uint128 legacyEarn, ) = IBiswapPoolV3(pool).userEarnX(key);
    ...
    function getEarnY(address pool, bytes32 key) private view returns(uint256, uint128, uint128) {
        (uint256 lastAccEarn, , , uint128 earn, uint128 legacyEarn, ) = IBiswapPoolV3(pool).userEarnY(key);
        return (lastAccEarn, earn, legacyEarn);
    }
    ...
    function _addLimOrder(
        ...
        if (addLimitOrderParam.sellXEarnY) {
            (order, acquire) = IBiswapPoolV3(pool).addLimOrderWithX(
            ...
            (order, acquire) = IBiswapPoolV3(pool).addLimOrderWithY(
                address(this), addLimitOrderParam.pt, addLimitOrderParam.amount,
                abi.encode(LimCallbackData({tokenX: addLimitOrderParam.tokenX, tokenY: addLimitOrderParam.tokenY, fee: addLimitOrderParam.fee, payer: msg.sender}))
            );
        }
    ...
    function newLimOrder(
        ...
        address pool = IBiswapFactoryV3(factory).pool(addLimitOrderParam.tokenX, addLimitOrderParam.tokenY, addLimitOrderParam.fee);
    ...
    function assignLimOrderEarn(
        ...
        if (isEarnY) {
            actualAssign = IBiswapPoolV3(pool).assignLimOrderEarnY(pt, amount, fromLegacy);
        } else {
            actualAssign = IBiswapPoolV3(pool).assignLimOrderEarnX(pt, amount, fromLegacy);
        }
    }
    ...
    function _updateOrder(
        ...
        if (order.sellXEarnY) {
            (, legacyAccEarn) = IBiswapPoolV3(pool).decLimOrderWithX(order.pt, 0);
        } else {
            (, legacyAccEarn) = IBiswapPoolV3(pool).decLimOrderWithY(order.pt, 0);
        }
        uint128 sold;
    ...
    function decLimOrder(
        ...
        if (order.sellXEarnY) {
            (actualDeltaRefund, ) = IBiswapPoolV3(pool).decLimOrderWithX(order.pt, actualDelta);
        } else {
            (actualDeltaRefund, ) = IBiswapPoolV3(pool).decLimOrderWithY(order.pt, actualDelta);
        }
    ...
    function collectLimOrder(
        ...
        IBiswapPoolV3(pool).collectLimOrder(recipient, order.pt, actualCollectDec, actualCollectEarn, order.sellXEarnY);
