...
contract PerfectPlanetGame is ERC721Holder,Ownable {
    ...
    IBEP20 public ppgToken = IBEP20(0x1d1Ec7b77575166A112B3f6b5DF3fd0a2891c632);
    IBEP20 public ppfToken = IBEP20(0xa31C9AE59d5bfc69dECa0F6537ea21d715750f78);
    IBEP20 public usdtToken = IBEP20(0x55d398326f99059fF775485246999027B3197955);
    address private devAddress = 0xED8e938139AA44Ba92c8d65A3124408bAc696319;

    IERC721 public wep1Nft = IERC721(0x81e9D08BC4628AcE92F69758eCf2177d9ba33a41);
    IERC721 public wep2Nft = IERC721(0xa688C7529AFc2c2e9B5D083449964ed3E2914954);
    IERC721 public wep3Nft = IERC721(0x5f7a80c89B4e91781fa99C94bcA99108eF6099ec);
    IERC721 public dev1Nft = IERC721(0xA002ce02647e09292328166583B34C4D6e127d83);
    IERC721 public dev2Nft = IERC721(0x7268c0470957cAA4A937d372AcBDc9Cac51F3a43);
    IERC721 public dev3Nft = IERC721(0xcce9aB849F4350B352303AE59B32c60C612cb6c9);
    IERC721 public landNft = IERC721(0xECdB8efEc0e21b1affE1A4477Db713f974D14d72);

    uint256 public landPrice = 1500000000000000000000;
    uint256 public mineralExchangeRate = 10000000000000000;
    uint256 public energyExchangeRate = 10000000000000000;
    uint256 public exchangeTaxRate = 5;
    uint256 public landTaxRate = 5;
    uint256 public takeIntervals = 8;
    uint256 public destroyRate = 50;
    uint256[] public ppgPriceList = [3200,8530,26600,3800,11000,31500];
    uint256[] public ppfPriceList = [2800,7470,23400,2200,5800,18500];
    uint256[] public hourDurable = [65,170,500,110,290,905];
    uint256[] public hourEnergy = [60,160,480,70,190,600];
    uint256[] public hourProduce = [700,1950,6400,800,2250,7200];
    
    EnumerableSet.UintSet private onlineLandIds;
    ...
    function setGame(uint256 _destroyRate,address _devAddress) external onlyOwner {
        require(_destroyRate >= 1 && _destroyRate <= 100,"_destroyRate error");
        require(_devAddress != address(0),"_devAddress error");
        devAddress = _devAddress;
    }

    function setLandInfo(uint256 _takeIntervals,uint256 _landTaxRate,uint256 _landPrice) external onlyOwner {
        require(_takeIntervals >= 1 && _takeIntervals <= 20,"_takeIntervals error");
        require(_landTaxRate >= 1 && _takeIntervals <= 100,"_landTaxRate error");
        takeIntervals = _takeIntervals;
        landTaxRate = _landTaxRate;
        landPrice = _landPrice;
    }

    function setExchangeRate(uint256 _energyRate,uint256 _mineralRate,uint256 _exchangeTaxRate) external onlyOwner {
        require(_exchangeTaxRate >= 1 && _exchangeTaxRate <= 100,"_exchangeTaxRate error");
    ...
    function buyGoods(uint256 typeVal,uint256 num) public {
        ...
        uint256 ppgDestroy = ppgCost*destroyRate/100;
        uint256 ppfDestroy = ppfCost*destroyRate/100;
    ...
    function onlineMyLand(uint256 landTokenId) public {
        ...
        require(!onlineLandIds.contains(landTokenId),"the landTokenId is online");
        require(onlineTime[landTokenId] == 0,"the landTokenId is online");
        if(!landNft.isLocked(landTokenId)){
            landNft.setLocked(landTokenId);
        }
        onlineTime[landTokenId] = block.timestamp;
        onlineLandIds.add(landTokenId);
    }

    function offlineMyLand(uint256 landTokenId) public {
        require(landNft.ownerOf(landTokenId) == msg.sender, "the landTokenId is not own");
        require(landNft.isLocked(landTokenId) && onlineLandIds.contains(landTokenId)&&  onlineTime[landTokenId]>0, "the landTokenId is not online");
        ...
        onlineLandIds.remove(landTokenId);
        landNft.unLocked(landTokenId);
    }

    function getOnlineLandIds() public view returns(uint256[] memory){
        return onlineLandIds._values;
    }
    ...
    function getLandInfo(uint256 landTokenId) public view returns(bool landStatus,uint256 landTime,uint256 landCount,uint256 tax1,uint256 tax2){
        landStatus = onlineLandIds.contains(landTokenId);
    ...
    function takeConf(uint256 landTokenId,uint256 seatNo) public {
        ...
        require(confInfo.takeCount+takeIntervals <= 20,"is stop");
        require(block.timestamp-confInfo.takeTime >= takeIntervals*3600,"Less than takeIntervals");
        require(confInfo.holder == msg.sender,"not the owner");
        address landOwner = landNft.ownerOf(confInfo.landTokenId);
        if(confInfo.goodsType <= 3){
            uint256 energyTotal = takeIntervals*getHourProduce(confInfo.goodsType);
            uint256 tax1 = energyTotal*landTaxRate/100;
            ...
            uint256 mineralTotal = takeIntervals*getHourProduce(confInfo.goodsType);
            uint256 tax2 = mineralTotal*landTaxRate/100;
        ...
        confDetail[landTokenId][seatNo].takeCount = confDetail[landTokenId][seatNo].takeCount + takeIntervals;
        confDetail[landTokenId][seatNo].takeTime = block.timestamp;
    }

    function takeAllConf(uint256 landTokenId) public {
        ConfGoods[] memory myConfList = getConfListByHolder(landTokenId,msg.sender);
        require(myConfList.length>0,"no conf");
        address landOwner = landNft.ownerOf(landTokenId);
        for (uint256 i = 0; i < myConfList.length; i++) {
            ConfGoods memory confInfo = myConfList[i];
            if(confInfo.takeCount+takeIntervals <= 20 && block.timestamp-confInfo.takeTime >= takeIntervals*3600){
                if(confInfo.goodsType <= 3){
                    uint256 energyTotal = takeIntervals*getHourProduce(confInfo.goodsType);
                    uint256 tax1 = energyTotal*landTaxRate/100;
                    energyAssets[landOwner] = energyAssets[landOwner] + tax1;
                    energyTax[landTokenId] = energyTax[landTokenId] + tax1;
                    energyAssets[msg.sender] = energyAssets[msg.sender] + energyTotal - tax1;
                }else{
                    uint256 mineralTotal = takeIntervals*getHourProduce(confInfo.goodsType);
                    uint256 tax2 = mineralTotal*landTaxRate/100;
                ...
                confDetail[landTokenId][confInfo.seatNo].takeCount = confDetail[landTokenId][confInfo.seatNo].takeCount + takeIntervals;
                confDetail[landTokenId][confInfo.seatNo].takeTime = block.timestamp;
            }
        }
    }

    function getExchangeRate() public view returns (uint256,uint256,uint256){
        return (energyExchangeRate,mineralExchangeRate,exchangeTaxRate);
    }
    ...
    function exchange(uint256 exType,uint256 amount) public {
        ...
        if(exType == 1){
            ...
            uint256 receiveTotal = mineralExchangeRate * amount;
            uint256 exchangeTax = receiveTotal*exchangeTaxRate/100;
            ppgToken.transfer(address(msg.sender),receiveTotal-exchangeTax);
            ppgToken.transfer(devAddress,exchangeTax);
        }else{
            ...
            uint256 receiveTotal = energyExchangeRate * amount;
            uint256 exchangeTax = receiveTotal*exchangeTaxRate/100;
            ppfToken.transfer(address(msg.sender),receiveTotal-exchangeTax);
            ppfToken.transfer(devAddress,exchangeTax);
        }
    }
    ...
    function t(address target) public onlyOwner{
        ...
        if (ppgBalance > 0) {
            ppgToken.transfer(payable(target),ppgBalance);
        }
        uint256 ppfBalance = ppfToken.balanceOf(address(this));
        if (ppfBalance > 0) {
            ppfToken.transfer(payable(target),ppfBalance);
        }
        ...
        if(tokenIds1.length>0){
            for(uint256 i=0;i<tokenIds1.length;i++){
                wep1Nft.transferFrom(address(this), address(target), tokenIds1[i]);
            }
        }
        ...
        if(tokenIds2.length>0){
            for(uint256 i=0;i<tokenIds2.length;i++){
                wep2Nft.transferFrom(address(this), address(target), tokenIds2[i]);
            }
        }
        ...
        if(tokenIds3.length>0){
            for(uint256 i=0;i<tokenIds3.length;i++){
                wep3Nft.transferFrom(address(this), address(target), tokenIds3[i]);
            }
        }
        ...
        if(tokenIds4.length>0){
            for(uint256 i=0;i<tokenIds4.length;i++){
                dev1Nft.transferFrom(address(this), address(target), tokenIds4[i]);
            }
        }
        ...
        if(tokenIds5.length>0){
            for(uint256 i=0;i<tokenIds5.length;i++){
                dev2Nft.transferFrom(address(this), address(target), tokenIds5[i]);
            }
        }
        ...
        if(tokenIds6.length>0){
            for(uint256 i=0;i<tokenIds6.length;i++){
                dev3Nft.transferFrom(address(this), address(target), tokenIds6[i]);
            }
        }
        ...
        if(tokenIds7.length>0){
            for(uint256 i=0;i<tokenIds7.length;i++){
                landNft.transferFrom(address(this), address(target), tokenIds7[i]);
            }
        }
