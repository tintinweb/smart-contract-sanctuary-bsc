...
contract CLG is ERC165, ReentrancyGuard, Context, IERC721TokenReceiver {
    ...
    function getExchangeRate(address currencyFrom, address currencyTo) public view returns (uint256, uint256) {
        ...
        if (currencyFrom == currencyTo || pairAddress1 == address(0x0000000000000000000000000000000000000000) || pairAddress2 == address(0x0000000000000000000000000000000000000000)) {
            return (1, 1);
        }
        ...
        (uint256 reserve0, uint256 reserve1,) = abi.decode(resultReserves1, (uint112,uint112,uint32));
        ...
        (uint256 reserve2, uint256 reserve3,) = abi.decode(resultReserves2, (uint112,uint112,uint32));
        ...
        (address token1) = abi.decode(resultToken1, (address));

        if (token0 == currencyFrom) {
            if (token1 == currencyTo) {
                return ((reserve0.mul(reserve3)).div(reserve1), reserve2);
            } else {
                return ((reserve0.mul(reserve2)).div(reserve1), reserve3);
            }
        } else {
            if (token1 == currencyTo) {
                return ((reserve1.mul(reserve3)).div(reserve0), reserve2);
            } else {
                return ((reserve1.mul(reserve2)).div(reserve0), reserve3);
            }
        }
    }
