...
abstract contract AbsToken is IERC20, Ownable {
    ...
    bool public limitBuyEnable = false;
    ...
    uint256 public _tokenDividendMin = 0 * 10 ** _decimals;
    uint256 public _buyFundFee = 600;
    uint256 public _buyLPDividendFee = 0;
    uint256 public _buyLPFee = 0;  //default 0
    uint256 public _buyTokenFee = 0;  //default 0

    uint256 public _sellFundFee = 100;
    uint256 public _sellLPDividendFee = 200;
    uint256 public _sellLPFee = 100;
    uint256 public _sellTokenFee = 200;
...
//        }
                ...
                if (_swapPairList[to]) {
                    if (!inSwap) {
                        ...
                        if (contractTokenBalance > 0) {
                            uint256 swapFee = _buyFundFee + _buyLPDividendFee + _buyLPFee + _buyTokenFee + _sellFundFee + _sellLPDividendFee + _sellLPFee + _sellTokenFee;
        ...
        //Dividend
        if (_sellLPDividendFee>0 || _buyLPDividendFee>0){
    ...
    function processReward(uint256 gas) private {
        ...
        //Dividend =====> swap usdt to doge and dividend by this token holder amount or dividend usdt
        swapUsdtForTokens(balance, _doge);

        IERC20 DOGE = IERC20(_doge);
        balance = DOGE.balanceOf(address(this));
        if (balance < 0) {
            return;
        }
        ...
        holdTokenTotal -= balanceOf(_mainPair);
//        holdTokenTotal -= balanceOf(address(0x000000000000000000000000000000000000dEaD));
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            if (tokenBalance > _tokenDividendMin && !excludeHolder[shareHolder]) {
    ...
    function swapUsdtForTokens(uint256 tokenAmount, address tokenContract) internal {
        address[] memory path = new address[](2);
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            uint256 swapFee;
            if (isSell) {
                swapFee = _sellFundFee + _sellLPDividendFee + _sellLPFee + _sellTokenFee;
                require(tAmount <= maxSellAmount, "over max sell amount");
            } else {
                swapFee = _buyFundFee + _buyLPDividendFee + _buyLPFee + _buyTokenFee;
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        uint256 BuyTokenAmount = usdtBalance * (_sellTokenFee+_buyTokenFee) * 2 / swapFee;
        ...
        //Buy This Token
        if (BuyTokenAmount > 0) {
            uint256 _buyUsdtAmount = BuyTokenAmount;
            if  (_buyUsdtAmount>USDT.balanceOf(address(this))) {
                _buyUsdtAmount=USDT.balanceOf(address(this));
            }
            swapUsdtForTokens(_buyUsdtAmount, address(this));
        }
    }
    ...
    function setTokenDividendMin(uint256 tokenDividendMin) external onlyOwner {
        _tokenDividendMin = tokenDividendMin;
    }
    ...
    function setBuyTokenFee(uint256 buyTokenFee) external onlyOwner {
        _buyTokenFee = buyTokenFee;
    }
    ...
    function setSellTokenFee(uint256 sellTokenFee) external onlyOwner {
        _sellTokenFee = sellTokenFee;
    }
...
contract ChatGPT is AbsToken {
    constructor() AbsToken(
        ...
        address(0xbA2aE424d960c26247Dd6c32edC70B295c744C43), //BSC DOGEAddress
        "ChatGPT", //Name
        "ChatGPT", //Symbol
        9, //Decimals
        1000000_00000000, //Supply
        address(0xF69546310f318bdbbCD9252b8E4655f444abB2a8)//FundAddress
    ){
    }
}