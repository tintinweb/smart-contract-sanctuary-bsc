...
contract GoldPay is ERC20, Ownable {
    ...
    address public  treasuryWallet = 0xF8280F7A5cc1A58C2dbb5F23d7160ae79d6dEeab;
    ...
    uint256 private accumulatedBuybackBNB;

    mapping (address => bool) private _isExcludedFromFees;
    mapping (address => bool) public automatedMarketMakerPairs;

    event ExcludeFromFees(address indexed account, bool isExcluded);
    event BuyFeesUpdated(uint256 treasuryFeeOnBuy, uint256 buyBackFeeOnBuy);
    event SellFeesUpdated(uint256 treasuryFeeOnSell, uint256 buyBackFeeOnSell);
    ...
    constructor () ERC20("GoldPay", "$GoldPay") 
    {   
        address newOwner = 0x6F49dDEf21eb063244B3F3b8775c1f9c0510C1fc;
        ...
        _mint(newOwner, 1e6 * (10 ** 18));
        swapTokensAtAmount = totalSupply() / 2500;
    }
    ...
    function updateBuyFees(uint256 _treasuryFeeOnBuy, uint256 _buyBackFeeOnBuy) external onlyOwner {
        require(_treasuryFeeOnBuy + _buyBackFeeOnBuy <= 10, "Fees cannot be more than 10%");

        treasuryFeeOnBuy = _treasuryFeeOnBuy;
        buyBackFeeOnBuy = _buyBackFeeOnBuy;
        buyFee = _treasuryFeeOnBuy+_buyBackFeeOnBuy;
        emit BuyFeesUpdated(_treasuryFeeOnBuy, _buyBackFeeOnBuy);
    }

    function updateSellFees(uint256 _treasuryFeeOnSell, uint256 _buyBackFeeOnSell) external onlyOwner {
        require(_treasuryFeeOnSell + _buyBackFeeOnSell <= 15, "Fees cannot be more than 15%");

        treasuryFeeOnSell = _treasuryFeeOnSell;
        buyBackFeeOnSell = _buyBackFeeOnSell;
        sellFee = _treasuryFeeOnSell+_buyBackFeeOnSell;
        emit SellFeesUpdated(_treasuryFeeOnSell, _buyBackFeeOnSell);
    }
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            uint256 buyBackShare = (newBalance*buyBackTokens) / contractTokenBalance;
            uint256 treasuryShare = newBalance - buyBackShare;
            
            if(treasuryShare > 0 && (treasuryFeeOnBuy + treasuryFeeOnSell> 0 )) {
                sendBNB(payable(treasuryWallet), treasuryShare);
            }  

            if (buyBackShare > 0 && (buyBackFeeOnBuy + buyBackFeeOnSell> 0 )) {

                accumulatedBuybackBNB += buyBackShare;
        ...
        if(takeFee) {
            ...
            if(from == uniswapV2Pair && buyFee > 0) {
                ...
                amount = amount - fees;
                buyBackTokens += (fees * buyBackFeeOnBuy) / buyFee;
            } 
            else if(to == uniswapV2Pair && sellFee > 0) {
                fees = amount * sellFee / 100;
                amount = amount - fees;
                buyBackTokens += (fees * buyBackFeeOnSell) / sellFee;
            }
            else{
                fees = amount * walletToWalletFee / 100;
                amount = amount - fees;
                buyBackTokens += fees;
            }
