/**
 * @notice Submitted for verification at bscscan.com on 2022-
 */
...
import "../../interfaces/ICloneInitializer.sol";
 ...
 * @dev Based on the code of Foundation at 0x3b612a5b49e025a6e4ba4ee4fb1ef46d13588059 `FNDCollectionFactory.sol`
 */
contract BMNCollectionFactory is ICloneFactory, Ownable {
    ...
    ///@dev The address of the BlockMelon market contract
    address public marketContract;
    ///@dev The address of the template all new collections will leverage.
    ...
    /**
     * @notice Creates a new collection contract.
     * @param name   The name of the new collection contract
     * @param symbol The token symbol of the new collection
    ...
    function createCollection(
        ...
        require(
            ...
            "caller is not an approved creator"
        );
        ...
        address proxy = implementation.cloneDeterministic(
            _getSalt(_msgSender(), nonce)
        );

        ICloneInitializer(proxy).initialize(
            payable(_msgSender()),
            name,
            symbol,
            marketContract
    ...
    /**
     * @notice Allows BlockMelon to change the collection implementation used for future collections.
     * This call will auto-increment the version.
     * Existing collections are not impacted.
     */
    function adminUpdateImplementation(address _implementation)
        external
        onlyBlockMelonAdmin
    {
    ...
    /**
     * @notice Allows a BlockMelon admin to change the market contract address.
     */
    function updateMarketContract(address _marketContract)
        external
        onlyBlockMelonAdmin
    {
        _updateMarketContract(_marketContract);
    ...
    function _updateMarketContract(address _marketContract)
        private
        isContract(_marketContract)
    {
     ...
     */
    function _updateImplementation(address _implementation)
        private
        isContract(_implementation)
    {
        ...
        ICloneInitializer(_implementation).initialize(
            payable(owner()),
            string(
                abi.encodePacked(
                    "BlockMelon Collection Template v",
                    version.toString()
                )
            ),
            string(abi.encodePacked("BMNv", version.toString())),
            marketContract
        );
...
/**
 * @notice Interface for cloned contract initialization
 */
interface ICloneInitializer {
    function initialize(
...
/**
 * @notice Interface for a minimal clone contract factory
 */
interface ICloneFactory {
    ///@dev Getter for the admin role cotract
    function adminContract() external returns (IAdminRole);
}
