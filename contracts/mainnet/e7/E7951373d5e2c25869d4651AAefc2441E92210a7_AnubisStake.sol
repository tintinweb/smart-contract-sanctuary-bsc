...
contract AnubisStake {
    ...
    //STAKING DURATION
    uint256[4] public Duration = [15 days, 30 days, 60 days, 90 days]; // [30 days, 60 days, 90 days, 180 days];

    //APY BONUS IN PERCENTAGE 
    uint256[4] public Bonus = [700, 2000, 5000, 10000]; //  [1, 2, 3, 4];
    ...
    totalFee = 0; // 100;
    maxStakeableToken = 1e34; // 10^34
    percentDivider = 1000;
    minimumStakeToken = 1e18; // 10^18
}
    ...
    function stake(uint256 amount1, uint256 timeperiod) public {
        ...
        uint256 rewardtokenPrice = amount.div(1e4); // (amount.mul(TKNVAL)).div(1e9);
    ...
    //     address BUSD_WBNB = 0x19c174123f520Fe17069D7E8f8E1513E6Dd2e2F8; //BUSD_WBNB pancake pool address
    //     IERC20 BUSDTOKEN = IERC20(0x3D88115F6142B11eb7be323Cb919e28fc12b1C8A); // BUSD Token address
    //     IERC20 WBNBTOKEN = IERC20(0xF5755c04e6a31132Fc9c611e0c67877fca2C344a); // WBNB Token address
    ...
    //     address TKN_WBNB = 0x3BaB93341D37F9f6B4758D7fa5d569c6D41660FF; // TKN_WBNB pancake pool address
    //     IERC20 TKNTOKEN = IERC20(0x4b6a72c7271Ae82F5A7396edEC2Dc9322922A192); // TKN Token address 
    ...
    function initStakeToken(address addr) public onlyowner{
        stakeToken = IERC20(addr);
    }

    function initRewardToken(address addr) public onlyowner{
