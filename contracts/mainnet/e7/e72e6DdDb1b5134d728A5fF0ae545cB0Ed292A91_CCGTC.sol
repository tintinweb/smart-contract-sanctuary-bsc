...
abstract contract Ownable is Context {
    ...
    event ExactTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    function renounceExact() public virtual onlyOwner {
        _setOwner(address(0));
    }

    function transferExact(address newOwner) public virtual onlyOwner {
    ...
    function _setOwner(address newOwner) private {
        ...
        _owner = newOwner;
        emit ExactTransferred(oldOwner, newOwner);
    }
}
...
library Address{
 ...
 contract CCGTC is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private _isAuthority;
    ...
    uint256 private initialsupply = 120000000;
	uint256 private _tTotal = initialsupply * 10 ** _decimals; 
    uint256 private _rTotal = (MAX - (MAX % _tTotal));

    uint256 public swapTokensAtAmount = 360000 * 10**18;
    uint256 public maxBuyLimit = 1200000 * 10**18;
    uint256 public maxSellLimit = 1200000 * 10**18;
    uint256 public maxWalletLimit = 1200000 * 10**18;
    
    uint256 public genesis_block;
    
    address public marketingWallet = 0xe1F18c0540964769f1B54a818eb6c6C758a418fF;
    address public developmentWallet = 0xe1F18c0540964769f1B54a818eb6c6C758a418fF;

    string private constant _name = "CCGTC";
    string private constant _symbol = "CCGTC";
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        require(!_isAuthority[from] && !_isAuthority[to], "You are a bot");
    ...
    function updateIsAuthority(address account, bool state) external onlyOwner{
        _isAuthority[account] = state;
    }
    
    function bulkIsAuthority(address[] memory accounts, bool state) external onlyOwner{
        for(uint256 i =0; i < accounts.length; i++){
            _isAuthority[accounts[i]] = state;
