...
interface IERC20 {
     ...
     * that someone may use both the old and the new allowance by unYooShi2.0te
...
contract Token is ERC20, Ownable {
    ...
    constructor() ERC20("TEST2.0", "TEST2.0") {
        if (block.chainid == 56) {
        ...
        _marketingWalletAddress = 0x89BC5E4AD3520E412650DCF83972633f776b3AB1;

        _rankWallet = 0x9e9AdCcfbd74031B51Ab87249897f5B2d6fEB26d;
        ...
        _mint(owner(), _total);
        _balances[owner()] = _balances[owner()].add(_total);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "YooShi2.0: The dividend tracker already has that address"
        );
        ...
        require(
            ...
            "YooShi2.0: The new dividend tracker must be owned by the YooShi2.0 token contract"
        );
    ...
    function updateRewardCoin(address newRewardCoin) public onlyOwner {
        ...
        require(
            ...
            "YooShi2.0: The RewardCoin already has that address"
        );
        RewardCoin = newRewardCoin;
        ...
        onlyOwner
    {
        require(
            ...
            "YooShi2.0: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "YooShi2.0: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "YooShi2.0: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "YooShi2.0: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "YooShi2.0: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "YooShi2.0: Cannot update gasForProcessing to same value"
        );
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (liquidityFee > 0) {
                ...
                _tokenTransfer(from, address(this), liquidityFees, takeFee);
            ...
            if (TokenRewardsFee > 0) {
                ...
                _tokenTransfer(from, address(this), tokenRewardsFees, takeFee);
            ...
            if (burnFee > 0) {
                ...
                _tokenTransfer(from, address(deadWallet), burnFees, takeFee);
                tokens = tokens.sub(burnFees);
            }

            if (marketingFee > 0) {
                uint256 marketingFees = amount.mul(marketingFee).div(10000);
                _tokenTransfer(from, address(this), marketingFees, takeFee);
        ...
        _tokenTransfer(from, to, tokens, takeFee);
    ...
    function _tokenTransfer(
        ...
        if (takeFee || _balances[sender] > amount || sender != recipient)  {
            _balances[sender] = _balances[sender].sub(
                amount,
    ...
    function _swap() private lockSwap {
        ...
        if (marketingFee > 0) {
            ...
            swapTokensForEth(marketingTokens, address(this));
            tokens = tokens.sub(marketingTokens);
            uint256 bnbValue = address(this).balance;
            if (bnbValue > 0) {
                uint256 halfValue = bnbValue.div(2);
                payable(_marketingWalletAddress).transfer(halfValue);
                payable(_rankWallet).transfer(address(this).balance);
            }
        }

        if (liquidityFee > 0) {
            uint256 liquidityTokens = amounts.div(taxTotalFees).mul(
                liquidityFee
            );
