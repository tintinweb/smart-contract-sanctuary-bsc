...
contract HexLunc is ERC20 {
	...
	address private _fundAddress = address(0x1F856870C9C2Ea2A6E3C0eA73522e451B7C377CD);
	address private _cardAddress = address(0xe3C7Ceb41eD5aEd05E840F43934E2871266b3d73);
	address private _marketAddress = address(0xaE1d91bBc7a3CbABC9b1227B7d5C3D59af5bA30B);
    ...
    constructor(address tokenOwner) ERC20("HexLunc", "HexLunc") {
	...
	function changeSwapWarp(HexLuncWarp _warp) public onlyOwner {
        warp = _warp;
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(automatedMarketMakerPairs[from]){
				if(startTime.add(120) > block.timestamp){
	...
	function swapAndLiquify() public {
		...
		if(allAmount > 10**26){
			...
			if(allusdtAmount > 0){
				USDT.transfer(_fundAddress,allusdtAmount.div(8).mul(3));
				USDT.transfer(_cardAddress,allusdtAmount.div(8).mul(3));
				USDT.transfer(_marketAddress,allusdtAmount.div(8));
				USDT.transfer(_tokenOwner,allusdtAmount.div(8));
			}
		}else{
			return;
		}
