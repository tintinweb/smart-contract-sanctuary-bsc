...
contract NACHOCHEESE is BEP20Detailed, Auth {
    ...
    mapping(address => bool) private currentBigBuyer;
    ...
    function checkCooldown(address sender, address recipient, uint256 amount) internal view {
        ...
        if(sender != pair && currentBigBuyer[sender] && block.timestamp <= bigBuyEnd && !_isFeeExempt[recipient]){
            require(cAmount <= _balances[sender].sub(bigBuyAmount[sender].mul(_PerFragment)), "Big Buy Competition in process");}
    ...
    function bigBuyCompetition(address recipient, uint256 amount) internal {
        ...
        if(bigBuyComp && block.timestamp > bigBuyEnd){
            bigBuyComp = false; 
            currentBigBuyer[biggestBuyer] = false;
    ...
    function checkBiggestBuy(address recipient, uint256 amount) internal {
        ...
        if(amount > biggestBuy){
            ...
            currentBigBuyer[recipient] = true;
