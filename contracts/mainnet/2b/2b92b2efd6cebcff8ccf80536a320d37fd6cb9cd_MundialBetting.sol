...
contract MundialBetting is Auth {
    ...
    IERC20 public token;
    uint256 public minBetAmt;
    uint256 public maxBetAmt;
    ...
    constructor(IERC20 _token, uint256 _minBetAmt, uint256 _maxBetAmt) Auth(msg.sender){
        token = _token;
        minBetAmt = _minBetAmt * 1e18;
        maxBetAmt = _maxBetAmt * 1e18;
    }
    ...
    function placeBet(uint256 _index, uint256 _outcome, uint256 _amount) external callerIsUser {
        require(_amount > minBetAmt && _amount < maxBetAmt, "Amount should be greater than the min bet amt and lower than max!");
    ...
    function setBetAmt(uint256 _minBetAmt, uint256 _maxBetAmt) external onlyOwner {
        require(_maxBetAmt > _minBetAmt, "Max bet amount has to be bigger than min bet amount.");
