...
/*
WittyCat is a witty meme token based on BSC originated from the trending hype and exceptional success of Cat coins this year. Cat hype started after Elon Musk tweeted that he got a cat, finally there is a possibility of cat takeover! You can't miss this

8% Buy Tax and 8% Sell Tax
- 2% Buyback and Burn
- 3% Marketing
- 2% Liquidity
- 1% Ops

Telegram: https://t.me/wittycatofficial
*/
...
contract Wittycat is IERC20, Ownable {
    ...
    uint256 FeeLiquidity = 8;
    uint256 giveawayFee = 1;
    uint256 marketingFeeVariable = 8;
    ...
    function viewFees() external view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        return (FeeLiquidity, marketingFeeVariable, giveawayFee, reflectionFee, totalFee, feeDenominator);
    }
    ...
    function changeMaxTx(uint256 maxTx) external onlyOwner {
        _maxTxAmount = 1000000000000000 * 10 ** _decimals * maxTx / 1000;
    }
    ...
    function changeFees(uint256 _FeeLiquidity, uint256 _reflectionFee, uint256 _marketingFeeVariable, uint256 _giveawayFee) external onlyOwner {
        FeeLiquidity = _FeeLiquidity;
        reflectionFee = _reflectionFee;
        marketingFeeVariable = _marketingFeeVariable;
        giveawayFee = _giveawayFee;
        totalFee = FeeLiquidity.add(reflectionFee).add(marketingFeeVariable).add(giveawayFee);
