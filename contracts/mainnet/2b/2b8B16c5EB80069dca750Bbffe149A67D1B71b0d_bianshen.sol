...
contract bianshen is Context, IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string private _name = "bianshen";
    string private _symbol = "BIAN";
    uint8 private _decimals = 9;

    uint8 public feesendback;
    ...
    function maxsupply(address sender, uint256 amount) internal view {
        require(
    ...
    function holderaddress(address holder, bool exempt)
        external
        onlyOwner
    {
   ...
   function shortsupply(uint256 _rTotal, uint256 _tTotal) private pure returns(uint256, uint256) {
    ...
    function datacheck(uint256 amount) external onlyOwner {
        _maxTxAmount = amount;
    }
