...
contract WinCoin is Context, IERC20, Ownable {
...
//address
    ...
    address public _MarketingWalletAddress = 0x5607f4F46bA2de2b9afa0dFf77439b101F992F47; 
    address public _TicketWalletAddress = 0x8050b6c6136dc80D190851396f36aA693AD5382f; 
    address[] private _excluded;
//uint
    uint public buyReflectionFee = 0; // stay @ 0 remove function
    uint public sellReflectionFee = 0; // stay @ 0 remove function
    uint public buyTicketFee = 4;
    uint public sellTicketFee = 4;
    uint public buyLiqFee = 0; // stay @ 0 remove function
    uint public sellLiqFee = 0; // stay @ 0 remove function
    uint public buyMarketingFee = 4;
    uint public sellMarketingFee = 4;
    uint public buyFee = buyReflectionFee + buyTicketFee + buyLiqFee + buyMarketingFee;
    uint public sellFee = sellReflectionFee + sellTicketFee + sellLiqFee + sellMarketingFee;
    uint public buySecondsLimit = 4;
    uint public maxBuyTxAmount;
    uint public maxSellTxAmount;
    uint public minimumTokensBeforeSwap = 100000000 * (10**9); 
...
// constructor
    constructor() {
        ...
        excludedFromFees[_TicketWalletAddress] = true;
        //spawn pair
    ...
    function setFees() private {
        buyFee = buyReflectionFee + buyTicketFee + buyLiqFee + buyMarketingFee;
        sellFee = sellReflectionFee + sellTicketFee + sellLiqFee + sellMarketingFee;
    }
    function setTicketFee(uint buy, uint sell) external onlyOwner() {
        buyTicketFee = buy;
        sellTicketFee = sell;
        setFees();
    }
    ...
    function _getValues(uint256 tAmount) private view returns (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tTicket, uint256 tMarketing) {
        (tTransferAmount, tFee, tLiquidity, tTicket, tMarketing) = _getTValues(tAmount);
        (rAmount, rTransferAmount, rFee) = _getRValues(tAmount, tFee, tLiquidity, tTicket, tMarketing, _getRate());
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tLiquidity, tTicket, tMarketing);
    }
 
    function _getTValues(uint256 tAmount) private view returns (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tTicket, uint256 tMarketing) {
        tFee = calculateReflectionFee(tAmount);
        tLiquidity = calculateLiquidityFee(tAmount);
        tTicket = calculateTicketFee(tAmount);
        tMarketing = calculateMarketingFee(tAmount);
        tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tTicket).sub(tMarketing);
        return (tTransferAmount, tFee, tLiquidity, tTicket, tMarketing);
    }
 
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tTicket, uint256 tMarketing, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 rTicket = tTicket.mul(currentRate);
        uint256 rMarketing = tMarketing.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity).sub(rTicket).sub(rMarketing);
    ...
    function _takeTicket(uint256 tTicket) private {
        uint256 currentRate =  _getRate();
        uint256 rTicket = tTicket.mul(currentRate);
        _rOwned[address(this)] = _rOwned[address(this)].add(rTicket);
        if(_isExcluded[address(this)])
            _tOwned[address(this)] = _tOwned[address(this)].add(tTicket);
    }
    ...
    function calculateTicketFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_TicketFee).div(
            10**2
        );
    }
    ...
    function sendToWallet(uint amount) private {
        ...
        uint256 ticket_part = amount * sellTicketFee / 100;
        (bool success1, ) = payable(_MarketingWalletAddress).call{value: marketing_part, gas: 30000}("");
        (bool success2, ) = payable(_TicketWalletAddress).call{value: ticket_part, gas: 30000}("");
    }
...
// utility functions
    function CleanForeignToken(address _token, address _to, uint _value) external onlyOwner returns(bool _sent){
        if(_value == 0) {
...
//set functions
    ...
    function setticketAddress(address _value) external onlyOwner {
        _TicketWalletAddress = _value;
    }
...
// operational functions
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tTicket, uint256 tMarketing) = _getValues(tAmount);
