...
contract MOVEY is ERC20Detailed, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;

    event LogRebase(uint256 indexed epoch, uint256 totalSupply);

    string public constant _name = "MOVEY TOKEN";
    string public constant _symbol = "MOVEY";
    ...
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 2e7 * 10**DECIMALS;

    uint256 public compoundingTime;

    uint256 public liquidityFee = 60;
    uint256 public treasuryFee = 40;
    uint256 public insuranceFundFee = 80;
    ...
    constructor() ERC20Detailed(_name, _symbol, uint8(DECIMALS)) Ownable() {
        ...
        autoLiquidityReceiver = 0x46ff3b5b05a9b4c55AD9c7D19eb9619923DeE8fF;
        treasuryReceiver = 0x1A09b440eD9f1e3FB3439b30d80C153fBb44aAA0;
        insuranceFundReceiver = 0x361Ec3e0934207C6d5fC8aFD7Ac8787e25a8b0a6;
    ...
    function takeFee(
        ...
        uint256 feeAmount = 0;
        if (recipient == pair) {
            feeAmount = gonAmount.div(feeDenominator).mul(totalFee());
        }
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(treasuryFee.add(insuranceFundFee))
        );
    ...
    function swapBack() internal swapping {
        ...
        (success, ) = payable(insuranceFundReceiver).call{
            value: amountETHToTreasuryAndHIF.mul(insuranceFundFee).div(
                treasuryFee.add(insuranceFundFee)
            ),
            gas: 30000
        }("");
    ...
    function setLiquidityFee(uint256 _liquidityFee) external onlyOwner {
        require(_liquidityFee <= maxLiquidityFee, "Over max fee");
        liquidityFee = _liquidityFee;
    }

    function setTreasuryFee(uint256 _treasuryFee) external onlyOwner {
        require(_treasuryFee <= maxTreasuryFee, "Over max fee");
        treasuryFee = _treasuryFee;
    }

    function setInsuranceFundFee(uint256 _insuranceFundFee) external onlyOwner {
        require(_insuranceFundFee <= maxInsuranceFundFee, "Over max fee");
