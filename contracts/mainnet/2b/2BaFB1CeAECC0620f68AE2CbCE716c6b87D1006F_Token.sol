...
/// @author  Wu (https://github.com/roger-wu)
...
contract Token is ERC20, Ownable, BaseToken {
    ...
    bool public swappingSwitsh = true;

    BABYTOKENDividendTracker public dividendTracker;

    address public rewardToken = payable(0x5fAc926Bf1e638944BB16fb5B787B5bA4BC85b0A);
	address public Awllet = payable(0x940d6Bc682bcdA87D334F9fFF93decDe1E40427b);
	address public Bwllet = payable(0xfaF9AfF62ecc2c5a07D79d267E93D5ac7CadbCcc);
	address public Cwllet = payable(0x66dfEC325661141fCC928326409451e1978708dC);
	address public deadAddress = payable(0x000000000000000000000000000000000000dEaD);
    uint256 public swapTokensAtAmount;

    uint256 public AFee = 15;
    uint256 public BFee = 15;
	uint256 public bonusFee = 80;
    uint256 public deadFee = 5;
	uint256 public CFee = 5;
	uint256 public contractFee = 115;
    ...
    constructor() ERC20("JFKing", "JFKing") {
        address specify = payable(0x4B98cE35d8727eE035209b27Dc9D210E07956B7C);
        uint256 totalSupply_ = 1000000000000000 * 10 ** 18;

        swapTokensAtAmount = 500000000 * 10 **18;

        // use by default 300,000 gas to process auto-claiming dividends
        gasForProcessing = 300000;

        dividendTracker = new BABYTOKENDividendTracker();
        dividendTracker.initialize(rewardToken, 10000000000000000000000);
        ...
        excludeFromFees(specify, true);
        excludeFromFees(address(this), true);
        excludeFromFees(Awllet, true);
        excludeFromFees(Bwllet, true);
        excludeFromFees(Cwllet, true);
        
        
        _mint(specify, totalSupply_);

        emit TokenCreated(specify, address(this), TokenType.baby, VERSION);
    }
    ...
    function setSwappingSwitsh(bool _swappingSwitsh) external onlyOwner {
        swappingSwitsh = _swappingSwitsh;
    }
	...
	function setAwllet(address _awllet) external onlyOwner {
	    Awllet = _awllet;
	}
	function setBwllet(address _bwllet) external onlyOwner {
	     Bwllet = _bwllet;
	}
	function setCwllet(address _cwllet) external onlyOwner {
	     Cwllet = _cwllet;
	}
    ...
    function _transfer(
        ...
        if ( tradeSwitsh && canSwap && !swapping &&
			...
			uint256 Aswap = contractTokenBalance.mul(AFee).div(contractFee);
			swapTokensForEth(Aswap,Awllet);
			
			uint256 Bswap = contractTokenBalance.mul(BFee).div(contractFee);
			swapTokensForEth(Bswap,Bwllet);
			
			uint256 Cswap = contractTokenBalance.mul(CFee).div(contractFee);
			swapAndLiquify(Cswap,Cwllet);
        ...
        if (takeFee) {
            uint256 coinFee = amount.mul(contractFee).div(1000);
            super._transfer(from, address(this), coinFee);

             uint256 _deadFee = amount.mul(deadFee).div(1000);
              super._transfer(from, deadAddress, _deadFee);

            amount = amount.sub(coinFee).sub(_deadFee);
        }
