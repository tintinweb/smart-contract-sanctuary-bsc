...
contract hotv is ERC20, Ownable {
    ...
    uint256 public totalFees = ETHRewardsFee.add(deadFee).add(marketingFee);
    bool public isL;
    uint256 public killNum = 3;
    uint256 public lunachB;

    address public _marketingWalletAddress = 0x9bd039dDfB5CBDce642D943523D6C000dC501841;
    ...
    constructor(address receiveAddress) public ERC20("Haya of the volcano", "HOTV") {
        ...
        */
        _mint(address(receiveAddress), 100000 * (10**18));
    }
    ...
    function setETHRewardsFee(uint256 value) external onlyOwner{
        ETHRewardsFee = value;
        totalFees = ETHRewardsFee.add(deadFee).add(marketingFee);
    }
    ...
    function _transfer(
        ...
        if(takeFee) {
        	...
        	uint256 fees = amount.mul(marketingFee+ETHRewardsFee).div(1000);
            
        	

            super._transfer(from, address(this), fees);
            uint256 dead_number = amount.mul(deadFee).div(1000);
            

            super._transfer(from, deadWallet, dead_number);
            amount = amount.sub(dead_number+fees);
        }
...
contract ETHBackDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor() public DividendPayingToken("ETHBack_Dividen_Tracker", "ETHBack_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 1 * (10**18); //must hold 200000+ tokens
    }
