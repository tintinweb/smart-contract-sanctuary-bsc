...
contract Creepers is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  constructor(string memory name, string memory symbol, string memory baseTokenURI) ERC721(name, symbol) {
      ...
      _wallet = 0x653B4734249dFC903dF888971a195A1097aaA55c;
      _regularMint();
  }
  ...
  function mint(uint256 _mintAmount) public payable {
    // Checks
    require(!paused, "Creepers: minting has not started yet");
    require(_mintAmount > 0, "Creepers: you have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "Creepers: you can only mint 10 at a time");
    require(lastSupply >= _mintAmount,  "Creepers: the collection is sold out");
    if (msg.sender != owner()) {
      require(msg.value >= cost * _mintAmount, "Creepers: total cost isn't match");
    }
    // Minting
  ...
  function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
    // Checks
    require(_winner != address(0), "Creepers: zero address");
    require(_amount > 0, "Creepers: you have to mint at least one");
    require(lastSupply != 0,  "Creepers: the collection is sold out");
  ...
  function tokenURI(uint256 _id) public view virtual override returns (string memory){
    require(_exists(_id),"Creepers: URI query for nonexistent token");
