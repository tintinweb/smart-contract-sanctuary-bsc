...
- REWARDS 7% ANGEL DUST!
- STRONG COMMUNITY
- LP LOCKED
- SAFU AND BASED DEV

Telegram - https://t.me/Micro_AngelDustHQ
...
contract MicroAngelDust is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    address DEV = 0xF208567f6ac43133BfB8699017f3aA43c627878d;
    ...
    string constant _name = "Micro Angel Dust";
    string constant _symbol = "$microAD";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 100000000000 * 10**0 * (10 ** _decimals);

    uint256 public _maxBuyTxAmount = _totalSupply * 100 / 10000; 
    uint256 private _maxSellTxAmount = _totalSupply * 100 / 10000; 
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0xF208567f6ac43133BfB8699017f3aA43c627878d;
    ...
    function getTotalFee(bool selling) public view returns (uint256) {
        if(releasedAt + supportFloor >= block.number){ return feeFromSupport.sub(1); }
    ...
    function takeFee(address sender, uint256 amount, bool isSell, bool GREEDMode) internal returns (uint256) {
        ...
        if (GREEDMode){
            if(isSell){
                feeAmount = amount.mul(totalFee).mul(3).div(2).div(feeFromSupport);
            } else {
                feeAmount = amount.mul(totalFee).div(2).div(feeFromSupport);
            }
        } else {
            feeAmount = amount.mul(totalFee).div(feeFromSupport);
        }
    ...
    function setBuyFees(uint256 _liquidityFeeBuy, uint256 _buybackFeeBuy, uint256 _reflectionFeeBuy, uint256 _marketingFeeBuy, uint256 _devFeeBuy, uint256 _feeFromSupport) external authorized {
        ...
        feeFromSupport = _feeFromSupport;
    }

    function setSellFees(uint256 _liquidityFeeSell, uint256 _buybackFeeSell, uint256 _reflectionFeeSell, uint256 _marketingFeeSell, uint256 _devFeeSell, uint256 _feeFromSupport) external authorized {
    ...
    function setRewardAddress(address _rewardAddress) external authorized {
    ...
    function setTargetLiquidity(uint256 _target, uint256 _feesupport) external authorized {
        targetLiquidity = _target;
        targetLiquidityDenominator = _feesupport;
    }
