    ...
    ERC721Burnable
{
    ...
    string private constant SIGNING_DOMAIN = "CowNFTSigning";
    ...
    bool public whiteListSaleEnable;
    ...
    address public fundReceiver = 0x27DF7e6A705270e088447eb09A273cdC81cB39b6;

    constructor()
        ERC721("CowNFT", "CowNFT")
        EIP712(SIGNING_DOMAIN, SIGNING_VERSION)
    {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(PAUSER_ROLE, msg.sender);
        _grantRole(MINTER_ROLE, msg.sender);
        whiteListSalePrice = 230000000000000000;
        publicSalePrice = 330000000000000000;
    }

    modifier onlyWhiteList() {
        require(whiteList[msg.sender], "CowNFT: not in white list");
        _;
    }
    ...
    function setPublicSaleEnable(bool _isEnable)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        publicSaleEnable = _isEnable;
    }

    function setWhiteListSaleEnable(bool _isEnable)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        whiteListSaleEnable = _isEnable;
    }
    ...
    function buy(LazyCowDatas calldata lzdatas)
        external
        payable
        whenNotPaused
        returns (uint256[] memory)
    {
        ...
        require(
            ...
            "CowNFT: Signature invalid or unauthorized"
        );

        require(
            checkTxNonce[lzdatas.to][lzdatas.nonce] == false,
            "CowNFT: Replay attacks"
        );

        uint256 price;

        require(
            publicSaleEnable || whiteListSaleEnable,
            "CowNFT: Not open for sale"
        );

        if (whiteListSaleEnable) {
            require(whiteList[msg.sender], "CowNFT: Not in the whitelist");
            price = whiteListSalePrice;
        } else {
        ...
        require(msg.value == paymentAmount, "CowNFT: Money transfer error");
    ...
    function _hash(LazyCowDatas calldata lzdatas)
        internal
        view
        returns (bytes32)
    {
        ...
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "LazyCowDatas(uint256 nonce,string[] uri,address to)"
                        ),
                        lzdatas.nonce,
                        result,
                        lzdatas.to
    ...
    function _verify(LazyCowDatas calldata lzdatas)
        internal
        view
        returns (address)
    {
        bytes32 digest = _hash(lzdatas);
        return ECDSA.recover(digest, lzdatas.signature);
    }
