...
//Daddy BitBurnReflect SmartContract
//Made to honor Our Grandfather BitBurnReflect 
//And the cleanest contract ever made
...
contract DaddyBitBurnReflect is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    DaddyBBRDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Daddy BitBurnReflect", "DADDY") {
        marketingWallet = payable(0x58b703247DfeA411641a0281577cabd637EbA554);
        devWallet = payable(0x28EE6607a729Dd8d96Eb572EFCEeC1DC1c5d0a05);
        ...
        dividendTracker = new DaddyBBRDividendTracker(
            payable(this),
            router,
        ...
        maxWallet = totalTokenSupply / 40000; // 0.05%
        swapTokensAtAmount = totalTokenSupply / 40000; // 0.05%;
    ...
    // set wallet, can not be lower than 0.05% of supply
    function setmaxWallet(uint256 value) external onlyOwner {
        value = value * (10**18);
        require(value >= _totalSupply / 40000, "max wallet cannot be set to less than 0.05%");
        maxWallet = value;
    }
...
contract DaddyBBRDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    DaddyBitBurnReflect public DaddyBitBurnReflectContract;
    ...
    constructor(
        ...
        defaultToken = token;
        DaddyBitBurnReflectContract = DaddyBitBurnReflect(mainContract);
    ...
    function _transfer(
        ...
        require(false, "DaddyBBR_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "DaddyBBR_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DaddyBBR contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(DaddyBitBurnReflectContract);

            uint256 prevBalance = DaddyBitBurnReflectContract.balanceOf(address(this));
            ...
            {
                uint256 received = DaddyBitBurnReflectContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    DaddyBitBurnReflectContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
