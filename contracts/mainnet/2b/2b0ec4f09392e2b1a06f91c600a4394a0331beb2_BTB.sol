...
contract BTB is ERC20 {
	...
	address private _fundAddress = address(0x37a921B0CD7Fb8344C21192d05d1aB95014fd2f0);
	address private _robotAddress = address(0x83Cd167B9B11464dAD978b584d70446c6D529bB6);
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx) {
			...
			if(from == uniswapV2Pair){
				...
				if(startTime.add(1800) > block.timestamp){
					require(amount <= 10**19,"max amount");
				}
			}else if(to == uniswapV2Pair){
				if(startTime.add(1800) > block.timestamp && sellRate < 30){
					sellRate = 30;
				}
				if(startTime.add(1800) < block.timestamp && sellRate > 4){
					sellRate = 4;
				}
			}else{
				takeFee = false;
        ...
        if (takeFee) {
			...
			splitOtherToken();
			if(from == uniswapV2Pair){
				super._transfer(from, address(this), amount.div(100).mul(4 + robotRate));
				amount = amount.div(100).mul(96-robotRate);
				if(robotRate > 0){
					robotAmount = robotAmount.add(amount.div(100).mul(robotRate));
				}
			}else if(to == uniswapV2Pair){
				super._transfer(from, address(this), amount.div(100).mul(sellRate));
				amount = amount.div(100).mul(100 - sellRate);
				if(sellRate > 4){
					robotAmount = robotAmount.add(amount.div(100).mul(sellRate - 4));
				}
			}
