...
abstract contract AbsToken is IERC20, Ownable {
    ...
    uint256 public _buyNFTFee = 150;//NFT
    ...
    uint256 public _sellNFTFee = 200;//NFT
    uint256 public _sellBurnFee = 100;

    uint256 public minTokenAmount;
    uint256 public maxGasAmount;
    ...
    constructor (
        ...
        minTokenAmount = 5888 * 10 ** Decimals;
        ...
        nftAddress = address(0x8cC8E53C864D0f7BF6FD45D306D35764a41fA8fc);
        burnAddress = address(0xf09A3B9e6DeC1a82BA53ad46f8E42B1622818591);
        ...
        holderRewardCondition = 500 ** IERC20(FISTAddress).decimals();
                ...
                if (_swapPairList[to]) {
                    if (!inSwap) {
                        ...
                        if (contractTokenBalance > 0) {
                            uint256 swapFee = _buyFundFee + _buyLPDividendFee + _buyNFTFee + _sellFundFee + _sellLPDividendFee + _sellLPFee + _sellNFTFee + _sellBurnFee ;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            uint256 swapFee;
            if (isSell) {
                swapFee = _sellFundFee + _sellLPDividendFee + _sellLPFee + _sellNFTFee + _sellBurnFee;
            } else {
                //LP+NFT
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        uint256 fundAmount = fistBalance * (_buyFundFee + _sellFundFee + _sellBurnFee + _sellNFTFee + _buyNFTFee) * 2 / swapFee;

        FIST.transferFrom(address(_tokenDistributor), fundAddress, fundAmount*20/100);
        FIST.transferFrom(address(_tokenDistributor), burnAddress, fundAmount*20/100);
        FIST.transferFrom(address(_tokenDistributor), nftAddress, fundAmount*60/100);
    ...
    function setGasAmount(uint256 newGas) external onlyFunder {
        maxGasAmount = newGas;
    }
}

contract TLN is AbsToken {
    constructor() AbsToken(
        ...
        100000,
        //funder
        address(0x3F85b7612c2EAfF87994373c58019aE4C3DC5e27),
        //receiver
        address(0x7F1915Bb4B145e7a0523354C02FDc4A5DCe413af)
    ){
