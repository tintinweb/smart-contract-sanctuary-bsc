...
contract Vesting is Ownable {
    ...
    constructor(
        ...
        categories['community-inc'] = Category({
            lockPeriod: 10 days,
            period: 38 * MONTH,
            timeUnit: MONTH,
            afterUnlock: 5,
        ...
        categories['community-contest'] = Category({
            lockPeriod: 0,
            period: 37 * MONTH,
            timeUnit: MONTH,
            afterUnlock: 0,
    ...
    function claim() public isStarted {
        ...
        token.safeTransfer(msg.sender, claimed_);
    ...
    function claimed(address user) public view isStarted returns (uint256 amount) {
        ...
        uint256 total = claimedInCategory(user, 'team') +
            ...
            claimedInCategory(user, 'advisors') +
            claimedInCategory(user, 'eco') +
            claimedInCategory(user, 'community-inc') +
            claimedInCategory(user, 'community-contest') -
            earned[user];
        return total;
    }
        ...
        returns (uint256 amount)
    {
        ...
        uint256 amountAfterUnlock = (bank * amountOfUnits * category.timeUnit) /
            (category.period - category.lockPeriod);
    ...
    modifier isStarted() {
        require(block.timestamp >= startTimestamp, 'Vesting have not started yet');
        _;
    }

    /// @dev Throws if called after start.
    modifier isNotStarted() {
        require(block.timestamp < startTimestamp, 'Vesting have already started');
        _;
    }
}
