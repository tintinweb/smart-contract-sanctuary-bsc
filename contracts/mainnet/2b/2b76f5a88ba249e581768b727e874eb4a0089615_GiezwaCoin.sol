...
*/
// CODE BY RZH DEV Telegram @Robdbeme
...
contract GiezwaCoin is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    GiezwaCoinDividendTracker public dividendTracker;

    address public deadWallet = 0x000000000000000000000000000000000000dEaD;

    address public immutable BUSD = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); //BUSD

    uint256 public _totalSupply = 88_888_888_888_888 * (10**18);
    uint256 public swapTokensAtAmount = 1_000_000 * (10**18); //50 millions. 
    ...
    address payable public _marketingWalletAddress = payable(0x7C3e16c7A14d22795610997FC67658Ae056ebb33);
    address payable public _buybackWalletAddress = payable(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
    ...
    constructor() BEP20("GiezwaCoin", "GiezwaCoin") 
    {
        dividendTracker = new GiezwaCoinDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "GiezwaCoin: The dividend tracker already has that address");
        GiezwaCoinDividendTracker newDividendTracker = GiezwaCoinDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "GiezwaCoin: The new dividend tracker must be owned by the GiezwaCoin token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "GiezwaCoin: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "GiezwaCoin: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "GiezwaCoin: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "GiezwaCoin: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "GiezwaCoin: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "GiezwaCoin: Cannot update gasForProcessing to same value");
...
contract GiezwaCoinDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("GiezwaCoin_Dividend_Tracker", "GiezwaCoin_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "GiezwaCoin_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "GiezwaCoin_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main GiezwaCoin contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "GiezwaCoin_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "GiezwaCoin_Dividend_Tracker: Cannot update claimWait to same value");
