...
contract Vesting1 is IVesting, Ownable, Destory {
  ...
  uint64 private _lock;
  uint64 private _duration;

  mapping(address => bool) private _operationAuthority;

  constructor(address __tokenAddress, address __beneficiaryAddress, uint64 startAt, uint64 lockedSeconds, uint64 durationSeconds) {
    require(__tokenAddress != address(0), "Vesting1: tokenAddress is zero");
    require(__beneficiaryAddress != address(0), "Vesting1: beneficiaryAddress is zero");
  ...
  modifier hasPerm() {
    require(_operationAuthority[_msgSender()], "Vesting1: caller is not has authority");
    _;
  }
  ...
  function addBalance(address beneficiary, uint256 amount) public override hasPerm returns (bool) {
    revert("Vesting1: addBalance not support");
  }
  ...
  function addOperationAuthority(address perm) public override onlyOwner returns (bool) {
    require(perm != address(0), "Vesting1: addOperationAuthority address is zero");
  ...
  function _vestingSchedule(uint256 totalAllocation, uint256 timestamp) internal view returns (uint256) {
    if (timestamp < _start + _lock) {
      return 0;
    } else if (timestamp >= _start + _lock + _duration) {
      return totalAllocation;
    } else {
      return (totalAllocation * (timestamp - _start - _lock)) / _duration;
    }
  }
}