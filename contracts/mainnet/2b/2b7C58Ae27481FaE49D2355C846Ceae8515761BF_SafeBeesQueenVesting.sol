...
contract SafeBeesQueenVesting is Ownable, AccessControl, ReentrancyGuard {
    ...
    */
    function lockTokens(address token, uint256 amount, uint256 unlockTime,
        ...
        require(unlockTime > block.timestamp + minimalLockTime, "TOO SMALL UNLOCK TIME");
        require(unlockTime < 10000000000, "INVALID UNLOCK TIME, MUST BE UNIX TIME IN SECONDS");

        lockId = _createLock(token, withdrawer, amount, unlockTime);
    }
    ...
    * @param withdrawer account that can withdraw tokens to it's balance
    */
    function vestTokens(address token, uint256 amount, uint256[] memory percents, uint256[] memory unlockTimes,
        address payable withdrawer) external payable nonReentrant {
        require(percents.length == unlockTimes.length, "ARRAY SIZES MISMATCH");
        require(percents.length >= 2, "LOW LOCKS COUNT");
        require(amount > 0, "ZERO AMOUNT");
        require(withdrawer != address(0), "ZERO WITHDRAWER");
        require(unlockTimes[0] > block.timestamp + minimalLockTime, "TOO SMALL UNLOCK TIME");


        uint256 percentsOverall = percents[0];

        //first lock
        uint256 remainingAmount = createVestingItem(token, withdrawer, amount, percents[0], unlockTimes[0], amount);

        for (uint32 i = 1; i < unlockTimes.length - 1; i++) {
            require(unlockTimes[i] > unlockTimes[i - 1], "UNSORTED UNLOCK TIMES");

            remainingAmount = createVestingItem(token, withdrawer, amount,
                percents[i], unlockTimes[i], remainingAmount);

            percentsOverall = percentsOverall.add(percents[i]);
            
        }
        _createLock(token, withdrawer, remainingAmount, unlockTimes[unlockTimes.length - 1]);
        require(percentsOverall.add(percents[percents.length - 1]) == PERCENT_FACTOR, "INVALID PERCENTS");

        require(unlockTimes[unlockTimes.length - 1] < 10000000000, "INVALID UNLOCK TIME, MUST BE UNIX TIME IN SECONDS");
    }
    ...
    function createVestingItem(
        ...
        uint256 currentLockAmount = amountToLock.mul(percent).div(PERCENT_FACTOR);
        _createLock(token, withdrawer, currentLockAmount, unlockTime);
        return remainingAmount.sub(currentLockAmount);
    }

    function _createLock(address token, address owner, uint256 amount, uint256 unlockTime) private returns (uint256){
        ...
        emit OnTokenLock(lockId, token, owner, amount, unlockTime);
        return lockId;
    }
    ...
    */
    function withdraw(uint256 lockId) external {
        ...
        withdrawPartially(lockId, lock.amount);
    }
    ...
    */
    function withdrawPartially(uint256 lockId, uint256 amount) public nonReentrant onlyLockOwner(lockId) {
        ...
        require(amount <= lock.amount, "LOCK AMOUNT IS LOWER");

        IERC20(lock.token).safeTransfer(lock.owner, amount);

        uint256 newAmount = lock.amount.sub(amount);
