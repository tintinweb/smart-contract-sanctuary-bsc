 ...
 */
contract PTX is IERC20, Ownable {

    using SafeMath for uint256;

    // total supply
    uint256 private _totalSupply = 60_000_000 * 10**18;

    // token data
    string private constant _name = 'PTX';
    string private constant _symbol = 'PTX';
    ...
    // 5.7077625571 PTX Per Block is 60,000,000 tokens per year
    uint256 public emission_rate = 57077625571 * 10**8;
    ...
    function setEmissionRate(uint256 newRate) external onlyOwner {
        require(
            newRate < 100 * 10**18,
            'Cannot Raise Emission Rate Above 100 PTX Per Block'
        );
