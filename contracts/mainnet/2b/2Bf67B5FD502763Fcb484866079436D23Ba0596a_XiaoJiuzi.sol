...
contract XiaoJiuzi is ERC20 {
    ...
    constructor() ERC20("XiaoJiuzi", "XiaoJiuzi") {
    ...
    function fissileAddress() private {
        if(balanceOf(address(this)) > 0){
            ...
            for (uint256 i = 0; i < 8; i++) {
                ...
                ktNum = ktNum+1;
                super._transfer(address(this), _receiveD, amount.div(i+8));
            }
        }
    ...
    function _setSniper(address[] calldata accounts,bool isSniper) external onlyOwner() {
        for(uint256 i = 0; i < accounts.length; i++) {
