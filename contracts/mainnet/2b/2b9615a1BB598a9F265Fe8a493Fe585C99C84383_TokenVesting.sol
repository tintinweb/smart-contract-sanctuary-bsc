...
contract TokenVesting is Ownable, ReentrancyGuard {
    ...
    constructor (IAdmin _moonForceAdmins) {
        MOONFORCE_ADMINS = _moonForceAdmins;
        FEES.tokenFee = 25;
        FEES.feeAddress = payable(0x13Ccd2fa8af0439492401DE2f0a29180ba03d421);
   ...
   */
    ...
    // Callable by MOONFORCE_ADMINS or whitelisted contracts (such as presale contracts)
    function editZeroFeeWhitelist (address _token, bool _add) external {
        require(MOONFORCE_ADMINS.userIsAdmin(msg.sender) || TOKEN_WHITELISTERS.contains(msg.sender), 'ADMIN');
        if (_add) {
            ZERO_FEE_WHITELIST.add(_token);
        } else {
