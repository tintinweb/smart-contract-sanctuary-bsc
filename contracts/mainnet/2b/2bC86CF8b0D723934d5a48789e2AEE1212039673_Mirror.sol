...
contract Mirror is ERC20, Ownable,AccessControl {
    ...
    constructor () ERC20("Mirror Swap", "MIS"){
        ...
        burn_fee = 700;
        minTotalSupply = 8000000*1e18;
        ERC20._mint(owner(), 10000000*1e18);
    }
    ...
    function setMinTotalSupply(uint256 _minTotalSupply) public onlyOwner {
        require(_minTotalSupply <= minTotalSupply,"minTotalSupply can not less than current minTotalSupply");
        minTotalSupply = _minTotalSupply;
    }

    function setBurnFee(uint256 _burn_fee) public onlyOwner {
        require(burn_fee >= 0,"burn fee can not less than 0");
        burn_fee = _burn_fee;
        require(burn_fee < 10000,"burn fee can not more than 100%");
    }

    function setBinder(address _binder, bool _status) public onlyOwner {
        binder[_binder] = _status;
    }
    ...
    function skim(address token,address to) public onlyOwner {
    ...
    function excludeFromToFee(address account) public onlyOwner {
        _isExcludedFromFee[account] = true;
        _isExcludedToFee[account] = true;
    }
    
    function includeFromToFee(address account) public onlyOwner {
    ...
    function includeInToFee(address account) public onlyOwner {
        _isExcludedToFee[account] = false;
    }
    ...
    function bindReferer(address to) public {
        require(binder[msg.sender], "caller not binder");
