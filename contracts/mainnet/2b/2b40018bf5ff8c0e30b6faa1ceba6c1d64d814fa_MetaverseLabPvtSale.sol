...
contract MetaverseLabPvtSale is Owned {
    ...
    uint256 public tokenRatePerEth = 25625e18;
    uint256 public rateDecimals = 0;
    
    uint256 public minEthLimit = 1e17; // 0.1 BNB
    uint256 public maxEthLimit = 1e18; // 0.9 BNB

    uint256 public maxTier2EthLimit = 6e17;

    uint256 public totalSupply;
    
    uint256 public soldTokens = 0;
    
    uint256 public intervalHours;
    
    uint256 public endTime = 15 hours;
    ...
    function startPrivateSale(uint256 numberOfdays) external onlyOwner {
        ...
        intervalHours = numberOfdays.mul(1 hours);
        endTime = block.timestamp.add(intervalHours);
        isPresaleOpen = true;
    }
    ...
    function setmaxTier2EthLimit(uint256 amount) external onlyOwner {
        maxTier2EthLimit = amount;    
    }
    ...
    function addTier2WhitelistedAddress(address _address) external onlyOwner {
        whitelistedAddresses[tokenAddress][_address] = maxTier2EthLimit;
    }
    ...
    function addMultipleTier2WhitelistedAddresses(address[] calldata _addresses) external onlyOwner {
         for (uint i=0; i<_addresses.length; i++) {
             whitelistedAddresses[tokenAddress][_addresses[i]] = maxTier2EthLimit;
         }
    }
