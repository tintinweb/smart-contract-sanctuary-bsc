...
/*
    Website: https://ecotirealliance.org/
    Contract Name: Ecotire
    Twitter: https://twitter.com/EcotireF
    Contract Version: 3.01
...
contract ETTOKEN is Context, IERC20, Ownable {
    using Address for address;
    address payable public foundationWalletAddress = payable(0x37CEf6033F07Ad55A1aa31F94dab6248B006B86e); // Foundation Wallet Address
    address payable public charityWalletAddress= payable (0xd0c1AD1C21BCF399AAF99241c1eF6c4f1f48Ddb7); // Charity Wallet Address
    address public safuDevWallet; //SAFU Dev Wallet for Record.
    ...
    string private _name = "EcoTire Token";
    string private _symbol = "ETTOKEN";
    uint8 private _decimals = 9;
    uint8 private _version = 3;

    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 23_000_000_000 * 10**_decimals;

    uint256 public _minimumSupply = 3_000_000_000 * 10**_decimals; //Supply cannot go lower than this amount for safety reasons adding it as protection.

    uint256 public constant _liquidityFee = 1;
    uint256 public constant _foundationFee = 2;

    uint256 public constant _charityFee = 2;

    uint256 public totalSwapableFee = _foundationFee + _liquidityFee + _charityFee;
    ...
    uint256 public foundationTokensCollected = 0;
    ...
    string private constant _safuDEV = "Blade";
    ...
    function swapAndLiquify() public lockTheSwap {
        ...
        uint256 walletsTotal = foundationTokensCollected + charityTokensCollected;

        uint256 ethForfoundation = (newBalance * foundationTokensCollected) /
            walletsTotal;
        uint256 ethForcharity = (newBalance * charityTokensCollected) / walletsTotal;

        transferToAddressETH(foundationWalletAddress, ethForfoundation);
        transferToAddressETH(charityWalletAddress, ethForcharity);

        liquidityTokensCollected = 0;
        foundationTokensCollected = 0;
    ...
    function countUpFeeShare(uint256 amount) private {
        ...
        foundationTokensCollected += (amount * _foundationFee) / 100;
    ...
    function setFoundationWalletAddress(address _foundationWallet)
        external
        onlyOwner
    {
        require(
            _foundationWallet != address(0),
            "setfoundationWalletAddress: ZERO"
        );
        foundationWalletAddress = payable(_foundationWallet);
        emit AuditLog(
            "We have Updated the foundationWallet:",
    ...
    function recoverETHfromContract() public virtual onlyOwner {
        payable(foundationWalletAddress).transfer(address(this).balance);
        emit AuditLog(
...
require(_tokenAddress != address(this), "Owner can't claim contract's balance of its own tokens");	
        IERC20(_tokenAddress).transfer(foundationWalletAddress, _amount);
        emit Log("We have recovered tokens from contract:", _amount);
    }

    //Update price based on Oracle.
    function updateSwapOutput (uint256 _newSwapOutput) external onlyOwner {
    require (_newSwapOutput >= 0, "New SwapOutput need to be greather than 0");
    require (_newSwapOutput != swapOutput, "This is the current value, you need to use a different one");
    swapOutput = _newSwapOutput;

    }

    //adding burn funcitons
  function burn(uint256 _amount) external {
     require (_amount <= _minimumSupply, "You cannot burn any more");
    _burn(msg.sender, _amount);
    emit Log("We have burned a total of:", _amount);
  }
  ...
  function _burn(address account, uint256 amount) internal {
    ...
    //end of Burn functions for SAFU Contract.
