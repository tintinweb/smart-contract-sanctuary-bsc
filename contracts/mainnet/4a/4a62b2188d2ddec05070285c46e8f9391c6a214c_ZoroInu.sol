...
contract ZoroInu is Context, IERC20, Ownable {
    ...
    address public constant advertisementWallet = 0x0121AB23a8d3F9c5d58E4D4Cb463B814A461c489;
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tAdvertisement) = _getValues(tAmount);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tAdvertisement) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tAdvertisement, _getRate());
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tAdvertisement);
    }

    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        uint256 tFee = calculateTaxFee(tAmount);
        uint256 tAdvertisement = calculateAdvestisementFee(tAmount);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tAdvertisement);
        return (tTransferAmount, tFee, tAdvertisement);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tAdvertisement, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rAdvertisement = tAdvertisement.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rAdvertisement);
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _advestisementFee == 0) return;
