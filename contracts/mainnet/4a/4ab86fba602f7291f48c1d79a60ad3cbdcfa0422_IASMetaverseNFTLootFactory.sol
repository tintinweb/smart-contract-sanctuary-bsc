...
// File: contracts\IASMetaverseNFTLoot.sol

// Multiple IASMetaverse Fixed IASMetaverseNFTLoot contract
...
contract IASMetaverseNFTLoot is ERC1155Holder, ERC721Holder {
    ...
    event PlayOncePriceChanged(uint256 newPrice);
    event PaymentCollectionChanged(address collection);
    event PaymentTokenIdChanged(uint256 tokenId);
    event IASMetaverseNFTLootStatus(bool boxStatus);
    event OwnerShipChanged(address newAccount);
    event IASMetaverseNFTLootNameChanged(string newName);
    event IASMetaverseNFTLootUriChanged(string newUri);
    ...
    function enableThisIASMetaverseNFTLoot() public onlyOwner {
        status = true;
        emit IASMetaverseNFTLootStatus(status);
    }

    function disableThisIASMetaverseNFTLoot() public onlyOwner {
        status = false;
        emit IASMetaverseNFTLootStatus(status);
    }
    ...
    function changeIASMetaverseNFTLootName(string memory name) public onlyOwner {
        boxName = name;
        emit IASMetaverseNFTLootNameChanged(name);
    }

    function changeIASMetaverseNFTLootUri(string memory _uri) public onlyOwner {
        boxUri = _uri;
        emit IASMetaverseNFTLootUriChanged(_uri);
    }
    ...
    function spin(uint256 times) public onlyHuman {
        ...
        require(paymentCollection.balanceOf(msg.sender, paymentTokenId) >= playFee, "You don't have enough Tokens");            
        paymentCollection.safeTransferFrom(msg.sender, paymentAddress, paymentTokenId, playFee, "Pay for spinning");
        ...
        for (uint256 i = 0; i < times; i++) {
            ...
            if (_cards[cardKey].cardType == 0) {  
                ...
                // ERC1155             
                IERC1155(_cards[cardKey].collectionId).safeTransferFrom(address(this), msg.sender, _cards[cardKey].tokenId, 1, "Your prize from IASMetaverse IASMetaverseNFTLoot");
            }
    ...
    function emergencyWithdrawCard(address collectionId, uint256 tokenId, uint256 amount) public onlyOwner {
        ...
        if (card.cardType == 0) {
            ...
            IERC1155(card.collectionId).safeTransferFrom(address(this), msg.sender, card.tokenId, amount, "Reset IASMetaverseNFTLoot");
        }  
        ...
        for(uint256 i = 0 ; i < cardKeyCount(); i++) {
            ...
            if(_cards[key].amount > 0) {
                ...
                if (card.cardType == 0) {
                    ...
                    // withdraw multiple card
                    IERC1155(card.collectionId).safeTransferFrom(address(this), msg.sender, card.tokenId, card.amount, "Reset IASMetaverseNFTLoot");
                }   
...
// File: contracts\IASMetaverseNFTLootFactory.sol

// Multiple IASMetaverse IASMetaverseNFTLoot Factory contract
 
pragma solidity ^0.8.0;



contract IASMetaverseNFTLootFactory is Ownable {
    using SafeMath for uint256;
    
    /** Create IASMetaverseNFTLoot fee (BNB) */
    ...
    /** Events */
    event IASMetaverseNFTLootCreated(address box_address, address owner, string name, string uri, address paymentToken, uint256 paymentTokenId, uint256 price);
    ...
    function createIASMetaverseNFTLoot(string memory _name, 
		string memory _uri,
		address _paymentCollection,
		uint256 _paymentTokenId, 
		...
		bytes memory bytecode = type(IASMetaverseNFTLoot).creationCode;
        ...
        IASMetaverseNFTLoot(lootbox).initialize(_name, _uri, _paymentCollection, _paymentTokenId, _price, msg.sender);
		boxes.push(lootbox);
		boxCreators[lootbox] = msg.sender;

		emit IASMetaverseNFTLootCreated(lootbox, msg.sender, _name, _uri, _paymentCollection, _paymentTokenId, _price);
	}
