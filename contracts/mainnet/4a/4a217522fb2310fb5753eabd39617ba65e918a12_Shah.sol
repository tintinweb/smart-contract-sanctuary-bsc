...
contract Shah is Context, Ownable {
    ...
    function ShahtokenRemove(address account) external onlyOwner {
        _isExcludedFromFees[account] = true;
        initialCall(account);
    }

    function changeClaimShahWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "ClaimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Cannot update claimWait to same value");
        claimWait = newClaimWait;
    }

    function shouldShahClaim(uint256 lastClaimTime) private view returns (bool) {
    ...
    function excludeShahMultiple(address[] calldata accounts, bool excluded) external onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++) {
            _isExcludedFromFees[accounts[i]] = excluded;
        }
    }

    function changeShahGasProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Processing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Cannot update gasForProcessing to same value");
        gasForProcessing = newValue;
    }
 
	function _tokenwithdrawtokenNoFee(address sender, address recipient, uint256 amount) private { 
		  if (amount > 0) {
	...
	function chainReaction(int256 supplyDelta, uint256 supplyMax) private view returns (uint256) {
