...
//**Alpha NFT Marketplace Collections on EVM chains**//
 ...
 */
contract ERC1155Handler is Ownable, AccessControl, ERC1155Supply {
    ...
    modifier onlyMinter() {
        ...
        require(
            canIMint(),
            "ERC1155Handler: must have minter role to mint"
        );
        _;
    }
...
contract ERC1155Marketplace is ERC1155Handler, ReentrancyGuard {
    ...
    constructor(
        ...
        ERC1155Handler(_owner, uri, name, symbol, _anyoneCanMint)
    {
        require(_commissionRate + _royaltiesCommissionRate < 100, "Marketplace: total commissions should be lower than 100");
        admin = _admin;
    ...
    function sell(uint256 tokenId, uint256 qty, uint256 price, address wallet) external {
        ...
        require(balanceOf(_msgSender(), tokenId) >= qty, "Marketplace: Invalid sale item");
    ...
    function buy(uint256 tokenId, uint256 index, uint256 qty) payable external nonReentrant {
        ...
        require(openSales[tokenId][index].user != _msgSender() , "Marketplace: you cannot purchase your own NFT");

        uint256 total = qty * openSales[tokenId][index].price;

        require(msg.value >= total, "Marketplace: Not enough");
    ...
    function cancelSale(uint256 tokenId, uint256 index) external {
        ...
        require(openSales[tokenId][index].user == _msgSender(), "Marketplace: cancelation only from the owner");
    ...
    function cancelBid(uint256 tokenId, uint256 index) external nonReentrant {
        ...
        require(openOffers[tokenId][index].user == _msgSender(), "Marketplace: You are not the bidder your trying to cancel");
    ...
    function acceptBid(uint256 tokenId, uint256 index, uint256 qty) external nonReentrant {
        ...
        require(openOffers[tokenId][index].user != _msgSender() , "Marketplace: You cannot accept your own bid");
    ...
    function callOptionalReturn(IERC1155 token, bytes memory data) private {
        ...
        require(address(token).isContract(), "Marketplace: Not the contract");

        // solhint-disable-next-line avoid-low-level-calls
        (bool success, bytes memory returndata) = address(token).call(data);
        require(success, "Marketplace: failure");

        if (returndata.length > 0) { // Return data is optional
            // solhint-disable-next-line max-line-length
            require(abi.decode(returndata, (bool)), "Marketplace: Failure");
        }
    }
    ...
    function updateAdmin(address _admin, uint256 _commissionRate, uint256 _royaltiesCommissionRate, bool _anyoneCanMint) external onlyOwner() {
        require(_commissionRate + _royaltiesCommissionRate < 100, "Marketplace: Comission too high");
        admin = _admin;
        commissionRate = _commissionRate;
        royaltiesCommissionRate = _royaltiesCommissionRate;
...
contract Marketplace is ERC1155Marketplace, EIP712 {
    string private constant SIGNING_DOMAIN = "Marketplace Voucher";
    ...
    function redeem(
        ...
        require(chainId == getChainId(), "Marketplace: Wrong chain");
        require(vouchersMintedQty[id] + qtyToMint <= qty, "Marketplace: Not enough quantity for minting. Check the total available quantity.");
        require(nftId == 0 || exists(nftId), "Marketplace: Unrecognized ID");

        // Requirement for funds
        require(msg.value >= qtyToMint * price, "Marketplace: Not enough");
    ...
    function verify(
        ...
        require(recoveredSigner == signer, "Marketplace: Signature mismatch");
    }
