...
contract WOMENSDAY is ERC20, Ownable {
    ...
    string private constant _name = "WOMENS DAY";
    string private constant _symbol = "WOMENSDAY";
    uint8 private constant _decimals = 9;

    bool private swapping;
    bool public tradingActive = false;

    address public marketingWallet = address(0x3E3fbeF4cF6D7589A895142f4B323716c313f4a7);
    ...
    function updateSwapTokensAtAmount(uint256 _amount) external onlyOwner {
        require(_amount >= (totalSupply() * 1) / 100000, "Swap amount cannot be lower than 0.001% total supply.");
        require(_amount <= (totalSupply() * 5) / 1000, "Swap amount cannot be higher than 0.5% total supply.");
    ...
    function excludeBots(address[] calldata addresses, bool excluded) external onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            bots[addresses[i]] = excluded;
        }
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (from != owner() && to != owner() && to != address(0) && to != address(0xdead) && !swapping) {
            ...
            require(!bots[from] && !bots[to], "TOKEN: Your wallet is blacklisted!");
