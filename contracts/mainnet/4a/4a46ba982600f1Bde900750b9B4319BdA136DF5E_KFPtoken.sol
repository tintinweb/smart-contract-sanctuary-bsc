...
contract ERC20 is Context,Ownable, IERC20, IERC20Metadata {
    ...
    mapping(address => uint256) public idolist;
    ...
    constructor(string memory name_, string memory symbol_) {
        ...
        foundation = address(0x791F3848B2287679399FB0023a0601B2D4b9f4f9);
        defaultNode = address(0x78e29B785A1bb212743350eFF581aaF0869ACbe2);
        MIN_AMOUNT = 219000000 * 10 ** 6;
        _taxFoundation = 2;
        _taxBurn = 1;
        _taxNode = 2;
        lpFee = 5;
    ...
    function setController(address controllerAddr) public onlyFunder {
    ...
    function addIdolist(address user,uint256 idonum) public onlyController {     
        idolist[user] = idonum;    
    }

    function updateEndTime(uint256 newtime) public onlyController {     
        endTime = newtime;    
    }
    
    function updateKfpPool(address newpool) public onlyController {     
        kfppool = newpool;    
    }
    ...
    function _transfer(
        ...
        if(idolist[sender] > 0 && block.timestamp < endTime)
        {
            if(recipient != kfppool)
            {
                require(_balances[sender].add(pool[sender]).sub(idolist[sender]).sub(amount) >= 0 , "ERC20: user in the idolist,balance not enough");  
            }
        }

        if(recipient == kfppool)
        {
            pool[sender] = pool[sender].add(amount);
        }

        if(sender == kfppool)
        {
            pool[sender] = pool[sender].sub(amount);
        }
...
contract KFPtoken is ERC20 {
    
    constructor() ERC20("Kung Fu Panda", "KFP") {      
        _mint(msg.sender,419000000 * 10 ** 6);
    }
