...
contract UniswapV2bot {
    ...
    address public router1 = 0x10ED43C718714eb63d5aA57B78B54704E256024E; //0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    ...
    function swapETHforTokens(address token, uint amount, uint slippage) public owner{
            if(amount == 0){
        ...
        uint amountOut = getAmountOut(amount, token);
        uint minAmountOut = amountOut - amountOut * slippage / 100;
        uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: amount}(minAmountOut,path,to,block.timestamp);
    }

    function swapETHforTokenAndTransfer(address token, uint amount, address dst, uint slippage) public owner{
    swapETHforTokens(token, amount, slippage);
    ERC20 TOKEN = ERC20(token);
    TOKEN.transfer(dst,TOKEN.balanceOf(address(this)));
    }

    function getAmountOut(uint amountIn, address token) public view returns (uint256)  {
        address[] memory path = new address[](2);
