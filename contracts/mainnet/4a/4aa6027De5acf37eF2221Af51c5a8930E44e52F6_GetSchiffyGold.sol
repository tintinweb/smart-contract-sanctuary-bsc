...
contract GetSchiffyGold is Context, IGOLD, Ownable {
    ...
    event Delivered(uint256 amount);
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) internal {
        ...
        _takePlayToEarn(sender, tPlayToEarn);
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tPlayToEarn, uint256 currentRate) internal pure returns (uint256, uint256, uint256) {
    ...
    function _takePlayToEarn(address sender, uint256 tPlayToEarn) internal {
        uint256 currentRate =  _getRate();
        uint256 rPlayToEarn = tPlayToEarn.mul(currentRate);
        ...
        if(tPlayToEarn > 0) 
            emit Transfer(sender, address(this), tPlayToEarn);
    }
    ...
    function calculatePlayToEarn(uint256 _amount) internal view returns (uint256) {
    ...
    event UpdateTaxes(uint256 taxFee_, uint256 playToEarn_);
    function setTaxes(uint256 taxFee_, uint256 playToEarn_) external onlyOwner {
        require(taxFee_ <= 25, "tax fee amount more than 2.5");
        require(playToEarn_ <= 50, "tax fee amount more than 5");
        _previousTaxFee = _taxFee;
        _previousPlayToEarn = _playToEarn;
        _taxFee = taxFee_;
        _playToEarn = playToEarn_;
        emit UpdateTaxes(taxFee_, playToEarn_);
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal {
        ...
        if(balanceOf(address(this)) >= swapThreshold) {
            swapAndSendFees(balanceOf(address(this)));
        }
    ...
    event UpdateSwapToken(address _SWAPTOKEN);
    function updateSwapToken(address _SWAPTOKEN) external onlyOwner {
        SWAPTOKEN = IGOLD(_SWAPTOKEN);
        emit UpdateSwapToken(_SWAPTOKEN);
    }

    function swapAndSendFees(uint256 tokensForFee) internal {
        swapTokensForTokens(tokensForFee, address(SWAPTOKEN));
        SWAPTOKEN.transfer(adminFeeReceiver, SWAPTOKEN.balanceOf(address(this)));
    }

    function swapTokensForTokens(uint256 tokenAmount, address tokenToSwap) internal {
    ...
    event UpdateThreshold(uint256 _newThreshold);
    ...
    function claimTokens() external onlyOwner {
        emit TokenClaimed(balanceOf(address(this)));
