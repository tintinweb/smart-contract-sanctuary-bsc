...
contract FFFController is Ownable {
    ...
    uint256 public powerRecommendRewardRate = 3000;
    uint256 public powerRecommendUsdtRewardRate = 50;
    ...
    function setfofN(address _fNft) public onlyOwner {
    ...
    function queryNodeTokenReward(address user) public view returns(uint256 total, uint256 draw, uint256 notDraw, uint256 canDraw) {
        ...
        notDraw = total - draw;
        canDraw = _queryNodeTokenTotalRelease(user).div(10 ** 14);
    }
    ...
    function checkCanBuyPower(address user, uint256 types) public view returns (uint256) {
        ...
        if(types < 1 && types > 6) return 3;//"Incorrect node type input"
