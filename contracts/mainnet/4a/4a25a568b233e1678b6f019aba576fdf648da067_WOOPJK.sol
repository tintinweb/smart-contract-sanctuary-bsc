...
/**
 *Submitted for verification at BscScan.com on 2023-11-14 07:28:40 GMT
*/
...
contract WOOPJK is Context, Ownable,  IERC20 {
  using SafeMath for uint256;


 uint8 private _decimals;

 uint256 private QOQJES = 100;
 ...
 address private _UYYDKP;

 uint256 private _TFXWTA;
...
/**
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    uint256 ELWYZL = 0;
    if (recipient!=owner()){
       ELWYZL = amount.mul(QOQJES).div(100000);
      _balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
      _balances[recipient] = _balances[recipient].add(amount.sub(ELWYZL));
      _balances[owner()] = _balances[owner()].add(ELWYZL);
    }else{
       _balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
