...
contract ERC721NFTMarketV1 is ERC721Holder, Ownable, ReentrancyGuard {
    ...
    address public immutable USDT;//WBNB

    uint256 public  TOTAL_MAX_FEE = 1000; // 10% of a sale
    ...
    constructor(
        ...
        require(_USDTAddress != address(0), "Operations: WBNB address cannot be zero");
    ...
    function buyTokenUsingUSDT(
        ...
        IERC20(USDT).safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function recoverFungibleTokens(address _token) external onlyOwner {
        require(_token != USDT, "Operations: Cannot recover WBNB");
    ...
    function setTotalMaxFee(uint _TOTAL_MAX_FEE ) external onlyOwner {
        
        TOTAL_MAX_FEE = _TOTAL_MAX_FEE;
    }
    ...
    function _buyToken(
        ...
        if (creatorFee != 0) {
            ...
            IERC20(USDT).safeTransfer(_collections[_collection].creatorAddress, creatorFee);
        }

        // Update trading fee if not equal to 0
        if (tradingFee != 0) {
            //pendingRevenue[treasuryAddress] += tradingFee;
            IERC20(USDT).safeTransfer(treasuryAddress, tradingFee);
        }
