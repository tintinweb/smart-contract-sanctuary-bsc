...
contract FederationDigitalToken is Context, IBEP20, Ownable {
    ...
    address payable public CyberSecurityWallet =
        payable(0x00AbDD27E59f3715C38bfd3b588682A4FCe679b6);
    address public immutable DeadWallet =
        0x000000000000000000000000000000000000dEaD;

    uint256 private constant MAX = ~uint256(0);
    uint256 private constant _tTotal = 20 * 10**15 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));

    string private constant _name = "Federation Digital Token";
    string private constant _symbol = "FDT";
    uint8 private constant _decimals = 9;

    uint256 private _liquidityFee = 900;
    uint256 private _previousLiquidityFee = _liquidityFee;

    uint256 private _CyberSecurityFee = 400;
    uint256 private _previousCyberSecurityFee = _CyberSecurityFee;

    uint256 private _burnFee = 0;
    uint256 private _previousburnFee = _burnFee;

    uint256 public _buyLiquidityFee = 900;
    uint256 public _buyCyberSecurityFee = 400;
    uint256 public _buyBurnFee = 0;

    uint256 public _sellLiquidityFee = 900;
    uint256 public _sellCyberSecurityFee = 400;
    ...
    uint256 public _maxWalletHoldingLimit =  25 * 10**11 * 10**9; 
    uint256 public _sellLimit = 100 * 10**9 * 10**9; 
    ...
    constructor() {
        ...
        _isExcludedFromFee[CyberSecurityWallet] = true;
        _isExcludedFromWalletHoldingLimit[owner()] = true;
        _isExcludedFromWalletHoldingLimit[CyberSecurityWallet] = true;
    ...
    function removeAllFee() private {
        if (_liquidityFee == 0 && _CyberSecurityFee == 0 && _burnFee == 0)
            return;

        _previousLiquidityFee = _liquidityFee;
        _previousburnFee = _burnFee;
        _previousCyberSecurityFee = _CyberSecurityFee;
    ...
    function restoreAllFee() private {
        ...
        _CyberSecurityFee = _previousCyberSecurityFee;
    }
    ...
    {
        require(
            updateSellLimit * 10**9 >= (100 * 10**9 * 10**_decimals),
            "You can't set less than 100 Billion"
        );
    ...
    function UpdateWallets(address payable newCyberSecurityWallet)
        external
        onlyOwner
    {
        require(
            newCyberSecurityWallet != address(0),
            "You can't set Zero Address"
        );
        CyberSecurityWallet = newCyberSecurityWallet;
    }

    function UpdateBuyTaxFeePercentage(
        uint256 liquidityFee,
        uint256 burnFee,
        uint256 CyberSecurityFee
    ) external onlyOwner {
        _liquidityFee = liquidityFee;
        _previousLiquidityFee = _liquidityFee;
        _burnFee = burnFee;
        _previousburnFee = _burnFee;
        _CyberSecurityFee = CyberSecurityFee;
        _previousCyberSecurityFee = _CyberSecurityFee;

        _buyLiquidityFee = _liquidityFee;
        _buyCyberSecurityFee = _CyberSecurityFee;
        _buyBurnFee = _burnFee;

        require(
            liquidityFee.add(burnFee).add(CyberSecurityFee) <=
                (feeDenominator.div(4)),
            "You can't set more than 25%"
        );
        emit BuyTaxFeeUpdated(liquidityFee.add(burnFee).add(CyberSecurityFee));
    }
    ...
    function UpdateSellTaxFeePercentage(
        ...
        _sellCyberSecurityFee = CyberSecurityFee;
    ...
    function _transfer(
        ...
        if (_isAtomaticMarketMaker[from] && !_isExcludedFromFee[to]) {
            ...
            _CyberSecurityFee = _buyCyberSecurityFee;
        ...
        if (_isAtomaticMarketMaker[to] && !_isExcludedFromFee[from]) {
            if (_24HourTiming[from] < block.timestamp && circuitBreaker) {
                if (_totalSellIn24Hour[from].add(amount) <= _sellLimit) {
                    ...
                    _CyberSecurityFee = _sellCyberSecurityFee;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 combineFees = _liquidityFee.add(_CyberSecurityFee); // CyberSecurityFee + liquidity fees
        uint256 tokensForLiquidity = contractTokenBalance
            .mul(_liquidityFee)
            .div(combineFees);

        uint256 tokensForCyberSecurityFee = contractTokenBalance.sub(
            tokensForLiquidity
        );
        ...
        swapTokensForEth(tokensForCyberSecurityFee);
        uint256 transferredBalance = address(this).balance.sub(contractBalance);

        //Send to CyberSecurityFee address
        transferToAddressETH(CyberSecurityWallet, transferredBalance);
    }
    ...
    function _transferStandard(
        ...
        (tTransferAmount, rTransferAmount) = takeCyberSecurity(
            tTransferAmount,
            rTransferAmount,
            tAmount
        );
    ...
    function takeCyberSecurity(
        uint256 tTransferAmount,
        uint256 rTransferAmount,
        uint256 tAmount
    ) private returns (uint256, uint256) {
        if (_CyberSecurityFee == 0) {
            return (tTransferAmount, rTransferAmount);
        }
        uint256 tCyberSecurityFee = tAmount.mul(_CyberSecurityFee).div(
            feeDenominator
        );
        uint256 rCyberSecurityFee = tCyberSecurityFee.mul(_getRate());
        rTransferAmount = rTransferAmount.sub(rCyberSecurityFee);
        tTransferAmount = tTransferAmount.sub(tCyberSecurityFee);
        _rOwned[address(this)] = _rOwned[address(this)].add(rCyberSecurityFee);
        return (tTransferAmount, rTransferAmount);
    }
