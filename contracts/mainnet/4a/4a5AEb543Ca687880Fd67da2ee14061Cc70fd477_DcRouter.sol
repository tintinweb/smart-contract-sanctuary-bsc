...
contract DcRouter is NonBlockingNonUpgradableBaseApp {
    ...
    function deposit(uint256 _strategyId, uint256 _stableAmount) external {
        ...
        userPosition[_msgSender()][_strategyId].pendingDeposit += _stableAmount;
    ...
    function initiateWithdraw(uint256 _strategyId, uint256 _stableAmount)
        external
    {
        userPosition[_msgSender()][_strategyId]
            .pendingWithdrawal += _stableAmount;
     ...
     * @param _strategyId - strategy number by which it will be identified
     * @return uint256 - get the many of tokens that the user has requested for withdrawal
     */
    function getPendingTokensToWithdraw(address _user, uint256 _strategyId)
        external
        view
        returns (uint256)
    {
        return userPosition[_user][_strategyId].pendingWithdrawal;
    }
    ...
    function transferDeposits(bytes memory _payload) public onlySelf {
        ...
        for (uint256 i = 0; i < users.length; i++) {
            uint256 pendingUserDeposit = userPosition[users[i]][strategyId]
                .pendingDeposit; //save gas on sload
            pendingTotal += pendingUserDeposit;
            pendingStrategyDeposits[strategyId] -= pendingUserDeposit;
            _updateUserShares(
                strategyTvl,
                strategyId,
                users[i],
            ...
            userPosition[users[i]][strategyId].pendingDeposit = 0;

            emit DepositTransferred(users[i], strategyId, pendingUserDeposit);
        }
    ...
    function approveWithdraw(
        ...
        uint256 stableWithdraw = (userPosition[_user][_strategyId]
            .pendingWithdrawal * _stableDeTokenPrice) / 1e18;
        require(
            userPosition[_user][_strategyId].pendingWithdrawal != 0,
            "DcRouter:request isn't exist"
        );
        userPosition[_user][_strategyId].pendingWithdrawal = 0;
        IERC20(_currentUSDCToken).transfer(_user, stableWithdraw);
        emit Withdrawn(_user, _strategyId, stableWithdraw);
        return true;
    }
     ...
     */
    function cancelWithdraw(address _user, uint256 _strategyId)
        public
        onlySelf
        returns (bool)
    {
        uint256 amount = userPosition[_user][_strategyId].pendingWithdrawal;
        require(amount != 0, "DcRouter:request isn't exist");
        userPosition[_user][_strategyId].pendingWithdrawal = 0;
        emit CancelWithdrawn(_user, _strategyId, amount);
        return true;
    }
}
