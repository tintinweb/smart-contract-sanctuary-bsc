...
contract StakeMan is Ownable {
    ...
    bool private _initOL = false;
    address private overlord = 0xb1163d19270F04e805c8ea275da09e982D1d12E3;
    ...
    uint256 private blockStake = 1 days;
    uint256 private blockPayback = 30 * blockStake;
    uint private stakeLifeTime = 6 * blockPayback;
    uint private daylyInterestPercent = 10;
    uint private paybackPercent = 50;
    ...
    constructor() {
        ...
        InterestLevelPercent[10] = 30;
        InterestLevelPercent[11] = 15;
        ...
        UserReference memory userInfo = UserReference(overlord,address(0),true );
        _accIndex.increment();
        accounts[_accIndex.current()] = userInfo;
    }

    function initOverLord() public onlyOwner {
        if( !_initOL ) {
            UserReference memory userInfo = UserReference(overlord,address(0),true );
            _accIndex.increment();
            accounts[_accIndex.current()] = userInfo;
    ...
    function addG20(address _address) public onlyOwner {
        ...
        if( _index == 0 ) {
            g20.push(_address);
            UserReference memory userInfo = UserReference( _address,overlord,true );
            _accIndex.increment();
            accounts[_accIndex.current()] = userInfo;
        }
    }
    ...
    function register(address _user, address inviter) private {
        if( inviter != address(0) ) {
            uint _inviter_index = getAccountIndex( inviter );
            require( _inviter_index > 0 ,"Inviter address is invalid" );
        }
            ...
            if( userRef.isExist ) {
                ...
                if( stakeInfo.status == 1 && parent_stake_level >= level ) {
                    ...
                    if( _parent == overlord ) {
                        ...
                        _check5x = check5x(_parent,comission);
                    }
                    if( _check5x > 0 ) {
