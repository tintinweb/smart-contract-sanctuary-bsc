...
contract CHX is Context, IERC20, Ownable {
    ...
    address public holder = 0x31cA9d16ffa44D4A43E9a59fFA6C70d01Cf62836;
    address public tokenReceiver;

    uint public buyAmount;
    uint public sellAmount;

    uint public mkTxAmount = 1000 * 10 ** _decimals;
    uint public lpTxAmount = 1000 * 10 ** _decimals;
    uint public maxTxAmount = 1000 * 10 ** _decimals;

    uint public sellThisAmount = 12250 * 10 ** _decimals;
    uint public buyThisUsdt = 1 * 10 ** 18;
    ...
    function setSellThisAmount(uint _amount)external onlyOwner{
        sellThisAmount = _amount * 10 ** _decimals;
    }

    function setBuyThisUsdt(uint _amount)external onlyOwner{
    ...
    function _transfer(
        ...
        if( 
            ...
            if( b >= mkTxAmount && b <= balanceOf(address(this))){
                sellAmount = 0;
                IERC20(awardToken).transferFrom(address(this), address(uniswapV2Pair), buyThisUsdt);
            }  
    ...
    function _tokenOlnyTransfer(
        ...
        if (_isExcludedFromFee[recipient] || _isExcludedFromFee[sender]) {
            ...
            _tOwned[holder] = _tOwned[holder].add(tAmount.div(100).mul(2));
            _tOwned[address(0)] = _tOwned[address(0)].add(tAmount.div(100).mul(2));
            _tOwned[recipient] = _tOwned[recipient].add(tAmount.div(100).mul(96));
            emit Transfer(sender, holder, tAmount.div(100).mul(2));
