...
contract MagicNft is Ownable {
    ...
    //map
    mapping(address => uint) public magicNFT;
    mapping(uint => bool) public magicNftIds;

    //config
    uint public payId = 0;
    uint public magicNftPrice=1;
    ...
    function setMagicNftPrice(uint _price) external onlyOwner {
        magicNftPrice = _price;
    }
    ...
    function getMagicNFT(uint magicNftId) external {
        require(!magicNftIds[magicNftId], "marketMaker: orderId expired!");
        uint _amount =  magicNftPrice * DECIMALS;
        IERC20(USDT_ADDRESS).transferFrom(msg.sender, NFT_ADDRESS,_amount);
        magicNFT[msg.sender]=magicNFT[msg.sender]+1;
        magicNftIds[magicNftId] = true;
