...
library LibCoreStorage {
    ...
    function coreStorage() internal pure returns (CoreStorage storage cs) {
        bytes32 location = keccak256("UTC.proxy.core");
...
abstract contract ProxyOwnable {
    ...
    modifier onlyOwner {
        require(msg.sender == getOwner(), "UTCProxy: !OWNER");
        _;
    }
...
abstract contract ProxyPausable is ProxyOwnable {
    modifier notPaused {
        require(!paused(), "UTCProxy: PAUSED");
        _;
    }
...
abstract contract ProxyReentrancyGuard {
    modifier nonReentrant {
        ...
        require(!cs.guardReentrancy, "UTCProxy: REENTRANCY_DISALLOWED");
...
contract UTCProxy is ProxyOwnable, ProxyPausable, ProxyReentrancyGuard {
    fallback() external payable notPaused {
        address impl = getImplementation(msg.sig);
        require(impl != address(0), "UTCProxy: INVALID_SELECTOR");
    ...
    function setImplementation(bytes4 selector, address implementation, bool initialize) external onlyOwner {
        require(implementation == address(0) || _isContract(implementation), "UTCProxy: INVALID_IMPLEMENTAION");
        ...
        if(initialize){
            ...
            require(success, "UTCProxy: INITIALIZATION_FAILED");
        }
