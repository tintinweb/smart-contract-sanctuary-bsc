...
contract StrategyWombex is StratManager, FeeManager {
    ...
    function deposit() public whenNotPaused {
        ...
        if (wantBal > 0) {
            ...
            if(underlyingBal > wantBal){ // Processing Coverage Ratio
                IERC20(underlyingAsset).safeTransfer(beefyFeeRecipient, (underlyingBal - wantBal) / 2);
            }
    ...
    function chargeFees(address callFeeRecipient) internal {
        uint256 toNative1 = IERC20(output1).balanceOf(address(this)).mul(45).div(1000);
        uint256 toNative2 = IERC20(output2).balanceOf(address(this)).mul(45).div(1000);
        if(toNative1 > 0) {
        ...
        IERC20(native).safeTransfer(beefyFeeRecipient, beefyFeeAmount + strategistFeeAmount);
    ...
    function rewardsAvailable() public view returns (uint256, uint256) {
        ...
        return (unharvestedBal1, rewardsAvailable2(unharvestedBal2));
    }

    function rewardsAvailable2(uint256 _amount) internal view returns (uint256) {
        uint256 unharvestedBal1 = 0;

        if(_amount > 0) {
            unharvestedBal1 = _amount;
            address operator = IWombexClaim(sUnderlyingAsset).operator();
            unharvestedBal1 = unharvestedBal1.mul(IWombexWMXClaim(operator).mintRatio()).div(IWombexWMXClaim(operator).DENOMINATOR());
            uint256 penalty = unharvestedBal1.mul(IWombexWMXClaim(operator).penaltyShare()).div(IWombexWMXClaim(operator).DENOMINATOR());
            unharvestedBal1 = unharvestedBal1.sub(penalty);
        }
        
        return unharvestedBal1;
    }
