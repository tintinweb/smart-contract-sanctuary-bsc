    ...
    BEP721BurnableUpgradeable
{
    ...
    function initialize() public initializer  {
        __Ownable_init();
        BEP721Upgradeable.__BEP721_init("Metaneum", "MTM");
        _tid = 1;
    ...
    function newtokenasbid(string memory tokenAss,address from, address admin, uint256 amount, uint256 tokenId) internal {
        ...
        require( approveValue >= val.div(10**Tokendecimals), "Invalid Approve Value");
    ...
    function _acceptBId(string memory tokenAss,address from, address admin, uint256 amount, uint256 tokenId) internal{
        ...
        require( approveValue >= val.div(10**Tokendecimals), "Invalid Price");
        require(balances[tokenId][msg.sender] > 0, "Is Not a Owner");
        (uint256 _adminfee, uint256 roy, uint256 netamount) = calc(amount, _royal[tokenId], serviceValue);
        require( approveValue >= (_adminfee.add(roy.add(netamount))).div(10**Tokendecimals), "Invalid Approve Value");
        if(_adminfee != 0){
    ...
    function _saleToken(
        ...
        (uint256 _adminfee, uint256 roy, uint256 netamount) = calc(amount, _royal[tokenId],serviceValue);
        require( msg.value == _adminfee.add(roy.add(netamount)), "Invalid msg.value");
        if(_adminfee != 0){
