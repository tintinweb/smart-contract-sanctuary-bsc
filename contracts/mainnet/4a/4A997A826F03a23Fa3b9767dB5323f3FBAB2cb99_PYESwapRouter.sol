...
abstract contract SupportingSwap is FeeStore, IPYESwapRouter {
    ...
    function _swap(address _feeCheck, uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            uint amountOut = amounts[i + 1];
            {
                ...
                (uint amount0Fee, uint amount1Fee) = PYESwapLibrary._calculateFees(_feeCheck, input, output, amountsI, amount0Out, amount1Out);
    ...
    function swapTokensForExactTokens(
        ...
        if(path[0] == pairFeeAddress[pair]) {
            ...
            amounts = PYESwapLibrary.amountsOut(amounts[0], path, totalFee);
    ...
    returns (uint[] memory amounts)
    {
        ...
        amounts = PYESwapLibrary.amountsOut(msg.value, path, totalFee);
    ...
    function _swapSupportingFeeOnTransferTokens(address _feeCheck, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            (uint amount0Fee, uint amount1Fee) = PYESwapLibrary._calculateFees(_feeCheck, input, output, amountInput, amount0Out, amount1Out);
...
library PYESwapLibrary {
    ...
    address constant factory = 0xD7201321184cD1B8D51690eBA8c6b43AE8Bf31B2;
    ...
    function pairFor(address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint160(uint(keccak256(abi.encodePacked(
                ...
                hex'0b2b5e082496ea54de39fd892ac195c7801e2d135c16e8ad5a0b642859c673c8' // init code hash
            )))));
    }
    ...
    function _calculateFees(address _feeCheck, address input, address output, uint amountIn, uint amount0Out, uint amount1Out) internal view returns (uint amount0Fee, uint amount1Fee) {
    ...
    function _calculateAmounts(address _feeCheck, address input, address output, address token0) internal view returns (uint amountInput, uint amountOutput) {
        ...
        amountOutput = getAmountOut(amountInput, reserveInput, reserveOutput, baseToken != address(0), totalFee);
    }
}
