...
contract MoonZero is IERC20, Ownable {
    ...
    event marketingGenerated(address marketing, uint256 ETHAmount, uint256 eventTime);
    ...
    function preTxCheck(address sender, address recipient, uint256 amount) internal view {
        ...
        require(sender != address(DEAD), "ERC20: transfer from the zero address");
    ...
    function checkMaxWallet(address sender, address recipient, uint256 amount) internal view {
        if(isCont(recipient) && sender != pair && !isCont(sender)){require((_balances[recipient].add(amount)) <= _totalSupply, "Exceeds maximum wallet amount.");}
    ...
    function swapBack(address sender, address recipient, uint256 amount) internal {
        if(shouldSwapBack(sender, recipient, amount)){
            swapAndLiquify(swapThreshold); swapTimes = uint256(0);
        emit swapbackOccurred(block.timestamp);}
    }

    function swapAndLiquify(uint256 tokens) private lockTheSwap {
        uint256 denominator = (liquidityFee.add(marketingFee)) * uint256(2);
        ...
        emit marketingGenerated(marketing_receiver, remainingBalance, block.timestamp);
    }
