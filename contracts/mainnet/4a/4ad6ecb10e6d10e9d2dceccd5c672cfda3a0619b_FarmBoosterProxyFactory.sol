...
contract FarmBoosterProxy is ReentrancyGuard {
     ...
     * @param _CSTToken: the address of the CST token
     */
    function initialize(
        address _admin,
    ...
    function harvestCST() internal {
        uint256 CSTBalance = CSTToken.balanceOf(address(this));
        if (CSTBalance > 0) {
            CSTToken.safeTransfer(msg.sender, CSTBalance);
        }
    }
}
...
contract FarmBoosterProxyFactory {
    ...
    function createFarmBoosterProxy() external {
        ...
        FarmBoosterProxy(farmBoosterProxyAddress).initialize(msg.sender, Farm_Booster, masterchefV2, CSTToken);
