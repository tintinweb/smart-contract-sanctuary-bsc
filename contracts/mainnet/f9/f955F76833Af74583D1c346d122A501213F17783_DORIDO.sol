...
import "../interfaces/IDNCPackage.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "@openzeppelin/contracts/utils/ReentrancyGuard.sol";

contract DORIDO is SafeOwnable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    IERC20 public dor;
    IDNCPackage public nftPackage;

    address public constant hole = 0x000000000000000000000000000000000000dEaD;

    bool public isAllowCollect = true;
    uint256 public tokenAmount;
    mapping(address => uint256) public userShop;

    event Shop(address account, uint256 _amount);

    constructor(address _dor, address _nftPackage) {
        dor = IERC20(_dor);
        nftPackage = IDNCPackage(_nftPackage);
        tokenAmount = 20000 * 10**18;
    }

    function allowCollect() external onlyOwner {
        isAllowCollect = true;
    }

    function prohibitCollect() external onlyOwner {
    ...
    function shop() external nonReentrant {
        require(isAllowCollect, "not isAllowCollect");
        dor.safeTransferFrom(msg.sender, hole, tokenAmount);
        userShop[msg.sender] = userShop[msg.sender] + tokenAmount;
        nftPackage.mint(msg.sender, 1);
        emit Shop(msg.sender, tokenAmount);
    }
}
...
interface IDNCPackage {
    function mint(address _recipient, uint256 _type) external;
}