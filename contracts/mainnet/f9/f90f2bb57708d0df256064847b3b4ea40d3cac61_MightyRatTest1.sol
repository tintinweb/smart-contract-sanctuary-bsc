...
contract MightyRatTest1 is BEP20, Ownable { 
    using SafeMath for uint256;
    ...
    address payable public APYwallet;
    ...
    uint256 public buyApyFee;
    uint256 public sellApyFee;
    ...
    event updateAPYWallet(address wallet);
    event UpdateUniswapV2Router(address indexed newAddress, address indexed oldAddress);
    event TradingEnabled();

    event UpdateFees(uint256 sellMarketingFees, uint256 sellBurnFee, uint256 buyMarketingFees,
                     uint256 buyBurnFee, uint256 buyDevFee, uint256 sellDevFee, uint256 buyApyFee,
                     uint256 sellApyFee);
    ...
    constructor() BEP20("MightyRatTest1", "MRatTest1") { 
        marketingWallet = payable(0xDce8ead0f384B2367931b8DD521b090a5459dc42); 
        devWallet = payable(0xEa51570b71d439fd638f9c09c27662bBCa6813F5); 
        APYwallet = payable(0xF3cCD3528eDFC783171787714fB6Ab87AC57292f);
    ...
    function setAPYWallet(address wallet) external onlyOwner {
        _isExcludedFromFees[wallet] = true;
        APYwallet = payable(wallet);
        emit updateAPYWallet(wallet);
    }
    ...
    function updateFees(uint256 marketingBuy, uint256 marketingSell, uint256 burnBuy,
                        uint256 burnSell, uint256 devBuy, uint256 devSell, 
                        uint256 apyBuy, uint256 apySell) public onlyOwner {
        ...
        require(apyBuy <= 5 && apySell <= 5, "APY fees cannot exceed 5%");
        require(burnBuy <= 10 && burnSell <= 10, "Burn Fees cannot exceed 10%");
        require(totalSellFees <= 25 && totalBuyFees <= 25, "total fees cannot be higher than 25%");

        emit UpdateFees(sellMarketingFees, sellBurnFee, sellDevFee, buyMarketingFees,
                        buyBurnFee, buyDevFee, buyApyFee, sellApyFee);
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (to == DEAD) {
            ...
            uint256 apytokens = amount.mul(apyFee).div(100);

            amount = amount.sub(fees + burntokens + apytokens);
            ...
            if (apytokens > 0) {
                super._transfer(from, APYwallet, apytokens);
            }
