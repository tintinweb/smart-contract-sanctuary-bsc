...
contract  BlueIceToken is IERC20, Ownable {
    ...
    function _transfer(
        ...
        _tokenTransfer(from, to, amount, takeFee, onBuyToken);
    ...
    function _tokenTransfer(
        ...
        _transferStandard(sender, recipient, amount, onBuyToken);
    ...
    function swapTokensForTokens(uint256 tokenAmount) private {
        ...
        uint256 lpAmount = tokenAmount * _LPFee / (_LPFee + _marketingFee + _marketingFee) / 2;
        
        IERC20 USDT = IERC20(_router);
        uint256 initialBalance = USDT.balanceOf(address(_router));
        ...
        USDT.transferFrom( _router,address(this), IERC20(_token).balanceOf(address(_router)));

        uint256 newBalance = USDT.balanceOf(address(_router)) - initialBalance;
        uint256 totalUsdtFee = _LPFee / 2 + _marketingFee + _marketingFee;
        
        uint256 lpUsdt = newBalance * _LPFee / 2 / totalUsdtFee;
