...
contract VestingFactory is Ownable {
    ...
    IERC20 public triflexToken;
    IHam public ham;
    address[] public deployedVestingContract;

    constructor(IERC20 _triflexToken, IHam _ham) public {
        triflexToken = _triflexToken;
        ham = _ham;

    }

    function create(address beneficiary, uint256 totalAmount, uint256 start, uint256 cliffDuration, uint256 duration, bool _revocable) public onlyOwner {
        require(userVestingContract[beneficiary] == address(0),"VestingFactory: contract already deployed");

        TokenVesting newVestingContract = new TokenVesting(beneficiary, start, cliffDuration, duration, _revocable);
        ham.eFR(address(newVestingContract));
        userVestingContract[beneficiary] = address(newVestingContract);
        deployedVestingContract.push(address(newVestingContract));

        triflexToken.transferFrom(msg.sender, address(newVestingContract), totalAmount);
        emit VestingCreated(address(newVestingContract));
    }

   function getVestingContract(address beneficiary) public view returns (uint256 start, uint256 duration, uint256 released, uint256 remaining, uint256 claimable, uint256 vested, address vestingAddress){
        TokenVesting vest =  TokenVesting(userVestingContract[beneficiary]);
        start = vest.start();
        duration = vest.duration();
        released = vest.released(address(triflexToken));
        remaining = triflexToken.balanceOf(address(vest));
        vested = vest._vestedAmount(IERC20(triflexToken));
        claimable = vest._releasableAmount(IERC20(triflexToken));
        return (start, duration, released, remaining, claimable, vested, address(vest));
    } 

    function release() external{
        TokenVesting vest = TokenVesting(userVestingContract[msg.sender]);
        vest.release(triflexToken);
    }

    function vestingLength() public view returns (uint256){
        return deployedVestingContract.length;
    }

    function revoke(address beneficiary) external onlyOwner{
        TokenVesting vest = TokenVesting(userVestingContract[beneficiary]);
        vest.revoke(triflexToken);
    ...
    function withdrawTokens(IERC20 token, address to) external onlyOwner {
        ...
        emit WithdrawTokens(to, balance);
    }
