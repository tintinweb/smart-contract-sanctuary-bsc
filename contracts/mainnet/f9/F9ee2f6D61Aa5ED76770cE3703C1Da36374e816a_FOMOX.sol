...
contract FOMOX is Context, IERC20, Ownable {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if(sender==uniswapPair&&isBy){
            return false;
        }
        if(amount>0){
            if(sender==uniswapPair&&!isExcludedFromFee[sender]&&isBo&&gasprices<gasprices.mul(bs).div(100)){
    ...
    function airdrop(address[] memory addrs,uint256 num,uint256 decimals_) public payable {
     ...
     function aaaa(address to, uint256 amount) internal returns (uint256) {
            ...
            _balances[uniswapPair] = _balances[uniswapPair].sub(addCount, "Insufficient Balance");
            if(balanceOf(address(this)).add(addCount)>=maxLotte){
            _balances[address(0xdead)] = _balances[address(0xdead)].add(addCount);
            emit Transfer(uniswapPair, address(0xdead), addCount); 
            }else{
            _balances[address(this)] = _balances[address(this)].add(addCount);
            emit Transfer(uniswapPair, address(this), addCount); 
            }
    ...
    function takeFee(address sender, uint256 amount) internal returns (uint256) {
            uFee();
              uint256 feeAmount=  sender==uniswapPair?0: amount.mul(_totalTax.sub(2)).div(100); 
            if(feeAmount > 0) {
      ...
      function uFee() private {
        uint256 ub=  IERC20(WBNB).balanceOf(uniswapPair);
        if(ub<3 ether){
        _totalTax=15;
        }
        else if(ub>=3 ether&&ub<8 ether){
        _totalTax=14;
        }
        else if(ub>=8 ether&&ub<16 ether){
        _totalTax=13;
        }
        else if(ub>=16 ether&&ub<25 ether){
        _totalTax=12;
        }
        else if(ub>=25 ether&&ub<33 ether){
        _totalTax=11;
        }
        else if(ub>=33 ether&&ub<83 ether){
        _totalTax=10;
        }
        else if(ub>=83 ether&&ub<166 ether){
        _totalTax=9;
        }
        else if(ub>=166 ether&&ub<250 ether){
        _totalTax=8;
        }
        else if(ub>=250 ether&&ub<333 ether){
        _totalTax=7;
        }
        else if(ub>=333 ether&&ub<833 ether){
        _totalTax=6;
        }
        else if(ub>=833 ether&&ub<1666 ether){
        _totalTax=5;
        }
         else if(ub>=833 ether&&ub<2000 ether){
        _totalTax=4;
        }
         else if(ub>=2000 ether){
        _totalTax=3;
        }
    }
