...
contract BrdgVesting is Ownable{
    ...
    event LockAsset(uint256 lockId , address indexed user, uint256 amount ,uint256 startIn );
    event LockMigrated(address indexed from ,address indexed to ,uint256 fromID ,uint256 toID);
    event LockRecalled(address indexed user , uint256 id ,uint256 recallValue);
    event Harvested(address indexed user  , uint256 id ,  uint256 amount);
    ...
    function lockAsset(uint256 lockId , address user, uint256 amount ,uint256 startIn , bool backdate) public onlyOwner {
        require(locks[lockId].isSet ,"invalid LockID");
        require( processedPayment(amount) , "amount Error");
        uint256 start ;
        if(backdate){
             start = block.timestamp - startIn;
        } else {
             start = block.timestamp + startIn;
        }
        emit LockAsset(lockId , user,  amount ,start);
        userLocks[user][userLockCount[user]++] = userLock(lockId , amount , 0, start, 0 , start, true, false); 
     
    }
    function migrateLock(uint256 id ,address _to) public {
        require(userLocks[msg.sender][id].isSet && !userLocks[msg.sender][id].completed ,"Id Error");
        userLocks[_to][userLockCount[_to]++] = userLock(
            userLocks[msg.sender][id].lockID ,
             userLocks[msg.sender][id].amount ,
              userLocks[msg.sender][id].claimed, 
              userLocks[msg.sender][id].lastclaimedAt,
              userLocks[msg.sender][id].harvestCount,
              userLocks[msg.sender][id].startTime,
                true, 
                false
                );
        emit LockMigrated(msg.sender , _to , id , userLockCount[_to] - 1);
        userLocks[msg.sender][id].isSet = false;


    }
    
     function batchlock(uint256 lockId , address[] memory users, uint256[] memory amounts ,uint256 startIn  , bool backdate) public onlyOwner {
        require(locks[lockId].isSet ,"invalid LockID");
     ...
     for(uint256  i ; i <users.length ; i++){
          emit LockAsset(lockId , users[i],  amounts[i] ,start);
    ...
    function available(address user,uint256 id) public view returns(uint256) {
         ...
         uint256 currentInterval = block.timestamp -  userLocks[user][id].lastclaimedAt;
         uint256 times =  currentInterval / locks[userLocks[user][id].lockID].intervals ;
         if(times == 0){
             return  0;
         }
        if(times + userLocks[user][id].harvestCount > locks[userLocks[user][id].lockID].period){
            times = locks[userLocks[user][id].lockID].period - userLocks[user][id].harvestCount;
        }
         return times * userLocks[user][id].amount / locks[userLocks[user][id].lockID].period;
    }
     function recallLock(address user , uint256 id ) public onlyOwner{
        ...
        emit LockRecalled(user , id , recallValue);
    ...
    function harvest(uint256 id) public {
        ...
        if(times + userLocks[msg.sender][id].harvestCount > locks[userLocks[msg.sender][id].lockID].period){
         ...
         emit Harvested(msg.sender , id , availableToClaim);
