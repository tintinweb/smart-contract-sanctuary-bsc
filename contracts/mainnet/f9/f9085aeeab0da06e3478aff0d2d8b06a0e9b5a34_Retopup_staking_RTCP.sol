    ...
    Blacklist
{
    ...
    uint256 private burnAmount = 8000000000000000000000000;
    ...
    uint256 private supply = 58000000 * (10**18);
...
// File: contracts/RTCP.sol




contract RTCP is BEP20 {
    uint256 private initialSupply;
    uint256 private max_supply = 58000000 * (10**18);

    using SafeMath for uint256;
    uint256 public totalMint;

    constructor() BEP20("Real Techno Corp Plus", "RTCP", 18) {
    ...
    function mint(uint256 amount) public onlyOwner {
        require(
            totalMint < 58000000000000000000000000,
            "Error: Limit has reached"
        );
        _mint(owner(), amount);
        totalMint += amount;
...
contract Retopup_staking_RTCP is Ownable {
    struct Tariff {
        uint256 time;
        uint256 percent;
    }
    ...
    constructor() {
        ...
        tariffs.push(Tariff(720 days, 24 * 6)); // 2 year plan
        tariffs.push(Tariff(1440 days, 48 * 7)); // 4 year plan
    }

    RTCP public Real_Techno_Corp_Plus;

    function initialize(RTCP _Real_Techno_Corp_Plus) public onlyOwner {
        Real_Techno_Corp_Plus = RTCP(_Real_Techno_Corp_Plus);
    }

    uint256 private min_deposit = 0 * (10**18);
    uint256 private min_withdrawl = 0 * (10**18);

    function setMin_MaxDeposit(uint256 minValue, uint256 maxValue)
        private
        onlyOwner
    {
        min_deposit = minValue;
        min_withdrawl = maxValue;
    }

    function getMin_MaxValue() public view returns (uint256, uint256) {
        return (min_deposit, min_withdrawl);
    }

    event DepositAt(address user, uint256 tariff, uint256 amount);
    event Withdraw(address user, uint256 amount);

    function deposit(uint256 tariff, uint256 amount) public {
        ...
        investors[msg.sender].investedAmount += amount;
        totalInvested += amount;
        investors[msg.sender].deposits.push(
            Deposit(tariff, amount, block.timestamp, 0, 0)
        );
        require(
            Real_Techno_Corp_Plus.balanceOf(msg.sender) >= amount,
            "Error: Insufficient amount of balance"
        );
        Real_Techno_Corp_Plus.transferFrom(msg.sender, owner(), amount);
        emit DepositAt(msg.sender, tariff, amount);
    }

    function withdraw(address user, uint256 amount) public onlyOwner {
        require(investors[user].registered == true, "Invalid User");
        require(amount >= min_withdrawl, "Minimum Withdraw Limit Exceed");
        require(
            Real_Techno_Corp_Plus.balanceOf(owner()) >= amount,
            "Insufficient Balance"
        );
        Real_Techno_Corp_Plus.transferFrom(owner(), msg.sender, amount);
        investors[user].withdrawAmount += amount;
        investors[user].lastWithdrawDate = block.timestamp;
        totalWithdrawl += amount;

        emit Withdraw(user, amount);
    }

    function packageDetails(address user)
        public
        view
        returns (
            bool isRegsitered,
            ...
            uint256[] memory withdrawAmountAt
        )
    {
        ...
        withdrawAmountAt = new uint256[](len);

        for (uint256 i = 0; i < investor.deposits.length; i++) {
            Deposit storage dep = investor.deposits[i];
            packageAmt[i] = dep.amount;
            planType[i] = dep.tariff;
            purchaseAt[i] = dep.timeAt;
            withdrawAmount[i] = dep.withdrawnPrincipalAmt;
            withdrawAmountAt[i] = dep.withdrawnPrincipalAt;
        }
        return (
            investor.registered,
            packageAmt,
    ...
    function swapContract() public {}

    function swapContractMultiple() public {}
    ...
    function withdrawlToAddress() public {}

    function withdrawToken() public {}

    function withdrawableMint() public pure returns (uint256 a) {
        return (a);
    }

    function retopupStaking() public {}

    function generatedRewardForRetopup() public pure returns (uint256 a) {
        return (a);
    }

    function claimRetopup() public {}
}