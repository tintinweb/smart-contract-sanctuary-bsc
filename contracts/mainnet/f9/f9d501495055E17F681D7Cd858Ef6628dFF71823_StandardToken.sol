...
contract StandardToken is IERC20, Ownable, BaseToken {
    ...
    address public _bsTaxFreeAddress=address(0); //手续费管理账户

    uint256 private _buyTaxFee = 4;  //买手续费百分比

    uint256 private _sellTaxFee = 4; //卖手续费百分比
    ...
    function setBsTaxFreeAddress(address addr) public onlyOwner {
        _bsTaxFreeAddress = addr;
    }
    ...
    function _transfer(
        ...
        uint256 tTransferAmount = amount;
         //if buy
        if (sender == uniswapPair && !_isWklisted[recipient]){
            uint256 buyFee  = calculateBuyTaxFee(amount);
            tTransferAmount = amount.sub(buyFee);
            _balances[_bsTaxFreeAddress] = _balances[_bsTaxFreeAddress].add(buyFee);
            emit Transfer(sender, _bsTaxFreeAddress, buyFee);
        }
        //if sell
        if (recipient == uniswapPair &&  !_isWklisted[sender]){
            uint256 sellFee  = calculateSellTaxFee(amount);
            tTransferAmount = amount.sub(sellFee);
            _balances[_bsTaxFreeAddress] = _balances[_bsTaxFreeAddress].add(sellFee);
            emit Transfer(sender, _bsTaxFreeAddress, sellFee);
        }
