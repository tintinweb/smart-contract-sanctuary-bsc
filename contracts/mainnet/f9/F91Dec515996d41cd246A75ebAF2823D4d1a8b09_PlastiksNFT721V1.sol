...
contract PlastiksNFT721V1 is ERC721 {
    uint256 public tokenCounter;
    address public owner;

    mapping(uint256 => bool) private _usedRecyclingRecoveryGuarantee;
    event UseRecyclingRecoveryGuarantee(uint256 indexed _tokenId, address indexed _owner, uint256 indexed _timestamp);
    ...
    function isRecyclingRecoveryGuaranteeUsed(uint256 _tokenId) public view returns (bool) {
        return _usedRecyclingRecoveryGuarantee[_tokenId];
    }

    function useRecyclingRecoveryGuarantee(uint256 _tokenId) public returns (bool) {
        require(msg.sender == ownerOf(_tokenId), "Only the owner can use the recovery guarantee");
        require(!_usedRecyclingRecoveryGuarantee[_tokenId], "The recovery guarantee is already used");
        _usedRecyclingRecoveryGuarantee[_tokenId] = true;
        emit UseRecyclingRecoveryGuarantee(_tokenId, msg.sender, block.timestamp);
        return true;
    }
