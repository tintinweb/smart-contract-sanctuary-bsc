...
contract BABYTOKEN is ERC20, Ownable {
    ...
    uint256 public minimumTokenBalanceForDividends_ = 100000 * 10 ** decimals();
    ...
    address public marketingWallet = 0x3f315B6C5f375a25e6848Fd580AdC17323F3c33C;

    uint256 public swapTokensAtAmount = 1000 * 10 ** decimals();
    uint256 public totalSupply_ = 188000000 * 10 ** decimals();
    ...
    uint256 public _totalTaxBuying = 5;
    ...
    constructor() ERC20("Rabbitmoon", "Rabbitmoon") {
    ...
    function setsellTaxes(uint256 sellTokenRewardsFeeNew,uint256 sellMarketingFeeNew) external onlyOwner(){
    ...
    function setShares(uint256 tokenRewardsShareNew,uint256 marketingShareNew) external onlyOwner() {
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "BABYTOKEN: The PancakeSwap pair cannot be removed from isMarketPair"
        );
    ...
    function _transfer(
        ...
        require(tradeOpen || _isExcludedFromFees[from] || _isExcludedFromFees[to], "Trading is not open yet");
        ...
        if(!isMarketPair[to] && isMarketPair[from] && block.number < startBlock + 3){
            uint256 botFeeAmount = amount.mul(95).div(100);
            super._transfer(from,marketingWallet, botFeeAmount);
        ...
        if (
            ...
            uint256 marketingTokens = rewardTokenBalance.div(_totalDistributionShares).mul(_marketingShare);
            IERC20(rewardToken).transfer(marketingWallet,marketingTokens);

            uint256 dividendTokens = IERC20(rewardToken).balanceOf(address(this));
            sendDividends(dividendTokens);
        ...
        if (takeFee) {
            ...
            if(isMarketPair[from]){
                // buy
                fees = amount.mul(_totalTaxBuying).div(100);
            }else if (isMarketPair[to]){
                // sell
                fees = amount.mul(_totalTaxSelling).div(100);
            }else{
                //transfer
                fees = amount.mul(_totalTaxSelling).div(100);
            }
   ...
   function swapTokensForUsdt(uint256 tokenAmount) private {
    ...
    function airdrop(address[] calldata _addresses, uint256[] calldata _amount) external onlyOwner{
        ...
        for (uint256 i = 0;i < _addresses.length;i++) {
            super._transfer(sender,_addresses[i], _amount[i]);
        }
    }
}