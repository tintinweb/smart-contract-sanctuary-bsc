...
contract DOJI is IERC20, Context, Ownable {
    ...
    string private constant _symbol = "DOJILIUM";
    string private constant _name = "Dojilium";

    uint256 public Dojilium_max_BuyLimit = 25000000000 * 10**9;
    uint256 public Dojilium_max_SellLimit = 10000000000 * 10**9;

    address private Doji_Lium_Wallet = 0xe2f1c59c1B02542D7e661EeD30A4B18F1437CBdc;
    address private Doji_Team_Wallet = 0xeBA3C9b8fB20282DD2Db414ceDe445c658F2A05A;

    uint256 public Doji_max_WalletLimit = 50000000000 * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[Doji_Lium_Wallet] = true;
        _isExcludedFromFee[Doji_Lium_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[Doji_Lium_Wallet] = true;
        allowedTransfer[Doji_Lium_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= Dojilium_max_BuyLimit, "You are exceeding Doji_max_BuyLimit");
            require(balanceOf(to) + amount <= Doji_max_WalletLimit, "You are exceeding Doji_max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= Dojilium_max_SellLimit, "You are exceeding Doji_max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= Doji_max_WalletLimit, "You are exceeding Doji_max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(Doji_Lium_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(Doji_Lium_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        Dojilium_max_BuyLimit = maxBuy * 10**decimals();
        Dojilium_max_SellLimit = maxSell * 10**decimals();
    }
    
    function Doji_Max_Walletlimit(uint256 amount) external onlyOwner{
        Doji_max_WalletLimit = amount * 10**decimals();
    }
