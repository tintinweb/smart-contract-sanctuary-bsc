...
import {TokenAirdrop} from "./TokenAirdrop.sol";

import {Token} from "./Token.sol";


contract SignatureAirdrop is Ownable, ReentrancyGuard {
    using ECDSA for bytes32;
    address private _systemAddress;
    mapping(string => bool) public _usedNonces;
    TokenAirdrop private _tokenAirdrop;
    Token private _token;

    function init(
        address systemAddress_,
        address tokenAirdropAddress_,
        address tokenAddress_
    ) external onlyOwner {
        _systemAddress = systemAddress_;
        _tokenAirdrop = TokenAirdrop(tokenAirdropAddress_);
    ...
    function airDrop(uint256 amount,
        ...
        _tokenAirdrop.airDrop(msg.sender, amount);
    }
...
contract TokenAirdrop is Ownable, ReentrancyGuard {
    ...
    SignatureAirdrop private _signatureAirdrop;
    ...
    function init(
        address signatureAirdropAddress_
    ) external onlyOwner {
        _signatureAirdrop = SignatureAirdrop(signatureAirdropAddress_);
    }
    ...
    function airDrop(address _beneficiary, uint256 _amount) public {
        require(address(_signatureAirdrop) == msg.sender || owner() == msg.sender, "TokenAirdrop: only whitelist can participate airdrop!");
        require(_airdropAmount[_beneficiary] <= 0, "TokenAirdrop: can participate airdrop only once");
        _token.transfer(_beneficiary, _amount);
        _airdropAmount[_beneficiary] = _amount;
    }

    function getAirdropAmount(address _beneficiary)
    public
    view
    returns (uint256){
        return _airdropAmount[_beneficiary];
    }
    ...
    onlyOwner {
        ...
        _token.safeTransfer(owner(), balance_);
    }
