...
// File contracts/swap/SwapV2.sol
// SPDX-License-Identifier: MIT
pragma solidity 0.8.9;

contract SwapV2 is Initializable, AccessControlUpgradeable, PausableUpgradeable {
  ...
  mapping(address => bool) public trustedNft;
  ...
  function initialize(
    ...
    TradeOffer memory tradeOffer = TradeOffer(
      0,
      address(0),
      0,
      address(0),
  ...
  function setTrustedNfts(address[] calldata _trustedNfts) external onlyRole(OPERATOR_ROLE) {
    for (uint256 i = 0; i < _trustedNfts.length; i++) {
      trustedNft[_trustedNfts[i]] = true;
    }
  }

  function removeTrustedNfts(address[] calldata _trustedNfts) external onlyRole(OPERATOR_ROLE) {
    for (uint256 i = 0; i < _trustedNfts.length; i++) {
      trustedNft[_trustedNfts[i]] = false;
    }
  }
  ...
  function createTradeOffer(
    address _traderNftAddress,
    uint256 _traderTokenId,
    address _friendNftAddress,
    uint256 _friendTokenId,
    address _friend
  ) external whenNotPaused returns (uint256) {
    require(trustedNft[_traderNftAddress], "INV_TRADER_NFT");
    require(trustedNft[_friendNftAddress], "INV_FRIEND_NFT");
    ...
    if (_traderNftAddress == _friendNftAddress) {
      require(_traderTokenId != _friendTokenId, "INV_TOKEN_ID");
    }

    require(msg.sender == IERC721(_traderNftAddress).ownerOf(_traderTokenId), "NOT_OWNER");
    require(_friend == IERC721(_friendNftAddress).ownerOf(_friendTokenId), "FRIEND_NOT_OWNER");
    ...
    if (tokenIdToOfferId[_traderTokenId] != 0) {
      ...
      emit TradeOfferCancelled(
        _offer.id,
        _offer.friendNftAddress,
        _offer.friendTokenId,
        _offer.traderNftAddress,
    ...
    TradeOffer memory offer = TradeOffer({
      id: _offerId,
      friendNftAddress: _friendNftAddress,
      friendTokenId: _friendTokenId,
      traderNftAddress: _traderNftAddress,
    ...
    emit TradeOfferCreated(
      offer.id,
      offer.friendNftAddress,
      offer.friendTokenId,
      offer.traderNftAddress,
      offer.traderTokenId,
      offer.trader,
      offer.friend
    );
  ...
  function executeTradeOffer(uint256 offerId) external whenNotPaused {
    ...
    if (
      offer.trader != IERC721(offer.traderNftAddress).ownerOf(offer.traderTokenId) ||
      offer.friend != IERC721(offer.friendNftAddress).ownerOf(offer.friendTokenId)
    ) {
    ...
    IERC721(offer.traderNftAddress).transferFrom(offer.trader, offer.friend, offer.traderTokenId);
    IERC721(offer.friendNftAddress).transferFrom(offer.friend, offer.trader, offer.friendTokenId);
  }
  ...
  function transferNFT(
    ...
    require(trustedNft[_nftAddress], "INV_NFT");
    require(msg.sender == IERC721(_nftAddress).ownerOf(_tokenId), "NOT_OWNER");
