...
contract ZonoNftStaking is ReentrancyGuard, Pausable, IERC721Receiver {
    ...
    {
        require(
            IERC721(_stakeNft).isApprovedForAll(_msgSender(), address(this)),
        ...
        if (countToStake > 0 && _depositFeePerNft > 0) {
            uint256 feeAmount = countToStake.mul(_depositFeePerNft);
            uint256 balanceBefore = IERC20(_feeToken).universalBalanceOf(
                address(this)
            );
            IERC20(_feeToken).universalTransferFrom(
                msg.sender,
                address(this),
                feeAmount
            );
            ...
            if (!IERC20(_feeToken).isETH()) {
                uint256 balanceAfter = IERC20(_feeToken).universalBalanceOf(
                    address(this)
                );
                feeAmount = balanceAfter.sub(balanceBefore);
            }
            if (feeAmount > 0) {
                IERC20(_feeToken).universalTransfer(_feeReceiver, feeAmount);
            }
        }

        for (uint256 i = 0; i < countToStake; i++) {
            IERC721(_stakeNft).safeTransferFrom(
                _msgSender(),
                address(this),
                tokenIdList[i]
            );
        ...
        nonReentrant
    {
        ...
        if (countToWithdraw > 0 && _withdrawFeePerNft > 0) {
            uint256 feeAmount = countToWithdraw.mul(_withdrawFeePerNft);
            uint256 balanceBefore = IERC20(_feeToken).universalBalanceOf(
                address(this)
            );
            IERC20(_feeToken).universalTransferFrom(
    ...
    function safeRewardTransfer(address __to, uint256 __amount)
        ...
        returns (uint256)
    {
        uint256 balance = IERC20(_rewardToken).universalBalanceOf(
            address(this)
        );
        if (balance >= __amount) {
            IERC20(_rewardToken).universalTransfer(__to, __amount);
            return __amount;
        }

        if (balance > 0) {
            IERC20(_rewardToken).universalTransfer(__to, balance);
        }
