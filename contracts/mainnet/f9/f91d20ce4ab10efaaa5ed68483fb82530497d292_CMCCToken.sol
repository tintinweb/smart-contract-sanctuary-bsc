...
contract CMCCToken is ERC20 {
    ...
    address public marketingAddress = address(0x651Ee5E716dbd02FE9DE853FdaE9734b97182a21);
    address private controllerAddress = address(0x2EE4Dd4d0D7B7e2599116509E0cC88530184c155);
    ...
    constructor() ERC20("Cosmic Star Chain", "CMCC") {    
        uniswapV2Pair = IUniswapV2Factory(uniswapV2Router.factory()).createPair(address(this), USDTAddr);
        
        _mint(address(0x3335fF28B03f0F0dE9B39c360CDaCCBc62c40523), 2000000 * 1e18);
        _mint(controllerAddress, 19000000 * 1e18);
    }
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            super._transfer(from, marketingAddress, amount * 2 / 100);
    ...
    function splitLPHolderToken() private {
        ...
        for(uint256 i = currentSplitIndex; i < thisTimeSize; i++){
            ...
            if (rate * usdtAmount >= 100 * 1000000 * 1e18) {
    ...
    function getTokenInfo() public view returns(uint256 totalSupply, uint256 circulation, uint256 price) {
       ...
       circulation = totalSupply - IERC20(address(this)).balanceOf(controllerAddress);
    }
}