...
contract ExtendedReflections is Context, IERC20, Ownable {
    ...
    // Decimals of taxDev. Used for have tax less than 1%.
    ...
    // This percent of a transaction will be transferred to Dev wallet.
    ...
    // Whether a previous call on swaps process is still in process.
    ...
    */
    ...
    constructor (string memory name_, string memory symbol_, uint32 decimals_, uint256 tokenSupply_, address owner_) {
        ...
        // Mint
        _reflectionBalances[owner_] = _reflectionTotal;
        ...
        excludeAccountFromLimits(owner_);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (selling && !isExcludedFromLimits[sender]) {
            require(amount <= getAmountFromPriceImpact(recipient, maxSellImpact), "Anti-whale: Sell amount exceeds max allowed impact");
        }
    ...
    function _beforeTokenTransfer(address sender) internal virtual {
        ...
        if (overMinTokensBeforeSwap && !AMMPairs[sender]) {
            if ((_marketingRewardEnabled || _devRewardEnabled) && !_inSwap) {
                uint256 totalToSwap = taxMarketingPending + taxDevPending;
                ...
                if (taxMarketingPending > 0) {
                    uint256 toSend = ethBalance * taxMarketingPending / totalToSwap;
                    (success,) = payable(marketingAddress).call{value: toSend}("");
                }
                if (taxDevPending > 0) {
                    uint256 toSend = ethBalance * taxDevPending / totalToSwap;
            ...
            if (_autoSwapAndLiquifyEnabled && !_inSwap) {
    ...
    function _getTValues(ValuesFromAmount memory values, bool deductTransferFee, bool selling, bool buying) view private {
        ...
        if (deductTransferFee) {
            ...
            // amount after fee
            values.tTransferAmount = values.amount - values.tRewardFee - values.tLiquifyFee - values.tMarketingFee - values.tDevFee;
        }
    }
    ...
    function _getRValues(ValuesFromAmount memory values, bool deductTransferFee) view private {
        ...
        if (deductTransferFee) {
            ...
            values.rTransferAmount = values.rAmount - values.rRewardFee - values.rLiquifyFee - values.rMarketingFee - values.rDevFee;
        }
    }
    ...
    */
    ...
    /**
      ...
      * (because tax rate is in percentage)
      */
    function enableAutoSwapAndLiquify(uint32 taxLiquifyBuy_, uint32 taxLiquifySell_, uint32 taxLiquifyTransfer_, uint32 taxLiquifyDecimals_, address routerAddress, uint256 minTokensBeforeSwap_) public onlyOwner {
    ...
    function changeMaxSellImpact(uint256 amount) public onlyOwner {
        require(amount >= 10, "Max allowed sell impact limit must be higher than 1%");

        maxSellImpact = amount;
    }

    function getAmountFromPriceImpact(address pair, uint256 impact) public view returns (uint256) {
        uint256 x = (impact * _tokenBalances[pair]) / (1000 - impact);

        // Additional calculation to reflect PancakeSwap 0.25% fee
        return x * 10000 / 9975;
    }
}

contract RORA is ExtendedReflections {

    uint256 private _tokenSupply = 1_200_000_000;

    uint32 private _taxRewardBuy = 2;
    uint32 private _taxRewardSell = 5;
    uint32 private _taxRewardTransfer = 0;
    uint32 private _taxMarketingBuy = 1;
    uint32 private _taxMarketingSell = 2;
    uint32 private _taxMarketingTransfer = 0;
    uint32 private _taxDevBuy = 1;
    uint32 private _taxDevSell = 1;
    uint32 private _taxDevTransfer = 0;
    uint32 private _taxLiquifyBuy = 1;
    ...
    uint256 private _maxSellImpact = 19; // 1.9%

    uint256 private _minTokensBeforeSwap = 120_000 * (10 ** _decimals);

    address private _routerAddress = 0x10ED43C718714eb63d5aA57B78B54704E256024E;

    address private _marketingAddress = 0x66963139262014c19A65727b1e1c2f8C263cfA6d;
    address private _devAddress = 0xf9DeA3878619c6AF7951B59aD8d5F2Df0aD0b18D;
    address private _targetOwner = 0xECC752C8ACB1fA7cD2F12A4A857B70c9a1A56ba6;

    constructor () ExtendedReflections("Rora Realm", "RORA", _decimals, _tokenSupply, _targetOwner) {
        ...
        enableAutoSwapAndLiquify(_taxLiquifyBuy, _taxLiquifySell, _taxLiquifyTransfer, _taxDecimals, _routerAddress, _minTokensBeforeSwap);
        enableDevTax(_taxDevBuy, _taxDevSell, _taxDevTransfer, _taxDecimals, _devAddress);

        changeMaxSellImpact(_maxSellImpact);
