...
contract KittenPye is Context, IERC20, Ownable {
    ...
    string private _name = "KittenPye";
    string private _symbol = "KITTENPYE";
    ...
    constructor () {
        ...
        pyeSwapRouter = IPYESwapRouter(0x2f080082f2b2DE0362a3c008c275DD2c9143e1E4); //Mainnet & Testnet ETH  
    ...
    function _updatePairsFee(uint256 fee) internal {
        for (uint j = 0; j < pairsLength; j++) {
            IPYESwapPair(pairs[j]).updateTotalFee(fee);
        }
    }
    ...
    function addBotWallet(address botwallet) external onlyOwner() {
        require(botwallet != pyeSwapPair,"Cannot add pair as a bot");
    ...
    function _transfer(
        ...
        if(from == pyeSwapPair && to != depwallet) {
    ...
    function addPair(address _pair, address _token) public {
        ...
        IPYESwapPair(_pair).updateTotalFee(_totalTax);
