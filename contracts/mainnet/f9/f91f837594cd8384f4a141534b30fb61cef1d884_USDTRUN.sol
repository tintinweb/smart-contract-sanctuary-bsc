...
contract USDTRUN {
    ...
    constructor(address payable marketingAddr,IBEP20 tokenAdd ) {
     ...
     function withdraw_bnb_amount(uint256 _amt) external{
    ...
    function invest(address payable referrer,uint256 token_quantity) public payable beforeStarted() {
        ...
        token.transfer(owner,tokenWei.mul(500).div(PERCENTS_DIVIDER));
    ...
    function withdraw() public beforeStarted() {
        ...
        token.transfer(owner,fees.div(2));
     ...
     function getTimer(address userAddress) public view returns (uint256) {
       ...
       return users[userAddress].checkpoint.add(5 minutes);
