...
//        require(priceTool.getPrice(_faf, _usdt, _fafAmount) >= 100, "price too little");
    ...
    function bind(address _target) external {
        ...
        PledgeOrder storage order;
        bool isTrue = true;
        if(top1 != address(0)){
            order = _orders[msgsender];
            teamTotal[top1] -= order.uAmount / 2;
            msgsender = top1;
            top1 = recommend[top1];
            if(top1 != address(0)){
                while(isTrue){
            ...
            recommendAmount[top1] --;
        }
        recommendAmount[_target] ++;
        recommend[msg.sender] = _target;
        PledgeOrder storage iorder = _orders[msg.sender];
        uint256 uAmount = iorder.uAmount / 2;
        top1 = _target;
        for(uint256 i = 0; i < 30; i ++){
            if(address(0) != top1){
