...
contract Config is Team {
    ...
    function setSedimentAddress(address addr) external isOwner returns (bool) {
        sedimentAddress = addr;
        return true;
    }
...
abstract contract ERC20 is IERC20, ERC20Detailed, Config {
    ...
    uint256 public AmountMarketingFeeBuy;
    uint256 public AmountMarketingFeeSell;
        ...
        if (whiteList[sender] || whiteList[recipient]) {
            ...
            if (sender == address(uniswapV2Pair)) {
                ...
                if (usdtSwitch) {
                    ...
                    AmountMarketingFeeBuy += usdtAmount;
                    if (_balances[address(this)] >= minSwapAndLiquifyLimit && AmountMarketingFeeBuy > 0) {
                        swapAndSendToFee(AmountMarketingFeeBuy);
                        AmountMarketingFeeBuy = 0;
                    }
                }
                ...
                if (usdtSwitch) {
                    ...
                    AmountMarketingFeeSell += usdtAmount;
                    if (_balances[address(this)] >= minSwapAndLiquifyLimit && AmountMarketingFeeSell > 0) {
                        swapAndSendToFee(AmountMarketingFeeSell);
                        AmountMarketingFeeSell = 0;
                    }
                }
    ...
    function swapAndLiquify() private lockTheSwap {
        ...
        uint256 newBalance = IERC20(usdtToken).balanceOf(smartVault).sub(
            initialBalance
        );
    ...
    function addLiquidity(uint256 usdtAmount, uint256 tokenAmount) private {
     ...
     function swapAndSendToFee(uint256 tokens) private  {
        uint256 initialCAKEBalance = IERC20(usdtToken).balanceOf(address(this));
        swapTokensForCake(tokens);
        uint256 newBalance = (IERC20(usdtToken).balanceOf(address(this))).sub(initialCAKEBalance);
        IERC20(usdtToken).transfer(usdtManageAddress, newBalance);
    }
