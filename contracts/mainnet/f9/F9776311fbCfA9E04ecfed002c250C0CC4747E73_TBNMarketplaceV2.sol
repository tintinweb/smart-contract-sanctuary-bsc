...
contract TBNMarketplaceV2 is ERC721Holder, AccessControl, ReentrancyGuard {
  ...
  function updateTbnNftAddress(address _tbnNftAddress) external onlyAdmin {
    require(
      _tbnNftAddress != address(0),
      "TBN address cannot be 0"
    );
    tbnNftAddress = _tbnNftAddress;
  }
    ...
    nonReentrant
  {
    ...
    if (!listing.isPriceBound) {
      ...
      if (
        ...
        paymentPrice = chainLink.getLatestPrice(
          listing.paymentTokenAddress
        );
        tbnTokenDecimals = 8;
        paymentTokenDecimals = 8;
        paymentTokensPerTBNToken = (tbnPrice * 1 ether) / paymentPrice;
      } else if(listing.paymentTokenAddress == address(0) || listing.paymentTokenAddress == wethAddress) {
        // tbn address is not on chainlink
        // WETH to ETH is 1 to 1
        (uint256 tbnWETHReserves, uint256 tbnTokenReserves) = oracle.getReservesForTokenPool(TBNAddress);
        ...
        (uint256 paymentWETHReserves, uint256 paymentTokenReserves) =  oracle.getReservesForTokenPool(listing.paymentTokenAddress);
