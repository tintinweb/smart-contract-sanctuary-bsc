...
contract ERC20 is Context, IERC20,Ownable {
     ...
     * construction.
     */
    constructor (string memory name_, string memory symbol_,uint256 _total) public {
        _name = name_;
        _symbol = symbol_;
        _decimals = 18;
        total = _total * _DECIMALFACTOR;
    }
...
contract mContract is ERC20{


    bool public isInit = false;

    constructor(uint256 _total, string memory name, string memory symbol) ERC20(name, symbol,_total) public {
    }
    ...
    function initSupply(address _preSale,
                ...
                address _team,
                address _burn,
                address _fee) external onlyOwner {
          require(!isInit, "inited");
          isInit = true;
          _mint(_preSale, total.mul(52).div(100));
          _mint(_marketingAddr, total.mul(1).div(100));
          _mint(_ecoSystem, total.mul(30).div(100));
          _mint(_team, total.mul(6).div(100));
          _mint(_burn, total.mul(10).div(100));
          _mint(_fee, total.mul(1).div(100));
    }
