...
contract AiExchange is Ownable {
    using SafeMath for uint256;
    ...
    constructor(address _swapRouter, address beneficiary) {
    ...
    function initialize(address _token) external {
        ...
        token = _token;
        pairToken = IPancakeSwapFactory(router.factory()).getPair(token, usdtAddress);
    }

    //buy token
    function swapUsdtForToken(uint256 tokenAmount, address to) public {
        address[] memory path = new address[](2);
        path[0] = usdtAddress;
        path[1] = token;

        IERC20(usdtAddress).transferFrom(to, address(this), tokenAmount);
    ...
    //sell token
    function swapForUsdt(uint256 tokenAmount, address to) public {
        address[] memory path = new address[](2);
        path[0] = token;
        path[1] = usdtAddress;

        IERC20(token).transferFrom(to, address(this), tokenAmount);
    ...
    function withdrawToken(address _token) public _onlyBeneficiary {
        uint256 value = getBalanceFromToken(_token);
        IERC20(token).transfer(beneficiaryAddress, value);
    }

    function withdrawToken(address _token, uint256 val) public _onlyBeneficiary {
        uint256 value = getBalanceFromToken(_token);
