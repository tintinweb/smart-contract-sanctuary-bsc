...
// File: contracts/celer/message/framework/MessageBusAddress.sol
...
// File: contracts/celer/message/libraries/MsgDataTypes.sol
...
// File: contracts/celer/interfaces/IBridge.sol
...
// File: contracts/celer/interfaces/IOriginalTokenVault.sol
...
// File: contracts/celer/interfaces/IOriginalTokenVaultV2.sol
...
// File: contracts/celer/interfaces/IPeggedTokenBridge.sol
...
// File: contracts/celer/interfaces/IPeggedTokenBridgeV2.sol
...
// File: contracts/celer/message/interfaces/IMessageBus.sol
...
// File: contracts/celer/message/libraries/MessageSenderLib.sol
...
// File: contracts/celer/interfaces/ISigsVerifier.sol
...
// File: contracts/celer/message/messagebus/MessageBusSender.sol
...
// File: contracts/celer/message/interfaces/IMessageReceiverApp.sol
...
// File: contracts/celer/message/messagebus/MessageBusReceiver.sol
...
// File: contracts/celer/message/messagebus/MessageBus.sol
...
// File: contracts/celer/message/framework/MessageSenderApp.sol
...
// File: contracts/celer/message/framework/MessageReceiverApp.sol
 ...
 */
contract DVFDepositContract is Ownable, MessageSenderApp, MessageReceiverApp {
  ...
  struct ProxiedWithdrawalRequest {
      // SwapInfo swap;
  ...
  event ProxiedBridgedWithdrawalExecuted(address indexed user, address indexed token, uint256 amount, uint64 executionChainId, uint64 nonce);
  event ProxiedBridgedWithdrawalRequested(address indexed user, address indexed token, uint256 amount, uint64 requestChainId, uint64 nonce);
  event ProxiedBridgedWithdrawalFallback(address indexed sender, address indexed token, uint256 amount, uint64 requestChainId, address indexed executor);
  event ProxiedBridgedWithdrawalRefund(address indexed sender, address indexed token, uint256 amount, uint64 requestChainId, address indexed executor);
  ...
  //   }
    ...
    function crossChainTransferWithSwap(
        address receiverContract,
        uint64 _dstChainId,
        address _finalFundsReceiver,
        address _tokenIn,
        address _tokenOut,
        uint256 _amount,
        uint256 amountOut,
        ...
        bytes memory message = abi.encode(
            ProxiedWithdrawalRequest({receiver: _finalFundsReceiver, token: _tokenOut, amount: amountOut, nonce: _nonce})
        );
        // bytes32 id = _computeProxiedWithdrawalRequestId(msg.sender, _chainId, _dstChainId, message);
        ...
        emit ProxiedBridgedWithdrawalRequested(
          _finalFundsReceiver,
          _tokenIn,
          _amount,
          // this.getChainID(),
    ...
    * @param _message ProxiedWithdrawalRequest message that defines the swap behavior on this destination chain
    */
  function executeMessageWithTransfer(
      ...
      ProxiedWithdrawalRequest memory message = abi.decode((_message), (ProxiedWithdrawalRequest));
      // require(_token == m.swap.path[0], "bridged token must be the same as the first token in destination swap path");
      IERC20Upgradeable(_token).safeTransfer(message.receiver, message.amount);
      emit ProxiedBridgedWithdrawalExecuted(message.receiver, _token, _amount, _srcChainId, message.nonce);
  ...
  function executeMessageWithTransferFallback(
    ...
    emit ProxiedBridgedWithdrawalFallback(_sender, _token, _amount, _srcChainId, _executor);
    return IMessageReceiverApp.ExecutionStatus.Success;
  }
    ...
    function executeMessageWithTransferRefund(
      ...
      emit ProxiedBridgedWithdrawalRefund(address(this), _token, _amount, uint64(block.chainid), _executor);
