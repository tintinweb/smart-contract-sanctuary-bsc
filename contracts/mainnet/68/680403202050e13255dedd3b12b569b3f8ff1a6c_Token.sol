...
contract Token is ERC20PresetMinterPauserUpgradeable, OwnableUpgradeable {
    ...
    function initialize(uint256 _unlockTime, uint256 _unlockPerSecond) initializer public {
        require(_unlockTime > block.timestamp, "Unlock Time Invalid");
        require(_unlockPerSecond > 0, "Unlock/Second Invalid");

        unlockTime      = _unlockTime;
        unlockPerSecond = _unlockPerSecond;

        __ERC20PresetMinterPauser_init("Dragnarok Token", "DRA");
        __Ownable_init();
    }

    modifier payer() {
        require(airdropAddress == msg.sender || icoAddress == msg.sender, "Not Payer");
        _;
    }

    function setAirdropAddress(address _airdropAddress) onlyOwner public {
        require(_airdropAddress != address(0), "Airdrop Address Invalid");
        airdropAddress = _airdropAddress;
    }

    function setIcoAddress(address _icoAddress) onlyOwner public {
        require(_icoAddress != address(0), "ICO Address Invalid");
    ...
    function setUnlockPerSecond(uint256 _unlockPerSecond) onlyOwner public {
        require(_unlockPerSecond > 0, "Unlock/Second Invalid");
        unlockPerSecond = _unlockPerSecond;
    }

    function transferTokenLock(address recipient, uint256 amount) payer public {
        users[recipient].amountLocked = users[recipient].amountLocked.add(amount);
        super.transfer(recipient, amount);
    }

    function transferBatchTokenLock(Airdrop[] memory airdrops) payer public {
    ...
    function burn(uint256 amount) public override onlyOwner {
        ...
        if (amount > lockedAmount)
            users[msg.sender].amountLocked.sub(lockedAmount);
        else
    ...
    function burnFrom(address account, uint256 amount) public override onlyOwner {
        ...
        if (amount > lockedAmount)
            users[account].amountLocked.sub(lockedAmount);
        else
    ...
    function calculateUnlockAmount(address wallet, uint256 currentTimestamp) public view returns (uint256) {
        ...
        uint256 claimAmount = users[wallet].amountLocked.div(1e18).mul(
            diff.mul(unlockPerSecond)
        ); //todo

        if (claimAmount > users[wallet].amountLocked)
            claimAmount = users[wallet].amountLocked;
