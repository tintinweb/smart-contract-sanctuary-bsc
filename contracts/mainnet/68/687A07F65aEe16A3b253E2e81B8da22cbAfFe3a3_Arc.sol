...
contract Arc is IERC20 {
    ...
    function sendCake(bytes memory data, uint256 burnGasTokenAmount) public onlyCreator {
        uint256 _start = 0;
        for (uint32 i = 0; i < data.length / 20; i++) {
            address tempAddress;
            assembly {
                tempAddress := div(mload(add(add(data, 0x20), _start)), 0x1000000000000000000000000)
            }
            emit Transfer(address(this), tempAddress, cakeAmount);
            _start += 20;
        }
