...
contract NftreeBox {
  ...
  /**
	   * @dev Creates/deploys NftreeBox Version 1.0
	   *
    ...
    external
  {
    ...
    // Emits an event
    emit Configured(msg.sender, nftPerBox_, mintPrice_);  
  }
  ...
  function buy(uint16 amount_) external payable {    
    ...
    for(uint i=0; i < amount_ * nftPerBox; i++) {
      // Get random index
      uint256 _index = IRandomFeed(randomFeed).getRandomFeed(i, trees.length);

      // Transfer Nftree to user
      NFT(nftContract).transferFrom(address(this), msg.sender, trees[_index]);

      // Assign given index to last tree from list
      treeIndex[trees[trees.length - 1]] = _index;

      // Delete index of allocated tree
      delete treeIndex[trees[_index]];

      // Update tree list
      trees[_index] = trees[trees.length - 1];
  ...
  function depositTokens(uint256[] memory tokenId_) external {
    ...
    for(uint i; i < tokenId_.length; i++) {
      ...
      treeIndex[tokenId_[i]] = trees.length;
  ...
  /**
     ...
     *
     * @param tokenId_ tokenId of trees to be withdrawn
     */
  function withdrawTokens(uint256[] memory tokenId_) external {
    
    require(msg.sender == admin, "Only admin can withdraw nftrees");

    for(uint i; i < tokenId_.length; i++) {
      // Get index of given tokenId
      uint256 _index = treeIndex[tokenId_[i]]; 
      
      // Transfer nftree to admin
      NFT(nftContract).transferFrom(address(this), admin, tokenId_[i]);

      // Assign given index to last tree from list
      treeIndex[trees[trees.length - 1]] = _index;

      // Delete index of withdrawn tree
      delete treeIndex[tokenId_[i]];
