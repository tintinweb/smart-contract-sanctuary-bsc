...
contract test is Context, IERC20, Ownable {
    ...
    address payable private teamAddress = payable(0xd21a4781EAE736741D4A3926d9f8266644b7737e); // team Address
    ...
    uint256 public _buyteamFee = 6; //team fee will be use for marketing and charity
    ...
    uint256 public _sellteamFee = 20; //team fee will be use for marketing and charity
    uint256 private _previoussellteamFee = _sellteamFee;
    
    uint256 private teamDivisor = 8;

    uint256 public _totalTaxIfBuying = 8;  
    uint256 public _totalTaxIfSelling = 22;  
    
    uint256 public _maxTxAmount = 10000 * 10**6 * 10**9;
    uint256 public _maxWalletToken = 20000 * 10**6 * 10**9;
    uint256 private minimumTokensBeforeSwap = 1245125145 * 10**9;
    ...
    function manualSend() external onlyOwner {
        ...
        payable(teamAddress).transfer(contractETHBalance);
    }
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner()) {
            require(amount <= _maxWalletToken, "Total Holding is currently limited, you can not buy that much.");
        }
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        transferToAddressETH(teamAddress, transferredBalance.div(_buyteamFee).mul(teamDivisor));
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        uint256 tFee = calculatebuytaxFee(tAmount);
        uint256 tLiquidity = calculatebuyteamFee(tAmount);
    ...
    function calculatebuyteamFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_buyteamFee).div(
            10**2
        );
    }
    ...
    function calculatesellteamFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_sellteamFee).div(
            10**2
        );
    }

    function calculatetotaltaxifbuying(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_buytaxFee).add(_buyteamFee).div(
            10**2
        );
    }

    function calculatetotaltaxifselling(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_selltaxFee).add(_sellteamFee).div(
            10**2
        );
    }
    ...
    function setbuyteamFeePercent(uint256 buyteamFee) external onlyOwner() {
