...
contract DESToken is IERC20, AccessControlEnumerable {

    bytes32 public constant MANAGER_ROLE = keccak256("MANAGER_ROLE");
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    bytes32 public constant BURNER_ROLE = keccak256("BURNER_ROLE");

    string private constant NAME = "Dragonaire Evolutionary Stone";
    string private constant SYMBOL = "DES";
    ...
    uint256 public maxTxAmount = 500_000 ether;
    uint256 public numTokensToSwap = 50_000 ether;

    bool inSwapAndLiquify;
    bool public swapAndLiquifyEnabled = true;

    event UpdatedFee(uint256 oldFee, uint256 newFees);
    ...
    event UpdatedMaxTxAmount(uint256 amount);
    ...
    function setMaxTxAmount(uint256 amount) external onlyRole(MANAGER_ROLE) {
        require(amount > 10000 * 10**18, "Max tx amount should be over zero");
        maxTxAmount = amount;
        emit UpdatedMaxTxAmount(amount);
    }

    function setNumTokensToSwap(uint256 amount) external onlyRole(MANAGER_ROLE) {
        require(numTokensToSwap != amount, "This value was already set");
        numTokensToSwap = amount;
        emit UpdateNumtokensToSwap(amount);
    }

    function setSwapAndLiquifyEnabled(bool enabled) external onlyRole(MANAGER_ROLE) {
    ...
    function setFeeRate(uint256 value) external onlyRole(MANAGER_ROLE) {
        require(value <= 20, "DESToken: fee can not be over 20%");
        emit UpdatedFee(feeRate, value);
        feeRate = value;
    }
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            tTransferAmount = _takeFees(sender, amount);
        }
