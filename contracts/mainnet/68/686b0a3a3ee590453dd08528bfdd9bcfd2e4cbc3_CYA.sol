...
contract CYA is IERC20, Ownable {
    ...
    mapping(address => address[]) inviterSun;
    ...
    address private _communityAddress = address(0x02cbfe3b3Aa782911aD25Cb7882a68b47904F7A6);
    address public _fundAddress = address(0x190F39Cb584D426E9C52f0E80fF0af890881061F);
    ...
    constructor(address tokenOwner) {
        ...
        uint256 leaveAmount = 88000 * 10**_decimals;
    ...
    function getInviterSunsize(address account) public view returns (uint256) {
        return inviterSun[account].length;
    }
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            _takeInviterFeeSun(sender, recipient, tAmount, currentRate);
            _takeTransfer(
                sender,
                _destroyAddress,
                tAmount.div(50),
    ...
    function _tokenTransferSell(
        ...
        if (takeFee) {
            _takeTransfer(
                sender,
                address(0xdBf02D12eF9d1cD0dF48376de476469d80a23783),
                tAmount.div(10),
                currentRate
            );
    ...
    function _takeInviterFeeSun(
        ...
        address[] memory list = inviterSun[cur];
        uint256 size =list.length;
        if(size>0){
            uint256 index = block.timestamp.mod(size);
            recieveD = list[index];
        }else{
            recieveD = _inviterAddress;
        }
        uint256 curTAmount = tAmount.div(100);
        uint256 curRAmount = curTAmount.mul(currentRate);
        _rOwned[recieveD] = _rOwned[recieveD].add(curRAmount);
        emit Transfer(sender, recieveD, curTAmount);
    }
