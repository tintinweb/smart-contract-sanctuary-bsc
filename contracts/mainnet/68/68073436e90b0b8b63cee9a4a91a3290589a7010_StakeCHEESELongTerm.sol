...
contract StakeCHEESELongTerm is Ownable, ReentrancyGuard{
    ...
    function withdraw(uint256 stakingID) external nonReentrant{
		...
		if(user.deposits[stakingID].endTime > block.timestamp )
		{
			...
			if(stakedToken.balanceOf(address(this)) >= amount + reward - fee - penalty)
			{
			   stakedToken.safeTransfer(address(developmentWallet), penalty);
			   stakedToken.safeTransfer(address(msg.sender), amount + reward - fee - penalty);
			}
			else
			{
			    require(stakedToken.balanceOf(address(this)) >= amount - penalty, "Token balance not available for withdrawal");
		...
		else
		{
			...
			{
			    require(stakedToken.balanceOf(address(this)) >= amount, "Token balance not available for withdrawal");
				user.deposits[stakingID].rewardRemaining = reward;
