...
contract BallKingPool  is Initializable, OwnableUpgradeable, UUPSUpgradeable  {
    ...
    onlyOwner 
    {  
        stakeToken =  IERC20(0xb338ae79314dCcDe43854578C43c6a5425f6e7fF);
        ...
        returns(uint256)
    {
        ...
        uint monthsDiff = (block.timestamp - _stakeInfo.startDate) / 60 / 60 / 24 / 3;
        return monthsDiff * (_stakeType.rewardPerMonth/10);
    }
    ...
    function getCanWithdrawReword(address _address) 
        external
        view
        returns(uint256)
    {
        return calculateCanWithdrawReword(_address);
    }
