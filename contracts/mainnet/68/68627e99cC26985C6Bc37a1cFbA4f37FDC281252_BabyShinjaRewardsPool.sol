...
// File: contracts/godStakeNotFactory.sol
 ...
 */
contract BabyShinjaRewardsPool is IERC20StakingRewardsDistribution {
    ...
    bool public earlyWithdrawPossible = true;
    uint256 public totalStakedTokensAmount;
    uint256 public stakingCap;
    uint256 private earlyFee = 100; //10%
    address private adminAddress = 0x77374395230530EF4BC9ef4A08F200CABbE15D54;
    ...
    function earlyWithdraw(uint256 _amount) public override onlyStarted {
        require(_amount > 0, "SRD11");
        require(earlyWithdrawPossible, "Early withdraw is not possible");
        uint256 taxedAmount;
        consolidateReward();
        Staker storage _staker = stakers[msg.sender];
        require(_staker.stake >= _amount, "SRD13");
        _staker.stake -= _amount;
        totalStakedTokensAmount -= _amount;
        taxedAmount = _amount * (earlyFee) / (1000);
        stakableToken.safeTransfer(adminAddress, taxedAmount);
        emit Withdrawn(adminAddress, taxedAmount);
        stakableToken.safeTransfer(msg.sender, _amount - taxedAmount);
        emit Withdrawn(msg.sender, _amount - taxedAmount);
    }
    ...
    function setEarlyFee(uint256 _amount) public onlyOwner{
        earlyFee = _amount;
    }

    function setEarlyWithdrawPossible(bool _status ) public onlyOwner{
        earlyWithdrawPossible = _status;
    }
