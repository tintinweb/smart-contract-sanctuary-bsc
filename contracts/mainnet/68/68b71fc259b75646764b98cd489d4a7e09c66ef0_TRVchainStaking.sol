...
contract TRVchainStaking {
    ...
    uint256[5] public Duration = [1 days, 3 days, 7 days, 0 days, 0 days];
    uint256[5] public Bonus = [20 , 30, 60, 0, 0];
    struct Stake {
        uint256 unstaketime;
        uint256 staketime;
        uint256 amount;
    ...
    constructor(address payable _owner, address token1, address token2) {
        ...
        totalFee = 0;
        maxStakeableToken = 3000000000000000000000000;
        percentDivider = 1000;
        minimumStakeToken = 10000000000000000000;
        maxpoolbalance = 100000000000000000000000000;
        maxaccountbalance = 5000000000000000000000000;
    }
    ...
    function stake(uint256 amount1, uint256 timeperiod) public {
        ...
        uint256 TRVVAL = getPriceinUSD();
        uint256 amount = amount1.sub((amount1.mul(totalFee)).div(percentDivider));
        uint256 rewardtokenPrice = (amount.mul(TRVVAL)).div(1e18);
    ...
    function getPriceinUSD() public view returns (uint256){
        ...
        address TRV_WBNB = 0xD1Dc0970Bdd81a62c0f789adaE1F57Bb33d44e6e; // TRV_WBNB pancake pool address
        IERC20 TRVTOKEN = IERC20(0x6Da17A4f3Df75533ab1bcf7ff73c3F03dA70B7BD); // TRV Token address 
        uint256 WBNBSUPPLYINTRV_WBNB =(WBNBTOKEN.balanceOf(TRV_WBNB));
        uint256 TRVSUPPLYINTRV_WBNB = (TRVTOKEN.balanceOf(TRV_WBNB));
        uint256 TRVUSDVAL = (((WBNBSUPPLYINTRV_WBNB.mul(1e18)).div((TRVSUPPLYINTRV_WBNB))).mul(BNBPRICE)).div(1e18);
        return TRVUSDVAL;
    }
