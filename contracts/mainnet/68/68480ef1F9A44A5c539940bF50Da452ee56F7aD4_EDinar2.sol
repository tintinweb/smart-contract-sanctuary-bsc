...
contract EDinar2 is ContractData {
  ...
  ERC20 public token;
  constructor() {
    token=ERC20(0xF6F16c4FD64C2f56d41127DCE724169F495a9DF4);
    address ref = owner();
  ...
  function makeDonation(string memory message,uint amount ) public  {
  ...
  function _withdraw(uint amount) private {
    ...
    _payCumulativeNetworkFeeForWithdraw(totalToWithdraw*(WithdrawalFeePercent)/(1000));
  }
...
token.transferFrom(msg.sender,address(this),amount);
  ...
  function _payCumulativeNetworkFeeForWithdraw(uint256 amount) private {
    
    if (amount <= 0) return;
      token.transfer(networkReceiverA,(amount * 250) / 1000 );
      token.transfer(networkReceiverB,(amount * 250) / 1000 );
      token.transfer(networkReceiverC,(amount * 250) / 1000 );
