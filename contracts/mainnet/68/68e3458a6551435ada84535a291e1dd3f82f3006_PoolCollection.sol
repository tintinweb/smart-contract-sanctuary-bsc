...
contract PoolCollection  {
    ...
    function getManyPoolInfo(address[] calldata Pool) external view returns (uint256[] memory start, uint256[] memory end, uint256[] memory rwpb, uint256[] memory ts) {
      ...
      uint256[] memory _start = new uint256[](arrl);
      uint256[] memory _end = new uint256[](arrl);
      uint256 [] memory _rwpb = new uint256[](arrl);
      uint256[] memory _ts = new uint256[](arrl);

        for (uint i = 0; i < arrl; i++) {
            (_start[i], _end[i], _rwpb[i], _ts[i]) = poolInfo(Pool[i]);
        }
        return (_start, _end, _rwpb, _ts);
    }
    ...
    function getManyUserInfo(address[] memory Pool, address User) public view returns (uint256[] memory amount, uint256[] memory pendingReward) {
      ...
      uint256 arrl = Pool.length;
      uint256[] memory _amount = new uint256[](arrl);
      uint256[] memory _pendingReward = new uint256[](arrl);

        for (uint i = 0; i < arrl; i++) {
            (_amount[i], _pendingReward[i]) = userInfo(Pool[i], User);
        }
        return (_amount, _pendingReward);
    }
}