...
contract CRYPTOSPORTTOKEN is ERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;
    address public  uniswapV2Pair;
    bool private swapping;
    CRYPTOSPORTTOKENDividendTracker public dividendTracker;
    address public constant deadWallet = 0x000000000000000000000000000000000000dEaD;
    address public constant BUSD = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
    uint256 public swapTokensAtAmount = 400000000 * (10**18);
    ...
    uint256 public totalFees = BUSDRewardsFee.add(liquidityFee).add(marketingFee).add(buybackFee).add(DevelopementFee);

    address public _marketingWalletAddress = 0xCbBc8F09eed4c5fA18f974F11E643B74Fc45AA29;
    address public _DevelopementWalletAddress = 0xDa93885022D18D3469F0dEa28ebbEc61544eB316;
    uint256 public gasForProcessing = 300000;
    
    // buyback parameters
    uint public buybackamount = 0.001 ether;
    uint public totalbuyback = 0;
    uint public accumulatedBuyback = 0;
    uint public buyBackthresholdLimit = 0;
    ...
    uint public _maxTxAmount = 1000000000000* (10**18);
    ...
    constructor() public ERC20("CRYPTOSPORTTOKEN", "$CSPT") {

    	dividendTracker = new CRYPTOSPORTTOKENDividendTracker();
        ...
        excludeFromFees(_DevelopementWalletAddress, true);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        CRYPTOSPORTTOKENDividendTracker newDividendTracker = CRYPTOSPORTTOKENDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "The new dividend tracker must be owned by the CRYPTOSPORTTOKEN token contract");
    ...
    function setDevelopementWallet(address payable wallet) external onlyOwner{
        _DevelopementWalletAddress = wallet;
    }

    function setBUSDRewardsFee(uint256 value) external onlyOwner{
        BUSDRewardsFee = value;
        totalFees = BUSDRewardsFee.add(liquidityFee).add(marketingFee).add(buybackFee).add(DevelopementFee);
    }
    ...
    function setDevelopementFee(uint256 value) external onlyOwner{
        DevelopementFee = value;
        totalFees = BUSDRewardsFee.add(liquidityFee).add(marketingFee).add(buybackFee).add(DevelopementFee);
    }
    ...
    function setBuyBackStatus(bool _enabled) external onlyOwner {
        buyBackEnabled = _enabled;
    }

    function setdividendLimitStatus(bool _enabled) external onlyOwner {
        dividendLimit = _enabled;
    }

     function setSwappingStatus(bool _enabled) external onlyOwner {
        swappingenabled = _enabled;
    }
    
    function changemaxdistributedividendLimit(uint256 _newvalue)external onlyOwner {
        maxdistributeDividends = _newvalue; 
    }

    function changebuyBackthresholdLimit(uint256 _newvalue)external onlyOwner {
        buyBackthresholdLimit = _newvalue; 
    }

    function setBuybackAmount(uint256 _amount) external onlyOwner {
    ...
    function swapOnDemand(bool sendToMarketingFee, bool sendToDevelopementFee, bool liquify, bool sendDividends, bool buybacktax) external onlyOwner {
        swapping = true;

        uint256 contractTokenBalance = balanceOf(address(this));

        if (sendToMarketingFee) {
            uint256 marketingTokens = contractTokenBalance.mul(marketingFee).div(totalFees);
            swapAndSendToFee(marketingTokens, _marketingWalletAddress);
        }

        if (sendToDevelopementFee) {
            uint256 DevelopementTokens = contractTokenBalance.mul(DevelopementFee).div(totalFees);
            swapAndSendToFee(DevelopementTokens, _DevelopementWalletAddress);
        }
    ...
    function buybackOnDemand(uint256 _amount) external onlyOwner {
        require (_amount < address(this).balance, "Amount exceed contract balance");
        totalbuyback = totalbuyback.add(_amount);
        swapETHForTokens(deadWallet, _amount);
    }
    ...
    function _transfer(
        ...
        if(takeFee) {
        	...
        	if(automatedMarketMakerPairs[to] && from != address(this) && from != owner()){
        	    fees += amount.mul(selltax).div(100);


            if (buyBackEnabled && address(this).balance > buybackamount && amount > buyBackthresholdLimit ) {
                totalbuyback = totalbuyback.add(buybackamount);
                swapETHForTokens(deadWallet, buybackamount);         
        }
...
contract CRYPTOSPORTTOKENDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("CRYPTOSPORTTOKEN_Dividend_Tracker", "CRYPTOSPORTTOKEN_Dividend_Tracker") {
    	claimWait = 3600;
