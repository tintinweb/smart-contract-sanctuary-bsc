...
// File: contracts/farms/RABBITSwap/IMasterChef.sol
...
// File: contracts/farms/RABBITSwap/RABBITVault.sol


pragma solidity 0.6.12;

contract RABBITVault is Ownable, Pausable {
    ...
    struct UserInfo {
        ...
        uint256 RABBITAtLastUserAction; // keeps track of RABBIT deposited at the last user action
        uint256 lastUserActionTime; // keeps track of the last user action time
    }

    IBEP20 public immutable token; // RABBIT token
    ...
    /**
     * @notice Constructor
     * @param _token: RABBIT token contract
    ...
    /**
     * @notice Deposits funds into the RABBIT Vault
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in RABBIT)
     */
    function deposit(uint256 _amount) external whenNotPaused notContract {
        ...
        user.RABBITAtLastUserAction = user.shares.mul(balanceOf()).div(totalShares);
    ...
    /**
     * @notice Reinvests RABBIT tokens into MasterChef
     * @dev Only possible when contract not paused.
     */
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the RABBITVault
     */
     ...
     * @return Expected reward to collect in RABBIT
     */
    function calculateHarvestRABBITRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingRABBIT(poolId, address(this));
     ...
     * @return Returns total pending RABBIT rewards
     */
    function calculateTotalPendingRpendingRABBITRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingRABBIT(poolId, address(this));
        amount = amount.add(available());
    ...
    /**
     * @notice Withdraws from funds from the RABBITVault
