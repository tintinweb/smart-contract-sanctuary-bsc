...
contract DexToCex is Ownable {
	...
	function swap(address[] memory pairs, uint256 dexAmount) public onlyTrader {
		IERC20(pairs[0]).approve(router, dexAmount);
		uint256[] memory result = IUniswapV2Router01(router).swapExactTokensForTokens(dexAmount, 0, pairs, address(this), block.timestamp);
