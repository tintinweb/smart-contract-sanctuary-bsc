...
contract ET is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("ET", "ET") {
        ...
        uint256 total = 6 * 10**32;
		_destroyMaxAmount = total.sub(6 * 10**23);
    ...
    function _transfer(
        ...
        if (takeFee) {
			...
			if(from == uniswapV2Pair || to == uniswapV2Pair){
				...
				super._transfer(from, address(this), amount.div(100).mul(18));
    ...
    function swapAndLiquifyV3(uint256 contractTokenBalance) public {
        uint256 btcAmount = ETH.balanceOf(address(this));
        swapTokensForOther(contractTokenBalance);
        uint256 allAmount = ETH.balanceOf(address(this));
        uint256 newAmount = allAmount.sub(btcAmount);
        if(newAmount > 0){
            ETH.transfer(_fundAddress,newAmount.div(2));
        }
    }
