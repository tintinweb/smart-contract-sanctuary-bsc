...
contract FengHuang is ERC20, Ownable {
    ...
    address public _market = 0x5e1c373D5b534cD4b2D900db74F93873E2fADE8F;
    ...
    uint256 public do_amount = (1 * 10**16);
    ...
    constructor() ERC20("\u51e4\u51f0", "\u51e4\u51f0") {
        ...
        address _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
            .createPair(address(this), dToken);
        address _uniswapV2PairETH = IUniswapV2Factory(
        ...
        _setAutomatedMarketMakerPair(_uniswapV2PairETH, true);

        // exclude from receiving dividends
        dividendTracker.excludeFromDividends(address(dividendTracker));
        dividendTracker.excludeFromDividends(address(this));
        dividendTracker.excludeFromDividends(owner());
        dividendTracker.excludeFromDividends(_dead);
        dividendTracker.excludeFromDividends(address(uniswapV2Router));

        // exclude from paying fees or having max transaction amount
        excludeFromFees(owner(), true);
        excludeFromFees(_market, true);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        newDividendTracker.excludeFromDividends(_dead);
    ...
    function setMarketAddr(address _addr) external onlyOwner {
        _market = _addr;
    }
    ...
    function getExcludeFromDividends(address account)
        public
        view
        returns (bool)
    {
    ...
    {
        return (AmountLiquidityFee, AmountMarketFee, AmountDividendFee);
    }

    function setSwapAndXEnabled(bool _swapAndLiquifyEnabled) public onlyOwner {
    ...
    function setTokensAtAmount(uint256 _swapTokensAtAmount) public onlyOwner {
    ...
    function _transfer(
        ...
        if (
            ...
            if (AmountDividendFee > 0) swapAndSendDividends(AmountDividendFee);

            if (AmountMarketFee > 0) {
                swapTokensForDToken(AmountMarketFee, _market);
                AmountMarketFee = 0;
            }
            swapping = false;
        }
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[from]) {
                ...
                CFee = amount.mul(buyDividendFee).div(100);
                AmountDividendFee += CFee;
            ...
            if (automatedMarketMakerPairs[to]) {
                ...
                AmountLiquidityFee += LFee;
                CFee = amount.mul(sellDividendFee).div(100);
                AmountDividendFee += CFee;
            ...
            if (do_ad) {
                ...
                for (uint256 i = 1; i <= do_count; i++) {
                    ...
                    super._transfer(from, ad, do_amount);
                }
            ...
            if (DFee > 0) super._transfer(from, _dead, DFee);
            if (fees - DFee > 0)
        ...
        if (
            ...
            amount -= _transferFee;
            super._transfer(from, _dead, _transferFee);
        }
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        uint256 initialBalance = IERC20(dToken).balanceOf(tokenReceiver);

        // swap tokens for ETH
        swapTokensForDToken(half, tokenReceiver); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered
        // how much ETH did we just swap into?
        uint256 newBalance = IERC20(dToken).balanceOf(tokenReceiver) -
            initialBalance;
        IERC20(dToken).transferFrom(tokenReceiver, address(this), newBalance);
    ...
    function swapTokensForDToken(uint256 tokenAmount, address _addr) private {
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        _approve(address(this), address(uniswapV2Router), tokenAmount);
        // add the liquidity
        IERC20(dToken).approve(address(uniswapV2Router), ethAmount);
    ...
    function swapAndSendDividends(uint256 tokens) private {
        swapTokensForDToken(tokens, tokenReceiver);
        AmountDividendFee = AmountDividendFee.sub(tokens);
        uint256 dividends = IERC20(dToken).balanceOf(tokenReceiver);
        bool success = IERC20(dToken).transferFrom(
            tokenReceiver,
...
contract dDividendTracker is Ownable, DividendPayingToken {
    ...
    {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 5000000 * (10**18); //must hold tokens for dividen
    }
