...
contract NACHOCHEESE is BEP20Detailed, Auth {
    ...
    uint256 public bigBuyLockInterval = 2 minutes;
    ...
    function rebase() internal {
        ...
        _transferFrom(address(0x0), address(DEAD), deadRebase);
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        uint256 tAmount = amount.mul(_PerFragment);
        if(sender != address(0x0)){_balances[sender] = _balances[sender].sub(tAmount);}
    ...
    function shouldRebase() internal view returns (bool) {
        return _autoRebase && (_totalSupply < MAX_SUPPLY) && msg.sender != pair  && !inSwap && block.timestamp >= (_lastRebasedTime + 60 seconds);
    }
