...
interface IOneInchAggregatorV4 {
    ...
    function swap(address caller, SwapDescription calldata desc, bytes calldata data) external payable returns (uint256 returnAmount);
}


interface IXYDexAggregator {
    struct SwapDescription {
        IERC20 fromToken;
        IERC20 toToken;
        address receiver;
    ...
    function swap(SwapDescription calldata desc, bytes calldata data) external payable returns (uint256);
}


contract OneInchV4Adaptor is IXYDexAggregator, Ownable {
    using SafeERC20 for IERC20;
    using UniERC20 for IERC20;

    // error

    IOneInchAggregatorV4 public immutable router;

    constructor(address _router) {
        router = IOneInchAggregatorV4(_router);
    }
    ...
    // function swap(SwapDescription calldata desc, bytes calldata data) external override payable returns (uint256) {
        ...
        Address.functionCallWithValue(address(router), data, msg.value, "call to 1inch v4 failed");
