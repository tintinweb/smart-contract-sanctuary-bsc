...
contract EthTest is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    EthtestDividendTracker public dividendTracker;
    ...
    constructor() ERC20("EthTest", "EthTest") {
        marketingWallet = payable(0x4A7Ccd75a4cE2F7BD39547e33a7E6584f5542557);
        devWallet = payable(0x4A7Ccd75a4cE2F7BD39547e33a7E6584f5542557);
        ...
        dividendTracker = new EthtestDividendTracker(
            payable(this),
            router,
...
contract EthtestDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    EthTest public EthTestContract;
    ...
    constructor(
        ...
        defaultToken = token;
        EthTestContract = EthTest(mainContract);
    ...
    function _transfer(
        ...
        require(false, "EthTest_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "EthTest_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EthTest contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(EthTestContract);

            uint256 prevBalance = EthTestContract.balanceOf(address(this));
            ...
            {
                uint256 received = EthTestContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    EthTestContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
