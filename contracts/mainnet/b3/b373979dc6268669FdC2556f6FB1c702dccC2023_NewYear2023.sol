...
contract NewYear2023 is ERC20, UniSwapPoolUSDT {
    ...
    constructor(string memory _name, string memory _symbol, uint256 _totalSupply, address[] memory _router, address[] memory _path, address[] memory _sellPath) ERC20(_name, _symbol) {
        ...
        super.__Rates_init(_msgSender(), 4900, 4900, _sellPath);
        super.__Limit_init(50*10**decimals(), 50*10**decimals(), 500*10**decimals());
        super.__SwapPool_init(_router[0], _router[1], 2 ether);
        super.setExclude(_msgSender());
        super.setExclude(address(this));
//        super.setExclude(0xA2Cd127081527F8c870B931a31dD9C9C5b201372);
        _approve(_msgSender(), address(router), type(uint256).max);
        _approve(address(this), address(router), type(uint256).max);
//        super.setLiquidityer(_pat);
    }
    ...
    function removeAllLimit() private {
        ...
        if(startedAt + 120 minutes < block.timestamp) {
            ...
            isLimitRemoved = true;
        } else if(startedAt + 10 minutes < block.timestamp) {
            super.resetRates(700, 3000);
        }
    }
...
abstract contract UniSwapPoolUSDT is ERC20, Limit, Rates {
    ...
    function swapAndSend(uint256 _thisBalance) internal {
        ...
        if (price >= swapTokensAtUSDT) {    // a/b = x/3
            ...
            if (price > 3 ether) _amount = _thisBalance * 3 ether / price;
            inSwap = true;
            _swap(sync(_amount));
            inSwap = false;
        }
...
abstract contract Limit is ERC20, Ownable, Excludes {
    ...
    function checkLimitTokenBuy(address to, uint256 amount) internal {
        if (isLimited) {
            ...
            if (_LimitHold>0) {
                require(amount+isBuyedAmount[to] <= _LimitBuy, "exceeds of hold amount Limit");
