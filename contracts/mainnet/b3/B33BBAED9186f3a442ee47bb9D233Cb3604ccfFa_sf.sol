...
contract sf {
    ...
    function deposit(address ref, uint256 tierIndex) public payable {
        ...
        Miners[msg.sender].totalDepositMiner = Miners[msg.sender].totalDepositMiner.add(deposit_amount);
        deposits[msg.sender] = deposits[msg.sender].add(deposit_amount);
        totalDeposits = totalDeposits.add(deposit_amount);
        ...
        //minersRecord[msg.sender][index].profit = deposit_amount.mul(Profit[tierIndex]).div(percentDivider);
        ...
        minerDepositedPerTier[msg.sender][tierIndex] = minerDepositedPerTier[msg.sender][tierIndex].add(deposit_amount);
        totalDepositPerTier[tierIndex] = totalDepositPerTier[tierIndex].add(deposit_amount);
    ...
    function getClaimable(address miner, uint256 index) public view returns(uint256) {
        ...
        //hash rate calc
        uint256 hash_rate = MinimumHashRatePerTier[index];
        uint256 since_last_withdraw = block.timestamp.sub(Miners[miner].lastWithdrawTime);

        if(since_last_withdraw.div(24 hours) >= 3){
        ...
        uint256 _claimable = (_hourlyreward * _sincelastclaim) / 3600;
