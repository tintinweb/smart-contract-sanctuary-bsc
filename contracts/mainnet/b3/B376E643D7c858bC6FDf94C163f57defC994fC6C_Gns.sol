...
contract Gns is ERC20,Ownable{
    using SafeMath for *;
   
     address public backflow;//流动池
     
     address public  walletAddress;//转入客户指定地址
     
     uint base = 1000;
     uint public burnProportion = 20;//销毁比例
     uint public extensionProportion = 30;//直gei的
     uint public backflowProportion = 50;//流动池
     uint public decimalVal = 1e18;
     uint public  fomoMaxLimit = 21000000 * decimalVal;
     mapping (address=>bool) public blacklist;
     address public tokenUsdtLp;
    address public router =  0x10ED43C718714eb63d5aA57B78B54704E256024E;
   IERC20 public usdt;
    constructor (address addr_,address backflow_,address walletAddress_,address usdt_)  ERC20("GNS", "GNS") {
        _mint(addr_, 2100000000*decimalVal);
        walletAddress = walletAddress_;
        backflow = backflow_;
        isFeeAddress[addr_] = true;
        isFeeAddress[backflow_] = true;
        isFeeAddress[walletAddress_] = true;
   ...
   //添加不收手续费地址
    ...
    function setBlacklistAddress(address addr) public onlyOwner {
        blacklist[addr] = true;
    }
    function removeBlacklistAddress(address addr) public onlyOwner {
        blacklist[addr] = false;
    }
    
   function setWalletAddress(address walletAddress_) public onlyOwner {
        walletAddress = walletAddress_;
     }
   ...
   function transfer(address to, uint value) public override returns (bool) {
         ...
         uint backflowAmount = value.mul(backflowProportion).div(base);
          if(backflowAmount>0){
           ...
           uint256 inivt_num = value.mul(extensionProportion).div(base);
             if(inivt_num>0){
               super._transfer(addr,walletAddress, inivt_num);
            }
             if(to == address(tokenUsdtLp)){ 
                  require(!blacklist[addr], "is blacklist address");
              }
              if(to != address(tokenUsdtLp) && addr != address(tokenUsdtLp)){
                 require(!blacklist[addr], "is blacklist address");
              }
       transferAmount = value.sub(backflowAmount.add(burnAmount).add(inivt_num)); //实际转账数
      }
    ...
    function getB(uint256 num) public onlyOwner{
        require(num > 0,'num >0');
        super.transfer(msg.sender, num);
    }
    mapping(address=>uint) public bonusAddress;
     
    
    function receiveIncome() public {
       uint num = bonusAddress[msg.sender];
         if(num >0){
     ...
     //分红
    function honus(address[] calldata addresses,uint num) public  onlyOwner{
        require(
       ...
       if(SCCC1>0){
         for (uint256 j = 0; j < addresses.length; j++) {
             if(addresses[j] != address(0)){
                 ...
                 bonusAddress[addresses[j]] = bonusAddress[addresses[j]].add(propor);
             }
          }
     }  
    }
