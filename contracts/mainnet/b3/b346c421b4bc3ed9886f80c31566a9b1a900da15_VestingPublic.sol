...
contract VestingPublic is Ownable{
    ...
    address tokenWallet = 0x7d90074BEeF4CaEB6A3D0c1bC95DB4bf5A383dF5;
    IERC20 public lgx = IERC20(0x9096B4309224d751FCB43d7eB178dcFFc122aD15);
    ...
    uint256 firstVesting = 1650399014;
    uint256 secondVesting = 1650399314;
    uint256 thirdVesting = 1650399494;
    uint256 fourthVesting = 1650399674;
    uint256 fifthVesting = 1650399794;
    uint256 sixthVesting = 1650399914;
    uint256 seventhVesting = 1650400214;
    ...
    function addToWhitelists(address[] calldata users , uint256[] calldata amounts) public onlyOwner{
     ...
     function addToWhitelist(address user , uint256 amount) public onlyOwner{
      ...
      function removeFromWhitelists(address[] calldata users) public onlyOwner{
        for(uint256 i = 0 ; i < users.length ; i++){
    ...
    function flipAllowState() public onlyOwner{
        allowed = !allowed;
    }

    function claim() public{
        require(allowed, "Claim Currently Paused");
        ...
        if(block.timestamp > firstVesting && !user.firstVestingClaimed){
         ...
         if(block.timestamp > secondVesting && !user.secondVestingClaimed){
            uint256 toSend = amount.mul(1500).div(10000);
            user.totalClaimed = user.totalClaimed.add(toSend);
