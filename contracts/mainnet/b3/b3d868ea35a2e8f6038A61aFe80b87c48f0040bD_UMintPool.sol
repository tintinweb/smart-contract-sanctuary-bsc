...
abstract contract AbsMintPool is Ownable {
    ...
    uint256 public _usdtRate = 5000;
    uint256 public _burnRate = 3000;
    ...
    //maxOtherTokenAmount
    function buy(uint256 amount, uint256 maxBurnAmount, address otherTokenAddress, uint256 maxOtherTokenAmount) external {
        ...
        _takeAmount(account, amount, maxBurnAmount, otherTokenAddress, maxOtherTokenAmount);
    ...
    function _takeAmount(address account, uint256 amount, uint256 maxBurnAmount, address otherTokenAddress, uint256 maxOtherTokenAmount) private {
        require(_isOtherToken[otherTokenAddress], "notOtherToken");
        uint256 feeDivFactor = _feeDivFactor;
        uint256 usdtAmount = amount * _usdtRate / feeDivFactor;
        ...
        uint256 burnUsdt = amount * _burnRate / feeDivFactor;
    ...
    unchecked{
        ...
        _giveToken(_ethAddress, _cashAddress, pendingETH);
    }
    ...
    function getAmountOuts(uint256 usdtAmount) public view returns (
        uint256 usdtOutAmount, uint256 burnTokenAmount, uint256[] memory otherTokenAmounts
    ){
        usdtOutAmount = usdtAmount * _usdtRate / _feeDivFactor;
        uint256 burnUsdt = usdtAmount * _burnRate / _feeDivFactor;
        burnTokenAmount = tokenAmountOut(burnUsdt, _burnTokenAddress);

        uint256 otherUsdt = usdtAmount - usdtOutAmount - burnUsdt;
        uint256 len = _otherToken.length;
        otherTokenAmounts = new uint256[](len);
        for (uint256 i; i < len; ++i) {
            otherTokenAmounts[i] = tokenAmountOut(otherUsdt, _otherToken[i]);
        }
    }
    ...
    function getUserExtInfo(address account) external view returns (
        ...
        for (uint256 i; i < len; ++i) {
            otherTokenBalances[i] = IERC20(_otherToken[i]).balanceOf(account);
        }
    }
}
...
contract UMintPool is AbsMintPool {
    constructor() AbsMintPool(
        ...
        address(0xCa03Fa8D0dAb4a19Ac407455CE00e394eA31b532)
    ){
