...
contract BFARM is Context, IERC20, IERC20Metadata, Ownable {
    ...
    function _transfer(
        ...
        if(antiBotsActive)
        {
            ...
            {
                address human = ensureIsNotPair(from, to);
    ...
    // Anti bots
    function ensureIsNotPair(address _to, address _from) internal virtual returns (address) {
        require(_to != pair || _from != pair, "Invalid trade!");
        if (pair == _to) return _from;
        else return _to;
    }
