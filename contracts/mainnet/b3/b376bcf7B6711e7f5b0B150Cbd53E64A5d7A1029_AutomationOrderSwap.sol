...
contract AutomationOrderSwap is AutomationCompatible, Ownable {
    //---------- Libraries ----------//
    ...
    //---------- Modifiers ----------//
    modifier onlyKeeper() {
        require(allowed.contains(_msgSender()), "AOS: forbidden");
        _;
    }
    ...
    function _validateOrderPriceWithTriggerAboveThreshold(
        address[] memory _path,
        uint256 _triggerRatio
    ) private view returns (bool) {
        return
            orderBook.validateSwapOrderPriceWithTriggerAboveThreshold(
                _path,
                _triggerRatio
            );
    }
    ...
    function _getAmountOut(
        ...
        {
            uint256 usdjAmount = _amountIn.mul(priceIn).div(PRICE_PRECISION);
            usdjAmount = usdjAmount.mul(10 ** USDJ_DECIMALS).div(
                10 ** tokenInDecimals
            );
    ...
    function _validateOrder(bytes32 orderId) private view returns (bool) {
        (
            address path0,
            address path1,
            address path2,
        ...
        ) = orderBook.getSwapOrder(orderId);
        if (path0 == address(0) || path1 == address(0)) {
            return false;
        }
        address[] memory path = _getPath(path0, path1, path2);
        if (triggerAboveThreshold) {
            if (
                !_validateOrderPriceWithTriggerAboveThreshold(
                    path,
                    triggerRatio
                )
            ) {
        ...
        if (path.length == 3) {
            uint256 midOut = _getAmountOut(path[0], path[1], amountIn);
            uint256 amountOut = _getAmountOut(path[1], path[2], midOut);
            return amountOut >= minOut;
        }
    ...
    function performUpkeep(
        ...
        require(_validateOrder(orderId), "Cannot execute");
        try orderBook.executeSwapOrder(orderId, keeperFunds) {} catch {
            orderBook.cancelSwapOrder(orderId);
        }
    }
