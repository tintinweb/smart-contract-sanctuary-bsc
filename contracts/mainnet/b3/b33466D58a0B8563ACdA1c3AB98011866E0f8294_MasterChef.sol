...
contract MasterChef is Initializable, Ownable, ReentrancyGuard
{
	...
	function depositOnBehalfOf(uint256 _pid, uint256 _amount, address _account, address _referral) external returns (uint256 _reward)
	{
		...
		return depositOnBehalfOf(_pid, _amount, _account);
	}
	...
	function claimReferral() public nonReentrant returns (uint256 _reward)
	{
		require(epochIndex > 0, "unavailable");
		uint256 _lastEpoch = epochIndex - 1;
		uint256 _volume = userReferralVolume[msg.sender][_lastEpoch];
		userReferralVolume[msg.sender][_lastEpoch] = 0;
		...
		_reward = _volume * _totalReward / _totalVolume;
		referralInfo[_index].volume = _totalVolume - _volume;
		referralInfo[_index].reward = _totalReward - _reward;
		if (_reward > 0) {
			allocReward -= _reward;
			IERC20(rewardToken).safeTransferFrom(faucet, msg.sender, _reward);
		}
		emit ClaimReferral(msg.sender, _reward);
		return _reward;
	}

	function compoundAll(uint256 _pid0) external returns (uint256 _reward)
	{
		PoolInfo storage _poolInfo = poolInfo[_pid0];
		require(_poolInfo.token == rewardToken, "invalid pid");
		_reward = claimReferral();
		for (uint256 _pid = 0; _pid < poolInfo.length; _pid++) {
			_reward += withdrawOnBehalfOf(_pid, 0, msg.sender);
		}
		depositOnBehalfOf(_pid0, _reward, msg.sender);
		emit CompoundAll(msg.sender, _pid0, _reward);
		return _reward;
	}
	...
	event CompoundAll(address indexed _account, uint256 indexed _pid, uint256 _amount);
}
