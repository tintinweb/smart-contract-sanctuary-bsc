...
contract The_Narrator is IBEP20, Ownable {
    ...
    string private constant _name               = 'The Narrator';
    string private constant _symbol             = 'TylerD';    
    ...
    uint16 private constant SellLimitDivider    = 50000;
    uint8 public constant MaxSellTax            = 10; // Max buytax = 10%

    //tax distribution
    //total normal tax = 12%
    uint8 private _devTax                       = 0;
    uint8 private _burnTax                      = 33; // 1.98% - 1.36%
    uint8 private _marketingTax                 = 33; // 1.98% - 1.36%
    uint8 private _liquidityTax                 = 17; // 1.02% - 2.64%
    uint8 private _stakingTax                   = 17; // 1.02% - 2.64%
    uint8 private _buyBackTax                   = 0; 

    //buy, sell and transfer tax
    uint8 public _buyTax                        = 6;
    uint8 public _sellTax                       = 8;
    ...
    address public MainReward = 0x66B881AD5738319b33E608943Ca0544E842D02e7;  
...
//OnlyOwner has control of LP functions
    ...
    //TotalTaxes is 100%. The max buyTax is 10% and the selltax can't be higher than 10% to prevent a honeypot; transferTax is also max. 10%
    ...
    function UpdateTaxes(uint8 burnTaxes, uint8 buybackTaxes, uint8 devTaxes, uint8 marketingTaxes, uint8 liquidityTaxes, uint8 stakingTaxes,uint8 buyTax, uint8 sellTax, uint8 transferTax) public authorized{
        ...
        require(buyTax<=10,"Buytax is higher than the 10%.");
        require(sellTax<=MaxSellTax,"Selltax is higher than 10%.");
        require(transferTax<=10,"TransferTax is higher than 10%.");
