...
abstract contract AbsUtil is Ownable, Context {
    ...
    constructor(address _wallet,IERC20 _token,address _nft,IERC20 _usdt) {
        wallet = _wallet;
        token = _token;
        usdt = _usdt;
        nft = NFT(_nft);
    ...
    function _tokenTransfer(IERC20 token,address user, uint256 amount)  private {
        token.safeTransferFrom(user,wallet,amount);
    }

    function tokenApprove(address spender, uint256 value) external {
        usdt.safeApprove(spender,value);
    }
    ...
    function setUsdt(IERC20 _usdt) external onlyOwner returns (bool){
        usdt = _usdt;
        return true;
    }

    function setToken(IERC20 _token) external onlyOwner returns (bool){
        token = _token;
        return true;
    }
...
contract Util is AbsUtil {
    ...
    //token
        IERC20(0x8de03381b78Fab5d3e27c7F7618d4BB9AbC3f163),
    //nft
        address(0xF96450b3C042b4ABE4CBFa8AC8112BbF6e9209EB),
    //usdt
        IERC20(0xbfA3855535Db097319b622eCb88083492FA16A5b)
    ){}
}