...
contract TaxCollector is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    address public holderRewardsAddress;
    /* ========== EVENTS ========== */
    ...
    function setHolderRewardsAddress(address _holderRewardsAddress) external onlyOwner {
        holderRewardsAddress = _holderRewardsAddress;
    }
    ...
    function _sendAmount() private {
        require(holderRewardsAddress != address(0), "holderRewardsAddress is Zero");

        uint256 holderRewardsAMT = collectedSellTax * 3/4;
        token.safeTransfer(holderRewardsAddress, holderRewardsAMT);

        token.safeTransfer(sellTaxAddress, collectedSellTax-holderRewardsAMT);
        token.safeTransfer(buyTaxAddress, collectedBuyTax);
        collectedBuyTax = 0;
        collectedSellTax = 0;
    }

    function distributeAmountsToPlaces() public onlyWhitelisted {
        _sendAmount();
    }
