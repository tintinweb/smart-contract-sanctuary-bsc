...
contract BasketTest is Ownable {
  ...
  function withdrawToBridge1(address account, address beefyzap, address vault, Swaper memory swper, uint256 amount, uint256 fee) public onlyManager {
    _approveTokenIfNeeded(vault, beefyzap);
    IBeefyUniV2Zap(beefyzap).beefOut(vault, amount);
  }

  function withdrawToBridge2(address account, address beefyzap, address vault, Swaper memory swper, uint256 amount, uint256 fee) public onlyManager {
  ...
  function withdrawToBridge3(address account, address beefyzap, address vault, Swaper memory swper, uint256 amount, uint256 fee) public onlyManager {
