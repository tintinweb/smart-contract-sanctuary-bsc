...
contract BeanCat is Context, IBEP20, Ownable 
{
    using SafeMath for uint256;
    using Address for address;

    address public marketingAddress = 0x7200a59dcAb71b7A1842B17CC8769D9eB67C5B7E; // Marketing wallet Address.
    address public buybackAddress = 0x7200a59dcAb71b7A1842B17CC8769D9eB67C5B7E; // buyback wallet address. 
    ...
    string private _name = "BeanCat";
    string private _symbol = "BEANCAT";
    ...
    uint256 _saleBuybackFee = 0;

    uint256 public _maxBuyTxAmount = _tTotal.div(100).mul(5); //5%  - max tx limit other than sell
    uint256 public _maxSaleTxAmount = _tTotal.div(100).mul(5); //5%  - max tx limit on sale only 

    uint256 private minimumTokensBeforeSwap = 1000_000_000 * 10**18;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        uint256 buybackTokens = amount.mul(_buybackFee).div(100);
        amount = amount.sub(marketingTokens).sub(buybackTokens);
        ...
        if(buybackTokens>0) { _transferStandard(sender, buybackAddress, buybackTokens); }
        restoreAllFee();
    }
    ...
    {
        _amount = _amount.mul(100).div(100-_marketingFee-_buybackFee);
    ...
    function setAllSaleFeePercent(uint256 liquidityFee, uint256 marketingFee, uint256 buybackFee) external onlyOwner() 
    {
        ...
        require(_saleLiquidityFee.add(_saleMarketingFee).add(_saleBuybackFee)<=40, "Too High Fee");
    }
