...
contract FurMax is BaseContract
{
     ...
     */
    function availableDividends(address participant_) public view whenNotPaused returns (uint256)
    {
        uint256 _available_ = _availableFurbotDividends(participant_) + _availableFurpoolDividends(participant_);
        uint256 _balance_ = _usdc.balanceOf(address(this));
        if(_available_ > _balance_) {
            _available_ = _balance_;
        }
        return _available_;
    }
    ...
    {
        uint256 _available_;
        if(totalFurbotInvestment > 0 && totalFurbotDividends > 0)
        {
            _available_ = ((totalFurbotDividends * furbotInvestment[participant_]) / totalFurbotInvestment) - furbotDividendsClaimed[participant_];
        }
        return _available_;
    }
    ...
    {
        uint256 _available_;
        if(totalFurpoolInvestment > 0 && totalFurpoolDividends > 0)
        {
            _available_ = ((totalFurpoolDividends * furpoolInvestment[participant_]) / totalFurpoolInvestment) - furpoolDividendsClaimed[participant_];
        }
        return _available_;
    }
    ...
    {
        uint256 _amount_ = availableDividends(msg.sender);
     ...
     */
    function join(bool acceptTerms_, uint256 furbet_, uint256 furbot_, uint256 furpool_) external whenNotPaused
    {
     ...
     * @param furpool_ Percent of earnings to be sent to Furpool.
     */
    function updateDistribution(uint256 furbet_, uint256 furbot_, uint256 furpool_) external whenNotPaused
    {
    ...
    {
        uint256 _amount_ = totalFurpoolPendingInvestment;
        uint256 _balance_ = _usdc.balanceOf(address(this));
        if(_amount_ > _balance_) _amount_ = _balance_;
        if(_amount_ == 0) return;
        _usdc.approve(address(_furpool), _amount_);
        _furpool.stake(address(_usdc), _amount_, 0);
        emit FurpoolStaked(_amount_);
        totalFurpoolInvestment += _amount_;
     ...
     */
    function setAdmin(address participant_, bool isAdmin_) external onlyOwner
    {
     ...
     */
    function withdrawFurbotPendingInvestment() external onlyOwner
    {
     ...
     */
    function depositFurbotDividends(uint256 amount_) external onlyOwner
    {
        require(amount_ > 0, "Furmax: Amount must be greater than 0.");
