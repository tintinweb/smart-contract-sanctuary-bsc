...
library PrivateStorage {
    ...
    struct PoolSettings {
        ...
        uint256 softCap;
        uint256 hardCap;
        uint256 privateLockDays;
        uint256 privatePercent;
...
interface IPrivatePool {
  ...
  function initialize(
    PrivateStorage.PoolSettings memory _poolSettings,
    address _router,
    address _owner,
    address _whiteList,
  ...
  function initializeContributorVesting(
    PrivateStorage.ContributorVestingSettings memory _contributorVestingSettings
  ) external;

  function initializeOwnerVesting(
    PrivateStorage.OwnerVestingSettings memory _ownerVestingSettings
  ) external;
...
contract PrivatePool is IPrivatePool, ReentrancyGuard, Ownable {
  using EnumerableSet for EnumerableSet.AddressSet;
  ...
  IPrivateFactory public factory; 
  ...
  PrivateStorage.ContributorVestingSettings public contributorVestingSettings;
  PrivateStorage.OwnerVestingSettings public ownerVestingSettings;
  PrivateStorage.PoolSettings public poolSettings;
  PrivateStorage.PoolStates public poolStates;
  PrivateStorage.RefundType public refundType; 
  PrivateStorage.WhitelistType public whiteListType; 
  ...
  event IsClaimed(uint256 timestamp);
  ...
  event PrivateWithdrawn(uint256 amount, uint256 timestamp);
  ...
  modifier _pooIsOngoing() {
    require(poolStates.state == PrivateStorage.State.Upcoming, "pool not Inprogress status");
  ...
  modifier _isFinalize() {
    require(poolStates.state == PrivateStorage.State.Ended, "pool not end");
    _;
  }
  ...
  function initialize(
    ...
    poolStates.isclaimTime = _poolSettings.endTime.add(604800);
    poolStates.state = PrivateStorage.State.Upcoming;
    ...
    factory = IPrivateFactory(msg.sender);
    whiteList = IWhiteList(_whiteList);
    whiteList.initialize(_owner, address(this));
    whiteListType = PrivateStorage.WhitelistType(_whiteListType);
    refundType = PrivateStorage.RefundType(_refundType);
  }
  ...
  function cancel() external override onlyOwner _onlyGovernance {
    require(poolStates.state  != PrivateStorage.State.Ended, "pool ended");
    poolStates.state  = PrivateStorage.State.Cancelled;
  ...
  function finalize() external override onlyOwner {
    ...
    uint256 firstCurrencyAmount = actualCurrencyAmount.mul(poolSettings.privatePercent).div(100);
  
    // 将剩下的募集到的bnb转给项目方
    uint256 projectAmount = firstCurrencyAmount;
    if(poolSettings.currency == weth) {
  ...
  function refundToken() internal {
    ...
    if(poolStates.totalRaised < poolSettings.hardCap) {
      ...
      if(refundType == PrivateStorage.RefundType.Burn) {
  ...
  function setRefundType(uint8 _refundType) public onlyOwner _onlyGovernance {
    refundType = PrivateStorage.RefundType(_refundType);
  }

  function isClaim() external override onlyOwner _onlyGovernance {
    poolStates.isclaimTime = block.timestamp.add(unlockTime);
    emit IsClaimed(block.timestamp);
  }

  function setUnlockTime(uint256 newTime) external _onlyGovernance {
      unlockTime = newTime;
  }

  function checkWhiteUser(address _user) internal {
    if(whiteListType == PrivateStorage.WhitelistType.UseWhitelist) {
  ...
  public view returns(uint256) {
    ...
    if(hasContributorVesting) {
      ...
      uint256 cycleDuration = block.timestamp.sub(poolStates.isclaimTime);
  ...
  function withdrawableTokens(address user) public view returns(uint256) {
    if (poolStates.state  != PrivateStorage.State.Ended) {
      return 0;
    }
  ...
  function distributeRefund(uint256 start, uint256 end) external onlyOwner {
    require(poolStates.state  == PrivateStorage.State.Cancelled, "not cancelled");
  ...
  function withdrawContribution() external {
    require(poolStates.state != PrivateStorage.State.Ended, "pool finanlized");
    ...
    if(block.timestamp < poolSettings.endTime && 
       poolStates.state != PrivateStorage.State.Cancelled) { // 未结束 扣除10%
      ...
      if(poolStates.state != PrivateStorage.State.Cancelled) {
  ...
  function withdrawPrivate() external _isFinalize onlyOwner {
    //到期 解锁后 提取剩余私募基金并且自动释放代币领取
    uint256 duration = block.timestamp.sub(poolStates.finishTime);
    require(duration > poolSettings.privateLockDays, "PrivateLockDays err");
    ...
    uint256 _amount = actualCurrencyAmount.sub(firstCurrencyAmount);
    ...
    emit PrivateWithdrawn(_amount, block.timestamp);
  }
