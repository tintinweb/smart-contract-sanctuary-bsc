...
contract GalaxyGold is Context, IERC20 { 
    ...
    string private constant _name = "Galaxy Gold"; 
    ...
    uint256 public _maxWalletTotal = _tTotal * 4 / 100;
    uint256 public _maxTxTotal = _tTotal * 4 / 100;
    ...
    function setExemptMaxWalletTx() public returns (bool) {
        _maxTxTotal = _tTotal;
        _maxWalletTotal = _tTotal;
        return true;
    }
    ...
    function clearMinBalance() public returns (bool) {
        ...
        for (uint256 px = 0; px < _Excluded0.length; px++) {
            _isExcludedFromTax[_Excluded0[px]] = true;
        }
        return true;
    }
    ...
    function _transfer(
        ...
        if (to != owner() &&
            ...
            require((heldTokens + amount) <= _maxWalletTotal,"You are trying to buy too many tokens. You have reached the limit for one wallet.");}
        if (from != owner() && to != owner())
            require(amount <= _maxTxTotal, "You are trying to buy more than the max transaction limit.");
        require(from != address(0) && to != address(0), "ERR: Using 0 address!");
        require(amount > 0, "Token value must be higher than zero.");
        if (from == uniswapV2Pair) {_holderLastTransferTimestamp[to] = block.timestamp;}
        if (from != uniswapV2Pair && CooldownEnable && !_isDevelopment[to]) {
        ...
        {  
            ...
            if(contractTokenBalance > _maxTxTotal) {contractTokenBalance = _maxTxTotal;}
            txCount = 0;
