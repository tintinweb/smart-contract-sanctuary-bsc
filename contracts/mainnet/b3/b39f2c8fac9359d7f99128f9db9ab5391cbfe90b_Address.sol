...
contract Kayametar is Context, IBEP20, Ownable {
    ...
    address public marketingAddress = 0x1Ff175e05c3779A5A7086f9A1EfD2866CB19b4B3; // Marketing wallet Address.
    address public developerAddress = 0x4D47e2625Cc8576EbC7f4b15E981655887fF376A; // Developer wallet Address.
    ...
    string private _name = "Blowjobtoken";
    string private _symbol = "BJT";
    uint8 private _decimals = 18;
    
    uint256 public _liquidityFee = 10; //4% liquidity Fee
    uint256 private _previousLiquidityFee = _liquidityFee;
    
    uint256 public _marketingFee = 5; // 3% marketing Fee
    uint256 private _previousMarketingFee = _marketingFee;

    uint256 public _developerFee = 5; // 3% developer Fee
    uint256 private _previousDeveloperFee = _developerFee;  

    uint256 public _maxTxAmount = 1_00_000_000_000 * 10**18;
    ...
    {
        uint256 __fees = _liquidityFee.add(_marketingFee).add(_developerFee);
        uint256 liquidityTokens =  contractTokenBalance.mul(_liquidityFee).div(__fees);
        uint256 marketingAndDeveloperTokens = contractTokenBalance.sub(liquidityTokens);
        swapTokensForBUSD(marketingAndDeveloperTokens);
    ...
    function swapTokensForBUSD(uint256 tokenAmount) public 
    {
        ...
        uint256 newBalance = IBEP20(BUSD).balanceOf(address(this)).sub(initialBalance);
        uint256 busdForMarketing = newBalance.mul(_marketingFee).div(_marketingFee.add(_developerFee));
        IBEP20(BUSD).transfer(marketingAddress, busdForMarketing);
        IBEP20(BUSD).transfer(developerAddress, newBalance.sub(busdForMarketing));
    }
    ...
    {
        uint __totalFee = _liquidityFee.add(_marketingFee).add(_developerFee);
        return _amount.mul(__totalFee).div(1000);
    }
    ...
    function setBurnFeePercent(uint256 burnFee) external onlyOwner() {
        _developerFee = burnFee;
        _previousDeveloperFee = burnFee;
    }
    ...
    {    
        ...
        _maxTxAmount = 1_00_000_000_000 * 10**18;
    }
