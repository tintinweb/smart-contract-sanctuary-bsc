...
contract Kwoniba is IERC20, Context, Ownable {
    ...
    string private constant _symbol = "Kwoniba";
    string private constant _name = "KWONIBA";

    uint256 public Starbucks_max_BuyLimit = 10000000 * 10**9;
    uint256 public Starbucks_max_SellLimit = 10000000 * 10**9;

    address public Starbucks_iniINU_Wallet = msg.sender;
    address public Starbucks_Team_Wallet = msg.sender;

    uint256 public Starbucks_max_WalletLimit = 20000000 * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[Starbucks_iniINU_Wallet] = true;
        _isExcludedFromFee[Starbucks_Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[Starbucks_iniINU_Wallet] = true;
        allowedTransfer[Starbucks_Team_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= Starbucks_max_BuyLimit, "You are exceeding Starbucks_max_BuyLimit");
            require(balanceOf(to) + amount <= Starbucks_max_WalletLimit, "You are exceeding Starbucks_max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= Starbucks_max_SellLimit, "You are exceeding Starbucks_max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= Starbucks_max_WalletLimit, "You are exceeding Starbucks_max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(Starbucks_iniINU_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(Starbucks_Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function Starbucks_Update_MarketingWallet(address newWallet) external onlyOwner{
        Starbucks_iniINU_Wallet = newWallet;
    }
    
    function Starbucks_Update_TeamWallet(address newWallet) external onlyOwner{
        Starbucks_Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        Starbucks_max_BuyLimit = maxBuy * 10**decimals();
        Starbucks_max_SellLimit = maxSell * 10**decimals();
    }
    
    function Starbucks_Max_Walletlimit(uint256 amount) external onlyOwner{
        Starbucks_max_WalletLimit = amount * 10**decimals();
    }
