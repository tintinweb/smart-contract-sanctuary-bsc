...
// File: PNSToken/interfaces/IRegistrationPool.sol
...
interface IRegistrationPool {
    ...
    function gainTokens(address receiver, uint256 token) external returns(uint256);
    function getRoundTokens(uint256 round) external returns(uint256);
}
// File: PNSToken/interfaces/IStakingPool.sol
...
// File: PNSToken/TokenDistributor.sol
...
contract TokenDistributor is Ownable, Pausable, ReentrancyGuard {
    ...
    event RewardReleased(uint256 indexed round, uint256 stakeReward, uint256 registerReward);
    event DistributorInitial(uint256 InitialBlock);
    
    uint256 public InitialBlock;
    uint256 public currentRound;
    uint256 public NumBlocksPerRound;
    uint256 public lastRewardBlock;
    IERC20 public PNSToken;
    IPNSToken public PNSInterface;
    IStakingPool public StakingInterface;
    IRegistrationPool public RegistrationInterface;
    address public RewardContract;
    address public StakingContract;
    address public PNSRegister;

    constructor(
        address _tokenAddr,
        uint256 _numBlocks
    ){
        PNSToken = IERC20(_tokenAddr);
        PNSInterface = IPNSToken(_tokenAddr);
    ...
    function initalizeDistributor() public onlyOwner {
        ...
        RegistrationInterface.initalizePool(InitialBlock);
        emit DistributorInitial(InitialBlock);
    }

    function updateAddress(uint256 index, address newAddr) public onlyOwner {
        if(index == 0){
            PNSToken = IERC20(newAddr);
            PNSInterface = IPNSToken(newAddr);
        } else if(index == 1){
    ...
    function setPNSRegister(address _register) public onlyOwner {
        PNSRegister = _register;
    }
    ...
    function releaseRewards(uint256 rewards) external onlyOwner {
        _validation();
        PNSInterface.mint(StakingContract, rewards);
        uint256 registerRewards = RegistrationInterface.getRoundTokens(currentRound);
        PNSInterface.mint(RewardContract, registerRewards);
        RegistrationInterface.deliverReward(currentRound, registerRewards);
        StakingInterface.deliverReward(currentRound, rewards);
        currentRound++;
        emit RewardReleased(currentRound-1, rewards, registerRewards);
    }
