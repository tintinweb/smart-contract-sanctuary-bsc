...
contract MCUToken is ERC20("MCU", "MCU"), Ownable {
    ...
    uint256 public foundtionFee = 1;

    uint256 public genFee = 20;
    uint256 public genSecondFee = 10;
    uint256 public genThirdFee = 5;
    uint256 public genFeeSell = 10;
    uint256 public genSecondFeeSell = 5;
    uint256 public genThirdFeeSell = 5;
    ...
    address public foundtionWallet = 0x760787C168fd173f855DC0F83F200D8D1aDfEbD4;
    address public desilterWallet = 0x000000000000000000000000000000000000dEaD;
    ...
    constructor() {
        ...
        liquidityWallet = 0x2A9aEC06c258b8D2335A1e85cf1d2eF0Ccd4c79d;
        ...
        limitExcluded[owner()] = true;
        limitExcluded[deadWallet] = true;
        limitExcluded[liquidityWallet] = true;
        limitExcluded[desilterWallet] = true;
        limitExcluded[foundtionWallet] = true;
        limitExcluded[address(uniswapV2Pair)] = true;
        limitExcluded[address(uniswapV2BNBPair)] = true;
    ...
    function updateLimited(address account, bool enabled) public onlyManager {
    ...
    function _takeFees(address sender, address recipient, FeeInfo memory _feeInfo) private {
        if (_feeInfo.rBurn > 0) {
            uint256 endTotal =  3000 * 10**decimals();
            if (totalSupply().sub(_feeInfo.tBurn).sub(balanceOf(deadWallet)) < endTotal) {
             ...
             _distributionFees(deadWallet, _feeInfo.rBurn, _feeInfo.tBurn);

        }

        if (_feeInfo.rFoundation > 0) {
            _distributionFees(foundtionWallet, _feeInfo.rFoundation, _feeInfo.tFoundation);
        ...
        address referrer3 = _referrerByAddr[referrer2];
        address referrer4 = _referrerByAddr[referrer3];
        address referrer5 = _referrerByAddr[referrer4];
        address referrer6 = _referrerByAddr[referrer5];
        address referrer7 = _referrerByAddr[referrer6];
    
        
        if (_feeInfo.rGen > 0) {
            referrer = _checkBalanceAndEmpty(referrer);
            _distributionFees(referrer, _feeInfo.rGen, _feeInfo.tGen);

        }

        if (_feeInfo.rGenSecond > 0) {
            
            referrer2 = _checkBalanceAndEmpty(referrer2);
            referrer3 = _checkBalanceAndEmpty(referrer3);
            _distributionFees(referrer2, _feeInfo.rGenSecond, _feeInfo.tGenSecond);
            _distributionFees(referrer3, _feeInfo.rGenSecond, _feeInfo.tGenSecond);
            
        }

        if (_feeInfo.rGenThird > 0) {

            referrer4 = _checkBalanceAndEmpty(referrer4);
            referrer5 = _checkBalanceAndEmpty(referrer5);
            referrer6 = _checkBalanceAndEmpty(referrer6);
            referrer7 = _checkBalanceAndEmpty(referrer7);
            
            _distributionFees(referrer4, _feeInfo.rGenThird, _feeInfo.tGenThird);
            _distributionFees(referrer5, _feeInfo.rGenThird, _feeInfo.tGenThird);
            _distributionFees(referrer6, _feeInfo.rGenThird, _feeInfo.tGenThird);
            _distributionFees(referrer7, _feeInfo.rGenThird, _feeInfo.tGenThird);
        }

    }

    function _checkBalanceAndEmpty(address _add) internal view returns(address){
        if (_add == address(0)) {
            return desilterWallet;
        } else if (balanceOf(_add) < 10000000000000000) {
            return desilterWallet;
        }
        return _add;
    }
    function _distributionFees(address _add, uint _rAmount, uint _tAmount) internal {
        _rOwned[_add] = _rOwned[_add].add(_rAmount);
        if (_isExcludedReward[_add]) {
        ...
        )
    {
        ...
        ) = _getTValues(tAmount, _isBuy, _isSell);
    ...
    function _getTValues(uint256 tAmount, bool _isBuy, bool _isSell)
        private
        view
        returns (
            uint256 tTransferAmount,
        ...
        )
    {
        ...
        if (_isBuy) {
            ...
            _feeInfoTmp.tGenSecond = tAmount.mul(genSecondFee).div(1000);
            _feeInfoTmp.tGenThird = tAmount.mul(genThirdFee).div(1000);
        } else if (_isSell){
            _feeInfoTmp.tGen = tAmount.mul(genFeeSell).div(1000);
            _feeInfoTmp.tGenSecond = tAmount.mul(genSecondFeeSell).div(1000);
            _feeInfoTmp.tGenThird = tAmount.mul(genThirdFeeSell).div(1000);
            _feeInfoTmp.tBurn = tAmount.mul(burnFeeSell).div(100);
        ...
        tTransferAmount = tTransferAmount.sub(_feeInfoTmp.tBurn).sub(_feeInfoTmp.tFoundation);
        tTransferAmount = tTransferAmount.sub(_feeInfoTmp.tFee).sub(_feeInfoTmp.tGen).sub(_feeInfoTmp.tGenSecond * 2).sub(_feeInfoTmp.tGenThird * 4);

        _feeInfoTmp.tTransferAmount = tTransferAmount;

        return (tTransferAmount, 0, _feeInfoTmp);
    }
        ...
        )
    {
        ...
        _feeInfoTmp.rGenThird = _feeInfoTmp.tGenThird.mul(currentRate);
        _feeInfoTmp.rAmount = rAmount;

        rTransferAmount = rTransferAmount.sub(_feeInfoTmp.rBurn).sub(_feeInfoTmp.rFoundation);
        rTransferAmount = rTransferAmount.sub(_feeInfoTmp.rFee).sub(_feeInfoTmp.rGen).sub(_feeInfoTmp.rGenSecond * 2).sub(_feeInfoTmp.rGenThird * 4);

        _feeInfoTmp.rTransferAmount = rTransferAmount;

        _feeInfo = _feeInfoTmp;

        return (rAmount, rTransferAmount, 0, _feeInfo);
    }
    ...
    function _transfer(
        ...
        if (_referrerByAddr[recipient] == address(0) && amount >= 10000000000 && !sender.isContract() && !recipient.isContract()) {
    ...
    function _tokenTransferWithFee(
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, FeeInfo memory _feeInfo) = _getValues(tAmount, _isBuy, _isSell);
        ...
        if (!limitExcluded[recipient]) {
            require(balanceOf(recipient) <= 60 * 10 ** decimals(), "LIMIT REACHED");
        }
