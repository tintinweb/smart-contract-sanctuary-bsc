...
// Inheritance
contract LpStaking is IStaking, ReentrancyGuard, Pausable {
    using SafeMath for uint256;
    ...
    function stake(uint256 amount) external override nonReentrant notPaused updateReward(msg.sender) {
        ...
        // release reward if already staked
        if (_balances[msg.sender] > 0) {
            releaseReward(msg.sender);
        }
    ...
    function setRewardRate(uint256 rewardsPerInterval, uint256 interval) external onlyOwner updateReward(address(0)) {
    ...
    function updateFinishPeriod(uint256 timestamp) external onlyOwner updateReward(address(0)) {
        require(timestamp > block.timestamp, "timestamp must be > current Time");
        require(timestamp < periodFinish, "timestamp must be < periodFinish");

        uint256 recoverableAmount = rewardRate.mul(periodFinish.sub(timestamp));
        periodFinish = timestamp;
        rewardsToken.safeTransfer(owner, recoverableAmount);
    }
    
    /* ========== INTERNAL FUNCTIONS ========== */   
    function releaseReward(address _user) internal {
