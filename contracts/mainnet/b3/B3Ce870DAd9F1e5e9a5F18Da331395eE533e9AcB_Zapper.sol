...
contract ZapperFee {
    ...
    EnumerableSet.AddressSet private treasuryTokens;
    ...
    {
        uint256 tSize = treasuryTokens.length();
        if (tSize == 0) {
        ...
        for (uint256 i = from; i < to; i += 1) {
            address token = treasuryTokens.at(i);
