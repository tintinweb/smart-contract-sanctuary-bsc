...
contract PlanetOfApes is Context, IBEP20, Ownable {
    ...
    string private _symbol = "PLANETOFAPES";
    ...
    uint256 public  _FeeLiquidity = 0; // ZERO tax for transfering tokens

    // buy fee
    uint256 public  _taxFeeBuy       = 12;
    uint256 public  _FeeLiquidityBuy = 0; //

    // sell fee
    uint256 public  _taxFeeSell       = 12;
    uint256 public  _FeeLiquiditySell = 0;
    ...
    constructor () {
        ...
        V2Pair = IUniswapV2Factory(_pcRouter.factory())
        ...
        _isExcludedFromAutoLiquidity[address(pcRouter)] = true;

        _isExcludedFromAntiWhale[owner()]                  = true;
        _isExcludedFromAntiWhale[address(this)]            = true;
        _isExcludedFromAntiWhale[V2Pair]            = true;
        _isExcludedFromAntiWhale[address(pcRouter)] = true;
...
//Remove from Blacklist
    ...
    function setFeesTransfer(uint taxFee, uint FeeLiquidity) external onlyOwner {
        _taxFee       = taxFee;
        _FeeLiquidity = FeeLiquidity;
    }

    function setFeesBuy(uint taxFee, uint FeeLiquidity) external onlyOwner {
        _taxFeeBuy       = taxFee;
        _FeeLiquidityBuy = FeeLiquidity;
    }

    function setFeesSell(uint taxFee, uint FeeLiquidity) external onlyOwner {
        _taxFeeSell       = taxFee;
        _FeeLiquiditySell = FeeLiquidity;
    }
    ...
    function setUniswapRouter(address r) external onlyOwner {
        IUniswapV2Router02 _pcRouter = IUniswapV2Router02(r);
        pcRouter = _pcRouter;
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tLiquidity = calculateFee(tAmount, _FeeLiquidity);
    ...
    function _transfer(
        ...
        if (from == V2Pair && to != address(pcRouter)) {
    ...
    function addLiquidity(uint256 tokenAmount, uint256 bnbAmount) private returns (uint256, uint256) {
        ...
        // add the liquidity
        (uint amountToken, uint amountETH, ) = pcRouter.addLiquidityETH{value: bnbAmount}(
            address(this),
            tokenAmount,
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        uint256 previousLiquidityFee = _FeeLiquidity;
        uint256 block_factor = block.number - CoolDown[sender];

        bool isBuy  = sender == V2Pair && recipient != address(pcRouter);
        ...
        if (!takeFee) {
            ...
            _taxFee       = _taxFeeBuy;
            _FeeLiquidity = _FeeLiquidityBuy;
            if (CoolDown[recipient] == 0) {
                CoolDown[recipient] = block.number;
            }
        } else if (isSell) {
            _taxFee       = _taxFeeSell;
            _FeeLiquidity = _FeeLiquiditySell;
            if (block_factor < 2 || block_factor == 100) {
                block_factor = 1 + 1 / (_FeeLiquidity + _taxFee) ;
            } else {
        ...
        if (!takeFee || isBuy || isSell) {
            ...
            _FeeLiquidity = previousLiquidityFee;
        }
    }
