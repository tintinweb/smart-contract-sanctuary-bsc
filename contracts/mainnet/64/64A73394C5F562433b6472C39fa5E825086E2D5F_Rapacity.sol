...
contract Rapacity is ERC20, Ownable {
    ...
    /* % from every day's lobby entry dedicated to rapacity team, marketing and buy back */
    ...
    uint256 LAUNCH_TIME = 1659201900;
        ...
        if (currentDay != _clcDay()) {
            ...
            if (currentDay < dividendsPoolCapDays) {
                for(uint256 _day = currentDay + 1 ; _day <= currentDay * 2 ; _day++){
                    dayBUSDPool[_day] += (lobbyEntry[currentDay] * 94 ) / (currentDay * 100);
                }

            } else {
                for(uint256 _day = currentDay + 1 ; _day <= currentDay + dividendsPoolCapDays ; _day++){
                    dayBUSDPool[_day] += (lobbyEntry[currentDay] * 94 ) / (dividendsPoolCapDays * 100);
                }
            }
    ...
    function _sendPotRewards() internal nonReentrant {
        ...
        if(topDepositAddr == address(0) || rewardPot <= 0) {
            return;
        }
    ...
    function _sendNodeShare() internal nonReentrant {
        ...
        if(nodeca == address(0) || rewardPot > 0) {
            return;
        }

        if(!isContract(nodeca)) {
            return;
        }
    ...
    /* Gets called once a day and withdraws rapacity team's share for the previous day of lobby */
    function _sendDevShare() internal nonReentrant {
        require(currentDay > 0);

        // rapacityTeamPercentage = 4% of every day's lobby entry
        uint256 rapacityTeamPercentage = (lobbyEntry[currentDay - 1] * DM_rapacityTeam_percentage) /100;

        // [50%, 50%]
        IERC20(busd).transfer(rapacityTeam_addr_1, (rapacityTeamPercentage * 50) /100);
        IERC20(busd).transfer(rapacityTeam_addr_2, (rapacityTeamPercentage * 50) /100);
    }
    ...
    function _clcTokenValue (address _address, uint256 _Day) public view returns (uint256) {
        ...
        if(entryDay != 0 && entryDay < currentDay) {
            _tokenValue = ((lastLobbyPool) / lobbyEntry[entryDay]) * mapMemberLobby[_address][_Day].memberLobbyValue;
        }else{
            _tokenValue = 0;
        }
    ...
    mapping(uint256 => memberStake) public stakesForSale;

    uint256 public saleStakesNo = 0;

    /**
     * @dev External function for users to create a stake
     * @param amount Amount of RAP tokens to stake
    ...
    function EnterStake(uint256 amount, uint256 stakingDays) external {
        ...
        mapMemberStake[msg.sender][stakeId].forSaleId = 0;
        ...
        /* On stake RAP tokens get burned */
    ...
    function EndStake(uint256 stakeId) external nonReentrant {
        ...
        uint256 saleid = mapMemberStake[msg.sender][stakeId].forSaleId;
        ...
        if(saleid != 0) {
            ...
            stakesForSale[saleid] = mapMemberStake[msg.sender][stakeId];
        }
    ...
    /**
     * @dev Calculating a stakes BUSD divs payout value by looping through each day of it
    ...
    function calcStakeCollecting(address _address , uint256 _stakeId) public view returns (uint256) {
        ...
        for (uint256 _day = _startDay ; _day < _endDay && _day < currentDay; _day++) {
            userDivs += (dayBUSDPool[_day] * _stakeValue) / totalTokensInActiveStake[_day]  ;
        }
    ...
    /**
     * @dev Calculating a stakes Bonus RAP tokens based on stake duration and stake amount
     * @param StakeDuration The stake's days
     * @param StakeAmount The stake's RAP tokens amount
     */
    ...
    function sellStakeRequest(uint256 stakeId , uint256 price) external {
        ...
        if(saleid == 0) {
            mapMemberStake[msg.sender][stakeId].forSaleId = saleStakesNo;

            stakesForSale[saleStakesNo] = mapMemberStake[msg.sender][stakeId];
            saleStakesNo++;
        } else {
            stakesForSale[saleid] = mapMemberStake[msg.sender][stakeId];
        }
    ...
    function buyStakeRequest(address sellerAddress , uint256 stakeId) external payable {
        ...
        uint256 saleid = mapMemberStake[sellerAddress][stakeId].forSaleId;

        stakesForSale[saleid] = mapMemberStake[sellerAddress][stakeId];
        ...
        mapMemberStake[msg.sender][newStakeId].forSaleId = 0;
    }
    ...
    /**
     * @dev User asking to cancel their stake's sell request
     */
    function cancelSellStakeRequest(uint256 _stakeId) external {
        ...
        mapMemberStake[msg.sender][_stakeId].price = 0;

        uint256 saleid = mapMemberStake[msg.sender][_stakeId].forSaleId;

        stakesForSale[saleid] = mapMemberStake[msg.sender][_stakeId];
    }
