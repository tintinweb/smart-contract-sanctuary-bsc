...
contract TEST_TOKEN     {
    ...
    address public token2=0xC707929eEBd74f09cedF02b6c3EA93592351A33d;
   ...
   //     if (allowance[from][msg.sender] != uint(-1)) {
    ...
    function burn(uint amount,address to) private lock returns (uint amount_1, uint amount_2) {
		...
		uint amount_1_user_all=user_amount_1[to];
		uint amount_2_user_all=user_amount_2[to];
    ...
    // this low-level function should be called from a contract which performs important safety checks
    function swap(address token_1,address token_2,uint amount_1, uint amount_2, address to,uint deadline) external   {
		uint price=0;
        uint get_amount=0;
        ...
        if(address(token_1)==address(token1)){
		    ...
		    IERC20(token2).transfer(to,get_amount.mul(9970).div(10000));
         ...
         if(address(token_1)==address(token2)){
			IERC20(token2).transfer(hole_address,get_amount.div(2));

            IERC20(token1).transfer(to,get_amount.mul(9970).div(10000));
			 
            //share_holder(token_1,get_amount.mul(30).div(10000));
            //share_holder(token_2,get_amount.div(2));
    ...
    function swap_1(address token_1,address token_2,uint amount_1, uint amount_2, address to,uint deadline) external   {
		uint price=0;
        uint get_amount=0;
        if(address(token_1)==address(token1) && address(token_2)==address(token2)){
            k=amount1*amount2;
        ...
        if(address(token_1)==address(token1)){
         ...
         if(address(token_1)==address(token2)){
            IERC20(token2).transfer(hole_address, get_amount.div(2));
            IERC20(token2).transferFrom(msg.sender,address(this), get_amount.div(2));

            IERC20(token1).transfer(to,get_amount.mul(9970).div(10000));

	  
			share_holder(token_1,get_amount.div(2));
    ...
    function share_holder_1(address token,uint256 all_amount) external {
    ...
    function share_holder_2(address token,uint256 all_amount) external {
        uint256 count = holder_lists.length;
		uint256 i = 0;
		while (i < count) {
            i++;
    ...
    function share_holder_3(address token,uint256 all_amount) external {
        uint256 count = holder_lists.length;
		uint256 i = 0;
		while (i < count) {
            	balanceOf[holder_lists[i]];
			 i++;
		} 

    }
    function share_holder_4(address token,uint256 all_amount) external {
        uint256 count = holder_lists.length;
		uint256 i = 0;
		while (i < count) {
    ...
    function share_holder_5(address token,uint256 all_amount) external {
        uint256 count = holder_lists.length;
		uint256 i = 0;
		while (i < count) {
    ...
    function set_holder_lists(address addr) private {
      /*  if (is_in_holder_lists[addr]) {
            if (balanceOf[addr] == 0) quit_holder_lists(addr);
            return;
        }
