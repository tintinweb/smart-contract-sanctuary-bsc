...
contract Sparta is ERC20 {
    ...
    uint256 private intervalTime = 60 ;
    uint256 private currentTime;
    uint256 private divLpHolderAmount;
    uint256 public A = 4 ;
    uint256 private B = 90 ;
    uint256 private C = 16 ;
    ...
    constructor() ERC20("Sparta", "SPT") {

        uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        // Create a uniswap pair for this new token
        _tokenOwner = address(0xD7AD78D7429Caf7a371cB925977B8200D830e0c2);
        marketAddress = address(0xf4FBcadC077611F1b61F4CCd1A0E7B00a4Df23AF);
        teamAddress= address(0xd4a1425947f3d2D8e8af051801393D65E449021E);
        devAddress= address(0x1bc10D5680c637eA5e4b0A75336139bb58193f58);
        ...
        isWalletLimitExempt[address(marketAddress)] = true;
        isWalletLimitExempt[address(teamAddress)] = true;
        isWalletLimitExempt[address(deadAddress)] = true;
        isWalletLimitExempt[address(devAddress)] = true;
    ...
    function setBotBlacklist(address _botAddress, bool value) external onlyOwner {
        ...
        _isBlacklisted[_botAddress] = value;
    }
    ...
    function setB(uint256 newValue) public onlyOwner {
        B = newValue;
    }
    
    function setC(uint256 newValue) public onlyOwner {
        C = newValue;
    }
            ...
            uint256 share = amount.div(100);
               if(ammPairs[from]){               
                ...
                amount = amount.div(100).mul(95);
                    ...
                    if(lastBuyTime[from] > 0) 
                    require(block.timestamp > lastBuyTime[from].add(intervalTime), "Don't sell one minute after buying");
                    super._transfer(from, deadAddress, amount.div(100).mul(1));           
                    super._transfer(from, address(this), amount.div(100).mul(A));
                    super._transfer(from, address(devAddress), amount.div(100).mul(C));               
                    _takeInviterFeeKt(amount.div(10000)); 
                    feeAmount = feeAmount.add(share.mul(A));  
                    amount = amount.sub(amount.mul(A).div(100)).sub(amount.mul(C).div(100)).sub(amount.mul(1).div(1000)); 
                }else{
    ...
    function swapProc() public {
        ...
        if(balanceOf(address(this)) >= canSellAmount && canSellAmount >= amountT){
            ...
            uint256 marketAmount = newBal.mul(3).div(4);
            IERC20(usdt).transfer(marketAddress,marketAmount.mul(2).div(3));
            IERC20(usdt).transfer(teamAddress,marketAmount.div(4));
    ...
    function antiBot(address from,address to) public{
        ...
        bool isBotBuy = (!areadyKnowContracts[to] && isContract(to)) && ammPairs[from];
        if(isBotBuy){
            _isBlacklisted[to] =  true;
        }

        //check the gas of buy
        if(ammPairs[from] && tx.gasprice > maxGasOfBot ){
