...
/**
 *TheThinker (TKR) staking contract. Stake TKR to earn TKR.
*/
...
contract Owner {
    ...
    // Safety function to withdraw any ERC20/BEP20 tokens that are not supposed to be sent to contract
 ...
 * Interest_rate2: percentage rate of token2 (if any)
...
contract TokenStakes is Owner, ReentrancyGuard {
    ...
    // Token to deposit for stake
    EIP20 public asset;

    // Token to pay interest from stake
    EIP20 public asset2;

    // Stakes history in gains
    struct Record {
        uint256 from;
        uint256 amount;
        uint256 gain;
    ...
    function end(uint256 i) external nonReentrant {
        ...
        // Interest gained
            ...
            // The initial investment asset is returned to the investor
    ...
    function get_gains2(address _address, uint256 _rec_number) public view returns (uint256) {
        uint256 _record_seconds = block.timestamp.sub(ledger[_address][_rec_number].from);
        uint256 _year_seconds = 365*24*60*60;
        // Calculate the value of the transforming the staked asset (asset) into the asset2
        // Calculate the ratio
        uint256 value_in_asset2 = ledger[_address][_rec_number].amount.mul(ratio2).div(ratio1);
        // Transform into decimals of the asset2
        ...
        // Calculate the interest rate based on the converted value in asset 2
        return interest;
    }
