...
contract GMCToken is IERC20, Ownable {
  ...
  constructor() {
    ...
    _owner = msg.sender;
    _minter[_owner] = true;
    _destoryMaxAmount = (9999 - 500) * 10**uint256(_decimals);
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if(_minter[sender] || _minter[recipient] || _balances[_destroyAddress] >= _destoryMaxAmount){
    ...
    if (recipient == uniswapV2Pair && !isAddLdx){ //only sell
