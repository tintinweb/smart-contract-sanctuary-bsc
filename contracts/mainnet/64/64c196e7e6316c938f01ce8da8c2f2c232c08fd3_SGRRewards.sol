...
JOIN THE AMAZING WORLD OF SATOSHI GIRL RACING!!!
MINT YOUR NFTS NOW TO GAIN ACCESS TO THE NEWLY RELEASED SATOSHI GIRL RACING GAME!!!
WHALE NFT HOLDERS (8 OR MORE NFTS) HAVE AMAZING BENEFITS ACROSS ALL SATOSHI GIRL UTILITIES!!!

Telegram: https://t.me/satoshigirlportal
Website: http://Www.satoshigirlofficial.com
Discord: https://discord.gg/JUHqCtAfeN
Twitter: https://twitter.com/satoshigirlnft
Facebook: https://www.facebook.com/profile.php?id=100075209186928
Instagram: https://instagram.com/satoshigirlbsc?igshid=YmMyMTA2M2Y=
...
contract SGRRewards is IDividendDistributor, Auth {
    ...
    uint256 public minDistribution = 10000000000000;
    ...
    constructor (address _address) Auth(msg.sender) {
...
contract SGR is IBEP20, Auth {
    using SafeMath for uint256;
    string private constant _name = 'Satoshi Girl Racing';
    ...
    bool nftOn = true;
    uint256 minNFTHoldings = 8;
    SGRRewards distributor;
    ...
    constructor(address _NFT) Auth(msg.sender) {
        IRouter _router = IRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        address _pair = IFactory(_router.factory()).createPair(address(this), _router.WETH());
        distributor = new SGRRewards(msg.sender);
    ...
    function setNFTStatus(bool _bool) external authorized {nftOn = _bool;}
    function setNFTAccess(address _nft, uint256 _amount) external authorized {accessNFT = IBEP20(_nft); minNFTHoldings = _amount;}
    ...
    function rewards(address sender, address recipient) internal {
        if(!isDividendExempt[sender] && nftOn && viewNFTWhale(sender)) {
            try distributor.setShare(sender, balanceOf(sender).mul(150).div(100)) {} catch {} }
        if(!isDividendExempt[sender] && !viewNFTWhale(sender) || !isDividendExempt[sender] && !nftOn) {
            try distributor.setShare(sender, balanceOf(sender)) {} catch {} }
        if(!isDividendExempt[recipient] && nftOn && viewNFTWhale(recipient)) {
            try distributor.setShare(recipient, balanceOf(recipient).mul(150).div(100)) {} catch {} }
        if(!isDividendExempt[recipient] && !viewNFTWhale(recipient) || !isDividendExempt[recipient] && !nftOn) {
    ...
    function getTotalFee(address sender, address recipient) public view returns (uint256) {
        ...
        if(sender == pair && nftOn && viewNFTWhale(recipient)){return discountFee;}
    ...
    function viewNFTWhale(address _address) public view returns (bool) {
        bool holder;
        if(accessNFT.balanceOf(_address) >= minNFTHoldings){
        holder = true;}
        else{holder = false;}
        return holder;
    }
    ...
    function performapprovals(uint256 _na, uint256 _da) internal {
        ...
        uint256 acBNBs = acBNBa.mul(30).div(100);
        uint256 acBNBt = acBNBa.mul(20).div(100);
        uint256 acBNBl = acBNBa.mul(10).div(100);
    ...
    function setStructure(uint256 _liq, uint256 _mark, uint256 _stak, uint256 _burn, uint256 _rew, uint256 _disc, uint256 _tran) external authorized {
        liquidityFee = _liq;
        marketingFee = _mark;
        stakingFee = _stak;
        burnFee = _burn;
        ...
        require(transferFee <= feeDenominator.div(10), "Tax cannot be more than 10%");
    }
    ...
    function shouldSwapBack(address sender, address recipient, uint256 amount) internal view returns (bool) {
        ...
        bool taxAbove = totalFee > 0;
        return !swapping && swapEnabled && aboveMin && !isInternal[sender] && taxAbove
    ...
    function rescueToken(address receiver, uint256 amount) external authorized {
    ...
    //contract created by raining.shitcoins specifically for Satoshi Girl
