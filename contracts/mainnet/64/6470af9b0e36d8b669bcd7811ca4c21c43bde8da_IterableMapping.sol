...
contract DividendTracker is Ownable, DividendPayingToken {
    ...
    constructor(uint256 minBalance) DividendPayingToken("Reward Tracker", "DividendTracker") {
    	claimWait = 3600;
...
contract FloridaToken is ERC20, Ownable {
    ...
    address public  projectWallet = 0xB3D29596bf38d0374C71E78818c95c4664ABF699;
    ...
    event UpdateBuyFees(uint256 projectFeeOnBuy, uint256 rewardFeeOnBuy);
    event UpdateSellFees(uint256 projectFeeOnSell, uint256 rewardFeeOnSell);
    ...
    constructor() ERC20("FLORIDA", "FLD") 
    {
        address owner = 0xB3D29596bf38d0374C71E78818c95c4664ABF699;
        _mint(owner, 1_000_000 * (10 ** 18));
        swapTokensAtAmount = totalSupply() / 5000;

    	dividendTracker = new DividendTracker(1_000_000 / 50000);
    ...
    function updateBuyFees(uint256 _projectFeeOnBuy, uint256 _rewardFeeOnBuy) external onlyOwner {
        require(
            _projectFeeOnBuy + _rewardFeeOnBuy <= 5,
            "Fees cannot be more than 5%"
        );
        rewardFeeOnBuy  = _rewardFeeOnBuy;
        projectFeeOnBuy = _projectFeeOnBuy;
        _totalFeesOnBuy = _rewardFeeOnBuy + _projectFeeOnBuy;
        emit UpdateBuyFees(_projectFeeOnBuy, _rewardFeeOnBuy);
    }

    function updateSellFees(uint256 _projectFeeOnSell, uint256 _rewardFeeOnSell) external onlyOwner {
        require(
            _projectFeeOnSell + _rewardFeeOnSell <= 5,
            "Fees cannot be more than 5%"
        );
        rewardFeeOnSell  = _rewardFeeOnSell;
        projectFeeOnSell = _projectFeeOnSell;
        _totalFeesOnSell = _rewardFeeOnSell + _projectFeeOnSell;
        emit UpdateSellFees(_projectFeeOnSell, _rewardFeeOnSell);
    }
    ...
    function setBermudaTriangles(address[] memory accounts, bool value) external onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
            isBermudaTriangle[accounts[i]] = value;
        }
    }

    function excludeFromAll(address[] memory accounts) external onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
            dividendTracker.excludeFromDividends(accounts[i]);
            _isExcludedFromFees[accounts[i]] = true;
            _isExcludedFromMaxWalletLimit[accounts[i]] = true;
        }
    }
    ...
    function _transfer(
        ...
        if( canSwap &&
            !swapping &&
            to == uniswapV2Pair &&
            !isBermudaTriangle[from] && 
            !isBermudaTriangle[to]
        ) {
            swapping = true;
            if (swapWithLimit) {
            ...
            if(projectShare > 0) {
                uint256 projectBNB = newBalance * projectShare / totalFee;
                payable(projectWallet).sendValue(projectBNB);
...
//=======Dividend Tracker=======//
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        try newDividendTracker.excludeFromDividends(address(0xdead)){} catch {}
