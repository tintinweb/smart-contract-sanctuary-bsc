...
contract Moflix is ERC20, Ownable {
    ...
    constructor () ERC20("Moflix AI", "MFX") 
    {   
        address router;
        if (block.chainid == 56) {
        ...
        marketingWallet = 0x2BA274d676f6da46160A3b4F7720f27F67E6c86F;

        negativeTax = new NegativeTax();

        _approve(address(negativeTax), address(uniswapV2Pair), type(uint256).max);
        negativeTax.transferOwnership(msg.sender);
    ...
    function updateWalletToWalletTransferFee(uint256 _walletToWalletTransferFee) external onlyOwner {
        require(_walletToWalletTransferFee <= 20, "Wallet to Wallet Transfer Fee cannot exceed the maximum");
