...
contract MTDWinery {
    ...
    /** Base parameters for Metadroid Winery **/
    uint256 public EGGS_TO_HIRE_1MINERS = 2592000;
    uint256 public REFERRAL = 50; 
    uint256 public PERCENTS_DIVIDER = 1000;
    uint256 public DEV = 10;
    uint256 public TEAM = 30;
    uint256 public MKT = 400;
    uint256 public MARKET_EGGS_PERCENT_MUL = 4;
    uint256 public MARKET_EGGS_PERCENT_DIV = 5;
    uint256 public ROI_BOOST = 2;

    uint256 public MIN_INVEST_LIMIT = 2000000 ether; /** min. 2000000 MTD  **/
    uint256 public WALLET_DEPOSIT_LIMIT = 100000000000 ether; /** max. 100000000000 MTD  **/

	uint256 public COMPOUND_BONUS_MAX_TIMES = 6; /** 6 Times Compound every 24 Hours / 6 days. **/

    uint256 public WITHDRAWAL_TAX = 800; // 80% tax for For Early Withdrawals - Penalties
    uint256 public COMPOUND_FOR_NO_TAX_WITHDRAWAL = 13; // Compound days, for no tax Withdrawal.
    ...
    constructor(address _dev1, address _dev2, address _team1, address _team2, address _mkt, address _token_address) {
		require(!isContract(_dev1) && !isContract(_dev2) && !isContract(_team1) && !isContract(_team2));
        owner = msg.sender;
        dev1 = _dev1;
        dev2 = _dev2;
        team1 = _team1;
        team2 = _team2;
        mkt = _mkt; //multisig contract
    ...
    function payFees(uint256 eggValue) internal returns(uint256){
        ...
        erctoken.transfer(dev2,devtax);
        erctoken.transfer(mkt,mktng);
        erctoken.transfer(team1,teamtax);
        erctoken.transfer(team2,teamtax);
        return 2*devtax + 2*teamtax + mktng;
    }
