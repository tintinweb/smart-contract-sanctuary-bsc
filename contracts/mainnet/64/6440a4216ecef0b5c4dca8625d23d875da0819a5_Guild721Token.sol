...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    function _setSymbol(string memory symbol2) internal {
        _symbol = symbol2;
    }

    function _setName(string memory name2) internal {
        _name = name2;
    }
...
// File: contracts/nft/Player721Token.sol

pragma solidity ^0.6.12;

contract Guild721Token is ERC721Tradable{
    ...
    function setSymbol(string memory symbol2) external onlyOwner {
        _setSymbol(symbol2);
    }

    function setName(string memory name2) external onlyOwner {
        _setName(name2);
    }
    ...
    function mint(address to, uint256 tokenId) public onlyMint returns (bool) {
        _safeMint(to, tokenId);
        return true;
    }

    function batchMint(address to, uint256 startTokenId, uint256 endTokenId) public onlyOwner returns (bool) {
     ...
     * - `tokenId` must exist.
     */
    function setTokenURI(uint256 tokenId, string memory tokenURI) public onlyMint returns (bool) {
     ...
     * @return A boolean that indicates if the operation was successful.
     */
    function safeMint(address to, uint256 tokenId, bytes memory _data) public onlyMint returns (bool) {
        _safeMint(to, tokenId, _data);
        return true;
    }

    function mintWithTokenURI(address to, uint256 tokenId, string memory tokenURI) public onlyMint returns (bool) {
        _safeMint(to, tokenId);
        _setTokenURI(tokenId, tokenURI);
        return true;
    }
