...
contract CrazyCatLady is Context, IERC20, Ownable {
    ...
    uint256 public maxWalletLimit = 4_000_000 * 10**9;
    
    uint256 public genesis_block;
    uint256 private deadline;
    
    address public marketingWallet = 0xc7205cC082Df13271eB06D910e42A11AE9000B1d;
    address private operationsWallet = 0xDd48672AC5eB824ff567852D4e5571B323A63165;
    address private charityWallet = 0xb0816093943866F8713f902F0eAC9bDed74B8d05;

    string private constant _name = "CrazyCatLady";
    ...
    Taxes public buyTaxes = Taxes(0, 3, 1, 1, 2);
    Taxes public sellTaxes = Taxes(0, 7, 1, 3, 4);
    ...
    function setBuyTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _operations, uint256 _charity) public onlyOwner {
        require((_rfi + _marketing + _liquidity + _operations + _charity) <= 20, "Fees can't exceed 20%");
        buyTaxes = Taxes(_rfi,_marketing,_liquidity,_operations,_charity);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _operations, uint256 _charity) public onlyOwner {
        require((_rfi + _marketing + _liquidity + _operations + _charity) <= 20, "Fees can't exceed 20%");
        sellTaxes = Taxes(_rfi,_marketing,_liquidity,_operations,_charity);
        emit FeesChanged();
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell, bool useLaunchTax) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.roperations, to_return.rcharity) = _getRValues2(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell, bool useLaunchTax) private view returns (valuesFromGetValues memory s) {
        ...
        s.tTransferAmount = tAmount-s.tRfi-s.tMarketing-s.tLiquidity-s.toperations-s.tcharity;
        return s;
    }
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rMarketing, uint256 rLiquidity){
        ...
        rTransferAmount =  rAmount-rRfi-rMarketing-rLiquidity-roperations-rcharity;
        return (rAmount, rTransferAmount, rRfi,rMarketing,rLiquidity);
    }
    
    function _getRValues2(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 roperations,uint256 rcharity) {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tMarketing + s.toperations+ s.tcharity);
        }
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.marketing + temp.operations + temp.charity) * 2;
