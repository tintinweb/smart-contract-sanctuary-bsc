...
abstract contract ContextFelix {
...
contract OwnableFelix is ContextFelix {
    address private _owner;

    event OwnerFelixpTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    constructor () {
        ...
        _owner = msgSender;
        emit OwnerFelixpTransferred(address(0), msgSender);
    }
     ...
     */
    modifier onlyOwner() {
        require(_owner == _msgSender(), "OwnableFelix: caller is not the owner");
        _;
    }

 
    function renounceOwnerFelixp() public virtual onlyOwner {
        emit OwnerFelixpTransferred(_owner, address(0));
        _owner = address(0);
    }

    /**
     * @dev Transfers ownerFelixp of the contract to a new account (`newOwner`).
     * Can only be called by the current owner.
     */
    function transferOwnerFelixp(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "OwnableFelix: new owner is the zero address");
        emit OwnerFelixpTransferred(_owner, newOwner);
        _owner = newOwner;
    }
}

contract Felix is ContextFelix, BinanceBNB, OwnableFelix {
