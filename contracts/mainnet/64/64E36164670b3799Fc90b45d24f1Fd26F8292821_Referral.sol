...
contract Referral is IReferral, SafeOwnable {
    IReferralAccess public referralAccess;
    address public immutable initReferrer;
    ...
    constructor(address _initReferrer) {
        initReferrer = _initReferrer;
    }

    function setReferralAccess(IReferralAccess _referralAccess)
        external
        onlyOwner
    {
    ...
    function setReferrer(address _referrer) public virtual override {
        require(
            msg.sender != initReferrer,
            "initial referrer cannot set referrer"
        );
        require(_referrers[msg.sender] == address(0), "repeat operation");
        require(
            isCanReferralSuccess(_referrer) || _referrer == initReferrer,
    ...
    function isCanReferralSuccess(address _referrer)
        public
        view
        returns (bool)
    {
        if (_referrer == initReferrer) {
            return true;
        }
        if (
            address(referralAccess) != address(0) &&
            !referralAccess.hasReferralAccess(_referrer)
        ) {
            return false;
        }
        if (_referrers[_referrer] == address(0)) {
            return false;
        }
        return true;
    }
