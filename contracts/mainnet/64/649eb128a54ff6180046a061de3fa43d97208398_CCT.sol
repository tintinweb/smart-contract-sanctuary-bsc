...
contract CCT is Context, IERC20, Ownable {
    ...
    address public MKADDRESS = 0x703aE24b5Da014B99440F82CfC43Ca2Ba6E1F1db;
    ...
    function _initParam(uint256 tAmount,Param memory param) private view  {
        ...
        param.tBurn = tAmount * _bonusFee / 100;
    ...
    function _sellParam(uint256 tAmount,Param memory param) private view  {
        param.tRRurn = tAmount * 10 / 100;
        param.tMKurn = tAmount * 5 / 100;
        uint tFee = tAmount * 15 / 100;
        param.tTransferAmount = tAmount.sub(tFee);
    }

    function _takeFee(Param memory param,address from)private {
        if( param.tLP > 0 ){
            _take(param.tLP, from, swapV2PairAddress);
        }
        if( param.tBurn > 0 ){
        ...
        if( param.tRRurn > 0 ){
            _tTotal = _tTotal.sub(param.tRRurn);
            _take(param.tRRurn, from, address(0));
        }
        if( param.tMKurn > 0 ){
            _tTotal = _tTotal.sub(param.tMKurn);
            _take(param.tMKurn, from, MKADDRESS);
        }
    }
