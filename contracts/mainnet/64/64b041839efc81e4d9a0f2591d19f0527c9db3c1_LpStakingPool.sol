...
contract LpStakingPool is USDTWrapper,Ownable {
    ...
    uint256 public dayReward=50*10**8;
    ...
    function setInToken(address a) public onlyOwner{
        stakeInToken = IERC20(a);
    }
    function setOutToken(address a) public onlyOwner{
        stakeOutToken = IERC20(a);
    }
    function setMinlp(uint256 a) public onlyOwner{
        minlp = a;
    }
    function setDayReward(uint256 a) public onlyOwner{
        dayReward = a;
    }
    function setRate(uint256 a,uint256 b) public onlyOwner{
        rate[a] = b;
    }
    ...
    modifier updateReward(address account) {
        perSecondRewardAll=dayReward/24/60/60;
    ...
    function earned(address account) public view returns (uint256) {
        ...
        return   balanceOf(account).mul(perSecondRewardAll).mul(block.timestamp - getRewardTime[account]).div(totalSupply());
    }
    ...
    function getReward() public updateReward(msg.sender) checkStart {
        ...
        if (reward > 0) {
            ...
            address p = stakeOutToken._inviter(msg.sender);
            if ( p!= address(0) && balanceOf(p)>=minlp && rate[0]>0) {
                stakeOutToken.safeTransfer(p, reward * rate[0]/ 1000);
                p = stakeOutToken._inviter(p);
                if (p != address(0) && balanceOf(p)>=minlp  && rate[1]>0) {
                    stakeOutToken.safeTransfer(p, reward * rate[1]/ 1000);
                }
            }
