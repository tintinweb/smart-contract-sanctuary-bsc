...
abstract contract Flask_Trade_Abstract is Base, Math, Adressable, AccessControl, With_DAORole, With_UpdaterRole,
                                             With_TradeManagerRole, With_BUSDToken, With_FundContract, With_TradeManagerContract,
                                             With_WithdrawerContract, With_CompetitionContract, I_Flask_Trade_Abstract{
    ////
    ////
    ////
    //////////////// Public variables ////////////////
    ...
    Fraction internal maxStopLoss;
    uint256 internal maxDurationNbBlock;
    ...
    function setInitBUSDBalance(uint256 _initBUSDBalance) public virtual onlyRole(TRADE_MANAGER_ROLE){
        ...
        emit Action(0, block.timestamp, initBUSDBalance, 0, address(0x0), address(0x0));
    }
    ...
    function _endTrade() internal{
        ...
        fund.updateWithdrawFees(balance, initBUSDBalance);
        ...
        emit Action(1, block.timestamp, balance, initBUSDBalance, address(0x0), address(0x0));
    }
...
interface I_Flask_Trade_Swap is I_Initializable, I_With_PancakeRouter, I_Flask_Trade_Abstract{
    struct LimitOrder{
        bool increaseSize;
        uint256 sizeIn;
...
abstract contract Flask_Trade_Swap is Base, Initializable, Flask_Trade_Abstract, With_PancakeRouter, I_Flask_Trade_Swap{
    ////
    ////
    ////
    //////////////// Public variables ////////////////
    ...
    ////
    //////////////// Private variables ////////////////
    uint256 internal constant nbMaxLimitOrders = 10;
    
    I_Flask_Trade_Deployer_Swap internal deployerSwap;
    ...
    //////////////// Constructor & Modifiers ////////////////
    constructor(){
        deployerSwap = I_Flask_Trade_Deployer_Swap(deployer);
    }
    ...
    ////
    //////////////// Public functions ////////////////
    function getRoute() external returns(address[] memory){
        return route;
    }
    
    function initTrade(address _owner, address _manager, address _fund, address _updater, address _withdrawer, address _competition) public onlyRole(DAO_ROLE) virtual override(I_Flask_Trade_Abstract, Flask_Trade_Abstract){
    ...
    function forceEndTrade() public onlyRole(TRADE_MANAGER_ROLE) virtual override(I_Flask_Trade_Abstract, Flask_Trade_Abstract){
        if (isInitialized(0)){
    ...
    function endTrade() public onlyRole(TRADE_MANAGER_ROLE) virtual override(I_Flask_Trade_Abstract, Flask_Trade_Abstract){
        emit Action(102, block.timestamp, 0, 0, address(0x0), address(0x0));
    ...
    function estimateBUSDBalance() public view virtual override(I_Flask_Trade_Abstract, Flask_Trade_Abstract) returns (uint256){
    ...
    function _swap(uint256 sizeIn, address[] storage _route) internal returns(uint256){
        ...
        uint256[] memory amounts = pancakeRouter.swapExactTokensForTokens(sizeIn, sizeOutMin, _route, thisAddr, block.timestamp);
        
        uint256 ammountOut =  amounts[amounts.length-1];
        
        emit Action(101, block.timestamp, sizeIn, ammountOut, _route[0], _route[_route.length-1]);
    ...
    function _swapTo(uint256 sizeOut, address[] storage _route) internal returns(uint256){
        ...
        emit Action(101, block.timestamp, amounts[0], sizeOut, _route[0], _route[_route.length-1]);
    ...
    function _setRoute(address[] memory _route, IERC20 _token) internal{
        ...
        emit Action(100, block.timestamp, 0, 0, address(_token), address(0x0));
        _initialize(0);
    }
...
interface I_Flask_Trade_Long is I_Flask_Trade_Swap{
}

contract Flask_Trade_Long is Base, Flask_Trade_Swap, I_Flask_Trade_Long{
    ////
    ////
    ////
    //////////////// Public variables ////////////////
    ...
    function placeSwapMarketOrder(bool increaseSize, uint256 sizeIn) external onlyRole(OWNER_ROLE) onlyInitialized(0){
        emit Action(300, block.timestamp, sizeIn, increaseSize?1:0, address(0x0), address(0x0));
        if (increaseSize){
            uint256 nbToken = getAvailableBUSD();
            require(sizeIn <= nbToken, "Flask_Trade_Swap: Not enough BUSD");
            
            _swap(sizeIn, route);
        }else{
            uint256 nbToken = token.balanceOf(thisAddr);
            require(sizeIn <= nbToken, "Flask_Trade_Swap: Not enough token");
    ...
    function placeSwapOrder(bool increaseSize, uint256 sizeIn, uint256 triggerPrice, bool isLimit) external onlyRole(OWNER_ROLE) onlyInitialized(0){
        ...
        if (isLimit){
            data += 2;
            data += swapLimitOrders.length*4;
        }else{
            data += swapStopOrders.length*4;
        }
        
        emit Action(301, block.timestamp, sizeIn, triggerPrice, address(uint160(data)), address(token));
        ...
        if (isLimit){
            require(swapLimitOrders.length < nbMaxLimitOrders, "Flask_Trade_Swap: Maximum of 10 orders reached.");
            swapLimitOrders.push(order);
        }else{
            require(swapStopOrders.length < nbMaxLimitOrders, "Flask_Trade_Swap: Maximum of 10 orders reached.");
    ...
    function getSwapLimitOrder(uint256 id) external view returns (LimitOrder memory){
        ...
        require(id < l, "Flask_Trade_Swap: Limit order id not found.");
    ...
    function getSwapStopOrder(uint256 id) external view returns (LimitOrder memory){
        ...
        require(id < l, "Flask_Trade_Swap: Stop order id not found.");
        
        return swapStopOrders[id];
    }
    
    function deleteSwapLimitOrder(uint256 id) external onlyRole(OWNER_ROLE){
        require(id < swapLimitOrders.length, "Flask_Trade_Swap: Limit order id not found.");
        
        emit Action(302, block.timestamp, id, 0, address(0x0), address(0x0));
        
        _deleteOrder(swapLimitOrders, id);
    }
    
    function deleteSwapStopOrder(uint256 id) external onlyRole(OWNER_ROLE){
        require(id < swapStopOrders.length, "Flask_Trade_Swap: Stop order id not found.");
        
        emit Action(303, block.timestamp, id, 0, address(0x0), address(0x0));
        
        _deleteOrder(swapStopOrders, id);
    }
    
    
    function updateNeeded() public view virtual override(I_Flask_Trade_Abstract, Flask_Trade_Abstract) returns (bool){
    ...
    function update() public virtual override(I_Flask_Trade_Abstract, Flask_Trade_Abstract) onlyRole(UPDATER_ROLE){
        super.update();
    ...
    function _updateOneSwapOrder(LimitOrder[] storage arr, LimitOrder storage order, uint256 i, bool isStop) internal{
        if (_oneSwapUpdateNeeded(order, isStop)){
            ...
            if (isStop){
                emit Action(305, block.timestamp, i, 0, address(0x0), address(0x0));
            }else{
                emit Action(304, block.timestamp, i, 0, address(0x0), address(0x0));
            }
    ...
    function _oneSwapUpdateNeeded(LimitOrder storage order, bool isStop) internal view returns (bool){
        uint256 tokenPrice;
        if (order.increaseSize){
            tokenPrice = getTokenPrice(order.sizeIn/10000);
        }else{
            tokenPrice = getTokenPriceFromTokenSize(order.sizeIn/10000);
        }
        
        if ((order.increaseSize && !isStop) || (!order.increaseSize && isStop)){
            return tokenPrice <= order.triggerPrice;
        }else{
            return tokenPrice >= order.triggerPrice;
        }
    }
