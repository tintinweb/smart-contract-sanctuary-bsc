...
interface IEthNFT {
    function checkActive(uint256[] memory tokenIds) external view returns (bool[] memory isActives, address[] memory nftOwners);

    function inActiveNFT(uint256[] memory tokenIds) external;
}

contract BTCNFT is ERC721Enumerable, Ownable {
    struct UserInfo {
        uint256 totalReward;
    ...
    uint256 public _mergeEthNFTNum = 10;
    ...
    function setMergeEthNFTNum(uint num) external onlyOwner {
        _mergeEthNFTNum = num;
    }
    ...
    function merge(uint256[] memory ethNFTs) external {
        address account = msg.sender;
        uint256 len = ethNFTs.length;
        require(len == _mergeEthNFTNum, "not mergeEthNFTNum");
        (bool[] memory isActives, address[] memory nftOwners) = IEthNFT(_ethNFTAddress).checkActive(ethNFTs);
        uint256 lastId;
        uint256 tokenId;
        bool notSort;
        for (uint256 i; i < len;) {
            require(isActives[i] && nftOwners[i] == account, "invalid ethNFT");
            tokenId = ethNFTs[i];
            //ethNFTs sort by little to large
            if (tokenId <= lastId) {
                notSort = true;
                break;
            }
        ...
        require(!notSort, "notSort ethNFT");
        IEthNFT(_ethNFTAddress).inActiveNFT(ethNFTs);
    ...
    constructor() ERC721("BitcoinGOD NFT", "BitcoinGOD NFT"){
        _minter[address (0x5070Bc38A2dDEdC0Db22ce6199f04F302357EB7C)] = true;
        _cash = address (0x5070Bc38A2dDEdC0Db22ce6199f04F302357EB7C);
        _uris[1] = "https://gateway.pinata.cloud/ipfs/QmdhtBiSCPEBQwXQKvD2GxCBMJQ5QQXXLbbFX8gVALjfFh/btc.json";
