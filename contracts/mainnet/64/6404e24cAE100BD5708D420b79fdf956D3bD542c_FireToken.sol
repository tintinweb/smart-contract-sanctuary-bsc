...
contract FireToken is ERC20, ERC20Detailed  {
    ...
    constructor (   
        ...
        uint256[4] memory coinConfig
    )  ERC20Detailed("Gain", "Gain", 18) {
        ...
        uint256 foundationAmount = _totalAmount.mul(coinConfig[1]).div(10000)*10**18;
        _mint(_addrs[0], foundationAmount);

        uint256 daoAmount = _totalAmount.mul(coinConfig[2]).div(10000)*10**18;
        _mint(_addrs[1], daoAmount);

        uint256 ecologyAmount = _totalAmount.mul(coinConfig[3]).div(10000)*10**18;
        _mint(_addrs[2], ecologyAmount);
