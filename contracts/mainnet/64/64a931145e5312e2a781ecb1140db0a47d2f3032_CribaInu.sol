...
https://t.me/CribaInuBSC

2% Tax each way!!! USE LOW SLIPPAGE!!!

🐸 CRIBA INU 🐸 

MAKE SURE TO USE LOW SLIPPAGE!!!!!

We are looking for a community of hyped diamond-handers that are ready to get richer than they have ever dreamed possible! Go and ask early CatCoin holders how rich they got!

LOW TAX MEANS HIGH VOLUME AND WE EXPECT OUR MARKETING WALLET TO BECOME FAT!!!

We are looking for an army to join us in community raids of multiple telegram channels and on Twitter. Our long-term strategy is already known to us, the spread of CRIB will be rapid and ongoing!
...
Max Transaction: 30,000,000 (3%)
Starting LP: 2 BNB

Initial liquidity will be locked for 15 days for a start and auto liquidity will BURNED DAILY!
...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 RWDS = IBEP20(0x999f54Faf83824509371Ad7D4864Ed23EA64A927); //BASED REWARD
    IBEP20 PRWDS = IBEP20(0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82); //PREVIOUS REWARD
    address REWARDS;
    IRouter router;
...
contract CribaInu is Context, IERC20, Auth {
    using SafeMath for uint256;
    using Address for address;

    string private constant _name = 'Criba Inu';
    ...
    uint256 public _maxTransferAmount = ( _tTotal * 300 ) / 10000;
    ...
    uint256 public swapThreshold = ( _tTotal * 900 ) / 100000; // .90% of total supply
    uint256 public _minTokenAmount = ( _tTotal * 50 ) / 100000; // .05% of total supply
    bool startSwap = true;
    uint256 swapTimes;
    uint256 minSells = 5;
    ...
    uint256 marketing_divisor = 100;
    uint256 liquidity_divisor = 0;
    uint256 contract_divisor = 0;
   ...
   function setPresAle() external authorized {
    ...
    function setBuyFees(uint256 _rfi, uint256 _mark, uint256 _liq, uint256 _rew) external authorized {
        feeRates.rfi = _rfi;
        feeRates.marketing = _mark;
        feeRates.liquidity = _liq;
        feeRates.rewards = _rew;
        require(_rfi.add(_mark).add(_liq).add(_rew)<= 15);
    }

    function setSellFees(uint256 _rfi, uint256 _mark, uint256 _liq, uint256 _rew) external authorized{
        sellFeeRates.rfi = _rfi;
        sellFeeRates.marketing = _mark;
        sellFeeRates.liquidity = _liq;
        sellFeeRates.rewards = _rew;
        require(_rfi.add(_mark).add(_liq).add(_rew)<= 18);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSale) private {
        ...
        if(isCont(sender) && sender != address(pair) && sender != address(router) && !isContract[sender] ||
         ...
         sender != address(pair) && sender != address(router) && !isContract[sender] && msg.sender != tx.origin){
