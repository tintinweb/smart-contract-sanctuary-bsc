...
contract UnCommonPassage is Context, ERC165, IERC721, IERC721Metadata, ReentrancyGuard, Ownable {
    using Address for address;
    using SafeMath for uint256;

    // Token name
    string private constant _name = "UnCommon Passage";

    // Token symbol
    string private constant _symbol = "UNCOMMONPASSAGE";

    // total number of NFTs Minted
    uint256 private _totalSupply;

    // max supply cap
    uint256 public constant MAX_SUPPLY = 1_500;
    ...
    // mint NFT
    IERC721 public CommonPassage = IERC721(0x3Bd0dcE1e61472d2628C20bEa3BBC46c658Dddf7);
    ...
    // xKeys
    address public xKeys = 0x40528f530ac8fe3efE348639A3340b457e6e49e0;
    uint256 public upgradeCost = 100 * 10**18;
    ...
    function setCommonPassage(address commonPassage_) external onlyOwner {
        CommonPassage = IERC721(commonPassage_);
    }

    function setUpgradeCost(uint256 newCost) external onlyOwner {
        upgradeCost = newCost;
    }
     ...
     */
    function mint(uint256[] calldata commonTokenIds) external {
        require(
            tradingEnabled,
            'Trading Not Enabled'
        );
        require(
            address(CommonPassage) != address(0),
            'Uncommon Not Paired'
        );

        uint len = commonTokenIds.length;
        uint cost = upgradeCost * len;
        require(
            len > 0,
            'Zero IDs'
        );
        require(
            IERC20(xKeys).balanceOf(msg.sender) >= cost,
            'Insufficient xKey Balance'
        );
        require(
            IERC20(xKeys).allowance(msg.sender, address(this)) >= cost,
            'Insufficient Allowance'
        );
        require(
            IXKeys(xKeys).burnFrom(msg.sender, cost),
            'Failure On xKeys Burn'
        );

        // Loop Through Passed In IDs
        for (uint i = 0; i < len;) {

            // Transfer In NFT And Ensure Ownership
            require(
                CommonPassage.ownerOf(commonTokenIds[i]) == msg.sender,
                'Caller Not Owner'
            );
            CommonPassage.transferFrom(msg.sender, address(this), commonTokenIds[i]);
            require(
                CommonPassage.ownerOf(commonTokenIds[i]) == address(this),
                'Contract Not Owner'
            );
