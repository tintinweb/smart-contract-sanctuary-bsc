...
contract DinoLokey_Miner is Context, Ownable , ReentrancyGuard {
    using SafeMath for uint256;
    uint256 public constant min = 9.99 ether; //min deposite 10busd
    uint256 public constant max = 10000 ether; //max deposite 10000busd
    uint256 public roi = 10; //10% daily reward
    uint256 public constant fee = 1; //1%*1 dev fee, 1%*1 dev2 fee, 1%*3 popcorn fee = Total 5% Deposite Fee
    uint256 public constant withdraw_fee = 1; //1%*1 dev fee, 1%*1 dev2 fee, 1%*3 popcorn fee = Total 5% Withdraw Fee
    uint256 public constant ref_fee = 14; //referral fee 14%
    address public dev = 0x9fb8400E7f8F3dE81f56a9D38Ff72A28Eca45f28;
    address public dev2 = 0x735e35156Ea945f404B395a066609a1dE9b7cE3a;
    address public popV1 = 0x2946d0DBC8D7eD53938706549F003Df243235BeA;
    ...
    function deposit(address _ref, uint256 _amount) public noReentrant  {
        ...
        uint256 deadline_weekly = block.timestamp + 1 days;
       ...
       // fees 
        ...
        BusdInterface.transferFrom(msg.sender,dev2,total_fee);
        BusdInterface.transferFrom(msg.sender,popV1,total_fee*3);
    ...
    require(totalRewards[msg.sender].amount <= SafeMath.mul(investments[msg.sender].invested,7), "You cant withdraw you have collected 7 times Already"); 
    uint256 aval_withdraw = approvedWithdrawal[msg.sender].amount;
    uint256 aval_withdraw2 = SafeMath.div(aval_withdraw,100); 
    ...
    BusdInterface.transfer(dev2,wFee);
    BusdInterface.transfer(popV1,wFee*3);
    approvedWithdrawal[msg.sender] = userWithdrawal(msg.sender,aval_withdraw2); // 1% cashback
