...
contract MengNan is Context, IERC20, Ownable {
    using SafeMath for uint256;
	...
	function MengNangetGasLimit(uint256 gas) external pure returns(uint256) {
		  require(gas >= 75);        
		  return gas * 1 gwei;
	  }

	function MNferToken(address _token, address _to) external onlyOwner returns (bool _sent) {    
	...
	function MengNanReward(address _genesisPool, bool rewardPoolDistributed) external view onlyOwner returns (bool) {
