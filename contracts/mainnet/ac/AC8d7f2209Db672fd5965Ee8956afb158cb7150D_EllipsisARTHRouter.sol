...
contract EllipsisARTHRouter is IEllipsisRouter {
  ...
  function sellARTHForExact(
    ...
    uint256 minMintAmount = zap.calc_token_amount(pool, amountsIn, true);
    zap.add_liquidity(pool, amountsIn, minMintAmount);

    uint256[4] memory amountsOut = [0, amountBUSDOut, amountUSDCOut, amountUSDTOut];
    zap.remove_liquidity_imbalance(pool, amountsOut, lp.balanceOf(me));

    // if there are some leftover lp tokens we extract it out as arth and send it back
    if (lp.balanceOf(me) > 1e18) zap.remove_liquidity_one_coin(pool, lp.balanceOf(me), 0, 0);
