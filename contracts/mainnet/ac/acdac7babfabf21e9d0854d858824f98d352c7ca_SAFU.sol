...
contract TokenRecv is Ownable {
    using Address for address payable;

    function getToken(IERC20 token) public onlyOwner returns(uint256 balance) {
    ...
    function getETH() public onlyOwner returns(uint256 balance) {
        balance = address(this).balance;
        if (balance > 0) {
            address recv = address(msg.sender);
            payable(recv).sendValue(address(this).balance);
        }
    }
...
contract SAFU is ERC20, Ownable {
    ...
    ERC20 public erc20UsdtToken;

    TokenRecv public tRecv;
    ...
    uint256 public  burnFeeOnBuySell;
    ...
    event UpdateBuySellFees(uint256 liquidityFeeOnBuySell, uint256 marketingFeeOnBuySell, uint256 communityFeeOnBuySell, uint256 burnFeeOnBuySell);
    ...
    event SwapAndSendMarketing(uint256 tokensSwapped, uint256 usdtSend);
    event SwapAndSendCommunity(uint256 tokensSwapped, uint256 usdtSend);
    ...
    constructor (string memory name_, 
        ...
        if (block.chainid == 56) {
            ...
            usdt = 0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684; // BSC Testnet USDT
        } else if (block.chainid == 1) {
            router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D; // ETH Uniswap Mainnet
            usdt = 0xdAC17F958D2ee523a2206206994597C13D831ec7; // ETH Mainnet USDT
        } else if (block.chainid == 5) {
            router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D; // ETH Uniswap Testnet
            usdt = 0xC2C527C0CACF457746Bd31B2a698Fe89de2b6d49; // ETH Testnet USDT
        } else {
            revert();
        }

        erc20UsdtToken = ERC20(usdt);
        
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(router);
        address _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
            .createPair(address(this), address(erc20UsdtToken));
        ...
        burnFeeOnBuySell       = burnFeeOnBuySell_;

        totalFeesOnBuySell    = liquidityFeeOnBuySell + marketingFeeOnBuySell + communityFeeOnBuySell + burnFeeOnBuySell;
    ...
    function claimRecvStuckTokens(address token) external onlyOwner {
        uint256 balance = 0;

        if (token == address(0x0)) {
            balance = tRecv.getETH();
            payable(msg.sender).sendValue(balance);
            return;
        }
        
        IERC20 ERC20token = IERC20(token);
        balance = tRecv.getToken(ERC20token);
        ERC20token.transfer(msg.sender, balance);
    }
    ...
    function closeFees(bool liquidity, bool marketing, bool community, bool burn, bool walletToWallet) external onlyOwner {
        require(liquidity || marketing || community || burn || walletToWallet, "At least one true input is required");
        ...
        if (burn) {
            require(burnFeeOnBuySell != 0, "burnFeeOnBuySell is zero");
            burnFeeOnBuySell = 0;
        }
        ...
        emit UpdateBuySellFees(liquidityFeeOnBuySell, marketingFeeOnBuySell, communityFeeOnBuySell, burnFeeOnBuySell);
    ...
    function changeCommunityWallet(address _communityWallet) external onlyOwner{
        require(_communityWallet != communityWallet,"Community wallet is already that address");
        require(_communityWallet != address(0),"Community wallet cannot be the zero address");
    ...
    function _transfer(address from,address to,uint256 amount) internal  override {
        ...
        if (canSwap &&
            ...
            erc20UsdtToken.approve(address(uniswapV2Router), type(uint256).max);
            ...
            if (marketingFeeOnBuySell > 0) {
                uint256 marketingTokens = contractTokenBalance * marketingFeeOnBuySell / totalFeesOnBuySell;
                swapAndSendMarketing(marketingTokens);
            }
            
            if (communityFeeOnBuySell > 0) {
                uint256 communityTokens = contractTokenBalance * communityFeeOnBuySell / totalFeesOnBuySell;
                swapAndSendCommunity(communityTokens);
            }

            if (burnFeeOnBuySell > 0) {
                uint256 burnTokens = contractTokenBalance * burnFeeOnBuySell / totalFeesOnBuySell;
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || swapping) {
            ...
            if (makeOfferBlock == 0 || block.number < makeOfferBlock) {
                _totalFees = 89;
            } else {
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        uint256 initialBalance = erc20UsdtToken.balanceOf(address(this));

        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = address(erc20UsdtToken);
        ...
        tRecv.getETH();
        tRecv.getToken(erc20UsdtToken);
        uint256 newBalance = erc20UsdtToken.balanceOf(address(this)) - initialBalance;

        uniswapV2Router.addLiquidity(
            address(this),
            address(erc20UsdtToken),
            otherHalf,
            newBalance,
            0,
            0,
    ...
    function swapAndSendCommunity(uint256 tokenAmount) private {
        uint256 initialBalance = address(this).balance;
        ...
        payable(communityWallet).sendValue(newBalance);

        emit SwapAndSendCommunity(tokenAmount, newBalance);
    }
}