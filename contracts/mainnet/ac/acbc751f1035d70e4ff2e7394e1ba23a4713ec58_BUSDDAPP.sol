...
contract BUSDDAPP {
    ...
    uint256 constant launch = 1669984200;  // Fri Dec 02 2022: 12.30 PM UTC
    ...
    constructor() {
            owner = address(0x8e6142c1496A56ce2Fb1208b19EcDEEafc935b66);
            dev = address(0x8e6142c1496A56ce2Fb1208b19EcDEEafc935b66);
            PercsKey[1] = DivPercs(86400, 100);
            PercsKey[2] = DivPercs(86400, 200);
            PercsKey[3] = DivPercs(86400, 200);
            PercsKey[4] = DivPercs(86400, 200);
            PercsKey[5] = DivPercs(86400, 10);
    ...
    function calcdiv(address dy) public view returns (uint256 totalWithdrawable) {
        ...
        for (uint256 i = 0; i < user.depoList.length; i++){	
            ...
            if (user.depoList[i].initialWithdrawn == false){
                if (elapsedTime <= PercsKey[1].daysInSeconds){ 
                    uint256 dailyReturn = amount.mul(PercsKey[1].divsPercentage).div(percentdiv);
                    uint256 currentReturn = dailyReturn.mul(elapsedTime).div(PercsKey[1].daysInSeconds / 1);
                    with += currentReturn;
                }
                if (elapsedTime > PercsKey[2].daysInSeconds && elapsedTime <= PercsKey[3].daysInSeconds){
                    uint256 dailyReturn = amount.mul(PercsKey[2].divsPercentage).div(percentdiv);
                    uint256 currentReturn = dailyReturn.mul(elapsedTime).div(PercsKey[1].daysInSeconds / 1);
                    with += currentReturn;
                }
                if (elapsedTime > PercsKey[3].daysInSeconds && elapsedTime <= PercsKey[4].daysInSeconds){
                    uint256 dailyReturn = amount.mul(PercsKey[3].divsPercentage).div(percentdiv);
                    uint256 currentReturn = dailyReturn.mul(elapsedTime).div(PercsKey[1].daysInSeconds / 1);
                    with += currentReturn;
                }
                if (elapsedTime > PercsKey[4].daysInSeconds && elapsedTime <= PercsKey[5].daysInSeconds){
