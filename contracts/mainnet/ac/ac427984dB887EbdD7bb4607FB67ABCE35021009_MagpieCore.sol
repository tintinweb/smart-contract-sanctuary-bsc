    ...
    IMagpieCore
{
    ...
    mapping(address => uint256) private gasFeeAccumulatedByToken;
    mapping(address => mapping(address => uint256)) private gasFeeAccumulated;
    ...
    function _wrapSwap(
        ...
        if (wrapSwapConfig.swap) {
            ...
            amountOuts = IMagpieCore(config.magpieRouterAddress).swap(swapArgs);
            amountOut = amountOuts.sum();
        }

        if (wrapSwapConfig.unwrapToAsset) {
        ...
        returns (uint256[] memory amountOuts)
    {
        ...
        (uint256 amountIn, uint256 transferFee) = bridgeOut(
            payload,
            fromAssetAddress,
        ...
        require(
            swapArgs.getAmountIn() <= amountIn + transferFee,
            "MagpieCore: invalid amountIn"
        );

        require(
            swapArgs.routes[0].amountIn >= transferFee,
            "MagpieCore: transferFee is greater than amountIn"
        );
        ...
        if (payload.destGasTokenAmount > 0) {
            ...
            _wrapSwap(args.gasTokenSwapArgs, gasWrapSwapConfig);
        }
...
contract MagpieBridge is IMagpieBridge {
    ...
    mapping(bytes => bool) private depositHashes;
    ...
    function getRewardAmount(
        ...
        if (bridgeType == BridgeType.Hyphen) {
            return
                IHyphenLiquidityPool(hyphenLiquidityPoolAddress)
                    .getRewardAmount(amount, assetAddress);
        }
        return 0;
    }
    ...
    function bridgeIn(
        ...
        bytes memory payloadOut = bytes.concat(
            ...
            abi.encodePacked(payload.recipientNetworkId, bridgeType)
        );
    ...
    function getPayload(bytes memory encodedVmCore)
        internal
        view
        returns (ValidationOutPayload memory payload, uint64 sequence)
    {
        ...
        ) = IWormholeCore(coreBridgeAddress).parseAndVerifyVM(encodedVmCore);
    ...
    function bridgeOut(
    ...
    ) internal returns (uint256 amount, uint256 transferFee) {
        ...
        if (payload.senderIntermediaryDecimals > receiverIntermediaryDecimals) {
            exponent =
                payload.senderIntermediaryDecimals -
                receiverIntermediaryDecimals;
            amount = payload.amountIn / 10**exponent;
        } else {
            exponent =
                receiverIntermediaryDecimals -
                payload.senderIntermediaryDecimals;
        ...
        if (payload.bridgeType == BridgeType.Wormhole) {
            ...
            require(
                !depositHashes[depositHash],
                "MagpieBridge: already used depositHash"
            );
            (, bool status) = IHyphenLiquidityPool(hyphenLiquidityPoolAddress)
                .checkHashStatus(
                    payload.fromAssetAddress,
                    amount,
                    payload.recipientCoreAddress,
            ...
            depositHashes[depositHash] = true;

            uint256 rewardAmount = payload.senderIntermediaryDecimals >
                receiverIntermediaryDecimals
                ? payload.rewardAmount / 10**exponent
                : payload.rewardAmount * 10**exponent;

            transferFee = IHyphenLiquidityPool(hyphenLiquidityPoolAddress)
                .getTransferFee(assetAddress, amount);

            transferFee = (transferFee / hyphenBaseDivisor) * amount;

            amount += rewardAmount - transferFee;
        }
    }
}
...
library LibBytes {
        ...
        returns (IMagpieBridge.ValidationOutPayload memory payload)
    {
        ...
        payload.rewardAmount = self.toUint256(i);
        i += 32;
