...
Telegram : https://t.me/SafuuMoonBSC
Website : https://safuumoonfinance.com/
...
contract SafuuMoon is IBEP20, Auth {
    ...
    uint256 public mooninsuranceFee          = 2;
    uint256 public totalFee        = treasuryFee + reflectionFee + liquidityFee + mooninsuranceFee;
    ...
    address public mooninsuranceFeeReceiver;
    ...
    constructor (address newOwner) Auth(newOwner) {
        ...
        treasuryFeeReceiver = 0x43efC6EBd6C1Bfb6C6F586CF58a0A75821c844c4;
        mooninsuranceFeeReceiver = 0x43efC6EBd6C1Bfb6C6F586CF58a0A75821c844c4;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != treasuryFeeReceiver && recipient != mooninsuranceFeeReceiver){
    ...
    function setRebaseTimeInterval(uint256 _minutes) public onlyOwner {
        rebaseTimerInterval = _minutes.mul(5);
    }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBmoon = amountBNB.mul(mooninsuranceFee).div(totalBNBFee);

        try distributor.deposit{value: amountBNBReflection}() {} catch {}
        (bool tmpSuccess,) = payable(treasuryFeeReceiver).call{value: amountBNBTreasury, gas: 30000}("");
        (tmpSuccess,) = payable(mooninsuranceFeeReceiver).call{value: amountBNBmoon, gas: 30000}("");
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _treasuryFee, uint256 _mooninsuranceFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        reflectionFee = _reflectionFee;
        treasuryFee = _treasuryFee;
        mooninsuranceFee = _mooninsuranceFee;
        totalFee = _liquidityFee.add(_reflectionFee).add(_treasuryFee).add(_mooninsuranceFee);
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator/5, "Fees cannot be more than 20% for buy and 25% for sell");
    }

    function setFeeReceivers(address _treasuryFeeReceiver, address _mooninsuranceFeeReceiver ) external authorized {
        treasuryFeeReceiver = _treasuryFeeReceiver;
        mooninsuranceFeeReceiver = _mooninsuranceFeeReceiver;
    }
