...
contract RedKing is ERC20, Ownable {
    ...
    uint256 public minInviterAmount;
    mapping(address => address) public inviter;

    uint256 public inviterFee = 7;  //10级分红总比例
    ...
    constructor(
        ...
        minInviterAmount = minInviterAmount_;
        
        tokenRewardsFee = feeSetting_[0];
        liquidityFee = feeSetting_[1];
        marketingFee = feeSetting_[2];
        fundFee = feeSetting_[3];
        deadFee = feeSetting_[4];

        require(tokenRewardsFee.add(liquidityFee).add(marketingFee).add(fundFee).add(deadFee).add(inviterFee) <= 25, "Total fee is over 25%");
   ...
   function setDeadFee(uint256 amount) public onlyOwner {
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            uint256 IFeeCurrent = takeInviter(from,to,amount);
    ...
    function takeInviter(address from, address to, uint256 amount) private returns(uint256)  {
            ...
            if (IFeeAmount > 0){
                    ...
                    // 10级分红。 一代 200 即 2% ， 二代 100 即1%， 后续 50 即0.5%
