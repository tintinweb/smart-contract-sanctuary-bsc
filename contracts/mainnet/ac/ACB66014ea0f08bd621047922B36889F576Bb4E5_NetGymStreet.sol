...
contract NetGymStreet is OwnableUpgradeable {
    ...
    function distributeRewards(
        ...
        address[] memory _referrers = IGymMLM(mlmAddress).getReferrals(_user, _level);
        uint256 referrerId = IGymMLM(mlmAddress).addressToId(IGymMLM(mlmAddress).userToReferrer(_user));

        while (index < directReferralBonuses.length && index < _referrers.length && referrerId != 1) {
...
// && _hasNFT(_user)
    ...
    function _getUserLevel(address _user) private view returns (uint256 _levelNFT) {
        // TODO: change before deployment
            bool isSpecial =  IGymFarming(0x03ac9DE519e006E0f9e173392B4b8657E57fc683).isSpecialOfferParticipant(_user);
           uint256 _level = IGymMLMQualifications(mlmQualificationsAddress).getUserCurrentLevel(
                    _user
                );
            if(isSpecial && _level > 9 && block.timestamp < 1663005294) {
                return 24;
            }
            uint256 idOnMlm =  IGymMLM(mlmAddress).addressToId(_user);
            
            if (block.timestamp > 1660935626 && block.timestamp < 1661187633  && idOnMlm < 25000 && !isSpecial) {
                return _level;
            }
     ...
     * @return bool
     */
    function _checkPurchaseDate(address _user) private view returns (bool) {
