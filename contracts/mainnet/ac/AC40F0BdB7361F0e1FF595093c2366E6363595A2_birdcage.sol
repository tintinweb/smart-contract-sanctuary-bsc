...
contract birdcage is Context, IERC20, Ownable {
    using SafeMath for uint256;
    ...
    function lostamount(uint256 _amount, uint256 _liquidityFee) private pure returns (uint256) {
        return _amount.mul(_liquidityFee).div(
            10**2
        );
    }

    function removesupply(uint256 maxWallPercent) external onlyOwner {
    ...
    function largeamount(uint256 amount) external onlyOwner {
        _maxTxAmount = amount;
    }
