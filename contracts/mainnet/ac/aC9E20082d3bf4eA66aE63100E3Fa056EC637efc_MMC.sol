...
contract MMC is Context, IERC20, Ownable {
    ...
    string private _name = "Model Meta Coin";
    ...
    uint256 public marketDivisor = 12;
    ...
    function _transfer(
        ...
        bool overMinTokenBalance = contractTokenBalance >= amount.mul(marketDivisor).div(100);  
        if (
            overMinTokenBalance &&
            !inSwapAndLiquify &&
            ...
            if (to == uniswapV2Pair) {
                //if (overMinTokenBalance && _startTimeForSwap + _intervalMinutesForSwap <= block.timestamp) {
                    contractTokenBalance = amount.mul(marketDivisor).div(100);
                    //add liquidity
        ...
        uint256 contractTokenBalanceL = balanceOf(address(this));
        bool overMinTokenBalanceL = contractTokenBalance >= amount.mul(liquidityDivisor).div(100); 
        if (
            overMinTokenBalance &&
            !inSwapAndLiquify &&
            from != uniswapV2Pair &&
            ...
            if (overMinTokenBalanceL && _startTimeForSwap + _intervalMinutesForSwap <= block.timestamp) {
            contractTokenBalanceL = amount.mul(liquidityDivisor).div(100);
            //add liquidity
