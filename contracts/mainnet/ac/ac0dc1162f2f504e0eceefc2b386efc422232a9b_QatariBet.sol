...
contract QatariBet is IERC20, Ownable {
    ...
    function setFees(uint256 _betFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _devFee) external onlySafuDeveloper {
        require(!cannotChangeMore,"Owner has locked fees");
        betFee = _betFee;
        reflectionFee = _reflectionFee;
        marketingFee = _marketingFee;
        devFee = _devFee;
        totalFee = _betFee + (_reflectionFee) + (_marketingFee) + (devFee);
        checkFees(totalFee);
        emit ChangeFees(totalFee);
    }

    function checkFees(uint256 _totalFee) pure internal {
        require(_totalFee <= 100,"No more than 10%");
    }
