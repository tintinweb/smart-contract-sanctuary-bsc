...
// File: contracts/TCRDividendPayingToken.sol
pragma solidity ^0.7.6;
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address private DividendToken = address(0);
  ...
  // When `balanceOf(_user)` is changed (via burning/transferring tokens),
...
contract TCR is ERC20, Ownable {
    ...
    TCRDividendTracker public dividendTracker;

    address private burnAddress;
    
    uint256 private maxBuyTransactionAmount = 30000000 * (10**18);
    uint256 private maxSellTransactionAmount = 1000000 * (10**18);
    uint256 public swapTokensAtAmount = 200000 * (10**18);
    uint256 private _maxWalletToken = 150000000 * (10**18); 
    ...
    address payable private smartchainAddress;
    address private marketTokenAddressForFee;
    address private smartchainTokenAddressForFee;
    // transfer fees
    uint256 public marketFee = 0;
    uint256 public smartchainFee = 1;
    uint256 public tokenRewardsFee = 0;
    uint256 public liquidityFee = 0;
    uint256 public totalFees = tokenRewardsFee.add(liquidityFee).add(marketFee).add(smartchainFee);
    // buy fees
    uint256 public buyMarketFee = 0;
    uint256 public buySmartchainFee = 1;
    uint256 public buyTokenRewardsFee = 1;
    uint256 public buyLiquidityFee = 1;
    uint256 public buyTotalFees = buyTokenRewardsFee.add(buyLiquidityFee).add(buyMarketFee).add(buySmartchainFee);
    // sell fees
    uint256 public sellMarketFee = 1;
    uint256 public sellSmartchainFee = 1;
    uint256 public sellTokenRewardsFee = 2;
    uint256 public sellLiquidityFee = 1;
    uint256 public sellTotalFees = sellTokenRewardsFee.add(sellLiquidityFee).add(sellMarketFee).add(sellSmartchainFee);
    // fee tracker
    uint256 private previousMarketFee = marketFee;
    uint256 private previousSmartchainFee = smartchainFee;
    ...
    uint256 private processDividendTime;
    uint256 private dividendTime = 3600;
    
    bool private isTransferFeesDisabled = false;
    ...
    uint256 public tradingEnabledTimestamp = 1666873092;

    // blacklisted from all transfers
    mapping (address => bool) private _isBlacklisted;
    // Transfer Limiter    
    mapping (address => uint256) public _transferTimeLog;    
    mapping (address => uint256) public _transferAmountLog;
    uint256 public _transferTimeLimit = 86400;
    uint256 public _transferAmountLimit = 150000 * (10**18);
    uint256 public _transferOverLimitTax = 5;    
    ...
    function restoreFees() internal {
            ...
            smartchainFee = previousSmartchainFee;
    ...
    function updateTransferLimits(uint256 transferTimeLimit_,uint256 transferAmountLimit_,uint256 transferOverLimitTax_) external onlyOwner {
        _transferTimeLimit = transferTimeLimit_;
        _transferAmountLimit = transferAmountLimit_;
        _transferOverLimitTax = transferOverLimitTax_;
    }      
    ...
    function updateSmartchainAddress(address payable newAddress) external onlyOwner {
        smartchainAddress = newAddress;
        _isExcludedFromFees[newAddress] = true;
        //dividendTracker.excludeFromDividends(smartchainAddress);
    }       

    function updateFees(uint256 _tokenRewardsFee, uint256 _liquidityFee, uint256 _marketFee, uint256 _smartchainFee) external onlyOwner {
        tokenRewardsFee = _tokenRewardsFee;
        liquidityFee = _liquidityFee;
        marketFee = _marketFee;
        smartchainFee = _smartchainFee;
        totalFees = _tokenRewardsFee.add(_liquidityFee).add(_marketFee).add(_smartchainFee);
        previousMarketFee = _marketFee;
        previousSmartchainFee = _smartchainFee;
        previousTokenRewardsFee = _tokenRewardsFee;
        previousLiquidityFee = _liquidityFee;
        previousTotalFees = _tokenRewardsFee.add(_liquidityFee).add(_marketFee).add(_smartchainFee);
    }    
    
    function updateBuyFees(uint256 _tokenRewardsFee, uint256 _liquidityFee, uint256 _marketFee, uint256 _smartchainFee) external onlyOwner {
        buyTokenRewardsFee = _tokenRewardsFee;
        buyLiquidityFee = _liquidityFee;
        buyMarketFee = _marketFee;
        buySmartchainFee = _smartchainFee;
        buyTotalFees = _tokenRewardsFee.add(_liquidityFee).add(_marketFee).add(_smartchainFee);
    }    
    
    function updateSellFees(uint256 _tokenRewardsFee, uint256 _liquidityFee, uint256 _marketFee, uint256 _smartchainFee) external onlyOwner {
        sellTokenRewardsFee = _tokenRewardsFee;
        sellLiquidityFee = _liquidityFee;
        sellMarketFee = _marketFee;
        sellSmartchainFee = _smartchainFee;
        sellTotalFees = _tokenRewardsFee.add(_liquidityFee).add(_marketFee).add(_smartchainFee);
    }
    //
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "TCR: The dividend tracker already has that address");

        TCRDividendTracker newDividendTracker = TCRDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "TCR: The new dividend tracker must be owned by the TCR token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "TCR: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        //require(_isExcludedFromFees[account] != excluded, "TCR: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "TCR: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "TCR: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "TCR: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "TCR: Cannot update gasForProcessing to same value");
    ...
    function swapAndLiquify(uint256 tokens) private  {
        ...
        uint256 smartchainFeeFull;
        
        if(marketFee != 0){
            marketFeeFull = tokens.mul(marketFee).div(100);
        }
        if(smartchainFee != 0){
            smartchainFeeFull = tokens.mul(smartchainFee).div(100);
        }

        uint256 tokensAfterFee = tokens - marketFeeFull - smartchainFeeFull;
        ...
        uint256 smartchainFeeBalance;

        if(marketFee != 0){
            marketFeeBalance = newBalance.mul(marketFee).div(100);
        }
        if(smartchainFee != 0){
            smartchainFeeBalance = newBalance.mul(smartchainFee).div(100);
        }
        
        uint256 finalBalance = newBalance - marketFeeBalance - smartchainFeeBalance;
        ...
        if(smartchainFeeBalance != 0){        
        if(smartchainTokenAddressForFee != address(0)){
            swapEthForTokens(smartchainFeeBalance, smartchainTokenAddressForFee, smartchainAddress);
        }else{
            (bool sent,) = smartchainAddress.call{value: smartchainFeeBalance}("");
        }}
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "TCR: This account cannot send tokens until trading is enabled");
        }
        ...
        if( 
            ...
            smartchainFee = sellSmartchainFee;
        ...
        if(_isExcludedFromFees[from] || _isExcludedFromFees[to] || isTransferFeesDisabled) {
            takeFee = false;
                ...
                // transfer log for limit handler
        _transferAmountLog[from] += amount;

        if(!isBuy && takeFee){
            if(amount > _transferAmountLimit){
                _transferTimeLog[from] = block.timestamp.add(_transferTimeLimit);
                uint256 _overLimitAmount = amount.sub(_transferAmountLimit);
                uint256 _overLimitFee = _overLimitAmount.mul((totalFees.add(_transferOverLimitTax))).div(100);
                amount = amount.sub(_overLimitAmount);
                super._transfer(from, address(this), _overLimitFee);
                super._transfer(from, address(this), (_overLimitAmount.sub(_overLimitFee)));
            }else{
                if(_transferTimeLog[from] < block.timestamp){
                    _transferTimeLog[from] = block.timestamp.add(_transferTimeLimit);
                    _transferAmountLog[from] = amount;    
                }else{
                    if(_transferAmountLog[from] > _transferAmountLimit){
                        takeFee = false;
                        uint256 _overLimitFee = amount.mul((totalFees.add(_transferOverLimitTax))).div(100);
                        amount = amount.sub(_overLimitFee);
                        super._transfer(from, address(this), _overLimitFee);
                    }
                }
            }
...
contract TCRDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("TCR_Dividend_Tracker", "TCR_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "TCR_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "TCR_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TCR contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "TCR_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "TCR_Dividend_Tracker: Cannot update claimWait to same value");
