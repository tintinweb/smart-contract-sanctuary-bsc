...
contract PoolManager is Ownable {
    ...
    function addPools(address[] calldata _pools, bool _isLegacy) public onlyOwner {
        if (_isLegacy) {
    ...
    function removePools(address[] calldata _pools, bool _isLegacy) public onlyOwner {
        if (_isLegacy) {
            for (uint i = 0; i < _pools.length; i++) { 
    ...
    function viewTotalHoldings(address userAddress, address stakingToken) external view returns (uint256) {
        ...
        for (uint256 i = 0; i < activePools.length; i++) { 
            ...
            if (legacyPoolList.contains(activePools[i])) {
                if (pool.stakeToken() == stakingToken) {
                ...
                if (pool.STAKE_TOKEN() == stakingToken) {
                    IRewardPool.UserInfo memory userInfo = pool.userInfo(userAddress);
                    uint256 stakedAmount = userInfo.amount;
                    stakedBalance += stakedAmount;
                }
