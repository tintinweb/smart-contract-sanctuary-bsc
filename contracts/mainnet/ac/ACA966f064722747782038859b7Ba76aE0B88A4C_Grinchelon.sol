...
contract Grinchelon is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  constructor(string memory name, string memory symbol, string memory baseTokenURI) ERC721(name, symbol) {
      ...
      // Save dev wallet
      _wallet = 0x5D5f0c3e15dD763774Ed285f669Adc7ee3567eeD;
  }
  ...
  function mint(uint256 _mintAmount) public payable {
    // Checks
    require(!paused, "GrinchElonTest: minting has not started yet");
    require(_mintAmount > 0, "GrinchElonTest: you have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "GrinchElonTest: you can only mint 10 at a time");
    require(lastSupply >= _mintAmount,  "GrinchElonTest: the collection is sold out");
    if (msg.sender != owner()) {
      require(msg.value >= cost * _mintAmount, "GrinchElonTest: total cost isn't match");
    }
    // Minting
  ...
  function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
    // Checks
    require(_winner != address(0), "GrinchElonTest: zero address");
    require(_amount > 0, "GrinchElonTest: you have to mint at least one");
    require(lastSupply != 0,  "GrinchElonTest: the collection is sold out");
  ...
  function tokenURI(uint256 _id) public view virtual override returns (string memory){
    require(_exists(_id),"GrinchElonTest: URI query for nonexistent token");
