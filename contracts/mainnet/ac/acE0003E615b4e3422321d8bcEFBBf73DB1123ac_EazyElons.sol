...
import "./IEazyElons.sol";

contract EazyElons is IEazyElons, Ownable, ERC721A {
    enum SaleStates {
        CLOSED,
        PUBLIC
    }
    ...
    constructor() ERC721A("EazyElons", "EElons") {}
        ...
        checkState(SaleStates.PUBLIC)
    {
        ...
        if (quantity > mintMax) revert WalletLimitExceeded();
    ...
    /// @param newPrice Newly intended `publicPrice` value.
    function changePrice(uint256 newPrice) external onlyOwner {
        price = newPrice;
    }

    /// @notice Function used to change max mint.
    /// @param newMintMax Newly intended `mintMax` value.
    function changeMintMax(uint256 newMintMax) external onlyOwner {
        mintMax = newMintMax;
    }
...
interface IEazyElons is IERC721A {
