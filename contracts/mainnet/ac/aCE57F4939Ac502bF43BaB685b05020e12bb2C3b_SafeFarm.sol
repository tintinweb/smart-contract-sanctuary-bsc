...
contract SafeFarm is ReentrancyGuard, AccessControl {
    ...
    */
    ...
    function _depositLP(
        ...
        LPToken memory lpt0 = _swapLPToken(unirouter, amountHalf, _route0);
        LPToken memory lpt1 = _swapLPToken(unirouter, (_amount - amountHalf), _route1);
        ...
        _amount = _addLpLiquidity(_depositor, unirouter, lpt0, lpt1);

        earn();
        uint256 _after = strategy.balanceOfPool();
        uint256 shares = (_after - _pool); // Additional check for deflationary tokens
        if (shares > _amount) {
            shares = _amount;
        }
