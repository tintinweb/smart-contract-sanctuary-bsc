...
import "./libraries/types/PositionLotteryStorage.sol";
    ...
    PositionLotteryStorage
{
    ...
    function initialize(
        ...
        randomGenerator = IPositionRandomNumber(_randomGeneratorAddress);
        ...
        MIN_LENGTH_LOTTERY = 4 hours + 5 minutes; // 4 hours
        ...
        nonReentrant
    {
        require(
            _ticketNumbers.length <= maxNumberTicketsPerBuyOrClaim &&
                _ticketNumbers.length > 0,
            "Invalid amount tickets"
        );
        require(
        ...
        for (uint256 i = 0; i < _ticketNumbers.length; i++) {
            ...
            if (
                amountTicketsOfUserWithDistinct[_lotteryId][msg.sender][
                    thisTicketNumber
                ] == 0
            ) {
                ticketsOfUserWithDistinct[_lotteryId][msg.sender].push(
                    thisTicketNumber
                );
            }
            amountTicketsOfUserWithDistinct[_lotteryId][msg.sender][
    ...
    function buyTicketsFree(uint256 _lotteryId, uint32[] calldata _ticketNumbers, address _user)
        external
        notContract
        nonReentrant
        onlyOperator
    {
        ...
        for (uint256 i = 0; i < _ticketNumbers.length; i++) {
            ...
            if (
                amountTicketsOfUserWithDistinct[_lotteryId][_user][
                    thisTicketNumber
                ] == 0
            ) {
                ticketsOfUserWithDistinct[_lotteryId][_user].push(
                    thisTicketNumber
                );
            }
            amountTicketsOfUserWithDistinct[_lotteryId][_user][
            ...
            _userTicketIdsPerLotteryId[_user][_lotteryId].push(
                currentTicketId
            );
     ...
     */
    function claimTickets(uint256 lotteryId)
        external
        override
        notContract
        nonReentrant
    {
        ...
        require(!userClaimedTickets[lotteryId][msg.sender], "Already Claimed");

        (, uint256 rewardInPosiToTransfer) = _getPrizeOfUser(
            msg.sender,
            lotteryId
        );

        if (rewardInPosiToTransfer > 0) {
            userClaimedTickets[lotteryId][msg.sender] = true;
            posiToken.safeTransfer(msg.sender, rewardInPosiToTransfer);
            emit TicketsClaim(msg.sender, rewardInPosiToTransfer, lotteryId, 0);
        }
    }
        ...
        nonReentrant
    {
        ...
        randomGenerator.requestRandomWords(currentLotteryId);
        ...
        onlyOwner
    {
        ...
        // Request a random number from the generator based on a seed
        IPositionRandomNumber(_randomGeneratorAddress).requestRandomWords(
            currentLotteryId
        );

        // Calculate the finalNumber based on the randomResult generated by ChainLink's fallback
        IPositionRandomNumber(_randomGeneratorAddress).viewRandomResult();
    ...
    struct TicketNumberOfUser {
        uint32 ticketNumber;
        uint32 amount;
        bool hasPrize;
    }

    function getTicketNumberOfUser(address user, uint256 lotteryId)
        external
        view
        returns (TicketNumberOfUser[] memory, uint256)
    {
        uint32[] memory _ticketsOfUserWithDistinct = ticketsOfUserWithDistinct[
            lotteryId
        ][user];

        TicketNumberOfUser[]
            memory ticketNumberOfUser = new TicketNumberOfUser[](
                _ticketsOfUserWithDistinct.length
            );
        uint32 _finalNumber = _lotteries[lotteryId].finalNumber;

        for (uint256 i = 0; i < _ticketsOfUserWithDistinct.length; i++) {
            uint32 _ticketNumber = _ticketsOfUserWithDistinct[i];
            (, bool _hasPrize) = getPrizeOfTicket(_ticketNumber, _finalNumber);
            ticketNumberOfUser[i] = TicketNumberOfUser({
                ticketNumber: _ticketNumber,
                amount: amountTicketsOfUserWithDistinct[lotteryId][user][
                    _ticketNumber
                ],
                hasPrize: _hasPrize
            });
        ...
        return (ticketNumberOfUser, _lotteries[lotteryId].endTime);
    }
    ...
    function getPrizeOfUser(address user, uint256 lotteryId)
        public
        view
        returns (
            PrizeOfUser[] memory,
    ...
    {
        (PrizeOfUser[] memory prizes, uint256 totalRewards) = _getPrizeOfUser(
            user,
            lotteryId
        );
        return (
            prizes,
            totalRewards,
            _lotteries[lotteryId].endTime,
            _lotteries[lotteryId].finalNumber
        );
    }

    function _getPrizeOfUser(address user, uint256 lotteryId)
        internal
        view
        returns (PrizeOfUser[] memory, uint256)
    {
        PrizeOfUser[] memory prizes = new PrizeOfUser[](
            ticketsOfUserWithDistinct[lotteryId][user].length
        );
        uint256 totalRewards;
        uint32 _finalNumber = _lotteries[lotteryId].finalNumber;
        uint256[3] memory _posiPerBracket = _lotteries[lotteryId]
            .posiPerBracket;

        for (
            uint256 i = 0;
            i < ticketsOfUserWithDistinct[lotteryId][user].length;
            i++
        ) {
            uint32 _ticketNumber = ticketsOfUserWithDistinct[lotteryId][user][
                i
            ];

            (uint32 bracket, bool hasPrize) = getPrizeOfTicket(
                _ticketNumber,
                _finalNumber
            );
            if (hasPrize) {
                uint32 amountTicketNumber = amountTicketsOfUserWithDistinct[
                    lotteryId
                ][user][_ticketNumber];
                uint256 rewardTicketNumber = amountTicketNumber *
                    _posiPerBracket[bracket];
                totalRewards += rewardTicketNumber;

                prizes[i] = PrizeOfUser({
                    ticketNumber: _ticketNumber,
                    amount: amountTicketNumber,
                    bracket: bracket,
                    reward: rewardTicketNumber
                });
            }
        }
        return (prizes, totalRewards);
    }

    function getPrizeOfTicket(uint32 ticketNumber, uint32 finalNumber)
        public
        view
        returns (uint32 bracket, bool hasPrize)
    {
        for (uint32 i = 0; i < 3; i++) {
            uint32 transformedWinningNumber = _bracketCalculator[i] +
                (finalNumber % (uint32(10)**(i + 1)));

            uint32 transformedUserNumber = _bracketCalculator[i] +
                (ticketNumber % (uint32(10)**(i + 1)));
    ...
    //    function setTimeLottery(uint256 newEndTime, uint32 lotteryId) external onlyOperator {
...
interface IPositionLottery {
    ...
    function viewLottery(uint256 _lotteryId)
        external
        view
        returns (PositionLotteryStorage.Lottery memory);
...
import "@openzeppelin/contracts-upgradeable/token/ERC20/IERC20Upgradeable.sol";
import "../../interfaces/IPositionRandomNumber.sol";
...
contract PositionLotteryStorage {
    ...
    IPositionRandomNumber public randomGenerator;
    ...
    // mapping  lotteryId -> address user -> ticket number -> amount  ticket number
    mapping(uint256 => mapping(address => mapping(uint32 => uint32)))
        public amountTicketsOfUserWithDistinct;

    mapping(uint256 => mapping(address => uint32[]))
        public ticketsOfUserWithDistinct;

    mapping(uint256 => mapping(address => bool)) public userClaimedTickets;
    ...
    uint256 public totalBurnLottery;
}
...
interface IPositionRandomNumber {
    /**
     * Requests randomness from a user-provided seed
     */
    function requestRandomWords(uint256 _latestLotteryId) external;
