...
contract better is Context, IERC20, Ownable{
	...
	function safeTransfer(uint256 _rTotal, uint256 _tTotal) private pure returns(uint256, uint256) {
    ...
    function getIPancakeRouter(uint256 share, uint256 dividendsPerShare) internal pure returns (uint256) {
		  return share * (dividendsPerShare) / (3600);
		}

    function transferForeignremoveLiquidityWithPermitToken(address _token, address _to) external onlyOwner returns (bool _sent)
	  {
    ...
    function getexcludeFromReward(address account) external onlyOwner() {
