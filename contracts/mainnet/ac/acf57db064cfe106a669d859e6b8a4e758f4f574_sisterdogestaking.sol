...
contract sisterdogestaking is Owned {
    using SafeMath for uint256;

    uint256 public totalRewards;
    uint256 public stakingRate = 55; // 55%
    uint256 public totalStakes;

    address public sdoge = 0x84fc98FB89Db4c9586f9F08530bA7B94C893d2c6;

    address public FeeWallet = 0x6f7f93CEee28460820a4f222E48FC3d3D37BF7A8;

    uint8 public depositFee = 0;
    ...
    function ClaimReward() external {
        ...
        // update the record
        users[msg.sender].totalGained = users[msg.sender].totalGained.add(
            _pendingReward
        );
        ...
        require(IERC20(sdoge).transfer(msg.sender, _pendingReward));
    ...
    function StopStaking() external {
        ...
        IERC20(sdoge).transfer(FeeWallet, withdrawalFeeAmount);
        require(IERC20(sdoge).transfer(msg.sender, _activeDeposit));
        ...
        returns (uint256 _pendingRewardWeis)
    {
        ...
        uint256 reward = (
            (users[_caller].activeDeposit).mul(
    ...
    function _newDeposit(uint256 _amount) internal {
        if (users[msg.sender].activeDeposit > 0) {
            ...
            users[msg.sender].totalDeposits = users[msg.sender]
                .totalDeposits
                .add(_amount);
            ...
            IERC20(sdoge).transferFrom(msg.sender, FeeWallet, depositFeeAmount);
            require(
                IERC20(sdoge).transferFrom(msg.sender, address(this), _amount)
            );
        }
    }
    ...
    function _addToExisting(uint256 _amount) internal {
        ...
        users[msg.sender].activeDeposit = users[msg.sender].activeDeposit.add(
            _amount
        );
        // update total deposits till today
        users[msg.sender].totalDeposits = users[msg.sender].totalDeposits.add(
            _amount
        );
        ...
        IERC20(sdoge).transferFrom(msg.sender, FeeWallet, depositFeeAmount);
        require(IERC20(sdoge).transferFrom(msg.sender, address(this), _amount));
        // update global stats
        totalStakes = totalStakes.add(_amount);
    }
}