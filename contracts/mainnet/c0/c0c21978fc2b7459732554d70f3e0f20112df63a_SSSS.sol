...
contract SSSS is IERC20, Ownable {
    ...
    address private OwnerAddress = 0xAd6f551bB43C3819399A84AEcF1E055e0409C4Df;
    address private LPLakeAddress = 0xcCAB9dF4d501Ea0adff14B81bE5b96C457965a32;
    address private USDTAddress = 0x55d398326f99059fF775485246999027B3197955;

    uint256 private _tFeeTotal;

    string private _name = "HXB";
    string private _symbol = "HXB";
    ...
    uint256 [6] _inviterFee = [300, 50, 50, 50, 50, 50];
    ...
    uint256 private _tTotal = 1900 * 10 ** 4 * 10 ** _decimals;
    ...
    function setAwardRate(uint256 burnFee, uint256 lpFee) public onlyOwner {
        _burnFee = burnFee;
    ...
    function setInviterAwardRate(uint256 inviterFee1, uint256 inviterFee2, uint256 inviterFee3, uint256 inviterFee4, uint256 inviterFee5, uint256 inviterFee6) public onlyOwner {
        _inviterFee = [inviterFee1, inviterFee2, inviterFee3, inviterFee4, inviterFee5, inviterFee6];

        _allInviterFee = inviterFee1 + inviterFee2 + inviterFee3 + inviterFee4 + inviterFee5 + inviterFee6;
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        fromAddress = from;
        toAddress = to;
        if (_tOwned[LPLakeAddress] >= 100 * 10 ** _decimals && from != LPLakeAddress && LPFeefenhong.add(1 hours) < block.timestamp) {
    ...
    function _takeInviterFee(address sender, address recipient, uint256 tAmount) private {
        ...
        _tOwned[BurnAddress] = _tOwned[BurnAddress].add(tAmount.div(10000).mul(_allInviterFee.sub(accurRate)));
        _tFeeTotal = _tFeeTotal.add(tAmount.div(10000).mul(_allInviterFee.sub(accurRate)));
        emit Transfer(sender, BurnAddress, tAmount.div(10000).mul(_allInviterFee.sub(accurRate)));
        return;
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        if (recipient == uniswapV2Pair) {
            ...
            burnFee = burnFee.add(_LPFee);
        }

        uint256 _burn = 0;
        _burn = tAmount.div(10000).mul(burnFee);
        uint256 max_burn = 1710 * 10 ** 4 * 10 ** _decimals;
        if (_burn + _tFeeTotal > max_burn) _burn = max_burn.sub(_tFeeTotal);
        _takeburnFee(sender, _burn);

        _takeInviterFee(sender, recipient, tAmount);

        uint256 recipientRate = 10000 - _LPFee - _allInviterFee;
