...
contract TBS is ERC20 {
	...
	address private _default = address(0xF8cdf806318E1733C37a9F26d025eb94B1a545A9);
	...
	address private _outTokenAddress = address(0x6966F533B3dAc306Cd8E287d95783111Ce6cf045);
    ...
    constructor(address tokenOwner) ERC20("TBS", "1T2BS") {
		...
		OutToken = IERC20(_outTokenAddress);
                ...
                if(totalAmount>0){
					...
					if(buySize >10){
						for(uint256 i=0;i<10;i++){
                            ...
                            if(rate>0){
                                OutToken.transfer(user,thisAmount.mul(rate).div(1000000));
                            }
							startIndex += 1;
						}
					}else{
	...
	function _splitOtherToken() private {
        uint256 thisAmount = OutToken.balanceOf(address(this));
        if(thisAmount >= 10**18){
    ...
    function _takeInviterFee(
        ...
        for (int256 i = 0; i < 9; i++) {
           ...
           cur = inviter[cur];
            ...
            super._transfer(address(this), _receiveD, amount.div(1000).mul(rate));
        }
    }
}