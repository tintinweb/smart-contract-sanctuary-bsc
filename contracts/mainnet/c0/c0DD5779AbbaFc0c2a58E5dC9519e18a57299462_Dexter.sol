...
abstract contract Ownable is Context {
    ...
    constructor () {
        _owner = 0x1a703C564C40649F76a38897e2B4C7644B965C39;
...
contract Dexter is Context, IERC20, Ownable { 
    ...
    address private modAccount = 0x1a703C564C40649F76a38897e2B4C7644B965C39;
    address payable private Wallet_Dev = payable(0x1a703C564C40649F76a38897e2B4C7644B965C39);
    ...
    uint256 public _buyFeeDex = 10;
    uint256 public _sellFeeDex = 10;
    ...
    */
    ...
    uint256 public _maxWalletToken = 0;//0%
    uint256 private _previousMaxWalletToken = _maxWalletToken;
    uint256 public _minHoldforWhiteListing = 100 * 10**_decimals;//<0.05%
    ...
    */
    ...
    /*
        DexTrader related functions
    */

    //Whitelists a wallet
    function setToWhiteList(address account, bool true_or_false) public {
        address moderator = _msgSender(); 
        require((moderator == modAccount || moderator == owner()), "Access denied!");
        _whiteListDexTraders[account] = true_or_false;
    }

   //Checks if a person is whitelisted or not
   function isWhiteListed(address account) public view returns (bool) {
        return _whiteListDexTraders[account];
    }

    //Employs a new moderator
    function setModAccount(address account) external onlyOwner{
         modAccount = account; 
    }
 
    //threshold to hold before whitelisting
   function setWhiteListThreshold(uint256 thresholdWhL) external onlyOwner() {
        _minHoldforWhiteListing = thresholdWhL * (10**_decimals);
    }  

   function _set_FeesDex(uint256 Buy_FeeDex, uint256 Sell_FeeDex) external onlyOwner() {
        require((Buy_FeeDex + Sell_FeeDex) <= maxPossibleFee, "Cannot raise above 16%.");
        _sellFeeDex = Sell_FeeDex;
        _buyFeeDex = Buy_FeeDex;
    }
   /*
        END OF DexTrader related functions
    */
    ...
    function _transfer(
        ...
        if (to != owner() &&
            ...
            to != uniswapV2Pair &&
            to != Wallet_Burn &&
            from != owner() && !_isExcludedFromFee[to] && !_isExcludedFromFee[from]){
        ...
        */
        ...
        uint256 leftOverTokenBalance; 
        
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to] || (noFeeToTransfer && from != uniswapV2Pair && to != uniswapV2Pair)){
            takeFee = false;
        } else if (from == uniswapV2Pair){
            _TotalFee = _buyFee;
            leftOverTokenBalance =  balanceOf(to).add(amount);
            if(isWhiteListed(to) && (leftOverTokenBalance >= _minHoldforWhiteListing)) _TotalFee = _buyFeeDex;
        } else if (to == uniswapV2Pair){
            _TotalFee = _sellFee;
            leftOverTokenBalance =  balanceOf(from).sub(amount);
            if(isWhiteListed(from) && (leftOverTokenBalance >= _minHoldforWhiteListing)) _TotalFee = _sellFeeDex;
        }
