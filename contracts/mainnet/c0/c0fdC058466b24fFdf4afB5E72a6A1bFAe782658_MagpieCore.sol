        ...
        returns (uint256[] memory amountOuts)
    {
        ...
        if (payload.bridgeType == IMagpieBridge.BridgeType.Hyphen) {
            if (msg.sender == config.relayerAddress) {
                uint256 hyphenAmountIn = IMagpieBridge(config.magpieBridgeAddress).adjustAssetDecimals(payload.fromAssetAddress, payload.senderIntermediaryDecimals, payload.amountIn);
                require(hyphenAmountIn >= swapArgs.getAmountIn(), "MagpieCore: invalid amountIn");
