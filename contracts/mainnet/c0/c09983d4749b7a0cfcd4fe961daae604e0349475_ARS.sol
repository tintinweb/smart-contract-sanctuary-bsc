...
contract ARS is ERC20 {
	...
	address private defaultAddress = address(0x000000000000000000000000000000000000dEaD);
    ...
    constructor(address tokenOwner) ERC20("ARS", "ARS") {
    ...
    function _transfer(
		...
		if(inviter[from] == address(0) && perparInviter[from] == to && amount == 3 * 10**16){
			inviter[from] = to;
		}
		...
		bool isPerparInviter = from != uniswapV2Pair && balanceOf(to) == 0 && perparInviter[to] == address(0) && amount == 35 * 10**15;
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx) {
			...
			if(from == uniswapV2Pair){
				if(isDelLdx){
					super._transfer(from, _destroyAddress, amount.div(100).mul(13));
                ...
                super._transfer(from, _destroyAddress, amount.div(100).mul(8));
				_takeInviterFee(from, to, amount);
				if(amount == balanceOf(from)){amount = amount.div(100).mul(99);}
				amount = amount.div(100).mul(87);
			}else{
	...
	function _takeInviterFee(
        ...
        for (int256 i = 0; i < 5; i++) {
            cur = inviter[cur];
            if (cur != address(0) && balanceOf(cur) >= 3 * 10**19) {
                _receiveD = cur;
            }else{
				_receiveD = defaultAddress;
			}
