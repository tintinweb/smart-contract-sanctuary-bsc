...
contract MainstreetLottery is Ownable, ReentrancyGuard, VRFConsumerBase {

    address     public  MONEY_MONKEYS;
    ...
    constructor(
        ...
        bytes32 _keyHash,
        uint256 _linkFee,
        address moneyMonkeys,
        address mainstreet
    ) VRFConsumerBase(_vrfCoordinator, _linkToken) {
        MONEY_MONKEYS = moneyMonkeys;
        MAINST = mainstreet;
        keyHash = _keyHash;
        linkFee = _linkFee;
    }
    ...
    function withdrawMainstTokens() public onlyOwner {
    ...
    function fulfillRandomness(bytes32 _requestId, uint256 _randomNumber) internal override {
        ...
        RANDOM_TOKEN_ID = (_randomNumber % (IMinter(MONEY_MONKEYS).TOKEN_ID()));
    }

    function reward(uint amount) public onlyOwner {
        if (IMinter(MONEY_MONKEYS).ownerOf(RANDOM_TOKEN_ID) != address(0)) {
            IBEP20(MAINST).transfer(IMinter(MONEY_MONKEYS).ownerOf(RANDOM_TOKEN_ID), amount);
