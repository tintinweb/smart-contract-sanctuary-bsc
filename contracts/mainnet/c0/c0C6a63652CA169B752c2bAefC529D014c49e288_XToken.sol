...
contract XToken {
    ...
    function permit(
        ...
        require(deadline >= block.timestamp, "X-Swap Token: EXPIRED");
        bytes32 digest =
            keccak256(
            abi.encodePacked(
                "\x19\x01",
        ...
        require(
            ...
            "X-Swap Token: INVALID_SIGNATURE"
        );
...
contract XTokenFactory is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize(address _xTokenContract) public initializer {
        xTokenContract = _xTokenContract;
        __Ownable_init();
    ...
    function createXToken(address _baseToken) external returns(address) {

        require(baseToken[_baseToken] == address(0), "xToken exists");
        require(xToken[_baseToken] == address(0), "baseToken exists");
        ...
        return _createXToken(_baseToken, _name, _symbol, _decimals);
    }
    ...
    function _createXToken(address _baseToken, string memory _name, string memory _symbol, uint8 _decimals) internal returns(address _xToken) {
        bytes32 salt = _salt(_baseToken);
        // clone cantract
        bytes20 targetBytes = bytes20(xTokenContract);
        assembly {
            let clone := mload(0x40)
            mstore(clone, 0x3d602d80600a3d3981f3363d3d373d3d3d363d73000000000000000000000000)
            mstore(add(clone, 0x14), targetBytes)
            mstore(add(clone, 0x28), 0x5af43d82803e903d91602b57fd5bf30000000000000000000000000000000000)
            _xToken := create2(0, clone, 0x37, salt)
        }

        IXToken(_xToken).init(_name, _symbol, _decimals, _baseToken);

        baseToken[_baseToken] = _xToken;
        xToken[_xToken] = _baseToken;

        emit CreateXToken(_xToken, _baseToken);
    }

    function mint(address _xToken, address _to, uint _amount) external onlyOwner {
        IXToken(_xToken).mint(_to, _amount);
    }

    function burn(address _xToken, uint _amount) external {
    ...
    function deposit(address _token, address _to, uint _amount) external {
        ...
        address _xToken = baseToken[_token];
        _token.safeTransferFrom(sender, address(this), _amount);
        IXToken(_xToken).mint(_to, _amount);
    }

    function withdraw(address _xToken, address _to, uint _amount) external {
        address sender = _msgSender();
        address _token = xToken[_xToken];
        IXToken(_xToken).burnFrom(sender, _amount);
