...
TG:     https://t.me/elephoge_portal
WEB:    www.elephoge.cc

TOKENOMICS:
Supply: 50,000,000 tokens

0.5% max sell Transaction, 2% max Wallet size
...
contract ELEPHOGE is Context, IERC20, Ownable {
    ...
    string private constant _name = "ELEPHOGE";
    string private constant _symbol = "ELEPHOGE";
    ...
    uint256 public restrictionPeriod = 1800;
    mapping (address => uint256) public accountBuys;
    ...
    feeRatesStruct public buyRates =
        feeRatesStruct({
            rfi: 15, // RFI rate,
            team: 5, // team fee
            marketing: 0, // marketing fee
            lp: 20, // lp rate
            toSwap: 25 // marketing + team + lp
        });
    ...
    feeRatesStruct public sellRatesJeet =
        feeRatesStruct({
            rfi: 0, // RFI rate
            team: 50, // team fee
            marketing: 350, // marketing
            lp: 0, // lp rate
            toSwap: 400 // marketing + team + lp
        });
    ...
    event MarketingAndTeamFeesAdded(uint256 teamFee, uint256 marketingFee);
    ...
    event AddLastPeriodBuyInfo(uint256 timestamp, uint256 amount);
    ...
    constructor() {
        ...
        marketingAddress = payable(0xf9558dA81CD53c8dbe4947b49c6bECE654b66906);
        teamAddress = payable(0x50c10A9Aa2D25Cff8699628E6c750904151E87a3);
        ...
        returns (valuesFromGetValues memory s)
    {
        ...
        s.tRfi = (tAmount * appliedRates.rfi) / feeDominator;
        s.tToSwap = (tAmount * appliedRates.toSwap) / feeDominator;
    ...
    function _transfer(
        ...
        if (takeFee) {
            if (isAutomatedMarketMakerPair[from]) {
                if ((block.timestamp < launchedAt + timeToWait) && isTeamMember[to] != true && isDiamondHand[to] != true) {
                    isBot[to] = true;
                    emit BlacklistedUser(to, true);
                    appliedRates = buyRatesSniper;
                }
                else if(tradingIsEnabled != true && isTeamMember[to] != true && isDiamondHand[to] != true) {
                    isBot[to] = true;
                    emit BlacklistedUser(to, true);
                    appliedRates = buyRatesSniper;
                }
                ...
                accountBuys[to] = block.timestamp;
                ...
                if(block.timestamp - accountBuys[from] <= restrictionPeriod && isTeamMember[from] != true) {
                  appliedRates = sellRatesJeet;
                }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 denominator = appliedRates.toSwap * 100 / feeDominator * 2;
        uint256 tokensToAddLiquidityWith = (contractTokenBalance *
            appliedRates.lp * 100 / feeDominator) / denominator;
        ...
        uint256 ETHToAddLiquidityWith = (deltaBalance * appliedRates.lp * 100 / feeDominator) /
            (denominator - appliedRates.lp * 100 / feeDominator);

        // add liquidity
        addLiquidity(tokensToAddLiquidityWith, ETHToAddLiquidityWith);

        // we give the remaining tax to team & marketing wallets
        uint256 remainingBalance = address(this).balance;
        uint256 teamFee = (remainingBalance * appliedRates.team* 100 / feeDominator) /
            (denominator - appliedRates.team* 100 / feeDominator);
        uint256 marketingFee = (remainingBalance * appliedRates.marketing * 100 / feeDominator) /
            (denominator - appliedRates.marketing * 100 / feeDominator);
        teamAddress.sendValue(teamFee);
    ...
    function setBuyFees(
        ...
        buyRates.team = _team;
        buyRates.lp = _lp;
        buyRates.toSwap = _marketing + _team + _lp;

        buyRatesSniper.rfi = _rfiSniper;
        buyRatesSniper.marketing = _marketingSniper;
        buyRatesSniper.team = _teamSniper;
        buyRatesSniper.lp = _lpSniper;
        buyRatesSniper.toSwap = _marketingSniper + _teamSniper + _lpSniper;        
    }
    ...
    function setSellFees(
        ...
        sellRates.team = _team;
        sellRates.lp = _lp;
        sellRates.toSwap = _marketing + _team + _lp;

        sellRatesSniper.rfi = _rfiSniper;
        sellRatesSniper.marketing = _marketingSniper;
        sellRatesSniper.team = _teamSniper;
        sellRatesSniper.lp = _lpSniper;
        sellRatesSniper.toSwap = _marketingSniper + _teamSniper + _lpSniper;      
    }

    function setSellJeetFees(
        uint8 _rfi,
        uint8 _marketing,
        uint8 _team,
        uint8 _lp
    ) external teamMember {
        sellRatesJeet.rfi = _rfi;
        sellRatesJeet.marketing = _marketing;
        sellRatesJeet.team = _team;
        sellRatesJeet.lp = _lp;
        sellRatesJeet.toSwap = _marketing + _team + _lp;
    }
    ...
    function rescueBNB(uint256 weiAmount) external onlyOwner{
        ...
        teamAddress.sendValue(weiAmount);
    }
    ...
    function addDiamondHand(address walletAddress, bool diamondHand) external onlyOwner {
        isDiamondHand[walletAddress] = diamondHand;
    }   
    ...
    function getAccountBuys (address account) public view returns (uint256) {
        return (accountBuys[account]);
    }

    function setRestrictionPeriod(uint256 _newPeriodHours)  external teamMember {
        emit SetRestrictionPeriod(restrictionPeriod, _newPeriodHours);
        restrictionPeriod = _newPeriodHours;
    }

    function setTradingIsEnabled(bool _enabled) public teamMember {
