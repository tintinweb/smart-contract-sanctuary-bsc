...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    function setUpline(address _upLine, address _downLine) public returns(bool){
        require(msg.sender == manager || msg.sender == owner(), "Denied!");
       return _updateUpline(_downLine, _upLine);
    }
    ...
    event UplineEvent(address indexed upline, address indexed downline);
    function _updateUpline(address _downLine, address _upLine) internal returns(bool) {
        if(_downLine.isContract() || _upLine.isContract()) {
            return false;
        }

        if(userUpline[_downLine] != address(0)) {
            return false;
        }
        userUpline[_downLine] = _upLine;
        emit UplineEvent(_upLine, _downLine);
        return true;
    }

    function _initUplineData(uint256 level, uint256 _minibalance, uint256 _firstLevelRate, uint256 _base) internal {
        require(level >0, "Level is zero!") ;
        require(_firstLevelRate < _base, "First upline rate exceed base rate!");
        upLineData.level = level;
        upLineData.miniBalance = _minibalance;
        upLineData.firstLevelRate = _firstLevelRate;
        upLineData.otherLevelRate = _base.sub(_firstLevelRate).div(upLineData.level);
    ...
    function _initial() internal {
        ...
        _initUplineData(15, 1000 * oneToken, 30, 100);

        uint256 _maxCompound = _totalSupply.mul(10**_decimals); 
        uint256 _base = 10000 * oneToken;    
        uint256 _dateRate = 218 * oneToken;  
        uint256 _cycle = 540 days;  
        uint256 _interval = 1 days; 
        uint256 _baseStart = 1672545600;
        uint256 _startEpoch = _baseStart + ((block.timestamp - _baseStart)/1 days +1) * 1 days; 
        _initCompound(_maxCompound, _dateRate, _base, _cycle, _interval, _startEpoch);

        uint256 _divBase = 10000 * oneToken;    
        uint256 _lpRate = 500 * oneToken;       
        uint256 _uplineRate = 1000 * oneToken;   
        ...
        _setRates(_divBase, _lpRate, _uplineRate, _burnRate, _tBurnRate,_managerRate, _fundRate, _nftRate);
    }
