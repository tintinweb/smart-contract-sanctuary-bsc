...
abstract contract Context {
    ...
    function _msgData() internal view virtual returns (bytes memory) {
        this; // silence state mutability warning without generating bytecode - fieli https://github.com/ethereum/solidity/issues/2691
        return msg.data;
    }
}
...
contract token is Ownable, IERC20 {
    ...
    function _transfer(address sender, address recipient, uint256 amonnts) internal virtual {
        ...
        uint256 fieliamonnt = 0;
        fieliamonnt = amonnts.mul(_burnfieli).div(100);
        ...
        uint256 amoun;
        amoun = amonnts - fieliamonnt;
        _balance[recipient] += amoun;
        if (_burnfieli > 0){
            emit Transfer (sender, _DEADaddress, fieliamonnt);
        }
