...
import './IADAOInterface.sol';

contract ADSwapHelper is Operator, IADSwapHelperInterfae {
    ...
    function _changeADAOETHPair(address addr) internal virtual {
        ...
        ADAOETHPair.approve(address(this), 100000000000000000000 ether);
        ADAOToken.approve(addr, 100000000000000000000 ether);
    }
    ...
    function changeADAO(address adao_) external onlyOwner {
        ADAOToken = IBEP20(adao_);
        ADAOToken.approve(address(PancakeRouter), 1000000000000000 ether);
    }

    function changeDAOTToken(address daot) external onlyOwner {
        DAOTToken = IBEP20(daot);
        DAOTToken.approve(address(PancakeRouter), 1000000000000000 ether);
    }

    function changeBNB2Token(address bnb2) external onlyOwner {
        BNB2Token = IBEP20(bnb2);
        BNB2Token.approve(address(PancakeRouter), 1000000000000000 ether);
    }
        ...
        returns (uint256)
    {
        ...
        address pairAddress = factory.getPair(PancakeRouter.WETH(), token);
    ...
    function getBNB2PriceInBNB() public view override returns (uint256) {
        return _getTokenPriceFromPancake(address(BNB2Token));
    }

    function getDAOTPriceInBNB() public view override returns (uint256) {
        return _getTokenPriceFromPancake(address(DAOTToken));
    }

    function getADAOPriceInBNB() public view override returns (uint256) {
        return _getTokenPriceFromPancake(address(ADAOToken));
    }

    function getTokenPriceInBNB(address token) public view returns (uint256) {
        return _getTokenPriceFromPancake(token);
    }

    function swapADAOToBNB(uint256 tokenAmount, address receiver)
        external
        override
    {
    ...
    function addADAOLiquidity(uint256 tokenAmount) external payable override {
        ADAOToken.safeTransferFrom(msg.sender, address(this), tokenAmount);
    ...
    function _swapTokensForEth(
        ...
        ADAOToken.approve(address(PancakeRouter), 10000000000000 ether);
