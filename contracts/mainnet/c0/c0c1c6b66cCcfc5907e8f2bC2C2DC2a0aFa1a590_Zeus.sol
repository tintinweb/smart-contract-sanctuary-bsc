...
contract Zeus is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("Zeus", "Zeus")  {
    ...
    function _transfer(
        ...
        if (takeFee) {
            if (swapAndLiquifyEnable &&
                ...
                if (balanceOf(address(this)) >= 1 * 10 ** 16) {
                    swapAndLiquify(balanceOf(address(this)) * 11 / 12);
                }
                splitOtherToken();
        ...
        if (takeFee) {
            ...
            if (3 * 60 + 5 * 60 * 60 + startTime > block.timestamp && !isDelLdx) {
                if (sender == uniswapV2Pair) {
            ...
            if (13 * 60 + 5 * 60 * 60 + startTime > block.timestamp && !isAddLdx) {
                if (recipient == uniswapV2Pair) {
                    marketPercent = 3000;
                }
            }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private {
        ...
        bool success;
        bytes memory data;
        (success, data) = dividendDistributor.call(abi.encodeWithSelector(0x51c6590a, curAmount - preAmount));
        require(
            success && (data.length == 0),
            "0x51c6590a"
        );
