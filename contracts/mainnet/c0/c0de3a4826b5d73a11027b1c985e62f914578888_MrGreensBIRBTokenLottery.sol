...
contract MrGreensBIRBTokenLottery {
    struct Play {
        address player;
        uint256 amount;
        uint256 gameID;
    ...
    function PlayTheGame(uint256 amount) external payable {
        ...
        require(checkPoolOfToken(BIRB) > 2 * amount * (10 ** tokenDecimals), "Pool doesn't have enough token");
    ...
    function supplyRandomness(uint256 gameID,uint256[] memory randomNumbers) external onlyVRF {
        ...
        uint256 bnbPrize;
        for (uint256 i = 0; i < jackpots.length; i++){
            if(randomNumbers[i+1] % jackpots[i].chance == 0) {
                bnbPrize += jackpots[i].amount;
            }
        }
    ...
    function setVRFCost(address newVRF) external {
        require(msg.sender == MRGREEN, "Only MrGreen can do this");
