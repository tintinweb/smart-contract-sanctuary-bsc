...
contract EverEscrow is ERC20, Ownable {
    using Address for address;
    using Address for address payable;

    uint256 public  rewardFee      = 1;
    uint256 public  marketingFee   = 4;
    uint256 public  totalFees      = rewardFee + marketingFee;

    address public marketingWallet = 0x4D772E99fc68c7db6F9d6c8bdAba001915f51292;
    ...
    event UpdateBuyFees( uint256 marketingFee, uint256 rewardFee);
    ...
    constructor() ERC20("EverEscrow Coin", "EEC") 
    {

        _mint(owner(), 1_000_000 * (10 ** 8));
    ...
    function updateFees(uint256 _marketingFee, uint256 _rewardFee) external onlyOwner {
        marketingFee = _marketingFee;
        rewardFee    = _rewardFee;

        totalFees = _rewardFee + _marketingFee;
        
        require(totalFees <= 12, "Fees cannot exceed 12%");
        emit UpdateBuyFees(marketingFee, rewardFee);
    }
    ...
    function changeMarketingWallet(address _marketingWallet) external onlyOwner {
        ...
        require(!isContract(_marketingWallet), "Marketing wallet cannot be a contract address");
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            uint256 marketingShare = (marketingFee * newBalance) / totalFees;
            
            if (marketingShare > 0){
                // send marketing wallet
                payable(marketingWallet).sendValue(marketingShare);
            }
            
            uint256 rewardsShare = address(this).balance - initialBalance;

            if (rewardsShare > 0){
                // send rewards
                swapAndSendDividends(rewardsShare); 
            }

            swapping = false;           
        }

        uint256 _totalFeesMarketing;
        uint256 _totalFeesReward;

        if((_isExcludedFromFees[from] || _isExcludedFromFees[to]) || swapping) {
            _totalFeesMarketing = 0;
            _totalFeesReward = 0;
        }else if(from == uniswapV2Pair) {
            _totalFeesMarketing = marketingFee;
            _totalFeesReward = rewardFee;
        ...
        uint256 _totalFees = _totalFeesMarketing + _totalFeesReward;
