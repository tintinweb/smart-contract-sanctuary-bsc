...
contract VotingEscrow is IVotingEscrow, ApolloxERC721EnumerableUpgradeable, UUPSUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable{
    ...
    function configForcedWithdraw(bool canForcedWithdraw) external onlyRole(DEV_OPERATOR){
    ...
    function configMinRatio(uint256 minRatio) external onlyRole(DEV_OPERATOR) {
        require(minRatio <= _maxRatio, "E1");
        _minRatio = minRatio;
    }

    function configMaxRatio(uint256 maxRatio) external onlyRole(DEV_OPERATOR) {
        require(maxRatio < MULTIPLIER, "E1");
        require(maxRatio >= _minRatio, "E2");
        _maxRatio = maxRatio;
    }

    function configPenaltyK(uint256 penaltyK) external onlyRole(DEV_OPERATOR) {
        require(penaltyK <= 10*MULTIPLIER, "E1");
        _penaltyK = penaltyK;
    }

    function configPenaltyAddress(address penaltyAddress) external onlyRole(DEV_OPERATOR) {
