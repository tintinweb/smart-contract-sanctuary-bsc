...
contract ANCHStakePool is LPTokenWrapper{
    ...
    mapping(address => uint256) public inviteLPAmount;
    ...
    mapping(address => uint256) public stakeUserIndexes;


    mapping(address => uint256) public stakeLPRewards;
    mapping(address => uint256) public stakeLPRewardsWithdrawed;
    mapping(address => uint256) public inviteRewards;
    mapping(address => uint256) public inviteRewardsWithdrawed;

    mapping(address => uint256[]) public newMillionRewardTimes;
    mapping(address => uint256[]) public newMillionRewardReleaseTotalAmount;
    mapping(address => uint256) public newMillionRewardWithdrawed;
    ...
    constructor (IERC20 _token, IERC20 _lpToken, IERC20 _usdtToken, uint256 _starttime) public{
      ...
      userInfo[owner()].isStake = true;
      userInfo[owner()].referrer = address(this);
    ...
    function deposit(uint256 amount) public  checkStart{ 
        ...
        if(!hasStakeUser(msg.sender)) {
            addStakeUser(msg.sender);
        }
        ...
        for(uint8 i=0; i<5; i++) {
            if(myRef != address(0) && myRef != address(this)) {
                inviteLPAmount[myRef] = inviteLPAmount[myRef].add(amount);
        ...
        if(totalUSDTAmount >= newMillionUSDTAmount) {
            ...
            if(newMillionTotalReleased.add(newMillionReleaseAmount) <= MAX_RELEASE_AMOUNT) {
                ...
                for(uint256 i=0; i<stakeUsers.length; i++) {
                    ...
                    newMillionRewardTimes[user].push(block.timestamp);
                    newMillionRewardReleaseTotalAmount[user].push(newMillionReleaseAmount.mul(userInfo[user].amount).div(totalSupply()));

                    
                }

                newMillionTotalReleased = newMillionTotalReleased.add(newMillionReleaseAmount);
            }
    ...
    function addStakeUser(address user) internal {
        stakeUserIndexes[user] = stakeUsers.length;
        stakeUsers.push(user);
    }

    function removeStakeUser(address user) internal {
        stakeUsers[stakeUserIndexes[user]] = stakeUsers[stakeUsers.length - 1];
        stakeUserIndexes[stakeUsers[stakeUsers.length - 1]] = stakeUserIndexes[user];
        stakeUsers.pop();
    }

    function hasStakeUser(address user) private view returns(bool){
        for(uint256 i=0; i<stakeUsers.length; i++) {
            if(user == stakeUsers[i])
                return true;
        }
        ...
        if(block.timestamp.sub(lastRebaseTime) >= dayPeriod) {
            ...
            if(rewardTokenAmount > 0 && totalRewarded.add(rewardTokenAmount) <= MAX_REWARD_AMOUNT) {
                ...
                for(uint256 i=0; i<stakeUsers.length; i++) {
                    ...
                    if(totalSupply() > 0)
                        stakeLPRewards[user] = stakeLPRewards[user].add(stakeReward.mul(userInfo[user].amount).div(totalSupply()));

                    if(totalInviteLPAmount > 0)
                        inviteRewards[user] = inviteRewards[user].add(inviteReward.mul(inviteLPAmount[user]).div(totalInviteLPAmount));
                }

                
                safeTransfer(address(token), rewardTokenAmount.mul(20).div(100));
    ...
    function earn() public checkStart {
        ...
        (uint256 totalReward,uint256 stakeLPReward , uint256 inviteReward, uint256 newMillionReward) = getReward(msg.sender);
        ...
        if(stakeLPReward > 0)
            stakeLPRewardsWithdrawed[msg.sender] = stakeLPRewardsWithdrawed[msg.sender].add(stakeLPReward);

        if(inviteReward > 0)
            inviteRewardsWithdrawed[msg.sender] = inviteRewardsWithdrawed[msg.sender].add(inviteReward);

        if(newMillionReward > 0)
            newMillionRewardWithdrawed[msg.sender] = newMillionRewardWithdrawed[msg.sender].add(newMillionReward);
    ...
    // rewardType 0: stakeLPReward, 1: inviteReward, 2:newMillionReward
    function earn(uint256 rewardType) public checkStart {

        (,uint256 stakeLPReward , uint256 inviteReward, uint256 newMillionReward) = getReward(msg.sender);
        ...
        if(rewardType == 0) {
            ...
            require(newMillionReward > 0, "Cannot earn 0");

            safeTransfer(msg.sender, newMillionReward);

            newMillionRewardWithdrawed[msg.sender] = newMillionRewardWithdrawed[msg.sender].add(newMillionReward);

            emit Earn(msg.sender, newMillionReward, rewardType);
        }
    ...
    function getReward(address account) public view returns (uint256, uint256, uint256, uint256){

        uint256 newMillionReward =  getNewMillionReward(account);
        uint256 stakeLPReward = stakeLPRewards[account].sub(stakeLPRewardsWithdrawed[account]);
        uint256 inviteReward = inviteRewards[account].sub(inviteRewardsWithdrawed[account]);

        return (stakeLPReward.add(inviteReward).add(newMillionReward), stakeLPReward, inviteReward, newMillionReward);
    }
    

    function getNewMillionReward(address account) public view returns (uint256){

        uint256 newMillionReward;

        for(uint256 i=0; i<newMillionRewardTimes[account].length; i++) {

            newMillionReward = newMillionReward.add(newMillionRewardReleaseTotalAmount[account][i].mul(10).div(100));
            if(block.timestamp.sub(newMillionRewardTimes[account][i]) >= dayPeriod.mul(30)) {
                newMillionReward = newMillionReward.add(newMillionRewardReleaseTotalAmount[account][i].mul(90).div(100));
            } else {
                uint256 time = (block.timestamp.sub(newMillionRewardTimes[account][i])).div(dayPeriod);
                newMillionReward = newMillionReward.add(newMillionRewardReleaseTotalAmount[account][i].mul(time.mul(3)).div(100));
            }
        }

        if(newMillionRewardWithdrawed[account] > 0) {
            newMillionReward = newMillionReward.sub(newMillionRewardWithdrawed[account]);
        }

        return newMillionReward;
    }


    function getRewardWithdrawed(address account) public view returns (uint256, uint256, uint256, uint256){

        uint256 newMillionReward =  newMillionRewardWithdrawed[account];
        uint256 stakeLPReward = stakeLPRewardsWithdrawed[account];
        uint256 inviteReward = inviteRewardsWithdrawed[account];

        return (stakeLPReward.add(inviteReward).add(newMillionReward), stakeLPReward, inviteReward, newMillionReward);
    }

    function getTotalReward(address account) public view returns (uint256, uint256, uint256, uint256){

        uint256 newMillionReward =  newMillionRewardWithdrawed[account].add(getNewMillionReward(account));
        uint256 stakeLPReward = stakeLPRewards[account];
        uint256 inviteReward = inviteRewards[account];
    ...
    function setDayPeriod(uint256 _dayPeriod) public onlyOwner {
