...
contract Ninneko is Initializable, ERC721Upgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    uint256 public ninoAltarFee;
    uint256 public mataAltarFee;
    ...
    event AltarPet(uint256 indexed petId, address indexed user, uint256 ninoFee, uint256 mataFee);
    ...
    function initialize(
        ...
        serviceFee = 500;
        rebornFee = 150 * 10**18;
        breedNINOFee = 150 * 10**18;
        minPriceSalePet = 1 * 10**16;
        breedCosts = [12500 * 10**18, 18000 * 10**18, 24500 * 10**18, 32000 * 10**18, 40500 * 10**18, 60500 * 10**18];
        allowBreed = false;
    ...
    function setAltarFee(uint256 _ninoFee, uint256 _mataFee) external onlyOwner {
        ninoAltarFee = _ninoFee;
        mataAltarFee = _mataFee;
    }
    ...
    function altarPet(uint256 _petId) external whenNotPaused nonReentrant onlyPetOwner(_petId) notInBlackList(_petId) {
        burn(_petId);
        NINOToken.safeTransferFrom(msg.sender, ninoReceiver, ninoAltarFee);
        MATAToken.safeTransferFrom(msg.sender, address(0xdEaD), mataAltarFee);
        emit AltarPet(_petId, msg.sender, ninoAltarFee, mataAltarFee);
    }
}
