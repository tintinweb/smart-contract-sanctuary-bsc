...
contract ITEM_V1 is AccessControlUpgradeable, ERC721EnumerableUpgradeable, OwnableUpgradeable {
    ...
    function _spawnAxie(uint256 _genes, address _owner) private returns (uint256 _axieId) {
        Axie memory _axie = Axie(_genes, block.timestamp);
        axies.push(_axie);
        _axieId = axies.length - 1;
        _safeMint(_owner, _axieId);
