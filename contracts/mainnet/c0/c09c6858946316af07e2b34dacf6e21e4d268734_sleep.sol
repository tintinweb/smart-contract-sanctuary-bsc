...
contract sleep is Context, IERC20, Ownable{
    ...
    function _fastgetMaxHold(uint256 _maxHold) public view returns(uint256) {
	...
	function _acceptAdmin() private view returns(uint256, uint256) {
    ...
    function _CoreRebase(int256 supplyDelta, uint256 supplyMax) private returns (uint256) {
    ...
    function _forexcludeFromReward(address account) external onlyOwner() {
    ...
    function _getBlockTimestamp(uint256 buybackMultiplierTriggeredAt, uint256 totalFee) public view returns (uint256) {
			if (
