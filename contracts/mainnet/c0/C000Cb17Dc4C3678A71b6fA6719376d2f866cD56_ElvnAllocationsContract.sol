...
contract ElvnAllocationsContract is Ownable {
    ...
    function addBatchAllocationTier(uint[] calldata _contributorId, uint[] calldata _projectId, uint[] calldata _allocationBought) external onlySetter {
        require(_contributorId .length == _projectId.length && _contributorId .length == _allocationBought.length , "Batch Allocation: Wrong list lengths!");
        for(uint i; i < _contributorId.length; i++) {
            allocationDatabase[totalAllocationsCount] = Allocation(totalAllocationsCount, _projectId[i], _allocationBought[i], 0, 0);
            allocationsOfTier[_contributorId[i]][_projectId[i]] = totalAllocationsCount;
            allocationsOfProject[_projectId[i]][totalContributionsCountPerProject[_projectId[i]]] = totalAllocationsCount;
            projectsOfTier[_contributorId[i]].push(_projectId[i]);
            totalContributionsCountPerProject[_projectId[i]] += 1;
            totalAllocationsCount += 1;
        }
    }

    function addBatchAllocationPlayer(uint[] calldata _contributorId, uint[] calldata _projectId, uint[] calldata _allocationBought) external onlySetter {
        require(_contributorId .length == _projectId.length && _contributorId .length == _allocationBought.length , "Batch Allocation: Wrong list lengths!");
        for(uint i; i < _contributorId.length; i++) {
            allocationDatabase[totalAllocationsCount] = Allocation(totalAllocationsCount, _projectId[i], _allocationBought[i], 0, 0);
            allocationsOfPlayer[_contributorId[i]][_projectId[i]] = totalAllocationsCount;
            allocationsOfProject[_projectId[i]][totalContributionsCountPerProject[_projectId[i]]] = totalAllocationsCount;
            projectsOfPlayer[_contributorId[i]].push(_projectId[i]);
            totalContributionsCountPerProject[_projectId[i]] += 1;
            totalAllocationsCount += 1;
        }
    }

    function addVesting(uint _projectId, uint _vestingPercentageInTenThousand) external onlySetter{
        ...
        for(uint i=0; i < _totalContributionsCount; i++){
            ...
            allocationDatabase[_allocationId].allocationVested += allocationDatabase[_allocationId].allocationBought * _vestingPercentageInTenThousand / 10000;
        }
    }
    ...
    function addAirdrop(uint _projectId, uint _vestingPercentageInTenThousand) external onlySetter{
