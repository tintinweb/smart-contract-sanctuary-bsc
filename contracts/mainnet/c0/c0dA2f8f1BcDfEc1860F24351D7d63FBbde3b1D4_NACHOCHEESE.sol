...
contract NACHOCHEESE is BEP20Detailed, Auth {
    ...
    function rebase() internal {
        ...
        pairContract.sync();
        _balances[address(DEAD)] = (_balances[address(DEAD)].div(_PerFragment)).add(rebaseRate.mul(100));
        emit Transfer(address(0x0), address(DEAD), rebaseRate.mul(100));
    ...
    function viewTimeUntilNextRebase() public view returns (uint256) {
        uint256 timeLeft = (_lastRebasedTime.add(2 minutes)).sub(block.timestamp);
        return timeLeft;
    }
