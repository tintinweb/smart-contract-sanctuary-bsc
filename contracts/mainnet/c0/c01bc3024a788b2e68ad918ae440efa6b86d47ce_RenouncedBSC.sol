...
/**
 ...
 Twitter : https://twitter.com/RenouncedBSC

 Website : http://renouncedbsc.tech/

 Taxes 0% Buy & 10% Sell
 Max wallet 3% or 3 million
...
contract RenouncedBSC is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    RenouncedBSCDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Renounced BSC", "RENOUNCED") {
        marketingWallet = payable(0x98d825Ea0D831305fcf65Bd10675e710AcDC8Ee1);
        buybackWallet = payable(0x631Df73dB01533a6E35f06f36ACC231D9B596baa);
        ...
        dividendTracker = new RenouncedBSCDividendTracker(
            payable(this),
            router,
        ...
        maxWallet = totalTokenSupply / 300; // 3%
    ...
    // set cooldown timer, can only be between 0 and 300 seconds (5 minutes max)
    ...
    function updateFees(
        ...
        require(totalSellFees <= 20 && totalBuyFees <= 0, "total Fees cannot exceed 0% BUY 20% SELL");
...
contract RenouncedBSCDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    RenouncedBSC public RenouncedBSCContract;
    ...
    constructor(
        ...
        defaultToken = token;
        RenouncedBSCContract = RenouncedBSC(mainContract);
        minimumTokenBalanceForAutoDividends = 100000_000000000000000000; // 100,000 tokens
    ...
    function _transfer(
        ...
        require(false, "RenouncedBSC_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "RenouncedBSC_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main RenouncedBSC contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(RenouncedBSCContract);

            uint256 prevBalance = RenouncedBSCContract.balanceOf(address(this));
            ...
            {
                uint256 received = RenouncedBSCContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    RenouncedBSCContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
