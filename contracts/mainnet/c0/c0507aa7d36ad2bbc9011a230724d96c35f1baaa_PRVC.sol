 ...
 */
contract PRVC is Ownable , IERC20, IERC20Metadata {

    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _totalSupply = 500000000000000*(10**decimals());
    ...
    uint _devTeamTaxFee = 2; // 0.002 %
    uint _burnTaxFee = 3; // 0.003 %
    ...
    function calculateDevTaxFee(uint _amount) internal view  returns (uint256) {
        return (_amount*(_devTeamTaxFee)/(100000));
    }

    function calculateBurnTaxFee(uint _amount) internal view  returns (uint256) {
        
        uint maxBurnFee = _totalSupply*9999988/10000000;
    ...
    function _transfer(
        ...
        _afterTokenTransfer(from, to, (amount-taxFee));
    }
    ...
    function setDevTeamWallet(address _account) external onlyOwner {

      require(_account != address(0),"invalid address");

      _devTeamWallet=_account;

    }

    function ExculdeUserFromFee(address _account) external onlyOwner {
    ...
    function IncludeUserFromFee(address _account) external onlyOwner {
