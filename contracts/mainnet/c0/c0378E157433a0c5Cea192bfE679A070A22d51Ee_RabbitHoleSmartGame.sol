...
/*Start main uints*/
address private owner=0x80322873b8e931A5501805725aBcf80b3C7df1F7; 
...
bool private isNextLVLsOpen =false;
...
/*Start Events*/
event BuyLvl(address indexed _addr, uint indexed lvl);
event BuyLvlForRef(address indexed from, address indexed referalWallet, uint indexed lvl2buy);
event CreateReferal(address indexed ref_addr, uint indexed id);
/*End events*/
...
TimeRegistration1[Marketing]=block.timestamp;
 TimeRegistration2[Marketing]=block.timestamp;
 TimeRegistration3[Marketing]=block.timestamp;
 TimeRegistration4[Marketing]=block.timestamp;
 TimeRegistration5[Marketing]=block.timestamp;
 TimeRegistration6[Marketing]=block.timestamp;
 isHolder1[Marketing]=true;
 isHolder2[Marketing]=true;
 isHolder3[Marketing]=true;
 isHolder4[Marketing]=true;
 isHolder5[Marketing]=true;
 isHolder6[Marketing]=true;
 addReferal(Marketing);
CurrentPosition1[Marketing]=1;
CurrentPosition2[Marketing]=1;
CurrentPosition3[Marketing]=1;
CurrentPosition4[Marketing]=1;
CurrentPosition5[Marketing]=1;
CurrentPosition6[Marketing]=1;
}
...
function addReferal(address addr) public isReferalAlready(addr) returns (uint)  {  
 ...
 emit CreateReferal(addr,referalID);
 referalID++;
 isReferal[addr]=true;
 return referalID-1;
 }
function startGame() public isOwner(){
   isStarted=true;
}
function stopGame() public isOwner(){
   isStarted=false;
}
function OpenNextLVLs() public isOwner(){
   isNextLVLsOpen=true;
}
...
function getPaymant1 (uint ID) public payable isHolder_1(msg.sender) minimumSend1(msg.sender,msg.value){
   require (isStarted==true, "The game is not started");  
 _CreateUser1(msg.sender,ID); 
    emit BuyLvl(msg.sender,1);
    emit BuyLvlForRef(msg.sender,referals_connect[msg.sender],1);
...
function getPaymant2 () public payable isHolder_2(msg.sender) minimumSend2(msg.sender,msg.value){
 ...
 totalBNB+=msg.value;
emit BuyLvl(msg.sender,2);
    emit BuyLvlForRef(msg.sender,referals_connect[msg.sender],2);
...
totalBNB+=msg.value;
emit BuyLvl(msg.sender,3);
    emit BuyLvlForRef(msg.sender,referals_connect[msg.sender],3);
payable(OurBank).transfer((msg.value/100)*bankPercent);
 payable(Marketing).transfer((msg.value/100)*adminsPercent); 
 payable(referals_connect[msg.sender]).transfer((msg.value/100)*refPercent);
...
function getPaymant4 () public payable isHolder_4(msg.sender) minimumSend4(msg.sender,msg.value){
   ...
   require (isNextLVLsOpen==true, "This is level is not started");  
 _CreateUser4(msg.sender); 

 totalBNB+=msg.value; //
emit BuyLvl(msg.sender,4);
    emit BuyLvlForRef(msg.sender,referals_connect[msg.sender],4);
...
function getPaymant5 () public payable isHolder_5(msg.sender) minimumSend5(msg.sender,msg.value){
 ...
 totalBNB+=msg.value; //
emit BuyLvl(msg.sender,5);
    emit BuyLvlForRef(msg.sender,referals_connect[msg.sender],5);
...
function getPaymant6 () public payable isHolder_6(msg.sender) minimumSend6(msg.sender,msg.value){
 ...
 totalBNB+=msg.value; //
emit BuyLvl(msg.sender,6);
    emit BuyLvlForRef(msg.sender,referals_connect[msg.sender],6);
