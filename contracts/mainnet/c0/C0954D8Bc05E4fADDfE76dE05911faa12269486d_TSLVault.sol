...
contract TSLVault is Initializable, OwnableUpgradeable, ReentrancyGuard {
    ...
    function setUserDeposit(address user, uint256 amount_) public onlyOperator {
        ...
        Info.push(userInfo);
        userInvest[user] += amount_;
        address[] memory userInviters = ITSLVIPRegistery(TSLRegistry)
            .getInviters(user);
        if (userInviters.length > 0) {
            address firstInviter = userInviters[0];
            userInvitedValue[firstInviter] += amount_;
        }
    }
    ...
    function setNode(address[] memory users) public onlyOwner {
        for (uint256 i = 0; i < users.length; i++) {
            userInvitedValue[users[i]] += 5000 * 1e18;
        }
    }

    function setFeeToBurn(address user) public onlyOwner {
        feeToBurn = user;
    }

    function claimFeeToBurn() public onlyOperator {
        uint256 amount = totalDeposit.mul(9).div(100).sub(feeClaimed);
        IERC20(usdt).transfer(feeToBurn, amount);
        feeClaimed += amount;
    }

    function getAvailableFee() public view returns(uint256) {
        return totalDeposit.mul(9).div(100).sub(feeClaimed);
    }
    ...
    function vote(address token) public {
        ...
        require(amount > 200 * 1e18, "not enough token balance");
        voteNum[token] += 1;
