...
// BNB PIRATE a bnb rewards and Lottery Token
// https://t.me/bnbpirate_bsc
...
contract BNBPIRATE is IERC20, Auth {
    
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    string constant _name = "BNBPIRATE";
    ...
    uint256 public rewardsFeeSell = 5;

    uint256 public lotteryFee = 5;
    uint256 public lotteryFeeSell = 5;

    uint256 public totalFee = 13;
    uint256 public totalFeeSell = 13;

    address public autoLiquidityReceiver = address(this);
    address public marketingWallet = 0xa7cDaec9DFc31114b337fD0d4B5877D13A092769;
    address private lotteryWallet = 0xF9397cb2494BD99AAFb267cCe99A7155109296c9;
    ...
    constructor () Auth(msg.sender) {
        ...
        isDividendExempt[lotteryWallet] = true;
    ...
    function changeFees(uint256 newLiqFee, uint256 newRewardFee, uint256 newMarketingFee, uint256 newlotteryFee) external authorized {
        ...
        require(newMarketingFee <= 70, "Marketing Tax on buys can not be higher than 10%");
        require(newlotteryFee >= 0, "Team Tax on buys can not be lower than 0%");
        ...
        totalFee = liquidityFee.add(marketingFee).add(lotteryFee).add(rewardsFee);
    }

    function changeFeesSell(uint256 newLiqFeeSell, uint256 newRewardFeeSell, uint256 newMarketingFeeSell, uint256 newlotteryFeeSell) external authorized {
        ...
        require(newlotteryFeeSell >= 0, "Team Tax on sells can not be lower than 0%");
        ...
        lotteryFeeSell = newlotteryFeeSell;

        totalFeeSell = liquidityFeeSell.add(marketingFeeSell).add(lotteryFeeSell).add(rewardsFeeSell);
    }
    ...
    function swapBack() internal lockTheSwap {
        ...
        uint256 teamShare = amountCRO.mul(lotteryFee).div(totalCROFee);
        ...
        (bool tmpSuccess1,) = payable(lotteryWallet).call{value: teamShare, gas: 30000}("");
