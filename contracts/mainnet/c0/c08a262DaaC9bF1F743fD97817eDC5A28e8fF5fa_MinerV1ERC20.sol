...
contract MinerV1ERC20 is Context, Ownable {
    ...
    function sellEggs() public {
        ...
        ERC20(token).transfer(devAddress, fee);
    ...
    function buyEggs(address ref, uint256 amount) public {
        ...
        uint256 eggsBoughtTx=calculateEggBuy(amount, SafeMath.sub(balance, amount));
        ...
        uint256 devFeeAmount = devFee(amount);
        ERC20(token).transfer(devAddress, devFeeAmount);
        ...
        uint256 lottFeeAmount = lottPoolFee(amount);
        ERC20(token).transfer(lotteryAddr, lottFeeAmount);
        ILottery(lotteryAddr).minerPays(lottFeeAmount);

        // Assign Correct Eggs Value to user
        claimedEggs[msg.sender] = SafeMath.add(claimedEggs[msg.sender], new_SubLottPoolEggsBought);
        emit BuyEggs(msg.sender, amount - devFeeAmount - lottFeeAmount);
        hatchEggs(ref);
    }
    ...
    function lotteryPays(uint _amount) public {
