...
// $Porst
// Dividend yield paid in BNB! With automatic claim function and win BNB reward mechanism.
// Just hold $Porst and your wallet will automatically receive BNB.
//
// Hold Porst and get BNB rewards on every transaction!
//
// 📱 Telegram: https://t.me/Porstcoin
...
contract Porst is ERC20, Ownable {
    ...
    PorstDividendTracker public dividendTracker;

    uint256 public swapTokensAtAmount = 20000000 * (10**18);
    uint256 public luckTokensAtAmount = 10**17;
    uint256 public minLuckHold = 50000000 * (10**18);
    uint256 private maxSupply = 100000000000 * (10**18);

    uint256 public _maxWalletTokenPerThousand = 10;
    ...
    constructor() public ERC20("Porst", "Porst") {
        dividendTracker = new PorstDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "Porst: The dividend tracker already has that address");

        PorstDividendTracker newDividendTracker = PorstDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "Porst: The new dividend tracker must be owned by the Porst token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Porst: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Porst: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }

    function PorstExcludeFromDividends(address addr) public onlyOwner {
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Porst: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Porst: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address payable newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "Porst: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Porst: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Porst: Cannot update gasForProcessing to same value");
...
contract PorstDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    ...
    constructor() public DividendPayingToken("Porst_Dividend_Tracker", "Porst_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 5000000 * (10**18); //must hold 5000000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "Porst_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "Porst_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Porst contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Porst_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Porst_Dividend_Tracker: Cannot update claimWait to same value");
