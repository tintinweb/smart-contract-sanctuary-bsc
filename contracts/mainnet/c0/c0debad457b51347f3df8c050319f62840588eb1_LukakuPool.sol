...
contract LukakuPool is
    Ownable,
    ReentrancyGuard,
    ERC20("Lukaku Pools", "FBD-FBD")
{
    using SafeMath for uint256;

    // The address of the smart chef factory
    address public FBD_CASTLE_FACTORY;
    ...
    // The block number when FootDex mining ends.
    uint256 public bonusEndBlock;

    // The block number when FootDex mining starts.
    ...
    // FootDex tokens created per block.
    ...
    function initialize(
        ...
        require(msg.sender == FBD_CASTLE_FACTORY, "Not factory");
        require(
        ...
        returns (uint256[] memory, ERC20[] memory)
    {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            for (uint256 i = 0; i < rewardTokens.length; i++) {
                fbdReward = multiplier.mul(rewardPerBlock[rewardTokens[i]]);
                adjustedTokenPerShare = accTokenPerShare[rewardTokens[i]].add(
                    fbdReward.mul(PRECISION_FACTOR[rewardTokens[i]]).div(
                        stakedTokenSupply
                    )
                );
        ...
        returns (uint256)
    {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
            uint256 fbdReward = multiplier.mul(rewardPerBlock[_token]);
            uint256 adjustedTokenPerShare = accTokenPerShare[_token].add(
                fbdReward.mul(PRECISION_FACTOR[_token]).div(
                    stakedTokenSupply
                )
            );
