...
contract StakingPool {
    ...
    constructor(ERC20 _busdToken, string memory _name) {
    ...
    function unstakeTokens(uint256 _amount) public payable {
        ...
        require(busdToken.balanceOf(address(this)) >= _amount);
    ...
    function transferTokenToInvestor(
        ...
        // Require that staking pool has enough tokens
        require(busdToken.balanceOf(address(this)) >= tempTotalStaking);
        ...
        if (tempTotalStaking >= _calculatedProjectTokens) {
            ...
            for (uint256 i = 0; i < stakers.length; i++) {
                ...
                if (balance > 0) {
                    busdToken.transfer(recipient, balance);
            ...
            busdToken.transfer(projectOwnerAddress, tempTotalStaking);
