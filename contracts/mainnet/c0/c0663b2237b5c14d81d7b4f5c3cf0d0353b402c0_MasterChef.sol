...
// RoyalToken with Governance.
contract RoyalToken is BEP20("Elysium Royale", "ROYAL") {
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    ///
    ///   pending reward = (user share * pool.accRoyalPerShare) - user.rewardDebt
    ///
    ...
    ///     ROYAL rewards each pool gets.
    /// `accRoyalPerShare` Accumulated ROYALs per share, times 1e12.
    ...
    ///     the ROYAL rewards to all the RoyalSwap products.
    /// `totalBoostedShare` The total amount of user shares in each pool. After considering the share boosts.
    struct PoolInfo {
        uint256 accRoyalPerShare;
    ...
    // The ROYAL TOKEN!
    RoyalToken public royal;

    // Dev address.
    address public devaddr;

    /// @notice The only address can withdraw all the burn ROYAL.
    ...
    ///  @notice the amount of royals per block in MC
    uint256 public masterchefRoyalPerBlock = 40 * 1e18;
    uint256 public constant MAX_MASTERCHEF_ROYAL_PER_BLOCK = 50 * 1e18;
    uint256 public constant ACC_ROYAL_PRECISION = 1e18;

    /// @notice The block number when royals mining starts.
    ...
    /// @notice total royal rate = toBurn + toRegular + toSpecial
    uint256 public constant ROYAL_RATE_TOTAL_PRECISION = 1e12;
    /// @notice The last block number of ROYAL burn action being executed.
    /// @notice ROYAL distribute % for burn
    uint256 public royalRateToBurn = 0;
    /// @notice ROYAL distribute % for regular farm pool
    uint256 public royalRateToRegularFarm = 250000000000;
    /// @notice ROYAL distribute % for special pools
    uint256 public royalRateToSpecialFarm = 750000000000;
    ...
    event UpdatePool(uint256 indexed pid, uint256 lastRewardBlock, uint256 lpSupply, uint256 accRoyalPerShare);
    event Deposit(address indexed user, uint256 indexed pid, uint256 amount);
    event Withdraw(address indexed user, uint256 indexed pid, uint256 amount);
    event EmergencyWithdraw(address indexed user, uint256 indexed pid, uint256 amount);

    event UpdateRoyalRate(uint256 burnRate, uint256 regularFarmRate, uint256 specialFarmRate);
    ...
    /// @param _royal The ROYAL token contract address.
    ...
    /// only for ROYAL distributions within RoyalSwap products.
    function add(
        uint256 _allocPoint,
        IBEP20 _lpToken,
        bool _isRegular,
        ...
        // stake ROYAL token will cause staked token and reward token mixed up,
        // may cause staked tokens withdraw as reward token,never do it.
        require(_lpToken != royal, "ROYAL token can't be added to farm pools");
    ...
    /// @notice Update the given pool's ROYAL allocation point. Can only be called by the owner.
    ...
    /// @notice View function for checking pending ROYAL rewards.
    /// @param _pid The id of the pool. See `poolInfo`.
    ...
    function pendingRoyal(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 royalReward = multiplier.mul(royalPerBlock(pool.isRegular)).mul(pool.allocPoint).div(
                (pool.isRegular ? totalRegularAllocPoint : totalSpecialAllocPoint)
            );
            accRoyalPerShare = accRoyalPerShare.add(royalReward.mul(ACC_ROYAL_PRECISION).div(lpSupply));
        }

        uint256 boostedAmount = user.amount.mul(getBoostMultiplier(_user, _pid)).div(BOOST_PRECISION);
        return boostedAmount.mul(accRoyalPerShare).div(ACC_ROYAL_PRECISION).sub(user.rewardDebt);
    }

    /// @notice Update royal reward for all the active pools. Be careful of gas spending!
    ...
    /// @notice Calculates and returns the `amount` of ROYAL per block.
    /// @param _isRegular If the pool belongs to regular or special.
    function royalPerBlock(bool _isRegular) public view returns (uint256 amount) {
        if (_isRegular) {
            amount = masterchefRoyalPerBlock.mul(royalRateToRegularFarm).div(ROYAL_RATE_TOTAL_PRECISION);
        } else {
            amount = masterchefRoyalPerBlock.mul(royalRateToSpecialFarm).div(ROYAL_RATE_TOTAL_PRECISION);
        }
    }

    /// @notice Calculates and returns the `amount` of ROYAL per block to burn.
    function royalPerBlockToBurn() public view returns (uint256 amount) {
        amount = masterchefRoyalPerBlock.mul(royalRateToBurn).div(ROYAL_RATE_TOTAL_PRECISION);
    }
    ...
    function updatePool(uint256 _pid) public returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (lpSupply > 0 && totalAllocPoint > 0) {
                ...
                pool.accRoyalPerShare = pool.accRoyalPerShare.add((royalReward.mul(ACC_ROYAL_PRECISION).div(lpSupply)));
                royal.mint(devaddr, royalReward.div(10));
                royal.mint(address(this), royalReward);
            }
            pool.lastRewardBlock = block.number;
            poolInfo[_pid] = pool;
            emit UpdatePool(_pid, pool.lastRewardBlock, lpSupply, pool.accRoyalPerShare);
        }
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            settlePendingRoyal(msg.sender, _pid, multiplier);
        }
        ...
        user.rewardDebt = user.amount.mul(multiplier).div(BOOST_PRECISION).mul(pool.accRoyalPerShare).div(
            ACC_ROYAL_PRECISION
        );
        poolInfo[_pid] = pool;
    ...
    /// @notice Send ROYAL pending for burn to `burnAdmin`.
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function burnRoyal(bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }

        uint256 multiplier = block.number.sub(lastBurnedBlock);
        uint256 pendingRoyalToBurn = multiplier.mul(royalPerBlockToBurn());

        // SafeTransfer ROYAL
        _safeTransfer(burnAdmin, pendingRoyalToBurn);
        lastBurnedBlock = block.number;
    }

    /// @notice Update the % of ROYAL distributions for burn, regular pools and special pools.
    /// @param _burnRate The % of ROYAL to burn each block.
    /// @param _regularFarmRate The % of ROYAL to regular pools each block.
    /// @param _specialFarmRate The % of ROYAL to special pools each block.
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function updateRoyalRate(
        uint256 _burnRate,
        ...
        require(
            ...
            "MasterChef: Royal rate must be greater than 0"
        );
        require(
            _burnRate.add(_regularFarmRate).add(_specialFarmRate) == ROYAL_RATE_TOTAL_PRECISION,
            "MasterChef: Total rate must be 1e12"
        );
        if (_withUpdate) {
            massUpdatePools();
        }
        // burn royal base on old burn royal rate
        burnRoyal(false);

        royalRateToBurn = _burnRate;
        royalRateToRegularFarm = _regularFarmRate;
        royalRateToSpecialFarm = _specialFarmRate;

        emit UpdateRoyalRate(_burnRate, _regularFarmRate, _specialFarmRate);
    }
    ...
    function updateBoostMultiplier(
        ...
        settlePendingRoyal(_user, _pid, prevMultiplier);

        user.rewardDebt = user.amount.mul(_newMultiplier).div(BOOST_PRECISION).mul(pool.accRoyalPerShare).div(
            ACC_ROYAL_PRECISION
        );
    ...
    /// @notice Update emissionRate.
    /// @param _emissionRate The emissionRate to be updated.
    function updateEmissionRate(uint256 _emissionRate) external onlyOwner {
        require(_emissionRate <= MAX_MASTERCHEF_ROYAL_PER_BLOCK, "MasterChef: The start block height should be less than 10 ROYAL");

        massUpdatePools();

        masterchefRoyalPerBlock = _emissionRate;
    }
    ...
    /// @notice Settles, distribute the pending ROYAL rewards for given user.
    /// @param _user The user address for settling rewards.
    /// @param _pid The pool id.
    /// @param _boostMultiplier The user boost multiplier in specific pool id.
    function settlePendingRoyal(
        address _user,
        uint256 _pid,
        ...
        uint256 accRoyal = boostedAmount.mul(poolInfo[_pid].accRoyalPerShare).div(ACC_ROYAL_PRECISION);
        uint256 pending = accRoyal.sub(user.rewardDebt);
        // SafeTransfer ROYAL
        _safeTransfer(_user, pending);
    }
    function dev(address _devaddr) external onlyOwner {
        require(_devaddr != address(0), "MasterChef: The dev address must be valid");
        devaddr = _devaddr;
        emit SetDev(msg.sender, _devaddr);
    }

    /// @notice Safe Transfer ROYAL.
    /// @param _to The ROYAL receiver address.
    /// @param _amount transfer ROYAL amounts.
    function _safeTransfer(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            uint256 balance = royal.balanceOf(address(this));
            if (balance < _amount) {
                _amount = balance;
            }
            royal.transfer(_to, _amount);
        }
    }
}