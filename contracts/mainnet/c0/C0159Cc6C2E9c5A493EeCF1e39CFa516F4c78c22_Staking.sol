...
contract Staking is Context, Pausable, Ownable {
    ...
    function pend(address account) private view returns (uint256) {
        uint256 interestDailyView = 32876712320; uint256 interestPerSecond = interestDailyView / 86400; uint256 interest =
        
        (block.timestamp - quandoStake[account]) * interestPerSecond;
        uint256 preRewards;
        preRewards = (holdingXstaking[account] * interest);
        preRewards = ((preRewards / 100000000000) / 100) - rewardsGiaPagati[account];
        return preRewards;
    }
