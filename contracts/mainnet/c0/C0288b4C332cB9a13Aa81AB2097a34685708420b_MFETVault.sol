...
contract MFETVault is Context, Ownable, ReentrancyGuard {
    ...
    function reward(uint256 _amount)
        external
        investOpen
        onlyEOA
        nonReentrant
        returns (uint256)
    {
        ...
        return rewardInvestors(_amount);
    }
        ...
        returns (uint256)
    {
        ...
        return investing(_user, _amount);
    }
        ...
        returns (uint256)
    {
        ...
        uint256 _tokens = investTokens(_user, _dividends);

        // fire event
        emit onReinvestment(_user, _tokens, block.timestamp);

        // stats update
        stats[_user].reinvested = SafeMath.add(
            stats[_user].reinvested,
            _tokens
        );
        stats[_user].xReinvested += 1;
        ...
        returns (uint256)
    {
        ...
        // stats update
        stats[_user].withdrawn = SafeMath.add(
            stats[_user].withdrawn,
            _dividends
        );
        stats[_user].xWithdrawn += 1;
        totalTxs += 1;
        totalWithdraws += _dividends;

        mToken.transfer(_user, _dividends);
        ...
        onlyInvestors
    {
        ...
        require(
            _amountOfTokens <= tokenBalanceLedger_[_user],
            "More then withdrawn"
        );
        ...
        tokenBalanceLedger_[_user] = SafeMath.sub(
            tokenBalanceLedger_[_user],
            _amountOfTokens
        );
        ...
        // fire event
        emit onTokenExit(_user, _taxedAmount, block.timestamp);
    ...
    function dailyEstimate(address _user) public view returns (uint256) {
        ...
        return
            (tokenSupply_ > 0)
                ? share.mul(tokenBalanceLedger_[_user]).div(tokenSupply_)
                : 0;
    }
    ...
    function rewardInvestors(uint256 _amount) internal returns (uint256) {
        ...
        /// amount add to dividend daily pools and instant share pool to distribute
