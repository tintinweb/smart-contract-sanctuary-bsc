...
contract SwitchViewBSC is SwitchRootBSC {
    ...
    function calculateBUSD(CalculateArgs memory args) internal view returns (uint256[] memory rets) {
        if (args.fromToken == busd || args.destToken == busd) {
            return new uint256[](args.parts);
        }
