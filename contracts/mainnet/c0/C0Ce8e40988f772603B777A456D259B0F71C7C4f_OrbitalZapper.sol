...
import {IOrbitalPair} from "./interfaces/IOrbitalPair.sol";
import {IOrbitalRouter02} from "./interfaces/IOrbitalRouter02.sol";
import {IWETH} from "./interfaces/IWETH.sol";
import {Babylonian} from "./libraries/Babylonian.sol";

/*
 * @author Inspiration from the work of Zapper and Beefy.
 * Implemented and modified by OrbitalSwap teams.
 */
contract OrbitalZapper is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // Interface for Wrapped BNB (WBNB)
    IWETH public WBNB;

    // OrbitalRouter interface
    IOrbitalRouter02 public orbitalRouter;

    // Maximum integer (used for managing allowance)
    uint256 public constant MAX_INT = 2**256 - 1;

    // Minimum amount for a swap (derived from OrbitalSwap)
    uint256 public constant MINIMUM_AMOUNT = 1000;

    // Maximum reverse zap ratio (100 --> 1%, 1000 --> 0.1%)
    uint256 public maxZapReverseRatio;

    // Address OrbitalRouter
    address private orbitalRouterAddress;
     ...
     * @param _orbitalRouter: address of the OrbitalRouter
    ...
    constructor(
        ...
        orbitalRouterAddress = _orbitalRouter;
     ...
     * @param _lpToken: LP token address (e.g. ORB/BNB)
     * @param _tokenAmountOutMin: minimum token amount (e.g. ORB) to receive in the intermediary swap (e.g. BNB --> ORB)
     */
     ...
     * @param _lpToken: LP token address (e.g. ORB/BUSD)
     * @param _tokenAmountOutMin: minimum token to receive (e.g. ORB) in the intermediary swap (e.g. BUSD --> ORB)
     */
    function zapInToken(
        address _tokenToZap,
     ...
     * @param _lpToken: LP token address (e.g. ORB/WBNB)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum amount to receive (in BNB/WBNB) in the intermediary swap (e.g. ORB --> BNB)
     */
    function zapOutBNB(
        address _lpToken,
     ...
     * @param _lpToken: LP token address (e.g. ORB/BUSD)
     * @param _tokenToReceive: one of the 2 tokens from the LP (e.g. ORB or BUSD)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum token to receive (e.g. ORB) in the intermediary swap (e.g. BUSD --> ORB)
     */
    ...
    {
        address token0 = IOrbitalPair(_lpToken).token0();
        address token1 = IOrbitalPair(_lpToken).token1();

        require(_tokenToZap == token0 || _tokenToZap == token1, "Zap: Wrong tokens");

        // Convert to uint256 (from uint112)
        (uint256 reserveA, uint256 reserveB, ) = IOrbitalPair(_lpToken).getReserves();

        uint256 pairFee = IOrbitalPair(_lpToken).feeAmount() + IOrbitalPair(_lpToken).controllerFeeShare();

        if (token0 == _tokenToZap) {
            swapTokenOut = token1;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveA, reserveB, pairFee);
            swapAmountOut = orbitalRouter.getAmountOut(swapAmountIn, reserveA, reserveB, pairFee);
        } else {
            swapTokenOut = token0;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveB, reserveA, pairFee);
            swapAmountOut = orbitalRouter.getAmountOut(swapAmountIn, reserveB, reserveA, pairFee);
        }
    ...
    {
        require(
            _token0ToZap == IOrbitalPair(_lpToken).token0() || _token0ToZap == IOrbitalPair(_lpToken).token1(),
            "Zap: Wrong token0"
        );
        require(
            _token1ToZap == IOrbitalPair(_lpToken).token0() || _token1ToZap == IOrbitalPair(_lpToken).token1(),
            "Zap: Wrong token1"
        );
        ...
        if (_token0ToZap == IOrbitalPair(_lpToken).token0()) {
        ...
        )
    {
        ...
        if (token1 == _tokenToReceive) {
            // sell token0
            uint256 tokenAmountIn = (_lpTokenAmount * reserveA) / IOrbitalPair(_lpToken).totalSupply();
            ...
            // sell token1
            uint256 tokenAmountIn = (_lpTokenAmount * reserveB) / IOrbitalPair(_lpToken).totalSupply();
    ...
    function _zapIn(
        ...
        uint256[] memory swapedAmounts = orbitalRouter.swapExactTokensForTokens(
            swapAmountIn,
            _tokenAmountOutMin,
            path,
            address(this),
        ...
        (, , lpTokenReceived) = orbitalRouter.addLiquidity(
            path[0],
            path[1],
    ...
    function _zapOut(
        ...
        (uint256 amount0, uint256 amount1) = IOrbitalPair(_lpToken).burn(address(this));

        require(amount0 >= MINIMUM_AMOUNT, "OrbitalRouter: INSUFFICIENT_A_AMOUNT");
        require(amount1 >= MINIMUM_AMOUNT, "OrbitalRouter: INSUFFICIENT_B_AMOUNT");
        ...
        // Swap tokens
        orbitalRouter.swapExactTokensForTokens(swapAmountIn, _tokenAmountOutMin, path, address(this), block.timestamp);
    ...
    function _approveTokenIfNeeded(address _token, uint256 _swapAmountIn) private {
        if (IERC20(_token).allowance(address(this), orbitalRouterAddress) < _swapAmountIn) {
            // Reset to 0
            IERC20(_token).safeApprove(orbitalRouterAddress, 0);
            // Re-approve
            IERC20(_token).safeApprove(orbitalRouterAddress, MAX_INT);
        }
    }
    ...
    function _calculateAmountToSwap(
        ...
        uint256 nominator = orbitalRouter.getAmountOut(halfToken0Amount, _reserve0, _reserve1, _pairFee);
        uint256 denominator = orbitalRouter.quote(
            halfToken0Amount,
    ...
    function _calculateAmountToSwapForRebalancing(
        ...
        if (sellToken0) {
            ...
            uint256 nominator = orbitalRouter.getAmountOut(token0AmountToSell, _reserve0, _reserve1, _pairFee);
            uint256 denominator = orbitalRouter.quote(
                token0AmountToSell,
            ...
            uint256 nominator = orbitalRouter.getAmountOut(token1AmountToSell, _reserve1, _reserve0, _pairFee);
