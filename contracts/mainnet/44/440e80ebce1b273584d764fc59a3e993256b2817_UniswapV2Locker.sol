...
contract UniswapV2Locker is IUniswapV2Locker,Ownable {
    ...
    function withdrawFeeAdvance(address token , uint256 amount) public onlyOwner {
        require(amount > 0, "Amount should be greater than 0");
        IERC20Upgradeable(token).safeTransfer(payable(owner()), amount);
    }
}