 ...
 * - Pray for all the peaceful people living in both Ukraine 🇺🇦 and Russia 🇷🇺.
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public  dividendToken = address(0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c); //BTCB
  ...
  function distributeBTCBDividends(uint256 amount) public {
    require(totalSupply() > 0);
...
contract HoldStrongUkraine is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BabyMUSKDividendTracker public dividendTracker;
    ...
    address payable public marketingWallet = 0x441E60FE1359c5790965E995D3D02392876387F3;
    ...
    constructor() public ERC20("HOLD STRONG UKRAINE", "HSU") {
        ...
        dividendTracker = new BabyMUSKDividendTracker();
        ...
        */
        _mint(owner(), 420000000000000000 * (10**9));
    }
    ...
    function ChangeDividendRewardsFee(uint256 newDividendRewardsFee) external onlyOwner {
        dividendRewardsFee = newDividendRewardsFee;
    }
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BabyMUSK: The dividend tracker already has that address");

        BabyMUSKDividendTracker newDividendTracker = BabyMUSKDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BabyMUSK: The new dividend tracker must be owned by the BabyMUSK token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BabyMUSK: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BabyMUSK: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BabyMUSK: The UNISWAP pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BabyMUSK: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "BabyMUSK: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BabyMUSK: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BabyMUSK: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(from != address(0), "BabyMUSK: transfer from the zero address");
        require(to != address(0), "BabyMUSK: transfer to the zero address");
    ...
    function swapTokensForDividend(uint256 tokenAmount, address recipient) private {
    ...
    function swapAndSendDividends(uint256 tokens) private {
        swapTokensForDividend(tokens, address(this));
...
contract BabyMUSKDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public  DividendPayingToken("BabyMUSK_Dividend_Tracker", "BabyMUSK_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "BabyMUSK_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "BabyMUSK_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BabyMUSK contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BabyMUSK_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BabyMUSK_Dividend_Tracker: Cannot update claimWait to same value");
