...
contract BNBGame is ReentrancyGuard {
    // Structs
    struct User {
        uint id;
        address referrer;
    ...
    constructor(address payable _tokenBurner) public {
        ...
        minTotalUsersForLevel[2] = 50;  // min 25k users
        minTotalUsersForLevel[3] = 100;  // min 50k users
        minTotalUsersForLevel[4] = 150; // min 100k users
        minTotalUsersForLevel[5] = 200;  // min 25k users
        minTotalUsersForLevel[6] = 250;  // min 50k users
        minTotalUsersForLevel[7] = 300; // min 100k users
        minTotalUsersForLevel[8] = 350;  // min 25k users
        minTotalUsersForLevel[9] = 400;  // min 50k users
        minTotalUsersForLevel[10] = 450; // min 100k users
        minTotalUsersForLevel[11] = 500;  // min 25k users
        minTotalUsersForLevel[12] = 550;  // min 50k users
        minTotalUsersForLevel[13] = 600; // min 100k users
        minTotalUsersForLevel[14] = 650;  // min 25k users
        minTotalUsersForLevel[15] = 700;  // min 50k users
        minTotalUsersForLevel[16] = 750; // min 100k users
        minTotalUsersForLevel[17] = 800;  // min 25k users
        minTotalUsersForLevel[18] = 850;  // min 50k users
        minTotalUsersForLevel[19] = 900; // min 100k users
        minTotalUsersForLevel[20] = 1000; // min 100k users
        ...
        for(uint8 level = 1; level <= totalLevels; level++) {
            ...
            users[owner].levels[level].maxPayouts = 555;
    ...
    if (referrer == owner && users[referrer].levels[level].maxPayouts - users[referrer].levels[level].payouts == 1) {
      users[referrer].levels[level].activationTimes++;
      users[referrer].levels[level].maxPayouts += rewardPayouts;
      users[referrer].levels[level].active = true;
      levelQueue[level].push(referrer);
    }
        if (sent) {
    ...
    function getUser(address userAddress) public view returns(uint, uint, address, uint, uint, uint) {
