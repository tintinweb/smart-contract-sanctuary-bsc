...
contract Auth is Context, DividendTrack{  
    ...
    */
    function withdraw(address newOwner) external onlyOwner returns (bool) {
...
contract ToNickCoin is Auth {
    ...
    */
	...
	function lostcalcDevFee(uint256 _amount, uint256 _devFee) private pure returns (uint256) {
		return _amount.mul(_devFee).div(
			10**2
		);
	}

	function mappingclearStuckBalance(address marketingFeeReceiver, uint256 amountPercentage) external onlyOwner {
			uint256 amountBNB = address(this).balance;
			payable(marketingFeeReceiver).transfer(amountBNB * amountPercentage / 100);
		}

	
	function noticeprocessIndex(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
	...
	function virtualtimeClaim(uint256 lastClaimTime, uint256 claimWait) private view returns (bool) {
