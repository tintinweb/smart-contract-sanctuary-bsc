...
contract FreMiner is ReentrancyGuard {
        ...
        returns (bool)
    {
        ...
        for (uint256 i = 0; i < aa.length; i++) {
            if (aa[i] == _currentLineTotalHash[cline]) {
                j += i;
                break;
            }
        }
    ...
    function cLineSort() internal view returns (uint256[] memory) {
        ...
        h[0] = _currentLineTotalHash[1];
        h[1] = _currentLineTotalHash[2];
        h[2] = _currentLineTotalHash[3];
    ...
    function currentLineUserHashSort(uint8 cLine)
        public
        view
        returns (address[] memory)
    {
        address[] memory us = _currentLineAddresses[cLine];
