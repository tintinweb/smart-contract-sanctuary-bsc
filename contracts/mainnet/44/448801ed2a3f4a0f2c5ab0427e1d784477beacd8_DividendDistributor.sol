...
/**
 * ChefCheemsInuSlap Protocol = CHEF  is a Dual Reward system combining the power
 * BUSD Dividen reward and a positive rebase reward of $CHEF with Auto-Staking and Auto-Compounding
 *
 * Website : https://chefcheemsinuslap.com/
 * Twitter : https://mobile.twitter.com/ChefCheemsInuSlap_BSC
 * Telegram : https://t.me/ChefCheemsInuSlapBSC
 */
...
contract ChefCheemsInuSlap is ERC20Detailed, Ownable {
    ...
    uint256 public busdDividendFee = 50;
    uint256 public sellFee = 20;
    uint256 public autofirePitFee = 45;
    uint256 public totalFee = liquidityFee.add(treasuryFee).add(busdDividendFee).add(autofirePitFee);
    ...
    address public CHEFDividendReceiver;
    ...
    event FeesChanged(uint256 liquidityFee, uint256 treasuryFee, uint256 busdDividendFee, uint256 autofirePitFee);
    event minShresRequiredChanged(uint256 minSharesRequired);
    event SwapEnabled(bool swapEnabled);
    event FeeReceiversChanged(address autoLiquidityReceiver, address treasuryReceiver, address autofirePit);

    constructor() ERC20Detailed("ChefCheemsInuSlap", "CHEF", uint8(DECIMALS)) Ownable() {
        ...
        //contract addreses
        autoLiquidityReceiver = 0xb035585E8a7D387a84eC6D3d762793F3985bE764;
        treasuryReceiver = 0x1C2A8Ec3e6C82706c1CC0bFaeFe854451108aCbF;   
        ...
        CHEFDividendReceiver = address(distributor);
    ...
    //once rebase enable it will auto calculate rebase rate according to time frame laid out. Rates listed are the 15 minute rebase compound rates. Epochs outlined below.
    ...
    function takeFee(
        ...
        if (recipient == pair) {
            ...
            _busdDividendFee = busdDividendFee.add(sellFee);
        }
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(treasuryFee.add(busdDividendFee))
        );
    ...
    function swapBack() internal swapping {
        ...
        try distributor.deposit {
            value: amountETHToTreasuryAndSIF.mul(busdDividendFee).div(treasuryFee.add(busdDividendFee))
        } () {} catch {}        
    }
    ...
    function setFees(uint256 _liquidityFee, uint256 _treasuryFee, uint256 _busdDividendFee, uint256 _autofirePitFee) external onlyOwner {
        liquidityFee = _liquidityFee;
        treasuryFee = _treasuryFee;
        busdDividendFee = _busdDividendFee;
        autofirePitFee = _autofirePitFee;

        totalFee = liquidityFee.add(treasuryFee).add(busdDividendFee).add(autofirePitFee);
        require(totalFee <= 2500, "Must keep fees at 25% or less");

        emit FeesChanged(_liquidityFee, _treasuryFee, _busdDividendFee, _autofirePitFee);
    }
    ...
    function setFeeReceivers(
        ...
        emit FeeReceiversChanged(_autoLiquidityReceiver, _treasuryReceiver, _autofirePit);      
    }
    ...
    function withdrawAllToTreasury() external swapping onlyOwner {
        ...
        uint256 amountToSwap = _gonBalances[address(this)].div(_gonsPerFragment);
        require( amountToSwap > 0,"There is no ChefCheemsInuSlap token deposited in token contract");
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();
        router.swapExactTokensForETHSupportingFeeOnTransferTokens(
