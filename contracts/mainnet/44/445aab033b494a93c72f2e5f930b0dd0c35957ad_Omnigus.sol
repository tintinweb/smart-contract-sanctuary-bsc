...
// File: contracts/omnigus-bsc.sol

pragma solidity ^0.8.7;

contract Omnigus is Ownable, ERC721, NonblockingReceiver {
    address public _owner;
    string private baseURI;
    uint256 nextTokenId = 0;
    uint256 MAX_MINT_BSC = 4000;
    uint256 public cost = 0.005 ether;
    bytes private payloadd;

    uint256 gasForDestinationLzReceive = 350000;

    constructor(string memory baseURI_, address _layerZeroEndpoint)
        ERC721("Omnigus", "Amogus")
    {
        _owner = msg.sender;
    ...
    // mint function
    // you can choose to mint 1 to  10
    // mint is nearly free to avoid bots, but payments are accepted
    function mint(uint8 numTokens) external payable {
        require(
            nextTokenId + numTokens <= MAX_MINT_BSC,
            "Omnigus: Mint exceeds supply"
        );
        if (msg.sender != _owner) {
             require(numTokens < 3, "Omnisgus: Max 2 NFTs per transaction");
             require(msg.value >= cost * numTokens);
            }
        if(numTokens>1){
    ...
    function traverseChains(uint16 _chainId, uint256 tokenId) public payable {
        ...
        require(
            ...
            "Omnigus: msg.value not enough to cover messageFee. Send gas for message fees"
        );
    ...
    function withdraw(uint256 amt) external onlyOwner {
        ...
        require(sent, "Omnigus: Failed to withdraw Ether");
    }
