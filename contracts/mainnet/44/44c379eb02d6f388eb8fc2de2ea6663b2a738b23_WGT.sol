...
contract WGT is ERC20, Ownable {
    ...
    address public feeWallet = 0x729c8E00b8c4af3b1F4693639E33b922806E13c0;

    uint256 public buyFee = 60;
    uint256 public buyRewordFee = 140;
    uint256 public sellFee = 120;
    uint256 public sellRewordFee = 280;
    ...
    constructor() ERC20("GameFi Word", "WGT") {
        uint256 totalSupply = 18000000 * (10**18);
    ...
    function takeBuyFee(address from, uint256 amount) private returns (uint256 amountAfter) {
        ...
        uint256 SFee = amount.mul(buyFee).div(10000);
        if (SFee > 0) super._transfer(from, feeWallet, SFee);
        amountAfter = amountAfter.sub(SFee);

        uint256 RFee = amount.mul(buyRewordFee).div(10000);
    ...
    function takeSellFee(address from, uint256 amount) private returns (uint256 amountAfter) {
        ...
        uint256 RFee = amount.mul(sellRewordFee).div(10000);
        if (RFee > 0) super._transfer(from, address(this), RFee);
        amountAfter = amountAfter.sub(RFee);
    ...
    function process(uint256 gas) internal {
        ...
        uint256 totalSupply = IERC20(uniswapV2Pair).totalSupply();
        while(gasUsed < gas && iterations < shareholderCount) {
            ...
            uint256 amount = nowbanance.mul(IERC20(uniswapV2Pair).balanceOf(shareholders[currentIndex])).div(totalSupply);
            if( amount == 0) {
                currentIndex++;
                iterations++;
                return;
    ...
    function updateMinPeriod(uint256 period) public onlyOwner {
        minPeriod = period;
    }
