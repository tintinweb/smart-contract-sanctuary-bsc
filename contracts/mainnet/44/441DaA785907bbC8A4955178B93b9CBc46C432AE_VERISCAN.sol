...
contract VERISCAN is IERC20, Context, Ownable {
    ...
    string private constant _name = "VERISCAN";

    uint256 public VS_Max_Buy_TX = 10000000000 * 10**9;
    uint256 public VS_Max_Sell_TX = 10000000000 * 10**9;

    address public VS_Marketing_Wallet = 0x52a00C0186a2a9c5a95650fD480eAD8b5D9377E9;
    address public VS_Team_Wallet = 0xB4430935b8AddFd68E7B604459E80C78E03cF1Db;

    uint256 public VS_max_Wallet = 20000000000 * 10**9;
    ...
    Taxes public sellTaxes = Taxes(0, 15, 2, 3);
    Taxes public taxes = Taxes(0, 15, 2, 3);
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[VS_Marketing_Wallet] = true;
        _isExcludedFromFee[VS_Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[VS_Marketing_Wallet] = true;
        allowedTransfer[VS_Team_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= VS_Max_Buy_TX, "You are exceeding VS_Max_Buy_TX");
            require(balanceOf(to) + amount <= VS_max_Wallet, "You are exceeding VS_max_Wallet");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= VS_Max_Sell_TX, "You are exceeding VS_Max_Sell_TX");
            if(to != pair){
                require(balanceOf(to) + amount <= VS_max_Wallet, "You are exceeding VS_max_Wallet");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(VS_Marketing_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(VS_Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function _Update_MarketingWallet(address newWallet) external onlyOwner{
        VS_Marketing_Wallet = newWallet;
    }
    
    function _Update_TeamWallet(address newWallet) external onlyOwner{
        VS_Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        VS_Max_Buy_TX = maxBuy * 10**decimals();
        VS_Max_Sell_TX = maxSell * 10**decimals();
    }
    
    function VS_Max_Wallet(uint256 amount) external onlyOwner{
        VS_max_Wallet = amount * 10**decimals();
    }
