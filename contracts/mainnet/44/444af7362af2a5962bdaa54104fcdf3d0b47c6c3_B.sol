...
contract BDividendTracker is DividendPayingToken, Ownable {
    ...
    function _transfer(
        ...
        require(false, "B_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "B_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main B contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(BContract);

            uint256 prevBalance = BContract.balanceOf(address(this));
            ...
            {
                uint256 received = BContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    BContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
