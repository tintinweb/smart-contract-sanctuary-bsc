...
contract Diamondcasinogames is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    DiamondcasinogamesDividendTracker public dividendTracker;

    address public deadWallet = 0x000000000000000000000000000000000000dEaD;

    address public immutable BUSD = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); //BUSD

    uint256 public _totalSupply = 500_000_000 * (10**18);
    ...
    address payable public _marketingWalletAddress = payable(0x7e02d322587b1BE9CC71683CC3c05F253f50485d);
    address payable public _buybackWalletAddress = payable(0xaf78Cc67BF67cdB6f3f456ad5b0652CBdc0dad76);
    ...
    constructor() BEP20("Diamond Casino Games", "DCG") 
    {
        dividendTracker = new DiamondcasinogamesDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "Diamondcasinogames: The dividend tracker already has that address");
        DiamondcasinogamesDividendTracker newDividendTracker = DiamondcasinogamesDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "Diamondcasinogames: The new dividend tracker must be owned by the Diamondcasinogames token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Diamondcasinogames: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Diamondcasinogames: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Diamondcasinogames: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Diamondcasinogames: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Diamondcasinogames: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Diamondcasinogames: Cannot update gasForProcessing to same value");
...
contract DiamondcasinogamesDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("Diamondcasinogames_Dividend_Tracker", "Diamondcasinogames_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "Diamondcasinogames_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "Diamondcasinogames_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Diamondcasinogames contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Diamondcasinogames_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Diamondcasinogames_Dividend_Tracker: Cannot update claimWait to same value");
