...
contract SafeUniverse is Context, IBEP20, Ownable {
    ...
    uint256 private _tTotal = 1e20 * 10**_decimals;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));

    uint256 public swapTokensAtAmount = 1e17 * 10**9;
    uint256 public maxBuyLimit = 2e18 * 10**9;
    uint256 public maxSellLimit = 2e18 * 10**9;
    uint256 public maxWalletLimit = 2e18 * 10**9;
    ...
    Taxes public taxes = Taxes(1, 5, 0, 3, 3);
    Taxes public sellTaxes = Taxes(1, 5, 0, 3, 3);
    ...
    function setTaxes(
        ...
        require((_rfi + _marketing + _liquidity + _dev + _reserve) <= 12, "Must keep fees at 12% or less");
        taxes = Taxes(_rfi, _marketing, _liquidity, _dev , _reserve);
        emit FeesChanged();
    }
    ...
    function setSellTaxes(
        ...
        require((_rfi + _marketing + _liquidity + _dev + _reserve) <= 12, "Must keep fees at 12% or less");
        sellTaxes = Taxes(_rfi, _marketing, _liquidity, _dev, _reserve);
        emit FeesChanged();
    }
    ...
    function updateSwapTokensAtAmount(uint256 amount) external onlyOwner {
        require(amount <= 1e18, "Cannot set swap threshold amount higher than 1% of tokens");
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell, uint256 maxWallet) external onlyOwner {
        require(maxBuy >= 1e17, "Cannot set max buy amount lower than 0.1% of tokens");
        require(maxSell >= 1e17, "Cannot set max sell amount lower than 0.1% of tokens%");
        require(maxWallet >= 1e18, "Cannot set max wallet amount lower than 1% of tokens");
