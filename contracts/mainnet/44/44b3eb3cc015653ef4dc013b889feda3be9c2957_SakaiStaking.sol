...
contract SakaiStaking is ERC20, Ownable, ReentrancyGuard {
    ...
    uint256 public immutable minimumStakeEqualtoUSDT;
    uint256 public immutable interestRate;
    uint256 public immutable interestRateDenominator;

    uint256 public bnbFee;
    address public backendWallet = 0x9Ffd803BD8048Ed59Fccb73d688326d7085A4e54;
    ...
    mapping(address => bool) public specialWallets;

    event BNBFeeUpdated(uint256 amount);
    event StakingTokenUpdated(address indexed stakingToken);
    event BackendWalletUpdated(address indexed backendWallet);
    ...
    function setBackendWallet(address _backendWallet) external onlyOwner {
        require(
            _backendWallet != backendWallet,
            "Backend wallet is already that address"
        );
        require(
            _backendWallet != address(0),
            "Backend wallet cannot be the zero address"
        );

        backendWallet = _backendWallet;

        emit BackendWalletUpdated(backendWallet);
    }
    ...
    function getBalanceInUSDT(address account) external view returns (uint256) {
        return _stakers[account].usdtBalance;
    }
    ...
    function deposit(uint256 amount) external payable {
        require(msg.sender == tx.origin, "Contract cannot call deposit");
        ...
        payable(backendWallet).transfer(msg.value);
        ...
        if (_stakers[msg.sender].balance == 0) {
            ...
            _stakers[msg.sender] = Stake(msg.sender, 0, timeNow, timeNow, 0, 0);
        }

        _stakers[msg.sender].balance += amount;
        _stakers[msg.sender].usdtBalance += amountsOut[1];
    ...
    function withdraw() external nonReentrant {
        ...
        require(_stakers[msg.sender].balance > 0, "Cannot claim 0 token");
    ...
    function setSpecialWallet(address wallet, bool status) external onlyOwner {
        require(
            specialWallets[wallet] != status,
            "Special Wallet already on that status"
        );

        specialWallets[wallet] = status;
        emit SpecialWalletUpdated(wallet, status);
    }

    function isSpecialWallet(address wallet) external view returns (bool) {
        return specialWallets[wallet];
    }
}