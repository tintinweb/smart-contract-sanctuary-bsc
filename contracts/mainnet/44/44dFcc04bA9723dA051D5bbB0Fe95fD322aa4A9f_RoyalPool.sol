...
contract RoyalPool is Ownable, Pausable {
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint256 shares; // number of shares for a user.
        uint256 lastDepositedTime; // keep track of deposited time for potential penalty.
        uint256 royalAtLastUserAction; // keep track of royal deposited at the last user action.
    ...
    event NewVRoyalContract(address VRoyal);
    ...
    /**
     * @notice Constructor
     * @param _token: Royal token contract
     * @param _masterchef: MasterChef contract
     * @param _admin: address of the admin
     * @param _treasury: address of the treasury (collects fees)
     * @param _operator: address of operator
     * @param _pid: royal pool ID in MasterChef
     */
    constructor(
        IERC20 _token,
    ...
    function init(IERC20 dummyToken) external onlyOwner {
        ...
        masterchef.deposit(royalPoolPID, balance);
        emit Init();
    }
    ...
    /**
     * @notice Checks if the msg.sender is either the royal owner address or the operator address.
     */
    modifier onlyOperatorOrRoyalOwner(address _user) {
        require(msg.sender == _user || msg.sender == operator, "Not operator or royal owner");
        _;
    }
    ...
    function updateBoostContractInfo(address _user) internal {
        if (boostContract != address(0)) {
            ...
            IBoostContract(boostContract).onRoyalPoolUpdate(
                _user,
                user.lockedAmount,
                lockDuration,
                totalLockedAmount,
    ...
    function updateUserShare(address _user) internal {
        ...
        if (user.shares > 0) {
            if (user.locked) {
                ...
                totalShares -= user.shares;
                user.shares = 0;
                uint256 earnAmount = totalAmount - user.royalAtLastUserAction;
    ...
    /**
     * @notice Unlock user royal funds.
     * @dev Only possible when contract not paused.
     * @param _user: User address
     */
    function unlock(address _user) external onlyOperatorOrRoyalOwner(_user) whenNotPaused {
        UserInfo storage user = userInfo[_user];
    ...
    /**
     * @notice Deposit funds into the Royal Pool.
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in ROYAL)
    ...
    function depositOperation(
        ...
        if (VRoyal != address(0)) {
            IVRoyal(VRoyal).deposit(_user, _amount, _lockDuration);
        }
        ...
        user.royalAtLastUserAction = (user.shares * balanceOf()) / totalShares - user.userBoostedShare;
    ...
    /**
     * @notice Withdraw funds from the Royal Pool.
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (VRoyal != address(0)) {
            IVRoyal(VRoyal).withdraw(msg.sender);
        }
        ...
        if (user.shares > 0) {
            user.royalAtLastUserAction = (user.shares * balanceOf()) / totalShares;
        } else {
            user.royalAtLastUserAction = 0;
        }
    ...
    /**
     * @notice Harvest pending ROYAL tokens from MasterChef
     */
    function harvest() internal {
        uint256 pendingRoyal = masterchef.pendingRoyal(royalPoolPID, address(this));
        if (pendingRoyal > 0) {
            uint256 balBefore = available();
            masterchef.withdraw(royalPoolPID, 0);
    ...
    /**
     * @notice Set VRoyal Contract address
     * @dev Callable by the contract admin.
     */
    function setVRoyalContract(address _VRoyal) external onlyAdmin {
        require(_VRoyal != address(0), "Cannot be zero address");
        VRoyal = _VRoyal;
        emit NewVRoyalContract(VRoyal);
    }
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Royal Pool
     */
    ...
    function calculatePerformanceFee(address _user) public view returns (uint256) {
        ...
        if (user.shares > 0 && !user.locked && !freePerformanceFeeUsers[_user]) {
            uint256 pool = balanceOf() + calculateTotalPendingRoyalRewards();
     ...
     * @return Returns total pending royal rewards
     */
    function calculateTotalPendingRoyalRewards() public view returns (uint256) {
        uint256 amount = masterchef.pendingRoyal(royalPoolPID, address(this));
        return amount;
    }

    function getPricePerFullShare() external view returns (uint256) {
        return totalShares == 0 ? 1e18 : (((balanceOf() + calculateTotalPendingRoyalRewards()) * (1e18)) / totalShares);
    }
