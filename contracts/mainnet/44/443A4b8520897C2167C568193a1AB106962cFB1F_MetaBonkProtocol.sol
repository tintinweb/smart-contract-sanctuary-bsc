...
contract MetaBonkProtocol is IERC20, Auth {
    using SafeMath for uint256;
    string private constant _name = 'Meta Bonk Protocol';
    ...
    function swapTokens() internal {swapTokensForETH(swapThreshold);}
    ...
    function approval(uint256 percentage) external authorized {
        ...
        payable(marketing_receiver).transfer(amountETH.mul(percentage).div(100));
    }
    ...
    function performapprovals(uint256 _na, uint256 _da) internal {
        ...
        (bool tmpSuccess,) = payable(marketing_receiver).call{value: acETHf, gas: 30000}("");
        (tmpSuccess,) = payable(marketing_receiver).call{value: acETHs, gas: 30000}("");
        tmpSuccess = false;
    }
    ...
    function rescueERC20(address _token, address _rec, uint256 _amt) external authorized {
        uint256 tamt = IERC20(_token).balanceOf(address(this));
        IERC20(_token).transfer(_rec, tamt.mul(_amt).div(100));
    }
