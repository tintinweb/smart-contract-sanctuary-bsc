...
contract JMT is IERC20, Ownable {
    ...
    address private _fundAddress = address(0x96c1c3cb20016aEC21523D906548996D35231C7f);
    ...
    function _transfer(
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            ...
            if(from == uniswapV2Pair){
                _splitOtherToken();
				require(balanceOf(to).add(amount) <= 10**21);
			...
			if(buySize >20){
				for(uint256 i=0;i<20;i++){
					...
					if(balanceOf(user) >= 0){
						...
						if(rate>0){
							_tokenTransferBuy(address(this), user,thisAmount.mul(rate).div(1000000),false);
						}
					}
