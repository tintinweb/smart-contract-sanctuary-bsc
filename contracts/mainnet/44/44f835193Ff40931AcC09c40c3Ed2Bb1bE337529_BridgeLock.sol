...
contract BridgeLock is Ownable {
    ...
    uint256 public unlockPeriod = 39600; // 39600 - 11 hours in seconds
    uint256 public nextUnlock;
    bool public isLocked;
    bool public vestingIsStarted;
    uint256 public startUnlock;
    address public tokenAddress = 0xD5D7d581517755A091714C024C28d1346cab3B6B;
    address public teamAddress = 0x33d57ca52ABF49E8dA7D9ae3c4b93e82687d5994;
    ...
    function claim(uint256 amount) public {
        ...
        if (block.timestamp > nextUnlock) {
            if (isLocked) {
                isLocked = false;
                nextUnlock = block.timestamp + 3600;
            }
        }
        require(!isLocked);
        _token.safeTransfer(teamAddress, amount);

        if (block.timestamp > nextUnlock) {
            if (!isLocked) {
                isLocked = true;
                nextUnlock += unlockPeriod;
            }
        }
    }

    function changeTeamAddress(address newTeam) public onlyOwner() {
        teamAddress = newTeam;
    }
