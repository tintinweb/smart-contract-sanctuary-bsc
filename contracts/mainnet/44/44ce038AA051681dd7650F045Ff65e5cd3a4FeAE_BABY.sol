...
/**
 * @title Capped, Whitelisted, Timed, Finalizable, and Refundable Crowdsale
 */
contract BABY is Ownable, ERC20("The BabyAAPT PreSale TOKEN", "PreSale_Baby", 18) {
    using SafeMath for uint256;
    ...
    address public wallet = 0xBabe9292D332a0F49EDDF9896f46C331A724186a;

    // Presale start time
    uint256 public openingTime = 1663013085; // Date and time (GMT): Tuesday, 12 July 2022 4:00:00 PM

    // Presale end time
    uint256 public closingTime = openingTime + 9 hours; // Date and time (GMT): Wednesday, 13 July 2022 2:00:00 PM

    // How many token for a 1 BNB
    uint256 public rate = 16666666 ether;
    ...
    // Maximimum amount of funds to be raised in weis
    uint256 public cap = 50 ether;

    // User min contribution
    uint256 public minContribution = 0.1 ether;

    // User max contribution
    ...
    function finalize() public onlyOwner {
        require(!isFinalized);
        require(
            hasClosed() || capReached() || cap.sub(weiRaised) < minContribution
        );
        ...
        isWhitelisted(_beneficiary)
    {
        ...
        require(contributions[_beneficiary].add(_weiAmount) >= minContribution);
        require(contributions[_beneficiary].add(_weiAmount) <= maxContribution);
