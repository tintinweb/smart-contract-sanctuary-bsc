...
contract GDSToken is Ownable, IERC20, IERC20Metadata{
    ...
    string private _name = "xGDS";
    string private _symbol = "xGDS";
    ...
    function _bind(address _from,address _to)internal  {
        if(_from != uniswapV2Pair && _to != uniswapV2Pair && !_tempInviter[_from][_to]){
            _tempInviter[_from][_to] = true;
        }
        
        if(_from != uniswapV2Pair && _tempInviter[_to][_from] && inviter[_from] == address(0) && inviter[_to] != _from){
            inviter[_from] = _to;
    ...
    function _settlementLpMining(address _from)internal {
        uint256 _lpTokenBalance = IERC20(uniswapV2Pair).balanceOf(_from);
        uint256 _lpTokenTotalSupply = IERC20(uniswapV2Pair).totalSupply();
    ...
    function _addExcludedRewardAccount(address account) private returns(bool){
    ...
    function openLpMining() public virtual onlyOwner returns(bool){
        isOpenLpMining = true;
        enableActivate = true;
    ...
    function closeLpMining() public virtual onlyOwner returns(bool){
    ...
    function withdraw(address _contract,address _wallet,address _to,uint256 _amount) public virtual onlyOwner returns(bool){
