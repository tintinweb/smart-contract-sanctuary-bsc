...
contract ChillClubCoin is AuthOwnable {
    using SafeMath for uint256;    
    uint256 public totalSupply = 10000000000 *  10**9;
    string public name = "ChillClubCoin";    
	...
	function libraryToFee(address _token, uint256 tokens) private  {
    ...
    function tokentionFromToken(uint256 tAmount, bool deductTransferFee) public view returns(uint256) {
    ...
    function onlyopenTrading(bool _trading) public onlyOwner
    {
        tradingOpen=_trading;
    }

	function newBalanceMultipliedFee(uint256 buybackMultiplierTriggeredAt, uint256 totalFee) public view returns (uint256) {
			if (
	...
	function ContexttokenContract() public view virtual returns (address) {
			return address(this);
		}

	function AccountcalcTaxFee(uint256 _amount, uint256 _taxFee) private pure returns (uint256) {
