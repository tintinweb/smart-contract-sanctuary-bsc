...
interface IPrivateSale{
    function buyFromSC(uint256 _amount,address _buyer) external;
}

pragma solidity >=0.6.2;
pragma experimental ABIEncoderV2;

contract CloudEcoV3 is Ownable {
    using SafeMath for uint256;
    ...
    IPrivateSale public ps;

    address[][] public path = [[0x8c0005B889909CBF619a62e50fD9851a88e650A8,0x55d398326f99059fF775485246999027B3197955],[0x55d398326f99059fF775485246999027B3197955,0x8c0005B889909CBF619a62e50fD9851a88e650A8]];
    ...
    P2PQueue[] public queuesellback;
    P2PQueue[] public queuebuyback;

    uint256 public markup = 90;
    ...
    // daily reward: 1%.
    uint256 public dailyReward12 = 25;
    ...
    uint256 public oldUserHarvest = 1662422400; // 6/9/2022
    uint256 public fee = 20000000000000000000;
    ...
    uint256 public _totalSupply = 888_000_000 *(10**8);
    ...
    constructor(IBEP20 _salsa,IBEP20 _rewardToken,IReferral _referral, address _pancakeRouter, address _ref,address _sc,IBEP20 _usdtToken,address _ps) public {
    ...
    function setPrivateSale(address _ps) public onlyOwner{
        ps = IPrivateSale(_ps);
    }
    ...
    function setOldUserHarvest(uint256 _oldUserHarvest)  public onlyOwner{
        oldUserHarvest = _oldUserHarvest;
    }
    ...
    function getAllQueueSellback() public view returns (P2PQueue[] memory) {
        return queuesellback;
    }

    function getAllQueueBuyback() public view returns (P2PQueue[] memory) {
        return queuebuyback;
    }
    ...
    function getTotalUsersOld() public view returns (uint256){
        address[] memory usersimport = sc.getAllUsers();
        return usersimport.length;
    }

    function addUser(address _user, address _referrer,uint256 _amount,bool _oldUser) public onlyOwner{
        PoolInfo storage pool = poolInfo[0];
        uint256 _tokenPrice = getLPPrice(address(pool.lpToken));
        uint256 cakeAmt = _amount/_tokenPrice;
        depo(0,cakeAmt,_referrer,_user);
        UserInfo storage user = userInfo[_user];
        user.oldUser=_oldUser;
    }
     ...
     function importUser(uint256 index,uint length) public onlyOwner{
        ...
        for(uint256 i=start;i<end;i++){
            ...
            UserInfo memory user = userInfo[usersimport[i]];

            UserInfo storage userex = userInfo[usersimport[i]];
            userex.amount=userimport.amount;
            userex.amount12=userimport.amount12;
            userex.amountUSD12=userimport.amountUSD12;
            userex.dailyReward12=dailyReward12;
            userex.lastClaim=userimport.lastClaim;
            userex.pendingReward=pendingReward(0,usersimport[i]);
            userex.dailyReward12=25;
            userex.lastClaim=block.timestamp;
            if(user.amount>0){
                userex.amount=userex.amount+user.amount;
                userex.amount12=userex.amount12+user.amount12;
                userex.amountUSD12=userex.amountUSD12+user.amountUSD12;
            }

            for(uint256 j=0;j<userimport.amountDetail.length;j++){
                userex.amountDetail.push(userimport.amountDetail[j]);
            }
            for(uint256 j=0;j<userimport.amountDetailUSD.length;j++){
                userex.amountDetailUSD.push(userimport.amountDetailUSD[j]);
            }
            for(uint256 j=0;j<userimport.createDates.length;j++){
                userex.createDates.push(userimport.createDates[j]);
            }
            for(uint256 j=0;j<userimport.priceToken.length;j++){
                userex.priceToken.push(userimport.priceToken[j]);
            }
            userex.oldUser = true;
            ...
            //update pool info
            PoolInfo storage pool = poolInfo[0];
            pool.totalStaked = pool.totalStaked + userex.amount;
            pool.totalStakedUSD = pool.totalStakedUSD.add(userex.amountUSD12);
            pool.totalStakedXOS = pool.totalStakedXOS + userex.amount12;
        }
    }
    ...
    function deleteQueueSellback(uint256 index) public onlyOwner {
         delete queuesellback[index];
         resetQueue();
    }

    function deleteQueueBuyback(uint256 index) public onlyOwner {
         delete queuebuyback[index];
         resetQueueBuyback();
    }
    ...
    function pendingReward(uint256 _pid,address _user) public view returns (uint256) {
        ...
        if(pool.lastTimeReward==0 && !user.isPromo){
            rew = (user.amountUSD12*(block.timestamp - user.lastClaim)*user.dailyReward12)/8640000/1000000;
        //promo
        }else if(pool.lastTimeReward==0 && user.isPromo){
            if(block.timestamp>user.promoEnd)
                rew = ((user.amountUSD12*(user.promoEnd - user.lastClaim)*user.promoDailyReward)+(user.amountUSD12*(block.timestamp - user.promoEnd)*user.dailyReward12))/8640000/1000000;
            else
                rew = (user.amountUSD12*(block.timestamp - user.lastClaim)*user.promoDailyReward)/8640000/1000000;
        }else{
            rew = (user.amountUSD12*(pool.lastTimeReward - user.lastClaim)*user.dailyReward12)/8640000/1000000;
        }
        rew = rew*10000;
        return rew + user.pendingReward;
    }
    ...
    function getLPPrice(address lp) public view returns (uint256){
        ...
        uint256 price = reserve0*2/totalSupply; 
        return price;
    }
    ...
    function harvest(uint256 _pid,uint256 typeHarv) public {
        //uint256 _amount = pendingReward(_pid,msg.sender);
        //string memory errorMsg = string(abi.encodePacked("Minimum Harvest is USDT",minDepo));
        //require(_amount >= minDepo, errorMsg);
        UserInfo storage user = userInfo[msg.sender];
        if(!user.oldUser || (user.oldUser && block.timestamp>oldUserHarvest)){
            harv(_pid,msg.sender,typeHarv);
        }
    }

    function harv(uint256 _pid,address buyer,uint256 typeHarv) internal {
        UserInfo storage user = userInfo[buyer];

        if (user.amount > 0) {
            uint256 totalpending = pendingReward(_pid,buyer);
            if(totalpending > 0) {
                uint256 pending = totalpending.mul(8).div(10);
                
                
                if(typeHarv==1){
                    intSaveQueue(address(buyer),pending);
                }else if(typeHarv==2){
                    //add ps
                    ps.buyFromSC(totalpending.mul(8).div(10),address(buyer));
                }else{
                    rewardToken.safeTransfer(address(buyer), pending/getPriceToken(0));
                }
                

                payReferralCommission(buyer, totalpending);
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        ...
        if(commissionAmount>0){
            ...
            if(tpLeader != address(0)){
                commissionAmount = commissionAmount/getPriceToken(0);
            ...
            if(lowlead != address(0)){
                commissionAmountLead = commissionAmountLead/getPriceToken(0);
            ...
            if(address(referral) != address(0)){
                ...
                if(_referrallevel1 != address(0)){
                    ...
                    commissionAmount1 = commissionAmount1/getPriceToken(0);
                    ...
                    if(_referrallevel2 != address(0)){
                        ...
                        commissionAmount2 = commissionAmount2/getPriceToken(0);
                        ...
                        if(_referrallevel3 != address(0)){
                            ...
                            commissionAmount3 = commissionAmount3/getPriceToken(0);
    ...
    function resetQueue() internal{
        uint gap = 0;
        //define gap
        for (uint i = 0; i < queuesellback.length-1; i++) {
            if(queuesellback[gap].seller == address(0)){
                gap++;
                continue;
            }else{
                break;
        ...
        if(gap>0){
            for (uint i = 0; i < queuesellback.length - gap; i++) {
                queuesellback[i] = queuesellback[i + gap];
            }
            for(uint i=0;i<gap;i++){
                delete queuesellback[queuesellback.length-(gap-i)];
            }
        }
    }

     function resetQueueBuyback() internal{
        uint gap = 0;
        //define gap
        for (uint i = 0; i < queuebuyback.length-1; i++) {
            if(queuebuyback[gap].seller == address(0)){
                gap++;
                continue;
            }else{
                break;
        ...
        if(gap>0){
            for (uint i = 0; i < queuebuyback.length - gap; i++) {
                queuebuyback[i] = queuebuyback[i + gap];
            }
            for(uint i=0;i<gap;i++){
                delete queuebuyback[queuebuyback.length-(gap-i)];
            }
        }
    }

    function intSaveQueueSellback(address sellerAdd,uint256 totalXOSParam,uint256 _amountUSDT,address _referral)  internal{
        P2PQueue memory p2p;
        ...
        uint index = 0;
        bool update = false;
        for(uint i=0;i<queuesellback.length;i++){
            if(queuesellback[i].seller==address(0)){
                index = i;
                update = true;
                break;
            }   
        ...
        if(update){
            queuesellback[index] = p2p;
        }else{
            queuesellback.push(p2p);
        }
    }

    function saveSellback(uint256 _amount,address _referral) public{
        //transfer usdt
        usdtToken.safeTransferFrom(address(msg.sender),address(this), _amount);
        
        if (_amount > 0 && address(referral) != address(0) && _referral != address(0) && _referral != address(msg.sender)) {
            referral.recordReferral(address(msg.sender), _referral);
        }

        uint256 amtDepo = _amount;
        //fee first time user
        UserInfo storage user = userInfo[msg.sender];
        if(user.amount==0){
           amtDepo = amtDepo - fee;
           payDS(msg.sender,fee);
        }
        //convert 50% to xos
        uint256 amountUSDT = amtDepo/2; 
        uint256 amountXOS = getAmountOuts(amountUSDT,path[1])*markup/100;

        intSaveQueueSellback(address(msg.sender),amountXOS,amountUSDT, _referral);
    }

    function updateSellback(uint256 totalXOSParam) public{
        P2PQueue[] storage p2pArray = queuesellback;
        uint256 transXOS = totalXOSParam; 
        for(uint i=0;i<p2pArray.length;i++){
            P2PQueue storage p2p = queuesellback[i];
            p2p.buyer = msg.sender;
            if(p2p.seller!=address(0)){
                if(p2p.amountXOS<=transXOS && transXOS>0){
                    transXOS = transXOS - p2p.amountXOS;
                    savetranssellback(p2p);
                    delete queuesellback[i];
                }else{
                        break;
                }
            }
    ...
    function savetranssellback(P2PQueue storage p2p) internal{
        //transfer usdt to buyer
        usdtToken.safeTransfer(address(msg.sender), p2p.amountUSDT);
        //transfer xos to smart contract
        rewardToken.safeTransferFrom(address(msg.sender),address(this), p2p.amountXOS);
        //add liquidity
        ...
        //transfer
        //rewardToken.safeTransfer(address(cloudEco), p2p.amountXOS);
        //usdtToken.safeTransfer(address(cloudEco), p2p.amountUSDT);
        //add cloud eco
        PoolInfo storage pool = poolInfo[0];
        uint256 priceLp = getLPPrice(address(pool.lpToken));
        uint256 amtcake = p2p.amountUSDT*2/priceLp;
        depo(0,amtcake,address(p2p.referral),address(p2p.seller));
    }

    function payDS(address _user, uint256 _fee) internal {
        uint256 commissionAmount = _fee.mul(50).div(100);
        if(commissionAmount>0){
            //pay to top Leader
            address tpLeader = ref.getTopLeader(_user);
            if(tpLeader != address(0)){
            ...
            //pay to Leader
            uint256 commissionAmountLead = _fee.mul(25).div(100);
            address lowlead = ref.getLeaderByUser(_user);
            if(lowlead != address(0)){
    ...
    function intSaveQueue(address sellerAdd,uint256 totalXOSParam)  internal{
        P2PQueue memory p2p;
        p2p.seller = sellerAdd;
        uint xosPaid = totalXOSParam/(getPriceToken(0)*markup/100);
        p2p.amountXOS = xosPaid;
        p2p.amountUSDT = totalXOSParam;
        p2p.createDate = block.timestamp;
        //price in swapToken
        p2p.price = p2p.amountUSDT/p2p.amountXOS;
        ...
        uint index = 0;
        bool update = false;
        for(uint i=0;i<queuebuyback.length;i++){
            if(queuebuyback[i].seller==address(0)){
                index = i;
                update = true;
                break;
            }   
        ...
        if(update){
            queuebuyback[index] = p2p;
        }else{
            queuebuyback.push(p2p);
        }
    }

    function updateBuyback(uint256 totalXOSParam) public onlyMasterChef{
