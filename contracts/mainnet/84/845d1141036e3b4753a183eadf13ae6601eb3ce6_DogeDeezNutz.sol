...
contract BUSDDividendTracker is DividendPayingToken  {
    ...
    constructor(address _dividentToken) DividendPayingToken("DDZ_Busd_Dividend_Tracker", "DDZ_Busd_Dividend_Tracker",_dividentToken) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 1 * (10**18); //must hold 1 tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "DDZ_Busd_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "DDZ_Busd_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Boda contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DDZ_Busd_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DDZ_Busd_Dividend_Tracker: Cannot update claimWait to same value");
...
contract DogeDeezNutz is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 private maxBuyTranscationAmount = 10000000 * (10**18);
    uint256 private maxSellTransactionAmount = 10000000 * (10**18);
    ...
    constructor() ERC20("DogeDeezNutz", "DDZ") {
        	
    	marketingWallet = 0x4F87AE07D056f722649b98137Caf094643dA038F ; 
        devWallet = 0x4F87AE07D056f722649b98137Caf094643dA038F;
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "DOZ: The marketing wallet is already this address");
 ...
 function updatedevWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != devWallet, "DOZ: The dev wallet is already this address");
        excludeFromFees(_newWallet, true);
    ...
    function setBuyFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _rewardFee, uint256 _devFee ) public onlyOwner {
        _marketingFeeBuy = _marketingFee;
        _LpFeeBuy = _liquidityFee;
        _busdDividendRewardsFeeBuy = _rewardFee;
        _devFeeBuy = _devFee;
       
    }
    function setSellFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _rewardFee, uint256 _devFee ) public onlyOwner {
        _marketingFeeSell = _marketingFee;
        _LpFeeSell = _liquidityFee;
        _busdDividendRewardsFeeSell = _rewardFee;
        _devFeeSell = _devFee;
    ...
    function updatebusdDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(busdDividendTracker), "smh: The dividend tracker already has that address");
 
        BUSDDividendTracker newbusdDividendTracker = BUSDDividendTracker(payable(newAddress));
 
        require(newbusdDividendTracker.owner() == address(this), "smh: The new dividend tracker must be owned by the Boda token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "smh: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "smh: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "smh: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "smh: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "smh: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "smh: Trading has not started yet");
        ...
        if(takeFee) {
            ...
            fees = tmpMarketingRewardPercent.add(tmpDevRewardPercent).add(tmpBusdDividedRewardsInContract)
