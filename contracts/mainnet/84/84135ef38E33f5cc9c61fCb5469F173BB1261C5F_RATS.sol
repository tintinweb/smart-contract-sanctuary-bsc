...
contract RATS is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    function mint(uint256 _mintAmount) public payable {
        // Checks
        require(!paused, "RATS: minting has not started yet");
        require(_mintAmount > 0, "RATS: you have to mint at least one");
        require(
            _mintAmount <= maxMintAmount,
            "RATS: you can only mint 10 at a time"
        );
        require(
            lastSupply >= _mintAmount,
            "RATS: the collection is sold out"
        );
        if (msg.sender != owner()) {
            require(
                msg.value >= cost * _mintAmount,
                "RATS: total cost isn't match"
            );
        }
        // Minting
    ...
    {
        // Checks
        require(_winner != address(0), "RATS: zero address");
        require(_amount > 0, "RATS: you have to mint at least one");
        require(lastSupply != 0, "RATS: the collection is sold out");
  ...
  */
    ...
    function claimRewards() public {
        require(!paysInTokens, "RATS: rewards are paid in tokens");
