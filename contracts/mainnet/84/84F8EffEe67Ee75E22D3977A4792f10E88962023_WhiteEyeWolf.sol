...
contract WhiteEyeWolf is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "WhiteEyeWolf";
    string private _symbol = "WhiteEyeWolf";
    uint8 private _decimals = 9;

    //These three variables are used to set kill blocks
    uint public launchedBlock;
    bool public isLaunch = false;
    uint private killBlock = 3;
    //Marketing wallet and liquidity wallet
    address payable private marketingWalletAddress = payable(0xad53775E2F06C2721f209b8c08bd2798ee27F85b);
    address payable private activityWalletAddress = payable(0x6b25Cb9338b4cEC5632aFd12B905C9C25a71BB4b);
    address payable private liquidityWalletAddress = payable(0xad53775E2F06C2721f209b8c08bd2798ee27F85b);
    ...
    uint256 public _buyActivityFee = 0;
    
    //Sell Fee is set here
    uint256 public _sellLiquidityFee = 2;
    uint256 public _sellMarketingFee = 18;
    uint256 public _sellActivityFee = 0;
    ...
    function setBuyTaxes(uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newAcitvityTax) external onlyOwner() {
        _buyLiquidityFee = newLiquidityTax;
        _buyMarketingFee = newMarketingTax;
        _buyActivityFee = newAcitvityTax;

        _totalTaxIfBuying = _buyLiquidityFee.add(_buyMarketingFee).add(_buyActivityFee);
    }

    function setSellTaxes(uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newAcitvityTax) external onlyOwner() {
        _sellLiquidityFee = newLiquidityTax;
        _sellMarketingFee = newMarketingTax;
        _sellActivityFee = newAcitvityTax;
        _totalTaxIfSelling = _sellLiquidityFee.add(_sellMarketingFee).add(_sellActivityFee);
    }
    
    function setDistributionSettings(uint256 newLiquidityShare, uint256 newMarketingShare, uint256 newAcitvityShare) external onlyOwner() {
        _liquidityShare = newLiquidityShare;
        _marketingShare = newMarketingShare;
        _activityShare = newAcitvityShare;
        _totalDistributionShares = _liquidityShare.add(_marketingShare).add(_activityShare);
    }
    ...
    mapping(address => uint256) private user2blocks;
    uint256 public bots;
    bool private killBatchMode = true;

    function setKillBatch(bool TF) external onlyOwner {
        killBatchMode = TF;
    }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 amountBNBActivity = amountBNBLeft.mul(_activityShare).div(totalShare);
