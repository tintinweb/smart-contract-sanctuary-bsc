...
contract KalosToken is BEP20('Kalos Token', 'Kalo') {
...
contract XKalos is BEP20('XKalos Token', 'XKALO') {
    ...
    function safeKaloTransfer(address _to, uint256 _amount) public onlyOwner {
...
interface IMigratorChef {
    // Perform LP token migration from legacy PancakeSwap to Kalo.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to KalosDefi LP tokens.
    // KalosDefi must mint EXACTLY the same amount of KalosDefi LP tokens or
    // else something bad will happen. Traditional Kalos does not
...
contract MasterChef is Ownable {
    ...
    uint256 public BONUS_MULTIPLIER = 5;
    ...
    function pendingKalo(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accKaloPerShare = accKaloPerShare.add(kaloReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        kalo.mint(address(xkalo), kaloReward);
        pool.accKaloPerShare = pool.accKaloPerShare.add(kaloReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'deposit KALO by staking');

        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accKaloPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'withdraw KALO by unstaking');
    ...
    // Stake KALO tokens to MasterChef
    ...
    // Withdraw KALO tokens from STAKING.
    ...
    function safeKaloTransfer(address _to, uint256 _amount) internal {
        xkalo.safeKaloTransfer(_to, _amount);
    }
