...
contract MasterChefV3Keeper is KeeperCompatibleInterface, Ownable, Pausable {
    ...
    uint256 public bufferSecond = 10 minutes;
    // Avoid re-execution caused by duplicate transactions.
    uint256 public upkeepBufferSecond = 11 hours;
    ...
    constructor(IMasterChefV3 _V3, IReceiver _receiver) {
        MasterChefV3 = _V3;
        Receiver = _receiver;
    }
    ...
    function checkUpkeep(bytes calldata) external view override returns (bool upkeepNeeded, bytes memory) {
        if (!paused()) {
            ...
            if (latestPeriodEndTime < block.timestamp + bufferSecond) upkeepNeeded = true;
        }
    }
