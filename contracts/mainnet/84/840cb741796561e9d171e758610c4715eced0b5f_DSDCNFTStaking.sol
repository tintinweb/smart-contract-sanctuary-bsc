...
abstract contract NFTStaking is ReentrancyGuard, Ownable {
    ...
    function stake(uint256 _stakingId, uint256[] memory tokenIds)
        public
        virtual;

    function unstake(uint256 _stakingId, uint256[] memory tokenIds)
        public
        virtual;

    function getEarnedRewards(uint256 _stakingId, address account)
        public
        view
        virtual
    ...
    function getStakedTokens(uint256 _stakingId, address _account)
        public
        view
        virtual
...
contract DSDCNFTStaking is NFTStaking {
    ...
    uint256 minAPR = 10;
    // the amount available to withdraw after tax
    uint16 earlyWithdrawPercentage = 10;

    constructor(address _stakingToken, address _rewardToken)
        NFTStaking(_stakingToken, _rewardToken)
    {
        plans[0].apr = 2433333333000000000000000;
        plans[0].stakeDuration = 0 days;

        plans[1].apr = 12166667000000000000000000;
    ...
    function stake(uint256 _stakingId, uint256[] memory tokenIds)
        ...
        override
    {
        require(_stakingId < planLimit, "Staking plan unavailable");
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                IERC721(stakingToken).ownerOf(tokenIds[i]) == msg.sender,
                "You can only stake NFTs which you own"
            );
            _staking.tokenIds.add(tokenIds[i]);
            _staking.stakeAt[tokenIds[i]] = block.timestamp;
            _staking.endstakeAt[tokenIds[i]] =
                block.timestamp +
                plan.stakeDuration;
    ...
    function getEarnedRewards(uint256 _stakingId, address _account)
        public
        view
        override
        returns (uint256, uint256)
    {
        ...
        for (uint256 i = 0; i < _staking.tokenIds.length(); i++) {
            ...
            uint256 stakeTime = block.timestamp - _staking.stakeAt[tokenId];
            uint256 earned = plan.apr.div(100).mul(stakeTime).div(periodicTime);
            if (block.timestamp >= _staking.endstakeAt[tokenId]) {
                _canClaim = _canClaim.add(earned);
            } else {
                _canClaim = _canClaim.add(
                    (earned * earlyWithdrawPercentage) / 100
                );
            }
            _earned = _earned.add(earned);
        }
        ...
        override
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                _staking.tokenIds.contains(tokenIds[i]),
                "Query for a token you haven't staked"
            );
                ...
                _rewards =
                    (_rewards.add(
                        plan
                            ...
                            .div(100)
                            .mul(
                                block.timestamp - _staking.stakeAt[tokenIds[i]]
                            )
                            .div(periodicTime)
    ...
    {
        Staking storage _staking = stakes[_stakingId][_account];
        uint256[] memory stakedTokenIds = new uint256[](
            _staking.tokenIds.length()
        );
    ...
    function setEarlyWithdrawPercentage(uint16 _percent) external onlyOwner {
        require(_percent <= 95);
        earlyWithdrawPercentage = _percent;
    }
