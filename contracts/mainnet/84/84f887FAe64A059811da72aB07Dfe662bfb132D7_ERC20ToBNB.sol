...
contract ERC20ToBNB is EmergencyWithdrawable {
    ...
    constructor(address _router, address _target, address[] memory _path) {
        ...
        path = _path;
        source = IERC20(_path[0]);
        require(_path[_path.length-1] == router.WETH(), "path needs to end in WBNB");

        source.approve(_router, type(uint256).max);
    }

    function setRoute(address _router, address[] memory _path) external onlyOwner {
        require(_path.length > 1, "Invalid path");
        require(_router != address(0), "Invalid router");

        source.approve(address(router), 0);
    ...
    function swap(uint256 minOutAmount) external {
        router.swapExactTokensForETH(source.balanceOf(address(this)), minOutAmount, path, target, block.timestamp);
    }

    function pending() external view returns (uint256) {
        uint256[] memory amounts = router.getAmountsOut(source.balanceOf(address(this)), path);
