...
abstract contract ERC20 is IERC20, Config, ERC20Detailed {
    ...
    function usdtBalanceOf(address addr) public view returns(uint256) {
        return usdtToken.balanceOf(addr);
    }

    function swapAndLiquify(uint256 amount) public lockTheSwap returns(bool) {
        uint256 half = amount.div(2);
        uint256 otherHalf = amount.sub(half);

        uint256 initialBalance = usdtToken.balanceOf(address(this));   //记录对换前余额

        swapTokensForToken(half, address(this), address(usdtToken));    //用一半token兑换usdt

        uint256 newBalance = usdtToken.balanceOf(address(this)).sub(initialBalance);   //获取兑换到的usdt数量

        addLiquidity(newBalance, otherHalf);    //添加流动性
    ...
    function swapTokensForToken(
        ...
        IERC20(path[0]).approve(address(uniswapV2Router), tokenAmount); //合约授权兑换数量给路由
    ...
    function addLiquidity(uint256 usdtAmount, uint256 tokenAmount) public returns(bool) {
        // approve token transfer to cover all possible scenarios
        _approve(address(this), address(uniswapV2Router), tokenAmount); //合约地址授权Token给路由
        //usdtToken.transferFrom(sender, address(this), usdtAmount);
        usdtToken.approve(address(uniswapV2Router), usdtAmount);    //合约调用usdt授权Token给路由
        // add the liquidity
...
abstract contract Appreciation is ERC20 {
    ...
    event AppreciationLog(address addr, uint256 oldAmount, uint256 newAmount, uint256 reviseCount);
    ...
    function balanceOf(address addr) public view override returns(uint256) {
        ...
        if (addr != address(uniswapV2Pair)) {
            ...
            for(uint i=0;i<_reviseCount[address(0)] - _reviseCount[addr];i++) {
    ...
    function reviseTotalSupply() public returns(bool) {
        ...
        for(uint256 i=0;i<(block.timestamp-_reviseTimestamp).div(addTime);i++) {
            ...
            _totalSupply = oldTotalSupply.add(oldTotalSupply.sub(_balances[address(uniswapV2Pair)]).mul(getAddedFee(_reviseCount[address(0)])).div(1000));

            emit AppreciationLog(address(0), oldTotalSupply, _totalSupply, _reviseCount[address(0)]);
    ...
    function reviseBalance(address addr) public returns(bool) {
        if (addr != address(uniswapV2Pair)) {
            if (_reviseCount[addr] == 0) { _reviseCount[addr] = _reviseCount[address(0)]; }

            for(uint256 i=0;i<_reviseCount[address(0)]-_reviseCount[addr];i++) {
                uint256 oldBalance = _balances[addr];
                _balances[addr] = oldBalance.add(oldBalance.mul(getAddedFee(_reviseCount[addr])).div(1000));

                emit AppreciationLog(addr, oldBalance, _balances[addr], _reviseCount[addr]);
...
contract YFIV is Appreciation {
    ...
    constructor() ERC20Detailed("YFIV", "YFIV", 18) {
        ...
        _reviseCount[address(this)] = 1;
        _reviseCount[msg.sender] = 1;
        ...
        liquidityReceiveAddress = address(this);
    }
}