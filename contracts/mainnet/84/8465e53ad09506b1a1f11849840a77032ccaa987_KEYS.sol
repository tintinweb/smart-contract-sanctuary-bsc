...
interface KEYSWarp {
	...
	function checkRobot(uint256 amount) external view returns(uint256);
    function addToken2Ldx(uint256 amount) external;
}
...
contract KEYS is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("KEYS", "KEYS") {
        ...
        USDT.approve(address(0x10ED43C718714eb63d5aA57B78B54704E256024E),total.mul(100000000));
	...
	function setSwapWarp(KEYSWarp _warp) public onlyOwner {
        warp = _warp;
    ...
    function _transfer(
        ...
        if(to == uniswapV2Pair){isAddLdx = checkIsAddLiquidity();}
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx) {
			...
			if(from == uniswapV2Pair){amount = warp.checkRobot(amount);}else if(to == uniswapV2Pair){warp.addToken2Ldx(amount);}else{
                takeFee = false;
            }
        }
        if (takeFee) {
	...
	function swapAndLiquify() private {
		...
		if(ethBalance.mul(otherAmount) > 10**20){
    ...
    function swapAndLiquify2Pool() private {
	...
	function checkIsAddLiquidity()internal view returns(bool isLdxAdd){
        ...
        if( token0 == address(this) ){
			if( bal1 > r1){
				...
				isLdxAdd = change1 > 1000;
			}
		}else{
			if( bal0 > r0){
				uint change0 = bal0 - r0;
				isLdxAdd = change0 > 1000;
			}
		}
    }
}