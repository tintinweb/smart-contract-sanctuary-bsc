...
contract TestCNN is Ownable, ERC20 {
    using Address for address;
    mapping(address => bool) public W;
    mapping(address => bool) public B;
    mapping(address => bool) public mannger;
    ...
    constructor () ERC20('TEST CNN', 'TCNN'){
        mannger[_msgSender()] = true;
        _mint(msg.sender, 33000000 ether);
        W[msg.sender] = true;
    }
    modifier onlyManger(){
        require(mannger[msg.sender], 'not admin');
        _;
    }
    ...
    function setW(address addr, bool b) external onlyManger {
        W[addr] = b;
    }

    function setMannger(address addr, bool b) external onlyOwner {
        mannger[addr] = b;
    }

    function setPairs(address addr, bool b) external onlyOwner {
        pairs[addr] = b;
    }
    ...
    function setB(address addr, bool b) external onlyManger {
        B[addr] = b;
    }
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        if (!W[msg.sender] && !W[recipient]) {
            if (pairs[msg.sender]) {
                ...
                uint temp = amount * 2 / 100;
                _transfer(msg.sender, wallet, temp);
                amount -= temp;
            }
        }
