...
// Links: httpS://rightsledgerinc.com/
...
contract RightsTokenSale is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    address public tokenIn = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    address public tokenOut = 0x6e6B548AaA2f86f085cd3BFe6A6Cc5D61f409F36;
	address payable public ADMIN;

    bool public saleStarted = false;
	
    uint256 public swapRate = 1065;

    constructor(address payable _admin) ReentrancyGuard() {
        ADMIN = payable(_admin);
    }

    function buyRTKN(uint256 inAmount) external {
        uint256 quota = IERC20(tokenOut).balanceOf(address(this));
        uint256 total = IERC20(tokenIn).balanceOf(msg.sender);
        uint256 outAmount = inAmount.mul(swapRate);

        require(saleStarted == true, "RightsToken Sale: Sale not started.");
        require(inAmount <= total, "RightsToken Sale: Insufficient fund.");
        require(outAmount <= quota, "RightsToken Sale: Quota not enough.");

        IERC20(tokenIn).safeTransferFrom(msg.sender, address(this), inAmount);
        IERC20(tokenOut).safeTransfer(msg.sender, outAmount);
		
		IERC20(tokenIn).safeTransfer(ADMIN, inAmount);
