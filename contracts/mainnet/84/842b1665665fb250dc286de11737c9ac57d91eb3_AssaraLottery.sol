...
contract AssaraLottery is Context, Ownable{
    ...
    function _PayForLockedTokens(uint256 _TotalVal) private {
        ...
        for(uint64 i = 0; i < ticketCode; i++){
            ...
            tokenx.transfer((Tickets[i].member_addr), CalforpayWithdraw(i, _TotalLockedToken, _TotalVal));
    ...
    function CalforpayWithdraw(uint64 _tickCode, uint256 _TotalLockedTokens, uint256 _TotalVals) private view returns(uint256){
        uint256 _tokenPersonLocked = TokenLockedPerson[(Tickets[_tickCode].member_addr)];
        uint256 _PrizeStakePerson = _tokenPersonLocked.mul(10**12).div(_TotalLockedTokens).mul(_TotalVals).div(10**12);
        uint256 _SumTokensPay = _tokenPersonLocked.add(_PrizeStakePerson);
        return _SumTokensPay;
    }
