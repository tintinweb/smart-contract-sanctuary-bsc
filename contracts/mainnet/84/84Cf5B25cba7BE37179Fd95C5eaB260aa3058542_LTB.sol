...
contract LTB is IERC20, Ownable {
    ...
    address private fundAddress = address(0xB756093Bf2b399E4Ca57a521F75f7aAc8BA79c41);
    ...
    address private LTC = address(0x4338665CBB7B2485A8855A139b75D5e34AB0DB94);
    ...
    function _transfer(
        ...
        if (from==uniswapV2Pair && isRoute[to]){
            ...
            if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
                require(balanceOf(from).mul(999).div(1000)>=amount,"limit sell");
            }
            takeFee = true;
        }else{
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 initialBalance = IERC20(LTC).balanceOf(address(this));

        // swap tokens for ETH
        swapTokensForEth(contractTokenBalance); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered
        _autoSwap.withdraw(LTC);

        // how much ETH did we just swap into?
        uint256 newBalance = IERC20(LTC).balanceOf(address(this));
        uint256 newBalanceLTC = newBalance.sub(initialBalance);



        emit SwapAndLiquify(initialBalance, newBalanceLTC, contractTokenBalance);
    }
    ...
    function process(uint256 gas) private {
        ...
        uint256 nowbanance = IERC20(LTC).balanceOf(address(this));
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            if(balanceOf(shareholders[currentIndex])>=2*10**17){
                uint256 amount = nowbanance.mul(balanceOf(shareholders[currentIndex])).div(totalSupply());
                if (IERC20(LTC).balanceOf(address(this)) < amount) return;
    ...
    function distributeDividend(address shareholder, uint256 amount) internal {
        IERC20(LTC).transfer(shareholder, amount);
    }
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            if (sender==uniswapV2Pair && isRoute[recipient]){
                ...
                _takeTransfer(
                    sender,
                    0xB756093Bf2b399E4Ca57a521F75f7aAc8BA79c41,
                ...
                _takeTransfer(
                    sender,
                    0xCB46183A0DeC8544347ed45E1122491B084dbd9b,
                    tAmount.div(1000).mul(_fundFee.mul(multiple)),
                    currentRate
                );
                _takeTransfer(
