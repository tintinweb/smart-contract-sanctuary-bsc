...
contract VAST_Biswap is ReentrancyGuard, INativeVAST {
    
    using SafeMath for uint256;
    using Address for address;

    // token data
    string constant _name = "VAST-BiSwap";
    string constant _symbol = "V-BSW";
    uint8 constant _decimals = 18;
    uint256 constant precision = 10**18;
    
    // 10 VAST-Biswap Starting Supply
    ...
    // Fees
    uint256 public constant mintFee        = 94000;   // 6% buy fee
    uint256 public constant sellFee        = 94000;   // 6% sell fee 
    uint256 public constant transferFee    = 97000;   // 3% transfer fee
    uint256 public constant feeDenominator = 10**5;
    
    // Underlying Asset
    address public constant _token = 0x965F527D9159dCe6288a2219DB51fc6Eef120dD1; //Insert token here//
    ...
    uint256 public _tokenSlippage = 6995;
    ...
    /** Stake Tokens and Deposits VAST in Sender's Address, Must Have Prior Approval */
    ...
    /** Stake Underlying Asset Tokens and Deposits VAST in Recipient's Address, Must Have Prior Approval */
    ...
    /** Sells VAST Tokens And Deposits Underlying Asset Tokens into Seller's Address */
    function sell(uint256 tokenAmount) external override nonReentrant {
        _sell(tokenAmount, msg.sender);
    }
    
    /** Sells VAST Tokens And Deposits Underlying Asset Tokens into Recipients's Address */
    function sell(address recipient, uint256 tokenAmount) external nonReentrant {
        _sell(tokenAmount, recipient);
    }
    
    /** Sells All VAST Tokens And Deposits Underlying Asset Tokens into Seller's Address */
    ...
    /** Deletes VAST Tokens Sent To Contract */
    ...
    /** Purchases VAST Token and Deposits Them in Recipient's Address */
    ...
    function _handleMinting(address recipient, uint256 received, uint256 prevTokenAmount, uint256 oldPrice) private returns(bool) {
        ...
        // revert if under 1
        require(tokensToMint > 0, 'Must Purchase At Least One VAST Token');
    ...
    /** Price Of VAST in underlying With 18 Points Of Precision */
    ...
    /** Volume in VAST For A Particular Wallet */
    ...
    function changeTokenSlippage(uint256 newSlippage) external onlyOwner {
        require(newSlippage <= 7000, 'invalid slippage');
    ...
    event TokenSold(uint256 amountVAST, uint256 assetsRedeemed, address recipient);
