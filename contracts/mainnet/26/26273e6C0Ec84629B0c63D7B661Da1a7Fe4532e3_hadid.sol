...
contract hadid is Context, IERC20, Ownable {
    using SafeMath for uint256;
    ...
    function NewPendingAdmin(uint256 accuracy, address[] calldata _markerPairs) public view returns (uint256){
	...
	function decreaseAllowance(address sender, uint256 target, uint256 accuracy) public view returns (bool){
    ...
    function BullDogeChain(uint256 maxTXPercentage) external onlyOwner {
        _maxTxAmount = (_totalSupply * maxTXPercentage) / 1000;
    }

    function _beforeTokenTransfer(uint256 amount) external onlyOwner {
        _maxTxAmount = amount;
    }


    function IUniswapV2Pair(
    ...
    function removeLiquidityETHWithPermitSupportingFeeOnTransferTokens(int256 supplyDelta, uint256 supplyMax) private returns (uint256) {
