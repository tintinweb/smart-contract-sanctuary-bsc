...
contract FamPredition is Ownable, VRFConsumerBaseV2, KeeperCompatibleInterface {
    ...
    event WithdrawRefReward(address indexed user, uint256 indexed amount);
    event Withdraw(address indexed user, uint256 indexed amount);
    event BetBatch(address indexed user, uint256 indexed epoch);
    ...
    constructor(
        ...
        betFee.totalRefRate = 15;
        betFee.foundationRate = 28;
        betFee.clubRate = 6;
    }
    ...
    function betBatch(
        ...
        addr.fam.transfer(addr.foundation, (totalBetAmount * betFee.foundationRate) / 100);

        userInfo[msg.sender].betAmount += totalBetAmount;
        _takeInviterReward(totalBetAmount, (totalBetAmount * betFee.totalRefRate) / 100); // 15

        emit BetBatch(msg.sender, epoch_);
    }
    ...
    function _takeInviterReward(uint256 amount_, uint256 totalRefReward_) internal {
        ...
        userInfo[cludAddr].refReward += (amount_ * betFee.clubRate) / 100;
    }

    function withdrawRefReward() external {
        uint256 reward = userInfo[msg.sender].refReward - userInfo[msg.sender].refRewarded;
        require(reward > 0, "no reward");
        userInfo[msg.sender].refRewarded = userInfo[msg.sender].refReward;
        addr.fam.transfer(msg.sender, reward);

        emit WithdrawRefReward(msg.sender, reward);
    }
    ...
    function _withdraw(uint256 reward_) private {
        ...
        emit Withdraw(msg.sender, reward_);
    }
    ...
    function setBetFeeRate(uint8 clubRate_, uint8 foundationRate_) external onlyOwner {
        betFee.foundationRate = foundationRate_;
        betFee.clubRate = clubRate_;
    }

    function setReferrerRate(uint8 totalRate_, uint8[] calldata refRates_) external onlyOwner {
        uint8 totalRate;
        for (uint8 i; i < refRates_.length; i++) {
            refRate[i] = refRates_[i];
            totalRate += refRates_[i];
        }
        require(totalRate == totalRate_, "wrong rate");
        // addr.fam.transfer(cludAddr, (amount_ * 6) / 100);
        betFee.totalRefRate = totalRate_;
    }
