...
import "./../interface/IBiswapRouter.sol";


contract LiquidCZapBiswap {
    using LowGasSafeMath for uint256;
    using SafeERC20 for IERC20;
    using SafeERC20 for ILiquidCVaultV6;

    IBiswapRouter public immutable router;
    ...
    function LiquidCOut (address liquidCVault, uint256 withdrawAmount) external {
        (ILiquidCVaultV6 vault, IBiswapPair pair) = _getVaultPair(liquidCVault);
    ...
    function _getVaultPair (address liquidCVault) private view returns (ILiquidCVaultV6 vault, IBiswapPair pair) {
        vault = ILiquidCVaultV6(liquidCVault);
        pair = IBiswapPair(vault.want());
    ...
    function _swapAndStake(address liquidCVault, uint256 tokenAmountOutMin, address tokenIn) private {
        ...
        if (isInputA) {
            swapAmountIn = _getSwapAmount(fullInvestment, reserveA, reserveB, pair.swapFee());
        } else {
            swapAmountIn = _getSwapAmount(fullInvestment, reserveB, reserveA, pair.swapFee());
        }
    ...
    function _getSwapAmount(uint256 investmentA, uint256 reserveA, uint256 reserveB, uint32 swapFee) private view returns (uint256 swapAmount) {
    ...
    function estimateSwap(address liquidCVault, address tokenIn, uint256 fullInvestmentIn) public view returns(uint256 swapAmountIn, uint256 swapAmountOut, address swapTokenOut) {
        (, IBiswapPair pair) = _getVaultPair(liquidCVault);
        ...
        swapAmountIn = _getSwapAmount(fullInvestmentIn, reserveA, reserveB, pair.swapFee());
        swapAmountOut = router.getAmountOut(swapAmountIn, reserveA, reserveB, pair.swapFee());
