...
contract EternityLock
{
    ...
    function get_Bunny() public view BunnyWithTinyShield returns(address)
    {
        return Bunny;
    }
    ...
    function EternityLockReadDur() public view BunnyWithTinyShield returns(uint)
    {
    ...
    function EternityLockIncreaseDur(uint number_of_days) public BunnyWithTinyShield returns(bool)
    {
        require(block.timestamp >= ProtectionTimelock[msg.sender]);
        ProtectionTimelock[msg.sender] = number_of_days*86400 + block.timestamp;
        return true;
    }
