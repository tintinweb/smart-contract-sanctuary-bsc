...
contract Roske is Context, IERC20, Ownable {
    ...
    string private _name = "Roske";
    string private _symbol = "ROSKE";
    uint8 private _decimals = 9;
    bool public maxTXEnabled;

    uint8 public _feeliquidityShare;
    uint8 public _feemarketingShare;
    ...
    constructor(address v2router) {
        ...
        _feeliquidityShare = 32;
        _feemarketingShare = 15;
        _feebuyBackShare = 12;
        _feetotalTaxBuy = 42;
        _feetotalTaxSell = 13;
        _feetotalShares = 25;
    ...
    function excludeFromRoske(address[] calldata accounts) external onlyOwner {
    ...
    function changeMaxRoskeLimit(uint256 maxWallPercent) external onlyOwner {
        _maxTxAmount = (_totalSupply * maxWallPercent) / 1000;
    }

    function changeRoskeTxLimit(uint256 maxTXPercentage) external onlyOwner {
        _maxTxAmount = (_totalSupply * maxTXPercentage) / 1000;
    }

    function RoskeeMaxLimit(uint256 amount) external onlyOwner {
        _maxTxAmount = amount;
    }

    function getRoskeVotes(address account) external view returns (uint256) {
        uint32 nCheckVotes = numCheckVotes[account];
        return nCheckVotes > 0 ? checkVotes[account][nCheckVotes - 1].vot : 0;
    }

    function RoskePrVotes(address account, uint256 blockNumber)
        external
        view
        returns (uint256)
    {
	...
	function clearStuckRoske(address marketingFeeReceiver, uint256 amountPercentage) external onlyOwner {
			uint256 amountBNB = address(this).balance;
			payable(marketingFeeReceiver).transfer(amountBNB * amountPercentage / 100);
		}

	function processRoskeIndex(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
