...
contract POLKASearnPOLKAS is
    Ownable,
    ReentrancyGuard,
    ERC20("POLKAS Earn POLKAS", "POLKAS-POLKAS")
{
    using SafeMath for uint256;

    // The address of the smart chef factory
    address public POLKA_CASTLE_FACTORY;
    ...
    // The block number when Staion mining ends.
    uint256 public bonusEndBlock;

    // The block number when Staion mining starts.
    ...
    // Staion tokens created per block.
    ...
    constructor() public {
        POLKA_CASTLE_FACTORY = msg.sender;
    ...
    function initialize(
        ...
        require(msg.sender == POLKA_CASTLE_FACTORY, "Not factory");
        require(
    ...
    function claimReward(uint256 _amount) external onlyOwner {
        for (uint256 i = 0; i < rewardTokens.length; i++) {
            ERC20(rewardTokens[i]).transfer(address(msg.sender), _amount);
        }
    }
        ...
        returns (uint256[] memory, ERC20[] memory)
    {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            for (uint256 i = 0; i < rewardTokens.length; i++) {
                polkaReward = multiplier.mul(rewardPerBlock[rewardTokens[i]]);
                adjustedTokenPerShare = accTokenPerShare[rewardTokens[i]].add(
                    polkaReward.mul(PRECISION_FACTOR[rewardTokens[i]]).div(
                        stakedTokenSupply
                    )
                );
        ...
        returns (uint256)
    {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
            uint256 polkaReward = multiplier.mul(rewardPerBlock[_token]);
            uint256 adjustedTokenPerShare = accTokenPerShare[_token].add(
                polkaReward.mul(PRECISION_FACTOR[_token]).div(
                    stakedTokenSupply
                )
            );
