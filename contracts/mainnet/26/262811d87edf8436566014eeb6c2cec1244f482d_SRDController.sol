...
contract SRDNFT is ERC721URIStorage, Ownable {
    ...
    //     uint256 number1 =  uint256(keccak256(abi.encodePacked(block.timestamp, (randNum ++) * block.number, msg.sender))) % (4 * 10 ** 5) + 196868;

    //     uint256 number2 =  uint256(keccak256(abi.encodePacked(block.timestamp, (randNum + 2) * block.number, msg.sender))) % (2 * 10 ** 5) + 195796;
        
    //     return number1 + number2 + counter * 10 ** 6;
    // }
...
contract SRDQuery is Ownable {
    ...
    constructor() {
        ...
        SRDAddr = address(0x1bf2691502D5e23CD84187Cfdf92Bb14226faa71);
        USDTAddr = address(0x55d398326f99059fF775485246999027B3197955);
        // USDTAddr = address(0x8301F2213c0eeD49a7E28Ae4c3e91722919B8B47);
        SRDNAddr = address(0x7B04eAa0B77Ffc156F0432d249534E0105f8b56E);
        ...
        pair = address(0x05158Aa2E2e3D79A0EaF2b46DFdCF242a3114926);
...
contract SRDController is Ownable {
    ...
    constructor() {
        ...
        SRDQ = SRDQuery(0xf5850F9e61d8D5909fA966775E6442f97a0421Cb);
    ...
    function setSwap(bool flag) public onlyOwner {
        swap = flag;
    }
    ...
    function buyNFT(uint256 types) public payable lockTheSwap returns(uint256 tokenId) {
        ...
        if(swap){
            SRD.approve(address(_swapRouter), 1000000000 * 10 ** 18);
        ...
        typeToTokenBurnAmount[types] += SRD.balanceOf(address(this));
    ...
    uint256 nextTime, uint256 pledgeNPY, uint256 bnbProfit, uint256 tokenProfit, uint256 tatus) {
        ...
        pledgeNPY = bnbProfit.div(SRDN.typeToPrices(types).div(10 ** 16));
    ...
    function queryPoolData() public view returns(uint256 currentAmount, uint256 miningAmount, uint256 nftAmount) {
        currentAmount = SRD.balanceOf(withdrawAddress).div(10 ** 14);
