...
// File contracts/interfaces/IWrappedEth.sol
...
// File contracts/ContractWhitelisted.sol
...
contract Zapper is Ownable, IZapper, Pausable, ContractWhitelisted {
	...
	constructor(
		...
		IERC20(_PEG).safeApprove(address(ROUTER), type(uint256).max);
		IERC20(_SHARE).safeApprove(address(ROUTER), type(uint256).max);
		IERC20(_USD).safeApprove(address(ROUTER), type(uint256).max);

		//approve to breakLP
		IERC20(_PEGUSD).safeApprove(address(ROUTER), type(uint256).max);
		IERC20(_SHAREUSD).safeApprove(address(ROUTER), type(uint256).max);

		// set route pairs for our tokens
		routePairAddresses[_SHARE] = _USD;
		routePairAddresses[_PEG] = _USD;
	}
