...
contract PistonTokenController is Initializable, ERC20Upgradeable, OwnableUpgradeable {
	...
	/**
		transfer amount directly to the race
	 */
	function transferToRace(uint256 amount) external onlyOwner { // pls note amount is w/o 0s for 18 decimals
		require(amount <= 10000, "amount is to big");

		require(pistonToken.transfer(_raceContractAddress, amount*10**18), "transfer race failed" );
  	}
		
	/**
		transfer amount directly to the liquidity wallet
	 */
	function transfertoLiquidityWallet(uint256 amount) external onlyOwner { // pls note amount is w/o 0s for 18 decimals
		require(amount <= 10000, "amount is to big");
		require(pistonToken.transfer(_liquidityWalletAddress, amount*10**18), "transfer to liquidity wallet failed" );
  	}
