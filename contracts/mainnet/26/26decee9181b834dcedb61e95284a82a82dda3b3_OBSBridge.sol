...
abstract contract Ownable is Context {
  ...
  constructor() {
    ...
    _owner = 0x436b5742D34D76A501cc4d2eA71a78f37bc9171b;
...
abstract contract BridgeBase is Ownable {
  ...
  event TokenDeposit(bytes32 txHash, bytes transactionID);
  ...
  function depositTokens(uint256 amount, address recipient) external virtual;
...
contract OBSBridge is BridgeBase {
  constructor(address token, address _validator)
    BridgeBase(token, _validator)
  {}

  uint256 nonce_count;

  function depositTokens(uint256 amount, address recipient) external override {
    require(
      !whiteListOn || isWhiteList[msg.sender],
      "OBSBridge: Forbidden in White List mode"
    );
    ...
    bytes32 txHash = keccak256(
      abi.encode(transAmount, nonce_count, recipient, msg.sender)
    );

    bytes memory _transactionID = abi.encode(
      transAmount,
      nonce_count,
      recipient,
      msg.sender
    ...
    emit TokenDeposit(txHash, _transactionID);
  }
  ...
  function withdrawTokens(
    ...
    (uint256 _amount, uint256 _nonce, address _to, address _from) = abi.decode(
      _transactionID,
      (uint256, uint256, address, address)
    );

    require(msg.sender == _to, "OBS Bridge: Irrelevant receiver");
    address signAddress;
    ...
    require(validator == signAddress, "OBS Bridge: wrong signature");

    uint256 adminBal = token.balanceOf(address(this));
    require(
      adminBal >= _amount,
      "OBS Bridge: insuffient funds in admin account."
    );

    require(
      !processedTransactions[_txHash],
      "OBSBridge: transaction already processed"
    );
