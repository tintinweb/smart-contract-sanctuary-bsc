...
contract RugpullProtector is Ownable {
    ...
    function getActiveRequestPage(uint256 page, uint256 size) external view returns (Request[] memory result) {
    ...
    function createProposal(uint256 requestId, Transaction calldata transaction) external {
        ...
        _removeToActiveRequests(requestId);
    ...
    function _validateProposedTransactionData(Condition memory condition, bytes calldata data) internal view returns(bool) {
        ...
        if(action == RequestAction.Transfer) {
            ...
            if(amount >= condition.assetAAmount && to == condition.to) return true; //if 0x -> pass(if user doesnt know receiver address)
        }
    ...
    function _removeToActiveRequests(uint256 requestId) internal {
