...
QC1 (Quantum One) is a BEP-20 token launched under the Quantum Compound porfolio which
...
contract Quantum is ERC20, Ownable, ERC20Burnable {
    ...
    event excludeFromWalletLimit (address _account);
    ...
    constructor() ERC20("Quantum One", "QC1") {
        ...
        isLimitExempt[ADMIN_WALLET] = true;
        isLimitExempt[BURN_WALLET] = true;
        isLimitExempt[address(this)] = true;
        isLimitExempt[pancakeV2Pair] = true;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiSniper(sender, recipient, msg.sender) {
        ...
        if (!isLimitExempt[recipient] && sender != ADMIN_WALLET)
            {
    ...
    function excludeFromHoldLimit(address _address) external onlyOwner {
        isLimitExempt[_address] = true;

        emit excludeFromWalletLimit(_address);
    }

    function includeFromHoldLimit(address _address) external onlyOwner {
        isLimitExempt[_address] = false;
    }
