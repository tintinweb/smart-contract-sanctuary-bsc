...
contract Presale is Ownable {
  ...
  function getClaimable(address _user) public view returns (bool) {
    return openClaim == true && claimed[_user] == false && boughtTokens[_user] && saleStartTime.add(privateSalePeriod) < block.timestamp;
  }

  function getTimeForClaim() external view returns (uint256) {
    if (saleStartTime.add(privateSalePeriod) > block.timestamp)
