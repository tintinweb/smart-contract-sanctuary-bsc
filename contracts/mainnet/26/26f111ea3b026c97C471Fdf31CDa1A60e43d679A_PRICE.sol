...
contract PRICE is Context, Ownable, IERC20 {
    ...
    function _transfer(address from, address to, uint256 amount) internal returns (bool) {
        ...
        if((is_buy(from, to) || is_sell(from, to)) &&  !inSwap && canSwap(from, to)) {
            uint256 contractTokenBalance = balanceOf(address(this));
            if(contractTokenBalance >= swapThreshold) { internalSwap(contractTokenBalance, is_buy(from, to), is_sell(from, to)); }
        }
    ...
    function internalSwap(uint256 contractTokenBalance, bool isbuy, bool issell) internal inSwapFlag {
        ...
        if (isbuy) {
            totalTax = buyTaxes.marketing + buyTaxes.rewards;
            if (totalTax > 0) {
                marketingBNB = bnbInContract * buyTaxes.marketing / totalTax;
                rewardsBNB = bnbInContract - marketingBNB;
            }            
        }
        
        if (issell) {
            totalTax = sellTaxes.marketing + sellTaxes.rewards;
            if (totalTax > 0) {
