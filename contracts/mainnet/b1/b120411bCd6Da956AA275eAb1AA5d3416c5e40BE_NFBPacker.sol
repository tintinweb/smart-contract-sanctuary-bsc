...
// File contracts/NFBPacker.sol

pragma solidity 0.8.9;

interface INFBContract {
...
contract NFBPacker is Ownable, ReentrancyGuard {
    ...
    /** Return NFBContract data */
    function getNFBContractData() external view returns(NFBContractData[] memory) {
        NFBContractData[] memory data =  new NFBContractData[](NFBContracts.length);
        for( uint256 i = 0; i < NFBContracts.length; i ++ ) {
            data[i] = 
                NFBContractData(
                    {
    ...
    function withdraw(address _wallet) external onlyOwner nonReentrant {
        address payable wallet = payable(_wallet);
