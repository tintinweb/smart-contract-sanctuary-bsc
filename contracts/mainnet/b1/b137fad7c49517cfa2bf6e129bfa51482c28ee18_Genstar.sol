...
contract Genstar is Context, IERC20, Ownable {
    ...
    string private _name = "Genstar";
    string private _symbol = "GSR";
    ...
    address payable public devWallet = payable(0x0b1ECb62B3891716599bce1161f0e5C70AC8F16B);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 liquidityTokenShare = contractTokenBalance.mul(_liquidityFee).div(_liquidityFee.add(_devFee));
        uint256 devTokenShare = contractTokenBalance.sub(liquidityTokenShare);
        ...
        swapTokensForEth(devTokenShare, devWallet);
    ...
    function setFeePercent(uint256 taxFee, uint256 liquidityFee, uint256 devFee) external onlyOwner() {
        require(taxFee.add(liquidityFee).add(devFee) <= 12, "total tax mustbe les than or equal to 12%");
        _taxFee = taxFee;
