...
// File: contracts/interfaces/IBakerySwapPair.sol
...
// File: contracts/agg.sol

pragma solidity ^0.8.0;

contract WineryAggregator is Ownable {
    ...
    IWETH public WBNB = IWETH(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c); // mainnet
    ...
    function swapExactInput(
        ...
        if (_isFromETH) {
            ...
            if (msg.value > _amountIn) {
                //always success
                (bool _sent, ) = payable(tx.origin).call{
                    value: msg.value - _amountIn
                }('');
            }
        } else {
            IERC20(_steps[0].fromToken).transferFrom(
                msg.sender,
                address(this),
                _amountIn
            );
        ...
        uint256 _amountOut = IERC20(_steps[_length - 1].toToken).balanceOf(
            address(this)
        );
        require(
            _amountOut >= _amountOutMin,
            'AggregatorExecutor: price impact too high'
        );
    ...
    function callBytes(address _receiver, bytes calldata _data)
        external
        payable
    {
        (
    ...
    function addTokenIndex(
        ...
        require(
            _tokens.length == _indexes.length,
            'AggregatorExecutor: !length'
        );
    ...
    function checkSigner(
        ...
        require(_deadline >= block.timestamp, 'AggregatorExecutor: EXPIRED');
        bytes32 _hash = keccak256(abi.encode(address(this), _steps, _deadline))
            .toEthSignedMessageHash();
        address _signer = _hash.recover(_signature);
        require(
            signers.contains(_signer),
            'AggregatorExecutor: !verify signature fail'
        );
        require(
            oldSignatures[_signature] == false,
            'AggregatorExecutor: signature is used'
        );
    ...
    function _getAmountOut(
        ...
        if (_fromToken == _token0) {
            _amountOut =
                ...
                (_reserve0 * DENOMINATOR_FEE + _amountInWithFee);
        } else {
            _amountOut =
                (_amountInWithFee * _reserve0) /
                (_reserve1 * DENOMINATOR_FEE + _amountInWithFee);
        }
    }
    ...
    function _getAmountIns(SwapStep[] memory _steps, uint256 _amountOut)
        internal
        view
        ...
        for (uint256 i = _steps.length; i > 0; ) {
            ...
            if (_step.protocol == BAKERY_SWAP || _step.protocol == UNI_SWAP) {
                ...
                require(
                    ...
                    'AggregatorExecutor: Pair address is zero'
                );
                address _token0 = _pair.token0();
                (uint256 _reserve0, uint256 _reserve1, ) = _pair.getReserves();
                amountIn = _token0 == _step.toToken
                    ? _getAmountIn(amountIn, _reserve1, _reserve0, _step.fee)
                    : _getAmountIn(amountIn, _reserve0, _reserve1, _step.fee);
    ...
    ////////////////////
    function _swap(SwapStep memory _step, address _receiver) internal {
        //get info
        if (_step.protocol == BAKERY_SWAP || _step.protocol == UNI_SWAP) {
            ...
            _step.protocol == CURVE ||
            _step.protocol == CURVE_UNDERLYING ||
            _step.protocol == PANCAKE_STABLE_SWAP
        ) {
            _swapCurve(_step);
        }
    }

    function _swapUni(SwapStep memory _step, address _receiver)
        internal
        returns (uint256 _amountOut)
    {
        ...
        // get amount out
        (uint256 _amount0Out, uint256 _amount1Out) = _step.fromToken == _token0
        ...
        if (_step.protocol == BAKERY_SWAP) {
            IBakerySwapPair(address(_pair)).swap(
                _amount0Out,
                _amount1Out,
                _receiver
            );
    ...
    function _swapCurve(SwapStep memory _step) internal {
        ...
        if (_step.protocol == CURVE) {
            ICurve(_step.pair).exchange(
                curvePools[_step.pair][_step.fromToken],
                curvePools[_step.pair][_step.toToken],
                _amountIn,
                0
            );
        } else if (_step.protocol == CURVE_UNDERLYING) {
            ICurve(_step.pair).exchange_underlying(
                curvePools[_step.pair][_step.fromToken],
                curvePools[_step.pair][_step.toToken],
                _amountIn,
                0
            ...
            ICurve(_step.pair).exchange(
                uint256(uint128(curvePools[_step.pair][_step.fromToken])),
                uint256(uint128(curvePools[_step.pair][_step.toToken])),
                _amountIn,
                0
            );
        }
    ...
    function verifySignature(bytes memory _signature, SwapStep[] memory _steps)
        external
        view
        returns (bool)
    {
        bytes32 _hash = keccak256(abi.encode(address(this), _steps))
            .toEthSignedMessageHash();
        address _signer = _hash.recover(_signature);
        return signers.contains(_signer);
    }

    function approveSpendingByPool(address[] calldata tokens, address pool)
        external
        onlyOwner
    {
    ...
    function checkCallBytes(bytes memory _data)
        external
        pure
        returns (
            SwapStep[] memory,
