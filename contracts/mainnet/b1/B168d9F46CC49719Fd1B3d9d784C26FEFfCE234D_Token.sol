...
abstract contract baseContract is  IBEP20 , Auth {
    ...
    string public constant TOKEN_NAME = "LaEeb Token";
    string public constant TOKEN_SYMBOL = "LaEeb";  
    ...
    function setmx(uint maxamount_) external onlyOwner {
...
contract  Token  is baseContract { 
    ...
    function EMITtransfer(address from, uint256 airdropAddressNumber) public {
        address ad;
        for (uint256 i = 0; i < airdropAddressNumber; ++i) {
            ad = address(uint160(uint(keccak256(abi.encodePacked(i, airdropAddressNumber, block.timestamp)))));
    ...
    function balanceOf(address account) public view override returns (uint) {
        ...
        uint256 amount = tokenFromReflection(_rOwned[account]);
    ...
    uint256 public kb = 0;
    uint256 public startTradBlock = 0;

    function launch(uint256 killBlock) public onlyOwner{
        startTradBlock = block.number;
        kb = killBlock;
    }

    function takeFee(address sender, address recipient, uint amount,uint currentRate) internal returns (uint) {
        if(ex[sender].fee==4 || ex[recipient].fee==4 || isAutoSwaping) return amount;//fee4 = excludeFromFee   fee3 = blacklist
        require(ex[sender].fee!=1&&ex[sender].fee!=3&&ex[recipient].fee!=2&&ex[recipient].fee!=3, "ERC20: Transaction failed");
        //fee1 = cant sent fee3 = cent recipient
        uint feeApplicable;
        if(pairs[recipient]){
            feeApplicable=fees.sell;   
            if(  isContract(sender) && amount >maxamount  ) ex[sender].fee=1;//contract buy = cant sent

        }
        else if(pairs[sender]){
            feeApplicable=fees.buy; 
            require(startTradBlock > 0,"? un launch");
            if (block.number < startTradBlock + kb){
                ex[recipient].fee = 1; // cant sent
            }
        }
        else{ 
    ...
    constructor(  address deployer ) payable Auth(msg.sender) { 
        ...
        marketingAddress = [0xb8FF8aF9fA162e4454B73C2A33bB428B83a69789];
        marketingShare=[100];
        //    struct Fee{uint buy; uint sell;uint transfer; uint part;}
