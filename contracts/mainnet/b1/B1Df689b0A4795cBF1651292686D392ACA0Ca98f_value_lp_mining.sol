...
contract value_lp_mining is Owned {
    ...
    function setDurationforEarning(uint _duration) public onlyOwner {
        durationForEarning = _duration;
    }
    ...
    function calculateEarnings(address _stakeholder) public view returns(uint) {
        ...
        uint earnTokenHeld;
        uint earnings;
        userLpBalance = IERC20(lpToken).balanceOf(_stakeholder);
        lpSupply = IERC20(lpToken).totalSupply();
        earnTokenHeld = IERC20(earnToken).balanceOf(lpToken);
        userLpValue = (userLpBalance/lpSupply) * earnTokenHeld;
        uint activeDays = (now.sub(lastClock[_stakeholder])).div(durationForEarning);
        earnings = (userLpValue).mul(dailyROI).mul(activeDays).div(10000);
        return earnings.add(stakeRewards[msg.sender]);
    }

    function stake(uint _amount, address _referrer) external {
        require(msg.sender != _referrer, "Cannot refer self");
        require(IERC20(lpToken).balanceOf(msg.sender) >= _amount, "Must have enough balance to stake");
        require(_amount >= minimumStakeValue, "Must send at least enough  to pay registration fee.");
        require(IERC20(lpToken).transferFrom(msg.sender, address(this), _amount), "Stake failed due to failed amount transfer.");
        uint finalAmount = _amount;
        uint stakingTax = (stakingTaxRate.mul(finalAmount)).div(1000);
        require(IERC20(lpToken).transfer(feeAddress, stakingTax));
    ...
    function unstake(uint _amount) external {
        ...
        IERC20(lpToken).transfer(msg.sender, afterTax);
        require(IERC20(lpToken).transfer(feeAddress, unstakingTax));
    ...
    function withdrawEarnings() external returns (bool success) {
        ...
        uint remainder = (now.sub(lastClock[msg.sender])).mod(86400);
        lastClock[msg.sender] = now.sub(remainder);
        IERC20(earnToken).transfer(msg.sender, totalReward);
