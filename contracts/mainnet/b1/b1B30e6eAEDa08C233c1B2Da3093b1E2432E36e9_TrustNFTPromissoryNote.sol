...
// File: contracts/lending/lending/ILending.sol
...
// File: contracts/lending/promissory/PromissoryNote.sol
...
contract TrustNFTPromissoryNote is ERC721, Ownable {
    // Fired when a user creates a new promissory note
    event MintPromissoryNote(uint256 indexed promissoryNoteId);
    ...
    // Needs to start with 1 so 0 can be used as "no promissory note assigned" status on the lending smart contract
    uint256 public promissoryNoteId = 1;
    ILending public lendingDataTemplate;
    ...
    // Allows a user to lock approved loans in the contract and mint a new Promissory Note
    function createPromissoryNote(uint256[] calldata loanIds) external {
        require(
            loanIds.length > 0,
            "You must submit an array with at least one loan ID"
        );

        // Allow loans to be used in the Promissory Note
        require(
            address(lendingDataTemplate) != address(0),
            "Lending contract not established"
        );
        ...
        for (uint256 i = 0; i < loanIds.length; ++i) {
            require(
                ...
                "One of the loans is already used by another promissory note"
            );
            require(
                lendingDataTemplate.getPromissoryPermission(loanIds[i]) ==
                    msg.sender
            );
        ...
        emit MintPromissoryNote(promissoryNoteId);
        ++promissoryNoteId;
    }
    ...
    function safeTransferFrom(
        ...
        lendingDataTemplate.exchangePromissoryNote(
            from,
            payable(to),
    ...
    /**
     * @notice Setter function for the main TrustNFT lending contract
     * @param _lendingDataAddress The address of the TrustNFT lending contract
     */
    function setLendingDataAddress(address _lendingDataAddress)
        external
        onlyOwner
    {
        lendingDataTemplate = ILending(_lendingDataAddress);
    }
