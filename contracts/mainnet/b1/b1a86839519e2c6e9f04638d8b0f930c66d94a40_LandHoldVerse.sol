...
/* 
    ...
    Website: https://www.landholdverse.com/  
    Contract Name: LAND HOLD VERSE 
    Telegram: https://t.me/landholdverse 
    Contract Supply: 300,000,000,000 
    Contract Tokenomics: 
...
contract LandHoldVerse is Context, IERC20, Ownable {
    using Address for address;
    address payable public marketingLandHoldVerseAddress =
        payable(0xb011fab44612F3Ab9856F3cE7066869B9aFD02BD); // Marketing Land Hold Verse Address
    address payable public teamLandHoldVerseAddress =
        payable(0x3A37594420aEB6e091fCB693b0f32EF77301dABd); // team Land Hold Verse Address
    address public constant deadWallet =
        0x000000000000000000000000000000000000dEaD;
    address private LandHoldVerseDeployer = 0x5bf55cb0CB502D568c657b1D7Ee97630D065fe94;
    ...
    uint256 private _tTotal = 300_000_000_000 gwei;
    uint256 private _tFeeTotal;

    string private constant _name = "LAND HOLD VERSE";
    string private constant _symbol = "LHV";
    ...
    uint256 private minimumTokensBeforeSwap = 10_000 gwei;
    ...
    constructor() {
        require(msg.sender == LandHoldVerseDeployer, "Your not the deployer of this contract");
    ...
    function swapAndLiquify() public lockTheSwap {
        ...
        uint256 walletsTotal = marketingTokensCollected + teamTokensCollected;

        uint256 ethForMarketing = (newBalance * marketingTokensCollected) /
            walletsTotal;
        uint256 ethForteam = (newBalance * teamTokensCollected) /
            walletsTotal;

        transferToAddressETH(marketingLandHoldVerseAddress, ethForMarketing);
        transferToAddressETH(teamLandHoldVerseAddress, ethForteam);
    ...
    function restoreAllFee() private {
        ...
        totalSwapableFee = _liquidityFee + _marketingFee + _teamFee;
    }
        ...
        onlyOwner
    {
        ...
        marketingLandHoldVerseAddress = payable(_marketingWallet);
        emit AuditLog(
            "We have Updated the MarketingWallet:",
            marketingLandHoldVerseAddress
        );
    }

    function setteamWalletAddress(address _teamWallet) external onlyOwner {
        require(_teamWallet != address(0), "setteamWalletAddress: ZERO");
        teamLandHoldVerseAddress = payable(_teamWallet);
        emit AuditLog("We have Updated the teamWallet:", teamLandHoldVerseAddress);
    }
    ...
    function recoverETHfromContract() public virtual onlyOwner {
        payable(marketingLandHoldVerseAddress).transfer(address(this).balance);
        emit AuditLog(
    ...
    function recoverTokensFromContract(address _tokenAddress, uint256 _amount) external onlyOwner {                               
        IERC20(_tokenAddress).transfer(marketingLandHoldVerseAddress, _amount);
        emit Log("We have recovered tokens from contract:", _amount);
    }

    function zeroTaxDay() external onlyOwner {
        require(totalSwapableFee != 0, "Taxes are already set to zero.");
        _liquidityFee = 0;
        _marketingFee = 0;
        _teamFee = 0;
        totalSwapableFee = 0;
                emit AuditLog(
            "We have implemented a zero tax day",
           msg.sender);
    }

    function restoreTaxDay() external onlyOwner  {
        require(totalSwapableFee == 0, "Taxes are already set and cannot be restored.");
        _liquidityFee = 1;
        _marketingFee = 2;
        _teamFee = 2;
        totalSwapableFee = _liquidityFee + _marketingFee + _teamFee;
                emit AuditLog(
            "We have restored traditional taxes",
            msg.sender);
    }
