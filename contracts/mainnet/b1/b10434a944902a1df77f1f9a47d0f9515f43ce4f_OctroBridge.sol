...
contract OctroBridge is Ownable, SignerRole {
    ...
    function withdraw(address to, address token, uint256 amount, uint256 fromChain, bytes32 txhash, SigData calldata signature) external checkChain(fromChain) {
        bytes32 message = prefixed(keccak256(abi.encodePacked(this, to, token, amount, fromChain, block.chainid, txhash)));
        ...
        pays[message] = true;
        emit Withdraw(to, token, amount, fromChain, message);
    }
