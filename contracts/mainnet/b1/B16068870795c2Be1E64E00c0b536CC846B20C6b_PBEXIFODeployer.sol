 ...
 */
contract PbexProfile is AccessControl, ERC721Holder {
    ...
    uint256 public numberPbexToReactivate;
    uint256 public numberPbexToRegister;
    uint256 public numberPbexToUpdate;
    ...
    constructor(
        IERC20 _pbexToken,
        uint256 _numberPbexToReactivate,
        uint256 _numberPbexToRegister,
        uint256 _numberPbexToUpdate
    ) {
        pbexToken = _pbexToken;
        numberPbexToReactivate = _numberPbexToReactivate;
        numberPbexToRegister = _numberPbexToRegister;
        numberPbexToUpdate = _numberPbexToUpdate;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @dev To create a user profile. It sends the NFT to the contract
     * and sends PBEX to burn address. Requires 2 token approvals.
     */
    function createProfile(
        ...
        pbexToken.safeTransferFrom(_msgSender(), address(this), numberPbexToRegister);
    ...
    function updateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        // Transfer PBEX token to this address
        pbexToken.safeTransferFrom(_msgSender(), address(this), numberPbexToUpdate);
    ...
    function reactivateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        pbexToken.safeTransferFrom(_msgSender(), address(this), numberPbexToReactivate);
    ...
    /**
     * @dev Claim PBEX to burn later.
     * Callable only by owner admins.
     */
    function claimFee(uint256 _amount) external onlyOwner {
        pbexToken.safeTransfer(_msgSender(), _amount);
    }
    ...
    /**
     * @dev Update the number of PBEX to register
     * Callable only by owner admins.
     */
    function updateNumberPbex(
        uint256 _newNumberPbexToReactivate,
        uint256 _newNumberPbexToRegister,
        uint256 _newNumberPbexToUpdate
    ) external onlyOwner {
        numberPbexToReactivate = _newNumberPbexToReactivate;
        numberPbexToRegister = _newNumberPbexToRegister;
        numberPbexToUpdate = _newNumberPbexToUpdate;
    }
...
contract PbexToken is ERC20("Pbex Token", "Pbex"), Ownable {
    using SafeMath for uint256;
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying PBEXs (not scaled);
...
contract SyrupBar is ERC20("SyrupBar Token", "SYRUP"), Ownable {
    ...
    constructor(PbexToken _pbex) {
        pbex = _pbex;
    }

    // Safe pbex transfer function, just in case if rounding error causes pool to not have enough PBEXs.
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of PBEXs
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. PBEXs to distribute per block.
        uint256 lastRewardBlock; // Last block number that PBEXs distribution occurs.
        uint256 accPbexPerShare; // Accumulated PBEXs per share, times 1e12. See below.
    }
    ...
    constructor(
        ...
        // staking pool
        poolInfo.push(PoolInfo({lpToken: _pbex, allocPoint: 1000, lastRewardBlock: startBlock, accPbexPerShare: 0}));
    ...
    function add(
        ...
        poolInfo.push(
            PoolInfo({lpToken: _lpToken, allocPoint: _allocPoint, lastRewardBlock: lastRewardBlock, accPbexPerShare: 0})
        );
        updateStakingPool();
    }
    ...
    // View function to see pending PBEXs on frontend.
    function pendingPbex(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function updatePool(uint256 _pid) public {
        ...
        pbex.mint(address(syrup), pbexReward);
    ...
    function safePbexTransfer(address _to, uint256 _amount) internal {
        syrup.safePbexTransfer(_to, _amount);
    }
 ...
 * @title PBEXPbexIfoV4
 */
contract PbexIfo is IIFOV4, ReentrancyGuard, Whitelist {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    // pbexProfile
    PbexProfile public pbexProfile;
     ...
     * @param _pbexProfileAddress: the address of the pbexProfile
    ...
    function initialize(
        ...
        pbexProfile = PbexProfile(_pbexProfileAddress);
    ...
    function depositPool(uint256 _amount, uint8 _pid) external override nonReentrant notContract {
        ...
        require(pbexProfile.getUserStatus(msg.sender), "Deposit: Must have an active profile");
        ...
        if (!_poolInformation[_pid].isSpecialSale) {
            ...
            (, uint256 profileNumberPoints, , address profileAddress, uint256 tokenId, bool active) = pbexProfile
    ...
    function _claimPoints(address _user) internal {
        if (!_hasClaimedPoints[_user]) {
            ...
            if (sumPools > thresholdPoints) {
                ...
                pbexProfile.increaseUserPoints(msg.sender, numberPoints, campaignId);
            }
        }
    }
    ...
    function setPbexProfile(address newPbexProfile) external onlyOwner {
        pbexProfile = PbexProfile(newPbexProfile);
    }
    ...
    function isQualifiedPoints(address _user) external view returns (bool) {
        if (!pbexProfile.getUserStatus(_user)) {
            return false;
        }

        (, uint256 profileNumberPoints, , , , ) = pbexProfile.getUserProfile(_user);
    ...
    function isQualifiedNFT(address _user) external view returns (bool) {
        ...
        (, , , address profileAddress, uint256 tokenId, ) = pbexProfile.getUserProfile(_user);
 ...
 * @title IFODeployerV3
 */
contract PBEXIFODeployer is Ownable {
    using SafeERC20 for IERC20;

    uint256 public MAX_BUFFER_BLOCKS = 400000; // 200,000 blocks (6-7 days on BSC)

    address public immutable pbexProfile;
    ...
    /**
     * @notice Constructor
     * @param _pbexProfile: the address of the pbexProfile
     */
    constructor(address _pbexProfile) {
        pbexProfile = _pbexProfile;
    }
    ...
    function createIFO(
        ...
        bytes memory bytecode = type(PbexIfo).creationCode;
