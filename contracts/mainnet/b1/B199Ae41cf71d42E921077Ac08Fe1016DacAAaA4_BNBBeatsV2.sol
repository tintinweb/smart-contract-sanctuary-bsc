...
contract BNBBeatsV2 is Ownable {
    ...
    function hatchBeats(uint256 beatsUsed) private {
        User storage user = users[msg.sender];
    ...
    function payReferrerCommission(address adr, uint256 _amount) private {
        payable(adr).transfer(_amount);
    }
    ...
    function buyBeats(address ref) external payable initializer {
        ...
        if (user.referrals == address(0) && user.referrals != msg.sender) {
            ...
            payReferrerCommission(user.referrals, amountReferrer);
        }
        ...
        hatchBeats(getMyBeats(msg.sender));
