...
contract IMAUToken is Context, IERC20, Ownable {
    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    string public name = "IMAU Token";
    string public symbol = "IMAU";
    uint256 public decimals = 18;

    uint256 private _totalSupply;
    uint256 public burnTotalSupply;
    uint256 public minTotalSupply = 100000 * 10 ** decimals;
    ...
    address market = 0xFA35C6226c00Eebc1Dc3962487c9a2334FDe6B0f;
    address daoFund = 0xAac199970796800583ce96fd3444F62EAca786c2;
    address platform = 0x63e70A310Cf69B1c568aAf31D8c117813fB5Bc55;
    address dividends = 0xD5066C3B6B8EF826cEe3bB773528F0eF75bb598c;
    ...
    bool lock;
    modifier swapLock() {
        require(!lock, "IMAUToken: swap locked");
        lock = true;
        _;
        lock = false;
    }
    ...
    function getRecommenderList(address _addr, uint256 _level) public view returns (address[] memory list) {
        require(_level <= 8, "IMAUToken: level > 8");
        require(excluded[_msgSender()], "IMAUToken: No permission");
    ...
    function initLiquidityManager(address _liquidityManager, uint256 _minLpDealTokenCount) external {
        require(_msgSender() == creator, "IMAUToken: no permission");
    ...
    function _transfer(
        ...
        require(from != address(0), "IMAUToken: transfer from the zero address");
        require(to != address(0), "IMAUToken: transfer to the zero address");
        require(!limits[from], "IMAUToken: limit address");

        _tradeControl(from, to, amount);

        uint256 fromBalance = _balances[from];
        require(fromBalance >= amount, "IMAUToken: transfer amount exceeds balance");
    ...
    function _mint(address account, uint256 amount) private {
        require(account != address(0), "IMAUToken: mint to the zero address");
    ...
    function _burn(address account, uint256 amount) private {
        require(account != address(0), "IMAUToken: burn from the zero address");

        uint256 accountBalance = _balances[account];
        require(accountBalance >= amount, "IMAUToken: burn amount exceeds balance");
    ...
    function _approve(
        ...
        require(owner != address(0), "IMAUToken: approve from the zero address");
        require(spender != address(0), "IMAUToken: approve to the zero address");
    ...
    function _spendAllowance(
        ...
        if (currentAllowance != type(uint256).max) {
            require(currentAllowance >= amount, "IMAUToken: insufficient allowance");
    ...
    function _tradeControl(address from, address to, uint256 amount) private {
        ...
        || from == address(uniswapV2PairBNB) || to == address(uniswapV2PairBNB)
            ...
            if (startTime > block.timestamp) {
                ...
                revert("IMAUToken: transaction not started");

            } else if (startTime + 8 > block.timestamp && from == address(uniswapV2Pair)) {
                limits[addr] = true;
            } else if (startTime + 600 > block.timestamp) {

                if (amount > tradeLimit) {
                    revert("IMAUToken: transaction limit 1000");
                }

                if (to == address(uniswapV2Pair)) {
                    if (amount > _balances[addr] / 2) {
                        revert("IMAUToken: sell limit 50%");
                    }
                }
            }
        }
    ...
    function _countFee(address from, address addr, uint256 amount) private returns (uint256 finalAmount) {
        ...
        uint256 dividendsFee = amount * dividendsRate / 1000;

        uint256 totalRate = 40;
        ...
        finalAmount = amount - marketFee - daoFundFee - lpFee - burnFee - dividendsFee - bonusFee;

        address recommender = recommenderMap[addr];
        for (uint256 i; i < 7 && recommender != address(0); i++) {
            if (_balances[recommender] >= minBalance) {
                uint256 rate = 5;
                if (i == 0) {
                    rate = 10;
                }
        ...
        _addBalance(from, dividends, dividendsFee);
