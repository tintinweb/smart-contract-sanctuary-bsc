...
contract ShitCoinInu is ERC20, Ownable {
    ...
    ShitCoinInuDividendTracker public dividendTracker;

    uint256 public swapTokensAtAmount = 1000000 * 10**decimals();
    uint256 public maxWalletBalance = 10000000 * 10**decimals();  // 1% of total supply
    uint256 public maxTxAmount = 10000000 * 10**decimals();    // 1% of total supply

    uint256 public  BUSDRewardsFee = 40;
    ...
    address public marketingWallet = 0xF9fA92349028DE542C8c36caFc6A4e546B4813c7; 
    address public liquidityWallet = 0xF9fA92349028DE542C8c36caFc6A4e546B4813c7;
    ...
    constructor()  ERC20("SHITCOIN INU", "SHITI") {
        
    	dividendTracker = new ShitCoinInuDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "Contract: The dividend tracker already has that address");

        ShitCoinInuDividendTracker newDividendTracker = ShitCoinInuDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "Contract: The new dividend tracker must be owned by the ShitCoinInu token contract");
    ...
    function updateRouter(address newAddress) public onlyOwner {
        require(newAddress != address(router), "Contract: The router already has that address");
        emit Updaterouter(newAddress, address(router));
        router = IRouter(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Contract: Account is already the value of 'excluded'");
    ...
    function _setAutomatedMarketMakerPair(address newPair, bool value) private {
        require(automatedMarketMakerPairs[newPair] != value, "Contract: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Contract: gasForProcessing must be between 200,000 and 500,000");
    ...
    function setAutoBoostThreshold(uint256 amount) external onlyOwner{
        autoBoostThreshold = amount * 10**decimals();
    }
...
contract ShitCoinInuDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    ...
    constructor()  DividendPayingToken("ShitCoinInu_Dividend_Tracker", "ShitCoinInu_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 1e5 * (10**18); //must hold 100_000 tokens
    }

    function _transfer(address, address, uint256) internal pure override{
        require(false, "ShitCoinInu_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "ShitCoinInu_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ShitCoinInu contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "ShitCoinInu_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "ShitCoinInu_Dividend_Tracker: Cannot update claimWait to same value");
