...
contract BabyBo is IBEP20, Auth {
    ...
    string constant _name = "BabyBo";
    string constant _symbol = "BAYBO";
    uint8 constant _decimals = 18;

    uint256 _totalSupply = 1 * 10**6 * 10**_decimals;           // 1 000 000 GAPE
    ...
    modifier canTrade(address _sender, address _recipient) {
        if (!isTxLimitExempt[_sender] && !isTxLimitExempt[_recipient]) {
            if (_recipient == pair || _sender == pair) {
                require(tradingEnabled, "GAPE: Trading locked");   
            }
        }
        _;
    }
    ...
    constructor () Auth(msg.sender) {
        ...
        WCOIN = router.WETH();
        pair = IDEXFactory(router.factory()).createPair(WCOIN, address(this));
