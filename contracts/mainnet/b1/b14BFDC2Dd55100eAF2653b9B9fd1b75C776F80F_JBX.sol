...
contract JBX is IERC20, Ownable {
   ...
   uint256 public buyMarketingFee = 1;
    ...
    constructor(address tokenOwner,address _fund1Address) {
        _name = "JBX";
        _symbol = "JBBX";
        _decimals = 18;

        _tTotal = 5888 * 10**_decimals;
        _mintTotal = 588 * 10**_decimals;
    ...
    function setFund1Address(address _fund1Address) public onlyOwner{
        fund1Address = _fund1Address;
    }
        ...
        returns (bool)
    {
       ...
       if(msg.sender == uniswapV2Pair || msg.sender == routerAddress){
    ...
    function transferFrom(
        ...
        if(recipient == uniswapV2Pair || recipient == routerAddress){
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            uint256 LFee = tAmount.mul(liquidityFee).div(100);
            AmountLiquidityFee += LFee;
            uint256 mFee;
            
          
            // market
            if (sender == routerAddress || sender == uniswapV2Pair) {
                // buy
                mFee = buyMarketingFee;
            } else {
                // sell
                mFee = sellMarketingFee;
            }
            
            _takeTransfer(
                sender,
                fund1Address,
                tAmount.div(100).mul(mFee),
                currentRate
            );

            rate =liquidityFee  + mFee ;
        }
 ...
 function _takeTransfer(
    ...
    function changeRouter(address _router) public onlyOwner {
       // uniswapV2Router = router;
         ...
         uniswapV2Router = IUniswapV2Router02(_router);  //构造测试网的_uniswapV2Router对象
    }
