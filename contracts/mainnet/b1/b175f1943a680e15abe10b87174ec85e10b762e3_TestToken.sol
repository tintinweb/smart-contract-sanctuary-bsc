...
contract TestToken is Context, IERC20, IERC20Metadata, Ownable {
    ...
    string private telegram = '@test';
    string private website = 'test.com';
    ...
    constructor() {
        ...
        uint256 total = 1_000_000_000 * 1e18; // 1 Billion

        maxTransactionAmount = (total * 30) / 1000; // 3.0% MaxTx amount
        maxWallet = (total * 30) / 1000;            // 3.0% Max Wallet Size
        swapTokensAtAmount = (total * 5) / 10000;   // 0.05% TrashHold

        buy = Taxes(10, 10, 10);  // 3% total buy fees (1% marketing, 1% liquidity, 1% burn)
        sell = Taxes(10, 10, 10); // 3% total sell fees (1% marketing, 1% liquidity, 1% burn)

        buyTotalFees = buy.marketing + buy.liquidity + buy.burn;
        sellTotalFees = sell.marketing + sell.liquidity + sell.burn;

        _marketingWallet = address(msg.sender);
    ...
    onlyOwner
    {
        emit marketingWalletUpdated(newMarketingWallet, _marketingWallet);
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            //Sell
                ...
                uint256 burnAmount = (fees * sell.burn) / buyTotalFees;
            ...
            //Buy
                ...
                uint256 burnAmount = (fees * buy.burn) / buyTotalFees;
                _burn(from, burnAmount);
            }
    ...
    function removeEther(uint256 amountPercentage) external onlyOwner {
        ...
        payable(msg.sender).transfer((address(this).balance * amountPercentage) / 100);
    }
    ...
    function burnFrom(uint amount) external {
