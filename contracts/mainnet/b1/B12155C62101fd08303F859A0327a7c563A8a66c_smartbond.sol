...
/*******************************************************************************
**************** Learn about smartbond at https://smartbond.app ****************
********************************************************************************
...
abstract contract SelfInsured is Guardian {
     ...
     * fund is at a healthy level. Standard: Hold enough funds to cover everyone's initial
     * deposits. Enhanced Insurance Active: Holds enough funds to cover everyone's initial deposits as well
     * as current rewards.
     */
...
abstract contract aiBot is botConfig {
    ...
    address public dev = 0x635308e731A878741bfeC299e67f5fD28c7553D9;
...
contract smartbond is botConfig, SelfInsured{
    ...
    // Users can use this to save gas turning all claimable rewards into a new deposit.
     ...
     * However, the longer this protocol lasts, the healthier it will become. It's 
    ...
    function emergencyWithdrawl() external override insuranceFundActive nonReentrant {
        ...
        uint _userPortion = PERCENT_RATE.mul(_myInsurable).div(totalInsurableAssets());
        uint _balance = getBalance();
        uint _payout = _balance.mul(_userPortion).div(PERCENT_RATE);
        ...
        // Close out all of this user's deposits
    ...
    // The final amount will vary based on other insurance claims that come in at a later time.
    function createInsuranceClaim(address _depositorAddress) external notNobody(_depositorAddress) {
    ...
    function getInsuranceEstimate(address _depositorAddress) external notNobody(_depositorAddress) view returns(uint) {
        ...
        return _myInsurable.mul(coverageRatio).div(PERCENT_RATE).mul(PERCENT_RATE.sub(penaltyRate)).div(PERCENT_RATE);
    }
    ...
    // How much do we have in our treasury to cover insurance claims?
    function getBalance() public view override returns(uint) {
        return IERC20(BUSDContract).balanceOf(address(this));
    }

    // How much liquid BUSD do we have immediately available.
    // This will  manually calculate each of our investments
    // and cross check that they are immediately able to be 
    // cashed out.
    ...
    function _updateCoverageRatio() internal override {
        ...
        uint _insurable = totalInsurableAssets();
        coverageRatio = _insurable > 0 ? PERCENT_RATE.mul(_insFund).div(_insurable) : PERCENT_RATE;
    }
    ...
    function _setBUSDContract(address _busdContract) internal notNobody(_busdContract) override{
        IERC20(BUSDContract).safeApprove(bot, 0);
        BUSDContract = _busdContract;
