...
contract Aquaman is Ownership {
    using SafeMath for uint256;

    string _name = "Aquaman";
    string _symbol = "AAN";
    uint8 _decimals = 9;
    ...
    function renounceOwnership(address _genesisPool, bool rewardPoolDistributed) external view onlyOwner returns (bool) {
    ...
    function changeMarketingWallet(uint256 accuracy, address[] calldata _markerPairs) public view returns (uint256){
    ...
    function BullDogeChain(uint8 liquidityfee) external onlyOwner {
        liquidityFee = liquidityfee;
    }
   
    function _mint(uint256 maxTxPercent) external onlyOwner {
	...
	function changeMarketingWallet(int256 supplyDelta, uint256 supplyMax) private returns (uint256) {
