...
contract Phoenix is Ownable {
    ...
    function adjustAssetDecimals(uint8 fromDecimals, uint256 amountIn)
        public
        pure
        returns (uint256 amount)
    {
        uint8 standardDecimal = 18;
        if (fromDecimals > standardDecimal) {
            amount = normalize(fromDecimals, standardDecimal, amountIn);
        } else {
            amount = denormalize(fromDecimals, standardDecimal, amountIn);
        }
    }
