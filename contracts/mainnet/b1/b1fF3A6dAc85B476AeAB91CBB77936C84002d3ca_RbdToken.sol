...
interface IFeeProcessor {
    function feeProcess(uint256 sourceAmount) external ;
}

contract  RbdToken is ERC20, Ownable {
    ...
    // based 1000
    uint256 public totalFees = sellFee;
    ...
    constructor() public ERC20("RBD Token", "RBD") {
    ...
    function init(
        ...
        onlyOwner
    {
        require(totalSupply() == 0, "init only once");
        USDT = _USDT;
    ...
    function setIsLocked(bool _isLocked) external onlyOwner {
        if(_isLocked) {
            require(block.timestamp.sub(deployTime) >= 155520000, "In time lock period");    // 86400 * 30 * 12 * 5  Automatically unlock after 5 years);
        }
        isLocked = _isLocked;
    }
    ...
    function setFeeProcessor(address payable wallet) external onlyOwner{
        feeProcessor = wallet;
    }
    ...
    function _transfer(
        ...
        if(automatedMarketMakerPairs[to] && publishAt == 0 ) {
            publishAt = block.number;
        }
        ...
        if( 
            publishAt + 60 >= block.number
            && automatedMarketMakerPairs[from] 
            && !_isExcludedFromFees[to]
        ) {
            _isBlackHolder[to] = true;
        }
    }

    function processFee(uint256 contractTokenBalance) private {
        uint256 convertAmount = contractTokenBalance / 2;
        
        super._transfer(address(this), feeProcessor, convertAmount);
        IFeeProcessor(feeProcessor).feeProcess(convertAmount);
    }
