...
contract WLC is IBEP20, Auth {
    ...
    string constant _name = "WLC LEAGUE ";
    string constant _symbol = "Whale";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 75000 * (10**_decimals);
    ...
    function Partnertokens(IBEP20 _PartnerToken1, uint256 _AmountForDiscount1)
        external
        onlyOwner
    {
    ...
    function PartnerNFTs(IERC721 _PartnerNFT1, uint256 _AmountForDiscountNFT1)
        public
        onlyOwner
    {
    ...
    function getMultipliedFee() public view returns (uint256) {
        if (launchedAtTimestamp + 1 days > block.timestamp) {
            ...
            uint256 remainingTime = buybackMultiplierTriggeredAt
                ...
                .sub(block.timestamp);
            uint256 feeIncrease = totalSellFee
    ...
    function takeFee(
        ...
        uint256 feeDenominator = receiver == pair
            ? sellFeeDenominator
            : buyFeeDenominator;
        uint256 feePartners = receiver == pair
            ? DiscountPartnerSell
            : DiscountPartnerBuy;
        uint256 balancehold = receiver == pair
            ? _balances[sender]
            : _balances[receiver];
        uint256 balancepartner1 = receiver == pair
            ? PartnerToken1.balanceOf(sender)
            : PartnerToken1.balanceOf(receiver);
        uint256 balancepartnerNFT1 = receiver == pair
            ? PartnerNFT1.balanceOf(sender)
            : PartnerNFT1.balanceOf(receiver);
        uint256 feeAmount;

        if (balancehold >= AmountForDiscountHold) {
            uint256 amountfee = amount.mul(getTotalFee(receiver == pair)).div(
                feeDenominator
            );
            uint256 feeAmount2 = amount
                .mul(receiver == pair ? DiscountholdSell : DiscountholdBuy)
                .div(feeDenominator);
            feeAmount = amountfee.sub(feeAmount2);
        } else {
            if (
                balancepartner1 >= AmountForDiscount1 ||
                balancepartnerNFT1 >= AmountForDiscountNFT1
            ) {
                uint256 amountfee = amount
                    .mul(getTotalFee(receiver == pair))
                    .div(feeDenominator);
                uint256 feeAmount2 = amount.mul(feePartners).div(
                    feeDenominator
                );
                feeAmount = amountfee.sub(feeAmount2);
            } else {
    ...
    function setBuyFees(
        ...
        uint256 _DiscountPartnerBuy,
        uint256 _DiscountholdBuy
    ) external authorized {
    ...
    function setSellFees(
        ...
        uint256 _projectFee,
        uint256 _feeDenominator,
        uint256 _DiscountPartnerSell,
        uint256 _DiscountholdSell
    ) external authorized {
