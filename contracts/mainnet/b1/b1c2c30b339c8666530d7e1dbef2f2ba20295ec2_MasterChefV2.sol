...
/// @notice The (older) MasterChef contract gives out a constant number of SELF tokens per block.
/// It is the only address with minting rights for SELF.
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    ///
    ///   pending reward = (user share * pool.accSELFPerShare) - user.rewardDebt
    ///
    ...
    ///     SELF rewards each pool gets.
    /// `accSELFPerShare` Accumulated SELFs per share, times 1e12.
    ...
    ///     the SELF rewards to all the SMBSwap products.
    /// `totalBoostedShare` The total amount of user shares in each pool. After considering the share boosts.
    struct PoolInfo {
        uint256 accSELFPerShare;
    ...
    /// @notice Address of SELF contract.
    IBEP20 public immutable SELF;

    /// @notice The only address can withdraw all the burn SELF.
    ...
    ///  @notice 40 SELFs per block in MCV1
    uint256 public constant MASTERCHEF_SELF_PER_BLOCK = 10 * 1e18;
    uint256 public constant ACC_SELF_PRECISION = 1e18;

    /// @notice Basic boost factor, none boosted user's boost factor
    uint256 public constant BOOST_PRECISION = 100 * 1e10;
    /// @notice Hard limit for maxmium boost factor, it must greater than BOOST_PRECISION
    uint256 public constant MAX_BOOST_PRECISION = 200 * 1e10;
    /// @notice total SELF rate = toBurn + toRegular + toSpecial
    uint256 public constant SELF_RATE_TOTAL_PRECISION = 1e12;
    /// @notice The last block number of SELF burn action being executed.
    /// @notice SELF distribute % for burn
    uint256 public SELFRateToBurn = 400000000000;
    /// @notice SELF distribute % for regular farm pool
    uint256 public SELFRateToRegularFarm = 150000000000;
    /// @notice SELF distribute % for special pools
    uint256 public SELFRateToSpecialFarm = 450000000000;
    ...
    event UpdatePool(uint256 indexed pid, uint256 lastRewardBlock, uint256 lpSupply, uint256 accSELFPerShare);
    event Deposit(address indexed user, uint256 indexed pid, uint256 amount);
    event Withdraw(address indexed user, uint256 indexed pid, uint256 amount);
    event EmergencyWithdraw(address indexed user, uint256 indexed pid, uint256 amount);

    event UpdateSELFRate(uint256 burnRate, uint256 regularFarmRate, uint256 specialFarmRate);
    ...
    /// @param _SELF The SELF token contract address.
    ...
    /// @notice Deposits a dummy token to `MASTER_CHEF` MCV1. This is required because MCV1 holds the minting permission of SELF.
    ...
    function init(IBEP20 dummyToken) external onlyOwner {
        ...
        // MCV2 start to earn SELF reward from current block in MCV1 pool
    ...
    /// only for SELF distributions within SMBSwap products.
    function add(
        uint256 _allocPoint,
        IBEP20 _lpToken,
        bool _isRegular,
        ...
        // stake SELF token will cause staked token and reward token mixed up,
        // may cause staked tokens withdraw as reward token,never do it.
        require(_lpToken != SELF, "SELF token can't be added to farm pools");
    ...
    /// @notice Update the given pool's SELF allocation point. Can only be called by the owner.
    ...
    /// @notice View function for checking pending SELF rewards.
    /// @param _pid The id of the pool. See `poolInfo`.
    ...
    function pendingSELF(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 SELFReward = multiplier.mul(SELFPerBlock(pool.isRegular)).mul(pool.allocPoint).div(
                (pool.isRegular ? totalRegularAllocPoint : totalSpecialAllocPoint)
            );
            accSELFPerShare = accSELFPerShare.add(SELFReward.mul(ACC_SELF_PRECISION).div(lpSupply));
        }

        uint256 boostedAmount = user.amount.mul(getBoostMultiplier(_user, _pid)).div(BOOST_PRECISION);
        return boostedAmount.mul(accSELFPerShare).div(ACC_SELF_PRECISION).sub(user.rewardDebt);
    }

    /// @notice Update SELF reward for all the active pools. Be careful of gas spending!
    ...
    /// @notice Calculates and returns the `amount` of SELF per block.
    /// @param _isRegular If the pool belongs to regular or special.
    function SELFPerBlock(bool _isRegular) public view returns (uint256 amount) {
        if (_isRegular) {
            amount = MASTERCHEF_SELF_PER_BLOCK.mul(SELFRateToRegularFarm).div(SELF_RATE_TOTAL_PRECISION);
        } else {
            amount = MASTERCHEF_SELF_PER_BLOCK.mul(SELFRateToSpecialFarm).div(SELF_RATE_TOTAL_PRECISION);
        }
    }

    /// @notice Calculates and returns the `amount` of SELF per block to burn.
    function SELFPerBlockToBurn() public view returns (uint256 amount) {
        amount = MASTERCHEF_SELF_PER_BLOCK.mul(SELFRateToBurn).div(SELF_RATE_TOTAL_PRECISION);
    }
    ...
    function updatePool(uint256 _pid) public returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (lpSupply > 0 && totalAllocPoint > 0) {
                ...
                pool.accSELFPerShare = pool.accSELFPerShare.add((SELFReward.mul(ACC_SELF_PRECISION).div(lpSupply)));
            }
            pool.lastRewardBlock = block.number;
            poolInfo[_pid] = pool;
            emit UpdatePool(_pid, pool.lastRewardBlock, lpSupply, pool.accSELFPerShare);
        }
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            settlePendingSELF(msg.sender, _pid, multiplier);
        }
        ...
        user.rewardDebt = user.amount.mul(multiplier).div(BOOST_PRECISION).mul(pool.accSELFPerShare).div(
            ACC_SELF_PRECISION
        );
        poolInfo[_pid] = pool;
    ...
    /// @notice Harvests SELF from `MASTER_CHEF` MCV1 and pool `MASTER_PID` to MCV2.
    ...
    /// @notice Send SELF pending for burn to `burnAdmin`.
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function burnSELF(bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }

        uint256 multiplier = block.number.sub(lastBurnedBlock);
        uint256 pendingSELFToBurn = multiplier.mul(SELFPerBlockToBurn());

        // SafeTransfer SELF
        _safeTransfer(burnAdmin, pendingSELFToBurn);
        lastBurnedBlock = block.number;
    }

    /// @notice Update the % of SELF distributions for burn, regular pools and special pools.
    /// @param _burnRate The % of SELF to burn each block.
    /// @param _regularFarmRate The % of SELF to regular pools each block.
    /// @param _specialFarmRate The % of SELF to special pools each block.
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function updateSELFRate(
        uint256 _burnRate,
        ...
        require(
            ...
            "MasterChefV2: SELF rate must be greater than 0"
        );
        require(
            _burnRate.add(_regularFarmRate).add(_specialFarmRate) == SELF_RATE_TOTAL_PRECISION,
            "MasterChefV2: Total rate must be 1e12"
        );
        if (_withUpdate) {
            massUpdatePools();
        }
        // burn SELF base on old burn SELF rate
        burnSELF(false);

        SELFRateToBurn = _burnRate;
        SELFRateToRegularFarm = _regularFarmRate;
        SELFRateToSpecialFarm = _specialFarmRate;

        emit UpdateSELFRate(_burnRate, _regularFarmRate, _specialFarmRate);
    }
    ...
    function updateBoostMultiplier(
        ...
        settlePendingSELF(_user, _pid, prevMultiplier);

        user.rewardDebt = user.amount.mul(_newMultiplier).div(BOOST_PRECISION).mul(pool.accSELFPerShare).div(
            ACC_SELF_PRECISION
        );
    ...
    /// @notice Settles, distribute the pending SELF rewards for given user.
    /// @param _user The user address for settling rewards.
    /// @param _pid The pool id.
    /// @param _boostMultiplier The user boost multiplier in specific pool id.
    function settlePendingSELF(
        address _user,
        uint256 _pid,
        ...
        uint256 accSELF = boostedAmount.mul(poolInfo[_pid].accSELFPerShare).div(ACC_SELF_PRECISION);
        uint256 pending = accSELF.sub(user.rewardDebt);
        // SafeTransfer SELF
        _safeTransfer(_user, pending);
    }

    /// @notice Safe Transfer SELF.
    /// @param _to The SELF receiver address.
    /// @param _amount transfer SELF amounts.
    function _safeTransfer(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            // Check whether MCV2 has enough SELF. If not, harvest from MCV1.
            if (SELF.balanceOf(address(this)) < _amount) {
