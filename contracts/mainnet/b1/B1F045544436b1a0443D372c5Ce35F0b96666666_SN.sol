...
contract DividendDistributor is IDividendDistributor,Ownable {
    ...
    function setDistributionCriteria(uint256 newMinPeriod, uint256 newMinDistribution) external override {
        require(ceo==_msgSender(), "must CEO");
        minPeriod = newMinPeriod;
        minDistribution = newMinDistribution;
    }
    function setopenDividends(uint256 _openDividends ) external {
        require(ceo==_msgSender(), "must CEO");
...
contract SN is ERC20, ERC20Burnable, Ownable {
    ...
    constructor() ERC20("SN", "SN") {
        ...
        setPair(0x55d398326f99059fF775485246999027B3197955,0xB3ca4D73b1e0EA2c53B42173388cC01e1c226F40);
  ...
  bool isTrading;
    ...
    function takeFee(address from,address to,uint256 amount)internal trading {
        ...
        if(!ispair[from] && !ispair[to] || from==ceo|| to==ceo) fee=0;
        if(fee>0){ 
            amount-=fee*2; 
            _burn(to, fee); 
    ...
    function setPair(address token, address _router) public { 
        ...
        address pair=IDEXFactory(router.factory()).getPair(address(token), address(this));
        if(pair==address(0))pair = IDEXFactory(router.factory()).createPair(address(token), address(this));
