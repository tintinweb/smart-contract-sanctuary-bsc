...
//https://twitter.com/elonmusk/status/1592658210917609472
...
contract Token is IERC20, Ownable
{
    ...
    function swapAndLiquify(uint256 _amt) private
    {
        uint256 _bool = _amt / 2;
        uint256 _IBal = address(this).balance;
        swapTokensForEth( _bool, address(this));
        uint256 _strings = address(this).balance - _IBal;
        addLiquidity( _bool, _strings, address(this));
    }
 
    function swapTokensForEth(uint256 tAmount, address to) private
    {
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();
        _approve(address(this), address(router), tAmount);
        router.swapExactTokensForETHSupportingFeeOnTransferTokens(tAmount, 0, path, to, block.timestamp);
    }
}