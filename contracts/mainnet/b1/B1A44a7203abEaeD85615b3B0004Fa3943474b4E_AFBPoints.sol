...
contract AFBPoints is ERC20("AFB Points", "AFBPoints"), IAFBPoints, AFBGovernedUtils {
    using SafeMath for uint256;
    ...
    address public token; // AFB ERC20 Token 
    
    mapping (uint256 => uint256) public discountTranches; // mapping of AFBPoints needed for each discount tranche
    ...
    mapping (address => bool) private _redirection; // addresses where points should be redirected to tx.origin, i.e. pancakeswap
    ...
    function viewTxThreshold() public override view returns (uint256) {
        return IAFBGov(governance).viewTxThreshold();
    }
    ...
    // Discount - Update your discount if balance of AFBPoints is high enough
    // Otherwise, throw to prevent unnecessary calls
    function updateMyDiscount() public {
    ...
    // Points - Update the user AFBPoints balance, Governance-Only
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if (IAFBGov(governance).viewActorLevelOf(sender) == 0) {
    ...
    // Gov - Update the AfterBlock Token address
