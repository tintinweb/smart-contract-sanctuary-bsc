...
contract BEP20Token is  Context, IBEP20, Ownable {
  ...
  function setNeedFeePair(address addr) public onlyOwner {
    needFeePair = addr;
  }
  ...
  function inputIDOFormBusd(address _IDO, uint256 _busd) public returns (uint256 _power) {
    ...
    _power = inputIDO(_IDO, _token, _busd);
  }
  ...
  function composeCardFormBusd(address _card, uint256 _busd) public returns (uint256 _pow, uint256 _power) {
    ...
    uint256 _token = _busd.mul(proportion).div(100);
    (_pow, _power) = composeCard(_card, _token, _busd);
  }
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    (sender == needFeePair || recipient == needFeePair)){
      uint256 Mfee = amount.mul(marketingFee).div(1000);
      uint256 Dfee = amount.mul(deadFee).div(1000);
      if (Mfee > 0){
        _balances[sender] = _balances[sender].sub(Mfee, "BEP20: transfer amount exceeds balance");
        _balances[marketingWallet] = _balances[marketingWallet].add(Mfee);
        amount = amount.sub(Mfee);
        emit Transfer(sender, marketingWallet, Mfee);
      }
      if (Dfee > 0){
        _balances[sender] = _balances[sender].sub(Dfee, "BEP20: transfer amount exceeds balance");
