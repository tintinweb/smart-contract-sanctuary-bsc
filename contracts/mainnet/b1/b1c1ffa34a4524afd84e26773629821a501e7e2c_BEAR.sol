...
contract BEAR is ERC20Detailed, Ownable {
    ...
    address payable public bearChestReceiver = payable(0xF041Ad7211d23Daa288Cfcd031809A1730513886);
    address payable public bearPot = payable(0x3851C29d939fAa0966d1d1de476fDA6D9568e382);
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
		...
		if (!inSwap && canSwap && _autoAddLiquidity && recipient == pair) 
		{

		     uint256 tokenToBearChest   = bearChestFeeTotal;
			 uint256 tokenToBearPot     = bearPotFeeTotal;
			 uint256 tokenToLiquidity   = liquidityFeeTotal;
			 uint256 liquidityHalf      = tokenToLiquidity.div(2);
			 uint256 tokenToSwap        = tokenToBearChest.add(tokenToBearPot).add(liquidityHalf);
			 
			 if(tokenToSwap >= swapAtAmount)
			 {

			      uint256 initialBalance = address(this).balance;			
				  swapTokensForBNB(swapAtAmount);
				  uint256 newBalance = address(this).balance.sub(initialBalance);
				  
				  uint256 bearChestPart  = newBalance.mul(tokenToBearChest).div(tokenToSwap);
				  uint256 bearPotPart    = newBalance.mul(tokenToBearPot).div(tokenToSwap);
				  uint256 liquidityPart  = newBalance.sub(bearChestPart).sub(tokenToSwap);
				  
				  
				  if(bearChestPart > 0)
				  {
					   payable(bearChestReceiver).call{ value: bearChestPart, gas: 30000};
					   bearChestFeeTotal = bearChestFeeTotal.sub(swapAtAmount.mul(tokenToBearChest).div(tokenToSwap));
				  }
				  
				  if(bearPotPart > 0)
				  {
					   payable(bearPot).call{ value: bearPotPart, gas: 30000};
					   bearPotFeeTotal = bearPotFeeTotal.sub(swapAtAmount.mul(tokenToBearPot).div(tokenToSwap));
				  }
				  
				  if(liquidityPart > 0)
				  {
				       addLiquidity(liquidityHalf, liquidityPart);
					   liquidityFeeTotal = liquidityFeeTotal.sub(swapAtAmount.mul(tokenToLiquidity).div(tokenToSwap));
				  }
			 }
		}
		...
		if(takeFee)
		{
		     uint256 fees = collectFee(sender, recipient, gonAmount);
    ...
    function collectFee(address sender, address recipient, uint256 gonAmount) internal returns (uint256) {
        uint256 feeAmount; 
		...
		uint256 _liquidityFee = gonAmount.mul(recipient != pair && sender != pair ? liquidityFee[2] : recipient==pair ? liquidityFee[1] : liquidityFee[0]).div(feeDenominator);
		         liquidityFeeTotal = liquidityFeeTotal.add(_liquidityFee.div(_gonsPerFragment));
		...
		uint256 _bearChestFee = gonAmount.mul(recipient != pair && sender != pair ? bearChestFee[2] : recipient==pair ? bearChestFee[1] : bearChestFee[0]).div(feeDenominator);
		         bearChestFeeTotal = bearChestFeeTotal.add(_bearChestFee.div(_gonsPerFragment));
		...
		uint256 _bearPotFee = gonAmount.mul(recipient != pair && sender != pair ? bearPotFee[2] : recipient==pair ? bearPotFee[1] : bearPotFee[0]).div(feeDenominator);
		         bearPotFeeTotal = bearPotFeeTotal.add(_bearPotFee.div(_gonsPerFragment));
	...
	function setAutoSwapLiquidity(bool _flag) external onlyOwner {
	...
	function addLiquidity(uint256 tokenAmount, uint256 bnbAmount) private swapping{
	...
	function swapTokensForBNB(uint256 tokenAmount) private swapping{
