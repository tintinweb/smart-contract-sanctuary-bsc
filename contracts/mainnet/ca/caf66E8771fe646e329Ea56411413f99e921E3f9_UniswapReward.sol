...
interface ERC721
{
  ...
  function isApprovedForAll(
    ...
    function getAllType(uint _tid) external view returns (uint);
...
contract UniswapReward is LPTokenWrapper{
    ...
    mapping(address => uint[]) public stakeTokenID;
    
    uint256 public  DURATION = 1;       
    uint256 public _initReward = 0;
    uint256 public true_initReward = 0;
    
    uint256 public _startTime =  now;
    uint256 public _periodFinish = now;
    ...
    uint public _nftType;

    uint public _stakeCounts;
    uint public startAmount = 500;
    ...
    constructor(address new_rewardToken,address new_nftAddr,uint256 new_true_initReward,address new_inviteAddr,uint new_nftType) public {
        _rewardToken = IERC20(new_rewardToken);
        _nftAddr = new_nftAddr;
        true_initReward = new_true_initReward;
        _inviteAddr = new_inviteAddr;
        _nftType = new_nftType;
    }
    ...
    function set_startAmount(uint256 new_startAmount) public onlyGovernance{
    ...
    function stake(uint256 _tokenid)
        ...
        checkHalve
    {
        require(ERC721(_nftAddr).ownerOf(_tokenid) == msg.sender,"ownerOf err");
        require(ERC721(_nftAddr).getAllType(_tokenid) == _nftType,"_nftType err");

        ERC721(_nftAddr).safeTransferFrom(msg.sender,address(this),_tokenid);
        stakeTokenID[msg.sender].push(_tokenid);
        stakeinternal(1);

        if(_totalSupply >= startAmount && !_hasStart)
        {
            _hasStart = true;
            _startTime = block.timestamp;
            _initReward = true_initReward;             
            DURATION = 1 days;
        ...
        _stakeCounts = _stakeCounts.add(1);
        _stakeAddr.push(msg.sender);
        ...
        checkHalve
    {
        ...
        require(stakeTokenID[msg.sender].length >= amount, "stakeTokenID err");

        for(uint i=0;i<amount;i++)
        {
            ERC721(_nftAddr).safeTransferFrom(address(this),msg.sender,stakeTokenID[msg.sender][stakeTokenID[msg.sender].length - i -1]);
        }

        for(uint i=0;i<amount;i++)
        {
             stakeTokenID[msg.sender].pop();
        }
    ...
    function getReward() public updateReward(msg.sender) checkHalve checkStart {
        ...
        if (reward > 0) {
            ...
            if(leftReward>0){
                ...
                IPlayerBook(_inviteAddr).settleReward(msg.sender, leftReward);
            }
