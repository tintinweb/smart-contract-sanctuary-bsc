...
contract RDOM is Context, IBEP20, Ownable {
  ...
  constructor(string memory newname,string memory newsymbol,address new_teamAddr) public {
    _name = newname;
    _symbol = newsymbol;
    _decimals = 18;
    _totalSupply = 100*1e26;
  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if(!_isExcludedFromFee[sender] && !_isExcludedFromFee[recipient] && !isAddLiquidity && !isDelLiquidity)
    {
        ...
        {
            _fee = amount.mul(_tranferfee).div(BASE);
            _balances[_teamAddr] = _balances[_teamAddr].add(_fee);
            amount = amount.sub(_fee);
            
            emit Transfer(sender, _teamAddr, _fee);
        }
    }
  ...
  function excludeFromFee(address account) public onlyOwner {
    ...
    function setFee(uint new_tranferfee) onlyOwner external  {
        _tranferfee =  new_tranferfee;
    }

    function setTeamAddr(address new_teamAddr) onlyOwner external  {
        _teamAddr =  new_teamAddr;
    }
