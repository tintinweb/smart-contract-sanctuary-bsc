...
contract TESTYDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor(
        ...
        defaultToken = token;
        TESTYContract = TESTY(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "TESTY_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TESTY contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(TESTYContract);

            uint256 prevBalance = TESTYContract.balanceOf(address(this));
            ...
            {
                uint256 received = TESTYContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    TESTYContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
