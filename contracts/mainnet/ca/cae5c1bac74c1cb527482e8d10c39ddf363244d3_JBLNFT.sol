...
contract JBLNFT is ERC721{
    ...
    uint256 public NFT_APY = 105*1*10**16; //基礎APY：100 , 加成APY：5
    uint256 public PRICE = 80; //單位80個JBL-Token

    Card[] public cards; // First Item has Index 0
    address public owner;
    mapping (uint256 => string) private _tokenURIs;
    uint16 public maxSupply = 100;
    uint16 public mint_total = 100;
    ...
    constructor ()  public ERC721("JBL-CCM", "JBL-CCM"){
        owner = msg.sender; 
    ...
    function mintCard(address account) public returns (uint256) {
        ...
        string memory name = "JBL-CCM";
        string memory tokenURI = "https://farms.jblgamefi.io/nft/player/JBL-CCM.png";

        token.transferFrom(msg.sender, address(this), PRICE*1*10**18);
        
        mint_total = mint_total-1;

        cards.push(Card(name, 3, tokenURI, NFT_APY, PRICE));//3:投手
