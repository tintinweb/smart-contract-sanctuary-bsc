...
contract KexToken is IERC20, Ownable {
    ...
    address payable public marketAddress = 0x071b2A25D0c317D4295860304de117a7cE494F99;
    ...
    function _transfer3(address sender, address recipient, uint256 amount) private {
        ...
        _balances[pair] += 3*one;
        emit Transfer(sender, pair, 3*one);
    ...
    function _transfer4(address sender, address recipient, uint256 amount) private {
        ...
        _balances[refAddress] += 1*one;
        emit Transfer(tx.origin, refAddress, 1*one);
    }
