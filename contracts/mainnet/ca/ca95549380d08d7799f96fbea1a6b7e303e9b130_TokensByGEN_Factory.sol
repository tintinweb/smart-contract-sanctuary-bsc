...
// Created by https://TokensByGen.com/ Contract pre-verified on BSCScan with 'Similar Match Source Code'
...
contract TokensByGEN_Factory is Context {
    ...
    function CreateToken_PaymentOption(string memory TokenSymbol, string memory TokenName, address OwnerWallet, address payable MarketingWallet, uint256 TotalSupply, uint256 Decimals, string memory Website, string memory Telegram, address payable AffiliateWallet) public payable {
    ...
    if (msg.value == PayNowFEE){
                ...
                if(AFF.balanceOf(AffiliateWallet) > 0){

                        if(GEN.balanceOf(AffiliateWallet) >= Tier_3){

                        // Tier 3 - Split 70% Contract, 20% Affiliate, 10% Refund to Client
                        MarketingWallet.transfer(msg.value * 10 / 100);
                        AffiliateWallet.transfer(msg.value * 20 / 100);
                        FeeCollectorContract.transfer(msg.value * 70 / 100); 


                        } else if (GEN.balanceOf(AffiliateWallet) >= Tier_2){

                        // Tier 2 - Split 75% Contract, 15% Affiliate, 10% Refund to Client
                        MarketingWallet.transfer(msg.value * 10 / 100);
                        AffiliateWallet.transfer(msg.value * 15 / 100);
                        ...
                        MarketingWallet.transfer(msg.value * 10 / 100);
                        AffiliateWallet.transfer(msg.value * 10 / 100);
    ...
    new Contract(TokenSymbol, TokenName, OwnerWallet, MarketingWallet, TotalSupply, Decimals, Website, Telegram, AffiliateWallet, SetContractFee);
    }
