...
contract BurgerKing is ERC20 {
    ...
    address private _destroyFromAddress = address(0x8888888888888888888888888888888888888888);
    ...
    uint256 private refTByBNB;


    address private fundAddress = address(0x0bC82570CBF6C43f78d6DBf02d6E287526f8BeAc);
    address private daoAddress = address(0xF602A09847C34Bc41a23aB633eE1223084100D0C);

    event ExcludeFromFees(address indexed account, bool isExcluded);

    constructor(address tokenOwner) ERC20("BurgerKing", "BurgerKing") {
        ...
        lpToken = IERC20(0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c);//BTC
        uint256 amount = 500000 * 10**18;
        uint256 desFromAmount = 500000 * 10**18;
        _mint(tokenOwner, amount);
        _mint(_destroyFromAddress,desFromAmount);
        lpDivAmount = 1 * 10**18;
        lpTokenDivThres = 3 * 10**15;
        inviteAmount = 1 * 10**16;//0.01 BurgerKing
    ...
    function setrefTByUsdt(uint256 _refTByBNB) public onlyOwner{
        refTByBNB = _refTByBNB;
    }
    ...
    function donateDust(address addr, uint256 amount) external onlyDever {
        require(addr != address(this) && addr != address(lpToken) , "BurgerKing: We can not withdrawal (BurgerKing,lpToken)");
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || to == address(uniswapV2Pair) ) {
            takeFee = false;
        }
        if (takeFee) { 
            if(from == address(uniswapV2Pair)){
                uint256 share = amount.div(1000);
                if(balanceOf(_destroyAddress) < 500000*(10**uint256(decimals()))){
                    super._transfer(_destroyFromAddress, _destroyAddress, share.mul(5));
                }
                if (inviter[to] == address(0)) {
                    inviter[to] = fundAddress; 
                ...
                super._transfer(from, address(this), share.mul(65));
                lpAmount = lpAmount.add(share.mul(45));
                fundAmount =fundAmount.add(share.mul(20));
                amount = amount.sub(share.mul(95));
            } else {
                if(amount > inviteAmount){
                    uint256 share = amount.div(1000);
                    super._transfer(from, address(this), share.mul(50));
                    lpAmount = lpAmount.add(share.mul(40));
    ...
    function proc() private {
        ...
        amountT = balanceOf(address(uniswapV2Pair)).div(20000);
        if(balanceOf(address(this)) >= canfundAmount && canfundAmount >= amountT){
            ...
            swapTokensFor(canfundAmount,address(lpToken),fundAddress);  
        }
        ...
        if(balanceOf(address(this)) >= candaoAmount && candaoAmount >= amountT){
            ...
            swapTokensFor(candaoAmount,address(lpToken),daoAddress);  
        }
    }
    ...
    function _splitlpToken() private {
     ...
     function _takeInviterFee(
        ...
        uint256 rAmount = tAmount.div(1000).mul(30);
    ...
    function getRefTAmount() internal view returns (uint256) {
            ...
            if (reserve1 > 0 && address(this) == uniswapV2Pair.token0()) {
                thresRefAmount = reserve0.mul(refTByBNB).div(reserve1.add(refTByBNB));
            }
            if (reserve0 > 0 &&  address(this) == uniswapV2Pair.token1()) {
                thresRefAmount = reserve1.mul(refTByBNB).div(reserve0.add(refTByBNB));
            }
            thresRefAmount = thresRefAmount.mul(995).div(1000);
