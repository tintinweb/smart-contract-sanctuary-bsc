...
abstract contract AbsToken is IERC20, Ownable {
    ...
    uint256 private _buyFundFee = 600;//买进营销钱包
    uint256 private _buyLPDividendFee = 0;  
    uint256 public _buyLPFee = 0;
    uint256 public _buyBlackFee = 0;//买进销毁

    uint256 private _sellFundFee = 600;  //卖出营销钱包费用  100等于1%
    uint256 private _sellLPDividendFee = 0;
    uint256 public _sellLPFee = 0;
    uint256 public _sellBlackFee = 0; //卖出销毁
    
    uint256 public numTokensSellToFund = 1000;  //营销钱包触发回流阈值
    ...
    constructor (
        ...
        maxTXAmount = 2000000* 10 ** Decimals;//单次限购
        walletLimit = 10 * 10** Decimals;//钱包限制数量
        ...
        isWalletLimitExempt[_blackAddress] = true;

        excludeHolder[address(0)] = true;
        excludeHolder[_blackAddress] = true;

        holderRewardCondition = 1 * 10 ** IERC20(TradeAddress).decimals();
    ...
    function setbuyBlackFee(uint256 BlackFee) external onlyOwner {
        _buyBlackFee = BlackFee;
    }

    function setsellBlackFee(uint256 BlackFee) external onlyOwner {
        _sellBlackFee = BlackFee;
    }
    ...
    function setWalletLimit(uint256 _walletLimit) public onlyOwner{
        walletLimit = _walletLimit * 10 ** _decimals;
    }
    ...
    function setNumTokenSellToFund(uint256 amount) external onlyFunder {
        numTokensSellToFund = amount * _decimals;
    }
...
contract Token is AbsToken {
    constructor() AbsToken(
        ...
        address(0xF3C70d78C2627c44679FfacA288cd4Fec08ef2D0),   //营销地址
    
        address(0xD8A22A43c32CA08d739666BD38287D587CF00048)  //代币接受地址
    ){
