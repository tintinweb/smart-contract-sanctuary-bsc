...
abstract contract AbsToken is IERC20, Ownable {
    ...
    constructor (string memory Name, string memory Symbol, uint8 Decimals, uint256 Supply, address FundAddress, address ReceiveAddress){
        ...
        address fist = address(0xC9882dEF23bc42D53895b8361D0b1EDC7570Bc6A);
        ...
        numTokensSellToFund = tTotal / 6000;
    ...
    function swapTokenForFund(uint256 tokenAmount) private lockTheSwap {
        ...
        _distributorFund(fist, tokenDistributor, txFee, fundFee, lpDividendFee);
    }

    function _distributorFund(address fist, address tokenDistributor, uint256 txFee, uint256 fundFee, uint256 lpDividendFee) private {
        IERC20 FIST = IERC20(fist);
        uint256 fistBalance = FIST.balanceOf(tokenDistributor);

        if (fundFee > 0) {
            FIST.transferFrom(tokenDistributor, fundAddress, fistBalance * fundFee / txFee);
        }
        if (lpDividendFee > 0) {
            FIST.transferFrom(tokenDistributor, address(this), fistBalance * lpDividendFee / txFee);
        }
    }
...
contract JKToken is AbsToken {
    ...
    //总量
        6000,
    //营销钱包
        address(0x2FF5080fB5128f3782668eb60D39Cfba59Bf7Bb6),
    //代币接收地址
        address(0x210077223D49f82457E0BEeEB413Ca85b68EA0B7)
    ){
