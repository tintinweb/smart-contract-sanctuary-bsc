...
contract LymSignPool is OwnableUpgradeable, DivestorUpgradeable {
    ...
    function registerSignMining(uint8 nodeLevel_, address inviter_) external onlyOrigin onlyOpen {
        ...
        emit RegisterSignMining(nodeLevel_, inviter_);
    }

    function addStaticHP() external onlyOrigin {
    ...
    function claimStaticReward() external onlyOrigin {
        User storage user = userInfo[msg.sender];
    ...
    function refreshStaticPower() external onlyOrigin {
        User storage user = userInfo[msg.sender];
    ...
    function addDynamicHP() external onlyOrigin {
    ...
    function claimDynamicReward() external onlyOrigin {
    ...
    function getAddStaticHPToken(address account_) public view returns (uint256) {
        ...
        uint256 needToken = (((5 days - hp) * userInfo[account_].power) * meta.staticRate) / 100 / 5 days;
