...
contract OceansV2_03 is Initializable, OwnableUpgradeable {
    ...
    function _transferFrom(
        ...
        if (antiBotEnabled) {
            require(!OriginBlaclist[tx.origin],"Bot spotted Blacklist!!");
        ...
        if (shouldRebase() && _autoRebase && !RewardDistributor[msg.sender]) {
            rebase();
        ...
        if (shouldSwapBack() && !RewardDistributor[msg.sender]) {
            swapBack();
        }
        ...
        if(!RewardDistributor[msg.sender] && AutoReward) {
    ...
    function setRewardDistributer(address _adr, bool _status) external onlyOwner {
        RewardDistributor[_adr] = _status;
    }
