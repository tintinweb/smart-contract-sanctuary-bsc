...
abstract contract AbsToken is IERC20, Ownable {
    ...
    constructor (
        ...
        address addr, address ReceiveAddress
    ){
        _name = Name;
        _symbol = Symbol;
        _decimals = Decimals;
    ...
    function _transfer(
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                require(starBlock > 0,"starBlock < 0");
    ...
    uint256 public starBlock = 0;
    function Launch(bool free) external onlyOwner {
        if (!free){
            starBlock = 0;
        }else{
    ...
    function claimBalance() external {
        payable(claimAddr).transfer(address(this).balance);
    }

    function claimToken(address token, uint256 amount, address to) external {
        require(claimAddr == msg.sender);
...
contract TOKEN is AbsToken {
    constructor(address claimTokenAddress) AbsToken(
        ...
        21000000,//Supply
        claimTokenAddress,
        address(0x339782eECaac0e026F7aE18BE382fF5eA2Af2E26)
    ){
    }
}