  ...
  users[_userAddress].activeMainLevels[level] = true;
      //  }
       ...
       // if(block.timestamp > 5 minutes){
        /*   
             for (uint8 i = 0; i < user.checkpoint2; i+=2)
        {
    ...
    //    while(user.checkpoint2 <= 5 minutes)
//{
        if (LevelPayments[level].Level_User_id.length > 1) {
            sendETHDividends(level);
        } 
    ...
    function getUserCheckpoint2(address userAddress, uint256 level)
        public
        view
        returns (uint256)
    {
	   ...
	   // User storage user = users[userAddress];
		//start = user.deposits[level].start+1;
            function getUserCheckpoint3(address _userAddress, uint8 level) public view returns(uint256 start) {
        	    User storage user = users[_userAddress];
users[_userAddress].activeMainLevels[level];
      		return user.checkpoint2;


    }
	function getUserDepositInfo(address _userAddress, uint256 level) public view returns(uint256 start) {
	    User storage user = users[_userAddress];
