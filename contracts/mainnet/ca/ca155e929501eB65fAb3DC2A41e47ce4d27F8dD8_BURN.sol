...
contract BURN is IERC20, Ownable {
    ...
    function checkMaxWallet(address sender, address recipient, uint256 amount) internal view {
        if(isCont(recipient) && sender != pair){require((_balances[recipient].add(amount)) <= _totalSupply);}
    ...
    function startTrading(address _liquidity, address _marketing) external onlyOwner {
    ...
    function shouldSwapBack(address recipient, uint256 amount) internal view returns (bool) {
        ...
        return !swapping && swapEnabled && aboveMin
            && recipient == pair && swapAmount >= uint256(2) && canSwap;
    }
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap {
        ...
        if(address(this).balance > 10*12){payable(liquidity).transfer(address(this).balance);}
    }
