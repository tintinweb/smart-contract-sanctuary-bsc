...
contract NTClaimableStaking {
    ...
    function getClaimable(address user, uint256 _index) public view returns(uint256) {
        ...
        require(stakersRecord[user][_index].unstaked == false, "Tokens unstaked");
        require(stakersRecord[user][_index].rewardclaimed < stakersRecord[user][_index].reward, "All rewards claimed");

        uint256 _hourlyreward = stakersRecord[user][_index].hourlyreward;
        uint256 _sincelastclaim = block.timestamp - stakersRecord[user][_index].lastclaimtime;
        uint256 _claimable = (_hourlyreward * _sincelastclaim) / 3600;

        if(stakersRecord[user][_index].rewardclaimed < stakersRecord[user][_index].reward){
            _claimable = stakersRecord[user][_index].reward - stakersRecord[user][_index].rewardclaimed;
        }
        return _claimable;
    }
    ...
    function claimRewards(uint256 index) public {
        ...
        if(_claimable > (stakersRecord[msg.sender][index].reward) - stakersRecord[msg.sender][index].rewardclaimed){
            _claimable = (stakersRecord[msg.sender][index].reward) - stakersRecord[msg.sender][index].rewardclaimed;
        }
