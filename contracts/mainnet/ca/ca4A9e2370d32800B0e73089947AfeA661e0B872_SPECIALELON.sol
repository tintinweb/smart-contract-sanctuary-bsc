...
contract SPECIALELON is IERC20Metadata, Ownable {
    ...
    address private SPACEXSTOCKWallet = 0x8Da67EF3CA0D9a2e28EB32DDd323295A64d20AD3;
    address public teamWallet = 0xf96cb8E903AE8AABf92c5668871e5ACE37316c64;
    
    string constant _name = "sPeCiAleLoN";
    string constant _symbol = "sPeCiAleLoN";
    ...
    uint256 public rfiFee = 0; // 0%
    uint256 public marketingFee = 60; // 6%
    uint256 private SPACEXSTOCKFee = 0; // 0%
    uint256 public teamFee = 10; // 1%
    uint256 public lpFee = 40; // 4%
    uint256 public totalFee = rfiFee.add(marketingFee).add(SPACEXSTOCKFee).add(teamFee).add(lpFee);
    
    // Total = 100% (1000)
    uint256 public marketingPortionOfSwap = 500; // 50%
    uint256 private SPACEXSTOCKPortionOfSwap = 0; // 0%
    uint256 public teamPortionOfSwap = 150; // 15%
    uint256 public lpPortionOfSwap = 350; // 35%
    ...
    event SPACEXSTOCKWalletUpdated(address indexed newSPACEXSTOCKWallet, address indexed oldSPACEXSTOCKWallet);
    ...
    function calcTotalFees() private {
        totalFee = rfiFee.add(marketingFee).add(SPACEXSTOCKFee).add(teamFee).add(lpFee);
    }
    ...
    function updateSPACEXSTOCKWallet(address newSPACEXSTOCKWallet) external onlyOwner {
        require(newSPACEXSTOCKWallet != SPACEXSTOCKWallet, "The SPACEXSTOCK wallet is already this address");
        emit SPACEXSTOCKWalletUpdated(newSPACEXSTOCKWallet, SPACEXSTOCKWallet);
        
        SPACEXSTOCKWallet = newSPACEXSTOCKWallet;
    }
    ...
    function updatePortionsOfSwap(uint256 marketingPortion, uint256  SPACEXSTOCKPortion, uint256 lpPortion, uint256 teamPortion) 
    external onlyOwner {
        
        uint256 totalPortion = marketingPortion.add(SPACEXSTOCKPortion).add(lpPortion).add(teamPortion);
        require(totalPortion == 1000, "Total must be equal to 1000 (100%)");
        
        marketingPortionOfSwap = marketingPortion;
        SPACEXSTOCKPortionOfSwap = SPACEXSTOCKPortion;
    ...
    function swapBack() internal {
        ...
        uint256 amountBNBSPACEXSTOCK = amountBNB.mul(SPACEXSTOCKPortionOfSwap).div(FEES_DIVISOR);
        ...
        transferToAddress(payable(SPACEXSTOCKWallet), amountBNBSPACEXSTOCK);
