...
contract MC is ERC20 {
    ...
    uint256 private buymarketIntervalTime = 45 * 60;
    uint256 private sellmarketAndTeamIntervalTime = 30* 60;
    uint256 private lpIntervalTime = 15 * 60;

    uint256 public buyMcurrentTime;
    uint256 public sellMTcurrentTime;
    ...
    uint256 private minholdshare = 10**17;
    bool haveMaxLimit = true;

    uint256 public buyLpFee = 30;
    uint256 public buyLiquidFee = 10;
    uint256 public buyMarketFee = 20;
    uint256 public sellLpFee = 30;
    uint256 public sellTeamFee = 20;
    uint256 public sellLiquidFee = 10;
    uint256 public sellMarketFee = 20;

    address public buyMarketAddress = 0xfA69D510B71332e7908E7F83D5B86bd4b8C639AA;
    address public sellTeamAddress = 0xE18BeFcb1280625210680D73C08c3Bd1242c330D;
    address public sellMarketAddress = 0xf62e6e523D4b2416fE214786E3da0480aC3d0893;
    address public destroyAddress = address(0x000000000000000000000000000000000000dEaD);
    address public liquidAddress = 0x2c2ec2f39A735c67C50e33c8953808954FC26f9D;

    uint256 public buyMarketCount;
    uint256 public sellMarketCount;
    uint256 public sellTeamCount;
    ...
    string internal tokenName = "MC";
    string internal tokenSymbol = "MC";
    ...
    constructor(address tokenOwner) ERC20(tokenName, tokenSymbol) { 
        ...
        buyMcurrentTime = block.timestamp;
        sellMTcurrentTime = block.timestamp;
    ...
    function setMaxLimit(bool enable) public onlyOwner {
        haveMaxLimit = enable;
    }
    ...
    function setFee(uint256 _buyLpFee, uint256 _buyLiquidFee, uint256 _buyMarketFee,uint256 _sellLpFee, uint256 _sellTeamFee,uint256 _sellLiquidFee,uint256 _sellMarketFee) public onlyOwner {
        buyLpFee = _buyLpFee;
        buyLiquidFee = _buyLiquidFee;
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair){
            ...
            if(haveMaxLimit && !_isExcludedFromFees[to]){
                require(balanceOf(to)+amount <= 10 * 10**18,"exceed maxLimit");
            }
        }
        ...
        }else if(buyMarketCount > swapTokensAtAmount && block.timestamp >= (buyMcurrentTime.add(buymarketIntervalTime))){
            if (
                    !swapping &&
                    _tokenOwner != from &&
                    _tokenOwner != to &&
                    ...
                    swapping = true;
                    whichway = 1;
                    swapAndLiquifyV3(buyMarketCount);
                    buyMarketCount = 0;
                    buyMcurrentTime = block.timestamp;
                    swapping = false;
                }
        }else if((sellMarketCount + sellTeamCount) > swapTokensAtAmount && block.timestamp >= (sellMTcurrentTime.add(sellmarketAndTeamIntervalTime))){
            if (
                    !swapping &&
                    _tokenOwner != from &&
                    _tokenOwner != to &&
                    ...
                    swapping = true;
                    whichway = 2;
                    swapAndLiquifyV3(sellMarketCount + sellTeamCount);
                    sellMarketCount = 0;
                    sellTeamCount = 0;
                    sellMTcurrentTime = block.timestamp;
                    swapping = false;
        ...
        if (takeFee) {
            ...
            if(from == uniswapV2Pair){
                ...
                uint256 allFee = amount.div(1000).mul(buyLpFee + buyLiquidFee + buyMarketFee);

                uint256 cmarketCount = amount.div(1000).mul(buyMarketFee);
                uint256 cliquidCount = amount.div(1000).mul(buyLiquidFee);

                super._transfer(from, address(this), allFee);

                buyMarketCount =buyMarketCount.add(cmarketCount);
                liquidCount =liquidCount.add(cliquidCount);

                LPCount = LPCount.add((allFee.sub(cmarketCount)).sub(cliquidCount));

                amount = amount.div(1000).mul(1000 - (buyLpFee + buyLiquidFee + buyMarketFee));
                
            }else if(to == uniswapV2Pair){

                uint256 allFee = amount.div(1000).mul(sellLpFee + sellTeamFee + sellLiquidFee + sellMarketFee);

                uint256 cmarketCount = amount.div(1000).mul(sellMarketFee);
                uint256 cteamCount = amount.div(1000).mul(sellTeamFee);
                uint256 cliquidCount = amount.div(1000).mul(sellLiquidFee);

                super._transfer(from, address(this), allFee);

                sellMarketCount =sellMarketCount.add(cmarketCount);
                sellTeamCount =sellTeamCount.add(cteamCount);
                liquidCount =liquidCount.add(cliquidCount);

                LPCount = LPCount.add(allFee - (cmarketCount + cteamCount + cliquidCount));

                amount = amount.div(1000).mul(1000 - (sellLpFee + sellTeamFee + sellLiquidFee + sellMarketFee));
    ...
    function setShare(address shareholder) private {
        if(_updated[shareholder]){
            ...
            if (balanceOf(shareholder) < minholdshare) quitShare(shareholder);              
            return;  
        }

        if(balanceOf(shareholder) < minholdshare) return; 
    ...
    function _splitOtherTokenSecond(uint256 thisAmount) private {
        ...
        if(buySize>0){
            address user;
            uint256 totalAmount = totalSupply()-balanceOf(uniswapV2Pair)-balanceOf(destroyAddress)-balanceOf(address(this));
            uint256 rate;
            if(buySize >20){
                for(uint256 i=0;i<20;i++){
                    ...
                    if(balanceOf(user) >= minholdshare){
