...
contract SpinApeBuy is ReentrancyGuard, Ownable, Pausable {
    ...
    function buy(uint256 amount, address refBy)
        public
        nonReentrant
        whenNotPaused
    {
        ...
        require(
            amount >= 50,
            "Error: Minimum allocation is $50 and max allocation is 1000$"
        );
        require(
            usdtToken.balanceOf(msg.sender) >= amount * 1 ether,
            "Error: Invalid balance of USD"
        );
        require(
            totalTokenSaled + (amount * rate) <= totalToken,
            "Error: Invalid balance of Token"
        );
        require(startDate <= block.timestamp, "Error: Invalid Time");
        require(
            nextBuy <= maxBuy,
            "Error: Minimum allocation is $50 and max allocation is 1000$"
        );

        uint256 totalUsdNeed = RefContract.calculateFundRef(
            msg.sender,
            (amount * 1 ether),
            refBy
        );
        usdtToken.transferFrom(
            msg.sender,
            fundAddress,
            (amount * 1 ether) - totalUsdNeed
        );
        usdtToken.transferFrom(msg.sender, refAddress, totalUsdNeed);
    ...
    function harvest() public {
        ...
        spinApe.transfer(msg.sender, getAmount);
...
contract SpinApeRef is Ownable, ReentrancyGuard {
    ...
    function calculateFundRef(
        ...
        if (
            ...
            if (rfL1 != address(0)) {
                totalUsdOfUser[rfL1] += this.calculatePercent(amount, 800);
