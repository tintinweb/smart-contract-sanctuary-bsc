...
interface Event {
    event Recharge(address indexed from, uint256 value);
    event Swap(address indexed from, address indexed token, uint256 valueIn, uint256 valueOut);
    event Cust(address indexed from, bytes log);
}
...
contract CYT is Event {
    ...
    function swap(address router, uint256 amountIn, uint256 amountOutMin, address[] calldata path) public returns (bool) {
        ...
        emit Swap(msg.sender, path[1], amountIn, amounts[1]);
        return sent;
    }

    function queryETHBalance() public view returns (uint256) {
        return _balanceseth[msg.sender];
    }

    function queryTokenBalance(address token) public view returns (uint256) {
