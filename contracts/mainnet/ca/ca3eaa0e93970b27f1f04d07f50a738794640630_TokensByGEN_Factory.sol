...
/*
    ...
    This is an unruggable token - Created by TokensByGen 
    For more information on how this tokens works, visit TokensByGEN.com

    Doxed to GEN? Have a field that shows this and only I can update it!
    If they dox to me I will update this - they can't update it

    Maybe show the liquidity percent to show how much is burned (as people can always add more LP!)
...
contract Contract is Context, IERC20 { 
    ...
    */
    ...
    Your buy and sell fees can be set independently. However, both are limited to a maximum of 10% 
    ...
    */
    ...
    // Set Buy Fees - Max possible 10% (including 1% contract fee if applicable)
    function Step_2__Set_Fees_on_Buy(uint256 BNB_on_BUY, 
        ...
        // Buyer protection - max fee can not be set over 10% (including possible 1% contract fee if chosen)
        require(BNB_on_BUY + Liquidity_on_BUY + Reflection_on_BUY + Burn_on_BUY + Tokens_on_BUY + _Fee_Buy_Contract <= 10, "Buy Fees too high"); 
    ...
    Remember that the total sell fee is limited to 10% and includes the 1% contract fee if applicable.

    */

    // Set Sell Fees - Max possible 10% (including 1% contract fee if applicable)
    function Step_3__Set_Fees_on_Sell(uint256 BNB_on_SELL,
        ...
        // Buyer protection - max fee can not be set over 10% (including possible 1% contract fee if chosen)
        require(BNB_on_SELL + Liquidity_on_SELL + Reflection_on_SELL + Burn_on_SELL + Tokens_on_SELL + _Fee_Sell_Contract <= 10, "Sell Fees too high"); 
    ...
    ------------------------------
    4. Auto Removal of Reflections
    ------------------------------

    If your contract does not have reflections (you set the buy and sell reflection fee to 0) then just skip this step. 
    
    If you have added reflection rewards to your contract, you have the ability to automatically remove them at a set future date. 

    Reflections are great when you don't have too many holders. But over time they become increasingly diluted. 
    Eventually, each holder receives such a tiny amount that they are no longer worth while. 

    At this time, the refection rewards fee could be much better utilized elsewhere.

    A 2% fee divided over thousands of holders is negligible, but collectively focused on marketing it can really make a difference,
    it helps to bring in new investors and pumps the token for everybody. 
    
    ------------
    Your Options
    ------------

    If you want to keep reflection rewards indefinitely, just skip this step and move on to step 5.

    If you want to remove them, the first thing you need to enter is the number of days before they'll be removed. 
    I recommend keeping reflections for at least 3 months. If you'd like to do this, enter 90 as the 'Number_of_Days'
    The minimum possible is 30 days.

    Next, you need to decide what to do with the reflections fee. 

    If you just want to remove the fee completely (so that your total buy and sell fees are reduced) then enter 'false' for
    the 'Move_Fee_to_BNB' and 'Move_Fee_to_Tokens'. 

    If you prefer, you can keep your total fee the same and have this percentage moved to the BNB or Tokens wallet. Here it can be used for 
    marketing or Promotions etc, which will help the token (and all holders) much more than an extremely diluted reflection reward.

    You can not split it. You can only choose to have it removed completely, or moved to the BNB or Tokens Wallet.

    Whichever you decide, enter true for that option and false for the others. 

    The disadvantage of moving the fee to the BNB wallet is that your contract sells will be slightly higher. The BNB fee needs to sell
    tokens to get BNB to send to the external wallet, and that sell lowers your token price and puts a red candle on the chart. 

    The Tokens wallet doesn't need to sell tokens. So the price isn't affected. If you decide to move the fee to the token wallet, from here 
    it can be used for giveaways, adding additional liquidity, paying team members etc. Or even sold to get BNB to pay for marketing. 

    The Token wallet is very flexible, so it's often the better choice. 

    Remember that the token wallet is not excluded from limits. When it's full it'll stop receiving tokens and the transaction fees will be 
    temporarily reduced until there is enough space for it to receive more. 

    Of course, whatever you decide, you must be fully transparent and tell your investors. Post your intentions clearly on your 
    project website alongside your reasoning behind your decision. 

    */

    bool private Move_Fee_BNB = false;
    bool private Move_Fee_TOK = false;
    bool private AutoRemoveReflections = false;
    uint256 private Remove_Reflections_After_X_Days = 0;

    // Remove Reflections 
    function Step_4__Auto_Remove_Reflections(uint256 Number_of_Days, bool Move_Fee_to_BNB, bool Move_Fee_to_Tokens) external onlyOwner {

    ///    require(Number_of_Days >= 30, "Must be 30 days minimum");
        require(!Move_Fee_to_BNB || !Move_Fee_to_Tokens, "You can't pick both!");
        AutoRemoveReflections = true;
        Remove_Reflections_After_X_Days = Number_of_Days;
        Move_Fee_BNB = Move_Fee_to_BNB;
        Move_Fee_TOK = Move_Fee_to_Tokens;
    ...
    */
    ...
    function Step_5__Add_Project_URL(string memory Project_URL) external onlyOwner{
    ...
    */
    ...
    function Step_6__Blacklist_Bots(address[] calldata Wallets) external onlyOwner {
    ...
    ------------------------------------------------
    7. Add Liquidity, Renounce Ownership and Launch!
    ------------------------------------------------
    ...
    */
    ...
    function Step_7__Add_Liquidity_Launch_and_Renounce() payable external onlyOwner {

        uint256 BNBAmount = msg.value;
        
        // Initial LP must be 1 BNB or more!
      ///  require (BNBAmount >= 1*10**18, "1 BNB Minimum for LP");
        require (BNBAmount >= 1*10**14, "1 BNB Minimum for LP");
    ...
    */
    ...
    function _transfer(
        ...
        // Removing Reflections when they become too diluted
        if (AutoRemoveReflections){

        ///    if (block.timestamp > LaunchBlock + (Remove_Reflections_After_X_Days * 1 days)) {
  if (block.timestamp > LaunchBlock + ((Remove_Reflections_After_X_Days * 1 days)/24)) {

                if (Move_Fee_BNB){

                    // Move the fee to the BNB wallet
                    _Fee_Buy_BNB        += _Fee_Buy_Reflection;
                    _Fee_Sell_BNB       += _Fee_Sell_Reflection;

                    // Update Swap Fees
                    _SwapFeeTotal_Buy   = _Fee_Buy_BNB + _Fee_Buy_Liquidity + _Fee_Buy_Contract;


                } else if (Move_Fee_TOK){

                    // Move the fee to the token wallet
                    _Fee_Buy_Tokens     += _Fee_Buy_Reflection;
                    _Fee_Sell_Tokens    += _Fee_Sell_Reflection;
                ...
                // Remove reflection fee on buy and sell
                _Fee_Buy_Reflection     = 0;
                _Fee_Sell_Reflection    = 0;

                // Don't check again!
                AutoRemoveReflections   = false;
