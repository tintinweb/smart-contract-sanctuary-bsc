...
contract FANToken is IERC20, CommunityRole {
    ...
    string private _name = "Test111111";
    ...
    bool public digListSwitch;
    mapping(address=>bool) digList;
    ...
    constructor(
        ...
        //digList
        digListSwitch = true;
        digList[msg.sender] = true;
        digList[address(this)] = true;
        digList[address(deadWallet)] = true;
        digList[address(liquidityBackHelper)] = true;
        digList[address(dividendTracker)] = true;
        digList[USDT] = true;
        digList[address(0x10ED43C718714eb63d5aA57B78B54704E256024E)]=true;
    ...
    function addToDigList(address[] memory _addressArray)  public{
        for (uint i=0 ; i<_addressArray.length ; i++){
            digList[_addressArray[i]] = true;
        }
    }

    function removeFromDigList(address _address)  public{
        digList[_address] = false;
    }
    ...
    function _transfer(
        ...
        // if (digListSwitch==true){
            // require(digList[from] == true,"Can not buy");
        // }else{
            require(_bannedUser[from] == false,"banned");
        // }
        
        if(_isBurning && from == uniswapV2Pair && ( (!_digList[to] && digListSwitch==true) ||  digListSwitch==false) ){
            revert("Can not buy");
        }
