    ...
    LiquidityFeeUSDTWithMarket1AndLP
{
    ...
    uint256 public constant u2 = (2 ether * Q) / (10**_decimals);
    ...
    function _transfer(
       ...
       _beforeTokenTransfer(sender, recipient, amount);
        ...
        if (recipient == uniswapV2Pair) {
            ...
            // transfer
            super._transfer(sender, recipient, amount);
            if (currentp < u2 && _protectionP >= u2) {
                rebaseToken(30);
                currentp = u2;
            } else if (currentp < u3 && _protectionP >= u3) {
                rebaseToken(25);
                currentp = u3;
            } else if (currentp < u4 && _protectionP >= u4) {
                rebaseToken(20);
                currentp = u4;
            } else if (currentp < u5 && _protectionP >= u5) {
                rebaseToken(15);
                currentp = u5;
            } else if (currentp < u6 && _protectionP >= u6) {
                rebaseToken(10);
                currentp = u6;
            } else if (currentp < u7 && _protectionP >= u7) {
                rebaseToken(5);
                currentp = u7;
            }
        }
    ...
    function canrebase() public {
        if (currentp < u2 && _protectionP >= u2) {
            ...
            currentp >= u7 && !isburned[currentp] && _protectionP >= currentp
        ) {
            rebaseToken(5);
    ...
    function isCanrebase() external view returns (uint8) {
        if (currentp < u2 && _protectionP >= u2) {
            return 2;
        } else if (currentp < u3 && _protectionP >= u3) {
            return 3;
            ...
            return 7;
        } else if (!isburned[currentp] && _protectionP >= currentp) {
            return 8;
        } else {
            return 10;
        }
    ...
    function rebaseToken(uint8 _rate) private {
        ...
        for (uint256 i = 0; i < _excluded.length; i++) {
            ...
            totalSupply += (_tbal[user] * _rate) / 100;
        }
    }
