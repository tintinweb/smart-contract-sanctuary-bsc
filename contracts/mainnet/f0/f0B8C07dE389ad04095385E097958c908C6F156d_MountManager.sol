...
// File: nftManager/mounts/Mount.sol
 ...
 */
contract Mount is Context, ERC165, IERC721, IERC721Metadata, IERC2309, Ownable {
    ...
    uint256 private mintAmount = 100000;

    uint256 public lastTokenTransfered;
    ...
    string public NFTSbaseURI;

    address public MINTER;
    bool public emited = false;
     ...
     * @param nftId id of the nft in DB
     * @param rarity of the nft
     */
    event TransferNFT(address indexed from, address indexed to, uint256 tokenId, uint256 nftId, uint256 rarity);

    /**
     * @dev Initializes the contract by setting a `name` and a `symbol` to the token collection.
     */
    constructor() {
        _name = "Forgotten Mount";
        _symbol = "FGCMOUNT";
        _decimals = 1;

        setBaseURI("https://data.forgottenchain.com/nfts/0/mounts");
    }
    ...
    function ownerOf(uint256 tokenId) public view virtual override returns (address) {
        ...
        address owner = _owners[tokenId];
        if (owner == address(0)) {
            return MINTER; //If no owner set, then the minter is the owner
        }
        return owner;
    }
    ...
    /**
     * @dev See {IERC721Metadata-decimals}.
     */
    function decimals() public view virtual  returns (uint256 ) {
        return _decimals.div(10);
    }
    ...
    function setSymbol(string memory newSymbol) public ownerOrApprovedByOwner {
        _symbol = newSymbol;
    }

    function setName(string memory newName) public ownerOrApprovedByOwner {
        _name = newName;
    }
	
    function setBaseURI(string memory _newBaseURI) public  {
        require(msg.sender == owner(), "Owner only");
        NFTSbaseURI = _newBaseURI;
    }

    function setLastTokenTransfered(
		uint256 value
	) public ownerOrApprovedByOwner {
		require(value < mintAmount, "set lastToken value too big");
		lastTokenTransfered = value;
	}
     ...
     */
    function approve(address to, uint256 tokenId) public virtual override {
        address owner = Mount.ownerOf(tokenId);
        require(to != owner, "ERC721: approval to current owner");
    ...
    function awardNFT(
        ...
        require(lastTokenTransfered < mintAmount, "There are no more nfts for sale from this contract");
        bytes memory data = abi.encode(nftId, rarity);

        for (uint256 tokenId = lastTokenTransfered; tokenId <= mintAmount; tokenId++) {
            if (ownerOf(tokenId) == MINTER) {
                safeTransferFrom(from, to, tokenId, data);
                lastTokenTransfered = tokenId + 1;
                break;
            }
        }
    }
    ...
    function _safeTransfer(
        ...
        if(_data.length > 0) {
            (uint256 nftType, uint256 rarity)  = abi.decode(_data, (uint256,uint256));

            emit TransferNFT(from, to, tokenId, nftType, rarity);
        }
     ...
     */
    function _mintConsecutive() public onlyOwner {
        require(_tokenCount == 0, "You can only mint once!");
        require(mintAmount > 0);
        require(emited == false, 'already minted');
        require(MINTER != address(0), "minter not set");

        _tokenCount = mintAmount;
        _balances[MINTER] = mintAmount;

        emit ConsecutiveTransfer(0, mintAmount - 1, address(0), MINTER);
    }
    ...
    function _transfer(
        ...
        require(Mount.ownerOf(tokenId) == from, "ERC721: transfer of token that is not own");
        require(to != address(0), "ERC721: transfer to the zero address");
    ...
    function _approve(address to, uint256 tokenId) internal virtual {
        _tokenApprovals[tokenId] = to;
        emit Approval(Mount.ownerOf(tokenId), to, tokenId);
    }
...
// File: nftManager/mounts/MountManager.sol
...
contract MountManager is IERC721Receiver, Pausable, Ownable{
    using SafeMath for uint256;
    ...
    uint256 public commonPrice = 10; 
    uint256 public uncommonPrice = 25; 
    uint256 public rarePrice = 75; 
    uint256 public epicPrice = 200; 
    uint256 public legendaryPrice = 500; 
    ...
    function checkAmount(uint256 value, uint256 nftCount, uint256 rarity) internal returns (bool) {
        ...
        uint256 nftPrice = commonPrice;

        if(rarity == 0) { 
          //common 
          nftPrice = commonPrice;
        } else if(rarity == 1) { 
          //uncommon 
          nftPrice = uncommonPrice;
        } else if(rarity == 2) { 
          //rare 
          nftPrice = rarePrice;
        } else if(rarity == 3) { 
    ...
    function airdropERC721(address[] calldata _to, uint256[] calldata _id) public ownerOrApprovedByOwner whenNotPaused {
        ...
        for (uint256 i = 0; i < _to.length; i++) {
            Mount(NFTAddress).safeTransferFrom(address(this), _to[i], _id[i]);
        }
    }
    ...
    function awardNfts(uint256 nftCount, uint256 nftId, uint256 rarity) public payable whenNotPaused {
      ...
      for(uint i=0; i < nftCount; i ++) {
        Mount(NFTAddress).awardNFT(address(this), msg.sender, nftId, rarity);
      }
