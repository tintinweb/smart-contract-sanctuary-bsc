...
contract GuaranteeMasterChef2 is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    mapping(uint256 => uint256) public rewardsPerBlockTable;
    ...
    mapping(uint256 => NfaStakeInfo) public nftStakeInfo;
   ...
   function setRewardsPerBlock(uint256[] calldata tokenIds, uint256[] calldata rewards) external onlyOwner {
        for (uint256 i=0;i<tokenIds.length;i++) {
            uint256 reward = rewards[i];
            rewardsPerBlockTable[tokenIds[i]] = reward;
        }
   }
 ...
 function setStakedToken(address _tokenAddr) external onlyOwner {
    ...
    function pendingStakedToken(uint256 _tokenId) external view returns (uint256) {
        ...
        uint256 lastClaimed = nftStakeInfo[_tokenId].lastClaimed;
        uint256 currentBlock = block.number;

        if (currentBlock > nftStakeInfo[_tokenId].endBlock) {
            currentBlock = nftStakeInfo[_tokenId].endBlock;
        }

          if (lastClaimed > nftStakeInfo[_tokenId].endBlock) {
            lastClaimed = nftStakeInfo[_tokenId].endBlock;
        }

        uint256 rewardsPerBlock = rewardsPerBlockTable[_tokenId];
    ...
    function activateNfa(uint256 _tokenId) external {
    ...
    // update pool variables
        ...
        nftStakeInfo[_tokenId] = NfaStakeInfo({
            tokenId: _tokenId,
    ...
    function claim(uint256 _tokenId) external {
        ...
        nftStakeInfo[_tokenId].lastClaimed = block.number;
