...
interface IGooodfellasBinions is IERC721Enumerable {
 ...
 */
contract GooodfellasBinionsPremintSeller is ERC721Holder, Recoverable {
    using Address for address;

    IGooodfellasBinions public binions;

    constructor(
        address _binions
    ) {
        require(_binions != address(0), "Invalid mintable");
        binions = IGooodfellasBinions(_binions);
    }

    function buyPreminted(address to, uint256 amount) external payable nonReentrant {
        uint256 cost = binions.priceFor(to) * amount;

        require(msg.value == cost, "Value sent does not match price");
        require(binions.balanceOf(address(this)) >= amount, "Exceeds supply");

        Address.sendValue(payable(address(binions)), msg.value);

        for (uint256 i = 0; i < amount; ++i) {
            uint256 tokenId = binions.tokenOfOwnerByIndex(address(this), 0);
            binions.safeTransferFrom(address(this), to, tokenId);
        }
    }
