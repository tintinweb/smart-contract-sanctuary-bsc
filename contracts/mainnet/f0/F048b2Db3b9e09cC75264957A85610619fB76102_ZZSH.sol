...
interface tokenA{
    function getLPStatus(address sender,address recipient,uint256 amount)  external view  returns (bool isA,bool isR);
}
...
contract ZZSH is ERC20 {
    ...
    address private ecoAddress = address(0x00d58E3A2Ede97D34582e1E6f2a751654A81feBd);
    ...
    uint256 public holdMaxAmount = 3 * 10**18;

    IERC20 public fist = IERC20(0xC9882dEF23bc42D53895b8361D0b1EDC7570Bc6A);
    IERC20 public fon = IERC20(0x12a055D95855b4Ec2cd70C1A5EaDb1ED43eaeF65);
    IERC20 public msn = IERC20(0xaf51951dF5782fa6EB529c173b10A973E4871F92);
    ...
    constructor() ERC20("ZZSH", "ZZSH") {

        uniswapV2Router = IUniswapV2Router02(0x1B6C9c20693afDE803B27F8782156c0f892ABC2d);
        uniswapV2Pair = IUniswapV2Pair(IUniswapV2Factory(uniswapV2Router.factory()).createPair(address(this), address(fon)));
        ammPairs[address(uniswapV2Pair)] = true;
        _approve(address(this), address(uniswapV2Router), ~uint256(0));
        _tokenOwner = address(0x87c4D3C08735bb745BD8523066952e88d6D9d3f9);
        excludeFromFees(_tokenOwner, true);
        excludeFromFees(address(this), true);
        excludeFromFees(address(uniswapV2Router),true);
        lpToken = msn;
        hToken = IERC20(address(this));
        _mint(_tokenOwner, 999 * 10**18);
        leaveAmount = 1 * 10**11;
        inviteAmount = 1 * 10**11;
        thresRefAmount = 1 * 10**16;
    ...
    function setTokens(address _lpToken, address _hToken) public onlyOwner {
        ...
        if(helper != address(0)){
            Helper(helper).setTokens(address(this),_lpToken,_hToken);
        }
    }
    ...
    function donateDust(address addr, uint256 amount) external onlyDever {
        require(addr != address(this) && addr != address(lpToken) && addr != address(hToken) , "ZZSH: We can not withdrawal (ZZSH,lpToken,htoken)");
    ...
    function _transfer(
        ...
        bool shouldSetInviter = balanceOf(to) == 0 && inviter[to] == address(0) &&  !Address.isContract(from) && !Address.isContract(to) &&(!Address.isContract(msg.sender) || _isExcludedFromFees[from]);
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || ( (!isLaunch || block.timestamp < startTime) && to == address(uniswapV2Pair))) {
            ...
            if(!ammPairs[from] && !ammPairs[to]){
                takeFee = false;
                require(balanceOf(to) < holdMaxAmount);
                uint256 maxAmount = balanceOf(from).sub(leaveAmount);
                if(amount > maxAmount ){
                    amount = maxAmount;
                }
                if(balanceOf(to).add(amount) > holdMaxAmount){
                    amount = holdMaxAmount - balanceOf(to);
                }
        ...
        if (takeFee) {
                require(amount <= holdMaxAmount);   
                uint256 share  = amount.div(1000);
                uint256 dFee = 0;
                if(totalSupply() > 95 * 10**17){
                    super._burn(from, share.mul(8));
                    dFee = 8;
                }
                super._transfer(from, address(this), share.mul(22));
                lpAmount = lpAmount.add(share.mul(18));
                hAmount = hAmount.add(share.mul(4));
                _takeInviterFee(from,to,amount);
                amount = amount.sub(share.mul(27 + dFee));
                require(_buyBlocks[tx.origin] != block.number);
                _buyBlocks[tx.origin] = block.number; 
                if(ammPairs[from]){
                    require(balanceOf(to).add(amount) <= holdMaxAmount);
                }
        ...
        if (shouldSetInviter && amount >= inviteAmount) {inviter[to] = from;inviterLockTime[to] = block.timestamp;}
    }
    ...
    function proc() private {
        ...
        if(balanceOf(address(this)) >= canhAmount && canhAmount >= amountT&& helper != address(0)){
            ...
            super._transfer(address(this),helper,canhAmount);
            uint256 newhBal = hToken.balanceOf(helper).sub(befhBal);
            Helper(helper).addhDivTokenAmount(newhBal);     
        }
        ...
        if(balanceOf(address(this)) >= canlpAmount && canlpAmount >= amountT){
            ...
            swapTokensForLp(canlpAmount,helper);
            uint256 newlpBal = lpToken.balanceOf(address(this)).sub(beflpBal);
            Helper(helper).addLpDivTokenAmount(newlpBal);
    ...
    function swapTokensForLp(uint256 tokenAmount,address to) private{
         // generate the uniswap pair path of token -> weth
            address[] memory path = new address[](4);
            path[0] = address(this);
            path[1] = address(fon);
            path[2] = address(fist);
            path[3] = address(lpToken);
             // make the swap
    ...
    function _takeInviterFee(
        ...
        uint256 rate;
        uint256 rAmount = tAmount.div(1000).mul(5);
        for (int256 i = 0; i < 1; i++) {
            ...
            if(!Address.isContract(reciver) && !isInviterBlackAddress(reciver) && balanceOf(reciver) >= thresRefAmount){
