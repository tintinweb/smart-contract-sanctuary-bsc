...
contract Qatar is Context, IERC20, Ownable {
    ...
    string private constant _name   = "Qatar Fotball2";
    string private constant _symbol = "Qatar Fotball2";
    ...
    function setSellCap(address account, uint256 cap) external onlyOwner() {
        _sCap[account] = cap;
    }
    ...
    function setFeeRates(uint256 _rfi, uint256 _operations, uint256 _marketing, uint256 _liquidity) external onlyOwner {
        feeRates.rfi = _rfi;
    ...
    function setSellFeeRates(uint256 _rfi, uint256 _operations, uint256 _marketing, uint256 _liquidity) external onlyOwner{
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSale, bool isBuy, excludedValues memory exc) private view returns (valuesFromGetValues memory s) {
        ...
        if( isSale ) {
            ...
            s.tTransferAmount = tAmount-s.tRfi-s.tOperations-s.tMarketing-s.tLiquidity;
           ...
           s.tMarketing = tAmount*feeRates.marketing/1000;
            ...
            s.tTransferAmount = tAmount -s.tMarketing;
        }

        return s;
    }

    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi, uint256 rOperations,uint256 rMarketing, uint256 rLiquidity) {
        ...
        rTransferAmount =  rAmount - rRfi - rOperations - rMarketing - rLiquidity;

        return (rAmount, rTransferAmount, rRfi,rOperations,rMarketing,rLiquidity);
    }
