...
contract GooCoinx is BEP20, Ownable {
    ...
    constructor() BEP20("GooCoinx", "GOX") {
        marketingWallet = payable(0x77Daf3c64513c182c8399C5C204F104A23B957B2); 
    ...
    function _transfer(
        ...
        if (_isLocked[from]) {
            require(balanceOf(from).sub(amount) >= tokensLocked[from]);
        }
        if (amount == 0) {
    ...
    function airdropToWallets(
        ...
        require(airdropWallets.length == amount.length);
        require(airdropWallets.length <= 200);
