...
contract Fqswap is IFqswap, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    function __Fqswap_tiers_init() internal initializer {
        ...
        _addTier(500, 100, 0, 300, address(0));
        _addTier(500, 100, 100, 100, address(0));
        _addTier(150, 150, 150, 150, address(0));
    }
    ...
    function checkFees(FeeTier memory _tier) internal view returns (FeeTier memory) {
        uint256 _fees = _tier.liquidityFee.add(_tier.taxFee).add(_tier.ownerFee).add(
            _tier.burnFee
        );
    ...
    function addTier(
        ...
        _addTier(_liquidityFee, _taxFee, _ownerFee, _burnFee, _owner);
    }
    ...
    function _addTier(
        ...
        FeeTier memory _newTier = checkFees(
            FeeTier(_liquidityFee, _taxFee, _ownerFee, _burnFee, _owner)
        );
        excludeFromReward(_owner);
