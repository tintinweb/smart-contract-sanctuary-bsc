...
// *OY DONT YOU SNIPE THIS STUFF BOIS n GALS. IM TESTING THE CA!
...
contract TheBinanceProtocol is IBEP20, Ownable {
    ...
    string private _name = "The Binance Protocol";
    ...
    TierTaxes public _buyTaxes = TierTaxes({first: 13, second: 11, third: 9});

    TierTaxes public _sellTaxes = TierTaxes({first: 13, second: 11, third: 9});
    ...
    TaxWallets public _taxWallet =
        TaxWallets({
            dev: 0x738b436fd6Cc27C5056F380279364fbf06fbf4aa,
            TreatBag: 0x738b436fd6Cc27C5056F380279364fbf06fbf4aa,
            PartySupplies: 0x738b436fd6Cc27C5056F380279364fbf06fbf4aa
        });

    MaxLimits public _limits;

    LimitRatios public _limitRatios =
        LimitRatios({wallet: 2, sell: 2, buy: 2, divisor: 200});
    ...
    address public mainReward = 0x8BaBbB98678facC7342735486C851ABD7A0d17Ca;
    ...
    event RatiosChanged(
        ...
        uint8 newLiquidity,
        uint8 newTreatBag,
        uint8 newPartySupplies,
        uint8 newRewards
    );
    event UpdateDevWallet(address newDevWallet);
    event UpdateTreatBagWallet(address newTreatBagWallet);
    event UpdatePartySuppliesWallet(address newPartySuppliesWallet);
    ...
    constructor() {
        ...
        _excluded.add(_taxWallet.PartySupplies);
        _excluded.add(_taxWallet.dev);
        _excluded.add(_taxWallet.TreatBag);
    ...
    function _authorized(address addr) private view returns (bool) {
        return
            addr == owner() ||
            addr == _taxWallet.PartySupplies ||
            addr == _taxWallet.dev ||
            addr == _taxWallet.TreatBag;
    }
    ...
    function withdrawTreatBag() external authorized {
        uint256 amount = TreatBagBalance;
        TreatBagBalance = 0;
        _sendBnb(_taxWallet.TreatBag, amount);
    }

    function withdrawPartySupplies() external authorized {
        uint256 amount = PartySuppliesBalance;
        PartySuppliesBalance = 0;
        _sendBnb(_taxWallet.PartySupplies, amount);
    }
    ...
    function setTreatBagWallet(address payable addr) external authorized {
        address prevTreatBag = _taxWallet.TreatBag;
        _excluded.remove(prevTreatBag);
        _taxWallet.TreatBag = addr;
        _excluded.add(_taxWallet.TreatBag);
        emit UpdateTreatBagWallet(addr);
    }

    function setPartySuppliesWallet(address payable addr) external authorized {
        address prevPartySupplies = _taxWallet.PartySupplies;
        _excluded.remove(prevPartySupplies);
        _taxWallet.PartySupplies = addr;
        _excluded.add(_taxWallet.PartySupplies);
        emit UpdatePartySuppliesWallet(addr);
    }
    ...
    function getWithdrawBalances()
        ...
        returns (
            ...
            uint256 dev,
            uint256 TreatBag,
            uint256 PartySupplies
        )
    {
        return (buybackBalance, devBalance, TreatBagBalance, PartySuppliesBalance);
    }
    ...
    uint256 private TreatBagBalance;
    uint256 private PartySuppliesBalance;
    ...
    function _distributeStake(uint256 bnbAmount, bool newStakingReward)
        private
    {
        uint256 PartySuppliesSplit = (bnbAmount * _taxRatios.PartySupplies) /
            distributeRatio;
        ...
        uint256 stakingSplit = (bnbAmount * _taxRatios.rewards) /
            distributeRatio;
        uint256 TreatBagSplit = (bnbAmount * _taxRatios.TreatBag) / distributeRatio;
        uint256 mainAmount = (stakingSplit * mainRewardSplit) / 100;
        uint256 miscAmount = (stakingSplit * miscRewardSplit) / 100;
        PartySuppliesBalance += PartySuppliesSplit;
        devBalance += devSplit;
        buybackBalance += buybackSplit;
        TreatBagBalance += TreatBagSplit;
        if (stakingSplit > 0) {
            if (newStakingReward) totalRewards += stakingSplit;
            uint256 totalShares = getTotalShares();
            if (totalShares == 0) PartySuppliesBalance += stakingSplit;
            else {
