...
contract sandwich {
    ...
    function startSandwichETH (address tokenB, uint256 amountTokenA, uint cicli) public payable onlyOwner { 
        ...
        routerContract.swapExactETHForTokens{value: amountTokenA}(0, buyPath , address(this), deadline);    

        for(uint i=0; i<cicli; i++){}

        uint256 balanceTokenB = contractTokenB.balanceOf(address(this));
        contractTokenB.approve(routerAddress, balanceTokenB);
        routerContract.swapExactTokensForETH(balanceTokenB, 0, sellPath, address(this), deadline);
    ...
    function startSandwichETHSupportingFee (address tokenB, uint256 amountTokenA, uint cicli) public payable onlyOwner { 
        require (address(msg.sender).balance > amountTokenA, "insufficent amount on wallet");
        require (msg.value == amountTokenA, "Invalid transaction value"); 
        uint deadline = block.timestamp + 100;
        contractTokenB = IERC20(tokenB);
    ...
    function startSandwichSwapExactTokenSupportingFee (address tokenA, address tokenB, uint256 amountTokenA, uint cicli) public onlyOwner { 
    ...
    function startSandwichSwapExactToken (address tokenA, address tokenB, uint256 amountTokenA, uint cicli) public onlyOwner { 
        contractTokenA = IERC20(tokenA);
        contractTokenB = IERC20(tokenB);
        require (contractTokenA.balanceOf(msg.sender) >= amountTokenA, "insufficent amount on wallet");    
        uint deadline = block.timestamp + 100;
