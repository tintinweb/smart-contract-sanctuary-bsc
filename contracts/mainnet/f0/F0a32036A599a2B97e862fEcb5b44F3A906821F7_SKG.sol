...
contract SKG is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    address public devAddress = address(0xbFb624C7a3d633d78f8263CCe4B90110b823Bd3B);
    address public devAddress2 = address(0x119Dfa87c510B1B4789c8d6aADF1FeD338e9bec0);

    address public holdAddr=0x000000000000000000000000000000000000dEaD;

    uint256 public gasForProcessing;

    // exlcude from fees and max transaction amount
    mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private isRoute;
    ...
    constructor() payable ERC20("DreamWorks SKG", "SKG") {
        rewardToken = address(this);

        swapTokensAtAmount = 100*10**18; // 0.002%

        // use by default 300,000 gas to process auto-claiming dividends
        gasForProcessing = 300000;

        dividendTracker = BABYTOKENDividendTracker(
            payable(Clones.clone(0x50f99946E5702cB21851cfbD684D1D6b23236fFA))
        );
    ...
    function _transfer(
        ...
        if (takeFee) {
            if (from==uniswapV2Pair && isRoute[to]){
                ...
                super._transfer(from, address(this), amount.mul(15).div(1000));
                super._transfer(from, uniswapV2Pair, amount.mul(10).div(1000));
                super._transfer(from, devAddress, amount.mul(40).div(1000));
                super._transfer(from, holdAddr, amount.mul(15).div(1000));
                super._transfer(from, devAddress2, amount.mul(20).div(1000));
                ...
                if (isRoute[from]){
                    ...
                    if(parentList[to]==address(0) && !isContract(from) && !isContract(to)){
                        parentList[to]=from;
                    }
                    totalFees=0;
                }
    ...
    function _takeInviterFee(
        ...
        for (int256 i = 0; i < 2; i++) {
            ...
            if (paddr == address(0)) {
                ...
                super._transfer2(addr, devAddress, _amount.mul(rate).div(1000));
            }else{
                super._transfer2(addr, paddr, _amount.mul(rate).div(1000));
            }
    ...
    function swapAndSendDividends(uint256 tokens) private {
        super._transfer2(address(this),address(dividendTracker),tokens);
