...
abstract contract AbsToken is IERC20, Ownable {
    ...
    constructor (
        ...
        holderRewardCondition = 15 * 10 ** IERC20(FISTAddress).decimals();

        _tokenDistributor = new TokenDistributor(FISTAddress);

        _numToSell = 20000 * 10 ** Decimals;
    }
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        uint256 fundAmount = fistBalance * _sellFundFee / swapFee;
        if (fundAmount > 0) {
            FIST.transferFrom(tokenDistributor, fundAddress, fundAmount);
        }

        uint256 dividendAmount = fistBalance * _sellLPDividendFee / swapFee;
        if (dividendAmount > 0) {
            FIST.transferFrom(tokenDistributor, address(this), dividendAmount);
        }
    }
...
contract FLYToken is AbsToken {
    constructor() AbsToken(
        ...
        18,
        10000000,
        address(0x7cF155f6ed3517436d95d2c13589899c947e04AE),
        address(0xb27Ccd59257A5733AEA12331336C00792095Afd2)
    ){
