...
contract TNTW is IERC20, Ownable {
    ...
    address private fundAddress = address(0xBFB6F5658a2b95DCe53aa7C4cfa31f444539ee67);
    address private fundAddress2 = address(0x31363722Af238115B846a77d355A4007537F3769);

    address private inviteAddress = address(0xBFB6F5658a2b95DCe53aa7C4cfa31f444539ee67);


    mapping(address => address) public inviter;
    mapping(address => address) public inviterbefore;
    ...
    event  invite(address add,address addr,uint8 tt);
    ...
    function _transfer(
        ...
        if (from==uniswapV2Pair && isRoute[to]){
            ...
            takeFee = true;
            if(inviterbefore[to]!=address(0) && inviter[to]==address(0)){
                inviter[to] = inviterbefore[to];
                emit invite(inviterbefore[to],inviter[to],7);
            }

        }else if (to==uniswapV2Pair) {
            //            if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            //                require(amount<=balanceOf(from).mul(99).div(100),"sell gt 99");
            //            }
            takeFee = true;
        ...
        bool shouldSetInviter = inviter[to] == address(0) && inviterbefore[to] == address(0) && !isContract(from) && !isContract(to);

        _tokenTransfer(from, to, amount, takeFee);


        if (shouldSetInviter) {
            inviterbefore[to] = from;
        }
        ...
        if (takeFee) {
            ...
            if (sender==uniswapV2Pair && isRoute[recipient]){
                ...
                if (sender!=uniswapV2Pair && isRoute[sender]){
                    ...
                    _takeInviterFee2(sender, recipient, tAmount, currentRate);
                }else {
                    _destroyFee = 2;
                    _fundFee = 3;
                    _taxFee = 1;
