...
contract RMCVest is Ownable, ReentrancyGuard {
    ...
    function unlock(uint _index) external nonReentrant {
        ...
        uint cycleTimes = (block.timestamp - lock.unlockedAt) / lock.unlockCycle;
        if (lock.lockedAt == lock.unlockedAt) {
            // In case of first unlocking...
            cycleTimes = ((block.timestamp - lock.lockedAt - lock.lockPeriod) / lock.unlockCycle) + 1;
        }
        uint unlockAmount = cycleTimes * lock.initialAmount * lock.unlockPercent / 100;
        if (unlockAmount > lock.amount) unlockAmount = lock.amount;
