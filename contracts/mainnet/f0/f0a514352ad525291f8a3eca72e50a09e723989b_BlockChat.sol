...
contract Message is ERC20 {
    ...
    modifier require_not_in_blacklist(address box) {
        require(check_receiver_blacklist(Block_owner[box]) != true, "You blacklisted by this block.");
        _;
    }
    ...
    function Block_blacklist_Block(address target, string memory password) public require_Block_password(msg.sender,password) {
        blacklisted[msg.sender][Block_owner[target]] = true;
    }
    ...
    function check_receiver_blacklist(address box) public view require_non_zero(box) returns(bool) {
        return blacklisted[Block_owner[box]][msg.sender];
    }
...
contract BlockChat is Message {
    ...
    function Block_Chat(address receiver, string memory _message) public require_not_in_blacklist(Block_owner[receiver]){
    ...
    function FiftyFifty() public require_level_and_balance(level_require,((1 + check_user_level()) * 10 ** decimals()) * MSGs_burnt / MSGs_minted) {
        ...
        if (Drawer == 50){
            ...
            FiftyDraw++;
            Block[address(this)].push(Chat(msg.sender, block.timestamp, string("I won 50x of my bet from FiftyDraw!!!")));
            Drawer = 0;
        }
        if (FiftyDraw >= 1){
            FiftyDraw--;
        ...
        if (FiftyDraw == 0){
            pool(msg.sender, check_saving_balance(msg.sender) + bet_size * 2);
            FiftyDraw++;
            Block[address(this)].push(Chat(msg.sender, block.timestamp, string("I won 2x $MSG from the FiftyDraw!!!")));
        }
    }
    function Block_coinThrowAttack(address spammer, uint amount) public require_level_and_balance(level_require,amount * 10 ** decimals()) {
        require(VIP[Block_owner[spammer]] == false, "Can not attack VIPs.");
        _burn(msg.sender, amount * 10 ** decimals());
        MSGs_burnt += (amount * 10 ** decimals());
        pool(msg.sender, check_saving_balance(msg.sender) + check_saving_balance(Block_owner[spammer]) / (amount * check_user_level() * 2));
        pool(Block_owner[spammer], check_saving_balance(Block_owner[spammer]) / (amount * check_user_level()));
    ...
    function Block_blockchat_pay(address receiver, uint amount, string memory password) public require_Block_password(msg.sender,password) {
        ...
        pool(Block_owner[receiver], check_saving_balance(Block_owner[receiver]) + (amount * 10 ** decimals()));
    ...
    function team_airdrop(address[] memory list, uint amount, string memory password) public require_Block_password(team,password) {
        ...
        while (airdrop_address_left > 0) {
            pool(Block_owner[list[airdrop_address_left - 1]], check_saving_balance(Block_owner[list[airdrop_address_left - 1]]) + (amount * 10 ** decimals()));
...
// BlockChat Research LTD 2022-10-30 (1.0 version on Binance Smart Chain)