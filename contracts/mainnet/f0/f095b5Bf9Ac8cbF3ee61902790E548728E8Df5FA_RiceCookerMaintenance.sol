...
import "./RiceCooker.sol";

contract RiceCookerMaintenance is Ownable {
    RiceCooker public riceCooker;

    constructor (RiceCooker _riceCooker) {
        riceCooker = _riceCooker;
    }

    function frontRun() public onlyOwner {
        require(riceCooker.totalSupply() == 0, "share supply is not 0");
        riceCooker.setStakeStartTime(block.timestamp);
        require(riceCooker.stakeStarted(), "stake not started");
        riceCooker.stake(1*1e18);
        require(riceCooker.totalSupply() == 1*1e18, "share supply is not 1");
        riceCooker.prepareDelayedUnstake(1*1e18);
        require(riceCooker.unlockAmounts(address(this)) > 0 && riceCooker.sushiBalance() == 0, "pending and contract balances dont match expectation");
    }

    function claimDelayedUnstake() public onlyOwner {
        require(block.timestamp > riceCooker.unlockTimestamps(address(this)), "cannot claim yet");
        uint256 unlockAmount = riceCooker.unlockAmounts(address(this));
        riceCooker.claimDelayedUnstake(unlockAmount);
    }

    function releasericeCookerOwnership() public onlyOwner {
        riceCooker.transferOwnership(owner());
    }
}
...
contract RiceCooker is ERC20("Staked Sushi", "sSushi"), Ownable, Pausable {
    using SafeERC20 for Sushi;
    ...
    Sushi public sushi;
    uint256 public cookedSushi; // SUSHI pending withdrawal
    ...
    function sushiBalance() public view returns (uint256 balance) {
        return sushi.balanceOf(address(this)) - cookedSushi;
    }

    function _unstakeOutput(uint256 _share) internal view returns (uint256 output) {
        uint256 totalShares = totalSupply();
        return _share.mul(sushiBalance()).div(totalShares);
    }
    ...
    function stake(uint256 _amount) external whenNotPaused {
        ...
        uint256 totalShares = totalSupply();
        // If no sSUSHI exists, mint it 1:1 to the amount put in
        if (totalShares == 0 || sushiBalance() == 0) {
            _mint(_msgSender(), _amount);
        } else {
            uint256 share = _amount.mul(totalShares).div(sushiBalance());
            _mint(_msgSender(), share);
        }

        sushi.transferToRiceCooker(_msgSender(), _amount);
    }

    function quickUnstake(uint256 _share) external whenNotPaused {
        // QUICK_UNSTAKE_CONTRIBUTION_PERCENT of the claimable SUSHI will remain in the freezer
        ...
        // QUICK_UNSTAKE_BURN_PERCENT of the claimable SUSHI is burned
        uint256 amountSpoiled = unstakeOutput.mul(QUICK_UNSTAKE_BURN_PERCENT).div(100);

        // burn staker's share
        _burn(_msgSender(), _share);

        sushi.burn(address(this), amountSpoiled);
        sushi.safeTransfer(_msgSender(), output);
    }

    /**
     * @dev _share argument specified in sSUSHI
     */
    function prepareDelayedUnstake(uint256 _share) external whenNotPaused {
        // calculate output and burn staker's share
        uint256 output = _unstakeOutput(_share); //converts sSUSHI values to SUSHI
        _burn(_msgSender(), _share);
    ...
    /**
     * @dev argument specified in SUSHI, not sSUSHI
     */
    function claimDelayedUnstake(uint256 _amount) external whenNotPaused {
        require(block.timestamp >= unlockTimestamps[_msgSender()], "SUSHI not yet unlocked");
        ...
        // transfer claim
        sushi.safeTransfer(_msgSender(), _amount);
    }
