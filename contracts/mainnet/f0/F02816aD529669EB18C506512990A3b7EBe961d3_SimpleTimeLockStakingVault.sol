...
contract SimpleTimeLockStakingVault is StaticFixedTimeLockStaking {
    function initialize(
        ...
        __FixedTimeLockStaking_init_unchained(_lockTime, _earlyUnlockPenalty);
    }
}
...
contract StaticFixedTimeLockStaking is FixedTimeLockStaking {
    ...
    function _start() internal virtual override(BaseStaking) {
        BaseStaking._start();
        unlockTime = block.timestamp + (lockTime * 1 days);
    }
    ...
    function lockEnded(address account) public view virtual override returns (bool) {
        return super.lockEnded(account) && block.timestamp >= 1662258367;
    }
