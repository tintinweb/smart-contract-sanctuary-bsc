    ...
    VRFConsumerBaseV2Upgradeable
{
    ...
    mapping(uint8 => string) public baseURIByType;
    mapping(uint256 => SharedStructs.RequestData) randomRequestMap;
    mapping(uint256 => SharedStructs.VechicleInfo) public vehiclesInfoMap;
    ...
    function initialize(
        ...
        __ERC721_init("Outer Ring Land Vehicle", "ORLV");
    ...
    function setBaseURI(uint8 _type, string memory _baseURI)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        baseURIByType[_type] = _baseURI;
    }
    ...
    function safeMint(
        address to,
        string memory uri,
        SharedStructs.VechicleInfo memory _vehiclesInfoMap
    ) public onlyRole(MINTER_ROLE) {
        uint256 _tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        _safeMint(to, _tokenId);
        emit Minted(to, _tokenId);
        vehiclesInfoMap[_tokenId].typeId = _vehiclesInfoMap.typeId;
        vehiclesInfoMap[_tokenId].typeName = _vehiclesInfoMap.typeName;
        vehiclesInfoMap[_tokenId].collectionId = _vehiclesInfoMap.collectionId;
        vehiclesInfoMap[_tokenId].collectionName = _vehiclesInfoMap
            .collectionName;
        vehiclesInfoMap[_tokenId].firstOwner = to;
        _setTokenURI(_tokenId, uri);
        emit URIGeneration(_tokenId, uri);
    }

    /// @notice This function is used for LootBoxes and random generation
    /// @param _to Recipient address
    /// @param _tier The tier of the loot box selected
    /// @param _landVehicleConfig Structure for land vehicles info
    function lootBoxMint(
        address _to,
        uint8 _tier,
        SharedStructs.LandVehicleTierConfig memory _landVehicleConfig
    ) public onlyRole(MINTER_ROLE) {
        uint256 _tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        ...
        vehiclesInfoMap[requestData.tokenId].typeName = _landVehicleConfig
            .landVehicleTypeName;
        vehiclesInfoMap[_tokenId].collectionId = _landVehicleConfig
            .collectionId;
        vehiclesInfoMap[_tokenId].collectionName = _landVehicleConfig
            .collectionName;
        vehiclesInfoMap[_tokenId].firstOwner = _to;
    ...
    function fulfillRandomWords(
        ...
        uint8 _type = uint8(randomWords[0] % module);
        vehiclesInfoMap[requestData.tokenId].typeId = _type;
        string memory _tokenURI = baseURIByType[_type];
