...
contract UFAILED is Ownable, IERC20 {
    ...
    string public constant _name = "UFAILEDWIN";
    string public constant _symbol = "UFAILED";
    ...
    constructor(address routerAddr, address _marketWallet) {
    ...
    function _burnFee(uint256 tFee) private {
        if (tFee == 0) {
            return;
        }
        _tFeeTotal = _tFeeTotal.add(tFee);
        _balances[address(this)] = _balances[address(this)].add(tFee);
        _burn(address(this), tFee);
    }
    ...
    function _takeMarket(uint256 tMarket) private {
        ...
        emit Transfer(msg.sender, address(this), tMarket);
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private nonReentrant {
        ...
        uint256 forEth = (contractTokenBalance *
            (_liquidityFee * 5 + _marketFee * 10)) /
            ((_liquidityFee + _marketFee) * 10);
        uint256 otherHalf = contractTokenBalance.sub(forEth);
        ...
        // swap tokens for ETH
        swapTokensForEth(forEth); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered

        // how much ETH did we just swap into?
        uint256 newBalance = address(this).balance.sub(initialBalance);
        uint256 forMarket = (newBalance * _marketFee) /
            (_liquidityFee + _marketFee);
        payable(marketWallet).transfer(forMarket);

        // add liquidity to uniswap
        addLiquidity(otherHalf, address(this).balance.sub(initialBalance));

        emit SwapAndLiquify(forEth, newBalance, otherHalf);
    }
