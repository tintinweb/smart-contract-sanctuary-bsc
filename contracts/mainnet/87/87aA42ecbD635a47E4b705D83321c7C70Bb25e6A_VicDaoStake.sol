...
contract VicDaoStake is BEPPausable {
    event fortuneDrawDone(address _address, uint256 ticket, uint256 reward);
    ...
    uint256 public forTuneDrawReward;
    ...
    struct StakeByDate{
     ...
     constructor() payable{
        ...
        VICDAOMainContract = 0x4F77189ac55379e798d462428eBFA18ca571Ea90;
        address VIMContract = VICDAOMainInterface(payable(VICDAOMainContract)).getVIMContract();
    ...
    function setTokenOfTicket(uint256 _tokenOfTicket) external onlyOwner{
        tokenOfTicket = _tokenOfTicket;
    }
    function setForTuneDrawReward(uint256 _forTuneDrawReward) external onlyOwner{
        forTuneDrawReward = _forTuneDrawReward;
    }
    ...
    function stake(uint256 amount, address tokenAddress) external whenNotPaused{
        ...
        IBEP20(tokenAddress).transferFrom(msg.sender,address(VICDAOMainContract),amount * 10**18);
        VICDAOMainInterface(payable(VICDAOMainContract)).mint_dNELUM_On_Stake(msg.sender, amount * 10**18);

        VICDAOMainInterface(payable(VICDAOMainContract)).updateHolder(msg.sender);
    ...
    function unStake(uint256 stakeId) external whenNotPaused{
        ...
        VICDAOMainInterface(payable(VICDAOMainContract)).burn_dNELUM_On_UnStake(msg.sender, unStakeObj.amount * 10**18);
        unStakeObj.unStakeAt = block.timestamp;
        unStakeNum++;
        VICDAOMainInterface(payable(VICDAOMainContract)).updateHolder(msg.sender);
        VICDAOMainInterface(payable(VICDAOMainContract)).onUserUnstake(msg.sender);
    ...
    function withdraw(uint256 stakeId) external whenNotPaused{
        ...
        IBEP20(withdrawObj.tokenContract).transferFrom(address(VICDAOMainContract), msg.sender, withdrawObj.amount * 10**18);
    ...
    function claim(uint stakeId) external whenNotPaused{
        ...
        if (totalReward > 0)
            IBEP20(NELUMContract).transferFrom(address(VICDAOMainContract), claimObj.user, totalReward);
    }
    ...
    {
        address NELUMContract = VICDAOMainInterface(payable(VICDAOMainContract)).getNELUMContract();
        return (mapStakeByDate[NELUMContract][lastTimeUpdateStake[NELUMContract]]);
    }

    function forTuneDraw() external whenNotPaused{
        require(msg.sender == admin || msg.sender == owner(), "you can not use this function");
        uint256 holderNum = VICDAOMainInterface(payable(VICDAOMainContract)).getNumHolder();
        address[] memory lsHolder = VICDAOMainInterface(payable(VICDAOMainContract)).getAllNodeAddress(address(0), uint32(holderNum));
        address dNELUMContract = VICDAOMainInterface(payable(VICDAOMainContract)).getdNELUMContract();
        address NELUMContract = VICDAOMainInterface(payable(VICDAOMainContract)).getNELUMContract();

        uint256 totalTicket = 0;
        for (uint256 i = 0; i < lsHolder.length; i++){
            uint256 dNULEM = IBEP20(dNELUMContract).balanceOf(lsHolder[i]);
            totalTicket += dNULEM / (tokenOfTicket * 10 **18);
        }
        address[] memory lsTicketAddress = new address[](totalTicket);

        uint256 x = 0;
        for (uint256 i = 0; i < lsHolder.length; i++){
            uint256 dNULEM = IBEP20(dNELUMContract).balanceOf(lsHolder[i]);
            uint numticket =  dNULEM / (tokenOfTicket * 10 **18);
            for (uint256 j = 0; j < numticket; j++){
                if (x < totalTicket){
                    lsTicketAddress[x] = lsHolder[i];
                    x++;
                }
            }
        }
        if (totalTicket > 0){
            ...
            if (add != address(0))
            {
                IBEP20(NELUMContract).transferFrom(VICDAOMainContract, add,forTuneDrawReward * 10 **18);
                emit fortuneDrawDone(add, randomTicket, forTuneDrawReward * 10 **18);
            }             
        }
    }
