...
contract BFARM is Context, IERC20, IERC20Metadata, Ownable {
    ...
    uint256 public maxReserveSwapPercentage = 1000; // 10%
    
    address public marketing_wallet;
    address public rewards_wallet;
    ...
    event Swap(uint swaped, uint sentToMarketing, uint sentToRewards);
    ...
    constructor() {
        // Editable
        string memory e_name = "Bus Farm";
        string memory e_symbol = "BFARM";
        marketing_wallet = 0x39590F5f67ce5d0543672A1D290aedC337717566;
        uint e_totalSupply = 1_000_000_000_000 ether;
        minTokensBeforeSwap = e_totalSupply; // Off by default
        maxSwapTokens = e_totalSupply;       // Off by default
        // End editable
    ...
    function _transfer(
        ...
        if (isFeeActive && !isTaxless[from] && !isTaxless[to] && !inSwap) {
            ...
            emit Transfer(from, address(this), feesCollected);
        }
    ...
    function calculatePercentage(
        ...
        return (amount * percentage) / (10**(feeDecimal + 2));
    }

    function swap() private lockTheSwap {
        // How much are we swaping?
        uint totalCollected = balanceOf(address(this));
        uint maxReserveSwap = calculatePercentage(balanceOf(pair), maxReserveSwapPercentage, 2);

        if(totalCollected > maxSwapTokens) totalCollected = maxSwapTokens;
        if(totalCollected > maxReserveSwap) totalCollected = maxReserveSwap;

        uint marketingFee = _marketingFee[0];
        uint rewardsFee = _rewardsFee[0];
        uint _marketingFeeCollected = (totalCollected * marketingFee) / (marketingFee + rewardsFee);
        uint _rewardsFeeCollected = totalCollected - _marketingFeeCollected;

        if(minTokensBeforeSwap > totalCollected) return;

        // Let's swap for busd now
        address[] memory sellPath = new address[](2);
        sellPath[0] = address(this);
        sellPath[1] = busdAddress;       

        _approve(address(this), address(router), totalCollected);
        if(_rewardsFeeCollected>0)
        {
            try router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
                _rewardsFeeCollected,
                0,
                sellPath,
                address(rewards_wallet),
                block.timestamp
            )
            {}
        ...
        emit Swap(totalCollected, _marketingFeeCollected, _rewardsFeeCollected);
    }
    ...
    function calculateFee(uint256 feeIndex, uint256 amount) internal view returns(uint256) {
        ...
        uint256 rewardsFee = (amount * _rewardsFee[feeIndex]) / (10**(_feeDecimal + 2));
        return marketingFee + rewardsFee;
    }
    ...
    function setMaxSwapTokens(uint256 amount) external onlyOwner {
        maxSwapTokens = amount;
    }

    function setMaxReserveSwapPercentage(uint256 percentage) external onlyOwner {
        require(percentage <= 2400, "Max reserve swap percentage is too high!");
        maxReserveSwapPercentage = percentage;
    }
    ...
    function setRewardsWallet(address wallet)  external onlyOwner {
        rewards_wallet = wallet;
    }

    function setMarketingFees(uint256 buy, uint256 sell, uint256 p2p) external onlyOwner {
        require(buy < 1000 && sell < 1000 &&  p2p < 1000,"Fees are too high! 10% max");
        _marketingFee[0] = buy;
        _marketingFee[1] = sell;
        _marketingFee[2] = p2p;
    }

    function setRewardsFees(uint256 buy, uint256 sell, uint256 p2p) external onlyOwner {
        require(buy < 1000 && sell < 1000 &&  p2p < 1000,"Fees are too high! 10% max");
        _rewardsFee[0] = buy;
        _rewardsFee[1] = sell;
        _rewardsFee[2] = p2p;
    ...
    function setBlockCooldown(uint value) external onlyOwner {
        require(value < 20,"Block cooldown is too high!");
