...
//
// $BCUSD offers long term security and passsive income.
//
// AUTOMATIC DIVIDEND YIELD PAID IN BUSD OR OTHER CRYPTO! With the auto-claim feature,
// simply hold $BCUSD and you receive yields automatically in your wallet.
// 
// Hold BCUSD and get rewarded on every transaction!
//
...
contract DividendPayingToken is BEP20, IDividendPayingToken, IDividendPayingTokenOptional, Ownable {
  using SafeMath for uint256;
  ...
  constructor(string memory _name, string memory _symbol, address _token) BEP20(_name, _symbol) {
...
contract BUSDDividendTracker is DividendPayingToken  {
    ...
    constructor(address _dividentToken) DividendPayingToken("BCUSD_Busd_Dividend_Tracker", "BCUSD_Busd_Dividend_Tracker",_dividentToken) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 1000 * (10**9); //must hold 1,000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "BCUSD_Busd_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "BCUSD_Busd_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BCUSD contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BCUSD_Busd_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BCUSD_Busd_Dividend_Tracker: Cannot update claimWait to same value");
...
contract BCUSDToken is BEP20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public maxBuyTranscationAmount = 100000 * (10**9);
    uint256 public maxSellTransactionAmount = 100000 * (10**9);
    uint256 public maxWalletBalance = 1000000000000000 * (10**9);
    ...
    constructor() BEP20("BlockChain DeFi USD", "BCUSD") {
        	
    	marketingWallet = 0xbC0d39120F941c0AF99e24e91D03B5AFC40254CD; 
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "BCUSD: The marketing wallet is already this address");
    ...
    function updatebusdDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(busdDividendTracker), "BCUSD: The dividend tracker already has that address");
 
        BUSDDividendTracker newbusdDividendTracker = BUSDDividendTracker(payable(newAddress));
 
        require(newbusdDividendTracker.owner() == address(this), "BCUSD: The new dividend tracker must be owned by the BCUSD token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "BCUSD: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "BCUSD: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BCUSD: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "BCUSD: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "BCUSD: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "BCUSD: Trading has not started yet");
    ...
    function swapAndSendBusdDividends(uint256 tokens) private {
        ...
        uint256 busdDividends = IBEP20(busdDividendToken).balanceOf(address(this));
