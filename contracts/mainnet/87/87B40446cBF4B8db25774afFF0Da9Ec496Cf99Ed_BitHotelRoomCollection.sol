...
contract BitHotelRoomCollection is
    IBitHotelRoomCollection,
    AccessControl,
    ERC721Enumerable,
    ERC721URIStorage,
    ERC2981PerTokenRoyalties
{
    ...
    function mintTo(address to, uint256 size) public onlyController(_msgSender()) {
        // solhint-disable-next-line reason-string
        require(_tokenID <= replicas(), "BitHotelRoomCollection: all tokens already minted.");
        require(_tokenID + size <= replicas(), "max supply reached");

        for (uint256 i = 1; i <= size; i++) {
            _mint(to, _tokenID);
            _setTokenURI(_tokenID, "https://ipfs.infura.io/ipfs/QmV9m7vXqeKv38dcrHJtFz9PJTr2SBrDdec7fQPRRDCTjb");

            emit TokenMint(_tokenID, to);
    ...
    function _beforeTokenTransfer(
        ...
        address to,
        uint256 tokenId
    ) internal virtual override(ERC721, ERC721Enumerable) notLocked(tokenId) onlyRole(DEFAULT_ADMIN_ROLE) {
