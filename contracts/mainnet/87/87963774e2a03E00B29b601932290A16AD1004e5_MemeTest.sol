...
contract MemeTest is Context, IERC20, Ownable {
    ...
    address public marketingWallet = 0xc3aFcd17Ce9B78E57721273dA5d2F47F10874d1b;
    ...
    constructor()
    { 
        address newOwner = 0x45BBf522344B8Dc643D5d5FA986b8adeae154194;
    ...
    function setBotFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _marketingFee == 30) return;
    ...
    function setEnableAntibot(bool _enableAntibot) public onlyOwner {
        require(enableAntibot != _enableAntibot, "Antibot is already set to that value");
        enableAntibot = _enableAntibot;
    }
...
//=======TaxAndTransfer=======//
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        else if (enableAntibot && launchTime > 0 && !_isExcludedFromFees[sender] && !_isExcludedFromFees[recipient]) {
            if (launchTime + 30 seconds > block.timestamp){
                if (sender == uniswapV2Pair){
                    lastTransactionTime[recipient] = block.timestamp;
                }
                else{
                    if (block.timestamp - lastTransactionTime[sender] < 5 seconds){
                        setBotFee();
                    }
                }
            }
...
//=======FeeManagement=======//
    ...
    function setSellTaxFeePercent(uint256 _marketingFeeOnSell, uint256 _liquidityFeeOnSell,uint256 _taxFeeOnSell) external onlyOwner {
        ...
        require(totalFeesOnSell <= 10, "Total fees on sell must be less than 10%");
    ...
    function setBuyTaxFeePercent(uint256 _marketingFeeOnBuy, uint256 _liquidityFeeOnBuy,uint256 _taxFeeOnBuy) external onlyOwner {
        ...
        require(totalFeesOnBuy <= 5, "Total fees on buy must be less than 5%");
