...
contract APEN is ERC20, Ownable {
    ...
    constructor(address dataAddr, address tokenAddr) ERC20("APEN", "APEN") {
        data = IData(dataAddr);
        ...
        _mint(lockWallet, 450 * 10**4 * 10**decimals());
    ...
    function unlock(uint256 _outputAmount) public onlyOwner {
    ...
    function _transfer(
        ...
        if (pairInclude(from) || pairInclude(to)) {
            if (pairInclude(from) && data.address2uintMapping(to) == 3) {
                super._transfer(from, to, amount);
                return;
            } else if (pairInclude(to)) {
                amount = amount.sub(takeFee(from, amount));
                super._transfer(from, to, amount);
            }
        } else {
            require(
    ...
    function takeFee(address from, uint256 amount) internal returns (uint256) {
        if (
            data.address2uintMapping(from) == 2 ||
        ...
        uint256 cFee = amount
            .mul(data.string2uintMapping("cryptoFeeRate"))
            .div(1000000)
            .div(100);
        uint256 dFee = amount
            .mul(data.string2uintMapping("dividendFeeRate"))
            .div(1000000)
            .div(100);
        uint256 rFee = amount
            .mul(data.string2uintMapping("reserveFeeRate"))
            .div(1000000)
            .div(100);
        uint256 mFee = amount
            .mul(data.string2uintMapping("marketingFeeRate"))
            .div(1000000)
            .div(100);

        uint256 totalFee = cFee.add(dFee).add(rFee).add(mFee);
    ...
    function swapToken() internal {
        ...
        uint256 cFee = data.string2uintMapping("cryptoFeeRate");
        uint256 dFee = data.string2uintMapping("dividendFeeRate");
        uint256 rFee = data.string2uintMapping("reserveFeeRate");
        uint256 mFee = data.string2uintMapping("marketingFeeRate");
        uint256 totalFee = cFee.add(dFee).add(rFee).add(mFee);

        usdt.transfer(
            data.string2addressMapping("cryptoFeeWallet"),
            _usdtAmount.mul(cFee).div(totalFee)
        );
        usdt.transfer(
            data.string2addressMapping("dividendFeeWallet"),
            _usdtAmount.mul(dFee).div(totalFee)
        );
        usdt.transfer(
            data.string2addressMapping("reserveFeeWallet"),
            _usdtAmount.mul(rFee).div(totalFee)
        );
        usdt.transfer(
            data.string2addressMapping("marketingFeeWallet"),
            _usdtAmount.mul(mFee).div(totalFee)
        );
    }
