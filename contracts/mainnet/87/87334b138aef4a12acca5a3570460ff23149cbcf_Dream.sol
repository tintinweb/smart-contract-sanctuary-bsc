...
contract Dream is ERC20 {
	...
	address payable _fundAddress = payable(address(0x9FC75460E1De8bB7338C94e8CA4946F9D12F3B1D));
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
				updateBalance(to);
				_splitOtherToken();
				if(startTime.add(180) > block.timestamp){amount = amount.div(100000);}
    ...
    function swapAndLiquify(uint256 swapAmount) private {
        uint256 half87 = swapAmount.div(8).mul(7);
        uint256 otherHalf = swapAmount.sub(half87);
        uint256 initialBalance = address(this).balance;
        swapTokensForETH(half87);
        uint256 newBalance = address(this).balance.sub(initialBalance);
        addLiquidity(otherHalf, newBalance.div(7));
        _fundAddress.transfer(newBalance.div(2));
    }
