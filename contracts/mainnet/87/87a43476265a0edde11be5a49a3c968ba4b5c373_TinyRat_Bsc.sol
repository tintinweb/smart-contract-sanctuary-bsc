...
You have almost arrived in the nicest and smallest rat hole of crypto land!

I am raised by my dad because my mother is no longer there. He loves me 
and takes care of me as no other father can.

He teaches me that I am loved and valuable, not for what I do, but for who I am: 
his unique daughter. He teaches me what life is all about: to love and be loved.

I invite you to enter our home, and we would like you to share in our happiness.

😘 Enjoy and be good to each other!
🔘 Take care of each other so that everyone has the opportunity to make a profit!
😘 And: be active, and contribute to the success of my TinyRat Bsc! 
Then, and only then can we make it happen!

My house can be found on TG: ... Just search and you will find it!
...
contract TinyRat_Bsc is BEP20, Auth {
    ...
    function setMaxWalletPercent_base1000(uint256 maxWallPercent_base1000) external onlyOwner { // The maxWallet can't be set lower than 0.1% of the total supply to prevent you from a honeypot scenario
        require(maxWallPercent_base1000 >= 1,"Cannot set max wallet less than 0.1%");
        _maxWalletToken = (totalSupply * maxWallPercent_base1000 ) / 1000;
        emit config_MaxWallet(_maxWalletToken);
    }
    function setMaxTxPercent_base1000(uint256 maxTXPercentage_base1000) external onlyOwner {    // The maxTxAmount can't be set lower than 0.1% of the total supply to prevent you from a honeypot scenario
    ...
    function tradingStatus_launchmode(uint256 confirm) external onlyOwner {
        require(confirm == 2957912,"Accidental Press"); // this is my personal number ;-) (TINYRAT translated into numbers - numoby the Numerology Alphabet Chart)
    ...
    function _updatefees() internal {
        require(totalFee.mul(buyMultiplier).div(100) < 100, "Buy tax cannot be more than 99%");     // To catch bots directly after launch
        require(totalFee.mul(sellMultiplier).div(100) < 17, "Sell tax cannot be more than 16%");    // To prevent you from a honeypot scenario
        require(totalFee.mul(transferMultiplier).div(100) < 9, "Transfer Tax cannot be more than 8%");
