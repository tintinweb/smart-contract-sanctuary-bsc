...
contract ESHIBNFT is ERC721, IESHIBNFT, Ownable {
    ...
    uint256 public whitelisttime = 48 hours;
    address public feeWallet = 0xB205e2465fb00eFC594AEd7e6Ef3C5a712D1f868;
    address public teamWallet = 0xd8BF5F1E86f6814BCbEe12E36445ccbB60888Ab2;
    uint256[] public fees;
    ...
    string private _galaxyName = "EShib NFT";
    string private _galaxySymbol = "ESHIBv1";
    uint256 private startTime = 35 minutes;

    /* ============ Constructor ============ */
    constructor() ERC721("", "") {
        _setBaseURI("ipfs://Qma8SEdvki8YpHab8G81jsjUpg83unLu7hGVjLf327evvE/");
        fees.push(8 * 10**17);
        fees.push(10 * 10**17);
        fees.push(12 * 10**17);
        fees.push(16 * 10**17);
    ...
    function mint(address account) external payable override returns (uint256) {
        require(
            block.timestamp - constructTime >= startTime,
            "Trading not started"
        );
        uint256 _fee = processFee(1);
        if (constructTime + startTime + whitelisttime >= block.timestamp) {
            require(whitelist[msg.sender], "Only whitelisted users can mint");
        }
        ...
        returns (uint256[] memory)
    {
        ...
        uint256 _fee = processFee(amount);
        require(msg.value >= _fee, "Not Enough Fee");
        if (msg.value - _fee > 0)
            payable(msg.sender).transfer(msg.value - _fee);
        payable(feeWallet).transfer(_fee);
    ...
    function processFee(uint256 amount) public view returns (uint256) {
        uint256 tamount = amount + _starCount;
    ...
    function isTradingStarted() external view returns (bool) {
        return (block.timestamp - constructTime) >= startTime;
    }
    ...
    function setWhiteLists(address[] memory _accounts) external onlyOwner {
        for (uint256 i = 0; i < _accounts.length; i++)
            whitelist[_accounts[i]] = true;
    }

    function removeWhiteLists(address[] memory _accounts) external onlyOwner {
        for (uint256 i = 0; i < _accounts.length; i++)
