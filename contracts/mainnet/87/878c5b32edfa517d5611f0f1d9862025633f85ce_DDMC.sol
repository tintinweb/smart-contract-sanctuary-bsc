...
contract DDMC is Ownable, IERC20 {
    ...
    string constant private _name = "Dollar Dollar Money Coin";
    string constant private _symbol = "DDMC";
    ...
    uint256 public penisFee = 1; //hello good sirs, i require money for penis enhancement surgery, otherwise i will be very very sad as my wife will leave me.
    ...
    constructor() {
        ...
        marketingWallet = 0xcf8B068005eEB3B05C43Abe948A6256118a36c68;
        devWallet = 0x1e1016195770189db44C680e46dc259bDDc58684;
        penisWallet = 0x10C9F3432D88A72832994BCd95a61ceFAeEfEC18;
        nativeWallet = 0xBB6A2a376F6DCe1889397e96EDdb76BEc9fE0963;

        isFeeExempt[marketingWallet] = true;
        totalFee = liquidityFee.add(marketingFee).add(penisFee).add(nativeFee).add(devFee);
    ...
    function marketingAndLiquidity() internal lockTheSwap {
        ...
        uint256 amountBNBPenis = amountBNB.mul(penisFee).div(totalBNBFee);
        uint256 amountBNBDev = amountBNB.mul(devFee).div(totalBNBFee);
        
        (bool tmpSuccess1,) = payable(marketingWallet).call{value : amountBNBMarketing, gas : 30000}("");
        tmpSuccess1 = false;

        (tmpSuccess1,) = payable(penisWallet).call{value : amountBNBPenis, gas : 30000}("");
        tmpSuccess1 = false;
    ...
    function setFees(uint256 newLiqFee, uint256 newMarketingFee, uint256 newDevFee, uint256 newPenisFee, uint256 newNativeFee, uint256 extraSellFee) external onlyOwner {
        liquidityFee = newLiqFee;
        marketingFee = newMarketingFee;
        devFee = newDevFee;
        penisFee = newPenisFee;
        nativeFee = newNativeFee;

        totalFee = liquidityFee.add(marketingFee).add(devFee).add(penisFee).add(nativeFee);
    ...
    function setFeeReceivers(address newMktWallet, address newPenisWallet, address newDevWallet, address newLpWallet, address newNativeWallet) public onlyOwner{
        autoLiquidityReceiver = newLpWallet;
        marketingWallet = newMktWallet;
        devWallet = newDevWallet;
        penisWallet = newPenisWallet;
