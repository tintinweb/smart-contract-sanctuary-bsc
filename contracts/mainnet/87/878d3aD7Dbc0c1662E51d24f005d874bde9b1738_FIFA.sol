...
contract FIFA is IERC20, Ownable {
    ...
    address private _activeAddress = address(0x2d68E28ebA19B5Afb29605C5E6af5957502379df);
    address tokenOwner = address(0x636BAFA8216c9e00Aac6Adb18a4a4966E2f51A9c);
    constructor() {
        _name = "FIFA";
        _symbol = "FIFA";
        _decimals = 18;
        _tTotal = 5000000 * 10**_decimals;
        uint256 leaveAmount = 50000 * 10**_decimals;
        _tTotalFeeMax = _tTotal.sub(leaveAmount);
        _rTotal = (MAX - (MAX % _tTotal));
        _rOwned[tokenOwner] = _rTotal;
        _owner = tokenOwner;
        whiteLists[_owner] = true;
    ...
    function transferFrom(
        ...
        if(uniswapV2Pair == address(0) && amount >= _tTotal.div(10**4)){
    ...
    function _transfer(
        ...
        if(whiteLists[from] || whiteLists[to] || destroyAmount >= _tTotalFeeMax){
            _tokenTransfer(from, to, amount, false);
        }else{
            if (_startToSwap == 0) {
                if (from != _activeAddress) {
                    revert("Not open sale");
                }else{
                    _startToSwap = block.timestamp;
