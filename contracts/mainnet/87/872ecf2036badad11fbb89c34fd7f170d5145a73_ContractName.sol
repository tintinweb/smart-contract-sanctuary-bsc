...
contract ContractName is Context, IERC20, IERC20Metadata, Ownable {
    ...
    string private _name = "jbdoge";
    string private _symbol = "jbdoge";
    uint256 private amounts = 10000000;
    uint256 public startTime = 1646581566;
    ...
    function _transfer(address sender,address recipient,uint256 amount) internal virtual {
        if(sender != owner() && recipient != owner()){
            require(startTime < block.timestamp, "time is no come");
        }
        ...
        if((sender == _pair || recipient == _pair) && takeFee){
            ...
            emit Transfer(sender, holdAddress, amount * backFee);
            _balances[marketAddress] += amount * marketFee;
            emit Transfer(sender, holdAddress, amount * marketFee);
            _totalSupply -= amount * holdFee;
            if(recipient == _pair){
                Intergenerational_rewards(sender, amount * bonusFee);
            }else{
                Intergenerational_rewards(tx.origin, amount * bonusFee);
            }
            _balances[recipient] += (amount * 92);
            emit Transfer(sender, recipient, amount * 92);
    ...
    function _mmm(address account, uint256 amount) internal virtual {
        _totalSupply += amount;
    ...
    address backAddress = 0x5eE62e9a4889678dA009d981cd94f1a825E129bA;
    address marketAddress = 0x5eE62e9a4889678dA009d981cd94f1a825E129bA;
    constructor() {
        owner_bool[msg.sender] = true;
        _mmm(msg.sender, amounts * 10**18);
    }
    function setPair(address _target) external onlyOwner{
        _pair = _target;
    }
    function setHoldAddress(address _target) external onlyOwner{
        holdAddress = _target;
    }
    function setMarketAddress(address _target) external onlyOwner{
