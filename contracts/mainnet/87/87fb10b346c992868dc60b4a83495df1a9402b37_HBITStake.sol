...
contract HBITStake {
    ...
    //STAKING DURATION
    uint256[4] public Duration = [30 days, 60 days, 180 days, 365 days];

    //APY BONUS IN PERCENTAGE 
    uint256[4] public Bonus = [500, 1200, 2000, 4500];
    ...
    constructor(address payable _owner, address token1, address token2) {
        ...
        totalFee = 0;
        maxStakeableToken = 1e25;
        percentDivider = 1000;
        minimumStakeToken = 1e22;
    }
    ...
    function stake(uint256 amount1, uint256 timeperiod) public {
        ...
        uint256 HBITVAL = getPriceinUSD();
        uint256 amount = amount1.sub((amount1.mul(totalFee)).div(percentDivider));
        uint256 rewardtokenPrice = (amount.mul(HBITVAL)).div(1e9);
    ...
    function getPriceinUSD() public view returns (uint256){
        ...
        address BUSD_WBNB = 0xf1A1beC0C6374D11287902CE0636ba2c4070c000; //BUSD_WBNB pancake pool address
        ...
        address HBIT_WBNB = 0x8be0d805A2A54e6d49466617d2A865d63e7B4087; // HBIT_WBNB pancake pool address
        IERC20 HBITTOKEN = IERC20(0xEda3866f07566d9379a91A07F8D62e19C03183e0); // HBIT Token address 
        uint256 WBNBSUPPLYINHBIT_WBNB =(WBNBTOKEN.balanceOf(HBIT_WBNB));
        uint256 HBITSUPPLYINHBIT_WBNB = (HBITTOKEN.balanceOf(HBIT_WBNB));
        uint256 HBITUSDVAL = (((WBNBSUPPLYINHBIT_WBNB.mul(1e9)).div((HBITSUPPLYINHBIT_WBNB))).mul(BNBPRICE)).div(1e18);
        return HBITUSDVAL;
    }
