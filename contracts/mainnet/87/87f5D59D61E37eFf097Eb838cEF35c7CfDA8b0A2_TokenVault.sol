...
interface IStakingPool {
    ...
    function apy(uint256 amount, uint256 duration)
        external
        pure
        returns (uint256 apy);

    function poolFactor(uint256 balance)
        external
        view
        returns (uint256 poolFactor);

    function getEntries(address account)
        external
        view
        returns (PoolEntry[] memory entries);
...
abstract contract Test is VestingWallet {
