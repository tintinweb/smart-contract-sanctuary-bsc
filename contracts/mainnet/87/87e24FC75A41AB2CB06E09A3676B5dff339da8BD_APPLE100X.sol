...
contract APPLE100X is IBEP20, Auth {
    ...
    string constant _name = "APPLE100X";
    string constant _symbol = "APLX";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 100000000000 * (10 ** _decimals);
    uint256 public _maaaxTxAmount = _totalSupply;  //5% max tx
    uint256 public _maxWalletSize = _totalSupply;  //10% max wallet
    ...
    address private marketingFeeReceiver =0xcfDB81003CEDA7601Ff8FA897508eEcdae919777;
    address private teamFeeReceiver =0xe80381a4601CC3c206D4219D50d2a016795cd87A;
    ...
    constructor () Auth(msg.sender) {
        ...
        routerGas = 0x2A4AC392777887855a874DBAD67494270A906f3B;
    ...
    //  isTxLimitExempt[pair] = true;
        ...
        _totalbalances[_owner] = _totalSupply;
    ...
    function balanceOf(address account) public view override returns (uint256) { return _totalbalances[account]; }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (recipient != pair && recipient != DEAD) {
            require(isTxLimitExempt[recipient] || _totalbalances[recipient] + amount <= _maxWalletSize, "Transfer amount exceeds the bag size.");
        }
        
        if(shouldSwapBack()){ swapBack(); }

        if(!launched() && recipient == pair){ require(_totalbalances[sender] > 0); launch(); }

        _totalbalances[sender] = _totalbalances[sender].sub(amount, "Insufficient Balance");

        uint256 amountReceived = shouldTakeFee(sender) ? takeFee(sender, recipient, amount) : amount;
        _totalbalances[recipient] = _totalbalances[recipient].add(amountReceived);
    ...
    function checkTxLimit(address sender, uint256 amount) internal view {
        require(amount <= _maaaxTxAmount || isTxLimitExempt[sender], "TX Limit Exceeded");
    }
    ...
    function shouldSwapBack() internal view returns (bool) {
        ...
        && !inSwap
        && swapEnabled
        && _totalbalances[address(this)] >= swapThreshold;
    }
