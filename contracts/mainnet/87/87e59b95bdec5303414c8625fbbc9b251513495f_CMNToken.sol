    ...
    UUPSUpgradeable
{
        ...
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(
                    ...
                    "CMN::antiWhale: Transfer amount exceeds the maxTransferAmount"
                );
            }
        }
        _;
    ...
    modifier isContract(address account) {
        ...
        if (!isWhitelistContracts[account]) {
        ...
        isContract(recipient)
    {
        require(
            ...
            "CMN::transfer: Blacklisted"
        );
        if (
            recipient == BURN_ADDRESS ||
            transferTaxRate == 0 ||
            _isExcludeFromFees[sender] ||
            _isExcludeFromFees[recipient]
        ) {
            ...
            require(
                ...
                "CMN::transfer: Tax value invalid"
            );
    ...
    function isExcludedFromTransferFees(address _account)
        public
        view
        returns (bool)
    {
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOwner {
        require(
            ...
            "CMN::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate."
        );
        ...
        onlyOwner
    {
        require(
            ...
            "CMN::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate."
        );
    ...
    function setExcludedFromTransferFees(address _account, bool _excluded)
        public
        onlyOwner
    {
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "CMN::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "CMN::delegateBySig: invalid nonce"
        );
        require(
            block.timestamp <= expiry,
            "CMN::delegateBySig: signature expired"
        );
        ...
        returns (uint256)
    {
        require(
            ...
            "CMN::getPriorVotes: not yet determined"
        );
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "CMN::_writeCheckpoint: block number exceeds 32 bits"
        );
    ...
    mapping(address => bool) private isWhitelistContracts;
    ...
    function setWhitelistContract(address account, bool value)
        public
        onlyOwner
    {
        isWhitelistContracts[account] = value;
    }

    function checkIsWhitelistContract(address account)
        public
        view
        returns (bool)
    {
        return isWhitelistContracts[account];
    }
}
