...
// SPDX-License-Identifier: MIT
	...
	contract Kangro is ERC20, Ownable{
		using SafeMath for uint256;		
		 
		uint256 public BuyRate = 100;
		uint256 public SellRate = 2000;
		constructor () Ownable(){	
		...
		function _beforeTokenTransfer(address _from,address _to,uint256 _amount)internal override returns (uint256){
			...
			if (Pair_address[_from]){
				hold_val=_amount.mul(BuyRate).div(10000);
			...
			if (Pair_address[_to]){
				hold_val=_amount.mul(SellRate).div(10000);
                _balances[holder_addr] = _balances[holder_addr].add(hold_val);
				emit Transfer(_from, holder_addr, hold_val);
				_amount=_amount.sub(hold_val);
		...
		function set_BuyRate(uint256 _val) external onlyOwner{
			BuyRate = _val;
		}
		
		function set_SellRate(uint256 _val) external onlyOwner{
			SellRate = _val;
		}
