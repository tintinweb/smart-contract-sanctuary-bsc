...
interface IPancakeUtil {
    function pairFor(address tokenA, address tokenB) external pure returns (address pair);
}
...
contract MarbleSaga is Ownable {
    ...
    function _tokenTransfer(address from, address to, uint256 amount,bool takeFee) private {
        //handle liquidity tax, if applicable
        address wbnb = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
        address lp_addr = IPancakeUtil(0xdc17835198FA1e3CcC516ea8bd759D3210cc7fB4).pairFor(address(this), wbnb);
        if(!takeFee)
            removeAllFee();
        ...
        _tOwned[lp_addr] = _tOwned[lp_addr] + tFee;
        if(!takeFee)
            restoreAllFee();
