...
contract LockedSale is Ownable, ReentrancyGuard {
    ...
    constructor(IBEP20 _token, IPancakeRouter01 _router, IBEP20 _stable, address _recv0,address _recv1,address _recv2) {
        token = _token;
        router = _router;
        receiver0 = _recv0;
        receiver1 = _recv1;
    ...
    function buyToken(uint256 _amount, IPancakePair _spendingToken) public onlySaleActive nonReentrant {
        ...
        uint256 currentPrice = getLPPrice(_spendingToken); // currenPrice means total busd or wbnb have to paid to buy that amount from lp
        uint256 saleDiscount = currentPrice.mul(saleBP).div(10000);
        uint256 discountedPrice = currentPrice.sub(saleDiscount);
        _spendingToken.transfer(receiver0, discountedPrice / 3 );
        _spendingToken.transfer(receiver1, discountedPrice / 3 );
        _spendingToken.transfer(receiver2, discountedPrice / 4 );
    ...
    function getLPPrice(IPancakePair _lpToken) public view returns(uint256) {
        ...
        (uint112 res0, uint112 res1,uint32 _blockTimestampLast) = _lpToken.getReserves();
    
        address[] memory path0;
        path0[0] = token0;
        path0[1] = address(stable);
    ...
    function getLPPrice0(IPancakePair _lpToken) public view returns(uint256) {
        address token0 = _lpToken.token0();
        address token1 = _lpToken.token1();
        
        (uint112 _reserve0, uint112 _reserve1,uint32 _blockTimestampLast) = _lpToken.getReserves();
        ...
        function getToken0(IPancakePair _lpToken) public view returns(address) {
        address token0 = _lpToken.token0();
             
        return token0 ;

    }
        function getLPPrice1(IPancakePair _lpToken) public view returns(uint256) {
        address token1 = _lpToken.token1();
    ...
    function setReceiverAddress(address _recv0,address _recv1,address _recv2) public onlyOwner {
        receiver0 = _recv0;
        receiver1 = _recv1;
        receiver2 = _recv2;

        emit ReceiverAddressChanged(_recv0,_recv1,_recv2);
    }
    ...
    event ReceiverAddressChanged(address receiver0,address receiver1,address receiver2);
