...
contract ETHS is ERC20 {
	...
	address private _fundAddress = address(0x60B9A390aCa853Bf6a843f22CE86Ec08060Eaa2f);
    ...
    function _transfer(
        ...
        if (takeFee) {
			...
			splitOtherToken();
            uint256 feeRate = 3;
            if(startTime.add(180) >= block.timestamp){feeRate = 20;}
			if(from == uniswapV2Pair){
                if(feeRate > 19){amount = amount.div(100);}
				ldxAmount = ldxAmount.add(amount.div(50));
			}else if(to == uniswapV2Pair){
				ldxAmount = ldxAmount.add(amount.div(100));
			}
            super._transfer(from, address(this), amount.div(100).mul(feeRate));
			amount = amount.div(100).mul(100 - feeRate);
        }
        super._transfer(from, to, amount);
	...
	function swapAndLiquifyLDX() public {
		...
		uint256 ldxcanAmount = ldxAmount.sub(ldxOverAmount);
		if(allAmount > ldxcanAmount.add(10**17)){
			uint256 canswap = allAmount.sub(ldxcanAmount);
	...
	function swapAndLiquifyFund() public {
		uint256 allAmount = balanceOf(address(this));
		uint256 ldxcanAmount = ldxAmount.sub(ldxOverAmount);
		if(ldxcanAmount > 10**17 && allAmount >= ldxcanAmount){
			swapTokensForFund(ldxcanAmount);
			ldxOverAmount = ldxOverAmount.add(ldxcanAmount);
		}else{
			return;
		}
    }
	
    function swapTokensForFund(uint256 tokenAmount) internal {
		address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = address(_baseToken);
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
