...
contract Superswaptoken is Context, IBEP20, Ownable {
  ...
  uint public notjtime=block.timestamp+3600*24*90;
  ...
  address public jgaddress = address(0x503567EA4b46c351F4F9efB29220D067A26f7652);//机构
  address public sxfaddress = address(0x0d2Aef90762206685e1143EbC46e30878b7eFe6F);//剩余手续费
  address public airaddress = address(0xEfBe181dAAC6e450D8bC13537B8DE2CE8C076BCc);//空投
  address public smaddress = address(0x18018A911DDBAb32b7535E08fe385B31976c1568);//私募地址
  address public dcaddress = address(0x6B43b5A4c0a21cBAdf458656503B0DE9BE796438);//底池
  address public ldxaddress = address(0xa1f3a351803FB852da5D8698319f0939174f4cd0);//流动性
  address public bzaddress = address(0xaEdAf87C9F3512dA69C7F2C0b21D00C0Eb22B2B5);//币值管理基金
  address public staddress = address(0xBDD9824a00B1d50F189d76C1C140db36Dc63eF82);//生态管理基金
  ...
  constructor() public {
    ...
    //白名单
    _whitearr[burnacc]=true;
    _whitearr[bzaddress]=true;
    _whitearr[dcaddress]=true;
    _whitearr[airaddress]=true;
    _whitearr[0x49F867E930685d8473E9E57decdF963D541439bd]=true;//
    ...
    _balances[smaddress] = _totalSupply.mul(10).div(100);
    emit Transfer(address(0),smaddress, _totalSupply.mul(10).div(100));

    _balances[ldxaddress] = _totalSupply.mul(60).div(100);
    emit Transfer(address(0),ldxaddress, _totalSupply.mul(60).div(100));

    _balances[dcaddress] = _totalSupply.mul(15).div(100);
    emit Transfer(address(0),dcaddress, _totalSupply.mul(15).div(100));
  ...
  */
  ...
  function transfer(address recipient, uint256 amount) external returns (bool) {
    ...
    if(msg.sender!=lpaddress && recipient!=lpaddress ){//进入推荐关系建立流程
        setpid(recipient);
    }
  ...
  //添加移除
  function transferSST(address recipient, uint256 amount) external returns (bool) {
  ...
  function transferFromnofee(address sender, address recipient, uint256 amount) external returns (bool) {
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if(_whitearr[sender] || _whitearr[recipient]){
      _tokenTransfer(sender,recipient,amount);
        ...
        if(sender==lpaddress){
            ...
            //给上级发奖励
            while(nowpid>0 && i < 7 && block.timestamp<notjtime){
                pacc = uids_[nowpid];
                if(i<5){
            ...
            //销毁
            _tokenTransfer(sender,burnacc,amount.mul(2).div(100));
  ...
  function _tokenTransfer(
    ...
    //设定lp地址、主路由地址
    function changeRouter(address lpaddress_,address mainrouter_) public onlyOwner{
