...
// File: contracts/token/BEP20/TaxableBEP20.sol
...
/**
 * @title TaxableBEP20
 * @dev Implementation of the TaxableBEP20
 */
contract TaxableBEP20 is BEP20, ERC20Decimals, ERC20Taxable, ServicePayer {
    constructor(
        string memory name_,
        ...
        ServicePayer(feeReceiver_, "TaxableBEP20", signature_, _msgSender())
    {
        require(initialBalance_ > 0, "TaxableBEP20: supply cannot be zero");
     ...
     * @dev Set the amount of tax fee per mille
     *
     * NOTE: restricting access to owner only. See {BEP20Taxable-_setTaxRate}.
     *
     ...
     *
     * NOTE: restricting access to owner only. See {BEP20Taxable-_setTaxAddress}.
     *
     ...
     * @dev Set the exclusion status from tax fee mechanism (both sending and receiving)
     *
     * NOTE: restricting access to owner only. See {BEP20Taxable-_setExclusionFromTaxFee}.
     */
    function setExclusionFromTaxFee(address account, bool status) public onlyOwner {
        _setExclusionFromTaxFee(account, status);
    }

    function transfer(address to, uint256 amount) public virtual override(IERC20, ERC20, ERC20Taxable) returns (bool) {
    ...
    function transferFrom(
        ...
        address to,
        uint256 amount
    ) public virtual override(IERC20, ERC20, ERC20Taxable) returns (bool) {
