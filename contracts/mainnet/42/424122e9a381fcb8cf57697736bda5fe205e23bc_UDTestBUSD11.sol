...
contract DividendDistributor is IDividendDistributor {
    ...
    function deposit() public payable override onlyToken {
        if(_rewardToken == _BUSD){
            ...
            uint256 amount = IBEP20(rewardToken).balanceOf(address(this)).sub(balanceBefore);
...
contract UDTestBUSD11 is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "UDTest BUSD 11";
    string constant _symbol = "UDTBUSD11";
    uint8 constant _decimals = 18;

    uint256 _totalSupply = 1000000 * (10 ** _decimals);
    uint256 public _maxTxAmount = 1 * 1e18; // $50 BUSD
