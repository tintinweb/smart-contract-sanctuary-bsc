...
contract BoostingService is PausableByOwner {
    ...
    uint16  public constant  MAX_PENALTY_RATE = 2500;
    uint16  public constant  MAX_PERFORMANCE_FEE = 500;
    ...
    constructor(
        ...
        require(_penaltyRate <= MAX_PENALTY_RATE);
    ...
    function stakeFor(address owner, uint128 amount) external {
        _stake(amount, owner, _msgSender());
    }

    function _stake(uint128 amount, address owner, address sourceOfFunds) internal whenNotPaused {
        bool transferred =
        ...
        _compoundAndRecalculateShares(staker, amount, owner);
    } 
    ...
    function _compoundAndRecalculateShares(Staker storage staker, uint128 amount, address owner) internal {
        uint128 compoundRewards = _receiveCompound();
        _receiveBoosting(compoundRewards);
        
        totalStakedCompounded += compoundRewards;
        
        _materializeShares(staker, owner);

        _stakeShares(staker, amount, owner);
    ...
    function _materializeShares(Staker storage staker, address owner) internal returns(uint128 compoundAmount, uint128 boostingAmount) {
        uint128 _stakerShares = staker.shares;
    ...
    function _stakeShares(Staker storage staker, uint128 amount, address owner) internal {
