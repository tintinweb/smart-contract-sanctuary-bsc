...
/*
 __ __ __ ____   ___  __       __    __   ___   ____  __    ____        ___   ___   __ __  __
 || || || || \\ // \\ ||       ||    ||  // \\  || \\ ||    || \\      //    // \\  || ||\ ||
 \\ // || ||_// ||=|| ||       \\ /\ // ((   )) ||_// ||    ||  ))    ((    ((   )) || ||\\||
  \V/  || || \\ || || ||__|     \V/\V/   \\_//  || \\ ||__| ||_//      \\__  \\_//  || || \||
                                                                                             
    Token Name: Viral World Coin
    Symbol: VWC
	Total Supply: 100_000_000_000 (100 Billion)
    Decimals: 9
    
    Tokenomics:
    12% Total fee for Buy & Sell
    4% Reflections in BUSD
    1% Liquidity
...
contract VIRALWORLDCOIN is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public _charityWallet = payable(address(0x4f811FB8BB664be43F9967D5EE359EB2307EA6fC)); //charityWallet
    address public _marketingWallet = payable(address(0x1b17fCC78c887b5D9d7EDe53EE004Ad00Ed44130)); //marketingWallet
    address public _salaryWallet = payable(address(0xCaAe6915D5232cC5D10476C36C0b11A95859736D)); //salaryWallet
    ...
    constructor() ERC20("Viral World Coin", "VWC") {
        buyFee.rewardFee = 4;
        buyFee.charityFee = 2;
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(
            0x10ED43C718714eb63d5aA57B78B54704E256024E // Pancakeswap Router //testnet 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3 //mainnet 0x10ED43C718714eb63d5aA57B78B54704E256024E
        );
   ...
   function setSalaryWallet (address wallet) external onlyOwner {
    _salaryWallet = wallet;
   }
    ...
    function setBuyFee(
        ...
        buyFee.salaryFee = salary;
        totalBuyFee = rewardfee + charity + liquidity + marketing + salary;
    }
    ...
    function setSellFee(
        ...
        sellFee.salaryFee = salary;
        totalSellFee = rewardfee + charity + liquidity + marketing + salary;
    }
   ...
   function setSwapTokensAtAmount(uint256 amount) external onlyOwner {
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 feeTokens = contractTokenBalance
                .mul(buyFee.charityFee + sellFee.charityFee + buyFee.marketingFee + sellFee.marketingFee + buyFee.salaryFee + sellFee.salaryFee)
                .div(totalFee);
    ...
    function swapAndSendToWallets(uint256 tokens) private lockTheSwap {
        ...
        uint256 newBalance = IERC20(BUSD).balanceOf(address(this)).sub(
            initialBalance);
        uint256 marketingShare = newBalance.mul(buyFee.marketingFee + sellFee.marketingFee).div(buyFee.charityFee + sellFee.charityFee + buyFee.marketingFee + sellFee.marketingFee + buyFee.salaryFee + sellFee.salaryFee);
        uint256 charityShare =newBalance.mul(buyFee.charityFee + sellFee.charityFee).div(buyFee.charityFee + sellFee.charityFee + buyFee.marketingFee + sellFee.marketingFee + buyFee.salaryFee + sellFee.salaryFee);
        uint256 salaryShare =newBalance.mul(buyFee.salaryFee + sellFee.salaryFee).div(buyFee.charityFee + sellFee.charityFee + buyFee.marketingFee + sellFee.marketingFee + buyFee.salaryFee + sellFee.salaryFee);
        IERC20(BUSD).transfer(_marketingWallet, marketingShare);
        IERC20(BUSD).transfer(_charityWallet, charityShare);
        IERC20(BUSD).transfer(_salaryWallet, salaryShare);
}
...
contract TokenDividendTracker is Ownable, DividendPayingToken {
    ...
    {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 100000000 * (10**9); //must hold 0.1% tokens
    }
