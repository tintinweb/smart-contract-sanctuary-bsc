...
contract FOOTBALLINU is IERC20, Context, Ownable {
    ...
    address private Finu____Marketing_Wallet = 0x43D8f403d6c6dB0030A82b6B5361F349a3179D32;
    address private Finu____Team_Wallet = 0x43D8f403d6c6dB0030A82b6B5361F349a3179D32;

    uint256 private Finu____max_BuyLimit = 10000000 * 10**9;
    uint256 private Finu____max_SellLimit = 10000000 * 10**9;

    uint256 private Finu____max_WalletLimit = 10000000  * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[Finu____Marketing_Wallet] = true;
        _isExcludedFromFee[Finu____Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[Finu____Marketing_Wallet] = true;
        allowedTransfer[Finu____Team_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= Finu____max_BuyLimit, "You are exceeding Finu____max_BuyLimit");
            require(balanceOf(to) + amount <= Finu____max_WalletLimit, "You are exceeding Finu____max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= Finu____max_SellLimit, "You are exceeding Finu____max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= Finu____max_WalletLimit, "You are exceeding Finu____max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(Finu____Marketing_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(Finu____Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function Finu____Update_MarketingWallet(address newWallet) external onlyOwner{
        Finu____Marketing_Wallet = newWallet;
    }
    
    function Finu____Update_TeamWallet(address newWallet) external onlyOwner{
        Finu____Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        Finu____max_BuyLimit = maxBuy * 10**decimals();
        Finu____max_SellLimit = maxSell * 10**decimals();
    }
    
    function Finu____Max_Walletlimit(uint256 amount) external onlyOwner{
        Finu____max_WalletLimit = amount * 10**decimals();
    }
