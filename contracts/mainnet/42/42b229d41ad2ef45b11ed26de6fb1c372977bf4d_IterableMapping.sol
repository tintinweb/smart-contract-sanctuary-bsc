...
contract WCCDividendTracker is DividendPayingToken, Ownable {
    ...
    function updateHoldingAmount(uint256 newAmount) external onlyOwner {
        require(newAmount != minimumTokenBalanceForDividends, "WCC_Dividend_Tracker: Cannot update minimumTokenBalanceForDividends to same value");
        minimumTokenBalanceForDividends = newAmount * (10**18);
    }
...
contract WCC is ERC20, Ownable {
    ...
    function updateMinimumAmountForReward(uint256 newAmount) external onlyOwner {
        dividendTracker.updateHoldingAmount(newAmount);
    }
    ...
    function getMinimumBalanceForReward() external view returns(uint256) {
