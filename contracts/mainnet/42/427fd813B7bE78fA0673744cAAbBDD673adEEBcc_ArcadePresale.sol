...
contract ArcadePresale is Ownable, ReentrancyGuard {
    ...
    function refundUser(address addr) public onlyOwner {
        require(presaleAmountBought[addr] > 0, "User didnt buy.");
        require(msg.sender == devWallet, "NON DEV.");

        uint256 toSend = presaleAmountBought[addr];

        presaleAmountBought[addr] = 0;

        IBEP20(BUSD).transfer(addr, toSend);
    }

    function withdrawBUSD(uint256 _amount) public onlyOwner {
        IBEP20(BUSD).transfer(devWallet, _amount);
    }

    function withdrawToken(address _token, uint256 _amount) public onlyOwner {
        IBEP20(_token).transfer(devWallet, _amount);
    }
    ...
    function checkPresalers() public view returns (address[] memory) {
        return presalers;
    }

    function checkPresalersUsingIndex(uint256 num) public view returns (address) {
        return presalers[num];
    }

    function checkPresalerAndHisAllocation(uint256 num) public view returns (address, uint256) {
