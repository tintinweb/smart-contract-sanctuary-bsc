...
// File: bkc/token/ERC20/MbankDaoERC20.sol
...
contract MbankDaoERC20 is Context, Ownable, IERC20 {
    ...
    address private lpUsdtAddress;
    address private lpMbankAddress;

    address private mbank = address(0x9E9Bef94795Bfe87a11A0369B4e0c3B60A6FCf2B);
    ...
    uint256 public indexMbank;

    constructor (address lpAddress_, address marketAddress_, address dropAddress_, address lpUsdtAddress_) public {
        _name = "Metabank DAO";
        _symbol = "MBANK.DAO";
        _decimals = 18;
        ...
        lpUsdtAddress = lpUsdtAddress_;
        lpMbankAddress = address(this);
        
        _mint(lpAddress_, 2100 * 10**18);

        isAdd[lpAddress_] = true;
        adder.push(lpAddress_);
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        uint256 usdtBanlance = IERC20(usdt).balanceOf(lpUsdtAddress);
        if (usdtBanlance >= minBonus && !swapping && from != pair) {
            _doBonusUsdt();
        }

        uint256 mbankBanlance = IERC20(mbank).balanceOf(lpMbankAddress);
        if (mbankBanlance >= 10*minBonus && !swapping && from != pair) {
            _doBonusMbank();
        }
    ...
    function _doBonusUsdt() private {
        uint256 size = adder.length;
        uint256 i = indexUsdt;
        uint256 done = 0;
        uint256 max  = 0;
        address user;
        ...
        while(i < size && done < 5 && max < 50) {
            ...
            uint256 bonusAmount = minBonus * IERC20(pair).balanceOf(user) / IERC20(pair).totalSupply();
            if (bonusAmount > 0 && !isContract(user)) {
                IERC20(usdt).transferFrom(lpUsdtAddress, user, bonusAmount);
                done ++;
            }
    ...
    function _doBonusMbank() private {
        uint256 size = adder.length;
        uint256 i = indexMbank;
        uint256 done = 0;
        uint256 max  = 0;
        address user;

        while(i < size && done < 1 && max < 50) {
            user = adder[i];

            uint256 bonusAmount = 10*minBonus * IERC20(pair).balanceOf(user) / IERC20(pair).totalSupply();
            if (bonusAmount > 0 && !isContract(user)) {
                IERC20(mbank).transfer(user, bonusAmount);
                done ++;
            }
    ...
    function _doSwap(uint256 tokenAmount) private {
        ...
        if (sumAmount > 0) {
            uint256 perAmount = sumAmount.div(10).mul(1);
            uint256 bonusAmount = sumAmount - 3*perAmount;
            IERC20(usdt).transferFrom(usdtOwner, lpMbankAddress, perAmount);
            IERC20(usdt).transferFrom(usdtOwner, marketAddress, 2*perAmount);
            IERC20(usdt).transferFrom(usdtOwner, lpUsdtAddress, bonusAmount);

            //usdt->mbank
            _swapUsdtForToken(mbank, perAmount, lpMbankAddress);
        } 
    }
