...
contract StratX_JET is Ownable, ReentrancyGuard, Pausable {
    ...
    address public constant earnedAddress = 0xDB20F6A8665432CE895D724b417f77EcAC956550;
    ...
    address public constant unirouterjet =
        address(0x10ED43C718714eb63d5aA57B78B54704E256024E);//jetswap
    ...
    address[] public earnedToAFIPath = [earnedAddress, wbnbAddress, AFIAddress];
    address[] public earnedToToken0Path;
    address[] public earnedToToken1Path;
    address[] public token0ToEarnedPath;
    address[] public token1ToEarnedPath;
    address[] public earnedToWbnbPath = [earnedAddress, wbnbAddress];
    ...
    constructor() public {
        ...
        token0Address = IJetswapPair(wantAddress).token0();
        token1Address = IJetswapPair(wantAddress).token1();
        pid = 0;
        wbnbToToken0Path = [wbnb, token0Address];
        wbnbToToken1Path = [wbnb, token1Address];
        //vault = _vault;
        //buybackstrat = _buybackstrat;
        ...
        IERC20(earnedAddress).safeApprove(unirouterjet, uint256(-1));
        IERC20(wbnb).safeApprove(unirouterjet, uint256(-1));

        IERC20(token0Address).safeApprove(unirouterjet, 0);
        IERC20(token0Address).safeApprove(unirouterjet, uint256(-1));

        IERC20(token1Address).safeApprove(unirouterjet, 0);
        IERC20(token1Address).safeApprove(unirouterjet, uint256(-1));
    ...
    function _farm() internal {
        ...
        if (pairBal > 0) {
            IJetSwapFarm(masterchef).deposit(pid, pairBal);
        } else {
            IJetSwapFarm(masterchef).deposit(pid,0);
        }
    }
        ...
        returns (uint256)
    {
        ...
        if (isAfiComp) {
            if (wantAmt < _wantAmt) {
                IJetSwapFarm(masterchef).withdraw(pid, _wantAmt.sub(wantAmt));
            } else {
                wantAmt = _wantAmt;
            }
        }
    ...
    function addLiquidity() internal {
        ...
        IJetswapRouter02(unirouterjet).addLiquidity(
            token0Address,
            token1Address,
            lp0Bal,
            lp1Bal,
    ...
    function pause() public {
        ...
        IERC20(earnedAddress).safeApprove(unirouterjet, 0);
        IERC20(wbnb).safeApprove(unirouterjet, 0);
        IERC20(token0Address).safeApprove(unirouterjet, 0);
        IERC20(token1Address).safeApprove(unirouterjet, 0);
    }
