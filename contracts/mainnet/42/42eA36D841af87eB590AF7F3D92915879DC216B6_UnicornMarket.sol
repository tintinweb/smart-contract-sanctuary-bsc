...
contract UnicornMarket is IERC721Receiver, UnicornFee {
    ...
    constructor(IERC20 _SOO,uint _feeRate,address _feeOwner, IUnicorn _unicorn) UnicornFee(_SOO,_feeRate,_feeOwner) {
        unicorn = _unicorn;
    }
    ...
    function onERC721Received(
        ...
        require(address(unicorn) == msg.sender,"token is not in whiteList");
    ...
    function cancelOrder(uint tokenId) public {
        ...
        unicorn.safeTransferFrom(address(this),msg.sender,order.tokenId);
...
abstract contract UnicornFee is Ownable {
    ...
    constructor(IERC20 _SOO,uint256 _feeRate,address _feeOwner){
        SOO = _SOO;
        feeRate = _feeRate;
        feeOwner = _feeOwner;
    }
    ...
    function chargeFee(address account, address receipt, uint amount) internal {
        ...
        SOO.safeTransferFrom(account,feeOwner,fee);
        SOO.safeTransferFrom(account,receipt,amount.sub(fee));
    }
...
contract UnicornExchange is Ownable,IERC721Receiver {
    
    using SafeERC20 for IERC20;
    using SafeERC20 for IURC20;
    using SafeMath for uint256;

    constructor(IUnicorn _unicorn, IURC20 _token, address _changer, uint _price)  {
        unicorn = _unicorn;
        token = _token;
        changer = _changer;
        price = _price;
    ...
    function setToken(IURC20 _token) public onlyOwner {
        token = _token;
    }
    ...
    function onERC721Received(
        ...
        require(_unicorn.birth.generation == 0, "only generation 0 is allowed");
        
        unicorn.safeTransferFrom(address(this),DEAD,tokenId);
        
        token.safeTransferFrom(changer,from,price);
...
contract UnicornBreed is IERC721Receiver, UnicornFee {
    ...
    mapping(address=>uint[]) public breedTokenIds;
    
    mapping(uint=>uint) public tokenIdIndex;
    ...
    function _owns(address account, uint256 tokenId) internal view returns (bool) {
        uint _index = tokenIdIndex[tokenId];
        return breedTokenIds[account][_index] == tokenId;
    }
    ...
    function addIndex(address account, uint tokenId) internal {
        uint _index = breedTokenIds[account].length;
        breedTokenIds[account].push(tokenId);
        tokenIdIndex[tokenId] = _index;
    }
    
    function removeIndex(address account, uint tokenId) internal {
         uint _index = tokenIdIndex[tokenId];
         
         uint lastIndex = breedTokenIds[account].length - 1;
         
         require(breedTokenIds[account][_index] == tokenId, "error tokenId");
         
         if(_index != lastIndex) {
             
             uint lastTokenId = breedTokenIds[account][lastIndex];
             
             breedTokenIds[account][_index] = lastTokenId;
             
             tokenIdIndex[lastTokenId] = _index;
         }
         
         breedTokenIds[account].pop();
         
         delete tokenIdIndex[tokenId];
    }

    function breed(address account, uint256 _tokenId) internal {

        UnicornLib.Unicorn memory unicorn = UNICORN.unicorns(_tokenId);

        require(_isReadyToBreed(_tokenId));
        
        chargeFee(account, breeds.getFee(unicorn.birth.generation));

        _breed(_tokenId);
        
        addIndex(account, _tokenId);
    }
    ...
    function giveBirth(MintLib.MintData memory mintData) public {
        ...
        removeIndex(msg.sender,birth.matronId);
        
        UNICORN.safeTransferFrom(address(this),msg.sender,birth.matronId);
        
        emit GiveBirth(mintData.owner, mintData.tokenId, birth.matronId);
        
    }
    
    function getBreeds(address account) public view returns(uint[] memory tokenIds, BreedLib.Pregnancy[] memory _pregnancies) {
        tokenIds = breedTokenIds[account];
        
        _pregnancies = new BreedLib.Pregnancy[](tokenIds.length);
        
        for(uint i; i< tokenIds.length; i++) {
            _pregnancies[i] = pregnancies[tokenIds[i]];
        }
    }
...
contract UnicronAuction is Ownable {
    ...
    constructor(IUnicorn _unicorn, IURC20 _DOM)  {
        unicorn = _unicorn;
        DOM = _DOM;
    }
    ...
    event AuctionCreate(uint indexed _tokenId, uint _price);
    event AuctionBid(uint indexed _tokenId, address indexed _buyer, uint bidPrice);
    event AuctionFinish(uint indexed _tokenId, address indexed _buyer, uint bidPrice);
    event Redeem(uint indexed _tokenId, address indexed _buyer);
    ...
    function createAuction(uint tokenId, uint startBlock, uint duration, uint price) public onlyOwner {
        ...
        emit AuctionCreate(tokenId, price);
    }
    ...
    function bid(uint tokenId, uint bidPrice) public {
        ...
        emit AuctionBid(tokenId, msg.sender, bidPrice);
    }
    ...
    function finishAuction(MintLib.MintData memory mintData) public {
        ...
        require(tokenId == mintData.tokenId,"error tokenId");
        ...
        emit AuctionFinish(tokenId, owner, amount );
    }
    ...
    function geAuctions() public view returns(AuctionLib.Auction[] memory _auctions, AuctionLib.Bid[] memory _bids) {
