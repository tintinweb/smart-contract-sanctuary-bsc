...
contract Presales is Ownable, ReentrancyGuard {
    ...
    mapping (address => uint256) public userInfoRecords;
    ...
    constructor(
        address _usdt, address _recipientAddress, uint256 _minContribution, uint256 _maxContribution, 
    ...
    function contribute(uint256 _amount) external nonReentrant {
        ...
        uint256 _totalContribution = userInfoRecords[msg.sender] + _amount;
        ...
        IERC20(USDT).safeTransferFrom(msg.sender, recipientAddress, _amount);

        UserInfo storage _userInfo = userInfo[msg.sender][userInfoRecords[msg.sender]];
        _userInfo.contributeAmount = _amount;
        _userInfo.contributeTimestamp = block.timestamp;

        // Update user total contribution
        userTotalContribution[msg.sender] += _amount;

        //Add participants count if participant never contribute before
        if(userInfoRecords[msg.sender] == 0)
            participantAddressList.push(msg.sender);

        // Update user record
        userInfoRecords[msg.sender]++;

        // Update total raised fund
        totalRaised += _amount;
