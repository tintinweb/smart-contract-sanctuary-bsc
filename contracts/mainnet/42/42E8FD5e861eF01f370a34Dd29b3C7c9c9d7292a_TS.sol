...
contract TS is ERC20, Ownable {
    ...
    uint256[2] public bf = [2,1];
    uint256[2] public sf = [2,1];
    uint256[2] public tf = [2,1];
    uint256[] public df = [2,1];

    address public _foundationWalletAddress;
    address public _blackHoleWallet = address(0);
    ...
    constructor(
        ...
        _deflation = deflation_ * (10 ** decimals_);
    ...
    function _transfer(
        ...
        if (takeF) {
            ...
            if (automatedMarketMakerPairs[from]) {
                OF = amount.mul(bf[0]).div(100);
                BF = amount.mul(bf[1]).div(100);
                totalFs = OF.add(BF);
            } else if (automatedMarketMakerPairs[to]) {
                OF = amount.mul(sf[0]).div(100);
                BF = amount.mul(sf[1]).div(100);
                totalFs = OF.add(BF);
            } else {
                OF = amount.mul(tf[0]).div(100);
                BF = amount.mul(tf[1]).div(100);
                totalFs = OF.add(BF);
            }
            if (balanceOf(address(0)) >= _deflation) {
                totalFs = totalFs.sub(BF);
                BF = 0;
            }
    ...
    function takeInviter(address from, address to, uint256 amount) private returns(uint256)  {
        ...
        for (uint256 i = 0; i < df.length; i++) {
            ...
            uint256 send = amount.mul(df[i]).div(100);
