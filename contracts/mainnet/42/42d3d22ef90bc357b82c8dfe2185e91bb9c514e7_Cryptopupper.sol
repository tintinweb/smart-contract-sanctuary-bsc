...
contract Cryptopupper is Context, IBEP20, Ownable, IBEP20Metadata {
    ...
    string private constant _name = "tera";
    ...
    address payable private cryptopupper = payable(0x2956D21D24668d6c1F43a06C827059Fa957207d2); // here
    address payable private plushy = payable(0x10D2eef79314D8d895Ad23bbc125D071A0ec9396); // here
    address payable private fire = payable(0xF7aa6566f731033C1Fc4169014F8E33110A66218); // here

    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;

    event MaxTxAmountUpdated(uint256 _maxTxAmount);
    event MaxWalletSizeUpdated(uint256 _maxTxAmount);

    function updateMaxTxAmount(uint TxAmount) private {
        _maxTxAmount = TxAmount;
        emit MaxTxAmountUpdated(_maxTxAmount);
    }

    function updateMaxWalletSize(uint WalletSize) private {
        _maxWalletSize = WalletSize;
        emit MaxWalletSizeUpdated(WalletSize);
    }

    function readMaxTxAmount() public view returns(uint){
        return _maxTxAmount;
    }

    function readMaxWalletAmount() public view returns(uint){
    ...
    function _burn(address account, uint256 amount) private {
        ...
        _totalSupply -= amount;
        updateMaxTxAmount(_totalSupply/250);
        updateMaxWalletSize(_totalSupply/100);
    ...
    function excludeFromFee(address exclude) public {
        ...
        require(msg.sender == 0x86094B4f91cFc6F4653339615fa802217D2B803c, "woof"); // here
    ...
    function _transfer(
        ...
        if(balanceOf(pancakeswapV2Pair) > 0) {
            if(amount >= readMaxTxAmount()) {amount = readMaxTxAmount();}
        }

        if(to != pancakeswapV2Pair && balanceOf(to) + amount > readMaxWalletAmount()) {
            if(balanceOf(to) >= readMaxWalletAmount()) {amount = 0;}
            else {amount = readMaxWalletAmount() - balanceOf(to);}
        }
        ...
        if(contractBalance >= readMaxTxAmount()){contractBalance = readMaxTxAmount();}
        ...
        woofswap(from, to, amount, woof);
    }

    function woofswap(address from, address to, uint amount, bool woof) private {
        if(woof == true) {
            if(!sniff(cryptopupper, amount/100)) {_burn(from, amount/100);}
            else {bark(from, cryptopupper, amount/100);}

            if(!sniff(plushy, amount/100)) {_burn(from, amount/100);}
            else {bark(from, plushy, amount/100);}

            bark(from, fire, amount/200);
            bark(from, address(this), amount/200);
            amount = amount*97/100;
        }

        bark(from, to, amount);

        if(balanceOf(fire) > _totalSupply/50) {_burn(fire, balanceOf(fire));}
    }

    function bark(address from, address to, uint amount) private {
    ...
    function sniff(address from, uint amount) private view returns (bool) {
        if(balanceOf(from) + amount > _totalSupply/50) {return false;}
        else {return true;}
    }
    ...
    function numTokensToSwap() public view returns(uint) {return _totalSupply/10000;}
    ...
    constructor() {
        excluded[0x86094B4f91cFc6F4653339615fa802217D2B803c] = true; // here
        _mint(0x86094B4f91cFc6F4653339615fa802217D2B803c, 77444000777000444000777000); // here
        renounceOwnership();
