...
contract WaifuRerollerShundereSpeed is Auth {
    ...
    function reroll(uint256 id) external onlyOwner{
        IShoujoStats(_shoujoStats).reroll(id,true,true);

        (uint256 _rarity, uint256 type_, uint256 _attack, uint256 _speed) = checkStats(id);
