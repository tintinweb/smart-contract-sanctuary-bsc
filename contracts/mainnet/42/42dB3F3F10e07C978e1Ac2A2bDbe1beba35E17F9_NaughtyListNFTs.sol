...
contract NaughtyListNFTs is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    uint256 public rate = 9999999999000000000;
    ...
    bool public mintsWithTokens = false;

    // Addresses
    address public DexRouter = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
    address public RewardToken = 0x07b724bd4f81669D4c035eB387DeD4eb0f85962b;
    address public FeeReceiver = 0x7762b342E2F264eB259299Cf9a310ca1e8dC03CA;
    ...
    // Setters
    function setMintsWithTokens(bool _mintsWithTokens) external onlyOwner {
        mintsWithTokens = _mintsWithTokens;
    }
    ...
    function mint(uint256 _mintAmount) public payable {
        // Checks
        require(!paused, "NaughtyListNFTs: minting has not started yet");
        require(_mintAmount > 0, "NaughtyListNFTs: you have to mint at least one");
        require(
            _mintAmount <= maxMintAmount,
            "NaughtyListNFTs: you can only mint 10 at a time"
        );
        require(
            lastSupply >= _mintAmount,
            "NaughtyListNFTs: the collection is sold out"
        );
        if (msg.sender != owner()) {
            require(
                msg.value >= cost * _mintAmount,
                "NaughtyListNFTs: total cost isn't match"
            );
        }
        // Minting
    ...
    function mintERC(uint256 _mintAmount) public {
        require(mintsWithTokens, "Mint only with BNB");
        require(!paused, "NaughtyListNFTs: minting has not started yet");
        require(_mintAmount > 0, "NaughtyListNFTs: you have to mint at least one");
        require(_mintAmount <= maxMintAmount,  "NaughtyListNFTs: you can only mint 5 at a time");
        require(lastSupply >= _mintAmount,  "NaughtyListNFTs: the collection is sold out");
        uint256 balance = token.balanceOf(msg.sender);
        if (msg.sender != owner()) {
            require(balance >= _mintAmount.mul(rate), "NaughtyListNFTs: total cost isn't match");
        }
        token.transferFrom(msg.sender, FeeReceiver, _mintAmount.mul(rate));
        // Minting
        for (uint256 i = 1; i <= _mintAmount; i++) {
        // Mint for caller
            _randomMint(msg.sender);
    ...
    {
        // Checks
        require(_winner != address(0), "NaughtyListNFTs: zero address");
        require(_amount > 0, "NaughtyListNFTs: you have to mint at least one");
        require(lastSupply != 0, "NaughtyListNFTs: the collection is sold out");
    ...
    function tokenURI(uint256 _id)
        ...
        returns (string memory)
    {
        require(_exists(_id), "NaughtyListNFTs: URI query for nonexistent token");
