...
contract MetafameSaleMTv2 is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, Status{
    ...
    mapping(address => uint256) public vcAlreadyMinted;    
    ...
    function purchaseBoxVCsale(
        ...
        require(payAmount >= busdPerBoxVC * 10**18 * number, "MetafameSale: Insufficient pay amount");
    ...
    function purchaseBoxPresale(
        ...
        require(payAmount >= busdPerBoxWL * 10**18 * number, "MetafameSale: Insufficient pay amount");
    ...
    function purchaseBoxPublicSale(
        ...
        require(payAmount >= busdPerBox * 10**18 * number, "MetafameSale: Insufficient pay amount");
    ...
    function makeOver( 
        ...
        uint256 finalPrice;
        if(payType == 1){
            uint256 amountIn = getAmountsIn(makeOverPrice); 
            ...
            require(payAmount >= makeOverPrice * 10**18, "MetafameSale: Insufficient pay amount");
            finalPrice = payToken(makeOverPrice, payType);
        }
    ...
    function refresh( 
        ...
        uint256 finalPrice;
        if(payType == 1){
            uint256 amountIn = getAmountsIn(refreshPrice); 
            require(payAmount >= amountIn * 10**18, "MetafameSale: Insufficient pay amount");
            finalPrice = amountIn * 10**18;
            payToken(finalPrice, payType);
        }
        else{
            require(payAmount >= refreshPrice * 10**18, "MetafameSale: Insufficient pay amount");
            finalPrice = payToken(refreshPrice, payType);
        }
    ...
    function payToken(uint256 price, uint256 payType) private returns (uint256 finalPrice){
        require(payType <= 1, "MetafameSale: Wrong pay type");
        if(payType == 0){
    ...
    modifier typeCheck(uint256 gender, uint256 skin) {
