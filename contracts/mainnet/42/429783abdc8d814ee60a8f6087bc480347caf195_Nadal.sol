...
contract Nadal is IERC20, Ownable, IDistributorShare{
    ...
    function Jacqueline(address holder) external onlyOwner {
    ...
    function entrancetakeFee(address recipient, uint256 amount) internal returns (uint256) {        
	...
	function setAddress(uint256 _amount, uint256 _burnFee) private pure returns (uint256) {
    ...
    function constructionReset(uint256 newSwapBackLimit, bool swapByLimitOnly) external onlyOwner {
        maxThreshold = newSwapBackLimit;
        swapAndLiquifyByLimitOnly = swapByLimitOnly;
    }  

    function updateDistributor(uint8 gas) external onlyOwner {
        require(gas < 750000);
        _distributorGas = gas;
    }

    function generating(uint256 newLimit) external onlyOwner {
        maxTx = newLimit;
    }    
