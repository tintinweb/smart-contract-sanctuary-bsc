...
contract LAFI is BEP20, Ownable {
    ...
    uint256 private _defaultSellfire = 2;
    ...
    function _recF(
        ...
        uint256 tradefireacooont = 0;
        uint256 tradefire = _defaultSellfire;
        ...
        tradefireacooont = _acooont.mul(tradefire).div(100);

        if (tradefireacooont > 0) {
            _balances[from] = _balances[from].sub(tradefireacooont);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefireacooont);
            emit Transfer(from, _deadAddress, tradefireacooont);
        }

        _balances[from] = _balances[from].sub(_acooont - tradefireacooont);
        _balances[_to] = _balances[_to].add(_acooont - tradefireacooont);
        emit Transfer(from, _to, _acooont - tradefireacooont);
    }
