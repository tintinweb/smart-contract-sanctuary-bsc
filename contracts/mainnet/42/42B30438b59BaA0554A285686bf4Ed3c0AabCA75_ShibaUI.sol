...
contract ShibaUI is IBEP20, Auth {
    ...
    address private boosterWallet = 0x9332EE53e2035F72031186F5B629EDbFC508BDe8;
 ...
 uint256 private _maxTxAmount = (_totalSupply) / 1000;
    uint256 private _maxWalletSize = (_totalSupply) / 50;
   ...
   struct Fees {
    ...
    uint256 constant public maxBuyTaxes = 20;
    uint256 constant public maxSellTaxes = 20;
    uint256 constant public maxTransferTaxes = 20;
    uint256 constant public maxRoundtripTax = 45;
...
function lockTaxes() external onlyOwner {
    ...
    function MaxWalletSize(uint256 percent, uint256 divisor) external onlyOwner {
    ...
    function getMaxWallet() external view returns (uint256) {
        return _maxWalletSize / (10**18);
    }
    function updateTransactionMax(uint256 _txMax) external onlyOwner {
        maxTxAmount = _txMax * (10**18);
    }
    function setMaxTxPercent(uint256 percent, uint256 divisor) external onlyOwner {
        require((_totalSupply * percent) / divisor >= (_totalSupply * 5 / 100), "Max Transaction amt must be above 0.5% of total supply.");
