...
contract NfterriumNet is Initializable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function setRefCounts(uint256 _id, uint256[8] memory _newCounts) public onlyRole(MANAGER_ROLE) {
        refCounts[_id] = _newCounts;
    }

    function setRefDirCounts(uint256 _id, uint256[8] memory _newCounts) public onlyRole(MANAGER_ROLE) {
    ...
    function getRefCounts(uint256 _id) public view returns (uint256[8] memory) {
        uint256[8] memory result = refCounts[_id];
        return result;
    }

    function getRefDirCounts(uint256 _id) public view returns (uint256[8] memory) {
        uint256[8] memory result = refDirCounts[_id];
        return result;
    }
