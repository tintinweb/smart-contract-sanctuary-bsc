...
contract MGA is CfoTakeable,IERC20Metadata {
    using Address for address;
    ...
    //mainnet: 
    string private constant _name = "MGA";
    //mainnet: 
    ...
    uint public burnRate = 2 * 1e16;
    uint public fundRate = 2 * 1e16;
    uint public personRate = 4 * 1e16;

    address public fundFeeTo = address(0xeB25d0552605EdA5BaB8C034798cd22b7A001F4f);
    address public personFeeTo = address(0x0836C05442D9a285375c1483f30131A662aebA35);
    ...
    constructor(){
        ...
        isWhiteList[fundFeeTo] = true;
        isWhiteList[personFeeTo] = true;
    ...
    function _transfer(
        ...
        if(recipient != address(0) && (isSwapPair(sender) || isSwapPair(recipient)) && !isWhiteList[sender] && !isWhiteList[recipient]){
    ...
    function isSwapPair(address pair) public view returns(bool){
        ...
        return bitbyteSwapFactory.getPair(address(this), usdt) == pair || pancakeSwapFactory.getPair(address(this), usdt) == pair || isOtherSwapPair[pair];
    }
    ...
    function setFeeRates(uint _burnRate,uint _fundRate,uint _personRate) external onlyOwner {
        require(_burnRate.add(_fundRate).add(_personRate) < 1e18,"sum of rates must less than 1e18");
        burnRate = _burnRate;
        fundRate = _fundRate;
        personRate = _personRate;
    }

    function setFeeTos(address _fundFeeTo,address _personFeeTo) external onlyOwner {
