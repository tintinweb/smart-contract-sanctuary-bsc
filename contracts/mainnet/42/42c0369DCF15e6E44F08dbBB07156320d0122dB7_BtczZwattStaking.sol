...
contract BtczZwattStaking {
    ...
    address constant public Zwatt = 0xAabC48B90e60163a80C664BD27b46ad95984f2b8;

    uint256 public ZwattPerHour = 1 ether;
    ...
    function Harvest() public returns (bool) {
        ...
        IERC20(Zwatt).transfer(msg.sender, User.amount / 1 ether * Timegose * ZwattPerHour / 3600);
    ...
    function setZwattPerHour(uint256 amount) public onlyOwner() returns (bool) {
        ZwattPerHour = amount;

        return true;
    }

    function withdrawZwatt(uint256 amount) public onlyOwner() returns (bool) {
        IERC20(Zwatt).transfer(msg.sender, amount);
