...
contract MagicUnicorn10 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    MagicUnicorn10DividendTracker public dividendTracker;
    ...
    constructor() ERC20("MagicUnicorn10", "MU10") {
        ...
        dividendTracker = new MagicUnicorn10DividendTracker(payable(this), router, 0x17Bd2E09fA4585c15749F40bb32a6e3dB58522bA,
                                                   "MagicUnicorn10TRACKER", "MU10TRACKER");
...
contract MagicUnicorn10DividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    MagicUnicorn10 public MagicUnicorn10Contract;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        MagicUnicorn10Contract = MagicUnicorn10(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main MagicUnicorn10 contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            address[] memory path = new address[](2);
            path[0] = uniswapV2Router.WETH();
            path[1] = address(MagicUnicorn10Contract);

            uint256 prevBalance = MagicUnicorn10Contract.balanceOf(address(this));
            ...
            {
                uint256 received = MagicUnicorn10Contract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    MagicUnicorn10Contract.transfer(account, received);
                } 
                else {
                    success = false;
                }
