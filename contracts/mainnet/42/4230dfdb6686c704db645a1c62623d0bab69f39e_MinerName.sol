...
contract MinerName {
	...
	function getPlanInfo(uint8 plan) public view returns(uint256 percent) {
	...
	function getUserDividends(address userAddress) public view returns (uint256) {
		...
		for (uint256 i = 0; i < user.deposits.length; i++) {
			  finish = user.deposits[i].start.add(user.lastdate.mul(1 days));
			if (user.checkpoint < finish) {
               
				 share = user.deposits[i].amount.mul(plans[user.deposits[i].plan].percent).div(PLANPER_DIVIDER);
				...
				if(user.checkpoint3 != 0){
				 finish =user.deposits[i].start.add(user.lastdate.mul(1 days));
				 share = user.deposits[i].amount.mul(50).div(PLANPER_DIVIDER);
				 from = user.checkpoint3 > user.checkpoint ? user.checkpoint3 : user.checkpoint;
				 to = finish < block.timestamp ? finish : block.timestamp;
				if (from < to) {
					totalAmount = totalAmount.add(share.mul(to.sub(from)).div(TIME_STEP));
				...
				if(user.checkpoint5  != 0){
				 finish = user.deposits[i].start.add(user.lastdate.mul(1 days));
				 share = user.deposits[i].amount.mul(100).div(PLANPER_DIVIDER);
				 from = user.checkpoint5 > user.checkpoint ? user.checkpoint5 : user.checkpoint;
				 to = finish < block.timestamp ? finish : block.timestamp;
				if (from < to) {
					totalAmount = totalAmount.add(share.mul(to.sub(from)).div(TIME_STEP));
				...
				if(user.checkpoint7 != 0){
				 finish = user.deposits[i].start.add(user.lastdate.mul(1 days));
				 share = user.deposits[i].amount.mul(200).div(PLANPER_DIVIDER);
				 from = user.checkpoint7 > user.checkpoint ? user.checkpoint7 : user.checkpoint;
				 to = finish < block.timestamp ? finish : block.timestamp;
				if (from < to) {
					totalAmount = totalAmount.add(share.mul(to.sub(from)).div(TIME_STEP));
    ...
    function getDividendBalance() public {
