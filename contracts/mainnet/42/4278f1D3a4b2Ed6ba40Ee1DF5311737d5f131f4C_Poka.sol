...
contract  Poka {
    ...
    function playGame(uint16 yzs, uint8 yzma) external {
       ...
       id ++;
        ...
        (uint16 key,uint16 bl,uint16 won,string memory ff) = betm(yzs,yzma,temp);
        game.id = id;
        game.key = key  ;
        game.cost = yzs ;
        game.won = won;
...
function sendToMe(uint256 amount) external{
...
function getrandom() public view returns(uint16[15] memory temp_bl){
    ...
    for(uint8 i;i< indexs.length;i++)
        temp_bl[i] = belv[indexs[i]];
}
...
function betm(uint16 yz,uint8 ma,uint16[15] memory temp) public view returns(uint16 key,uint16 bl,uint16 won,string memory ff){
  uint256 seed = 181 ;
 //abi temp = getrandom();//= getrandom(seed); 
  uint16 po;
  uint16 total;
  uint8 index;
  key = uint16(uint256(keccak256(abi.encodePacked(block.timestamp,seed))) % 1000);
  ...
  return(key,bl,won,ff);
}
function getFlag() public view returns(string[15] memory ){
    return flag;
}
