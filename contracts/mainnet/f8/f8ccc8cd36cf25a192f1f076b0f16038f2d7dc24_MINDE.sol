...
contract MINDE is ERC20, Ownable {
    ...
    uint256 public _SellLimit;   //Anti-Whale
    ...
    constructor() ERC20("Day Miner", "MINDE")  {
        ...
        _SellLimit = totalSupply.mul(3).div(100);
    ...
    function setSellLimitByAmount(uint amount) public onlyOwner {
        _SellLimit = amount;
    }

    function setSellLimit(uint256 _percentage,uint _divider) public onlyOwner {
        // uint restrictedAmount =  totalSupply().mul(3).div(100);
        // require(amount <= restrictedAmount,"Sell Limit Must not be more than 3%");
        _SellLimit = totalSupply().mul(_percentage).div(10 ** _divider);
    }
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            if(automatedMarketMakerPairs[to]){
                ...
                if(!isSellLimitExempt[from]) {
                    require(amount <= _SellLimit, "Sell amount exceeds the maxSellAmount.");
                }  
