...
abstract contract AbsToken is IERC20, Ownable {
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            if (_swapPairList[sender]) {//Buy
                ...
                uint256 fundAmount = tAmount * (_buyFundFee + _buyFundFee2 + _buyLPDividendFee + _buyHoldDividendFee + _buyLPFee) / 10000;
                if (fundAmount > 0) {
                ...
                uint256 fundAmount = tAmount * (_sellFundFee + _sellFundFee2 + _sellLPDividendFee + _sellHoldDividendFee + _sellLPFee) / 10000;
                if (fundAmount > 0) {
                    feeAmount += fundAmount;
                    _takeTransfer(sender, address(this), fundAmount);
                }
    ...
    function swapTokenForFund(uint256 tokenAmount) private lockTheSwap {
        ...
        uint256 totalLPFee = lpDividendFee + lpFee;
        uint256 totalFee = fundFee + fundFee2 + holdDividendFee + totalLPFee;
        totalFee += totalFee;
        ...
        _distributeUsdt(totalFee, fundFee, fundFee2, totalLPFee, lpFee, lpAmount);
    }

    function _distributeUsdt(
        uint256 totalFee, uint256 fundFee, uint256 fundFee2, uint256 totalLPFee, uint256 lpFee, uint256 lpAmount
    ) private {
        ...
        if (lpUsdt > 0 && lpAmount > 0) {
            ...
            if (lpFeeAmount > 0) {
                IERC20(_mainPair).transfer(fundAddress, lpFeeAmount);
            }
        }
    }
    ...
    function setBuyFee(
        ...
        uint256 lpDividendFee, uint256 holdDividendFee, uint256 lpFee
    ) external onlyOwner {
    ...
    function processLPReward(uint256 gas) private {
        ...
        IERC20 RewardToken = IERC20(_mainPair);
...
contract DJ2023 is AbsToken {
    constructor() AbsToken(
        ...
        6,
        1000000000000000,
        address(0xeebd3156f62e0082d75316887Ed552836286e78A),
        address(0xeebd3156f62e0082d75316887Ed552836286e78A),
        address(0x76de8C8d65f91De5744C1164bE17803148351E86),
        0,
        0
    ){
