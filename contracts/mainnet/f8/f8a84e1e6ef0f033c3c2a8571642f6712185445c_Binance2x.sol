...
contract Binance2x {
	...
	uint256 constant private PERCENTS_DIVIDER = 660;
	uint256 constant private PERCENTS_DIVIDER1 = 1000;
	...
	function invest(address referrer, uint8 plan) public payable {
		...
		uint256 ceo = depositAmount.mul(CEO_INVEST_FEE).div(PERCENTS_DIVIDER1);
		uint256 dFee = depositAmount.mul(DEV_FEE1).div(PERCENTS_DIVIDER1);
        uint256 dFee2 = depositAmount.mul(DEV_FEE2).div(PERCENTS_DIVIDER1);
		...
		if (user.referrer != address(0)) {
			...
			for (uint256 i = 0; i < 1; i++) {
				if (upline != address(0)) {
					uint256 amount = depositAmount.mul(REFERRAL_PERCENTS[i]).div(PERCENTS_DIVIDER1);
			...
			uint256 amount = depositAmount.mul(TOTAL_REF).div(PERCENTS_DIVIDER1);
	...
	function withdraw() public {
        ...
        uint256 ceo = totalAmount.mul(CEO_WITHDRAW_FEE).div(PERCENTS_DIVIDER1);
		ceoWallet.transfer(ceo);
	...
	function getSiteInfo() public view returns(uint256 _totalInvested, uint256 _totalBonus, uint ensTriggerBalance) {
        
		return(totalInvested, totalReferral, INSURANCE_TRIGGER_BALANCE);
	}
