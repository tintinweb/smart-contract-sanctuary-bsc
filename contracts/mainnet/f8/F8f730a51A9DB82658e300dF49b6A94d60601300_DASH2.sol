...
contract DASH2 is Context, IBEP20, Ownable {
    ...
    function _getTBasics(uint256 tAmount, uint256 taxFee, uint256 burnFee, uint256 charityFee) private view returns (uint256, uint256, uint256) {
        uint256 tFee = ((tAmount.mul(taxFee)).div(_GRANULARITY)).div(1000);
        uint256 tBurn = ((tAmount.mul(burnFee)).div(_GRANULARITY)).div(1000);
        uint256 tCharity = ((tAmount.mul(charityFee)).div(_GRANULARITY)).div(1000);
