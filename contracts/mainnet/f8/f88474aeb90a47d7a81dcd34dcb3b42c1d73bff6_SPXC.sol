...
contract SPXC is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 100000000000 * 10**10 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    address public _devWalletAddress = 0xc8b2b0C262D0518899ecE1e67cCa1BA7Ea4D8d29;
    ...
    function setBUYFeePercent(uint256 taxFee, uint256 marketingFee, uint256 devFee) external onlyOwner() {
        _BUYtaxFee = taxFee;
        _BUYliquidityFee = 0;
    ...
    function _takeDev(address sender, uint256 tDev) private returns(uint256){
      uint256 rDev = calculateDevFee(tDev);
      _rOwned[_devWalletAddress] = _rOwned[_devWalletAddress].add(rDev);
      _rOwned[sender] = _rOwned[sender].sub(tDev);
      emit Transfer(sender, _devWalletAddress, rDev);
      return rDev;
    }

     function _takeSPX(address sender, uint256 tSPX) private returns(uint256){
      uint256 rSPX = calculateSPXFee(tSPX);
      _rOwned[_SPXWalletAddress] = _rOwned[_SPXWalletAddress].add(rSPX);
      _rOwned[sender] = _rOwned[sender].sub(tSPX);
      emit Transfer(sender, _SPXWalletAddress, rSPX);
      return rSPX;
    }
    ...
    function _takeMarketing(address sender, uint256 tMarketing) private returns(uint256){
      ...
      _rOwned[sender] = _rOwned[sender].sub(tMarketing);
      emit Transfer(sender, _marketingWalletAddress, rMarketing);
      return rMarketing;
    }
     
    function removeAllFee() private {
        if(_taxFee == 0 && _devFee == 0 && _marketingFee == 0 && _liquidityFee == 0) return;
    ...
    function _transfer(
        ...
        if(takeFee == 1)
        {
            if(from == uniswapV2PairAddr)
            {
                ...
                uint256 devAmount = _takeDev(from, amount);
                uint256 marketingAmount = _takeMarketing(from, amount);

                amount = amount - (devAmount + marketingAmount);
            ...
            if(to == uniswapV2PairAddr)
            {
                ...
                uint256 SPXAmount = _takeSPX(from, amount);
                uint256 marketingAmount = _takeMarketing(from, amount);

                amount = amount - (SPXAmount + marketingAmount);
            }
