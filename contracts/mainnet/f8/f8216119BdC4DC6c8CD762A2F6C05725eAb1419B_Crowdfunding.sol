...
contract Crowdfunding is AccessControlEnumerable {
    ...
    address public constant lp = 0xAbF64b7EA6220eFE9C42e4283b16583A83A66Edc; //lp token
    address public constant metaPoint = 0x0a29702D828C3bd9bA20C8d0cD46Dfb853422E98; //metaPoint address
    address public donate = 0x8DDeaD5dA29A08E35110eE0c216A85cBE2C65884; //捐赠池 功能?
    address public stake = 0x9c88692C6740359D849230139171176a77a96d63; //质押池
    address public fee = 0x221A126E0F2B4A6c5848689145589f9A229dc3aa; //手续费地址
    ...
    function setMaxReserveRound(uint16 maxRound) external onlyRole(OPERATOR_ROLE) {
    ...
    function fund(uint256 _regionIndex, uint256 _amount) external {
        ...
        Project memory p = projects[_regionIndex][_round];
        //前一期超时超时重置
        ...
        require(_amount <= getMaxAmountPerTime(p.targetAmount, regions[_regionIndex].poolType), "max amount limited");
        ...
        projects[_regionIndex][_round] = p;
        //自动结束本期
    ...
    function reserve(
        ...
        if (p.state > 1 || block.timestamp > p.startTime || _round >= _maxRound) revert ProjectStateError();
        if (block.timestamp > p.startTime) revert ProjectTimeError();
        require((_amount + p.reserveFundAmount) * 2 <= p.targetAmount, "no reserve amount left");
    ...
    function addProjectId(
        ...
        if (harvestableIds[_account][_regionIndex].add(_round)) {
    ...
    function resetProjects(uint256 _regionIndex, uint256 _round) internal {
        ...
        bool resetTime = block.timestamp > projects[_regionIndex][_round].startTime + _period + min(RESTART_TIME_GAP, _period);
    ...
    //查询收益 0:投入本金,1:返回本金,2:静态收益,3:免费提现额度,4:优先额度
    function pending(
        address _account,
        uint256 _regionIndex,
        uint256 _round
    ...
    function recordMetaPoint(address _account) internal returns (bool) {
        if (!accounts[_account].pointFlag) {
            ...
            if (accounts[_account].vips == 0) {
                accounts[_account].vips = account.vip < 2 ? account.vip : 1;
            }
    ...
    function pendingForBrokerReward(
        ...
        for (uint256 i = 0; i < _roundIds.length; i++) {
            ...
            states[i] = state;
            if (state == 5 && brokerRewards[_account][_regionIndex][_round].version == version && brokerRewardIds[_account][_regionIndex].contains(_round)) {
    ...
    function getProjectIds(address _account, uint256 _regionIndex) public view returns (uint256[] memory) {
        return projectIds[_account][_regionIndex];
    }

    function getHarvestableIds(address _account, uint256 _regionIndex) public view returns (uint256[] memory) {
        return harvestableIds[_account][_regionIndex].values();
    }

    function getBrokerRewardIds(address _account, uint256 _regionIndex) public view returns (uint256[] memory) {
        return brokerRewardIds[_account][_regionIndex].values();
    }

    //单次最大投资金额
    function getMaxAmountPerTime(uint256 _targetAmount, uint16 _poolType) public view returns (uint256) {
        uint256 _max = (_targetAmount * (_poolType == 0 ? rates[9] : rates[10])) / 1000;
        return _max > MAX_FUND_AMOUNT ? _max : MAX_FUND_AMOUNT;
    }
