...
contract BaseContract is Pausable, Ownable, ReentrancyGuard {
    ...
    function _sendToken(
        ...
        if (thereIsAMessage) {
            require(baseStorage.whitelistContracts[_dAppReceiverContract], "Third-party message handler contract not whitelisted");

            address receivedToken = _nativeOut ? NULL_ADDRESS : _token;
            try IRangoMessageReceiver(_dAppReceiverContract)
                .handleMessage(receivedToken, _amount, processStatus, _dAppMessage)
            {
                emit CrossChainMessageCalled(_dAppReceiverContract, receivedToken, _amount, processStatus, true, "", new bytes(0));
            } catch Error(string memory reason) {
                emit CrossChainMessageCalled(_dAppReceiverContract, receivedToken, _amount, processStatus, false, reason, new bytes(0));
            } catch (bytes memory lowLevelData) {
                emit CrossChainMessageCalled(_dAppReceiverContract, receivedToken, _amount, processStatus, false, "", lowLevelData);
            }
        }
    }
