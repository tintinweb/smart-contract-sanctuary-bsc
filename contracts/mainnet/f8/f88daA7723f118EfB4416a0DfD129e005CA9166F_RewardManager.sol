...
contract RewardManager is AbstractYielder, IRewardManager {
    ...
    uint256 public minRewardBalance ;
    IFund public fund;
    IERC20 public token;
    ...
    modifier onlyTokenContractOrOwner() {
        require(msg.sender == address(token) || hasRole(MANAGER_ROLE, msg.sender), "Only token contract can call this function");
        _;
    }

    function initialize(ERC20 _token, address _feeAddress) public initializer {
        __AbstractYielder__init();
        token = _token;
        feeAddress = _feeAddress;
        minRewardBalance = 800_000 * 1e18;
    }
    ...
    function setFundAddress(address newAddress) public onlyRole(MANAGER_ROLE) {
    ...
    function setTokenAddress(address newAddress) public onlyRole(MANAGER_ROLE) {
    ...
    function setFeeAddress(address newAddress) public onlyRole(MANAGER_ROLE) {
    ...
    function setFeeProvider(address newAddress) public onlyRole(MANAGER_ROLE) {
    ...
    function updateMinRewardBalance(uint256 _minRewardBalance) external onlyRole(MANAGER_ROLE) {
        minRewardBalance = _minRewardBalance;
    }

    function banAddress(address _address) external onlyRole(MANAGER_ROLE) {
    ...
    function includeAddress(address _address) public virtual override onlyRole(MANAGER_ROLE) {
    ...
    function deposit(uint256 amount) override virtual public onlyRole(MANAGER_ROLE) {}
    function withdraw(uint256 amount) override virtual public onlyRole(MANAGER_ROLE) {}
...
abstract contract AbstractYielder is
    IYielder,
    PausableUpgradeable,
    AccessControlUpgradeable
{
    ...
    uint256 public globalRewardMultiplier ;
    ...
    function __AbstractYielder__init() public onlyInitializing {
        __AccessControl_init();
        __Pausable_init();
        __AbstractYielder__init_unchained();
    }

    function __AbstractYielder__init_unchained() public onlyInitializing {
    ...
    function _totalReward() internal view virtual returns (uint256);

    // Called whenever a rewards reqests a claim
    function _claimReward(uint256 reward, address receiver) internal virtual;
    ...
    function _sweepOutstandingReward(uint256 reward) internal virtual;

    // Handles fund transfers on deposit
    function _deposit(uint256 amount, address _sender) internal virtual;

    // Handles fund transfer on witdraw
    function _withdraw(uint256 amount, address _receiver) internal virtual;

    // Returns the pending reward for a given investor
    function pendingReward(address _investor)
        public
        view
        virtual
        override
    ...
    function excludeAddress(address _blacklistAddress)
        public
        virtual
    ...
    function includeAddress(address _blacklistAddress)
        public
        virtual
        onlyRole(MANAGER_ROLE)
    {
    ...
    function calculateRewardShare(
        ...
        uint256 rewardPointShare = calculateRewardPointShare(
            _userInfo,
            _rewardPoints
        );
    ...
    function calculateRewardPointShare(
        InvestorInfo memory _userInfo,
        uint256 _rewardPoints
    ) internal view returns (uint256) {
        uint256 globalMultiplerOnJoin = getGlobalMultiplierSnapshot(
            _userInfo.joinRound
        );
        ...
        if (currentRound > _userInfo.joinRound) {
            rewardDebt =
                (rewardDebt * globalRewardMultiplier * roundMultiplier) /
                (globalMultiplerOnJoin * joinRoundMultiplier);
        }

        return _rewardPoints - rewardDebt;
    }

    function getGlobalMultiplierSnapshot(uint256 round)
        internal
        view
        returns (uint256)
    {
    ...
    function getDepositAmountSnapshot(uint256 round)
        internal
        view
        returns (uint256)
    {
    ...
    function updateRoundInfo() internal {
        ...
        if (currentReward < totalRewards) {
            ...
            globalRewardMultiplier =
                (globalRewardMultiplier * roundMultipiler) /
                1000;

            currentRound++;
            roundInfo[currentRound]
                .globalMultiplierSnapshot = globalRewardMultiplier;
    ...
    function claimRewardTo(uint256 amount, address receiver)
        public
        virtual
        override
        whenNotPaused
    ...
    function updateMultiplierAndPoints() private {
        ...
        if (depositSnap > 0) {
            setRoundMultiplier(
                currentRound,
                (totalDeposited * 1000) / depositSnap
            );
        }

        roundMultipiler = getRoundMultiplier(currentRound);
        rewardPoints =
            (roundInfo[currentRound].points * roundMultipiler) /
            1000;
    }
    ...
    function depositTo(uint256 amount, address _address)
        internal
        returns (bool)
    {
    ...
    function withdrawFrom(uint256 amount, address _address)
        internal
        returns (bool)
    {
        updateRoundInfo();
