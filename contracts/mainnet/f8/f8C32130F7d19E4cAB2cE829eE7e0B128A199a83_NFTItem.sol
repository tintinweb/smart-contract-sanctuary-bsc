...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function _baseURI() internal view virtual returns (string memory) {
        return "https://people.top/lib/images/partner/NFTlogo.png";
    }
...
contract NFTItem is Ownable,ERC721,IERC721Enumerable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    
    DispatchHandle public dispatchHandle = DispatchHandle(0xbc9D55a5ce716B8c993a45b7C6144d5E6873688f);
    ...
    uint256 public indexId = 1000;

    constructor() ERC721("PEOPLE NFT", "PEOPLE NFT") {
    }
    
    function mintIdoNFT(address account) public onlyMinter returns (uint256) {
        _safeMint(account, indexId);
        indexId++;
        return indexId - 1;
    }
    
    function batchMintNft(address account,uint256 k) public onlyMinter {
    ...
    function batchBurn(uint256[] memory _nftIds) public onlyMinter {
        for (uint256 i; i < _nftIds.length; i++) {
            _burn(_nftIds[i]);
        }
    }
    
    function batchTransfer(address to,uint256[] memory _nftIds) public {
        for (uint256 i; i < _nftIds.length; i++) {
            safeTransferFrom(msg.sender,to,_nftIds[i]);
        }
    }
    ...
    function setdispatchHandle(address _account) public onlyOwner {
        dispatchHandle = DispatchHandle(_account);
    }
