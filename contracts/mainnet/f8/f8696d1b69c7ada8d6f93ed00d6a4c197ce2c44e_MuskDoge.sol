...
contract MuskDoge is IBEP20, Ownable
{
    ...
    uint32 public constant SellLimitDivider=1000; // 1,000 (0.1%)
    ...
    uint32 public constant MaxSellLockTime= 604800; // 30 Minutes i.e 1800 sec
    ...
    uint256 private constant DefaultLiquidityLockTime= 3600;
    //DevWallets
	...
	uint256 private taxChangeTime = 60; // 1 Minutes in seconds.
    ...
    uint16 private _transferTax;
    uint16 private _sniperSellTax;
    uint16 private _burnTax;
    uint16 private _liquidityTax;
    uint16 private _stakingTax;
    ...
    constructor () {
		...
		//Sets sellLockTime
        sellLockTime=1800; // 20 Seconds
		
		//Sets buyLockTime
        buyLockTime=20; // 2 Seconds
    ...
    function _taxedTransfer(address sender, address recipient, uint256 amount,bool isBuy,bool isSell) private{
        ...
        if(isSell){
            ...
            if (block.timestamp - lastReceived[sender] < 3 hours && antiSniper) {
                _sellTax = _sniperSellTax;
            }
            ...
            //Sells can't exceed the sell limit(1,000 Tokens at start, can be updated to circulating supply)
    ...
    function _calculateFee(uint256 amount, uint16 tax, uint16 taxPercent) private pure returns (uint256) {
    ...
    //marketing/
    uint16 public marketingShare=80;
    ...
    function claimREWARDS(address addr) private{
        ...
        path[1] = 0xbA2aE424d960c26247Dd6c32edC70B295c744C43; //REWARDS
    ...
    function getTaxes() public view returns(uint256 burnTax,uint256 liquidityTax,uint256 marketingTax, uint256 buyTax, uint256 sellTax, uint256 transferTax, uint256 sniperSellTax){
        return (_burnTax,_liquidityTax,_stakingTax,_buyTax,_sellTax,_transferTax,_sniperSellTax);
    }
    ...
    function TeamChangeRewardTokenName(string memory TokenName) public onlyOwner{
        REWARDNAME  = TokenName;
    }

    function TeamChangeRewardTokenAddress(address rewardAddress) public onlyOwner{
    ...
    function TeamSetTaxes(uint16 burnTaxes, uint16 liquidityTaxes, uint16 stakingTaxes,uint16 buyTax, uint16 sellTax, uint16 transferTax, uint16 sniperSellTax) public onlyOwner{
        uint16 totalTax=burnTaxes+liquidityTaxes+stakingTaxes;
        ...
        _buyTax=buyTax;
        _sellTax=sellTax;
        _sniperSellTax = sniperSellTax;
        _transferTax=transferTax;
    }

    //How much of the staking tax should be allocated for marketing
    function TeamChangeMarketingShare(uint16 newShare) public onlyOwner{
        require(newShare<=80); 
