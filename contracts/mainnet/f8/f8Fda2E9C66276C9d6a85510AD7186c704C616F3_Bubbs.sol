...
contract Bubbs is IERC2981, ERC721A, Ownable {
  ...
  IERC20 public payableToken =
    IERC20(0x5774B2fc3e91aF89f89141EacF76545e74265982);
  ...
  function updatePresale(bool _open, bytes32 root) external onlyOwner {
    presaleOpen = _open;
    merkleRoot = root;
  }

  function updateReveal(bool _revealed, string memory _uri) external onlyOwner {
    revealed = _revealed;
  ...
  function updateRoyalties(address _recipient, uint256 _fee) external onlyOwner {
    ...
    onlyOwner
  {
    ...
    for (uint256 i = 0; i < _amount.length; i++) {
      require(_supply + _amount[i] <= _totalSupply, "reached max supply");
  ...
  function numberMintedOfOwner(address _address) external view returns (uint256) {
    ...
    returns (string memory)
  {
    ...
    string memory base = revealed ? baseUri : uriNotRevealed;
