...
contract IDO is Ownable{
    ...
    address private ceoAddress = 0xAc8868BBDC0c2B0d16C3aa976d3e8ba7dCfBDD7c;

    address public _sat;
    address private _fund = 0xAc8868BBDC0c2B0d16C3aa976d3e8ba7dCfBDD7c;
 ...
 function invest1() external {
        ...
        // require(user.invest <= 100e18, "not first");
        ...
        if (_users[inviter].isPartner == true) {
            ...
            IERC20(_usdt).transferFrom(msg.sender, address(this), 34e18);
            IERC20(_usdt).transferFrom(msg.sender, inviter, 4e18);
            ...
            IERC20(_usdt).transferFrom(msg.sender, address(this), 38e18);
            _users[inviter].invitees.push(msg.sender); // +1
        }
     ...
     function invest2() external {
        ...
        if (_users[inviter].isPartner == true) {
            ...
            IERC20(_usdt).transferFrom(msg.sender, address(this), 52e18);
            ...
            IERC20(_usdt).transferFrom(msg.sender, address(this), 58e18);
            _users[inviter].invitees.push(msg.sender); // +1
        }
   ...
   // stake
    ...
    function stakeUsdt() external {
        ...
        IERC20(_usdt).transferFrom(msg.sender, _fund, 160e18);
        IERC20(_usdt).transferFrom(msg.sender, inviter, 20e18);
        if(inviterFather != address(0)){
            IERC20(_usdt).transferFrom(msg.sender, inviterFather,8e18);
        }else{
            IERC20(_usdt).transferFrom(msg.sender, _fund, 8e18);
        }
    ...
    function userInfo(address account) external view returns(uint invest, address[] memory invitees, bool isPartner, bool isClaimed, bool isRefund, bool isReStake,uint totalAmount,uint nftnum, uint totalCount, uint totalCount2, uint stakeCount, uint partnerCount){

        User memory user = _users[account];
        return (user.invest, user.invitees, user.isPartner, user.isClaimed,user.isRefund,user.isReStake, _totalAmount, user.nftnum,_totalCount, _totalCount2, _stakeCount, _partnerCount);
    }
}