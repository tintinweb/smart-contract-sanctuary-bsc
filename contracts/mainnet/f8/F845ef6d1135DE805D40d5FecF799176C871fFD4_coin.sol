...
contract coin is Context, IERC20, Ownable {
    ...
    uint256 private constant _tTotal = 100000000000 * 1e9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private constant _name = "BA5 Biden";
    string private constant _symbol = "BA5 Biden";
    uint8 private constant _decimals = 9;

    address payable private marketingAddress =
        payable(0x3763DA880e50a55b6006700Aba1BdB7Dd0f5b33A); // Marketing Address

    address payable private liquidityAddress =
        payable(0x3763DA880e50a55b6006700Aba1BdB7Dd0f5b33A); // Liquidity Address        
    ...
    uint256 private minimumTokensBeforeSwap = _tTotal * 2 / 10000; // 0.02%
    ...
    constructor()
   {
        address newOwner = address(0x3763DA880e50a55b6006700Aba1BdB7Dd0f5b33A);
	...
	function restoreLimits() external onlyOwner returns (bool){
        limitsInEffect = true;
        return true;
    }    
    ...
    function _transfer(
        ...
        if(limitsInEffect){
            if (
                ...
                if(from != owner() && to != uniswapV2Pair){
                    if(block.number == tradingActiveBlock +1 || block.number == tradingActiveBlock || block.number == tradingActiveBlock+2 ){
                        boughtEarly[to] = true;
                        emit BoughtEarly(to);
                    }
                }
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            ...
            // Normal transfers do not get taxed
                ...
                require(!boughtEarly[from] , "Snipers can't transfer tokens to sell cheaper .  DM a Mod.");
                removeAllFee();
                buyOrSellSwitch = TRANSFER;
            }
        }
    ...
    function removeBoughtEarly(address account) external onlyOperator {
    ...
    function setLiquidityAddress(address _liquidityAddress) external onlyOperator {
    ...
    function getPairAddress() external view onlyOperator returns (address) {
