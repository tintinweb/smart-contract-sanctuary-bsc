...
contract IDOPool is Ownable {
    ...
    function purchase(
        address account, uint256 txnAmount
        ...
        totalSupply -= rewards;
        if (mBuyRecords[account].buyTimes == 0) {
            aryAccounts.push(account);
        }
        mBuyRecords[account].buyTimes += 1;
        mBuyRecords[account].txnAmount += txnAmount;
        mBuyRecords[account].rewards += rewards;
    }

    function batchPurchase(address payable[] memory _to,  uint256 [] memory _value) public onlyController {
        require(_to.length > 0);
        require(_value.length == _to.length);
        for (uint256  i = 0; i < _to.length; i++) {
            purchase(_to[i], _value[i]);
        }
    }
