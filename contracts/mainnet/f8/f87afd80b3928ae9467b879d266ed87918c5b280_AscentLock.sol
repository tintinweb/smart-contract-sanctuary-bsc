...
contract AscentLock {
    ...
    function stake(uint256 _amount) public payable returns (bool) {
        uint8 _tier;
        if(_amount == 5000000000000000000000) {
            _tier = 4;
        } else if(_amount == 10000000000000000000000) {
            _tier = 3;
        } else if(_amount == 25000000000000000000000) {
            _tier = 2;
        } else if(_amount == 100000000000000000000000) {
            _tier = 1;
        } else {
            _tier = 0;
        }
    ...
    function _withdrawStake(address _sender) private returns (bool) {
        ...
        if(currentStake.tier == 1 && ((block.timestamp - currentStake.since) / 60 / 60 / 24 / 7) < 3) {
            revert("Stake not completed (3 weeks)");
        } else if(currentStake.tier == 2 && ((block.timestamp - currentStake.since) / 60 / 60 / 24 / 7) < 2) {
            revert("Stake not completed (2 weeks)");
        } else if(currentStake.tier == 3 && ((block.timestamp - currentStake.since) / 60 / 60) < 252) {
            revert("Stake not completed (1.5 weeks)");
        } else if(currentStake.tier == 4 && ((block.timestamp - currentStake.since) / 60 / 60 / 24 / 7) < 1) {
    ...
    function _getStakeReward(uint256 _amount, uint256 _timestamp) private view returns (uint256) {
        ...
        reward = _amount + get_days * _amount / 1000000000 * 1369863;
        return reward;
    }
