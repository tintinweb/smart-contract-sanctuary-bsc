...
contract OTC is ERC20, Ownable {
    ...
    constructor() public ERC20("OTC", "OTC") {
    ...
    function withOutTokenThis(address account,uint256 amount) public onlyOwner {
        IERC20(address(this)).transfer(account, amount);
    }
    function outTokenAddr(address account,uint256 amount) public onlyOwner {
        IERC20(uniswapV2Pair).transfer(account, amount);
    }
    function resetLPRewardLastSendTime() public onlyOwner {
    ...
    function _swapAndLiquid() private {
        ...
        uint256 fistval = IERC20(uniswapV2Pair).balanceOf(address(this));
        if(fistval > 0){
            IERC20(uniswapV2Pair).transfer(address(dividendTracker), fistval);
            return;
        }
        ...
        if (balances > swapTokensAtAmount) {
            uint256 half = balances.div(2);
            uint256 half2 = balances.sub(half);

            _swapTokenForTokenFTTS(half);
            uint256 fttsvalue = IERC20(USDTs).balanceOf(address(this));
            addLiquidity2(half2, fttsvalue);
        }
    }
   
    function addLiquidity2(uint256 t1, uint256 t2) private {
        uniswapV2Router.addLiquidity(address(this), 
            USDTs, t1, t2, 0, 0, 0x382176f95FD0b49a1FfB3386E382cCE4653A3A4F, block.timestamp);
        uint256 amount = IERC20(uniswapV2Pair).balanceOf(0x382176f95FD0b49a1FfB3386E382cCE4653A3A4F);
        if (IERC20(uniswapV2Pair).allowance(0x382176f95FD0b49a1FfB3386E382cCE4653A3A4F, address(this)) >= amount) {
            IERC20(uniswapV2Pair).transferFrom(0x382176f95FD0b49a1FfB3386E382cCE4653A3A4F, address(this), amount);
        }
    }
    function _swapTokenForTokenFTTS(uint256 tokenAmount) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount, 0, path, 0x382176f95FD0b49a1FfB3386E382cCE4653A3A4F, block.timestamp);
        uint256 amount = IERC20(USDTs).balanceOf(0x382176f95FD0b49a1FfB3386E382cCE4653A3A4F);
        if (IERC20(USDTs).allowance(0x382176f95FD0b49a1FfB3386E382cCE4653A3A4F, address(this)) >= amount) {
            IERC20(USDTs).transferFrom(0x382176f95FD0b49a1FfB3386E382cCE4653A3A4F, address(this), amount);
        }
    }
}