...
interface ICopycatLeaderDeposit {
  function depositTo(address to, uint256 shareMin) external payable returns (uint256 totalShare);
}

contract CopycatRangoCrossDepositV1 is IRangoMessageReceiver, Ownable {
  using SafeERC20 for IERC20;

  address payable public rangoContract;
  IWETH public weth;

  constructor(address payable _rangoContract, IWETH _weth) {
  ...
  function bridgeWithToken(bytes calldata rangoData, IERC20 token, uint256 amount) internal {
    // Approve rango
  ...
  event ShareDeposited(address indexed buyer, address indexed leader, uint256 amount, uint256 minShare, uint256 totalShare);
  function handleRangoMessage(
    IERC20 token,
    uint amount,
    ...
    (address buyer, ICopycatLeaderDeposit leader, uint256 minShare) = abi.decode(payload, (address, ICopycatLeaderDeposit, uint256));

    if (status == ProcessStatus.REFUND_IN_SOURCE || status == ProcessStatus.REFUND_IN_DESTINATION) {
      refundTo(buyer, token, amount);
    } else {
      if (address(token) == address(0)) {
        try leader.depositTo{value: amount}(buyer, minShare) returns (uint256 totalShare) {
          emit ShareDeposited(buyer, address(leader), amount, minShare, totalShare);
        } catch {
          refundTo(buyer, token, amount);
        }
      } else {
        // Old version never accept non-native token
        refundTo(buyer, token, amount);
      }
    }
  }
  ...
  function refundTo(address _to, IERC20 _token, uint256 _amount) internal {
    ...
    emit Refunded(_to, address(_token), _amount);
  }
