...
contract Xpot is Ownable, ReentrancyGuard, Pausable {
    ...
    constructor(
        ...
        uint256 decimalStakedtoken = uint256(stakedToken.decimals());

        PRECISION_FACTOR = uint256(10**(uint256(30).sub(decimalStakedtoken)));
    ...
    function deposit(uint256 _amount) external nonReentrant whenNotPaused onlyEOA {
        ...
        if (user.amount != 0) {
            ...
            if (pending != 0) stakedToken.safeTransfer(msg.sender, pending);
        }
        ...
        if (_amount != 0) {
            ...
            if (stakeFee != 0) {
                stakeFeeAmount = _amount.mul(stakeFee).div(10000);

                stakedToken.safeTransfer(treasury, stakeFeeAmount.mul(20).div(100));
                stakedToken.safeTransfer(jackPot, stakeFeeAmount.sub(stakeFeeAmount.mul(20).div(100)).sub(stakeFeeAmount.mul(60).div(100)));
                _amount = _amount.sub(stakeFeeAmount);
            }
            user.amount = user.amount.add(_amount);
            totalStaked = totalStaked.add(_amount);

            if (stakeFee != 0) {
                _updatePool(stakeFeeAmount.mul(60).div(100));
            }
    ...
    function withdraw(uint256 _amount) external nonReentrant whenNotPaused onlyEOA {
        ...
        if (_amount != 0) {
            ...
            userShares = uint256(1e18).mul(_amount).div(totalStaked).mul(_shares).div(1e18);
            uint256 preBalance = stakedToken.balanceOf(address(this));

            cakeVault.withdraw(userShares);

            uint256 tokenBal = stakedToken.balanceOf(address(this)).sub(preBalance);

            if (tokenBal != 0) {
                uint256 withdrawFee;

                if (unstakeFee != 0) {
                    withdrawFee = tokenBal.mul(unstakeFee).div(10000);
                    _amount = _amount.sub(withdrawFee);
                    tokenBal = tokenBal.sub(withdrawFee);
                    stakedToken.safeTransfer(treasury, withdrawFee);
                }

                user.amount = user.amount.sub(_amount).sub(withdrawFee);
                totalStaked = totalStaked.sub(_amount).sub(withdrawFee);
    ...
    function pendingAmount(address _user) external view returns (uint256) {
        ...
        (uint256 _shares, , , , , , , , ) = cakeVault.userInfo(address(this));
        return _shares.mul(cakeVault.getPricePerFullShare()).mul(user.amount).div(totalStaked).div(1e18);
    }
    ...
    function _updatePool(uint256 _rewardAmount) internal {
        ...
        accTokenPerShare = accTokenPerShare.add(_rewardAmount.mul(PRECISION_FACTOR).div(totalStaked));
    }
}
