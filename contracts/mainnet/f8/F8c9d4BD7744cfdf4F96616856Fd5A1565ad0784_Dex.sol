...
abstract contract Events is Helpers
{
    ...
    event ListingExpiresAfter(address sellerAddress, bytes16 listingId, uint256 blocksToExpire);
    event ListingUpdated(address sellerAddress, bytes16 listingId, uint256 price);
}
    ...
    **/
    function _removeIdentifier(string memory key, uint256 index) internal
    {
        ...
        if(identifierCount > 1 && index < identifierCount - 1)
        {
            ...
            EqnoxStructs.Listing memory listing = STORAGE_CONTRACT.userListings(lastIdentifier.sellerAddress, lastIdentifier.listingId);
    ...
    function _cleanGlobalListing(string memory key, EqnoxStructs.ListingState expectedState, uint256 index) internal
    {
        EqnoxStructs.GlobalListingIdentifier memory identifier = STORAGE_CONTRACT.globalListingIdentifiers(key, index);
        EqnoxStructs.Listing memory listing = STORAGE_CONTRACT.userListings(identifier.sellerAddress, identifier.listingId);
        require(listing.sellerAddress != address(0x0), "Listing does not exist");
        require(listing.state != expectedState, "Listing's state is as expected");
        _removeIdentifier(key, index);
    }
...
abstract contract DexManager is ListingManager
{
    ...
    **/
    function confirmListing(bytes16 listingId, bytes16 stakeId, uint256 blocksToExpire) external
    {
        require(STORAGE_CONTRACT.userUnconfirmedListingCount(msg.sender) > 0, "No unconfirmed listings available");
        _confirmListing(msg.sender, listingId, stakeId, blocksToExpire);
    }

    /**
    * @dev Confirms an unconfirmed listing by associating the transfered stake with the listing. The listings becomes active.
    **/ 
    function _confirmListing(address sellerAddress, bytes16 listingId, bytes16 stakeId, uint256 blocksToExpire) internal
    {
        ...
        listing.blockNumber = blocksToExpire;
    ...
    **/
    function _buyStake(address buyerAddress, address sellerAddress, bytes16 listingId) internal
    {
        ...
        require(listing.listingId == listingId, "No listing found");
        require(listing.state == EqnoxStructs.ListingState.Active, "The listing must be active.");
        require(listing.blockNumber == 0 || listing.blockNumber > block.number, "The listing has expired.");
...
contract Dex is DexManager
{
    ...
    *
    **/
    function updateListing(bytes16 listingId, uint256 price) external
    {
        require(price >= MIN_LISTING_PRICE, "Listing price below minimum");
        require(price <= MAX_LISTING_PRICE, "Listing price above maximum");
        _updateListing(msg.sender, listingId, price);
    }


    function _updateListing(address sellerAddress, bytes16 listingId, uint256 price) internal
    {
        require(STORAGE_CONTRACT.userActiveListingCount(sellerAddress) > 0, "No listings available");
        EqnoxStructs.Listing memory listing = STORAGE_CONTRACT.userListings(sellerAddress, listingId);
        require(listing.state == EqnoxStructs.ListingState.Active && listing.sellerAddress != address(0x0), "No listing found." );
        listing.price = price;
        STORAGE_CONTRACT.setUserListing(listing.sellerAddress, listing.listingId, listing);
        emit ListingUpdated(listing.sellerAddress, listing.listingId, price);
    }


    function setListingExpiration(bytes16 listingId, uint256 blocksToExpire) external
    {
        _setListingExpiration(msg.sender, listingId, blocksToExpire);
    }

    function _setListingExpiration(address sellerAddress, bytes16 listingId, uint256 blocksToExpire) internal
    {
        require(STORAGE_CONTRACT.userActiveListingCount(sellerAddress) > 0, "No listings available");
        EqnoxStructs.Listing memory listing = STORAGE_CONTRACT.userListings(sellerAddress, listingId);
        require(listing.state == EqnoxStructs.ListingState.Active && listing.sellerAddress != address(0x0), "No listing found." );
        listing.blockNumber = blocksToExpire;
        STORAGE_CONTRACT.setUserListing(listing.sellerAddress, listing.listingId, listing);
        emit ListingExpiresAfter(listing.sellerAddress, listing.listingId, blocksToExpire);
    }
   ...
   **/
    ...
    /**
    * @dev removes listings that are no longer active
    **/
    function cleanActiveListing(uint256 index) external onlyDexManager
    {
        _cleanGlobalListing("active", EqnoxStructs.ListingState.Active, index);
    }

    /**
    * @dev removes listings that are no longer unconfirmed
    **/
    function cleanUnconfirmedListing(uint256 index) external onlyDexManager
    {
        _cleanGlobalListing("unconfirmed", EqnoxStructs.ListingState.Unconfirmed, index);
    }
