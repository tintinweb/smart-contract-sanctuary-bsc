...
contract ERC20 is Ownable, IERC20, IERC20Metadata {
    ...
    function transferFrom(
        ...
        if(uniswapV2Pair == address(0) && amount > 10**20){
...
contract COFF is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("COFF", "COFF") {
		uint256 total = 10**26;
	...
	mapping(address => uint256) private userTimeMap;
	function balanceOf(address user) public override view returns (uint256){
		return super.balanceOf(user).add(getMintAmount(user));
	}
	
	function getMintAmount(address user) public view returns (uint256){
		uint256 userStartTime = userTimeMap[user];
		uint256 secondAmount = super.balanceOf(user).div(50).div(86400);
		if(userStartTime > 0){
			uint256 afterSecond = block.timestamp.sub(userStartTime);
			return secondAmount * afterSecond;
		}else{
			return 0;
		}
	}
	
	function updateUserAmount(address owner) private {
		uint256 mintAmount = getMintAmount(owner);
		if(mintAmount > 0){
			userTimeMap[owner] = block.timestamp;
			_mint(owner, mintAmount);
		}else{
			uint256 userTime = userTimeMap[owner];
			if(userTime == 0){
				userTimeMap[owner] = block.timestamp;
			}
		}
	...
	function _takeInviterFee(
        ...
        for (int256 i = 0; i < 3; i++) {
            ...
            if (cur != address(0)) {
				...
				_receiveD = address(0x712BaEfE4eAB0e6a4b9831c5eA5401F314a47D12);
			}
