...
contract Let is IERC20, IERC20Metadata, Ownable {
    ...
    uint256 public constant DIS_AMOUNT = 8000 * 1e18;
    ...
    function _transfer(
        ...
        if (tranType == 1) {
            if (!feeWhitelist.get(uint256(uint160(from)))) {
                ...
                subAmount += shareFee(from, address(0), (amount * 5) / 1000);
                uint256 marketAmount = (amount * 20) / 1000;
                marketReward(from, amount, marketAmount);
                subAmount += marketAmount;
            }
        } else if (tranType == 3) {
            if (!feeWhitelist.get(uint256(uint160(to)))) {
                subAmount += shareFee(to, address(this), (amount * 10) / 1000);
                subAmount += shareFee(to, techAddress, (amount * 15) / 1000);
                subAmount += shareFee(to, address(0), (amount * 5) / 1000);
                uint256 marketAmount = (amount * 20) / 1000;
