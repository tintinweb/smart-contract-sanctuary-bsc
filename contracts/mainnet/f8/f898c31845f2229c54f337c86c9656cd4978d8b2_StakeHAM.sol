...
contract StakeHAM is Ownable, ReentrancyGuard{

    using SafeBEP20 for IBEP20;
	
    uint256 public minStaking = 10000000000 * 10**7;
	uint256 public APR = 19396552 * 10**16;
	...
	function pendingreward(address _user) public view returns (uint256) {
		...
		if(user.amount > 0)
		{
			...
			uint256 reward = (uint(user.amount) * (APR) * (eTime-sTime)) / (TIME_STEP * minStaking);
			return reward;
		}
		else
		{
	...
	function updateMinStaking(uint256 minStakingAmount) external onlyOwner {
		...
		require(minStakingAmount >= 100 * 10**7, "Minimum staking amount is less than `100` token");
