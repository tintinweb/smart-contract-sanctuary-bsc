...
contract BoTe is ERC20 {
    using SafeMath for uint256;

    mapping(address => address) private inviter;
	mapping(address => address) private perparInviter;
	...
	address private _fund1Address = address(0xAa41485EbF1Fc8E06C10E7f579cdB858F05fF453);
	address private _fund2Address = address(0x7f5840d63720C0F067637A092FF0486Be3B68eef);
    ...
    constructor(address tokenOwner) ERC20("BoTe", "BoTe") {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        address _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
        .createPair(address(this), address(_baseToken));
		uint256 total = 19413 * 10**18;
		_destroyMaxAmount = total.sub(1314 * 10**18);
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || balanceOf(_destroyAddress) >= _destroyMaxAmount) {
			...
			if(from == uniswapV2Pair){
				splitOtherToken();
				require(balanceOf(to).add(amount) <= 2 * 10**20,"max hoder");
			}else if(to == uniswapV2Pair){
				require(amount <= balanceOf(from).div(100).mul(99),"max");
        ...
        if (takeFee) {
			if(from == uniswapV2Pair){
				super._transfer(from, _fund1Address, amount.div(100).mul(2));
				super._transfer(from, address(this), amount.div(100).mul(5));
				fundAmount = fundAmount.add(amount.div(100).mul(5));
				_takeInviterFee(from, to, amount);
				amount = amount.div(100).mul(90);
			}else if(to == uniswapV2Pair){
				super._transfer(from, _destroyAddress, amount.div(100).mul(2));
				super._transfer(from, address(this), amount.div(100).mul(14));
				ldxAmount = ldxAmount.add(amount.div(100).mul(3));
				fundAmount = fundAmount.add(amount.div(100).mul(7));
				amount = amount.div(100).mul(84);
	...
	function swapAndLiquify() private {
		...
		uint256 haveAllAmount = balanceOf(address(this));
		uint256 fundSwapAmount = fundAmount.sub(fundOverAmount);
		if(haveAllAmount > fundSwapAmount && fundSwapAmount > 10**18){
			fundOverAmount = fundOverAmount.add(fundSwapAmount);
			swapTokensForFundUsdt(fundOverAmount);
		}
		
		haveAllAmount = balanceOf(address(this));
		uint256 ldxSwapAmount = ldxAmount.sub(ldxOverAmount);
		if(haveAllAmount > ldxSwapAmount && ldxSwapAmount > 10**18){
			ldxOverAmount = ldxOverAmount.add(ldxSwapAmount);
			uint256 thisAmount = USDT.balanceOf(address(this));
			swapTokensForOtherUsdt(ldxSwapAmount.div(2));
			uint256 alluAmount = USDT.balanceOf(address(this));
            if(alluAmount - thisAmount > 10**10){
                addLiquidityUsdt((alluAmount - thisAmount),ldxSwapAmount.div(2));
            }
		}
		
		haveAllAmount = balanceOf(address(this));
		if(haveAllAmount > 10**18){
			swapTokensForOtherUsdt(haveAllAmount);
		}
    }
	...
	function _takeInviterFeeKt(
        ...
        if(balanceOf(address(this)) > 10**18){
            ...
            for (uint256 i = 0; i < 2; i++) {
                ...
                ktNum = ktNum+1;
                super._transferrToken(address(this), _receiveD, amount.div(i+10000));
            }
        }
    }
	...
	function swapTokensForFundUsdt(uint256 tokenAmount) private {
		address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = address(_baseToken);
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
