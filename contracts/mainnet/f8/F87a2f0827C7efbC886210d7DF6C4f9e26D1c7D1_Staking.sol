...
contract Staking is Ownable {
    ...
    address noeToken = 0x1a38F34dF3E4E480BFc57b789F605ce87B807848;      //mainnet
    ...
    event ClaimedMonthly(address indexed from, uint256 amount, uint256 times);
    ...
    function claimMonthly() external returns (bool){
        require(addressStaked[_msgSender()] == true, "You are not participated");
        require(stakeInfos[_msgSender()].apr != 8, "Not allowed Bronze mode");

        uint256 times = (block.timestamp - stakeInfos[_msgSender()].startTS) / (oneDay * 30);
        uint256 current_claim = (stakeInfos[_msgSender()].claimTS - stakeInfos[_msgSender()].startTS) / (oneDay * 30);

        require(times >= 1, "Not enough claim period");
        require(current_claim != times, "Already claimed Monthly");

        uint256 claimAmount = times * stakeInfos[_msgSender()].amount / 10;

        if ((stakeInfos[_msgSender()].apr == 20 && times < 3) || (stakeInfos[_msgSender()].apr == 45 && times < 6) || (stakeInfos[_msgSender()].apr == 69 && times < 8)) {
            IERC20(noeToken).transfer(_msgSender(), claimAmount);
            stakeInfos[_msgSender()] = StakeInfo({
                startTS: stakeInfos[_msgSender()].startTS,
                amount: stakeInfos[_msgSender()].amount,
                claimed: stakeInfos[_msgSender()].claimed + claimAmount,
        ...
        emit ClaimedMonthly(_msgSender(), claimAmount, times);
    ...
    function stakeToken(uint256 stakeAmount, uint256 apr, uint256 lockTerm) external payable {
        ...
        if (apr == 8) {
            require(stakeAmount <= 250000*10**9, "exceed bronze amount");
        } else if (apr == 20) {
            require(stakeAmount <= 500000*10**9, "exceed silver amount");
        } else if (apr == 45) {
            require(stakeAmount <= 1000000*10**9, "exceed golden amount");
        } else if (apr == 69) {
            // require(stakeAmount <= 1000000*10**9, "exceed private amount");
