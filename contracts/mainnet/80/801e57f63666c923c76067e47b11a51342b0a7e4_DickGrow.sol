...
Telegram Portal  :
https://t.me/DickGrowBscPortal

Twitter Handle :
https://twitter.com/DickGrowBsc
...
contract DickGrow is Context, IERC20, Ownable {
	...
	string private _name = " Dick Grow";
    ...
    uint256 private _previousbuyBackFee = SetFee;
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        transferToAddressETH(marketingFeeReceiver, transferredBalance.div(SetFee).mul(marketingFee));
    ...
    function calculatebuyBackFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(SetFee).div(
            10**2
        );
    }

        function removeAllFee() private {
        if(taxFee == 0 && SetFee == 0) return;
        
        _previousDevelopmentFee = taxFee;
        _previousbuyBackFee = SetFee;
	...
	function setFees(uint256 _SetFee) external onlyOwner {
        SetFee = _SetFee;
