...
contract LUNT is Context, IERC20, Ownable {
    ...
    bool private minimumHolding = false;
    ...
    string private _name = "LunaticV2";
    string private _symbol = "LUNT";
    uint8 private _decimals = 18;
    
    //Buy Fees
    uint256 public _buyTaxFee = 4;
    uint256 public _buyLiquidityFee = 2;
    uint256 public _buyCharityFee = 4;
    ...
    address payable public charityWallet = payable(0xD5C92B59FA4e7a57852ac353F3b78F052321675E);
    address payable public marketingWallet = payable(0x43A8Db16791a564a92Fe88b649bDc233c7298035);
    ...
    uint256 public minimumHoldingForReflection = 100000 * (10**18);
    ...
    function _transfer(
        ...
        if(minimumHolding){
        if(balanceOf(to)< minimumHoldingForReflection) {
            _isExcluded[to] = true;
        } else if(balanceOf(to) >= minimumHoldingForReflection){
            require(_isExcluded[to] = true, "Already included in reward");
            includeInReward(to);
        }
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 tokensForLiquidity = contractTokenBalance.mul(_liquidityFee).div(_charityFee.add(_liquidityFee).add(_marketingFee)); 

        uint256 tokensForDev = contractTokenBalance.mul(_charityFee).div(_liquidityFee.add(_marketingFee).add(_charityFee));
        
        uint256 tokensForMarketing = contractTokenBalance.mul(_marketingFee).div(_liquidityFee.add(_marketingFee).add(_charityFee));
        ...
        swapTokensForWallets(tokensForDev, charityWallet);
    }
    ...
    returns (uint256, uint256)
    {
        ...
        uint256 tDev = tAmount.div(100).mul(_charityFee);
    ...
    function setBuyFeePercent(uint256 taxFee, uint256 MarketingFee, uint256 liquidityFee, uint256 charityFee) external onlyOwner() {
        _buyTaxFee = taxFee;
        _buyMarketingFee = MarketingFee;
        _buyLiquidityFee = liquidityFee;
        _buyCharityFee = charityFee;
        require(_buyTaxFee + _buyCharityFee + _buyLiquidityFee + _buyMarketingFee <= 20, "Must keep at 20% or less");
    }

    function setSellFeePercent(uint256 taxFee, uint256 MarketingFee, uint256 liquidityFee, uint256 charityFee) external onlyOwner() {
        _sellTaxFee = taxFee;
        _sellMarketingFee = MarketingFee;
        _sellLiquidityFee = liquidityFee;
        _sellCharityFee = charityFee;
        require(_sellTaxFee + _sellCharityFee + _sellLiquidityFee + _sellMarketingFee<= 20, "Must keep at 20% or less");
    }
