...
contract STC_Token is BEP20, BEP20Burnable, Pausable, Ownable {

    address public signerAddress;

    event MintBySigner(address indexed to, uint amount);
    event SignerUpdated(address indexed newSignerAddress);
    
    constructor(address _signerAddress) BEP20("STC_TOKEN","STC"){
        signerAddress = _signerAddress;
    }

    modifier  _OnlySigner {
        require(signerAddress == msg.sender, "Only signer can mint!");
        _;
    }
    ...
    function mintBySigner(address to, uint amount) public _OnlySigner {
        require(amount > 0, "Amnt > 0");
        _mint(to, amount);
        emit MintBySigner(to, amount);
    }

    function updateSignerAddress(address newSigner) public onlyOwner{
        require(newSigner != address(0), "0 sig Addr");
        assembly{sstore(signerAddress.slot, newSigner)}
