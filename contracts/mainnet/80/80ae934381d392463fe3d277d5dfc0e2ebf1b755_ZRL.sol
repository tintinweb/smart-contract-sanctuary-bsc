...
contract TokenNameDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("TokenName_Dividend_Tracker", "TokenName_Dividend_Tracker", 9) {
    ...
    function withdrawDividend() public override {
        require(false, "TokenName_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TokenName contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "TokenName_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "TokenName_Dividend_Tracker: Cannot update claimWait to same value");
...
contract ZRL is BEP20, Ownable {
    using SafeMath for uint256;

    IPancakeswapV2Router02 public pancakeswapV2Router;
    address public immutable pancakeswapV2Pair;
    address public marketingWallet = 0xC83d522Da3a03d3eBD08242A6CBe4e822ca2383b;
    ...
    uint256 public swapTokensAtAmount = 9 * (10**11) * (10**9);
    uint256 public _BNBRewardsFeeBuy = 7;
    uint256 public _liquidityFeeBuy = 3;
    uint256 public _marketingFeeBuy = 2; 
    uint256 public _BNBRewardsFeeSell = 7;
    ...
    constructor() public BEP20("ZRL", "ZRL", 9) {
        
        totalFeesBuy = _BNBRewardsFeeBuy.add(_liquidityFeeBuy).add(_marketingFeeBuy);
        totalFeesSell = _BNBRewardsFeeSell.add(_liquidityFeeSell).add(_marketingFeeSell);
    ...
    //  IPancakeswapV2Router02 _pancakeswapV2Router = IPancakeswapV2Router02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
        ...
        tokenContract = IBEP20(address(this));
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        require(newDividendTracker.owner() == address(this), "The new dividend tracker must be owned by the TokenName token contract");
    ...
    function _transfer(
        ...
        if(
            ...
            uint256 sellTokens = contractTokenBalance.mul(_BNBRewardsFeeSell).div(totalFeesSell);
            swapAndSendDividends(sellTokens);
            // send lp
            uint256 lpTokens = contractTokenBalance.mul(_liquidityFeeSell).div(totalFeesSell);
    ...
    //     numTokenName = tokenContract.balanceOf(pancakeswapV2Pair);
    //     uint256 price;
    //     if(numWBNB == 0){
    //         price = 0;
    //     }
    //     else{
    //         price = numWBNB.mul(10**30).div(numTokenName);
    //     }
    ...
    function getLiquidnumTokenName() public view returns (uint256) {
        return numTokenName;
    }
    ...
    function addBot (address _address) internal {
        if(!botAddresses[_address]){
    ...
    function removeBotAddress (address _address) external onlyOwner {
       if(!botAddresses[_address]){
            botAddresses[_address] = false;
       }
    }
    
    function setFee(uint256 lpFeeOnBuy, uint256 maFeeOnBuy, uint256 reFeeOnBuy, uint256 lpFeeOnSell, uint256 maFeeOnSell, uint256 reFeeOnSell) public onlyOwner {
         _BNBRewardsFeeBuy = reFeeOnBuy;
         _liquidityFeeBuy = lpFeeOnBuy;
         _marketingFeeBuy = maFeeOnBuy; 
         _BNBRewardsFeeSell = reFeeOnSell;
         _liquidityFeeSell = lpFeeOnSell;
         _marketingFeeSell = maFeeOnSell;
         totalFeesBuy = _BNBRewardsFeeBuy.add(_liquidityFeeBuy).add(_marketingFeeBuy);
         totalFeesSell = _BNBRewardsFeeSell.add(_liquidityFeeSell).add(_marketingFeeSell);
    }
