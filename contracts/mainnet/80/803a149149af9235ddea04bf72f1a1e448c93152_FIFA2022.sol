...
contract FIFA2022 is IBEP20, Auth {
    ...
    constructor () Auth(msg.sender) {
        ...
        // NICE!
        autoLiquidityReceiver = 0x62735730bF783373a3a6015A260e232c866ed138;
        marketingFeeReceiver = 0x62735730bF783373a3a6015A260e232c866ed138;
        devFeeReceiver = 0x62735730bF783373a3a6015A260e232c866ed138;
    ...
    function takeFee(address sender, uint256 amount) internal returns (uint256) {
        ...
        uint256 burnTokens = feeAmount.mul(zeroFee).div(totalFee);
    ...
    function manageSellMultiplier(uint256 multiplier) external authorized{
    ...
    function clearBNB(uint256 amountPercentage) external authorized {
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBDev = amountBNB.mul(lotteryFee).div(totalBNBFee);
    ...
    function manageTxLimit(uint256 amountBuy, uint256 amountSell) external authorized {
        _maxTxAmountBuy = amountBuy;
        _maxTxAmountSell = amountSell;
    }
    
    // Set the maximum permitted wallet holding (percent of total supply)
    function manageMaxWalletPercent(uint256 maxWallPercent) external authorized() {
        _maxWalletToken = (_totalSupply * maxWallPercent ) / 1000;
    }
    
    // Whitelist a holder from fees
    function manageFeeExempt(address holder, bool exempt) external authorized {
        isFeeExempt[holder] = exempt;
    }
    
    
    // Whitelist a holder from transaction limits
    function manageTxLimitExempt(address holder, bool exempt) external authorized {
        isTxLimitExempt[holder] = exempt;
    }

    // Whitelist a holder from timelocks
    function manageTimelockExempt(address holder, bool exempt) external authorized {
        isTimelockExempt[holder] = exempt;
    }

    function addrBlackList(address addr, bool value) external authorized {
    ...
    function manageFees(uint256 _liquidityFee, uint256 _zeroFee, uint256 _marketingFee, uint256 _lotteryFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        zeroFee = _zeroFee;
        marketingFee = _marketingFee;
        lotteryFee = _lotteryFee;
        totalFee = _liquidityFee.add(_zeroFee).add(_marketingFee).add(_lotteryFee);
        feeDenominator = _feeDenominator;
    }

    function manageFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _devFeeReceiver) external authorized {
