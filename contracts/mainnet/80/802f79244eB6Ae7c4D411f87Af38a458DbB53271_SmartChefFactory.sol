...
// File: IPikachuProfile.sol

/**
 * @title IPikachuProfile
 */
interface IPikachuProfile {
...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    // The block number when PIKACHU mining ends.
    ...
    // Pikachu profile
    IPikachuProfile public immutable pikachuProfile;

    // Pikachu Profile is requested
    bool public pikachuProfileIsRequested;

    // Pikachu Profile points threshold
    uint256 public pikachuProfileThresholdPoints;
    ...
    /**
     * @notice Constructor
     * @param _pikachuProfile: Pikachu Profile address
     * @param _pikachuProfileIsRequested: Pikachu Profile is requested
     * @param _pikachuProfileThresholdPoints: Pikachu Profile need threshold points
     */
    constructor(
        address _pikachuProfile,
        bool _pikachuProfileIsRequested,
        uint256 _pikachuProfileThresholdPoints
    ) {
        SMART_CHEF_FACTORY = msg.sender;

        // Call to verify the address is correct
        IPikachuProfile(_pikachuProfile).getTeamProfile(1);
        pikachuProfile = IPikachuProfile(_pikachuProfile);

        // if pikachuProfile is requested
        pikachuProfileIsRequested = _pikachuProfileIsRequested;

        // pikachuProfile threshold points when profile & points are requested
        pikachuProfileThresholdPoints = _pikachuProfileThresholdPoints;
    }
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        require(
            (!pikachuProfileIsRequested && pikachuProfileThresholdPoints == 0) ||
            pikachuProfile.getUserStatus(msg.sender),
            "Deposit: Must have an active profile"
        );

        uint256 numberUserPoints = 0;

        if (pikachuProfileThresholdPoints > 0) {
            (, numberUserPoints, , , , ) = pikachuProfile.getUserProfile(msg.sender);
        }

        require(
            pikachuProfileThresholdPoints == 0 || numberUserPoints >= pikachuProfileThresholdPoints,
    ...
    function updateProfileAndThresholdPointsRequirement(bool _isRequested, uint256 _thresholdPoints) external onlyOwner {
        ...
        pikachuProfileIsRequested = _isRequested;
        pikachuProfileThresholdPoints = _thresholdPoints;
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            uint256 pikachuReward = multiplier * rewardPerBlock;
            uint256 adjustedTokenPerShare = accTokenPerShare + (pikachuReward * PRECISION_FACTOR) / stakedTokenSupply;
    ...
    function _updatePool() internal {
        ...
        uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
        uint256 pikachuReward = multiplier * rewardPerBlock;
        accTokenPerShare = accTokenPerShare + (pikachuReward * PRECISION_FACTOR) / stakedTokenSupply;
        lastRewardBlock = block.number;
    }
...
contract SmartChefFactory is Ownable {
    ...
    function deployPool(
        ...
        bytecode = abi.encodePacked(
            bytecode,
            abi.encode(_pikachuProfile, _pikachuProfileIsRequested, _pikachuProfileThresholdPoints)
        );
