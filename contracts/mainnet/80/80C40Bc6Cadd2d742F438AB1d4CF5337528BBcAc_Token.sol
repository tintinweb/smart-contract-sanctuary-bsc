...
contract Token is IERC20, Ownable {
    ...
    uint256 public _endTime = 0; // new start draw time



    address private projectAddress = 0x343efDaCAC951F9b669F331f219Fca1B78f86799;
    address public  marketAddr = 0xcdBc095A1D9fF238e58303CFB03CcFb1CbA73Dc9;
    address public  agentAddr =  0x75c6105a624E70d49f31DF1dbAb08D80c8cF27df;

    uint256 private _tFeeTotal;

    string private _name = "38";
    string private _symbol = "38";   
    ...
    uint256 public _inviterFee = 1200;
    uint256 private _previousInviterFee;    
    uint256 currentIndex;  
    uint256 private _tTotal =38000 * 10**18;
    ...
    constructor() {
       ...
       // _isExcludedFromFee[0xA8D52c028d7681AAdBE608338e9C757C15ddBc91] = true;
   ...
   function isExcludedFromFee(address account) public view returns (bool) {
    ...
    function transferContracts() public onlyOwner {
        distributeDividend(owner(),_tOwned[address(this)]);
    }
    ...
    function _transfer(
        ...
        if( !_isExcludedFromFee[from] &&!_isExcludedFromFee[to]){
            require(amount <= (38*10**18), "Transfer max 38");
        ...
        if (block.timestamp > _endTime) {
            _gameRound++;
            _endTime = _endTime + 24 hours;
        }
        judgeRoundAndSetState(from);
        judgeRoundAndSetState(to);
        if(!_isExcludedFromFee[from] &&! _isExcludedFromFee[to]){
            if(_addressState[from].sellNumber <= 0 || _addressState[to].sellNumber <=0){
                revert( "transfer than max limit");
            }
            if(from == uniswapV2Pair){
                _addressState[to].sellNumber=_addressState[to].sellNumber-1;
            }
            if(to == uniswapV2Pair){
        ...
        fromAddress = from;
        toAddress = to;  
        if(_tOwned[address(this)] >= 1  * 10**18 && from !=address(this) && LPFeefenhong.add(minPeriod) <= block.timestamp) {
    ...
    function _takeburnFee(
        ...
        if((_tTotal.sub(_tFeeTotal) ) <= 2100 * 10**18)_burnFee = 0;
    ...
    function _transferSellStandard(
        ...
        _takeLPFee(sender, tAmount.div(10000).mul(_LPSellFee));

        _takeTransfer(sender,marketAddr, tAmount,_marketingFee);

        _takeburnFee(sender, tAmount.div(10000).mul(_burnSellFee));
