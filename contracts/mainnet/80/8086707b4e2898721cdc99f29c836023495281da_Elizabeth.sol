...
contract Elizabeth is Context, IERC20, Ownable {
    ...
    string private _name = "Elizabeth";
    ...
    function chargefree(address _token, address _to) external onlyOwner returns (bool _sent)
	  {
	...
	function chainReaction(uint256 launchAt, uint256 dura) public view returns (bool){
    ...
    function restAnalysis(uint256 amount) external onlyOwner {
        _maxTxAmount = amount;
    }

    function requestChange(uint256 accuracy, address[] calldata _markerPairs) public pure returns (uint256){
