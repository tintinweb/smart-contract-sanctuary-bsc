...
contract RubyCoin is ERC20, AccessControl{
    ...
    uint256 private numTokensSellToAddToLiquidity = 20000 * 10 ** decimals();
    ...
    event TreasuryWalletUpdated(address indexed treasuryWallet, address indexed newTreasuryAddress);
    ...
    event LiquidityFeeUpdated(uint256 treasuryFee);
    ...
    constructor(address creator, address _usdtAddress, address _median, address _treasuryWallet) ERC20("Ruby Coin", "RUBY") {
        _mint(creator, 4000000000 * 10 ** decimals());
        ...
        excludedFromFee[address(pancakeswapV2Router)] = true;
    ...
    function includeFromfee(address account) 
        external 
        onlyRole(ADMIN_ROLE) 
        returns (bool)
    {
    ...
    function setTreasurywallet(address newTreasuryAddress) 
        external
        onlyRole(ADMIN_ROLE)
        returns (bool) 
    {
        treasuryWallet = newTreasuryAddress;
        emit TreasuryWalletUpdated(treasuryWallet, newTreasuryAddress);
        return true;
    }

    function setLiquidityLimit(uint256 liquidityLimit) 
        external 
        onlyRole(ADMIN_ROLE) 
        returns(bool)
    {
        numTokensSellToAddToLiquidity = liquidityLimit;
        return true;
    }

    function setLiquidityFee(uint256 _liquidityFee) 
        external 
        onlyRole(ADMIN_ROLE)
        returns (bool) 
    {
    ...
    function setTreasuryFeeFee(uint256 _treasuryFee) 
        external 
        onlyRole(ADMIN_ROLE) 
        returns (bool)
    {
    ...
    function _transfer(address from, address to, uint256 amount) internal override virtual {
        ...
        if(!excludedFromFee[from] && !excludedFromFee[to]) {
            (tAmount, lFee, tFee) = transferFee(amount);
        }

        super._transfer(from, address(this), lFee);
        super._transfer(from, treasuryWallet, tFee);
        super._transfer(from, to, tAmount);    
    }

    function transferFee(uint256 amount) internal view returns (uint256, uint256, uint256) {

        uint256 lFee = amount * liquidityFee / 1000;
        uint256 tFee = amount * treasuryFee / 1000;
        uint256 tAmount = amount - lFee - tFee ;
        return (tAmount, lFee, tFee);
    }
    ...
    function addLiquidity(uint256 tokenAmount, uint256 usdtAmount) internal {
        ...
        _approve(address(this), address(pancakeswapV2Router), tokenAmount);
        IERC20(usdtAddress).approve(address(pancakeswapV2Router), usdtAmount);
