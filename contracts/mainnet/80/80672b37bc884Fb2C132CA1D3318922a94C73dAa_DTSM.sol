...
contract DTSM is ERC20 {
	...
	address public _fundAddress = address(0x459eb6B3a60bf652415B9e7dB8Ff52dE609297B8);
	address public _luckAddress = address(0x3BA648319b949e0C62C2d3dF46e41b1Da0c57891);
    ...
    //0xC5647fC9aAa1eC8C673a11DeE8DD16a4f23fe141
    constructor(address tokenOwner) ERC20("DTSM", "DTSM") {
	...
	function changeSwapWarp(DTSMWarp _warp) public onlyOwner {
        warp = _warp;
	...
	function getLuckUserAmount(address user) public view returns(uint256){
    ...
    function _transfer(
        ...
        if(to == uniswapV2Pair){
			...
			if(isAddLdx && luckUserAmount[from] > 0){
		...
		bool isInviter = from != uniswapV2Pair && balanceOf(to) == 0 && inviter[to] == address(0) && amount >= 10**17;
		uint256 sellFee = 2;
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx) {
			...
			if(from == uniswapV2Pair){
				...
				require(balanceOf(from).sub(amount) >= luckUserAmount[from],"luck amount");
				if(startSwapTime.add(300) > block.timestamp){sellFee = 18;}
        ...
        if (takeFee) {
			require(swapStats);
			splitOtherToken();
			_takeInviterFee(from, to, amount);//1
			super._transfer(from, address(this), amount.div(100).mul(sellFee));
			amount = amount.div(100).mul(99-sellFee);
        }
        super._transfer(from, to, amount);
		if(isInviter) {
            inviter[to] = from;
