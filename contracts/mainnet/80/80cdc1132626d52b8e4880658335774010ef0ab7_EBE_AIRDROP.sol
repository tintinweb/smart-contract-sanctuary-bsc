...
contract EBE_AIRDROP is  Ownable, ReentrancyGuard {
    ...
    uint private  airdropPrice = 0.014 ether;                                         
    uint private  airdropTokensAmount = 2000000000000000000000;     
    uint private  salePrice = 100000;                               


    function airdrop() external payable nonReentrant() {
        require(msg.value==airdropPrice, "wrong BNB amount");
        ...
        payable(address(refer)).transfer(airdropPrice*referFee/100); 
    }
    ...
    function setAirdropPrice(uint _airdropPrice) external onlyOwner {
        airdropPrice=_airdropPrice;
    }
    function setAirdropTokenAmount(uint _airdropTokensAmount) external onlyOwner {
        airdropTokensAmount=_airdropTokensAmount;
    }
