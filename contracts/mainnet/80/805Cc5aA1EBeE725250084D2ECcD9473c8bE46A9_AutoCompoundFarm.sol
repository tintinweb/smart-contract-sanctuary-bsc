...
contract AutoCompoundFarm is Ownable, IERC20 {
    ...
    // Staking Token
    IERC20 public constant token = IERC20(0xB592BfF35a34EFe9C02Fe917c43F7adD9d48A957);
    ...
    // Swap Path
    address[] private path = [ MDB, router.WETH() ];
    ...
    constructor(
        ...
        require(yieldFarm_ != address(0), 'Zero Address');
    ...
    function deposit(address user, uint256 amount) external nonReentrant {
        ...
        // stake all LP tokens into the farm
        _stakeIntoFarm();
    }
    ...
    function withdraw(uint256 amount) public nonReentrant returns (uint256) {
        ...
        // withdraw amount from farm -- NOTE EARLY WITHDRAW FEES!!!!
        uint256 received = _withdrawFromFarm(sendAmount);
        ...
        emit Withdraw(msg.sender, received);
        return received;
    }
    ...
    function compound() public {
        ...
        IYieldFarm(yieldFarm).claimRewards();
    ...
    function _swapForLiquidity() internal {
        ...
        // swap half balance
        router.swapExactTokensForETHSupportingFeeOnTransferTokens(balance / 2, 0, path, address(this), block.timestamp + 100);
    }

    function _stakeIntoFarm() internal {
        uint balance = token.balanceOf(address(this));
        if (balance <= 10) {
            return;
        }
        token.approve(yieldFarm, balance);
        IYieldFarm(yieldFarm).stake(balance);
    }

    function _withdrawFromFarm(uint256 amount) internal returns (uint256) {

        uint before = token.balanceOf(address(this));
        IYieldFarm(yieldFarm).withdraw(amount);
    ...
    function _pairLiquidity() internal {
        ...
        // swap half mdb balance into bnb
        uint256 balance = IERC20(MDB).balanceOf(address(this));
        if (balance <= 1000 || address(this).balance <= 1000) {
            return;
        }
    ...
    /** Returns the number of LP Tokens currently staked in the farm */
    function balanceOf() public view returns (uint256) {
        return IERC20(yieldFarm).balanceOf(address(this));
    } 
