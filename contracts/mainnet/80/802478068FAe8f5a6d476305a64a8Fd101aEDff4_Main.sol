...
contract Main is ReentrancyGuardUpgradeable {
    ...
    function __WithdrawAssets(address receiver, address token) external {
        require(msg.sender == IConfig(config).feeRecipient(), "only feeReceipt");
        uint256 balance = IAsset(asset).balances(token);
        require(balance > 0, "no balance to withdraw");
        IAsset(asset).withdraw(payable(receiver), token, balance);
    }
