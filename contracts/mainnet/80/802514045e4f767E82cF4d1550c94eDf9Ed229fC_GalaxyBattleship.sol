...
contract GalaxyBattleship is Ownable {
    ...
    // address public aecAddress = 0x896650125A5303791f2003F635931b851D610191;
    // address public botAddress = 0x262AD514E58dBC2A4D36C861f5C0087fe22905b1;
    IBEP20 public aecToken = IBEP20(0x896650125A5303791f2003F635931b851D610191);
    IBEP20 public botToken = IBEP20(0x262AD514E58dBC2A4D36C861f5C0087fe22905b1);
    ...
    IPancakePair pancakePair = IPancakePair(0xeCcE32E5de6d5f83bAF5a8EB853484044157566B);

    uint256 public botPrice = 3000000000000000000;//BOT指导价
    uint256 public totalPower = 0;//总算力
    uint256 public dailyOutput = 336000000000000000000;//每日产量
    uint256 public minPower = 200000000000000000000;//最小算力标准
    ...
    //奖品分数
    uint256[] public prizeNumList = [0,0,0,0];
    //每份奖品的数量
    uint256[] public prizeAmountList = [500000000000000000,1000000000000000000,1500000000000000000,0];
    uint256 public botCostOfBox = 10000000000000000000;//每个盲盒要消耗的BOT数量
    ...
    function setAecAddress(address newAddress) external onlyOwner{
        aecToken = IBEP20(newAddress);
    }

    function setBotAddress(address newAddress) external onlyOwner{
        botToken = IBEP20(newAddress);
    }

    function setUsdtAddress(address newAddress) external onlyOwner{
        usdtToken = IBEP20(newAddress);
    }
    ...
    function setTotalPower(uint256 newPower) external onlyOwner{
        totalPower = newPower;
    }

    function setTimeLimit(uint256 newTimeLimit) external onlyOwner{
        takeTimeLimit = newTimeLimit;
    }



    function setUserInfo(address userAddress,bool isConf,uint256 myPower,uint256 myUsdtAmount,uint256 myAecAmount,uint256 mySoldAmount,uint256 myTakeTime,uint256 myTakeCount,address parent, uint256 profitAmount) external onlyOwner{
        _userMap[userAddress].isConf = isConf;
        _userMap[userAddress].myPower = myPower;
        _userMap[userAddress].myUsdtAmount = myUsdtAmount;
        _userMap[userAddress].myAecAmount = myAecAmount;
        _userMap[userAddress].mySoldAmount = mySoldAmount;
        _userMap[userAddress].myTakeTime = myTakeTime;
        _userMap[userAddress].myTakeCount = myTakeCount;
        _userMap[userAddress].parent = parent;
        _userMap[userAddress].profitAmount = profitAmount;
    }   
