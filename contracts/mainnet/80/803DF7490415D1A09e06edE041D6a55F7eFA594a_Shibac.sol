...
contract Shibac is Context, IBEP20, Ownable {
    ...
    function _transfer(
        ...
        if (!inSwap && swapEnabled && to == uniswapV2Pair) {
            ...
            // transfer tokens
            uint256 shibacBalance = balanceOf(address(this));
            if (shibacBalance > _minBalance) {
                transferTokens(shibacBalance);
            }
    ...
    /**
     * @dev Swap tokens from shibac to bnb
     */
    ...
    /**
     * @dev Add shibac token and bnb as same ratio on pancakeswap router
     */
    ...
    /**
     * @dev the Owner can swap regarding the shibac token's amount of contract balance
    ...
    /**
     * @dev the Owner can send regarding the shibac token's amount of contract balance
     * this is for manual function
