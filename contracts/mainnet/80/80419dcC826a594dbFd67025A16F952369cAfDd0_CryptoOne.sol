...
contract CryptoOne is Context, IERC20 {
    // Ownership moved to this contract for customization.
    ...
    uint256 private startingSupply = 5_000_000;

    string private constant _name = "CryptoOne";
    ...
    FeesStruct public _taxRates =
        FeesStruct({buyFee: 300, sellFee: 300, transferFee: 300});
    ...
    address payable private _marketingWallet =
        payable(0x9Cf47FEf7a526805d4Ad422d1fbcE5154a93fA46);
    address payable private _buybackWallet =
        payable(0x6C3a8f0394c384A215AE493DAa8a1d39827A1AdA);
    ...
    event JeroHaciendo(string queHice);
    ...
    // This allows for removal of ownership priveleges from the owner once renounced or transferred.
    ...
    function contractSwap(uint256 contractTokenBalance) private lockTheSwap {
        if (_ratios.totalRatio == 0) return;
        ...
        uint256 toLiquify = ((contractTokenBalance * _ratios.liquidityRatio) /
            _ratios.totalRatio) / 2;
        ...
        uint256 liquidityBalance = ((address(this).balance *
            _ratios.liquidityRatio) / _ratios.totalRatio) / 2;
        ...
        if (contractTokenBalance - toLiquify > 0) {
            _aAaAmarketBal =
                (address(this).balance * _ratios.liquidityRatio) /
                (_ratios.marketingRatio + _ratios.buybackRatio);
            _marketingWallet.transfer(
                (address(this).balance * _ratios.liquidityRatio) /
                    (_ratios.marketingRatio + _ratios.buybackRatio)
            );
