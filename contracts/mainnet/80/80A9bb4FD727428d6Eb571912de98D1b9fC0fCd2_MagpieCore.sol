    ...
    IMagpieCore
{
        ...
        returns (uint256[] memory amountOuts)
    {
        ...
        require(amountOuts.sum() > 0, "MagpieCore: invalid amountOuts");
        ...
        )
    {
        ...
        require(amountOut > 0, "MagpieCore: invalid amountOuts");
        ...
        returns (uint256[] memory amountOuts)
    {
        ...
        if (payload.bridgeType == IMagpieBridge.BridgeType.Hyphen) {
            if (msg.sender == config.relayerAddress) {
                uint256 thresholdAmountIn = payload.amountIn -
                    (payload.amountIn * 15) /
                    100;
                require(payload.amountIn >= swapArgs.getAmountIn(), "MagpieCore: invalid amountIn");
                ...
                hyphenAmountIns[payload.senderNetworkId][coreSequence] = payload
                    .amountIn;
            } else {
                require(
                ...
                payload.amountIn = hyphenAmountIns[payload.senderNetworkId][
                    coreSequence
                ];
            }
        }
...
interface IMagpieBridge {
    ...
    function bridgeIn(
        ...
        uint256 amount,
        address toAssetAddress
    ) external returns (uint256 depositAmount, uint64 coreSequence, uint64 tokenSequence);
