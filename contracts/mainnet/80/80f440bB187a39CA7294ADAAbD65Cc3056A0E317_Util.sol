...
abstract contract AbsUtil is Ownable, Context {
    ...
    function upgradeLevel1() external returns (bool) {
        ...
        bool transfer = _tokenTransfer(account, usdt, level1UsdtQuantity);
        if(transfer){
    ...
    function upgradeLevel2() external returns (bool) {
        address account = msg.sender;
        bool transfer = _tokenTransfer(account, usdt, level2UsdtQuantity);
        if(transfer){
    ...
    function _upgradeCommon(address user,uint256 tokenQuantity) private {
        _addTokenBalance(msg.sender, level1TokenQuantity);
        _addBoxBalance(msg.sender,1);
        ...
        if(_binders[inviter].length % 10 == 0){
    ...
    function _tokenTransfer(address user,address token, uint256 amount)  private returns (bool) {
        return IERC20(token).transferFrom(wallet,user, amount);
    }
    ...
    function seUsdt(address _usdt) external onlyOwner returns (bool){
        usdt = _usdt;
        return true;
    }
    ...
    function getToltalCount() external view returns (uint256) {
        return _totalCount;
    }
