...
contract ARA  is ERC20 {
    ...
    constructor() ERC20(" aurora", "ARA") {
        ...
        _tokenOwner = address(0xdBBD2390312Ff2C5a94921bF373F26b11913fE3C);
        fundAddress = address(0x7fe0aB8d40d9A7361ADc841cBcb42cf13A57E792);
        devAddress = address(0xBc5D4BE43f3a9E6A1f28a6277cbd6Df5b7B0c499);
    ...
    function setDevAddress(address payable newWallet) external onlyFunder{
    ...
    function _transfer(
        ...
        if((ammPairs[from]  || ammPairs[to]) && swapstatus == false){
            require( from == address(_tokenOwner) || to == address(_tokenOwner)|| from == address(fundAddress) || to == address(fundAddress)||from == address(_owner) || to == address(_owner)|| from == address(devAddress) || to == address(devAddress),"only _tokenOwner||_owner||fundAddress||devAddress");
        }
    ...
    function swapProc() public {
        ...
        if(balanceOf(address(this)) >= canSellAmount && canSellAmount >= amountT){
            ...
            uint256 fundAmount = UsdtBalance.mul(3).div(4);
            ...
            usdt.transferFrom(address(_tokenDistributor), devAddress, secfundamount);
