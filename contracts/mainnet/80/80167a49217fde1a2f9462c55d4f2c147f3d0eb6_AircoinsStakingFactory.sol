...
// File: contracts/AircoinsStaking.sol

pragma solidity 0.6.12;

contract AircoinsStakingInitializable is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;
    
    // The address of the Aircoins Observatory factory
    address public AIRCOINS_STAKING_FACTORY;
    ...
    // The block number when Aircoins mining ends.
    uint256 public bonusEndBlock;

    // The block number when Aircoins mining starts.
    ...
    // Aircoins tokens created per block.
    ...
    constructor() public {
        AIRCOINS_STAKING_FACTORY = msg.sender;
    }
    ...
    function initialize(
        ...
        require(msg.sender == AIRCOINS_STAKING_FACTORY, "Not factory");
    ...
    function withdrawFees() external onlyOwner {
        ...
        stakedToken.safeTransfer(msg.sender, feeAmount);
        fees = 0;  
...
// File: contracts/AircoinsStakingFactory.sol
...
contract AircoinsStakingFactory is Ownable {
    event NewAircoinsStakingContract(address indexed AircoinsStaking);
     ...
     * @param _withdrawalInterval: the withdrawal interval for stakedToken (if any, else 0)
     * @param _admin: admin address with ownership
     * @return address of new Aircoins Observatory contract
     */
    function deployPool(
        IBEP20 _stakedToken,
        IBEP20 _rewardToken,
        ...
        bytes memory bytecode = type(AircoinsStakingInitializable).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(_stakedToken, _rewardToken, _startBlock));
        address AircoinsStakingAddress;

        assembly {
            AircoinsStakingAddress := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }

        AircoinsStakingInitializable(AircoinsStakingAddress).initialize(
            _stakedToken,
            _rewardToken,
            _rewardPerBlock,
            _startBlock,
        ...
        emit NewAircoinsStakingContract(AircoinsStakingAddress);
    }
}