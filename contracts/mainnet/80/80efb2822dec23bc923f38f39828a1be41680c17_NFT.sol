...
contract NFT is ERC721, ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    uint256[] public minParity = [1500 ether, 5000 ether, 25000 ether, 100000 ether];
    uint256[] public salesToAchieveRank = [1500 ether, 5000 ether, 25000 ether, 50000 ether];
    ...
    // Legs
    mapping(address => address) public legAddressToAddress;
    ...
    function getDirectCommissions(address addr) public view returns (uint256) {
        uint256 weeklyDirectCustomersVolume = addressToWeeklyDirectCustomersVolume[addr];
        uint256 rank = rankOf(addr);
    ...
    function getIndirectCommissions(address addr) public view returns (uint256) {   
        bool hasActiveSubs = checkIfActiveSubscription(addr);
        ...
        returns (uint256[] memory)
    {
        ...
        weeklyLegsTurnover[0] = getTreeSum(leg1Address) + addressToLeftTurnover[rootAddress];
        weeklyLegsTurnover[1] = getTreeSum(leg2Address) + addressToRightTurnover[rootAddress];
    ...
    function getBinaryExtraBonus(address addr)
        public
        view
        returns (uint256)
    {
    ...
    function rankOf(address addr) public view returns (uint256) {
        ...
        if (sales >= salesToAchieveRank5 || parity >= minParity[3]) {
            ...
            rank = 3;
        } else if (sales >= salesToAchieveRank2 || isFounder[addr] || parity >= minParity[0]) {
            rank = 2;
        }
    ...
    function subscribe(uint256 tokenId) public nonReentrant {
        ...
        address sponsor = legAddressToAddress[msg.sender];
    ...
    function safeMint(
        ...
        if (referralAddress != address(0)) {
            if (leg1Addr == address(0)) {
                ...
                legAddressToAddress[to] = referralAddress;
    ...
    function setWeeklyEarnings() public onlyAdmin {   
        uint256 totalEarnings = getFoundersBonusAmount() + getChampionBonusAmount();
        ...
        for (uint256 i; i < addresses.length; i++) {
            ...
            uint256 earnings = getDirectCommissions(addr) + getIndirectCommissions(addr);
       ...
       for (uint256 i; i < addresses.length; i++) {
            ...
            if (hasChampionBonus(addr)) {
                addressToEarnings[addr] += getChampionBonusAmount() / (divider * champions);
            }

            if (isFounder[addr]) {
                addressToEarnings[addr] += getFoundersBonusAmount() / (divider * founders);
            }
        }
    ...
    function setDirectCommissions(address addr, uint256 divider) public onlyAdmin {   
        uint256 commissions = getDirectCommissions(addr);

        addressToEarnings[addr] += commissions / divider;
    }

    function setIndirectCommissions(address addr, uint256 divider) public onlyAdmin {   
        uint256[] memory weeklyLegsTurnover = getWeeklyLegsTurnover(addr);
        uint256 leftTurnover = weeklyLegsTurnover[0];
        uint256 rightTurnover = weeklyLegsTurnover[1];
    ...
    function setMinParity(
        uint256[] calldata _minParity
    ) public onlyAdmin { 
        ...
        onlyAdmin
    {
        uint256 subsPrice = tokenIdToPrice[tokenId];
        uint256 volume = (subsPrice * subscriptionPercentage) / 100;
        address sponsor = legAddressToAddress[addr];
    ...
    function setExtraBonus(uint256[] calldata _extraBonusMinAmount, uint256[] calldata _extraBonusPerc) external virtual onlyAdmin {
    ...
    function setFounderBonusPercentage(uint256 _founderBonusPercentage) external virtual onlyAdmin {
