...
contract FairLaunch is Ownable {
    ...
    uint256 private constant EXCHANGE_RATE = 5; // 10 asset -> 1 CST

    uint256 private constant MARKET_PRICE = 15; // 1 CST: $15
    ...
    function finalize() external onlyOwner {
        ...
        require(address(development) != address(0), "0 addr: development");

        uint256 reserveAmount = totalPurchased.mul(40e18).div(100e18); // 40%
        ...
        uint256 mintForFairLaunch = totalPurchased.div(EXCHANGE_RATE).div(1e9);
        ...
        uint256 developAmount = reserveAmount.mul(40e18).div(100e18); // 40% for develop
        uint256 treasuryAmount = reserveAmount.sub(developAmount); // 60% for treasury

        //move treasury
        BUSD.approve(treasury, 0);
        BUSD.approve(treasury, treasuryAmount);
        uint256 profit = ITreasury(treasury).valueOf(address(BUSD), treasuryAmount);
        ITreasury(treasury).deposit(treasuryAmount, address(BUSD), profit);

        //move development
        BUSD.transfer(development, developAmount);
