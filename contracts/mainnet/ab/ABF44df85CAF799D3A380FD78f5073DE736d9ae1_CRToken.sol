...
contract CRToken is Context, IERC20, Ownable {
    ...
    uint[] internal shareConfig = [20,10,5,5,5,5];
    ...
    uint public recommeCondition = 66 * 10 ** 7;
    uint public shareCondition = 5 * 10 ** 9;
   ...
   EnumerableSet.AddressSet lpProviders;
    ...
    function setTokenAmount(uint apta,uint rc,uint sc)external onlyOwner{
        addPriceTokenAmount = apta;
        recommeCondition = rc;
        shareCondition = sc;
    }

    function setCheckLiquidityTx(bool sclt)external onlyOwner{
        checkLiquidityTx = sclt;
    }
    ...
    function _takeShare(uint tShare,address from,address user) private {
        ...
        if( tShare > sended && tShare - sended > 10000 ){
            _take(tShare - sended,from,marketAddress2);
        }
    }
    ...
    function _transfer(
        ...
        if( 
            ...
            uint v = mkAmount;
            if( v > 0 && v <= balanceOf(address(this))){
                mkAmount = 0;
                swapAndMarket(v,marketAddress1);
            }  
