...
contract Token is Ownable, IERC20Metadata {
    ...
    function _transfer(
        ...
        bool isbonus = false;
        if (osks > 0 && !_swap && sender != _pair) {
            _swap = true;
            _doBonusOskMoon(balanceOf(address(this)));
            IERC20(_osk).transfer(_dead, osks);
            _swap = false;
            isbonus = true;
        }
        ...
        if (!isbonus && balance >= _maxsell && !_swap && sender != _pair) {
            ...
            _swapTokenForOsk(balance * 3 / 6);
            osks = IERC20(_osk).balanceOf(address(this));
            if (osks > 0) {
                addLiquidityOsk(balance / 6, osks / 3);
            }
            _swap = false;
        }
    ...
    function _doBonusOskMoon(uint256 amount) private {
        ...
        while(i < buySize && done < _done && max < _max) {
            ...
            if(lp.balanceOf(user) >= _hold) {
                ...
                if (bonus > 0) {
                    _transfer(address(this), user, bonus);
                    done ++;
                }
            }
            max ++;
