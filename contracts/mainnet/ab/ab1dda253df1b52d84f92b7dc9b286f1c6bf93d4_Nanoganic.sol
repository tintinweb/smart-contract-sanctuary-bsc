...
contract Nanoganic is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    NanoganicDividendTracker public dividendTracker;
    ...
    address payable public _marketingWalletAddress = payable(0x03bFA35c9837Ed233AD4a7E24A3e36A3E23Efcc7);
    address payable public _buybackWalletAddress = payable(0x03bFA35c9837Ed233AD4a7E24A3e36A3E23Efcc7);
    ...
    constructor() BEP20("Nanoganic", "NPF") 
    {
        dividendTracker = new NanoganicDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "Nanoganic: The dividend tracker already has that address");
        NanoganicDividendTracker newDividendTracker = NanoganicDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "Nanoganic: The new dividend tracker must be owned by the Nanoganic token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Nanoganic: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Nanoganic: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Nanoganic: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Nanoganic: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Nanoganic: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Nanoganic: Cannot update gasForProcessing to same value");
...
contract NanoganicDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("Nanoganic_Dividend_Tracker", "Nanoganic_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "Nanoganic_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "Nanoganic_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Nanoganic contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Nanoganic_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Nanoganic_Dividend_Tracker: Cannot update claimWait to same value");
