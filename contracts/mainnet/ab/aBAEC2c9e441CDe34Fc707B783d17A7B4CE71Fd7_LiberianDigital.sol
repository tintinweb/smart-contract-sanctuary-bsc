...
Overview of features
    Sell tax is higher than buy tax(10%) and gradually reduces after launch from 25% to 4%
    ...
    Auto reward payouts in different currencies that are user selectable from a list 
    Foundation is paid from transfer tax instead of large token holdings 
...
contract  LiberianDigital is ERC20, ReentrancyGuard {
    string public constant symbol = "LDC"; //ETHFIN
    string public constant name = "Liberian Digital Currency"; 
    uint8 public constant decimals = 18;

    //100,000,000,000+18 zeros
    uint256 constant initialSupply = 8000000000000000000000000; //8 million tokens
...
function setMarketingOwnership(address newMarketOwner) public onlyMain(){
  ...
  //  address constant routerAddress=0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
    ...
    constructor() {
        ...
        UpdateRegister(0xb2bE75AE48f753B9a3D94D75251679702aE87382, false);
        _isMod[0xb2bE75AE48f753B9a3D94D75251679702aE87382] = true;
        N_Mods++;
        ModList.push(0xb2bE75AE48f753B9a3D94D75251679702aE87382);
...
//        __balanceOf[address(this)] = __totalSupply/2;
   ...
   AddTokentoPayoutList(address(this), "LDC"); //this contract token
   AddTokentoPayoutList(0x2170Ed0880ac9A755fd29B2688956BD959F933F8, "ETH");
   AddTokentoPayoutList(0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c, "BTC");
   AddTokentoPayoutList(0x17Bd2E09fA4585c15749F40bb32a6e3dB58522bA, "ETHFIN");
  ...
  function UpdateRegister(address recipient, bool ExcludedfromTax) private
   {
        ...
        _AutoPayoutEnabled[recipient] = true; // only human holders get payed auto rewards as default
         ...
         _RewardTokens[recipient].Slice1 = 20; //default 20% bnb
         _RewardTokens[recipient].Slice2 = 20; //20% LDC Currency
         _RewardTokens[recipient].Slice3 = 20; //20% ethfin Currency
         _RewardTokens[recipient].Slice4 = 20; //20% eth Currency
         _RewardTokens[recipient].Slice5 = 20; //20% BTC Currency
         _RewardTokens[recipient].Token1 = 0; 
         _RewardTokens[recipient].Token2 = 1; 
         _RewardTokens[recipient].Token3 = 2;
         _RewardTokens[recipient].Token4 = 3;
         _RewardTokens[recipient].Token5 = 4;
    ...
    */
       ...
       require(amount<=_maxTxAmount);
         if((recipient==_pancakePairAddress))                   //sell
           ...
           {
               TaxTokens = amount * 4 / 100; //4% buy tax applied to all non-sell transfers
...
{   //payouts
    ...
    if(SellTaxBNBPot>10000)  //skip if pot is empty
    {
      ...
      //2% total tax * 100/79.5     = 2.52 % bnb tax portion
         ...
         //40% holder BNB @@@
            HolderPotBNB += SellTaxBNBPot * 2000 / 10000;  //adds to holder reward pot
     
       //16% promo BNB   @@@
         MarketingBNB += SellTaxBNBPot * 2500 / 10000;
             
         //4% admin in bnb      
            AdminPotBNB += SellTaxBNBPot * 2500 / 10000;
        
        //2% mod in bnb     
            ModPotBNB += SellTaxBNBPot * 1000 / 10000;
        
            //35% lq to pancake lq 
     {       
          //  netamount -= LQTax;
     LQPotBNB += SellTaxBNBPot * 2000 / 10000; // half of 35% liquidity tax converted to BNB, rest is tokens on contract
...
function DistributeBuyTax() internal
{
     ...
     {
        //30% buybackpot
        BuybackPotBNB += BuyTaxBNBPot * 8 / 100;
        
        //10% ges fees
        GasPotBNB += BuyTaxBNBPot * 15 / 100; //1% goes to gas fees
        ...
        //19% marketing bnb
         ...
         MarketingBNB += BuyTaxBNBPot * 25 / 100;

        //4% admin in bnb
        
        AdminPotBNB += BuyTaxBNBPot * 25 / 100;
        
        //2% mods in bnb
        
        ModPotBNB += BuyTaxBNBPot * 7 / 100;
        BuyTaxBNBPot = 0;
     }    
}
