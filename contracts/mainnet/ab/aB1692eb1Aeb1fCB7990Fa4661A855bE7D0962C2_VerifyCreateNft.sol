...
abstract contract MarketNft{
    function resaleNft(uint256 tokenId,uint256 amountToWei,address reseller) external virtual;
}
...
contract VerifyCreateNft is Comn{
    ...
    event VerifyResale(uint amountToWei);
    ...
    // 转售验证
    function resaleVerify(uint amountToWei,address sender,string memory tokenIdStr,string memory seed, bytes32 _r, bytes32 _s,uint8 _v) external isRuning isMember isBlack nonReentrant  returns(bytes memory signaturn){
        signaturn = createVerify(sender,tokenIdStr,seed,_r,_s,_v);
        uint tokenId = stringToUint(tokenIdStr);
        MarketNft(marketContract).resaleNft(tokenId,amountToWei,msg.sender);
        emit VerifyResale(amountToWei);
    }

    // 铸造验证
    function createVerify(address sender,string memory tokenIdStr,string memory seed, bytes32 _r, bytes32 _s,uint8 _v) public isRuning isMember isBlack nonReentrant  returns(bytes memory signaturn){
    ...
    address private marketContract;                                 //[设置]  交易市场合约
