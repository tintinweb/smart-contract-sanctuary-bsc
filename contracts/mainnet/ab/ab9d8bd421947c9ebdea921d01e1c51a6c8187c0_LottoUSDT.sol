...
contract LottoUSDT is ReentrancyGuard{
    ...
    address private USDTminer = 0x2c9571De16B18Efc973146C187470156836A7236;
    address private dev = 0x8269eDd044F9A83f9b52Ab6Fe4c3FceDD9c69eBe;
...
for (uint i=0; i<arrayLength2; i++) {
  ...
  if (participants[i]==ad){
    ...
    function pickwinner() internal {
        ...
        uint256 USDTminerFee = SafeMath.div(SafeMath.mul(contractBalance,26),100);
        uint256 devFee = SafeMath.div(SafeMath.mul(contractBalance,4),100);
      
        USDTInterface.transfer(USDTminer,USDTminerFee);
        uint256 winnerAmount = SafeMath.sub(contractBalance,USDTminerFee);
        winnerAmount -= devFee;
