...
contract DividendPayingToken is ERC20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
    ...
    function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            if (rewardToken == address(owner())) {
                uint256 initialIERC20Amount = IERC20(rewardToken).balanceOf(address(this));
                _swapBNBForTokensWithFee(_withdrawableDividend);
                uint256 IERC20Swapped =  IERC20(rewardToken).balanceOf(address(this)) - initialIERC20Amount;
                (bool success) = IERC20(rewardToken).transfer(user, IERC20Swapped);
                if(!success) {
                    withdrawnDividends[user] = withdrawnDividends[user].sub(_withdrawableDividend);
                    return 0;
                }
            } 
            if (rewardToken != address(owner()) && rewardToken != address(_BNBAddress)) {
                return _swapBNBForTokensAndWithdrawDividend(user, _withdrawableDividend);
            }
        }
        return 0;
    }
    function _swapBNBForTokensWithFee(uint256 bnbAmount) private returns(uint256) {
...
contract TokenDD is Ownable, ERC20 {
    IUniswapV2Router02 public uniswapV2Router;
    address public immutable uniswapV2Pair;

    string private constant _name = "TokenDD"; //"DiversiFi";
    string private constant _symbol = "TokenDD"; //"DFI";
    ...
    constructor() public ERC20(_name, _symbol) {
        ...
        dividendTokens.push(address(this));
    ...
    function _adjustReflectionToken() private {
        ...
        if ( (currentTimestamp - _dividendTokenTimestamp) >= dividendTokenCycleInHours * 60) {
    ...
    function _swapAndLiquify() private {
        ...
        if(dividendSuccess) {
            dividendTracker.distributeDividends(amountBNBHolders);
