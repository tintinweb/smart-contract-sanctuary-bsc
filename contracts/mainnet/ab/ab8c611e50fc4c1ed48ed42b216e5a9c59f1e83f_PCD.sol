...
contract PCD is Context,IERC20Metadata,Ownable {

    using SafeMath for uint256;

    mapping(address => uint256) private _balances;

    mapping(address => address) private _pharosBindParent;

    mapping(address => address) private _powerBindParent;
    ...
    //mainnet: 
    string private constant _name = "PinkCoinDao";
    //mainnet: 
    string private constant _symbol = "pcd";

    address public lpContractAddress;
    
    uint256 private _totalSupply = 7500000 * 1e18;
   ...
   //event payFeeEV(address indexed topc, address from, address to, uint256 feeCBD);
    ...
    function setLPContractAddress(address _address) onlyOwner public returns (bool success) {
        ...
        function TotalFee() view public returns (uint) {
       
        return totalFee;
    }

          function TotalBurn() view public returns (uint) {
    ...
    function pharosBindParent(address account) public view  returns (address) {
        return _pharosBindParent[account];
    }

    function setPharosBindParent(address account,address parent) public  returns (bool) {
              _pharosBindParent[account]=parent;
           return true;
    }


    function powerBindParent(address account) public view  returns (address) {
        return _powerBindParent[account];
    }

    function setPowerBindParent(address account,address parent) public  returns (bool) {
              _powerBindParent[account]=parent;
           return true;
    }
