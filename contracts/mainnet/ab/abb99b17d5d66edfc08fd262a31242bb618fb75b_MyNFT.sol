...
contract MyNFT is ERC721,Ownable {
    ...
    function mint(address recipient_) public onlyOwner returns(uint256) {
    ...
    function mintMultiple(uint n, address _to) external onlyOwner {
        for (uint i = 0; i < n; i++) {
            mint(_to);
        }
    }
    ...
    function sesysname(string memory name) external onlyOwner {
        _symbol = name;
    }
