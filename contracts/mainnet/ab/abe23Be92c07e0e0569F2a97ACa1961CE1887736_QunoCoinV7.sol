...
contract QunoCoinV7{
    ...
    function userWithdrawal() public returns(bool){
        ...
        bool status;
        if(u.totalIncomeNew > u.withdrawanNew){
        uint256 amount = (u.totalIncomeNew - u.withdrawanNew);
        u.withdrawanNew = (u.withdrawanNew + amount);
    ...
    function UserAddon(address _referral_address, uint _amount,address _useraddress ) public {
        require(msg.sender == admin, 'permission denied!' );
        User storage user = userdata[_useraddress];
        require(getDepositLength(_useraddress) == 0 , "User already exist");
        uint getbnb = getCalculatedBnbRecieved(_amount);
        user.refferal_code = _referral_address;
        user.amount += getbnb;
        user.TotalamountInUSD += _amount;
        user.timestamp = block.timestamp;
        user.deposit.push(Deposit( getbnb ,  _amount, block.timestamp));
