...
DustINU Official Smart Contract

DINU is a token on the Binance Smart Chain that rewards its holders with BNB passively.
...
contract DustINUDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("DustINU_Dividend_Tracker", "DustINU_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 1000000 * (10**18); //must hold 1000000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "DustINU_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "DustINU_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DustINU contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DustINU_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "DustINU_Dividend_Tracker: Cannot update claimWait to same value");
...
contract DustINU is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    DustINUDividendTracker public dividendTracker;
    ...
    constructor() ERC20("DustINU", "DINU") {
        BNBRewardsFee = 6;
        liquidityFee = 2;
        extraFeeOnSell = 0; // extra fee on sell
        marketingFee = 2;
        marketingWallet = payable(0x55A28037ff7E321E567272d3d27158d4f0AA2b9F);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new DustINUDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "DustINU: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "DustINU: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DustINU: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "DustINU: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "DustINU: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "DustINU: Cannot update gasForProcessing to same value");
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function getStuckToken(address token) external onlyOwner {
        ...
        if(token == address(this)){
            uint256 totalFee = BNBRewardsFee.add(liquidityFee).add(marketingFee);
