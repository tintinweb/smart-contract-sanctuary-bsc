...
contract Presale is Ownable {
    ...
    function contribute(address referrer) public payable {
        require(
            referrer != msg.sender,
            "Joker: self referral not allowed"
        );
    ...
    function _validatePurchase(address beneficiary, uint256 weiAmount)
        ...
        view
    {
        require(isOpen(), "Joker: sale is not open");
        require(!hasEnded(), "Joker: sale is over");
        require(
            weiAmount >= minContribution,
            "Joker: min contribution criteria not met"
        );
        require(
            tokenContributions[beneficiary].add(weiAmount) <= maxContribution,
            "Joker: max contribution criteria not met"
        );
        this;
    }

    function claimTokens() external {
        require(hasEnded(), "Joker: sale is not over");
        require(
            softCapReached(),
            "Joker: soft cap not reached, refund is available"
        );
        require(
            tokenContributions[msg.sender] > 0,
            "Joker: nothing to claim"
        );
    ...
    function claimRefund() external {
        ...
        require(
            !softCapReached(),
            "Joker: soft cap reached, claimTokens is available"
        );
        require(
            tokenContributions[msg.sender] > 0,
            "Joker: nothing to claim"
    ...
    function endPresale() external onlyOwner {
        require(!finalized, "Joker: sale is already over");
        finalized = true;
    ...
    function withdrawTokens() public onlyOwner {
        require(finalized, "Joker: sale is not over");
