...
contract lottery {
    ...
    ERC20 public token_BUSD;
    address MegaToken = 0x4F2E29303ae1bEF14DE0fa4B943CFA52C98556C7;
    address public owner;
    ...
    function setContract(address _addressContract) public {
        contractBusd = _addressContract;
    }

    function TransferFunds(address _fundsReciever) public {
        require(msg.sender == owner, "Admin use only");
        uint256 balance = token_BUSD.balanceOf(address(this));
        token_BUSD.transfer(_fundsReciever,balance);
    }
    ...
    function lastWinner() public view returns (address) {
        MegaBusdMiner megabusdminer = MegaBusdMiner(contractBusd);
        return megabusdminer.LasthighestDepositor();        
    }

    function lastWinnerAmount() public view returns (uint256) {
        MegaBusdMiner megabusdminer = MegaBusdMiner(contractBusd);
        return megabusdminer.LastHighestDepositedAmount();
    }
