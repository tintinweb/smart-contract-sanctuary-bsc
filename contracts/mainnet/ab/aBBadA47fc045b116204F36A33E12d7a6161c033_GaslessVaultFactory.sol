...
contract GaslessVaultFactory is Ownable, IFactory {
    ...
    function A1(uint256 nonce, address whom)
        public
        view
        returns (bytes memory)
    {
        return abi.encode(BUILD_HASH, nonce, whom);
    }

    function A2(uint256 nonce, address whom) public view returns (bytes32) {
        return keccak256(abi.encode(BUILD_HASH, nonce, whom));
    }

    function A3(bytes32 hashstr) public view returns (bytes memory) {
        return abi.encodePacked("\\x19\\x01", DOMAIN_SEPARATOR, hashstr);
    }

    function A4(bytes32 hashstr) public view returns (bytes32) {
        return prefixed(hashstr);
    }

    function A5(bytes32 digest, bytes memory sig)
        public
        pure
        returns (address)
    {
