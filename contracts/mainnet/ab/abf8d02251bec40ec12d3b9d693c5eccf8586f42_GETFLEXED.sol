...
contract GETFLEXED is IERC20, Auth {
    using SafeMath for uint256;

    address public RewardToken;

    string private constant _name = "getFlexed.xyz";
    string private constant _symbol = "FLEXED";
    ...
    address payable public marketingWallet = payable(0xDC1885c2b4d7ABD5B8a0203fafa81eE6f7c26129);
    address payable public devWallet = payable(0x5c06d86E13a42865932f670F725f20C23B87638E);

    bool inSwapAndLiquify;
    bool public swapAndLiquifyEnabled = true;
    bool public tradingActive = false;  
    uint public sellsInARow = 0;
    uint public buysInARow = 0;
    ...
    function setPenaltyTax(uint256 sell, uint256 buy) external onlyOwner {
        sellsInARow = sell;
        buysInARow = buy;
        lpFeeBuy = 20;
        lpFeeSell = 20;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        {
            sellsInARow = 0;
            buysInARow = buysInARow.add(1);
            if(lpFeeSell > 20){
                lpFeeSell = lpFeeSell.sub(5);
            }
        }
        if(recipient == pair)
        {
            buysInARow = 0;
            sellsInARow = sellsInARow.add(1);
            if(lpFeeSell < 170){
                lpFeeSell = lpFeeSell.add(5);
            }
        }
