...
contract NoftGameManagerV2 is Ownable {
    ...
    constructor(address _token, address payable _wallet, address _manager, address _rentAddress, address _expAddress) {
        wallet = _wallet;
        manager = _manager;
        token = _token;
    ...
    function addBots(uint[] memory tokenIds, uint gameId, uint[] memory strategyIds) external {
        ...
        uint botsCount = getBotsCount(gameId) + tokenIds.length;

        require(botsCount <= game.playersCount - 3);
    ...
    function endGame(uint gameId, uint[] calldata winnerIds, uint[]  calldata tokenIds, uint[] calldata exps) external onlyOwnerOrManagerOrServer {
        Game memory game = games[gameId];
        require(game.status == GameStatus.PICKED);
        require(winnerIds.length == winPercentages.length);
        ...
        for (uint i = 0; i < winnerIds.length; i++) {
            ...
            uint winnerPrize = game.bank.div(100).mul(winPercentages[i]);
            ...
            emit PlayerChanged(_msgSender(), gameId, winnerIds[i], false);
        }
    ...
    function setWinPercentages(uint[] calldata _percentages) external onlyOwnerOrManager {
        winPercentages = _percentages;
    }
    ...
    function getWinPercentages(uint index) external view returns (uint) {
        return winPercentages[index];
    }
