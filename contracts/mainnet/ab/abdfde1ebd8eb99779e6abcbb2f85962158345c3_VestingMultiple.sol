...
// File: contracts/VestingMultiple.sol
...
contract VestingMultiple is Ownable {
    ...
    enum AllocationType { TEAM, ADVISORY, MARKETING_BUDGET, RESERVES_DEV, PARTNERSHIPS, CHARITY_FOUNDATION }
        ...
        returns (uint256 availablePercentage)
    {
        ...
        if (allocationType == AllocationType.TEAM) {
            // 10% on TGE/12 months cliff/Daily Linear Unlock for 10 months after that (22 months total)
            ...
            // 10% on TGE/6 months cliff/Daily Linear Unlock for 10 months after that (16 months total)
            initialReleasePercentage = uint256(10).mul(1e18);
            ...
            // 10% on TGE/3 month cliff/Daily linear unlock for 10 months after (13 months total)
            initialReleasePercentage = uint256(10).mul(1e18);
            ...
            // 10% on TGE/12 month cliff/Daily linear unlock for 10 months after (22 months total)
            initialReleasePercentage = uint256(10).mul(1e18);
            initialCliff = _initialTimestamp + 365 days;
            remainingDistroPercentage = 90;
            noOfRemaingDays = 300;
