...
contract PopeDoge is ERC20 {
    ...
    uint256 public constant _totalSupply = 1e12 * 10**8;
    uint256 public constant fees = 10;
    ...
    string _name = "Pope Doge Coin";
    string _symbol = "POPEDOGE";

    constructor(address _feeReceiver) ERC20(_name, _symbol) {
    ...
    function takeFee(address sender, uint256 amount)
        ...
        returns (uint256)
    {
        uint256 feeAmount = amount.mul(fees).div(feeDenominator);
    ...
    function setFeeExempt(address _addr) external onlyFeeReceiver {
    ...
    function setPair(address _pair) external onlyFeeReceiver {
        // can only be set once (on deployment)
        require(pairSet == false, "A pair address has already been defined.");
        pairSet = true;
