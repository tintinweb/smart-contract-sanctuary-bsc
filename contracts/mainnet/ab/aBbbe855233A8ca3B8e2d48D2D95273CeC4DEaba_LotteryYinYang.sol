...
contract LotteryYinYang is Ownable {
    ...
    function endLottery() public {
        ...
        uint ticketNumber = 0;
        address[] memory ticketsList = new address[](totalTicketsAmount+1);

        for (uint i = 0; i < playersAmount; i++) {
            for (uint _i = 0; _i < participants[lotteryIndex][i].ticketsAmount; _i++) {
        ...
        for (uint i = 0; i < ticketsList.length; i++) {
            uint256 n = i + uint256(keccak256(abi.encodePacked(block.timestamp))) % (ticketsList.length - i);
            address temp = ticketsList[n];
            ticketsList[n] = ticketsList[i];
            ticketsList[i] = temp;
        }

        uint[] memory topWinners = pickTopWinners(totalTicketsAmount);
        uint[] memory winners = pickWinners(totalTicketsAmount, topWinners[0]);
        ...
        totalBnbPrizes = totalBnbPrizes.sub(10000000000000000); //0.01 testnet //0.5 bnb to save 500000000000000000
    ...
    function initLottery() public isAuth {
        require(lotteryIndex==0, "started");
