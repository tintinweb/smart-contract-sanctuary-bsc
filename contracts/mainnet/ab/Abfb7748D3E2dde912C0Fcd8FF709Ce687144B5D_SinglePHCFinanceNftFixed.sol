...
// File: contracts\SinglePHCFinanceNft.sol

// Single PHCFinance token

pragma solidity ^0.8.0;


contract SinglePHCFinanceNft is ERC721 {
    ...
    constructor() ERC721("SinglePHCFinanceNft","SinglePHCFinanceNft") {
        factory = msg.sender;
    }
...
// File: contracts\SinglePHCFinanceNftFixed.sol

// Single PHCFinance Fixed Price Marketplace contract
...
contract SinglePHCFinanceNftFixed is Ownable, ERC721Holder {
    using SafeMath for uint256;
	using EnumerableSet for EnumerableSet.AddressSet;
	...
	function createCollection(string memory _name, string memory _uri, bool bPublic) public returns(address collection) {
		bytes memory bytecode = type(SinglePHCFinanceNft).creationCode;
        ...
        ISinglePHCFinanceNft(collection).initialize(_name, _uri, msg.sender, bPublic);
    ...
    function singleList(address _collection, uint256 _tokenId, address _tokenAdr, uint256 _price) OnlyItemOwner(_collection,_tokenId) public {
		...
		ISinglePHCFinanceNft nft = ISinglePHCFinanceNft(_collection);        
    ...
    function singleDelist(uint256 _id) external {        
        ...
        ISinglePHCFinanceNft(pairs[_id].collection).safeTransferFrom(address(this), pairs[_id].owner, pairs[_id].tokenId);        
    ...
    function singleBuy(uint256 _id) external payable {
		...
		ISinglePHCFinanceNft(pairs[_id].collection).safeTransferFrom(address(this), msg.sender, pair.tokenId);
	...
	modifier OnlyItemOwner(address tokenAddress, uint256 tokenId){
        ISinglePHCFinanceNft tokenContract = ISinglePHCFinanceNft(tokenAddress);
