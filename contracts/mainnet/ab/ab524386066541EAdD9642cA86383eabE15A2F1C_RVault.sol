...
contract RVault is RouterCrossTalk {
    ...
    constructor(address _token, IBridge _bridge, address _handler, address _bridgeReserve) RouterCrossTalk(_handler)
    {
        token = IERC20(_token);
    ...
    // @note this should go inside the reserve contract
    // function balance(uint8 _chainId) public view returns (uint256) {
    //     return token.balanceOf(reserves[_chainId]).add(IStrategy(strategies[_chainId]).balanceOf());
    // }
    ...
    function setCrossChainGas(uint256 _gas) external onlyGovernance {
        crossChainGas = _gas;
    }
    ...
    // function _routerCrossTalkSendDeposit(uint8 _chainId) internal {
    ...
    function withdraw(uint256 _shares, uint8 _chainId) public {
        address reserve = reserves[_chainId];
        IReserve(reserve).withdraw(_shares);
    ...
    function _routerSyncHandler(bytes4 _interface, bytes memory _data)
        ...
        if(bytes4(keccak256("deposit(_amount)")) == _interface) {
    ...
    // function getPricePerFullShare(uint8 _chainId) public view returns (uint256) {
    //     return balance(_chainId).mul(1e18)
    //     .div(IReserve(reserves[_chainId]).totalSupply());
    // }
}
