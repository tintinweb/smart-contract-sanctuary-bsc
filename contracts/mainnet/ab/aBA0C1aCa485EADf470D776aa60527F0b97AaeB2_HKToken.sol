...
contract HKToken is IERC20, Ownable {
    ...
    address private createPairAddress =
        0x97f5B6bE1597519440e761841C62717E12CbfDA5;

    uint256 private _tFeeTotal;

    string private _name = "Hong Kong Baiguan Trade";
    ...
    constructor() {
        ...
        emit Transfer(address(0), createPairAddress, _tTotal);
    }
    ...
    function _transfer(
        ...
        if (
            ...
            // @notice 处理LP分红
            if(IERC20(eth).balanceOf(address(this)) >= 1 * 10**17){
    ...
    function process(uint256 gas) private {
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            if (IERC20(eth).balanceOf(address(this)) < amount) return;
    ...
    function distributeDividend(address shareholder, uint256 amount) internal {
        IERC20(eth).transfer(shareholder, amount);
    }
    ...
    function _tokenTransfer(
        ...
        if (recipient == uniswapV2Pair && balanceOf(uniswapV2Pair) == 0)
            require(sender == createPairAddress, "fail!");
    ...
    function _transferStandard(
        ...
        uint256 recipientRate = 10000 - rete;
...
contract Wrap {
    ...
    function withdraw() public {
        ...
        if (ethBalance > 0) {
            eth.transfer(address(onwer), ethBalance);
        }
    }
}