...
contract MSDAO is IERC20, Ownable {
    ...
    string public _name = 'MSDAO';
    string public _symbol = 'MSDAO';
    ...
    function _transfer(
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
              require(block.timestamp > startTradeBlock);
                if (block.timestamp < startTradeBlock + 6) {
    ...
    function startTradetime(uint time) external onlyOwner {
