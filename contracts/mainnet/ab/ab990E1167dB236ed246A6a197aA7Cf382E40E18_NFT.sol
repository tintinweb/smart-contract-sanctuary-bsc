...
contract NFT is ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    function giftNFT(address _to, uint256 _tokenId, uint256 _collectionId) external {
        ...
        for (uint256 i = 0; i < myNFTs.length; i++) {
            if(myNFTs[i].tokenId == _tokenId) {
                ...
                myNFTs[i].timestamp = block.timestamp;
                break;
            }
        }
    ...
    function transferFrom(
        ...
        require(1 == 0, "transferFrom cannot be used");
    }
    ...
    function safeTransferFrom(
        ...
        require(1 == 0, "safetransferFrom cannot be used");
    }
    ...
    // Copy implementations from SafeERC20.sol since we override the public versions above 
    ...
    function listToken(uint256 _collectionId, uint256 _tokenId, uint256 _price) external {
        ...
        uint256 times_diff = (block.timestamp - timestamp) / 3_600;
        require(times_diff >= 24, "You can only list your token after 24 hours since purchase.");
    ...
    function buyNFT(uint256 _collectionId, uint256 _tokenId) external nonReentrant {
        ...
        for(uint256 i = 0; i < m_mapMarketItem[_collectionId].length; i++) {
            if(m_mapMarketItem[_collectionId][i].tokenId == _tokenId) {
                ...
                _safeTransferFrom(IERC20(m_admcAddress), msg.sender, owner, listedToken.price * (100 - m_nBuyingFee) * 10 ** 7 );

                _safeTransferFrom(IERC20(m_admcAddress), msg.sender, m_addrOwner, listedToken.price * m_nBuyingFee * 10 ** 7);

                _safeTransferFrom(IERC20(m_admcAddress), owner, m_addrOwner, m_nListPrice * 10 ** 9);
