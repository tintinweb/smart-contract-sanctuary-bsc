...
/** @title KazamaSwap Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract KazamaSwapLottery is ReentrancyGuard, IKazamaSwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInKazama = 30 ether;
    uint256 public minPriceTicketInKazama = 0.005 ether;
    ...
    uint256 public constant MAX_TREASURY_FEE = 1700; // 17%

    IERC20 public kazamaToken;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInKazama;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee;
        uint256[6] kazamaPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInKazama;
        uint32 finalNumber;
    }
     ...
     * @param _kazamaTokenAddress: address of the KAZAMA token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _kazamaTokenAddress, address _randomGeneratorAddress) {
        kazamaToken = IERC20(_kazamaTokenAddress);
    ...
    *  what is deposited for the tickets.
     */
    ...
    * @dev If ever needed.
        ...
        nonReentrant
    {
        ...
        // Calculate number of KAZAMA to this contract
        uint256 amountKazamaToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInKazama,
            _ticketNumbers.length
        );

        uint256 tokensToBurn = burnPercentage(amountKazamaToTransfer);
        uint256 finalAmount = amountKazamaToTransfer - tokensToBurn;

        // Transfer Kazama tokens to this contract
        kazamaToken.safeTransferFrom(address(msg.sender), address(this), amountKazamaToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInKazama += finalAmount;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInKazamaToTransfer
        uint256 rewardInKazamaToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInKazamaToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        kazamaToken.safeTransfer(msg.sender, rewardInKazamaToTransfer);

        emit TicketsClaim(msg.sender, rewardInKazamaToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInKazama) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in KAZAMA for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].kazamaPerBracket[j] =
                ...
                // A. No KAZAMA to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].kazamaPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInKazama - amountToShareToWinners);

        // Transfer KAZAMA to treasury address
        kazamaToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        kazamaToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInKazama += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInKazama >= minPriceTicketInKazama) && (_priceTicketInKazama <= maxPriceTicketInKazama),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInKazama: _priceTicketInKazama,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            kazamaPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInKazama: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(kazamaToken), "Cannot be KAZAMA token");
    ...
    /**
     * @notice Set KAZAMA price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInKazama: minimum price of a ticket in KAZAMA
     * @param _maxPriceTicketInKazama: maximum price of a ticket in KAZAMA
     */
    function setMinAndMaxTicketPriceInKazama(uint256 _minPriceTicketInKazama, uint256 _maxPriceTicketInKazama)
        external
        onlyOwner
    {
        require(_minPriceTicketInKazama <= _maxPriceTicketInKazama, "minPrice must be < maxPrice");

        minPriceTicketInKazama = _minPriceTicketInKazama;
        maxPriceTicketInKazama = _maxPriceTicketInKazama;
    }
     ...
     * @param _priceTicket price of a ticket (in KAZAMA)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].kazamaPerBracket[_bracket];
        } else {
            return 0;
        }
    }
