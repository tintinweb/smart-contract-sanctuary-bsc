...
contract trustcronos is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    trustcronosDividendTracker public dividendTracker;
    ...
    constructor() ERC20("trustcronos", "TRUSTCRO") {
        ...
        dividendTracker = new trustcronosDividendTracker(
            payable(this),
            router,
...
contract trustcronosDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    trustcronos public trustcronosContract;
    ...
    constructor(
        ...
        defaultToken = token;
        trustcronosContract = trustcronos(mainContract);
    ...
    function _transfer(
        ...
        require(false, "trustcronos_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "trustcronos_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main trustcronos contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(trustcronosContract);

            uint256 prevBalance = trustcronosContract.balanceOf(address(this));
            ...
            {
                uint256 received = trustcronosContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    trustcronosContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
