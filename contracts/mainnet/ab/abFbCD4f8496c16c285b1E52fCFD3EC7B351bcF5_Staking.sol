 ...
 * Staking of : Coin Coin_YBNB
 * Coin Address : 0x361b467A00fAD1d21ae8B76F36626739d1cf9763
 * Number of schemes : 1
 ...
 * updates referralRecordMap (Element referringAddress) (Entity unclaimedRewards) as (referralRecordMap with element referringAddress with element unclaimedRewards) + ((4) * (_amt))
...
*/
	function addReferral(uint256 _amt) internal {
		...
		referralRecordMap[referringAddress].unclaimedRewards  = (referralRecordMap[referringAddress].unclaimedRewards + (uint256(4) * _amt));
 ...
 * ERC20 Transfer : 0x361b467A00fAD1d21ae8B76F36626739d1cf9763, _stakeAmt
...
*/
	function stake(uint256 _stakeAmt) public {
		...
		ERC20(0x361b467A00fAD1d21ae8B76F36626739d1cf9763).transferFrom(msg.sender, address(this), _stakeAmt);
 ...
 * calls ERC20's transfer function  with variable recipient as the address that called this function, variable amount as ((_unstakeAmt) * ((1000000) - (40000))) / (1000000)
 * updates totalWithdrawals as (totalWithdrawals) + (interestToRemove)
 * updates principalTaxBank as (principalTaxBank) + (((thisRecord with element stakeAmt) * (4)) / (100))
...
*/
	function unstake(uint256 _unstakeAmt) public {
		...
		ERC20(0x361b467A00fAD1d21ae8B76F36626739d1cf9763).transfer(msg.sender, ((_unstakeAmt * (uint256(1000000) - uint256(40000))) / uint256(1000000)));
		totalWithdrawals  = (totalWithdrawals + interestToRemove);
		principalTaxBank  = (principalTaxBank + ((thisRecord.stakeAmt * uint256(4)) / uint256(100)));
...
*/
	function withdrawInterestWithoutUnstaking(uint256 _withdrawalAmt) external {
		...
		payable(msg.sender).transfer(_withdrawalAmt);
...
*/
	function withdrawPrincipalTax() public onlyOwner {
		ERC20(0x361b467A00fAD1d21ae8B76F36626739d1cf9763).transfer(msg.sender, principalTaxBank);
