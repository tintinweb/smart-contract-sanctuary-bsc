...
contract OGStake {
    ...
    uint256 tax = 10_000;
    //address
    IBEP20 public TOKEN = IBEP20(0x0E386a048649c602f1411432F9253E5D0393af43);
        ...
        validUser(msg.sender)
    {
        ...
        if (calcRewards(msg.sender, _depositId) > 0) {
            ...
            msg.sender,
            calcRewards(msg.sender, _depositId) -
                ((calcRewards(msg.sender, _depositId) * tax) /
                    percentDivider)
        );
        TOKEN.transferFrom(
            RewardAddress,
    ...
    function _withdraw(
        ...
        if (calcRewards(_user, _depositId) > 0) {
            ...
            calcRewards(_user, _depositId) -
                ((calcRewards(_user, _depositId) * tax) /
                    percentDivider)
        );
        TOKEN.transferFrom(
            RewardAddress,
    ...
    function forceUnstake(uint256 _depositId, address _user)
        external
        validDepositId(_depositId)
        onlyAdmin
    {
        ...
        delete stakedata.Plan[_depositId][_user];
    }
    ...
    function updatePlanPercentages(uint256 _depositId, uint256 _percent,uint256 _percentPremium)
        external
        onlyAdmin
    {
        stakedata.stakeplan[_depositId].StakePercent = _percent;
        stakedata.stakeplan[_depositId].StakePercentPremium = _percentPremium;
    }
    function updatePlanMinMax(uint256 _depositId, uint256 _minamount,uint256 _maxamount)
        external
        onlyAdmin
    {
        stakedata.stakeplan[_depositId].maxStake = _minamount;
        stakedata.stakeplan[_depositId].minStake = _maxamount;

    }
    function updatePlanVestingPeriod(uint256 _depositId, uint256 _VestingPeriod)
        external
        onlyAdmin
    {
        stakedata.stakeplan[_depositId].StakePeriod = _VestingPeriod;
    }
