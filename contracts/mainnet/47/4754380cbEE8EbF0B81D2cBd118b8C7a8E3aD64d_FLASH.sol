...
contract FLASH is IERC20 {
    ...
    string constant private _name = "Flash Transfer";
    ...
    uint256 public taxesUnlockTime;
    ...
    TaxWallets public _taxWallets = TaxWallets({
        marketing: payable(0x60808eaaED5AfA68DC2d7B1b8B3efF84c846754d),
        staking: payable(0x60808eaaED5AfA68DC2d7B1b8B3efF84c846754d)
    });
...
//===============================================================================================================
    ...
    function getSecondsToTaxUnlock() public view returns (uint256 secondsToUnlock) {
        if (block.timestamp > taxesUnlockTime) {
            return 0;
        }
        return taxesUnlockTime - block.timestamp;
    }

    function lockTaxes(uint256 timeToLockInMinutes) external onlyOwner {
        taxesUnlockTime = block.timestamp + (timeToLockInMinutes * 1 minutes);
    }

    function setTaxes(uint16 buyFee, uint16 sellFee, uint16 transferFee) external onlyOwner {
        require(getSecondsToTaxUnlock() == 0, "Taxes are currently locked.");
    ...
    function setRatios(uint16 liquidity, uint16 marketing, uint16 staking, uint16 _burn) external onlyOwner {
        _ratios.liquidity = liquidity;
        _ratios.marketing = marketing;
        _ratios.burn = _burn;
        _ratios.staking = staking;
        _ratios.totalSwap = liquidity + marketing + staking + _burn;
        uint256 total = _taxRates.buyFee + _taxRates.sellFee;
        require(_ratios.totalSwap <= total, "Cannot exceed sum of buy and sell fees.");
    }

    function setWallets(address payable staking, address payable marketing) external onlyOwner {
    ...
    function contractSwap(uint256 contractTokenBalance) internal lockTheSwap {
        ...
        if (toLiquify > 0) {
            try dexRouter.addLiquidityETH{value: amount}(
                address(this),
                toLiquify,
                0,
                0,
                ...
                emit AutoLiquify(amount, toLiquify);
            } catch {
                return;
            }
        }
    ...
    function takeTaxes(address from, bool buy, bool sell, uint256 amount) internal returns (uint256) {
        ...
        uint256 stakingAmount = (feeAmount * ratios.staking) / ratios.totalSwap;
        uint256 burnAmount = (feeAmount * ratios.burn) / ratios.totalSwap;
        uint256 swpAmt = (feeAmount * ratios.liquidity) / ratios.totalSwap;
        uint256 marketingAmount = feeAmount - (stakingAmount + burnAmount + swpAmt);
        address receiver;
        if (swpAmt > 0) {
        ...
        if (stakingAmount > 0) {
            receiver = _taxWallets.staking;
            _tOwned[receiver] += stakingAmount;
            emit Transfer(from, receiver, stakingAmount);
        }
        if (marketingAmount > 0) {
            receiver = _taxWallets.marketing;
            _tOwned[receiver] += marketingAmount;
            emit Transfer(from, receiver, marketingAmount);
        }
        if (burnAmount > 0) {
            receiver = address(0);
            _tTotal -= burnAmount;
            emit Transfer(from, receiver, burnAmount);
        }
