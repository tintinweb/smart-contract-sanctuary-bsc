...
contract MoraStaking is Ownable{
    ...
    bool private isActive = true; // Contract State
    uint8 private tier1APY = 30;  // Tier 1 APY %
    uint8 private tier2APY = 50; // Tier 2 APY %
    uint32 private tierBracklet = 15000; 
    uint32 private minStakeAmount = 1000;
    ...
    mapping (uint8 => address []) stakerList;
    ...
    function setMinStake(uint32 _minStakeAmount) external onlyOwner returns (bool result) { 
        minStakeAmount = _minStakeAmount;
    }

    function getMinStake() external view returns (uint32 result) { 
        return minStakeAmount;
    }


    function setContractState(bool _state) external onlyOwner returns  (bool result) { 
    if (_state == true) { isActive = true; return true;}
    if (_state == false) { isActive = false; return false;}
    }

     function setTierBracklet(uint32 _tierBracklet) external onlyOwner returns (bool result) { 
        tierBracklet = _tierBracklet; return true;
    }

    function getTierBracklet() external view returns (uint32 result) { 
        return tierBracklet;
    }
       

    function setTier1APY(uint8 _tier1APY) external onlyOwner returns (bool result) { 
        tier1APY = _tier1APY; return true;
    }

      function getTier1APY() external view returns (uint8 result) { 
        return tier1APY;
    }

    function setTier2APY(uint8 _tier2APY) external onlyOwner returns (bool result) { 
        tier2APY = _tier2APY; return true;
    }

    function getTier2APY() external view returns (uint8 result) { 
        return tier2APY;
    }

    function bburn(uint256 _amount) external onlyOwner returns (bool result) { 
        require(token.transfer(address(this), _amount),"Failed");
        return true;
    }


   function Stake(uint256 _amount) external isContractActive returns(bool result) {
    require(_amount >= minStakeAmount,"must-be-greater-than-min-stake-amount");
    _amount = _amount.mul(1e18);
    uint APY;
    uint _stakeDate = block.timestamp;
    if ( _amount <  tierBracklet.mul(1e18) ) { APY = tier1APY;} 
    if ( _amount >= tierBracklet.mul(1e18) ) { APY = tier2APY;}                              
    stakeBoxs.push(StakeBox(msg.sender, _amount, APY, _stakeDate , 0, 0, 0, true));
  ...
  function UnstakeAndClaim(uint _stakeID) external returns (bool result) {
    ...
    require(stakeBoxs[_stakeID].amount > 0,"no-token");
    uint256 _amount = stakeBoxs[_stakeID].amount;
    uint256 _reward = calcReward(_stakeID);
    uint256 _claimedAmount = _amount.add(_reward);
    ...
    function calcReward(uint _stakeID) public view returns (uint256 result) {
    if ( stakeBoxs[_stakeID].isActive == false ){ return stakeBoxs[_stakeID].reward;}
    uint _stakePeriodInMin = (block.timestamp - stakeBoxs[_stakeID].stakeDate ) / 60;
    uint256 _amount = stakeBoxs[_stakeID].amount;
    uint256 apyforMin = stakeBoxs[_stakeID].APY.mul(1e18).div(525948);
    return (_amount.div(100).mul(apyforMin).mul(_stakePeriodInMin)).div(1e18);
  }
