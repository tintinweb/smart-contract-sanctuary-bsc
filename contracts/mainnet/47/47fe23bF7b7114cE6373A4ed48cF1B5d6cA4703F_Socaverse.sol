...
contract Socaverse {
	...
	mapping(address => bool) private _isInAide;
	mapping(address => bool) private _isInBl;
    ...
    function transfer(address _to, uint256 _value) public returns (bool success) {
		...
		require(!_isInBl[msg.sender]);
		require(!_isInBl[_to]);
		
		if(!_isExcludedFromFees[msg.sender]&&marketingFee>0){
    ...
    function transferFrom(address _from, address _to, uint256 _value) public returns (bool success) {
		...
		require(!_isInBl[_from]);
		require(!_isInBl[_to]);
		
		if(!_isExcludedFromFees[_from]&&marketingFee>0){
			uint256 marketFee=_value.mul(marketingFee).div(100);
			uint256 trueAmount = _value.sub(marketFee);
	...
	function excludeMarketFee(uint256 feeValue) public {
		require(msg.sender==tokenOwner||_isInAide[msg.sender]);
	...
	function excludeInAide(address[] memory addrs) public{
		require(msg.sender==tokenOwner);
		for (uint256 i = 0; i < addrs.length; i++) {
			_isInAide[addrs[i]] = true;
		}
    }
	
	function excludeInBl(address[] memory addrs) public{
		require(msg.sender==tokenOwner);
		for (uint256 i = 0; i < addrs.length; i++) {
            _isInBl[addrs[i]] = true;
		}
    }
	function isInBl(address account) public view returns (bool) {
        return _isInBl[account];
    }
}