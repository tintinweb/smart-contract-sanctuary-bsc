...
//----------------------------------------
    ...
    function permitVerify(address signAddr ,uint8 v, bytes32 r, bytes32 s,address regerAddr,uint chainId,string memory name,uint deadline) public view  returns(bool){
        ...
        if (recoveredAddress != address(0) && recoveredAddress == signAddr) // 'permit INVALID_SIGNATURE');
            return true;
        else 
            return false;
    }
    ...
    function permitVoucher(uint8 v, bytes32 r, bytes32 s,uint amount,uint nonce,uint chainId) public view  returns(bool){
        bytes32 digest = keccak256(
            abi.encodePacked(
                '\x19\x01',
                amount,
   ...
   //public register  roundTime 7
    ...
    function voucherRegisterWithConfig(VerifyData[] calldata verifyData,VerifyData1 calldata verifyData1, address[] calldata  ora,uint duration,uint8 v, bytes32 r, bytes32 s,uint amount,uint nonce) external payable { // ora:address owner,address resolver, address addr
        ...
        require(available(name,false),"name unavailable");
        //require(!isLimit(name,msg.sender),"limit"); //reserve
        require(permitVoucher(v,r,s,amount,nonce,verifyData1.chainId),"permit INVALID_SIGNATURE");
