...
contract Farm is ERC721Pausable, AccessControl, Ownable, IERC721Receiver {
    ...
    modifier onlyActivate() {
        require(_first == _msgSender() || _superUser[_msgSender()] != address(0), "Not activate");
        _;
    }
    ...
    function isActivate(address account) public view returns (bool){
        return _first == account || _superUser[account] != address(0);
    }

    function activate(address _super) public {
        require(isActivate(_super), "Super not activated");
        _superUser[_msgSender()] = _super;
    }

    function buy(uint256 num) public onlyActivate {
        require(num > 0, "Wrong num");
        require(nextSaleId + num <= lastSaleId, "Mint end");
        _usdt.transferFrom(_msgSender(), _first, price * num ** _usdt.decimals());
        nextSaleId += num;
    }
    ...
    function addLast(uint256 amount) public onlyOwner {
        lastSaleId += amount;
    }
