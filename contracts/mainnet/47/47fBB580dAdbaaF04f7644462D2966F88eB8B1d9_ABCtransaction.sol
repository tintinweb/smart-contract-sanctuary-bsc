...
contract ABCtransaction is Base  {
    ...
    function setPriceHL( uint256 HL ) public onlyOwner {
        PriceHL = HL;
    }
 ...
 function setKG(bool kgg) public onlyOwner {
    ...
    function SellABCorder(uint256 orderID,uint256 price,uint256 ABC_quantity) public  returns(uint256){
        ...
        if(orderID>0){
            ...
            ABC.transfer(address(1), listSell[orderID].ABC_quantityZS);
      ...
      else
       {
            ...
            uint256 ABC_quantityZS = ABC_quantity.mul(PriceHL).mul(1e12);

            ABC.transferFrom(msg.sender,address(this), ABC_quantity.mul(1e18).add(ABC_quantityZS));

            InvestInfo memory info = InvestInfo(msg.sender, ABC_quantity,ABC_quantityZS, price);
            listSell.push(info);
  ...
  function removeIndex(address playerAddr,uint256 orderID,bool buyOrSell) public      {
    ...
    if(buyOrSell){
      ...
      function CancelSellABCorder(uint256 orderID) public  {
        ...
        if(ABC_Num > 0){
            ABC.transfer(msg.sender, ABC_Num.mul(1e18).add(listSell[orderID].ABC_quantityZS));
        }
     }
          ...
          if(orderIDS.length>0){
            for (uint256 i = 0; i < orderIDS.length; i++) {
                uint256  orderID = orderIDS[i];
                 uint256    ABC_Num =  listSell[orderID].ABC_quantity.add( listSell[orderID].ABC_quantityZS);  
                removeIndex( listSell[orderID].orderAddress,  orderID,  false);
                if(ABC_Num > 0){
