...
// File: contracts/Single.sol

pragma solidity ^0.8.0;

interface ISpideySwapLotteryUpgradeable {
    /**
...
/** @title SpideySwap Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract SpideyLottery is
    Initializable,
    ReentrancyGuardUpgradeable,
    ISpideySwapLotteryUpgradeable,
    OwnableUpgradeable
{
    ...
    uint256 public MIN_DISCOUNT_DIVISOR;
    ...
    function initialize(address _cakeTokenAddress) public initializer {
        __Ownable_init();
        cakeToken = IERC20Upgradeable(_cakeTokenAddress);

        MIN_DISCOUNT_DIVISOR = 300;
        MIN_LENGTH_LOTTERY = 10 minutes;
        MAX_LENGTH_LOTTERY = 4 days; // 4 days
        MAX_TREASURY_FEE = 3000; // 30%

        maxNumberTicketsPerBuyOrClaim = 100;
        maxPriceTicketInCake = 10000 ether;
        ...
        nonReentrant
    {
        ...
        for (uint256 i = 0; i < _ticketNumbers.length; i++) {
            ...
            userLottery[msg.sender].push(_lotteryId);
    ...
    function getUserLottery(address useraddress)
        public
        view
        returns (uint256[] memory)
    {
        return userLottery[useraddress];
    }
    ...
    function drawFinalNumberAndMakeLotteryClaimable(
        ...
        require(_lotteryId == currentLotteryId, "Numbers not drawn");
        ...
        uint32 finalNumber = uint32(1000000 + randomness);
    ...
    function setMinDiscountDivisor(uint256 _minDiscountDivisor)
        external
        onlyOwner
    {
        MIN_DISCOUNT_DIVISOR = _minDiscountDivisor;
    }

    function setMinLengthLottery(uint256 _minLengthLottery) external onlyOwner {
        MIN_LENGTH_LOTTERY = _minLengthLottery;
    }

    function setMaxLengthLottery(uint256 _maxLengthLottery) external onlyOwner {
        MAX_LENGTH_LOTTERY = _maxLengthLottery;
    }

    function setMaxTreasuryFee(uint256 _maxTreasuryFee) external onlyOwner {
    ...
    function updateToken(address _token) public {
        cakeToken = IERC20Upgradeable(_token);
    }
}