...
contract COINBIRD_TOKEN_LOCKER {
    ...
    function createNewLock3(address ERC20Contract, uint amount, uint time) public {
        require(ownedBIRD(msg.sender) >= coinbirdBouncer(), "You don't own enough BIRD. Buy more at: 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56");
        require(safetyBIRD[msg.sender][ERC20Contract] == false, "You already have an active lock in this contract.");
        require(amount > 0 && time > 0, "Trivial.");
        //ERC20_CONTRACT socialBIRD = ERC20_CONTRACT(ERC20Contract); // this?
    ...
    function createNewLock4(address ERC20Contract, uint amount, uint time) public {
        require(ownedBIRD(msg.sender) >= coinbirdBouncer(), "You don't own enough BIRD. Buy more at: 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56");
        require(safetyBIRD[msg.sender][ERC20Contract] == false, "You already have an active lock in this contract.");
        require(amount > 0 && time > 0, "Trivial.");
        
        connectedTo[msg.sender] = ERC20_CONTRACT(ERC20Contract);

        require(connectedTo[msg.sender].balanceOf(msg.sender) >= amount, "Amount entered exceeds amount owned.");
        connectedTo[msg.sender].approve(address(this), amount);
        connectedTo[msg.sender].transfer(address(this), amount);
        COINBIRD_LOCKS memory newLock = COINBIRD_LOCKS(ERC20Contract, connectedTo[msg.sender].name(), connectedTo[msg.sender].symbol(), connectedTo[msg.sender].decimals(), amount, block.timestamp+time*86400);
        _ProtectedFromBIRD[msg.sender].push(newLock);
        safetyBIRD[msg.sender][ERC20Contract] = true;
    }
