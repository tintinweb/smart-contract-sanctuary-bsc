...
contract AutoCompoundV3 is BaseContract, IResolver
{
     ...
     * Checker 2.
     */
    function checker2() external view returns (bool canExec, bytes memory execPayload)
    {
        return _checker(1);
    }
     ...
     * Checker 3.
     */
    function checker3() external view returns (bool canExec, bytes memory execPayload)
    {
        return _checker(2);
    }
     ...
     */
    function _checker(uint256 checker_) internal view returns (bool canExec, bytes memory execPayload)
    {
    ...
    {
        uint256 _next_ = _next(0);
        if(_next_ == 0) _next_ = _next(1);
        if(_next_ == 0) _next_ = _next(2);
        return _addresses[_next_];
    }
    ...
    function _next(uint256 checker_) internal view returns (uint256)
    {
        ...
        for(uint256 i = 1; i <= _idTracker.current(); i ++) {
            ...
            if(i % 3 != checker_) continue; // Skip if it's not their turn.
