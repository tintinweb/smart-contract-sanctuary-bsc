...
contract FireCatPool is IFireCatPool, ModifyControl, FireCatTransfer {
    ...
    */
    ...
    function calculateEarnings(uint256 amount, uint256 start_, uint256 end_)  public view returns(uint256) {
        ...
        uint256 _timePeriod = end_ - start_;

        // uint256 res = amount * (10 ** 18 * _timePeriod / _cycleTime + rewardRatePerSecond * _timePeriod) / (10 ** 18);
        return amount.mul(_baseUnit.mul(_timePeriod).div(_cycleTime).add(rewardRatePerSecond.mul(_timePeriod))).div(_baseUnit);
    }
    ...
    function _shareInviterReward(address user_, uint256 amount_) internal {
        ...
        uint256 reward = amount_.mul(inviterRewardRate).div(1000000000);  // 17e6 / 1e9 = 0.017
        _inviterReward[inviter] = _inviterReward[inviter].add(reward);
    }

    function _shareReservesReward(uint256 amount) internal {
        uint256 reward = amount.mul(reservesRewardRate).div(1000000000);  // 17e6 / 1e9 = 0.017
