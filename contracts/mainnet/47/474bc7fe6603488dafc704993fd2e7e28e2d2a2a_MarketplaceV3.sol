...
contract MarketplaceV3 is Initializable, AccessControlUpgradeable, PausableUpgradeable {
  ...
  function _executeOrder(uint256 orderId, bool payWithBusd) internal whenNotPaused {
    ...
    if (payWithBusd) {
      emit OrderExecuted(orderId, assetId, seller, amount, nftAddress, price, msg.sender, true);
    } else {
      emit OrderExecuted(orderId, assetId, seller, amount, nftAddress, price, msg.sender, false);
    }
    // Transfer asset
    if (assetType == AssetType.ERC20) {
      IERC20 nft = IERC20(nftAddress);
