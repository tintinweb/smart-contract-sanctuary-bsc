...
contract NFKeyToken is IERC721, Operable, Pausable, ERC721Enumerable {
	// An address who has permissions to mint Keys
	mapping(address => bool) public minters;
	address[] public mintersList;

	// The Keys token URI descriptor
	IKeysDescriptor public descriptor;
	bool public isDescriptorLocked;

	mapping(uint256 => IKeysDescriptor.Key) public keys;

	// The internal key ID tracker
	uint256 private currentKeyId;
	...
	constructor(IKeysDescriptor _descriptor) ERC721("NFKey", "NFKEY") {
	...
	function setContractURIHash(string memory newContractURIHash) external onlyOperator {
		contractURIHash = newContractURIHash;
	}

	function setProxyRegistry(IProxyRegistry newProxyRegistry) external onlyOperator {
	...
	function mint(
		...
		uint48 label,
		uint48 access
	) public onlyMinter whenNotPaused returns (uint256) {
		require(background < descriptor.backgroundsCount(), "Background idx not exist");
		require(head < descriptor.layersCount(0), "Head idx not exist");
		require(body < descriptor.layersCount(1), "Body idx not exist");
		require(label < descriptor.layersCount(2), "Label idx not exist");
		require(access < descriptor.layersCount(3), "Access idx not exist");
		currentKeyId++;
		keys[currentKeyId] = IKeysDescriptor.Key(background, head, body, label, access);

		_mint(address(this), to, currentKeyId);
		emit KeyCreated(currentKeyId);

		return currentKeyId;
	}

	function keyAccess(uint256 tokenId) public view returns (uint48 access, string memory name) {
		require(_exists(tokenId), "KeysToken: URI query for nonexistent token");
		return descriptor.getKeyAccess(keys[tokenId]);
	}

	function tokenURI(uint256 tokenId) public view override returns (string memory) {
		require(_exists(tokenId), "KeysToken: URI query for nonexistent token");
		return descriptor.dataURI(tokenId, keys[tokenId]);
	}
	...
	function setDescriptor(IKeysDescriptor _descriptor) public onlyOperator whenDescriptorNotLocked {
		descriptor = _descriptor;
		emit DescriptorUpdated(_descriptor);
	}

	function lockDescriptor() public onlyOperator whenDescriptorNotLocked {
	...
	// ----------------------------- EVENTS --------------------------------
	event KeyCreated(uint256 indexed tokenId);
	event KeyBurned(uint256 indexed tokenId);
	event MinterSet(address minter, bool state);
	event DescriptorUpdated(IKeysDescriptor descriptor);
