...
interface ILaunchpad {
    function getUserContribution(address) external view
...
contract Helper is Ownable {
    ...
    returns (UserTokensToReceive[] memory usersTokensToReceive){
        ...
        for (uint256 i = 0; i < users.length; i++) {
            (, uint256 tokensToReceive) = launchpad.getUserContribution(users[i]);
            usersTokensToReceive[i] = UserTokensToReceive(users[i], tokensToReceive);
        }
