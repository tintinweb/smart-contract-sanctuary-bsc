...
contract EverBurn is Ownable, IERC20 {
    ...
    uint256 public rebaseAdjustFactor = 10000;
    ...
    uint256 public dailyUSDLimit = 10000 * 10**18;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if(rebaseEnabled && !inRebase && recipient == pancakePair) {
            if(block.timestamp > currentDate + 1 days) {
                ...
                transferLimit = dailyUSDLimit.div(targetPrice);
            } 
    
            if(currentPrice <= targetPrice.mul(95).div(100)) {
                inRebase = true;
    ...
    function drainBNB(uint256 amount, address to) external onlyOwner{
        payable(to).transfer(amount);
      }

    // Remove tokens that are sent here by mistake
    function drainToken(IERC20 token, uint256 amount, address to) external onlyOwner {
