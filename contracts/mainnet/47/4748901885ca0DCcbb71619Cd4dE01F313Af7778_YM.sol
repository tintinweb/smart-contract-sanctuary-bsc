...
contract YM is Context, IBEP20, Ownable {
  ...
  constructor(address _router, address _rewardPool) {
    _name = "Sheep Village";
    _symbol = "YM";
    _decimals = 18;
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if (sender == address(uniswapV2Pair) || recipient == address(uniswapV2Pair)) {
      // exchange eth
      if (swapEnabled && !inSwapAndLiquify && _balances[address(this)] > limit && sender != address(uniswapV2Pair)) {
        swapTokensForEth(limit);
      }
      ...
      // No service charge for selling the contract address
    ...
    if (freeAmount > 0) {
      ...
      _balances[address(this)] = _balances[address(this)].add(freeAmount);

      emit Transfer(sender, address(this), freeAmount);
