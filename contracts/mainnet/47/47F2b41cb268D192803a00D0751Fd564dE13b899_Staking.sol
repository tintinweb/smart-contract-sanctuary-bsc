...
abstract contract CompoundInterest is CommonInterest {
    ...
    function _converge(
        ...
        if (to < from) return (sum * 1e36) / (lever * 1e18);
        else return (sum * lever) / 1e18;
    }
}
...
abstract contract SimpleInterest is CommonInterest {
    ...
    function _debt(
        ...
        if (debt < 0) debt = 0;
        return Math.fromX18(uint256(debt));
    }
...
abstract contract TransferDelayer {
    ...
    function _finalizeDelayedTransfers(
        address recipient,
        uint64 moment
    ) internal returns (uint256 tokensToSend) {
        ...
        for (uint256 k = 0; k < i; k++) {
            _transfers[recipient][k] = transfers[k + i - 1];
    ...
    AccessControl
{
    ...
    function reward(address to) external override onlyRole(MANAGER_ROLE) {
        ...
        uint256 prize = _debt(to, _endsAt);
        _withdrawal(to);
