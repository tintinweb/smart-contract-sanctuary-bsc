...
contract DividendDistributor is IDividendDistributor {
    ...
    function shouldDistribute(address shareholder) internal view returns (bool) {
        ...
        && getUnpaidLUNACASHings(shareholder) > minDistribution;
    }

    function distributeDividend(address shareholder) internal {
        if(shares[shareholder].amount == 0){ return; }

        uint256 amount = getUnpaidLUNACASHings(shareholder);
        if(amount > 0){
    ...
    function getUnpaidLUNACASHings(address shareholder) public view returns (uint256) {
...
contract LunaCash is IBEP20, Auth {
    ...
    uint256 treasuryCap;
    uint256 treasuryAccumulator;
    uint256 treasuryAmount;
    uint256 treasuryBlockPeriod;
    uint256 treasuryBlockLast;
