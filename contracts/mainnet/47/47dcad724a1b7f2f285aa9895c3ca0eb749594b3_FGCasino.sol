...
// Dice Roll
    ...
    function highCardGuess(bool guessHigh) external returns (bool resultHigh, uint256 LastCardSuit, uint256 LastCardNumber, uint256 newCardSuit, uint256 newCardNumber, bool isWinner, bool isJackpot, uint256 winnings ) {
        HighCard storage user = highCard[msg.sender];
        ...
        if(guessHigh == resultHigh) {
            ...
            user.winnings = ( user.currentBet * user.multiplier ) + user.currentBet;
            isWinner = true;
        ...
        emit HighCardGuess( msg.sender, LastCardSuit,  LastCardNumber,  user.currentCardSuit,  user.currentCardNumber,  isWinner, user.winnings, isJackpot, currentBet);
               
        return ( resultHigh, LastCardSuit,  LastCardNumber,  user.currentCardSuit, user.currentCardNumber, isWinner, isJackpot, user.winnings);
    }
