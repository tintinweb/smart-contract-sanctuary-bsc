    ...
    ReentrancyGuard
{
    ...
    uint256 maxSummonCount;

    event AnitoMinted(
        address indexed player,
        uint256 indexed tokenId,
        uint256 indexed gachaCategory
    );
    ...
    mapping(uint256 => AnitoInfo) public anitoInfo;
    mapping(uint256 => uint256[]) public summons;
    modifier onlyOperators() {
        require(operationsAddresses[msg.sender] == true);
        _;
    }

    constructor(string memory _notRevealedUri) ERC721("Anito", "ANITO") {
    ...
    function setMaxSummonCount(uint256 _maxSummonCount) public onlyOwner {
        maxSummonCount = _maxSummonCount;
    }

    function safeMintSummoning(
        address to,
        uint256 tokenId,
        uint256 summoner1,
        uint256 summoner2,
        ...
        anitoInfo[tokenId].rarityStoneType = rarityStoneType;
        anitoInfo[tokenId].classStoneType = classStoneType;
        anitoInfo[tokenId].statStoneType1 = statStoneType1;
        anitoInfo[tokenId].statStoneType2 = statStoneType2;
        emit AnitoSummoned(
            to,
            tokenId,
            summoner1,
    ...
    function safeMintSingle(
        ...
        emit AnitoMinted(player, tokenId, gachaCategory);
        return tokenId;
    }
    ...
    function executeSummoning(
        ...
        require(
            ownerOf(summoner1) == player && ownerOf(summoner2) == player,
            "Only owner"
        );
        require(summoner1 != summoner2, "Invalid masters combination");
        require(
            anitoInfo[summoner1].summonCount < maxSummonCount &&
                anitoInfo[summoner2].summonCount < maxSummonCount,
            "Maximum apprentice reached"
        );

        _tokenIdCounter.increment();
        uint256 tokenId = _tokenIdCounter.current();

        anitoInfo[summoner1].summonCount++;
        anitoInfo[summoner2].summonCount++;

        summons[summoner1].push(tokenId);
        summons[summoner2].push(tokenId);
    ...
    function getAnitoOfAddress(
        address player
    ) public view returns (uint256[] memory, AnitoInfo[] memory) {
        uint256 currentBalance = balanceOf(player);
        uint256[] memory tokens = new uint256[](currentBalance);
        AnitoInfo[] memory myAnito = new AnitoInfo[](currentBalance);
        for (uint256 i = 0; i < tokens.length; i++) {
            tokens[i] = tokenOfOwnerByIndex(player, i);
            myAnito[i] = anitoInfo[tokens[i]];
        }
        return (tokens, myAnito);
    }
}
