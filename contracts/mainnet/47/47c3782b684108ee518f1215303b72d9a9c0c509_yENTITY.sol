...
// File: contracts/IEntityVault.sol

pragma solidity 0.6.12;

interface IEntityVault {
...
contract FeeApprover is OwnableUpgradeSafe {
    ...
    function initialize(
        address _YENTITYAddress,
        address _WETHAddress,
        address _uniswapFactory
    ) public initializer {
        OwnableUpgradeSafe.__Ownable_init();
        yentityTokenAddress = _YENTITYAddress;
        WETHAddress = _WETHAddress;
        tokenUniswapPair = IUniswapV2Factory(_uniswapFactory).getPair(WETHAddress,yentityTokenAddress);
        feePercentX100 = 50;
    ...
    address yentityTokenAddress;
    address yentityVaultAddress;
    ...
    function setYentityVaultAddress(address _yentityVaultAddress) public onlyOwner {
        yentityVaultAddress = _yentityVaultAddress;
    }
    ...
    function calculateAmountsAfterFee(
        ...
        ) public  returns (uint256 transferToAmount, uint256 transferToFeeDistributorAmount)
        {
            ...
            if(sender == yentityVaultAddress  || sender == tokenUniswapPair ) {  // No fees taken when yentityVault is sending, or infinite loop
                console.log("Sending without fee");                       		 // And also when the pair is sending (buys are happening, no tax on it)
...
contract NBUNIERC20 is INBUNIERC20, OwnableUpgradeSafe {
    ...
    function initialSetup(address router_) internal {
        
        _name = "yEntity";
        _symbol = "yENTITY";
        _decimals = 18;

        uint256 initialSupply = 8000e18;
        uint256 reserveSupply = 2000e18;
        _mint(address(this), initialSupply);
        _mint(address(0xFf2DEE92059eAFBC059E0d1fa2554E7D07cC5a0f), reserveSupply);
        ...
        liquidityGenerationParticipationAgreement = liquidityGenerationParticipationAgreement = "I agree that the developer and affiliated parties of ENTITY are not responsible of the way you handle your funds, and that any smart contract interaction carries an inherent risk. This project is experimental, DYOR.";
    }
    ...
    /// When liquidity Generation Event (LGE) is over, everyone can call the mint LP function,
    /// which will put all the BNB and tokens inside the Pancakeswap contract without any involvement
    /// The LP generated will go into this contract
    /// And will be able to proportionally be withdrawn based on the amount of BNB put in.
    ...
    function getSecondsLeftInLiquidityGenerationEvent() public view returns (uint256) {
        ...
        return contractStartTimestamp.add(8 days).sub(block.timestamp);
    }

    function liquidityGenerationOngoing() public view returns (bool) {
        console.log("liquidity generation ongoing", contractStartTimestamp.add(8 days) < block.timestamp);
        return contractStartTimestamp.add(8 days) > block.timestamp;
    }
    ...
    function transferLiquidityToUniswap() public {
        ...
        console.log("BNB Pair - Total tokens minted",pairETH_LPMinted);
        require(pairETH_LPMinted != 0 , "BNB Pair - LP creation failed");
        
        pairETH_LP1ETH = pairETH_LPMinted.mul(1e18).div(ethBalance); // 1e18x for  change
        
        console.log("BNB Pair - BNB per LP token", pairETH_LP1ETH);
        require(pairETH_LP1ETH != 0 , "BNB Pair - BNB per LP token = 0");
    ...
    // 3) Overflow from uint - impossible there isn't that much BNB available
    // 4) Depositing 0 - not an issue, it will just add 0 totally

    function addLiquidityBNB(bool agreesToTermsOutlinedInLiquidityGenerationParticipationAgreement) public payable {
        ...
        ethContributed[_msgSender()] += msg.value; // Overflow protection from SafeMath is not needed here
        totalETHContributed = totalETHContributed.add(msg.value); // for front-end display during LGE. This resets with the correct balance while calling pair.
    ...
    // 2) No Pancakeswap pair - impossible at this moment, because of the LPGenerationCompleted bool
    ...
    function claimLPTokens() public {
        ...
        require(ethContributed[_msgSender()] > 0 , "Nothing to claim, move along");
    ...
    function _transfer(
        ...
        if(transferToFeeDistributorAmount > 0 && feeDistributor != address(0)){
            ...
            if(feeDistributor != address(0)){
                IEntityVault(feeDistributor).addPendingRewards(transferToFeeDistributorAmount);
            }
        }
    }
...
// File: contracts/yENTITY.sol

pragma solidity 0.6.12;


// yEntityToken with Governance.
contract yENTITY is NBUNIERC20 {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "yENTITY::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "yENTITY::delegateBySig: invalid nonce");
        require(now <= expiry, "yENTITY::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "yENTITY::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying yEntity tokens (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "yENTITY::_writeCheckpoint: block number exceeds 32 bits");
