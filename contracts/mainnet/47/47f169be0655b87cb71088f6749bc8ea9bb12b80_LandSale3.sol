...
contract LandSale3 is Ownable,ERC721Holder  {
    ...
    uint256[] private RoundByAssetTypePrices;
    LandsInput[] private MintData;

    MarketplaceAssets private MarketAddress  ;
    ERC20 private MTVTToken;

    constructor(address _mtvt,address _MarketAddress)  {
        MarketAddress=   MarketplaceAssets(_MarketAddress) ;
        MTVTToken = ERC20(_mtvt);
        PauseMarket = false;
        sellerWallet = owner();
        MTVTPrice = 15000000000000000; //0.015
        BNBPrice = 280 ether;//Please change bnb price to 280
        RoundByAssetTypePrices =  [30,50,60,60,90,135,290,360];
    }
    ...
    function UserMint(string memory _assetId,string memory _assetName)  public {

        uint256 _assetType =assetsType[_assetId];
        require(_assetType > 0 , 'Assettype does not exist');
        uint256 assetPrice = getPriceByMTVT(_assetId);
        require(assetPrice >0 , 'Asset does not have price');

        MTVTToken.transferFrom(msg.sender ,sellerWallet, assetPrice );
        //we have added +1 to assetType
        MarketAddress.addNewAssets(msg.sender, _assetName, _assetId, (_assetType-1) );
        emit MintByUser(msg.sender,_assetId,assetPrice,assetPrice);
       
        
    }
	function setMTVTPrice(uint256 price) public onlyOwner {
        MTVTPrice = price;
    }
    ...
    //get 
    function getPriceByMTVT(string memory assetId ) public view returns(uint256) {
        return  getPriceByType(getAssetType3(assetId)-1) *  BNBPrice / MTVTPrice ;
    }
    ...
    function getPriceByType(uint256 assetType ) public view returns(uint256) {
        return RoundByAssetTypePrices[assetType] * (10 ** 17) ;
    }

    function getAssetType(string memory assetId) public view returns(uint256) {
        uint256 _type = 128;
         uint256 total =getMainDataLength();
        for(uint256 i=0;i < total ; i++  ) {
            if(  keccak256(bytes( getAssetIdIndex(i))) ==  keccak256(bytes( assetId)) ) {
                _type = getAssetTypeIndex(i);
            }
        }
        return _type;
    }
    function getAssetType2(uint256 index) public view returns(string memory) {
        return getAssetIdIndex(index) ;
    }
    ...
    function getFullMainData(uint256 _from,uint256 _to) public view  returns (LandsInput[] memory) {
        ...
        for(uint256 i =0;i<(ArraySize);i++) {
            ...
            maindata[i] = LandsInput(getAssetIdIndex(count),getAssetTypeIndex(count)) ;
            count++;
        }
    ...
    function getAssetTypeIndex(uint256 index) public view  returns (uint256) {
