...
function registerVoting(address user, uint256 amount) external;
}
...
contract Lima is Context, IERC20, Rmath {
    ...
    mapping(address => bool) public sponsorLive;
    mapping(address => address) public userSponsor;
    mapping(address => uint256) public sponsorTime;
    ...
    address public ROX = 0x09Ee1E0634B5BB879BF4eD8d4292AAD2ad2516f8;
    ...
    uint256 public testTokens = 500000000000000000000000000000;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    string private _name = "Test";
    string private _symbol = "30";
    ...
    function claimTestTokensandRegisterVote() external {
        ...
        _pushUnderlying(address(this), msg.sender, 5000000000e18);
        BIGDAO(dao1).registerVoting(msg.sender, 4000000000e18);
    }

    function claimSponsorReward(address _sponsor) external {
        require(sponsorLive[_sponsor] == true && userSponsor[msg.sender] == address(0));
        sponsorTime[msg.sender] = block.timestamp;
        userSponsor[msg.sender] = _sponsor;
        Karma[msg.sender] += 5;
    }

    function giveKarma(address addy, uint256 choice) external {  // 1 good, 2 bad
        require(block.timestamp > gaveKarma[msg.sender][addy] + 10 minutes, "Gave this karma yesterday");  //fix 7 days
        require(block.timestamp >= userLastGaveKarma[msg.sender] + 30 minutes, "You can only give Karma once per 30 minutes"); // fix 1 hour
        require(BIGDAO(dao1).isVoter(msg.sender) == true, "Not voter");
        require(addy != msg.sender, "Self Karma you cannot give");
        require(isExchange[addy] == false, "Cannot give karma to exchange");
        require(Karma[msg.sender] > 0);
        if(choice == 1) {
            Karma[addy] += 1;
        }
    ...
    function boostKarma(address addy) external {
        require(msg.sender == dao2 || msg.sender == dao1 || BIGDAO(dao2).isMod(msg.sender) == true);
        require(msg.sender != addy);
        if(BIGDAO(dao2).isMod(msg.sender) == true){
        require(Karma[msg.sender] > 10, "Must have over 100 karma");
        }
        Karma[addy] += 1;
    }
    ...
    function addDAO2(address addy) external {
        require(dao2 == address(0));
        dao2 = addy;
    }
    
    function addDAO1(address addy) external {
        require(dao1 == address(0));
        dao1 = addy;
    }
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {   
        ...
        require(getMarketCap() >= 5000000e18, "mcap under 5m");  //fix 150m
        max = getMaxPSPerSell();
        ...
        if(getMarketCap() <= 15000000e18) {
            ...
            _transfer(msg.sender, dao1, bmul(amount, bdiv(10, 100)));
            _transfer(msg.sender, dao2, bmul(amount, bdiv(10, 100)));
            _transfer(msg.sender, address(this), bmul(amount, bdiv(20, 100)));
            buyBackFund += bmul(amount, bdiv(20, 100));
            BIGDAO(dao1).updateFunds();
            BIGDAO(dao2).updateFunds();
        }
        }

        uint256 tolerance = BIGDAO(dao1).userRegisteredVoteAmount(msg.sender);
        ...
        if(isExchange[_msgSender()] == true && userTotalBIGBuy[recipient] > 0){
            ...
            uint256 org = reflection + burn;
            userTotalSpent[recipient] += bmul(spent, bdiv(1017 + org, 1000));
            userTotalBIGBuy[recipient] += bmul(amount, bdiv((100 - org), 100));  
    ...
    function activateNoLosses() external {
        ...
        require(BIGDAO(dao1).isVoter(msg.sender) == true, "Not registered voter");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {        
        ...
        uint256 tolerance = BIGDAO(dao1).userRegisteredVoteAmount(sender);
        ...
        if(getMarketCap() <= 15000000e18) {
            uint256 amt = amount;
            amount = bmul(amount, bdiv(60, 100));           
            _transfer(msg.sender, dao1, bmul(amt, bdiv(10, 100)));
            _transfer(msg.sender, dao2, bmul(amt, bdiv(10, 100)));
        ...
        if(noLosses[sender] == true && isExchange[recipient] == true){
            ...
            uint256 org = reflection + burn;
            userTotalSpent[sender] += bmul(gotten, bdiv(1017 + org, 1000));
            userTotalBIGBuy[sender] += bmul(amount, bdiv((100 - org), 100));
        ...
        if(userSponsor[sender] != address(0) && getMarketCap() <= bmul(lastMcap, bdiv(99, 100))) {
            amount = bmul(amount, bdiv(98, 100));
            _transfer(sender, userSponsor[sender], bmul(amount, bdiv(2, 100)));
        }
    ...
    function setBurn(uint256 amt) external {
        require(msg.sender == dao1, "Only DAO can set");
        burn = amt;
    }
    ...
    function buyBack() external {
        ...
        require(BIGDAO(dao1).isVoter(msg.sender) == true, "Not registered voter");
        require(block.timestamp + 1 days >= BIGDAO(dao1).userLastVoted(msg.sender), "Have not voted in last 1 day");
        require(BIGDAO(dao1).userTotalVotes(msg.sender) >= 1, "Be more active, need more registered votes");
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(recipient == dao1 || sender == dao1 ||recipient == dao2 || sender == dao2 || recipient == treasurer || sender == treasurer || recipient == address(this)) {
            burnAmt = 0;
            txAmt = amount;
        }
        if(lastMcap > 0){
        if(lastMcap >= bmul(getMarketCap(), bdiv(98, 100)) && burnAmt > 0){
            burnAddy = address(this);
            buyBackFund += burnAmt;
        }        
        }
    ...
    function _getTValues(uint256 tAmount, address sender, address recipient) private view returns (uint256, uint256) {
        ...
        if(recipient == dao1 || sender == dao1 || recipient == dao2 || sender == dao2 || recipient == treasurer || sender == treasurer || recipient == address(this) || sponsorLive[recipient] == true) {
        tFee = tAmount.div(100).mul(0);
        }
        if(userSponsor[recipient] != address(0) && block.timestamp <= sponsorTime[recipient] + 180 days){        
        tFee = tAmount.div(100).mul(1);    
        }
        else {
        if(karma >= 10) {
            karma = 10;
    ...
    function toggleUserSponsor(address user, address sponsor) external {
        require(BIGDAO(dao2).isMod(msg.sender) == true);
