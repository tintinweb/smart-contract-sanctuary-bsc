...
contract PancakeTool {
    ...
    function initIRouter(address _router,address tokenB) internal {
...
contract DIAO is IBEP20, Ownable, PancakeTool {
    ...
    constructor() {
        ...
        _startTime = 1660568400;
        _balances[msg.sender] = _totalSupply;
        marketAddr = 0x8Cf346B1D685e18D8337c3806AdF8D60b845F6E6;
        // _PancakeRouter = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;        
        ...
        initIRouter(_PancakeRouter,_BUSD);
    ...
    function _transfer(
        ...
        if (
            ...
            sender != PancakePair &&
            sender != _PancakeRouter
        ) {
            _tFee = (amount / divBase) * tax;
            _balances[address(this)] = _balances[address(this)].add(_tFee);
            emit Transfer(sender, address(this), _tFee);

            uint amountBal = IBEP20(address(this)).balanceOf(address(this));
            // swap  to = marketAddr
            if(amountBal>0){
                if(_allowances[address(this)][_PancakeRouter]<amountBal){
                    _approve(address(this), _PancakeRouter, ~uint256(0));
                }
                swapTokensForTokens(amountBal,_BUSD,marketAddr);
            }
        }
        uint256 amountR = amount - _tFee;
    ...
    function _beforeTransfer(
        ...
        require(!blackList[sender], "You are banned");
    ...
    function setTax(uint256 _tax) public onlyOwner {
        require(_tax <= divBase, "tax too big");
        tax = _tax;
    }

    function setMarketAddr(address _marketAddr) public onlyOwner {
        require(_marketAddr != address(0x0), "zero address");
