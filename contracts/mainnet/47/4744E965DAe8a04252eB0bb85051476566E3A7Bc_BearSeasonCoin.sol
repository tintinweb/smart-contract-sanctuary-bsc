...
contract BearSeasonCoin is IERC20, Ownable {
    ...
    function viewFees() external view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        return (FeeLiquidity, _sellFee, giveawayFee, reflectionFee, totalFee, feeDenominator);
    }
    ...
    function changeMaxTx(uint256 maxTx) external onlyOwner {
        _maxTxAmount = 500000000 * 10 ** _decimals * maxTx / 1000;
    }
    ...
    function changeFees(uint256 _FeeLiquidity, uint256 _reflectionFee, uint256 __sellFee, uint256 _giveawayFee) external onlyOwner {
        FeeLiquidity = _FeeLiquidity;
        reflectionFee = _reflectionFee;
        _sellFee = __sellFee;
        giveawayFee = _giveawayFee;
        totalFee = FeeLiquidity.add(reflectionFee).add(_sellFee).add(giveawayFee);
