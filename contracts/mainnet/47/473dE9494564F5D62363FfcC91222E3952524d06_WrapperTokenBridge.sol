...
contract WrapperTokenBridge is Ownable, ReentrancyGuard
{
	...
	function deposit(uint256 _pid, uint256 _type, address _token, uint256 _amountIn, uint256 _minAmountOut) external returns (uint256 _amountOut, uint256 _reward)
	{
		return deposit(_pid, _type, _token, _amountIn, _minAmountOut, address(0));
	}

	function deposit(uint256 _pid, uint256 _type, address _token, uint256 _amountIn, uint256 _minAmountOut, address _referral) public nonReentrant returns (uint256 _amountOut, uint256 _reward)
	{
		require(_type < 3, "invalid type");
		(address _ptoken,,,,,,,) = MasterChef(masterChef).poolInfo(_pid);
		if (_type == 1) {
			_ptoken = IWrapperToken(_ptoken).token();
		}
		else
		if (_type == 2) {
			_ptoken = IMagikVault(_ptoken).want();
		}
		...
		if (_token == _ptoken) {
			...
			address _token0 = IUniswapV2Pair(_ptoken).token0();
			address _token1 = IUniswapV2Pair(_ptoken).token1();
			if (_token == _token0) {
				(uint256 _swapInAmount0, uint256 _swapOutAmount1) = _zapin0(_ptoken, _token0, fee, _amountIn, address(this));
				_amountOut = _mint(_ptoken, _token0, _token1, _amountIn - _swapInAmount0, _swapOutAmount1, address(this));
			}
			else
			if (_token == _token1) {
				(uint256 _swapInAmount1, uint256 _swapOutAmount0) = _zapin1(_ptoken, _token1, fee, _amountIn, address(this));
				_amountOut = _mint(_ptoken, _token0, _token1, _swapOutAmount0, _amountIn - _swapInAmount1, address(this));
			}
			else {
		...
		if (_type == 1) {
			...
			IERC20(_ptoken).safeApprove(_wtoken, _amountOut);
			IWrapperToken(_wtoken).wrap(_amountOut);
			_amountOut = IERC20(_wtoken).balanceOf(address(this));
			IERC20(_wtoken).safeApprove(masterChef, _amountOut);
		}
		else
		if (_type == 2) {
			(address _vtoken,,,,,,,) = MasterChef(masterChef).poolInfo(_pid);
			IERC20(_ptoken).safeApprove(_vtoken, _amountOut);
			IMagikVault(_vtoken).deposit(_amountOut);
			_amountOut = IERC20(_vtoken).balanceOf(address(this));
			IERC20(_vtoken).safeApprove(masterChef, _amountOut);
		}
		else {
			IERC20(_ptoken).safeApprove(masterChef, _amountOut);
		}
		require(_amountOut >= _minAmountOut, "high slippage");
		_reward = MasterChef(masterChef).depositOnBehalfOf(_pid, _amountOut, msg.sender, _referral);
		return (_amountOut, _reward);
	}

	function withdraw(uint256 _pid, uint256 _type, address _token, uint256 _amountIn, uint256 _minAmountOut) external nonReentrant returns (uint256 _amountOut, uint256 _reward)
	{
		require(_type < 3, "invalid type");
		(address _ptoken,,,,,,,) = MasterChef(masterChef).poolInfo(_pid);
		_reward = MasterChef(masterChef).withdrawOnBehalfOf(_pid, _amountIn, msg.sender);
		_amountIn = IERC20(_ptoken).balanceOf(address(this));
		if (_type == 1) {
			IWrapperToken(_ptoken).unwrap(_amountIn);
			_ptoken = IWrapperToken(_ptoken).token();
			_amountIn = IERC20(_ptoken).balanceOf(address(this));
		}
		else
		if (_type == 2) {
			IMagikVault(_ptoken).withdraw(_amountIn);
			_ptoken = IMagikVault(_ptoken).want();
			_amountIn = IERC20(_ptoken).balanceOf(address(this));
		}
		if (_token == _ptoken) {
			...
			address _token1 = IUniswapV2Pair(_ptoken).token1();
			if (_token == _token0) {
				(uint256 _burnOutAmount0, uint256 _burnOutAmount1) = _burn(_ptoken, _amountIn, address(this));
				uint256 _swapOutAmount0 = _swap1(_ptoken, _token1, fee, _burnOutAmount1, address(this));
				_amountOut = _burnOutAmount0 + _swapOutAmount0;
			}
			else
			if (_token == _token1) {
				(uint256 _burnOutAmount0, uint256 _burnOutAmount1) = _burn(_ptoken, _amountIn, address(this));
				uint256 _swapOutAmount1 = _swap0(_ptoken, _token0, fee, _burnOutAmount0, address(this));
