...
contract CONCERNS is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;

    uint256 public constant VERSION = 1;
    uint256 public constant TOTALSUPPLY = 1 * 10**8 * 10**18;
    uint256 public constant MININUM_TOKEN_BALANCE_FOR_DIVIDENDS =
        1 * 10**2 * 10**18;
    ...
    // Fees
    uint256 public constant tokenRewardsFee = 4;
    uint256 public constant liquidityFee = 2;
    uint256 public constant marketingFee = 6;
    ...
    // ETH
    address public rewardToken = 0x2170Ed0880ac9A755fd29B2688956BD959F933F8;
    ...
    constructor(
        ...
        dividendTracker.initialize(
            rewardToken,
            MININUM_TOKEN_BALANCE_FOR_DIVIDENDS
        );
    ...
    function _transfer(
        ...
        if (takeFee) {
            uint256 fees = amount
                .mul(launchedAt + 1 >= block.number ? 99 : totalFees)
                .div(100);
        ...
        if (launchedAt == 0 && to == uniswapV2Pair) launchedAt = block.number;
    }
