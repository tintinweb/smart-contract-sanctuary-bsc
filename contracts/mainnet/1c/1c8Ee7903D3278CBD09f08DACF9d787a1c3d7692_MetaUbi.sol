...
import "./interface/ITaxDepartment.sol";
import "./interface/ITreasuryDepartment.sol";

/**
 * @title MetaUbi token contract
 * @dev The MetaUbi token has modular systems for tax and treasury handler as well as governance capabilities.
 */
contract MetaUbi is IERC20,  Ownable {
    ...
    ITaxDepartment public taxDepartment;

    /// @notice The contract that performs treasury-related operations.
    ITreasuryDepartment public treasuryDepartment;

    /// @notice Emitted when the tax handler contract is changed.
    event TaxDepartmentChanged(address oldAddress, address newAddress);

    /// @notice Emitted when the treasury handler contract is changed.
    event TreasuryDepartmentChanged(address oldAddress, address newAddress);

    /// @notice Emitted when the tax ed.
    event TaxPaid(address from, uint256 originalAmount, uint256 taxAmount);
     ...
     * @param taxDepartmentAddress Initial tax handler contract.
     * @param treasuryDepartmentAddress Initial treasury handler contract.
     */
    constructor(
        string memory name_,
        string memory symbol_,
        address taxDepartmentAddress,
        address treasuryDepartmentAddress
    ) {
        _name = name_;
        _symbol = symbol_;

        taxDepartment = ITaxDepartment(taxDepartmentAddress);
        treasuryDepartment = ITreasuryDepartment(treasuryDepartmentAddress);
    ...
    function totalSupply() public pure override returns (uint256) {
        // 1 Million Million , i.e., 1,000,000,000,000,000 tokens.
        return 1e15 * 1e9;
    }
    ...
    function transferFrom(
        ...
        require(
            ...
            "MetaUbi:transferFrom:ALLOWANCE_EXCEEDED: Transfer amount exceeds allowance."
        );
        unchecked {
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) external returns (bool) {
        ...
        require(
            ...
            "MetaUbi:decreaseAllowance:ALLOWANCE_UNDERFLOW: Subtraction results in sub-zero allowance."
        );
        unchecked {
     ...
     * @param taxDepartmentAddress Address of new tax handler contract.
     */
    function setTaxDepartment(address taxDepartmentAddress) external onlyOwner {
        address oldTaxDepartmentAddress = address(taxDepartment);
        taxDepartment = ITaxDepartment(taxDepartmentAddress);

        emit TaxDepartmentChanged(oldTaxDepartmentAddress, taxDepartmentAddress);
    }

    /**
     * @notice Set new treasury handler contract.
     * @param treasuryDepartmentAddress Address of new treasury handler contract.
     */
    function setTreasuryDepartment(address treasuryDepartmentAddress) external onlyOwner {
        address oldTreasuryDepartmentAddress = address(treasuryDepartment);
        treasuryDepartment = ITreasuryDepartment(treasuryDepartmentAddress);

        emit TreasuryDepartmentChanged(oldTreasuryDepartmentAddress, treasuryDepartmentAddress);
    }    
    ...
    function _approve(
        ...
        require(owner != address(0), "MetaUbi:_approve:OWNER_ZERO: Cannot approve for the zero address.");
        require(spender != address(0), "MetaUbi:_approve:SPENDER_ZERO: Cannot approve to the zero address.");
    ...
    function _transfer(
        ...
        require(from != address(0), "MetaUbi:_transfer:FROM_ZERO: Cannot transfer from the zero address.");
        require(to != address(0), "MetaUbi:_transfer:TO_ZERO: Cannot transfer to the zero address.");
        require(amount > 0, "MetaUbi:_transfer:ZERO_AMOUNT: Transfer amount must be greater than zero.");
        require(amount <= _balances[from], "MetaUbi:_transfer:INSUFFICIENT_BALANCE: Transfer amount exceeds balance.");

        treasuryDepartment.beforeTransferHandler(from, to, amount);

        uint256 tax = taxDepartment.getTax(from, to, amount);
        ...
        if (tax > 0) {
            _balances[address(treasuryDepartment)] += tax;


            emit Transfer(from, address(treasuryDepartment), tax);
        }

        treasuryDepartment.afterTransferHandler(from, to, amount);
...
interface  ITaxDepartment {
  ...
  /**
     ...
     * @param amount Number of tokens in the transfer.
     * @return Number of tokens to pay as wax.  
  */
