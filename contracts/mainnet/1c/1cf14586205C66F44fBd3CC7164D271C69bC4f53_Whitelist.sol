...
contract Whitelist is Governance, Context {
    ...
    address public expensesAddress;

    uint256 public price;
    uint256 public SNTOAmount;
    ...
    constructor(
        ...
        router = _router;
        usdt = _usdt;
        expensesAddress = _expensesAddress;
        collectAddress = _collectAddress;

        enable = true;

        price = 100 * 10 ** ERC20(usdt).decimals();
        SNTOAmount = 500000 * 10 ** ERC20(snto).decimals();
        liquidityRate = 5000;
    }

    function setAddress(address _root,
        address payable _dapp,
        address _expensesAddress,
        address _collectAddress) public onlyGovernance {
        root = _root;
        dapp = _dapp;
        collectAddress = _collectAddress;
        expensesAddress = _expensesAddress;
    ...
    function setPrice(uint256 _price) public onlyGovernance {
        price = _price;
    }
    ...
    function setSNTOAmount(uint256 _SNTOAmount) public onlyGovernance {
        SNTOAmount = _SNTOAmount;
    }

    function setLiquidityRate(uint256 _liquidityRate) public onlyGovernance {
        liquidityRate = _liquidityRate;
    }

    function JoinWhiteList(address _parent) public {
        require(!joined[_msgSender()], "address in whitelist");
        require(enable, "disabled");

        if (DAPP(dapp).parents(_msgSender()) == address(0) && _parent != address(0)) {
            DAPP(dapp).setParentByGovernance(_msgSender(), _parent);
        }

        IERC20(snto).transferFrom(expensesAddress, address(this), SNTOAmount);



        ERC20(usdt).transferFrom(_msgSender(), address(this), price);
        uint256 liquidityAmount = price.mul(liquidityRate).div(10000);

        ERC20(snto).approve(router, SNTOAmount);
        ERC20(usdt).approve(router, liquidityAmount);
        ...
        address[] memory ps = DAPP(dapp).getParentsByLevel(_msgSender(), 5);
        uint256[5] memory amounts = DAPP(dapp).getAmounts(price);
        for (uint256 i = 0; i < 5; i++) {
            if (ps[i] != address(0) && joined[ps[i]]) {
                ERC20(usdt).transfer(ps[i], amounts[i]);
            } else {
                ERC20(usdt).transfer(root, amounts[i]);
            }
        }

        ERC20(usdt).transfer(
            collectAddress,
            ERC20(usdt).balanceOf(address(this))
        );
        joined[_msgSender()] = true;
    }
...
contract Pool is Governance, Context {
    ...
    function checkEffect(address _user) private {
        uint256 _pid = 0;
        PoolInfo storage pool = pools[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 price = getLpUSDTPrice(address(pool.lpToken));
        if (price/1e18 * user.amount >= effectAmount) {
            effects[_user] = true;
        } else {
            effects[_user] = false;
        }
...
contract DAPP is Governance, Context {
    ...
    function removeLiquidity(uint256 _amount, address _parent) public {
        ...
        if (parents[_msgSender()] == address(0) && _parent != address(0)) {
            require(_parent != _msgSender(), "parent can not be self");
