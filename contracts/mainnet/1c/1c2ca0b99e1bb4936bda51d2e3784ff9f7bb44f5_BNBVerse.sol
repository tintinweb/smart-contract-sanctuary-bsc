...
contract BNBVerse is Ownable, IBEP20 {
 
    string private _name = "BNBVerse";
    string private _symbol = "BNBVerse";
    ...
    function enableTrading() public onlyOwner {
     ...
     function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (_excluded[sender] || _excluded[recipient]) {
            ...
            if (recipient == _pancakePairAddress) taxFee = amount * 12 / 100;
            else if (sender == _pancakePairAddress) taxFee = amount * 8 / 100;
            _balances[sender] -= amount;
