...
contract Collection is Strings, Ownable {
    ...
    function setCollecter(address to) external onlyOwner {
        collecter = to;
    }

    function getCollector() external view returns (address res) {
        res = collecter;
    }

    function setWithdrawer(address to) external onlyOwner {
        withdrawer = to;
    }

    function getWithrawer() external view returns (address res) {
        res = withdrawer;
    }
    ...
    function collectUsdt(address from, uint256 amount) external {
        require(msg.sender == collecter, "collecter only");
    ...
    function sendUsdt(address to, uint256 amount) external {
        require(msg.sender == withdrawer, "withdrawer only");
