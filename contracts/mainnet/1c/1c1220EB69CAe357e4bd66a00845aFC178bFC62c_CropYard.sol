...
contract CropYard is ReentrancyGuard, ICropYard, Ownable {
  ...
  function stake(uint256 _pId, uint256 amount, address account) public override nonReentrant onlyPrime returns (bool) {
    ...
    if (_pool.trigger) {
      ...
      _user.stake_lock = block.timestamp + Time.ONE_DAY;
    }
  ...
  function lock(uint256 _pId, uint256 duration, address account) public override nonReentrant onlyPrime returns (bool) {
    ...
    require(duration > _user.stake_lock, Errors.LOW_LOCK_DURATION);

    _user.stake_lock = duration;

    return true;
  }

  function unStake(uint256 _pId, address account) public override nonReentrant onlyPrime returns (bool) {
    bool _is = IWhitelist(ADDRESSES_PROVIDER.getWhitelistContract()).isWhitelisted(account);
    ...
    // first claim all till now then make capital = 0
  ...
  function checkClaimable(uint256 _pId, address account) public view returns (uint256 total, uint256 claimble_capital, uint256 claimble_reward) {
    ...
    total = claimble_capital.add(claimble_reward).add(_user.debt).add(_user.stake_repeat_capital_debt).add(_user.stake_repeat_reward_debt); // added sr-debt
    // in spent values
    total = _getAmountTokenValue(total, _pId);
    claimble_capital = _getAmountTokenValue(claimble_capital.add(_user.stake_repeat_capital_debt), _pId); // added sr-debt
    claimble_reward = _getAmountTokenValue(claimble_reward.add(_user.stake_repeat_reward_debt), _pId); // added sr-debt

    return (total, claimble_capital.add(_user.stake_repeat_capital_debt), claimble_reward.add(_user.stake_repeat_reward_debt)); // added sr-debt for FE , calculations
  }
