...
contract Sun is ERC20 {
	...
	address private _fundAddress = address(0x52DD0D51B0f8e1C4E1D01cDDB61b83AAa5786fc1);
	address private _marketAddress = address(0xa490488f7c8A4Fe369979A8CafA33A461D19842D);
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx) {
			...
			if(from == uniswapV2Pair){
                if(startTime.add(60) > block.timestamp){amount = amount.div(10);}
	...
	function swapAndLiquify() public {
		...
		ETH.transfer(_marketAddress,ethBalance.div(5).mul(4));
		if(ethBalance.mul(otherAmount) > 10**34){
			addLiquidityUsdt(ethBalance.div(5), otherAmount);
		}
    }
    ...
    function swapAndLiquifyStepv1() public {
        ...
        addLiquidityUsdt(ethBalance, tokenBalance);
    }
