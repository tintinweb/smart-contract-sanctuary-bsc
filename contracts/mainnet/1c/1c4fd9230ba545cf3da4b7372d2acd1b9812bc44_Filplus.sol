...
contract Filplus is IERC20, Ownable {
    ...
    //50%释放部分
    mapping(address => uint256) public claimIdoDay;//最后一次领取日期
    mapping(address => uint256) public claimIdoRestDay;//剩余领取天数
    // mapping(address => uint256) public claimIdoTotal;//50%总计
    mapping(address => uint256) public claimIdoRest;//50%剩余可领数量
    mapping(address => uint256) public claimIdoOneDay;//每天可领数量
    ...
    constructor(address tokenOwner,address addrance) {
        if (block.chainid == 56) {
        ...
        ecology = _tTotal.mul(5).div(100);//基金会5%
    ...
    function restAirdropNum() public view returns (uint256) {
        return restAirdrop;
    }
    ...
    function tokenAirdrop(address _from,address _to, uint _amt) public {//_amt:多少份
        ...
        uint256 startAmountwei = Amountwei.div(2);//先释放50%
        claimIdoRest[_from] = claimIdoRest[_from].add(startAmountwei);//剩余未释放部分总计
        claimIdoOneDay[_from] = claimIdoRest[_from].div(50);//每天释放数量，前面未释放部分也重新50天分配
        claimIdoRestDay[_from] = 50;//重置50天

         _rOwned[address(this)] = _rOwned[address(this)].sub(startAmountwei);
         _rOwned[_from] = _rOwned[_from].add(startAmountwei);
         emit Transfer(address(this), _from, startAmountwei);
...
//获取可领取数量（ido 50%未释放部分）
    function getClaimIdo(address addr) public returns (uint256) {
            uint256 ClaimIdo;
            time = currTimeStamp();
            if(claimIdoRestDay[addr] > 0 &&  claimIdoDay[addr] != time.getDay() && isStart){
                ClaimIdo = claimIdoOneDay[addr];
            }
            return ClaimIdo;
        }

    function claimIdo(address addr) public {
                require(msg.sender == addr,"No permission");
                require(isStart == true,"Not started to collect");
                time=currTimeStamp();
                uint256 claimIdoNum = getClaimIdo(addr);
                require(claimIdoNum > 0,"erro");
                
                _rOwned[addr] = _rOwned[addr].add(claimIdoNum);
                _rOwned[address(this)] = _rOwned[address(this)].sub(claimIdoNum);
                emit Transfer(address(this), addr, claimIdoNum);
                claimIdoDay[addr] = time.getDay(); 
                claimIdoRestDay[addr] = claimIdoRestDay[addr].sub(1);      
        }
    ...
    function changeHoldBase(uint256 num) public {//800
     ...
     function changeIsStart(bool status) public {//开始领取IDO未释放部分
        require(msg.sender == poperator,"No operator permission");
        isStart = status;
    }
