...
contract Bamboo is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    uint256 public blacklistEndTime;
    ...
    uint256 public maxTxAmount = _tokenTotal.div(1000); // 0.5%
    uint256 public minTokensBeforeSwap = 100_000 * (10**_decimals);
    uint256 public maxWalletToken = _tokenTotal.div(1000); // 0.5%
    ...
    event Swap(uint256 tokensSwapped, uint256 bnbReceived);
    ...
    constructor(
        ...
        blacklistEndTime = block.timestamp + 2 days;
    ...
    function _transfer(
        ...
        require(isTaxless[sender] || isTaxless[recipient] || tradingEnable, "trading is disable");
        require(
    ...
    function swap() private lockTheSwap {
        if (minTokensBeforeSwap > _liqFeeCollected.add(_vaultFeeCollected)) return;

        uint256 amountToLiquify = _liqFeeCollected.div(2);
        uint256 amountToSwap = _liqFeeCollected.add(_vaultFeeCollected).sub(amountToLiquify);
        ...
        _approve(address(this), address(router), amountToSwap.add(amountToLiquify));
        try
            ...
            )
        {
            ...
            emit Swap(amountToSwap, amountBNB);
    ...
    function setBuyFee(uint256 vaultFee_, uint256 liquidityFee_) external onlyOwner {
        _vaultFee[0] = vaultFee_;
        _liqFee[0] = liquidityFee_;

        require(vaultFee_.add(liquidityFee_) < 2500, "fee too high");
    }

    function setSellFee(uint256 vaultFee_, uint256 liquidityFee_) external onlyOwner {
        _vaultFee[1] = vaultFee_;
        _liqFee[1] = liquidityFee_;

        require(vaultFee_.add(liquidityFee_) < 2500, "fee too high");
    }

    function setP2PFee(uint256 vaultFee_, uint256 liquidityFee_) external onlyOwner {
        _vaultFee[2] = vaultFee_;
        _liqFee[2] = liquidityFee_;
    ...
    function setMaxTxAmount(uint256 _amount) external onlyOwner {
        require(_amount >= _tokenTotal.div(1000), "amount too low"); // 0.1%
        maxTxAmount = _amount;
    }
    ...
    function setTrading(bool value) external onlyOwner {
        require(!tradingEnable, "Trading already initialized");
        tradingEnable = value;
    }

    function setBlackList(address _address, bool value) external onlyOwner {
        require(_address != pair && _address != address(this), "cannot blacklist pair or bamboo");
        require(block.timestamp < blacklistEndTime, "cannot blacklist now");
