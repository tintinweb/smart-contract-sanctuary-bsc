...
contract WLM is ERC20, ReentrancyGuard, Ownable {
    ...
    uint internal nextTotal;
    uint internal endTime;
    bool internal presale = true;
    
    constructor(address pool, uint supplyWholeTokens, uint price) ERC20("WEN LUNA MOON", "WLM") {
        donationPool = pool;
        _mint(msg.sender, supplyWholeTokens * 10 ** 18);
        maxsupply = 3 * supplyWholeTokens * 10 ** 18;
        baseprice = price;
        multiplier = 1;
        step = maxsupply / 30;
        nextTotal = maxsupply/3 + step;
        endTime = block.timestamp + 24 hours;
    }

    function buyPresale() external payable nonReentrant {
        require(presale, "Presale finished");
        uint count = msg.value / baseprice;
        count = count / multiplier;
        require(count < maxsupply / 100, "you can only buy 1% of supply at once");
        _mint(msg.sender, count);
        while(totalSupply() >= nextTotal) {
            nextTotal += step;
            multiplier *= 2;
        }
        payable(donationPool).transfer(msg.value/2);
        payable(owner()).transfer(msg.value/2);
        if (block.timestamp > endTime || totalSupply() >= maxsupply) {
