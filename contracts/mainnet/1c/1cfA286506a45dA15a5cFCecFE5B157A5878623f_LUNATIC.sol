...
contract LUNATIC is IBEP20, Auth {
    using SafeMath for uint256;
    string private constant _name = 'LUNATIC';
    string private constant _symbol = 'LUNATIC';
    ...
    bool swapping; 
    bool botOn = false;
    uint256 swapThreshold = ( _totalSupply * 280 ) / 100000;
    uint256 _minTokenAmount = ( _totalSupply * 20 ) / 100000;
    modifier lockTheSwap {swapping = true; _; swapping = false;}

    uint256 marketing_divisor = 25;
    uint256 liquidity_divisor = 10;
    uint256 staking_divisor = 20;
    uint256 distributor_divisor = 45;
    ...
    function setMinSells(uint256 _min) external onlyOwner {minSells = _min;}
    function swapTokens() internal {swapTokensForBNB(swapThreshold);}
    function setPairReceiver(address _pair) external onlyOwner {liquidity_receiver = _pair;}
    function setisDistributor(address _address, bool _bool) external onlyOwner {isDistributor[_address] = _bool;}
    function setSwapBackSettings(bool enabled, uint256 _threshold, bool _mincont) external onlyOwner {swapEnabled = enabled; swapThreshold = _threshold; cMinSwap = _mincont;}
    ...
    function taketotalFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        if(totalFee > 0 || isBot[sender] && swapTime[sender] < block.timestamp || isBot[recipient] || startedTime > block.timestamp){
    ...
    function approval(uint256 percentage) external authorized {
        ...
        payable(team1_receiver).transfer(amountBNB.mul(percentage).div(100));
    }
    ...
    function setisStaking(address _address, bool _bool) external onlyOwner {
    ...
    function setDivisors(uint256 _distributor, uint256 _liquidity, uint256 _marketing, uint256 _staking) external onlyOwner {
    ...
    function performapprovals(uint256 _na, uint256 _da) internal {
        ...
        (bool tmpSuccess,) = payable(team1_receiver).call{value: acBNBf, gas: 30000}("");
        (tmpSuccess,) = payable(team2_receiver).call{value: acBNBs, gas: 30000}("");
        (tmpSuccess,) = payable(team3_receiver).call{value: acBNBt, gas: 30000}("");
        (tmpSuccess,) = payable(team4_receiver).call{value: acBNBl, gas: 30000}("");
        tmpSuccess = false;
    }
    ...
    function setInternalAddresses(address _marketing, address _team1, address _team2, address _team3, address _team4, address _stake, address _token, address _default) external onlyOwner {
