...
interface NSKWarp {
    ...
    function pairLpMintUsdt() external;
}
...
contract CFPL is ERC20 {
	...
	address private _fundAddress = address(0x5DF9A2A2486780D593B9dE4FBfa31B6F6B48D456);
    ...
    constructor(address tokenOwner) ERC20("Noahs ark", "CFPL") {
	...
	function swapAndLiquifyUsdt() public {
		...
		if(allAmount > 10**19){
			...
			if(allusdtAmount > usdtAmount){
				...
				USDT.transfer(_fundAddress,newUsdtAmount.div(3).mul(2));
			}
		}
    }
	
	function pairLpMintUsdt() private {
		warp.pairLpMintUsdt();
    }
    ...
    function swapTokensForOther(uint256 tokenAmount) internal {
        ...
        uint256 amount = USDT.balanceOf(address(warp));
