...
library Utils {
    ...
    function calcStacked(StackingStruct.stacking memory tmpstacking, uint256 totalsupply) public view returns (uint256) {
        uint256 reward;
        ...
        {
            reward = uint256(tmpstacking.hardcap) * tmpstacking.amount / totalsupply * stackedTotal / 1E6;
            if (reward >= initialBalance) reward = 0;

            if (reward == 0 || initialBalance.sub(reward) < tmpstacking.hardcap)
            {
                reward = initialBalance - calcReward(initialBalance, totalsupply / tmpstacking.amount, stacked, 15);
                reward += initialBalance.sub(reward) * tmpstacking.amount / totalsupply * rest / 1E6;
            }
        } else {
            reward = initialBalance - calcReward(initialBalance, totalsupply / tmpstacking.amount, stacked, 15); 
            reward += initialBalance.sub(reward) * tmpstacking.amount / totalsupply * rest / 1E6;
    ...
    */
    ...
    function getStacked(address _address) public view returns (uint256) {
        ...
        if (tmpStack.enabled) {
            ...
            return Utils.calcStacked(tmpStack, totalsupply);
        }
        return 0;
    }
