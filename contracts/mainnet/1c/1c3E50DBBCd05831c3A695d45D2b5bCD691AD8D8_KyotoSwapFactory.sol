...
contract KyotoSwapERC20 is IUniswapV2ERC20 {
    ...
    string public constant name = "KyotoSwap LP";
    string public constant symbol = "KSWAP-LP";
...
import './KyotoSwapPair.sol';
import '@uniswap/v2-core/contracts/interfaces/IUniswapV2Factory.sol';


contract KyotoSwapFactory is IUniswapV2Factory {
    bytes32 public constant INIT_CODE_PAIR_HASH = keccak256(abi.encodePacked(type(KyotoSwapPair).creationCode));
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        require(tokenA != tokenB, 'KyotoSwapFactory: IDENTICAL_ADDRESSES');
        
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'KyotoSwapFactory: ZERO_ADDRESS');
        require(getPair[token0][token1] == address(0), 'KyotoSwapFactory: PAIR_EXISTS'); // single check is sufficient
        bytes memory bytecode = type(KyotoSwapPair).creationCode;
    ...
    function setFeeTo(address _feeTo) external {
        require(msg.sender == feeToSetter, 'KyotoSwapFactory: FORBIDDEN');
        feeTo = _feeTo;
    }
