...
contract FarmContractTest is Initializable, OwnableUpgradeable, UUPSUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(uint256 => mapping(address => uint256)) public level;
    mapping(uint256 => mapping(address => uint256)) public refferalRewardPercent;
    mapping(uint256 => mapping(address => uint256)) public nodeRewardPercent;
    mapping(uint256 => mapping(address => uint256)) public userClaimedReferralRewards;
    mapping(uint256 => mapping(address => uint256)) public userClaimedNodeRewards;
    ...
    mapping(address => uint256) public userTotalClaimedReferralRewards;
    mapping(address => uint256) public userTotalClaimedNodeRewards;
    ...
    function pendingReferralReward(uint256 _pid, address _user) public view validatePid(_pid) returns(uint256 totalReferralBonus){
        address[] memory myInviteeAddress = IInviteContract(inviteContract).getInvitees(_user);
        for (uint256 i = 0; i < myInviteeAddress.length; i++) {
            uint256 referralBonus = ((pendingMiningReward(_pid, myInviteeAddress[i]).add(mintAmount[_pid][_user]))).mul(getRefferalRewardPercent(_pid, _user)).div(rateDenominator);
            totalReferralBonus = totalReferralBonus.add(referralBonus);
        }
        totalReferralBonus = totalReferralBonus.sub(userClaimedReferralRewards[_pid][_user]);
        return totalReferralBonus;
    }

    function pendingNodeReward(uint256 _pid, address _user) public view validatePid(_pid) returns(uint256 totalNodeBonus){
        address[] memory myInviteeAddress = IInviteContract(inviteContract).getInvitees(_user);
        for (uint256 i = 0; i < myInviteeAddress.length; i++) {
            uint256 nodeBonus = ((pendingMiningReward(_pid, myInviteeAddress[i]).add(mintAmount[_pid][_user]))).mul(getNodeRewardPercent(_pid, _user)).div(rateDenominator);
            totalNodeBonus = totalNodeBonus.add(nodeBonus);
        }
        totalNodeBonus = totalNodeBonus.sub(userClaimedNodeRewards[_pid][_user]);
        return totalNodeBonus;
    }

    function getAllPendingReferralReward(address _user) public view returns(uint256 totalReferralBonus){
        for (uint256 i = 0; i < poolInfo.length; i++) {
            uint256 referralBonus = pendingReferralReward(i,_user);
            totalReferralBonus = totalReferralBonus.add(referralBonus);
        }
    }
    ...
    function getAllPendingNodeReward(address _user) public view returns(uint256 totalNodeBonus){
        for (uint256 i = 0; i < poolInfo.length; i++) {
            uint256 nodeBonus = pendingNodeReward(i,_user);
            totalNodeBonus = totalNodeBonus.add(nodeBonus);
        }
    }
    ...
    function claimReferralReward() internal returns(uint256 totalReward) {
        for (uint256 i = 0; i < poolInfo.length; i++) {
            if(pendingReferralReward(i, _msgSender()) > 0) {
                uint256 refferalReward = pendingReferralReward(i, _msgSender());
                userClaimedReferralRewards[i][_msgSender()] = userClaimedReferralRewards[i][_msgSender()].add(refferalReward);
                totalReward = totalReward.add(refferalReward);
            }
        }
    }

    function claimNodeReward() internal returns(uint256 totalReward) {
        for (uint256 i = 0; i < poolInfo.length; i++) {
            if(pendingNodeReward(i, _msgSender()) > 0) {
                uint256 nodeReward = pendingNodeReward(i, _msgSender());
                userClaimedNodeRewards[i][_msgSender()] = userClaimedNodeRewards[i][_msgSender()].add(nodeReward);
    ...
    function claimAllReferralReward() public {
        require(getAllPendingReferralReward(_msgSender()) > 0, "You have no referral rewards to claim");
        uint256 pendingReward = claimReferralReward();
        safeMiningTokenTransfer(_msgSender(), pendingReward);
    }

    function claimAllNodeReward() public {
        require(getAllPendingNodeReward(_msgSender()) > 0, "You have no node rewards to claim");
        uint256 pendingReward = claimNodeReward();
        safeMiningTokenTransfer(_msgSender(), pendingReward);
    }
    ...
    function getUSDTAmountTransferToLPAmount(uint256 _pid, uint256 _amount) public view validatePid(_pid) returns(uint256) {
        uint256 totalLpAmount = poolInfo[_pid].lpToken.totalSupply().sub(1000);
        uint256 usdtTotal = IERC20Upgradeable(usdt).balanceOf(address(poolInfo[_pid].lpToken));
        return totalLpAmount.mul(_amount).div(usdtTotal);
    }

    function decisionLevel(uint256 _pid) public validatePid(_pid) {
        if(depositAmount[_pid][_msgSender()] >= getUSDTAmountTransferToLPAmount(_pid, oneStarUserpledgeThreshold)) {
            level[_pid][_msgSender()] = 1;
            refferalRewardPercent[_pid][_msgSender()] = referralRewardRate;
        } else if(depositAmount[_pid][_msgSender()] >= getUSDTAmountTransferToLPAmount(_pid,twoStarUserpledgeThreshold)) {
            level[_pid][_msgSender()] = 2;
            refferalRewardPercent[_pid][_msgSender()] = referralRewardRate;
            nodeRewardPercent[_pid][_msgSender()] = nodeRewardRate;
        } else if(depositAmount[_pid][_msgSender()] >= getUSDTAmountTransferToLPAmount(_pid, threeStarUserpledgeThreshold)) {
            level[_pid][_msgSender()] = 3;
            refferalRewardPercent[_pid][_msgSender()] = referralRewardRate;
            nodeRewardPercent[_pid][_msgSender()] = nodeRewardRate;
        } else {
            level[_pid][_msgSender()] = 0;
            refferalRewardPercent[_pid][_msgSender()] = 0;
            nodeRewardPercent[_pid][_msgSender()] = 0;
        }
    }

    function deposit(uint256 _pid, uint256 _amount) public validatePid(_pid) whenNotPaused nonReentrant {
        require(enableContract == true,"The contract has not started");
        require(IInviteContract(inviteContract).getInviter(msg.sender) != address(0), "You are not bound to an inviter");
        require(_amount > 0, "Please enter a quantity greater than 0");
        updatePool(_pid);
        poolInfo[_pid].lpToken.safeTransferFrom(address(_msgSender()), address(this), _amount);
        ...
        totalDepositAmount[_msgSender()] = totalDepositAmount[_msgSender()].add(_amount);
        emit Deposit(_msgSender(), _pid, _amount);
    }

    function claim(uint256 _pid) public validatePid(_pid) whenNotPaused nonReentrant {
        require(enableContract == true,"The contract has not started");
        ...
        totalMintAmount[_msgSender()] = totalMintAmount[_msgSender()].add(deservedMiningReward);
        safeMiningTokenTransfer(_msgSender(), deservedMiningReward);
        ...
        totalDividendAmount[_msgSender()] = totalDividendAmount[_msgSender()].add(deservedDividendReward);
        safeDividendTokenTransfer(_msgSender(), deservedDividendReward);
        emit Claim(_msgSender(), _pid, deservedDividendReward);
    }

    function withdraw(uint256 _pid) public validatePid(_pid) whenNotPaused nonReentrant {
        require(enableContract == true,"The contract has not started");
        require(depositAmount[_pid][_msgSender()] > 0,"The amount you stake in this pool is 0");
        ...
        require(canClaimDividendReward, "The dividend reward token balance of this contract is insufficient"); 
        updatePool(_pid);
        if(pendingMiningReward(_pid, _msgSender()) > 0 && canClaimMiningReward) {
            uint256 deservedMiningReward = claimPendingMiningReward(_pid);
            mintAmount[_pid][_msgSender()] = mintAmount[_pid][_msgSender()].add(deservedMiningReward);
            totalMintAmount[_msgSender()] = totalMintAmount[_msgSender()].add(deservedMiningReward);
            safeMiningTokenTransfer(_msgSender(), deservedMiningReward);
       ...
       if(pendingDividendReward(_pid, _msgSender()) > 0 && canClaimDividendReward) {
            uint256 deservedDividendReward = claimPendingDividendReward(_pid);
            dividendAmount[_pid][_msgSender()] = dividendAmount[_pid][_msgSender()].add(deservedDividendReward);
            totalDividendAmount[_msgSender()] = totalDividendAmount[_msgSender()].add(deservedDividendReward);
            safeDividendTokenTransfer(_msgSender(), deservedDividendReward);
        ...
        totalDepositAmount[_msgSender()] = totalDepositAmount[_msgSender()].sub(getTotalRedeemableLPAmount(_pid, _msgSender()));
    ...
    function emergencyWithdraw(uint256 _pid) public validatePid(_pid) whenNotPaused nonReentrant {
        require(enableContract == true,"The contract has not started");
        ...
        totalDepositAmount[_msgSender()] = totalDepositAmount[_msgSender()].sub(depositAmount[_pid][_msgSender()]);
    ...
    function closeAllRedeemedDepositInfo(uint256 _pid) internal {
        for(uint256 i = 0; i < userDepositInfo[_pid][_msgSender()].length; i++) {
            if(canWithdrawOrNot(userDepositInfo[_pid][_msgSender()][i].depositTime) && userDepositInfo[_pid][_msgSender()][i].onGoing) {
                userDepositInfo[_pid][_msgSender()][i].withdrawTime = block.timestamp;
    ...
    function safeMiningTokenTransfer(address to, uint256 amount) internal {
        uint256 miningTokenBalance = IERC20Upgradeable(miningToken).balanceOf(address(this));
        if (amount > miningTokenBalance) {
            IERC20Upgradeable(miningToken).transfer(to, miningTokenBalance);
        } else {
    ...
    function safeDividendTokenTransfer(address to, uint256 amount) internal {
        uint256 dividendTokenBalance = IERC20Upgradeable(dividendToken).balanceOf(address(this));
        if (amount > dividendTokenBalance) {
            IERC20Upgradeable(dividendToken).transfer(to, dividendTokenBalance);
        } else {
    ...
    function retrieveMiningToken() public onlyOwner {
        ...
        safeMiningTokenTransfer(_msgSender(), rewardTokenBalance);
    }
    ...
    function retrieveDividendToken() public onlyOwner {
        ...
        safeDividendTokenTransfer(_msgSender(), rewardTokenBalance);
    }
    ...
    function getLevel(uint256 pid, address user) public view validatePid(pid) returns(uint256) {
        if(depositAmount[pid][user] >= getUSDTAmountTransferToLPAmount(pid, threeStarUserpledgeThreshold)) {
            return 3;
        } else if(depositAmount[pid][user] >= getUSDTAmountTransferToLPAmount(pid, twoStarUserpledgeThreshold)) {
            return 2;
        } else if(depositAmount[pid][user] >= getUSDTAmountTransferToLPAmount(pid, oneStarUserpledgeThreshold)) {
            return 1;
        } else {
            return 0;
        }
    ...
    function geLPAmountTransferToUSDTAmount(uint256 _pid, uint256 _amount) public view validatePid(_pid) returns(uint256) {
        uint256 totalLpAmount = poolInfo[_pid].lpToken.totalSupply().sub(1000);
        uint256 usdtTotal = IERC20Upgradeable(usdt).balanceOf(address(poolInfo[_pid].lpToken));
        return usdtTotal.mul(_amount).div(totalLpAmount);
    }

    function getMyLevel(uint256 pid, address user) public view validatePid(pid) returns(uint256) {
        if(geLPAmountTransferToUSDTAmount(pid, depositAmount[pid][user]) >= threeStarUserpledgeThreshold) {
            return 3;
        } else if(geLPAmountTransferToUSDTAmount(pid, depositAmount[pid][user]) >= twoStarUserpledgeThreshold) {
            return 2;
        } else if(geLPAmountTransferToUSDTAmount(pid, depositAmount[pid][user]) >= oneStarUserpledgeThreshold) {
            return 1;
        } else {
            return 0;
        }
