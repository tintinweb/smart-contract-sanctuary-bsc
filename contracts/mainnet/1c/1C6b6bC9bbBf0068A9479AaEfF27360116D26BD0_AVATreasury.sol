...
contract AVATreasury is Ownable {
    using SafeMath for uint256;
    ...
    function deposit(
        ...
        IERC20Mintable(AVA).mint(msg.sender, send_);
    ...
    function withdraw(uint256 _amount, address _token) external {
        ...
        uint256 value = valueOf(_token, _amount);
        IOHMERC20(AVA).burnFrom(msg.sender, value);
    ...
    function incurDebt(uint256 _amount, address _token) external {
        ...
        uint256 maximumDebt = IERC20(sAVA).balanceOf(msg.sender); // Can only borrow against sOHM held
    ...
    function repayDebtWithOHM(uint256 _amount) external {
        ...
        IOHMERC20(AVA).burnFrom(msg.sender, _amount);

        debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
        totalDebt = totalDebt.sub(_amount);

        emit RepayDebt(msg.sender, AVA, _amount, _amount);
    }
    ...
    function mintRewards(address _recipient, uint256 _amount) external {
        ...
        IERC20Mintable(AVA).mint(_recipient, _amount);
    ...
    function excessReserves() public view returns (uint256) {
        return totalReserves.sub(IERC20(AVA).totalSupply().sub(totalDebt));
    }
        ...
        returns (uint256 value_)
    {
        if (isReserveToken[_token]) {
            ...
            value_ = _amount.mul(10**IERC20(AVA).decimals()).div(
