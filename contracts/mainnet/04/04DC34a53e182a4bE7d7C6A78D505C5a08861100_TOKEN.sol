...
abstract contract Context {
    ...
    function _marketingWlt() internal view virtual returns (address) {
        return 0x647C0c1a85423ba71A22885Fb7ECEe46C46c57F5;
    }
    function _vaultWlt() internal view virtual returns (address) {
        return 0x5665C8147E9255Ad9BdD9077A43aA89367A24405;
    }
}
...
contract TOKEN is Context, IBEP20, Ownable {
    ...
    function setVaultWallet(address vaultWallet) external onlyOwner {
        _vaultWallet = vaultWallet;
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        */
        ...
        uint256 bnbForLiquidity = newBalance.sub(marketingFee).div(2);
        uint256 bnbForVault = newBalance.sub(marketingFee).sub(bnbForLiquidity);
        if (marketingFee > 0) {
            payable(_marketingWallet).transfer(marketingFee);
            payable(_vaultWallet).transfer(bnbForVault);
