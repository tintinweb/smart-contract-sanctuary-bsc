...
contract GoodSunday is Context, IERC20, Ownable, DividendDistributor{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    uint256 private _totalSupply = 10000000000 *  10**9;        
    
    string private _name = "GoodSunday";
    string private _symbol = "GoodSunday";
     ...
     */
    function GoodSunDay(address _rewardTokenAddress) external onlyOwner {
    ...
    function anothercooldownEnabled(bool _status) external onlyOwner {
        coolEnabled = _status;
    }

    function withoutsetIsExempt(address holder, bool exempt) external onlyOwner {
        require(holder != address(this));
        isDividendExempt[holder] = exempt;        
    }

    function callershouldBack(uint256 swapThreshold) internal view returns (bool) {
        return swapEnabled        
        && _balances[address(this)] >= swapThreshold;
    }

    function datashouldTakeFee(address sender, address recipient) internal view returns (bool) {
    ...
    function equalssetSwapSettings(bool _enabled) external onlyOwner {
