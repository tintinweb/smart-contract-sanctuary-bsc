    ...
    OwnableUpgradeable
{
    ...
    modifier lockUpgrade() {
        require(!_upgraded, "BirdBro: Already upgraded");
        _;
        _upgraded = true;
    }

    modifier checkTierIndex(uint256 _index) {
        require(feeTiers.length > _index, "BirdBro: Invalid tier index");
        _;
    }
    ...
    function initialize(address _router) public initializer {
        ...
        __BirdBro_v2_init_unchained(_router);
    }

    function __BirdBro_v2_init_unchained(address _router) internal initializer {
        _name = "Bird Bro";
        _symbol = "BRO";
        _decimals = 9;
        ...
        numTokensSellToAddToLiquidity = 5 * 10**3 * 10**9;
        ...
        __BirdBro_tiers_init();

        emit Transfer(address(0), _msgSender(), _tTotal);
    }

    function __BirdBro_tiers_init() internal initializer {
    ...
    function whitelistAddress(address _account, uint256 _tierIndex)
        ...
        preventBlacklisted(
            _account,
            "BirdBro: Selected account is in blacklist"
        )
    {
        require(_account != address(0), "BirdBro: Invalid address");
    ...
    function excludeWhitelistedAddress(address _account) public onlyOwner {
        ...
        require(
            ...
            "BirdBro: Account is not in whitelist"
        );
        _accountsTier[_account] = 0;
    }
        ...
        returns (FeeTier memory)
    {
        uint256 _fees = _tier
            ...
            .add(_tier.ownerFee)
            .add(_tier.burnFee);
        require(_fees <= _maxFee, "BirdBro: Fees exceeded max limitation");
        ...
        checkTierIndex(_tierIndex)
    {
        require(
            ...
            "BirdBro: Address Zero is not allowed"
        );
    ...
    function setOwnerFeeAddress(uint256 _tierIndex, address _owner)
        ...
        checkTierIndex(_tierIndex)
    {
        require(_owner != address(0), "BirdBro: Address Zero is not allowed");
    ...
    function _approve(
        ...
        private
        preventBlacklisted(owner, "BirdBro: Owner address is blacklisted")
        preventBlacklisted(spender, "BirdBro: Spender address is blacklisted")
    {
    ...
    function _transfer(
        ...
        private
        preventBlacklisted(_msgSender(), "BirdBro: Address is blacklisted")
        preventBlacklisted(from, "BirdBro: From address is blacklisted")
        preventBlacklisted(to, "BirdBro: To address is blacklisted")
    {
    ...
    function migrate(address account, uint256 amount) external override {
        require(migration != address(0), "BirdBro: Migration is not started");
        require(_msgSender() == migration, "BirdBro: Not Allowed");
    ...
    OwnableUpgradeable
{
    ...
    function setNumTokensSellToAddToLiquidity(uint256 tokensSellLiquidity) external onlyOwner {
    ...
    function swapAndEvolve() public onlyOwner lockTheSwap {
        ...
        // swap creates, and not make the liquidity event include any BNB that
        // has been manually sent to the contract
        uint256 initialBalance = IBirdBro(address(this)).balanceOf(msg.sender);
        // swap BNB for Tokens
        swapBnbForTokens(half);

        // how much BNB did we just swap into?
        uint256 newBalance = IBirdBro(address(this)).balanceOf(msg.sender);
        uint256 swapeedToken = newBalance.sub(initialBalance);

        _approve(msg.sender, address(this), swapeedToken);
        IBirdBro(address(this)).transferFrom(
            msg.sender,
            address(this),
            swapeedToken
        );
    ...
    function withdrawToken(address _token, uint256 _amount) public onlyOwner {
        IBirdBro(_token).transfer(msg.sender, _amount);
    }
