...
// BlockifyToken.
contract BlockifyToken is BEP20, MinterRole {
    ...
    address public buyBackToken = address(0xCDC491Ef9A9ccBb3D00b15Af252d050700F813F3); //buyBackToken address ( BUILD TOKEN )
    ...
    event BLFISwapRouterUpdated(address indexed operator, address indexed router, address indexed pair);
    ...
    function updateBLFISwapRouter(address _router) public onlyOwner {
        ...
        require(hashSwapPair != address(0), "BLFI::updateBLFISwapRouter: Invalid pair address.");
        emit BLFISwapRouterUpdated(_router, address(hashSwapRouter), hashSwapPair);  
    }
    ...
    function setSellFees(uint256 _liquiditySellFee, uint256 _buybackSellFee, uint256 _marketingSellFee, uint256 _burnSellFee) external onlyOwner{
        ...
        require(sellTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "BLFI: Total fees can not be bigger than max value");
    }
