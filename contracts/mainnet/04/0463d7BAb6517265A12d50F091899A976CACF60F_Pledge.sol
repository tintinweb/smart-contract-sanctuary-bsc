...
//        require(priceTool.getPrice(_faf, _usdt, _fafAmount) >= 100, "price too little");
        ...
        uint256 total = priceTool.getPrice(_usdt, _tkf, _usdtAmount) * 4;
        if(_orders[msg.sender].isExist == false){
            createOrder(total, _usdtAmount);
        }else{
            PledgeOrder storage order=_orders[msg.sender];
        ...
        //usdt分红 记录团队金额
        bonus(_usdtAmount);
        setTeamPerformance(_usdtAmount);
        return true;
    }
    ...
    function takeProfit() public {
        ...
        uint256 profits = priceTool.getPrice(_usdt, _tkf, uA);
    ...
    //钱包地址 分销可领取余金额 奖池总额 当前奖池剩余额度 个人质押u数量
    function setOrder(address _target, uint256 receiveAmount, uint256 totalAmount, uint256 rewardAmount, uint256 uAmount) external {
