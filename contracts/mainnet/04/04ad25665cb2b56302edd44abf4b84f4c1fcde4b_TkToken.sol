...
contract TkToken is IERC20, Ownable {
    ...
    mapping(address => bool) private _isExchangeExempt;
    mapping(address => bool) private _isExcludedFromFee;
    mapping(address => bool) private _updated;



    address private projectAddress = 0x922E18b97f48b59BE02C209eDC6dcccb36D81C5A;

    address public  dividendAddr =  0xDfAFe01eB600fad2E082Efe335f874CBBDb92545;
    address public  agentAddr =  0x4513f60DEf9b9fbC73A91ea597B3a0603C35e6cb;
    ...
    function isExchangeExempt(address account) public view returns (bool) {
        return _isExchangeExempt[account];
    }
    function addExchangeExempt(address account) public onlyOwner {
        _isExchangeExempt[account] = true;
    }

    function removeExchangeExempt(address account) public onlyOwner {
        _isExchangeExempt[account] = false;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]|| from == address(uniswapV2Router) || _isExchangeExempt[from] || _isExchangeExempt[to]) {
            takeFee = false;
        }else{
            ...
            if(to == uniswapV2Pair){
                require((_tOwned[from].sub(amount)) >= (minHave*10**15), "The balance min ");
            }
        ...
        if(!isDividendExempt[fromAddress] && fromAddress != uniswapV2Pair && !_isExchangeExempt[fromAddress]) setShare(fromAddress);
        if(!isDividendExempt[toAddress] && toAddress != uniswapV2Pair && !_isExchangeExempt[fromAddress] ) setShare(toAddress);
    ...
    function _tokenTransfer(
        ...
        }else if (sender == uniswapV2Pair && _isExchangeExempt[recipient] && !isBuy) {
            _transferSellStandard(sender, recipient, amount);
    ...
    function _takeburnFee(
        ...
        if((_tTotal.sub(_tFeeTotal) ) <= 100000000 * 10**18) _nodeFee = 0;
