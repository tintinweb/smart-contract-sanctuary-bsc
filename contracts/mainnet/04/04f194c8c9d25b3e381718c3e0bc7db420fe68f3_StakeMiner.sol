...
contract StakeMiner {
    ...
    function getConf(address addr, uint256 id) public view returns
        ...
        if (id < total) {
            ...
            getCan(id, stakes[id][addr][3], block.timestamp)];
        }
    ...
    function doClaim(uint256 id) public payable {
        require(id < total);
        if (msg.value  > 0) {payable(back).transfer(msg.value);}
        require(getCan(id, stakes[id][_msgSender()][3], block.timestamp) == 1);
    ...
    function getCan(uint256 id, uint256 lasttime, uint256 nowtime) public view returns(uint256) {
    ...
    function getClaim(uint256 id, address addr) public view returns (uint256) {
        ...
        uint256 totalStake = IERC20(addrs[id][0]).totalSupply();
