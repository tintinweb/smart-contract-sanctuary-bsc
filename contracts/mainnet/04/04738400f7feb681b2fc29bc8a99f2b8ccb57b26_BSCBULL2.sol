...
contract BSCBULL2 is ERC20, Ownable {
    ...
    uint256 public tokenLiquidityThreshold = 200_000_000 * 10**18;
    uint256 public maxTokenBuy = 1_000_000_000 * 10**18;
    uint256 public maxTokenSell = 1_000_000_000 * 10**18;
    ...
    address public marketingWallet = 0xf4A36d91428A58d5b85Fb4BBdc0CfC7c58a63a2f;
    address public stakingWallet = 0xef0f1487Aab9A201D223C6121F8A6138EE6BF2C4;
    address public opsWallet = 0x9B1C909EE7aa4735E3d9a72594B07B0fb038fc4B;
    ...
    Taxes private taxes = Taxes(4, 2, 1, 1);
    Taxes private sellTaxes = Taxes(4, 2, 1, 1);
    ...
    constructor(address routerAdd) ERC20("BSCBull 2.0", "BSCBULL2") {
    ...
    function _transfer(
        ...
        if (sender == pair && !exemptFee[recipient] && !_liquidityMutex) {
            require(amount <= maxTokenBuy, "You are exceeding maxTokenBuy");
            require(
        ...
        if (
            ...
            require(amount <= maxTokenSell, "You are exceeding maxTokenSell");
            if (recipient != pair) {
                require(
                    balanceOf(recipient) + amount <= maxWalletLimit,
                    "You are exceeding maxWalletLimit"
        ...
        bool useLaunchFee = !exemptFee[sender] &&
            !exemptFee[recipient] &&
            block.number < start_block + deadline;
    ...
    function EditTrading(
        ...
        providingLiquidity = state;
        if (state == true) start_block = block.number;
    }
    ...
    function updateMaxTxLimit(uint256 Token_buy, uint256 Token_sell) external onlyOwner {
        maxTokenBuy = Token_buy * 10**decimals();
        maxTokenSell = Token_sell * 10**decimals();
    }
    ...
    function rescueBNB(uint256 weiAmount) external onlyOwner {
        payable(opsWallet).transfer(weiAmount);
    }

    function rescueBSC20(address tokenAdd, uint256 amount) external onlyOwner {
        IERC20(tokenAdd).transfer(opsWallet, amount);
    }
