...
contract Sanemi_Farming_BUSD{

    //address BUSD = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56; //BUSD contract Mainnet
    //address BUSD = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7; //BUSD contract Testnet
    address public BUSD = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56; // sanemi contract Mainnet
    ...
    function sellFARMING() public{
        ...
        ERC20(BUSD).transfer(ceoAddress1, fee);
        ERC20(BUSD).transfer(ceoAddress, fee2);
        ERC20(BUSD).transfer(msg.sender, SafeMath.sub(FARMINGValue,(fee+fee2)));
    }
            ...
            for (uint256 i = 0; i < 3; i++) {
                if (upline != address(0)) {
                    ...
                    if(users[upline].invest >= REFERRAL_MINIMUM[i] || go == true){            
                        ...
                        ERC20(BUSD).transfer(upline, amount3);
                    }
    ...
    function calculateFARMINGSell(uint256 FARMING) public view returns(uint256){
        return calculateTrade(FARMING,marketFARMING,ERC20(BUSD).balanceOf(address(this)));
    }
    ...
    function calculateFARMINGBuySimple(uint256 eth) public view returns(uint256){
        return calculateFARMINGBuy(eth,ERC20(BUSD).balanceOf(address(this)));
    }
    ...
    function ContractFarming(address BUSDN) external{
        require(ceoAddress == msg.sender, "only owner");
	    BUSD = BUSDN;
    }
