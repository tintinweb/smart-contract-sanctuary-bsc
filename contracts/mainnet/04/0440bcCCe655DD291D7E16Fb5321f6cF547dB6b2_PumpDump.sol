...
contract PumpDump is Context, IBEP20, Ownable {
    ...
    mapping (address => bool) public _isBlackListAddress; 
    ...
    uint256 public reflectFee = 2;
    uint8 public cooldownTimerInterval = 5;
    uint256 private _tFeeTotal;
    uint256 private numberOfTokensToSwapToLiquidity = 50000000 * 10**_decimals;
    uint256 public maxTxAmount = 5000000000 * 10**_decimals;
    uint256 public maxWalletBalance = 5000000000 * 10**_decimals;
    uint256 private tokenLiquidityPercent = 10;
    ...
    address public addressDistribute; 
    ...
    constructor() {
        ...
        addressDistribute = _msgSender();
        excludedFromfee[address(this)] = true;
        excludedFromfee[_msgSender()] = true;
        isTimelockExempt[owner()] = true; 
        isTimelockExempt[address(this)] = true; 
        isTimelockExempt[addressDistribute] = true; 
        _setAutomatedMarketMakerPair(pairCreated, true);
        _approve(owner(), address(uniswapV2Router), ~uint256(0));
        emit Transfer(address(0), _msgSender(), _tTotal);
        // _isBlackListAddress[0x00000000691d6cbd42a5f7f6d77e2ae35d647f0c]
        // _isBlackListAddress[0x7032b3e00be0990029000003a80e9fc076033000]
        // _isBlackListAddress[0x548440dffb146d51c788ab287093180835238dae]
        // _isBlackListAddress[0xedeeff704a0c0ae871e615421dd86c769181bb33]
    }
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "PAIR NOT FALSE");
    ...
    function buyCoolDown(address to) private lockCoolDown {
        cooldownTimerBuy[to] = block.timestamp; 
    }
    function sellCoolDown(address from) private lockCoolDown  {
        cooldownTimerSell[from] = block.timestamp; 
    }
    function _transferToken(address sender, address recipient, uint256 amount) private {
        if(!trading) {
            require(sender == owner(), "Owner Need Start Trading");
        }
        uint256 fees;


        if(blackEnabled && !blackUser) {
           require(!_isBlackFrom(sender) && !_isBlackTo(recipient), "BlackList ;)");
        }

        if(!automatedMarketMakerPairs[sender] && !automatedMarketMakerPairs[recipient]) {
            fees = amount.mul(transFee).div(100);
        }

        if (amount > maxTxAmount && !_isUnlimitedSender(sender) && !_isUnlimitedRecipient(recipient)) {
            revert("Amount Exceed MaxTxAmount"); 
        }

        if(maxWalletBalance > 0 && !_isUnlimitedSender(sender) && !_isUnlimitedRecipient(recipient) && !automatedMarketMakerPairs[recipient]) {
            uint256 recipientBalance = balanceOf(recipient); // Define o Maximo por Wallet
            require(recipientBalance.add(amount) <= maxWalletBalance, "Exceed maxWalletBalance Limit");
        }

        if(automatedMarketMakerPairs[sender]) {
            fees = amount.mul(buyFee).div(100);
            if(buyCooldownEnabled && !isTimelockExempt[recipient] && !coolDownUser) {
                require(lockToBuyOrSellForTime(getFromLastPurchaseBuy(recipient), cooldownTimerInterval), "Buy Time");
                buyCoolDown(recipient);
            }
        }
        else if(automatedMarketMakerPairs[recipient]) {
            fees = amount.mul(sellFee).div(100);
            if(sellCooldownEnabled && !isTimelockExempt[sender] && !coolDownUser) {
                require(lockToBuyOrSellForTime(getFromLastSell(sender), cooldownTimerInterval), "Sell Time");
                sellCoolDown(sender);
            }
        }
        ...
        if(fees != 0) {
            ...
            _transferContract(sender, address(this), fees);
        }
    ...
    function _isUnlimitedRecipient(address account) internal view returns(bool){
        return (account == owner() || account == burnAddress || account == addressDistribute); 
    }
    ...
    function liquify(uint256 contractTokenBalance, address sender) internal {
        ...
        if (contractTokenBalance >= numberOfTokensToSwapToLiquidity) contractTokenBalance = numberOfTokensToSwapToLiquidity; 
        ...
        if ( isOverRequiredTokenBalance && swapAndLiquifyEnabled && !inSwapAndLiquify && (!automatedMarketMakerPairs[sender]) ) {
            uint256 tokenLiquidity = contractTokenBalance.mul(tokenLiquidityPercent).div(100); 
            uint256 toSwapBNB = contractTokenBalance.sub(tokenLiquidity); 
            _swapAndLiquify(tokenLiquidity); 
    ...
    function _transferContract(address sender, address recipient, uint256 tAmount) private {
    ...
    function setDevlopmentAddress(address _addressDistribute) external onlyOwner {
        addressDistribute = _addressDistribute;
    }
    function setFees(uint256 _buyFee, uint256 _sellFee, uint256 _transFee, uint256 _reflectFees, uint256 _tokenLiquidityPercent) external onlyOwner {
        buyFee = _buyFee;
        sellFee = _sellFee;
        transFee = _transFee;
        reflectFee = _reflectFees;
        tokenLiquidityPercent = _tokenLiquidityPercent;
    }
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "Pair note Remove");
        _setAutomatedMarketMakerPair(pair, value);
    }
    function setTokenSell(uint256 _numberOfTokensToSwapToLiquidity) external onlyOwner {
        numberOfTokensToSwapToLiquidity = _numberOfTokensToSwapToLiquidity * 10**_decimals;
    }
    function setMaxTx(uint256 _maxTxAmount, uint256 _maxWalletBalance) external onlyOwner {
        maxTxAmount = _maxTxAmount  * 10**_decimals;
        maxWalletBalance = _maxWalletBalance * 10**_decimals;
    }
    ...
    function setActiveCoolDown(bool _buyCooldownEnabled, bool _sellCooldownEnabled, uint8 _cooldownTimerInterval) external onlyOwner {
        ...
        sellCooldownEnabled = _sellCooldownEnabled; 
    ...
    function setExcludedAddress(address account, bool isVerify) external onlyOwner {
        excludedFromfee[account] = isVerify;
    }
    ...
    function setEnableBlackAndSniper(bool _blackEnabled) external onlyOwner {
        blackEnabled = _blackEnabled;
    }
    function setIsSwap(bool isTrue) external onlyOwner {
        swapAndLiquifyEnabled = isTrue; 
