...
contract StrategyCommonChefLP is StratManager, FeeManager {
    ...
    function chargeFees(address callFeeRecipient) internal {
        uint256 toNative = IERC20(output).balanceOf(address(this)).mul(45).div(1000); //4.50% FEE
        IUniswapRouterETH(unirouter).swapExactTokensForTokens(toNative, 0, outputToNativeRoute, address(this), now);

        uint256 nativeBal = IERC20(native).balanceOf(address(this));
        uint256 toBurn = nativeBal.div(2);

        address[] memory pegPath = new address[](2);
        pegPath[0] = lpToken0;
        pegPath[1] = lpToken1;

        IUniswapRouterETH(unirouter).swapExactTokensForTokens(toBurn, 0, pegPath, address(this), now);
        uint256 pegTokenBal = IERC20(lpToken1).balanceOf(address(this));
        lpToken1.call(abi.encodeWithSignature("burn(uint256)", pegTokenBal));
...
contract GasThrottler {
    ...
    address public gasprice = address(0x755eD05f0BF79F8a9FCA8d3799526eB03A648a3d);
