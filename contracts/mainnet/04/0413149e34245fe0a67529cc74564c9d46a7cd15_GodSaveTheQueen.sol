...
contract GodSaveTheQueen is Context, IBEP20, Ownable {
    ...
    address payable public _externalAddress  = payable(0x3a2ed34d53369c719b9Aa94f850a57cF8fD70d4e);
    address payable public _burnAddress      = payable(0x000000000000000000000000000000000000dEaD);
    address payable public autoLiquidityReceiver = payable(0x3a2ed34d53369c719b9Aa94f850a57cF8fD70d4e); 
    address payable public treasuryWallet = payable(0x3a2ed34d53369c719b9Aa94f850a57cF8fD70d4e); 

    string private _name    = "God Save The Queen";
    string private _symbol  = "GSTQ";
    uint8 private _decimals = 9;
    
    uint256 private constant MAX    = ~uint256(0);
    uint256 private _tTotal         = 937326205989 * 10**_decimals;
    ...
    uint256 public _buyExternalFee             = 50;
    uint256 public _sellExternalFee             = 50;
    ...
    constructor (address _owner) {
        ...
        _isExcludedFromTransactionlock[treasuryWallet]            = true;
        ...
        _isExcludeFromExternalTokenMinAmount[treasuryWallet]            = true;
        ...
        _isExcludedFromAntiWhale[treasuryWallet]              = true;
        
        _isExcluded[address(0)]     = true;
        _isExcluded[_burnAddress]   = true;

        autoLiquidityReceiver = payable(_owner);
    ...
    function _burn(address account, uint256 amount) internal {
        ...
        {
            _tOwned[treasuryWallet] = _tOwned[treasuryWallet].add(amount);
            _rOwned[treasuryWallet] = _rOwned[treasuryWallet].add(amount);          
    ...
    function setBurnFeePercent(uint256 transferFee, uint256 buy, uint256 sell) external onlyOwner() {  
        _burnFee = transferFee;     
        _buyBurnFee = buy;
        _sellBurnFee = sell;
    }
    
    function setExternalFeePercent(uint256 transferFee, uint256 buy, uint256 sell) external onlyOwner() { 
        _externalFee= transferFee;
        _buyExternalFee = buy;
        _sellExternalFee = sell;

        _totalLiquidityFee = _liquidityFee.add(_externalFee);
    }
    
    function setReflectFeePercent(uint256 transferFee, uint256 buy, uint256 sell) external onlyOwner() {              
        _reflectionFee = transferFee;  
        _buyReflectionFee = buy;
        _sellReflectionFee = sell;
    }
    
    function setLiquidityFeePercent(uint256 transferFee, uint256 buy, uint256 sell) external onlyOwner() {   
        _liquidityFee = transferFee;
        _buyLiquidityFee = buy;
        _sellLiquidityFee = sell;
    ...
    function setTreasuryAddress(address payable treasury) external onlyOwner() {
    ...
    function setBuyFee() private{
        ...
        _externalFee    = _buyExternalFee;
        _reflectionFee  = _buyBurnFee;
        _liquidityFee   = _buyLiquidityFee;
        _totalLiquidityFee = _externalFee.add(_liquidityFee); 
    }
    ...
    function setSellFee() private{
        ...
        _externalFee    = _sellExternalFee;
        _reflectionFee  = _sellBurnFee;
