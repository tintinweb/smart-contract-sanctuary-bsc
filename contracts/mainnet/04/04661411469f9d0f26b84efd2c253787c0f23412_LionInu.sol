...
contract LionInu is IERC20, Context, Ownable {
    ...
    address private Lion__Inu__Marketing_Wallet = 0xfd868621FE787B08C7473B68fD32A4D089c4DF07;
    address private Lion__Inu__Team_Wallet = 0xfd868621FE787B08C7473B68fD32A4D089c4DF07;
    
    string private constant _symbol = "LIONINU";
    string private constant _name = "Lion Inu";

    uint256 public Lion__Inu__max_BuyLimit = 2000000 * 10**9;
    uint256 public Lion__Inu__max_SellLimit = 400000 * 10**9;

    uint256 public Lion__Inu__max_WalletLimit = 2000000  * 10**9;
    ...
    Taxes public sellTaxes = Taxes(0, 25, 10, 0);
    Taxes public taxes = Taxes(0, 25, 10, 0);
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[Lion__Inu__Marketing_Wallet] = true;
        _isExcludedFromFee[Lion__Inu__Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[Lion__Inu__Marketing_Wallet] = true;
        allowedTransfer[Lion__Inu__Team_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= Lion__Inu__max_BuyLimit, "You are exceeding Lion__Inu__max_BuyLimit");
            require(balanceOf(to) + amount <= Lion__Inu__max_WalletLimit, "You are exceeding Lion__Inu__max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= Lion__Inu__max_SellLimit, "You are exceeding Lion__Inu__max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= Lion__Inu__max_WalletLimit, "You are exceeding Lion__Inu__max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(Lion__Inu__Marketing_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(Lion__Inu__Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function Lion__Inu__Update_MarketingWallet(address newWallet) external onlyOwner{
        Lion__Inu__Marketing_Wallet = newWallet;
    }
    
    function Lion__Inu__Update_TeamWallet(address newWallet) external onlyOwner{
        Lion__Inu__Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        Lion__Inu__max_BuyLimit = maxBuy * 10**decimals();
        Lion__Inu__max_SellLimit = maxSell * 10**decimals();
    }
    
    function Lion__Inu__Max_Walletlimit(uint256 amount) external onlyOwner{
        Lion__Inu__max_WalletLimit = amount * 10**decimals();
    }
