...
contract ATHPool2 is Ownable {
    ...
    constructor(IATHPool _athPool, IATHPool _athPool1, IATHPool _athPool2, IATHPool _athPool3) {
        ...
        athPool2 = _athPool2;
        athPool3 = _athPool3;
        athPool4 = IATHPool(0xc8bb48490c5EB32fB3C412C3B51c9cC34ACA676C);
    ...
    function availableClaimAmount(uint _pid, address _user) public view returns(uint available) {
        ...
        uint buyAmount = athPool.isBuyer(_user, idoToken) + athPool1.isBuyer(_user, idoToken) + athPool2.isBuyer(_user, idoToken) + athPool3.isBuyer(_user, idoToken) + athPool4.isBuyer(_user, idoToken);
        uint userClaimed = athPool.claimed(_user, idoToken) + athPool1.claimed(_user, idoToken) + athPool2.claimed(_user, idoToken) + athPool3.claimed(_user, idoToken) + claimed[_user][idoToken] + athPool4.claimed(_user, idoToken) + claimed[_user][idoToken];
