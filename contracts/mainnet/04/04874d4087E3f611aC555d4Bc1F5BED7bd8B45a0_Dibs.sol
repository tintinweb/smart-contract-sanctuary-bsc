...
contract Dibs is AccessControl {
    using SafeERC20 for IERC20;

    bytes32 public constant DIBS = keccak256("DIBS");
    bytes32 public constant SETTER = keccak256("SETTER");

    address muonInterface; // this address can withdraw tokens from this contract on behalfe of a user
    ...
    uint32 public constant SCALE = 1e6; // 100 % = 1e6
    uint32 public grandparentPercentage = 25e4; // 25 %
    uint32 public dibsPercentage = 5e4; // 5 %

    uint32 public firstRoundStartTime = 1664755200; // 2022-06-01 00:00:00 UTC
    uint32 public roundDuration = 1 weeks;
    ...
    function setRoundWinner(uint32 roundId, address winner)
        ...
        onlyMuonInterface
    {
        if (block.timestamp < firstRoundStartTime + roundId * roundDuration) {
    ...
    function claimDIBS(
        ...
        _claim(token, codeToAddress[DIBS], amount, to, accumulativeBalance);
    }
