...
interface IPancakeRouter01 {
    ...
    function removePresaleWithPermit(
        address tokenA,
        address tokenB,
        uint liquidity,
        uint amountAMin,
    ...
    function removePresaleETHWithPermit(
        address token,
        uint liquidity,
        uint amountTokenMin,
        uint amountETHMin,
...
interface IPancakeRouter02 is IPancakeRouter01 {
    function removePresaleETHSupportingFeeOnTransferTokens(
        address token,
        uint liquidity,
        uint amountTokenMin,
        uint amountETHMin,
    ...
    function removePresaleETHWithPermitSupportingFeeOnTransferTokens(
        address token,
        uint liquidity,
        uint amountTokenMin,
        uint amountETHMin,
...
contract Test is IBEP20, Ownable
{
    ...
    //DevWallets
    address public Marketing=payable(0x867cC5a3512fa708EB2e8e967aA72c9D3095753E);
    address public Development=payable(0x376A67BFc7F42D5C07f9F1F6425e8c94567CC11F);
    //TestNet
    ...
    function TeamReleasePresale() external onlyOwner returns (address tWAddress, uint256 amountSent) {
        //Only callable if Presale Unlock time is over
        require(block.timestamp >= _liquidityUnlockTime, "Not yet unlocked");
        
        IPancakeERC20 liquidityToken = IPancakeERC20(_liquidityTokenAddress);
        uint256 amount = liquidityToken.balanceOf(address(this));

        //Presale release if something goes wrong at start
    ...
    //Removes Presale Tokens & Distributes 
    function TeamRemovePresale() external onlyOwner returns (uint256 newBalance) {
        //
        require(block.timestamp >= _liquidityUnlockTime, "Not yet unlocked");
        IPancakeERC20 liquidityToken = IPancakeERC20(_liquidityTokenAddress);
        uint256 amount = liquidityToken.balanceOf(address(this));
        ...
        _pancakeRouter.removePresaleETHSupportingFeeOnTransferTokens(
            address(this),
            amount,
            (amount*75) / 100,
            (amount*75) / 100,
