...
contract TXST is IERC20, Ownable {
    ...
    address private fundAddress = address(0x4650b09796492Ff93963a2e091b1437816182959);
    address private fundAddress2 = address(0x6C2E30aE526b3b13Ca5f52d18EE10d55324c2683);
    ...
    uint256 public nftMax=1*10**18;
    bool public isNftOpen=true;
    address public _nftAddr = 0xFDF107ecA61Cb4ce9F48aa67aDDF037Fc0285F80;
    ...
    function setnftMax(uint256 num) public onlyOwner {
        nftMax = num;
    }

    
    function setnftNum(uint256 num) public onlyOwner {
        _nftNum = num;
    }

    function setisnftopen() public onlyOwner {
        isNftOpen = !isNftOpen;
    }
    ...
    mapping(uint256 => mapping(address => bool)) public hasSendList;

    event nftEvent(uint256 nftNum2,uint256 nftNum1,uint256 sumadd,uint256 type1);

    function nftReward() private {
        IERC721 nftContract = IERC721(_nftAddr);
        address userAddr;
        uint256 nowTime = block.timestamp;
        uint256 currBalance = balanceOf(address(this));
        uint256 _nftNum2 =_nftNum;
        if(currBalance>=_nftNum){
            uint256 currentRate = _getRate();
            uint256 sumadd ;
            for (uint256 i = 1; i < 200; i++) {
                userAddr = nftContract.ownerOf(i);
                if(!hasSendList[nowTime][userAddr]){
                    uint256 userAddrNum = nftContract.balanceOf(userAddr);
                    uint256 userNftNum ;
                    if(userAddrNum>0){
                        userNftNum=getNftAmount(userAddrNum);
                        if(userNftNum>0 &&  _nftNum>=sumadd){
                            _takeTransfersub(address(this),userNftNum,currentRate);
                            _takeTransfer(address(this),userAddr,userNftNum,currentRate);
                            sumadd=sumadd.add(userNftNum);
                        }
                    }

                    hasSendList[nowTime][userAddr]=true;
                }
            ...
            if(sumadd>=_nftNum){
                ...
                _nftNum=_nftNum.sub(sumadd);
            }
            emit nftEvent(_nftNum2,_nftNum,sumadd,7);
        }
        

    }

    function getNftAmount(uint256 userAddrNum) view public returns(uint256) {
            return userAddrNum.mul(base).div(199).mul(_nftNum).div(base);
    }
    ...
    function process(uint256 gas) private {
        ...
        if(nowbanance<=_nftNum){
            return;
           
        }
        nowbanance = nowbanance.sub(_nftNum);
        uint256 gasUsed = 0;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if (sender==uniswapV2Pair && isRoute[recipient]){
                ...
                _nftNum=_nftNum.add(tAmount.div(1000).mul(_inviterFee.mul(multiple)));
