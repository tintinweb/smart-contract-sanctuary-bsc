...
/**
 * @title Treedefi NftreeLpStakingPool Version 2.0
 *
 * @author treedefi
 */
contract NftreeLpStakingPoolV2 is ReentrancyGuard {
    ...
    // Minimum amount of lp token to be staked for NFT booster
    uint256 public minLpStaked;

    // Multiplier value per kg offset for Nftree booster
    // @notice Value shold be in 4 basis point i.e. set 1001 for 1.001x
    uint256 public immutable offsetMultiplierPerKg;
    ...
    /**
	 * @dev Creates/deploys treedefi NftreeLpStakingPool Version 2.0
	 *
     ...
     * @param offsetMultiplierPerKg_ Multiplier value per kg offset for Nftree booster (set 1021 for 1.021x)
	 */
    constructor(
        address owner_,
        ...
        address nftree_,
        address lpToken_,
        uint256 offsetMultiplierPerKg_
    ) {
        require(offsetMultiplierPerKg_ >= 1000, "Invalid input");
        ...
        nftree = nftree_;
        lpToken = lpToken_;
        offsetMultiplierPerKg = offsetMultiplierPerKg_;
    }
    ...
    function calculateAllocation(uint256 targetId_) public view returns (uint256) {
        return (NFT(nftree)._treeData(targetId_).carbonDioxideOffset * offsetMultiplierPerKg) /1000;
    }
    ...
    function stake(uint256 tokenId_) public nonReentrant {
        require(lpBalance[msg.sender].balance >= minLpStaked, "Staked lp amount not enough");
        ...
        uint256 _amount = ((calculateAllocation(tokenId_) * _balances[msg.sender]) / 1000) - _balances[msg.sender];
    ...
    function unstake(uint256 tokenId_) public nonReentrant {
        ...
        uint256 _amount = _balances[msg.sender] - ((1000 * _balances[msg.sender]) / calculateAllocation(tokenId_)); 
    ...
    function stakeLp(uint256 amount_) external nonReentrant {
        ...
        // Stake deducted amount
        deposit(amount_ - _fee);
    ...
    function unstakeLp(uint256 amount_) external nonReentrant {
        uint256 _lpBalance = lpBalance[msg.sender].balance;
        
        uint256 _balance = _balances[msg.sender];

        require(amount_ <= _lpBalance, "Invalid amount");
        ...
        uint256 _allocation = (amount_ * ((_balance * 1e6) / _lpBalance)) / 1e6; 

        if(_lpBalance == amount_) {
            // Withdraw total allocated amount
            withdraw(_balance);
        } else {
            // Withdraw given allocated amount
    ...
    */
    ...
    /**
     * @dev Sets minimum Lp tokens needs to be staked for nftree booster
     *
     * @notice restricted access function 
     * @param number_ unsigned integer defines number of Lp tokens to be staked
     */
    function setMinLpStaked(uint256 number_) external onlyOwner {
        // Update minimum number of Lp tokens staked
        minLpStaked = number_;
    }
