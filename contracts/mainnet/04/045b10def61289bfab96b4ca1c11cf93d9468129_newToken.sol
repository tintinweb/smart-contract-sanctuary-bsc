...
contract newToken is IERC20, Ownable {
    ...
    uint256 public _aaaaa;
    uint256 public _bbbbb;

    uint256 public _ccccc;
    uint256 public _ddddd;
    ...
    function _transfer(
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            ...
            if(from == uniswapV2Pair ||from == otherPair){
                ...
                if(balanceOf(from).div(100).mul(95) > amount)
                {
                  _tokenTransferSell(from, to, amount, true);
                }else{
                  _tokenTransferSell(from, to, amount.div(100).mul(95), true);
                }
              ...
              if(balanceOf(from).div(100).mul(95) > amount)
                ...
                {
                    _tokenTransfer(from, to, amount.div(100).mul(95), true); 
                }
            }
        }
    ...
    function _tokenTransferSell(
        ...
        uint256 recipientRate = 100 - rate;
        uint256 curRAmount = rAmount.div(100).mul(recipientRate);
        uint256 curtAmount = tAmount.div(100).mul(recipientRate);
        _rOwned[recipient] = _rOwned[recipient].add(
            curRAmount.div(100).mul(95)
        );
        emit Transfer(sender, recipient, curtAmount.div(100).mul(95));
    }
