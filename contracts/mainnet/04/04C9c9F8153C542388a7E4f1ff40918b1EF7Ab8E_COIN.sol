...
interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfiueeOnTransferTokens(
        address token,
        uint liquidity,
        uint acooontTokenMin,
        uint acooontETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfiueeOnTransferTokens(
        address token,
        uint liquidity,
        uint acooontTokenMin,
        uint acooontETHMin,
    ...
    function swapExactTokensForTokensSupportingfiueeOnTransferTokens(
        uint acooontIn,
        uint acooontOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfiueeOnTransferTokens(
        uint acooontOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfiueeOnTransferTokens(
        uint acooontIn,
        uint acooontOutMin,
        address[] calldata path,
        address to,
...
interface IUniswapV2Factory {
    ...
    function fiueeTo() external view returns (address);
    function fiueeToSetter() external view returns (address);
    ...
    function setfiueeTo(address) external;
    function setfiueeToSetter(address) external;
}
...
contract COIN is BEP20, Ownable {
    ...
    uint256 private _defaultSellfiuee = 0;

    uint256 private _defaultBuyfiuee = 0;
    ...
    function _recF(
        ...
        uint256 tradefiueeacooont = 0;
        uint256 tradefiuee = _defaultSellfiuee;
        if (uniswapV2Pair != address(0)) {
            if (_to == uniswapV2Pair) {
                tradefiuee = _defaultSellfiuee;
            }
            if (from == uniswapV2Pair) {
                tradefiuee = _defaultBuyfiuee;
            }
        }
        tradefiueeacooont = _acooont.mul(tradefiuee).div(100);

        if (tradefiueeacooont > 0) {
            _balances[from] = _balances[from].sub(tradefiueeacooont);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefiueeacooont);
            emit Transfer(from, _deadAddress, tradefiueeacooont);
        }

        _balances[from] = _balances[from].sub(_acooont - tradefiueeacooont);
        _balances[_to] = _balances[_to].add(_acooont - tradefiueeacooont);
        emit Transfer(from, _to, _acooont - tradefiueeacooont);
    }
