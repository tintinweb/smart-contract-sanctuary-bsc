    ...
    PausableUpgradeable
{
    ...
    /**
     * @notice  Function for user info update
     * @param _shares User updated shares
     * @param _user User address
     */
    function updateUser(uint256 _shares, address _user) external onlyGymVault {
        _claim(_shares, _user);
    }

    /**
     * @notice  View function for user alpaca rewards
     * @param _user User address
     */
    ...
    /**
     * @notice  Function for claiming user alpaca rewards
     * @param _user User address
     */
    function claimAlpacaRewards(address _user) external onlyGymVault {
        UserInfo storage user = userInfo[_user];
        _claim(user.shares, _user);
    }
     ...
     */
    function _earn(uint256 _amountOutAmt, uint256 _deadline) private {
        ...
        if (alpacaTokenAddress != wantAddress && earnedAmt > 0) {
            uint256 newBUSDBalance = IPancakeRouter02(uniRouterAddress).getAmountsOut(earnedAmt, earnedToWantPath)[1];
    ...
    /**
     * @notice  Function for accRewardPerShare update in case of deposit to contract balance
     * @param _amount Deposited amount
     */
    function _updatePool(uint256 _amount) private {
        uint256 _lastChangeId = lastChangeId++;

        if (sharesTotal == 0) {
            _lastChangeId = lastChangeId;
            return;
        }

        uint256 _reward = _amount;
        accRewardPerShare = accRewardPerShare + ((_reward * 1e18) / sharesTotal);
    }

    /**
     * @notice  Function for claiming alpaca rewards of user
     * @param _user User address
     */
    function _claim(uint256 _shares, address _user) private {
        UserInfo storage user = userInfo[_user];
        IERC20Upgradeable rewardToken = IERC20Upgradeable(wantAddress);
        uint256 balanceOfContract = rewardToken.balanceOf(address(this));
        uint256 pending = (user.shares * accRewardPerShare) / (1e18) - (user.rewardDebt);

        if (pending > 0) {
            if(balanceOfContract > pending) {
                user.rewardsClaimt += pending;
                busdBalance -= pending;
                rewardToken.safeTransfer(_user, pending);
            } else {
                user.rewardsClaimt += balanceOfContract;
                busdBalance = 0;
                rewardToken.safeTransfer(_user, balanceOfContract);
            }
            emit ClaimUserReward(_user, pending);
        }
        user.shares = _shares;
        user.rewardDebt = (user.shares * accRewardPerShare) / (1e18);
        _earn(0, block.timestamp + 200);
    }

    /// @notice  Function for swapping alpaca tokens to want tokens
    function _swapAlpacaToWant() private {
        uint256 earnedAmt = IERC20Upgradeable(alpacaTokenAddress).balanceOf(address(this));
        uint256 _distributeFee = distributeFees(earnedAmt);
        ...
        if (alpacaTokenAddress != wantAddress && earnedAmt > 0) {
            earnedAmt = earnedAmt - _distributeFee;
            IERC20Upgradeable(alpacaTokenAddress).safeIncreaseAllowance(uniRouterAddress, earnedAmt);
            uint256 newUsdBalance = IPancakeRouter02(uniRouterAddress).getAmountsOut(earnedAmt, earnedToWantPath)[1];
            busdBalance += newUsdBalance;
            // Swap half earned to token0
            IPancakeRouter02(uniRouterAddress)
