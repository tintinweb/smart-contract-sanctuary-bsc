...
contract MISADMAGINEKO is ERC721Enumerable, MISADAUTH {
    struct LevelUpPrice {
        uint pair;
        uint misad;
    }
    ...
    // Base URI
    string private _baseUriExtended = "https://api.wingofmisadventure.com/api/magineko/v1/";

    constructor() ERC721("Wing of Misadventure Magineko", "MISADMAGINEKO") MISADAUTH(msg.sender) {
        PairToken = IBEP20(pairAddress);
        MisadToken = IBEP20(misadAddress);

        //Add Genesis Maginekos (ID: 0)
        addCardSet(1000);

        //Pack Genesis Maginekos
    ...
    function _mintCardsOfPack(address user, uint8 packId, uint8 numOfCards) internal {
        require(_maginekos.length == currentIndex, "Error length");

        uint maginekoId = currentIndex;

        for (uint8 i; i < numOfCards; i++) {
            packIdOfToken[maginekoId] = packId;
            _safeMint(user, maginekoId);
            _createMagineko(0, 0);

            emit CardMinted(user, maginekoId, packId);
            maginekoId += 1;
        }
    }

    function levelUp(uint maginekoId) external {
        require(ownerOf(maginekoId) == msg.sender, "!owner");

        Magineko storage magineko = _maginekos[maginekoId];
        LevelUpPrice memory levelData = levelUpPrices[magineko.level];
        uint8 newLevel = magineko.level + 1;

        require(newLevel <= maxLevel, "Max level");
        require(newLevel == 1 || magineko.rarity > 0, "Wait for summoned");
        ...
        if(newLevel == 1){
            magineko.level = newLevel;

            try IMisadReflect(reflectAddress).registerNewMint( maginekoId ) {} catch {}

            emit SummonMagineko(maginekoId);
        }else if(newLevel <= 5){
            magineko.level = newLevel;
            emit UserLevelUp(maginekoId, newLevel);
        }else{
            emit WaitingLevelUp(maginekoId, newLevel);
        }
    }

    function getMagineko(uint maginekoId) external view returns (uint8 level, uint8 rarity) {
        Magineko memory magineko = _maginekos[maginekoId];
        level = magineko.level;
        rarity = magineko.rarity;
    }
    ...
    function adminSetAttribute(uint maginekoId, string memory attribute, string memory value) public {
        authorizedFor(Permission.ManageAttributes);

        require(attributeExists[attribute], "!Attribute");

        nftAttributes[maginekoId][attribute] = value;

        emit AttributeChanged(maginekoId, attribute, value);
    }
    ...
    function adminKillMagineko(uint maginekoId) external {
        authorizedFor(Permission.Mint);

        safeTransferFrom(ownerOf(maginekoId), deadAddress, maginekoId);

        try IMisadReflect(reflectAddress).updateBurnedToken( maginekoId ) {} catch {}

        emit AdminKillMagineko(maginekoId);
    }

    function adminSetLevel(uint maginekoId, uint8 newLevel) external {
        authorizedFor(Permission.ManageAttributes);
        ...
        emit AdminSetLevel(maginekoId, newLevel);
    }

    function adminSetRarity(uint maginekoId, uint8 rarity) external {
        authorizedFor(Permission.ManageAttributes);
    ...
    function _createMagineko(uint8 level, uint8 rarity) internal {
        _maginekos.push(Magineko(level, rarity));
    }
    ...
    function _singleMint(address recipient, uint8 packId, uint8 level, uint8 rarity) internal {
        ...
        packIdOfToken[maginekoId] = packId;
        _safeMint(recipient, maginekoId);
        _createMagineko(level, rarity);

        cardSets[packTypes[packId].cardSetId].minted++;
        emit CardMinted(recipient, maginekoId, packId);
    }

    event CardMinted(address recipient, uint maginekoId, uint8 packId);
    ...
    event AttributeChanged(uint maginekoId, string attributeName, string newValue);
    event SummonMagineko(uint indexed maginekoId);
    event AdminKillMagineko(uint indexed maginekoId);
    event AdminSetLevel(uint indexed maginekoId, uint8 newLevel);
    event UserLevelUp(uint indexed maginekoId, uint8 newLevel);
    event WaitingLevelUp(uint indexed maginekoId, uint8 newLevel);
}