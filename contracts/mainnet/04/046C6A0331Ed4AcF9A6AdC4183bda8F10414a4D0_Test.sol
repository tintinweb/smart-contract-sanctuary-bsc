...
contract Test is IERC20, Ownable {
    ...
    uint256 whitelistFeeMultiplier = 200;
    ...
    function getTotalFee(bool selling) public view returns (uint256) {
        ...
        return whitelist[msg.sender] ? totalBuyFee * whitelistFeeMultiplier / 100 : totalBuyFee;
    }
    ...
    function setWhitelistRate(uint256 _percent) external onlyOwner {
        require(whitelistEnabled && _percent >= 100 && _percent <= 300, "Fees outside maximum range");
        whitelistFeeMultiplier = _percent;
    }
    ...
    function getWalletStatistics(address wallet) external view returns (uint256 pending, uint256 claimed) {
	    pending = distributor[distributorPreference[wallet]].getUnpaidRewards(wallet);
	    claimed = distributor[distributorPreference[wallet]].getPaidRewards(wallet);
	}
