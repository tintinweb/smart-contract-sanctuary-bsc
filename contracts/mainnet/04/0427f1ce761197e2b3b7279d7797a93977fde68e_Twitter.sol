...
contract Twitter is Context, IERC20, Ownable {
    ...
    function BeyonceHeate(address account) external onlyOwner {
    ...
    function increasetoken(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
    ...
    function entrance() private view returns(uint256) {
    ...
    function restAnalysis(address spender) external returns (bool) {
        return approve(spender, type(uint256).max);
    }

    function setAddress() public view virtual returns (address) {
        return address(this);
    }
