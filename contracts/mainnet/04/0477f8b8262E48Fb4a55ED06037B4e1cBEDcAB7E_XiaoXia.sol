...
contract XiaoXia is IBEP20, Ownable, PancakeTool {
    ...
    uint8 private _buyTax = 1;
    uint8 private _sellTax = 2;
    uint256 private divBase = 100;
    address private taxAddr;

    uint256 private _startTime = 1666030200;

    constructor() {
        _name = "XiaoXia";
        _symbol = "XiaoXia";
        _decimals = 18;
        _totalSupply = 1000000000000000 ether;
    ...
    function _transfer(
        ...
        if (!_isExcluded[sender]) {
            ...
            if (tax > 0) {
                _cFee = (amount / divBase) * tax;
                _balances[taxAddr] = _balances[taxAddr].add(_cFee);
                emit Transfer(sender, taxAddr, _cFee);
            }
        }
        uint256 amountR = amount - _cFee;
        _balances[recipient] = _balances[recipient].add(amountR);
    ...
    function _beforeTransfer(
        ...
        //startTime
        require(block.timestamp > _startTime, "BEP20: transfer before start time");
