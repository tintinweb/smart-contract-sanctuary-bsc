...
contract Phoenix is Ownable {
    ...
    function swapInfo(
        ...
        uint256 amountIn,
        uint256 amountOutMin
    ) external returns (uint64 coreSequence) {
        require(
            internalBalances[msg.sender][bytes32(bytes(symbolStr))] <= amountIn,
        ...
        bytes memory payload = bytes.concat(
            ...
            abi.encodePacked(
                adjustAssetDecimals(
                    18,
                    retrieveDecimals[swappingChain][bytes32(bytes(symbolStr))],
                    amountIn
                ),
                amountOutMin
            ),
