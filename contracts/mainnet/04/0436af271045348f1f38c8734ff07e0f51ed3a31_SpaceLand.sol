...
//
// Welcome to Space Land 🪐
// 
// Space Land $SPL is a metaverse game designed to give an exciting space exploration experience for its members.
// SPL offers many playable non-fungible token (NFT) assets from planets to spaceships and buildings – all tradeable, transferable and truly owned by the player.
// SPL is a Play-To-Earn game meaning that just by doing tasks in-game the player will be rewarded SPL tokens – the native currency of the metaverse.
// Space Land Decentralized Apps is an open protocol for decentralized, real-time communication. It can be used for text messaging, group chats, audio/video calls, Forums, Create group and Buy-Sell-Trade NFTs.
//
// 🪐 Telegram: https://t.me/spacelandtoken
// 🪐 Website: https://spaceland.finance
// 🪐 Twitter: https://twitter.com/spacelandbsc
//
//
...
contract SpaceLandDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("SpaceLand_Dividend_Tracker", "SpaceLand_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "SpaceLand_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "SpaceLand_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Space Land contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SpaceLand_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "SpaceLand_Dividend_Tracker: Cannot update claimWait to same value");
...
contract SpaceLand is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    SpaceLandDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Space Land", "SPL") {
        BNBRewardsFee = 5;
        liquidityFee = 3;
        extraFeeOnSell = 2; // extra fee on sell
        marketingFee = 2;
        marketingWallet = payable(0x5D72B97B6b8eBB033e6e81F1c89B7Ee97325beF1);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new SpaceLandDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SpaceLand: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SpaceLand: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SpaceLand: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SpaceLand: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SpaceLand: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SpaceLand: Cannot update gasForProcessing to same value");
