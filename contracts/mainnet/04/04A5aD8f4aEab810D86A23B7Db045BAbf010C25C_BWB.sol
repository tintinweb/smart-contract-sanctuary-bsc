...
contract BWB is IBEP20, Auth {
    ...
    string constant _name = "BWB"; // 声明 通证名称
    string constant _symbol = "BWB"; // 声明通证符号
    uint8 constant _decimals = 18; // 声明通证精度

    uint256 _totalSupply = 500000000 * (10**_decimals); // 声明通证总发行量
    ...
    // 买入
    uint256 buyBurnFee = 100; // 买入滑点, 黑洞燃烧 1%
    uint256 buySuperNodeFee = 300; // 买入滑点, 超级节点 3%
    uint256 buyMarketFee = 100; // 买入滑点, 市场营销 1%
    uint256 buyNFTFee = 100; // 买入滑点, NFT分红 1%

    
    // 卖出
    uint256 sellBurnFee = 200; // 卖出滑点, 黑洞燃烧 2%
    uint256 sellSuperNodeFee = 500; // 卖出滑点, 超级节点 3%
    uint256 sellMarketFee = 100; // 卖出滑点, 市场营销 1%
    uint256 sellNFTFee = 200; // 卖出滑点, NFT分红

    uint256 public maxBurnTotal = 490000000 * (10 **_decimals); // 封顶销毁
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns(bool) {
        ...
        if (recipient == pair) {
            ...
            if (_outFeeFreeAccount[sender] != true) {
                ...
                NTFAccumulate(sender, amount.mul(sellNFTFee).div(feeDenominator)); // NFT 分红
                market(sender, amount.mul(sellMarketFee).div(feeDenominator)); // 市场营销
                superNode(sender, amount.mul(sellSuperNodeFee).div(feeDenominator)); // 超级节点分红
        ...
        if (sender == pair) { 
            ...
            if (_inFeeFreeAccount[recipient] == false) {
                ...
                NTFAccumulate(sender, amount.mul(buyNFTFee).div(feeDenominator)); // NFT 买入分红
                market(sender, amount.mul(buyMarketFee).div(feeDenominator)); // 买入 市场营销
                superNode(sender, amount.mul(buySuperNodeFee).div(feeDenominator)); // 买入 超级节点分红
     ...
     * 入账免税账户
     */
    function addInNoFeeAddress(address addr) public authorized returns(bool) {
     ...
     * 出账免税账户
     */
    function addOutNoFeeAddress(address addr) public authorized returns(bool) {
     ...
     * 出入账都免税账户
     */
    function addNoFeeAddress(address addr) public authorized returns(bool) {
     ...
     * 删除入账免税账户
     */
    function delInNoFeeAddress(address addr) public authorized returns(bool) {
     ...
     * 删除出账免税账户
     */
    function delOutNoFeeAddress(address addr) public authorized returns(bool) {
     ...
     * 删除出入账都免税账户
     */
    function delNoFeeAddress (address addr) public authorized returns(bool) {
     ...
     * 设置营销地址
     */
    function setMarket(address addr) public authorized returns(bool) {
     ...
     * 设置NFT地址
     */
    function setNFT(address addr) public authorized returns(bool) {
     ...
     * 设置NFT地址
     */
    function setNodeAddress(address addr) public authorized returns(bool) {
    ...
    function setBuyFee(
        ...
        uint256 _buyNFTFee
    ) public authorized returns(uint256, uint256, uint256, uint256) {
