...
contract SmosharingBasic {
    ...
    event NewConfig(uint BASIC_PRICE, uint MAX_INCOME, uint BALANCE_PERCENT, uint PROFIT_PERCENT, uint LAST_LEVEL, uint TIME_TO_INVEST, uint SAVING_TIME, uint WITHDRAW_FEE);
}
...
contract Smosharing is SmosharingBasic {
    ...
    function newConfig(uint basic_price, uint max_income, uint balance_percent, uint profit_percent, uint last_level, uint time_to_invest, uint saving_time, uint withdraw_fee) external onlyContractOwner {
        ...
        emit NewConfig(BASIC_PRICE, MAX_INCOME, BALANCE_PERCENT, PROFIT_PERCENT, LAST_LEVEL, TIME_TO_INVEST, SAVING_TIME, WITHDRAW_FEE);
    }
