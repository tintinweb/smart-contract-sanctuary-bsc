...
contract King is Context, IERC20, Ownable {
    ...
    address private _anyERC20Token = address(0);
    ...
    uint256 private numTokensSellToAddToLiquidity = 50 * 10**uint256(_decimals);
    ...
    bool public swapFundAndMarketingEnabled = false;
    ...
    function setSwapFundAndMarketingEnabled(bool _enabled) public onlyOwner {
        swapFundAndMarketingEnabled = _enabled;
    }
    ...
    function _takeFund(address sender, uint256 tFund) private {
        if(!swapFundAndMarketingEnabled){
            _balances[_fundAddress] = _balances[_fundAddress].add(tFund);
            emit Transfer(sender, _fundAddress, tFund);
        } else {
            _balances[address(this)] = _balances[address(this)].add(tFund);
            emit Transfer(sender, address(this), tFund);
        }
    ...
    function swapAndERC20Liquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 lsFundFee = _sellFundFee;
        uint256 lsMarketingFee = _sellMarketingFee;
        uint256 lsLiquidityFee = _buyLiquidityFee;
        ...
        uint256 addLiquidityRate = uint256(100).mul(lsLiquidityFee).div(lsLiquidityFee.add(lsFundFee).add(lsMarketingFee));
        uint256 fundRate = uint256(100).mul(lsFundFee).div(lsLiquidityFee.add(lsFundFee).add(lsMarketingFee));
        uint256 addLiquidityNumber = contractTokenBalance.mul(addLiquidityRate).div(100);
        uint256 fundNumber = contractTokenBalance.mul(fundRate).div(100);
        uint256 marketingNumber = contractTokenBalance.sub(addLiquidityNumber).sub(fundNumber);
        
        uint256 half = addLiquidityNumber.div(2);
        uint256 otherHalf = addLiquidityNumber.sub(half);

        uint256 initialBalance = IERC20(_anyERC20Token).balanceOf(address(this));

        // swap tokens for anyERC20Token
        swapTokensForAnyERC20Token(half); // <- this breaks the ETH -> King swap when swap+liquify is triggered

        // how much anyERC20Token did we just swap into?
        ...
        if(swapFundAndMarketingEnabled) {
            swapTokensForDividendERC20Token(fundNumber.add(marketingNumber));
        }
    ...
    function swapTokensForAnyERC20Token(uint256 tokenAmount) private {
        if (tokenAmount > 0) {
            ...
            uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
                tokenAmount,
                0, // accept any amount of anyERC20Token token
                path,
                address(this),
                block.timestamp
            );
    ...
    function swapTokensForDividendERC20Token(uint256 tokenAmount) private {
        
        uint256 fundRate = uint256(100).mul(_sellFundFee).div(_sellFundFee.add(_sellMarketingFee));
        if (tokenAmount > 0) {
            address[] memory path = new address[](3);
            path[0] = address(this);
            path[1] = uniswapV2Router.WETH();
            ...
            uint256 dividends = IERC20(_anyERC20Token).balanceOf(address(this));
            uint256 divFund = dividends.mul(fundRate).div(100);
            uint256 divMarketing = dividends.sub(divFund);
            IERC20(_anyERC20Token).transfer(address(_fundAddress), divFund);
            IERC20(_anyERC20Token).transfer(address(_marketingAddress), divMarketing);
        }
    }
    
    function swapTokensForDividendBNBToken(uint256 tokenAmount) private {
        ...
        if (tokenAmount > 0) {
            ...
            uint256 divFund = dividends.mul(fundRate).div(100);
            uint256 divMarketing = dividends.sub(divFund);
            payable(_fundAddress).transfer(divFund);
            payable(_marketingAddress).transfer(divMarketing);
        }
    }
}