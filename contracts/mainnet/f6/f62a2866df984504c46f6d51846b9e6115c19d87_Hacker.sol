...
contract BNBDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("BNB_Dividend_Tracker", "BNB_Dividend_Tracker", 18) {
    	claimWait = 3600; 
        minimumTokenBalanceForDividends = 1 * (10**18); //0.01%
    }
...
contract Hacker is BEP20, Ownable{
   ...
   using SafeMath for uint256;
    ...
    uint256 public swapTokensAtAmount = 1 * (10**17); 
    uint256 public maxTransactionAmount = 1000 * (10**18); //2%
    ...
    uint256 public _SellBNBRewardsFee = 2;
    ...
    uint256 public maxWalletToken = 1000 * (10**18); //2%

    address public _marketingWalletAddress = 0xd29264Be0eA0Bf03A2C585903dC8766D5148BFAA;//
    address public _developmentWalletAddress = 0x4E5AB57B153581e6d01Fce26DaA790258E4622d8;//
    ...
    constructor() BEP20("BNHacker Token", "Hacker", 18) {
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "Hacker: The dividend tracker already has that address");

        BNBDividendTracker newDividendTracker = BNBDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "Hacker: The new dividend tracker must be owned by the Hacker token contract");
    ...
    function updatePancakeswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeswapV2Router), "Hacker: The router already has that address");
    ...
    function excludeOrIncludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Hacker: Account is already the value of 'excluded'");
    ...
    {
        require(_isExcludedFromMaxTx[account] != excluded, "Hacker: Account is already the value of 'excluded'");
    ...
    {
        require(_isExcludedFromMaxWallet[account] != excluded, "Hacker: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != pancakeswapV2Pair, "Hacker: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Hacker: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Hacker: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Hacker: Cannot update gasForProcessing to same value");
   ...
   function setMaxtx(uint256 _maxTxAmount) public onlyOwner {
    ...
    function _transfer(
        ...
        if (lanuchTime == 0 && to == pancakeswapV2Pair) {
        ...
        if(takeFee) {
           ...
           if ((block.number <= launchedBlock + killblock) && from == pancakeswapV2Pair) {   
