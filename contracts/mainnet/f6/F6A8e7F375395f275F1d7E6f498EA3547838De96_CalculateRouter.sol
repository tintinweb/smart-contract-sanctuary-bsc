...
contract CalculateRouter is Ownable {
    ...
    function addFactory(IFactory _factory, uint _fee) external onlyOwner {
        for (uint i = 0; i < factories.length; i ++) {
            require(address(factories[i]) != address(_factory), "factory already exists");
        }
    ...
    function getReserves(IFactory _factory, address _tokenA, address _tokenB) internal view returns (uint reserveA, uint reserveB) {
        (address token0,) = sortTokens(_tokenA, _tokenB);
        address pair = _factory.getPair(_tokenA, _tokenB);
    ...
    function getAmountsOut(IFactory[] memory _factories, uint[] memory _fees, uint _amountIn, address[] memory _path) internal view returns (uint[] memory amounts, uint[] memory impact) {
        ...
        for (uint i; i < _path.length - 1; i++) {
            (uint reserveIn, uint reserveOut) = getReserves(_factories[i], _path[i], _path[i + 1]);
    ...
    function getAmountsIn(IFactory[] memory _factories, uint[] memory _fees, uint amountOut, address[] memory _path) internal view returns (uint[] memory amounts, uint[] memory impact) {
        assert(_path.length >= 2 && _factories.length == _fees.length && _factories.length + 1 == _path.length);
        amounts = new uint[](_path.length);
        impact = new uint[](_path.length - 1);
        amounts[amounts.length - 1] = amountOut;
        for (uint i = _path.length - 1; i > 0; i--) {
            (uint reserveIn, uint reserveOut) = getReserves(_factories[i - 1], _path[i - 1], _path[i]);
    ...
    function calculateAmountOut(address _tokenA, address _tokenB, uint _amountIn) external view returns (RouteInfo[] memory routes) {
        ...
        for (uint i = 0; i < paths.length; i ++) {
            ...
            (uint[] memory amountsOut, uint[] memory impact) = getAmountsOut(path.factories, path.fees, _amountIn, path.path);
    ...
    function calculateAmountIn(address _tokenA, address _tokenB, uint _amountOut) external view returns (RouteInfo[] memory routes) {
        ...
        for (uint i = 0; i < paths.length; i ++) {
            SwapPath memory path = paths[i];
            (uint[] memory amountsIn, uint[] memory impact) = getAmountsIn(path.factories, path.fees, _amountOut, path.path);
