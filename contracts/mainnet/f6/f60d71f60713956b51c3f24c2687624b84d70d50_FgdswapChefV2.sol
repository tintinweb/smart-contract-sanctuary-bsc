...
// MasterChef is the master of FGDswap. He can make FGDswap and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once FGDswap is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract FgdswapChefV2 is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        uint256 pendingDebt;    // Reward debt. before withdraw time.
    }

    // Info of each pool.
    struct PoolInfo {
        IBEP20 lpToken;           // Address of LP token contract.
        uint256 allocPoint;       // How many allocation points assigned to this pool. FGDswap to distribute per block.
        uint256 lastRewardBlock;  // Last block number that FGDswap distribution occurs.
        uint256 accTokenPerShare;   // Accumulated FGDswap per share, times 1e12. See below.
    ...
    // The FGDswap TOKEN!
    address public zeusAddress;
    //Referrer Address
    address public referralAddress;
    // FGDswap tokens created per block.
    ...
    // The block number when ZEFGDswap mining starts.
    ...
    function setWithdrawTime(uint256 _time) public onlyOwner {
        withdrawTime = _time;
    }
    ...
    function pendingToken(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 lpSupply = address(pool.lpToken) == address(zeusAddress) ? totalStakedAmount : pool.lpToken.balanceOf(address(this));
        ...
        return user.amount.mul(accTokenPerShare).div(1e12).sub(user.rewardDebt).add(user.pendingDebt);
    }
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                if(block.timestamp >= withdrawTime){
                    uint256 pendingTotal = pending.add(user.pendingDebt);
                    safeTokenTransfer(msg.sender, pendingTotal);
                    payReferralCommission(msg.sender, pendingTotal);
                    user.pendingDebt = 0;
                 }else{
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (_amount > 0) {
            ...
            require(block.timestamp >= withdrawTime, "time not arrived");
        }
        user.rewardDebt = user.amount.mul(pool.accTokenPerShare).div(1e12);
    ...
    function safeTokenTransfer(address _to, uint256 _amount) internal {
        uint256 tokenBalance = IBEP20(zeusAddress).balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > tokenBalance) {
            transferSuccess = IBEP20(zeusAddress).transfer(_to, tokenBalance);
        } else {
            transferSuccess = IBEP20(zeusAddress).transfer(_to, _amount);
        }
