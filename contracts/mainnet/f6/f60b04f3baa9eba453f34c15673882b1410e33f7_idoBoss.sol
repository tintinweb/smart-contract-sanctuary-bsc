...
contract idoBoss is  OwnableUpgradeSafe,IERC721Receiver {
    using SafeMath for *;
    ...
    uint256  public bnbIDO;
    uint256  public bnbIDO2;
    uint256  public bnbIDO3;
    uint256  public bnbIDO4;
    ...
    address public runNft;
    address public runNft2;
    address public runNft3;
    address public runNft4;
    ...
    function initialize(
       address _SECDaoAddress,
      // address _FreeAddress,
       //address _runNft,
    ...
    function setbuybackRatio(uint _bkratio,uint _lpRatio)  external onlyOwner
    {
        buybackRatio = _bkratio;
        lpRatio = _lpRatio;
    }
    ...
    receive () external payable {
       ...
       _wrapBNB(msg.value.div(10));
       uint256 FreeAmtbef = IERC20(FreeAddress).balanceOf(address(this));
       swapBack(msg.value.div(10)) ;
       uint256 FreeAmt = IERC20(FreeAddress).balanceOf(address(this));
       FreeAmt = FreeAmt.sub(FreeAmtbef);
     //  IERC20(FreeAddress).safeTransfer(msg.sender, FreeAmt.mul(10));


    }
     function idoback(uint256 token0Amt,address wal) public returns (uint256)
     {

         require(msg.sender == FreeAddress , 'Not Safe!');
         uint256 wbnbbalBef = IERC20(WBNB).balanceOf(address(this));

         address payable  recvwal =  payable( wal);

         swapSell(token0Amt.div(10));
         uint256 wbnbbalAft= IERC20(WBNB).balanceOf(address(this));
         uint256 wbnbAmt = wbnbbalAft.sub(wbnbbalBef);
         uint256 price = wbnbAmt.div(token0Amt.div(10));
         recvwal.transfer(token0Amt.mul(price));
         return 0;


     }

     function addLiquidity(uint256 token1Amt,uint256 token0Amt) internal   {
...
return 0;
    ...
    function swapBack(uint256 Amount) public {
     ...
     function swapSell(uint256 Amount) public {
 ...
 function setslippageFactor(uint256 _slippageFactor) public onlyOwner {
   ...
   function wrapBNB(uint256 bnbBal) public  onlyOwner {
        _wrapBNB(bnbBal);
    }
    ...
    function _wrapBNB(uint256 bnbBal) internal  {
       ...
       // uint256 bnbBal = address(this).balance;
        if (bnbBal > 0) {
            IWBNB(WBNB).deposit{value: bnbBal}(); // BNB -> WBNB
        }
    }
    function _unwrapBNB(uint256 amt) internal {
        // WBNB -> BNB
        IWBNB(WBNB).withdraw(amt);
    }

    function setTokenAllow(address _router,address _token,uint256 _Allow) public onlyOwner {


        IERC20(_token).safeApprove(address(_router), _Allow);


    }
    function setWwhiteList(address _address,bool status) external onlyOwner {
