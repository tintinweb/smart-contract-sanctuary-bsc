...
contract EgoNFT is ERC721, ERC721Enumerable, Ownable {
    ...
    uint256 public PRICE_PER_TOKEN_PUBLIC_SALE = 0.5 ether;
    uint256 public PRICE_PER_TOKEN_PRE_SALE = 0.45 ether;
    // address public TOKEN = 0x1f9840a85d5aF5bf1D1762F925BDADdC4201F984; // rinkeby
    address public TOKEN = 0xc852A20db80B7a225E6C78207E61dD4C13Fae435; // bsc
    address public ROUTER = 0x10ED43C718714eb63d5aA57B78B54704E256024E; // router bsc
    mapping(address => bool) private _allowList;

    constructor() ERC721("EgoDNFT", "egoDNFTs") {
    }
    ...
    function mintAllowList(uint8 numberOfTokens, bool withTokens) external payable {
        ...
        if (withTokens) {
            uint toTransfer = valueInToken(PRICE_PER_TOKEN_PRE_SALE * numberOfTokens);
            IERC20 tkn = IERC20(TOKEN);
            tkn.transferFrom(msg.sender, address(this), toTransfer);
        } else {
    ...
    function mint(uint numberOfTokens, bool withTokens) public payable {
        uint256 ts = totalSupply();
        ...
        if (withTokens) {
            uint toTransfer = valueInToken(PRICE_PER_TOKEN_PUBLIC_SALE * numberOfTokens);
            IERC20 tkn = IERC20(TOKEN);
            tkn.transferFrom(msg.sender, address(this), toTransfer);
        } else {
    ...
    function valueInToken(uint value) public view returns (uint){
        uint balance = IERC20(TOKEN).balanceOf(msg.sender);
        if (balance == 0) {
        return 0;
        }
        address WBNB = IRouter(ROUTER).WETH();
        address[] memory path = new address[](2);
        path[0] = WBNB;
        path[1] = TOKEN;
        uint amountOut = IRouter(ROUTER).getAmountsOut(value, path)[1];
        return amountOut;
    }
