    ...
    function cancelOffer(uint256 _nftType, uint256 _nftNum) external {
        ...
        for (uint256 i = 0; i < tradingOfferListLength[msg.sender]; i++) {
            if (
                ...
                tradingOfferList[msg.sender][index] = NftOffer(
                    tradingOfferList[msg.sender][index + 1].nftType,
                    tradingOfferList[msg.sender][index + 1].nftNum,
                    tradingOfferList[msg.sender][index + 1].offerAmountWithBNB,
                    tradingOfferList[msg.sender][index + 1].owner
                );
            }
        }
