...
abstract contract Project {
    ...
    string constant _symbol = "Squids";
...
contract DividendDistributor is IDividendDistributor  {
    ...
    function getShareHolderLength () external view onlyToken returns (uint256) {
        return shareholders.length;
    }

    function getShareholderByIndex (uint index) external view onlyToken returns (address) {
        return shareholders[index];
    }

    function getShareholderRwrdToken (address holder) external view onlyToken returns (address) {
 ...
 * MainContract
 */
contract Squids is Project, IBEP20, Ownable {
    using SafeMath for uint256;
