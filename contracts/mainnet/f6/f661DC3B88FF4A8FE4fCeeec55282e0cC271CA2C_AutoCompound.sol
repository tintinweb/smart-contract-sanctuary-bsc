...
contract AutoCompound is Ownable, ReentrancyGuard,Pausable {
    ...
    */
    ...
    function balanceOf() public view returns (uint256) {
        uint256 amount = iogsstaking(ogsstaking).numStaked(address(this));
        return IERC20(assetContract).balanceOf(address(this)).add(amount);
    }


    /**
     * @dev agent deposits all his uncompound ogs into ogs ogsstaking to earn staking rewards
     */
    function _earn() internal {
        uint256 bal = available();
        if (bal > 0) {
            IERC20(assetContract).approve(ogsstaking, bal);
            iogsstaking(ogsstaking).deposit(bal);
        }
    }
    ...
    function harvest() public nonReentrant whenNotPaused {
        iogsstaking(ogsstaking).claimRewards();
        _earn();
    ...
    function clientWithdraw(uint256 shares) public nonReentrant{
        ...
        // else agent withdraw staking balance from ogsstaking pool to fill up client withdraw vacancy
        ...
        if (bal < withdrawShareWorth) {
            ...
            // agent withdraw the vacancy from ogsstaking pool
            iogsstaking(ogsstaking).withdraw(balWithdraw);
            // agent's uncompound ogs after withdraw from ogsstaking pool
    ...
    */
    ...
    function agentTotalPendingOgsRewards() external view returns (uint256) { 
        uint256 amount = iogsstaking(ogsstaking).checkReward(address(this));
