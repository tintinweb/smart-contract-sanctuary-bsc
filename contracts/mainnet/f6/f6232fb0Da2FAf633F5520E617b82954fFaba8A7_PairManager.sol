...
contract PairManager is Initializable, IPairManager, Block, PairManagerStorage {
    ...
    modifier whenNotPaused(address trader) {
        require(owner == trader, "paused");
        _;
    }
...
//        );
    ...
    function forceCancelLimitOrder(uint128 pip, uint64 orderId) external onlyOwner returns (uint256 remainingSize, uint256 partialFilled) {
        return _internalCancelLimitOrder(pip, orderId);
    }
