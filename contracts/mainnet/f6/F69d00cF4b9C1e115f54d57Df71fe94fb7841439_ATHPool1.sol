...
contract ATHPool1 is Ownable {
    ...
    function availableClaimFirstAmount(uint _pid, address _user) public view returns(uint available) {
        ...
        if(timespend >= minmax[idoToken][0].lockTime) {
            available = isBuyer[_user][idoToken] * p.tgePercent / 100;
    ...
    function availableClaimAmount(uint _pid, address _user) public view returns(uint available) {
        ...
        if(!isATHpool1[_user]) available += athPool.availableClaimAmount(_pid, _user);
    }
