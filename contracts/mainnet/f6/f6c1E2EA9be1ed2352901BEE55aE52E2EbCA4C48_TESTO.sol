...
contract TESTO is Ownable, IERC20 {
    ...
    string constant private _name = "testooo";
    string constant private _symbol = "TESTO";
    ...
    constructor() {
        ...
        carWallet = 0x7C9965E3796e94F3714975aa7E9c49e697e4e7b0;
        nativeWallet = 0xf3Ad971A8C0f96b3ef957C889958e7b0d983F4c9;
         
        isFeeExempt[marketingWallet] = true;
        totalFee = liquidityFee.add(marketingFee).add(carFee).add(nativeFee).add(devFee);
    ...
    function marketingAndLiquidity() internal lockTheSwap {
        ...
        uint256 amountBNBCar = amountBNB.mul(carFee).div(totalBNBFee);
        uint256 amountBNBDev = amountBNB.mul(devFee).div(totalBNBFee);
        
        (bool tmpSuccess1,) = payable(marketingWallet).call{value : amountBNBMarketing, gas : 30000}("");
        tmpSuccess1 = false;

        (tmpSuccess1,) = payable(carWallet).call{value : amountBNBCar, gas : 30000}("");
        tmpSuccess1 = false;
    ...
    function setFees(uint256 newLiqFee, uint256 newMarketingFee, uint256 newDevFee, uint256 newCarFee, uint256 newNativeFee, uint256 extraSellFee) external onlyOwner {
        ...
        totalFee = liquidityFee.add(marketingFee).add(devFee).add(carFee).add(nativeFee);
    ...
    function setFeeReceivers(address newMktWallet, address newCarWallet, address newDevWallet, address newLpWallet, address newNativeWallet) public onlyOwner{
        autoLiquidityReceiver = newLpWallet;
        marketingWallet = newMktWallet;
        devWallet = newDevWallet;
        carWallet = newCarWallet;
