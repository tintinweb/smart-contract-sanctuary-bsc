...
contract BIGClaim is Ownable {
    using SafeMath for uint256;

    IERC20 public immutable BIG_TOKEN;
    string public constant CLAIM_BIG_SEPARATOR = "BIG_CLAIM";
    ...
    constructor(address _token) {
        ...
        BIG_TOKEN = IERC20(_token);
    }

    event ChangedAdminSigner(address _newSigner);
    event ClaimedBIG(uint256 _nonce, address _to, uint256 _amount);

    function setAdminSigner(address _newSigner) external onlyOwner {
        adminSigner = _newSigner;
        emit ChangedAdminSigner(_newSigner);
    }
    ...
    function claimBIG(
        ...
        require(block.timestamp <= _deadline, "NOT_RIGHT_TIME");
        ...
        BIG_TOKEN.transfer(_to, _amount.mul(10**18).div(conversionRate));

        emit ClaimedBIG(_nonce, _to, _amount);
    }
