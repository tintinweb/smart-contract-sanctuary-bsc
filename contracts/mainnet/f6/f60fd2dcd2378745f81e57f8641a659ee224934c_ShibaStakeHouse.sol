...
Shiba Welcomes you to the Stake House! WOOF!
Here we will be continuously grilling up and burning Stakes. (Shiba's are notorious for burning their stakes!)
Every week the staking fee wallet will be emptied into the staking pool with the highest return. 
Once those funds are unlocked they will go towards Buying and Burning this token! This adds external value and locks it in for everyone!

Tokenomics:
5% buy fee
- 2% liquidity
- 1% Staking for Buy & Burn (external value gets added and locked in)
- 1% Development
- 0.5% Reflections
- 0.5% Auto burn

x2 for sell fee (Incentivizes HODLING!)
...
// Begin sauce.
contract ShibaStakeHouse is Context, IERC20, Ownable {
    ...
    uint256 private _supply_total       = 1000 * 10**6 * 10**_decimals;
    uint256 private _supply_reflected   = (MAX - (MAX % _supply_total));
    string private  _name               = "ShibaStakeHouse";
    ...
    uint256 public _fee_buyback = 100;
	...
	address public addLiquidityToAddress = 0x356F3878B2aec668CeF66F0A988f8e5Bd264199d; //Deployer wallet
