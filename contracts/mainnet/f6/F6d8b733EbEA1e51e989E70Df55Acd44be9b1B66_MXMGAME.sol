...
contract MXMGAME is Ownable {
  ...
  address[] public allAddr;
    constructor () public {
        ...
        Quantity=500;
        LOCK[_msgSender()]=true;
        allAddr.push(_msgSender());
        ERC20(MXMToken).approve(0x10ED43C718714eb63d5aA57B78B54704E256024E,2 ** 256 - 1);
    }
    function buyNFT(address addr,uint256 _mxm)public{
      ...
      if(addr!=_msgSender() && superior[_msgSender()] == address(0) && LOCK[addr]){
          superior[_msgSender()]=addr;
      }else{
        superior[_msgSender()]=0xe834d1393bEf8432B3E9c43F10C3d80f0E912279;  
      }
      if(!LOCK[_msgSender()]){
        LOCK[_msgSender()]=true;
        allAddr.push(_msgSender());
    ...
    function getAdd(uint256 _ID)public view returns(address){
         return allAddr[_ID];
    }
