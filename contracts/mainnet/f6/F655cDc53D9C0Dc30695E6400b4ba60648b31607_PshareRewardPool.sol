...
contract PshareRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 accBPsharePerPshare; // Accumulated PSHAREs per pshare, times 1e18. See below.
    ...
    uint256 public PsharePerSecond = 0.00190258 ether; // 60000 pshare / (365 days * 24h * 60min * 60s)
    ...
    function pendingPshare(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accBPsharePerPshare = pool.accBPsharePerPshare;
        uint256 tokenSupply = pool.token.balanceOf(address(this));
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _pshareReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            accBPsharePerPshare = accBPsharePerPshare.add(_pshareReward.mul(1e18).div(tokenSupply));
        }
        return user.amount.mul(accBPsharePerPshare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _pshareReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            pool.accBPsharePerPshare = pool.accBPsharePerPshare.add(_pshareReward.mul(1e18).div(tokenSupply));
        }
        pool.lastRewardTime = block.timestamp;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 _pending = user.amount.mul(pool.accBPsharePerPshare).div(1e18).sub(user.rewardDebt);
            if (_pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accBPsharePerPshare).div(1e18);
