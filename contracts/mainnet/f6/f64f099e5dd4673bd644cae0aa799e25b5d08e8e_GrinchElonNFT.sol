...
contract GrinchElonNFT is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  constructor(string memory name, string memory symbol, string memory baseTokenURI) ERC721(name, symbol) {
      ...
      _wallet = 0xA9d3a7BC8F0E832A05485c7Dc40BD59941F00f4B;
  }
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    require(!paused, "GrinchElonNFT: minting has not started yet");
    require(_mintAmount > 0, "GrinchElonNFT: you have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "GrinchElonNFT: you can only mint 10 at a time");
    require(lastSupply >= _mintAmount,  "GrinchElonNFT: the collection is sold out");
    if (msg.sender != owner()) {
      require(msg.value >= cost * _mintAmount, "GrinchElonNFT: total cost isn't match");
    }
  ...
  function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
    ...
    require(_winner != address(0), "GrinchElonNFT: zero address");
    require(_amount > 0, "GrinchElonNFT: you have to mint at least one");
    require(lastSupply != 0,  "GrinchElonNFT: the collection is sold out");
  ...
  function tokenURI(uint256 _id) public view virtual override returns (string memory){
    require(_exists(_id),"GrinchElonNFT: URI query for nonexistent token");
