...
contract SPOOKYMARVIN is ERC20, Auth {
    ...
    string constant private _name = "Spooky Marvin";
    string constant private _symbol = "$SPOOKYMARVIN";
    ...
    uint256 private sellFeejackpot = 1;
    ...
    //totalFees
    uint256 private totalBuyFee = liquidityFee.add(marketingFee).add(jackpotFee).add(devFee).add(stakingFee);
    uint256 private totalSellFee = sellFeeLiquidity.add(sellFeeMarketing).add(sellFeejackpot).add(sellFeeDev).add(sellFeeStaking);

    uint256 private feeDenominator  = 100;

    address private autoLiquidityReceiver;
    address private marketingFeeReceiver = 0xF3f305D5Fe67Ba7679b1002AbA0158fF53aF2D48;
    address private jackpotFeeReceiver = 0x428d9d389958e778F13a59368d97Cc92b95504A4;
    address private stakingFeeReceiver = 0xF3f305D5Fe67Ba7679b1002AbA0158fF53aF2D48;
    address private devFeeReceiver = 0xaF00a740d4b8e3DF6465B651717103A8E0B95Ada;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBjackpot = amountBNB.sub(amountBNBLiquidity).sub(amountBNBMarketing).sub(amountBNBDev);
        ...
        (tmpSuccess,) = payable(jackpotFeeReceiver).call{value: amountBNBjackpot}("");
    ...
    function setBuyFees(uint256 _liquidityFee, uint256 _marketingFee, uint256 _jackpotFee, uint256 _devFee, uint256 _stakingFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        marketingFee = _marketingFee;
        jackpotFee = _jackpotFee;
        devFee = _devFee;
        stakingFee = _stakingFee;
        totalBuyFee = _liquidityFee.add(_marketingFee).add(_jackpotFee).add(_devFee).add(_stakingFee);
        feeDenominator = _feeDenominator;
        require(totalBuyFee <= maxFee, "Fees cannot be more than 30%");

        emit BuyFeesChanged(_liquidityFee, _marketingFee, _jackpotFee, _devFee, _stakingFee);
    }

    function setSellFees(uint256 _liquidityFee, uint256 _marketingFee, uint256 _jackpotFee, uint256 _devFee, uint256 _stakingFee, uint256 _feeDenominator) external authorized {
        sellFeeLiquidity = _liquidityFee;
        sellFeeMarketing = _marketingFee;
        sellFeejackpot = _jackpotFee;
        sellFeeDev = _devFee;
        sellFeeStaking = _stakingFee;
        totalSellFee = _liquidityFee.add(_marketingFee).add(_jackpotFee).add(_devFee).add(_stakingFee);
        feeDenominator = _feeDenominator;
        require(totalSellFee <= maxFee, "Fees cannot be more than 30%");

        emit SellFeesChanged(_liquidityFee, _marketingFee, _jackpotFee, _devFee, _stakingFee);
    }
    ...
    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _jackpotFeeReceiver, address _devFeeReceiver, address _stakingFeeReceiver) external authorized {
        require(_autoLiquidityReceiver != address(0) && _marketingFeeReceiver != address(0) && _jackpotFeeReceiver != address(0)  && _devFeeReceiver != address(0) && _stakingFeeReceiver != address(0), "Zero Address validation" );
        ...
        emit SetFeeReceivers(_autoLiquidityReceiver, _marketingFeeReceiver, _jackpotFeeReceiver,_devFeeReceiver, _stakingFeeReceiver);
    }
