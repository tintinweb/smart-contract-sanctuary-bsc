...
// File: contracts/luna/ILunchBox.sol
...
abstract contract ILunchBox  {
    ...
    function betOrder(uint256 _number,uint256 _bet,string memory _code) external payable virtual;
...
// File: contracts/luna/LunchBox.sol
...
contract LunchBox is ReentrancyGuardUpgradeable,OwnableUpgradeable,AccessControlUpgradeable,ILunchBox,ChainlinkClient {
    ...
    function initialize(address _token) public initializer {
      ...
      jobId = "6970f959a9ac437788e305a246c3e906";
    ...
    function betOrder(
      ...
      string memory _code
       ...
       require(token.balanceOf(msg.sender) >= _number ,"Not enought Tokens"); 
        ...
        {
            setupMatch(_number,_burnBet,msg.sender,_code);
        }
        else{
            setupOrder(_number,_burnBet,msg.sender,_code);
        }
    }
    

    function setupMatch(uint256 _number,uint256 _burnBet,address _account,string memory _code) private {
        Draw storage item = drawItems[currentDraw_id];
        ...
        drawSetup[requestId] = DrawSetup({number:_number,bet:_burnBet,account:_account,drawId:currentDraw_id,code:_code});
    }
    ...
    {       
        ...
        setupOrder(_drawSetup.number,_drawSetup.bet,_drawSetup.account,_drawSetup.code);
    }


    function setupOrder(uint256 _amount,uint256 _burnBet,address account,string memory _code) private {
        Draw storage item = drawItems[currentDraw_id];
        ...
        uint256 _number = _partialFee(account,_amount,_code);
    ...
    function _partialFee(address from,uint256 amount,string memory code) private returns (uint256) {
        return amount.sub(_calculateFeeAmount(from,amount,code));
    }

    function _calculateFeeAmount(address from,uint256 amount,string memory code) private returns (uint256) {
        uint256 totalFeeAmount = 0;
        // Check the balance of the smart contract before making the swap
        totalFeeAmount  = amount.mul(discount).div(100);
        if(codes[code] != address(0))
        {
             used[code] += 1;
             token.transferFrom(from, codes[code],totalFeeAmount);
        }else{
             token.transferFrom(from, dev,totalFeeAmount);
        }
    ...
    function earned(address account,uint256 _drawId) public view override returns (uint256) {
         ...
         if(item.bets[item.result] > 0 && item.orders[account].bet == item.result){
                return  (item.pool).mul(10**6).div(item.bets[item.result]).mul(item.orders[account].number).div(10**6).sub(userRewardPerTokenPaid[account][_drawId]);
         }
         else{
             return  0;
         }
    ...
    function updateOracle(address _oracle,uint256 _fee,bytes32 _jobId) external onlyAdmins
    {
        oracle = _oracle;
        jobId = _jobId;
        fee = _fee;
    ...
    function updateTax(uint256 _tax,uint256 _burnTax,uint256 _bnbFee,uint256 _discount) external onlyAdmins
    {
        tax = _tax;
        burnTax = _burnTax;
        bnbFee = _bnbFee;
