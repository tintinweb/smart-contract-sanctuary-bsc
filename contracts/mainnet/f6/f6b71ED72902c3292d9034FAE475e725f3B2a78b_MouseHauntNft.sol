...
// File contracts/nft/MouseHauntNft.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.9;

/**
 * @title Mouse Haunt NFT
  ...
  ERC721Pausable
{
  ...
  event NftMinted(address booster, address ghostOwner, uint256 nftId);

  /// @notice Event emitted when a special NFT is minted
  event SpecialNftMinted(string data, address ghostOwner, uint256 nftId);
  ...
  function unboxBooster(address boosterAddress, uint256 amount) public {
    ...
    // If the booster is an ERC20 token, amount must be 1
   ...
   */
  function mintNftByRarity(address to, address rarity) public onlyRole(MINTER_ROLE) {
    uint256 id = _tokenIdTracker.current();
    _doMint(to);

    emit NftMinted(rarity, to, id);
  }
   ...
   */
  function mintSpecialNft(address to, string calldata data) public onlyRole(MINTER_ROLE) {
    uint256 id = _tokenIdTracker.current();
    _doMint(to);

    emit SpecialNftMinted(data, to, id);
  }
  ...
  function burnFrom(address account, uint256 tokenId) public onlyRole(BURNER_ROLE) {
    require(ownerOf(tokenId) == account, "Account is not the token owner");
    _burn(tokenId);
  }
}