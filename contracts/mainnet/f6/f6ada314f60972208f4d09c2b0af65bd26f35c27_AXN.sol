...
contract AXN_Reward_Token is RewardPayingToken, Ownable {
    ...
    uint256 public rewardableAmount;
    bool public endCalculateReward = false;
    uint256 public lastProcessedIndexReward;
    ...
    //if(numClaimsAccount <= 365){newBalance = balanceOf(account);}
...
contract AXN is ERC20, Ownable, SafeToken, LockToken {
    ...
    uint256 public rewardRate = 3553496471; //with 12 decimals divider -> 0,3553496471%
...
//this will be used to exclude from rewards
    ...
    open(from, to) internal override 
    {
        ...
        try rewardTracker.setBalance((from), balanceOf(from)) {} catch {}
        try rewardTracker.setBalance((to), balanceOf(to)) {} catch {}
    ...
    function sendAndLiquify() internal  {
        
        uint256 rewards;
            (bool check, uint256 period) = SafeMath.trySub(block.timestamp,lastSentToContract);

        if(check && period >= rewardTracker.rewardInterval()){ //
    ...
    function buyToken () public payable returns (bool){//uint256 tokens
       ...
       address _holder = msg.sender;
        ...
        try rewardTracker.setBalance((_holder), balanceOf(_holder)) {} catch {}
    ...
    function claimAllToken () public returns (bool ){
        ...
        USDT.transfer(msg.sender, balance.sub(fee));
        USDT.transfer(projectWallet, fee);
        try rewardTracker.setBalance((_holder), 0) {} catch {}
    ...
    function claimToken (uint256 _claim) public returns (bool ){
        ...
        USDT.transfer(projectWallet, fee);
        try rewardTracker.setBalance((_holder), oldBalance.sub(_claim)) {} catch {}
