...
contract ANTIBOTS is Context {
     ...
     */
    modifier onlyOwner() {
        require(_owner == _msgSender(), "ANTIBOTS: caller is not the owner");
        _;
    }
     /**
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "ANTIBOTS: new owner is the zero address");
        emit OwnershipTransferred(_owner, newOwner);
        _owner = newOwner;
    }
}

contract FreedomInu is _IBEP20, ANTIBOTS {
    ...
    string _name = "Freedom Inu";
    string _symbol = "FREEINU";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 100_000_000 * (10 ** _decimals);
    uint256 public _maxWalletTX = 100_000_000 * (10 ** _decimals);   // 2% 
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (recipient != pair && recipient != DEAD && !isTxLimitExempt[recipient]) {
            require(balanceOf(recipient) + amount <= _maxWalletTX, "Max Wallet Exceeded");
        }
    ...
    function changeMaxWallet(uint256 percent, uint256 denominator) external onlyOwner {
        ...
        _maxWalletTX = _totalSupply.mul(percent).div(denominator);
    }
