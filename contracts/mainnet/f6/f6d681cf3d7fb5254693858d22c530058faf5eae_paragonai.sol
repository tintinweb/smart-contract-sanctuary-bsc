...
contract paragonai is Ownable, ReentrancyGuard{
    ...
    constructor(address _devWallet, address _withdrawWallet) {
        require(_devWallet!=address(0),"Please provide a valid dev wallet address");
        require(_withdrawWallet !=address(0),"Please provide a valid withdraw wallet address");
    ...
    function setWithdrawWallet(address _withdrawWallet) public onlyOwner {
        require(_withdrawWallet != address(0), "Please input a valid withdraw address");
        withdrawWallet = _withdrawWallet;
    }

    function setBusdAddress(address _busdAddress) public onlyOwner {
        require( _busdAddress != address(0), "Please input a valid BUSD Contract address");
        BUSDContract =  _busdAddress;
    }
    ...
    function deposit(uint256 _amount, address _referrer) external isLaunched {
        ...
        if(investors[msg.sender].referrer != address(0)){
           ...
           IERC20(BUSDContract).safeTransfer(investors[msg.sender].referrer, referrerAmount);
        }
    ...
    function withdraw(uint256 amount) external onlyOwner{
        ...
        require( amount <= getBalance(), "Contract Balance is low");
        IERC20(BUSDContract).safeTransfer( withdrawWallet , amount);
    }
}