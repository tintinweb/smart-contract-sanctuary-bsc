...
contract AfterStakeRegulator is IAfterStakeRegulator, AfterStakeUtils {
    using SafeMath for uint256;
    ...
    uint256 public priceMultiplier; // price peg control, AFB_PRICE = (AFBPoints_PRICE * priceMultiplier) / 1000
    uint256 public lastRewardBlock; // last block that rewards were received
    uint256 public buybackBalance; // total pending AFB awaiting stabilization
    uint256 public buybackRate; // rate of rewards stockpiled for stabilization
    uint256 public rewardsPerShare; // AFB rewards per AFBPoints, times 1e18 to prevent underflow
    uint256 public pendingRewards; // total pending AFB rewards
    ...
    function stabilize(uint256 amount) internal {
        if (isAbovePeg()) {
            // Above Peg: sell AFBPoints, buy AFB, add to rewards

            IERC20(AFBPoints).transfer(vault, amount);
            IAfterStakeVault(vault).buyAFBWithTokens(AFBPoints, amount);
        } else {
            // Below Peg: sell AFB, buy AFBPoints, burn all proceeds (deflationary)

            IAfterStakeVault(vault).buyPointsWithTokens(AFBToken, buybackBalance);
            IAFBPoints(AFBPoints).burn(amount);
            IAFBPoints(AFBPoints).overrideLoyaltyPoints(vault, 0);
            buybackBalance = 0;
        }
    }
    ...
    // Pool - Deposit AFB Points (AFBPoints) to earn AFB and stablize token prices
    ...
    function _deposit(address _user, uint256 _amount) internal {
        ...
        IERC20(AFBPoints).transferFrom(_user, address(this), _amount);
    ...
    function _withdraw(address _user, uint256 _amount) internal {
        ...
        IERC20(AFBPoints).transfer(_user, remainingUserAmount);
    ...
    function _migrate(address _user) internal {
        ...
        IERC20(AFBPoints).approve(migrator, balance);
        IAfterStakeMigrator(migrator).migrateTo(_user, AFBPoints, balance);
    ...
    function emergencyWithdraw() external NoReentrant(msg.sender) {
        ...
        IAFBPoints(AFBPoints).burn(feeAmount);
        safeTokenTransfer(msg.sender, AFBPoints, remainingUserAmount);
        emit EmergencyWithdraw(msg.sender, remainingUserAmount);
    }

    function isAbovePeg() public view returns (bool) {
        uint256 tokenPrice = IAfterStakeVault(vault).getTokenPrice(AFBToken, AFBTokenLp);
        uint256 pointsPrice = IAfterStakeVault(vault).getTokenPrice(AFBPoints, AFBPointsLp);
    ...
    // Governance - Set AFB Points price multiplier
