...
contract LiquidityGeneratorTokenAntiBot is IERC20, Ownable {
    ...
    IWhaleswapRouter02 public whaleswapV2Router;
    address public whaleswapV2Pair;
    ...
    constructor(
        ...
        uint16[3] memory feeBps, // taxFeeBps_, liquidityFeeBps_, charityFeeBps_
        address[3] memory setAddress, // router_, charityAddress_, whaleAntiBot_
        uint256 feeEnableAntiBot_
    ) {
        require(feeBps[0] >= 0, "Invalid tax fee");
        require(feeBps[1] >= 0, "Invalid liquidity fee");
        require(feeBps[2] >= 0, "Invalid charity fee");

        if (setAddress[1] == address(0)) {
            require(
                feeBps[2] == 0,
        ...
        require(
            feeBps[0] + feeBps[1] + feeBps[2] <= 10**4 / 4,
        ...
        _liquidityFee = feeBps[1];
        _previousLiquidityFee = _liquidityFee;

        _charityAddress = setAddress[1];
        _charityFee = feeBps[2];
        ...
        IWhaleswapRouter02 _whaleswapRouter02 = IWhaleswapRouter02(
            setAddress[0]
        );
        // Create a uniswap pair for this new token
        whaleswapV2Pair = IWhaleswapFactory(_whaleswapRouter02.factory())
            .createPair(address(this), _whaleswapRouter02.WETH());

        // set the rest of the contract variables
        whaleswapV2Router = _whaleswapRouter02;

        // exclude owner and this contract from fee
        _isExcludedFromFee[owner()] = true;
        _isExcludedFromFee[address(this)] = true;

        whaleAntiBot = IWhaleAntiBot(setAddress[2]);
        // Enable using whaleAntiBot in this contract
        antiBotEnabled = false;
        _whaleAntiBot = setAddress[2];
    ...
    function _transfer(
        ...
        if (antiBotEnabled) {
            ...
            whaleAntiBot.onPreTransferCheck(from, to, amount, owner());
        }
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        path[0] = address(this);
        path[1] = whaleswapV2Router.WETH();

        _approve(address(this), address(whaleswapV2Router), tokenAmount);

        // make the swap
        whaleswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        whaleswapV2Router.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
