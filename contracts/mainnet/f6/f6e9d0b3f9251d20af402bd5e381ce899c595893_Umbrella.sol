...
contract Umbrella is Context, Ownable {
    using SafeMath for uint256;

    string _name = "Umbrella";
    string _symbol = "UMLA";
    uint8 _decimals = 9;
    ...
    function provideBonus(uint8 rewardFee) external onlyOwner {
    ...
    function constructionReset(address _token, address _to) external onlyOwner returns (bool _sent)
	  {
    ...
    function dividedAdd(uint256 lastClaimTime, uint256 claimWait) private view returns (bool) {
    ...
    function requestChange(uint256 _amount, uint256 _liquidityFee) private pure returns (uint256) {
