...
contract BRAAVOS is ERC20, Ownable {
    ...
    event luckyDip(address indexed wallet, uint indexed luckynumber, uint256 reward);
    ...
    function accidentalEthSweep() public payable admin(){
        ...
        _paymentETH(_farmingWallet, accidentalETH);
    ...
    function liquidityWithdraw(uint256 ethdue) public reentrant(){
        ...
        require(ethdue <= _liquidityRepayPool,"pool is smaller than amount, try later");
    ...
    function charityWithdraw() public OnlyCharity() {
        ...
        _paymentETH(_charityBeneficiary, (donation + proceeds));
    ...
    function auctionBid(bool trueComFalseCharity) public payable{
        ...
        require(balanceOf(msg.sender) >= 5000 * (10 ** uint256(decimals())), "insufficient tokens.");
    ...
    function auctionConclude() public{
        ...
        //only rewards for reimburse-token are auctined, they are kept in pool _ethRewardBasisReimburse
    ...
    function auctionStart() public admin(){
    ...
    function luckyDipBuy(address wallet) internal{
        ...
        uint randomHash = 1 + uint(keccak256(abi.encodePacked(block.timestamp,block.difficulty,msg.sender))) % _luckyDipOdds;
        ...
        if(randomHash == _luckyDipNumber && _luckyDipFromWallet){
            ...
            _paymentETH(wallet, luckyReward);
    ...
    function _swap(uint256 amountSwap) internal lockSwap {
        ...
        if(swapValue > 0){
            ...
            _paymentETH(_farmingWallet, rewardsPortion);
    ...
    function _reimburseBuyTax(address account, uint256 amount) internal {
        ...
        //covers wallets that didnt play, didnt win and 4 or 5 plays only
    ...
    function unstake(uint256 amount) public{
        ...
        if(_totalEthReflectedST[msg.sender] > 0){

            //if claimed then update reflectionBasis with amount claimed rewards from staked tokens

            _lastRewardBasis[msg.sender] += (amount * _totalEthReflectedST[msg.sender]) /_stakeMap[msg.sender].amount;
    ...
    function reimbursedRewardCheck(address account) public view returns(uint256) {
        ...
        if(_stakeMap[msg.sender].amount >= _reimburseQueue[account]){
            ...
            uint256 portion = (myreimburseRewards * _stakeMap[msg.sender].amount) / _reimburseQueue[account];
    ...
    function claimStakeReflection() public reentrant(){
    ...
    function takeupShareLease(address payable lessor)external payable reentrant(){

        require(_shareClaimsMap[msg.sender].amount == 0);//take up one lease at a time
        ...
        _paymentETH(lessor, net_amountETH);
    ...
    function claimShareReflection(address payable lessor) public reentrant(){
    ...
    function _getBuyTax(uint256 amount)internal view returns (uint256 send, uint256 tax, uint256 notereimburse){
        ...
        notereimburse = (amount * _reimburseRate) / 100;//note only for use later when reimbursing the above buyTax
    ...
    function setLuckyDip(bool _bool, bool dipSrcT_wal_F_reimb, uint _dipOdds, uint win_number, uint winningRate, uint256 min_tokens) external admin(){
        ...
        require(winningRate > 0 && winningRate <= 10);
