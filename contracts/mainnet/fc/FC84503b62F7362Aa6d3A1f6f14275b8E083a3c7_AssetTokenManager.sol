...
contract AssetMinter is Ownable, Pausable, ReentrancyGuard {
    ...
    function fulfillFee() public view returns (uint256) {
        if (fee == address(0)) {
            return 0;
        }
        return IFee(fee).fulfill();
    }
    ...
    function fulfill(uint256 id, uint256 pricePerUnit) external onlyRelayer {
        Order storage order = orders[id];
        require(order.status == OrderStatus.PENDING, "!pending");
        require(pricePerUnit <= order.pricePerUnit, "!price");
        order.status = OrderStatus.FULFILLED;

        uint256 useAmount = (order.assetAmount * pricePerUnit) / 1 ether;
        uint256 refundAmount = order.inputAmount - useAmount;
        order.refundAmount = refundAmount;

        IERC20(inputToken).safeTransfer(vault, useAmount);
    ...
    function reject(uint256 id) external onlyRelayer {
        ...
        order.refundAmount = order.inputAmount + order.feeAmount;
...
interface IFee {
    ...
    function fulfill() external view returns (uint256);
}
...
contract AssetRedeemer is Ownable, Pausable, ReentrancyGuard {
    using SafeERC20 for IERC20;
    ...
    function redeem(
        ...
        uint256 outputAmount = (assetAmount * pricePerUnit) / 1 ether;
    ...
    function fulfill(
        ...
        uint256 outputAmount = ((order.assetAmount * pricePerUnit) / 1 ether) -
            fee;
        IERC20(order.asset).approve(address(manager), order.assetAmount);
        manager.burn(order.asset, order.assetAmount);
        IERC20(outputToken).safeTransferFrom(
            vault,
            address(this),
            outputAmount
        );
        IERC20(outputToken).safeTransfer(order.sender, outputAmount);
...
// AssetTokenManager is the owner of all assets
contract AssetTokenManager is Ownable, Pausable {
    ...
    event Deploy(address token, string name, string symbol);
    ...
    function deployAssetToken(string memory name, string memory symbol)
        external
        onlyOwner
    {
        AssetToken assetToken = new AssetToken(name, symbol);
        emit Deploy(address(assetToken), name, symbol);
    }
...
contract Fee is Ownable {
    uint256 private fee;
    uint256 public constant FEE_DIVIDER = 10_000; // fee 100% = 10,000
    uint256 public constant MAX_FEE = 100; // 1%
    uint256 public fulfill; // wei

    event SetFee(address sender, uint256 fee);
    event SetFulfill(address sender, uint256 fee);

    function setFee(uint256 _fee) external onlyOwner {
        require(_fee <= MAX_FEE, "exceed max fee");
        fee = _fee;
        emit SetFee(msg.sender, fee);
    }

    function setFulfill(uint256 _fee) external onlyOwner {
        fulfill = _fee;
        emit SetFulfill(msg.sender, fulfill);
    }
