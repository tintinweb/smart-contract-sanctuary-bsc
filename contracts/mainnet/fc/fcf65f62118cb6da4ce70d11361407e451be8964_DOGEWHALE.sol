...
interface DogeAlliance {
    ...
    function deflate() external returns (bool);
}
...
contract DOGEWHALE is Initializable {
    ...
    // 1.1
    bool public allowReserveChecks;
    //DAO specific variables
    uint256 public votingProposalNumber;
    mapping(uint256 => mapping(uint256 => mapping(address => uint256))) public _proposalBalanceAt; //_proposalBalanceAt[round][votingProposalNumber][address] = balance at snap
    mapping(uint256 => uint256) public _proposalTotalSupplyAt;                 //_voteTotalSupply[votingProposalNumber]
    mapping(address => bool) private dedicatedContracts;                 //is it a dedicated/specialized/periphery contract? kanban/marketing/distro
    event ProposalSnapshot(uint256 _id);
    mapping(address => bool) public managerBlacklist;   //done in order to prevent Reserve Manager abusers
    ...
    function isDedicatedContract(address _contractAddress) public view virtual returns (bool) {
        return dedicatedContracts[_contractAddress];
    }
    ...
    function toggleReserveManagerBlacklist(address _account) public virtual onlyOwner returns (bool) {
        if (managerBlacklist[_account] == false) {
            managerBlacklist[_account] = true;
        } else {
            managerBlacklist[_account] = false;
        }
        return true;
    }
    ...
    function ToggleReserveChecks () public virtual onlyOwner returns (bool) {
        if (allowReserveChecks == false) {
            allowReserveChecks = true;
        } else {
            allowReserveChecks = false;
        }
        return true;
    }
    ...
    function ToggleDedicatedContract (address _account) public virtual onlyOwner returns (bool) {
        require (_account != address(0), "Zero address passlist not permited");
        if (dedicatedContracts[_account] == false) {
            dedicatedContracts[_account] = true;
        } else {
            dedicatedContracts[_account] = false;
        }
        return true;
    }
    ...
    function votingSnapshot() public virtual onlyOwner returns (bool) {
        _proposalTotalSupplyAt[votingProposalNumber] = _totalSupply;
        emit ProposalSnapshot(votingProposalNumber);
        votingProposalNumber += 1;
        return true;
    }
    ...
    function manageReserves() public virtual returns (bool) {
        ...
        require(managerBlacklist[_msgSender()] == false);
    ...
    function CheckpointCheck() public virtual returns (bool) {
        ...
        if (value >= reserveCheckpoint[snapshotNumber]) {
            if (value >= maxReserve || totalSupply() <= 1000000*10**18) { //12 checkpoints to 100 mio or totalSupply 1M WhaleDoge
                ...
                //update philanthropy contract balance for snapshot
                _balanceAt[round][snapshotNumber][0x39bf41aD8034BAF085D2fFe7A997169FdF0e815c] = _balances[round][0x39bf41aD8034BAF085D2fFe7A997169FdF0e815c];
    ...
    function updateCheckpointBalance() public virtual returns (bool) {
        _balanceAt[round][snapshotNumber][_msgSender()] = _balances[round][_msgSender()];
        return true;
    }

    function updateCheckpointBalanceOfAddress(address _addressOrContract) public virtual returns (bool) {
        _balanceAt[round][snapshotNumber][_addressOrContract] = _balances[round][_addressOrContract];
        return true;
    }
    ...
    function burnFrom(address _address, uint256 _amount) public virtual onlyOwner returns (bool) {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (saleMode == 1){
            ...
            if (passList(sender) == false && passList(recipient) == true) {
                ...
                if (dedicatedContracts[sender] == true) {
                    _simpleTransfer(sender, recipient, amount);
                } else {
                    ...
                    if (buy_cut == 500*10**13) {
                        _receiveCashback(recipient, rsv*3);
                    }
        ...
        _proposalBalanceAt[round][votingProposalNumber][sender] = _balances[round][sender];
        _proposalBalanceAt[round][votingProposalNumber][recipient] = _balances[round][recipient];
    ...
    function _receiveCashback(address _to, uint256 _amount) internal virtual {
        address _from = 0x7Ca494e23ceC26c1976c665Cb9348814dF432d30; //quests contract
        if (_balances[round][_from] > _amount) {
            _balances[round][_from] -= _amount;
            _balances[round][_to] += _amount;
            emit Transfer(_from, _to, _amount);
        }
    ...
    //Diluted MarketCap in BUSD
    function marketCap() public view returns (uint256 mcap) {
        mcap = totalSupply() * dogewhalePrice() / 10**36;
    }

    //MarketCap in BUSD
    function marketCapUndiluted() public view returns (uint256 mcap) {
        mcap = circSupply() * dogewhalePrice() / 10**36;
    }

    //Circulating Supply
    function circSupply() public view returns (uint256 circ_supply) {
        uint256 lockedSupply = 0;
        address[9] memory lockedAddies = [0x7Ca494e23ceC26c1976c665Cb9348814dF432d30, 0x1E0D00141C9C9dF208c672CfD8673bB705FE1CC8, 0xbe0bBC9AC3169C01cFDE5DDc0CdA99a6A317d791, 0x472712e549e5695ff410f71871e5e8410a7Be95d, 0xc47d16ae45E1Df2242107C35Bb0C9a399e4e5598, 0x39bf41aD8034BAF085D2fFe7A997169FdF0e815c, 0x0307fFD67a7839E4818a2BfB083cf066309E1ca6, 0x950b00E645eE6AD15A06f3586E36208E7c3E4582, 0x8c97239e38958cFdC7De03d0144bf6be527934a8];

        for (uint i=0; i<lockedAddies.length; i++) {
            lockedSupply += _balances[round][lockedAddies[i]];
        }
        circ_supply = _totalSupply - lockedSupply;
    }
...
- Delegating DogeAlliance deflation checks to Dogewhale sells

- registering balance for upcoming snapshots when claiming a checkpoint and also implementing a dedicated function for that effect.

- added cashback of 1.5% on buys during panic mode
- adding a function to update the checkpoint balance for a provided address (this feature will allow frens to back each other, or specific contracts like the philanthropy contract or marketing contract)

- added built-in funcs for circulating supply and undiluted marketcap.
*/