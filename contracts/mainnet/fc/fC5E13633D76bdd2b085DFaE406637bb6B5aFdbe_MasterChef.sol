...
contract MetaswapstarToken is Context, IERC20, Ownable {
    ...
    function setMasterChef(address _masterChefAddress) public onlyAdmin {
        masterChefAddress = _masterChefAddress;
    }
    ...
    function _transfer(
        ...
        require(!_isBlack[from], "cant trade, your account is banned");
        require(!_isBlack[to], "cant trade, receive account is banned");
    ...
    function setBlack(address account) public onlyAdmin {
        require(!_isBlack[account], "Account is already black");
        _isBlack[account] = true;
    }

    function removeBlack(address account) public onlyAdmin {
        require(_isBlack[account], "Account is not black");     
...
contract MasterChef is Ownable ,ReentrancyGuard{
    ...
    function updatePool(uint256 _pid) public PoolValidation(_pid) {
        ...
        metaswapstar.mintMasterChef(address(this), metaswapstarReward);
