...
// File contracts/lod-721-lazy-minter-v2.sol
    ...
    UUPSUpgradeable
{
  ...
  function redeem(address redeemer, NFTVoucher calldata voucher) public payable returns (uint256) {
    ...
    require(hasRole(MINTER_ROLE, signer), "NFT:Signature invalid or unauthorized");

    // make sure that the redeemer is paying enough to cover the buyer's cost
    require(msg.value >= voucher.minPrice, "NFT:Insufficient funds to redeem");
  ...
  function withdraw() public {
    require(hasRole(MINTER_ROLE, msg.sender), "NFT:Only authorized minters can withdraw");
  ...
  function batchMintLOD(address to, uint offset, uint amount) external {
      ...
      for(uint i = offset; i < offset + amount; i++){
          super._mint(to, i);
      }
  }

  function batchTransferLOD(address to, uint offset, uint amount) external {
      require(hasRole(MINTER_ROLE, _msgSender()), "NFT: must have minter role");
      require(to != address(0), "NFT: zero address");
      require(amount > 0 && amount < 0xFF , "NFT: invalid amount");
      for(uint i = offset; i < offset + amount; i++){
  ...
  // reset to it's own id base uri
  function batchResetTokenUri(uint offset, uint amount) external {
      require(hasRole(MINTER_ROLE, _msgSender()), "NFT: must have minter role");
      require(amount > 0 && amount < 0xFF , "NFT: invalid amount");
      for(uint i = offset; i < offset + amount; i++){
  ...
  function lockLOD(uint tokenId) public {
    ...
    require(hasRole(MINTER_ROLE, _msgSender()), "NFT: must have minter role to lock");
    require(!lockedTokens[tokenId], "NFT:already locked");
    lockedTokens[tokenId] = true;
  }

  function unlockLOD(uint tokenId) public {
    require(hasRole(MINTER_ROLE, _msgSender()), "NFT: must have minter role to unlock");
  ...
  function resetURI(uint tokenId, string memory tokenURI_) public {
    require(hasRole(MINTER_ROLE, _msgSender()), "NFT: must have minter role to reset uri");
    _setTokenURI(tokenId, tokenURI_);
  }
