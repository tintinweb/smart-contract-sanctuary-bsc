...
contract DividendDistributor is IDividendDistributor {
    ...
    modifier onlyToken() {
        require(msg.sender == _token, "Caller is not authorized");
        _;
    }
...
contract RhesusProtocol is IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string constant _name = "Rhesus Protocol";
    string constant _symbol = "RhP";
    ...
    // fees = should be 100 in taotal
    uint256 public marketingFee = 30;
    uint256 public developerFee = 20;
    uint256 public rewardFee = 30;
    ...
    address public marketingFeeReceiver = 0x708F57982B77871b085C7c02Ffd9947d20dc82aE;
    address public devFeeReceiver = 0x8E0fd07cDD3A45ab447A8C83079eB6dCbbD5b8eD;
    ...
    function changeSwapToken(address token) external onlyOwner {
        require(token != address(0), "Token passed is zero address.");
        require(token != address(this), "Token can not be contract address.");
        SWAPTOKEN = token;
    }

    function updateTaxesFess(uint256 _buyFee, uint256 _sellFee, uint256 _transferFee) external onlyOwner {
        buyFee = _buyFee;
        sellFee = _sellFee;
        transferFee = _transferFee;
        require(_buyFee <= 20 && _sellFee <= 20 && _transferFee <= 20, "Total taxes exceed maximum");
    }
    ...
    function updateFees(
        ...
        uint256 totalFee = marketingFee.add(developerFee).add(rewardFee).add(liquidityFee).add(burnFee);
        require(totalFee <= 100, "Total fee should be 100");
    }
    ...
    function includeMeinRewards() public {
        require(!isDividendExempt[msg.sender],"You are not allowed to get rewards");
        try
    ...
    function switchToken(address rewardToken, bool isIncludeHolders) public onlyOwner {
        require(rewardToken != address(0), "Reward token is a zero address");
        require(rewardToken != WBNB, "Can not reward BNB in this tracker");
        require(rewardToken != address(this), "Can not reward itself");
        REWARD = rewardToken;
        // get current shareholders list
        address[] memory currentHolders = dividendDistributor.getShareHoldersList();
    ...
    function claimRewards(bool tryAll) public {
    ...
    function swapBackInBnb() internal swapping {
        ...
        uint256 tokensToLiquidity = liquidityFee != 0 ? contractTokenBalance.mul(liquidityFee).div(100) : 0;
        uint256 tokensToReward = rewardFee != 0 ? contractTokenBalance.mul(rewardFee).div(100) : 0;
        uint256 tokensToBurn = burnFee != 0 ? contractTokenBalance.mul(burnFee).div(100) : 0;
        // calculate tokens amount to swap
        uint256 tokensToSwap = contractTokenBalance.sub(tokensToLiquidity).sub(tokensToReward).sub(tokensToBurn);
        // swap the tokens
        if(tokensToSwap > 0) {
        ...
        uint256 totalSwapFee = marketingFee.add(developerFee);
        uint256 marketingFeeBnb = totalSwapFee != 0 ? swappedBnbAmount.mul(marketingFee).div(totalSwapFee) : 0;
    ...
    function swapBackInTokens() internal swapping {
        uint256 contractTokenBalance = _balances[address(this)];
        uint256 rewardTokens = rewardFee != 0 ? contractTokenBalance.mul(rewardFee).div(100) : 0;
        uint256 liquidityTokens = liquidityFee != 0 ? contractTokenBalance.mul(liquidityFee).div(100) : 0;
        uint256 tokensToBurn = burnFee != 0 ? contractTokenBalance.mul(burnFee).div(100) : 0;
        uint256 tokensForFee = contractTokenBalance.sub(rewardTokens).sub(liquidityTokens).sub(tokensToBurn);
    ...
    function swapAndSendFees(uint256 tokensForFee) private {
        ...
        uint256 marketingToken = totalSwapFee != 0 ? currentTokenBalance.mul(marketingFee).div(totalSwapFee) : 0;
    ...
    function setIsDividendExempt(address holder, bool exempt) external onlyOwner {
        require(holder != address(0), "Holder address is zero address.");
        require(holder != address(this) && holder != pair, "Holder address is either contract address or pair address");
    ...
    function setIsFeeExempt(address holder, bool exempt) external onlyOwner {
        require(holder != address(0), "Holder address passed is zero address.");
        isFeeExempt[holder] = exempt;
    }

    function setFeeReceivers(address _marketingFeeReceiver, address _devFeeReceiver) external onlyOwner {
        require(_marketingFeeReceiver != address(0), "zero address passed");
        require(_devFeeReceiver != address(0), "zero address passed");
