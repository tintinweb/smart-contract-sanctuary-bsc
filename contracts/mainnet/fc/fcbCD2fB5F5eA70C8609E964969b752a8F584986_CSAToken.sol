...
contract CSAToken is ERC20, Ownable {
    ...
    exchequerLike public vault = exchequerLike(0xbA2b8EE3D93353e665660772217F1839642FEf26);
    ...
    uint256 public minHolder = 1000 * 1E18;
    ...
    function _transfer(address from,address to,uint256 amount) internal override {
        ...
        if(canSwap && !swapping && !automatedMarketMakerPairs[from] && from != owner() && to != owner()) {
            swapping = true;
            uint256 burnTokens = contractTokenBalance.mul(5).div(65);
            super._transfer(address(this),deadWallet,burnTokens);

            uint256 awardTokens = contractTokenBalance.mul(40).div(65);
            super._transfer(address(this),address(vault),awardTokens);
        ...
        if(takeFee) {
        	uint256 fees = amount.mul(70).div(1000);
            super._transfer(from, address(this), fees);
            if (tier) {
                uint256 referralBonuses = amount.mul(5).div(1000);
                address dst;
                if(automatedMarketMakerPairs[from]) dst = to;
                else dst = from;
                address _referrer = Inviter.inviter(dst);
                if(automatedMarketMakerPairs[_referrer] || balanceOf(_referrer) < minHolder) _referrer = operationAddress;
