...
contract StakingPool is Ownable, ReentrancyGuard {
  ...
  function withdraw(uint256 amount) external nonReentrant {
    ...
    if (amount + rewards > balance) {
      uint256 ownerAmount = balance.mul(ownerfee).div(10000);

      BUSDToken.transfer(owneraddr, ownerAmount);
      BUSDToken.transfer(msg.sender, (balance - ownerAmount));
    } else {
      uint256 ownerAmount = (amount + rewards).mul(ownerfee).div(10000);

      BUSDToken.transfer(owneraddr, ownerAmount);
      BUSDToken.transfer(msg.sender, (amount + rewards - ownerAmount));
    }
  }
  ...
  function withdrawAll() external nonReentrant {
    ...
    if (stakeItem.amount + rewards > balance) {
      ...
      uint256 ownerAmount = (stakeItem.amount + rewards).mul(ownerfee).div(10000);

      BUSDToken.transfer(owneraddr, ownerAmount);
      BUSDToken.transfer(msg.sender, (stakeItem.amount + rewards - ownerAmount));
    }
