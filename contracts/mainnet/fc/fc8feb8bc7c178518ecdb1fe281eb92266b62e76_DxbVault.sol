...
interface IMasterChef {
    ...
    function pendingDxb(uint256 _pid, address _user) external view returns (uint256);
...
contract DxbVault is Ownable, Pausable {
    ...
    function calculateHarvestCakeRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingDxb(0, address(this));
