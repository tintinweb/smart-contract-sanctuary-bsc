...
contract NACHOCHEESE is BEP20Detailed, Auth {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(bigBuyComp){bigBuyerCheck(sender, amount);}
    ...
    function bigBuyerCheck(address sender, uint256 amount) internal view {
        uint256 cAmount = amount.mul(_PerFragment);
        if(sender == biggestBuyer){
            require(cAmount <= _balances[sender].sub(biggestBuy.mul(_PerFragment)), "Big Buy Competition in process");}
    }
