...
contract BVBridge is ERC721HolderUpgradeable, ERC1155HolderUpgradeable, OwnableUpgradeable, DivestorUpgradeable {
    ...
    event AcrossERC20(address indexed account, address indexed token, bool indexed isNew, uint256 amount, uint256 targetChainId);
    event AcrossERC721(address indexed account, address indexed token, uint256[] cardIds, uint256[] tokenIds, uint256 targetChainId);
    ...
    function initialize(
        ...
        tokenInfo[address(0)] = Token(true,0,20,0,address(0));
    }
    ...
    function acrossERC20(
        ...
        emit AcrossERC20(msg.sender, token_, isNew_, amount_, chainId);
    }

    function acrossERC721(address token_, uint256[] calldata tids_, uint chainId) public isOpen {
        Token memory tInfo = tokenInfo[token_];
        require(tInfo.isOpen, "not open");
        ...
        emit AcrossERC721(msg.sender, token_, cids_, tids_, chainId);
    }


    function setRate(uint rate_) public onlyOwner {
        rate = rate_;
    }
    ...
    function sendERC20(
        ...
        if (token_ == address(0)) {
            ...
            emit SendERC20(id_, account_, token_, amount_);
            return;
        }


        if (token_ == 0x29AB77a43Ec69D0DFFf3Ff72bff457e09b258351) {
            amount_ = amount_ / rate;
        }
