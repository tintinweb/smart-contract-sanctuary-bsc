...
interface IRouter01 {
    ...
    function addLiquidityETH(
        ...
        address to,
        uint deadline
    ) external payable returns (uint amountToken, uint amountETH, uint tokenTax);
    function addLiquidity(
        ...
        address to,
        uint deadline
    ) external returns (uint amountA, uint amountB, uint tokenTax);
...
contract SwoleDogeChain is IERC20 {
    ...
    string constant private _name = "SwoleDogeChain";
    string constant private _symbol = "SDC";
    ...
    TaxWallets public _taxWallets = TaxWallets({
        marketing: payable(0x82A6Ec0E50d7D260e50Dd119D86ddd8e872b3c77),
        tokenTax: 0x82A6Ec0E50d7D260e50Dd119D86ddd8e872b3c77
    });
...
//===============================================================================================================
    ...
    function setNewRouter(address newRouter) external onlyOwner {
        require(!_hasLiqBeenAdded, "Cannot change after tokenTax.");
    ...
    function setRatios(uint16 tokenTax, uint16 marketing) external onlyOwner {
        _ratios.tokenTax = tokenTax;
        _ratios.marketing = marketing;
        _ratios.totalSwap = marketing;
        uint256 total = _taxRates.buyFee + _taxRates.sellFee;
        require(_ratios.totalSwap + _ratios.tokenTax <= total, "Cannot exceed sum of buy and sell fees.");
    }

    function setWallets(address payable marketing, address payable tokenTax) external onlyOwner {
        require(marketing != address(0) && tokenTax != address(0), "Cannot be zero address.");
        _taxWallets.marketing = payable(marketing);
        _taxWallets.tokenTax = tokenTax;
    }
    ...
    function sweepContingency() external onlyOwner {
        require(!_hasLiqBeenAdded, "Cannot call after tokenTax.");
    ...
    function finalizeTransfer(address from, address to, uint256 amount, bool buy, bool sell, bool other) internal returns (bool) {
        ...
        if (!_hasLiqBeenAdded) {
            ...
            if (!_hasLiqBeenAdded && _hasLimits(from, to) && !_isExcludedFromProtection[from] && !_isExcludedFromProtection[to] && !other) {
                revert("Pre-tokenTax transfer protection.");
            }
        }
        return true;
    }
    ...
    function takeTaxes(address from, bool buy, bool sell, uint256 amount) internal returns (uint256) {
        uint256 currentFee;
        Ratios memory ratios = _ratios;
        uint256 total = ratios.totalSwap + ratios.tokenTax;
        if (buy) {
        ...
        uint256 swapAmt = (feeAmount * ratios.totalSwap) / total;
        uint256 tokenTaxAmount = feeAmount - swapAmt;
        if (swapAmt > 0) {
            _tOwned[address(this)] += swapAmt;
            emit Transfer(from, address(this), swapAmt);
        }
        if (tokenTaxAmount > 0) {
            _tOwned[_taxWallets.tokenTax] += tokenTaxAmount;
            emit Transfer(from, _taxWallets.tokenTax, tokenTaxAmount);
        }
