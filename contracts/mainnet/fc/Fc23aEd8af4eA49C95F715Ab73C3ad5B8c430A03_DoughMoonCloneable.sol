...
contract DoughMoonCloneable is DoughTokenCloneable  {
    ...
    /**
      * @dev Overriden from DoughTokenCloneable, allowing DoughMoonCloneable specific initialisation
      *
      * NOTE: This function is only called when the clone is initialised
      *
      * @param _tokenInfo A struct containing general information about the token
    ...
    function __DoughToken_init (SharedStructures.TokenInfo memory _tokenInfo) internal override { 
        require (_tokenInfo.decimals == 9, createErrorMessage ("Moon tokens need 9 decimals"));
        require (otherFeeInfo.length == 1, createErrorMessage ("1 otherFee needed for reflection"));

        __Versioned_init ("0.1.5");
        
        // Set total supply and reflective total supply
        uint256 _totalSupply = _tokenInfo.totalSupplyWithoutDecimals * 10**_tokenInfo.decimals;
        ...
        // Record balance of msg.sender
        uint256 balance =  ERC20Upgradeable.balanceOf (msg.sender);
        tBalances[msg.sender] = balance;
        rBalances[msg.sender] = balance * currentRate;
        
        // Record balance of burn address
        balance = ERC20Upgradeable.balanceOf (BURN_ADDRESS);
        tBalances[BURN_ADDRESS] = balance;
        rBalances[BURN_ADDRESS] = balance * currentRate;

        // Exclude the swap pair from reflections (stops skimming)
    ...
    /**
      * @dev Get the current number of tokens in "normal" token space given a reflected balance
      * 
      * @param rAmount A reflected token amount
      * @return The "normal" token amount (rAmount / getRate (current total supply))
      */
    ...
    /**
      * @dev Get the current conversion rate from reflected token balance to "normal" token balance
      * 
      * @param _tTotal The current total supply
      * @return A multiplier used to convert a "normal" token balance to a reflected token balance
      */
    function getRate (uint256 _tTotal) private view returns (uint256) {
        uint256 rSupply = rTotal;
        uint256 _rTotal = rTotal;
        uint256 tSupply = _tTotal;
        uint256 length = excluded.length;
    ...
    /**
      * @dev Allows an address to be excluded from receiving reflections. Will most likely be used to avoid centralised exchanges or farming contracts
    ...
    /**
      * @dev Allows an excluded account to be included in receiving reflections again
      *
      * NOTE: This function can only be called by the contract owner
      * 
    ...
    /**
      * @dev Gets the balance of account overriding the ERC20 standard to allow it to deal with reflections
      * 
      * @param account The address to return the balance of
      * @return The balance of he account, taking into account the number of reflections they have earned (if they are eligible)
      */
    ...
    /**
      ...
      * @param transferToAmount The amount of tokens transferred to the recipient (includes any fee impact)
      */
    function postTransferActions (address from, address to, uint256 transferFromAmount, uint256 transferToAmount, uint8 feeType) internal override {
        uint256 currentRate = getRate (totalSupply());

        // Sort out fee tokens if they will have been taken
        if (!isExcludedFromFees[from] && !isExcludedFromFees[to]) {
            if (feeType == EOW_TRANSFER) {
                receiveTokens (address(this), (transferFromAmount * feeInfo[BUY].platformFeePermille) / 1000, currentRate);
            } else {
                uint256 _marketingTokens = (transferFromAmount * feeInfo[feeType].marketingFee) / 100;
                uint256 _contractTokens = (transferFromAmount * ((feeInfo[feeType].liquidityFee * 10) + feeInfo[feeType].platformFeePermille)) / 1000;
                uint256 _burnTokens = (transferFromAmount * feeInfo[feeType].burnFee) / 100;
                
                if (feeInfo[feeType].marketingFundsInTokens)
                    receiveTokens (addressInfo.marketingWallet, _marketingTokens, currentRate);
                else
                    _contractTokens += _marketingTokens;

                receiveTokens (BURN_ADDRESS, _burnTokens, currentRate);
                receiveTokens (address(this), _contractTokens, currentRate);
            }
        }

        // Deal with sender and recipient
        if (isExcluded[from])
            tBalances[from] -= transferFromAmount;

        rBalances[from] -= transferFromAmount * currentRate;

        receiveTokens (to, transferToAmount, currentRate);
    ...
    /**
      * @dev Increments the recipients wallet holdings
      * 
      * @param wallet The address of the recipient
      * @param amount The amount of tokens transferred to increment
      * @param currentRate The rate to use for conversion between real and reflective space
      */
    function receiveTokens (address wallet, uint256 amount, uint256 currentRate) private {
        if (isExcluded[wallet])
            tBalances[wallet] += amount;

        rBalances[wallet] += amount * currentRate;
    }
}