...
contract PTG is Context, IERC20, Ownable {
	...
	function _getTValues(address sender, address recipient, uint256 tAmount) private view returns (uint256, uint256) {
		...
		// sell
		if (recipient == uniswapV2Pair || recipient == uniswapV2Pair2 || recipient == address(this)) {
	...
	private lockTheSwap {
		if (amount > 0) {
			...
			//_sellToBurn(address(this), amount);
			// swap token to husd
            ...
            /** 
			uint256 shareUNum = address(this).balance;
	...
	function _getSellFeeList(uint256 uAmount2) private view returns (uint256, uint256, uint256){
		...
		uint256 shareFee = leftAmount.div(100).mul(_sellInviteFee).div(100);
		...
		if (amount != 0){
			for (int256 i = 0; i < 13; i++) {
				...
				if (IERC20(swapToken).balanceOf(cur1) >= _minSwapCoin && cur1 != address(0)){
					IERC20(husdtToken).transfer(cur1, curTAmount);
				} else {
					IERC20(husdtToken).transfer(inviteBurnAddress, curTAmount);
				}
			}
			...
			for (int256 i = 0; i < 5; i++) {
				...
				if (IERC20(swapToken).balanceOf(cur2) >= _minSwapCoin && cur2 != address(0)){
					IERC20(husdtToken).transfer(cur2, curTAmount);
				} else {
					IERC20(husdtToken).transfer(inviteBurnAddress, curTAmount);
				}
			}
	...
	function _toLpPool(uint amount) private{
		IERC20(husdtToken).transfer(lpAddress, amount);
	}
	...
	function _toDao(uint amount) private{
		IERC20(husdtToken).transfer(marketAddress, amount);
	}
