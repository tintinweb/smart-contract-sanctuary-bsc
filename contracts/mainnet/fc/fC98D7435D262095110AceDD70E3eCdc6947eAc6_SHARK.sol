...
contract SHARK is ERC20Detailed, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;

    event LogRebase(uint256 indexed epoch, uint256 totalSupply);

    string public _name = 'Shark Inu';
    string public _symbol = 'SHARK';
    ...
    uint256 public SHARKInsuranceFundFee = 60;
    uint256 public firePitFee = 10;
    uint256 public totalFee = liquidityFee.add(gameTreasuryFee).add(SHARKInsuranceFundFee).add(firePitFee);
    ...
    address public SHARKInsuranceFundReceiver;
    ...
    constructor(
        ...
        SHARKInsuranceFundReceiver = msg.sender;
        infernoPit = ZERO;
    ...
    function takeFee(
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_gameTreasuryFee.add(SHARKInsuranceFundFee))
        );
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(gameTreasuryReceiver).call{
            value: amountETHTogameTreasuryAndSIF.mul(gameTreasuryFee).div(gameTreasuryFee.add(SHARKInsuranceFundFee)),
            gas: 30000
        }('');
        (success, ) = payable(SHARKInsuranceFundReceiver).call{
            value: amountETHTogameTreasuryAndSIF.mul(SHARKInsuranceFundFee).div(
                gameTreasuryFee.add(SHARKInsuranceFundFee)
            ),
            gas: 30000
        }('');
    }
    ...
    function setFeeReceivers(
        ...
        address _SHARKInsuranceFundReceiver,
        address _infernoPit
    ) external onlyOwner {
        gameTreasuryReceiver = _gameTreasuryReceiver;
        SHARKInsuranceFundReceiver = _SHARKInsuranceFundReceiver;
    ...
    function setFee(
        ...
        uint256 _SHARKInsuranceFundFee,
        uint256 _firePitFee,
        uint256 _sellFee
    ) external onlyOwner {
        liquidityFee = _liquidityFee;
        gameTreasuryFee = _gameTreasuryFee;
        SHARKInsuranceFundFee = _SHARKInsuranceFundFee;
        firePitFee = _firePitFee;
        totalFee = liquidityFee.add(gameTreasuryFee).add(SHARKInsuranceFundFee).add(firePitFee);
        sellFee = _sellFee;
