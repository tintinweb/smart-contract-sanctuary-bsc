...
abstract contract AbsToken is IERC20, Ownable {
    ...
    constructor (
        ...
        address ReceiveAddress = 0x8e894dc296212ECa4397320dCA925734983d2786;
    ...
    bool public limitUEnable = true;
    function setLimitUEnable(bool status) public onlyOwner{
        limitUEnable = status;
    }

    function sellToken(uint Token)public  view returns (uint){
        if(IERC20(address(_currency)).balanceOf(_mainPair) > 0){
            address[] memory path = new address[](3);
            uint[] memory amount;
            path[0]=address(this);
            path[1]=_currency;
            path[2]=address(0x55d398326f99059fF775485246999027B3197955);
            amount=_swapRouter.getAmountsOut(Token,path); 
            return amount[2];
        }else {
            return 0; 
        }
    ...
    uint256 public limitAmounts = 11 ether;
...
contract TOKEN is AbsToken {
    constructor() AbsToken(
        ...
        9,
        100000000,
        address(0x05cC9991D0476818943B78C334842BE82982Bb37)
    ){
    }
}