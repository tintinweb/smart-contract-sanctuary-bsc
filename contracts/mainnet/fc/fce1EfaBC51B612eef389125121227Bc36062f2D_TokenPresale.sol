...
contract TokenPresale is Ownable, ReentrancyGuard {
    ...
    @param _walletToFund In case the owner wants to send the raised funds 
    ...
    **/
    ...
    constructor(
        ...
        MIN_BUY = configs[0];
        MAX_BUY = configs[1];
        if (MAX_BUY > 0) require(MAX_BUY > MIN_BUY, "CF1"); // dev: Max buy is less than min buy
        tokensToSell = configs[6];
        saleStart = configs[8];
        if (_walletToFund != address(0)) fundWallet = _walletToFund;
    }
    ...
    // TODO do this
    // function claimBackFunds() external {}
    ...
    function getWhitelistStatus(address _user) internal returns (bool) {
        if (whitelist[_user]) return true;
        if (address(WHITELIST_TOKEN) != address(0)) {
    ...
    function withdraw() external payable onlyOwner {
        require(totalRaised >= SOFT_CAP, "Softcap not reached");
        uint256 raised;
        bool succ;
        if (address(RAISE_TOKEN) == address(0)) {
            raised = address(this).balance;
            (succ, ) = payable(fundWallet).call{value: raised}("");
            require(succ, "Unsuccessful, withdraw");
        } else {
            raised = RAISE_TOKEN.balanceOf(address(this));
            succ = RAISE_TOKEN.transfer(fundWallet, raised);
        }
        emit FundsClaimed(fundWallet, raised);
    }

    function updateFundWallet(address _newFund) external onlyOwner {
