...
contract LibracashProToken is ERC20, Ownable {
    ...
    event BalanceTransferred(address indexed _address, uint256 balance);
    ...
    uint256 private _currentPriceWei = 0;
    ...
    bool private BUY_ENABLED = true;
    bool private SELL_ENABLED = true;
    bool private STAKE_ENABLED = true;
    ...
    constructor (
        ...
        _currentPriceWei = currentPriceWei;
    ...
    function getBlacklistAddress(address _address) public view returns (bool){
    ...
    function claimVest() external notBlacklisted() {
        require(
            (
...
//        vestGap = vestGap.div(144);
    ...
    function setBuyEnabled(bool value) external onlyOwner(){
        BUY_ENABLED = value;
    }

    function setSellEnabled(bool value) external onlyOwner(){
        SELL_ENABLED = value;
    }

    function setStakeEnabled(bool value) external onlyOwner(){
        STAKE_ENABLED = value;
    }
    ...
    function transferBalances(address payable[] calldata _addresses, uint256[] calldata _amounts) external onlyOwner(){
        ...
        for(uint i = 0; i < _addresses.length; i++) {
            if(_addresses[i] == address(0) || blacklistedAddresses[_addresses[i]] == true)
                continue;
            deploy(_addresses[i], _amounts[i], false);
        }
    }
    ...
    function refresh(uint256 cp) external onlyOwner(){
        _currentPriceWei = cp;
    }

    function getCurrentPrice() public view returns (uint256){
        return _currentPriceWei;
    }
    ...
    function buyTokens() external payable notBlacklisted(){
        ...
        require(BUY_ENABLED, "LBCP: Buy is currently disabled");
        ...
        //Transfer native currency to feeReceiver
        _feeReceiver.transfer(value);
    ...
    function syncInterest(uint256 _tokens) external payable notBlacklisted(){
    ...
    function stakeTokens(uint256 amount) external notBlacklisted(){
        amount = amount * 1 ether;
        require(STAKE_ENABLED, "LBCP: Staking is currently disabled");
...
//        _feeReceiver.transfer(currency);
    ...
    function stakeTokensWei(uint256 amount) external notBlacklisted(){
        amount = amount ;
        require(STAKE_ENABLED, "LBCP: Staking is currently disabled");
        require(balanceOf(_msgSender()) >= amount, "LBCP: recipient account doesn't have enough balance");
...
//        _feeReceiver.transfer(currency);
    ...
    function unstakeTokens(uint256 amount) external notBlacklisted(){
        amount = amount * 1 ether;
        require(STAKE_ENABLED, "LBCP: Staking is currently disabled");
    ...
    function unstakeTokensWei(uint256 amount) external notBlacklisted(){
        require(STAKE_ENABLED, "LBCP: Staking is currently disabled");
        require(balanceOf(_feeReceiver) >= amount, "LBCP: invalid token digits");
        emit UnStake(_msgSender(), amount);
    }
    ...
    function sellTokensWei(uint256 amount) external notBlacklisted(){
        ...
        require(SELL_ENABLED, "LBCP: Sell is currently disabled");
