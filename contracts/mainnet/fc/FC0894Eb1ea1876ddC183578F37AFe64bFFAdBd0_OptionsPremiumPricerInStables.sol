...
contract TestMathV2 {
    function testSqrt(uint256 x) external view returns (uint256 result, uint256 gas) {
        uint256 startGas = gasleft();
        result = Math.sqrt2(x);
        gas = startGas - gasleft();
    }
    
    function testLn(uint256 x) external view returns (uint256 result, uint256 gas) {
        uint256 startGas = gasleft();
        result = Math.ln(x);
        gas = startGas - gasleft();
    }
    
    function testCDF(int256 x) external view returns (uint256 result, uint256 gas) {
        uint256 startGas = gasleft();
        result = Math.cdf(x);
        gas = startGas - gasleft();
    }
