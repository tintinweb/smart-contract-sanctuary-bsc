...
contract XCSToken is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 2189 * 10 ** 9;

    string private _name = "XCS";
    string private _symbol = "XCS";
    
    uint public _buyLpFee = 10;
    uint public _buyLqFee = 10;
    uint public totalBuyFee = 20;

    uint public _sellLpFee = 20;
    uint public _sellMkFee = 10;
    uint public totalSellFee = 30;

    uint public _transferFee = 50;
    ...
    function _transfer(
        ...
        if( takeFee ){
            ...
            if( !isTrans ){
                ...
                param.isBuy = ammPairs[from] && !isDelLiquidity;

                if( param.isBuy){
                    buyAmounts[to] += amount;

                    if( block.timestamp - startTime <  swapLimitTime){
                        require(buyAmounts[to] <= 2e9,"exceed buy amount");
                    }
                }
            }
        }
