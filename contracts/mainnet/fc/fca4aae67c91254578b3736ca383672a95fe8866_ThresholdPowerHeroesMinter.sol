...
contract ThresholdPowerHeroesMinter is ExternalEligibilityContract {
    ...
    TargetNFT public targetNFTsc;                               // The Target (to mint) NFT smart contract 
    ...
    public {
        ...
        targetNFTsc = TargetNFT(_targetNFTsc);
    }
    ...
    function isEligibleExt(address user_address, address, uint16, uint level, uint8 sourceID) external onlyPowermadeContract returns (bool is_eligible) {
        ...
        if (sourceID == 1) {        // Case Package Buy
            ...
            // Minting part (requires minter role configured in the target NFT smart contract)
            targetNFTsc.mintRandomNFTto(user_address);
            return true;
        } else if (sourceID == 2) {     // Case sponsor commission (direct)
            return true;
        } else if (sourceID == 3) {     // Case level commission. param1 is the distributed level
