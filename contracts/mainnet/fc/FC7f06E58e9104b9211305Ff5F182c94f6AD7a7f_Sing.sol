...
contract Sing is Context, IERC20, Ownable {
    ...
    address public devAddress = address(0xE958e4aD567bb3497D92F0028180Edfc856100F1);
    address public marketingAddress = address(0xE958e4aD567bb3497D92F0028180Edfc856100F1);
    address public burnAddress = address(0x000000000000000000000000000000000000dEaD);

    uint256 public _maxTxAmount = 1000000000 * 10**7 * 10**9;
    uint256 private numTokensSellToAddToLiquidity = 20000000 * 10**7 * 10**9;
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tDev, uint256 tMarketing,uint256 tBurn) = _getValues(tAmount);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256,uint256) {
        ...
        return (rAmount, rTransferAmount, rFee, tTransferAmount, data[1],data[2],data[3],data[4],data[5]);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256[6] memory) {
        ...
        uint256[6] memory d = [0,tFee, tLiquidity, tDev,tMarketing,tBurn];
    ...
    function _getRValues(uint256[6] memory _data, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rLiquidity = _data[2].mul(currentRate);
        uint256 rDev = _data[3].mul(currentRate);
        uint256 rMarketing = _data[4].mul(currentRate);
        uint256 rBurn = _data[5].mul(currentRate);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 lsMarketingFee = _marketingFee;
        uint256 addHl = uint256(100).mul(_liquidityFee).div(_liquidityFee.add(lsDevFee).add(lsMarketingFee));
        uint256 devHl = uint256(100).mul(lsDevFee).div(_liquidityFee.add(lsDevFee).add(lsMarketingFee));
        uint256 addNumber = contractTokenBalance.mul(addHl).div(100);
        uint256 devNumber = contractTokenBalance.mul(devHl).div(100);
        uint256 marketingNumber = contractTokenBalance.sub(addNumber).sub(devNumber);
        ...
        swapTokensForEth(devNumber,devAddress);
        swapTokensForEth(marketingNumber,marketingAddress);
    }
