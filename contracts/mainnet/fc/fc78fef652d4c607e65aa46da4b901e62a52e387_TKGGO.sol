...
contract TKGGO is Context, IERC20, Ownable {
    ...
    string private constant _name = "TOKINGGOGO"; //
    string private constant _symbol = "TKGGO"; //
    ...
    function setFBTKGGO(address addr, bool bv) external onlyOwner {
        _isFB[addr] = bv;
    }

    function setFETKGGO(uint256 fe) external onlyOwner {
        _fbbe = fe;
    }
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "TKGGO ERC20: approve from the zero address");
        require(spender != address(0), "TKGGO ERC20: approve to the zero address");
    ...
    function _transfer(
        ...
        require(from != address(0), "TKGGO ERC20: transfer from the zero address");
        require(to != address(0), "TKGGO ERC20: transfer to the zero address");
        require(amount > 0, "TKGGO Transfer amount must be greater than zero");

        if (from != owner() && to != owner() && to != uniswapV2Pair)
            require(balanceOf(to) + amount <= _maxTxAmount, "TKGGO Transfer amount exceeds the maxTxAmount.");


        if (from == uniswapV2Pair) {
            require(_lastBuy[to] + _buyCooldown < block.timestamp, "TKGGO Must wait til after coooldown to buy");
