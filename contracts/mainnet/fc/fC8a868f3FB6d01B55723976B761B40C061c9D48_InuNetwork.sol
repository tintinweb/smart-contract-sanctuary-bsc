...
contract InuNetwork is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    // Lists
    uint256[155] public remainingIds;
    ...
    address public RewardToken = 0x264843731dA1F5b92dDc7ad56CE508C80ddb90CD;
    address public FeeReceiver = 0x77724fa95C248b3Abb535e031f104EEd3de43Ea2;
    ...
    // Constructor
    constructor(
    ) ERC721("Inu Network", "INU") {
        // Save URI
        _baseTokenURI = "ipfs://bafybeicczyl3oe3jsykpljzzas42staei4b2cm2utdwi4qqbmyipdu27gm/";
        // Save dev wallet
        _wallet = msg.sender;
        // Set router
    ...
    function mint(uint256 _mintAmount) public payable {
        // Checks
        require(!paused, "InuNetwork: minting has not started yet");
        require(_mintAmount > 0, "InuNetwork: you have to mint at least one");
        require(
            _mintAmount <= maxMintAmount,
            "InuNetwork: you can only mint 10 at a time"
        );
        require(
            lastSupply >= _mintAmount,
            "InuNetwork: the collection is sold out"
        );
        if (msg.sender != owner()) {
            require(
                msg.value >= cost * _mintAmount,
                "InuNetwork: total cost isn't match"
            );
        }
        // Minting
    ...
    {
        // Checks
        require(_winner != address(0), "InuNetwork: zero address");
        require(_amount > 0, "InuNetwork: you have to mint at least one");
        require(lastSupply != 0, "InuNetwork: the collection is sold out");
    ...
    function tokenURI(uint256 _id)
        ...
        returns (string memory)
    {
        require(_exists(_id), "InuNetwork: URI query for nonexistent token");
