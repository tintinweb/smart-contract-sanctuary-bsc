...
contract LinearClaimer is IClaimer, Ownable {
    ...
    function claimAll(address account) external override {
        require(!_isPaused, 'Claimer: Paused');
    ...
    function getUnlockedAmount(uint256 amount) internal view returns (uint256) {
        if (block.timestamp >= vestingStartDate + vestingDuration) {
            return amount;
        }
        ...
        if (block.timestamp > initialUnlockDate) {
    ...
    function getUnlockedVestingLocksN() internal view returns (uint256) {
        ...
        uint256 unlockedN = 1 + (block.timestamp - vestingStartDate) / unitOfTime;
