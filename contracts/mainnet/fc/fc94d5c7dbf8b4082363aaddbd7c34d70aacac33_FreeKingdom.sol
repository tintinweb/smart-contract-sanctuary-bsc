...
contract FreeKingdom {
    ...
    function addCrystals(address ref) initialized external payable {
        ...
        ref = towers[user].ref;
        towers[ref].crystals += (crystals * 5) / 100;      //You get 5% in crystals from each deposit your ally
        towers[ref].money += (crystals * 100 * 3) / 100;   //You get 3% in gold from each deposit your ally
        towers[ref].refDeps += crystals;
        towers[user].crystals += crystals;
        towers[manager].crystals += (crystals * 0) / 100;  //crystals * 0  No Deposit fees
        payable(manager).transfer((msg.value * 0) / 100);  //msg.value * 0 No Deposit fees
    }
    ...
    function getUpgradePrice(uint256 towerId, uint256 chefId) internal pure returns (uint256) {
        if (chefId == 1) return [600, 6000, 18000, 36000, 60000][towerId];
        if (chefId == 2) return [900, 9000, 27000, 54000, 90000][towerId];
        if (chefId == 3) return [1360, 13500, 40500, 81000, 135000][towerId];
        if (chefId == 4) return [2040, 20260, 60760, 121500, 202500][towerId];
        if (chefId == 5) return [3060, 30400, 91140, 182260, 303760][towerId];
        if (chefId == 6) return [4590, 45600, 136710, 273390, 455640][towerId];
