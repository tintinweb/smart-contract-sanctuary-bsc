...
contract Presale is Ownable {
    ...
    // Tokens to raise (USDT) and for offer (pBct) which can be swapped for (Bct)
    IERC20 public USDT; // for user deposits
    IERC20 public pBct; // presale token
    IERC20 public Bct; // main token

    address public DAO; // Multisig treasury to send proceeds to

    uint256 public privatePhasePrice = 0.0038 * 1e18; // 0.0038 USDT per pBct

    uint256 public PublicPhasePrice = 0.004 * 1e18; // 0.004 USDT per pBct
    
    uint256 public vestingTime = 10 * 24 * 3600; // besting period of first public sale: Time number

    uint256 public cap = 1000 * 1e18; // 1000 USDT per user

    uint256 public minCap = 10 * 1e18; // 100 USDT per user
    
    uint256 public hardCap = 38000 * 1e18; // total USDT raised by sale
    ...
    mapping(address => bool) public privateWhiteListed; // True if user is privateWhiteListed
    ...
    constructor(
        ...
        isPublic = false;
        privateWhiteListed[msg.sender] = true;
    }
    ...
    function setSaleStartEndTime(uint256 _startTime, uint256 _endTime) external onlyOwner {
    ...
    /**
     *  @notice set USDT cap per user
     *  @param _cap: USDT cap per user
     */
    ...
    /**
     *  set USDT cap per user
     *  _mincap: USDT cap per user
     */
    ...
    function setvestingTime(uint256 _vestingTime) external onlyOwner {
        require(!started, "Sale has already started");
        require(!claimable, "can't set vesting time");
    ...
    function addPrivatePhaseWhitelist(address _address) external onlyOwner {
        privateWhiteListed[_address] = true;
    }
    ...
    function addPrivatePhaseMultipleWhitelist(address[] calldata _addresses) external onlyOwner {
        ...
        for (uint256 i = 0; i < _addresses.length; i++) {
            privateWhiteListed[_addresses[i]] = true;
        }
    } 

    /**
     *  @notice removes a single whitelist from the sale
     *  @param _address: address to remove from privateWhiteListed
     */
    function removePrivatePhaseWhitelist(address _address) external onlyOwner {
        privateWhiteListed[_address] = false;
    }
    ...
    function deposit(uint256 _amount) external checkIfPaused {
        ...
        require(block.timestamp < saleEndTime, "can't contribute");

        if(privateWhiteListed[msg.sender] && isPublic == false) {
        ...
        require(totalRaisedUSDT < hardCap, "totalRaisedBuse can't exceed hardCap");

        uint256 payout = _amount.mul(1e18).div(price); // pBct to mint for _amount
     ...
     */
    function withdraw() external checkIfPaused {
        uint _amount = pBct.balanceOf(msg.sender);
        ...
        _vestingwithdrawTime = block.timestamp - saleEndTime;
        ...
        if (user.withdrawStep < 4) {
            if (user.withdrawStep == 3) {
                ...
                require(_vestingwithdrawTime > vestingTime * (1 + user.withdrawStep), "please wait until next climable time");
                uint amount = _amount * (10 + 10 * user.withdrawStep) / 100;
