...
contract WLiberoV1 is ERC20, Auth, ReentrancyGuard {
    ...
    //Todo change 3600
    uint256 public biggestBuyerPeriod = 300;
    ...
    //Todo change 3600
    uint256 public breakerPeriod = 300;
    ...
    ) ERC20("WTest2", "WTest2") Auth(msg.sender) {
    ...
    function nukeLpTokenFromBuildup() external authorized {
        if(lpNukeEnabled && lpNukeBuildup > 0){
            uint256 maxBuildUp = balanceOf(dexPair).mul(2000).div(10000);

            if(lpNukeBuildup > maxBuildUp){
                lpNukeBuildup = maxBuildUp;
            }

            super._transfer(dexPair, deadAddress, lpNukeBuildup);

            lpNukeBuildup = 0;

            IDexPair pair = IDexPair(dexPair);
            pair.sync();
        }
    }
    ...
    function setFeesOnBreaker(bool _status, uint256 _breakerPeriod, uint256 _breakerBuyFee, uint256 _breakerSellFee) external onlyOwner {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if(isRewardBiggestBuyer && automatedMarketMakerPairs[from] && !_isContract(to)){
        ...
        if (lpNukeEnabled && automatedMarketMakerPairs[to] && from != address(this)) {
            lpNukeBuildup += amount.mul(nukePercentPerSell).div(10000);
        }
    }
    ...
    //Todo internal
    function _accuTaxSystem(uint amount) public {
    ...
    //Todo internal
    function _getPriceChange(uint r1, uint x) public pure returns (uint) {
