...
contract NetGymFarming is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    struct SpecialOfferConditions {
        uint128 dollarValueThreshold;
        uint128 dollarValueMultiplier;
    }

    /**
     * @notice Info of token prices when user makes deposit during special offer time
     * @param bnbUsdPrice USD price of bnb token as is
     * @param gymnetUsdPrice USD price of Gymnet token times 1e18
     */
    ...
    address[] public wbnbToUsdt;

    SpecialOfferConditions public specialOfferConditions;
    mapping(address => UserDepositPriceInfo) public userSpecialOfferDeposit;
    ...
    event WhitelistAddress(address indexed _address, bool isWhitelisted);
    ...
    function initialize(
        ...
        require(address(_bankAddress) != address(0), "GymFarming::Bank address is zero");
    ...
    function setMLMAddress(address _address) external onlyOwner {
        require(address(_address) != address(0), "GymFarming::MLM address is zero");
        relationship = _address;
    }

    function setLevelPoolAddress(address _levelPoolAddress) external onlyOwner {
        require(address(_levelPoolAddress) != address(0), "GymFarming::Level Pool address is zero");
    ...
    function setTreasuryAddress(address _newTreasury) external onlyOwner {
        require(address(_newTreasury) != address(0), "GymFarming::Treasury address is zero");
    ...
    function setWbnbToUsdtPath(address[] memory _path) external onlyOwner {
        wbnbToUsdt = _path;
    }

    function setSpecialOfferConditions(
        uint128 _dollarValueThreshold,
        uint128 _multiplier
    ) external onlyOwner {
        specialOfferConditions.dollarValueThreshold = _dollarValueThreshold;
        specialOfferConditions.dollarValueMultiplier = _multiplier;
    }
    ...
    function speedStake(
        ...
        require(IGymMLM(relationship).addressToId(msg.sender) > 0, "You are not part of MLM");
        IPancakeRouter02 router = IPancakeRouter02(ROUTER_ADDRESS);

        updatePool(_pid);
        IPancakeswapPair lpToken = IPancakeswapPair(
        ...
        if (_isSpecialOfferThresholdPassed(bnbAmount, _tokenAmount)) {
            _saveSpecialOfferDepositInfo(msg.sender);
        }
        ...
        IERC20(wbnbAddress).safeTransfer(
            buyBack,
            (bnbAmount * buyBackComission) / 100
        );
        ...
        _deposit(_pid, lpTokens, msg.sender);
    }
    ...
    function claimAndDeposit(
        ...
        uint256 _deadline,
        uint256 _minBurnAmt
    ) external payable onlyOwner poolExists(_pid) {
     ...
     */
     function harvest(uint256 _pid) public nonReentrant poolExists(_pid) {
    ...
    function _deposit(
        ...
        require(IGymMLM(relationship).addressToId(_from) > 0, "You are not part of MLM");
     ...
     */
    function _getBnbPrice() private view returns (uint256) {
        uint256[] memory bnbPriceInUsd = IPancakeRouter02(ROUTER_ADDRESS)
            .getAmountsOut(1, wbnbToUsdt);

        return bnbPriceInUsd[1];
    }

     /**
     * Returns the latest Gymnet token price
     */
    function _getGymnetPrice() private view returns (uint256) {
        uint256[] memory gymPriceInUsd = IPancakeRouter02(ROUTER_ADDRESS)
            .getAmountsOut(1e18, rewardTokenToWBNB);

        return gymPriceInUsd[1] * _getBnbPrice();
    }

    /**
     * @notice Calculate user deposit dollar value for given amounts
     * @param bnbAmount BNB deposit amount
     * @param gymnetAmount Gymnet token deposit amount
     * @return dollarValue total deposit amount in USD
     */
    function _getDepositDollarValue(uint256 bnbAmount, uint256 gymnetAmount)
        private
        view
        returns (uint256 dollarValue)
    {
        dollarValue =
            (gymnetAmount * _getGymnetPrice()) /
            1e36 +
            (bnbAmount * _getBnbPrice()) /
            1e18;
    }

    /**
     * @notice Check if user deposit fits special offer conditions
     * @param bnbAmount BNB deposit amount
     * @param gymnetAmount Gymnet token deposit amount
     * @return bool True if special offer threshold in USD value passed, otherwise false
     */
    function _isSpecialOfferThresholdPassed(
        uint256 bnbAmount,
        uint256 gymnetAmount
    ) private view returns (bool) {
        uint256 dollarValue = _getDepositDollarValue(bnbAmount, gymnetAmount);
        uint256 multipliedValue = (dollarValue *
            specialOfferConditions.dollarValueMultiplier) / 100;

        return multipliedValue >= specialOfferConditions.dollarValueThreshold;
    }

    /**
     * @notice Save tokens price info at the moment of deposit
     * @param _user user address
     */
    function _saveSpecialOfferDepositInfo(address _user) private {
        userSpecialOfferDeposit[_user] = UserDepositPriceInfo({
            bnbUsdPrice: (_getBnbPrice()*specialOfferConditions.dollarValueMultiplier) /100,
            gymnetUsdPrice: (_getGymnetPrice()*specialOfferConditions.dollarValueMultiplier) /100
        });
    }

    function isSpecialOfferParticipant(address _user)
        external
        view
        returns (bool)
    {
        return userSpecialOfferDeposit[_user].gymnetUsdPrice != 0;
    }
        ...
        )
    {
        ...
        uint256[] memory swapResult = IPancakeRouter02(ROUTER_ADDRESS)
            .swapExactETHForTokens{value: bnbAmount / 2}(
            _minAmountOut,
            path,
            address(this),
            _deadline
        ...
        uint256 gymnetToLpAmount = swapResult[1];

        IERC20(rewardToken).approve(ROUTER_ADDRESS, gymnetToLpAmount);

        (tokensAdded, bnbAdded, lpTokens) = IPancakeRouter02(ROUTER_ADDRESS)
            .addLiquidityETH{value: bnbAmount / 2}(
            address(rewardToken),
            gymnetToLpAmount,
            _amountTokenMin,
            _amountETHMin,
    ...
    //
    function withdrawFrom(uint256 _pid, address _from)
        external
        nonReentrant
        poolExists(_pid)
    {
        require(whitelist[msg.sender], "GymFarming: only whitelisted");
        ...
        uint256 pending = (user.amount * pool.accRewardPerShare) /
            ...
            uint256 rewardTokenBal = rewardToken.balanceOf(address(this));
                ...
                require(pending < rewardTokenBal, "GymFarming::No sufficient rewards");
                uint256 comission = (pending * affilateComission) / 100;
                rewardToken.transfer(relationship, comission);
                IGymMLM(relationship).distributeRewards(
                    pending,
                    address(rewardToken),
                ...
                uint256 poolComissionAmount = (pending * poolComission) / 100;
                rewardToken.transfer(treasury, poolComissionAmount);
                rewardToken.transfer(
                    _from,
                    (pending - comission - poolComissionAmount)
                );
        }
        emit Harvest(_from, _pid, pending);
        user.amount = 0;
        ...
        emit Withdraw(_from, _pid, withdrawAmount);
    }
}
