...
//    ___  __    ________  ________  ________  _____ ______   ________          ________  ___  ___  ________ _________  ___  ________  ________   ________      
//   |\  \|\  \ |\   __  \|\_____  \|\   __  \|\   _ \  _   \|\   __  \        |\   __  \|\  \|\  \|\   ____\\___   ___\\  \|\   __  \|\   ___  \|\   ____\     
//   \ \  \/  /|\ \  \|\  \\|___/  /\ \  \|\  \ \  \\\__\ \  \ \  \|\  \       \ \  \|\  \ \  \\\  \ \  \___\|___ \  \_\ \  \ \  \|\  \ \  \\ \  \ \  \___|_    
//    \ \   ___  \ \   __  \   /  / /\ \   __  \ \  \\|__| \  \ \   __  \       \ \   __  \ \  \\\  \ \  \       \ \  \ \ \  \ \  \\\  \ \  \\ \  \ \_____  \   
//     \ \  \\ \  \ \  \ \  \ /  /_/__\ \  \ \  \ \  \    \ \  \ \  \ \  \       \ \  \ \  \ \  \\\  \ \  \____   \ \  \ \ \  \ \  \\\  \ \  \\ \  \|____|\  \  
//      \ \__\\ \__\ \__\ \__\\________\ \__\ \__\ \__\    \ \__\ \__\ \__\       \ \__\ \__\ \_______\ \_______\  \ \__\ \ \__\ \_______\ \__\\ \__\____\_\  \ 
//       \|__| \|__|\|__|\|__|\|_______|\|__|\|__|\|__|     \|__|\|__|\|__|        \|__|\|__|\|_______|\|_______|   \|__|  \|__|\|_______|\|__| \|__|\_________\
//                                                                                                                                                  \|_________|
...
/**
 * @title KazamaSwap Farms Auctions.
 * @notice Auctions for new Farms, including multiplier.
 */
contract KazamaFarmAuction is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    using EnumerableSet for EnumerableSet.AddressSet;

    IERC20 public immutable kazamaToken;
    ...
    constructor(
        ...
        kazamaToken = IERC20(_kazamaToken);
        operatorAddress = _operatorAddress;
        maxAuctionLength = _maxAuctionLength;
    }

    // We need to apply the burn settings from the KAZAMA token to this {bid} function.
    // This in order to make sure the bid amount equals the amount send after the burn.
     ...
     * @param _amount: amount of the bid in $Kazama token
    ...
    function bid(uint256 _amount) external nonReentrant {
        ...
        // We deduct the tokens that are burned in the transaction..
        uint256 correctedBid = _amount - tokensToBurn;

        if (auctionBids[currentAuctionId][msg.sender].totalAmount == 0) {
            require(correctedBid >= auctions[currentAuctionId].initialBidAmount, "Bid: Incorrect initial bid amount");
        }

        /**
         * @note This stays _amount and not correctedBid.
         * 
         * Because _amount needs to be sended in order to equal correctedBid (due the burn).
         * If we don't do this, we will again, have a difference in sended KAZAMA and the final bid.
         */
        kazamaToken.safeTransferFrom(address(msg.sender), address(this), _amount);

        auctionBids[currentAuctionId][msg.sender].totalAmount += correctedBid;
        ...
        /**
         * @note Emit the bid with correctedBid (so it matches _amount minus the burn).
         * 
         * Normally the bid would be _amount .. Since KAZAMA has a burn function on every transaction, the bid
         * would not have matched the amount of KAZAMA that would be received by the contracts.
         * 
         * So the bid will be correctedBid now which is _amount minus the burned amount of KAZAMA tokens.
         * Now the bid equals the actual received KAZAMA.
         */
        emit AuctionBid(currentAuctionId, msg.sender, correctedBid);
    }
    ...
    function claimAuction(uint256 _auctionId) external nonReentrant {
        ...
        kazamaToken.safeTransfer(address(msg.sender), claimableAmount);
    ...
    /**
     * @notice Close an auction round, and store $Kazama leaderboard threshold
     * @param _bidLimit: minimal $Kazama committed to be a winner
     * @dev Callable by operator
     */
    ...
    /**
     * @notice Claim $Kazama for all bids by bidders in leaderboard
     * @param _auctionId: auction id
     * @param _bidders: addresses of bidders
    ...
    function recoverToken(address _token, uint256 _amount) external onlyOwner {
        require(_token != address(kazamaToken), "Recover: Cannot be the Kazama token");
