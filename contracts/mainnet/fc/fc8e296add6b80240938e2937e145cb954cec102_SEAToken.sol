...
contract SEAToken is ERC20, Ownable {
    ...
    address public fundationWallet = 0x5AAb6CD253Fa02B7CB9Cb1A3A6aaA8B4eF5Ed69e;
    address public lpWallet = 0x25cBfD2dce66033807A6e606e4411A4715701C0A;
    address public nodeWallet = 0x37F148C478cCd512f6aD155Cb8D55850e8Eb9eEd;

    address public deadWallet = 0x000000000000000000000000000000000000dEaD;

    uint256 public deadFee = 100;
    uint256 public fundationFee = 200; 
    uint256 public lpFee = 200;
    uint256 public nodeFee = 200;
    uint256 public transFee = 1500;
    ...
    constructor() ERC20("SEA", "SEA")  {
        ...
        excludeFromFees(fundationWallet, true);
    ...
    function setFundationWallet(address _address) public onlyOwner {
        fundationWallet = _address;
    }
   ...
   function setDeadFee(uint256 val) public onlyOwner {
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            if (AmountFundationFee > 0) {
                swapLPRewardToken(AmountFundationFee, fundationWallet);
                AmountFundationFee = 0;
            }
            if (AmountNodeFee > 0) {
                swapLPRewardToken(AmountNodeFee, nodeWallet);
                AmountNodeFee = 0;
            }
            if (AmountLpFee > 0) {
                swapLPRewardToken(AmountLpFee, lpWallet);
                AmountLpFee = 0;
            }
            swapping = false;
        }
    ...
    function takeSwapFee(address from, uint256 amount) private returns(uint256 amountAfter) {
        ...
        uint256 FFee = amount.mul(fundationFee).div(10000);
        amountAfter = amountAfter.sub(FFee);
        AmountFundationFee = AmountFundationFee.add(FFee);

        uint256 NFee = amount.mul(nodeFee).div(10000);
        amountAfter = amountAfter.sub(NFee);
        AmountNodeFee = AmountNodeFee.add(NFee);

        uint256 LFee = amount.mul(lpFee).div(10000);
        amountAfter = amountAfter.sub(LFee);
        AmountLpFee = AmountLpFee.add(LFee);

        super._transfer(from, address(this), FFee.add(NFee).add(LFee));
    }
    ...
    function swapLPRewardToken(uint256 tokenAmount, address receiver) private {
