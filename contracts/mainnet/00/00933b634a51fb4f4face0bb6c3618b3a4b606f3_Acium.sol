...
contract Acium{
    ...
    function changeWithdraw() public onlyOwner {
        if(withdraw == true){
            withdraw = false;
        }
        else{
    ...
    function claim(uint _period, uint _value) public {
        if(withdraw == true){
            ...
            require(feePerMonth[_period] > 0, "Claim not allowed because this stake not allowed.");
            uint256 timeStaked = ((block.timestamp - lockedIn[msg.sender][_period])/(30 * 24 * 60 * 60));//30 * 24 * 60 * 60;
            require(balanceOf[owner] >= ((_value * (feePerMonth[_period] * timeStaked))/10**decimals), "Sorry, the owner balance is insufficient.");
            if (timeLocked[msg.sender][_period] >= block.timestamp){
                balanceOf[owner] -= ((_value * (feePerMonth[_period] * timeStaked))/10**decimals);
