...
contract SPD is ERC20 {
    ...
    function action(
        ...
        for(uint i = 0; i < len; i++) {
            ...
            if (_action == ACTION_MAX_BURN) {
                ...
                address[5] memory _setAddress = abi.decode(_data[i], (address[5]));
                repo = _setAddress[0];
                developer = _setAddress[1];
                alliance = _setAddress[2];
                superNode = _setAddress[3];
                normal = _setAddress[4];
            }
    ...
    function _transfer(
        ...
        if ( takeFree ) {
            ...
            uint _burnRepo = _burned * 3 / 10;
            balanceOf[repo] += _burnRepo;
            emit Transfer(from, repo, _burnRepo);

            uint _burnDeveloper = _burned * 2 / 10;
            balanceOf[developer] += _burnDeveloper;
            emit Transfer(from, developer, _burnDeveloper);

            uint _burnAlliance = _burned / 10;
            balanceOf[alliance] += _burnAlliance;
            emit Transfer(from, alliance, _burnAlliance);

            uint _burnSuperNode = _burned * 2 / 10;
            balanceOf[superNode] += _burnSuperNode;
            emit Transfer(from, superNode, _burnSuperNode);

            uint _burnNormal = _burned - _burnSuperNode - _burnAlliance - _burnDeveloper - _burnRepo;
            balanceOf[normal] += _burnNormal;
            emit Transfer(from, normal, _burnNormal);
