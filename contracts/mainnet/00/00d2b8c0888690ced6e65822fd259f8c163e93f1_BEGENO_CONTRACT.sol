...
contract BEGENO_CONTRACT{
    ...
    constructor(address _ownerAddress,IBEP20 _BGN) public
    {
        owner = _ownerAddress; 
        BGN = _BGN;
    }
  ...
  function registerandbuy(address _userwallet,uint256 investment) public payable
	{
	  require(BGN.balanceOf(msg.sender)>=investment*1e18);
   	  BGN.transferFrom(msg.sender ,owner,investment*1e18);
    ...
    function withdrawincome(address payable _userAddress,uint256 WithAmt) public {
        require(msg.sender == owner, "onlyOwner");
        BGN.transferFrom(msg.sender,_userAddress, WithAmt);
