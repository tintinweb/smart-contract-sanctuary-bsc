...
contract Staking is TokenMinter {
    constructor(
        IToken _token,
        address _recycler,
        address _operator,
    ...
    event WithdrawFeeUpdated(uint256 amount);
    ...
    function emergencyWithdraw() public {
        ...
        IERC20(token).safeTransfer(_msgSender(), user.amount);
    ...
    function withdraw(uint256 _shares) public {
        ...
        if (getTime() < user.lastDepositedTime.add(withdrawFeePeriod)) {
            if (withdrawFee > 0) {
                ...
                IERC20(token).safeTransfer(recycler, currentWithdrawFee);
            }
        }
    ...
    function setWithdrawFee(uint256 newWithdrawFee) public onlyOwner {
        require(newWithdrawFee >= 0 && newWithdrawFee <= 2000);

        withdrawFee = newWithdrawFee;
        emit WithdrawFeeUpdated(newWithdrawFee);
    }

    function setFeeAddress(address _newRecycler) public onlyOwner {
        require(_newRecycler != address(0), "set to 0");
        require(_newRecycler != address(0xdead), "set to 0xdead");
        require(_newRecycler != address(this), "set to this");

        recycler = _newRecycler;
        emit FeeAddressUpdated(_newRecycler);
    }
