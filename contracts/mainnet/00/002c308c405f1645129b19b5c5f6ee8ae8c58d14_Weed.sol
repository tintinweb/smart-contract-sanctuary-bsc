...
Started out at DEFI meme token, Weed features.
...
contract Weed is Context, IERC20, Ownable {
    ...
    address payable public WeedFundAddress = payable(0x000000000000000000000000000000000000dEaD);
    ...
    event SetWeedFundAddress(address newAddress);
    ...
    function setWeedFundAddress(address newAddress) external onlyOwner() {
        WeedFundAddress = payable(newAddress);

        emit SetWeedFundAddress(WeedFundAddress);
    }
   ...
   function setFees(uint256 newRewardFee, uint256 newLiquidityFee, uint256 newProjectFee, uint256 newBuyBackFee) external onlyOwner() {
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(forWallets > 0 && _projectFee > 0) 
        {
            ...
            uint256 newBalance = address(this).balance.sub(initialBalance);
            transferToAddressBNB(WeedFundAddress, newBalance);
