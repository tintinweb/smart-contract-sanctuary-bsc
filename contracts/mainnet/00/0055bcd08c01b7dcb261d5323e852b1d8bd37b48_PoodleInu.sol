...
contract PoodleInu is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    PoodleInuDividendTracker public dividendTracker;
    ...
    address payable public _marketingWalletAddress = payable(0x7AD740230146C1F601e50952da51607F9e5AF5d7);
    address payable public _buybackWalletAddress = payable(0x2012dD3F6DE1AeB1cD6CCAE7a6387A6d6104A20a);
    ...
    constructor() BEP20("PoodleInu", "POOD") 
    {
        dividendTracker = new PoodleInuDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "PoodleInu: The dividend tracker already has that address");
        PoodleInuDividendTracker newDividendTracker = PoodleInuDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "PoodleInu: The new dividend tracker must be owned by the PoodleInu token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "PoodleInu: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "PoodleInu: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "PoodleInu: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "PoodleInu: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "PoodleInu: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "PoodleInu: Cannot update gasForProcessing to same value");
...
contract PoodleInuDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("PoodleInu_Dividend_Tracker", "PoodleInu_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "PoodleInu_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "PoodleInu_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PoodleInu contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "PoodleInu_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "PoodleInu_Dividend_Tracker: Cannot update claimWait to same value");
