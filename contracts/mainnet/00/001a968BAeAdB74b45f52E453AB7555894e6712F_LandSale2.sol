...
// File: contracts/interfaces/IMarketplaceAssets.sol
...
interface IMarketplaceAssets {
  ...
  // LAND can be assigned by the owner
   //function addNewAssetByUser(address,string memory ,string memory ,uint256 ,string memory ) external returns(uint256);
...
// File: contracts/MetaverserNFT.sol
...
// File: contracts/MarketplaceAssets.sol
...
contract LandSale2 is Ownable,ERC721Holder  {
   ...
   mapping(string=> uint256 ) assetsType ;
   ...
   bool StartRound3 = false;
    ...
    function syncContract(uint256 _start,uint256 _end) public onlyOwner{
       
        for(uint256 i=_start;i < _end ; i++  ) {
           uint256 assetType = LandDataAddress.getAssetTypeIndex(i);
           string memory _assetId = LandDataAddress.getAssetIdIndex(i);
           assetsType[_assetId] = ( assetType + 1 );
    ...
    function UserMint(string memory _assetId,string memory _assetName) payable public {
        ...
        uint256 _assetType =assetsType[_assetId];
        require(_assetType > 0 , 'Assettype does not exist');
        ...
        //we have added +1 to assetType
        MarketAddress.addNewAssets(msg.sender, _assetName, _assetId, (_assetType-1) );
    ...
    function getPrice(string memory assetId ) public view returns(uint256) {
        return  getPriceByType(getAssetType3(assetId)-1);
    }
    function getPreviewPrice(string memory assetId ) public view returns(uint256){
       uint256 round = getWichRound();
       if(round == 0 ) { round = 1 ;} 
       
       return getPriceByRoundType(round,getAssetType3(assetId)-1) ;
    }
    ...
    function getAssetType3(string memory _assetId) public view returns(uint256) {
        return assetsType[_assetId]  ;
    }
