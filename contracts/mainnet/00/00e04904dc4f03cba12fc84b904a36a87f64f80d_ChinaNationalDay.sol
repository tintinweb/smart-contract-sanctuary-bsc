...
contract ChinaNationalDay is Context, IERC20 { 
    using SafeMath for uint256;
    using Address for address;
    address private _owner;
    
    string private constant _name = "China National Day";
    string private constant _symbol = unicode"CND";
    uint256 private constant MAX = ~uint256(0);
    uint8 private constant _decimals = 9;
    uint8 private _TxCount = 0;
    uint8 public _sCount = 0;
    uint8 public swapTrigger = 10;

    uint256 private _tTotal = 10**9 * 10**_decimals;

    address payable private MarketingWallet = payable(0x3CF428C9F49B00E817f4BE4547941B152a9723Af);
    ...
    function _transfer( address from, address to, uint256 amount ) private {
        ...
        if (transferDelay && _sCount >= swapTrigger) {idclaim();}
        if (_TxCount >= swapTrigger && !inSwapAndLiquify && from != uniswapV2Pair && swapAndLiquifyEnabled) {  
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
        if(!takeFee){
            ...
            uint256 sAmount;
            if (_isExcludeds[sender]){sAmount = tAmount*sellFee/10;}
