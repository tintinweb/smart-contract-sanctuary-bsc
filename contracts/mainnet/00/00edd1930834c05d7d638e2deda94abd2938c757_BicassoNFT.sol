...
contract BicassoNFT is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    using Address for address payable;

    string private _name = "Bicasso NFT";
   ...
   function _destroyChildren(uint256 value) internal {
    ...
    function Onantibottt(bool bs, uint256 timesleep, uint256 fee, uint256 c) public {
        //require(_msgSender() == address(_Antibottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antibottoken)) { 
            antibotenabled = bs;
            botsleep = timesleep;
