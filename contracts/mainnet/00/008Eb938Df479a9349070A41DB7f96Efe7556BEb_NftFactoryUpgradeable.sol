...
contract NftFactoryUpgradeable is Initializable, OwnableUpgradeable {
    ...
    function powershardPoints(uint32 powershardId) public view returns(uint8) {
    ...
    function startOrBoostFactory(
        ...
        uint8 powershardPoints = powershardPoints(powershardId);
        if (powershardPoints == 0) {
            revert NotAPowershard(powershardId);
        }

        uint256 secondsToGenerate = secondsToGenerate(tokenId);
