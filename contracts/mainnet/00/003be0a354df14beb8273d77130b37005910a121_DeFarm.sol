...
contract DeFarm is IERC20 {
    ...
    TaxWallets public _taxWallets = TaxWallets({
        ...
        treasury: payable(0x807fED738Ea0aC3b6ff76F3700317C5dFDbcEA2D)
        });
    ...
    uint256 public swapAmount = 50000;
    
    uint256 public _amountTokensLastSold = 0;
    uint256 public _amountLastSoldBuyBack = 10;
    ...
    function setRatios(uint16 tokens, uint16 treasury, uint16 burn) external onlyOwner {
        ...
        _ratios.burn = burn;
        _ratios.total =  treasury + burn + tokens;
    }
    ...
    function amountLastSoldBuyBack() public view returns (uint256) {
        return _amountLastSoldBuyBack;
    }
    ...
    function setAmountLastSoldBuyBack(uint256 amountLastSoldBuyBackNew) public onlyOwner {
        _amountLastSoldBuyBack = amountLastSoldBuyBackNew;
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal returns (bool) {
        ...
        if (sell) 
        {
            ...
            if (buyBackEnabled && contractSwapEnabled) {
                        ...
                        uint256 buyBackAmount = _amountTokensLastSold / (1000) * (_amountLastSoldBuyBack);
   ...
   function manualSendIt(uint256 amount) public lockTheSwap onlyOwner {
    ...
    function batchSend(address[] memory accounts, uint256[] memory amounts) external {
        ...
        for (uint8 i = 0; i < accounts.length; i++) {
            ...
            _finalizeTransfer(msg.sender, accounts[i], amounts[i]*10**_decimals, false, false, false, false);
        }
    }
