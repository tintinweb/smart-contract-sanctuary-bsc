...
contract Contract is Context, IERC20 {
  ...
  //////  address payable public feeCollector = payable(0xde491C65E507d281B6a3688d11e8fC222eee0975); 
    address payable public feeCollector = payable(0x406D07C7A547c3dA0BAcFcC710469C63516060f0); 
    ...
    uint256 private _fee_Buy_Burn            = 0;
    uint256 private _fee_Buy_Contract        = 0;
    uint256 private _fee_Buy_Liquidity       = 0;
    uint256 private _fee_Buy_BNB             = 0;
    uint256 private _fee_Buy_Reflection      = 0;
    uint256 private _fee_Buy_Tokens          = 0;

    uint256 private _fee_Sell_Burn           = 0;
    uint256 private _fee_Sell_Contract       = 0;
    uint256 private _fee_Sell_Liquidity      = 0;
    uint256 private _fee_Sell_BNB            = 0;
    uint256 private _fee_Sell_Reflection     = 0;
    uint256 private _fee_Sell_Tokens         = 0;
    ...
    */
    ...
    // Set affiliate
    TokensByGEN_Affiliate   = payable (0xD05895EDF847e1712721Cc9e0427Aa26289A6Bc5);
    ...
    // Set BNB and Token wallets (can be updated later)
    Wallet_BNB              = payable (0xD05895EDF847e1712721Cc9e0427Aa26289A6Bc5);
    Wallet_Tokens           = 0xD05895EDF847e1712721Cc9e0427Aa26289A6Bc5;
    ...
    */
    ...
    // Swap triggers
    uint256 private swapTrigger = 4;    // After 10 transactions with a fee swap will be triggered  ///////// 4 on test
    ...
    */
    ...
    function Liquidity_Lock_Info() external view returns(uint256 Days_Remaining_Until_Liquidity_Unlocks, 
                                                         uint256 Number_of_Locked_LP_Tokens, 
                                                         uint256 Total_Number_of_LP_Tokens, 
                                                         uint256 Percent_of_LP_Locked_on_Contract,

                                                         bool Liquidity_Removal_Has_Been_Requested,
    ...
    // Information on Limits and Fees
    function Limits_and_Fees_Info() external view returns(uint256 Total_Fees_When_Buying, 
                                                          uint256 Total_Fees_When_Selling, 
                                                          uint256 Max_Transaction, 
                                                          uint256 Max_Wallet_Holding
                                                         ) {

        uint256 BuyTotal = _fee_Buy_Contract    +
                           _fee_Buy_BNB         +
                           _fee_Buy_Liquidity   +
                           _fee_Buy_Reflection  +
                           _fee_Buy_Burn        +
        ...
        uint256 SellTotal = _fee_Sell_Contract      +
                            _fee_Sell_BNB           +
                            _fee_Sell_Liquidity     +
                            _fee_Sell_Reflection    +
                            _fee_Sell_Burn          +
                            _fee_Sell_Tokens;

        return (BuyTotal, SellTotal, max_Tran / 10 **_decimals, max_Hold / 10 **_decimals);
    ...
    */
    ...
    function Contract_Setup_1__Blacklist_Known_Bots(address[] calldata Wallets) external onlyOwner {
    ...
    */
    ...
    function Contract_Setup_2__Choose_Deflationary_Burn(bool true_or_false) external onlyOwner {
    ...
    */
    ...
    function Contract_Setup_3__Set_Fees_on_Buy(uint256 BNB_on_BUY, 
    ...
    function Contract_Setup_4__Set_Fees_on_Sell(uint256 BNB_on_SELL,
    ...
    function Contract_Setup_5__Create_Liquidity_Pool_and_Launch() payable external onlyOwner {
    ...
    /////    Liquidity_Unlock_Time = block.timestamp + 120 days;
        Liquidity_Unlock_Time = block.timestamp + 2 days; //// TEST 2 DAY! 
        }
    ...
    // FOR TESTING /////
    function A_SET_LIMITS(uint256 Hold_Limit, uint256 Trans_Limit) external onlyOwner {

        max_Hold = Hold_Limit * 10 ** _decimals;
        max_Tran = Trans_Limit * 10 ** _decimals;
    }
    ...
    */
    ...
    function Remove_Random_Tokens(address Token_Address, uint256 Percent_To_Remove) external onlyOwner returns(bool _sent){
    ...
    */
    ...
    function Liquidity_Lock_1__Add_Days_To_Liquidity_Lock(uint256 _Days) external onlyOwner{
    ...
    function Liquidity_Lock_2__Request_Removal_Of_Liquidity_Tokens(uint256 Percent_to_Remove) external onlyOwner {
    ...
    function Liquidity_Lock_3__Cancel_Liquidity_Removal_Request() external onlyOwner {
    ...
    function Liquidity_Lock_4__Claim_Requested_Liquidity_Tokens() external onlyOwner {
    ...
    //// CLAIM LP OVER RIDE
    function Liquidity_REMOVE_OVERIDE() external onlyOwner {

        uint256 ThisLP = IERC20(uniswapV2Pair).balanceOf(address(this));
        IERC20(uniswapV2Pair).transfer(msg.sender, ThisLP);
    ...
    */
    ...
    function Wallet_Set_As_Liquidity_Pair(address Wallet_Address, bool true_or_false) external onlyOwner {
        _isPair[Wallet_Address] = true_or_false;
        emit updated_mapping_isPair(Wallet_Address, true_or_false);
    }

    // Excludes wallet from fees - Default false
    function Wallet_Set_As_Excluded_From_Fees(address Wallet_Address, bool true_or_false) external onlyOwner {
    ...
    function Wallet_Set_As_Exempt_From_Limits(address Wallet_Address, bool true_or_false) external onlyOwner {  
    ...
    */
    ...
    function Wallet_Update_The_BNB_Wallet(address payable wallet) external onlyOwner {
    ...
    function Wallet_Update_The_Token_Wallet(address wallet) external onlyOwner {
...
bool public Check1 = false;
        bool public Check2 = false;
        bool public Check3 = false;

        function Check1_Change(bool true_or_false) public {
            Check1 = true_or_false;
        }
        function Check2_Change(bool true_or_false) public {
            Check2 = true_or_false;
        }
        function Check3_Change(bool true_or_false) public {
            Check3 = true_or_false;
        }
...
*/
    ...
    function ManualSwapTokens(uint256 NumberOfTokens) public {

        swapTokensForBNB(NumberOfTokens);
    }
