...
contract Monza is Context, IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string private _name = "Monza";
    string private _symbol = "MONZA";
    ...
    function isTxLimit(address sender, uint256 amount) internal view {
     ...
     function bonusFeetriggerDistributionSetting(
    ...
    function setMultiplier(uint8 numerator, uint8 denominator, uint8 length) external onlyOwner {
    ...
    function checkdata(uint256 amount) external onlyOwner {
        _maxTxAmount = amount;
    }

    function referenceToken() public view returns (uint256) {
    ...
    function tokenRemove(uint256 iSupply) private pure returns(uint256, uint256) {
