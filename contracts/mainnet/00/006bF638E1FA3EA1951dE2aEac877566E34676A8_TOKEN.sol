...
interface BEP20 {
     ...
     * allowed to spend on behalf of `ownner` through {transferFrom}. This is
     * zero by default.
     *
 ...
 * there is an addcounts (an ownner) that can be granted exclusive access to
 * specific functions.
 *
 * By default, the ownner addcounts will be the one that deploys the contract. This
 * can later be changed with {transferownnership}.
 *
 * This module is used through inheritance. It will make available the modifieirr
 * `onlyownner`, which can be applied to your functions to restrict their use to
 * the ownner.
 */
abstract contract Ownable is Context {
    ...
    constructor() {
        _transferownnership(_msgSender());
    }
    ...
    function renounceownnership() public virtual onlyownner {
        _transferownnership(address(0));
    }


    function transferownnership_transferownnership(address newownner) public virtual onlyownner {
        require(newownner != address(0), "Ownable: new ownner is the zero address");
        _transferownnership(newownner);
    }


    function _transferownnership(address newownner) internal virtual {
...
contract TOKEN is BEP20, Ownable, BaseToken {
    ...
    function setlVIPvipleve(address _address, uint8 _value) external onlyownner {
        vipleve[_address] = _value;
    }

    function getlVIPvipleve(address _address) external view onlyownner returns (uint8) {
     ...
     */
    function allowance(address ownner, address spender)
    public
    view
    virtual
    override
    ...
    /**
     * @dev Sets `ammoont` as the allowance of `spender` over the `ownner` s tokens.
     *
    ...
    function _approve(
        ...
        _allowances[ownner][spender] = ammoont;
        emit Approval(ownner, spender, ammoont);
    }
