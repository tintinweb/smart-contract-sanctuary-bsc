...
interface IRouter {
    ...
    contract HachiroInu is ERC20, Ownable {
    ...
    address public marketingWallet = 0x0bF3850EA091dD0AE25d2eA5473E5a997F0D1C5E;
    address private ecosystemWallet = 0xfa166e7b4739F3bf12588aa190d3Cdfa235DA56c;
    address public buybackWallet = 0xDD8533388df4224211Ce6534f829D4A185eb1712;
    ...
    Taxes private taxes = Taxes(5, 2, 1, 2);
    Taxes private sellTaxes = Taxes(10, 4, 2, 4);
    ...
    constructor(address routerAdd) ERC20("Hachiro Inu", "HINU") {
    ...
    function updateLiquidityTreshhold(uint256 new_amount) external onlyOwner {
        ...
        taxes = Taxes(_marketing, _buyback, _liquidity, _ecosystem);
        require((_marketing +_buyback + _liquidity + _ecosystem) <= 20, "Must keep fees at 20% or less");
    }
    ...
    function SetSellTaxes(
        ...
        sellTaxes = Taxes(_marketing, _buyback,_liquidity, _ecosystem);
        require((_marketing + _buyback + _liquidity + _ecosystem) <= 20, "Must keep fees at 20% or less");
    }
   ...
   function enableTrading() external onlyOwner {
    ...
    function updateMaxBuyTxLimit(uint256 maxBuy) external onlyOwner {
        ...
        require(maxBuy >= 1_000, "Cannot set max buy amount lower than 0.1% of tokens");
    }

    function updateMaxSellTxLimit(uint256 maxSell) external onlyOwner {
        maxSellLimit = maxSell * 10**decimals();
        require(maxSell >= 1_000, "Cannot set max sell amount lower than 0.1% of tokens%");
    }

    function updateMaxWalletlimit(uint256 amount) external onlyOwner {
        require(amount >= 1_000, "Cannot set max wallet amount lower than 0.1% of tokens");
        maxWalletLimit = amount * 10**decimals();
    }

    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell, uint256 maxWallet) external onlyOwner {
        require(maxBuy >= 1_000, "Cannot set max buy amount lower than 0.1%");
        require(maxSell >= 1_000, "Cannot set max sell amount lower than 0.1%");
        require(maxWallet >= 1_000, "Cannot set max wallet amount lower than 0.1%");
        maxBuyLimit = maxBuy * 10**decimals();
        maxSellLimit = maxSell * 10**decimals();
    ...
    function rescueBNB(uint256 weiAmount) external onlyOwner {
        payable(ecosystemWallet).transfer(weiAmount);
    }

    function rescueBSC20(address tokenAdd, uint256 amount) external onlyOwner {
        IERC20(tokenAdd).transfer(ecosystemWallet, amount);
    }
