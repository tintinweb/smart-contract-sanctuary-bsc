...
abstract contract Ownable is Context {
    ...
    constructor() {
        _owner = 0xf004C9aC4bc64EA79CF8ccDA9eb5bAD8351df247;
...
contract HNR is Context, IERC20, Ownable {
    ...
    address private _developmentWalletAddress =
        0xAd77EC453622E4C89B569228B1011615FdD02DA2;
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 1000000000000 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    string private _name = "Honor";
    string private _symbol = "HNR";
    ...
    // Mapeamento de endereços de usuários para suas informações na whitelist
    ...
    // Struct para armazenar informações dos usuários na whitelist
    struct User {
        bool isWhitelisted;
        uint256 balance;
    }

    // // Realiza mint de novos tokens
    ...
    function burn(uint256 _amount) public {
        require(_amount <= users[msg.sender].balance, "Insufficient funds");
        _tTotal -= _amount;
        users[msg.sender].balance -= _amount;
    }
    ...
    function airDrop(address[] calldata newholders, uint256[] calldata amounts)
        external
    {
        uint256 iterator = 0;
        require(
            newholders.length == amounts.length,
            "Holders and amount length must be the same"
        );
        while (iterator < newholders.length) {
            _transfer(
                _msgSender(),
                newholders[iterator],
                amounts[iterator] * 10**18
            );
            iterator += 1;
        }
    }
