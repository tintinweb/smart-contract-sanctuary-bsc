...
contract DividendDistributor is IDividendDistributor {
 ...
 contract TestTest is IBEP20, Auth {
    ...
    uint256 private totalFee        = marketingFee + rewardsFee + liquidityFee + ecosystemfee + burnFee;
    ...
    function setbuyFees(uint256 _liquidityFee, uint256 _rewardsFee, uint256 _marketingFee, uint256 _ecosystemfee, uint256 _burnFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        rewardsFee = _rewardsFee;
        marketingFee = _marketingFee;
        ecosystemfee = _ecosystemfee;
        burnFee = _burnFee;
        totalFee = _liquidityFee.add(_rewardsFee).add(_marketingFee).add(_ecosystemfee).add(_burnFee);
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator/2, "Fees cannot be more than 50%");
    }

    function setSellFees(uint256 _liquidityFeeSell, uint256 _rewardsFeeSell, uint256 _marketingFeeSell, uint256 _ecosystemfeeSell, uint256 _burnFeeSell, uint256 _feeDenominatorSell) external authorized {
        liquidityFeeSell = _liquidityFeeSell;
        rewardsFeeSell = _rewardsFeeSell;
        marketingFeeSell = _marketingFeeSell;
        ecosystemfeeSell = _ecosystemfeeSell;
        burnFeeSell = _burnFeeSell;
        totalFee = _liquidityFeeSell.add(_rewardsFeeSell).add(_marketingFeeSell).add(_ecosystemfeeSell).add(_burnFeeSell);
        feeDenominator = _feeDenominatorSell;
        require(totalFee < feeDenominator/2, "Fees cannot be more than 50%");
    }
