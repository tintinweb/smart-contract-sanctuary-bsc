...
contract MindMusicFoundersSeries is ERC721Enumerable, Ownable {
    ...
    IBEP20 public externalToken;
    uint256 public externalMinAmount = 20000000000;  // set external token amount
    uint256 public MINT_PRICE = 0.3 ether; // Public mint price
    uint256 public MAX_TOTAL_SUPPLY = 1000; // The maximum total supply of tokens
    bool public _enableMint;
    ...
    uint256 public mint_num_per_action;
    uint256 public mint_num_per_wallet;

    uint256 public increase = 0;

    uint256 public supply1;
    uint256 public supply2;
    uint256 public supply3;
    uint256 public supply4;
    uint256 public supply5;
    ...
    mapping(address => uint256) private NFTHolderReward;
    ...
    constructor(string memory _baseUri, address _token) ERC721("Mind Music Founders Series", "Mind Music Founders Series") {
        baseTokenURI = _baseUri;
        externalToken = IBEP20(_token);
        rewardToken = IBEP20(0xCB0B74476d72BBBC77f14724a859b2e960Ca1803);
        mint_num_per_action = 5;
        mint_num_per_wallet = 1000;
        _enableMint = false;
        supply1 = 400;
        supply2 = 700;
        supply3 = 1000;
    ...
    function setPriceRange(uint256 step1, uint256 step2, uint256 step3, uint256 step4, uint256 step5) public onlyOwner {
        supply1 = step1;
        supply2 = step2;
        supply3 = step3;
        supply4 = step4;
    ...
    function setMintPrice(uint256 _price1, uint256 _price2, uint256 _price3, uint256 _price4, uint256 _price5, uint256 _price6) public onlyOwner {
        price1 = _price1;
        price2 = _price2;
        price3 = _price3;
        price4 = _price4;
    ...
    function mint(uint256 _amount) external payable {
        ...
        if (flag == false && externalTokenNeeded == true) {
        ...
        if (supply5 >= supply && supply > supply4) {
            MINT_PRICE = price5;
        }
        if (supply4 >= supply && supply > supply3) {
            MINT_PRICE = price4;
        }
        if (supply3 >= supply && supply > supply2) {
            MINT_PRICE = price3;
        }
        if (supply2 >= supply && supply > supply1) {
            MINT_PRICE = price2;
        }
        if (supply1 >= supply) {
            MINT_PRICE = price1;
        ...
        require( MINT_PRICE * _amount == msg.value, "Price must be mint price for each NFT");

        uint256 flagNum = 0;
        uint256 id;
        uint256 i = 0;
        do  {
            id = uint256(keccak256(abi.encodePacked(block.timestamp, i))) % MAX_TOTAL_SUPPLY;
            id = id +  1;
            if (isMint[id] == false) {
    ...
    // Minting function to provide successful migration for previous NFT owners of the old contract
    function mintForPrevOwner(address _rAddress, uint256 _tId) external onlyOwner {

      _safeMint(_rAddress, _tId);
      isMint[_tId] = true;

      addressMintCount[_rAddress] = addressMintCount[_rAddress] + 1;
    }
    ...
    function depositReward(uint256 _amount) external onlyOwner {
      uint256 length = 1000;
      uint256 perAmount = _amount / length;
      for (uint256 i=0; i<length; i++) {
          address cowner = ownerOf(i);
        NFTHolderReward[cowner] = NFTHolderReward[cowner] + perAmount;
      }
    }

    function rewardTransfer(IBEP20 token, address recipient, uint amount) private {bool sent = token.transfer(recipient, amount);
        require(sent, "Token transfer failed");
    }

    function claimReward() external {
      if(NFTHolderReward[msg.sender]>0){
        bool sent = rewardToken.transfer(msg.sender, NFTHolderReward[msg.sender]);
            require(sent, "Token transfer failed");
      }
    }
