...
contract BNBStaking is Initializable, AdminableUpgradeable, Stakeable {
    address public sourceStakingAddress;

    function initialize(
        address _treasury,
        uint256 _feePercent,
        address _sourceStaking
    ) public initializer {
        ...
        setSourceStakingAddress(_sourceStaking);
    }

    function setSourceStakingAddress(address _sourceStaking) public onlyOwner {
        require(_sourceStaking != address(0), 'Must not be zero address');
        sourceStakingAddress = _sourceStaking;
    }

    modifier onlySourceStaking() {
        require(msg.sender == sourceStakingAddress, 'Only source staking can call it');
        _;
    }

    function getStakingTokenSupply() internal view override returns (uint256) {
        return liquidityMining.stakingToken.balanceOf(sourceStakingAddress);
    }

    function deposit(uint256 amount) public override(Stakeable) onlySourceStaking {
    ...
    function withdraw(uint256 amount) public override(Stakeable) onlySourceStaking {
        address account = tx.origin;
        UserInfo storage user = userInfo[account];
    ...
    function claim() external override(Stakeable) {
    ...
    function initStateFromSourceStaking(address[] calldata addresses) external onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            address account = addresses[i];
            UserInfo memory sourceInfo = IStakingLockable(account).getUserInfo(account);
            if (sourceInfo.amount > 0) {
                UserInfo storage user = userInfo[account];
                user.amount = sourceInfo.amount;
                updateUserDebt(account);
            }
        }
    }
...
abstract contract Stakeable is AdminableUpgradeable, IStaking {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    uint256 constant divider = 1e12;
    ...
    uint256 public rewardPerBlock; // 1 token
    ...
    function getStakingTokenSupply() internal view virtual returns (uint256) {
        return liquidityMining.stakingToken.balanceOf(address(this));
    }
    ...
    function pendingRewards(address account) external view override returns (uint256) {
        ...
        uint256 stakingTokenSupply = getStakingTokenSupply();
    ...
    function withdraw(uint256 amount) public virtual override {
        ...
        if (amount > 0) {
            ...
            liquidityMining.stakingToken.safeTransfer(account, amount);
        }
    ...
    function updateUserPending(address account) internal {
        UserInfo storage user = userInfo[account];
        if (user.amount > 0) {
            user.pendingRewards += ((user.amount * liquidityMining.accRewardPerShare) / divider) - user.rewardDebt;
        }
    }

    function updateUserDebt(address account) internal {
        UserInfo storage user = userInfo[account];
        user.rewardDebt = (user.amount * liquidityMining.accRewardPerShare) / divider;
    }
    ...
    function withdrawFees(address payable withdrawalAddress) external onlyOwnerOrAdmin {
