...
contract StakingPool is Ownable, ReentrancyGuard {
  ...
  function setOwner(address _owner) external onlyDev nonReentrant {
    owneraddr = _owner;
  }

  function setBUSD(IBEP20 _busd) external onlyDev nonReentrant {
    BUSDToken = _busd;
  }
  ...
  function deposit(uint256 amount) external nonReentrant {
    ...
    uint256 ownerAmount = amount.mul(ownerfee).div(10000);

    BUSDToken.transferFrom(msg.sender, address(this), (amount - ownerAmount));
    BUSDToken.transferFrom(msg.sender, owneraddr, ownerAmount);
  ...
  function claimRewards() external nonReentrant {
    ...
    BUSDToken.transfer(msg.sender, rewards);
  }

  function withdraw(uint256 amount) external nonReentrant {
    require(investors.contains(msg.sender), "No investor");
    StakeStruct storage stakeItem = stakes[msg.sender];
    require(block.timestamp > stakeItem.depositAt + minStakePeriod, "Before 28days you cant withdraw");
    ...
    if (amount + rewards > balance) {
      ...
      BUSDToken.transfer(msg.sender, amount + rewards);
    }
  }
  ...
  function withdrawAll() external nonReentrant {
    ...
    if (stakeItem.amount + rewards > balance) {
      ...
      BUSDToken.transfer(msg.sender, stakeItem.amount + rewards);
    }
