...
contract BaseV1Pair {
    ...
    bytes32 internal constant PERMIT_TYPEHASH = 0x0f2e5ae39140b1e2c1ff1e7f2bca5fba8720ff714d05d140c1ba4814d0354b29;
    ...
    constructor() {
        ...
        if (_stable) {
            name = string(abi.encodePacked("USDFI.com StableV1 AMM - ", erc20(_token0).symbol(), "/", erc20(_token1).symbol()));
            symbol = string(abi.encodePacked("sAMM-", erc20(_token0).symbol(), "/", erc20(_token1).symbol()));
            fee = IBaseV1Factory(factory).baseStableFee();
        } else {
            name = string(abi.encodePacked("USDFI.com VolatileV1 AMM - ", erc20(_token0).symbol(), "/", erc20(_token1).symbol()));
            symbol = string(abi.encodePacked("vAMM-", erc20(_token0).symbol(), "/", erc20(_token1).symbol()));
            fee = IBaseV1Factory(factory).baseVariableFee();
        }
    ...
    function usdfiMaker() external returns (address) {
        return IBaseV1Factory(factory).usdfiMaker();
    }
    ...
    // Allow to recover token sent here by mistake
    function drainWrongToken(address token, address to) external {
        require(msg.sender == IBaseV1Factory(factory).admin(),"Pair: only factory's owner");
        require(token != token0 && token != token1, "Pair: invalid token");
