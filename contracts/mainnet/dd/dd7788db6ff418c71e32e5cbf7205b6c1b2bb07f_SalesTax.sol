...
/**
Sales Tax: $SALE
- 10% Tax on Buys, 30% Tax on Sells

Tokenomics:

Buys:
5% of each buy goes to existing holders via Reflections
5% of each buy goes into auto-MCC buyback.

Sales:
15% of each sell goes to existing holders via Reflections
15% of each sell goes into auto-MCC buyback.
*/
...
contract SalesTax is Context, IERC20, Ownable {
  ...
  string private _name = 'Sales Tax';
  ...
  address payable public _MCCContractAddress;
  ...
  constructor(
    address payable MCCContract,
    address payable marketingWalletAddress
  ) {
    _MCCContractAddress = MCCContract;
  ...
  function sendETHToTeam(uint256 amount) private {
    _MCCContractAddress.call{ value: amount.div(2) }('');
  ...
  function _setMCCContract(address payable MCCContract) external onlyOwner {
    _MCCContractAddress = MCCContract;
  }
