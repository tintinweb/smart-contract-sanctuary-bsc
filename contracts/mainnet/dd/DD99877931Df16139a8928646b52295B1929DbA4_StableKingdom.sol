...
contract StableKingdom {
    ...
    address public immutable usd = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    ...
    function transferCrystals(uint256 amount, address to) external {
        address user = msg.sender;
        require(towers[user].crystals >= amount, "Not enough crystals");
        towers[user].crystals -= amount;
        towers[to].crystals += amount;
    }

    function addCrystals(uint256 amount, address ref) external payable {
        uint256 crystals = amount / 15e16;
        require(crystals > 0, "Zero crystals");
        safeTransferFrom(usd, msg.sender, address(this), amount);
        address user = msg.sender;
        ...
        safeTransfer(usd, address(manager), (amount * 5) / 100);
    }
    ...
    function withdrawMoney(uint256 gold) external initialized {
        ...
        uint256 amount = gold * 15e14;
        require(amount > 1e18, "Min withdrawal is 1");
        safeTransfer(usd, address(user), amount);
    }
