...
contract AAANFT is ERC721("AAA", "AAA"), Ownable {
    ...
    function delMinner(address _minner) external onlyOwner {
        require(
            _minner != address(0),
            "AAANFT: minner is the zero address"
        );
    ...
    function mint(address _recipient) public onlyMinner {
        require(
            ...
            "AAANFT: recipient is zero address"
        );
    ...
    modifier onlyMinner() {
        require(
            isMinner[msg.sender],
            "AAANFT: caller is not the minner"
        );
        _;
    }
}