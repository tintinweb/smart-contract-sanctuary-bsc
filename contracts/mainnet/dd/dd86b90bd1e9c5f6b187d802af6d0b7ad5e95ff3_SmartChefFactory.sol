...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    // Smex profile
    ISmexProfile public immutable smexProfile;

    // Smex Profile is requested
    bool public smexProfileIsRequested;

    // Smex Profile points threshold
    uint256 public smexProfileThresholdPoints;
    ...
    /**
     * @notice Constructor
     * @param _smexProfile: smex Profile address
     * @param _smexProfileIsRequested: smex Profile is requested
     * @param _smexProfileThresholdPoints: smex Profile need threshold points
     */
    constructor(
        address _smexProfile,
        bool _smexProfileIsRequested,
        uint256 _smexProfileThresholdPoints
    ) {
        SMART_CHEF_FACTORY = msg.sender;

        // Call to verify the address is correct
        ISmexProfile(_smexProfile).getTeamProfile(1);
        smexProfile = ISmexProfile(_smexProfile);

        // if smexProfile is requested
        smexProfileIsRequested = _smexProfileIsRequested;

        // smexProfile threshold points when profile & points are requested
        smexProfileThresholdPoints = _smexProfileThresholdPoints;
    }
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        require(
            (!smexProfileIsRequested && smexProfileThresholdPoints == 0) ||
            smexProfile.getUserStatus(msg.sender),
            "Deposit: Must have an active profile"
        );

        uint256 numberUserPoints = 0;

        if (smexProfileThresholdPoints > 0) {
            (, numberUserPoints, , , , ) = smexProfile.getUserProfile(msg.sender);
        }

        require(
            smexProfileThresholdPoints == 0 || numberUserPoints >= smexProfileThresholdPoints,
    ...
    function updateProfileAndThresholdPointsRequirement(bool _isRequested, uint256 _thresholdPoints) external onlyOwner {
        ...
        smexProfileIsRequested = _isRequested;
        smexProfileThresholdPoints = _thresholdPoints;
...
contract SmartChefFactory is Ownable {
    ...
    function deployPool(
        ...
        bytecode = abi.encodePacked(
            bytecode,
            abi.encode(_smexProfile, _smexProfileIsRequested, _smexProfileThresholdPoints)
        );
