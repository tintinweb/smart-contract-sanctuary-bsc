...
contract StakingRouterRecoverableNMXAmount {
    using ABDKMath64x64 for int128;

    address constant NMX = 0xd32d01A43c869EdcD1117C640fBDcfCFD97d9d65;

    function getRecoverableAmount(address routerAddress) external view returns (uint256) {        
        ConstantComplexityStakingRouter router = ConstantComplexityStakingRouter(routerAddress);

        uint256 pendingSupply = 0;
        address[] memory _activeServices = router.getActiveServices();
        uint256 _totalSupply = router.totalSupply();
        for(uint256 i = 0; i < _activeServices.length; i++) {
            (uint256 processedSupply, int128 share) = router.supplyStates(_activeServices[i]);
            pendingSupply += share.mulu(_totalSupply - processedSupply);
        }

        uint256 balance = IERC20(NMX).balanceOf(routerAddress);
        if (balance <= pendingSupply) {
            return 0;
        }
