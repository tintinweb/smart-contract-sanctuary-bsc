...
contract TearsFaucet is Ownable, ReentrancyGuard {
    ...
    function redeem(address token, uint256 amount) external nonReentrant {
        ...
        uint256 toDistribute = getOutput(token, amount);
    ...
    function getOutput(address token, uint256 amount) public view returns(uint256) {
        return amount * tears.totalSupply() / (IERC20(token).totalSupply() * 100000);
    }
