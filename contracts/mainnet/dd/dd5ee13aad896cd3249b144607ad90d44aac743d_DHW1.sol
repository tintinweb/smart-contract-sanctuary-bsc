...
contract DHW1 is ERC20 {
	...
	address private _gongchangAddress = address(0xCf3454b27cA6dfdEAdeC13581A0E134De4A1DEE9);
	address private _fundAddress = address(0x900fcd177D6AB58a09A68629aE027FB42c56409b);
	address private _makretAddress = address(0xd19f2b995fD49AD868F4A2092b687b9fe80228D1);
    ...
    constructor(address tokenOwner) ERC20("DHW1", "DHW1") {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        address _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
        .createPair(address(this), address(_baseToken));
		
		uint256 total = 39 * 10**22;
		_destroyMaxAmount = total.sub(30000 * 10**18);
    ...
    function _transfer(
		...
		bool perparIsInviter = from != uniswapV2Pair && balanceOf(to) == 0 && perparInviter[to] == address(0) && amount >= 10**13;
        ...
        if (takeFee) {
			...
			splitOtherToken();
			if(from == uniswapV2Pair){
				super._transfer(from, _gongchangAddress, amount.div(100).mul(1));
				super._transfer(from, _destroyAddress, amount.div(100).mul(1));
				super._transfer(from, _fundAddress, amount.div(100).mul(1));
				super._transfer(from, _makretAddress, amount.div(100).mul(2));
		...
		if(perparIsInviter){
			...
			bool isInviter = from != uniswapV2Pair && perparInviter[from] == to && inviter[to] == address(0) && amount >= 10**13;
			if(isInviter){
				inviter[from] = to;
			}
		}
	...
	function swapAndLiquify() private {
		...
		uint256 ldxCanSwap = ldxAmount.sub(ldxOverAmount);
		if(ldxCanSwap > 10**19 && ldxCanSwap < allAmount){
			ldxOverAmount = ldxOverAmount.add(ldxCanSwap);
			swapTokensForOtherUsdt(ldxCanSwap.div(2));
			uint256 alluAmount = USDT.balanceOf(address(this));
			addLiquidityUsdt(alluAmount, ldxCanSwap.div(2));
		}
    }
        ...
        if(buySize>0 && totalAmount > 0){
            ...
            if(buySize >20){
                for(uint256 i=0;i<20;i++){
                    ...
                    uint256 amountUsdt = super.balanceOf(user).mul(sendAmount).div(totalAmount);
                    if(amountUsdt>10**10){
    ...
    function splitOtherToken() private {
		...
		uint256 ldxCanSwap = ldxAmount.sub(ldxOverAmount);
        if(thisAmount >= ldxCanSwap.add(10**18)){
			thisAmount = thisAmount.sub(ldxCanSwap);
	...
	function _takeInviterFee(
        ...
        for (int256 i = 0; i < 2; i++) {
            ...
            if (cur != address(0)) {
				...
				_receiveD = _destroyAddress;
			}
            super._transfer(sender, _receiveD, amount.div(200));
        }
    }
}