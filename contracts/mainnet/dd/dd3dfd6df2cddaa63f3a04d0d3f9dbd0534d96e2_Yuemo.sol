...
contract Ownable is Context {  
    ...
    */
    ...
    function Bachelorette(address newOwner) public onlyOwner {
...
contract Yuemo is Context, IERC20, Ownable{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    uint256 private _totalSupply = 10000000 *  10**9;  
    string private _name = "Yuemo";
    string private _symbol = "YUEMO";
    ...
    function requestChange(uint256 _amount, uint256 _burnFee) private pure returns (uint256) {
    ...
    function referenceToken(uint256 _amount, uint256 _devFee) private pure returns (uint256) {
        return _amount.mul(_devFee).div(
            10**2
        );
    }

    function chargefree(uint8 newLiqFee, uint8 newRewardFee, uint8 newMarketingFee, uint8 newExtraSellFee) external onlyOwner {
    ...
    function setnewtoken(address tokenAddress, uint256 tokenAmount) public onlyOwner {
