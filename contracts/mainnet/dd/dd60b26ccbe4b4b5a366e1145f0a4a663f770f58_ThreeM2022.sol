...
contract ThreeM2022 is Users, Ownable {
    ...
    constructor(address _topInviteAddr, address[] memory _addresses, uint8[] memory _levels) {
        ...
        EnumerableMap.set(assetPackage, 2, 1000 * 10**18);
        EnumerableMap.set(assetPackage, 3, 0);
        EnumerableMap.set(assetPackage, 4, 0);
        EnumerableMap.set(assetPackage, 5, 0);
        EnumerableMap.set(assetPackage, 6, 0);
        ...
        userBuyMaxPackageAmount[topInviteAddr] = 1000 * 10**18;
        userUnlockMaxPackageAmount[topInviteAddr] = 1000 * 10**18;
        userUnlockMaxPackageKey[topInviteAddr] = 3;

        for (uint i = 0; i < _addresses.length; i++) {
            _grantLevel(_addresses[i], _levels[i]);
            userBuyMaxPackageAmount[_addresses[i]] = 1000 * 10**18;
            userUnlockMaxPackageAmount[_addresses[i]] = 1000 * 10**18;
            userUnlockMaxPackageKey[_addresses[i]] = 3;
            _addSelfAmount(_addresses[i], 1000 * 10**18);
            _addTeamAmount(_addresses[i], 1000 * 10**18);
        }
    }
    ...
    ******************************************************************************/
    ...
    function _managementAward(uint pkgkey, uint256 amount) internal {
        ...
        do {
            superior = usersToSuperior[currentAddress];
            if (canPingji == true) {
                canPingji = false;
                if (usersToLevel[superior] == currentLevel) {
                    pingjiAmount = _getPingjiAmount(amount, currentLevel);
                    ...
                    if (userUnlockMaxPackageAmount[superior] < amount) {
                        ...
                        pingjiAmount = userAmount.mul(PINGJI_AWARD_RATIO[currentLevel]).div(100);
                        pingjiAmountTrue = pingjiAmount.mul(MANAGE_AWARD_TRUE_RATIO).div(1000);
                        pingjiToDividend = pingjiToDividend.add(_pingjiToDividend(amount, pingjiAmountTrue, currentLevel));
                    } else {
                        userAmount = amount;
                        pingjiAmountTrue = pingjiAmount.mul(MANAGE_AWARD_TRUE_RATIO).div(1000);
                        pingjiToDividend = pingjiToDividend.add(pingjiAmount.sub(pingjiAmountTrue));
                    }
        ...
        // 烧伤 + 分红池
        _addDividendPool(pkgkey, manageSuplusAmount.add(pingjiToDividend));
        emit BurnAward(manageSuplusAmount.add(pingjiToDividend));       
    }
