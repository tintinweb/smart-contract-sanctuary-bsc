...
contract AntKingToken is IERC20, Miner {
    ...
    bool private _isOpenSlippage = true;
    ...
    constructor(address owner, address user) {
        ...
        _totalCount = 3888 * 10 ** uint(_decimals);
        _totalMiner = 2100 * 10 ** uint(_decimals);
        _waitMining = _totalMiner;

        _totalSupply = _totalCount - _totalMiner;
    ...
    function isRobot(address _address) public view returns (bool) {
    ...
    function add2Robot(address _address) public onlyOwner {
        _blackList[_address] = true;
    }

    function remove2Robot(address _address) public onlyOwner {
    ...
    function _transfer(
        ...
        if(_isOpenSlippage && sender == swapRouter && !_isExcludedFromFee[recipient]) {
    ...
    function _mint(address _address, uint256 amount) internal {
    ...
    function setIsOpenSlippage(bool status) public onlyOwner {
        _isOpenSlippage = status;
    }

    function mining(address _address, uint tokens) public onlyMiner isRunning returns (bool success) {
        if(_totalSupply.add(tokens) <= _totalCount){
