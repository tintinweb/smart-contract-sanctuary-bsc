 ...
 * is concerned).
 *
 * This contract is only required for int   ermediate, library-like contracts.
 */
...
contract PANCAKE is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    LULTokenDividendTracker public dividendTracker;
    ...
    constructor() ERC20("ELON TEST", "TEST") {
        dividendTracker = new LULTokenDividendTracker();

        marketingWallet = 0x4025342f8079D0F8E8FB00aD978E650Df4a91804;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "LUL: The dividend tracker already has that address"
        );

        LULTokenDividendTracker newDividendTracker = LULTokenDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "LUL: The new dividend tracker must be owned by the LUL token contract"
        );
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "LUL: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "LUL: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "LUL: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "LUL: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "LUL: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "LUL: Cannot update gasForProcessing to same value"
        );
    ...
    function _transfer(
        ...
        if (tradingIsEnabled && automatedMarketMakerPairs[to]) {
            ...
            if (!swapping && canSwap) {
                ...
                uint256 bnbSwap =( buyMarketingFee
                );
...
contract LULTokenDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()
        DividendPayingToken("LUL_Dividend_Tracker", "LUL_Dividend_Tracker")
    {
        claimWait = 3600;
    ...
    function _transfer(
        ...
        require(false, "LUL_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "LUL_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main LUL contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "LUL_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "LUL_Dividend_Tracker: Cannot update claimWait to same value"
        );
