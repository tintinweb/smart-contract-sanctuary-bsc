...
contract Workers is ERC721Enumerable, Ownable, WorkerFactory, PriceConsumerWTN {
    ...
    uint256 currentGenQuantity  = 5000;
    ...
    function value() public view returns (uint256) {
        ...
        uint256 amountBNBReward = (10 * 1 ether) / BNB_USD_Price; 
   ...
   function canPayCreate() internal view returns (bool) {
    ...
    function evolveWorker(uint256 tokenId) external {
        ...
        _createWorker(v1.class_id, v1.rarity, v1.gender_id, _nftId, 0);
        _nftId++;
    }

    function createWorkerFee() public {
        uint256 amountExp               = value();
        IERC20(tokenAddress).transfer(exchangeAddress,    amountExp.mul(100 - (developmentTax + nextRewardsTax + burnTax)).div(100));
    ...
    function createWorker() external {
        ...
        _safeMint(msg.sender, _nftId);
        _createWorker(_class_id, rarity, _gender_id, _nftId, currentGen);
        _nftId++;
    ...
    function createWorkerFromChest(string memory _name, uint256 _chestId) external {
        ...
        _safeMint(msg.sender, _nftId);
        _createWorker(_class_id, rarity, _gender_id, _nftId, 0);
        _nftId++;
    }
...
contract WorkerFactory {    
    ...
    function _createWorker(uint256 _class_id, uint256 rarity, uint256 _gender_id, uint256 _nftId, uint8 _gen) internal {
        workers.push(Worker(_class_id, rarity, _gender_id, 1, 0, _gen));
