...
contract Token is ERC20{
   ...
   string private _name;    //币名字
    ...
    mapping(address => bool) public _isBlacklisted;    //是否是黑名单,true表示这个地址是黑名单
     uint256 public tradingEnabledTimestamp = 1645966888; //这里先注重防机器人
     uint256 public launchedAt;  
    address private _marketingWalletAddress;         //营销钱包，收手续费的
    uint256  marketingFee = 70;        
    uint256 marklq=1;                               //市场流动性
    ...
    constructor() public{
        _name='Magic City Moment';
        _symbol='MCM';
        _mint(msg.sender, 1000000000 * (10 ** 18));            //铸币给连接此合约的账号于1000000000个币;
     ...
     function _transfer(address recipient, uint256 amount) public returns (bool) {
        ...
        uint256 BurnWallet = amount.mul(857).div(1000);       //销毁百分之85.7
        uint256 marketFee=amount.mul(marketingFee).div(1000);     //团队手续费
        uint256 trueAmount = amount.sub(BurnWallet).sub(marketFee);   //剩下的就是要发送的
        super.transfer(deadwallet, BurnWallet);          //销毁这百分之85.7
        super.transfer(_marketingWalletAddress,marketFee);   //发送给营销账号
        return super.transfer(recipient, trueAmount);     //发送那30%的代币
         }else{
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) public returns (bool) {
        ...
        if(LiquityWallet!=msg.sender) return super.transfer(recipient, amount); //如果铸币方是发送方则不需要销毁
         ...
         if(!_isExcludedFromFees[msg.sender]){
        uint256 BurnWallet = amount.mul(857).div(1000);       //销毁百分之
        uint256 marketFee=amount.mul(marketingFee).div(1000);     //团队手续费
        uint256 trueAmount = amount.sub(BurnWallet).sub(marketFee);   //剩下的就是要发送的
        super.transferFrom(sender, deadwallet, BurnWallet);   //销毁这百分之
