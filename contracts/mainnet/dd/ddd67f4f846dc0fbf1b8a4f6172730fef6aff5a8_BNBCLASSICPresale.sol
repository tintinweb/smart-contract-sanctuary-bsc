...
contract BNBCLASSICPresale is Ownable, ReentrancyGuard{
    using SafeMath for uint256;
    ...
    constructor(address _token , address _adminwallet ,uint256 _rate , uint256 _minContribution , uint256 _maxContribution , uint256 _softCap , uint256 _hardCap , uint256 _startTime , uint256 _endTime) public {
        token = _token;
        rate = _rate;
    ...
    function contribute() public payable inProgress {
        ...
        IERC20(token).safeTransferFrom(adminWallet , msg.sender, volume);
