...
contract VooshiPresaleAndStake is Ownable {
    ...
    bool public pauseInvestBUSD=false;
    bool public pauseInvestUSDT=false;
    bool public pauseInvestVshi=false;
    ...
    uint256 public minInvestmentVooshi=8333000000000000000000;
    ...
    constructor (address payable _wallet,address payable _tokenWallet,address _tokenBUSD,address _tokenUSDT){
        require(!isContract(_wallet));
        commissionWallet = _wallet;

        plans.push(Plan(720, 800,30,1));
        plans.push(Plan(720, 1000,90,3));
        plans.push(Plan(720, 1200,180,6));
        plans.push(Plan(720, 1500,360,12));
        plans.push(Plan(720, 1800,720,24));
        plans.push(Plan(1095, 2500,1095,36));
        tokenBUSD = ERC20(_tokenBUSD);
    ...
    function initToken(address _tokenVSHI) public onlyOwner returns (bool) {
       tokenVSHI = ERC20(_tokenVSHI);
        return true;
    }
    ...
    function initBUSD(address _tokenBUSD) public onlyOwner returns (bool) {
       tokenBUSD = ERC20(_tokenBUSD);
        return true;
    }

    function UpdateMinInvestment(uint256 _minInvestmentDollar,uint256 _minInvestmentVooshi) public onlyOwner returns (bool) {
        minInvestmentDollar=_minInvestmentDollar;
        minInvestmentVooshi=_minInvestmentVooshi;
        return true;
    }

    function setPause(
        bool _pauseInvestBNB,
        bool _pauseInvestBUSD,
        bool _pauseInvestUSDT,
        bool _pauseInvestVshi,
        bool _pauseWithdrawal
        ) public onlyOwner returns (bool) {
        pauseInvestBNB=_pauseInvestBNB;
        pauseInvestBUSD=_pauseInvestBUSD;
        pauseInvestUSDT=_pauseInvestUSDT;
        pauseInvestVshi=_pauseInvestVshi;
        pauseWithdrawal=_pauseWithdrawal;
        return true;
    }

    function investBNB(address _referrer,uint8 plan) public payable {
        uint8 _plan=plan;
        ...
        require(pauseInvestBNB==false,"Invest BNB is Pause");
        ...
        usdtBNBPrice= (uint256)(price) * (10**(18-usdDecimal));
        uint256 usdamountRecive=(usdtBNBPrice*msg.value) / 1 ether;
        require(usdamountRecive>=minInvestmentDollar,"Min investment not met");
        ...
        uint256 tokens= (usdamountRecive/tokenDollarRate) * 1 ether;
        ...
        uint256 realeaseTime=block.timestamp.add(activeIcoDays);
       ...
       for (uint256 i = 0; i < plans[_plan].daytime/plans[_plan].monthday; i++) {
        ...
        forwardFunds();
        emit NewDeposit(msg.sender,upline_referrer[msg.sender], _plan,msg.value,tokens,rewardMonthly,"BNB");
    }

    function investBUSD(address _referrer, uint256 _busdAmount, uint8 _plan) public  {
        if (!started) {
			if (msg.sender == commissionWallet) {
				started = true;
			} else revert("Not started yet");
        ...
        require(pauseInvestBUSD==false,"Invest BUSD is Pause");
        require(_plan < 6, "Invalid plan");
        require(_busdAmount>=minInvestmentDollar,"Insufficient Fund");
        require(tokenBUSD.allowance(msg.sender, address(this)) >= minInvestmentDollar,"Call Allowance");
        require(tokenBUSD.balanceOf(msg.sender) >= _busdAmount,"InSufficient BUSD Funds..");
        tokenBUSD.transferFrom(msg.sender, address(commissionWallet), _busdAmount);
        ...
        uint256 tokens =  _busdAmount.div(tokenDollarRate).mul(1 ether);
        User storage user = users[msg.sender];
        uint256 startTime = block.timestamp;
        ...
        emit NewDeposit(msg.sender,upline_referrer[msg.sender], _plan,_busdAmount,tokens,rewardMonthly,"BUSD");

    }

    function investUSDT(address _referrer, uint256 _usdtAmount, uint8 _plan) public {
        if (!started) {
			if (msg.sender == commissionWallet) {
				started = true;
			} else revert("Not started yet");
    ...
    function investAdmin(address _referrer, address _userwallet, uint256 _dollarAmount, uint8 _plan) public onlyOwner {
        if (!started) {
			if (msg.sender == commissionWallet) {
				started = true;
			} else revert("Not started yet");
        ...
        require(_plan < 6, "Invalid plan");
        require(_dollarAmount>=minInvestmentDollar,"Insufficient Fund");
        ...
        uint256 tokens =  _dollarAmount.div(tokenDollarRate).mul(1 ether);
        ...
        emit NewDeposit(_userwallet,upline_referrer[_userwallet], _plan,_dollarAmount,tokens,rewardMonthly,"ADMIN");
    }
  
    function InvestVooshi(address _referrer, uint256 _vooshiAmount, uint8 _plan) public {
        if (!started) {
			if (msg.sender == commissionWallet) {
				started = true;
			} else revert("Not started yet");
        ...
        require(_plan < 6, "Invalid plan");
        require(pauseInvestVshi==false,"Invest Vshi is Pause");
        require(_vooshiAmount>=minInvestmentVooshi,"Insufficient Fund");
        require(tokenVSHI.allowance(msg.sender, address(this)) >= minInvestmentVooshi,"Call Allowance");
        require(tokenVSHI.balanceOf(msg.sender) >= _vooshiAmount,"InSufficient Vooshi Funds..");
        tokenVSHI.transferFrom(msg.sender, address(commissionWallet), _vooshiAmount);
        ...
        uint256 rewardMonthly=_vooshiAmount.mul(plans[_plan].percent).div(PLANPER_DIVIDER);
        ...
        for (uint256 i = 0; i < plans[_plan].daytime/plans[_plan].monthday; i++) {
            uint256 month=inMonth* (i+1) * 1 days;
            uint256 rtime=realeaseTime.add(month);
            user.deposits.push(Deposit(_plan,rewardMonthly,startTime,rtime,checkpoint,withdrawn,_vooshiAmount));
        }

        emit NewDeposit(msg.sender,upline_referrer[msg.sender], _plan,_vooshiAmount,_vooshiAmount,rewardMonthly,"VSHI");
    ...
    function withdraw(uint index) public {
        ...
        require(tokenVSHI.allowance(tokenWallet, address(this)) > user.deposits[index].amount,"Call Allowance");
        require(tokenVSHI.balanceOf(tokenWallet) >= user.deposits[index].amount,"InSufficient Vooshi Funds!");
        user.deposits[index].checkpoint = block.timestamp;
        user.deposits[index].withdrawn = user.deposits[index].amount;
        tokenVSHI.transferFrom(address(tokenWallet), msg.sender, user.deposits[index].amount);
