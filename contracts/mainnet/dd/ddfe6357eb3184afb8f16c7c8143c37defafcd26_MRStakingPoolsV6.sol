...
contract MRStakingPoolsV6 is Context, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public halfLock = 15 days;
    uint256 public fullLock = 30 days;
    ...
    function emergencyUnstake(uint poolNo_, uint256 amount) external {
        Staker memory staker = stakers[_msgSender()][poolNo_];
        require(staker.amount != 0, "Sorry, wallet has no staking.");
        require(staker.amount >= amount, "Not enough staking amount.");
        require(block.timestamp >= staker.stakeTime.add(_getLockPeriod(staker.apyTime)), "MRStaking: lock time is not over yet.");
        _token.transfer(_msgSender(), staker.amount);
