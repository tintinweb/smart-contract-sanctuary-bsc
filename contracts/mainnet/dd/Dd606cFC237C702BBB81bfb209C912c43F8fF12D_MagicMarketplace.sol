...
contract MagicMarketplace is PausableUpgradeable, OwnableUpgradeable {
    ...
    mapping(address => mapping(uint256 => bool)) public isMCRT;

    // Map from token ID to token auction.
    address public mcrtToken;
    ...
    event AuctionMCRTCancelled(address indexed _nftAddress, uint256 indexed _tokenId);
    ...
    function bidMCRT(
        ...
        // _bid will throw if the bid or funds transfer fails
        _escrowMCRT(mcrtToken, msg.sender, _amount);
        _bidMCRT(_nftAddress, _tokenId, _amount);
        _transfer(_nftAddress, msg.sender, _tokenId);
    }
    ...
    function cancelAuction(address _nftAddress, uint256 _tokenId) external {
        ...
        if (!isMCRT[_nftAddress][_tokenId]) _cancelAuction(_nftAddress, _tokenId, _auction.seller);
        else _cancelMCRTAuction(_nftAddress, _tokenId, _auction.seller);
    }
    ...
    /// @dev Set MCRT token address
    /// Only the owner may do this
    /// @param _token - Address of ERC20 token.
    function setMCRTToken(address _token) external onlyOwner {
    ...
    function _addAuction(
        ...
        isMCRT[_nftAddress][_tokenId] = true;

        emit AuctionMCRTCreated(
            _nftAddress,
            _tokenId,
    ...
    function _removeAuction(address _nftAddress, uint256 _tokenId) internal {
        delete isMCRT[_nftAddress][_tokenId];
    ...
    function _cancelMCRTAuction(
        ...
        _removeAuction(_nftAddress, _tokenId);
        _transfer(_nftAddress, _seller, _tokenId);
        emit AuctionMCRTCancelled(_nftAddress, _tokenId);
    }
    ...
    /// @dev Escrows the MCRT tokens, assigning ownership to this contract.
    /// Throws if the escrow fails.
    /// @param _mcrtToken - The address of mcrt token.
    /// @param _owner - Current owner address of token to escrow.
    /// @param _amount - amount of mcrt token
    function _escrowMCRT(
        address _mcrtToken,
        address _owner,
        uint256 _amount
    ) internal {
        require(_mcrtToken != address(0), "Invalid mcrt token address");

        IERC20 _mcrtContract = IERC20(_mcrtToken);
        // It will throw if transfer fails
        _mcrtContract.transferFrom(_owner, address(this), _amount);
    }
    ...
    function _bid(
        ...
        require(_isOnAuction(_auction), "Auction is not active");
        require(!isMCRT[_nftAddress][_tokenId], "Auction is not listed by BNB");
    ...
    function _bidMCRT(
        ...
        // return an auction object that is all zeros.)
        require(_isOnAuction(_auction), "Auction is not active");
        require(isMCRT[_nftAddress][_tokenId], "Auction is not listed by MCRT");
        ...
        IERC20 _mcrtContract = IERC20(mcrtToken);
        // Transfer proceeds to seller (if there are any!)
        if (_price > 0) {
            //  Calculate the auctioneer's cut.
            // (NOTE: _computeCut() is guaranteed to return a
            ...
            // can DoS is the sale of their own asset! (And if it's an
            // accident, they can call cancelAuction(). )
            _mcrtContract.transfer(_seller, _sellerProceeds);
            _mcrtContract.transfer(treasury, _auctioneerCut);
        }
        ...
        if (_bidAmount > _price) {
            ...
            // not susceptible to a re-entry attack because the auction is
            // removed before any transfers occur.
            _mcrtContract.transfer(msg.sender, _bidExcess);
        }

        // Tell the world!
        emit AuctionMCRTSuccessful(_nftAddress, _tokenId, _price, msg.sender);
