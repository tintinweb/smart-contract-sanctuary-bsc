...
contract StratManager is Ownable, Pausable {
    ...
    function chargeFees(address _token) internal returns (uint256) {
        ...
        uint256 frfiFeeAmount = feeBal.mul(frfiFee).div(MAX_FEE);
        if (frfiFeeAmount > 0)
            token.safeTransfer(frfiFeeRecipient, frfiFeeAmount);

        uint256 strategistFeeAmount = feeBal.sub(callFeeAmount).sub(frfiFeeAmount);
