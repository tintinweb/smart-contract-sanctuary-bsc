 ...
 */
contract KIPSLockedWallet is Ownable {
    using SafeERC20 for IERC20;

    // 100 day timestamp when token release is enabled
    uint256 public releaseTime = 1628346191;
    
    address public NgenSwapToken = 0x1c57eEed03F36d8954c6a54b0624E2c094841eF1;
    address public lpToken = 0x28FDe83Cf381E80069ee7156D031360D3F0e850B;

    /**
     * @notice Transfers tokens held by timelock to owner.
     */
    function releaseNgenSwapToken() external onlyOwner {
        // solhint-disable-next-line not-rely-on-time
        require(block.timestamp >= releaseTime, "TokenTimelock: current time is before release time");

        uint256 amount = IERC20(NgenSwapToken).balanceOf(address(this));
        require(amount > 0, "TokenTimelock: no tokens to release");

        IERC20(NgenSwapToken).safeTransfer(owner(), amount);
    }
    ...
    function getNgenSwapTokenBalance() external view returns(uint256) {
        return IERC20(NgenSwapToken).balanceOf(address(this));
    }
    ...
    function setNgenSwapToken(address _NgenSwapToken) external onlyOwner {
        NgenSwapToken = _NgenSwapToken;
    }
