...
#busd, usdc, ust, dai, usdt


FEE_DENOMINATOR: constant(uint256) = 10 ** 10
LENDING_PRECISION: constant(uint256) = 10 ** 18
PRECISION: constant(uint256) = 10 ** 18  # The precision to convert to
PRECISION_MUL: constant(uint256[N_COINS]) = [1, 1, 1, 10**12]
RATES: constant(uint256[N_COINS]) = [10**18, 10**18, 10**18, 10**30]
FEE_INDEX: constant(int128) = 2  # Which coin may potentially have fees (USDT)

MAX_ADMIN_FEE: constant(uint256) = 10 * 10 ** 9
MAX_FEE: constant(uint256) = 5 * 10 ** 9
MAX_A: constant(uint256) = 10 ** 6
MAX_A_CHANGE: constant(uint256) = 10**4

ADMIN_ACTIONS_DELAY: constant(uint256) = 10
MIN_RAMP_TIME: constant(uint256) = 10

N_COINS: constant(int128) = 4  # <- change
n_coins: public(int128)


pool_name: public(String[30])
pool_id: public(uint256)
