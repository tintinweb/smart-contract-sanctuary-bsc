...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
   ...
   // The price to sell share
    ...
    function initialize(
     ...
     function checkValue(uint256 amount) public pure  returns  (bool){
        require(amount > 0, "amount must be more than 0");
        uint256 y;
        y = 100*10**18;

        if (amount.mod(y) > uint256(0)){
            return false;
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        require (_amount >= 100*10**18, 'Min value is 100*10**18');
         require(checkValue(_amount), "Must be multiple of 100");
        ...
        uint256 amountTopayShareHolder = _amount.div(price);
        user.rewardDebt = user.rewardDebt.add(amountTopayShareHolder);
        rewardToken.safeTransfer(address(msg.sender), amountTopayShareHolder);
    ...
    function busdWithdraw() external onlyOwner {
        ...
        amountWithdraw = stakedToken.balanceOf(address(this));
