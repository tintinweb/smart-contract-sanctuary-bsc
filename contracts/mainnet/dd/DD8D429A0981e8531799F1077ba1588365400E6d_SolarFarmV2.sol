...
//    }
    ...
    function shootingStar(uint amplifier) external onlyOwner
    {
        require(amplifier <= 250, "SolarGuard: Shooting Star value amplifier exceeds 25%!");

        shootingStarTimestamp = block.timestamp;
        shootingStarAbusePenalty = 500;

        gridPower = computeFraction(gridPower, 1000 - amplifier);

        emit ShootingStar(amplifier, block.timestamp);
    }
