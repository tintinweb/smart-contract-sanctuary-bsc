...
contract FPC is Context, IERC20, Ownable {
    ...
    string private _name = "FuckPutinCoin";
    ...
    address payable public marketingWallet = payable(0x593E2D679F67E2931d524f41624eD80484594DC6);

    uint256 public _charityFee = 6;
    uint256 private _previousCharityFee = _charityFee;
    address payable public charityWallet = payable(0xFc6306aF430D83951DB5AbE35AdE50efe466dd15);
    ...
    function _transfer(
        ...
        //transfer amount, it will take tax, marketing, charity, burn and liquidity fee
        _tokenTransfer(from,to,amount);
    }

    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 liquidityTokenShare = contractTokenBalance.mul(_liquidityFee).div(_liquidityFee.add(_marketingFee).add(_charityFee));
        uint256 marketingTokenShare = contractTokenBalance.mul(_marketingFee).div(_liquidityFee.add(_marketingFee).add(_charityFee));
        uint256 charityTokenShare = contractTokenBalance.sub(liquidityTokenShare.add(marketingTokenShare));
        ...
        swapTokensForEth(charityTokenShare, charityWallet);
    ...
    function setFeePercent(uint256 taxFee, uint256 liquidityFee, uint256 burnFee, uint256 marketingFee, uint256 charityFee) external onlyOwner() {
        _taxFee = taxFee;
    ...
    function setMaxWalletTokend(uint256 _maxToken) external onlyOwner {
  	    require(_maxToken*10**18 >= totalSupply().div(100));
