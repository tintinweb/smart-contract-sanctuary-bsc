...
contract Chick is ReentrancyGuard, Pausable
{
	...
	address public address_wbnb_vault;
	...
	// Getters and Setters
	//---------------------------------------------------------------
	function set_address_vault(address _address_busd_vault, address _address_wbnb_vault) external onlyOperator
	{
		address_busd_vault = _address_busd_vault;
		address_wbnb_vault = _address_wbnb_vault;
		emit SetValutCB(msg.sender, _address_busd_vault, _address_wbnb_vault);
	}
	...
	function handle_stuck(address _token, uint256 _amount, address _to) external onlyOperator nonReentrant
	{
	...
	{
		uint256 arrow_amount = IERC20(_native_token).balanceOf(address(this));
		if(arrow_amount < swap_threshold)
			return;

		uint256 amount_busd = (arrow_amount * 1e4 * bnb_per_busd_vault_ratio_e4) / 1e8;
		uint256 amount_wbnb  = arrow_amount - amount_busd;
	...
	function _collect_my_wbnb() internal
	{
		// 1 in WBNB
		...
		// 2 in this contract
		uint256 my_wbnb_amount2 = address(this).balance;
		payable (address_wbnb_vault).transfer(my_wbnb_amount2);
	}
