...
2% TX Fee goes to buying DOSA tokens on ETH and multisending to your ETH wallet (same as BSC wallet address)
...
contract DOSA is Ownable, IBEP20 {
    using SafeMath for uint256;
    ...
    uint256 private dosatokenFee   = 2;
    uint256 private devFee         = 2;
    uint256 public burnFee         = 0;
    uint256 public totalFee        = marketingFee + rewardFee + liquidityFee + dosatokenFee + burnFee + devFee;
    ...
    address private dosatokenFeeReceiver;
    ...
    constructor () {
       router = IDEXRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        ...
        marketingFeeReceiver = 0x2CA483fa8ef03CcB59dfb1F82681f52d732042ca;
        dosatokenFeeReceiver = 0x7F78756255050177115e82e36fF977D4D1989E58;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBdosatoken = amountBNB.mul(dosatokenFee).div(totalBNBFee);
        uint256 amountBNBdev = amountBNB.mul(devFee).div(totalBNBFee);
    
        try distributor.deposit{value: amountBNBreward}() {} catch {}
        (bool tmpSuccess,) = payable(marketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
        (tmpSuccess,) = payable(dosatokenFeeReceiver).call{value: amountBNBdosatoken, gas: 30000}("");
    ...
    function setFees(uint256 _liquidityFee, uint256 _rewardFee, uint256 _marketingFee, uint256 _dosatokenFee, uint256 _devFee, uint256 _burnFee, uint256 _feeDenominator) public onlyOwner {
        liquidityFee = _liquidityFee;
        rewardFee = _rewardFee;
        marketingFee = _marketingFee;        
        dosatokenFee = _dosatokenFee;
        devFee = _devFee;
        burnFee = _burnFee;
        totalFee = _liquidityFee + _rewardFee + _marketingFee + _dosatokenFee + _burnFee + _devFee;
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator/2, "Fees cannot be more than 49%");
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _dosatokenFeeReceiver, address _burnFeeReceiver, address _devFeeReceiver ) public onlyOwner {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        marketingFeeReceiver = _marketingFeeReceiver;
        dosatokenFeeReceiver = _dosatokenFeeReceiver;
