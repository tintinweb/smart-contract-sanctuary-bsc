...
contract WicketWitch is Context, IERC20, Ownable {
    ...
    address public Mkting = 0x4D5BdB3bEf2AB54c4CeEB80820bF46aC73EfC9D5;
    address private Buyback = 0xf360431EA975502e58EC6B4D2557fcdF02b9269b;
    address public Charity = 0x7cFD6CdC72ef10f4198Fb4aa0d369d3C0d8e5804;

    string private constant _name = "Wicked Witch";
    ...
    Taxes public taxes = Taxes(1, 10, 2, 4, 0);
    Taxes public sellTaxes = Taxes(1, 10, 2, 4, 0);
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[Mkting] = true;
        ...
        allowedTransfer[Mkting] = true;
        allowedTransfer[Buyback] = true;
    ...
    //@Buyback kept original RFI naming -> "reward" as in reflection
    ...
    function setTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _Buyback, uint256 _Charity) public onlyOwner {
       taxes = Taxes(_rfi,_marketing,_liquidity,_Buyback,_Charity);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _Buyback, uint256 _Charity) public onlyOwner {
       sellTaxes = Taxes(_rfi,_marketing,_liquidity,_Buyback,_Charity);
        emit FeesChanged();
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rBuyback, to_return.rCharity) = _getRValues2(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.tTransferAmount = tAmount-s.tRfi-s.tMarketing-s.tLiquidity-s.tBuyback-s.tCharity;
        return s;
    }
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rMarketing, uint256 rLiquidity){
        ...
        rTransferAmount =  rAmount-rRfi-rMarketing-rLiquidity-rBuyback-rCharity;
        return (rAmount, rTransferAmount, rRfi,rMarketing,rLiquidity);
    }
    
    function _getRValues2(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 rBuyback,uint256 rCharity) {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tMarketing + s.tBuyback+ s.tCharity);
        }
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.marketing + temp.Buyback + temp.Charity) * 2;
        ...
        if(marketingAmt > 0){
            payable(Mkting).sendValue(marketingAmt);
        }
        uint256 BuybackAmt = unitBalance * 2 * temp.Buyback;
        if(BuybackAmt > 0){
            payable(Buyback).sendValue(BuybackAmt);
        }
    ...
    function updateMkting(address newWallet) external onlyOwner{
        Mkting = newWallet;
    }
    
    function updateBuyback(address newWallet) external onlyOwner{
        Buyback = newWallet;
    }
