...
contract AniwarNft is ERC721Enumerable, Ownable {
    ...
    event RequestedAniwarItem(string aniType, uint256 indexed requestId, address indexed requester);
    constructor() ERC721("Aniwar Nft", "ANIN") {
        tokenCounter = 0; 
    }

    function _createAniwarItem(string memory aniwarType, address owner) private onlyWhiteList {  
    ...
    function createManyAniwarItem(uint8 _count, address owner, string memory aniwarType) external { 
    ...
    function setWhiteList(address[] memory _whiteList, bool _state) public onlyOwner {
        require(_whiteList.length > 0,"Not get it!");
        for (uint256 i = 0; i < _whiteList.length; i++) {
            require(_whiteList[i] != address(0), "address = 0x0!");
            whiteList[_whiteList[i]] = _state;
            if(_state) {
                emit AniwarNftWhiteListAdded(IAniwarNftController(_whiteList[i]).aniwarType(), _whiteList[i]);
            }
        }
    }
