...
contract FlokitaLPFarm is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of Flokita
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;         // How many allocation points assigned to this pool. Flokita to distribute per block.
        uint256 lastRewardBlock;    // Last block number that Flokita distribution occurs.
        uint256 accTokenPerShare;   // Accumulated Flokita per share, times 1e12. See below.
    ...
    // The Flokita TOKEN!
    IBEP20 public token;
    // Flokita tokens created per block.
    ...
    // The block number when Flokita mining starts.
    ...
    constructor() {
        token = IBEP20(0xFF107D6b39737828552A4B2f2a7e5cb33a2A8423);
        startBlock = block.number;
        tokenPerBlock = 8 ether;
    ...
    // Update the given pool's Flokita allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending Flokita on frontend.
    ...
    // View function to see if user can harvest Flokita.
    ...
    // Deposit LP tokens to Farm for Flokita allocation.
    ...
    function withdraw(uint256 _pid) external nonReentrant {
        ...
        if(!canHarvest(_pid, msg.sender)){
            ...
            if(penaltyAmount > 0)
                pool.lpToken.safeTransfer(marketingAddress, penaltyAmount);
        }
    ...
    // Safe token transfer function, just in case if rounding error causes pool to not have enough Flokita.
