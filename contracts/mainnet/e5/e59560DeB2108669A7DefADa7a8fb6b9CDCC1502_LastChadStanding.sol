...
Come show us just how big of a Chad you really are!  Real Chads buy ATH.

Who will be the Last Chad?
...
contract LastChadStanding is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    LastChadStandingDividendTracker public dividendTracker;
    ...
    constructor() ERC20("LastChadStanding", "LCS") {
        marketingWallet = payable(0x3e0d076af0a8Dc2B073Bf9E7F73F098849898655); 
        ...
        dividendTracker = new LastChadStandingDividendTracker(
            payable(this),
            router,
            0xc7d28C031e08d2bAe157eaf5AF89CAb2740c107B,
            "LastChadStandingTRACKER",
            "LCSTRACKER"
        );
...
contract LastChadStandingDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    LastChadStanding public LastChadStandingContract;
    ...
    constructor(
        ...
        defaultToken = token;
        LastChadStandingContract = LastChadStanding(mainContract);
    ...
    function _transfer(
        ...
        require(false, "LastChadStanding_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "LastChadStanding_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main LastChadStanding contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(LastChadStandingContract);

            uint256 prevBalance = LastChadStandingContract.balanceOf(address(this));
            ...
            {
                uint256 received = LastChadStandingContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    LastChadStandingContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
