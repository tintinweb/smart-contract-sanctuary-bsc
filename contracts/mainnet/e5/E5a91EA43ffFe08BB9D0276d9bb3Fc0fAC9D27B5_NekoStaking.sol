...
contract NekoStaking is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function getArrBlockChangeBalance(address _user) external view returns (uint256[] memory) {
        return arrBlockOfUser[_user];
    }

    function getNumberStakeAtBlock(address _user, uint256 _block) external view returns (uint256) {
        if( ! staker[_user]) {
