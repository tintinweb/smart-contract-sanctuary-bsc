...
import { HmineCompound } from "./HmineCompound.sol";
...
contract Snapshot
{
	...
	function snapshotToken(address _token, address[] memory _wallets) external view returns (WalletInfo[] memory _local, uint256[1] memory _global)
	{
		uint256 _length = _wallets.length;
		_local = new WalletInfo[](_length);
		for (uint256 _i = 0; _i < _length; _i++) {
			address _account = _wallets[_i];
			uint256 _amount = IERC20(_token).balanceOf(_account);
			bool _isContract = _account.isContract();
			_local[_i] = WalletInfo({ account: _account, amount: _amount, isContract: _isContract });
		}
		_global[0] = IERC20(_token).totalSupply();
	...
	function snapshotVDC(address _vdc) external view returns (VDCInfo[] memory _local, uint256[6] memory _global)
	{
		uint256 _length = HmineCompound(_vdc).accountIndexLength();
		_local = new VDCInfo[](_length);
		for (uint256 _i = 0; _i < _length; _i++) {
			address _account = HmineCompound(_vdc).accountIndex(_i);
			(uint256 _amount, uint256 _reward, uint256 _drip, uint256 _accRewardDebt, uint256 _accDripDebt, bool _whitelisted, bool _exists) = HmineCompound(_vdc).accountInfo(_account);
			_local[_i] = VDCInfo({ account: _account, amount: _amount, reward: _reward, drip: _drip, accRewardDebt: _accRewardDebt, accDripDebt: _accDripDebt, whitelisted: _whitelisted, exists: _exists });
		}
		_global[0] = HmineCompound(_vdc).totalStaked();
		_global[1] = HmineCompound(_vdc).totalDrip();
		_global[2] = HmineCompound(_vdc).allocDrip();
		_global[3] = HmineCompound(_vdc).totalReward();
		_global[4] = HmineCompound(_vdc).accRewardPerShare();
		_global[5] = HmineCompound(_vdc).accDripPerShare();
		return (_local, _global);
	}
}
