...
Elon Tweet: https://twitter.com/elonmusk/status/1586690742151307264
TG: https://t.me/DickButtOfficial
...
contract DickButt is IBEP20, Ownable {
    using SafeMath for uint256;

    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

    string constant _name = "DickButt";
    string constant _symbol = "$DickButt";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 10000000000*10**_decimals; 

    uint256 public _maxTxAmount = _totalSupply.div(100).mul(80);
    uint256 public _maxSellTxAmount = _totalSupply.div(100).mul(80);
    uint256 public _maxWalletToken = _totalSupply.div(100).mul(80);
    ...
    uint256 public donationsAndPamps = 0;
    uint256 public sellFee = 5;
    uint256 public totalFee        = marketingFee + liquidityFee + donationsAndPamps;
    ...
    uint256 public minSol = 1;
    uint256 public graceBlocks = 1;
    bool public dynamicBurn = true;

    uint256 public maxBurn = 1;
    ...
    function checkValidity(address sender, address recipient, uint256 _amount) internal view {
        ...
        if (_botMode && sender != pair){
            require(!_botList[sender] || sender == address(this), "Thats not Happy Pappy!");
        }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBBuyBack = amountBNB.mul(donationsAndPamps).div(totalBNBFee);
    ...
    function setFees(uint256 _liquidityFee, uint256 _marketingFee, uint256 _donationsAndPamps, uint256 _sellFee, uint256 _feeDenominator) external onlyOwner {
        liquidityFee = _liquidityFee;
        marketingFee = _marketingFee;
        donationsAndPamps = _donationsAndPamps;
        sellFee = _sellFee;
        totalFee = _liquidityFee + _marketingFee +_donationsAndPamps;
