...
contract MAMA is Context, IBEP20, Ownable {
    ...
    uint256 private _tTotal = 25e12 * 10**_decimals;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));

    uint256 public swapTokensAtAmount = 25e9 * 10**9;

    uint256 public genesis_block;
    uint256 private deadline = 3;

    address public deadWallet = 0x000000000000000000000000000000000000dEaD;
    address public marketingWallet = 0xf9432a7cf75130460c7D238145bd42fa6E9abaF8;
    address public chartWallet = 0xf9432a7cf75130460c7D238145bd42fa6E9abaF8;
    address public spinWallet = 0xf9432a7cf75130460c7D238145bd42fa6E9abaF8;
    address public charityWallet = 0xf9432a7cf75130460c7D238145bd42fa6E9abaF8;

    string private constant _name = "MAMA Santa";
    ...
    Taxes public taxes = Taxes(0, 1, 0, 1, 1, 0);
    Taxes public sellTaxes = Taxes(1, 3, 0, 1, 1, 1);
    Taxes private launchtax = Taxes(0, 99, 0, 0, 0, 0);
    ...
    constructor(address routerAddress) {
        ...
        _isExcludedFromFee[chartWallet] = true;
        _isExcludedFromFee[spinWallet] = true;
    ...
    function setTaxes(
        ...
        require((_rfi + _marketing + _liquidity + _chart + _spin + _charity ) <= 12, "Must keep fees at 12% or less");
        taxes = Taxes(_rfi, _marketing, _liquidity, _chart, _spin, _charity);
        emit FeesChanged();
    }
    ...
    function setSellTaxes(
        ...
        require((_rfi + _marketing + _liquidity + _chart + _spin + _charity ) <= 12, "Must keep fees at 12% or less");
        sellTaxes = Taxes(_rfi, _marketing, _liquidity, _chart, _spin, _charity);
        emit FeesChanged();
    }
    ...
    function _takeSpin(uint256 rSpin, uint256 tSpin) private {
        totFeesPaid.spin += tSpin;

        if (_isExcluded[address(this)]) {
            _tOwned[address(this)] += tSpin;
        }
        _rOwned[address(this)] += rSpin;
    }
    ...
    function _getValues(
        ...
        (to_return.rChart, to_return.rSpin, to_return.rCharity) = _getRValues2(
            to_return,
            takeFee,
            _getRate()
        );
        ...
        )
    {
        ...
        uint256 rSpin = s.tSpin * currentRate;
        ...
        )
    {
        ...
        rSpin = s.tSpin * currentRate;
        rCharity = s.tCharity * currentRate;
        return (rChart, rSpin, rCharity);
    }
    ...
    function _tokenTransfer(
        ...
        if (s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(
                sender,
                address(this),
                s.tLiquidity + s.tMarketing + s.tChart + s.tSpin + s.tCharity
            );
        }
        if (s.rMarketing > 0 || s.tMarketing > 0) _takeMarketing(s.rMarketing, s.tMarketing);
        if (s.rChart > 0 || s.tChart > 0) _takeChart(s.rChart, s.tChart);
        if (s.rSpin > 0 || s.tSpin > 0) _takeSpin(s.rSpin, s.tSpin);
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap {
        ...
        if (chartAmt > 0) {
            payable(chartWallet).sendValue(chartAmt);
        }

        uint256 spinAmt = unitBalance * 2 * temp.spin;
        if (spinAmt > 0) {
            payable(spinWallet).sendValue(spinAmt);
        }
    ...
    function updateChartWallet(address newWallet) external onlyOwner {
        require(newWallet != address(0),"Fee Address cannot be zero address");
        chartWallet = newWallet;
    }


    function updateSpinWallet(address newWallet) external onlyOwner {
        require(newWallet != address(0),"Fee Address cannot be zero address");
        spinWallet = newWallet;
    }
    ...
    function updateSwapTokensAtAmount(uint256 amount) external onlyOwner {
        require(amount <= 25e10, "Cannot set swap threshold amount higher than 1% of tokens");
