...
import { IBarterPair } from "./interface/IBarterPair.sol";
import { IBarterRouter } from "./interface/IBarterRouter.sol";
import { IWETH } from "../tokenization/interface/IWETH.sol";
import { Babylonian } from "../../common/libraries/helpers/Babylonian.sol";
import { IBarterZap } from "./interface/IBarterZap.sol";
import { IAddressProviderZap } from "./interface/IAddressProviderZap.sol";

/*
 * @author Inspiration from the work of Zapper and Beefy.
 * Implemented and modified by PancakeSwap teams.
 */
contract BarterZap is Ownable, ReentrancyGuard, IBarterZap {
  using SafeERC20 for IERC20;

  // Interface for Wrapped BNB (WBNB)
  IWETH public WBNB;

  // BarterRouter interface
  IBarterRouter public barterRouter;
  ...
  // Address BarterRouter
  address private barterRouterAddress;
  ...
  IAddressProviderZap public immutable ADDRESSES_PROVIDER;
  ...
  function _onlyPrime() internal view virtual {
    require(ADDRESSES_PROVIDER.getPrimeContract() == msg.sender, "CALLER_NOT_PRIME_CONTRACT");
  }

  /*
   * @notice Constructor
   * @param _WBNBAddress: address of the WBNB contract
   * @param _barterRouter: address of the BarterRouter
   * @param _maxZapReverseRatio: maximum zap ratio
   */
  constructor(address _WBNBAddress, address _barterRouter, uint256 _maxZapReverseRatio, IAddressProviderZap provider) {
    WBNBAddress = _WBNBAddress;
    WBNB = IWETH(_WBNBAddress);
    barterRouterAddress = _barterRouter;
    barterRouter = IBarterRouter(_barterRouter);
  ...
  function zapInToken(
    ...
    address account
  ) external override nonReentrant onlyPrime {
    // Transfer tokens to this contract
    IERC20(_tokenToZap).safeTransferFrom(address(account), address(this), _tokenAmountIn);

    // Call zap function
    uint256 lpTokenAmountTransferred = _zapIn(_tokenToZap, _tokenAmountIn, _lpToken, _tokenAmountOutMin, address(account));

    // Emit event
    emit ZapIn(_tokenToZap, _lpToken, _tokenAmountIn, lpTokenAmountTransferred, address(account));
  }
  ...
  function zapInTokenRebalancing(
    ...
    // Transfer tokens to this contract
    IERC20(_token0ToZap).safeTransferFrom(address(account), address(this), _token0AmountIn);
    IERC20(_token1ToZap).safeTransferFrom(address(account), address(this), _token1AmountIn);
    ...
    // Emit event
    emit ZapInRebalancing(_token0ToZap, _token1ToZap, _lpToken, _token0AmountIn, _token1AmountIn, lpTokenAmountTransferred, address(account));
  }
  ...
  function zapOutToken(
    ...
    IERC20(_lpToken).safeTransferFrom(address(account), address(_lpToken), _lpTokenAmount);

    uint256 tokenAmountToTransfer = _zapOut(_lpToken, _tokenToReceive, _tokenAmountOutMin);

    IERC20(_tokenToReceive).safeTransfer(address(account), tokenAmountToTransfer);

    emit ZapOut(_lpToken, _tokenToReceive, _lpTokenAmount, tokenAmountToTransfer, account);
  }
  ...
  function estimateZapInSwap(
    ...
    address token0 = IBarterPair(_lpToken).token0();
    address token1 = IBarterPair(_lpToken).token1();

    require(_tokenToZap == token0 || _tokenToZap == token1, "BarterZap: Wrong tokens");

    // Convert to uint256 (from uint112)
    (uint256 reserveA, uint256 reserveB, ) = IBarterPair(_lpToken).getReserves();

    if (token0 == _tokenToZap) {
      swapTokenOut = token1;
      swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveA, reserveB);
      swapAmountOut = barterRouter.getAmountOut(swapAmountIn, reserveA, reserveB);
    } else {
      swapTokenOut = token0;
      swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveB, reserveA);
      swapAmountOut = barterRouter.getAmountOut(swapAmountIn, reserveB, reserveA);
    }
  ...
  function estimateZapInRebalancingSwap(
    ...
    require(_token0ToZap == IBarterPair(_lpToken).token0() || _token0ToZap == IBarterPair(_lpToken).token1(), "BarterZap: Wrong token0");
    require(_token1ToZap == IBarterPair(_lpToken).token0() || _token1ToZap == IBarterPair(_lpToken).token1(), "BarterZap: Wrong token1");

    require(_token0ToZap != _token1ToZap, "BarterZap: Same tokens");

    // Convert to uint256 (from uint112)
    (uint256 reserveA, uint256 reserveB, ) = IBarterPair(_lpToken).getReserves();

    if (_token0ToZap == IBarterPair(_lpToken).token0()) {
      ...
      // Calculate the amount that is expected to be swapped
      swapAmountIn = _calculateAmountToSwapForRebalancing(_token0AmountIn, _token1AmountIn, reserveB, reserveA, sellToken0);
  ...
  function estimateZapOutSwap(
    ...
    require(_tokenToReceive == token0 || _tokenToReceive == token1, "BarterZap: Token not in LP");
    ...
    if (token1 == _tokenToReceive) {
      // sell token0
      uint256 tokenAmountIn = (_lpTokenAmount * reserveA) / IBarterPair(_lpToken).totalSupply();
      ...
      // sell token1
      uint256 tokenAmountIn = (_lpTokenAmount * reserveB) / IBarterPair(_lpToken).totalSupply();
  ...
  function _zapIn(
    ...
    require(_tokenAmountIn >= MINIMUM_AMOUNT, "BarterZap: Amount too low");
...

    {
      ...
      require((reserveA >= MINIMUM_AMOUNT) && (reserveB >= MINIMUM_AMOUNT), "BarterZap: Reserves too low");

      if (token0 == _tokenToZap) {
        swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveA, reserveB);
        path[1] = token1;
        require(reserveA / swapAmountIn >= maxZapReverseRatio, "BarterZap: Quantity higher than limit");
      } else {
        swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveB, reserveA);
        path[1] = token0;
        require(reserveB / swapAmountIn >= maxZapReverseRatio, "BarterZap: Quantity higher than limit");
      }
    }
    ...
    uint256[] memory swapedAmounts = barterRouter.swapExactTokensForTokens(swapAmountIn, _tokenAmountOutMin, path, address(this), block.timestamp);
    ...
    (, , lpTokenReceived) = barterRouter.addLiquidity(
      path[0],
      path[1],
    ...
    IBarterPair(_lpToken).transfer(address(account), lpTokenReceived);
  ...
  function _zapInRebalancing(
...

    {
      ...
      if (_token0ToZap == IBarterPair(_lpToken).token0()) {
        swapAmountIn = _calculateAmountToSwapForRebalancing(_token0AmountIn, _token1AmountIn, reserveA, reserveB, _isToken0Sold);
        require(reserveA / swapAmountIn >= maxZapReverseRatio, "BarterZap: Quantity higher than limit");
      } else {
        swapAmountIn = _calculateAmountToSwapForRebalancing(_token0AmountIn, _token1AmountIn, reserveB, reserveA, _isToken0Sold);

        require(reserveB / swapAmountIn >= maxZapReverseRatio, "BarterZap: Quantity higher than limit");
      }
    }

    require(swapAmountIn <= _tokenAmountInMax, "BarterZap: Amount to swap too high");
    ...
    lpTokenReceived = _approveAndAddLiq(_isToken0Sold, _token0ToZap, _token1ToZap, _token0AmountIn, _token1AmountIn, path, swapedAmounts);
  ...
  function _zapOut(address _lpToken, address _tokenToReceive, uint256 _tokenAmountOutMin) internal returns (uint256) {
    ...
    (uint256 amount0, uint256 amount1) = IBarterPair(_lpToken).burn(address(this));

    require(amount0 >= MINIMUM_AMOUNT, "BarterRouter: INSUFFICIENT_A_AMOUNT");
    require(amount1 >= MINIMUM_AMOUNT, "BarterRouter: INSUFFICIENT_B_AMOUNT");
    ...
    // Swap tokens
    barterRouter.swapExactTokensForTokens(swapAmountIn, _tokenAmountOutMin, path, address(this), block.timestamp);
  ...
  function _approveTokenIfNeeded(address _token) private {
    if (IERC20(_token).allowance(address(this), barterRouterAddress) < 1e24) {
      // Re-approve
      IERC20(_token).safeApprove(barterRouterAddress, MAX_INT);
    }
  }
  ...
  function _calculateAmountToSwap(uint256 _token0AmountIn, uint256 _reserve0, uint256 _reserve1) private view returns (uint256 amountToSwap) {
    ...
    uint256 nominator = barterRouter.getAmountOut(halfToken0Amount, _reserve0, _reserve1);
    uint256 denominator = barterRouter.quote(halfToken0Amount, _reserve0 + halfToken0Amount, _reserve1 - nominator);
  ...
  function _calculateAmountToSwapForRebalancing(
    ...
    require(sellToken0 == _isToken0Sold, "BarterZap: Wrong trade direction");

    if (sellToken0) {
      uint256 token0AmountToSell = (_token0AmountIn - (_token1AmountIn * _reserve0) / _reserve1) / 2;
      uint256 nominator = barterRouter.getAmountOut(token0AmountToSell, _reserve0, _reserve1);
      uint256 denominator = barterRouter.quote(token0AmountToSell, _reserve0 + token0AmountToSell, _reserve1 - nominator);
      ...
      uint256 nominator = barterRouter.getAmountOut(token1AmountToSell, _reserve1, _reserve0);

      uint256 denominator = barterRouter.quote(token1AmountToSell, _reserve1 + token1AmountToSell, _reserve0 - nominator);

      // Calculate the amount to sell (in token1)
      token1AmountToSell = (_token1AmountIn - ((_token0AmountIn * (_reserve1 + token1AmountToSell)) / (_reserve0 - nominator))) / 2;

      // Adjustment for price impact
      amountToSwap = 2 * token1AmountToSell - Babylonian.sqrt((token1AmountToSell * token1AmountToSell * nominator) / denominator);
    }
