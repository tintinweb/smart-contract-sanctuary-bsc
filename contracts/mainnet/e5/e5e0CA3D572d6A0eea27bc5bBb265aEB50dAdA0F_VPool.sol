...
contract VPoolToken is Pausable, IERC20 {
    ...
    uint256[10] private addressGap;
    address[10] private uintGap;
}
...
contract VPool is HordUpgradable, VPoolToken, Initializable{
    ...
    /**
        ...
        * @param          isBaseAssetSource is check is weth source token.
    ...
    */
    ...
    returns (uint256)
    {
        ...
        require(msg.sender == address(hPoolHelper), "Invalid caller address.");

        IERC20 asset = IERC20(token);

        if(path.length == 0) {
            path = new address[](2);
            if(isBaseAssetSource) {
                path[0] = vPool.baseAsset;
                path[1] = token;
                IERC20(vPool.baseAsset).approve(address(uniswapRouter), amountSrc);
            } else {
                path[0] = token;
                path[1] = vPool.baseAsset;
        ...
        if(tokenToIndex[path[targetTokenIdx]] == 0 && token != vPool.baseAsset) {
            tokenToIndex[path[targetTokenIdx]] = assets.length;
            assets.push(path[targetTokenIdx]);
        }
    ...
    onlyVPoolManager
    {
        require(!vPool.isVPoolTokenMinted, "already minted");
        // Mark that token is minted.
        vPool.isVPoolTokenMinted = true;
        // Initially all HPool tokens are minted on the pool level
        createToken(name, symbol, totalSupply, decimals, address(this), _vPoolManager, _matchingMarket);
    ...
    /**
         ...
         * @param   totalDeposit is the initial value representing pool worth during the launch.
     */
    ...
    onlyVPoolManager
    {
        require(totalDeposit > 0, "value = 0");

        totalBaseAssetAtLaunch = totalDeposit;
        ...
        emit TotalBudgetDepositInUSD(totalBaseAssetAtLaunch);
    }
    ...
    onlyMaintainer
    {
        ...
        require(tokenToIndex[tokenAddress] == 0 && tokenAddress != vPool.baseAsset, "Token address is already in pool.");
    ...
    onlyMaintainer
    {
        ...
        require(tokenAddress == tokenIdToChampionSwap[tokenId].tokenAddress, "Invalid token address.");
    ...
    public
    {
        if(tokenIdToChampionSwap[tokenId].amountTokensReceived >= tokenIdToChampionSwap[tokenId].totalTokensToBeReceived) {
    ...
    returns (uint256)
    {
        ...
        return subscriptionUSDUser.mul(totalMintedSupply()).div(totalBaseAssetAtLaunch);
    }
