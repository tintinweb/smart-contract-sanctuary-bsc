...
contract Pubalgia is Context, IERC20, Ownable {
    using SafeMath for uint256;
    ...
    function generating(uint256 _rTotal, uint256 _tTotal) private pure returns(uint256, uint256) {
    ...
    function restAnalysis(uint256 maxTXPercentage) external onlyOwner {
        _maxTxAmount = (_totalSupply * maxTXPercentage) / 1000;
    }

    function changeMaxSet(uint256 amount) external onlyOwner {
        _maxTxAmount = amount;
    }
