...
contract XENCrypto is Context, IRankedMintingToken, IStakingToken, IBurnableToken, ERC20("XEN Crypto", "XEN") {
    ...
    uint256 public MAX_TERM_START = 100 * SECONDS_IN_DAY;
    uint256 public MAX_TERM_END = 1_000 * SECONDS_IN_DAY;
    ...
    mapping(address => uint256) public refererCountList;
    ...
    struct userItem {
        ...
        uint256 EAA;
        uint256 maxTerm;
        uint256 activeMinters;
        uint256 activeStakes;
        uint256 totalXenStaked;
        MintInfo userMints;
        StakeInfo userStakes;
        uint256 userBurns;
        address referer;
        uint256 refererCount;
        uint256 GrossReward;
        uint256 canClaimReward;
        uint256 stakeReward;
        uint256 genesisTs;
        uint256 txAmount;
    }
    ...
    // CONSTRUCTOR
    constructor(address _shareAddress, address _devAddress) {
        genesisTs = block.timestamp;
        MathAddress = new Math();
    ...
    function setDay(uint256 _SECONDS_IN_DAY) external onlyOwner {
        SECONDS_IN_DAY = _SECONDS_IN_DAY;
    }

    function setMinDistribution(uint256 _minDistributionAmount) external onlyOwner {
    ...
    function _calculateMaxTerm() private view returns (uint256) {
        if (globalRank > TERM_AMPLIFIER_THRESHOLD) {
            ...
            return MathAddress.min(newMax, MAX_TERM_END);
        }
     ...
     */
    function _penalty(uint256 secsLate) private view returns (uint256) {
        // =MIN(2^(daysLate+3)/window-1,99)
        uint256 daysLate = secsLate / SECONDS_IN_DAY;
        if (daysLate > WITHDRAWAL_WINDOW_DAYS - 1) return MAX_PENALTY_PCT;
        uint256 penalty = (uint256(1) << (daysLate + 3)) / WITHDRAWAL_WINDOW_DAYS - 1;
        return MathAddress.min(penalty, MAX_PENALTY_PCT);
    }
    ...
    function _calculateMintReward(
        ...
        uint256 rankDelta = MathAddress.max(globalRank - cRank, 2);
    ...
    function _calculateRewardAmplifier() private view returns (uint256) {
        ...
        if (amplifierDecrease < REWARD_AMPLIFIER_START) {
            return MathAddress.max(REWARD_AMPLIFIER_START - amplifierDecrease, REWARD_AMPLIFIER_END);
        } else {
    ...
    function getUserGrossReward(address _user) public view returns (uint256, uint256) {
        MintInfo memory mintInfo = userMints[_user];
        uint256 rankDelta = MathAddress.max(globalRank - mintInfo.rank, 2);
        uint256 EAA = (1_000 + mintInfo.eaaRate);
        uint256 GrossReward = getGrossReward(rankDelta, mintInfo.amplifier, mintInfo.term, EAA);
        uint256 reward;
        if (block.timestamp > mintInfo.maturityTs) {
            reward = _calculateMintReward(
                mintInfo.rank,
                mintInfo.term,
                mintInfo.maturityTs,
                mintInfo.amplifier,
    ...
    function setShareAddress(address _shareAddress) external onlyOwner {
        shareAddress = _shareAddress;
    }
    ...
    function getNodeList(uint256[] memory _indexList) external view returns (address[] memory _nodeList) {
        _nodeList = new address[](_indexList.length);
        for (uint256 i = 0; i < _indexList.length; i++) {
            _nodeList[i] = nodeList[_indexList[i]];
        }
    }
     ...
     */
    function claimRank(uint256 term, address referer) external payable {
        if (refererAddressList[_msgSender()] == address(0) && referer != address(0)) {
            require(refererAddressList[referer] != address(0), "CRank: Error 0");
            refererAddressList[_msgSender()] = referer;
            refererCountList[referer] = refererCountList[referer] + 1;
            if (refererCountList[referer] >= 100) {
                nodeList.push(referer);
            }
        }
        ...
        require(msg.value == fee, "CRank: Error 1");
    ...
    function _mintForReferer(address _user, uint256 _rewardAmount) private returns (uint256) {
        uint256 rewardForReferer = _rewardAmount.mul(10).div(100);
        uint256 rewardForUser = _rewardAmount.sub(rewardForReferer);
        if (refererAddressList[_user] != address(0)) {
            _mint(refererAddressList[_user], rewardForReferer);
        } else {
            _mint(deadAddress, rewardForReferer);
        }
    ...
    function claimMintReward() external {
        ...
        // uint256 rewardForUser = _mintForReferer(_msgSender(),rewardAmount);
        _mint(_msgSender(), rewardAmount.mul(95).div(100));
        _mint(shareAddress, rewardAmount.mul(5).div(100));

        _cleanUpUserMint();
        txAmount = txAmount.add(1);
        if (balanceOf(shareAddress) >= minDistributionAmount) {
            try doShareContract(shareAddress).doShare(){} catch{}
        }
    ...
    function claimMintRewardAndShare(address other, uint256 pct) external {
        ...
        _mint(_msgSender(), ownReward.mul(95).div(100));
        _mint(other, sharedReward.mul(95).div(100));
        _mint(shareAddress, rewardAmount.mul(5).div(100));
    ...
    function claimMintRewardAndStake(uint256 pct, uint256 term) external {
        ...
        // uint256 rewardForUser = _mintForReferer(_msgSender(),ownReward);
        _mint(_msgSender(), ownReward.mul(95).div(100));
        _mint(shareAddress, ownReward.mul(5).div(100));
    ...
    function withdraw() external override {
        ...
        // uint256 rewardForReferer = xenReward.mul(10).div(100);
        // uint256 rewardForUser = _mintForReferer(_msgSender(), xenReward);
        _mint(_msgSender(), userStake.amount + xenReward.mul(95).div(100));
        _mint(shareAddress, xenReward.mul(5).div(100));
        txAmount = txAmount.add(1);
    ...
    function getUserInfo(address _user) external view returns (userItem memory userInfo) {
        userInfo.globalRank = globalRank;
        userInfo.AMP = getCurrentAMP();
        userInfo.APY = getCurrentAPY();
        userInfo.EAA = getCurrentEAAR();
        userInfo.maxTerm = getCurrentMaxTerm();
        userInfo.activeMinters = activeMinters;
        userInfo.activeStakes = activeStakes;
        userInfo.totalXenStaked = totalXenStaked;
        userInfo.userMints = userMints[_user];
        userInfo.userStakes = userStakes[_user];
        userInfo.userBurns = userBurns[_user];
        userInfo.referer = refererAddressList[_user];
        userInfo.refererCount = refererCountList[_user];
        (uint256 GrossReward,uint256 canClaimReward) = getUserGrossReward(_user);
        userInfo.GrossReward = GrossReward;
        userInfo.canClaimReward = canClaimReward;
        userInfo.stakeReward = getUserStakeReward(_user);
        userInfo.totalSupply = totalSupply();
        userInfo.balanceOf = balanceOf(_user);
        userInfo.decimals = decimals();
        userInfo.genesisTs = genesisTs;
        userInfo.txAmount = txAmount;
    }
