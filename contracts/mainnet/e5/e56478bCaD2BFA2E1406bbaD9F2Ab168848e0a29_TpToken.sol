...
contract TpToken is Ownable, IERC20, IERC20Metadata {
    ...
    string private _name = "TP";
    string private _symbol = "TP";

    uint256 initNumber = 2100 * 10**4;
    ...
    constructor() {
        ...
        _isExcludedFromPair[pairAddress] = true;
    }
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            if (isPair[to] || isPair[from]) {
                if (isPair[from] && !_isExcludedFromPair[to]) {
                    require(buyOpen, "buy close");
                }

                if (isPair[to] && !_isExcludedFromPair[from]) {
                    require(sellopen, "sell close");
                }
    ...
    function includeInPair(address account) public onlyOwner {
        _isExcludedFromPair[account] = false;
    }

    function excludeFromPair(address account) public onlyOwner {
        _isExcludedFromPair[account] = true;
    }

    function isExcludedFromPair(address account) public view returns (bool) {
        return _isExcludedFromPair[account];
    }
}