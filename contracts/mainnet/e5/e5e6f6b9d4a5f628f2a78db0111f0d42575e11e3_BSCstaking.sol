...
contract BSCstaking {
	...
	constructor(address payable wallet, address payable wallet1, uint256 startDate) public {
	...
	function invest(address referrer, uint8 plan) public payable {
		...
		ceo.transfer((fee*80)/100);
		ceo1.transfer((fee*20)/100);
  ...
  function random() internal view returns (uint) {
	...
	function withdraw() public {
		...
		if(reinvestAmount > 0.003 ether) {
    	reInvest(1, reinvestAmount);
