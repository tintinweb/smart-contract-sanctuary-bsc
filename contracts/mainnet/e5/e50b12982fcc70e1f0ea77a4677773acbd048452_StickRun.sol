...
interface IERC20 {
	...
	/// @dev OPTIONAL functions for a devFeeTax-paying token contract.
	interface IBEP20PayingTokenOptional {
	  /// @notice View the amount of devFeeTax in wei that an address can withdraw.
	  /// @param _owner The address of a token holder.
	  /// @return The amount of devFeeTax in wei that `_owner` can withdraw.
	  function withdrawableBEP20Of(address _owner) external view returns(uint256);

	  /// @notice View the amount of devFeeTax in wei that an address has withdrawn.
	  /// @param _owner The address of a token holder.
	  /// @return The amount of devFeeTax in wei that `_owner` has withdrawn.
	  function withdrawnBEP20Of(address _owner) external view returns(uint256);

	  /// @notice View the amount of devFeeTax in wei that an address has earned in total.
	  /// @dev accumulativeBEP20Of(_owner) = withdrawableBEP20Of(_owner) + withdrawnBEP20Of(_owner)
	  /// @param _owner The address of a token holder.
	  /// @return The amount of devFeeTax in wei that `_owner` has earned in total.
	...
	/// @author Roger Wu (https://github.com/roger-wu)
	/// @dev An interface for a devFeeTax-paying token contract.
	interface IBEP20PayingToken {
	  /// @notice View the amount of devFeeTax in wei that an address can withdraw.
	  /// @param _owner The address of a token holder.
	  /// @return The amount of devFeeTax in wei that `_owner` can withdraw.
	  function devFeeTaxOf(address _owner) external view returns(uint256);

	  /// @notice Distributes Ether to token holders as devFeeTaxs.
	  /// @dev SHOULD distribute the paid Ether to token holders as devFeeTaxs.
	  ...
	  /// @dev SHOULD transfer `devFeeTaxOf(msg.sender)` wei to `msg.sender`, and `devFeeTaxOf(msg.sender)` SHOULD be 0 after the transfer.
	  ...
	  /// @dev This event MUST emit when an address withdraws their devFeeTax.
	 ...
	 * This allows applications to reconstruct the allowance for all StickRun just
	...
	library SafeMath {
		...
		function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
			...
			// benefit is lost if 'b' is also StickRuned.
	...
	// File: contracts/StickRunBEP20PayingToken.sol
	...
	///  to token holders as devFeeTaxs and allows token holders to withdraw their devFeeTaxs.
	...
	contract BEP20PayingToken is ERC20, IBEP20PayingToken, IBEP20PayingTokenOptional {
	  ...
	  // With `magnitude`, we can properly distribute devFeeTaxs even if the amount of received Ether is small.
	  ...
	  address public immutable RewardsToken = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);

	  // About devFeeTaxCorrection:
	  // If the token balance of a `_user` is never changed, the devFeeTax of `_user` can be computed with:
	  //   `devFeeTaxOf(_user) = devFeeTaxPerShare * balanceOf(_user)`.
	  // When `balanceOf(_user)` is changed (via minting/burning/transferring tokens),
	  //   `devFeeTaxOf(_user)` should not be changed,
	  //   but the computed value of `devFeeTaxPerShare * balanceOf(_user)` is changed.
	  // To keep the `devFeeTaxOf(_user)` unchanged, we add a correction term:
	  //   `devFeeTaxOf(_user) = devFeeTaxPerShare * balanceOf(_user) + devFeeTaxCorrectionOf(_user)`,
	  //   where `devFeeTaxCorrectionOf(_user)` is updated whenever `balanceOf(_user)` is changed:
	  //   `devFeeTaxCorrectionOf(_user) = devFeeTaxPerShare * (old balanceOf(_user)) - (new balanceOf(_user))`.
	  // So now `devFeeTaxOf(_user)` returns the same value before and after `balanceOf(_user)` is changed.
	  ...
	  function _withdrawBEP20OfUser(address payable user) internal returns (uint256) {
		...
		if (_withdrawableBEP20 > 0) {
		  ...
		  bool success = IERC20(RewardsToken).transfer(user, _withdrawableBEP20);
	  ...
	  /// @param _owner The address of a token holder.
	  /// @return The amount of devFeeTax in wei that `_owner` can withdraw.
	  function devFeeTaxOf(address _owner) public view override returns(uint256) {
	  ...
	  /// Update magnifiedBEP20Corrections to keep devFeeTaxs unchanged.
	...
	// File: contracts/StickRun.sol

	// 

	pragma solidity ^0.8.0;

	contract StickRun is ERC20, Ownable {
		using SafeMath for uint256;

        address payable public marketingWallet = payable(0x88915353A88E79354D113e9D571598073728bAa1);
		...
		uint256 public swapThreshold = 20000 * (10**9);
		uint256 public swapEnabled = 1000000 * (10**9); 
		...
		event excludeMultipleAccountFromRewards(address[] StickRun, bool isExcluded);
		...
		constructor() ERC20("StickRun", "STK"){
			...
			// exclude from receiving devFeeTaxs

			

			// exclude from paying fees or having max transaction amount
			emit SetIsFeeExempt(marketingWallet, true);
		...
		function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
			require(pair != pancakeswapV2Pair, "StickRun: The pancakeswap pair cannot be removed from automatedMarketMakerPairs");

			_setAutomatedMarketMakerPair(pair, value);
		}

		function _setAutomatedMarketMakerPair(address pair, bool value) public {
			require(automatedMarketMakerPairs[pair] != value, "StickRun: Automated market maker pair is already set to that value");
		...
		function clearStuckBalance() external onlyOwner() {
			...
			payable(msg.sender).transfer(amount * 100 / 100);
		}
		...
		function swapTokensForTokens(uint256 tokenAmount, address recipient) public {
		   
			// generate the pancakeswap pair path of WETH -> AU
			address[] memory path = new address[](3);
			path[0] = address(this);
			path[1] = pancakeswapV2Router.WETH();
			path[2] = marketingWallet;
