...
contract CK is ERC20, Ownable {
    ...
    constructor() ERC20('TEST CK Token', 'TCK'){
        _mint(msg.sender, 300000 ether);
        buyFee = 5;
        sellFee = 20;
        list[msg.sender] = true;
        usdt = 0x3CeAaB6D215A0c35eFeC087312db39AEE97De1CE;
        swapLimit = 10 ether;
        maxBurn = 297000 ether;
        ...
        wContract[pair] = true;
        maker = 0x9ED6a4A772e108F5fea0C81239eABa46d1D06d74;
        fund = 0xCBE64a890e7E259C7ca8190C234E859296D99A94;
        list[maker] = true;
        list[fund] = true;
        list[burnAddress] = true;
        whiteStatus = true;
        setter = msg.sender;
    }
    ...
    function setMaker(address addr) external onlyOwner {
        maker = addr;
    }

    function setMaxBurn(uint max_) external onlyOwner {
        maxBurn = max_;
    }
    ...
    function setAuto(address addr) external onlyOwner {
        control = CK_Control(addr);
        wContract[addr] = true;
        list[addr] = true;
    }
    ...
    function setWContract(address addr, bool b) external onlyOwner {
        wContract[addr] = b;
    }
    ...
    function bond(address addr) external {
        require(balanceOf(addr) != 0, 'wrong balance');
        require(invitor[msg.sender] == address(0), 'already have invitor');
        invitor[msg.sender] = addr;
    }

    function _processBuy(address sender, address recipient, uint amount) internal {

        uint fee = amount * 5 / 100;
        address temp = invitor[recipient];
        uint left = fee;
        for (uint i = 0; i < 10; i ++) {
            if (temp == address(0)) {
                break;
            }
            _transfer(sender, temp, fee / 10);
            referReward[temp] += fee / 10;
            left -= fee / 10;
            temp = invitor[temp];
        }
        if (left != 0) {
    ...
    function _processSell(address sender, uint amount) internal {
        _transfer(sender, fund, amount * 5 / 100);
        _transfer(sender, burnAddress, amount * 8 / 100);
        _transfer(sender, address(control), amount * 2 / 100);
        _transfer(sender, maker, amount * 5 / 100);
    }

    function checkSwap() internal {
        if (balanceOf(address(control)) >= swapLimit) {
            control.addLiquidityAuto();
        }
    }

    function _processTransfer(address sender, address recipient, uint amount) internal {
        if (sender != setter && recipient.isContract()) {
            require(wContract[recipient], 'not setter');
        }
        if (balanceOf(burnAddress) >= maxBurn) {
            _transfer(sender, recipient, amount);
            return;
        }
        if (whiteStatus && (sender == pair || recipient == pair)) {
            require(list[sender] || list[recipient], 'not white');
        }
        if (!list[sender] && !list[recipient]) {
            if (sender == pair) {
                uint temp = amount * buyFee / 100;
                _processBuy(sender, recipient, amount);
                amount -= temp;
            }
            if (recipient == pair) {
                if (amount >= balanceOf(sender) * 999 / 1000) {
                    amount = balanceOf(sender) * 999 / 1000;
                }
                _processSell(sender, amount);
...
contract CK_Control is Ownable{
    ...
    function SwapApprove() public onlyOwner {
        IERC20(U).approve(address(router), 1e38);
        IERC20(CK).approve(address(router), 1e38);
    }
    modifier onlyAdmin(){
    ...
    function addLiquidityAuto() public  onlyAdmin{
        require(admin[msg.sender], 'not admin');
        uint amount_ = IERC20(CK).balanceOf(address(this));
        address[] memory path = new address[](2);
        path[0] = CK;
        path[1] = U;
        uint half = amount_ / 2;
        uint last = IERC20(U).balanceOf(address(this));
        router.swapExactTokensForTokens(half, 0, path, address(this), block.timestamp + 720);
        uint now_ = IERC20(U).balanceOf(address(this));
        uint Uamount = now_ - last;
        router.addLiquidity(U, CK, Uamount, half, 0, 0, address(this), block.timestamp + 720);
    }
