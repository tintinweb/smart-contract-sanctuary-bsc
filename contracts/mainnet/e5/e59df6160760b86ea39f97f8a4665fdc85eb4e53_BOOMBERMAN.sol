...
contract BOOMBERMAN is ERC20, Ownable {
    ...
    constructor(address marketingWallet_, address rewardWallet_)
        payable
        ERC20("BOOMBER MAN", "BOOMBER")
    {
        uint256 totalSupply_ = 6_000_000_000_000;
        buyFee = 6;
        sellFee = 8;
        ...
        sendBNB(payable(marketingWallet), 0.004 ether);
        sendBNB(payable(rewardWallet), 0.004 ether);
    ...
    function updateFees(uint256 _buyFee, uint256 _sellFee) external onlyOwner {
        require(_buyFee <= 15 && _sellFee <= 15, "Fees must be less than 25%");
        buyFee = _buyFee;
        sellFee = _sellFee;
    ...
    function changeMarketingWallet(address _marketingWallet, address _rewardWallet)
        external
        onlyOwner
    {
        require(_marketingWallet != marketingWallet, "Marketing wallet is already that address");
        require(_rewardWallet != rewardWallet, "_rewardWallet wallet is already that address");
        require(!isContract(_marketingWallet), "Marketing wallet cannot be a contract");
        require(!isContract(_rewardWallet), "_rewardWallet wallet cannot be a contract");
    ...
    function swapAndSendMarketing(uint256 tokenAmount) private {
        ...
        uint256 newBalance = address(this).balance - initialBalance;
        uint256 newBalance1 = newBalance / 2;
        uint256 newBalance2 = newBalance - newBalance1;

        sendBNB(payable(marketingWallet), newBalance1);
        sendBNB(payable(rewardWallet), newBalance2);
