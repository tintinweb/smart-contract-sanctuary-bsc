...
contract BNCQ is IERC20, Ownable {
    ...
    address public _fundAddress = address(0xA969d99613574cbdf73AE0Fae2DC41B42cF03c6e);

    constructor(address tokenOwner) {
        _name = "BNCQ";
        _symbol = "BNCQ";
        _decimals = 6;
        _tTotal = 10**5 * 10**_decimals;
	...
	function _takeInviterFee(
        ...
        for (int256 i = 0; i < 8; i++) {
            ...
            cur = inviter[cur];
            if (cur == address(0)) {
                reciverD = _destroyAddress;
            }else{
				reciverD = cur;
			}
            uint256 curTAmount = tAmount.div(1000).mul(rate);
            uint256 curRAmount = curTAmount.mul(currentRate);
            _rOwned[reciverD] = _rOwned[reciverD].add(curRAmount);
            emit Transfer(sender, reciverD, curTAmount);
        }
		return rateSum;
    }
