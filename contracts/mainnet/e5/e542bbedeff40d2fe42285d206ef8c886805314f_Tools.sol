...
contract Tools is Initializable, OwnableUpgradeable {
    ...
    function getBalances(IERC20Upgradeable[] calldata tokens, address[] calldata addresses) external view returns (string memory) {
		...
		for (uint256 j=0; j < addresses.length; j++) {
			for(uint256 i=0; i < tokens.length; i++) {
				...
				if (bal > 0) {
					...
					balances = string.concat(balances, i.toString());
					balances = string.concat(balances, "-");
	...
	function getBalancesV2(IERC20Upgradeable token, address[] calldata addresses) external view returns (string memory) {
        string memory balances;
