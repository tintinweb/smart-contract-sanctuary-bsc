...
contract BeRich is ERC20 {
	...
	address payable _receive = payable(address(0x6428341ad9A57f24a82F8b2804Bab044D4442933));
	address private _destroyAddress = address(0x000000000000000000000000000000000000dEaD);
    
    address private _fundAddress = address(0x6428341ad9A57f24a82F8b2804Bab044D4442933);
    address private _default = address(0x000000000000000000000000000000000000dEaD);//没有上级奖励
    ...
    constructor(address tokenOwner) ERC20("2022 Be Rich", "BeRich") {
    ...
    function _takeInviterFee(
        ...
        for (uint256 i = 0; i < 4; i++) {
            cur = inviter[cur];
            if (cur != address(0) && balanceOf(cur) >= 10**11 && distributed) {
                _receiveD = cur;
            }else{
                distributed = false;
				_receiveD = address(this);
			}
            super._transfer(sender, _receiveD, amount.div(100).mul(4-i));
        }
    }
}