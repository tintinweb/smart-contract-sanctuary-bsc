...
abstract contract AbsBizVarUtil is Ownable, Context, Random {
    ...
    function getBinders(address account) external returns (address[] memory){
       return _binders[account];
    }

    function getValidBinders(address account) external returns (address[] memory){
...
abstract  contract AbsBizUtil is AbsBizVarUtil {
    ...
    function _upgradeCommon(address user,uint256 tokenQuantity) private {
        _addTokenBalance(user, tokenQuantity);
