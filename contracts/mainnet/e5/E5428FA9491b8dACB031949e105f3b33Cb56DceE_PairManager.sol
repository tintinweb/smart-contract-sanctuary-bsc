...
contract PairManager is Initializable, IPairManager, Block, PairManagerStorage {
    ...
    function claimAmountFromLiquidityPool(
        ...
        if (quoteAmount > 0)
            quoteAsset.transfer(user, quoteAmount);

        if (baseAmount > 0)
            baseAsset.transfer(user, baseAmount);
    }
    ...
    function cancelGridOrders(bytes32[] memory _orderIds) public override onlyLiquidityPool returns (uint256 base, uint256 quote) {
        ...
        if(base > 0) baseAsset.transfer(liquidityPool, base);
        if(quote > 0) quoteAsset.transfer(liquidityPool, quote);
    }
...
//    {}
    ...
    function accumulatePoolExchangedData(
        ...
        for (uint256 i = 0; i < _orderIds.length; i++) {
            if (
                BitMathLiquidity.checkBitIsSetOrNot(
                    i,
                    soRemovablePosBuy,
                    soRemovablePosSell
                )
            ) continue;
            _d.orderId = _orderIds[i];
    ...
    {
        return (singleSlot.pip, basisPoint);
    }
    ...
    function _accumulatePoolExchangedData(
        ...
        if (isBuy) {
            ...
            params.baseAdjust -= (filledSize.toI128() -
                _feeRefundCalculator(partialFilled, _feeShareRatio, _feeBasis)
                    .toI128());
        }
    }
