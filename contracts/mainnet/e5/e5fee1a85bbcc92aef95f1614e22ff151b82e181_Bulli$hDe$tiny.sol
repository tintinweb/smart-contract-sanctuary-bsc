...
contract Bulli$hDe$tiny  is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    Bulli$hDe$tinyDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Bulli$h De$tiny", "De$tiny") {
        marketingWallet = payable(0x2D35f99aFBBEFE4769fe9c0885215f2d5276402E);
        devWallet = payable(0x3DBAdE6b7d9A6d0FcDF80ce27eA336BD9Feacb27);
        ...
        dividendTracker = new Bulli$hDe$tinyDividendTracker(
            payable(this),
            router,
            0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56,
            "Bulli$hDe$tinyTRACKER",
            "De$tinyTRACKER"
        );
...
contract Bulli$hDe$tinyDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    Bulli$hDe$tiny public Bulli$hDe$tinyContract;
    ...
    constructor(
        ...
        defaultToken = token;
        Bulli$hDe$tinyContract = Bulli$hDe$tiny(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Bulli$hDe$tiny_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Bulli$hDe$tiny_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Bulli$hDe$tiny contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(Bulli$hDe$tinyContract);

            uint256 prevBalance = Bulli$hDe$tinyContract.balanceOf(address(this));
            ...
            {
                uint256 received = Bulli$hDe$tinyContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    Bulli$hDe$tinyContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
