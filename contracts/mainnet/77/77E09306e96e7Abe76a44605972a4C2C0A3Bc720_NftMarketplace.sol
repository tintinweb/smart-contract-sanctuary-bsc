...
contract RoyaltiesInfo is AccessControlEnumerable {
    ...
    /// @notice Holds information about royalties that are set by an admin.
    ...
    /// @notice Event is emitted when an admin of the contract (`manager`) has added a new royalty config (`royaltyReceiver` will receive `royaltyPercentage` percentages) for a collection `token`.
    ...
    /// @notice Event is emitted when an admin of the contract (`manager`) has deleted royalty config for a collection `token`.
    ...
    /// @notice Event is emitted when an admin of the contract (`manager`) has changed value for defaultFeeForOwner variable from `oldValue` to `newValue`.
    ...
    /// @notice Admin function (ROYALTY_MANAGER role) for deleting royalty config for a collection `token`.
    /// @dev Changes mapping royaltiesInfo.
    /// @param token Address of a collection.
    ...
    /// @return royaltyAmount Amount of royalty in tokens.
