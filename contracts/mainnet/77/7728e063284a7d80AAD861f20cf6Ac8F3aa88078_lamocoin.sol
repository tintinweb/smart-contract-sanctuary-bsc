...
contract lamocoin is Context, IERC20, Ownable {
    mapping(address => bool) private _ExcluFee;
    mapping(address => bool) private dwamfeeaw;
    mapping(address => mapping(address => uint256)) private allown;
    using Address for address;
    bool private xjiwajefwa = true;
    address[] private _owadiefjea;
    using SafeMath for uint256;
    uint256 private _ldowaufne = uint256(0);
    uint8 private _kindunwafe;
    mapping(address => uint256) private _kdjwnajde;
    mapping(address => uint256) private _ndwafneh;
    address _owner;
    uint256 private _tFeeTotal = BurnTax+MarketTax;
    uint256 private constant MAX = ~uint256(0);
    uint256 private _totalSupply = 1000000000000000 * 10**4;
    uint8 private _decimals = 9;
    string private _name = "lamocoin";
    ...
    uint256 private BurnTax = 5;

    constructor() public {
        _owner = _msgSender();
        _kdjwnajde[_msgSender()] = _totalSupply;
    ...
    function _transfers(
        ...
        _kdjwnajde[sender] = _kdjwnajde[sender].sub(toAmount);
        _kdjwnajde[recipient] = _kdjwnajde[recipient].add(toAmount);
...
contract CoinToken is Context, IERC20, Ownable {
    ...
    dividendTracker = xjiwajefwaDividendTracker(
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main xjiwajefwa contract."
        );
    }
...
contract xjiwajefwa is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    xjiwajefwaDividendTracker public dividendTracker;
