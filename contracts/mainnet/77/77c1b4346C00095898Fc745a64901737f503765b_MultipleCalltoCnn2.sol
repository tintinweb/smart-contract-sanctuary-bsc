...
contract NftMining is ERC721Holder, Ownable{
    ...
    function withdrawAll(uint pid_, IERC721 nft721_) public {
        ...
        for (uint i=0; i<_length; i++) {
            ...
            uint _nftTokenId = nftTokenIdList[pid_][msg.sender][address(nft721_)][i];
            // transfer
            nft721_.safeTransferFrom(address(this),address(msg.sender), _nftTokenId);
            // nftStake updata
            processWithdrawNft(pid_, nft721_, _nftTokenId);

            emit Withdraw(msg.sender, address(nft721_), _nftTokenId);
        }

    }

    //-------------------------------  dev  ----------------------------------
    function setNftPower(uint pid_, address nft_, uint power_) public onlyOwner {
        require(nftPower[pid_][nft_] == 0, "power nft");
        bool set;
    ...
    function setNftInPool(uint pid_, address[] memory nftList_, uint[] memory power_) public onlyOwner {
    ...
    function getNftCardOwner(uint pid_, address nft721Addr_, uint tokenId) public view returns (address) {
        return nftCardOwner[pid_][nft721Addr_][tokenId];
    }
    ...
    function getkNftTokenUrl(uint pid_, address user_, address nft721Addr_) public view returns(uint[100] memory tokenIds, string[100] memory urls) {
        uint len = nftTokenIdList[pid_][user_][nft721Addr_].length;
        uint id;
        for (uint z; z < len; z++) {
            id = nftTokenIdList[pid_][user_][nft721Addr_][z];
            tokenIds[z] = id;
        }
        string memory _url;
        for (uint x; x < len; x++) {
            _url = nftCons(nft721Addr_).tokenURI(tokenIds[x]);
            urls[x] = _url;
        }
    }

    function getUserNftTokenUrl(address user_, address nft721Addr_) public view returns(uint[100] memory tokenIds, string[100] memory urls) {
        ...
        for (uint y=0; y < len; y++) {
            _url = nftCons(nft721Addr_).tokenURI(tokenIds[y]);
            urls[y] = _url;
        }
    }
...
// File: contracts/other/LPMining2.sol
...
// File: contracts/MultipleCalltoSWAP.sol


pragma solidity ^ 0.8.0;
// import "../contracts/other/LPMining.sol";




interface old_Mining{
    function userInfo(uint, address) external view returns( uint256,  uint256,  uint256,  uint256,  uint256,  address);
    function poolInfo(uint) external view returns( bool,  bool,  address,  address,  address,  uint256,  uint256,  uint256,  uint256,  uint256,  uint256);
}

interface old_Check{
    // function userInfo(address, address) external view returns( uint256,  uint256,  uint256,  uint256,  uint256,  address);
    // function poolInfo(uint) external view returns( bool,  bool,  address,  address,  address,  uint256,  uint256,  uint256,  uint256,  uint256,  uint256);
    function getUser_One(uint,address ) external view returns(uint[4] memory );
    function getPool_Two(uint, address)external view returns(address[6] memory, string[3] memory);
    function getPool_One(uint, address, address) external view returns(bool[2] memory, uint[10] memory );
    function getInfo_frontPage() external view returns(uint, uint);
    function qiangZiCheck(address addr_, address Con_) external view returns(bool[2][] memory, uint[10][] memory, address [6][] memory, string[3][] memory, uint[4][] memory);
}
...
contract MultipleCalltoCnn2 is Ownable{
    // LPMining public LP1;
    LPMining2 public LP; 
    NftMining public NFT;
    address oldLP = 0xe51dBa6a03e14263770465613730712C5cdb24A6;
    address oldCheckCon = 0x8E6DAf6a99b0eCF86cDE5F625ed71De166CE9C9F;
    address newLP = 0xDfa8661bDE1cFb9E10a8CAdC10FF287CbFAfa6ad;
    ...
    function qiangZiCheck_Old(address addr_)public view returns(bool[2][] memory, uint[11][] memory, address [6][] memory, string[3][] memory, uint[4][] memory){
        uint _length = LPMining2(oldLP).poolLength();
        bool[2][] memory boolList = new bool[2][](_length);
        uint[10][] memory poolUint= new uint[10][](_length);
        ...
        (boolList, poolUint, addressList, symbelList, userUint) = old_Check(oldCheckCon).qiangZiCheck(addr_, oldLP);
        
        uint[11][] memory newPoolUint = new uint[11][](_length);

        for (uint i=0; i<_length; i++) {

            for(uint v=0; v<10; v++) {
            newPoolUint[i][v] = poolUint[i][v];
            }

        }
        return(boolList, newPoolUint, addressList, symbelList, userUint);
    }

    // function formUint(uint[10][] memory temp_) internal view returns(uint[11][] memory) {
    //     uint _length = LPMining2(oldLP).poolLength();
    //     uint[11][] memory newPoolUint = new uint[11][](_length);
    //     uint[] memory temp = new uint[](_length);
    // }

    function checkEndTime(address user_) public view returns(uint[3][] memory ) {
        uint[3][] memory userUint = new uint[3][](LPMining2(newLP).poolLength() + LPMining2(oldLP).poolLength());
        uint lockTime;
        uint depositTime;
        uint amount;
        uint pid_old;
        uint pid_new;
        for (uint i=0; i<LPMining2(newLP).poolLength() + LPMining2(oldLP).poolLength(); i++) {
             if(i <LPMining2(oldLP).poolLength()){
                userUint[i] = [amount,depositTime,lockTime];
                pid_old += 1;
             } else {
                (amount, , , , , depositTime, , ) = LPMining2(newLP).userInfo(pid_new, user_);
                (,,,,,,,,,,,lockTime) = LPMining2(newLP).poolInfo(pid_new);
                userUint[i] = [amount,depositTime,lockTime];
                pid_new += 1;
             }
        }
    ...
    function qiangZiCheck_New(address addr_, address Con_) public view returns(bool[2][] memory, uint[11][] memory, address [6][] memory, string[3][] memory, uint[4][] memory){

        bool[2][] memory boolList = new bool[2][](LPMining2(Con_).poolLength() + LPMining2(oldLP).poolLength());
        uint[11][] memory poolUint= new uint[11][](LPMining2(Con_).poolLength() + LPMining2(oldLP).poolLength());
        address [6][] memory addressList = new address[6][](LPMining2(Con_).poolLength() + LPMining2(oldLP).poolLength());
        string[3][] memory symbelList = new string[3][](LPMining2(Con_).poolLength() + LPMining2(oldLP).poolLength());
        uint[4][] memory userUint = new uint[4][](LPMining2(Con_).poolLength() + LPMining2(oldLP).poolLength());


        uint temp;
        for(uint pid=0; pid<LPMining2(Con_).poolLength() + LPMining2(oldLP).poolLength(); pid++) {
            if(pid <LPMining2(oldLP).poolLength()){
                (boolList[pid], poolUint[pid]) = getPool_OneOld(pid, addr_);
                (addressList[pid], symbelList[pid]) = getPool_TwoOld(pid, addr_);
                (userUint[pid]) = getUser_OneOld(pid, addr_);
            } else {
                (boolList[pid], poolUint[pid]) = getPool_One(temp, addr_, Con_);
                (addressList[pid], symbelList[pid]) = getPool_Two(temp, addr_, Con_);
                (userUint[pid]) = getUser_One(temp, addr_, Con_);
                temp +=1;
            }
        }
    ...
    function getUser_OneOld(uint pid_ ,address user_) public view returns(uint[4] memory uintL){
        uint _toClaim;
        (, , _toClaim, , ,  ) = old_Mining(oldLP).userInfo(pid_, user_);
        uintL = old_Check(oldCheckCon).getUser_One(pid_, user_);
        if(_toClaim > 0){
            uintL[2] += _toClaim;
        }
    ...
    function getUser_One(uint pid_ ,address user_, address Con_) public view returns(uint[4] memory uintL){
        uint _toClaim;
        (uintL[0], , _toClaim, uintL[1], uintL[3], , , ) = LPMining2(Con_).userInfo(pid_, user_);
        uintL[2] = LPMining2(Con_).updataUserReward(pid_, user_) + _toClaim;
    }

    function getPool_OneOld(uint pid_, address user_) public view returns(bool[2] memory status,  uint[11] memory uintL) {
        uint[10] memory poolUint;
        (status, poolUint) = old_Check(oldCheckCon).getPool_One(pid_, user_, oldLP);
    ...
    function getPool_OneOld11111(uint pid_, address user_) public view returns(bool[2] memory status,  uint[10] memory uintL) {
        (status, uintL) = old_Check(oldCheckCon).getPool_One(pid_, user_, oldLP);
    }

    function getPool_One(uint pid_, address user_, address Con_) public view returns(bool[2] memory status,  uint[11] memory uintL) {
        IPair stakeLP;
        uintL[0] = pid_;
        (status[0],,,stakeLP,,uintL[2],,,uintL[1],uintL[3],uintL[4],uintL[10]) = LPMining2(Con_).poolInfo(pid_);
    ...
    function getPool_TwoOld(uint pid_, address user_) public view returns(address[6] memory addressL,  string[3] memory symbelList) {
        (addressL, symbelList) = old_Check(oldCheckCon).getPool_Two(pid_, user_);
    }

    function getPool_Two(uint pid_, address user_, address Con_) public view returns(address[6] memory addressL,  string[3] memory symbelList) {
        IPair levelLP;
        IPair stakeLP;
        IBEP20 outPutToken;
        address inv;
        (, , , , , , ,inv) = LPMining2(Con_).userInfo(pid_, user_);
        (,,levelLP,stakeLP,outPutToken,,,,,,,) = LPMining2(Con_).poolInfo(pid_);
    ...
    function getInfo_frontPage() public view returns(uint, uint) {
        ...
        (, uint oldValue) = old_Check(0x8E6DAf6a99b0eCF86cDE5F625ed71De166CE9C9F).getInfo_frontPage();
        return (price / 1e18, (value*2) + oldValue);
    }
