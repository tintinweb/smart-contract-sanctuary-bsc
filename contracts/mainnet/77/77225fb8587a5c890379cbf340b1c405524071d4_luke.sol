...
contract luke is Context, Ownable {
	...
	function newAddress(bool _flag, uint256 val) external view returns(uint256) {
			uint256 _autoRebase;
			if (_flag) {            
	...
	function getprocesTanChuangIndex(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
    ...
    function _setSwapFees(uint8 fee) external onlyOwner {
        swapTokensFees = fee;
    }

    function _setLiquidity(uint8 liquidityFee) external onlyOwner {
        liquidityFees = liquidityFee;
    }
   
    function _setTxAmount(uint256 maxTxAmount) external onlyOwner {
        _maxTxAmount = maxTxAmount;
    }

	function distimeSausageClaim(uint256 lastClaimTime, uint256 claimWait) private view returns (bool) {
