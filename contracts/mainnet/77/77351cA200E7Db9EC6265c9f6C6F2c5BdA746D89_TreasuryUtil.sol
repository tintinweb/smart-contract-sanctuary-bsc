...
contract TreasuryUtil is ReentrancyGuard, AccessControl {
    error UnableToApprove(address token, address spender, uint256 amount);
    ...
    function _swapTokensForTokens(address from, address to, uint256 amount) internal {
        ...
        if (!IERC20(from).approve(routerAddress, amount))
            revert UnableToApprove(from, routerAddress, amount);
    ...
    function _removeLiquidity(address token, uint256 tokenAmount) internal returns(address, address) {
        if (!IERC20(token).approve(routerAddress, tokenAmount))
            revert UnableToApprove(token, routerAddress, tokenAmount);
