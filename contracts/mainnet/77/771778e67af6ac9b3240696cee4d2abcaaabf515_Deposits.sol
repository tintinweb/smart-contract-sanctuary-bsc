...
contract Deposits is _deposits {
    ...
    function treasuryClaim(uint256 amount, ERC20 token) external {
        _treasuryClaim(amount, token);
    }
    function getAppByIndex(uint256 aIndex) external view returns(address) {
        return address(dsb_gameDeposits[aIndex]);
    }
