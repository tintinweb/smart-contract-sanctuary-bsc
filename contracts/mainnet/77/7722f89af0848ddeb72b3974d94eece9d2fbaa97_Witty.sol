...
contract Witty is ReentrancyGuard {
    ...
    event BuyerCancel(address indexed from,uint tradeid,uint tradeidcount,uint amt,uint AdminAmount,bool status,uint time);
    event SellerCancel(address indexed from,uint tradeid,uint tradeidcount,uint amt,uint AdminAmount,bool status,uint time);
    ...
    function buyerTransfer(uint _tradeID,uint _amount,uint _count,uint8 choice) internal {
        ...
        require(traderList[buyer][_tradeID].buyerAmt >= _amount + _refPercent,"Buyer not have enough money");
        if (choice == 0) {
        if (_Trade.favour == 1) {
    ...
    function sellerCancel(uint _tradeid,uint _count) public isLock Trade(_tradeid) nonReentrant {
        ...
        if (_Trade.types == 1) {
            ...
            feeAmount,cancelStatus[msg.sender][_tradeid],block.timestamp);
        }
    ...
    function dispute(uint _tradeid,uint _time) public isLock {
        require(block.timestamp > trade[_tradeid].expiry + _time, "Not enough time to raise dispute");
