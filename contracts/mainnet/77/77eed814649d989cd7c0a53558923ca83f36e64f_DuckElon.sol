...
//  Total Tax: 15% (Reflections: 5% | Liquidity: 5% | Marketing: 5%)
// 
//  Telegram: t.me/DuckElonOfficial
...
contract DuckElon is IBEP20, Ownable {
    
    using SafeMath for uint256;

    string _name = unicode"DuckElon";
    string _symbol = unicode"ElonDuck";
    ...
    uint256 public totalFee = liquidityFee + reflectionFee + marketingFee; // Reflections fee - 5%, Liquidity fee - 5% , Marketing/Development Fee - 5% 
    ...
    uint256 launchedAt;
    uint256 private _swapThreshold = _totalSupply / 50; // 2% max tx
...
//		router = IDEXRouter(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3); // Testnet
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!isMaxHoldExempt[recipient]){
            require((_balances[recipient] + (amount - amount * totalFee / _feeDenominator)) <= _maxHold, "Wallet cannot hold more than 2%");
        }
        _balances[sender] = _balances[sender].sub(amount, "Insufficient Balance");
