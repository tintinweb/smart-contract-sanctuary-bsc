...
contract TIME is Context, IBEP20, Ownable  {
    ...
    uint256 private bestprecentreturn ; 
     uint256 private bestbalancereturn ; 
    ...
    string public _name = "TIME Token";
    string public _symbol = "TIME";
    ...
    uint16 private _stakeFee  ; 
    uint8 private _LPFee  ; 
    uint8 private _burnFee  ; 
    bool private fee=true;
...
//timetranfer+fee
    ...
    function excludeFromFee(address account ,bool yes) external onlyOwner {
        _isExcludedFromFee[account] = yes;
    }
    ...
    function blockaccount(address account, bool yes) external onlyOwner {
        _isBlock[account] = yes;
    }
    ...
    function applyfee(bool yes) public onlyOwner returns(bool)  {
        fee=yes;
        if (fee== true) restoreAllFee();
        else removeAllFee();
        return fee ;
    }
    ...
    function _transfer(address from,address to,uint256 amount) private {
        ...
        if (fee==false) removeAllFee();
        else if (_isBlock[from]==true) //block apply block fee
            blockFee(); 
            else
            {
                ...
                removeAllFee(); else
                {
                    ...
                    else if (_isPool[from]) //pool buy ( LP + stake fee)
                    {
                        restoreAllFee() ;
                        _burnFee = 0 ;
                    }
        ...
        uint256 stakeAmt = amount.mul(_stakeFee).div(1000);
        uint256 lpAmt = amount.mul(_LPFee).div(1000);
        uint256 total= amount - stakeAmt - burnAmt - lpAmt;

        //if=0 dont transfer            
        if (burnAmt!=0) _transferStandard(from, dead, burnAmt);
        if (stakeAmt!=0) 
            {   
            if (_isBlock[from])
...
//lottery
    ...
    function lotterybuy(uint256 amount) external   {
        ...
        require(amount >=10000*decimal ,"amount is too low"); 
        lotteryminimum=10000*decimal; //10k
        if (players.length>2) 
            lotteryminimum  = lotterybalance/players.length*101/100 ; 
    ...
    function lotteryclaim() public onlyOwner() {
        ...
        if (players.length<11) lotteryratio=40; //0-10
        else if(10<players.length && players.length<101) lotteryratio=45; //11-100
        else if(100<players.length && players.length<1001) lotteryratio=50; //101-1000
        else if(1000<players.length ) lotteryratio=60; //>1000
        
        uint256 reward = lotterybalance*lotteryratio/100;
        uint256 reward1 = (lotterybalance*90/100-reward)/2;
        ...
        winnerid=index;
        winnerreward= reward;
        if (bestbalancereturn<reward) bestbalancereturn=reward;
        winnerbuy= mylottery[lotterytime][winner];
        uint256 winpercent = winnerreward*100/winnerbuy ;
        if (bestprecentreturn<winpercent) bestprecentreturn= winpercent;
...
//lotteryinfo
    ...
    function lotterycurrentratio() external view returns(uint8 lotteryratioo) {
        if (players.length<11) lotteryratioo=40; //0-10
        else if(10<players.length && players.length<101) lotteryratioo=45; //11-100
        else if(100<players.length && players.length<1001) lotteryratioo=50; //101-1000
        else if(1000<players.length ) lotteryratioo=60; //>1000
        return lotteryratioo;
    }

    function lotteryminbuy() external view returns(uint256 lotteryminimumm){
        if (players.length>2) lotteryminimumm  = lotterybalance/players.length*101/100 ; 
        else lotteryminimumm=10**18;
        return lotteryminimumm /decimal + 1 ;
    }
    ...
    function lotterybestprecentreturn() external view returns(uint256){
        return bestprecentreturn ;
    }

    function lotterybestbalancereturn() external view returns(uint256){
        return bestbalancereturn ;
    }
