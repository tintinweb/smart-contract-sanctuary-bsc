...
/*  Baby Cat 
    https://t.me/BabyCatTokenOfficial
*/
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional, Ownable {
  ...
  address public DividendsToken = address(0x0173295183685F27C84db046B5F0bea3e683c24b);
...
contract BabyCat is Context, ERC20, Ownable {
    using SafeMath for uint256;
   ...
   BabyCATDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("Baby Cat", "BabyCAT") {
        _buyRewardsFee = 4;
        _buyLiquidityFee = 1;
		_buyMarketingFee = 4;
        _buyDevFee = 1;
    	...
    	dividendTracker = new BabyCATDividendTracker();

    	liquidityWallet = owner();
		
		marketingWallet = payable(0x26876D2d7F11Ae54299D9D42f03EAaE2a8e2e1CB);
        _devWallet = payable(0xe6C6973Ab8E357AB8Ff1d7ff58408411ca184e0e);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BabyCAT: The dividend tracker already has that address");

        BabyCATDividendTracker newDividendTracker = BabyCATDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BabyCAT: The new dividend tracker must be owned by the BabyCAT token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "BabyCAT: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BabyCAT: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BabyCAT: Account is already the value of 'excluded'");
    ...
    function addToBlacklist(address account, bool blacklisted) public onlyOwner {
        require(_isBlacklisted[account] != blacklisted, "BabyCAT: Account is already the value of 'blacklisted'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BabyCAT: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BabyCAT: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "BabyCAT: The liquidity wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "BabyCAT: The marketing wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BabyCAT: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BabyCAT: Cannot update gasForProcessing to same value");
...
contract BabyCATDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("BabyCAT_Dividend_Tracker", "BabyCAT_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function withdrawDividend() public pure override {
        require(false, "BabyCAT_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BabyCAT contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 0 && newClaimWait <= 86400, "BabyCAT_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BabyCAT_Dividend_Tracker: Cannot update claimWait to same value");
