...
interface Invite {
    function idoConfirm(address _user) external;
    function memberAddress(address _user) external view returns ( bool );
}

contract UPPreSale is Ownable {
    ...
    mapping(address => bool) public boughtUP;
    ...
    function purchaseaUP() external returns (bool) {
        saleAmount = saleAmount.add(amount);
        require(boughtUP[msg.sender] == false, 'Already participated');
        require(startTimestamp < block.timestamp,"Not started yet");
        require(started == true," not started");

        boughtUP[msg.sender] = true;
        IERC20(USDT).safeTransferFrom(msg.sender ,DAOAddress, amount);
        uint _purchaseAmount = _calculateSaleQuote(amount);
        IERC20(aUP).safeTransfer(msg.sender, _purchaseAmount);

        invite.idoConfirm(msg.sender);
        return true;
    }
