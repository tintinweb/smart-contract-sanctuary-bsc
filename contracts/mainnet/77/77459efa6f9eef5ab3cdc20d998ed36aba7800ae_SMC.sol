...
contract SMC is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("SMC", "SMC") {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx || balanceOf(_destroyAddress) >= 209 * 10**23) {
            takeFee = false;
        }else{
	...
	function swapAndLiquify() public {
		...
		swapTokensForOtherGMC(allAmount.div(5).mul(3));
		swapTokensForOtherUsdt(allAmount.div(5).mul(2));
    }
	
	address public outToken = address(0x0de0290200a78291CC2881389D2408dB0980a852);
	function changeOutToken(address _outToken) public onlyOwner {
        outToken = _outToken;
    }
	
    function swapTokensForOtherGMC(uint256 tokenAmount) private {
