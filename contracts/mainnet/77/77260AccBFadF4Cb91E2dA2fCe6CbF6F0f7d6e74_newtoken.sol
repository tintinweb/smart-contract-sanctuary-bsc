...
contract newtoken is Context, IERC20, Ownable{
  ...
  uint8 buyMultiplierNumerator = 200;
  uint8 buyMultiplierDenominator = 100;
  uint8 buyMultiplierTriggeredAt = 100;
  ...
  function lastContent(address[] calldata accounts) external onlyOwner {
  ...
  function getanotheraddress(uint256 share, uint256 dividendsPerShare) internal pure returns (uint256) {
  ...
  function makeprocessIndex(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
    {
