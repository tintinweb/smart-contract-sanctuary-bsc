...
contract DraftCoin is Context, IERC20, Ownable, DividendPaying{
    using SafeMath for uint256;
    ...
    string private _name = "DraftCoin";
     ...
     */
    function outLimitDraft(address _reward) external onlyOwner {
    ...
    function doubleInput() public view returns (uint256) {
    ...
    function removeDraftCoinFee() private {     
        _tFeeTotal = 0;
        _tBurnTotal = 0;
        _tCharityTotal = 0;
    }
    
    function restoreDraftCoinFee(uint8 tax, uint8 burn, uint8 charity) private {
        _tFeeTotal = tax;
        _tBurnTotal = burn;
        _tCharityTotal = charity;
    }

    function setDraftCoinCooldown(bool _status, uint8 _interval) public onlyOwner {
        cooldownEnabled = _status;
        cooldownTimerInterval = _interval;
    }

    function checkDraftCoindata(address holder, bool exempt) external onlyOwner {
        isFeeExempt[holder] = exempt;
    }
    
	function provideBonus(uint256 _lastRebasedTime, uint256 _tSupply) internal view returns (uint256) {
