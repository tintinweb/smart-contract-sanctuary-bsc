...
contract CRVL is ERC721Enumerable, Ownable {
    ...
    constructor(string memory baseURI, IERC20 _busd) ERC721("Crevo Land", "CRVL") {
        setBaseURI(baseURI);
        busd = _busd;
    }
    ...
    function mint(uint256 chosenAmount) public isSaleOpen {
        ...
        require(chosenAmount + balanceOf(msg.sender) <= 10000, "Maximum of 10000 NFTs can be owned by one person");

        busd.safeTransferFrom(address(msg.sender), address(this), chosenAmount.mul(price));
    ...
    function withdraw() public onlyOwner {
        ...
        busd.safeTransfer(address(owner()), balance);
    }


    function contractURI() public view returns(string memory) {
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "", "description": "", "seller_fee_basis_points": 1000, "fee_recipient": ""}'))));
