...
contract StakingMapV2MilestoneForERC1155WithLimitAndExtraReward is Ownable, Pausable {
    ...
    iCardNFT NFT_CARD_REWARDS_CONTRACT;
    uint256 EXTRA_NFT_CARD_ID;
    uint256 REWARD_ID_FOR_EXTRA_NFT_CARD;
    ...
    constructor(
        ...
        uint32[] memory _reward_limit_rewardIds,
        uint32[] memory _reward_limit_remaining,
        uint32[] memory _reward_limit_max,
        iCardNFT _nft_card_rewards_contract,
        uint256 _extra_nft_card_id,
        uint256 _reward_id_for_extra_nft_card
    ) {
        ...
        setRewardsLimits(_reward_limit_rewardIds, _reward_limit_remaining, _reward_limit_max);
        NFT_CARD_REWARDS_CONTRACT = _nft_card_rewards_contract;
        EXTRA_NFT_CARD_ID = _extra_nft_card_id;
        REWARD_ID_FOR_EXTRA_NFT_CARD = _reward_id_for_extra_nft_card;
    }

    function setRewardsLimits(uint32[] memory _rewardIds, uint32[] memory _remaining, uint32[] memory _max) public onlyOwner {
    ...
    function receiveRewards(uint256 _rewardId, uint256 _rewardAmount) external whenNotPaused {
        ...
        if (REWARD_ID_FOR_EXTRA_NFT_CARD == _rewardId) {
            for (uint256 i = 0; i < _rewardAmount; i++) {
                NFT_CARD_REWARDS_CONTRACT.mint(_msgSender(), EXTRA_NFT_CARD_ID);
            }
        }
    ...
    function receiveMultipleRewards(uint256[] calldata _rewardIds, uint256[] calldata _rewardAmounts) external whenNotPaused {
        ...
        for (uint256 i = 0; i < _len; i++) {
            ...
            if (REWARD_ID_FOR_EXTRA_NFT_CARD == _rewardIds[i]) {
