...
import {ISkewVault} from "./intf/ISkewVault.sol";
import {IUnitroller} from "./intf/IUnitroller.sol";
import {ISToken} from "./intf/ISToken.sol";
import {IERC1271} from "./intf/IERC1271.sol";
...
contract Skew is Initializable,OwnableUpgradeable, IERC1271 {
    ...
    uint256 public exchangeDecimal;
    ...
    address public XVS;
    address inch_aggregation_router;
    address vault;
    uint256 public totalDeposit;
    address public treasuryAddress;
    uint256 withdrawFee;
    uint256 withdrawFeeBase;

    // MAGICVALUE is defined in eip 1271,
    // as the value to return for valid signatures
    bytes4 internal MAGICVALUE;
    bytes4 internal INVALID_SIGNATURE;
    ...
    function initialize(address _dodo, 
        address _unitroller, 
        address _vault,
        address _vBase, 
        address _vQuote,
        ...
        MAX_AMOUNT = 2**256-1;
        exchangeDecimal = 10**6;
        withdrawFee = 1000;
        withdrawFeeBase = 10000;
    ...
    function depositToVenus(address _vToken, uint256 _amount) external {
        require( _msgSender() == vault, "Caller is not vault contract");
    ...
    function claimRewardFromVenus( address _vToken ) external onlyOwner {
        address _token = getUnderlyingAddress(_vToken);
        IvBEP20 vToken;
        vToken = IvBEP20(_vToken);
        uint256 rate = vToken.exchangeRateStored();
        uint256 curVBalance = vToken.balanceOf(address(this));
        uint256 totalAmount = rate * curVBalance / (10**18);
        require( totalAmount > totalDeposit, "no reward");
        uint256 rewardAmount = totalAmount - totalDeposit;
        assert(vToken.redeemUnderlying(rewardAmount)==0);
        uint256 feeAmount = rewardAmount * withdrawFee / withdrawFeeBase;
        IERC20(_token).transfer( treasuryAddress, feeAmount);
        rewardAmount -= feeAmount;
        IERC20(_token).approve(vault, rewardAmount);
        ISkewVault(vault).depositReward(rewardAmount);
    }

    function claimXVSReward( ) external onlyOwner {
        IUnitroller troll = IUnitroller(Unitroller_address);
    ...
    function SwapXVSReward( uint minOut, bytes calldata _data) external onlyOwner{
        ...
        address vaultToken = ISkewVault(vault).getVaultToken();
        require(amount>=desc.amount, "Not enough xvs token");
        require(desc.dstReceiver == address(this), "only Skew contract can receive");
        require(address(desc.srcToken) == XVS, "src token must be XVS");
        require(address(desc.dstToken) == vaultToken,"swap token must the same token as vault token");

        IERC20(XVS).approve(inch_aggregation_router, desc.amount);
        ...
        uint256 assetAmount = _Swap( minOut, _data);
        //transfer asset to vault as a reward 
        uint256 feeAmount = assetAmount * withdrawFee / withdrawFeeBase;
        IERC20(desc.dstToken).transfer( treasuryAddress, feeAmount);
        assetAmount -= feeAmount;

        // IERC20(desc.dstToken).approve(vault, assetAmount);
        // ISkewVault(vault).depositReward(assetAmount);
        IERC20(desc.dstToken).transfer(_msgSender(), assetAmount);
    }
