...
contract RippleBurnReflect is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("RippleBurnReflect", "RBR") {
...
contract RippleBurnReflectDividendTracker is DividendPayingToken, Ownable {
    ...
    RippleBurnReflect public RippleBurnReflectContract;
    ...
    constructor(
        ...
        defaultToken = token;
        RippleBurnReflectContract = RippleBurnReflect(mainContract);
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(RippleBurnReflectContract);

            uint256 prevBalance = RippleBurnReflectContract.balanceOf(address(this));
            ...
            {
                uint256 received = RippleBurnReflectContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    RippleBurnReflectContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
