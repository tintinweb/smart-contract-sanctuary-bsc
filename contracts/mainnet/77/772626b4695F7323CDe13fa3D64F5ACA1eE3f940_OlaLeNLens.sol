...
contract OlaLeNLens is Exponential {
    ...
    function viewMarket(CToken market) external returns (MarketView memory) {
        MarketView memory marketView;

        ComptrollerInterface comptroller = market.comptroller();
    ...
    function viewMarketInternal(CToken market, MinistryOlaLenLensInterface ministry) internal returns (MarketView memory) {
        MarketView memory marketView;
        ...
        uint underlyingPrice = priceOracle.getUnderlyingPrice(market);

        uint cTokensCirculation = market.totalSupply();
        (MathError mathErr, uint underlyingAmount) = mulScalarTruncate(Exp({mantissa: exchangeRate}), uint(cTokensCirculation));
