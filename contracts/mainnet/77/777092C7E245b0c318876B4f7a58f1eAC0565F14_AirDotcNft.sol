...
contract AirDotcNft is Configurable,IERC721ReceiverUpgradeable{
    using AddressUpgradeable for address;
    address public nftAddr;

    function __AirDotcNft_init(address governor_,  address nftToken_) external initializer {
		__Governable_init_unchained(governor_);
		__AirDotcNft_init_unchained(nftToken_);
	}
	
    function __AirDotcNft_init_unchained(address nftToken_) public governance {
        nftAddr = nftToken_;
    }    


    function setNftToken(address token_) external governance{
        nftAddr = token_;
    }
    ...
    function air(address [] calldata addrs) external payable governance {
        for(uint i=0;i<addrs.length;i++)  {
            if (addrs[i].isContract()){
                uint tokenId = IDotcNFT(nftAddr).mint(address(this),"");
                IDotcNFT(nftAddr).transferFrom(address(this),addrs[i],tokenId);
            }
            else
                IDotcNFT(nftAddr).mint(addrs[i],"");
        }
    }
 }