...
contract ComLand721  is ERC721, Owner {
      ...
      uint256 price2 = 0.01 ether;
      uint256 price3 = 0.01 ether;
      uint256 price4 = 0.01 ether;
      uint256 price5 = 0.01 ether;


      uint256 private latestTokenId1 = 0;
      uint256 private latestTokenId2 = 20000;
      uint256 private latestTokenId3 = 30000;
      uint256 private latestTokenId4 = 33000;
      uint256 private latestTokenId5 = 34000;
    ...
    function mintTYPE1(address recipient_) external payable returns (uint256) {
        require(miners[msg.sender] || msg.value >= price1, "get out");
        uint256 tokenId = _getNextTokenId(NFTType.COMLANDN);
        _safeMint(recipient_,tokenId);
        _setTokenURI(tokenId,string(abi.encodePacked(uri,tokenId,".json")));
        _incrementTokenId(NFTType.COMLANDN);
        return tokenId;
    }

    
    function mintTYPE2(address recipient_) external payable returns (uint256) {
        require(miners[msg.sender] || msg.value >= price2, "get out");
        uint256 tokenId = _getNextTokenId(NFTType.COMLANDR);
        _safeMint(recipient_,tokenId);
        _setTokenURI(tokenId,string(abi.encodePacked(uri,tokenId,".json")));
        _incrementTokenId(NFTType.COMLANDR);
        return tokenId;
    }

    
    function mintTYPE3(address recipient_) external payable returns (uint256) {
        require(miners[msg.sender] || msg.value >= price3, "get out");
        uint256 tokenId = _getNextTokenId(NFTType.COMLANDSR);
        _safeMint(recipient_,tokenId);
        _setTokenURI(tokenId,string(abi.encodePacked(uri,tokenId,".json")));
        _incrementTokenId(NFTType.COMLANDSR);
        return tokenId;
    }

    
    function mintTYPE4(address recipient_) external payable returns (uint256) {
        require(miners[msg.sender] || msg.value >= price4, "get out");
        uint256 tokenId = _getNextTokenId(NFTType.COMLANDSSR);
        _safeMint(recipient_,tokenId);
        _setTokenURI(tokenId,string(abi.encodePacked(uri,tokenId,".json")));
        _incrementTokenId(NFTType.COMLANDSSR);
        return tokenId;
    }

    
    function mintTYPE5(address recipient_) external payable returns (uint256) {
        require(miners[msg.sender] || msg.value >= price5, "get out");
        uint256 tokenId = _getNextTokenId(NFTType.COMLANDSSSR);
        _safeMint(recipient_,tokenId);
        _setTokenURI(tokenId,string(abi.encodePacked(uri,tokenId,".json")));
        _incrementTokenId(NFTType.COMLANDSSSR);
        return tokenId;
    }
    ...
    function _getNextTokenId(NFTType _type) internal view returns (uint256) {
        if(_type == NFTType.COMLANDN) {
            ...
            return latestTokenId4 + 1;
        }else if(_type == NFTType.COMLANDSSSR) {
            return latestTokenId5 + 1;
        }
    }
