...
abstract contract Token is IERC20, Ownable {
    ...
    constructor (
        ...
        maxTXAmount = 5000000 * 10 ** Decimals;
                ...
                if (_swapPairList[to]) {
                    if (!inSwap) {
                        ...
                        if (contractTokenBalance > 0) {
                            uint256 swapFee = _maishui + _buyLPDividendFee + _waishui + _sellLPDividendFee + _sellLPFee;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            uint256 swapFee;
            if (isSell) {
                swapFee = _waishui + _sellLPDividendFee + _sellLPFee;
            } else {
                require(tAmount <= maxTXAmount);
                swapFee = _maishui + _buyLPDividendFee;
            }
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        uint256 fundAmount = fistBalance * (_maishui + _waishui) * 2 / swapFee;
        uint firstfundamount = fundAmount / 2;
        uint secfundamount = fundAmount - firstfundamount;
        address secFundAddress = 0xca8504ae5Fd03d72935A519d145aD30b0f7868Aa;//   营销
    ...
    function setmaishui(uint256 shui) external onlyOwner {
        _maishui = shui;
    }
    uint256 public startAddLPBlock;

    function setwaishui(uint256 shui) external onlyOwner {
        _waishui = shui;
    }
...
contract QatarInu is Token {
    constructor() Token(
        ...
        address(0xd5bB183c4A7BA16A3638C4Dc5719374c340A2B58),//fund address
    
        address(0xd5bB183c4A7BA16A3638C4Dc5719374c340A2B58)//dev address
    ){
