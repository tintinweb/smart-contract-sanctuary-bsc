...
contract JTD_NFT_Lottery is ERC721, ERC721Enumerable {
    using Strings for uint256;
    ...
    uint256 public maxNftsPerWallet = 500;
    uint256 public mintPriceBnb = 0.03 ether;
    uint256 public mintPriceBusd = 10 ether;
    bool private priceInBUSD = true;    
    address public BUSD;
    ...
    uint256 private emergencyRecoveryDate = type(uint256).max;
    ...
    constructor(address admin_, address _BUSD, string memory _name, string memory _symbol) ERC721(_name, _symbol) {
        _admin = admin_;
        BUSD = _BUSD;
    }
    ...
    function mintPriceBnbInBusd() public view returns (uint256) {
        address[] memory pathFromBNBToBUSD = new address[](2);
        pathFromBNBToBUSD[0] = router.WETH();
        pathFromBNBToBUSD[1] = BUSD;
        return router.getAmountsOut(mintPriceBnb, pathFromBNBToBUSD)[1];
    }

    function mintPriceBusdInBnb() public view returns (uint256) {
        address[] memory pathFromBUSDToBNB = new address[](2);
        pathFromBUSDToBNB[0] = BUSD;
        pathFromBUSDToBNB[1] = router.WETH();
        return router.getAmountsOut(mintPriceBusd, pathFromBUSDToBNB)[1];
    }

    function checkNewPrice() internal {
        if(priceInBUSD) mintPriceBnb = mintPriceBusdInBnb();
        else mintPriceBusd = mintPriceBnbInBusd();
    }

    function mintWithBNB(uint256 numberOfNfts) external payable{
        ...
        if(_mintsStarted >= MAX_NFT_SUPPLY) lotteryFinished = true;
        if(msg.value - price > 0) payable(msg.sender).transfer(msg.value - price);
    }

    function mintWithBUSD(uint256 numberOfNfts) external {
        if(pendingCount == 0 || numberOfNfts == 0 || balanceOf(msg.sender) + numberOfNfts > maxNftsPerWallet || lotteryFinished) return;
        if(_mintsStarted + numberOfNfts > MAX_NFT_SUPPLY) numberOfNfts = MAX_NFT_SUPPLY - _mintsStarted;
        if(!priceInBUSD) checkNewPrice();
        uint256 price = mintPriceBusd * numberOfNfts;
        IBEP20(BUSD).transferFrom(msg.sender, address(this), price);
        minterAtNonce[_nonce] = msg.sender;
        if(address(this).balance < vrfCost*numberOfNfts) sellBusdForBnb();
    ...
    function sellBusdForBnb() internal {
        IBEP20(BUSD).approve(address(router), type(uint256).max);
        uint256 tokensThatTheContractWillSell = IBEP20(BUSD).balanceOf(address(this));
        if(tokensThatTheContractWillSell > 100 ether) tokensThatTheContractWillSell = 100 ether;
    ...
    function  sellBnbForBusd() internal {
        address[] memory pathFromBNBToBUSD = new address[](2);
        pathFromBNBToBUSD[0] = router.WETH();
        pathFromBNBToBUSD[1] = BUSD;
    ...
    function swapBnbForBusd() external onlyOwner{
        sellBnbForBusd();
    }
    ...
    function supplyRandomness(uint256 nonce, uint256[] memory randomNumbers) external onlyVRF {
        address _to = minterAtNonce[nonce];
            ...
            uint256 index = (randomNumbers[0] % pendingCount);
            uint256 tokenId = _popPendingAtIndex(index);
            _totalSupply++;
            _mintToken(_to, tokenId);
            emit NftMinted(_to, tokenId);

            if(tokenId > 1234) return;
            if(tokenId < 5) {
                prizePoolWon += 2000 ether;
                return;
            }
            if(tokenId < 15) {
                prizePoolWon += 1000 ether;
                return;
            }
            if(tokenId < 35) {
                prizePoolWon += 500 ether;
                return;
            }
            if(tokenId < 235) {
                prizePoolWon += 100 ether;
                return;
            }
            prizePoolWon += 20 ether;
    }

    function claimPrize(uint256 nftId) public {
        if(prizeClaimed[nftId] || ownerOf(nftId) != msg.sender || nftId > 1234) return;
        if(nftId < 5) {
            if(lotteryFinished){
                prizeClaimed[nftId] = true;
                IBEP20(BUSD).transfer(msg.sender, 2000 ether);
            }
            return;
        }
        ...
        if(nftId < 15) {
            if(lotteryFinished){
                prizeClaimed[nftId] = true;
                IBEP20(BUSD).transfer(msg.sender, 1000 ether);
            }
            return;
        }
        ...
        if(nftId < 35) {
            prizeClaimed[nftId] = true;
            IBEP20(BUSD).transfer(msg.sender, 500 ether);
            return;
        }

        if(nftId < 235) {
            prizeClaimed[nftId] = true;
            IBEP20(BUSD).transfer(msg.sender, 100 ether);
            return;
        }

        prizeClaimed[nftId] = true;
        IBEP20(BUSD).transfer(msg.sender, 20 ether);
        return;
    }

    function claimAllPrizes() external {
        for (uint i = 0; i < balanceOf(msg.sender); i++) {
            claimPrize(tokenOfOwnerByIndex(msg.sender, i));
        }
    }
    ...
    function _popPendingAtIndex(uint256 _index) internal returns (uint256) {
        ...
        if (_index != pendingCount) _pendingIds[_index] = _getPendingAtIndex(pendingCount) - _index;
        pendingCount--;
        return tokenId;
    }
    ...
    function setMintPriceWithBUSD(uint256 value) external onlyOwner{
        mintPriceBusd = value;
        priceInBUSD = true;
    }
    ...
    function rescueBUSD() external onlyOwner {
        ...
        emergencyRecoveryDate = block.timestamp + 365 days;
        uint256 amount = IBEP20(BUSD).balanceOf(address(this)) - prizePoolWon;
        IBEP20(BUSD).transfer(_admin, amount);
    }

    function rescueBUSDemergency() external onlyOwner {
        if(emergencyRecoveryDate > block.timestamp) return;
