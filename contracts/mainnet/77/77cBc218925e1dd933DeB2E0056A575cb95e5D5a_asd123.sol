...
contract asd123 is Ownable {
    ...
    function getAmounts(address token, address lp) private view returns(uint256 amount0Out, uint256 amount1Out) {
    ...
    function xd(
        ...
        (uint256 amount0Out, uint256 amount1Out) = getAmounts(token, lp);
        ...
        (amount0Out, amount1Out) = getAmounts(token, lp);
        IUniswapPair(lp).swap(amount0Out, amount1Out, address(this), new bytes(0));
