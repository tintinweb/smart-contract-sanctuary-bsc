...
contract FarmAuction is Ownable, ReentrancyGuard {
    ...
    /**
     * @notice Constructor
     * @param _SELFToken: address of the $SELF token
     ...
     * @param _amount: amount of the bid in $SELF token
    ...
    function claimAuction(uint256 _auctionId) external nonReentrant {
        ...
        SELFToken.safeTransfer(address(msg.sender), claimableAmount);
    ...
    /**
     * @notice Close an auction round, and store $SELF leaderboard threshold
     * @param _bidLimit: minimal $SELF committed to be a winner
     * @dev Callable by operator
     */
    ...
    /**
     * @notice Claim $SELF for all bids by bidders in leaderboard
     * @param _auctionId: auction id
     * @param _bidders: addresses of bidders
    ...
    function recoverToken(address _token, uint256 _amount) external onlyOwner {
        require(_token != address(SELFToken), "Recover: Cannot be SELF token");
