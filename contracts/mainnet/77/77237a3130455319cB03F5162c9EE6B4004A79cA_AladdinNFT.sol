    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize() public initializer {
        ...
        __ERC721_init("Aladdin NFT", "ALD");
        idoAddress = address(0xd97Bc4dE09D27A6B553EEf42cE364d194a0D5187);
    }

    function setUrl(string memory url) external onlyOwner {
        baseUrl = url;
    }
    function seIdoAddress(address _ido) external onlyOwner {
        idoAddress = _ido;
    }
    /**
    ...
    /**
     * @dev mint an additional nft token
     * @param _to nft token receiving address
     */

    function mint(address _to,uint256 _tokenId) external nonReentrant returns (uint256 tokenid) {
        require(
            _msgSender() == idoAddress,
            "Caller not in Whitelist"
        );
        _safeMint(_to, _tokenId);
        _totalSupply = _totalSupply + 1;
        _allTokens.add(_tokenId);
        return _tokenId;
    }
    function multiBurnFrom(address from,uint256[] memory tokenIds) external nonReentrant{
         require(
            _msgSender() == idoAddress,
            "Caller not in Whitelist"
        );
        for (uint256 i=0;i< tokenIds.length;i++){
            require(ownerOf(tokenIds[i]) == from,"Burn:Not nft owner");
            _burn(tokenIds[i]);
            _allTokens.remove(tokenIds[i]);
        }
    ...
    {
        Trait memory lamp = IIDO(idoAddress).getNftTrait(_tokenId);
        return
            string(
                abi.encodePacked(
                   '{"id":',
                    ...
                    lamp.name,
                    '", "image": "',
                    string(abi.encodePacked(baseUrl, lamp.name, ".png")),
                    '", "trait":[',
                    getTraitsJson(lamp),
                    "]"
                    "}"
    ...
    function getTraitsJson(Trait memory lamp)
        private
        pure
        returns (string memory)
    {
        return
            string(
                abi.encodePacked('{ "level":"', lamp.level.toString(), '"}')
            );
    }
    ...
    function getUserAllTokens(address _user)
        external
        view
    ...
    function tokenOfOwnerByIndex(address owner, uint256 index) public view virtual  returns (uint256) {
        require(index < _userTokens[owner].length(), "ERC721Enumerable: owner index out of bounds");
...
interface IIDO {
    function getNftTrait(uint256 tokenId)
        external
        view
