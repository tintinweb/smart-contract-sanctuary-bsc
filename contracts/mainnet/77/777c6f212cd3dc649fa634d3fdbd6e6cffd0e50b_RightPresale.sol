...
contract RightPresale {
	...
	event FeePayed(address indexed user, uint256 totalAmount);
        ...
        BUSD = IERC20(0x08D4e1D410316F807440cfd35805c8eafF3B53eB);
    }

        function buyRight(address ref) public payable {
    
      //  require(BUSD.balanceOf(address(this)) >= msg.value*tokenprice,'the smart contract dont hold the enough tokens');
        uint256 amountRight;  
        uint256 refComm;      
        uint256 amountBuy= msg.value;
        amountRight= amountBuy.mul(price);
        refComm=amountRight.div(1000);
        totalsold += msg.value*price;
        BUSD.safeTransfer(msg.sender, amountRight/10);
        devWallet.transfer(msg.value);
        BUSD.safeTransfer(ref, refComm);
    }
    function setPrice(uint256 rate) external{
      //  require(msg.sender == devWallet , ' you are not the admin');
         price=rate;
    }
