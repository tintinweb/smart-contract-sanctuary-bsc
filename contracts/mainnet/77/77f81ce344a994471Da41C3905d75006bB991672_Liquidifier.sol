...
contract Operations is AccessControl, ReentrancyGuard {
    ...
    function diference(uint256 amountA, uint256 amountB)
        internal
        view
        returns (uint256 _diference)
    {
        _diference = amountA - amountB;
        return _diference;
    }
    ...
    function cakeCalculation(
        ...
        (
            ...
            bool isRegular
        ) = IMasterChef(_chef).poolInfo(id);
        uint256 cakePerBlock = IMasterChef(_chef).cakePerBlock(true);
        (uint256 userAmount, uint256 userRewardDebt) = IMasterChef(_chef)
        ...
        uint256 boostedAmount = (userAmount *
            (IMasterChef(_chef).getBoostMultiplier(msg.sender, id))) /
            BOOST_PRECISION;

        return (boostedAmount * accCakePerShare) / 1e18 - userRewardDebt;
    }
    ...
    function rewardsRegistry(
        ...
        (uint256 accCakePerShare, , , , ) = IMasterChef(_chef).poolInfo(id);
