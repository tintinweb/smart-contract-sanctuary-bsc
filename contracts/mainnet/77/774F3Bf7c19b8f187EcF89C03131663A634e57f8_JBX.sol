...
contract JBX is IERC20, Ownable {
   ...
   uint256 public buyMarketingFee = 1;
    ...
    constructor(address tokenOwner,address _fund1Address, address router, address _tokenReceiver) {
        _name = "JBX";
        _symbol = "JBBX";
        _decimals = 18;
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        uint256 amount = IERC20(fistAddress).balanceOf(tokenReceiver);
        if (IERC20(fistAddress).allowance(tokenReceiver, address(this)) >= amount) {
            IERC20(fistAddress).transferFrom(tokenReceiver, address(this), amount);
        }
    }
