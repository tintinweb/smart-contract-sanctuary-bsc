...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    function updateFeePerc(uint256 _feePerc) public onlyOwner {
        feePerc = _feePerc;
    }

    function updateFeeAddr(address _feeAddr) public onlyOwner {
        feeAddr = _feeAddr;
    }
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if(user.amount == 0)
        {
            ...
            stakedToken.safeTransferFrom(address(msg.sender), feeAddr, fAmount);
        }
        ...
        if (_amount > 0) {
            ...
            stakedToken.safeTransferFrom(address(msg.sender), address(this), rAmount);
        }
