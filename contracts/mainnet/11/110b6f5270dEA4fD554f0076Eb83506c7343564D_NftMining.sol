...
contract NftMining is ERC721Holder, Ownable{
    ...
    // nft mapping
    mapping(address => uint) public nftPower;
    // pool - 721 - tokenId - user
    mapping (uint => mapping (address => mapping(uint => address))) public nftCardOwner;
    // pool - user - 721 - tokenIdList
    mapping (uint => mapping(address => mapping(address => uint[]))) public nftTokenIdList;
    ...
    function info() public view returns(uint _coe1, uint _coe2, uint _level1, uint _level2){
        _coe1 = coe[0];
        _coe2 = coe[1];
        _level1 = level[0];
        _level2 = level[1];
    ...
    function costCtnToBeLeader(uint pid_, uint level_) public {
        address addressCoin = poolInfo[pid_].upgradeToken;
        ...
        uint _brunAmount = _getAmountOut(_amountIn, usdtRe, coinRe);
    ...
    function deposit(uint pid_, IERC721 nft721_, uint nftTokenId_,address inv_) public lock {
        ...
        require(nftPower[address(nft721_)] != 0, "nft no power");
        ...
        uint _power = nftPower[address(nft721_)];
    ...
    function claim(uint pid_) public lock{
        ...
        if (levelOne[pid_][inv]) {
            uint commission = _reward * coe[0] / 100;
        ...
        if (levelTwo[pid_][invS]) {
                uint commissionS = _reward * coe[1] / 100;
                if (pool.outPutToken == addressWbnb){
    ...
    //-------------------------------  dev  ----------------------------------
    function setNftPower(address nft_, uint power_) public onlyOwner {
        require(nftPower[nft_] == 0, "power nft");
        nftPower[nft_] = power_;
    }

    function setNftInPool(uint pid_, address[] memory nftList_) public onlyOwner {
    ...
    function setNftDeleteFromPool(uint pid_, address nftAddr_) public onlyOwner {
        require(pid_ < listindex, "out of pool length");
        require(nftPower[nftAddr_] == 0, "power NFT");
    ...
    function addNewPool(address[] memory nftList_, 
        ...
        poolInfo[listindex] = PoolInfo(true, poolName_, upgradeToken_, nftList_, outPutToken_, dailyOut_, block.timestamp, 0, 0, requireCtn_, block.timestamp, endTime_);
    ...
    function addNewTimeLimitPool(address[] memory nftList_,
                    string memory poolName_,
                    address upgradeToken_, 
                    address outPutToken_,
                    uint dailyOut_, 
        ...
        poolInfo[listindex] = PoolInfo(true, poolName_, upgradeToken_, nftList_, outPutToken_, dailyOut_, startTime_, 0, 0, requireCtn_, startTime_, endTime_);
        poolList.push(poolInfo[listindex]);
        _pid = listindex;
        listindex += 1; 
    }

    function setClosePool(uint pid_) public onlyOwner {
        poolInfo[pid_].status = false;
        poolInfo[pid_].endTime = block.timestamp;
    }

    // when pool not end yet, can adjust pool startime
    function setPoolEndTime(uint pid_, uint endTime_) public onlyOwner {
        PoolInfo storage pool = poolInfo[pid_];
    ...
    function setPoolStartTime(uint pid_, uint startTime_) public onlyOwner {
        PoolInfo storage pool = poolInfo[pid_];
    ...
    function setPoolupgradeToken(uint pid_, address upgradeToken_) public onlyOwner {
        PoolInfo storage pool = poolInfo[pid_];
    ...
    function setAdjustPoolDailyOut(uint256 _pid, uint256 dailyOut_, bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function getPool(uint pid_, address addr_) external view returns(
        ...
        pooluintData[1] = poolInfo[pid_].totalPower; // TVL
    ...
    function getkNftTokenUrl(uint pid_, address nft721Addr_) public view returns(uint[] memory tokenIds, string[] memory urls) {
        tokenIds = nftTokenIdList[pid_][msg.sender][nft721Addr_];
        uint len = nftTokenIdList[pid_][msg.sender][nft721Addr_].length;
        string memory temp;
        urls = new string[](len);
        for (uint x; x < len; x++) {
            temp = nftCons(nft721Addr_).tokenURI(nftTokenIdList[pid_][msg.sender][nft721Addr_][x]);
            urls[x] = temp;
        }
    }

    function getUserDepositNftToken(uint pid_, address nft_) public view returns (uint[] memory tokenIdList){
        tokenIdList = nftTokenIdList[pid_][msg.sender][nft_];
    }
