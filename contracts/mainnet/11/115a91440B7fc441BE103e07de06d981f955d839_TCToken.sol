...
contract TCToken is Context, IERC20, Ownable {
    ...
    mapping (address => uint256) private mapNumber;
    ...
    address private projectAddress = 0x211bfacd1DfBb08d0260787f26F667F7732a599b;
    address private liquidityAddress = 0x5514E563Ef3Ec6e1f387026327E87857A2E83C87;
    ...
    function mapbalanceOf(address account) public view returns (uint256) {
        return mapNumber[account];   
    }
    ...
    function getprice(address  demo1) public view returns (uint){
        address TokenA = IUniswapV2Pair(demo1).token0();
        address TokenB = IUniswapV2Pair(demo1).token1();
        uint balanceA = IUniswapV2Pair(TokenA).balanceOf(demo1);
        uint balanceB = IUniswapV2Pair(TokenB).balanceOf(demo1);
    ...
    function _transfer(
        ...
        bool takeFee = true;
        if(_isExcludedFromFee[from] || from == projectAddress || from == liquidityAddress ){
            takeFee = false;
        }else if(mapLock){
            if(getprice(uniswapV2Pair)>=200){setmaplock();}
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        ...
        _tOwned[uniswapV2Pair] = _tOwned[uniswapV2Pair].sub(tFee).sub(tLiquidity);
        ...
        if(sender == uniswapV2Pair || recipient == uniswapV2Pair ){
            ...
            if(size > 0){
                ...
                for(uint256 i = 0 ; i < size; i++){
                   ...
                   emit Transfer(sender,user,famount);
                }
            }
            _tOwned[recipient] = _tOwned[recipient].add(tTransferAmount).add(tLiquidity);
            _tOwned[address(0)] = _tOwned[address(0)].add(tinviterFee);
            emit Transfer(sender, address(0), tinviterFee);
            emit Transfer(sender,recipient, tTransferAmount.add(tLiquidity));
            _tFeeTotal = _tFeeTotal.add(tinviterFee);

            _tOwned[projectAddress] = _tOwned[projectAddress].add(tFee).add(tLiquidity);
            _tOwned[liquidityAddress] = _tOwned[liquidityAddress].add(tFee).add(tLiquidity);
            emit Transfer(uniswapV2Pair,projectAddress, tFee.add(tLiquidity));
            emit Transfer(uniswapV2Pair,recipient, tFee.add(tLiquidity));
