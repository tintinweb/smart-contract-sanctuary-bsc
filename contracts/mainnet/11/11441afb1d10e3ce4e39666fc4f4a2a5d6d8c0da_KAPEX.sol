...
contract KAPEX is Context, IBEP20, LockOwnable { // change contract name
    ...
    address payable public _feeManager = payable(0xdE2379768295879F71d6886EFCD9c5A580884C26); // kapex liquidity address
    address payable public _liquidityManager = payable(0xc262B612BB9fc186713d31874688A4E93B1df6b1); // kapex liquidity address

    string private _name = "KAPEX Token"; // token name
    string private _symbol = "KAPEX"; // token symbol
    uint8 private constant _decimals = 18; // token decimals(1 token can be divided into 1e_decimals parts)

    uint256 private _totalSupply = 12*10**(12 + _decimals);
    ...
    uint256 public _transactionFee = 1250; // 12.5%
    ...
    function mint(address account, uint256 amount) external {
        require(msg.sender == _feeManager, "KAPEX: can't mint");
    ...
    function setTransactionFee(uint256 fee) external onlyOwner {
        ...
        require(block.timestamp > _buyFeeReducedEndTime || fee >= _buyFee, "KAPEX: Transaction fee should be between (buyFee)% - 12.5% (buyFee - 1250)");
        require(block.timestamp > _sellFeeIncreaseEndTime || fee <= _sellFee, "KAPEX: Transaction fee should be between 0% - (sellFee)% (0 - sellFee)");
        _transactionFee = fee;
    ...
    function setBuyFee(uint256 duration, uint256 newBuyFee) external onlyOwner {
        require(newBuyFee < _transactionFee, "KAPEX: Decreased buy fee should be less than (transfer fee)%");
        _buyFee = newBuyFee;
    ...
    function setSellFee(uint256 duration, uint256 newSellFee) external onlyOwner {
        require(newSellFee > _transactionFee && newSellFee <= 2500, "KAPEX: Increased sell fee should be between (transfer fee)% - 25%");
        _sellFee = newSellFee;
