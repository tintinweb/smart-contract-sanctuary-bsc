...
contract WIENERStake {
    ...
    uint256[1] public Duration = [1 days];
    uint256[1] public Bonus = [0];
    ...
    constructor(address payable _owner, address token1, address token2) {
        ...
        percentDivider = 1000;
        minimumStakeToken = 1e2;
    }

    function stake(uint256 amount, uint256 timeperiod) public {
        require(timeperiod >= 0 && timeperiod <= 1, "Invalid Time Period");
        require(amount >= minimumStakeToken, "stake more than minimum amount");
        uint256 WIENERVAL = getPriceinUSD();
        uint256 rewardtokenPrice = (amount.mul(WIENERVAL)).div(1e18);
    ...
    function getPriceinUSD() public view returns (uint256){
        ...
        address WIENER_WBNB = 0x0Aa8181F76217135794884DFdA87b7fABFB10694;
        
        IERC20 WIENERTOKEN = IERC20(0xAD3df854B453B313F48F3d6bfaFcC6B303eE1aBe);
        IERC20 WBNBTOKEN = IERC20(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);
        
        uint256 WIENERSUPPLYINWIENER_WBNB = WIENERTOKEN.balanceOf(WIENER_WBNB);
        uint256 WBNBSUPPLYINWIENER_WBNB = WBNBTOKEN.balanceOf(WIENER_WBNB);
        
        uint256 BNBPRICE = (WIENERSUPPLYINWIENER_WBNB.mul(1e18)).div(WBNBSUPPLYINWIENER_WBNB);

        // address USX_WBNB = 0x8146A90e3B35CA7E2229A3aA14D8A6E1dd016091;
        // IERC20 USXTOKEN = IERC20(0x170524577ebE8aF70C2Fff4FCE531A2C9FAE014a);

        // uint256 WBNBSUPPLYINUSX_WBNB =(WBNBTOKEN.balanceOf(USX_WBNB));
        // uint256 USXSUPPLYINUSX_WBNB = (USXTOKEN.balanceOf(USX_WBNB));

        uint256 WIENERUSDVAL = (((WBNBSUPPLYINWIENER_WBNB.mul(1e9)).div((WIENERSUPPLYINWIENER_WBNB))).mul(BNBPRICE)).div(1e18);
        return WIENERUSDVAL;
    }
