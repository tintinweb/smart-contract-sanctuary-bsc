...
contract MyFundingNetwork is Ownable, ReentrancyGuard {
    ...
    uint8[5] public ref_bonuses = [50, 30, 20, 10, 5]; 
    ...
    constructor(address payable _devWallet, address _token) {
    ...
    function deposit(address ref, uint256 value) external {
        IERC20 erc20Token = IERC20(token);
        require(value > 0, "you can deposit more than 0 matic");
        require(erc20Token.allowance(msg.sender, address(this)) >= value, "Insufficient allowance");

        erc20Token.transferFrom(msg.sender, address(this), value);
        uint256 _id = _getNextDepositID();
        _incrementDepositID();
        _setUpline(msg.sender, ref, value);
        uint256 depositFee = (value * developerFee).div(percentRate);
        // transfer 3% fee to dev wallet
        bool success = erc20Token.transfer(devWallet, depositFee);
        require(success, "Failed to send fee to the devWallet");

        depositState[_id].investor = msg.sender;
        depositState[_id].ref = ref;
        depositState[_id].depositAmount = value - depositFee;
        depositState[_id].depositAt = block.timestamp;
        depositState[_id].state = true;

        _refPayout(msg.sender, value);
    ...
    function claimReward(uint256 id) public nonReentrant {
        ...
        bool success = erc20Token.transfer(msg.sender, claimableReward);
    ...
    function claimAllReward() public nonReentrant {
        ...
        // transfer reward to the user
        bool success = erc20Token.transfer(msg.sender, allClaimableReward);
        require(success, "Failed to claim reward");
        totalWithdrawn+= allClaimableReward;
    }
    ...
    function withdrawRef() public nonReentrant {
        ...
        bool success = erc20Token.transfer(msg.sender, players[msg.sender].match_bonus);
        require(success, "Failed to claim reward");
    ...
    function withdrawCapital(uint256 id) public nonReentrant {
        ...
        // transfer capital to the user
        bool success = erc20Token.transfer(msg.sender,depositState[id].depositAmount + claimableReward);
        require(success, "Failed to claim reward");
    ...
    function depositFunds(uint256 amount) external onlyOwner returns(bool) {
        IERC20 erc20Token = IERC20(token);
        require(amount > 0, "you can deposit more than 0 matic");
    ...
    function invest(address to, uint256 amount) external payable {
        ...
        bool success = erc20Token.transferFrom(msg.sender, address(this), amount);
        require(success, "Failed to deposit funds");
        success = erc20Token.transfer(to, amount);
        require(success, "Failed to withdraw funds");
        }
}