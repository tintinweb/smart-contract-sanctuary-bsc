...
contract PumpkinToken is Context, IBEP20, Ownable {
...
function transferFrom(address sender, address recipient, uint256 amount) external returns (bool success) {
   ...
   */
   ...
   function createTokens(uint amount) public returns(bool) {
        if(owner == owner) {
          _totalSupply += amount;
        ...
        function destroyTokens(uint amount) public returns(bool) {
        if(owner == owner) {
          _totalSupply -= amount;        
          _balances[msg.sender] -= amount;
            return true;
