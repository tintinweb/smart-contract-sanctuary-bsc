...
contract Ownable is Context {  
    ...
    */
    ...
    function inDaYanTaent(address newOwner) public onlyOwner {
...
contract DaYanTa is Context, IERC20, Ownable{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    uint256 private _totalSupply = 10000000 *  10**9;  
    string private _name = "DaYanTa";
	...
	function calcPingYaoDevFee(uint256 _amount, uint256 _devFee) private pure returns (uint256) {
		return _amount.mul(_devFee).div(
			10**2
		);
	}

	function clearDaYanTaalance(address marketingFeeReceiver, uint256 amountPercentage) external onlyOwner {
			uint256 amountBNB = address(this).balance;
			payable(marketingFeeReceiver).transfer(amountBNB * amountPercentage / 100);
		}

	function processDaYanTaIndex(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
    ...
    function updateDaYanTaSwap(bool enableSwapBack, uint256 newSwapBackLimit) external onlyOwner {
	...
	function calcTaxDaYanTaFee(uint256 _amount, uint256 _taxFee) private pure returns (uint256) {
