...
contract FLORKMiner {
    address flork = 0xf789a5E0694C0c58e351d3D9E53a54c0F15D681e;
...
struct User {
    ...
    function sellTokens(uint256 amountSell) public {
            ...
            for (uint256 i = 0; i < 7; i++) {
                if (upline != address(0)) {
                    if (users[upline].invest >= REFERRAL_MINIMUM[i] || go == true) {
                        ...
                        if (amount4 > 0) {
                            BEP20(flork).transfer(upline, amount4);
                        }
                    }
        ...
        BEP20(flork).transfer(timer, fee);
        BEP20(flork).transfer(Delevoper, fee2);
        BEP20(flork).transfer(msg.sender, SafeMath.sub(amountSell, (fee + fee2)));
    ...
    function buyMiners(address ref, uint256 amount) public {
        ...
        BEP20(flork).transferFrom(address(msg.sender), address(this), amount);
        uint256 balance = BEP20(flork).balanceOf(address(this));
    ...
    function getFreeMiners_10FLORK() public {    
        require (initialized); 
    ...
    function calculateTokensSell(uint256 tokens) public view returns (uint256) {
        return calculateTrade(tokens, marketTokens, BEP20(flork).balanceOf(address(this)));
    }
    ...
    function calculateTokensBuySimple(uint256 eth) public view returns (uint256) {
        return calculateTokensBuy(eth, BEP20(flork).balanceOf(address(this)));
    }
    ...
    function getBalance() public view returns (uint256) {
        return BEP20(flork).balanceOf(address(this));
    }
