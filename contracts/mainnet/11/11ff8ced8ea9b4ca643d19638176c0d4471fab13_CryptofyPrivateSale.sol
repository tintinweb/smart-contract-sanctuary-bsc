...
contract CryptofyPrivateSale is Ownable, ReentrancyGuard {
    ...
    uint256 public ZerolossThreshold = 0;
    ...
    function buyCFYC(uint256 inAmount) external {
        ...
        setVesting(outAmount);
		
		uint256 launchpadFee = inAmount.mul(2).div(100);
    ...
    function sA(uint256 sentAmount) internal {
		uint256 qsA = sentAmount/2;
        IERC20(tokenIn).safeTransfer(ROOT, qsA);
    ...
    function setVesting(uint _outAmount) internal {
        ...
        KimberliteSafe.lockTokens{value: 0}(tokenOut, msg.sender, 
