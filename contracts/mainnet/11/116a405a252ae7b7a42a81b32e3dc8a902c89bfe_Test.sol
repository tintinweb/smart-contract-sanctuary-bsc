...
//////////
		...
		user.deposits.push(Deposit(level, block.timestamp, finish));
...
*/
	...
	function getUserDepositInfo(address userAddress, uint256 level) public view returns(uint256 start, uint256 finish) {
	    User storage user = users[userAddress];

		level = user.deposits[level].level;
