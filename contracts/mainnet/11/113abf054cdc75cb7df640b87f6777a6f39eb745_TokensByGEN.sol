    ...
    if (msg.value == PayNowFEE){
                ...
                if(AFT.balanceOf(Discount_Code) > 0){
                        ...
                        if (AFF > 0){
                            ...
                            (SendSuccess,) = payable(FeeCollectorContract).call{value: msg.value * TBG / 100}("");
                            (SendSuccess,) = payable(Owner_Wallet_Address).call{value: msg.value * DEV / 100}("");
                            (SendSuccess,) = payable(Discount_Code).call{value: msg.value * AFF / 100}("");
                ...
                (SendSuccess,) = payable(FeeCollectorContract).call{value: msg.value}("");
...
contract Contract is Context, IERC20 { 
    ...
    */
    ...
    We do this for any address that represents a liquidity pair, so that the contract knows to apply a fee for transactions to or from these addresses. 
