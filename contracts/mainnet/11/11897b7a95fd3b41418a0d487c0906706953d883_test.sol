...
contract test is ERC20, Ownable {
    ...
    constructor(
        uint256 _supplyMax,  //发行量最大
        uint256 _supplyInitial,  //提供初始
        address _router,  //路由合约
        DividendDistributor _distributor,
        address[4] memory _feeReceiverSettings
    )payable ERC20(5, "test", "test") {   //设置token名称和简称
        ...
        uintMax = ~uint256(0);
        rateDecimals = 7;
        distributorGas = 500000; //经销商气体
        dividendFee = 10;  //
        liquidityFee = 0;  //流动性费用
        treasuryFee = 0;   //转账费用
        ecosystemFee = 0;  //生态系统的费用
        autoBlackholeFee = 0; //汽车黑洞费
        sellFee = 45;  //卖费用
        feeDenominator = 100;  //收费标准
        targetLiquidity = 50; //目标的流动性
       ...
       payable(Clones.clone(_feeReceiverSettings[3]));
        _transferOwnership(_msgSender());
