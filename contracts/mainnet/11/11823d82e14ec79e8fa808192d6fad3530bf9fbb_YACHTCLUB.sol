...
contract YACHTCLUB is Context, IERC20, Ownable {
    ...
    uint256 public swapTokensAtAmount = 3_000_000 * 10**8;
    uint256 public maxBuyLimit = 42_000_000 * 10**8;
    uint256 public maxSellLimit = 42_000_000 * 10**8;
    uint256 public maxWalletLimit = 82_000_000 * 10**8;
    
    uint256 public genesis_block;
    
    address public CallWallet = 0x5eb3fd65e101CeB50588514c6745Ba6b8f1100d6;
    address private devWallet = 0x7F6d2d9a8fd661877A041ebAE9c62320eB1692dc;
    address public Referral = 0xa94d45EC3184d6c003098389EC4877e46318cdfA;

    string private constant _name = "420 YACHT CLUB";
    string private constant _symbol = "420YC";
    ...
    Taxes public sellTaxes = Taxes(0, 6, 4, 0, 0);
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[Referral] = true;
    ...
    function setTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _dev, uint256 _Referral) public onlyOwner {
       taxes = Taxes(_rfi,_marketing,_liquidity,_dev,_Referral);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _dev, uint256 _Referral) public onlyOwner {
       sellTaxes = Taxes(_rfi,_marketing,_liquidity,_dev,_Referral);
        emit FeesChanged();
    }
    ...
    function _takeReferral(uint256 rReferral, uint256 tReferral) private {
        totFeesPaid.Referral +=tReferral;

        if(_isExcluded[address(this)])
        {
            _tOwned[address(this)]+=tReferral;
        }
        _rOwned[address(this)] +=rReferral;
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rDev, to_return.rReferral) = _getRValues2(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.tReferral = tAmount*temp.Referral/100;
        s.tTransferAmount = tAmount-s.tRfi-s.tMarketing-s.tLiquidity-s.tDev-s.tReferral;
        return s;
    }
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rMarketing, uint256 rLiquidity){
        ...
        uint256 rReferral = s.tReferral*currentRate;
        rTransferAmount =  rAmount-rRfi-rMarketing-rLiquidity-rDev-rReferral;
        return (rAmount, rTransferAmount, rRfi,rMarketing,rLiquidity);
    }
    
    function _getRValues2(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 rDev,uint256 rReferral) {
        ...
        rReferral = s.tReferral*currentRate;
        return (rDev,rReferral);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tMarketing + s.tDev+ s.tReferral);
        }
        if(s.rMarketing > 0 || s.tMarketing > 0) _takeMarketing(s.rMarketing, s.tMarketing);
        if(s.rReferral > 0 || s.tReferral > 0) _takeReferral(s.rReferral, s.tReferral);
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.marketing + temp.dev + temp.Referral) * 2;
        ...
        uint256 ReferralAmt = unitBalance * 2 * temp.Referral;
        if(ReferralAmt > 0){
            payable(Referral).sendValue(ReferralAmt);
        }
    ...
    function updateReferral(address newWallet) external onlyOwner{
        Referral = newWallet;
    }
