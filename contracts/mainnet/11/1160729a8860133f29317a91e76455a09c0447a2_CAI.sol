...
contract CAIDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("CAI_Dividend_Tracker", "CAI_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "CAI_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "CAI_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CAI contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "CAI_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "CAI_Dividend_Tracker: Cannot update claimWait to same value");
...
contract CAI is ERC20, Ownable, SafeToken {
    using SafeMath for uint256;
    ...
    CAIDividendTracker public dividendTracker;
    ...
    function setFee(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _buybackFee) public onlyOwner {
        ...
        require(totalFees <= 7, "the fees can't be set more then 7%");
    }

    function setMaxSelltx(uint256 _maxSellTxAmount) public onlyOwner {
        maxSellTransactionAmount = _maxSellTxAmount *10**18;
        require(maxSellTransactionAmount >= 5000 * 10**18, "can't set sell transaction less then 5000 tokens");
    }
    ...
    constructor() ERC20("Caishencoin", "CAI") {
        BNBRewardsFee = 0;
        liquidityFee = 0;
        buybackFee = 0;
        buybackWallet = payable(0x7C1fE77f4CF76A5689Ef71E0C5dc8b32E33e7Ee3);

        totalFees = BNBRewardsFee.add(liquidityFee).add(buybackFee); // total fee transfer and buy

    	dividendTracker = new CAIDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "CAI: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "CAI: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "CAI: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "CAI: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "CAI: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "CAI: Cannot update gasForProcessing to same value");
