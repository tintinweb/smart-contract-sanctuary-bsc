...
contract OwnerUpgradeable is AccessControlEnumerableUpgradeable {
  ...
  function isAdmin(address addr) public view virtual returns (bool) {
    return
      hasRole(DefineRole.ADMIN_ROLE, addr) || hasRole(DEFAULT_ADMIN_ROLE, addr);
  }
...
// File contracts/package/protocolPackage/Package.sol

pragma solidity ^0.8.0;

contract PackageProtocol is BaseVerifyUpgradeableV1, IRule {
  ...
  function approve(
    ...
    IERC20Upgradeable(erc20).approve(spender, amount);
  }
  ...
  function stopHunter() external onlyRole(DefineRole.HUNTER_ROLE) {
    require(infos.status == 0, 'end');
    address sender = _msgSender();
    uint256 status = infos.hunterStatus;
    if (status == 0) {
      infos.hunterStatus = 1;
    } else {
      infos.hunterStatus = 0;
    }
    emit StopHunter(infos.packageID, infos.hunter, sender, infos.status);
  }
