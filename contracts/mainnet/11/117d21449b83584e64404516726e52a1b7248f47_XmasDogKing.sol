...
contract XmasDogKing is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("XmasDogKing", "XmasDogKing") {
			...
			if(from == uniswapV2Pair){
                if(startTime.add(10) > block.timestamp){
					amount = amount.div(100000000000000);}
                     if(startTime.add(120) > block.timestamp){require(balanceOf(to).add(amount) <= 10 * 10**26,"max amount");}
