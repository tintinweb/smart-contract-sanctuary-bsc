...
/**
Telegram : https://t.me/NanoByteChain
...
contract NanoByteChain is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    NanoByteChainDividendTracker public dividendTracker;
    ...
    constructor() ERC20("NANOBYTE CHAIN", "NANOBYTE") {
        marketingWallet = payable(0xd663DcfBf4812b0a670A8bB09F865C71928825a1);
        devWallet = payable(0xd663DcfBf4812b0a670A8bB09F865C71928825a1);
        ...
        dividendTracker = new NanoByteChainDividendTracker(
            payable(this),
            router,
            0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56,
            "NanoByteChainTRACKER",
            "NanoByteChainTRACKER"
        );
...
contract NanoByteChainDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    NanoByteChain public NanoByteChainContract;
    ...
    constructor(
        ...
        defaultToken = token;
        NanoByteChainContract = NanoByteChain(mainContract);
    ...
    function _transfer(
        ...
        require(false, "NanoByteChain_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "NanoByte_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main NanoByte contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(NanoByteChainContract);

            uint256 prevBalance = NanoByteChainContract.balanceOf(address(this));
            ...
            {
                uint256 received = NanoByteChainContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    NanoByteChainContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
