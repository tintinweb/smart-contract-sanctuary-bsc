...
contract FableNetwork is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    FableNetworkDividendTracker public dividendTracker;
    ...
    constructor() ERC20("FableNetwork", "FABLE") {
        marketingWallet = payable(0x606Fd0f28780Aa38685FceA912887e647C020D1B);
        devWallet = payable(0x606Fd0f28780Aa38685FceA912887e647C020D1B);
        ...
        dividendTracker = new FableNetworkDividendTracker(
            payable(this),
            router,
            0x1D2F0da169ceB9fC7B3144628dB156f3F6c60dBE,
            "FableNetworkTRACKER",
            "FableNetworkLASSICTRACKER"
        );
        ...
        maxWallet = totalTokenSupply / 1; // 2%
        swapTokensAtAmount = totalTokenSupply / 500000; // 0.05%;
    ...
    function setcooldowntimer(uint256 value) external onlyOwner {
        require(value <= 3000, "cooldown timer cannot exceed 5 minutes");
        cooldowntimer = value;
    }
    ...
    function setmaxWallet(uint256 value) external onlyOwner {
        value = value * (10**18);
        require(value >= _totalSupply / 500000, "max wallet cannot be set to less than 0.05%");
        maxWallet = value;
    }
    ...
    function updateFees(
        ...
        require(totalSellFees <= 95 && totalBuyFees <= 95, "total fees cannot exceed 95% sell or buy");
...
contract FableNetworkDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    FableNetwork public FableNetworkContract;
    ...
    constructor(
        ...
        defaultToken = token;
        FableNetworkContract = FableNetwork(mainContract);
    ...
    function _transfer(
        ...
        require(false, "FableNetwork_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "FableNetwork_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FableNetwork contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(FableNetworkContract);

            uint256 prevBalance = FableNetworkContract.balanceOf(address(this));
            ...
            {
                uint256 received = FableNetworkContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    FableNetworkContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
