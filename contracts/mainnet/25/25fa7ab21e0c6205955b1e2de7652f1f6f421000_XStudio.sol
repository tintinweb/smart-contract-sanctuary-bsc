...
interface IFeeProcessor {
    ...
    event Processed(address indexed source, address indexed target, uint256 amount, uint256 flags);

    struct Fees {
        uint32 transferFee;
        uint32 buyFee;
        uint32 sellFee;
        uint32 liquidityInjectionRatio;
        uint32 burnRatio;
        uint32 marketingRatio;
        uint64 _unused;
    }

    function process(address source_, address target_, uint256 amount_, uint state_) external returns (uint);
...
contract XStudio is XStudioFlags, UsingBatchTransfer, UsingERC20WithStorage, UsingPermitWithStorage, UsingUUPS {
    ...
    function setup(address routerAddress_, address marketingAddress_, FeesAndAllocations memory fees_) external requiresAdmin {
    ...
    function feeAllocations() public view returns (FeeAllocations memory) {
        return FeeAllocations(_feesAndAllocations.liquidityAllocation, _feesAndAllocations.marketingAllocation);
    }
    ...
    /// @dev this can only be called by the admin
    function setFeeAllocations(uint32 liquidityAllocation_,  uint32 marketingAllocation_) external requiresAdmin {
        FeeAllocations memory oldFeeAllocations = feeAllocations();
        (_feesAndAllocations.liquidityAllocation, _feesAndAllocations.marketingAllocation) = (liquidityAllocation_, marketingAllocation_);
    ...
    function _transfer(address source_, address target_, uint256 amount_) internal override(UsingERC20, UsingBatchTransfer) returns (bool) {
        ...
        if (!_checkFlags(flags, SOURCE_IS_FEE_EXEMPT | TARGET_IS_FEE_EXEMPT, 0)) {
            fee = _process(source_, amount_, flags);
        }
    ...
    function _process(address source_, uint amount_, uint flags_) internal returns (uint) {
        if (_checkFlags(flags_, SOURCE_IS_LIQUIDITY_PAIR, 0)) {
            ...
            if (liquidityBalance > marketingBalance) {
                _executeLiquidityInjection(liquidityBalance);
            } else {
                _executeMarketingSwap(marketingBalance);
            }
            if (amount_ > 0) {
                _depositFees(source_, amount_);
            }
    ...
    function _executeMarketingSwap(uint marketingBalance_) internal  {
        if (marketingBalance_  > 0) {
            super._transfer(_marketingContract, address(this), marketingBalance_);
            try _router.swapExactTokensForETHSupportingFeeOnTransferTokens(marketingBalance_, 0, _path, _marketingWallet, block.timestamp) {
            } catch {}
        }
    }
