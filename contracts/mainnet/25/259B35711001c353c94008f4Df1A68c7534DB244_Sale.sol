...
contract Sale is AccessControl, Pausable, ReentrancyGuard {
    ...
    constructor() {
        bscsDevAddress = payable(_msgSender());
    }

    modifier onlyBscsDev() {
        require(bscsDevAddress == _msgSender(), "Only Sale Owner");
        _;
    }
    ...
    modifier investorOnly() {
        require(investments[_msgSender()] > 0, "Not an investor");
        _;
    }
    ...
    modifier notYetClaimedOrRefunded() {
        require(claimed[_msgSender()] < getTokenAmount(investments[_msgSender()]));
        _;
    }
    ...
    function allowClaim() external onlyBscsDev {
        require(closeTime > 0);
        claimAllowed = true;
    }
        ...
        presaleIsNotCancelled
    {
        ...
        uint256 totalInvestmentInWei = investments[_msgSender()].add(msg.value);
        require(
    ...
    function transferUnsoldTokens() external nonReentrant onlyBscsDev presaleIsNotCancelled {
    ...
    {
        uint256 tokenAmount = getTokenAmount(investments[_msgSender()]);

        if (claimed[_msgSender()].add(tokenAmount) > tokenAmount) {
            tokenAmount = tokenAmount.sub(claimed[_msgSender()]);
        }
        claimed[_msgSender()] = claimed[_msgSender()].add(tokenAmount); // make sure this goes first before transfer to prevent reentrancy
        ...
        notYetClaimedOrRefunded
    {
        if (!presaleCancelled) {
            ...
            require(claimed[_msgSender()] == 0, "Already claimed");
        }

        claimed[_msgSender()] = getTokenAmount(investments[_msgSender()]); // make sure this goes first before transfer to prevent reentrancy
        uint256 investment = investments[_msgSender()];
        ...
        if (investment > 0) {
            payable(_msgSender()).transfer(investment);
        }
    }

    function cancelAndTransferTokensToDev() external nonReentrant onlyBscsDev {
        if (bscsDevAddress != _msgSender()) {
            revert();
        }
    ...
    function collectFundsRaised() external nonReentrant onlyBscsDev {
        require(!presaleCancelled);
