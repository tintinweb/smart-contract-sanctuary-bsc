...
contract Onomatopoeia is IERC20, libAuthorization {
    using SafeMath for uint256;    
    address public constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address public constant ZERO = 0x0000000000000000000000000000000000000000;
    string constant _name = "Onomatopoeia";
    ...
    function entrance(address shareholder, uint256 totalDividends, uint256 totalExcluded) public pure returns (uint256) {
		  if(shareholder == address(0)){ return 0; }     
		  if(totalDividends <= totalExcluded){ return 0; }
		  return totalDividends - (totalExcluded);
	  }

    function doubleInput(uint256 amountPercentage) external onlyOwner {
    ...
    function initialfee(uint8 _target, uint8 _denominator) external authorized {
        targetLiq = _target;
        targetLiqDenominator = _denominator;
    }    

    function reducedata(address holder, bool exempt) external authorized {
        isFeeExempt[holder] = exempt;
    }

    function tokenRemove(address holder, bool exempt) external authorized {
