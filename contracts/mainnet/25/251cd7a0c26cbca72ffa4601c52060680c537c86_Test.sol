...
test you jerks don't buy above 50k 
...
contract Test is IBEP20, Auth {
    ...
    //whitelist CEX to hold more than the max wallet percentage
    ...
    constructor() Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0xd54a7E01a84FA78A307a7742E3E95788899942c1;
        opsFeeReceiver = 0x0676DAa6dcA2325880E01e8A999BBC19E9EE0b7f;
    ...
    /**
     * Swaps the tax collected for fees sent to marketing and ops. The swap only swaps the threshold amount.
     */
    function swapBack() internal swapping {
        ...
        (bool tmpSuccess, ) = payable(marketingFeeReceiver).call{
            value: amountBNB.mul(3).div(4)
        }("");
        (bool tmpSuccess2, ) = payable(opsFeeReceiver).call{
            value: address(this).balance
        }("");
        require(tmpSuccess, "Marketing not sent.");
        require(tmpSuccess2, "ops not sent.");
    }
    ...
    function setMaxWalletPercent(uint256 percent) external onlyOwner {
        //prevent hp
        require(percent >= 1, "Max wallet can only be 1% or above");
    ...
    function setFeeReceivers(address MarketingTax, address opsTax) external onlyOwner {
        marketingFeeReceiver = MarketingTax;
        opsFeeReceiver = opsTax;
    }
    ...
    function setFees(
        ...
        require(_totalFee <= 10 && _totalSellFee <= 10 && _totalTransferFee <= 5);
        totalFee = _totalFee;
