...
interface AutoPool
{
    function OnBuy(address buyer,uint256 amount) external;
    function OnSell(address seller,uint256 amount,uint256 cutcount) external;
    function OnTransfer(address sender,address receipt,uint256 amount,uint256 cutcount) external;
}
...
contract FCTTOKEN is  Ownable
{
    ...
    string constant  _name = 'FCT';
    string constant _symbol = 'FCT';
    ...
    function setAutoPool(address pool) public
    {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(!_exclude[sender] && !_exclude[recipient])
        {
            ...
            else if(recipient == _ammPool)
            {
                ...
                AutoPool(_autoPool).OnSell(sender, amount,cutcount);
            ...
            else
            {
                ...
                toamount= amount.sub(cutcount);
                AutoPool(_autoPool).OnTransfer(sender,recipient, amount,cutcount); 
            }
