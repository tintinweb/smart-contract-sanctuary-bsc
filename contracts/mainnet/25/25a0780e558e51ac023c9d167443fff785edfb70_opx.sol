...
contract opx is Ownable {
    ...
    mapping(address => uint) public opxNFT;
    mapping(uint => bool) public opxNftIds;
    mapping(address => uint) public opxNode;
    mapping(uint => bool) public opxNodeIds;
    ...
    //config
    uint public payId = 0;
    uint public opxNftPrice=200;
    uint public opxNftNum=1;
    uint public opxNodePrice=2000;
    uint public opxNodeNum=1;
    uint public DECIMALS = 10 ** 18;

    event WithdrawReward(address indexed user, uint amount, uint withdrawRewardId);

    constructor (address _nodeAddress,address _nftAddress,address _secretSigner) {
        NODE_ADDRESS = _nodeAddress;
        NFT_ADDRESS = _nftAddress;
        secretSigner = _secretSigner;
    }

    function setOpxNftPrice(uint _price) external onlyOwner {
        opxNftPrice = _price;
    }

    function setOpxNftNum(uint _price) external onlyOwner {
        opxNftNum = _price;
    }

    function setOpxNodePrice(uint _price) external onlyOwner {
        opxNodePrice = _price;
    }

    function setOpxNodeNum(uint _price) external onlyOwner {
        opxNodeNum = _price;
    }

    function setNodeAddress(address _account) external onlyOwner {
        NODE_ADDRESS = _account;
    }

    function setNftAddress(address _account) external onlyOwner {
        NFT_ADDRESS = _account;
    }
    ...
    function payToken(uint _amount,uint orderId,address payAddress) external {
        require(!payOrderIdOf[orderId], "marketMaker: orderId expired!");
        require(payAddress==msg.sender, "marketMaker: payAddress unequal to msg.sender!");
    ...
    function getOpxNFT(uint opxNftId) external {
        require(!opxNftIds[opxNftId], "marketMaker: orderId expired!");
        require(opxNFT[msg.sender]<opxNftNum, "marketMaker: orderId expired!");
        uint _amount =  opxNftPrice * DECIMALS;
        IERC20(USDT_ADDRESS).transferFrom(msg.sender, NFT_ADDRESS,_amount);
        opxNFT[msg.sender]=opxNFT[msg.sender]+1;
        opxNftIds[opxNftId] = true;

    }

    function getOpxNODE(uint opxNodeId) external {
        require(!opxNodeIds[opxNodeId], "marketMaker: orderId expired!");
        require(opxNode[msg.sender]<opxNodeNum, "marketMaker: orderId expired!");
        uint _amount =  opxNodePrice * DECIMALS;
        IERC20(USDT_ADDRESS).transferFrom(msg.sender, NODE_ADDRESS,_amount);
        opxNode[msg.sender]=opxNode[msg.sender]+1;
        opxNodeIds[opxNodeId] = true;
    ...
    function withdrawReward(uint _withdrawRewardId, address _to, uint _amount,uint withdrawTimestamp, uint8 _v, bytes32 _r, bytes32 _s) external {
        ...
        IERC20(USDT_ADDRESS).safeTransfer(_to, _amount);
