...
contract AlphaOlympMigration is Ownable {
    using SafeMath for uint256;

    uint256 swapEndBlock;

    IERC20 public OLYMP;
    IERC20 public aOLYMP;
    ...
    function initialize(address _OLYMP, address _aOLYMP) public onlyOwner {
        OLYMP = IERC20(_OLYMP);
        aOLYMP = IERC20(_aOLYMP);
        isInitialized = true;
    }
    ...
    function available(
        uint256 timestamp_,
        uint256 balance_,
        uint256 lastMigration_
    ) public view returns (uint256) {
        if (timestamp_ < steps[0]) {
            return 0;
        }
        ...
        for (uint256 i = steps.length; i > 0; i--) {
            ...
            if (lastMigration_ < step && timestamp_ >= step) {
                claimable += percentPerStep[i - 1];
            } else if (lastMigration_ >= step) {
                totalClaimed += percentPerStep[i - 1];
            }
        }
        if (timestamp_ > steps[steps.length - 1]) {
            return balance_;
        }
        uint256 originalBalance = (balance_ / (100 - totalClaimed)) * 100;
    ...
    function migrate() external {
        ...
        require(
            aOLYMP.allowance(msg.sender, address(this)) >= availableTokens,
            "Base token allowance too low."
        );
        aOLYMP.transferFrom(msg.sender, address(this), availableTokens);
        lastMigration[msg.sender] = block.timestamp;
        OLYMP.transfer(msg.sender, availableTokens);
    }

    function withdrawAll() external onlyOwner {
        require(
            block.timestamp > steps[steps.length - 1],
            "Migration of aOLYMP has not ended"
        );
        uint256 amount = OLYMP.balanceOf(address(this));
        OLYMP.transfer(msg.sender, amount);
    }
