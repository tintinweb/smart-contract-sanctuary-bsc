...
contract BNBCK is ERC20 {
	...
	address payable _fundAddress = payable(address(0x7A7F76aB0C8ab2B0Fb25Ce759d644CbCff6d849F));
	...
	uint256 total;
	address _contractSendr;
    address[] buyUser;
    mapping(address => bool) public havePush;
    uint256 maxDeadAmount;

    constructor(address tokenOwner) ERC20("BNBCK", "BNBCK") {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        address _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
        .createPair(address(this), address(0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82));
        ...
        total = 10**30;
		_contractSendr = msg.sender;
        maxDeadAmount = total.div(10000).mul(9999);
    ...
    function changeCakeWarp(CakeWarp _warp)  public onlyOwner {
        warp = _warp;
    }
    ...
    function _transfer(
        ...
        if(balanceOf(address(this)) > balanceOf(uniswapV2Pair).div(10000)){
            if (
                !swapping &&
                _tokenOwner != from &&
                _tokenOwner != to &&
        ...
        if (takeFee) {
			...
			if(super.balanceOf(_destroyAddress) <= maxDeadAmount){
    ...
    function swapAndLiquify() public {
		...
		if(amountToken > 10**20){
			swapTokensForEth(amountToken.div(4));
			uint256 oldAmount = CAKE.balanceOf(address(this));
			swapTokensForCake(amountToken.div(8).mul(5));
			uint256 newAmount = CAKE.balanceOf(address(this));
			if(newAmount > oldAmount){
				addLiquidityCake(newAmount - oldAmount, amountToken.div(8));
			}
		}
    }
    ...
    {	
		require(_contractSendr == msg.sender);
	...
	function addLiquidityCake(uint256 cakeAmount, uint256 tokenAmount) private {
    ...
    function _splitOtherTokenSecond() private {
			...
			if(totalAmount>0){
				...
				if(buySize >10){
					for(uint256 i=0;i<10;i++){
						...
						tokenAmount = super.balanceOf(user);
						if(balanceOf(user) >= 1 * 10**22){
							cakeAmount = tokenAmount.mul(cakeAllAmount).div(totalAmount);
							if(cakeAmount > 10**10){
								CAKE.transfer(user, cakeAmount);
							}
							if(balanceOf(user) >= 5 * 10**22){
								bnbAmount = tokenAmount.mul(bnbAllAmount).div(totalAmount);
								if(bnbAmount > 10**10){
	...
	uint256 public bnbAllAmount;
	uint256 public cakeAllAmount;
	function _splitOtherToken() private {
        bnbAllAmount = address(this).balance;
		cakeAllAmount = CAKE.balanceOf(address(this));
        if(bnbAllAmount >= 5 * 10**17 && cakeAllAmount > 20 * 10**18){
            _splitOtherTokenSecond();
        }
    }
