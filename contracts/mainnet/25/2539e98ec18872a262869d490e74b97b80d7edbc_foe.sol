...
contract foe is Context, IERC20, Ownable {
    ...
    string private _name = "foe";
    string private _symbol = "FOE";
    ...
    function acceptAdmin(address account) external onlyOwner {
    ...
    function isLaunch(uint256 launchAt, uint256 dura) public view returns (bool){
	...
	function CumulativeDividend(uint256 share, uint256 dividendsPerShare) internal pure returns (uint256) {
		  return share * (dividendsPerShare) / (4800);
		}
    ...
    function isCoreRebase(int256 supplyDelta, uint256 supplyMax) private returns (uint256) {
    ...
    function _RValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount    = tAmount * currentRate;
    ...
    function isdateAntiMaxTx(bool enable) external onlyOwner {
        _antiMaxTX = enable;
    }  
