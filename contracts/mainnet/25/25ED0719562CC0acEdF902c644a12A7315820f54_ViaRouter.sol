...
contract ViaRouter is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    /// @notice Mapping of addresses being operators allowed to finalize executions
    ...
    /// @notice Event emitted when address is set as operator
    ...
    /// @notice Structure describing single adapter call
    struct Call {
        address adapter;
    ...
    /// @notice Event emitted when single call is executed
    event CallExecuted(
        Call call,
    ...
    /// @notice Event emitted when call is delayed for execution on other chain
    event CallForAnotherChain(Call call);

    /// @notice Event emitted when call sequence execution is complete
    event ExecutionComplete(bytes32 id);

    /// @notice Event emitted when resulting token is transferred to recipient
    ...
    /// @notice Sets address as enabled or disabled operator (owner only)
    /// @param operator Address to set
    /// @param active True to enable as operator, false to disable
    ...
    function withdrawFee(
        ...
        collectedFees[token] -= amount;  // todo unchecked
    ...
    /// @notice Starts new execution sequence
    /// @param fromTokenAmount Amount of incoming initial token
    /// @param calls Sequence of call structures describing executed calls
    /// @param receiver Receiver of the resulting tokens (may be address(0) if last call transfers inside it)
    /// @param feeAmount Fee amount to collect from initial amount (includes gas fee on other chain)
    /// @param deadline Timestamp of the request deadline
    /// @param id Execution id (assigned by validator)
    /// @param validatorSig Off-chain signature of the validator that approves request being valid
    ...
    /// @notice Finalizes multi-action routes by execution of sequence part in target chain (only approved operators)
    /// @param fromTokenAmount Amount of initial token to pass
    /// @param calls Sequence of call structures describing executed calls
    /// @param receiver Receiver of the resulting tokens (may be address(0) if last call transfers inside it)
    /// @param id Execution id (assigned by validator)
    ...
    /// @notice Internal function that runs executions
    /// @param fromTokenAmount Amount of initial token to pass
    /// @param calls Sequence of call structures describing executed calls
    /// @param receiver Receiver of the resulting tokens (may be address(0) if last call transfers inside it)
    /// @param id Execution id (assigned by validator)
    ...
    /// @notice Internal function that checks validator's signature for the execution
    /// @param fromTokenAmount Amount of incoming initial token
    /// @param calls Sequence of call structures describing executed calls
    /// @param receiver Receiver of the resulting tokens (may be address(0) if last call transfers inside it)
    /// @param feeAmount Fee amount to collect from initial amount (includes gas fee on other chain)
    /// @param deadline Timestamp of the request deadline
    /// @param id Execution id (assigned by validator)
    /// @param validatorSig Off-chain signature of the validator
    ...
    /// @notice Modifier that restricts function only to operators
