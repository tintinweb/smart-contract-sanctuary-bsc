...
contract BIGNFTPresale is ReentrancyGuard, Ownable, Pausable {
    ...
    constructor(uint _startTime, uint _endTime, address _oracle, address _routerAddress,address _wbnb, address _usdt, address _usdc, address _busd, address _dai, address _ibat) {
    ...
    function getIBATLatestPrice() public view returns (uint) {
        uint256 _amountIN = 1*(10**9);
        address[] memory path = new address[](3);
        path[0] = IBATtoken;
        path[1] = uniswapV2Router.WETH();
        path[2] = USDTtoken;
        uint[] memory slotprice = uniswapV2Router.getAmountsOut(_amountIN,path);
        uint256 price = slotprice[2];
        return uint(price);
        }
    ...
    function buyWithIBAT(uint amount) external checkSaleState(amount) whenNotPaused {
        uint usdPrice = calculatePrice(amount);
        uint IBATAmount = (usdPrice * baseDecimals) / getIBATLatestPrice();
        inSale -= amount;
        userDeposits[_msgSender()] += (amount * baseDecimals);
    ...
    function getIBATAmount(uint amount) external view returns (uint IBATAmount) {
        uint usdPrice = calculatePrice(amount);
        IBATAmount = (usdPrice * baseDecimals) / getIBATLatestPrice();
    }
