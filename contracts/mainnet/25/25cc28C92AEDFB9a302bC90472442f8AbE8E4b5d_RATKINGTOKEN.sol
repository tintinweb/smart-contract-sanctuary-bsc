...
contract RATKINGTOKEN is IERC20, Context, Ownable {
    ...
    string private constant _symbol = "RATKING";
    string private constant _name = "RATKING TOKEN";

    uint256 private Rat_max_BuyLimit = 20000000 * 10**9;
    uint256 private Rat_max_SellLimit = 20000000 * 10**9;

    address private Rat_King_Wallet = 0x29791A1be4EE691a555705f13F031d98A2A1a6Dd;
    address private Rat_Team_Wallet = 0x29791A1be4EE691a555705f13F031d98A2A1a6Dd;

    uint256 public Rat_max_WalletLimit = 40000000 * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[Rat_King_Wallet] = true;
        _isExcludedFromFee[Rat_Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[Rat_King_Wallet] = true;
        allowedTransfer[Rat_Team_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= Rat_max_BuyLimit, "You are exceeding Rat_max_BuyLimit");
            require(balanceOf(to) + amount <= Rat_max_WalletLimit, "You are exceeding Rat_max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= Rat_max_SellLimit, "You are exceeding Rat_max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= Rat_max_WalletLimit, "You are exceeding Rat_max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(Rat_King_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(Rat_Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function Rat_Update_MarketingWallet(address newWallet) external onlyOwner{
        Rat_King_Wallet = newWallet;
    }
    
    function Rat_Update_TeamWallet(address newWallet) external onlyOwner{
        Rat_Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        Rat_max_BuyLimit = maxBuy * 10**decimals();
        Rat_max_SellLimit = maxSell * 10**decimals();
    }
    
    function Rat_Max_Walletlimit(uint256 amount) external onlyOwner{
        Rat_max_WalletLimit = amount * 10**decimals();
    }
