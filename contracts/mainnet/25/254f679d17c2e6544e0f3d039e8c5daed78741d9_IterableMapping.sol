...
//
// Welcome to SimbaX Token
// 
// The highest paying rewards and reflection token.
// Automatically recieve BNB and SIMBAX
// with the ability to auto reinvest for compounded interest.
// 8% Rewards and Reflection, 2% Added to marketing & 2% Added to liquidity pool
//
// 📱 Telegram: https://t.me/simbaxtoken
// 🌎 Website: https://simbaxtoken.finance/
// 🌐 Twitter: https://twitter.com/simbaxtoken
//
//
...
contract SimbaXDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("SimbaX_Dividend_Tracker", "SimbaX_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "SimbaX_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "SimbaX_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SimbaX contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SimbaX_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "SimbaX_Dividend_Tracker: Cannot update claimWait to same value");
...
contract SimbaX is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    SimbaXDividendTracker public dividendTracker;
    ...
    constructor() ERC20("SimbaX", "SIMBAX") {
        ...
        marketingFee = 2;
        marketingWallet = payable(0xE7E29fAf96A0D784C321015cAA7D9BA761ae5e0d);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new SimbaXDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SimbaX: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SimbaX: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SimbaX: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SimbaX: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SimbaX: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SimbaX: Cannot update gasForProcessing to same value");
