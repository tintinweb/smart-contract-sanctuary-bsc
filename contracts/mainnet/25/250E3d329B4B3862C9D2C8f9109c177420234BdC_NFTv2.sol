    ...
    IERC721EnumerableUpgradeable
{
    ...
    function initialize(
        ...
        baseExtension = ".json";
        minted = IERC721EnumerableUpgradeable(nftV1).totalSupply();
        maxSupply = 3000;
    ...
    // mapping(address => uint256) private _balances; DONE
    // mapping(address => mapping(uint256 => uint256)) private _ownedTokens; DONE
    // mapping(uint256 => address) private _owners; DONE
    // mapping(uint256 => uint256) private _ownedTokensIndex; DONE
    // mapping(address => Player) public players; DONE
    // mapping(uint256 => Node) public _nodes; DONE
    // mapping(address => bool) public isBlacklisted; DONE

    // uint256[] private _allTokens; DONE
    // mapping(uint256 => uint256) private _allTokensIndex; DONE
    ...
    function _migrate(address user) internal {
        uint256 balance = IERC721Upgradeable(NFT_V1).balanceOf(user);
        require(
            balance > 0 && _balances[user] == 0,
            "MIGRATE: Already migrated"
        );
        _balances[user] = balance;

        isBlacklisted[user] = INFTv1(NFT_V1).isBlacklisted(user);
        ...
        {
            for (uint8 i = 0; i < balance; ) {
                uint256 id = INFTv1(NFT_V1).tokenOfOwnerByIndex(user, i);
                _ownedTokens[user][i] = id;
                _ownedTokensIndex[id] = i;
                _owners[id] = user;
                (
                    ,
        ...
        MANAGER.migrationCompound(user);
    }
    ...
    function migrateAllTokensArray() public onlyOwner {
        uint256 minted_ = minted;
        for (uint16 i = 1; i < minted_; ) {
            _allTokensIndex[i] = _allTokens.length;
            _allTokens.push(i);
            unchecked {
                i++;
            }
    ...
    function updateTotalClaimed(uint256 id, uint256 rewards)
        public
        onlyManager
    {
