...
contract DividendPayingToken is ERC20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
  ...
  address public immutable SOLANA = address(0x570A5D26f7765Ecb712C0924E4De545B89fD43dF);
...
pragma solidity ^0.6.12;
contract SolanaDaoDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("SolanaDao_Dividen_Tracker", "SolanaDao_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 500000 * (10**18); //must hold 500000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "SolanaDao_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "SolanaDao_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SolanaDao contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SolanaDao_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SolanaDao_Dividend_Tracker: Cannot update claimWait to same value");
...
pragma solidity ^0.6.12;
contract SolanaDao is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    SolanaDaoDividendTracker public dividendTracker;

    address public deadWallet = 0x000000000000000000000000000000000000dEaD;
    address public _DevWalletAddress = 0x82C934800bdB0a1C3d033085c9b84c7b16e7138C;
    address public _FoudWalletAddress = 0xd6D0D90aD3d8e44Ed77A1F55E2c0FA43E0baa73D;
    ...
    uint256 public SOLANARewardsFee = 10;
    uint256 public SOLANARewardsShare = 5;
    ...
    // lockWallet
    address public _AppWalletAddress = 0xc8674C2D531f2e0f78C8569CF0d14F2a22a1aE18;
    address public _CrossWalletAddress = 0xA6872623A59913dCbE31313E1319211e3cd7F3d9;
    ...
    constructor() public ERC20("SolanaDao", "SOLD") {

    	dividendTracker = new SolanaDaoDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "SolanaDao: The dividend tracker already has that address");

        SolanaDaoDividendTracker newDividendTracker = SolanaDaoDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "SolanaDao: The new dividend tracker must be owned by the SolanaDao token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SolanaDao: The router already has that address");
    ...
    function setFee(
        uint256 _SOLANARewardsFee,
        uint256 _SOLANARewardsShare
    ) public onlyOwner {
        SOLANARewardsFee = _SOLANARewardsFee;
        SOLANARewardsShare = _SOLANARewardsShare;
    }

    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SolanaDao: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SolanaDao: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SolanaDao: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SolanaDao: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(liquidityLaunched && block.number <= lastSnipeTaxBlock && !isFirstLaunch){
            uint256 fees = amount.mul(SOLANARewardsFee).div(100);
        ...
        if( canSwap &&
            ...
            uint256 BNBAmountForReward = BNBAmount.mul(SOLANARewardsShare).div(SOLANARewardsFee);
    ...
    function swapBNBForSOLANA(uint256 tokenAmount) private {
    ...
    function swapAndSendDividends(uint256 tokens) private{
        swapBNBForSOLANA(tokens);
        uint256 dividends = IERC20(SOLANA).balanceOf(address(this));
        bool success = IERC20(SOLANA).transfer(address(dividendTracker), dividends);
