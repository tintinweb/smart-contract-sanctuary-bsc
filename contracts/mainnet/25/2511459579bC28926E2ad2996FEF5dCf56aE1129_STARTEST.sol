...
contract STARTEST is Ownable, ReentrancyGuard, ERC20 {
    ...
    uint256 public starCutoff1;
    uint256 public starCutoff2;
    uint256 public starCutoff3;
    uint256 public starMultiplier1;
    uint256 public starMultiplier2;
    uint256 public starMultiplier3;
    uint256 public totalAdjustedStaked;
    uint256 public totalStakedTokens;

    uint8 starTier0 = 0; // apple balance of 0 to < 150k
    uint8 starTier1 = 1; // apple balance of 150k < 300k
    uint8 starTier2 = 2; // apple balance of 300k < 750k
    uint8 starTier3 = 3; // apple balance of 750k+
    ...
    // apple interface
    address public AppleToken;
    IApple AppleTokenInterface;
    ...
    // The reward token
    Apple public rewardToken;

    // The wbnb token and BUSD token
    ...
    struct UserInfo {
        ...
        uint256 userAdjustedStaked; // a "fake" helper balance used to assist in tier reward calculations
        uint256 starTier; // anywhere from 0 to 3 inclusive, depending on user's APPLE balance
    }

    struct Share {
        uint256 amount;
        uint256 totalExcludedWBNB;
        uint256 totalRealisedWBNB;
        uint256 totalExcludedBUSD;
        uint256 totalRealisedBUSD; 
    }
...
// ----------------- BEGIN WBNB Variables -----------

    uint256 public unallocatedWBNBRewards;
    uint256 public totalShares;
    uint256 public totalRewardsWBNB;
    uint256 public totalDistributedWBNB;
    uint256 public rewardsPerShareWBNB;
    uint256 public rewardsPerShareAccuracyFactor = 10 ** 36; // Keeping same accuracy factor in the BUSD Token Variables

// ----------------- BEGIN BUSD Token Variables -----------

    uint256 public unallocatedBUSDRewards;
    uint256 public totalRewardsBUSD;
    uint256 public totalDistributedBUSD;
    uint256 public rewardsPerShareBUSD;

// ----------------- END BUSD Token Variables -----------    
    ...
    constructor(IBEP20 _stakedToken, Apple _rewardToken, uint256 _rewardPerBlock, uint256 _startBlock, uint256 _lockTime, address _pyeSlice) ERC20("","") {

        // Make this contract initialized
        isInitialized = true;

        starCutoff1 = 1 * 10**18;
        starCutoff2 = 10 * 10**18;
        starCutoff3 = 20 * 10**18;
        starMultiplier1 = 125;
        starMultiplier2 = 150;
        starMultiplier3 = 200;
        AppleToken = 0xF65Ae63D580EDe49589992b6E772b48E61EaDed2;
        AppleTokenInterface = IApple(AppleToken);
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user.userAdjustedStaked.mul(accTokenPerShare).div(PRECISION_FACTOR).sub(user.rewardDebt);
            if (pending > 0) {
        ...
        if (_amount > 0) {
            ...
            uint256 currentPYESliceBalance = IBEP20(pyeSlice).balanceOf(msg.sender);
            ...
            distributeRewardWBNB(msg.sender);
            distributeRewardBUSD(msg.sender);
        }
        uint256 stakersAppleBal = getAppleBalance(msg.sender);

        if (stakersAppleBal > starCutoff3) {
            totalAdjustedStaked -= userInfo[msg.sender].userAdjustedStaked; // wipe their adjusted Staked from the total adjusted staked
            userInfo[msg.sender].userAdjustedStaked = ((userInfo[msg.sender].amount)*starMultiplier3)/100;
            userInfo[msg.sender].starTier = 3;
            totalAdjustedStaked += userInfo[msg.sender].userAdjustedStaked; // re add to the total Staked with the updated amount
        } else if (stakersAppleBal >= starCutoff2 && stakersAppleBal < starCutoff3) {
            totalAdjustedStaked -= userInfo[msg.sender].userAdjustedStaked; 
            userInfo[msg.sender].userAdjustedStaked = ((userInfo[msg.sender].amount)*starMultiplier2)/100;
            userInfo[msg.sender].starTier = 2;
            totalAdjustedStaked += userInfo[msg.sender].userAdjustedStaked;
        } else if (stakersAppleBal >= starCutoff1 && stakersAppleBal < starCutoff2) {
            totalAdjustedStaked -= userInfo[msg.sender].userAdjustedStaked; 
            userInfo[msg.sender].userAdjustedStaked = ((userInfo[msg.sender].amount)*starMultiplier1)/100;
            userInfo[msg.sender].starTier = 1;
            totalAdjustedStaked += userInfo[msg.sender].userAdjustedStaked;
        } else if (stakersAppleBal < starCutoff1) {
            totalAdjustedStaked -= userInfo[msg.sender].userAdjustedStaked; 
            userInfo[msg.sender].userAdjustedStaked = userInfo[msg.sender].amount;
            userInfo[msg.sender].starTier = 0;
            totalAdjustedStaked += userInfo[msg.sender].amount;
        }
        user.rewardDebt = user.userAdjustedStaked.mul(accTokenPerShare).div(PRECISION_FACTOR);
    ...
    function emergencyWithdraw() external nonReentrant {
        ...
        user.amount = 0;
        user.rewardDebt = 0;
        userInfo[msg.sender].userAdjustedStaked = 0;
        userInfo[msg.sender].starTier = 0;
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && totalStakedTokens != 0) {
            ...
            accTokenPerShare.add(appleReward.mul(PRECISION_FACTOR).div(totalAdjustedStaked));
            return user.userAdjustedStaked.mul(adjustedTokenPerShare).div(PRECISION_FACTOR).sub(user.rewardDebt);
        } else {
            return user.userAdjustedStaked.mul(accTokenPerShare).div(PRECISION_FACTOR).sub(user.rewardDebt);
        }
    }
    ...
    function _updatePool() internal {
        ...
        accTokenPerShare = accTokenPerShare.add(appleReward.mul(PRECISION_FACTOR).div(totalAdjustedStaked));
        lastRewardBlock = block.number;
    }
    ...
    function setShare(address staker, uint256 amount) internal {
        if(shares[staker].amount > 0){
            distributeRewardWBNB(staker);
            distributeRewardBUSD(staker);
        }
        ...
        shares[staker].totalExcludedWBNB = getCumulativeRewardsWBNB(shares[staker].amount);
        shares[staker].totalExcludedBUSD = getCumulativeRewardsBUSD(shares[staker].amount);
    }
    
    // WBNB STUFF

    function distributeRewardWBNB(address staker) internal {
        if(shares[staker].amount == 0){ return; }

        uint256 amount = getUnpaidEarningsWBNB(staker);
        if(amount > 0){
            totalDistributedWBNB = totalDistributedWBNB.add(amount);
            IERC20(WBNB).transfer(staker, amount);
            stakerClaims[staker] = block.timestamp;
            shares[staker].totalRealisedWBNB = shares[staker].totalRealisedWBNB.add(amount);
            shares[staker].totalExcludedWBNB = getCumulativeRewardsWBNB(shares[staker].amount);
        }
    }

    function claimWBNB() external {
        distributeRewardWBNB(msg.sender);
    }

    function getUnpaidEarningsWBNB(address staker) public view returns (uint256) {
        if(shares[staker].amount == 0){ return 0; }

        uint256 stakerTotalRewardsWBNB = getCumulativeRewardsWBNB(shares[staker].amount);
        uint256 stakerTotalExcludedWBNB = shares[staker].totalExcludedWBNB;

        if(stakerTotalRewardsWBNB <= stakerTotalExcludedWBNB){ return 0; }

        return stakerTotalRewardsWBNB.sub(stakerTotalExcludedWBNB);
    }

    function getCumulativeRewardsWBNB(uint256 share) internal view returns (uint256) {
        return share.mul(rewardsPerShareWBNB).div(rewardsPerShareAccuracyFactor);
    }
     ...
     * @dev Only callable by owner. Needs to be for emergency.
     */
    function emergencyWBNBWithdraw(uint256 _amount) external onlyOwner {
        IERC20(WBNB).transfer(address(msg.sender), _amount);
    }


// ------------------- BEGIN BUSD TOKEN FUNCTIONS ---------------

    function depositBUSDToStakingContract(uint256 _amountBUSD) external onlyToken {
        if (totalShares == 0) {unallocatedBUSDRewards = unallocatedBUSDRewards.add(_amountBUSD); return; } 
        
        if (unallocatedBUSDRewards > 0) {
            uint256 amount = _amountBUSD.add(unallocatedBUSDRewards);
            totalRewardsBUSD = totalRewardsBUSD.add(amount);
            rewardsPerShareBUSD = rewardsPerShareBUSD.add(rewardsPerShareAccuracyFactor.mul(amount).div(totalShares));
            unallocatedBUSDRewards = 0;
        } else {
            totalRewardsBUSD = totalRewardsBUSD.add(_amountBUSD);
            rewardsPerShareBUSD = rewardsPerShareBUSD.add(rewardsPerShareAccuracyFactor.mul(_amountBUSD).div(totalShares));
        }   
    }

    function depositBUSD(uint256 _amount) external onlyOwner {
        uint256 balanceBefore = IERC20(address(BUSD)).balanceOf(address(this));

        IERC20(BUSD).transferFrom(address(msg.sender), address(this), _amount);

        uint256 amount = IERC20(address(BUSD)).balanceOf(address(this)).sub(balanceBefore);
    ...
    function getCumulativeRewardsBUSD(uint256 share) internal view returns (uint256) {
        return share.mul(rewardsPerShareBUSD).div(rewardsPerShareAccuracyFactor);
    }

    function getUnpaidEarningsBUSD(address staker) public view returns (uint256) {
        if(shares[staker].amount == 0){ return 0; }

        uint256 stakerTotalRewardsBUSD = getCumulativeRewardsBUSD(shares[staker].amount);
        uint256 stakerTotalExcludedBUSD = shares[staker].totalExcludedBUSD;

        if(stakerTotalRewardsBUSD <= stakerTotalExcludedBUSD){ return 0; }

        return stakerTotalRewardsBUSD.sub(stakerTotalExcludedBUSD);
    }

    function distributeRewardBUSD(address staker) internal {
        if(shares[staker].amount == 0){ return; }

        uint256 amount = getUnpaidEarningsBUSD(staker);
        if(amount > 0){
            totalDistributedBUSD = totalDistributedBUSD.add(amount);
            IERC20(BUSD).transfer(staker, amount);
            stakerClaims[staker] = block.timestamp;
            shares[staker].totalRealisedBUSD = shares[staker].totalRealisedBUSD.add(amount);
            shares[staker].totalExcludedBUSD = getCumulativeRewardsBUSD(shares[staker].amount);
        }
    }
    ...
    function setStarCutoffs(uint256 _cutoff1, uint256 _cutoff2, uint256 _cutoff3) external onlyOwner {
        starCutoff1 = _cutoff1;
        starCutoff2 = _cutoff2;
        starCutoff3 = _cutoff3;
    }

    function setStarMultipliers(uint256 _mult1, uint256 _mult2, uint256 _mult3) external onlyOwner {
        starMultiplier1 = _mult1;
        starMultiplier2 = _mult2;
        starMultiplier3 = _mult3;
    } 

    function getAppleBalance(address _staker) public view returns (uint256) {
        return AppleTokenInterface.getOwnedBalance(_staker);
    }

    function setAppleAddress(address _apple) external onlyOwner {
        AppleToken = _apple;
    }
    ...
    function addPYESwapDonation(uint256 _pyeSwapDonation) external nonReentrant {
        ...
        IERC20(WBNB).transferFrom(address(msg.sender), address(this), _pyeSwapDonation);
        uint256 amount = IERC20(address(WBNB)).balanceOf(address(this)).sub(balanceBefore);
        totalRewardsWBNB = totalRewardsWBNB.add(amount);
        rewardsPerShareWBNB = rewardsPerShareWBNB.add(rewardsPerShareAccuracyFactor.mul(amount).div(totalShares));
    ...
    function addPYELabDonation(uint256 _pyeLabDonation) external nonReentrant {
        uint256 balanceBefore = IERC20(address(WBNB)).balanceOf(address(this));
        IERC20(WBNB).transferFrom(address(msg.sender), address(this), _pyeLabDonation);
        uint256 amount = IERC20(address(WBNB)).balanceOf(address(this)).sub(balanceBefore);
        totalRewardsWBNB = totalRewardsWBNB.add(amount);
        rewardsPerShareWBNB = rewardsPerShareWBNB.add(rewardsPerShareAccuracyFactor.mul(amount).div(totalShares));
        totalDonations += amount;
    ...
    function addMiniPetsDonation(uint256 _miniPetsDonation) external nonReentrant {
        uint256 balanceBefore = IERC20(address(WBNB)).balanceOf(address(this));
        IERC20(WBNB).transferFrom(address(msg.sender), address(this), _miniPetsDonation);
        uint256 amount = IERC20(address(WBNB)).balanceOf(address(this)).sub(balanceBefore);
        totalRewardsWBNB = totalRewardsWBNB.add(amount);
        rewardsPerShareWBNB = rewardsPerShareWBNB.add(rewardsPerShareAccuracyFactor.mul(amount).div(totalShares));
        totalDonations += amount;
    ...
    function addPYEWalletDonation(uint256 _pyeWalletDonation) external nonReentrant {
        uint256 balanceBefore = IERC20(address(WBNB)).balanceOf(address(this));
        IERC20(WBNB).transferFrom(address(msg.sender), address(this), _pyeWalletDonation);
        uint256 amount = IERC20(address(WBNB)).balanceOf(address(this)).sub(balanceBefore);
        totalRewardsWBNB = totalRewardsWBNB.add(amount);
        rewardsPerShareWBNB = rewardsPerShareWBNB.add(rewardsPerShareAccuracyFactor.mul(amount).div(totalShares));
        totalDonations += amount;
    ...
    function addPYEChartsDonation(uint256 _pyeChartsDonation) external nonReentrant {
        uint256 balanceBefore = IERC20(address(WBNB)).balanceOf(address(this));
        IERC20(WBNB).transferFrom(address(msg.sender), address(this), _pyeChartsDonation);
        uint256 amount = IERC20(address(WBNB)).balanceOf(address(this)).sub(balanceBefore);
        totalRewardsWBNB = totalRewardsWBNB.add(amount);
        rewardsPerShareWBNB = rewardsPerShareWBNB.add(rewardsPerShareAccuracyFactor.mul(amount).div(totalShares));
        totalDonations += amount;
...
// Apple with Governance.
contract Apple is BEP20, AccessControl {
    ...
    constructor(address minter, address burner, address marketing) BEP20('Apple', 'APPLE') {
    ...
    function setMarketingFee(uint256 _percent) public {
        require(hasRole(FEE_SETTER_ROLE, msg.sender), "APPLE: NOT_ALLOWED");
        require(_percent <= 10000, "APPLE: INVALID_MARKETING_FEE");
        marketingFee = _percent;
    }

    function setMarketingAddress(address _marketing) public {
        require(hasRole(FEE_SETTER_ROLE, msg.sender), "APPLE: NOT_ALLOWED");
        require(_marketing != address(0), "APPLE: INVALID_ADDRESS");
        marketingAddress = _marketing;
    }

    function transfer(address recipient, uint256 amount) public override returns (bool) {
        uint256 fee = amount.mul(marketingFee).div(10 ** 4);
    ...
    function transferFrom(
        ...
        if(fee > 0) {
            _transfer(sender, marketingAddress, fee);
        }
        _approve(
            sender,
            _msgSender(),
