/// @title NFT Marketplace V7
/// @notice contracts/NFTMarketplaceV7.sol
...
contract TSV_NFTMarketplaceV7 is
    ...
    /**
     * @notice Event to keep track when the manager added.
     * @param managerAddress: The newly added manager address.
     */
    event ManagerAdded(
     ...
     /**
     * @notice Event to keep track when the manager removed.
     * @param managerAddress: The removed manager address.
     */
     ...
     * @param tos: An array of The next owner address.
     * @param uris: An array of The token URL.
     * @param priceInWeis: An array of The token price in wei.
     * @param noOfMonths: An array of The token holding months.
     */
    function batchIssueNFTToAddressWithoutERC20 (
        address[] calldata tos,
        string[] calldata uris,
        uint256[] calldata priceInWeis,
        uint256[] calldata noOfMonths
    ) external nonReentrant whenNotPaused {
        /// @notice Requirements.
        _requireManager(msg.sender);
        uint256 arrLength = tos.length;
        require(
            (arrLength == uris.length) &&
            (arrLength == priceInWeis.length) &&
            (arrLength == noOfMonths.length),
            "Array length must be same."
        );

        uint256 mInS = _monthInSeconds;

        for(uint256 i=0; i<arrLength; i++){
            require(tos[i] != address(0), "Address should not be zero.");
            require(priceInWeis[i] > 0, "Dollars should not be zero.");
            // require(isNoOfMonthsAvailable[noOfMonths], "No of months not available.");
            /// @notice Getting the tokenId and MarketItemId.
            uint256 tokenId = _tokenIdCounter.current();
            uint256 itemId = _itemIdCounter.current();
            ...
            marketItemMetadata.marketItemId = itemId;
            marketItemMetadata.seller = payable(owner());
            marketItemMetadata.buyer = tos[i];
            marketItemMetadata.price = priceInWeis[i];
            marketItemMetadata.listedAt = block.timestamp;
            marketItemMetadata.lockInMonths = noOfMonths[i];
            marketItemMetadata.unlockAt = block.timestamp + (noOfMonths[i] * mInS);
            marketItemMetadata.isTokenMinted = false;
        }
    }
    ...
    function transferBackToken (
        ...
        /// @dev Changing state for breaking the blockage by resting buyer of the NFT to zero address
        /// i.e. You cannot transfer your NFT till the holding period. Line `1008`
        MarketItemMetadata storage marketItemMetadata = marketItemByTokenId[tokenId];
        marketItemMetadata.buyer = address(0);

        /// Transferring the token back to manager address who calls the function.
        super._transfer(from, msg.sender, tokenId);
    }
