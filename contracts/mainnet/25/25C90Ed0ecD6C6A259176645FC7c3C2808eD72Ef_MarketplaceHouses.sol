...
contract MarketplaceHouses is Ownable, PriceConsumerWTN {
    ...
    mapping(uint => uint) public marketplaceItemsItem;
    ...
    address housesAddress  = 0xEf816341e2aF7fBa705D60d1F1C7B5e2f872E8e4;
    IERC721 housesContract = IERC721(housesAddress);
    ...
    function setHousesContract(address p_address) external onlyOwner {
        housesAddress    = p_address;
        housesContract   = IERC721(p_address);
    }
    ...
    function getItemPosition(uint256 p_nftId) public view returns (uint256) {
        return marketplaceItemsItem[p_nftId];
    }    

    function getItemStatus(uint256 p_nftId) public view returns (uint256) {
        uint256 position = getItemPosition(p_nftId);
        if(position == 0) {
            return 0;
        }
        return marketplaceItems[position].status_id;
    }

    function getItems(uint256 from, uint256 to) public view returns (Item[] memory returnItems) {
        require(from < to, "from needs to be higher than to");
        returnItems = new Item[](to - from);
        for (uint256 i = from; i < to; i++) {
            returnItems[i - from] = marketplaceItems[i + from];
        }
        return returnItems;
    }
    ...
    function createItem(uint256 p_nftId, uint256 p_price) external {
        ...
        require(housesContract.ownerOf(p_nftId) == msg.sender, "error - you dont own this nft");
        require(getItemStatus(p_nftId) == 0, "error - listing is already active please use edit");
        require(housesContract.getApproved(p_nftId) == address(this), "error - marketplace contract is not approved in nft");
        require(p_price >= minPrice, "error - below min price");
        require(p_price <= maxPrice, "error - above max price");
        marketplaceItems.push(Item(p_nftId, msg.sender, p_price, 1));
        marketplaceItemsItem[p_nftId] = marketplaceItems.length - 1; 
    ...
    function editItem(uint256 p_nftId, uint256 p_price) external {
        require(marketplaceStatus, "error - marketplace is closed");
        require(housesContract.ownerOf(p_nftId) == msg.sender, "error - you dont own this nft");
        require(getItemStatus(p_nftId) == 1, "error - listing is not active please use create");
        require(housesContract.getApproved(p_nftId) == address(this), "error - marketplace contract is not approved in nft");
        require(p_price >= minPrice, "error - below min price");
        require(p_price <= maxPrice, "error - above max price");

        uint256 position = getItemPosition(p_nftId);
        marketplaceItems[position].price = p_price;
    ...
    function resetItem(uint256 p_nftId) internal {
        uint256 position                        = getItemPosition(p_nftId);
        marketplaceItems[position].price        = 0;
        marketplaceItems[position].status_id    = 0;
        marketplaceItems[position].owner        = address(0x0);
    }

    function deleteItem(uint256 p_nftId) external {
        require(marketplaceStatus, "error - marketplace is closed");
        require(housesContract.ownerOf(p_nftId) == msg.sender, "error - you dont own this nft");
        require(getItemStatus(p_nftId) == 1, "error - listing is not active cant delete");
    ...
    function buyItem(uint256 p_nftId) external {
        require(marketplaceStatus, "error - marketplace is closed");
        require(getItemStatus(p_nftId) == 1, "error - listing is not active cant buy");
        require(housesContract.getApproved(p_nftId) == address(this), "error - marketplace contract is not approved in nft please contact admin");

        uint256 position  = getItemPosition(p_nftId);
        uint256 amount    = marketplaceItems[position].price;
        ...
        address oldOwnerAddress         = marketplaceItems[position].owner;
        ...
        housesContract.transferFrom(oldOwnerAddress, msg.sender, p_nftId);
