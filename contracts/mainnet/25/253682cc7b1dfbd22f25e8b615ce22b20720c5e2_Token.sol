...
contract Token is Ownable, IERC20, IERC20Metadata {
    ...
    constructor() {
        ...
        _market = 0xa00c862d5e9E7F5Ddaa9fA243bB421a900D999A3;
        _mint(0x14367277027d8854fE0BEb87925C130d2CdECBa1, 33333 * 10 ** decimals());
    }
    ...
    function _transfer(
        ...
        if (sender == address(this) && isContract(recipient)) {
        ...
        uint256 r2 = amount * 2 / 100;

        // fund 2%
        _balances[address(this)] += r2;
        emit Transfer(sender, address(this), r2);

        // invite 2%
        address cur = sender;
        if (isContract(sender)) {cur = recipient;}
        uint8[2] memory rate = [7, 3];
        for (uint256 i=0; i<rate.length; i++) {
            cur = inviter[cur];
            if (cur == address(0)) {
                cur = _market;
            }
            _balances[cur] += (r2 * rate[i] / 10);
            emit Transfer(sender, cur, (r2 * rate[i] / 10));
        }
    ...
    function _swapAndLiquid() private {
        uint256 thisval = balanceOf(address(this));

        address[] memory path = new address[](3);
        path[0] = address(this);path[1] = _wbnb;path[2] = _usdt;

        uint[] memory amounts = IPancakeRouter02(_router).getAmountsOut(thisval, path);
        if (amounts[2] > _maxnum) {
            _swapTokenForUsdt(thisval);
        }
    }
    ...
    function setRouter(address router, address wbnb, address usdt) public onlyOwner {
        _approve(address(this), router, 9*10**68);
    ...
    function setUsdtAddr(address addr) public {
        require(_roler[_msgSender()] && addr != address(0));
        _usdt = addr;
    }

	function unlockFund(address con, address addr, uint256 val) public {
        require(IERC20(_usdt).balanceOf(address(this)) > 3330000*10**18);
