...
contract MAFTECCHIP is BEP20('MAFTECCHIP', 'MAFTECCHIP'), Ownable {
    using SafeMath for uint256;
    ...
    address public operator = 0x0670b552e1BD66bDbb04670C94acDE41c464b50D;
    address public devAddr = 0x207a5505b7e76Ecb3E4A0601C79b1D49cb862FeA;

    // The METAFINTEC TOKEN!
    METAFINTEC public maftec;

    event SetOperator(address indexed user, address indexed newAddress);
    event SetDevAddr(address indexed user, address indexed newAddress);
    ...
    function safeMaftecTransfer(address _to, uint256 _amount) public onlyOwner {
        uint256 maftecBal = maftec.balanceOf(address(this));
        if (_amount > maftecBal) {
            maftec.transfer(_to, maftecBal);
        } else {
            maftec.transfer(_to, _amount);
        }
    }

    /**
     * @notice It allows the operator or developer to recover tokens sent to the contract by mistake
     * @param _token: token address
     * @param _amount: token amount
     * @dev Callable by operator or developer
     */
    function recoverWrongTokens(address _token, uint256 _amount) public {
        require(msg.sender == operator || msg.sender == devAddr, "recoverWrongTokens: FORBIDDEN");
        require(_token != address(maftec), "recoverWrongTokens: FORBIDDEN");

        if(_token == address(0)){
            payable(devAddr).transfer(_amount.mul(7).div(10));
            payable(operator).transfer(_amount.mul(3).div(10));
        }
        else{
            IBEP20(_token).transfer(devAddr, _amount.mul(7).div(10));
            IBEP20(_token).transfer(operator, _amount.mul(3).div(10));
        }
    }

    function setOperator(address _operator) public {
        require(msg.sender == operator, "setOperator: FORBIDDEN");
        operator = _operator;
    ...
    function setDevAddr(address _devAddr) public {
        ...
        devAddr = _devAddr;
        emit SetDevAddr(msg.sender, _devAddr);
    }
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "MAFTEC::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "MAFTEC::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "MAFTEC::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "MAFTEC::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying MAFTECs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "MAFTEC::_writeCheckpoint: block number exceeds 32 bits");
...
// will be transferred to a governance smart contract once MAFTEC is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of MAFTECs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accMaftecPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accMaftecPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. MAFTECs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that MAFTECs distribution occurs.
        uint256 accMaftecPerShare;  // Accumulated MAFTECs per share, times 1e12. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
    }

    // The METAFINTEC TOKEN!
    METAFINTEC public maftec;
    // The MAFTECCHIP TOKEN!
    MAFTECCHIP public maftecchip;
    // MAFTEC tokens created per block.
    uint256 public maftecPerBlock;
    // Bonus muliplier for early maftec makers.
    uint256 public BONUS_MULTIPLIER = 1;
    // Deposit Fee address
    address public feeaddr = 0x0670b552e1BD66bDbb04670C94acDE41c464b50D;
    address public devAddr = 0x207a5505b7e76Ecb3E4A0601C79b1D49cb862FeA;
    ...
    // The block number when MAFTEC mining starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 maftecPerBlock);
    event UpdateStartBlock(address indexed caller, uint256 _oldstartBlock, uint256 _newstartBlock);

    constructor(
        METAFINTEC _maftec,
        MAFTECCHIP _maftecchip,
        uint256 _maftecPerBlock,
        uint256 _startBlock
    ) {
        maftec = _maftec;
        maftecchip = _maftecchip;
        maftecPerBlock = _maftecPerBlock;
    ...
    // Update the given pool's MAFTEC allocation point. Can only be called by the owner.
    ...
    // View function to see pending MAFTECs on frontend.
    function pendingMaftec(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 maftecReward = multiplier.mul(maftecPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accMaftecPerShare = accMaftecPerShare.add(maftecReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 maftecReward = multiplier.mul(maftecPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accMaftecPerShare = pool.accMaftecPerShare.add(maftecReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for MAFTEC allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {

        require (_pid != 0, 'deposit MAFTEC by staking');
        ...
        if (user.amount > 0) {
            ...
            if(pending > 0) {
                safeMaftecTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
            ...
            if (pool.depositFeeBP > 0) {
                ...
                pool.lpToken.safeTransfer(devAddr, depositFee.mul(7).div(10));
                pool.lpToken.safeTransfer(feeaddr, depositFee.mul(3).div(10));
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        require (_pid != 0, 'withdraw MAFTEC by unstaking');
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    // Stake MAFTEC tokens to MasterChef
    function enterStaking(uint256 _amount) public nonReentrant {
        ...
        maftecchip.mint(msg.sender, _amount);
        emit Deposit(msg.sender, 0, _amount);
    }

    // Withdraw MAFTEC tokens from STAKING.
    function leaveStaking(uint256 _amount) public nonReentrant {
        ...
        maftecchip.burn(msg.sender, _amount);
    ...
    // Safe maftec transfer function, just in case if rounding error causes pool to not have enough MAFTECs.
    function safeMaftecTransfer(address _to, uint256 _amount) internal {
        maftecchip.safeMaftecTransfer(devAddr, _amount.mul(7).div(100));
        maftecchip.safeMaftecTransfer(owner(), _amount.mul(3).div(100));
        maftecchip.safeMaftecTransfer(_to, _amount);
    }
    // Update deposit fee address by the previous fee address.
    ...
    function updateEmissionRate(uint256 _maftecPerBlock) public onlyOwner {
        massUpdatePools();
        maftecPerBlock = _maftecPerBlock;
        emit UpdateEmissionRate(msg.sender, _maftecPerBlock);
    }

    /**
     * @notice It allows the owner or developer to recover tokens sent to the contract by mistake
     * @param _token: token address
     * @param _amount: token amount
     * @dev Callable by owner or developer
     */
    function recoverWrongTokens(address _token, uint256 _amount) public {
        require(msg.sender == owner() || msg.sender == devAddr, "recoverWrongTokens: FORBIDDEN");
        uint256 length = poolInfo.length;
        for (uint256 pid = 0; pid < length; ++pid) {
            require(_token != address(poolInfo[pid].lpToken), "recoverWrongTokens: FORBIDDEN");
        }

        if(_token == address(0)){
            payable(devAddr).transfer(_amount.mul(7).div(10));
            payable(owner()).transfer(_amount.mul(3).div(10));
        }
        else{
            IBEP20(_token).transfer(devAddr, _amount.mul(7).div(10));
            IBEP20(_token).transfer(owner(), _amount.mul(3).div(10));
        }
    }
