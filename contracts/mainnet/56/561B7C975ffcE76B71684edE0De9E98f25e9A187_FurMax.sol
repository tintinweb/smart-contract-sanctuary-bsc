...
contract FurMax is BaseContract
{
    ...
    function availableDividends(address participant_) public view whenNotPaused returns (uint256)
    {
        ...
        return _getAvailable(_available_);
    }
    ...
    {
        if(totalFurpoolPendingInvestment == 0) return;
        uint256 _amount_ = _getAvailable(totalFurpoolPendingInvestment);
     ...
     */
    function _getAvailable(uint256 originalAmount_) internal view returns (uint256)
    {
        uint256 _balance_ = _usdc.balanceOf(address(this));
        if(originalAmount_ > _balance_) return _balance_;
