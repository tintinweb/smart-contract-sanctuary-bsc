...
contract ShinFlokiV2 is ERC20, AccessControl, ERC721Holder {
    ...
    constructor(address owner, address _collectionAddress) ERC20("ShinFlokiV2", "SFLOKIV2") AccessControl(){
        _mint(owner, 1500000 * 10 ** 18);
        ...
        welcomeBonusAmount = 0 * 10 ** 18; // 10 tokens welcome bonus, only paid once per tokenId
    }
    ...
    function stake(uint256 tokenId) public {
        ...
        require(IERC721(collectionAddress).ownerOf(tokenId) == address(this),
            "ERC20: Error while transferring token");
    ...
    function setCollectionAddress(address newAddress) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddress != address(0), "ERC20: update to zero address not possible");
