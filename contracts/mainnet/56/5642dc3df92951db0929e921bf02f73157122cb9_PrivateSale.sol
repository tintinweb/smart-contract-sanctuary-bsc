...
contract PrivateSale is Ownable {
    ...
    uint256 public tokensPerBNB = 3289473;
    ...
    function buyTokensWithoutReferral() public payable {
        
        require(!hasEnded(), "Presale: sale is over");
        require(msg.value >= minContribution, "Presale: min contribution criteria not met");

        uint256 tokenAmount = msg.value.mul(tokensPerBNB);
        
        require(tokenAmount > 0, "You need to send some token.");

        contributions[msg.sender].contribution = contributions[msg.sender].contribution.add(msg.value);
