...
interface IAccount {
    ...
    function info(address addr) external view returns (address parent, uint8 level, uint256 id);
...
contract LoveBank is ReentrancyGuard, AccessControlEnumerable {
    ...
    address usdt = 0x9948a363C8CD2442156824Fe7600290183588c29;
    address gas = 0x7906615c4e3c745FE018ACF5F560dd7923dB04B8;
    address collect = 0x4c596c6595c11c5B11E837b68D84aA63EC5d8D99;
    address gasStake = 0x50dD158B8b27BCa3CB2AbD937b97D0cDB6ac8FE3;

    function baseInit(address _usdt, address _gas, address _collect, address _gasStake, address _random, address _account)
    ...
    constructor() {
        ...
        iAccount = IAccount(0x726E50b3b6E1e50f567Ea344a54BD227c5541Aab);
        iRandom = IRandom(0x294E3EA1001403CCEf5d7B84317563773d532f93);
    ...
    function setDepositInterval(uint8 _index, uint32 _interval) external onlyRole(OPERATOR_ROLE) {
        depositInterval[_index] = _interval;
    }

    function setDepositEach(uint8 _index, uint256 amount) external onlyRole(OPERATOR_ROLE) {
    ...
    function allocAmount(uint256 _allocAmount, uint256 amount) private {
        uint256 brokerAmount = (_allocAmount * 19) / 100;
        sprintBalance += (_allocAmount * 4) / 1000;
        restartBalance += (_allocAmount * 5) / 1000;
        depositTotal += (_allocAmount * 795) / 1000;
        processBonus(msg.sender, brokerAmount);
        processFomo(msg.sender, amount);
        fomoBalance += (_allocAmount * 1) / 1000;
        processSprint(msg.sender);
        IERC20(usdt).safeIncreaseAllowance(gasStake, (_allocAmount * 5) / 1000);
        IGasStake(gasStake).recharge((_allocAmount * 5) / 1000);
    }
    ...
    function processBonus(address addr, uint256 amount) private {
        ...
        for (uint256 i = 0; i < 30; i++) {
            ...
            if (parentLevel == level && level == 5) {
                ...
                uint8 diffPercent = bonusPercent[parentLevel] - bonusPercent[level];
    ...
    function dealBlacklist(uint256 index) external nonReentrant {
        ...
        deposit.status = 40;
        IERC20(gas).transferFrom(msg.sender, collect, 100e18 * brokers[msg.sender].punishTimes);
    ...
    function fomoWithdraw() external nonReentrant {
        require(!checkBlacklist(msg.sender), "LoveBank: you are in blacklist.");
        require(fomoDrawables[msg.sender] > 0, "LoveBank: insufficient of fomo drawable.");
        IERC20(gas).safeTransferFrom(msg.sender, collect, fomoDrawables[msg.sender] / 100);
        collectTotal += fomoDrawables[msg.sender] / 100;
        IERC20(usdt).safeTransfer(msg.sender, fomoDrawables[msg.sender]);
        brokers[msg.sender].profitAmount += int256(fomoDrawables[msg.sender]);
    ...
    function sprintWithdraw() external nonReentrant {
        require(!checkBlacklist(msg.sender), "LoveBank: you are in blacklist.");
        require(sprintDrawables[msg.sender] > 0, "LoveBank: insufficient of sprint drawable.");
        IERC20(gas).safeTransferFrom(msg.sender, collect, sprintDrawables[msg.sender] / 100);
    ...
    function restartInit(uint256 _restartTimes, uint256 _restartRate) external onlyRole(OPERATOR_ROLE) {
        restartTimes = _restartTimes;
        restartRate = _restartRate;
    ...
    function lossWithdraw(uint32 _version) external nonReentrant {
        ...
        losses[msg.sender][_version].lastWithdrawTimestamp = uint32(block.timestamp - (block.timestamp % interval));
    }

    function lossPending(address addr, uint32 _version) public view returns (uint256 drawable) {
