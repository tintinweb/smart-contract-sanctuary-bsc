 ...
 * Main Token Contract
 */
contract Revoltance is IERC20, IERC20Metadata, Ownable {
    ...
    string constant _name = "Revoltance";
    string constant _symbol = "RVLT";
    uint8 constant _decimals = 18;

    uint256 _totalSupply =  50000000000 * (10 ** 18);

    uint256 public maxTxAmount = _totalSupply.div(50);
    ...
    uint256 public operationsFee = 250;
    uint256 public liquidityFee = 50;
    uint256 public totalFee = 300;
    uint256 public feeDenominator = 10000;

    uint256 public contractReserve = .1 * (10**18);
    ...
    function _transferFrom(address sender_, address recipient_, uint256 amount_) internal returns (bool) {
        require(!freeze_contract || isFreezeExempt[sender_] || isFreezeExempt[recipient_], "Contract frozen!");
        require(amount_ <= maxTxAmount || isMaxTxExempt[sender_], "TX Limit Exceeded");
    ...
    function swapBack() internal swapping {
        ...
        try pcsRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            ...
            uint256 amountBNB = address(this).balance.sub(contractReserve);
            ...
            uint256 operationsBNB = amountBNB.sub(liquidityBNB);

            payable(address(operationsReceiver)).transfer(operationsBNB);
        
            if(amountToLiquify > 0){
                try pcsRouter.addLiquidityETH{ value: liquidityBNB }(
                    address(this),
                    amountToLiquify,
                    0,
                    0,
                    ...
                    emit AutoLiquify(amountToLiquify, liquidityBNB);
                } catch {
    ...
    function setFees(
            uint256 recevierFee_,
            uint256 liquidityFee_,
            uint256 feeDenominator_
    ) external onlyOwner {
        operationsFee = recevierFee_;
        liquidityFee = liquidityFee_;
        totalFee = operationsFee.add(liquidityFee);
    ...
    function setOperationsReceiver(address operationsReceiver_) external onlyOwner {
    ...
    function setisMaxTxExempt(address holder_, bool exempt_) external onlyOwner {
        isMaxTxExempt[holder_] = exempt_;
    }
    ...
    function setIsBlacklist(address wallet_, bool blacklisted_) external onlyOwner {
        isBlacklisted[wallet_] = blacklisted_;
    }
    ...
    function setContractReserve(uint256 contractReserve_) external onlyOwner {
        contractReserve = contractReserve_;
    }
