...
contract ERC1155 is Context, ERC165, IERC1155{
    ...
    function _burnBatch(address account, uint256[] memory ids, uint256[] memory amounts) internal virtual {
        ...
        for (uint i = 0; i < ids.length; i++) {
            require(_balances[ids[i]][account] >= amounts[i], "ERC1155: burn amount exceeds balance");
...
abstract contract ERC1155Base is HasSecondarySaleFees, Ownable, ERC1155 {
    ...
    function uri(uint256 _id) override virtual external view returns (string memory) {
        return string(abi.encodePacked(tokenURIPrefix, _tokenURIs[_id]));
    }
...
contract HubGameNFT is ERC1155Base, SignerRole {
    string public name;
    ...
    */
    constructor() {
        name = "HubGameNFT";
        symbol = "HUBNFT";
        tokenURIPrefix = "https://gateway.pinata.cloud/ipfs/";
    ...
    function changeUriPrefix(string memory _prefixUri) public onlyAdmin{
        tokenURIPrefix = _prefixUri;
    }

    function mint(uint256 _tokenId, Fee[] memory _fees, uint256 _supply, string memory _uri) public {
        require(
            isSigner(_msgSender()),"invalid sender"
        );
