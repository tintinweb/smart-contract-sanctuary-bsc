...
contract memegang is ERC20, Auth {
    ...
    event SellFeesChanged(uint256 _liquidityFee, uint256 _marketingFee, uint256 _operationsFee,uint256 _devFee, uint256 _stakingFee);
    event BuyFeesChanged(uint256 _liquidityFee, uint256 _marketingFee, uint256 _operationsFee, uint256 _devFee, uint256 _stakingFee);
    ...
    string constant private _name = "memegang";
    string constant private _symbol = "MemeG";
    ...
    uint256 private donationsFee = 2;
    ...
    uint256 private sellFeeDonations = 2;
    ...
    //totalFees
    uint256 private totalBuyFee = liquidityFee.add(marketingFee).add(donationsFee).add(devFee).add(stakingFee);
    uint256 private totalSellFee = sellFeeLiquidity.add(sellFeeMarketing).add(sellFeeDonations).add(sellFeeDev).add(sellFeeStaking);

    uint256 private feeDenominator  = 100;

    address private autoLiquidityReceiver;
    address private marketingFeeReceiver = 0xcb50f4Dd0f1E4c79AAbd9E7Bfa5BF088ef1bb401;
    address private donationsFeeReceiver = 0x5DEd9e5ab03386e4b2D51deB781aC0c65dfaa71e;
    address private stakingFeeReceiver = 0x7FAe320Ee5236bE10dd72FF45309796640343540;
    address private devFeeReceiver = 0x37e63071Ab5f7474f8336D2C588AEb731BE3F79d;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBDev = amountBNB.mul(devFee.add(sellFeeDev)).div(totalBNBFee);
        uint256 amountBNBDonations = amountBNB.sub(amountBNBLiquidity).sub(amountBNBMarketing).sub(amountBNBDev);
        ...
        (tmpSuccess,) = payable(donationsFeeReceiver).call{value: amountBNBDonations}("");
    ...
    function setBuyFees(uint256 _liquidityFee, uint256 _marketingFee, uint256 _donationsFee, uint256 _devFee, uint256 _stakingFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        marketingFee = _marketingFee;
        donationsFee = _donationsFee;
        devFee = _devFee;
        stakingFee = _stakingFee;
        totalBuyFee = _liquidityFee.add(_marketingFee).add(_donationsFee).add(_devFee).add(_stakingFee);
        feeDenominator = _feeDenominator;
        require(totalBuyFee <= maxFee, "Fees cannot be more than 30%");

        emit BuyFeesChanged(_liquidityFee, _marketingFee, _donationsFee, _devFee, _stakingFee);
    }

    function setSellFees(uint256 _liquidityFee, uint256 _marketingFee, uint256 _donationsFee, uint256 _devFee, uint256 _stakingFee, uint256 _feeDenominator) external authorized {
        sellFeeLiquidity = _liquidityFee;
        sellFeeMarketing = _marketingFee;
        sellFeeDonations = _donationsFee;
        sellFeeDev = _devFee;
        sellFeeStaking = _stakingFee;
        totalSellFee = _liquidityFee.add(_marketingFee).add(_donationsFee).add(_devFee).add(_stakingFee);
        feeDenominator = _feeDenominator;
        require(totalSellFee <= maxFee, "Fees cannot be more than 30%");

        emit SellFeesChanged(_liquidityFee, _marketingFee, _donationsFee, _devFee, _stakingFee);
    }
    ...
    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _donationsFeeReceiver, address _devFeeReceiver, address _stakingFeeReceiver) external authorized {
        require(_autoLiquidityReceiver != address(0) && _marketingFeeReceiver != address(0) && _donationsFeeReceiver != address(0)  && _devFeeReceiver != address(0) && _stakingFeeReceiver != address(0), "Zero Address validation" );
        autoLiquidityReceiver = _autoLiquidityReceiver;
        marketingFeeReceiver = _marketingFeeReceiver;
        donationsFeeReceiver = _donationsFeeReceiver;
        devFeeReceiver = _devFeeReceiver;
        stakingFeeReceiver = _stakingFeeReceiver;

        emit SetFeeReceivers(_autoLiquidityReceiver, _marketingFeeReceiver, _donationsFeeReceiver,_devFeeReceiver, _stakingFeeReceiver);
    }
