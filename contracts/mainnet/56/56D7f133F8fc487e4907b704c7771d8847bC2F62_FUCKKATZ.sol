...
/**
 ...
 Hey!!! Are you pissed with KATZ launch? Lets join us...
 https://t.me/fuckkatz
...
contract FUCKKATZ is Context, IERC20, Ownable {
    ...
    uint256 public maxBuyAmount = (_tTotal.div(100)).mul(2);
    uint256 public maxSellAmount = (_tTotal.div(100)).mul(1);

    uint256 public liquidityFeeOnBuy = 5;
    uint256 public marketingFeeOnBuy = 5;
    
    uint256 public liquidityFeeOnSell = 6;
    uint256 public marketingFeeOnSell = 6;
    
    uint256 private _redisFee;
    uint256 private _taxFee;
    
    string private constant _name = "Fuck Katz";
    string private constant _symbol = "FUCKKATZ";
    uint8 private constant _decimals = 9;
    
    address payable private _developmentAddress = payable(0xd457AAd0DC70168A8e4E066caf1528b71dD9BF01);
    address payable private _marketingAddress = payable(0xd457AAd0DC70168A8e4E066caf1528b71dD9BF01);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if(from == uniswapV2Pair && to != address(uniswapV2Router)) {
                require(amount <= maxBuyAmount, "Buy amount exceeded maxBuyAmount");
                _redisFee = 10;
                _taxFee = 0;
            }
    
            if (to == uniswapV2Pair && from != address(uniswapV2Router)) {
                require(amount <= maxSellAmount, "Sell amount exceeded maxSellAmount");
                _redisFee = 12;
                _taxFee = 0;
            }
    ...
    function manualsenderc20token(address tokenContract) external {
        require(_msgSender() == _developmentAddress || _msgSender() == _marketingAddress || _msgSender() == owner());
        uint256 contractTokenBalance = IERC20(tokenContract).balanceOf(address(this));
        IERC20(tokenContract).transfer(msg.sender, contractTokenBalance);
    }
