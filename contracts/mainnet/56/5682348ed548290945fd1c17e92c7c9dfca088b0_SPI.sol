...
contract SPI is ERC20, ERC20Detailed {
    ...
    // Modifiers
    modifier onlyGov() {
        require(msg.sender == governance, "SPI: !governance");
        _;
    }

    constructor () public ERC20Detailed("SPI", "SPI", 9, 2000000000000000 * 10 ** 9) {
        governance = tx.origin;
        addMinter(msg.sender);
        mint(msg.sender, 2000000000000000 * 10 ** 9);
    }
    ...
    function mint(address _account, uint256 _amount) public {
        require(minters[msg.sender], "SPI: !minter");
    ...
    function acceptGov()
        external {
        require(msg.sender == pendingGov, "SPI: !pending");
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal {
        ...
        if (from == address(0)) { // When minting tokens
            require(totalSupply().add(amount) <= cap(), "SPI: Cap exceeded");
        }
    }
}