...
contract ContractGuard {
    ...
    function checkSameAccountReentranted(address _account) internal view returns (bool) {
        return _status[block.number][_account];
    }

    modifier onlyOneBlock(address _account) {
        require(!checkSameAccountReentranted(tx.origin), "ContractGuard: one block, one function");
        require(!checkSameAccountReentranted(_account), "ContractGuard: one block, one function");

        _;

        _status[block.number][tx.origin] = true;
        _status[block.number][_account] = true;
    }
}
...
interface ITreasury {
    ...
    function getKrakenPrice() external view returns (uint256);

    function buyBonds(uint256 amount, uint256 targetPrice) external;

    function redeemBonds(uint256 amount, uint256 targetPrice) external;

    function krakenPriceOne() external view returns(uint256);
...
contract ShareWrapper {
    ...
    function _stake(address sender, address recipient, uint256 amount) internal virtual {
        ...
        share.safeTransferFrom(sender, address(this), amount);
    }
...
// File: SSBoardroom.sol

contract SSBoardroom is ShareWrapper, ContractGuard, Operator {
    using SafeERC20 for IERC20;
    using Address for address;
    using SafeMath for uint256;
    ...
    function getKrakenPrice() external view returns (uint256) {
        return treasury.getKrakenPrice();
    }
    ...
    function stakeFor(address recipient, uint256 amount) public {
    ...
    function _stake(address sender, address recipient, uint256 amount) internal override onlyOneBlock(recipient) updateReward(recipient) {
        require(amount > 0, "Boardroom: Cannot stake 0");
        super._stake(sender, recipient, amount);
        members[recipient].epochTimerStart = treasury.epoch(); // reset timer
        emit Staked(recipient, amount);
    }

    function withdraw(uint256 amount) public override onlyOneBlock(msg.sender) memberExists updateReward(msg.sender) {
    ...
    function claimReward() public updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            kraken.safeTransfer(msg.sender, reward);
            emit RewardPaid(msg.sender, reward);
        }
    }

    function allocateSeigniorage(uint256 amount) external onlyOneBlock(msg.sender) onlyOperator {
        ...
        kraken.safeTransferFrom(msg.sender, address(this), amount);
