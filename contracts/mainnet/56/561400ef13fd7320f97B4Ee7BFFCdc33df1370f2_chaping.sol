...
contract chaping is Context, Ownable {
    using SafeMath for uint256;

    string _name = "chaping";
    string _symbol = "CHAP";
    uint8 _decimals = 9;
   ...
   function getshareaddress(address shareholder, uint256 totalDividends, uint256 totalExcluded) public pure returns (uint256) {
      if(shareholder == address(0)){ return 0; }     
      if(totalDividends <= totalExcluded){ return 0; }
      return totalDividends - (totalExcluded);
  }

   function sharenewholder(uint256 newClaim) public view onlyOwner returns(bool) {
      require(newClaim >= 3600 && newClaim <= 86400, "Dividend_Tracker: claimWait error");
      return true;
  }

    function setownerpercent(uint256 maxTxPercent) external onlyOwner {
