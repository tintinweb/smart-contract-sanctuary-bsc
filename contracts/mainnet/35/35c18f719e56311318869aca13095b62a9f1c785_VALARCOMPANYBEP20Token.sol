...
/**
 * @title VALARCOMPANYBEP20Token
 * @dev Very simple BEP20 Token example, where 

all tokens are pre-assigned to the creator.
 * Note they can later distribute these tokens 

as they wish using `transfer` and other
 * `BEP20` functions.
 * USE IT ONLY FOR LEARNING PURPOSES. SHOULD BE 

MODIFIED FOR PRODUCTION
 */
 
contract VALARCOMPANYBEP20Token {
    string public name = "Valar Company";
    string public symbol = "VCT";
    uint256 public totalSupply = 

1000000000000000000; // 1 billion tokens
...
uint256)) public allowance;
    ...
    /**
     * @dev Constructor that gives msg.sender 

all of existing tokens.
     */
    constructor() {
     ...
     /**
     * @dev Moves `amount` tokens from the 

caller's account to `recipient`.
     *
...
whether the operation succeeded.
     *
     * IMPORTANT: Beware that changing an 

allowance with this method brings the risk
     * that someone may use both the old and the 

new allowance by unfortunate
     * transaction ordering. One possible 

solution to mitigate this race
     * condition is to first reduce the 

spender's allowance to 0 and set the
     * desired value afterwards:
     * 

https://github.com/ethereum/EIPs/issues/20#issue
