...
contract uMetaLands is ERC721Enumerable, Ownable {
    using Strings for uint256;

    uint256 public constant RESERVED_TOKENS = 0;
    uint256 public constant FOR_SALE_TOKENS = 10000 - RESERVED_TOKENS;
    address public constant RESERVED_TOKENS_ADDRESS = 0x0000000000000000000000000000000000000000;
    ...
    uint256 public maxTokensPerAddressPresale = 10;
    ...
    string public tokenBaseURI = "ipfs://Qmeq9nJhRKpZA1qJE7NjgGJcJzVcGvSmPrbCpomrvq5grA/";

    uint256 public soldAmount = 0;
    mapping(address => uint256) public purchased;
    mapping(address => uint256) public presalePurchased;

    constructor() ERC721("uMeta Lands", "uMeta Lands Test") {
        if (RESERVED_TOKENS > 0)
            for (uint256 i = 0; i < RESERVED_TOKENS; i++) {
                _safeMint(msg.sender, 1 + i);
            }
    }
    ...
    function getMaxTokensForPhase(address target, bytes32[] memory proof) public view returns (uint256) {
        ...
        if (currentStage == STAGE_PUBLIC) {
            uint256 presaleLeftover = 0;
            if(whitelisted) {
                presaleLeftover = maxTokensPerAddressPresale - presalePurchased[target];
            }
            return maxTokensPerAddress + presaleLeftover;
    ...
    function mint(uint256 amount, bytes32[] calldata proof) external payable {
        ...
        for(uint256 i = 0; i < amount; i++) {
            _safeMint(msg.sender, 1 + i + soldAmount + RESERVED_TOKENS);
        }
    ...
    function burn(uint256 tokenId) external {
        require(_isApprovedOrOwner(msg.sender, tokenId), "Not approved to burn token");
        _burn(tokenId);
    }
