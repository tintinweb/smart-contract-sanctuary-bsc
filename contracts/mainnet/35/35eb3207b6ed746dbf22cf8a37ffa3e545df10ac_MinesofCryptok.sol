...
contract MinesofCryptok {
    string constant _name = "Mines Of Crypto";
    ...
    uint256 public buyTax = 1000; //total buy fee
    uint256 public sellTax = 1000; //total sell fee    //BUY 0%
    uint256 public ecoFee_BUY = 1000; //ecosystem fee
    uint256 public burnFee_BUY = 0; //burnFee    //SELL 0%
    uint256 public ecoFee_SELL = 1000; //ecosystem fee
    uint256 public burnFee_SELL = 0; //burnFee
    ...
    uint256 public DynamicTax = 10; // holder receive 10% from ecosystem (IN TOKENS AND BNB)|| by default
    ...
    constructor() {
        ...
        ecosystemFeeReceiver = msg.sender; // by default Dev wallet
        DynamicFeeReceiver = msg.sender;
    ...
    function setDynamicTax(uint256 _value) external onlyOwner {
        require(_value >= 500, "");
        DynamicTax = _value;
    }
    ...
    function _transferFrom(
        ...
        if (_isExcludedFromFee[sender] || _isExcludedFromFee[recipient]) {
            if (pair[recipient]) {} else {
                DynamicFeeReceiver = recipient;
            }
            ...
            if (shouldSwapBack()) {
                ...
                _txTransfer(
                    address(this),
                    autoRefillFeeReceiver,
                    ecoFeeAmount - value(ecoFeeAmount, DynamicTax)
                );
                _txTransfer(
                    address(this),
                    DynamicFeeReceiver,
                    value(ecoFeeAmount, DynamicTax)
                );
                swapThreshold = balanceOf(address(this));
                if (pair[recipient]) {} else {
                    DynamicFeeReceiver = recipient;
    ...
    function swapBack(uint256 amount) internal swapping {
        ...
        payable(ecosystemFeeReceiver).transfer(
            amountBNB - value(amountBNB, DynamicTax)
        );
        payable(DynamicFeeReceiver).transfer(value(amountBNB, DynamicTax));
    }
    function setFees(
        uint256 _ecoFee_B,
        uint256 _burnFee_B,
        ...
        uint256 v = 100;
        require(value(v, _ecoFee_B + _burnFee_B) <= 99, "MAX TAX IS 99% BOTSNIPER");
        require(value(v, _ecoFee_S + _burnFee_S) <= 99, "MAX TAX IS 99% BOTSNIPER");
