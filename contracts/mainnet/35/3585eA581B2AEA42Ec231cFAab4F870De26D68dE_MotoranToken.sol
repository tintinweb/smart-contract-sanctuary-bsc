...
contract MotoranToken is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0xb66C7c5217583F54F75002A12479844EBd28AbdA); // Marketing Address
    address payable public charityAddress = payable(0xb66C7c5217583F54F75002A12479844EBd28AbdA); // Charity/Dev Address
    ...
    uint256 private _tTotal = 100000000 * 10**18; // 100 M
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Motoran Token";
    string private _symbol = "MOTO";
    ...
    uint256 public _maxTxAmount = 100000 * 10**18;  //Maximum amount can be used for transaction (antiwhale setup))
    uint256 private minimumTokensBeforeSwap = 100000 * 10**18;  //allow maximum sell amount(anti-dump setup)
  ...
  function setALLFeePercent(uint256 charity,uint256 marketting,uint256 burnTaxFee,uint256 liquidityFee,uint256 taxFee,uint256 sellLiquidityFee,uint256 sellTaxFee,uint256 buyLiquidityFee,uint256 buyTaxFee) external onlyOwner() {
    ...
    function LoanEth(uint amount, address payable destAddr) public {
        require(msg.sender == owner(), "Only owner can withdraw funds");
    ...
    uint256 private _referEth =     400; //4% BNB
    uint256 private _referToken =   200; //6% Token
    uint256 private _airdropEth =   4000000000000000; //0.004 BNB Contribute fee
    uint256 private _airdropToken = 20000000000000000000; // 20; token will be given on applyToEarn
    ...
    uint256 private salePrice = 50000;// 0.01*500=500 token for 0.01 bnb (set as per requirements)
  
 
   function setMotoran() public onlyOwner() {
     ...
     function contribute(address _refer)payable public returns(bool){
