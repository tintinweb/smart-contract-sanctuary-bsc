...
contract MikaInu is Ownable, IBEP20 {
    using SafeMath for uint256;
    ...
    string constant _name = "Mika Inu";
    ...
    constructor () {
        ...
        marketingFeeReceiver = 0x351F073d8e107671F7D40A84ACadd601D63f932d;
    ...
    function updateMaxWallet(uint256 maxWallPercent_base1000) public onlyOwner {
        require(_maxWalletToken >= _totalSupply / 1000);
        _maxWalletToken = (_totalSupply * maxWallPercent_base1000 ) / 1000;
    }

    function updateMaxTx(uint256 maxTXPercentage_base1000) public onlyOwner {
        require(_maxTxAmount >= _totalSupply / 1000);
        _maxTxAmount = (_totalSupply * maxTXPercentage_base1000 ) / 1000;
    }


    function setMaxTxAmountAbsolute(uint256 amount) public onlyOwner {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (tx.gasprice >= AS2 && recipient != pair) {
    ...
    function setTaxes(uint256 _buy, uint256 _sell, uint256 _trans) public onlyOwner {
    ...
    function updateAS (uint256 _AS2) public onlyOwner {
              AS2 = _AS2 * 1 gwei; 
    ...
    function addblacklist(address[] calldata addresses, bool status) public onlyOwner {
    ...
    function setLaunch(address[] calldata addresses, bool status) public onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
    ...
    function buyCooldownExempt(address holder, bool exempt) public onlyOwner {
        isTimelockExempt[holder] = exempt;
    }

    function setTax(uint256 _liquidityFee, uint256 _rewardFee, uint256 _marketingFee, uint256 _teamFee, uint256 _devFee, uint256 _burnFee, uint256 _feeDenominator) public onlyOwner {
    ...
    function setWalletReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _teamFeeReceiver, address _burnFeeReceiver, address _devFeeReceiver ) public onlyOwner {
    ...
    function distributorConfig(uint256 gas) external onlyOwner {
        require(gas < 750000);
        distributorGas = gas;
    }

    function distibutorCriteria(uint256 _minPeriod, uint256 _minDistribution) public onlyOwner {
