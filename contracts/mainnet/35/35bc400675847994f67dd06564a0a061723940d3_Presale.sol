...
contract Presale is Ownable {
    ...
    function buyToken() public payable returns (address) {
        ...
        uint256 initialtokenamount = 0;
        uint256 twentyfiveperlockamount = tokenAmount.mul(25).div(100);
        uint256 fiftyperlockamount = tokenAmount.mul(50).div(100);
        ...
        wallet = createTokenLockedWallets(block.timestamp+ (30 * 1 days),fiftyperlockamount,_msgSender());
        wallet = createTokenLockedWallets(block.timestamp+ (60 * 1 days),twentyfiveperlockamount,_msgSender());
        wallet = createTokenLockedWallets(block.timestamp+ (90 * 1 days),twentyfiveperlockamount,_msgSender());
