...
contract MagpieBridge is OwnableUpgradeable, UUPSUpgradeable, IMagpieBridge {
    ...
    function bridgeOut(
        ...
        if (payload.bridgeType == BridgeType.Wormhole) {
            amount = adjustAssetDecimals(assetAddress, payload.senderIntermediaryDecimals, payload.amountIn);
    ...
    function adjustAssetDecimals(
        ...
        if (fromDecimals > receiverIntermediaryDecimals) {
            amount = normalize(fromDecimals, receiverIntermediaryDecimals, amountIn);
        } else {
            amount = denormalize(fromDecimals, receiverIntermediaryDecimals, amountIn);
        }
    }
