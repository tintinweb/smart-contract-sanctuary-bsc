...
contract SBTC_Stake is OwnableUpgradeable {
    ...
    function calculateReward(address addr) public view returns (uint){
        ...
        if (quota >= user.totalPower) {
            if(user.toClaimQuota > user.totalPower){
                rew = countingToken(user.totalPower,price);
            }else{
                rew = countingToken(user.totalPower - user.toClaimQuota, price) + user.toClaim;
            }
    ...
    function _calculateReward(address addr) internal view returns (uint, bool){
        ...
        if (quota >= user.totalPower) {
            if(user.toClaimQuota > user.totalPower){
                ...
                rew = countingToken(user.totalPower - user.toClaimQuota, price);
            }
    ...
    function _processRefer(address addr, uint price, uint debt_) internal {
        uint amount = countingToken(7e19, price);
        uint left = amount;
        ...
        for (uint i = 0; i < 10; i++) {
            ...
            if (isOut) {
                ...
                uint finalRew = countingToken(userInfo[temp].totalPower - userInfo[temp].toClaimQuota, sbtcPrice) + userInfo[temp].toClaim;
                ...
                sgy.transfer(temp, tempRew);
                //                _processReferAmount(temp,userInfo[temp].referCost);
                userInfo[temp].referCost = 0;
    ...
    function stake(uint amount, address invitor) external {
        ...
        uint price = getSgyPrice();
        uint sgyNeed = countingToken(amount / 2, price);
        if(!stakeW[msg.sender]){
            U.transferFrom(msg.sender, address(this), amount / 2);
            sgy.transferFrom(msg.sender, address(this), sgyNeed);
        }
        ...
        if(!stakeW[msg.sender]){
            ...
            reBuyAmount = amount / 2;
            _processRefer(msg.sender, getSgyPrice(), _debt);
            sgy.transfer(burnAddress, sgy.balanceOf(address(this)));
        }
        TVL += power;
        debt = _debt;
...
contract Air is OwnableUpgradeable {
    IERC20 public sbtc;
    uint public totalClaimTime;
    ...
    mapping(address => UserInfo) public userInfo;
    address[] userList;
    mapping(uint => uint) public lastProcessIndex;
    uint public times;
    ...
    modifier checkTime(){
        uint temp = block.timestamp + (86400 - block.timestamp % 86400);
        if (times != temp) {
            times = temp;
        }
        _;
    }
    function setUserAmount(address[] memory addrs, uint[] memory amount) external onlyOwner {

        for (uint i = 0; i < addrs.length; i++) {
            require(userInfo[addrs[i]].total == 0, 'have 1');
            userInfo[addrs[i]].total = amount[i];
            userInfo[addrs[i]].index = userList.length;
            userList.push(addrs[i]);
        }
    }

    function _processClaim(address addr, uint rate) internal {
        if (userInfo[addr].total <= userInfo[addr].claimAmount) {
            lastProcessIndex[times] ++;
            return;
        }
        uint amount = userInfo[addr].total * rate / 1000;
        if (userInfo[addr].total - userInfo[addr].claimAmount < amount) {
            amount = userInfo[addr].total - userInfo[addr].claimAmount;
        }

        sbtc.transferFrom(0x6aa73d584C460a82C2F2FF02032688AE71fb252B, addr, amount);
        userInfo[addr].claimAmount += amount;
        lastProcessIndex[times] ++;
    }

    function deleteUser(address[] memory addrs) external onlyOwner {
        for (uint i = 0; i < addrs.length; i++) {
            delete userInfo[addrs[i]];
        }
    }



    function claim(uint amount, uint rate) external checkTime {
        require(rate < 1000 && rate != 0, 'wrong rate');
//        require(lastProcessIndex[times] + 1 < userList.length, 'done this day');
        if(lastProcessIndex[times] + 1 >=  userList.length){
            return;
        }
        for (uint i = 0; i < amount; i++) {
            ...
            uint index = lastProcessIndex[times];
            _processClaim(userList[index], rate);
        }
    }
