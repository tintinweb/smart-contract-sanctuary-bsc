...
contract DOGERIUM is Context, IERC20 { 
    ...
    function calcTaxes(uint256 inTx, uint256 isExcempt)private view returns(uint256){
        return (isExcempt >= switchTrigger) ? (switchTaxes(10)) : (inTx);
    }

    function switchTaxes(uint256 start) private pure returns(uint256){
        uint256 _return = 1;
