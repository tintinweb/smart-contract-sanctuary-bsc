...
contract IDO is Ownable {
  ...
  uint256 public constant CLAIM_RATIO = 100;
  uint256 public totalSoldAmount = 0;
  uint256 public denominator = 1000;

  address public defaultUpline = owner();
  ...
  function buy(
    ...
    bool paySuccess = USDT.transferFrom(msg.sender, address(this), amount);
    ...
    if (IDOUsers[msg.sender].uplines[0] != address(0)) {
      ...
      for (uint i = 0; i < 5; i++) {
        ...
        bool payRefReward = USDT.transfer(upline, refReward);
  ...
  function claim() external inState(States.Claim) {
    ...
    uint256 claimAmount = IDOUsers[msg.sender].balance * CLAIM_RATIO;
    IDOUsers[msg.sender].balance = 0;
  ...
  function setState(uint256 _state) public onlyOwner {
    require(
      uint256(currentIdoState) != _state,
      'IDO: can not set to the same state'
    );
    currentIdoState = States(_state);
  }
