...
//File: [IMasterChef_BSC_KyotoSwap.sol]

interface IMasterChef_BSC_KyotoSwap
{
    ...
    function poolInfo(uint256 _pid) external view returns(uint256 accKswapPerShare, uint256 lastRewardBlock, uint256 allocPoint, uint256 totalBoostedShare, bool isRegular);
...
abstract contract VaultStrategyV1 is IVaultStrategy, VaultAccessManager
{
    ...
    function setLPSwapPathes(address[] memory _pathRewardToToken0, address[] memory _pathRewardToToken1) external
    {
        //check
        requireManager();
        if (_pathRewardToToken0.length > 0)
        {
            require(_pathRewardToToken0.length >= 2, "Invalid path lengt");
            require(_pathRewardToToken0[0] == address(rewardToken), "Invalid path lengt");
            require(_pathRewardToToken0[_pathRewardToToken0.length - 1] == address(lpToken0), "Invalid path lengt");
        }
        if (_pathRewardToToken1.length > 0)
        {
            require(_pathRewardToToken1.length >= 2, "Invalid path lengt");
            require(_pathRewardToToken1[0] == address(rewardToken), "Invalid path lengt");
            require(_pathRewardToToken1[_pathRewardToToken1.length - 1] == address(lpToken1), "Invalid path lengt");
        }

        //set
        path_rewardToToken0 = _pathRewardToToken0;
        path_rewardToToken1 = _pathRewardToToken1;

        //events
        emit ConfigChanged("SetLPSwapPathes", 0);
    }
    ...
    function withdraw(address _user, uint256 _amount) external virtual override
    {
        ...
        //withdraw from pool
        (uint256 poolWithdrawAmount, uint256 untaxedAmount) = withdrawFromPool(_amount);

        //withdraw from strategy
        withdrawFromStrategy(_user, _amount, poolWithdrawAmount);
    }

    function withdrawFromPool(uint256 _amount) internal virtual returns (uint256 poolWithdrawAmount, uint256 untaxedAmount)
    {
        //withdraw from pool
        uint256 currentBalance = balanceOfStrategy();
        poolWithdrawAmount = 0;
        untaxedAmount = _amount;
        if (currentBalance < _amount)
        {
            poolWithdrawAmount = _amount.sub(currentBalance);
            untaxedAmount = _amount.sub(poolWithdrawAmount);
        ...
        return (poolWithdrawAmount, untaxedAmount);
    }
    ...
    function rewardToDepositLPToken() internal virtual
    {
        ...
        if (lpToken0 != rewardToken)
        {
            swapTokens(halfReward, rewardToken, lpToken0, path_rewardToToken0);
        }

        //swap reward to token 1
        if (lpToken1 != rewardToken)
        {
            swapTokens(halfReward, rewardToken, lpToken1, path_rewardToToken1);
        }
    ...
    function routerSwapTokens(uint256 _amount, address[] memory _path) internal virtual
    {
        //swap
    ...
    function makeSwapPath(IToken _from, IToken _to) internal virtual view returns (address[] memory)
	{
	    address[] memory path;
...
contract VaultStrategy_BSC_KyotoSwap_MasterChef is VaultStrategyV1
{
    //========================
    // LIBS
    //========================
    ...
    IToken public constant REWARD_TOKEN = IToken(0x29ABc4D03D133D8Fd1F1C54318428353CE08727E);
    address public constant MASTERCHEF = 0x0e553D0c74E107aE6e7DD8426577680C2303debf;
    address public constant ROUTER = 0x9fd7764e2303E7748736D115304350eC64E403B2;
    ...
    { 
        //init
        depositToken = IToken(IMasterChef_BSC_KyotoSwap(MASTERCHEF).lpToken(_poolID));
    ...
    function balanceOfPool() public view virtual override returns (uint256)
    {
        (uint256 _amount, , ) = IMasterChef_BSC_KyotoSwap(masterChef).userInfo(poolID, address(this));
        return _amount;
    }

    function poolAllocPoints() public view virtual override returns (uint256)
    {
        (, , uint256 allocPoints, , ) = IMasterChef_BSC_KyotoSwap(masterChef).poolInfo(poolID);
        return allocPoints;
    }

    function poolPending() public view virtual override returns (uint256)
    {
        return IMasterChef_BSC_KyotoSwap(masterChef).pendingKswap(poolID, address(this));
    }
    ...
    {
        IMasterChef_BSC_KyotoSwap(masterChef).deposit(poolID, _amount);
    }

    function poolWithdraw(uint256 _amount) internal override
    {
       
        IMasterChef_BSC_KyotoSwap(masterChef).withdraw(poolID, _amount);
    }

    function emergencyWithdraw() internal override
    {
        IMasterChef_BSC_KyotoSwap(masterChef).emergencyWithdraw(poolID);
    }
}