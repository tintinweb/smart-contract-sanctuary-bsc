...
interface IERC20 {
    ...
    function userClaim() external{
       ...
       //合约中需要发放的token
       uint tokenBalance = presaleTokenUser.balanceOf(address(this));
       ...
       require(tokenBalance>mount*10**18,'Insufficient token, please contact the administrator');
       require(judgeSale[msg.sender]!=false,'Insufficient token, please contact the administrator');
        //向项目方转币
        usdtTokenUser.transferFrom(msg.sender, owner, money);
       //合约向用户转币
       presaleTokenUser.transferFrom(address(this), msg.sender, mount);
       //成功后将用户打入黑名单不可在认购
    ...
    function claimToken(address token, uint256 amount, address to) external onlyOwner {
        IERC20(token).transfer(to, amount*10**18);
    }
