...
contract WCBet is Ownable, ReentrancyGuard {
    ...
    */
    ...
    event Claim(address indexed user, uint256 indexed matchId, uint256 amount);
    event Referral(address indexed user, address indexed referral, uint256 indexed matchId, uint256 amount);
    ...
    function setRef(uint256 _ref) external onlyOwner {
        REF = _ref;
    }
    ...
    function bet(
        ...
        validMatchId(_matchId) 
        onlyMatchNotStart(_matchId) 
        onlyMatchNotEnded(_matchId) 
        nonReentrant 
    {
        ...
        if(referrals[msg.sender] == address(0) && _referral != address(0) && _referral != msg.sender) {
        ...
        b.bet = _bet;
        b.amount += msg.value;
        emit UserBet(msg.sender, _matchId, _bet, _bet0, _bet1, msg.value);
    }
    ...
    function claim(uint256 _matchId) external validMatchId(_matchId) onlyMatchEnded(_matchId) {
        ...
        if(amount > 0) {
            ...
            if(ref != address(0)) {
                refAmount = (amount * REF) / 100000;
            }
            require(address(this).balance >= amount + refAmount, "Not enough token in pool to claim");
            (bool sent,) = msg.sender.call{value: amount}("");
            require(sent, "Transfer claim amount failed");
            if(refAmount > 0) {
                (bool sentRef,) = ref.call{value: refAmount}("");
                require(sentRef, "Transfer ref amount failed");
                emit Referral(msg.sender, ref, _matchId, refAmount);
            }
            
            emit Claim(msg.sender, _matchId, amount);
        }
    }
}