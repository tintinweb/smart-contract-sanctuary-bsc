...
contract ProjectWindowLicker is BEP20, Ownable {
    using SafeMath for uint256;
    ...
    mapping(address => bool) private _isTeamVesting;
    mapping(address => uint256) private tokensVesting;
    mapping(address => uint256) private teamTokensVesting;
    ...
    constructor() BEP20("ProjectWindowLicker", "LickedWindows") {
        marketingWallet = payable(0x335Fa4b78211EeF3f29EfdF67AA5Dc67406B5cC5);
        devWallet = payable(0x70D0CC26167d8AAB809D3e71Cae6710bAb5c044d);
        ...
        maxWallet = _totalSupply * 3 / 120; // 2.5%
    ...
    function updateFees(uint256 marketingBuy, uint256 marketingSell, uint256 liquidityBuy,
        ...
        require(totalBuyFees <= 15 && totalSellFees <= 15, "total fees cannot exceed 15%");
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (_isVesting[from]) {
            if (block.timestamp < launchtimestamp + 1 minutes) {
                require(amount <= tokensVesting[from] * 3 / 10, "30%");
            }
            else if (block.timestamp > launchtimestamp + 1 minutes && block.timestamp < launchtimestamp + 11 minutes) {
                require(amount <= tokensVesting[from] * (block.timestamp - launchtimestamp - 1 minutes) / 10 minutes, 
                    "only 10% of vested tokens are unlocked every week");
            }
        }

        if (_isTeamVesting[from]) {
            require (block.timestamp > launchtimestamp + 12 minutes, "team can't sell yet");
            if (block.timestamp > launchtimestamp + 12 minutes && block.timestamp < launchtimestamp + 17 minutes) {
                require(amount <= teamTokensVesting[from] * (block.timestamp - launchtimestamp - 1 minutes) / 5 minutes, 
                    "only 20% of vested tokens are unlocked every week");
            }
        }
    ...
    function airdropAndTeamVest(
        ...
        for (uint256 i = 0; i < airdropWallets.length; i++) {
            ...
            uint256 airdropAmount = amount[i] * (10**18);
            super._transfer(msg.sender, wallet, airdropAmount);
            teamTokensVesting[wallet] = balanceOf(wallet);
            _isTeamVesting[wallet] = true;
        }
    }
}