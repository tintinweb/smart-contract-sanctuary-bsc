...
// Space.
contract Space is ERC20('Space Tour', 'SPACE') {
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of Space
    ...
    event SpaceMintError(bytes reason);
    ...
    function pendingSpace(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            accSpacePerShare = accSpacePerShare.add(SpaceReward.mul(1e18).div(pool.lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        try token.mint(devaddr, SpaceReward.div(10)) {
        } catch (bytes memory reason) {
            SpaceReward = 0;
            emit SpaceMintError(reason);
        }
        
        try token.mint(address(this), SpaceReward) {
        } catch (bytes memory reason) {
            SpaceReward = 0;
            emit SpaceMintError(reason);
        }
        
        pool.accSpacePerShare = pool.accSpacePerShare.add(SpaceReward.mul(1e18).div(pool.lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accSpacePerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
    ...
    // Safe Space transfer function, just in case if rounding error causes pool to not have enough ALPHAs.
    function safeSpaceTransfer(address _to, uint256 _amount) internal {
        uint256 SpaceBal = token.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > SpaceBal) {
            transferSuccess = token.transfer(_to, SpaceBal);
        } else {
    ...
    function updateEmissionRate(uint256 _SpacePerBlock) external onlyOwner {
        require(_SpacePerBlock <= MAX_EMISSION_RATE, "INCORRECT INPUT");
        massUpdatePools();
        SpacePerBlock = _SpacePerBlock;
