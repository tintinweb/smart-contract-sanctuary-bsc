...
interface IVPoolConfiguration {
    ...
    function isHordChampion(address champion) external view returns (bool);
...
// File contracts/interfaces/ITokenMetadata.sol
 ...
 * Github: s2imonovic
 */
contract VPoolManager is HordUpgradable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    IVPoolConfiguration private vPoolConfiguration;
    // Instance of uniswap
    IUniswapV2Router02 public uniswapRouter;
    // Instance of Hord treasury contract
    IHordTreasury private hordTreasury;
    // Instance of HPool Factory contract
    IHPoolFactory private hPoolFactory;
    ...
    external
    {
        require(vPoolConfiguration.isHordChampion(msg.sender), "Wrong champion address.");
    ...
    function subscribeForVPool(
        ...
        uint256 poolId,
        uint256 participationAmountUsd
    )
    external
    isPoolIdValid(poolId)
    whenNotPaused
    nonReentrant
    {
        ...
        uint256 feeAmountInUsd = participationAmountUsd
        ...
        s.subscriptionAmount = s.subscriptionAmount.add(participationAmountUsd);
        s.totalFeeAmount = s.totalFeeAmount.add(feeAmountInUsd);
        s.user = msg.sender;

        require(
            participationAmountUsd >= vPoolConfiguration.minUserParticipation() &&
            s.subscriptionAmount <= vPoolConfiguration.maxUserParticipation(),
            "Wrong amount."
        );

        IERC20(baseToken).safeTransferFrom(msg.sender, address(this), participationAmountUsd.add(feeAmountInUsd));

        vp.followersUSDDeposit = vp.followersUSDDeposit.add(participationAmountUsd);
        vp.treasuryFeePaid = vp.treasuryFeePaid.add(feeAmountInUsd);
    ...
    isPoolIdValid(poolId)
    {
        ...
        poolContract.mintVPoolToken(name, symbol, vp.followersUSDDeposit, ITokenMetadata(baseToken).decimals(), address(vPoolConfiguration), address(matchingMarket));
        ...
        payServiceFeeToTreasury(poolId, vp.treasuryFeePaid);

        IERC20(baseToken).safeTransfer(address(poolContract), vp.followersUSDDeposit);

        poolContract.depositBudget(vp.followersUSDDeposit);
    ...
    nonReentrant
    {
        ...
        vp.followersUSDDeposit = vp.followersUSDDeposit.sub(s.subscriptionAmount);
        vp.treasuryFeePaid = vp.treasuryFeePaid.sub(s.totalFeeAmount);

        uint256 amountToWithdraw = s.subscriptionAmount.add(s.totalFeeAmount);

        // Mark that user withdrawn his subscription.
        s.isSubscriptionWithdrawnPoolTerminated = true;
        s.subscriptionAmount = 0;
        s.totalFeeAmount = 0;
    ...
    returns (uint256, uint256, bool)
    {
        ...
        return (subscription.subscriptionAmount, subscription.totalFeeAmount, subscription.isSubscriptionWithdrawnPoolTerminated);
    }
