...
contract Templar is Owner {
    ...
    event MinerEnable(address indexed miner, bool enable, uint256 fee);
    ...
    function setWhites(
        ...
        for (uint256 i = 0; i < userAddress.length; i++) {
            accountInfo[userAddress[i]].isWhite = enable;
            accountInfo[userAddress[i]].fee = fee;
            emit MinerEnable(userAddress[i], enable, fee);
        }
    }

    function getAllWhites() public view returns (address[] memory miners) {
        uint256 amount;
        for (uint256 i = 0; i < allMiners.length; i++) {
            if (accountInfo[allMiners[i]].isWhite) {
                miners[0] = allMiners[i];
                amount++;
            }
        }
        return miners;
    ...
    function multMint(
        ...
        uint256 fee;
        if (accountInfo[msg.sender].isWhite) {
            fee = nftAmount * accountInfo[msg.sender].fee;
        } else {
            fee = nftAmount * supplyInfo[supplyId].fee;
        }
        ...
        for (uint256 index = 0; index < nftAmount; index++) {
            ...
            _mint(tokenId, supplyId, index, tokenUrls);
        }
    }
    ...
    function _mint(
        ...
        for (uint256 i = 0; i < supplyInfo[supplyId].levelCode.length; i++) {
            if (r < supplyInfo[supplyId].levelOffset[i]) {
                ...
                if (
                    supplyInfo[supplyId].haveSupply[i] ==
                    supplyInfo[supplyId].maxSupply[i]
                ) {
                    _mint(tokenId, supplyId, index + 1, tokenUrls);
                }

                supplyInfo[supplyId].haveSupply[i] += 1;
                break;
            }
        }
        if (tokenInfo[tokenId].levelCode == 700) {
            SuperLuckyAmount++;
        }
        ...
        emit Mint(
            msg.sender,
            supplyInfo[supplyId].fee,
            tokenId,
            tokenInfo[tokenId].levelCode
        );
    }
    ...
    function setSupplyInfo(
        ...
        supplyInfo[id].haveSupply = new uint256[](levelCode.length);
    ...
    function getReward(address userAddress) public view returns (uint256) {
        ...
        uint256[] memory available = myInventory(userAddress, 700);
        ...
        if (amount == 0) {
            ...
            return
                (AllRewardPool * amount) /
                (SuperLuckyAmount - HasSuperLuckyAmount);
        }
    }
    ...
    function claim() public {
        ...
        uint256[] memory available = myInventory(msg.sender, 700);
        uint256 amount = 0;
        for (uint256 i = 0; i < available.length; i++) {
            if (available[i] > 0) {
                amount++;
        ...
        if (address(this).balance >= myReward) {
            ...
            (suc, ) = msg.sender.call{value: address(this).balance}(
                new bytes(0)
            );
        }
