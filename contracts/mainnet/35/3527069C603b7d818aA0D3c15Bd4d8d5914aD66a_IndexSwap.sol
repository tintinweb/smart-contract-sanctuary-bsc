...
// File: venus/ComptrollerInterface.sol
...
// File: interfaces/IAccessController.sol
...
// File: interfaces/IIndexSwap.sol
...
// File: interfaces/IIndexSwapLibrary.sol
...
contract IndexSwap is TokenBase, Initializable {
    // IERC20 public token;
    ...
    event WithdrawFromFundInBNB(
        uint256 time,
        address user,
        uint256 tokenAmount,
        uint256 bnbAmount
    );
    event WithdrawFromFundInToken(
        uint256 time,
        address user,
        uint256[] tokenAmount
    );
    ...
    function init(
        ...
        require(
            _outAsset != address(0) &&
                _vault != address(0) &&
                _maxInvestmentAmount > 0 &&
                _minInvestmentAmount > 0 &&
                _indexSwapLibrary != address(0) &&
                _adapter != address(0) &&
                _accessController != address(0) &&
                _tokenMetadata != address(0) &&
                _feePointBasis > 0 &&
                _treasury != address(0) &&
                _minInvestmentAmount < _maxInvestmentAmount
        );
    ...
    function investInFund(uint256 _slippage) public payable nonReentrant {
        ...
        require(investedAmountAfterSlippageBNB > 0);
    ...
    function withdrawFund(
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            ...
            require(tokenBalance.mul(tokenAmount) >= totalSupplyIndex);
            ...
            if (!isMultiAsset) {
                if (_tokens[i] == adapter.getETH()) {
                    ...
                    uint256 sw = adapter._swapTokenToETH(
                        _tokens[i],
                        amount,
                        msg.sender,
                        _slippage
        ...
        (, uint256 vaultBalance) = indexSwapLibrary.getTokenAndVaultBalance(
            IIndexSwap(address(this))
        );
        uint256 tokenAmountBNB = 0;
        if (balanceOf(msg.sender) > 0 && vaultBalance > 0) {
            tokenAmountBNB = indexSwapLibrary._getTokenPriceUSDETH(
                vaultBalance.mul(balanceOf(msg.sender)).div(totalSupply())
            );
        }

        require(
            tokenAmountBNB == 0 || tokenAmountBNB >= MIN_INVESTMENTAMOUNT,
