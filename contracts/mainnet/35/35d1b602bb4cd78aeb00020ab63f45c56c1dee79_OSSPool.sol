...
contract OSSPool is USDTWrapper, Ownable {
    ...
    event Destroy(address indexed _buyAddr, address indexed _sellAddr, uint256 amount);
    ...
    function setPlatFormToken(address platFormToken_) public onlyOwner returns (bool){
        require(platFormToken_ != address(this), "Can't let you take all native token");
        platFormToken = IERC20(platFormToken_);
        return true;
    }

    function setUsdtToken(address usdtToken_) public onlyOwner returns (bool){
        require(usdtToken_ != address(this), "Can't let you take all native token");
        usdtToken = IERC20(usdtToken_);
        return true;
    }
    ...
    function getReward(IERC20 quoteToken, address _user, uint256 reward) public {
        ...
        if (reward > 0) {
            if (address(quoteToken) != address(0)) {
                ...
                quoteToken.safeTransfer(_user, reward);
            } else {
    ...
    function destroy(address _buyUser, address _sellUser, uint256 amount) public {
        require(WhiteList[msg.sender], "invalid address");
        if (amount > 0) {
            require(amount <= platFormToken.balanceOf(address(this)), "NOT_ENOUGH_TOKEN");
            platFormToken.safeTransfer(address(0x0000000000000000000000000000000000000001), amount);
            emit Destroy(_buyUser, _sellUser, amount);
        }
    }

    function getDestroyTotal() public view returns (uint256){
        return platFormToken.balanceOf(address(0x0000000000000000000000000000000000000001));
    }
}