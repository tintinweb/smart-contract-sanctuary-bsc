...
contract Port3Avatar is ERC721Enumerable, Ownable {
  ...
  event NftPerAddressLimitChanged(uint256 newNftPerAddressLimit);
  event CostChanged(uint256 newCost);
  event MaxMintAmountChanged(uint256 newMaxMintAmount);
  event BaseURIChanged(string newBaseURI);

  event NftPause(bool newState);

  // === EIP 712 ===
  // TODO regenerate type hash
  string public constant domainName = "Port3Avatar";
  ...
  ) ERC721("Port3 Avatar", "PORT3AVATAR") {
  ...
  modifier onlySigner(address to, bytes calldata signature) {
      ...
      require(signer == mintSigner, "Port3Avatar: invalid signer");
      _;
  }
  ...
  // public
  function mint(uint256 _mintAmount, address _to, bytes calldata _signature) 
    public payable onlySigner(_to, _signature)
  {
    ...
    require(balance + _mintAmount <= nftPerAddressLimit, "max NFT limit per address exceeded");
  ...
  //only owner
  function setNftSigner(address _signer) public onlyOwner {
    mintSigner = _signer;
  ...
  function setNftPerAddressLimit(uint256 _limit) public onlyOwner {
    ...
    emit NftPerAddressLimitChanged(_limit);
  }
