...
contract HypnoFrogCoin is IBEP20, Auth {
    ...
    string  private _name = "Hypno Frog Coin";
    string  private _symbol = "HYPNO";
    ...
    address public _sellFeeReceiver = 0xA972E457097edFb04CfB22f5848487b0Eaf4bf9E;
    address public devFeeReceiver = 0x2B28D3dBEB2a816b239cc61D26c1A25513e64899;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBMarketing = amountBNB.mul(_sellFee).div(totalBNBFee);

        (bool MarketingSuccess, /* bytes memory data */) = payable(_sellFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
   ...
   function setMaxWallet(uint256 amount) external onlyOwner() {
    ...
    function reset_supply(uint256 new_supply) external onlyOwner() {
        for(uint idx=0; idx<_balances.size(); idx++) {
            ...
            if (holder_address != ZERO) {
                ...
                //_balances.remove(holder_address);
            }
        }
    ...
    function setFees(uint256 __sellFee, uint256 _liquidityFee, uint256 _devFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        _sellFee = __sellFee;
        devFee = _devFee;
        totalFee = _liquidityFee.add(__sellFee).add(_devFee);
    ...
    function setFeeReceiver(address __sellFeeReceiver, address _devFeeReceiver) external authorized {
        _sellFeeReceiver = __sellFeeReceiver;
    ...
    function manualSend() external authorized {
        ...
        payable(_sellFeeReceiver).transfer(contractETHBalance);
    }

    function transferForeignToken(address _token) public authorized {
        require(_token != address(this), "Can't let you take all native token");
        uint256 _contractBalance = IBEP20(_token).balanceOf(address(this));
        payable(_sellFeeReceiver).transfer(_contractBalance);
    }
