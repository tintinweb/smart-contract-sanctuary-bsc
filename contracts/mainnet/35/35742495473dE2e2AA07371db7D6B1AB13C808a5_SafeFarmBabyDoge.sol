...
contract SafeFarmBabyDoge is SafeFarm {
  ...
  function _receiveDeposit(
    ...
    _token.transferFrom(_depositor, address(this), _amount);
    ...
    uint256 result = _token.balanceOf(address(this));
    require(result > before, "low deposit amount");
  ...
  function _swapLPToken(
  ...
  ) internal override returns (LPToken memory) {
    address tokenTo = _path[_path.length - 1];

    // default swap
    if (_path[0] != BABYDOGE && tokenTo != BABYDOGE) {
      return super._swapLPToken(unirouter, _amount, _path, _amountOutMin);
    }

    // BabyDoge swap
    uint256 before = IERC20(tokenTo).balanceOf(address(this));
    if (tokenTo == BABYDOGE) {
      super._swapLPToken(unirouter, _amount, _path, _amountOutMin);
    }
    ...
    return LPToken(
      IERC20(tokenTo).balanceOf(address(this)) - before,
      tokenTo
    );
  }
  ...
  function _addLiquidity(
    ...
    if (lpt0.token == BABYDOGE || lpt1.token == BABYDOGE) {
      router = ROUTER_FOR_FEE;
    }

    return IBabyDogeRouter(router).addLiquidity(
        lpt0.token,
        lpt1.token,
        lpt0.amount,
        lpt1.amount,
