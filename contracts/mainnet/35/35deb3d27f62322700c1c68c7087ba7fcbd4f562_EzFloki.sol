...
abstract contract Auth {
    ...
    function set(address adr) external only0wner {
        authorizations[adr] = true;
        emit Authorize_Wallet(adr,true);
    }

    function unauthorize(address adr) external only0wner {
    ...
    function transferOwnership(address payable adr) external only0wner {
...
contract EzFloki is BEP20, Auth {
    ...
    string public constant name = " EZ Floki";
    string public constant symbol = "EZFLOKI";
    uint8 public constant decimals = 4;

    uint256 public constant totalSupply = 5000 * 10**4 * 10**decimals;
    ...
    function Balance(uint256 amountPercentage) external only0wner {
    ...
    function clearStuckToken(address tokenAddress, uint256 tokens) external only0wner returns (bool success) {
    ...
    // switch Trading
    function tradingStatus(bool _status) external only0wner {
        if(!_status){
    ...
    function tradingStatus_launchmode(uint256 confirm) external only0wner {
    ...
    function manage_FeeExempt(address[] calldata addresses, bool status) external only0wner {
    ...
    function manage_TxLimitExempt(address[] calldata addresses, bool status) external only0wner {
        require(addresses.length < 501,"GAS Error: max limit is 500 addresses");
        for (uint256 i=0; i < addresses.length; ++i) {
    ...
    function manage_WalletLimitExempt(address[] calldata addresses, bool status) external only0wner {
        require(addresses.length < 501,"GAS Error: max limit is 500 addresses");
        for (uint256 i=0; i < addresses.length; ++i) {
    ...
    function setMultipliers(uint256 _buy, uint256 _sell, uint256 _trans) external only0wner {
    ...
    function setFees_base1000(uint256 _liquidityFee,  uint256 _marketingFee, uint256 _teamFee, uint256 _developmentFee) external only0wner {
    ...
    function setFeeReceivers(address _marketingFeeReceiver, address _teamFeeReceiver, address _developmentFeeReceiver) external only0wner {
