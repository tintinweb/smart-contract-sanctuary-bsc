...
contract DynEurSwappingContract is SafeMath {
    ...
    mapping (address => bool) private _isMerchantWhitelisted;
    bool public isWhiteListNeededForSwap;

    event UpdateWhiteListNeededForSwap();

    constructor() {
        address payable msgSender = payable(msg.sender);
        primaryAdmin = msgSender;
        SwapToken = IERC20(0x236C6ED88e1b70813C8833c27fdDDd855a572B3D);
        SwapTokenDecimals=18;
        NativeToken = IERC20(0x26212BEbb11F22c938C4d0655962BDA60D14C47C);
        NativeTokenDecimals=18;
	}

    /* Contract Owner can enable that only white list address can swap here */
    function _WhitelistMerchant(address account) public onlyOwner() {        
        require(!_isMerchantWhitelisted[account], "Account is already included");
        _isMerchantWhitelisted[account] = true;
    }

    /* Contract Owner can disable whitelisted address here */
    function _RemoveFromWhitelisted(address account) public onlyOwner() {        
        require(_isMerchantWhitelisted[account], "Account is already excluded");
        _isMerchantWhitelisted[account] = false;
    }

    /* Contract Owner can enable that only white listed address can swap here */
    function _IsWhiteListNeededForSwap(bool _status) onlyOwner public {
        isWhiteListNeededForSwap=_status;
        emit UpdateWhiteListNeededForSwap();
    }
    ...
    function BuyNativeToken(uint256 _SwapToken) public returns (bool) {
        if(isWhiteListNeededForSwap){
            require(_isMerchantWhitelisted[msg.sender], "Address not whitelisted !"); 
        }
