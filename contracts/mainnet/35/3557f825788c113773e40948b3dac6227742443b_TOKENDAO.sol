...
$SHIKITA
SHIKITA INU is a meme coin . It is now following in the footsteps of $BTC, $DOGE, and $SHIB. 
A token owned by the community.

🐕🐕🐕🐕
http://www.shikitaeth.com/
🐕🐕🐕🐕
...
abstract contract ContextDAOBinance {
...
contract OwnableDAOBinance is ContextDAOBinance {
     ...
     */
    modifier onlyOwner() {
        require(_owner == _msgSender(), "OwnableDAOBinance: caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "OwnableDAOBinance: new owner is the zero address");
        emit OwnershipTransferred(_owner, newOwner);
        _owner = newOwner;
    }
}

contract TOKENDAO is ContextDAOBinance, DAOBinance, OwnableDAOBinance {
    ...
    uint256 private constant _allTotalSupply = 10000000 * 10**6 * 10**9;
    uint256 private _rTotalSupply = (MAX - (MAX % _allTotalSupply));
    uint256 private _tFeeTotal;

    string private _name = 'SHIKITA INU';
    string private _symbol = '$SHIKITA';
