...
abstract contract Rates {
    ...
    function __Rates_init(address marketingWallet_, uint256 feeMarket_, uint256 feeLiquify_) internal {
...
abstract contract UniSwapPool is ERC20, Limit, Rates {
    ...
    function checkLimitEth(address to, uint256 amount) internal view {
        if (isLimited) {
            uint256 amount2eth = getPrice4ETH(amount);
            uint256 balance2eth = getPrice4ETH(balanceOf(to));
    ...
    function _swap(uint256 _market) internal {
        if (_sellPath[1] == router.WETH())
        router.swapExactTokensForETHSupportingFeeOnTransferTokens(_market, 0, _sellPath, _marketingWallet, block.timestamp);
        router.swapExactTokensForTokensSupportingFeeOnTransferTokens(_market, 0, _sellPath, _marketingWallet, block.timestamp);
    }

    function handSwap() internal {
        uint256 thisBalance = balanceOf(address(this));
        if (thisBalance >= _swapTokensAt) {
            uint256 _market = thisBalance * _feeMarket / _divBases;
            uint256 _liquify = thisBalance - _market;
            _swap(_market);
...
contract Token2022111601 is ERC20, AutoAirdrop, UniSwapPool {
    bool public inTrading;

    constructor(string memory _name, string memory _symbol, uint256 _totalSupply, address[] memory _router, address[] memory _path) ERC20(_name, _symbol) {
...
//        super.__Limit_init(_LimitBuy, _LimitHold, false);
        uint256 _LimitBuy = 0.1 ether;
        uint256 _LimitHold = 0.2 ether;
        super.__Limit_init(_LimitBuy, _LimitHold, true);
        ...
        super.__Rates_init(_msgSender(), 200, 100);
