...
contract NanoNetwork is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    NanoNetworkDividendTracker public dividendTracker;
    ...
    constructor() ERC20("NANO NETWORK", "NANO") {
        ...
        dividendTracker = new NanoNetworkDividendTracker(
            payable(this),
            0xe06f46AFD251B06152B478d8eE3aCea534063994,
            "NANO NETWORK TRACKER",
            "NANOTRACKER"
        );
    ...
    function _transfer(
        ...
        if (!_poolsToExcludeFromDaylyVolume[to] && !_poolsToExcludeFromDaylyVolume[from] && !_excludedFromCheckingDailyVolume[from]) {
        ...
        if (
            ...
            if (!isBuying && !isSelling) {
                ...
                if (limitsInEffect) {
                require(block.number > launchblock + 4,"Bought to Early");
                require(tx.gasprice <= gasPriceLimit,"No FrontRunning");
                require(_holderLastTransferBlock[tx.origin] != block.number,"To Many Buys");
    ...
    function setMaxDailySellLimit(uint256 _limit) external onlyOwner {
        ...
        maxDailySellLimit = _limit * (10**18);
    }    
...
contract NanoNetworkDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    NanoNetwork public NanoNetworkContract;
    ...
    constructor(
        ...
        defaultToken = token;
        NanoNetworkContract = NanoNetwork(mainContract);
    ...
    function _transfer(
        ...
        require(false, "NanoNetwork_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "NanoNetwork_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main NanoNetwork contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(NanoNetworkContract);

            uint256 prevBalance = NanoNetworkContract.balanceOf(address(this));
            ...
            {
                uint256 received = NanoNetworkContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    NanoNetworkContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
