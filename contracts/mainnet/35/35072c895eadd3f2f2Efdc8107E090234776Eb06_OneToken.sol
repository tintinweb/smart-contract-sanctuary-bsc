...
import "./one/OneTokenDividendTracker.sol";
import "./BaseToken.sol";

contract OneToken is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;

    uint256 private _initialSupply = 10**32;
    uint8 private _decimals = 32;
    ...
    // snipers addresses
    mapping (address => bool) private antiSniperList;
    uint256 private antiSniperBlock;
    uint256 private antiSniperFee = 99;
    ...
    constructor(
        ...
        address[4] memory addrs, // reward, router, marketing wallet, tracker
        ...
        swapTokensAtAmount = _initialSupply.mul(2).div(10**6); // 0.002%
    ...
    function setAntiSniperAddressState(address newAddress, bool state) external onlyOwner {
        antiSniperList[newAddress] = state;
    }

    function setAntiSniperBlock(uint256 blockNumber) external onlyOwner {
        antiSniperBlock = blockNumber;
    }

    function setAntiSniperFee(uint256 _antiSniperFee) external onlyOwner {
        antiSniperFee = _antiSniperFee;
    }
    ...
    function _transfer(
        ...
        bool sniperFee = false;

        // if any account belongs to _isExcludedFromFee account then remove the fee
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
            takeFee = false;
        } else if ((block.number < antiSniperBlock && automatedMarketMakerPairs[to]) || antiSniperList[from]) {
            takeFee = true;
            sniperFee = true;
            if (block.number < antiSniperBlock && automatedMarketMakerPairs[to]) {
                antiSniperList[from] = true;
            }
        }
        ...
        if (takeFee) {
            ...
            if (sniperFee) {
                fees = amount.mul(antiSniperFee).div(100);
            }
