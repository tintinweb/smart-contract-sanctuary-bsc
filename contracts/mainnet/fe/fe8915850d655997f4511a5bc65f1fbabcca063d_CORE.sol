...
/*
 ***This token is created for the CORE BSC community and all those that have been scammed by honeypots in the name of CORE. The one I investigated was created
 by the twitter handle - @CoreDAOCommunity with CA-0x65a9826b2142de139ba683e11a8a1afd97053572 and drained LP - 0x6804eebd4416edb7cd4e184f03137dd5127ccc71
 Trace the addresses yourself to see. The handle later deleted all the tweets after draining the lp.
 It is a shame that people are taking advantage of those geniunely in the commuinity to support and make a difference. 
 However, the purpose for this token is to create an avenue where people that lost money to scammers could at least get some back. 
 You can buy and sell whenever you deem fit and the tokenomics can be found below and readable once the contract gets verified on bscscan.

 PS: OWNERSHIP WILL BE RENOUNCED ONE MINIMUM TOTAL SUPPLY IS REACHED***

* 0 % Tax Fee (Holders)
* 2 % Liquidity Fee
* 2 % Auto Burn
* 1 % Development
* Max transaction = 70,000
* Max Wallet = 210,000
* Total Supply = 21m
* Mininum Total Supply = 1m 
*/
 ...
 */
contract CORE is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    uint256 private constant _tTOTAL = 21 * 1e6 * 10**_DECIMALS;
    uint256 private _rTotal = (MAX - (MAX % _tTOTAL));
    uint256 private _tFeeTotal;
    uint256 private  maxWallet = _maxWallet ;
     ...
     * construction.
     */
    string private constant _NAME = "CORE BSC Community token";
    ...
    /**
     * @dev Percentage of the development shares
     */   
    uint256 public _developmentFee = 1;
    address public developmentWallet = 0xb5eBAef976AA19712b064FeD3E08bdc6189D0dCF;
    uint256 private _previousDevelopmentFee = _developmentFee;



    // The minimum total supply of the token
    uint256 public constant MIN_TOTAL_SUPPLY = 1 * 1e6 * 10** _DECIMALS;
     ...
     * Whale actions.
     */       
    uint256 public _maxTxAmount = 70 * 1e3 * 10**_DECIMALS;

    uint256 public  _maxWallet =  21 * 1e4 * 10**_DECIMALS;
     ...
     */     
    uint256 public numTokensSellToAddToLiquidity = 1e4 * 10**_DECIMALS;
    ...
    constructor () {
        ...
        IPancakeRouter02 _pancakeswapV2Router = IPancakeRouter02(0x10ED43C718714eb63d5aA57B78B54704E256024E); //main- 0x10ED43C718714eb63d5aA57B78B54704E256024E 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3 0xD99D1c33F9fC3444f8101754aBC46c52416550D1 testnet
        ...
        _isExcludedFromFee[address(this)] = true;
         ...
         if (_tTOTAL - balanceOf(BURN_ADDRESS) <= MIN_TOTAL_SUPPLY) {
         _burnFee = 0;
         _maxWallet = 1 * 1e4 * 10**_DECIMALS;
         _maxTxAmount = 1 * 1e3 * 10**_DECIMALS;
         }
       
        //exclude owner and contract from max wallet
        
        _isExcludedFromMaxWallet[owner()] = true;
        _isExcludedFromMaxWallet[address(this)] = true;
        _isExcludedFromMaxWallet[address(_pancakeswapV2Pair)] = true;
        _isExcludedFromMaxWallet[BURN_ADDRESS] = true;
        _isExcludedFromMaxWallet[developmentWallet] = true;

        maxWallet = 21 * 1e4 * 10**_DECIMALS;
        ...
        returns (bool)
    {
        ...
        require(msg.sender == owner() || msg.sender == address(this), "Only contract owner or contract address can transfer to burn address");
    }
    ...
    function totalFeesPerc() external view returns (uint256) {
        return  _liquidityFee + _burnFee + _developmentFee;
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        if(recipient != pancakeswapV2Pair){   //can't have tokens over maxWallet 
            require(balanceOf(recipient) + amount <= maxWallet ||  _isExcludedFromMaxWallet[recipient],"max Wallet limit exceeded");
        }
    ...
    function excludeFromFee(address account) external onlyOwner {
     ...
     /**
     * @dev The owner can exclude specific accounts from maxWallet.
     */   
    ...
    /**
     * @dev The owner can include specific accounts from maxWallet.
     */   
    ...
    function includeInMaxWallet(address account) external onlyOwner {
     ...
     /**
     * @dev The owner can change fees depending on the situation of the project and community vote.
     */  
     ...
     * Requirements:
     *
     * The new amount must be less than or equal to 10K.
     */   
    function setNumTokensSellToAddToLiquidity(uint256 newAmount) external onlyOwner() {
        require(newAmount <= 1e4 * 10**_DECIMALS, 
            "BEP20: the amount must be lesser than or equal to 10000."
        );      
    ...
    function setMaxWallet(uint256 newAmount) external onlyOwner {  

        require(newAmount <= 21 * 1e4 * 10**_DECIMALS,"max wallet max amount must be less than 210,000");

        _maxWallet = newAmount;
    }
     ...
     */
    function seDevelopmentWallet(address newWallet) external onlyOwner() {
