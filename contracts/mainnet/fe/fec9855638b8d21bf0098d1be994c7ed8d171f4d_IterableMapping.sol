...
contract TKRDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor() DividendPayingToken("TKRDT", "TKRDT") {
        claimWait = 3600;
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "TKRDT: withdrawDividend disabled. Use the 'claim' function on the main contract."
        );
    }
    ...
    {
        require(
            (_newMinimumBalance * (10**18)) != minimumTokenBalanceForDividends,
            "New minimum balance for dividend cannot be same as current minimum balance"
        );
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "TKRDT: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "TKRDT: Cannot update claimWait to same value"
        );
...
contract TOKENRUGGED is ERC20, Ownable {
    ...
    uint256 public autoBurnFee = 2;
    uint256 public BUSDRewardsFee = 2;
    uint256 public totalFees =
        marketingFee.add(liquidityFee).add(autoBurnFee).add(BUSDRewardsFee);
    uint256 public sellIncreaseFactor = 170;
    uint256 public quickSellIncreaseFactor = 340;
    uint256 public quickSellInterval = 48;
    ...
    function manualWithdrawTokens() external {
        require(
            ...
            "TKR: Only marketingWallet/owner can call this function"
        );
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(
            ...
            "TKR: The dividend tracker already has that address"
        );
        TKRDividendTracker newDividendTracker = TKRDividendTracker(
            payable(newAddress)
        );
        require(
            newDividendTracker.owner() == address(this),
            "TKR: The new dividend tracker must be owned by the TKR token contract"
        );
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(
            ...
            "TKR: The router already has that address"
        );
    ...
    function blacklistAddress(address account, bool value) external onlyOwner {
        require(
            value != _isBlacklisted[account],
            "TKR: Cannot update to same value"
        );
    ...
    function excludeFromFees(address account, bool value) public onlyOwner {
        require(
            value != _isExcludedFromFees[account],
            "TKR: Cannot update to same value"
        );
    ...
    function setLastBuy(address account, uint256 value) external onlyOwner {
        require(value != _lastBuy[account], "TKR: Cannot update to same value");
        _lastBuy[account] = value;
    }
    ...
    function setSellIncreaseFactor(uint256 value) external onlyOwner {
        require(
            value >= 100,
            "TKR: sellIncreaseFactor must be greater than or equal to 100"
        );
        require(
            value != sellIncreaseFactor,
            "TKR: Cannot update to same value"
        );
    ...
    function setQuickSellIncreaseFactor(uint256 value) external onlyOwner {
        require(
            value >= 100,
            "TKR: quickSellIncreaseFactor must be greater than or equal to 100"
        );
        require(
            value != quickSellIncreaseFactor,
            "TKR: Cannot update to same value"
        );
    ...
    function setSwapTokensAtAmount(uint256 value) external onlyOwner {
        require(
            (value * (10**18)) != swapTokensAtAmount,
            "TKR: Cannot update to same value"
        );
    ...
    function setMarketingFee(uint256 value) external onlyOwner {
        ...
        marketingFee = value;
        totalFees = marketingFee.add(liquidityFee).add(autoBurnFee).add(
            BUSDRewardsFee
        );
    }
    ...
    function setAutoBurnRewardsFee(uint256 value) external onlyOwner {
        require(value != autoBurnFee, "TKR: Cannot update to same value");
        autoBurnFee = value;
        totalFees = marketingFee.add(liquidityFee).add(autoBurnFee).add(
            BUSDRewardsFee
        );
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "TKR: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            value != _automatedMarketMakerPairs[pair],
            "TKR: Cannot update to same value"
        );
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(
            newValue >= 300000 && newValue <= 900000,
            "TKR: gasForProcessing must be between 300000 and 900000"
        );
        require(
    ...
    function getRemainingSupply() public view returns (uint256) {
        uint256 supplyBalance = totalSupply().sub(balanceOf(deadWallet));
        supplyBalance = supplyBalance.div(10**18);
    ...
    function getLastBuy(address account) public view returns (uint256) {
        return _lastBuy[account];
    }

    function getAutomatedMarketMakerPairs(address account)
        public
        view
        returns (bool)
    {
    ...
    function _transfer(
        ...
        uint256 adjustedSellIncreaseFactor = sellIncreaseFactor;
        if (!excludedAccount) {
            require(tradingIsEnabled, "Trading has not started yet");
            ...
            if (quickSellInterval > 0) {
                if (_automatedMarketMakerPairs[from]) {
                    ...
                    _lastBuy[from] > 0 &&
                    block.timestamp <
                    (_lastBuy[from] + (quickSellInterval * 60 * 60))
                ) {
        ...
        if (
            ...
            if (autoBurnFee > 0) {
                uint256 autoBurnTokens = contractTokenBalance
                    .mul(autoBurnFee)
                    .div(totalFees);
                super._transfer(from, deadWallet, autoBurnTokens);
            }
            if (BUSDRewardsFee > 0) {
                uint256 BUSDRewardsTokens = contractTokenBalance
                    .mul(BUSDRewardsFee)
                    .div(totalFees);
        ...
        if (!swapping && !excludedAccount && totalFees > 0) {
