...
contract PrivateSellToken is VersionedInitializable{
  ...
  uint public blockLockExtra = 19419988;
  ...
  /**
   * @dev Set lock block extra
   * @param totalBlock Total Lock Block
   */
  function setBlockLockExtra(uint totalBlock) public onlyAdmin {
    blockLockExtra = totalBlock;
  } 
  ...
  function getClaimable(address buyerAddress) public view returns (uint256) {
    ...
    if (block.number < blockLockExtra) {
        return 0;
    }
    ...
    uint progressBlock = block.number - userStartUnlockBlock - 2592000;
