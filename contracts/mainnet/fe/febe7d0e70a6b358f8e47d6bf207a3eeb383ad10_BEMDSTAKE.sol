...
contract BEMDSTAKE is Ownable {
    ...
    */ 
    ...
    uint public minimumBalance = 100e18;
    string constant public lockPeriod = '3 Months';  // locked period in text to be shown by the GUI
    string constant public lockMax = '1 year';  // max period in text to be shown by the GUI
    uint256 private contractLockPeriod = 7776000; // 90 days in seconds 
    uint256 private contractMaxPeriod = 31104000; // 360 days in seconds 
    ...
    */
    function getPeriodRewardTokenCount() public view returns(uint256) {
    ...
    */
    function getpoolUnusedRewardAddress() public view returns(address) {
    ...
    */
    function getPoolTotalLostRewardAmount() public view returns(uint256) {
    ...
    */
    function getCalculationFactor() public view returns(uint) {
    ...
    */
    ...
    /*
    * Returns the remaining period for a user to get max reward
    */
    function remainMaxTime(address _user) 
        public
        view
        returns (uint256)
    {
        UserInfo storage user = userInfo[_user];
        uint256 timeElapsed = block.timestamp.sub(user.depositTime);
        uint256 remainingMaxTime = 0;
        if (user.depositTime == 0) {
            remainingMaxTime = 0;
        } else if(timeElapsed < contractMaxPeriod) {
            remainingMaxTime = contractMaxPeriod.sub(timeElapsed);
        }

        return remainingMaxTime;
    }
    ...
    * This is necessary on any change to any user blance i.e. (deposit, withdrawal, emergencyWithdraw)
    ...
    function _MassUpdate() internal {
        ...
        if (_updateTime.sub(pool.lastMassUpdate) >= rewardDuration) {
            for (uint256 i = 1; i <= userCount; i++) {
                reward = calculateReward(userMapping[i], _updateTime);
    ...
    */
        ...
        returns (uint256)
    {
        ...
        if (_calculationTime > pool.lastMassUpdate && totalSupply > 0) {
            duration = _calculationTime.sub(user.rewardTime);
    ...
    /*
    * View function to help mass update calculate pending reward tokens.
    */
    function calculateReward(address _user, uint256 _calculationTime)
        public
        view
        returns (uint256)
    {
    ...
    */
    function deposit(uint256 _amount) public {
        require(_amount >= minimumBalance, "Mininum investment not met.");
        PoolInfo storage pool = poolInfo; 
        UserInfo storage user = userInfo[msg.sender];

        _MassUpdate(); // Update and store the earned reward before a new deposit

        user.rewardEarned = user.rewardEarned.add(calculateReward(msg.sender, 0));
    ...
    */
    ...
    function bonus() public {
        ...
        uint256 remainMax = remainMaxTime(msg.sender);

        require(remainMax <= 0, "bonus: maxtime remains!");
        require(user.bonus > 0, "bonus: already taken!");

        _MassUpdate();
        uint256 _up = 36;
        uint256 _down = 10;
        uint256 _perc = _up.div(_down);
        uint256 _amount = user.amount.mul(_perc);
        user.bonus = 1;
