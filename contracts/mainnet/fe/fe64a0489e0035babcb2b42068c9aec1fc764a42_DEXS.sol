...
contract GlobalsAndUtility is ERC20 {
    ...
    */
    ...
    /* ERC20 constants */
    string public constant name = "DEX STOCK MARKET-PLATFORM";
    string public constant symbol = "DEXS";
    uint8 public constant decimals = 8;

    /* Suns per Satoshi = 10,000 * 1e8 / 1e8 = 1e4 */
    uint256 private constant SUNS_PER_DEXS = 10 ** uint256(decimals); // 1e8
    ...
    uint256 private constant BPB_MAX_DEXS = 7 * 1e6;
    uint256 internal constant BPB_MAX_SUNS = BPB_MAX_DEXS * SUNS_PER_DEXS;
        ...
        external
    {
        ...
        require(g._currentDay > CLAIM_PHASE_START_DAY, "DEXS: Too early");

        if (beforeDay != 0) {
            require(beforeDay <= g._currentDay, "DEXS: beforeDay cannot be in the future");
    ...
    {
        require(beginDay < endDay && endDay <= globals.dailyDataCount, "DEXS: range invalid");
        ...
        view
    {
        ...
        require(stakeIdParam == stRef.stakeId, "DEXS: stakeIdParam not in stake");
...
contract StakeableToken is GlobalsAndUtility {
        ...
        external
    {
        ...
        require(newStakedDays >= MIN_STAKE_DAYS, "DEXS: newStakedDays lower than minimum");
        ...
        external
    {
        ...
        require(stakeLists[stakerAddr].length != 0, "DEXS: Empty stake list");
        require(stakeIndex < stakeLists[stakerAddr].length, "DEXS: stakeIndex invalid");
        ...
        require(g._currentDay >= st._lockedDay + st._stakedDays, "DEXS: Stake not fully served");

        /* Stake must still be locked */
        require(st._unlockedDay == 0, "DEXS: Stake already unlocked");
        ...
        external
    {
        ...
        /* require() is more informative than the default assert() */
        require(stakeListRef.length != 0, "DEXS: Empty stake list");
        require(stakeIndex < stakeListRef.length, "DEXS: stakeIndex invalid");
        ...
        internal
    {
        ...
        require(newStakedDays <= MAX_STAKE_DAYS, "DEXS: newStakedDays higher than maximum");
        ...
        require(newStakeShares != 0, "DEXS: newStakedSuns must be at least minimum shareRate");
        ...
        returns (uint256 bonusSuns)
    {
        /*
            ...
            Bonus percentage scaled 0% to 10% for the first 7M DEXS of stake.
...
contract TransformableToken is StakeableToken {
        ...
        payable
    {
        ...
        require(rawAmount != 0, "DEXS: Amount required");
    ...
    function xfLobbyExit(uint256 enterDay, uint256 count)
        external
    {
        require(enterDay < _currentDay(), "DEXS: Round is not complete");
        ...
        if (count != 0) {
            require(count <= qRef.tailIndex - headIndex, "DEXS: count invalid");
            endIndex = headIndex + count;
        } else {
            endIndex = qRef.tailIndex;
            require(headIndex < endIndex, "DEXS: count invalid");
        }
        ...
        external
    {
        ...
        require(address(this).balance != 0, "DEXS: No value");

        require(LAST_FLUSHED_DAY < _currentDay(), "DEXS: Invalid day");
        ...
        returns (uint256 rawAmount, address referrerAddr)
    {
        ...
        require(entry.rawAmount != 0, "DEXS: Param invalid");
    ...
    {
        /* 1342465753424 = ~ 4900000 * SUNS_PER_DEXS / 365 */
...
contract DEXS is TransformableToken {
