...
contract LegacyStakeMatrix is Ownable {
    ...
    function preProcessUsers(uint fromID, uint toID) public onlyOwner {
        ...
        for (uint id = fromID; id <= toID; id++) {
            ...
            user.id = totalUsers;
            if (_refCount > 0) user.refCount = _refCount;
    ...
    function preProcessOneUser(uint _userID) public onlyOwner {
        require(stage == 0, "preProcessUsers: Contract enabled");
        address _user = PreLSTM.idToAddress(_userID);
        (uint _id,uint _refCount,,address _invitedBy,,) = PreLSTM.User(_user);
        Users storage user = User[_user];
