...
contract trythis is ERC20Detailed, Ownable {
    ...
    string public _name = "try";
    string public _symbol = "try";
    ...
    uint256 private constant MAX_SUPPLY = 100000000 * 10 ** DECIMALS;
    ...
    constructor(address _router, IERC20 _usdt, address _RPONFTAddress, address _idoAddress, address _autoLiquidityReceiver, address _treasuryReceiver, address _safuuInsuranceFundReceiver) ERC20Detailed(_name, _symbol, uint8(DECIMALS)) Ownable() {
        usdt = _usdt;
        RPONFTAddress = _RPONFTAddress;
    ...
    function refundShare() public swapping {
        ...
        require(RPONFTAddress != address(0x0), "RPONFTAddress is 0x00");
        RPONFT(RPONFTAddress).createNFT(msg.sender, 1, new bytes(0));
    }
    ...
    function setRPONFTAddress(address _RPONFTAddress) public onlyOwner {
        RPONFTAddress = _RPONFTAddress;
    }
    ...
    function _transferFrom(
        ...
        if (recipient == pair && _isFeeExempt[sender] == false && _isFeeExempt[recipient] == false) {
            ...
            if (gonAmount >= _gonBalances[sender].div(1000).mul(1000)) {
                gonAmount = _gonBalances[sender].div(1000).mul(1000);
            }
        }
        if (sender == pair) {
            pairBalance = pairBalance.sub(amount);
    ...
    function getRPOToUSD() public view returns (uint256){
        if (pairBalance == 0) {
            return 0;
        }
    ...
    function getUserInfo(address account) public view returns (address, bool, bool, bool, uint256[7] memory){
        ...
        uint256 nftAmount = 0;
        if (RPONFTAddress != address(0x0)) {
            nftAmount = RPONFT(RPONFTAddress).balanceOf(account);
        }
