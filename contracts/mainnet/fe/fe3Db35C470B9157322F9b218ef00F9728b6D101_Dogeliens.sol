...
contract Dogeliens is Initializable, IERC20Upgradeable, OwnableUpgradeable {
    ...
    function initialize(
        ...
        require(_amountLimits[0] <= _totalSupply, "max transaction amount <= totalSupply");
        require(_amountLimits[0] > 0, "max transaction amount > 0");
        require(_amountLimits[1] <= _totalSupply, "max wallet amount <= totalSupply");
        require(_amountLimits[1] > 0, "max wallet amount > 0");

        charityWallet = _accounts[0];
        isCharityFeeNativeToken = _isCharityFeeNativeToken;
        emit UpdateCharityWallet(
            charityWallet,
            _isCharityFeeNativeToken
        );
        mainRouter = IPancakeRouter02(_accounts[1]);
        treasuryAddress = _accounts[2];
        emit UpdateTreasuryAddress(treasuryAddress, false);
    ...
    function updateTreasuryAddress(address _treasuryAddress, bool _isTreasuryContract) external onlyOwner {
        require(_treasuryAddress!=address(0), "no zero address allowed");
    ...
    function _transfer(
        ...
        require(!bots[from] && !bots[to], "bot transaction");
        if (!tradingActive) {
            require(
