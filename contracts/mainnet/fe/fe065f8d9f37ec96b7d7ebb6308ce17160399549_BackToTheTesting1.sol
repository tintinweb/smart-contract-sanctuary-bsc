...
/**
BackToTheTesting1 SmartContract 
*/
...
contract BackToTheTesting1 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BackToTheTesting1DividendTracker public dividendTracker;
    ...
    bool public multiplierOn = true;
    bool public diamondhandsOn = true;
    ...
    constructor() ERC20("BackToTheTesting1", "BTTT1") {
        ...
        dividendTracker = new BackToTheTesting1DividendTracker(payable(this), router, 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56,
                                                   "BackToTheTesting1TRACKER", "BTTT1TRACKER");
    ...
    // turn limits on and off
    function setMultiplierOn(bool value) external onlyOwner {
        multiplierOn = value;
    }

    // turn limits on and off
    function setDiamondHandsOn(bool value) external onlyOwner {
        diamondhandsOn = value;
    }
...
contract BackToTheTesting1DividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    BackToTheTesting1 public BackToTheTesting1Contract;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        BackToTheTesting1Contract = BackToTheTesting1(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main BackToTheTesting1 contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(BackToTheTesting1Contract);

            uint256 prevBalance = BackToTheTesting1Contract.balanceOf(address(this));
            ...
            {
                uint256 received = BackToTheTesting1Contract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    BackToTheTesting1Contract.transfer(account, received);
                } 
                else {
                    success = false;
                }
