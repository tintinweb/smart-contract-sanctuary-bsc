...
contract paySystem is Ownable {
    ...
    //config
    mapping(uint => uint) public typeRate;
    ...
    function setTypeRate(uint _type, uint _rate) external onlyOwner {
        typeRate[_type] = _rate;
    }

    
    function millionMine(uint _orderId,uint _price,uint _type,address _payTokenAddr,address[] memory _path) external payable {
        ...
        if(0 < msg.value){
            ...
            require(_price < amountOutMin, "amountOutMin: _price < BNB value");
            FUNDAmount = amountOutMin * typeRate[_type] / _decimals;
            merchantAmount = amountOutMin - FUNDAmount;
            ...
            if(_payTokenAddr == USDT_ADDRESS){// USDT
                ...
                FUNDAmount = _price * typeRate[_type] / _decimals;
                ...
                FUNDAmount = payamountOutMin * typeRate[_type] / _decimals;
        ...
        //IERC20(USDT_ADDRESS).safeTransferFrom(address(this),FUND_ADDRESS, payamountOutMin);
        //IERC20(USDT_ADDRESS).safeTransfer(FUND_ADDRESS, payamountOutMin);
