...
abstract contract Project {
    ...
    string constant PROJECT_NAME = "qwerty";
    string constant SYMBOL = "qwerty";
 ...
 * MainContract
 */
contract Qwerty is Project, IBEP20, Ownable {
    using SafeMath for uint256;
...
// ***
     ...
     */
        ...
        if(payableFeesInfo.marketing > 0) {
            (bool marketingSendSuccess,) = payable(wallets.marketing).call{value: payableFeesInfo.marketing, gas: 30000}("");
            require(marketingSendSuccess, "Failed to send to MW");
        }

        if(payableFeesInfo.treasury > 0) {
            (bool treasurySendSuccess,) = payable(wallets.treasury).call{value: payableFeesInfo.treasury, gas: 30000}("");
            require(treasurySendSuccess, "Failed to send to Treasury");
        }

        if(payableFeesInfo.coinwars > 0) {
            (bool coinwarsSendSuccess,) = payable(wallets.coinwars).call{value: payableFeesInfo.coinwars, gas: 30000}("");
