...
contract DefiWzToken is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    function _transfer(
        ...
        require(!isSwap || enableSwap(),"swap not open");
        //绑定推荐关系
        if(!isSwap){
            referral.ref(from,to);
            referral.acceptRef(from,to);
        }
        ...
        if( canSwap &&
            ...
            if(AmountLiquidityFee > 0){
                swapAndLiquify(AmountLiquidityFee,liquidityReceiveAddress);
                AmountLiquidityFee = 0;
            }
            if(AmountLpRewardFee > 0){
                swapAndLiquify(AmountLpRewardFee,address(dividendTracker));
                AmountLpRewardFee = 0;
            }
            swapping = false;
        }
        ...
        if(takeFee) {
            address parent = from == uniswapV2Pair?referral.getParent(to):referral.getParent(from);
            if(!isSwap || parent == address(0)){
                parent = rWalletAddress;
            }
            amount =  takeAllFee(from, amount, parent);
        }
        require(0 == maxHold || to == uniswapV2Pair || _isExcludedFromFees[to] || balanceOf(to).add(amount) <= maxHold, "gt max hold");
    ...
    function takeAllFee(address from,uint256 amount,address _parent) private returns(uint256 amountAfter) {
        ...
        //推荐费
        uint256 refFee = amount.mul(referralFee).div(100);
        if(refFee > 0) super._transfer(from, _parent, refFee);
        amountAfter = amountAfter.sub(refFee);
