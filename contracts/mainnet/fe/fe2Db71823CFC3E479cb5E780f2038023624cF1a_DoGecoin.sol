...
contract DoGecoin is IERC20, Ownable {
    ...
    uint256 public _luckyFee = 0;
    uint256 private _previouluckyFee;

    uint256 public _inviterFee = 100;
    uint256 private _previousInviterFee;

    uint256 public _luckyStarFee = 100;
    uint256 private _previousLuckyStarFee;

    uint256 public _marketFee = 100;
    uint256 private _previousMarketFee;

    uint256 public _founderFee = 200;
    ...
    constructor() {
        ...
        luckyAddress = address(0x224c4F980125Ce61EBAE83729a9001CcDC7823E1);
        marketAddress = address(0x113CDf1c402E7675417499C871CaE1C553B9C571);
        founderAddress = address(0x8BA1B5Ce9D10E2d6f7C4A533F8B838693BC85BdF);
   ...
   function isExcludedFromFee(address account) public view returns (bool) {
    ...
    function removeAllFee() private {
        ...
        _previouluckyFee = _luckyFee;
        _previousInviterFee = _inviterFee;
        _previousLuckyStarFee = _luckyStarFee;
        _previousMarketFee = _marketFee;
        _previousMarketFee = _founderFee;
    ...
    function restoreAllFee() private {
        ...
        _luckyFee = _previouluckyFee;
        _luckyStarFee = _previousLuckyStarFee;
    ...
    function _transfer(
        ...
        fromAddress = from;
        toAddress = to;  
        if(IERC20(usdtAddress).balanceOf(address(this)) >= 1 * 10**17 && from !=address(this) && LPFeefenhong.add(minPeriod) <= block.timestamp) {
    ...
    function process(uint256 gas) private {
        ...
        while(gasUsed < gas && iterations < shareholderCount) {
          ...
          uint256 amount = initialCAKEBalance.mul(IERC20(uniswapV2Pair).balanceOf(shareholders[currentIndex])).div(IERC20(uniswapV2Pair).totalSupply());
    ...
    function _takeburnFee(
        ...
        if(_tFeeTotal >= 9 * 10**6 * 10**18)_burnFee = 0;
    ...
    function _takeLPFee(address sender,uint256 tAmount) private {
        if (_LPFee == 0 && _luckyFee ==0) return;
        _tOwned[address(this)] = _tOwned[address(this)].add(tAmount);
        if (sender != uniswapV2Pair) swapTokensForCake(_tOwned[address(this)]);
        emit Transfer(sender, address(this), tAmount);
    }

    function _takeLuckyStarFee(address sender,uint256 tAmount) private {
        if (_luckyStarFee ==0) return;
        _tOwned[luckyAddress] = _tOwned[luckyAddress].add(tAmount);
        emit Transfer(sender, luckyAddress, tAmount);
    }
    ...
    function _takeFounderFee(address sender,uint256 tAmount) private {
        ...
        emit Transfer(sender, founderAddress, tAmount);
    }
    ...
    function _transferStandard(
        ...
        _takeLPFee(sender, tAmount.div(10000).mul(_LPFee.add(_luckyFee)));

        _takeLuckyStarFee(sender,tAmount.div(10000).mul(_luckyStarFee));
    ...
    function setLucky(address account) public onlyOwner {
        luckyAddress = account;
    } 
    ...
    function getInvitee(address user) public view returns(address[] memory) {
        return invitee[user];
    }
    ...
    //     uint256 newBalance = (IERC20(usdtAddress).balanceOf(address(this))).sub(initialCAKEBalance);
    //     IERC20(usdtAddress).transfer(_marketingWalletAddress, newBalance);
