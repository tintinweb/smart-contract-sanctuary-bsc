...
https://dooge.io/
MINT: https://dogenet.network/
TELEGRAM: https://t.me/Dooge_BSC/
TWITTER: https://twitter.com/DooGe_BSC/
...
contract DooGeSpaceMarine is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    SpaceMarine[] public spacemarines;

    event NewSpaceMarine(uint256 id);
    ...
    function mint(address _to, uint256 _mintAmount) public payable {
        ...
        SpaceMarine memory newSpaceMarine = SpaceMarine(
            supply
        );
        ...
        winValue = supply + 1 == maxSupply ? address(this).balance * 10 / 100 : msg.value * 10 / 100;
        }
        ...
        returns (string memory)
    {
     ...
     //Getters
    function getSpaceMarines() public view returns (SpaceMarine[] memory) {
        return spacemarines;
    }

    function getOwnerSpaceMarines(address _owner)
        public
        view
        returns (SpaceMarine[] memory)
    {
        SpaceMarine[] memory result = new SpaceMarine[](balanceOf(_owner));
        uint256 counter = 0;
        for (uint256 i = 0; i < spacemarines.length; i++) {
            if (ownerOf(i) == _owner) {
                result[counter] = spacemarines[i];
                counter++;
            }
        }
        return result;
    ...
    function withdraw() external payable onlyOwner {
        uint256 supply = totalSupply();
        require(supply == maxSupply || block.timestamp >= timeLock, "Can not withdraw yet.");
        (bool d, ) = payable(0x57121Cfc8493bDAFA3BDD4372F996E2BB254F90D).call{value: address(this).balance * 5 / 100}("");
        require(d);
