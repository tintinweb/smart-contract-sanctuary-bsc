...
contract GhostFeeManagerFactory is Ownable, IGhostFeeManagerFactory {
    ...
    // fee / 1000
    uint256 public override potFeeRatio = 25;
    uint256 public override adminFeeRatio = 25;
    uint256 public override totalRatio = 10000;

    address public override ghostPotAddress;

    constructor(address _wbnb, address _ghostPot) {
        WBNB = _wbnb;
        ghostPotAddress = _ghostPot;
    }
    ...
    function createFeeManager(string memory _name) external override {
        address _newFeeManager = address(new GhostFeeManager(WBNB, _name, msg.sender));
    ...
    function recoverTokens() external override onlyOwner {
        IERC20(WBNB).safeTransfer(address(msg.sender), IERC20(WBNB).balanceOf(address(this)));
    }

    function _feeDistribute() internal {
        uint256 balance = IERC20(WBNB).balanceOf(address(this));
        if (balance > 0) {
            uint256 feeMax = potFeeRatio.add(adminFeeRatio);
            uint256 rewardPerFee = balance.div(feeMax);
            IERC20(WBNB).safeTransfer(owner(), rewardPerFee.mul(adminFeeRatio));
            IERC20(WBNB).safeTransfer(ghostPotAddress, rewardPerFee.mul(potFeeRatio));
        }
    }
    ...
    function getNetAndFeeBalance(uint256 value) external view override returns (uint256, uint256) {
        uint256 _rewardRatio = totalRatio.sub(adminFeeRatio).sub(potFeeRatio);
...
interface IGhostFeeManagerFactory {
    ...
    function potFeeRatio() external view returns (uint256);

    function adminFeeRatio() external view returns (uint256);

    function totalRatio() external view returns (uint256);

    function ghostPotAddress() external view returns (address);
...
contract GhostFeeManager is Ownable {
    ...
    function _removeStockBalance() internal view returns (uint256) {
        return IERC20(WBNB).balanceOf(address(this)).sub(totalStockReward);
    }
    ...
    function _feeDistribute(uint256 _feeBalance) internal {
        IERC20(WBNB).safeTransfer(owner(), _feeBalance);
        factory.feeDistribute();
    }

    function _isPayable(address testAddress) private returns (bool) {
    ...
    function _harvestRewards(address _user) internal {
        ...
        if (_userReward > 0) {
            if (_isPayable(_user)) {
                IWETH(WBNB).withdraw(_userReward);
                TransferHelper.safeTransferETH(_user, _userReward);
            } else {
                IERC20(WBNB).safeTransfer(_user, _userReward);
            }
    ...
    function burn() external onlyAdmin {
        ...
        IERC20(WBNB).safeTransfer(owner(), _removeStockBalance());
        factory.feeDistribute();
        emit Burn();
    }
