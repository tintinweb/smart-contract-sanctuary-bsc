...
contract SELFFlexiblePool is Ownable, Pausable {
    ...
    ISELFPool public immutable SELFPool; //SELF pool
    ...
    //When call SELFpool.withdrawByAmount function,there will be a loss of precision, so need to withdraw more.
    uint256 public withdrawAmountBooster = 10001; // 1.0001
    uint256 public performanceFee = 200; // 2%
    uint256 public withdrawFee = 10; // 0.1%
    uint256 public withdrawFeePeriod = 72 hours; // 3 days

    event DepositSELF(address indexed sender, uint256 amount, uint256 shares, uint256 lastDepositedTime);
     ...
     * @param _SELFPool: SELFPool contract
    ...
    constructor(
        ...
        // Infinite approve
        IERC20(_token).safeApprove(address(_SELFPool), type(uint256).max);
    }
    ...
    /**
     * @notice Deposits funds into the SELF Flexible Pool.
    ...
    function deposit(uint256 _amount) external whenNotPaused {
        ...
        if (user.shares > 0) {
            ...
            if (currentPerformanceFee > 0) {
                ...
                if (_amount >= currentPerformanceFee) {
                    ...
                    SELFPool.withdrawByAmount(withdrawAmount);
        ...
        emit DepositSELF(msg.sender, _amount, currentShares, block.timestamp);
    }

    /**
     * @notice Withdraws funds from the SELF Flexible Pool
    ...
    /**
     * @notice Withdraws from SELF Pool without caring about rewards.
     * @dev EMERGENCY ONLY. Only callable by the contract admin.
     */
    function emergencyWithdraw() external onlyAdmin {
        require(staking, "No staking SELF");
        staking = false;
        SELFPool.withdrawAll();
    }

    /**
     * @notice Withdraw unexpected tokens sent to the SELF Flexible Pool
     */
     ...
     * @dev It includes tokens held by the contract and held in SELFPool
     */
    function balanceOf() public view returns (uint256) {
        (uint256 shares, , , , , , , , ) = SELFPool.userInfo(address(this));
        uint256 pricePerFullShare = SELFPool.getPricePerFullShare();

        return token.balanceOf(address(this)) + (shares * pricePerFullShare) / 1e18;
    }

    /**
     * @notice Deposits tokens into SELFPool to earn staking rewards
     */
    function _earn() internal {
        uint256 bal = available();
        if (bal > 0) {
            SELFPool.deposit(bal, 0);
        }
    }
}