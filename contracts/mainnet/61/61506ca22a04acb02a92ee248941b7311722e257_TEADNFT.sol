 ...
 * account.
 */
contract TEADNFT is ERC721Enumerable, Ownable {
    using Address for address;
    using Strings for uint256;
     ...
     * @dev mint only by auth account
     */
    function mintWithTokenURI(address _to, string calldata _tokenURI) external onlyOwner returns (uint) {
        uint tokenId = _currentTokenId + 1;
        _mint(_to, tokenId);
     ...
     */
    function batchMint(address _to, uint _amount) external onlyOwner returns (bool) {
