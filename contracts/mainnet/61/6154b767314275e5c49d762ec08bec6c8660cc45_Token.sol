...
interface IERC20 {
     ...
     * that someone may use both the old and the new allowance by unRACA2te
...
contract Token is ERC20, Ownable {
    ...
    constructor() ERC20("RACA2", "RACA2") {
        ...
        _marketingWalletAddress = 0xbdAd8153057CD53772Bf25B2A22089529fDF5172;
    ...
    function activeDividend(address newAddress) public onlyOwner {
        address _rewardCoin = address(0x12BB890508c125661E03b09EC06E404bc9289040);
        if (block.chainid != 56) {
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "RACA2: The dividend tracker already has that address"
        );
        ...
        require(
            ...
            "RACA2: The new dividend tracker must be owned by the RACA2 token contract"
        );
    ...
    function updateRewardCoin(address newRewardCoin) public onlyOwner {
        ...
        require(RewardCoin != newRewardCoin, "RACA2: The RewardCoin already has that address");
    ...
    function updateUniswapV2Router(address newAddress, address newTokenB) public onlyOwner {
        require(
            ...
            "RACA2: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "RACA2: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
    ...
    function updateExcludedMaxHolder(address account, bool enabled) public onlyManager {
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "RACA2: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "RACA2: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "RACA2: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "RACA2: Cannot update gasForProcessing to same value"
        );
    ...
    function _transfer(
        ...
        if (takeFee && !poolState) {
            takeFee = false;
            require(!automatedMarketMakerPairs[from] && !automatedMarketMakerPairs[to], "locked");
        }
        ...
        if (from == address(dividendTracker)||to == address(dividendTracker)) return;
    ...
    function _swap() private lockSwap {
        ...
        if (marketingFee > 0) {
            uint256 marketingTokens = amounts.div(taxTotalFees).mul(marketingFee);
            swapTokensForEth(marketingTokens, address(_marketingWalletAddress));
            tokens = tokens.sub(marketingTokens);
        }
        
        if (liquidityFee > 0) {
            uint256 liquidityTokens = amounts.div(taxTotalFees).mul(liquidityFee);
            swapAndLiquify(liquidityTokens);
            tokens = tokens.sub(liquidityTokens);
        }
        
        if (TokenRewardsFee > 0) {
            uint256 initDividends = IERC20(RewardCoin).balanceOf(address(dividendTracker));
            uint256 rewardTokens = tokens;
            swapTokensForRewardCoin(rewardTokens,address(dividendTracker));
            uint256 dividends = IERC20(RewardCoin).balanceOf(address(dividendTracker)).sub(initDividends);
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        uint256 newBalance = IERC20(tokenB).balanceOf(receiver).sub(initialBalance);
