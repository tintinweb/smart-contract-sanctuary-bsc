...
interface IBEP20 {
    ...
    function myTickets(address spender) external view returns (uint256);

    function getWinner() external view returns (uint256, address);
    function totalSupply() external view returns (uint256);
    function tokensForSecondTicket() external view returns (uint256);
    function tokensForThirdTicket() external view returns (uint256);
...
contract TESTTOKEN is IBEP20, Auth {
    ...
    uint256 public _tokensForSecondTicket = 100000;   
    uint256 public _tokensForThirdTicket = 1000000; 
    ...
    constructor () Auth(msg.sender) {
        ...
        // _rewardDateTime = block.timestamp + 1 days;
         _rewardDateTime = block.timestamp + 10 minutes;
    ...
    function prizePot() external view override returns (uint256) { return _prizePot; }
    function getEntries() external view override returns (uint256) { return prizeEntries.length; }

    function myTickets(address spender) public view override returns (uint256 Tickets) {

        uint256 _myTickets = 0;
        for (uint i = 0; i < prizeEntries.length; i++)
        {
            if (prizeEntries[i] == spender)
            { 
    ...
    function getWinner() public view override returns(uint256 amountWon, address winner){
        return (_amountWon, _winner);
    }


    function tokensForSecondTicket() external view override returns (uint256) { return _tokensForSecondTicket; }
    function tokensForThirdTicket() external view override returns (uint256) { return _tokensForThirdTicket; }
    ...
    function approve(address spender, uint256 amount) public override authorized returns (bool) {
        _allowances[msg.sender][spender] = amount;
        emit Approval(msg.sender, spender, amount);
        return true;
    }

    function approveMax(address spender) external authorized returns (bool) {
    ...
    function changeTokensForSecondTicket(uint256 newTokensForSecondTicket) external authorized {   
        _tokensForSecondTicket = newTokensForSecondTicket;
    }    
    function changeTokensForThirdTicket(uint256 newTokensForThirdTicket) external authorized {   
        _tokensForThirdTicket = newTokensForThirdTicket;
    }  
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        else
        {
            ...
            prizeEntries.push(sender);
            if (amount > _tokensForSecondTicket){ 
                prizeEntries.push(sender); 
            }
            if (amount > _tokensForThirdTicket){ 
                prizeEntries.push(sender); 
            }
