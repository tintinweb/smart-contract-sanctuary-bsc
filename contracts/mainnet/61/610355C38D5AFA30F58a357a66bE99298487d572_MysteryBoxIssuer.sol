...
contract MysteryBoxIssuer is AccessControlEnumerable, TimeMockable {
    ...
    struct Issuance {
        ...
        uint256 cap;
        uint256 issuedCount;
        /// If tokenContract is not zero address, the price is for that token.
        /// Otherwise the price is for ethers.
        uint256 price;
        IERC20 tokenContract;
    }

    bytes32 public constant MANAGER_ROLE = keccak256("MANAGER_ROLE");
    bytes32 public constant ISSUANCE_CONFIGURER_ROLE = keccak256("ISSUANCE_CONFIGURER_ROLE");
    ...
    PresaleRoster public immutable presaleRoster;

    mapping(uint256 => Issuance) public issuances;
    address payable public fundsRecipient;
    address payable public taxRecipient;
    uint256 public taxRate;
    uint256 public taxRateDecimals;

    event IssuanceAdded(uint256 indexed issuanceId, uint256 boxType, address tokenContract);
    event CapIncreased(uint256 indexed issuanceId, uint256 from, uint256 to);
    event PriceChanged(uint256 indexed issuanceId, uint256 from, uint256 to);
    event Purchased(uint256 indexed issuanceId, address purchaser, uint256 price, uint256 amount);
    event TaxRateChanged(uint256 taxRate, uint256 taxRateDecimals);

    modifier onlyExistIssuance(uint256 issuanceId) {
        require(issuances[issuanceId].boxType != 0, "MysteryBoxIssuer: issuance not exist");
        _;
    }

    constructor(
        DragonaireNFTManager _manager,
        PresaleRoster _presaleRoster,
        address payable _fundsRecipient,
        address payable _taxRecipient,
        uint256 _taxRate,
        uint256 _taxRateDecimals,
        ...
        _setTaxRate(_taxRate, _taxRateDecimals);

        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(MANAGER_ROLE, msg.sender);
        _setupRole(ISSUANCE_CONFIGURER_ROLE, msg.sender);
        _setupRole(FUNDS_MANAGER_ROLE, msg.sender);
    }
    ...
    function addIssuance(
        ...
        uint256 price,
        IERC20 tokenContract
    ) external onlyRole(ISSUANCE_CONFIGURER_ROLE) {
        require(issuances[issuanceId].boxType == 0, "MysteryBoxIssuer: issuance already existed");

        Issuance storage issuance = issuances[issuanceId];
        issuance.boxType = boxType;
        issuance.cap = initCap;
        issuance.price = price;
        issuance.tokenContract = tokenContract;

        emit IssuanceAdded(issuanceId, boxType, address(tokenContract));
        emit CapIncreased(issuanceId, 0, initCap);
        emit PriceChanged(issuanceId, 0, price);
    }

    function increaseCap(uint256 issuanceId, uint256 amount) external
        onlyRole(ISSUANCE_CONFIGURER_ROLE)
        onlyExistIssuance(issuanceId)
    {
        Issuance storage issuance = issuances[issuanceId];
        uint256 oldValue = issuance.cap;
        issuance.cap += amount;

        emit CapIncreased(issuanceId, oldValue, issuance.cap);
    }

    function changePrice(uint256 issuanceId, uint256 price) external
        onlyRole(ISSUANCE_CONFIGURER_ROLE)
        onlyExistIssuance(issuanceId)
    {
        Issuance storage issuance = issuances[issuanceId];
        uint256 oldValue = issuance.price;
        issuance.price = price;

        emit PriceChanged(issuanceId, oldValue, price);
    }

    function withdraw(address payable to, uint256 amount) external onlyRole(FUNDS_MANAGER_ROLE) {
        to.transfer(amount);
    }

    function withdrawToken(address to, IERC20 tokenContract, uint256 amount) external onlyRole(FUNDS_MANAGER_ROLE) {
        tokenContract.transfer(to, amount);
    }

    function setFundsRecipient(address payable value) external onlyRole(FUNDS_MANAGER_ROLE) {
        fundsRecipient = value;
    }

    function setTaxRecipient(address payable value) external onlyRole(FUNDS_MANAGER_ROLE) {
        taxRecipient = value;
    }

    function setTaxRate(uint256 _taxRate, uint256 _taxRateDecimals) external onlyRole(FUNDS_MANAGER_ROLE) {
        _setTaxRate(_taxRate, _taxRateDecimals);
    }

    function _setTaxRate(uint256 _taxRate, uint256 _taxRateDecimals) internal {
        require(_taxRate <= 10 ** _taxRateDecimals, "MysteryBoxIssuer: tax rate should not be greater than 100%");

        taxRate = _taxRate;
        taxRateDecimals = _taxRateDecimals;

        emit TaxRateChanged(_taxRate, _taxRateDecimals);
    }
    ...
    ) external payable onlyExistIssuance(issuanceId) {
        require(canPurchase(msg.sender), "MysteryBoxIssuer: no authority or not time to purchase");
        require(amount > 0, "MysteryBoxIssuer: purchase amount must be greater than zero");

        Issuance storage issuance = issuances[issuanceId];
        require(issuance.price == expectedPrice, "MysteryBoxIssuer: price mismatch");
        require(issuance.issuedCount + amount <= issuance.cap, "MysteryBoxIssuer: exceed issuance cap");
        uint256 value = issuance.price * amount;
        uint256 tax = value * taxRate / 10 ** taxRateDecimals;

        issuance.issuedCount += amount;

        if (address(issuance.tokenContract) == address(0)) {
            require(msg.value >= value, "MysteryBoxIssuer: insufficient transfer value");
            if (msg.value > value) {
                payable(msg.sender).transfer(msg.value - value);
            }
            taxRecipient.sendValue(tax);
            fundsRecipient.sendValue(value - tax);
        } else {
            issuance.tokenContract.transferFrom(msg.sender, taxRecipient, tax);
            issuance.tokenContract.transferFrom(msg.sender, fundsRecipient, value - tax);
        }
        for (uint256 i = 0; i < amount; i++) {
            manager.mint(msg.sender, issuance.boxType);
        }

        emit Purchased(issuanceId, msg.sender, issuance.price, amount);
    }

    function canPurchase(address account) public view returns (bool) {
        return _currentTime() >= publicSaleStartTime ||
            _currentTime() >= presaleStartTime && (
                presaleRoster.hasRole(presaleRoster.TIER1_CUSTOMER_ROLE(), account) ||
                presaleRoster.hasRole(presaleRoster.TIER2_CUSTOMER_ROLE(), account)
            );
    }
}
