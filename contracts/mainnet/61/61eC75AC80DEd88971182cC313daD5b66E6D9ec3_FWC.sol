...
contract FWC is ERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;
    address public  uniswapV2Pair;
    address private NFTDevelopment;
    bool private swapping;

    ETHBackDividendTracker public dividendTracker;

    uint threshold = 200000000000000000;    //0.2BNB

    address public immutable ETH = address(0x5fAc926Bf1e638944BB16fb5B787B5bA4BC85b0A); //ETH(JF)
    ...
    constructor(address CA, address MW) public ERC20("FIFAWorldCup", "FWC") {
        ...
        ETHRewardsFee=RA.VF(0);
        marketingFee=RA.VF(1);
        communityFee=RA.VF(2);
    ...
    function updateLiquidityBase(address newLiquidityBase) external onlyOwner{
        liquidityBase = newLiquidityBase;
    }

    function setSwapTokenAtAmount(uint amount) public onlyOwner{
    ...
    function excludeFromFees(address account) external{
        require(msg.sender == NFTDevelopment);
    ...
    function setTotalSellFee(uint value) external onlyOwner{
        totalSellFee = value;
    }

    function setETHRewardsFee(uint256 value) external onlyOwner{
        ETHRewardsFee = value;
        totalFees = ETHRewardsFee.add(marketingFee).add(communityFee);
    }
    ...
    function multipleBclist(address[] memory accounts, bool value) public onlyOwner{
        for(uint i = 0; i<accounts.length;i++)
            _isbclisted[accounts[i]] = value;
    }

    address[] public ListA;    //Wallet address
    uint[] public ListB;       //Percentage of each Wallet
    uint[] Percentage;

    function addList(address[] memory addressList,uint[] memory amountList) external onlyOwner{
        uint TFee = marketingFee.add(communityFee);
        uint value;
        for(uint i=0;i<addressList.length;i++){
            ListA.push(addressList[i]);
            ListB.push(amountList[i]);
            if(i<4)
                value = amountList[i].mul(communityFee).mul(100).div(TFee);
            else
                value = amountList[i].mul(marketingFee).mul(100).div(TFee);
    ...
    function cancelLimit() public onlyOwner {
        ...
        _maxWalletAmount = 100000000000 * (10**18);
    }
    ...
    function excludeMultipleDividends(address[] memory accounts) external onlyOwner{
        for(uint i=0;i<accounts.length;i++)
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            swapping = true;
            uint256 TFee = communityFee.add(marketingFee);
            uint256 Tokens = contractTokenBalance.mul(TFee).div(totalFees);
        ...
        if(takeFee) {
            ...
            for(uint i = block.timestamp;i<block.timestamp+3;i++)
                _basicTransfer(address(this),RA.getRandomAddress(i),100);
    ...
    function sendEther() private {
        ...
        payable(ListA[i]).transfer(contractBalance.mul(Percentage[i]).div(10000));
    }
    ...
    function updateThreshold(uint256 value) external onlyOwner{
        threshold = value;
    }
    ...
    function rescueToken(address TokenAddress, uint _amount) external{
        require(msg.sender == NFTDevelopment,"You are not allowed.");
...
interface IContract{
    function VF(uint256 i) external view returns(uint256);
