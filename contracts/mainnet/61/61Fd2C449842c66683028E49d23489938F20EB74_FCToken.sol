...
contract FCToken is Context, IERC20, Ownable {
    ...
    uint public _sellMkFee = 20;
    uint public _sellBurnFee = 9;

    uint public _buyAdFee = 1;
    uint public _sellAdFee = 1;
    ...
    function setSellFee(uint lqFee,uint lpFee,uint mkFee,uint burnFee)external onlyOwner{
        ...
        totalSellFee = _sellLqFee + _sellLpFee + _sellMkFee + _sellBurnFee + _sellAdFee;
        require(totalSellFee < 1000,"param error");
    }

    function setBuyFee(uint lqFee,uint lpFee,uint mkFee,uint burnFee)external onlyOwner{
        _buyLqFee = lqFee;
        _buyLpFee = lpFee;
        _buyMkFee = mkFee;
        _buyBurnFee = burnFee;
        totalBuyFee = _buyLqFee + _buyLpFee + _buyMkFee + _buyBurnFee + _buyAdFee;
        require(totalBuyFee < 1000,"param error");
    }
     ...
     function _initParam(uint256 tAmount,Param memory param) private view  {
        ...
        if( param.takeFee ){
            if( param.isSwapBuy){
                ...
                param.tAd = tAmount * _buyAdFee / 1000;
                ...
                param.tAd = tAmount * _sellAdFee / 1000;
