...
contract TokenRedeem is Ownable  {
    ...
    uint256 maxBuy = 5000*1e18;
    bool public isPresalePaused = false;
    uint256[] public referralTier = [100,300,500];
    ...
    function buy(uint256 amount) public
    {
        ...
        require(BuyerList[msg.sender].add(amount.mul(rate).div(1e2))<=maxBuy,"Can not buy over maxBuy");
        busd.safeTransferFrom(
            msg.sender,
            address(this),
            amount.mul(rate).div(1e2)
        );
        BuyerList[msg.sender] = BuyerList[msg.sender].add(amount.mul(rate).div(1e2));
    ...
    function payReferralCommission(
        ...
        if (depth < referralTier.length-1) {
            if (address(referralContract) != address(0)) {
                ...
                uint256 commissionAmount = _pending
                    .mul(referralTier[depth])
                    .div(10000);
                if (commissionAmount > 0 && _referrer!=address(0)) {
                    busd.safeTransfer(_referrer, commissionAmount);
    ...
    function setReferralTier(uint256[] memory _referralTier) public onlyOwner {
        referralTier = _referralTier;
    }
    ...
    function setReferralContract(IReferral _referral) public onlyOwner {
