...
// File: contracts/farm/IToken.sol
...
contract BitstasioTokenFarm {
    ...
    event BurnShares(address ref, uint256 sharesBurned);
    ...
    function compoundBits(address ref) public {
        ...
        if (referral.referredBy != address(0x0)) {
            refer(referral.referredBy, bitUsed);
        }
    ...
    function sellBits() public {
        ...
        uint256 fee = getFee(bitValue).mul(2);
        uint256 sharesToBurn = (sharesOwned.mul(2)).div(100);
        ...
        if (sharesOwned >= 50) {
            burnShares(sharesToBurn); // burn 2% shares
        }
    ...
    function buyBits(address ref, uint256 amount) public isNotContract {
    ...
    function getFee(uint256 amount) public view returns (uint256) {
        return (amount.mul(feePerc)).div(100);
    }
    ...
    function getReferredBitsReceived(address referred)
        public
        view
        returns (uint256)
    {
