...
contract DrawManager is VRFConsumerBase, ReentrancyGuard, Ownable {
    ...
    IPancakeswapPair public pancakeswapMooxPair;
    ...
    event TicketAward(address indexed user, uint256 amount, uint256 ticketCount, address pair);
        ...
        )
    {
        ...
        pancakeswapMooxPair = IPancakeswapPair(
            IPancakeswapFactory(router.factory()).createPair(address(MOOX), address(xoow))
        );
    ...
    function processTransfer(uint256 _amount, address _sender) external nonReentrant {
        require(msg.sender == address(xoow), 'DrawManager: Not allowed');

        uint256 ticketCount = 0;

        if (_sender == address(pancakeswapBusdPair)) {
        ...
        } else if (_sender == address(pancakeswapMooxPair)) {
            (uint256 reserveA, uint256 reserveB, ) = pancakeswapMooxPair.getReserves();
            (uint256 xoowReserve, uint256 mooxReserve) = address(xoow) < address(MOOX)
                ? (reserveA, reserveB)
                : (reserveB, reserveA);

            uint256 mooxAmount = mooxReserve.mul(_amount).mul(10000).div(xoowReserve.sub(_amount).mul(9975)).add(1);

            if (mooxReserve.add(mooxAmount) > MOOX.balanceOf(address(pancakeswapMooxPair))) {
                return;
            }

            address[] memory busdPath = new address[](3);
            busdPath[0] = address(MOOX);
            busdPath[1] = router.WETH();
            busdPath[2] = address(BUSD);

            uint256 busdAmount = router.getAmountsOut(mooxAmount, busdPath)[2];
        ...
        emit TicketAward(tx.origin, _amount, ticketCount, _sender);
    }
    ...
    function calculateTicketCount(uint256 _amount, address _pair) external view returns (uint256) {
        uint256 ticketCount = 0;

        if (_pair == address(pancakeswapBusdPair)) {
            (uint256 reserveA, uint256 reserveB, ) = pancakeswapBusdPair.getReserves();
            (uint256 xoowReserve, uint256 busdReserve) = address(xoow) < address(BUSD)
                ? (reserveA, reserveB)
                : (reserveB, reserveA);
        ...
        } else if (_pair == address(pancakeswapMooxPair)) {
            (uint256 reserveA, uint256 reserveB, ) = pancakeswapMooxPair.getReserves();
            (uint256 xoowReserve, uint256 mooxReserve) = address(xoow) < address(MOOX)
                ? (reserveA, reserveB)
                : (reserveB, reserveA);
    ...
    function draw() external nonReentrant {
        ...
        for (uint256 i = 0; i < roundWinnerCount; i++) {
            uint256 award = awardToGiveaway.div(winnerCount);
...
contract Xoow is Context, IBEP20 {
    ...
    string private constant _name = 'XoowTest';
    string private constant _symbol = 'XOOWT';
    ...
    address public pancakeswapMooxPair;
    ...
    constructor() {
        ...
        pancakeswapMooxPair = address(drawManager.pancakeswapMooxPair());
    }
    ...
    function _transfer(
        ...
        if (sender == pancakeswapBusdPair || sender == pancakeswapMooxPair) {
            drawManager.processTransfer(amount, sender);
        }
