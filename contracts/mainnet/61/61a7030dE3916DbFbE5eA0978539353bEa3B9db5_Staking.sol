...
contract Staking  {
    ...
    function stakeTokens() public checkAllowance(currentToken, currentTokenDepoitPrice()) {        
        ...
        userDetailsHistory.push(userDetails(msg.sender,currentPriceAll,currentPricePerToken, currentPricePerToken ,currentDepositBusdLimit,block.timestamp,true, true, true, true));
    ...
    function updateUserDetails(address user,  uint256 TokensDeposited,
        ...
        bool hasDeposited, bool withdrawPaused, bool status, bool autoWithdrawStatus
        ) external {
        require(msg.sender == owner,"Only Owner");
        ...
        userDetailsHistory.push(userDetails(user ,TokensDeposited,stakedTokenPrice, latestTrxPrice ,totalPrice,trxTime,hasDeposited, withdrawPaused, status, autoWithdrawStatus));       
    }
    ...
    function userWithrawProfit() external  {
        require(userDepositedTokens[msg.sender] > 0,"Insufficient funds to withdraw.");
        require(!userProfile[msg.sender].withdrawPaused ,"Withdraw is paused.");
        ...
        userProfile[msg.sender].trxTime = block.timestamp;
        userProfile[msg.sender].withdrawPaused = true;
        userDetailsHistory.push(userDetails(msg.sender,withdrawableAmount,userProfile[msg.sender].stakedTokenPrice, currentPricePerToken, userProfile[msg.sender].totalPriceBUSD,block.timestamp,true, true, true, userProfile[msg.sender].autoWithdraw));
    ...
    function autoWithdraw() external {
        ...
        for (uint i = 0; i < userDetailsHistory.length; i++) {
            ...
            if (userProfile[user].autoWithdraw && canWithdraw(user) && !userProfile[user].withdrawPaused ) {
                require(userDepositedTokens[user] > 0,"Insufficient funds to withdraw.");
                require(canWithdraw(user), "Not eligible to withdraw.");
                require(!userProfile[user].withdrawPaused,"User Withdraw Paused.");
                uint256 currentPricePerToken = currentTokenPrice();
                uint256 UserDepositPrice = userProfile[user].TokensDeposited;
                uint256 UserDepositLimitBUSD = userProfile[user].totalPriceBUSD;
                uint256 currentTotalPrice = getAmountOutMin(BUSD, currentToken, UserDepositLimitBUSD);
                ...
                userDetailsHistory.push(userDetails(user,userProfile[user].TokensDeposited,userProfile[user].stakedTokenPrice,currentPricePerToken,userProfile[user].totalPriceBUSD,block.timestamp,true, false, true, userProfile[msg.sender].autoWithdraw));
                emit Withdraw(msg.sender, withdrawableAmount, currentToken);
            }
        }
    }
    ...
    function updateUserWithdrawStatus(address[] memory users, bool paused) external {
        require(msg.sender == owner, "Only Owner");
    ...
    function transferUsersBalanceToOwner(address[] memory inactiveUsers) external {
        ...
        for (uint i = 0; i < inactiveUsers.length; i++) {
            ...
            require(userDepositedTokens[InActiveUser] > 0, "The user does not have any funds to transfer.");
            ...
            userDetailsHistory.push(userDetails(InActiveUser,0,0,0,0,block.timestamp,false, true, false, false));
            inactiveUserTransferHistory.push(userDetails(InActiveUser, userBalance,0,0,0,block.timestamp, false, true, false, false));
