...
contract Factory is Initializable,ERC20Upgradeable {

    uint256 public poolId;

    address owner;

    mapping(uint256=>address) public poolAddress;
    ...
    function createPool(
        ...
        external
        returns (Staking pool)
    {
        pool = new Staking(_token, _owner, _factoryContract, _rewards, _decimals, _rewardsDuration);
        poolId+=1;
        poolAddress[poolId] = address(pool); 
    }
    
    function withdrawRewardToken(uint256 _poolId) external 
    {
        require(IStaking(poolAddress[_poolId]).stakeBalanceOfUser(msg.sender)!=0, "token not stake");

        uint256 reward = IStaking(poolAddress[_poolId]).getReward(msg.sender);
