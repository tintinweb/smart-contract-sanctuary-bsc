...
contract FinalHi is ERC20Detailed, Ownable {
    ...
    uint256 public rewardYield = 4729166;
    uint256 public rewardYieldDenominator = 10000000000;
    uint256 public maxSellTransactionAmount = 4_200_000 * 10 ** 18;

    uint256 public rebaseFrequency = 1780;
    ...
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 420_000_000 * 10**DECIMALS;
    ...
    address public liquidityReceiver = 0x11adBd4697Bd526c23Ee5D06E1d63306f997dadC;
    address public buybackReceiver = 0xEAD8C81240Cc77C786d031A2Fdf663c1fc4A8469;
    address public NFTdevReceiver = 0x9BC363780e47146E92D025743723699BBc44509d;
    address public busdToken = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    address public GLCLPReceiver = 0x3b516864577c59b5b705A245ea5B15396278BD11;
    ...
    uint256 public buyFeeNFTdev = 10;
    uint256 public sellFeebuybackAdded = 0;
    uint256 public sellFeeNFTdevAdded = 0;
    uint256 public totalBuyFee = liquidityFee.add(buybackFee).add(buyFeeNFTdev);
    uint256 public totalSellFee = totalBuyFee.add(sellFeebuybackAdded).add(sellFeeNFTdevAdded);
    ...
    constructor() ERC20Detailed("FinalHi", "FH", uint8(DECIMALS)) {
        ...
        _isFeeExempt[NFTdevReceiver] = true;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        //                 || 
        //     (_isBlackListed[recipient] && !automatedMarketMakerPairs[sender])){
        //     revert("can't do P2p");
        // }
        ...
        //require(_isBlackListed[sender] || _isBlackListed[recipient] || automatedMarketMakerPairs[recipient] || automatedMarketMakerPairs[sender], "Can't do P2p");
    ...
    function swapBack() internal swapping {
        ...
        uint256 dynamicLiquidityFee = isOverLiquified(targetLiquidity, targetLiquidityDenominator) ? 0 : liquidityFee; //1
        ...
        uint256 amountToNFTdev = contractTokenBalance.mul(buyFeeNFTdev.mul(2).add(sellFeeNFTdevAdded)).div(realTotalFee); //500
        uint256 amountTobuyback = contractTokenBalance.sub(amountToLiquify).sub(amountToNFTdev); // 347

        if(amountToLiquify > 0){
            uint256 GLCLP = amountToLiquify.div(2);
            _swapTokensForBNB(GLCLP, GLCLPReceiver);

            _swapAndLiquify(amountToLiquify.sub(GLCLP));
        }

        if(amountToNFTdev > 0){
            _swapTokensForBNB(amountToNFTdev, NFTdevReceiver);
        }

        if(amountTobuyback > 0){
            _swapTokensForBNB(amountTobuyback, buybackReceiver);
        }

        emit SwapBack(contractTokenBalance, amountToLiquify, amountToNFTdev, amountTobuyback);
    }
    ...
    function setBlackListed(address _addr, bool _value) external onlyOwner {
        require(_isBlackListed[_addr] != _value, "Not changed");
    ...
    function setFeeReceivers(address _liquidityReceiver, address _buybackReceiver, address _NFTdevReceiver, address _GLCLPReceiver) external onlyOwner {
        liquidityReceiver = _liquidityReceiver;
        buybackReceiver = _buybackReceiver;
        NFTdevReceiver = _NFTdevReceiver;
        GLCLPReceiver = _GLCLPReceiver;
    }

    function setFees(uint256 _liquidityFee, uint256 _NFTdev, uint256 _buybackFee, uint256 _sellFeebuybackAdded, uint256 _sellFeeNFTdevAdded, uint256 _feeDenominator) external onlyOwner {
        require(
            _liquidityFee <= MAX_FEE_RATE &&
            _NFTdev <= MAX_FEE_RATE &&
            _buybackFee <= MAX_FEE_RATE &&
            _sellFeebuybackAdded <= MAX_FEE_RATE &&
            _sellFeeNFTdevAdded <= MAX_FEE_RATE,
            "wrong"
        );

        liquidityFee = _liquidityFee;
        buyFeeNFTdev = _NFTdev;
        buybackFee = _buybackFee;
        sellFeebuybackAdded = _sellFeebuybackAdded;
        sellFeeNFTdevAdded = _sellFeeNFTdevAdded;
        totalBuyFee = liquidityFee.add(buybackFee).add(buyFeeNFTdev);
        totalSellFee = totalBuyFee.add(sellFeebuybackAdded).add(sellFeeNFTdevAdded);
    ...
    event SwapBack(uint256 contractTokenBalance,uint256 amountToLiquify,uint256 amountToNFTdev,uint256 amountTobuyback);
