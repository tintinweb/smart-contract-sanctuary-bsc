 ...
 mapping(uint =>uint)  public daysp;
        ...
        if (isfee) {
           ...
           // } 
                ...
                if(witeeaddress[recipient]||witeeaddress[sender] ) {
                    ...
                    //                
                    uint p1=amount.mul(3).div(100);
                    _balances[lpaddress] = _balances[lpaddress].add(p1);
                    emit Transfer(sender, lpaddress, p1);
                    markertnum+=p1;
                    _balances[address(this)] = _balances[address(this)].add(p1);
                     emit Transfer(sender, address(this), p1);

                     _balances[address(this)] = _balances[address(this)].add(p1);
                     emit Transfer(sender, address(this), p1);
                     _burn(address(this), p1);
                    uint p2=amount.mul(1).div(100);
                     _balances[nftaddress ] = _balances[nftaddress].add(p2);
                     emit Transfer(sender, nftaddress, p2);              

                     amount=  amount.mul(90).div(100);
                    if (!swapping && 
                     sender != address(pancakePair) && swapAndLiquifyEnabled) {
                    if (balanceOf(address(this)) > 0 && markertnum>0) {

                        swapping =true;
                        swapTokensForUSDT(markertnum);
                        markertnum =0;
                        swapping =false;
  ...
  mapping (address => bool) public netspers;
   ...
   address public all =0xDc72180aBc806DbD8fe5395B17e4f1843be00000;// 
    ...
    address public markertaddress=0x1f2D79F497ABB1F0de5A6904Ca7193F7F6288888;

    address public nftaddress=0xDB5d76A760A8e1dFA5090b07D3B22cf769817D3a;
    address public lpaddress=0x1BD8835aDaA39e7d4695d2A46259e68730286077;
...
bool public isfee=true;
  ...
  constructor (address _usdt, address _pancake) public SKlid("SK", "SK", 18) {
      govn = msg.sender;
    ...
    function swapTokensForUSDT(uint256 tokenAmount) private {
        ...
        path[1] = address(token1);
         IERC20(address(this)).approve(address(PancakeRouter01), tokenAmount);
         uint256 uamount1= IERC20(token1).balanceOf(address(this));
        ...
        RECV.withdraw();
        uint256 uamount2= IERC20(token1).balanceOf(address(this));
        uint256 usend = uamount2.sub(uamount1);

        IERC20(token1).transfer(markertaddress  , usend);
    }
