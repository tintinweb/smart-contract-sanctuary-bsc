...
contract PledgeV2 is Ownable,ReentrancyGuard {
    ...
    constructor(
            ...
            plans[1] = plan(true,35,100,100,sgtAddress);
            plans[2] = plan(true,150,100,100,eneAddress);
        }
    ...
    function getSgtNumberByLp(uint256 amount) public view returns(uint256){
    ...
    function pledgeLp(uint256 planId,uint256 amount) public nonReentrant {
        ...
        uint256 sgtNumber = getSgtNumberByLp(amount);
        //create order
    ...
    function earn(address userAddress,uint256 planId,uint256 orderId) public view returns(uint256) {
        ...
        return (order_info.sgtNumber * order_info.rate) / order_info.rateBase;
    ...
    function getReward(uint256 planId,uint256 orderId) public nonReentrant  {
        ...
        if(order_info.tokenAddress==sgtAddress){
            require(IERC20(sgtAddress).transfer(address(msg.sender),reward), "transfer reward token failed");
            userInfos[msg.sender].receivedSgt +=reward;
        }else{
            require(mintToken(eneAddress).mint(address(msg.sender), reward), "mint failed");
    ...
    event Relock(address user, uint256 planId, uint256 orderId, uint256 lpAmount, uint256 sgtAmount,uint256 time);
    ...
    function relock(uint256 planId, uint256 orderId) external nonReentrant {
        ...
        orderIds[msg.sender][planId]++;
        emit Relock(msg.sender, planId, orderId,amount, sgtNumber,block.timestamp);
    }
}