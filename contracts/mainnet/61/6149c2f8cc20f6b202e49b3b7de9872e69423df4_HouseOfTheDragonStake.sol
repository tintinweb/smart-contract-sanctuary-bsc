...
contract HouseOfTheDragonStake is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    IBEP20 public immutable fireToken;
    IBEP20 public immutable rewardToken;

    uint256 public maxUserStaking;
    ...
    constructor(
        ...
        fireToken = IBEP20(0x01563597c1590179946c359B68E7C7E3A5354b2e);
        rewardToken = IBEP20(0x01563597c1590179946c359B68E7C7E3A5354b2e);
        rewardPerBlock = 10000000000000;
        startBlock = block.number;
        bonusEndBlock = 99999999;

        // staking pool
        poolInfo.push(PoolInfo({
            lpToken: fireToken, // Coin
            allocPoint: 100000,
    ...
    function deposit(uint256 _amount) public nonReentrant {
        ...
        require (_amount.add(user.amount) <= maxUserStaking, 'exceed max stake');
    ...
    function claimRewards() public {
        PoolInfo storage pool = poolInfo[0];
        UserInfo storage user = userInfo[msg.sender];
        //require(user.amount >= _amount, "withdraw: attempting to claim too many rewards");
        updatePool(0);
        uint256 pending = user.amount.mul(pool.accTokensPerShare).div(1e12).sub(user.rewardDebt);
        if(pending > 0) {
    ...
    function updateMaxUserStaking(uint256 _amount) external onlyOwner {
        updatePool(0);
        maxUserStaking = _amount;
    }

    function updateAllocPoint(uint256 _amount) external onlyOwner {
        updatePool(0);
        allocPoint = _amount;
    }
