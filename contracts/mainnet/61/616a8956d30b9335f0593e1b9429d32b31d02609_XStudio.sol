...
error AdminRequired();
error Initialized();
abstract contract XStudioFlags is UsingFlagsWithStorage {
    ...
    struct Flags {
        ...
        uint256 feeExempt;
        uint256 blocked;
        uint256 transferDisabledExempt;
    }
    ...
    uint constant TOKEN = ADMIN << 1; // 4
    uint constant LIQUIDITY_PAIR = TOKEN << 1; // 5
    uint constant ROUTER = LIQUIDITY_PAIR << 1; // 6
    uint constant FEE_EXEMPT = ROUTER << 1; // 7
    uint constant BLOCKED = FEE_EXEMPT << 1; // 8
    uint constant TRANSFER_DISABLED_EXEMPT = BLOCKED << 1; // 9
    uint constant STAKING_POOL = TRANSFER_DISABLED_EXEMPT << 1; // 10

    uint constant SWAP_MARKETING_TOKENS_STATE = 0;
    uint constant SWAP_LIQUIDITY_TOKENS_STATE = 1;
    uint constant PAIR_LIQUIDITY_STATE = 2;
    uint constant TOTAL_STATES = 3;
    ...
    uint constant SENDER_IS_TOKEN = TOKEN << SENDER_OFFSET;
    uint constant SENDER_IS_ROUTER = ROUTER << SENDER_OFFSET;
    uint constant SENDER_IS_TRANSFER_DISABLED_EXEMPT = TRANSFER_DISABLED_EXEMPT << SENDER_OFFSET;
...
contract XStudio is XStudioFlags, UsingBatchTransfer, UsingERC20WithStorage, UsingPermitWithStorage, UsingUUPS {
    ...
    struct FeeAllocations {
        ...
        uint80 marketingAllocation;
        uint80 stakingAllocation;
    }
    ...
    uint _state;
    IStakingPool _stakingContract;

    receive() external payable {}

    function setStakingContract(address stakingPool_) external requiresAdmin {
        if (address(_stakingContract) != address(0)) {
            _setFlags(address(_stakingContract), 0, STAKING_POOL);
            _flaggedAccounts[STAKING_POOL].remove(address(_stakingContract));
        }
        _setFlags(stakingPool_, STAKING_POOL, 0);
        _flaggedAccounts[STAKING_POOL].add(stakingPool_);
        _stakingContract = IStakingPool(stakingPool_);
    }
    ...
    /// @notice set the marketing contract address
    /// @dev the marketing contract is just an empty vessel to store tokens
    function setMarketingContract(address marketingContract_) external requiresAdmin {
        _marketingContract = marketingContract_;
    }

    /// @notice get the marketing wallet address
    function getMarketingContract() external view returns (address) {
        return _marketingContract;
    }

    /// @notice set the liquidity receiver address
    function setLiquidityReceiver(address liquidityReceiver_) external requiresAdmin {
        _liquidityReceiver = liquidityReceiver_;
    }

    /// @notice get the liquidity receiver address
    function getLiquidityReceiver() external view returns (address) {
        return _liquidityReceiver;
    }

    /// @notice set the router address
    function setRouter(address router_) external requiresAdmin {
    ...
    function feeAllocations() public view returns (FeeAllocations memory) {
        return FeeAllocations(_feesAndAllocations.liquidityAllocation, _feesAndAllocations.marketingAllocation, _feesAndAllocations.stakingAllocation);
    }
    ...
    /// @dev this can only be called by the admin
    function setFeeAllocations(uint32 liquidityAllocation_,  uint32 marketingAllocation_, uint32 stakingAllocation_) external requiresAdmin {
        FeeAllocations memory oldFeeAllocations = feeAllocations();
        (_feesAndAllocations.liquidityAllocation, _feesAndAllocations.marketingAllocation, _feesAndAllocations.stakingAllocation) = (liquidityAllocation_, marketingAllocation_, stakingAllocation_);
    ...
    /// @dev this can only be called by the admin
    function setMarketingWalletAddress(address marketingAddress_) external requiresAdmin {
        address oldMarketingAddress = _marketingWallet;
        _marketingWallet = marketingAddress_;
        emit MarketingAddressChanged(msg.sender, oldMarketingAddress, _marketingWallet);
    }
    ...
    /// @notice deposit tokens to the staking contract and bypass autostaking
    function depositTokensForStaking(uint256 amount_) external requiresAdmin {
        super._transfer(msg.sender, address(_stakingContract), amount_);
        _stakingContract.directDepositRewards(amount_);
    }
    ...
    function _transfer(address source_, address target_, uint256 amount_) internal override(UsingERC20, UsingBatchTransfer) returns (bool) {
        ...
        _assertFlags(flags, THIS_TRANSFERS_ENABLED | SENDER_IS_ADMIN | SENDER_IS_TRANSFER_DISABLED_EXEMPT, SOURCE_IS_BLOCKED | TARGET_IS_BLOCKED);
        uint fee;
        if (!_checkFlags(flags, SOURCE_IS_FEE_EXEMPT | TARGET_IS_FEE_EXEMPT, 0)) {
            fee = _process(source_,target_, amount_, flags);
            if (fee == amount_) return true;
        }
        return super._transfer(source_, target_, amount_ - fee);
    }

    function _process(address source_, address target_, uint amount_, uint flags_) internal returns (uint) {
        if (_checkFlags(flags_, SOURCE_IS_LIQUIDITY_PAIR, 0)) {
            ...
            _depositFees(source_, amount_);
        } else if (flags_.any(TARGET_IS_STAKING_POOL)) {
            super._transfer(source_, target_, amount_);
            _stakingContract.autoStake(source_, amount_);
        } else if (flags_.any(SOURCE_IS_STAKING_POOL)) {
            super._transfer(source_, target_, amount_);
        } else {
            ...
            uint state = _state % TOTAL_STATES;
            unchecked {
                ++_state;
            }
            if (state == SWAP_MARKETING_TOKENS_STATE) {
                uint marketingBalance = _balanceOf(_marketingContract);
                if (marketingBalance > 0) {
                    super._transfer(_marketingContract, address(this), marketingBalance);
                    _executeSwap(marketingBalance, _marketingWallet);
                }
                return amount_;
            }
            uint liquidityBalance = _balanceOf(address(this));
            if (state == SWAP_LIQUIDITY_TOKENS_STATE) {
                if (liquidityBalance > 0) {
                    _executeSwap(liquidityBalance, address(this));
                }
            } else {
                uint balance = address(this).balance;
                if (balance > 0) {
                    _executeLiquidityInjection(balance, liquidityBalance);
                }
            }
        }
        return amount_;
    ...
    function _depositFees(address source_, uint amount_) internal {
        super._transfer(source_, address(this), _calculatePercentage(amount_, _feesAndAllocations.liquidityAllocation));
        super._transfer(source_, _marketingContract, _calculatePercentage(amount_, _feesAndAllocations.marketingAllocation));
        uint stakingAllocation = _calculatePercentage(amount_, _feesAndAllocations.stakingAllocation);
        if (stakingAllocation > 0) {
            super._transfer(source_, address(_stakingContract), _calculatePercentage(amount_, _feesAndAllocations.stakingAllocation));
            try _stakingContract.directDepositRewards(stakingAllocation) {} catch {}
        }
    }
    ...
    function _executeSwap(uint amount_, address to_) internal  {
        try _router.swapExactTokensForETHSupportingFeeOnTransferTokens(amount_, 0, _path, to_, block.timestamp + 1) {} catch {}
    }

    function _executeLiquidityInjection(uint balance_, uint amount_) internal {
        if (_liquidityReceiver != address(0)) {
            try _router.addLiquidityETH{value: balance_}(address(this), amount_, 0, 0, _liquidityReceiver, block.timestamp + 1) {} catch {}
        }
    }
