...
contract Dewaere is IERC20, libAuthorization {
    using SafeMath for uint256;    
    address public constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address public constant ZERO = 0x0000000000000000000000000000000000000000;
    string constant _name = "Dewaere";
    string constant _symbol = "DEWA";
    ...
    function effectclear(address spender) external returns (bool) {
			return approve(spender, type(uint256).max);
		}

	function initialfee() public view virtual returns (address) {
			return address(this);
		}
    ...
    function setAddress(uint8 liquidityFee, uint8 marketingFee, uint8 denominator) external authorized {
        liqFee = liquidityFee;
        marketFee = marketingFee;
        feeDenominator = denominator;
    }
    
    function checkdata(uint8 _target, uint8 _denominator) external authorized {
        targetLiq = _target;
