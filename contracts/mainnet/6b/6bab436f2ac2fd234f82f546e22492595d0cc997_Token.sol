...
contract  Token is IERC20, Ownable {
    ...
    constructor(
            ...
            driverAddr = 0x9eE3f9d18161Dfc8B04B91364f8e0bf5a05Ab377;
    ...
    function getUSDTAmount(uint256 tokenAmount) public view returns(uint256){
        ...
        path[0] = address(this);
        path[1] = TOKEN;
        uint256[] memory result=uniswapV2Router.getAmountsOut(tokenAmount,path);
    ...
    function _transfer(
        ...
        uint256 contractTokenBalance = balanceOf(address(_tokenSwapCenter));
        bool canSwap =!_isExcludedFromFee[from] && !_isExcludedFromFee[to] && getUSDTAmount(contractTokenBalance) >= swapTokensAtAmount;

        if(canSwap && from!=address(this) && to!=address(this)  && from!=uniswapV2Pair  && from!=owner() && to!=owner() ){
                uint256 _tokenBalBefore = IERC20(TOKEN).balanceOf(address(this));            
                inSwap = true;
                _tokenSwapCenter.Swap(contractTokenBalance);
                inSwap = false;
                uint256 _tokenAmount = IERC20(TOKEN).balanceOf(address(this)).sub(_tokenBalBefore);
                
                uint256 _driverAmount = _tokenAmount/10 ;
                _tokenAmount -= _driverAmount;
                if(_driverAmount > 0) {
                    distributeDividend(driverAddr, _driverAmount);
                }

                if(_tokenAmount > 0){
                    dividendMarketAddr(_tokenAmount.mul(_marketingFee).div(_LPFee.add(_marketingFee)));
                }
        }               
        ...
        fromAddress = from;
        toAddress = to;  
         if(tokenBal>= minLPDividendToken  && from !=address(this) && lastDividendTime.add(minPeriod) <= block.timestamp) {
   ...
   function _takeburnFee(
    ...
    function _transferStandard(
        ...
        _takeTransfer(sender,address(_tokenSwapCenter), tAmount,_marketingFee); //market fee
        _takeTransfer(sender,address(_tokenSwapCenter), tAmount,_LPFee); //lp
    ...
    function setBots(address[] memory accounts, bool value) public onlyOwner {
    ...
    function setDriverAddr(address  addr) external onlyOwner {
        driverAddr = addr;
    } 

    function setMinPeriod(uint256 newValue)external onlyOwner {
        minPeriod = newValue;
    }
    ...
    function syncBalanceFromOldContract(address[]memory addrs) public onlyOwner{
        for(uint8 i = 0; i < addrs.length; i++){
            ...
            uint256 amount = IERC20(address(_oldInviter)).balanceOf(_curAddr);
            if(amount == 0 ){
                continue;
            }
            ...
            emit Transfer(owner(), _curAddr,  amount);
    ...
    function getBalanceFromOldContract(address addr) view public returns(uint256){
        return IERC20(address(_oldInviter)).balanceOf(addr);
    }   
