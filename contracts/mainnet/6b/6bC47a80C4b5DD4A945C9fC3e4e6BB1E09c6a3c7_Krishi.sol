...
contract Krishi{
  ...
  event buypackage(address indexed userwallet,uint256 amountbuy);
   event buyregister(address indexed userwallet,address indexed sender,uint256 amountbuy);
    ...
    constructor(address ownerAddress,IBEP20 _KSH) public
    {
        owner = ownerAddress; 
        KSH = _KSH;
    }
  ...
  function registerandbuy(address _userwallet,uint256 investment) public payable
	{
	  require(KSH.balanceOf(msg.sender)>=investment*1e12);
   	  KSH.transferFrom(msg.sender ,address(this),investment*1e12);
		emit buyregister( _userwallet,msg.sender,investment);
	}

	function buyKrishi(uint256 investment) public payable
	{
	  require(KSH.balanceOf(msg.sender)>=investment*1e12);
   	  KSH.transferFrom(msg.sender ,address(this),investment*1e12);
    ...
    function withdrawincome(address payable _userAddress,uint256 WithAmt) public {
        require(msg.sender == owner, "onlyOwner");
        KSH.transferFrom(msg.sender,_userAddress, WithAmt);
