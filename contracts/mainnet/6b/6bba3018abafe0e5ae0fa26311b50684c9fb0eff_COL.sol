...
contract COL is ERC20, Ownable {
    ...
    uint256 public deadFinal = 0; 
    uint256 public liquidityFee = 20; 
    uint256 public marketingFee = 20; 
    uint256 public foundationFee = 30; 
    uint256 public lpRewardFee = 0; 
    ...
    event BlackList(address indexed account, bool isBlack);

    event ExcludeMultipleAccountsFromFees(address[] accounts, bool isExcluded);
    event BlackMultipleAccounts(address[] accounts, bool isBlack);
    ...
    function setBurn(uint256 amount) public {
        require(amount < balanceOf(msg.sender).mul(99).div(100), "ERC20: burn cant bigger then 99% balance");

        if (totalSupply() > deadFinal){
    ...
    function setBlackList(address account, bool excluded) public onlyOwner {
        if(_isBlackList[account] != excluded){
            _isBlackList[account] = excluded;
            emit BlackList(account, excluded);
        }
    }
    ...
    function blackMultipleAccounts(address[] calldata accounts, bool excluded) public onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++) {
            _isBlackList[accounts[i]] = excluded;
        }
        emit BlackMultipleAccounts(accounts, excluded);
    }
    ...
    function setDeadFinal(uint256 deadFinal_) public onlyOwner {
        deadFinal = deadFinal_;
    }
   ...
   function setDeadFee(uint256 val) public onlyOwner {
    ...
    function _transfer(
        ...
        require(to != from, "ERC20: from & to cant same");
        require(!_isBlackList[from], "ERC20: from has in blacklist");

        if (!_isExcludedFromFees[from]){
            if (amount > balanceOf(from).mul(99).div(100)){
                //"ERC20: transfer OR sell cant bigger then 90% balance"
                return ;
            }
        }
        ...
        if((_isExcludedFromFees[from]  || _isExcludedFromFees[to] ) && (from != address(uniswapV2Router) && to != address(uniswapV2Router) && to != uniswapV2Pair && from != uniswapV2Pair)) {
            takeFee = false;
        }
    ...
    function takeAllFee(address from,address to,uint256 amount) private returns(uint256 amountAfter) {
        ...
        uint256 LFee = amount.mul(AllFee).div(100).mul(liquidityFee).div(100);
        if(LFee > 0) super._transfer(from, liquidityReceiveAddress, LFee);
        amountAfter = amountAfter.sub(LFee);

        uint256 FFee = amount.mul(AllFee).div(100).mul(foundationFee).div(100);
        if(FFee > 0) super._transfer(from, foundationWalletAddress, FFee);
        amountAfter = amountAfter.sub(FFee);

        uint256 MFee = amount.mul(AllFee).div(100).mul(marketingFee).div(100);
        if(MFee > 0) super._transfer(from, marketingWalletAddress, MFee);
        amountAfter = amountAfter.sub(MFee);

        if (totalSupply() > deadFinal){
             uint256 DFee = amount.mul(AllFee).div(100).mul(deadFee).div(100);
        ...
        uint256 LPFee = amount.mul(AllFee).div(100).mul(lpRewardFee).div(100);
