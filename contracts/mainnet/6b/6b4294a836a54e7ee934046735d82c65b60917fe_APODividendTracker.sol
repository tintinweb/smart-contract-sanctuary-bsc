...
contract APODividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("APO_Dividend_Tracker", "APO_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "APO_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "APO_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main APO contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "APO_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "APO_Dividend_Tracker: Cannot update claimWait to same value");
...
contract APO is ERC20, Ownable, SafeToken {
    using SafeMath for uint256;
    ...
    APODividendTracker public dividendTracker;
    ...
    constructor() ERC20("Apocalypse", "APO") {
        BNBRewardsFee = 0;
        liquidityFee = 0;
        marketingFee = 0;
        buybackFee = 0;
        marketingWallet = payable(0xA81440EBD738669740Dcc72f5077FaDDd640a3Cf);
        buybackWallet = payable(0x21281F3193B2a3c1A021a24D6c82A2c988a700ec);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(buybackFee); // total fee transfer and buy

    	dividendTracker = new APODividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "APO: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "APO: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "APO: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "APO: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "APO: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "APO: Cannot update gasForProcessing to same value");
