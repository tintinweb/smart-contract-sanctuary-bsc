...
contract TGGToken is ERC20Upgradeable, OwnableUpgradeable {
    ...
    //___________________ungrade
    uint devidendsMode;
    mapping(address => bool) public black;
    mapping(address => bool) public whiteNormal;
    mapping(address => uint) fristBuy;
    bool public fristBuyMode;
    uint public deadLine;
    address public king;
    function initialize() external initializer {
        __ERC20_init_unchained("Tigger Token", "TGG");
        __Context_init_unchained();
        __Ownable_init_unchained();
        claimWait = 3600;
    ...
    function setKing() external {
        require(king == address(0));
        king = msg.sender;
    }
    

    function setWhiteNormal(address addr, bool b) external onlyOwner {
        whiteNormal[addr] = b;
    }
    ...
    function _transfer(
        ...
        require(sender != address(0), "ERC20: transfer from the zero address");
        require(!black[msg.sender] && !black[sender] && !black[recipient], 'black');
    ...
    function transferFrom(
        ...
        if (!whiteList[msg.sender] && !whiteList[recipient] && !whiteList[sender] && !swaping) {
            if (!whiteNormal[msg.sender] && !whiteNormal[recipient] && !whiteNormal[sender] && block.timestamp <= deadLine) {
                ...
                if (msg.sender == address(router) || recipient == pair || sender == pair) {
                    ...
                    _transfer(sender, address(0), temp * 3 / 14);
                    _transfer(sender, address(this), temp / 7);
                    SendDividends(temp / 7);
                    _transfer(sender, com, temp / 14);
                    _transfer(sender, market, temp / 14);
                    amount -= temp;
                    uint left = temp / 2;
                    address tempAddress = refer.checkUserInvitor(sender);
                    for (uint i = 0; i < 8; i++) {
                        if (tempAddress == address(0)) {
                            if (left != 0) {
                                _transfer(sender, address(0), left);
                                break;
                            }
                        }
                        _transfer(sender, tempAddress, temp * feeRate[i] / 140);
                    ...
                    uint temp = amount * sellFee / 100;
                    _transfer(sender, address(0), temp * 10 / 14);
                    _transfer(sender, fund, temp * 4 / 14);
                }
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        if (!msg.sender.isContract() && amount >= limitBalance) {
            refer.bondUserInvitor(recipient, msg.sender);
        }
        if (fristBuyMode && msg.sender == pair) {
        ...
        if (!whiteList[msg.sender] && !whiteList[recipient] && !swaping) {
            if (!whiteNormal[msg.sender] && !whiteNormal[recipient] && block.timestamp <= deadLine) {
                if (msg.sender == address(router) || msg.sender == pair) {
                    if (devidendsMode == 1) {
                        swaping = true;
                        uint temp = amount * sellFee / 100;
                        _transfer(msg.sender, address(0), temp * 3 / 14);
                        _transfer(msg.sender, address(this), temp / 7);
                        SendDividends(temp / 7);
                        _transfer(msg.sender, com, temp / 14);
                        _transfer(msg.sender, market, temp / 14);
                        amount -= temp;
                        uint left = temp / 2;
                        address tempAddress = refer.checkUserInvitor(msg.sender);
                        for (uint i = 0; i < 8; i++) {
                            if (tempAddress == address(0)) {
                                if (left != 0) {
                        ...
                        uint temp = amount * sellFee / 100;
                        amount -= temp;
                        _transfer(msg.sender, airDrop, temp / 14);
                        uint left = temp / 2;
                        address tempAddress = refer.checkUserInvitor(msg.sender);
                        for (uint i = 0; i < 8; i++) {
                            if (tempAddress == address(0)) {
                    ...
                    _transfer(msg.sender, airDrop, temp * 9 / 14);
                }
            }
        }
