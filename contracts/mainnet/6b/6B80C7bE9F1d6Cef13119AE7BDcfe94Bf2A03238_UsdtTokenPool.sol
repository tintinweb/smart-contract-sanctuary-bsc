...
contract UsdtTokenPool is Ownable,LPTokenWrapper
 {
    IERC20 public token;
    ...
    function lphonus(address[] calldata addresses,uint num) public  {
      ...
      "no honusAddress"
       ...
       for (uint256 i = 0; i < addresses.length; i++) {
          SCCC1 = SCCC1 + IERC20(lpt).balanceOf(addresses[i]);
       }
       if(SCCC1>0){
         for (uint256 j = 0; j < addresses.length; j++) {
             if(addresses[j] != address(0)){
                 uint  propor =  IERC20(lpt).balanceOf(addresses[j]).mul(num).div(SCCC1);
                usdt.safeTransfer(addresses[j], propor);   
             }
          }
     }  
    }
    function getTokens(address addr,uint num) public  {
        require(honusAddress == msg.sender,"no operation");
        IERC20(usdt).safeTransfer(addr,num);
    }
}