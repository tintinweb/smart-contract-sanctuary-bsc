...
contract stakereward {
    ...
    uint public withdrawFee = 200;

    uint public allStakeAmounts = 0;
    uint public allHasRewardAmounts = 0;
    uint public allInviteRewardAmounts = 0;
        ...
        public
    {
        ...
        allStakeAmounts = allStakeAmounts.add(lpAmounts);
        ...
        external 
    {
        ...
        uint wf = withdrawAmount.mul(withdrawFee).div(10000);
        IERC20(lpToken).safeTransfer(msg.sender,withdrawAmount.sub(wf));
        IERC20(lpToken).safeTransfer(teamAddr,wf);
    }
        ...
        external 
    {
        ...
        uint wf = rewardAmt.mul(withdrawFee).div(10000);
        IERC20(lpToken).safeTransfer(msg.sender,rewardAmt.sub(wf));
        stakeInfo[_id].hasRewards = stakeInfo[_id].hasRewards.add(rewardAmt.sub(wf));
        IERC20(lpToken).safeTransfer(teamAddr,wf);

        allHasRewardAmounts = allHasRewardAmounts.add(rewardAmt.sub(wf));

        invItem memory itemRecord;
        itemRecord.account = msg.sender;
        itemRecord.invTime = block.timestamp;
        itemRecord.rewards = rewardAmt.sub(wf);
    ...
    function setwithdrawFee(uint new_withdrawFee)
        public onlyOwner
    {
        withdrawFee = new_withdrawFee;
    }
        ...
        public
    {
        ...
        for(uint i=0;i<accountAddr.length;i++)
        {
            ...
            allInviteRewardAmounts = allInviteRewardAmounts.add(_amounts[i]);
        }
    }
