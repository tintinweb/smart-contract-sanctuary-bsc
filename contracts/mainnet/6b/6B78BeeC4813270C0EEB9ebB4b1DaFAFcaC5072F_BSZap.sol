...
library BSLibrary {
        ...
        returns (IERC20 token0, IERC20 token1)
    {
        ...
        require(address(token0) != address(0), "BSLibrary: ZERO_ADDRESS");
    }
        ...
        )
    {
        ...
        uint256 vReserve0;
        uint256 vReserve1;
        (reserve0, reserve1, vReserve0, vReserve1, feeInPrecision) = IBSPool(pool).getTradeInfo();
    ...
    function getReserves(
        ...
        (IERC20 token0, ) = sortTokens(tokenA, tokenB);
        (uint256 reserve0, uint256 reserve1) = IBSPool(pool).getReserves();
    ...
    function quote(
        ...
        require(reserveA > 0 && reserveB > 0, "BSLibrary: INSUFFICIENT_LIQUIDITY");
    ...
    function getAmountOut(
        ...
        require(reserveIn > 0 && reserveOut > 0, "BSLibrary: INSUFFICIENT_LIQUIDITY");
        ...
        require(reserveOut > amountOut, "BSLibrary: INSUFFICIENT_LIQUIDITY");
    }
    ...
    function getAmountIn(
        ...
        require(reserveIn > 0 && reserveOut > amountOut, "BSLibrary: INSUFFICIENT_LIQUIDITY");
...
// File: contracts/periphery/BSZap.sol
...
contract BSZap {
    ...
    mapping(IBSFactory => bool) public whitelistedFactories;
    address public immutable weth;

    event ConfigMasterUpdated(address oldConfigMaster, address newConfigMaster);
    event FactoryAdded(IBSFactory factory);
    event FactoryRemoved(IBSFactory factory);

    modifier onlyConfigMaster() {
        require(msg.sender == configMaster, "BSZap: Forbidden");
        _;
    }

    modifier whitelistedFactory(IBSFactory _factory) {
        require(whitelistedFactories[_factory], "BSZap: Forbidden");
        _;
    }

    modifier ensure(uint256 deadline) {
        require(deadline >= block.timestamp, "BSZap: Expired");
        _;
    }

    constructor(IBSFactory _factory, address _weth) public {
    ...
    function addFactory(IBSFactory _factory) external onlyConfigMaster {
        whitelistedFactories[_factory] = true;
        emit FactoryAdded(_factory);
    }

    function removeFactory(IBSFactory _factory) external onlyConfigMaster {
    ...
    function zapInEth(
        ...
        lpQty = IBSPool(pool).mint(to);
        require(lpQty >= minLpQty, "IBSRouter: INSUFFICIENT_MINT_QTY");
    }
    ...
    function calculateZapOutAmount(
        ...
        amountOut += BSLibrary.getAmountOut(
            amountIn,
            data.rIn,
            data.rOut,
            data.vIn,
    ...
    function calculateSwapAmounts(
        ...
        require(factory.isPool(tokenIn, tokenOut, pool), "INVALID_POOL");
        (uint256 rIn, uint256 rOut, uint256 vIn, uint256 vOut, uint256 feeInPrecision) = BSLibrary
            .getTradeInfo(pool, tokenIn, tokenOut);
        amountSwap = _calculateSwapInAmount(rIn, rOut, vIn, vOut, feeInPrecision, userIn);
        amountOutput = BSLibrary.getAmountOut(amountSwap, rIn, rOut, vIn, vOut, feeInPrecision);
    }
    ...
    function _swap(
        ...
        (IERC20 token0, ) = BSLibrary.sortTokens(tokenIn, tokenOut);
        (uint256 amount0Out, uint256 amount1Out) = tokenIn == token0
            ? (uint256(0), amountOut)
            : (amountOut, uint256(0));
        IBSPool(pool).swap(amount0Out, amount1Out, to, new bytes(0));
    }
    ...
    function _zapOut(
        ...
        uint256 amountIn;
        {
            ...
            (uint256 amount0, uint256 amount1) = IBSPool(pool).burn(address(this));
            (IERC20 token0, ) = BSLibrary.sortTokens(tokenIn, tokenOut);
        ...
        uint256 swapAmount;
        {
            (
                ...
                uint256 vOut,
                uint256 feeInPrecision
            ) = BSLibrary.getTradeInfo(pool, tokenIn, tokenOut);
            swapAmount = BSLibrary.getAmountOut(amountIn, rIn, rOut, vIn, vOut, feeInPrecision);
        }
        ...
        )
    {
        ...
        (data.rIn, data.rOut, data.vIn, data.vOut, data.feeInPrecision) = BSLibrary.getTradeInfo(
            pool,
            tokenIn,
            tokenOut
        );
        uint256 totalSupply = _calculateSyncTotalSupply(factory, IBSPool(pool), data);
        bool isAmpPool = (IBSPool(pool).ampBps() != 10000);
        // calculate amountOut
