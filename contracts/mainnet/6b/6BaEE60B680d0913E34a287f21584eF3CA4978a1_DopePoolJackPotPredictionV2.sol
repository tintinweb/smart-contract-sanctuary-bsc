...
/**
 * @title DopePoolJackPotPredictionV2
 */
contract DopePoolJackPotPredictionV2 is Ownable, Pausable, ReentrancyGuard {
     ...
     * @param epoch: epoch
     */
    function betBear(uint256 epoch, address _upline) external payable whenNotPaused nonReentrant notContract {
        require(epoch == currentEpoch, "Bet is too early/late");
        require(_bettable(epoch), "Round not bettable");
        require(msg.value >= minBetAmount, "Bet amount must be greater than minBetAmount");
        require(ledger[epoch][msg.sender].amount == 0, "Can only bet once per round");
        uint256 refAmount = msg.value * refFee / 10000;
        refBonus[_upline] += refAmount;
        payable(_upline).transfer(refAmount);
        // Update round data
        uint256 amount = msg.value * (10000 - refFee) / 10000;
     ...
     * @param epoch: epoch
     */
    function betBull(uint256 epoch, address _upline) external payable whenNotPaused nonReentrant notContract {
        require(epoch == currentEpoch, "Bet is too early/late");
        require(_bettable(epoch), "Round not bettable");
        require(msg.value >= minBetAmount, "Bet amount must be greater than minBetAmount");
        require(ledger[epoch][msg.sender].amount == 0, "Can only bet once per round");
    ...
    function setRefFee(uint256 _refFee) external whenPaused onlyAdmin {
        refFee = _refFee;
    }
