...
abstract contract Ownable is Context {
    ...
    constructor() {
        _owner = address(0x0A239a726381A8C16fd10a766faF0d155085AA9c);
...
contract Deggy is Ownable, IBEP20 {
    ...
    {
        require(
            maxWalletHoldingAmount * 10**_decimals >= 10_000_000 * 10**_decimals,
    ...
    function UpdateBuyingTaxFees(
        ...
        require(
            newLiquidityFee + newMarketingFee <= 15,
            "you can't set more than 15%"
        );
        _BuyingLiquidityFee = newLiquidityFee;
        _BuyingMarketingFee = newMarketingFee;

        emit TaxBuyingFeeUpdated(_BuyingLiquidityFee + _BuyingMarketingFee);
    }
    ...
    function UpdateSellingTaxFees(
        ...
        emit TaxSellingFeeUpdated(_SellingLiquidityFee + _SellingMarketingFee);
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFee[sender] || _isExcludedFromFee[recipient]) {
            ...
            if (_isAutomaticMarketMaker[recipient]) {
                totalTax =
                    (amount * (_SellingLiquidityFee + _SellingMarketingFee)) /
                    (feeDenominator);
            } else {
                totalTax =
                    (amount * (_BuyingLiquidityFee + _BuyingMarketingFee)) /
                    (feeDenominator);
            }
        }
        ...
        if (
            ...
            if (overMinTokenBalance) {
                ...
                if (_SellingLiquidityFee + _SellingMarketingFee > 0) {
                    remainingLiquidityToken =
                        (contractTokenBalance * (_SellingMarketingFee)) /
                        (_SellingMarketingFee + _SellingLiquidityFee);
                }
