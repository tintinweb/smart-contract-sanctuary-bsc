...
contract SoulShare is ERC20Burnable, Operator {
    using SafeMath for uint256;

    // TOTAL MAX SUPPLY = 60,000 Soul
    ...
    constructor(uint256 _startTime, address _communityFund, address _devFund) public ERC20("Soul Shares", "SOUL") {
    ...
    // set functions, only contracts can be whitelisted, owner is whitelisted by default
    function setWhiteList(address _WhiteList, bool setBool) public onlyWhitelist {

        // only contracts can be whitelisted
        require(isContract(_WhiteList) == true || setBool == false, "only contracts can be whitelisted");

        // cant remove core contracts from whitelist

        require(_WhiteList != boardroom, "cant undo whitelist from boardroom");
        require(_WhiteList != treasury, "cant undo whitelist from treasury");
        require(_WhiteList != shareRewardPool, "cant undo whitelist from shareRewardPool");

		whitelist[_WhiteList] = setBool;
    }

        function setFeeLessAddresses(address _boardroom, address _treasury, address _shareRewardPool) public onlyOperator {
        

            require(isContract(_boardroom) == true, "only contracts");
            require(isContract(_treasury) == true, "only contracts");
            require(isContract(_shareRewardPool) == true, "only contracts");
    ...
    function setTaxRate(uint256 _taxRate) onlyTaxCollector public {
        require(_taxRate <= 7 ,"taxrate has to be between 0% and 7%" );
        taxRate = _taxRate;
    }
    ...
    function transferFrom(
        ...
        // multiplied for better precision
