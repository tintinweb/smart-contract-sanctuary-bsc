...
contract CSCToken is ERC20, Ownable {
    ...
    uint256 private _totalSupply = 10000 * 10**6 * 10**_decimals;
    ...
    constructor() ERC20("CSC Token", "CSC") {
    ...
    function setAntiWhale(bool _isAntiWhale) external onlyOwner {
     ...
     function antiWhale(
        ...
        uint256 curTime = block.timestamp;
         if (antiWhaleStart < curTime && curTime < antiWhaleEnd && !whitelist[_recipient]) {
             // anti bot buy
        ...
        if (antiWhaleStart < curTime && curTime < antiWhaleEnd && !whitelist[_sender]) {
            // anti bot sell
    ...
    function setAntiWhaleTime(
        uint256 _antiWhaleStart,
        uint256 _antiWhaleEnd
        ) external onlyOwner {
