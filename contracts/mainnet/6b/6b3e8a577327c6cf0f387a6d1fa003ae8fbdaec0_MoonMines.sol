...
contract MoonMines is Context, Ownable {
    ...
    uint256 private MINERALS_TO_HIRE_1MINERS = 1080000;//for final version should be seconds in a day
    ...
    function hireMinerals(address ref) public {
        require(initialized);
        ...
        uint256 newMiners = SafeMath.div(mineralsUsed,MINERALS_TO_HIRE_1MINERS);
    ...
    function MineralsRewards(address adr) public view returns(uint256) {
        uint256 hasMinerals = getMyMinerals(adr);
        uint256 mineralValue = calculateMineralSell(hasMinerals);
        return mineralValue;
    }
    
    function buyMinerals(address ref) public payable {
        require(initialized);
        uint256 mineralsBought = calculateMineralBuy(msg.value,SafeMath.sub(address(this).balance,msg.value));
    ...
    function calculateMineralBuySimple(uint256 eth) public view returns(uint256) {
        return calculateMineralBuy(eth,address(this).balance);
    }
    ...
    function getMineralsSinceLastHire(address adr) public view returns(uint256) {
        uint256 secondsPassed=min(MINERALS_TO_HIRE_1MINERS,SafeMath.sub(block.timestamp,lastHire[adr]));
