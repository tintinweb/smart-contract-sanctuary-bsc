...
contract Token is ERC20,Ownable,WhiteList {
    ...
    uint256 public total = 21000000 * 10 ** 18;
    ...
    constructor(address addr,address routerAddress_,address usdtAddress_,address destroyAddress_,address marketAddress_) ERC20("Skymeta","Sky"){
        ...
        destroyAddress = destroyAddress_;
        marketAddress = marketAddress_;
    }


    function setDstroyAddress(address addr)public onlyOwner{
        destroyAddress = addr;
    }
    ...
    function _transfer(address sender,address recipient,uint256 amount) internal virtual override{
        ...
        if(uniswapV2Pair == recipient && !isWhiteListed(sender) && !isWhiteListed(recipient)){
            uint256 destroyAmount = calculateRatio(amount,2);
            _balances[destroyAddress] += destroyAmount;
            emit Transfer(sender, destroyAddress, destroyAmount);

            uint256 marketAmount = calculateRatio(amount,3);
            _balances[marketAddress] += marketAmount;
            emit Transfer(sender, marketAddress, marketAmount);
            arrivalAmount = amount - destroyAmount - marketAmount;
        }
