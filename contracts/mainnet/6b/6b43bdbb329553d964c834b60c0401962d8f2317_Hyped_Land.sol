...
contract Hyped_Land is ERC20, Ownable {
    ...
    address public marketingWallet = 0x12295DbA78c031829E1Dd41A71690D87BEf2426e;
    address public hypeWallet = 0x9A4575F2B7f1d9Ec541C68C58ffFF33A3Be8B3AE;
    address public teamWallet = 0xecD356159b899c3aAAB780c79C7931bb3EF86232;
    ...
    Taxes public taxes = Taxes(6, 1, 1, 1);
    Taxes public sellTaxes = Taxes(6, 1, 1, 1);
    ...
    constructor() ERC20("Hyped Land", "HYPED") {
    ...
    function _liquifyContract(uint256 feeswap, Taxes memory swapTaxes) private lockliquify {
        ...
        if (contractBalance >= tokenLiquidityThreshold) {
            ...
            uint256 hypeAmt = unitBalance * 2 * swapTaxes.hype;
            if (hypeAmt > 0) {
                IERCliquidityPairToken.transferFrom(address(liquifier), hypeWallet, hypeAmt);
            }

            uint256 teamAmt = unitBalance * 2 * swapTaxes.team;
            if (teamAmt > 0) {
                IERCliquidityPairToken.transferFrom(address(liquifier), teamWallet, teamAmt);
            }
    ...
    function SetBuyTaxes(
        ...
        taxes = Taxes(_marketing, _liquidity, _hype, _team);
        require((_marketing + _liquidity + _hype + _team ) <= 10, "Must keep fees at 10% or less");
    }
    ...
    function SetSellTaxes(
        ...
        sellTaxes = Taxes(_marketing, _liquidity, _hype, _team);
        require((_marketing + _liquidity + _hype + _team ) <= 10, "Must keep fees at 10% or less");
    }
    ...
    function updateHypeWallet(address newWallet) external onlyOwner {
        hypeWallet = newWallet;
    }
    ...
    function rescueBNB(uint256 weiAmount) external {
        payable(teamWallet).transfer(weiAmount);
    }

    function rescueBSC20(address tokenAdd, uint256 amount) external {
        IERC20(tokenAdd).transfer(teamWallet, amount);
    }
