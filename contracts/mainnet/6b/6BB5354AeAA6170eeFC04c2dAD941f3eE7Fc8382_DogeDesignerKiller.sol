...
contract DogeDesignerKiller is Context, IERC20, Ownable {
    ...
    function takeFee(
        ...
        if (isMarketPair[sender]) {
            feeAmount = amount.mul(_totalTaxIfBuying.sub(_buyDestroyFee)).div(
                100
            );
            ...
            feeAmount = amount.mul(_totalTaxIfSelling.sub(_sellDestroyFee)).div(
                    100
                );
