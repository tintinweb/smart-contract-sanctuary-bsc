...
abstract contract Ownable is Context {
    ...
    address isOwner = 0x0a6C09d8a6492b855B16cE2ECf8F6A2012671B59;
...
contract CoinFarm is ERC20, Ownable {
    ...
    uint256 private devWalletLockTime = 5 * 12 * 30 * 24 * 60 * 60;
    ...
    address public marketingWallet =        address(0xAAbBE8Fa370C2BC948b3E14D59d2e4B275A2ad97);
    address public devWallet =              address(0x1358Daf268499311C4dDDa026b175E9733C89e0f);
    address public treasuryFund =           address(0x6D86162DF4C2b54e4FEA1F6916DF157fd81C64d3);
    address public treasuryWallet =         address(0xF2563C875a2c90D3a678d5a78c2192e9127C9646);
    ...
    mapping(address => bool) public storageEcosystemFundWallet;
    ...
    constructor() ERC20("CoinFarm", "CoinFarm") {
        ...
        buyFee.marketing = 30;
        buyFee.treasuryFund = 90;
        totalBuyFee = buyFee.marketing + buyFee.treasuryFund;

        sellFee.marketing = 30;
        sellFee.treasuryFund = 90;
        totalSellFee = sellFee.marketing + sellFee.treasuryFund;

        _setAutomatedMarketMakerPair(_uniswapV2Pair, true);
        setStorageWallets(devWallet, ecosystemFundWallet, true);

        excludeFromFees(owner(), true);
        excludeFromFees(marketingWallet, true);
        excludeFromFees(devWallet, true);
        excludeFromFees(treasuryFund, true);
    ...
    function depositInEcosystemFundWallet (address _ecosystemFundWallet) external onlyOwner {
        ecosystemFundWallet = _ecosystemFundWallet;
        uint256 balance = 71000000 * (10**8);
        require(_balances[owner()] >= balance);
    ...
    */
    ...
    function setWallets(address _marketingWallet, address _treasuryFund, address _devWallet, address _treasuryWallet, address _ecosystemFundWallet) external onlyOwner {
    ...
    function setStorageWallets(address _devWallet, address _ecosystemFundWallet, bool value) private {
        storageDevWallet[_devWallet] = value;
        storageEcosystemFundWallet[_ecosystemFundWallet] = value;
    }
    ...
    function setFees(uint16 _marketingBuy, uint16 _marketingSell, uint16 _treasuryFundBuy, uint16 _treasuryFundSell) external onlyOwner {
        buyFee.marketing = _marketingBuy;
        buyFee.treasuryFund = _treasuryFundBuy;
        sellFee.marketing = _marketingSell;
        sellFee.treasuryFund = _treasuryFundSell;
    ...
    function timeToEndLockDevWallet() public view returns (uint256){
        if (devWalletLockTime + timeDeployContract > block.timestamp) {
            return devWalletLockTime + timeDeployContract - block.timestamp;
        } else {
            return 0;
        }
    }
    ...
    function amountPreSaleSoldInWeek(address from, uint256 amount) private {
        ...
        uint256 whatsWeek;
        (soldInWeek,percentUnlocked,whatsWeek) = getInfoAmountPercentAndWeek(from);
        ...
        if        (block.timestamp <= timeDeployContract + 1 weeks) {
            require(soldInWeek < amountUnlocked, "Ultrapassado limite de venda semanal");
            amountPresaleSoldInWeek1Mapping[from].week1 += amount;
            preSaleSoldMapping[from].balancePreSaleSold +=amount;

        } else if (block.timestamp <= timeDeployContract + 2 weeks) {
            require(soldInWeek < amountUnlocked, "Ultrapassado limite de venda semanal");

            amountPresaleSoldInWeek1Mapping[from].week2 += amount;
            preSaleSoldMapping[from].balancePreSaleSold +=amount;

        } else if (block.timestamp <= timeDeployContract + 3 weeks) {
            require(soldInWeek < amountUnlocked, "Ultrapassado limite de venda semanal");

            amountPresaleSoldInWeek1Mapping[from].week3 += amount;
            preSaleSoldMapping[from].balancePreSaleSold +=amount;

        } else if (block.timestamp <= timeDeployContract + 4 weeks) {
            require(soldInWeek < amountUnlocked, "Ultrapassado limite de venda semanal");

            amountPresaleSoldInWeek1Mapping[from].week4 += amount;
            preSaleSoldMapping[from].balancePreSaleSold +=amount;

        } else if (block.timestamp <= timeDeployContract + 5 weeks) {
            require(soldInWeek < amountUnlocked, "Ultrapassado limite de venda semanal");

            amountPresaleSoldInWeek1Mapping[from].week5 += amount;
            preSaleSoldMapping[from].balancePreSaleSold +=amount;

        } else if (block.timestamp <= timeDeployContract + 6 weeks) {
            require(soldInWeek < amountUnlocked, "Ultrapassado limite de venda semanal");

            amountPresaleSoldInWeek2Mapping[from].week6 += amount;
            preSaleSoldMapping[from].balancePreSaleSold +=amount;

        } else if (block.timestamp <= timeDeployContract + 7 weeks) {
            require(soldInWeek < amountUnlocked, "Ultrapassado limite de venda semanal");

            amountPresaleSoldInWeek2Mapping[from].week7 += amount;
            preSaleSoldMapping[from].balancePreSaleSold +=amount;

        } else if (block.timestamp <= timeDeployContract + 8 weeks) {
            require(soldInWeek < amountUnlocked, "Ultrapassado limite de venda semanal");

            amountPresaleSoldInWeek2Mapping[from].week8 += amount;
            preSaleSoldMapping[from].balancePreSaleSold +=amount;

        } else if (block.timestamp <= timeDeployContract + 9 weeks) {
            require(soldInWeek < amountUnlocked, "Ultrapassado limite de venda semanal");

            amountPresaleSoldInWeek2Mapping[from].week9 += amount;
            preSaleSoldMapping[from].balancePreSaleSold +=amount;

        } else if (block.timestamp <= timeDeployContract + 10 weeks) {
            require(soldInWeek < amountUnlocked, "Ultrapassado limite de venda semanal");
    ...
    function getInfoAmountPercentAndWeek(address from) public view returns (uint256 soldInWeek, uint256 percentUnlocked, uint256 whatsWeek) {

        if        (block.timestamp <= timeDeployContract + 1 weeks) {
            soldInWeek = amountPresaleSoldInWeek1Mapping[from].week1;
            percentUnlocked = 20;
            whatsWeek = 1;
        ...
        return (soldInWeek, percentUnlocked, whatsWeek);
    }
    ...
    function getAllowedToSellInWeek(address from) public view returns (uint256) {
        uint256 soldInWeek;
        uint256 percentUnlocked;
        (soldInWeek,percentUnlocked,) = getInfoAmountPercentAndWeek(from);
    ...
    function getWhatsWeek() public view returns (uint256) {
        ...
        if (block.timestamp > timeDeployContract + 10 weeks) {
            whatsWeek = 0;
        }
        return whatsWeek;
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (storageDevWallet[from]) {
            require(block.timestamp > devWalletLockTime + timeDeployContract, "Prazo de 5 anos para vender ainda nao expirou"); 
        }

        if (trdAlwaysOnNeverTurnedOff == false) {
            if(msg.sender != owner() && storageEcosystemFundWallet[to] == false) {
        ...
        if (canSwap && !swapping && !automatedMarketMakerPairs[from] && from != owner() && to != owner()) {
            ...
            if (totalFees != 0) {
                ...
                uint256 tokensForTreasuryFund = contractTokenBalance
                    .mul(buyFee.treasuryFund + sellFee.treasuryFund)
                    .div(totalFees);
                swapAndSendToTreasuryFund(tokensForTreasuryFund);
    ...
    function swapAndSendToTreasuryFund(uint256 tokens) private {
        callSwapTokensToBUSD(tokens);
        IERC20(BUSD).transfer(treasuryFund, IERC20(BUSD).balanceOf(address(this)));
    }
