...
contract Four01Programe is UUPSUpgradeable, OwnableUpgradeable {
    ...
    function getUserPercentageIn401Programe(address _address) public view returns (uint256){
        if(optInMap[_address]) return optInHoldersMap.get(_address);
        return 0;
    }

    function withdraw401KReward(address _account) public view returns(uint256 mAmount, uint256 four01kAmount, uint256 contributionLife ) {
        // withdraw process
    ...
    function resetUserStatus(address _account) public onlyOwner {
    ...
    function updateWithdrawTimeMap(uint256 index, uint256 _time) public onlyOwner {
    ...
    function export401kRewardList() public view returns (
