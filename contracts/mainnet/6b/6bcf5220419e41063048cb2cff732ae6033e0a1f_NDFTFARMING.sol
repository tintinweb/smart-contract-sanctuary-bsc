...
contract NDFTFARMING is ERC721, Ownable{
    ...
    constructor(address _Tokenaddress,address _admin) ERC721("NDFT FARMING", "NFT Farm"){
        Admin = _admin;
    ...
    function NFTFarming(uint256 amount,uint256 _pakages,string memory url,bytes memory signature) public returns(bool){
    ...
    event claimtoken(address user,uint256 amount,string orderid,uint256 claimtime);
    function claim(uint256 _amount,uint256 _timecount,string memory _islock,bytes memory _sign) public returns (bool) {
        ...
        ISlock[_islock] = true;
        emit claimtoken( msg.sender, _amount, _islock, block.timestamp);
        return true;
    }
