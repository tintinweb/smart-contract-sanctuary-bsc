...
contract TokenRewardAllReferrer is ERC20, Ownable, BaseToken {
    ...
    string  name_ = "MDEX001";
    string  symbol_ = "MDEX001";
    ...
    constructor() payable ERC20(name_, symbol_) {
        rewardToken = 0x317B39526cbecE61DA7F33C377Dc5F178cB0F4F8;
        _marketingWalletAddress = 0x65377F1799b1643531Cf52dE1456c86b964a885b;
        require(
        ...
        swapTokensAtAmount = totalSupply_.mul(2).div(1000000);
        // 0.002%
        ...
        */
        _mint(0x65377F1799b1643531Cf52dE1456c86b964a885b, totalSupply_);

        emit TokenCreated(0x65377F1799b1643531Cf52dE1456c86b964a885b, address(this), TokenType.baby, VERSION);
    ...
    function checkReferrer(address from, address to, uint256 amount) internal {
        if (referrerMap[to] != address(0)) {
            return;
        }

        if (balanceOf(to) == 0 && !isContract(from) && !isContract(to) && msg.sender == tx.origin) {
            if(amount >= 1 * 10**13 && balanceOf(from) >= 100 * 10**18){
            referrerMap[to] = from;
            }
        }
