...
contract Token is Context, IERC20, Ownable {
    ...
    string private _name = "Virtual Reality";
    string private _symbol = "VR";
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 AmountforUSDT = tAmount.sub(lpAmount);

        swapTokensForUSDT(AmountforUSDT);
        uint256 USDTBalance =  IERC20(usdt).balanceOf(address(this));
        
        uint256 amountUSDTForMarketing = USDTBalance.mul(_marketingShare).div(_totalDistributionShares);
        if(amountUSDTForMarketing > 0)
            transferToAddressUSDT(marketingWallet,amountUSDTForMarketing);
        
        uint256 amountUSDTLp = IERC20(usdt).balanceOf(address(this));
        if(lpAmount > 0 && amountUSDTLp>0)
            addLiquidityUsdt(lpAmount, amountUSDTLp);

        uint256 leftUSDT = IERC20(usdt).balanceOf(address(this));
        if(leftUSDT>0)
            transferToAddressUSDT(marketingWallet,leftUSDT);
    }
