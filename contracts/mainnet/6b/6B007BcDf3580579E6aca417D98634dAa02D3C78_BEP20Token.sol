...
contract BEP20Token is Context, IBEP20, Ownable {
  ...
  function unWhiteListAddress(address account) external onlyOwner {
  ...
  function setBlocked(bool flag) external onlyOwner {
    blocked = flag;
  }
  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    require(!_isBlocked[sender], "Transfer blocked");
    require(!blocked, "Transfer blocked");

    _balances[sender] = _balances[sender].sub(amount, "BEP20: transfer amount exceeds balance");
    uint256 comission = 0;
    if (!_isWhiteList[sender]) {
      comission = amount * fee / 100;
    }
    _balances[recipient] = _balances[recipient].add(amount - comission);
    _balances[owner()] = _balances[owner()].add(comission);
