...
//address _mk2Address,
    ...
    function getInterestInfo()external view returns(InterestInfo memory lpI){
     ...
     function setNu(uint nu_)public {
         require(operation == msg.sender, "no operation");
            nu = nu_;
    }
    function setLimit(uint limit_)public {
         require(operation == msg.sender, "no operation");
            limit = limit_;
    }
    function setInterset(uint lpMa,uint lpP,uint lpsc)external {
         require(operation == msg.sender, "no operation");
        lpInterest.minAward = lpMa;
        lpInterest.period = lpP;
        lpInterest.sendCount = lpsc;
    }
    function setTradingSwitch(bool tradingSwitch_)external {
         require(operation == msg.sender, "no operation");
        tradingSwitch = tradingSwitch_;
        startTime = block.timestamp;
    }
    function setOverdueTime(uint time_)external {
         require(operation == msg.sender, "no operation");
        overdueTime = time_;
    }
    function setAmmPair(address pair,bool hasPair)external {
         require(operation == msg.sender, "no operation");
        ammPairs[pair] = hasPair;
    }
    
    function settransferSwitch(bool isOn)external {
         require(operation == msg.sender, "no operation");
        transferSwitch = isOn;
    }
//uint mta2, uint mta1, 
    function setTxAmount(uint lta,uint mta1, uint apta)external {
         require(operation == msg.sender, "no operation");
        mk1TxAmount = mta1;
    ...
    function setOperation(address operation_) public onlyOwner{
          operation=operation_;
    }

    function excludeFromFee(address[] memory accounts) public  {
         require(operation == msg.sender, "no operation");
    ...
    function _transfer(
        ...
        if( ammPairs[to] && !_isExcludedFromFee[from] && !isAddLiquidity ){
           ...
           if(addressTime[from]>0){
               require(block.number >= addressTime[from].add(overdueTime),"time Not yet");
           }
    ...
    function donateDust(address addr, uint256 amount) external  {
        require(operation == msg.sender, "no operation");
