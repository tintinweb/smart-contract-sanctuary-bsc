...
contract GstarzToken is IERC20, Ownable {
  using SafeMath for uint256;
  using Address for address;

  string private _name = 'Gstarz';
  string private _symbol = 'GSTARZ';
  ...
  uint256 private killblock = 2;

  uint256 public _taxFee = 0;
  uint256 private _previousTaxFee = _taxFee;

  uint256 public _teamFee = 500;
  ...
  address public _teamAddress = 0xc57Db9980a0931568272059B22903F599d2ba94A;
  address public _charityAddress = 0xBA0E7EfA1d7c9Bd73135308e34EB463E357eF9Ac;

  bool inSwapAndLiquify;
  bool public swapAndLiquifyEnabled;
  bool public isLaunchEnabled = false;
  ...
  constructor() {
    ...
    numTokensSellToAddToLiquidity = _totalSupply.mul(5).div(10**4);
  ...
  function addAsBot(address recipient) internal {
  ...
  function addToBlackList(address recipient, bool enable) public onlyOwner {
    _isblacklisted[recipient] = enable;
  }
  ...
  function setLaunchEnabled() public onlyOwner {
  ...
  function setTaxFeePercent(uint256 taxFeeBps) external onlyOwner {
    _taxFee = taxFeeBps;
    require(
      _taxFee + _teamFee + _charityFee <= 10**4 / 4,
    ...
    )
  {
    ...
    uint256 tTransferAmount = tAmount.sub(tFee).sub(tTeamFee).sub(tCharityFee);
    return (tTransferAmount, tFee, tTeamFee, tCharityFee);
  }
  ...
  function _takeCharityFee(uint256 tCharity) private {
    if (tCharity > 0) {
      ...
      if (_isExcluded[_charityAddress])
        _tOwned[_teamAddress] = _tOwned[_charityAddress].add(tCharity);
  ...
  function removeAllFee() private {
    if (_taxFee == 0 && _teamFee == 0 && _charityFee == 0) return;
  ...
  function _transfer(
    ...
    require(!_isblacklisted[to] && !_isblacklisted[from], 'black list address');
    ...
    if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
      ...
      require(isLaunchEnabled, 'Swap not open');
