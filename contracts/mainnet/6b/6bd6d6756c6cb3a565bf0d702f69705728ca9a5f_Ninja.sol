...
contract Ninja is ERC721, ERC721Enumerable, ERC721URIStorage, ERC721Burnable, Ownable {
        ...
        IERC721 private migrationContract = IERC721(0x68591f34ac1c7cD8e8c9f0d847edE1E6006a6dC2);

        InjectedWallets[] public wallets;

        string private _baseURIextended = "https://rose-manual-scorpion-701.mypinata.cloud/ipfs/QmSAbbBExKxFhsk5693T4p5SGSRSMLoTvpZiYTHRMFYcMt/";
        string private  _name;                                                          
        string private  _symbol; 
        ...
        */
        ...
        /**
        * Quick Migrate from old contract
        * Cap too 500 total supply.
        */
        function migrateAirdrop(uint start, uint length) external onlyOwner() {
             require(totalSupply() <= _max_supply, "NFT: Supply Exeded Cap");
             for(uint256 i = start; i < length; i++) {
                _safeMint(migrationContract.ownerOf(i), i);
                _setTokenURI(i, string(abi.encodePacked(_baseURI(), Strings.toString(i), ".json")));             
             }
        }
