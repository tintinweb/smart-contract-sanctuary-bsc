...
/*
This is the official contract of the Lucky Seven coin. This coin implements an innovative jackpot mechanism called the Luckypot.

Every buy and sell will feed the Luckypot (5%). If for 7 mins, no buys are
recorded, the last buyer will receive a portion of the Luckypot. This will drive
a consistent buy pressure.

The Luckypot has a hard limit ($7K) that, if reached, will trigger the Lucky Hit event. A portion
of the Luckypot will be cashed out to the buyback wallet. The buyback wallet will
then either burn the tokens or dedicate a portion of it towards staking.

Website: https://www.luckysevencoin.com
Telegram: https://t.me/luckysevencoin
Twitter: https://twitter.com/luckysevencoin
Instagram: https://instagram.com/luckysevencoin
...
contract LuckySeven is Context, IERC20, Ownable {
    ...
    // At any given time, buy and sell fees can NOT exceed 49% each
    uint256 private constant TOTAL_FEES_LIMIT = 4900;
    ...
    // Luckypot hard limits
    uint256 private constant LUCKYPOT_TIMESPAN_LIMIT_MIN = 30;
    uint256 private constant LUCKYPOT_TIMESPAN_LIMIT_MAX = 1800;

    uint256 private constant LUCKYPOT_LUCKYHIT_MIN = 100 * 10**USDT_DECIMALS;
    uint256 private constant LUCKYPOT_LUCKYHIT_MAX = 250000 * 10**USDT_DECIMALS;

    uint256 private constant LUCKYPOT_BUYER_SHARE_MIN = 1000;
    uint256 private constant LUCKYPOT_BUYER_SHARE_MAX = 10000;

    uint256 private constant LUCKYPOT_MINBUY_MIN = 5 * 10**(BNB_DECIMALS - 2);
    uint256 private constant LUCKYPOT_MINBUY_MAX = 5 * 10**(BNB_DECIMALS - 1);

    uint256 private constant LUCKYPOT_CASHOUT_MIN = 1000;
    uint256 private constant LUCKYPOT_CASHOUT_MAX = 10000;

    uint256 private constant LUCKYPOT_LUCKYHIT_BUYBACK_MIN = 1000;
    uint256 private constant LUCKYPOT_LUCKYHIT_BUYBACK_MAX = 10000;
    ...
    // 5% luckypot
    uint256 public bLuckypotFee = 500;
    ...
    uint256 private _luckypotTokens = 0;

    // Luckypot related variables
    // 80% luckypot cashout to last buyer
    uint256 public luckypotCashout = 8000;
    // 50% of luckypot cashout to last buyer
    uint256 public luckypotBuyerShare = 5000;
    // Buys > 0.05 BNB will be eligible for the luckypot
    uint256 public luckypotMinBuy = 5 * 10**(BNB_DECIMALS - 2);
    // Luckypot time span is initially set to 7 mins
    uint256 public luckypotTimespan = 7 * 60;
    // Luckypot hard limit, BNB value
    uint256 public luckypotHardLimit = 18 * 10**(BNB_DECIMALS);
    // Luckypot hard limit buyback share
    uint256 public luckypotHardBuyback = 8000;
    ...
    uint256 private _lastLuckyHitCash = 0;
    uint256 private _lastLuckyHitTokens = 0;
    uint256 private _lastLuckyHitTimestamp = 0;
    ...
    uint256 private _pendingLuckypotBalance = 0;

    // Total BNB/LAS collected by various mechanisms (dev, marketing, luckypot)
    uint256 private _totalMarketingFeesCollected = 0;
    uint256 private _totalDevFeesCollected = 0;
    uint256 private _totalLuckypotCashedOut = 0;
    uint256 private _totalLuckypotTokensOut = 0;
    uint256 private _totalLuckypotBuyer = 0;
    uint256 private _totalLuckypotBuyback = 0;
    uint256 private _totalLuckypotBuyerTokens = 0;
    uint256 private _totalLuckypotBuybackTokens = 0;
    ...
    event LuckyHit(uint256 cashedOut, uint256 tokensOut);
    ...
    event LuckypotFeaturesChanged(
        uint256 luckypotCashout,
        uint256 luckypotBuyerShare,
        uint256 lucktpotMinBuy
    );

    event LuckypotTimespanChanged(uint256 luckypotTimespan);
    ...
    event LuckyHitFeaturesChanged(
        uint256 luckypotHardBuyback,
        uint256 luckypotHardLimit
    );

    event LuckypotFund(uint256 bnbSent, uint256 tokenAmount);
    ...
    function totalLuckypotOut() external view returns (uint256, uint256) {
        return (_totalLuckypotCashedOut, _totalLuckypotTokensOut);
    }

    function totalLuckypotBuyer() external view returns (uint256, uint256) {
        return (_totalLuckypotBuyer, _totalLuckypotBuyerTokens);
    }

    function totalLuckypotBuyback() external view returns (uint256, uint256) {
        return (_totalLuckypotBuyback, _totalLuckypotBuybackTokens);
    }
    ...
    function setBuyFees(
        ...
        require(
            liquidityFee.add(marketingFee).add(devFee).add(luckypotFee) <=
                TOTAL_FEES_LIMIT,
        ...
        bDevFee = devFee;
        bLuckypotFee = luckypotFee;

        emit BuyFeesChanged(bLiquidityFee, bMarketingFee, bDevFee, bLuckypotFee);
    }

    function getBuyTax() public view returns (uint256) {
        return bLiquidityFee.add(bMarketingFee).add(bDevFee).add(bLuckypotFee);
    }
    ...
    function getSellTax() public view returns (uint256) {
        return sLiquidityFee.add(sMarketingFee).add(sDevFee).add(sLuckypotFee);
    }

    function setLuckypotFeatures(
        uint256 _luckypotCashout,
        uint256 _luckypotBuyerShare,
        uint256 _luckypotMinBuy
    ) external onlyAuthorized {
        require(
            _luckypotCashout >= LUCKYPOT_CASHOUT_MIN &&
                _luckypotCashout <= LUCKYPOT_CASHOUT_MAX,
            "Luckypot cashout percentage needs to be between 10% and 100%"
        );
        require(
            _luckypotBuyerShare >= LUCKYPOT_BUYER_SHARE_MIN &&
                _luckypotBuyerShare <= LUCKYPOT_BUYER_SHARE_MAX,
            "Luckypot buyer share percentage needs to be between 10% and 100%"
        );
        require(
            _luckypotMinBuy >= LUCKYPOT_MINBUY_MIN &&
                _luckypotMinBuy <= LUCKYPOT_MINBUY_MAX,
            "Luckypot min buy needs to be between 0.05 and 0.5 BNB"
        );
        luckypotCashout = _luckypotCashout;
        luckypotBuyerShare = _luckypotBuyerShare;
        luckypotMinBuy = _luckypotMinBuy;

        emit LuckypotFeaturesChanged(
            luckypotCashout,
            luckypotBuyerShare,
            luckypotMinBuy
        );
    ...
    function setLuckypotHardFeatures(
        uint256 _luckypotHardBuyback,
        uint256 _luckypotHardLimit
    ) external onlyAuthorized {
        require(
            _luckypotHardBuyback >= LUCKYPOT_LUCKYHIT_BUYBACK_MIN &&
                _luckypotHardBuyback <= LUCKYPOT_LUCKYHIT_BUYBACK_MAX,
            "Luckypot hard buyback percentage needs to be between 10% and 100%"
        );
        luckypotHardBuyback = _luckypotHardBuyback;

        uint256 hardLimitUsd = usdEquivalent(_luckypotHardLimit);
        require(
            hardLimitUsd >= LUCKYPOT_LUCKYHIT_MIN &&
                hardLimitUsd <= LUCKYPOT_LUCKYHIT_MAX,
            "Luckypot hard value limit for the luckyhit needs to be between 100 and 250K USD"
        );
        luckypotHardLimit = _luckypotHardLimit;

        emit LuckyHitFeaturesChanged(luckypotHardBuyback, luckypotHardLimit);
    }

    function setLuckypotTimespanInSeconds(uint256 _luckypotTimespan)
        external
        onlyAuthorized
    {
        require(
            _luckypotTimespan >= LUCKYPOT_TIMESPAN_LIMIT_MIN &&
                _luckypotTimespan <= LUCKYPOT_TIMESPAN_LIMIT_MAX,
            "Luckypot timespan needs to be between 30 and 1800 seconds (30 minutes)"
        );
        luckypotTimespan = _luckypotTimespan;

        emit LuckypotTimespanChanged(luckypotTimespan);
    }
    ...
    function fundLuckypot(uint256 tokenAmount) external payable onlyAuthorized {
        require(
            balanceOf(msg.sender) >= tokenAmount,
            "You don't have enough tokens to fund the luckypot"
        );
        uint256 bnbSent = msg.value;
        _pendingLuckypotBalance = _pendingLuckypotBalance.add(bnbSent);
        if (tokenAmount > 0) {
            transferBasic(msg.sender, address(this), tokenAmount);
            _luckypotTokens = _luckypotTokens.add(tokenAmount);
        }

        emit LuckypotFund(bnbSent, tokenAmount);
    }

    function isLuckypotEligible(uint256 tokenAmount) public view returns (bool) {
        if (luckypotMinBuy == 0) {
            return true;
        }
        ...
        .getAmountsOut(luckypotMinBuy, path)[1].mul(MAX_PCT.sub(ROUTER_FEE)).div(
        ...
        )
    {
        ...
        (luckypotTokens, accSum) = getUsedTokens(
            accSum,
            tokenAmount,
            _luckypotTokens
        );
        _luckypotTokens = _luckypotTokens.sub(luckypotTokens);

        return (liquidityTokens, marketingTokens, devTokens, luckypotTokens);
    }
    ...
    function transfer(
        ...
        // Luckypot mechanism locks the swap if triggered. We should handle it as
        // soon as possible so that we could award the luckypot on a sell and on a buy
        if (!_inSwapAndLiquify && _pendingLuckypotBalance >= luckypotHardLimit) {
            processLuckyHit();
        } else if (
            // We can't award the luckypot in swap and liquify
            // Pending balances need to be untouched (externally) for swaps
            !_inSwapAndLiquify &&
            _lastBuyer != address(0) &&
            _lastBuyer != address(this) &&
            block.timestamp.sub(_lastBuyTimestamp) >= luckypotTimespan
        ) {
            awardLuckypot();
        }
    ...
    function getLuckypot() public view returns (uint256, uint256) {
        return (_pendingLuckypotBalance, _luckypotTokens);
    }

    function luckypotBuyerShareAmount() public view returns (uint256, uint256) {
        uint256 bnb = _pendingLuckypotBalance
            .mul(luckypotCashout)
            .div(MAX_PCT)
            .mul(luckypotBuyerShare)
            .div(MAX_PCT);
        uint256 tokens = _luckypotTokens
            .mul(luckypotCashout)
            .div(MAX_PCT)
            .mul(luckypotBuyerShare)
            .div(MAX_PCT);
    ...
    function luckypotBuybackAmount() public view returns (uint256, uint256) {
        uint256 bnb = _pendingLuckypotBalance
            .mul(luckypotCashout)
            .div(MAX_PCT)
            .mul(MAX_PCT.sub(luckypotBuyerShare))
            .div(MAX_PCT);
        uint256 tokens = _luckypotTokens
            .mul(luckypotCashout)
            .div(MAX_PCT)
    ...
    {
        return (_lastLuckyHitCash, _lastLuckyHitTokens, _lastLuckyHitTimestamp);
    }
    ...
    {
        return (_liquidityTokens, _marketingTokens, _devTokens, _luckypotTokens);
    }

    function processLuckyHit() private lockTheSwap {
        uint256 cashedOut = _pendingLuckypotBalance.mul(luckypotHardBuyback).div(
            MAX_PCT
        );
        uint256 tokensOut = _luckypotTokens.mul(luckypotHardBuyback).div(MAX_PCT);

        buybackWallet().transfer(cashedOut);
        transferBasic(address(this), buybackWallet(), tokensOut);
        emit LuckyHit(cashedOut, tokensOut);

        _lastLuckyHitCash = cashedOut;
        _lastLuckyHitTokens = tokensOut;
        _lastLuckyHitTimestamp = block.timestamp;

        _pendingLuckypotBalance = _pendingLuckypotBalance.sub(cashedOut);
        _luckypotTokens = _luckypotTokens.sub(tokensOut);

        _totalLuckypotCashedOut = _totalLuckypotCashedOut.add(cashedOut);
        _totalLuckypotBuyback = _totalLuckypotBuyback.add(cashedOut);
        _totalLuckypotTokensOut = _totalLuckypotTokensOut.add(tokensOut);
        _totalLuckypotBuybackTokens = _totalLuckypotBuybackTokens.add(tokensOut);
    }

    function awardLuckypot() private lockTheSwap {
        require(
            _lastBuyer != address(0) && _lastBuyer != address(this),
            "No last buyer detected"
        );
        uint256 cashedOut = _pendingLuckypotBalance.mul(luckypotCashout).div(
            MAX_PCT
        );
        uint256 tokensOut = _luckypotTokens.mul(luckypotCashout).div(MAX_PCT);
        uint256 buyerShare = cashedOut.mul(luckypotBuyerShare).div(MAX_PCT);
        uint256 tokensToBuyer = tokensOut.mul(luckypotBuyerShare).div(MAX_PCT);
        ...
        _totalLuckypotCashedOut = _totalLuckypotCashedOut.add(cashedOut);
        _totalLuckypotTokensOut = _totalLuckypotTokensOut.add(tokensOut);
        _totalLuckypotBuyer = _totalLuckypotBuyer.add(buyerShare);
        _totalLuckypotBuyerTokens = _totalLuckypotBuyerTokens.add(tokensToBuyer);
        _totalLuckypotBuyback = _totalLuckypotBuyback.add(toBuyback);
        _totalLuckypotBuybackTokens = _totalLuckypotBuybackTokens.add(
            tokensToBuyback
        );
    }
    ...
    function swapAndLiquify(uint256 tokenAmount) private lockTheSwap {
        ...
        // Same for Luckypot
        uint256 balanceToLuckypot = gainedBnb.mul(luckypotTokens).div(
            tokensForBnbExchange
        );
        _pendingLuckypotBalance += balanceToLuckypot;

        uint256 remainingBnb = gainedBnb
            .sub(balanceToMarketing)
            .sub(balanceToDev)
            .sub(balanceToLuckypot);
    ...
    function transferStandard(
        ...
        uint256 tFees = tLiquidity.add(tMarketing).add(tDev).add(tLuckypot);
        ...
        )
    {
        ...
        uint256 tLuckypot = tAmount.mul(_luckypotFee).div(MAX_PCT);
        uint256 tTransferAmount = tAmount.sub(
            tLiquidity.add(tMarketing).add(tDev).add(tLuckypot)
        );
        return (tTransferAmount, tLiquidity, tMarketing, tDev, tLuckypot);
    }
