...
contract CrocboyFarm is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of CrockBoy
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;         // How many allocation points assigned to this pool. CrockBoy to distribute per block.
        uint256 lastRewardBlock;    // Last block number that CrockBoy distribution occurs.
        uint256 accTokenPerShare;   // Accumulated CrockBoy per share, times 1e12. See below.
    ...
    // CrockBoy tokens created per block.
    ...
    // The block number when CrockBoy mining starts.
    ...
    // Update the given pool's CrockBoy allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending CrockBoy on frontend.
    ...
    // View function to see if user can harvest CrockBoy.
    ...
    // Deposit LP tokens to MasterChef for CrockBoy allocation.
    ...
    // Pay or lockup pending CrockBoy.
    ...
    // Safe token transfer function, just in case if rounding error causes pool to not have enough CrockBoy.
