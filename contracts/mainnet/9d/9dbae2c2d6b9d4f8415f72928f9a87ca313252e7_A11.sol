...
// Buy/Sell - 10%
// 3% for strengthening our liquidity pool / 7% for accumulating funds for advertisement and exposure
// Telegram: https://t.me/apollo11bsc
// Medium: https://medium.com/@Apollo11BSC
...
contract A11 is IBEP20, Auth {
    ...
    string constant private _name = "Apollo 11";
    string constant private _symbol = "APOLLO11";
    ...
    address public varMarketingFeeReceiver = 0xfbD7F862e3C733D490BCF851e95f7D6b86ea1643;
    address public devFeeReceiver = 0x2d0881FBcf6c66E4cfecB74E7c7847eFfAc23249;
    ...
    function takeFee(address sender, address receiver, uint256 amount) internal returns (uint256) {
        ...
        // switch Trading
    function tradingEnable(bool _status) public authorized {
        tradingOpen = _status;
        if(tradingOpen){
    ...
    function triggerManualBuyback() internal view returns (bool) {
    ...
    function swapBack() internal swapping {
        ...
        (bool MarketingSuccess, /* bytes memory data */) = payable(varMarketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
   ...
   function setMaxWalletLimit(uint256 amount) external onlyOwner() {
    ...
    function changeFees(uint256 _varMarketingFee, uint256 _lpFee, uint256 _devFee, uint256 _feeDenominator) external authorized {
        lpFee = _lpFee;
        varMarketingFee = _varMarketingFee;
        devFee = _devFee;
        totalFee = _lpFee.add(_varMarketingFee).add(_devFee);
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator/3);
    }
        // enable cooldown between trades
    function setcooldown(bool _status, uint8 _interval) public authorized {
    ...
    function setdevWallet(address _varMarketingFeeReceiver, address _devFeeReceiver) external authorized {
        varMarketingFeeReceiver = _varMarketingFeeReceiver;
        devFeeReceiver = _devFeeReceiver;
    }

    // Set the maximum transaction limit
    function setmaxTxAmount(uint256 amountBuy) external authorized {
    ...
    function addToBlacklist(address _address, bool _value) public authorized{
        isBlacklisted[_address] = _value;
    }
    function manualSend() external authorized {
        uint256 contractETHBalance = address(this).balance;
        payable(varMarketingFeeReceiver).transfer(contractETHBalance);
    }

    function transferForeignToken(address _token) public authorized {
        require(_token != address(this), "Can't let you take all native token");
        uint256 _contractBalance = IBEP20(_token).balanceOf(address(this));
        payable(varMarketingFeeReceiver).transfer(_contractBalance);
    }
