...
contract DHXY is Context, IERC20, Ownable{
    ...
    address public nftwallet=0xd04Ba900BAb8ef29C134078aAfC26e0b43740C1C;
    ...
    function mintInit(uint256 totalSuply) internal{
        ...
        _mint(nftwallet,_total.mul(10).div(100));
        tokenHolders.push(nftwallet);
        _holderIsExist[nftwallet] = true;
        _mint(address(this),_total.mul(65).div(100));
        tokenHolders.push(address(this));
        _holderIsExist[address(this)] = true;
    }
    ...
    function processFee(
                ...
                difidendAmount= amount.mul(FeeToDifidend).div(10000);
                if(difidendAmount>0){
                    _balances[buysell] = _balances[buysell].add(difidendAmount);
                ...
                if(_balances[address(0)] < (_total.mul(amountToStopBurn).div(10000)) ){
                    burnAmount = amount.mul(feeToburn).div(1000);
                    _balances[address(0)] = _balances[address(0)].add(burnAmount);
                    _totalSupply = _totalSupply.sub(burnAmount);
                    emit Transfer(sender,address(0),burnAmount);
                }

                uint256 totalFeeAmount = amount.sub(difidendAmount).sub(difidendLPAmount);
    ...
    function difidendToLPHolders(address sender, uint256 amount) private {
        ...
        for (uint256 i = 0; i < tokenHolders.length; i++) {
            ...
            if (LPAmount > 0) {
                ...
                uint256 difidendAmount = amount.mul(LPAmount).div(totalLPAmount);
    ...
    function withDraw(uint256 amount) public {
        ...
        require(waitIncome >=amount,"Draw fail");
