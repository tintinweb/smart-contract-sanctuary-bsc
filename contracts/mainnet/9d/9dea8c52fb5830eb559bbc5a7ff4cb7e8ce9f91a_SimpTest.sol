...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional, Ownable {
  ...
  address public DividendsToken = address(0x2C14CBffD9963C17fF65028C52f75Ea6f14205Ff);
...
contract SimpTest is Context, ERC20, Ownable {
    using SafeMath for uint256;
    IUniswapV2Router02 public uniswapV2Router;
    address public immutable uniswapV2Pair;
    address public DividendsToken = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
    bool private swapping;
	bool private trading = true;
    bool public burning;
   SimpTestDividendTracker public dividendTracker;
	address private marketingWallet;
    address private movieWallet;
    uint256 public swapTokensAtAmount;
    uint256 private _buyRewardsFee;
    uint256 private _buyMarketingFee;
    uint256 private _buyMovieFee;
    uint256 private _sellRewardsFee;
	uint256 private _sellMarketingFee;
    uint256 private _sellMovieFee;
    ...
    uint256 private previousMovieFee;
    ...
    event MovieWalletUpdated(address indexed newMovieWallet, address indexed oldMovieWallet);
    ...
    constructor() ERC20 ("SimpTest", "SIMP") {
		_buyMarketingFee = 4;
        _buyMovieFee = 4;
        _buyRewardsFee = 4;
		_sellMarketingFee = 4;
        _sellMovieFee = 4;
        _sellRewardsFee = 4;
		contractTokenBalanceAmount = 10000000 * 10**18;
        swapTokensAtAmount = 100000000 * (10**18);
        _maxWallet = 30000000 * (10**18);
        _maxBuy = 30000000 * (10**18);
        _maxSell = 30000000 * (10**18);
        totalBuyFees = _buyRewardsFee.add(_buyMarketingFee).add(_buyMovieFee);
        totalSellFees = _sellRewardsFee.add(_sellMarketingFee).add(_sellMovieFee);
    	dividendTracker = new SimpTestDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "SimpTest: The dividend tracker already has that address");
        SimpTestDividendTracker newDividendTracker = SimpTestDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "SimpTest: The new dividend tracker must be owned by the SimpTest token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "SimpTest: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SimpTest: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SimpTest: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;
        emit ExcludeFromFees(account, excluded);
    }
    function addToBlacklist(address account, bool blacklisted) public onlyOwner {
        require(_isBlacklisted[account] != blacklisted, "SimpTest: Account is already the value of 'blacklisted'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SimpTest: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SimpTest: Automated market maker pair is already set to that value");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "SimpTest: The marketing wallet is already this address");
        excludeFromFees(newMarketingWallet, true);
        emit MarketingWalletUpdated(newMarketingWallet, marketingWallet);
        marketingWallet = newMarketingWallet;
    }
    function updateMovieWallet(address newMovieWallet) public onlyOwner {
        require(newMovieWallet != movieWallet, "SimpTest: The movie wallet is already this address");
        excludeFromFees(newMovieWallet, true);
        emit MovieWalletUpdated(newMovieWallet, movieWallet);
        movieWallet = newMovieWallet;
    }
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SimpTest: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SimpTest: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
		...
		if(canSwap && !swapping && !automatedMarketMakerPairs[from] && from != marketingWallet && to != marketingWallet && !_isExcludedFromFees[to] && !_isExcludedFromFees[from]) {
		    ...
		    uint256 swapTokens;
			uint256 swapAmount = _sellMarketingFee.add(_sellRewardsFee).add(_sellMovieFee);
            swapping = true;
            ...
            if (_sellMovieFee > 0) {
            uint256 movieAmount = address(this).balance.mul(_sellMovieFee).div(swapAmount);
            transferToWallet(payable(movieWallet), movieAmount);
            emit Transfer(address(this), movieWallet, movieAmount);
            }
	...
	function letsStabilize() external onlyOwner() {
        updateBuyFees(4,4,4);
        updateSellFees(8,4,4);
        updateMaxWallet(30000000);
        updateMaxBuySell(30000000, 30000000);
        burning = false;
    }
    function letsLaunchAntiBot() external onlyOwner() {
        updateBuyFees(4,4,4);
        updateSellFees(16,4,4);
        updateMaxWallet(30000000);
        updateMaxBuySell(30000000, 30000000);
        burning = false;
    }
    function updateBuyFees(uint8 newBuyMarketingFee, uint8 newBuyRewardsFee, uint8 newBuyMovieFee) public onlyOwner {
        _buyMarketingFee = newBuyMarketingFee;
        _buyRewardsFee = newBuyRewardsFee;
        _buyMovieFee = newBuyMovieFee;
        totalFees();
    }
    function updateSellFees(uint8 newSellMarketingFee, uint8 newSellRewardsFee, uint8 newSellMovieFee) public onlyOwner {
        _sellMarketingFee = newSellMarketingFee;
        _sellRewardsFee = newSellRewardsFee;
        _sellMovieFee = newSellMovieFee;
        totalFees();
    }
...
contract SimpTestDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("SimpTestInu_Dividend_Tracker", "SimpTestInu_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function withdrawDividend() public pure override {
        require(false, "SimpTestInu_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SimpTestInu contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 0 && newClaimWait <= 86400, "SimpTestInu_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SimpTestInu_Dividend_Tracker: Cannot update claimWait to same value");
