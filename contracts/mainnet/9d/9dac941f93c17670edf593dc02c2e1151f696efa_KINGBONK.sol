...
contract KINGBONK is ERC20, ERC20Burnable, Ownable, ERC20Permit {
    ...
    address WETH;
    uint96 public maxSupply = 1000000000 * 10**18;
    ...
    uint32 public buyTax = 300;
    uint32 public sellTax = 1000;
    uint32 public transferTax = 1000;
    uint32 public liquidityShare = 1000;
    uint32 public marketingShare = 7000;
    uint32 public rewardsShare = 2000;
    uint256 public maxTokenForWallet = maxSupply / 400;
    ...
    modifier buyLimit(address sender, address recipient, uint256 amount) {
        if(walletLimit) {
            if(isMarketMaker[sender] && !isWhitelisted[recipient] && !inSwap) {
                require(balanceOf(recipient).add(amount) <= maxTokenForWallet, "MaxTokenInWallet");
            }
        }
        _;
    }

    constructor() ERC20("KING BONK", "KBONK") ERC20Permit("KBONK") {}
    ...
    function setDistributionCriteria(uint80 _minPeriod, uint256 _minDistribution) external onlyOwner {
        distributor.setDistributionCriteria(_minPeriod, _minDistribution);
    }

    function setDistributorSettings(uint96 gas) external onlyOwner {
        require(gas < 1000000);
        distributorGas = gas;
    }
    ...
    function setWalletLimit(bool _walletLimitEnabled, uint256 _limitAmount) external onlyOwner {
    ...
    function setShares(uint32 newLiquidityShare, uint32 newMarketingShare, uint32 newRewardsShare) external onlyOwner {
