...
contract PresaleBUSDWhitelist is ReentrancyGuard, Ownable {
    using SafeERC20 for IERC20;

    //IERC20 public presaleToken;
    address public BUSD = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56; // BSC Mainnet
    
    
    address public treasury = 0xD898A08817F664A3404A3e21f4990937a33b755D;
    address public party = 0xd8a32156E0Cd693C9B7B5F8Cc80Cda94923186FC;
    address public admin = 0x527a819db1eb0e34426297b03bae11F2f8B3A19E; //gelato bsc
    ...
    uint256 public constant TOKEN_PRICE = 1500000000000000000;  //1600000000000000000
    ...
    uint256 public constant CONTRIBUTION_MAX = 5000 ether;
    ...
    modifier onlyParty() {
        require(msg.sender == owner() || msg.sender == admin, "Caller is not Admin!");
        _;
    }
    ...
    function removeFromWhitelist(address _beneficiary) external onlyOwner {
        whitelistedAddresses[_beneficiary] = false;
    }

    function whitelisted(address _whitelistedAddress) public view returns(bool) {
    ...
    function contribute(uint256 _amount) public nonReentrant isWhitelisted(msg.sender) {
   ...
   function withdraw() external nonReentrant {
        require(status != PresaleStatuses.Started, "TokenSale: Presale is not finished");

        if (_msgSender() == owner()){
            if (status == PresaleStatuses.Finished) {
                uint256 totalAmount = IERC20(BUSD).balanceOf(address(this));
                uint256 fee = (totalAmount * 5) / 100;
                _safeTransferBUSD(treasury, fee);
                _safeTransferBUSD(party, totalAmount - fee);
            } 
        } else {
            ContributeData storage funder = funders[_msgSender()];
            ...
            if (status == PresaleStatuses.Canceled) {
    ...
    function closePresale() external nonReentrant onlyParty {
    ...
    function adminSetStatus(PresaleStatuses _status) external onlyOwner {
        status = _status;
    }
    ...
    function setParty(address _address) external onlyOwner {
        party = _address;
    }

    function setOps(address _address) external onlyOwner {
        admin = _address;
    }
