...
https://t.me/AnalProtocol

https://twitter.com/analprotocol
...
contract AnalProtocol is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    AnalProtocolDividendTracker public dividendTracker;
    ...
    constructor() ERC20("AnalProtocol", "ANAL") {
        marketingWallet = payable(0x4F52f743aB03DebdE264b29E586305Ea1aA9D853);
        treasuryWallet = payable(0xd500B702cF0e283b5B79265782D2cAb092392662);
        ...
        dividendTracker = new AnalProtocolDividendTracker(
            payable(this),
            router,
...
contract AnalProtocolDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    AnalProtocol public AnalProtocolContract;
    ...
    constructor(
        ...
        defaultToken = token;
        AnalProtocolContract = AnalProtocol(mainContract);
    ...
    function _transfer(
        ...
        require(false, "AnalProtocol_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "AnalProtocol_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main AnalProtocol contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(AnalProtocolContract);

            uint256 prevBalance = AnalProtocolContract.balanceOf(address(this));
            ...
            {
                uint256 received = AnalProtocolContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    AnalProtocolContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
