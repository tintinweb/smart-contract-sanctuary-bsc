...
/** 
    ...
    Ryker.Vip 7 in 1 Mining - BUSD Edition. 

    🖌 7 ways to earn in 1 investment
      🖊  4% Miner Daily Yield (Fixed 4% Daily Rate for the Miner Protocol)
      🖊  1% Fixed Income Yield. (Increased to 2% once every month.)
      🖊  6% Referral Rewards, (3% Referrer/3% Referee.)
      🖊  Lottery Every 6 hours. (Rewards will be used to buy miners and be distributed to the winning address.)
      🖊  Top Referrer Rewards every 24 hours. (10% of the total pool will be distributed to top 5 referrers as miners.)
      🖊  Last Deposit Jackpot Rewards every 12 hours. (50% used to buy Miners and sent to the winning address, 50% sent to winner as BUSD Reward)
      🖊  Late Investor Income. (Last 10% of the total investors will be given the late investor funds every week.)

    🖌 5 Unique Features:
      🖊  Automated Compound Feature. (To activate, users will deposit the amount of gas fee per compound, I.E: gas fee = 0.001, no. of compound = 7 '0.001 x 7 = 0.007 is the gas fee amount to be deposited.'
      🖊  Automated Anti-Bot Feature. (Contract will blacklist addresses that will try to interact more than 2 times before launch.) 
      🖊  Over-Income Tax Deduction. (income over 300% = -70% Tax, income over 400% = -80% Tax, income over 500% = -90% Tax)
      🖊  Flip Coin - Miner Betting Feature runs every 6 hours. (Bet your miners in a 50/50 chance of winning.)
      🖊  Horse Race - Miner Betting Feature runs every 8 hours. (Bet on any of the 5 horses and get the chance to win based on the odds.)

    🖌 1 Upcoming Feature:
      🖊  Hold Ryker Tokens for increase in rewards. (Ryker token holders will have the benefit of having higher referral bonus and compound bonus based of the token in their wallets.)  
...
contract ryker_busd {
    ...
    /** limits **/
    uint256 private minDeposit = 10 ether;
    ...
    constructor(address devt, address lif, address mkt, address exec) {
        ...
        tokenERC20        = IERC20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); //0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56
    ...
    function betTossCoin(uint256 betNumber, uint256 betMiners) public contractActivated {
        ...
        require((users[msg.sender].miners.mul(100)).div(1000) >= betMiners, "Can only bet 20% of the bettors total miners");
    ...
    function betHorseRacing(uint256 betNumber, uint256 betMiners) public contractActivated {
        ...
        require(users[msg.sender].miners > 0 && (users[msg.sender].miners.mul(100)).div(1000) >= betMiners, "Can only bet 20% of the bettors total miners");
