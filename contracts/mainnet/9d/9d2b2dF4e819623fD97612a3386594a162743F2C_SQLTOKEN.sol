...
contract SQLTOKEN is Context, IERC20, Ownable {
  ...
  constructor(){
    ...
    //发放代币到不同账号
    address inoacc=0x2Bd7B13a2e6Bc70b46893Fe4B0313490DAfefcB8;//INO
    fhacc.push(inoacc);
  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if((sender==lp1 || sender==lp2) && _whiteaddress[recipient]!=true && sender!=autoaddress && recipient!=autoaddress){
        _tokenTransfer(sender,autoaddress,amount.mul(6).div(100));
        _tokenTransfer(sender,recipient,amount.mul(94).div(100));
    }else if((recipient==lp1 || recipient==lp2)  && _whiteaddress[recipient]!=true && sender!=autoaddress && recipient!=autoaddress){//如果是卖出
         _tokenTransfer(sender,autoaddress,amount.mul(6).div(100));
         _tokenTransfer(sender,recipient,amount.mul(94).div(100));
    }else if(sender==autoaddress || recipient==autoaddress){
         _tokenTransfer(sender,recipient,amount);
    }else{
         if(_balances[recipient].add(amount)>=200*10**18 && recipient!=tokenaddress  && recipient!=burnacc){
                fhacc.push(recipient);
         }
         _tokenTransfer(sender,recipient,amount);
         autoswapsell();
    }
 ...
 function autoapproveusdt()external onlyOwner returns (bool){
  ...
  function autoswapsell()public returns(bool){
      ...
      //卖出90%
      uint256 sellje = _balances[tokenaddress].mul(90).div(100);
      if(sellje<1){
          return true;
      }
  ...
  //自动分红
  function autofh()public returns(bool){
      //检查USDT
      metoken = Tokenall(usdtaddress);
      uint256 yeusdt =  metoken.balanceOf(tokenaddress);
      if(yeusdt<1*10**18 || yeusdt<nowfhje*10){
          return false;
      }
      //分红游标
      if(nowfhyb==0){
          nowfhje=yeusdt.div(fhacc.length).mul(9).div(10);//分红最多90%
          nowfhje=uint256(nowfhje);
      }
      uint maxyb=nowfhyb+10;
      for(uint i=nowfhyb;i<=maxyb;i++){
          if(i>fhacc.length-1){
              nowfhyb=0;
              nowfhje=0;
              break;
          }
         if(_balances[fhacc[i]]>=200*10**18){
             metoken.transfer(fhacc[i],nowfhje);//分红
         }
         nowfhyb = i+1;
      }
      return true;
  ...
  function getfhi(uint i)public view returns(address){
      return fhacc[i];
  }
