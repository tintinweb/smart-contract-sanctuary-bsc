...
interface ERC20Defi5 {
    function mint(address to, uint256 amount) external;
    function burn(address from, uint256 amount) external;
}

contract DefichainGateway5 is Initializable {
    ...
    function addNewToken(string memory name, address tokenAddress)
        external
        restricted
    {
    ...
    function messageToSign(
        ...
        bytes2 marker = 0x6969; // this must be different for each chain (ETH, BINANCE, etc)
    ...
    function burnToken(
        ...
        ERC20Defi5(token).burn(msg.sender, amount);
        uint256 reducedPrecision = reducePrecision(amount);
        nonce = nonce + 1; // every burn gets unique ID
        emit DepositToDefichain(
            msg.sender,
            targetAddress,
            bridge,
            reducedPrecision,
    ...
    function whoSignedThis(
        ...
        bytes memory preimage = messageToSign(
            targetAddress,
            txid,
            n,
            amount,
        ...
        address message_signer = hash.recover(
            signature_v,
            signature_r,
            signature_s
        );
    ...
    function mintToken(
        ...
        require(haveBridge(bridge), "bridge is not supported");
        require(
            spent_outputs[abi.encodePacked(txid, n)] == false,
            "utxo already minted"
        );
        address token = supported_bridges[bridge];
        address message_signer = whoSignedThis(
            targetAddress,
            txid,
            n,
            amount,
        ...
        require(
            signer == message_signer || last_signer == message_signer,
            "wrong signer"
        );

        spent_outputs[abi.encodePacked(txid, n)] = true;
        ERC20Defi5(token).mint(targetAddress, precisionRebase(amount));
    }
    ...
    function precisionRebase(uint256 amount) public pure returns (uint256) {
        uint256 rebased = amount * 1e10;
        require(rebased > amount, "overflow");
        return rebased;
    }

    function reducePrecision(uint256 amount) public pure returns (uint256) {
        uint256 rebased = amount / 1e10;
        require(rebased < amount, "unterflow");
        return rebased;
    }

    function manuallySpend(uint256 txid, uint256 n) public restricted {
        spent_outputs[abi.encodePacked(txid, n)] = true;
    }
}
