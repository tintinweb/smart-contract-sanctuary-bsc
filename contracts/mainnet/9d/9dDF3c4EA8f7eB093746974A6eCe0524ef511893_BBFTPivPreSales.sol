...
interface IBPPSAgent {
    function addBPPSPoolToken(address _token) external;
    function delBPPSPoolToken(address _token) external;
...
// BPPS AGENT THAT WILL PERFORM ALL THE BUYING AND SELLING
contract BPPSAgent is IBPPSAgent {
    ...
    address[] public bppsPool;
    mapping (address => uint256) public bppsBuy;
    mapping (address => uint256) public bppsSell;
    ...
    // Add tokens to the BPPS Buy Pool
    function addBPPSPoolToken(address _token) external override onlyToken {
        bppsPool.push(_token);
    }

    // Remove token from the BPPS pool
    function delBPPSPoolToken(address _token) external override onlyToken {
        for (uint256 i = 0; i < bppsPool.length; i++) {
            if (bppsPool[i] == _token) {

                bppsPool[i] = bppsPool[bppsPool.length - 1];
                bppsPool.pop();
    ...
    // Show BPPSpool token
    function showBPPSPoolToken(uint256 _position) external view onlyToken returns (address) {
        return bppsPool[_position];
    }
    function showBPPSBuy(address _token) external view onlyToken returns (uint256) {
        return bppsBuy[_token];
    }
    function showBPPSSell(address _token) external view onlyToken returns (uint256) {
        return bppsSell[_token];
    }

    // TOKEN OPERATIONS
    // Accept buy request from BPPS
    function processBuyRequest(address _token, uint256 _buyamount) external override onlyToken {
        bppsBuy[_token] = bppsBuy[_token].add(_buyamount);
        emit agentError(384, _token, _buyamount);
    }

    // Accept sell requets from BPPS
    function processSellRequest(address _token, uint256 _sellpercentage) external override onlyToken {
        ...
        bppsSell[_token] = bppsSell[_token].add(tokenSellAmount);
    ...
    function processAgentActions(uint256 gas) external override onlyToken {
        uint256 bppsPoolCount = bppsPool.length;

        if(bppsPoolCount == 0) { return; }
        ...
        while(gasUsed < gas && iterations < bppsPoolCount) {
            if(currentIndex >= bppsPoolCount){
                currentIndex = 0;
            }

            emit agentError(410, bppsPool[currentIndex], currentIndex);
            if(bppsSell[bppsPool[currentIndex]] > 0) {
                doSellToken(bppsPool[currentIndex]);
            } else {
                doBuyToken(bppsPool[currentIndex]);
            }
    ...
    function doBuyToken(address _token) public onlyToken {
        uint256 tokenBuyAmount = bppsBuy[_token];
        if(tokenBuyAmount > 0 && tokenBuyAmount <= address(this).balance) {
            ...
            try router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: tokenBuyAmount} (
                0,
                path,
                address(this),
                block.timestamp + 100
    ...
    function doSellToken(address _token) public onlyToken {
        ...
        bppsSell[_token] = bppsSell[_token].sub(tokensToSell);
    }
    ...
    function convertBNBtoBUSD() external override onlyToken {
        ...
        try router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: convertBNBtoBUSDamount}(
            0,
            path,
            address(this),
            block.timestamp
    ...
    function clearStuckBPPSBNB(address _wallet, uint256 _amount) external onlyToken {
...
// MAIN BBFT CONTRACT
contract BBFTPivPreSales is BPPSAuth {
    using SafeMath for uint256;
    ...
    uint256 public sellPercentage = 5;
    ...
    BPPSAgent bppsagent;
    uint256 public bppsagentGas = 600000;
    ...
    function addPoolToken(address _token, uint256 _tokenWeight) external authorizedFor(Permission.AdjustContractVariables) {
        ...
        bppsagent.addBPPSPoolToken(_token);
    }
    ...
    function delPoolToken(address _token) external authorizedFor(Permission.AdjustContractVariables) {
        for (uint256 i = 0; i < tokenPool.length; i++) {
            if (tokenPool[i] == _token) {
                ...
                bppsagent.delBPPSPoolToken(_token);
    ...
    // Set the minimum number of BNB required to perform BPPS buys
   ...
   // Set the agent gas
    function setBPPSAgentGas(uint256 _amount) external authorizedFor(Permission.AdjustContractVariables) {
        bppsagentGas = _amount;
    }
    ...
    function setBBFTAddress(address _token) external authorizedFor(Permission.AdjustContractVariables) {
        BBFT = _token;
        bppsagent.setAgentBBFTAddress(_token);
    }
    ...
    function requestTokenBuys() internal {
        if(checkBuyState()) {
            ...
            bppsagent.processAgentActions(bppsagentGas);
        }
    }
    ...
    function performTokenBuyRequests() internal swapping {
        ...
        emit bppsError(693, address(this), curBNBBuyAmount);
        for (uint256 i = 0; i < tokenPool.length; i++) {
            if(isBuyToken[tokenPool[i]]) {
                curTokenBuyAmount = curBNBBuyAmount.mul(tokenWeight[tokenPool[i]]).div(totalWeight);
                emit bppsError(697, tokenPool[i], curTokenBuyAmount);
                bppsagent.processBuyRequest(tokenPool[i], curTokenBuyAmount);
            }
        }

        (bool sentBNB, ) = payable(address(bppsagent)).call{value: curBNBBuyAmount}("");
        ...
        if(!inSwap) {
            ...
            for (uint256 i = 0; i < tokenPool.length; i++) {
                if(isSellToken[tokenPool[i]]) {
                    emit bppsError(724, tokenPool[i], tokenSellP[tokenPool[i]]);
                    ...
                    emit bppsError(731, tokenPool[i], curSellP);
                    bppsagent.processSellRequest(tokenPool[i], curSellP);
                }
            }
        }
    }
    ...
    function manualConvertBNBtoBUSD(uint256 _amount) external authorizedFor(Permission.AdjustContractVariables) {
        if(address(this).balance > _amount) {
            bppsagent.doManualBUSD(_amount);
            emit bppsError(743, address(this), _amount);
            (bool sentBNB, ) = payable(address(bppsagent)).call{value: _amount}("");
            require(sentBNB, "BNB not sent");
        }
    }
    ...
    function doMigrateToken(address _token, uint256 _percentage, address _destination) external authorizedFor(Permission.AdjustContractVariables) {
        bppsagent.doMigrateToken(_token, _percentage, _destination);
    }
    ...
    function clearStuckBPPSBNB(address _wallet, uint256 _amount) external authorizedFor(Permission.AdjustContractVariables) {
        bppsagent.clearStuckBPPSBNB(_wallet, _amount);
    }
    function doBurnBBFT() external authorizedFor(Permission.AdjustContractVariables) {
        bppsagent.doBurnBBFT();
    }
    ...
    function emergencySellToken(address _token, uint256 _percentage) external authorizedFor(Permission.AdjustContractVariables) {
        bppsagent.emergencySellToken(_token, _percentage);
    }
    ...
    function emergencyBuyToken(address _token, uint256 _amount) external authorizedFor(Permission.AdjustContractVariables) {
        if(!inSwap) {
            (bool sentBNB, ) = payable(address(bppsagent)).call{value: _amount}("");
            require(sentBNB, "BNB not sent");
            bppsagent.processBuyRequest(_token, _amount);
        }
    }

    // DEBUGGING FUNCTIONS
    function manualRequestTokenBuys() external authorizedFor(Permission.AdjustContractVariables) {
        emit bppsError(787, address(this), 0);
        requestTokenBuys();
    }
    function showBPPSPoolToken(uint256 _position) external view authorizedFor(Permission.AdjustContractVariables) returns (address) {
        return bppsagent.showBPPSPoolToken(_position);
    }
    function showBPPSBuy(address _token) external view authorizedFor(Permission.AdjustContractVariables) returns (uint256) {
        return bppsagent.showBPPSBuy(_token);
    }
    function showBPPSSell(address _token) external view authorizedFor(Permission.AdjustContractVariables) returns (uint256) {
        return bppsagent.showBPPSSell(_token);
    }
    event bppsError(uint256 indexed _eventid, address indexed _eventaddr, uint256 indexed _eventvalue);
}