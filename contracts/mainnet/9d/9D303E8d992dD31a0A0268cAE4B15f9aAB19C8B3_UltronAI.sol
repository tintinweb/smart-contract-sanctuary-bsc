...
contract BonusHolder {
    address public owner;
    IERC20 IERC20_token;

    constructor() {
        owner = msg.sender;
        IERC20_token = IERC20(owner);
    }
...
contract UltronAI is IERC20 {
    ...
    string constant private _name = "Ultron AI";
    string constant private _symbol = "Ultron AI";
    ...
    uint256 constant public maxNegativeTaxes = 500;
    uint256 constant masterTaxDivisor = 10000;
    bool public taxesAreLocked;
    IRouter02 public dexRouter;
    address public lpPair;
    address constant public DEAD = 0x000000000000000000000000000000000000dEaD;
    address payable public marketingWallet = payable(0x5e77AFf2EE68059D7387067B664B69eF205CBcED);
    ...
    BonusHolder public bonusHolder;
    ...
    constructor () payable {
        ...
        _isExcludedFromFees[address(bonusHolder)] = true;
...
//===============================================================================================================
    ...
    function setTaxes(uint16 buyFee, uint16 sellFee, uint16 negativeTax) external onlyOwner {
        require(!taxesAreLocked, "Taxes are locked.");
        require(buyFee <= maxBuyTaxes
                && sellFee <= maxSellTaxes
                && negativeTax <= maxNegativeTaxes,
                "Cannot exceed maximums.");
        _taxRates.buyFee = buyFee;
        _taxRates.sellFee = sellFee;
        _taxRates.negativeTax = negativeTax;
    }

    function setRatios(uint16 buyback, uint16 marketing, uint16 negativeTax) external onlyOwner {
        _ratios.buyback = buyback;
        _ratios.marketing = marketing;
        _ratios.negativeTax = negativeTax;
        _ratios.totalSwap = buyback + marketing;
        uint256 total = _taxRates.buyFee + _taxRates.sellFee;
        require(_ratios.totalSwap + _ratios.negativeTax <= total, "Cannot exceed sum of buy and sell fees.");
    }
    ...
    function finalizeTransfer(address from, address to, uint256 amount, bool buy, bool sell, bool other) internal returns (bool) {
        ...
        if (buy && to != DEAD && _taxRates.negativeTax > 0) {
    ...
    function takeTaxes(address from, bool buy, bool sell, uint256 amount) internal returns (uint256) {
        Ratios memory ratios = _ratios;
        uint256 total = ratios.totalSwap + ratios.negativeTax;
        uint256 currentFee;
        if (buy) {
        ...
        uint256 bonusAmount = feeAmount * ratios.negativeTax / total;
        uint256 swapAmt = feeAmount - bonusAmount;
        if (swapAmt > 0) {
            _tOwned[address(this)] += swapAmt;
            emit Transfer(from, address(this), swapAmt);
        }
        if (bonusAmount > 0) {
            _tOwned[address(bonusHolder)] += bonusAmount;
            emit Transfer(from, address(bonusHolder), bonusAmount);
        }
