...
contract PCToken is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private _isBlackAddr;

    uint8 private _decimals = 18;
    uint256 private _tTotal = 1000 * 10**8 * 10**18;
 
    string private _name = "Pickled Cabbage";
    string private _symbol = "PC";
    
    uint256 public _buyLiquidityFee = 0;
    uint256 public _buyNodeFee = 30;
    uint256 public _buyTechFee = 30;
    uint256 public _buyMarketFee = 20;
 
    uint256 public _sellMarketFee = 30;
    uint256 public _sellLiquidityFee = 0;
    uint256 public _sellNodeFee = 30;
    uint256 public _sellTechFee = 30;
    ...
    constructor (
        ...
        _isBlackAddr[address(this)] = false;
    ...
    function setBlackWallet(address payable wallet) external onlyOwner{
        BlackAddress = wallet;
    }
    ...
    function AddToBlack(address account) public onlyOwner {
        _isBlackAddr[account] = true;
    }
    ...
    function RemoveFromBlack(address account) public onlyOwner {
        _isBlackAddr[account] = false;
    }
    ...
    function isBlackAddr(address account) public view returns(bool) {
        return _isBlackAddr[account];
    }
    ...
    function _getBuyParam(uint256 tAmount,Param memory param) private view  {
        ...
        param.tNode = tAmount * _buyNodeFee / 1000;
        param.tTech = tAmount * _buyTechFee / 1000;
    ...
    function _getSellParam(uint256 tAmount,Param memory param) private view  {
        ...
        param.tNode = tAmount * _sellNodeFee / 1000;
        param.tTech = tAmount * _sellTechFee / 1000;
        param.tFund = tAmount * _sellFundFee / 1000;
    ...
    function _takeFee(Param memory param,address from)private {
        if( param.tNode > 0 ){
            _take(param.tNode, from, NodeWalletAddress);
        }
        ...
        uint MFee = 0;
        if( param.tMarket > 0 ){
            MFee = MFee.add(param.tMarket);
        }
        if( param.tTech > 0 ){
            MFee = MFee.add(param.tTech);
        }
        if( param.tFund > 0 ){
            MFee = MFee.add(param.tFund); 
        }
        if( MFee > 0 ){
