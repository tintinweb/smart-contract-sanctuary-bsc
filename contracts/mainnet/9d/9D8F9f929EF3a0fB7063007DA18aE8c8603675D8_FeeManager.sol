...
contract FeeManager is IFeeManager, Ownable {
    ...
    constructor(ERC20 _token, ERC20 _usdToken, WERC20 _nativeToken , IUniswapV2Router02 _router) {
        token = _token;
        router = _router;
        usdToken = _usdToken;
    ...
    function saveToken(ERC20 _token, address _receiver) public onlyOwner {
        _token.transfer(_receiver, _token.balanceOf(address(this)));  
    }

    function unwrapAndSave() public onlyOwner {
        nativeToken.withdraw(nativeToken.balanceOf(address(this)));
        distributeETHFees();
    }
    ...
    function processFee() external override  {
        ...
        if(swapFees){
            // Swap the tokens to usdToken use a 1% slippage
            try SwapUtils.swapTokensForTokens(router, token, usdToken, balance, 100) {
                distributeBusdFees();
            } catch { 
                distributeFees();
            }
    ...
    function processBusdFee(uint256 amount) external override {
        usdToken.transferFrom(address(msg.sender), address(this), amount);
        distributeBusdFees();
    }
    ...
    function distributeBusdFees() override public addressesAreSet() {
        ...
        usdToken.transfer(devAddress, devTokens);
        usdToken.transfer(marketingAddress, marketingTokens);

        usdToken.increaseAllowance(address(investmentAddress), investmentTokens);
    ...
    function destroy() external onlyOwner {
        ...
        require(usdToken.balanceOf(address(this)) == 0 , "Contract still has usdToken");
...
library SwapUtils {
    ...
    function swapExactETHForTokens(IUniswapV2Router02 router, IERC20 token, uint256 ethAmount, uint256 slippage) external returns(uint256) {
        ...
        uint256[] memory amounts = router.swapExactETHForTokens{
            value: ethAmount
        }(minAmount, path, address(this), block.timestamp + 10 minutes);
