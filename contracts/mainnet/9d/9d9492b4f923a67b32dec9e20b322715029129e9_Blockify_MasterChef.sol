...
// BLOCKIFY.
contract BLOCKIFY is BEP20, MinterRole {
    ...
    address public constant BUILD = address(0xCDC491Ef9A9ccBb3D00b15Af252d050700F813F3); //BUILD
    ...
    constructor() BEP20('BLOCKIFY', 'BLFI') {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        if(transferTaxRate == 0 || _excludedFromFees[sender] || _excludedFromFees[recipient]) {
            ...
            require(amount == taxAmount + sendAmount, "BLFI:: transfer: Tax value invalid");
    ...
    function setExcludeFromFees(address _account, bool _excluded) public onlyOwner {
        require(_excludedFromFees[_account] != _excluded, "BLFI: Account is already the value of 'excluded'");
    ...
    function updateXHASHSwapRouter(address _router) public onlyOwner {
        ...
        require(hashSwapPair != address(0), "BLFI::updateXHASHSwapRouter: Invalid pair address.");
    ...
    function setFees(uint256 _liquidityFee, uint256 _buybackFee) external onlyOwner{
        ...
        require(transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "BLFI: Total fees can not be bigger than max value");
    }
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "BLFI::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "BLFI::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "BLFI::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "BLFI::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying BLFIs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "BLFI::_writeCheckpoint: block number exceeds 32 bits");
...
contract Blockify_MasterChef is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. BLFIs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that BLFIs distribution occurs.
        uint256 accXhashPerShare; // Accumulated BLFIs per share, times 1e12. See below.
    ...
    // The BLFI TOKEN!
    BLOCKIFY public xhash;
    // Dev address.
    address public devaddr = 0xB279Df09ff80a6381A37C9cDA4dF77C58bDc0BD3;
    // BLFI tokens created per block.
    uint256 public xhashPerBlock;
    // Deposit Fee address
    address public feeAddress = 0xB279Df09ff80a6381A37C9cDA4dF77C58bDc0BD3;
    // Max deposit fee: 10%.
    ...
    // The block number when BLFI mining starts.
    ...
    // Update the given pool's BLFI allocation point. Can only be called by the owner.
    ...
    // View function to see pending BLFIs on frontend.
    ...
    // Deposit LP tokens to MasterChef for BLFI allocation.
    ...
    // Safe xhash transfer function, just in case if rounding error causes pool to not have enough BLFIs.
