...
contract WhalesCandy {
    ...
    uint256 public LAUNCH_TIME = 1675551600; // TODO
          ...
          if (contractETHBalance > 1000){
            ...
            if (collectedThatDay != 0 && !liqAdded[lastBUYINday] ) {
                ...
                if (ethBal > 0.01 ether) {
                  uint256 addedEth = ETHtoAdd - ethBal;
                  uint256 currentLiqRatio = (tokenToAdd.sub(remainingToken)).mul(10000).div(addedEth);
                ...
                if (remainingToken > 1000*10**18){
                    _burn(contrAddr, remainingToken.sub(1 ether));
                }
