...
contract TEST is ERC20 {
    ...
    //回流钱包
	address private _fundAddress = address(0xbDC482C0655f7947cF66035ff80A907a7Df84Cd2);
    //营销钱包
	address private _mrketAddress = address(0x1361223C9cC5825a5bFf9D9f34178a99A8cCB68A);
    //团队钱包
	address private _applicationAddress = address(0x33b0bD666657a28125431b27e94A37e5ae016D2B);
    //去除手续费
    ...
    //卖手续费
	uint256 public sellRate = 30;
    //互转手续费
	uint256 public transferRate = 30;
    //买开关
    bool public swapBuyStats;
    //卖开关
    bool public swapSellStats;
    constructor(address tokenOwner) ERC20("TEST", "TEST") {
        ...
        //最大销毁数量
		_destroyMaxAmount = total.sub(888 * 10**18);
    ...
    function changeFeeRate(uint256 _buyRate,uint256 _sellRate,uint256 _transferRate) public onlyOwner {
        buyRate = _buyRate;
		sellRate = _sellRate;
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx || balanceOf(_destroyAddress) >= _destroyMaxAmount) {
			...
			if(from == uniswapV2Pair){
				...
				splitOtherToken();
				if(startTime.add(1800) > block.timestamp){require(amount <= 5 * 10**18);}
        ...
        if (takeFee) {
			...
			if(from == uniswapV2Pair){
				super._transfer(from, address(this), amount.div(1000).mul(buyRate - 10));
				amount = amount.div(1000).mul(1000 - buyRate);
				ldxAmount = ldxAmount.add(amount.div(200));
			}else if(to == uniswapV2Pair){
				super._transfer(from, address(this), amount.div(1000).mul(sellRate - 10));
				amount = amount.div(1000).mul(1000 - sellRate);
				ldxAmount = ldxAmount.add(amount.div(200));
			}else{
                super._transfer(from, address(this), amount.div(1000).mul(transferRate - 10));
				amount = amount.div(1000).mul(1000 - transferRate);
				ldxAmount = ldxAmount.add(amount.div(200));
            }
        }
        super._transfer(from, to, amount);
	...
	function swapAndLiquify() public {
		...
		if(allAmount > ldxcanAmount.add(10**17)){
			allAmount = allAmount.sub(ldxcanAmount);
		}else{
			return;
		}
		uint256 canswap = allAmount.div(300).mul(225);
		...
		if(allUsdtBalance > usdtBalance){
			...
			USDT.transfer(_mrketAddress,newAmount.div(225).mul(100));
			USDT.transfer(_applicationAddress,newAmount.div(225).mul(100));
			addLiquidityUsdt(newAmount.div(225).mul(25), otherAmount);
		}
    }
            ...
            if(buySize >10){
                for(uint256 i=0;i<10;i++){
                    ...
                    if(amountUsdt>10**10){
						...
						ldxOverAmount = ldxOverAmount.add(amountUsdt);
                    }
    ...
    function splitOtherToken() public {
		uint256 thisAmount = ldxAmount.sub(ldxOverAmount);
