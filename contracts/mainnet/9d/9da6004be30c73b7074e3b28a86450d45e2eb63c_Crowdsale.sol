...
interface ICrowdsale {
    ...
    function wsiAddress() external view returns (address);
    ...
    function addBulkToWhitelist(address[] calldata addresses) external;

    function buyTokenBNB(uint256) external payable;

    function getBusdForBnb(uint256)
        external
        view
...
interface IVestingManager {
    ...
    function setDuration(uint256) external;

    function setUnlockPercentage(uint256) external;
}
...
contract VestingWallet is Context {
    ...
    address internal immutable _vestingManagerAddress;

    /**
     * @dev Set the beneficiary, start timestamp and vesting duration of the vesting wallet.
     */
    constructor(address beneficiaryAddress, address vestingManagerAddress) {
        require(
    ...
    function duration() public view virtual returns (uint256) {
        return IVestingManager(_vestingManagerAddress).duration();
    }
...
// File contracts/CustomVestingWallet.sol
...
contract CustomVestingWallet is VestingWallet {
    constructor(address beneficiaryAddress, address vestingManagerAddress)
        VestingWallet(beneficiaryAddress, vestingManagerAddress)
    {}

    /**
     * @dev Getter for the vesting initial unlock percentage.
     */
    function unlockPercentage() public view returns (uint256) {
        return IVestingManager(_vestingManagerAddress).unlockPercentage();
    }
        ...
        returns (uint256)
    {
        if (timestamp < start()) {
            ...
            uint256 initialUnlocked = (totalAllocation * unlockPercentage()) /
                100;

            if (initialUnlocked >= totalAllocation) {
                return initialUnlocked;
            }

            uint256 vestingAmount = totalAllocation - initialUnlocked;
            return
                initialUnlocked +
                ((vestingAmount * (timestamp - start())) / duration());
        }
    }
}
...
// File contracts/BaseCrowdsale.sol
...
abstract contract BaseCrowdsale is
    ICrowdsale,
    Ownable,
    EmergencyGuard,
    ReentrancyGuard
{
    ...
    address private immutable _wsiAddress;
    ...
    constructor(
        ...
        _price = 32; // 0.032 BUSD
        _minAmountUsd = 200 ether;
        _maxAmountUsd = 1_000 ether;
        _busdAddress = addressBusd;
        _usdtAddress = addressUsdt;
        _wsiAddress = addressWsi;
        _pancakeRouterAddress = addressPancakeRouter;
        _vestingManagerAddress = addressVestingManager;
    }

    function price() public view override returns (uint256 value) {
        return _price;
    }

    function minAmountUsd() public view override returns (uint256 value) {
        return _minAmountUsd;
    }

    function maxAmountUsd() public view override returns (uint256 value) {
        return _maxAmountUsd;
    }

    function vestingWallet(address beneficiary)
        public
        view
        override
    ...
    function startTimestamp() public view override returns (uint256 value) {
        return _startTimestamp;
    }

    function endTimestamp() public view override returns (uint256 value) {
        return _endTimestamp;
    }

    function soldToken() public view override returns (uint256 value) {
        return _soldToken;
    }
    ...
    function usdtAddress() public view override returns (address value) {
        return _usdtAddress;
    }

    function wsiAddress() public view override returns (address value) {
        return _wsiAddress;
    }
    ...
    function whitelistEnabled() public view override returns (bool value) {
        return _whitelistEnabled;
    }

    function setStartTimestamp(uint256 value) external override onlyOwner {
        require(
    ...
    function setEndTimestamp(uint256 value) external override onlyOwner {
        require(
            value > block.timestamp,
    ...
    function setWhitelistEnabled(bool value) external override onlyOwner {
        _whitelistEnabled = value;
    }

    function addBulkToWhitelist(address[] calldata addresses)
        external
        override
        onlyOwner
    {
    ...
    function getBusdForBnb(uint256 amountBNB)
        public
        view
        override
        returns (uint256[] memory amounts)
    ...
    function _swapBnbToBusd(uint256 minAmountOut)
        internal
        nonReentrant
        returns (uint256 amount)
    {
        ...
        pancakeRouter.swapExactETHForTokensSupportingFeeOnTransferTokens{
            value: msg.value
        }(minAmountOut, path, address(this), block.timestamp);
    ...
    function _execute(address beneficiary, uint256 amountWsi) internal {
        address walletAddress = _getVestingWalletAddress(beneficiary);

        _soldToken = _soldToken + amountWsi;
        emit TokenTransferred(walletAddress, amountWsi);
        emit TokenSold(beneficiary, amountWsi);

        IERC20 wsi = IERC20(wsiAddress());
        require(
            wsi.transfer(walletAddress, amountWsi),
            "Crowdsale: WSI transfer failed"
        );
    }

    function _getVestingWalletAddress(address beneficiary)
        private
        returns (address)
    {
        ...
        if (existingWallet == address(0x0)) {
            CustomVestingWallet wallet = new CustomVestingWallet(
                beneficiary,
     ...
     * Validation
     */
    function _isWhitelisted(address beneficiary)
        private
        view
    ...
    function _preValidate() internal view {
        ...
        require(
            ...
            "Crowdsale: Address not whitelisted"
        );
    }

    function _validateAmountUsd(uint256 amountUsd) internal view {
        require(
            amountUsd >= minAmountUsd(),
            "Crowdsale: Amount lower than min. amount"
        );
...
contract Crowdsale is BaseCrowdsale {
    constructor(
        address addressBusd,
        address addressUsdt,
        address addressWsi,
     ...
     * Buying
     */
    function buyTokenBNB(uint256 minAmountBusd)
        external
        payable
        virtual
        override
    {
        ...
        uint256 amountWsi = (amountBusd * 1000) / price();

        _execute(msg.sender, amountWsi);
    }

    function buyTokenBUSD(uint256 amountBusd) external virtual override {
        _buyTokenErc20(amountBusd, busdAddress());
    }

    function buyTokenUSDT(uint256 amountUsdt) external virtual override {
    ...
    function _buyTokenErc20(uint256 amountUsd, address tokenAddress) private {
        ...
        require(allowance >= amountUsd, "Crowdsale: amount exceeds allowance");
        ...
        uint256 amountWsi = (amountUsd * 1000) / price();
...
contract VestingManager is IVestingManager, Ownable {
    ...
    uint256 private _unlockPercentage;
    ...
    function unlockPercentage() public view virtual override returns (uint256) {
        return _unlockPercentage;
    }

    function setStartTimestamp(uint256 value)
        public
        virtual
        override
        onlyOwner
    ...
    function setDuration(uint256 value) public virtual override onlyOwner {
        _duration = value;
    }

    function setUnlockPercentage(uint256 value)
        public
        virtual
        override
        onlyOwner
    {
        _unlockPercentage = value;
    }
}