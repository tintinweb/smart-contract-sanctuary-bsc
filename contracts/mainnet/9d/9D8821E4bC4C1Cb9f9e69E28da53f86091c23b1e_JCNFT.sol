...
contract JCNFT is ERC721Enumerable, Ownable {
    struct SaleInfo {
        uint256 property;
        uint256 priceUSDT;
        uint256 priceToken;
    ...
    function buy(uint256 saleId, uint256 num) external {
        address account = msg.sender;
        SaleInfo storage saleInfo = _saleInfo[saleId];
        uint256 usdtAmount = saleInfo.priceUSDT * num;
        _takeToken(_usdtAddress, account, usdtAmount);
        uint256 tokenAmount = saleInfo.priceToken * num;
        _takeToken(_tokenAddress, account, tokenAmount);
        _batchMint(account, saleInfo.property, num);
    }
    ...
    function getSaleInfo(uint256 saleId) public view returns (
        ...
        priceUsdt = saleInfo.priceUSDT;
        priceToken = saleInfo.priceToken;
    }
    ...
    function getAllSaleInfo() public view returns (
        ...
        priceToken = new uint256[](len);
        for (uint256 i; i < len;) {
            (property[i], priceUsdt[i], priceToken[i]) = getSaleInfo(i);
        unchecked{
            ++i;
        }
        }
    ...
    function getNFTList(address account, uint256 start, uint256 length) external view returns (uint256 returnLen, uint256[] memory ids, uint256[] memory properties){
    ...
    function _batchMint(address to, uint256 property, uint256 num) private {
    ...
    function setSaleProperty(uint256 saleId, uint256 property) external onlyOwner {
        SaleInfo storage saleInfo = _saleInfo[saleId];
        saleInfo.property = property;
    }
    
    function setSalePrice(uint256 saleId, uint256 priceUSDT, uint256 priceToken) external onlyOwner {
        SaleInfo storage saleInfo = _saleInfo[saleId];
        saleInfo.priceUSDT = priceUSDT;
        saleInfo.priceToken = priceToken;
    }
    ...
    function setTokenUri(uint256 property, string memory uri) external onlyOwner {
        _uris[property] = uri;
    }
    ...
    function setCash(address cash) external onlyOwner {
        _cash = cash;
    }
    ...
    constructor() ERC721("JC NFT", "JC NFT"){
        _cash = address(0x5a5260FE654f4064c0eF72Bc07d6cDBDd48ae601);
        _usdtAddress = address(0x55d398326f99059fF775485246999027B3197955);
        _tokenAddress = address(0x451D8d18759916b091a66F9B669a0FFE567D8322);
        _baseId[1] = 100001;
        _baseId[2] = 200001;
        _baseId[3] = 300001;

        uint256 usdtUnit = 10 ** IERC20(_usdtAddress).decimals();
        uint256 tokenUnit = 10 ** IERC20(_tokenAddress).decimals();
        _saleInfo.push(SaleInfo({
        property : 1,
        priceUSDT : 60 * usdtUnit,
        priceToken : 50 * tokenUnit
        }));
        _saleInfo.push(SaleInfo({
        property : 2,
        priceUSDT : 120 * usdtUnit,
        priceToken : 100 * tokenUnit
        }));
        _saleInfo.push(SaleInfo({
        property : 3,
        priceUSDT : 180 * usdtUnit,
        priceToken : 150 * tokenUnit
        }));
    }
}