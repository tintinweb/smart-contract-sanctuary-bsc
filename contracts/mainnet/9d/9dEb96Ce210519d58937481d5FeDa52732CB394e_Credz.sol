...
contract Credz is ERC20, Ownable {
    uint public maxTokensTx;
    
    IUniswapV2Router02 public uniswapV2Router;
    address public  uniswapV2Pair;
    address public _rewWallet;
    address public _opeWallet;
    address public _liqWallet;
    address public _marWallet;
    ...
    event FeeUpdated(uint8 liqFee);
    event MinTokensBeforeSwapUpdated(uint128 minTokensBeforeSwap);
    event MaxTokensPerTxUpdated(uint maxTokensPerTx);
    ...
    event ExcludeFromFee(address address_,bool enable_);
    event ExcludeFromMaxTx(address address_,bool enable_);
    ...
    constructor(
        ...
        ///@dev create pair this coint with WBNB
        uniswapV2Pair = IUniswapV2Factory(0xcA143Ce32Fe78f1f7019d7d551a6402fC5350c73).createPair(
            address(this), 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);
        maxTokensTx = 350e18;
        pinkAntiBot = IPinkAntiBot(pinkAntiBot_);
        pinkAntiBot.setTokenOwner(msg.sender);
        antiBotEnabled = true;
        excludeFee[msg.sender] = true;
        excludeMaxTx[msg.sender] = true;
    }


    function setExcludeFromMaxTx(address address_, bool enable_) external onlyOwner {
        excludeMaxTx[address_] = enable_;
        emit ExcludeFromMaxTx(address_,enable_);
    }

    function setExcludeFromFee(address address_, bool enable_) external onlyOwner {
        excludeFee[address_] = enable_;
        emit ExcludeFromFee(address_,enable_);
    }
    ...
    function setPair(address pairAddress_) external onlyAdmin {
        uniswapV2Pair = pairAddress_;
    }

    function setAdminContract(address adminAddress_) external onlyAdmin {
        _cyverseAdmin = ICAdmin(adminAddress_);
    }

    function getAdminContract() external view onlyAdmin returns (address) {
        return address(_cyverseAdmin);
    }
    ...
    function saveTaxFee(address from_, uint amount_, uint feePercentage_) internal returns (uint) {
        ...
        if (taxToRewardPool) { //if tax to reward pull then don't do Liquidity
            ...
            super._transfer(from_, _liqWallet, lfee);
        }
        return (amount_ - lfee);
    }
    
    ///TODO: REVIEW REMOVE GAMERTRADE, RELIQ, 
    function _transfer(
        address from,
        address to,
        uint amount
        ...
        if (maxTokensTx > 0) {
            if (!(excludeMaxTx[from] || excludeMaxTx[to])) {
                require(amount <= maxTokensTx,"LIMIT EXCEEDED");
            }
        }
        if (antiBotEnabled) {
        ...
        uint balanceAmount = amount;
        if (fee > 0) {
            balanceAmount = saveTaxFee(from, amount, fee);
        }
    ...
    function inGamePurchase(address payee_, uint amount_, PurchaseDistribution memory distSplit_)
        external
        returns (bool)
    {   ///TODO: Change margeting to get reward pool get left
        ...
        uint rewardPool = amount_ - (marketing + operations + liquidity); //distSplit should be equal to 100 so get rest to make sure we don't leave fractions out
    ...
    function setWallets(address rewWallet_, address opeWallet_, address liqWallet_, address marWallet_) external onlyAdmin {
        _rewWallet = rewWallet_;
        _opeWallet = opeWallet_;
        _liqWallet = liqWallet_;
        _marWallet = marWallet_;
    ...
    ////////////// Trading Rules //////////////////////////////////////////
    function addToBlackList(address walletAddress_) external onlyAdmin {
        blacklisted[walletAddress_] = true;
    }

    function removeFromBlackList(address walletAddress_) external onlyAdmin {
        delete blacklisted[walletAddress_];
    }

    function updateFee(bool taxToRewardPool_, uint8 liqFee_) public onlyAdmin {
        require(liqFee < 31, "INVALID"); /// @Dev max tax can be 30
        taxToRewardPool = taxToRewardPool_;
        liqFee = liqFee_;
        emit FeeUpdated(liqFee_);
    }

    function updateMaxTokensPerTx(uint maxTokensPerTx_) external onlyOwner {
        maxTokensTx = maxTokensPerTx_  * (10 ** decimals());
        emit MaxTokensPerTxUpdated(maxTokensTx);
    }
    //////////////End Trading Rules //////////////////////////////////////////////////////////////
}
