...
contract BuyMlonFixed is ReentrancyGuard, Ownable{
    ...
    constructor(address _usdt, address _mlon, address _treasury) public {
        usdt = IBEP20(_usdt);
        mlon = IBEP20(_mlon);
        treasury = _treasury;
    }
    ...
    function buyMLON(uint256 _amount) public nonReentrant{
        require(usdt.balanceOf(msg.sender) >= _amount, "buyMLON: user has insufficient mgm to convert all");
        require(mlon.balanceOf(address(this)) == 0, "buyMLon: please top up contract");
        require(_amount != 0, "buyMlon: amount is 0");
        usdt.transferFrom(msg.sender, treasury, _amount);
        mlon.transfer(msg.sender, _amount);
    }
