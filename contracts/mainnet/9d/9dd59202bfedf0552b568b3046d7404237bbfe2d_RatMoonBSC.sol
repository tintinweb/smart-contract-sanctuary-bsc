...
contract ERC20 is Context, IERC20, IERC20Metadata {
	...
	function _firstSupply(address account, uint256 amount) internal virtual {
...
contract RatMoonBSC is ERC20, Ownable {
	...
	constructor (
		...
		uint256 _supply = 1000000000000000 * (10 ** 9);
		
        swapTokensAtAmount = _supply / 10000000;
        _firstSupply(owner(), _supply);
    ...
    function updateFees(uint256 _buyFee, uint256 _sellFee) external onlyOwner {
        require(_buyFee <= 7 && _sellFee <= 7, "Fees must be less than 7%");
        buyFee = _buyFee;
        sellFee = _sellFee;
    ...
    function _transfer(
		...
		if(!launched() && to == uniswapV2Pair && amount > 0){
			launch(); 
		} else {
			if(from == uniswapV2Pair){
				require(block.number > launchedAt + 2, "ERC20: anti snip bot transaction");
			}
		}
    ...
    modifier _maxWalletAvailable() {
        require(maxWalletAvailable, "Max wallet limit is 8%");
        _;
    }
    ...
    function setMaxWalletRate_Denominator1000(uint256 _val) external onlyOwner _maxWalletAvailable {
        require(_val >= 50, "Max wallet percentage cannot be lower than 2%");
    ...
    modifier _maxTransactionAvailable() {
        require(maxTransactionAvailable, "Max transaction limit is 8%");
        _;
    }
