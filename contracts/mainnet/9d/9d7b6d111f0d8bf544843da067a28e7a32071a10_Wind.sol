...
contract Wind  is IBEP20, Auth {
    ...
    uint256 development_divisor = 10;
    ...
    function setstartSwap(uint256 _input) external authorized {
        require (!swapEnabled, "Cannot call this function again once trading is enabled");
        startSwap = true; botOn = true; startedTime = block.timestamp.add(_input);
        }
    function setSwapBackSettings(bool enabled, uint256 _threshold) external authorized {
        require (_threshold >= ( _totalSupply * 10 ) / 100000, "Threshold should be greater than 0.1%");
        swapEnabled = enabled; swapThreshold = _threshold;
        }
    ...
    function taketotalFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        uint256 dAmount = feeAmount.mul(developmentFee).div(totalFees);
        uint256 cAmount = feeAmount.sub(bAmount).sub(dAmount);
        if(bAmount > 0){
        _balances[address(DEAD)] = _balances[address(DEAD)].add(bAmount);
        emit Transfer(sender, address(DEAD), bAmount);}
        if(dAmount > 0){
        _balances[address(token_receiver)] = _balances[address(token_receiver)].add(dAmount);
        emit Transfer(sender, address(token_receiver), dAmount);}
        if(cAmount > 0){
    ...
    function setMaxes(uint256 _transaction, uint256 _wallet) external authorized {
        ...
        require(newTx >= _totalSupply.mul(10).div(1000) && newWallet >= _totalSupply.mul(10).div(1000), "Max TX and Max Wallet cannot be less than 1%");
    }
    ...
    function rescueBEP20(address _tadd, address _rec, uint256 _amt) external authorized {
        require(_tadd != address(this), "Cannot withdraw the native tokens");
    ...
    function setDivisors(uint256 _distributor, uint256 _development, uint256 _liquidity, uint256 _marketing) external authorized {
        distributor_divisor = _distributor;
        development_divisor = _development;
    ...
    function setStructure(uint256 _liq, uint256 _mark, uint256 _dev, uint256 _burn, uint256 _tran) external authorized {
        liquidityFee = _liq;
        marketingFee = _mark;
        developmentFee = _dev;
        burnFee = _burn;
        transferFee = _tran;
        totalFee = liquidityFee.add(marketingFee).add(developmentFee).add(burnFee).add(transferFee);
        require(totalFee <= feeDenominator.div(10), "Tax cannot be more than 10%");
    }

    function setInternalAddresses(address _marketing, address _team1, address _team2, address _team3, address _team4, address _devp, address _token, address _default) external authorized {
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap {
        uint256 denominator= (liquidity_divisor.add(development_divisor).add(marketing_divisor).add(distributor_divisor)) * 2;
        ...
        uint256 xrAmt = unitBalance.mul(2).mul(development_divisor);
        if(xrAmt > 0){
          payable(development_receiver).transfer(xrAmt); }
    }
