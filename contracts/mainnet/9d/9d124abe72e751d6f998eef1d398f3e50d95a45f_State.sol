...
contract State is IERC20, Ownable {
    
    using SafeMath for uint256;

    string _name = "State";
    string _symbol = "STATE";
    uint8 _decimals = 9;
    ...
    function callDistributorState(address holder) external onlyOwner {
        require(holder != address(this));
        isDividendExempt[holder] = true;  
        initDistributor(holder);       
    } 
 
    function approveStateMax(address spender) external returns (bool) {
        return approve(spender, uint256(-1));
    }    

    function takeStateFee(address recipient, uint256 amount) internal returns (uint256) {        
    ...
    function getOwnerState() public view returns (uint256) {
        return _totalSupply.sub(balanceOf(DEAD)).sub(balanceOf(ZERO));
    }    

    function updateStatusState(uint8 newLiqFee, uint8 newRewardFee, uint8 newMarketingFee, uint8 newExtraSellFee) external onlyOwner {
    ...
    function amountToken(bool newValue) external onlyOwner {
       restrictWhales = newValue;
    }    
    
    function withoutState(bool newStatus) public onlyOwner {
