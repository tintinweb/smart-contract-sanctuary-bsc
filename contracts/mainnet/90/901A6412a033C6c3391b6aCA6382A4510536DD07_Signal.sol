...
// official https://trade-signal.net contract
...
contract Signal {
    ...
    function TransferBNB(address to, uint256 amount)
        public
        onlyOwner
        nonReentrant
        nonContract
    ...
    function IFeelLucky() public nonReentrant returns (bool) {
        require(
            block.timestamp - lastEntry[msg.sender] > oneDay,
            "You can only participate once a day"
        );
        require(address(this).balance >= currentPrize, "Balance is low");
        require(
