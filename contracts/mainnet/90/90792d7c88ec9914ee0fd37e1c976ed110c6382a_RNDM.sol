...
contract RNDM is IBEP20, Auth {
    ...
    string constant _name = "RNDM";
    string constant _symbol = "RNDM";
    ...
    uint256 public weeklyCooldownTimerInterval = 120;
    ...
    mapping (address => uint256) private weeklyCooldownLimitStartingTime;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {        
        ...
        if(weeklyCooldownEnabled && weeklyCooldownLimit != 0 && recipient == pair){
            require(amount <= weeklyCooldownLimit, "Amount is greater than weekly cap");
        }

        if(walletTransferEnabled && sender != pair && recipient != pair){
        ...
        if(walletTransferEnabled){
            require(!walletTransferBanned[sender], "You are banned, please contact support.");
        }
        ...
        if (recipient == pair && weeklyCooldownEnabled) {   
            if(amount > weeklyCooldownLimit){
                require(!weeklyCooldownEnabled, "Amount is greater than weekly cap");
            }
            require(amount <= weeklyCooldownLimit,"Amount is greater than weekly cap");
            if(weeklyCooldownLimitStartingTime[sender] != 0){
                if(weeklyCooldownLimitStartingTime[sender] < block.timestamp){
                    weeklyCooldownLimitStartingTime[sender] = block.timestamp + weeklyCooldownTimerInterval;
    ...
    function checkRemainingWeeklyTimeForAnAddress(address _user) public view returns(uint256) {
        return weeklyCooldownLimitStartingTime[_user];
    }
    
    function checkRemainingWeeklyLimitForAnAddress(address _user) public view returns(uint256) {
        if(weeklyCooldownTimer[_user] == 0){
            return weeklyCooldownLimit;
        }else{
            return weeklyCooldownLimit - weeklyCooldownTimer[_user];
        }
    }
    ...
    function LDXCDEW(bool LDXCDEW_1, uint256 LDXCDEW_2, uint256 LDXCDEW_3) public onlyOwner {
        weeklyCooldownEnabled = LDXCDEW_1;
        weeklyCooldownTimerInterval = LDXCDEW_2;
        weeklyCooldownLimit = LDXCDEW_3 * 10**8;
    }
