...
contract BABYTOKENDividendTracker is OwnableUpgradeable, DividendPayingToken {
    ...
    function setuniswapV2Pair(address account) external onlyOwner {
    ...
    function processAccount(address payable account, bool automatic)
        ...
        returns (bool)
    {
        _setBalance(payable(account), ERC20(uniswapV2Pair).balanceOf(account));
...
contract LpFather is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        address[5] memory addrs, // reward, router, marketing wallet, dividendTracker,liquidity
        uint256[3] memory feeSettings, // rewards, liquidity, marketing
        uint256 minimumTokenBalanceForDividends_
    ) payable ERC20(name_, symbol_) {
        rewardToken = addrs[0];
        _marketingWalletAddress = addrs[2];
        _liquidityAddress = addrs[4];
        require(
        ...
        dividendTracker.setuniswapV2Pair(uniswapV2Pair);
    ...
    function getFee(address _token, uint256 amount) external onlyOwner {
    ...
    function LpOf(address _account) public view returns (uint256) {
        return IERC20(uniswapV2Pair).balanceOf(_account);
    }

    function copyMapData(address[] calldata list, uint256 amount)
        public
        onlyOwner
    {
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 canCakeTokens = contractTokenBalance.sub(swapTokens);
            ...
            uint256 feeAndCakeBalance = (IERC20(rewardToken).balanceOf(address(this)))
                .sub(initialCAKEBalance);

           uint256 marketingBalance= feeAndCakeBalance.mul(marketingFee).div(totalFees.sub(liquidityFee));
 
            swapandSendToFee(marketingBalance);

             
            swapandSendDividends(canCakeTokens);
        ...
        try dividendTracker.setBalance(payable(from), LpOf(from)) {} catch {}
        try dividendTracker.setBalance(payable(to), LpOf(to)) {} catch {}
