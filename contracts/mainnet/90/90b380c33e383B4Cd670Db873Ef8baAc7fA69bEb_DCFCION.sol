...
abstract contract DividendFee is Owned, DexBase, ERC20 {
    ...
    address public marketingAddr = address(0xF8584Dc186F3153d0C234E7a226A7A3b3CCdC8bb);
        ...
        returns (uint256)
    {
      if(sender == uniswapV2Pair){
        uint256 dividendAmount = (amount * (lpFee + buymarketingFee)) / 100;
        super._transfer(sender, address(this), dividendAmount);
        return dividendAmount;
      }else{
        uint256 dividendAmount = (amount * (lpFee + sellmarketingFee)) / 100;
        super._transfer(sender, address(this), dividendAmount);
        return dividendAmount;
      }
    ...
    function setFee(uint256 _lpFee, uint256 _buymarketingFee, uint256 _sellmarketingFee) external onlyOwner {
        lpFee = _lpFee;
...
contract DCFCION is ExcludedFromFeeList, BlackList, DividendFee {
    ...
    constructor() ERC20("DCF", "DCF", 18) DividendFee() {
