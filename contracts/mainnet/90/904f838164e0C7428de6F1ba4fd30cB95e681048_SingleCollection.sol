...
/*     modifier canSetUpdateOperator(uint256 estateId) {
        ...
        require(isApprovedOrOwner(msg.sender, estateId), "Unauthorized user");
        _;
    } */
    ...
    function mint(uint256 tokenId, uint8 v, bytes32 r, bytes32 s, Fee[] memory _fees, string memory tokenURI, uint256 nonce) public  {
...
/*     function _isApprovedOwnerOrConsumer(uint256 tokenId) internal view returns (bool) {
    ...
    function removeConsumer(uint256 _tokenId) external {
        address owner = this.ownerOf(_tokenId);
        require(msg.sender == owner || msg.sender == getApproved(_tokenId) || isApprovedForAll(owner, msg.sender),
            "ERC721Consumable: changeConsumer caller is not owner nor approved");
        address _consumer = address(0);
    ...
    * @param tokenId - Estate id
    * @param operator - address of the account to be set as the updateOperator
    */
 /*    function setUpdateOperator(uint256 tokenId, address operator) public canSetUpdateOperator(tokenId){
        _tokenConsumers[tokenId] = operator;
        emit UpdateOperator(tokenId, operator);
    } */
    ...
    * @param tokenId Estate id to update
    * @param metadata string metadata
    */
    function updateMetadata(uint256 tokenId, string calldata metadata) external onlyUpdateAuthorized(tokenId){
        _updateMetadata(tokenId, metadata);
        emit Update(tokenId, ownerOf(tokenId), msg.sender, metadata);
    }
    ...
    * @param metadata string metadata
    */
    function _updateMetadata(uint256 tokenId, string memory metadata) internal {
...
contract SingleUserCollection is SingleCollection {
    ...
    ///        The URI must link to JSON object with various fields: `name, description, image, external_url, attributes`.
    ///        Can also contain another various fields.
    function mint(uint256 tokenId, Fee[] memory _fees, string memory tokenURI, uint256 nonce) onlyOwner public {
        require(!usedNonces[nonce], "Nonce should be valid");
        usedNonces[nonce] = true;
        _mint(msg.sender, tokenId, _fees);
        _setTokenURI(tokenId, tokenURI);
