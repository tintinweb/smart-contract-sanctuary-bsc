...
contract CoinFlip is Ownable {
    ...
    function setPayOutDivisor(uint256 newPayOutDivisor) external onlyOwner {
        require(newPayOutDivisor >= 50 && newPayOutDivisor <= 100, "must be between 50 and 100");
    ...
    function flipTheCoin(uint256 _betAmount, bool chooseHeads) internal returns (bool resultIsHeads, bool isWinner) {
        ...
        uint256 result = rng();
        if(result == 0) resultIsHeads = false;
        if(chooseHeads == resultIsHeads) isWinner = true;

        if(isWinner) {
            IERC20(gameToken).transfer(msg.sender, (_betAmount * payOutDivisor * 100));
