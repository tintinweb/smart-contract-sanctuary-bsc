...
contract BNBWheel {
    ...
    function withdraw() external {
        ...
        uint256 holdBonus = player.dividends * timeMultiplier / PERCENTS_DIVIDER;

        uint256 amount = player.dividends + player.match_bonus + holdBonus;
    ...
    function playerInfo(address _addr) view external returns(uint256 for_withdraw, uint256 total_invested, uint256 total_withdrawn, uint256 total_match_bonus, uint256[BONUS_LINES_COUNT] memory structure, uint256 _checkpoint,uint256 _holdBonus) {
        Player storage player = players[_addr];
        ...
        uint256 timeMultiplier =(block.timestamp - player.checkpoint) / (TIME_STEP) * (100); //10% per day
        uint256 holdBonus = player.dividends * timeMultiplier / PERCENTS_DIVIDER;
