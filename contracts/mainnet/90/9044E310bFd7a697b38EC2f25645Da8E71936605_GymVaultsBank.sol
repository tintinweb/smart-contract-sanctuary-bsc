    ...
    RewardRateConfigurable
{
    ...
    function claimAlpacaRewards(uint256 _pid) external {
        PoolInfo memory pool = poolInfo[_pid];
        require(userInvestment[_pid][msg.sender] > 0, "GymVaultsBank:: User doesn't have enough shares");
        IStrategy(pool.strategy).claimAlpacaRewards(msg.sender);
    }
    
    function seedUsers(uint256 _pid, address[] calldata _users) external onlyOwner {
        PoolInfo memory pool = poolInfo[_pid];
        for (uint256 i; i < _users.length; i++) {
            UserInfo memory user = userInfo[_pid][_users[i]];
            IStrategy(pool.strategy).setUserShares(_users[i], user.shares);
        }
    }
    ...
    function _deposit(uint256 _pid, uint256 _wantAmt) private {
        ...
        if (_wantAmt > 0) {
            ...
            IStrategy(pool.strategy).updateUser(userInvestment[_pid][msg.sender], msg.sender);
    ...
    function _withdraw(uint256 _pid) private {
        ...
        //TODO: change before deployment
        if (
            !ICommissionActivation(0x3E1240E879b4613C7Ae6eE1772292FC80B9c259e).getCommissionActivation(msg.sender, 1)
        ) {
...
interface IStrategy {
    ...
    function claimAlpacaRewards(address _user) external;
