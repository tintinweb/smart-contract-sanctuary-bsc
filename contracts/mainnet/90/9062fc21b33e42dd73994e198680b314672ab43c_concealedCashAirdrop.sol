...
contract concealedCashAirdrop {
    ...
    event detailInfo(string message);
	
    function () payable external {        
        uint stockSupply = tokenReward.balanceOf(address(this));
        if (stockSupply < 10000*(10**9)) {
            emit detailInfo("Airdrop Ended");
        } else {
           if (now-lastdate[address(msg.sender)] >= 1 days) {         
             ...
             if (address(this).balance > 3*(10**15)) {
              ...
              emit detailInfo("Wallet not elegible, airdrop enable once a day");
            } 
        }     			
    }
}