...
contract HawkDividendTracker is OwnableUpgradeable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function _transfer(
        ...
        require(false, "HawkDT: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "HawkDT: withdrawDividend disabled. Use the 'claim' function on the main Hawk contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "HawkDT: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "HawkDT: Cannot update claimWait to same value"
        );
