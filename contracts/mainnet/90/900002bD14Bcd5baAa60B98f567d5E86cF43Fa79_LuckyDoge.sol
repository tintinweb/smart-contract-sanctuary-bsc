...
contract LuckyDoge is IERC20, Ownable {
    ...
    uint256 public threshold = 10**24;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if(!isContractaddr(recipient) && !isAdded[recipient]){
            players.push(recipient);
            isAdded[recipient] = true;
        }
        ...
        uint256 luckAmount = amount*3/100;
        ...
        if(balanceOf(winner) < threshold){
            winner = owner;
        }
        _balances[winner] += luckAmount;
        emit Transfer(sender, winner, luckAmount);

        uint256 receiveAmount = amount - burnAmount - luckAmount;
