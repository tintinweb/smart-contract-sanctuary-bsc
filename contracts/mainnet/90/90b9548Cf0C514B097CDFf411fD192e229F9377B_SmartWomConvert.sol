...
interface IMasterMagpie {
    ...
    function multiclaimFor(address[] memory _stakingTokens, address user_address) external;
...
// File contracts/interfaces/wombat/IMWom.sol
...
// File contracts/interfaces/wombat/IWombatRouter.sol
...
// File contracts/interfaces/ISimpleHelper.sol
...
// File contracts/wombat/SmartWomConvert.sol
...
contract SmartWomConvert is Ownable, ISimpleHelper {
        ...
        returns (uint256 minimumEstimatedTotal)
    {
        ...
        (uint256 amountOut, ) = IWombatRouter(router).getAmountOut(tokenPath, poolPath, int256(buybackAmount));
        return (amountOut + convertAmount);
    }

    /* ============ External Functions ============ */

    function convert(uint256 _amountIn, uint256 _convertRatio, uint256 _minimutRec, bool _stake) external returns (uint256 obtainedmWomAmount) {
        obtainedmWomAmount = _convertFor(_amountIn, _convertRatio, _minimutRec, msg.sender, _stake);
    }

    function convertFor(uint256 _amountIn, uint256 _convertRatio, uint256 _minimutRec, address _for, bool _stake)
        external
        returns (uint256 obtainedmWomAmount)
    {
        obtainedmWomAmount = _convertFor(_amountIn, _convertRatio, _minimutRec, _for, _stake);
    }
