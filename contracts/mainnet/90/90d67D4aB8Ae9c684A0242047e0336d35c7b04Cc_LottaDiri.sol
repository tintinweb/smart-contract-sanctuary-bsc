...
contract LottaDiri is ERC20, Ownable {
  ...
  /*  function updateTransferFee(uint256 newTransferFee) public onlyOwner {
    ...
    function distributeBuy(uint256 tokens) private{
        ...
        IERC20(buyTaxToken).transfer(address(buyLottery), (distributions*buyLotteryFee)/100);
        IERC20(buyTaxToken).transfer(address(DEAD), (distributions*buyBurnFee)/100);
       
        swapping = false;
    }


     function distributeSell(uint256 tokens) private{

        swapTokensForSOLCash(tokens);

        uint256 distributions = IERC20(sellTaxToken).balanceOf(address(this));

        IERC20(sellTaxToken).transfer(address(sellLottery), (distributions*buyLotteryFee)/100);
        IERC20(sellTaxToken).transfer(address(DEAD), (distributions*buyBurnFee)/100);
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFees[to] && to != uniswapV2Pair){
            require(balanceOf(to).add(amount) <= maxWallet, "Recipient balance is exceeding maxWalletBalance");
        }
        ...
        if (!swapping && !_isExcludedFromFees[from] && !_isExcludedFromFees[to]) 
        {
           ...
           isBuying = automatedMarketMakerPairs[from];
            ...
            bool canSwap = contractTokenBalance >= swapTokensAtAmount;
             ...
             {
                 swapping = true;
                 distributeSell(contractTokenBalance);
             }
             else if(canSwap && !isSelling)
             {
                 swapping = true;
                 distributeBuy(contractTokenBalance);
             }
