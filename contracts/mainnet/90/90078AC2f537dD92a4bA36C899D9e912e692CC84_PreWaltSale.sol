...
contract PreWaltSale is Ownable {
    ...
    IERC20 public pWalt;
    address public DAO;
    uint256 public pWaltPrice;
    uint256 public pWaltSold;
    ...
    function initialize(
        ...
        bool whiteListEnable_
    ) external onlyOwner {
        require(!initialized, "Already initialized");
        require(minBuyAmount < maxBuyAmount_, "Not valid");
        pWalt = IERC20(pWalt_);
        busd = IERC20(busd_);
        pWaltPrice = pWaltPrice_;
        minBuyAmount = minBuyAmount_;
        maxBuyAmount = maxBuyAmount_;
        DAO = dao_;
        whiteListEnabled = whiteListEnable_;
        initialized = true;
    }

    function setPreWaltPrice(uint256 newPreWaltPrice_)
        external
        onlyOwner
        returns (uint256)
    {
        pWaltPrice = newPreWaltPrice_;
        return pWaltPrice;
    }
    ...
    function buy(uint256 amountPaid_) external returns (bool) {
        if (whiteListEnabled) {
            require(approvedBuyers[msg.sender], 'Buyer not approved.');
        }
        require(!purchased[msg.sender], "Already purchased");
        require(amountPaid_ >= minBuyAmount, 'Amount too low');
        require(amountPaid_ <= maxBuyAmount, 'Amount too high');
        uint256 pWaltAmountPurchased = amountPaid_.div(pWaltPrice) * 1e18;
        busd.safeTransferFrom(msg.sender, DAO, amountPaid_);
        pWalt.safeTransfer(msg.sender, pWaltAmountPurchased);
        emit Purchase(amountPaid_, msg.sender);
        purchased[msg.sender] = true;
        pWaltSold += amountPaid_;
        return true;
    }
