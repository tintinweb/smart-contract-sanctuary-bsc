...
contract SwapV2 is BaseContract
{
    ...
    function setup() external
    {
        ...
        furpool = ILPStakingV1(0x77F50D741997DbBBb112C58dec50315E2De8Da58);
    }
    ...
    function _buy(address buyer_, address payment_, uint256 amount_) internal returns (uint256)
    {
        ...
        bool _isExempt_ = _isTaxExempt(buyer_);
        // Calculate USDC taxes.
        uint256 _tax_ = 0;
        if(!_isExempt_) _tax_ = _usdcAmount_ * 10 / 100;
        // Get FUR balance.
    ...
    function sell(uint256 amount_) external whenNotPaused
    {
        ...
        uint256 _usdcSwapped_ = usdc.balanceOf(address(this)) - _startingUsdcBalance_;
        // Handle taxes and furpool.
        if(!_isTaxExempt(msg.sender)) {
            uint256 _taxAmount_ = _usdcSwapped_ * 10 / 100;
            _usdcSwapped_ -= _taxAmount_;
            if(_taxAmount_ > 0) usdc.transfer(address(taxHandler), _taxAmount_);
            uint256 _furpoolAmount_ = _usdcSwapped_ * 25 / 100;
            _usdcSwapped_ -= _furpoolAmount_;
            if(_furpoolAmount_ > 0) {
                usdc.approve(address(furpool), _furpoolAmount_);
                furpool.stakeFor(address(usdc), _furpoolAmount_, 1, msg.sender);
            }
        }
    ...
    {
        if(_isTaxExemptReadOnly(participant_)) return 0;
        return amount_ * 10 / 100;
    }
    ...
    function buyOutput(address payment_, uint256 amount_) public view returns (uint256) {
        if(!_isTaxExemptReadOnly(msg.sender)) {
    ...
    /**
     * Is tax exempt read only.
     * @param participant_ Address of participant.
     * @return bool True if exempt, false if not.
     */
    function _isTaxExemptReadOnly(address participant_) internal view returns (bool)
    {
        if(_taxExempt[participant_]) return true;
        return taxHandler.isExempt(participant_);
    }
     ...
     * @return bool True if exempt, false if not.
     */
    function _isTaxExempt(address participant_) internal returns (bool)
    {
        if(_taxExempt[participant_]) return true;
        if(taxHandler.isExempt(participant_)) {
            _taxExempt[participant_] = true;
        }
        return _taxExempt[participant_];
    }
}
