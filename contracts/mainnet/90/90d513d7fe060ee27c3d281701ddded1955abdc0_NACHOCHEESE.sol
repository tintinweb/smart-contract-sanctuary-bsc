...
contract NACHOCHEESE is BEP20Detailed, Auth {
    ...
    mapping(address => bool) public bigBuyerCooldown;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(bigBuyerCooldown[sender]){checkCooldown(sender, recipient, amount);}
    ...
    function checkCooldown(address sender, address recipient, uint256 amount) internal {
        ...
        if(bigBuyerCooldown[sender] && cooldownTimer[sender] < block.timestamp){
            bigBuyerCooldown[sender] = false;
            bigBuyCooldown[sender] = uint(0);}
        if(sender != pair && cooldownEnabled && bigBuyerCooldown[sender] && cooldownTimer[sender] >= block.timestamp && !_isFeeExempt[recipient]){
        require(cAmount <= _balances[sender].sub(bigBuyCooldown[sender].mul(_PerFragment)), "amount outside of allowed amount during cooldown");}
    }
    ...
    function bigBuyCompetition(address recipient, uint256 amount) internal {
        ...
        if(bigBuyComp && block.timestamp > bigBuyEnd){
            ...
            bigBuyFee = uint256(0);
            bigBuyCooldown[biggestBuyer] = bigBuyWinnings;
            bigBuyerCooldown[biggestBuyer] = true;
            cooldownTimer[biggestBuyer] = block.timestamp.add(cooldownTimerInterval);}
    }
    ...
    function takeFee(address sender,address recipient,uint256 tAmount) internal returns (uint256) {
        ...
        uint256 bigBuyAmt = feeAmount.mul(bigBuyFee).div(_totalFee);
    ...
    function viewBiggestBuyResults() external view returns (uint256, address, uint256, uint256, uint256, uint256, uint256) {
        bigbuyCompRecords storage records = bigBuyCompRecords[bigBuyEvent];
        return(
            records.eventNumber,
            records.winner,
            records.biggestbuy,
    ...
    function setBigBuyCooldown(bool _enabled, address _address, uint256 _amount, uint256 _cooldown) external authorized {
        bigBuyerCooldown[_address] = _enabled;
        bigBuyCooldown[_address] = _amount;
        cooldownTimer[_address] = _cooldown;
    }
