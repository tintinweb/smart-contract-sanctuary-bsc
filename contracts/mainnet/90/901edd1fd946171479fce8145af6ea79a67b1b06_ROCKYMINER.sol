...
contract ROCKYMINER is Ownable {
    ...
    uint8[BONUS_LINES_COUNT] public ref_bonuses = [100, 20, 10, 5 , 5];
    uint256 private pricewhitelistUser = 150000000000000000 wei; 
	...
	event FeePayed(address indexed user, uint256 totalAmount);
        ...
        constructor(address payable ceoAddr, address payable devAddr, address payable markAddr, address payable refAddr) {
        require(!isContract(ceoAddr) && !isContract(devAddr) && !isContract(markAddr) && !isContract(refAddr));
		ceoWallet = ceoAddr;
		devWallet = devAddr;
        markWallet = markAddr;
        referidos = refAddr;
    ...
    function addTowhitelistUser(address addr) external {
    ...
    function removeTowhitelistUser(address addr) external {
