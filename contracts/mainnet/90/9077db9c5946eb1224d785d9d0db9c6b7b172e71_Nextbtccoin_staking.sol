...
contract Nextbtccoin_staking is ERC20 {
    ...
    if(fund.status == 0)
		{
		uint256 nextPayout = (user.lastNonWokingWithdraw>0)?user.lastNonWokingWithdraw + 1 days:user.deposits[0].start;
		...
		uint256 contractBalance = next_TOKEN.balanceOf(address(this));
            if (contractBalance > finalamount) {
                ...
                require(totalAmount >= 100, "Minimum withdrawal 100 NextBtc Coin");
