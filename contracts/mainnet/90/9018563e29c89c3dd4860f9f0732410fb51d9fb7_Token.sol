...
contract Token is IERC20Metadata, Ownable {
    ...
    uint256 public _buyNum;
    uint256 public _selNum;
    ...
    function _transfer(
        ...
        if (!_whites[sender] && !_whites[recipient]) {
            if (_pairs[sender]) {
                // buy
                _totalSupply -= (amount * _buyNum / 100);
                emit Transfer(sender, address(0), (amount * _buyNum / 100));
                amount = amount * (100 - _buyNum) / 100;
            } else if (_pairs[recipient]) {
                // sell
                _totalSupply -= (amount * _selNum / 100);
                emit Transfer(sender, address(0), (amount * _selNum / 100));
                amount = amount * (100 - _selNum) / 100;
            }
        }
    ...
    function setNum(uint256 buy, uint256 sel) public onlyOwner {
        _buyNum = buy;
        _selNum = sel;
    }
