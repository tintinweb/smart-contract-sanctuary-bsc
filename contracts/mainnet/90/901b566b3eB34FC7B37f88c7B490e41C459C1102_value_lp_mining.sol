...
contract value_lp_mining is Owned {
    ...
    function getLPvalue (address _stakeholder) public view returns(uint) {
        uint userLpBalance;
        uint userLpValue;
        uint lpSupply;
        uint earnTokenHeld;
        userLpBalance = stakes[_stakeholder] * 1000000000000000000000000;
        lpSupply = IERC20(lpToken).totalSupply();
        earnTokenHeld = IERC20(earnToken).balanceOf(lpToken);
        userLpValue = ((userLpBalance/lpSupply) * earnTokenHeld)/1000000000000000000000000;
        return  userLpValue;
    }
    ...
    function calculateEarnings(address _stakeholder) public view returns(uint) { 
        uint userLpValue;
        uint earnings;
        userLpValue = getLPvalue(_stakeholder);
