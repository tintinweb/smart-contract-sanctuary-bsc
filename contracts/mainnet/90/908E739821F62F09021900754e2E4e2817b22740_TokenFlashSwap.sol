...
function pancakeCall(address sender, uint amount0, uint amount1, bytes calldata data) external {
       ...
       ( address endRouter, uint repay) = abi.decode(data, (address, uint));
        ...
        IERC20 token =  IERC20(path[0]);
            ...
            uint[] memory minOuts = IPancakeRouter02(endRouter).getAmountsOut(amountToken, path); 
            uint[] memory amountReceived = IPancakeRouter02(endRouter).swapExactTokensForTokens(amountToken, minOuts[1] , path, address(this), block.timestamp);
           
            require(token.transfer(msg.sender, repay),"could not repay"); // return tokens to V2 pair
            token.transfer(sender, token.balanceOf(address(this))); // keep the rest! (tokens)
    }
         function withdrawToken(address _tokenContract) external onlyAdmin {
        IERC20(_tokenContract).transfer(Beneficiary, IERC20(_tokenContract).balanceOf(address(this)));
    }
