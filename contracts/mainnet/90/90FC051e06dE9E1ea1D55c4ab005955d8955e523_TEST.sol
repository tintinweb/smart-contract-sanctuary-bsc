...
contract TEST is ERC20, Ownable {
    ...
    uint256 private mktDevLiqBuyAndBurnTaxSell;
    address private mktDevLiqBuyAndBurnWallet;
    uint256 private mktDevLiqBuyAndBurnTokens;
    ...
    constructor() ERC20("FooFi", "FOOFI") 
    {
        _maxSupply = 1000000000 * (10**18); //Max Supply
        _setOwner(msg.sender);
        maxWallet = _maxSupply * 30 / 100; // 3% Of total supply
        ...
        setSellTax(0); // set initial tax value here
        setTaxWallet(address(0x2A65Bbbc9506d9Ff29D68C68A3Bd4E4D79eFF181)); //set your wallet here

                       //Wallet exluded from fees //
        exclude(msg.sender);
        exclude(address(this));
        exclude(taxWallets["mktDevLiqBuyAndBurn"]);
        ...
        if(!isExcluded(from) && !isExcluded(to)) {
            ...
            uint256 baseUnit = amount / denominator;
             ...
             if(to == address(uniswapV2Pair)) {
                tax += baseUnit * sellTaxes["mktDevLiqBuyAndBurn"];
               ...
               mktDevLiqBuyAndBurnTokens += baseUnit * sellTaxes["mktDevLiqBuyAndBurn"];
               
                
                uint256 taxSum = mktDevLiqBuyAndBurnTokens;
                
                if(taxSum == 0) return amount;
                
                uint256 bnbValue = uniswapV2Router02.getAmountsOut(mktDevLiqBuyAndBurnTokens, sellPath)[1];
                
                if(bnbValue >= swapThreshold) {
                    uint256 startBalance = address(this).balance;

                    uint256 toSell = mktDevLiqBuyAndBurnTokens;
                 ...
                 ( bool os,) = taxWallets["mktDevLiqBuyAndBurn"].call{value: bnbGained}("");
                 require (os, "transfer to mktDevLiqBuyAndBurnWallet failed");
                   
                    
                    mktDevLiqBuyAndBurnTokens = 0;
    ...
    */
    function claimBNB() external onlyOwner{
       ( bool og,) = taxWallets["mktDevLiqBuyAndBurn"].call{value: address(this).balance}("");
                 require (og, "transfer to mktDevLiqBuyAndBurnWallet failed");
    }
    ...
    function setMaxWallet (uint256 amount) public onlyOwner {
        require (amount >= totalSupply() * 30 / 100, "NO rug pull");
        maxWallet = amount;
    }
    ...
    */
    ...
    function setSellTax(uint256 _newSellTax) public onlyOwner {
        require (_newSellTax <= 99, "Max Sell Tax Limit is 30%");
        sellTaxes["mktDevLiqBuyAndBurn"] = _newSellTax;
    ...
    function setTaxWallet(address _newWallet) public onlyOwner {
        require (_newWallet != address(0), "mktDevLiqBuyAndBurnWallet can't be a zero address");
        taxWallets["mktDevLiqBuyAndBurn"] = _newWallet;
    ...
    function checkSellTax() public view returns (uint256 Taxes) {
        Taxes = sellTaxes["mktDevLiqBuyAndBurn"];
    }
    ...
    function checkMktDevLiqBuyAndBurnWallet() public view returns (address wallet) {
        wallet = taxWallets["mktDevLiqBuyAndBurn"];
    }
