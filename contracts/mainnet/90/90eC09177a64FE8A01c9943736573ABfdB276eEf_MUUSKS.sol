...
contract MUUSKS is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  constructor(string memory name, string memory symbol, string memory baseTokenURI) ERC721(name, symbol) {
      ...
      // Save dev wallet
      _wallet = 0x63C0C00B327ea5E930b881AaC5a1D1b614B5876A;
      _regularMint();
  ...
  function mint(uint256 _mintAmount) public payable {
    // Checks
    require(!paused, "MUUSKS: minting has not started yet");
    require(_mintAmount > 0, "MUUSKS: you have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "MUUSKS: you can only mint 10 at a time");
    require(lastSupply >= _mintAmount,  "MUUSKS: the collection is sold out");
    if (msg.sender != owner()) {
      require(msg.value >= cost * _mintAmount, "MUUSKS: total cost isn't match");
    }
    // Minting
  ...
  function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
    // Checks
    require(_winner != address(0), "MUUSKS: zero address");
    require(_amount > 0, "MUUSKS: you have to mint at least one");
    require(lastSupply != 0,  "MUUSKS: the collection is sold out");
  ...
  function tokenURI(uint256 _id) public view virtual override returns (string memory){
    require(_exists(_id),"MUUSKS: URI query for nonexistent token");
