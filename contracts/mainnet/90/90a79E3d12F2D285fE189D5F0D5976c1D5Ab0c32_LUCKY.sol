...
contract LUCKY is IERC20, Ownable {
    ...
    string constant _name = "Lucky Metaverse";
    string constant _symbol = "LMETA";
    ...
    uint256 public buyTotalFees = 14;
    // sell fees
    ...
    uint256 public rewardSwap = 5;
    uint256 public marketingSwap = 5;
    uint256 public liquiditySwap = 2;
    uint256 public lotterySwap = 2;
    ...
    constructor() {
        ...
        address deployer = 0xda13DacA89f20D27358325484F391fA071c00B5C;
        ...
        marketingFeeReceiver = 0x1AFdBd22069cB3c0865f5A0C3413E9949E43dd03;
        lotteryFeeReceiver = 0xeB342FB496bC854E6Aa8FFCbf6aF7c99d3979C28;
        stakePool = 0x68853E350Bf75bc314ecfeeA722A418Ac04E9d36;

        pinkAntiBot.setTokenOwner(deployer);
    ...
    function takeFee(
        ...
        uint256 feeAmount = 0;
        uint256 stakePoolFee = 0;
        if (to == pair) {
            feeAmount = amount.mul(sellTotalFees).div(100);
            if (sellStakePoolFee > 0) {
                stakePoolFee = feeAmount.mul(sellStakePoolFee).div(
                    sellTotalFees
                );
                _balances[stakePool] = _balances[stakePool].add(feeAmount);
                emit Transfer(sender, address(stakePool), stakePoolFee);
            }
        } else {
            feeAmount = amount.mul(buyTotalFees).div(100);
            if (buyStakePoolFee > 0) {
                stakePoolFee = feeAmount.mul(buyStakePoolFee).div(buyTotalFees);
                _balances[stakePool] = _balances[stakePool].add(feeAmount);
                emit Transfer(sender, address(stakePool), stakePoolFee);
            }
        }
        _balances[address(this)] = _balances[address(this)].add(
            feeAmount.sub(stakePoolFee)
        );
        emit Transfer(sender, address(this), feeAmount.sub(stakePoolFee));
    ...
    function updateBuyFees(
        ...
        buyLiquidityFee = lottery;
        buyStakePoolFee = staking;
        buyTotalFees = reward.add(marketing).add(liquidity).add(lottery).add(
            staking
        );
        require(buyTotalFees <= 30, "Fees can not be greater than 30%");
    }
    ...
    function updateSellFees(
        ...
        sellTotalFees = reward.add(marketing).add(liquidity).add(lottery).add(
            staking
        );
        require(sellTotalFees <= 30, "Fees can not be greater than 30%");
    }
    ...
    function updateSwapPercentages(
        ...
        lotterySwap = lottery;
        totalSwap = reward.add(marketing).add(liquidity).add(lottery);
    }
    ...
    function swapBackInBnb() internal swapping {
        ...
        uint256 lotteryBnb = swappedBnbAmount.mul(lotterySwap).div(
            totalSwapFee
        );
        uint256 tokensToReward = swappedBnbAmount.sub(marketingFeeBnb).sub(
            lotteryBnb
        );
        // calculate reward bnb amount
        if (tokensToReward > 0) {
            (bool sendDividendBnb, ) = payable(address(dividendDistributor))
                .call{value: tokensToReward, gas: 30000}("");
            sendDividendBnb = false;
            try dividendDistributor.deposit(tokensToReward) {} catch {}
        }
        ...
        if (lotteryBnb > 0) {
            (bool affiliateSuccess, ) = payable(lotteryFeeReceiver).call{
                value: lotteryBnb,
                gas: 30000
            }("");
