...
// File: contracts\HodlHandsSale.sol
...
contract HodlHandSale is Ownable, ReentrancyGuard, ERC721Holder {
    ...
    address public tokenBnbPair;
    address public tokenReceiver;

    uint256 public discountRate;

	constructor(address _token, address _tokenBnbPair, address _tokenReceiver, uint256 _discountRate) {
		token = IERC20(_token);
        hand = IHODLHAND(0x7E82123bCb6465133D6E9E1Ad94d0115DE041b3D);
        wbnb = IERC20(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);
        tokenBnbPair = address(_tokenBnbPair);
    ...
    function purchase(uint256 _handAmount) public nonReentrant {
		uint256 tokenAmount = _handAmount.mul(getTokenAmountFromOneHand());
		token.safeTransferFrom(msg.sender, tokenReceiver, tokenAmount);
        mintHand(_handAmount);
	}    

    function getTokenAmountFromOneHand() public view returns (uint256) {
        uint256 reserve0 = token.balanceOf(address(tokenBnbPair));
        uint256 reserve1 = wbnb.balanceOf(address(tokenBnbPair));
        uint256 handPrice = hand.getPrice();
        uint256 tokenAmount = handPrice.mul(100-discountRate).div(100).mul(reserve0).div(reserve1);
        return tokenAmount;
    }

    function setDiscountRate(uint256 _discountRate) external onlyOwner {
        require(_discountRate >= 0 && _discountRate <= 100, "This is percent!");
        discountRate = _discountRate;
    }
    ...
    function changeReceiver(address _tokenReceiver) external onlyOwner {
        tokenReceiver = address(_tokenReceiver);
    }
