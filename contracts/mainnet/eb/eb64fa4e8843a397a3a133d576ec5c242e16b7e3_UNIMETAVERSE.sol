...
contract UNIMETAVERSE is Context, IBEP20, Ownable {
    ...
    address public marketingWallet = 0x4888A6f287d83ab6Bb1c74D5a7785EFeD2f6CDFB;
    ...
    string private _name = "Uni Metaverse";
    ...
    function _getLPMultiplier() private view  returns (uint256) {
        if(isSale == true){
            uint256 time_since_start = block.timestamp - _start_timestamp;
            uint256 month = 2629743;

            if (time_since_start < month) {               //42% total fees
                return (6);
            } else if (time_since_start < month*2) {      //32% total fees
                return (4);
            } else if (time_since_start < month*3) {    //22% total fees
                return (2);
            } else {                            //12% total fees
                return (1);
            }
    ...
    function _getTaxMultiplier() private view returns (uint256){
        if(isSale == true) {
            uint256 time_since_start = block.timestamp - _start_timestamp;
            uint256 month = 2629743;
            if(time_since_start < month*3) {
                return (2);
            }
            else{
                return (1);
    ...
    {
        uint256 LPmultiplier = _getLPMultiplier();
        uint256 TaxMultiplier = _getTaxMultiplier();
        uint256 tFee = tAmount.div(10**2).mul(_taxFee).mul(TaxMultiplier);
        uint256 tLiquidity =
            tAmount.div(10**2).mul(_liquidityFee).mul(LPmultiplier);
    ...
    function setRouterAddress(address newRouter) public onlyOwner() {
       ...
       IPancakeRouter02 _newPancakeRouter = IPancakeRouter02(newRouter);
        // Create a uniswap pair for this new token
        pcsV2Pair = IPancakeFactory(_newPancakeRouter.factory()).createPair(address(this), _newPancakeRouter.WETH());
