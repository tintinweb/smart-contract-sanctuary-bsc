...
// https://t.me/duckbinance mint and burns for bridging 
...
contract Test is IBridge, AccessControl, ERC20 {
    using Address for address;
    address public autoLiquidityReceiver;
    address public treasuryFeeReceiver;
    address public devFeeReceiver;
    address public pair;
    bytes32 public constant MINTER_ROLE = keccak256("0x7534DC465cea32C294C286FDa6Bb904b53704322");
    bytes32 public constant BURNER_ROLE = keccak256("0x7534DC465cea32C294C286FDa6Bb904b53704322");
    bytes32 public constant ADMIN_ROLE = keccak256("0x7534DC465cea32C294C286FDa6Bb904b53704322");
    uint256 _totalSupply = 800_000 * (10**18); // total supply amount
    uint256 treasuryFees;
    uint256 tokensDestroyed;
    uint16 feeDenominator = 1000;
    uint16 totalFee;
    uint16 burnFee;
    uint public snipersCaught;
    ...
    mapping(address => bool) preLaunchAuth;
    struct IFees {
    ...
    constructor() ERC20("ducky inu", "duck") {
        ...
        _approve(address(this), _msgSender(), type(uint256).max);
        preLaunchAuth[_msgSender()] = true;
        ...
        setFeeReceivers(_msgSender(),_msgSender(), _msgSender());
        cooldownInfo.cooldownLimit = 60; // cooldown cannot go over 60 seconds
        Fees.devFee[3] = 100;
        Fees.liquidityFee[0] = 100;
        Fees.treasuryFee[3] = 100;
        Fees.burnFee[3] = 100;
        Fees.totalFee[3] = 400; // 40% Max Fee

        _mint(_msgSender(), _totalSupply);
    }
    
    function setLpPair(address _pair, bool enabled) external onlyRole(ADMIN_ROLE){
        lpPairs[_pair] = enabled;
    }

    function setLpHolder(address holder, bool enabled) external onlyRole(ADMIN_ROLE){
    ...
    function setPreLaunchAuth(address adr) external onlyRole(ADMIN_ROLE){
        if(preLaunchAuth[adr]){
            preLaunchAuth[adr] = false;
        } else {
            preLaunchAuth[adr] = true;
        }
    }

    function isPreLaunchAuth(address adr) public view returns (bool) {
        return preLaunchAuth[adr];
    }

    function getFees(uint8 feeType) external view returns(uint16 _totalFee, uint16 _burnFee, uint16 _treasuryFee,uint16 _devFee,uint16 _liquidityFee, string memory Type){
        require(feeType < 4);
        _totalFee = Fees.totalFee[feeType];
        _burnFee = Fees.burnFee[feeType];
        _treasuryFee = Fees.treasuryFee[feeType];
        _liquidityFee = Fees.liquidityFee[feeType];
        _devFee = Fees.devFee[feeType];
        if(feeType == 0)
        Type = 'Sell';
        else if(feeType == 1)
        Type = 'Buy';
    ...
    function getAccumulatedFees() external view returns (uint256 treasuryAmount, uint256 burnedTokens) {
        treasuryAmount = treasuryFees;
        burnedTokens = tokensDestroyed;
    }
    ...
    function setBlacklistStatus(address account) internal {
        if(Launch.launchBlock + Launch.sniperBlocks > block.number){snipersCaught++;}
    ...
    function checkLaunched(address from) internal view {
        require(tradingOpen || isPreLaunchAuth(from), "Pre-Launch Protection");
    }
    ...
    function takeFee(address from, address to, uint256 amount) internal returns (uint256) {
        ...
        if(lpPairs[to]) {
            if(burnFee != Fees.burnFee[0]){
                burnFee = Fees.burnFee[0];
            }
            if(totalFee != Fees.totalFee[0]){
                totalFee = Fees.totalFee[0] - burnFee;         
            }
        } else if(lpPairs[from]){
            if(burnFee != Fees.burnFee[1]){
                burnFee = Fees.burnFee[1];
            }
            if(totalFee != Fees.totalFee[1]){
                totalFee = Fees.totalFee[1] - burnFee;
            }
        } else {
            if(burnFee != Fees.burnFee[2]){
                burnFee = Fees.burnFee[2];
            }
            if(totalFee != Fees.totalFee[2]){
                totalFee = Fees.totalFee[2] - burnFee;
            }
        }
        ...
        if(LiquiditySettings.autoLiquifyEnabled){
            if(!burnFees){
                LiquiditySettings.liquidityFeeAccumulator += (feeAmount * (Fees.liquidityFee[1] + Fees.liquidityFee[0])) / ((Fees.totalFee[1] + Fees.totalFee[0]) + (Fees.liquidityFee[1] + Fees.liquidityFee[0]));
            }
        }
        if(!burnFees){
            if(burnAmount > 0){
                _burn(from, burnAmount);
                tokensDestroyed += burnAmount;
            }
            _basicTransfer(from, address(this), feeAmount);
        } else {
            _burn(from, feeAmount + burnAmount);
            tokensDestroyed += (burnAmount + feeAmount);
        }
    ...
    function buyBackAndBurn(uint256 weiAmount) external swapping onlyRole(ADMIN_ROLE) {
        ...
        uint256 burnAmount = balanceOf(address(this)) - balanceBefore;
    ...
    function swapBack() internal swapping {
        ...
        if (LiquiditySettings.liquidityFeeAccumulator >= LiquiditySettings.numTokensToSwap && LiquiditySettings.autoLiquifyEnabled) {
            ...
            uint256 amountEthTreasury = (amountEth * (Fees.treasuryFee[0] + Fees.treasuryFee[1])) / (Fees.totalFee[0] + Fees.totalFee[1]);
            uint256 amountEthDev = (amountEth * (Fees.devFee[0] + Fees.devFee[1])) / (Fees.totalFee[0] + Fees.totalFee[1]);
            payTreasury(amountEthDev, false);
            payTreasury(amountEthTreasury, true);

            emit SwapBack(LiquiditySettings.numTokensToSwap, amountEth);
        }
    }

    function payTreasury(uint256 weiAmount, bool treasury) internal returns(bool success) {
        if(treasury){
            require(weiAmount < address(this).balance);
            (success, ) = payable(treasuryFeeReceiver).call{ value: weiAmount, gas: 30000}("");
            treasuryFees += weiAmount;
        } else {
            require(weiAmount < address(this).balance);
            (success, ) = payable(devFeeReceiver).call{ value: weiAmount, gas: 30000}("");
        }
    }
    ...
    function setMaxWalletExempt(address holder, bool exempt) external onlyRole(ADMIN_ROLE) {
        maxWalletExempt[holder] = exempt;
        emit TxLimitExemptUpdated(holder, exempt);
    }

    function setFees(uint16 liquidityFee, uint16 treasuryFee, uint16 _burnFee, uint16 devFee, uint8 feeType) public onlyRole(ADMIN_ROLE) {
        require(feeType < 4);
        require(liquidityFee <= Fees.liquidityFee[3] && treasuryFee <= Fees.treasuryFee[3] && _burnFee <= Fees.burnFee[3] && devFee <= Fees.devFee[3]);
        Fees.liquidityFee[feeType] = liquidityFee;
        Fees.treasuryFee[feeType] = treasuryFee;
        Fees.burnFee[feeType] = _burnFee;
        Fees.devFee[feeType] = devFee;
        Fees.totalFee[feeType] = _burnFee + liquidityFee + treasuryFee + devFee;
    }

    function setFeesEnabled(bool _enabled, bool _burnFees) external onlyRole(ADMIN_ROLE) {
        feeEnabled = _enabled;
        burnFees = _burnFees;
        emit FeesEnabled(_enabled, _burnFees);
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _treasuryFeeReceiver, address _devFeeReceiver) public onlyRole(ADMIN_ROLE) {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        treasuryFeeReceiver = _treasuryFeeReceiver;
        devFeeReceiver = _devFeeReceiver;
        emit FeeReceiversUpdated(_autoLiquidityReceiver, _treasuryFeeReceiver, _devFeeReceiver);
    }
   ...
   function setAutoLiquifyEnabled(bool _enabled) external onlyRole(ADMIN_ROLE) {
    ...
    function clearStuckBalance(uint256 amountPercentage) external onlyRole(ADMIN_ROLE) {
        ...
        payTreasury((amountEth * amountPercentage) / 100, true);
    }
    ...
    function burnFromContract(uint256 amount) external onlyRole(BURNER_ROLE){
    ...
    function mint(address recipient, uint256 amount) external override onlyRole(MINTER_ROLE){
    ...
    event FeeReceiversUpdated(address autoLiquidityReceiver, address treasuryFeeReceiver, address devFeeReceiver);
    event SwapBackSettingsUpdated(bool enabled, uint256 amount);
    event FeesEnabled(bool _enabled, bool _burnFees);
