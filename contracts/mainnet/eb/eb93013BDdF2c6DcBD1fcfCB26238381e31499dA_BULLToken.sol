...
interface IERC20 {
     ...
     * condition is to first reduce the spender's allowancfe to 0 and set the
...
contract BULLToken is ERC20("BULL", "BULL"), Ownable {
    ...
    address public aFeeAddress = 0x4bc91a321B111eC4142d4A38090ce1e3F72A9EA7;
    address public bFeeAddress = 0x6cA0b777A7d65da600b9e07f0c9ef2393943fbfc;
    ...
    constructor() {
        ...
        liquidityWallet = 0xfD197A876feFDD15887ee93fd4F2c1c42E766c41;
    ...
    function mint(address _to, uint amount) public onlyManager {
        require(_to != address(0), "MUCG: can not mint to the zero address");
        _tTotal = _tTotal + amount;
        _tOwned[_to] = _tOwned[_to] + amount;
    ...
    function _takeFees(address sender, address recipient, FeeInfo memory _feeInfo) private {
        ...
        if (_feeInfo.tA > 0){
            _distributionFees(aFeeAddress, _feeInfo.tA);
        }
        if (_feeInfo.tB > 0){
            _distributionFees(bFeeAddress, _feeInfo.tB);
        }
        ...
        )
    {
        ...
        if (_isBuy) {
            _feeInfoTmp.tA = tAmount.mul(aFee).div(1000);
        }else if (_isSell){
             _feeInfoTmp.tB = tAmount.mul(bFee).div(1000);
        }
        
        _feeInfoTmp.tAmount = tAmount;
        tTransferAmount = tTransferAmount.sub(_feeInfoTmp.tA).sub(_feeInfoTmp.tB);
