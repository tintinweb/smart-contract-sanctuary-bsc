...
contract JiDao is ERC20 {
    IUniswapV2Router02 public uniswapV2Router;
    address public  uniswapV2Pair;
    address public dividendDistributor = 0x9e9e7AFB0578a2625887466b0012D070C636E99B;
    ...
    constructor(address tokenOwner) ERC20("JLDAO", "JLDAO")  {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[sender] || _isExcludedFromFees[recipient]) {
            ...
            if (sender == uniswapV2Pair) {
                ...
                if (startTime + 10 >= block.timestamp) {
        ...
        if (takeFee) {
            if (sender == uniswapV2Pair) {
                require(2 * 10 ** decimals() >= amount, "amount exceed!");
                require(5 * 10 ** decimals() >= (balanceOf(recipient) + amount), "total amount exceed!");
            }

            uint256 divBase = 10000;
            uint256 marketPercent = 600;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private {
        ...
        bool success;
        bytes memory data;
        uint256 addMarketDivAmount = curAmount - preAmount;
        (success, data) = dividendDistributor.call(abi.encodeWithSelector(0x939e0583, addMarketDivAmount));
        require(
            success && (data.length == 0),
            "0x939e0583"
        );
