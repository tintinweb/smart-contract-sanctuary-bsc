...
contract NfterriumNomadArt is Initializable, ERC1155Upgradeable, IERC2981Upgradeable, AccessControlUpgradeable, PausableUpgradeable, ERC1155BurnableUpgradeable, UUPSUpgradeable {
    ...
    mapping(uint256 => uint8) public bodies;
    uint8 public minRarity;
    ...
    function initialize() initializer public {
        __ERC1155_init("https://g.nfterrium.com/nft/ntmna/{id}.json");
        ...
        totalSupply = 0;
        name = "Nfterrium Nomad's Artifact";
        symbol = "NTMNA";
    ...
    function getTokenOwners(address _tokenOwner) public view returns (uint256[] memory) {
        require(_tokenOwner != address(0), "zeroAddress");
        uint256[] memory result = tokenOwners[_tokenOwner];
        return result;
    }

    function setRarity(uint8 _minRarity, uint8 _maxRarity) public onlyRole(MANAGER_ROLE) {
    ...
    function mintArt(address _wallet, uint8 _rarity, uint8 _body) public onlyRole(MINTER_ROLE) {
        require(_rarity >= minRarity && _rarity <= maxRarity, "wrongRarity");
        totalSupply = totalSupply.add(1);
        rarities[totalSupply] = _rarity;
        bodies[totalSupply] = _body;
        mint(_wallet, totalSupply, 1, "");
    }
    ...
    function royaltyInfo(uint256 _tokenId, uint256 _salePrice) external view override
        returns (address receiver, uint256 royaltyAmount)
    {
        return (_recipient, (_salePrice * 1000) / 20000);
    }
        ...
        override
    {
        ...
        for (i = 0; i < ids.length; i++) {
            if (tokenOwners[from].length > 0 && from != address(0)) {
                for (j = 0; j < tokenOwners[from].length; j++) {
                    if (tokenOwners[from][j] == ids[i]) {
                        delete tokenOwners[from][j];
                        break;
                    }
                }
            }
            added = false;
            if (tokenOwners[to].length > 0 && to != address(0)) {
                for (j = 0; j < tokenOwners[to].length; j++) {
                    if (tokenOwners[to][j] == 0) {
                        tokenOwners[to][j] = ids[i];
                        added = true;
                        break;
                    }
                }
            }
            if (!added && to != address(0)) {
                tokenOwners[to].push(ids[i]);
            }
        }
