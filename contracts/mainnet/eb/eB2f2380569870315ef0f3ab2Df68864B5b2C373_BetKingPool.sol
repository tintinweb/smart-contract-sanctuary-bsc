...
contract BetKingPool is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 public rewardPerBlockPrecisionFactor;
    uint256 public actualTokenRatio;
    uint256 public rewardPerBlock;
    ...
    constructor(
        ...
        cap = _cap * (10**_stakingTokensDecimal);
        apr = _apr;
        tokenRatio = _tokenRatio;
        startBlock = _startBlock;
        bonusEndBlock = BLOCK_PER_DAY.mul(_endBlockInDays) + block.number;
        claimableBlock = startBlock;
        actualTokenRatio = _tokenRatio;

        if (_rewardToken.decimals() > _stakedToken.decimals()) {
            actualTokenRatio = _tokenRatio.mul(
                10**(_rewardToken.decimals() - _stakedToken.decimals())
            );
        }
        ...
        if (_rewardToken.decimals() < _stakedToken.decimals()) {
            actualTokenRatio = _tokenRatio.div(
                10**(_stakedToken.decimals() - _rewardToken.decimals())
            );
        }

        rewardPerBlock = _apr.mul(actualTokenRatio).div(
            BLOCK_PER_DAY.mul(DAY_PER_YEAR)
        );
        rewardPerBlockPrecisionFactor = RATIO_PRECISION
    ...
    modifier rewardDone() {
        require(
            ...
            "SparkPoolV2: Pool not yet ended"
        );
        _;
    }
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        require(totalDeposit < cap, "pool capacity is reached");

        harvest();

        uint256 _remaining = cap.sub(totalDeposit);
        ...
        totalDeposit = totalDeposit.add(_actualAmount);
        rewardDebt = rewardDebt.add(
            _calculateReward(
                _actualAmount,
                bonusEndBlock.sub(_lastRewardBlock())
            )
        );
        ...
        if (stakedToken == rewardToken) {
            require(
                rewardDebt <=
                    rewardToken.balanceOf(address(this)).sub(totalDeposit),
                "insufficient reward reserve"
            );
        } else {
            require(
                rewardDebt <= rewardToken.balanceOf(address(this)),
                "insufficient reward reserve"
            );
        }
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (bonusEndBlock >= block.number) {
                ...
                uint256 blockLeft = bonusEndBlock.sub(block.number);
                uint256 amountLeft = user.amount.sub(_amount);

                rewardDebt = rewardDebt.add(_reward).sub(
                    _calculateReward(user.amount.sub(amountLeft), blockLeft)
                );
            }
    ...
    function emergencyRewardWithdraw() external onlyOwner rewardDone {
        ...
        if (stakedToken == rewardToken) {
            rewardBalance = rewardToken.balanceOf(address(this)).sub(
                totalDeposit
            );
        } else {
    ...
    function changeApr(uint256 _apr) external onlyOwner{
        require(block.number > bonusEndBlock, "pool have not ended");
        apr = _apr*(10**18);
        rewardPerBlock = _apr.mul(actualTokenRatio).div(
            BLOCK_PER_DAY.mul(DAY_PER_YEAR)
        );
    ...
    function updateStartAndEndBlocks(
        ...
        require(block.number > bonusEndBlock, "Pool has not ended");
        require(
    ...
    function calculateReward(uint256 _amount, uint256 _calculateFromBlock)
        internal
        view
        returns (uint256)
    {
    ...
    function _calculateReward(uint256 _amount, uint256 _blocks)
        internal
        view
        returns (uint256)
    {
        return
            _amount.mul(_blocks.mul(rewardPerBlock)).div(
