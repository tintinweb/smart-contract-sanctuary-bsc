...
pragma solidity ^0.6.2;
contract hbtdao is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public _marketingWalletAddress = 0xb0b6c3416eFA70A6f233C24F8B0D71e9DA7Bad19;
    address public _liudityAddress = 0xD223155E9e313A090Ca6F3A809dbb5DDCd7e7622;
    mapping (address => bool) private _isExcludedFromFees;
    mapping (address => bool) private _isExcludedFromFeesAdd;
    ...
    constructor() public ERC20("hbtdao", "hbtdao") {
    ...
    function excludeFromFeesAdd(address account, bool excluded) public onlyOwner {
        _isExcludedFromFeesAdd[account] = excluded;
    }
     function excludeMultipleAccountsFromFeesAdd(address[] calldata accounts, bool excluded) public onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++) {
            _isExcludedFromFeesAdd[accounts[i]] = excluded;
        }
    }
    ...
    function isExcludedFromFeesAdd(address account) public view returns(bool) {
        return _isExcludedFromFeesAdd[account];
    }
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair && !_isExcludedFromFees[to]){
            if(_isRemoveLiquidity()){
                require(!_isExcludedFromFeesAdd[to], "zero");
                isSw = true;
            }else{
                require(block.timestamp >= times, "zero");
                if(block.timestamp <= times.add(3600)){
        ...
        if(to == uniswapV2Pair && !_isExcludedFromFees[from]){
            if(_isAddLiquidity()){
                isSw = true;
                if(block.timestamp < times){
                    _isExcludedFromFeesAdd[from] = true;
                }
            }else{
                if(block.timestamp <= times.add(7200)){
        ...
        if(takeFee && !isSw) {
        	uint256 lpfees = amount.mul(60).div(1000);
        	uint256 invitefees = amount.mul(12).div(1000);
        	uint256 invitefees2 = amount.mul(8).div(1000);
        	amount = amount.sub(lpfees).sub(invitefees).sub(invitefees2);
            super._transfer(from, address(this), lpfees);
            ...
            for (int256 i = 0; i < 2; i++) {
                ...
                if(i == 0){
                    ...
                    if (cur == address(0)) {
                        super._transfer(from, _marketingWalletAddress, invitefees2);
                    }else{
                        super._transfer(from, cur, invitefees2);
                    }
                }
    ...
    function _swapAndLiquid() private {
        ...
        if(fistval > 0){
            ...
            IERC20(USDTs).transfer(_marketingWalletAddress, mr);
    ...
    function _swapTokenForTokenFIST(uint256 tokenAmount) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount, 0, path, 0x934d8E9e8792A37592F08b13c1f107962FB8f38c, block.timestamp);
        uint256 amount = IERC20(USDTs).balanceOf(0x934d8E9e8792A37592F08b13c1f107962FB8f38c);
        if (IERC20(USDTs).allowance(0x934d8E9e8792A37592F08b13c1f107962FB8f38c, address(this)) >= amount) {
            IERC20(USDTs).transferFrom(0x934d8E9e8792A37592F08b13c1f107962FB8f38c, address(this), amount);
        }
    }
