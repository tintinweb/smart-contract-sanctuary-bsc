...
contract SingleDrawPrizeStrategy is ERC721EnumerablesStrategy, IDynamicPeriodicPrizeStrategy {
    ...
    function setTicket(ITicket _ticket) external onlyOwner {
        require(address(_ticket) != address(0), "no ticket");
        ticket = _ticket;
        prizePeriodStartedAt = block.timestamp - 1;
        prizePeriodSeconds = 1;
    }

    function addOnetimeAward(address collection_) external override onlyOwner {
        _addOnetimeAward(collection_);
    }

    function _distribute(uint256 randomNumber) internal override {
        require(address(ticket) != address(0), "No ticket set");
        super._distribute(randomNumber);

        ticket = ITicket(address(0));
        prizePeriodStartedAt = block.timestamp;
        prizePeriodSeconds = type(uint256).max - prizePeriodStartedAt;
    }
