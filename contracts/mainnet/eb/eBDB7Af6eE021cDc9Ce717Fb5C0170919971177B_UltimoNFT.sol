...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function _transfer(
        ...
        if (distributing) {
            distributor.setShares(from, _balances[from]);
            distributor.setShares(to, _balances[to]);
        }
...
contract UltimoNFT is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable, ERC721Burnable {
    ...
    address constant marketing = 0xB41311ED794155B1FD7677FEEe9630b13C4a54f1;
    address constant bank = 0xC37a87b009D744a606dd09646781226D9312DE37;
    address constant team = 0x2f4A29f3abbfa2C0Eb098DDa87F9ca8b764b61ED;
    address constant tech = 0x1d684Fb741d16ac3C598e660eb8E4E7fAB0af9fc;
    address dev;
    uint256 constant mintingLimit = 500;
    uint256 public price = 3500 * 10**18;
    ...
    constructor(address _dev) ERC721("Ultimo NFT", "ULTIMO") {
        dev = _dev;
    }
   ...
   function mint(uint256 amount) external whenNotPaused {
        require(balanceOf(msg.sender) + amount <= maxMint && amount > 0, "Minting limits exceeded");
        uint256 totalCost = feeWhitelist[msg.sender] ? 0 : price * amount;
        
        uint256 bal = USDT.balanceOf(address(this));
        USDT.transferFrom(msg.sender, address(this), totalCost);

        require (USDT.balanceOf(address(this)) - bal == totalCost, "Incorrect amount paid");
        ...
        if (totalCost > 0) {
            USDT.transfer(marketing, 630 * 10**18);
            USDT.transfer(bank, 2000 * 10**18);
            USDT.transfer(tech, 635 * 10**18);
            if (1 <= 250) {
                USDT.transfer(team, 200 * 10**18);
                USDT.transfer(dev, 35 * 10**18);
            } else  {
                USDT.transfer(team, 235 * 10**18);
            }
        }
        
        if (distributing)
            distributor.setShares(msg.sender, balanceOf(msg.sender));
  } 
  ...
  function withdrawLostFunds() external onlyOwner {
    ...
    function getMyNFTs(address me, uint256 start, uint256 limit, uint256 _gas) external view returns (uint256[] memory myNFTIDs, string[] memory myNFTuris, uint256 lastId) {
        ...
        if (currentId > 1 && balance > 0) {
            ...
            for (uint256 i=start-1; gasUsed < (_gas > 0 ? _gas : gas) && i < balance; i++) {
    ...
    function toggleDistribution(bool _toggle) external onlyOwner {
        distributing = _toggle;
    }
    ...
    function deposit(uint256 _amount) external {
        USDT.transferFrom(msg.sender, address(distributor), _amount);
