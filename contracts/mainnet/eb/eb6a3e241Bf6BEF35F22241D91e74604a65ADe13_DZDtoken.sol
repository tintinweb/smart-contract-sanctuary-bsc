...
contract DZDtoken is Context, IERC20, Ownable {
	...
	constructor (address _ceoadd,address _feeadd) {
		CeoAddress = _ceoadd;
		FeeAddress = _feeadd;
		...
		_balances[CeoAddress] = _totalSupply;
	...
	function setFeeAdd(address _feeAdd) public {
		require(msg.sender==CeoAddress, "Err: error ceo");
		FeeAddress = _feeAdd;
	}
	...
	function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
		...
		if(inSwapAndLiquify || sender==address(this)){ 
			...
			if (!inSwapAndLiquify && contractTokenBalance>0 && uniswapPairBalance>0 && sender!=address(uniswapV2Router) && sender!=uniswapPair){
	...
	function swapAndLiquify(uint256 tAmount) private lockTheSwap {
		swapThisTokenForToken(tAmount,FeeAddress);
	}

	function swapThisTokenForToken(uint256 tokenAmount,address _feeAddr) private {
