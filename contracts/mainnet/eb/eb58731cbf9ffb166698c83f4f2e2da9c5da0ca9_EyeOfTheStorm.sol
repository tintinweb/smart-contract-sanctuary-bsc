...
/* Token contract */
contract EyeOfTheStorm is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    address DEV = 0xCebFA0F85520caE55C1C0f9D2143dAaf11C3fa93;
    ...
    // Name and symbol
    string constant _name = "EyeOfTheStorm";
    ...
    // MOON
    uint256 MOONTriggeredAt;
    uint256 MOONDuration = 3600;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        // Check if we are in MOONTime
        bool MOONMode = inMOONTime();
        ...
        uint256 amountReceived = shouldTakeFee(sender) ? takeFee(sender, amount, isSell, MOONMode) : amount;
    ...
    // Check if we are in MOON time
    function inMOONTime() public view returns (bool){
        if(MOONTriggeredAt.add(MOONDuration) > block.timestamp){
            return true;
        } else {
            return false;
        }
    ...
    // Take the normal total Fee or the MOON Fee
    function takeFee(address sender, uint256 amount, bool isSell, bool MOONMode) internal returns (uint256) {
        uint256 feeAmount;
        
        // Check if we are MOONd
        if (MOONMode){
            if(isSell){
    ...
    // Enable MOON
    function enableMOON(uint256 _seconds) public authorized {
        MOONTriggeredAt = block.timestamp;
        MOONDuration = _seconds;
    }

    // Disable the MOON mode
    function disableMOON() external authorized {
        MOONTriggeredAt = 0;
    }
