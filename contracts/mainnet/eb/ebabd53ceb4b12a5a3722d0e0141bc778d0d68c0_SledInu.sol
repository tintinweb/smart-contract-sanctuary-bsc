...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional, Ownable {
  ...
  address public DividendsToken = address(0x2AB0e9e4eE70FFf1fB9D67031E44F6410170d00e);
...
contract SledInu is Context, ERC20, Ownable {
    using SafeMath for uint256;
   ...
   SLEDDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("Sled Inu", "SLED") {
        _buyRewardsFee = 7;
        _buyLiquidityFee = 1;
		_buyMarketingFee = 1;
        _buyDevFee = 0;
    	...
    	dividendTracker = new SLEDDividendTracker();

    	liquidityWallet = owner();
		
		marketingWallet = payable(0x82C0cF6a073775752B965d57c0013D463681aE7f);
        _devWallet = payable(0x82C0cF6a073775752B965d57c0013D463681aE7f);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "SLED: The dividend tracker already has that address");

        SLEDDividendTracker newDividendTracker = SLEDDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "SLED: The new dividend tracker must be owned by the SLED token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "SLED: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SLED: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SLED: Account is already the value of 'excluded'");
    ...
    function addToBlacklist(address account, bool blacklisted) public onlyOwner {
        require(_isBlacklisted[account] != blacklisted, "SLED: Account is already the value of 'blacklisted'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SLED: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SLED: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "SLED: The liquidity wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "SLED: The marketing wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SLED: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SLED: Cannot update gasForProcessing to same value");
...
contract SLEDDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("SLED_Dividend_Tracker", "SLED_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function withdrawDividend() public pure override {
        require(false, "SLED_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SLED contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 0 && newClaimWait <= 86400, "SLED_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SLED_Dividend_Tracker: Cannot update claimWait to same value");
