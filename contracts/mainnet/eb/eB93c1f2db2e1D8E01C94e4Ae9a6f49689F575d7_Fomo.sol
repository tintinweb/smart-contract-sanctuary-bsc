...
contract Fomo {
    ...
    uint256 public INTERVAL = 600; // 4 hours 
	uint256 public userDinaBalance = 100000 * 10**18; // 4 hours 
    ...
    function transferNotify(address user) external {
		...
		uint256 balance = IERC20(dina).balanceOf(user);
        if (balance >= userDinaBalance && block.timestamp.sub(lastTransfer, "calc interval") > INTERVAL) {
	...
	function setUserDinaBalance(uint256 _userDinaBalance) public {
	    require(msg.sender == dev, "permission denied");
	    userDinaBalance = _userDinaBalance;
	}
	
 
    function extractCoin(address token,address to) public {
        require(msg.sender == dev, "permission denied");
