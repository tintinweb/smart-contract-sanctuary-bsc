...
contract MruralContract is IBEP20, Ownable  {
    ...
    */
    ...
    ---------------------------------
    */
    constructor (address _addressDistribute) {
    ...
    */
    function _approve(address owner, address spender, uint256 amount) internal {
        ...
        require(spender != address(0), "MRURAL:Owner nao pode ser Address 0");
        ...
        if(!takeFee) removeAllFee(); // Remove todas as Taxa
            ...
            if(!automatedMarketMakerPairs[from] && !automatedMarketMakerPairs[to]) {
                ...
                if (amount > maxSellLimit && !_isUnlimitedSender(from) && !_isUnlimitedRecipient(to)) {
                    revert("MRural: Montante de Venda nao pode ultrapassar limite"); 
                }
            }
            ...
            if(automatedMarketMakerPairs[from]) {
                ...
                if(buyCooldownEnabled && !isTimelockExempt[to] && !coolDownUser) {
                    require(lockToBuyOrSellForTime(getFromLastPurchaseBuy(to), cooldownTimerInterval), "MRural:Por favor, aguarde o cooldown entre as compras");
                    buyCoolDown(to);
                }
                ...
                if(amount > maxBuyLimit && !_isUnlimitedSender(from) && !_isUnlimitedRecipient(to)) {
                    revert("MRural: Montante de Compra nao pode ultrapassar limite"); // Define o Valor de compra para o padrão após 1:30 Minutos
                }
                ...
                if(sellCooldownEnabled && !isTimelockExempt[from] && !coolDownUser) {
                    require(lockToBuyOrSellForTime(getFromLastSell(from), cooldownTimerInterval), "MRural:Por favor, aguarde o cooldown entre as vendas");
                    sellCoolDown(from);
                }
            ...
            if(maxWalletBalance > 0 && !_isUnlimitedSender(from) && !_isUnlimitedRecipient(to) && !automatedMarketMakerPairs[to]) {
                ...
                require(recipientBalance.add(amount) <= maxWalletBalance, "MRural:Nao pode Ultrapassar o limite por Wallet");
            }
