...
contract Trader is Initializable {
    ...
    function setTreasuryCap(uint256 _amount) external onlyOwner {
        TREASURY_CAP = _amount;
    }
    ...
    function deposit(uint256 _amount) external {
        ...
        require(
            IERC20(TOKEN).balanceOf(address(this)).add(_amount) <= TREASURY_CAP
        );
        IERC20(TOKEN).transferFrom(msg.sender, address(this), _amount);
        balances[msg.sender][round] += _amount;
    }

    function claim() external {
        require(!can_deposit);
        require(balances[msg.sender][round] > 0);
        uint256 percentage = mulDiv(
            10000000,
            balances[msg.sender][round],
            tokenBalance
        );

        uint256 bnbAmount = mulDiv(bnbBalance, percentage, 10000000);
        payable(msg.sender).transfer(bnbAmount);
        balances[msg.sender][round] = 0;
    }
