...
contract TDOGE is ERC20, Ownable {
    ...
    TDOGEDividendTracker public dividendTracker;
    ...
    uint256 public maxTransactionAmount = 5000 * (10**9) * (10**18); //1%

    uint256 public _BuyUSDTRewardsFee = 4;
    uint256 public _BuyLiquidityFee = 1;
    uint256 public _BuyMarketingFee = 5;
    uint256 public _buyBackCharityFee = 2;
  
    uint256 public _BuyTotalFees = _BuyUSDTRewardsFee.add(_BuyLiquidityFee).add(_BuyMarketingFee).add(_buyBackCharityFee);
    
    uint256 public _SellUSDTRewardsFee = 8;
    uint256 public _SellLiquidityFee = 1;
    uint256 public _SellMarketingFee = 5;
    uint256 public sellBackCharityFee = 2;
    uint256 public _SellTotalFees = _SellUSDTRewardsFee.add(_SellLiquidityFee).add(_SellMarketingFee).add(sellBackCharityFee);
    
    uint256 public maxWalletToken = 5000 * (10**9) * (10**18); //1%

    address public _marketingWalletAddress = 0x54E6e43159ad0F0a521140DAaBB992EB8C9060Ee;
    address public _TDOGEWalletAddress = 0x54E6e43159ad0F0a521140DAaBB992EB8C9060Ee;
    ...
    constructor() ERC20("komichodragon", "$Kdrake") {

    	dividendTracker = new TDOGEDividendTracker();
        ...
        dividendTracker.excludeFromDividends(_TDOGEWalletAddress);
        ...
        excludeOrIncludeFromFees(_TDOGEWalletAddress, true);
        ...
        excludeOrIncludeFromMaxTx(_TDOGEWalletAddress, true);
        ...
        excludeOrIncludeFromMaxWallet(_TDOGEWalletAddress, true);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "TDOGE: The dividend tracker already has that address");

        TDOGEDividendTracker newDividendTracker = TDOGEDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "TDOGE: The new dividend tracker must be owned by the TDOGE token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "TDOGE: The router already has that address");
    ...
    function excludeOrIncludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "TDOGE: Account is already the value of 'excluded'");
    ...
    {
        require(_isExcludedFromMaxTx[account] != excluded, "TDOGE: Account is already the value of 'excluded'");
    ...
    {
        require(_isExcludedFromMaxWallet[account] != excluded, "TDOGE: Account is already the value of 'excluded'");
    ...
    function setTDOGEWallet(address payable wallet) external onlyOwner{
         require(wallet != address(0), "Invalid address");
        _TDOGEWalletAddress = wallet;
    }

      function setBuyFees(uint256 buyUSDTRewardFee, uint256 buyLiquidityFee, uint256 buyMarketingFee, uint256 buyBackCharityFee) external onlyOwner
    {
        _BuyUSDTRewardsFee = buyUSDTRewardFee;
        _BuyLiquidityFee = buyLiquidityFee;
        _BuyMarketingFee = buyMarketingFee;
        _buyBackCharityFee = buyBackCharityFee;
        _BuyTotalFees = _BuyUSDTRewardsFee.add(_BuyLiquidityFee).add(_BuyMarketingFee).add(_buyBackCharityFee);
        require(_BuyTotalFees >= 0 && _BuyTotalFees <= 20, "Fees are exceeding the limits");
    }

    function setSellFees(uint256 sellUSDTRewardFee, uint256 sellLiquidityFee, uint256 sellMarketingFee, uint256 sellBackTDOGEFee) external onlyOwner
    {
        _SellUSDTRewardsFee = sellUSDTRewardFee;
        _SellLiquidityFee = sellLiquidityFee;
        _SellMarketingFee = sellMarketingFee;
        sellBackCharityFee = sellBackTDOGEFee;
        _SellTotalFees = _SellUSDTRewardsFee.add(_SellLiquidityFee).add(_SellMarketingFee).add(sellBackCharityFee);
        require(_SellTotalFees >= 0 && _SellTotalFees <= 20, "Fees are exceeding the limits");
    }
      function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "TDOGE: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 800000, "TDOGE: gasForProcessing must be between 200,000 and 800,000");
        require(newValue != gasForProcessing, "TDOGE: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        
        {
            ...
            uint256 walletTokens = contractTokenBalance.mul(_SellMarketingFee.add(sellBackCharityFee)).div(_SellTotalFees);
            uint256 contractBalance = address(this).balance;
            swapTokensForEth(walletTokens);
            uint256 newBalance = address(this).balance.sub(contractBalance);
            uint256 marketingShare = newBalance.mul(_SellMarketingFee).div(sellBackCharityFee.add(_SellMarketingFee));
            uint256 developmentShare = newBalance.sub(marketingShare);
            payable(_marketingWalletAddress).transfer(marketingShare);
            payable(_TDOGEWalletAddress).transfer(developmentShare);
...
contract TDOGEDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("TDOGE_Dividend_Tracker", "TDOGE_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 200000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "TDOGE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "TDOGE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TDOGE contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "TDOGE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "TDOGE_Dividend_Tracker: Cannot update claimWait to same value");
