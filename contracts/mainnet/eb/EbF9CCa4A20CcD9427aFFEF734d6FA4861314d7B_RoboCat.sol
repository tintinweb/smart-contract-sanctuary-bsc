...
Other features  : SUPER High APY for Holders 
*/
...
contract RoboCat is ERC20Detailed, Ownable {
    ...
    constructor() ERC20Detailed("RoboCat", "RCAT", uint8(DECIMALS)) Ownable() {
        ...
        _gonBalances[treasuryReceiver] = gons.div(100).mul(99);
        _gonBalances[msg.sender] = gons.div(100).mul(1);
        ...
        emit Transfer(address(0x0), treasuryReceiver, _totalSupply.div(100).mul(99));
		emit Transfer(address(0x0), msg.sender, _totalSupply.div(100).mul(1));
    }
	...
	function AdminCallSetFeeValue(uint256 _liquidityFee, uint256 _treasuryFee, uint256 _insuranceFundFee, uint256 _burnCatFee, uint256 _sellFeeLiquidity, uint256 _sellFeeTreasury, uint256 _sellFeeInsurance, uint256 _sellFeeBurn) external onlyOwner {
		require(_liquidityFee + _treasuryFee + _insuranceFundFee + _burnCatFee + _sellFeeLiquidity + _sellFeeTreasury + _sellFeeInsurance + _sellFeeBurn <= 250, "Total buy/sell fee shoul be less than 25%");
