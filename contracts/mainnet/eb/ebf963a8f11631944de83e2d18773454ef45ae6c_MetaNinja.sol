...
contract MetaNinja is BEP20,  LockToken {
    using SafeMath for uint256;
    ...
    MetaNinjaDividendTracker public dividendTracker;

    address public deadWallet = 0x000000000000000000000000000000000000dEaD;

    address public immutable BUSD = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); //BUSD

    uint256 public _totalSupply = 69_000_000_000_000_000_000_000 * (10**9);
    uint256 public swapTokensAtAmount = _totalSupply.div(1000_0000_0000);
    ...
    address public _marketingWalletAddress = 0x51921Ce37327c27a0416a64D5B55e24F44D41aFB;
    ...
    constructor() BEP20("Meta Ninja", "NINJA") {

        dividendTracker = new MetaNinjaDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "MetaNinja: The dividend tracker already has that address");

        MetaNinjaDividendTracker newDividendTracker = MetaNinjaDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "MetaNinja: The new dividend tracker must be owned by the MetaNinja token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "MetaNinja: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MetaNinja: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "MetaNinja: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MetaNinja: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MetaNinja: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MetaNinja: Cannot update gasForProcessing to same value");
    ...
    function _transfer(address from, address to, uint256 amount) internal open(from, to) override 
    {
        ...
        !automatedMarketMakerPairs[from] && from != owner()) 
        {
            ...
            swapAndSendBNBToFee(marketingTokens);
    ...
    function swapAndSendBUSDToFee(uint256 tokens) private  
    {
    ...
    function swapAndSendBNBToFee(uint256 tokens) private  
    {
...
contract MetaNinjaDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("MetaNinja_Dividen_Tracker", "MetaNinja_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**9); //must hold 200000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "MetaNinja_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "MetaNinja_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MetaNinja contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "MetaNinja_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "MetaNinja_Dividend_Tracker: Cannot update claimWait to same value");
