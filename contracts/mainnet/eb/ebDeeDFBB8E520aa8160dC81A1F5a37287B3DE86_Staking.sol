...
contract Staking is Ownable, ReentrancyGuard {
    ...
    uint256 private StakedReward; //?REWARD OF THIS MONTH
    ...
    function stake(uint256 _amount)
        ...
        zeroAmount(_amount)
    {
        if (stakedTime[msg.sender] != 0) {
            require(
                stakedTime[msg.sender] == stakeTimes,
        ...
        haveShares(msg.sender)
    {
        ...
        uint256 rewardTaken;
        if (reward > 0) {
            rewardTaken = getReward(msg.sender);
        }
        ...
        emit Unstake(msg.sender, deposited, shares, rewardTaken);
        ...
        returns (uint256 commingReward, uint256 reward)
    {
        ...
        if (witdrawTimes >= stakedTime[sender]) {
            ...
            if (stakedTime[sender] == witdrawTimes) commingReward = reward;
        }
        ...
        returns (uint256 valueLpPerBusd)
    {
        ...
        uint256 pairValue = BusdAmount * 2;
        ...
        )
    {
        ...
        if (commingReward == 0) reward = reward_;
