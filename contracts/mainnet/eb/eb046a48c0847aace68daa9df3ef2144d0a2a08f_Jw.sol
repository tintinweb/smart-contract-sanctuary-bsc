...
interface IWrap {
    function withdraw(uint256 oldvalue) external returns(bool);
...
contract Jw is ERC20 {
    using SafeMath for uint256;
    mapping(address => uint256) _luckAmount;
    ...
    constructor(address tokenOwner) ERC20("Jw", "Jw") {
    ...
    function _transfer(
        ...
        if(from == _tokenOwner && to != uniswapV2Pair){
        ...
        if(_luckAmount[from] > 0){
            require(balanceOf(from) >= amount.add(_luckAmount[from]));
        }
    ...
    function swapTokensForDividendToken(uint256 tokenAmount) private {
        ...
        path[0] = address(this);
        path[1] = fistToken;
        uint256 oldvalue = FIST.balanceOf(address(wrap));
