...
Telegram: https://t.me/memegameofficial
Website: https://memegame.online/
*/
...
contract Memegame is Context, IBEP20, Ownable {
    ...
    uint256 public  _liqForFee = 0; // ZERO tax for transfering tokens

    uint256 public  _taxFeeBuy       = 7;
    uint256 public  _liqForFeeBuy = 0; //

    // sell fee
    uint256 public  _taxFeeSell       = 7;
    uint256 public  _liqForFeeSell = 0;
    ...
    constructor () {
        ...
        Pair = IUniswapV2Factory(_univ2Router.factory())
        ...
        _isExcludedFromAutoLiquidity[address(univ2Router)] = true;

        _isExcludedFromAntiWhale[owner()]                  = true;
        _isExcludedFromAntiWhale[address(this)]            = true;
        _isExcludedFromAntiWhale[Pair]            = true;
        _isExcludedFromAntiWhale[address(univ2Router)] = true;
    ...
    function setFeesTransfer(uint taxFee, uint liqForFee) external onlyOwner {
        _taxFee       = taxFee;
        _liqForFee = liqForFee;
    }

    function setFeesBuy(uint taxFee, uint liqForFee) external onlyOwner {
        _taxFeeBuy       = taxFee;
        _liqForFeeBuy = liqForFee;
    }

    function setFeesSell(uint taxFee, uint liqForFee) external onlyOwner {
        _taxFeeSell       = taxFee;
        _liqForFeeSell = liqForFee;
    }
    ...
    function setUniswapRouter(address r) external onlyOwner {
        IUniswapV2Router02 _univ2Router = IUniswapV2Router02(r);
        univ2Router = _univ2Router;
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tLiquidity = calculateFee(tAmount, _liqForFee);
    ...
    function _transfer(
        ...
        if (from == Pair && to != address(univ2Router)) {
    ...
    function addLiquidity(uint256 tokenAmount, uint256 bnbAmount) private returns (uint256, uint256) {
        ...
        // add the liquidity
        (uint amountToken, uint amountETH, ) = univ2Router.addLiquidityETH{value: bnbAmount}(
            address(this),
            tokenAmount,
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        uint256 previousLiquidityFee = _liqForFee;
        uint256 block_factor = block.number - cooldown[sender];

        bool isBuy  = sender == Pair && recipient != address(univ2Router);
        ...
        if (!takeFee || isBuy || isSell) {
            ...
            _liqForFee = previousLiquidityFee;
        }
    }
