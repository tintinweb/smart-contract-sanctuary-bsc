...
// File: contracts\library\DSMath.sol
...
// File: contracts\Lepricon_Staking.sol
 ...
 * @author @Pedrojok01
 */
contract Lepricon_Staking is DSMath, Ownable, ReentrancyGuard, Pausable {
    using SafeERC20 for IERC20;

    // L3P token address = 0xdeF1da03061DDd2A5Ef6c59220C135dec623116d;
    IERC20 public token;
    address public yieldPayment;
    address private admin; // = back-end server's address
    ...
    /**
     * @notice Create a new stake for the sender. Will remove the amount to stake from the sender and store it in a container
     */
    function _stake(
        uint256 _amount,
        uint256 _timeLock,
        ...
        if (_isNFT && !boost[msg.sender].isBoost) {
            _setNftStatus(msg.sender, _isNFT, _NftContractAddress, _tokenId, _NftBoost);
        }
     ...
     * @param _amount is the amount of tokens that the user wish to stake
     * @param _timeLock is the duration of staking (in months) that the user chose: 0 | 3 | 6 | 12. This duration determines the APR.
     */
    function stake(
        uint256 _amount,
        uint256 _timeLock,
        bool _isNFT,
        address _NftContractAddress, // In case different NFT are deployed
        uint256 _tokenId,
        uint256 _NftBoost
        ...
        _stake(_amount, _timeLock, _isNFT, _NftContractAddress, _tokenId, _NftBoost); // Handle the new stake
    }
    ...
    function _withdrawStake(uint256 _amount, uint256 _index) private returns (uint256) {
        ...
        if (current_stake.amount == 0) {
            ...
            // uint256 newLock = current_stake.unlockTime - current_stake.since; // = lockTime
            // stakeholders[user_index].address_stakes[_index].unlockTime = block.timestamp + newLock;
     ...
     */
    function withdrawStake(uint256 amount, uint256 stake_index) public nonReentrant whenNotPaused {
        uint256 reward = _withdrawStake(amount, stake_index);
        // Return staked tokens to user
        token.safeTransfer(msg.sender, amount);
        // Pay earned reward to user
        token.safeTransferFrom(yieldPayment, msg.sender, reward);
    }
    ...
    function _setNftStatus(
        ...
        else if (boost[_account].isBoost) {
    ...
    function setNftStatus(
        ...
        require(
            _account == msg.sender || msg.sender == admin, // if set afterwards in case of transfer/sale
            "Not authorized"
        );
        _setNftStatus(_account, _isNFT, _NftContractAddress, _tokenId, _NftBoost);
    }
    ...
    function resetNftStatus(address _account) public {
    ...
    function setYieldPayment(address _yieldPayment) external onlyOwner {
        require(_yieldPayment != address(0), "Zero address");
        yieldPayment = _yieldPayment;
    }
