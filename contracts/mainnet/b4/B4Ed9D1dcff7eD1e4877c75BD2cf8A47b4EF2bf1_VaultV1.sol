...
contract VaultV1 is EIP712Upgradeable, AccessControlUpgradeable {
    IToken private _token;

    // Alice constant
    uint constant ALICE = 0;
    // Bob constant
    uint constant BOB = 1;
    ...
    function initialize(
        ...
        _setupRole(WITHDRAWER_ROLE, _withdrawerAddress);
    ...
    function verify(ClaimRequest calldata req) public view {
        ...
        require(req.cumulativeDebits[BOB] <= balances[req.addresses[BOB]], "Balance {Bob} KO.");
    ...
    function _emergencyWithdraw(address alice) private {
        ...
        balances[bob] = balances[bob] + cumulativeDebitAlice - cumulativeDebitBob;

        uint amountToTransfer = balances[alice];
        balances[alice] = 0;

        _token.transfer(alice, amountToTransfer);
    }
    ...
    function stopEmergencyWithdraw(ClaimRequest calldata req) external {
        ...
        if (claim.id == emergencyWithdrawRequest.claimTransaction.id && claim.nonce > emergencyWithdrawRequest.claimTransaction.nonce) {
            if (emergencyWithdrawRequest.requester == alice) {
                ...
                if (balances[bob] >= penalty) {
                    balances[bob] -= penalty;
    ...
    function withdrawAlice(ClaimRequest calldata req) public onlyAlice() onlyAliceRequest(req) {
        ...
        require(balances[bob] + cumulativeDebitAlice >= cumulativeDebitBob, "Balance {Bob} KO.");
    ...
    function initWithdrawBob(uint256 amount) external onlyRole(WITHDRAWER_ROLE) {
        require(amount > 0, "Amount must be a positive number.");
        require(bobWithdrawRequest.timestamp == 0, "Another withdraw request is running.");
        require(balances[serverAddress] >= amount, "You cannot withdraw this amount.");
        bobWithdrawRequest = BobWithdrawRequest(block.timestamp, amount);
    }

    function withdrawBob() external onlyRole(WITHDRAWER_ROLE) {
        require(bobWithdrawRequest.timestamp > 0, "No withdraw request found.");
        require(block.timestamp - bobWithdrawRequest.timestamp > 6 days, "You cannot withdraw yet.");
        require(balances[serverAddress] >= bobWithdrawRequest.amount, "You cannot withdraw this amount.");

        balances[serverAddress] -= bobWithdrawRequest.amount;

        _token.transfer(msg.sender, bobWithdrawRequest.amount);

        bobWithdrawRequest.timestamp = 0;
        bobWithdrawRequest.amount = 0;

        emit WithdrawBob(bobWithdrawRequest.amount);
    }
}
