...
contract SzechuanSauce is Context, IERC20, Ownable {
    ...
    address public devWallet = 0x21fBBEf04aD43A49836e900852b63da9dbB594e9;

    string private constant _name = "Szechuan Sauce";
    string private constant _symbol = "SAUCE";
    ...
    Taxes public sellTaxes = Taxes(1, 2, 2);
    ...
    function setTradingStatus(
        ...
        swapEnabled = state;
        deadline = _deadline;
        launchtax = Taxes(rfi, dev, liquidity);
    ...
    function setTaxes(uint256 _rfi, uint256 _dev, uint256 _liquidity) public onlyOwner {
       taxes = Taxes(_rfi,_dev,_liquidity);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _dev, uint256 _liquidity) public onlyOwner {
       sellTaxes = Taxes(_rfi,_dev,_liquidity);
        emit FeesChanged();
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell, bool useLaunchTax) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi, to_return.rdev, to_return.rLiquidity) = _getRValues1(to_return, tAmount, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell, bool useLaunchTax) private view returns (valuesFromGetValues memory s) {
        ...
        s.tTransferAmount = tAmount-s.tRfi-s.tdev-s.tLiquidity;
        return s;
    }

    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rdev, uint256 rLiquidity){
        ...
        rTransferAmount =  rAmount-rRfi-rdev-rLiquidity;
        return (rAmount, rTransferAmount, rRfi,rdev,rLiquidity);
    }
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.dev) * 2;
