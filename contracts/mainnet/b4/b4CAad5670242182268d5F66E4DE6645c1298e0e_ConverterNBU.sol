...
contract ConverterNBU is Ownable, Pausable, ReentrancyGuard {
    ...
    IERC20 public immutable nbuToken;
    ...
    event Convert(address to, uint256 purchaseAmount, uint256 receiveAmount, uint256 lockAmount, uint256 time);
    ...
    constructor(address _nbuToken, address _receiveToken) {
        require(Address.isContract(_nbuToken), 'Purchase token should be a contract');
        require(Address.isContract(_receiveToken), 'Receive token should be a contract');

        nbuToken = IERC20(_nbuToken);
    ...
    function convert(uint256 amount) external whenNotPaused nonReentrant {
        ...
        require(nbuToken.allowance(msg.sender, address(this)) >= amount, "Purchase price is more then allowance");
        require(nbuToken.balanceOf(msg.sender) >= amount, "Purchase price is less then sender balance");
    ...
    function convertWithPermit(uint256 amount, uint256 permitDeadline, uint8 v, bytes32 r, bytes32 s) external whenNotPaused nonReentrant {
        ...
        IEIP20Permit(address(nbuToken)).permit(msg.sender, address(this), amount, permitDeadline, v, r, s);
    ...
    function _convert(address user, uint256 amount) private {
        ...
        nbuToken.safeTransferFrom(user, InitialHolder, amount);
        uint256 lockAmount;
        ...
        emit Convert(user, amount, receiveAmount, lockAmount, block.timestamp);
    }
    ...
    function getEquivalentAmount(uint256 amount) public view returns (uint256) {
        ...
        (uint256 rate, uint256 precision) = priceFeed.queryRate(address(nbuToken), address(receiveToken));
    ...
    /**
     * @notice Update limited amounts for NBU token
     * @param wallets Wallets
     * @param amounts Amounts to limit
     */
    function updateLimitedAmounts(address[] memory wallets, uint256[] memory amounts) external onlyOwner {
