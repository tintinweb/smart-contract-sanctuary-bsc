...
contract FuoIdo is Ownable {
    ...
    uint256 public futNft2Num = 999;
    uint256 public futNft3Num = 183;
    ...
    function pledgeTake() public {
        ...
        if(pledgeMap[msg.sender].waitProfit > 0){
            uint256 oldFutAmount = pledgeMap[msg.sender].waitProfit * 10**18 / calcFutPrice();
            usdtValue = usdtValue + pledgeMap[msg.sender].waitProfit;
            futAmount = futAmount + oldFutAmount;
    ...
    function calcRate(uint256 amount) public pure returns (uint256) {
        if(amount == 0){
            ...
            return 2;
        }else if(amount>7000 && amount<=16000){
            return 4;
        }else if(amount>16000 && amount<=35000){
            return 6;
        }else if(amount>35000 && amount<=710000){
            return 9;
        }else if(amount>710000 && amount<=150000){
            return 13;
    ...
    function init(address target,uint256 nodeNum,uint256 nodeLevel,uint256 inviteNodeNum, uint256 nft2Num,uint256 nft3Num,address parent,uint256 usdtProfit) public onlyOwner {
        uint256 futAmount = 0;
        uint256 usdtAmount = 0;
        if(nodeLevel == 3){
            usdtAmount = 5000 * 10**18;
            futAmount = 6000000 * 10**18;
        }else{
            usdtAmount = nodeNum * 288 *10**18;
            futAmount = nodeNum * 320000 * 10**18;
        }
    ...
    function init2(uint256 _idoTotalNode,uint256 _idoTotalUser) public onlyOwner{
        idoTotalNode = _idoTotalNode;
        idoTotalUser = _idoTotalUser;
    }
}