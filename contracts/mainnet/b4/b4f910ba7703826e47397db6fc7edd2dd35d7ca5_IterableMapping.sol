...
/**
 TOKEN NAME: Paper Shiba
 
 Paper Shiba is more than just a meme token, it's a movement.
 Come join the Paper Shiba family!
 ...
 Official Site: papershib.net
 Telegram: t.me/paper_shiba
 Twitter: twitter.com/paper_shiba
*/
...
contract PaperShiba is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    PaperShibaDividendTracker public dividendTracker;
    ...
    constructor() BEP20("Paper Shiba", "PaperShiba") 
    {
        dividendTracker = new PaperShibaDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "PaperShiba: The dividend tracker already has that address");
        PaperShibaDividendTracker newDividendTracker = PaperShibaDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "PaperShiba: The new dividend tracker must be owned by the PaperShiba token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "PaperShiba: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "PaperShiba: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "PaperShiba: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "PaperShiba: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "PaperShiba: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "PaperShiba: Cannot update gasForProcessing to same value");
...
contract PaperShibaDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("PaperShiba_Dividend_Tracker", "PaperShiba_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "PaperShiba_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "PaperShiba_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PaperShiba contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "PaperShiba_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "PaperShiba_Dividend_Tracker: Cannot update claimWait to same value");
