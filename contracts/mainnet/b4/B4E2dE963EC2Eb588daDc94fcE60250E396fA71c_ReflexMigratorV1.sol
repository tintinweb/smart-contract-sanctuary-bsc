 ...
 * ```
 */
contract ReflexMigratorV1 is Initializable, OwnableUpgradeable, PausableUpgradeable, UUPSUpgradeable {
  ...
  */
  ...
  /**
    * @dev Destroys contract, sending v1 & v2 token balance from contract to the
    * receiver.
  */
  /// @custom:oz-upgrades-unsafe-allow selfdestruct
  function destroy(address bep20TokenReceiver) external onlyOwner whenNotPaused {
    IERC20Upgradeable legacyTokenContract = IERC20Upgradeable(_legacyToken);
    uint256 legacyTokenBalance = legacyTokenContract.balanceOf(address(this));
    legacyTokenContract.transfer(bep20TokenReceiver,legacyTokenBalance);

    IERC20Upgradeable newTokenContract = IERC20Upgradeable(_newToken);
    uint256 newTokenBalance = newTokenContract.balanceOf(address(this));
    newTokenContract.transfer(bep20TokenReceiver,newTokenBalance);
    selfdestruct(payable(bep20TokenReceiver));
  }
