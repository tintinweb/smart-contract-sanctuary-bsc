...
contract LobbyFomo is Ownable, ReentrancyGuard {
    ...
    struct User {
        ...
        uint256 lobbyStart; // Tracks first lobby that user joined.
        uint256 lobbyEnd; // Tracks the latest lobby that user joined.
    }
    ...
    uint256 public fomoDeposit = 1e17; // 0.1 HMINE
    uint256 public timeIncrement = 1 minutes; //
    uint256 public fomoRoundInit = 20 minutes; //

    uint256 public lobbyStartTime;
        ...
        nonReentrant
    {
        ...
        if (block.timestamp >= fomoCountDown) {
            ...
            // Need to separate previous reward from the current jackpot.
    ...
    function getRoundInfo(uint256 _round)
        external
        view
        returns (AuctionRound memory _roundInfo)
    {
        _round = _getCurrentRound();
        _roundInfo = auctionRounds[_round];
    }
    ...
    function joinLobby(uint256 _amount) external nonReentrant {
        ...
        if (
            users[msg.sender].lobbyEnd == 0 && users[msg.sender].lobbyStart == 0
        ) {
            users[msg.sender].lobbyEnd = _round;
            users[msg.sender].lobbyStart = _round;
        } else {
            users[msg.sender].lobbyEnd = _round;
        }
    ...
    function claimAllLobby() external nonReentrant {
        User memory _user = users[msg.sender];
        uint256 _round = _getCurrentRound();
        uint256 _lobbyReward;
        require(_user.lobbyEnd != 0, "No entry yet");

        uint256 _minRound = _round;
        if (_user.lobbyEnd + 1 < _minRound) _minRound = _user.lobbyEnd + 1;
        for (uint256 _i = _user.lobbyStart; _i < _minRound; _i++) {
            // Make sure round is less than the active round, and prevent divide by zero for shares.
            if (auctionRounds[_i].sharesInRound > 0) {
                _lobbyReward +=
                    (userSharesInLobby[msg.sender][_i] *
                        auctionRounds[_i].hmineInRound) /
                    auctionRounds[_i].sharesInRound;

                // Updates user share for that round so you can't claim anymore
                userSharesInLobby[msg.sender][_i] = 0;
            }
        }

        _user.lobbyStart = _round;
        _user.lobbyEnd = _round;
