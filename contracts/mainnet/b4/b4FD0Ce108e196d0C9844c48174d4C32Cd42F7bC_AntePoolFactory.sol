...
contract AntePool is IAntePool, ReentrancyGuard {
    ...
    struct UserInfo {
        // How much MATIC this user deposited.
        uint256 startAmount;
    ...
    /// @dev Amount of decay to charge each challengers MATIC per block
    /// 15 gwei decay per block per MATIC is ~20-25% decay per year
    ...
    uint16 public constant CHALLENGER_BLOCK_DELAY = 84;

    /// @dev Minimum challenger stake is 1 MATIC
    uint256 public constant MIN_CHALLENGER_STAKE = 1e17;
    ...
    /// @dev convenience constant for 1 MATIC worth of wei
    ...
    function stake(bool isChallenger) external payable override testNotFailed {
        ...
        if (isChallenger) {
            require(amount >= MIN_CHALLENGER_STAKE, "ANTE: Challenger must stake more than 1 MATIC");
