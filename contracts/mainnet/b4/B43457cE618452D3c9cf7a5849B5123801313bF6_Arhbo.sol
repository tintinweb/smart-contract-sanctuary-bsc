...
contract Arhbo is Context, IERC20, Ownable {
    ...
    string private _name = "Arhbo";//Arhbo
    string private _symbol = "Arhbo";
    ...
    address payable private marketingWalletAddress = payable(0xc9c57B3FaB2bc8911A63B8cC112C55bc6dd915D3);
    address payable private NFTWalletAddress = payable(0xbAccD87d6de2d3c17ba7B9B6daf82910c50BC532);
    ...
    function setBuyTaxes(uint256 newMarketingTax, uint256 newNFTTax) external onlyOwner() {
        _buyMarketingFee = newMarketingTax;
        _buyNFTFee = newNFTTax;

        _totalTaxIfBuying = _buyMarketingFee.add(_buyNFTFee);
    }

    function setSellTaxes(uint256 newMarketingTax, uint256 newNFTTax) external onlyOwner() {
        _sellMarketingFee = newMarketingTax;
        _sellNFTFee = newNFTTax;
        _totalTaxIfSelling = _sellMarketingFee.add(_sellNFTFee);
    }
    
    function setDistributionSettings(uint256 newMarketingShare, uint256 newNFTShare) external onlyOwner() {
        _marketingShare = newMarketingShare;
        _NFTShare = newNFTShare;
        _totalDistributionShares = _marketingShare.add(_NFTShare);
    }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 totalShare = _NFTShare.add(_marketingShare);
        uint256 amountBNBMarketing = amountReceived.mul(_marketingShare).div(totalShare);
        uint256 amountBNBNFT = amountReceived.sub(amountBNBMarketing);
