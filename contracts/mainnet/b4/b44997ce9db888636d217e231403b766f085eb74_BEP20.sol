...
contract MERLINSBUSD is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    MERLINSBUSDDividendTracker public dividendTracker;
    ...
    address payable public _marketingWalletAddress = payable(0x9e34B8d08038eCaAcbE3fb6FDCc800dC924c1105);
    address payable public _buybackWalletAddress = payable(0x9e34B8d08038eCaAcbE3fb6FDCc800dC924c1105);
    ...
    constructor() BEP20("MERLINS BUSD", "MUSD") 
    {
        dividendTracker = new MERLINSBUSDDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "MERLINSBUSD: The dividend tracker already has that address");
        MERLINSBUSDDividendTracker newDividendTracker = MERLINSBUSDDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "MERLINSBUSD: The new dividend tracker must be owned by the MERLINSBUSD token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "MERLINSBUSD: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MERLINSBUSD: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "MERLINSBUSD: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MERLINSBUSD: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MERLINSBUSD: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MERLINSBUSD: Cannot update gasForProcessing to same value");
...
contract MERLINSBUSDDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("MERLINSBUSD_Dividend_Tracker", "MERLINSBUSD_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "MERLINSBUSD_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "MERLINSBUSD_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MERLINSBUSD contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "MERLINSBUSD_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "MERLINSBUSD_Dividend_Tracker: Cannot update claimWait to same value");
