...
contract CsaLottery {
    ...
    function setStart(uint _day, uint256 _time) external auth {
        startTime[_day] = _time; //First startup time
    }
    ...
    function luckilystar() public {
        ...
        for (uint i = 0; i<3 ; ++i) {
            ...
            if (!random) {
               ...
               luckily = uint256(hash)%(10**m);
            } 
            else  luckily = randomContract.getrandomNumber()%(10**m);   
    ...
    function selfaward() public {
        ...
        for (uint i = 0; i <limit ; ++i) {
            ...
            if (csa.balanceOf(address(this)) < wad) {
