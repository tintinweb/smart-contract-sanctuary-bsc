...
import './IBotdexRouter01.sol';
pragma abicoder v1;
interface IBotdexRouter02 is IBotdexRouter01 {
    function removeLiquidityETHSupportingFeeOnTransferTokens(
        address token,
        uint liquidity,
        uint amountTokenMin,
...
library BotdexLibrary {
    // returns sorted token addresses, used to handle return values from pairs sorted in this order
    function sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
        require(tokenA != tokenB, 'BotdexLibrary: EQUAL_ADDRESSES');
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'BotdexLibrary: ZERO_ADDRESS');
    }
                ...
                uint256(
                    keccak256(
                        abi.encodePacked(
                            ...
                            hex'bb4323c93b859d4705f2400a98e528a797f31ab8d2e117479a57995719b94b92' // init code hash
                        )
                    )
                )
            )
    ...
    function getReserves(
        ...
        (uint256 reserve0, uint256 reserve1, ) = IBotdexPair(pairFor(factory, tokenA, tokenB)).getReserves();
    ...
    function quote(
        ...
        require(amountA != 0, 'BotdexLibrary: INSUFFICIENT_AMOUNT');
        require(reserveA != 0 && reserveB != 0, 'BotdexLibrary: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountOut(
        ...
        require(amountIn != 0, 'BotdexLibrary: INSUFFICIENT_INPUT_AMOUNT');
        require(reserveIn != 0 && reserveOut != 0, 'BotdexLibrary: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountIn(
        ...
        require(amountOut != 0, 'BotdexLibrary: INSUFFICIENT_OUTPUT_AMOUNT');
        require(reserveIn != 0 && reserveOut != 0, 'BotdexLibrary: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountsOut(
        ...
        //unchecked {
        require(path.length >= 2, 'BotdexLibrary: INVALID_PATH');
