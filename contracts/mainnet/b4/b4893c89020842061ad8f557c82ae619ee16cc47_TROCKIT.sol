...
contract TROCKIT is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public swapTokensAtAmount = 2 * 10**2 * (10**9);
    uint256 public maxBuyAmount = 1 * 10**4 * 10**5;
    uint256 public maxSellAmount = 1 * 10**4 * 10**5;
    uint256 public maxWalletAmount = 1 * 10**6 * 10**5;
    uint256 public buybackLimit = 1 ether;

    address public marketingWallet = address(0xc85bFE12db63992F2c0Bd32654c1A406766A1888); //marketing wallet
    address public developmentWallet = address(0xc85bFE12db63992F2c0Bd32654c1A406766A1888); //development wallet
    ...
    uint16 public rewardsFee = 200; // 2% fee
    uint16 public liquidityFee = 200; // 2% fee
    uint16 public marketingFee = 200; // 2% fee
    uint16 public developmentFee = 300; // 3% fee
    uint16 public buybackFee = 100; // 1% fee
    ...
    constructor() ERC20("TROCKIT SOCIAL NETWORK", "TROCKIT") {
    ...
    function setFees(
      ...
      require(_market <= 100, "2% max marketing fee");
      require(_liq <= 200, "2% max liq fee");
      require(_reward <= 500, "2% max reward fee");
      require(_develop <= 100, "3% max develop fee");
      require(_buyback <= 100, "1% max buyback fee");
    ...
    function setTransferFees(
       ...
       require(_liq <= 200, "1% max transferliq fee");
       require(_reward <= 500, "1% max transferreward fee");
    ...
    function setMaxBuy(uint256 amount) external onlyOwner {
        require ( amount >= 1 * 10**4 * 10**5, "Can't set lower than 100 million tokens, no rug pull");
        maxBuyAmount = amount;
    }
...
require ( amount >= 1 * 10**6 * 10**5, "Can't set lower than 10 billion tokens, no rug pull");
...
contract TOKENDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor()
        DividendPayingToken("Trockit_Dividend_Tracker", "Trockit_Dividend_Tracker")
    {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 20000 * (10**5); //must hold 20000+ tokens
    }
