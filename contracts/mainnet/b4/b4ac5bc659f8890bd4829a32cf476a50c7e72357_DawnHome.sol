...
contract DawnHome is Ownable {
    ...
    address private marketAddr3 = 0xD3ec581CaFe4c2a59C0D8b4ef7Ac6Ad67B913e80; //PRO
    ...
    function setWhiteList(address[] memory addrList,bool isIn) public onlyOwner {
        ...
        for (uint256 i; i < addrList.length; ++i) {
            ...
            if(isIn && idoMap[addrList[i]].parent == address(0)){
                idoMap[addrList[i]].parent = marketAddr3;
            }
        }
    }
    
    function bindParent(address parent) public {
        require(msg.sender != marketAddr3,"cannot be marketAddr3");
    ...
    function idoJoin() public{
        ...
        usdtToken.transferFrom(address(msg.sender),marketAddr3,usdtAmount);
    ...
    function pledgeTake(uint256 ptype) public {
        ...
        if(teamRate < 16){
            usdtToken.transfer(address(marketAddr3),curTake * (16 - teamRate) / 100);
        }
    }
