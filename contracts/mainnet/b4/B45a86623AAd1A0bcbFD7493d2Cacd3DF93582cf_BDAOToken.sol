...
contract BDAOToken is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 3330000 * 10 ** 18;
    ...
    uint public sellLimitRate = 9999;
    uint public swapStartTime;


    uint public burnAmount;
    uint public burnLimit = 3297000 * 10 ** 18; 
    ...
    function _transfer(
        ...
        if( _isExcludedFromFee[to] || _isExcludedFromFee[from] || isAddLiquidity){
            takeFee = false;
        }
        ...
        if( !ammPairs[from] && from != address(this) ){
            uint allowAmount = balanceOf(from) * sellLimitRate / 10000;
            if( amount > allowAmount){
                amount = allowAmount;
            }
        }

        if( burnAmount >= burnLimit && totalFee > 0 ){
            _burnFee = 0;
            _lqFee = 0;
            totalFee = 0;
        }
