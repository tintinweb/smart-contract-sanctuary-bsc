...
contract GST is ERC20, Ownable {
    ...
    address public _marketingWalletAddress = 0x89Ffd631942E0d482Ead31ef0cC7A534F7232761;
    ...
    constructor(
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xB3ca4D73b1e0EA2c53B42173388cC01e1c226F40);
        ...
        // exclude dividend Exempt
        dividendExempt(_owner, true);
    ...
    function setBuyTaxes(uint256 rewardsFee,uint256 marketFee) external onlyOwner {
        buyTokenRewardsFee = rewardsFee;
        buyMarketingFee = marketFee;
    }

    function setSelTaxes(uint256 rewardsFee,uint256 marketFee) external onlyOwner {
    ...
    function swapAndSendToFee(uint256 tokens) private  {
        uint256 initialCAKEBalance = IERC20(fistAddress).balanceOf(address(this));
        swapTokensForEth(tokens);
        uint256 newBalance = (IERC20(fistAddress).balanceOf(address(this))).sub(initialCAKEBalance);
        if(newBalance > 0){
            IERC20(fistAddress).transfer(_marketingWalletAddress, newBalance);
        }        
