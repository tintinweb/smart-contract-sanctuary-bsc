...
library Address{
 ...
 contract MEOWGHOST is Context, IERC20, Ownable {
    ...
    uint256 public maxBuyLimit = 10_00_000 * 10**9;
    uint256 public maxSellLimit = 5_00_000 * 10**9;
    uint256 public maxWalletLimit = 10_00_000 * 10**9;
    
    uint256 public genesis_block;
    
    address public marketingWallet = 0x7EF9359f26AADc9A386a9f216Aa5141Dd6a82ED4;
    address public eCoWallet = 0x7EF9359f26AADc9A386a9f216Aa5141Dd6a82ED4;

    string private constant _name = "Meow Ghost";
    string private constant _symbol = "MWG";
    ...
    constructor (address routerAddress) {
        ...
        allowedTransfer[eCoWallet] = true;
    ...
    function setTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _eCo) public onlyOwner {
       taxes = Taxes(_rfi,_marketing,_liquidity,_eCo);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _eCo) public onlyOwner {
       sellTaxes = Taxes(_rfi,_marketing,_liquidity,_eCo);
        emit FeesChanged();
    }
    ...
    function _takeeCo(uint256 reCo, uint256 teCo) private {
        totFeesPaid.eCo +=teCo;

        if(_isExcluded[address(this)])
        {
            _tOwned[address(this)]+=teCo;
        }
        _rOwned[address(this)] +=reCo;
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.reCo) = _getRValues2(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.teCo = tAmount*temp.eCo/100;
        s.tTransferAmount = tAmount-s.tRfi-s.tMarketing-s.tLiquidity-s.teCo;
        return s;
    }
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rMarketing, uint256 rLiquidity){
        ...
        uint256 reCo = s.teCo*currentRate;
        rTransferAmount =  rAmount-rRfi-rMarketing-rLiquidity-reCo;
        return (rAmount, rTransferAmount, rRfi,rMarketing,rLiquidity);
    }
    
    function _getRValues2(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 reCo) {

        if(!takeFee) {
          return(0);
        }

        reCo = s.teCo*currentRate;
        return (reCo);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tMarketing + s.teCo);
        }
        if(s.rMarketing > 0 || s.tMarketing > 0) _takeMarketing(s.rMarketing, s.tMarketing);
        if(s.reCo > 0 || s.teCo > 0) _takeeCo(s.reCo, s.teCo);
        emit Transfer(sender, recipient, s.tTransferAmount);
        
    }

    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.marketing + temp.eCo) * 2;
        ...
        uint256 eCoAmt = unitBalance * 2 * temp.eCo;
        if(eCoAmt > 0){
            payable(eCoWallet).sendValue(eCoAmt);
        }
    }
