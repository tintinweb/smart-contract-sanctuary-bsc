 ...
 */
contract DevoMarketplace is AccessControl, Pausable, ERC1155Holder {
  ...
  */
        ...
        whenNotPaused
    {
        ...
        if (_tokenUsed) {
            ...
            payable(order.maker).transfer(msg.value - sellFees);
            uint256 totalFees = buyFees + sellFees;
    ...
    function finishAuction(uint256 orderId) external whenNotPaused {
        ...
        if (numBids > 2) {
            ...
            if (lastBid.isToken) {
                ...
                payable(order.maker).transfer(lastBid.amount - sellFees);
                payable(feeReceiver1).transfer((totalFees * 30) / 100);
                payable(feeReceiver2).transfer((totalFees * 70) / 100);
            }
            _cancelOrder(orderId, true);
            ...
            if (numBids > 0) {
                if (lastBid.isToken) {
                    ...
                    payable(lastBid.bidder).transfer(lastBid.amount - sellFees);
                    payable(feeReceiver1).transfer((totalFees * 30) / 100);
                    payable(feeReceiver2).transfer((totalFees * 70) / 100);
                }
            }
