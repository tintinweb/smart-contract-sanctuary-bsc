...
contract StakingPool{
    ...
    function depositOriginalToken(uint _amount) public{
        ...
        uint mA = (pT * _amount) / 100;
        uint cA = _amount - mA;

        originalToken.transferFrom(msg.sender,address(this),cA);
    ...
    function updateRate() private{
        rate = ((split * originalTokenBalance()) / 100) / cakeBalance(); // so no user rugs the staking pool
    }
    function changeSplit(uint _split) public onlyOwner {
        split = _split;
    }
    function calculateCakeRate(uint _amount) private view returns(uint){
        return rate * _amount;
    }
    ...
    // function sendOriginalToken(uint _amount, address _receipient) public onlyOwner {
    //     originalToken.approve(address(this),_amount);
    //     originalToken.transferFrom(address(this),_receipient,_amount);
    //     updateRate();
    // }
