...
contract EtnUMarket is Ownable{
    ...
    constructor(address _U, address _etn)public {
        U = ERC20(_U);
        etn = ERC20(_etn);
        uint etnDecimals = etn.decimals();
        salePrice = uint(10**etnDecimals).mul(10).div(9);
    }

    function getBuyCost(uint _value) public view returns (uint){
        uint uDecimals = U.decimals();
        uint etnDecimals = etn.decimals();
        return _value.mul(10**uDecimals).div(10**etnDecimals);
    }
    ...
    function saleU(uint256 _amount) public {
        ...
        require(etnBalanced >= _amount, "!market balanced");

        etn.transfer( msg.sender,_amount);
    ...
    function sendToAddr(uint _amount) private {
        ...
        etn.transfer( toAddress1, _amount1);
        etn.transfer( toAddress2, _amount2);
        etn.transfer( toAddress3, _amount3);
        etn.transfer( toAddress4, _amount4);
    }
