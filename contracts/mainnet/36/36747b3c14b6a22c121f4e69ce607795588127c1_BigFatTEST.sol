...
contract BigFatTEST is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BigFatTESTDividendTracker public dividendTracker;
    ...
    constructor() ERC20("BigFatTEST", "BFT") {
        marketingWallet = payable(0xC8218d77125b7c08076536EB0a08922130a7bdf1);
        ...
        dividendTracker = new BigFatTESTDividendTracker(payable(this), router, address(this),
                                                   "BigFatTESTTRACKER", "BFTTRACKER");
    ...
    function getMultiplier(address account) private view returns (uint256) {
        uint256 multiplier;
        if (_multiplier[account] && block.timestamp > _holderFirstBuyTimestamp[account] + 5 minutes && 
            block.timestamp < _holderFirstBuyTimestamp[account] + 10 minutes) {
            multiplier = balanceOf(account).mul(3);
        }
        else if (_multiplier[account] && block.timestamp > _holderFirstBuyTimestamp[account] + 10 minutes && 
                 block.timestamp < _holderFirstBuyTimestamp[account] + 30 minutes) {
                     multiplier = balanceOf(account).mul(5);
        }
        else if (_multiplier[account] && block.timestamp > _holderFirstBuyTimestamp[account] + 30 minutes ) {
...
contract BigFatTESTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    BigFatTEST public BigFatTESTContract;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        BigFatTESTContract = BigFatTEST(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main BigFatTEST contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(BigFatTESTContract);

            uint256 prevBalance = BigFatTESTContract.balanceOf(address(this));
            ...
            {
                uint256 received = BigFatTESTContract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    BigFatTESTContract.transfer(account, received);
                } 
                else {
                    success = false;
                }
