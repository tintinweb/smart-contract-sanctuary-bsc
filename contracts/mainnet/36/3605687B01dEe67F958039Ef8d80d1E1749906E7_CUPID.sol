...
contract CUPID is ERC20Detailed, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;

    event LogRebase(uint256 indexed epoch, uint256 totalSupply);

    string public _name = "CUPID Protocol";
    string public _symbol = "CUPID";
    ...
    uint256 public cupidInsuranceFundFee = 50;
    uint256 public sellFee = 20;
    uint256 public firePitFee = 25;
    uint256 public totalFee =
        liquidityFee.add(treasuryFee).add(cupidInsuranceFundFee).add(
            firePitFee
        );
    ...
    address public cupidInsuranceFundReceiver;
    ...
    constructor() ERC20Detailed(_name, _symbol, uint8(DECIMALS)) Ownable() {
        ...
        autoLiquidityReceiver = 0x7852831531a9b67C118dB0c3cc9d2f3294d84654;
        treasuryReceiver = 0x8Ba93BB5A786d126A598b2DAb8b1895a0DA8a985;
        cupidInsuranceFundReceiver = 0x0F5b3d6F34962da30D0e16dD614FcB137bD6f3cb;
        firePit = 0x5b9EC53b7f8575D5731869Ca4E5637065d165A74;
    ...
    function takeFee(
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(
                _treasuryFee.add(cupidInsuranceFundFee)
            )
        );
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(treasuryReceiver).call{
            value: amountETHToTreasuryAndSIF.mul(treasuryFee).div(
                treasuryFee.add(cupidInsuranceFundFee)
            ),
            gas: 30000
        }("");
        (success, ) = payable(cupidInsuranceFundReceiver).call{
            value: amountETHToTreasuryAndSIF.mul(cupidInsuranceFundFee).div(
                treasuryFee.add(cupidInsuranceFundFee)
            ),
            gas: 30000
        }("");
    ...
    function setFeeReceivers(
        ...
        uint256 gonAmount = _firePit.mul(_gonsPerFragment);
        _gonBalances[_autoLiquidityReceiver] = _gonBalances[
            _autoLiquidityReceiver
        ].sub(gonAmount);
        _gonBalances[_treasuryReceiver] = _gonBalances[_treasuryReceiver].add(
            gonAmount
        );
    }
