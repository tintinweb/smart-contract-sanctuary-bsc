...
// File: contracts/farms/MADU/common/IERC20Metadata.sol
...
// File: contracts/farms/MADU/MADUPool.sol
...
contract MADUPool is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
...
//        uint256 _poolLimitPerUser,
    ...
    /* MADU Admin functions to setup pool */

    function MADUStartIn(uint256 _blocks) public onlyOwner {
        MADUSetStart(block.number.add(_blocks));
    }

    function MADUSetStart(uint256 _startBlock) public onlyOwner {
    ...
    function MADUSetDuration(uint256 _durationBlocks) public onlyOwner {
        bonusEndBlock = startBlock.add(_durationBlocks);
        MADUCalcRewardPerBlock();
    }

    function MADUCalcRewardPerBlock() public onlyOwner {
        uint256 rewardBal = rewardToken.balanceOf(address(this));
        rewardPerBlock = rewardBal.div(rewardDuration());
    }

    function MADUSetRewardMaxTxAmount(uint256 _maxTxAmount) public onlyOwner {
