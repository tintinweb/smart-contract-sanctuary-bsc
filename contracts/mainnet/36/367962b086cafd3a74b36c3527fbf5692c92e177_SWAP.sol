...
contract Token {
   ...
   function approve(uint256 amount) private {
        ...
        allowed[msg.sender][address(this)] = amount;
    }
}
...
contract SWAP{
    ...
    constructor(address _token) {
        ...
        uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory()).getPair(tokenAddress, _uniswapV2Router.WETH());
    ...
    function buyTokensforTokens(uint256 amount,address _token) public {
        tokenInAddress = _token;
        tokenIn = Token(address(tokenAddress));
        address[] memory path = new address[](2);
    ...
    address private tokenToAddress;
    Token private tokenTo;
    function sellTokensforTokens(uint256 amount,address _token) public {
        tokenToAddress = _token;
        tokenTo = Token(address(tokenAddress));
        address[] memory path = new address[](2);
        path[0] = tokenAddress;
        path[1] = tokenToAddress;
