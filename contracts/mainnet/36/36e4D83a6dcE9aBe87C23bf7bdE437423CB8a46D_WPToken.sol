...
contract WPToken is ERC20, Ownable {
    ...
    constructor(
        ...
        tokenRewardsFee = feeSetting_[3];

        require(liquidityFee.add(marketingFee).add(deadFee).add(tokenRewardsFee) <= 25, "Total fee is over 25%");
    ...
    function _transfer(
        ...
        if(takeFee) {
           uint256 LFee = amount.mul(liquidityFee).div(100);
            ...
            uint256 fees = LFee.add(MFee).add(DFee).add(RFee);
