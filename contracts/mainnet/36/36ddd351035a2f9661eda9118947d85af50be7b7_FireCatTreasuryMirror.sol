...
import {IFireCatTreasuryMirror} from "../src/interfaces/IFireCatTreasuryMirror.sol";
...
/**
 * @title FireCat's treasuryMirror Contract
 * @notice Add treasury to this contract
 * @author FireCat Finance
 */
contract FireCatTreasuryMirror is IFireCatTreasuryMirror, FireCatTransfer, FireCatAccessControl {
    ...
    event SetTreasuryToken(address treasuryToken_);
    ...
    function initialize(address treasuryToken_, address fireCatTreasury_) initializer public {
        treasuryToken = treasuryToken_;
        fireCatTreasury = fireCatTreasury_;
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);   
    }

    /// @inheritdoc IFireCatTreasuryMirror
    function treasuryOf(uint256 tokenId) public view returns (uint256) {
        return IFireCatTreasury(fireCatTreasury).treasuryOf(tokenId) - claimed[tokenId];
    }

    /// @inheritdoc IFireCatTreasuryMirror
    function addTreasury(uint256 addAmount) external onlyRole(SAFE_ADMIN) returns (uint256) {
        // totalTreasury + actualAddAmount
        require(IERC20(treasuryToken).balanceOf(msg.sender) >= addAmount, "TRS:E01");
        uint totalTreasuryNew;
        uint actualAddAmount;

        actualAddAmount = doTransferIn(treasuryToken, msg.sender, addAmount);
        totalTreasuryNew = totalTreasury.add(actualAddAmount);

        /* Revert on overflow */
        require(totalTreasuryNew >= totalTreasury, "TRS:E02");

        totalTreasury = totalTreasuryNew;

        emit AddTreasury(msg.sender, actualAddAmount, totalTreasuryNew);
        return actualAddAmount;
    }

    /// @inheritdoc IFireCatTreasuryMirror
    function swapTreasury(uint256 tokenId) external onlyRole(FIRECAT_GATE) returns (uint) {
        require(treasuryOf(tokenId) > 0, "TRS:E01");
        uint totalTreasuryNew;
        uint actualSubAmount;

        uint amount = treasuryOf(tokenId);
        actualSubAmount = doTransferOut(treasuryToken, msg.sender, amount);
        totalTreasuryNew = totalTreasury.sub(actualSubAmount);

        require(totalTreasuryNew <= totalTreasury, "TRS:E03");

        totalTreasury = totalTreasuryNew;
        claimed[tokenId] = actualSubAmount;

        emit SwapTreasury(msg.sender, tokenId, actualSubAmount);
        return actualSubAmount;
    }

    /// @inheritdoc IFireCatTreasuryMirror
    function withdrawTreasury(uint256 amount) external nonReentrant onlyRole(SAFE_ADMIN) returns (uint) {
        require(amount <= totalTreasury, "TRS:E03");
        uint totalTreasuryNew;
        uint actualSubAmount;
    ...
    function withdrawRemaining(address token, address to, uint256 amount) external nonReentrant onlyRole(SAFE_ADMIN) returns (uint) {
        require(token != treasuryToken, "TRS:E06");
...
/**
* @notice IFireCatTreasuryMirror
*/
interface IFireCatTreasuryMirror {
    ...
    */
    ...
    /**
    * @notice The interface of treasury adding, call from safe_admin.
    * @dev add token to contract.
    * @param addAmount uint256.
    * @return actualAddAmount.
    */
    function addTreasury(uint256 addAmount) external returns (uint256);
