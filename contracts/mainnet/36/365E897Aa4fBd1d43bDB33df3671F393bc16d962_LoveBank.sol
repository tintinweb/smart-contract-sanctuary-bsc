...
interface IRandom {
    ...
    function withdrawDays(uint256 k, uint8 level) external view returns (uint256 days_);
}
...
contract LoveBank is ReentrancyGuard, AccessControlEnumerable {
    ...
    uint32 constant INTERVAL = 1 * 60; // todo:改为24 * 60 * 60
    address usdt; // = 0xD6e6f3E18Dd89D51128F36761fd2FF447822257A;
    address gas; // = 0x5beb5e1074EAf26e5e13485E186a535A362C2FD5;
    address gasStake; // = 0x28B3FE907820F7d162dD2D6466cfe772d891D306;
    address blc;//  = 0x70dadd0e85E9049e3A81826328E559348976A937;
    address router; // = 0x10ED43C718714eb63d5aA57B78B54704E256024E;

    function baseInit(address usdtAddr, address gasAddr, address gasStakeAddr, address blcAddr, address account, address random, address routerAddr,string calldata nameStr)
    ...
    uint8[] public bonusPercents = [0, 7, 10, 13, 15, 18];
    ...
    constructor() {
        ...
        // iAccount = IAccount(0xd162b46b893F0B10D91f51836164363FA2e47D96);
        // iRandom = IRandom(0x7De0bf9ca3CF1d413a0053cA994Af10D1DD8e0CA);
    ...
    function setBaseRate(uint8 base, uint8 comment, uint8 topBonus) external onlyRole(OPERATOR_ROLE) {
        baseRate = base;
        commentRate = comment;
    ...
    function accountAdd(address addr, address parent) private {
        ...
        IGas(gas).burnFrom(msg.sender, 1e18);
    ...
    function preDeposit(uint256 amount, bool useCommentReward) external nonReentrant {
        ...
        require(depositRemainToday() > 0, "LoveBank: insufficient remain today.");
        require(!useCommentReward || commentTimes[msg.sender] > 0, "LoveBank: not commentReward.");
        (, uint8 level, uint256 id) = iAccount.info(msg.sender);
        require(id != 0, "LoveBank: account have not active.");
        require(amount <= depositEach[level], "LoveBank: amount limit of your level.");
        ...
        deposit.endFinalPayTimestamp = uint32(block.timestamp + (days_ + 5) * INTERVAL);  // todo:上线时改为5 -> 2
        deposit.status = 10;
    ...
    function finalDeposit(uint256 idx) external nonReentrant {
        ...
        require(block.timestamp >= deposit.startFinalPayTimestamp, "LoveBank: deposit not allow final pay.");
        require(block.timestamp < deposit.endFinalPayTimestamp, "LoveBank: deposit is overtime.");

        uint256 finalAmount = deposit.finalAmount;
        deposit.status = 20;

        uint256 days_ = iRandom.withdrawDays(depositWithdrawRate(), iAccount.level(msg.sender));
    ...
    function withdraw(uint256 idx) external nonReentrant {
        ...
        require(deposit.status == 20 || deposit.status == 50, "LoveBank: deposit can`t withdraw");
        require(deposit.drawableTimestamp <= block.timestamp, "LoveBank: withdraw time have not yet.");
    ...
    function verifyBonus(
        ...
        uint256 bonus = (amount * bonusPercent) / 19;
    ...
    function bonusWithdraw() external nonReentrant {
        ...
        if (amount > 0) {
            IGas(gas).burnFrom(msg.sender, amount / 100);
    ...
    function dealBlacklist(uint256 index) external nonReentrant {
        ...
        deposit.status = 40;
        IGas(gas).burnFrom(msg.sender, 100e18 * brokers[msg.sender].punishTimes);
    }
    ...
    uint256 public fomoStartBalance = 500e18; // todo:上线时需提供上线的参数
    ...
    function fomoWithdraw() external nonReentrant {
        ...
        require(drawable > 0, "LoveBank: insufficient of fomo drawable.");
        brokers[msg.sender].profitAmount += int256(drawable);
        fomoDrawables[msg.sender] = 0;
        IGas(gas).burnFrom(msg.sender, drawable / 100);
    ...
    //     require(drawable > 0, "LoveBank: insufficient of sprint drawable.");
    //     sprintDrawables[msg.sender] = 0;
    //     IGas(gas).burnFrom(msg.sender, drawable / 100);
    //     IERC20(usdt).safeTransfer(msg.sender, drawable);
    // }

    uint8 public commentRate = 1;
    ...
    uint256 public restartTimes = 50; // todo: 上线时改为50
    ...
    function restartInit(uint256 times, uint256 rate) external onlyRole(OPERATOR_ROLE) {
        restartTimes = times;
        restartRate = rate;
    }

    function restart(uint256 depositIndex) external {
        require(deposits[msg.sender][depositIndex].status == 20, "LoveBank: deposit status error.");
        require(block.timestamp > deposits[msg.sender][depositIndex].createTimestamp + 2 * INTERVAL, "LoveBank: this deposit not satisfy to restart.");
        require(withdrawTotal / depositTotal >= restartRate, "LoveBank: bank no need to restart.");
        restartSubmitTimes += 1;
    ...
    uint256 public baseLmt = 6_0000e18;
    uint256 public offsetLmt = 20_0000e18;

    function setDayLmt(uint256 base, uint256 offset) external onlyRole(OPERATOR_ROLE) {
        baseLmt = base;
        offsetLmt = offset;
    }

    function depositRemainToday() public view returns (uint256) {
        uint32 days_ = ((uint32(block.timestamp) - versions[versionTimes])) / INTERVAL;
        return baseLmt * days_ + offsetLmt - depositAmountDays[today()];
    }
