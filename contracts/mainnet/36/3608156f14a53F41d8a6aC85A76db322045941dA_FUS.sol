...
contract FUS is ERC20, Ownable {
    ...
    address public marketingWallet = 0x0a23B847B8d7944D975D74935129324E34D629A5;
    address public tokenOwner = 0x3084bfd9E0333304ae340Cf6B9a7819eFDb9C544;

    uint256 public numTokensSellToSwap = 14 * 1e17;
    ...
    uint256 public transferBurnFee = 30;
    ...
    constructor() ERC20("Fusion", "FUS") {
        ...
        _approve(address(this), pancakeRouterAddr, type(uint256).max);
        IERC20(USDT).approve(address(uniswapV2Router), type(uint256).max);
        /*
            _mint is an internal function in ERC20.sol that is only called here,
            and CANNOT be called ever again
        */
        _mint(tokenOwner, 14319 * 1e18);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        if(takeFee) {
            ...
            if(sender == uniswapV2Pair) { //buy
                ...
                uint256 feeAmount = amount * transferBurnFee / 100;
    ...
    function swapAndDividend(uint256 tokenAmount) private lockTheSwap {
        uint totalBuyShare = buyMarketingFee + buyLpFee + buyLiquidityFee;
        uint totalSellShare = sellMarketingFee + sellLpFee + sellLiquidityFee;

        uint amountToLp = tokenAmount * (buyLiquidityFee + sellLiquidityFee) / (totalBuyShare + totalSellShare);
        swapAndLiquify(amountToLp);

        uint amountToLpHolders = tokenAmount * (buyLpFee + sellLpFee) / (totalBuyShare + totalSellShare);
        ...
        // make the swap
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount - amountToLp - amountToLpHolders,
            0, // accept any amount of usdt
            path2,
            marketingWallet,
            block.timestamp
    ...
    function dividendToLPHolders(uint256 gas) private lockTheDividend {
        ...
        uint256 totalRewards = IERC20(GLC).balanceOf(address(this));
        ...
        while (gasUsed < gas && iterations < numberOfTokenHolders) {
            ...
            if (LPAmount >= minAmountForLPDividend) {
                ...
                if (dividendAmount > 0) {
                    uint256 balanceOfThis = IERC20(GLC).balanceOf(address(this));
                    if (balanceOfThis < dividendAmount)
                        return;
                    IERC20(GLC).transfer(cur, dividendAmount);
                }
