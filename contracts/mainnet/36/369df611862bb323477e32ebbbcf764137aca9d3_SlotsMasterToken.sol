...
contract SlotsMasterToken {
    ...
    address public _public_sale1;
    address public _public_sale2;
    address public _public_sale3;
    ...
    uint256 public _public_sale1_cut;
    uint256 public _public_sale2_cut;
    uint256 public _public_sale3_cut;
     ...
     */
    event Buy(address indexed account, address indexed upline, uint256 value, uint256 token);
    ...
    constructor(
        address public_sale1, 
        address public_sale2, 
        address public_sale3, 
        address token_sale,
        address fee_addr
    ) {
        _totalSupply = 100000000000 * 10 ** 18; // 100 billion
        _name = "Slots Master Token"; 
        _symbol = "$SM";
        _decimals = 18;

        _owner = msg.sender;
        _public_sale1 = public_sale1;
        _public_sale2 = public_sale2;
        _public_sale3 = public_sale3;
        _token_sale = token_sale;
        _fee_addr = fee_addr;

        _public_sale1_cut = 5000; // 50%
        _public_sale2_cut = 2500; // 25%
        _public_sale3_cut = 2500; // 25%

        _total_sold = 0;
        _ico_max_cap = 10000000000 * 10 ** 18; // 10 billion
        _cap = 0;
        
        _minimumBuy = 0; // 0 BNB
        _salePrice = 20000000;
        _airdropToken = 100000 * 10 ** 18; // 100,000 BNB;
        _fee = 3000000000000000; // 0.003 BNB
        ...
        user_types[0] = UserType("User", 2000, 2000); // 20% 20%
        user_types[1] = UserType("Manager", 2000, 2000); // 20% 20%
    ...
    function setCommissions(uint256 _id, uint256 _bnb, uint256 _token) external onlyOwner {
        user_types[_id].direct_bnb = _bnb;
        user_types[_id].direct_token = _token;
    }
    ...
    function set(uint8 _tag, uint256 _value) public onlyAllowed{
        if(_tag==1){
            ...
            _public_sale1_cut = _value;
        }else if(_tag==6){
            _public_sale2_cut = _value;
        }else if(_tag==7){
            _public_sale3_cut = _value;
        }
    }
    ...
    function buy(address _refer) payable public {
        require(_msgSender() != _refer && _refer != address(0) && _balances[_refer] > 0, "Error: Not a valid referror.");
        uint256 _value = msg.value; 
        uint256 _token = _value.mul(_salePrice);
        _total_sold += _token;

        require((_total_sold + _token) <= _ico_max_cap, "Error: Max cap reached.");
        require(uint40(block.timestamp) <= _saleEnd, "Error: ICO is closed.");
        require(msg.value >= _minimumBuy,"Error: Not enough amount to buy.");
        ...
        // check percentages
        uint256 bnb_percentage = user_types[0].direct_bnb;
        if(users[_refer].type_id == 1) bnb_percentage = user_types[1].direct_bnb;
        ...
        // send bnb
        uint256 public_sale_percentage = _value.mul(10000 - bnb_percentage).div(10000);
        payable(_refer).transfer(_value.mul(bnb_percentage).div(10000));

        payable(_public_sale1).transfer(public_sale_percentage.mul(_public_sale1_cut).div(10000));
        payable(_public_sale2).transfer(public_sale_percentage.mul(_public_sale2_cut).div(10000));
        payable(_public_sale3).transfer(public_sale_percentage.mul(_public_sale3_cut).div(10000));

        emit Buy(_msgSender(), _refer, _value,_token);
    }

    function airdrop() payable public {
        require((_total_sold + _airdropToken) <= _ico_max_cap, "Error: Max cap reached.");
        require(uint40(block.timestamp) <= _saleEnd, "Error: Claiming airdrop is closed.");
        require(msg.value >= _fee,"Error: Not enough balance.");
        require(_airdrop[_msgSender()] == false, "Error: Airdrop already claimed.");

        if(_fee > 0) payable(_fee_addr).transfer(_fee);
