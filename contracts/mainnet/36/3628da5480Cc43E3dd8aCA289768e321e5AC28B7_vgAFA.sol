...
contract vgAFA is IvgAFA, ERC20Permit, Ownable {
    using SafeMath for uint256;

    event LogSupply(uint256 indexed epoch, uint256 totalSupply);
    event LogRebase(uint256 indexed epoch, uint256 rebase, uint256 index);
    event LogGStakingContractUpdated(address gStakingContract);

    /* ========== MODIFIERS ========== */

    modifier onlyStakingContract() {
        require(msg.sender == gStakingContract, 'StakingContract:  call is not staking contract');
        _;
    }
    ...
    address public gStakingContract; // balance used to calc rebase
    ...
    constructor() ERC20('Staked gAFA', 'vgAFA', 18) ERC20Permit('Staked gAFA') {
    ...
    function initialize(address _stakingContract) external {
        ...
        gStakingContract = _stakingContract;

        _gonBalances[address(this)] = TOTAL_GONS;

        //approve
        IvgAFA(address(this)).approve(address(_stakingContract), 1e45);

        emit Transfer(address(0x0), address(this), _totalSupply);
        emit LogGStakingContractUpdated(gStakingContract);

        initializer = address(0);
    }

    function approveGStaking() external {
        //approve
        IvgAFA(address(this)).approve(address(gStakingContract), 1e45);
    }
