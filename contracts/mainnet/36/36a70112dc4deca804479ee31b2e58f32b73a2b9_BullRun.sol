...
contract BullRun is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 100_000_00 * 10**_decimals;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));

    uint256 public swapTokensAtAmount = 100_00 * 10**9;
    uint256 public maxBuyLimit = 1_000_00 * 10**9;
    uint256 public maxSellLimit = 1_000_00 * 10**9;
    uint256 public maxWalletLimit = 2_000_00 * 10**9;
    
    uint256 public genesis_block;
    uint256 private deadline;
    
    address public TreasuryWallet = 0x962A537767aEF4dB2BDAFbC5EE8aD8Fe691A50b1;
    address private MarketingWallet = 0x13eE399e2843A00a21c7942bD8098402e39e8Ecc;
    address private BuybackWallet = 0xEEB4BbB7ce53E8eAc7878558dE28999C42c2c246;
    ...
    Taxes public buyTaxes = Taxes(0, 20, 23, 1, 1);
    Taxes public sellTaxes = Taxes(0, 70, 75, 5, 5);
    ...
    function setBuyTaxes(uint256 _rfi, uint256 _Treasury, uint256 _liquidity, uint256 _Marketing, uint256 _Buyback) public onlyOwner {
        require((_rfi + _Treasury + _liquidity + _Marketing + _Buyback) <= 20, "Fees can't exceed 20%");
        buyTaxes = Taxes(_rfi,_Treasury,_liquidity,_Marketing,_Buyback);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _Treasury, uint256 _liquidity, uint256 _Marketing, uint256 _Buyback) public onlyOwner {
        require((_rfi + _Treasury + _liquidity + _Marketing + _Buyback) <= 20, "Fees can't exceed 20%");
        sellTaxes = Taxes(_rfi,_Treasury,_liquidity,_Marketing,_Buyback);
        emit FeesChanged();
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell, bool useLaunchTax) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rMarketing, to_return.rBuyback) = _getRValues2(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell, bool useLaunchTax) private view returns (valuesFromGetValues memory s) {
        ...
        s.tTransferAmount = tAmount-s.tRfi-s.tTreasury-s.tLiquidity-s.tMarketing-s.tBuyback;
        return s;
    }
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rTreasury, uint256 rLiquidity){
        ...
        rTransferAmount =  rAmount-rRfi-rTreasury-rLiquidity-rMarketing-rBuyback;
        return (rAmount, rTransferAmount, rRfi,rTreasury,rLiquidity);
    }
    
    function _getRValues2(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 rMarketing,uint256 rBuyback) {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tTreasury + s.tMarketing+ s.tBuyback);
        }
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.Treasury + temp.Marketing + temp.Buyback) * 2;
