...
contract StakePool is Ownable {
    ...
    mapping(address => uint256) public currentPaidReward;
    ...
    constructor(address _tokenAddr) {
        ...
        penalityFees[1] = 3e18;
        penalityFees[2] = 6e18;
        penalityFees[3] = 12.5e18;
        penalityFees[4] = 25e18;
    }
    ...
    function PoolStake(uint256  _amount) external returns (bool) {
        
        require(_amount <= Token(tokenAddr).balanceOf(msg.sender),"Token Balance of user is less");
        Token(tokenAddr).safeTransferFrom(msg.sender,address(this), _amount);
        ...
        users[msg.sender].pool_deposit_time = (block.timestamp);
        users[msg.sender].lastClaimTime = (block.timestamp);
        currentPaidReward[msg.sender] = 0;
    ...
    function claimPool(bool onlyReward) public returns(bool)
    {
        ...
        currentPaidReward[msg.sender] += calculatedRewards;
        if(onlyReward){
            users[msg.sender].lastClaimTime = (block.timestamp);
            Token(tokenAddr).safeTransfer(msg.sender, calculatedRewards);
            users[msg.sender].rewardEarned += calculatedRewards;
        }
        else{
            totalUsers = totalUsers - 1;
            uint256 penality;
            ...
            if(block.timestamp < users[msg.sender].pool_deposit_time + fullMaturityTime){
                if (block.timestamp >= users[msg.sender].pool_deposit_time + 21 days) {
                    ...
                    penality = amount*penalityFees[4]/100e18;
                    _update(msg.sender,amount-penality,calculatedRewards,penality);
                }
            }
            else{
                Token(tokenAddr).safeTransfer(msg.sender, amount);
                Token(tokenAddr).safeTransfer(msg.sender, calculatedRewards);
    ...
    function _update(address _user,uint256 _amount,uint256 _reward,uint256 _penality) internal {

            Token(tokenAddr).safeTransfer(_user, _amount);
            Token(tokenAddr).safeTransfer(_user, _reward);
            Token(tokenAddr).safeTransfer(owner(), _penality);

            users[msg.sender].rewardEarned += _reward;
            emit RewardClaimed(_user, _reward);
            PoolBalance -= (_amount + _penality);
            users[_user].lastClaimTime = (block.timestamp);
    ...
    function calculateRewards(uint256 _amount,address userAdd) internal view returns(uint256){
        return (((_amount*poolRewardPercent)/100)/360)* ((block.timestamp - users[userAdd].lastClaimTime)/divisorDays);
    }

    function rewardsCalculate(address userAddress) public view returns(uint256){
        uint256 rewards;
        uint256 max_payout = this.maxPayoutOf(users[userAddress].poolBal) - currentPaidReward[userAddress];
    ...
    function updatePenalityFee(uint8[] memory _level,uint[] memory _amount) external onlyOwner {
    ...
    function retrieveBnbStuck(address payable wallet) external onlyOwner returns(bool){
        wallet.transfer(address(this).balance);
        return true;
    }

    function retrieveBEP20TokenStuck(address _tokenAddr,uint256 amount,address toWallet) external onlyOwner returns(bool){
        Token(_tokenAddr).safeTransfer(toWallet, amount);
        return true;
    }

    /* Calculate Remaining Staking Claim time of Users */
    function stakeTimeRemaining(address _userAdd) external view returns (uint256){
    ...
    function updatePoolCapacity(uint256 PoolAmount) external onlyOwner returns(bool){
        Pool = PoolAmount;
        return true;
    }
    
    /* Maturity Date */
    function maturityDate(address userAdd) external view returns(uint256){
