...
library PancakeLibrary {
    ...
    function getAmountsOut(address[] memory factories, uint[] memory fees, uint amountIn, address[] memory path) internal view returns (uint[] memory amounts) {
...
contract CustomRouter {
    ...
    using SafeMath for uint;
    address public immutable WETH; //WBNB address
    ...
    function getReserves2(address tokenA, address tokenB, address pair) external view returns (uint reserveA, uint reserveB) {
        (address token0,) = sortTokens(tokenA, tokenB); 
        (uint reserve0, uint reserve1,) = IPancakePair(pair).getReserves();
        (reserveA, reserveB) = tokenA == token0 ? (reserve0, reserve1) : (reserve1, reserve0);
    }
    ...
    function getAmountOut2(uint amountIn, uint reserveIn, uint reserveOut, uint fee) external view returns (uint amountOut) {
        require(amountIn > 0, 'PancakeLibrary: INSUFFICIENT_INPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'PancakeLibrary: INSUFFICIENT_LIQUIDITY');
        uint amountInWithFee = amountIn.mul(10000 - fee);
        uint numerator = amountInWithFee.mul(reserveOut);
    ...
    function getAmountsOut(uint amountIn, address[] memory path, address[] memory pairs, uint[] memory fees) internal view returns (uint[] memory amounts) {
        require(path.length >= 2, 'PancakeLibrary: INVALID_PATH');
        amounts = new uint[](path.length);
        amounts[0] = amountIn;
        for (uint i; i < path.length - 1; i++) {
    ...
    function getAmountsOut2(uint amountIn, address[] memory path, address[] memory pairs, uint[] memory fees) external view returns (uint[] memory amounts) {
        require(path.length >= 2, 'PancakeLibrary: INVALID_PATH');
        amounts = new uint[](path.length);
        amounts[0] = amountIn;
        for (uint i; i < path.length - 1; i++) {
    ...
    ) external view returns (uint[] memory amounts)
    {
        ...
        require(path.length - 1 == pairs.length,"PATH PAIRS length error"); 
        require(pairs.length == fees.length,"PAIRS FEES length error"); 
        amounts = getAmountsOut(amountIn, path, pairs, fees);
    }
    ...
    function _swap(uint[] memory amounts, address[] memory path, address[] memory pairs, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            IPancakePair(pairs[i]).swap(
                amount0Out, amount1Out, to, new bytes(0)
            );
        }
    }

    //30 fee == 0.3% same as fee = 0.003
    ...
    ) external virtual payable ensure(deadline) returns (uint[] memory amounts)
    {
        ...
        amounts = getAmountsOut(msg.value, path, pairs, fees);  
