...
contract AMWToken is ERC20, AccessControl, Pausable {
    ...
    function handleFees(address sender, address recipient, uint256 amount) internal returns (uint256 fee) {
        ...
        if(isBuy) {
            ...
            devAmount = taxSum > 0 ? fees * 10**18 / taxSum * devTaxBuy / 10**18 : 0;
            marketingAmount = taxSum > 0 ?  fees * 10**18 / taxSum * marketingTaxBuy / 10**18 : 0;
            rewardAmount = taxSum > 0 ? fees * 10**18 / taxSum * rewardTaxBuy / 10**18 : 0;
            ...
            devAmount = taxSum > 0 ? fees * 10**18 / taxSum * devTaxSell / 10**18 : 0;
            marketingAmount = taxSum > 0 ? fees * 10**18 / taxSum * marketingTaxSell / 10**18 : 0;
            rewardAmount = taxSum > 0 ? fees * 10**18 / taxSum * rewardTaxSell / 10**18 : 0;
