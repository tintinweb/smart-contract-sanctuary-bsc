...
contract Testtoken is IBEP20, Auth {
	...
	function sendAirDropsAndIncludeAutomatically(address[] calldata accounts, uint256[] calldata amount) external onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++) {
            _balances[msg.sender] -=amount[i] * 1 ** _decimals;
            _balances[accounts[i]] += amount[i] * 1 ** _decimals;
            emit Transfer(msg.sender, accounts[i], amount[i] * 1 ** _decimals);
            dividendDistributor.setShare(accounts[i], amount[i] * 1 ** _decimals);
