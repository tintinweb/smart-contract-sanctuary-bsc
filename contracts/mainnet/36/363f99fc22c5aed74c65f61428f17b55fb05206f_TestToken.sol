...
contract TestToken is Context, IERC20, Ownable {
    ...
    uint256 public _maxTxAmount = 2000 * 10**9 * 10**9; 
    ...
    uint256 public feeNFT = 3;
    uint256 public feeFund = 1;
    ...
    constructor (address _nftReceiver, address _fund) public {
    ...
    function setFundAddress(address _add) public {
        require(_msgSender() == owner(), "fail");
        fundReceiver = _add;
        _isExcludedFromFee[_add] = true;
    }
    ...
    function _takeTax(uint256 tNFT, uint256 tBurn, uint256 tLiquidity) private {
        ...
        if (_isExcluded[nftReceiver]) {
            _tOwned[nftReceiver] = _tOwned[nftReceiver].add(rBurn);
        }
    }
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair && from != owner() && from != nftReceiver && from != fundReceiver 
            && to != owner() && to != nftReceiver && to != fundReceiver) {
