...
/* Token contract */
contract PikashibInu is IBEP20, Auth, Context {
    ...
    string constant _name = "Pikashib Inu";
    ...
    function transfer(address recipient, uint256 amount) external override returns (bool) {
        return _transfer(msgSender(), recipient, amount);
    }

    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        if(_allowances[sender][msgSender()] != type(uint256).max){
