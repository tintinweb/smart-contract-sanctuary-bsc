...
contract CHTH is Context, IERC20, IERC20Metadata, Ownable{
    ...
    uint256 public _addAndLiquifyFee=1;
    ...
    constructor () {
        _mint(_msgSender(), 10000000000 * 10**9);
        emit Transfer(address(0), _msgSender(), 10000000000 * 10**9);
    ...
    function transferFrom(
        ...
        require(currentAllowance >= amount, "CHTH: transfer amount exceeds allowance");
        unchecked {
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
        ...
        require(currentAllowance >= subtractedValue, "CHTH: decreased allowance below zero");
        unchecked {
    ...
    function _transfer(
        ...
        require(sender != address(0), "CHTH: transfer from the zero address");
        require(recipient != address(0), "CHTH: transfer to the zero address");
        uint256 senderBalance = _balances[sender];
        require(senderBalance >= amount, "CHTH: transfer amount exceeds balance");
        uint256 _amount = amount;

        //记录第一次添加流动性时的区块时间戳
        if (launchTime == 0 &&DEXs[recipient]) {
            require(balanceOf(sender) > 0);
            launchTime = block.timestamp;
        }
        //卖出
        if(DEXs[recipient]&&!_isExcludedFromFee[sender]){
            //钱包余额必须大于1个才能卖出
            require(senderBalance>1, "CHTH: transaction failure");
            //如果是清仓，给钱包留1个b
        ...
        //不是白名单，在开盘后60秒内不能购买
        if(DEXs[sender]&& recipient != address(uniswapV2Router) && !_isExcludedFromFee[recipient]) {
            require(block.timestamp > launchTime + 60, "You have no power here!");
        }
        //非白名单买卖，扣除手续费
        if(!_isExcludedFromFee[sender]||!_isExcludedFromFee[recipient]){
            ...
            //总计收5个点的手续费，其中1个点直接销毁，剩余的在合约中累计，当达到触发回流的数量时，会重新分配1/4添加流动性、3/4回流到营销钱包
            burn(amount.div(100).mul(1));
            _balances[address(this)] = _balances[address(this)].add(amount.div(100).mul(4));
            emit Transfer(sender, address(this), amount.div(100).mul(4));
    ...
    function _burn(address account, uint256 amount) internal virtual {
        require(account != address(0), "CHTH: burn from the zero address");
        uint256 accountBalance = _balances[account];
        require(accountBalance >= amount, "CHTH: burn amount exceeds balance");
        unchecked {
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 tokensForLP = tAmount.mul(_addAndLiquifyFee).div(4).div(2);
        uint256 tokensForSwap = tAmount.sub(tokensForLP);

        swapTokensForEth(tokensForSwap);
        uint256 amountReceived = address(this).balance;

        uint256 amountBNBLiquidity = amountReceived.div(4).mul(_addAndLiquifyFee);
