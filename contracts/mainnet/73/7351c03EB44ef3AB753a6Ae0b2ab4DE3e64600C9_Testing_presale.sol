...
contract Testing_presale is ReentrancyGuard, Ownable, Pausable {
    ...
    constructor() {
        ...
        baseDecimals = (10**18);
        salePrice = 18 * (10**15); //0.45 USD
        hardcapSize = 20_000_000;
        totalTokensForPresale = 20_000_000;
        minimumBuyAmount = 1;
    ...
    function startStep(uint256 stepIndex) public onlyOwner {
        currentStep = stepIndex;
        if (stepIndex == 1) {
            salePrice = 18 * (10**15); //0.018 USD
            nextPrice = 25 * (10**15); //0.025 USD
            hardcapsizeUSD = 2;
            totalUsdValueForPresale = 2;
            inSaleUSDvalue = 2000000000000000000;
        } else if (stepIndex == 2) {
        ...
        returns (uint256 totalValue)
    {
        ...
        if (msg.sender != dAddress) {
            ...
            if (
                ...
                require(false, "hardcapUSD reached");
            }
        }
        require(isPresalePaused != true, "presale paused");
        ...
        nonReentrant
    {
        ...
        uint256 BNBAmount = (usdPrice * (10**18)) / getBNBLatestPrice();
        require(msg.value >= BNBAmount, "Less payment");
        uint256 excess = msg.value - BNBAmount;
        if (amount > inSale) {
            uint256 upfrontSale = amount - inSale;
        ...
        whenNotPaused
    {
        ...
        (tokenInterface).transferFrom(_msgSender(), owner(), usdPrice);
    ...
    function getBNBAmount(uint256 amount)
        external
        view
