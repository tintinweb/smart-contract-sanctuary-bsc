...
contract XStudio is XStudioFlags, UsingBatchTransfer, UsingERC20WithStorage, UsingPermitWithStorage, UsingUUPS {
    ...
    function _process(address source_, uint amount_, uint flags_) internal returns (uint) {
        if (_checkFlags(flags_, SOURCE_IS_LIQUIDITY_PAIR, 0)) {
            ...
            uint marketingBalance = _balanceOf(_marketingAddress);
    ...
    function _depositFees(address source_, uint amount_) internal {
        ...
        super._transfer(source_, _marketingAddress, _calculatePercentage(amount_, _feesAndAllocations.marketingAllocation));
    }
    ...
    function _executeMarketingSwap(uint marketingBalance_) internal  {
        if (marketingBalance_  > 0) {
            super._transfer(_marketingAddress, address(this), marketingBalance_);
            try _router.swapExactTokensForETHSupportingFeeOnTransferTokens(marketingBalance_, 0, _path, _marketingAddress, block.timestamp) {
            } catch {}
        }
    }
