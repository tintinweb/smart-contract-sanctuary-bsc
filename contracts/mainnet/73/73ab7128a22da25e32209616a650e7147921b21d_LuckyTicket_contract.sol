...
/*
    *** Lucky Ticket Official Contract ***
...
contract LuckyTicket_contract is Auth {
    ...
    address public FeeAddress = 0xC3fDA1510AaeE108Bd7C994844D608A70cec29CD;
    uint8 constant _decimals = 9;
    uint256 constant _one = 1 * (10 ** _decimals);
    uint256 public LuckyTicket = 0.07 ether;
    uint private LuckyTicketFeePercent = 8;
    uint private LuckyTicketPrizePercent = 18;
    ...
    uint public LuckyTicketMaxPurchased = 4;
    ...
    function deposit() external payable {
        ...
        require(msg.sender.balance >= LuckyTicket && msg.value >= LuckyTicket, "You don't have enough BNB!");
        require(msg.value >= LuckyTicket, "Unvalid LuckyTicket price!");
        ...
        (bool isSuccesPaid, bytes memory succesPaidData) = FeeAddress.call{value: LuckyTicket.div(100).mul(LuckyTicketFeePercent), gas: 5000}(
            abi.encodeWithSignature("", "BUY LUCKYTICKET", 1)
        );
    ...
    function rescueBNB(uint256 amount, address to) external authorized{
        payable(to).transfer(amount);
    }

    function setFeeAddress(address _FeeAddress) external authorized{
        FeeAddress = _FeeAddress;
    }

    function setLuckyTicket(uint256 _LuckyTicket) external authorized{
        LuckyTicket = _LuckyTicket;
    }
    
    function setLuckyTicketFeePercent(uint _LuckyTicketFeePercent) external authorized{
        LuckyTicketFeePercent = _LuckyTicketFeePercent;
    }

    function setLuckyTicketPrizePercent(uint _LuckyTicketPrizePercent) external authorized{
        LuckyTicketPrizePercent = _LuckyTicketPrizePercent;
    }

    function setLuckyTicketMaxPurchased(uint _LuckyTicketMaxPurchased) external authorized{
