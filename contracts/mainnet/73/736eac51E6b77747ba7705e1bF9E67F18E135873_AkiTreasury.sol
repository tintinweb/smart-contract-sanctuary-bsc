  ...
  function addPayment(
    ...
    for (uint256 i = 0; i < rewardContracts_.length; i++) {
      if (rewardContracts_[i].shares != 0) {
        uint256 currentAmount = rewardContracts_[i].shares * amount / totalShares;
        ...
        splitter.addPayment(address(this), token, currentAmount, rewardContracts_[i].nftAddress, rewardStartTime, rewardEndTime);
...
contract AkiRewardSplitter is Ownable {
  ...
  function addPayment(
    ...
    // SafeERC20.safeApprove(token, from, amount);
  ...
  function pullPayment() public {
    ...
    for (uint64 i = 0; i < envelopes.length; i++) {
      if (infos[i].currentAmount != 0) {
        ...
        SafeERC20.safeApprove(env.tokenAddress, address(this), currentAmount);

        env.totalShares -= info.share;
        env.amountRemains -= info.currentAmount;
        SafeERC20.safeTransferFrom(env.tokenAddress, address(this), receiver, info.currentAmount);
