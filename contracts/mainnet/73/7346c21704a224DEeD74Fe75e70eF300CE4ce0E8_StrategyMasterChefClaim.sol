...
contract StrategyMasterChefClaim is BaseStrategyLP {
    ...
    // Distributes fees to (_to)
    function distributeFeesTo(uint256 _earnedAmt, address _to)
        internal
        returns (uint256)
    {
        if (controllerFee > 0) {
            ...
            _safeSwapWeth(fee, earnedToWethPath, _to);
    ...
    function earn() public override nonReentrant whenNotPaused {
        ...
        if (earnedAmt > 0) {
            earnedAmt = distributeFeesTo(earnedAmt, msg.sender);
            earnedAmt = distributeRewards(earnedAmt);
            earnedAmt = buyBack(earnedAmt);

            if (earnedAddress != claimTokenAddress && earnedAmt > 0) {
                // Swap half earned to token0
                _safeSwap(earnedAmt, earnedToClaimPath, address(this));
            }

            uint256 claimTokenAmt = IERC20(claimTokenAddress).balanceOf(
                address(this)
            );
            sharesClaimToken = sharesClaimToken.add(
                claimTokenAmt.div(vaultSharesTotal())
            );
        }
    }

    // This is overloading the earn function with one function parameter _to

    // Allows govAddress to set new govAddress
    function changeGov(address _newGov) external onlyGov {
        govAddress = _newGov;
    }

    function changePid(uint256 _newPid) external onlyGov {
        pid = _newPid;
    }
