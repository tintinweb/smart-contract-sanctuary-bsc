...
contract Test123 is Context, IERC20, Ownable {
    ...
    uint256 private constant _tTotal = 1000 * (10**20) * (10**9);
    ...
    string private constant _symbol = "Test123";
    uint8 private constant _decimals = 9;
    
    address payable private _appAddress = payable(0xc992E8cC866978dc61EA0e0584E7629b2d63a3c3);
    address payable private _marketingAddress = payable(0x2c8e8bE84902Eb118Ae5FB9e124bf9ef2438709C);
    address payable private _burnAddress = payable(0x210aa10ef0E50b7CAF7a2A4A5f646f1D6e9C2624);
    ...
    uint256 public swapEnablelastTime;
    uint256 public swapEnableLockTime = 30*86400;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if (!inSwap && from != uniswapV2Pair && swapEnabled && contractTokenBalance > 0) {
                ...
                if (block.timestamp > swapEnablelastTime) {
                    swapEnabled = false;
                }
    ...
    function sendETHToFee(uint256 amount) private {
        ...
        uint256 mktAmount = amount.div(3);
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 rTeam, uint256 tTransferAmount, uint256 tTeam) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        uint256 rBurn = 0;
        rBurn = rTeam.div(7);
        rTeam = rTeam.sub(rBurn);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        uint256 currentRate =  _getRate();
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 rTeam) = _getRValues(tAmount, tFee, tTeam, currentRate);
        return (rAmount, rTransferAmount, rFee, rTeam, tTransferAmount, tTeam);
    }
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tTeam, uint256 currentRate) private pure returns (uint256, uint256, uint256, uint256) {
