...
contract test1 is ERC20, Ownable {
    ...
    string private name_ = "test1";
    string private symbol_ = "test1";
    ...
    uint256 public sellTokenRewardsFee = 11;
    uint256 public buySustainabilityFee = 2;
    uint256 public sellSustainabilityFee = 5;
    ...
    address public _sustainabilityWalletAddress = 0x860F1b8c1ee9781316c3BE8aA115b4FC8aaa92Dd;
    ...
    uint256 public AmountSustainabilityFee;
    ...
    uint256 public gasForProcessing = 210000;
    ...
    constructor() payable ERC20(name_, symbol_)  {
        ...
        excludeFromFees(_sustainabilityWalletAddress, true);
    ...
    function swapManual() public onlyOwner {
        ...
        if(AmountSustainabilityFee > 0) swapAndSendToFee();
        swapping = false;
    }
    ...
    function setSustainabilityWallet(address payable wallet) external onlyOwner{
        _sustainabilityWalletAddress = wallet;
    }
    ...
    function setBuySustainabilityFee(uint256 amount) public onlyOwner {
        buySustainabilityFee = amount;
    }
    function setSellSustainabilityFee(uint256 amount) public onlyOwner {
        sellSustainabilityFee = amount;
    }
    ...
    function _transfer(
        ...
        bool canSwap = AmountSustainabilityFee > swapTokensAtAmount || AmountMarketingFee > swapTokensAtAmount || AmountLiquidityFee > swapTokensAtAmount || AmountTokenRewardsFee > swapTokensAtAmount;
        if( canSwap &&
            !swapping &&
            ...
            if(AmountSustainabilityFee > swapTokensAtAmount) swapAndSendToFee();
        ...
        if(takeFee) {
            ...
            if(automatedMarketMakerPairs[from]){
                ...
                BFee = amount.mul(buySustainabilityFee).div(100);
                AmountSustainabilityFee += BFee;

                fees = LFee.add(RFee).add(MFee).add(BFee);
            }
            if(automatedMarketMakerPairs[to]){
                ...
                AmountMarketingFee += MFee;
                BFee = amount.mul(sellSustainabilityFee).div(100);
                AmountSustainabilityFee += BFee;
    ...
    function swapAndSendToFee() private  {
        ...
        uint256 newSustainability = (IERC20(rewardToken).balanceOf(address(this))).sub(initialCAKEBalance);
        IERC20(rewardToken).transfer(_sustainabilityWalletAddress, newSustainability);
        AmountSustainabilityFee = 0;
    }
