 ...
 */
contract RevivalMAXI is IERC20 {
    ...
    string private constant _name = 'RVLX MAXI';
    string private constant _symbol = 'SRVLX';
    ...
    function airdropInitialBalances(address[] calldata users, uint256[] calldata amounts) external nonReentrant onlyOwner {
        require(
        ...
        for (uint i = 0; i < length;) {
            ...
            address userToDrop = users[i];
            
            if (userInfo[userToDrop].balance > 0) {
                unchecked { ++i; }
                continue;
            }
            ...
            if (_totalSupply == 0 || previousBalance == 0) {
                _registerFirstPurchase(userToDrop, received);
            } else {
                _mintTo(userToDrop, received, previousBalance);
            }

            // set share for user
            IDistributor(distributor).setShare(userToDrop, userInfo[userToDrop].balance);
            unchecked { ++i; }
        }
    }
