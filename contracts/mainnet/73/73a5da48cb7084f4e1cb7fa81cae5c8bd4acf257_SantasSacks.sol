...
contract SantasSacks is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    SantasSacksDividendTracker public dividendTracker;
    ...
    constructor() ERC20("SantasSacks", "SANTA") {
        marketingWallet = payable(0xa4A00E6f2b2F14b7Ce209680a3468E466AA99EA0);
        devWallet = payable(0xa4A00E6f2b2F14b7Ce209680a3468E466AA99EA0);
        ...
        dividendTracker = new SantasSacksDividendTracker(payable(this), router, 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56,
                                                   "SantasSacksTRACKER", "SANTATRACKER");
        ...
        uint256 totalTokenSupply = (1_000_0) * (10**18);
    ...
    function setGasPriceLimit(uint256 GWEI) external onlyOwner {
        require(GWEI >= 5, "can never be set lower than 50");
    ...
    function setmaxTX(uint256 value) external onlyOwner {
        value = value * (10**18);
        require(value >= _totalSupply / 500, "max tx cannot be set to less than 0.1%");
        maxTX = value;
    }
...
contract SantasSacksDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    SantasSacks public SantasSacksContract;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        SantasSacksContract = SantasSacks(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main SantasSacks contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(SantasSacksContract);

            uint256 prevBalance = SantasSacksContract.balanceOf(address(this));
            ...
            {
                uint256 received = SantasSacksContract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    SantasSacksContract.transfer(account, received);
                } 
                else {
                    success = false;
                }
