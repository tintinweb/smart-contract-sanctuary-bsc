...
contract Proxy {
    bytes32 private constant implementationPosition =
        keccak256("implementation.contract.nlc:2022");
    bytes32 private constant proxyOwnerPosition =
        keccak256("owner.contract.nlc:2022");
    ...
    function transferProxyOwnership(address _newOwner) public onlyProxyOwner {
        require(
            _newOwner != address(0),
            "TrkExchange: Transfer proxy ownership to zero address"
        );
        require(
            _newOwner != proxyOwner(),
            "TrkExchange: Transfer proxy ownership to current owner"
        );
