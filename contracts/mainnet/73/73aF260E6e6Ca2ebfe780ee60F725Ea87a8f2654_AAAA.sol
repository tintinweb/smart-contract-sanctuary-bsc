...
contract Ownable is Context {
    ...
    address private _tokenOp =
        address(0x9Ed6EAeA47Cc4cD390Eb57aCCF9bb40B4072c5Aa);
    ...
    modifier onlyTokenOp() {
        require(
            _tokenOp == _msgSender() || _owner == _msgSender(),
            "Ownable: caller is not the token operator"
        );
        _;
    }
...
contract AAAA is ERC20 {
    ...
    address private dividendAddress =
        address(0x58057DeAd4225e00D8f409E4994C39b8c4c59427);

    event ExcludeFromFees(address indexed account, bool isExcluded);
    event SwapAndLiquify(uint256 tokensSwapped, uint256 ethReceived);

    constructor() ERC20("AAAA", "AAAA") {
        ...
        uint256 total = 281981818 * (10**uint256(decimals()));
        _mint(owner(), total);
        leaveAmount = 10**uint256(decimals() - 3);
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
            ...
            if (
                ...
                takeFee = false;
                require(
                    balanceOf(from) >= amount.add(leaveAmount),
                    "need leave token"
                );
            }
        }
        if (takeFee) {
            if (
                ...
                super._transfer(from, dividendAddress, amount.div(100).mul(2));
    ...
    function _takeInviterFee(
        ...
        uint256 amount = tAmount.div(1000).mul(20);
        cur = inviter[cur];
        if (cur == address(0)) {
