...
interface IOracleForLens is IOracle {
...
interface IPoolForLens is IPool {
    ...
    function allAssets(uint256) external view returns (address);
...
contract PoolLens {
        ...
        returns (PositionView memory result)
    {
        ...
        uint256 indexPrice = oracle.getLastPrice(_indexToken);
    ...
    function getTrancheValue(address _pool, address _tranche) external view returns (uint256) {
        return _getTrancheValue(IPoolForLens(_pool), _tranche);
    }

    function _getTrancheValue(IPoolForLens _pool, address _tranche) internal view returns (uint256 sum) {
        SignedInt memory aum = SignedIntOps.wrap(uint256(0));
        IOracleForLens oracle = _pool.oracle();

        for (uint256 i = 0; i < _pool.getAllAssetsLength();) {
            address token = _pool.allAssets(i);
            AssetInfo memory asset = _pool.trancheAssets(_tranche, token);
            uint256 price = oracle.getLastPrice(token);
            if (_pool.isStableCoin(token)) {
                aum = aum.add(price * asset.poolAmount);
            } else {
                aum = aum.add(_calcManagedValue(_pool, token, asset, price));
            }
            unchecked {
                ++i;
            }
    ...
    function _calcManagedValue(IPoolForLens _pool, address _token, AssetInfo memory _asset, uint256 _price)
        internal
        view
        returns (SignedInt memory aum)
    {
        uint256 averageShortPrice = _pool.poolTokens(_token).averageShortPrice;
