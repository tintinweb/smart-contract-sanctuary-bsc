...
contract ActToken is Context, IERC20, Ownable {
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    
    string private _name = 'ACT TOKEN';
    string private _symbol = 'ACT';
    uint8 private _decimals = 18;
    uint256 private _totalSupply = 28888 * 10**uint256(_decimals);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if(!whiteList[recipient]&&!whiteList[sender]){
            uint256 burnFee = amount.mul(transferFee).div(100);
    ...
    function setFee(uint256 _transferFee) public  onlyOwner {
