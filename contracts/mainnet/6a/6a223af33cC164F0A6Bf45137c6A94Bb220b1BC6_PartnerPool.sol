...
contract PartnerPool is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accTokenPerShare; // Accumulated rewardtoken per share, times PRECISION_FACTOR. See below.
        uint16 depositFeeBP;      // V1 Deposit fee in basis points
        uint16 withdrawalFeeBP;   // V1 Withdrawal fee in basis points
    }
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accTokenPerShare = accTokenPerShare.add(tokenReward.mul(PRECISION_FACTOR).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 tokenReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accTokenPerShare = pool.accTokenPerShare.add(tokenReward.mul(PRECISION_FACTOR).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(0);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accTokenPerShare).div(PRECISION_FACTOR).sub(user.rewardDebt);
            if(pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accTokenPerShare).div(PRECISION_FACTOR);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        if(_amount > 0) {
            ...
            if (pool.withdrawalFeeBP > 0 && block.number < bonusEndBlock) {
                uint256 withdrawalFee = _amount.mul(pool.withdrawalFeeBP).div(10000);
                pool.lpToken.safeTransfer(withdrawalFeeAddress, withdrawalFee);
    ...
    function updateDepositFeeBP(uint256 _pid, uint16 _depositFeeBP) public onlyOwner {
        require(_depositFeeBP <= 3000, "updateDepositFeeBP: invalid deposit fee basis points");
