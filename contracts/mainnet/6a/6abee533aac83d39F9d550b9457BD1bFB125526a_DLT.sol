...
contract DLT is ERC20Upgradeable, AccessControlUpgradeable {
    ...
    uint256 public _fomoMin;
    uint256 private enterCount;
    uint256 public _swapMin;
    ...
    function getFomoFee(
        ...
        address to,
        uint256 amount
    ) private view returns (uint256 newAmount, uint256 useFee) {
        bool takeFee = true;
    ...
    function processFomo(
        ...
        if (enterCount == 1) {
            ...
            if (from != _uniswapV2Pair && from != address(_fomo)) {
                if (balanceOf(address(_fomo)) >= _swapMin) {
                    _fomo.swap();
                }
            }
        }
    ...
    function transferFrom(
        ...
        address to,
        uint256 amount
    ) public virtual override transferCounter returns (bool) {
        processFomo(from, to, amount);

        (uint256 newAmount, uint256 useFee) = getFomoFee(from, to, amount);
    ...
    function setFomoMin(uint256 min) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _fomoMin = min;
    }

    function setSwapMin(uint256 min) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _swapMin = min;
    }
