...
contract BEP20TokenCloned is Ownable {
  ...
  function initialize(
    ...
    string calldata name_, // token name
    string calldata symbol_, // token symbol
    uint8 decimals_ // original token decimals
  ) 
    external 
  {
    require(!_isInitialized, "Already Initialized");
    _isInitialized = true;
    bridge = msg.sender;  // set bridge as minter
    _name = name_;
    _symbol = symbol_;
    _decimals = decimals_;
    _owner = newOwner;
