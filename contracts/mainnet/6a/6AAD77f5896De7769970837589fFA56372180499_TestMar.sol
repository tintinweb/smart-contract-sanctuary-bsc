...
contract TestMar is IERC20, Ownable, BaseToken {
    ...
    uint256 public _maxWalletPercent;
    ...
    constructor(
        ...
        if (developerAddress_ == address(0)) {
            require(
                Bps[3] == 0,
                "Cant set both developer address to address 0 and developer percent more than 0"
            );
        }
        require(
            Bps[0] + Bps[1] + Bps[2] + Bps[3] <= MAX_FEE,
        ...
        _maxWalletPercent = maxWalletPercent_;
        ...
        // exclude owner and this contract from fee and max holding
    ...
    function _transferBothExcluded(
        ...
        _takeCharityFee(values.tCharity);
        _takeDeveloperFee(values.tDeveloper);
    ...
    function excludeFromMaxHolding(address account) public onlyOwner {
        _isExcludedFromMaxHolding[account] = true;
    }

    function includeInMaxHolding(address account) public onlyOwner {
        _isExcludedFromMaxHolding[account] = false;
    }

    function setTaxFeePercent(uint256 taxFeeBps) external onlyOwner {
        _taxFee = taxFeeBps;
        require(
            _taxFee + _liquidityFee + _charityFee + _developerFee <= MAX_FEE,
            "Total fee is over 25%"
        );
    }
    ...
    function setDeveloperFeePercent(uint256 developerFeeBps) external onlyOwner {
        _developerFee = developerFeeBps;
        require(
            _taxFee + _liquidityFee + _charityFee + _developerFee <= MAX_FEE,
            "Total fee is over 25%"
        );
    ...
    function setMaxHolderFeePercent(uint256 maxWalletFee) external onlyOwner {
        _maxWalletPercent = maxWalletFee;
    }
        ...
        returns (TValues memory tValues)
    {
        ...
        tValues.tCharityFee = calculateCharityFee(tAmount);
        tValues.tDeveloperFee = calculateDeveloperFee(tAmount);
        tValues.tTransferAmount = tAmount.sub(tValues.tFee).sub(tValues.tLiquidity).sub(
            tValues.tCharityFee
        ).sub(tValues.tDeveloperFee);
        return (tValues);
    }
    ...
    function _takeDeveloperFee(uint256 tDeveloper) private {
        if (tDeveloper > 0) {
            uint256 currentRate = _getRate();
            uint256 rDeveloper = tDeveloper.mul(currentRate);
            _rOwned[_developerAddress] = _rOwned[_developerAddress].add(rDeveloper);
            if (_isExcluded[_developerAddress])
                _tOwned[_developerAddress] = _tOwned[_developerAddress].add(
                    tDeveloper
                );
            emit Transfer(_msgSender(), _developerAddress, tDeveloper);
        }
    }
    ...
    function calculateDeveloperFee(uint256 _amount)
        ...
        returns (uint256)
    {
        if (_developerAddress == address(0)) return 0;
        return _amount.mul(_developerFee).div(10**4);
    }
    ...
    function _getMaxHolding() internal view returns (uint256) {
        return  (_tTotal * _maxWalletPercent) / 100;
    }
    ...
    function _transfer(
        ...
        if (!isExcludedFromMaxHolding(to)) {
            require(_tOwned[to] + amount <= _getMaxHolding(), "Max holding cap breached.");            
        }
