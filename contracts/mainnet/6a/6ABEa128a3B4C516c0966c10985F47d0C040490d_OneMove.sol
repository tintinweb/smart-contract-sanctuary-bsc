...
contract OneMove is Ownable, ERC1155Holder, ERC721Holder, ReentrancyGuard {
    ...
    function withdrawWhitelist(uint256 _bnbAmount, uint256 _1mtAmount, uint256 _1rtAmount) external onlyOwnerWhitelist {
        // require(address(this).balance >= 300e18, "Not enough BNB Balance (>300)");
        // require(tokenMT1.balanceOf(address(this)) >= 10e18, "Not enough 1MT Balance (>10e18)");
        // require(tokenRT1.balanceOf(address(this)) >= 10e18, "Not enough 1RT Balance (>10e18)");
        // payable(contributeAddress[0]).transfer((200e18*50)/100);
        // payable(contributeAddress[1]).transfer((200e18*20)/100);
        // payable(contributeAddress[2]).transfer((200e18*15)/100);
        // payable(contributeAddress[3]).transfer((200e18*15)/100);
        // tokenMT1.transfer(contributeAddress[0], 5e18);
        // tokenRT1.transfer(contributeAddress[0], 5e18);

        payable(contributeAddress[0]).transfer((_bnbAmount*20)/100);
        payable(contributeAddress[1]).transfer((_bnbAmount*15)/100);
        payable(contributeAddress[2]).transfer((_bnbAmount*15)/100);
        payable(contributeAddress[3]).transfer((_bnbAmount*50)/100);
        if(_1mtAmount > 0)
            tokenMT1.transfer(contributeAddress[0], _1mtAmount);
        if(_1rtAmount > 0)
            tokenRT1.transfer(contributeAddress[0], _1rtAmount);
