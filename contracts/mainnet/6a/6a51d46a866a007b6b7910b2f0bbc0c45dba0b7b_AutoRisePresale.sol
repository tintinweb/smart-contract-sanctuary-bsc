...
// AUTORISE PROTOCOL COPYRIGHT (C) 2022 
// https://autorise.finance/
...
contract WhitelistedRole is Context, WhitelistAdminRole {
    ...
    uint256 private _whitelistClosingTime;
    ...
    function joinWhitelist(address account) public {
        require (_whitelistClosingTime >= block.timestamp , "TimedCrowdsale: opening time is before current time") ;
        require (address (msg.sender) == address (account) , 'FairLaunch: Message and account not the same user.');
        _addWhitelisted(account);
        _totalWhitelisted = _totalWhitelisted.add(1);
    }
    
    function _changeWhitelistClosingTime(uint256 newWhitelistClosingTime) public onlyWhitelistAdmin(){
            _whitelistClosingTime = newWhitelistClosingTime;    
    }
    ...
    function renounceWhitelisted() public onlyWhitelistAdmin {
    ...
    function _removeWhitelisted(address account) internal onlyWhitelistAdmin {
 ...
 * behavior.
 */
contract Crowdsale is Context, ReentrancyGuard, WhitelistedRole {
    using SafeMath for uint256;
    ...
    function _getTokenAmount(uint256 weiAmount) internal view returns (uint256) {
        return (weiAmount.mul(_rate)).div(1e13);
    }
...
contract TimedCrowdsale is Crowdsale {
     ...
     */
    function _changeClosingTime(uint256 newClosingTime) public onlyOwner(){
         _closingTime = newClosingTime;
        emit TimedCrowdsaleExtended(_closingTime, newClosingTime);
       
    }
        function _changeOpeningTime(uint256 newOpeningTime) public onlyOwner(){
            _openingTime = newOpeningTime;
        emit TimedCrowdsaleExtended(_openingTime, newOpeningTime);
