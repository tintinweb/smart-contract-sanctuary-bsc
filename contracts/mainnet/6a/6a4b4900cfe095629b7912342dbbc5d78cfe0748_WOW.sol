...
abstract contract Supervisable is Context {
    ...
    /**
     * @dev Throws if the sender is not the supervisor.
     */
    function _checkSupervisor() internal view virtual {
...
/**
 * @dev Contract for WOW Token
 */
contract WOW is Pausable, Supervisable, Burnable, Freezable, Lockable, BEP20 {
    uint256 private constant _initialSupply = 10_000_000_000e18;

    constructor() BEP20("WOWM", "WOW") {
        _mint(_msgSender(), _initialSupply);
    }

    /**
     * @dev Recover BEP20 coin in contract address.
     ...
     */
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal override(BEP20) {
     ...
     */
    function renounceSupervisorOwnership() external onlySupervisor whenNotPaused {
     ...
     */
    function transferSupervisorOwnership(address newSupervisor) external onlySupervisor whenNotPaused {
     ...
     */
    function freeze(address account) external onlyOwner whenNotPaused {
        _freeze(account);
    }
     ...
     */
    function unfreeze(address account) external onlySupervisor whenNotPaused {
        _unfreeze(account);
    }
     ...
     */
    function addBurner(address account) external onlyOwner whenNotPaused {
        _addBurner(account);
    }
     ...
     */
    function removeBurner(address account) external onlyOwner whenNotPaused {
     ...
     */
    function addLocker(address account) external onlyOwner whenNotPaused {
        _addLocker(account);
    }
     ...
     */
    function removeLocker(address account) external onlyOwner whenNotPaused {
     ...
     */
    function addTimeLock(address account, uint256 amount, uint256 expiresAt) external onlyLocker whenNotPaused {
     ...
     */
    function removeTimeLock(address account, uint8 index) external onlySupervisor whenNotPaused {
     ...
     */
    function addVestingLock(address account, uint256 startsAt, uint256 period, uint256 count) external onlyLocker whenNotPaused {
     ...
     */
    function removeVestingLock(address account) external onlySupervisor whenNotPaused {
