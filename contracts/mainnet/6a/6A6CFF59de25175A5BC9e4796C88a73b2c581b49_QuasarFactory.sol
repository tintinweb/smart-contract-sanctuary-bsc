...
contract QuasarERC20 is ERC20 {
  ...
  constructor() ERC20('Quasar V1', 'QUASAR-LP') {
    uint256 chainId;
    assembly {
      chainId := chainid()
    }
...
contract QuasarPair is IQuasarPair, QuasarERC20, ReentrancyGuard {
  using SafeMath for uint256;
  ...
  function initialize(address _token0, address _token1) external {
    require(_msgSender() == factory, 'Quasar: FORBIDDEN'); // sufficient check
    token0 = _token0;
    token1 = _token1;
  }
  ...
  function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
    address feeTo = IQuasarFactory(factory).feeTo();
  ...
  function mint(address to) external nonReentrant returns (uint256 liquidity) {
    ...
    require(liquidity > 0, 'Quasar: INSUFFICIENT_LIQUIDITY_MINTED');
    _mint(to, liquidity);
  ...
  function swap(
    ...
    (uint112 _reserve0, uint112 _reserve1, ) = getReserves(); // gas savings
    require(amount0Out < _reserve0 && amount1Out < _reserve1, 'Quasar: INSUFFICIENT_LIQUIDITY');
    ...
    uint256 balance1;
    {
      ...
      require(to != _token0 && to != _token1, 'Quasar: INVALID_TO');
    ...
    require(amount0In > 0 || amount1In > 0, 'Quasar: INSUFFICIENT_INPUT_AMOUNT');
    {
      // scope for reserve{0,1}Adjusted, avoids stack too deep errors
      uint256 balance0Adjusted = balance0.mul(1000).sub(amount0In.mul(3));
      uint256 balance1Adjusted = balance1.mul(1000).sub(amount1In.mul(3));
      require(balance0Adjusted.mul(balance1Adjusted) >= uint256(_reserve0).mul(_reserve1).mul(1000**2), 'Quasar: K');
    }
...
// File contracts/QuasarFactory.sol
