...
contract InsuranceFi is Context, IBEP20, Ownable {
    ...
    uint256 public numTokensToSwap = 1 * 10**2 * 10**18;
    ...
    constructor(address newOwner) {
        ...
        insuranceFundAddress = payable(
            0x1217b9728f31197Dd94deAd4D5792af698d8edc6
        );
        prPoolAddress = payable(
            0x85781bf68De49A9ADa3356fFF281b4c058f17314
        );
        busdPoolAddress = payable(
            0x19049Babe0aFee8c20A5Fa19c88A0feE096666eB
        );
    ...
    function setPoolsAddress(
        ...
        require(_insurance != insuranceFundAddress,"insuranceFund wallet is already that address");
        require(!isContract(_insurance),"insuranceFund wallet cannot be a contract");

        require(_pr != prPoolAddress,"prPool wallet is already that address");
        require(!isContract(_pr),"prPool wallet cannot be a contract");

        require(_busd != busdPoolAddress,"busdPool wallet is already that address");
        require(!isContract(_busd),"busdPool wallet cannot be a contract");

        insuranceFundAddress = payable(_insurance);
        prPoolAddress = payable(_pr);
        busdPoolAddress = payable(_busd);

        emit UpdatePoolsAddress(_insurance, _pr, _busd);
    }
    ...
    function setFees(
        ...
        require(_busdFee + _prFee + _insuranceFundFee <= 10, "Sell fees must be lower than 10%");
        require(_buyFee <= 10, "Buy fee must be lower than 10%");
    ...
    function swapAndCharge(uint256 tokenBalance) private lockTheSwap {
        ...
        uint256 _totalFee = insuranceFundFee.add(busdPoolFee).add(
            prAndDevFee
        );
        uint256 insurancePart = tokenBalance.mul(insuranceFundFee).div(_totalFee);
        uint256 swapBalance = tokenBalance - insurancePart;
        uint256 initialBalance = address(this).balance;
        swapTokensForEth(swapBalance);
        uint256 currentBalance = address(this).balance.sub(initialBalance);
        uint256 _currentFee = busdPoolFee.add(
            prAndDevFee
        );

        _transfer(address(this),insuranceFundAddress, insurancePart);
    ...
    event UpdatePoolsAddress(address insurance, address pr, address busd);
