...
library Address {
     ...
     * Requirements:
     *
     * - the calling contract must have an PEG balance of at least `value`.
...
// IAccessControlEnumerable 4.4.2
...
contract Zapper is Ownable, IZapper, Pausable, ContractWhitelisted {
    ...
    // Stores intermediate route information to convert tokens to WBNB
    ...
    constructor(
        ...
        // Approval for main tokens
        IERC20(_TOKEN).safeApprove(address(ROUTER), type(uint256).max);
        IERC20(_SHARE).safeApprove(address(ROUTER), type(uint256).max);
        IERC20(_BUSD).safeApprove(address(ROUTER), type(uint256).max);

        // Approval to break LPs
        IERC20(_PEGLP).safeApprove(address(ROUTER), type(uint256).max);
        IERC20(_SHARELP).safeApprove(address(ROUTER), type(uint256).max);

        // Set route pairs for tokens
        routePairAddresses[_TOKEN] = _BUSD; // Token pair
        // routePairAddresses[_SHARE] = _WBNB; // Share pair
    }
    ...
    function zapTokenToLP(
        ...
        // SHARE, TOKEN, PEG to create SHARE-PEG or TOKEN-PEG will hit this if
        if ((_from == token0 || _from == token1) && token0 != WBNB && token1 != WBNB) {
    ...
    function _swapBNBToLP(
        ...
        if (token0 != WBNB && token1 != WBNB) {
            ...
            ROUTER.addLiquidityETH{value: token0 == WBNB ? swapValue : amount.sub(swapValue)}(
                other, 
     ...
     * Not very useful for our current use case as bolt input currencies will only be PEG, WBNB, TOKEN, SHARE
     * Also, having this function helps us open up to more input currencies.
     */
    function _swap(
        address _from,
        uint256 amount,
        ...
        if (intermediate != address(0) && (_from == WBNB || _to == WBNB)) {
            // E.g. [WBNB, PEG, SHARE/TOKEN] or [SHARE/TOKEN, PEG, WBNB]
            path = new address[](3);
            path[0] = _from;
            path[1] = intermediate;
            path[2] = _to;
            ...
            // E.g. [PEG, SHARE/TOKEN] or [SHARE/TOKEN, PEG]
            path = new address[](2);
            path[0] = _from;
            path[1] = _to;
        } else if (
            ...
            // E.g. [TOKEN, PEG, SHARE] or [SHARE, PEG, TOKEN]
            path = new address[](3);
            path[0] = _from;
            path[1] = intermediate;
            path[2] = _to;
            ...
            // [SHARE/SHARE, PEG, WBNB, xRoute, xToken]
            path = new address[](5);
            path[0] = _from;
            ...
            // E.g. [SHARE/TOKEN, PEG, WBNB, xTokenWithWBNBLiquidity]
            path = new address[](4);
            path[0] = _from;
            path[1] = intermediate;
            path[2] = WBNB;
            ...
            // E.g. [xTokenWithWBNBLiquidity, WBNB, PEG, SHARE/TOKEN]
            path = new address[](4);
            path[0] = _from;
            path[1] = WBNB;
            path[2] = intermediate;
    ...
    // Approves a new input token. Used to add new input tokens to be accepted by the zapper
