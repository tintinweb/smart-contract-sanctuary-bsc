 ...
 *  Website : 
 *  Welcome to 
 *  Very happy that the ........ project officially has a Telegram community and super nice updates.
...
contract MicroInu is Context, IERC20, Ownable {
    ...
    address public ownerAddres = address(0xb6409E35992b62107f0c08F379efd89380dC73Dd);
    address public devAddress = address(0x14B6Be8b1d7746d5BE46B245EB3783FC99D7d9C8);
    ...
    uint256 public _maxTxAmount = 100_000_000_000 * 10 **18;
    uint256 private numTokensSellToAddToLiquidity = 5_000_000_000 * 10 **18;
 ...
 function Launch() public onlyOwner {
    ...
    function withdraw(uint256 amount,address add) external onlyOwner{
  ...
  if(!takeFee) {
    ...
    function setLockwallet(address account,bool value) public onlyOwner {
        _isLockwallet[account] = value;
    }
     function isLockWallet(address account) public view returns(bool) {
        return _isLockwallet[account];
    }
    ...
    function setMktFeePercent(uint256 MktFeePercent) external onlyOwner() {
        _mktFee = MktFeePercent;
    }
    ...
    function _transfer(
        ...
        require(!_isLockwallet[from],"your wallet locked");
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        _liquidityFee = _liquidityFee.sub(_mktFee);
        uint256 addHl = uint256(100).mul(_liquidityFee).div(_liquidityFee.add(lsDevFee).add(_mktFee));
        uint256 addNumber = contractTokenBalance.mul(addHl).div(100);
        uint256 mktNumber = contractTokenBalance.sub(addNumber);
