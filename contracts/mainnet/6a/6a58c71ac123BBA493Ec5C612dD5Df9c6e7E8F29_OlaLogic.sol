...
contract OlaLogic is Logic {
    ...
    onlyStorage
  {
    ...
    collateralFactor = ((collateralFactor * 100) / (10**18)) - 2; // 70% - 2%(to avoid liquidation) = 68%

    uint256 redeemAmount = (amount * (collateralFactor ^ circlesCount)) / (100 ^ circlesCount);
    for (uint256 i = circlesCount; i > 0; i--) {
      IXToken(cToken).redeemUnderlying(redeemAmount);
      IXToken(cToken).repayBorrow(redeemAmount);
      redeemAmount = redeemAmount / collateralFactor * 100;
    }
    IXToken(cToken).redeemUnderlying(redeemAmount);
