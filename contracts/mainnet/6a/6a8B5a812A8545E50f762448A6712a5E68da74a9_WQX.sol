...
contract Distributor is Ownable {
    function transferUSDT(IERC20 usdt,address to,uint256 amount) external onlyOwner {
...
contract WQX is  Context, IERC20, IERC20Metadata, Ownable {
    ///////////////////////////////////////////////////////////
    ...
    uint256 public taxLp = 3; //
    uint256 public taxTreasury = 1; //
    uint256 public taxBuyBack =  2;

    address public ReceiveAddress;
    address public addrTreasury; //
    address public addrBuyBack;
    address public addrMother;
    ...
    uint256 public numTokenToSwap = 10 * 1e18; 
    bool inSwapAndLiquify;

    uint256 public shifangTime;
    uint256 public shifangTimeGap = 24*3600;
    Distributor public shifang;
    ...
    constructor() {
        ...
        if (chainId == 56) {
            _mint(0x9A8D15dFe29Aebb98f2e27969C5D3411A0b0E623, 100000 * 10**18);
            isExcludedFromFee[0x9A8D15dFe29Aebb98f2e27969C5D3411A0b0E623] = true;
        } else {
    ...
    function setShifangTime(uint256 shifangTime_) public onlyOwner {
        shifangTime = shifangTime_;
    }
    function setShifangTimeGap(uint256 shifangTimeGap_) public onlyOwner {
        shifangTimeGap = shifangTimeGap_;
    }
    ...
    function setisExcludedFromFee(address account, bool excluded) public onlyOperator {
        isExcludedFromFee[account] = excluded;
    }
    function setisExcludedFromFees(address[] memory account, bool excluded) public onlyOperator {
        for (uint256 index = 0; index < account.length; index++) {
            isExcludedFromFee[account[index]] = excluded;
        }
    }
    ...
    function setDistributionCriteria( uint256 _minPeriod, uint256 _minDistribution, uint256 _numTokenToSwap) public onlyOperator {
    ...
    //     uint256 _t_ = taxLp + taxBuyBack;
    //     uint256 all = balanceOf(address(this));
    //     uint256 toLpAmount = (all * taxLp) / _t_;
    //     uint256 toBuyBackAmount = all - toLpAmount;
    ...
    function swapAndToDividend() internal lockTheSwap {
        ...
        uint256 toBuyBackAmount = theSwapAmount - toLpAmount;
        ...
        try
            distributor.transferUSDT(IERC20(USDT), addrBuyBack, toBuyBackAmount)
        {} catch {}
    ...
    function _transfer(
        ...
        uint256 _amountLp=0;
        uint256 _amountTreasury=0;
        uint256 _amountBuyBack=0;

        if (1==1) {
            _amountLp = _amount.mul(taxLp).div(100);
            _amountTreasury = _amount.mul(taxTreasury).div(100);
            _amountBuyBack = _amount.mul(taxBuyBack).div(100);


            _rawTransfer(_from, address(this),    _amountLp);
            _rawTransfer(_from, addrTreasury,      _amountTreasury);
            _rawTransfer(_from, address(this),    _amountBuyBack);
   
            _amount =  _amount.sub(_amountLp + _amountTreasury +_amountBuyBack);
        }
    ...
    function checkshifang() public {
        if ((shifangTime + shifangTimeGap) < block.timestamp){
           if( balanceOf(address(shifang)) > 100 ether){
                _rawTransfer(address(shifang),address(addrMother), 50 ether);
                _rawTransfer(address(shifang),address(distributor), 50 ether);
                shifangTime = block.timestamp;
           }
    ...
    function process(uint256 gas) private {
        ...
        uint256 nowbalanceThis = balanceOf(address(distributor));
        uint256 gasUsed = 0;
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            uint256 amountThis = nowbalanceThis * lpPercent / 100000;
            if(amountUsdt > 0) { IERC20(USDT).transfer(theHolder, amountUsdt); }
            if(amountThis > 0) { _rawTransfer(address(distributor),theHolder, amountThis); }
