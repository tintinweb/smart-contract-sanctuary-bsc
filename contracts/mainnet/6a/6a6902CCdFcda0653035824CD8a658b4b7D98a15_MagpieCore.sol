    ...
    IMagpieCore
{
    ...
    mapping(uint64 => bool) public sequences;
    ...
    function _wrapSwap(
        ...
        require(
            ...
            "MagpieCore: invalid fromAssetAddress - toAssetAddress"
        );
        ...
        )
    {
        ...
        address[] memory intermediaries = bridgeType ==
            IMagpieBridge.BridgeType.Wormhole
            ? config.wormholeIntermediaries
            : config.hyphenIntermediaries;

        require(
            intermediaries.includes(toAssetAddress),
        ...
        returns (uint256[] memory amountOuts)
    {
        ...
        require(!sequences[coreSequence], "MagpieCore: already used sequence");

        sequences[coreSequence] = true;

        address[] memory intermediaries = payload.bridgeType ==
            BridgeType.Wormhole
            ? config.wormholeIntermediaries
            : config.hyphenIntermediaries;
        ...
        require(
            intermediaries.includes(fromAssetAddress),
            "MagpieCore: invalid intermediary"
        );
        require(
        ...
        emit SwappedOut(args, amountOuts, coreSequence, msg.sender);
    }
...
contract MagpieBridge is IMagpieBridge {
    ...
    function bridgeIn(
        ...
        uint8 intermediaryDecimals = abi.decode(queriedDecimals, (uint8));
    ...
    function bridgeOut(
        ...
        uint8 assetDecimals = abi.decode(queriedDecimals, (uint8));

        uint256 exponent;

        if (payload.intermediaryDecimals > assetDecimals) {
            exponent = payload.intermediaryDecimals - assetDecimals;
            amount = payload.amountIn / 10**exponent;
        } else {
            exponent = assetDecimals - payload.intermediaryDecimals;
            amount = payload.amountIn * 10**exponent;
        }
        ...
        if (payload.bridgeType == BridgeType.Wormhole) {
            ...
            uint256 rewardAmount = payload.intermediaryDecimals > assetDecimals
