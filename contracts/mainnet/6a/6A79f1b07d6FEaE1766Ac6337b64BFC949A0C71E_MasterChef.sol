...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of ACRs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accAcrPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accAcrPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. ACR to distribute per block.
        uint256 lastRewardBlock;  // Last block number that ACR distribution occurs.
        uint256 accAcrPerShare;   // Accumulated ACR per share, times 1e12. See below.
        uint16 withdrawFeeBP;      // Withdraw fee in basis points
    }

    // The ACRUX TOKEN!
    ACRUXTOKEN public acr;
    // ACRUX tokens created per block.
    uint256 public acrPerBlock;
    // Bonus muliplier for early ACR makers.
    ...
    // The block number when ACR mining starts.
    ...
    event EmissionRateUpdated(address indexed user, uint256 acrPerBlock);

    constructor(
        ACRUXTOKEN _acr,
        address _feeAddDev,
        uint256 _acrPerBlock,
        uint256 _startBlock
    ) public {
        acr = _acr;
        feeAddDev = _feeAddDev;
        acrPerBlock = _acrPerBlock;
    ...
    // Update the given pool's ACR allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending ACRs on frontend.
    function pendingAcr(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accAcrPerShare = pool.accAcrPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 acrReward = multiplier.mul(acrPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accAcrPerShare = accAcrPerShare.add(acrReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accAcrPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 acrReward = multiplier.mul(acrPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        acr.mint(acrReward);
        pool.accAcrPerShare = pool.accAcrPerShare.add(acrReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for ACR allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant poolExists(_pid) {
        ...
        user.rewardDebt = user.amount.mul(pool.accAcrPerShare).div(1e12);
    ...
    // Safe ACR transfer function, just in case if rounding error causes pool to not have enough ACRs.
    function safeAcrTransfer(address _to, uint256 _amount) internal {
        uint256 acrBal = acr.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > acrBal) {
            transferSuccess = acr.transfer(_to, acrBal);
        } else {
            transferSuccess = acr.transfer(_to, _amount);
        }
        require(transferSuccess, "safeAcrTransfer: transfer failed");
    }
    ...
    function harvest(uint256 _pid) internal {
        ...
        uint256 pending = user.amount.mul(pool.accAcrPerShare).div(1e12).sub(user.rewardDebt);
        if (pending > 0) {
            // send rewards
            safeAcrTransfer(msg.sender, pending);
        }
    }

    // Pancake has to add hidden dummy pools in order to alter the emission, here we make it simple and transparent to all.
    function updateEmissionRate(uint256 _acrPerBlock) public onlyOwner {
        massUpdatePools();
        emit EmissionRateUpdated(msg.sender, _acrPerBlock);
        acrPerBlock = _acrPerBlock;
    }
