...
contract SafeGuard is OwnableUpgradeSafe {
   ...
   constructor() public payable {  
    ...
    function rescueTokens() external onlyOwner {
        uint256 balance = Rescue(contractAddress).balanceOf(address(this));
		Rescue(contractAddress).transfer(Owner, balance);
    }

}

// yENTITY Vault distributes fees equally amongst staked pools
// Have fun reading it. Hopefully it's bug-free. God bless.
contract YentityVault is OwnableUpgradeSafe, SafeGuard {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of yENTITY
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accYentityPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws  tokens to a pool. Here's what happens:
        //   1. The pool's `accYentityPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. yENTITY to distribute per block.
        uint256 accYentityPerShare; // Accumulated yENTITY per share, times 1e12. See below.
    ...
    function add(
        ...
        poolInfo.push(
            PoolInfo({
                ...
                accYentityPerShare: 0,
                withdrawable : _withdrawable
            })
        );
    }

    // Update the given pool's yENTITY allocation point. Can only be called by the owner.
    ...
    // View function to see pending yENTITY on frontend.
    function pendingYentity(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        uint256 accYentityPerShare = pool.accYentityPerShare;

        return user.amount.mul(accYentityPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    // ----
    // Function that adds pending rewards, called by the yENTITY token.
    // ----
    uint256 private yentityBalance;
    function addPendingRewards(uint256 _) public {
        uint256 newRewards = yentity.balanceOf(address(this)).sub(yentityBalance);

        if(newRewards > 0) {
            yentityBalance = yentity.balanceOf(address(this)); // If there is no change the balance didn't change
    ...
    function updatePool(uint256 _pid) internal returns (uint256 yentityRewardWhole) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        yentityRewardWhole = pendingRewards // Multiplies pending rewards by allocation point of this pool and then total allocation
        ...
        uint256 yentityRewardFee = yentityRewardWhole.mul(DEV_FEE).div(10000);
        uint256 yentityRewardToDistribute = yentityRewardWhole.sub(yentityRewardFee);

        pending_DEV_rewards = pending_DEV_rewards.add(yentityRewardFee);

        pool.accYentityPerShare = pool.accYentityPerShare.add(
            yentityRewardToDistribute.mul(1e12).div(tokenSupply)
        );
    ...
    // Deposit tokens to YentityVault for yENTITY allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accYentityPerShare).div(1e12);
    ...
    function depositFor(address depositFor, uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accYentityPerShare).div(1e12); /// This is deposited for address
    ...
    // Withdraw  tokens from YentityVault.
    ...
    function _withdraw(uint256 _pid, uint256 _amount, address from, address to) internal {
        ...
        massUpdatePools();
        updateAndPayOutPending(_pid,  pool, user, from); // Update balances of from this is not withdrawal but claiming yENTITY farmed
    ...
    function updateAndPayOutPending(uint256 _pid, PoolInfo storage pool, UserInfo storage user, address from) internal {
        ...
        uint256 pending = user
            .amount
            .mul(pool.accYentityPerShare)
            .div(1e12)
            .sub(user.rewardDebt);

        if(pending > 0) {
            safeYentityTransfer(from, pending);
        }
    ...
    // Safe yENTITY transfer function, just in case if rounding error causes pool to not have enough yENTITY.
    function safeYentityTransfer(address _to, uint256 _amount) internal {
        if(_amount == 0) return;

        uint256 yentityBal = yentity.balanceOf(address(this));
        if (_amount > yentityBal) {
            yentity.transfer(_to, yentityBal);
            yentityBalance = yentity.balanceOf(address(this));

        } else {
            yentity.transfer(_to, _amount);
            yentityBalance = yentity.balanceOf(address(this));
    ...
    function transferDevFee() public {
        ...
        uint256 yentityBal = yentity.balanceOf(address(this));
        if (pending_DEV_rewards > yentityBal) {
            yentity.transfer(devaddr, yentityBal);
            yentityBalance = yentity.balanceOf(address(this));
        } else {
            yentity.transfer(devaddr, pending_DEV_rewards);
            yentityBalance = yentity.balanceOf(address(this));
        }
