...
contract DRE is IERC20, Ownable {
    ...
    constructor (){
        ...
        uint256 total = 66666 * 10 ** _decimals;
        _tTotal = total;
        maxHolder = total;

        fundAddress = 0x4E00de34d770890b1656357b259e6617F7d450cb;
        ...
        _limitAmount = 66666 * 10 ** _decimals;
                ...
                if (_swapPairList[to]) {
                    ...
                    if (!inSwap) {
                        ...
                        if (contractTokenBalance > 0) {
                            uint256 swapFee = _buyFundFee + _buyLPDividendFee + _sellFundFee + _sellLPDividendFee + _buyLPFee + _sellLPFee;
        ...
        uint256 feeAmount;
         ...
         { 
            ...
            if (sender == _mainPair) {//Buy
                    swapAmount = tAmount * (_buyFundFee + _buyLPFee + _buyLPDividendFee) / 10000;
                    if(swapAmount > 0){
                     ...
                     swapAmount = tAmount * (_sellFundFee + _sellLPFee + _sellLPDividendFee) / 10000;
                    if(swapAmount > 0){
                        _takeTransfer(sender, address(this), swapAmount);
                        feeAmount += swapAmount;
                    }
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        uint256 lpUsdt = fistBalance * lpFee / swapFee;
        if (lpUsdt > 0 && lpAmount > 0) {
            _swapRouter.addLiquidity(
                address(this), _fist, lpAmount, lpUsdt, 0, 0, fundAddress, block.timestamp
            );
        }
