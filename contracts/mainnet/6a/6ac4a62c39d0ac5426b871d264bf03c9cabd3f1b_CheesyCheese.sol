...
contract CheesyCheese is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    CheesyCheeseDividendTracker public dividendTracker;
    ...
    //staking variables
    mapping(address => uint256) private tokensStaked1Week;
    mapping(address => uint256) private tokensStaked1Month;
    mapping(address => uint256) private tokensStaked6Months;
    mapping(address => bool) private _isStaked1Week; 
    mapping(address => bool) private _isStaked1Month;  
    mapping(address => bool) private _isStaked6Months;
    ...
    address public SwissArmyContractAddress;
    ...
    constructor() ERC20("CheesyCheese", "CheesePlx") {
        ...
        dividendTracker = new CheesyCheeseDividendTracker(payable(this), router, 0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48,
                                                   "CheesyCheeseTRACKER", "CheesePlxTRACKER");
    ...
    function Unstake() public {
        if (_isStaked1Week[msg.sender] && block.timestamp >= _staked1WeekTimestamp[msg.sender] + 10 minutes) {
            _isStaked1Week[msg.sender] = false;
        }

        if (_isStaked1Month[msg.sender] && block.timestamp >= _staked1MonthTimestamp[msg.sender] + 15 minutes) {
            _isStaked1Month[msg.sender] = false;
        }

        if (_isStaked6Months[msg.sender] && block.timestamp >= _staked6MonthsTimestamp[msg.sender] + 20 minutes) {
    ...
    function getStakingBalance(address account) private view returns (uint256) {
        ...
        if (_isStaked1Week[account]) {
            stakedBalance1Week = tokensStaked1Week[account].mul(55).div(100);
        } 
        ...
        stakingBalance = unstakedBalance.add(stakedBalance1Week + stakedBalance1Month + stakedBalance6Months);
...
contract CheesyCheeseDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    CheesyCheese public CheesyCheeseContract;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        CheesyCheeseContract = CheesyCheese(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main CheesyCheese contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(CheesyCheeseContract);

            uint256 prevBalance = CheesyCheeseContract.balanceOf(address(this));
            ...
            {
                uint256 received = CheesyCheeseContract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    CheesyCheeseContract.transfer(account, received);
                } 
                else {
                    success = false;
                }
