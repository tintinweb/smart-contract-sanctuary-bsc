...
contract AlveyDAO is IBEP20, Auth {
    ...
    string constant _name = "Alvey DAO";
    string constant _symbol = "wALVDAO";
    ...
    uint256 private DAOFee         = 1;
    uint256 private developmentFee = 0;
    uint256 public stakingFee      = 0;
    uint256 public totalFee        = developmentFee + marketingFee + liquidityFee + DAOFee + stakingFee;
    ...
    address private DAOFeeReceiver;
    ...
    constructor () Auth(msg.sender) {
        ...
        address routerV2 = 0x27451be644bf298E77DdcDD824FdB7CEce1E3A41; 

        _allowances[address(this)][address(router)] = type(uint256).max;

        isFeeExempt[msg.sender] = true;
        isFeeExempt[marketingFeeReceiver] = true;
        isFeeExempt[DAOFeeReceiver] = true;
        ...
        isTimelockExempt[DAOFeeReceiver] = true;

        isTxLimitExempt[msg.sender] = true;
        isTxLimitExempt[pair] = true;
        isTxLimitExempt[marketingFeeReceiver] = true;
        isTxLimitExempt[DAOFeeReceiver] = true;

        autoLiquidityReceiver = msg.sender;
        marketingFeeReceiver = 0x27451be644bf298E77DdcDD824FdB7CEce1E3A41;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBDAO = amountBNB.mul(DAOFee).div(totalBNBFee);

        (bool tmpSuccess,) = payable(marketingFeeReceiver).call{value: amountBNBMarketing}("");
        (tmpSuccess,) = payable(DAOFeeReceiver).call{value: amountBNBDAO}("");
    ...
    function setFees(uint256 _liquidityFee, uint256 _developmentFee, uint256 _marketingFee, uint256 _DAOFee, uint256 _stakingFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        developmentFee = _developmentFee;
        marketingFee = _marketingFee;
        DAOFee = _DAOFee;
        stakingFee = _stakingFee;
        totalFee = _liquidityFee.add(_developmentFee).add(_marketingFee).add(_DAOFee).add(_stakingFee);
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator/2, "Buy Fees cannot be more than 50%");
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _DAOFeeReceiver, address _stakingFeeReceiver, address _developmentFeeReceiver) external authorized {
