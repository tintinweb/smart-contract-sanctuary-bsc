...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            withdrawTime[msg.sender] = block.number.add(403200);
        }
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        uint256 blockWithdraw = withdrawTime[msg.sender];
        _updatePool();

        uint256 pending = user.amount.mul(accTokenPerShare).div(PRECISION_FACTOR).sub(user.rewardDebt);

        if (_amount > 0) {
            if(block.number < bonusEndBlock && block.number > blockWithdraw){
                // apply 10% penalty
                user.amount = user.amount.sub(_amount);
                uint256 feeAmount = _amount.mul(10).div(100); // 10% fee
                stakedToken.safeTransfer(0x5A89fBB6C4a7988c2d9ac676CFc0817b86749923, feeAmount);
                stakedToken.safeTransfer(address(msg.sender), _amount.sub(feeAmount));
            } else {
            user.amount = user.amount.sub(_amount);
