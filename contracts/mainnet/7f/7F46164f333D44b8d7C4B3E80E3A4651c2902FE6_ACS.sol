...
contract ACS is ERC20 {
    ...
    constructor(
        ...
        uint maxSupply = 1_000_000 ether;
        _mint(sender, maxSupply);

        maxBurn = 50_000 ether;
    ...
    function action(
        ...
        for(uint i = 0; i < len; i++) {
            ...
            if (_action == ACTION_MAX_BURN) {
                ...
                address[2] memory _setAddress = abi.decode(_data[i], (address[2]));
                node = _setAddress[0];
                ranking = _setAddress[1];
            }
        }
    }
    ...
    function _transfer(
        ...
        if ( takeFree ) {
            ...
            uint _burnNode = _burned * 3 / 10;
            balanceOf[node] += _burnNode;
            emit Transfer(from, node, _burnNode);

            uint _burnRanking = _burned * 2 / 10;
            balanceOf[ranking] += _burnRanking;
            emit Transfer(from, node, _burnRanking);
            
            uint _burnAmount = _burned - _burnNode - _burnRanking;
