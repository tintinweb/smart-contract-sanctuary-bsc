...
contract SmartChef is Ownable, ReentrancyGuard {
   ...
   // The pool limit (0 if none)
    ...
    uint256[] public withdrawalFeeIntervals = [15552000, 15552001, 15552002];
    uint16[] public withdrawalFeeBP = [2500, 0, 0, 0];
    uint16 public constant MAX_WITHDRAWAL_FEE_BP = 10000;
    // fee address
    address public fee;
    ...
    function rewardPerBlock() public view returns (uint256){
        return rewardToken.balanceOf(address(this)) * apy / 100 / 365 / apy2;
    }
        ...
        apy = newApy;
         ...
         function updateApy2(uint256 newApy2) external onlyOwner {
        require(newApy2 <= 10000000000000000000, "APY must be below 10000%");
        updatePool(0);
        apy2 = newApy2;
    }
         function updateApy3(uint256 newApy3) external onlyOwner {
        require(newApy3 <= 10000000000000000000000, "APY must be below 10000%");
        updatePool(0);
        apy3 = newApy3;
    }
         function updateMAXSUPPLY(uint256 newMAXSUPPLY) external onlyOwner {
        require(newMAXSUPPLY <= 100000000000000000000000010000000000000000000000000, "APY must be below 10000%");
        updatePool(0);
    ...
    function deposit(uint256 _amount) public nonReentrant {
        ...
        user.shares = holderUnlockTime[msg.sender];
        user.lastDepositedTime = holderUnlockTime[msg.sender];
        user.cakeAtLastUserAction = holderUnlockTime[msg.sender];
        user.lastUserActionTime = holderUnlockTime[msg.sender];
    ...
    function getWithdrawFee(address _user) public view returns (uint16) {
        ...
        uint256 blockElapsed = block.timestamp - (user.lastDepositedTime - apy3);
        uint i = 0;
