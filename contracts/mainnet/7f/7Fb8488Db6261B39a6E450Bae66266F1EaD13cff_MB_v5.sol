...
contract MB_v5{
    ...
    function burnToken(uint _amount ) public {
       rewardToken.burn_internal(_amount, msg.sender);
       users[msg.sender].burnedToken += _amount;
    ...
    function withdraw( address _user ,uint _amount ) public {
        ...
        require(user.burnedToken >= halfWithdraw,"you don't burned enough tokens");
    ...
    function getUserRefferal( address _useraddress) public view returns (address){
         User storage u = users[_useraddress] ;
         return (u.referrer);
    }

    function mintToken(address _useraddress, uint256 _amount) public{
        require(msg.sender == ownerWallet, "permission Denied");
        rewardToken.mintTokens(_useraddress, _amount);
    }

    function updatefeeWallet(address payable _useraddress) public{
        require(msg.sender == ownerWallet, "permission Denied");
        feewallet = _useraddress;
    }
