...
contract Killer is IERC20, Auth {
    ...
    */
    ...
    IERC20 public pairToken = IERC20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
    ...
    string constant public _name = "KLRTEST";
    string constant public _symbol = "KT";
    

    bool public antiSnipeEnabled = true;

    constructor (address tokenOwner, address stableCoin) Auth(tokenOwner) {
        router = IDEXRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E); //Mainnet Pancakeswap
        WETH = address(router.WETH());
        pairToken = IERC20(stableCoin);
        pair = IDEXFactory(router.factory()).createPair(address(pairToken), address(this));
        pairs[pair] = true;
        _allowances[address(this)][address(router)] = _totalSupply;
        
        pairToken.approve(address(router), pairToken.totalSupply());
    ...
    function airdrop(address[] calldata toWhoArray, uint[] calldata amountArray)external returns (bool success) {
        require(toWhoArray.length == amountArray.length, "Killer: amounts length must match Accounts length");
        uint totalTokens = 0;
        for(uint i = 0; i< amountArray.length; i++){
            totalTokens += amountArray[i] *10**_decimals;
        }
        require(_balances[msg.sender] >= totalTokens, "Killer: you cannot transfer more than you own");
         for(uint i = 0; i< amountArray.length; i++){
             if(!isMaxHoldExempt[toWhoArray[i]]){
                require(_balances[toWhoArray[i]] + amountArray[i]*10**_decimals <= _maxHoldAmount, "Killer: you cannot put an address over the balance they can max hold");
             }
    ...
    function burn(uint amount)external returns(bool success){
        require(_balances[msg.sender] > amount * 10 ** _decimals, "Killer: cannot burn more than you own");
    ...
    function setSwapThresholdDivisor(uint divisor)external authorized {
        require(divisor >= 100, "Killer: max sell percent is 1%");
    ...
    function setMaxHoldPercentage(uint256 percentageMul10) external authorized {
        require(percentageMul10 >= 5, "Killer, max hold cannot be less than 0.5%"); // cant change percentage below 0.5%, so everyone can hold the percentage
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!canTrade){
            require(sender == owner , "Killer, Only owner or presale Contract allowed to add LP"); // only owner allowed to trade or add liquidity
        }
        if(sender != owner && recipient != owner ){
            if(!pairs[recipient] && !isMaxHoldExempt[recipient]){
                require (balanceOf(recipient) + amount <= _maxHoldAmount, "Killer, cant hold more than max hold dude, sorry");
            }
        }
        ...
        if(shouldSwapBack(pairs[recipient])){ swapBack(); }
    ...
    function claimtokensback(IERC20 tokenAddress) external authorized {
        ...
        require(address(tokenAddress) != address(this), "Killer: not allowed to remove from swap/tax pool");
    ...
    */
    function setTxLimit(uint256 amount) external authorized {
        require(amount >= _totalSupply / 1000, "Killer, must be higher than 0.1%");
        require(amount > _maxTxAmount, "Killer, can only ever increase the tx limit");
        _maxTxAmount = amount;
    }
    ...
    */
    function setBuyFees(uint8 _liquidityFee, uint8 _marketingFee, uint8 _gameDevFee) external authorized {
        require(_liquidityFee <= 5 && _marketingFee <= 5 && _gameDevFee <= 5, "Killer, Taxes can only be set to 0.5% individual values");
    ...
    */
    ...
    function swapBack() internal swapping {
        ...
        if(dynamicLiquidityFee > 0) {
             amountToLiquify = (swapThreshold*(dynamicLiquidityFee)) / ((totalBuyFee)/(2)); // leave some tokens for liquidity addition
             amountToSwap -= amountToLiquify; // swap everything bar the liquidity tokens. we need to add a pair
        }
        ...
        uint256 amountPair = pairToken.balanceOf(address(this)) - (balanceBefore);

        uint256 totalETHFee = totalBuyFee - (dynamicLiquidityFee / 2);
        if(totalETHFee > 0 ){
            if(marketingFee > 0){
                uint256 amountETHMarketing = amountPair * (marketingFee)/(totalETHFee);
                payable(marketingFeeReceiver).transfer(amountETHMarketing);
            }
            if(developmentFee > 0){
                uint256 amountETHDev = amountPair*(developmentFee)/(totalETHFee);
                payable(devFeeReceiver).transfer(amountETHDev);
            }
                   
            if(amountToLiquify > 0){
                uint256 amountETHLiquidity = amountPair * (dynamicLiquidityFee) / (totalETHFee)/(2);
                router.addLiquidity(
                    address(pairToken),
                    address(this),
                    amountETHLiquidity,
