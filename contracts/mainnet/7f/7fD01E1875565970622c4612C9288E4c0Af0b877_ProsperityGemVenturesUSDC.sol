...
contract ProsperityGemVenturesUSDC is Context, Ownable {
   ...
   uint16 constant FEE_DIVIDER = 1000; 
    ...
    function HuntGems(uint256 amount) external {
	...
	function banGemHunter(address wallet) public onlyOwner returns (bool success) {
        banned[wallet] = 1;
        nextBannedWallet++;
        return true;
    }
	
	function unbanGemHunter(address wallet) public onlyOwner returns (bool success) {
        banned[wallet] = 0;
        if(nextBannedWallet > 0){ nextBannedWallet--; }
        return true;
    }	

    function exitGemHunt(address wallet) public onlyOwner returns (bool success) {
