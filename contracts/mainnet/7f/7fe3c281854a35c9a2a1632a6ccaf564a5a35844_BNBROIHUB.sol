...
contract BNBROIHUB {
    ...
    uint constant public INVEST_MIN_AMOUNT = 1e16 ;
    uint constant public INVEST_MAX_AMOUNT = 500 ether;
    uint constant public BASE_PERCENT = 150;
    uint[] public REFERRAL_PERCENTS = [500, 200, 100, 100, 50, 50];
    uint constant public MARKETING_FEE = 200; 
    ...
    uint constant public CONTRACT_BALANCE_STEP = 100 ether;
	...
	User storage user = users[msg.sender];
	require(user.deposits.length < DEPOSITS_MAX, " max 300 depsoits");
    uint256 divamount = getUserAvailable(msg.sender);
    require(divamount > 0, "ROIHUB: No amount to reinvest");	
    ...
    function getUserLastcheckpoint(address userAddress) public view returns (uint) {
        User storage user = users[userAddress];
    ...
    function getUserReferralsStats(address userAddress) public view returns (address, uint64, uint24[6] memory) {
        User storage user = users[userAddress];
