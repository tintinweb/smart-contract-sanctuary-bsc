...
contract DividendPayingToken is ERC20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
  ...
  constructor(string memory _name, string memory _symbol, address _rewardTokenAddress) ERC20(_name, _symbol) { //Look Here to fix
        require(_rewardTokenAddress!= address(0), "Cannot set zero address as Reward Token"); //----> FIX
 ...
 function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
    require(user != address(0),"Address Invalid! Cannot pass zero address");
...
contract TokenDividendTracker is Ownable, DividendPayingToken {
    ...
    event SetMinimumTokenBalanceForDividends(uint256 val); // ---->Fix

    event  MapSet(address indexed key, uint val); ///-----> Fix

    event MapRemove(address indexed key);
    ...
    function setMinimumTokenBalanceForDividends(uint256 val) external onlyOwner {
        ...
        emit SetMinimumTokenBalanceForDividends(val); ///---->FIX
    }

    function excludeFromDividends(address account) external onlyOwner {
        require(!excludedFromDividends[account] && account != address(0),"Account Already Excluded or Invalid Address"); //--> Missing error message FIX
    ...
    function MAPSet(address key, uint val) internal { //--->Fix for medium and one low
        if (tokenHoldersMap.inserted[key]) {
    ...
    emit MapSet(key, val); //-->Fix
    }


    function MAPRemove(address key) internal { //--> Fix for Medium and one low
        if (!tokenHoldersMap.inserted[key]) {
            return;
        }
        ...
        emit MapRemove(key); //-->Fix
    }
    
}


contract MZEGAME is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    event SetMarketingWallet(address indexed wallet); //--->Fix

    event Enemyaddress(address indexed account, bool indexed value); ///--->Fix

    event SetBuyTaxes(uint256 liquidity, uint256 rewardsFee, uint256 marketingFee, uint256 deadFee); //-->Fix

    event SetSellTaxes(uint256 liquidity, uint256 rewardsFee, uint256 marketingFee, uint256 deadFee); //-->Fix
    ...
    constructor(
        ...
        buyTokenRewardsFee = buyFeeSetting_[1];
        buyLiquidityFee = buyFeeSetting_[1];
        buyMarketingFee = buyFeeSetting_[2];
        buyDeadFee = buyFeeSetting_[1];

        sellTokenRewardsFee = sellFeeSetting_[1];
        sellLiquidityFee = sellFeeSetting_[2];
    ...
    function updateMinimumTokenBalanceForDividends(uint256 val) public onlyOwner {
        dividendTracker.setMinimumTokenBalanceForDividends(val); //--> FIx on Line973
    }
    ...
    function excludeMultipleAccountsFromFees(address[] calldata accounts, bool excluded) public onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++) {
            require(accounts[i]!=address(0), "Zero Address cannot be excluded from fees."); //-----> Fix
    ...
    function setMarketingWallet(address payable wallet) external onlyOwner{
        ...
        require(wallet!=_marketingWalletAddress && wallet != address(0),"Invalid Address"); //--> 
        _marketingWalletAddress = wallet; //--> Missing Zero check
        emit SetMarketingWallet(wallet);
    }

    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value); //--> Missing Zero check
    }

    function EnemyAddress(address account, bool value) external onlyOwner{
        require(account != address(0),"Invalid Address");
        _isEnemy[account] = value; //--> Blacklisting any address at any time
        emit Enemyaddress(account, value); //---->Fix
    }
    ...
    function setSwapTokensAtAmount(uint256 amount) public onlyOwner {
        swapTokensAtAmount = amount; //--> Missing Check
    }

    // function setDeadWallet(address addr) public onlyOwner { -----> This function should not exist
    //     deadWallet = addr; //--> should not exist
    // }
    ...
    function setBuyTaxes(uint256 liquidity, uint256 rewardsFee, uint256 marketingFee, uint256 deadFee) external onlyOwner {
        ...
        buyDeadFee = deadFee;
        emit SetBuyTaxes(liquidity, rewardsFee, marketingFee, deadFee); //--->Fix
    }
    ...
    function setSellTaxes(uint256 liquidity, uint256 rewardsFee, uint256 marketingFee, uint256 deadFee) external onlyOwner {
        ...
        sellDeadFee = deadFee;
        emit SetSellTaxes(liquidity, rewardsFee, marketingFee, deadFee); //---->Fix
    }
