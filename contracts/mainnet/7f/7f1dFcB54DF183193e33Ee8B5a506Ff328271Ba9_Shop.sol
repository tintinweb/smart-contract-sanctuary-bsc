...
contract Shop is Ownable {
    ...
    function getUserCatCounts(address _addr) external view returns(uint[7] memory) {
        uint b1 = IERC721(catAddress[Cat.SnoopDogg]).balanceOf(_addr);
        uint b2 = IERC721(catAddress[Cat.Urban]).balanceOf(_addr);
        uint b3 = IERC721(catAddress[Cat.Rothschild]).balanceOf(_addr);
        uint b4 = IERC721(catAddress[Cat.SakuragiHanamichi]).balanceOf(_addr);
        uint b5 = IERC721(catAddress[Cat.MasterShifu]).balanceOf(_addr);
        uint b6 = IERC721(catAddress[Cat.Spock]).balanceOf(_addr);
        uint b7 = IERC721(catAddress[Cat.ChanKwongYan]).balanceOf(_addr);
