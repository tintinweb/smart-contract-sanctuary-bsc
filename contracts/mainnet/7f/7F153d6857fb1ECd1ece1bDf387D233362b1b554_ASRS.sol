...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    contract ASRS is ERC20, Ownable {
        using SafeMath for uint256;
        using Address for address;

        string private    _name = "ASRS ALLIANCE";
        string private  _symbol = "ASRS";
   ...
   address payable charityWallet = payable(0x6947e56803e832cA2f37A83645733440d80d2cEe);
    ...
    function totalFees() public view returns (uint256) {
        return _liquidityFee.add(_charityFee);
    }
    ...
    function changeCharityWallet(address payable wallet) public onlyOwner
    {
    ...
    function removeAllFee() private {
        if (_liquidityFee == 0 && _charityFee == 0 ) {
            return;
        }
    ...
    function swapBack(uint256 tokensToLiquify) internal lockTheSwap {
        ...
        uint256 bnbForCharity = bnbBalance.mul(_charityFee).div(bnbFeeFactor);
      
      

        addLiquidity(tokensToLP, bnbForLiquidity);

        payable(charityWallet).transfer(bnbForCharity);
    }
