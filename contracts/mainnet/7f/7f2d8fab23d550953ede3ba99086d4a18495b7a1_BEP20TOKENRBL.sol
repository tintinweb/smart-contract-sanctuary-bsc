...
contract BEP20TOKENRBL is Context, IBEP20, Ownable {
    ...
    constructor() {
        ...
        // IPancakeRouter _router = IPancakeRouter(0xCc7aDc94F3D80127849D2b41b6439b7CF1eB4Ae0); //for test
        IPancakeRouter _router = IPancakeRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E);//pancake main

        // _pairAddress = IPancakeFactory(_router.factory()).createPair(address(this), _usdt);
        _routerAddress = address(_router);
        //啦啦啦啦
        _addresses[0]=0x5f792dC8C5e98cA8fb3dC7d71Af47527b213D6F7;
        _addresses[1]=0x1A3007A637F342E49bE72E64857175eE50C1e72f;
        _addresses[2]=0x3B6B9E44383f9392a3918ae12738837811260494;
        _addresses[3]=0x081A0192Ba09c694CBCf60aA7b9497cEe1Ee5136;//
        //        _addresses.push(0xadA37191D9396a28588B711652377dD9d0b25
        _addresses[4]=0x14502A2f1d684B65aCAcAF67eDc8d7b5c505d07c;//4
        _addresses[5]=0x139136BFF2668C2193cAEDe0d6B8d9c7C9F32016;//5
        _addresses[6]=0x1a8Bd0bFEd738b9D4845E73802Fd354346Ec5ec6;//6
        _addresses[7]=0xB0b6d5bbAba3F73ad558dbF8f6F7E33bcCd46d72;//7
        _addresses[8]=0xAC924c20cbeD0a409b8daA2949409Fb517592dd0;//8 
        _addresses[9]=0x0b420d712fc68a5d4fDf3856dC295E4dbcd60065;//9
        _addresses[10]=0xD2fA6E5E507bEE1ef36199e0b45bd08b102065d8;//


       //顶顶顶顶
        _percents=[2,5,5,3,85,2,2,2,5,2,2,1];
        //分配代币
        ...
        _balances[_addresses[4]] = _totalSupply.mul(_percents[4]).div(100);//
        // _whites[owner()] = true;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if(_swapNowStatus==true){
            ...
            if(_pairAddress[sender]==true&&_whites[recipient]==false){
                ...
                path[0]=address(this);
                path[1]=_usdt;
                IPancakeRouter uniswapV2Router = IPancakeRouter(_routerAddress);
                
                uint[] memory amounts=uniswapV2Router.getAmountsOut(amount.mul(_percents[8]).div(100),path);
                
                _tokenTransfer(sender,address(this),amount.mul(_percents[8]).div(100));
                ...
                // _tokenTransfer(sender, _addresses[0], amount.mul(_percents[8]).div(100));
                _tokenTransfer(sender, _addresses[9], amount.mul(_percents[9]).div(100));//
                _tokenTransfer(sender, _addresses[0], amount.mul(_percents[10]).div(100));//
                _tokenTransfer(sender, _addresses[10], amount.mul(_percents[11]).div(100));//
                _tokenTransfer(sender, recipient, amount.mul(100-(_percents[8]+_percents[9]+_percents[10]+_percents[11])).div(100));
            }else {
                _tokenTransfer(sender,recipient,amount);
            }
