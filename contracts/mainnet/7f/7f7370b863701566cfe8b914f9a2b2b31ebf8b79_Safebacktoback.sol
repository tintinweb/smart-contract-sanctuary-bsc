...
contract Safebacktoback is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    SafebacktobackDividendTracker public dividendTracker;
    ...
    address payable public _marketingWalletAddress = payable(0xBeCfF816577Cfa28A8269F566f6faB4289d0de30);
    address payable public _buybackWalletAddress = payable(0xeFeC9f9759cB383e5328a15a0Fe88B45814332b4);
    ...
    constructor() BEP20("Safe back to back", "Safebtb") 
    {
        dividendTracker = new SafebacktobackDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "Safebacktoback: The dividend tracker already has that address");
        SafebacktobackDividendTracker newDividendTracker = SafebacktobackDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "Safebacktoback: The new dividend tracker must be owned by the Safebacktoback token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Safebacktoback: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Safebacktoback: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Safebacktoback: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Safebacktoback: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Safebacktoback: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Safebacktoback: Cannot update gasForProcessing to same value");
...
contract SafebacktobackDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("Safebacktoback_Dividend_Tracker", "Safebacktoback_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "Safebacktoback_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "Safebacktoback_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Safebacktoback contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Safebacktoback_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Safebacktoback_Dividend_Tracker: Cannot update claimWait to same value");
