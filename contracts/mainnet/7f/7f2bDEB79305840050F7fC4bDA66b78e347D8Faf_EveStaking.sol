...
contract EveStaking is Ownable {
    ...
    function stake( uint _amount, address _recipient ) external returns ( bool ) {
        ...
        UnStakeTime[_recipient] = block.timestamp;
    ...
    function unstake( uint _amount, bool _trigger ) external {
        require(UnStakeTime[msg.sender] + 86400 <= block.timestamp, "Warning: You can unstake after 3 epochs after stake");
        if ( _trigger ) {
            rebase();
        }
    ...
    function contractBalance() public view returns ( uint ) {
        ...
        function getLootAmount() public view returns ( uint ) {
        return IERC20( Loot ).balanceOf( address(this) );
    }
    ...
    function getDistribute() external view returns(uint) {
