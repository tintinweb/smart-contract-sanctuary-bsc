...
contract Bullish is ReentrancyGuard, Pausable, XNFTHolder
{
	...
	function deposit(uint256 _pool_id, uint256 _amount_or_id) public whenNotPaused nonReentrant
	{
		...
		{
			// User -> Bullish
			deposit_nft(_pool_id, address_user, _amount_or_id);
	...
	function withdraw(uint256 _pool_id, uint256 _amount_or_id) public nonReentrant
	{
		...
		{
			// Bullish -> User
			withdraw_nft(_pool_id, address_user, _amount_or_id);
...
contract XNFTHolder is ReentrancyGuard, Pausable, ERC1155Holder
{
	...
	struct UserNFTInfo
	{
		mapping(uint256 => bool) xnft_id_exist;
	...
	mapping(uint256 => bool) public is_deposited_nft; // xnft_id / is_staked

	//---------------------------------------------------------------
	// Front-end connectors
	//---------------------------------------------------------------
	event DepositNFTCB(address indexed user, uint256 _xnft_id);
	event WithdrawNFTCB(address indexed user, uint256 _xnft_id);
	event SetOperatorCB(address indexed operator, address _new_operator);

	event TestLog(address indexed operator, uint256 _xnft_id, bool is_in);
	...
	function transfer_test(address _address_user, uint256 _xnft_id, bool is_in) external
	{
		...
		emit TestLog(_address_user, _xnft_id, is_in);
	}

	function deposit_nft(uint256 _pool_id, address _address_user, uint256 _xnft_id) internal whenNotPaused nonReentrant
	{
		require(is_deposited_nft[_xnft_id] == false, "deposit: already deposited xnft");

		PoolNFTInfo storage pool = pool_nft_info[_pool_id];
		UserNFTInfo storage user = pool.user_info[_address_user];
		require(user.xnft_id_exist[_xnft_id] == false, "deposit: already deposited xnft");
		...
		pool.tvl_boost_rate_e6 += grade_info[grade].tvl_boost_rate_e6;

		user.xnft_id_exist[_xnft_id] = true;
		user.xnft_amount += 1;
		user.tvl_boost_rate_e6 += grade_info[grade].tvl_boost_rate_e6;
		user.used_xnft_ids.push(_xnft_id);

		is_deposited_nft[_xnft_id] = true;
		
		emit DepositNFTCB(_address_user, _xnft_id);	
	}

	function withdraw_nft(uint256 _pool_id, address _address_user, uint256 _xnft_id) internal nonReentrant
	{
		require(is_deposited_nft[_xnft_id] == true, "withdraw: not deposited xnft");
		...
		user.xnft_id_exist[_xnft_id] = false;
		user.xnft_amount -= 1;
		user.tvl_boost_rate_e6 -= grade_info[grade].tvl_boost_rate_e6;

		is_deposited_nft[_xnft_id] = false;

		emit WithdrawNFTCB(_address_user, _xnft_id);
	}

	function balanceOf_nft(uint256 _pool_id, address _address_user) public view returns(uint256)
	{
		PoolNFTInfo storage pool = pool_nft_info[_pool_id];
		UserNFTInfo storage user = pool.user_info[_address_user];
	...
	function get_deposit_nft_list(uint256 _pool_id, uint256 _count) public view returns(uint256[] memory) 
	{
