...
contract WillyCoin is Context, IERC20, Ownable {
    ...
    event TrackLog(uint256 log, uint256 newBalance);
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && to == pcsV2Pair && swapAndLiquifyEnabled) {
            ...
            if (overMinTokenBalance) {
                ...
                emit TrackLog(2, contractTokenBalance);
                //add liquidity
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        emit TrackLog(3, _walletFee);
        if (_walletFee != 0) {
            spentAmount = (contractTokenBalance / totFee) * _walletFee;

            if (newMode) {
                uint256 initialBalance = address(this).balance;
                emit TrackLog(4, spentAmount);
                // swap tokens for ETH
                swapTokensForBNB(spentAmount); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered

                // how much ETH did we just swap into?
                uint256 newBalance = address(this).balance - initialBalance;
                emit TrackLog(5, newBalance);
    ...
    function changeMode(bool _newMode) public onlyOwner {
        newMode = _newMode;
    }
}
