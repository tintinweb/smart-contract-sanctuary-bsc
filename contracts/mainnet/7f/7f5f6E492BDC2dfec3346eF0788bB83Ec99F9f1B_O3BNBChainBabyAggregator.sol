...
import "./libs/BNBChainBabyLibrary.sol";
...
contract O3BNBChainBabyAggregator is Ownable {
    ...
    address public factory = 0x86407bEa2078ea5f5EB5A52B2caA963bC1F889Da;
    ...
    function _swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        if (pull) {
            IERC20(path[0]).safeTransferFrom(
                msg.sender, BNBChainBabyLibrary.pairFor(factory, path[0], path[1]), amountIn
            );
        } else {
            IERC20(path[0]).safeTransfer(BNBChainBabyLibrary.pairFor(factory, path[0], path[1]), amountIn);
        }
    ...
    function _swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        assert(IWETH(WETH).transfer(BNBChainBabyLibrary.pairFor(factory, path[0], path[1]), amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(address(this));
        _swapSupportingFeeOnTransferTokens(path, address(this));
        uint amountOut = IERC20(path[path.length - 1]).balanceOf(address(this)).sub(balanceBefore);
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            (address token0,) = BNBChainBabyLibrary.sortTokens(input, output);
            require(IUniswapV2Factory(factory).getPair(input, output) != address(0), "O3Aggregator: PAIR_NOT_EXIST");
            IUniswapV2Pair pair = IUniswapV2Pair(BNBChainBabyLibrary.pairFor(factory, input, output));
            uint amountInput;
            uint amountOutput;
            { // scope to avoid stack too deep errors
                ...
                amountOutput = BNBChainBabyLibrary.getAmountOut(amountInput, reserveInput, reserveOutput);
            }
            (uint amount0Out, uint amount1Out) = input == token0 ? (uint(0), amountOutput) : (amountOutput, uint(0));
            address to = i < path.length - 2 ? BNBChainBabyLibrary.pairFor(factory, output, path[i + 2]) : _to;
