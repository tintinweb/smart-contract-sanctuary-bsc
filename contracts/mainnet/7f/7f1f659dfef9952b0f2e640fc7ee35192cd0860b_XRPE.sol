...
The first BBR fork with 3% XRP-BEP20 rewards! 
💧TAX: 10/11
💧3% XRP Rewards
💧Burns on transactions

TG: https://t.me/RippleEffectBSC
...
contract XRPE is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    XRPEDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Ripple Effect", "XRPE") {
        marketingWallet = payable(0xF4E62C086b86079a99E2E978333c6c0aD4D2437C);
        devWallet = payable(0x4de0e2b9c2001d77C9e12493FEFEF62bA9D9dBb9);
        ...
        totalSellFees = sellRewardsFee
         ...
         dividendTracker = new XRPEDividendTracker(
            payable(this),
            router,
...
contract XRPEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        XRPEContract = XRPE(mainContract);
    ...
    function _transfer(
        ...
        require(false, "XRPE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "XRPE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main XRPE contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(XRPEContract);

            uint256 prevBalance = XRPEContract.balanceOf(address(this));
            ...
            {
                uint256 received = XRPEContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    XRPEContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
