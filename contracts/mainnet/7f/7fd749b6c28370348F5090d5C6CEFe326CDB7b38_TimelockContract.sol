...
contract TimelockContract is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(uint256 => Item) private lockedToken;
    ...
    modifier onlyMultiWallet() {
        require(msg.sender == multiSignWallet, "Timelock: Caller is invalid");
        _;
    }
    ...
    function setMultiSignWallet(address _multiSignWallet) public onlyMultiWallet {
        multiSignWallet = _multiSignWallet;
    }
    ...
    function deposit(
        ...
        uint256 _amount,
        uint256 _unlockTime
    ) external nonReentrant onlyOwner returns (uint256 _id) {
    ...
    function payoutToken(address _tokenAddr, address _beneficiary, uint256 _amount) external nonReentrant onlyMultiWallet {
        ...
        IERC20Upgradeable(_tokenAddr).safeTransfer(_beneficiary, _amount);
    }
    ...
    function getBytesWithdraw(
        ...
        return abi.encodeWithSignature("withdraw(uint256,uint256,address)",_id,_amount,_withdrawalAddress);
    }

    function getBytesSetMultiSignWallet(
        address _multiSignWallet
    ) public pure returns (bytes memory) {
        return abi.encodeWithSignature("setMultiSignWallet(address)",_multiSignWallet);
    }

    function getBytesPayoutToken(
        address _tokenAddr,
        address _beneficiary,
        uint256 _amount
    ) public pure returns (bytes memory) {
        return abi.encodeWithSignature("payoutToken(address,address,uint256)",_tokenAddr,_beneficiary,_amount);
    }
}
