 ...
 *
 */
contract BillNft is IVestingNFT, AccessControlEnumerableUpgradeable, ERC721EnumerableUpgradeable {
    ...
    function pendingPayout(uint256 tokenId) external view returns (uint256 payout) {
        payout = ICustomBill(billAddresses[tokenId]).billInfo(tokenId).payout;
    }
    ...
    function payoutToken(uint256 tokenId) external view returns (address tokenAddress) {
        tokenAddress = ICustomBill(billAddresses[tokenId]).payoutToken();
    }
}
