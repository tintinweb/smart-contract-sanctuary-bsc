...
contract JurassicStore is AccessControl, Pausable {
    ...
    uint private constant PRICE_LEGENDARY = 5 ether;
    ...
    JurassicCryptoTokens constant private NFT_CNT = JurassicCryptoTokens(0x566344f4015da9f5B3dF715B2948c97772815595);

    // The EOA that will receive the payments, set by the constructor
    address payable constant private OWNER = payable(0x9D8E4579A1e1B03233ed1A6f0Ba3901e2Dc7B14e);

    constructor() {  // solhint-disable-line func-visibility
    ...
    //      Getters      //
    //////////////////////
    function getJrscAmount(uint256 amount, uint256 price) public view returns(uint256) {
        ...
        //return CAKE_ROUTER_CNT.getAmountsIn(_price, path)[0] * amount;
    ...
    function purchaseEgg(address to, uint amount) external whenNotPaused() {
        ...
        require(
            JRSC_CNT.transferFrom(msg.sender, OWNER, getJrscAmount(amount, PRICE)),
    ...
    function purchaseLegendaryEgg(uint[] calldata burnedDinos) external whenNotPaused() {
        require(burnedDinos.length == 3, "EDinoNumber");
        Traits memory mp;
        bytes memory _data = hex"01";

        // The the burned tokens are dinos
        require(NFT_CNT.BatchIsTokenIdDino(burnedDinos), "ENotDino");

        // Sanity checks, the dinos should be of dino kind, not in sauria, and owned by caller
        address[] memory player = new address[](3);
        player[0] = msg.sender; player[1] = msg.sender; player[2] = msg.sender;
        // call the ERC1155 cnt to check owner
        uint256[] memory res = NFT_CNT.balanceOfBatch(player, burnedDinos);
        for (uint256 i = 0; i < 3; i++){
            require(res[i] == 1, "ERC1155NotOwned");
        }

        // Transfer the payment to jrsc main
        require(
            JRSC_CNT.transferFrom(msg.sender, OWNER, getJrscAmount(1, PRICE_LEGENDARY)), "ERC20NotApprovedOrBalance");

        // Burn the dinos
        uint256[] memory units = new uint256[](3);
        units[0] = 1; units[1] = 1; units[2] = 1;
        NFT_CNT.burnBatch(msg.sender, burnedDinos, units);

        // Call the NFT contract to mint the tokens
        NFT_CNT.batchMintDino(msg.sender, 1, mp, 0, _data);
    }
