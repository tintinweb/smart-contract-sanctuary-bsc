...
interface IERC20UpgradeableFrontRunDao is IERC20Upgradeable, ITransferLock {}

library SafeERC20UpgradeableFrontRunDao {
    using AddressUpgradeable for address;

    function safeTransfer(
        IERC20UpgradeableFrontRunDao token,
        address to,
        uint256 value
    ) internal {
        _callOptionalReturn(
...
contract FrontRunIDO is Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    using SafeERC20UpgradeableFrontRunDao for IERC20UpgradeableFrontRunDao;
    IERC20UpgradeableFrontRunDao public tokenFrontRunDao;
    ...
    function initialize(
        uint256 _startTimeIDO,
        uint256 _endTimeIDO,
        uint256 _totalAmountPerBUSD,
        address _busdContractAddress,
        address _tokenFrontRunDaoContract,
        address _priceFeedAddress
    ) public initializer {
        require(_startTimeIDO < _endTimeIDO, "invalid IDO time");
        require(_totalAmountPerBUSD > 0, "invalid rate buy IDO by BUSD");
        require(
            _busdContractAddress != address(0),
            "invalid contract address"
        );
        require(
            _tokenFrontRunDaoContract != address(0),
            "invalid contract address"
        );
        require(_priceFeedAddress != address(0), "invalid operator address");

        startTimeIDO = _startTimeIDO;
        endTimeIDO = _endTimeIDO;
        totalAmountPerBUSD = _totalAmountPerBUSD;

        tokenBUSD = IERC20Upgradeable(_busdContractAddress);
        tokenFrontRunDao = IERC20UpgradeableFrontRunDao(_tokenFrontRunDaoContract);
    ...
    function buyByBUSD(uint256 amount) external {
        ...
        tokenFrontRunDao.transferLockToken(sender, buyAmountToken);
    ...
    function claimToken() external onlyAdmin {
        address sender = _msgSender();
        uint256 remainAmountToken = tokenFrontRunDao.balanceOf(address(this));
        tokenFrontRunDao.safeTransfer(sender, remainAmountToken);
    }

    function setContractToken(address contractToken) external onlyOwner {
        tokenFrontRunDao = IERC20UpgradeableFrontRunDao(contractToken);
    }
    ...
    function setRoundInfo(
        ...
        require(_startTimeIDO < _endTimeIDO, "invalid time");
    ...
    function _precheckBuy(uint256 amount) internal view {
        ...
        require(block.timestamp >= startTimeIDO, "ICO time dose not start now");
        require(block.timestamp <= endTimeIDO, "ICO time is expired");
    }
