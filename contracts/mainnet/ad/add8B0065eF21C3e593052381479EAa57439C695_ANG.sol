...
contract ANG is IERC20,Ownable {
    ...
    address public walletAngel = 0xECA9feAa7324DF2B54931d95B75e953E8E394eD4;
    address public walletUnion = 0x5962676fD65F166E8ED41DB4EbBE69cfb43b3eF3;
    address public walletPoolAngel = 0xDE9D0B8980Fd815b3b4C0253eb0730eFf4EB03f5;
    ...
    uint256 public dayseconds = 60*10;//86400
    uint256[] public dailyRates = new uint256[](16);
    mapping(address => uint256) public userKeepDay;//days of user holding tokens without selling or transfer
    ...
    //angtool test 0x69Cd0FD5059DA77FcB1d4f8158C5E984cc13Ea5E main 0xb5c67Fe67A9c1B85425Ec4258b9B8962728F588A
    ...
    function profiting(address user)internal returns (uint256){
        ...
        if(userDailyProfit[user] < today && getRate(user) > 0){
            uint256 daydiff = today - userDailyProfit[user];

            profit = (_balances[user]*(getRate(user)**daydiff)).div(1000**daydiff) - _balances[user];
        ...
        if(userKeepStartTime[user] > 0){
            keepdays = (block.timestamp-userKeepStartTime[user]).div(dayseconds);
        }else{
            userKeepStartTime[user] = block.timestamp;
        }
        userKeepDay[user] = keepdays;
        return profit;
    }
    ...
    function getRate(address user)public view returns (uint256){
        if(userKeepDay[user]>=15){
            return dailyRates[15];//11=0.
        }
        return dailyRates[userKeepDay[user]];
    }
    ...
    function takeAllFee(address from, address recipient, uint256 amount) private returns(uint256 amountAfter) {
        ...
        if(from == uniswapV2Pair){
            ...
            uint256 feeBuy = amount.mul(29).div(200);//14.5%
            amountAfter = amountAfter.sub(feeBuy);

            if(angtool.parent1(recipient) != address(0)){
                feeBuy = feeBuy.sub(amount.mul(4).div(100));//4%
                doTransfer(from, angtool.parent1(recipient), amount.mul(4).div(100));
                if(angtool.parent2(recipient) != address(0)){
                    feeBuy = feeBuy.sub(amount.mul(5).div(200));//2.5%
                    doTransfer(from, angtool.parent2(recipient), amount.mul(5).div(200));
                    if(angtool.parent3(recipient) != address(0)){
                        feeBuy = feeBuy.sub(amount.div(200));//0.5%
                        doTransfer(from, angtool.parent3(recipient), amount.div(200));
                        if(angtool.parent4(recipient) != address(0)){
                            feeBuy = feeBuy.sub(amount.div(200));//0.5%
                            doTransfer(from, angtool.parent4(recipient), amount.div(200));
                            if(angtool.parent5(recipient) != address(0)){
                                feeBuy = feeBuy.sub(amount.div(200));//0.5%
                                doTransfer(from, angtool.parent5(recipient), amount.div(200));
                                if(angtool.parent6(recipient) != address(0)){
                                    feeBuy = feeBuy.sub(amount.div(200));//0.5%
                                    doTransfer(from, angtool.parent6(recipient), amount.div(200));
                                    if(angtool.parent7(recipient) != address(0)){
                                        feeBuy = feeBuy.sub(amount.div(200));//0.5%
                                        doTransfer(from, angtool.parent7(recipient), amount.div(200));
                                        if(angtool.parent8(recipient) != address(0)){
                                            feeBuy = feeBuy.sub(amount.div(200));//0.5%
                                            doTransfer(from, angtool.parent8(recipient), amount.div(200));
                                            if(angtool.parent9(recipient) != address(0)){
                                                feeBuy = feeBuy.sub(amount.div(200));//0.5%
                                                doTransfer(from, angtool.parent9(recipient), amount.div(200));
                                                if(angtool.parent10(recipient) != address(0)){
                                                    feeBuy = feeBuy.sub(amount.div(200));//0.5%
                                                    doTransfer(from, angtool.parent10(recipient), amount.div(200));
                                                    if(angtool.parent11(recipient) != address(0)){
                                                        feeBuy = feeBuy.sub(amount.div(200));//0.5%
                                                        doTransfer(from, angtool.parent11(recipient), amount.div(200));
                                                        if(angtool.parent12(recipient) != address(0)){
                                                            feeBuy = feeBuy.sub(amount.div(200));//0.5%
                                                            doTransfer(from, angtool.parent12(recipient), amount.div(200));
                                                            if(angtool.parent13(recipient) != address(0)){
                                                                feeBuy = feeBuy.sub(amount.div(200));//0.5%
                                                                doTransfer(from, angtool.parent13(recipient), amount.div(200));
                                                                if(angtool.parent14(recipient) != address(0)){
                                                                    feeBuy = feeBuy.sub(amount.div(200));//0.5%
                                                                    doTransfer(from, angtool.parent14(recipient), amount.div(200));
                                                                    if(angtool.parent15(recipient) != address(0)){
                                                                        feeBuy = feeBuy.sub(amount.div(200));//0.5%
                                                                        doTransfer(from, angtool.parent15(recipient), amount.div(200));
                                                                    }
                                                                }
                                                            }
                                                        }
