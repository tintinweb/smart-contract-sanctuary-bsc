...
contract tsc is ERC20Detailed, Ownable {
    ...
    uint256 public buyAutoBurn = 10;
    uint256 public buyRewardsFee = 30;
    uint256 public buyDevelopmentFee = 20;

    uint256 public sellLiquidityFee = 30;
    uint256 public sellMarketingFee = 50;
    uint256 public sellAutoBurn = 20;
    uint256 public sellRewardsFee = 40;
    uint256 public sellDevelopmentFee = 20;

    uint256 public AmountLiquidityFee;
    uint256 public AmountMarketingFee;
    uint256 public AmountAutoBurnFee;
    uint256 public AmountRewardsFee;
    uint256 public AmountDeveloperFee;

    uint256 public feeDenominator = 1000;

    address public _marketingWalletAddress = 0xB7DA13D9Ad08E31BAd6DB65A997D5cd2e5f4c468;
    address public _developerWalletAddress = 0xB7DA13D9Ad08E31BAd6DB65A997D5cd2e5f4c468;
    ...
    uint256 public txfee = 100;
    bool public feesOnNormalTransfers = true;
    uint256 public TwentyFourhours = 86400;   //24 hour

    DividendDistributor distributor;
    address public StableDividendReceiver;
    ...
    constructor() ERC20Detailed("Test stable", "ts", uint8(DECIMALS)) Ownable() {
        ...
        StableDividendReceiver = address(distributor);
    ...
    function _transferFrom(
        ...
        bool excludedAccount = _isExcludedFromFees[sender] || _isExcludedFromFees[recipient];
        ...
        if (
            ...
            !excludedAccount &&
            !isSellLimitExempt[sender]
        ) {

            uint blkTime = block.timestamp;
          
            uint256 onePercent = balanceOf(sender).mul(txfee).div(1000); //Should use variable
            require(amount <= onePercent, "ERR: Can't sell more than 25%");
            ...
            if( blkTime > tradeData[sender].lastTradeTime + TwentyFourhours) {
                ...
                require(tradeData[sender].tradeAmount + amount <= onePercent, "ERR: Can't sell more than 25% in One day");
    ...
    function takeFee(
        ...
        uint256 totalTxFee = 250;   //Transfer Fee Always 25%
        uint256 feeAmount;
        uint LFEE;
        uint MFEE;
        uint ABFEE;
        ...
        if(automatedMarketMakerPairs[sender]){
            ...
            ABFEE = amount.mul(buyAutoBurn).div(feeDenominator);
            AmountAutoBurnFee += ABFEE;
            RFEE = amount.mul(buyRewardsFee).div(feeDenominator);
            AmountRewardsFee += RFEE;
            DFEE = amount.mul(buyDevelopmentFee).div(feeDenominator);
            AmountDeveloperFee += DFEE;

            feeAmount = LFEE.add(MFEE).add(ABFEE).add(RFEE).add(DFEE);
        }
            ...
            AmountMarketingFee += MFEE;
            ABFEE = amount.mul(sellAutoBurn).div(feeDenominator);
            AmountAutoBurnFee += ABFEE;
            RFEE = amount.mul(sellRewardsFee).div(feeDenominator);
            AmountRewardsFee += RFEE;
            DFEE = amount.mul(sellDevelopmentFee).div(feeDenominator);
            AmountDeveloperFee += DFEE;
            ...
            feeAmount = amount.mul(totalTxFee).div(feeDenominator);
        }
    ...
    function manualSwap() public onlyOwner swapping { 
        ...
        if(AmountDeveloperFee > 0) swapForDev(AmountDeveloperFee);
        if(AmountMarketingFee > 0) swapForMarketing(AmountMarketingFee);
        if(AmountAutoBurnFee > 0) swaptoBurn(AmountAutoBurnFee);
    ...
    function addLiquidity() internal swapping {
        ...
        if(AmountAutoBurnFee > 0){
            swaptoBurn(AmountAutoBurnFee);
        }
    ...
    function setBuyFee(
        ...
        uint subtotal = _newLp.add(_newMarketing).add(_newBurn).add(_newReward).add(_newDev);
        require(subtotal <= MAX_TAX_FEE,"Error: Tax must be set within 25%");

        buyLiquidityFee = _newLp;
        buyMarketingFee = _newMarketing;
        buyAutoBurn = _newBurn;
        buyRewardsFee = _newReward;
        buyDevelopmentFee = _newDev;
    }
    ...
    function setSellFee(
        ...
        sellAutoBurn = _newBurn;
        sellRewardsFee = _newReward;
        sellDevelopmentFee = _newDev;
    }
    ...
    function setFeeReceivers(address _marketing, address _developer) public onlyOwner {
    ...
    function setSellLimitExempt(address _addr,bool _status) external onlyOwner {
        require(isSellLimitExempt[_addr] != _status, "Error: Not changed");
        isSellLimitExempt[_addr] = _status;
    }
    ...
    function swaptoBurn(uint _tokens) private {
        _balances[address(this)] = _balances[address(this)].sub(_tokens);
        _balances[deadWallet] = _balances[deadWallet].add(_tokens);
        AmountAutoBurnFee = AmountAutoBurnFee.sub(_tokens);
        emit Transfer(address(this),deadWallet,_tokens);
    }

    function swapForDev(uint _tokens) private {
