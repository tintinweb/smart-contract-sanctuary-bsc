...
contract Distributor is Ownable{
    ...
    function distribute(address token, uint256 amount) external onlyOwner{
        uint256 sum = 0;
        for(uint256 i = 0; i < _holders.length; i++){
            Holder memory holder = _holders[i];
            uint256 part = amount * holder.rate / base;
            IERC20(token).transfer(holder.account, part);
            sum += part;
        }
        distributed[token] += sum;
        emit Distribute(token, sum);
    }
