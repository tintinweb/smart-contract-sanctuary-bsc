...
contract PancakeFlashSwap {
    ...
    // Allow public view balance of the contract
    function getBalanceOfToken(address _address) public view returns (uint256) {
        return IERC20(_address).balanceOf(address(this));
    }

    // PLACE A TRADE
    // Execute placing trade
    function placeTrade(
        address _fromToken,
        address _toToken,
        uint256 _amountIn
        ...
        // Calculate amount out Arbitrage - Swap for antoher token
        ...
        require(amountReceived > 0, "Aborted Tx: Trade return zero");
    ...
    // CHECK PROFITABILITY
    // checks if output is greater then input
    ...
    // Initiate Arbitrage
    // Begins receiving Loan to engage performing arbitrage trade
    function startArbitrage(address _tokenBorrow, uint256 _amount) external {
        ...
        // Figure out what token (0 or 1) has the amount and assign
        ...
        // Passing Data as bytes so the swap function knows it is flashswap
        bytes memory data = abi.encode(_tokenBorrow, _amount, msg.sender);

        // Execute the initial swap to get loan
