...
// IFSwapxToken.
contract IFWV2 is BEP20, MinterRole {
    ...
    uint256 public buybackSellFee = 30;
    uint256 public marketingSellFee = 20;
    uint256 public burnSellFee = 30;
    address public marketingFeeReceiver = 0xBD57b06349DDe6Bc296bE8f1639690E25A8D1FC5;
    ...
    address public buyBackToken = address(0x947E5203ce6c9B25dA67338Ba79ADE052EC52CD0); //buyBackToken
    ...
    event XBLOXSwapRouterUpdated(address indexed operator, address indexed router, address indexed pair);
    ...
    constructor() BEP20('IFSWAP V2', 'IFWV2') {
        ...
        if (tradingOpen || sender == owner()) {
            if (
                ...
                if (transferTaxRate == 0 || _excludedFromFees[sender] || _excludedFromFees[recipient]) {
                    ...
                    require(amount == taxAmount + sendAmount, "XBX:: transfer: Tax value invalid");
    ...
    function setExcludeFromFees(address _account, bool _excluded) public onlyOwner {
        require(_excludedFromFees[_account] != _excluded, "XBX: Account is already the value of 'excluded'");
    ...
    function updateXBLOXSwapRouter(address _router) public onlyOwner {
        ...
        require(hashSwapPair != address(0), "XBX::updateXBLOXSwapRouter: Invalid pair address.");
        emit XBLOXSwapRouterUpdated(_router, address(hashSwapRouter), hashSwapPair);  
    }

    function setTxFees(uint256 _transactionFee) external onlyOwner{
        transferTaxRate = _transactionFee;
        require(transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "XBX: Total fees can not be bigger than max value");
    }
    ...
    function setSellFees(uint256 _liquiditySellFee, uint256 _buybackSellFee, uint256 _marketingSellFee, uint256 _burnSellFee) external onlyOwner{
        ...
        require(sellTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "XBX: Total fees can not be bigger than max value");
    }
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "XBX::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "XBX::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "XBX::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "XBX::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying XBXs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "XBX::_writeCheckpoint: block number exceeds 32 bits");
