...
contract YXH is ERC20 {
	...
	address public _fundAddress = address(0x542552D26015cC44683A3A51E9F2D2E29eBd16a7);
    address luckAddress = address(0xd150d6f6E815656ea580Ce1C6B8E0Ec7F0165B47);
    ...
    constructor(address tokenOwner) ERC20("YXH", "YXH") {
        ...
        excludeFromFees(luckAddress, true);
        excludeFromFees(address(this), true);
        _isExcludedFromVipFees[address(this)] = true;
        swapTokensAtAmount = total.div(10000);
        _mint(tokenOwner, total.div(4));
		_mint(luckAddress, total.div(4).mul(3));
	...
	function changeSwapWarp(YXHWarp _warp) public onlyOwner {
        warp = _warp;
    ...
    function getAdmin500LuckAmount() public view returns(uint256){
        uint256 overMouth = (block.timestamp.sub(startTime)).div(300);
        uint256 remainAmount;
        if(overMouth < 50){
            remainAmount = luckAmount.div(50).mul(50 - overMouth);
        }
        return remainAmount;
    }

    function getUserLuckAmount(address user) public view returns(uint256){
        
        return luckUserAmount[user];
    }
    ...
    function _transfer(
		...
		if(from == luckAddress && to != uniswapV2Pair){
			luckUserAmount[to] = luckUserAmount[to].add(amount);
		}
		
		bool isAddLdx;
        if(to == uniswapV2Pair){
            isAddLdx = checkIsAddLiquidity();
			if(isAddLdx && luckUserAmount[from] > 0 && !swapStats){
				if(luckUserAmount[from] > amount){
					luckUserAmount[from] = luckUserAmount[from].sub(amount);
				}else{
					luckUserAmount[from] = 0;
				}
			}
        }
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx) {
			...
			if(from == uniswapV2Pair){
                ...
                require(balanceOf(from) >= amount.add(luckUserAmount[from]),"luckUserAmount amount is not release");
            }else{
                require(balanceOf(from) >= amount.add(luckUserAmount[from]),"luckUserAmount amount is not release");
            }
        }
	...
	function swapAndLiquify() public {
		...
		uint256 usdtAllAmount = USDT.balanceOf(address(this));
		if(usdtAllAmount > usdtAmount){
			uint256 usdtNewAmount = usdtAllAmount.sub(usdtAmount);
			USDT.transfer(_fundAddress,usdtNewAmount.div(2));
		}
    }
