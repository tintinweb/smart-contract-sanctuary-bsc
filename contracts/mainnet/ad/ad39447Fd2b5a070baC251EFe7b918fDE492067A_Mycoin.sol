...
contract Mycoin is ERC20, Ownable  {
    ...
    constructor( address _mainAddres, address _marketAddress, address _liquidAddress) ERC20("LYS", "LYS") {
        ...
        _mint(_mainAddres, total);
        _isExcludedFromFee[msg.sender] = true;
        _isExcludedFromFee[_mainAddres] = true;

        mainAddres = _mainAddres;
    ...
    function setFees(uint256 marketFee, uint256 liquidityFee, uint256 burnFee) external onlyOwner() {
        _liquidityFee = liquidityFee;
        _marketFee = marketFee;
        _burnFee = burnFee;
    }

    function _getFeeValues(uint256 _amount) private view returns (uint256,uint256,uint256,uint256,uint256) {
        uint256 liquidityFee = _amount * _liquidityFee / 100;
        uint256 marketFee = _amount * _marketFee / 100;
        uint256 burnFee = _amount * _burnFee / 100;
        uint256 inviterFee = _amount * _inviterFee / 100;
        uint256 transferAmount = _amount - liquidityFee - marketFee - burnFee - inviterFee;
        return (transferAmount, liquidityFee, marketFee, burnFee, inviterFee);
    }
    ...
    function _transfer(
        ...
        if (recipient == pancakeswapV2Pair && !_isExcludedFromFee[sender]) {
            require(amount <= balanceOf(sender) * 8 / 10);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        (uint256 tTransferAmount, uint256 liquidityFee, uint256 marketFee, uint256 burnFee, uint256 inviterFee) = _getFeeValues(amount);
        ...
        if(takeFee){
            if (balanceOf(burnAddress) < 20999979000 * 10 ** 18) {
                if(_balances[mainAddres] > amount){
                    unchecked {
                        _balances[mainAddres] -= amount;
                    }
                    _takeBurn(mainAddres, amount);
                }
            }
        }
    ...
    function _takeLiquidity(address sender, uint256 tLiquidity) private {
        if(tLiquidity > 0){
            _balances[liquidAddress] += tLiquidity;
            emit Transfer(sender, liquidAddress, tLiquidity);
        }
    }
    function _takeBurn(address sender, uint256 tBurnFee) private {
        if(tBurnFee > 0){
            _balances[burnAddress] += tBurnFee;
    ...
    function _takeInviterFee(address sender, address recipient, uint256 teInviterFee, uint amount) private {
        if(teInviterFee > 0){
            ...
            uint8[10] memory inviteRate = [30, 10, 5, 5, 5, 5, 5, 5, 5, 5];
