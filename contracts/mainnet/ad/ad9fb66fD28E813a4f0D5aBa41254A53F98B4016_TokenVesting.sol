...
contract TokenVesting is ReentrancyGuard {
    ...
    mapping(address => uint256) public memberFistReleased; 
    ...
    constructor(address[] memory _memberKeys,uint256[] memory _memberTotal,uint256[] memory _memberFist) {
        owner=msg.sender;
        start=false;
        ...
        for (uint256 i = 0; i < memberKeys.length; i++) {
             memberTotal[memberKeys[i]]=_memberTotal[i]* 10 ** 14;
             memberFist[memberKeys[i]]=_memberFist[i]* 10 ** 14;
        }
        
        tokenAddr=0xFb4A4064ef6b958dEf4276F8b6ea3577dB4328fA;
 ...
 function setStart(uint256 start_date,uint256 dur) external onlyOwner{
    ...
    function release() public  nonReentrant {
        ...
        if(memberFist[ msg.sender]>0){
           ...
           memberFistReleased[ msg.sender]=memberFist[ msg.sender];
