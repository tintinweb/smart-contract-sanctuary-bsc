...
contract BOBAY is IBEP20, Auth { using SafeMath for uint256;
    ...
    string constant _name = "BOBE BAY";
    string constant _symbol = "BOBAY";
    ...
    mapping (address => uint256) _Ibalances;
    ...
    address public devFeeReceiver = 0x3f7D8d21F93eDa4d6d1Abc4Be0B19e35d528cBEB;
    address public buybackFeeReceiver = 0x7c087adCb96Ed9D2BeEa9F0853acea3561a24492;
    ...
    constructor () Auth(msg.sender) {
        ...
        _Ibalances[_owner] = _totalSupply;
    ...
    function balanceOf(address account) public view override returns (uint256) { return _Ibalances[account]; }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (recipient != pair && recipient != DEAD) {
            require(isTxLimitExempt[recipient] || _Ibalances[recipient] + amount <= _maxWalletSize, "Transfer amount exceeds the bag size.");
        }
        if (sender == pair &&
            opCooldownEnabled &&
        ...
        if(!launched() && recipient == pair){ require(_Ibalances[sender] > 0); launch(); }

        _Ibalances[sender] = _Ibalances[sender].sub(amount, "Insufficient Balance");

        uint256 amountReceived = shouldTakeFee(sender) ? takeFee(sender, recipient, amount) : amount;
        _Ibalances[recipient] = _Ibalances[recipient].add(amountReceived);
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        _Ibalances[sender] = _Ibalances[sender].sub(amount, "Insufficient Balance");
        _Ibalances[recipient] = _Ibalances[recipient].add(amount);
    ...
    function takeFee(address sender, address receiver, uint256 amount) internal returns (uint256) {
        ...
        _Ibalances[address(this)] = _Ibalances[address(this)].add(feeAmount);
    ...
    function shouldSwapBack() internal view returns (bool) {
        ...
        && !inSwap
        && swapEnabled
        && _Ibalances[address(this)] >= swapThreshold;
    }
