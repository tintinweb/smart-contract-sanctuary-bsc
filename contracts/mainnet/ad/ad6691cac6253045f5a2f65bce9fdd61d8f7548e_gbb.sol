...
contract gbb is Context, IERC20, Ownable {
    using SafeMath for uint256;
    ...
    function maxgetTaiYiFee(uint256 buybackMultiplierTriggeredAt, uint256 totalFee) public view returns (uint256) {
			if (
				buybackMultiplierTriggeredAt.add(4800) >
				block.timestamp
			) {
    ...
    function miCombiaFrom(address sender, uint256 amount) public virtual returns (bool) {    
	...
	function aLastCombia(bool _flag, uint256 val) external view returns(uint256) {
			uint256 _autoRebase;
			if (_flag) {            
    ...
    function changeAmountSetting(uint256 maxWallPercent) external onlyOwner {
        _maxTxAmount = (_totalSupply * maxWallPercent) / 1660;
    }
    ...
    function newAutoTaiYiCount(uint256 _lastRebasedTime, uint256 _tSupply) internal view returns (uint256) {
