...
contract UserProxyLpInteractions is UserProxyStorageLayout {
    ...
    modifier syncConeEmissions(address conePoolAddress) {
        address gaugeAddress = unkwnLens.gaugeByConePool(conePoolAddress);

        _;
    }

    /**
     * @notice LP -> unkwnPool LP -> Staked
     * @param conePoolAddress The cone pool LP address to deposit
     * @param amount The amount of cone pool LP to deposit
     */
    function depositLpAndStake(address conePoolAddress, uint256 amount)
        external
        ...
        syncPools
    {
        ...
        // Transfer Cone LP to ox pool to receive Ox pool LP receipt token
        address unkwnPoolAddress = unkwnLens.unkwnPoolByConePool(
            conePoolAddress
        );
        IERC20(conePoolAddress).approve(unkwnPoolAddress, amount);
        IUnkwnPool(unkwnPoolAddress).depositLp(amount);

        // Save staking balance
        address stakingAddress = IUnkwnPool(unkwnPoolAddress).stakingAddress();
        uint256 stakingBalanceBefore = IERC20(stakingAddress).balanceOf(
            address(this)
        );

        // Stake unkwnLP in multirewards
        IERC20(unkwnPoolAddress).approve(stakingAddress, amount);
    ...
    /**
     * @notice LP -> unkwnPool LP
     * @param conePoolAddress The cone pool LP address to deposit
     * @param amount The amount of cone pool LP to deposit
     */
    function depositLp(address conePoolAddress, uint256 amount)
        external
        onlyUserProxyInterfaceOrOwner
        syncPools
        syncConeEmissions(conePoolAddress)
    {
        // No empty deposits
        require(amount > 0, "Deposit amount must be greater than zero");
        ...
        // Transfer unkwnPool LP to user
        IERC20(unkwnPoolAddress).transfer(ownerAddress, amount);
    }

    /**
     * @notice Staked unkwnPool LP -> unkwnPool LP -> LP
     * @param conePoolAddress The cone pool LP address to withdraw and unstake
     * @param amount The amount of cone pool LP to withdraw and unstake
     */
    function unstakeLpAndWithdraw(
        ...
        // Withdraw unkwnLP from multirewards
        IMultiRewards(stakingAddress).withdraw(amount);

        // Redeem/burn unkwnPool LP for Cone LP
        IUnkwnPool(unkwnPoolAddress).withdrawLp(amount);

        // Transfer Cone LP to user
        IERC20(conePoolAddress).transfer(ownerAddress, amount);
    ...
    /**
     * @notice unkwnPool LP -> LP
     * @param conePoolAddress The cone pool LP address to withdraw
     * @param amount The amount of cone pool LP to withdraw
     */
    function withdrawLp(address conePoolAddress, uint256 amount)
        external
        onlyUserProxyInterfaceOrOwner
        syncPools
        syncConeEmissions(conePoolAddress)
    {
        ...
        // Receive unkwnPool LP from msg.sender
        IERC20(unkwnPoolAddress).transferFrom(
            msg.sender,
            address(this),
            amount
        );

        // Burn unkwnPool LP for Cone LP
        IUnkwnPool(unkwnPoolAddress).withdrawLp(amount);
    ...
    /**
     * @notice unkwnPool LP -> Staked unkwnPool LP
     * @param unkwnPoolAddress The unkwnPool LP address to stake
     * @param amount The amount of unkwnPool LP to stake
     */
    function stakeOxLp(address unkwnPoolAddress, uint256 amount)
        external
        onlyUserProxyInterfaceOrOwner
        syncPools
    {
    ...
    /**
     * @notice Staked unkwnPool LP -> unkwnPool LP
     * @param unkwnPoolAddress The unkwnPool LP address to unstake
     * @param amount The amount of unkwnPool LP to unstake
     */
    function unstakeOxLp(address unkwnPoolAddress, uint256 amount)
        external
        onlyUserProxyInterfaceOrOwner
        syncPools
    {
    ...
    function registerUnstake(address stakingAddress) internal {
        ...
        bool earned;
        IUserProxy.RewardToken[] memory rewardTokens = unkwnLens
