...
contract MOMINDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("MOMIN_Dividend_Tracker", "MOMIN_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**9); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "MOMIN_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "MOMIN_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MOMIN contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "MOMIN_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "MOMIN_Dividend_Tracker: Cannot update claimWait to same value");
...
contract MOMIN is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    MOMINDividendTracker public dividendTracker;
    ...
    uint256 public ZakatFee;
    address payable public  ZakatWallet;
    ...
    function setFee(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _ZakatFee) public onlyOwner {
        BNBRewardsFee = _bnbRewardFee;
        liquidityFee = _liquidityFee;
        ZakatFee = _ZakatFee;

        totalFees = BNBRewardsFee.add(liquidityFee).add(ZakatFee); // total fee transfer and buy
    ...
    function setZakatWallet(address payable _newZakatWallet) public onlyOwner {
        ZakatWallet = _newZakatWallet;
    }
/**

Token Name : Momin World Token
Symbol : MOMIN

1. Zakat Wallet: 3% in BNBs 
2. Reflection: 3% in BNBs
3. Auto liquidity: 3%
1 quad = 1000, 000, 000, 000, 000
*/
    constructor() ERC20("Momin World Coin", "MOMIN") {
        BNBRewardsFee = 3;
        liquidityFee = 3;
        extraFeeOnSell = 0; // extra fee on sell
        ZakatFee = 3;
        ZakatWallet = payable(0xC2209bC8fF831A2731Cd5b2B35C1b47A79ed9ffc);

        totalFees = BNBRewardsFee.add(liquidityFee).add(ZakatFee); // total fee transfer and buy

    	dividendTracker = new MOMINDividendTracker();
        ...
        excludeFromFees(ZakatWallet, true);
        excludeFromFees(address(this), true);
        
        // exclude from max tx
        _isExcludedFromMaxTx[owner()] = true;
        _isExcludedFromMaxTx[address(this)] = true;
        _isExcludedFromMaxTx[ZakatWallet] = true;
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "MOMIN: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MOMIN: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "MOMIN: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MOMIN: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MOMIN: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MOMIN: Cannot update gasForProcessing to same value");
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // worthy Zakat fee
        uint256 ZakatAmount = deltaBalance.sub(bnbToAddLiquidityWith).div(totalFees.sub(liquidityFee)).mul(ZakatFee);
        ZakatWallet.transfer(ZakatAmount);
    ...
    function swapAndSendBNBToZakat(uint256 tokenAmount) private {
        swapTokensForBnb(tokenAmount, ZakatWallet);
    }
