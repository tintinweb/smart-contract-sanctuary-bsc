...
contract Deem is Context, IERC20, Ownable {
    ...
    string private _name = "Deem";
    string private _symbol = "DM";
    uint8 private _decimals = 9;

    address payable public marketingWallet = payable(0xe9F20D537336790ea83f6dCd54C227F71E5Ee7B7);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 liquidityTokenShare = contractTokenBalance.mul(_saleLiquidityFee).div(_saleLiquidityFee.add(_saleMarketingFee));
        uint256 marketingTokenShare = contractTokenBalance.sub(liquidityTokenShare);
        ...
        swapTokensForEth(marketingTokenShare, marketingWallet);
    ...
    {
        if(_buyMarketingFee==0) {  return(tTransferAmount, rTransferAmount); }
        uint256 tMarketing = tAmount.div(100).mul(_buyMarketingFee);
    ...
    external onlyOwner() {
        require(taxFee.add(liquidityFee.add(marketingFee)) <= 10, "total fee must be less than or equal to 10");
        _saleTaxFee = taxFee;
    ...
    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        require(maxTxAmount > totalSupply().div(100), "transaction amount must be greater than 1% of total supply");
