...
// Modified by aknashu Date 01-12-2022
   ...
   if(businessLogs[msg.sender][_index].isclaim == true && businessLogs[msg.sender][_index].ispaid == false){
    require(businessLogs[msg.sender][_index].received>0,"you don't have funds");
        uint nextcycledate;
        if(flushBusiness[msg.sender].cycle == 1){
            nextcycledate = flushCycleDays[0];
        }else if(flushBusiness[msg.sender].cycle == 2){
...
// Modified by aknashu Date 01-12-2022
    ...
    function businessLogChange(address _user,uint _index) public {
        require(msg.sender == supportWallet, "permission Diened");
         businessLogs[_user][_index].ispaid = true;
    }
    ...
    function changePlan() public {
        ...
        plan = [100,1000,2500,100000];
        Group_Rewards_Program = [1000*1e8,10000*1e8,50000*1e8,100000*1e8,200000*1e8,500000*1e8];
    }
