...
contract TEP is Ownable, IERC20 {
    ...
    string constant private _name = "tELONphone";
    string constant private _symbol = "TEP";
    ...
    constructor() {
        ...
        lpWallet = msg.sender;
        projectAddress = 0xaa9bD1387c7E6b46F08c52153512B3f62b4b70e2;
        ...
        isWhitelisted[projectAddress] = true;
        isWhitelisted[devWallet] = true;

        totalFee = liquidityFee.add(marketingFee).add(tokenFee).add(betFee);
    ...
    function marketingAndLiquidity() internal lockTheSwap {
        ...
        uint256 amountETHDev = amountETH.mul(betFee).div(totalETHFee);
    ...
    function addPermissions(address target) public onlyOwner{
    ...
    function changeFees(uint256 newLiqFee, uint256 newMarketingFee, uint256 newBetFee, uint256 newNativeFee, uint256 extraSellFee) external onlyOwner {
        ...
        totalFee = liquidityFee.add(marketingFee).add(betFee).add(tokenFee);
    ...
    function disableAllBlacklist() public onlyOwner{
        blacklistMode = false;
    }
