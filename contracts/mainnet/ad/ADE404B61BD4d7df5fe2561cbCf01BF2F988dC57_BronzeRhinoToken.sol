...
import "./BronzeRhinoTokenStorage.sol";
import {List} from  "../struct/RhinoTokenStructs.sol";

contract BronzeRhinoToken is ERC721, ERC721Enumerable, ReentrancyGuard, Ownable, BronzeRhinoTokenStorage {
    ...
    constructor(string memory baseURI_, address payee_) ERC721("BronzeRhinoToken","BRT") {
        baseURI = baseURI_;
        payee = payee_;
    }
    ...
    function withdraw(uint256 tokenId) external nonReentrant returns(uint _mintTokenId){
        ...
        for (uint i; i < list.length; i++) {
            if(list[i] == tokenId){
                ...
                firstHolder[tokenId] = _recipient;
        ...
        for(uint i; i < tokenId.length; i++){
            ...
            for(uint j; j < list.length; j++){
                if(list[j] == tokenId[i]){
                    ...
                    firstHolder[tokenId[i]] = _recipient;
    ...
    function synthesisMint(address _recipient) external onlyMerge returns(address, uint256){
        require(_recipient != address(0), "ERC721: mint to the zero address");
        ...
        tokenIdRecord[newTokenId] = true;
        firstHolder[newTokenId] = _recipient;
        emit SynthesisMint(_recipient, newTokenId);
        return (_recipient, newTokenId);
    }

    function synthesis(address _owner, uint256[] memory _tokenIds) external onlyMerge returns(bool){
        require(_tokenIds.length == 5, "ERC721: Not enough nft cards");

        for(uint i; i < _tokenIds.length; i++){
            address owner = ownerOf(_tokenIds[i]);
            require(_owner == owner, "ERC721: NFT card owner error");

            bool marker = tokenIdMarker[_tokenIds[i]];
            require(marker == false, "ERC721: The NFT card has been synthesized and used");

            tokenIdMarker[_tokenIds[i]] = true;
        }
        return true;
    }
...
contract BronzeRhinoTokenStorage {

    uint256 internal constant MAX_SUPPLY = 33000;

    mapping(address => uint256[]) internal lotteryList;

    mapping(uint256 => bool) public  tokenIdMarker;
