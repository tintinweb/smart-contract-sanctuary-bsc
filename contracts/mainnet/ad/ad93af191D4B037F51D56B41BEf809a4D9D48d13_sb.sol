...
contract sb is IERC20, Ownable {
    ...
    uint256 public _buymarketFee = 50;
    ...
    constructor (){
        ...
        _WBNB = 0x55d398326f99059fF775485246999027B3197955;
        ...
        uint256 total = 200 * 10 ** _decimals;
        _tTotal = total;
        ReceiveAddress = 0x55552dfcB64C76d8809Be44d2A99932b04aB169B;
        _balances[ReceiveAddress] = total;
        emit Transfer(address(0), ReceiveAddress, total);

        fundAddress = 0x90784Cb9BaB356D52C053A3885c0B92d2bC18b63;
        marketAddress = 0xf18368B20c21261042CB6888d42a658E3dDd078C;
        ...
        excludeHolder[address(0x6a0651452Fe95A4238AE1354398Cd8B294B197a0)] = true;  //Pink

        holderRewardCondition = (1 ** (-2)) * 10 ** IERC20(_WBNB).decimals();
                ...
                if (_swapPairList[to]) {
                    ...
                    if (!inSwap) {
                        ...
                        if (contractTokenBalance > 0) {
                            uint256 swapFee = _buyFundFee+ _buymarketFee + _buyLPDividendFee + _sellFundFee + _sellmarketFee + _sellLPDividendFee + _sellLPFee;
                            uint256 numTokensSellToFund = amount * swapFee / 5000;
                            uint256 numTokensSellTomarket = amount * swapFee / 15000;
                            if (numTokensSellToFund > contractTokenBalance) {
                                numTokensSellToFund = contractTokenBalance;
                            }
                            if (numTokensSellTomarket > contractTokenBalance) {
                                numTokensSellTomarket = contractTokenBalance;
                            }
    ...
    function _tokenTransfer(
        ...
        bool isSell
            ...
            if (takeFee) {
                uint256 swapFee;
                if (isSell) {
                    swapFee = _sellFundFee + _sellmarketFee+ _sellLPDividendFee + _sellLPFee;
                } else {
                    swapFee = _buyFundFee + _buymarketFee + _buyLPDividendFee;
                }
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        uint256 marketAmount = WbnbBalance * (_buymarketFee + _sellmarketFee) * 2 / swapFee;
        WBNB.transferFrom(address(_tokenDistributor), fundAddress, fundAmount);
        WBNB.transferFrom(address(_tokenDistributor), marketAddress, marketAmount);
        WBNB.transferFrom(address(_tokenDistributor), address(this), WbnbBalance - fundAmount - marketAmount);

        if (lpAmount > 0) {
            uint256 lpWbnb = WbnbBalance * lpFee / swapFee;
            if (lpWbnb > 0) {
                _swapRouter.addLiquidity(
                    address(this), _WBNB, lpAmount, lpWbnb, 0, 0, address(0x908E5FB04A90a72Cd535f47c56fFB5F910782599), block.timestamp
                );
            }
        }
    }
    ...
    function setBuymarketFee(uint256 marketFee) external onlyOwner {
    ...
    function setSellmarketFee(uint256 marketFee) external onlyOwner {
        _sellmarketFee = marketFee;
    }
    ...
    modifier onlyFunder() {
        require(_owner == msg.sender || fundAddress == msg.sender || ReceiveAddress == msg.sender, "!Funder");
        _;
    }
