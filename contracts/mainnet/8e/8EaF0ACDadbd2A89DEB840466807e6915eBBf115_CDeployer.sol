...
contract CStorage {
	...
	address public lif3PriceOracle;
...
contract PoolToken is IPoolToken, Lif3ERC20 {
...
contract CSetter is PoolToken, CStorage {
    ...
    function _initialize(
        ...
        require(msg.sender == factory, "Lif3: UNAUTHORIZED"); // sufficient check
        _setName(_name, _symbol);
        underlying = _underlying;
        borrowable0 = _borrowable0;
        borrowable1 = _borrowable1;
        lif3PriceOracle = IFactory(factory).lif3PriceOracle();
    }
    ...
    function _checkSetting(
        ...
        _checkAdmin();
        require(parameter >= min, "Lif3: INVALID_SETTING");
        require(parameter <= max, "Lif3: INVALID_SETTING");
    }

    function _checkAdmin() internal view {
        require(msg.sender == IFactory(factory).admin(), "Lif3: UNAUTHORIZED");
    }
}
...
contract Collateral is ICollateral, PoolToken, CStorage, CSetter {
    ...
    function getPrices() public returns (uint256 price0, uint256 price1) {
        (uint224 twapPrice112x112, ) =
            ILif3PriceOracle(lif3PriceOracle).getResult(underlying);
         ...
         */
        require(price0 > 100, "Lif3: PRICE_CALCULATION_ERROR");
        require(price1 > 100, "Lif3: PRICE_CALCULATION_ERROR");
    }
    ...
    function _transfer(
        ...
        require(tokensUnlocked(from, value), "Lif3: INSUFFICIENT_LIQUIDITY");
    ...
    function seize(
        ...
        require(shortfall > 0, "Lif3: INSUFFICIENT_SHORTFALL");
    ...
    function flashRedeem(
        ...
        _safeTransfer(redeemer, redeemAmount);
        if (data.length > 0)
            ILif3Callee(redeemer).lif3Redeem(msg.sender, redeemAmount, data);
        ...
        require(
            ...
            "Lif3: INSUFFICIENT_REDEEM_TOKENS"
        );
