...
contract Vault is ERC20 {
    ...
    event Received(address, uint);
    event ParameterUpdated(address, address, address, address, uint16, uint16, uint16, uint16, uint16, uint256);
    ...
    function setParameters(
        ...
        emit ParameterUpdated(addrStakers, addrAlgoDev, addrUpbots, addrPartner, pctDeposit, pctWithdraw, pctPerfAlgoDev, pctTradUpbots, pctPerfPartners, maxCap);
    }
    ...
    function takeDepositFees(address token, uint256 amount) private returns(uint256) {
        ...
        IERC20(token).transferFrom(address(this), addrPartner, amount);
        return amount - fees;
    }
