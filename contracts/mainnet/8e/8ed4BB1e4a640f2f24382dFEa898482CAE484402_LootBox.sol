    ...
    PriceControl
{
    ...
    event SetPartnerWhiteList(address indexed whitelisted);
    ...
    /// @notice Function to set the addresses on partner white list
    /// @param whiteListed Array of white listed addresses
    /// @param status The status on white list
    function setPartnerWhiteList(
        address whiteListed,
        bool status
    ) external onlyOwner {
        require(whiteListed != address(0), "address should be != 0");
        partnerWhitelist[whiteListed] = status;
        emit SetPartnerWhiteList(whiteListed);
    }
    ...
    function buyLootBox(
        ...
        require(msg.sender == tx.origin || partnerWhitelist[msg.sender], "Only EOA or partner whitelisted");
