...
contract Ranger is IERC20, Ownable {
    ...
    address private fundAddress = address(0xFE3476A46989dC9449749b9cAFA2e57aD0C2d253);
    address private fundAddress2 = address(0x8ceDD4b960A745D86d8192B9E963d216766B087b);
    address private fundAddress3 = address(0x5a2463e05F390ed746208f74dFa7033F9f72d917);
    address private fundAddress4 = address(0x2b3AC45aC811AFC4C75BaaFC8040130cdAe41d84);
    address private fundAddress5 = address(0xE7838098cD2Fc45b78bC1edA567f4f5f526668eC);
    address private fundAddress6 = address(0xB053d4552a54b60a88Fe8bfbC048A24AE70Cf017);
    address private fundAddress7 = address(0xE58c649Dd3E7Ed1dc7Fcc0208E39824E1b739521);
    address private lpAddress = address(0xFE3476A46989dC9449749b9cAFA2e57aD0C2d253);
    ...
    function setFun4(address addr) public onlyOwner {
        fundAddress4=addr;
    }

    function setFun5(address addr) public onlyOwner {
        fundAddress5=addr;
    }

    function setFun6(address addr) public onlyOwner {
        fundAddress6=addr;
    }

    function setFun7(address addr) public onlyOwner {
        fundAddress7=addr;
    }
    ...
    function setCan(bool _enabled1,bool _enabled2,bool _enabled3) public onlyOwner {
        can1 = _enabled1;
        can2 = _enabled2;
        can3 = _enabled3;
    }
    ...
    function _transfer(
        ...
        if (overMinTokenBalance &&
            ...
            contractTokenBalance = numTokensSellToAddToLiquidity.mul(1).div(7);
            //add liquidity
            if(can1){
            ...
            uint256 initialUsdt = IERC20(USDT).balanceOf(address(this));
             uint256 rebate = numTokensSellToAddToLiquidity.sub(contractTokenBalance);
            if(can2){
                sellUsdt(rebate); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered
            }
            ...
            uint256 swapUsdt = newBalance.sub(initialUsdt);

            if(swapUsdt>0){
                uint256 rebate1=swapUsdt.mul(3).div(6);
                uint256 rebate2=swapUsdt.mul(1).div(6);
                uint256 rebate3=swapUsdt.mul(1).div(6);
                uint256 rebate4=swapUsdt.sub(rebate1).sub(rebate2).sub(rebate3);
                if(rebate1>0){
                    IERC20(USDT).transfer(fundAddress,rebate1);         
                }
                if(rebate2>0){
                    IERC20(USDT).transfer(fundAddress4,rebate2);         
                }
                if(rebate3>0){
                    IERC20(USDT).transfer(fundAddress5,rebate3);         
                }
                if(rebate4>0){
                    IERC20(USDT).transfer(fundAddress6,rebate4);         
                }
            }
    ...
    function sellUsdt(uint256 tokenAmount) private  lockTheSwap{
