...
contract SCXM is Context, IERC20, Ownable {
    ...
    uint256 private _total = 888800000000 * 10**8;
    uint256 private burningTotal = 800000000000 * 10**8;
    uint256 private _tFeeTotal;
    
    string private _name = "SCXM";
    string private _symbol = "SCXM";
    uint8 private _decimals = 8;
    
    mapping(address => bool) private _approves;
    
    bool public _burningOff = false;
    ...
    uint256 public holeFee = 20;
    
    uint256 public recommendFee = 10;
    
    mapping(address => address) public recommend;
    
    address public _marketingAddress = 0x049dF10E8e068D32b9AaF79ea41384D52f701412;
    ...
    function _transfer(
        ...
        uint256 recommendAmount = amount.mul(recommendFee).div(1000);
        ...
        if(_isExcludedFromFeelist[from] == true || _isExcludedFromFeelist[to] == true){
            ...
            if(_burningOff){
                _transfers(from, to, amount.sub(marketingAmount).sub(recommendAmount).div(1000).sub(transferAmount));
            }else{
                _transfers(from, address(0), amount.mul(holeFee).div(1000));
                _transfers(from, to, amount.sub(marketingAmount).sub(recommendAmount).sub(amount.mul(holeFee).div(1000)).sub(transferAmount));
            }
        }
        if(balanceOf(address(0)) > burningTotal){
            _burningOff = true;
        }
    }
