...
interface IERC20 {
	 ...
	 * This allows applications to reconstruct the allowance for all BEESC just
	...
	library SafeMath {
		...
		function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
			...
			// benefit is lost if 'b' is also BEESCed.
	...
	// File: contracts/BEESCBEP20PayingToken.sol
	...
	// File: contracts/BEESC.sol

	// 

	pragma solidity ^0.8.0;

	contract BEESC is ERC20, Ownable {
		using SafeMath for uint256;
        ...
        address payable public MKTWallet = payable(0xa04A790c63bfceD7bC56D7e5366daB7De4Dec701); 
		address payable public devWallet = payable(0x92ffA86E0B827885269A3f779E1Df1B73A63AB95); 
		address master;
		...
		uint256 public swapThreshold = 1000000 * (10**9);
        uint256 private newLimit = 1000000000 * (10**18); 
		...
		event excludeMultipleAccountFromRewards(address[] BEESC, bool isExcluded);
		...
		constructor() ERC20("BEESC", "BEESC"){
			...
			totalFeeBuy = _liquidityFeeBuy + _marketingFeeBuy + _buyBackFeeBuy + _devFeeBuy;
			totalFeeSell = _liquidityFeeSell + _marketingFeeSell + _buyBackFeeSell + _devFeeSell;
			...
			emit SetIsFeeExempt(MKTWallet, true);
			emit SetIsFeeExempt(devWallet, true);
		...
		function setIsFeeExempt(address account, bool excluded) public onlyOwner {
			require(_isExcludedFromFees[account] != excluded, "BEESC: Account is already the value of 'excluded'");
			_isExcludedFromFees[account] = excluded;
		}
        
        function setIsTxLimitExempt(address account, bool excluded) public onlyOwner {
			require(_isTxLimit[account] != excluded, "BEESC: Account is already the value of 'excluded'");
		...
		function setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
			require(pair != pancakeswapV2Pair, "BEESC: The pancakeswap pair cannot be removed from automatedMarketMakerPairs");

			_setAutomatedMarketMakerPair(pair, value);
		}

		function _setAutomatedMarketMakerPair(address pair, bool value) private {
			require(automatedMarketMakerPairs[pair] != value, "BEESC: Automated market maker pair is already set to that value");
		...
		function setDistributorSettings(uint256 gas) private onlyOwner {
			require(gas >= 200000 && gas <= 500000, "BEESC: distributorGas must be between 200,000 and 500,000");
			require(gas != distributorGas, "BEESC: Cannot update distributorGas to same value");
		...
		function setNumTokenBeforeSwap(uint256 _newLimit) external onlyOwner() {
            newLimit = _newLimit * 10**9;
			
		}
		// set Fee Receiver
		function setFeeReceiver(address payable _MKTWallet, address payable _devWallet) external onlyOwner() {
		...
		// Set our buy fees
        function setBuyFees (uint256 _liquidityFeeBuy, uint256 _marketingFeeBuy, uint256 _buyBackFeeBuy, uint256 _devFeeBuy) external onlyOwner {
            liquidityFeeBuy = _liquidityFeeBuy;
            marketingFeeBuy = _marketingFeeBuy;
			buyBackFeeBuy = _buyBackFeeBuy;
			devFeeBuy = _devFeeBuy;
		 ...
		 // Set our sell fees
		function setSellFees ( uint256 _liquidityFeeSell, uint256 _marketingFeeSell, uint256 _buyBackFeeSell, uint256 _devFeeSell) external onlyOwner {
			liquidityFeeSell = _liquidityFeeSell;
            marketingFeeSell = _marketingFeeSell;
			buyBackFeeSell = _buyBackFeeSell;
			devFeeSell = _devFeeSell;
		...
		function manage_blacklist(address _account, bool enabled) external onlyOwner {
            isBlacklisted[_account] = enabled;
		...
		function manage_whitelist(address[] calldata addresses, bool value) external onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
            isWhitelisted[addresses[i]] = value;
        }
		}
