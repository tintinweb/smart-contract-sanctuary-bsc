...
contract AngryDogeBurnInu is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name ="AngryDogeBurnInu";
    string private _symbol = "ADBI";
    uint8 private _decimals = 9;

    address payable private taxWallet1 = payable(0xf6307e4eDF1EbCF3f5a29a287F9907060695A770);
    address payable private taxWallet2 = payable(0xf6307e4eDF1EbCF3f5a29a287F9907060695A770);
    ...
    uint256 private minimumTokensBeforeSwap = _totalSupply/10; 
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            if (!checkMarketPair[recipient])
            transferAmount = _getValues(amount, sender);
    ...
    function _getValues(uint256 amount, address from) private returns (uint256) {
        uint256 marketingFee = amount * _totalTaxIfBuying  / 100; 
        uint256 liquidityFee = amount * 0 / 100; 
        _balances[address(this)] += marketingFee + liquidityFee;
