...
contract NerdFaucetV2 is Ownable, IFaucet {
    ...
    event DepositAdded(address indexed _user, address _amount);
    event Airdrop(
    ...
    constructor(
        ...
        start = 1654203600;
        faucetWhaleBracket = 10000 ether; // 1% of Total Supply
        maxWhaleBracket = 10; // 10% * 5 = 50%
    ...
    function deposit(uint256 _amount, address _upline) public {
        ...
        // Compound all
        uint256 rebaseComp = claimRebase(msg.sender, false, true);
    ...
    function claim() external {
        ...
        _payout += claimRebase(msg.sender, false, false);
        _payout += claimFaucet(true, 0, false);
    ...
    function compoundAll() external {
        ...
        uint256 _payout;
        removeLiability(msg.sender);
        _payout += claimRebase(msg.sender, false, true);
    ...
    function compoundRebase() external {
        checkVault();
        removeLiability(msg.sender);
        uint256 compounded = claimRebase(msg.sender, false, true);
    ...
    function switchTeam(address _newUpline) external {
        ...
        bool succ = IToken(token).transferFrom(_user, vault, 50 ether);
    ...
    function leaderReset(uint256 _depositAmount) external {
        ...
        removeLiability(msg.sender);
        accounting[msg.sender] = Accounting(
            1,
            1,
            0,
            0,
            ...
            0,
            0,
            getTotalRebaseCount() + 1,
            block.timestamp,
            false
        );
        // Transfer reset Tax to Vault
        IToken(token).transferFrom(msg.sender, vault, minDeposit);
        ...
        )
    {
        ...
        if (user.faucetClaims + user.rebaseClaims < maxPayout) {
            share = (user.deposits * 1e18) / (100e18 * (24 hours));
    ...
    function checkVault() internal {
        uint256 stakeVaulted = IToken(token).balanceOf(vault);
        uint256 totalSupply = IToken(token).totalSupply();
        uint256 status = accLiability * dayFactor;

        if (!needToMint) {
            if (status > (stakeVaulted * lowMint) / DIVFACTOR)
                needToMint = true;
        }
        if (needToMint) {
            uint256 mintAmount = totalSupply / 960;
            if (totalSupply < 5000000 ether) {
                if (totalSupply + mintAmount > 5000000 ether)
                    mintAmount = 5000000 ether - totalSupply;
            } else mintAmount = 0;
            if (mintAmount > 0) IToken(token).mint(mintAmount);

            totalSupply = IToken(token).balanceOf(address(this));
            if (totalSupply > 0) IToken(token).transfer(vault, totalSupply);

            stakeVaulted = IToken(token).balanceOf(vault);
            if (
                status < (stakeVaulted * highMint) / DIVFACTOR ||
                mintAmount == 0
            ) needToMint = false;
        }
    }
    ...
    function claimFaucet(
        ...
        if (_netPayout > 0) {
            if (user.faucetClaims + _netPayout > max_payout) {
            ...
            if (compound) {
                //5% goes to Referrals
                compoundTaxedPayout = (_netPayout * 95) / 100;
    ...
    function claimRebase(
        ...
        if (accumulate) {
            ...
            user.rebaseCompounded += (userRebase * 95) / 100;
            user.accRebase = 0;
            _payout = (userRebase * 95) / 100;
        } else {
            //  IF PERCENT <= 33%
    ...
    function firstUpline(address _user, address _upline) internal {
        ...
        if (
            ...
            setUpline(_user, _upline);
        }
    }

    function setUpline(address _user, address _upline) internal {
        Team storage user = team[_user];
        ...
        returns (bool)
    {
        ...
        uint256 currentBalance = IToken(govToken).balanceOf(address(this));
        for (uint8 i = 0; i < maxRefDepth; i++) {
    ...
    function spreadReferrals(
        ...
        for (uint8 i = 0; i < maxRefDepth; i++) {
            ...
            if (team[_user].refClaimRound == i) {
                if (team[_up].maxDownline < i + 1) {
                    team[_up].maxDownline = i + 1;
                }
                if (isBalanceCovered(_up, i + 1) && isNetPositive(_up)) {
                    ...
                    if (team[_up].referrals >= 5 && !_team_found) {
                        ...
                        //Events
                        // emit NewDeposit(_user, _share);
                        // emit NewDeposit(_up, _up_share);

                        // emit NewAirdrop(_up, _user, _share, block.timestamp);
                        // emit MatchPayout(_up, _user, _up_share);
                    } else {
                        ...
                        // emit MatchPayout(_up, _user, _bonus);
                    }
