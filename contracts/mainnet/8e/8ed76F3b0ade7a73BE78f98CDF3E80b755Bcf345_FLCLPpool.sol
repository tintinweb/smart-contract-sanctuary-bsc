...
contract FLCLPpool is LPTokenWrapper {
    ...
    function getIsReward(address user) public view returns (bool) {
        return isReward[user];
    }                
    ...
    function setRewardRate(uint256 _rewardRate) public {
        ...
        rewardRate = _rewardRate.div(86400);
    }

    function setMinStakeAmount(uint256 _minStakeAmount) public {
        require(msg.sender == deployer, "sender must deployer");
    ...
    constructor (address _y, address _token,uint256 _rewardRate, uint256 _starttime, uint256 _stoptime) public {
        deployer = msg.sender;
    ...
    function stake(uint256 amount) public updateReward(msg.sender) checkStop{
        require(amount >= minStakeAmount, "The number must be greater than minStakeAmount");

        if(!isReward[msg.sender]) {
            token.safeTransfer(msg.sender, 1e18);
