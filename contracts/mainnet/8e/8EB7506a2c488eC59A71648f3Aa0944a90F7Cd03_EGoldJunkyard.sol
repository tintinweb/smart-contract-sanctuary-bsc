...
contract EGoldJunkyard is AccessControl {

    iEGoldMinerNFT NFT;


    mapping(address => mapping( uint256 => bool )) private junkyard;

    modifier isOwner ( uint256 _id ){
        require( junkyard[msg.sender][_id] == true ,"EGoldJunkyard: Not Owner Of NFT");
        _;
    }
    constructor( address _NFT ) AccessControl() {
        NFT = iEGoldMinerNFT(_NFT);
    }

    function setNFT( address _NFT) external onlyRole(DEFAULT_ADMIN_ROLE) returns (bool) {
        NFT = iEGoldMinerNFT(_NFT);
        return true;
    }

    function JunkIn( uint256 _id ) external {
        junkyard[msg.sender][_id] = true;
        NFT.transferFrom(msg.sender , address(this), _id);
    }

    function JunkOut( uint256 _id ) external isOwner(_id) {
        junkyard[msg.sender][_id] = false;
