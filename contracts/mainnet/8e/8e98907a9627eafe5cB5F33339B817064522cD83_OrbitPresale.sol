...
contract OrbitPresale is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    using EnumerableSet for EnumerableSet.AddressSet;

    address public presaleToken;
    uint256 public presalePrice; // wei
    uint256 public totalInvestedAmount;
    EnumerableSet.AddressSet investors;

    uint256 public presaleStartTime; // presale start
    uint256 public presaleEndTime; // claim start
    ...
    function deposit(uint256 amount) external {
        require(
            ...
            "OrbitPresale: Presale not started!"
        );
        require(
            block.timestamp < presaleEndTime,
            "OrbitPresale: Presale ended!"
        );
        require(
            whitelist[msg.sender],
            "OrbitPresale: You are not whitelisted!"
        );
    ...
    function getAvailableAmount(address _user) public view returns (uint256) {
        if (userWithdrawn[_user]) {
            return 0;
        } else {
            return depositInfo[msg.sender].mul(10**18).div(presalePrice);
        }
    }
    ...
    function claim() external {
        require(
            block.timestamp > presaleEndTime,
            "OrbitPresale: Claim not started!"
        );
        require(
            depositInfo[msg.sender] > 0,
            "OrbitPresale: Didn't deposit BUSD!"
        );
        uint256 amountToSend = getAvailableAmount(msg.sender);
        uint256 presaleTokenBalance = IERC20(presaleToken).balanceOf(
            address(this)
        );
        require(
            presaleTokenBalance > amountToSend,
            "OrbitPresale: Not enough OrbitToken to send"
        );
        IERC20(presaleToken).safeTransfer(msg.sender, amountToSend);
    ...
    function setPresaleTokenAddress(address _newTokenAddress)
        ...
        onlyOwner
    {
        presaleToken = _newTokenAddress;
    }

    function setPresaleStartEndTime(uint256 _startTime, uint256 _endTime)
        external
        onlyOwner
    {
        presaleStartTime = _startTime;
