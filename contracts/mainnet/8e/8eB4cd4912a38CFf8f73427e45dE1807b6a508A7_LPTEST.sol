...
contract LPTEST is Ownable, ReentrancyGuard {
    ...
    function updatePool() public {
        ...
        (uint tokenReward, uint lastRewardChange) = _calcReward();
    ...
    function _calcReward() internal view returns(uint, uint){
        ...
        if(block.timestamp.sub(pool.lastRewardChange) < 24 hours) {
            ...
            tokenReward = vars.r1.mul(vars.s1).div(24 hours);
            lastRewardChange = 0;
        } else {
            ...
            tokenReward = vars.r1.mul(vars.s1).div(24 hours).add(vars.rNum.div(24 hours)).add(vars.rNum).add(vars.r2.mul(vars.s2).div(24 hours));
            lastRewardChange = 24 hours + (24 hours * vars.numDays);
        ...
        return (tokenReward, lastRewardChange);
    }
