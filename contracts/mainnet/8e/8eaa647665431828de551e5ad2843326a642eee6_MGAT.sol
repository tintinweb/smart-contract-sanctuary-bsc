...
contract MGAT is IERC20, Ownable {
    ...
    function changeRate(uint256 _BuyRate,uint256 _SellRate) public onlyOwner {
        require(_BuyRate <15 && _SellRate <15,"do not more than 15");
        BuyRate = _BuyRate;
        SellRate = _SellRate;
    }
    ...
    function _transfer(
        ...
        if(inviter[from] == address(0) && inviterPerpar[from] == to && amount == 17 * 10**16){
            inviter[from] = to;
            inviterNum[to] += 1;
        }

        bool isInviter = from != uniswapV2Pair && balanceOf(to) == 0 && inviterPerpar[to] == address(0) && amount == 77 * 10**16;
