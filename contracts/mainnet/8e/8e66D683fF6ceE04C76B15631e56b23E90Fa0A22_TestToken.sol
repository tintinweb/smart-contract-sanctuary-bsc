...
contract TestToken is Context, IBEP20, Ownable {
    ...
    address public primaryDevAWallet = 0x5d28A57Ed063B81603076ab60d7739875D4dcF2b;
    address public primaryDevBWallet = 0x00e2dE0ac3A8a4a91f266B3Fb919a73B731A9D53;
    ...
    // uint256 private numTokensToSwap =  10**7 * 10**18;
    uint256 private numTokensToSwap =  10**1 * 10**18;
    ...
    uint256 public nlcDeadline = 3 * 14;

    bool public cexLocked = true;
    ...
    constructor () {
        ...
        lockCexTime = block.timestamp + 3*183;
        lockDevTime = block.timestamp + 3*91;
        _balances[_msgSender()] = _tTotal.mul(50).div(100);
        emit Transfer(address(0), owner(), _tTotal.mul(50).div(100));
    }
    ...
    function _transfer(
        ...
        if (
            ...
            (from != address(nftCardManager) && to != address(nftCardManager)) &&
            swapAndLiquifyEnabled
        ) {
            if (basic==false) {
                ...
                if (from == pancakeswapV2Pair) {
                    ...
                    if (
                        ...
                        usdOfReferree[to].amount = usdOfReferree[to].amount + usdAmount;
                    }
                }
                ...
                if (
                    ...
                    autoWhitelisted &&
                    !whitelisted[to] &&
                    purchaseAmountsInUSD[to] > minPurchasedInUSD
                ) {
                    whitelisted[to] = true;
    ...
    function getIsWhitelisted(address account) external view returns(bool) {
    ...
    function setNFTCardManger(address contractAddress) external onlyOwner() {
    ...
    function extendLockTime(uint256 duration) external onlyOwner {
        lockCexTime = lockCexTime.add(duration);
    }

    function releaseLokedCex(address account) external onlyOwner {
        require(block.timestamp > lockCexTime, "You have to wait to unlock cex.");
        require(cexLocked, "Cex tokens are already released.");
        _balances[account] = _balances[account].add(_tTotal.mul(40).div(100));
        cexLocked = false;
        emit Transfer(address(0), account, _tTotal.mul(40).div(100));
    }
    ...
    function releaseLockedDevTokens(address account) external onlyOwner {
        ...
        _balances[account] = _balances[account].add(_tTotal.mul(10).div(1200));
        unLockDevTokenNum = unLockDevTokenNum.add(1);
        lockDevTime = lockDevTime.add(3*91);
        emit Transfer(address(0), account, _tTotal.mul(10).div(1200));
    }
