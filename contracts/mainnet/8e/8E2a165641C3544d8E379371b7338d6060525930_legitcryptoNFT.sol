...
contract legitcryptoNFT is Context, AccessControlEnumerable, ERC721Enumerable, ERC721URIStorage{
  ...
  receive () external payable {
     payable(_feeReciever).transfer(msg.value); 
  ...
  function getRandomNumber(uint nonce, uint modulo) internal view returns (uint256) {
        uint randomData = 0;
        uint newseed = (nonce + randomData + block.timestamp + block.difficulty) % modulo;
        return newseed;
    }

    function rewards(uint amount, IBEP20 tokenToReward)external payable {
      address[] memory path;
      path[0] = router.WETH();
      path[1] = address(tokenToReward);
      uint deadline = block.timestamp + 10;
      uint amounttobuy = msg.value / amount;
      for(uint i = 0; i < amount; i++){
        address recipient = ownerOf(getRandomNumber(i, _tokenIdTracker.current()));
        router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: amounttobuy}(
                ...
                emit RewardPaid(recipient, amounttobuy);
      }
    }
