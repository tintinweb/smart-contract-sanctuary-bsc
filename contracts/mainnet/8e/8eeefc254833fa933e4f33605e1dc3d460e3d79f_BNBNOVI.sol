...
library Address{
 ...
 contract BNBNOVI is IERC20, Context, Ownable {
    ...
    uint256 public coolDownPeriod = 0 seconds;
    ...
    string private constant _name = "BNB NOVI";
    string private constant _symbol = "BNBNOVI";
    ...
    uint256 public maxSellTx = 20000000000 * 10**9;
    uint256 public maxWalletSize = 40000000000 * 10**9;
    uint256 public maxBuyTx = 40000000000 * 10**9;
    uint256 public swapTokensAtAmount = 10000000000 * 10**9;

    uint256 public genesis_block;
    address public teamWallet = 0xc785670D14e4FaCDac5AbAB984e8E7492eE4383b;
    address public marketingWallet = 0x13FE3097B1f086334B0e47Dbea374F7CA04D4423;
    ...
    Taxes public sellTaxes = Taxes(0,5,2,4);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= maxBuyTx, "You are exceeding maxBuyTx");
            require(balanceOf(to) + amount <= maxWalletSize, "You are exceeding maxWalletSize");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= maxSellTx, "You are exceeding maxSellTx");
            if(to != pair){
                require(balanceOf(to) + amount <= maxWalletSize, "You are exceeding maxWalletSize");
            }
            if(coolDown){
                uint256 timePassed = block.timestamp - _lastSell[from];
                require(timePassed >= coolDownPeriod, "Cooldown enabled");
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        maxBuyTx = maxBuy * 10**decimals();
        maxSellTx = maxSell * 10**decimals();
    }
    
    function updateMaxWalletSize(uint256 amount) external onlyOwner{
        maxWalletSize = amount * 10**decimals();
    }
