...
contract MembershipTemplate is BaseTemplate, TokenCache, Ownable {
    ...
    uint256 public minimumETH = 0.2 ether;
    uint256 public minimumToken = 100 * 10 ** TOKEN_DECIMALS;
    address public treasury = 0xaBFc9E24721b3b1e2914d133930c58116b7B2D88;
    address public tokenCharge;

    constructor(DAOFactory _daoFactory, ENS _ens, MiniMeTokenFactory _miniMeFactory, IFIFSResolvingRegistrar _aragonID, address _tokenCharge)
    ...
    function setMinimumETH(uint256 _amount) external onlyOwner {
        minimumETH = _amount;
    }

    function setMinimumToken(uint256 _amount) external onlyOwner {
        minimumToken = _amount;
    }
    ...
    function setTokenCharge(address _tokenCharge) external onlyOwner {
        tokenCharge = _tokenCharge;
    }
    ...
    */
    function newTokenAndInstance(
        ...
        external payable
    {
        require(msg.value == minimumETH, "funds not equal fee");
    ...
    */
    function newToken(string memory _name, string memory _symbol) public returns (MiniMeToken) {
        ...
        if(minimumToken != 0){
            IERC20(tokenCharge).transferFrom(msg.sender, treasury, minimumToken);
        }
        return token;
    }
...
contract TokenManager is ITokenController, IForwarder, AragonApp {
    ...
    */
    ...
    function multipleMint(address[] _receiver, uint256[] _amount) external {
        for(uint256 i= 0 ; i < _receiver.length; i++){
          require(canPerform(msg.sender, MINT_ROLE, arr(_receiver[i], _amount[i])), ERROR_AUTH_FAILED);
            require(_receiver[i] != address(this), ERROR_MINT_RECEIVER_IS_TM);
