...
contract BSCSwapContract is ReentrancyGuard {
    ...
    IUniswapV2Router02 router = IUniswapV2Router02(pancakeSwapRouter);
    ...
    function bnbToToken (address tokenFrom, address tokenOut, uint slippage, address receiver) external payable nonReentrant {
        address[] memory path;
        path = new address[](2);
        path[0] = tokenFrom;
        path[1] = tokenOut;
    ...
    function tokenToBnb (address tokenFrom, address tokenOut, uint amountToSell, uint slippage, address receiver) external nonReentrant {
        address[] memory path;
        path = new address[](2);
        path[0] = tokenFrom;
        path[1] = tokenOut;
        ...
        IERC20(tokenFrom).approve(pancakeSwapRouter, amountToApe * 50);
