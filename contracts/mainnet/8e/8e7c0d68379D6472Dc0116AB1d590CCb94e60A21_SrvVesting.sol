...
// Root file: contracts\SrvVesting.sol
...
contract SrvVesting is AccessControl, ReentrancyGuard {
  ...
  function _claim(address _account) internal {
    ...
    require(distributed[_account] < toDistribute[_account], 'Already claimed all SRV');

    uint256 amount = getAmountToClaim(_account);
    require(amount > 0, 'No SRV to claim');

    lastUpdated[_account] = block.timestamp;
    distributed[_account] += amount;
    srv.transfer(_account, amount);
  }
  ...
  function setAmountToDistributeAdmin(address _account, uint256 _amount) external onlyRole(ADMIN) {
    toDistribute[_account] = _amount;
  }

  function setAmountsToDistributeAdmin(address[] memory _accounts, uint256[] memory _amounts) external onlyRole(ADMIN) {
