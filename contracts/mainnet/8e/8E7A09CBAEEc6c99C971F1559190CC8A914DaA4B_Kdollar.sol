...
contract Kdollar is ERC20Burnable, Operator {
    ...
    // Initial distribution for the day 2-5 KDOLLA-KRONOS LP -> KDOLLAR pool
    // uint256 public constant INITIAL_KDOLLAR_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the KDOLLAR ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("KDOLLAR", "KDOLLAR") {
        // Mints 1 KDOLLAR to contract creator for initial pool setup
    ...
    function _getKdollarPrice() internal view returns (uint256 _kdollarPrice) {
        try IOracle(kdollarOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Kdollar: failed to fetch KDOLLAR price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _kdollarPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_kdollarPrice >= taxTiersTwaps[tierId]) {
    ...
    function setKdollarOracle(address _kdollarOracle) public onlyOperatorOrTaxOffice {
        require(_kdollarOracle != address(0), "oracle address cannot be 0 address");
        kdollarOracle = _kdollarOracle;
    }
    ...
    /**
     * @notice Operator mints KDOLLAR to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of KDOLLAR to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentKdollarPrice = _getKdollarPrice();
            currentTaxRate = _updateTaxRate(currentKdollarPrice);
            if (currentKdollarPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(address _genesisPool) external onlyOperator {
        ...
        // _mint(_cometPool, INITIAL_KDOLLAR_POOL_DISTRIBUTION);
