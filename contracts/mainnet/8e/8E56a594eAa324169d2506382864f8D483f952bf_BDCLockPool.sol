...
contract BDCLockPool {
    ...
    uint256 public dailyReleaseRate = (1 * RATIO) / 300;
    ...
    modifier checkRelease() {
        _;
        if (!isDecrease && lockedTotal.div(100) >= 150000 * 10**9) {
            isDecrease = true;
        }

        while (
            isDecrease &&
            withdrawTotal / (500000 * 10**9) > decreaseDate.length &&
...
contract BDCToken is ERC20Capped {
    ...
    constructor()
        ...
        ERC20Capped(6000000000 * 10**9)
    {
        _mint(msg.sender, 6000000000 * 10**9);
    }
