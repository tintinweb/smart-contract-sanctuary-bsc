...
contract SOSTAKE is ERC20Detailed, Ownable {
    ...
    uint256 public rfvSellFee = 20;
    uint256 public DividendFeeBuy = 0;
    uint256 public DividendFeeSell = 0;
    uint256 public sellFee = 40;
    uint256 public burnFee = 10;
    uint256 public totalFee = liquidityFee.add(marketingFee).add(burnFee).add(rfvFee);

    uint256 public totalFeeSell = liquidityFee.add(marketingFee).add(sellFee).add(rfvSellFee).add(burnFee);
    ...
    constructor() ERC20Detailed("SOSTAKE", "SOS", uint8(DECIMALS)) Ownable() {
        ...
        autoLiquidityReceiver = 0x0504f3297e8186b16f9aB577E1CB91C5D5fba9aA;
        marketingReceiver = 0x9432f7c5370876Df5aF4397cDaFBDc50fE33FB11;   
        RiskFreeValueReceiver = 0x77a956F45745d907CcE2Cb19acF7eb66B127a2E6;
        ...
        isDividendExempt[RiskFreeValueReceiver] = true;
    ...
    function takeFee(
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_marketingFee.add(_rfvFee))
        );
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(marketingReceiver).call{
            value: amountETH.mul(marketingFee).div(
                marketingFee.add(rfvFee)
            ),
            gas: 30000
        }("");
        (success, ) = payable(RiskFreeValueReceiver).call{
            value: amountETH.mul(rfvFee).div(
                marketingFee.add(rfvFee)
            ),
            gas: 30000
        }("");
    ...
    function withdrawAllToMarketing() external swapping onlyOwner {
        ...
        uint256 amountToSwap = _gonBalances[address(this)].div(_gonsPerFragment);
        require( amountToSwap > 0,"There is no sostake token deposited in token contract");
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();
        router.swapExactTokensForETHSupportingFeeOnTransferTokens(
    ...
    function setFeeReceivers(
        ...
        RiskFreeValueReceiver = _rfvReceiver;    
    }
