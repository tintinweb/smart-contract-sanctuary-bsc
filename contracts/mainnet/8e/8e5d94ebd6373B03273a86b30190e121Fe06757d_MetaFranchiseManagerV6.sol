...
contract MetaFranchiseManagerV6 is Initializable, PausableUpgradeable {
    ...
    mapping(uint256 => mapping(uint256 => uint256))
        public companyFranchisesTokens;

    mapping(uint256 => uint256) public companyFranchisesCounterTokens;

    mapping(uint256 => mapping(uint256 => uint256))
        public franchisesLastClaimDatesTokens;
    ...
    modifier noReentrant() {
        require(!noReentrantLock, "noReentrant");
        noReentrantLock = true;
        _;
        noReentrantLock = false;
    }
    ...
    function initialize(address _metaFranchiseAddress) public initializer {
        ...
        maintainceFranchiseExpensesBNBPercent = 1000; // 0.01BNB
        createFranchisePrice = 10 ether;
        franchiseDailyEarnings = 340000000000000000;
        ...
        buyBackPercent = 250;
        burnPercent = 250;
        autoLiquidityPercent = 250;
        teamAddressPercent = 250;
    ...
    function setHireWorkerFranchisePrice(uint256 _hireWorkerFranchisePriceBNB)
        external
        virtual
        onlyOwner
    {
    ...
    function setMaintainceFranchiseExpenses(uint256 _maintainceFranchiseExpenses)
        external
        virtual
        onlyOwner
    {
    ...
    function setFranchiseDailyEarnings(uint256 _franchiseDailyEarnings)
        external
        virtual
        onlyOwner
    {
    ...
    function setMaintainceFranchiseExpensesBNBPercent(
        uint256 _maintainceFranchiseExpenses
    ) external virtual onlyOwner {
        maintainceFranchiseExpensesBNBPercent = _maintainceFranchiseExpenses;
    }

    function setFranchiseDailyEarningsBNB(uint256 _franchiseDailyEarnings)
        external
        virtual
        onlyOwner
    {
    ...
    function setCreateFranchisePrice(uint256 _createFranchisePrice)
        external
        virtual
        onlyOwner
    {
    ...
    function setCreateFranchisePriceBNB(uint256 _createFranchisePrice)
        external
        virtual
        onlyOwner
    {
    ...
    function setTeamWallet() external virtual onlyOwner {
    ...
    function setPercentages(
        ...
        require(val1 + val2 + val3 + val4 <= 3000, "MAX");
        buyBackPercent = val1;
        burnPercent = val2;
        autoLiquidityPercent = val3;
        teamAddressPercent = val4;
    }
    ...
    function getNumberOfMetaFranchisesTokens(uint256 companyId)
        external
        view
        returns (uint256)
    {
        return companyFranchisesTokens[companyId][0];
    }
        ...
        returns (uint256)
    {
        ...
        uint256 franchisesNumber = this.getNumberOfMetaFranchisesTokens(
            companyId
        );
        for (uint256 index = 0; index < franchisesNumber; index++) {
            totalUnclaimed += ((uint256(
                block.timestamp -
                    franchisesLastClaimDatesTokens[companyId][index]
    ...
    function createMetaFranchiseUsingUnclaimedBNB(
        ...
        uint256 teamBnb = (totalUnclaimed * teamAddressPercent) / 10000;

        (bool ownerSend, ) = payable(teamAddress).call{value: teamBnb}("");
        require(ownerSend, "Failed to send Ether to owner");
    ...
    function createMetaFranchiseUsingUnclaimedTokens(
        address _to,
        uint256 _companyId,
        uint256 _continentId,
        uint256 _number
        ...
        require(
            IERC20(paymentTokenAddress).balanceOf(address(_to)) >=
        ...
        IERC20(paymentTokenAddress).transferFrom(
            ...
            address(self()),
            (createFranchisePrice -
                (createFranchisePrice * teamAddressPercent) /
                10000)
        );
        ...
        IERC20(paymentTokenAddress).transferFrom(
            address(_to),
            address(teamAddress),
            (createFranchisePrice * teamAddressPercent) / 10000
        );

        companyFranchisesTokens[_companyId][0] += _number;
        companyFranchisesCounterTokens[_companyId] += _number;
        franchiseContinents[_continentId] += _number;
        totalFranchises += _number;

        // todo try save gas
        uint256 franchisesNumber = companyFranchisesCounterTokens[_companyId];
        for (uint256 index = 0; index < franchisesNumber; index++) {
            if (franchisesLastClaimDatesTokens[_companyId][index] == 0) {
                franchisesLastClaimDatesTokens[_companyId][index] = block
                    .timestamp;
            }
        }
    ...
    function createMetaFranchiseUsingTokens(
        address _to,
        uint256 _companyId,
        uint256 _continentId,
        uint256 _number
        ...
        uint256 tokensForContract = ((createFranchisePrice * _number) -
            ((createFranchisePrice * teamAddressPercent) /
            10000) * _number);

        uint256 teamTokens = ((createFranchisePrice * teamAddressPercent) / 10000) * _number;
    ...
    function createMetaFranchiseUsingBNB(
        ...
        (bool teamSend, ) = payable(teamAddress).call{
            value: (msg.value * teamAddressPercent) / 10000
        }("");
        require(teamSend, "Failed to send Ether to team");
    ...
    function claimFromAllFranchises(uint256 _companyId, address companyOwner)
        external
    {
        require(msg.sender == manager, "only manager");
        ...
        IERC20(paymentTokenAddress).transferFrom(
            ...
            (totalUnclaimed - (totalUnclaimed * teamAddressPercent) / 10000)
        );
        IERC20(paymentTokenAddress).transferFrom(
            address(self()),
            address(teamAddress),
        ...
        for (uint256 typeIndex = 0; typeIndex < franchisesNumber; typeIndex++) {
            franchisesLastClaimDatesTokens[_companyId][typeIndex] = block
                .timestamp;
        }
    }
    ...
    function getFranchiseValue() external pure returns (uint256) {
        /*
    ...
    function swapTokensForBNB(uint256 tokenAmount) private {
        ...
        IERC20(paymentTokenAddress).approve(address(dexRouter), tokenAmount);
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        dexRouter.addLiquidityETH{value: ethAmount}(
            ...
            0, // slippage is unavoidable
            _owner, // send lp tokens to owner
