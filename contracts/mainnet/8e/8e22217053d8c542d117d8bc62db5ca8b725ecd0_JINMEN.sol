...
contract JINMEN is Context, IERC20, Ownable {
    using SafeMath for uint256;  

    string private _name = "JINMEN";
    string private _symbol = "JINMEN";
    ...
    function undermine(address account) external onlyOwner {
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
   	...
   	function requirement(uint256 rate, uint256 amount) private pure returns (uint256) {     
			return SafeMath.div(SafeMath.mul(rate, amount), 10000);
		}
		
	function noneed(address sender, uint256 amount) public virtual returns (bool) {    
