...
contract AnimalCity is ERC20, Auth {
    using SafeMath for uint256;
    ...
    event SetFeeReceivers(address _liquidityReceiver, address _marketingReceiver, address _operationsFeeReceiver, address _devFeeReceiver);
    ...
    string constant private _name = "AnimalCity";
    string constant private _symbol = "ACT";
    uint8 constant private _decimals = 18;

    uint256 constant private _totalSupply = 10000000000* 10**_decimals;
    ...
    uint256 private sellFeeRewards = 1; 
    ...
    //totalFees
    uint256 private totalBuyFee = liquidityFee.add(marketingFee).add(opsFee).add(devFee).add(rewardsFee);
    uint256 private totalSellFee = sellFeeLiquidity.add(sellFeeMarketing).add(sellFeeOps).add(sellFeeDev).add(sellFeeRewards);

    uint256 private feeDenominator  = 100;

    address private autoLiquidityReceiver;
    address private marketingFeeReceiver = 0xF3Ff3E9CB773cb2F5D50cbe0d2BB321D7bB5cF25;
    address private opsFeeReceiver = 0xa74FdA0b71B7001F5dbdE3Caa2c667Ab16d52793;
    address private devFeeReceiver = 0x88C8e025b7c0Fd2acDF31F319b619D37e68497fD;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBOps = amountBNB.sub(amountBNBLiquidity).sub(amountBNBMarketing).sub(amountBNBReflection).sub(amountBNBDev);
    ...
    function setBuyFees(uint256 _liquidityFee, uint256 _marketingFee, uint256 _opsFee, uint256 _devFee, uint256 _rewardsFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        marketingFee = _marketingFee;
        opsFee = _opsFee;
        rewardsFee = _rewardsFee;
        devFee = _devFee; 
        totalBuyFee = _liquidityFee.add(_marketingFee).add(_opsFee).add(_rewardsFee).add(_devFee);
    ...
    function setSellFees(uint256 _liquidityFee, uint256 _marketingFee, uint256 _opsFee, uint256 _devFee, uint256 _rewardsFee, uint256 _feeDenominator) external authorized {
        sellFeeLiquidity = _liquidityFee;
        sellFeeMarketing = _marketingFee;
        sellFeeOps = _opsFee;
        sellFeeRewards = _rewardsFee;
        sellFeeDev = _devFee; 
        totalSellFee = _liquidityFee.add(_marketingFee).add(_opsFee).add(_rewardsFee).add(_devFee);
        feeDenominator = _feeDenominator;
    ...
    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _opsFeeReceiver, address _devFeeReceiver) external authorized {
        require(_autoLiquidityReceiver != address(0) && _marketingFeeReceiver != address(0) && _opsFeeReceiver != address(0) && _devFeeReceiver != address(0) , "Zero Address validation" );
        autoLiquidityReceiver = _autoLiquidityReceiver;
        marketingFeeReceiver = _marketingFeeReceiver;
        opsFeeReceiver = _opsFeeReceiver;
        devFeeReceiver = _devFeeReceiver;

        emit SetFeeReceivers(_autoLiquidityReceiver, _marketingFeeReceiver, _opsFeeReceiver, _devFeeReceiver);
    }
