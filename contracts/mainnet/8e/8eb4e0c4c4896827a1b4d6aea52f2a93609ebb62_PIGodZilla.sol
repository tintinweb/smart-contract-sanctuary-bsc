...
library Address{
 ...
 contract PIGodZilla is IERC20, Context, Ownable {
    ...
    string private constant _name = "PIGO";

    uint256 public PIGO_max_BuyLimit = 10000000000 * 10**9;
    uint256 public PIGO_max_SellLimit = 10000000000 * 10**9;

    address private PIGO_Wallet = 0x951B2F18C4cE5E77b82d1c9bE272F0afD751e888;
    address private PIGO_Team_Wallet = 0x951B2F18C4cE5E77b82d1c9bE272F0afD751e888;

    uint256 public PIGO_max_WalletLimit = 30000000000 * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[PIGO_Wallet] = true;
        _isExcludedFromFee[PIGO_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[PIGO_Wallet] = true;
        allowedTransfer[PIGO_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= PIGO_max_BuyLimit, "You are exceeding PIGO_max_BuyLimit");
            require(balanceOf(to) + amount <= PIGO_max_WalletLimit, "You are exceeding PIGO_max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= PIGO_max_SellLimit, "You are exceeding PIGO_max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= PIGO_max_WalletLimit, "You are exceeding PIGO_max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(PIGO_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(PIGO_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function Doji_Update_MarketingWallet(address newWallet) external onlyOwner{
        PIGO_Wallet = newWallet;
    }
    
    function Doji_Update_TeamWallet(address newWallet) external onlyOwner{
        PIGO_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        PIGO_max_BuyLimit = maxBuy * 10**decimals();
        PIGO_max_SellLimit = maxSell * 10**decimals();
    }
    
    function Doji_Max_Walletlimit(uint256 amount) external onlyOwner{
        PIGO_max_WalletLimit = amount * 10**decimals();
    }
