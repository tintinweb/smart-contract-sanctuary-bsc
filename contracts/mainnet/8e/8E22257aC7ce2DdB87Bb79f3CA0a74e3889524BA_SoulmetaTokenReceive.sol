...
contract SoulmetaTokenReceive is DivestorUpgradeable{
    ...
    function getRate() public view returns (uint){
        ...
        if(cSoulRouter.payCountRecord(time - 1) == 0){
            ...
            if(frequency >= 750) openIndex = 2;
        }
        return rate[openIndex];
    }

    function soulReceive() public{
        require(isOpen, "not open");
        require(userReceive[_msgSender()].receiveTime != block.timestamp/86400, "user has received");
        require(userReceive[_msgSender()].amount > 0, "balance is 0");
        uint amount = 50 * userReceive[_msgSender()].amount / 100;

        userReceive[_msgSender()].receiveTime = block.timestamp/86400;
