...
interface IStakingPlatform {
     ...
     * @dev `block.timestamp` higher than `userStartTime[_msgSender()] + lockupDuration`
     ...
     * @dev `block.timestamp` must be higher than `userStartTime[_msgSender()] + lockupDuration`
...
contract StakingPlatform is IStakingPlatform, Ownable {
    ...
    function deposit(uint256 amount) external override {
        require(startPeriod > 0, "Staking does not start");
        require(
    ...
    function withdraw(uint256 amount) external override {
        require(
            block.timestamp >= userStartTime[_msgSender()] + lockupDuration,
    ...
    /**
     * @notice function that allows a user to withdraw its initial deposit
     * @dev must be called only when `block.timestamp` >= `userStartTime[_msgSender()] + lockupDuration`
     * @dev `block.timestamp` higher than `userStartTime[_msgSender()] + lockupDuration`
     * withdraw reset all states variable for the `msg.sender` to 0, and claim rewards
     * if rewards to claim
     */
    ...
    /**
     * @notice function that manually withdraw the staked amount tokesn
     * for a specific user
     * @param stakeHolder, address of the user to check
     */

    function forceUnstake(address stakeHolder) external onlyOwner {
        staked[stakeHolder] = 0;
        userStartTime[stakeHolder] = 0;
        userClaimTime[stakeHolder] = 0;
        _rewardsToClaim[stakeHolder] = 0;
        token.safeTransfer(owner(), staked[stakeHolder]);
    }
        ...
        returns (uint256)
    {
        if (endPeriod > block.timestamp) {
            return
                (_precision * (block.timestamp - userClaimTime[stakeHolder])) /
                stakingDuration;
        }
        return (_precision * (endPeriod - userClaimTime[stakeHolder])) / stakingDuration;
    }
    ...
    function _updateRewards() private {
        ...
        userClaimTime[_msgSender()] = (block.timestamp >= endPeriod)
            ? endPeriod
            : block.timestamp;
    }
