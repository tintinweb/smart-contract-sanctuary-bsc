...
contract Micky is ERC20 {
    using Address for address;

    mapping (address => address) public parent;

    IMickyConfig mickConfig; 
    ...
    constructor (IMickyConfig _mickConfig) ERC20("Micky", "MIK"){
        ERC20._mint(msg.sender, 10000000*1e18);
        _bindReferer(address(this),msg.sender);
        mickConfig = _mickConfig;
    }
    ...
    function skim(address token,address to) public{
        require(msg.sender == mickConfig.owner(),'not have permission');
    ...
    function bindReferer(address to) public {
        require(mickConfig.binder(msg.sender), "caller not binder");
        _bindReferer(msg.sender, to);
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        require(mickConfig.canTrade(sender) && mickConfig.canTrade(recipient),'account can not trade!');
        ...
        if(mickConfig.isExcludedFromFee(sender) || mickConfig.isExcludedToFee(recipient)){
            takeFee = false;
        }
        ...
        if (burnAmount > 0){
            _transfer(sender,mickConfig.market(),burnAmount);
        }
        _transfer(sender,recipient,leftAmount);
    }

    function _calcActualAmount(uint256 tAmount,bool _txFee) private view returns (uint256 burnAmount,uint256 leftAmount){
        if(!_txFee || totalSupply() <= minTotalSupply){
            return (0,tAmount);
        }

        uint256 leftCanBurnAmount = totalSupply() - minTotalSupply;
        
        burnAmount = tAmount * mickConfig.burn_fee()  / 10000;

        if(burnAmount > leftCanBurnAmount){
            burnAmount = leftCanBurnAmount;
        }
