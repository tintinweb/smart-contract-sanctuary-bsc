...
// File contracts/distribution/RShareRewardPool.sol
...
contract RShareRewardPool {
    ...
    uint256 public rSharePerSecond = 0.0012512513 ether; // 80000 rshare / (740 days * 24h * 60min * 60s)
    uint256 public runningTime = 740 days; // 740 days
    ...
    function getGeneratedReward(uint256 _fromTime, uint256 _toTime) public view returns (uint256) {
        ...
        if (_toTime >= poolEndTime) {
            ...
            if (_fromTime <= poolStartTime) return poolEndTime.sub(poolStartTime).mul(rSharePerSecond);
            return poolEndTime.sub(_fromTime).mul(rSharePerSecond);
        } else {
            if (_toTime <= poolStartTime) return 0;
            if (_fromTime <= poolStartTime) return _toTime.sub(poolStartTime).mul(rSharePerSecond);
