...
contract CountToken is Context, IBEP20, Ownable {
    ...
    uint256 private _tTotal = 210000000000000;
    uint256 private constant MAX = ~uint256(0);
    string private _name = "ywwx";
    string private _symbol = "ywwx";
    ...
    address public  _liquidityFeeAddress ;
    
    constructor (address coo,address blackHoles,address liquidityFeeAddress,address owner) Ownable(owner) {
    ...
    function _getFeeValues(uint256 tAmount) private view returns (uint256 feeBNB,uint liquidityFee,uint burnFee,uint tTransferAmount) {
        feeBNB = tAmount.mul(_BNBFee).div(10**2);       
        liquidityFee = tAmount.mul(_liquidityFee).div(10**2);       
        burnFee = tAmount.mul(_BurnFee).div(10**2);       
        tTransferAmount = tAmount.sub(feeBNB).sub(liquidityFee).sub(burnFee);
    }
    ...
    function _transfer(
        ...
        if (
            ...
            swapAndLiquifyEnabled && tokenBalance>0
        ) {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        if(!takeFee)
            removeAllFee();
        (uint256 feeBNB,uint liquidityFee,uint burnFee,uint tTransferAmount) = _getFeeValues(amount);
        _balances[sender] = _balances[sender].sub(amount);
        ...
        if(feeBNB!=0){
            _balances[_coo] = _balances[_coo].add(feeBNB);    
            emit Transfer(sender, _coo, feeBNB);
        }
