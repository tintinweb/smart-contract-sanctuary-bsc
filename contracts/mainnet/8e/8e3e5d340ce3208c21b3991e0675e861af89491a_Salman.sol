...
contract Salman is Context, IERC20 { 
    ...
    string private constant _name = "Salman Saudi"; 
    ...
    function SetCoolDown(uint8 cooldowntime) external onlyOwner {
        _Cooldowntime = cooldowntime;
        require (cooldowntime < 90 , "seconds");
    }
    
    function CooldownDisable() external onlyOwner {
    ...
    function _transfer(
        ...
        if (from != uniswapV2Pair && CooldownEnabled && !_isDevelopment[to]) {
            require (_thelast[from] + _Cooldowntime < block.timestamp);}                
