...
// interface LPMining {
//     function updataUserReward(uint pid_, address addr_)external view returns (uint);
//     function poolInfo(uint256) view external returns (bool,bool,address,address,address,uint256,uint256,uint256,uint256,uint256,uint256);
//     function getPool(uint , address ) external view returns(bool , bool ,
//                                                     uint ,
//                                                     uint ,
//                                                     address[3] memory ,
//                                                     uint[2] memory ,
...
contract check {
    ...
    function getLpInfo(uint pid_) public view returns(bool status, uint lpTVL, uint poolOutput, uint decimals){
        uint startTime;
        uint endTime;
        uint dailyOut;
        IBEP20 outPutToken;

        (status,,,,outPutToken,dailyOut,,,lpTVL,startTime,endTime) = LP.poolInfo(pid_);

        // (status,lockToTime,levelLp,lpToken,outPutToken,dailyOut,lastRewardTime,debtInPool,lpTVL,startTime,endTime)
        // (isOpen,isLock,dailyOut,TVL,poolAddressList,poolTimeList,poolBonusCoe,poolLevelAmount,symbelList,invitor,userList) = LP.getPool(pid_, address(this));

        uint rate = dailyOut / 1 days;

        if (endTime >= block.timestamp){
            poolOutput = rate * (block.timestamp - startTime);
        } else if (endTime < block.timestamp){
            poolOutput = rate * (endTime - startTime);
        }
        decimals = ERC20(address(outPutToken)).decimals();
    }

    function getLpUserInfo(uint pid_, address addr_) public view returns(uint amount, uint claimed, uint toClaim){

        (amount, , , claimed, ,) = LP.userInfo(pid_, addr_);

        toClaim = LP.updataUserReward(pid_, addr_);
    }
}
        ...
        if (addressCoin != addressUsdt){
            ...
            if (level_ == 2) {
                if (levelOne[pid_][msg.sender]) {
                    require(pool.upgradeAmount[1] > pool.upgradeAmount[1], "level 2 is null");
                    _amountIn = pool.upgradeAmount[1] - pool.upgradeAmount[1];
                } else {
