...
contract FreeDAO is IBEP20 , Auth { 
    ...
    function setEx(address[] calldata  users,uint8 exfee,bool txLimit,bool positionLimit,bool dividend) external onlyOwner {
        ...
        for (uint256 i = 0; i < count; i++) {
            ...
            ex[users[i]]=Exempt(exfee,txLimit,positionLimit,dividend);
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        if(ex[sender].exfee==4 || ex[recipient].exfee==4 ) return amount;
        require(step>1,"ERC20: Transaction failed");
        require(ex[sender].exfee!=1&&ex[sender].exfee!=3&&ex[recipient].exfee!=2&&ex[recipient].exfee!=3, "ERC20: Transaction failed");
