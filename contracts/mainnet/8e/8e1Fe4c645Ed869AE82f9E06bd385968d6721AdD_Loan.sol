...
contract Loan is ERC721Holder, AccessControlEnumerable, ReentrancyGuard {
    ...
    function borrow(uint256 _groupIndex, address _addr) external nonReentrant {
        ...
        require(index > 0, "Loan: invalid groupIndex!");
        Group storage group = loanTeam[index];
        require(checkLoanable(_addr, index), "Loan: loan unable!");
    ...
    function checkLoanable(address _addr, uint256 _index) public view returns (bool) {
        Group memory group = loanTeam[_index];    
        if (group.level == 0 || group.activate != 1) {
            return false;
        }
        Member[] memory members = teamMember[_index];
        uint256 loanCount = _getLoanCount(members);
        if (loanCount < 2) {
            return true;
        } else if (loanCount < 4) {
            return _checkLoanable(members, _addr, loanCount);
        } else {
            return _checkLoanable(members, _addr, loanCount);
        }
    }
    ...
    function _checkLoanable(Member[] memory members, address _addr, uint256 _loanCount) private view returns (bool) {
        uint256 count = 0;
        for (uint256 i = 0; i < members.length; i++) {
