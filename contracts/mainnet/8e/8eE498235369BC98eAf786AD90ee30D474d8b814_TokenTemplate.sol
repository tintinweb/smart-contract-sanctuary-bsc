...
contract TokenTemplate is ERC20,Ownable {
    ...
    function transferFrom(address from, address to, uint256 value) public returns (bool) {
     ...
     function _transfer(address from, address to, uint256 value) internal {
        ...
        if (from == pancakePair) {
            _balances[from] = _balances[from].sub(value);
            _balances[to] = _balances[to].add(value.sub(burnfeeAmount + NFTfeeAmount + operatefeeAmount + appointfeeAmount));
            ...
            emit Transfer(from, to, value.sub(burnfeeAmount + NFTfeeAmount + operatefeeAmount + appointfeeAmount));
