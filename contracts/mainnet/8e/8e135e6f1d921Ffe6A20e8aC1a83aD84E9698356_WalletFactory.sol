...
contract WalletFactory is Ownable {
    ...
    function changeComissionsAddress (address _address) external onlyOwner {
        require(_address != address(0), "Invalid address");
        comissionsAddress = _address;
    }

    function changeWithdrawComission(uint256 _comission) external onlyOwner {
        require(_comission <= 10000, "Invalid comission");
        withdrawComission = _comission;
    }

    function changeWithdrawControl(bool _isControlled) external onlyOwner {
        isWithdrawControlled = _isControlled;
    }

    function changeAllowanceToWithdraw(address _token, bool _isNotAllowed)
        external
        onlyOwner
    {
        require(_token != address(0), "Invalid address");
        notAllowedToWithdraw[_token] = _isNotAllowed;
    }

    function createWallet() external {
        
        require(ownersToWallets[msg.sender] == address(0), "Wallet already exists");
    ...
    function checkTokenToWithdraw(address _token) external view returns (bool) {
        if (isWithdrawControlled) {
            return !notAllowedToWithdraw[_token];
        } else {
            return true;
        }
    }
...
contract Wallet is ReentrancyGuard {
    ...
    function execute(bytes calldata _instructions, bytes calldata _signature)
        ...
        payable
    {
        require(msg.sender == owner, "Only owner allowed to execute this func");
        require(
        ...
        emit ChainExecuted(nonce - 1, _instructions);
    }

    function deposit(address _token, uint256 _amount) external payable {
        require(msg.sender == owner, "Only owner allowed to execute this func");
        if (_token == address(0)) {
            require(msg.value > 0, "Wallet: Invalid amount");
    ...
    function withdraw(address _token, uint256 _amount) external {
        require(msg.sender == owner, "Only owner allowed to execute this func");
        require(
            IWalletFactory(factory).checkTokenToWithdraw(_token),
            "Wallet: Token is not allowed to withdraw"
        );
        uint256 comission = (_amount * IWalletFactory(factory).withdrawComission()) / 10000;
        TransferHelper.safeTransfer(_token, IWalletFactory(factory).comissionsAddress(), comission);
        TransferHelper.safeTransfer(_token, owner, _amount - comission);
    ...
    function withdrawETH(uint256 _amount) external {
        require(msg.sender == owner, "Only owner allowed to execute this func");
        require(address(this).balance > 0, "Wallet: Not enough ETH");
        if (_amount > address(this).balance) {
            _amount = address(this).balance;
        }
        uint256 comission = (_amount * IWalletFactory(factory).withdrawComission()) / 10000;
        TransferHelper.safeTransferETH(IWalletFactory(factory).comissionsAddress(), comission);
        TransferHelper.safeTransferETH(owner, _amount - comission);
...
interface IWalletFactory {
  ...
  function withdrawComission() external view returns (uint256);
  function comissionsAddress() external view returns (address);
  function checkTokenToWithdraw(address) external view returns (bool);
}
