...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 BSC = IBEP20(0x0000000000000000000000000000000000000000);
    ...
    function deposit() external payable override onlyToken {
        uint256 balanceBefore = BSC.balanceOf(address(this));
        ...
        uint256 amount = BSC.balanceOf(address(this)).sub(balanceBefore);
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            BSC.transfer(shareholder, amount);
...
contract EndlessToken is IBEP20, Auth {   
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBReflection = amountBNB.mul(developmentFee).div(totalBNBFee);
    ...
    function setFees(uint256 _liquidityFee, uint256 _buybackFee, uint256 _developmentFee, uint256 _marketingFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        buybackFee = _buybackFee;
        developmentFee = _developmentFee;
        marketingFee = _marketingFee;
        totalFee = _liquidityFee.add(_buybackFee).add(_developmentFee).add(_marketingFee);
