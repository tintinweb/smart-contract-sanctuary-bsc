...
contract StratosphereV2 is ERC2771Recipient, IERC20, Ownable, Janitable {
    ...
    function addBNBToReward() public payable onlyJanitorOrOwner {
        require(msg.value >= 0);
        _BNBRewards = _BNBRewards + msg.value;
    ...
    function swap(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if (tokensForLiquidity < contractTokenBalance) {
            ...
            uint256 acquiredBNB = address(this).balance - initialBalance;
            // calculate share for liquidity or rewards
            uint256 bscForLiquidity = (acquiredBNB * tokensForLiquidity) /
                tokensToSell;
            uint256 bscForRewards = acquiredBNB - bscForLiquidity;
            // update rewards
            _BNBRewards = _BNBRewards + bscForRewards;
            // add liquidity
    ...
    function swapTokensForBNB(uint256 tokenAmount) private {
        // Generate the bscswap pair path of token -> BNB
    ...
    function rewards(address recipient) public view returns (uint256) {
        ...
        uint256 brut = (_BNBRewards * balanceOf(recipient)) / total;
    ...
    function claim() public {
        ...
        if (_boughtTotal > 0) {
            uint256 total = _tTotal -
                balanceOf(0x000000000000000000000000000000000000dEaD);
            uint256 brut = (_BNBRewards * balanceOf(_msgSender())) / total;
            ...
            emit ClaimedBNB(_msgSender(), toclaim);
        }
    }
    ...
    function claimToken(address token) public {
        ...
        if (_boughtTotal > 0) {
            ...
            uint256 toclaim = brut - _claimed[_msgSender()];
            _claimed[_msgSender()] = _claimed[_msgSender()] + toclaim;
            uint256 tokenAmount = swapBNBForToken(toclaim, _msgSender(), token);
