...
contract BoxOpen is VRFConsumerBaseV2, Context, Ownable, ReentrancyGuard {
    ...
    uint8[COLLECTION_SIZE] public nftTypes;
    ...
    function loadNftTypes(uint8[] memory types)
        ...
        onlyOwner
    {
        require(types.length == COLLECTION_SIZE,"Types array must have the same length with the collection");
    ...
    function openBox(uint8 boxId) public nonReentrant {
        ...
        nftAddress.safeMint(origin, getNftTypeById(boxOpenCount));
        emit BoxOpened(getNftTypeById(boxOpenCount));
        boxOpenCount++;
    }
