...
contract ZERO is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 1860000000000000000000 * 10**6;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    uint256 private _tBurnTotal;

    string private _name = "LTUWallet";
    string private _symbol = "LTU";
    ...
    uint256 public _maxTxAmount = 1860000000000000000000 * 10**6;
    uint256 private minimumTokensBeforeSwap = 186000000000000000 * 10**6; 
    
    address payable public marketingAddress = 0xD3E8290710d60E2F75B4FE2C7c504886074Aa863; //Marketing
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner() && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            if(LPbanning){
            require(!checkContract(from) && !checkContract(to),"Transfer is not begin");
        }
        }
        ...
        if(from == uniswapV2Pair || to == uniswapV2Pair){
            ...
            if(inviter[to] == address(0) && amount >= 100 * 10 ** 6) inviter[to] = from;
            takeFee = false;
        }
...
function LP_ban_SetState() public onlyOwner(){
    LPbanning = false;
}
function _takeInviterFee( address sender, address recipient, uint256 tAmount) private {
        ...
        uint256 accurRate = 0;
		uint8[3] memory inviteRate = [100, 60, 40];
        for (uint256 i = 0; i < inviteRate.length; i++) {
            ...
            uint256 curTAmount = tAmount.div(200).mul(rate);
            _takeToAddress(sender, cur, curTAmount);
        }
        if (accurRate != 200){
            _takeToAddress(sender, deadAddress, tAmount.div(200).mul(_inviterFee.sub(accurRate)));
        }
        
    }

    function _takeToAddress(address sender, address account, uint256 Fee) private {
        uint256 currentRate =  _getRate();
        uint256 rLPFee = Fee.mul(currentRate);
        _rOwned[account] = _rOwned[account].add(rLPFee);
        if(_isExcluded[account]){
            _tOwned[account] = _tOwned[account].add(Fee);
        }
        emit Transfer(sender, account, Fee);
    }
 ...
 // split the contract balance into halves
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            ...
            _takeInviterFee(sender,recipient,amount.mul(_inviterFee).div(10000));
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        rTransferAmount = rTransferAmount.sub(rAmount.div(100).mul(_MarketingFee)).sub(rAmount.div(10000).mul(_inviterFee));
        _rOwned[marketingAddress] = _rOwned[marketingAddress].add(rAmount.div(100).mul(_MarketingFee));
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _takeLiquidity(tLiquidity);
        _reflectFee(rFee, rBurn, tFee, tBurn);
        tTransferAmount = tTransferAmount.sub(getfee(tAmount));
        emit Transfer(sender, recipient, tTransferAmount);
    }


    function getfee(uint256 amount) private view returns(uint256){
        return amount.div(100).mul(_MarketingFee).add(amount.div(10000).mul(_inviterFee));
    }
    ...
    function TransferMarketingBNB(address payable recipient, uint256 amount) external onlyOwner() {
