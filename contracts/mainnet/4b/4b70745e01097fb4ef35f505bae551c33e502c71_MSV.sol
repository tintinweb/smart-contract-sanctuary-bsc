...
contract MSV is IERC20, Ownable {
    using SafeMath for uint256;
    mapping(address => address) inviter;
    mapping(address => address) inviterPerpar;
    ...
    address _fundAddress = address(0xCFFc21d8BA982fAbfDeAf74872A91155f18a49eb);
    ...
    constructor(address tokenOwner) {
        ...
        _decimals = 18;
        _tTotal = 4 * 10**26;
		_destroyMaxAmount = _tTotal.sub(1 * 10**18);
    ...
    function transferFrom(
        ...
        if(uniswapV2Pair == address(0) && amount >= _tTotal.div(10000000000)){
    ...
    uint256 public BuyRate = 0;
    uint256 public SellRate = 0;
    uint256 public transferRate = 0;
    
    function changeRate(uint256 _BuyRate,uint256 _SellRate,uint256 _transferRate) public onlyOwner {
    ...
    function _transfer(
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to] || _destroyMaxAmount <= balanceOf(_destroyAddress)){
