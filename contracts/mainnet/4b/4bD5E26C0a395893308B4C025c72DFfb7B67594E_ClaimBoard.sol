...
contract ClaimBoard is Ownable {
    ...
    event AllocationRemoved(address, uint256);
     ...
     **/
    constructor(address _mrhbToken) {
        //The vesting schedule below is a temporary set of values which is bound to change according to the tokenomics
        mrhbToken = IERC20(_mrhbToken);
        
        // Seed Round
        nonLinearUnlocks[0] = [
            [uint256(0), 29], // Month 1
            [uint256(106), 1], // 1% 

            [uint256(0), 29], // Month 2 
            [uint256(0), 1], // 0%

            [uint256(0), 29], // Month 3 
            [uint256(106), 1], // 1.06%      

            [uint256(0), 29], // Month 4 
            [uint256(106), 1], // 1.06%

            [uint256(0), 29], // Month 5 
            [uint256(163), 1], // 1.06%

            [uint256(0), 29], // Month 6 
            [uint256(1596), 1], // 15.96%

            [uint256(0), 29], // Month 7
            [uint256(106), 1], // 1.06%

            [uint256(0), 29], // Month 8
            [uint256(106), 1], // 1.06%

            [uint256(0), 29], // Month 9
            [uint256(106), 1], // 1.06%

            [uint256(0), 29], // Month 10
            [uint256(106), 1], // 1.06%

            [uint256(0), 29], // Month 11
            [uint256(106), 1], // 1.06%

            [uint256(0), 29], // Month 12
            [uint256(1070), 1], // 10.7%

            [uint256(0), 29], // Month 13
            [uint256(319), 1], // 3.19%

            [uint256(0), 29], // Month 14
            [uint256(319), 1], // 3.19%

            [uint256(0), 29], // Month 15
            [uint256(213), 1], // 2.13%

            [uint256(0), 29], // Month 16
            [uint256(1070), 1], // 10.7%

            [uint256(0), 29], // Month 17
            [uint256(319), 1], // 3.19%

            [uint256(0), 29], // Month 18
            [uint256(319), 1], // 3.19%

            [uint256(0), 29], // Month 19
            [uint256(319), 1], // 3.19%

            [uint256(0), 29], // Month 20
            [uint256(1070), 1], // 10.7%

            [uint256(0), 29], // Month 21
            [uint256(319), 1], // 3.19%

            [uint256(0), 29], // Month 22
            [uint256(319), 1], // 3.19%

            [uint256(0), 29], // Month 23
            [uint256(213), 1], // 2.13%

            [uint256(0), 29], // Month 24
            [uint256(2000), 1] // 20 %            
        ];

        // Private WL & Exclusive Entry Round
         nonLinearUnlocks[1] = [
            [uint256(0), 29], // Month 1
            [uint256(0), 1], // 0 %
        ...
        // Strategic Round
         nonLinearUnlocks[2] = [
            ...
            [uint256(0), 15], // Month 1.5
            [uint256(1081), 1], // 10.81%

            [uint256(0), 15], // Month 2
            [uint256(1081), 1], // 10.81%

            [uint256(0), 15], // Month 2.5
            [uint256(1081), 1], // 10.81%

            [uint256(0), 15], // Month 3
            [uint256(1081), 1], // 10.81%

            [uint256(0), 15], // Month 3.5
            [uint256(1081), 1], // 10.81%

            [uint256(0), 15], // Month 4
            [uint256(1081), 1], // 10.81%

            [uint256(0), 15], // Month 4.5
            [uint256(1081), 1], // 10.81%

            [uint256(0), 15], // Month 5
            [uint256(1081), 1], // 10.81%

            [uint256(0), 15], // Month 5.5
            [uint256(1081), 1], // 10.81%

            [uint256(0), 15], // Month 6
            [uint256(1081), 1] // 10.81%
        ];
        ...
        nonLinearUnlocks[3] = [
            ...
            [uint256(0), 29], // Month 25
            [uint256(600), 1], // 6%

            [uint256(0), 29], // Month 26
            [uint256(300), 1], // 3%

            [uint256(0), 29], // Month 27
            [uint256(500), 1], // 5%

            [uint256(0), 29], // Month 28
            [uint256(300), 1], // 3%

            [uint256(0), 29], // Month 29
            [uint256(600), 1], // 6%

            [uint256(0), 29], // Month 30
            [uint256(300), 1], // 3%

            [uint256(0), 29], // Month 31
            [uint256(500), 1], // 5%

            [uint256(0), 29], // Month 32
            [uint256(300), 1], // 3%

            [uint256(0), 29], // Month 33
            [uint256(500), 1], // 5%

            [uint256(0), 29], // Month 34
            [uint256(300), 1], // 

            [uint256(0), 29], // Month 35
            [uint256(300), 1], // 3%

             [uint256(0), 29], // Month 36
            [uint256(500), 1] // 5%
        ];

        // 0: PreSeed/Seed 30.00% - 300,000,000 - cliff defined in nonLinearUnlocks, non linear schedule defined above
        vestingTypes.push(VestingType(0, 0 days, true, 3000, 0));

        // 2: Private WL & Exclusive Entry 9.00% - 90,000,000, cliff defined in nonLinearUnlocks, non linear schedule defined above
        vestingTypes.push(VestingType(0, 0 days, true, 900, 0));

        // 3: Strategic 3% - 30,000,000,  cliff defined in nonLinearUnlocks,  non linear schedule defined above
        vestingTypes.push(VestingType(0, 0 days, true, 300, 0));

        // 4: Team and Advisor 20% - 200,000,000,  cliff defined in nonLinearUnlocks,  non linear schedule defined above
        vestingTypes.push(VestingType(0, 0 days, true, 2000, 0));

        // 5: Partnerships/Growth  7.00% - 70,000,000 - 72 months @ rate of 1.38% per month 
        vestingTypes.push(VestingType(1380000000000000000, 0 days, false, 700, 0));

        // 6: Ecosystem  30.00% - 300,000,000 - 72 months @ 1.38% per month
        vestingTypes.push(VestingType(1380000000000000000, 0 days, false, 3000, 0));
    }

    // Vested tokens wont be available before the listing time
    function getListingTime() public pure returns (uint256) {
        return 1640178000; //Tue Dec 22 2021 13:00:00 GMT+0000
    }
    ...
    function addAllocations(
        ...
        require(vestingTypes[vestingTypeIndex].totalAllocated
                <= (mrhbToken.totalSupply()*vestingTypes[vestingTypeIndex].percent)/10000, "Can not allocate more then round limit");
        ...
        require(mrhbToken.balanceOf(address(this)) >= totalAllocations, "Not enough contract balance.");
        return true;
    }
    ...
    // function updateVestingWallet(uint256 vestingTypeIndex, address wallet, address newWallet) external onlyOwner {
    //     require(
    //         vestingWallets[vestingTypeIndex][wallet].totalAmount != 0,
    //         "Vesting wallet not found"
    //     );

    //     require(wallet != address(0x0), "Invalid new address!");
    //     require(newWallet != address(0x0), "Invalid new address!");

    //     VestingWallet memory vestingWallet = vestingWallets[vestingTypeIndex][wallet];
    //     vestingWallet.wallet = newWallet;

    //     vestingWallets[vestingTypeIndex][wallet] = vestingWallet;
    // }

    function resetVestingWallet(uint256 vestingTypeIndex, address wallet) external onlyOwner {
        require(
            vestingWallets[vestingTypeIndex][wallet].totalAmount != 0,
            "Vesting wallet not found"
        );
        ...
        // Existing Amount before resetting
        uint256 totalAmount = vestingWallet.totalAmount;
        delete vestingWallets[vestingTypeIndex][wallet];
        emit AllocationRemoved(wallet, totalAmount);
    }
    ...
    function calculateNonLinear(uint256 _days, uint256 amount, uint vestingTypeIndex)
        ...
        returns (uint256)
    {
        require(vestingTypeIndex == 0 || vestingTypeIndex == 1 || vestingTypeIndex == 2 || vestingTypeIndex == 3, "Invalid vesting type");
    ...
    /**
    Returns stats against wallet address i.e
     ...
     */
    function getWalletStats(address walletAddress) external view returns (uint256, uint256, uint256, uint256){
        uint256 totalAllocated = 0;

        for(uint256 i = 0; i < vestingTypes.length;  i++){
            // calculating total allocations
            totalAllocated+= vestingWallets[i][walletAddress].totalAmount;
        }
        // total amount claimed by wallet address
        uint256 totalClaimed = claimed[walletAddress];
        // total unclaimed amount ready to be claimed
        uint256 totalClaimable = getunclaimed(walletAddress);
        // total unclaimed amount but locked
        uint256 totalUnclaimedLocked = totalAllocated - totalClaimed;

        return (totalAllocated, totalClaimed, totalClaimable, totalUnclaimedLocked);
    }
     ...
     */    
     function getRestAmount(address sender) external view returns (uint256) {
        ...
        return totalAmount - this.getUnlockedVestingAmount(sender) - claimed[sender];
    }
     ...
     * @param sender a vesting wallet.
     */
    function getunclaimed(address sender) public view returns (uint256) {
    ...
    function claim(uint256 amount) external {
        ...
        mrhbToken.transfer(msg.sender,amount);
        emit Claimed(msg.sender, amount);
    }

    function withdrawAndDestroy() external onlyOwner {
        mrhbToken.transfer(msg.sender, mrhbToken.balanceOf(address(this)));
    }
}
