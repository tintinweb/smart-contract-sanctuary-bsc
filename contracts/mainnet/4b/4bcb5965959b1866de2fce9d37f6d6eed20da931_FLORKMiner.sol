...
contract FLORKMiner {
    address florkt = 0xf789a5E0694C0c58e351d3D9E53a54c0F15D681e;
    uint256 public SECONDS_WORK_MINER = 1728000; 
    uint256 PSN = 10000;
    uint256 PSNH = 5000;
    uint256 BONUS_10_FLORK = 10000000000000000000; // bonus
...
struct User {
    ...
    function sellTokens(uint256 amountSell) public {
            ...
            for (uint256 i = 0; i < 7; i++) {
                if (upline != address(0)) {
                    if (users[upline].invest >= REFERRAL_MINIMUM[i] || go == true) {
                        ...
                        if (amount4 > 0) {
                            ERC20(florkt).transfer(upline, amount4);
                        }
                    }
        ...
        ERC20(florkt).transfer(timer, fee);
        ERC20(florkt).transfer(Delevoper, fee2);
        ERC20(florkt).transfer(msg.sender, SafeMath.sub(amountSell, (fee + fee2)));
    ...
    function buyMiners(address ref, uint256 amount) public {
        ...
        ERC20(florkt).transferFrom(address(msg.sender), address(this), amount);
        uint256 balance = ERC20(florkt).balanceOf(address(this));
    ...
    function getFreeMiners_10USDT() public {    
        ...
        lastHatch[msg.sender] = now; 
        usersMiner[msg.sender] = SafeMath.add(SafeMath.div(calculateTokensBuySimple(BONUS_10_FLORK),SECONDS_WORK_MINER),usersMiner[msg.sender]);
    ...
    function calculateTokensSell(uint256 tokens) public view returns (uint256) {
        return calculateTrade(tokens, marketTokens, ERC20(florkt).balanceOf(address(this)));
    }
    ...
    function calculateTokensBuySimple(uint256 eth) public view returns (uint256) {
        return calculateTokensBuy(eth, ERC20(florkt).balanceOf(address(this)));
    }
    ...
    function getBalance() public view returns (uint256) {
        return ERC20(florkt).balanceOf(address(this));
    }
