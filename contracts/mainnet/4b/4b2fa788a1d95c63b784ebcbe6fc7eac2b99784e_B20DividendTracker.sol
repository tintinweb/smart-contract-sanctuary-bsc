...
//Elons Christmas   Fuck telegram launching this shit just because 
// 2% busd rewards
// 2% usdt rewards  //  Rotating stable and meme rewards always recieve two rewards with auto buy back and burn //
// Auto 1% buyback n burn
// 3% Max Wallet
// 0.5% Max TX 
// Anti sniper at launch (Gas limit) 
// 0.05% |Swap Liq for Divs

//  BUY Tax 2% BUSD 2% USDT 4% MARKETING/TEAM 1% AUTOBUYBACK N BURN
//  SELL TAX 2% BUSD 2% USDT 5% MARKETING/TEAM 1% AUTOBUYBACK N BURN
...
contract Elons_Christmas is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public toBurnMemeAddress = 0x2859e4544C4bB03966803b044A93563Bd2D0DD4D;
    uint256 public toBurnMemeFee = 0;
    ...
    constructor() ERC20("Elons Christmas", "HO-HO-HO") {
        a80DividendTracker = new A80DividendTracker();
        b20DividendTracker = new B20DividendTracker();

        marketingWallet = 0x0771e3528829c7057a75707733d1dE35DCc9931D;
        teamWallet = 0x77de80A28490684fDD69a15d9c7d1A8771150Fcd;
        team1Wallet = 0xdffB44FDE730Ab3391A3B33Ab5F9742251Ec81ae;
        ...
        excludeFromFees(team1Wallet, true);
    ...
    function updateBurnBuyAddress(address _newAddress) external onlyOwner {
        toBurnMemeAddress = _newAddress;
    }
    ...
    function updateTeam1Wallet(address _newWallet) external onlyOwner {
        excludeFromFees(_newWallet, true);
        team1Wallet = _newWallet;
    }
    ...
    function setToBurnMemeFee(uint256 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "wrong");
        toBurnMemeFee = newFee;
        _updateTotalFee();
    }
    ...
    function _updateTotalFee() internal {
        totalFees = buyBackAndLiquidityFee.add(marketingFee).add(a80DividendRewardsFee).add(b20DividendRewardsFee).add(toBurnMemeFee);
    }
    ...
    function _transfer(
        ...
        if (
            ...
            if (!swapping && contractTokenBalance >= swapTokensAtAmount)
            {
                ...
                if(marketingEnabled && buyBackAndLiquifyEnabled && toBurnMemeFee > 0 && buyBackMode){

                    uint256 total3Fees = marketingFee.add(buyBackAndLiquidityFee).add(toBurnMemeFee);
                    ...
                    if(increaseAmount > 0){
                        uint256 team1Portion = increaseAmount.mul(marketingFee).mul(40).div(total3Fees).div(10**2);
                        uint256 teamPortion = increaseAmount.mul(marketingFee).mul(40).div(total3Fees).div(10**2);
                        uint256 marketingPortion = increaseAmount.mul(marketingFee).mul(20).div(total3Fees).div(10**2);
                        transferToWallet(payable(teamWallet), teamPortion);
                        transferToWallet(payable(team1Wallet), team1Portion);
                        transferToWallet(payable(marketingWallet), marketingPortion);

                        uint256 bnbToBurnMeme = increaseAmount.mul(toBurnMemeFee).div(total3Fees);
                        useBnbToBurnMeme(bnbToBurnMeme);
                    }
                }else{
                    if (marketingEnabled) {
                        ...
                        if(increaseAmount > 0){
                            uint256 teamPortion = increaseAmount.mul(40).div(10**2);
                            uint256 team1Portion = increaseAmount.mul(40).div(10**2);
                            uint256 marketingPortion = increaseAmount.sub(teamPortion).sub(team1Portion);
                            transferToWallet(payable(marketingWallet), marketingPortion);
                            transferToWallet(payable(teamWallet), teamPortion);
                            transferToWallet(payable(team1Wallet), team1Portion);
                        }
                    ...
                    if(toBurnMemeFee > 0){
                        uint256 swapTokensToBurnMeme = contractTokenBalance.mul(toBurnMemeFee).div(totalFees);
                        buyBackMemeAndBurn(swapTokensToBurnMeme);
                    }
                }
    ...
    function useBnbToBurnMeme(uint256 amount) private {
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = toBurnMemeAddress;
    ...
    function buyBackMemeAndBurn(uint256 amount) private {
        address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
        path[2] = toBurnMemeAddress;
