        ...
        while(iterations < automateCount) {
            ...
            if(hasCubic > 0){
                if ((block.timestamp - automations[adr].lastrun) >= 3600) {  //86400=24hrs, 3600=1hr
                    uint256 autoCubic = hasCubic.mul(AUTOTAX).div(PERCENTS_DIVIDER);
                    uint256 cubicValue = calculateCubicSell(autoCubic);
                    token_BUSD.transfer(autoadr, cubicValue);
                    users[adr].claimedCubic = users[adr].claimedCubic.sub(autoCubic);
