...
contract InukoCoin is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address payable public marketingAddress =
        payable(0xA046320E88988cC8122d9ab0E89834B3157dd93C); // Marketing Address

    address payable public buyAddress =
        payable(0xb0757D35e673a04e0D9899c8F98f74050555B402); // BuyBack Address

    address payable public liquidityAddress =
        payable(0xFC85019B34E2055667879bfFeB6d2Ef1148Ea24A); // Liquidity Address
    ...
    string private constant _name = "InukoCoin";
    string private constant _symbol = "$INUKO";
    ...
    uint256 public _buyBuyback = 0;

    uint256 public _sellTaxFee = 2;
    uint256 public _sellLiquidityFee = 2;
    uint256 public _sellMarketingFee = 3;
    uint256 public _sellBuyback = 3;
    ...
    uint256 private _buyTokensToSwap;
    ...
    event UpdatedBuyAddress(address buyback);
    ...
    constructor() {
        ...
        maxTransactionAmount = _tTotal * 10 / 1000; // 5% maxTransactionAmountTxn
        maxWalletAmount = _tTotal * 10 / 1000; // 5% maxWalletToken
        ...
        _isExcludedFromFee[buyAddress] = true;
    ...
    function _transfer(
     ...
     if(limitsInEffect){
        ...
        uint256 totalTokensToSwap = _liquidityTokensToSwap + _marketingTokensToSwap + _buyTokensToSwap;
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            ...
            if (automatedMarketMakerPairs[from]) {
                removeAllFee();
                _taxFee = _buyTaxFee;
                _liquidityFee = _buyLiquidityFee + _buyMarketingFee + _buyBuyback;
                buyOrSellSwitch = BUY;
            }
            // Sell
                ...
                removeAllFee();
                _taxFee = _sellTaxFee;
                _liquidityFee = _sellLiquidityFee + _sellMarketingFee + _sellBuyback;
    ...
    function swapBack() private lockTheSwap {
        ...
        uint256 totalTokensToSwap = _liquidityTokensToSwap.add(_buyTokensToSwap).add(_marketingTokensToSwap);
        ...
        uint256 bnbForBuy = bnbBalance.mul(_buyTokensToSwap).div(totalTokensToSwap);

        uint256 bnbForLiquidity = bnbBalance.sub(bnbForMarketing).sub(bnbForBuy);

        _liquidityTokensToSwap = 0;
        _marketingTokensToSwap = 0;
        _buyTokensToSwap = 0;

        (bool success,) = address(buyAddress).call{value: bnbForBuy}("");
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        if(buyOrSellSwitch == BUY){
            ...
            _buyTokensToSwap += tLiquidity * _buyBuyback / _liquidityFee;
            ...
            _buyTokensToSwap += tLiquidity * _sellBuyback / _liquidityFee;
    ...
    function setBuyFee(uint256 buyTaxFee, uint256 buyLiquidityFee, uint256 buyMarketingFee, uint256 buyBuyback)
        external
        onlyOwner
    {
        ...
        _buyBuyback = buyBuyback;
        require(_buyTaxFee + _buyLiquidityFee + _buyMarketingFee + _buyBuyback <= 20, "Must keep taxes below 20%");
        emit SetBuyFee(buyMarketingFee, buyLiquidityFee, buyTaxFee, buyBuyback);
    }

    function setSellFee(uint256 sellTaxFee, uint256 sellLiquidityFee, uint256 sellMarketingFee, uint256 sellBuyback)
        external
        onlyOwner
    {
        ...
        require(_sellTaxFee + _sellLiquidityFee + _sellMarketingFee + _sellBuyback <= 30, "Must keep taxes below 30%");
        emit SetSellFee(sellMarketingFee, sellLiquidityFee, sellTaxFee, sellBuyback);
    }
    ...
    function setBuyAddress(address _buyAddress) external onlyOwner {
        require(_buyAddress != address(0), "_buyAddress address cannot be 0");
        buyAddress = payable(_buyAddress);
        _isExcludedFromFee[buyAddress] = true;
        emit UpdatedBuyAddress(buyAddress);
    }
