...
// File contracts/rmai/RmaiPoolFactory.sol



pragma solidity ^0.8.4;
contract RmaiPool is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20Metadata;

    // The address of the smart chef factory
    address public RMAI_POOL_FACTORY;
    ...
    // The block number when pool ends.
    ...
    function initialize(
        ...
        require(msg.sender == RMAI_POOL_FACTORY, "Not factory");
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (pending > 0) {
            ...
            user.pending = pending - rewardTransferred;
        }
        ...
        if (_amount > 0) {
            ...
            if (depositFee > 0) {
                feeAmount = (_amount * depositFee) / 10000;
                if (feeAmount > 0) {
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (block.timestamp < user.lastDepositAt + lockPeriod) {
                require(isFlexibleLock, "Unable to withdraw when locked");
                uint256 withdrawFee = (_amount * unlockFee) / 10000;
                if (withdrawFee > 0) {
        ...
        onlyOwner
    {
        ...
        IERC20Metadata(_tokenAddress).safeTransfer(msg.sender, _tokenAmount);
    ...
    function pendingReward(address _user) public view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedSupply != 0) {
            ...
            adjustedTokenPerShare =
                accTokenPerShare +
                ((rewardAmount * PRECISION_FACTOR) / stakedSupply);
        }
    ...
    function _updatePool() internal {
        ...
        uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
        uint256 rewardAmount = multiplier * rewardPerBlock;
        accTokenPerShare += (rewardAmount * PRECISION_FACTOR) / stakedSupply;
        lastRewardBlock = block.number;
    }
...
contract RmaiPoolFactory is Ownable {
    uint16 public constant MAX_FEE = 2000;
    ...
    event NewRmaiPoolContract(address indexed smartChef);
    ...
    function deployPool(
        ...
        bytes memory bytecode = type(RmaiPool).creationCode;
        ...
        RmaiPool(smartChefAddress).initialize(
            _stakedToken,
            _rewardToken,
            _rewardPerBlock,
            _startBlock,
        ...
        RmaiPool(smartChefAddress).updateFeePercentage(
            defaultDepositFee,
            defaultUnlockFee
        );
        RmaiPool(smartChefAddress).updateLockProps(
            defaultLockPeriod,
            defaultIsFlexibleLock
        );
        RmaiPool(smartChefAddress).updateFeeAddress(defaultFeeAddress);
        RmaiPool(smartChefAddress).transferOwnership(_admin);

        emit NewRmaiPoolContract(smartChefAddress);
    }
}