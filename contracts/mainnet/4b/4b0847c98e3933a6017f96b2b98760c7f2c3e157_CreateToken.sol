...
contract CreateToken is Context, IERC20, IERC20Metadata,Ownable {
    ...
    function _transfer(
        ...
        if(sender==pairAddress&&takeFee){
            if(dexTaxBuyFee>0){
                uint256 taxFee = amount.mul(dexTaxBuyFee).div(10000);
                if(!is_swap_usdt){
                    _balances[taxBuyAddress] = _balances[taxBuyAddress].add(taxFee);
                    emit Transfer(sender, taxBuyAddress, taxFee);
                }else{
                     _balances[address(this)] = _balances[address(this)].add(taxFee);
                     emit Transfer(sender, address(this), taxFee);
                     swapTokensForUsdt(taxFee,taxBuyAddress);
                }
