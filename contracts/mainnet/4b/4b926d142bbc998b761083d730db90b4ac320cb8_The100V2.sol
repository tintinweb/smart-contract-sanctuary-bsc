...
The harder you hold, the more you get rewarded.
If you like rewards, don't sell.
Selling more than once in 24 hours or more than one token at a time will exclude you from receiving rewards.

For more details, join our Telegram group: @The100Token
*/
...
contract The100V2 is IBEP20, Auth {
    ...
    address public theMarketingWallet = 0x53e8054e66d730fa0B7069729601d05E3DD06969;
    address public theDevWallet = 0xe6497e1F2C5418978D5fC2cD32AA23315E7a41Fb;
    address public theBridgeWallet = 0x215E8855D5132C5268A3A6E715A38552F9c8D100;
    address public theProjectWallet = 0xf63ecA647cbBFFe771227C320348746452081337;
...
////////////////////////////////////////////////////////////////////// Some custom functions
    ...
    function stuffHappeningWhenSelling(address sender, uint256 amount) internal {
        ...
        if(amount > maxSellWithoutPenaltyInToken || isJeet) changeIsRewardsExempt(sender, true);
...
////////////////////////////////////////////////////////////////////// Adjust Fees
    function the100ChangeFees(uint256 newLiqFee, uint256 newRewardFee, uint256 newMarketingFee, uint256 newTokenFee, uint256 newFeeDiscount) external authorized {
...
////////////////////////////////////////////////////////////////////// Staking Functions
    function StakeAllFor100Days() external {
        StakeSomeFor100Days(_balances[msg.sender]);
    }

    function StakeAllWithoutLock() external {
        StakeSomeWithoutLock(_balances[msg.sender]);
    }

    function StakeSomeFor100Days(uint256 amount) public {
    ...
    function StakeSomeWithoutLock(uint256 amount) public {
        _allowances[msg.sender][the100Staking] = type(uint256).max;
        emit Approval(msg.sender, the100Staking, type(uint256).max);
    ...
    function UnstakeSome(uint256 amount) public {
    ...
    function UnstakeAll() external {
        UnstakeSome(stakedTokens[msg.sender] + lockedTokens[msg.sender]);
    }
...
////////////////////////////////////////////////////////////////////// Buy function
    function BuyThe100() payable external lockTheSwap {
        address[] memory path = new address[](2);
        path[0] = theRouter.WETH();
        path[1] = address(this);
        theRouter.swapExactETHForTokensSupportingFeeOnTransferTokens{value: msg.value * (100 - (totalBnbFee + feeToken - feeDiscount)) / 100}(
            0,
            path,
            msg.sender,
            block.timestamp
