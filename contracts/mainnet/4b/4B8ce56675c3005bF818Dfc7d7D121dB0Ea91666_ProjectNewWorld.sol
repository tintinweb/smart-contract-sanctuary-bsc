...
- WEBSITE : http://projectnewworld.com

- TELEGRAM : https://t.me/portalNWorld
*/
...
contract ProjectNewWorld is IBEP20, Auth {
    
    using SafeMath for uint256;

    string public constant _name = "Project New World"; //PNW
    string public constant _symbol = "$PNW"; //PNW
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingWallet = 0xb5Fc7583Ce7094A1c4b799a091A43bAbb88b7608; 
        devWallet = 0xC3Eab09d82D0C7D9307c7A7F5c4Cae045BcE685f; 
		prizewallet = 0xb5Fc7583Ce7094A1c4b799a091A43bAbb88b7608; 
    ...
    function changeTxLimit_PNW(uint256 newLimit) external authorized {
        require( newLimit >= 5, "Max tx cant be bellow 0.5%");
        _maxTxAmount = newLimit.mul(_totalSupply).div(1000);
        emit maxTxChanged(newLimit);
    }
    
    function checkTxLimit_PNW(address sender, uint256 amount) internal view {
        require(amount <= _maxTxAmount || isTxLimitExempt[sender], "TX Limit Exceeded");
    }

    function changeWalletLimit_PNW(uint256 newLimit) external authorized {
    ...
    function changeRestrictWhales_PNW(bool newValue) external authorized {
       restrictWhales = newValue;
    }
    
    function changeIsFeeExempt_PNW(address holder, bool exempt) external authorized {
    ...
    function changeFees_PNW(uint256 newLiqFee, uint256 newRewardFee, uint256 newMarketingFee, uint256 newDevFee, uint256 newprizewalletfee, uint256 extraSellFee) external authorized {
    ...
    function changeFeeReceivers_PNW(address newLiquidityReceiver, address newMarketingWallet, address newprizewalletWallet, address newDevWallet) external authorized {
    ...
    function changeSwapBackSettings_PNW(bool enableSwapBack, uint256 newSwapBackLimit, bool swapByLimitOnly) external authorized {
    ...
    function changeDistributionCriteria_PNW(uint256 newMinPeriod, uint256 newMinDistribution) external authorized {
    ...
    function changeDistributorSettings_PNW(uint256 newgas) external authorized {
        require(newgas < 300000);
        distributorGas = newgas;
    }

    function rescueBalance_PNW() external authorized{
        dividendDistributor.rescueDividends(msg.sender);
    }
    
    function setRewardToken_PNW(address _rewardToken) external authorized {
    ...
    function enable_blacklist_PNW(bool _status) public authorized {
        blacklistMode = _status;
    }
        
    function manage_blacklist_PNW(address[] calldata addresses, bool status) public authorized {
        for (uint256 i; i < addresses.length; ++i) {
            isBlacklisted[addresses[i]] = status;
        }
    }

    function rescueToken_PNW(address tokenAddress, uint256 tokens) public authorized returns (bool success) {
        return IBEP20(tokenAddress).transfer(msg.sender, tokens);
    }

    function clearStuckBalance_PNW(uint256 amountPercentage) external authorized {
