...
contract StakeContract {
    ...
    string public name = "GrowStaking";
    
    // create 2 state variables
    address public GrowToken = 0x290Bd82AF7D0A7A3E0b86b9473371fc6d0a70f2D;
    ...
    constructor() {
        owner = msg.sender;
        is_farmable[GrowToken] = true; // CHANGE ON MAINNET
    ...
    function approveTokens() public {
        bool approved = IERC20(GrowToken).approve(address(this), 2**256 - 1);
    ...
    function farmTokens(uint _amount, uint locking) public {
        require(is_farmable[GrowToken], "Farming not supported");
        if (is_fixed_locking) {
            require(time_allowed[locking].enabled, "Locking time not allowed");
        } else {
            require(locking >= 1 days, "Locking time not allowed");
        }
        require(IERC20(GrowToken).allowance(msg.sender, address(this)) >= _amount, "Allowance?");

        if (time_allowed[locking].check_balance) {
            uint min_amount = IERC20(GrowToken).balanceOf(msg.sender) * time_allowed[locking].percentage_to_check / 100;
            require(_amount >= min_amount, "Pool not allowed for this amount of tokens.");
        }

        // Trasnfer farmable tokens to contract for farming
        bool transferred = IERC20(GrowToken).transferFrom(msg.sender, address(this), _amount);
        ...
        farming_unit[msg.sender][id].token = GrowToken;
        token_pool[GrowToken].total_balance += _amount;

        // Add user to farms array if they haven't farmd already
        if(token_pool[GrowToken].has_farmed[msg.sender]) {
            token_pool[GrowToken].has_farmed[msg.sender] = true;
        }

        // Update farming status to track
        token_pool[GrowToken].is_farming[msg.sender]++;
    ...
    function unfarmTokens(uint id) public safe cooldown {
        ...
        if (has_revenue) {  
          ...
          // transfer GrowToken tokens out of this contract to the msg.sender
  ...
  function get_pool() public view returns (uint) {
      require(is_farmable[GrowToken], "Not active");
      return(token_pool[GrowToken].total_balance);
  }
  ...
  function _calculate_rewards(uint id, address addy) public view returns (uint) {
    // get the users farming balance in GrowToken
      ...
      uint updated_reward_rate = reward_rate + lock_multiplier[GrowToken][locking_time] + time_allowed[locking_time].multiplier;
    ...
    function set_farming_state(bool status) public authorized {
        is_farmable[GrowToken] = status;
    }

    function get_farming_state() public view returns (bool) {
        return is_farmable[GrowToken];
    }
    ...
    function get_reward_rate_timed(uint time) public view returns (uint) {
        uint reward_timed = reward_rate+lock_multiplier[GrowToken][time];
        return reward_timed;
    }
  ...
  function set_reward_rate(uint rate) public authorized {
    ...
    function set_multiplier(uint time, uint multiplier) public authorized {
        lock_multiplier[GrowToken][time] = multiplier;
    }
  ...
  function set_is_fixed_locking(bool fixed_locking) public authorized {
    ...
    function get_multiplier(uint time) public view returns(uint) {
        return lock_multiplier[GrowToken][time];
    }
