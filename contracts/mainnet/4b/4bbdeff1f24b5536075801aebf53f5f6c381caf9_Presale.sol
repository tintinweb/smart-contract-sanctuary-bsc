...
contract Presale{
    ...
    IERC20 public token;
    uint256 public _price = 5500000000000000 wei;
    ...
    function buyToken(address ref) public payable {
        ...
        sendReferral(ref, msg.value / 10);
        user.tokenPresale = user.tokenPresale + msg.value;
    }

    function claimBeats() public {
        require (End == true, "Presale hasn't finished");
        User storage user = users[msg.sender];
        require (user.tokenPresale > 0, "You don't have beats"); 
        uint256 noOfTokens = calculateTokens(user.tokenPresale);
        ...
        )
    {
        ...
        tokenPresale = user.tokenPresale;
