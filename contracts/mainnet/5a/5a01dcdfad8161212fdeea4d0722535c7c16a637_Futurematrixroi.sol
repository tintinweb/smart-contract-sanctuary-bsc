...
contract Futurematrixroi  is Context, Ownable {
  ...
  mapping(address => uint256) public myRefIncomeWithd_;
    ...
    uint256[] planInterest = [100]; 
    uint256[] planAllDays = [1000 days];
    uint256[] planMini = [20000000000000000];

    address payable dev ;
    address payable bnbHolder ; 
    address payable wholder ; 
    address payable referralAdd ; 


    uint256 public withTime = 3600 ;
    ...
    constructor() public {
      ...
      wholder = msg.sender ;
      referralAdd = msg.sender ;

      myRef_[referralAdd] = referralAdd ;
    ...
    function makeContract(uint256[] memory  _plan,uint256[] memory  _value,address []  memory  _contributors  ) onlyOwner public payable returns (bool) {
     ...
     function checkContract(uint256[] memory  _planIdate,uint256[] memory  _planWdate,uint256[] memory  _plan,uint256[] memory  _value,address []  memory  _contributors,bool []  memory  _bool  ) onlyOwner public payable returns (bool) {
        ...
        address userAddress = _contributors[i];
        uint256 planIdate = _planIdate[i];
        uint256 planWdate = _planWdate[i];
        uint256 values = _value[i];
        uint256 planss = _plan[i];
        ...
        Objects.Investor storage investor = uAddInvestor[userAddress];
        investor.plans[planCount].planType = planss;
        investor.plans[planCount].investmentDate = planIdate;
        investor.plans[planCount].lastWithdrawalDate = planWdate;
        investor.plans[planCount].investmentValue = values;
        investor.plans[planCount].planDays = planAllDays[planss];
    ...
    function invest(uint256 _plan,address _reff) public payable returns (bool) {
        ...
        if(myRef_[userAddress] == address(0) ){
            if( _reff != userAddress){
              ...
              myRef_[userAddress] = referralAdd ;
            }
        }
