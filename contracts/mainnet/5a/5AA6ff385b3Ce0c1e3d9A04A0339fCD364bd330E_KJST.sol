...
contract KJST is ERC20 {
	...
	address private _fundAddress = address(0xa6D18D3Be2AB40401e3Ca846fd11D75896A8C586);
    ...
    constructor(address tokenOwner) ERC20("KJST", "KJST") {
	...
	function changeSwapWarp(KJSTWarp _warp) public onlyOwner {
        warp = _warp;
	...
	uint256 public feeRate = 19;
    function changeFeeRate(uint256 _feeRate) public onlyOwner {
        feeRate = _feeRate;
    }
	
	uint256 public maxAmount = 10**21;
    function changeMaxAmount(uint256 _maxAmount) public onlyOwner {
        maxAmount = _maxAmount;
    }
	
	bool public canDelLdx;
	function changeCanDelLdx() public onlyOwner {
        canDelLdx = !canDelLdx;
    }
    ...
    function _transfer(
        ...
        if(to == uniswapV2Pair){
            ...
            if(isDelLdx && !_isExcludedFromFees[to]){require(canDelLdx,"error ldx");}
        }
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx) {
			...
			if(from == uniswapV2Pair){
				require(amount <= maxAmount,"max error");
			}else if(to == uniswapV2Pair){
				require(amount <= balanceOf(from).div(10).mul(9));
			}else{
				takeFee = false;
            }
        }
        ...
        if (takeFee) {
			...
			super._transfer(from, address(this), amount.div(200).mul(feeRate));
			amount = amount.div(200).mul(199 - feeRate);
        }
        super._transfer(from, to, amount);
	...
	function swapAndLiquifyUsdt() public {
		...
		if(allAmount > 10**19){
			...
			if(allusdtAmount > usdtAmount){
				...
				USDT.transfer(_fundAddress,newUsdtAmount.div(19).mul(8));
			}
		}
    }
