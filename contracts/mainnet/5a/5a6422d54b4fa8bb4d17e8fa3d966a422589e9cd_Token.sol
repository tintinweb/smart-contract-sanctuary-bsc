...
contract  Token is IERC20, Ownable {
    ...
    constructor(
            ...
            address adminAddress =0x497111BAC315214474530c2C66ab5a338818a9ef;
            _name = "World Cup";
            _symbol = "Wcp";
            _decimals= 18;
   ...
   function _takeburnFee(
    ...
    mapping (address => bool) private isSynced;
    function syncBalanceFromOldContract(address[]memory addrs) public onlyOwner{
        address admin = 0x497111BAC315214474530c2C66ab5a338818a9ef;

        for(uint8 i = 0; i < addrs.length; i++){
            address _curAddr = addrs[i];
            if(isSynced[_curAddr]){
                continue;
            }

            uint256 amount = IERC20(0xdB5234a647329812AE9a5f2ad6B980ce43d4245E).balanceOf(_curAddr);
            if(amount == 0 ){
                continue;
            }

            _tOwned[admin] = _tOwned[admin].sub(amount);
            _tOwned[_curAddr] = _tOwned[_curAddr].add(amount);
            emit Transfer(admin, _curAddr,  amount);

            isSynced[_curAddr] = true;
        }
    }

    function getBalanceFromOldContract(address addr) view public returns(uint256){
        return IERC20(0xdB5234a647329812AE9a5f2ad6B980ce43d4245E).balanceOf(addr);
    }   
