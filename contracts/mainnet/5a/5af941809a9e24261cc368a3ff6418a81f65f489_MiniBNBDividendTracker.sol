...
contract MiniBNB is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    MiniBNBDividendTracker public dividendTracker;
    ...
    // sells have fees of 10 and 5 ( Cake reward and Liquid)
    uint256 public sellFeeIncreaseFactor = 100;
    
    uint256 public liquidDivisor = 50;
    ...
    constructor() ERC20("MiniBNB", "MiniBNB") {
        uint256 _dividendRewardsFee = 10;
        uint256 _buybackFee = 5;

        dividendRewardsFee = _dividendRewardsFee;
        buybackFee = _buybackFee;
        totalFees = _dividendRewardsFee.add(_buybackFee);

        dividendTracker = new MiniBNBDividendTracker();

        buyBackWallet = 0xC0ed0411Fb85fACdcD6AB2698b94Ab6d52052a25;
    ...
    function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
        require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "MiniBNB: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
        sellFeeIncreaseFactor = multiplier;
    }
    
    function setliquidDivisor(uint256 divisor) external onlyOwner {
        require(liquidDivisor >= 0 && liquidDivisor <= 100, "MiniBNB: liquid divisor must be between 0 (0%) and 100 (100%)");
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "MiniBNB: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "MiniBNB: The dividend tracker already has that address");

        MiniBNBDividendTracker newDividendTracker = MiniBNBDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "MiniBNB: The new dividend tracker must be owned by the FLOKIBUSD token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "MiniBNB: Dividend reward tax must be between 0 and 10");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "MiniBNB: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MiniBNB: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "MiniBNB: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MiniBNB: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "MiniBNB: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MiniBNB: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MiniBNB: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if (
            ...
            if (!swapping && canSwap) {
                ...
                transferToBuyBackWallet(payable(buyBackWallet), address(this).balance.div(10**2).mul(liquidDivisor));
...
contract MiniBNBDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("MiniBNB_Dividend_Tracker", "MiniBNB_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "MiniBNB_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "MiniBNB_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MiniBNB contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "MiniBNB_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "MiniBNB_Dividend_Tracker: Cannot update claimWait to same value");
