...
contract TESTIFY is Context, IERC20, Ownable {
    ...
    string private _name = "TESTI-FY";
    string private _symbol = "TESTIFY";
    ...
    function _transfer(
        ...
        // max sell 3% of total supply
        if (isMaxSellActive && !isTimelockExempt[from] && to == pair) {
            require(amount < (totalSupply() * 3).div(100), "Transfer amount must be lower than 3% of total supply");
        }
    ...
    function isMaxSellActiveStatus(bool _status) external onlyOwner {
        isMaxSellActive = _status;
    }

    function antiwhaleStatus(bool _status) external onlyOwner {
        antiwhale = _status;
    }
    ...
    function callOracle() external onlyOwner() {
