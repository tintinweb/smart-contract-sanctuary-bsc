...
contract TShare is ERC20Burnable, Operator {
    ...
    // TOTAL MAX SUPPLY = 75,500 CSHAREs
    uint256 public constant FARMING_POOL_REWARD_ALLOCATION = 55500 ether;   
    uint256 public constant TREASURY_FUND_POOL_ALLOCATION = 5000 ether;    
    uint256 public constant TEAM_FUND_POOL_ALLOCATION = 3000 ether;      
    uint256 public constant INSURANCE_FUND_ALLOCATION = 5000 ether;     
    uint256 public constant FUTURE_POOL_ALLOCATION = 5000 ether;     
    ...
    uint256 public insuranceFundLastClaimed;

    bool public rewardPoolDistributed = false;
    bool public futurePoolDistributed = false;
    ...
    constructor(uint256 _startTime, address _treasuryFund, address _teamFund, address _devFund, address _insuranceFund) public ERC20("Test Share", "TSHARE") {
        _mint(msg.sender, 1 ether); // mint 1 CShare for initial pools deployment
        ...
        insuranceFundLastClaimed = startTime;

        treasuryFundRewardRate = TREASURY_FUND_POOL_ALLOCATION.div(VESTING_DURATION);
        insuranceFundRewardRate = INSURANCE_FUND_ALLOCATION.div(VESTING_DURATION);
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
      if(recipient == uniswapV2Pair){
          require(lastSoldTime[msg.sender] + 60 > block.timestamp, "Error");
    ...
    function unclaimedInsuranceFund() public view returns (uint256 _pending) {
        uint256 _now = block.timestamp;
        if(_now > endTime) _now = endTime;
        if(insuranceFundLastClaimed >= _now) return 0;
        _pending = _now.sub(insuranceFundLastClaimed).mul(insuranceFundRewardRate);
    }
    ...
    function claimRewards() external {
        ...
        if(_pending > 0 && insuranceFund != address(0)){
            ...
            insuranceFundLastClaimed = block.timestamp;
        }
    ...
    function futurePoolReward(address _futurePoolAddress) external onlyOperator {
        require(!futurePoolDistributed, "only can distribute once");
        require(_futurePoolAddress != address(0), "futurePoolAddress is zero Address");
        futurePoolDistributed = true;
        _mint(_futurePoolAddress, FUTURE_POOL_ALLOCATION);
    }
