...
contract MarsToken is BEP20('MarsSwap Token', 'Mars') {
...
// MarsBar with Governance.
contract MarsBar is BEP20('MarsBar Token', 'BAR') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterChef).
    function mint(address _to, uint256 _amount) public onlyOwner {
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
    ...
    function safeMarsTransfer(address _to, uint256 _amount) public onlyOwner {
...
interface IMigratorChef {
    // Perform LP token migration from legacy MarsSwap to MarsSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to MarsSwap LP tokens.
    // MarsSwap must mint EXACTLY the same amount of MarsSwap LP tokens or
    // else something bad will happen. Traditional MarsSwap does not
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of Marss
    ...
    function updatePool(uint256 _pid) public {
        ...
        mars.mint(address(bar), marsReward);
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'deposit MARS by staking');
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'withdraw MARS by unstaking');
    ...
    // Stake MARS tokens to MasterChef
    ...
    // Withdraw MARS tokens from STAKING.
    ...
    function safeMarsTransfer(address _to, uint256 _amount) internal {
        bar.safeMarsTransfer(_to, _amount);
    }
