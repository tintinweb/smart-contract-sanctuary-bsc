...
contract AthTrader {
    ...
    // Address of Gelato for Native swaps
    // address public immutable gelatoBase = 0x0c30D3d66bc7C73A83fdA929888c34dcb24FD599;
 ...
 * @dev Sets fee for given level
    ...
    // function to interact with gelato to place both limit and stop_limit orders
    function depositToken(
        uint256 _amount,
        address _module,
        address _inputToken,
        ...
        // require( _inputToken == 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c, "Invalid input token");
        // require(outputToken == 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56, "Invalid output token");
        // require(_pair != 0x58F876857a02D6762E0101bb5C46A8c1ED44Dc16, "Invalid pair");        
        require(checkPair(outputToken, _inputToken, _pair),"Invalid pair");
        ...
        // TODO: implement emit
        // emit DepositToken(msg.sender, _amount, _module, _inputToken, _owner, _witness, _data, _secret);
    }
    ...
    /**
     * @dev view function to check that provided pair is allowed, and that given tokens are part of the pair
     * @param _pair address of pair
     * @param _outputToken address of output token
     * @param _inputToken address of input token
     */
    function checkPair(address _outputToken, address _inputToken, address _pair) internal view returns (bool) {
        if (allowedV2PairsAndRouters[_pair] && (
            (IPAIR(_pair).token0() == _outputToken && IPAIR(_pair).token1() == _inputToken) ||
            (IPAIR(_pair).token0() == _inputToken && IPAIR(_pair).token1() == _outputToken)) 
        ) {
            return true;
        } else {
            return false;
