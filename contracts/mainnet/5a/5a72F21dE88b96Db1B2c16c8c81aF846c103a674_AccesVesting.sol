...
// File: contracts/AccesVestingFinal.sol
...
contract AccesVesting {
    ...
    function setAllowance(address _investor) internal{
        ...
        investorAllowance[_investor].monthlyAllowed = investorAllowance[_investor].yearlyAllowed.div(12);
    }
    ...
    function claimMonthlyAmount() external isInvestor(msg.sender) isNotBlackListed(msg.sender) nonReentrant{
        ...
        if(samePercentage == true){
            monthlyAmount = investor[msg.sender].yearlyAllowance.mul(mP).div(hPercent);
        }
        else{
