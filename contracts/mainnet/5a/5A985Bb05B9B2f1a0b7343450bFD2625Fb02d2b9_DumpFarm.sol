...
contract DumpFarm is Ownable, IERC20 {
    using SafeMath for uint256;

    // lock time in blocks
    uint256 public lockTime = 432000;
    ...
    event SetEarlyFee(uint256 earlyFee);
    ...
    function name() public pure override returns (string memory) {
        return "Staked DUMP-BNB Lp";
    }

    /** Token Ticker Symbol */
    function symbol() public pure override returns (string memory) {
        return "StakedDUMPBNB";
    }
    ...
    function withdraw(uint256 amount) external {
        ...
        userInfo[msg.sender].totalExcluded = getCumulativeDividends(
            userInfo[msg.sender].amount
        );

        uint256 fee = timeUntilUnlock(msg.sender) == 0
            ? 0
            : (amount * leaveEarlyFee) / 100;
        if (fee > 0) {
            require(
    ...
    function depositRewards(uint256 amount) external {
        ...
        dividendsPerShare = dividendsPerShare.add(
            precision.mul(received).div(totalShares)
        );
        ...
        returns (uint256)
    {
        ...
        uint256 received = IERC20(_token).balanceOf(address(this)) - before;
        require(
    ...
    receive() external payable {
        ...
        uint256 received = IERC20(reward).balanceOf(address(this)).sub(before);
