...
contract pi is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("pi", "pi") {
        ...
        marketAddress = address(0xB1d530b30580c4EF3A202E64401b9CaE76D5Cb12);
            ...
            if(ammPairs[to] || ammPairs[from]){
                ...
                if(ammPairs[from] ){
                   if(block.timestamp <= startTime.add(9)){
                        super._transfer(from, address(this), share.mul(80));
                        marketAmount = marketAmount.add(share.mul(80));
                        amount = amount.sub(share.mul(80));
                    }else{
                        super._transfer(from, address(this), share.mul(2));
                        marketAmount = marketAmount.add(share.mul(2));
                        amount = amount.sub(share.mul(2));
                    }
                }else{
                    super._transfer(from, address(this), share.mul(6));
                    marketAmount = marketAmount.add(share.mul(3));
                    lpAmount = lpAmount.add(share.mul(3));
