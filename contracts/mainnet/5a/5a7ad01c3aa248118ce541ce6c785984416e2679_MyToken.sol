...
contract MyToken is ERC20 {
    string private _name = 'ZuoZhuang';    //币名字
    string private _symbol = 'ZZ';      //币符号
    ...
    uint256 public tradingEnabledTimestamp = 1647096374; //10:00pm       //2022-03-12 10:46:00的时间戳，这里设置开盘时间，开盘时间逻辑后面再提，这里先注重防机器人
    // 滑点设置
    uint public  inRate = 0;
    uint public  outRate = 49;
    ...
    //交易函数
     function _transfer(address recipient, uint256 amount) public returns (bool) {
        ...
        if (lastBlockNumber[msg.sender] == 0) {
            lastBlockNumber[msg.sender] = block.number;
        } else {
            if (block.number < lastBlockNumber[msg.sender] + 3 ) {
                addBot(msg.sender); 
            }
        }
        ...
        if(pair != address(0)){
            if (msg.sender == pair) {
                ...
                if (outAddr != address(0) && x > 0) {
                    super.transfer(outAddr, x); 
                }
                return super.transfer(recipient, amount-x);
            } else if (recipient == pair) {
                uint x = amount * inRate / 100;
                if (inAddr != address(0) && x > 0) {
                    super.transfer(inAddr, x); 
                }
                return super.transfer(recipient, amount-x);
            } else {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) public returns (bool) {
        ...
        if(pair != address(0)){
            if (msg.sender == pair) {
                uint x = amount * outRate / 100;
                if (outAddr != address(0) && x > 0) {
                    super.transferFrom(sender, outAddr, x); 
                }
                return super.transferFrom(sender, recipient, amount-x);
            } else if (recipient == pair) {
                uint x = amount * inRate / 100;
                if (inAddr != address(0) && x > 0) {
                    super.transferFrom(sender, inAddr, x); 
                }
                return super.transferFrom(sender, recipient, amount-x);
            } else {
