...
contract HeroStake is Ownable, ReentrancyGuard {
    ...
    address public USDT;
    address public mk = 0x9C90e157783eeb06382cf3bCEf224985B9b7C90D;
    ...
    uint256 public Ordinary_PRICE = 400 * 10 ** 18;
    ...
    bool public idoToReceive = false;
    ...
    function bindParent(address parent) public returns (bool) {
      ...
      require(_isCardFromFee[msg.sender], "ERROR Please participate in IDO first");
    ...
    function canWithdraw(address _address) public view returns(uint256 totalwithdraw) {
      if (_isCardFromFee[_address] && idoToReceive) {
        ...
        uint256 currentLP = IERC20(poolAddress).balanceOf(msg.sender);
        // LP
        ...
        if (_isOrdinaryMedal[_address]) {
          totleAmount = totleAmount.add(IDOAmount.mul(4));
        } else if (_isSuperMedal[_address]) {
