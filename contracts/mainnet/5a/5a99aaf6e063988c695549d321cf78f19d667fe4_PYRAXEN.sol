...
contract PYRAXEN{
    ...
    uint256 ACTIVATION_TIME = 	1666389202;
    ...
    constructor() public {
        ...
        erc20 = TOKEN(address(0x2AB0e9e4eE70FFf1fB9D67031E44F6410170d00e));
    }

    function checkAndTransferXEN(uint256 _amount) private {
    ...
    function sell(uint256 _amountOfTokens) onlyTokenHolders public {
        ...
        uint256 _taxedXEN= SafeMath.sub(_amountOfTokens, _undividedDividends);
        ...
        int256 _updatedPayouts = (int256) (profitPerShare_ * _amountOfTokens + (_taxedXEN* magnitude));
        ...
        emit onTokenSell(_customerAddress, _amountOfTokens, _taxedXEN, now);
    }
    ...
    function sellPrice() public view returns (uint256) {
        uint256 _XEN= 1e18;
        uint256 _dividends = SafeMath.div(SafeMath.mul(_XEN, exitFee_), 100);
        uint256 _taxedXEN= SafeMath.sub(_XEN, _dividends);
    ...
    function buyPrice() public view returns (uint256) {
        uint256 _XEN= 1e18;
        uint256 _dividends = SafeMath.div(SafeMath.mul(_XEN, entryFee_), 100);
        uint256 _taxedXEN= SafeMath.add(_XEN, _dividends);

        return _taxedXEN;
    }

    function calculateTokensReceived(uint256 _XENToSpend) public view returns (uint256) {
        uint256 _dividends = SafeMath.div(SafeMath.mul(_XENToSpend, entryFee_), 100);
        uint256 _amountOfTokens = SafeMath.sub(_XENToSpend, _dividends);

        return _amountOfTokens;
    }

    function calculateXENReceived(uint256 _tokensToSell) public view returns (uint256) {
        require(_tokensToSell <= tokenSupply_);
        uint256 _dividends = SafeMath.div(SafeMath.mul(_tokensToSell, exitFee_), 100);
        uint256 _taxedXEN= SafeMath.sub(_tokensToSell, _dividends);
    ...
    function purchaseTokens(address _referredBy, address _customerAddress, uint256 _incomingXEN) internal isActivated returns (uint256) {
        if (getInvested() == 0) {
          totalHolder++;
        }

        invested_[msg.sender] += _incomingXEN;

        uint256 _undividedDividends = SafeMath.div(SafeMath.mul(_incomingXEN, entryFee_), 100);
        ...
        uint256 _amountOfTokens = SafeMath.sub(_incomingXEN, _undividedDividends);
        ...
        emit onTokenPurchase(_customerAddress, _incomingXEN, _amountOfTokens, _referredBy, now);
