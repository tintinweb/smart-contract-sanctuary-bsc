...
// File contracts/SingleStakingRewardPool.sol
...
contract SingleStakingRewardPool is ReentrancyGuard, Ownable {
    ...
    uint256[] public stakingTires = [21 days, 15 days, 7 days, 0];
    uint256[] public feeTires = [500, 300, 200, 100];
    ...
    uint256 public sharesPerSecond;         // Amount of reward tokens per second
    ...
    function setSharesPerSecond(uint256 _sharesPerSecond) external onlyOwner {
        sharesPerSecond = _sharesPerSecond;
    }

    function setPoolEndTime(uint256 _poolEndTime) external onlyOwner {
        poolEndTime = _poolEndTime;
    }

    // this really exists for the unit tests to sync with the test EVM
    function blockTimestamp() external view returns(uint256) {
        return block.timestamp;
    }

    function init(address _stakingToken, address _rewardToken, address _feeWallet, uint256 _poolStartTime, uint256 _runningTime) public onlyOwner {
        ...
        sharesPerSecond = totalRewards / runningTime;
    ...
    function _add(uint256 _allocPoint, IERC20 _token, bool _withUpdate, uint256 _lastRewardTime) private onlyOwner {
    ...
    function getGeneratedReward(uint256 _fromTime, uint256 _toTime) public view returns (uint256) {
        ...
        if (_toTime >= poolEndTime) {
            ...
            if (_fromTime <= poolStartTime) return (poolEndTime - poolStartTime) * sharesPerSecond;
            return (poolEndTime - _fromTime) * sharesPerSecond;
        } else {
            if (_toTime <= poolStartTime) return 0;
            if (_fromTime <= poolStartTime) return (_toTime - poolStartTime) * sharesPerSecond;
            return (_toTime - _fromTime) * sharesPerSecond;
        }
    }
    ...
    function setFeesStatus(bool _enableFees) external onlyOwner {
        enableFees = _enableFees;
    }

    function setStakingTiersEntry(uint8 _index, uint256 _value) external onlyOwner {
    ...
    function setFeeTiersEntry(uint8 _index, uint256 _value) external onlyOwner {
        require(_index >= 0, "Index has to be higher than 0");
    ...
    function drainPool(address to) public onlyOwner {
