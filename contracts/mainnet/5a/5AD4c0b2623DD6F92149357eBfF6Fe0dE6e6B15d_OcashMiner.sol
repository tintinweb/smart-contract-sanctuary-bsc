...
contract OcashMiner is Initializable, OwnableUpgradeable {
    using SafeMathUpgradeable for uint256;

    IERC20Upgradeable public Ocash; //Ocash token
    ...
    uint256 public restrictedBuyAmountOcash;
    ...
    function store() public initializer {
        ...
        SpellSyrup = 750000;
        restrictedBuyAmountOcash = 1000_00000; // 1000 Ocash
        bagOfOceans = 1000_00000;  // 1000 Oceans
        initialized = false;
        startTime = block.timestamp;
        developerWallet = 0x7358F144CEf26fC02365FcD6aB53e1381CB93B83;
        Ocash = IERC20Upgradeable(0x2A54F9710ddeD0eBdde0300BB9ac7e21cF0E8DA5);
    ...
    function OcashRewards(address adr) public view returns (uint256) {
    ...
    function buySeas(address ref, uint256 amount) public {
        ...
        require(amount <= restrictedBuyAmountOcash,"Error: Cann't Purchase More than 10K!!");
        require(!blacklisted[msg.sender],"Opps: Your Blacklisted by Security!!");
        ...
        IERC20Upgradeable(Ocash).transferFrom(
            address(msg.sender),
            address(this),
            amount
        );
        ...
        IERC20Upgradeable(Ocash).transfer(developerWallet,developerFee);

        addInvesters(msg.sender);

        uint256 balance = IERC20Upgradeable(Ocash).balanceOf(address(this));
        ...
        require(trident[msg.sender]._invested.add(amount) <= restrictedBuyAmountOcash,"Error: Cann't Purchase More than 10K!!");
    ...
    function magicSurf(address _target) internal {
        ...
        IERC20Upgradeable(Ocash).transfer(developerWallet,developerFee);
        IERC20Upgradeable(Ocash).transfer(address(_target), deductedAmount);
    ...
    function calculateSeasSell(uint256 seas) public view returns (uint256) {
        return
            calculateTrade(
                seas,
                marketSeas,
                IERC20Upgradeable(Ocash).balanceOf(address(this))
            );
    }
    ...
    function getEstimatedSeas(uint _amount) public view returns (uint256) {
        uint _balance = IERC20Upgradeable(Ocash).balanceOf(address(this));
    ...
    function getBalance() public view returns (uint256) {
        return IERC20Upgradeable(Ocash).balanceOf(address(this));
    }
    ...
    function setOcashInCan(uint256 _Ocash) public onlyOwner {
        Seas_TO_HATCH_1MINERS = _Ocash;
    }

    function setOcash(address _adr) public onlyOwner {
        require(!initialized); //ones start cannot change token
        Ocash = IERC20Upgradeable(_adr);
    }
