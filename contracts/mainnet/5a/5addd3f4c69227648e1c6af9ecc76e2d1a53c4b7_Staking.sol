...
// File: contracts/IUniRouter02.sol
...
contract Staking is Ownable, ReentrancyGuard {
    ...
    uint256 public duration = 180; // 180 days
    ...
    address public buyBackWallet = taxWallet;
    uint256 public serviceFee = 5000000000000000;
    ...
    event LockupUpdated(uint8 _type, uint256 _duration, uint256 _fee0, uint256 _fee1, uint256 _rate, bool _serviceFeeEnabled, bool _earlyWithdrawEnabled);
    ...
    function initialize(
        ...
        require(msg.sender == CREATOR, "Not factory");
        ...
        lockups.push(Lockup(0, 180, 0, 1500, 380517503805175000000000, 0, 0, 0, false, true)); 
    ...
    function deposit(uint256 _amount, uint8 _stakeType) external nonReentrant {
        ...
        if (lockup.depositFee > 0) {
            ...
            if (fee > 0) {
                stakingToken.safeTransfer(taxWallet, fee);
    ...
    function withdraw(uint256 _amount, uint8 _stakeType) external nonReentrant {
        ...
        require(_stakeType < lockups.length, "Invalid stake type");
                Lockup storage lockup = lockups[_stakeType];
        if (lockup.serviceFeeEnabled == true) {
         _transferserviceFee();
         }
        _updatePool(_stakeType);
 ...
 //               }
   //             compounded = compounded.add(_pending);
     //            //stake.amount = stake.amount.add(_pending);
        ...
        if(realAmount > 0) {
                if (lockup.earlyWithdrawEnabled == true) {
 ...
 //               compounded = compounded.add(_pending);
   ...
   //         } else {
    ...
    function _transferserviceFee() internal {
        ...
        require(msg.value >= serviceFee, 'Service fee needs to be paid');

        payable(buyBackWallet).transfer(serviceFee);
        if(msg.value > serviceFee) {
            payable(msg.sender).transfer(msg.value.sub(serviceFee));
        }
    }
    ...
    *************************/
    function harvest() external onlyOwner {        
        ...
        if(pendingReflection > 0) {
            if(address(dividendToken) == address(0x0)) {
                payable(taxWallet).transfer(pendingReflection);
            } else {
                dividendToken.safeTransfer( taxWallet, pendingReflection);
            }
            totalReflections = totalReflections.sub(pendingReflection);
        }
     ...
     */
     ...
     function updateServiceFee (uint256 _serviceFeeEther) external onlyOwner {
         require (_serviceFeeEther < 10000000000000000, "Service Fee too high, must be less than 0.01 ether");
        serviceFee = _serviceFeeEther;
           
     }
     function updateServiceFeeEnabled (uint8 _stakeType, bool _serviceFeeEnabled) external onlyOwner {
         require(_stakeType < lockups.length, "Lockup Not found");
         Lockup storage _lockup = lockups[_stakeType];
        _lockup.serviceFeeEnabled = _serviceFeeEnabled;
     }
...
function updateEarlyWithdrawalFee (uint8 _stakeType, uint256 _withdrawFee, bool _earlyWithdrawEnabled) external onlyOwner {
    require(_stakeType < lockups.length, "Lockup Not found");
    require(_withdrawFee < 2000, "Invalid withdraw fee");
    Lockup storage _lockup = lockups[_stakeType];
    _lockup.withdrawFee = _withdrawFee;
    _lockup.earlyWithdrawEnabled = _earlyWithdrawEnabled;
}
    function updateLockup(uint8 _stakeType, uint256 _duration, uint256 _depositFee, uint256 _withdrawFee, uint256 _rate, bool _serviceFeeEnabled, bool _earlyWithdrawEnabled) external onlyOwner {
        ...
        emit LockupUpdated(_stakeType, _duration, _depositFee, _withdrawFee, _rate, _serviceFeeEnabled, _earlyWithdrawEnabled);
    }

    function addLockup(uint256 _duration, uint256 _depositFee, uint256 _withdrawFee, uint256 _rate, bool _serviceFeeEnabled, bool _earlyWithdrawEnabled) external onlyOwner {
        require(_depositFee < 2000, "Invalid deposit fee");
        require(_withdrawFee < 2000, "Invalid withdraw fee");
        ...
        emit LockupUpdated(uint8(lockups.length - 1), _duration, _depositFee, _withdrawFee, _rate, _serviceFeeEnabled, _earlyWithdrawEnabled);
    }
