...
// File: contracts/BuyBackOfficial.sol
...
event BuybackWeightsAdjusted(uint indexed newBurn, uint indexed newReflect, uint indexed newLiquidity);
    ...
    function addLiquidity(uint _amount, uint _percentMinOutput) internal returns (bool)
    {
        ...
        if (IERC20(_WETHTokenPath[1]).allowance(address(this), pancakeSwapRouter02) != 0)
        {
            IERC20(_WETHTokenPath[1]).safeApprove(pancakeSwapRouter02, 0);    
        }
        IERC20(_WETHTokenPath[1]).safeApprove(pancakeSwapRouter02, _soldtipsy*2);
