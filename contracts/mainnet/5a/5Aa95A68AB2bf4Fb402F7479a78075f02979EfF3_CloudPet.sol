...
contract CloudPet is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable private cloudpetAddress;
    ...
    constructor (string memory _n, string memory _s, uint8 _d, uint256 _ts, uint256 _tax, uint256 _bb, uint256 _mkt, address _ma) {
        ...
        cloudpetAddress = payable(0xf6a896281545301d6e19C3f429df9A42d53A8F36);
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        transferToAddressETH(cloudpetAddress, transferredBalance.div(_liquidityFee).mul(cloudpetFee));
        transferToAddressETH(marketingAddress, transferredBalance.div(_liquidityFee).mul(marketingFee.sub(cloudpetFee)));
    ...
    function setMarketingFee(uint256 _marketingFee) external onlyOwner() {
        require(_marketingFee >= cloudpetFee, 'Marketing fee can not be less than 5 percent');
