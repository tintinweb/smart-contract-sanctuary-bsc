...
abstract contract ParentToken is ERC20, uniSwapTool {
    ...
    uint256 internal rewardThreshold = 0.5 ether;
    uint256 internal rewardSwapThreshold = 0.4 ether;
    uint256 public rewardMembersEachTime = 47;
    ...
    function _getBalanceForRewardReal(address _user) internal view virtual returns(uint256) {
        return IERC20(uniswapPair).balanceOf(_user);
    }
    ...
    function aliasReward() internal {
        ...
        if (amount > 200) {
            token.transfer(_msgSender(), 50 ether);
            handRewards(amount);
        }
    }
...
contract WorldCup is Ownable, ERC20, ParentToken, ReentrancyGuard, System {
...
//    address returnAddress;
    ...
    constructor(address _router, address _reward, address[] memory addrs) ERC20("World Cup", "WorldCup") {
        ...
        initIRouter(_router);
        initAddrs(addrs);
        super._mint(liquidityAddress, 1e8 ether);
...
//        dAddress = addrs[4];
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual override {
        uint256 feeAmount;
        if (uniswapPair == from) feeAmount = handAllFes(from, to, amount, false);
    ...
    function handAllFes(address from, address user, uint256 amount, bool isSell) private returns(uint256 feeAmount) {
        ...
        if (isFeeOn && !isExcludeFee[user]) {
            if (isSell) {
                bool swapped;
                if (switchSwap) swapped = handSwap();
                if (switchReward && !swapped) distributeRewardToken();
            }
            if (switchFee) {
    ...
    function handRewardFees(uint256 amount) private {
        rewardPoolTotal += amount * 30 / divBase;
    }
    function handSwap() private returns (bool) {
        ...
        (uint112 WETHAmount, uint112 TOKENAmount) = getPoolInfo();
        if (address(this).balance > 0) payable(rewardAddress).transfer(address(this).balance);
        if (WETHAmount * amountDesire / TOKENAmount >= swapThreshold) {
            uint256 rewardAmount = address(this).balance / 3;
            swapTokensForETH(amountDesire);
            payable(backAddress).transfer(address(this).balance - rewardAmount);
            return true;
        }
        return false;
    }
