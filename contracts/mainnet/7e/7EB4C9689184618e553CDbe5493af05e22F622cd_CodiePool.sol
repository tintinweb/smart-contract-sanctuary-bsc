...
contract CodiePool is Ownable {
    ...
    uint256 public DURATION = 356 * 5 days;
    ...
    mapping(address => uint256) public rewardReferral;
    mapping(address => uint256) public rewardReferralPending;
    ...
    function stake(uint256 amount) public updateReward(msg.sender) checkHalve checkStart {
        ...
        dispatchHandle.stake(msg.sender, amount);
        stakedToken.safeTransferFrom(msg.sender, address(this), amount);
        referralHandler.updatePerformance(msg.sender, int256(amount));
    ...
    function withdraw(uint256 amount) public updateReward(msg.sender) checkHalve {
        ...
        dispatchHandle.withdraw(msg.sender, amount);
        uint256 fee = amount.mul(withdrawFee).div(100);
        stakedToken.safeTransfer(msg.sender, amount.sub(fee));
        dispatchWithdrawFee(fee);
        referralHandler.updatePerformance(msg.sender, -(int256(amount)));
        emit Withdrawn(msg.sender, amount);
    }

    function dispatchWithdrawFee(uint256 fee) private {
        stakedToken.safeTransfer(dev, fee.mul(2).div(100));
        stakedToken.safeTransfer(address(0xdead), fee.mul(3).div(100));
        address account = referralHandler.referrers(msg.sender);
        if (account == address(0)) {
            account = dev;
        }
        stakedToken.safeTransfer(account, fee.mul(50).div(100));

        stakedToken.safeTransfer(address(dispatchHandle), fee.mul(45).div(100));
    }
    ...
    function getReward() public updateReward(msg.sender) {
        ...
        uint256 referralReward = reward.mul(10).div(100);
        address referrer1 = referralHandler.referrers(msg.sender);
        address referrer2 = referralHandler.referrers(referrer1);
        if (referrer1 != address(0)) {
            rewardReferralPending[referrer1] = rewardReferralPending[referrer1]
                .add(referralReward);

            if (referrer2 != address(0)) {
                rewardReferralPending[referrer2] = rewardReferralPending[
                    referrer2
                ].add(referralReward);
            } else {
                rewardToken.safeTransfer(dev, referralReward);
            }
        } else {
            rewardToken.safeTransfer(dev, referralReward * 2);
        }
    ...
    function withdrawReferralReward() external {
        uint256 reward = rewardReferralPending[msg.sender];
        rewardToken.safeTransfer(msg.sender, reward);
        rewardReferral[msg.sender] = rewardReferral[msg.sender].add(reward);
        rewardReferralPending[msg.sender] = 0;
    }
