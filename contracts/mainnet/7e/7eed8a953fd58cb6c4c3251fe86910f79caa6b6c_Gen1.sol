...
contract Gen1 {
    ...
    uint256 public MIN_Deposits = 6;
    uint256 public MIN_Deposits_amount = 50000000000000;
    uint256 public sCompound = 950;
  ...
  function Compound() public view returns(uint256 StartBlock ){
    return(sCompound);
  }
  function CompoundEdit(uint256 _sCompound) public {
    require (msg.sender ==  Owner);
    sCompound = _sCompound;
    ...
    if ( getUserTotalDeposits(userAddress) >= MIN_Deposits_amount){
        ...
        token.transfer(msg.sender, totalAmount);
        emit Withdrawn(msg.sender, totalAmount);
         uint256 fee= SafeMath.div(SafeMath.mul(totalAmount,sCompound),100);
         uint256 fee2=fee;
