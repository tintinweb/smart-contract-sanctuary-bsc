...
contract BULLRUN is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256[] public tiers = [150, 250, 350];
    uint256[] public prices = [0.25 ether, 0.3 ether, 0.35 ether];
    uint256[] public maxMints = [10, 10, 10];
    ...
    function mint(uint256 _amount) external payable {
        require(!onlyWL,'Only Whitelist Mint Allowed At This Stage');
        ...
        if (msg.sender != owner()) {
            ...
            require(
                _amount + currBalance <= maxMintAmount,
    ...
    {
        uint256 supply = totalSupply();
        uint256 totalAmount = receivers.length * _amount;
        require(supply + totalAmount <= maxSupply, "max NFT limit exceeded");
    ...
    function setOnlyWL(bool _newState) external onlyOwner {
        onlyWL = _newState;
    }
