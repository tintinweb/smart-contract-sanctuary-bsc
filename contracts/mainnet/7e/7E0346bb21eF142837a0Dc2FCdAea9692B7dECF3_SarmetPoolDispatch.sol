...
contract SmartReserve is SafeOwnable {
    function getApproval(IERC20 token, uint256 value) external onlyOwner {
        token.approve(msg.sender, value);
    }
}

contract SarmetPoolDispatch is SafeOwnable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    SmartReserve public smartReserve;
    ...
    constructor(address stakedToken_, address[] memory rewardTokens_) {
        ...
        bytes memory bytecode = type(SmartReserve).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(address(this)));
        address _smartReserve;
        assembly {
            _smartReserve := create2(
                0,
                add(bytecode, 32),
                mload(bytecode),
                salt
        ...
        smartReserve = SmartReserve(_smartReserve);
    }
    ...
    function withdraw(uint256 amount) external updateDispatch(msg.sender) {
        ...
        smartReserve.getApproval(IERC20(stakedToken), amount);
        IERC20(stakedToken).safeTransferFrom(
            address(smartReserve),
            account,
            amount
