...
contract Gen1 {
   ...
   // uint256 public val;
    ...
    uint256 public Percent_1 = 1;
    uint256 public Percent_2 = 2;
    uint256 public Percent_3 = 3;
    uint256 public TPercent_1 = 1;
    uint256 public TPercent_2 = 2;
    uint256 public TPercent_3 = 3;
    uint256 public startUNIX;
    address payable public OwnerAddress;
    
   // uint256 public percent=1;
   // uint256 public time=1;
   // uint256 public withdrawl=1;
    struct Plan {
        uint256 time;
        uint256 percent;
        uint256 withdrawl;
    ...
    constructor(address payable wallet) public {
        token = IBEP20(busd);
 plans.push(Plan(1, 4, 180));
 plans.push(Plan(2, 5, 600));
 plans.push(Plan(3, 6, 800));
...
function Reward_Staking_0 (uint8 plan,uint256 vakue_time, uint256 vakue_percent,uint256 vakue_withdrawl ) external {
...
// plans.push(Plan(1, 4, 180));
 ...
 //plans.push(Plan(3, 6, 800));
   ...
   /* function Reward_Staking_0 (uint8 plan,uint256 time, uint256 percent,uint256 withdrawl ) external {
        require(msg.sender == OwnerAddress, "Admin use only.");
        //require(value >= 1 && value <= 15);
          time = 1;
         percent = 1;
...
*/
    ...
    function Reward_Staking_1 (uint256 value) external {
        require(msg.sender == OwnerAddress, "Admin use only.");
        require(value >= 1 && value <= 15);
        Percent_1 = value;
    }
    function Reward_Staking_2 (uint256 value) external {
        require(msg.sender == OwnerAddress, "Admin use only.");
        require(value >= 1 && value <= 50);
        Percent_2 = value;
    }
     function Reward_Staking_3 (uint256 value) external {
        require(msg.sender == OwnerAddress, "Admin use only.");
        require(value >= 1 && value <= 80);
        Percent_3 = value;
    }


    function tReward_Staking_1 (uint256 value) external {
        require(msg.sender == OwnerAddress, "Admin use only.");
        require(value >= 1 && value <= 15);
        Percent_1 = value;
    }
    function tReward_Staking_2 (uint256 value) external {
        require(msg.sender == OwnerAddress, "Admin use only.");
        require(value >= 1 && value <= 50);
        TPercent_2 = value;
    }
     function tReward_Staking_3 (uint256 value) external {
        require(msg.sender == OwnerAddress, "Admin use only.");
        require(value >= 1 && value <= 80);
        TPercent_3 = value;
    }
...
if (user.referrer == address(0)) {
	...
	//	user.holdBonusCheckpoint = block.timestamp;
      ...
      /*  User storage user = users[msg.sender];
        ...
        token.transfer(msg.sender, totalAmount);
        emit Withdrawn(msg.sender, totalAmount);
         uint256 fee= SafeMath.div(SafeMath.mul(totalAmount,Compound),100);
         uint256 fee2=fee;
        IBEP20(busd).transfer(address(this), fee2);
    }*/
...
*/
	...
	function getUserLastDepositInfo(address userAddress) public view returns(uint8 plan, uint256 percent, uint256 amount, uint256 start, uint256 finish, uint256 profit, uint256 cooldown) {
	    User storage user = users[userAddress];
		if(user.deposits.length > 0){
