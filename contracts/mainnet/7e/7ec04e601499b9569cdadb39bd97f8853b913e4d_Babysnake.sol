...
// Baby snake  rewarding in 20% in space snake
// TG : https://t.me/BabySnake
...
contract Main is BEP20, SharedOwnable, SafeToken {
    ...
    constructor(string memory name_, string memory symbol_, uint8 decimal_, address reward1_, address marketing_, address team_, address multiuse_, uint256 supply_, uint256 dispatch_, uint8 maxtaxes_) BEP20(name_, symbol_) {
        ...
        dividendTracker1 = new DIVIDENDTracker1("Babysnake", "BSnake", Token1Adress, minTokenBalanceForDividends, decimal, Creator);
    ...
    function Launch(uint256 Blocks) external onlySharedOwners {
        ...
        require(Blocks <= 60, "Not more than 2mn");
        tradingEnabled = true;
    ...
    function SetJeetsTax(bool jeetsfee, bool jeetsburn, bool jeetsstaking, uint8 vmaxdiv, uint8 vmindiv, uint8 maxjeetsfee)  external onlySharedOwners {
        require (vmaxdiv >= 10 && vmaxdiv <= 50, "cannot set Vmax outside 10%/50% ratio");
        require (vmindiv >= 1 && vmindiv <= 10, "cannot set Vmin outside 1%/10% ratio");
        require (maxjeetsfee >= 1 && maxjeetsfee <= 50, "max jeets fee must be betwwen 1% and 50%");
        JeetsFee = jeetsfee;
        JeetsBurn = jeetsburn;
...
contract Babysnake is Main {

    constructor() Main (
        "Babysnake",       // Name
        "BSnake",        // Symbol
        9,                  // Decimal
        0x337C4930B612FA27072abB7b8fDE693e9D129405,     // Reward 1
