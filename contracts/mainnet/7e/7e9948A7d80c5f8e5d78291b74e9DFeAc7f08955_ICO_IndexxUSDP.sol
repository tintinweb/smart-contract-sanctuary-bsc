...
// File: contracts\TimeLock_IndexxUSDP.sol
 ...
 */
contract TimeLock_IndexxUSDP {
    using SafeMath for uint256;

    /**
     * @dev IndexxUSD+ Token Address
     */
    ...
    /**
     * @dev Constructor Function to add IndexxUSD+ Token Address
     */
...
// File: contracts\Chainlink_IndexxUSDP.sol


pragma solidity ^0.8.4;
contract Chainlink_IndexxUSDP {
    ...
    function getUSDTLatestPrice() internal view returns (uint256) {
        (
            ,
            /*uint80 roundID*/
...
// File: contracts\ICO_IndexxUSDP.sol


pragma solidity ^0.8.4;
/**
 * @title ICO Contract for IndexxUSD+ Token
 * @dev For Presales Rounds and multiple ICO phases
 */
contract ICO_IndexxUSDP is Chainlink_IndexxUSDP {
    using SafeMath for uint256;

    /// @dev Address where the ICO and Presales funds are collected
    address public reserveWallet;
    /// @dev Address of IndexxUSD+
    address public token;
    ...
    constructor() {
        ...
        timelockContract = address(0x44D37703A37C617b6B824Eb45615aC5304740305);
        token = address(0xa18f33e2C63C0A781f6836f9Ae8F5f6517Ce4e90);
    ...
    {
        if (discount > 0) {
            IndexxUSDP(token).mint(timelockContract, _tokenAmount);
            TimeLock_IndexxUSDP(timelockContract).initiateTokenLock(
                _beneficiary,
                _tokenAmount
            );
        } else {
            IndexxUSDP(token).mint(_beneficiary, _tokenAmount);
        }
    }
    ...
    function _getTokenAmount(uint256 _weiAmount, address tokenAddress)
        ...
        returns (uint256)
    {
        uint256 usdtPrice = getUSDTLatestPrice();
        uint256 indexxPriceUSD = usdtPrice.mul(10**10);
        uint256 price;
    ...
    function withdraw() external onlyIcoAdmin {
