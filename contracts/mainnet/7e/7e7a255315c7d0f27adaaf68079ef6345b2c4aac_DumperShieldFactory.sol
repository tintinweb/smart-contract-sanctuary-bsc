...
contract DumperShieldFactory is Ownable, Voting {
    ...
    struct Order {
        ...
        OrderStatus status;     // 1 - created, 2 - completed; 3 - canceled; 4 -restricted
    ...
    function balanceOf(address token, address user) public view returns (uint256 total, uint256 available) {
        ...
        require(dsUser != address(0), "user 0"); //User is not in dumper shield
    ...
    function buyOrder(uint256 orderId) external payable {
        ...
        if (value > 0) {
            ...
            if (token == address(0)) {
                ...
                (o.seller).safeTransferETH(msg.value);

            } else {

                uint256 tokenAmount = getQuote(o.router, WETH, token, value);

                if (address(dumperShieldTokens[token]) == address(0)) {

                    (o.wantToken).safeTransferFrom(msg.sender, o.seller, tokenAmount);

                } else {

                    _transfer(o.wantToken, msg.sender, o.seller, o.sellAmount);
