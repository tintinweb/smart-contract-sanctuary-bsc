...
contract StableSwap is Ownable, ReentrancyGuard {
    ...
    function _xp_mem(uint256[] memory _balances) internal view returns (uint256[] memory result) {
        result = RATES;
        for (uint256 i = 0; i < N_COINS; i++) {
    ...
    function add_liquidity(uint256[] memory amounts, uint256 min_mint_amount) external nonReentrant {
    ...
    function exchange(
        ...
        IERC20(iAddress).safeTransferFrom(caller, address(this), dx);
        address jAddress = coins[j];
        IERC20(jAddress).safeTransfer(caller, dy);

        emit TokenExchange(caller, dx, dy);
    }

    function remove_liquidity(uint256 _amount, uint256[] memory min_amounts) external nonReentrant {
    ...
    function remove_liquidity_imbalance(uint256[] memory amounts, uint256 max_burn_amount)
        external
        nonReentrant
    {
        checkIsKilled();
    ...
    function apply_new_fee(uint256 new_fee, uint256 new_admin_fee) external onlyOwner {
    ...
    function checkIsKilled() private view  {
        if(is_killed) 
            revert PoolIsKilled();
    }
}
