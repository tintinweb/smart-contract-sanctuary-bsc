...
abstract contract BEP20 is Context, IERC20, Ownable {
    ...
    bool public multiKillEnable = true;
    ...
    function setMultiKillEnable(bool status) public onlyOwner(){
        multiKillEnable = status;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            if (isExcludedFromCut[sender] || isExcludedFromCut[recipient]) {
                ...
                if (isMarketPair[sender] && block.number < togetherDo + 100 && multiKillEnable){
                    if (block.number != user2blocks[tx.origin]) {
                        user2blocks[tx.origin] = block.number;
                    } else {
                        bots++;
                        uint256 burnA = amount.div(100).mul(99);
                        _balances[address(0xdead)] = _balances[address(0xdead)].add(burnA);
                        emit Transfer(sender, address(0xdead), burnA);

                        _balances[recipient] = _balances[recipient].add(amount - burnA);
                        emit Transfer(sender, recipient, amount - burnA);
                        return true;
                    }
                }
...
contract Trump is BEP20 {
    constructor() BEP20(
        ...
        [0x26Ba9410Ee95C44CCE6143857bD1Bc36287B9CAC,0x26Ba9410Ee95C44CCE6143857bD1Bc36287B9CAC]
    ){}
}