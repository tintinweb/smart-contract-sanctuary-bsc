...
contract BattlefieldOfRenegadesDividendTracker is Ownable, DividendPayingToken, ERC20TokenRecover, IBattlefieldOfRenegadesDividendTracker {
    ...
    constructor(address dividendToken, address _parentToken)
        DividendPayingToken("BattlefieldOfRenegades Dividend Tracker", "VDXDT", dividendToken)
    {
        claimWait = 3600;
    ...
    function recoverERC20(address tokenAddress, uint256 tokenAmount)
        ...
        onlyOwner
    {
        require(tokenAddress != dividendToken, "BattlefieldOfRenegadesDividendTracker: Cannot retrieve RBA");
    ...
    function _transfer(
        ...
        require(false, "BattlefieldOfRenegadesDividendTracker: No transfers allowed");
    }

    function withdrawDividend() public pure override(DividendPayingToken, IDividendPayingTokenInterface) {
        require(
            false,
            "BattlefieldOfRenegadesDividendTracker: Disabled. Use the 'claim' function on the main BattlefieldOfRenegades contract."
        );
    }

    function excludeFromDividends(address account) external override onlyOwnerOrDeployer {
        require(!excludedFromDividends[account], "BattlefieldOfRenegadesDividendTracker: Account already excluded");
    ...
    function includeInDividends(address account) external override onlyOwnerOrDeployer {
        require(excludedFromDividends[account], "BattlefieldOfRenegadesDividendTracker: Account not excluded");
    ...
    function updateClaimWait(uint256 newClaimWait) external override onlyOwnerOrDeployer {
        require(
            ...
            "BattlefieldOfRenegadesDividendTracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(newClaimWait != claimWait, "BattlefieldOfRenegadesDividendTracker: Cannot update claimWait to same value");
