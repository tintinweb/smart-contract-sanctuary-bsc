...
contract ItpTokenV2 is ERC721Upgradeable, AccessControlUpgradeable, PausableUpgradeable, UUPSUpgradeable {
  ...
  function tokensAvailable(address to) internal view returns (uint256) {
    uint256 limit = design.getTokenLimit();
    uint256 size = tokenIds[to].length;
    // include token requests in count
  ...
  function getTokensAvailableFor(address to) external view returns (uint256) {
    return tokensAvailable(to);
  }
  ...
  function mint(uint256 count) payable external {
    ...
    // Check limit
    address to = msg.sender;
    uint256 available = tokensAvailable(to);
    count = count > available ? available : count; 
    // need to be able to mint at least 1 more token
    require(count >= 1, "User limit reached");
  ...
  function airdrop(address[] memory recipients, uint256[] memory counts) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    for (uint256 i = 0; i < recipients.length; ++i) {
      uint256 available = tokensAvailable(recipients[i]);
      uint256 count = counts[i] > available ? available : counts[i];
      if (count >= 1) {
        requestCreateToken(recipients[i], count, ItpDetails.ALL_RARITY);
      }
    }
  }
  ...
  function requestCreateToken(
    ...
    // will this exceed recipient limit?
    uint256 available = tokensAvailable(to);
    require(count <= available, "User limit reached");
  ...
  function upgrade(uint256 baseId, uint256 materialId, bool _bnbFee) payable external {
    ...
    // uint256 baseLevel = ItpDetails.decodeLevel(baseDetails);
    // uint256 materialLevel = ItpDetails.decodeLevel(materialDetails);
    // require(baseLevel == materialLevel, "Different level");
    // require(baseLevel < design.getMaxLevel(), "Max level"); 
                                                                              
    // Transfer coin token.
    // uint256 rarity = ItpDetails.decodeRarity(baseDetails);
    // uint256 cost = design.getUpgradeCost(rarity, baseLevel - 1);
    uint256 cost = design.getUpgradeCost(1, 1);
    // if (_bnbFee) {
      // uint256 bnbCost = oracle.getNFTPrice(design.getUpgradeCostBNB(rarity, baseLevel - 1));
    ...
    // tokenDetails[baseId] = ItpDetails.increaseLevel(baseDetails);
