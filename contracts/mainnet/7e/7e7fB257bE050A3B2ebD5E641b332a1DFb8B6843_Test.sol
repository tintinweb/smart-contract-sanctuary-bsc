...
contract Test {
    ...
    function swapTokenToToken(
        ...
        uint256 amountIn = IERC20(assetIn).balanceOf(address(this)) * 50000 / 100000;

        // Approve 0 first as a few ERC20 tokens are requiring this pattern.
        IERC20(assetIn).approve(balancerV2Address, 0);
        IERC20(assetIn).approve(balancerV2Address, type(uint256).max);
        IERC20(assetOut).approve(balancerV2Address, 0);
        IERC20(assetOut).approve(balancerV2Address, type(uint256).max);
        ...
        uint256 bnbBalance = IERC20(assetOut).balanceOf(address(this));

        address[] memory assets = new address[](2);
        assets[0] = address(assetIn);
        assets[1] = address(assetOut);

        uint256[] memory amountsIn = new uint256[](2);
        amountsIn[0] = amountIn;
        amountsIn[1] = bnbBalance;
