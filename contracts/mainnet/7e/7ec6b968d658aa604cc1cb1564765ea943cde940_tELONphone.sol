...
contract tELONphone is Context, ERC20, Ownable {
    ...
    constructor (address payable mktgWallet, address payable devWallet, address payable liqWallet) ERC20("tELONphone", "TEP") {
    ...
    function setBuyFee(uint256 _buyMktgFee, uint256 _buyDevFee, uint256 _buyLiqFee) public onlyOwner {
        require(_buyMktgFee.add(_buyDevFee).add(_buyLiqFee) <= 100, "Must keep buy taxes below 10%");
    ...
    function setSellFee(uint256 _sellMktgFee, uint256 _sellDevFee, uint256 _sellLiqFee) public onlyOwner {
        require(_sellMktgFee.add(_sellDevFee).add(_sellLiqFee) <= 100, "Must keep sell taxes below 10%");
