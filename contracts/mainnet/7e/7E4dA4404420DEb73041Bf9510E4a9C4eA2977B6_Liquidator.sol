...
contract Liquidator is IPancakeCallee {
    ...
    function liquidateSNWithPrice(
        ...
        liquidateSN(_borrowers, _qiTokens);
    }

    function liquidateSN(address[] calldata _borrowers, address[] calldata _qiTokens) public {
        uint i;

        for (i = 0; i < _borrowers.length; i++) {
            liquidateS(_borrowers[i], _qiTokens[i * 2], _qiTokens[i * 2 + 1]);
        }
    }
    ...
    function liquidateSWithPrice(
        ...
        string[] calldata _symbols,
        address _borrower,
        address _repayQiToken,
        address _seizeQiToken
    ) external {
        priceOracle.postPrices(_messages, _signatures, _symbols);
        liquidateS(_borrower, _repayQiToken, _seizeQiToken);
    }
     ...
     * @param _repayQiToken (address): a QiToken for which the user is in debt
     * @param _seizeQiToken (address): a QiToken for which the user has a supply balance
     */
    function liquidateS(address _borrower, address _repayQiToken, address _seizeQiToken) public {
        ...
        uint repayMax = ABep20(_repayQiToken).borrowBalanceCurrent(_borrower) * closeFact / uint(10**18);
        uint seizeMax = ABep20(_seizeQiToken).balanceOfUnderlying(_borrower) * uint(10**18) / liqIncent;

        uint uPriceRepay = priceOracle.getUnderlyingPrice(_repayQiToken);
        // Gas savings -- instead of making new vars `repayMax_Avax` and `seizeMax_Avax` just reassign
        repayMax *= uPriceRepay;
        seizeMax *= priceOracle.getUnderlyingPrice(_seizeQiToken);
        ...
        liquidate(_borrower, _repayQiToken, _seizeQiToken, ((repayMax < seizeMax) ? repayMax : seizeMax) / uPriceRepay);
    }
     ...
     * @param _repayQiToken (address): a QiToken for which the user is in debt
     * @param _seizeQiToken (address): a QiToken for which the user has a supply balance
     * @param _amount (uint): the amount (specified in units of _repayQiToken.underlying) to flash loan and pay off
     */
    function liquidate(address _borrower, address _repayQiToken, address _seizeQiToken, uint _amount) public {
        address pair;
        address r;

        if (_repayQiToken == _seizeQiToken || _seizeQiToken == ABNB) {
            r = ABep20Storage(_repayQiToken).underlying();
            pair = PancakeLibrary.pairFor(FACTORY, r, WBNB);
        }
        else if (_repayQiToken == ABNB) {
            r = WBNB;
            pair = PancakeLibrary.pairFor(FACTORY, WBNB, ABep20Storage(_seizeQiToken).underlying());
        }
        else {
            r = ABep20Storage(_repayQiToken).underlying();
            uint maxBorrow;
            (maxBorrow, , pair) = PancakeLibrary.getReservesWithPair(FACTORY, r, ABep20Storage(_seizeQiToken).underlying());

            if (_amount * 100000 > maxBorrow * SLIPPAGE_THRESHOLD_FACT) pair = IPancakeFactory(FACTORY).getPair(r, WBNB);
        }

        // Initiate flash swap
        bytes memory data = abi.encode(_borrower, _repayQiToken, _seizeQiToken);
    ...
    function pancakeCall(address sender, uint amount0, uint amount1, bytes calldata data) override external {
        ...
        (address borrower, address repayQiToken, address seizeQiToken) = abi.decode(data, (address, address, address));
        ...
        if (repayQiToken == seizeQiToken) {
            uint amount = amount0 != 0 ? amount0 : amount1;
            address estuary = amount0 != 0 ? token0 : token1;

            // Perform the liquidation
            IERC20(estuary).safeApprove(repayQiToken, amount);
            ABep20(repayQiToken).liquidateBorrow(borrower, amount, seizeQiToken);

            // Redeem aTokens for underlying ERC20
            ABep20(seizeQiToken).redeem(IERC20(seizeQiToken).balanceOf(address(this)));
        ...
        if (repayQiToken == ABNB) {
            ...
            // Perform the liquidation
            ABnb(repayQiToken).liquidateBorrow{value: amount}(borrower, seizeQiToken);
        ...
        if (seizeQiToken == ABNB) {
            uint amount = amount0 != 0 ? amount0 : amount1;
            address source = amount0 != 0 ? token0 : token1;

            // Perform the liquidation
            IERC20(source).safeApprove(repayQiToken, amount);
            ABep20(repayQiToken).liquidateBorrow(borrower, amount, seizeQiToken);
        ...
        // Redeem aTokens for underlying ERC20 or BNB
        uint seized_uUnits = ABep20(seizeQiToken).balanceOfUnderlying(address(this));
        ABep20(seizeQiToken).redeem(IERC20(seizeQiToken).balanceOf(address(this)));
        address seizeUToken = ABep20Storage(seizeQiToken).underlying();
