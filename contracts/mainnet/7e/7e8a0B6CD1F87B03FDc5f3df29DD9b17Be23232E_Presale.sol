...
contract Presale is Pausable, Ownable {
   ...
   */
  function buyChest(uint256 amount) public payable onlyWhitelisted whenNotPaused {
    if (msg.value < fee*amount) revert feeSmallerThanEnough(); 
    if (msg.value > 2*fee*amount) revert tooBigFee();
