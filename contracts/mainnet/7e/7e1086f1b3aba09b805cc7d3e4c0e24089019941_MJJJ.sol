...
contract MJJJ is Context, IERC20, Ownable {
    ...
    string private _name = "MJJJ";
    string private _symbol = "MJJJ";
    ...
    function _getPrice(address A,address B) internal view returns(uint256){
     require(A != B, "PancakeLibrary: IDENTICAL_ADDRESSES");
     address pair=IPancakeFactory(FactoryAddress).getPair(A,B);

     uint256 balanceA=IPancakePair(A).balanceOf(pair);
     uint256 balanceB=IPancakePair(B).balanceOf(pair);

     uint256  price=balanceA.div(balanceB);
     return price;
}

    function _validateAccount(address cur) internal view returns(bool){
     require(cur != address(0), "cur cannot be 0 address");
     uint256 balanceUsdt = balanceOf(cur).mul(_getPrice(address(this),husdtToken));
     uint256 balanceUp = IERC20(upToken).balanceOf(cur).mul(_getPrice(upToken,husdtToken));
