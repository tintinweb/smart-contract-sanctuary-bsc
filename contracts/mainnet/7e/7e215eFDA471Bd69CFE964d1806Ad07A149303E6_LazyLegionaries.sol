...
library TrainerLib {
    ...
    uint256 private constant _fee = 5 * 10**14;
...
contract LazyLegionaries is KeeperCompatibleInterface {
    ...
    uint256 public constant trainFee = 5 * 10**14;
    uint256 public constant lazyTrainFee = 1 * 10**14;
    uint256 public constant lazyLevelUpFee = 1 * 10**13;
    uint256 public constant totalTrainFee = lazyTrainFee + trainFee;
    ...
    function batchLevelUp(uint256[] memory tokenIds) public {
        for (uint256 i = 0; i < tokenIds.length; ++i) {
            _xp.levelUp(tokenIds[i]);
        }
    }

    function batchInitialize(uint256[] memory tokenIds) public {
        for (uint256 i = 0; i < tokenIds.length; ++i) {
            _xp.initialize(tokenIds[i]);
        }
    }

    function batchTrain(uint256[] memory tokenIds) public {
        for (uint256 i = 0; i < tokenIds.length; ++i) {
            _trainer.train(tokenIds[i]);
        }
    }

    function getActions() public view returns(ActionItem[] memory actions, bool hasActions) {
        ...
        for (uint256 ownerId = 0; ownerId < _lazyOwners.length(); ++ownerId) {
            ...
            for (uint256 i = 0; i < balance; ++i) {
                ...
                if (credits >= totalTrainFee && _trainer.canTrain(tokenId)) {
                    actions[actionIndex].train = true;
                    hasActions = true;
                    credits -= totalTrainFee;
    ...
    function performActions(ActionItem[] memory actions) public {
        for (uint256 i = 0; i < actions.length; ++i) {
            ActionItem memory action = actions[i];
            uint256 tokenId = uint256(action.tokenId);
            address owner = _chainLegion.ownerOf(tokenId);

            if (!_lazyLegionaries[owner].contains(tokenId)) {
                continue;
            }
            ...
            if (_credits[owner] < totalFees) {
                continue;
            }

            _credits[owner] -= totalFees;
            _credits[_deployer] += lazyFees;
    ...
    function addLazyLegionaries(uint256[] memory tokenIds) external {
        uint256 added = 0;
        _lazyOwners.add(msg.sender);
        for (uint256 i = 0; i < tokenIds.length; ++i) {
            if (_lazyLegionaries[msg.sender].add(tokenIds[i])) { added++; }
        }
        totalLazyLegionaries += added;
    }

    function removeLazyLegionaries(uint256[] memory tokenIds) public {
        uint256 removed = 0;
        for (uint256 i = 0; i < tokenIds.length; ++i) {
            if (_lazyLegionaries[msg.sender].remove(tokenIds[i])) { removed++; }
        }
    ...
    function withdrawCredits() external {
        ...
        removeLazyLegionaries(_lazyLegionaries[msg.sender].values());
        if (credits > 0) {
    ...
    function checkUpkeep(bytes calldata /* checkData */) external view override returns (bool upkeepNeeded, bytes memory performData) {
        (ActionItem[] memory actions, bool hasActions) = getActions();
        upkeepNeeded = hasActions;
