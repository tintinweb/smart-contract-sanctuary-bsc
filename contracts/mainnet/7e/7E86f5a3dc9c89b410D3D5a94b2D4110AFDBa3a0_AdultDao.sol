...
contract AdultDao is BEP20 {

    // Buy Taxes
    // Transfer tax rate in basis points. (default 10%)
    uint16 public buyTransferTaxRate = 1000; // divider = 10000
    
    // Burn Fee, in percent of transferTaxRate.
    uint16 public buyBurnFee = 20; // divider = 100
    
    // Liq Fee, in percent of transferTaxRate.
    uint16 public buyLiqFee = 20; // divider = 100

    // Liq Fee, in percent of transferTaxRate.
    uint16 public buyEcoFee = 60; // divider = 100

    // Sell Taxes
    // Transfer tax rate in basis points. (default 10%)
    uint16 public sellTransferTaxRate = 1000; // divider = 10000
    
    // Burn Fee, in percent of transferTaxRate.
    uint16 public sellBurnFee = 20; // divider = 100
    
    // Liq Fee, in percent of transferTaxRate.
    uint16 public sellLiqFee = 20; // divider = 100

    // Liq Fee, in percent of transferTaxRate.
    uint16 public sellEcoFee = 60; // divider = 100

    // Ecosystem address /LP and Token Tax receiver addr.
    address public ecoAddr = 0x1eBD56fc500e36603aCE75d5209F757cD4A03c68;
    
    // The lpEarner Address, receives the LPs created by Auto Lquidity Function.
    address public lpEarner = 0x822d7C9087b38c57996912fF57CAaE273A3242f6;
    
    // Max transfer tax rate: 20%.
    uint16 public constant maxTaxRate = 2000;
    
    // Burn address
    address public brunAddr = 0x000000000000000000000000000000000000dEaD;
    ...
    // Initial Token Supply, not extendable.
    uint256 public tSupply = 100 * 10 ** 9 * (10 ** 18);
    ...
    event BuyTransferTaxRateUpdated(address indexed operator, uint256 previousRate, uint256 newRate);
    ...
    modifier transferTaxFree {
        ...
        uint16 _buyTransferTaxRate = buyTransferTaxRate;
        sellTransferTaxRate = 0;
        buyTransferTaxRate = 0;
        _;
        sellTransferTaxRate = _sellTransferTaxRate;
        buyTransferTaxRate = _buyTransferTaxRate;
    }

    /**
     * @notice Constructs the Token contract.
     */
    constructor() public BEP20("AdultDao", "ADULT") {
        _operator = _msgSender();
        emit OperatorTransferred(address(0), _operator);

        // Set BSC Pancake Router -MAINNET: 0x10ED43C718714eb63d5aA57B78B54704E256024E  -TESTNET: 0xD99D1c33F9fC3444f8101754aBC46c52416550D1
        ...
        // Mint the token total supply to the contract creator
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        bool _isNonTaxedRecipient = isExcludedFromTaxReceiver(recipient);

        // Token transfer when there is no tax or sender/recipient is excluede from tax
        if (_isNonTaxedSender || _isNonTaxedRecipient || buyTransferTaxRate == 0 || sellTransferTaxRate == 0 ) {
            super._transfer(sender, recipient, amount);  // transfer with no Tax
        }

        // buying
        else if (sender == swapPair)   {
        // Buy Tax amounts calculations
            uint256 taxAmount = amount.mul(buyTransferTaxRate).div(10000);
            uint256 burnAmount = taxAmount.mul(buyBurnFee).div(100);
            uint256 liqAmount = taxAmount.mul(buyLiqFee).div(100);
            uint256 ecoAmount = taxAmount.mul(buyEcoFee).div(100);
            require(taxAmount >= burnAmount + liqAmount + ecoAmount, "TOKEN::transfer: wrong Tax calculation!");            
        ...
        // token Transfer
            ...
            super._transfer(sender, ecoAddr, ecoAmount);
            super._transfer(sender, recipient, sendAmount);
        }

        // selling
        else if (recipient == swapPair) {
        // Sell Tax amounts calculations
            uint256 taxAmount = amount.mul(sellTransferTaxRate).div(10000);
            uint256 burnAmount = taxAmount.mul(sellBurnFee).div(100);
            uint256 liqAmount = taxAmount.mul(sellLiqFee).div(100);
            uint256 ecoAmount = taxAmount.mul(sellEcoFee).div(100);
            require(taxAmount >= burnAmount + liqAmount + ecoAmount, "TOKEN::transfer: wrong Tax calculation!");            
        ...
        // regular Transfer, that is no buy/sell and no tax free address sending or receiving
        else {
             super._transfer(sender, recipient, amount);  // transfer with no Tax
        }
    ...
    // Update the buy transfer tax rate.
    function updateBuyTransferTaxRate(uint16 _buyTransferTaxRate) public onlyOperator {
        require(_buyTransferTaxRate <= maxTaxRate, "TOKEN::updateBuyTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
        emit BuyTransferTaxRateUpdated(msg.sender, buyTransferTaxRate, _buyTransferTaxRate);
        buyTransferTaxRate = _buyTransferTaxRate;
    }

    
    // Update the sell transfer tax rate.
    function updateSellTransferTaxRate(uint16 _sellTransferTaxRate) public onlyOperator {
        require(_sellTransferTaxRate <= maxTaxRate, "TOKEN::updateSellTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
        emit SellTransferTaxRateUpdated(msg.sender, sellTransferTaxRate, _sellTransferTaxRate);
        sellTransferTaxRate = _sellTransferTaxRate;
    }

    // Update all Buy Fees
    function updateAllBuyFees(uint16 _buyBurnFee, uint16 _buyLiqFee, uint16 _buyEcoFee) public onlyOperator {
        require(_buyBurnFee + _buyLiqFee + _buyEcoFee == 100, "TOKEN::updateAllBuyFees: Summ of all Buy Fees must be 100!");        
    ...
    // Update all Buy Fees
    function updateAllSellFees(uint16 _sellBurnFee, uint16 _sellLiqFee, uint16 _sellEcoFee) public onlyOperator {
        require(_sellBurnFee + _sellLiqFee + _sellEcoFee == 100, "TOKEN::updateAllSellFees: Summ of all Sell Fees must be 100!");        
    ...
    // Update the swap Router and Pair
    ...
    // Transfers Ecosystem Address to a new Address.
    function transferEcoAddr(address _ecoAddr) public onlyOperator {    
        ecoAddr = _ecoAddr;
    }
    
    // Transfers Burn Address to a new Address.
    function transferBurnAddr(address _brunAddr) public onlyOperator {    
        brunAddr = _brunAddr;
    }
    
    // function to withdraw ETH from this Contract
    function withdrawEth() public onlyOperator {
        require(address(this).balance > 0, "TOKEN::withdrawEth: No ETH balance");
    ...
    function withdrawToken(address _token, uint256 _amount) public onlyOperator {
        require(_token != address(this), "TOKEN::withdrawToken: Withdraw-Token can not be this Token!");
