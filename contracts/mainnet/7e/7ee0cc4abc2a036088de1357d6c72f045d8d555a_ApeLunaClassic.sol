...
Hyper-deflationary token, Not simply a Project, it is a Movement Dedicated in helping the LUNA CLASSIC #LUNC community Burning $LUNC tokens, that will help to increase the LUNC value.
Visit our Telegram and become part of the Ape Luna Classic family!
...
contract ApeLunaClassic is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ApeLunaClassicDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Ape Luna Classic", "ALUNC") {
        marketingWallet = payable(0x02cf23239C845BB92d804689B2f3a511097c1592);
        devWallet = payable(0x6298d46c23B4c32f5aabc7075d5E2ADb6bAdD0e1);
        ...
        dividendTracker = new ApeLunaClassicDividendTracker(
            payable(this),
            router,
            0x156ab3346823B651294766e23e6Cf87254d68962,
            "ApeLunaClassicTRACKER",
            "ALUNCTRACKER"
        );
...
contract ApeLunaClassicDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    ApeLunaClassic public ApeLunaClassicContract;
    ...
    constructor(
        ...
        defaultToken = token;
        ApeLunaClassicContract = ApeLunaClassic(mainContract);
    ...
    function _transfer(
        ...
        require(false, "ApeLunaClassic_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "ApeLunaClassic_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ApeLunaClassic contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(ApeLunaClassicContract);

            uint256 prevBalance = ApeLunaClassicContract.balanceOf(address(this));
            ...
            {
                uint256 received = ApeLunaClassicContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    ApeLunaClassicContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
