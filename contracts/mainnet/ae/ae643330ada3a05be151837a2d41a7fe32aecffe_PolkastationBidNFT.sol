...
interface IGnftPolkaStartPools {
...
contract PolkastationBidNFT is 
    IBidNFT, 
    ERC721Holder, 
    Ownable, 
    Pausable 
{
    ...
    // INO nft id sold by polka or not
    ...
    function buyTokenTo(uint256 _tokenId, address _to) public override whenNotPaused {
        ...
        if(InoNftList[_tokenId] && nftInoSold[_tokenId] != true){
            // very first time selling nft INO item (selling by polka)
        ...
        if (nftPoolAddr != address(0)) {
            IGnftPolkaStartPools(nftPoolAddr).updateGenesisNft(_tokenId, _to, address(nft));
        }
    ...
    function readyToSellTokenTo(
        ...
        //  admin add genesis nft data to nft pool so later on user will able to stake it
        if (nftPoolAddr != address(0) && address(_msgSender()) == genesisNftCreator) {
            IGnftPolkaStartPools(nftPoolAddr).addGenesisNft(
                _tokenId, 
                _price, 
                address(this), 
                address(nft),
        ...
        // admin add genesis INO nft data to nft pool so later on user will able to stake it
