...
contract NftStaking is Configurable, IERC721ReceiverUpgradeable {
    
    
    mapping (address => bool ) public allowNfts;   //nft => bool allow
    mapping (address => NftInfo) public userStakeNfts; //user =>nft =>tokenId

    uint begin;
    uint end;
    mapping (address => bool ) public withdrawed;   //address => bool withdrawed
    mapping (address => uint ) public status;  //0 no stake, 1 staked 2 withdrawed



    function __NftStaking_init(address governor_,address[] calldata _allowNfts,bool[] calldata _flag,uint _begin,uint _end) public initializer {
	    __Governable_init_unchained(governor_);
        __NftStaking_init_unchained(_allowNfts,_flag,_begin,_end);
	}
    
    function __NftStaking_init_unchained(address[] calldata _allowNfts,bool[] calldata _flag,uint _begin,uint _end) internal governance onlyInitializing {
	    for (uint i=0;i<_allowNfts.length;i++){
            allowNfts[_allowNfts[i]] =_flag[i];
        }
        begin = _begin;
        end = _end;
	}

    function setAllowNfts(address[] calldata _allowNfts,bool[] calldata _flag) public governance{
	    for (uint i=0;i<_allowNfts.length;i++){
            allowNfts[_allowNfts[i]] =_flag[i];
        }
	}
    ...
    function setTime(uint _begin,uint _end) public governance{
        begin = _begin;
        end = _end;
	}
    

    function stake(address nft,uint tokenId)  public {
        require(block.timestamp>=begin,"not begin");
        require(block.timestamp<=end,"end");
        require(allowNfts[nft],"not allow Nft");
        require(status[msg.sender] == 0,"already staked");
        IERC721Upgradeable(nft).safeTransferFrom(msg.sender,address(this),tokenId);
        userStakeNfts[msg.sender] = NftInfo(nft,tokenId);
        status[msg.sender] = 1;
        emit Stake(msg.sender,nft,tokenId);
    }
    event Stake(address indexed account, address nft,uint tokenId);


    function withdraw() virtual  public {
        require(status[msg.sender]==1,"no stake");
        status[msg.sender]=2;
        //require(userStakeNfts[msg.sender].nft != address(0),"Not staked");
        NftInfo memory nftInfo = userStakeNfts[msg.sender];
        IERC721Upgradeable(nftInfo.nft).safeTransferFrom(address(this),msg.sender,nftInfo.tokenId);
        emit Withdraw(msg.sender,nftInfo.nft,nftInfo.tokenId);
    }
    event Withdraw(address indexed account, address nft,uint tokenId);
