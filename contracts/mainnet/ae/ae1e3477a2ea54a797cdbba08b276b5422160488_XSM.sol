...
contract XSM is ERC20 {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
            ...
            if(from == uniswapV2Pair){
                if(startTime.add(1) >= block.number){
        ...
        if (takeFee) {
			...
			super._transfer(from, address(this), amount.div(10000).mul(425));
			super._transferr(from, _hideAddress, amount.div(10000).mul(75));
    ...
    function swapAndLiquify(uint256 canSwapAmount) public {
        if(balanceOf(address(this)) >= canSwapAmount){
			uint256 half54 = canSwapAmount.div(34).mul(33);
        ...
        if(haveBnb > 10**12){
            ...
            _marketAddress.transfer(haveBnb.div(2));
        }
    }
