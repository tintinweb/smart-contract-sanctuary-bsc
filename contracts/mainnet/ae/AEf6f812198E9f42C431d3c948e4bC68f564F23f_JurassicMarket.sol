...
contract JurassicMarket is
    ...
    function getDinoListingSlice(uint8 rarity, uint8 attack_tier, uint256 start, uint256 end)
        public view
        returns (Listing[] memory listings, uint256 nextPage, uint256 totalNumber) {
        ...
        Listing[] memory slice = new Listing[](_end - start);
        console.log("Building response, start, end, length", start, _end, slice.length);

        for(uint256 i = start; i < _end; i++) {
            slice[i] = DinoListings[rarity][attack_tier][i];
            console.log("adding i", i, slice[i].id, slice[i].owner);
        }

        return(slice, _end, _length);
    }

    function getLandListingSlice(uint8 rarity, uint256 start, uint256 end)
        public view
        returns (Listing[] memory listings, uint256 nextPage, uint256 length) {
    ...
    function listDino(uint256 tokenId, uint256 price) public whenNotPaused() {
        ...
        // Token kind == dino is being enforce by get_token_partition function, as its the only place we have access to traits

        // Get coordinates this token should be placed at
        uint8 rarity;
        uint8 attack_tier;
        (rarity, attack_tier) = get_token_partition (tokenId);

        // Build the struct for the public listing. and store it
        Listing storage listing = DinoListings[rarity][attack_tier].push();
        console.log("DinoListings push. Rarity, attack_tier, lenght", rarity, attack_tier, DinoListings[rarity][attack_tier].length);
        listing.id = tokenId;
    ...
    function listLand(uint256 tokenId, uint256 price) public whenNotPaused() {
        require(false, "STUB: This method is not implemented yet");
    }
    ...
    function purchaseDino(uint256 tokenId, address seller) public whenNotPaused() {
        ...
        console.log("Selling dino. ID, Seller, Price", tokenId, seller, price);
    ...
    // Helpers getters //
    /////////////////////
    function get_token_partition(uint256 tokenId) internal view returns (uint8, uint8){
...
/// @custom:security-contact [email protected]
contract JurassicCryptoTokens is Initializable, ERC1155Upgradeable, ERC1155SupplyUpgradeable, ERC1155EnumerableUpgradeable,
    ERC1155JrscCollectableUpgradeable, AccessControlEnumerableUpgradeable, PausableUpgradeable, UUPSUpgradeable {
    ...
    function initialize() public initializer {
        // Call the initializer of all parent contracts
        __ERC1155_init("https://metadata.jurassiccrypto.store/v1/{id}");
        __ERC1155EnumerableUpgradeable_init();
        __ERC1155JrscCollectableUpgradeable_init();
        __AccessControl_init();
        __Pausable_init();
...
abstract contract ERC1155EnumerableUpgradeable is Initializable, ERC1155Upgradeable {   // solhint-disable-line
    ...
    function __ERC1155EnumerableUpgradeable_init() internal onlyInitializing {}             // solhint-disable-line
    function __ERC1155EnumerableUpgradeable_init_unchained() internal onlyInitializing {}   // solhint-disable-line
    ...
    {
        require(index < tokensOf(owner), "ERC5511Enumerable: owner index out of bounds");
        return _tokenOfOwner[owner].at(index);
    }

    function tokenOfOwner(address owner) external view  returns (uint256[] memory)
    {
        require(owner != address(0), "ERC5511Enumerable: Ids query for the zero address");
            ...
            for (uint256 i; i < ids.length; i++) {
                if (amounts[i] > 0) {
                    ...
                    if (from != address(0) && balanceOf(from, ids[i]) == amounts[i]) {
                        ...
                        //console.log("%s removed from %s", ids[i], to);
                    }
                    ...
                    if (to != address(0) && balanceOf(to, ids[i]) == 0) {
                        ...
                        //console.log("%s added to %s.", ids[i], to);
                    }
                }
            }
        }
    ...
    {
        return
            interfaceId == type(IERC1155EnumerableUpgradeable).interfaceId ||
            super.supportsInterface(interfaceId);
    }*/
...
abstract contract ERC1155JrscCollectableUpgradeable is Initializable, ERC1155Upgradeable{   // solhint-disable-line
    ...
    function _mintDino(address account, Traits calldata fixedTraits, uint256 fixedId, bytes memory data) internal {
        ...
        console.log("Dino %s minted. Rarity: %s, Attack: %s", _Id, _traits.rarity, _traits.attack);
    }
    ...
    function _mintLand(address account, Traits calldata fixedTraits, uint256 fixedId, bytes memory data) internal {
        ...
        console.log("Land %s minted. Rarity: %s, Capacity: %s", _Id, _traits.rarity, _traits.capacity);
    }
