...
contract HokbMinter is Context, Ownable {
  ...
  function MintNew(address to,uint256 refid) external returns (bool) {
    ...
    uint256 updateRefRewardAmount = buyprice*120/1000;
    IERC20(tokenAddress).transfer(refwallet,updateRefRewardAmount);
    ...
    triggeradr = safereferral(to);
    st[0] = triggeradr;
    participants[triggeradr][1] += 1;
