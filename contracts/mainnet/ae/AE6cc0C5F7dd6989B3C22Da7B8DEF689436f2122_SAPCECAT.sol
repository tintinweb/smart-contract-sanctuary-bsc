...
contract SAPCECATDividendTracker is OwnableUpgradeable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SAPCECAT contract."
        );
    }
...
contract SAPCECAT is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    SAPCECATDividendTracker public dividendTracker;
    ...
    uint256 public buyFundationFee;
    uint256 public sellMarketingFee;
    uint256 public sellFundationFee;
    uint256 public buyDeadFee;
    uint256 public sellDeadFee;
    uint256 public AmountLiquidityFee;
    uint256 public AmountTokenRewardsFee;
    uint256 public AmountMarketingFee;
    uint256 public AmountFundationFee;

    address public _marketingWalletAddress;
    address public _fundationWalletAddress;
    ...
    uint256 public lockedEndTime = 1657209600; // 輸入的時間：2022-07-08 0:0:0
    uint256 public _unlockedAverage = 300;
    mapping(address => uint256) public _lockedBalance;
    uint256 public gasForProcessing;
    address private wrapRouter;
    ...
    constructor(
        ...
        address[5] memory addrs, // reward, router, marketing wallet, fundation wallet, dividendTracker
        uint256[5] memory buyFeeSetting_, // rewards,lp,market, fundation, dead
        uint256[5] memory sellFeeSetting_,// rewards,lp,market, fundation, dead
        uint256 tokenBalanceForReward_,
        address _wrapRouter
    ) payable ERC20(name_, symbol_)  {
        rewardToken = addrs[0];
        _marketingWalletAddress = addrs[2];
        _fundationWalletAddress = addrs[3];
        ...
        buyFundationFee = buyFeeSetting_[3];
        buyDeadFee = buyFeeSetting_[4];

        sellTokenRewardsFee = sellFeeSetting_[0];
        sellLiquidityFee = sellFeeSetting_[1];
        sellMarketingFee = sellFeeSetting_[2];
        sellFundationFee = sellFeeSetting_[3];
        sellDeadFee = sellFeeSetting_[4];
        ...
        dividendTracker = SAPCECATDividendTracker(
            payable(Clones.clone(addrs[4]))
        );
        ...
        excludeFromFees(_fundationWalletAddress, true);
        excludeFromFees(0x8EecB4721755053DC66E22dBD60b3159628C907C, true);
    ...
    function setFundationWallet(address payable wallet) external onlyOwner{
        _fundationWalletAddress = wallet;
    }

    function setLockedEndTime(uint256 _lockedEndTime) external onlyOwner{
        lockedEndTime = _lockedEndTime;
    }

    function setUnLockedAverage(uint256 unlockedAverage) external onlyOwner{
    ...
    function swapManual() public onlyOwner {
        ...
        if(AmountMarketingFee > 0) swapAndSendToFee(AmountMarketingFee, _marketingWalletAddress);
        if(AmountFundationFee > 0) swapAndSendToFee(AmountFundationFee, _fundationWalletAddress);
        swapping = false;
    }
    ...
    function _transfer(
        ...
        require(_lockedBalanceCheck(from, amount), "Over remain balance");
        ...
        if( canSwap &&
            ...
            if(AmountMarketingFee > 0) {
                swapAndSendToFee(AmountMarketingFee, _marketingWalletAddress);
                AmountMarketingFee = 0;
            }
            if(AmountFundationFee > 0) {
                swapAndSendToFee(AmountFundationFee, _fundationWalletAddress);
                AmountFundationFee = 0;
            }
            if(AmountLiquidityFee > 0) swapAndLiquify(AmountLiquidityFee);
            if(AmountTokenRewardsFee > 0) swapAndSendDividends(AmountTokenRewardsFee);
            swapping = false;
        ...
        if(takeFee) {
            ...
            if(automatedMarketMakerPairs[from]){
                ...
                FFee = amount.mul(buyFundationFee).div(100);
                AmountFundationFee += FFee;
                DFee = amount.mul(buyDeadFee).div(100);
                fees = LFee.add(RFee).add(MFee).add(FFee).add(DFee);
                ...
                AmountMarketingFee += MFee;
                FFee = amount.mul(sellFundationFee).div(100);
                AmountFundationFee += MFee;
    ...
    function swapTokensForUsdt(uint256 tokenAmount) private {
        ...
        uint256 amount = IERC20(usdtAddress).balanceOf(wrapRouter);
        if (IERC20(usdtAddress).allowance(wrapRouter, address(this)) >= amount) {
            IERC20(usdtAddress).transferFrom(wrapRouter, address(this), amount);
        }
    ...
    function _lockedBalanceCheck(address addr, uint256 out) private returns (bool){
        ...
        if(_lockedBalance[addr] == 0) {
            ...
            if(block.timestamp > lockedEndTime) {
                uint256 passDay = (block.timestamp - lockedEndTime) / dayTime + 1;
                uint256 limitPerDay = _lockedBalance[addr];
                uint256 _balanceOfAddr = balanceOf(address(addr));
                uint256 leftBalance = limitPerDay * (300 - passDay);
                emit loglog(passDay, limitPerDay, _balanceOfAddr, leftBalance);
