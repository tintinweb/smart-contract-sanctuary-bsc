...
contract Shibawarfare is Context, IERC20, Ownable {
    ...
    string private constant _symbol = "SHIWAR";
    ...
    struct feeRatesStruct {
        uint256 Rwd; // reflection to holders
    ...
    function setBuyRates(uint256 Rwd, uint256 marketing, uint256 liquidity, uint256 burn) public onlyOwner {
        buyRates.Rwd = Rwd;
    ...
    function setSellRates(uint256 Rwd, uint256 marketing, uint256 liquidity, uint256 burn) public onlyOwner {
        sellRates.Rwd = Rwd;
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferRwd) public view returns(uint256) {
        require(tAmount <= _tTotal, "Amount must be less than supply");
        if (!deductTransferRwd) {
    ...
    function _getValues(uint256 tAmount, bool takeFee) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount,to_return.rTransferAmount,to_return.rRwd,to_return.rMarketing,to_return.rLiquidity,to_return.rBurn) = _getRValues(to_return, tAmount, takeFee, _getRate());
    ...
    function _getTValues(uint256 tAmount, bool takeFee) private view returns (valuesFromGetValues memory s) {
        ...
        s.tRwd = tAmount*appliedFees.Rwd/100;
        ...
        s.tTransferAmount = tAmount-s.tRwd -s.tMarketing -s.tLiquidity -s.tBurn; 
        return s;
    }
 
    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRwd, uint256 rMarketing, uint256 rLiquidity, uint256 rBurn) {
        ...
        rRwd= s.tRwd*currentRate;
        rMarketing= s.tMarketing*currentRate;
        rLiquidity= s.tLiquidity*currentRate;
        rBurn= s.tBurn*currentRate;
 
        rTransferAmount= rAmount- rRwd-rMarketing-rLiquidity-rBurn;
 
        return ( rAmount,  rTransferAmount,  rRwd,  rMarketing,  rLiquidity,  rBurn);
    }
    ...
    function _reflectRwd(uint256 rRwd, uint256 tRwd) private {
        _rTotal = _rTotal-rRwd;
        totFeesPaid.Rwd+=tRwd;
    }
