...
contract Token is ERC20, Ownable {
    ...
    {
        uint256 initSupply = 180000000 * 10**uint256(decimals());
    ...
    function setLiquidity(address _liquidity) public onlyOwner {
        ...
        super._transfer(address(this), liquidity, _totalSupply.div(10));
    }

    function setPool(address _pool) public onlyOwner {
        require(pool == address(0), "already exists");
        pool = _pool;
        super._transfer(address(this), pool, _totalSupply.mul(9).div(10));
    }
    ...
    function sellToken(uint256 slipPoint) public {
        require(msg.sender == sellAddress, "msg.sender can't sell token");
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = address(usdt);
        uint256 amount = balanceOf(address(this));
        uint256 usdtOut = uint256(usdt.balanceOf(pair))
            .mul(1e18)
            .div(uint256(balanceOf(pair)))
            .mul(amount)
            .div(1e18);
        uint256[] memory amounts = router.swapExactTokensForTokens(
            amount,
            usdtOut.mul(slipPoint).div(1000),
            path,
            pool,
            block.timestamp
        );
        uint256 sellUsdtAmount = amounts[amounts.length - 1];
        emit SellToken(amount, sellUsdtAmount, block.timestamp);
    }
}
