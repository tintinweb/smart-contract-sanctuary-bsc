...
contract PelosiToken is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "Killer Pelosi";
    string private _symbol = "Killer Pelosi";
    ...
    address payable public marketingWalletAddress = payable(0x4831F9cfA4F1DD99b838f3FAE5522c7953CbB801);
    address payable public liquidityWalletAddress = payable(0x4831F9cfA4F1DD99b838f3FAE5522c7953CbB801);
    ...
    function Launch(uint256 kill) public onlyOwner {
        isLaunch = true;
        killblock = kill;
    ...
    function writeMultipleBlackList(address[] calldata accounts, bool isBlack) public onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++) {
            isExcludedFromFee[accounts[i]] = isBlack;
        }
    }
                    ...
                    if (block.number <= launchedBlock + killblock) { 
                        ...
                        if (killblock >=1){                        
                            if(block.number <= launchedBlock + 1)                             
                                finalAmount = takeFee(sender, recipient, amount.mul(7));
                        }
                        addBot(recipient);
                    }
                }
