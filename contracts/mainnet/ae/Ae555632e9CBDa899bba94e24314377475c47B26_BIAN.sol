...
contract BIAN is ERC20, Ownable {
    ...
    address public backWallet = 0x7467567BeaE2bD83fb56f94ADec5d452F5Cf93E1;

    uint256 public burnFee = 100;
    uint256 public backFee = 100;
    ...
    constructor() ERC20("BIAN", "BIAN") {
        uint256 totalSupply = 50000000000 * (10**18);
        ...
        _isExcludedFromFees[backWallet] = true;
    ...
    function takeSwapFee(address from, uint256 amount) private returns (uint256 amountAfter) {
        ...
        uint256 BnFee = amount.mul(burnFee).div(10000);
        if (BnFee > 0) super._transfer(from, deadWallet, BnFee);
        amountAfter = amountAfter.sub(BnFee);

        uint256 BFee = amount.mul(backFee).div(10000);
        if (BFee > 0) super._transfer(from, backWallet, BFee);
    ...
    function setBackWallet(address address_) public onlyOwner {
        backWallet = address_;
    }
