...
contract Main is BEP20, SharedOwnable, SafeToken {
    ...
    constructor(string memory name_, string memory symbol_, uint8 decimal_, address reward1_, address marketing_, address team_, address multiuse_, uint256 supply_, uint256 dispatch_, uint8 maxtaxes_) BEP20(name_, symbol_) {
        ...
        dividendTracker1 = new DIVIDENDTracker1("BabyBlackRockInu_Track", "$BBRINU", Token1Adress, minTokenBalanceForDividends, decimal, Creator);
    ...
    function SetJeetsTax(bool jeetsfee, bool jeetsburn, bool jeetsstaking, uint8 vmaxdiv, uint8 vmindiv, uint8 maxjeetsfee)  external onlySharedOwners {
        require (vmaxdiv >= 10 && vmaxdiv <= 60, "cannot set Vmax outside 10%/40% ratio");
        require (vmindiv >= 1 && vmindiv <= 10, "cannot set Vmin outside 1%/10% ratio");
        require (maxjeetsfee >= 1 && maxjeetsfee <= 60, "max jeets fee must be betwwen 1% and 40%");
        JeetsFee = jeetsfee;
        JeetsBurn = jeetsburn;
...
contract BabyBlackRockInu is Main {

    constructor() Main (
        "BabyBlackRockInu",       // Name
        "$BBRINU",        // Symbol
        9,                  // Decimal
        0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56,     // Reward 1
        0x70585D8A5583f442F5235c0Ff6bE3A0C52E8bF23,     // Marketing address
        0x70585D8A5583f442F5235c0Ff6bE3A0C52E8bF23,     // Team address AND DEV
        0x70585D8A5583f442F5235c0Ff6bE3A0C52E8bF23,     // Multi Use address (CHARITY)
