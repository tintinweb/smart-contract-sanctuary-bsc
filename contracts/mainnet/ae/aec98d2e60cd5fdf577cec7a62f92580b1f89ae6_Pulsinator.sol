...
contract Pulsinator is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    PulsinatorDividendTracker public dividendTracker;

    address payable public MarketingAndPulseTrading;
    ...
    uint256 public sellMarketingAndPulseTradingFees;
    uint256 public sellLiquidityFee;
    uint256 public buyDeadFees;
    uint256 public buyMarketingAndPulseTradingFees;
    ...
    event UpdateFees(
        ...
        uint256 sellMarketingAndPulseTradingFees,
        uint256 sellLiquidityFee,
        uint256 sellRewardsFee,
        uint256 buyDeadFees,
        uint256 buyMarketingAndPulseTradingFees,
    ...
    constructor() ERC20("Pulsinator", "Pulse") {
        MarketingAndPulseTrading = payable(0xc7E51e4DfA4A039Bd3a3708022509c0D44022e9f);
        devWallet = payable(0x1973Aef7dB46330e4187c29082bc327311886030);
        address router = 0x10ED43C718714eb63d5aA57B78B54704E256024E;

        buyDeadFees = 2;
        sellDeadFees = 2;
        buyMarketingAndPulseTradingFees = 4;
        sellMarketingAndPulseTradingFees = 4;
        buyLiquidityFee = 2;
        sellLiquidityFee = 2;
        buyRewardsFee = 4;
        sellRewardsFee = 4;
        ...
        totalBuyFees = buyRewardsFee
            .add(buyLiquidityFee)
            .add(buyMarketingAndPulseTradingFees)
            .add(buyDevFee);
        totalSellFees = sellRewardsFee
            .add(sellLiquidityFee)
            .add(sellMarketingAndPulseTradingFees)
            .add(sellDevFee);

         dividendTracker = new PulsinatorDividendTracker(
            payable(this),
            router,
        ...
        dividendTracker.excludeFromDividends(MarketingAndPulseTrading);
        dividendTracker.excludeFromDividends(owner());

        // exclude from paying fees or having max transaction amount
        _isExcludedFromFees[address(this)] = true;
        _isExcludedFromFees[address(dividendTracker)] = true;
        _isExcludedFromFees[address(MarketingAndPulseTrading)] = true;
    ...
    function updateFees(
        uint256 deadBuy,
        uint256 deadSell,
        uint256 MarketingAndPulseTradingBuy,
        uint256 MarketingAndPulseTradingSell,
        ...
        buyDeadFees = deadBuy;
        buyMarketingAndPulseTradingFees = MarketingAndPulseTradingBuy;
        buyLiquidityFee = liquidityBuy;
        buyRewardsFee = RewardsBuy;
        sellDeadFees = deadSell;
        sellMarketingAndPulseTradingFees = MarketingAndPulseTradingSell;
        ...
        emit UpdateFees(
            sellDeadFees,
            sellMarketingAndPulseTradingFees,
            sellLiquidityFee,
            sellRewardsFee,
            buyDeadFees,
            buyMarketingAndPulseTradingFees,
            buyLiquidityFee,
            buyRewardsFee,
            buyDevFee,
            sellDevFee
    ...
    function _transfer(
        ...
        uint256 RewardsFee;
        uint256 deadFees;
        uint256 MarketingAndPulseTradingFees;
        ...
        if (amount == 0) {
            ...
            if (!isBuying && !isSelling) {
                ...
                MarketingAndPulseTradingFees = sellMarketingAndPulseTradingFees;
                ...
                MarketingAndPulseTradingFees = buyMarketingAndPulseTradingFees;
            ...
            uint256 totalFees = RewardsFee
                .add(liquidityFee + MarketingAndPulseTradingFees + devFees);
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        if (buyRewardsFee > 0) {
            ...
            .div(totalAmount)
            .mul(buyRewardsFee)
            .div(buyRewardsFee + buyMarketingAndPulseTradingFees + buyDevFee);
        }
        if (sellRewardsFee > 0) {
            ...
            .div(totalAmount)
            .mul(sellRewardsFee)
            .div(sellRewardsFee + sellMarketingAndPulseTradingFees + sellDevFee);
        }
        ...
        uint256 _completeFees = sellMarketingAndPulseTradingFees.add(sellDevFee) +
            buyMarketingAndPulseTradingFees.add(buyDevFee);

        uint256 feePortions;
        if (_completeFees > 0) {
            feePortions = address(this).balance.div(_completeFees);
        }
        uint256 MarketingAndPulseTradingPayout = buyMarketingAndPulseTradingFees.add(sellMarketingAndPulseTradingFees) * feePortions;
        uint256 devPayout = buyDevFee.add(sellDevFee) * feePortions;

        if (MarketingAndPulseTradingPayout > 0) {
            (successOp1, ) = address(MarketingAndPulseTrading).call{value: MarketingAndPulseTradingPayout}("");
        }
        if (devPayout > 0) {
        ...
        emit SendDividends(
            dividends,
            MarketingAndPulseTradingPayout + devPayout,
...
contract PulsinatorDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    Pulsinator public PulsinatorContract;
    ...
    constructor(
        ...
        defaultToken = token;
        PulsinatorContract = Pulsinator(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Pulsinator_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Pulsinator_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Pulsinator contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(PulsinatorContract);

            uint256 prevBalance = PulsinatorContract.balanceOf(address(this));
            ...
            {
                uint256 received = PulsinatorContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    PulsinatorContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
