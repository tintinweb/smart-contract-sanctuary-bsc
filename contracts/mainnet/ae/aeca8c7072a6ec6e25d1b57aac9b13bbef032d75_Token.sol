...
contract MktCap is Ownable {
    ...
    modifier canDividend() {
        if (address(dividends)==address(0)) return;
        _;
    }
        ...
        )
    {
        ...
        uint256 amount2Dividend = amount.mul(allot.dividend).div(total2Fee).div(
            2
        );
    ...
    function targetDividend(address from, address to) external canDividend(){
        try dividends.setShare(from) {} catch {}
        try dividends.setShare(to) {} catch {}
        try dividends.process(200000) {} catch {}
    }
    ...
    function trigger(uint256 t) external canSwap(t) {
        ...
        if (amount2Dividend > 0) {
            IERC20(token1).transfer(address(dividends), amount2Dividend);
            try
