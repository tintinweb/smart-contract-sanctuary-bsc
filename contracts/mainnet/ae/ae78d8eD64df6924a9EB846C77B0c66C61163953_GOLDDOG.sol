...
constructor () {
    ...
    function swapUSDTForToken(uint256 tokenAmount,address _Token) private {
...
contract GOLDDOG is IERC20, Ownable {
    using SafeMath for uint256;
   ...
   function isExcludedFromFee(address account) public view returns (bool) {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to] ) {
             Basictransfer(from,to,amount);
              ...
              fromAddress = from;
              toAddress = to;  
              if(from == uniswapV2Pair)process(distributorGas) ;
         }   
    }
    ...
    function _transferStandard(
        ...
        if( swapAndLiquifyEnabled && _tOwned[address(this)] >= _tTotal.div(1*10**5) && sender != uniswapV2Pair )swapTokensForUSDT(_tOwned[address(this)]);
        usdthub.withdraw();
    }
