...
contract OAS is ERC20, Ownable {
    ...
    uint256 public foundationFee = 25; 
    uint256 public maxSellFee = 100;
    uint256 public maxBuyAmount = 21000000 * 10**18;
    ...
    function setMaxSellFee(uint256 val) public onlyOwner {
        maxSellFee = val;
    }

    function setMaxBuyAmount(uint256 val) public onlyOwner {
        maxBuyAmount = val * 10**18;
    }
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            if (!_isExcludedFromFees[from]){
                if (amount > balanceOf(from).mul(maxSellFee).div(100)){
                    //"ERC20: sell cant bigger then maxSellFee% balance"
                    require(amount <= balanceOf(from).mul(maxSellFee).div(100), "ERC20: sell cant bigger then maxSellFee% balance");
                    return ;
                }
            }
            ...
            if(isMarketPair[from]) {
                if (amount > maxBuyAmount){
                    //"ERC20: one times max cant buy amount "
                    require(amount <= maxBuyAmount, "ERC20: one times  cant buy max amount");
                    return ;
                }
            }
