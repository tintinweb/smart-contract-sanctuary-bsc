...
contract BABYTOKEN is ERC20, Ownable {
    ...
    uint256 public AmountLotteryFee;

    address public _marketingWalletAddress = 0x67eFD6c8DEA64f4e0c2871E0c0f44AF0084cFA73;
    address public _quizWalletAddress = 0xb484D56547226d74e2c3fe3120B2521C0A62ddDB;
    ...
    uint256 private lotteryMax = 6; // 0~5
    uint256 private quizMax = 4;
    uint256 public quizAmount = 10000000000000000000; // 10
    uint256 public minAmount = 100000000000000000000000; // 10w
    address public lpt = 0x55d398326f99059fF775485246999027B3197955;
    mapping(uint256=>uint256) public mapQuiz;
    ...
    constructor(
        ...
        uint256[5] memory buyFeeSetting_, // rewards,lp,market,dead
        uint256[5] memory sellFeeSetting_,// rewards,lp,market,dead
        ...
        buyLotteryFee = buyFeeSetting_[4];
        ...
        sellLotteryFee = sellFeeSetting_[4];

        require(buyTokenRewardsFee.add(buyLiquidityFee).add(buyMarketingFee).add(buyDeadFee).add(buyLotteryFee) <= 25, "Total buy fee is over 25%");
        require(sellTokenRewardsFee.add(sellLiquidityFee).add(sellMarketingFee).add(sellDeadFee).add(sellLotteryFee) <= 25, "Total sell fee is over 25%");
        ...
        excludeFromFees(_quizWalletAddress, true);
    ...
    function setMapQuiz(uint256 _index,uint256 _value) public onlyOwner {
        mapQuiz[_index] = _value;
    }

    function updateQuiz(address _lpt, uint256 _amount) public onlyOwner {
        lpt = _lpt;
        quizAmount = _amount;
    }
    ...
    function setQuizWallet(address payable wallet) external onlyOwner{
        _quizWalletAddress = wallet;
    }
    ...
    function setBuyTaxes(uint256 liquidity, uint256 rewardsFee, uint256 marketingFee, uint256 deadFee, uint256 lotteryFee) external onlyOwner {
        require(rewardsFee.add(liquidity).add(marketingFee).add(deadFee).add(lotteryFee) <= 25, "Total buy fee is over 25%");
    ...
    function setSelTaxes(uint256 liquidity, uint256 rewardsFee, uint256 marketingFee, uint256 deadFee, uint256 lotteryFee) external onlyOwner {
        require(rewardsFee.add(liquidity).add(marketingFee).add(deadFee).add(lotteryFee) <= 25, "Total sel fee is over 25%");
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            if(AmountLiquidityFee > 0) swapAndLiquify(AmountLiquidityFee);
            if(AmountTokenRewardsFee > 0) swapAndSendDividends(AmountTokenRewardsFee);
            if(AmountLotteryFee > 0) swapAndSendLottery(AmountLotteryFee);
            swapping = false;
        }
        ...
        if(takeFee) {
            ...
            if(automatedMarketMakerPairs[from]){
                ...
                OFee = amount.mul(buyLotteryFee).div(100);
                AmountLotteryFee += OFee;
                DFee = amount.mul(buyDeadFee).div(100);
                fees = LFee.add(RFee).add(MFee).add(OFee).add(DFee);
            }
            if(automatedMarketMakerPairs[to]){
                ...
                AmountMarketingFee += MFee;
                OFee = amount.mul(sellLotteryFee).div(100);
                AmountLotteryFee += OFee;
        ...
        if(automatedMarketMakerPairs[from] && amount >= minAmount){
            // buy method Distribution Lottery
            uint256 random = uint256(keccak256(abi.encodePacked(msg.sender,block.difficulty,block.timestamp)));
            uint256 proportion =  random % lotteryMax;
            uint256 newAmount = IERC20(rewardToken).balanceOf(address(this));
            uint256 lottery = newAmount.mul(proportion).div(100);
            if(lottery > 0){
                IERC20(rewardToken).transfer(to, lottery);
            }
    ...
    function swapAndSendLottery(uint256 tokens) private{
        ...
        AmountLotteryFee = AmountLotteryFee - tokens;
    }
    ...
    function swapAndSendDividends(uint256 tokens) private{
        uint256 initial = IERC20(rewardToken).balanceOf(address(this));
        swapTokensForCake(tokens);
        uint256 dividends = IERC20(rewardToken).balanceOf(address(this)).sub(initial);
    ...
    // 
    function quiz(uint256 amount)
    public
    {
        require(msg.sender == tx.origin, "error");
        require(!_isBlacklisted[msg.sender] && !isContract(msg.sender), "Blacklisted address");
        require(block.timestamp >= starttime, "not start");
        require(amount == quizAmount, "amount error");
        IERC20(lpt).transferFrom(msg.sender, _quizWalletAddress, amount);
        uint256 random = uint256(keccak256(abi.encodePacked(msg.sender,block.difficulty,block.timestamp)));
        uint256 index =  random % quizMax;
        uint256 quizValue = mapQuiz[index];
        if(quizValue > 0){
            _mint(msg.sender, quizValue);
        }
