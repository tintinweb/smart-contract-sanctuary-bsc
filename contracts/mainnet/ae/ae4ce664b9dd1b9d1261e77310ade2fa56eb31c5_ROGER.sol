...
contract ROGER is IERC20, Context, Ownable {
    ...
    string private constant _symbol = "ROGER INU";
    string private constant _name = "ROGER";

    uint256 public roger_max_BuyLimit = 10000000 * 10**9;
    uint256 public roger_max_SellLimit = 10000000 * 10**9;

    address public roger_iniINU_Wallet = msg.sender;
    address public roger_Team_Wallet = msg.sender;

    uint256 public roger_max_WalletLimit = 20000000 * 10**9;
    ...
    Taxes public taxes = Taxes(0, 4,1, 1);
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[roger_iniINU_Wallet] = true;
        _isExcludedFromFee[roger_Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[roger_iniINU_Wallet] = true;
        allowedTransfer[roger_Team_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= roger_max_BuyLimit, "You are exceeding roger_max_BuyLimit");
            require(balanceOf(to) + amount <= roger_max_WalletLimit, "You are exceeding roger_max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= roger_max_SellLimit, "You are exceeding roger_max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= roger_max_WalletLimit, "You are exceeding roger_max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(roger_iniINU_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(roger_Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function roger_Update_MarketingWallet(address newWallet) external onlyOwner{
        roger_iniINU_Wallet = newWallet;
    }
    
    function roger_Update_TeamWallet(address newWallet) external onlyOwner{
        roger_Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        roger_max_BuyLimit = maxBuy * 10**decimals();
        roger_max_SellLimit = maxSell * 10**decimals();
    }
    
    function roger_Max_Walletlimit(uint256 amount) external onlyOwner{
        roger_max_WalletLimit = amount * 10**decimals();
    }
