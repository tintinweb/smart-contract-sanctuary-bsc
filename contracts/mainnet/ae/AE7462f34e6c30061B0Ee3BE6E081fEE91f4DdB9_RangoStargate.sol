...
contract BaseProxyContract is PausableUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function onChainSwapsPreBridge(
        ...
        require(_bridgeProxy != NULL_ADDRESS, 'Bridge proxy address in Rango contract not set');
        ...
        value = (request.toToken == NULL_ADDRESS ? (out > 0 ? out : request.amountIn) : 0) + extraFee;
        return (out, value);
    }
    ...
    function callSwapsAndFees(SwapRequest memory request, Call[] calldata calls) private returns (bytes[] memory) {
        ...
        for(uint256 i = 0; i < calls.length; i++) {
            require(baseProxyStorage.whitelistContracts[calls[i].target], "Contract not whitelisted");
        }
...
contract RangoStargate is IRangoStargate, IStargateReceiver, BaseInterchainContract {
    ...
    /// @param _stargateRouter The new address of Stargate contract
    /// @param _stargateRouterEth The new address of Stargate contract
    function initialize(address _nativeWrappedAddress, address _stargateRouter, address _stargateRouterEth) public initializer {
        BaseProxyStorage storage baseStorage = getBaseProxyContractStorage();
        baseStorage.nativeWrappedAddress = _nativeWrappedAddress;

        updateStargateAddressInternal(_stargateRouter, _stargateRouterEth);
    ...
    function updateStargateAddress(address _router, address _routerEth) public onlyOwner {
        updateStargateAddressInternal(_router, _routerEth);
    }

    /// @inheritdoc IRangoStargate
    function stargateSwap(
        address _fromToken,
        uint _inputAmount,
        StargateRequest memory _stargateRequest
    ) external override payable whenNotPaused nonReentrant {
        require(msg.value >= _stargateRequest._stgFee, "stgFee is bigger than the input");

        address router = _fromToken == NULL_ADDRESS ? stargateRouterEth : stargateRouter;
        require(router != NULL_ADDRESS, "Stargate router address not set");
        ...
        bytes memory payload = _stargateRequest._bridgeType == StargateBridgeType.TRANSFER_WITH_MESSAGE
            ? abi.encode(_stargateRequest._payload)
            : new bytes(0);
            
        uint value = _fromToken == NULL_ADDRESS ? msg.value : _stargateRequest._stgFee;

        if (_fromToken == NULL_ADDRESS) {
            if (_stargateRequest._payload.dstChainId > 0) {
                revert("Payload not supported on swapETH");
            }

            IStargateRouter(router).swapETH(
                _stargateRequest._dstChainId,
                _stargateRequest._refundAddress,
                _stargateRequest._to,
                _inputAmount, 
                _stargateRequest._minAmountLD
            );
        } else {
            IStargateRouter.lzTxObj memory lzTx = IStargateRouter.lzTxObj(
                _stargateRequest._dstGasForCall,
                _stargateRequest._dstNativeAmount,
                _stargateRequest._dstNativeAddr
            );
            IStargateRouter(router).swap{value: value}(
                _stargateRequest._dstChainId,
                _stargateRequest._srcPoolId,
                _stargateRequest._dstPoolId,
                _stargateRequest._refundAddress,
                _inputAmount,
                _stargateRequest._minAmountLD,
                lzTx,
        ...
        emit StargateSwapStatusUpdated(
            _fromToken,
            _inputAmount,
            OperationStatus.Created,
            _stargateRequest._payload.originalSender,
            _stargateRequest._payload.recipient
        );
    }
    ...
    function sgReceive(
        ...
        (address receivedToken, uint dstAmount, OperationStatus status) = handleDestinationMessage(_token, amountLD, m);
    ...
    function updateStargateAddressInternal(address _router, address _routerEth) private {
        address oldAddressRouter = stargateRouter;
        stargateRouter = _router;

        address oldAddressRouterEth = stargateRouterEth;
        stargateRouterEth = _routerEth;
