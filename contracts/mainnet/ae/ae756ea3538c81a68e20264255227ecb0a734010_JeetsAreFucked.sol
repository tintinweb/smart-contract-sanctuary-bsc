...
contract JeetsAreFucked is IBEP20 {
    ...
    event PartyStarted(uint256 partyNumber, uint256 startingTax, uint256 sellsUntilPartyIsOver);
    event Buy(uint256 amount, uint256 buysLeft);
    event JeetSold(uint256 rewardsInBusd);
    event ContractSell(uint256 rewards);
    event PartyStatistics(uint256 partyNo, JTD partySummary, bool crashed);
    event PartyUpdate(JTD partyStats, bool jeet, bool diamondHand);

    struct Share {uint256 amount;uint256 totalExcluded;uint256 totalRealised;}
    struct JTD {
        uint256 buysLeftUntilParty;
        bool partyActive;
        uint256 currentBuyTax;
        uint256 currentSellTax;
        uint256 buysLeftToCrashTheParty;
        uint256 sellsLeftUntilPartyIsOver;
        uint256 sellTaxAfterTheParty;
        uint256 rewardsPaidSincePartyStartedInBusd;
        uint256 rewardsAllPartysCombinedInBusd;
        address[] diamondHandsCrashingThisParty;
        address[] jeetsThatSoldDuringThisParty;
    }
    ...
    function GetPartyStats() public view returns (JTD memory partyStats) {
        partyStats.partyActive = letTheJeetsOutEvent;
        partyStats.buysLeftUntilParty = letTheJeetsOutEvent ? 0 : buysUntilEvent - buys;
        partyStats.currentBuyTax = letTheJeetsOutEvent ? 0 : tax;
        partyStats.currentSellTax = jeetTaxActive ? letTheJeetsOutEvent ? (jeetTax - sells) : tax * sellMultiplier  + jeetTax : tax * sellMultiplier;
        partyStats.buysLeftToCrashTheParty = letTheJeetsOutEvent ? buysToStopEvent - buys : 0;
        partyStats.sellsLeftUntilPartyIsOver = letTheJeetsOutEvent ? jeetTax - sells : 0;
        partyStats.sellTaxAfterTheParty = letTheJeetsOutEvent ? initialJeetTax - ((block.timestamp - launchTime) / timeUntilJeetTaxDecrease) : tax * sellMultiplier  + jeetTax;
        partyStats.rewardsPaidSincePartyStartedInBusd = partyRewards[partyNumber] / 1 ether;
        partyStats.rewardsAllPartysCombinedInBusd = totalRewards / 1 ether;
        partyStats.diamondHandsCrashingThisParty = diamondHandPartyCrashers;
        partyStats.jeetsThatSoldDuringThisParty = jeetsThatSoldDuringParty;
    }
    ...
    function letTheJeetsOut() internal {
        ...
        sells = 0;
        buys = 0;
        emit PartyStarted(partyNumber, tax * sellMultiplier + jeetTax, jeetTax);
    }

    function stopLettingTheJeetsOut() internal {
        partyHistory[partyNumber] = GetPartyStats();
