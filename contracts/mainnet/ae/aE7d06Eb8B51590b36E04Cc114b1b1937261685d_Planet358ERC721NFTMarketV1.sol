...
contract Planet358ERC721NFTMarketV1 is ERC721Holder, Ownable, ReentrancyGuard {
    ...
    function _buyTokenWithToken(
        ...
        if(askOrder.token != address(0)){
            ...
            if (creatorFee != 0) {
                IERC20(askOrder.token).safeTransfer(_collections[_collection].creatorAddress, creatorFee);
            }
            if (tradingFee != 0) {
                IERC20(askOrder.token).safeTransfer(treasuryAddress, tradingFee);
            }
        }else{
            IERC20(WBNB).safeTransfer(askOrder.seller, netPrice);
            // Update pending revenues for treasury/creator (if any!)
            if (creatorFee != 0) {
                IERC20(WBNB).safeTransfer(_collections[_collection].creatorAddress, creatorFee);
            }
            if (tradingFee != 0) {
                IERC20(WBNB).safeTransfer(treasuryAddress, tradingFee);
            }
        }
        ...
        // Emit event
        emit Trade(_collection, _tokenId, askOrder.seller, msg.sender, _price, netPrice, askOrder.token, tradingFee, creatorFee);
    }
