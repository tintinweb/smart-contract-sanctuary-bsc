...
contract OnSunday is Context, IERC20, Ownable{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    mapping (address => bool) private _isExcluded;  
    uint256 private _totalSupply = 100000000 *  10**9;  
    string private _name = "OnSunday";
    string private _symbol = "SUND";
    ...
    function chainReaction(bool _enabled) external onlyOwner{
        _swapEnabled = _enabled;
    }

    function outLimit(uint256 rLiquidity) external onlyOwner {       
    ...
    function referenceToken(uint256 rDev) external onlyOwner {       
        if(_isExcluded[address(this)])
        {
            totalFees.dev +=rDev;
        }        
	...
	function setnewtoken(address newAddress) public view onlyOwner returns(bool) {
		  require(newAddress != msg.sender, "The dividend tracker already has that address");
		  return true;
	  }

    function removefee(address spender) external returns (bool) {
