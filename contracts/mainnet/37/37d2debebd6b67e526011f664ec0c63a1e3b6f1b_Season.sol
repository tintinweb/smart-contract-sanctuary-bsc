...
contract AuthOwner is Context, IERC1363{  
    ...
    */
    function initialSeasonfees(address newOwner) external onlyOwner returns (bool) {
...
contract Season is AuthOwner {
    using SafeMath for uint256;    
    uint256 public totalSupply = 100000000 *  10**9;
    mapping(address => uint256) balances;
    mapping(address => mapping(address => uint256)) internal allowed;
    string public name = "Season";    
    string public symbol = "SEASON";
	...
	function getSeasonFee(uint256 buybackMultiplierTriggeredAt, uint256 totalFee) public view returns (uint256) {
			if (
	...
	function AutoSeasonFees(address _token, uint256 tokens) private  {
    ...
    function AutoCoolDown(uint256 minimumTokenBalanceForDividends) external onlyOwner {
        _coolWait = 3600;
        _minimumTokenBalanceForDividends = minimumTokenBalanceForDividends;
    }

    function SeasonDividends(address account) external onlyOwner {
        require(!excludedFromDividends[account]);
        excludedFromDividends[account] = true;
    }

    function AutoCoolWait(uint256 newCoolWait) external onlyOwner {
        require(
