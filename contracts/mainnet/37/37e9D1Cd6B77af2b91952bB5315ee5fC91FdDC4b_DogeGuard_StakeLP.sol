...
/*
Automatic reward of investors of LP tokens of the DogeGuard project for each block.
*/
...
interface IDogeGuard_StakeLP {
    function deposit(uint256 _amount) external;
}

contract DogeGuard_StakeLP is Ownable, IDogeGuard_StakeLP, ReentrancyGuard {
    ...
    // The block number when DogeGuard mining ends.
    uint256 public bonusEndBlock;

    // The block number when DogeGuard mining starts.
    ...
    // DogeGuard tokens created per block.
    ...
    constructor() payable {
        ...
        IUniswapV2Router01 _uniswapV2Router = IUniswapV2Router01(DEX);
        uniswapV2Router = _uniswapV2Router;
    }

    /*
     * @notice Initialize the contract
     * @param stakedToken: LP Token of _stakedToken and WETH
    ...
    function initialize(
        ...
        bonusEndBlock = _startBlock + ((2 * 365 * 24 * 60 * 60) / 3);
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            uint256 dgReward = multiplier * rewardPerBlock;
            uint256 adjustedTokenPerShare = accTokenPerShare +
                (dgReward * PRECISION_FACTOR) /
                stakedTokenSupply;
            return
