...
contract MinimaxMain is IMinimaxMain, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function withdrawImpl(uint positionIndex, uint amount, bool withdrawAll) private {
        PositionInfo storage position = positions[positionIndex];
    ...
    function withdrawAll(uint positionIndex) external nonReentrant {
        withdrawImpl(positionIndex, 0 /* amount */, true /* withdrawAll */);
    ...
    function withdraw(uint positionIndex, uint amount) external nonReentrant {
        withdrawImpl(positionIndex, amount /* amount */, false  /* withdrawAll */);
    ...
    function estimateLpParts(address lpToken, uint amountIn) external view returns (uint, uint) {
        return market.estimateLpParts(lpToken, amountIn);
    } 

    struct SlotInfo {
        uint withdrawedBalance;
        address lpToken;
        uint amount0;
        uint amount1;
        address token0;
    ...
    function withdrawAllWithSwap(
        ...
        uint withdrawedBalance = position.stakedToken.balanceOf(address(position.callerAddress));
        position.callerAddress.transferAll(position.stakedToken, address(this));

        uint amountOut = PositionLib.makeSwapOneInch(
            withdrawedBalance, address(position.stakedToken), oneInchRouter, PositionLib.StakeSwapOneInch(oneInchCallData));
    ...
    function withdrawAllWithSwapLp(
        ...
        slot.withdrawedBalance = position.stakedToken.balanceOf(address(position.callerAddress));
        position.callerAddress.transferAll(position.stakedToken, address(this));
        ...
        IERC20Upgradeable(slot.lpToken).transfer(address(slot.lpToken), slot.withdrawedBalance);
        ...
        slot.amountFirstSwapOut = PositionLib.makeSwapOneInch(
            slot.amount0, slot.token0, oneInchRouter, PositionLib.StakeSwapOneInch(oneInchCallDataToken0));

        slot.amountSecondSwapOut = PositionLib.makeSwapOneInch(
            slot.amount1, slot.token1, oneInchRouter, PositionLib.StakeSwapOneInch(oneInchCallDataToken1));

        SafeERC20Upgradeable.safeTransfer(IERC20Upgradeable(withdrawalToken), msg.sender, slot.amountFirstSwapOut + slot.amountSecondSwapOut);
    }
    ...
    function automationResolve(uint positionIndex) public returns (bool canExec, bytes memory execPayload) {
        ...
        uint8 outDecimals = IERC20Decimals(busdAddress).decimals();
        uint8 inDecimals = IERC20Decimals(address(position.stakedToken)).decimals();
        canExec = PositionExchangeLib.isPriceOutsideRange(position, amountOut, amountIn, outDecimals, inDecimals);
        if (!canExec) {
            return (canExec, execPayload);
        }
        ...
        if (address(oracle) != address(0)) {
            ...
            canExec = PositionExchangeLib.isPriceOutsideRange(position, oraclePrice, oracleMultiplier, 0, 0);
            if (!canExec) {
                return (canExec, execPayload);
            }
        }
...
contract Market is IMarket, OwnableUpgradeable {
    ...
    function estimateLpParts(address lpToken, uint amountIn) external view returns (uint, uint) {
        return pancakeLpMarket.estimateLpParts(lpToken, amountIn);
    }
...
library PositionBalanceLib {
    ...
    function getMany(
        ...
        for (uint i = 0; i < positionIndexes.length; ++i) {
            balances[i] = get(poolAdapters, positions[positionIndexes[i]]);
        }
        return balances;
    }
...
contract PancakeLpMarket is OwnableUpgradeable {
    ...
    function estimateLpParts(address lpToken, uint amountIn) public view returns(uint, uint) {
    ...
    function _estimatePairToSingle(
        ...
        (uint amount0, uint amount1) = estimateLpParts(tokenIn, amountIn);
