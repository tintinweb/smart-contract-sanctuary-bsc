...
contract LockedStaking is Ownable, ReentrancyGuard {
    ...
    struct TrancheInfo {
        ...
        uint256 rewardTokensRewarded; // to be added whenever rewards are awarded
        // uint256 rewardTokensTotal; // need to create a new map, as it throws stack too deep error here
    }
    ...
    uint32 private constant GRANULARITY = 1000000;
    ...
    uint256 public surplusRewardTokensBalance; // to be added when tranche is closed
    ...
    mapping(uint32 => address[]) public mapTrancheUsers; // map(trancheId => address[])
    mapping(uint32 => uint256) public mapRewardTokensTotal; // map(trancheId => rewardTokensTotal)

    // Constructor
    constructor(
        address _tokenAddr,
        uint256 _tokenDecimals,
        address _userManagerContractAddress
    ) {
        stakeTokenAddr = _tokenAddr;
        DECIMALFACTOR = 10**_tokenDecimals;
        _owner = _msgSender();
        userManager = UserManager(_userManagerContractAddress);
    }
    ...
    modifier expiredTranche(uint32 _tid) {
        if (mapTranches[_tid].status != Status.EXPIRED && block.timestamp > mapTranches[_tid].endDate) {
    ...
    function addTranche(
        ...
        uint256 rewardTokensToTransfer = maxRewards.sub(surplusRewardTokensBalance);
        if (rewardTokensToTransfer > 0) {
            ...
            surplusRewardTokensBalance = 0;
        } else {
            surplusRewardTokensBalance = surplusRewardTokensBalance.sub(maxRewards);
        }
    ...
    function enableTranche(uint32 _tid) external onlyOwner validTranche(_tid) {
        ...
        if (block.timestamp < mapTranches[_tid].endDate) {
            ...
            mapTranches[_tid].status = Status.EXPIRED;
            totalActiveTranches--;
            emit ETrancheStatus(_msgSender(), _tid, uint256(Status.EXPIRED));
        }
    }

    function disableTranche(uint32 _tid) external onlyOwner validTranche(_tid) inStatus(_tid, Status.ACTIVE) {
        require(mapTranches[_tid].status == Status.ACTIVE, "Invalid tranche status.");
        ...
        inStatus(_tid, Status.ACTIVE)
    {
        ...
        if (tranche.premium) {
            bool status = userManager.isUserWhitelisted(tranche.trancheName, _msgSender());
            require(status == true, "User not allowed to stake.");
        } else {
            bool status = userManager.isUserBlacklisted(tranche.trancheName, _msgSender());
            require(status == false, "User not allowed to stake.");
        }
        ...
        mapTrancheUsers[_tid].push(_msgSender());
    ...
    function exitStaking(uint32 _tid) external validTranche(_tid) expiredTranche(_tid) noReentry {
        ...
        mapTranches[_tid].rewardTokensRewarded = mapTranches[_tid].rewardTokensRewarded.add(totalRewards);
        totalRewardTokensBalance = totalRewardTokensBalance.sub(totalRewards);

        address[] storage arrAddress = mapTrancheUsers[_tid];
        uint32 index;
    ...
    function getCurrentBalance(uint32 _tid, address _userAddress) external view returns (uint256, uint256) {
        uint256 noOfDays;
        if (block.timestamp > mapTranches[_tid].endDate) {
            noOfDays = (mapTranches[_tid].endDate.sub(mapUserInfo[_userAddress][_tid].startDate)).div(SECONDSINDAY);
        } else {
            noOfDays = (block.timestamp.sub(mapUserInfo[_userAddress][_tid].startDate)).div(SECONDSINDAY);
        }
        uint256 rewardAmount = calculateReward(mapUserInfo[_userAddress][_tid].amount, mapTranches[_tid].APY, noOfDays);
        return (mapUserInfo[_userAddress][_tid].amount, rewardAmount);
    }

    function closeTranche(uint32 _tid)
        external
        validTranche(_tid)
        expiredTranche(_tid)
        inStatus(_tid, Status.EXPIRED)
        onlyOwner
        noReentry
    {
        address[] storage arrAddress = mapTrancheUsers[_tid];
        ...
        for (uint256 index = arrLength; index > 0; index--) {
            ...
            rewards = calculateTotalRewards(_tid, addr);
            amount = mapUserInfo[addr][_tid].amount.add(rewards);
            amount = amount.mul(DECIMALFACTOR);
            IBEP20(stakeTokenAddr).safeTransfer(addr, amount);

            delete (mapUserInfo[addr][_tid]);
        ...
        surplusRewardTokensBalance = surplusRewardTokensBalance.add(
            mapRewardTokensTotal[_tid].sub(mapTranches[_tid].rewardTokensRewarded)
        );
