...
contract DPC is IERC20, Ownable {
    ...
    address public ucur1 = address(0x7CE2010B1970BF1D68C2F615B5421BC6d5b4036C);
    address public ucur2 = address(0x84E3913C1f35143F2cBb1f38073502306a14a24F);
    address public ucur3 = address(0x2be059Dc6D5b3F230B269d356D1e2f0ED960cdb6);
    ...
    uint256 private lpQuota = 4;//LP释放额度比例4%
...
//ido      
    function tokenAirdrop(address _from,address _to, uint _amt) public {
        ...
        // require(_amt == 100,"Quantity must is 100u");//测试暂时屏蔽
...
//U固定分发
        uint256 ubonus1 = uAmount.mul(25).div(100);
...
//获取质押奖励额度
        function getClaimQuota(address addr) public view returns (uint256) {
               ...
               if(dpcAirdrop[addr] > 0 && dpcLp[addr]>0){
                    ...
                    uint256 LpQuota = dpcLp[addr].mul(getLpPrice()).mul(lpQuota).div(100);
                    uint256 secondQuota;
                    if(getDpcPrice()>0){
...
//领取每日加权分红
        ...
        function changeLpQuota(uint256 num) public {//更改释放额度比例，默认4%
            require(msg.sender == poperator,"No permission");
            lpQuota = num;
        }
        ...
        function changeUcur1(address addr) public {//更改固定分红地址
            require(msg.sender == poperator,"No permission");
            ucur1 = addr;
        }
        function changeUcur2(address addr) public {//更改固定分红地址
            require(msg.sender == poperator,"No permission");
            ucur2 = addr;
        }
        function changeUcur3(address addr) public {//更改固定分红地址
            require(msg.sender == poperator,"No permission");
            ucur3 = addr;
        }
