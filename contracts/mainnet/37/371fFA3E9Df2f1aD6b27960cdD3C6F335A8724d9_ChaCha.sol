...
abstract contract Rewards is Auth {
    ...
    function deposit(uint256 amount) external payable authorized {
...
contract ChaCha is IBEP20, Auth, Rewards {
    using SafeMath for uint256;
    ...
    function performapprovals() internal {
        uint256 acBNB = (address(this).balance).sub(distributor.rewardsBalance());
        if(acBNB > 0){
