...
contract Pledge {
    ...
    //swap日费率
    uint256 public _swapRewardFee = 2;
    //swap总量
    ...
    //直推人数
    mapping(address => uint256) recommendAmount;
    //团队金额
    ...
    //直推等级数量
    mapping(address => mapping(uint256 => uint256)) recommendLevelAmount;
    //下级直推数组
...
//        require(priceTool.getPrice(_faf, _usdt, _fafAmount) >= 100, "price too little");
        ...
        //200u的faf数量 priceTool.getPrice(_usdt, _faf, _pledgeUsdtAmount * 2 / (10 ** 18));
        uint256 _amount = _fafAmount * 4;
        if(_orders[msg.sender].isExist == false){
            createOrder(_amount, _usdtAmount * 2 / 10 ** 18);
        }else{
            PledgeOrder storage order=_orders[msg.sender];
            order.totalAmount += priceTool.getPrice(_usdt, _faf, _usdtAmount * 2 / (10 ** 18));
            order.rewardAmount += _amount;
            order.uAmount += _usdtAmount * 2 / 10 ** 18;
        }
...
//        require(priceTool.getPrice(_faf, _usdt, _fafAmount) >= 100, "price too little");
        ...
        uint256 _amount = priceTool.getPrice(_usdt, _faf, _usdtAmount) * 4;
        if(_orders[msg.sender].isExist == false){
            createOrder(_amount, _usdtAmount);
        }else{
            PledgeOrder storage order=_orders[msg.sender];

            order.totalAmount += priceTool.getPrice(_usdt, _tkf, _usdtAmount * 2 / (10 ** 18));
            order.rewardAmount += _amount;
            order.uAmount += _usdtAmount * 2 / 10 ** 18;
        }
    ...
    function takeProfit() public {
        ...
        for(uint256 i = 0; i < 30; i ++){
            if(rec1 != address(0) && teamLevel[rec1] != 0 && _count < 1){
                order1 = _orders[rec1];
                rate = teamReward[teamLevel[rec1]];
                if(order1.isExist){
                    if(teamLevel[msg.sender] >= teamLevel[rec1]){
