...
contract BEP20 is Context, Ownable, IBEP20 {
    ...
    uint256 private numTokensSellToAddToLiquidity = 80 * 10 ** 18;
    uint256 public _maxHolder = 9000 * 10 ** 18;
    uint256 public _maxBuy = 1000 * 10 ** 18;
    bool    private inSwapAndLiquify;

    address public Fund = 0x1d09241160A54daf8915B967DEE45CE0C90e247e;
    address public Marketing = 0xC49f9357430a94819DC19BF6075564F320c2aDb8;
    address public Lp = 0xC49f9357430a94819DC19BF6075564F320c2aDb8;
    address public DeadAddress = 0x000000000000000000000000000000000000dEaD;
    address public NoViterFee = 0xC49f9357430a94819DC19BF6075564F320c2aDb8;
    ...
    function _takeFee(address sender, address recipient, uint256 amount) internal returns (uint256 netAmount)  {
        ...
        if ((!isMarketPair[recipient] && isRouter[sender]) || (_buyFeeEnable && isMarketPair[sender]) || (_sellFeeEnable && isMarketPair[recipient])) {
            tax = amount.mul(_alltaxFee).div(100);
        }
    ...
    function _takeLpFee(
        ...
        address recipient,
        uint256 tax) private {
        uint256 fee = tax.mul(_lpFee).div(_alltaxFee);
    ...
    function _takeLiquidityFee(
        ...
        address recipient,
        uint256 tax) private {
        uint256 fee = tax.mul(_liquidityFee).div(_alltaxFee);
        _balances[recipient] = _balances[recipient].add(fee);
        emit Transfer(sender, recipient, fee);
    }
    ...
    function _takeBurnFee(
        ...
        address recipient,
        uint256 tax) private {
        uint256 fee = tax.mul(_burnFee).div(_alltaxFee);
        _balances[recipient] = _balances[recipient].add(fee);
    ...
    function _takeFundFee(
        ...
        address recipient,
        uint256 tax) private {
        uint256 fee = tax.mul(_fundFee).div(_alltaxFee);
        _balances[recipient] = _balances[recipient].add(fee);
        emit Transfer(sender, recipient, fee);
    }
    ...
    function _takeMarketingFee(
        ...
        address recipient,
        uint256 tax) private {
        uint256 fee = tax.mul(_marketingFee).div(_alltaxFee);
        _balances[recipient] = _balances[recipient].add(fee);
        emit Transfer(sender, recipient, fee);
    }
    ...
    function _takeInviterFee(
        ...
        uint256 fee = tax.mul(_inviterFee).div(_alltaxFee);
        address cur = sender;
    ...
    function setMarkingFee(uint256 _num) external onlyOwner() {
        _marketingFee = _num;
    }

    function setburnFee(uint256 _num) external onlyOwner() {
        _burnFee = _num;
    }
    function setliquidityFee(uint256 _num) external onlyOwner() {
        _liquidityFee = _num;
    }

    function setinviterFee(uint256 _num) external onlyOwner() {
        _inviterFee = _num;
    }

    function setfundFee(uint256 _num) external onlyOwner() {
        _fundFee = _num;
    }

    function setallFee(uint256 _num) external onlyOwner() {
        _alltaxFee = _num;
    }
}
...
contract FUBABA is BEP20Detailed {

    constructor() BEP20Detailed("FUBABA", "FUBABA", 18) public {
        _totalSupply = 9977 * (10**18);
