...
contract StakeAndEarn is Ownable, ReentrancyGuard{
    ...
    function deposit(uint256 amount, uint256 package) external nonReentrant{
		...
		uint256 totalLocked = stakedToken.lockedAmount(msg.sender);
		...
		require(available >= amount, "balance not available for staking");
		...
		stakedToken.lockToken(amount, msg.sender);
    ...
    function withdraw(uint256 stakingID) external nonReentrant{
		...
		require(stakedToken.balanceOf(address(this)) >= reward - fee, "Token balance not available for withdrawal");
        
		stakedToken.unlockToken(msg.sender);
		...
		stakedToken.lockToken(lockedAmount[msg.sender] - amount, msg.sender);
		lockedAmount[msg.sender] = lockedAmount[msg.sender] - amount;
