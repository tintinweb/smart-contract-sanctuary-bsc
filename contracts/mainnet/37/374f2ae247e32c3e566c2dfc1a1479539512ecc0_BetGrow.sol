...
/*
Telegram - http://t.me/HouseoftheDegens
*/
...
library Address{
 ...
 contract BetGrow is Context, IERC20, Ownable {
    ...
    uint256 public swapTokensAtAmount = 5_000_000_000 * 10**12; // 0.5%
    uint256 public maxBuyLimit = 10_000_000_000 * 10**12; // 1%
    uint256 public maxSellLimit = 10_000_000_000 * 10**12; // 1%
    uint256 public maxWalletLimit = 20_000_000_000 * 10**12; // 2%
    
    uint256 public genesis_block;
    
    address public developmentWallet = 0x99A5AbE6dE1013ad239e62125d75707845037B0f;
    address public buybackWallet = 0x3ed4f7A4D42d5e8026D36C7C761E36c6549dF049;

    string private constant _name = "Bet Grow";
    string private constant _symbol = "BGROW";
    ...
    Taxes public taxes = Taxes(0, 5, 1, 4);
    Taxes public sellTaxes = Taxes(0, 5, 1, 4);
    ...
    function setTaxes(uint256 _rfi, uint256 _buyback, uint256 _liquidity, uint256 _development) public onlyOwner {
       taxes = Taxes(_rfi,_buyback,_liquidity,_development);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _buyback, uint256 _liquidity, uint256 _development) public onlyOwner {
       sellTaxes = Taxes(_rfi,_buyback,_liquidity,_development);
        emit FeesChanged();
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi, to_return.rbuyback, to_return.rLiquidity) = _getRValues1(to_return, tAmount, takeFee, _getRate());
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.tTransferAmount = tAmount-s.tRfi-s.tbuyback-s.tLiquidity-s.tdevelopment;
        return s;
    }

    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rbuyback, uint256 rLiquidity){
        ...
        rTransferAmount =  rAmount-rRfi-rbuyback-rLiquidity-rdevelopment;
        return (rAmount, rTransferAmount, rRfi,rbuyback,rLiquidity);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tbuyback + s.tdevelopment);
        }
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.buyback + temp.development) * 2;
        ...
        if(developmentAmt > 0){
            payable(buybackWallet).sendValue(developmentAmt);
        }
    }
