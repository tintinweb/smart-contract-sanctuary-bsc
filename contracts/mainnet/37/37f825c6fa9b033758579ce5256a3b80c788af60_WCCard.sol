...
contract WCCard is AccessControlEnumerable, ERC721Enumerable, ERC721Burnable {
	...
	function setURI(string memory _uri) public virtual {
		require(
			...
			'WCCard: must have admin role to set'
		);

		require(bytes(_uri).length > 0, 'WCCard: uri is invalid');
	...
	function mint(uint256 _tokenId, address _to) public virtual {
		require(
			...
			'WCCard: must have minter role to mint'
		);
		...
		virtual
	{
		...
		require(
			...
			'WCCard: array length is invalid'
		);
