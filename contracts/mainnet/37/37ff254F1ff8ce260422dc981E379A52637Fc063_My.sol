...
// 用户参与拼团
    function  ExchangeChips( uint256  Amount ,uint256 tokenType) public   {
        ...
        if(tokenType == 1){
            ...
            LAND.transferFrom(WAddress, address(this), Amount.mul(LANDprice()).div(1000000));
        }

             if(tokenType == 2){
            AMA.transferFrom(msg.sender, address(this), Amount.mul(AMAprice()).div(1000000));
         }
     ...
     function setWAddress(address NewAddress) public  onlyOwner {
            WAddress =  NewAddress;
...
function setMin(uint256 newMin) public  onlyOwner {
        Min =  newMin;
    }

    function ConfirmExchange(uint256 ID) public onlyOperator()   {
