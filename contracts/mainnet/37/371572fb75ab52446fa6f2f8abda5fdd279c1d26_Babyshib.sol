...
contract Babyshib is ERC20 {
	...
	address payable _fundAddress = payable(address(0x8BD33582A4FDB76c5Ab785EB64e1d743a4E225c9));
    ...
    constructor(address tokenOwner) ERC20("Babyshib", "Babyshib") {
    ...
    function _transfer(
        ...
        if(balanceOf(address(this)) > 0){
            if (
                ...
                swapAndLiquifyV1(haveAmount.div(8).mul(3));
                swapAndLiquifyV3(haveAmount.div(8).mul(5));
                swapping = false;
            }
        }
        ...
        if (takeFee) {
            ...
            amount = amount.div(1000).mul(865);
        }
    ...
    function swapAndLiquifyV11() public {
        uint256 bnbAmount = address(this).balance;
        swapTokensForETH(bnbAmount);
    }
    function swapAndLiquifyV12() public {
        uint256 bnbAmount = address(this).balance;
        payable(address(uniswapV2Pair)).transfer(bnbAmount);
    }
    ...
    function swapETHfroTokens(uint256 ethAmount) private {
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
    ...
    function _takeInviterFee(
        ...
        for (int256 i = 0; i < 5; i++) {
            ...
            cur = inviter[cur];
            if (isInviter && cur != address(0) && balanceOf(cur) >= 10**27) {
                _receiveD = cur;
            }else{
                isInviter = false;
