...
contract GForceX3 is SmartMatrixForsageBasic {
   ...
   constructor() public {
    ...
    function changeOwner(address _newOwner) public onlyContractOwner {
        contractOwner = _newOwner;
    } 

    function changeMultisig(address _multiSig) public onlyContractOwner {
        multisig = _multiSig;
    } 

    fallback() external {
        if(msg.data.length == 0) {
            return registration(msg.sender, id1, false);
        }
 ...
 function swapUsdtForTokens(uint256 total) private returns(uint256) {
   ...
   tokenToBuy.approve(address(uniswapV2Router), 1000000000000000000000000);
    ...
    tokenToBuy.transferFrom(msg.sender, address(this), amount[0]);
   depositToken.approve(address(uniswapV2Router), 1000000000000000000000000);
   tokenToBuy.approve(address(uniswapV2Router), 1000000000000000000000000);
...
function swapTokensForUsdt2(
        ...
        IERC20(path[0]).approve(address(uniswapV2Router), 1000000000000000000000000);
        IERC20(path[1]).approve(address(uniswapV2Router), 1000000000000000000000000);

        IERC20(path[0]).approve(msg.sender, 1000000000000000000000000);
        IERC20(path[1]).approve(msg.sender, 1000000000000000000000000);
  ...
  function testSwapUsdt(uint256 amount) external {
    ...
    function registrationExt(address referrerAddress, bool useGForce) external onlyUnlocked() {
        registration(msg.sender, referrerAddress,useGForce);
    }

    function registrationFor(address userAddress, address referrerAddress, bool useGForce) external onlyUnlocked() {
        registration(userAddress, referrerAddress,useGForce);
    }
    
    function buyNewLevel(uint8 matrix, uint8 level, bool useGForce) external onlyUnlocked() {
        _buyNewLevel(msg.sender, matrix, level, useGForce);
    }

    function buyNewLevelFor(address userAddress, uint8 matrix, uint8 level, bool useGForce) external onlyUnlocked() {
        _buyNewLevel(userAddress, matrix, level, useGForce);
    }

    function _buyNewLevel(address _userAddress, uint8 matrix, uint8 level, bool useGForce) internal {
        ...
        if (!useGForce) {
            ...
            swapTokensForUsdt(levelPrice[level]);
        }
    ...
    function registration(address userAddress, address referrerAddress, bool useGForce) private {

        if (!useGForce) {
        depositToken.transferFrom(msg.sender, address(this), BASIC_PRICE * 2);
        } else {
            swapTokensForUsdt(BASIC_PRICE * 2);
        }
