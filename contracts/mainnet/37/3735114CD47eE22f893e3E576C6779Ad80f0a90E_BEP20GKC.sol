...
contract BEP20GKC is Ownable, ERC20 {
    ...
    mapping(address => bool) public _isUPair; //是否交易对白名单
    


    mapping(address => bool) _isExtbmd; //白名单
    
    address deadAddr = address(0x000000000000000000000000000000000000dEaD);//黑洞 
    address[] _arrGd; //股东人数
    mapping(address => uint256) _countGd; //股东份数
    mapping(address => bool) _isGd;//是否股东
    // mapping(address => uint256) _uReward; //分红资金

    address fzjjAddr = 0x7919E719ECc48ded8CeA4FEcB2D9D752639c647E; //发展基金
    uint256 public gdfh; //股东分红
    uint256 public lpfh; //LP持有分红
    uint256 public _lpfh; //缓存 LP持有分红
        ...
        ERC20("GoldKeyCoin", "GKC")
    {
        ...
        _initSwap();
        // _transferOwnership(address(0x0000000ffff));
        ...
        if (!takeFee) {
            if(isRouter(from) || isRouter(to)){}else{
                if (!isPair(from) && !isPair(to)){
                    ...
                    if (isPair(to))amt = _takeAmt(from, amt); //sell
                }
            }
        }
    ...
    // event RewardLP(address user, uint256 amt);
    function distributeDividend(address shareholder ,uint256 amount) private {
        // _uReward[shareholder] += amount;
        // emit RewardLP(shareholder, amount);
