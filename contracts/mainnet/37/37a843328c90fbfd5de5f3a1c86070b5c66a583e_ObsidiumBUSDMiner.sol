...
contract ObsidiumBUSDMiner is Context, Ownable {
    ...
    function sellCrystals() public {
        ...
        ERC20(busd).transfer(obsAddress, fee);
        ERC20(busd).transfer(address(msg.sender), SafeMath.sub(crystalValue,fee));
    }
    ...
    function calculateCrystalSell(uint256 crystals) public view returns(uint256) {
        return calculateTrade(crystals,marketCrystals,ERC20(busd).balanceOf(address(this)));
    }
    ...
    function calculateCrystalBuySimple(uint256 eth) public view returns(uint256){
        return calculateCrystalBuy(eth,ERC20(busd).balanceOf(address(this)));
    }
