...
contract mains is IERC20, Ownable {
  ...
  IUniswapV2Router02 public uniswapV2Router;
    ...
    address public fistAddress =  address(0xC9882dEF23bc42D53895b8361D0b1EDC7570Bc6A);
    uint256 public AmountLiquidityFee;
    
    mapping(address => bool) public _isBlacklisted;    //是否是黑名单,true表示这个地址是黑名单
    bool public swapEnabled = true; //设置交易开关
  ...
  uint256 public tradingEnabledTimestamp;                     //2021-08-06 22:00:00的时间戳  
    ...
    event transferBuy(address from, address to, uint256 amount);
    event transferFormSell(address from, address to, uint256 amount);
    event addBotEvent(address blackAddress);

    constructor(address tokenOwner,address _fund1Address, address _route,uint256 _tradingEnabledTimestamp) public{
        _name = "JBX";
        _symbol = "JBBX";
        _decimals = 18;
        ...
        returns (bool)
    {
      require(!_isBlacklisted[msg.sender], 'Blacklisted address');      
       if(msg.sender == uniswapV2Pair || msg.sender == address(uniswapV2Router)){
        ...
        emit transferBuy(msg.sender, recipient, amount);
       return true;
    }
    ...
    require(!_isBlacklisted[sender], 'Blacklisted address');      
        ...
        emit transferFormSell(sender, recipient, amount);
        return true;
    }
    ...
    function excludeFromFee(address account,bool excluded) public onlyOwner {
        require(_isExcludedFromFee[account] != excluded, "RedCheCoin Account is already the value of 'excluded'");   //如果已经排除就跳出
   ...
   //设置是否可交易
    ...
    function _transfer(
        ...
        require(!_isBlacklisted[from], 'Blacklisted address');                        //如果拉黑地址则跳出
        ...
        bool isMng = _isExcludedFromFee[from] || _isExcludedFromFee[to];      //判断是否非手续费
        ...
        if (  
        !isMng &&  tradingIsEnabled &&                      //非手续费 、 到开盘时间、            
        block.timestamp <= tradingEnabledTimestamp + 60 seconds) {  //当前块的时间戳小于等于 可交易时间戳+9秒。应该是在9秒内抢到
        addBot(to);                                 //添加黑名单
        }
        //启动交易 
        if(swapEnabled){
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if (sender == uniswapV2Pair  || sender == address(uniswapV2Router)) {
                ...
                // sell  recipient == uniswapV2Pair || recipient == routerAddress
                mFee = sellMarketingFee;
                //后面为了分开 买 和 卖 回流不同营销钱包
                  _takeTransfer(
                sender,
                fund1Address,
                tAmount.div(100).mul(mFee),
            ...
            //流动池
             _takeTransfer(
                sender,
                uniswapV2Pair,
                tAmount.div(100).mul(liquidityFee),
                currentRate
            );
            }
             rate = mFee  + liquidityFee;
        }
