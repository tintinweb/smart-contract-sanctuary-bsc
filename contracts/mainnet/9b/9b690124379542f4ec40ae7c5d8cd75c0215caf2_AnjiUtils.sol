...
contract AnjiUtils {
    ...
    function getTokensPairInfo(address[] memory token_addresses, address[] memory token_pair_addresses) public view virtual returns (PairReserveInfo[] memory) {
        PairReserveInfo[] memory reserveInfo = new PairReserveInfo[](token_addresses.length);

        for (uint256 index = 0; index < token_pair_addresses.length; index++) {
            reserveInfo[index] = getTokenPairReserveInfo(token_addresses[index], token_pair_addresses[index], address(0));
        }
