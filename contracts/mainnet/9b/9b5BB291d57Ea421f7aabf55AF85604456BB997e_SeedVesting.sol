...
contract SeedVesting is Pausable, Ownable, ReentrancyGuard {
    ...
    uint256 public TGE_RELEASE = 60; // 6%
    uint256 public TGE_CLIFF = 86400 * 30 * 3; // 3 months
    uint256 public VESTING_DURATION = 86400 * 30 * 18; // 18 months
    uint256 public TOKEN_PRICE = 40000000000000000; // 0.04 USD / token
    uint256 public TOTAL_ALLOCATION = 45000000000000000000000000; // 45.000.000 token
    ...
    mapping(address => uint256) private locks; // amount token locks of address
    mapping(address => uint256) private released; // amount token released of address

    event EventClaim(address indexed account, uint256 amount, uint256 time);
    ...
    function initial(address _token) external onlyOwner {
    ...
    ) external canSetup onlyOwner {
        require(_users.length == _balance.length, "Invalid input");
        for (uint256 i = 0; i < _users.length; i++) {
            //calculate
            uint256 tokenAmount = (_balance[i] * 10**18) / TOKEN_PRICE;
            locks[_users[i]] += tokenAmount;
    ...
    function setTime(uint256 _time) external canSetup onlyOwner {
        ...
        for (uint256 i = 0; i < whilelists.length; i++) {
            uint256 tokenAmount = (locks[whilelists[i]] * TGE_RELEASE) / 1000;
            locks[whilelists[i]] -= tokenAmount;
            tokenReward.safeTransfer(whilelists[i], tokenAmount * 10**18);
        }
    }
        ...
        onlyOwner
    {
        ...
        uint256 tokenAmount = (_newBalance * 10**18) / TOKEN_PRICE;
        locks[_user] = tokenAmount;
    }

    function claim() external canClaim nonReentrant whenNotPaused {
        ...
        tokenReward.safeTransfer(_msgSender(), amount * 10**18);

        emit EventClaim(_msgSender(), amount, block.timestamp);
    }
