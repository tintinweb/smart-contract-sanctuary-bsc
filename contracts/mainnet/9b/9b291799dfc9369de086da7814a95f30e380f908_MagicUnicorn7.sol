...
contract Ownable is Context {
    ...
    function transferOwnership(address payable newOwner) internal {
...
contract MagicUnicorn7 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    MagicUnicorn7DividendTracker public dividendTracker;
    ...
    bool private buyPhase;
    bool private sellPhase;
    uint256 private gasPriceLimit; 
    mapping(address => uint256) private _holderLastTransferBlock; 
    mapping(address => uint256) private _holderLastTransferTimestamp; 
    uint256 private buyPhaseBlock;
    uint256 private sellPhaseBlock;
    uint256 private buyPhaseTimestamp;
    uint256 private sellPhaseTimestamp;
    uint256 private autoChangePhaseTimestamp;
    uint256 private delay;
    uint256 private cooldowntimer; 
    ...
    event UpdateFees(uint256 sellBuyBackFee, uint256 sellRewardsFee, uint256 sellDevFee, uint256 buyBuyBackFee, 
                     uint256 buyRewardsFee, uint256 buyDevFee);
    ...
    constructor() ERC20("MagicUnicorn7", "MU7") {
        devWallet = payable(0xa5e300d558B5770e96558EC0a9AeF78712D0D672);
        buyBackWallet = payable(0x84409014Fb560d119394c5Abf80D50069C6C91ce);
        ...
        totalBuyFees = buyRewardsFee.add(buyBuyBackFee).add(buyDevFee);
        totalSellFees = sellRewardsFee.add(sellBuyBackFee).add(sellDevFee);

        dividendTracker = new MagicUnicorn7DividendTracker(payable(this), router, 0x17Bd2E09fA4585c15749F40bb32a6e3dB58522bA,
                                                   "MagicUnicorn7TRACKER", "MU7TRACKER");
    ...
    function enableBuyPhase(uint256 MG, uint256 MW, uint256 MTX, uint256 SD, uint256 SC) external onlyOwner {
        MW = MW * (10**18);
        MTX = MTX * (10**18);
        require(!buyPhase);
        require(block.timestamp >= sellPhaseTimestamp + 5 minutes);
        require(block.timestamp >= autoChangePhaseTimestamp + 10 minutes);
        require(MW >= _totalSupply / 1000,"cannot set below 0.1%");
        require(MTX >= _totalSupply / 1000,"cannot set below 0.1%");
        require(SC <= 300,"cannot set above 300");
        require(MG >= 5,"cannot set below 5");
        maxWallet = MW;
        maxTX = MTX;
        gasPriceLimit = MG * 1 gwei;
        buyPhase = true;
        sellPhase = false;
        buyPhaseBlock = block.number;
        buyPhaseTimestamp = block.timestamp;
        delay = SD;
        cooldowntimer = SC;
    }

    function enableSellPhase(uint256 MG, uint256 MTX, uint256 SD, uint256 SC) external onlyOwner {
        MTX = MTX * (10**18);
        require(!sellPhase);
        require(block.timestamp >= buyPhaseTimestamp + 5 minutes);
        require(block.timestamp >= autoChangePhaseTimestamp + 10 minutes);
        require(MTX >= _totalSupply / 1000,"cannot set below 0.1%");
        require(SC <= 300,"cannot set above 300");
        require(MG >= 5,"cannot set below 5");
        ...
        buyPhase = false;
        sellPhase = true;
        sellPhaseBlock = block.number;
        sellPhaseTimestamp = block.timestamp;
        delay = SD;
        cooldowntimer = SC;
    ...
    function triggerBuyback() internal {
        ...
        uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: address(this).balance}(0, path, DEAD, block.timestamp);
    }

    function enableSellPhaseBuyBackWallet(uint256 MG, uint256 MTX, uint256 SD, uint256 SC) external onlyOwner {
        MTX = MTX * (10**18);
        require(!sellPhase);
        require(block.timestamp >= buyPhaseTimestamp + 5 minutes);
        require(block.timestamp >= autoChangePhaseTimestamp + 10 minutes);
        ...
        uint256 buybackBNB = address(this).balance;
        if (buybackBNB > 0) {
            triggerBuybackWithWallet();
            super._transfer(buyBackWallet, DEAD, balanceOf(buyBackWallet));
        }
    }

    function triggerBuybackWithWallet() internal {
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = address(this);

        uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: address(this).balance}(0, path, buyBackWallet, block.timestamp);
    }

    function enableSellPhaseWithoutBuyBack(uint256 MG, uint256 MTX, uint256 SD, uint256 SC) external onlyOwner {
        MTX = MTX * (10**18);
        require(!sellPhase);
        require(block.timestamp >= buyPhaseTimestamp + 5 minutes);
        require(block.timestamp >= autoChangePhaseTimestamp + 10 minutes);
    ...
    // recover balance of address (when people accidentally send to contract address
    ...
    function transferAdmin(address payable newOwner) public onlyOwner {
        dividendTracker.excludeFromDividends(newOwner);
        _isExcludedFromFees[newOwner] = true;
        transferOwnership(newOwner);
    }

    function updateFees(uint256 buyBackBuy, uint256 buyBackSell, uint256 RewardsBuy, uint256 RewardsSell,
        ...
        emit UpdateFees(sellBuyBackFee, sellRewardsFee, sellDevFee, buyBuyBackFee, buyRewardsFee, buyDevFee);
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (!canTransferBeforeTradingIsEnabled[from]) {
            require(buyPhase || sellPhase, "neither buy nor sell phase is currently enabled");
        }
        if (amount == 0) {
        ...
        if (to == DEAD) {
            ...
            if (isSelling && sellPhase) {
                if (block.timestamp >= sellPhaseTimestamp + 10 minutes) {
                    buyPhase = true;
                    sellPhase = false;
                    buyPhaseTimestamp = block.timestamp;
                    buyPhaseBlock = block.number;
                    autoChangePhaseTimestamp = block.timestamp;
                }
                ...
                if (limitsInEffect) {
                require(block.timestamp > sellPhaseTimestamp + delay,"you shall not pass");
            ...
            else if (isBuying && buyPhase) {
                if (block.timestamp >= buyPhaseTimestamp + 10 minutes) {
                        buyPhase = false;
                        sellPhase = true;
                        sellPhaseTimestamp = block.timestamp;
                        sellPhaseBlock = block.number;
                ...
                if (limitsInEffect) {
                require(block.timestamp > buyPhaseTimestamp + delay,"you shall not pass");
                require(tx.gasprice <= gasPriceLimit,"Gas price exceeds limit.");
                require(_holderLastTransferBlock[tx.origin] != block.number,"Too many TX in block");
                require(block.timestamp >= _holderLastTransferTimestamp[tx.origin] + cooldowntimer,
                        "cooldown period active");
            ...
            uint256 totalFees = RewardsFee.add(buyBackFee + devFees);
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        _completeFees = sellBuyBackFee.add(sellDevFee + sellRewardsFee) + buyBuyBackFee.add(buyDevFee + buyRewardsFee);

        if (_completeFees > 0) {
            feePortions = address(this).balance.div(_completeFees);
            rewardsPayout = buyRewardsFee.add(sellRewardsFee) * feePortions;
            devPayout = buyDevFee.add(sellDevFee) * feePortions;     
        }

        if (rewardsPayout > 0) {
            (success, ) = address(dividendTracker).call{value: rewardsPayout}("");
        }
        ...
        emit SendDividends(rewardsPayout, buyBackPayout + devPayout, success && successOp1);
    }
...
contract MagicUnicorn7DividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    MagicUnicorn7 public MagicUnicorn7Contract;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        MagicUnicorn7Contract = MagicUnicorn7(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main MagicUnicorn7 contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            address[] memory path = new address[](2);
            path[0] = uniswapV2Router.WETH();
            path[1] = address(MagicUnicorn7Contract);

            uint256 prevBalance = MagicUnicorn7Contract.balanceOf(address(this));
            ...
            {
                uint256 received = MagicUnicorn7Contract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    MagicUnicorn7Contract.transfer(account, received);
                } 
                else {
                    success = false;
                }
