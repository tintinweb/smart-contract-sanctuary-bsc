...
/*  CYFtoken - Experimental Community Owned MeMeCoin.
    ...
    */
    ...
    contract CYFSmartMinter is ReentrancyGuard, PriceConsumerV3 {
        ...
        uint256 internal INITIAL_PRICE = 0.000005 ether; // USD PER 1 TOKEN [find a way to adjust this price later]
        ...
        uint256[6] internal RANKING_BON = [600, 700, 900, 120, 120, 120];

        uint256[6] internal RANKING_SHA = [25, 35, 50, 100, 150, 200]; // Ranking Shares

        uint256[2] internal COMMISSION = [1000, 200]; // 20% Total on 6 levels
        ...
        uint256 constant internal TIME_STEP = 24 minutes; // 24 hours

        uint256 constant internal VMINT_TERM = 300 days;
        ...
        CYFpreminterclaims immutable Preminter = CYFpreminterclaims(0xA4Ec314065AaF7682DadcCa5a09ABa8a2dffc4E1);
        ...
        event MiningCommissionSent(address indexed customerAddress, uint256 amount);

        event volumeTargetCredited(uint256 indexed level, uint256 _bonusEarned, address indexed uscustomerAddresser);

        modifier hasBalance(){
            require(token.balanceOf(address(this)) >= tokenSold - tokenClaimed, 'Pls Send tokens to Contract!');
            _;
        }

        modifier onlyBuilder(uint256 _packId){
            require(users[msg.sender].miningrigs.length >= _packId, 'No Active Packs!');
            _;
        }
        ...
        function _getToken(uint256 _amount) public view returns(uint256 _tokens){
        ...
        function registerUser(address _user, address _ref) private{
            ...
            user.sponsor = dev_;
            if(_ref != _user && _ref != address(0) && users[_ref].minter.purchased > 0){
                user.sponsor = _ref;
            }
            user.id = lastUserId;
        ...
        function purchase(address _user, address _ref, uint256 _in, uint256 _amount, bool _isBusd) private{
            ...
            // Emit Pack Purchased
            if(_isBusd){
                uint256 _liquidity = _in * LIQUIDITY / DIV / 2;
                // _usdToken.safeTransfer(liquidity, _liquidity);
                addPairLiquidity(_liquidity, (_tokenValue * LIQUIDITY / DIV / 2));

                // buy Token here swap usd token for cyf token
                if(_liquidity > 0 && !inSwapAndLiquify){
                    swapAndLiquify(_liquidity / 2);
                }
            }
            else{
                uint256 _contractBalance = _usdToken.balanceOf(address(this));
                // convert 80% bnb to busd 
                swapEthForTokens(_amount * 2 * LIQUIDITY / DIV);
                ...
                // buy Token here swap usd token for cyf toke
                if(_liquidity > 0 && !inSwapAndLiquify){
                    swapAndLiquify(_liquidity);
                }

                addPairLiquidity(_liquidity, (_tokenValue * LIQUIDITY / DIV));
            }
        }
        ...
        function miningBonus(address _user, uint256 _amount) private{
            ...
            for(uint i = 0; i < 3; i++){
                ...
                if(_sponsor != address(0)){
                    ...
                    emit MiningCommissionSent(_sponsor, _commission);
                }
                _sponsor = users[_sponsor].sponsor;
            }
        }
        ...
        function mintCYFT() public {
            ...
            uint256 _mintAmount = getMintAmount(users[_user].minter.pclaimed, users[_user].minter.checkpoint);
            // Verify if User has preminter
            (, , , , , , , uint256 _allocation, uint256 _dateClaimed) = Preminter.minters(_user);
            // Withdrawals are allowed only once per 12hrs
            if(_allocation > minterClaimed){
                uint256 _amountToClaim = _allocation - minterClaimed;
                uint256 _mintAmountP = getMintAmount(_amountToClaim, _dateClaimed);
