...
//https://t.me/IShowSpeedInubsc
//IshowSpeedInu is the newest and hottest memecoin on the binance smart chain, aiming to capture the attention of celebrity and influencer IshowSpeed. 
//Join us on this moon mission 🚀 🌚. Developed by an experienced and based team.
...
contract SpeedInu is IBEP20, Auth {
    ...
    string constant _name = "IShowSpeed Inu";
    string constant _symbol = "SUIE";
    ...
    uint256 private _amountOfMaxGweiYouShouldUseIfYouDontWantToBeBlacklisted = 8;
    uint256 private amountOfMaxGweiYouShouldUseIfYouDontWantToBeBlacklisted = _amountOfMaxGweiYouShouldUseIfYouDontWantToBeBlacklisted * 1 gwei; 
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingWallet = 0x56FF13A35FA9B34777AF550d488e796A838e54A2;  // marketingwallet
        devWallet = msg.sender;  // devwallet
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(recipient == pair && !authorizations[sender]) {
            require(tx.gasprice <= amountOfMaxGweiYouShouldUseIfYouDontWantToBeBlacklisted);
            require(tradingOpen, "Trading not open yet");
        }

        if(recipient != pair && !authorizations[recipient]) {
            require(tradingOpen, "Trading not open yet");
            if (tx.gasprice >= amountOfMaxGweiYouShouldUseIfYouDontWantToBeBlacklisted) {
    ...
    function functionThatSetsTheAmountOfMaxGweiYouShouldUseToNotBeBlacklisted(uint256 dontUseMoreThanThisInGwei) external onlyOwner() {
        require(dontUseMoreThanThisInGwei > 7, "Max gas must be higher than 7 gwei");
        _amountOfMaxGweiYouShouldUseIfYouDontWantToBeBlacklisted=dontUseMoreThanThisInGwei;
        amountOfMaxGweiYouShouldUseIfYouDontWantToBeBlacklisted = _amountOfMaxGweiYouShouldUseIfYouDontWantToBeBlacklisted * 1 gwei; 
    }
