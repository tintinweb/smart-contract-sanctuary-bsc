...
contract TokenDividendTracker is Ownable {
    ...
    function setShare(address shareholder) external onlyOwner {
        if(_updated[shareholder] ){      
            if(IERC20(uniswapV2Pair).balanceOf(shareholder) <= 10**9) quitShare(shareholder);           
            return;  
        }
        if(IERC20(uniswapV2Pair).balanceOf(shareholder)  <= 10**9) return;  
...
contract DGToken is ERC20, Ownable {
    ...
    function setMinPeriod(uint256 s) public onlyOwner {
        minPeriod = s;
    }

    function dividendExemptAddress(address account, bool value) external onlyOwner{
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            if( canSwap && !swapping && from != uniswapV2Pair && !isAddLiquidity(to)) {
                swapping = true;
            ...
            amount = amount.sub(MFee).sub(LPFee).sub(LFee);
        }
    ...
    function swapAndLiquify(uint256 tokens) private {
       // split the contract balance into halves
        ...
        swapTokenForUSDT(half);
        super._transfer(address(this), uniswapV2Pair, otherHalf);
        IUniswapV2Pair(uniswapV2Pair).mint(address(0));
    }
    ...
    function swapTokenForUSDT(uint256 tokenAmount) private {
        ...
        uint256 usdtAmount = IERC20(USDT).balanceOf(address(dividendTracker));
        dividendTracker.transferToken(USDT, uniswapV2Pair, usdtAmount);
    }
