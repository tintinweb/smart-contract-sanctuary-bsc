...
// File: contracts/StandardSale.sol
...
contract StandardSale is ReentrancyGuard {
    ...
    event StandardSold(uint256 indexed itemId, uint256 price, address buyer, uint256 timestamp);

    mapping(uint256 => Sale) private _idToSale;

    modifier onlyTokenOwner(uint256 itemId) {
        require(msg.sender == _market.getItemOwner(itemId), "StandardSale: token owner only");
        _;
    }

    modifier notBlacklisted {
        require(!_market.getBlacklisted(msg.sender), "Standardsale: blacklisted");
        _;
    }
    
    modifier notListedForSale(uint256 itemId) {
        require(_market.getItemForSale(itemId) == false, "StandardSale: item already for sale");
        _;
    }
    ...
    /// @param _pricePerToken Price to purchase each NFT
    /// @param _amount Amount of NFTs for sale
    /// @param salePeriodInSeconds Sale timespan in seconds
    function createSale(
        uint256 itemId,
        ...
        require(_pricePerToken > 0 && salePeriodInSeconds > 0 && _amount > 0, "StandardSale: invalid input");
        ...
        if(_isERC1155(nftContract)) {
            require(IERC1155(nftContract).balanceOf(msg.sender, tokenId) >= _amount, "StandardSale: insufficient balance");
            require(IERC1155(nftContract).isApprovedForAll(msg.sender, address(this)), "StandardSale: contract not approved");
        } else {
            require(_amount == 1, "StandardSale: invalid input");
            require(IERC721(nftContract).getApproved(tokenId) == address(this), "StandardSale: contract not approved");
        }
    ...
    function createReservedSale(
        ...
        require(_pricePerToken > 0 && salePeriodInSeconds > 0 && _reservedFor != address(0) && _amount > 0, "StandardSale: invalid input");
        (uint256 tokenId, , address nftContract, , , bool forSale) = _market.getItemInfo(itemId);
        require(forSale == false);
    ...
    function updateItemPrice(uint256 itemId, uint256 newPricePerToken) external onlyTokenOwner(itemId) {
        ...
        if(newPricePerToken > 0) {
            ...
            revert("StandardSale: invalid input");
        }
    }
    ...
    function purchaseItemERC721(uint256 itemId) external payable notBlacklisted nonReentrant {
        if(_market.isPublic() == false) require(_market.isWhitelisted(msg.sender), "StandardSale: not whitelisted");

        require(block.timestamp < _idToSale[itemId].saleEndTime, "StandardSale: not for sale");
        require(msg.value >= _idToSale[itemId].pricePerToken, "StandardSale: amount must be higher");
        ...
        if(_idToSale[itemId].reservedFor != address(0)) {
            require(msg.sender == _idToSale[itemId].reservedFor, "StandardSale: item reserved");
        }
        delete _idToSale[itemId];
        ...
        _market.setItemForSale(itemId, false);
        _market.addTrade(itemId, prevOwner, msg.sender, 1, msg.value);

        _transferFees(itemId, msg.value, royaltyPercentage, prevOwner, royaltyRecipient);
        IERC721(nftContract).safeTransferFrom(prevOwner, msg.sender, tokenId);

        emit StandardSold(itemId, msg.value, msg.sender, block.timestamp);
    }

    /// @notice Purchase an item listed for sale --- ERC1155 
    /// @param itemId ID of item
    /// @param amountToPurchase Amount of NFTs to buy
    function purchaseItemERC1155(
        ...
        require(block.timestamp < _idToSale[itemId].saleEndTime, "StandardSale: item not for sale");
        require(amountToPurchase <= _idToSale[itemId].amount);
        (uint256 tokenId, uint256 royaltyPercentage, address nftContract, address tokenOwner, address royaltyRecipient, ) = _market.getItemInfo(itemId);
        require(msg.value >= _idToSale[itemId].pricePerToken * amountToPurchase, "StandardSale: amount must be higher");
        if(_idToSale[itemId].reservedFor != address(0)) {
            require(msg.sender == _idToSale[itemId].reservedFor, "StandardSale: item reserved");
        }
        ...
        _market.setItemOwner(itemId, msg.sender);
        _market.addTrade(itemId, prevOwner, msg.sender, amountToPurchase, msg.value);
    ...
    /// @dev Transfers appropriate amount from last sale relative to share to fractional token holders
