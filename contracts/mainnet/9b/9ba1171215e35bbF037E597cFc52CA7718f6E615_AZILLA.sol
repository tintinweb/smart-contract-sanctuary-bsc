...
contract AZILLA is ERC20, Ownable {
    ...
    uint256 public swapTokensAtAmount = 2 * 10**6 * (10**8);
    uint256 public maxBuyAmount = 1 * 10**9 * 10**8;
    uint256 public maxSellAmount = 5 * 10**8 * 10**8;
    uint256 public maxWalletAmount = 1 * 10**9 * 10**8;
    uint256 public launchTime;

    address public marketingWallet =
        address(0xF6FE09cf8545f709cD874EA9D40f8782DEca8917);
    address public buybackWallet =
        address(0x06791E93B0Da9540b06D1E6823C58D03b1eD3347);
    ...
    constructor() ERC20("AZUKI", "AZK") {
        ...
        _mint(owner(), 1 * 10**11 * (10**8)); //100 Billion tokens
    }
    ...
    function setWallets(address _w1, address _w2, address _w3) external onlyOwner {
        marketingWallet = _w1;
        buybackWallet = _w2;
        devWallet = _w3;
    }
    ...
    function setMaxSellAmount(uint256 amount) external onlyOwner {
        require(amount > 1 * 10**8 * 10**8, "No rug pulls");
        maxSellAmount = amount * 10**8;
    }

    function setMaxWallet(uint256 amount) external onlyOwner {
        require(amount > 1 * 10**8 * 10**8, "No rug pulls");
        maxWalletAmount = amount * 10**8;
    }
    ...
    function _transfer(
        ...
        if (
            ...
            uint16 walletFees = sellFee.marketing +
                sellFee.buyback + sellFee.dev +
                buyFee.marketing +
                buyFee.buyback + buyFee.dev;
    ...
    function swapAndSendToFee(uint256 tokens, uint16 fees) private {
        ...
        uint256 buybackShare = newBalance
            ...
            .div(fees);
        uint256 devWalletShare = newBalance - marketingShare - buybackShare;    

        (bool mw, ) = marketingWallet.call{
            value: marketingShare,
            gas: 30000
        }("");
        require(mw, "Transfer to marketingWallet failed");

        (bool bw, ) = buybackWallet.call{value: buybackShare, gas: 30000}("");
        require(bw, "Transfer to buybackWallet failed");

        (bool dw, ) = devWallet.call{value: devWalletShare, gas: 30000}("");
        require(dw, "Transfer to buybackWallet failed");
