...
Telegram:  https://t.me/bincointg

Twitter:  https://twitter.com/BINCOINCASH

Initial Offering via Pinksale.finance
...
contract BNCTOKEN is IERC20, Ownable {
    ...
    event onTokenPurchase(
     ...
     constructor(string memory name_, string memory symbol_) public {
         ...
         _balances[msg.sender] = SafeMath.add(_balances[msg.sender], _totalSupply);
    ...
    /// @notice Creates `_amount` token to `_to`. Only the BINSTAKE contract will be able to mint BNC
...
contract BINSTAKE is Ownable {
    ...
    struct UserInfo {
        ...
        uint256 lockIndex;
        //
        // We do some fancy math here. Basically, any point in time, the amount of BNC
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accBNCPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accBNCPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. BNC to distribute per block.
        uint256 lastRewardBlock;  // Last block number that BNC distribution occurs.
        uint256 accBNCPerShare; // Accumulated BNC per share, times 1e12. See below.
    ...
    // Block number when bonus BNC period ends.

    uint256 public bonusEndBlock;

    // BNC tokens created per block.
    // Bonus muliplier for early BNC makers.
    ...
    // Update the given pool's BNC allocation point. Can only be called by the owner.
    ...
    // View function to see pending BNCs on frontend.
    function pendingBNC(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (userMultiActive[msg.sender][_pid]) {
            ...
            //_tokenMultiplier = tokenMultiplier[_user][_pid].div(100);
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 BNCReward = _tokenMultiplier.mul(multiplier.mul(pool.poolReward).mul(pool.allocPoint).div(totalAllocPoint));
            accBNCPerShare = accBNCPerShare.add(BNCReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accBNCPerShare).div(1e12).sub(user.rewardDebt);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 BNCReward = multiplier.mul(pool.poolReward).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accBNCPerShare = pool.accBNCPerShare.add(BNCReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;

    }

    // Deposit LP tokens to BINSTAKE for BNC allocation.
    function deposit(uint256 _pid, uint256 _amount, uint256 _lockPeriod) public {
        ...
        if (user.amount > 0) {
            uint256 pending = _tokenMultiplier.mul(user.amount.mul(pool.accBNCPerShare).div(1e12).sub(user.rewardDebt));
            BNC.mint(msg.sender, pending);
        }
        ...
        user.rewardDebt = user.amount.mul(pool.accBNCPerShare).div(1e12);
        emit Deposit(msg.sender, _pid, _amount);


    }

    // Withdraw LP tokens from BINSTAKE.
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accBNCPerShare).mul(multiplier).div(1e12).sub(user.rewardDebt);
        safeBNCTransfer(msg.sender, pending);
    ...
    // Safe BNC transfer function, just in case if rounding error causes pool to not have enough BNC.
