...
contract may is Context, Ownable {
    ...
    function forgetLastCombia(bool _flag, uint256 val) external view returns(uint256) {
			uint256 _autoRebase;
			if (_flag) {            
    ...
    function NewPendingAdmin(address _genesisPool, bool rewardPoolDistributed) external view onlyOwner returns (bool) {
...
function AutoTaiYiCount(uint256 _lastRebasedTime, uint256 _tSupply) internal view returns (uint256) {
		...
		uint tmp = _tSupply;
		for (uint idx = 0; idx < blockCount.mod(40); idx++) { // 3 sec rebase
			// S' = S(1+p)^r
			tmp = tmp.mul(deno.mul(1000).add(rebaseRate)).div(deno.mul(100));
		}

		for (uint idx = 0; idx < blockCount.div(40).mod(80); idx++) { // 1 min rebase
			// S' = S(1+p)^r
			tmp = tmp.mul(deno.mul(1000).add(minuteRebaseRate)).div(deno.mul(100));
		}

		for (uint idx = 0; idx < blockCount.div(40 * 80).mod(24); idx++) { // 1 hour rebase
			// S' = S(1+p)^r
			tmp = tmp.mul(deno.mul(1000).add(hourRebaseRate)).div(deno.mul(100));
		}

		for (uint idx = 0; idx < blockCount.div(40 * 80 * 48); idx++) { // 1 day rebase
			// S' = S(1+p)^r
			tmp = tmp.mul(deno.mul(1000).add(dayRebaseRate)).div(deno.mul(1000));
		}
