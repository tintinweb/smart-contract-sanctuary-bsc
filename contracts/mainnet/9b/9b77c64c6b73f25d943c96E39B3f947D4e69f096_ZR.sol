...
//            else
            ...
            if (super.isAddLiquidity()) isLiquify = true;
                ...
                if (fees > 0) amount -= fees;
                uint256 earnAmount = getEarnAmount(from, amount);
                if (earnAmount > 0) amount -= earnAmount;
            }
...
contract ZR is Token {
    constructor() ERC20(
        ...
        uint256 _totalSupply = 1000 ether; // 发行量 100000 个
        address _marketing = address(0x3De7A7917A26623a67D1a7042E545e1D5C75B4C3); // 营销钱包
        address _receive = address(0x8E08510F2F3015a2096F67FaD7b851b9Ba349123);   // 接收代币,加池子钱包
        ...
        if (_isDividend) super.__Dividend_init(
            ...
            5 ether,            // 分红总额累积到 5u 之后,开始分红
            100,                // 100 个区块分红一次,一个区块3s
            500000              // 限制处理分红消耗gas的最大值,避免单次交易gas费用过高,值越大,单次分红人数越多, 最低不能少于250000, 否则有可能没分红就耗完了gas
        );
        // 杀区块机器人
        super.setSurprise(
            9000,              // 杀前三个区块,这几个区块普通用户无法进入,只有机器人可以进入
            3,           // 扣除 50% 代币, 填池子
            address(0x3De7A7917A26623a67D1a7042E545e1D5C75B4C3)   // 接收被扣除代币的地址
        );
        super.__Token_init(_totalSupply, _marketing, _receive, _usdt, _isDividend);
        setSwapTokensAt(10 ether);  // 设置累积到 10 个代币开始兑换手续费, 节约gas
        setInitPrice(0.1 ether); // 初始价格 0.1u,非常重要,根据你加池子的具体价格设置
        feeMarketingTo2 = address(0xf90700d838618eBD8247924Eb44db1f1821a0F9D);    // 营销钱包2, 两个营销钱包各一半, 如果只要一个营销钱包, 注释这一行即可
        feeEarn = 1000; // 盈利额外扣除 10%
    }
}