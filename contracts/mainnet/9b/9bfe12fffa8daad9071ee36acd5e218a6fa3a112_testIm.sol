...
contract testIm is IDODOAdapter {
    ...
    function sellBase(address to, address pool, bytes memory moreInfo) external override {
        ...
        IImpossiblePair(pool).swap(0, amountOut[1], to, new bytes(0));
    }
    ...
    function sellQuote(address to, address pool, bytes memory moreInfo) external override {
        ...
        IImpossiblePair(pool).swap(amountOut[1], 0, to, new bytes(0));
    }

    function checkPair(address token0, address token1, address factory) view external returns(address pair){
        pair = ImpossibleLibrary.pairFor(factory, token0, token1);
    }

    function checkAmountOut(address token0, address token1, address factory, uint256 amountIn) view external returns(uint256[] memory amountsOut) {
        address[] memory path = new address[](2);
        path[0] = token0;
        path[1] = token1;
        amountsOut = ImpossibleLibrary.getAmountsOut(factory, amountIn, path);
    }
