...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    contract TRIOTIS is ERC20, Ownable {
        using SafeMath for uint256;
        using Address for address;

        string private    _name = "TRIOTIS";
        string private  _symbol = "TRIOT";
    ...
    uint256 public _buySellFee = 5; 
    
   
    
    uint256 private _previousLiquidityFee;
    uint256 private _previousBuySellFee;
  
   address payable triotisWallet = payable(0xA75C898592d47e721534fdf0c366Ca4340b318d9);
    ...
    constructor () ERC20() {
        ...
        _isExcludedFromFee[triotisWallet]       = true;
    ...
    function totalFees() public view returns (uint256) {
        return _liquidityFee.add(_buySellFee);
    }
    ...
    function changetriotisWallet(address payable wallet) public onlyOwner
    {
        triotisWallet = wallet;
    }

    function changeLiquidityPercentage(uint256 percentage) public onlyOwner
    {
        require(percentage<=10,"cannot set higher than 10");
   ...
   function changeTaxPercentage(uint256 percentage) public onlyOwner
    ...
    function removeAllFee() private {
        if (_liquidityFee == 0 && _buySellFee == 0 ) {
            return;
        }
    ...
    function swapBack(uint256 tokensToLiquify) internal lockTheSwap {
        ...
        uint256 bnbForBuySell = bnbBalance.mul(_buySellFee).div(bnbFeeFactor);
      
      

        addLiquidity(tokensToLP, bnbForLiquidity);

        payable(triotisWallet).transfer(bnbForBuySell);
    }
