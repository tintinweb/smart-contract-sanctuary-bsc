...
contract SALES is BEP20 {
    ...
    function createOrder(uint value) external returns(uint) {
        require(value > 0, "The value must be greater than zero");
        require(Orders[msg.sender] == 0, "Order already completed");
        busd.transferFrom(msg.sender, address(this), value);
        uint output = value;
        if (!_isExcludedFromFee[msg.sender]) {
            output = output.sub(value.mul(FEE).div(1000));
        }else{
            output = value;
        }
        Orders[msg.sender] = output;
        return output;
    }

    function cancelOrder() external returns(uint){
        require(Orders[msg.sender] > 0, "No order available");
        uint total = Orders[msg.sender];
        Orders[msg.sender] = 0;
        busd.transfer(msg.sender, total);
        return total;
    }

    function closeOrder(address seller, address buyer) external onlyTeam returns(uint) {
        require(Orders[seller] > 0, "No order available");
        uint total = Orders[seller];
        Orders[seller] = 0;
        busd.transfer(buyer, total);
        return total;
    }
    ...
    function getOrder(address _addr) public view returns(uint){
        return Orders[_addr];
    }
    ...
    function getSytemActivated() external view returns(bool){
        return _actived;
    }
