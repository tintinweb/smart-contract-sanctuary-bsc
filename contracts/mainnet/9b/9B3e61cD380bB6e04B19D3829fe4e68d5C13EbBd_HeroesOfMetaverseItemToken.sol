...
import "./@rarible/royalties/contracts/LibRoyaltiesV2.sol"; // _INTERFACE_ID_ROYALTIES


contract HeroesOfMetaverseItemToken is
  ERC721,
  ERC721Enumerable,
  ERC721URIStorage,
  ERC721Burnable,
  ...
  RoyaltiesV2Impl
{
    ...
    constructor() ERC721("Heroes of Metaverse Item Token", "HOMIT") {
    ...
    function safeMint(address to, string memory uri, LibPart.Part memory royalty) public onlyRole(MINTER_ROLE) {
    ...
    function safeMint(address to, string memory uri) public onlyRole(MINTER_ROLE) {
        ...
        royalty.account = payable(_msgSender());
        royalty.value = 300;
        _setRoyalty(tokenId, royalty);
    ...
    function safeMintMultiple(address[] memory to, string [] memory uris) public onlyRole(MINTER_ROLE) {
        require(to.length == uris.length, "both arrays should have same size");
    ...
    public
    view
    override(ERC721, ERC721URIStorage) // todo: test this!!
    ...
    function _setRoyalty(uint _tokenId, LibPart.Part memory royalty)
    internal
    {
        // todo: this needs to be reviewed. mintable allows only one royalty. rarible allow a list of royalties.
        require(royalty.account != address(0x0), "Recipient should be present");
        require(royalty.value <= 300, "Royalties value should not be more than 3%");
        // Rarible
        LibPart.Part[] memory _royalties = new LibPart.Part[](1);
        _royalties[0]=royalty;
        _saveRoyalties(_tokenId, _royalties);
        // Mintable
        super._setTokenRoyalty(_tokenId, royalty.account, royalty.value);
    }
     ...
     */
    function withdraw(uint256 amount) virtual onlyRole(WITHDRAWABLE_ROLE) nonReentrant public {
    ...
    function withdrawERC20(
        ...
        uint256 amount
    ) external virtual nonReentrant onlyRole(WITHDRAWABLE_ROLE) {
    ...
    ) external virtual onlyRole(WITHDRAWABLE_ROLE) nonReentrant {
...
abstract contract AbstractRoyalties {
    mapping (uint256 => LibPart.Part[]) public royalties; // todo: check if it can be internal
