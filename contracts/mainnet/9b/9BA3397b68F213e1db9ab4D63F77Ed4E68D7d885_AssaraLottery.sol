...
// File: ASSARA_contract_final/IERC20.sol
...
// File: ASSARA_contract_final/Context.sol
...
// File: ASSARA_contract_final/SafeMath.sol
...
// File: ASSARA_contract_final/Assa_Contract.sol
...
contract AssaraLottery is Context, Ownable{
    ...
    function _StartLottery(uint64 _ig) private {
               ...
               lottToken = lottToken.sub(_Winner_val).sub(_Charity_fee).sub(_Burn_fee);
                ...
                uint64 winnerIndex = random(ticketCode , i , _ig);
    ...
    // interanl randomize
    function random(uint64 amount, uint64 ix, uint64 ig) private returns (uint64) {
    ...
    function HowMuchTokenMustPay() public view returns(uint){
        ...
        if(pair.token0() == token_address){
            howmuchtoken = getTokenPrice0(10);
        }else{
            howmuchtoken = getTokenPrice1(10);
        }
        return howmuchtoken;
    }
