...
import "./interfaces/IJlpPool.sol";

contract SellTaxDistributorBNB is Ownable, ReentrancyGuard {
    //---------- Libraries ----------//
    ...
    //---------- Events -----------//
    event ProfitsDestributed(uint256 Stake, uint256 StakeLP, uint256 Team);

    //---------- Constructor ----------//
    constructor(address _pool, address _poolLp, address _team) public {
        pool = payable(_pool);
        poolLP = payable(_poolLp);
        team = payable(_team);
        minDistribution = 1 ether;
        beneficiariesFees.Stake = 2000; // 20%
        beneficiariesFees.StakeLP = 4000; // 40%
        beneficiariesFees.Team = 4000; // 40%
    }
    ...
    function _distributeProfits() internal {
        ...
        uint256 toStakeLP = (amount * beneficiariesFees.StakeLP) / 10000;
        uint256 toTeam = amount.sub(toStake.add(toStakeLP));
        IJlpPool(pool).deposit{value: toStake}();
        IJlpPool(poolLP).deposit{value: toStakeLP}();
        team.sendValue(toTeam);
        emit ProfitsDestributed(toStake, toStakeLP, toTeam);
    }
    ...
    function canDistribute() public view returns (bool) {
        uint256 amount = balanceBNB();
        uint256 staked = IJlpPool(pool).totalStaked();
        uint256 stakedLP = IJlpPool(poolLP).totalStaked();
        if (staked == 0 || stakedLP == 0) {
            return false;
        }
    ...
    function setBeneficiariesFee(
        ...
        require(_Stake + _StakeLP + _Team == 10000, "Fees out of bounds");
