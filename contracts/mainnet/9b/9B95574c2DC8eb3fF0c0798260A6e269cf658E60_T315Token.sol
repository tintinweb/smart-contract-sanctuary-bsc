...
contract T315Token is Context, IERC20, Ownable {
    ...
    mapping (address => bool) public _isLimitSellers;
    mapping (address => bool) public _limitFirst;
    mapping (address => bool) public _limitSecond;

    uint8 private _decimals = 6;
    uint256 private _tTotal = 3150000 * 10 ** 6;
    ...
    constructor (
        ...
        lpAwardCondition = LpAwardCondition(315e5,315e5);
        holder = _holder;
        ...
        lpInterest.minAward = 1e2;
        lpInterest.period = 3600;
        lpInterest.sendCount = 50;
        emit Transfer(address(0), _holder, _tTotal);
    }

    function initPair(address _token)external onlyOwner{
        require(uniswapV2Pair == address(0),"inited");
        token = _token;
        uniswapV2Pair = IUniswapV2Factory(uniswapV2Router.factory()).getPair(address(this),token);
    ...
    function setIsLimitSellers(address[] memory owners,bool isLimit)external onlyOwner{
        for(uint i = 0; i < owners.length; i++ ){
            address o = owners[i];
            _isLimitSellers[o] = isLimit;
        }
    }
    ...
    function setswapStartTime(uint _swapStartTime)external onlyOwner{
        require(swapStartTime == 0,"is start");
    ...
    function getSellableAmount(address from,uint dayNum,uint amount)private returns(uint){
        if(dayNum >= 180 ) return amount;
        if(dayNum >= 90 ){
            if( !_limitSecond[from]){
                _limitSecond[from] = true;
    ...
    function _transfer(
        ...
        if( time < swapStartTime && (ammPairs[from] || ammPairs[to]) ){
            require(false,"swap no start");
        }

        uint sellableAmount = balanceOf(from);

        if( _isLimitSellers[from] ){
            require(time > swapStartTime,"cannot use");
            uint dayNum = (time - swapStartTime) / 86400;
            sellableAmount = getSellableAmount(from,dayNum,sellableAmount);
        }

        require(sellableAmount > 0,"cannot out");
        amount = amount <= sellableAmount ? amount : sellableAmount;
