...
contract AirdropSubscription is Ownable {
  ...
  function payoutSKP() external {
    ...
    skpToken.transfer(A, (skpTokenAmount*67000000000000000000)/100000000000000000000);
    skpToken.transfer(B, (skpTokenAmount*16500000000000000000)/100000000000000000000);
    skpToken.transfer(C, (skpTokenAmount*16500000000000000000)/100000000000000000000);
  }

  function transferOwnership(address _A) public override {
    require(msg.sender == A);
    super.transferOwnership(_A);
    A = payable(_A);
  }

  function updateBAddress(address _B) external {
    require(msg.sender == B);
    B = payable(_B);
  }

  function updateCAddress(address _C) external {
