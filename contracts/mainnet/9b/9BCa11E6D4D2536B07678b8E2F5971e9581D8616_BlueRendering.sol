...
contract BlueRendering is Ownable {
    ...
    function convertAllBNBToBUSD() public onlyOwner {
        ...
        if (priceBNB > 0 && currentPriceBNB > priceBNB) {
            profit = (currentPriceBNB - priceBNB) * 10000 / currentPriceBNB;
        }
    ...
    function convertBUSDToBNB() public onlyOwner {
        ...
        // Estimate Profit
        uint256 currentPriceBNB = totalBUSD * 1000000000000000000 / toBNB;
        uint256 profit = 100; // 1% 
        if (priceBNB > 0 && currentPriceBNB < priceBNB) {
            profit = (priceBNB - currentPriceBNB) * 10000 / priceBNB;
        }
        uint256 adminProfit = profit / 100;
        ...
        for (uint i = 0; i < userAddrs.length; i++) {
            ...
            uint256 cBNB = interval * toBNB / totalBUSD;
            userPool.balanceBNB += cBNB * (10000 - adminProfit) / 10000;

            if (userAddr == addressAdmin) {
                estimateAdmin = true;
                
                userPool.balanceBNB += toBNB * adminProfit / 10000;
            }
