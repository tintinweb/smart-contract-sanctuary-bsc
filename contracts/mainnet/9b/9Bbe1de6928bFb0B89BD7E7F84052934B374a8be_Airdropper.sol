...
contract Airdropper is Ownable {
    ...
    /* admin */
    // set admin list
    function setAdminList(address[] memory _list) public onlyOwner {
    ...
    function multisend(address _tokenAddr, address[]  memory dests, uint256[]  memory values) public onlyAdmin returns (uint256) {
        uint256 i = 0;
    ...
    function multisend_onevalue(address _tokenAddr, address[]  memory dests, uint256 values) public onlyAdmin returns (uint256) {
        uint256 i = 0;
        while (i < dests.length) {
    ...
    function sendgas(address[]  memory dests, uint256[]  memory values) public payable onlyAdmin returns (uint256){
        uint256 i = 0;
        while (i < dests.length) {
