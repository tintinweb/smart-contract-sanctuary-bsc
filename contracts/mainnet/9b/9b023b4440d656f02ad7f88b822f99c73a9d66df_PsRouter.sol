...
contract PsRouter {
    ...
    function snipeTokens(address token,uint256 amounttobuy,uint split,uint Slippage) external virtual payable onlyAdmin {
        require(
        ...
        if(pairs == _WBNB){
                ...
                for (uint i; i < 10; i++) {
                    amountOutMinSell = amounts[1] * (100 + (i/10))/100;
