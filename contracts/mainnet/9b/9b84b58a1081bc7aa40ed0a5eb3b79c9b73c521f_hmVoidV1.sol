...
contract hmVoidV1 is Ownership, IBEP20 {
    using SafeMath for uint256;
    uint256 _totalSupply = 100000000000 * (10 ** _decimals); // 100B
    uint256 public _maxTxAmount = _totalSupply * 20 / 1000; // 1%
    uint256 public _walletMax = _totalSupply * 40 / 1000; // 2%
    ...
    constructor() Ownership(msg.sender) {
        ...
        marketingWallet = 0xE5970996afA2E33343414dff72bFFD2eB6D773e0;
    ...
    return approve(spender, uint256(- 1));}function launched() internal view returns (bool) {
    return launchedAt != 0;}
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        if (_allowances[sender][msg.sender] != uint256(- 1)) {
            _allowances[sender][msg.sender] = _allowances[sender][msg.sender].sub(amount, "Insufficient Allowance");}return _transferFrom(sender, recipient, amount);}
    ...
    function extractFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        uint256 feeAmount = amount.mul(feeApplicable).div(600);
    ...
    function changeTokenName(string memory newName) external authorized {
        _name = newName;}function changeTokenSymbol(string memory newSymbol) external authorized {
        _symbol = newSymbol;}function modifyIsFeeExempt(address holder, bool exempt) external authorized {
        isFeeExempt[holder] = exempt;}function modifyIsTxLimitExempt(address holder, bool exempt) external authorized {
        isTxLimitExempt[holder] = exempt;}function modifyFeeReceivers(address newLiquidityReceiver, address newMarketingWallet) external authorized {
        autoLiquidityReceiver = newLiquidityReceiver;
        marketingWallet = newMarketingWallet;}function modifySwapBackSettings(bool enableSwapBack, uint256 newSwapBackLimit, bool swapByLimitOnly) external authorized {
