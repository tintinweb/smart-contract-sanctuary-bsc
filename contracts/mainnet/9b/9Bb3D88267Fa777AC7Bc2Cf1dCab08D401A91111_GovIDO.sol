...
contract GovIDO is Ownable, IKBKGovIDO {
    using SafeERC20 for IERC20;

    uint internal constant PriSaleLimit = 1 * 1e8 * 1e18;
    ...
    uint internal constant Day15 = 15 * Day1;// 15 day
    
    uint internal constant PriceFactor = 1e9;//decimal is 9
    uint internal constant StartPriSalePrice = 1e9 / 100;//decimal is 9
    ...
    function getPriSalePrice() public view returns (uint) {
        if (block.timestamp < priSaleStartTime) {
            return 0;
        } else if (block.timestamp > priSaleStartTime && block.timestamp <= priSaleStartTime + Day15) {
            return StartPriSalePrice;
        } else if (block.timestamp > priSaleStartTime + Day15 && block.timestamp <= priSaleStartTime + Day15 * 2) {
            return StartPriSalePrice * 100 / 90;
        } else {
            return StartPriSalePrice * 100 / 80;
        }
    }
    ...
    function releasePriSale(address user) public override {
        ...
        uint priSalePrice = priSoldInfo[user].privateSalePrice;
        if (priSalePrice != 0 && price >= 2 * priSalePrice) {
