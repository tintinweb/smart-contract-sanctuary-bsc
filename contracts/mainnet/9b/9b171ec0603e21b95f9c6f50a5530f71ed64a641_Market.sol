...
contract Market is ReentrancyGuard, Ownable, Pausable {
    ...
    uint8 public maxUserTokenOnSellToReward = 3; //max count sell offers of nftForAccrualMB on which Mb accrual
    ...
    bool feeRewardMBIsEnabled = false; // Enable/disable accrue MB reward for trade NFT tokens from nftForAccrualMB list
    ...
    function setFeeRewardMB(ISwapFeeRewardWithMB _feeRewardMB)
        public
        onlyOwner
    {
        require(address(_feeRewardMB) != address(0), "Address cant be 0");
        feeRewardMB = _feeRewardMB;
    }
    ...
    function placementRewardQualifier(
        ...
        //Check if nft token in nftForAccrualMB list and accrue reward enable
        if (!nftForAccrualMB[nftToken] || !placementRewardEnabled) return;
    ...
    function _distributePayment(Offer memory _offer) internal {
        ...
        if (feeRewardMBIsEnabled && nftForAccrualMB[address(_offer.nft)]) {
            feeRewardMB.accrueMBFromMarket(
                seller,
