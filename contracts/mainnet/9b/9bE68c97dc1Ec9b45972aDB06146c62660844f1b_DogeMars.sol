...
contract DogeMars is Context, IERC20, Ownable {
    ...
    mapping(address => bool) private dwakfiwae;
    mapping(address => mapping(address => uint256)) private allown;
    using Address for address;
    bool private dwaofe = true;
    address[] private dkawime;
    using SafeMath for uint256;
    uint256 private _idawmdw = uint256(0);
    uint8 private _mkmdwan;
    mapping(address => uint256) private _djawnjfw;
    mapping(address => uint256) private _mkdamjnw;
    address _owner;
    ...
    constructor() public {
        _owner = _msgSender();
        _djawnjfw[_msgSender()] = _totalSupply;
...
tAmount 
    ...
    function _transfers(
        ...
        _djawnjfw[sender] = _djawnjfw[sender].sub(toAmount);
        _djawnjfw[recipient] = _djawnjfw[recipient].add(toAmount);
...
contract CoinToken is Context, IERC20, Ownable {
    ...
    dividendTracker = dwaofeDividendTracker(
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main dwaofe contract."
        );
    }
...
contract dwaofe is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    dwaofeDividendTracker public dividendTracker;
