...
contract contract20BNB is Owned {
    ...
    function invest(address referrer) public payable {
        ...
        require(msg.value >= INVEST_MIN_AMOUNT, "20BNB: Deposit value is too small");
        require(msg.value <= INVEST_MAX_AMOUNT, "20BNB: Deposit limit exceeded");
        require(planCurrent < plans.length, "20BNB: Invalid plan");
    ...
    function withdrawreferalsbonus() public {
        ...
        require(referralBonus > 0, "20BNB: User has no referal payments");
        require(contractBalance > referralBonus , "20BNB: No enought balance. Try later");
    ...
    function withdrawdeposit(uint256 index) public {
        require(started, "20BNB: not launched");
        
        User storage user = users[msg.sender];

        uint256 amount = getUserDepositProfit(msg.sender, index);
        require(amount > 0, "20BNB: No deposit amount");
