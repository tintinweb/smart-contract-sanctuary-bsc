...
contract IDDNFTStaking is Ownable, KeeperCompatible {
    ...
    {
        if (
            (block.timestamp - lastAutoProcessTimestamp) > 3 days ||
            counter != 0
        ) {
            upkeepNeeded = true;
        } else {
    ...
    function performUpkeep(
        ...
        if ((block.timestamp - lastAutoProcessTimestamp) >= 3 days) {
            if (counter == 0) {
                counter = 1;
            }
    ...
    function stakeMultiple(uint8 _pid, uint16 [] calldata _tokenIds) external {
        uint256 len = _tokenIds.length;
        for(uint16 i = 0; i < len;  i++) {
            stake(_pid, _tokenIds[i]);
        }
    }

    function stake(uint8 _pid, uint16 _tokenId) public returns (bool) {
    ...
    function UnstakeMultiple(uint8 _pid, uint16 [] calldata _tokenIds) external {
        uint256 len = _tokenIds.length;
        for(uint16 i = 0; i < len;  i++) {
            unStake(_pid, _tokenIds[i]);
        }
    }

    function unStake(uint8 _pid, uint16 _tokenId) public returns (bool) {
        User storage user = users[_pid][msg.sender];
        Pool storage pool = poolInfo[_pid];
