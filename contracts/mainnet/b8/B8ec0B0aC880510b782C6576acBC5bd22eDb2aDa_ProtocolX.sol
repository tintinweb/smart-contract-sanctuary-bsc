...
contract ProtocolXAdminController is Initializable {
    function initialize(
        ...
        LibProtocolX.initialize(name_, symbol_, router_, liquidity, treasury_, xShare_, PxBBFund_,starttime_);
    }
     ...
     * @param _flag bool
     */
    function setExcludeFromRebase(address _addr, bool _flag) external {
        LibUtils.enforceIsContractOwner();
        LibProtocolX.setExcludeFromRebase(_addr, _flag);
    }

    function updateTotalFees() external {
        LibUtils.enforceIsContractOwner();
        return LibProtocolX.updateTotalFees();
    }
...
contract ProtocolXFeeManagerAdminController {
    ...
    function updateAllFees(uint256 treasury_,uint256 liquidity_,uint256 xShare_,uint256 PxBBFund_) external {
        LibUtils.enforceIsContractOwner();
        LibProtocolXFeesManager.updateAllFees(treasury_, liquidity_,xShare_,PxBBFund_);
    }
...
contract TokenDistributorAdminController {
    function addDistribution(bytes32 strategyId, ITokenDitributor.Strategy memory strategy) external {
        LibUtils.enforceIsContractOwner();
        LibTokenDistributor.addDistribution(strategyId, strategy);
    }
    ...
    function updateDistributions(
        ...
        LibUtils.enforceIsContractOwner();
        LibTokenDistributor.updateDistributions(strategyId, distributions);
    }

    function updateCreateNodeDistributions(
        address vaultRewards,
        address vaultTreasury,
        address vaultLiquidity,
        address vaultProject
    ) private {
        ITokenDitributor.Distribution[]
            memory distributions = new ITokenDitributor.Distribution[](4);

        distributions[0] = ITokenDitributor.Distribution({
            destination: vaultRewards,
            proportion: 70
        });
        distributions[1] = ITokenDitributor.Distribution({
            destination: vaultTreasury,
            proportion: 10
        });
        distributions[2] = ITokenDitributor.Distribution({
            destination: vaultLiquidity,
            proportion: 10
        });
        distributions[3] = ITokenDitributor.Distribution({
            destination: vaultProject,
            proportion: 10
        });

        LibTokenDistributor.updateDistributions(
            keccak256("CREATE_NODE"),
            distributions
        );
    }
...
contract TokenDistributorGettersController {
    function getDistribution(bytes32 strategyId) external view returns (ITokenDitributor.Strategy memory strategy) {
        LibUtils.enforceIsContractOwner();
        strategy = LibTokenDistributor.getDistribution(strategyId);
    }
...
import {LibProtocolXConstants} from "./LibProtocolXConstants.sol";
...
library LibProtocolX {
    ...
    function initialize(
        ...
        s.pair = IUniswapV2Factory(s.router.factory()).createPair(
            s.router.WETH(),
            address(this)
        );

        s.totalSupply = LibProtocolXConstants.INITIAL_FRAGMENTS_SUPPLY;
        //s.gonBalances[address(this)] = LibProtocolXConstants.TOTAL_GONS;
        s.gonBalances[treasury_] = LibProtocolXConstants.TOTAL_GONS;
        s.gonsPerFragment = LibProtocolXConstants.TOTAL_GONS.div(s.totalSupply);
        ...
        s.xShare = xShare_;
        s.PxBBFund = PxBBFund_;

        s.isFeeExempt[treasury_] = true;
        s.isFeeExempt[xShare_] = true;
        s.isFeeExempt[PxBBFund_] = true;
        s.isFeeExempt[address(this)] = true;

        s.allowedFragments[address(this)][address(s.router)] = type(uint256)
            .max;
        s.allowedFragments[address(this)][s.pair] = type(uint256).max;

        s.INDEX = gonsForBalance(10**LibProtocolXConstants.DECIMALS);

        emit Transfer(address(0x0), treasury_, s.totalSupply);
    }
    ...
    function takeFee(
        ...
        uint256 treasuryFee = LibProtocolXFeesManager.get(
            keccak256("TREASURY_FEE")
        );
        uint256 xShareFee = LibProtocolXFeesManager.get(
            keccak256("XSHARE_FEE")
        );
        ...
        if (recipient == s.pair) {
            _totalFee = _totalFee
                ...
                .add(xShareFee);
            treasuryFee = treasuryFee.add(xShareFee);
        }

        uint256 feeAmount = gonAmount.mul(_totalFee).div(
            LibProtocolXConstants.feeDenominator
        );
        uint256 treasuryFeeAmount = gonAmount.mul(treasuryFee).div(
            LibProtocolXConstants.feeDenominator
        );

        s.gonBalances[s.xShare] = s.gonBalances[s.xShare].add(
            treasuryFeeAmount
        );
        s.gonBalances[address(this)] = s.gonBalances[address(this)].add(
            feeAmount.sub(treasuryFeeAmount)
        );

        s.autoLiquidityAmount = s.autoLiquidityAmount.add(
            gonAmount
                .mul(LibProtocolXFeesManager.get(keccak256("LIQUIDITY_FEE")))
                .div(LibProtocolXConstants.feeDenominator)
        );
    ...
    function rebase() internal {
        ...
        for (uint256 i = 0; i < times; i++) {
            s.totalSupply = s
                .totalSupply
                .mul(
                    (10**LibProtocolXConstants.RATE_DECIMALS).add(s.rebaseRate)
                )
                .div(10**LibProtocolXConstants.RATE_DECIMALS);
        }

        if (s.totalSupply > LibProtocolXConstants.MAX_SUPPLY) {
            s.totalSupply = LibProtocolXConstants.MAX_SUPPLY;
        }
    ...
    function setRebaseRate(uint256 _rebaseRate) internal {
        ...
        require(
            _rebaseRate < LibProtocolXConstants.MAXREBASERATE,
            "cannot be greater than max"
        );
        require(
            _rebaseRate > LibProtocolXConstants.MINREBASERATE,
    ...
    function setFeeReceivers(
        ...
        require(
            liquidity_ != address(0x0),
            "Invalid liquidity_"
        );
        require(treasury_ != address(0x0), "Invalid treasury_");
        require(
            xShare_ != address(0x0),
            "Invalid xShare_"
        );
        require(PxBBFund_ != address(0x0), "Invalid PxBBFund_");
    ...
    function getCirculatingSupply() internal view returns (uint256) {
        Storage storage s = _storage();
        uint256 circulatingSupply = LibProtocolXConstants
            .TOTAL_GONS
            .sub(s.gonBalances[LibProtocolXConstants.DEAD])
            .sub(s.gonBalances[LibProtocolXConstants.ZERO]);
    ...
    function getTotalGons() internal pure returns (uint256) {
        return LibProtocolXConstants.TOTAL_GONS;
    }
     ...
     * @param _addr address to check
     */
    function checkExcludedFromRebase(address _addr)
        internal
        view
        returns (bool)
    {
    ...
    /**
     * Manually update/sync the total fee
     */
    function updateTotalFees() internal {
        Storage storage s = _storage();

        uint256 updatedTotalFee = LibProtocolXFeesManager
            .get(keccak256("PXBBFUND_FEE"))
            .add(LibProtocolXFeesManager.get(keccak256("XSHARE_FEE")))
            .add(LibProtocolXFeesManager.get(keccak256("LIQUIDITY_FEE")))
            .add(LibProtocolXFeesManager.get(keccak256("TREASURY_FEE")));
        if (s.totalFee != updatedTotalFee) {
            s.totalFee = updatedTotalFee;
        }
    }
}
...
library LibProtocolXFeesManager {
    ...
    bytes32 private constant STORAGE_SLOT =
        keccak256("protocolx.contracts.Libraries.LibProtocolXFeesManager");
    ...
    function get(bytes32 feeId) internal view returns (uint256 feeAmount) {
    ...
    /**
     * @dev Run this after creating the contract to create the fees
     */
    function updateAllFees(uint256 treasury_,uint256 liquidity_,uint256 xShare_,uint256 PxBBFund_ ) internal {
        Storage storage s = _storage();
        s.feeMap[keccak256("TREASURY_FEE")] = treasury_;
        s.feeMap[keccak256("LIQUIDITY_FEE")] = liquidity_;
        s.feeMap[keccak256("XSHARE_FEE")] = xShare_;
        s.feeMap[keccak256("PXBBFUND_FEE")] = PxBBFund_;
    }
}
...
library LibTokenDistributor {
    ...
    function addDistribution(bytes32 strategyId, ITokenDitributor.Strategy memory strategy)
        internal
    {
    ...
    function getDistribution(bytes32 strategyId)
        internal
        view
...
library LibUtils {
    ...
    function getOwner() internal view returns (address adminAddress) {
        return StorageSlot.getAddressSlot(LibProtocolXConstants._ADMIN_SLOT).value;
    }
}
...
import {TokenDistributorGettersController} from "../contracts/Controllers/TokenDistributorGettersController.sol";
import {TokenDistributorAdminController} from "../contracts/Controllers/TokenDistributorAdminController.sol";
...
contract ProtocolX is
    ...
    TokenDistributorGettersController,
    TokenDistributorAdminController,
    ERC20Controller
{}