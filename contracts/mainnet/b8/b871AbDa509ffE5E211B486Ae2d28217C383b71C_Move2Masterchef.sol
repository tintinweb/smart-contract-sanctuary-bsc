...
contract Move2Masterchef is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct PoolInfo {       
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. Move2Farmes to distribute per block.
        uint256 lastRewardBlock;  // Last block number that Move2Farmes distribution occurs.
        uint256 accMove2FarmPerShare;   // Accumulated Move2Farmes per share, times 1e18. See below.
    ...
    // The Move2Farm TOKEN!
    IERC20 public Move2Farm;
    address public feeAddress;

    // Move2Farm tokens created per block.
    uint256 public Move2FarmPerBlock = 2 ether;
    ...
    // The block number when Move2Farm mining starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 Move2FarmPerBlock);
    ...
    // Update the given pool's Move2Farm allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending Move2Farmes on frontend.
    function pendingMove2Farm(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accMove2FarmPerShare = pool.accMove2FarmPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 Move2FarmReward = multiplier.mul(Move2FarmPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accMove2FarmPerShare = accMove2FarmPerShare.add(Move2FarmReward.mul(1e18).div(lpSupply));
        }
        return user.amount.mul(accMove2FarmPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 Move2FarmReward = multiplier.mul(Move2FarmPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accMove2FarmPerShare = pool.accMove2FarmPerShare.add(Move2FarmReward.mul(1e18).div(lpSupply));
    ...
    // Deposit LP tokens to MasterChef for Move2Farm allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accMove2FarmPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
                safeMove2FarmTransfer(msg.sender, pending);
            }
        ...
        user.rewardDebt = user.amount.mul(pool.accMove2FarmPerShare).div(1e18);
    ...
    // Safe Move2Farm transfer function, just in case if rounding error causes pool to not have enough .
    function safeMove2FarmTransfer(address _to, uint256 _amount) internal {
        uint256 Move2FarmBal = Move2Farm.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > Move2FarmBal) {
            transferSuccess = Move2Farm.transfer(_to, Move2FarmBal);
        } else {
            transferSuccess = Move2Farm.transfer(_to, _amount);
        }
        require(transferSuccess, "safeMove2FarmTransfer: Transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _Move2FarmPerBlock) external onlyOwner {
        massUpdatePools();
        Move2FarmPerBlock = _Move2FarmPerBlock;
        emit UpdateEmissionRate(msg.sender, _Move2FarmPerBlock);
    }
