...
import "./IVCelebrity.sol";

contract CelebrityPool is Ownable, Pausable {
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint256 shares; // number of shares for a user.
        uint256 lastDepositedTime; // keep track of deposited time for potential penalty.
        uint256 celebrityAtLastUserAction; // keep track of celebrity deposited at the last user action.
    ...
    uint256 public celebrityPoolPID;
    ...
    event NewVCelebrityContract(address VCelebrity);
    ...
    /**
     * @notice Constructor
     * @param _token: Celebrity token contract
     * @param _masterchefV2: MasterChefV2 contract
     * @param _admin: address of the admin
     * @param _treasury: address of the treasury (collects fees)
     * @param _operator: address of operator
     * @param _pid: celebrity pool ID in MasterChefV2
     */
    constructor(
        IERC20 _token,
    ...
    function init(IERC20 dummyToken) external onlyOwner {
        ...
        masterchefV2.deposit(celebrityPoolPID, balance);
        emit Init();
    }
    ...
    /**
     * @notice Checks if the msg.sender is either the celebrity owner address or the operator address.
     */
    modifier onlyOperatorOrCelebrityOwner(address _user) {
        require(msg.sender == _user || msg.sender == operator, "Not operator or celebrity owner");
        _;
    }
    ...
    function updateBoostContractInfo(address _user) internal {
        if (boostContract != address(0)) {
            ...
            IBoostContract(boostContract).onCelebrityPoolUpdate(
                _user,
                user.lockedAmount,
                lockDuration,
                totalLockedAmount,
    ...
    function updateUserShare(address _user) internal {
        ...
        if (user.shares > 0) {
            if (user.locked) {
                ...
                totalShares -= user.shares;
                user.shares = 0;
                uint256 earnAmount = totalAmount - user.celebrityAtLastUserAction;
    ...
    /**
     * @notice Unlock user celebrity funds.
     * @dev Only possible when contract not paused.
     * @param _user: User address
     */
    function unlock(address _user) external onlyOperatorOrCelebrityOwner(_user) whenNotPaused {
        UserInfo storage user = userInfo[_user];
    ...
    /**
     * @notice Deposit funds into the Celebrity Pool.
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in CELEBRITY)
    ...
    function depositOperation(
        ...
        if (VCelebrity != address(0)) {
            IVCelebrity(VCelebrity).deposit(_user, _amount, _lockDuration);
        }
        ...
        user.celebrityAtLastUserAction = (user.shares * balanceOf()) / totalShares - user.userBoostedShare;
    ...
    /**
     * @notice Withdraw funds from the Celebrity Pool.
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (VCelebrity != address(0)) {
            IVCelebrity(VCelebrity).withdraw(msg.sender);
        }
        ...
        if (user.shares > 0) {
            user.celebrityAtLastUserAction = (user.shares * balanceOf()) / totalShares;
        } else {
            user.celebrityAtLastUserAction = 0;
        }
    ...
    /**
     * @notice Harvest pending CELEBRITY tokens from MasterCelebrity
     */
    function harvest() internal {
        uint256 pendingCelebrity = masterchefV2.pendingCelebrity(celebrityPoolPID, address(this));
        if (pendingCelebrity > 0) {
            uint256 balBefore = available();
            masterchefV2.withdraw(celebrityPoolPID, 0);
    ...
    /**
     * @notice Set VCelebrity Contract address
     * @dev Callable by the contract admin.
     */
    function setVCelebrityContract(address _VCelebrity) external onlyAdmin {
        require(_VCelebrity != address(0), "Cannot be zero address");
        VCelebrity = _VCelebrity;
        emit NewVCelebrityContract(VCelebrity);
    }
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Celebrity Pool
     */
    ...
    function calculatePerformanceFee(address _user) public view returns (uint256) {
        ...
        if (user.shares > 0 && !user.locked && !freePerformanceFeeUsers[_user]) {
            uint256 pool = balanceOf() + calculateTotalPendingCelebrityRewards();
     ...
     * @return Returns total pending celebrity rewards
     */
    function calculateTotalPendingCelebrityRewards() public view returns (uint256) {
        uint256 amount = masterchefV2.pendingCelebrity(celebrityPoolPID, address(this));
        return amount;
    }

    function getPricePerFullShare() external view returns (uint256) {
        return totalShares == 0 ? 1e18 : (((balanceOf() + calculateTotalPendingCelebrityRewards()) * (1e18)) / totalShares);
    }
