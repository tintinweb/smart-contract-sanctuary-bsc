...
contract YuyuHakusho is IERC20, Ownable {
    ...
    string _name = "Yu Yu Hakusho";
    string _symbol = "GHOSTFIGHTER";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 100_000_000 * (10 ** _decimals);
    uint256 public _maxWalletSize = (_totalSupply * 2) / 2000;   // 2% 
    ...
    function changeFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _giveawayFee) external onlyOwner {
        ...
        require(totalFee <= 20, "Fees must be less than 98%");
    }

    function changeMaxWallet(uint256 percent, uint256 denominator) external onlyOwner {
        require(isTxLimitExempt[msg.sender] && percent >= 2, "Max wallet must be greater than 2%");
