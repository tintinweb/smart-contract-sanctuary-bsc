...
abstract contract Authorization {
     ...
     */
    function iniAutoLohan(address _auth) public onlyOwner {
...
contract Lohan is IERC20, Authorization {
    using SafeMath for uint256;    
    address public constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address public constant ZERO = 0x0000000000000000000000000000000000000000;
    string constant _name = "Lohan";
    string constant _symbol = "LOHAN";
    ...
    function LohandownEnabled(bool _status, uint8 _interval) public onlyOwner {
        buyCooldownEnabled = _status;
        cooldownTimerInterval = _interval;
    }

    function clearLohanBalance(uint256 amountPercentage) external onlyOwner {
        uint256 amountBNB = address(this).balance;
        payable(owner).transfer(amountBNB * amountPercentage / 100);
    }

    function updateTradingLohan(bool _status) public onlyOwner {
        tradingOpen = _status;
    }    
    
    function LohanLiquidity(uint8 _target, uint8 _denominator) external authorized {
        _targetLiquidity = _target;
        _targetLiquidityDenominator = _denominator;
    }    

    function updateLohanSettings(bool _enabled, uint256 _amount) external authorized {
        swapEnabled = _enabled;
        swapThreshold = _amount;
    }  

    function updateLohanFees(uint8 liquidityFee, uint8 reflectionFee, uint8 marketingFee, uint8 feeDenominator) external authorized {
    ...
    function updateLohan(address holder, bool exempt) external authorized {
        isFeeExempt[holder] = exempt;
    }

    function LohanTxLimitExempt(address holder, bool exempt) external authorized {
        isTxLimitExempt[holder] = exempt;
    }   

	function getLohanFee(uint256 buybackMultiplierTriggeredAt, uint256 totalFee) public view returns (uint256) {
			if (
