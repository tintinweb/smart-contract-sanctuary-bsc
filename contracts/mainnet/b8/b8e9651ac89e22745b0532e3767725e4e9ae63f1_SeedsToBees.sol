...
contract SeedsToBees is Context, IERC20, Ownable {
    ...
    string private _name = "SeedsToBees";
    string private _symbol = "SEEDS";
    uint8 private _decimals = 9;
    uint private _totalSupply = 1_000_000_000;
    ...
    uint256 public _growthFee = 200; // All taxes are divided by 100 for more accuracy.
    uint256 private _previousGrowthFee = _growthFee;
    uint256 public _boostedGrowthFee = 400;

    uint256 public _buyBackFee = 0;
    uint256 private _previousBuyBackFee = _buyBackFee;
    uint256 public _boostedBuyBackFee = 0;

    uint256 public _seedFee = 200;   
    uint256 private _previousSeedFee = _seedFee;
    uint256 public _boostedSeedFee = 200;
    ...
    address payable private _growthWallet = payable(0xBD7Ec6a640314B0aABe489DbdD2b0B8D9429ed76);
    address payable private _seedWallet = payable(0xB1514a7F273f55ccaF3794221BA4fAf0dBbefF0c);
    address payable private _liquidityWallet = payable(0xD7410529E2FC3e8Ac0615B68CFf9be6F09e2Eec2);
    ...
    uint256 public maxTxAmountUI = (_totalSupply * maxTxPercent) / maxTxDivisor; // Actual amount for UI's
    
     // Max TX Wallet is 2% of the total supply.
    ...
    uint256 public maxWalletAmountUI = (_totalSupply * maxWalletPercent) / maxWalletDivisor; // Actual amount for UI's
    ...
    function addExemptFromLimits(address wallet) public onlyOwner() {
        _exemptFromLimits[wallet] = true;
    }

    function removeExemptFromLimits(address wallet) public onlyOwner() {
        _exemptFromLimits[wallet] = false;
    }
    ...
    function setTaxes(uint256 reflectFee, uint256 liquidityFee, uint256 growthFee, uint256 buyBackFee, uint256 seedFee) external onlyOwner() {
        require(reflectFee + liquidityFee + growthFee + buyBackFee + seedFee <= maximumTaxesPercent);
    ...
    function setBoostedTaxes(uint256 reflectFee, uint256 liquidityFee, uint256 growthFee, uint256 buyBackFee, uint256 seedFee) external onlyOwner() {
        require(reflectFee + liquidityFee + growthFee + buyBackFee + seedFee <= maximumTaxesPercent);
        _boostedReflectFee = reflectFee;
        _boostedLiquidityFee = liquidityFee;
        _boostedGrowthFee = growthFee;
        _boostedBuyBackFee = buyBackFee;
        _boostedSeedFee = seedFee;
    }

    function setMaxTxPercent(uint256 percent, uint256 divisor) external onlyOwner() {
        require(divisor <= 10000); // Cannot set lower than 0.01%
        _maxTxAmount = _tTotal.mul(percent).div(divisor);
        maxTxAmountUI = _totalSupply.mul(percent).div(divisor);
    }
    ...
    function setMaxWallet(uint256 percent, uint256 divisor) external onlyOwner() {
        ...
        maxWalletAmountUI = _totalSupply.mul(percent).div(divisor);
    }
    ...
    function setgrowthWallet(address payable newWallet) external onlyOwner {
        require(_growthWallet != newWallet, "Wallet already set!");
        _growthWallet = payable(newWallet);
    }

    function setseedWallet(address payable newWallet) external onlyOwner {
        require(_seedWallet != newWallet, "Wallet already set!");
        _seedWallet = payable(newWallet);
    }
    ...
    function setliquidityWallet(address payable newWallet) external onlyOwner {
        ...
        _liquidityWallet = payable(newWallet);
        _isExcludedFromFee[_liquidityWallet] = true;
    }
    ...
    function setBuyBackSellLimit(uint256 limit) external onlyOwner() {
        buyBackSellLimit = limit * 10**_decimals;
    }
    ...
    function _hasLimits(address from, address to) private view returns (bool) {
        return from != owner()
            && to != owner()
            && !_exemptFromLimits[to]
            && !_exemptFromLimits[from]
            && to != burnAddress
            && to != address(0);
    }
    ...
    function deliver(uint256 tAmount) public {
        ...
        require(!_isExcluded[sender], "Exc address cannot call this function");
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(_hasLimits(from, to)
            ...
            require(contractBalanceRecepient + amount <= _maxWalletAmount, "xfer amount exceeds the maxWallet");
            }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 totalBNBFees = _growthFee.add(_liquidityFee).add(_buyBackFee).add(_seedFee);
        if (totalBNBFees == 0)
            return;
        uint256 toGrowth = contractTokenBalance.mul(_growthFee).div(totalBNBFees);
        uint256 toBuyBack = contractTokenBalance.mul(_buyBackFee).div(totalBNBFees);
        uint256 toSeed = contractTokenBalance.mul(_seedFee).div(totalBNBFees);
        uint256 toLiquify = contractTokenBalance.sub(toGrowth).sub(toBuyBack).sub(toSeed);
        ...
        // swap tokens for ETH
        uint256 toSwapForEth = half.add(toGrowth).add(toBuyBack).add(toSeed);
        ...
        uint256 seedBalance = fromSwap.mul(toSeed).div(toSwapForEth);
        uint256 growthBalance = fromSwap.sub(liquidityBalance.add(seedBalance).add(buyBackBalance));
        ...
        transferEthOut(seedBalance, growthBalance);
    }

    function transferEthOut(uint256 seedBalance, uint256 growthBalance) internal {
        _seedWallet.transfer(seedBalance);
        _growthWallet.transfer(growthBalance);
    }
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee.add(_buyBackFee).add(_growthFee).add(_seedFee)).div(masterTaxDivisor);
    }
    ...
    function boostSellTaxes() internal {
        ...
        _growthFee = _boostedGrowthFee;
        _seedFee = _boostedSeedFee;
    }
}