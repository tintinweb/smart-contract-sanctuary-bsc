...
contract LotteryNFT is ERC721, Ownable {
    ...
    constructor() public ERC721("Meow Lottery Ticket", "MLT") {}
...
contract Lottery is Ownable {
    ...
    function buy(uint256 _price, uint8[4] memory _numbers) external inDrawingPhase {
        ...
        meow.safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function multiBuy(uint256 _price, uint8[4][] memory _numbers) external inDrawingPhase {
        ...
        meow.safeTransferFrom(address(msg.sender), address(this), totalPrice);
    ...
    function claimReward(uint256 _tokenId) external {
        ...
        if(reward>0) {
            meow.safeTransfer(address(msg.sender), reward);
        }
    ...
    function multiClaim(uint256[] memory _tickets) external {
        ...
        if(totalReward>0) {
            meow.safeTransfer(address(msg.sender), totalReward);
        }
    ...
    function seedLottery(uint256 _amount) external onlyOwner {
        ...
        meow.safeTransferFrom(address(msg.sender), address(this), _amount);
    }

    function adminWithdraw(uint256 _amount) external onlyOwner {
        meow.safeTransfer(address(msg.sender), _amount);
