...
contract AppleToken is ERC20, Ownable {
    ...
    event SetDev(address indexed _dev);

    constructor(address _routerAddress, address _dev) ERC20("Apple Token", "AT"){
        super._mint(msg.sender, 100000000000 * (10 ** 18));
        dev = _dev;
        ISwapRouter _router = ISwapRouter(_routerAddress);
        weth = _router.WETH();
        swapPairAddress = ISwapFactory(_router.factory()).createPair(address(this), weth);
        emit SetDev(_dev);
    }
    ...
    function _transfer(
        ...
        if (sender != owner() && recipient != owner()) {
            if (swapPairAddress != address(0) && (swapPairAddress == sender || swapPairAddress == recipient)) {
                uint256 _fee = amount.mul(feeRate).div(1000);
                uint256 _burn = _fee.mul(burnRate).div(30);
                super._transfer(sender, address(1), _burn);
                super._transfer(sender, address(this), _fee.sub(_burn));
                amount = amount.sub(_fee);
            }
        }
        if (!inSwap && sender != swapPairAddress) {
            //add liquidity
