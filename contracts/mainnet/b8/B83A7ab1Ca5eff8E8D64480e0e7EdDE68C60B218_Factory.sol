...
contract Factory is Ownable {
    ...
    address private constant adminAddress = 0xa10593fad651D9e9019220eb9AeFa71a072587AD;

    event PoolCreated(address owner, address rewarderAddress, address fundraiserAddress);

    function createPool(string calldata baseURI) external returns (address fundraiserAddress, address rewarderAddress) {
        ...
        fundraiser.setBaseURI(baseURI);
        fundraiser.transferOwnership(msg.sender);
        fundraiserAddress = address(fundraiser);

        Rewarder rewarder = new Rewarder(adminAddress, fundraiserAddress);
...
interface INonFungible is IERC721 {
...
import "./interfaces/INonFungible.sol";
...
contract Rewarder is ReentrancyGuard, Ownable {
    ...
    IERC20 public token;
    INonFungible public immutable nonFungible;
    ...
    constructor(address _adminAddress, address _nft) {
        require(_adminAddress != address(0), "Already set");
        require(_nft != address(0), "Already set");
        adminAddress = _adminAddress;
        nonFungible = INonFungible(_nft);
    }
    ...
    function pendingReward(address user) public view returns (uint256) {
        ...
        uint256 allocation = nonFungible.balanceOf(user).mul(bank).div(nonFungible.totalSupply());
    ...
    function claim() public nonReentrant {
        ...
        uint256 totalNftSupply = nonFungible.totalSupply();
        uint256 userNftBalance = nonFungible.balanceOf(msg.sender);
...
contract Fundraiser is Ownable, Pausable, ERC721A, ReentrancyGuard {
  using SafeERC20 for IERC20;
  ...
  uint256 standardPrice = 100 * 10 ** 18;
  address private adminAddress;

  constructor(address _adminAddress, address dai, address usdc, address usdt, address busd) ERC721A("Pool shares", "PLSHRS") {
    require(dai != address(0), "Already set");
    require(usdc != address(0), "Already set");
    require(usdt != address(0), "Already set");
    require(busd != address(0), "Already set");
    require(_adminAddress != address(0), "Already set");
    DAI = dai;
    USDC = usdc;
    USDT = usdt;
  ...
  function publicSaleMint(address token, uint256 quantity) external whenNotPaused {
    receiveFunds(token, standardPrice * quantity);
