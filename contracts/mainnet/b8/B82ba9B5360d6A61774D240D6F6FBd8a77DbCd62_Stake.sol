...
contract Stake is Ownable {
    ...
    function viewTotalROI(uint256 _stakingId) public view returns(uint256){
        if (block.timestamp < stakeDetails[_stakingId].unstakeTime) {
            return( ( (block.timestamp - stakeDetails[_stakingId].stakeTime) * stakeDetails[_stakingId].amount  * pool[stakeDetails[_stakingId].poolId].rewardPer / 10000) / 365 days);
        } else {
            return( ( (stakeDetails[_stakingId].unstakeTime - stakeDetails[_stakingId].stakeTime) * stakeDetails[_stakingId].amount * pool[stakeDetails[_stakingId].poolId].rewardPer / 10000) / 365 days);
        }
    }
