...
contract MNBYPool is Ownable, Pausable {
    ...
    modifier onlyOperatorOrCakeOwner(address _user) {
        require(msg.sender == _user || msg.sender == operator, "Not operator or mnby owner");
        _;
    }
    ...
    function harvest() internal {
        uint256 pendingMnby = masterchefV2.pendingMnby(cakePoolPID, address(this));
        if (pendingMnby > 0) {
    ...
    function calculatePerformanceFee(address _user) public view returns (uint256) {
        ...
        if (user.shares > 0 && !user.locked && !freePerformanceFeeUsers[_user]) {
            uint256 pool = balanceOf() + calculateTotalPendingMnbyRewards();
     ...
     */
    function calculateTotalPendingMnbyRewards() public view returns (uint256) {
        uint256 amount = masterchefV2.pendingMnby(cakePoolPID, address(this));
        return amount;
    }

    function getPricePerFullShare() external view returns (uint256) {
        return totalShares == 0 ? 1e18 : (((balanceOf() + calculateTotalPendingMnbyRewards()) * (1e18)) / totalShares);
    }
