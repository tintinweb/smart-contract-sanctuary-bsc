...
contract FOFSynchData is Ownable {

    using SafeMath for uint256;


    event SynchData(address from, uint256 counter);


    FOFPlayerController FPC0;
    ...
    constructor() {
        ...
        FPC0 = FOFPlayerController(payable(0x205A4A94391cdA7Bf47F024317bE5727Cb0ef30c));

        FPC1 = FOFPlayerController1(payable(0x43688F057b9E55Ba881739664BEFaAfc6B566DA8));
    }

    function updateFOFPlayerController(address FPC0Address) public onlyOwner {
        FPC0 = FOFPlayerController(payable(FPC0Address));
    }

    function updateFOFPlayerController1(address FPC1Address) public onlyOwner {
        FPC1 = FOFPlayerController1(payable(FPC1Address));
    }

    function synchComponentsData(uint256 start, uint256 end) public onlyOwner {

        uint256 counter;

        for(uint256 i = start; i < end; i++){
           
            (uint256 id,address holder,uint256 types,uint256 rarity,uint256 speed,uint256 stability,uint256 controllability,uint256 explosive) = FPC0.components(i);

            if(id != 0){
                FPC1.addMyComponent(id, holder, types, rarity, speed, stability, controllability, explosive);
                counter++;
            }
        }

        emit SynchData(msg.sender, counter);
    }

    function synchSettlementTimesToTime(uint256 end) public onlyOwner {

        uint256 counter;

        for(uint256 i = 0; i < end; i++){
            uint256 time = FPC0.settlementTimesToTime(i);

            if(time > 0){
                FPC1.setSettlementTimesToTime(time);
            }
    ...
    function synchSettlementTimesToDividends(uint256 end) public onlyOwner {
        ...
        for(i = 0; i < end; i++){
            ...
            for(j = 1; j < 4; j++){
                uint256 dividend = FPC0.settlementTimesToDividends(i, j);

                FPC1.setSettlementTimesToDividends(i, j, dividend);
    ...
    function querryDataVehicle(uint256 index) public view returns(uint256 id,address holder,uint256 types,uint256 rarity,uint256 speed,uint256 stability,uint256 controllability,uint256 explosive) {
        (id, holder, types, rarity, speed, stability, controllability, explosive) = FPC0.components(index);
    }

    function queryMyComponentIds(address user) public view returns(uint256[] memory){
        return FPC0.queryMyComponentIds(user);
    }
