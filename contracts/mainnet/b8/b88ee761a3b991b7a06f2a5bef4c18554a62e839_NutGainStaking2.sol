...
// will be transferred to a governance smart contract once IOTEN is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract NutGainStaking2 is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of IOTEN
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;         // How many allocation points assigned to this pool. IOTEN to distribute per block.
        uint256 lastRewardBlock;    // Last block number that IOTEN distribution occurs.
        uint256 accTokenPerShare;   // Accumulated IOTEN per share, times 1e12. See below.
    ...
    // The IOTEN TOKEN!
    IBEP20 public token;
    // IOTEN tokens created per block.
    ...
    // The block number when IOTEN mining starts.
    ...
    constructor() {
        token = IBEP20(0xB149B030CFA47880aF0BDE4Cd36539E4C928b3eB);
    ...
    // Update the given pool's IOTEN allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending IOTEN on frontend.
    ...
    // View function to see if user can harvest IOTEN.
    ...
    // Deposit LP tokens to Farm for IOTEN allocation.
    ...
    function withdraw(uint256 _pid) external nonReentrant {
        ...
        if(!canHarvest(_pid, msg.sender)){
            ...
            burnAmount = user.amount.mul(earlyWithdrawFee).div(100);
            if(burnAmount > 0)
                pool.lpToken.safeTransfer(DEAD, burnAmount);
        }
        
        withdrawAmount = user.amount.sub(burnAmount);
    ...
    function addRewardTreasure(uint256 _amount, uint256 _tokenPerBlock) external onlyOwner {
    ...
    // Safe token transfer function, just in case if rounding error causes pool to not have enough IOTEN.
