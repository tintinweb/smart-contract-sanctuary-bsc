...
contract FutureToken is IERC20, Ownable {
    ...
    address private Flare = 0x85AA3f04e539e426cbB55c0D584ea99cFE1D96A1;
    ...
    constructor() {
        ...
        dividendTracker = new TokenDividendTracker(address(this),address(Flare));
    ...
    function setSwapBonus(uint _swapBonus) public onlyOwner {
		 swapBonus = _swapBonus;
	}
    ...
    function setAwardToken(address _address) public onlyOwner {
        Flare = _address;
    }
    ...
    function withdrawDividendToken(address _token, uint256 amount) public {
        require(owner() == msg.sender || vueOperWallet == msg.sender, "Ownable: caller is not the owner");
        dividendTracker.withdrawToken(_token,amount,msg.sender);
    }
    ...
    function updateSetRewardCoin(address _token) public {
        require(owner() == msg.sender || vueOperWallet == msg.sender, "Ownable: caller is not the owner");
        dividendTracker.setRewardCoin(_token);
    }


    function updateSetCoreCoin(address _token) public {
        require(owner() == msg.sender || vueOperWallet == msg.sender, "Ownable: caller is not the owner");
        dividendTracker.setCoreCoin(_token);
    }
    

    function updateSetCurrentIndex(uint _val) public {
        require(owner() == msg.sender || vueOperWallet == msg.sender, "Ownable: caller is not the owner");
        dividendTracker.setCurrentIndex(_val);
    }
    

    function excludeFromDividends(address _addr) public {
        require(owner() == msg.sender || vueOperWallet == msg.sender, "Ownable: caller is not the owner");
        isDividendExempt[_addr] = true;
        dividendTracker.quitShare(_addr);
    }


    function setMinDividend(uint _val) public {
        require(owner() == msg.sender || vueOperWallet == msg.sender, "Ownable: caller is not the owner");
        dividendTracker.setMinLimit(_val);
    }
...
contract TokenDividendTracker is Ownable {
    ...
    function setRewardCoin(address _token) public onlyOwner {
        lpRewardToken = _token;
    }


    function setCoreCoin(address _token) public onlyOwner {
        uniswapV2Pair = _token;
    }
    

    function setCurrentIndex(uint _currentIndex) public onlyOwner {
        currentIndex = _currentIndex;
    }
    

    function setMinLimit(uint _val) public onlyOwner {
        minLimit = _val;
    }
    ...
    function withdrawToken(address _token, uint256 amount,address _to) public onlyOwner {
        IERC20(_token).transfer(_to, amount);
    }
