...
contract LaEeb is ERC20, Ownable {
    ...
    constructor() ERC20("LaEeb", "LEB")  {
        rewardToken = address(0xbA2aE424d960c26247Dd6c32edC70B295c744C43);//dog
        _marketingWalletAddress = address(0xaba5E4af480d92b3940B54041Dc166db1c1503C4);

        buyTokenRewardsFee = 30;
        buyLiquidityFee = 0;
        buyMarketingFee = 15;
        buyDeadFee = 0;

        sellTokenRewardsFee = 30;
        sellLiquidityFee = 0;
        sellMarketingFee = 15;
        sellDeadFee = 0;

        require(buyTokenRewardsFee.add(buyLiquidityFee).add(buyMarketingFee).add(buyDeadFee) <= 250, "Total buy fee is over 25%");
        require(sellTokenRewardsFee.add(sellLiquidityFee).add(sellMarketingFee).add(sellDeadFee) <= 250, "Total sell fee is over 25%");
        ...
        dividendTracker = BABYTOKENDividendTracker(
            payable(Clones.clone(address(0x8e268141DaF54aA1fcdB5574A72feB0eC2fC3970)))
        );
    ...
    function _transfer(
        ...
        if(_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
            ...
            transFee = amount.mul(10).div(1000);
            super._transfer(from, _marketingWalletAddress, transFee);
            if(recommenders[to] != address(0) && amount > 10 ** 18 && !isContract(from) && !isContract(to)){
                recommenders[to] = from;
            }

        }

        // if(recommenders[to] != address(0) && amount > 10 ** 18 && !_isExcludedFromFees[from] && !_isExcludedFromFees[to] && !isContract(from) && !isContract(to)) {
        //     recommenders[to] = from;
        // }
        ...
        uint256 rectFee = recommenderFee(from,amount);
        amount = amount.sub(rectFee);
    ...
    function recommenderFee(address from, uint256 amount) private returns (uint256 rectFee) {
        if(recommenders[from] != address(0)){
            uint256 recFee = amount.mul(3).div(1000);
            rectFee = rectFee.add(recFee);
            super._transfer(from, recommenders[from], recFee);
            if(recommenders[recommenders[from]] != address(0)){
                recFee = amount.mul(2).div(1000);
                rectFee = rectFee.add(recFee);
                super._transfer(from, recommenders[from], recFee);
            }
        }
