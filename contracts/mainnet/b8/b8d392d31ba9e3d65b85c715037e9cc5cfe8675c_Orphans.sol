...
contract Orphans is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    OrphansDividendTracker public dividendTracker;
    ...
    address payable public _marketingWalletAddress = payable(0xe7659cf53080eFacCA6cB6F63532695d2d7b84cB);
    address payable public _buybackWalletAddress = payable(0xe7659cf53080eFacCA6cB6F63532695d2d7b84cB);
    ...
    constructor() BEP20("Orphans", "Orphans") 
    {
        dividendTracker = new OrphansDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "Orphans: The dividend tracker already has that address");
        OrphansDividendTracker newDividendTracker = OrphansDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "Orphans: The new dividend tracker must be owned by the Orphans token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Orphans: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Orphans: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Orphans: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Orphans: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Orphans: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Orphans: Cannot update gasForProcessing to same value");
...
contract OrphansDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("Orphans_Dividend_Tracker", "Orphans_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "Orphans_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "Orphans_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Orphans contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Orphans_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Orphans_Dividend_Tracker: Cannot update claimWait to same value");
