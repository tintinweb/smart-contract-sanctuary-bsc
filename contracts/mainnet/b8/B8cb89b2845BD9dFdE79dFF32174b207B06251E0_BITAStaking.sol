...
// File contracts/BITAStaking.sol


pragma solidity 0.8.4;
contract BITAStaking is Ownable, ReentrancyGuard {
  using SafeMath for uint256;
  using SafeERC20 for IERC20;

  struct UserInfo {
    uint256 amount;           // How many BITA tokens the user has provided.
  ...
  event TradeWalletSet(address tradeWallet);

  address public bitaToken;
  address private tradeWallet;
  mapping (address => UserInfo) public userInfo;

  uint256 public rewardsDuration = 90 days;
  uint256 public totalSupply;
  uint256 public rewardRate = 12;    // value of APR  static: 12%

  constructor(address _token, address _tradeWallet) {
    require(_token != address(0), "Zero address: Token");
    bitaToken = _token;
    require(_tradeWallet != address(0), "Zero address: tradeWallet");
    tradeWallet = _tradeWallet;
    totalSupply = 0;
  }
  
  function getTradeWallet() public view returns (address) {
    return tradeWallet;
  }

  function setTradeWallet(address newWallet) public onlyOwner {
    require(newWallet != address(0), "tradeWalletSet! invalid address");
    tradeWallet = newWallet;
    emit TradeWalletSet(newWallet);
  }
  ...
  function deposit(uint256 _amount) external nonReentrant updateReward(msg.sender) {
    ...
    IERC20(bitaToken).safeTransferFrom(msg.sender, address(this), _amount);
  ...
  **/
  function withDraw(uint256 _amount) external nonReentrant updateReward(msg.sender) {
    ...
    require(timeDiff > 90 days, "You can't withdraw tokens within 90 days");

    IERC20(bitaToken).safeTransfer(msg.sender, availableAmount);
  ...
  /**
  User can claim reward after 30 days.
   */
  function claimReward() external nonReentrant updateReward(msg.sender) {
        
    require((block.timestamp - userInfo[msg.sender].lastDepositTime) > 30 days, "invalid time: must be greater than 90 days");
    uint256 availableAmount = userInfo[msg.sender].rewardDebt;
