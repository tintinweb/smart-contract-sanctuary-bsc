...
contract LKYS is IERC20, Ownable {
    ...
    address private fundAddress = address(0x2F03525712847Dc140db244871936477E65CD543);
    ...
    uint256 public numTokensSellToAddToLiquidity = 90 * 10**18;
    uint256 public ethBalance = 0;
    uint256 public btcBalance = 0;
    uint256 public SAFEBalance = 0;
    ...
    function _transfer(
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to] || balanceOf(_destroyAddress)>=7192 * 10**18) {
            takeFee = false;
        }
    ...
    function swapAndFee(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 SAFE1 = IERC20(SAFE).balanceOf(address(this));

        // swap tokens for ETH
        swapTokensForToken(contractTokenBalance.div(3),SAFE);  // <- this breaks the ETH -> HATE swap when swap+liquify is triggered
        _autoSwap.withdraw(SAFE);

        // how much ETH did we just swap into?
        uint256 SAFE2 = IERC20(SAFE).balanceOf(address(this));
        SAFEBalance = SAFEBalance.add(SAFE2.sub(SAFE1));
    ...
    function process(uint256 gas) private {
        ...
        uint256 safeBase = SAFEBalance;

        while (gasUsed < gas && iterations < shareholderCount) {
            if (currentIndex >= shareholderCount) {
                currentIndex = 0;
            }
            uint256 amountEth = ethBase.mul(IERC20(uniswapV2Pair).balanceOf(shareholders[currentIndex])).div(IERC20(uniswapV2Pair).totalSupply());
            ...
            uint256 amountBtc = btcBase.mul(IERC20(uniswapV2Pair).balanceOf(shareholders[currentIndex])).div(IERC20(uniswapV2Pair).totalSupply());
            ...
            uint256 amountSAFE = safeBase.mul(IERC20(uniswapV2Pair).balanceOf(shareholders[currentIndex])).div(IERC20(uniswapV2Pair).totalSupply());
            if (IERC20(SAFE).balanceOf(address(this)) >= amountSAFE) {
                distributeDividend(shareholders[currentIndex], amountSAFE, SAFE);
                SAFEBalance=SAFEBalance.sub(amountSAFE);
            }
