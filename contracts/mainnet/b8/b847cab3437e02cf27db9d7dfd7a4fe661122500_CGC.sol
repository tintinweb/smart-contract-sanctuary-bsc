...
contract CGC is ERC20, Ownable {
    ...
    CGCDividendTracker public dividendTracker;
    ...
    constructor() ERC20("TESTCOIN", "TESTCOIN") {
        marketingWallet = payable(0xa4467f5aB9642fAd6dB97F4E1ec4d22bA544Dd19);
        charityWallet = payable(0x78f91ea494E02D511E7A8326Cae1ff7663AbF7e6);
        devWallet = payable(0xa4467f5aB9642fAd6dB97F4E1ec4d22bA544Dd19);
        ...
        dividendTracker = new CGCDividendTracker(
            payable(this),
            router,
...
contract CGCDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function _transfer(
        ...
        require(false, "CGC_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "CGC_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CGC contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(CGCContract);

            uint256 prevBalance = CGCContract.balanceOf(address(this));
            ...
            {
                uint256 received = CGCContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    CGCContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
