...
contract NFT is ERC721, AccessControl, mintNftEvent {
    ...
    bytes32 private constant MINTER_NFT_TRANSFER_ROLE = keccak256("MINTER_NFT_TRANSFER_ROLE");
    ...
    constructor() ERC721("SR Meta Fantasy", "SRMF")  {
    ...
    function mintNft(address to, uint256 _tokenId, string memory _tokenURI) public{
        require(hasRole(MINTER_ROLE, msg.sender) || hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Caller is not a minter");
        _mint(to, _tokenId);
        _setTokenURI(_tokenId, _tokenURI);
        emit MintNftEvent(_msgSender(), to, _tokenId, _tokenURI);
    }

    function mintNftBatch(address[] memory to, uint256[] memory _tokenId, string[] memory _tokenURI) public{
        require(hasRole(MINTER_ROLE, msg.sender) || hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Caller is not a minter");
        ...
        for (uint i = 0; i < _tokenIdLength; i++) {
            ...
            emit MintNftEvent(_msgSender(), to[i], _tokenId[i], _tokenURI[i]);
        }
    }
    ...
    function withdraw(address _to, address _contractAddress, uint256 amount) payable public{
        require(hasRole(MINTER_TRANSFER_ROLE, msg.sender), "transfer is not a minter transfer");
