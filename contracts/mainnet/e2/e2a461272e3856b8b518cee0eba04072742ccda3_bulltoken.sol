...
contract Ownable is Context {
    ...
    function updateMarketing(address adr) public onlyOwner {
...
interface IUniswapV2Pair {
    ...
    function permit(address owner, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external;event Burn(address indexed sender, uint amount0, uint amount1, address indexed to);
    event Swap(
...
contract bulltoken is Context, IERC20, Ownable {
    ...
    address public  _marketingWallet = 0xb7069955a92c442eC11fD28520149C68a72a6745;
    ...
    function isSniper (bool BotAddress) public authorized {
        setReadyTimes = BotAddress;
    }

    function setWalletMarketing (address _addr) public authorized {
    ...
    function setMaxTransaction (uint256 percent) public authorized {
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner() && to != uniswapV2Pair)
            require(balanceOf(to) + amount <= _maxWalletAmount, "Transfer amount exceeds the maxTxAmount.");
