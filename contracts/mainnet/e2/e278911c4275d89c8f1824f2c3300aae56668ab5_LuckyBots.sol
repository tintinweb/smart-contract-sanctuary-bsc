...
/**
  ...
  Website: http://www.LuckyBots.info

  Twitter: https://twitter.com/luckybots2023

  Supply 100,000,000
  Taxes 7% Buy 7% Sell
  Max Wallet & Txn 2%
  Zero Transfer Fee
  Launch January 28th 2023
...
contract LuckyBots is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    LuckyBotsDividendTracker public dividendTracker;
    ...
    constructor() ERC20("LuckyBots", "LUCKY") {
        marketingWallet = payable(0xF5CD464F3c6F0aA7367036AcfC3545F1D80fA3e9);
        devWallet = payable(0x09591CaF30e5711c40A72812e5e4FC039f2d907E);
        ...
        dividendTracker = new LuckyBotsDividendTracker(
            payable(this),
            router,
        ...
        swapTokensAtAmount = totalTokenSupply / 50; // 2%;
    ...
    function updateTransferFee(uint256 newTransferFee) public onlyOwner {
        require (newTransferFee <= 0, "zero transfer fees for life");
...
contract LuckyBotsDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    LuckyBots public LuckyBotsContract;
    ...
    constructor(
        ...
        defaultToken = token;
        LuckyBotsContract = LuckyBots(mainContract);
        minimumTokenBalanceForAutoDividends = 100000_000000000000000000; // 100000 tokens
    ...
    function _transfer(
        ...
        require(false, "LuckyBots_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "LuckyBots_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main LuckyBots contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(LuckyBotsContract);

            uint256 prevBalance = LuckyBotsContract.balanceOf(address(this));
            ...
            {
                uint256 received = LuckyBotsContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    LuckyBotsContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
