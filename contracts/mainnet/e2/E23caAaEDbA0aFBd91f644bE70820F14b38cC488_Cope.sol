...
contract Cope is Context, Ownable {
    ...
    constructor () {
        ...
        _maxWalletPercent = 24;
        _maxWalletDivisor = 59;
        _start = 10;
        _antiblock = 12;
    ...
    function changeRewardsCopeFee(uint8 value) external onlyOwner{
        _rewardsFee = value;
    }

    function changeCopeLiquiditFee(uint8 value) external onlyOwner{
        _liquidityFee = value;
    }

    function CopechangeMarketingFee(uint8 value) external onlyOwner{
        _marketingFee = value;
    }

    function changeGasForCope(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Processing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Cannot update gasForProcessing to same value");
        gasForProcessing = newValue;
    }

    function changeCopeClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "ClaimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Cannot update claimWait to same value");
        claimWait = newClaimWait;
    }

    function CopeshouldClaim(uint256 lastClaimTime) private view returns (bool) {
