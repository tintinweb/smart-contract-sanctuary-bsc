...
contract Node {
    ...
    IERC20 public constant TOKEN = IERC20(0xB1A6b1ff02D5C108F450FBc74F721C87916bEF83);
    uint256[] public tierAllocPoints = [1 ether, 1 ether, 1 ether];
    uint256[] public tierAmounts = [15 ether, 50 ether, 50 ether];
    struct User {
    ...
    function addUser(address _addressToWhitelist) external onlyDev {
        whitelistedAddresses[_addressToWhitelist] = true;
    }

    function removeFromWhitelist(address _beneficiary) external onlyDev {
    ...
    function addManyToWhitelist(address[] memory _beneficiaries) external onlyDev {
    ...
    function isWhitelist(bool _enabled) external onlyDev {
        whitelist = _enabled;
    }
    ...
    function create(uint256 nodeTier, uint256 numNodes) external {
        ...
        if(whitelist){
            require(verifyUser(_sender), "Not Whitelisted");
        }
