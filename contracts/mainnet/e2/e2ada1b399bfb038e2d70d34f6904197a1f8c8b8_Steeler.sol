...
contract Steeler is Ownership {
    using SafeMath for uint256;

    string _name = "Steeler";
    string _symbol = "Steeler";
    uint8 _decimals = 9;
    ...
    function lpRewardSteelerFee(uint8 rewardfee) external onlyOwner {
    ...
    function maxTxSteelerSetting(uint256 maxTxAmount) external onlyOwner {
        _maxTxAmount = maxTxAmount;
    }

	function getSteelerFees(uint256 buybackMultiplierTriggeredAt, uint256 totalFee) public view returns (uint256) {
			if (
