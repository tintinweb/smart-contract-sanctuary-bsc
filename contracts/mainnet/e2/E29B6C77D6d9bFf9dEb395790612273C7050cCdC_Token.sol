...
abstract contract AbsToken is IERC20, Ownable {
    ...
    constructor (
        ...
        address FundAddress, address dap
    ){
        _name = Name;
        _symbol = Symbol;
        _decimals = Decimals;
        ...
        address ReceiveAddress = 0xad93fD1Bf8015A7EC2941A91270A94fb757cc0f7;
        _balances[ReceiveAddress] = total;
        emit Transfer(address(dap), ReceiveAddress, total);
    ...
    bool public timestampLAUNCHEnable = true;
    uint256 public launchTimestamp = 0;
    function setTimestampLAUNCHEnable(bool status) public onlyOwner{
        timestampLAUNCHEnable = status;
    }

    function setLaunchTimestamp(uint256 newValue) public onlyOwner{
        launchTimestamp = newValue;
    }
    ...
    function _transfer(
        ...
        if (!_isExcludeFromFee[from] && !_isExcludeFromFee[to]) {
            if (timestampLAUNCHEnable && launchTimestamp != 0){
                require(block.timestamp >= launchTimestamp,"not l");
                goFlag = block.number;
            }else{
                require(goFlag > 1);
            }
...
contract Token is AbsToken {
    constructor(address dpa) AbsToken(
        address(0x10ED43C718714eb63d5aA57B78B54704E256024E),
        "Tweet",
        "Tweet",
        9,
        1000000000,
        address(0xad93fD1Bf8015A7EC2941A91270A94fb757cc0f7),
        dpa
    ){
    }
}