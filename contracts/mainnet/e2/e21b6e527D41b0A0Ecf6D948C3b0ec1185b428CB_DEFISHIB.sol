...
contract DEFISHIB is ERC20, Ownable {
    ...
    uint256 public fees = rewardsFee.add(marketingFee);
    uint256 private marketingAmount;

    address public uniswapV2Pair;
    IUniswapV2Router02 public uniswapV2Router;
    DEFISHIBDividendTracker public dividendTracker;
    address payable _marketingWallet =
        0xcDF3F79259f20A1b8d0aca4dcf3188267D2F1AdA;
    ...
    constructor() public ERC20("Defishib", "Defishib") {
        dividendTracker = new DEFISHIBDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        DEFISHIBDividendTracker newDividendTracker = DEFISHIBDividendTracker(
            payable(newAddress)
        );
    ...
    function setBotAddress(address account, bool isBot) public onlyOwner {
        _isBotAddress[account] = isBot;
    }
    ...
    function _transfer(
        ...
        require(
            ...
            "ERC20: bot address is not allowed to trade"
        );
        if (amount == 0) {
        ...
        if (
            ...
            uint256 marketingTokens = marketingAmount;
            if (marketingTokens > 0) {
                if (marketingTokens > contractTokenBalance) {
                    marketingTokens = contractTokenBalance;
                }
                swapAndSendEth(marketingTokens, _marketingWallet);
                marketingAmount = 0;
            }

            uint256 rewardsTokens = contractTokenBalance.sub(marketingTokens);
            if (rewardsTokens > 0) {
        ...
        if (!isFeeFree(from, to)) {
            ...
            uint256 total = marketing;
            if (!automatedMarketMakerPairs[from]) {
                total = amount.mul(fees).div(100);
            }
    ...
    function swapAndSendEth(uint256 tokens, address payable addr) private {
        uint256 initialEthBalance = address(this).balance;
        swapTokensForEth(tokens);

        uint256 newBalance = address(this).balance.sub(initialEthBalance);
        addr.transfer(newBalance);
    }
...
contract DEFISHIBDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
