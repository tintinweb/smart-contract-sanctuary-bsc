...
interface IMCGRand {
   ...
   * Views random result
   */
  function viewRandomResult(address _contractAddr)
    external
    view
    returns (uint32);
   ...
   * Views random result
   */
  function viewRandomResult256(address _contractAddr)
    external
    view
    returns (uint256);
}
...
contract MCGRand is VRFConsumerBase, IMCGRand, Ownable {
   ...
   * @notice View latest id for a requesting contract
   */
  function viewLatestId(address _contractAddr)
    external
    view
    override
    returns (uint256)
    ...
    override
  {
    ...
    latestIds[requester] = IMCGRandConsumer(requester).viewCurrentTaskId();
