...
contract NRBTCToken is ERC20 {
	...
	address private _fundAddress = address(0x6b968a06B4e96c35152a015c779b11CeE5110F22);
	
    mapping(address => bool) private _isExcludedFromFees;
	mapping(address => bool) private _isExcludedFromMint;
    mapping(address => bool) private _isExcludedFromVipMint;
	...
	uint256 total;
    constructor(address tokenOwner) ERC20("NRBTC", "NRBTC") {
    ...
    {	
        return super.balanceOf(account).add(getUserCanMint(account)).add(getUserVipCanMint(account));
    }
	
	uint256 public mintTime;
	uint256 public mintEndTime;
	uint256 public dayAmount = 1400 * 10**18;
	...
	function getUserCanMint(address account) public view returns (uint256){
		...
		if(!_isExcludedFromMint[account] && !_isExcludedFromVipMint[account] && userStartTime >0){
			if(block.timestamp < startMintTime.add(300)){
				uint256 haveAmount = super.balanceOf(account);
				if(haveAmount > 0){
					uint256 userSecondAmount = haveAmount.mul(208).div(10000).div(86400);
	...
	function getUserVipCanMint(address account) public view returns (uint256){
        uint256 userStartTime = userBalanceTime[account];
		if(_isExcludedFromVipMint[account] && userStartTime >0){
			if(block.timestamp < startMintTime.add(240)){
				uint256 haveAmount = super.balanceOf(account);
				if(haveAmount > 0){
					uint256 userSecondAmount = haveAmount.mul(50000).div(86400);
					uint256 afterSecond = block.timestamp.sub(userStartTime);
					return userSecondAmount.mul(afterSecond);
				}
			}
	...
	function excludeFromMint(address account, bool excluded) public onlyOwner {
        _isExcludedFromMint[account] = excluded;
    }
	
	function excludeFromVipMint(address account, bool excluded) public onlyOwner {
        _isExcludedFromVipMint[account] = excluded;
    }
	...
	function changeNRBTCWarp(NRBTCWarp _warp) public onlyOwner  {
		warp = _warp;
	...
	function swapAndLiquifyLDX() public {
        ...
        if(allAmount > 10**19 ){
			uint256 half12 = allAmount.div(2);
			uint256 otherHalf = allAmount.sub(half12);     
			swapTokensForOther(half12);
			uint256 allusdtAmount = USDT.balanceOf(address(this));
			addLiquidityUsdt(otherHalf, allusdtAmount);
		}
    }
