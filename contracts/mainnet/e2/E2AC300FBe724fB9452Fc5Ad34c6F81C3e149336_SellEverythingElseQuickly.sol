...
/*  
    https://t.me/SellEverythingElseQuickly
*/
...
contract SellEverythingElseQuickly is Context, ERC20, Ownable {
    using SafeMath for uint256;
   ...
   SeriouslyDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("Sell Everything Else Quickly", "SEEQ") {
        _buyRewardsFee = 4;
        _buyLiquidityFee = 2;
		_buyMarketingFee = 5;
        _buyDevFee = 1;
    	...
    	dividendTracker = new SeriouslyDividendTracker();

    	liquidityWallet = owner();
		
		marketingWallet = payable(0xdDbbB9ae241e76cB3AC05265d8941eaB68f6A33F);
        _devWallet = payable(0x05BD7b323B2C2F5777bfFd6bE0Ea26880786eBaF);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "Seriously: The dividend tracker already has that address");

        SeriouslyDividendTracker newDividendTracker = SeriouslyDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "Seriously: The new dividend tracker must be owned by the Seriously token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "Seriously: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Seriously: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Seriously: Account is already the value of 'excluded'");
    ...
    function addToBlacklist(address account, bool blacklisted) public onlyOwner {
        require(_isBlacklisted[account] != blacklisted, "Seriously: Account is already the value of 'blacklisted'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Seriously: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Seriously: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "Seriously: The liquidity wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "Seriously: The marketing wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Seriously: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Seriously: Cannot update gasForProcessing to same value");
...
contract SeriouslyDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("Seriously_Dividend_Tracker", "Seriously_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function withdrawDividend() public pure override {
        require(false, "Seriously_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Seriously contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 0 && newClaimWait <= 86400, "Seriously_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Seriously_Dividend_Tracker: Cannot update claimWait to same value");
