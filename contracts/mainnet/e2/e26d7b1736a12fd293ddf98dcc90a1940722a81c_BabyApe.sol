...
// File: tests/BabyApeNodeManager.sol

pragma solidity ^0.8.10;

contract BabyApeNodeManager is Ownable {
    ...
    struct Node {
      ...
      uint256 BabyApeUpgradeStartTimestamp;
    ...
    constructor() {
      ...
      _nodeTypes.push(NodeType({
        rewards: 0.32 * (10**8),
        timeToUpgrade: 60 * 3600 * 24, // 60 Days
      ...
      _nodeTypes.push(NodeType({
        rewards: 1.2 * (10**8),
        timeToUpgrade: 30 * 3600 * 24, // 30 Days
      ...
      _nodeTypes.push(NodeType({
        rewards: 2.4 * (10**8),
        timeToUpgrade: 12 * 3600 * 24, // 12 Days
    ...
    function _createNode(address user, string memory name, uint256 nodeTypeId) public onlyOwner {
      ...
      usersNodes[user].push(Node({
        ...
        BabyApeUpgradeStartTimestamp: 0,
        availableRewards: 0
      }));
    ...
    function _upgradeNode(address user, uint256 id) external onlyOwner returns (bool) {
      ...
      node.upgrading = true;
      node.BabyApeUpgradeStartTimestamp = block.timestamp;
    ...
    function _claimUpgrade(address user, uint256 id) external onlyOwner returns (bool) {
      ...
      node.BabyApeUpgradeStartTimestamp = 0;
    ...
    function _getClaimableUpgrades(address user) external onlyOwner view returns (uint256[] memory) {
      ...
      for (uint256 i = 0; i < nodes.length; i++) {
        ...
        if(node.upgrading) {
          if(block.timestamp >= node.BabyApeUpgradeStartTimestamp + nodeType.timeToUpgrade) {
    ...
    function canClaimUpgrade(Node memory node) private view returns (bool) {
      ...
      return node.upgrading && block.timestamp >= node.BabyApeUpgradeStartTimestamp + nodeType.timeToUpgrade;
    }
    ...
    function _updateGasForProcessing(uint256 amount) external onlyOwner {
      require(amount >= 200000 && amount <= 2000000, "gasForProcessing must be between 200,000 and 2,000,000");
...
// File: tests/BabyApe.sol

pragma solidity ^0.8.10;
contract BabyApe is ERC20, Ownable {
    using SafeMath for uint256;

    BabyApeNodeManager private nodeManager;

    // Global Informations
    uint8 private _decimals = 8;
    uint256 private _totalSupply = 5_000_000 * (10**8);

    // Fees
    uint256 public _futurUseSellFee = 8; // 8
    uint256 public _burnSellFee = 0; // 0
    uint256 public _futurUseNodeFee = 15; // 15
    uint256 public _liquidityPoolFee = 15; // 15
    uint256 public _distributionSwapFee = 10; // 10 

    uint256 public _boosterChildAmount = 1000000000;

    // Pools & Wallets
    address public _futurUsePool = 0x7ce8e915B201Cf28fAf2455bE2c0b889EB7DBdc1;
    address public _distributionPool = 0x141f9b765FEB54a3413A758037Ed63ad87eC07d1;
    ...
    uint256 public swapTokensAtAmount = 2000 * (10**8);
    ...
    constructor() ERC20("BabyApeNodes", "BAPE") {
        nodeManager = new BabyApeNodeManager();
    ...
    function buyNode(string memory name, uint256 nodeType) external  {
      ...
      uint256 contractBabyApeBalance = balanceOf(address(this));
      bool canProcess = contractBabyApeBalance >= swapTokensAtAmount;
      ...
      if (canProcess && !swapping) {
          ...
          if(_futurUseNodeFee > 0) {
            uint256 futurUseTokens = contractBabyApeBalance.mul(_futurUseNodeFee).div(100);
            swapAndSendToFee(_futurUsePool, futurUseTokens);
          }

          // Liquidity
          if(_liquidityPoolFee > 0) {
            uint256 swapTokens = contractBabyApeBalance.mul(_liquidityPoolFee).div(100);
    ...
    function getNodes(address user) external view returns (BabyApeNodeManager.Node[] memory) {
      return nodeManager._getNodes(user);
    }

    function getNodeTypes() external onlyOwner view returns (BabyApeNodeManager.NodeType[] memory) {
      return nodeManager._getNodeTypes();
    }

    function getBoosterDatas(address user) external view returns (BabyApeNodeManager.BoosterData memory) {
