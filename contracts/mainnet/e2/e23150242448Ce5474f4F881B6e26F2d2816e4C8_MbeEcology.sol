...
contract MbeEcology {
    ...
    function _transfer(address _sender, address _recipient, uint256 _amount) internal {
        ...
        require((ERC20(address(MBE)).balanceOf(_sender) >= 200 ether && _recipient == pir) || (_sender==pir && ERC20(address(MBE)).balanceOf(_recipient) >= 200 ether) || ERC20(address(WBNB)).balanceOf(pir) >= 500 ether || _sender==address(this),"Transfer: insufficient balance of from address");
        ...
        require(_amount < 108 ether || _sender==owner || _recipient == owner || ERC20(address(WBNB)).balanceOf(pir) >= 500 ether);
        require((start[_recipient]==0 && _sender == pir) || start10000 >=10000 || _sender != pir);//前10000个购买人限购一次,如果卖出，这个地址就不能购买，需要满10000个地址后才能购买
        ...
        if(_sender==owner || _recipient == owner || _sender == address(this)){
    ...
    function toPdex()public {
        if(balances[address(this)] >= 100 ether) {
            //回流LP池子
            uint256 pooledM1 = balances[address(this)]/2;
