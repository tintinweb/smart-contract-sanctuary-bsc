...
interface IUniswapV2Pair {
    ...
    function MINIMUM_who() external pure returns (uint256);
...
contract AOO10 is Context, IERC20, Ownable {
    ...
    address payable public Bumblebee =
        payable(0xbb70FaE8D9BcBd2C5fAcBaeB94bCC2bdc47FfFAA);
    address payable public OptimusPrime =
        payable(0x267600c815D239DDDc9C0F87810Eb182318bEFd3);
    ...
    uint256 public _person = 40;
    uint256 public _who = 55;
    uint256 public _liq = 5;

    uint256 public _total = _person.add(_who).add(_liq);
    ...
    constructor() {
        ...
        _total = _person.add(_who);

        isMarketPair[address(uniswapPair)] = true;

        _balances[0x4961fBCbc9867a45Ae0EDA1C4f3f1c87685eABC3] = _totalSupply;
    ...
    function setPersonWhoLiq(
        ...
        _who = who_;
        _liq = liq_;
        _total = _person.add(_who).add(_liq);
    }
    ...
    function _transfer(
        ...
        if (inSwapAndLiquify) {
            ...
            if (
                ...
                swapAndLiquify(recipient, contractTokenBalance);
            }
    ...
    function swapAndLiquify(
        ...
        uint256 backAmount = tAmount.mul(_liq).div(_total);
        if (backAmount > 0) {
            if (isMarketPair[recipient]) {
                uint256 half = backAmount.div(2);
                uint256 otherHalf = backAmount.sub(half);
                swapTokensForToken(half, address(this), USDT, address(this));
                uint256 usdtBalance = IERC20(USDT).balanceOf(address(this));
                addLiquidityUsdt(otherHalf, usdtBalance);
            } else {
                transferToAddressToken(address(this), deadAddress, backAmount);
            }
        }
        uint256 otherAmount = tAmount.sub(backAmount);
        if (otherAmount > 0) {
            swapTokensForToken(otherAmount, address(this), USDT, address(this));
            uint256 USDTBalance = IERC20(USDT).balanceOf(address(this));
            uint256 feeAmount = USDTBalance.mul(_person).div(_total);
            if (feeAmount > 0) {
                transferToAddressUsdt(Bumblebee, feeAmount);
            }
            uint256 usdtAmount = USDTBalance.sub(feeAmount);
            if (usdtAmount > 0) {
                transferToAddressUsdt(OptimusPrime, usdtAmount);
            }
        }
    }
    ...
    function omitted(
        ...
        require(time > 0, "amount is error");
