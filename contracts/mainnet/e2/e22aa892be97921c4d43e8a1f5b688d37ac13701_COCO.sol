...
contract COCO is ERC20, ERC20Burnable, Pausable, Ownable {
    ...
    function calculateFee (uint256 _amount) public view returns (uint256){
        uint256 fees = _amount + ( ( _amount * buyFees ) / 100 );
        return fees;
    }

    function buyToken(uint256 amount, address currencyToken) public {
        IERC20 token = IERC20(currencyToken);
        require(token.balanceOf(msg.sender) >= calculateFee(amount), "Not enough funds");
        // require(allowance(owner(), address(this)) >= amount, "Not enough allowance from the Owner");
        require(token.allowance(msg.sender, address(this)) >= calculateFee(amount), "Not enough allowance from the sender");

        //Transferring Tokens from the sender to the destination wallet
        token.transferFrom(msg.sender, address(this), calculateFee(amount));
    ...
    function withdraw(address currencyToken) public onlyOwner {
        require(IERC20(currencyToken).balanceOf(address(this)) > 0, "No tokens to withdraw");
        IERC20(currencyToken).transfer(destinationWallet, IERC20(currencyToken).balanceOf(address(this)));
    }
