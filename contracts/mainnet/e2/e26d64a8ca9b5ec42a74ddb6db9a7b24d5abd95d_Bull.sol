...
contract Bull is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the BULL ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) ERC20("BULL", "BULL") {
        // Mints 1 BULL to contract creator for initial pool setup
    ...
    function _getBullPrice() internal view returns (uint256 _bullPrice) {
        try IOracle(bullOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Bull: failed to fetch BULL price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _bullPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_bullPrice >= taxTiersTwaps[tierId]) {
    ...
    function setBullOracle(address _bullOracle) public onlyOperatorOrTaxOffice {
        require(_bullOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints BULL to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of BULL to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentBullPrice = _getBullPrice();
            currentTaxRate = _updateTaxRate(currentBullPrice);
            if (currentBullPrice < burnThreshold) {
                burnTax = true;
            }
        }
