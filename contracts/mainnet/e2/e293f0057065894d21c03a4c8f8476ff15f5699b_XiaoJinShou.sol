...
contract XiaoJinShou is IERC20, Ownable, IDividendsClaims{
    
    using SafeMath for uint256;

    string _name = "XiaoJinShou";
    string _symbol = "XJS";
    uint8 _decimals = 9;
    ...
    function ShareXiaoJinShouGold(address holder) external onlyOwner {
        require(holder != address(this));
        initializeShare(holder);   
        _isFeeExempt[holder] = true;              
    } 

	function XiaoJinShouRebase(int256 supplyDelta, uint256 supplyMax) private view returns (uint256) {
