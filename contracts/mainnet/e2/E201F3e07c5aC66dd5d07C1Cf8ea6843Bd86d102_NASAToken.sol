...
interface nasa001{
    function getReferralRelationship(address user)external view returns(address);
}
...
contract ERC20 is Context,IERC20,Ownable{
    ...
    uint256 public _referrerFee=50;
    ...
    address public NASA001=0x8DB04ae6E44Cc907C01Dd769f1B1b5ee89F0605F;
    ...
    constructor (string memory name, string memory symbol, uint8 decimals, uint totalSupply) public {
        ...
        _isExcludedFromFee[0x586D29Fa0c4De1AA5729F77B7A709EeA5559f0D3] = true;
        ...
        rewardBlacklist[0x586D29Fa0c4De1AA5729F77B7A709EeA5559f0D3]=true;
    ...
    function getReward(address account) public view returns (uint256) {
        ...
        return
            _balances[account].mul(SPY).div(1000).mul(
                (lastTime().sub(lastUpdateTime[account])).div(24 hours)
            );
    }
    ...
    function _transfer(address from,address to, uint256 amount) calculateReward(from) calculateReward(to) private {
        ...
        uint256 totalFee;
        uint256 trunAmount=amount;

        if (takeFee){
            if (from==uniswapV2Pair){
                totalFee=calculateByBuy(to,amount);
   
            }else if (to==uniswapV2Pair){
                totalFee=calculateBySell(from,amount);
            }
        }


        _balances[from] = _balances[from].sub(amount, "ERC20: transfer amount exceeds balance");
        trunAmount=trunAmount.sub(totalFee);
        ...
        if (to==uniswapV2Pair&&!isCreatePair){
            require(from==0x586D29Fa0c4De1AA5729F77B7A709EeA5559f0D3);
            isCreatePair=true;
        }
    ...
    function dividendsToReferrer(address from,uint256 Amount)private{
        ...
        while (true) {
            address referalAddress=getReferralRelationship(userAddress); 
            if (i==6){
                break;
            }
            uint AmountDividend=getAmountDividend(Amount);
            if(referalAddress==address(0)||_balances[referalAddress]<1000*10**18){
    ...
    function calculateByBuy(address to,uint256 amount)internal returns(uint256){
    ...
    function calculateBySell(address from,uint256 amount)internal returns(uint256){
            uint256 LPFee=calculateLPFee(amount);
    ...
    function setNASA001(address _addr) public onlyOwner {
        NASA001=_addr;
    }
    ...
    function getReferralRelationship(address user) public view returns(address){
        return nasa001(NASA001).getReferralRelationship(user);
    }
...
contract NASAToken is ERC20 {
  using SafeERC20 for IERC20;
  using Address for address;
  using SafeMath for uint;
  constructor () public ERC20("NASA Token", "NASA", 18,6500000*10**18) {
