...
contract SwapCheck is Ownable {
    ...
    function checkBuyFee(
        address router,
        uint amountIn,
        address[] calldata path
    ) external returns (uint256 calBuyAmount, uint256 realBuyAmount){
        ...
        IERC20(spendToken).transferFrom(account, address(this), amountIn);
        ...
        try swapRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            amountIn, 0, path, account, block.timestamp
        ){} catch{}
