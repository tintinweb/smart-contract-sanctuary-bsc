...
contract StandardErc20Token is IERC20TokenInterface,Ownable,ITokenFactory{
...
function mint(address _owner,uint256 _amount) override   onlyMinter external {
    require(_amount>0,"ERC20:  amount error");
    balances[_owner]+=_amount;
    _totalSupply+=_amount;
    emit Transfer(address(0),_owner, _amount);
}

function burn(address _owner,uint256 _amount) override   onlyMinter external{
    require(_amount>0&&balances[_owner]>=_amount,"ERC20:  amount error");
    balances[_owner]-=_amount;
    _totalSupply-=_amount;
...
contract CTOToken is StandardErc20Token{
    string private  name="Creation Orbs";
    string private  symbol="CTO";
    string private  version="1.0";
    uint256 private  decimals=18;
    uint256 private totalSupply =0 * 10**uint(decimals);
