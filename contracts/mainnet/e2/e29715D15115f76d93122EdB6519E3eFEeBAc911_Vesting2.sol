...
contract Vesting2 is IVesting, Ownable, Destory {
  ...
  constructor(address __tokenAddress, address __beneficiaryAddress, uint64 startAt) {
    require(__tokenAddress != address(0), "Vesting2: tokenAddress is zero");
    require(__beneficiaryAddress != address(0), "Vesting2: beneficiaryAddress is zero");
  ...
  modifier hasPerm() {
    require(_operationAuthority[_msgSender()], "Vesting2: caller is not has authority");
    _;
  }
  ...
  function addBalance(address beneficiary, uint256 amount) public override hasPerm returns (bool) {
    revert("Vesting2: addBalance not support");
  }
  ...
  function release() public override hasPerm returns (bool) {
    ...
    _released = balance;
    if (balance > 0) {
        IERC20(_tokenAddress).transfer(_beneficiaryAddress, balance);
    }
    return true;
  }
  ...
  function addOperationAuthority(address perm) public override onlyOwner returns (bool) {
    require(perm != address(0), "Vesting2: addOperationAuthority address is zero");
