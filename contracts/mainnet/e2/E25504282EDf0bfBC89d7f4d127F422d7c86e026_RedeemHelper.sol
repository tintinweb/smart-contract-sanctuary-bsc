...
contract RedeemHelper is Ownable {
    ...
    function redeemAll( address _recipient,uint[] memory _ids,  bool _stake ) external {
        for( uint i = 0; i < bonds.length; i++ ) {
            if ( bonds[i] != address(0) ) {
                for(uint ii = 0;ii<_ids.length;ii++){
                    if ( IBond( bonds[i] ).pendingPayoutFor( _ids[ii] ) > 0 ) {
                        IBond( bonds[i] ).redeem( _recipient,_ids[ii], _stake );
                    }
                }
            }
        }
