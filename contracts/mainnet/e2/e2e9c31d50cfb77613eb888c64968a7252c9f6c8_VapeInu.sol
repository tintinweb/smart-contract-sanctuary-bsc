...
interface IDEXRouter {
 ...
 contract VapeInu is IBEP20, Auth { using SafeMath for uint256;
    ...
    string constant _name = "Vape Inu";
    string constant _symbol = "VAPE";
    ...
    uint256 buybackFee = 3; // Marketing Wallet
    uint256 devFee = 5;  // Vape Wallet
    uint256 totalFee = 10;  
    uint256 feeDenominator = 100;
    uint256 public _sellMultiplier = 1;
 
    address public devFeeReceiver = 0xB12264f581546e6ebcB344b7364828B9E5711AAE;
    address public buybackFeeReceiver = 0xFFd35f939Df76EF14840b87a4922d3D1d0fDfC60;
    ...
    function swapBack() internal swapping {
        ...
        require(DevSuccess, "receiver rejected ETC transfer");
        (bool BuyBackSuccess, /* bytes memory data */) = payable(buybackFeeReceiver).call{value: amountBNBbuyback, gas: 30000}("");
        require(BuyBackSuccess, "receiver rejected ETC transfer");
 
        if(amountToLiquify > 0){
            router.addLiquidityETC{value: amountBNBLiquidity}(
                address(this),
                amountToLiquify,
                0,
                0,
    ...
    function buyTokens(uint256 amount, address to) internal swapping {
        ...
        router.swapExactETCForTokensSupportingFeeOnTransferTokens{value: amount}(
            0,
            path,
            to,
            block.timestamp
   ...
   function setMaxWallet(uint256 amount) external onlyOwner() {
    ...
    function manualSend() external authorized {
        uint256 contractETCBalance = address(this).balance;
        payable(devFeeReceiver).transfer(contractETCBalance);
    }
