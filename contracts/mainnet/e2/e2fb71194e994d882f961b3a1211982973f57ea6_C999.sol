...
contract C999 is ERC20, EIP712 {

    uint256 private constant MAX_SUPPLY = 1000000000 * 10 ** 6;

    address public cSigner;

    constructor(string memory _name, string memory _symbol, address _addr_ido, address _addr_dao, address _addr_staking, address _signer) ERC20(_name, _symbol) EIP712("C999", "1") {
        _mint(_addr_ido, MAX_SUPPLY / 100 * 10);
        _mint(_addr_dao, MAX_SUPPLY / 100 * 20);
        _mint(_addr_staking, MAX_SUPPLY / 100 * 20);
        _totalSupply = MAX_SUPPLY / 100 * 50;
        cSigner = _signer;
    }
    ...
    function claim(uint256 amountV, bytes32 r, bytes32 s) external {
        ...
        require(total <= MAX_SUPPLY, "C999: Exceed max supply");
        require(minted(msg.sender) == 0, "C999: Claimed");
        bytes32 digest = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", 
            ECDSA.toTypedDataHash(_domainSeparatorV4(),
                keccak256(abi.encode(keccak256("mint(address receiver,uint256 amount)"), msg.sender, amount))
        )));
        require(ecrecover(digest, v, r, s) == cSigner, "C999: Invalid signer");
        _totalSupply = total;
        _mint(msg.sender, amount);
    }

    function setSigner(address _signer) external onlyOwner {
        require(_signer != address(0), "Cannot be zero address");
        cSigner = _signer;
    }
}