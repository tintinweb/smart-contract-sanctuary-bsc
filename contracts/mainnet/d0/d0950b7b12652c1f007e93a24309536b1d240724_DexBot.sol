...
contract DexBot is Initializable {
    ...
    event staking(bool isMine, address indexed user, uint256 amount);
    ...
    function initialize(
        ...
        UserInfo storage user = userData[_receivar];
        user.userid = 1;
        user.plan = 2;
        user.isUser = true;
        ...
        queenReward[8] = 3e16;
        queenReward[9] = 3e16;
        withdrawMinLimit = 2592000;
        withdrawMinLimitStaking = 2592000;
    }
    ...
    function updateQueenReward(uint256 _plan, uint256 _percentageReward)
        ...
        onlyContractOwner
    {
        queenReward[_plan] = _percentageReward;
    }

    function updateChangeConractOwner(address _owner) public onlyContractOwner {
        contractOwner = _owner;
    }

    function updateQbtAddress(address _adress) public onlyContractOwner {
        QbtAddresss = _adress;
    }

    function updateBusdAddress(address _adress) public onlyContractOwner {
        BusdAddresss = _adress;
    }

    function updatePriceProvider(AggregatorV3Interface pp)
        public
        onlyContractOwner
    {
        priceProvider = pp;
    ...
    function updateKingReward(uint256 _plan, uint256 _percentageReward)
        public
        onlyContractOwner
    {
        kingReward[_plan] = _percentageReward;
    }

    function updatePrice(uint256 _plan, uint256 _amount)
        public
        onlyContractOwner
    {
        price[_plan] = _amount;
    ...
    function updateWithdrawLimit(uint256 _minLimit) public onlyContractOwner {
        withdrawMinLimit = _minLimit;
    }

    function updateWithdrawStakingLimit(uint256 _minLimit)
        public
        onlyContractOwner
    {
        withdrawMinLimitStaking = _minLimit;
    }
    ...
    function add(
        ...
        poolInfo.push(
            StakePoolInfo({
                ...
                maxStakeAmount: _maxStakeAmount * 1e18, //  120
    ...
    function set(
        ...
        pool.maxStakeAmount = _maxStakeAmount * 1e18; //  120
    ...
    function distributeStakeReferral(
        address userNew,
        uint256 cnt,
        uint256 plan
    ) internal {
        if (cnt < 10 && userNew != 0x0000000000000000000000000000000000000000) {
            ...
            cnt++;
            distributeStakeReferral(userstake.parent, cnt, plan);
        }
    }
    ...
    function stake(uint256 amount, uint256 _pid) public {
        ...
        require(amount <= pool.maxStakeAmount, "Maximum error");
        ...
        UserInfo storage user = userData[msg.sender];
        if (
            block.timestamp > userstake.lastWithdrawRewards &&
            userstake.amount > 0
        ) {
            uint256 multiplier = getMultiplier(
                userstake.lastWithdrawRewards,
                block.timestamp
            );
            ...
            if (!pool.isMine) {
                uint256 pendingReward = (userstake.amount *
                    multiplier *
                    pool.rewardPercent) / (perMonthsec * 100);
        ...
        if (!user.distibuteReward) {
            UserInfo storage parent = userData[msg.sender];
            distributeStakeReferral(parent.parent, 0, user.plan);
            user.distibuteReward = true;
        }
        userstake.amount += amount;
        userstake.lastStaked = block.timestamp;
        userstake.lastWithdrawRewards = block.timestamp;

        emit staking(pool.isMine, msg.sender, amount);
    }

    function withdrawStakeRewards(uint8 _pid, bool _unStake) public {
        StakePoolInfo storage pool = poolInfo[_pid];
        UserInfo storage parent = userData[msg.sender];
        UserInfo storage userstake = userInfo[_pid][msg.sender];
        ...
        uint256 multiplier;
        uint256 userPending;
        uint256 companyPending;
        uint256 perMonthsec = 86400 * pool.duration;
        uint256 Maxdiffrence = userstake.lastStaked + perMonthsec;
        if (!pool.isMine) {
            require(
                block.timestamp >
                    userstake.lastWithdrawRewards + withdrawMinLimitStaking,
                "Still withdraw locked."
            );

            if (Maxdiffrence >= block.timestamp) {
                ...
                userstake.lastWithdrawRewards = Maxdiffrence;
            }
            if (multiplier > 0) {
                userstake.stakeReward +=
                    (userstake.amount * multiplier * pool.rewardPercent) /
                    (perMonthsec * 100);
                userPending = ((userstake.stakeReward * referalEarnings[0]) /
                    100);
                companyPending =
                    (userstake.stakeReward * referalEarnings[10]) /
                    100;
                distributeEarnings(parent.parent, 1, _pid, msg.sender);
                userstake.stakeReward = 0;
            }
        } else {
            require(
                block.timestamp >
                    userstake.lastWithdrawRewards + withdrawMinLimit,
                "Still withdraw locked."
            );
            multiplier = getMultiplier(
                userstake.lastWithdrawRewards,
            ...
            if (multiplier > 0) {
                userstake.mineReward +=
                    (userstake.amount * multiplier * pool.rewardPercent) /
                    (perMonthsec * 100);
                userPending = ((userstake.mineReward * referalEarnings[0]) /
                    100);
                companyPending =
        ...
        if (_unStake) {
            if (!pool.isMine) {
                require(
                    Maxdiffrence <= block.timestamp,
                    "Unable to unstake now"
                );
            }
            userPending += userstake.amount;
            userstake.amount = 0;
        }
        if (userPending > 0) {
    ...
    function getrewardAmount(uint8 _pid, address userAddr)
        public
        view
        returns (uint256)
    {
        StakePoolInfo storage pool = poolInfo[_pid];
        UserInfo storage userstake = userInfo[_pid][userAddr];
        uint256 multiplier;
        uint256 userPending;
        uint256 perMonthsec = 86400 * pool.duration;
        if (!pool.isMine) {
            ...
            if (multiplier > 0) {
                uint256 reward = userstake.stakeReward +
                    (userstake.amount * multiplier * pool.rewardPercent) /
                    (perMonthsec * 100);
                userPending = ((reward * referalEarnings[0]) / 100);
            }
        } else {
            multiplier = getMultiplier(
                userstake.lastWithdrawRewards,
            ...
            if (multiplier > 0) {
                uint256 reward = userstake.mineReward +
                    (userstake.amount * multiplier * pool.rewardPercent) /
                    (perMonthsec * 100);
                userPending = ((reward * referalEarnings[0]) / 100);
            }
    ...
    function distributeEarnings(
        ...
        UserInfo storage user = userInfo[_pid][userWithdraw];
        UserInfo storage userParent = userData[_user];
        StakePoolInfo storage pool = poolInfo[_pid];
        if (cnt < 10 && _user != 0x0000000000000000000000000000000000000000) {
            ...
            userParent.gainReward += amountShare;
            cnt++;
            distributeEarnings(userParent.parent, cnt, _pid, userWithdraw);
        }
    }
    ...
    function withdrawReferalReward() public {
        ...
        IERC20(BusdAddresss).transfer(msg.sender, totalRefrerRewards);
        parent.totalRefrerRewards = 0;
        parent.gainBusd = 0;
        emit withdraw(msg.sender, totalRefrerRewards, 6);
    }

    function withdrawLevelReferalReward() public {
        UserInfo storage parent = userData[msg.sender];
        uint256 totalRefrerRewards = parent.gainReward;
        require(totalRefrerRewards > 0, "Insufficient referal rewards.");
        IERC20(QbtAddresss).transfer(msg.sender, totalRefrerRewards);
        parent.gainReward = 0;
        emit withdraw(msg.sender, totalRefrerRewards, 7);
    }
}