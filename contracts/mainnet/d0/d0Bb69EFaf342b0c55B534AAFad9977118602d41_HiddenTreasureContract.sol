...
contract HiddenTreasureContract is Ownable {
    ...
    //Users can pay with BUSD to DROP a chest currently drop fee per chest is set to 1 USD
    function dropHiddentreasure(
        bool isToken,
        uint256 value,
        int256 latitude,
        int256 longitude,
        ...
        if (isToken) {
            ...
            require(msg.value == value, 'Did not receive BNB');
        }
        chestObjects[idNumber].trueLocationLat = keccak256(
            abi.encodePacked(latitude)
        );
        chestObjects[idNumber].trueLocationLong = keccak256(
            abi.encodePacked(longitude)
        );
        chestObjects[idNumber].isToken = isToken;
        chestObjects[idNumber].value = value;
        chestObjects[idNumber].idNumber = idNumber;
        chestObjects[idNumber].tokenAddress = _tokenAddress;
        chestObjects[idNumber].info = info;
        ...
        onlyWriter
    {
        for (uint256 i = from; i < to; i++) {
            require(
                chestObjects[activeChestIds[i]].status == 4 &&
                    chestObjects[activeChestIds[i]].protestDeadline <
                    block.number
            );
            activeChestIds[i] = activeChestIds[activeChestIds.length - 1];
            activeChestIds.pop();
