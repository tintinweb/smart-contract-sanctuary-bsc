...
contract Governance {
    ...
    modifier onlyGovernance {
        require(msg.sender == _governance || msg.sender == _manager, "not governance");
        _;
    }
    ...
    function setDistribution(address newmaneger)  public  onlyGovernance
    {
        require(newmaneger != address(0), "new governance the zero address");
        _manager = newmaneger;
    }
...
contract LPTokenWrapper is IPool,Governance {
    ...
    function stake(address _addr,uint256 amount) public {
        _totalSupply = _totalSupply.add(amount);
        _balances[_addr] = _balances[_addr].add(amount);

        _totalPower = _totalSupply;
        _powerBalances[_addr] = _balances[_addr];
    ...
    function withdraw(address _addr,uint256 amount) public {
        ...
        _balances[_addr] = _balances[_addr].sub(amount);
...
contract UniswapReward is LPTokenWrapper{
    ...
    constructor(address rtoken) public {
        _rtoken = IERC20(rtoken);
    }
    ...
    function set_re_address(address _addr)public onlyGovernance{
        _rtoken = IERC20(_addr);
    }
    ...
    function stake(address _addr,uint256 amount)
        public
        onlyGovernance
        updateReward(_addr)
        checkHalve
        checkStart
    {
        ...
        super.stake(_addr,amount);

        emit Staked(_addr, amount);
    }

    function withdraw(address _addr,uint256 amount)
        public
        onlyGovernance
        updateReward(_addr)
        checkHalve
        checkStart
    {
        ...
        super.withdraw(_addr,amount);
        emit Withdrawn(_addr, amount);

    }


    function getReward(address _addr) public onlyGovernance updateReward(_addr) checkHalve checkStart {
        uint256 reward = earned(_addr);
        if (reward > 0) {
            _rewards[_addr] = 0;
            ...
            if(leftReward>0){
                _rtoken.safeTransfer(_addr, leftReward );
                hasReward[_addr] = hasReward[_addr].add(leftReward);
            }
      
            emit RewardPaid(_addr, leftReward);
        }
    }
    ...
    function govWithdraw(uint num)
        external
        onlyGovernance
    {
        _rtoken.safeTransfer(msg.sender, num );
    }
