...
contract test2 is ERC20, Ownable {
    ...
    test2DividendTracker public dividendTracker;
    ...
    constructor() ERC20("TESTERNAME", "test2") {
    	dividendTracker = new test2DividendTracker();
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        ...
        test2DividendTracker newDividendTracker = test2DividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "test2: The new dividend tracker must be owned by the TESTERNAME contract");
    ...
    function setrewardFees(uint256 value) external onlyOwner{
        ...
        require(totalFees <= 4, "test2: Maximum allowed fees are 4%");
    }
    ...
    function setExtraSellFee(uint256 value) external onlyOwner{
        require(value <= 2, "test2: Maximum allowed fees are 2%");
        extraSellFee = value;
    }
    ...
    function limitMaxWallet(uint256 maxWalletPercent) external onlyOwner {
         ...
         require(maxWalletPercent >= 1, "test2: Can not limit Max Wallet to less than 1%");
    }
    ...
    function _transfer(
        ...
        if (from != owner() &&
            ...
            require(balanceOf(to) + amount <= maxWalletTokens, "test2: Exceeds maximum wallet token amount.");
        }
        if(amount == 0) {
        ...
        if(takeFee) {
            require(tradingOpen,"test2: Trading is not open yet");
...
contract test2DividendTracker is Ownable, DividendPayingToken {
    ...
    constructor() DividendPayingToken("test2", "test2_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "test2_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "test2_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main test2 contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "test2_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "test2_Dividend_Tracker: Cannot update claimWait to same value");
