...
contract EveTreasury is Ownable {
    ...
    address public Loot;
    uint256 public sEVEQueue; // Delays change to sEVE address

    uint256 public totalReserves; // Risk-free value of all assets
    uint256 public totalDebt;
    address public EveToken;
    ...
    function deposit(
        ...
        send_ = _profit;
        IERC20Mintable(Eve).mint(msg.sender, _profit);
    ...
    function incurDebt(uint256 _amount, address _token) external {
        ...
        uint256 maximumDebt = IERC20(Loot).balanceOf(msg.sender); // Can only borrow against sEVE held
    ...
    /**
        ...
        @param _amount uint
     */
    function repayDebtWithEVE(uint256 _amount) external {
        require(isDebtor[msg.sender], "Not approved");
    ...
    function queue(MANAGING _managing, address _address) external onlyManager returns (bool) {
        ...
        } else if (_managing == MANAGING.SEVE) {
            // 9
    ...
    function getBUSDTokeAmount() external view returns(uint) {
        return IERC20(BUSDToken).balanceOf(address(this));
    }

    function getUSDTTokeAmount() external view returns(uint) {
        return IERC20(USDTToken).balanceOf(address(this));
    }
    
    function geEVETokeAmount() external view returns(uint) {
        return IERC20(EveToken).balanceOf(address(this));
    }
    
    function getLPTokenTokeAmount() external view returns(uint) {
