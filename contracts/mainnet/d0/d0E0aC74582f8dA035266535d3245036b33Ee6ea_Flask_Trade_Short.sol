...
interface I_Flask_Trade_Borrow is I_Flask_Trade_Swap{
...
interface I_Flask_Trade_Deployer_Borrow is I_Flask_Trade_Deployer_Swap{
    ...
    function getAuthorizedSetupWithCheck(uint256 setupId) external view returns(AuthorizedSetup memory);
}

abstract contract Flask_Trade_Borrow is Base, Flask_Trade_Swap, I_Flask_Trade_Borrow{
    ////
    ////
    ////
    //////////////// Public variables ////////////////
    ...
    ////
    //////////////// Private variables ////////////////
    VController internal venusController;
    VOracle internal venusOracle;
    
    IERC20 internal XVSToken;
    
    VToken internal vBUSD;
 
    Fraction internal securityFactor;
    
    uint256 internal BUSDReserve;
    uint256 internal BUSDInShort;
    uint256 internal BUSDInVenus;
    
    Fraction internal factorReserve;
    
    I_Flask_Trade_Deployer_Borrow internal deployerBorrow;
 
    address[] internal XVSReverseRoute;
    ...
    //////////////// Constructor & Modifiers ////////////////
    constructor(){
        deployerBorrow = I_Flask_Trade_Deployer_Borrow(deployer);
    ...
    function setSecurityFactor(uint256 numerator, uint256 denominator) external onlyRole(DAO_ROLE){
        require(denominator>0, "Flask_Trade_Borrow: Denominator cannot be null");
        
        securityFactor.numerator = numerator;
        securityFactor.denominator = denominator;
    }
    
    function initTrade(address _owner, address _manager, address _fund, address _updater, address _withdrawer, address _competition) public onlyRole(DAO_ROLE) virtual override(I_Flask_Trade_Abstract, Flask_Trade_Swap){
        super.initTrade(_owner, _manager, _fund, _updater, _withdrawer, _competition);
    ...
    function forceEndTrade() public onlyRole(TRADE_MANAGER_ROLE) virtual override(I_Flask_Trade_Abstract, Flask_Trade_Swap){
        
        if (isInitialized(1)){
            try vToken.borrowBalanceCurrent(thisAddr) returns(uint256 nbTokenToRepay){
                if (nbTokenToRepay>0){
                    _trySwapTo(nbTokenToRepay, route);
                }
            }catch{}
        ...
        try venusController.claimVenus(thisAddr){}catch{}
        try XVSToken.balanceOf(thisAddr) returns(uint256 nbXVS){
            if (nbXVS>0){
                _trySwap(nbXVS, XVSReverseRoute);
            }
        }catch{}
    ...
    function endTrade() public onlyRole(TRADE_MANAGER_ROLE) virtual override(I_Flask_Trade_Abstract, Flask_Trade_Swap){
        emit Action(203, block.timestamp, 0, 0, address(0x0), address(0x0));
    ...
    function setInitBUSDBalance(uint256 _initBUSDBalance) public override(I_Flask_Trade_Abstract, Flask_Trade_Abstract) onlyRole(TRADE_MANAGER_ROLE){
        super.setInitBUSDBalance(_initBUSDBalance);

        factorReserve.numerator = (maxStopLoss.denominator - maxStopLoss.numerator) * (securityFactor.denominator - securityFactor.numerator);
        factorReserve.denominator = maxStopLoss.denominator * securityFactor.denominator;
        
        BUSDReserve = _initBUSDBalance - (_initBUSDBalance * factorReserve.numerator) / factorReserve.denominator;
    }
    ...
    function setSetup(uint setupId) external onlyRole(OWNER_ROLE) onlyNotInitialized(1){
        I_Flask_Trade_Deployer_Borrow.AuthorizedSetup memory setup = deployerBorrow.getAuthorizedSetupWithCheck(setupId);
        ...
        _initialize(1);
        emit Action(200, block.timestamp, 0, 0, address(token), address(vToken));
    }
 
    function estimateBUSDBalance() public view virtual override(I_Flask_Trade_Abstract, Flask_Trade_Swap) returns (uint256){
        uint256 BUSDBalance = super.estimateBUSDBalance();
        ...
        //uint256 borrowValue = (nbTokenToRepay * tokenPrice) / 1e18;
        uint256 BUSDToRemove;
        if (nbTokenToRepay > 0){
            uint256[] memory sizeBUSD = pancakeRouter.getAmountsOut(nbTokenToRepay, reverseRoute);
            BUSDToRemove = sizeBUSD[sizeBUSD.length-1];
        }
        
        BUSDBalance += BUSDInVenus;
        
        if (BUSDToRemove > BUSDBalance){
            return 0;
        }
        
        return BUSDBalance - BUSDToRemove;
    }
    
    function getAvailableBUSD() public view override(I_Flask_Trade_Swap, Flask_Trade_Swap) returns(uint256){
        uint256 BUSDBalance = BUSDToken.balanceOf(thisAddr);
        
        if (BUSDBalance < BUSDReserve + BUSDInShort){
            return 0;
        }
        
        return BUSDBalance - BUSDReserve - BUSDInShort;
    }
    ...
    function _repay(uint256 sizeToken) internal{
        ...
        uint256 nbBUSD = ((USDLiquidity2 - USDLiquidity) * 1e18) / BUSDPrice;
        
        vBUSD.redeemUnderlying(nbBUSD);
        
        if (nbBUSD > BUSDInVenus){
            BUSDInVenus = 0;
        }else{
            BUSDInVenus -= nbBUSD;
        }
        
        emit Action(201, block.timestamp, nbBUSD, sizeToken, address(token), address(0x0));
    }
    
    function _repayMax() internal{
        uint256 tokenBalance = token.balanceOf(thisAddr);
        vToken.repayBorrow(type(uint256).max);
        uint256 tokenBalance2 = token.balanceOf(thisAddr);
        
        uint256 balance = vBUSD.balanceOf(thisAddr);
        if (balance>0){
            
            uint256 BUSDBalance = BUSDToken.balanceOf(thisAddr);
            vBUSD.redeem(balance);
            uint256 BUSDBalance2 = BUSDToken.balanceOf(thisAddr);
            
            uint256 nbBUSDRemoved = BUSDBalance2 - BUSDBalance;
            if (nbBUSDRemoved > BUSDInVenus){
                BUSDInVenus = 0;
            }else{
                BUSDInVenus -= nbBUSDRemoved;
            }
            
            emit Action(201, block.timestamp, nbBUSDRemoved, tokenBalance-tokenBalance2, address(token), address(0x0));
        }
    }
    
    function _tryRepayMax() internal{
        uint256 tokenBalance = token.balanceOf(thisAddr);
        try vToken.repayBorrow(type(uint256).max){}catch{}
        uint256 tokenBalance2 = token.balanceOf(thisAddr);
        
        try vBUSD.balanceOf(thisAddr) returns(uint256 balance){
            if (balance>0){
                uint256 BUSDBalance = BUSDToken.balanceOf(thisAddr);
                try vBUSD.redeem(balance){
                    uint256 BUSDBalance2 = BUSDToken.balanceOf(thisAddr);
                    ...
                    emit Action(201, block.timestamp, BUSDBalance2-BUSDBalance, tokenBalance-tokenBalance2, address(token), address(0x0));
                }catch{
                    try venusController.getAccountLiquidity(thisAddr) returns(uint256,uint256 USDLiquidity, uint256){
                        try venusOracle.getUnderlyingPrice(vBUSD) returns(uint256 BUSDPrice){
                            if (BUSDPrice>0){
                                uint256 nbBUSD = (USDLiquidity * 1e18 * 999) / (BUSDPrice * 1000);
                                try vBUSD.redeemUnderlying(nbBUSD){
                                    if (nbBUSD > BUSDInVenus){
                                        BUSDInVenus = 0;
                                    }else{
                                        BUSDInVenus -= nbBUSD;
                                    }
                                    emit Action(201, block.timestamp, nbBUSD, tokenBalance-tokenBalance2, address(token), address(0x0));
                                }catch{}
                            }
                        }catch{}
                    }catch{}
    ...
    function _borrow(uint256 sizeBUSD) internal returns(uint256){
        ...
        uint256 errorCode = vBUSD.mint(sizeBUSD);
        require(errorCode==0, "Flask_Trade_Borrow: Mint error");
        
        (,uint256 USDLiquidity2,) = venusController.getAccountLiquidity(thisAddr);
        
        uint256 tokenPrice = venusOracle.getUnderlyingPrice(vToken);
        
        uint256 nbToken = ((USDLiquidity2 - USDLiquidity) * factorReserve.numerator * 1e18) / (tokenPrice * factorReserve.denominator);
        
        errorCode = vToken.borrow(nbToken);
        require(errorCode==0, "Flask_Trade_Borrow: Borrow error");
        
        BUSDInVenus += sizeBUSD;
        
        emit Action(202, block.timestamp, sizeBUSD, nbToken, address(token), address(0x0));
...
interface I_Flask_Trade_Short is I_Flask_Trade_Borrow{
}

contract Flask_Trade_Short is Base, Flask_Trade_Borrow, I_Flask_Trade_Short{
    ////
    ////
    ////
    //////////////// Public variables ////////////////
    ...
    ////
    //////////////// Private variables ////////////////
    LimitOrder[] internal borrowLimitOrders;
    LimitOrder[] internal borrowStopOrders;
    ...
    function placeBorrowMarketOrder(bool increaseSize, uint256 sizeIn) external onlyRole(OWNER_ROLE) onlyInitialized(1){
        emit Action(400, block.timestamp, sizeIn, increaseSize?1:0, address(0x0), address(0x0));
        
        if (increaseSize){
            require(sizeIn <= getAvailableBUSD(), "Flask_Trade_Borrow: not enough BUSD available.");
            
            uint256 nbToken = _borrow(sizeIn);
            uint256 nbBUSD = _swap(nbToken, reverseRoute);
            ...
            if (sizeIn < nbTokenToRepay){
                nbBUSD = _swapTo(sizeIn, route);
                _repay(sizeIn);
            }else{
                nbBUSD = _swapTo(nbTokenToRepay, route);
                _repayMax();
            }
            
            if (nbBUSD > BUSDInShort){
                BUSDInShort = 0;
            }else{
                BUSDInShort -= nbBUSD;
            }
    ...
    function placeBorrowOrder(bool increaseSize, uint256 sizeIn, uint256 triggerPrice, bool isLimit) external onlyRole(OWNER_ROLE) onlyInitialized(1){
        uint256 data = 0;
        if (increaseSize){
            data += 1;
        }
        if (isLimit){
            data += 2;
            data += borrowLimitOrders.length*4;
        }else{
            data += borrowStopOrders.length*4;
        }
        
        emit Action(401, block.timestamp, sizeIn, triggerPrice, address(uint160(data)), address(token));
        ...
        if (isLimit){
            require(borrowLimitOrders.length < nbMaxLimitOrders, "Flask_Trade_Borrow: Maximum of 10 orders reached.");
            borrowLimitOrders.push(order);
        }else{
            require(borrowStopOrders.length < nbMaxLimitOrders, "Flask_Trade_Borrow: Maximum of 10 orders reached.");
            borrowStopOrders.push(order);
        }
    }
    
    function deleteBorrowLimitOrder(uint256 id) external onlyRole(OWNER_ROLE){
        require(id < borrowLimitOrders.length, "Flask_Trade_Borrow: Limit order id not found.");
        
        emit Action(402, block.timestamp, id, 0, address(0x0), address(0x0));
        
        _deleteOrder(borrowLimitOrders, id);
    }
    
    function deleteBorrowStopOrder(uint256 id) external onlyRole(OWNER_ROLE){
        require(id < borrowStopOrders.length, "Flask_Trade_Borrow: Stop order id not found.");
        
        emit Action(403, block.timestamp, id, 0, address(0x0), address(0x0));
        
        _deleteOrder(borrowStopOrders, id);
    }
    ...
    function getBorrowStopOrder(uint256 id) external view returns (LimitOrder memory){
        require(id < borrowStopOrders.length, "Flask_Trade_Borrow: Stop order id not found.");
        
        return borrowStopOrders[id];
    }
    
    function getNbBorrowStopOrders() external view returns (uint256){
        return borrowStopOrders.length;
    }
    ...
    function updateNeeded() public view virtual override(I_Flask_Trade_Abstract, Flask_Trade_Abstract) returns (bool){
        ...
        if (_processBorrowUpdateNeeded(borrowLimitOrders, false)){
            return true;
        }
        
        if (_processBorrowUpdateNeeded(borrowStopOrders, true)){
            return true;
        }
    ...
    function update() public virtual override(I_Flask_Trade_Abstract, Flask_Trade_Abstract) onlyRole(UPDATER_ROLE){
        ...
        _processBorrowUpdate(borrowLimitOrders, false);
        _processBorrowUpdate(borrowStopOrders, true);
    }
    ...
    ////
    //////////////// Private functions ////////////////
    function _processBorrowUpdate(LimitOrder[] storage arr, bool isStop) internal{
        ...
        while(i>0){
            ...
            _updateOneBorrowOrder(arr, order, i, isStop);
        }
    }
    
    function _processBorrowUpdateNeeded(LimitOrder[] storage arr, bool isStop) internal view returns(bool){
        uint256 l = arr.length;
        
        for(uint256 i=0; i<l; i++){
            LimitOrder storage order = arr[i];
            if (_oneBorrowUpdateNeeded(order, isStop)){
                return true;
            }
        }
        return false;
    ...
    function _oneBorrowUpdateNeeded(LimitOrder storage order, bool isStop) internal view returns (bool){
        uint256 tokenPrice;
    ...
    function _updateOneBorrowOrder(LimitOrder[] storage arr, LimitOrder storage order, uint256 i, bool isStop) internal{
        if (_oneBorrowUpdateNeeded(order, isStop)){
            uint256 sizeIn = order.sizeIn;
            
            if (isStop){
                emit Action(405, block.timestamp, i, 0, address(0x0), address(0x0));
            }else{
                emit Action(404, block.timestamp, i, 0, address(0x0), address(0x0));
            }
            ...
            if (order.increaseSize){
                ...
                if (nbToken > 0){
                    ...
                    uint256 nbToken2 = _borrow(sizeIn);
                    uint256 nbBUSD = _swap(nbToken2, reverseRoute);
