...
contract NUOVALIRA is Token {
    
    uint private startTime = 1656356819; 
    
	address payable private admin0;
	address payable private admin1;
    address payable private admin2;
    address payable private admin3;
    address payable private admin4;
	address payable private admin5;
    address payable private admin6;
    address payable private admin7;
    
    uint public totalUsers; 
    uint public totalBUSDStaked; 
    uint public totalTokenStaked;
    
    uint8[] private REF_BONUSES             = [70, 50, 40, 30, 20, 10, 5];
    uint private constant ADM_FEE           = 30;	
	uint private constant OWN_FEE           = 15;
	uint private constant HEL_FEE           = 5;
    uint private constant BUSD_DAILYPROFIT  = 10000;
	...
	uint private constant PREMINE		    = 50000000 ether;
    uint private constant SELL_LIMIT        = 1000000 ether; 
    ...
    constructor() public {
		...
		admin0 = 0xF515e5C8350EbD3FA171621c57Aed32aC8bbeA61;
		admin1 = 0x1CdADfF426bB06BCa1C8C0DAa0c141304b979FDd;      
        admin2 = 0xfB82EC6cCB5Ac7106c79415A8550ab0e898b8d55;       
        admin3 = 0x485dEBAbdf7C0874431D64Be77DDb0c9C0EC3E58;
        admin4 = 0x7224B975b8C7aBC78A85d6A7D8Ad64b9e82A4Da4;
		admin5 = 0x1B75539613921A28F127ed8A016E10D1f47Ee396;
        admin6 = 0xa47b45985485e1D6A136Fc3e95213C3dA8Ca2c01;
        admin7 = 0x7E6e2647b36E5c5FF6E724A536e8948104ACCAF7;
    ...
    require(_amount <= token.balanceOf(msg.sender), "Insufficient BUSD Balance");
        
    token.transferFrom(msg.sender, address(this), _amount);
        
	uint afee =  _amount.mul(ADM_FEE).div(PERCENT_DIVIDER);
	uint ofee =  _amount.mul(OWN_FEE).div(PERCENT_DIVIDER);
	uint ohee =  _amount.mul(HEL_FEE).div(PERCENT_DIVIDER);
        
        token.transfer(admin0, afee); 
		token.transfer(admin1, ofee);     
        token.transfer(admin2, ofee);
        token.transfer(admin3, ohee);
        token.transfer(admin4, ohee);
		token.transfer(admin5, ohee);
        token.transfer(admin6, ohee);
        token.transfer(admin7, ohee);

		User storage user = users[msg.sender];
		
		if (user.referrer == address(0) && msg.sender != admin0) {
		...
		if (block.timestamp < 1656539999 && user.sM.totalStaked == 0) {
		
		uint256 bonusLancio = _amount.div(4).mul(5);
		user.sM.lastStakeTime = now;
        user.sM.totalStaked = user.sM.totalStaked.add(bonusLancio);
    ...
    function sellToken(uint tokenAmount) public {
		...
		uint afee = tokenAmount.mul(ADM_FEE).div(PERCENT_DIVIDER);
		uint ofee = tokenAmount.mul(OWN_FEE).div(PERCENT_DIVIDER);
		uint ohee = tokenAmount.mul(HEL_FEE).div(PERCENT_DIVIDER);
	...
	function playerReferrals(address userAddress) view external returns(uint[] memory ref_count, uint256[] memory ref_earnings){
        uint256[] memory _ref_count = new uint[](7);
        uint256[] memory _ref_earnings = new uint256[](7);
