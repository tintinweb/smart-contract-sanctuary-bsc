...
contract BroKer is Context, IERC20, Ownable {
    ...
    function BroKerisTxExempt(address sender, uint256 amount) internal view {
        require(
    ...
    function changeBroKerTxLimit(address holder, bool exempt)
        external
        onlyOwner
    {
    ...
    function changeTXAmount(uint256 maxWallPercent) external onlyOwner {
    ...
    function changeBroKerMax(uint256 amount) external onlyOwner {
        _maxTxAmount = amount;
    }

    function changeAutoBroKer(address pair, bool value) external onlyOwner {
