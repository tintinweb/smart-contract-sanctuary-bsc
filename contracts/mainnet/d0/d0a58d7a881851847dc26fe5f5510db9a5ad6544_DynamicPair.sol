    ...
    */
    ...
    function getAmountOutAndFee(uint amountIn, address tokenIn, address tokenOut) public view returns(uint amountOut, uint fee) {
        ...
        if (tokenIn < tokenOut) {
            ...
            fee = uint(9999).sub(amountOut.mul(10000).mul(2**56)/(balanceIn.mul(ma)));

        } else {

            fee = uint(9999).sub(amountOut.mul(10000).mul(ma)/(balanceIn.mul(2**56)));
    ...
    function getAmountInAndFee(uint amountOut, address tokenIn, address tokenOut) public view returns(uint amountIn, uint fee) {
        ...
        if (tokenIn < tokenOut) {
            ...
            fee = uint(9999).sub(balanceOut.mul(10000 * 2**56)/(balanceIn.mul(ma)));

        } else {

            fee = uint(9999).sub(balanceOut.mul(10000 * ma)/(balanceIn.mul(2**56)));
