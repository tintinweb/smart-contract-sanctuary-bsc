...
contract TaxCollector is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    modifier onlyWhitelist() {
        require(msg.sender == tokenAddress || msg.sender == whitelistedUser , "Not Whitelisted");
        _;
    }
...
// all initialize parameters are mandatory
    ...
    function updateWhitelistAddress(address _whitelistedUser) public onlyOwner {
        whitelistedUser = _whitelistedUser;
    }
    function updateInteractions() public onlyOwner {
        token = IERC20Upgradeable(tokenAddress);
        router = IPancakeRouter02(routerAddress);
    }
    ...
    uint256 totalBalance = token.balanceOf(address(this));
        ...
        _swapWholeBalanace(totalBalance);
        _sendAmount();
        return true;
    }

    function _swapWholeBalanace(uint256 amount) internal {
   ...
   function _sendAmount() public  {
      uint256 amountDivided = address(this).balance / 2;
       payable(sellTaxAddress).transfer(amountDivided);
       payable(buyTaxAddress).transfer(amountDivided);
   }
}
