...
contract ECOLock is Ownable {
    ...
    uint256 public unlockPercent = 0; // 0 its 0.00%
    ...
    address public teamAddress = 0x5E93614edB0Ac0D528d0e5f7bdc8169881Cce892;
    ...
    function claim() public {
        ...
        if (block.timestamp > nextUnlock) {
            level += 1;
            if (level == 1) unlockPercent = 232;
            if (level == 2) unlockPercent = 222;
            avalableToClaimGlobal += unlockPercent;
            if (avalableToClaimGlobal == 10000) {
        ...
        uint256 _amount = distributionAmount * _avalableToClaim / 10000;
