...
/*
A vault that helps users stake in SPY farms and pools more simply.
Supporting auto compound in Single Staking Pool.
*/

contract BNBSpyVault is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    ITreasury public vaultReferralTreasury;
    ...
    function initialize(
        ...
        vaultReferralTreasury = ITreasury(_vaultReferralTreasury);
        percentFeeForCompounding = 10; //default 1%
    }

    function canCompound() public view returns (bool) {
        return masterchef.canHarvest(farmPid, address(this)) && pendingSpyNextCompound() > 0;
    }
    ...
    function totalPoolPendingRewards() public view returns (uint256) {
        ...
        return masterchef.pendingSpy(farmPid, address(this)).add(rewardLockedUp);
    }
    ...
    function getSwappingPair() internal view returns (IUniswapV2Pair) {
        return IUniswapV2Pair(
            factory.getPair(address(spy), address(weth))
        );
    }

    function updateReferralTreasury(ITreasury _treasury) external onlyOwner {
        vaultReferralTreasury = _treasury;
    }

    function updateSpyReferral(ISpyReferral _spyReferral) external onlyOwner {
    ...
    function approve() public {
        spy.approve(address(masterchef), MAX_INT);
        spy.approve(address(router), MAX_INT);
    ...
    function getReserveInAmount1ByLP(uint256 lp) public view returns (uint256 amount) {
        ...
        uint256 balance0 = spy.balanceOf(address(pair));
    ...
    * lpNeeded returns 10
    * once remove liquidity, 10 LP will get back 500 BUSD and an amount in SPY corresponding to 500 BUSD
    */
    ...
    * exp. amount = 1000 SPY
    * lpNeeded returns 10
    * once remove liquidity, 10 LP will get back 500 SPY and an amount in BUSD corresponding to 500 SPY
    */
    ...
    function deposit(address referrer) external payable updateReward(msg.sender) nonReentrant noCallFromContract waitForCompound {
        ...
        uint256 spyOut = swapBnbToSpy(amountToSwap);
        uint256 amountLeft = amount.sub(amountToSwap);
        (,uint256 amountBNB,uint256 liquidityAmount) = _addLiquidity(spyOut, amountLeft);
    ...
    function depositTokenPair(uint256 amountSpy, address referrer) external payable updateReward(msg.sender) nonReentrant noCallFromContract waitForCompound {
        uint256 amountBNB = msg.value;
        uint256 balanceOfSpyBeforeTrasnfer = spy.balanceOf(address(this));
        spy.transferFrom(msg.sender, address(this), amountSpy);
        uint256 amountSpyReceived = spy.balanceOf(address(this)) - balanceOfSpyBeforeTrasnfer;
        // note spyAdded is might reduced by ~1%
        (uint256 spyAdded, uint256 bnbAdded, uint256 liquidityAmount) = _addLiquidity(amountSpyReceived, amountBNB);
        // transfer back amount that didn't add to the pool
        if(amountSpyReceived.mul(99).div(100) > spyAdded){
            uint256 amountLeft = amountSpyReceived.mul(99).div(100) - spyAdded;
            if(spy.balanceOf(address(this)) >= amountLeft)
                spy.transfer(msg.sender, amountLeft);
        }
    ...
    function _depositLP(address account, uint256 liquidityAmount, address referrer) internal {
        if (
            address(spyReferral) != address(0) &&
            referrer != address(0) &&
            referrer != account
        ) {
            spyReferral.recordReferral(account, referrer);
        }
        //stake in farms
    ...
    function withdraw(uint256 amount, bool isReceiveBnb) external updateReward(msg.sender) nonReentrant noCallFromContract waitForCompound {
        ...
        if(isReceiveBnb){
            ...
            payable(msg.sender).transfer(swapSpyToBnb(amountA).add(amountB));
        }else{
            spy.transfer(msg.sender, amountA);
    ...
    function harvest(bool isReceiveBnb) external updateReward(msg.sender) nonReentrant noCallFromContract waitForCompound {
        ...
        if (reward > 0) {
            ...
            (uint256 amountSpy,uint256 amountBNB) = removeLiquidity(lpNeeded);
            if(isReceiveBnb) {
                payable(msg.sender).transfer(swapSpyToBnb(amountSpy).add(amountBNB));
            }else{
                spy.transfer(msg.sender, swapBnbToSpy(amountBNB).add(amountSpy));
            }
    ...
    function compound() external updateReward(address(0)) nonReentrant {
        ...
        if (_canCompound) {
            ...
            uint256 amountCollected = spy.balanceOf(address(this));
            ...
            (uint256 res0,) = getPairReserves();
            uint256 spyAmountToSwap = SwapMath.calculateSwapInAmount(res0, rewardForPool);
            uint256 busdOut = swapSpyToBnb(spyAmountToSwap);
            (,, uint256 liquidityAmount) = _addLiquidity(rewardForPool.sub(spyAmountToSwap), busdOut);
            depositStakingPool(liquidityAmount);
            spy.transfer(msg.sender, rewardForCaller);
    ...
    function pendingSpyNextCompound() public view returns (uint256){
        (,,uint256 rewardLockedUp,) = masterchef.userInfo(farmPid, address(this));
        return masterchef.pendingSpy(farmPid, address(this)).add(rewardLockedUp);
    }

    function rewardForCompounder() external view returns (uint256){
        return pendingSpyNextCompound().mul(percentFeeForCompounding).div(1000);
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                if(vaultReferralTreasury.payReward(referrer, commissionAmount))
    ...
    function swapBnbToSpy(uint256 amountToSwap) internal returns (uint256 amountOut) {
        uint256 spyBalanceBefore = spy.balanceOf(address(this));
        ...
        amountOut = spy.balanceOf(address(this)).sub(spyBalanceBefore);
    }

    function swapSpyToBnb(uint256 amountToSwap) internal returns (uint256 amountOut) {
    ...
    function removeLiquidity(uint256 lpAmount) internal returns (uint256 amountSpy, uint256 amountBNB){
        uint256 spyBalanceBefore = spy.balanceOf(address(this));
        ...
        amountSpy = spy.balanceOf(address(this)).sub(spyBalanceBefore);
    }
    ...
    function getPairReserves() internal view returns (uint reserveA, uint reserveB) {
        address token0 = address(spy) < address(weth) ? address(spy) : address(weth);
        (uint reserve0, uint reserve1, ) = getSwappingPair().getReserves();
        (reserveA, reserveB) = address(spy) == token0 ? (reserve0, reserve1) : (reserve1, reserve0);
    }


    function getBnbToSpyRoute() private view returns (address[] memory paths) {
        paths = new address[](2);
        paths[0] = address(weth);
        paths[1] = address(spy);
    }

    function getSpyToBnbRoute() private view returns (address[] memory paths) {
        paths = new address[](2);
        paths[0] = address(spy);
...
interface IMasterChef {
    function pendingSpy(uint256 _pid, address _user)
    external
    view
    returns (uint256);
...
interface ITreasury {
    function payReward(address _address, uint256 _rewards) external returns (bool);
}
