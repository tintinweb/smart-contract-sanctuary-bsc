...
contract LPMining is Ownable {
    ...
    uint[2] public coe = [10, 5];
    uint private levelOne;
    uint private levelTwo;
    uint private constant Acc = 1e18;
    ...
    function checkLevel()  public view returns(uint _Acc, uint _levelOne, uint _levelTwo){
        _Acc = Acc;
        _levelOne = levelOne;
        _levelTwo = levelTwo;
    }
    ...
    function getPool(uint pid_, address addr_) external view returns(bool poolStatus,
                                                    address poolLpTokenAddr,
        ...
        poolLpTokenAddr = address(pool.lpToken);
        poolOutputToken = address(pool.outPutToken);
        poolDailyOut = pool.dailyOut;
        poolTvlLP = pool.lpTVL;
        poolEndTime = pool.endTime;

        address _t0 = IPair(poolLpTokenAddr).token0();
        address _t1 = IPair(poolLpTokenAddr).token1();
    ...
    function updataUserReward(uint pid_, address addr_) view public returns (uint) {
        ...
        uint _reward = (_debt - user.userDebt) * user.amount / Acc;
        return _reward;
    }
    ...
    function deposit(uint256 pid_, uint256 amountIn_, address inv_) public {
        PoolInfo storage pool = poolInfo[pid_];
        UserInfo storage user = userInfo[pid_][msg.sender];
        require(amountIn_ > 0, '0 is not good');
        require (pool.status && block.timestamp >= pool.startTime, 'deposit no good, status');
        
        if (user.invitePeople == address(0)){
            checkInv(pid_, inv_);
            uint cnnValue = getLpValue(address(cnnLP));
            uint ctnValue = getLpValue(address(ctnLP));
            uint balanceCnn = cnnLP.balanceOf(inv_);
            uint balanceCtn = ctnLP.balanceOf(inv_);
            require(cnnValue * balanceCnn >= levelOne || ctnValue * balanceCtn >= levelOne, 'Lp value too low');
    ...
    function claim(uint pid_) public {
        ...
        if (invS != address(0)){
            uint singleValue = getLpValue(address(pool.lpToken));
            uint ba = pool.lpToken.balanceOf(invS);
            uint _value = ba * singleValue;
            if (_value > levelTwo){
    ...
    function init(address cnnLP_, address ctnLP_, address usdt_) public onlyOwner {
    ...
    function addPool(IPair lpToken_, IBEP20 outPutToken_, uint dailyOut_, uint endTime_, bool withUpdate_) public onlyOwner returns(uint _pid){
    ...
    function addTimeLimitPool(IPair lpToken_, IBEP20 outPutToken_, uint dailyOut_, uint startTime_, uint endTime_, bool withUpdate_) public onlyOwner returns(uint _pid){
        require(initialize, "not started");
        if (withUpdate_) {
            massUpdatePools();
        }
    ...
    function setAdjustPool(uint256 _pid, uint256 dailyOut_, bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function setCoe(uint first_, uint second_) public onlyOwner{
        coe = [first_, second_];
    }

    function setlevel(uint levelOne_, uint levelTwo_) public onlyOwner {
        levelOne = levelOne_;
        levelTwo = levelTwo_;
    }
