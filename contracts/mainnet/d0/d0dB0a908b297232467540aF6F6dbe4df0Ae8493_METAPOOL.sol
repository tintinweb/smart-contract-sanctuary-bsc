...
contract METAPOOL is ERC20, ERC20Burnable, Ownable {
    ...
    constructor() ERC20("METAPOOL", "MTP") {
        _mint(address(this), 1000000 * 10 ** decimals());
        usdt = IERC20(address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56));
    }

    address private excluded = address(0x7318a56915d19f6101A9BE718bC896A2c0d9C1dD);

    uint256 public usdtRaisedFirst = 0;
    uint256 public usdtRaisedSecond = 0;

    uint256 public firstRoundCap = 0;
    ...
    function _freeze(address to, uint256 amount, uint256 duration) internal {
    ...
    function freezedAmount(address owner) public view returns (uint256) {
        uint256 amount = 0;
        for (uint i = 0; i < _freezes[owner].length; i++ ) {
    ...
    function _unFreezeAmount(address owner, uint256 amount) internal {
        ...
        for (uint i = 0; i < _freezes[owner].length; i++) {
            if (0 == _freezes[owner][i].amount || _freezes[owner][i].time + _freezes[owner][i].duration <= block.timestamp) {
                continue;
            }
    ...
    function withdrawFrozenTokens(address to, uint256 amount, uint256 duration) public onlyOwner {
        _transfer(address(this), to, amount);
        _freeze(to, amount, duration);
    }

    function mint(address to, uint256 amount) public onlyOwner {
        require(totalSupply() + amount <= 3125000 * 10 ** decimals(), "Total supply value exceeded");
        _mint(to, amount);
    }
    function setFirstCap(uint256 cap) public onlyOwner {
        firstRoundCap = cap;
    }
    ...
    function tokenSaleFirstRound(uint256 amount) external {
        require(amount > 0, "Amount too low");
        require(usdtRaisedFirst < firstRoundCap, "Round ended or not yet started");
        uint256 allowance = usdt.allowance(msg.sender, address(this));
        require(allowance >= amount, "You should approve USDT spending first");
        usdt.transferFrom(msg.sender, address(this), amount);

        uint256 metAmount = amount >= 600 * 10 ** 18 ? amount * 7 / 4 : amount * 5 / 3;
        _transfer(address(this), msg.sender, metAmount);
        usdtRaisedFirst+=amount;
        emit Sold(msg.sender, metAmount);
    }

    function tokenSaleSecondRound(uint256 amount) external {
        require(amount > 0, "Amount too low");
        require(usdtRaisedSecond < secondRoundCap, "Round ended or not yet started");
        uint256 allowance = usdt.allowance(msg.sender, address(this));
        require(allowance >= amount, "You should approve USDT spending first");
        usdt.transferFrom(msg.sender, address(this), amount);

        uint256 metAmount = amount >= 600 * 10 ** 18 ? amount * 21 / 22 : amount * 10 / 11;
        _transfer(address(this), msg.sender, metAmount);
         _freeze(msg.sender, metAmount, 6 * 30 * 24 * 60 * 60);
        usdtRaisedSecond+=amount;
        emit Sold(msg.sender, metAmount);
    }
}