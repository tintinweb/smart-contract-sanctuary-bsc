...
contract OmnichainRouterV2 is IOmnichainRouter, NonblockingLzApp, IAxelarExecutable, ReentrancyGuard {
    ...
    */
    ...
    function send(string memory dstChainName, address dstOA, bytes memory fnData, uint gas, address user, uint256 redirectFee) external override payable {
        bytes memory _payload = abi.encode(uint8(PacketType.SEND_TO_APTOS), dstChainName, addressToBytes32(dstOA), fnData, gas, addressToBytes32(msg.sender), chainName, addressToBytes32(user), redirectFee);
     ...
     */
    // TODO (Must) Adapter params as bytes function argument, not calculated on-chain: https://layerzero.gitbook.io/docs/evm-guides/advanced/relayer-adapter-parameters#airdrop
    function _getAdapter(uint gas) private pure returns (bytes memory) {
        ...
        uint16 v = 2;
        uint native = 0;
        return abi.encodePacked(v, gas, native, address(0));
    }
