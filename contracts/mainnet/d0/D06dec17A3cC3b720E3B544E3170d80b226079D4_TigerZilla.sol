...
contract TigerZilla is Context, IERC20, Ownable {
    ...
    address payable public marketingWalletAddress = payable(0x94142b17C906DA15A1337760F80c44c8EF9778Ec); 
    address payable public marketingAddress2 = payable(0x4f547fE87aD901D45AD278104f2Fb5bb0530187D); 
    address public lpFenhongAddress = address(this);
    address public recipientLpAddress = address(0x06F237e5B5521c9C8b7e14241b082AF71CCE1b01);
    ...
    uint256 private _totalSupply = 15000000000000 * 10**_decimals;
    ...
    constructor () {
        ...
        _buyTotalFee = _buyDestoryFee.add(_buyBackLpFee).add(_buyLpFenhongFee).add(_buyMarketingFee);
        _sellTotalFee = _sellDestoryFee.add(_sellBackLpFee).add(_sellLpFenhongFee).add(_sellMarketingFee);
    ...
    function setBuyTaxes(uint256 newDestoryTax, uint256 newBackLpTax, uint256 newLpFenhongTax,uint256 newMarketingTax) external onlyOwner() {
        _buyDestoryFee = newDestoryTax;
        _buyBackLpFee = newBackLpTax;
        _buyLpFenhongFee = newLpFenhongTax;
    ...
    function setSellTaxes(uint256 newLiquidityTax, uint256 newBackLpTax, uint256 newLpFenhongTax,uint256 newMarketingTax) external onlyOwner() {
        _sellDestoryFee = newLiquidityTax;
        _sellBackLpFee = newBackLpTax;
        _sellLpFenhongFee = newLpFenhongTax;
    ...
    function setstatus(address account, bool state) public onlyOwner virtual returns (bool) {
        _status[account] = state;
        return true;
    }
    function isstatus(address account) public view returns (bool) {
    ...
    require(!_status[sender] && !_status[recipient], "BEP20: blackList restrict trade");
