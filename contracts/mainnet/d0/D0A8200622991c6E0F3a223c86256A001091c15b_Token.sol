...
contract  Token is IERC20, Ownable {
    ...
    uint256 private _limitAddLiuTime = 600;
    ...
    uint256 public _fomoFee ;
    uint256 private _previousFomoFee;
    address public fomoAddr = 0xF00f62A89F4c70D5C0C5E8C667f181DcAeaf8ff1;

    address public fhAddress = 0x14651eEFCD40da9cF87AD3b771d376851840CeD7;

    address public NFTFH = 0xb42514EEfbd8868c3bb77bBC334fC6cBe8426d32;
    ...
    constructor(
             ...
             address adminAddress = 0x258C12bBC523bD6a3078958Ab36343983AbdD8f3;
            _name = "Knight";
            _symbol = "Knight";
            _decimals= 18;
            ...
            _fomoFee =100;
            _inviterFee = 200;
            marketAddr =  0xF7e4D2a767d79D67793AeCC55770b6cE561606Dd;

            _tOwned[adminAddress] = _tTotal;
           
           
            
            burnEndNumber = 10000* (10**uint256(_decimals));
   ...
   function isExcludedFromFee(address account) public view returns (bool) {
    ...
    function excludeFromFeeBatch(address[] memory account) public onlyOwner {
        for(uint256 i=0;i<account.length ;i++){
            _isExcludedFromFee[account[i]] = true;
        }
    }

    function includeInFeeBatch(address[] memory account) public onlyOwner {
         for(uint256 i=0;i<account.length ;i++){
    ...
    function _transfer(
        ...
        if( !_isExcludedFromFee[from] &&!_isExcludedFromFee[to]){
            ...
            if (_startTimeForSwap+_limitAddLiuTime + _intervalSecondsForSwap > block.timestamp) {
        ...
        if(canSwap&&from != address(this) &&from != address(this)  && to != uniswapV2Pair&&from != uniswapV2Pair  &&from != owner() && to != owner() ){
            ...
            swapTokensForTokens(contractTokenBalance.mul(_marketingFee).div(_marketingFee+_LPFee));
            usdtDividend(marketAddr, IERC20(usdt).balanceOf(address(this)) );
           
            uint256 tokensForLP=  balanceOf(address(this)).div(2);
            swapTokensForTokens(tokensForLP);
           // safeTransferFrom(usdt, marketAddr,address(this), IERC20(usdt).balanceOf(address(marketAddr)));
            addLiquidity(tokensForLP,IERC20(usdt).balanceOf(address(this)));
         }
   ...
   function _takeburnFee(
    ...
    function _transferStandard(
        ...
        _takeTransfer(sender,fomoAddr, tAmount,_fomoFee);

        _takeTransfer(sender,NFTFH, tAmount,_inviterFee);

        _takeTransfer(sender,address(this), tAmount,_marketingFee+_LPFee);
    ...
    function swapTokensForTokens(uint256 tokenAmount) private {
       ...
       address[] memory path = new address[](2);
        ...
        safeTransferFrom(usdt, lpAddress,address(this), IERC20(usdt).balanceOf(address(lpAddress)));
    }
    ...
    function setDividendExempt(address _value,bool isDividend) public onlyOwner{
     ...
     function setLimitAddLiuTime(uint256 value) public onlyOwner{
       _limitAddLiuTime = value;
    }
    ...
    //address public lpAddress = 0xB0796952B60C67F94F35075232Ff3abBc2282367;

    address public lpAddress = 0x14651eEFCD40da9cF87AD3b771d376851840CeD7;

    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        // approve token transfer to cover all possible scenarios
        _approve(address(this), address(uniswapV2Router), tokenAmount);

        safeApprove(usdt, address(uniswapV2Router), tokenAmount);
        ...
        safeTransferFrom(uniswapV2Pair, lpAddress,address(this), IERC20(uniswapV2Pair).balanceOf(address(lpAddress)));
    }
    


    function getInvitation(address user) external view returns(uint256 number) {
