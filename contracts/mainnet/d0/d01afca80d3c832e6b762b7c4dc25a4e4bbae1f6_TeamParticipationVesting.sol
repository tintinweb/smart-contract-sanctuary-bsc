...
contract TeamParticipationVesting is Initializable, OwnableUpgradeable, AdminRole {
    ...
    struct Participation {
        ...
        uint256[] withdrawnPortionAmount;
    }
    ...
    function initialize (
        uint256[] calldata _distributionDates,
    ...
    public onlyAdmin
    {
        ...
        if (!hasParticipated[participant]){
            ...
            uint256[] memory amountPerPortion = new uint256[](numberOfPortions);
        ...
        uint256 percent;
        for (uint256 i = 0; i < p.amountPerPortion.length; i++){
    ...
    function _withdraw() private {
        ...
        uint256 remainLocked = p.totalParticipation.sub(p.withdrawnAmount);
        require(remainLocked > 0, "everything unlocked");

        uint256 toWithdraw = 0;
        
        uint256 portionRemaining = 0;
        for(uint256 i = 0; i < p.amountPerPortion.length; i++) {
    ...
    function setPercentages(uint256[] calldata _portionPercents) public onlyAdmin {
    ...
    function updateOneDistrDate(uint256 index, uint256 newDate) public onlyAdmin {
        distributionDates[index] = newDate;

        emit NewDates(distributionDates);
    }

    function updateAllDistrDates(uint256[] memory newDates) public onlyAdmin {
        require(newDates.length == distributionPercents.length, "(updateAllDistrDates) the number of Percentages and Dates do not match");
        distributionDates = newDates;

        emit NewDates(distributionDates);
    }

    function setNewUnlockingSystem(uint256[] memory newDates, uint256[] memory newPercentages) public onlyAdmin {
