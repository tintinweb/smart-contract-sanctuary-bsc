...
contract RoyaltiesRegistry is IRoyaltiesProvider, OwnableUpgradeable {
    ...
    function getRoyalties(address token, uint256 tokenId, address buyer)
        external
        view
        override
        returns (LibPart.Part[] memory)
    {
        ...
        ) = royaltiesFromProvider(token, tokenId, buyer);
        if (result == false) resultRoyalties = royaltiesFromContract(token, tokenId, buyer);
        return resultRoyalties;
    }

    function getGeneralRoyalties(address token, address buyer, LibPart.Part[] memory existingRoyalties)
        external
        view
        override
        returns (LibPart.Part[] memory)
    {
        ...
        (
            bool result,
            LibPart.Part[] memory resultRoyalties
        ) = generalRoyaltiesFromProvider(token, buyer, existingRoyalties);
        if (result == false) resultRoyalties = generalRoyaltiesFromContract(token, buyer, existingRoyalties);
        return resultRoyalties;
    }

    function royaltiesFromContract(address token, uint256 tokenId, address buyer)
        internal
        view
        returns (LibPart.Part[] memory)
    {
        if (
            ...
            try IRoyalties(token).getRoyalties(tokenId, buyer) returns (
    ...
    function royaltiesFromProvider(address token, uint256 tokenId, address buyer)
        internal
        view
    ...
    function generalRoyaltiesFromContract(address token, address buyer, LibPart.Part[] memory existingRoyalties)
        internal
        view
        returns (LibPart.Part[] memory)
    {
        if (
            ...
            try IRoyalties(token).getGeneralRoyalties(buyer, existingRoyalties) returns (
                LibPart.Part[] memory result
            ) {
                return result;
            } catch {}
    ...
    function generalRoyaltiesFromProvider(address token, address buyer, LibPart.Part[] memory existingRoyalties)
        internal
        view
        returns (bool result, LibPart.Part[] memory royalties)
    {
        ...
        if (royaltiesProviders[token] != address(0x0)) {
            try
                IRoyaltiesProvider(royaltiesProviders[token]).getGeneralRoyalties(
                    token,
                    buyer,
                    existingRoyalties
                )
...
interface IRoyalties is IERC165Upgradeable {
    ...
    function getRoyalties(uint256 id, address buyer)
        external
        view
        returns (LibPart.Part[] memory);
