...
contract SUC is ERC20 {
	...
	address public WHT = address(0x38FB0879051BFB1218a0Af52110E6253Bbd77C04);  
	address public ROUTER = address(0x10ED43C718714eb63d5aA57B78B54704E256024E);
    constructor(address tokenOwner) ERC20("FAFA", "FAFA") { 
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(ROUTER);
        address _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
        .createPair(address(this), WHT);
		total = 100000;
        _approve(address(this), address(ROUTER), total.mul(10**5));
    ...
    function swapAndLiquify() public {
		uint256 swapAmount = ldxAmount.sub(ldxOverAmount);
		if(swapAmount > swapTokensAtAmount){
			ldxOverAmount = ldxOverAmount.add(swapAmount);
			uint256 half12 = swapAmount.div(2);
	...
	function _splitOtherTokenSencond(uint256 thisAmount) private {
        ...
        if(buySize>0){
            ...
            if(buySize >20){
                for(uint256 i=0;i<20;i++){
                    ...
                    address user = buyUser[startIndex];
                    rate = balanceOf(user).mul(100000000).div(total);
                    uint256 damount = thisAmount.mul(rate).div(100000000);
                    if(damount>=10**0){
    ...
    function splitOtherToken() public {
        ...
        if(thisAmount > 10**8){
            _splitOtherTokenSencond(thisAmount.div(5).mul(4));
        }
    }
