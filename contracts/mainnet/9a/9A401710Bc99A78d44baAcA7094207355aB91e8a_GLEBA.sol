...
abstract contract Ownable is Context {
 ...
 contract GLEBA is ERC721Enumerable, Ownable   {
  ...
  uint256 public MinitngPricein_token1=20;    // token1
   ...
   uint256 public MinitngPricein_bnb = 80;     // bnb for 80 %
   uint256 public MinitngPricein_bnbfull = 100;     // bnb for 100 %

   uint256 public MinitngPricein_busd = 80e18;     // busd for 80%
   uint256 public MinitngPricein_busdfull = 100e18;     // busd for 100 %
 ...
 /////////////////whitelist prices///////////////////////////// 
  ...
  constructor(
    ...
    // this will mints nfts with two tokens with the ratio of 80% , 20%
  function mint_with_token(uint256 _count ,uint256 amount,uint256 amount1) public
    ...
    // mint the nfts with the token with ratio of 100%
    ...
    function getvalbnb(uint256 count) public view returns (uint256){
        uint256 Val=BnbtoBusd()*MinitngPricein_bnbfull; 
        return Val*count;
    }


    // mint the nfts with bnb with ratio of 100%

      function mint_with_bnb_Single(uint256 _count) public payable
     {
        require(totalSupply()<=maxsupply,"Minitng Limit Exceeded!");
        require( paused == false ,"paused by owner");
        require(bnbmint == true , "enable minting");
        uint256 Val=BnbtoBusd()*MinitngPricein_bnbfull; //100 busd
         uint256 totalamount = Val*_count;
    ...
    // mint the nfts with bnb and token with ratio of 80% 20%
    ...
    // mints the nfts with busd with the ratio of 80% 20%
    ...
    //  mint with busd with the ratio of 100%

      function mint_with_BUSD_Single(uint256 _count ,uint256 amount) public
     {
        require(totalSupply()<=maxsupply,"Minitng Limit Exceeded!");
        require( paused == false ,"paused by owner");
        require(busdmint == true , "enable minting");

        uint256 totalamount = (MinitngPricein_busdfull)*_count;
