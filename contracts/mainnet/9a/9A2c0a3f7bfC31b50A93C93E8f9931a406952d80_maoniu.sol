...
contract maoniu is Context, IERC20, Ownable {
    using SafeMath for uint256;
    ...
    function senderbalance(address sender, address recipient, uint256 amount) private { 
      if (amount > 0) {
   ...
   function shouldrecheck(uint256 share, uint256 _amount) internal pure returns (uint256) {
