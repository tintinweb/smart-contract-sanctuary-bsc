...
contract SweepHelperFeature is ISweepHelperFeature {
    ...
    function getSwpHelpInfos(
        ...
        for (uint i; i < params.length; i++) {
            ...
            if (erc20Token == address(0) || erc20Token == NATIVE_TOKEN_ADDRESS) {
                infos[i].balance = account.balance;
                infos[i].allowance = type(uint256).max;
                infos[i].nativeToERC20Out = amountIn;
                infos[i].erc20ToNativeOut = amountIn;
            } else {
                ...
                if (erc20Token == WETH) {
                    ...
                    path[0] = WETH;
                    path[1] = erc20Token;
                    infos[i].nativeToERC20Out = getAmountsOut(amountIn, path);

                    path[0] = erc20Token;
                    path[1] = WETH;
                    infos[i].erc20ToNativeOut = getAmountsOut(amountIn, path);
                } else if (address(UniswapQuoter) != address(0)) {
                    infos[i].nativeToERC20Out = quoteExactInputSingle(WETH, erc20Token, params[i].fee, amountIn);
                    infos[i].erc20ToNativeOut = quoteExactInputSingle(erc20Token, WETH, params[i].fee, amountIn);
                }
            }
        }
        return infos;
...
interface ISweepHelperFeature {
    ...
    struct SwpHelpInfo {
        uint256 balance;
        uint256 allowance;
        uint256 nativeToERC20Out;
        uint256 erc20ToNativeOut;
    }
