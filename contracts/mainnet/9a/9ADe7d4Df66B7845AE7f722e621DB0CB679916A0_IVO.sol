...
contract IVO is AccessControl, ReentrancyGuard {
    ...
    uint256 constant IVO_FIRST_RELEASE_RATE = 10;
    uint256 constant IVO_RECEIVE_INTERVAl = 1 days;

    uint256 constant MEMBER_MIN_HOLD_DEFI_WZ = (6 * 1e18) / 100;
    uint256 constant SHAREHOLDER_MIN_HOLD_DEFI_WZ = 1e18;
    uint256 constant SHAREHOLDER_IVO_AMOUNT = 2000 * 1e18;
    uint256 constant MEMBER_IVO_AMOUNT = 560 * 1e18;
    uint256 constant RECEIVE_REWARD_CONDITION = 10;

    IReferral public immutable referral;
    IERC20 public immutable usdt;
    IERC20 public immutable defiWz;
    IERC20 public wzSwap;
    IWzNFT public wzNft;

    uint256[2] public referralRewardRates = [3, 2];
    ...
    {
        uint256 firstReleaseAmount = _ivoData
            .amount
            .mul(IVO_FIRST_RELEASE_RATE)
            .div(100);
        if (_ivoData.balance == _ivoData.amount) {
            amount = firstReleaseAmount;
        } else {
            amount = _ivoData.amount.sub(firstReleaseAmount).div(
                IVO_RELEASE_DAYS
            );
    ...
    function setWzNft(IWzNFT _wzNft) external onlyOwner {
        wzNft = _wzNft;
    }

    function setWzSwap(IERC20 _wzSwap) external onlyOwner {
        wzSwap = _wzSwap;
    }
