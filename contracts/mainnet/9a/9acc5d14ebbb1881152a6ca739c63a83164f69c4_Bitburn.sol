...
abstract contract BitburnFlags is UsingFlags, UsingDefaultFlags, UsingAdmin {
    using bits for uint256;
    ...
    function _DISTRIBUTOR_DISABLED_FLAG() internal pure virtual returns (uint256) {
        return _ACCOUNT_FLAG() >> 1;
    }

    function _BURN_EXEMPT_FLAG() internal view returns (uint256) {
        return _DISTRIBUTOR_DISABLED_FLAG() >> 1;
    }
    ...
    function _isBurnExempt(address account_) internal view virtual returns (bool) {
        return _getFlags(account_).check(_BURN_EXEMPT_FLAG(), 0);
    }
...
contract BitburnFlagsWithStorage is UsingFlagsWithStorage, BitburnFlags {
    using bits for uint256;
...
abstract contract UsingFee is UsingDefaultFlags, UsingPrecision {

    function _setFee(uint256 fee_) internal virtual {
        _setFeeStorage(fee_);
    }

    function _getFee() internal view virtual returns (uint) {
        return _getFeeStorage();
    }

    function _calculateFee(address account_, uint amount_) internal virtual view returns (uint) {
        if (!_isFeeExempt(account_)) {
            return _getFee() * amount_ / DEFAULT_PRECISION;
        }
        return 0;
    }

    function _getFeeStorage() internal view virtual returns (uint);
    function _setFeeStorage(uint256 fee_) internal virtual;
}

abstract contract UsingFeeWithStorage is UsingFee {

    uint256 _fee;

    function _initializeFeeWithStorage(uint256 fee_) internal virtual {
        _setFeeStorage(fee_);
    }

    function _getFeeStorage() internal view virtual override returns (uint) {
        return _fee;
    }

    function _setFeeStorage(uint256 fee_) internal override virtual {
        _fee = fee_;
    }
}
...
interface IServiceProvider is IService {
    ...
    function setServiceFlags(address[] calldata service_, uint256 set_, uint256 clear_) external;
...
error BitburnExcessiveTransferFee();
error BitburnFixedTotalSupply();

/// @title Bitburn
/// @notice Bitburn finance token contract
/// @author [email protected]
contract Bitburn is UsingERC20WithStorage, BitburnFlagsWithStorage, UsingFeeWithStorage, UsingInitializer, UsingUUPS {
    using bits for uint256;
    IServiceProvider _serviceProvider;

    function initialize(address serviceProvider_) external initializer  {
        _initializeERC20WithStorage();
        _initializeAdmin(msg.sender);
        _setFlags(serviceProvider_, _PROVIDER_FLAG() | _SERVICE_FEE_EXEMPT_FLAG(), 0);
        _serviceProvider = IServiceProvider(serviceProvider_);
        _setFlags(address(this), _SERVICES_DISABLED_FLAG() | _TRANSFER_DISABLED_FLAG(), 0);
        _mint(msg.sender, 21000000 * 10 ** _getDecimalStorage());
    }

    function setBurnRate(uint burn_) external requires(msg.sender, _ADMIN_FLAG(), 0) {
        _setFee(burn_);
    }

    function setServiceProvider(address serviceProvider_) external requires(msg.sender, _ADMIN_FLAG(), 0) {
        _setFlags(address(_serviceProvider), 0, _PROVIDER_FLAG() | _SERVICE_FEE_EXEMPT_FLAG());
        _serviceProvider = IServiceProvider(serviceProvider_);
        _setFlags(serviceProvider_, _PROVIDER_FLAG() | _SERVICE_FEE_EXEMPT_FLAG(), 0);
    }
    ...
    function _transfer(address from_, address to_, uint amount_) internal override requires(from_, 0, _BLOCKED_FLAG()) returns (bool) {
        ...
        if (!_isServiceExempt(from_, to_) && !_getFlags(address(this)).all(_SERVICES_DISABLED_FLAG())) {
            serviceFee = _serviceProvider.process(from_, to_, amount_);
            if (_isLPPair(from_, to_) && serviceFee > 0) {
                if (serviceFee > amount_) {
                    revert BitburnExcessiveTransferFee();
                }
                super._transfer(from_, address(_serviceProvider), serviceFee);
            } else{
                serviceFee = 0;
            }
        }
        if (!_isBurnExempt(from_) && !_isBurnExempt(to_)) {
            unchecked {
                burnAmount = amount_ * _fee / _PRECISION();
            }
            _burn(from_, burnAmount);
        }
        return super._transfer(from_, to_, amount_ - serviceFee - burnAmount);
    }

    function burn(address from_, uint256 amount_) external requires(msg.sender, _SERVICE_FLAG(), 0) {
        _burn(from_, amount_);
    }
