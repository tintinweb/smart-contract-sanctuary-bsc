...
contract ReferBook is IReferBook, Ownable {
    struct ReferInfo {
        address refer;
        bool confirmed;
    }

    mapping(address => ReferInfo) private _refermap;
    mapping(address => bool) public whitelist;

    event ReferSaved(address referral, address referrer);

    function addWhitelist(address[] memory addresses, bool[] memory values) public onlyOwner {
        require(addresses.length == values.length, "PARAM_ERROR");
    ...
    function addReferForNoExist(address referral, address referrer) public override {
        ...
        ReferInfo storage ri = _refermap[referrer];
        if (ri.refer == referral && ri.confirmed) return;

        if (ri.refer == referral && !ri.confirmed) {
            ri.confirmed = true;
            emit ReferSaved(referrer, referral);
        }

        ri = _refermap[referral];
        if (ri.refer == address(0)) {
    ...
    function getRefer(address referee) public override view returns (address) {
        ReferInfo storage ri = _refermap[referee];
        return ri.confirmed ? ri.refer : address(0);
    }
}