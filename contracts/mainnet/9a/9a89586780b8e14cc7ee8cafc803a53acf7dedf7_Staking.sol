...
contract Staking is Owned {
    ...
    address public ATGX_LP = 0xb888Df359a6c08F7F9172aBc494Fbb6f9467F7D8;
    ...
    function ClaimReward() external {
        ...
        // update the record
        users[msg.sender][ATGX].totalGained = users[msg.sender][ATGX]
            .totalGained
            .add(_pendingReward);
        users[msg.sender][ATGX].lastClaimedDate = block.timestamp;
        users[msg.sender][ATGX].pendingGains = 0;
    ...
    function StopStaking() external {
        require(
            users[msg.sender][ATGX_LP].activeDeposit >= 0,
            "No active stake"
        );
        uint256 _activeDeposit = users[msg.sender][ATGX_LP].activeDeposit;

        // update staking stats
        // check if we have any pending rewards, add it to previousGains var
        users[msg.sender][ATGX].pendingGains = PendingReward(msg.sender);
        // update amount
        users[msg.sender][ATGX_LP].activeDeposit = 0;
        ...
        require(IERC20(ATGX_LP).transfer(msg.sender, _activeDeposit));
        ...
        returns (uint256 _pendingRewardWeis)
    {
        uint256 _totalStakingTime = block.timestamp.sub(
            users[_caller][ATGX].lastClaimedDate
        );
        ...
        uint256 reward = (
            (users[_caller][ATGX_LP].activeDeposit).mul(
                _totalStakingTime.mul(_reward_token_second)
            )
        ).div(10**14); // remove extra 10^12 // 10^2 are for 100 (%)

        return reward.add(users[_caller][ATGX].pendingGains);
    }
    ...
    {
        return users[_user][ATGX_LP].activeDeposit;
    }
    ...
    {
        return users[_user][ATGX_LP].totalDeposits;
    }
    ...
    {
        return users[_user][ATGX_LP].startTime;
    }
    ...
    {
        return users[_user][ATGX].totalGained;
    }

    function _newDeposit(uint256 _amount) internal {
        if (users[msg.sender][ATGX_LP].activeDeposit > 0) {
            AddToStake(_amount);
        } else {
            ...
            users[msg.sender][ATGX_LP].activeDeposit = _amount;
            users[msg.sender][ATGX_LP].totalDeposits = users[msg.sender][ATGX_LP]
                .totalDeposits
                .add(_amount);
            users[msg.sender][ATGX_LP].startTime = block.timestamp;
            users[msg.sender][ATGX].lastClaimedDate = block.timestamp;
            ...
            require(
                IERC20(ATGX_LP).transferFrom(msg.sender, address(this), _amount)
            );
        }
    }
    ...
    function _addToExisting(uint256 _amount) internal {
        ...
        users[msg.sender][ATGX_LP].activeDeposit = users[msg.sender][ATGX_LP]
            .activeDeposit
            .add(_amount);
        ...
        require(IERC20(ATGX_LP).transferFrom(msg.sender, address(this), _amount));
        // update global stats
        totalStakes = totalStakes.add(_amount);
    }
}