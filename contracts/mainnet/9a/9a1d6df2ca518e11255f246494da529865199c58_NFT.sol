...
//构造函数abi,,必须要先合成然后部署,remix不能传构造函数的参数！,,用https://abi.hashex.org/#是对的，有时候有说不对我日
...

contract NFT is
    ERC721,
    ERC721Enumerable,
    ERC721Burnable,
    ...
    AccessControl
{
    ...
    function setExchangeAddress(address exchangeAddress)
        public
    ...
    function publishMintByAdmin(address to)
        public
        onlyRole(MINTER_ROLE)
        returns (uint256)
    {
        uint256 itemId = totalSupply() + 1;
        _safeMint(to, itemId);
        return itemId;
    }

    function publishMint(address to) public returns (uint256) {
        require(
            msg.sender == EXCHANGE_ADDRESS,
            "publish call only call by exchange contract"
        );
        uint256 itemId = totalSupply() + 1;
        _safeMint(to, itemId);
        return itemId;
    ...
    function setBaseURI(string memory url) public onlyRole(DEFAULT_ADMIN_ROLE) {
        __baseURI = url;
    }
