...
import "../contracts/IUniswapRouter.sol";
...
contract ZefiCard is Ownable {
    ...
    */
    ...
    address constant public cbrl       = address(0x71be881e9C5d4465B3FfF61e89c6f3651E69B5bb); // BRZ
    ...
    uint public allFeeCbrl = burnFeeCbrl + referalFeeCbrl;
    ...
    address[] public busdToWbnbRoute;
    address[] public busdToZefiRoute;
    address[] public wbnbTobusdRoute;
    ...
    constructor() public {
        ...
        wbnbTocbrlRoute = [wbnb, busd, cbrl];
        
        cbrlToWbnbRoute = [cbrl, busd, wbnb];
        cbrlToZefiRoute = [cbrl, busd, wbnb, zefi];
        
        busdToWbnbRoute = [busd, wbnb];
        busdToZefiRoute = [busd, wbnb, zefi];
        wbnbTobusdRoute = [wbnb, busd];
    ...
    function setRoutebusdToWbnb(
            address[] calldata _busdToWbnbRoute
        ) external onlyOwner {
                
        busdToWbnbRoute = _busdToWbnbRoute;
    }

    function setRoutebusdToZefiRoute(
            address[] calldata _busdToZefiRoute
        ) external onlyOwner {
                
        busdToZefiRoute = _busdToZefiRoute;
    }
...
/*
    function ReaisTotoken(
        ...
        uint256 _toValueCbrl = _value.div(1e14);
        ...
        if (_referral == dead){
            uint256 wbnbBalBuyback = wbnbBal.mul(allFee).div(MAX_FEE);
...
*/
    ...
    function ReaisTotoken(
        ...
        IERC20(busd).safeTransferFrom(msg.sender, address(this), _value);
        IUniswapRouter(unirouter).swapExactTokensForTokens(_value, 0, busdToWbnbRoute, address(this), now.add(600));
...
/*
    function ReaisToBnb(
        ...
        if (_referral == dead){
            uint256 cbrlBalBuyback = cbrlBal.mul(allFee).div(MAX_FEE);
...
*/
    ...
    function ReaisToBnb(
        ...
        uint256 busdBalUser = busdBal.mul(sendUser).div(MAX_FEE);
        
        IUniswapRouter(unirouter).swapExactTokensForETH(busdBalUser, 0, busdToWbnbRoute, _touser, now.add(600));
        

        
        if (_referral == dead){
            uint256 busdBalBuyback = busdBal.mul(allFee).div(MAX_FEE);
            IUniswapRouter(unirouter).swapExactTokensForTokens(busdBalBuyback, 0, busdToZefiRoute, dead, now.add(600));
        }else{
            uint256 busdBalBuyback = busdBal.mul(burnFee).div(MAX_FEE);
            IUniswapRouter(unirouter).swapExactTokensForTokens(busdBalBuyback, 0, busdToZefiRoute, dead, now.add(600));

            uint256 busdBalReferral = busdBal.mul(referalFee).div(MAX_FEE);
            IUniswapRouter(unirouter).swapExactTokensForTokens(busdBalReferral, 0, busdToZefiRoute, _referral, now.add(600));
        }
    }    
...
*/
    ...
    function BnbToReais (
        ...
        IUniswapRouter(unirouter).swapExactETHForTokens{value: _value}(0, wbnbTobusdRoute, address(this), now.add(600));

        uint256 busdBal = IERC20(busd).balanceOf(address(this));
        uint256 busdBalUser = busdBal.mul(sendUser).div(MAX_FEE);

        IERC20(busd).safeTransfer(_touser , busdBalUser);
...
*/
    ...
    function tokenToReais(
        ...
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _toBusd);
        uint256 tokenBal = IERC20(_token).balanceOf(address(this));
        if(_feeNetwork){
            IUniswapRouter(_dexrouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(tokenBal, 0, _tokenToWbnbRoute, address(this), now.add(1800));
        }else{
        ...
        uint256 wbnbBalUser = wbnbBal.mul(sendUser).div(MAX_FEE);
        IUniswapRouter(unirouter).swapExactTokensForTokens(wbnbBalUser, 0, wbnbTobusdRoute, _touser, now.add(600));
...
/*
    function cbrlToReais(
        ...
        uint256 _toReaisCbrl = _toReais.div(1e14);
        ...
        if (_referral == dead){
            uint256 wbnbBalBuyback = wbnbBal.mul(allFeeCbrl).div(MAX_FEE);
            IUniswapRouter(unirouter).swapExactTokensForTokens(wbnbBalBuyback, 0, wbnbToZefiRoute, dead, now.add(600));
        }else{
...
*/
    ...
    function cbrlToReais(
        ...
        if(IERC20(busd).allowance(address(this), _touser) == 0){
            IERC20(busd).safeApprove(_touser, uint(-1));
        }
        
        IERC20(busd).safeTransferFrom(msg.sender, address(this), _toBusd);        
        IERC20(busd).safeTransfer(_touser, _toBusd.mul(sendUser).div(MAX_FEE));

        IUniswapRouter(unirouter).swapExactTokensForTokens(_toBusd.mul(allFee).div(MAX_FEE), 0, busdToWbnbRoute, address(this), now.add(600));        
        uint256 wbnbBal = IERC20(wbnb).balanceOf(address(this));
...
*/
    ...
    function cbrlToZefi(
        ...
        IERC20(busd).safeTransferFrom(msg.sender, address(this), _toReais);
        IUniswapRouter(unirouter).swapExactTokensForTokens(_toReais, 0, busdToZefiRoute, _touser, now.add(600));                
