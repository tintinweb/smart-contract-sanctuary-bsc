...
contract Ownable {
    ...
    constructor() {
        _owner = 0x0Be0cAA432570Ce1378b31b3bB00C7f18972DbE4;
    }
...
contract PAYC is ERC20Detailed, Ownable {
    ...
    uint256 public PAYCDividendFee = 50;
    uint256 public sellFee = 70;
    uint256 public autofirePitFee = 20;
    uint256 public totalFee =
        liquidityFee.add(treasuryFee).add(PAYCDividendFee).add(
            autofirePitFee
        );
    ...
    address public PAYCDividendReceiver;
    ...
    constructor() ERC20Detailed("PAYC FINANCE", "PAYC", uint8(DECIMALS)) Ownable() {
       ...
       // router = IPancakeSwapRouter(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3); 
        ...
        autoLiquidityReceiver = 0x57D914D8Cf2527Aa79c324144c37AA12eCdf190a;
        treasuryReceiver = 0xdc893c2CBDe625173D881e04f71c505f222e6130;   
        ...
        PAYCDividendReceiver = address(distributor);
    ...
    function takeFee(
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_treasuryFee.add(PAYCDividendFee))
        );
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountETHToTreasuryAndPAYC = address(this).balance.sub(
            balanceBefore
        );

        (bool success, ) = payable(treasuryReceiver).call{
            value: amountETHToTreasuryAndPAYC.mul(treasuryFee).div(
                treasuryFee.add(PAYCDividendFee)
            ),
            gas: 30000
        }("");

        try distributor.deposit {
            value: amountETHToTreasuryAndPAYC.mul(PAYCDividendFee).div(treasuryFee.add(PAYCDividendFee))
        } () {} catch {}        
    }
    ...
    function withdrawAllToTreasury() external swapping onlyOwner {
        ...
        uint256 amountToSwap = _gonBalances[address(this)].div(_gonsPerFragment);
        require( amountToSwap > 0,"There is no PAYC token deposited in token contract");
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();
        router.swapExactTokensForETHSupportingFeeOnTransferTokens(
