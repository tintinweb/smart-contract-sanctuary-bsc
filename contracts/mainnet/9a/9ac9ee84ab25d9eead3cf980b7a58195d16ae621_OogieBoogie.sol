...
contract OogieBoogieDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("OogieBoogie_Dividend_Tracker", "OogieBoogie_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "OogieBoogie_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "OogieBoogie_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main OogieBoogie contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "OogieBoogie_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "OogieBoogie_Dividend_Tracker: Cannot update claimWait to same value");
...
contract OogieBoogie is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    OogieBoogieDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Oogie Boogie", unicode"🎃🎃🎃") {
        BNBRewardsFee = 1;
        liquidityFee = 1;
        extraFeeOnSell = 7; // extra fee on sell
        marketingFee = 1;
        marketingWallet = payable(0x31435559bDb0f4ab36a49DfF6AC2741C53B0b9e4);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new OogieBoogieDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "OogieBoogie: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "OogieBoogie: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "OogieBoogie: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "OogieBoogie: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "OogieBoogie: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "OogieBoogie: Cannot update gasForProcessing to same value");
