...
contract VaultRouter is Ownable, ReentrancyGuard
{
    ...
    function Deposit(uint256 amount, uint256 workerId) public onlyRegisteredUsers nonReentrant
    {
        ...
        address worker = Workers[workerId];
        IERC20(IAlpacaWorker(Workers[workerId]).BaseToken()).safeTransferFrom(address(msg.sender),
         worker,  amount);
    ...
    function WithDraw(uint256 share, uint256 workerId) public onlyRegisteredUsers nonReentrant
    {
        ...
        require(IERC20(baseToken).balanceOf(workerAddress) >= amount, "VaultRouter: WithDraw failed. Insufficient cash.");
        require(IERC20(ShareTokens[workerId]).balanceOf(_msgSender()) >= share,
    ...
    function UpdateShareVal(uint256 workerId) public {
        ...
        IShareToken token = IShareToken(ShareTokens[workerId]);
        uint256 totalShare = IERC20(ShareTokens[workerId]).totalSupply();
        if(totalShare > 1e17){
    ...
    function SetWorker(address worker, address shareToken, address workerConfig,
    uint256 workerId) public onlyOwner {
        _updateWorkerAddress(worker, workerId);
        _updateTokenAddress(shareToken, workerId);
        _updateWorkerConfig(workerConfig, workerId);
    }
    ...
    function TransferConfigOwnership(address newOwner, uint256 workerId) public onlyOwner{
        require(newOwner != address(0), "VaultRouter: new owner is address zero." );
        Operatable(WorkerConfigs[workerId]).transferOwnership(newOwner);
    }
     /**
    ...
    function _updateWorkerConfig(address newWorkerConfig, uint256 workerId) internal{

        address previousWorkerConfig = WorkerConfigs[workerId];
        WorkerConfigs[workerId] = newWorkerConfig;
        emit TokenAddressUpdated(previousWorkerConfig, newWorkerConfig, workerId);
    }
    ...
    function UpdateWorkerRates(int256 rate, int256 time, uint256 workerId) public onlyFairlaunchOrOwner{
        IWorkerConfig config = IWorkerConfig(WorkerConfigs[workerId]);
        config.Add(rate,time);
    }
