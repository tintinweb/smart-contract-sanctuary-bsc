    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if(lpPoolAddress==recipient||lpPoolAddress==sender){
            ...
            if(!_isExcluded[sender]&&!_isExcluded[recipient]){
                ...
                if(fee>0){
                _balances[address(config.getAwordPoolAddress())] = _balances[address(config.getAwordPoolAddress())].add(fee);
                emit Transfer(sender, address(config.getAwordPoolAddress()), fee);
                }
                _balances[sender] = _balances[sender].sub(amount);
