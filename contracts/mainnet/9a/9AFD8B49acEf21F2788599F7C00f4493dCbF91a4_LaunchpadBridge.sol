...
import "./ILaunchpadStream.sol";

contract LaunchpadBridge is Ownable, FeeCollector {
    ...
    IBank public bank;
    ILaunchpadStream public prevVault;
    ILaunchpadStream public nextVault;
    ...
    event VaultAddressChanged(address indexed prev, address indexed next);
    ...
    public onlyOwner
    {
        require(address(srcToken) == address(0), 'LaunchpadBridge: token address already set');
        require(address(trgToken) == address(0), 'LaunchpadBridge: token address already set');
        require(address(srcTokenAddress) != address(0), 'LaunchpadBridge: cannot set zero-address as one of the tokens');
        require(address(trgTokenAddress) != address(0), 'LaunchpadBridge: cannot set zero-address as one of the tokens');
        require(srcTokenDec > 0, 'LaunchpadBridge: token decimals needs to be higher than zero');
        require(trgTokenDec > 0, 'LaunchpadBridge: token decimals needs to be higher than zero');
    ...
    function setLaunchpadVault(ILaunchpadStream _prevVault, ILaunchpadStream _nextVault) public onlyOwner {
        prevVault = _prevVault;
        nextVault = _nextVault;
        emit VaultAddressChanged(address(prevVault), address(nextVault));
    }

    function setStartBlock(uint256 _startBlock) public onlyOwner {
        require(startBlock == 0, 'LaunchpadBridge: start block already set');
        require(_startBlock > 0, 'LaunchpadBridge: start block needs to be higher than zero!');
        startBlock = _startBlock;
        emit StartBlockChanged(startBlock);
    }

    function setCloseBlock(uint256 _closeBlock) public onlyOwner {
        require(startBlock != 0, 'LaunchpadBridge: start block needs to be set first');
        require(closeBlock == 0, 'LaunchpadBridge: close block already set');
        require(_closeBlock > startBlock, 'LaunchpadBridge: close block needs to be higher than start one!');
    ...
    function exchangeTokens() public payable {
        exchangeTokens(prevVault.userInfo(msg.sender).mintAmount, 0);
    }
    ...
    function exchangeTokens(uint256 amount, uint256 minAmountBack) public payable collectFee('exchangeTokens') {
        ...
        require(startBlock > 0 && block.number >= startBlock, 'LaunchpadBridge: not started yet');
        require(closeBlock == 0 || block.number <= closeBlock, 'LaunchpadBridge: not active anymore');
        require(amount > 0, 'LaunchpadBridge: amount needs to be higher than zero');
        require(address(bank) != address(0), 'LaunchpadBridge: bank needs to be set');
        require(address(prevVault) != address(0) && address(nextVault) != address(0), 'LaunchpadBridge: prevVault needs to be set');
        ...
        ILaunchpadStream.UserInfo memory userInfo = prevVault.userInfo(msg.sender);
        
        uint256 expAmount = userInfo.mintAmount;
        require(expAmount == srcAmount, 'LaunchpadBridge: you need to exchange all of your tokens at once!');
        
        uint256 trgAmount = estimateTokens(userInfo.baseAmount);

        require(minAmountBack == 0 || minAmountBack <= trgAmount,
            'LaunchpadBridge: exchange cancelled due to higher than expected slippage');

        bank.withdrawFrom(owner(), trgAmount);
        trgToken.approve(address(nextVault), trgAmount);
        nextVault.depositFor(msg.sender, trgAmount, 0, userInfo.lockedUntil, userInfo.lockedUntil-userInfo.lockedSince);
