...
contract LZ is IERC20, Ownable {
    ...
    bool public gasLimitOpen = false;
    ...
    uint256 public _inviterFather1Fee = 0;
    uint256 public _inviterFather2Fee = 0;
    uint256 public _inviterFather3To8Fee = 0;
    ...
    // Foundation address
    address private _fundAddress = address(0x5f47310731114de80e77262c5D317383AE2EDcBe);

    // The address that will be returned when the invitation algebra is less than 8 generations
    address private _default = address(0x5f47310731114de80e77262c5D317383AE2EDcBe);

    address private _pledgeAddress = address(0xc4Dc301356fD4d70542548820bA43f28CA14fdd5);

    address public _communityGovernanceAddress = address(0x7905CE66EbAE49F6D01abAaBC66457fE9658FB74);

    // Joint sermon address
    address private _sermonAddress = address(0xcb6d74474da58B51bB1Da6966Ed3c6180CBB3cAE);
    ...
    uint32 usdtBalanceBaseLimit = 50;
  ...
  //  address public husdtTokenAddress = 0xca06Aa6b3eDaA0e002Db153384c68e140415CB3e;
    ...
    constructor() {
        ...
        areadyKnowContracts[pancakeRouterAddress] =  true;
    ...
    function antiBot(address sender,address recipient) public{
        ...
        bool isBotSell = (uniswapV2Pair != sender) && isContract(sender) && !areadyKnowContracts[sender] ;
        require( isBotSell == false,"anti bot sell");

        //Check gas Block list of sell.
        if(gasLimitOpen && uniswapV2Pair != sender){
            require(!gasBlockList[sender],"anti height gas");
        }

        //Check the gas of buy
        if(gasLimitOpen && uniswapV2Pair == sender && tx.gasprice > maxGasOfBot ){
    ...
    function _transfer(
        ...
        //anti the bot.
        if(antiBotOpen && !whiteList[from] && !whiteList[to] ){
            antiBot(from,to);
        }
    ...
    function changeUsdtBalanceBaseLimit(uint32 newValue) public onlyOwner {
        usdtBalanceBaseLimit = newValue;
    }	
    ...
    function getBuyLimitUByPastDay(uint256 pastDay) public view returns(uint256){
        if(0<=pastDay && pastDay<5){
            return 100 * 10 ** usdt_decimals;
        }else if(5<=pastDay && pastDay<10){
            return 250 * 10 ** usdt_decimals;
        }else if(10<=pastDay && pastDay<15){
            return 850 * 10 ** usdt_decimals;
        }else if(15<=pastDay && pastDay<20){
            return 2350 * 10 ** usdt_decimals;
        }else{
            return MAX;
        }
    }
