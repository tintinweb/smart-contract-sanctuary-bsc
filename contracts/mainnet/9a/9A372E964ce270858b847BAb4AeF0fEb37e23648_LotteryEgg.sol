...
contract LotteryEgg {
    ...
    function BuyEggs(uint16 LotCount, uint8 mul,uint232 Lucknum,address ref) external payable {
        ...
        if(referrals[msg.sender]==DevAddress){
            ...
            Usersinfo[referrals[msg.sender]].ReferralProfit +=referralprofit;
    ...
    function InvestmentWithdrawal(uint256 WithdrawalAmount) external {
        ...
        require(block.timestamp>= InvestorsBalance[msg.sender].LastInvestTime+1 weeks,"The withdrawal is at least one week from the latest investment");
        require(SafeMathsub(address(this).balance,(InvestorsProfit+DevProfit+TotalReferralProfit+WithdrawalAmount))>=Maximum_Investment_pool/2,"Prize pool must keep more than 5000BNB");
