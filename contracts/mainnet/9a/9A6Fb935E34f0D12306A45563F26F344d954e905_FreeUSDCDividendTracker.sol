...
contract FreeUSDCDividendTracker is Ownable, DividendPayingToken, ERC20TokenRecover, IFreeUSDCDividendTracker {
    ...
    constructor(address dividendToken, address _parentToken)
        DividendPayingToken("FreeUSDC Dividend Tracker", "FUSDDT", dividendToken)
    {
        claimWait = 3600;
    ...
    function recoverERC20(address tokenAddress, uint256 tokenAmount)
        ...
        onlyOwner
    {
        require(tokenAddress != dividendToken, "FreeUSDCDividendTracker: Cannot retrieve USDT");
    ...
    function _transfer(
        ...
        require(false, "FreeUSDCDividendTracker: No transfers allowed");
    }

    function withdrawDividend() public pure override(DividendPayingToken, IDividendPayingTokenInterface) {
        require(
            false,
            "FreeUSDCDividendTracker: Disabled. Use the 'claim' function on the main FreeUSDC contract."
        );
    }

    function excludeFromDividends(address account) external override onlyOwnerOrDeployer {
        require(!excludedFromDividends[account], "FreeUSDCDividendTracker: Account already excluded");
    ...
    function includeInDividends(address account) external override onlyOwnerOrDeployer {
        require(excludedFromDividends[account], "FreeUSDCDividendTracker: Account not excluded");
    ...
    function updateClaimWait(uint256 newClaimWait) external override onlyOwnerOrDeployer {
        require(
            ...
            "FreeUSDCDividendTracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(newClaimWait != claimWait, "FreeUSDCDividendTracker: Cannot update claimWait to same value");
