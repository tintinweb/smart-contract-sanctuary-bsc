...
contract Referral is IReferral, Ownable {
    ...
    function recordRef(address _user, address _referrer,uint16 pos) public{
        refPos[_user] = pos;
        RefDetail memory refDetail;
    ...
    function updateReferral(address _user, address _under) public  onlyMasterChef {
        if (_user != address(0)&& _under != address(0)
        && _user != _under&& referrers[_user].under == address(0)) {
            RefDetail storage refDetail = referrers[_user];
            refDetail.under=_under;
        }
    }
