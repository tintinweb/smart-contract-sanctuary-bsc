...
// File: contracts/AuctionSale.sol
...
contract AuctionSale is ReentrancyGuard {
    ...
    uint256 private _waitingPeriod = 10 days; //CHANGE AS NEEDED

    IMarketplace private _market = IMarketplace(0xb318cC52febfBBd5876E9F6f14bDA97a835a42cA); 
    IFractionalSale private _fSale = IFractionalSale(0x6bFAD137547F3A67b91e955dcb8BDCf9179A7a62);

    event AuctionSold(uint256 indexed itemId, uint256 price, address buyer, uint256 timestamp);

    mapping(uint256 => Auction) private _idToAuction;

    modifier notBlacklisted {
        require(!_market.getBlacklisted(msg.sender), "AuctionSale: blacklisted");
        _;
    }

    modifier onlyTokenOwner(uint256 itemId) {
        require(msg.sender == _market.getItemOwner(itemId), "AuctionSale: token owner only");
        _;
    }

    modifier notListedForSale(uint256 itemId) {
        require(_market.getItemForSale(itemId) == false, "AuctionSale: already listed for sale");
        _;
    }
    
    function setContracts(address marketplaceAddr, address fractionalSaleAddr) external {
        require(msg.sender == _market.owner(), "AuctionSale: mp owner only");
    ...
    /// @param itemId Item ID of NFT
    /// @param _amount Amount of NFTs for sale
    /// @param _reserve Minimum bidding price - optional
    /// @param _biddingPeriodInSeconds Auction timespan in seconds
    function createAuction(
        ...
        require(_biddingPeriodInSeconds > 0 && _amount > 0, "AuctionSale: invalid input");
        (uint256 tokenId, , address nftContract, , , bool forSale) = _market.getItemInfo(itemId);
        require(forSale == false, "AuctionSale: item already for sale");

        if(IERC1155(nftContract).supportsInterface(0xd9b67a26)) {
            require(IERC1155(nftContract).isApprovedForAll(msg.sender, address(this)), "AuctionSale: contract not approved");
        } else {
            require(_amount == 1, "AuctionSale: amount must be 1");
            require(IERC721(nftContract).getApproved(tokenId) == address(this), "AuctionSale: not approved");
        }
    ...
    /// @param itemId Item ID of NFT
    function placeBid(uint256 itemId) external payable notBlacklisted nonReentrant {
        if(_market.isPublic() == false) require(_market.isWhitelisted(msg.sender), "AuctionSale: not whitelisted");
        Auction memory auction = _idToAuction[itemId];
        require(block.timestamp <= auction.auctionEndTime, "AuctionSale: not for sale");

        if(auction.highestBid == 0) {
            require(msg.value >= auction.reserve, "AuctionSale: insufficient amount");
        }else{
            require(msg.value > auction.highestBid, "AuctionSale: insufficient amount");
            _refundPrevHighestBidder(auction.highestBidder, auction.highestBid);
        }

        _idToAuction[itemId].highestBidder = msg.sender;
        _idToAuction[itemId].highestBid = msg.value; 
    }
    ...
    function acceptBid(uint256 itemId) external nonReentrant {
        ...
        require(prevOwner == msg.sender, "AuctionSale: token owner only");
        require(auction.auctionEndTime < block.timestamp, "AuctionSale: auction still active");
        if(auction.highestBid == 0) {
        ...
        emit AuctionSold(itemId, hBid, hBidder, block.timestamp);
    }
    ...
    function rejectBid(uint256 itemId) external onlyTokenOwner(itemId) nonReentrant {
        ...
        _market.setItemForSale(itemId, false);
        _refundPrevHighestBidder(hBidder, hBid);
    }

    /// @notice Take back highest bid in case of unresponsive buyer - Caller must be highest bidder 
    /// @notice Must wait _waitingPeriod after auction end to reclaim funds
    /// @param itemId ID of item
    function withdrawBid(uint256 itemId) external nonReentrant {
        require(block.timestamp > _idToAuction[itemId].auctionEndTime + _waitingPeriod, "AuctionSale: already listed for sale"); 
        require(msg.sender == _idToAuction[itemId].highestBidder, "AuctionSale: only highest bidder");
    ...
    /// @param _waitingPeriodInSeconds New waiting period in seconds
    function setWaitingPeriod(uint256 _waitingPeriodInSeconds) external {
        require(msg.sender == _market.owner(), "AuctionSale: only mp owner");
    ...
    function _transferFees(
        ...
        uint256 sellerAmt = amount - (feeAmt + royaltyAmt);
        ...
        if(frToken == address(0)) {
            (bool success3, ) = payable(seller).call{value: sellerAmt}("");
            require(success3);
        }else {
    ...
    function _transferShare(uint256 itemId, uint256 lastSoldPrice, uint256 lastSoldTimestamp, address frToken, address shareholder) private {
        require(lastSoldPrice > 0, "AuctionSale: item not sold");
        uint256 claimTimestamp = _fSale.getSharesClaimedTimestamp(itemId, shareholder);
        if(claimTimestamp > 0)
            require(lastSoldTimestamp > claimTimestamp, "AuctionSale: already claimed");

        _fSale.setSharesClaimedTimestamp(itemId, block.timestamp, shareholder);
        uint256 shares = IERC20(frToken).balanceOf(shareholder);
        require(shares > 0, "AuctionSale: no shares");
        uint256 amt = lastSoldPrice * shares / IERC20(frToken).totalSupply();

        (bool success, ) = payable(shareholder).call{value: amt}("");
        require(success);
    }    
