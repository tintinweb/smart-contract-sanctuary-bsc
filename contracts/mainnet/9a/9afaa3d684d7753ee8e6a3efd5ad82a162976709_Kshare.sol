...
contract Kshare is ERC20Taxable {
    using SafeMath for uint256;
    
    // TOTAL MAX SUPPLY = 37,000 KSHARE
    ...
    constructor(
        ...
        address _devFund
    ) public ERC20Taxable("Kshare", "KSHARE") {
        _mint(msg.sender, 1 ether); // mint 1 Kshare for initial pools deployment
    ...
    function setCommunityFund(address _communityFund) external {
        require(msg.sender == devFund, "Error: Kshare - you are not the dev");
        communityFund = _communityFund;
    }
