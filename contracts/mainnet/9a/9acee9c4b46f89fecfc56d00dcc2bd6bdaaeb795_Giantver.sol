...
contract Giantver is IBEP20, Auth {
    ...
    string constant _name = "Giant Universe";
    string constant _symbol = "Giantver";
    ...
    address private marketingFeeReceiver = 0x717502FF98C49073E2d318571BED2FcA1519b44B;
    address private buybackFeeReceiver = 0x717502FF98C49073E2d318571BED2FcA1519b44B;
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        if (_allowances[sender][msg.sender] != type(uint256).max) {
            _allowances[sender][msg.sender] = _allowances[sender][msg.sender].sub(amount, "Giantver seems to happen Insufficient Allowance!!!!");
        }
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        _balances[sender] = _balances[sender].sub(amount, "Giantver seems to happen Insufficient Balance!!!!");
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (recipient != UniswapV2Pair && recipient != DEAD && sender != owner && recipient != owner) {
            require(isTxLimitExempt[recipient] || _balances[recipient] + amount <= _maxWalletSize, "Giantver Transfer seems to happen amount exceeds the bag size!!!!");
        }
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        uint256 feeAmount = amount.mul(lockFee).div(feeDenominator);
    ...
    function checkTxLimit(address sender, uint256 amount) internal view {
        require(amount <= _maxTxAmount || isTxLimitExempt[sender], "Giantver TX Limit Exceeded");
    }
    ...
    function swapBack() internal swapping {
        ...
        require(MarketingSuccess, "Giantver receiver rejected ETH transfer");
        (bool BuyBackSuccess, /* bytes memory data */) = payable(buybackFeeReceiver).call{value : amountBNBbuyback, gas : 30000}("");
        require(BuyBackSuccess, "Giantver receiver rejected ETH transfer");
    ...
    function transferForeignToken(address _token) public authorized {
        require(_token != address(this), "Giantver Can't let you take all native token");
