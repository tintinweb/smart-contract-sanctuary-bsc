...
contract VAULT is BEP20 {
    ...
    function transferBUSD(address addr, uint amount) public payable stakingCaller SytemActivated returns(bool){
        busd.transfer(payable(addr), amount);
        return true;
    }

    function transferDUC(address addr, uint amount) public payable stakingCaller SytemActivated returns(bool){
        duc.transfer(addr, amount);
        return true;
    }

    function transferFees() external payable onlyOwner{
