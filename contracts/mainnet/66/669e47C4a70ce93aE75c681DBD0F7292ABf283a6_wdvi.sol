...
contract wdvi is DVDERC20RoleBase(msg.sender,"Whitelisted Dividend Bearing", "wDVI") 
{
  ...
  function burn(address from, uint256 amount) external override onlyBurner {
    _burn(from, amount);
  }
  ...
  function distribute(uint256 _amount) external onlyAdmin {
    require(dvdToken.balanceOf(msg.sender) >= _amount, "wDVI::distributeReward: Insufficient balance");
...
abstract contract BaseDividends is IBaseDividends {
  ...
  function cumulativeDividendsOf(address account) public view override returns (uint256) {
    return pointsPerShare
      ...
      .toUint256().div(POINTS_MULTIPLIER);
  }
  ...
  function _distributeDividends(uint256 amount) internal {
    ...
    if (amount > 0) {
      pointsPerShare = pointsPerShare.add(
        amount.mul(POINTS_MULTIPLIER).div(shares)
      );
