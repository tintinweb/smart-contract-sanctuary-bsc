...
contract LandKingdom is Ownable {
    ...
    function seasonEnd() external initialized seasonCheck {
        ...
        uint256 userBalance = IERC20(PAPCTokenAddress).balanceOf(address(this));
        ...
        for(uint i; i < airdropCount / 2; i++) {
            ...
            IERC20(PAPCTokenAddress).transfer(startPlayer, avarage);
            IERC20(PAPCTokenAddress).transfer(endPlayer, avarage);
        }
    }
    ...
    function addCrystals(address referral, uint256 value) external initialized {
        ...
        uint256 userBalance = IERC721(CertificateAddress).balanceOf(user);

        if(userBalance == 0) {
            if(whiteList[user]) {
                referral = blackHole;
            } else {
                uint256 referraluserBalance =IERC721(CertificateAddress).balanceOf(referral);
        ...
        IERC20(PAPCTokenAddress).transferFrom(msg.sender, manager, managerValue);
        IERC20(PAPCTokenAddress).transferFrom(msg.sender, developer, developerValue);
        IERC20(PAPCTokenAddress).transferFrom(msg.sender, address(this), value - valueToManager);
    }
    ...
    function withdrawMoney(uint256 gold) external initialized {
        ...
        if(msg.sender == manager) {
            ...
            IERC20(PAPCTokenAddress).transfer(manager, userBalance < managerValue ? userBalance : managerValue);
            IERC20(PAPCTokenAddress).transfer(developer, userBalance < developerValue ? userBalance : developerValue);
        } else {
            IERC20(PAPCTokenAddress).transfer(user, userBalance < amount ? userBalance : amount);
        }
    }
