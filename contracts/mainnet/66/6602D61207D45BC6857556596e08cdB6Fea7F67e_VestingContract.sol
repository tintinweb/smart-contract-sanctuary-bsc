...
contract VestingContract is Ownable {
    ...
    uint public immutable STARTING_PERCENTAGE = 0;
    uint public immutable PERIOD_PERCENTAGE;
    uint public withdrawn = 0;

    constructor(uint _maxAmount, IBEP20 _everdomeToken, address _owner, uint _periodPercentage, uint _period){
        everdomeToken = _everdomeToken;
        maxAmount = _maxAmount;
        PERIOD_PERCENTAGE = _periodPercentage;
    ...
    function getLegalPercentage() public view returns(uint){
        uint percentage = STARTING_PERCENTAGE + PERIOD_PERCENTAGE*((block.timestamp - start)/VESTING_PERIOD);
        if(percentage>100){
            return 100;
        }else{
            return percentage;
