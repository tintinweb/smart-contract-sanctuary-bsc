...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0 && block.timestamp > user.lastRewardTimestamp) {
            ...
            uint256 perDaysecond = 86400;
            uint256 perYear = perDaysecond.mul(365);
            uint256 perSecond = apy.mul(1e14).div(perYear);
            uint256 rewardAmount = multiplier.mul(perSecond);
            uint256 pending = user.amount.mul(rewardAmount).div(1e18);
            if (pending > 0) {
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        uint256 userwithdrawAmount = _amount;
        if (_amount > 0) {
             if (withdrawfee > 0) {
                  require(
                        withdrawfee <= 1000,
                ...
                uint256 withdrawfees = _amount.mul(withdrawfee).div(10000);
                stakedToken.safeTransfer(feeaddress, withdrawfees);
                userwithdrawAmount = userwithdrawAmount.sub(withdrawfees);
            } else {
                userwithdrawAmount = userwithdrawAmount;
            }
            user.amount = user.amount.sub(_amount);
            stakedToken.safeTransfer(address(msg.sender), userwithdrawAmount);
        }
        user.lastRewardTimestamp = block.timestamp;
    ...
    function updateRewardApy(uint256 _apy) external onlyOwner {
        apy = _apy;
        emit NewRewardPerBlock(_apy);
    }
