...
contract ERC1155Marketplace is ERC1155YuzuInternal, ReentrancyGuard {
    ...
    {
        require(_commissionRate + _royaltiesCommissionRate < 100, "PiDNFT1155: total commissions should be lower than 100");
        admin = _admin;
    ...
    function sell(uint256 tokenId, uint256 qty, uint256 price, address wallet) external {
        ...
        // Limit the sell to owned items, the user cannot sell items do not have
        require(balanceOf(_msgSender(), tokenId) >= qty, "PiDNFT1155: you do not have enough tokens to sell");
    ...
    function buy(uint256 tokenId, uint256 index, uint256 qty) external nonReentrant {
        ...
        // validate user
        require(openSales[tokenId][index].user != _msgSender() , "PiDNFT1155: the user cannot buy his own offer");
    ...
    function cancelSale(uint256 tokenId, uint256 index) external {
        ...
        require(openSales[tokenId][index].user == _msgSender(), "PiDNFT1155: only the original seller can cancel his sales");
    ...
    function cancelBid(uint256 tokenId, uint256 index) external nonReentrant {
        ...
        // Only the original bidder can cancel his bids
        require(openOffers[tokenId][index].user == _msgSender(), "PiDNFT1155: only the original bidder can cancel his bids");
    ...
    function acceptBid(uint256 tokenId, uint256 index, uint256 qty) external nonReentrant {
        ...
        // validate user
        require(openOffers[tokenId][index].user != _msgSender() , "PiDNFT1155: the user cannot accept his own bid");
    ...
    function callOptionalReturn(IERC1155 token, bytes memory data) private {
        ...
        require(address(token).isContract(), "PiDNFT1155: call to non-contract");

        // solhint-disable-next-line avoid-low-level-calls
        (bool success, bytes memory returndata) = address(token).call(data);
        require(success, "PiDNFT1155: low-level call failed");

        if (returndata.length > 0) { // Return data is optional
            // solhint-disable-next-line max-line-length
            require(abi.decode(returndata, (bool)), "PiDNFT1155: ERC20 operation did not succeed");
        }
    }
 ...
 */
contract PiDNFT1155 is ERC1155Marketplace, EIP712 {
    string private constant SIGNING_DOMAIN = "PiDNFT1155-Voucher";
    ...
    function redeem(
        ...
        require(chainId == getChainId(), "PiDNFT1155: Wrong chain");
        require(vouchersMintedQty[id] + qtyToMint <= qty, "PiDNFT1155: the qty exceeds the amount available for minting");
        require(nftId == 0 || exists(nftId), "PiDNFT1155: Wrong NFT ID");
    ...
    function verify(
        ...
        require(recoveredSigner == signer, "PiDNFT1155: wrong signature");
    }
