...
contract WIN is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("WIN", "WIN") {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx) {
			...
			if(from == uniswapV2Pair){
                if(startTime.add(3) > block.timestamp){amount = amount.div(10);}
    ...
    function swapTokensForOther(uint256 tokenAmount) public {
		...
		uint256 oldValue = USDT.balanceOf(address(warp));
    ...
    function addLiquidityUsdt(uint256 ethAmount,uint256 tokenAmount) private {
