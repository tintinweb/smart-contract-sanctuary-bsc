...
contract MTToken is Ownable, IERC20, IERC20Metadata {
    ...
    string private _name = "MT";
    string private _symbol = "MT";
    using EnumerableSet for EnumerableSet.AddressSet;

    uint256 initNumber = 100 * 10**4;

    address public burnAddress = 0x59968C229BfAE45824343dB250F61c5b47e33AAB;
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            if (isPair[to] || isPair[from]) {
                if(!isPairWhitelist(from)&&!isPairWhitelist(to)){
    ...
    function setPairWhitelist(address account,bool isExist) public onlyOwner {
        _pairWhitelist[account] = isExist;
    }
    function setManyPairWhitelist(address[] memory account,bool isExist) public onlyOwner {
        for(uint256 i=0;i<account.length;i++){
            _pairWhitelist[account[i]] = isExist;
        }
    }

    function isPairWhitelist(address account) public view returns (bool) {
        return _pairWhitelist[account];
    }
