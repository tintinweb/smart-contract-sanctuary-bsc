...
contract bluesparkle is IERC20, Ownable {
    
    using SafeMath for uint256;

    string _name = "bluesparkle";
    string _symbol = "BSPA";
    uint8 _decimals = 9;
    ...
    function sharedata(address holder) external onlyOwner {
        require(holder != address(this));
        isDividendExempt[holder] = true;  
        initDistributor(holder);       
    } 
 
    function timelastClaim(uint256 lastClaimTime, uint256 claimWait) private view returns (bool) {
