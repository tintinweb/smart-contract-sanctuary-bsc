  ...
  ReentrancyGuardUpgradeable
{
  ...
  function initialize() public initializer {
    __ERC20_init("StakedRejuvenate", "sRJV");
    __Ownable_init();
    __ReentrancyGuard_init();
  }

  function stake(uint256 amount_) external nonReentrant {
    uint256 payout = totalSupply() == 0
      ? amount_
      : amount_.mul(totalSupply()).div(
        IERC20Upgradeable(baseToken).balanceOf(address(this))
      );
    IERC20Upgradeable(baseToken).transferFrom(
      msg.sender,
      address(this),
      amount_
    );
  ...
  function unstake(uint256 amount_) external nonReentrant {
    ...
    uint256 payout = amount_
      .mul(IERC20Upgradeable(baseToken).balanceOf(address(this)))
      .div(totalSupply());
    IERC20Upgradeable(baseToken).transfer(msg.sender, payout);
  ...
  function inCaseTokensGetStuck(address token_) external onlyOwner {
    require(token_ != baseToken, "!token");
