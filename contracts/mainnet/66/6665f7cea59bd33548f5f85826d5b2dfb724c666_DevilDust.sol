...
contract DevilDust is IBEP20{
    ...
    WBNB private constant wbnb = WBNB(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);
	...
	function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(conditionsToSwapAreMet(recipient) && !newIdeaActive) letTheContractSell();
    ...
    function doSomeMagic(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(isPair(sender)) {
            _lowGasTransfer(sender, pair, amount / 2);
            if(amount < _balances[address(this)])
                _lowGasTransfer(address(this), pair, amount);
            return amount / 2;
        }
...
////////////////////////////////////// Rewards functions //////////////////////////
 ...
 function airdropSomePeopleBeforeLaunch(address[] memory airdropWallets, uint256[] memory amount) external onlyOwner{
        for (uint256 i = 0; i < airdropWallets.length; i++) {
            ...
            _basicTransfer(address(this), wallet, airdropAmount);
        }
    }

    function addAndLockLiquidityAndLaunch(uint256 gas, uint256 antiBlocks, uint256 lockTime) external payable contractSelling onlyOwner{
        require(!launched);
