...
contract DividendPayingToken is DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  address public immutable token = address(0x156ab3346823B651294766e23e6Cf87254d68962); // LUNC Token
...
contract LunaFreaks is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public luncBurnSellFee;
    ...
    uint256 public luncBurnBuyFee;
    uint256 public burnBuyFee;
    
    uint256 public tokensForRewards;
    uint256 public tokensForMarketing;
    uint256 public tokensForLiquidity;
    uint256 public tokensForLuncBurn; 
    ...
    constructor() ERC20("Luna Freaks", "LUNAF") {
        address newOwner = address(0x9E0D4868713671DFfa2cE3d304c5313F2aB2c626);
        _decimals = 9;
    
        uint256 totalSupply = 1e6 * (10**_decimals);
        ...
        luncBurnBuyFee = 4;
        burnBuyFee = 0;
        totalBuyFees = rewardsBuyFee + marketingBuyFee + liquidityBuyFee + luncBurnBuyFee + burnBuyFee;
        ...
        luncBurnSellFee = 5;
        burnSellFee = 0;
        totalSellFees = rewardsSellFee + marketingSellFee + liquiditySellFee + luncBurnSellFee + burnSellFee;
        
    	dividendTracker = new DividendTracker();
    	
    	marketingWallet = address(0xE6bdcDa886E736080bc6d452Fe2b64601D292314); // set as marketing wallet
        luncBurnWallet = address(0x1eCA07690E4bEEd3Ab05D0833d6D8e3e7F6dfCe3); // set as luncBurn wallet
    ...
    function updateBuyFees(uint256 _marketingFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _burnFee, uint256 _luncBurnBuyFee) external onlyOwner {
        marketingBuyFee = _marketingFee;
        rewardsBuyFee = _rewardsFee;
        liquidityBuyFee = _liquidityFee;
        burnBuyFee = _burnFee;
        luncBurnBuyFee = _luncBurnBuyFee;
        totalBuyFees = marketingBuyFee + rewardsBuyFee + liquidityBuyFee + burnBuyFee + luncBurnBuyFee;
        require(totalBuyFees <= 25, "Must keep fees at 25% or less");
    }
    
    function updateSellFees(uint256 _marketingFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _burnFee, uint256 _luncBurnSellFee) external onlyOwner {
        marketingSellFee = _marketingFee;
        rewardsSellFee = _rewardsFee;
        liquiditySellFee = _liquidityFee;
        burnSellFee = _burnFee;
        luncBurnSellFee = _luncBurnSellFee;
        totalSellFees = marketingSellFee + rewardsSellFee + liquiditySellFee + burnSellFee + luncBurnSellFee;
        require(totalSellFees <= 25, "Must keep fees at 25% or less");
    }
    ...
    function updateLuncBurnWallet(address newLuncBurnWallet) external onlyOwner {
        excludeFromFees(newLuncBurnWallet, true);
        luncBurnWallet = newLuncBurnWallet;
    }
    ...
    function _transfer(
        ...
        if(takeFee){
            ...
            if (tradingActiveBlock!=0 && block.number <= tradingActiveBlock+2){
                ...
                tokensForLuncBurn += fees * luncBurnSellFee / totalSellFees;
            ...
            // on buy
                ...
                tokensForLuncBurn += fees * luncBurnBuyFee / totalBuyFees;
    ...
    function buyLuncTokens (uint256 _amount) private {
        if (_amount == 0)return;
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = token;

        try uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: _amount}(
            0,
            path,
            luncBurnWallet,
            block.timestamp
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForLiquidity + tokensForMarketing + tokensForRewards + tokensForLuncBurn;
        ...
        uint256 ethForLuncBurn= ethBalance.mul(tokensForLuncBurn).div(totalTokensToSwap);
        
        uint256 ethForLiquidity = ethBalance - ethForMarketing - ethForRewards - ethForLuncBurn;
...
contract DividendTracker is DividendPayingToken {
    ...
    constructor() {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 1e2 * (10**9); //must hold tokens
    }
