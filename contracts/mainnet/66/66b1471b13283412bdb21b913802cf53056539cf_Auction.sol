...
/** 
 *  SourceUnit: d:\GitWork\04-organization\09-MYNT\mynt-contracts\contracts\Auction.sol
*/
...
contract Auction is Ownable, ERC721Holder {
    ...
    {   
        ...
        IMYNTNFT nft = IMYNTNFT(_collectionId); 
    ...
    function finalizeAuction(uint256 _auctionId) public {
        ...
        if(bidsLength == 0) {
            IMYNTNFT(myAuction.collectionId).safeTransferFrom(address(this), myAuction.owner, myAuction.tokenId);
            ...
            address _creator = IMYNTNFT(myAuction.collectionId).creatorOf(myAuction.tokenId);
            uint256 royalty = IMYNTNFT(myAuction.collectionId).royalties(myAuction.tokenId);
            ...
            IMYNTNFT(myAuction.collectionId).safeTransferFrom(address(this), lastBid.from, myAuction.tokenId);		
            auctions[_auctionId].active = false;
