...
contract NFTSaleWithLimo is ERC721Holder, Ownable {
  ...
  constructor(address _nft, address _quoteToken) {
    require(_nft != address(0) && _nft != address(this), "Invalid address");
    nft = IERC721(_nft);
  ...
  function getAsks(uint256 _tokenId) external view returns(uint256, uint256, bool) {
    return (asks[_tokenId].tokenId, asks[_tokenId].price, asks[_tokenId].exist);
  }
  ...
  function setNftOwner(address _address) external onlyOwner{
    require(_address != address(0) && _address != address(this), 'wrong nft owner');
    nftOwner = _address;
  ...
  function buyToken(uint256 _tokenId) public payable {
    ...
    require(nftOwner == owner, "token already sold");

    uint256 allowanceAmount = quoteToken.allowance(msg.sender,address(this));
    AskEntry storage askEntry = asks[_tokenId];
    require(askEntry.exist, "token not in sale");
    require(allowanceAmount >= askEntry.price, "token failed to approve");

    askEntry.exist = false;

    quoteToken.transferFrom(msg.sender, receiver, askEntry.price);
