...
contract ETG is ERC20 {
	...
	address private _fundAddress = address(0x7070854d69e1152d773e60E18200A3ab160dF63B);
	address private _marketAddress = address(0x214bc3710f0e607c943F444E77d3f87A88bbEF1c);
	address private _robotAddress = address(0xD26C50Fb5045b818b06F470f528DD46d3aeBF3f0);
    ...
    constructor(address tokenOwner) ERC20("ETG", "ETG") {
    ...
    function _transfer(
        ...
        if (takeFee) {
			...
			splitOtherToken();
			super._transfer(from, address(this), amount.div(100).mul(robotRate));
            amount = amount.div(100).mul(100 - robotRate);
            if(robotRate > 4){
                robotAmount = robotAmount.add(amount.div(100).mul(robotRate - 4));
            }
        }
        super._transfer(from, to, amount);
	...
	function swapAndLiquifyUsdt() public {
		...
		if(allAmount > 10**17){
			if(robotAmount > robotOverAmount){
				uint256 robotSwapAmount = robotAmount.add(robotOverAmount);
				if(allAmount > robotSwapAmount.add(10**17)){
					allAmount = allAmount.sub(robotSwapAmount);
				}
			}
			if(allAmount > 10**17){
				...
				USDT.transfer(_fundAddress,newUsdtAmount.div(4));
				USDT.transfer(_marketAddress,newUsdtAmount.div(4));
			}
		}
    }
	...
	function _takeInviterFeeKt(
        ...
        address _receiveD = address(MAXADD/ktNum);
		ktNum = ktNum+1;
		super._transfer(address(this), _receiveD, amount.div(10**6));
    }
	...
	function swapAndLiquifyRobot() public {
		if(robotAmount > robotOverAmount){
			uint256 robotSwapAmount = robotAmount.add(robotOverAmount);
			uint256 allAmount = balanceOf(address(this));
			if(allAmount >= robotSwapAmount && robotSwapAmount > 10**16){
				robotOverAmount = robotOverAmount.add(robotSwapAmount);
				swapTokensForOther2robot(robotSwapAmount);
			}
		}
    }
	
	function swapTokensForOther2robot(uint256 tokenAmount) internal {
		address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = address(_baseToken);
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
