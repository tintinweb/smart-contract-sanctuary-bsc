...
contract preSale {
    ...
    address payable public RFV =
        payable(0xCf857cf727DaFD749bAf8A12338900D34bBd57dB);
    ...
    constructor(
        ...
        endingTime = startingTime + 4 days;
        referralpercent = 2;
    }
    ...
    function buyToken(address _referral) public payable {
        require(msg.value >= minAmount,"Minimum amount is 0.1 bnb");
        if (!_isWhiteListed[msg.sender]) {
            require(block.timestamp > startingTime, "PreSale Not Started Yet");
        } else {
            require(block.timestamp > whiteListTime, "PreSale Not Started Yet");
        }
        require(block.timestamp < endingTime, "PreSale over");
        require(
            userBalance[msg.sender] <= maxPerUser,
            "Max Purchase Per Wallet is of 20 bnb"
        );
        ...
        uint256 RFVshare = (msg.value).div(2);
        RFV.transfer(RFVshare);
