...
contract Airdrop2 is Ownable, Pausable, ReentrancyGuard {
    ...
    function getClaimable(address account, uint256 totalAmount, bytes32[] calldata merkleProof) public view returns (uint256) {
        ...
        uint256 claimable = (totalAmount * 5 / 100) + (totalAmount * 95 / 100) * ((block.timestamp - startVestingTime) / intervals) / (vestingPeriodCount) - claimed;
    ...
    function setTotalRewardMerkleRoot(bytes32 _totalRewardMerkleRoot) external onlyOwner {
        totalRewardMerkleRoot = _totalRewardMerkleRoot;
    }
    ...
    function setVestingPeriodCount(uint256 _vestingPeriodCount) external onlyOwner {
        vestingPeriodCount = _vestingPeriodCount;
    }

    function setIntervals(uint256 _intervals) external onlyOwner {
        intervals = _intervals;
    }
