...
contract Factory is IFactory, Ownable {
    ...
    function deploy(address token_, uint256[] calldata amounts_, uint128[] calldata percents_) external onlyOwner {
        Staking deployedContract = new Staking(token_);

        deployedContract.setIntervals(amounts_, percents_);
        deployedContract.transferOwnership(msg.sender);

        contracts.push(address(deployedContract));

        emit Deployed(msg.sender, address(deployedContract));
    }
}
...
interface IFactory {
    event Deployed(address indexed sender, address contractAddress);

    /**
     * @notice Deploy new Staking contract
     * @param token_ Token address
     * @param amounts_ Token amount for interval
     * @param percents_ Percents per second
     */
    function deploy(address token_, uint256[] calldata amounts_, uint128[] calldata percents_) external;
}
