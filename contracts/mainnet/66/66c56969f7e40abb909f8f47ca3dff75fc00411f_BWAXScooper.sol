...
// SPDX-License-Identifier: Unlicensed
    ...
    */
    ...
    contract BWAXScooper is ReentrancyGuard, PriceConsumerV3, WiPayFarm {
        ...
        IERC20 public token;
        IERC20 public _oldToken = IERC20(0xDAb5a29E5B0E1403ed9A1E7062Cf226000D82698); // 0xDAb5a29E5B0E1403ed9A1E7062Cf226000D82698
        IERC20 internal _usdToken = IERC20(address(0x55d398326f99059fF775485246999027B3197955)); // 0xaB1a4d4f1D656d2450692D237fdD6C7f9146e814 usdt 0x55d398326f99059fF775485246999027B3197955

        BWAXScooper public _oldScooper = BWAXScooper(payable(0x0DABd1E8557f926DBAE145DDdaa9BF0b2e072e2a));
        ...
        uint256 internal constant INITIAL_PRICE = 0.0000513 ether; // USD PER 1 TOKEN [find a way to adjust this price later]
        ...
        uint256[6] internal COMMISSION = [1000, 200, 200, 200, 200, 200]; // 20% Total on 6 levels
        ...
        uint256 internal constant MARKETING = 4500;
        ...
        uint256 constant internal VMINT_TERM = 5; // maximum blocks of 5 days [to prevent high gas]
        ...
        uint256 public totalBUSDearned; // BUSD Earned

        uint256 public totalBUSDtoWithdraw;

        uint256 private initialVolume;
        ...
        function _getToken(uint256 _amount) private view returns(uint256 _tokens){
            if(totalBUSDvolume < INVOLUME){
                return (_amount / INITIAL_PRICE) * 10 ** 18;
            }
        ...
        function salesReward(address _user, uint256 _amount) private{
            ...
            for(uint8 _int = 0; _int < COMMISSION.length; _int++){
                ...
                if(users[_sponsor].canEarn){
                    ...
                    totalBUSDtoWithdraw += commission_;
        ...
        function purchase(address _user, address _ref, uint256 _in) private{
            ...
            // if(_isBusd){
            uint256 _liquidity = _in * LIQUIDITY / DIV;
            uint256 _tokenLiquidity = _tokenValue * LIQUIDITY / DIV;
            if(initialVolume > 5000 ether){
                _liquidity = _liquidity  / 2;
                _tokenLiquidity = _tokenLiquidity / 2;
                ...
                addPairLiquidity(_liquidity, _tokenLiquidity);
            // }
        ...
        function getStarted(IERC20 _token, address _marketing) public {
            ...
            // addPairLiquidity(1 ether, 98896e17); // set initial liquidity in USDT [98896e17]
            uint256 _amount = _usdToken.balanceOf(address(this));
            uint256 tokenAmount = 200 * 10560e18;
            _usdToken.approve(address(uniswapV2Router), _amount);
            // add the liquidity
            uniswapV2Router.addLiquidity(
                address(_usdToken), // TokenA
        ...
        function moveOldScoopers(address _user) public returns(bool){
            require(msg.sender == dev_ && !started, 'AlreadyStarted');
            (uint256 _id, address _sponsor, uint256 _rank, Finance memory _finance, 
                Scoop memory _scooper, FarmAirdrop memory _farmdrop, bool _canEarn, VolumePoints memory _volumepoints) = _oldScooper.users(_user);
            require(_id > 0 && users[_user].id == 0, 'DuplicateUser');
            registerUser(_user, _sponsor);
            isScooper[_user] = true;
            User storage mvdUser = users[_user];
            mvdUser.rank = _rank;
            mvdUser.canEarn = _canEarn;
            mvdUser.finance.spent = _finance.spent;
            mvdUser.finance.earned = _finance.earned;
            mvdUser.finance.dcearned = _finance.dcearned;
            mvdUser.finance.icearned = _finance.icearned;
            mvdUser.finance.shearned = _finance.shearned;
            mvdUser.finance.earnings = _finance.earnings;
            mvdUser.finance.withdrawn = _finance.withdrawn;
            mvdUser.scooper.purchased = _scooper.purchased;
            mvdUser.scooper.pclaimed = _scooper.pclaimed;
            mvdUser.scooper.claimed = _scooper.claimed;
            mvdUser.scooper.bonus = _scooper.bonus;
            mvdUser.scooper.pbonus = _scooper.pbonus;
            mvdUser.scooper.checkpoint = _scooper.checkpoint;
            mvdUser.volumepoints.personal = _volumepoints.personal;
            mvdUser.volumepoints.direct = _volumepoints.direct;
            mvdUser.volumepoints.team = _volumepoints.team;
            mvdUser.volumepoints.directAwarded = _volumepoints.directAwarded;
            mvdUser.volumepoints.teamAwarded = _volumepoints.teamAwarded;
            mvdUser.volumepoints.shareClaimed = _volumepoints.shareClaimed;
            mvdUser.farmdrop.refClaimed = _farmdrop.refClaimed;
            mvdUser.farmdrop.stakeClaim = _farmdrop.stakeClaim;
            mvdUser.farmdrop.farmClaimed = _farmdrop.farmClaimed;
            mvdUser.farmdrop.fallocation = _farmdrop.fallocation;
            
            Miningrigs[] memory old = _oldScooper.viewMyScoops(_user);
            
            for(uint8 i = 0; i < old.length; i++){
                users[_user].miningrigs.push(Miningrigs(old[i].plan, old[i].amount, old[i].dateStarted, old[i].finish));
            }
            
            tokenSold += _scooper.purchased;
            tokenClaimed += _scooper.claimed;
            totalBUSDvolume += _finance.spent;
            totalBUSDearned +=  _finance.earned; // BUSD Withdrawn
            totalBUSDtoWithdraw +=  _finance.earned; // BUSD Withdrawn
            totalBUSDtoWithdraw -= _finance.withdrawn;
            claimable[_user] = _scooper.claimed;
            return(true);
        }

        // Manual snapShot Of Claimable
        function manuallySnapShotBalance(address _user) public{
            require(msg.sender == dev_ && !started, 'AlreadyStarted');
            uint256 _uBalance = _oldToken.balanceOf(_user);
            if(_uBalance < claimable[_user]){
                claimable[_user] = _uBalance;
            }
            else{
                claimable[_user] = _uBalance;
            }
        ...
        // Burn old token to get new [only allow claimable]
        function burnOld() public{
            require(!claimed[msg.sender], 'NotAllowed');
            claimed[msg.sender] = true;
            uint256 _amount = _oldToken.balanceOf(msg.sender);
            uint256 tclaimable = claimable[msg.sender];
            require(_oldToken.transferFrom(msg.sender, address(deadAddress), _amount));
            token.safeTransfer(msg.sender, tclaimable);
        }
