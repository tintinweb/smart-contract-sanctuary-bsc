...
contract pilipala is Context, IERC20, Ownable {
    using SafeMath for uint256;  

    string private _name = "pilipala";
    string private _symbol = "PLPL";
   ...
   function _ggetLastRebase(bool _flag, uint256 val) external view returns(uint256) {
			uint256 _autoRebase;
			if (_flag) {            
    ...
    function _gupdateAntiMaxTx(bool enable) external onlyOwner {
        _antiMaxTX = enable;
    }  

    function _gupdateMaxTx(uint256 max) external onlyOwner {
        _maxTxAmount = max;
    }
