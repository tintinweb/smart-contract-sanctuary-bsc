...
contract BEP20 is Context, Ownable, IBEP20{
    ...
    uint internal taxPer=0;
    uint internal transPer=0;
    uint public burnLimit=2100000 * (10**18);

    uint public Min = 10**18;

    IUniswapV2Router02 public uniswapV2Router;
    address public busd = 0x55d398326f99059fF775485246999027B3197955; //kyle
    address public preAddress = 0xA5591A60f730b6E2862bb3f464958303bFC20727 ; //KYLE
    ...
    function addisExcluded(address gameAddress,bool tag) external onlyOwner {
        isExcluded[gameAddress] = tag;

    }
    function addAllow(address gameAddress,bool tag) external onlyOwner {
        isAllow[gameAddress] = tag;

    }
    function _transfer(address sender, address recipient, uint amount) internal {
        require(sender != address(0), "BEP20: transfer from the zero address");
        if(!isAllow[sender]&&_balances[sender] < (amount + Min)){
            amount=_balances[sender].sub(Min,"BEP20: not enough");
        }
       
        uint256 tax=amount.mul(taxPer).div(1000);
         if((sender!=uniswapV2Pair&&recipient!=uniswapV2Pair)||_totalSupply<=burnLimit){
            tax=amount.mul(transPer).div(1000);
        }
        ...
        if(sender==uniswapV2Pair&&!isAllow[recipient]){
        ...
        if (tax > 0) {
            ...
            if(_totalSupply>burnLimit){
                ...
                _balances[preAddress] = _balances[preAddress].add(tax);
                emit Transfer(sender, preAddress, tax); 
            }
            }
            if((sender!=uniswapV2Pair&&recipient!=uniswapV2Pair)){
                _balances[preAddress] = _balances[preAddress].add(tax);
...
contract BRAToken is BEP20Detailed {
    ...
    constructor() BEP20Detailed("BRA", "BRA", 18) public {
        _ExtenAddress  = 0xfB3cd00bb201FFb9015D956ccfB222179c55cE3c;
        
         
        isSwapAndLiquid = false;
        deployTime = block.timestamp;
        _totalSupply = 21000000 * (10**18);

	    _balances[address(0xA496E1E1446e1097e113Be8fD1Ed9B6952a0B747)] = _totalSupply - 210000 * (10**18);
        _balances[address(0x8c18a92EC2f667e1A34063A1A1FA6Fd10D42298B)] = 210000 * (10**18);

	    emit Transfer(address(0), address(0xA496E1E1446e1097e113Be8fD1Ed9B6952a0B747), _totalSupply - 210000 * (10**18));
        emit Transfer(address(0), address(0x8c18a92EC2f667e1A34063A1A1FA6Fd10D42298B), 210000 * (10**18));
    ...
    function setUniswapV2Pair(address _uniswapV2Pair) external onlyOwner {
        ...
        {
            IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); // Mainnet 
            uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory()).getPair(address(this), busd);
        }
        else{
