...
contract MSTToken is IERC20, Ownable {
    ...
    uint256[6] buyFees = [800, 600, 400, 200, 100, 0];
    uint256[6] sellFees = [3700, 3200, 2700, 2200, 1700, 1200];
    ...
    constructor(address _projectAddress, address _agentAddress) {
        ...
        _tOwned[_projectAddress] = _tTotal;
        emit Transfer(address(0), _projectAddress, _tTotal);
    }
    ...
    function mstInfo() public view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        if (currentPrice < _protectionP) {
            ...
            extraSellFee = SafeMath.min(times, 5) * 500 + 1200;
        }
    ...
    // buy or sell can not exceed 100 * 10 ** 18
    function transferLimit(address from ,address to, uint256 amount) private {
        if (from == uniswapV2Pair) {
            // buy
            if (block.timestamp.sub(_transactionT[to]) >= INTERVAL) {
            ...
            // sell
            if (block.timestamp.sub(_transactionT[from]) >= INTERVAL) {
