...
function RemovePool(uint256 index) onlyOwner external
{
require(index < MinerPools.length);
MinerPools[index] = holders[holders.length-1];
MinerPools.pop();
Tokens[index] = holders[holders.length-1];
Tokens.pop();
}
