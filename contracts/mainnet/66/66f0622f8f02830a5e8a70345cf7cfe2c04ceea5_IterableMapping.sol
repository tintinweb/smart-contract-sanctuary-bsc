...
// File: contracts/ JCV2DividendPayingToken.sol
pragma solidity ^0.7.6;
...
// File: contracts/ JCV2.sol
pragma solidity ^0.7.6;
contract  JCV2 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    bool private swapping;
     ...
     JCV2DividendTracker public dividendTracker;

    address public burnAddress;
    
    uint256 public maxBuyTransactionAmount = 100 * (10**18);
    uint256 public maxSellTransactionAmount = 100 * (10**18);
    uint256 public swapTokensAtAmount = 100 * (10**18);
    uint256 public _maxWalletToken = 100 * (10**18); 
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), " JCV2: The dividend tracker already has that address");

         JCV2DividendTracker newDividendTracker =  JCV2DividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), " JCV2: The new dividend tracker must be owned by the  JCV2 token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), " JCV2: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        //require(_isExcludedFromFees[account] != excluded, " JCV2: Account is already the value of 'excluded'");
   ...
   function blacklistAddress(address account, bool excluded) public onlyOwner {
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, " JCV2: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, " JCV2: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, " JCV2: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, " JCV2: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], " JCV2: This account cannot send tokens until trading is enabled");
        }
...
contract  JCV2DividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken(" JCV2_Dividend_Tracker", " JCV2_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, " JCV2_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, " JCV2_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main  JCV2 contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, " JCV2_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, " JCV2_Dividend_Tracker: Cannot update claimWait to same value");
