...
contract StonkFeesV2 is Ownable {
    ...
    */
    function distributeFees() public {
        ...
        uint newBalanceRatio = newStonksBalance * 100 / totalStakedStonks;
        ...
        if (newBalanceRatio < 100) {
            newBalanceRatio = 100;
            newStonksBalance = totalStakedStonks;
        }
        ...
        for (uint i = 0; i < stakersAddresses.length; i++) {
            ...
            uint stakerNewAmount = stakers[stakersAddress].amount * newBalanceRatio / 100;
            if (stakerNewAmount > stakers[stakersAddress].amount) {
                stakers[stakersAddress].amount = stakerNewAmount;
            }
        }
    ...
    */
    ...
    function addStaker(uint amount, address receiver) internal {
        if (stakers[receiver].exists == 0) {
            ...
            stakers[receiver].amount += amount;
        }

        totalStakedStonks += amount;
    }
