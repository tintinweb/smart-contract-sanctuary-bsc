...
TG: https://t.me/ELONFANTOKENPORTAL
...
contract EFT is ERC20, Ownable {
    ...
    constructor() ERC20("ELON FAN TOKEN", "EFT") {
        marketingWallet = payable(0xC4A8175D32b8dBa6BffB4F42943EF956b806F8eB);
        devWallet = payable(0x8AF8FA4b4b040E4ab055c39bf8EBa798356513B0);
...
contract EFTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "EFT_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EFT contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(EFTContract);

            uint256 prevBalance = EFTContract.balanceOf(address(this));
            ...
            {
                uint256 received = EFTContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    EFTContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
