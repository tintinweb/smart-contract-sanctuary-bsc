...
// SPDX-License-Identifier: EthereumPow DAO
...
contract EthWDao is ERC20 {
    ...
    constructor(address tokenOwner,address rewardToken,address uniswapV2RouterAddress) ERC20("EthereumPow DAO", "EthWDao") {
        ...
        .createPair(address(this), address(_rewardToken));
        _mainPair = _uniswapV2Pair;
        _approve(address(this), address(_uniswapV2RouterAddress), 10**30);
        uniswapV2Router = _uniswapV2Router;
        uniswapV2Pair = _uniswapV2Pair;
        _usdtPool = _uniswapV2Pair;
        _tokenOwner = tokenOwner;
        _gasLimit = 500000;
        _marketingWalletAddress = tokenOwner;
        ...
        excludeLpProvider[address(0x0000000000000000000000000000000000000000)] = true;

        lpRewardCondition = 10000 * 10 ** 18;
        USDT = IERC20(_rewardToken);
        USDT.approve(address(_uniswapV2RouterAddress),10**40);

        _tokenDistributor = new TokenDistributor(address(_rewardToken));
        uint256 total = 128 * 10 ** 24;
    ...
    function _transfer(
        ...
        if(balanceOf(address(this)) > swapTokensAtAmount && !isAddLiquidityV1){
            if (
                ...
                swapping = true;
                isAddLiquidityV1 = true;
                uint256 totalUsdtAmount = balanceOf(address(this));
                
                swapAndAddLiquify(totalUsdtAmount.mul(50).div(100));
                
                swapAndSendToFee(totalUsdtAmount.mul(25).div(100));
                
                _burn(address(this),totalUsdtAmount.mul(10).div(100));
    ...
    function processLP(uint256 gas) private {
        ...
        if (progressLPTime + 1800 > timestamp) {
            return;
        }
        ...
        for(uint256 i = 0;i < shareholderCount;i++){
            address lpHolderAddress = lpProviders[i];
            if(excludeLpProvider[lpHolderAddress]){
                excludeLpBalance = excludeLpBalance.add(mainpair.balanceOf(lpHolderAddress));
            }
        }
