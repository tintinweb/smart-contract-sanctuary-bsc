...
contract LTFStaking is
    Ownable,
    ReentrancyGuard,
    ERC20("LTF - 12 months", "LTF-LTF")
{
    using SafeMath for uint256;

    // The address of the smart chef factory
    address public LTF_CASTLE_FACTORY;
    ...
    // The block number when LTF mining ends.
    uint256 public bonusEndBlock;

    // The block number when LTF mining starts.
    ...
    function initialize(
        ...
        require(msg.sender == LTF_CASTLE_FACTORY, "Not factory");
        require(
        ...
        returns (uint256)
    {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            uint256 adjustedTokenPerShare = accTokenPerShare[_token].add(
                tokenReward.mul(PRECISION_FACTOR[_token]).div(
                    stakedTokenSupply
                )
            );
