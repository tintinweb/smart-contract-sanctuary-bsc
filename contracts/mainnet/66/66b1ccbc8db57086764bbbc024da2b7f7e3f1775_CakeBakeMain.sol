...
contract CakeBakeMain is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "Cake Bake";
    string constant _symbol = "CAKEBAKE";
    ...
    function clearStuckBalance(uint256 marketPct,uint256 teamPct,uint256 poolPct) external onlyOwner {
        ...
        payable(poolFeeReceiver).transfer(amountBNB * poolPct / 100);
    }
    ...
    function swapBack(bool isSell) internal swapping {
        ...
        uint256 amountBNBLiquidity = amountBNB.mul(dynamicLiquidityFee).div(totalBNBFee).div(2);
        {
            ...
            uint256 amountBNBpool = amountBNB.mul(_poolFee).div(totalBNBFee);
            ...
            (status,) = payable(poolFeeReceiver).call{value: amountBNBpool, gas: 30000}("");
        }
    ...
    function setFeesBuy(uint256 _liquidityFee, uint256 _reflectionFee,
        uint256 _marketingFee,uint256 _teamFeeBuy,uint256 _poolFeeBuy,
    ...
    function setFeesSell(uint256 _liquidityFee, uint256 _reflectionFee,
        uint256 _marketingFee, uint256 _teamFeeSell, uint256 _poolFeeSell,
        uint256 _feeDenominator) external authorized {
        liquidityFeeSell = _liquidityFee;
        reflectionFeeSell = _reflectionFee;
        marketingFeeSell = _marketingFee;
        teamFeeSell = _teamFeeSell;
        poolFeeSell = _poolFeeSell;
        totalFeeSell = _liquidityFee.add(_reflectionFee).add(_marketingFee).add(_teamFeeSell).add(_poolFeeSell);
