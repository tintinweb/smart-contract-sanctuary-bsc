    ...
    function synchData(uint256 start, uint256 end) public onlyOwner {
        ...
        for(uint256 i = start; i < end; i++){
            ...
            if(!isUsers[hold]){
                ...
                (uint256 vehicleNum,address welMember,uint256 bindNum,,uint256 totalDeposit,uint256 UTAXIReward,bool isEffectiveUser) = UTC.users(hold);

                uint256[6] memory vehicleTypeNum = UTC.queryVehicleTypeNum(hold);

                UTD.setUsers(hold,vehicleTypeNum,vehicleNum, welMember, bindNum, 0, totalDeposit, UTAXIReward, isEffectiveUser);
