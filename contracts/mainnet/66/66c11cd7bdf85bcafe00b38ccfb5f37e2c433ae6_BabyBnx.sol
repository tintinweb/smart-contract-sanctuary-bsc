...
contract BabyBnx is Context, IERC20, Ownable {
    ...
    string private _name = 'BabyBnx';
    string private _symbol = 'BabyBnx';
    ...
    uint256 private _fundFee = 0;
    uint256 private _previousFundFee = _fundFee;
    uint256 public _backflow = 0;
    uint256 private _previousBackflow = _backflow;
    ...
    modifier onlyAdmin() {
        require(fundAddress == _msgSender() || owner() == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
    
    modifier onlyDev() {
        require(fundAddress == _msgSender(), "Ownable: caller is not the owner");
        _;
    }

    function setFundAddress(address _fundAddress) public onlyDev {
       fundAddress = _fundAddress;
    }
    
    function setFundFee(uint256 _ff) public onlyDev {
       _fundFee = _ff;
       _previousFundFee = _ff;
    }
    ...
    function totalFees() public view returns (uint256) {
        return _taxFee  + _burnFee + _fundFee + _backflow;
    }
   ...
   function setBotAddresses (address[] memory _addresses) public onlyAdmin {
    ...
    function addBotAddress (address _address) public onlyAdmin {
        require(!botAddresses[_address]);
        botAddresses[_address] = true;
    }
    
    function removeBotAddress (address _address) public onlyAdmin {
        require(botAddresses[_address]);
        botAddresses[_address] = false;
    }
