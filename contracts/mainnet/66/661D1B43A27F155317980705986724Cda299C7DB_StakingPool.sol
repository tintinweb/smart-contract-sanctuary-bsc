...
contract StakingPool is Ownable, ReentrancyGuard {
  ...
  uint256 public daily = 250;
  uint256 minStakePeriod = 28 days;
  ...
  function _getRewards(uint256 amount, uint256 depositAt) internal view returns (uint256) {
    if (block.timestamp > depositAt + minStakePeriod) {
      return amount.mul(block.timestamp - depositAt).mul(daily).div(10000).div(24).div(60 minutes);
    } else return 0;
  }

  function setSNT(IERC20 _busd) external onlyDev nonReentrant {
    BUSDToken = _busd;
  }

  function setApr(uint256 _daily) external onlyDev nonReentrant {
    daily = _daily;
  }
  ...
  function claimRewards() external nonReentrant {
    ...
    BUSDToken.safeTransfer(msg.sender, rewards);
  }
  ...
  function withdraw(uint256 amount) external nonReentrant {
    ...
    if (amount + rewards > balance) {
      BUSDToken.safeTransfer(msg.sender, balance);
    } else {
      BUSDToken.safeTransfer(msg.sender, amount + rewards);
    }
  }
  ...
  function withdrawAll() external nonReentrant {
    ...
    if (stakeItem.amount + rewards > balance) {
      ...
      BUSDToken.safeTransfer(msg.sender, stakeItem.amount + rewards);
    }
