...
contract Staking is Ownable, ReentrancyGuard {
    ...
    updateReward(msg.sender)
  {
    ...
    emit Stake(msg.sender, amount_, balances[msg.sender]);
  }
  ...
  function unstake(uint256 amount_)
    ...
    updateReward(msg.sender)
  {
    require(balances[msg.sender] >= amount_, "Not enought staked");
    totalStaked -= amount_;
    balances[msg.sender] -= amount_;
    _data[block.number] = totalStaked;
    IERC20(token).transfer(msg.sender, amount_);
    emit Unstake(msg.sender, amount_, balances[msg.sender]);
  }
