...
contract random is Governance {
    ...
    string[] public urls = ["QmXfuqKwYeYBgnKhHnuxpGRTRCv172f1RWjkPuisGBGLWJ",
                            "QmeoDmUA235TTc1b1vjQkqBdVtrbAQaHgK2EPr9xqVR7ut",
                            "QmQDDnCMqjpkJr1cf8g57k7knqrk3A4MTC2LpDemfFKaR8",
                            "QmQ2Uueyyzt2atiZGTT9LFtRCvBjGcLMNY7y4Cu2Huv3df",
                            "QmVhxRpXvQ3ERFz24hZD4PwfpQq7s4WzhwMYzvPd6kJjHa"];

    uint public desNum = 10*1e18;
    ...
    function getRandom() external onlylock {
        ...
        // require(leftAmounts2 > 0,"left Amounts");
        //     leftAmounts2 = leftAmounts2.sub(1);
        ...
        nfttoken(_nfttoken).mint(msg.sender,urls[_parame],1);
    ...
    function setTimes(uint _period) external onlyGovernance { 
        require(!_periodStart[_period],"_period err");  
        require(_period == curPeriod,"curPeriod err");
        _periodStart[_period] = true;

        startTime = block.timestamp;
        endTime = startTime.add(period);

        getPeriod[curPeriod]._startTime = block.timestamp;
        getPeriod[curPeriod]._endTime = endTime;
    }

    function Reward(uint _period,address[] calldata accunts) external onlyGovernance {
        require(getPeriod[curPeriod]._endTime <= block.timestamp ,"_endTime err");
        require(_period == 0 || _periodStart[_period],"setTimes err");
