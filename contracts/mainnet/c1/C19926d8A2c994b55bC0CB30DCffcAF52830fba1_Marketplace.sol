...
contract Marketplace is AccessControl, ReentrancyGuard {
  ...
  function storeOldPurchase(
    Product[] memory _purchasedProducts,
    Purchase memory _purchase
  ) external onlyRole(ADMIN) {
    for (uint256 i = 0; i < _purchasedProducts.length; i++) {
      purchasedProducts[numberOfPurchases][i] = _purchasedProducts[i];
    }

    purchases[numberOfPurchases] = _purchase;
    purchasesByUser[_purchase.buyer].push(purchases[numberOfPurchases]);
