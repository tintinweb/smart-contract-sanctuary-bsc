...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        totalAllocPoint = allocPoint1.add(allocPoint2).add(allocPoint3);
        treasury = _treasury;
    }
    ...
    function setEarlyWithdrawalFee(uint _fee) external onlyOwner {
        fee = _fee;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            ...
            uint balBefPending = pool.lpToken.balanceOf(address(this));
            safeTokenTransfer(address(this), pending);
            uint balAftPending = pool.lpToken.balanceOf(address(this));
            finalPending = balAftPending.sub(balBefPending);
        }
        ...
        user.amount = user.amount.add(_amount).add(finalPending);
        user.rewardDebt = user.amount.mul(pool.accTokenPerShare).div(1e18);
        pool.balance = pool.balance.add(_amount).add(finalPending);
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(block.timestamp.sub(user.depositTime) >= pool.duration, "Too early to withdraw");
    ...
    function emergencyWithdraw(uint256 _pid) external nonReentrant{
        ...
        uint feeAmount = amount.mul(fee).div(10000);
        pool.lpToken.safeTransfer(treasury, feeAmount);
        pool.lpToken.safeTransfer(msg.sender, amount.sub(feeAmount));
