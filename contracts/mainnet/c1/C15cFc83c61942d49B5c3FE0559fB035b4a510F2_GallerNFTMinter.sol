...
contract GallerNFTMinter is Ownable {

    uint256 private constant MAX_LAUNCHPAD_SUPPLY = 800;

    address public launchpad;
    DragonaireNFTManager public manager;
    uint256 private _launchpadSupply = 0;

    constructor(
        address _launchpad,
        DragonaireNFTManager _manager
    ) {
        require(_launchpad != address(0), "GallerNFTMinter: launchpad is zero address");
    ...
    function getMaxLaunchpadSupply() external view returns (uint256) {
        return MAX_LAUNCHPAD_SUPPLY;
    }

    function getLaunchpadSupply() external view returns (uint256) {
        return _launchpadSupply;
    }

    function setLaunchpad(address value) external onlyOwner {
        launchpad = value;
    }

    function setManager(DragonaireNFTManager value) external onlyOwner {
        manager = value;
    }

    function mintTo(address to, uint256 size) external {
        require(msg.sender == launchpad, "GallerNFTMinter: caller must be launchpad");
        require(_launchpadSupply + size <= MAX_LAUNCHPAD_SUPPLY, "GallerNFTMinter: max supply reached");

        _launchpadSupply += size;

        manager.mintBatch(to, 1, size);
    }
}
    ...
    ItemFactoryManager
{
    ...
    modifier onlySupportedBoxType(uint256 boxType_) {
        require(_supportedBoxTypes.contains(boxType_), "DragonaireNFT: unsupported box type");
        _;
    }

    modifier onlyTokenOwner(uint256 tokenId_) {
        require(nftCore.ownerOf(tokenId_) == _msgSender(), "DragonaireNFT: caller is not owner");
        _;
    }
    ...
    modifier onlyExistedToken(uint256 tokenId_) {
        require(nftCore.exists(tokenId_), "DragonaireNFT: token does not exists");
        _;
    }
    ...
    function setUseChainLinkRandomGen(bool value) public onlyRole(MANAGER_ROLE) {
    ...
    function updateItemFactory(address itemFactory_) public onlyRole(CONTRACT_UPGRADER) {
        require(itemFactory_ != address(0), "DragonaireNFT: itemFactory_ is the zero address");
        _updateItemFactory(itemFactory_);
    }
    ...
    function updateRandomGenerator(address randomGenerator_) public onlyRole(CONTRACT_UPGRADER) {
        require(randomGenerator_ != address(0), "DragonaireNFT: randomGenerator_ is the zero address");
        _updateRandomGenerator(randomGenerator_);
    }
    ...
    function mint(address to_, uint256 artifacts_, bytes32 dna_) public onlyRole(GAME_MASTER_ROLE) returns (uint256 tokenId) {
    ...
    function _doUnbox(uint256 tokenId_) internal onlyExistedToken(tokenId_) onlyTokenOwner(tokenId_) onlyMysteryBox(tokenId_) {
        require(unboxStageOf(tokenId_) == UnboxStage.INITIAL, "DragonaireNFTManager: token unboxing or unboxed");
    ...
    function unboxBatch(uint256[] calldata tokenIds) external {
        require(tokenIds.length > 0 && tokenIds.length <= 20, "DragonaireNFTManager: unboxing amount of tokens should be in 1-20");
     ...
     * 0~7:  boxType, len=8               //盲盒类型
     * 8~15: Level, len=8                 //等级 都是一级
     * 16~23: ItemType, len = 8           //龙类型 0x01 三星龙 0x02 五星龙
     * 24~39: DragonaireId, len=16        //转十进制 ==> 龙ID
     * 40~47: ItemType, len=8             //龙类型 0x01 三星龙 0x02 五星龙
     * 48~55: star, len=8                 //星级
     * ...
     *
     * dna format:
    ...
    function updateCapability(uint256 tokenId, uint256 capability) external onlyRole(GAME_MASTER_ROLE) {
        nftCore.updateCapability(tokenId, capability);
    }

    function updateCapabilityInBatch(DragonaireNFTCore.CapabilityUpdateRequest[] calldata requests) external onlyRole(GAME_MASTER_ROLE) {
