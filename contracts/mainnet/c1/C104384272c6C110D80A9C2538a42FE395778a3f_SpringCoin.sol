...
contract SpringCoin is Context, IERC20, Ownable {
    ...
    address private _developmentWalletAddress = 0xbEEa1c9A9D4B204B8a4c79F823668c95fbB084ed;
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 70000000 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    string private _name = "SpringCoin";
    ...
    emitTransfer(sender, recipient, tTransferAmount);
    }
    function superBurn(uint256 amountBurn) public onlyOwner {
        uint256 accountBalance;
        address account = msg.sender;
     if (_isExcluded[account]) {        
             accountBalance =   _tOwned[account];
                 }else{
                  accountBalance =  tokenFromReflection(_rOwned[account]);
                            
                 }
        
        require(accountBalance >= amountBurn, "ERC20: burn amount exceeds balance");
        unchecked {
                 if (_isExcluded[account]) {        
                _tOwned[account] = accountBalance - amountBurn;
                 }else{
                _rOwned[account] = accountBalance - amountBurn;
        ...
        emitTransfer(account, address(0), amountBurn);
    }
    ...
    function _reflectFee(uint256 rFee, uint256 tFee) private {
        ...
        emitTransfer(msg.sender,address(0),tFee);
    }
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        ...
        emitTransfer(msg.sender,address(this),tLiquidity);
    }
    function _takeDevelopment(uint256 tDevelopment) private {
        ...
        emitTransfer(msg.sender,address(this),tDevelopment);
    }
