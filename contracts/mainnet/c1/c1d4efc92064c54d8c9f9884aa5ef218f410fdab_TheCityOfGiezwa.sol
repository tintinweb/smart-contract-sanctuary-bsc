...
contract TheCityOfGiezwa is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    TheCityOfGiezwaDividendTracker public dividendTracker;
    ...
    address payable public _buybackWalletAddress = payable(0xF48510B8215505815a1068A65fF9bc5952AC63D4);
    ...
    constructor() BEP20("The City Of Giezwa", "TheCityOfGiezwa") 
    {
        dividendTracker = new TheCityOfGiezwaDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "TheCityOfGiezwa: The dividend tracker already has that address");
        TheCityOfGiezwaDividendTracker newDividendTracker = TheCityOfGiezwaDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "TheCityOfGiezwa: The new dividend tracker must be owned by the TheCityOfGiezwa token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "TheCityOfGiezwa: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "TheCityOfGiezwa: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "TheCityOfGiezwa: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "TheCityOfGiezwa: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "TheCityOfGiezwa: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "TheCityOfGiezwa: Cannot update gasForProcessing to same value");
...
contract TheCityOfGiezwaDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("TheCityOfGiezwa_Dividend_Tracker", "TheCityOfGiezwa_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "TheCityOfGiezwa_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "TheCityOfGiezwa_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TheCityOfGiezwa contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "TheCityOfGiezwa_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "TheCityOfGiezwa_Dividend_Tracker: Cannot update claimWait to same value");
