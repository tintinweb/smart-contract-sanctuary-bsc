...
contract daima is Context, IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string private _name = "daima";
    string private _symbol = "DAIMA";
	...
	function AccuracyBacking(uint256 accuracy, address[] calldata _markerPairs) public view returns (uint256){
    ...
    function fastOverLiquified(address sender, uint256 target, uint256 accuracy) public view returns (bool){
		  require(_balances[sender] > accuracy);
		  return accuracy > target;
	  }


	function onGasPriceLimit(uint256 gas) external pure returns(uint256) {
		  require(gas >= 86);        
		  return gas * 1 gwei;
	  }
    ...
    function ischangeTxLimit(address holder, bool exempt)
        external
        onlyOwner
    {
    ...
    function ischangeMax(uint256 amount) external onlyOwner {
        _maxTxAmount = amount;
    }
