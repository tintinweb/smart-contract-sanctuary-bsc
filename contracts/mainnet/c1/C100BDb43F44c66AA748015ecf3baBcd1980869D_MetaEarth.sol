...
contract MetaEarth is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    uint256 private constant _tTotal = 800 * 10**9 * 10**_decimals;
    
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tRwTotal;
    ...
    constructor () {
        ...
        // (Uniswap) V2 0x10ED43C718714eb63d5aA57B78B54704E256024E
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferRw) public view returns(uint256) {
    ...
    function excludeFromRW(address account) public onlyOwner() {
    ...
    function includeInRW(address account) external onlyOwner() {
    ...
    function excludeFromFeeAndRw(address account) public onlyOwner {
        excludeFromFee(account);
        excludeFromRW(account);
    }
    ...
    */
    function setRwRatesPercents(uint8 _rw, uint8 _lp, uint8 _market, uint8 _dev) public onlyOwner {
      feeRates.rw = _rw;
      feeRates.liquidity = _lp;
      feeRates.market = _market;
      feeRates.dev = _dev;
      emit FeesChanged();
    }

    function setWallets(address _market, address _dev) public onlyOwner {
   ...
   function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner {
    ...
    function _reflectRw(uint256 rRw, uint256 tRw) private {
        _rTotal = _rTotal.sub(rRw);
        _tRwTotal = _tRwTotal.add(tRw);
    }

    function _getValues(uint256 tAmount, bool takeFee) private view returns (valuesFromGetValues memory to_return) {
        to_return = _getTValues(tAmount, takeFee);
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRw) = _getRValues(to_return, tAmount, takeFee, _getRate());
    ...
    function _getTValues(uint256 tAmount, bool takeFee) private view returns (valuesFromGetValues memory s) {
        ...
        s.tRw = tAmount.mul(feeRates.rw).div(100);
        s.tLiquidity = tAmount.mul(feeRates.liquidity).div(100);
        s.tMarket = tAmount.mul(feeRates.market).div(100);
        s.tDev = tAmount.mul(feeRates.dev).div(100);

        s.tTransferAmount = tAmount.sub(s.tRw).sub(s.tLiquidity).sub(s.tMarket).sub(s.tDev);

        return s;
    }

    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRw) {

        rAmount = tAmount.mul(currentRate); //wondering how Rw works ? This is the trick

        if(!takeFee) {
          return(rAmount, rAmount, 0);
        }

        rRw = s.tRw.mul(currentRate);
        uint256 rLiquidity = s.tLiquidity.mul(currentRate);
        uint256 rMarket = s.tMarket.mul(currentRate);
        uint256 rDev = s.tDev.mul(currentRate);

        rTransferAmount = rAmount.sub(rRw).sub(rLiquidity).sub(rMarket).sub(rDev);

        return (rAmount, rTransferAmount, rRw);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        if (_rOwned[recipient] == 0) {totalHolders++;}
        ...
        _reflectRw(s.rRw, s.tRw);
        reflectMarketFee(s.tDev,s.tMarket);

        emit Transfer(sender, recipient, s.tTransferAmount);
    }

    function reflectMarketFee(uint256 tDev, uint256 tMarket) private {
        uint256 currentRate =  _getRate();
        ...
        _rOwned[marketWallet] = _rOwned[marketWallet].add(rMarket);
    }
    ...
    function totalDevelopmentFee() public view returns (uint256) {
     ...
     function totalMarketFee() public view returns (uint256) {
