...
contract Farm is Ownable {
  ...
  function add(
    ...
    require(_earlyWithdrawFee <= 100, 'earlyWithdrawFee must be <= 100');
    require(_allocPoint > 0, 'allocPoint must be > 0');
    require(_lockPeriod >= 0, 'lockPeriod must be >= 0');
  ...
  function _rewardUser(uint256 _pid, address _address) internal {
    ...
    if (user.amount > 0) {
      ...
      if (
        ...
        uint256 fee = pendingAmount.mul(pool.earlyWithdrawFee).div(100);
        erc20Transfer(owner(), fee);
        rewards = pendingAmount.sub(fee);
        emit CollectFee(_address, owner(), _pid, fee);
      }
...
contract FarmFactory is Ownable {
  ...
  Farm[] public farms;
  Farm[] public pendingFarms;
  Farm[] public listedFarms;
  Farm[] public deniedFarms;
  ...
  constructor(address payable _serviceFeeReceiver, uint256 _serviceFee) {
    serviceFeeReceiver = _serviceFeeReceiver;
    serviceFee = _serviceFee;
  }

  function deploy(CreateFarmParams memory info) public payable {
    Farm farm = new Farm(info.erc20, info.rewardPerBlock, info.startBlock);
    ...
    farms.push(farm);
    pendingFarms.push(farm);
    isPending[address(farm)] = true;

    emit FarmCreated(msg.sender, address(info.erc20), address(farm));

    (bool sent, ) = serviceFeeReceiver.call{value: serviceFee}('');
  ...
  function setServiceFeeReceiver(address _serviceFeeReceiver)
    external
    onlyOwner
  {
    serviceFeeReceiver = _serviceFeeReceiver;
  ...
  function listedFarmsLength() external view returns (uint256) {
    return listedFarms.length;
  }

  function deniedFarmsLength() external view returns (uint256) {
    return deniedFarms.length;
  }

  function pendingFarmsLength() external view returns (uint256) {
    return pendingFarms.length;
  }

  function _excludeFromListedFarms(uint256 index) internal {
    require(index < listedFarms.length);
    listedFarms[index] = listedFarms[listedFarms.length - 1];
    listedFarms.pop();
  }

  function _excludeFromDeniedFarms(uint256 index) internal {
    require(index < deniedFarms.length);
    deniedFarms[index] = deniedFarms[deniedFarms.length - 1];
    deniedFarms.pop();
  }

  function _excludeFromPendingFarms(uint256 index) internal {
    require(index < pendingFarms.length);
    pendingFarms[index] = pendingFarms[pendingFarms.length - 1];
    pendingFarms.pop();
  }

  function _findFarmIndex(Farm[] memory array, address farmAddress)
    internal
    pure
    returns (bool, uint256)
  {
    ...
    for (index = 0; index < array.length; index++) {
      if (address(array[index]) == farmAddress) {
        farm = array[index];
        found = true;
        break;
      }
  ...
  function changeFarmListingStatus(address farmAddress, bool status)
    external
    onlyOwner
  {
    bool wasPending = isPending[farmAddress];
    bool wasListed = isListed[farmAddress];
    bool wasDenied = isDenied[farmAddress];

    if (status) {
      require(!wasListed, 'Farm is already listed');
    } else {
      require(!wasDenied, 'Farm is already denied');
    }

    if (wasPending) {
      (bool found, uint256 index) = _findFarmIndex(pendingFarms, farmAddress);
      if (found) {
        _excludeFromPendingFarms(index);
        isPending[farmAddress] = false;
      }
    }

    isListed[farmAddress] = status;
    isDenied[farmAddress] = !status;

    if (status) {
      if (wasDenied) {
        (bool found, uint256 index) = _findFarmIndex(deniedFarms, farmAddress);
        if (found) {
          _excludeFromDeniedFarms(index);
        }
      }
      listedFarms.push(Farm(farmAddress));
    } else {
      if (wasListed) {
        (bool found, uint256 index) = _findFarmIndex(listedFarms, farmAddress);
        if (found) {
          _excludeFromListedFarms(index);
        }
      }
      deniedFarms.push(Farm(farmAddress));
    }
  }
}