...
contract CoinToken is Context, IBEP20, Ownable {
    ...
    uint256 public constant limitAmount = 200*10**18;
    uint256 public endLimitTime; 
    ...
    event SetMarketMakerPair(address indexed pair, bool indexed value);

    constructor (string memory _name, string memory _symbol, uint256 _supply, uint256 _burnFee,address _FeeAddress,address tokenOwner,address service, address _router, address _usdtAddr, uint256 _endLimitTime) payable   {
		_NAME = _name;
		_SYMBOL = _symbol;
		_DECIMALS = 18;
        ...
        _setMarketMakerPair(_uniswapV2Pair, true);
    ...
    function setMarketMakerPair(address pair, bool value)
        ...
        onlyOwner
    {
        require(pair != uniswapV2Pair, "PancakeSwap pair cannot be removed from marketMakerPairs");

        _setMarketMakerPair(pair, value);
    }

    function _setMarketMakerPair(address pair, bool value) private {
        require(marketMakerPairs[pair] != value, "Market maker pair is already set to that value");
        marketMakerPairs[pair] = value;

        emit SetMarketMakerPair(pair, value);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (endLimitTime >= duration) {
            if (FeeAddress != sender && FeeAddress != recipient) {
                if (marketMakerPairs[sender]) {
                    burnAmount = amount.mul(2).div(100);
                    amount = amount.sub(burnAmount);
                    buyedTokens[recipient] = buyedTokens[recipient].add(amount);
                    require(buyedTokens[recipient] <= limitAmount, "exceeds limit");
                }
            }
        }
