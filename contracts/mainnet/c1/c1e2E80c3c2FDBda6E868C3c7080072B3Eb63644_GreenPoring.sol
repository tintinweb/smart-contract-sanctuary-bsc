...
contract GreenPoring is JellopyWrapper, Ownable {
    ...
    function notifyRewardAmountAlreadySent() external onlyOwner updateReward(address(0)) {
        uint256 balance = rewardToken.balanceOf(address(this));
        uint256 userRewards = rewardBalance;

        uint256 reward = balance - userRewards; // increase in reward
        ...
        if (block.timestamp >= periodFinish) {
            ...
            // if period is not over
            // check increase in reward
            uint256 remaining = periodFinish - block.timestamp;
            uint256 leftover = remaining * rewardRate;
            rewardRate = (reward + leftover) / duration;
        }
