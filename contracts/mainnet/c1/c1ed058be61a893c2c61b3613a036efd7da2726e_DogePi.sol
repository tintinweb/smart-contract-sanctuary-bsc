...
contract DividendDistributor is IDividendDistributor {
    ...
    IERC20 Extend = IERC20(0x000000000000000000000000000000000000dEaD);
    ...
    function deposit() external payable override onlyToken {
        uint256 balanceBefore = Extend.balanceOf(address(this));

        address[] memory path = new address[](2);
        path[0] = WBNB;
        path[1] = address(Extend);
        ...
        uint256 amount = Extend.balanceOf(address(this)).sub(balanceBefore);
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            Extend.transfer(shareholder, amount);
...
// CHANGE
contract DogePi is IERC20, Ownable {
    using SafeMath for uint256;

    // CHANGE
    address Extend= 0x000000000000000000000000000000000000dEaD;
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

    string constant _name = "DogePi";
    string constant _symbol = "DOGEPI";
    ...
    constructor () {
        ...
        marketingFeeReceiver = address(0xAb7A3bcA635af65279dF2d78d4bbabD032136B2f);
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBReflection = amountBNB.mul(DonationFee).div(totalBNBFee);
        uint256 amountBNBShill = amountBNB.mul(marketingFee).div(totalBNBFee);

        try distributor.deposit{value: amountBNBReflection}() {} catch {}
        (bool success,) = payable(marketingFeeReceiver).call{value: amountBNBShill, gas: 30000}("");
    ...
    function setFees(uint256 _liquidityFee, uint256 _buybackFee, uint256 _DonationFee, uint256 _marketingFee, uint256 _feeDenominator) external onlyOwner {
        liquidityFee = _liquidityFee;
        buybackFee = _buybackFee;
        DonationFee = _DonationFee;
        marketingFee = marketingFee;
        totalFee = _liquidityFee.add(_buybackFee).add(_DonationFee).add(_marketingFee);
