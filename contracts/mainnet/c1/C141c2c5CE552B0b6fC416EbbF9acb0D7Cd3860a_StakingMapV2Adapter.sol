...
contract StakingMapV2Adapter is Ownable {
    ...
    function getStakes(address _userAddress) external view returns (StakingMapV2Contract.StakeOutput[] memory _stakes, uint256[] memory _indexes){
        ...
        for (uint256 i = 0; i < _stakes.length; i++) {
            _stakes[i].bonusPercents = _calculateBonuses(_stakes[i].createdAt, _stakes[i].age, _stakes[i].currentPos);
        }
    }
    ...
    function _calculateBonuses(uint32 _createdAt, uint32 _age, uint32 _currentPos) internal view returns (uint32[] memory){
        ...
        for (uint256 i = 0; i < MAPS.length; i++) {
            ...
            if ((_createdAt - _age) < START_TIMESTAMP + MAPS[i].period) {
                // If stake finished map
                if ((_currentPos) > MAPS[i].period) {
