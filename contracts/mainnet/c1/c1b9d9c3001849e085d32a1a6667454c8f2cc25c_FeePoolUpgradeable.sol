...
mapping(string => uint256) public feesInCurrentEpoch;
    ...
    function getConfirmedStartEpochFee(
        ...
        uint256 lpsFee = getLPsFee(totalFeesCollected, params._tokenTicker, params.epochStartIndex-1);
        uint256 feeEarned = (params.depositedAmount * liquidityBlocks * lpsFee) / (totalActiveLiquidity * epochLength);
    ...
    function getLPsFee(
        ...
        uint256 totalBoostedUsers = bridgeUpgradeable.getEpochTotalDepositors(tokenTicker, epochIndex);

        uint256 noOfDepositors = tokenBridgeRegistryUpgradeable.getNoOfDepositors(tokenTicker);
    ...
    function getConfirmedEpochsFee(
        ...
        uint256 lpsFee = getLPsFee(totalFeesCollected, _tokenTicker, _index);
        uint256 epochFee = (depositedAmount * lpsFee / totalActiveLiquidity);
    ...
    function getAdminTokenFees(
        ...
        for (uint256 epochIndex = adminClaimedTillEpoch[_tokenTicker] + 1; epochIndex < epochsLength; epochIndex++) {
            ...
            uint256 totalBoostedUsers = bridgeUpgradeable.getEpochTotalDepositors(_tokenTicker, epochIndex);
...
contract BridgeUpgradeable is Initializable, OwnableUpgradeable, RouterCrossTalkUpgradeable {
    ...
    function calculatePassedEpoch(string memory _tokenTicker) internal returns (uint256, uint256) {
        ...
        if(block.number > nextEpochBlock[_tokenTicker]) {
            ...
            if(epochs[_tokenTicker].length > 0) {
                ...
                nextEpochBlock[_tokenTicker] = epochStartBlock + (passedEpochs + 1) * epochLength - 1;
            }
        }
    ...
    function addPassedEpochs(
        ...
        for (uint256 index = 0; index < passedEpochs - 1; index++) {
    ...
    function crossChainTransferOut(
        ...
        // addTransferMap(_tokenTicker, _noOfTokens);
        // return (true, transferIndex);
    }

    // function addTransferMap(string memory _tokenTicker, uint256 _noOfTokens) internal {
    ...
    function transferOut(
        ...
        if(bridgeType == 0) {
            ...
            if(noOfTokens < feesDeducted) {
                ...
                token.transfer(_userAddress, noOfTokens);
                token.mintTokens(_userAddress, _noOfTokens - noOfTokens - feesDeducted);
                if(feesDeducted > 0) {
    ...
    function getEpochTotalDepositors(
        ...
        return epochs[_tokenTicker][_epochIndex].noOfDepositors;
    } 
