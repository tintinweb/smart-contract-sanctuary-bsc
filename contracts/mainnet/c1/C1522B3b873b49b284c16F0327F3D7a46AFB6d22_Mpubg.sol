 ...
 */
contract Mpubg is ERC20 {

    using SafeMath for uint256;

    address public owner;
    address public pubgWbnbPair;
    ...
    constructor(string memory tokenName, string memory tokenSymbol) ERC20(tokenName, tokenSymbol) {
        owner = _msgSender();
        _mint(owner, 2000 * 10 ** 7 * 10 ** 18);
    }

    function init(address _router, address _usdt) public {
        ...
        address _pubgWbnbPair = IUniswapV2Factory(uniswapV2Router.factory()).getPair(address(this), wbnb);
        if (pubgWbnbPair != _pubgWbnbPair) {
            pubgWbnbPair = _pubgWbnbPair;
        }
    ...
    function getLpPrice() public view returns(uint256) {
        (uint112 pwreserve0, uint112 pwreserve1, ) = IUniswapV2Pair(pubgWbnbPair).getReserves();
        if(pwreserve0 == 0 || pwreserve1 == 0){
            return 0;
        }

        address pwtoken0 = IUniswapV2Pair(pubgWbnbPair).token0();
        uint256 pubgPriceInWbnb;
        if(pwtoken0 == address(this)){
            pubgPriceInWbnb = uint256(pwreserve1).mul(priceMagnitude).div(uint256(pwreserve0));
        } else {
            pubgPriceInWbnb = uint256(pwreserve0).mul(priceMagnitude).div(uint256(pwreserve1));
        }
        ...
        return pubgPriceInWbnb.mul(wbnbPriceInUsdt).div(priceMagnitude);
    }
}