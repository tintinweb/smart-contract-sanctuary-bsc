...
contract Cbbb is Context, IBEP20, Ownable {
    ...
    uint256 private _tTotal = 260 * 10**3 * 10**18;
    uint256 private finalTotalToken = 26 * 10**3 * 10**18;
    string private _name = "Cbbb";
    string private _symbol = "Cbbb";
    ...
    uint256 public _maxTxAmount =  2 * 10**3 * 10**18;
    uint256 public numTokensToSwap =  3 * 10**2 * 10**18;
    ...
    function _transfer(
        ...
        if (to == pancakeswapV2Pair && balanceOf(pancakeswapV2Pair) == 0) {//to 是pair合约则是购买 需要结束预售才可以开启
            require(addLiquifyEnabled == true, "You are not allowed to add liquidity now,Please wait");
        }
        if(
    ...
    function dealWithShareBonus(uint256 tokenBalance) private  {
        if(_tTotal>finalTotalToken){
            ...
            if(burnBonus>0){
                ...
                tokenBalance= tokenBalance.sub(burnBonus);
            }
        }
        if(tokenBalance>0 && backAddress!=address(0)){
             _balances[backAddress] = _balances[backAddress].add(tokenBalance);   
        }
    }
