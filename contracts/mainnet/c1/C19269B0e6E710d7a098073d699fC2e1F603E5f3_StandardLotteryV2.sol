...
abstract contract MCGLotterysUpgradeable is
	Initializable,
	OwnableUpgradeable,
...
abstract contract MCGLotterysAbstract is
  MCGLotterysUpgradeable,
  IMCGRandConsumer
{
  using SafeERC20Upgradeable for IERC20Upgradeable;
  using AddressUpgradeable for address;
  ...
  uint256 public maxPriceTicketInMCG;
  uint256 public minPriceTicketInMCG;
  ...
  IERC20Upgradeable public mcgToken;

  IMCGRand public randomGenerator;
   ...
   * @dev Callable by owner
   */
  function setRandomGenerator(IMCGRand _address) external onlyOwner {
  ...
  /**
   * @notice Transfers $MCG to address
   * @param _addr destination address
   * @param _amount $MCG token amount
  ...
  function transferTo(address _addr, uint256 _amount)
    ...
    onlyTransferer
  {
    mcgToken.safeTransfer(_addr, _amount);
  }
 ...
 */
contract StandardLotteryV2 is MCGLotterysAbstract {
  ...
  struct Lottery {
    ...
    uint256 startTime;
    uint256 endTime;
    uint256 ticketRate; // $MCG price per ticket
    ...
    uint256 amountCollectedToken; // Collected $MCG token amount which transfered to DeLotto
  ...
  function __StandardLottery_init(
    IERC20Upgradeable _mcgToken,
    IMCGRand _randomGenerator
  ) public initializer {
    MCGLotterysUpgradeable.initialize();
    ...
    deGrandAddress = msg.sender;
    teamWallet = msg.sender;

    maxNumberTicketsPerBuyOrClaim = 100;

    maxPriceTicketInMCG = 50000 * (10**5);
    minPriceTicketInMCG = 1000 * (10**5);
  ...
  /**
   * @notice Buy a bundle for the current lottery staking $MCG
  ...
  function buyTickets(
    ...
    // Calculate number of $MCG to breakdown
    uint256 amountMCGToTransfer = _calculateTotalPriceForBulkTickets(
      _lotteries[_lotteryId].ticketRate,
      _purchasedTicketCount
    );

    uint256 deLottoAmount = amountMCGToTransfer.mul(breakDownDeLottoPot).div(
      10000
    );
    uint256 deGrandAmount = amountMCGToTransfer.mul(breakDownDeGrandPot).div(
      10000
    );
    uint256 teamAmount = amountMCGToTransfer.mul(breakDownTeamWallet).div(
      10000
    );
    mcgToken.safeTransferFrom(
      address(msg.sender),
      address(this),
      deLottoAmount
    );
    ...
    mcgToken.safeTransferFrom(address(msg.sender), teamWallet, teamAmount);
    mcgToken.safeTransferFrom(
      address(msg.sender),
      DEAD_ADDRESS,
      amountMCGToTransfer.sub(deLottoAmount).sub(deGrandAmount).sub(
        teamAmount
      )
    );
  ...
  function claimTickets(
    ...
    // Initializes the rewardInMCGnToTransfer
    uint256 rewardInMCGToTransfer;
    ...
    for (uint256 i = 0; i < _ticketIds.length; i++) {
      ...
      rewardInMCGToTransfer += rewardForTicketId;
    }

    // Transfer money to msg.sender
    mcgToken.safeTransfer(msg.sender, rewardInMCGToTransfer);

    emit TicketsClaim(
      msg.sender,
      rewardInMCGToTransfer,
      _lotteryId,
      _ticketIds.length
    );
  }
    ...
    whenNotPaused
  {
    ...
    // Calculate prizes in $MCG for each bracket by starting from the highest one
   ...
   * @param _ticketRate price of a ticket in $MCG
  ...
  function startLottery(
    ...
    require(
      (_ticketRate >= minPriceTicketInMCG) &&
        (_ticketRate <= maxPriceTicketInMCG),
      "Outside of limits"
    );
  ...
  function burnUnclaimed(uint256 _lotteryId) external onlyOperator {
    ...
    uint256 remain = mcgToken.balanceOf(address(this));
    if (remain > 0) {
      mcgToken.safeTransfer(DEAD_ADDRESS, remain);
    }
  ...
  /**
   * @notice Increase pot by MCG team
   * @param _lotteryId lottery id
    ...
    onlyOwner
  {
    ...
    mcgToken.safeTransferFrom(address(msg.sender), address(this), _amount);
  ...
  /**
   * @notice Set $MCG price ticket upper/lower limit
   * @dev Only callable by owner
   * @param _minPriceTicketInMCG: minimum price of a ticket in $MCG
   * @param _maxPriceTicketInMCG: maximum price of a ticket in $MCG
   */
  function setMinAndMaxTicketPriceInMCG(
    uint256 _minPriceTicketInMCG,
    uint256 _maxPriceTicketInMCG
  ) external onlyOwner {
    require(
      _minPriceTicketInMCG <= _maxPriceTicketInMCG,
      "minPrice must be < maxPrice"
    );

    minPriceTicketInMCG = _minPriceTicketInMCG;
    maxPriceTicketInMCG = _maxPriceTicketInMCG;
  }
