...
contract PlanetZap is ReentrancyGuard, Ownable, Pausable {
    ...
    function zapInFrom3GStables(
        ...
        address _fromStableTokenAddress,
        uint256 _amountIn,
        ...
        uint256 stablesReceived = _pullTokens(_fromStableTokenAddress, _amountIn);

        _approveTokenIfNeeded(_fromStableTokenAddress, _stablePoolAddress);
        uint256 halfAmount = (StablePool(_stablePoolAddress).exchange(stableSwapPoolData[0], stableSwapPoolData[1], stablesReceived, stableSwapPoolData[2], address(this)))/2;
    ...
    function convertLiquidityFromStables(
        ...
        _approveTokenIfNeeded(wantAddress,_stablePoolAddress);  
        ...
        address convertedToken = path1[path1.length - 1];

        if( convertedToken == wbnbAddress){
            _unwrapBNB(output);
            SafeERC20.safeTransferETH(msg.sender,output);         
        }
        else{
            IERC20(convertedToken).safeTransfer(msg.sender,output);
        }
