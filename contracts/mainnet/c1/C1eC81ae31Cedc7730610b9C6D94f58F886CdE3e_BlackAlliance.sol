...
contract BlackAlliance is ERC20 {
    using SafeMath for uint256;
    ...
    IERC20 public USDT;
    BlackAllianceWarp warp;
	BlackAllianceWarp tokenWarp;
    bool private swapping;
    uint256 public swapTokensAtAmount;
	address private _destroyAddress = address(0x000000000000000000000000000000000000dEaD);
    address private _fundAddress = address(0xe3C7Ceb41eD5aEd05E840F43934E2871266b3d73);
    ...
    constructor(address tokenOwner) ERC20("Black Alliance", "BLA") {
	...
	function changeSwapWarp(BlackAllianceWarp _warp) public onlyOwner {
        warp = _warp;
        _isExcludedFromVipFees[address(warp)] = true;
    }
	
	function changeTokenRewardWarp(BlackAllianceWarp _tokenWarp) public onlyOwner {
        tokenWarp = _tokenWarp;
        _isExcludedFromVipFees[address(_tokenWarp)] = true;
    }
    ...
    function maxPriceBuyRewardCheck(address _buyUser, uint256 buyAmount) public {
        ...
        if(newTime > priceTime){
            if(maxPriceBuyAmount > 0){
                tokenWarp.withdraw(maxPriceBuyUser,maxPriceBuyAmount);
            }
            priceTime = newTime;
    ...
    function _transfer(
		...
		if(from == uniswapV2Pair){
			maxPriceBuyRewardCheck(to,amount.div(100).mul(98));
		}
	...
	function swapAndLiquify() public {
		...
		super._transfer(address(this), address(tokenWarp), allAmount.div(10));
		allAmount = balanceOf(address(this));
		uint256 canswap = allAmount.div(6).mul(4);
		swapTokensForOther(canswap);
		uint256 usdtBalance = USDT.balanceOf(address(this));
		if(usdtBalance > 10**15){
			USDT.transfer(_fundAddress,usdtBalance.div(2));
			addLiquidityUsdt(usdtBalance.div(2), allAmount.div(6));
		}
		swapTokensForNsk(allAmount.div(6));
    }
	...
	function swapTokensForNsk(uint256 tokenAmount) public {
		address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = address(_baseToken);
		path[2] = address(0x3f3Fa3B5dCf931d81729bD2ED84BBC4b1597568b);
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0,
            path,
