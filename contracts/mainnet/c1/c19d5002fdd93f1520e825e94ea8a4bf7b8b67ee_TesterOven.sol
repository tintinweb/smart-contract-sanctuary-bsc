...
contract TesterOven is ERC20, Ownable {                                                                                          
    ...
    uint256 public maxTx = 5 * TOTAL_SUPPLY / 1000;               // 0.5%  of total supply
    uint256 public swapTokensAtAmount = 5 * TOTAL_SUPPLY / 10000;   // 0.05% of total supply                                             
    uint256 public maxWallet = 20 * TOTAL_SUPPLY / 1000;          // 2.0%  of total supply
    ...
    constructor() ERC20('TESTER', '$TEST') {                                                     
        ...
        liquidityFee = uint256(10);
        marketingFee = uint256(10);
        devFee = uint256(10);
        treasuryFee = uint256(10);
        sellFeeIncrease = uint256(40);
    ...
    function launch(uint256 _nAntiBotBlocks, uint256 _antiBotDuration, uint256 _tradeCooldown) external onlyOwner{
    ...
    function checkValidTrade(address from, address to, uint256 amount) private view{
        if (from != owner() && to != owner() && !isExcludedFromFees[from]) {
            ...
            if (isPair[from] && !isExcludedFromFees[to]){
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if(takeFee){
            ...
            if((block.number < launchBlock + nAntiBotBlocks) || (block.timestamp < launchTime + antiBotDuration)){
                    fees = amount * 250 / 1000; }

           else{(uint256 fee, bool burnFees) = calculateFee(from);
            _burnFees = burnFees;
    ...
    function swapTokens(uint256 tokens) private inSwap {
        ...
        (temp,) = payable(devAddress).call{value: BNBForDev, gas: 30000}("");
