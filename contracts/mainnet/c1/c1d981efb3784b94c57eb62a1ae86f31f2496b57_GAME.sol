...
interface IERC20 {
	...
	function buy_game_max(address owner) external view returns(uint);
...
contract GAME is Owner {
 ...
 function set_game_num(uint _val) external onlyOwner {
    ...
    function base_output(uint cur_id) external   lock returns(bool) {
		...
		uint256 buy_game_maxbuy_game_max = IERC20(game_addr).buy_game_max(msg.sender);
        ...
        day_base_out_lists[msg.sender]=day_base_out_lists[msg.sender]+buy_game_maxbuy_game_max;
