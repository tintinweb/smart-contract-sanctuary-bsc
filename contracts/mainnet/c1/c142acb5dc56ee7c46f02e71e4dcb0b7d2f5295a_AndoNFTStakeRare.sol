...
/*
website = https://www.andotoken.com/
staking = https://staking.antotoken.com
telegram = https://t.me/andotoken
version 1.6
Rarity Contract: Rare
NFT Staking for Ando NFTs , this contract will stake NFT and give out Ando Token as reward.
Notice the Rewards are given token x hour, current default rate is 5 tokens x hour.
...
contract AndoNFTStakeRare is Ownable, ReentrancyGuard {
...
//end of variables for dapp
    ...
    // The block number when AndoCake mining starts.
    ...
    uint256 public rewardsPerHour = 2400000000000000000;
 ...
 mapping(uint256 => bool) private approvedTokenid;
    ...
    constructor() {
        ...
        nftCollection = IERC721(0xF5db804101d8600c26598A1Ba465166c33CdAA4b); //AirNFT
        rewardsToken = IERC20(0xbC08fF6d564851Cff4423611a02c18cc3127301a); //Ando Token
        } else if (block.chainid == 97) {
        nftCollection = IERC721(0x4E701D1c1F7A6FB309b1d2c9222Bb6d1B1439bFf);
        rewardsToken = IERC20(0x3B8Ec22EA36800e0B59d33E8bbcF06da240BDC1f);
        } else {
            revert();
        }
//Variables have been defined now we move on the rest of the configuration	
    ...
    function stake(uint256[] calldata _tokenIds) external nonReentrant {
        ...
        for (uint256 i; i < len; ++i) {
			require(approvedTokenid[_tokenIds[i]] != false, "ERC721: NFT not Approved");										   
            require(
...
//external functions
    ...
    // Safe token transfer function, just in case if rounding error causes pool to not have enough AndoCake.
    ...
    function recoverRewardTokensFromContract(address _tokenAddress, uint256 _amount) public onlyOwner {                               
...
//need to add unstake
    ...
    returns (uint256)
  {
    ...
    {
        require(
            stakerAddress[tokenId] != nullAddress,
            "Token is not staked!"
        );
...
//Configuration for AirNFT Support
    }
    function setApprovedTokenid(uint256 tokenId, bool exempt) external onlyOwner {
        approvedTokenid[tokenId] = exempt;
    }
    function getApprovedTokenid(uint256 tokenId) public view returns (bool) {
        return approvedTokenid[tokenId];
    }
