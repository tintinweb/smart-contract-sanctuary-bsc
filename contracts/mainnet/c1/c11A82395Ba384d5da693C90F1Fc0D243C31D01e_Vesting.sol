...
contract Vesting is Ownable, Pausable {
    ...
    mapping (address => uint256) private viewAmount;
    ...
    function viewVestorAmount(address _user) external view returns(uint256 vestAmount) {
        return viewAmount[_user];
    }
    ...
    function unVestToken(uint256 _vestID ) external whenNotPaused {
        ...
        require(user.vestingTime > 0,"user not found");
        require(user.unVestingTime == 0,"user already unVested");
        uint256 withdrawAmount = user.vestingAmount;
        viewAmount[user.user] -= user.vestingAmount;
    ...
    function updateAnation (address _newAnation) external onlyOwner {
        AnationToken = IBEP20(_newAnation);
    }
