...
contract LITTLELUNC is Context, IERC20, Ownable {
    ...
    string private constant _name = "Little Lunc";
    string private constant _symbol = "LLUNC";

    address private Lunc_Marketing_Wallet = 0x2d88c08D5842aB14ccebAA987592f4A54637059A;
    address private Lunc_Team_Wallet = 0x2d88c08D5842aB14ccebAA987592f4A54637059A;

    uint256 public Lunc_max_WalletLimit = 20000000000000 * 10**9; //2%
    uint256 public swapTokensAtAmount = 3000000000000 * 10**9; //0.3%
    uint256 public Lunc_max_BuyLimit = 10000000000000 * 10**9; // 1%
    uint256 public Lunc_max_SellLimit = 10000000000000 * 10**9; //1%
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= Lunc_max_BuyLimit, "You are exceeding Lunc_max_BuyLimit");
        ...
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= Lunc_max_SellLimit, "You are exceeding Lunc_max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= Lunc_max_WalletLimit, "You are exceeding Lunc_max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        Lunc_max_BuyLimit = maxBuy * 10**decimals();
        Lunc_max_SellLimit = maxSell * 10**decimals();
    }
    
    function Lunc_Max_Walletlimit(uint256 amount) external onlyOwner{
