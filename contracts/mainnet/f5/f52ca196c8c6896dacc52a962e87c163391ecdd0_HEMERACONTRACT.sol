...
╚═╝  ╚═╝╚══════╝╚═╝     ╚═╝╚══════╝╚═╝  ╚═╝╚═╝  ╚═╝
    ...
    Tokenomics
    Reward fee distributes to Holders in BUSD: 3%
    Development fee for future project developments: 2%
    Liquidity fee sent to Liquidity pool to stability in price: 2%
    Marketing fee to boost up our project: 5%
    Total Buy/Sell fee 12%
**/
...
contract HEMERACONTRACT is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public swapTokensAtAmount = 1 * 10**4 * (10**9); //0.01% of the supply

    uint256 public maxBuyAmount = 5 * 10**6 * 10**9; // 5% of the supply
    uint256 public maxSellAmount = 5 * 10**6 * 10**9; // 5% of the supply
    uint256 public maxWalletAmount = 5 * 10**6 * 10**9; // 5% of the supply
    ...
    address public _devWallet = payable(address(0x0e9121fA148fDb0c5331e18A8bAac880a68fFb87)); // developmentWallet
    address public _marketingWallet = payable(address(0x0e9121fA148fDb0c5331e18A8bAac880a68fFb87)); //marketingWallet
    ...
    constructor() ERC20("HEMERA", "HMR") {
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(
            ...
            //0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3 // Pancakeswap Router testnet
        );
        ...
        */
//        _mint(owner(), 1 * 10**14 * (10**9)); // 100 Trillion Tokens
        _mint(owner(), 1 * 10**8 * (10**9)); // 100 Million Tokens
   ...
   function setMarketingWallet (address wallet) external onlyOwner {
    ...
    function setBuyFee(
        ...
        totalBuyFee = rewardfee + dev + liquidity + marketing;
    }
    ...
    function setSellFee(
        ...
        totalSellFee = rewardfee + dev + liquidity + marketing;
    }
   ...
   function setSwapTokensAtAmount(uint256 amount) external onlyOwner {
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 feeTokens = contractTokenBalance
                .mul(buyFee.devFee + sellFee.devFee + buyFee.marketingFee + sellFee.marketingFee)
                .div(totalFee);
    ...
    function swapAndSendToWallets(uint256 tokens) private lockTheSwap {
        ...
        uint256 newBalance = IERC20(BUSD).balanceOf(address(this)).sub(
            initialBalance);
        uint256 marketingShare = newBalance.mul(buyFee.marketingFee + sellFee.marketingFee).div(buyFee.devFee + sellFee.devFee + buyFee.marketingFee + sellFee.marketingFee);
        uint256 devShare =newBalance.mul(buyFee.devFee + sellFee.devFee).div(buyFee.devFee + sellFee.devFee + buyFee.marketingFee + sellFee.marketingFee);
