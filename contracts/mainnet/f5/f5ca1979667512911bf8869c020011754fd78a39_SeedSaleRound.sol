...
contract SeedSaleRound  is Ownable {
    ...
    constructor(uint256 _rate, address _usdt, address _bgt, uint256 _hardcap, uint256 _allowedUserBalance, uint256 _minAmount)  {
        rate = _rate;
        usdt = IERC20(_usdt);
        bgt = IERC20(_bgt);
        presaleOver = true;
    ...
    function buyTokenWithUSDT(uint256 _amount, address referral) external {
        ...
        require(address(0) != referral, "Can't use msg.sender as referral");
        ...
        //     doTransferOut(address(bgt), referral, tokensPurchased.mul(referralRate).div(1e4));
