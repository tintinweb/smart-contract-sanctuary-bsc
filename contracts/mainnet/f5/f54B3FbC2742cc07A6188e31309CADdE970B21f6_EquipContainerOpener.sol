...
import "common-contracts/contracts/ERC721Distributor.sol";
import "common-contracts/contracts/Interfaces/LiquidiftyCollection.sol";

contract EquipContainerOpener is ERC721Distributor{

    mapping(uint => bool) acceptableContainers;
    
    LiquidiftyCollection lqtCollection;

    constructor(
        address _items, 
        address _fromAddress, 
        ChainlinkParams memory _params,
        LiquidiftyCollection _lqtCollection, 
        uint[] memory _acceptableContainers) ERC721Distributor(_items, _fromAddress, _params) 
    {
        lqtCollection = _lqtCollection;
        for (uint i = 0; i < _acceptableContainers.length; i++) {
            acceptableContainers[_acceptableContainers[i]] = true;
        }
    }

    function openContainer(uint id, uint8 amount) external {
        require(acceptableContainers[id], "invalid token id");

        lqtCollection.burn(msg.sender, id, amount);
...
contract ERC721Distributor is Ownable, Pausable, ERC721Holder, VRFConsumerBaseV2 {
    uint public pendingItems;
    ...
    uint32 public gasPerTx = 150000;
    uint32 public amountCap = 10;
    address public fromAddress;

    ERC721Enumerable private items;

    mapping(uint => Request) private requests;

    event RequestCreated(address indexed to, uint8 amount, uint requestId);
    ...
    constructor(address _items, address _fromAddress, ChainlinkParams memory params) VRFConsumerBaseV2(params.coordinator) {
        items = ERC721Enumerable(_items);
    ...
    function _getItems(uint8 amount) internal whenNotPaused {
        uint supply = items.balanceOf(fromAddress);
        require(pendingItems < supply, "no items left");
        
        supply -= pendingItems;
        require(amount > 0 && amount <= supply && amount <= amountCap, "wrong amount");

        uint totalGas = amount*gasPerTx;
        require(totalGas < 2500000, "this tx will use too much gas");

        uint id = requestRandomWords(amount);

        requests[id] = Request(msg.sender, amount);
        pendingItems += amount;
    }

    function setSubscription(uint64 id) external onlyOwner{
        subscriptionId = id;
    }

    function setFromAddress(address newAddress) external onlyOwner{
        fromAddress = newAddress;
    }

    function setGasLimitPerItem(uint32 newValue) external onlyOwner{
        gasPerTx = newValue;
    }
    
    function setAmountCap(uint32 newValue) external onlyOwner{
        amountCap = newValue;
    }

    function resendRequest(uint id) external onlyOwner{
        require(requests[id].to != address(0), "wrong request id");

        uint newId = requestRandomWords(requests[id].amount);
    ...
    function requestRandomWords(uint8 amount) private returns(uint id){
        ...
        emit RequestCreated(fromAddress, amount, id);
    }
    ...
    function fulfillRandomWords(
        ...
        require(requests[requestId].to != address(0), "wrong request id");
        ...
        for (uint256 i = 0; i < amount; i++) {
            items.transferFrom(fromAddress, to, items.tokenOfOwnerByIndex(fromAddress, randomWords[0] % (supply-i)));
        }
...
contract LiquidiftyCollection is Ownable, ERC1155Base {
    string public name;
    ...
    function mint(uint256 _id, Fee[] memory fees, uint256 supply, string memory uri) public {
        _mint(_id, fees, supply, uri);
    }
}
...
contract ERC1155 is IERC1155, ERC165Storage {
    using Address for address;
    ...
    */
    ...
    function _doSafeTransferAcceptanceCheck(address _operator, address _from, address _to, uint256 _id, uint256 _value, bytes memory _data) internal {
        ...
        require(ERC1155Receiver(_to).onERC1155Received(_operator, _from, _id, _value, _data) == ERC1155_ACCEPTED, "contract returned an unknown value from onERC1155Received");
    }
    ...
    function _doSafeBatchTransferAcceptanceCheck(address _operator, address _from, address _to, uint256[] memory _ids, uint256[] memory _values, bytes memory _data) internal {
        ...
        require(ERC1155Receiver(_to).onERC1155BatchReceived(_operator, _from, _ids, _values, _data) == ERC1155_BATCH_ACCEPTED, "contract returned an unknown value from onERC1155BatchReceived");
    }
}
