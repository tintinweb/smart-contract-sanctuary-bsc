...
/**
BackToTheTesting2 SmartContract 
*/
...
contract BackToTheTesting2 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BackToTheTesting2DividendTracker public dividendTracker;
    ...
    constructor() ERC20("BackToTheTesting2", "BTTT2") {
        ...
        dividendTracker = new BackToTheTesting2DividendTracker(payable(this), router, 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56,
                                                   "BackToTheTesting2TRACKER", "BTTT2TRACKER");
...
contract BackToTheTesting2DividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    BackToTheTesting2 public BackToTheTesting2Contract;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        BackToTheTesting2Contract = BackToTheTesting2(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main BackToTheTesting2 contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(BackToTheTesting2Contract);

            uint256 prevBalance = BackToTheTesting2Contract.balanceOf(address(this));
            ...
            {
                uint256 received = BackToTheTesting2Contract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    BackToTheTesting2Contract.transfer(account, received);
                } 
                else {
                    success = false;
                }
