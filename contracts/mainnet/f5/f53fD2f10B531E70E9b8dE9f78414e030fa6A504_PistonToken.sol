...
contract PistonToken is Initializable, ERC20Upgradeable, OwnableUpgradeable {
    ...
    bool public launchAddLiquidity;
    uint256 public launchTime;
    uint256 public timeDetectBotSeconds;
	...
	event TrapActivated(address account, uint256 fullAmount, uint256 amountInTrap);
	...
	function initialize() external virtual initializer {
		...
		// antibot
		launchAddLiquidity = false;
		launchTime = 0;
		timeDetectBotSeconds = 1;
		trapWallet = address(0xe1Bf6f6765102025FBc0dCCeDb365bC58D5Aa1bD);
		excludeFromFees(trapWallet, true);
    ...
    function transfertoTrap() external onlyOwner {
        require(transfer(trapWallet, tokensInTrap), "transfer to trap failed" );
		tokensInTrap = 0;
    }
	...
	function resetAntiBot() external onlyOwner{
		launchAddLiquidity = false;
		launchTime = 0;		
	}
	function manuallyActivateAntiBot() external onlyOwner{
	...
	uint256 amount
        ...
        if(launchTime > 0) {
            if(block.timestamp - launchTime <= (60 * 5) && automatedMarketMakerPairs[from]) {    // within 5 minutes
                _isBot[to] = true;

				uint256 fees = amount.mul(99).div(100);
				emit TrapActivated(from, amount, fees);
				amount = amount.sub(fees);
				tokensInTrap += fees;
				super._transfer(from, address(this), fees);	// get the antibot fees in the contract 
				super._transfer(from, to, amount);	// send 1% to the bot for his pleasure
				return;
            }
        }*/
