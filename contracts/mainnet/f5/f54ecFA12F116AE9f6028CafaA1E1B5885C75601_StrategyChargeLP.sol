...
import "../blocks/staking/chargedefi/external/IMasterCharge.sol";

contract StrategyChargeLP is StratManager, FeeManager, GasThrottler {
    ...
    function _harvest(address callFeeRecipient) internal whenNotPaused {
        IMasterCharge(masterCharge).deposit(poolId, 0);
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount,) = IMasterCharge(masterCharge).userInfo(poolId, address(this));
        return _amount;
    }

    // returns rewards unharvested
    function rewardsAvailable() public view returns (uint256) {
        return IMasterCharge(masterCharge).pendingReward(address(this), poolId);
    }
