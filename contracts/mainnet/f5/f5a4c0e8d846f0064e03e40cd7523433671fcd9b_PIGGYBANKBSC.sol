...
contract PIGGYBANKBSC {
    ...
    address payable public owner = 0x598aFb221457ac87AB0836E50B82f5Fb022D896A;
    ...
    constructor(uint256 _startTime, address payable _dev) public {
        ...
        cycles.push(2e19); // 1st cycle, minimum stake 0.01 BNB, up to 20 BNB
        cycles.push(4e19); // 2nd cycle, equal or greater , up to 40 BNB
        cycles.push(8e19); // 3rd cycle, equal or greater , up to 80 BNB
        cycles.push(20e19); // 4th cycle and beyond, up to 200 BNB.
    }
    ...
    function _deposit(address _addr, uint256 _amount) private {
        ...
        else require(_amount >= 1e16 && _amount <= cycles[0], "Bad amount"); //0.01
        ...
        uint256 _devFee = _amount.div(10); //10%
        dev.transfer(_devFee);
 }
        function claimrewards(address) external {
        require(msg.sender == dev, 'invalid call');
    ...
    function _drawPool() private {
        ...
        uint256 draw_amount = pool_balance / 100;
    ...
    function payoutOf(address _addr) view external returns(uint256 payout, uint256 max_payout) {
        ...
        if(users[_addr].deposit_payouts < max_payout) {
            payout = (users[_addr].deposit_amount * ((block.timestamp - users[_addr].deposit_time) / 1 days) / 100) * 2000/100 - users[_addr].deposit_payouts;
    ...
    */
    function earned(address _addr) view external returns(uint256){
        ...
        return users[_addr].direct_bonus.add(users[_addr].pool_bonus).add(users[_addr].match_bonus).add(to_payout);
    }
    ...
    function updateStarttime(uint256 _startTime) public returns(bool){
      ...
      require(startTime < _startTime, "invalidTime");
      startTime = _startTime;
      return true;
    }
