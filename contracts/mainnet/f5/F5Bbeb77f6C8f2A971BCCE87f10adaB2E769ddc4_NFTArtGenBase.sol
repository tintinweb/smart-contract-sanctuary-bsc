...
contract NFTArtGen is ERC721, Ownable {
    ...
    function mint(address to, uint32 count) external payable onlyOwner preMintChecks(count) {
        require(open == true, "Mint not open");
        performMint(to,count);
    }

    function performMint(address to,uint32 count) internal {
        for (uint32 i = 0; i < count; i++) {
            _safeMint(to, ++supply, "");
        }
        
        addressMintedMap[to] += count;
