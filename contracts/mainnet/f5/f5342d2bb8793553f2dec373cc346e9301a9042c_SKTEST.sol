...
contract SKTEST is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    SKTESTDividendTracker public dividendTracker;
    ...
    constructor() ERC20("SKTEST", "SKT") {
        marketingWallet = payable(0x5074B8730Bc087dFaf9f53Ec53f39cf76Fb8AD8E);
        devWallet = payable(0x5074B8730Bc087dFaf9f53Ec53f39cf76Fb8AD8E);
        ...
        dividendTracker = new SKTESTDividendTracker(
            payable(this),
            router,
    ...
    function updateFees(
        ...
        require(totalSellFees <= 20 && totalBuyFees <= 20, "total fees cannot exceed 15% sell or buy");
...
contract SKTESTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        SKTESTContract = SKTEST(mainContract);
    ...
    function _transfer(
        ...
        require(false, "SKTEST_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "SKTEST_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SKTEST contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(SKTESTContract);

            uint256 prevBalance = SKTESTContract.balanceOf(address(this));
            ...
            {
                uint256 received = SKTESTContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    SKTESTContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
