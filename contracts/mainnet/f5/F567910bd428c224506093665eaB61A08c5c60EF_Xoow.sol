...
contract DrawManager is VRFConsumerBase, ReentrancyGuard, Ownable {
    ...
    function processTransfer(uint256 _amount, address _sender) external nonReentrant {
        ...
        if (!(_sender == address(pancakeswapBusdPair) || _sender == address(pancakeswapMooxPair))) {
            return;
        }
        ...
        } else if (!_isMooxPairBuy(_amount)) {
            return;
        }
    ...
    function _isMooxPairBuy(uint256 _amount) private view returns (bool) {
    ...
    function draw() external nonReentrant {
        ...
        if (!xoow.transfer(_protocolFeeTo, _currentAward.mul(10).div(100))) {
            emit XoowTransferFailed(_protocolFeeTo, _currentAward.mul(10).div(100));
        }
    ...
    function addAwardToCurrent(uint256 _award) external {
...
contract Xoow is Context, IBEP20 {
    ...
    function _transfer(
        ...
        if (drawManager.shouldTakeFee(sender, recipient, amount)) {
            uint256 drawAmount = amount.mul(5).div(1000);
            ...
            deductedAmount = deductedAmount.add(drawAmount.mul(2));
        }
