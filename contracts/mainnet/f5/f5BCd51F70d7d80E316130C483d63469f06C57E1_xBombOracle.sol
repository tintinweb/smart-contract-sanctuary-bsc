...
contract xBombOracle is IOracle {
    using FixedPoint for *;
    using BoringMath for uint256;
    uint256 public constant PERIOD = 10 minutes;
    IAggregator public constant BTC_USD = IAggregator(0xB6064eD41d4f67e353768aA239cA86f4F73665a1);
    IUniswapV2Pair public constant BOMB_BTC = IUniswapV2Pair(0x84392649eb0bC1c1532F2180E58Bae4E1dAbd8D6);
    IERC20 public constant BOMB = IERC20(0x522348779DCb2911539e76A1042aA922F9C47Ee3);
    IERC20 public constant XBOMB = IERC20(0xAf16cB45B8149DA403AF41C63AbFEBFbcd16264b);
    ...
    function _get(uint32 blockTimestamp) public view returns (uint256) {
        uint256 priceCumulative = BOMB_BTC.price1CumulativeLast();

        // if time has elapsed since the last update on the pair, mock the accumulated price values
        (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast) = IUniswapV2Pair(BOMB_BTC).getReserves();
    ...
    function toXBOMB(uint256 amount) internal view returns (uint256) {
        return amount.mul(BOMB.balanceOf(address(XBOMB))) / XBOMB.totalSupply();
    }
    ...
    function get(bytes calldata) external override returns (bool, uint256) {
        ...
        pairInfo.priceAverage = uint144(
            1e44 /
                ...
                ).mul(uint256(BTC_USD.latestAnswer()))
        );
        pairInfo.blockTimestampLast = blockTimestamp;
    ...
    function peekSpot(bytes calldata) external view override returns (uint256 rate) {
        (uint256 reserve0, uint256 reserve1, ) = BOMB_BTC.getReserves();
        rate = 1e44 / toXBOMB(reserve0.mul(1e18) / reserve1).mul(uint256(BTC_USD.latestAnswer()));
    }
