...
contract Vault is BaseContract
{
    ...
    function _claim(address participant_, uint256 taxRate_) internal returns (bool)
    {
        ...
        uint256 _effectiveClaims_ = _effectiveClaims(participant_, 1);
        if(_effectiveClaims_ >= _properties.negativeClaims) {
            // User is negative
    ...
    {
        //uint256 _period_ = ((block.timestamp - _participants[participant_].lastRewardUpdate) / _properties.period) * 10000 ;
