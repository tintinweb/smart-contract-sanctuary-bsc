...
interface IClashFantasy {
    ...
    function getUserTokenById(address _address, uint256 _tokenId) external view returns (uint256 token, uint256 typeOf, bool exists, uint256 presale, uint256 aditionalInfo); 
}

contract ClashFantasyOpenChestV3 is Initializable {
        ...
        validateChest(_tokenId)
    {
        ...
        (, uint256 typeOf, , , ) = contractBase.getUserTokenById(msg.sender, _tokenId);
        (uint256 hasEnergy,) = contractBaseChest.getChestHasEnergyByChestId(typeOf);
        (uint256 rarity, uint256 amount) = getChestChoosen(typeOf);
        arrayOpenChest[msg.sender][_tokenId] = openChestSkeleton(_tokenId, block.timestamp, false, true, typeOf, rarity, amount,hasEnergy);
    }
    ...
    //internal
    function random(uint256 _aditional) internal view returns (uint256) {
    ...
    { 
        ...
        contractBase.burn(msg.sender, _tokenId );
        ...
        returns(uint256)
    {
        ...
        uint256 purchasenumber = uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, msg.sender, initialNumber))) % 100;
        return (myArray[purchasenumber]);
    }

    function returnRandomCards(uint256 _tokenId, uint256 _aditional, uint256 amountCards) 
        internal
        view
        returns(uint256)
    {
        uint256 choose =  uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender, _tokenId, _aditional))) * amountCards;        
        return choose;
    }
    ...
    function setContractbaseChest(IClashFantasyChest _contractbaseChest) public onlyAdminOwner {
        contractBaseChest = _contractbaseChest;
    }

    function setContractbase(IClashFantasy _contractBase) public onlyAdminOwner {
        contractBase = _contractBase;
    }

    function setLootChest(uint256 _typeOf, LootChestInfo[] memory _lootChestInfo) public onlyAdminOwner {
        delete lootChest[_typeOf];
        lootChest[_typeOf].typeOf = _typeOf;
        for (uint256 i = 0; i < _lootChestInfo.length; i++) {
            lootChest[_typeOf].info.push(_lootChestInfo[i]);
        }
    }
        ...
        returns(CardInfo[] memory) 
    {
        ...
        for (uint256 i = _index; i<_length-1; i++){
