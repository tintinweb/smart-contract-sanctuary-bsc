...
contract RealboxVault is Ownable, ERC721Holder, ReentrancyGuard {
    ...
    event BuyVaultToken(address indexed buyer, uint256 amount, uint256 price, string uid);
    ...
    event CrowdFundingRefund(address indexed buyer, uint256 amount, uint256 price, string uid);
    ...
    function finalize(uint256 _totalSupply) external onlyOwner onlyState(CrowdFundingState.Ended) {
        if (currentSupply < minSupply) {
            ...
            bytes32 salt = keccak256(abi.encodePacked(address(realx)));
    ...
    function claimOrRefund(uint256 _size) external nonReentrant onlyState(CrowdFundingState.Finalized) {
        ...
        for (uint256 i = 0; i < _size; i++) {
            ...
            bool isDirectBuy = bytes(txInfo.uid).length == 0;
            ...
            if (refundToken > 0) {
                ...
                emit CrowdFundingRefund(txInfo.sender, refundToken, txInfo.unitPrice, txInfo.uid);
            }
            if (claimToken > 0) {
                if (isDirectBuy) {
    ...
    function buyPublicToken(uint256 _amount) external nonReentrant onlyState(CrowdFundingState.PublicStarted) {
        ...
        _buyToken(_amount, publicUnitPrice, '');
    }
    ...
    function _buyToken(
        ...
        _txInfo[txIds] = TxInfo(msg.sender, _amount, _price, _uid);
        txIds += 1;
        currentSupply = currentSupply.add(_amount);
        emit BuyVaultToken(msg.sender, _amount, _price, _uid);
    }
...
contract RealboxVaultToken is Ownable, Pausable, ERC20Snapshot, ERC20Permit {
