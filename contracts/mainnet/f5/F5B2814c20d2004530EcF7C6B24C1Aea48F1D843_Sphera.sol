...
contract Sphera is Context, IERC20, Ownable {
    ...
    string private constant _name = "Sphera";
    string private constant _symbol = "SFRA";
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner() && ! _isExcludedFromFee[to] && ! _isExcludedFromFee[from]) {
            if(_abmode == true){
                require(bots[from] != false, "No bot transactions during antibotmode.");
            }
