...
/**
    ...
    SThe Keihin–Tōhoku Line (Japanese: 京浜東北線, Hepburn: Keihin-tōhoku-sen) is a railway line in Japan 
    which connects the cities of Saitama, Kawaguchi, Tokyo, Kawasaki, and Yokohama. 
    It is part of the East Japan Railway Company (JR East) network.
 
    Twitter : https://twitter.com/KHTHOfficial
    Telegram : https://t.me/KHTHOfficial
 
    Circulating Supply: 137.000.000 $KHTH
    Maximum Wallet: 4.110.000 $KHTH
    Slippage: 3-4%
    Transaction Tax: 3% / 1% LP - 2% KeihinTouhokuSenFund
...
contract KeihinTouhokuSen is IERC20, Ownable {       
    string private constant _name = "KeihinTouhokuSen";
    string private constant _symbol = "KHTH";
    ...
    uint256 public constant sniperTax = 30;
    ...
    address payable private constant KeihinTouhokuSenFund = payable(0x98CA3Cab3b119F6CaD74b0b88dB7b4f5b3Fc91B8);
    ...
    modifier tradingLock(address sender) {
        require(tradingOpen || sender == deployerAddress || sender == KeihinTouhokuSenFund);
        _;
    }
    ...
    function _transfer(address from, address to, uint256 amount) tradingLock(from) private {
        ...
        if (from != address(this) && from != KeihinTouhokuSenFund && to != KeihinTouhokuSenFund && to != deployerAddress) {
    ...
    function swapback(uint256 tokenAmount) private swapLock {
        ...
        if(contractETHBalance > 0) {
            KeihinTouhokuSenFund.transfer(contractETHBalance);
        }
    }
