...
contract Private is TokenVesting {
    ...
    function removeInvestor(address _account) public onlyOwner {
        ...
        uint256 currentBalance = investmentBalance[_account];
        investmentBalance[_account] = 0;
        countInvestmentInput[_account] = 0;
        for (uint256 i = 0; i < countTotalInvestor; i++) {
            //assign increment number for each investor except  to deleted address
            if (listOfAllInvestor[i] == _account) {
                delete listOfAllInvestor[i];
            }
        }
        //Decrease 1 total investor
        countTotalInvestor--;
        totalTokenVested -= currentBalance;
        investmentTotal[_account] = 0;
        emit RemoveInvestor(block.timestamp, _account);
    }
}