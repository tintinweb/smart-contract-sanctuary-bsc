...
contract VAST is ERC20, Ownable {
    ...
    uint256[7] public genFeeList = [20, 5, 5, 5, 5, 5, 5];
    ...
    uint256 public minTotal = 100 * 10**4 * 10**decimals();
    ...
    constructor() ERC20("VAST", "VAST") {
        ...
        usdtas01 = new AutoSwap(address(this));
        ...
        usdtw01 = address(0x0EBDa748a00E4236D4d5817B37bB7401019c2D3A);
    ...
    function _tokenTransfer(
        ...
        if (
            ...
            if (_rate > markRate) {
                ...
                for (uint256 i = 0; i < diffRate; i++) {
                    if (totalSupply().mul(deflationRate).div(deflationAnchor) > minTotal) {
        ...
        if (sender == uniswapV2Pair && takeFee) {
            ...
            if (teamFee > 0) {//1.5%
                ...
                _balances[address(usdtas01)] += rTeamTokens;
                emit Transfer(sender, address(usdtas01), teamTokens);
            }
    ...
    function _swap() private lockSwap {
        ...
        usdtas01.withdraw(address(this));
        swapTokensForTokenB(balanceOf(address(this)), usdtw01);
