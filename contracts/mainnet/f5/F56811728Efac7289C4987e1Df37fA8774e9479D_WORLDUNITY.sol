...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
  ...
  function _mint(address account, uint256 value) public override {
  ...
  function _burn(address account, uint256 value) public override {
...
contract WUTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("WUT_Dividend_Tracker", "WUT_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "WUT_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "WUT_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main WUT contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "WUT_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "WUT_Dividend_Tracker: Cannot update claimWait to same value");
...
contract WORLDUNITY is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    WUTDividendTracker public dividendTracker;
    ...
    uint256 public peoplefee;

    address payable public  marketingWallet;
    address payable public  peopleWallet;
    ...
    function setFee(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _marketingFee,uint256 _peoplefee) public onlyOwner {
        BNBRewardsFee = _bnbRewardFee;
        liquidityFee = _liquidityFee;
        marketingFee = _marketingFee;
        peoplefee = _peoplefee;

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(peoplefee); // total fee transfer and buy
    }
    ...
    function setpeopleWallet(address payable _newpeopleWallet) public onlyOwner {
        peopleWallet = _newpeopleWallet;
    }

    constructor() ERC20("World Unity", "WUT") {
        BNBRewardsFee = 8;
        liquidityFee = 2;
        ...
        marketingWallet = payable(0xa3D962A411B94768A5D01ba18e6ad94a4c33C5FF);
        peopleWallet = payable(0xC3209b1E8FF9A5F3435e9CAc2D79e325448A2e4D);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(peoplefee); // total fee transfer and buy

    	dividendTracker = new WUTDividendTracker();
        ...
        excludeFromFees(peopleWallet, true);
        ...
        _isExcludedFromMaxTx[peopleWallet] = true;
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "WUT: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "WUT: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "WUT: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "WUT: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "WUT: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "WUT: Cannot update gasForProcessing to same value");
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 peopleAmount = deltaBalance.sub(bnbToAddLiquidityWith).div(totalFees.sub(liquidityFee)).mul(peoplefee);
        peopleWallet.transfer(peopleAmount);
    ...
    function swapAndSendBNBTopeople(uint256 tokenAmount) private {
        swapTokensForBnb(tokenAmount, peopleWallet);
    }
