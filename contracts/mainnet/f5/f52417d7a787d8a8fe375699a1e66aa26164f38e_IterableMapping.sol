...
pragma solidity ^0.6.12;
contract DOGEMOONDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("DOGEMOON_Dividen_Tracker", "DOGEMOON_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 500000 * (10**18); //must hold 500000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "DOGEMOON_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "DOGEMOON_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SolanaDao contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DOGEMOON_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DOGEMOON_Dividend_Tracker: Cannot update claimWait to same value");
...
pragma solidity ^0.6.12;
contract STMDDOGE is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    DOGEMOONDividendTracker public dividendTracker;
    ...
    uint256 public DOGERewardsShare = 5;
    ...
    constructor() public ERC20("STMDDOGE", "STMDDOGE") {

    	dividendTracker = new DOGEMOONDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "DOGEMOON: The dividend tracker already has that address");

        DOGEMOONDividendTracker newDividendTracker = DOGEMOONDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "DOGEMOON: The new dividend tracker must be owned by the DOGEMOON token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "DOGEMOON: The router already has that address");
    ...
    function setFee(
        ...
        uint256 _DOGERewardsShare
    ) public onlyOwner {
        DOGERewardsFee = _DOGERewardsFee;
        DOGERewardsShare = _DOGERewardsShare;
    }

    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DOGEMOON: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "DOGEMOON: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "DOGEMOON: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "DOGEMOON: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(liquidityLaunched && block.number <= lastSnipeTaxBlock && !isFirstLaunch){
            uint256 fees = amount.mul(DOGERewardsFee).div(100);
        ...
        if( canSwap &&
            ...
            uint256 BNBAmountForReward = BNBAmount.mul(DOGERewardsShare).div(DOGERewardsFee);
