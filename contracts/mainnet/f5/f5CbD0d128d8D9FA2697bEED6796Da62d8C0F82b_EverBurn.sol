...
contract EverBurn is Ownable, IERC20 {
    ...
    uint256 private quoteBase = 1 * 10**18;
    uint256 public targetPrice;
    uint256 public minRebasePercent = 10100;
    uint256 public rebaseAdjustFactor = 9000;
    uint256 private currentDate = 1654473600;
    uint256 public lastRebase;
    uint256 public rebaseCooldown = 5 minutes;
    ...
    uint256 public dailyUSDLimit = 10000;
    ...
    function setRebaseCooldown(uint256 _seconds) external onlyOwner {
        rebaseCooldown = _seconds;
    }

    function setTransferLimit(uint256 _amount) external onlyOwner {
    ...
    function transfer(address recipient, uint256 amount) external virtual override returns (bool) {
        if(DailyTransfers[msg.sender].startTime > block.timestamp) {
            require(amount <= transferLimit || isAddressWhitelistedOut[msg.sender], "EverBurn: transfer amount exceeds daily limit");
            ...
            require(DailyTransfers[msg.sender].periodTransfers.add(amount) <= transferLimit || isAddressWhitelistedOut[msg.sender], "EverBurn: transfer amount exceeds daily limit");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external virtual override returns (bool) {
        if(DailyTransfers[sender].startTime > block.timestamp) {
            ...
            require(DailyTransfers[sender].periodTransfers.add(amount) <= transferLimit || isAddressWhitelistedOut[sender], "EverBurn: transfer amount exceeds daily limit");
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        require(!_isContract(sender) && !_isContract(recipient) && !isAddressBlacklistedIn[recipient], "EverBurn: not eligible for transfer");
        ...
        uint256 currentPrice = getQuote().mul(1 * 10**5).div(quoteBase);

        if( block.timestamp > currentDate.add(86400)) {
            currentDate = currentDate.add(86400);
            ...
            transferLimit = targetPrice.mul(dailyUSDLimit);
        } 
        
        if(currentPrice <= targetPrice.mul(95).div(100) && block.timestamp >= lastRebase + rebaseCooldown) {
...
contract Rebaser is IRebaser {
    ...
    uint256 public percentToRemove = 2500;
    ...
    function setRebaseEnabled(bool flag) external onlyAdmin {
        rebaseEnabled = flag;
    }
    ...
    function setTreasuryFee(uint256 _percent) external onlyAdmin {
        treasuryFee = _percent;
    }

    function setPancakePair(address _pair) external onlyAdmin {
        pancakePair = _pair;
    ...
    function rebase(uint256 currentPrice, uint256 targetPrice) external override onlyToken lockTheSwap {
        if(rebaseEnabled){
            ...
            uint256 feeAmount = amountBUSDAvailable.mul(treasuryFee).div(divisor);
            uint256 amountBUSDtoAdd = amountBUSDAvailable.sub(feeAmount);
            buyTokens(amountToSwap, amountBUSDtoAdd);
            IERC20(BUSD).transfer(treasury, feeAmount);
    ...
    function removeLiquidity() internal {
        uint256 amountToRemove = IERC20(BUSD).balanceOf(address(this)).mul(percentToRemove).div(divisor);
       
        IERC20(pancakePair).approve(address(pancakeRouter), IERC20(pancakePair).balanceOf(address(this)));
    ...
    function drainBNB(uint256 amount, address to) external onlyAdmin{
        payable(to).transfer(amount);
      }

    // Rescue tokens that are sent here by mistake
    function drainToken(IERC20 token, uint256 amount, address to) external onlyAdmin {
