...
contract VToken is IERC20, Ownable {
	...
	string private constant _name =  "VToken";
	string private constant _symbol = "VToken";
	...
	// Base taxes
	CustomTaxPeriod private _base = CustomTaxPeriod('base',0,0,1,1,2,2,1,1,2,2);
    ...
    function setFeeOnSelectedWalletTransfers(address account, bool value) external onlyOwner {
		require(_feeOnSelectedWalletTransfers[account] != value, "Varadium: The selected wallet is already set to the value");
		_feeOnSelectedWalletTransfers[account] = value;
		emit FeeOnSelectedWalletTransfersChange(account, value);
	}
	function _setAutomatedMarketMakerPair(address pair, bool value) private {
		require(automatedMarketMakerPairs[pair] != value, "Varadium: Automated market maker pair is already set to that value");
		automatedMarketMakerPairs[pair] = value;
		emit AutomatedMarketMakerPairChange(pair, value);
	}
	function blockAccount(address account) external onlyOwner {
		require(!_isBlocked[account], "Varadium: Account is already blocked");
		if (_launchStartTimestamp > 0) {
			require((block.timestamp - _launchStartTimestamp) < 172800, "Varadium: Time to block accounts has expired");
		}
		_isBlocked[account] = true;
		emit BlockedAccountChange(account, true);
	}
	function unblockAccount(address account) external onlyOwner {
		require(_isBlocked[account], "Varadium: Account is not blcoked");
		_isBlocked[account] = false;
		emit BlockedAccountChange(account, false);
	}
	function excludeFromFees(address account, bool excluded) external onlyOwner {
		require(_isExcludedFromFee[account] != excluded, "Varadium: Account is already the value of 'excluded'");
		_isExcludedFromFee[account] = excluded;
		emit ExcludeFromFeesChange(account, excluded);
	}
	function excludeFromMaxTransactionLimit(address account, bool excluded) external onlyOwner {
		require(_isExcludedFromMaxTransactionLimit[account] != excluded, "Varadium: Account is already the value of 'excluded'");
		_isExcludedFromMaxTransactionLimit[account] = excluded;
		emit ExcludeFromMaxTransferChange(account, excluded);
	}
	function excludeFromMaxWalletLimit(address account, bool excluded) external onlyOwner {
		require(_isExcludedFromMaxWalletLimit[account] != excluded, "Varadium: Account is already the value of 'excluded'");
		_isExcludedFromMaxWalletLimit[account] = excluded;
		emit ExcludeFromMaxWalletChange(account, excluded);
	}
	function excludeFromDividends(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromDividends[account] != excluded, "Varadium: Account is already the value of 'excluded'");
		if(excluded) {
	...
	function setWallets(address newLiquidityWallet, address newOperationsWallet, address newBuyBackWallet) external onlyOwner {
		if(liquidityWallet != newLiquidityWallet) {
			require(newLiquidityWallet != address(0), "Varadium: The liquidityWallet cannot be 0");
			emit WalletChange('liquidityWallet', newLiquidityWallet, liquidityWallet);
			liquidityWallet = newLiquidityWallet;
		}
		if(operationsWallet != newOperationsWallet) {
			require(newOperationsWallet != address(0), "Varadium: The operationsWallet cannot be 0");
			emit WalletChange('operationsWallet', newOperationsWallet, operationsWallet);
			operationsWallet = newOperationsWallet;
		}
		if(buyBackWallet != newBuyBackWallet) {
			require(newBuyBackWallet != address(0), "Varadium: The buyBackWallet cannot be 0");
	...
	function setUniswapRouter(address newAddress) external onlyOwner {
		require(newAddress != address(uniswapV2Router), "Varadium: The router already has that address");
		emit UniswapV2RouterChange(newAddress, address(uniswapV2Router));
		uniswapV2Router = IRouter(newAddress);
	}
	function setMaxTransactionAmount(uint256 newValue) external onlyOwner {
		require(newValue != maxTxAmount, "Varadium: Cannot update maxTxAmount to same value");
		emit MaxTransactionAmountChange(newValue, maxTxAmount);
		maxTxAmount = newValue;
	}
	function setMaxWalletAmount(uint256 newValue) external onlyOwner {
		require(newValue != maxWalletAmount, "Varadium: Cannot update maxWalletAmount to same value");
		emit MaxWalletAmountChange(newValue, maxWalletAmount);
		maxWalletAmount = newValue;
	}
	function setMinimumTokensBeforeSwap(uint256 newValue) external onlyOwner {
		require(newValue != minimumTokensBeforeSwap, "Varadium: Cannot update minimumTokensBeforeSwap to same value");
		emit MinTokenAmountBeforeSwapChange(newValue, minimumTokensBeforeSwap);
		minimumTokensBeforeSwap = newValue;
	}
	function claimLaunchTokens() external onlyOwner {
		require(_launchStartTimestamp > 0, "Varadium: Launch must have occurred");
		require(!_launchTokensClaimed, "Varadium: Launch tokens have already been claimed");
		require(block.number - _launchBlockNumber > 5, "Varadium: Only claim launch tokens after launch");
		uint256 tokenBalance = balanceOf(address(this));
		_launchTokensClaimed = true;
		require(launchTokens <= tokenBalance, "Varadium: A swap and liquify has already occurred");
	...
	function claimBNBOverflow(uint256 amount) external onlyOwner {
		require(amount < address(this).balance, "Varadium: Cannot send more than contract balance");
	...
	function tokenFromReflection(uint256 rAmount) public view returns(uint256) {
		require(rAmount <= _rTotal, "Varadium: Amount must be less than total reflections");
		uint256 currentRate =  _getRate();
		return rAmount / currentRate;
	}
	function reflectionFromToken(uint256 tAmount, bool deductTransferFee) external view returns (uint256) {
		require(tAmount <= _tTotal, "Varadium: Amount must be less than supply");
		uint256 currentRate = _getRate();
	...
	uint256 amount
		...
		require(amount > 0, "Varadium: Transfer amount must be greater than zero");
		require(amount <= balanceOf(from), "Varadium: Cannot transfer more than balance");

		if(!_isAllowedToTradeWhenDisabled[from] && !_isAllowedToTradeWhenDisabled[to]) {
			require(isTradingEnabled, "Varadium: Trading is currently disabled.");
			require(!_isBlocked[to], "Varadium: Account is blocked");
			require(!_isBlocked[from], "Varadium: Account is blocked");
			if (!_isExcludedFromMaxTransactionLimit[to] && !_isExcludedFromMaxTransactionLimit[from]) {
				require(amount <= maxTxAmount, "Varadium: Buy amount exceeds the maxTxBuyAmount.");
			}
			if (!_isExcludedFromMaxWalletLimit[to]) {
				require((balanceOf(to) + amount) <= maxWalletAmount, "Varadium: Expected wallet amount exceeds the maxWalletAmount.");
			}
		}
	...
	function _getTValues(uint256 tAmount, bool takeFee) private view returns (uint256,uint256,uint256){
		if (!takeFee) {
			...
			uint256 tOther = tAmount * (_liquidityFee + _operationsFee + _buyBackFee) / 100;
	...
	function _setCustomSellTaxPeriod(CustomTaxPeriod storage map,
        ...
        require((_liquidityFeeOnSell+_operationsFeeOnSell+_buyBackFeeOnSell+_holdersFeeOnSell) <= 10, "Varadium: Tax exceeds maximum value of 10%");
	...
	function _setCustomBuyTaxPeriod(CustomTaxPeriod storage map,
        ...
        require((_liquidityFeeOnBuy+_operationsFeeOnBuy+_buyBackFeeOnBuy+_holdersFeeOnBuy) <= 10, "Varadium: Tax exceeds maximum value of 10%");
	...
	function _swapAndLiquify() private {
		...
		uint8 operationsFeePrior = _operationsFee;
		...
		uint256 totalBNBFee = totalFeePrior - (liquidityFeePrior / 2) - (holdersFeePrior);

		uint256 amountBNBLiquidity = BNBBalanceAfterSwap * liquidityFeePrior / totalBNBFee / 2;
		uint256 amountBNBOperations = BNBBalanceAfterSwap * operationsFeePrior / totalBNBFee;
		uint256 amountBNBBuyBack = BNBBalanceAfterSwap - (amountBNBLiquidity + amountBNBOperations);

        Address.sendValue(payable(operationsWallet),amountBNBOperations);
        Address.sendValue(payable(buyBackWallet),amountBNBBuyBack);
		...
		_operationsFee = operationsFeePrior;
