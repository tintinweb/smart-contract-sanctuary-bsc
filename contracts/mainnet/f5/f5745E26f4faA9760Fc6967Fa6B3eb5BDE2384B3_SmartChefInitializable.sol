...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        bool isRewardAllowed = this.isRewardAllowed();
        if (isRewardAllowed) {
            _updatePool();
    ...
    /*
     * @notice View function to see if reward is allowed
     * @param _user: user address
     * @return Boolean variable representing if reward is allowed or not
     */
    function isRewardAllowed() external view returns (bool) {
        return block.number >= lastRewardBlock + lockPeriod;
    }
