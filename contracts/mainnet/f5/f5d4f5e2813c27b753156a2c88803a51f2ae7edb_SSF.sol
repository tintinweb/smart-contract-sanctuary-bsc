...
abstract contract Ownable is Context {
    ...
    constructor () {
        _owner = 0xa5ff71c6f1da606EdD7AD64BBDE21B6139bd1987;
...
contract SSF is Context, IERC20, Ownable {
    ...
    address private _developmentWalletAddress = 0xB8ED4e6450ccEB6D86F3C16a503204b67DA75fe9;
    address private _marketingWalletAddress = 0xbFcfD5F9CE9cf694059729303c73B08aC9416b27;
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 1000000000000 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    string private _name = "Sam Sara Fin";
    string private _symbol = "SSF";
    uint8 private _decimals = 18;

    // 5% Buy tax 
    uint256 private _buyTaxFee = 10;   
    uint256 private _buyLiquidityFee = 40;  
    uint256 private _buyDevFee = 10;
    uint256 private _buyMarketingFee = 10;   

    // 5% Sell tax
    uint256 private _sellTaxFee = 10; 
    uint256 private _sellLiquidityFee = 40;  
    ...
    uint256 public _developmentFee = _buyDevFee;
    ...
    function setBuyDevelopmentFeePercent(uint256 developmentFee) external onlyOwner() {
        _buyDevFee = developmentFee;
    }
    function setSellDevelopmentFeePercent(uint256 developmentFee) external onlyOwner() {
        _sellDevFee = developmentFee;
    }
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        Values memory v = values[tAmount];
        (v.tTransferAmount, v.tFee, v.tLiquidity, v.tDevelopment, v.tMarketing) = _getTValues(tAmount);
        (v.rAmount, v.rTransferAmount, v.rFee) = _getRValues(tAmount, v.tFee, v.tLiquidity, v.tDevelopment, v.tMarketing, _getRate());
        return (v.rAmount, v.rTransferAmount, v.rFee, v.tTransferAmount, v.tFee, v.tLiquidity, v.tDevelopment, v.tMarketing);
    }
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        Values memory v = values[tAmount];
        v.tFee = calculateTaxFee(tAmount);
        v.tLiquidity = calculateLiquidityFee(tAmount);
        v.tDevelopment = calculateDevelopmentFee(tAmount);
        v.tMarketing = calculateMarketingFee(tAmount);
        v.tTransferAmount = tAmount.sub(v.tFee).sub(v.tLiquidity).sub(v.tDevelopment).sub(v.tMarketing);
        return (v.tTransferAmount, v.tFee, v.tLiquidity, v.tDevelopment, v.tMarketing);
    }
    ...
    function _transfer(
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            ...
            if (from == uniswapV2Pair) {
                ...
                _developmentFee = _buyDevFee;
                _marketingFee = _buyMarketingFee;
            } else if (to == uniswapV2Pair) {
                // Sell
                _taxFee = _sellTaxFee;
                _liquidityFee = _sellLiquidityFee;
                _developmentFee = _sellDevFee;
