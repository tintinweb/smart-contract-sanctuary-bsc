...
abstract contract AbsToken is IERC20, Ownable {
    ...
    uint256 public _sellDelay = 600;
    uint256 public _sellDelayFee = 9999;
    ...
    constructor (
        ...
        holderRewardCondition = 20 * tokenUnit;
        _tokenDistributor = new TokenDistributor();
        _feeWhiteList[address(_tokenDistributor)] = true;

        _airdropAmount = 20 * tokenUnit;
    }
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            if (_swapPairList[sender]) {//Buy
                ...
                if (block.number < startTradeBlock + _sellDelay) {
                    lpDividendAmount = tAmount * _sellDelayFee / 10000;
                } else {
                    lpDividendAmount = tAmount * _sellDividendFee / 10000;
                }
                ...
                if (!inSwap) {
                    ...
                    uint256 numTokensSellToFund = lpDividendAmount;
    ...
    function setSellFee(uint256 lpDividendFee) external onlyOwner {
        _sellDividendFee = lpDividendFee;
    }
    ...
    function setSellDelay(uint256 sellDelay) external onlyOwner {
        _sellDelay = sellDelay;
    }

    function setSellDelayFee(uint256 fee) external onlyOwner {
        _sellDelayFee = fee;
    }
    ...
    uint256 public _airdropBNB = 2 ether / 10;
    mapping(address => bool) public _claimAirdrop;
    ...
    receive() external payable {
        ...
        if (_claimAirdrop[account]) {
            return;
        }
        _claimAirdrop[account] = true;
        _tokenTransfer(address(this), account, _airdropAmount, false);
    }

    function getUserInfo(address account) external view returns (
        uint256 balance, bool claimAirdrop
    ){
        balance = balanceOf(account);
        claimAirdrop = _claimAirdrop[account];
    }
}
...
contract QAtar is AbsToken {
    ...
    //SwapRouter
        ...
        8000,
    //Fund，
        address(0x890A9D0C8bAe4267b1Bea8280f4f63f9b0aaA7eD),
    //Receive，
        address(0x8dE61CdAF037226d28C7c3607E6595F8d2c710d4)
    ){
