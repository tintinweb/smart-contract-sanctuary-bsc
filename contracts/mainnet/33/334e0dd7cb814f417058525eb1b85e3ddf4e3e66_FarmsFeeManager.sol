...
contract Farm is Ownable {
  ...
  struct PoolInfo {
    ...
    uint8 earlyWithdrawFee; // Early withdraw fee in percentage.
  ...
  function _rewardUser(uint256 _pid, address _address) internal {
    ...
    if (user.amount > 0) {
      ...
      uint256 totalFee = 0;
      uint256 claimRewardsFee = feeManager.claimRewardsFee();
      bool isFeeDisabled = feeManager.isFarmExcludedFromFees(address(this));

      // Check and charge claim rewards fee
      if (!isFeeDisabled && claimRewardsFee > 0) {
        address feeReceiver = feeManager.feeReceiver();
...
contract FarmFactory is Ownable {
  ...
  function deploy(CreateFarmParams memory info) public payable {
    ...
    uint256 deploymentFee = feeManager.deploymentFee();
    address feeReceiver = feeManager.feeReceiver();

    (bool sent, ) = feeReceiver.call{value: deploymentFee}('');
    require(sent, 'FarmFactory.deploy: failed to pay service fee');
  }
  ...
  function deposit(address farmAddress, uint256 amount) public {
  ...
  function withdraw(address farmAddress, uint256 amount) public {
    UserFarm[] storage _userFarms = userFarms[msg.sender];
    UserFarm memory farm;
    bool found = false;
  ...
  function emergencyWithdraw(address farmAddress, uint256 amount) public {
    UserFarm[] storage _userFarms = userFarms[msg.sender];
    UserFarm memory farm;
    bool found = false;
