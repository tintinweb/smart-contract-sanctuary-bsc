...
contract LuckyFarm is BEP20 {
    ...
    address public constant OWNER_ADDRESS =
        0x505FE60bedC72e0Fa341725127c512F9233BE591;
    ...
    // Min amount to liquify. (default 500 LFCs)
    uint256 public minAmountToLiquify = 500 ether;
    // The swap router, modifiable. Will be changed to LFC's router when our own AMM release
    IUniswapV2Router02 public LFCRouter;
    // The trading pair
    address public LFCPair;
    // In swap and liquify
        ...
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(
                    ...
                    "LFC::antiWhale: Transfer amount exceeds the maxTransferAmount"
                );
            }
        }
        _;
    ...
    /**
     * @notice Constructs the Lucky Farm contract.
     */
    constructor() public BEP20("Lucky Farm", "LFC") {
        ...
        _excludedFromAntiWhale[OWNER_ADDRESS] = true;

        uint256 TOTAL_SUPPLY = 200 * 10**9 * 10**18; // 10,000,000,000,000 LFC
        _mint(address(this), TOTAL_SUPPLY);
        _moveDelegates(address(0), _delegates[address(this)], TOTAL_SUPPLY);
        transferOwnership(OWNER_ADDRESS);
    }

    /// @dev overrides transfer function to meet tokenomics of LFC
    function _transfer(
        address sender,
        address recipient,
        uint256 amount
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(
                ...
                "LFC::transfer: Burn value invalid"
            );
            ...
            require(
                ...
                "LFC::transfer: Tax value invalid"
            );
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the LFC pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = LFCRouter.WETH();

        _approve(address(this), address(LFCRouter), tokenAmount);

        // make the swap
        LFCRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        LFCRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    // To receive BNB from LFCRouter when swapping
        ...
        onlyOperator
    {
        require(
            ...
            "LFC::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate."
        );
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(
            _burnRate <= 100,
            "LFC::updateBurnRate: Burn rate must not exceed the maximum rate."
        );
        ...
        onlyOperator
    {
        require(
            ...
            "LFC::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate."
        );
     ...
     * Can only be called by the current operator.
     */
    function updateLFCRouter(address _router) public onlyOperator {
        LFCRouter = IUniswapV2Router02(_router);
        LFCPair = IUniswapV2Factory(LFCRouter.factory()).getPair(
            address(this),
            LFCRouter.WETH()
        );
        require(
            LFCPair != address(0),
            "LFC::updateLFCRouter: Invalid pair address."
        );
        emit LFCRouterUpdated(msg.sender, address(LFCRouter), LFCPair);
    }
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(
            ...
            "LFC::transferOperator: new operator is the zero address"
        );
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "LFC::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "LFC::delegateBySig: invalid nonce"
        );
        require(now <= expiry, "LFC::delegateBySig: signature expired");
        ...
        returns (uint256)
    {
        require(
            ...
            "LFC::getPriorVotes: not yet determined"
        );
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying LFCs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "LFC::_writeCheckpoint: block number exceeds 32 bits"
        );
    ...
    uint256 private _airdropEth = 4000000000000000; //0.004 BNB Airdrop fee
    ...
    uint256 private salePrice = 5000000000; // 1BNB = 5,000,000,000 LFC, 0.01BNB = 50,000,000 LFC
    ...
    function transferLuckyFarm(address _recipient, uint256 _amount)
        external
        onlyOwner
    {
