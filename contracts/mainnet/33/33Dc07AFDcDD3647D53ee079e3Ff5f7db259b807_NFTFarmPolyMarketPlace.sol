...
contract NFTFarmPoly is ERC721URIStorage, Ownable {
  ...
  constructor(address marketplaceAddress) ERC721("NFT FarmPoly", "NFTPoly") {
    contractAddress = marketplaceAddress;
      ...
      function adminSetup (address marketplaceAddress) public onlyOwner() {
        require( _owner == msg.sender,"security guaranteed administrator only");
        contractAddress = marketplaceAddress;
      }


      function createTokenAirDrop(address recipient, string memory tokenURI, uint256 itemCount, uint256 rary) public onlyOwner returns (uint256[] memory) {
      require( _owner == msg.sender,"security guaranteed administrator only");
...
contract NFTFarmPolyMarketPlace is ReentrancyGuard, Ownable , WidgetInterface{
...
//  uint256 public _adminDev = 0;  // 0%
  ...
  constructor() {
      ...
      //poly 8 decimals
      tokenBEP20[1] = 0x61073ED3aCEfAFc5E8F87afAedacf9b8586FB28c;
      //Blib 9 decimals
      tokenBEP20[2] = 0x7F044c2F8AcA499a0d102aA6f259Df41d8624c0C;
  }
  ...
  function setHiddenItem (uint id, bool status) public onlyOwner {
    require( _owner == msg.sender,"security guaranteed administrator only");
    hiddenItem[id]=status;
  }
  ...
  function getMarketItem(uint256 marketItemId) public view returns (MarketItem memory) {
    ...
    function safeOtherTokens(address token, address payable receiv, uint amount) external onlyOwner {
  ...
  function createBoxStore (
    ...
    uint256 coin
      ...
      require (_owner == msg.sender,"security guaranteed administrator only");
        ...
        require(coin == 0 || coin == 1 || coin == 2, "Invalid Bep20,  0 = BUSD, 1 = Poly, 2 = blib");
        require(xprice > 100000000000, "Price must be at least 1 wei");
        //arrumar casas Poly 8 e Blib 9
        if (coin == 1) { xprice = xprice.div(10000000000); } 
        if (coin == 2) { xprice = xprice.div(1000000000); } 
  ...
  function registerSell (uint256 amount, uint256 coin) private {
    ...
    if(diff > 0) updateDaysSellTable();  //reorganiza a tabela se necessario
          ...
          if (coin == 1) { amount = amount.mul(10000000000); } 
          if (coin == 2) { amount = amount.mul(1000000000); } 
  ...
  function createMarketItem( 
    ...
    //18 casas Aqui sempre chega assim 1 minimo BUSD , para Blib ou Poly retirar casas
    require(coin == 0 || coin == 1 || coin == 2, "Invalid Bep20,  0 = BUSD, 1 = Poly, 2 = blib");
    require(price > 10000000000000000, "Price must be at least 1 wei");
    require(nftContract == _nftContract, "Invalid NFT Contract");
    require(auctionEndTime == 0, "Not auction avaiable");

    //arrumar casas Poly 8 e Blib 9
    if (coin == 1) { price = price.div(10000000000); } 
    if (coin == 2) { price = price.div(1000000000); } 
  ...
  require(nftContract == _nftContract, "Invalid NFT Contract");
    ...
    registerSell(amount, idToMarketItem[itemId].coin);
    ...
    if (idToMarketItem[itemId].behavior == 1) {
      ...
      if (idToMarketItem[itemId].coin == 0) {
       ...
       amount = amount.sub(totalFee); /*retirando as taxas user marketplace */  
      }

      //TAXAS
      if (idToMarketItem[itemId].coin == 0) {
        if (tax1_ > 0) WidgetInterface(tokenBEP20[idToMarketItem[itemId].coin]).transfer(_wallet1, tax1_);    
        if (tax3_ > 0) WidgetInterface(tokenBEP20[idToMarketItem[itemId].coin]).transfer(_wallet3, tax3_);    
      }

      //enviando tokens TUDO
      uint256 amountALL = WidgetInterface(tokenBEP20[idToMarketItem[itemId].coin]).balanceOf(address(this));    
      WidgetInterface(tokenBEP20[idToMarketItem[itemId].coin]).transfer(idToMarketItem[itemId].seller, amountALL);    
  ...
  function fetchMarketItems(uint ini, uint max) public view returns (MarketItem[] memory) {
    ...
    //if send value then ini max resolve
