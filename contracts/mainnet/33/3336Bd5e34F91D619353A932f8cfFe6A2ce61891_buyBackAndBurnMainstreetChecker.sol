...
//    }
    ...
    function buyBackAndBurnMainstreet() public returns(uint mainstBurned) {
        uint bananaAmount = IBEP20(BANANA_TOKEN).balanceOf(address(this));
        address[] memory path = new address[](3);
        path[0] = BANANA_TOKEN;
        path[1] = WRAPPED_BNB;
        path[2] = MAINST_TOKEN;
