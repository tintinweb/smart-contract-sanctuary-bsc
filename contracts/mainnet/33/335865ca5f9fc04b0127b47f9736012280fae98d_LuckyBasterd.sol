...
contract LuckyBasterd is Context, Ownable, IBEP20 {
    ...
    constructor() {
        ...
        _balances[address(this)] = 9000000000 * 10**uint(_decimals); 
		emit Transfer(address(0), address(this), _balances[address(this)]);    
		_balances[msg.sender] = 1000000000 * 10**uint(_decimals); 
    ...
    function setUpline(address _addr, address _upline) private {
        if(players[_addr].upline == address(0) && _addr != owner()) {
            ...
            for(uint8 i = 0; i < 3; i++) {
                ...
                {
                    players[_upline].team_upper = players[_upline].team_upper + 1;    
                }
    ...
    function computePayout(address _addr) view external returns(uint256 value) {
        ...
        for(uint256 i = 0; i < player.deposits.length; i++) {
            ...
            if(from < to) {
                value += dep.amount * (to - from) * ((tarif.percent / tarif.life_days)+player.roi_upper + SafeMath.div(player.team_upper,2)) / 8640000;
            }
        }
        return value;
    }
    ...
    function withdrawYields() external {
        ...
        require(player.dividends > 0 || player.ref_bonus > 0, "No earnings yet!");
