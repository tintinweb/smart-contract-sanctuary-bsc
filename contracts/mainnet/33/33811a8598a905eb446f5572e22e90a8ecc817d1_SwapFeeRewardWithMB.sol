...
interface IAnpanswapNFT {
    function accrueMB(address user, uint amount) external;
    ...
    function getInfoForStaking(uint tokenId) external view returns(address tokenOwner, bool stakeFreeze, uint monkeyBoost);
}
...
contract SwapFeeRewardWithMB is Ownable, ReentrancyGuard {
    ...
    uint public maxAccruedMBInPhase = 5000 ether;

    uint public currentPhase = 1;
    uint public currentPhaseMB = 1;
    uint256 public totalMined = 0;
    uint public totalAccruedMB = 0;
    uint public mbWagerOnSwap = 1500; //Wager of MB
    uint public mbPercentMarket = 10000; // (div 10000)
    uint public mbPercentAuction = 10000; // (div 10000)
    IAnpanToken public anpanToken;
    IOracle public oracle;
    IAnpanswapNFT public anpanswapNFT;
    address public targetToken;
    address public targetMBToken;
    ...
    //percent of distribution between feeReward and monkeyBoost [0, 90] 0 => 90% feeReward and 10% monkeyBoost; 90 => 100% monkeyBoost
    ...
    event NewPhase(uint);
    event NewPhaseMB(uint);
    event NewAuction(address);
    event NewAnpanswapNFT(IAnpanswapNFT);
    ...
    constructor(
        ...
        IOracle _Oracle,
        IAnpanswapNFT _anpanswapNFT,
        address _targetToken,
        address _targetMBToken
    ...
    function getSwapFee(address tokenA, address tokenB) internal view returns (uint swapFee) {
        //SFR-05
        swapFee = uint(1000).sub(IAnpansPair(pairFor(tokenA, tokenB)).swapFee());
    }
    ...
    function setPhaseMB(uint _newPhase) public onlyOwner returns (bool){
        currentPhaseMB = _newPhase;
        //BNF-01, SFR-01
        emit NewPhaseMB(_newPhase);
        return true;
    }
    ...
    view
    returns(
        uint feeReturnInANPAN,
        uint feeReturnInUSD,
        uint monkeyBoostAccrue
    )
    {
        ...
        if (pool.pair != pair || pool.enabled == false || !isWhitelist(input) || !isWhitelist(output)) {
            ...
            (uint feeAmount, uint mbAmount) = calcAmounts(amount, account);
            uint256 fee = feeAmount.div(pairFee);
            uint256 quantity = getQuantity(output, fee, targetToken);
            feeReturnInANPAN = quantity.mul(pool.percentReward).div(100);
            monkeyBoostAccrue = getQuantity(output, mbAmount.div(mbWagerOnSwap), targetMBToken);
            feeReturnInUSD = getQuantity(targetToken, feeReturnInANPAN, targetMBToken);
        }
    }
    ...
    function swap(address account, address input, address output, uint256 amount) public onlyRouter returns (bool) {
        ...
        (uint feeAmount, uint mbAmount) = calcAmounts(amount, account);
        uint256 fee = feeAmount.div(pairFee);
        mbAmount = mbAmount.div(mbWagerOnSwap);
        //SFR-05
        _accrueMB(account, output, mbAmount);
    ...
    function calcAmounts(uint amount, address account) internal view returns(uint feeAmount, uint mbAmount){
        feeAmount = amount.mul(defaultFeeDistribution.sub(feeDistribution[account])).div(100);
        mbAmount = amount.sub(feeAmount);
    }

    function accrueMBFromMarket(address account, address fromToken, uint amount) public onlyMarket {
        //SFR-05
        amount = amount.mul(mbPercentMarket).div(10000);
        _accrueMB(account, fromToken, amount);
    }

    function accrueMBFromAuction(address account, address fromToken, uint amount) public onlyAuction {
        //SFR-05
        amount = amount.mul(mbPercentAuction).div(10000);
        _accrueMB(account, fromToken, amount);
    }

    //SFR-05
    function _accrueMB(address account, address output, uint amount) private {
        uint quantity = getQuantity(output, amount, targetMBToken);
        if (quantity > 0) {
            //SFR-06
            totalAccruedMB = totalAccruedMB.add(quantity);
            if(totalAccruedMB <= currentPhaseMB.mul(maxAccruedMBInPhase)){
                anpanswapNFT.accrueMB(account, quantity);
            }
        }
    }
    ...
    function withdraw(uint8 v, bytes32 r, bytes32 s) public nonReentrant returns (bool){
        ...
        if (balance > 0) {
            ...
            //SFR-04
            if(anpanToken.transfer(msg.sender, balance)){
    ...
    function getQuantity(address outputToken, uint256 outputAmount, address anchorToken) public view returns (uint256) {
        ...
        if (outputToken == anchorToken) {
        ...
        } else if (IAnpansFactory(factory).getPair(outputToken, anchorToken) != address(0) && checkPairExist(outputToken, anchorToken)) {
            ...
            for (uint256 index = 0; index < length; index++) {
                ...
                if (IAnpansFactory(factory).getPair(outputToken, intermediate) != address(0) && IAnpansFactory(factory).getPair(intermediate, anchorToken) != address(0) && checkPairExist(intermediate, anchorToken)) {
    ...
    function setAnpanswapNFT(IAnpanswapNFT _anpanswapNFT) public onlyOwner {
        require(address(_anpanswapNFT) != address(0), "SwapMining: new anpanswapNFT is the zero address");
        anpanswapNFT = _anpanswapNFT;
        //BNF-01, SFR-01
        emit NewAnpanswapNFT(_anpanswapNFT);
    }
    ...
    function setMonkeyBoostReward(uint _mbWagerOnSwap, uint _percentMarket, uint _percentAuction) public onlyOwner {
        mbWagerOnSwap = _mbWagerOnSwap;
        mbPercentMarket = _percentMarket;
        mbPercentAuction = _percentAuction;
    }
