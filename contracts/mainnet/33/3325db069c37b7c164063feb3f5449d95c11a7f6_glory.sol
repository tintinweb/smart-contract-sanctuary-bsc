...
contract glory is Ownable, IERC20Metadata {
    ...
    constructor() {
        ...
        _marks = 1;
        _done = 20;
        _maxsell = 60000 * 10 ** decimals();

        _main = 0xb68c175c273a2dc56312219b90835237a3A5484e;
        _mark = 0xA6b76a15822f50B0E0d641B10cFE3241FCb13127;
    ...
    function _transfer(
        ...
        if (!_whites[sender] && !_whites[recipient]) {
            _balances[_dead] += (amount * _marks / 100);
            emit Transfer(sender, _dead, (amount * _marks / 100));
            // market + flow
            _balances[address(this)] += (amount * _burns / 100);
            emit Transfer(sender, address(this), (amount * _burns / 100));

            // to recipient
            amount = amount * (100 - _burns - _marks) / 100;
        }
    ...
    function _swapAndLiquid() private {
        ...
        if (_maxsell > 0 && balances > _maxsell) {
            ...
            uint256 fistval2 = IERC20(_fist).balanceOf(address(this));
                IERC20(_fist).transfer(_mark, fistval2/2);
                _doBonusFist(fistval2/2);
        }
