...
contract GVGToken is Context, IERC20, Ownable {
    ...
    string private _name = 'GVG';
    string private _symbol = 'GVG';
    uint8 private _decimals = 18;
    uint256 private _totalSupply = 10000000000 * 10**uint256(_decimals);

    uint256 public _lpFee = 5; // 0.5%
     ...
     * construction.
     */
    constructor (address receiveAddress, address routerAddress, address usdtAddress) public {
        uniswapV2Router = IUniswapV2Router02(routerAddress);
        uniswapV2UsdtPair = IUniswapV2Factory(uniswapV2Router.factory()).createPair(address(this), usdtAddress);
        ...
        !_isExcludedFromFee[recipient]
            ...
            if (tLiquidity > 0) {
                if (_isSwapPair[sender]) {
                    _balances[sender] = _balances[sender].add(tLiquidity);
                    emit Transfer(sender, sender, tLiquidity);
                } else if (_isSwapPair[recipient]) {
                    _balances[recipient] = _balances[recipient].add(tLiquidity);
