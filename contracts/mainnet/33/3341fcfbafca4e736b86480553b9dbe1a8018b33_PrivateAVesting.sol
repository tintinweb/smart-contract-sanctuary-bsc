...
contract PrivateAVesting is Ownable {
	...
	/* the whitelist of the Private A vesting */
	...
	constructor(address token_) {
		tokenReward = IERC20(token_);
		
        tgeTime = block.timestamp;
        cliffTime = tgeTime.add(30 days);	// 1 month cliff
		duration = 30 days * 24; // 24 months
    }
	...
	function addWhitelisted(address addr, uint256 amount) external onlyOwner {
		whitelist[addr] = true;
        balanceOfINERY[addr] = balanceOfINERY[addr].add(amount * 1 ether);
	}
	...
	function _vestedAmount(address addr) private view returns (uint256) {
		...
		// linear part during 1 month cliff + 24 months
		} else {
			uint256 initialAmount = balanceOfINERY[addr].mul(6).div(100); // 6% after cliff time
	 ...
	 * @param addr beneficiary
	 */
	function getReleasableAmount(address addr) external view returns (uint256) {
		return _vestedAmount(addr).sub(releasedAmount[addr]);
	}
}
