...
contract OfferNFT is Ownable{
    ...
    function setHolders(uint256 base_, Holder[] memory holders_) external onlyOwner{
        if(_holders.length > 0) delete _holders;
        uint32 sum = 0;
        for(uint256 i = 0; i < holders_.length; i++){
            _holders.push(holders_[i]);
            sum += holders_[i].rate;
        }
        require(sum == base_, "sumError");
        base = base_;
    }
    
    function holders() external view returns(Holder[] memory){
        return _holders;
    }
    ...
    function mint(uint256 round, uint32 quantity) external {
        ...
        uint256 reserve2 = amount2;
        for(uint256 i = 0; i < _holders.length; i++){
            Holder memory holder = _holders[i];
            uint256 tAmount1 = (i == _holders.length - 1) ? reserve1 : (amount1 * holder.rate / base);
            reserve1 -= tAmount1;
            IERC20(r.currency1).transferFrom(msg.sender, holder.account, tAmount1);
            uint256 tAmount2 = (i == _holders.length - 1) ? reserve2 : (amount2 * holder.rate / base);
            reserve2 -= tAmount2;
            IERC20(r.currency2).transferFrom(msg.sender, holder.account, tAmount2);
        }
