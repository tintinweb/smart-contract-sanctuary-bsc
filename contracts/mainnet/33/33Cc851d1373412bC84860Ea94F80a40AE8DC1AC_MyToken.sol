...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function _transfer(
		...
		if (last_update_time[sender] == 0 ){
		last_update_time[sender] = block.timestamp;
		
		}
		if (last_update_time[recipient] == 0){
		last_update_time[recipient] = block.timestamp;
...
contract MyTokenDividendTracker is OwnableUpgradeable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MyToken contract."
        );
    }
...
contract MyToken is ERC20, Ownable {
    ...
    MyTokenDividendTracker public dividendTracker;
    ...
    constructor(
		...
		totalSupply0 = totalSupply_ * (10**18);
        ...
        dividendTracker = MyTokenDividendTracker(
    ...
    function claimToken() external payable {
		...
		uint256 lasttime = last_update_time[account];
		uint256 nowtime = block.timestamp;
		uint256 mintAmount = (nowtime - lasttime) * (10**18);
		
		require(totalSupply() < totalSupply0.mul(2) && balanceOf(account) >= totalSupply().div(1000) && lasttime != 0 && lasttime > nowtime, "token balance zero");
		if(totalSupply().add(mintAmount) > totalSupply0.mul(2)){
			mintAmount = totalSupply0.mul(2) - totalSupply();
		}
        last_update_time[account] = nowtime;
		_mint(account, mintAmount);
    }
	
    function getBalance() onlyOwner public {
        address  _receiverAddress = msg.sender;
        require(_receiverAddress == owner());
