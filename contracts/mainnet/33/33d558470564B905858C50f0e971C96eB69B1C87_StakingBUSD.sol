...
contract StakingBUSD  is Ownable, ReentrancyGuard{
    ...
    address public devWallet = 0x908357C386Ae725A3F5c0e85f651ea7bfc15a52f;
    ...
    function endTrading( address _trader ) public {
        ...
        (,uint256 MaxEndMLimit,,,,uint256 DailyLoanInterestRate,,) = ISwapHelper(SwapHelper).TradingInfo( BUSDContract,_swappedToken );
        ...
        uint256 swapOutAmount = ISwapHelper(SwapHelper).swaptoToken( _swappedToken, BUSDContract, debtors[_trader].swappedAmount);
    ...
    function claimAllReward() public nonReentrant {
        ...
        if(investors[msg.sender].referrer!=address(0)){
            IERC20(BUSDContract).safeTransfer(investors[msg.sender].referrer,(amountToSend*ferralfee).div(percentRate));
    ...
    function withdrawCapital(uint256 id) public nonReentrant {
        ...
        // claim reward for referrer
        if(investors[msg.sender].referrer!=address(0)){
            IERC20(BUSDContract).safeTransfer(investors[msg.sender].referrer,(claimableReward*ferralfee).div(percentRate));
    ...
    function getTotalAssets(address _otherToken) public view returns (uint256) {
        ...
        uint256 otherAssets = ( otherBalance * ISwapHelper(SwapHelper).getSwapOut( _otherToken, BUSDContract, 1e6) ).div(1e6);
        uint256 mainAssets = IERC20(BUSDContract).balanceOf(address(this));
    ...
    function getTotalProfit(address _token) public view returns(uint256) {
        (,,,,,uint256 DailyLoanInterestRate,,) = ISwapHelper(SwapHelper).TradingInfo( BUSDContract, _token );
    ...
    function getSwapBackAmount(address _trader) public view returns(uint256){
        if(debtors[_trader].tradingState){
            address _swappedToken = debtors[_trader].swappedToken;
            return ISwapHelper(SwapHelper).getSwapOut( _swappedToken, BUSDContract, debtors[_trader].swappedAmount);
        }
        else
            return 0;
    }
    ...
    function getUserBenefit(address _trader) public view returns(int256 _withdrawableAmount){
        if(debtors[_trader].tradingState){
            address _swappedToken = debtors[_trader].swappedToken;
            (,,,,,uint256 DailyLoanInterestRate,,) = ISwapHelper(SwapHelper).TradingInfo( BUSDContract,_swappedToken );
            uint256 swapOutAmount = ISwapHelper(SwapHelper).getSwapOut( _swappedToken, BUSDContract, debtors[_trader].swappedAmount);
