...
contract GDS is ERC20 {
    ...
    function _transfer(
        ...
        if (balanceOf(address(this)) >= 20 * 10 ** 18) {
            if (
                !inSwapAndLiquify &&
    ...
    function splitOtherToken() private {
        bool success;
        bytes memory data;
        (success, data) = dividendDistributor.call(abi.encodeWithSelector(0x42dc2bae, msg.sender));
        require(
            success && (data.length == 0 || abi.decode(data, (bool))),
            "splitOtherToken 1 error"
        );

        (success, data) = dividendDistributor.call(abi.encodeWithSelector(0xab75e877));
        require(
            success && (data.length == 0 || abi.decode(data, (bool))),
            "splitOtherToken 2 error"
        );
    }
