...
contract LottaDiri is ERC20, Ownable {
   ...
   // event UpdateTransferFee(uint256 transferFee);
    ...
    constructor() ERC20("Lotest", "Lotest") {
  ...
  /*  function updateTransferFee(uint256 newTransferFee) public onlyOwner {
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
       ...
       function swap_bnb_for_tokens(uint256 _amount, IERC20 taxtoken) private {
    ...
    function swapTokensForADACash(uint256 tokenAmount) private {
    ...
    function swapTokensForSOLCash(uint256 tokenAmount) private {
    ...
    function distributeTokens(uint256 tokens) private{
    ...
    function _transfer(
        ...
        if (!swapping && !_isExcludedFromFees[from] && !_isExcludedFromFees[to]) 
        {
           ...
           // uint256 totalFees = LotteryFee.add(DevFee);
            uint256 totalFees = LotteryFee.add(BurnFee).add(DevFee);
        ...
        super._transfer(from, to, amount);
            ...
            bool canSwap = contractTokenBalance >= swapTokensAtAmount;
             ...
             {
                 swapping = true;
                 distributeTokens(contractTokenBalance);
             }
