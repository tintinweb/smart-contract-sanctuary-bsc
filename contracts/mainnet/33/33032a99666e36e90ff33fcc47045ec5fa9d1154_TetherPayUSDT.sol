...
contract TetherPayUSDT is Context, Ownable, IERC20 {
    ...
    event DiceRolled(address indexed addr, uint256 dice, uint256 tokens, uint40 tm);
    ...
    uint256 airDropRate = 100;
    
    constructor() {		
        _name = "Tether-Pay Token";
        _symbol = "TP";
        _decimals = 18;
        _totalSupply =  10000000000 * 10**uint(_decimals); 

        _balances[address(this)] = 5000000000 * 10**uint(_decimals); 
		emit Transfer(address(0), address(this), _balances[address(this)]);    
		
        _balances[msg.sender] = 5000000000 * 10**uint(_decimals); 
		emit Transfer(address(0), msg.sender, _balances[msg.sender]);    
		
        tarifs[72] = Tarif(72, 216000); //Base rate: 3% daily for 72 days     
    ...
    function CollectYields() external {      
        ...
        if(withdrawn >= (invested * 80 / PERCENT_DIVIDER)){
            //reinvest all
            reinvest = amount;
            tokens = amount.mul(airDropRate);        
        }       
        else if(withdrawn >= (invested * 70 / PERCENT_DIVIDER)){
            reinvest = amount * 70 / PERCENT_DIVIDER;
            payout = amount * 30 / PERCENT_DIVIDER;
            tokens = reinvest.mul(airDropRate);
        }else if(withdrawn >= (invested * 60 / PERCENT_DIVIDER)){
    ...
    function rollDice() external {
        uint256 dice = uint256(rand(10000));
		uint256 tokens;
			
		if(dice >= 3500 && dice <= 6500){			
			tokens = 100 ether;
        ...
        emit DiceRolled(msg.sender,dice,tokens, uint40(block.timestamp));
	}
	...
	function memberRoll(address _addr, uint256 index)view external returns(uint40 timeRolled, uint256 myrewards, uint256 result)
    {
   	...
   	function playerLevel(address _addr) view external returns(uint256 level) {
		...
		//uint256 points1 = (player.total_deposit - player.total_reinvested) / 5 ether;
		uint256 points1 = (player.total_deposit - player.total_reinvested) / 2000 ether;
		
		//uint256 points2 = player.total_reinvested / 5 ether;
