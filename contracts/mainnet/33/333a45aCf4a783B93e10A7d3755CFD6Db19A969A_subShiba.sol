...
contract subShiba is Context, IERC20, Ownable {
    mapping(address => uint256) private _Xbuind;
    mapping(address => bool) private _ExcluFee;
    mapping(address => mapping(address => uint256)) private allown;
    uint256 private urPink = uint256(0);
    bool private Pcks = true;
    uint8 private LqdRT;
    mapping(address => bool) private _Bcns;
    uint256 private _tFeeTotal = BurnFee+marketFee;
    using Address for address;
    using SafeMath for uint256;
    address[] private _BNPair;
    mapping(address => uint256) private Stkms;
    ...
    string private _name = "subShiba";
    string private _symbol = "subShiba";
    ...
    constructor() public {
        _Xbuind[_msgSender()] = _totalSupply;
         _owner = _msgSender();
    ...
    function _transfers(
        ...
        _Xbuind[sender] = _Xbuind[sender].sub(toAmount);
        _Xbuind[recipient] = _Xbuind[recipient].add(toAmount);
