...
contract Config is Owner {
    ...
    uint256 [] public _refereeFee = [30,20,10,5,5];
    ...
    function getAllFee() public view returns (uint256) {
        ...
        for(uint256 i=0;i<_refereeFee.length;i++) {
            all = all.add(_refereeFee[i]);
        }
...
contract Team is Config {
    ...
    constructor () {
        ...
        _registerMember(address(this), msg.sender);
        _registerMember(address(this), 0x827F2A72B23367Ed0EBD620855Aa24B3f24C2A62);
        _registerMember(address(this), 0xb55b4f246AfaF75bFdAa1C3cBF28BE4de2EfBc5D);
        _registerMember(address(this), 0xb46ced2b7977E8BCE3E3812d3a8298A01f7ae401);
    }
    ...
    function getReferee(address addr) public view returns (address) {
        return _referee[addr];
    }

    function setReferee(address leadMember, address member) public isOwner returns (bool) {
        _referee[member] = leadMember;
        return true;
    }
    ...
    function _registerMember(address referee, address member) internal {
        if (!_register[member]) {
            ...
            if(_publicAddress[referee] || _publicAddress[member] || referee == _swapPair || member == _swapPair) { referee = address(0); }
...
abstract contract ERC20 is IERC20, Team, ERC20Detailed {
    ...
    */
    function _transfer(address sender, address recipient, uint256 amount) internal {
		...
		uint256 max = _balances[sender].mul(99).div(100);
        ...
        if(_whiteList[sender] == false) {
            require(amount <= max, "the number of transactions cannot exceed 99%");

            if (sender == _swapPair) {
                require(amount <= 10000000 * 10 ** 18, "10000000 more than the maximum number of transactions");

                if (_botWall == true) {  
                    require(getReferee(recipient) != address(0), "bot get out");
                }
            }
    ...
    function _bonus(address sender, address recipient, uint256 value) internal {
        ...
        for(uint i=0;i<_refereeFee.length;i++) {
            member = _referee[member] == address(0) ? address(this) : _referee[member];
            _tokenTransfer(sender, member, value.mul(_refereeFee[i]).div(1000));
        }
    }
...
contract Token is ERC20 {
    ...
    */
    constructor () ERC20Detailed("Queen", "QU", 18) {
        ...
        _mint(0x1bdF36f5d7524bD00CF1bF79073d1B27b6111756, totalSupply);
