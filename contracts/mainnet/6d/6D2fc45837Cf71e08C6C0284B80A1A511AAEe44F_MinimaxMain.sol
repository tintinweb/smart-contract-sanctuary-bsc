...
contract MinimaxMain is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    // Assume that all exchanges have the same interface for now
    // TODO: use generalized interface with adapter
    mapping(IERC20Upgradeable => IPancakeRouter) public tokenExchanges;

    // gelato
    IGelatoOps public gelatoOps;

    address payable public gelatoPayee; // TODO: remove when deploy clean version
    ...
    // Token exchanges
    function setTokenExchanges(IERC20Upgradeable[] calldata tokens, IPancakeRouter[] calldata exchanges)
        external
        onlyOwner
    {
    ...
    function getTokenExchangeSafe(IERC20Upgradeable token) public view returns (IPancakeRouter) {
        IPancakeRouter exchange = tokenExchanges[token];
        require(address(exchange) != address(0), "token exchange not found");
        return exchange;
    }
    ...
    function getPositionBalance(uint positionIndex) public returns (PositionBalance memory) {
        ...
        uint stakingBalance = position.callerAddress.stakingBalance(adapter, position.poolAddress);
    ...
    function stakeToken(
        ...
        require(adapter.stakedToken(stakingPool) == address(stakingToken), "stakeToken: invalid staking token");
        address rewardToken = adapter.rewardToken(stakingPool);

        stakingToken.safeTransferFrom(address(msg.sender), address(this), tokenAmount);

        uint userFeeShare = getUserFee();
        uint userFeeAmount = (tokenAmount * userFeeShare) / FEE_MULTIPLIER;
    ...
    function liquidateByIndexImpl(uint positionIndex) private {
        ...
        PositionInfo storage position = positions[positionIndex];
        position.callerAddress.withdrawAll(getPoolAdapterSafe(position.poolAddress), position.poolAddress);
    ...
    function proxyDeposit(PositionInfo storage position, uint amount) private {
        ...
        position.callerAddress.deposit(getPoolAdapterSafe(position.poolAddress), position.poolAddress, amount);
    }
    ...
    function withdrawImpl(
        ...
        if (amountAll) {
            position.callerAddress.withdrawAll(poolAdapter, position.poolAddress);
        } else {
            position.callerAddress.withdraw(poolAdapter, position.poolAddress, amount);
        }

        position.callerAddress.transferAll(position.stakedToken, position.owner);
        position.callerAddress.transferAll(position.rewardToken, position.owner);

        uint poolBalance = position.callerAddress.stakingBalance(poolAdapter, position.poolAddress);
    ...
    function swapToStable(uint positionIndex, uint stakedAmount) private {
        PositionInfo storage position = positions[positionIndex];

        IPriceOracle oracle = getPriceOracleSafe(position.stakedToken);
        IPancakeRouter tokenExchange = getTokenExchangeSafe(position.stakedToken);

        uint oracleMultiplier = 10**oracle.decimals();
        uint oraclePrice = uint(oracle.latestAnswer());

        // Optimistic conversion BUSD amount
        uint minAmountOut = (stakedAmount * oraclePrice) / oracleMultiplier;
        // Accounting slippage
        minAmountOut = minAmountOut - (minAmountOut * position.maxSlippage) / SLIPPAGE_MULTIPLIER;

        address[] memory path = new address[](3);
        path[0] = address(position.stakedToken);
        path[1] = address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c); // TODO: find more reliable solution
        path[2] = busdAddress;

        position.stakedToken.approve(address(tokenExchange), stakedAmount);

        tokenExchange.swapExactTokensForTokens(
            stakedAmount,
            minAmountOut,
            path, /* path */
            position.owner, /* to */
...
library ProxyCallerApi {
    function deposit(
        ...
        (bool success, ) = proxy.exec(
            ...
            abi.encodeWithSignature("deposit(address,uint256,bytes)", pool, amount, "") /* data */
        );
    ...
    function stakingBalance(
        ...
        (bool success, bytes memory data) = proxy.exec(
            true, /* delegate */
            address(adapter), /* target */
            abi.encodeWithSignature("stakingBalance(address,bytes)", pool, "") /* data */
        );
    ...
    function rewardBalance(
        ...
        (bool success, bytes memory data) = proxy.exec(
            true, /* delegate */
            address(adapter), /* target */
            abi.encodeWithSignature("rewardBalance(address,bytes)", pool, "") /* data */
        );
    ...
    function withdraw(
        ...
        (bool success, ) = proxy.exec(
            true, /* delegate */
            address(adapter), /* target */
            abi.encodeWithSignature("withdraw(address,uint256,bytes)", pool, amount, "") /* data */
        );
    ...
    function withdrawAll(
        ...
        (bool success, ) = proxy.exec(
            true, /* delegate */
            address(adapter), /* target */
            abi.encodeWithSignature("withdrawAll(address,bytes)", pool, "") /* data */
        );
