...
/**
Welcome to The Greenest! $GreenCoin. We have special tokenomics to keep the graph green permanently!
10 000 GreenCoins. You can only buy 40 per transactions and there is a 3 minute cooldown between each time you are allowed to buy!
You can also only sell 20 Green Coins per transcation, and there is a 1 hour cooldown between each time you are allowed to sell!

https://t.me/TheGreenestChart
...
contract TheGreenest is IBEP20, Auth {
    ...
    string constant _name = "The Greenest";
    string constant _symbol = "GreenCoin";
    ...
    address public coinFeeReceiver = 0x75457b2C690E95060DFd82a2B4222C036D50256b;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBCoin = amountBNB.mul(coinFee).div(totalBNBFee);


        (bool CoinSuccess, /* bytes memory data */) = payable(coinFeeReceiver).call{value: amountBNBCoin, gas: 30000}("");
        require(CoinSuccess, "receiver rejected ETH transfer");
   ...
   function setMaxWallet(uint256 amount) external onlyOwner() {
    ...
    function setFees(uint256 _liquidityFee, uint256 _coinFee, uint256 _buybackFee, uint256 _feeDenominator, uint256 multiplier) external authorized {
        liquidityFee = _liquidityFee;
        coinFee = _coinFee;
        buybackFee = _buybackFee;
        totalFee = _liquidityFee.add(_coinFee).add(_buybackFee);
    ...
    function setFeeReceiver(address _coinFeeReceiver, address _buybackFeeReceiver) external authorized {
        coinFeeReceiver = _coinFeeReceiver;
    ...
    function maxTxAmountSell(uint256 amountSell) external authorized {
        _maxTxAmountSell = amountSell;
        
    }
    function cooldown(uint256 cooldownsec) external authorized {
    ...
    function manualSend() external authorized {
        ...
        payable(coinFeeReceiver).transfer(contractETHBalance);
    }

    function transferForeignToken(address _token) public authorized {
        require(_token != address(this), "Can't let you take all native token");
        uint256 _contractBalance = IBEP20(_token).balanceOf(address(this));
        payable(coinFeeReceiver).transfer(_contractBalance);
    }
