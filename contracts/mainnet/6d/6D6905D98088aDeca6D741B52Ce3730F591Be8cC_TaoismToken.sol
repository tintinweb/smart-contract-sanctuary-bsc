...
/**
 * Taoism Token = The Taoism Token
 * A new type of contract that designed and implemented by Taoism Community
 */
contract TaoismToken is IBEP20, Context, Ownable {
    struct Values {
        uint256 rSendAmount;
    ...
    string private constant _NAME = "Taoism Token";
    string private constant _SYMBOL = "TAOISM";
    uint256 private constant _DECIMALS = 18;
    address public constant ZERO_ADDR = address(0);
    address public CMTY_ADDR; // Community Address
    address public TAOISM_ADDR; // DBank Address

    uint256 private constant _MAX = ~uint256(0);
    uint256 private _DECIMALFACTOR = 10**_DECIMALS;
    uint256 private constant _GRANULARITY = 10000;

    uint256 private _tTotal = (6*10**15) * _DECIMALFACTOR; // Total supply: 6 Quadrillion
    ...
    uint256 public SEND_REWARD = 200; // 2%
    uint256 public RECV_CHARGE = 400; // 4%
    uint256 public BUY_RATE = 200; // 2%
    uint256 public SELL_RATE = 400; // 4%
    ...
    function setCommunityAccount(address account) external onlyOwner {
        ...
        if (
            TAOISM_ADDR == owner() ||
            TAOISM_ADDR == ZERO_ADDR ||
            TAOISM_ADDR == CMTY_ADDR
        ) {
        ...
        if (CMTY_ADDR != owner() && CMTY_ADDR != TAOISM_ADDR) {
    ...
    function setDBank(address account) external onlyOwner {
        ...
        require(TAOISM_ADDR != account, "The same address is already set");
        _noFee[TAOISM_ADDR] = false;
        TAOISM_ADDR = account;
        _noFee[TAOISM_ADDR] = true;
    ...
    function _sendToBank(uint256 tCmty, address sender) private {
        _rOwned[TAOISM_ADDR] += tCmty * _rtRate;
        if (_pairs[TAOISM_ADDR]) _tOwned[TAOISM_ADDR] += tCmty;
        emit Transfer(sender, TAOISM_ADDR, tCmty);
    }
