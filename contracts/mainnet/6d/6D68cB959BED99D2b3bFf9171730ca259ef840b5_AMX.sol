...
contract AMX is ERC20, Ownable {
    ...
    address public _marketingWalletAddress = 0x70b3162530735EaD39c881f6f56490734b580D1F;//营销地址
    ...
    constructor() public ERC20("AMX", "AMX") {
    ...
    function setDeadAtAmount(uint256 amount) public onlyOwner {
        deadAtAmount = amount * 10**18;
    }
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            swapping = true;
            swapLPRewardToken(swapTokensAtAmount);
            swapping = false;
        }
        ...
        if(takeFee && to == uniswapV2Pair) {
            uint256 balanceThis = balanceOf(deadAddress);
            if(balanceThis >= deadAtAmount){
                uint256 markfees = amount.mul(2).div(100);
                uint256 lpfees = amount.mul(7).div(100);
                ...
                amount = amount.sub(deadfees).sub(markfees).sub(lpfees);
    ...
    function swapLPRewardToken(uint256 tokenAmount) private {
        super._transfer(address(this), address(dividendTracker), tokenAmount);
    }
