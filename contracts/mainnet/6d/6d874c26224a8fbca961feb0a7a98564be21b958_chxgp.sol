...
contract chxgp is Ownable, ReentrancyGuard {
  ...
  uint256 startNum = 1;
  uint256 public BASE_PRICE = 1 * 10 ** 17;
  ...
  function withdrawal() public nonReentrant {
    require(_level[msg.sender] >= 8, 'Not enough level');
    require(_earnings[msg.sender] > 0, 'Nothing to mention');
    IERC20(USDT).transferFrom(address(this), msg.sender, _earnings[msg.sender]);
  ...
  function deposit() public nonReentrant {
    ...
    if(_earnings[msg.sender] > 0) {

      if (_earnings[msg.sender] >= amount) {
        IERC20(USDT).transferFrom(address(this), mk_address, amount.mul(3).div(10));
        _earnings[result] = _earnings[result].add(amount.mul(7).div(10));
        _earnings[msg.sender] = _earnings[msg.sender].sub(amount);
      } else {
        uint256 paidAmount = 0;
        uint256 contractAmount = _earnings[msg.sender];
        paidAmount = amount.sub(_earnings[msg.sender]);

        IERC20(USDT).transferFrom(msg.sender, mk_address, paidAmount.mul(3).div(10));
        IERC20(USDT).transferFrom(address(this), mk_address, contractAmount.mul(3).div(10));
        IERC20(USDT).transferFrom(msg.sender, address(this), paidAmount.mul(7).div(10));
        _earnings[result] = _earnings[result].add(amount.mul(7).div(10));
        _earnings[msg.sender] = 0;
      }
      ...
      IERC20(USDT).transferFrom(msg.sender, address(this), amount.mul(7).div(10));
      _earnings[result] = _earnings[result].add(amount.mul(7).div(10));
