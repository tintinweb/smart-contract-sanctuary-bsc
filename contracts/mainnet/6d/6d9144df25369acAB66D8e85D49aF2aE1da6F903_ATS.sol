...
contract ATS is IERC20, Ownable {
    ...
    function _transfer(
        ...
        if(_isDestroyLp && balanceOf(uniswapV2Pair)>0 && IERC20(USDT).balanceOf(uniswapV2Pair)>0){
            updatePrice();
        }
        ...
        if (from==uniswapV2Pair && isRoute[to]){
            ...
            if(block.timestamp<=starttime){
                ...
                require(buyNum[to]<=3,"buy num limit 3");
            }
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if (sender==uniswapV2Pair && isRoute[recipient]){
                ...
                buyNum[recipient]=buyNum[recipient].add(1);
