...
contract TransferFee {
    ...
    internal
    {
        ...
        require(sellFee_ <= 5, "TransferFee: fee must be less or equal 20%");
        require(normalFee_ <= 1, "TransferFee: fee must be less or equal 5%");
        _transferFee.to = to_;
    ...
    function _getTransferFeeAddress()
    internal
    view
    returns (address)
    {
...
contract PLTToken is ERC20, Ownable, GasPriceController, TransferFee, DexListing {
    ...
    function _transfer(
        ...
        if (!listingFinished) {
            ...
            if (fee > 0) {
                super._transfer(sender_, _getTransferFeeAddress(), fee);
            }
            ...
            if (transferFee > 0) {
                super._transfer(sender_, _getTransferFeeAddress(), transferFee);
            }
            if (transferA > 0) {
                super._transfer(sender_, recipient_, transferA);
            }
    ...
    function removeBlackList(address address_) external onlyOwner {
        blackListedList[address_] = false;
    }

    function removeBlackLists(address[] calldata listAddress_) external onlyOwner {
        uint256 count = listAddress_.length;
        for (uint256 i = 0; i < count; i++) {
            blackListedList[listAddress_[i]] = false;
        }
    }
