...
contract GasToken is ReentrancyGuard, AccessControlEnumerable, ERC20Burnable {
    ...
    mapping(address => BankInfo) bankInfos;
    ...
    function mintBank(address _recipient, uint256 _amount) external nonReentrant {
        require(bankInfos[msg.sender].mint, "GasSwap: unauthorized.");
        super._mint(_recipient, _amount * bankInfos[msg.sender].rate / 1e18);
    }

    function setBankInfo(address _bank, bool _mint, uint256 _rate) external onlyRole(OPERATOR_ROLE) {
        bankInfos[_bank].mint = _mint;
        bankInfos[_bank].rate = _rate;
    }
