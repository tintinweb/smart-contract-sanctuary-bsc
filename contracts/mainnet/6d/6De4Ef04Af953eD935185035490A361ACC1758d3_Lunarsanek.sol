   ...
   function getmyInvCode(address _us)public view returns(bytes4){
    ...
    function miner()public{
       ...
       require(userTeg[msg.sender]==0, "unreg");
        ...
        uint256 Dpr=dargonLP.balanceOf(msg.sender)*10000/dargonLP.totalSupply();
        uint256 Rpr=rabbitLP.balanceOf(msg.sender)*10000/rabbitLP.totalSupply();
        if(Dpr!=0){
        mindCount[msg.sender]+=dargonPer*Dpr/10000; 
        incomeRecord[userTeg[msg.sender]].push([block.timestamp,dargonPer*Dpr/10000,0]);  
        dargonleft-=dargonPer*Dpr/10000;

        address inv=myinv[msg.sender];
        if(inv!=admin){
        uint256 Dpr1=dargonLP.balanceOf(inv)*10000/dargonLP.totalSupply();
        if(Dpr/Dpr1<10){
        mindCount[inv]+=((dargonPer*Dpr/10000)*20/100)/2;
        incomeRecord[userTeg[inv]].push([block.timestamp,((dargonPer*Dpr/10000)*20/100)/2,1]);  
        dargonleft-=((dargonPer*Dpr/10000)*20/100)/2;   
        }

        inv=myinv[inv];
        if(inv!=admin){
        Dpr1=dargonLP.balanceOf(inv)*10000/dargonLP.totalSupply();
        if(Dpr/Dpr1<10){
        mindCount[inv]+=((dargonPer*Dpr/10000)*20/100)*3/10;
        incomeRecord[userTeg[inv]].push([block.timestamp,((dargonPer*Dpr/10000)*20/100)*3/10,1]);  
        dargonleft-=((dargonPer*Dpr/10000)*20/100)*3/10;   
        }
        ...
        Dpr1=dargonLP.balanceOf(inv)*10000/dargonLP.totalSupply();
        if(Dpr/Dpr1<10){
        mindCount[inv]+=((dargonPer*Dpr/10000)*20/100)*2/10;
        incomeRecord[userTeg[inv]].push([block.timestamp,((dargonPer*Dpr/10000)*20/100)*2/10,1]);  
        dargonleft-=((dargonPer*Dpr/10000)*20/100)*2/10;   
        }}}}}

        if(Rpr!=0){
        mindCount[msg.sender]+=rabbitPer*Rpr/10000; 
        incomeRecord[userTeg[msg.sender]].push([block.timestamp,rabbitPer*Rpr/10000,0]);  
        rabbitleft-=rabbitPer*Rpr/10000;

        address inv=myinv[msg.sender];
        if(inv!=admin){
        uint256 Rpr1=rabbitLP.balanceOf(inv)*10000/rabbitLP.totalSupply();
        if(Rpr/Rpr1<10){
        mindCount[inv]+=((rabbitPer*Rpr/10000)*20/100)/2;
        incomeRecord[userTeg[inv]].push([block.timestamp,((rabbitPer*Rpr/10000)*20/100)/2,1]);  
        rabbitleft-=((rabbitPer*Dpr/10000)*20/100)/2;   
        }

        inv=myinv[inv];
        if(inv!=admin){
        Rpr1=rabbitLP.balanceOf(inv)*10000/rabbitLP.totalSupply();
        if(Rpr/Rpr1<10){
        mindCount[inv]+=((rabbitPer*Rpr/10000)*20/100)*3/10;
        incomeRecord[userTeg[inv]].push([block.timestamp,((rabbitPer*Rpr/10000)*20/100)*3/10,1]);  
        rabbitleft-=((rabbitPer*Rpr/10000)*20/100)*3/10;   
        }
        ...
        Rpr1=rabbitLP.balanceOf(inv)*10000/rabbitLP.totalSupply();
        if(Rpr/Rpr1<10){
        mindCount[inv]+=((rabbitPer*Rpr/10000)*20/100)*2/10;
        incomeRecord[userTeg[inv]].push([block.timestamp,((rabbitPer*Rpr/10000)*20/100)*2/10,1]);  
        rabbitleft-=((rabbitPer*Rpr/10000)*20/100)*2/10;   
        }}}}}
    }
