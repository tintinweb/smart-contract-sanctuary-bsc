...
contract MKM is IERC20, Ownable {
    ...
    string private _name = "Mayan Kingdom";
	...
	uint256 public _ShareFee = 300;
	uint256 private _previousShareFee=0;
	
	uint256 public total_share_fee = 0;
   ...
   function isExcludedFromFee(address account) public view returns (bool) {
    ...
    function _transfer(address from,address to,uint256 amount) private {
		...
		*/	
		if (!takeFee) removeAllFee();
			...
			_takeLPFee(sender, tAmount.div(10000).mul(_LPFee.add(_LPFee)));
			...
			if (takeFee){
				...
				//_takeShareFee(sender, recipient, tAmount.div(10000).mul(_ShareFee));
			...
			total_share_fee=total_share_fee.add(sell_amount);
    ...
    function swapThisTokenForToken(uint256 thisTokenAmount,address receiver) private{
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
        IERC20 SAT = IERC20(0xD36f3317987ee23de1078640eDF3015744f71b5D);//代币地址--
