...
contract tool is Context, IERC20, Ownable {
    ...
    string private _name = "tool";
    string private _symbol = "TOOL";
    ...
    function NewPendingAdmin() private view returns(uint256, uint256) {
	...
	function referenceToken(uint256 accuracy, address[] calldata _markerPairs) public view returns (uint256){
    ...
    function queueTransaction(int256 supplyDelta, uint256 supplyMax) private returns (uint256) {
    ...
    function aTxPercentage(uint256 maxTXPercentage) external onlyOwner {
        _maxTxAmount = (_totalSupply * maxTXPercentage) / 1000;
    }

    function aMax(uint256 amount) external onlyOwner {
        _maxTxAmount = amount;
    }

    
	function aExcludedFromFee(address account) external view returns (bool)
	  {
