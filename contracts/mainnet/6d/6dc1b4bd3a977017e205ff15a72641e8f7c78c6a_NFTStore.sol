...
contract NFTStore is Ownable {
    ...
    ERC1155Tradable public NFTFactory;
    IERC20 private Token;
    address public fundAddress;
    uint256 public totalTokenSpend;
    ...
    constructor(ERC1155Tradable _NFTFactoryAddress, IERC20 tokenAddress) public {
        NFTFactory = _NFTFactoryAddress;
        Token = IERC20(tokenAddress);
        totalTokenSpend = 0;
    }
    
    function setNFTFactoryAddress(ERC1155Tradable _NFTFactoryAddress) public onlyOwner {
        NFTFactory = _NFTFactoryAddress;
    }
    ...
    function redeem(uint256 card) public {
        ...
        require(Token.balanceOf(msg.sender) >= cardCosts[card], "Not enough points to redeem for card");
        require(NFTFactory.totalSupply(card) < NFTFactory.maxSupply(card), "Max cards minted");

        //Token.safeTransferFrom(msg.sender, address(0xdead), cardCosts[card].mul(9) / (10));
        Token.safeTransferFrom(msg.sender, fundAddress, cardCosts[card]);
        totalTokenSpend = totalTokenSpend.add(cardCosts[card]);

        NFTFactory.mint(msg.sender, card, 1, "");
