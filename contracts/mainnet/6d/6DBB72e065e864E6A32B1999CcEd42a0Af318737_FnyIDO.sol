...
contract FnyIDO is Ownable {
    ...
    function purchaseCLAM(uint256 _amountMAI) external returns (bool) {
        ...
        require(_purchaseAmount >= Math.min(20 * 1e9, totalAmount), 'Amount invalid');
    ...
    function finalize(address _receipt) external onlyOwner {
        ...
        uint256 maiInTreasure = 20000 * 1e18;
        IERC20(MAI).approve(treasury, maiInTreasure);

        uint256 clamMinted = ITreasury(treasury).deposit(maiInTreasure, MAI, 0);
        require(clamMinted == 20000 * 1e9);

        IERC20(MAI).transfer(maiClamLP, 30000 * 1e18);
        IERC20(CLAM).transfer(maiClamLP, 6000 * 1e9);
