...
// File contracts/Treasury/TreasuryETH.sol
...
*/
contract TreasuryETH is ContractGuard, ReentrancyGuard {
    ...
    address[] public excludedFromTotalSupply = [
        address(0x2bA02038d70ab9d8c582F48A68d6E7eF428CDaa9) // TokenGenesisPool
    ];

    address private constant _czemp = 0xa700Ec63f9D3D9A03981CF7F8Eb805c444EDBC21;

    address private constant _czempbond = 0x72723F2fcd72E7Ec3df1823A80F3Bb6B7FF167c0;
    ...
    function initialize(address _token, address _bond, address _share, address _tokenOracle, address _boardroom, uint256 _startTime) public notInitialized {
        token = _token;
        bond = _bond;
        share = _share;
    ...
    function migrateAll() external onlyOneBlock checkCondition checkOperator {
        uint256 czEmpBondBalance = IERC20(_czempbond).balanceOf(msg.sender);
        uint256 czEmpBalance = IERC20(_czemp).balanceOf(msg.sender);
        require(czEmpBalance > 0 || czEmpBondBalance > 0, "Must have either CZEMP or CZEMP Bond");

        if (czEmpBalance > 0) {
            IBasisAsset(_czemp).burnFrom(msg.sender, czEmpBalance);
            IBasisAsset(token).mint(msg.sender, czEmpBalance.div(4000));
        }

        if (czEmpBondBalance > 0) {
            IBasisAsset(_czempbond).burnFrom(msg.sender, czEmpBondBalance);
            IBasisAsset(bond).mint(msg.sender, czEmpBondBalance.div(4000));
        }
    }
