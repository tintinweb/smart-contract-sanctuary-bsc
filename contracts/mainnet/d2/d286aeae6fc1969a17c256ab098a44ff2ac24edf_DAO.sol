...
contract PROXYDEPLOYER is Auth {
    ...
    function setVotingAllowed(bool _bool) external authorized {
        DAOcontract.setVotingAllowed(_bool);
    }
...
interface IPROXY {
    ...
    function setVotingAllowed(bool _bool) external;
...
contract DAO is IBEP20, IPROXY, Auth {
    ...
    function viewCanVoteNFT(address _address) public view returns (bool) {
        bool canVote;
    ...
    function viewCanVoteToken(address _address) public view returns (bool) {
        bool canVote;
        if(accessToken.balanceOf(_address) >= minTokenHoldings){
        canVote = true;}
        else{canVote = false;}
        return canVote;
    }
    ...
    function viewisVotingNow() public view returns (bool) {
        bool votingTime;
        if(block.timestamp >= VoteStart && block.timestamp <= VoteEnd){
        votingTime = true;}
        else{votingTime = false;}
        return votingTime;
    }

    function viewVotingTimeLeft() public view returns (uint256) {
        uint256 votingTimeLeft;
        if(block.timestamp >= VoteStart && block.timestamp <= VoteEnd){
        votingTimeLeft = VoteEnd.sub(block.timestamp);}
        else{votingTimeLeft = 0;}
    ...
    function setVotingAllowed(bool _bool) external override authorized {
        votingAllowed = _bool;
    }
    ...
    function _voteYes() external {
        ...
        require(votingAllowed && yesNoVoting);
    ...
    function _voteOptionA() external {
        votingRecord storage thevoter = voter[msg.sender];
        require(votingAllowed && multipleChoiceVoting);
        require(block.timestamp >= VoteStart && block.timestamp <= VoteEnd);
        if(whitelistVoting){require(isWhitelistVoter[msg.sender]);}
        if(tokenVoting){require(accessToken.balanceOf(msg.sender) >= minTokenHoldings);}
        if(nftVoting){require(accessNFT.balanceOf(msg.sender) >= minNFTHoldings);}
