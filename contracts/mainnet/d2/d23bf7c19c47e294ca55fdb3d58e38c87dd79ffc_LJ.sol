...
interface operation {//konwnsec//IDividends 接口
    ...
    function updatecommunity(uint256 id, uint256 OutGold,uint256 communitySEOSQuantity, bool isjf) external;
...
// IDO参与 一个地址只能参与一次   创世节点  20%    超级节点15%
    function pIDO(address Destination) external {
        ...
        _USDTAddr.transferFrom(address(msg.sender), address(_operation), BuyPrice);
        if(id > 0){
            ...
            if(sjtemp[5]> 0){
                ...
                if(sjtemp[6] > 0){
                    ...
                    _USDTAddr.transfer(Uaddress, BuyPrice.sub(BuyPrice.mul(15).div(100)));
                }else{
...
// 提取产出（挖矿）
   function TX() public {
    ...
    // 提取产出（级别）
   function community( bool isjf) public {
        operation diviIns = operation(_operation);
        uint256 id = diviIns.getIDByAddress(msg.sender);
        require(id > 0, "isplayer"); 
        ...
        uint256 OutGold = 0;
        uint256 communitySEOSQuantity = 0;
        ...
        if(PlayercommunitySEOSQuantity > 0 &&  PlayerOutGold > 0&& !isjf){

            uint256 EOSSPrice  =   Spire_Price(_EOSAddr, _EOSLPAddr);
            uint256 EOSSnum = PlayercommunitySEOSQuantity.mul(EOSSPrice).div(10000000);

            if(PlayerOutGold >= PlayercommunitySEOSQuantity){
                _EOSAddr.transfer(msg.sender, PlayercommunitySEOSQuantity);
                OutGold  = OutGold.sub(PlayercommunitySEOSQuantity);
                 communitySEOSQuantity  = 0;
            }else{
                uint256 Unum =PlayerOutGold.mul(10000000).div(EOSSPrice);
                EOSSnum = PlayerOutGold.mul(EOSSPrice).div(10000000);

                _EOSAddr.transfer(msg.sender, EOSSnum);
                 communitySEOSQuantity  = PlayercommunitySEOSQuantity.sub(Unum);
                OutGold  = 0;
            }
        }else
        {
            if( PlayerOutGold > 0&& temp[8]> 0){
                uint256 EOSSPrice  =   Spire_Price(_SEOSAddr, _SEOSLPAddr);
                uint256 Unum = temp[9] .mul(10000000).div(EOSSPrice);
                if(PlayerOutGold >= Unum){
                    _SEOSAddr.transfer(msg.sender, temp[9]);
                    OutGold  = PlayerOutGold.sub(Unum);
                    communitySEOSQuantity  = 0;
                }else{
                    uint256 EOSSnum = temp[9].mul(10000000).div(EOSSPrice);
                    _SEOSAddr.transfer(msg.sender, EOSSnum);
                    communitySEOSQuantity  = temp[9].sub(EOSSnum);
                    OutGold  = 0;
                }
            }
        }

        diviIns.updatecommunity(id,OutGold,communitySEOSQuantity, isjf);
