...
contract WebooFarmingV2 is Auth, ERC20 {
    ...
    modifier onlyToken() {
        require(_msgSender()==tokenA,"WebooFarming: OnlyToken");
        _;
    }
    ...
    constructor(
    ...
    )  Auth(msg.sender) ERC20("Weboo Farming PCs", "WebooBNB-FP") {
        tokenA = _tokenA;
        tokenB = _tokenB;
        ...
        wbnbAddress = IWebooFarmingRouter(_routerFarmingAddress).wbnbAddress();
    ...
    function calculateShare() public {
        IWebooRewardV2(rewardAddress).claimFarmingReward(address(this));
    ...
    function setRewardShare() public {
        IWebooRewardV2(rewardAddress).setShare(address(this),ERC20(tokenA).balanceOf(pairAddress));
    }
    ...
    function getAmountForFarm(address holder,uint256 amount) internal view returns(uint256){
        ...
        require(currentBalance > minimumWebooLeftOnHolderWallet,"WebooFarming: Insufficient Balance For Farming");
    ...
    function removeFarming(uint256 amountRemove) external {
        require(!isDisableFromUnstake[_msgSender()],"WebooFarming: This Address Is Locked Liquidity");
        require(balanceOf(_msgSender()) >= amountRemove,"WebooFarming: Insufficient Amount");
        // Exclude From Fee for Pair
        IWebooToken(tokenA).setIsExcludeFromFee(pairAddress,true);
        ...
        IWebooToken(tokenA).setIsExcludeFromFee(pairAddress,false);
    ...
    function setLockedLiquidity(address pairAddress, address lockedAddress, address recipient) external onlyOwner {
        ...
        require(balance > 0,"WebooFarming: Insufficient Amount");
    ...
    function removeLockedLiquidity(address pairAddress) external onlyOwner {
        require(balanceOf(lockedReflectTo[pairAddress]) > 0,"WebooFarming: No Locked Found for This pair");
    ...
    function setPercentClaimToOtherToken(uint256 percent) external onlyOwner {
        require(percent >= 100 && percent <= 5000, "WebooFarming: min 1% and max 50%");
...
import "./IWebooReward.sol";

interface IWebooRewardV2 is IWebooReward {
