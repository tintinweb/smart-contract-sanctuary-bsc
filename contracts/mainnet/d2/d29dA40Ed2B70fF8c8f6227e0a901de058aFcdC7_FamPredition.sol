...
contract FamPredition is Ownable, VRFConsumerBaseV2, KeeperCompatibleInterface {
    ...
    function betBatch(
        ...
        for (uint256 i; i < positions_.length; ++i) {
            ...
            uint256 betNumber = infos_[i][0];
            if (positions_[i] == BetType.FIVE) {
                require(betNumber >= 111 && betNumber <= 144, "mode error");
            } else {
                require(betNumber >= 1000 && betNumber <= 1999, "mode error");
            }
            userBets[epoch_][msg.sender].push(BetInfo({ position: positions_[i], betNumber: betNumber, betRate: infos_[i][1], baseFee: baseFee_ }));
    ...
    function findUserClub(address current) public view returns (address) {
        address last;
        address inviter;
        while (current != address(0)) {
            inviter = addr.refer.getReferrer(current);
            if (inviter == last) {
                break;
            }
            if (userInfo[inviter].club != address(0)) {
                return userInfo[inviter].club;
            }
            last = current;
            current = inviter;
        }
