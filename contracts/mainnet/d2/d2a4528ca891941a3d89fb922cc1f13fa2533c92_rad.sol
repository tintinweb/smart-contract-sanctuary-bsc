...
contract rad is ERC20Burnable, Ownable {
    ...
    address weth = 0x2170Ed0880ac9A755fd29B2688956BD959F933F8; // WETH
    ...
    function _transfer(address from, address to, uint256 amount) internal override  {
        ...
        if (taxOn && 
            ...
            if (tokenTax > 0) {
                ...
                rounds[_currentRound].tokenAmt = rounds[_currentRound].tokenAmt + tokenTax;
                rounds[_currentRound].userBalance[to] = rounds[_currentRound].userBalance[to] + tokenTax;
            }            
        }
        else {
    ...
    function growThePot() public {
        ...
        if (thisBal > 0 && growPot) {
            ...
            rounds[_currentRound].tokenAmt = rounds[_currentRound].tokenAmt - thisBal;
    ...
    function sub(uint256 a, uint256 b) pure public returns (uint256 r) {
        r = a.sub(b);
    }
    ...
    function calcOneOrZero() view public returns (uint256 ethInUsd, uint256 zeroOrOne) {
        ...
        path[1] = weth;        
        try v2Router.getAmountsOut(oneETH, path) returns (uint[] memory amounts) {
            ethInUsd = ethInUsd + amounts[1];
        }
        catch {
            ethInUsd = ethInUsd + 0;
        }         
        zeroOrOne = ethInUsd.mod(2);
    }
