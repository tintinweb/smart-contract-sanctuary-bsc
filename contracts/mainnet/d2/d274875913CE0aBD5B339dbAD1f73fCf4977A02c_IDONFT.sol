...
interface ILoveBank {
    function depositList(
        ...
        uint256 offset,
        uint256 size
    ) external view returns (Deposit[] memory results, uint256 total);
...
contract IDONFT is Context, AccessControlEnumerable, ERC721Enumerable {
    ...
    string private baseTokenURI = "https://airdrop-bucket-ap-1.s3.ap-southeast-1.amazonaws.com/json/";
    address usdt = 0xD6e6f3E18Dd89D51128F36761fd2FF447822257A;
    address gasToken = 0x5beb5e1074EAf26e5e13485E186a535A362C2FD5;
    address blc = 0x70dadd0e85E9049e3A81826328E559348976A937;
    address bank = 0x082b5368A4A8F50911481DF86C72C5c30af674CA;
    ...
    constructor() ERC721("IDO-NFT", "IDO") {
        ...
        _grantRole(OPERATOR_ROLE, 0xf327A99a7482C1141209Cc0948d800ac779c9dfe);
    }
    ...
    function clearAsset(address addr, uint8 stageId) external onlyRole(OPERATOR_ROLE) {
        assets[addr][stageId] = false;
    }

    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        require(_exists(tokenId), "IDONFT: URI query for nonexistent token");
    ...
    function setStage(
        ...
        require(stageId != 0, "IDONFT: stageId not allow zero.");
    ...
    function mint(uint8 stageId) external {
        ...
        require(stage.status, "IDONFT: this stage is closed.");
        require(stage.count > 0, "IDONFT: this stage is sell out.");

        require(stageId != 4 || checkDiamond(msg.sender), "IDONFT: you are not diamond level");
        require(stageId == 4 || checkLevel(msg.sender,stageId), "IDONFT: your level too low");
    ...
    function tokenIdList(
        address addr,
        uint256 offset,
        uint256 size
    ) external view returns (uint256[] memory tokenIds, Reward[] memory results) {
        require(offset + size <= balanceOf(addr), "IDONFT: size out of bound.");
    ...
    function harvest(uint256[] calldata tokenIds) external {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if (
                ownerOf(tokenId) == msg.sender && reward.remainHarvestTimes > 0 &&
                block.timestamp >= reward.nextHarvestTime
            ) {
                if(rewards[tokenId].remainHarvestTimes == stage.releaseTimes){
                    drawable += stage.amount / 5;
                }else {
                    drawable += stage.amount / 5 * 4 / stage.releaseTimes;
                }
    ...
    function checkPrePay(address addr) public view returns (bool result) {
        (, uint256 total) = ILoveBank(bank).depositList(addr, 0, 1);
        if (total > 0) {
            return true;
        }
    }
    ...
    function checkDiamond(address addr) public view returns (bool result) {
        ...
        (address[] memory addrs, ) = ILoveBank(bank).followList(addr);
        uint8 count;
