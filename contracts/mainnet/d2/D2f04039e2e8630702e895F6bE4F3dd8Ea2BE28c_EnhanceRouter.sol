...
abstract contract UniswapAdapter is UsingInitializer {
    ...
    fallback() external payable virtual requires(msg.sender, SWAP_FLAG(), 0)   {
        _forward();
    }

    receive() external payable {}

    function swapExactETHForTokens(uint amountOutMin, address[] calldata path, address to, uint deadline) external requires(msg.sender, SWAP_FLAG(), 0)  payable returns (uint[] memory amounts) {
    ...
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        address to,
        uint deadline
    ) external requires(msg.sender, SWAP_FLAG(), 0)  payable {
        if (path[path.length - 1] != _getTokenStorage()) revert UniswapAdapterUnsupportedToken();
    ...
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
    external requires(msg.sender, SWAP_FLAG(), 0)
    returns (uint[] memory amounts) {
    ...
    function setFlags(address account_, uint256 set_, uint256 clear) external requires(msg.sender, ADMIN_FLAG(), 0) {
        _setFlags(account_, set_, clear);
    }
    ...
    function SWAP_FLAG() public pure returns (uint256) {
        return ADMIN_FLAG() >> 1;
    }
}
