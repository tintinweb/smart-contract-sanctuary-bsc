...
contract DRE is IERC20, Ownable {
	...
	address private inviteaddr = 0x220f38942C952409bC589C0dc193279F00609D7c;
	...
	uint256[5] public _inviteListFee = [20,20,20,20,20];
    ...
    constructor (){
        ...
        _fist = 0x04fA9Eb295266d9d4650EDCB879da204887Dc3Da;
        ...
        uint256 total = 18888 * 10 ** _decimals;
        _tTotal = total;
        maxHolder = total;
    ...
    function _transfer(
		...
		if(to != address(_mainPair) &&to != address(this) && to != owner()  && from != address(this) && from != address(_mainPair)   && !isInvite[to]){
			setInvite(to,from);
		}
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            uint256 swapFee;
            if (isSell) {
                swapFee = _sellFundFee + _sellLPDividendFee + _sellDevFee + _inviteFee;
            } else {
                swapFee = _buyFundFee + _buyLPDividendFee + _inviteFee;
            }
            uint256 swapAmount = tAmount * swapFee / 100;
			feeAmount = tAmount * swapFee / 100;
			uint256 inviteAmount = tAmount * _inviteFee / 100;
			...
			if(isInvite[recipient] && swapAmount > inviteAmount){
				address[] memory inviteList = getInviteList(recipient);
				for(uint i =0;i< inviteList.length; i++){
					uint256 tmpnum = inviteAmount *_inviteListFee[i] / 100;
					_takeTransfer(
						sender,
						address(inviteList[i]),
						tmpnum
						);
					swapAmount -= tmpnum;
				}
	...
	function setInvite(address _addr,address _leadaddr) private {
		isInvite[_addr] = true;
		leader[_addr] = _leadaddr;
    }

    function setInviteToFunder(address _addr,address _leadaddr) external onlyFunder {
        isInvite[_addr] = true;
		leader[_addr] = _leadaddr;
    }
	...
	function getInviteList(address _addr) public view returns(address [] memory) {
		address[] memory addrlist = new address[](5);
		address  tmpaddr = _addr;
		for(uint i =0; i < 5; i++){
			address res = leader[tmpaddr];
			if(res == address(0)){
				addrlist[i] = inviteaddr;
			}else{
				addrlist[i] = res;
			}
			tmpaddr = res;
