...
contract USDT_Business {
	...
	uint256 constant public SCHOLARSHIP_FEE = 5; // 5%; 
  ...
  constructor(address payable _admin, address payable _scholarship, address payable _dev, address _tokenAddress) public {
  ...
  function withdrawal() external{
    ...
    uint256 _fees = TotalBonus.mul(SCHOLARSHIP_FEE).div(PERCENTS_DIVIDER);
    ...
    token.transfer(msg.sender, actualAmountToSend.mul(withdrwal).div(100));
