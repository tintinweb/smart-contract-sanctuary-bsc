...
contract TdogePassCard is ERC721, Ownable, ReentrancyGuard {
    ...
    uint256 busd_price_pernft = 100 ether;
    uint256 tdoge_price_pernft = 2000000 ether;

    uint256 MAXCOUNT = 1200;

    // 'https://cloudflare-ipfs.com/ipfs/QmZHXaSV7eRu23bGj1Td13mkyZ7YXwqkYsEdwZnM1injDf'

    constructor(address busd_address, address tdoge_address) ERC721('TdogePassCard', 'TdogePassCard') {
        BUSD = IERC20(busd_address);
        TDOGE = IERC20(tdoge_address);
    }

    // function set_wallet_address(address wallet) external onlyOwner {
    //     withdraw_wallet = wallet;
    // }

    function set_token_address(address busd_address, address tdoge_address) external onlyOwner {
        BUSD = IERC20(busd_address);
        TDOGE = IERC20(tdoge_address);
    }
    ...
    // airdrop
    function airdrop(address wallet) external onlyOwner {
         _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
        
        require(MAXCOUNT >= newItemId, 'Not enough TDOGE token to mint');
    ...
    function mint() public payable {
        uint256 busd_balance = BUSD.balanceOf(msg.sender);
        uint256 tdoge_balance = TDOGE.balanceOf(msg.sender);

        _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
        
        require(busd_balance >= busd_price_pernft, 'Not enough BUSD in wallet');
        require(tdoge_balance >= tdoge_price_pernft, 'Not enough TDOGE in wallet');
        require(MAXCOUNT >= newItemId, 'Not enough TDOGE token to mint');

        

        BUSD.transferFrom(msg.sender, address(this), busd_price_pernft);
        TDOGE.transferFrom(msg.sender, address(this), tdoge_price_pernft);
        
        _mint(msg.sender, newItemId);
    }


    function withdraw_tokens(address wallet) external onlyOwner {
        require(wallet != address(0), 'Wallet should not be 0');
        BUSD.transfer(wallet,BUSD.balanceOf(address(this)));
        TDOGE.transfer(wallet,TDOGE.balanceOf(address(this)));
    }

    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        tokenId;
        return "https://cloudflare-ipfs.com/ipfs/QmZHXaSV7eRu23bGj1Td13mkyZ7YXwqkYsEdwZnM1injDf";
    }
