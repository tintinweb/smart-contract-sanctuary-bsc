...
contract AKUSD is ERC20Burnable, Operator {
    ...
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("AKUSD", "AKUSD") {
    ...
    function _getTokenPrice() internal view returns (uint256 _akusdPrice) {
    ...
    function _updateTaxRate(uint256 _akusdPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_akusdPrice >= taxTiersTwaps[tierId]) {
    ...
    function setTokenOracle(address _akusdOracle) public onlyOperatorOrTaxOffice {
    ...
    function distributeReward(
        ...
        require(_akusdPool != address(0), "!_tokenPool");
        require(_airdropWallet != address(0), "!_pegRegulation");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_akusdPool, INITIAL_DEX_POOL_DISTRIBUTION);
