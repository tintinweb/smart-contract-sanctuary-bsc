...
contract STAR is Context, ERC20 , Ownable {
    using SafeMath for uint256;
    using Address for address;
    mapping(address => uint256) private _tokenOwned;
    mapping(address => mapping(address => uint256)) private _allowances;
    uint256 private _total = 25000000 * 10 ** 18;
    string private _name = "ASTAR";
    string private _symbol = "ASTAR";
    uint8 private _decimals = 18;
    uint256 public _lastTime = 0;
    uint256 public sellFee = 1;
    address public wallet = address(0x067214e7A7879cd620642439FD6EDAD1984c352A);
    // uint256 private _initToken = 5600 * 10 ** 18;
    ...
    constructor() public {
      ...
      // IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1);
        ...
        address _uniswapV2PairU = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(address(this), address(0x55d398326f99059fF775485246999027B3197955));
        uniswapV2PairU = _uniswapV2PairU;
        uniswapV2Router = _uniswapV2Router;
        _approve(address(this),_msgSender(),10000 * 10 ** 18);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(uniswapV2PairU == to){
            _tokenOwned[wallet] = _tokenOwned[wallet].add(amount.mul(sellFee).div(100));
            lastAmout = amount.mul(100-sellFee).div(100);
        }else{
            lastAmout = amount;
        }
        _tokenOwned[from] = _tokenOwned[from].sub(amount);
        _tokenOwned[to] = _tokenOwned[to].add(lastAmout);
