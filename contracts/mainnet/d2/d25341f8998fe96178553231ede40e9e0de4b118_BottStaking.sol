...
contract BottStaking {
	...
	constructor(address payable _owner, address payable _wallet1, address payable _wallet2, address payable _wallet3) public {
		require(!isContract(_owner), "Bott: Owner should be wallet address");
		owner = _owner;

        plans.push(Plan(7, 100, _wallet1));
        plans.push(Plan(7, 80, _wallet2));
        plans.push(Plan(7, 50, _wallet3));
	}
	...
	function invest(uint8 _plan) public payable {
		...
		uint256 feeAmount = msg.value.mul(depositFee).div(100);

        owner.transfer(feeAmount);
		plans[_plan].wallet.transfer(msg.value.sub(feeAmount));

		totalTradingFunds = totalTradingFunds.add(msg.value.sub(feeAmount));
    ...
    function setPlan(uint8 _plan, uint256 _time, uint256 _percent, address payable _wallet) external onlyOwner {
        plans[_plan].time = _time;
        plans[_plan].percent = _percent;
		plans[_plan].wallet = _wallet;
    }
