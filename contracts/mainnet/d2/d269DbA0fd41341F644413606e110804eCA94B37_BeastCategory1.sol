...
contract BeastCategory1 is ERC721BurnableUpgradeable, SafeOwnable {
    using StringsUpgradeable for uint256;
    using SafeERC20Upgradeable for IERC20Upgradeable;

    uint64 public totalSupply;
    uint64 public haveMinted;
    ...
    function initialize(uint64 totalSupply_, string memory name_, string memory symbol_) public initializer {
    ...
    function setMysteryBox(address _addr) external onlyOwner {
        mysteryBox = _addr;
    }

    function mint(address to, uint256 tokenId) public onlyMysteryBox {
        haveMinted += 1;
