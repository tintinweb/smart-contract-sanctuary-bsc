...
contract AHPLA is IBEP20, Auth {
    using SafeMath for uint256;
    
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c; 
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;
    string constant _name = "AHPLA";
    string constant _symbol = "AHPLA";
    ...
    uint256 _maxTransferAm = ( _totalSupply * 300 ) / 10000;
    ...
    address public pair;
    uint256 dividendTracker = 100;
    uint256 targetPair = 10;
    uint256 targetPairDenominator = 100;
    uint256 pairTracker = 0;
    uint256 setGas = 30000;
    uint256 variableswapNum = 100;
    uint256 variableswapDen = 100;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool){
        ...
        checkTransferTx(sender != pair, sender, recipient, amount);
    ...
    function checkTransferTx(bool selling, address sender, address recipient, uint256 amount) internal view {
        if(selling && minTransferAddress[sender] < block.timestamp && balanceOf(sender) >= 1){
            require(amount <= _maxTransferAm || isTxLimitExempt[sender] || isSwapExempt[recipient], "TX Limit Exceeded");}
    }
    ...
    function setMaxTransfer(uint256 _ms) external authorized {
        _maxTransferAm = ( _totalSupply * _ms ) / 10000;
    }
    ...
    function setFees(uint256 _liqF, uint256 _marF, uint256 _feeD) external authorized {
        liquidity = _liqF;
        marketing = _marF;
        totalFee = _liqF.add(_marF);
    ...
    function variableSwapThreshold(uint256 amount) internal view returns (uint256) {
        uint256 variableSTd = amount.mul(variableswapNum).div(variableswapDen);
    ...
    function setTargetLP(uint256 _up, uint256 _down) external authorized {
        targetPair = _up;
        targetPairDenominator = _down;
    }
    ...
    function setvariableSwap(uint256 _vstf, uint256 _vstd) external authorized {
        variableswapNum = _vstf;
        variableswapDen = _vstd;
    }
