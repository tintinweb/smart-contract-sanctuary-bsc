...
// File: https://github.com/peng1027/smartcontracts/blob/main/WJJToken_reward_0719.sol
...
// File: contracts/staking0817v2.sol
...
contract NFTStaking is Ownable, IERC721Receiver {
    ...
    function refer_son(address referrer) external {
        bool shouldSetInviter = balanceOf(msg.sender) == 0 &&
        inviter[referrer] == address(0x0);
        if (shouldSetInviter) {
            User storage user = users[msg.sender];
            if (user.referrer == address(0x0)) {
                if (referrer != msg.sender) {
                    user.referrer = referrer;
                    // user.levels = user.levels.add(1);
                }
    ...
    function getUserReferrer1(address userAddress)
    internal view
    returns (address){
        return users[userAddress].referrer;
    }
    ...
    function stake(uint256[] calldata tokenIds) external {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            address father = getUserReferrer1(msg.sender);
            rewardmath = 700 ether;
            earned = rewardmath / 10;
            token.mint(father, earned);
        }
    }
    ...
    function _claim(address account, uint256[] calldata tokenIds, bool _unstake) internal {
        ...
        address upline = getUserReferrer1(msg.sender);
        for (uint256 i = 0; i < 3; i++) {
            if (upline != address(0x0)) {
                if (i == 0) {
                    earned_father = earned * 1 / 10;
                }
                if (i == 1) {
                    earned_father = earned * 3 / 10;
                }
                if (i == 2) {
                    earned_father = earned * 7 / 10;
                }
                token.mint(upline, earned_father);

                upline = getUserReferrer1(upline);
