...
contract AlphaOrionis is IERC20, Context, Ownable {
    ...
    uint256 private AO__Max_Buy = 50000000 * 10**9;
    uint256 private AO__Max_Wallet = 50000000  * 10**9;
    uint256 private AO__Threshold = 3000000 * 10**9;
    uint256 private AO__Max_Sell = 10000000 * 10**9;
    uint256 private AO__genesis_block;

    uint8 private constant _decimals = 9;
    uint256 private constant MAX = ~uint256(0);

    address private AO_Marketing = 0xB77066990AfCe419CA94F6DEBBBB2de5A2CB3290;
    address private AO_Team_Wallet = 0xB77066990AfCe419CA94F6DEBBBB2de5A2CB3290;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[AO_Marketing] = true;
        _isExcludedFromFee[AO_Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[AO_Marketing] = true;
        allowedTransfer[AO_Team_Wallet] = true;
    ...
    function setTradingStatus(bool state) external onlyOwner{
        ...
        swapEnabled = state;
        if(state == true && AO__genesis_block == 0) AO__genesis_block = block.number;
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to] && block.number <= AO__genesis_block + 3) {
            require(to != pair, "Sells not allowed for first 3 blocks");
        }
        
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= AO__Max_Buy, "You are exceeding AO__Max_Buy");
            require(balanceOf(to) + amount <= AO__Max_Wallet, "You are exceeding AO__Max_Wallet");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= AO__Max_Sell, "You are exceeding AO__Max_Sell");
            if(to != pair){
                require(balanceOf(to) + amount <= AO__Max_Wallet, "You are exceeding AO__Max_Wallet");
            }
            if(coolDownEnabled){
        ...
        bool canSwap = balanceOf(address(this)) >= AO__Threshold;
        if(!swapping && swapEnabled && canSwap && from != pair && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            if(to == pair)  swapAndLiquify(AO__Threshold, sellTaxes);
            else  swapAndLiquify(AO__Threshold, taxes);
        }
        bool takeFee = true;
        bool isSell = false;
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(AO_Marketing).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(AO_Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function updateThreshold_Amount(uint256 amount) external onlyOwner{
        AO__Threshold = amount * 10**_decimals;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        AO__Max_Buy = maxBuy * 10**decimals();
        AO__Max_Sell = maxSell * 10**decimals();
    }
    
    function update_AO__Max_Wallet(uint256 amount) external onlyOwner{
        AO__Max_Wallet = amount * 10**decimals();
    }
