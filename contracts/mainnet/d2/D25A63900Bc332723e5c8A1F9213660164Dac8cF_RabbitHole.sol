...
interface IRABBITToken {
    ...
    function fragmentToRABBIT(uint256 value) external view returns (uint256);

    function RABBITToFragment(uint256 RABBIT) external view returns (uint256);

    function balanceOfUnderlying(address who) external view returns (uint256);

    function balanceOf(address who) external view returns (uint256);

    function RABBITsScalingFactor() external view returns (uint256);
}
...
contract RabbitHole is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        uint256 RABBITsScalingFactorWhenStake;
        //
    ...
    // TRABBIT
    IRABBITToken public RABBIT;
    // TRABBIT LP address
    IUniswapPair public RABBITLp;
    // TRABBIT tokens reward per block.
    ...
    // The block number when TRABBIT mining starts.
    ...
    constructor(
        IRABBITToken _RABBIT,
        IUniswapPair _RABBITLp,
    ...
    // Update the given pool's TRABBIT allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
    ...
    // View function to see pending RABBIT on frontend.
        ...
        returns (uint256)
    {
        ...
        if (address(pool.lpToken) == address(RABBIT)) {
            lpSupply = RABBIT.balanceOfUnderlying(address(this));
        }
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 RABBITReward = ((block.number - pool.lastRewardBlock) *
                rewardPerBlock *
                pool.allocPoint) / totalAllocPoint;
            accRewardPerShare += (RABBITReward * 1e12) / lpSupply;
        }
        return
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 RABBITReward = ((block.number - pool.lastRewardBlock) *
            rewardPerBlock *
            pool.allocPoint) / totalAllocPoint;
        pool.accRewardPerShare += (RABBITReward * 1e12) / lpSupply;
        pool.lastRewardBlock = block.number;
    ...
    // Deposit tokens to RABBITChef for TRABBIT allocation.
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (address(pool.lpToken) == address(RABBIT)) {
            user.amount = user.amount.mul(user.RABBITsScalingFactorWhenStake).div(10**24); // fragment
            user.amount = RABBIT.fragmentToRABBIT(user.amount);
            _amount = RABBIT.fragmentToRABBIT(_amount);
            user.RABBITsScalingFactorWhenStake = RABBIT.RABBITsScalingFactor();
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        require(user.amount >= _amount, "user amount is less than the want amount");
        ...
        user.rewardDebt = (user.amount * pool.accRewardPerShare) / 1e12;
        if (address(pool.lpToken) == address(RABBIT)) {
            _amount = _amount.mul(user.RABBITsScalingFactorWhenStake).div(10**24);
            if (RABBIT.balanceOf(address(this)) < _amount) {
                RABBIT.mint(address(this), _amount - RABBIT.balanceOf(address(this)));
            }
        }
