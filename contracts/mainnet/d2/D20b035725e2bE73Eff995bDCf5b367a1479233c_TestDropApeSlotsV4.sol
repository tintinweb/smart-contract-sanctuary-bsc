...
// Dependency file: contracts/vrf/CryptoDropApeSlotsV4.sol
...
/// @notice CryptoDrop CryptoDrop ApeSlots V4

contract CryptoDropApeSlotsV4 is VRFElementIdUint256 {
    using SafeERC20 for IERC20;
    using Address for address;
    ...
    bytes1 internal constant A = 0x0a;
    bytes1 internal constant B = 0x0b;
    bytes1 internal constant C = 0x0c;
    bytes1 internal constant D = 0x0d;
    bytes1 internal constant E = 0x0e;
    ...
    struct MODE {
        ...
        uint256 storagePercent;
        uint256 teamPercent;
        uint256 committeePercent;
        uint256 leaderboardPercent;
    }
    ...
    mapping(MODE_TYPE => MODE) public modes;

    mapping(MODE_TYPE => mapping(bytes1 => uint256)) public slots;

    event PayoutBet(uint256 id, uint256 amount, address gamer);
    event ProcessBet(uint256 id, address gamer, uint256 amount, uint256 reward, bytes hash, uint8 mode);
    event SetBetSize(uint8 mode, uint256 size);
    event SetWinSize(uint8 mode, bytes1 symbol, uint256 percent);

    constructor(address _vrfCoordinator, address _linkToken) VRFConsumerBase(_vrfCoordinator, _linkToken) {
        //NORMAL MODE
        modes[MODE_TYPE.NORMAL].bet = 0.01 ether;
        modes[MODE_TYPE.NORMAL].jackpotPercent = 90;
        modes[MODE_TYPE.NORMAL].storagePercent = 4;
        modes[MODE_TYPE.NORMAL].teamPercent = 2;
        modes[MODE_TYPE.NORMAL].committeePercent = 2;
        modes[MODE_TYPE.NORMAL].leaderboardPercent = 2;

        slots[MODE_TYPE.NORMAL][A] = 0.004 ether; //0.4%
        slots[MODE_TYPE.NORMAL][B] = 0.0032 ether; //0.32%
        slots[MODE_TYPE.NORMAL][C] = 0.0024 ether; //0.24%
        slots[MODE_TYPE.NORMAL][D] = 0.0016 ether; //0.16%
        slots[MODE_TYPE.NORMAL][E] = 0.0008 ether; //0.08%

        //DEGEN MODE

        modes[MODE_TYPE.DEGEN].bet = 0.025 ether;
        modes[MODE_TYPE.DEGEN].jackpotPercent = 90;
        modes[MODE_TYPE.DEGEN].storagePercent = 4;
        modes[MODE_TYPE.DEGEN].teamPercent = 2;
        modes[MODE_TYPE.DEGEN].committeePercent = 2;
        modes[MODE_TYPE.DEGEN].leaderboardPercent = 2;

        slots[MODE_TYPE.DEGEN][A] = 0.01 ether; //1%
        slots[MODE_TYPE.DEGEN][B] = 0.008 ether; //0.8%
        slots[MODE_TYPE.DEGEN][C] = 0.006 ether; //0.6%
        slots[MODE_TYPE.DEGEN][D] = 0.004 ether; //0.4%
        slots[MODE_TYPE.DEGEN][E] = 0.002 ether; //0.2%
    }

    /// @notice place bet

    function placeBet(MODE_TYPE _mode, uint256 _vrfFee) external payable virtual nonReentrant whenNotPaused {
        require(!msg.sender.isContract(), "CryptoDrop V4: sender cannot be a contract");
        require(tx.origin == msg.sender, "CryptoDrop V4: msg sender is not original user");

        require(msg.value >= modes[_mode].bet, "CryptoDrop V4: not enough value");

        uint256 betAmount = msg.value - _vrfFee;

        if (betAmount > modes[_mode].bet) {
            //refund extra
            _deliverFunds(_msgSender(), betAmount - modes[_mode].bet, "CryptoDrop V4: failed transfer BNB to address");
        }

        Bet memory bet = Bet("", block.number - 1, betAmount, _vrfFee, 0, "", _mode);

        bets[msg.sender].push(bet);

        uint256 id = bets[msg.sender].length - 1;

        _beforeProcessBet(id, modes[_mode].bet, _vrfFee, _mode);

        _processBet(msg.sender, id);
    }

    function setBetSize(MODE_TYPE _mode, uint256 _size) external onlyOwner {
        modes[_mode].bet = _size;
        emit SetBetSize(uint8(_mode), _size);
    }
    ...
    function setWinSize(
        ...
        slots[_mode][_symbol] = _percent;
        emit SetWinSize(uint8(_mode), _symbol, _percent);
    }
    ...
    function _beforeProcessBet(
        ...
        uint256 jackpotFee = (_amount * modes[_mode].jackpotPercent * PRECISION) / 100 / PRECISION;
        uint256 storageFee = (_amount * modes[_mode].storagePercent * PRECISION) / 100 / PRECISION;
        uint256 teamFee = (_amount * modes[_mode].teamPercent * PRECISION) / 100 / PRECISION;
        uint256 committeeFee = (_amount * modes[_mode].committeePercent * PRECISION) / 100 / PRECISION;
        uint256 leaderboardFee = (_amount * modes[_mode].leaderboardPercent * PRECISION) / 100 / PRECISION;

        // increase jackpot
        jackpotAmount += jackpotFee;

        _deliverFunds(storageAddress, storageFee, "CryptoDrop V4: failed transfer BNB to Staking Storage");

        _deliverFunds(teamAddress, teamFee, "CryptoDrop V4: failed transfer BNB to Team");

        _deliverFunds(committeeAddress, committeeFee, "CryptoDrop V4: failed transfer BNB to Committee");

        _deliverFunds(leaderboardAddress, leaderboardFee, "CryptoDrop V4: failed transfer BNB to Leadboard");
    }
    ...
    function _processBetWithRandomness(RandomnessRequestData storage _data, uint256 _randomness) internal virtual override {
        ...
        if (slots[bet.mode][b[5]] > 0) {
            uint256 prize = (jackpotAmount * slots[bet.mode][b[5]]) / PRECISION;
            ...
            //sent to gamer prize
            _deliverFunds(gamer, prize, "CryptoDrop V4: failed transfer BNB to Gamer");

            emit PayoutBet(betId, prize, gamer);
        }

        emit ProcessBet(betId, gamer, bet.amount, bet.reward, bet.hash, uint8(bet.mode));
    }
}


// Root file: contracts/test/TestDropApeSlotsV4.sol


pragma solidity ^0.8.0;

// import "contracts/vrf/CryptoDropApeSlotsV4.sol";

contract TestDropApeSlotsV4 is CryptoDropApeSlotsV4 {
    constructor(address _vrfCoordinator, address _linkToken) CryptoDropApeSlotsV4(_vrfCoordinator, _linkToken) {}
}