...
contract PWORLD is ERC20, Ownable {
    IUniswapV2Router02 public uniswapV2Router;
    address public  uniswapV2Pair;
    bool private swapping;
    PWORLDDividendTracker public dividendTracker;
    ...
    address payable public _developmentWalletAddress = payable(0xCEbEF49091AcA5B7bFaD5Ebd1dC70fb2AAabF437);
    ...
    constructor() ERC20("PEACEWORLD", "PWORLD") {
    	dividendTracker = new PWORLDDividendTracker();
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "PWORLD: The dividend tracker already has that address");
        PWORLDDividendTracker newDividendTracker = PWORLDDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "PWORLD: The new dividend tracker must be owned by the PEACEWORLD contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "PWORLD: The router already has that address");
    ...
    function setrewardFees(uint256 value) external onlyOwner{
        ...
        require(totalFees <= 25, "PWORLD: Maximum allowed fees are 25%");
    }
    ...
    function setExtraSellFee(uint256 value) external onlyOwner{
        ...
        require(totalSellFees <= 25, "PWORLD: Maximum allowed fees are 25%");
    }
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "PWORLD: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function limitMaxWallet(uint256 maxWalletPercent) external onlyOwner {
         ...
         require(maxWalletPercent >= 1, "PWORLD: Can not limit Max Wallet to less than 1%");
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "PWORLD: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "PWORLD: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "PWORLD: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if (from != owner() &&
            ...
            require(balanceOf(to) + amount <= maxWalletTokens, "PWORLD: Exceeds maximum wallet token amount.");
        }
        if(amount == 0) {
        ...
        if(takeFee) {
            require(tradingOpen,"PWORLD: Trading is not open yet");
...
contract PWORLDDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor() DividendPayingToken("PWORLD", "PWORLD_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 1000 * (10**18); //must hold 1000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "PWORLD_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "PWORLD_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PWORLD contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "PWORLD_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "PWORLD_Dividend_Tracker: Cannot update claimWait to same value");
