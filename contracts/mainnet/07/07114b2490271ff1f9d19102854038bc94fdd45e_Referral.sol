...
contract Referral is IReferral, Ownable {
    ...
    mapping(address => RefDetail) public referrers; // user address => referrer address
    mapping(address => uint16) public refPos;
    ...
    modifier onlyMasterChef() {
        require(msg.sender == address(masterChef) || smartChef[msg.sender] == true, "Only MasterChef can call this function");
        _;
    }
    ...
    function recordReferral(address _user, address _referrer) public override onlyMasterChef {
        if (_user != address(0)
            && _referrer != address(0)
            && _user != _referrer
            && referrers[_user].upline == address(0)
        ) {
            RefDetail storage refDetail;
            refDetail.upline=_referrer;
            refDetail.pos=refPos[_referrer];
            refDetail.bonusLeft=0;
            refDetail.bonusRight=0;
            referrers[_user] = refDetail;
            refPos[_user] = 1;
    ...
    function getReferrer(address _user) public override view returns (address) {
        return referrers[_user].upline;
    }

    function getRefDetail(address _user) public view returns  (RefDetail memory) {
        return referrers[_user];
    }

    function updateRefPos(address _user,uint16 _pos) public onlyMasterChef{
        refPos[_user] = _pos;
    }

    function updateBonus(address _user,uint256 _bonusLeft,uint256 _bonusRight) public onlyMasterChef{
        RefDetail storage ref = referrers[_user];
        ref.bonusLeft = _bonusLeft;
        ref.bonusRight = _bonusRight;
    }

    function getRefPos(address _user) public view returns (uint16) {
        return refPos[_user];
    }
}