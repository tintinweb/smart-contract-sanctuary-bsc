...
contract SquidGame is ReentrancyGuard, Ownable, VRFConsumerBase {
    ...
    event AddedToPrizePool(bytes32 indexed betId, uint256 sqmAmount);
    event RewardClaimed(address indexed player, uint256 score, uint256 sqmAmount);
    ...
    function _resolveNone(bytes32 _betId, Bet memory _bet) private {
        ...
        emit AddedToPrizePool(_betId, _squidBought - _squidToAdd);
    }
    ...
    function _resolveWin(bytes32 _betId, Bet memory _bet) private {
        ...
        uint256 _amountBought = _swapBnbForSquid(_winnerTaxAmount);
        emit AddedToPrizePool(_betId, _amountBought);
    ...
    function distribute() external nonReentrant onlyIfIsOver {
        ...
        for (uint256 i = 0; i < _topWinnersLength; ++i) {
            ...
            emit RewardDistributed(winners[_id].player, winners[_id].reward);
        }
    ...
    function claim() external nonReentrant {
        ...
        emit RewardClaimed(_winner.player, _winner.score, _winner.reward);
    }
