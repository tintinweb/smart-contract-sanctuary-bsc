...
contract SDAO is IERC20, Ownable, Referral {
    using SafeMath for uint256;
    ...
    uint256[5] public sellAmountRate = [1, 2, 3, 4, 5];
    ...
    function _transfer(
        ...
        if (takeFee) {
            if (sellLimit && to == uniswapV2Pair) {
                ...
                if (amount > _tOwned[from] * sellRate / 100) {
                    amount = _tOwned[from] * sellRate / 100;
                }
    ...
    function getSellAmount() public view returns (uint256){
        ...
        if (price > 40 * decimalRate) {
            return sellAmountRate[4];
        } else if (price > 20 * decimalRate) {
            return sellAmountRate[3];
        } else if (price > 10 * decimalRate) {
            return sellAmountRate[2];
        } else if (price > 5 * decimalRate) {
