...
contract Sky is ERC20 {
	...
	address private _fundAddress = address(0xA4990B9413A6Fd11f8E53e53BC3ff53fA7cEF977);
    ...
    constructor(address tokenOwner) ERC20("Sky", "Sky") {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        address _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
        .createPair(address(this), address(_baseToken));
		uint256 total = 39 * 10**21;
        _destroyMaxAmount = total.sub(13 * 10**20);
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx || _destroyMaxAmount <= balanceOf(_destroyAddress)) {
            takeFee = false;
        }else{
	...
	function swapAndLiquify() private {
		...
		uint256 fundswap = fundAmount.sub(fundOverAmount);
		if(allAmount > fundswap.add(10**18) && fundswap > 10**19){
			fundOverAmount = fundOverAmount.add(fundswap);
			swapTokensForFund(fundswap);
		}
		
		allAmount = balanceOf(address(this));
		uint256 ldxswap = ldxAmount.sub(ldxOverAmount);
		if(allAmount > ldxswap.add(10**18) && ldxswap > 10**19){
			ldxOverAmount = ldxOverAmount.add(ldxswap);
			uint256 usdtOldBalance = USDT.balanceOf(address(this));
			swapTokensForOther(ldxswap.div(2));
			uint256 usdtBalance = USDT.balanceOf(address(this));
			if(usdtBalance > usdtOldBalance.add(10**12)){
				addLiquidityUsdt((usdtBalance - usdtOldBalance), ldxswap.div(2));
			}
		}
