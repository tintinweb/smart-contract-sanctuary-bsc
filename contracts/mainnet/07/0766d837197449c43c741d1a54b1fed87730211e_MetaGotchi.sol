...
/*
	Website: http://metagotchi.net/
	Telegram: https://t.me/metagotchiofficial
    Twitter: https://twitter.com/MetaGotchis?t=g1zcSNz27vNyRJDZ13JShA&s=09
	Donate: 0xEcC075cB2926564568F0b7C9a98ac0DC496817D1
	Contract Presale will be in https://www.pinksale.finance/#/.
    Contract Name: BladeStandard
	Contract Version: 1.04
	Contract Supply: 100,000,000 /100 Million
	Contract Tokenomics:
	3% Liquidity.
	2% Marketing.
	10% Development
    ...
    Deployed on Testnet Under: https://testnet.bscscan.com/address/0x3DB20DDCD3039080a49F10adD2D0c8D8343D1Aa0
...
contract MetaGotchi is Context, IERC20, Ownable
{
    ...
    string private _name = "MetaGotchi";
    string private _symbol = "MeGo";
    uint8 private _decimals = 9;
    
    uint256 public _taxFee = 0; //reflection is disable on this token, we leave this as placeholder so the code wont break
	...
	uint256 public totalBuyFee = _devFee.add(_liquidityFee).add(_marketingFee).add(_TeamFee); //15% Total Buy Fee
    
    

    uint256 public _saleDevFee = 5;
    uint256 public _saleLiquidityFee = 3;
    uint256 public _saleMarketingFee = 5;
    uint256 public _saleTeamFee = 2;

    uint256 public totalSaleFee = _saleDevFee.add(_saleLiquidityFee).add(_saleMarketingFee).add(_saleTeamFee); //15% Total Sale Fee
    

    address payable public marketingWallet =  payable(0xDbDBe5535E258bA3DACEFC6cB9837825c464084c);
	address payable public TeamWallet =  payable(0x99ADEf0f55273413E444CeF729f9F70611c55a11);
	address payable public DevWallet =  payable(0xDbDBe5535E258bA3DACEFC6cB9837825c464084c);
	address public PinkSaleAddr =  0x7ee058420e5937496F5a2096f04caA7721cF70cc; //Define PInksale Presale //Prod  0x4650C6f8A56B1bC8Db534BD0Cb5feD505398b89c
    ...
    uint256 public _maxSaleAmount = _tTotal;//1_000_000_0 * 10**9; //1%
    uint256 public _maxBuyAmount = _tTotal;//2_000_000_0 * 10**9; //2%
    uint256 private numTokensSellToAddToLiquidity = 1_000_00 * 10**9;
    uint256 public maxLimit = _tTotal;//2_000_000_0 * 10**9; //max allowed tokens in one wallet. 
    ...
    function setAllBuyFees(uint256 devFee, uint256 liquidityFee, uint256 marketingFee, uint256 TeamFee) public onlyOwner() 
    {
        _devFee = devFee;
        ...
        uint256 totalBuyFees = _devFee.add(_liquidityFee).add(_marketingFee).add(_TeamFee);
        require(totalBuyFees <= 25, "Total fee is over 25%");
	
    }

    function setAllSaleFees(uint256 devFee, uint256 liquidityFee, uint256 marketingFee, uint256 TeamFee) public onlyOwner() 
    {
        _saleDevFee = devFee;
        _saleLiquidityFee = liquidityFee;
        _saleMarketingFee = marketingFee;
        _saleTeamFee = TeamFee;
        uint256 totalSaleFees = _saleDevFee.add(_saleLiquidityFee).add(_saleMarketingFee).add(_saleTeamFee);
    ...
    {  
        return _amount.mul(_liquidityFee.add(_marketingFee).add(_TeamFee).add(_devFee)).div(100);
    }
    ...
    {
        uint256 allFee = _liquidityFee.add(_marketingFee).add(_TeamFee).add(_devFee);
        uint256 halfLiquidityTokens = contractTokenBalance.div(allFee).mul(_liquidityFee).div(2);
        //uint256 halfLiquidityTokens = contractTokenBalance.div(allFee);
        ...
        TeamWallet.transfer(newBalance.div(allFee).mul(_TeamFee));
