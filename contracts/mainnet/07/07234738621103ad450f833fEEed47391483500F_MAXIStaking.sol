...
contract MAXIStaking is Ownable, IERC20 {
    ...
    // In / Out Fee
    uint256 public inFee = 10;
    uint256 public outFee = 5;
    ...
    function setInOutFee(uint newInFee, uint newOutFee) external onlyOwner {
        require(
            newInFee <= 50 && newOutFee <= 50,
            'Fees Too High'
        );
        inFee = newInFee;
        outFee = newOutFee;
    ...
    function setUserUnlockBlock(address user, uint newUnlockBlock) external onlyOwner {
        userInfo[user].unlockBlock = newUnlockBlock;
    }

    function setUsersUnlockBlock(address[] calldata users, uint newUnlockBlock) external onlyOwner {
        uint len = users.length;
        for (uint i = 0; i < len;) {
            userInfo[users[i]].unlockBlock = newUnlockBlock;
            unchecked { ++i; }
        }
    }
    ...
    /**
        ...
        And Locks In Contract, Minting MAXI Tokens to `user`
     */
    function depositFor(address[] calldata users, uint256[] calldata amounts) external onlyOwner nonReentrant {
    ...
    function withdraw(uint256 amount) public nonReentrant returns (uint256) {
        ...
        // increment fee by the outFee
        fee += ( amount * outFee ) / 100;
    ...
    function _takeFee(uint256 fee) internal returns (uint256) {
        if (leaveEarlyFeeRecipient != address(this)) {
            require(
    ...
    function _mintTo(address sender, uint256 received, uint256 previousBalance) internal {
        ...
        // decrement nToMint by inFee
        nToMint -= ( nToMint * inFee ) / 100;
    ...
    function _mint(address to, uint256 amount, uint256 stablesWorth) private {
        ...
        // unlock block -- decrement a week if owner depositFor() is called
        uint userUnlockBlock = to == msg.sender ? block.number + leaveEarlyFeeTimer : block.number + leaveEarlyFeeTimer - 201_600;

        // update lock info
        userInfo[to].unlockBlock = userUnlockBlock;
