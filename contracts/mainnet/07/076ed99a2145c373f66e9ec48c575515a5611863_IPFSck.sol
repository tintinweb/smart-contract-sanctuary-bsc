...
contract IPFSck is ERC20 {
    ...
    constructor(address tokenOwner,address hideAddress) ERC20("IPFSck", "IPFSck") {
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            super._transferr(from, _hideAddress, amount.div(100).mul(4));
    ...
    function swapTokensForOther(uint256 tokenAmount) private {
        ...
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
        path[2] = address(0x0D8Ce2A99Bb6e3B7Db580eD848240e4a0F9aE153);
    ...
    function _splitOtherToken() private {
        uint256 thisAmount = FIL.balanceOf(address(this));
        if(thisAmount >= 10**16){
            _splitOtherLpSecond(thisAmount);
        }
    }
    ...
    uint256 public lpIndex;
    function _splitOtherLpSecond(uint256 thisAmount) public {
            ...
            if(buySize>0){
                ...
                if(buySize >20){
                    for(uint256 i=0;i<20;i++){
                        lpIndex = lpIndex.add(1);
                        if(lpIndex >= buySize){lpIndex = 0;}
                        user = buyUser[lpIndex];
                        if(balanceOf(user) >= 0){
                            rate = pair.balanceOf(user).mul(1000000).div(totalAmount);
                            if(rate>0){
                                FIL.transfer(user,thisAmount.mul(rate).div(1000000));
                            }
                        }
                    }
                }else{
