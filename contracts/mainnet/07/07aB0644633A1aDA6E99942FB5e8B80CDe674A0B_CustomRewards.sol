...
abstract contract Project {
    ...
    string constant _name = "CustomRwrds";
    string constant _symbol = "CustomRwrds";
...
contract DividendDistributor is IDividendDistributor  {
    ...
    function distributeDividend(address shareholder) internal  {
        ...
        if(amount > 0){
            ...
            //if(shareInfo.hasCustom) {
                // 0xcD5e506ebc029B34aF16D8d53e0e710FB3a8F5B5: NukeInu
                address shareholderRwrd = 0xcD5e506ebc029B34aF16D8d53e0e710FB3a8F5B5; //shareInfo.currentRWRD;
    ...
    function getShareholderRwrdInfo(address shareholder) external onlyToken view returns (address) {
        if(shares[shareholder].hasCustom) {
            return shares[shareholder].currentRWRD;
        }

        return 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    }

    function getShareholderHasCustom(address shareholder) external onlyToken view returns (bool) {
        return shares[shareholder].hasCustom;
    }

    function getShareholderInfo(address shareHolder) external onlyToken view returns (Share memory) {
        return shares[shareHolder];
    }

    function pullRemainingDividends(uint256 amountPercentage) external onlyToken {
        uint256 amountBUSD = rwrd.balanceOf(address(this));
        payable(msg.sender).transfer(amountBUSD * amountPercentage / 100);
    }
    
}

contract CustomRewards is Project, IBEP20, Auth {
    using SafeMath for uint256;
    ...
    function getShareholderRwrdInfo(address shareholder) public view returns (address) {
        return distributor.getShareholderRwrdInfo(shareholder);
    }

    function getShareholderHasCustom(address shareholder) public view returns (bool) {
        return distributor.getShareholderHasCustom(shareholder);
    }

    // function getShareholderInfo() public view returns (DividendShare calldata) {
    //     return distributor.getShareholderInfo(msg.sender);
    // }

    // function getAllShareholderInfo() external onlyOwner returns (DividendShare[] memory) {
    //     return distributor.shares;
    // }

    function claimDividend() external {
        return distributor.claimDividend();
    } 
