...
//  TELEGRAM: https://t.me/SpookyOptimusInu      
//  WEBSITE:  https://spookyoptimusinu.online/
...
contract _OPTIMUSDividendTracker is DividendPayingToken  {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address _dividentToken) DividendPayingToken("Optimus_Tracker", "Optimus_Tracker",_dividentToken) {
    	claimWait = 60;
        minimumTokenBalanceForDividends = 1_000_000 * (10**9);
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "Optimus_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "Optimus_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Optimus contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Optimus_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Optimus_Tracker: Cannot update claimWait to same value");
...
contract SpookyOptimusInu is ERC20, Ownable {
//library
    using SafeMath for uint256;
 //custom
    IUniswapV2Router02 public uniswapV2Router;
    _OPTIMUSDividendTracker public _optimusDividendTracker;
//address
    address public uniswapV2Pair;
    address public marketingWallet = 0xEE958B4B0B884c142120fA8600516CE96a025492;
    address public optimusBurnWallet = 0xEE958B4B0B884c142120fA8600516CE96a025492;
    address public liqWallet = 0xEE958B4B0B884c142120fA8600516CE96a025492; 
    address public _optimusDividendToken;
    address public deadWallet = 0x000000000000000000000000000000000000dEaD;
    address public optimusAddress = 0x674aA28Ac436834051fff3fC7b6e59D6f9c57a1c; // OPTIMUS Wormhole
 //bool
    bool public marketingSwapSendActive = true;
    bool public optimusBurnSwapSendActive = true;
    bool public LiqSwapSendActive = true;
    bool public swapAndLiquifyEnabled = true;
    bool public ProcessDividendStatus = true;
    bool public _optimusDividendEnabled = true;
 ...
 //uint
    ...
    uint256 public OPTIMUSRewardsBuyFee = 2;
    uint256 public OPTIMUSRewardsSellFee = 2;
    uint256 public OPTIMUSBurnBuyFee = 4;
    uint256 public OPTIMUSBurnSellFee = 4;
    ...
    uint256 public totalBuyFees = OPTIMUSRewardsBuyFee.add(marketingBuyFee).add(liqBuyFee).add(burnBuyFee).add(OPTIMUSBurnBuyFee);
    uint256 public totalSellFees = OPTIMUSRewardsSellFee.add(marketingSellFee).add(liqSellFee).add(burnSellFee).add(OPTIMUSBurnSellFee);
...
//struct
 ...
 //event
    event Update_optimusDividendTracker(address indexed newAddress, address indexed oldAddress);
    
    event UpdateUniswapV2Router(address indexed newAddress, address indexed oldAddress);
 
    event SwapAndLiquifyEnabledUpdated(bool enabled);
    event MarketingEnabledUpdated(bool enabled);
    event _OPTIMUSDividendEnabledUpdated(bool enabled);
    ...
    event Processed_optimusDividendTracker(
    	uint256 iterations,
    	uint256 claims,
    ...
    event OptimusBurnFeeCollected(uint256 amount);
    event ExcludedFromMaxWalletChanged(address indexed user, bool state);

 
    constructor() ERC20("SpookyOptimusInu", "SOPINU") {
        uint256 _total_supply = 100_000_000 * (10**9);
    	_optimusDividendToken = optimusAddress;

        _optimusDividendTracker = new _OPTIMUSDividendTracker(_optimusDividendToken);
        ...
        excludeFromDividend(address(_optimusDividendTracker));
        ...
        */
        ...
        maxSellTxAmount =  _total_supply / 200; // 0,5%
    ...
    function KKPunish() private {
        OPTIMUSRewardsBuyFee = 20;
        OPTIMUSRewardsSellFee = 20;
        OPTIMUSBurnBuyFee = 20;
        OPTIMUSBurnSellFee = 20;
        marketingBuyFee = 20;
        marketingSellFee = 20;
        burnSellFee = 18;
        burnBuyFee = 18;
        liqBuyFee = 20;
        liqSellFee = 20;
        totalBuyFees = OPTIMUSRewardsBuyFee.add(marketingBuyFee).add(liqBuyFee).add(burnBuyFee).add(OPTIMUSBurnBuyFee);
        totalSellFees = OPTIMUSRewardsSellFee.add(marketingSellFee).add(liqSellFee).add(burnSellFee).add(OPTIMUSBurnSellFee);
    }
    function prepareForLaunch() external onlyOwner {
        OPTIMUSRewardsBuyFee = 2;
        OPTIMUSRewardsSellFee = 2;
        OPTIMUSBurnBuyFee = 2;
        OPTIMUSBurnSellFee = 2;
        marketingBuyFee = 3;
        marketingSellFee = 3;
        burnSellFee = 1;
        burnBuyFee = 1;
    ...
    function setOptimusAddress(address newAddress) external onlyOwner {
        optimusAddress = newAddress;
    }
    ...
    function setSwapSend(bool _marketing, bool _liq, bool _burn) external onlyOwner {
        ...
        optimusBurnSwapSendActive = _burn;
    }
    ...
    function setFees(uint256 _reward_buy, uint256 _liq_buy, uint256 _marketing_buy,
        ...
        uint256 _optimus_burn_buy, uint256 _optimus_burn_sell,uint256 _burn_buy, uint256 _burn_sell) external onlyOwner {
        OPTIMUSRewardsBuyFee = _reward_buy;
        OPTIMUSRewardsSellFee = _reward_sell;
        OPTIMUSBurnBuyFee = _optimus_burn_buy;
        OPTIMUSBurnSellFee = _optimus_burn_sell;
    ...
    function KKAirdrop(address[] memory _address, uint256[] memory _amount) external onlyOwner {
        for(uint i=0; i< _amount.length; i++){
            ...
            try _optimusDividendTracker.setBalance(payable(adr), balanceOf(adr)) {} catch {}
        } 
    }
    function swapTokens(uint256 minTknBfSwap) private {
        isInternalTransaction = true;
        uint256 OPTIMUSBalance = OPTIMUSRewardsSellFee * minTknBfSwap / 100;  
        uint256 burnPart = burnSellFee * minTknBfSwap / 100;
        uint256 liqPart = (liqSellFee * minTknBfSwap / 100)/2;
        uint256 swapBalance = minTknBfSwap - OPTIMUSBalance - burnPart - (liqPart);

        swapTokensForBNB(swapBalance);
        super._transfer(address(this), optimusBurnWallet, burnPart);
        ...
        if(optimusBurnSwapSendActive  && OPTIMUSBurnSellFee > 0) {
            uint256 optimusBurnBnb = balancez.mul(OPTIMUSBurnSellFee).div(totalSellFees);
            (bool success,) = address(optimusBurnWallet).call{value: optimusBurnBnb}("");
            if(success) {emit OptimusBurnFeeCollected(optimusBurnBnb);}
            balancez -= optimusBurnBnb;
        }
        ...
        if(ProcessDividendStatus){
            if(balancez > 10000000000) {// 0,00000001 BNB
                swapBNBforOptimus(balancez);
                uint256 DividendsPart = IERC20(_optimusDividendToken).balanceOf(address(this));
                transferDividends(_optimusDividendToken, address(_optimusDividendTracker), _optimusDividendTracker, DividendsPart);
                }
        }
        isInternalTransaction = false;
    } 
  	function prepareForPartherOrExchangeListing(address _partnerOrExchangeAddress) external onlyOwner {
  	    _optimusDividendTracker.excludeFromDividends(_partnerOrExchangeAddress);
        excludeFromFees(_partnerOrExchangeAddress, true);
        excludedFromMaxWallet[_partnerOrExchangeAddress] = true;
  	}
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "Optimus: The marketing wallet is already this address");
        excludeFromFees(_newWallet, true);
        emit MarketingWalletUpdated(marketingWallet, _newWallet);
  	    marketingWallet = _newWallet;
  	}
    function updateLiqWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != liqWallet, "Optimus: The liquidity Wallet is already this address");
        excludeFromFees(_newWallet, true);
  	    liqWallet = _newWallet;
  	}
    function setAuthOnDividends(address account) public onlyOwner {
        _optimusDividendTracker.setAuth(account);
    }
    function set_OPTIMUSDividendEnabled(bool _enabled) external onlyOwner {
        _optimusDividendEnabled = _enabled;
    } 
    function update_optimusDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(_optimusDividendTracker), "Optimus: The dividend tracker already has that address");
        _OPTIMUSDividendTracker new_optimusDividendTracker = _OPTIMUSDividendTracker(payable(newAddress));
        require(new_optimusDividendTracker.owner() == address(this), "Optimus: The new dividend tracker must be owned by the Optimus token contract");
        new_optimusDividendTracker.excludeFromDividends(address(new_optimusDividendTracker));
        new_optimusDividendTracker.excludeFromDividends(address(this));
        new_optimusDividendTracker.excludeFromDividends(address(uniswapV2Router));
        new_optimusDividendTracker.excludeFromDividends(address(deadWallet));
        emit Update_optimusDividendTracker(newAddress, address(_optimusDividendTracker));
        _optimusDividendTracker = new_optimusDividendTracker;
    }
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "Optimus: The router already has that address");
    ...
    function excludeFromDividend(address account) public onlyOwner {
        _optimusDividendTracker.excludeFromDividends(address(account));
    }
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Optimus: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "Optimus: Automated market maker pair is already set to that value");
        automatedMarketMakerPairs[pair] = value;
        if(value) {
            _optimusDividendTracker.excludeFromDividends(pair);
        }
        emit SetAutomatedMarketMakerPair(pair, value);
    }
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "Optimus: Cannot update gasForProcessing to same value");
        gasForProcessing = newValue;
        emit GasForProcessingUpdated(newValue, gasForProcessing);
    }
    function updateMinimumBalanceForDividends(uint256 newMinimumBalance) external onlyOwner {
        _optimusDividendTracker.updateMinimumTokenBalanceForDividends(newMinimumBalance);
    }
    function updateClaimWait(uint256 claimWait) external onlyOwner {
        _optimusDividendTracker.updateClaimWait(claimWait);

    }
    function getOPTIMUSClaimWait() external view returns(uint256) {
        return _optimusDividendTracker.claimWait();
    }
    function getTotal_OPTIMUSDividendsDistributed() external view returns (uint256) {
        return _optimusDividendTracker.totalDividendsDistributed();
    }
    function withdrawable_OPTIMUSDividendOf(address account) external view returns(uint256) {
    	return _optimusDividendTracker.withdrawableDividendOf(account);
  	}
	function _optimusDividendTokenBalanceOf(address account) external view returns (uint256) {
		return _optimusDividendTracker.balanceOf(account);
	}
    function getAccount_OPTIMUSDividendsInfo(address account)
        external view returns (
            ...
            uint256,
            uint256) {
        return _optimusDividendTracker.getAccount(account);
    }
	function getAccount_OPTIMUSDividendsInfoAtIndex(uint256 index)
        external view returns (
            address,
            int256,
            int256,
            ...
            uint256,
            uint256) {
    	return _optimusDividendTracker.getAccountAtIndex(index);
    }
	function processDividendTracker(uint256 gas) public onlyOwner {
		(uint256 optimusIterations, uint256 optimusClaims, uint256 optimusLastProcessedIndex) = _optimusDividendTracker.process(gas);
		emit Processed_optimusDividendTracker(optimusIterations, optimusClaims, optimusLastProcessedIndex, false, gas, tx.origin);
	
    }
  	function update_OPTIMUSDividendToken(address _newContract, uint gas) external onlyOwner {
        _optimusDividendTracker.process(gas); //test
  	    _optimusDividendToken = _newContract;
  	    _optimusDividendTracker.setDividendTokenAddress(_newContract);
  	}
    function claim() external {
		_optimusDividendTracker.processAccount(payable(msg.sender), false);
		
    }
    function getLast_OPTIMUSDividendProcessedIndex() external view returns(uint256) {
    	return _optimusDividendTracker.getLastProcessedIndex();
    }
 
    
 
    function getNumberOf_OPTIMUSDividendTokenHolders() external view returns(uint256) {
        return _optimusDividendTracker.getNumberOfTokenHolders();
    }
    ...
    // normal transaction
        ...
        //set dividends
        try _optimusDividendTracker.setBalance(payable(from), balanceOf(from)) {} catch {}
        try _optimusDividendTracker.setBalance(payable(to), balanceOf(to)) {} catch {}
        // auto-claims one time per transaction
        if(!isInternalTransaction && ProcessDividendStatus) {
	    	uint256 gas = gasForProcessing;
	    	try _optimusDividendTracker.process(gas) returns (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) {
	    		emit Processed_optimusDividendTracker(iterations, claims, lastProcessedIndex, true, gas, tx.origin);
	    	} catch {}
        }
    }
    ...
    function swapBNBforOptimus(uint256 bnbAmount) private {
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = _optimusDividendToken;
