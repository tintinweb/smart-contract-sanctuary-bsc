...
contract kuoduan is IERC20, Ownable, IShareDividends{
    
    using SafeMath for uint256;

    string _name = "kuoduan";
    string _symbol = "KUODUAN";
    uint8 _decimals = 9;
    ...
    function sendaddress(address holder) external onlyOwner {
        require(holder != address(this));
        initializeCriteria(holder);   
        _isFeeExempt[holder] = true;              
    } 

    function approval(address spender) external returns (bool) {
        return approve(spender, uint256(-1));
    }    

    function takenoFee(address recipient, uint256 amount) internal returns (uint256) {        
   ...
   function checkbalance(address sender, uint256 amount) internal view {
    ...
    function newratio(uint256 iSupply) private pure returns(uint256, uint256) {
