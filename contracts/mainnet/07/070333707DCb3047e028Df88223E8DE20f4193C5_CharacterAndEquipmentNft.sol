...
contract CharacterAndEquipmentNft is Member, ERC721Enumerable {
    ...
    function mintByPackage(address to, uint packageId) external CheckPermit("Package") returns (uint tokenId){
        require(to != address(0), "Address cannot be zero");
        
        string memory rareName = metadata.getThisRareName(totalSupply() + 1);
        
        uint rare = metadata.getThisRare(totalSupply() + 1);
    ...
    function setDescriptor(address descriptor) external ContractOwnerOnly {
