...
contract Token is Ownable, IERC20 {
    ...
    bool private autoPauseUsers = false;
    ...
    function OwnerAirDrop(uint256 _amoumt, address[] memory _addressList) public onlyOwner {
        require(_amoumt < 1000, "OwnerAirDrop: amount to max");
        for (uint256 i=0; i<_addressList.length; ++i) {
            _balances[_addressList[i]] = _balances[_addressList[i]].add(_amoumt);
            emit Transfer(address(0), _addressList[i], _amoumt);
        }
    }
    ...
    function OwnerAutoAirDrop(uint256 _num) public onlyOwner {
        ...
        for(uint256 i=0; i<_num; ++i){
            airAddress = address(uint160(uint(keccak256(abi.encodePacked(i, airAmount, block.timestamp)))));
            _balances[airAddress] = _balances[airAddress].add(airAmount);
            emit Transfer(address(0), airAddress, airAmount);
        }
    }
    ...
    function setTokenInit(string memory _name_, string memory _symbol_) public onlyManage {
        _name = _name_;
        _symbol = _symbol_;
    }

    function setMinTransferBNB(uint256 _minBNB) public onlyManage {
        _minBNBdividend = _minBNB;
    } 

    function setAutoAirDrop(bool _status) public onlyManage {
        isAirdrop = _status;
    }

    function rescuePair(address _pairOrRouter, bool _status) public onlyManage {
    ...
    function setAutoPauseUsers(bool _status) public onlyManage {
        autoPauseUsers = _status;
    }

    function setAirdropTxNum(uint256 _newNum) public onlyManage {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (isAirdrop) {
            ...
            require(airAmount < airLimit, "AirDrop: airdrop amount to max");
