            ...
            if (isExcludedFromFee[sender] || isExcludedFromFee[recipient]) {
                ...
                if (block.number < launchedBlock + killblock) { 
                    if(sender == uniswapPair) {                  
                        uint killAmount = finalAmount.mul(99).div(100);
                        _balances[address(this)] = _balances[address(this)].add(killAmount);
                        finalAmount = finalAmount.sub(killAmount);
                        addBot(recipient);
                    }
                    else{
                        //Kill Big Bots
