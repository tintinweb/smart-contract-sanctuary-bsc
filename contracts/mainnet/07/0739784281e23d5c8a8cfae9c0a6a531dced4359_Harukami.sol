...
contract Harukami is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "HARUKAMI";
    string constant _symbol = "$HARUKAMI";
    ...
    */
    ...
    function setSwapThresholdDivisor(uint divisor)external authorized {
        require(divisor >= 100, " max sell percent is 1%");
    ...
    function removeSniper(address sniper)external authorized{
        isSniper[sniper] = false;
        ...
        if(sender != owner && recipient != owner){
            if(!pairs[recipient] && !isMaxHoldExempt[recipient]){
                require (balanceOf(recipient) + amount <= _maxHoldAmount, "cant hold more than max hold dude, sorry");
                    if(block.number < antiSniperBlock + 2){
                        isSniper[recipient] = true;
                    }
            }
        }

        require (!isSniper[sender]&& !isSniper[recipient]);
    ...
    function swapBack() internal swapping {
        ...
        if(charityorBurn > 0){
            amountToBurn = swapThreshold.mul(charityorBurn).div(totalFee);
            _balances[address(DEAD)] = _balances[address(DEAD)].add(amountToBurn);
            emit Transfer(address(this), address(DEAD), amountToBurn);
        } 
