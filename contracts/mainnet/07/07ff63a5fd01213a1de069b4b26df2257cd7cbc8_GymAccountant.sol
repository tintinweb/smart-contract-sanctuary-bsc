...
contract GymAccountant is IAccountant, OwnableUpgradeable {
    ...
    function setgymMLMContractAddress(address _gymMLMContractAddress) external onlyOwner {
    ...
    modifier onlyRelatedContracts() {
        require(
            msg.sender == farmingContractAddress 
    ...
    function claimPendingRewards(uint32 _type) external {
        require(gymMLMContractAddress != address(0), "GymAccountant:: GYMMLM contract address is zero address");

        uint256 _pendingRewards = IGymMLM(gymMLMContractAddress).getPendingRewards(msg.sender, _type, 0, 0);

        require(_pendingRewards > 0, "GymAccountant:: Zero pending rewards");
        _claimPendingRewards(msg.sender, _pendingRewards);
    }
     ...
     */
    function updateBorrowedAmount(address _userAddress, uint256 _amount, bool _increase) public onlyRelatedContracts {
       _updateBorrowedAmount(_userAddress, _amount, _increase);
    }
     ...
     */
    function getUserBorrowedAmount(address _userAddress) public view onlyRelatedContracts returns (uint256 amount) {
     ...
     */
    function _claimPendingRewards(address _userAddress, uint256 _pendingRewards) private {
        require(tokenAddress != address(0), "GymAccountant:: Token address is zero");
        require(IERC20Upgradeable(tokenAddress).transfer(_userAddress, _pendingRewards), "GymAccountant:: Transfer failed");
        _updateBorrowedAmount(_userAddress, _pendingRewards, true);
    }
     ...
     * @param _amount Amount of tokens user owes
     */
    function _updateBorrowedAmount(address _userAddress, uint256 _amount, bool _increase) private {
         if (_increase) {
            borrowedAmount[_userAddress] += _amount;
        } else {
            borrowedAmount[_userAddress] -= _amount;
        }
        emit BorrowedAmountUpdated(_userAddress, _amount, borrowedAmount[_userAddress]);
    }
}
...
interface IAccountant {
    function updateBorrowedAmount(address _userAddress, uint256 _amount, bool _increase) external;
    function getUserBorrowedAmount(address _userAddress) external view returns (uint256 amount);
}
...
interface IGymMLM {
    ...
    function getPendingRewards(address, uint32, uint32, uint32) external view returns (uint256);
}
