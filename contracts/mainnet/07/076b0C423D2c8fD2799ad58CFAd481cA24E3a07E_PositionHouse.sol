...
//                }
    ...
    // settle not correct positions, will remove in the stable
    function settlePosition(IPositionManager _positionManager, address _trader, uint256 m) external nonReentrant onlyOwner {
        address _pmAddress = address(_positionManager);
        Position.Data memory _oldPosition = getPosition(_pmAddress, _trader);
        _withdraw(_pmAddress, _trader, m == 0 ? _oldPosition.margin : m);
        clearPosition(_pmAddress, _trader);
    }

    // function to refund lost amount from a cheater, will remove in the stable version
    function refundProtocol(IPositionManager _positionManager, address _trader, uint256 m) external onlyOwner {
        address _pmAddress = address(_positionManager);
        _deposit(_pmAddress, _trader, m, 0);
    }
