...
contract CSTPool is Ownable, Pausable {
    ...
    event NewVCSTContract(address VCST);
     ...
     * @param _pid: CST pool ID in MasterChefV2
     */
    constructor(
        IERC20 _token,
    ...
    function init(IERC20 dummyToken) external onlyOwner {
        ...
        masterchefV2.deposit(CSTPoolPID, balance);
        emit Init();
    }
    ...
    /**
     * @notice Checks if the msg.sender is either the CST owner address or the operator address.
     */
    modifier onlyOperatorOrCSTOwner(address _user) {
        require(msg.sender == _user || msg.sender == operator, "Not operator or CST owner");
        _;
    }
    ...
    function updateBoostContractInfo(address _user) internal {
        if (boostContract != address(0)) {
            ...
            IBoostContract(boostContract).onCSTPoolUpdate(
                _user,
                user.lockedAmount,
                lockDuration,
                totalLockedAmount,
    ...
    /**
     * @notice Unlock user CST funds.
     * @dev Only possible when contract not paused.
     * @param _user: User address
     */
    function unlock(address _user) external onlyOperatorOrCSTOwner(_user) whenNotPaused {
        UserInfo storage user = userInfo[_user];
    ...
    /**
     * @notice Deposit funds into the CST Pool.
     * @dev Only possible when contract not paused.
    ...
    function depositOperation(
        ...
        if (VCST != address(0)) {
            IVCST(VCST).deposit(_user, _amount, _lockDuration);
        }
        ...
        user.CSTAtLastUserAction = (user.shares * balanceOf()) / totalShares - user.userBoostedShare;
    ...
    /**
     * @notice Withdraw funds from the CST Pool.
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (VCST != address(0)) {
            IVCST(VCST).withdraw(msg.sender);
        }
    ...
    /**
     * @notice Harvest pending CST tokens from MasterChef
     */
    function harvest() internal {
        uint256 pendingCST = masterchefV2.pendingCST(CSTPoolPID, address(this));
        if (pendingCST > 0) {
            uint256 balBefore = available();
            masterchefV2.withdraw(CSTPoolPID, 0);
    ...
    /**
     * @notice Set VCST Contract address
     * @dev Callable by the contract admin.
     */
    function setVCSTContract(address _VCST) external onlyAdmin {
        require(_VCST != address(0), "Cannot be zero address");
        VCST = _VCST;
        emit NewVCSTContract(VCST);
    }
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the CST Pool
     */
    ...
    function calculatePerformanceFee(address _user) public view returns (uint256) {
        ...
        if (user.shares > 0 && !user.locked && !freePerformanceFeeUsers[_user]) {
            uint256 pool = balanceOf() + calculateTotalPendingCSTRewards();
     ...
     */
    function calculateTotalPendingCSTRewards() public view returns (uint256) {
        uint256 amount = masterchefV2.pendingCST(CSTPoolPID, address(this));
        return amount;
    }

    function getPricePerFullShare() external view returns (uint256) {
        return totalShares == 0 ? 1e18 : (((balanceOf() + calculateTotalPendingCSTRewards()) * (1e18)) / totalShares);
    }
