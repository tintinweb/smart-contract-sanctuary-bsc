...
contract TokieVault is Ownable, Pausable {
    ...
    function canCompound() public view returns (bool) {
        return masterchef.canHarvest(0, address(this)) && calculateTotalPendingTokieRewards() > 0;
    }
     ...
     */
    function calculateHarvestTokieRewards() public view returns (uint256) {
        uint256 amount = calculateTotalPendingTokieRewards();
     ...
     */
    function calculateTotalPendingTokieRewards() public view returns (uint256) {
        (,,uint256 rewardLockedUp,) = IMasterChef(masterchef).userInfo(0, address(this));
        uint256 amount = IMasterChef(masterchef).pendingTokie(0, address(this)).add(rewardLockedUp);
