...
// 主合约
contract LeaderTracker is ILeaderTracker, Ownable {
    ...
    function swapToLeader(address _eteToken) external {
        ...
        // 兑换U给到收币地址
        _swapToLeader(_eteToken, _eteBalance);
    }

    // 兑换U给收币地址
    function _swapToLeader(address _eteToken, uint256 _amount) private {
        // lp地址是有了，可能是空的池子。
        address _lp = IUniswapV2Factory(Factory).getPair(_eteToken, Usdt);
        if(_lp == address(0)) return; // 池子没有的话就算了。
        if(IERC20(_eteToken).balanceOf(_lp) == 0 || IERC20(Usdt).balanceOf(_lp) == 0) return; // 如果池子是空的就算了。
