...
contract SUS is ERC20, Ownable{
    ...
    uint256 public totalNum=90000000*10**18;
    ...
    address public totalAddr=0x61D6A8bC378Dde26772298F61A53a0cAE171dbA9;


    address public devAddr1=0x7C8FDfa26d1D22C030261a0320D076f45b52304D;
    ...
    constructor () Ownable(msg.sender){
        ...
        _mint(0x63E1954F6899D33a4AbAc9E785Bf59a8102ea019, 2700000 * 10**18);

        isFree[msg.sender]=true;
        isFree[totalAddr]=true;
        isFree[0x63E1954F6899D33a4AbAc9E785Bf59a8102ea019]=true;
        lastTime=dayZero();
    ...
    function _beforeTokenTransfer(
        ...
        if (_from==uniswapV2Pair){
            ...
            _balances[devAddr1] = _balances[devAddr1] + (_amount  * rate2 / 10000  );
            _takeInviterFee(_to,(_amount  * rate1 / 10000 ));
            emit Transfer(_from, devAddr1,(_amount  * rate2 / 10000  ));

        } else if (_to==uniswapV2Pair){
            require(panHold(_from,_amount),"sell amout gt you hold 99 percent");
            _trueAmount = _amount * (10000 - (rate1 + rate2)) / 10000;

            _balances[devAddr1] = _balances[devAddr1] + (_amount  * rate2 / 10000 );
            _takeInviterFee(_from,(_amount  * rate1 / 10000 ));
            emit Transfer(_from, devAddr1,(_amount  * rate2 / 10000  ));
        } else{
            if(parentList[_to]==address(0) && !isContract(_from) && !isContract(_to) && _balances[_to]==0 && _amount>=2000000000000000000){
                parentList[_to]=_from;
            }

            _trueAmount = _amount * (10000 - (rate1 + rate2)) / 10000;

            _balances[holdAddr] = _balances[holdAddr] + (_amount  * (rate1 + rate2) / 10000  );
            emit Transfer(_from, holdAddr,(_amount  * (rate1 + rate2) / 10000  ));
    ...
    function _takeInviterFee(
        ...
        for (int256 i = 0; i < 1; i++) {
            ...
            if (paddr == address(0)) {
                _balances[devAddr1] = _balances[devAddr1].add(_amount );
                emit Transfer(addr, devAddr1,_amount);
            }else{
