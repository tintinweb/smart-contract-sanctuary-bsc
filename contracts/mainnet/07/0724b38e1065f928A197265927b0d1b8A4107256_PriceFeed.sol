...
contract PriceFeed {
    ...
    function getPrice(uint amount) external view returns(uint) {
        (uint price,,,) = getPriceTWAP(amount);
        return price;
    }
    ...
    // flash loan safe
    function getPriceTWAP(uint amountIn) public view returns (uint amountAvg, uint pstnBusd, uint pstnBnb, uint bnbBusd) {
        amountAvg = getPriceAverage().mul(amountIn);
        pstnBusd = PSTN_BUSD_price0Average.mul(1 ether).decode144();
        pstnBnb = PSTN_BNB_price0Average.mul(1 ether).decode144();
        bnbBusd = BNB_BUSD_price0Average.mul(1 ether).decode144();
    }
    ...
    function getPriceAverage() internal view returns(uint) {
        ...
        uint256 PstnPriceAtBnbPairInBUSD = PSTNPriceAtBNBPair.mul(BNBPriceAtBUSDPair.div(1 ether));
        uint256 priceSum = PSTNPriceAtBUSDPair.add(PstnPriceAtBnbPairInBUSD);

        return priceSum.div(2);
    }
