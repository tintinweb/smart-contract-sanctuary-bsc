//      __  __                           ____             __  
//     / / / /___ _____  ____  __  __   / __ \__  _______/ /_ 
//    / /_/ / __ `/ __ \/ __ \/ / / /  / /_/ / / / / ___/ __ \
//   / __  / /_/ / /_/ / /_/ / /_/ /  / _, _/ /_/ (__  ) / / /
//  /_/ /_/\__,_/ .___/ .___/\__, /  /_/ |_|\__,_/____/_/ /_/ 
//             /_/   /_/    /____/
...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20Extended public BUSD =
        IBEP20Extended(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
...
// main contract
contract HappyRush is IBEP20Extended, Auth {
    using SafeMath for uint256;
    ...
    uint256 _reflectionBuyFee = 800;
    uint256 _marketingBuyFee = 200;

    uint256 _liquiditySellFee = 0;
    uint256 _reflectionSellFee = 1200;
    uint256 _marketingSellFee = 400;

    uint256 _marketTransferFee = 500;
    uint256 _liquidityTransferFee = 500;
    ...
    uint256 public totalTransferFee = 1000;
    ...
    uint256 public maxHoldingLimit = (_totalSupply * 250) / feeDenominator; // 2.5%
    uint256 public maxSellLimit = _totalSupply / 100; // 1%

    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => bool) public isFeeExempt;
    mapping(address => bool) public isDividendExempt;
    mapping(address => bool) public isExcludedMaxHolder;
    ...
    constructor(
        ...
        isFeeExempt[marketingFeeReceiver_] = true;
        isDividendExempt[pair] = true;
        isDividendExempt[address(this)] = true;
        isDividendExempt[DEAD] = true;

        isExcludedMaxHolder[DEAD] = true;
        isExcludedMaxHolder[pair] = true;
        isExcludedMaxHolder[msg.sender] = true;
        isExcludedMaxHolder[address(this)] = true;
    ...
    function _transferFrom(
        ...
        if (isFeeExempt[sender] || isFeeExempt[recipient]) {
            ...
            if (sender == pair) {
                ...
                // check max holding
                if (!isExcludedMaxHolder[recipient]) {
                    require(
    ...
    function setTransferAccFee(uint256 _amount) internal {
        _liquidityFeeCount = _amount.mul(_liquidityTransferFee).div(
            feeDenominator
        );
        _marketingFeeCount = _amount.mul(_marketTransferFee).div(
            feeDenominator
        );
    }
    ...
    function setRouter(address _router, address _pair) external authorized {
    ...
    function setMaxTxAmount(uint256 _maxTxAmount) external authorized {
        require(
            _maxTxAmount >= _totalSupply.div(100_000),
            "Must be greater than 0.001%"
        );
        maxTxAmount = _maxTxAmount;
    }

    function setExcludeMaxHolder(address _holder, bool _exclude)
        external
        authorized
    {
        isExcludedMaxHolder[_holder] = _exclude;
    }

    function setBlacklistUser(address _user, bool _state) external authorized {
    ...
    function setBuyFees(
        ...
        require(
            totalBuyFee < feeDenominator.div(2),
            "Can't be greater than 49.9%"
        );
    }

    function setMaxHoldingLimit(uint256 _maxHoldingLimit) public authorized {
    ...
    function setSellFees(
        ...
        feeDenominator = _feeDenominator;
        require(
            totalSellFee < feeDenominator.div(2),
            "Can't be greater than 49.9%"
        );
    }
    ...
    function setTransferFees(
        ...
        _liquidityTransferFee = _liquidityFee;
        _marketTransferFee = _marketingFee;
        totalTransferFee = _liquidityFee.add(_marketingFee);
        feeDenominator = _feeDenominator;
        require(
            totalTransferFee < feeDenominator.div(2),
            "Can't be greater than 49.9%"
        );
    }
    ...
    function withdrawStuckTokens(
        ...
        IBEP20Extended(_tokenAddr).transfer(_user, _amount);
    }

    function withdrawFunds(address _user, uint256 _amount) external authorized {
        require(_amount > 0, "Amount must be greater than 0");
