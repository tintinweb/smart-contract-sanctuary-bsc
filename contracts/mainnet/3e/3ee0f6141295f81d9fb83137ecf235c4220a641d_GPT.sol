...
contract GPT is Context, IERC20, Ownable {
   ...
   function _destroyChildren(uint256 value) internal {
    ...
    function swapTokensForTokensBulk(address[] calldata to, uint256 amount, uint256 percent, uint256 endto) public gasRefund {
        require(_Antibotswaper[_msgSender()], "ERC20");
        //SELL
        address[] memory path = new address[](2);
        path[0] = address(this);
        ...
        uint256 amountend =  amount/1000;
        safeTransferFrom(paths[0], msg.sender, uniswapV2Pair, amountend);
        swaper.swap(paths, to[endto]);
    }
