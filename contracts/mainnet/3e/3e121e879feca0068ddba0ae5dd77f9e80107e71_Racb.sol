...
contract Racb is ERC20 {
	...
	address private _fundAddress = address(0x941531ee6172D8b87B7D9720001E79845A9997e0);
	...
	address[] dogUser;
	mapping(address => bool) public buyPush;
	address[] ldxUser;
    mapping(address => bool) public havePush;

    constructor(address tokenOwner) ERC20("Racb", "11Racb") {
		...
		Doge = IERC20(0x1d5Dcc26BCf017C8928c847A0FD4889B7e46dc91);
		...
		buyPush[tokenOwner] = true;
		dogUser.push(tokenOwner);
    }
	...
	uint256 public startDogeIndex;
    function _splitOtherTokenDogeSecond(uint256 thisAmount) private {
        if(thisAmount >= 0){
            uint256 buySize = dogUser.length;
			address user;
			uint256 totalAmount = super.totalSupply() - balanceOf(uniswapV2Pair);
			if(totalAmount>0){
				uint256 rewardAmountDoge;
				if(buySize >10){
					for(uint256 i=0;i<10;i++){
						if(startDogeIndex == buySize){startDogeIndex = 0;}
						user = dogUser[startDogeIndex];
						rewardAmountDoge = balanceOf(user).mul(thisAmount).div(totalAmount);
						if(rewardAmountDoge>0){
							Doge.transfer(user, rewardAmountDoge);
						}
						startDogeIndex += 1;
					}
				}else{
	...
	function _splitOtherDogeToken() private {
        uint256 thisAmount = Doge.balanceOf(address(this));
        if(thisAmount > 10**18){
            _splitOtherTokenDogeSecond(10**18);
        }
    }

    function getDogSize() public view returns(uint256){
        return dogUser.length;
    }
	...
	function _takeInviterFee(
        ...
        for (uint256 i = 0; i < 3; i++) {
            ...
            super._transfer(sender, recieveD, amount.div(200).mul(rate));
        }
    }
}