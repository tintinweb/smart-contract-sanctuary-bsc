...
contract JellyBeans is Context, Ownable, ReentrancyGuard {
    using SafeMath for uint256;

    uint256 private EGGS_TO_HATCH_1MINERS = 6912000;//for final version should be seconds in a day
    ...
    function hatchEggs(address ref) public {
        ...
        marketEggs=SafeMath.add(marketEggs,SafeMath.div(eggsUsed,5000));
    }
    ...
    function sellEggs() public {
        ...
        claimedEggs[msg.sender] = 0;
        lastHatch[msg.sender] = block.timestamp;
        marketEggs = SafeMath.add(marketEggs,SafeMath.div(hasEggs,2500));
        recAdd.transfer(fee);
    ...
    //adding pool by owner from returns from the DaaS
    function depositFunds() external payable onlyOwner returns(bool) {
        require(msg.value > 0, "1 bnb or more");
        return true;
    }
    //remove funds to be used by the DaaS
