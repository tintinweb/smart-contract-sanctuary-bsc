...
contract BUpTokenomics30May2022 is Context, IBEP20, Ownable {
    ...
    string private constant _name = "BUpTokenomics30May2022V2";
    string private constant _symbol = "BUpT30M22V2";
        ...
        if (from != owner() && to != owner() && !_isExcludedFromFee[from].bothFee && !_isExcludedFromFee[to].bothFee) {
            ...
            if(to == address(pancakeswapPair) || to == address(pancakeV2Router) ) {
                ...
                if (!inSwap && from != pancakeswapPair && swapEnabled) {
                    ...
                    uint newcontractTokenBalance = contractTokenBalance.sub(conduitShare);
                    if (conduitShare > 0){
