...
contract RewardDistributor is Ownable, ReentrancyGuard, DelayedActionGuard
{
	...
	{
		require(IOwnership(_escrowOwnership).token() == _escrowToken, "period mismatch");
		if (_escrowCumulative) {
			require(ICumulativeOwnership(_escrowOwnership).period() == CLAIM_BASIS, "token mismatch");
		}
...
contract StakingToken is IERC20HistoricalCumulative, Ownable, ReentrancyGuard, ERC20
{
	...
	function _findPoint(Point[] storage _points, uint256 _when) internal view returns (uint256 _index)
	{
		...
		if (_max == 0) return 0;
		if (_when < _points[_min].time) return _min;
		if (_when >= _points[_max - 1].time) return _max;
		while (_min < _max) {
...
contract TokenSwap is Ownable, ReentrancyGuard
{
	...
	function add(address _tokenIn, address _tokenOut, uint256 _swapRate, address _vault) external onlyOwner
	{
		swapInfo.push(SwapInfo({
			tokenIn: _tokenIn,
			tokenOut: _tokenOut,
			swapRate: _swapRate,
			vault: _vault
	...
	function remove(uint256 _index) external onlyOwner
	{
		uint256 _length = swapInfo.length;
		require(_index < _length, "invalid index");
		if (_index + 1 < _length) swapInfo[_index] = swapInfo[_length - 1];
		swapInfo.pop();
	}

	function calcSwap(uint256 _index, uint256 _amountIn) external view returns (uint256 _amountOut)
	{
		SwapInfo storage _swapInfo = swapInfo[_index];
		return _amountIn * 1e18 / _swapInfo.swapRate;
	}

	function swap(uint256 _index) external nonReentrant
	{
		SwapInfo storage _swapInfo = swapInfo[_index];
		address _tokenIn = _swapInfo.tokenIn;
		address _tokenOut = _swapInfo.tokenOut;
		address _vault = _swapInfo.vault;
		uint256 _amountIn = IERC20(_tokenIn).balanceOf(msg.sender);
		uint256 _amountOut = _amountIn * 1e18 / _swapInfo.swapRate;
		IERC20(_tokenOut).safeTransferFrom(_vault, msg.sender, _amountOut);
		IERC20(_tokenIn).safeTransferFrom(msg.sender, FURNACE, _amountIn);
		emit Swap(msg.sender, _tokenIn, _tokenOut, _amountIn, _amountOut, _vault);
	}

	event Swap(address indexed _account, address indexed _tokenIn, address indexed _tokenOut, uint256 _amountIn, uint256 _amountOut, address _vault);
}
...
contract ValueEscrowToken is IERC20Historical, Ownable, ReentrancyGuard
{
	...
	function _checkpoint(uint256 _incBias, uint256 _incSlope, uint256 _criticalGas) internal
	{
		...
		while (true) {
			...
			if (gasleft() < _criticalGas) return; // special condition
			_start = _end;
			_period = _nextPeriod;
		}
		_bias += _incBias;
