...
contract FarmContract is ERC20, Ownable {
    ...
    /**获取矿信息*/
    function getFarmInfo() public view returns (FarmInfo memory info) {
        info = FarmInfo(
            _name,
            _symbol,
            rebates,
    ...
    function stakingEth(uint256 lockDay, address agent) external payable {
        uint256 amount = msg.value;
        require(
            mainToken == WETHAddr && amount > 0 && lockDay > 0,
            "INSUFFICIENT_BALANCE"
        );
        _stakings(amount, lockDay, agent);
    }
    ...
    function staking(uint256 amount, uint256 lockDay, address agent) external {
        ...
        require(
            token.balanceOf(msg.sender) >= amount && amount > 0 && lockDay > 0,
            "INSUFFICIENT_BALANCE"
        );
    ...
    function _stakings(uint256 amount, uint256 lockDay, address agent) private {
        if (
        ...
        totalReceives += Reward; //记录此次挖矿奖励
        _staking.addr = msg.sender;
        //如果锁定时间大于现在时间 - 增加质押时长; 否则重置
        _staking.lockTime = _staking.lockTime > block.timestamp
            ? _staking.lockTime.add(lockDay.mul(1 days))
            : block.timestamp.add(lockDay.mul(1 days));
    ...
    //查询收益
    function canCaim(address to) external view returns (uint256 _res) {
    ...
    //计算利息
    function _caclClaim(
        stakingInfo memory _staking
    ) private view returns (uint256 _res) {
        ...
        uint256 lockDay = (_staking.lockTime - _staking.stakingTime).div(
            1 days
        ); //获得锁定天数
        uint256 rate = lockDay.mul(lockApy); //总锁定天数 * 锁定利率
        if (lockDay >= stakDay) {
            //锁定时间大于已质押时间
            uint256 rateAmount = totalAmount.mul(rate).div(10000).div(
                lockApy.mul(1 days)
            ); //得到每秒的利息
            ...
            _res = rateAmount.add(
                totalAmount.mul((stakDay - lockDay).mul(Apy)).div(10000)
            );
        }
    }
    ...
    function Claim(address payable to) external {
        ...
        totalReceives += Reward; //记录此次挖矿奖励
        totalStaking -= _staking.staking; //扣除总质押
    ...
    /**获取线下总数 */
    function getInvitationLength(
        address account
    ) external view returns (uint256) {
        return InvitationSon[account].length();
    }
    ...
    function getInvitationList(
        ...
        uint256 from,
        uint256 limit
    ) external view returns (stakingInfo[] memory items) {
    ...
    function getUserInfo(
        address addr
    ) public view returns (stakingInfo memory info) {
    ...
    //更换质押代币
    function setStakingToken(address _mainToken) external onlyOwner {
        mainToken = _mainToken;
    }

    //更换WEth合约
    function setWEthToken(address _mainToken) external onlyOwner {
        WETHAddr = _mainToken;
    }
...
contract TeaFactory is ERC20, Ownable {
    ...
    function deployFarm(
        ...
        uint256 _lockApy,
        uint256 _Apy,
        string memory m_name,
        string memory m_symbol
    ) external payable {
        if (!_excludedFees[msg.sender]) {
            ...
            if (address(mainToken) != address(0)) {
                //扣除矿场创建费用
                try
                    mainToken.callPayMint{value: msg.value}(msg.sender)
                {} catch {}
            }
        }
    ...
    function getPairsList(
        uint256 from,
        uint256 limit
    ) external view returns (address[] memory items) {
