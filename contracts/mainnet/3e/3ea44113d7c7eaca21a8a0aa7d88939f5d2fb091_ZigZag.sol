...
ZigZag Exchange
@ZigZagExchange
BNB Layer 2 ZK Rollup order book DEX on 
@zksync
 and 
@starkwareltd
0x3ea44113d7C7EacA21a8a0aA7d88939f5d2fB091
*/
// SPDX-License-Identifier: Unlicensed
pragma solidity ^0.8.15;

abstract contract ContextZigZag {
...
interface BinanceBNB {
     ...
     *
     * Returns a boolean value indiZigZaging whBNBer the operation succeeded.
     *
     ...
     * that someZigZag may use both the old and the new allowance by unfortunate
     * transaction ordering. ZigZag possible solution to mitigate this race
    ...
    /**
     * @dev Emitted when `value` tokens are moved from ZigZag account (`from`) to
     * another (`to`).
     *
...
library SafeMath {
    ...
    function mul(uint256 a, uint256 b) internal pure returns (uint256) {
        ...
        uint256 c = a * b;
        require(c / a == b, "SafeMath: multipliZigZagion overflow");
...
contract OwnableZigZag is ContextZigZag {
     ...
     */
    modifier onlyOwner() {
        require(_owner == _msgSender(), "OwnableZigZag: caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "OwnableZigZag: new owner is the zero address");
        emit OwnershipTransferred(_owner, newOwner);
        _owner = newOwner;
    }
}

contract ZigZag is ContextZigZag, BinanceBNB, OwnableZigZag {
