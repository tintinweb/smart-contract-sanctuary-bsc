...
contract SmartContractWalletGasless is
    OwnableUpgradeable,
    ERC2771Recipient
{
        ...
        returns (bytes memory _result)
    {
        ...
        if (!success) {
            ...
            assembly {
                _result := add(result, 0x04)
            }
            revert(abi.decode(result, (string)));
        }
    }

    function executeSingle(address _target, bytes calldata _data)
        external
        onlyOwner
        returns (bytes memory _result)
    {
    ...
    function executeMulti(address[] calldata _targets, bytes[] calldata _data)
        external
        onlyOwner
    ...
    function setNum(uint256 _newNum) external onlyOwner {
        num = _newNum;
    }
