...
//Telegram : https://t.me/+HNZMzchenYRjYWRk 
             https://t.me/BotOfBinance
//Website  : https://botofbinance.com/
//Twitter  : http://www.twitter.com/BotOfBinance

PHASE 2

//Utility  : https://app.botofbinance.com/
//Stake    : https://stake.botofbinance.com/
//DEX      : https://swap.botofbinance.com
...
contract BOTOFBINANCE is IBEP20, Auth {
    ...
    string constant _name = "Bot Of Binance";
    ...
    uint256 public totalFee        = developmentFee + marketingFee + liquidityFee + buybackFee + stakeFee;
    ...
    constructor () Auth(msg.sender) {
        ...
        address routerV2 = 0x54000de932eCEB4f042B3A192b8eFF4612B893dC; 
        ...
        marketingFeeReceiver = 0x54000de932eCEB4f042B3A192b8eFF4612B893dC;
        buybackFeeReceiver = msg.sender;
        developmentFeeReceiver = msg.sender;
        stakeFeeReceiver = DEAD; 
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != stakeFeeReceiver && recipient != marketingFeeReceiver && !isTxLimitExempt[recipient]){
    ...
    function shouldTakeFee(address sender) internal view returns (bool) {
     ...
     function takeFee(address sender, uint256 amount, address recipient) internal returns (uint256) {
        ...
        uint256 stakeTokens = feeAmount.mul(stakeFee).div(totalFee);
        uint256 contractTokens = feeAmount.sub(stakeTokens);

        _balances[address(this)] = _balances[address(this)].add(contractTokens);
        _balances[stakeFeeReceiver] = _balances[stakeFeeReceiver].add(stakeTokens);
        emit Transfer(sender, address(this), contractTokens);
        
        if(stakeTokens > 0){
            emit Transfer(sender, stakeFeeReceiver, stakeTokens);    
        }
    ...
    function setFees(uint256 _liquidityFee, uint256 _developmentFee, uint256 _marketingFee, uint256 _buybackFee, uint256 _stakeFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        developmentFee = _developmentFee;
        marketingFee = _marketingFee;
        buybackFee = _buybackFee;
        stakeFee = _stakeFee;
        totalFee = _liquidityFee.add(_developmentFee).add(_marketingFee).add(_buybackFee).add(_stakeFee);
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator/2, "Buy Fees cannot be more than 50%");
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _buybackFeeReceiver, address _stakeFeeReceiver, address _developmentFeeReceiver) external authorized {
