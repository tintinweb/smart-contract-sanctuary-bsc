...
contract FoeVesting is Ownable, ReentrancyGuard {
    ...
    function nextclaim_( address account) external view returns (uint256){
        return vests[account].nextClaim;
    }

     function vestingBalance(address account) external view  returns (uint256){
        return vests[account].availableAmount;
    }

    function vest(address account,uint256 _amount, uint256 initailReleasePercentage, uint256 lockDurationInMonth, uint256 vestDurationInMonth) external onlyOwner nonReentrant returns(bool){
        require(!vests[account].status, "Already vested");
        require(_amount > 0, "Amount should be greater than zero");
        uint256 initaitReleaseAmount;
        uint256 taxDeductionAmt;
        taxDeductionAmt = _amount - calculatePercentage(_amount , transferTaxPercentage);
        if(initailReleasePercentage != 0)
            initaitReleaseAmount = calculatePercentage(taxDeductionAmt , initailReleasePercentage);
        vests[account] = VestDetail(
            account,
            taxDeductionAmt,
            taxDeductionAmt - initaitReleaseAmount,
            block.timestamp,
            block.timestamp + lockDurationInMonth * 30 days,
            vestDurationInMonth,
            block.timestamp + lockDurationInMonth * 30 days,
            initaitReleaseAmount,
            taxDeductionAmt - initaitReleaseAmount,
            true
        );
        require(token.transferFrom(msg.sender, address(this), _amount), "token transfer failed");
        if(initailReleasePercentage != 0)
            require(token.transfer(account,initaitReleaseAmount ), "token transfer to user failed");
        emit Vested(account, vests[account].vestingAmount);
        return true;
    }
    ...
    function claim() external nonReentrant returns(bool) {
        ...
        uint256 amountPerMonth = vests[msg.sender].vestingAmount / vests[msg.sender].vestingDurationInMonth;
        uint256 claimAmount = amountPerMonth;
        uint256 count = 1;
        if(timeDiff > 30 days) {
            count = timeDiff / 30 days;
        ...
        if(vests[msg.sender].availableAmount != 0 )
            vests[msg.sender].nextClaim += count * 30 days;
        else{
            vests[msg.sender].nextClaim = 0;
            vests[msg.sender].status = false;
        }
    ...
    function calculatePercentage(uint256 val , uint256 percentage) internal pure returns( uint256 result){
