...
contract Dummy is ERC721Enumerable, Ownable {
  ...
  constructor(string memory name, string memory symbol, string memory baseTokenURI) ERC721(name, symbol) {
      ...
      // Save dev wallet
      _wallet = 0x563b0Fc7bb3144C609b8C3272f773FA0ce524316;
  }
  ...
  function mint(uint256 _mintAmount) public payable {
    // Checks
    require(!paused, "Dummy: minting has not started yet");
    require(_mintAmount > 0, "Dummy: you have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "Dummy: you can only mint 10 at a time");
    require(lastSupply >= _mintAmount,  "Dummy: the collection is sold out");
    if (msg.sender != owner()) {
      require(msg.value >= cost * _mintAmount, "Dummy: total cost isn't match");
    }
    // Minting
  ...
  function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
    // Checks
    require(_winner != address(0), "Dummy: zero address");
    require(_amount > 0, "Dummy: you have to mint at least one");
    require(lastSupply != 0,  "Dummy: the collection is sold out");
  ...
  function tokenURI(uint256 _id) public view virtual override returns (string memory){
    require(_exists(_id),"Dummy: URI query for nonexistent token");
