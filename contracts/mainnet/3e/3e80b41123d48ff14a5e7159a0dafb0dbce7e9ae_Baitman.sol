...
/**
 .----------------.  .----------------.  .----------------.  .----------------.  .----------------.  .----------------.  .-----------------.
| .--------------. || .--------------. || .--------------. || .--------------. || .--------------. || .--------------. || .--------------. |
| |   ______     | || |      __      | || |     _____    | || |  _________   | || | ____    ____ | || |      __      | || | ____  _____  | |
| |  |_   _ \    | || |     /  \     | || |    |_   _|   | || | |  _   _  |  | || ||_   \  /   _|| || |     /  \     | || ||_   \|_   _| | |
| |    | |_) |   | || |    / /\ \    | || |      | |     | || | |_/ | | \_|  | || |  |   \/   |  | || |    / /\ \    | || |  |   \ | |   | |
| |    |  __'.   | || |   / ____ \   | || |      | |     | || |     | |      | || |  | |\  /| |  | || |   / ____ \   | || |  | |\ \| |   | |
| |   _| |__) |  | || | _/ /    \ \_ | || |     _| |_    | || |    _| |_     | || | _| |_\/_| |_ | || | _/ /    \ \_ | || | _| |_\   |_  | |
| |  |_______/   | || ||____|  |____|| || |    |_____|   | || |   |_____|    | || ||_____||_____|| || ||____|  |____|| || ||_____|\____| | |
| |              | || |              | || |              | || |              | || |              | || |              | || |              | |
| '--------------' || '--------------' || '--------------' || '--------------' || '--------------' || '--------------' || '--------------' |
 '----------------'  '----------------'  '----------------'  '----------------'  '----------------'  '----------------'  '----------------' 
*/
...
contract Baitman is Context, IERC20, Ownable {
    ...
    string private _name = "Baitman";
    string private _symbol = "BMAN";
    ...
    address public charityWallet = 0xDe962066e4a79558452CEA1c7276462e3f2d892f;
    ...
    uint256 private numTokensSellToAddToLiquidity = 250000 * 10**9; // 0.1%
    ...
    function setMaxTxTokens(uint256 maxTxAmount) external onlyOwner() {
        _maxTxAmount = maxTxAmount;
    }

    function setTokensToSwap(uint256 tokensToSwap) external onlyOwner() {
        numTokensSellToAddToLiquidity = tokensToSwap;
    }

    function setMaxWalletTokens(uint256 maxWallTokens) external onlyOwner() {
        _maxWalletSize = maxWallTokens;
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if(sender != owner() && recipient != owner() && sender!= address(this))
        {
            ...
            charityAmount = calculateCharityFee(amount);
        }


        
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, amount.sub(burnAmount).sub(charityAmount));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, amount.sub(burnAmount).sub(charityAmount));
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender, recipient, amount.sub(burnAmount).sub(charityAmount));
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, amount.sub(burnAmount).sub(charityAmount));
        } else {
            _transferStandard(sender, recipient, amount.sub(burnAmount).sub(charityAmount));
        }

        //Temporarily remove fees to transfer to burn and charity address
        removeAllFee();
        ...
        if(charityAmount > 0) {
            _transferStandard(sender, charityWallet, charityAmount);
        }
