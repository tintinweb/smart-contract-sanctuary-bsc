...
contract GloryICO is Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    function buyICOByBUSDWhiteList(uint256 amount) external payable onlyWhiteList {
        ...
        customerInfos[msg.sender].tokenNumber = customerInfos[msg.sender].tokenNumber.add(buyAmountToken);
        customerInfos[msg.sender].busd = customerInfos[msg.sender].busd.add(amount);
        customerInfos[msg.sender].buyTime = block.timestamp;
        customerInfos[msg.sender].totalRound1 = customerInfos[msg.sender].totalRound1.add(amount);
    }

    function buyICOByUSDTWhiteList(uint256 amount) external payable onlyWhiteList {
        _preCheckBuyPublicSale(amount);
        _preCheckTotalAmount(msg.sender, amount);
        ...
        customerInfos[msg.sender].tokenNumber = customerInfos[msg.sender].tokenNumber.add(buyAmountToken);
        customerInfos[msg.sender].usdt = customerInfos[msg.sender].usdt.add(amount);
        customerInfos[msg.sender].buyTime = block.timestamp;
        customerInfos[msg.sender].totalRound1 = customerInfos[msg.sender].totalRound1.add(amount);
    }
    ...
    function depositByBUSD(uint256 amount) external payable {
        ...
        userDeposits[msg.sender].tokenAmount = userDeposits[msg.sender].tokenAmount.add(buyAmountToken);
        userDeposits[msg.sender].busd = userDeposits[msg.sender].busd.add(amount);
        userDeposits[msg.sender].totalAmount = userDeposits[msg.sender].totalAmount.add(amount);
    ...
    function depositByUSDT(uint256 amount) external payable {
        ...
        userDeposits[msg.sender].tokenAmount = userDeposits[msg.sender].tokenAmount.add(buyAmountToken);
        userDeposits[msg.sender].usdt = userDeposits[msg.sender].usdt.add(amount);
        userDeposits[msg.sender].totalAmount = userDeposits[msg.sender].totalAmount.add(amount);
        addressDeposits.push(msg.sender);
    }

    function setRemainIcoTokenAmount(uint256 _remainIcoTokenAmount) external onlyAdmin {
        require(_remainIcoTokenAmount > 0, "remainIco invalid");
        remainIcoTokenAmount = _remainIcoTokenAmount;
    }

    function setPriceFeedAddress(address _priceFeedAddress) external onlyOwners {
    ...
    function withdrawBUSDWhitelist() external onlyWhiteListWithDraw {
        ...
        userDeposits[msg.sender].totalAmount = userDeposits[msg.sender].totalAmount.sub(busd);
        userDeposits[msg.sender].tokenAmount = userDeposits[msg.sender].tokenAmount.sub(buyAmountTokenRound2);
    ...
    function withdrawUSDTWhitelist() external onlyWhiteListWithDraw {
        ...
        userDeposits[msg.sender].totalAmount = userDeposits[msg.sender].totalAmount.sub(usdt);
        userDeposits[msg.sender].tokenAmount = userDeposits[msg.sender].tokenAmount.sub(buyAmountTokenRound2);
    ...
    function setRoundWhiteListInfo(
        ...
        require(_startTimeWhitelistICO < _endTimeWhitelistICO, "invalid whitelist time");
    ...
    function setWhiteListWithdraw(address[] memory whitelistWithdraw) external onlyAdmin {
    ...
    function deleteListWhitelistWithdraw(address[] memory customer) external onlyAdmin {
        for (uint256 i = 0; i < customer.length; i++) {
            isConfirmedWithDraw[customer[i]] = false;
        }
    }

    function setTotalAmountPerBUSD(uint256 _totalAmountPerBUSD) external onlyAdmin {
        require(_totalAmountPerBUSD > 0, "invalid rate buy ICO by BUSD");
        totalAmountPerBUSD = _totalAmountPerBUSD;
    }
    ...
    function getAddressPosition(uint256 position) public view returns (address){
    ...
    function _preCheckBuyPublicSale(uint256 amount) internal view {
        require(amount > 0, "value must be greater than 0");
        require(block.timestamp >= startTimeWhitelistICO, "ICO whitelist time does not start now");
        require(block.timestamp <= endTimeWhitelistICO, "ICO whitelist  time is expired");
    }

    function _preCheckTotalAmountRound2(address customer, uint256 amount) internal view {
        uint256 total = userDeposits[customer].totalAmount;
        require(total.add(amount).div(10 ** 18) <= 5000, "address holds more than allowed amount");
    }
    ...
    function _preCheckTotalAmount(address owner, uint256 amount) internal view {
        ...
        if (round == 1) {
            ...
            uint256 totalRound = customerInfos[owner].totalRound1.add(customerInfos[owner].totalRound2);
            total = customerInfos[owner].usdt.add(customerInfos[owner].busd).sub(totalRound);
        }
        require(total.add(amount).div(10 ** 18) <= 10000, "address holds more than allowed amount");
    }

    function _preCheckLimitBuyRound2(uint256 buyTokenAmount) internal view {
        require(totalRoundWhiteList.add(buyTokenAmount).div(10 ** 18) <= 6000000, "sold out round whitelist");
    }
