...
contract FUNDS is Context, IBEP20, Ownable {

    using SafeMath for uint256;
    using Address for address;
    
    string private _name     = "FREEDOM FUND";
    ...
    address public _growthFundAddress = 0x66010855F936C4A8946B0E99379474F28a0591e5; 
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount) private {
		...
		if (tFee > 0) {
            ...
            uint256 growthTokens = tFee.mul(growthFee).div(_totalTax);
            ...
            // nftReflection wallet fee 
             ...
             // growth wallet fee 
             if(growthTokens > 0) {
                _growthReflectionTokens(growthTokens);
                 emit Transfer(sender, _growthFundAddress, growthTokens);
             }
    ...
    function _growthReflectionTokens(uint256 tFee) private {
        uint256 currentRate = _getRate();
        uint256 rTakeAmount = growthFee.mul(currentRate);
         _rOwned[_growthFundAddress] = _rOwned[_growthFundAddress].add(rTakeAmount);
        if(_isExcluded[_growthFundAddress]) {
            _tOwned[_growthFundAddress] = _tOwned[_growthFundAddress].add(tFee);
        }
    }
    ...
    function setGrowthFundAddress(address growthAddress) external onlyOwner() {
        _growthFundAddress = growthAddress;
    }
