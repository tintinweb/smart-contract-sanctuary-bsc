...
// File: contracts/interfaces/IPanaSwapLottery.sol
library SafeMath {
    /**
...
interface IPanaSwapLottery {
    ...
    /**
     * @notice Draw the final number, calculate reward in PANA per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in PANA token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInPana: price of a ticket in PANA
...
// File: contracts/PanaSwapLottery.sol

pragma solidity ^0.8.4;
pragma abicoder v2;

/** @title PanaSwap Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract PanaSwapLottery is ReentrancyGuard, IPanaSwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInPana = 50 ether;
    uint256 public minPriceTicketInPana = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInPana;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] panaPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInPana;
        uint32 finalNumber;
    }
     ...
     * @param _panaTokenAddress: address of the PANA token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _panaTokenAddress, address _randomGeneratorAddress) {
        panaToken = IERC20(_panaTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of PANA to this contract
        uint256 amountPanaToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInPana,
            _ticketNumbers.length
        );

        // Transfer pana tokens to this contract
        panaToken.safeTransferFrom(address(msg.sender), address(this), amountPanaToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInPana += amountPanaToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInPanaToTransfer
        uint256 rewardInPanaToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInPanaToTransfer += rewardForTicketId;
        }
        ...
        if(referrer != address(0) && referrer != msg.sender && referralCommissionRate > 0){
            uint256 commissionAmount  = rewardInPanaToTransfer.mul(referralCommissionRate).div(10000);
            panaToken.safeTransfer(referrer, commissionAmount);
            IREFERRAL(referralContract).recordReferralCommission(referrer, commissionAmount);
        }
        // Transfer money to msg.sender
        panaToken.safeTransfer(msg.sender, rewardInPanaToTransfer);       

        emit TicketsClaim(msg.sender, rewardInPanaToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInPana) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in PANA for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].panaPerBracket[j] =
                ...
                // A. No PANA to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].panaPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInPana - amountToShareToWinners);

        // Transfer PANA to treasury address
        panaToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        panaToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInPana += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInPana >= minPriceTicketInPana) && (_priceTicketInPana <= maxPriceTicketInPana),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInPana: _priceTicketInPana,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            panaPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInPana: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(panaToken), "Cannot be PANA token");
    ...
    /**
     * @notice Set PANA price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInPana: minimum price of a ticket in PANA
     * @param _maxPriceTicketInPana: maximum price of a ticket in PANA
     */
    function setMinAndMaxTicketPriceInPana(uint256 _minPriceTicketInPana, uint256 _maxPriceTicketInPana)
        external
        onlyOwner
    {
        require(_minPriceTicketInPana <= _maxPriceTicketInPana, "minPrice must be < maxPrice");

        minPriceTicketInPana = _minPriceTicketInPana;
        maxPriceTicketInPana = _maxPriceTicketInPana;
    }
     ...
     * @param _priceTicket price of a ticket (in PANA)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].panaPerBracket[_bracket];
        } else {
            return 0;
        }
    }
