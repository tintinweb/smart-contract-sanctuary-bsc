...
contract SHARBI_BSC is IBEP20, Ownable {
    using SafeMath for uint256;
    ...
    function getDynamicSellFee() internal view returns(uint256) {
        if(block.timestamp <= launchedAt.add(1 minutes)){
            return 300;
        } else if(block.timestamp > launchedAt.add(5 minutes) && block.timestamp <= launchedAt.add(5 minutes)){
            return 250;
        } else if(block.timestamp > launchedAt.add(10 minutes) && block.timestamp <= launchedAt.add(10 minutes)){
            return 200;
        } else if(block.timestamp > launchedAt.add(15 minutes) && block.timestamp <= launchedAt.add(15 minutes)){
            return 100;
        }  else {
            return totalSellFee;
        }
