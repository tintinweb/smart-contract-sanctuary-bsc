...
contract Main is BEP20, SharedOwnable, SafeToken {
    ...
    constructor(string memory name_, string memory symbol_, uint8 decimal_, address reward1_, address marketing_, address team_, address multiuse_, uint256 supply_, uint256 dispatch_, uint8 maxtaxes_) BEP20(name_, symbol_) {
        ...
        MaxSwap = supply_ * 2 / 1000;
        MaxSell = supply_ * 2 / 100;
        ...
        dividendTracker1 = new DIVIDENDTracker1("XrpGrow_Track", "XRPGROW", Token1Adress, minTokenBalanceForDividends, decimal, Creator);
    ...
    function SetLimits(uint256 _maxWallet, uint256 _maxSell, uint256 _minswap, uint256 _swapmax, uint256 newMinimumBalance, uint256 NewGas, uint256 claimWait, uint256 MaxTax, bool _swapAndLiquifyEnabled, bool autoreward, bool autodistribution) external onlySharedOwners {
        ...
        require(NewGas >= 100000, "Gas can't be lower than 100000");
    ...
    function Launch(uint256 Blocks) external onlySharedOwners {
        ...
        require(Blocks <= 6000, "Not more than 1mn");
        tradingEnabled = true;
        counter = block.number + Blocks;
    }

    function SetbotTax(bool jeetsfee, bool jeetsburn, bool jeetsstaking, uint8 vmaxdiv, uint8 vmindiv, uint8 maxjeetsfee)  external onlySharedOwners {
        require (vmaxdiv >= 10 && vmaxdiv <= 60, "cannot set Vmax outside 10%/60% ratio");
        require (vmindiv >= 1 && vmindiv <= 10, "cannot set Vmin outside 1%/10% ratio");
        require (maxjeetsfee >= 1 && maxjeetsfee <= 60, "max jeets fee must be betwwen 1% and 60%");
        JeetsFee = jeetsfee;
        JeetsBurn = jeetsburn;
...
contract SafuElonXmas is Main {

    constructor() Main (
        "SafuElonXmas",       // Name
        "SEX",        // Symbol
        9,                  // Decimal
        0xFeF0c1670cb569008bB3d070918922dBb4F92De4,     // Reward 1
        0x864ff887986e57e688223FbaF0BA4d22a7FEE4eA,     // Marketing address
        0x864ff887986e57e688223FbaF0BA4d22a7FEE4eA,     // Team address AND DEV
        0x864ff887986e57e688223FbaF0BA4d22a7FEE4eA,     // Multi Use address (CHARITY)
