...
contract MagicUnicorn3 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    MagicUnicorn3DividendTracker public dividendTracker;
    ...
    constructor() ERC20("MagicUnicorn3", "MU3") {
        ...
        dividendTracker = new MagicUnicorn3DividendTracker(payable(this), router, 0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48,
                                                   "MagicUnicorn3TRACKER", "MU3TRACKER");
...
// test buyback sending tokens directly to dead address
    function testBuyBack1() external onlyOwner {
        uint256 buybackBNB = address(this).balance;
        if (buybackBNB > 0) {
            triggerBuyback();
            _totalSupply = _totalSupply - buybackBNB;
...
contract MagicUnicorn3DividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    MagicUnicorn3 public MagicUnicorn3Contract;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        MagicUnicorn3Contract = MagicUnicorn3(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main MagicUnicorn3 contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            address[] memory path = new address[](2);
            path[0] = uniswapV2Router.WETH();
            path[1] = address(MagicUnicorn3Contract);

            uint256 prevBalance = MagicUnicorn3Contract.balanceOf(address(this));
            ...
            {
                uint256 received = MagicUnicorn3Contract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    MagicUnicorn3Contract.transfer(account, received);
                } 
                else {
                    success = false;
                }
