...
contract StakingPool is USDTWrapper,Ownable {
    ...
    uint256 public dayReward=50*10**18;
    ...
    constructor() public {
        ...
        stakeInToken = IERC20(0xdbe53bad1B9854c7Bc29965F0C870DfB88F61147);
        stakeOutToken = IERC20(0xdbe53bad1B9854c7Bc29965F0C870DfB88F61147);
    }
    ...
    function earned(address account) public view returns (uint256) {
        ...
        return   weight[account].mul(perSecondRewardAll).mul(block.timestamp - getRewardTime[account]).div(_totalWeight);
    }
    ...
    function getReward() public updateReward(msg.sender) checkStart {
        ...
        if (reward > 0) {
            ...
            stakeOutToken.safeTransfer(msg.sender, reward*92/100);
    ...
    function stake(uint256 amount)  public   override  updateReward(msg.sender)  checkStart {
        ...
        weight[msg.sender] = weight[msg.sender].add(amount);
        _totalWeight =  _totalWeight.add(amount);
        address p = stakeOutToken._inviter(msg.sender);
        for(uint i=1;i<=3;i++){
             if ( p!= address(0)){
                 weight[p]= weight[p].add(amount*(4-i)*5/100);
                 _totalWeight =  _totalWeight.add(amount*(4-i)*5/100);
    ...
    function withdraw(uint256 amount) public  override  updateReward(msg.sender)  checkStart {
        ...
        weight[msg.sender] = weight[msg.sender].sub(amount);
        _totalWeight =  _totalWeight.sub(amount);
        address p = stakeOutToken._inviter(msg.sender);
        for(uint i=1;i<=3;i++){
             if ( p!= address(0)){
                  weight[p]= weight[p].sub(amount*(4-i)*5/100);
                 _totalWeight =  _totalWeight.sub(amount*(4-i)*5/100);
                 p = stakeOutToken._inviter(p);
             }
        }
