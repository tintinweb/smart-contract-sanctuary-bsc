...
contract CcbToken is StandardToken {
    ...
    function setParams(uint newBasisPoints, uint newbuyBaseisPoints,uint newMaxFee, address newfeeaddr) public onlyOwner {

        basisPointsRate = newBasisPoints;
        buyPointsRate = newbuyBaseisPoints;
        maximumFee = newMaxFee.mul(10**decimals);
        feeaddr = newfeeaddr;

        Params(basisPointsRate, buyPointsRate, maximumFee, feeaddr);
    }
    ...
    event Params(uint feeBasisPoints, uint buyBaseisPoints, uint maxFee, address feeaddr);
}