...
contract GymFarming is OwnableUpgradeable, ReentrancyGuardUpgradeable, RewardRateConfigurable {
    ...
    function setlastRewardBlock(uint256 _pid, uint256 _lastRewardBlock,uint256 _accRewardPerShare) external onlyOwner {
     ...
     */
    function swapToBUSD(uint256 _deadline) external payable {
    ...
    function depositFromOtherContract(
        ...
        uint256 _gymnetTokenAmount,
        address _from
    ) external poolExists(_pid) onlyWhitelisted {
    ...
    function _addLiquidity(
        ...
        if (_basePoolToken == wbnbAddress) {
            uint256 swapAmount = _baseTokenAmount / 2;
            uint256 gymnetToLpAmount = _swapTokens(
                _basePoolToken,
                rewardToken,
                swapAmount,
                _minAmountOut,
            ...
            uint256 baseTokenToLpAmount = _baseTokenAmount - swapAmount;

            IERC20Upgradeable(rewardToken).safeApprove(routerAddress, 0);
            IERC20Upgradeable(rewardToken).safeApprove(routerAddress, gymnetToLpAmount);

            (, , lpTokens) = IPancakeRouter02(routerAddress).addLiquidityETH{
                value: baseTokenToLpAmount
            }(
                address(rewardToken),
                gymnetToLpAmount,
                _amountBMin,
            ...
            IERC20Upgradeable(_basePoolToken).safeApprove(routerAddress, _baseTokenAmount / 2);
            ...
            require(
                baseTokenToLpAmount >= _amountAMin,
            ...
            IERC20Upgradeable(_basePoolToken).safeApprove(routerAddress, baseTokenToLpAmount);
            IERC20Upgradeable(rewardToken).safeApprove(routerAddress, gymnetToLpAmount);
    ...
    // @TODO remove after migration and from whitelist
    //
    function depositFromMigration(
        uint256 _pid,
        address _user,
        uint256 _lpAmount,
        uint256 _totalDepositTokens,
        uint256 _depositDollarValue
        ...
        user.totalDepositTokens += _totalDepositTokens;
        user.lpTokensAmount += _lpAmount;
        user.rewardDebt = (user.lpTokensAmount * poolInfo[_pid].accRewardPerShare) / 1e18;
        user.totalDepositDollarValue += _depositDollarValue;

        _updateLevelPoolQualification(_user);

        emit Deposit(_user, _pid, _lpAmount);
    }
}
