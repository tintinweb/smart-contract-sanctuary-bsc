...
contract TYNToken is Context, IERC20, Ownable {
    ...
    string private _name = 'TYN';
    string private _symbol = 'TYN';
    uint8 private _decimals = 18;
    uint256 private _totalSupply = 1000000 * 10**uint256(_decimals);

    address public _burnAddr = address(0x000000000000000000000000000000000000dEaD);
    address public _fundAddr = address(0x88659122CaA3D938c6EC427762546dab1d4A2D95);
    address public _nftAddr = address(0xf476724aF6c3a22bD0c973a0Dca0Bc8E9Bd2BB7f);
    address public _fomoAddr = address(0x949f162BA662966c0485e56d93940Efd7Eb1DaCf);
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        (uint256 tTransferAmount, uint256 tBurn, uint256 tLiquidity, uint256 tFomo, uint256 tFund, uint256 tNft) = _getValues(tAmount);
        ...
        !_isExcludedFromFee[recipient]
            ...
            emit Transfer(sender, _burnAddr, tBurn);

            _takeLPFee(sender, tLiquidity);
    ...
    function _takeLPFee(address sender,uint256 tAmount) private {
        ...
        if (bnbAmount > 0) {
            _balances[uniswapV2BnbPair] = _balances[uniswapV2BnbPair].add(bnbAmount);
            emit Transfer(sender, uniswapV2BnbPair, bnbAmount);
        }

        if (usdtAmount > 0) {
            _balances[uniswapV2UsdtPair] = _balances[uniswapV2UsdtPair].add(usdtAmount);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tBurn, uint256 tLiquidity, uint256 tFomo, uint256 tFund, uint256 tNft) = _getTValues(tAmount);

        return (tTransferAmount, tBurn, tLiquidity, tFomo, tFund, tNft);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        uint256 tTotalFee = tBurn.add(tLiquidity).add(tFomo).add(tFund).add(tNft);
    ...
    function removeAllFee() private {
        if(_burnFee == 0 && _liquidityFee == 0 && _fomoFee == 0 && _fundFee == 0 && _nftFee == 0) return;
