...
library AutoMintMath {
    function calculateAutoMintAmount(
        uint256 balanceOf,
        uint256 mintRate,
        uint256 dayCount
    ) internal pure returns (uint256) {
        ...
        if (dayCount > 0) {
            for (uint256 i = 0; i < dayCount; i++) {
                amount = (amount * mintRate) / 100 + amount;
            }
            return amount - balanceOf;
        }
        return 0;
    }
}
...
contract APEToken is ERC20, Ownable {
    ...
    uint256 public totalFee =
        buybackFee + marketingFee + liquidityFee + burnFee;
    ...
    event UpdateUSDT(address newUSDT);

    event SetFees(
        uint256 buybackFee,
        uint256 marketingFee,
        uint256 liquidiytFee,
        uint256 burnFee
    );

    event SetWhiteList(address account, bool isWhitelist);
    ...
    function updatePancakeRouter(address newAddress) public onlyOwner {
        require(
            ...
            "APEToken: The router already has that address"
        );
    ...
    function updateUsdtAddress(address newUsdt) public onlyOwner {
        require(
            newUsdt != usdt && newUsdt != address(0),
            "APEToken: The usdt already has that address"
        );
        usdt = newUsdt;
        emit UpdateUSDT(newUsdt);
    }
    ...
    function setFees(
        ...
        burnFee = _burnFee;
        uint256 totalFees = buybackFee + marketingFee + liquidityFee + burnFee;
        require(totalFees < 51, "Too much");
        emit SetFees(_buybackFee, _marketingFee, _liquidityFee, _burnFee);
    }
    ...
    function setWhiteList(address account, bool isWhitelist)
        external
        onlyOwner
    {
    ...
    function _transfer(
        ...
        if (!tradingEnabled) {
            require(
                ...
                "APEToken: token not enabled for trading"
            );
        }
        ...
        bool isWhitelist = whitelist[to] ||
            whitelist[from] ||
            whitelist[msg.sender];

        if (tradingEnabled && !swapping && swappingEnabled && !isWhitelist) {
            swapping = true;
            {
                uint256 liquidityAmount = amount.mul(liquidityFee).div(100);
                uint256 marketingAmount = amount.mul(marketingFee).div(100);

                uint256 swapTokensAtAmount = liquidityAmount.add(
                    marketingAmount
                );

                amount = amount.sub(swapTokensAtAmount);
                super._transfer(from, address(this), swapTokensAtAmount);
                uint256 marketingUSDT = swapTokensForUsdt(marketingAmount);
                IERC20(usdt).transfer(marketingWallet, marketingUSDT);
                swapAndAddLiquidity(from, liquidityAmount);
            }
            swapping = false;
        }
        ...
        if (tradingEnabled && !swapping && swappingEnabled && !isWhitelist) {
            ...
            uint256 amountBurn = buybackAmount.add(burnAmount);
            super._transfer(from, address(this), amountBurn);
            _burn(address(this), burnAmount);
            super._transfer(address(this), pancakePair, buybackAmount);
            amount = amount.sub(amountBurn);
        }
    ...
    function swapAndAddLiquidity(address from,uint256 tokens) private {
        uint256 swapAmount = tokens.div(2);

        if (balanceOf(address(this)) >= tokens) {
            uint256 amountU = swapTokensForUsdt(swapAmount);
            if (amountU > 0) {
