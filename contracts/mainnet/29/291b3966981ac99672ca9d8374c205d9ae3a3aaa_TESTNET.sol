...
contract TESTNET is ERC20, Ownable { 
    ...
    address public _devWalletAddress = address(0x7F5A4bEbD25b6F71704174df9BC670cb233D0c1D); // Dev Wallet
    address public _setupWalletAddress = address(0x90C7aAcE263c7e952424772CbBdf89BBA7bf3A63); //Setup Wallet
    ...
    constructor() ERC20("TESTNET", "TEST") {
        ...
        buyFee.setupFee = 80; // 8% Setup fee
        buyFee.devFee = 20;  // 2% Dev fee
        totalBuyFee = buyFee.setupFee + buyFee.devFee;

        sellFee.setupFee = 60; // 6% Setup fee
        sellFee.devFee = 40;   // 4% Dev fee
        totalSellFee = sellFee.setupFee + sellFee.devFee;
        ...
        excludeFromFees(_setupWalletAddress, true);
    ...
    function setSetupWallet(address payable wallet) external onlyOwner{
        _setupWalletAddress = wallet;
    }

    function setBuyFee(uint16 liqFee, uint16 dev) external onlyOwner {
        buyFee.setupFee = liqFee;
        buyFee.devFee = dev;
        totalBuyFee = buyFee.setupFee + buyFee.devFee;
        require(totalBuyFee <= 10, "max fee 10%");
    }

    function setSellFee(uint16 liqFee, uint16 dev) external onlyOwner {
        sellFee.setupFee = liqFee;
        sellFee.devFee = dev;
        totalSellFee = sellFee.setupFee + sellFee.devFee;
        require(totalSellFee <= 15, "max fee 15%");
    }
    ...
    function swapAndSend(uint256 tokens) private {
        ...
        uint256 projectShare = newBalance.mul(buyFee.setupFee + sellFee.setupFee).div(totalFees);

        payable(_setupWalletAddress).transfer(projectShare);
