...
contract NxToken is IERC20, Ownable, MemeberManage, Mode {
    ...
    function _checkAvailableTransferAndFee(address sender, address recipient, uint256 amount) private view returns (uint256 fee, uint256 rev,bool isHole) {
        ...
        require(!_fromBlackList[sender], "ERC20: transfer refuse by sender");
    ...
    function addFromBlackList(address[] memory whos) public onlyOwner {
        for(uint256 i=0;i<whos.length;i++){
            _fromBlackList[whos[i]] = true;
        }
    }

    function rmFromBlackList(address[] memory whos) public onlyOwner {
        for(uint256 i=0;i<whos.length;i++){
            _fromBlackList[whos[i]] = false;
        }
    }
