...
contract stakereward {
    ...
    uint[] public levels = [1000*1e18,2000*1e18,3000*1e18];
    uint[] public levelRates = [16000,18000,20000];
        ...
        public
    {
      ...
      require(!accounts[msg.sender].isStaked,"err");
      ...
      {
          totalStake = totalStake.add(nftvalue);
          hasStake[msg.sender] = nftvalue;
            addrProviders.add(msg.sender);
            addrExist[msg.sender] = true;
      }

        uint rateAmount = nftvalue.mul(_dateRate).div(BASE).div(_periodFinish);
      rate[msg.sender] = rateAmount;

      accounts[msg.sender].lastUpdate = block.timestamp;
      accounts[msg.sender].isStaked = true;
      ...
      for(uint i=0;i<_referRewardRate.length;i++)
        {
            ...
            affReward = rateAmount.mul(_referRewardRate[i]).div(BASE);
            if(curlastAddr == address(0)){
                break;
            }
            accounts[curlastAddr].leftReward = getRewardNum(curlastAddr);
            accounts[curlastAddr].lastUpdate = block.timestamp;
            accounts[curlastAddr].extraRate = accounts[curlastAddr].extraRate.add(affReward);
            curAddr = curlastAddr;
        }
    ...
    function getRewardBalance(address _usr) internal view returns(uint ret){
        uint _timeinfo = 0;
        ...
        ret = rate[_usr].add(accounts[_usr].extraRate).mul(_timeinfo);
    ...
    function getRewardNum(address _usr) public view returns(uint ret){
        ...
        ret = getRewardBalance(_usr).add(accounts[_usr].leftReward);

    }
    

    function getUSRRate(address _usr) public view returns(uint ret){
        
        ret = rate[_usr].add(accounts[_usr].extraRate);
    }
    ...
    function getReward() public {
        ...
        hasReward[msg.sender] = hasReward[msg.sender].add(rewardAmounts);
        IERC20(punkplusToken).safeTransfer(msg.sender,rewardAmounts);
    ...
    function setDateRate(uint256 new_dateRate) public  
        onlyOwner
    {
        _dateRate = new_dateRate;
    }
        ...
        onlyOwner
    {
        ...
        while (iterations < addrCount) {
            ...
            uint256 reward = hasStake[curAddr].mul(amounts).div(totalStake);
