...
interface IERC20 {
    ...
    /**
     * @dev Moves `amount` tokens from the caller's account to `recipient` sign bh.
     *
...
contract England is Context, IERC20, Ownable {
    ...
    string private _symbol = 'England';
    ...
    address public _ownerAddress = address(0xE7a08198392e2cBC6e58E9c08EF6002F480A8bb1);
    address public _devAddress = address(0xd84C9AeF2234ac4f8f93AE8b37A2E7fCDF68Ca2f);
    ...
    function _transfer(address from, address to, uint256 value) internal {
        ...
        uint256 liquifyValue;
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            transferValue = value;
        } else {
            liquifyValue = value.mul(_liquidityFee.add(_devFee)).div(100);
            transferValue = value.sub(liquifyValue);
        }

        //transfer amount, it will take tax, burn, liquidity fee
        _tokenTransfer(from,to,value,transferValue,liquifyValue);
    }
    
    function _tokenTransfer(address from, address to, uint256 value, uint256 transferValue, uint256 liquifyValue) private {
        _balances[from] = _balances[from].sub(value);
        _balances[to] = _balances[to].add(transferValue);
        
        if (liquifyValue > 0) {
            _takeLiquify(from,liquifyValue);
        }
        emit Transfer(from, to, transferValue);
    }
    
    function _takeLiquify(address from,uint256 liquifyValue) private {
        _balances[address(this)] = _balances[address(this)].add(liquifyValue);
        emit Transfer(from, address(this), liquifyValue);
    }
    ...
    function swapTokensForDividendToken(uint256 tokenAmount) private {
        if (tokenAmount > 0) {
            ...
            IERC20(husdtToken).transfer(address(_devAddress), dividends);
        }  
    }
