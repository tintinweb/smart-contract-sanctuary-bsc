...
/**
SHORT CRONOS TOKEN CONTRACT

CONTRACT ADDRESS DEVELOPED 

https://t.me/shortcronos
*/
...
contract shortcronos is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    shortcronosDividendTracker public dividendTracker;
    ...
    constructor() ERC20("ShortCronos", "SHORTCRO") {
        ...
        dividendTracker = new shortcronosDividendTracker(
            payable(this),
            router,
        ...
        maxTransactionAmount = totalTokenSupply / 200; // 0.5%
    ...
    // set max wallet, can not be lower than 0.01% of supply & higher than 4%
    function setmaxWallet(uint256 value) external onlyOwner {
        value = value * (10**18);
        require(value >= _totalSupply / 10000, "max wallet cannot be set to less than 0.01%");
        require(value <= _totalSupply / 25, "max wallet cannot be higher than 4%");
        maxWallet = value;
    }

    // set max Tx, can not be lower than 0.005% of supply & higher than 2%
    function setmaxTX(uint256 value) external onlyOwner {
        ...
        require(value <= _totalSupply / 50, "max TX cannot be higher than 2%");
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            require(contractBalanceRecipient + amount <= maxTransactionAmount,
...
contract shortcronosDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    shortcronos public shortcronosContract;
    ...
    constructor(
        ...
        defaultToken = token;
        shortcronosContract = shortcronos(mainContract);
    ...
    function _transfer(
        ...
        require(false, "shortcronos_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "shortcronos_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main shortcronos contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(shortcronosContract);

            uint256 prevBalance = shortcronosContract.balanceOf(address(this));
            ...
            {
                uint256 received = shortcronosContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    shortcronosContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
