...
contract Astronomics is ERC20Detailed, Ownable {
    ...
    string public _name = "ASTRONOMICS";
    string public _symbol = "ASN";
    ...
    uint256 public astroInsuranceFundFee = 45;
    uint256 public treasuryFee = 25;
    uint256 public sellFee = 25;
    uint256 public blackholeFee = 25;
    uint256 public nftFee = 15;
    uint256 public totalFee = liquidityFee.add(treasuryFee).add(astroInsuranceFundFee).add(blackholeFee);
    ...
    address public astroInsuranceFundReceiver;
    ...
    constructor() ERC20Detailed("ASTRONOMICS", "ASN", uint8(DECIMALS)) Ownable() {
        ...
        treasuryReceiver = 0xcA25a86e086D17FEc3626faEf9405898BD95A195; 
        autoLiquidityReceiver = 0x000000000000000000000000000000000000dEaD;
        astroInsuranceFundReceiver = 0xA8aB9BFF76bB67C0efA694EE432922655F34C82d;
        devFeeReceiver = 0x36459A3754911F5b9ff5A01B1448513F4D2E76E6;
        blackhole = DEAD;
        ...
        _initRebaseStartTime = 	1654005600;	
        _lastRebasedTime = 	1654005600;
    ...
    function takeFee(
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_treasuryFee.add(astroInsuranceFundFee))
        );
        _gonBalances[autoLiquidityReceiver] = _gonBalances[autoLiquidityReceiver].add(
            gonAmount.div(feeDenominator).mul(liquidityFee)
        );
        _gonBalances[devFeeReceiver] = _gonBalances[devFeeReceiver].add(
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountWBNBToTreasuryAndAIF = IERC20(WBNB).balanceOf(address(this)).sub(balanceBefore);

        IERC20(WBNB).transfer(treasuryReceiver, amountWBNBToTreasuryAndAIF.mul(treasuryFee).div(treasuryFee.add(astroInsuranceFundFee)));
        IERC20(WBNB).transfer(astroInsuranceFundReceiver, amountWBNBToTreasuryAndAIF.mul(astroInsuranceFundFee).div(treasuryFee.add(astroInsuranceFundFee)));
    }

    function withdrawAllToTreasury() external swapping onlyOwner {

        uint256 amountToSwap = _gonBalances[address(this)].div(_gonsPerFragment);
        require( amountToSwap > 0,"There is no Astro token deposited in token contract");
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = WBNB;
        router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
    ...
    function setFeeReceivers(
        ...
        address _astroInsuranceFundReceiver
    ) external onlyOwner {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        treasuryReceiver = _treasuryReceiver;
        astroInsuranceFundReceiver = _astroInsuranceFundReceiver;
    }

    function setDevFeeReceivers(address _devFeeReceiver) external {
        require(msg.sender == devFeeReceiver, '!fee');
        require(_devFeeReceiver != address(0), "zero");
    ...
    function setLimit(uint256 _holdLimit, uint256 _sellLimit) public onlyOwner {
        require(_holdLimit >= 5 && _holdLimit <= 100000, "Invalid hold limit");
        require(_sellLimit >= 5 && _sellLimit <= 100000, "Invalid sell limit");
