...
contract Aggregate is ReentrancyGuard,Ownable {
    ...
    function swapTokensForExactTokens(address leftCoin,address middleCoin,address rightCoin,
                                      address leftRoute,address rightRoute,
                                      uint amountInMax,uint amountOut,
                                      address to,uint deadline) external nonReentrant payable {
        require(block.timestamp <= deadline, "EXPIRED");
        uint middle=calcInputs(IRouter(rightRoute),middleCoin,rightCoin,amountOut);
        if(feeAddress!=address(0)&&feeRate>0) {
            middle=middle*10000/(10000-feeRate);
        }
        uint result=calcInputs(IRouter(leftRoute),leftCoin,middleCoin,middle);
        require(result<=amountInMax,"EXCESSIVE_INPUT_AMOUNT");
        _swap(leftCoin,middleCoin,rightCoin,leftRoute,rightRoute,result,to);
    }

    function swapExactTokensForTokens(address leftCoin,address middleCoin,address rightCoin,
                                      address leftRoute,address rightRoute,
                                      uint amountIn,uint amountMinOut,
                                      address to,uint deadline) external nonReentrant payable {
        require(block.timestamp <= deadline, "EXPIRED");
        uint middle=calcOutputs(IRouter(leftRoute),leftCoin,middleCoin,amountIn);
        if(feeAddress!=address(0)&&feeRate>0) {
            middle=middle*(10000-feeRate)/10000;
        }
        uint result=calcOutputs(IRouter(rightRoute),middleCoin,rightCoin,middle);
        require(result>=amountMinOut,"INSUFFICIENT_OUTPUT_AMOUNT");
        _swap(leftCoin,middleCoin,rightCoin,leftRoute,rightRoute,amountIn,to);
    }

    function _swap(address leftCoin,address middleCoin,address rightCoin,
                                      address leftRoute,address rightRoute,
                                      uint amountIn,address to) internal {
        leftToMiddle(IRouter(leftRoute),leftCoin,middleCoin,amountIn);
        getFee(middleCoin);
        middleToRight(IRouter(rightRoute),middleCoin,rightCoin,to);
    }

    function middleToRight(IRouter route,address input,address output,address to) internal {
        if(input==output) {
            if(input!=address(0)){
    ...
    function getFee(address middleCoin) internal {
        if(feeAddress!=address(0)&&feeRate>0) {
            uint middle=IERC20(middleCoin).balanceOf(address(this));
    ...
    function calcOutputs(IRouter route,address input,address output,uint amountInput) view internal returns(uint) {
        ...
        address wapperOutput=(output==address(0)?weth:output);
        IPair pair=route.factory().getPair(wapperInput,wapperOutput);
        return calcOutput(route,pair,wapperInput,wapperOutput,amountInput);
    }

    function calcOutput(IRouter route,IPair pair,address input,address output,uint amountInput) view internal returns(uint amountOutput) {
    ...
    function calcInputs(IRouter route,address input,address output,uint amountOutput) view internal returns(uint) {
        ...
        address wapperOutput=(output==address(0)?weth:output);
        IPair pair=route.factory().getPair(wapperInput,wapperOutput);
        return calcInput(route,pair,wapperInput,wapperOutput,amountOutput);
    }

    function calcInput(IRouter route,IPair pair,address input,address output,uint amountOutput) view internal returns(uint amountInput) {
        (address token0,) = sortTokens(input, output);
        (uint reserve0, uint reserve1,) = pair.getReserves();
        (uint reserveInput, uint reserveOutput) = input == token0 ? (reserve0, reserve1) : (reserve1, reserve0);
