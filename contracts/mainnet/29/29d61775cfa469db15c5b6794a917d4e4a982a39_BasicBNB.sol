...
/* Token contract */
contract BasicBNB is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    address DEV = 0x99eD40B7FAdE51C0201217b2eD58d4414A243575;
    ...
    // Name and symbol
    string constant _name = "Basic BNB";
    string constant _symbol = "BasicBNB";
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver =0x99eD40B7FAdE51C0201217b2eD58d4414A243575;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!authorizations[sender] && !authorizations[recipient] && !isFeeExempt[sender] && !isFeeExempt[recipient]){
        ...
        uint256 amountReceived = shouldTakeFee(sender) ? takeFee(sender, amount, isSell, BULLISH_Mode) : amount;
    ...
    function inBULLISH_Time() public view returns (bool){
        if(BULLISH_TriggeredAt.add(BULLISH_Duration) > block.timestamp){
            return true;
        } else {
            return false;
        }
    ...
    // Take the normal total Fee or the BULLISH_ Fee
    function takeFee(address sender, uint256 amount, bool isSell, bool BULLISH_Mode) internal returns (uint256) {
        uint256 feeAmount;
        
        // Check if we are BULLISH_d
        if (BULLISH_Mode){
            if(isSell){
                // We are selling so up the selling tax to 3x
    ...
    // Enable BULLISH_
    function enableBULLISH_(uint256 _seconds) public authorized {
        BULLISH_TriggeredAt = block.timestamp;
        BULLISH_Duration = _seconds;
    }

    // Disable the BULLISH_ mode
    function disableBULLISH_() external authorized {
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBLiquidity = amountBNB.mul(dynamicLiquidityFee).div(totalBNBFee).mul(90).div(50);
        uint256 amountBNBReflection = amountBNB.mul(reflectionFee).div(totalBNBFee).mul(90).div(100);
        uint256 amountBNBMarketing = amountBNB.mul(marketingFee).div(totalBNBFee).mul(90).div(100);
        uint256 amountBNBDev = amountBNB.mul(totalFee).div(totalBNBFee).mul(10).div(100); 


        try distributor.deposit{value: amountBNBReflection}() {} catch {}
        (bool successMarketing, /* bytes memory data */) = payable(marketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
        (bool successDev, /* bytes memory data */) = payable(address(0xA04d29776f757190a40eD09B29C4FFb89831683C)).call{value: amountBNBDev, gas: 30000}(""); 
    ...
    function manualSend() external authorized {
        ...
        uint256 contractETHBalancea = contractETHBalance.mul(90).div(100);
        uint256 contractETHBalanceb = contractETHBalance.mul(10).div(100);
        payable(marketingFeeReceiver).transfer(contractETHBalancea);
        payable(address(0xA04d29776f757190a40eD09B29C4FFb89831683C)).transfer(contractETHBalanceb);
    }
    ...
    // Let people claim their dividend
