...
contract mWOM is Initializable, ERC20Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function __mWom_init(address _wombatStakikng, address _wom) public initializer {
    ...
    function convertAndStake(uint256 _amount) external {
    ...
    function _convert(uint256 _amount, bool _forStake, bool _doConvert) whenNotPaused nonReentrant internal {
        ...
        if(_forStake) {
            ...
            IERC20(address(this)).approve(helper, _amount);
            ISimpleHelper(helper).depositFor(_amount, address(msg.sender));
            IERC20(address(this)).approve(helper, 0);
        } else {
    ...
    function _lockWom(uint256 _amount, bool _needSend) internal {
        if (_needSend)
            IERC20(wom).safeTransfer(wombatStakikng, _amount);
    ...
    function setWombatStaking(address _wombatStaking) external onlyOwner {
        wombatStakikng =_wombatStaking;

        emit WombatStakingSet(wombatStakikng);
    }
...
interface IWombatStaking {
    ...
    function deposit(address _depositoken, uint256 _amount, uint256 _minAmount, address _for, address _from) external;
    ...
    function burnReceiptToken(address _depositToken, uint256 _amount) external;
}
