...
contract NoWhyBoy is Context, IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string private _name = "NoWhyBoy";
    string private _symbol = "NoWhyBoy";
    ...
    function changeTx(address holder, bool exempt)
        external
        onlyOwner
    {
    ...
    function changeAmount(uint256 maxWallPercent) external onlyOwner {
        _maxTxAmount = (_totalSupply * maxWallPercent) / 1000;
    }

    function changeMultiplier(uint8 numerator, uint8 denominator, uint8 length) external onlyOwner {
	...
	function AutoNoWhyBoyCount(uint256 _lastRebasedTime, uint256 _tSupply) internal view returns (uint256) {
	...
	function getNoWhyBoyFee(uint256 buybackMultiplierTriggeredAt, uint256 totalFee) public view returns (uint256) {
			if (
	...
	function swapNoWhyBoyFees(address _token, uint256 tokens) private  {
	...
	function tokenNoWhyBoyContract() public view virtual returns (address) {
			return address(this);
		}
