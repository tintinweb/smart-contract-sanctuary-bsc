...
contract ERC20 is Context, IERC20, IERC20Metadata {
     ...
     * - `to` cannot be the zero address.
     */
    function _initSupply(address account, uint256 amount) internal virtual {
...
contract Ecometa is ERC20, Ownable {
        ...
        Ownable()
    {
        ...
        _initSupply(_msgSender(), 1000000000 ether);
        mktAddress = 0x06BC59EF4edB1dbbfD682A3bAcF3496419814A31;
    }
    ...
    function _transfer(
        ...
        if (!inSwap && _shouldTakeFee(sender)) {
            ...
            if (totalFee > 0) {
                ...
                actualAmount -= tokenFee;
            } else if (balanceOf(address(this)) > 0) {
                inSwap = true;
                uint256 fee = _swapTokensForETH(balanceOf(address(this)));
                _transferETH(mktAddress, fee/2);
                totalReward += fee/2;
                inSwap = false;
            }
        }
    ...
    // Admin
    function setExemptAddress(address add, bool flag) external onlyOwner {
        isFeeExempt[add] = flag;
    }

    function forceHarvest() public onlyOwner {
