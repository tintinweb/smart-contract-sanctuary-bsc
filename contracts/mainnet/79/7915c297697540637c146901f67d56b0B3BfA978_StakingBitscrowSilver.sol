...
contract StakingBitscrowSilver {
    ...
    constructor(IERC20 _erc20_contract_address, uint _apy) {
        // Set contract owner
        owner = msg.sender;
        // Set staking period
        timePeriod = 30 days;
    ...
    function timeLeftTillUnstake(address user) public  view returns(uint256){
        if(block.timestamp >= timestart[user] + timePeriod || balances[user] == 0) {
            return 0;
        }else {
    ...
    function contractInfo()external view returns(uint cost, uint seats, uint Stakingperiod, uint Apy) {
