...
/**
    Telegram Channel: https://t.me/ShibaCeoBurn
...
contract ShibaCEOBurn is Ownable, IBEP20 {
    using SafeMath for uint256;

    string constant _name = "ShibaCEOBurn";
    string constant _symbol = "SHIBCB";
    ...
    function takeFee(address sender, uint256 amount, address recipient) internal returns (uint256) {
        uint256 dynamicBuyFee;
        uint256 dynamicSellFee;
        if(block.timestamp <= launchedAt.add(1 minutes)){
            dynamicSellFee = 30;
            dynamicBuyFee = 20;
        } else if(block.timestamp > launchedAt.add(1 minutes) && block.timestamp <= launchedAt.add(5 minutes)){
            dynamicSellFee = 25;
            dynamicBuyFee = 15;
            ...
            dynamicSellFee = totalFee;
            dynamicBuyFee = totalFee;
        }
        uint256 dynamicFee = dynamicSellFee;
        if(sender == pair) {
            dynamicFee = dynamicBuyFee;
        } else if(recipient == pair) {
            dynamicFee = dynamicSellFee;
        }

        uint256 feeAmount = amount.mul(dynamicFee).div(feeDenominator);
