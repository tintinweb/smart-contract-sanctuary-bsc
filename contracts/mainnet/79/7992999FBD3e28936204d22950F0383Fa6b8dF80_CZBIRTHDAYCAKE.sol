...
/**
WE ARE HERE TO CELEBRATE CZ BIRTHDAYS!! CZ, CEO OF BINANCE, TURNS 45 THIS 10TH SEPTEMBER!! 
$CZ BIRTHDAY CAKE
*/
...
library Address{
 ...
 contract CZBIRTHDAYCAKE is Context, IERC20, Ownable {
    ...
    address public marketingWallet = 0x2804C1C975F41001ffd6cDEa75911702710c41FF;
    address public CZCakeWallet = 0x2804C1C975F41001ffd6cDEa75911702710c41FF; 

    string private constant _name = "WE ARE HERE TO CELEBRATE CZ BIRTHDAYS!! CZ, CEO OF BINANCE, TURNS 45 THIS 10TH SEPTEMBER!!";
    string private constant _symbol = "CZ BIRTHDAY CAKE";
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[CZCakeWallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[marketingWallet] = true;
        allowedTransfer[CZCakeWallet] = true;
    ...
    function setTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _CZCake) public onlyOwner {
       taxes = Taxes(_rfi,_marketing,_liquidity,_CZCake);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _CZCake) public onlyOwner {
       sellTaxes = Taxes(_rfi,_marketing,_liquidity,_CZCake);
        emit FeesChanged();
    }
    ...
    function _takeCZCake(uint256 rCZCake, uint256 tCZCake) private {
        totFeesPaid.CZCake +=tCZCake;

        if(_isExcluded[address(this)])
        {
            _tOwned[address(this)]+=tCZCake;
        }
        _rOwned[address(this)] +=rCZCake;
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rCZCake) = _getRValues2(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.tCZCake = tAmount*temp.CZCake/100;
        s.tTransferAmount = tAmount-s.tRfi-s.tMarketing-s.tLiquidity-s.tCZCake;
        return s;
    }
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rMarketing, uint256 rLiquidity){
        ...
        uint256 rCZCake = s.tCZCake*currentRate;
        rTransferAmount =  rAmount-rRfi-rMarketing-rLiquidity-rCZCake;
        return (rAmount, rTransferAmount, rRfi,rMarketing,rLiquidity);
    }
    
    function _getRValues2(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 rCZCake) {

        if(!takeFee) {
          return(0);
        }

        rCZCake = s.tCZCake*currentRate;
        return (rCZCake);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tMarketing + s.tCZCake);
        }
        if(s.rMarketing > 0 || s.tMarketing > 0) _takeMarketing(s.rMarketing, s.tMarketing);
        if(s.rCZCake > 0 || s.tCZCake > 0) _takeCZCake(s.rCZCake, s.tCZCake);
        emit Transfer(sender, recipient, s.tTransferAmount);
        
    }

    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.marketing + temp.CZCake) * 2;
        ...
        uint256 CZCakeAmt = unitBalance * 2 * temp.CZCake;
        if(CZCakeAmt > 0){
            payable(CZCakeWallet).sendValue(CZCakeAmt);
        }
    }
    ...
    function updateCZCakeWallet(address newWallet) external onlyOwner{
        CZCakeWallet = newWallet;
    }
