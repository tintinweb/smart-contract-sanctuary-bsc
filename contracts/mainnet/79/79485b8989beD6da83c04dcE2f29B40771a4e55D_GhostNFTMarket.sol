...
import './interfaces/IGhostPot.sol';
import './interfaces/IGhostNFT.sol';
...
contract GhostNFTMarket is ERC721Holder, Ownable, ReentrancyGuard {
    ...
    address public ghostPotAddress;
    ...
    uint256 public _treasuryFee = 100;
    uint256 public _ghostPotFee = 150;
     ...
     * @param _ghostPotAddress: address of the ghostPot
    ...
    constructor(
        ...
        require(_ghostPotAddress != address(0), 'Operations: Pot address cannot be zero');
        ...
        ghostPotAddress = _ghostPotAddress;
        WBNB = _WBNBAddress;
    ...
    function _addCollection(
        ...
        require(
            _treasuryFee + _ghostPotFee + _creatorFee + _refererFee <= TOTAL_MAX_FEE,
    ...
    function _buyToken(
        ...
        // Update trading fee
        IERC20(WBNB).safeTransfer(treasuryAddress, treasureFee);

        // Update ghostPot fee
        IERC20(WBNB).approve(ghostPotAddress, ghostPotFee);
        IGhostPot(ghostPotAddress).depositRewards(WBNB, ghostPotFee);

        // Update pending revenues for treasury/creator (if any!)
        if (creatorFee != 0) {
            address _creatorAddress;
            if (mintableCollections[_collection]) {
                _creatorAddress = IGhostNFT(_collection).getCreatorAddress(_tokenId);
            } else {
                _creatorAddress = _collections[_collection].creatorAddress;
            }
            IERC20(WBNB).safeTransfer(_creatorAddress, creatorFee);
        }

        // Update refere fee if not equal to 0
        if (refererFee != 0) {
            IERC20(WBNB).safeTransfer(_refererAddress, refererFee);
        }
        ...
        )
    {
        ...
        ghostPotFee = (_askPrice * _ghostPotFee) / 10000;
        ...
        netPrice = _askPrice - treasureFee - ghostPotFee - creatorFee - refererFee;

        return (netPrice, treasureFee, ghostPotFee, creatorFee, refererFee);
    }
...
interface IGhostPot {
    event Harvest(address indexed from, address indexed token, uint256 value);
    ...
    function updateMoneyPotPeriodNbBlocks() external returns (uint256);

    function lastUpdateMoneyPotBlocks() external returns (uint256);

    function startBlock() external returns (uint256);

    function pendingTokenAmount() external returns (uint256);

    function harvestedTokenAmount() external returns (uint256);

    function ghostSquadLevel(uint256 _tokenId) external returns (uint256);

    function ghostSquadPendingRewards(uint256 _tokenId) external returns (uint256);

    function getTokenAmountFromGhostPot() external view returns (uint256);

    function getPendingReward(address _user) external view returns (uint256);

    function setGhostSquadLevel(uint256 _tokenId, uint256 _level) external;

    function updateGhostSquadLevel(uint256 _tokenId, uint256 _level) external;

    function harvestRewards() external;

    function updateGhostPot() external;

    function depositRewards(address _token, uint256 _amount) external;

    function getNextGhostPotBlock() external view returns (uint256);
}
...
interface IGhostNFT {
    ...
    function storageOwner() external view returns (address);
    ...
    function updateStorage(uint256 _storageFee, address _storageOwner) external;
