...
contract PlotHandler is AccessControl, Pausable {
    ...
    event Rent(address indexed renter, uint plotId, uint nrOfDigs, uint digsLeft);
    event CloseRentAndMint(address renter, address plotOwner, uint plotId, uint digsClosed);
    event SetPlotRent(address owner, uint plotId, uint newRent);
    ...
    function openRentPlot(
        ...
        if(rent < playerMinRent && planetPlot.ownerOf(_plotId) != MoDTaxAccount) {
            rent = playerMinRent;
        }
        ...
        address plotOwner = planetPlot.ownerOf(_plotId);
        if(plotOwner == marketplace) {
            plotOwner = IMarketplace(marketplace).getSellerFor(address(planetPlot), _plotId);
        }

        MoDApproveProxy.DAR_transferFrom(msg.sender, MoDTaxAccount, tax);
        MoDApproveProxy.DAR_transferFrom(msg.sender, plotOwner, rentToPay - tax);
        emit Rent(msg.sender, _plotId, _digsToOpen, left);
    }
    ...
    function closeRentAndMint(
        ...
        address plotOwner = planetPlot.ownerOf(_plotId);
        emit CloseRentAndMint(msg.sender, plotOwner, _plotId, _digsToClose);
    }

    function setPlotRent(uint _plotId, uint _rent) public whenNotPaused(){
        require(_rent >= playerMinRent, "Invalid rent");
        require(msg.sender == planetPlot.ownerOf(_plotId) || IMarketplace(marketplace).getSellerFor(address(planetPlot), _plotId) == msg.sender, "Sender not owner of plot");
        plotState.setRent(_plotId, _rent);

        emit SetPlotRent(msg.sender, _plotId, _rent);
    }
    ...
    function updateMarketplace(address _marketplace) external onlyRole(GAME_CONTROL) {
...
interface IMarketplace {
    function getSellerFor(address _contractAddress, uint _tokenId) external view returns (address);
}
