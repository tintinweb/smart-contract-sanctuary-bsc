...
contract QwertyTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SQWER }

    address public immutable QWER;
    ...
    address public sQWER;
    uint public sQWERQueue; // Delays change to sQWER address
    ...
    /**
        @notice allow approved address to deposit an asset for QWER
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint QWER needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( QWER ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn QWER for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IQWERERC20( QWER ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sQWER ).balanceOf( msg.sender ); // Can only borrow against sQWER held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with QWER
        @param _amount uint
     */
    function repayDebtWithQWER( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IQWERERC20( QWER ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, QWER, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( QWER ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( QWER ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns QWER valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match QWER decimals
            value_ = _amount.mul( 10 ** IERC20( QWER ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SQWER ) { // 9
            sQWERQueue = block.number.add( blocksNeededForQueue );
        } else return false;
