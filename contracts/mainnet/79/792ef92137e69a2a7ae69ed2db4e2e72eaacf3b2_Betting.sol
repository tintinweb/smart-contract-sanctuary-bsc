...
contract Betting is Ownable, Initializable {
    ...
    function tokenBet(address user, uint t, address tokenAddress, bool _supportsBot) internal {
        ...
        } else if (priceOracleMapper[tokenAddress].pairOracle != address(0)) {
            uint priceInPair = PairOracleConsumer(priceOracleMapper[tokenAddress].pairOracle).consult(tokenAddress, uint256(10) ** decimals);
            uint chain_link_decimals = uint256(10) ** ChainlinkPriceConsumer(priceOracleMapper[tokenAddress].chainlinkOracle).getDecimals();
            uint chain_link_price = uint256(ChainlinkPriceConsumer(priceOracleMapper[tokenAddress].chainlinkOracle).getLatestPrice()) * (PRICE_PRECISION) / chain_link_decimals;
            token_usd_price = (priceInPair * chain_link_price) / chain_link_decimals;
            tokenValue = token_usd_price * typePrice[t];
