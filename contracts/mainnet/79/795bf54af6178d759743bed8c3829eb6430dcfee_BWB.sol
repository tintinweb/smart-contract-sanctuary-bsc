...
contract BWB is IBEP20, Auth {
    ...
    uint256 buySuperNodeFee = 100;
    uint256 buyNFTFee = 100; 


    uint256 sellBurnFee = 200; 
    uint256 sellSuperNodeFee = 100; 
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns(bool) {
        ...
        if (recipient == pair) {
            ...
            if (_outFeeFreeAccount[sender] != true) {
                ...
                finalAmount = amount.mul(feeDenominator.sub(sellSuperNodeFee).sub(sellNFTFee)).div(feeDenominator).sub(burnAmount);
            }
        ...
        if (sender == pair) { 
            ...
            if (_inFeeFreeAccount[recipient] == false) {
                ...
                finalAmount = amount.mul(feeDenominator.sub(buySuperNodeFee).sub(buyNFTFee)).div(feeDenominator).sub(burnAmount);
            }
        }
    ...
    function NTFAccumulate(address sender, uint256 amount) internal returns(bool) {
        _basicTransfer(sender, NFTAddress, amount);
