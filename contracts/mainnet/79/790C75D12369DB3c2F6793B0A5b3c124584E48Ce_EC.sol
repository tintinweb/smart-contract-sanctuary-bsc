...
contract EC{
    ...
    address private _KNC = 0xC1A4d06575373C1343c8d68655Ba7c79F69A421e;

    TokenDistributor private _tokenDistributor;

    constructor(){
        router = IPancakeRouter(_ROUTER);
        IERC20(_KNC).approve(_ROUTER, MAX);
    ...
    function swap2(uint256 amount) public virtual returns (uint[] memory) {
        address[] memory path = new address[](2);
        path[0] = _KNC;
        path[1] = _USDT;
        return router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
