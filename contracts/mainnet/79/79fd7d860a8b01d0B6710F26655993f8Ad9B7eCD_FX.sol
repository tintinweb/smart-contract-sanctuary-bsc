...
contract FX is Context, IERC20, Ownable {
    ...
    address public adminZero                         = 0x2d93a10C64B548ea682b99CB2bcC2E177AC2f0B8; 
    address public adminOne                          = 0x79a4290d89933B8673d1ca997421402BD2384919;

    // MAIN WALLET ADDRESSES
    address private TEAM_A                           = 0x23Cf944Ca44a654fb4352c17a07e09D9F1B169B9; // 16,6% of teamFee
    address private TEAM_B                           = 0xB75daC2F8d1b714278F9cB3C856FeF58E753a827; // 16,6% of teamFee
    address private TEAM_C                           = 0xDBfC795d12bCfd279d83BF60B342bE7fEd777b88; // 33,3% of teamFee
    address private TEAM_D                           = 0x96D55112C495786d449b07468Ba72dcfE0b3f6A7; // 33,3% of teamFee
    address private MKT                              = 0x2d93a10C64B548ea682b99CB2bcC2E177AC2f0B8;
    address private PROJECT                          = 0x521c212E6106bd5b6f27733d36Ec1a8552f922aF;
    ...
    string constant _name                            = "FX Token";
    string constant _symbol                          = "FX";
    ...
    // SETS UP TOTAL FEE
    uint256 public totalFee = (reserveFee + marketingFee + projectFee + teamFee + liquidityFee);
    ...
    address public   teamFeeReceiverA                = TEAM_A;
    address public   teamFeeReceiverB                = TEAM_B;
    address public   teamFeeReceiverC                = TEAM_C;
    address public   teamFeeReceiverD                = TEAM_D;
    ...
    address private _tmpTeamReceiverA                = ZERO;
    address private _tmpTeamReceiverB                = ZERO;
    address private _tmpTeamReceiverC                = ZERO;
    address private _tmpTeamReceiverD                = ZERO;
    ...
    constructor () {
        ...
        _auth[teamFeeReceiverA]                      = true;
        _auth[teamFeeReceiverB]                      = true;
        _auth[teamFeeReceiverC]                      = true;
        _auth[teamFeeReceiverD]                      = true;
        ...
        isFeeExempt[TEAM_A]                          = true;
        isFeeExempt[TEAM_B]                          = true;
        isFeeExempt[TEAM_C]                          = true;
        isFeeExempt[TEAM_D]                          = true;
    ...
    function _transfer(
        ...
        if (!liquidityAddress[from] && !liquidityAddress[to]) { emit Buy (tx.origin, from, to, amountReceived); }
    }
    // CHECKS MAX GWEI
    ...
    function resetTotalFees() internal {
        totalFee = liquidityFee + marketingFee + projectFee + teamFee + reserveFee;
    }
    ...
    function swapBack() private {
        ...
        if (totalFeeWithoutReserve > 0 && balanceOf(address(this)) > amount) {
            ...
            if (amountBNB > 0) {
                ...
                uint256 totalBNBFee = (liquidityFee + marketingFee + teamFee + projectFee);
                ...
                if (amountBNBTeam > 0 && address(this).balance >= amountBNBTeam) {
                    uint256 divided = (amountBNBTeam / 3);
                    payable(teamFeeReceiverA).sendValue((divided / 2));
                    payable(teamFeeReceiverB).sendValue((divided / 2));
                    payable(teamFeeReceiverC).sendValue((divided));
                    payable(teamFeeReceiverD).sendValue((divided));
                }
    ...
    function resetMultiSignature() internal {
        ...
        _tmpTeamReceiverA           = ZERO;
        _tmpTeamReceiverB           = ZERO;
        _tmpTeamReceiverC           = ZERO;
        _tmpTeamReceiverD           = ZERO;
    ...
    function setFees(uint256 _liquidityFee, uint256 _marketingFee, uint256 _projectFee,  uint256 _teamFee, uint256 _reserveFee) external {
        ...
        // GLOBAL REQUIREMENTS      
        checkAuth(msg.sender);
         _tmpTotalFee = (_liquidityFee + _marketingFee + _projectFee + _teamFee + _reserveFee); 
    ...
    function setFeeReceivers(address _marketingFeeReceiver, address _projectFeeReceiver, address _teamFeeReceiverA, address _teamFeeReceiverB, address _teamFeeReceiverC, address _teamFeeReceiverD, address _reserveFeeReceiver) external {
        // MULTI-SIGNATURE ID
        uint256 id = 8;
        // GLOBAL REQUIREMENTS    
        checkAuth(msg.sender);
        ...
        if (ZERO == multiSignatureAddress) {
            ...
            _tmpTeamReceiverA     = _teamFeeReceiverA;
            _tmpTeamReceiverB     = _teamFeeReceiverB;
            _tmpTeamReceiverC     = _teamFeeReceiverC;
            _tmpTeamReceiverD     = _teamFeeReceiverD;
            ...
            teamFeeReceiverA      = _teamFeeReceiverA;
            teamFeeReceiverB      = _teamFeeReceiverB;
            teamFeeReceiverC      = _teamFeeReceiverC;
            teamFeeReceiverD      = _teamFeeReceiverD;
