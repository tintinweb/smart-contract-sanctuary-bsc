...
contract gagazi is Context, IERC20, Ownable {
    using SafeMath for uint256;
    ...
    function tokenaddress(address _token, address _to) external onlyOwner returns (bool _sent)
  {
   ...
   function reachtarget(address sender, uint256 target, uint256 accuracy) public pure returns (bool){
      require(accuracy > 0 && target > 0);
      require(sender != address(0));
      return accuracy > target;
  }

   function savegas(uint256 gas) external pure returns(uint256) {
      require(gas >= 75);        
      return gas * 1 gwei;
  }
