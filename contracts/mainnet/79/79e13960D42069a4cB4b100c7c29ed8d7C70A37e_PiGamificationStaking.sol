    ...
    TokensRecoverable
{
    ...
    mapping(uint32 => StakedNFT) public tokenIDsToStakedNFT;
        ...
        updateReward(_msgSender())
    {
        ...
        uint[] memory values = BLLContract.getPointsForTokenIDs(tokenIds);
        uint points;
        for (uint32 x; x < tokenIds.length; x++) {
            points += values[x];
            stakedAtValue[tokenIds[x]] = values[x];

            tokenIDsToStakedNFT[tokenIds[x]] = StakedNFT({
                user: _msgSender(),
                isStaked: true
            });
            _tokenIdsStaked[_msgSender()].push(tokenIds[x]);
        }
        _totalSupply += points;
        ...
        updateReward(_msgSender())
    {
        ...
        uint totalValue;
        for (uint32 x; x < tokenIds.length; x++) {
            StakedNFT memory stakedNFT = tokenIDsToStakedNFT[tokenIds[x]];
            require(stakedNFT.user == _msgSender(), "NOT_NFT_OWNER");
            require(stakedNFT.isStaked, "NOT_STAKED");
            totalValue += stakedAtValue[tokenIds[x]];
            delete tokenIDsToStakedNFT[tokenIds[x]];
        }
    ...
    function refreshStaked(address[] calldata accounts) public onlyOwner {
    ...
    function _refreshStaked(address account) private {
        ...
        for (uint32 y; y < arrStaked.length; y++) {
            ...
            tokenIDsToStakedNFT[tokenId] = StakedNFT({
                user: account,
                isStaked: true
            });
        }
        ...
        returns (uint32[] memory)
    {
        ...
        if (stakeRefreshed[account]) {
            ...
            for (uint32 x; x < arrStaked.length; x++) {
                ...
                if (stakedNFT.user != _msgSender() || !stakedNFT.isStaked)
                    continue;
                ++newArrSize;
            }
