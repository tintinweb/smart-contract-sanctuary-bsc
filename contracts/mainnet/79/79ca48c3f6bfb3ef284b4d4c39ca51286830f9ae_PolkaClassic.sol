...
/**
 *Submitted for verification at Etherscan.io & Contract Audit on 2022-03-07
 */
...
contract PolkaClassic is ERC20("Polka Classic", "DOTC"), Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    uint256 public constant MAX_SUPPLY = 100000000000000 * 1e18; // 1T max supply

    uint16 private MAX_BP_RATE = 10000;
    uint16 private MIN_BP_RATE = 2500;
    uint16 private liquidityTaxRate = 200;
    uint16 private liquiditySellTaxRate = 400;
    uint16 private burnTaxRate = 100;
    uint16 private marketingTaxRate = 500;
    uint16 private marketingSellTaxRate = 500;
    uint16 private buybackTaxRate = 500;
    ...
    address public marketingWallet = 0x92463533486903Ab24Ac89b041Ce63784B5d904b;
    ...
    modifier transferTaxFree() {
        ...
        uint16 _liquidityTaxRate = liquidityTaxRate;
        uint16 _liquiditySellTaxRate = liquiditySellTaxRate;
        uint16 _marketingTaxRate = marketingTaxRate;
        uint16 _marketingSellTaxRate = marketingSellTaxRate;
        uint16 _buybackTaxRate = buybackTaxRate;
        burnTaxRate = 0;
        liquidityTaxRate = 0;
        liquiditySellTaxRate = 0;
        marketingTaxRate = 0;
    ...
    function _transfer(
        ...
        if (_isExcludedFromFee[_sender]) {
            ...
            uint256 liquidityFee = _amount.mul(liquidityTaxRate).div(MAX_BP_RATE);
            uint256 liquiditySellFee = _amount.mul(liquiditySellTaxRate).div(MAX_BP_RATE);
            ...
            if (uniswapPair == _recepient) {
                ...
                super._transfer(_sender, address(this), liquiditySellFee);
                super._transfer(_sender, address(this), marketingSellFee);
                super._transfer(_sender, address(this), buybackFee);
                totalMarketing = totalMarketing.add(marketingSellFee);
                totalLiquidity = totalLiquidity.add(liquiditySellFee);
            } else {
                _amount = _amount.sub(buyTotalFee);
                super._transfer(_sender, _recepient, _amount);
                super._transfer(_sender, address(this), liquidityFee);
                super._transfer(_sender, address(this), marketingFee);
                totalMarketing = totalMarketing.add(marketingFee);
                totalLiquidity = totalLiquidity.add(liquidityFee);
            }
            totalBuybacks = totalBuybacks.add(buybackFee);
        }
    }
    ...
    function manualWithdraw() external onlyOperator {
        ...
        uint256 totalTokensToSwap = totalLiquidity + totalMarketing + totalBuybacks;

        if(contractTokenBalance == 0 || totalTokensToSwap == 0) {return;}
        uint256 liquidityTokens = contractTokenBalance * totalLiquidity / totalTokensToSwap / 2;
        uint256 amountToSwapForETH = contractTokenBalance.sub(liquidityTokens);
        uint256 initialETHBalance = address(this).balance;
        swapTokensForEth(amountToSwapForETH); 
        uint256 ethBalance = address(this).balance.sub(initialETHBalance);
        uint256 ethForMarketing = ethBalance.mul(totalMarketing).div(totalTokensToSwap);
        uint256 ethForBuyback = ethBalance.mul(totalBuybacks).div(totalTokensToSwap);
        ...
        uniswapRouter.swapExactETHForTokensSupportingFeeOnTransferTokens{value: ethForBuyback}(
            0,
            path,
    ...
    function updateFees(
        ...
        uint16 _liquiditySellTaxRate,
        uint16 _marketingTaxRate,
        uint16 _marketingSellTaxRate
    ) external onlyOwner {
        require(
            _buybackRate + _burnTaxRate + _liquidityTaxRate + _marketingSellTaxRate + _liquiditySellTaxRate + _marketingTaxRate <=  MIN_BP_RATE,
            "!values"
        );
      
        buybackTaxRate = _buybackRate;
        burnTaxRate = _burnTaxRate;
        liquidityTaxRate = _liquidityTaxRate;
        liquiditySellTaxRate = _liquiditySellTaxRate;
        marketingTaxRate = _marketingTaxRate;
        ...
        returns (uint256)
    {
        require(
            ...
            "POLKA::getPriorVotes: not yet determined"
        );
