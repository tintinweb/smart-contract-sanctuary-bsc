...
Notes:
 - Enable Optimization while compiling the contract
...
/*Change values below as required - only commented*/
 ...
 contract LunarEclipse is Context, IERC20, Ownable { //Contract Name
    using Address for address payable;

    string private constant _name = "LunarEclipse"; //Token Name
    string private constant _symbol = "LEC"; //Token Symbol
    uint8 private constant _decimals = 9;

    uint256 private constant MAX = ~uint256(0);

    uint256 private initialsupply = 100000000 * (10 ** _decimals); //Total Supply
	uint256 private _tTotal = initialsupply; 
    uint256 private _rTotal = (MAX - (MAX % _tTotal));

    uint256 public maxBuyLimit = (initialsupply * 1000) / 1000; //Max Buy Amount (10 means 1%)
    uint256 public maxSellLimit = (initialsupply * 1000) / 1000;  //Max Sell Amount (5 means 0.5%)
    uint256 public maxWalletLimit = (initialsupply * 1000) / 1000; //Max Wallet (20 means 2%)
    
    //Set wallets below 
    address private marketingWallet = 0x0EFF85c3cFFEd7E9110c5CE8b3bDd94f92052d0E; //Marketing Wallet
    address private teamWallet = 0x0EFF85c3cFFEd7E9110c5CE8b3bDd94f92052d0E; //Team Wallet
    ...
    Taxes public buyTaxes = Taxes(1, 0, 0, 1); //Buy Taxes (Reflection, Marketing, Liquidity, Team)
    Taxes public sellTaxes = Taxes(1, 0, 0, 1); //Sell Taxes (Reflection, Marketing, Liquidity, Team)
...
/* Dont change the values below this line unless you know what you are doing! */
    ...
    //Enable trading after adding Liquidity using the setTradingStatus function to set the correct genesis block
    ...
    /*function maxTXPercentage1000*/     
    function updateMaxTxPercent(uint256 maxBuy, uint256 maxSell) external authorized(){
        maxBuyLimit = maxBuy / 1000 * totalSupply();
        maxSellLimit = maxSell / 1000 * totalSupply();
    }
    ...
    /*function maxWalletPercentage1000*/     
    function updateMaxWalletPercent(uint256 amount) external authorized(){
        maxWalletLimit = amount / 1000 * totalSupply();
    }
