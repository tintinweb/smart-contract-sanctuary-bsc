...
contract USDCoinTest is ERC20Detailed, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;

    // USDCoinTest Settings
    uint256 private constant MAX_REBASE_FREQUENCY = 1800;
    uint256 public usddRebase = 4000; // APY: 69% APY
    uint256 public usddRebaseDenominator = 100000000;
    ...
    uint256 public buyliquidityFee = 1; // $USDD Liquidity Pool Growth
    ...
    uint256 public buyfirepitfee = 0; // Firepit: Used to burn $USDD tokens out of the supply
    uint256 public totalBuyFee = buyliquidityFee.add(buyyieldFee).add(buytreasuryFee).add(buyfirepitfee);

    // Default Sell Tax Fees = 8%
    uint256 public sellliquidityFee = 2; // $USDD Liquidity Pool Growth
    ...
    uint256 public sellfirepitfee = 2; // Firepit: Used to burn $USDD tokens out of the supply
    ...
    uint256 public referee = 1; // $USDD Liquidity Pool Growth
    ...
    address public yieldtoptreasuryReceiver = 0x738BD1f54831022b70F917D24bEB938CfBC3A3d1;
    address public usddtreasuryreservesReceiver = 0x738BD1f54831022b70F917D24bEB938CfBC3A3d1;
    address public usddliquidityReceiver = 0x738BD1f54831022b70F917D24bEB938CfBC3A3d1;
    address public usddfirepitReceiver = 0x738BD1f54831022b70F917D24bEB938CfBC3A3d1;
    ...
    constructor() ERC20Detailed("USDCoinTest", "USD", uint8(DECIMALS)) {
        ...
        isFeeExempt[usddtreasuryreservesReceiver] = true;
    ...
    function swapBack() internal swapping {
        ...
        if(amountToYieldTop > 0){
            _swapTokensForBusd(amountToYieldTop, usddtreasuryreservesReceiver);
        }
        ...
        if(amountToTreasury > 0){
         ...
         if(amountToFirepit > 0){
            _swapTokensForBNB(amountToFirepit, usddfirepitReceiver);
        }
    ...
    function _rebase() private {
        if(!inSwap) {
            ...
            int256 supplyDelta = int256(circulatingSupply.mul(usddRebase).div(usddRebaseDenominator));
    ...
    function setFeeReceivers(address setusddliquidityReceiver, address setyieldtoptreasuryReceiver, address setusddtreasuryreservesReceiver, address setusddfirepitReceiver) external onlyOwner {
        usddliquidityReceiver = setusddliquidityReceiver;
        yieldtoptreasuryReceiver = setyieldtoptreasuryReceiver;
        usddtreasuryreservesReceiver = setusddtreasuryreservesReceiver;
        usddfirepitReceiver = setusddfirepitReceiver;
    }
    ...
    function setUsddRebase(uint256 enterRebase, uint256 rebaseDenominator) external onlyOwner {
        require(rebaseDenominator >= 1, "Rebase Denominator can't be set to 0");
        usddRebase = enterRebase;
        usddRebaseDenominator = rebaseDenominator;
         emit SetUsddRebase(usddRebase, usddRebaseDenominator);
    }
    ...
    event SetUsddRebase(uint256 usddRebase, uint256 usddRebaseDenominator);
