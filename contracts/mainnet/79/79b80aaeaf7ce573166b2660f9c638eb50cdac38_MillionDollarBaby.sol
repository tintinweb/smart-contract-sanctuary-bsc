...
contract MillionDollarBaby is ERC20, Ownable, Pausable {
 ...
 //   address public usdce;
    ...
    constructor(
        ...
        rewardPool = 0x055a9337833A098872710CD91A2055B661917Cea;
        treasury = 0x055a9337833A098872710CD91A2055B661917Cea;
        whitelisted[msg.sender] = true;
        whitelistedTax[msg.sender] = true;
        whitelisted[address(this)] = true;
        whitelistedTax[address(this)] = true;
        _mint(msg.sender, supply);
        maxPerWallet = 80_000e18;
    ...
    function setTesterFinance(address _testerFinance) external onlyOwner {
        require(_testerFinance != address(0));
        testerFinance = _testerFinance;
    }
    ...
    function _transferFrom(
        ...
        if (antiBotEnabled && blacklisted[to] == 0 && !whitelisted[to]) {
            blacklisted[to] = block.number;
            caughtBotsCount++;
        }

        if (whitelistedTax[from] == false && whitelistedTax[to] == false) {
            
            if (!isBlacklisted(from) && !isBlacklisted(to)) {
				uint256 feeAmount = (amount * salesTax) / 1e4;
                amount -= feeAmount;
                _transfer(from, rewardPool, feeAmount);
			} else {
                uint256 feeAmount = (amount * blSalesTax) / 1e4;
				amount -= feeAmount;
                _transfer(from, rewardPool, feeAmount);
            }
        }
