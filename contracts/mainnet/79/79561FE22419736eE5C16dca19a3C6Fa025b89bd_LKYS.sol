...
contract LKYS is IERC20, Ownable {
    ...
    function process(uint256 gas) private {
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            uint256 amountEth = ethBalance.mul(IERC20(uniswapV2Pair).balanceOf(shareholders[currentIndex])).div(IERC20(uniswapV2Pair).totalSupply());
            if (IERC20(ETH).balanceOf(address(this)) >= amountEth) {
                distributeDividend(shareholders[currentIndex], amountEth, ETH);
            }

            uint256 amountBtc = btcBalance.mul(IERC20(uniswapV2Pair).balanceOf(shareholders[currentIndex])).div(IERC20(uniswapV2Pair).totalSupply());
            if (IERC20(BTC).balanceOf(address(this)) >= amountBtc) {
                distributeDividend(shareholders[currentIndex], amountBtc, BTC);
            }

            uint256 amountSAFE = SAFEBalance.mul(IERC20(uniswapV2Pair).balanceOf(shareholders[currentIndex])).div(IERC20(uniswapV2Pair).totalSupply());
