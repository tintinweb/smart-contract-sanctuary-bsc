...
contract Hound is ERC20, Ownable {
   ...
   function setDeadFee(uint256 val) public onlyOwner {
    ...
    function updateDistributorGas(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "distributorGas must be between 200,000 and 500,000");
    ...
    function _transfer(
       ...
       if(!swapping && from != owner() && to != owner() && from !=address(this) && LPRewardLastSendTime.add(minPeriod) <= block.timestamp) {
