...
contract StandardToken is BEP20 {
    constructor(
        ...
        _mint(_owner,_tokenSupply*10**18);
...
contract BurnableToken is BEP20 {
    constructor(
        string memory _tokenName,
        string memory _tokenSymbol,
        uint256 _tokenSupply,
...
contract NftBook is Ownable {
    ...
    mapping(address => mapping(uint256=>uint256)) public deployedTimes;
  ...
  function withdrawRemainingToken(IBEP20 token,address _recipient) public onlyOwner {
    ...
    function deployNewPool(
        ...
        deployedPools[msg.sender][counter[msg.sender]]=_nftAddress;
        deployedTimes[msg.sender][counter[msg.sender]]=block.timestamp;

        PoolRegistered(_nftAddress,
        _earningTokenAddress,
        _rewardPerDay,_tokenlogo,_nftlogo,_gatewayurl);
