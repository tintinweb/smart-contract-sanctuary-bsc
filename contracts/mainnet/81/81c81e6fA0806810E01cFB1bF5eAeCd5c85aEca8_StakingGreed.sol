...
contract StakingGreed is RewardsDistributionRecipient, ReentrancyGuard, Pausable  {
    ...
    function stake(uint256 amount) external nonReentrant whenNotPaused updateReward(msg.sender) {
        ...
        require(IGreed(stakingToken)._isExcludedFromFees(address(this)), "NOT_FEES_EXCLUDED");
    ...
    function withdraw(uint256 amount) internal nonReentrant updateReward(msg.sender) {
        ...
        if(block.timestamp < lockingPeriodStaking[msg.sender]){
            ...
            IERC20(stakingToken).safeTransfer(devFundAdd, devFee);
            uint256 stakingFee = amount * (stakingPoolFeeWithdraw) / (100000); // feeWithdraw = 100000 = 100%
            IERC20(stakingToken).safeTransfer(stakingPoolFeeAdd, stakingFee);
            uint256 remAmount = amount - (devFee) - (stakingFee);
            IERC20(stakingToken).safeTransfer(msg.sender, remAmount);
        }
        else    
...
interface IGreed
{
    function _isExcludedFromFees(address _address) external returns(bool);
}
