...
contract GlobalChainDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("GlobalChain_Dividend_Tracker", "GlobalChain_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "GlobalChain_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "GlobalChain_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main global chain contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "GlobalChain_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "GlobalChain_Dividend_Tracker: Cannot update claimWait to same value");
...
contract GlobalChain is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    GlobalChainDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Global Chain", "GBC") {
        BNBRewardsFee = 1;
        liquidityFee = 1;
        extraFeeOnSell = 0; // extra fee on sell
        marketingFee = 1;
        marketingWallet = payable(0xD0738F306f546cc319cBcAec3b1E214609f407A2);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new GlobalChainDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "GlobalChain: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "GlobalChain: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "GlobalChain: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "GlobalChain: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "GlobalChain: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "GlobalChain: Cannot update gasForProcessing to same value");
