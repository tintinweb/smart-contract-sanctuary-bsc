...
contract DOTC is IERC20, Ownable {
    ...
    // 设置成分红的合约地址
    address public nodeAddress = 0xd0A152ccB3bc21B6f3b6CA115e9b42EDbA6bF3b3;

    // 钱包地址
    address public marketAddress = 0xd0A152ccB3bc21B6f3b6CA115e9b42EDbA6bF3b3;
    ...
    function _burn(address account, uint256 amount) internal virtual {
        _tOwned[account] = _tOwned[account].sub(amount); //Subtract from the sender

        _tTotal = _tTotal.sub(amount); // Updates
    ...
    function setWhiteKG(bool trueOrFalse) public onlyOwner {
        WhiteList = trueOrFalse;
    }
    ...
    function _takenNodeFee(address sender, uint256 tAmount) private {
        if (_nodeFee == 0) return;

        _tOwned[nodeAddress] = _tOwned[nodeAddress].add(tAmount);

        emit Transfer(sender, nodeAddress, tAmount);
    }

    function _takenBurnFee(address sender, uint256 tAmount) private {
        if (_burnFee == 0) return;

        _tOwned[address(1)] = _tOwned[address(1)].add(tAmount);

        emit Transfer(sender, address(1), tAmount);
    }
    ...
    function _transferStandard(
        ...
        _takeLPFee(sender, tAmount.div(20000).mul(_LPFee));

        _takenNodeFee(sender, tAmount.div(10000).mul(_nodeFee));

        _takenBurnFee(sender, tAmount.div(10000).mul(_burnFee));
