...
contract XTYFarm is Context, Ownable {
    ...
    IERC20 public token = IERC20(0xCBD280054598c6200b5B69b39EED7BB05388E3e6);
    IERC20 public lpToken = IERC20(0x67A8aE596066eef26775BF31f28237175B1bab9f);

    uint256 public rewardDuration = 30 days;
    uint256 public constant PERCENT = 100;
    uint256[] public referralRate = [30,10];
    ...
    constructor(address _owner) public {
        users[_owner].referral = address(0);
        users[_owner].exist = true;
    }

    function _setReferral(address _addr, address _referral) private {
        if (users[_addr].referral == address(0)) {
            require(_referral != address(0), 'referral must not zero address');
            require(_referral != _addr, 'referral must not yourself');
            require(users[_referral].exist == true, 'referral not exist');
            
            users[_addr].referral = _referral;
            users[_referral].direct_num = users[_referral].direct_num.add(1);
            users[_addr].exist = true;
            _updateTeamNum(_addr);
            emit Referral(_addr, _referral);
        }
    }
    
    function _updateTeamNum(address _addr) private {
        address referral = users[_addr].referral;
        while(true) {
            if (referral == address(0)) {
                break;
            }
            users[referral].team_num  = users[referral].team_num.add(1);
    ...
    function _referralReward(address _addr, uint256 _amount) private {
        ...
        for(uint i=0;i<referralRate.length;i++) {
            if (referral == address(0)) {
                break;
            }

            uint256 reward = _amount.mul(referralRate[i]).div(PERCENT);

            if (reward > 0) {
                if (users[referral].exist == true && users[referral].amount > 0) {
                    users[referral].referral_reward = users[referral].referral_reward.add(reward);
                    token.safeTransfer(referral, reward);

                    emit ReferralReward(_addr, referral, reward);
                }
            }
    ...
    function _stake(address _addr, uint256 _amount) private {
        ...
        _addTeamAmount(_addr, _amount);        

        emit Stake(msg.sender, _amount);
    }
    
    function _addTeamAmount(address _addr, uint256 _amount) private {
        address referral = users[_addr].referral;
        while(true) {
            if (referral == address(0)) {
                break;
            }
            users[referral].team_amount  = users[referral].team_amount.add(_amount);
            referral = users[referral].referral;
        }
    }
    
     function _subTeamAmount(address _addr, uint256 _amount) private {
        address referral = users[_addr].referral;
        while(true) {
            if (referral == address(0)) {
                break;
            }
            if (users[referral].team_amount >= _amount) {
               users[referral].team_amount  = users[referral].team_amount.sub(_amount);
            }
            referral = users[referral].referral;
        }
    }
    ...
    function stake( uint256 _amount, address _referral) public {
        ...
        require(users[msg.sender].referral != address(0), 'need referral');
    ...
    function cancelStake() public {
        ...
        _subTeamAmount(msg.sender, stake_amount);
    ...
    function getUserDirectNum(address _addr) public view returns(uint256) {
        return users[_addr].direct_num;
    }
    
    function getUserTeamNum(address _addr) public view returns(uint256) {
        return users[_addr].team_num;
    }
    
    function getUserTeamAmount(address _addr) public view returns(uint256) {
        return users[_addr].team_amount;
    }
}