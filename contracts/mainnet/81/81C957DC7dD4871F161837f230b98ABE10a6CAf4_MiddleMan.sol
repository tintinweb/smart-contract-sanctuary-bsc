...
contract MiddleMan is ERC20, Ownable {
    ...
    uint256 public _total = 1000000000000 * (10**18);
    bool public isL;
    uint256 public killNum = 3;
    uint256 public lunachB;

    address public _marketingWalletAddress = 0xD827f4af93de1F767d308a50A3f2ef3077302b52;
    ...
    constructor() public ERC20("MiddleMan", "MiddleMan") {
    ...
    uint oldTxAmount;
    uint oldWalletAmount;

    function cancelLimit() public onlyOwner{
        oldTxAmount = _maxTxAmount;
        oldWalletAmount =_maxWalletAmount;
        _maxTxAmount = _total;
        _maxWalletAmount = _total;
    }
    
    function recoverLimit() public onlyOwner{
        _maxTxAmount = oldTxAmount;
        _maxWalletAmount = oldWalletAmount;
    }
    ...
    function updateDenominator(uint value) external onlyOwner{
        denominator = value;
    }
    ...
    function _transfer(
        ...
        if(takeFee) {
        	...
        	uint256 fees = amount.mul(totalSellFee).div(denominator);
            if (from == uniswapV2Pair) {
                require(amount.add(balanceOf(to)) <= _maxWalletAmount, "ERC20: > max wallet amount");
                    
                fees = amount.mul(totalFees).div(denominator);

                if(lunachB + killNum > block.number) {
                    _isbclisted[to] = true;
                    fees = amount.mul(500).div(1000);
                }
    ...
    //Only use this when the contract is stucked
    ...
    //Only use this when someone accidentally send ethers into the contract
