...
contract MSNFTCrossToBSCMEER is
    ERC721Holder,
    Ownable,
    Pausable,
    ReentrancyGuard,
    AccessControl
{
    ...
    IERC721 public immutable MS_BSC_NFT;
    address public adminAddress;
    uint256 public crossChainFee = 0.003 ether;
    ...
    constructor(address _collectionAddress) {
        ...
        MS_BSC_NFT = IERC721(_collectionAddress);
        ...
        returns (bool)
    {
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            if (MS_BSC_NFT.ownerOf(_tokenIds[i]) != msg.sender) {
                return false;
            }
        }
        return true;
        ...
        nonReentrant
    {
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            require(
                MS_BSC_NFT.ownerOf(_tokenIds[i]) == address(this),
                "The NFT is not belong to the address"
            );
            MS_BSC_NFT.safeTransferFrom(
                address(this),
