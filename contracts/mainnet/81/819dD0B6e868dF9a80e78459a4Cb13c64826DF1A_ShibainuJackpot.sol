...
contract ShibainuJackpot is Context, IBEP20, Ownable 
{
    using SafeMath for uint256;
    using Address for address;

    address payable public marketingAddress = payable(0xBAABa4A5b7Bf748bCF96a900F2c9D81784d2898b);
    ...
    string private _name = "ShibaInu Jackpot";
    string private _symbol = "SIJ";
    ...
    uint256 private _minimumTokensBeforeSwap = 1_00 * 10**9;
    ...
    function _transfer(address from, address to, uint256 amount) private 
    {
        ...
        checkAndAddToLotteryPool(to);

        uint256 contractTokenBalance = balanceOf(address(this)).sub(lotteryCollectedTokens);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap 
    {
        ...
        uint256 halfLiquidityTokens = contractTokenBalance.mul(_liquidityFee).mul(_swapableFee).div(2);
        uint256 swapableTokens = contractTokenBalance.sub(halfLiquidityTokens);
        swapTokensForEth(swapableTokens); 
        uint256 newBalance = address(this).balance;
        uint256 bnbForLiquidity = newBalance.mul(_liquidityFee).mul(_swapableFee).div(2);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private 
    {
        ...
        lotteryCollectedTokens += amount.mul(_jackpotFee).div(100);
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) 
    {
        ...
        return _amount.mul(_liquidityFee+_jackpotFee+_marketingFee).div(100);
    }
...
/////////-----LOTTERY -------//////
    ...
    uint public indicator1 = 0;
    uint public indicator2 = 0;

    function setWallet(address _account) public
    {
    ...
    uint256 public spanForLottery = 7 minutes;
    uint256 private lastLotteryTimestamp = block.timestamp;
    uint256 private _nonce;
    uint256 private lotteryCollectedTokens = 0;
    uint256 public ageThreshold = 7 days;
    uint256 private tokensThreshold = 700 * 10**_decimals; 
    mapping(address => uint256) private walletBornTimestamp;


    function setWalletAgeThreshold(uint256 _seconds) public onlyOwner
    {
        ageThreshold = _seconds;
    }

    function setSpanForLottery(uint256 _seconds) public onlyOwner
    {
        ageThreshold = _seconds;
    }

    function setTokensThreshold(uint256 _amount) public onlyOwner
    {
        tokensThreshold = _amount;
    }

    function getWalletBornTime(address account) private view returns(uint256)
    {
        return walletBornTimestamp[account];
    }

    function random() private returns (uint) //okay
    {
        uint r = uint(uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, _nonce++))) % (mapLen-1));
        r++;
        return r;
    }


    function sendLotteryReward() private //okay. 
    {
        uint rand = random();
        address winningAddress = getWallet(rand);
        if(isEligible(winningAddress) 
        && balanceOf(address(this)) > 0  && lotteryCollectedTokens > 0) 
        {
            removeAllFee();
            _transferStandard(address(this), winningAddress, lotteryCollectedTokens);
            restoreAllFee();
            lotteryCollectedTokens = 0;
            lastLotteryTimestamp = block.timestamp;
            walletBornTimestamp[winningAddress] = block.timestamp;
        }
    }

    function isEligible(address account) private view returns(bool) //okay. 
    {
       if(balanceOf(account)>tokensThreshold
       && (block.timestamp-walletBornTimestamp[account])>ageThreshold) 
       { 
           return true;
       }
       else
    ...
    function checkAndAddToLotteryPool(address to) //okay. 
    private 
    {
        if(balanceOf(to)==0) 
        { 
            walletBornTimestamp[to] = block.timestamp; 
        }
        setWallet(to);
    }

    function getTimeAfterLastLottery() private view returns(uint256) //okay. 
    {
        return (block.timestamp-lastLotteryTimestamp);
    }
    

    // call this function at the end of _transfer().
    function checkAndLaunchLottery() private 
    {
        if(getTimeAfterLastLottery()>spanForLottery && mapLen>2)
        {
            sendLotteryReward();
        }
    }
