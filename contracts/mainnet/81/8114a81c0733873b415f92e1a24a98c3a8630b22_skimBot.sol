...
contract skimBot is  Ownable {
    ...
    function goodOnesLength() public view returns(uint256) {
        return goodOnes.length;
    }

    function addLP(address _newLP) external {
        require(!_isListed[_newLP], "LP already added" );
                _isListed[_newLP] = true;
                goodOnes.push(_newLP);    
    }

    function removeLP(address _oldLP) external {
        require(_isListed[_oldLP], "LP not available to remove");
        for (uint256 i = 0; i < goodOnes.length; i++) {
            if (goodOnes[i] == _oldLP) {
                goodOnes[i] = goodOnes[goodOnes.length - 1];
                delete _isListed[_oldLP];
                goodOnes.pop();
            }
        }
    }
    ...
    function _SkimIt(address LPAddress) external {
            IFarmageddonPair LP = IFarmageddonPair(LPAddress);
            LP.skim(treasury);
    }
    ...
    function _checkFromGoodOnes(uint _j) external view returns (uint256 BNBraw, uint256 BUSDraw, address LPAddress){
           uint amount0 = 0;
           uint amount1 = 0;
           uint256 bnb = 0;
           uint256 busd = 0;
    ...
    function _takeFromGoodOnes(uint j) external {
            address pair = goodOnes[j];
            address token0 = IFarmageddonPair(pair).token0();
            address token1 = IFarmageddonPair(pair).token1();
    ...
    function _checkOneLP(address _j) external view returns (uint256 BNBraw, uint256 BUSDraw){
            uint amount0 = 0;
           uint amount1 = 0;
           uint256 bnb = 0;
           uint256 busd = 0;
    ...
    function CheckABunch(uint start, uint howMany, address factory) external view returns (uint256 BNBraw, uint256 BUSDraw, address LPAddress){
      
        IFactory FA = IFactory(factory);
        for (uint i = start; i < start + howMany; i++) {
           uint amount0 = 0;
           uint amount1 = 0;
           uint256 bnb = 0;
           uint256 busd = 0;
