...
contract CEEKLandGate is ERC165, ReentrancyGuard, Context, IERC721TokenReceiver {
    ...
    EIP20 public contract_payment_token;
    ...
    // Mapping from token ID to sell price in contract_payment_token EIP20 token
    mapping(uint256 => uint256) internal sellBidPrice;
    ...
    event OnSale(uint256 indexed tokenId, uint256 indexed price, address indexed wallet);
	...
	constructor(
        address _owner, EIP20 _payment_token, EIP20[] memory _payment_tokens, 
         ...
         address _wbnbToken)
    {
        ...
        contract_payment_token = _payment_token;
    ...
    function getExchangeRate(address currency) public view returns (uint256, uint256) {
        address pairAddress = IUniswapV2Factory(swapFactory).getPair(address(contract_payment_token), currency);
        ...
        if (token0 == address(contract_payment_token)) {
    ...
    function getExchangeRates() public view returns (ExchangeRate memory) {
        ...
        address pairAddress;
        address token0;
        bytes memory resultReserves;
        bytes memory resultToken0;

        j=0;
        for (i=0; i<payment_tokens.length; i++) {
            if (address(payment_tokens[i]) == address(contract_payment_token)) {
                continue;
            }

            pairAddress = IUniswapV2Factory(swapFactory).getPair(address(contract_payment_token), address(payment_tokens[i]));

            if (pairAddress == address(0x0000000000000000000000000000000000000000)) {
                continue;
            }

            resultReserves = Address.functionStaticCall(pairAddress, abi.encodeWithSignature("getReserves()"), "CEEKLANDGATE_030");

            (reserve0, reserve1,) = abi.decode(resultReserves, (uint112,uint112,uint32));

            resultToken0 = Address.functionStaticCall(pairAddress, abi.encodeWithSignature("token0()"), "CEEKLANDGATE_031");
            ...
            if (token0 == address(contract_payment_token)) {
                exRates.contract_payment_token_reserve[j] = reserve0;
                exRates.payment_token_reserve[j] = reserve1;
            } else {
    ...
    function checkIsOwner(uint256 tokenId, bytes memory ceekUserIdParam) private {
        ...
        checkIsBlocked(ceekUserId, _msgSender());
    }
    ...
    function checkIsBlocked(string memory ceekUserId, address walletAddress) public view {
        bool blocked = false;
        uint256 i;
        if (blockedCeekUser.length > 0) {
            for(i=0;i<blockedCeekUser.length;i++) {
                if (keccak256(abi.encodePacked((blockedCeekUser[i]))) == keccak256(abi.encodePacked((ceekUserId)))) {
                    blocked = true;
                    break;
                }
            }
        ...
        if (blockedAddress.length > 0) {
           for(i=0;i<blockedAddress.length;i++) {
               if (address(blockedAddress[i]) == walletAddress) {
                   blocked = true;
                   break;
               }
            } 
    ...
    function sell(uint256 tokenId, uint256 price, address payable wallet, bytes memory ceekUserIdParam, address[] memory paymentTokenNft, string[] memory _allowedToBuyCeekUserId, address[] memory _allowedToBuyWallet) public {	
        ...
        if (price > 0) {
            ...
            emit OnSale(tokenId, price, _wallets[tokenId]);
        } else {
            emit OnSale(tokenId, price, address(0));
        }
    }
		
    function beforeBuyCheck1(uint256 tokenId, uint256 tokenPrice, address paymentTokenAddress) private view {
        uint256 i;
        bool ptOk1 = false;
        bool onSale = false;
        ...
        if (sellBidPrice[tokenId] > 0) {
            ...
            require(sellBidPrice[tokenId]==tokenPrice, "CEEKLANDGATE_032");
        }
    ...
    function beforeBuyCheck2(uint256 tokenId, uint256 tokenPrice, bytes memory ceekUserIdParam, address paymentTokenAddress) private returns(address, string memory, Currency memory) {
        ...
        if (sellBidPrice[tokenId] > 0) {
            ...
            onSale = true;
            saleBidPriceCurrency.payment_token[0] = address(contract_payment_token);
            saleBidPriceCurrency.value[0] = sellBidPrice[tokenId];
        }
    ...
    function getPriceInCurrency(uint256 tokenId, EIP20 payment_token) public view returns (uint256) {
        ...
        if (payment_token == contract_payment_token) {
            return sellBidPrice[tokenId];
        } else {
            (uint256 exRates1, uint256 exRates2) = getExchangeRate(address(payment_token));

            token_price = sellBidPrice[tokenId].mul(exRates1).div(exRates2);
    ...
    // Buy option
    function buy(uint256 tokenId, uint256 price, bytes memory ceekUserIdParam, EIP20 payment_token) public payable nonReentrant {
        uint256 token_price;
        
        beforeBuyCheck1(tokenId, price, address(payment_token));
        
        if (payment_token == contract_payment_token) {
            token_price = sellBidPrice[tokenId];
        } else {
            (uint256 exRates1, uint256 exRates2) = getExchangeRate(address(payment_token));
        ...
        if (address(payment_token) == wbnbToken) {
            ...
            // to owner
            require(payment_token.transferFrom(msg.sender,_wallets[tokenId], amount4owner), "CEEKLANDGATE_022");
    ...
    function updateAdmin(EIP20 _payment_token, EIP20[] memory _payment_tokens, IERC721 _contract_ceekland,
                         address _contract_decoder, address payable _admin, address payable _ceekDepositAdmin,
    ...
    function getPrice(uint256 tokenId, string memory groupId, uint256 coord_land, uint256 coord_row, uint256 coord_col) public view returns (uint256, uint256, Currency memory) {
		if (tokenId == 0) {
		...
		if (sellBidPrice[tokenId] > 0) {
            ...
            return (sellBidPrice[tokenId], 0, emptyCurrencyArr);
        }
        
		return (0, 0, soldFor[tokenId]);
    }

    function setBlockedUsers(string[] memory _blockedCeekUser, address[] memory _blockedAddress) public {
        require(_msgSender()==contract_owner, "CEEKLANDGATE_024");
        blockedCeekUser = _blockedCeekUser;
