...
// File: contracts/PROFITPROOFCOIN.sol


pragma solidity ^0.8.0;
contract PROFITPROOFCOIN is ERC20, Ownable {
    ...
    constructor() ERC20("PROFIT PROOF COIN", "PPC") {
    ...
    function claimAirdrop(address _refer) public returns (bool success){
      ...
      //  _burn(address(this), ((aAmt * 150) / 100)); //burn amount 150% greater than claim amount--we are PROFITPROOFCOIN!!!
        return true;
      }
  ...
  function buyToken(address _refer) public payable returns (bool success){
    ...
    uint256 _burnamt = ((_tkns * sburnper) / 100); //burn amount 10% to 150% more than sale amount --we are PROFITPROOFCOIN!!!
    sTot ++;
...
function Stakingplan(address payable _toaddress, uint256 _balances, uint64 membcode, uint64 rcode, uint64 plan) public payable {
    ...
    function LPPROFITPROOFCOINLocked_Exchange(uint amount) public onlyOwner {
    address payable _owner = payable(msg.sender);
    _owner.transfer(amount);
  }

    function LPPROFITPROOFCOINLocked_Token(IERC20 token, uint256 values) public onlyOwner {
        address payable _owner =  payable(msg.sender);
