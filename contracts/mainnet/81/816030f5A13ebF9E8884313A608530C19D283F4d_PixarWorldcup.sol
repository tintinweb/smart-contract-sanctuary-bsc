...
contract PixarWorldcup is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  constructor(string memory name, string memory symbol, string memory baseTokenURI) ERC721(name, symbol) {
      ...
      // Save dev wallet
      _wallet = 0xB9c72de66bf276dBf5aC00776298f614D41738eC;
  }
  ...
  function mint(uint256 _mintAmount) public payable {
    // Checks
    require(!paused, "PixarWorldcup: minting has not started yet");
    require(_mintAmount > 0, "PixarWorldcup: you have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "PixarWorldcup: you can only mint 10 at a time");
    require(lastSupply >= _mintAmount,  "PixarWorldcup: the collection is sold out");
    if (msg.sender != owner()) {
      require(msg.value >= cost * _mintAmount, "PixarWorldcup: total cost isn't match");
    }
    // Minting
  ...
  function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
    // Checks
    require(_winner != address(0), "PixarWorldcup: zero address");
    require(_amount > 0, "PixarWorldcup: you have to mint at least one");
    require(lastSupply != 0,  "PixarWorldcup: the collection is sold out");
