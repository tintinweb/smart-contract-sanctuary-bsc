...
contract ShibaGo is ERC20, Ownable {
    ...
    ShibaGoDividendTracker public dividendTracker;

    address public liquidityWallet;

    address payable public operations1Address =
        payable(0xaE5175e32899F26250e0a6b3504a6dFb8F48Fbcf);
    ...
    uint256 public SHIBARewardsBuyFee = 5;
    uint256 public SHIBARewardsSellFee = 5;
    ...
    event UpdateFees(
        uint256 operations,
        uint256 liquidity,
        uint256 SHIBARewardsBuy,
        uint256 SHIBARewardsSell
    );
    ...
    constructor() ERC20("ShibaGo", "ShibaGo") {
        dividendTracker = new ShibaGoDividendTracker(payable(this));
        ...
        totalSellFees = SHIBARewardsSellFee.add(liquidityFee).add(operationsFees);
        totalBuyFees = SHIBARewardsBuyFee.add(liquidityFee).add(operationsFees);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        ShibaGoDividendTracker newDividendTracker = ShibaGoDividendTracker(
            payable(newAddress)
        );
    ...
    function updateFees(
        ...
        require(
            SHIBARewardsBuy >= 1 && SHIBARewardsBuy <= 50,
            "SHIBA reward fees must be between 1 and 50"
        );
        require(
            SHIBARewardsSell >= 1 && SHIBARewardsSell <= 50,
            "SHIBA reward fees must be between 1 and 50"
        );

        operationsFees = operations;
        liquidityFee = liquidity;
        SHIBARewardsBuyFee = SHIBARewardsBuy;
        SHIBARewardsSellFee = SHIBARewardsSell;

        totalSellFees = SHIBARewardsSellFee.add(liquidityFee).add(operationsFees);
        totalBuyFees = SHIBARewardsBuyFee.add(liquidityFee).add(operationsFees);

        emit UpdateFees(operations, liquidity, SHIBARewardsBuy, SHIBARewardsSell);
    }
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            uint256 SHIBARewardsFee;

            if (isSelling) {
                SHIBARewardsFee = SHIBARewardsSellFee;
            } else {
                SHIBARewardsFee = SHIBARewardsBuyFee;
            }

            uint256 totalFees = SHIBARewardsFee.add(liquidityFee).add(operationsFees);
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 dividendsFromBuy = address(this)
            ...
            .mul(buyAmount)
            .div(totalAmount)
            .mul(SHIBARewardsBuyFee)
            .div(SHIBARewardsBuyFee.add(operationsFees));

        uint256 dividendsFromSell = address(this)
            .balance
            .mul(sellAmount)
            .div(totalAmount)
            .mul(SHIBARewardsSellFee)
            .div(SHIBARewardsSellFee.add(operationsFees));
...
contract ShibaGoDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract)
        DividendPayingToken(
            "ShibaGo_Dividend_Tracker",
            "ShibaGo_Dividend_Tracker"
        )
    {
        shibaGo = ShibaGo(mainContract);
    ...
    function _transfer(
        ...
        require(false, "ShibaGo_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "ShibaGo_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ShibaGo contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(shibaGo);

            uint256 prevBalance = shibaGo.balanceOf(address(this));
            ...
            {
                uint256 received = shibaGo.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    shibaGo.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            // if no tokenAddress assume shiba payout
            if (
                !allowCustomTokens ||
