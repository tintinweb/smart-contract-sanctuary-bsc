...
contract SeedifyLaunchpadNFT is ERC1155, Ownable, Pausable, ERC1155Burnable { 
    ...
    mapping(uint256=>data) public getProjectDataByID;   

    struct data1 {
        string num;
    }
    mapping(string=>data1) public getProjectDataByTokenID; 
    mapping(uint256 => uint256) internal getProjectID;
    ...
    function mint(uint256 amount, uint256 projectID, string memory uri)
        ...
        whenNotPaused
    {   
        require(isWhitelisted(msg.sender), "You are not eligible to Mint item");
        tokenIDs = tokenIDs + 1;
        string memory strTokenIDs=Strings.toString(tokenIDs);
    ...
    function mintBatch(uint256[] memory amounts, uint256 projectID, string[] memory uri)
        ...
        whenNotPaused
    {
        require(isWhitelisted(msg.sender), "You are not eligible to Mint items");
        require(amounts.length <= 10, "You are allow to mint max 10 items at a time");
        uint256[] memory ids = new uint256[](amounts.length);
        string memory strTokenIDs = "";
    ...
    // append numbers
    function appendStrings(uint256 num, string memory str) public view returns(string memory) {
        return getProjectDataByID[num].num.concat(str).concat(",");
    }
    ...
    // Owner can pause the minting
    function pause() public onlyOwner {
        _pause();
    }

    // Owner can pause/unpause the minting
