...
contract Blazar_StakePool_1 is Ownable,ReentrancyGuard {
    ...
    uint256 public poolMinStake = 20000;
    uint256 public poolMaxStake = 50000000000;
    uint256 public poolRewardPercent = 30;
    uint256 public earlyPenalty = 75;
    ...
    function PoolStake(uint256  _amount) public nonReentrant returns (bool) {
    ...
    function claimPool() public nonReentrant returns(bool){
        ...
        if(block.timestamp < (users[msg.sender].pool_deposit_time + fullMaturityTime)){
            amount = amount.sub((amount).mul(earlyPenalty).div(100));
            reward = 0;
        }
