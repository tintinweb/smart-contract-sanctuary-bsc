...
https://twitter.com/WorldCupFiBsc
...
abstract contract Auth {
    ...
    function renounceOwnership() public onlyOwner {
        owner = address(0);
        emit OwnershipTransferred(0x000000000000000000000000000000000000dEaD);
    }
...
contract WorldCupFi is ERC20, Auth {
    ...
    event ChangedPrivateRestrictions(
    ...
    string private constant _name = "WorldCupFi";
    ...
    uint256 private totalSellFee =
        sellFeeLiquidity.add(sellFeeMarketing).add(sellFeeBurn);

    uint256 private feeDenominator = 100;

    address private autoLiquidityReceiver =
       0xa286F5cC2E6b57C128A9F731d1f15EEb7dd9aE81;
    ...
    function _transferFrom(
        ...
        if (automatedMarketMakerPairs[sender]) {
            if (!isFeeExempt[recipient]) {
                ...
                require(
                    amount <= _maxTxAmount || isTxLimitExempt[recipient],
                    "TX Limit Exceeded"
                );
    ...
    // Fees
    function takeBuyFee(address sender, uint256 amount)
        internal
        returns (uint256)
    {
        uint256 feeAmount = amount.mul(totalBuyFee.sub(BurnFee)).div(
            feeDenominator
        );
        ...
        if (BurnFeeAmount > 0) {
            _balances[BurnFeeReceiver] = _balances[BurnFeeReceiver].add(
                BurnFeeAmount
            );
    ...
    function takeSellFee(address sender, uint256 amount)
        internal
        returns (uint256)
    {
        uint256 feeAmount = amount.mul(totalSellFee.sub(sellFeeBurn)).div(
            feeDenominator
        );
    ...
    function takeTransferFee(address sender, uint256 amount)
        internal
        returns (uint256)
    {
    ...
    function swapBack() internal swapping {
        ...
        uint256 realTotalFee = totalBuyFee.add(totalSellFee).sub(BurnFee).sub(
            sellFeeBurn
        );
        ...
        uint256 amountETHLiquidity = amountETH
            .mul(liquidityFee.add(sellFeeLiquidity))
            .div(totalETHFee)
            .div(2);
        uint256 amountETHMarketing = amountETH
            .mul(MarketingFee.add(sellFeeMarketing))
            .div(totalETHFee);
