...
contract TestCA is ERC20, Ownable {
    ...
    constructor() ERC20("OptimizedTestTest", "TestCA") {
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        ...
        require(newDividendTracker.owner() == address(this), "TEST: The new dividend tracker must be owned by the OptimizedTest contract");
    ...
    function setrewardFees(uint256 value) external onlyOwner{
        ...
        require(totalFees <= 25, "TEST: Maximum allowed fees are 25%");
    }
    ...
    function setExtraSellFee(uint256 value) external onlyOwner{
        ...
        require(totalSellFees <= 25, "TEST: Maximum allowed fees are 25%");
    }
    ...
    function limitMaxWallet(uint256 maxWalletPercent) external onlyOwner {
         ...
         require(maxWalletPercent >= 1, "TEST: Can not limit Max Wallet to less than 1%");
    }
    ...
    function _transfer(
        ...
        if (from != owner() &&
            ...
            require(balanceOf(to) + amount <= maxWalletTokens, "TEST: Exceeds maximum wallet token amount.");
        }
        if(amount == 0) {
        ...
        if(takeFee) {
            require(tradingOpen,"TEST: Trading is not open yet");
