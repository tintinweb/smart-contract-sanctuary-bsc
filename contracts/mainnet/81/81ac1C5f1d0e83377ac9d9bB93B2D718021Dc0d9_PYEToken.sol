...
contract PYEToken is Context, IPYE, Ownable {
    ...
    */
    function getOwnedBalance(address account) external view returns (uint256){
        return shares[account].amount.add(_balances[account]);
    }
    ...
    */
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if(isStakingContract[recipient]) { 
            setShare(sender, amount);
        }

        if(isStakingContract[sender]) {
            uint256 newAmount = shares[recipient].amount.sub(amount);
            setShare(recipient, newAmount);
        }
