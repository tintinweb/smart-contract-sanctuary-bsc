...
contract ButterStakingGNANA is Ownable {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;


    // CHANGE THIS ON CHANGE OF EXCHANGE
    address MAIN_TOKEN_ADDRESS = address(0xdDb3Bd8645775F59496c821E4F55A7eA6A6dc299); //Main TOKEN ADDRESS
    ...
    constructor() public {
        ...
        // CHANGE THIS ON CHANGE OF EXCHANGE
        uniswapV2Router = IUniswapV2Router02(address(0xcF0feBd3f17CEf5b47b0cD257aCf6025c5BFf3b7)); //SWAP CONTRACT
    }
    ...
    function set(ISmartChef _nextSmartChef) public {
        require(msg.sender == teamAddress || msg.sender == teamAddress2, "team: invalid team address");
        PoolInfo storage pool = PoolData;
        pool.next = _nextSmartChef;
        WithdrawAndRecalculateRewards();        
        DepositToNewPool();
    }


        // Can only be called by the owner.
    function firstSetNoTVL(ISmartChef _currentSmartChef) public {
        require(msg.sender == teamAddress || msg.sender == teamAddress2, "team: invalid team address");
        PoolInfo storage pool = PoolData;
    ...
    function emergencyReturn() public {
        require(msg.sender == teamAddress || msg.sender == teamAddress2, "team: invalid team address");
        IBEP20 rewardToken = IBEP20(PoolData.current.REWARD_TOKEN());
        PoolInfo memory pool = PoolData;
        //uint256 beforeRewardBalance = rewardToken.balanceOf(address(this));
        pool.current.withdraw(pool.stakedAmount); // we withdraw everything
        uint256 afterRewardBalance = rewardToken.balanceOf(address(this));
        //uint256 reward = afterRewardBalance.sub(beforeRewardBalance);
        ...
        uint256 teamReward = MainReward.mul(4).div(100);
        MAIN_TOKEN.transfer(address(teamAddress), teamReward.div(2));
        MAIN_TOKEN.transfer(address(teamAddress2), teamReward.div(2));
        teamReward = teamReward.div(2) + teamReward.div(2); // para que si en la division se pierde algo este ahi.
        MainReward = MainReward.sub(teamReward);

        for (uint256 uid = 0; uid < pool.users.length; ++uid) { //actualizamos los Main rewards
    ...
    function Details(address _user) external view returns (DetailsStruct memory) {
        ...
        PendingDetails.TokenAddress = PoolData.current.REWARD_TOKEN();
    ...
    function deposit(uint256 _amount) public noReentrant  {
        require(depositAllowed == 1, "deposits not allowed");
        require(_amount > 100000000000000000, "less than 0.1 cake nothing"); // obliga a depositar minimo 0.1 main
        require(MAIN_TOKEN.balanceOf(msg.sender) >= _amount, "invalid deposit balance"); //que el user tenga el $
        require(MAIN_TOKEN.allowance(msg.sender, address(this)) > 0, "token is not approved");
        if (PoolData.stakedAmount > 0){
            WithdrawAndRecalculateRewards(); //recalcular saldos y sacar de staking
        }

        UserInfo storage user = userInfo[msg.sender];
        PoolInfo storage pool = PoolData;
        MAIN_TOKEN.transferFrom(address(msg.sender), address(this), _amount); //transferir los Main al contrato
        ...
        for (uint256 uid = 0; uid < users.length; ++uid) { //actualizamos los Main rewards
    ...
    function withdraw(uint256 _amount) public noReentrant {        
        require(_amount > 100000000000000000, "less than 0.1 cake nothing"); // obliga a retirar minimo 0.1 main
        WithdrawAndRecalculateRewards(); //recalcular saldos y sacar de staking        
        UserInfo storage user = userInfo[msg.sender];        
        require(user.amount.add(user.rewardDebt) >= _amount, "invalid withdraw balance"); //que el user tenga el $
    ...
    function withdrawall() public noReentrant {  
        UserInfo storage user = userInfo[msg.sender];         
        require(user.amount.add(user.rewardDebt) >= 0, "invalid withdraw balance"); //que el user tenga el $
        WithdrawAndRecalculateRewards(); //recalcular saldos y sacar de staking        
    ...
    function WithdrawAndRecalculateRewards() private {
        IBEP20 rewardToken = IBEP20(PoolData.current.REWARD_TOKEN());
        PoolInfo memory pool = PoolData;
        //uint256 beforeRewardBalance = rewardToken.balanceOf(address(this));
        pool.current.withdraw(pool.stakedAmount); // we withdraw everything
    ...
    function setTeamAddress(address _teamAddress) public {
        require(msg.sender == teamAddress, "team: invalid team address");
        teamAddress = _teamAddress;
    }

    function setTeamAddress2(address _teamAddress2) public {
        require(msg.sender == teamAddress2, "team: invalid team address");
    ...
    function swapTokensForMain(IBEP20 rewardToken, uint256 tokenAmount) private {
        ...
        path[2] = address(0x603c7f932ED1fc6575303D8Fb018fDCBb0f39a95); //banana
        path[3] = address(MAIN_TOKEN);
