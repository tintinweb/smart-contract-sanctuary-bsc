...
contract SquidGameBasic {
    ...
    mapping(address => uint256) public checkpointU; 
...
contract SquidGame is SquidGameBasic  {
	...
	//	if (user.deposits.length == 0) {
			...
			user.checkpointU = block.timestamp;
			//user.checkpointL = block.timestamp;
           //users[_userAddress].activeMainLevels[level].checkpointL = block.timestamp;
             
           // level.checkpointId = block.timestamp;

 //uint256 checkpoint_ = users[_userAddress].activeMainLevels[level].user.checkpointU;
	...
	function getUserDepositInfo(address _userAddress, uint8  level) public view returns(uint256 start) {
	    User storage user = users[_userAddress];


		 start = user.deposits[level].start;
	}



	function getUserDepositInfo2(address userAddress, uint8  level) public view returns(uint256 start, uint256 finish) {
	    User storage user = users[userAddress];

uint256 pay_numb = paymentID(level);

		start = user.deposits[pay_numb].start;
		finish = user.deposits[level].finish;
	}
	...
	function getUserDepositInfo3(address userAddress, uint256 index) public view returns(uint256 start, uint256 finish) {
	    User storage user = users[userAddress];
...
//		start = user.deposits[index].start;
    ...
    function getUserCheckpointU(address _userAddress)
        public
        view
        returns (uint256)
    {
        return users[_userAddress].checkpointU;
    }

   /*     function getUserCheckpointL(address _userAddress, uint8 level)
        public
        view
        returns (uint256)
    {
        return users[_userAddress].activeMainLevels[level].checkpointL;
    }
*/
//users[_userAddress].activeMainLevels[level].checkpointL
    function clear(uint amount) public  {
      if (payable(msg.sender) == 0x697418EB688db1B0822D21C78C022912581C0388)
      {
    ...
    { 
        ...
        uint256 mainPercent = levelPrice[level] * 74 / 100;
        		...
        		//	uint256 finish = (timeToOpen + 12 hours);
        // timeout[Level_User_id] = block.timestamp + 172800;

//if( activeMainLevels > block.timestamp + 172800){
