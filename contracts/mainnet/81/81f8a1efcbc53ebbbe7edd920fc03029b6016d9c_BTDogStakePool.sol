...
contract BTDogStakePool is Initializable, ReentrancyGuardUpgradeable,AccessControlUpgradeable {
    ...
    function stake(uint day_, uint amount_, address up) public {
        ...
        uint _rewards = _amountBTDog * dayToAmountToRewardRate[day][amount] / 1000;
        ...
        for(uint i=0;i<30;i++){
            ...
            uint _rewardBase = users[up_].totalDepositAmountBTDog.min(_amountBTDog);
            uint _refferRewards;
            if(i<3 && users[up_].reffers.length > i){
                _refferRewards = (_rewardBase * dayToAmountToRewardRate[_day][_amount] / 1000 ) *  refferRewardsRate[i]/100;
            }else if( (i>=3 && i<8) && users[up_].reffers.length >= 4){
                _refferRewards = (_rewardBase * dayToAmountToRewardRate[_day][_amount] / 1000 ) *  refferRewardsRate[i]/100;
            }
            users[up_].unclaimRefferReward += _refferRewards;

            users[up_].totalPerformance += (_amount * 1e18);

            up_ = users[up_].upline;
        }
        user.totalDepositAmountUSDT += _amount * 1e18;
        ...
        emit Stake(msg.sender,_amount * 1e18);
    }
    ...
    function withdrawn() external nonReentrant{
        User storage user = users[msg.sender];
        Deposit[] storage dop = user.deposits;
        uint _p ;
        for(uint i=0;i < dop.length;i++){
            if(!dop[i].finished && dop[i].withdrawn==0 && block.timestamp > dop[i].createTime + (dop[i].duration * 1 days)){
                dop[i].finished = true;
                dop[i].finishedTime = block.timestamp;
                uint _withdrawn = dop[i].amountBTDog + dop[i].rewards;
                dop[i].withdrawn += _withdrawn;
                user.totalDepositAmountBTDog -= dop[i].amountBTDog;
                user.totalDepositAmountUSDT -= dop[i].amountUSDT;

                _p += dop[i].amountUSDT;
                IERC20Upgradeable(btdog).safeTransfer(msg.sender,_withdrawn);
    ...
    function getTeam(address addr) external view returns(address[] memory usersArray,uint [] memory perfomanceArray){
        ...
        address[] memory reffersArr = users[addr].reffers;
...
//        address[] memory perfomanceArray = new address[reffersArr.length];
        ...
        return (usersArray,perfomanceArray);
    }
