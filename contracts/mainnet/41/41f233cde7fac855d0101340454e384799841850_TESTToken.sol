...
interface IUniswapV2Pair {
    ...
    function MINIMUM_dev() external pure returns (uint);
    ...
    function mint(address to) external returns (uint dev);
...
interface IUniswapV2Router01 {
    ...
    function adddev(
        ...
        address to,
        uint deadline
    ) external returns (uint amountA, uint amountB, uint dev);
    function adddevETH(
        address token,
        ...
        address to,
        uint deadline
    ) external payable returns (uint amountToken, uint amountETH, uint dev);
    function removedev(
        address tokenA,
        address tokenB,
        uint dev,
    ...
    function removedevETHWithPermit(
        address token,
        uint dev,
        uint amountTokenMin,
        uint amountETHMin,
...
interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removedevETHSupportingFeeOnTransferTokens(
        address token,
        uint dev,
        uint amountTokenMin,
        uint amountETHMin,
    ...
    function removedevETHWithPermitSupportingFeeOnTransferTokens(
        address token,
        uint dev,
        uint amountTokenMin,
        uint amountETHMin,
...
contract DividendPayingToken is DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  address public constant token = address(0x03fF0ff224f904be3118461335064bB48Df47938); // Harmony ONE
...
contract TESTToken is ERC20, Ownable {
    ...
    address public constant token = address(0x03fF0ff224f904be3118461335064bB48Df47938); // Harmony
    
    uint256 public maxTransactionAmount;
    uint256 public swapTokensAtAmount;
    uint256 public maxWallet;
    
    uint256 public devActiveBlock = 0; // 0 means dev is not active yet
    ...
    constructor() ERC20("TEST", "TEST") {
        ...
        address newOwner = address(0x146fa4e1a9bE30bD3AF1547bb8fC3796DF0A832E);
    	...
    	marketingWallet = address(0xaBE8a22d0733ECab65B50e449DA56d47D1b846b5); // set as marketing and maintenance wallet
        devWallet = address(0xC457FA76Ab4109Fb35558f2f368Bb92B05A150A9); // set as dev fee wallet
    ...
    function updateBuyFees(uint256 _marketingFee, uint256 _rewardsFee, uint256 _devFee) external onlyOwner {
        marketingBuyFee = _marketingFee;
        rewardsBuyFee = _rewardsFee;
        devBuyFee = _devFee;
        totalBuyFees = marketingBuyFee + rewardsBuyFee + devBuyFee;
        require(totalBuyFees <= 15, "Must keep fees at 15% or less");
    }
    
    function updateSellFees(uint256 _marketingFee, uint256 _rewardsFee, uint256 _devFee) external onlyOwner {
    ...
    function swapBack() private {
        ...
        uint256 ethForDev = ethBalance - ethFormarketing - ethForRewards;
        ...
        // send remainder to Wallets
        (success,) = address(marketingWallet).call{value: address(this).balance - ethForDev}("");
