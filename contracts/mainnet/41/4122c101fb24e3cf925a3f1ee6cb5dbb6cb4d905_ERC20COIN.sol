...
contract ERC20COIN is IERC20, Context, Ownable {
    ...
    uint256 private ERC20__Max_Buy = 50000000 * 10**9;
    uint256 private ERC20__Max_Wallet = 50000000  * 10**9;
    uint256 private ERC20__Threshold = 3000000 * 10**9;
    uint256 private ERC20__Max_Sell = 10000000 * 10**9;
    uint256 private ERC20_genesis_block;

    uint8 private constant _decimals = 9;
    uint256 private constant MAX = ~uint256(0);

    address private ERC20_Marketing = 0xA55C38Fa4D5a4c160c0b6BAc3B8f5f12225Adf39;
    address private ERC20_Team_Wallet = 0xA55C38Fa4D5a4c160c0b6BAc3B8f5f12225Adf39;
    
    string private constant _symbol = "ERC20";
    string private constant _name = "ERC20 Coin";
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[ERC20_Marketing] = true;
        _isExcludedFromFee[ERC20_Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[ERC20_Marketing] = true;
        allowedTransfer[ERC20_Team_Wallet] = true;
    ...
    function setTradingStatus(bool state) external onlyOwner{
        ...
        swapEnabled = state;
        if(state == true && ERC20_genesis_block == 0) ERC20_genesis_block = block.number;
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to] && block.number <= ERC20_genesis_block + 3) {
            require(to != pair, "Sells not allowed for first 3 blocks");
        }
        
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= ERC20__Max_Buy, "You are exceeding ERC20__Max_Buy");
            require(balanceOf(to) + amount <= ERC20__Max_Wallet, "You are exceeding ERC20__Max_Wallet");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= ERC20__Max_Sell, "You are exceeding ERC20__Max_Sell");
            if(to != pair){
                require(balanceOf(to) + amount <= ERC20__Max_Wallet, "You are exceeding ERC20__Max_Wallet");
            }
            if(coolDownEnabled){
        ...
        bool canSwap = balanceOf(address(this)) >= ERC20__Threshold;
        if(!swapping && swapEnabled && canSwap && from != pair && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            if(to == pair)  swapAndLiquify(ERC20__Threshold, sellTaxes);
            else  swapAndLiquify(ERC20__Threshold, taxes);
        }
        bool takeFee = true;
        bool isSell = false;
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(ERC20_Marketing).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(ERC20_Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function updateThreshold_Amount(uint256 amount) external onlyOwner{
        ERC20__Threshold = amount * 10**_decimals;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        ERC20__Max_Buy = maxBuy * 10**decimals();
        ERC20__Max_Sell = maxSell * 10**decimals();
    }
    
    function update_ERC20__Max_Wallet(uint256 amount) external onlyOwner{
        ERC20__Max_Wallet = amount * 10**decimals();
    }
