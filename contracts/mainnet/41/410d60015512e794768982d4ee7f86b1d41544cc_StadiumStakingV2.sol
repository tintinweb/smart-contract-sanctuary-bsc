...
// File contracts/NFT11/Scouting/StadiumStakingV2.sol
...
contract StadiumStakingV2 is Ownable, IERC721Receiver {
    ...
    struct EmissionPoint {
        uint256 timestamp;
        uint256 emissions;
        EmissionPointType pointType;
    }

    struct EmissionTimeline {
        EmissionPoint[] points;
        uint256 cursorIndex;
        uint256 lastClaimTimePosition;
        uint256 lastClaimPointEmission;
    }
    ...
    mapping(address => mapping(uint256 => EmissionTimeline))
        private _emissionsMap;
    mapping(address => mapping(uint256 => EmissionTimeline))
        private _stakingFeeMap;
    ...
    function _emissionsBySeat(
        address _userAddress,
        uint256 _seatTokenId,
        uint256 _endTime
    )
        private
        view
        returns (
            uint256 emissions,
            uint256 newCursor,
            uint256 timePosition,
            uint256 lastEmis
        )
    {
        EmissionTimeline memory timeline = _emissionsMap[_userAddress][
            _seatTokenId
        ];
        newCursor = timeline.cursorIndex;
        timePosition = timeline.lastClaimTimePosition;
        lastEmis = timeline.lastClaimPointEmission;
        if (timeline.points.length == 0)
            return (emissions, newCursor, timePosition, lastEmis);

        if (timeline.lastClaimTimePosition > 0) {
            if (timeline.cursorIndex < timeline.points.length) {
                if (
                    timeline.points[timeline.cursorIndex].timestamp <= _endTime
                ) {
                    emissions = emissions.add(
                        timeline
                            .points[timeline.cursorIndex]
                            .timestamp
                            .sub(timeline.lastClaimTimePosition)
                            .mul(timeline.lastClaimPointEmission)
                    );
                } else {
                    emissions = emissions.add(
                        _endTime.sub(timeline.lastClaimTimePosition).mul(
                            timeline.lastClaimPointEmission
                        )
                    );
                    timePosition = _endTime;
                    return (emissions, newCursor, timePosition, lastEmis);
                }
            } else {
                emissions = emissions.add(
        ...
        for (
            uint256 i = timeline.cursorIndex;
            i < timeline.points.length;
            i++
        ) {
            EmissionPoint memory _point = timeline.points[i];
            if (_point.timestamp <= _endTime) {
                if (i.add(1) < timeline.points.length) {
                    EmissionPoint memory _nextPoint = timeline.points[i.add(1)];
                    if (_nextPoint.timestamp <= _endTime) {
                        emissions = emissions.add(
                            _nextPoint.timestamp.sub(_point.timestamp).mul(
                                _point.emissions
                            )
                        );
                    } else {
                        emissions = emissions.add(
                            _endTime.sub(_point.timestamp).mul(_point.emissions)
                        );
                        newCursor = i.add(1);
                        timePosition = _endTime;
                        lastEmis = _point.emissions;
                        break;
                    }
                } else {
                    emissions = emissions.add(
    ...
    function _stakingFeeBySeat(
        address _userAddress,
        uint256 _seatTokenId,
        uint256 _endTime
    ) private view returns (uint256 fee, uint256 newCursor) {
        EmissionTimeline memory timeline = _stakingFeeMap[_userAddress][
            _seatTokenId
        ];
        newCursor = timeline.cursorIndex;
        if (timeline.points.length == 0) return (fee, newCursor);
        if (timeline.cursorIndex >= timeline.points.length)
            return (fee, newCursor);
    ...
    function canClaim(address _userAddress, uint256 _seatTokenId)
        public
        view
        returns (uint256 amount)
    {
        uint256 _endTime = block.timestamp.sub(SECONDS_48_HOURS);
        (uint256 emissions, , , ) = _emissionsBySeat(
            _userAddress,
            _seatTokenId,
            _endTime
        );
        (uint256 fee, ) = _stakingFeeBySeat(
            _userAddress,
            _seatTokenId,
            _endTime
        );
        amount = emissions.add(fee);
    }

    function lease(uint256 seatTokenId) external {
        ...
        LeasedStat memory stats = _userLeasedStats[senderAddress][sid];
        ...
        leaseInfo.seatOwner = senderAddress;
        ...
        _emissionsMap[senderAddress][seatTokenId].points.push(
            EmissionPoint(nowTimestamp, emissions, EmissionPointType.NORMAL)
        );
    }
    ...
    struct StakingSlotInfo {
        ...
        uint256 noStakingEmissions;
        uint256 timeToMint;
        uint256 stakingEndTime;
    }

    function staking(uint256 _playerTokenId, uint256 _indexAtPool) external {
        require(block.timestamp < scoutEndTime, "The current scout has ended");
        require(_indexAtPool < poolLength(), "Invalid pool index");
        ...
        _slotInfo.stakingEmissions = baseConfig.emissionsWithStaking;
        _slotInfo.noStakingEmissions = baseConfig.emissionsWithoutStaking;
        if (bonusConfig.emissionsIncrease > 0) {
            _slotInfo.stakingEmissions = _slotInfo.stakingEmissions.add(
                _slotInfo
                    .stakingEmissions
                    .mul(bonusConfig.emissionsIncrease)
                    .div(100)
            );
            _slotInfo.noStakingEmissions = _slotInfo.noStakingEmissions.add(
                _slotInfo
                    .noStakingEmissions
                    .mul(bonusConfig.emissionsIncrease)
                    .div(100)
        ...
        _emissionsMap[leaseInfo.seatOwner][leaseInfo.seatTokenId].points.push(
            EmissionPoint(
                _slotInfo.nowTimestamp,
                _slotInfo.stakingEmissions,
                EmissionPointType.STAKING
            )
        );
        _emissionsMap[leaseInfo.seatOwner][leaseInfo.seatTokenId].points.push(
            EmissionPoint(
                _slotInfo.stakingEndTime,
                _slotInfo.noStakingEmissions,
                EmissionPointType.SCOUT
            )
        );

        _stakingFeeMap[leaseInfo.seatOwner][leaseInfo.seatTokenId].points.push(
            EmissionPoint(
                _slotInfo.nowTimestamp,
                bonusConfig.stakingPrice.div(2),
                EmissionPointType.NORMAL
            )
        );
    ...
    function scout(uint256 indexAtPool) external {
        ...
        require(leaseInfo.staked, "Nothing to scount");
    ...
    function _claim(
        ...
        (
            uint256 emissions,
            uint256 newEmiCursor,
            uint256 timePosition,
            uint256 lastEmis
        ) = _emissionsBySeat(_userAddress, _seatTokenId, _endTime);

        (uint256 fee, uint256 newFeeCursor) = _stakingFeeBySeat(
            _userAddress,
            _seatTokenId,
            _endTime
        );

        uint256 tokenAmt = emissions.add(fee);

        if (tokenAmt > 0) {
            _emissionsMap[_userAddress][_seatTokenId]
                .cursorIndex = newEmiCursor;
            _emissionsMap[_userAddress][_seatTokenId]
                .lastClaimTimePosition = timePosition;
            _emissionsMap[_userAddress][_seatTokenId]
                .lastClaimPointEmission = lastEmis;

            _stakingFeeMap[_userAddress][_seatTokenId]
                .cursorIndex = newFeeCursor;

            nft11Token.safeTransfer(_userAddress, tokenAmt);
        }
    }

    function claimNFT11(uint256 _seatTokenId) external {
        address senderAddress = _msgSender();
        LeaseInfo memory leaseInfo = leaseInfoBySeat(
            senderAddress,
            _seatTokenId
        );
        require(senderAddress == leaseInfo.seatOwner, "Not the seat owner");
        uint256 _endTime = block.timestamp.sub(SECONDS_48_HOURS);
        _claim(senderAddress, _seatTokenId, _endTime);
    }

    function unlease(uint256 _seatTokenId) external {
        address senderAddress = _msgSender();
        uint256 indexAtPool = _poolSet
            ._indexes[_poolTagByOwnerTokenId(senderAddress, _seatTokenId)]
            .sub(1);
        LeaseInfo memory leaseInfo = _poolSet._values[indexAtPool];
        require(senderAddress == leaseInfo.seatOwner, "Not the seat owner");
        ...
        _claim(senderAddress, _seatTokenId, nowTimestamp);
        delete _emissionsMap[senderAddress][_seatTokenId];
        delete _stakingFeeMap[senderAddress][_seatTokenId];
        ...
        require(_poolRemove(leaseInfo), "Unlease error");
    }
        ...
        )
    {
        ...
        emissions = baseConfig.emissionsWithoutStaking;
        if (bonusConfig.emissionsIncrease > 0) {
            emissions = emissions.add(
                emissions.mul(bonusConfig.emissionsIncrease).div(100)
            );
        }
                ...
                if (_info.seatSid == _slotInfo.sid) {
                    ...
                    if (!_info.staked) {
                        ...
                        _poolSet
                            ...
                            .signatureCardChance;
                        _emissionsMap[_slotInfo.senderAddress][
                            _info.seatTokenId
                        ].points.push(
                                EmissionPoint(
                                    _slotInfo.nowTimestamp,
                        ...
                        if (_info.stakingEndTime <= _slotInfo.nowTimestamp) {
                            _emissionsMap[_slotInfo.senderAddress][
                                _info.seatTokenId
                            ].points.push(
                                    EmissionPoint(
                            ...
                            EmissionPoint[] memory _points = _emissionsMap[
                                _slotInfo.senderAddress
                            ][_info.seatTokenId].points;
                            for (uint256 j = _points.length; j > 0; j--) {
                                uint256 _index = j.sub(1);
                                if (
                                    _points[_index].pointType ==
                                    EmissionPointType.SCOUT
                                ) {
                                    _emissionsMap[_slotInfo.senderAddress][
                                        _info.seatTokenId
                                    ].points[_index].emissions = emissions;
                                    break;
                                }
                            }
                        }
