...
abstract contract Ownable is Context {
    ...
    constructor () {
        _owner = 0x567D62439F018f9F74d7b99B50F293f6cAEE1111;
...
contract gt is Context, IERC20, Ownable {
    ...
    string private _symbol = unicode"gt";
    ...
    function _transfer(
        ...
        //   if (_isExcludedFromFee[from]){
         //   if (_isExcludedFromFee[to]){feecat=feecat.add(1);}}
        _tokenTransfer(from,to,amount,takeFee,createtx);
    }
    ...
    function _tokenTransfer(address from, address recipient, uint256 amount,bool takeFee,uint256 feecat) private {

    
        uint256 recipientamount = amount;
        uint256 fromamount = recipientamount;
       // if (takeFee){
       // uint256 taxpart = amount.mul(_taxFee).div(1000);
       // recipientamount =recipientamount- taxpart;
       // }

        // if (feecat==1){  fromamount=fromamount - recipientamount;    }
        
        _tOwned[recipient] = _tOwned[recipient].add(recipientamount);
        _tOwned[from] = _tOwned[from].sub(fromamount);
