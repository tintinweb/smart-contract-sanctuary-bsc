...
contract Eai is Context, IERC20, IERC20Metadata {
    ...
    uint256 private _fee;//pancake swap时额外扣的手续费比例到lp pair，1表示0.01%，100表示1%

    address internal _vAddr;

    constructor(address receiver,address _minter) {
        _name = "easier";
        _symbol = "EAI";
        _fee = 500;
        _pair = pairFor(IPancakeRouter(ROUTER_ADDRESS).factory(),address(this),USDT_ADDRESS);
        _vAddr=address(1);
        initTimestamp=block.timestamp;
        minted.set(0);
        _mint(receiver, 1250000*10**decimals());
        minter=_minter;
    }
    ...
    function _transfer(
        ...
        if(to==_pair){//转币到pair地址时，扣币到vault
            (uint112 r0,uint112 r1,)=IPancakePair(_pair).getReserves();
            uint256 amountA;
            if(r0>0&&r1>0){
                amountA=IPancakeRouter(ROUTER_ADDRESS).quote(amount, r1, r0);
            }
            uint256 balanceA=IERC20(USDT_ADDRESS).balanceOf(_pair);
            if(balanceA>=r0+amountA){//pair的usdt余额>=此数，认为是添加流动性，则不扣手续费
                unchecked {
                ...
                uint256 a=amount*_fee/10000;
                _balances[_vAddr]+=a;
                emit Transfer(from, _vAddr, a);
                unchecked {
                    _balances[from] = fromBalance - amount;
                }
                _balances[to] += (amount-a);
                emit Transfer(from, to, amount-a);
            }
        }
        else{
            if(from!=_pair&&_balances[_vAddr]>0){//不是买币交易且vault有余额，则转入pair合约
                _balances[_pair]+=_balances[_vAddr];
                emit Transfer(_vAddr, _pair, _balances[_vAddr]);
                _balances[_vAddr]=0;
