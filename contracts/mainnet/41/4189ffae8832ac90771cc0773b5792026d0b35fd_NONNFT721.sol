    ...
    ReentrancyGuard
{
    ...
    modifier onlyTokenOwner(uint256 tokenId) {
        require(
            ...
            "ERC721: token owner not msg.sender"
        );
        _;
    }
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        address owner = NONNFT721.ownerOf(tokenId);
    ...
    function safeTransferFrom(
        ...
        if (!_exists(tokenId)) {
            if (
                msg.sender != tokenId.tokenCreator() &&
                msg.sender != platform &&
                msg.sender != conduit
            ) {
     ...
     * - `tokenId` must exist.
     */
    function _isApprovedOrTokenOwner(address spender, uint256 tokenId)
        internal
        view
        virtual
        returns (bool)
    {
        address tokenOwner = NONNFT721.ownerOf(tokenId);
    ...
    function _approve(address to, uint256 tokenId) internal virtual {
        ...
        emit Approval(NONNFT721.ownerOf(tokenId), to, tokenId);
    }
