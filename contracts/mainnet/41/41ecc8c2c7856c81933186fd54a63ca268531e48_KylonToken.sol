...
contract KylonToken is ERC20, ERC20Burnable, Ownable, AccessControl, ERC20Permit {
    ...
    uint96 public swapThreshold = 5000*10**18;

    IPancakeSwapV2Router02 public PancakeSwapV2Router;

    uint48 public launchTime;
    ...
    uint32 public transferGas = 3000;
    uint32 public buyTax = 650;
    uint32 public sellTax = 650;
    uint32 public transferTax = 650;
    uint32 public liquidityShare = 3000;
    uint32 public marketingShare = 4000;
    uint32 public charityShare = 3000;
    uint32 public burnShare = 0;
    ...
    modifier onlyManager() {
        require(hasRole(MANAGEMENT_ROLE, msg.sender), "!PERMISSION");
        _;
    }

    constructor() ERC20("KYLON", "KYLN") ERC20Permit("KYLON") {
        wallets["management"] = Wallet("management", 0xef8D787e68c068d5D347AC402FaE5399559E0433); // Multisign for Project Management
        wallets["marketing"] = Wallet("marketing", 0xd1B691e65782E41d87516A31f71A62f2d6b737C0); // Multisign for Project Marketing
        _setupRole(DEFAULT_ADMIN_ROLE, wallets["management"].addr);
        _setupRole(MANAGEMENT_ROLE, wallets["management"].addr);
    }
    ...
    function initializeProject() external onlyOwner {
        ...
        isWhitelisted[wallets["management"].addr] = true;
        isWhitelisted[wallets["marketing"].addr] = true;

        transferOwnership(address(0x000000000000000000000000000000000000dEaD));
        _mint(wallets["management"].addr, 500000000 * 10**18);
        _mint(wallets["marketing"].addr, 500000000 * 10**18);
    ...
    function _takeTax(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        uint256 taxAmount = amount * tax / DENOMINATOR;
    ...
    function _swapBack() internal swapping {
        ...
        uint256 totalBNBShares = DENOMINATOR;

        uint256 burnAmount = 0;
        uint256 liquidityTokens = 0;

        if(burnShare > 0) {
            burnAmount = swapThreshold * burnShare / DENOMINATOR;
            IERC20(address(this)).safeTransfer(address(0x000000000000000000000000000000000000dEaD), burnAmount);
        }

        uint256 netAmount = swapThreshold - burnAmount;

        if(liquidityShare > 0) {
           liquidityTokens = netAmount * liquidityShare / DENOMINATOR / 2;
        }           

        uint256 amountToSwap = netAmount - liquidityTokens;
        ...
        if(liquidityTokens > 0) {
            totalBNBShares = totalBNBShares - (liquidityShare / 2);
            amountBNBLiquidity = amountBNB * liquidityShare / totalBNBShares / 2;
    ...
    // Owner
    function disableDumpProtection() external onlyManager {
        dumpProtectionEnabled = false;
        emit DisableDumpProtection();
    }

    function removeSniperTax() external onlyManager {
    ...
    function triggerSwapBack() external onlyManager {
        _swapBack();
        emit TriggerSwapBack();
    }

    function recoverBNB(string memory walletName) external onlyManager {
    ...
    function recoverBEP20(IERC20 token, address recipient) external onlyManager {
        uint256 amount = token.balanceOf(address(this));
        token.safeTransfer(recipient, amount);
        emit RecoverBEP20(address(token), amount);
    }

    function setIsWhitelisted(address account, bool value) external onlyManager {
        isWhitelisted[account] = value;
        emit SetWhitelisted(account, value);
    }

    function setIsCEX(address account, bool value) external onlyManager {
        isCEX[account] = value;
        emit SetCEX(account, value);
    }

    function setIsMarketMaker(address account, bool value) external onlyManager {
        require(account != PancakeSwapV2Pair);
        isMarketMaker[account] = value;
        emit SetMarketMaker(account, value);
    }

    function setTaxes(uint32 newBuyTax, uint32 newSellTax, uint32 newTransferTax) external onlyManager {
    ...
    function setShares(uint32 newLiquidityShare, uint32 newMarketingShare, uint32 newCharityShare, uint32 newBurnShare) external onlyManager {
        liquidityShare = newLiquidityShare;
        marketingShare = newMarketingShare;
        charityShare = newCharityShare;
        burnShare = newBurnShare;
        require(newLiquidityShare + newMarketingShare + newCharityShare + newBurnShare == DENOMINATOR, "Incorrect Values");
        emit SetShares(liquidityShare, marketingShare, charityShare, burnShare);
    }

    function setSwapBackSettings(bool status, uint32 amount) external onlyManager {
        autoSwap = status;
        swapThreshold = amount * 10**18;
        emit SetSwapBackSettings(status, swapThreshold);
    }

    function setTransferGas(uint32 newGas) external onlyManager {
        require(newGas >= 25000 && newGas <= 50000);
        emit UpdateGasForProcessing(newGas,transferGas);
        transferGas = newGas;
    }

    function setWallets(Wallet memory wallet) external onlyManager {
