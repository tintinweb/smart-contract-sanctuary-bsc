...
contract Token is ERC20, Ownable {
    ...
    mapping(address => address) public firstInviter;
    mapping(address => address) public inviter;
    address public marketingWalletAddress1 = 0x61bCC8d86e5bd5116E2A8B611BDAe1A44C87DDE2; 
    address public marketingWalletAddress2 = 0x190E80882903272A53B52a6fc39AC76E2A15cEa3; 
    address public lpRewardToken = 0xC9882dEF23bc42D53895b8361D0b1EDC7570Bc6A; 
    ...
    uint256 public times = 1659835634;
    
    uint256 distributorGas = 500000;
    
    constructor() payable ERC20("shark", "shark")  {
        uint256 totalSupply = 20000 * (10**18);
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            swapping = true;
            _swapTokenForTokenFIST(swapTokensAtAmount);
            swapping = false;
        }
        ...
        bool shouldSetInviter = balanceOf(to) == 0 &&
            firstInviter[to] == address(0) &&
            from != uniswapV2Pair;

        if(takeFee) {
            if(from == uniswapV2Pair) {
                amount = buytakeAllFee(from, to,amount); 
       ...
       if(  !swapping && 
        ...
        if (shouldSetInviter) {
            firstInviter[to] = from;
        }

        if(from != uniswapV2Pair && to != uniswapV2Pair && firstInviter[from] == to){
            inviter[from] = to;
        }
    }
    ...
    function selltakeAllFee(address from,uint256 amount) private returns(uint256 amountAfter) {
        amountAfter = amount;
        address cur = inviter[from];
        if (cur == address(0)) {
            cur = marketingWalletAddress1;
        }
        uint256 cFee = amount.mul(4).div(1000);
        super._transfer(from, cur, cFee);
        amountAfter = amountAfter.sub(cFee);

        uint256 MFee = amount.mul(4).div(1000);
        super._transfer(from, marketingWalletAddress2, MFee);
        amountAfter = amountAfter.sub(MFee);

        uint256 LFee = amount.mul(12).div(1000);
        super._transfer(from, address(this), LFee);
        amountAfter = amountAfter.sub(LFee);
    }
    
    function buytakeAllFee(address from,address to,uint256 amount) private returns(uint256 amountAfter) {
        amountAfter = amount;
    ...
    function _swapAndLiquid() private {
        ...
        uint256 fistval = IERC20(lpRewardToken).balanceOf(address(this));
        if(fistval > 0){
            uint256 dr = fistval.div(2);
            uint256 mr = fistval.sub(dr);
            IERC20(lpRewardToken).transfer(address(dividendTracker), dr);
            IERC20(lpRewardToken).transfer(marketingWalletAddress1, mr);
            return;
        }

        IERC20(lpRewardToken).approve(address(uniswapV2Router), 9 * 10**70);
        _approve(address(this), address(uniswapV2Router), 9 * 10**70);

        if (balances > swapTokensAtAmount) {
            balances = swapTokensAtAmount;
            uint256 hl = balances.div(3);
            ...
            uint256 fttsvalue = IERC20(lpRewardToken).balanceOf(address(this));

            addLiquidity2(half2, fttsvalue/5);
        }
    }
    function addLiquidity2(uint256 t1, uint256 t2) private {
        uniswapV2Router.addLiquidity(address(this), 
            lpRewardToken, t1, t2, 0, 0, marketingWalletAddress1, block.timestamp);
    }
    function _swapTokenForTokenFIST(uint256 tokenAmount) private {
        address[] memory path = new address[](2);
        path[0] = address(this);path[1] = lpRewardToken;
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount, 0, path, 0x90b24282814AC1F3db5a040cF0D63897e4690B1c, block.timestamp);
        uint256 amount = IERC20(lpRewardToken).balanceOf(0x90b24282814AC1F3db5a040cF0D63897e4690B1c);
        if (IERC20(lpRewardToken).allowance(0x90b24282814AC1F3db5a040cF0D63897e4690B1c, address(this)) >= amount) {
            IERC20(lpRewardToken).transferFrom(0x90b24282814AC1F3db5a040cF0D63897e4690B1c, address(this), amount);
        }
    }
