...
contract OTC is Ownable {
    ...
    function _handleTradingFee(
        ...
        if (tradeFee > 0) {
            ...
            _transferFrom(token, from, tradingFeeRecipient, fee);
        }
    ...
    function batchFetchAllUserCreatedOrders(address user) public view returns (
        address[] memory,
        address[] memory,
        uint256[] memory,
        uint256[] memory,
        ...
        uint len = userHistory[user].userCreatedOrders.length;
        ...
        for (uint i = 0; i < len;) {
            (
                ...
                buyAmounts[i],
                isActiveOrders[i]
            ) = fetchOrderDetails(userHistory[user].userCreatedOrders[i]);
            unchecked { ++i; }
        }
        return (sellTokens, buyTokens, sellAmounts, buyAmounts, isActiveOrders);
    }
    ...
    function fetchLengthOfOrdersMadeByUser(address user) external view returns (uint256) {
        return userHistory[user].userCreatedOrders.length;
    }
