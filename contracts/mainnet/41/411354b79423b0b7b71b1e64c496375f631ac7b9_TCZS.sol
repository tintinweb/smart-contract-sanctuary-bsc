...
contract TCZS is ERC20 {
	...
	address private _fundAddress = address(0x2Eb31654eBCC922672691440eA6fAac37E24ED23);//1
	address private _marketAddress = address(0xF8cdf806318E1733C37a9F26d025eb94B1a545A9);//2
    ...
    constructor(address tokenOwner) ERC20("the twelve Chinese zodiac signs", "TCZS") {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        address _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
        .createPair(address(this), address(_baseToken));
		uint256 total = 12 * 10**32;
	...
	function changeRabbitoken(address rabbitoken) public onlyOwner {
        _rabbitoken = rabbitoken;
		RABBIT = IERC20(rabbitoken);
    }
	...
	function swapAndLiquify() public {
		...
		if(allAmount > 10**22){
			...
			swapTokensForOther(allAmount.div(7).mul(3));
			swapTokensForDead(allAmount.div(7).mul(2));
			swapTokensForRibb(allAmount.div(7).mul(2));
			...
			if(allusdtAmount > usdtAmount){
				...
				USDT.transfer(_marketAddress,newUsdtAmount.div(3).mul(2));
			}
		}else{
			return;
		}
	...
	function swapTokensForDead(uint256 tokenAmount) internal {
		address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = address(_baseToken);
		path[2] = address(_rabbitoken);
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0,
            path,
	...
	function swapTokensForRibb(uint256 tokenAmount) internal {
		address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = address(_baseToken);
		path[2] = address(_rabbitoken);
    ...
    function _splitOtherSecond() private {
        ...
        if(buySize>0 && totalAmount > 0){
            ...
            if(buySize >10){
                for(uint256 i=0;i<10;i++){
					...
					uint256 amountRabbit = thisRibbitAmount.mul(rate).div(1000000);
                    if(amountRabbit>10**10){
						RABBIT.transfer(user,amountRabbit);
                    }
	...
	uint256 public thisRibbitAmount;
    function splitOtherToken() private {
		thisRibbitAmount = RABBIT.balanceOf(address(this));
        if(thisRibbitAmount > 10**22){
			_splitOtherSecond();
        }
    }
