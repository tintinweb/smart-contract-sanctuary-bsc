...
// File: contracts/strategies/USDDLPStrategyStakeOnlyV5.sol
...
interface CurveLikeExchange{
    ...
    function exchange(int128, int128, uint256, uint256, bool) external payable; // Exchange tokens
...
contract USDDLPStrategyStakeOnlyV5 is Ownable {
    using SafeMath for uint256;
    ...
    address constant EPX_BNB_ADDRESS = address(0xE014A89c9788dAfdE603a13F2f01390610382471); // EPX, WBNB
    ...
    function withdraw(address _depositor, uint256 _share, uint256 _total, bool nonContract) public onlyZSBToken returns (uint256) {
        ...
        if(nonContract == true){
            if( _share > _total.mul(percentTradeTrigger).div(DIVISION_FACTOR)){
                harvestRewards(); // This will claim rewards, sell DDD and EPX for more USDDLP and some for WBNB
            }
        }
    ...
    */
    ...
    function checkHarvestWBNB() internal view returns (uint256) {
        ...
        // We no longer stake EPX, we just sell it as whole for WBNB
        rewards = rewards.add(rewardList[0].ddd); // DDD
        uint256 wbnbReward = 0;
        ...
        if(rewards > 0){
            ...
            wbnbReward = rewards.mul(DIVISION_FACTOR.sub(percentStakeDepositor)).div(DIVISION_FACTOR); // Only a certain part goes to executors/treasury/STBB
        }

        // Now EPX
        IERC20 epx = IERC20(EPX_ADDRESS);
        rewards = epx.balanceOf(address(this)); // Get any stored EPX
        rewards = rewards.add(rewardList[0].epx); // EPX
        if(rewards > 0){
            // Get via Ellipsis
            CurveLikeExchange curve = CurveLikeExchange(EPX_BNB_ADDRESS);
            rewards = curve.get_dy(0, 1, rewards); // EPX -> BNB
            wbnbReward = rewards.mul(DIVISION_FACTOR.sub(percentStakeDepositor)).div(DIVISION_FACTOR).add(wbnbReward); // Only a certain part goes to executors/treasury/STBB
        }
    ...
    function harvestRewards() internal {
        ...
        tokens[0] = address(tokenList[0].token);
        DotDotStaker.Amounts[] memory rewardList = staker.claimable(address(this), tokens);
        if(rewardList[0].ddd > 0 || rewardList[0].epx > 0){
            // No longer stake EPX
            staker.claim(address(this), tokens, 0); // This will claim DDD and EPX
        }
        TradeRouter router = TradeRouter(PANCAKE_ROUTER);
        ...
        IERC20 epx = IERC20(EPX_ADDRESS);
        rewards = epx.balanceOf(address(this)); // Get any EPX here
        if(rewards > 0){
            // Convert via Curvelikedex
            CurveLikeExchange curve = CurveLikeExchange(EPX_BNB_ADDRESS);
            epx.safeApprove(address(curve), 0);
            epx.safeApprove(address(curve), rewards);
            curve.exchange(0, 1, rewards, 1, false); // Will return WBNB
        }
