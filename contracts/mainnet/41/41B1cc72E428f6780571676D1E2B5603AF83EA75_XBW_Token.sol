...
abstract contract DividendFee is Owned, DexBase, ERC20 {
    uint256 constant distributefee = 2;
    uint256 constant marketfee = 1;

    address constant marketAddr = 0x43E0C9355e7f8C486A08ba3965E3D276253B7DC8;
    ...
    constructor(uint256 _numTokenToDividend, address _divToken) {
        DOGE = _divToken;
        numTokenToDividend = _numTokenToDividend;

        allowance[address(this)][address(uniswapV2Router)] = type(uint256).max;
        distributor = new DividendDistributor(DOGE, 21600, 100 ether);
        ...
        distributor.excludeFromDividends(
            0x5AaeE47DCB0caD809c1B39a1b6444CB0B0d3a6C3
        );
        distributor.excludeFromDividends(
            0xED9260FC7318B5dCAa322466f415955eB6186D5A
        );
        distributor.excludeFromDividends(
            0xc01a870310b7CAC3e6BA1701D1A7acC48e7E90D7
        );
    }
    ...
    function swapAndToDividend() internal lockTheSwap {
        ...
        uint256 toLiqUsdt = (swapAmount * distributefee) /
            (distributefee + marketfee);
        uint256 tomarket = swapAmount - toLiqUsdt;
        distributor.transferUSDT(marketAddr, tomarket);
        distributor.distributeDividends(toLiqUsdt);
    }
...
contract XBW_Token is ExcludedFromFeeList, DividendFee {
    uint256 private constant _totalSupply = 2100_0000 * 1e18;

    constructor()
        ERC20(unicode"王阳明心币王", "XBW", 18)
        DividendFee(_totalSupply / 10_0000, USDT)
    {
