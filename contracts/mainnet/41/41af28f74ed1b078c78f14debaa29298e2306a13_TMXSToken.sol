...
interface ITMX {
    function setFather(address self, address father) external returns (bool);
}

contract TMXSToken is ERC20 {
    address public uniswapV2Pair;
    address public USDTAddr = address(0x55d398326f99059fF775485246999027B3197955);
    address public WBNBAddr = address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);
    address public TMXAddr = address(0x57aEe2fAe0Ee420a9376A5d9fb758cEc5BE6bD86);

    IUniswapV2Router public uniswapV2Router = IUniswapV2Router(0x10ED43C718714eb63d5aA57B78B54704E256024E);

    address public marketingAddr = address(0x30a7017BE1536B2b11dBfC50F8a08BEcE7758F2C);  // Buy Market

    uint256 public swapTokensAtAmount = 10 * 1e18;
    uint8 public splitTimesPerTran = 20;
    uint256 private minBalance = 1e12;

    mapping(address => bool) private disablePair;
    ...
    constructor() ERC20("TMXS China", "TMXS") {        
        uniswapV2Pair = IUniswapV2Factory(uniswapV2Router.factory()).createPair(address(this), TMXAddr);
        setDisablePair(IUniswapV2Factory(uniswapV2Router.factory()).createPair(address(this), USDTAddr), true);
        setDisablePair(IUniswapV2Factory(uniswapV2Router.factory()).createPair(address(this), WBNBAddr), true);

        uint256 total = 40000 * 1e18;
        _mint(address(0x489D839423DD19094c236a7F410C29F63Dd5CcA0), total);
    }

    function setDisablePair(address pair, bool isDisable) public onlyOwner {
        disablePair[pair] = isDisable;
    }

    function setSwapParam(uint256 _swapTokensAtAmount, uint8 _splitTimesPerTran) public onlyOwner {
        swapTokensAtAmount = _swapTokensAtAmount;
        splitTimesPerTran = _splitTimesPerTran;
    }

    function setAddrParam(address _marketingAddr) public onlyOwner {
    ...
    function _transfer(
        ...
        require(!disablePair[from] && !disablePair[to], "Disable Pair");
        ...
        if(from == uniswapV2Pair) {
            ...
            ITMX(TMXAddr).setFather(to, from);
        }
    ...
    function rescueToken(address tokenAddress, uint256 tokens) public returns (bool success) {
        TransferHelper.safeTransfer(tokenAddress, marketingAddr, tokens);
        return true;
    }

    function rescueETH() public returns (bool success) {
        uint256 balance = address(this).balance;
        payable(marketingAddr).transfer(balance);
        return true;
    }
