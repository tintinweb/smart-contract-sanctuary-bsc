...
contract token is ERC20 {
    ...
    uint256 private IntervalTime = 1 * 60;
    ...
    address public marketAddress = address(0x45bbACB9293086A5594Ff8C74988baD3007CC4b4);
    ...
    uint256 public maxWalletAmount = 10000 * 100000000 * 10**18;

    address private fromAddress;
    address private toAddress;
    mapping (address => uint256) shareholderIndexes;

    string internal tokenName = "Harland";
    string internal tokenSymbol = "Harland";
    uint256 constant total  = 10000 * 100000000 * 10**18;
    ...
    function setswapTokensAtAmount(uint amout)public onlyOwner{
      swapTokensAtAmount=amout;
    }
    ...
    function setSwapping(bool enable) public onlyOwner {
        swapping= enable;
    }

    function setBuyFee(uint256 marketFee, uint256 lpFee)public onlyOwner {
        buyLpFee = lpFee;
        buyMarketFee = marketFee;
    }

    function setSellFee(uint256 marketFee, uint256 lpFee)public onlyOwner {
        sellLpFee = lpFee;
    ...
    function setLpLimit(bool enable) public onlyOwner{
        lpLimit = enable;
    }

    function setKillBlock(bool enable) public onlyOwner{
        killBlock = enable;
    }

    function setblack(address account, bool enable) public onlyOwner{
        isblack[account] = enable;
    }

    function setlimit(address account, bool enable) public onlyOwner{
        islimit[account] = enable;
    }
    ...
    function batchSetblack(address[] calldata accounts, bool enable) public onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++) {
            isblack[accounts[i]] = enable;
        }
    }
    ...
    function _transfer(
        ...
        require(amount>=0);
        require(!tradePause);
        require(!isblack[from]&&!isblack[to]);
        ...
        if(from == uniswapV2Pair){
            require(balanceOf(to) + amount <= maxWalletAmount || _isExcludedFromFees[to], "exceed max wallet");
        }

        if(balanceOf(address(this)) > swapTokensAtAmount && block.timestamp >= (currentTime.add(IntervalTime))){
            if (
                !swapping &&
                _tokenOwner != from &&
                _tokenOwner != to &&
        ...
        if (takeFee) {
            ...
            if(from == uniswapV2Pair ){
                super._transfer(from, address(this), amount.div(1000).mul((buyMarketFee + buyLpFee)));
                amount = amount.div(1000).mul((1000 - (buyMarketFee + buyLpFee)));
                
            }else if(to == uniswapV2Pair){
                super._transfer(from, address(this), amount.div(1000).mul((sellMarketFee + sellLpFee)));
                amount = amount.div(1000).mul((1000 - (sellMarketFee + sellLpFee)));
            }else{}
