...
contract IDOContract is  Context,  Ownable{
    ...
    string constant public Version = "IDO V1.0.5";
    ...
    function getProportion() public view returns (uint256){
        if (proportion == 0) {
            ...
            uint256 pariProportion = (_price[0] * 100) / _price[1];
    ...
    function AddIDO(address _composer, uint256 _token, uint256 _busd) public returns (uint256 _power){
        ...
        IBEP20(token).transferFrom( _composer,deadWallet,_token);
        ...
        if (buytoken != address(0)) swaping(_busd, deadWallet);
    ...
    function swaping(uint256 _busd, address to) internal {
        address[] memory path = new address[](2);
