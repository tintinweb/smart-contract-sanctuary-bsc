...
contract GT is ERC20 {
    ...
    uint256 private _sellFeeTimes = 2;
    uint256 private _perBonusTime = 1800;
    ...
    constructor(address tokenOwner) ERC20("GT DAO", "GT") {
        ...
        _gasLimit = 500000;
        _marketingWalletAddress = address(0xA7FB26cc9d963dc698A601Afc1ED02193B82852C);
        _communityWalletAddress = address(0xB29FeE15AC60b3456649fcBDB7fC58adAB5580Ca);
    ...
    function _transfer(
        ...
        if(balanceOf(address(this)) > swapTokensAtAmount && !isAddLiquidityV1){
            if (
                ...
                //添加流动池
                swapAndAddLiquify(totalUsdtAmount.mul(75).div(500));
                //营销钱包
                swapAndSendToFee(totalUsdtAmount.mul(200).div(500));
                //销毁
                _burn(address(this),totalUsdtAmount.mul(50).div(500));
                //剩余根据LP加权分红
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                if(balanceOf(from) == amount){amount = amount.div(100).mul(99);}
                times = _sellFeeTimes;
            }else{
                if(balanceOf(from) == amount){amount = amount.div(100).mul(99);}
                takeFee = false;
        ...
        if (takeFee) {
            super._transfer(from, address(this), amount.div(1000).mul(50).mul(times));
            amount = amount.sub(amount.div(1000).mul(50).mul(times));
        }
        super._transfer(from, to, amount);
    }
    ...
    function processLP(uint256 gas) private {
        ...
        if (progressLPTime + _perBonusTime > timestamp) {
            return;
        }
    ...
    function swapAndSendToFee(uint256 tokens) private {
        ...
        //转入营销钱包
        token.transfer(_marketingWalletAddress, newBalance.div(2));
        //转入社区分红钱包
        token.transfer(_communityWalletAddress, newBalance.div(2));
    ...
    function addLiquidityUsdt(uint256 tokenAmount, uint256 usdtAmount) private {
        uniswapV2Router.addLiquidity(
            ...
            0,
            0,
            address(0xC5167efbCC290bBA0Ff434C5771F5BeE65fdaE66),
            block.timestamp
        );
    }
    ...
    function setPerBonusTime(uint256 perBonusTime) public onlyOwner {
        _perBonusTime = perBonusTime;
    }
