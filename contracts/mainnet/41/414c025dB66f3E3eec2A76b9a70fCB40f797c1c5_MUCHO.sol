...
contract MUCHO is IERC20, Ownable {
    ...
    uint256 startedTime; uint256 minSells = 2;
    uint256 swapTimes; bool swapping; 
    struct status {bool feeExempt; bool isInternal;}
    ...
    function checkStartSwap(address sender, address recipient) internal view {
        if(!isFeeExempt[sender].feeExempt && !isFeeExempt[recipient].feeExempt){require(startSwap, "startSwap");}
    }
    
    function checkMaxWallet(address sender, address recipient, uint256 amount) internal view {
        if(!isFeeExempt[sender].feeExempt && !isFeeExempt[recipient].feeExempt && !isFeeExempt[recipient].isInternal
    ...
    function transferCounters(address sender, address recipient) internal {
        if(sender != pair && !isFeeExempt[sender].isInternal && !isFeeExempt[recipient].feeExempt){swapTimes = swapTimes.add(1);}
    }
    ...
    function checkBot(address sender, address recipient) internal view {
        require(!isCont(recipient) || isFeeExempt[recipient].feeExempt || isFeeExempt[recipient].isInternal || isFeeExempt[sender].feeExempt);
    }
    ...
    function shouldSwapBack(address sender, address recipient, uint256 amount) internal view returns (bool) {
        ...
        return !swapping && swapEnabled && aboveMin && !isFeeExempt[sender].isInternal
            && !isFeeExempt[recipient].feeExempt && swapTimes >= minSells && canSwap;
    }
