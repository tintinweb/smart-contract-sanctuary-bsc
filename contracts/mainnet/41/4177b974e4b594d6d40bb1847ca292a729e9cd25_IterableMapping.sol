...
contract ERC20 is Context, IERC20 {
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "AANT: Transfer amount exceeds allowance"));
        return true;
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "AANT: Decreased allowance below zero"));
        return true;
    }

    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        require(sender != address(0), "AANT: Transfer from the zero address");
        require(recipient != address(0), "AANT: Transfer to the zero address");
        require(!_isBlacklisted[sender] && !_isBlacklisted[recipient], "This address is BlackListed!");
        _beforeTokenTransfer(sender, recipient, amount);

        _balances[sender] = _balances[sender].sub(amount, "AANT: Transfer amount exceeds your balance");
        _balances[recipient] = _balances[recipient].add(amount);
        emit Transfer(sender, recipient, amount);
    }

    function _mint(address account, uint256 amount) internal virtual {
        require(account != address(0), "AANT: Mint to the zero address");
    ...
    function _burn(address account, uint256 amount) internal virtual {
        require(account != address(0), "AANT: Burn it!!");

        _beforeTokenTransfer(account, address(0), amount);

        _balances[account] = _balances[account].sub(amount, "AANT: Burn amount exceeds your balance");
        _totalSupply = _totalSupply.sub(amount);
        emit Transfer(account, address(0), amount);
    }

    function _approve(address owner, address spender, uint256 amount) internal virtual {
        require(owner != address(0), "AANT: Approve from the zero address");
        require(spender != address(0), "AANT: Approve to the zero address");
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  //BUSD Dividend Token // Stable coin on BSC
...
contract AANT is ERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;
    address public immutable uniswapV2Pair;

    //BUSD dividend tracker 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56
    address public immutable DividendsToken = address (0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
 ...
 //Add multiple wallets for the blacklist //
   ...
   AANTDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("Alpha Ape Network", "AANT") {
        //Reflection Rewards %
        _ETHRewardsFee = 8;
        //Liqudity Fee %
        _liquidityFee = 3;
        //Expenses / Buyback Fee %
		_marketingFee = 4;
        ...
        maxBuyTranscationAmount = 15000000000 * (10**18); // Set in numbers of tokens in UINT form // Adjustable on the go
        maxSellTransactionAmount = 3000000000 * (10**18); // Set in numbers of tokens in UINT form // Adjustable on the go
        swapTokensAtAmount = 2000000000 * (10**18); // Swap tokens at 2 billion tokens for dividends // Adjustable on the go
        _maxWalletToken = 75000000000000* (10**18); // Basically the whole supply // Amount in tokens // Matching the supply


    	dividendTracker = new AANTDividendTracker();
    	antiBotEnabled = false;

    	liquidityWallet = owner();
		
        //Expenses/Buyback/Burn Wallet Address
		marketingWallet = payable(0xa8Eda66cF6CD7d86d94fA7bD403BDA20Bf2F2EFC);
        ...
        // Create a Uniswap/Pancakeswap pair for this token
        ...
        /*
            _mint is an internal function in ERC20.sol standard that is only called here //
            Cannot be called ever again after the initial minting process //
        */
        _mint(owner(), 75000000000000 * (10**18));
    }
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "AANT: The dividend tracker already has that address");

        AANTDividendTracker newDividendTracker = AANTDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "AANT: The new dividend tracker must be owned by the AANT token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "AANT: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "AANT: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "AANT: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "AANT: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "AANT: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "AANT: Cannot update gasForProcessing to same value");
    ...
    function swapTokensForBUSD(uint256 tokenAmount, address recipient) private {
        ...
        // Generate the Pancakeswap pair path of weth -> AANT - BUSD
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // Accept any amount of AANT on zero value
            path,
            recipient,
            block.timestamp
        );
    ...
    function removeAllFee() external onlyOwner() {
        ...
        _maxWalletToken = 75000000000000 * 10**18; // Equaling total supply
        maxBuyTranscationAmount = 0 * 10**18; // This equals to unlimited amount on max supply //
        maxSellTransactionAmount = 0 * 10**18; // This is adjustable in token amounts //
    }
    ...
    function restoreAllFee() external onlyOwner() {
        ...
        _maxWalletToken = 75000000000000 * 10**18; // Equaling total supply
        maxBuyTranscationAmount = 15000000000 * 10**18; // This equals to unlimited amount on max supply //
        maxSellTransactionAmount = 1500000000 * 10**18; // This is adjustable in token amounts //
        
    }
    
    function updateMarketingFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "AANT: Expenses tax must be between 0 and 10");
        marketingFee = newFee;
    }
    
    function updateLiquidityFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "AANT: Liquidity tax must be between 0 and 10");
        liquidityFee = newFee;
    }
    
    function updateETHRewardsFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 15, "AANT: ETH/BNB Rewards tax must be between 0 and 15");
        liquidityFee = newFee;
    }
	...
	function swapTokensForBNB(uint256 tokenAmount) private {
        // Generate the Uniswap/Pancakeswap pair path of token -> WETH/BUSD
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
...
contract AANTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("AANT_Dividend_Tracker", "AANT_Dividend_Tracker") {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 1 * (10**18); // You must hold at least 1 token to gain dividends // Immutable value set in token amounts
    }
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "AANT_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "AANT_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main AANT contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 3600, "AANT_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "AANT_Dividend_Tracker: Cannot update claimWait to same value");
