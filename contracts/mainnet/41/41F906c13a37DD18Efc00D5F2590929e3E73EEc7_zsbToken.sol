...
interface StabilizeStrategy {
    ...
    function withdrawTokenReserves() external returns (address, uint256);
    ...
    function withdrawAvailable(uint256, uint256) external returns (bool); // Unique to certain strategies, such as Dforce
}

pragma solidity =0.6.6;

contract zsbToken is ERC20("Stabilize Strategy BSC BUSD", "zsb-BUSD"), Ownable, ReentrancyGuard {
    ...
    function withdrawAvailable(uint256 _share) public returns (bool) {
        if(totalSupply() == 0){
            return false;
        }
        if(currentStrategy != StabilizeStrategy(address(0))){
            return StabilizeStrategy(currentStrategy).withdrawAvailable(_share, totalSupply());
        }else{
            return true;
        }
    }
    ...
    function valueOfVaultAndStrategy() public returns (uint256) { // The total value of the tokens
