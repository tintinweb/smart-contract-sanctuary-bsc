...
// File: contracts/IKalmySwapBestRateQuery.sol

pragma solidity >=0.5.0;

interface IKalmySwapBestRateQuery {
  function oneRoute(
    address src,
    address dest,
    uint256 srcAmount,
  ...
  function kalmyswap() external returns (address);
}

// File: contracts/IKalmySwapRouter.sol

pragma solidity >=0.5.0;

interface IKalmySwapRouter {
...
contract SwapV2 is Ownable, ReentrancyGuard {
  using SafeToken for address;
  using SafeMath for uint256;

  IKalmySwapBestRateQuery public query;
  IKalmySwapRouter public bestrate;
  uint256[] public routerIndex;
  uint256 public partnerIndex;
  address public etherERC20;

  mapping(address => bool) public okStrats;

  constructor(IKalmySwapBestRateQuery _query, uint256[] memory _routerIndex) public {
    query = _query;
    bestrate = IKalmySwapRouter(_query.kalmyswap());
    routerIndex = _routerIndex;
    partnerIndex = 0;
    etherERC20 = bestrate.etherERC20();
  }
  ...
  function swapQuery(uint256 amount, address src, address dest) external payable nonReentrant onlyStrat {
    if (etherERC20 != src) {
    ...
    if (etherERC20 == dest) {
      ...
      beforeBalance = dest.myBalance();
    }

    (uint256 routeIndex,) = query.oneRoute(src, dest, amount, routerIndex);
    if (etherERC20 == src) {
      bestrate.trade.value(msg.value)(routeIndex, src, amount, dest, 0);
    } else {
      bestrate.trade(routeIndex, src, src.myBalance(), dest, 0);
    }

    if (etherERC20 == dest) {
      uint256 afterBalance = address(this).balance.sub(beforeBalance);
      SafeToken.safeTransferETH(msg.sender, afterBalance);
    } else {
      SafeToken.safeApprove(src, address(bestrate), 0);
      SafeToken.safeTransfer(dest, msg.sender, dest.myBalance().sub(beforeBalance));
    }
  }

  function swap(uint256 amount, address src, address dest, uint256 routeIndex) external payable nonReentrant onlyStrat {
    if (etherERC20 != src) {
      require(amount <= SafeToken.balanceOf(src, msg.sender), "insufficient balance");
      SafeToken.safeTransferFrom(src, msg.sender, address(this), amount);
      SafeToken.safeApprove(src, address(bestrate), uint(-1));
  ...
  function setQuery(IKalmySwapBestRateQuery _query, uint256[] calldata _routerIndex) external onlyOwner nonReentrant {
    query = _query;
    bestrate = IKalmySwapRouter(_query.kalmyswap());
    routerIndex = _routerIndex;
    etherERC20 = bestrate.etherERC20();
