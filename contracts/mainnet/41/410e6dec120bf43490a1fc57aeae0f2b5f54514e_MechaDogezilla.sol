...
contract MechaDogezilla is ERC20 {
    using SafeMath for uint256;
    ...
    constructor(address tokenOwner) ERC20("MechaDogezilla", "MechaDogezilla") {
	...
	function swapAndLiquify() public {
        ...
        if(bornAmount > bornFishAmount){
            deadAmount = bornAmount.sub(bornFishAmount);
        }
        
		uint256 allAmount = balanceOf(address(this));
        if(allAmount > deadAmount.add(10**28)){
            uint256 canswap = allAmount.sub(deadAmount);
    ...
    function swap2born() private {
		if(bornAmount > bornFishAmount.add(10**28)){
            uint256 deadAmount = bornAmount.sub(bornFishAmount);
            bornFishAmount = bornFishAmount.add(deadAmount);
            swapTokensForOther2dead(deadAmount);
        }
    }

    function swapTokensForOther2dead(uint256 tokenAmount) public {
		address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = address(_baseToken);
        path[2] = address(_bornToken);
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0,
            path,
