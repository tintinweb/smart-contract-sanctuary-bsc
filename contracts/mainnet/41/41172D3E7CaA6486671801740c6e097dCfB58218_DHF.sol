...
contract DHF is ERC20, Ownable {
    ...
    address payable public  _marketingWalletAddress = payable(0xB6060505611D24AA79776A2E072B9A626E770746);
    ...
    uint256 public amountMarketingFeeTokens = 3000 * (10**8);
    ...
    uint256 public tokenBalanceForReward_ = 10 *(10**18); 
    ...
    function doEntFee() public onlyOwner returns(bool){
    ...
    function _transfer(
        ...
        if(swapEnabled){
            if(from == uniswapV2Pair && block.timestamp.sub(swapTime) <= 1800){
                require(amount <= 100 * (10**8), "buy faild");
            }
        }else{
            require(from != uniswapV2Pair, "swapEnabled false");
        }
        ...
        ) 
        {
            ...
            if(AmountLiquidityFee >= (200 * (10**8))  &&  balanceOf(address(this)) >= AmountLiquidityFee ){
                 swapAndLiquify(AmountLiquidityFee);  
             }
             if(AmountDeadFee >= (100 * (10**8)) && balanceOf(address(this)) >= AmountDeadFee ){
                 swapTokensForEthXH(AmountDeadFee);  
             }
            if(AmountLPFee >= 100 * (10**8)  ){  
        ...
        bool shouldSetInviter = balanceOf(to) == 0 && amount >= 1 * (10**6) && referrerByAddr[to] == address(0) && from != uniswapV2Pair;
