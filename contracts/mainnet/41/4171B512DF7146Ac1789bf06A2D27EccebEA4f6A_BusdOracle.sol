...
contract BusdOracle is Context, Ownable {
    ...
    function deposit(address _ref, uint256 _amount) public  {
        ...
        uint256 total_contract = _amount - total_fee * 2; //2% for dev and mkt each
        BusdInterface.transferFrom(msg.sender,dev,total_fee);
        BusdInterface.transferFrom(msg.sender,mkt,total_fee);
    ...
    uint256 totalAmountToWithdraw = aval_withdraw - wFee * 2; // 2% for dev and mkt each
    BusdInterface.transfer(msg.sender,totalAmountToWithdraw);
    BusdInterface.transfer(dev,wFee);
    BusdInterface.transfer(mkt,wFee);
    ...
    function changeDev (address _dev, address _mkt) external onlyOwner {
        dev = _dev;
        mkt = _mkt;
    }
