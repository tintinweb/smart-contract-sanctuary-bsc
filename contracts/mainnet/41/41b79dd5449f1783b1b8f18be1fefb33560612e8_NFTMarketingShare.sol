...
interface NFTMarketing {
    ...
    function toSilverTimeList(address _user) external view returns(uint256 _time);
}
...
contract NFTMarketingShare is Ownable {
    ...
    function setShare() public {
        ...
        userInfoItem memory _userInfo = NFTMarketingAddress.getUserInfo(address(this),false);
        uint256 _shareNumber = _userInfo.allBronzeNum;
    ...
    function claimById(uint256 _shareId) public returns (uint256){
        ...
        uint256 _time = NFTMarketingAddress.toSilverTimeList(msg.sender);
        configItem memory config = NFTMarketingAddress.config();
        userInfoItem memory _userInfoNew = NFTMarketingAddress.getUserInfo(msg.sender,false);
        bool isGold = _userInfoNew.isGold;
        bool isSilver = _userInfoNew.isSilver;
        bool isBronze = _userInfoNew.isBronze;
        if (!isGold && !isSilver && !isBronze) {
            return 0;
        }
        if ((_time == 0 || _time > shareList[_shareId]._createTime) && !claimStatusList[msg.sender][_shareId]) {
    ...
    function getUserInfoForShare(address _user, uint256 _shareId) public view returns (userInfoItem2 memory _userInfo) {
        uint256 _time = NFTMarketingAddress.toSilverTimeList(_user);
        userInfoItem memory _userInfoNew = NFTMarketingAddress.getUserInfo(_user,false);
        bool isGold = _userInfoNew.isGold;
        bool isSilver = _userInfoNew.isSilver;
        bool isBronze = _userInfoNew.isBronze;
        _userInfo._claimAmount = shareList[_shareId]._claimAmountPerShare;
        _userInfo._inWhiteList = (_time == 0 || _time > shareList[_shareId]._createTime) && !claimStatusList[_user][_shareId] && (isGold || isSilver || isBronze);
