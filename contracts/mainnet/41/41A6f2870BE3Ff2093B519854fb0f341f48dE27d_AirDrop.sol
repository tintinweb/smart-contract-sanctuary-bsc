...
contract AirDrop is Ownable {
    ...
    function distributeAirDrop(address _tokenAirdrop, address[] memory _listWallet, uint256 _amounts) public {
        require(isAirdrop[msg.sender], "Must have permission");
        for (uint256 i = 0; i < _listWallet.length; i++) {
            IBEP20(_tokenAirdrop).safeTransfer(_listWallet[i], _amounts);
        }
    }
