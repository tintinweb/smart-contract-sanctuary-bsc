...
contract LiberaV2 is Initializable, UUPSUpgradeable, AuthUpgradeable, IERC20Upgradeable  {
    ...
    function _migrate() external onlyOwner { //can call this only once 
        ...
        _transfer(address(this), deadAddress,deadBalance);
        uint256 burnBalance=IERC20Upgradeable(liberaV1).balanceOf(burnTreasury);
        _transfer(address(this), burnTreasury,burnBalance);
        uint256 liberoBalance=IERC20Upgradeable(liberaV1).balanceOf(liberoTreasury);
        _transfer(address(this), liberoTreasury,liberoBalance);
        _transfer(address(this), owner,balanceOf(address(this))); // owner will transfer to current holders
