...
// File: MAGIC-Token/contracts/interfaces/IRewardDistributor.sol
...
// File: MAGIC-Token/contracts/interfaces/IPYESwapRouter01.sol
...
// File: MAGIC-Token/contracts/interfaces/IPYESwapRouter.sol
...
// File: MAGIC-Token/contracts/interfaces/IPYESwapPair.sol
...
// File: MAGIC-Token/contracts/interfaces/IPYESwapFactory.sol
...
// File: MAGIC-Token/contracts/interfaces/IWETH.sol
...
// File: MAGIC-Token/contracts/interfaces/IPYE.sol
...
// File: MAGIC-Token/contracts/RewardDistributor.sol
...
contract RewardDistributor is IRewardDistributor{
    ...
    constructor () {
        pyeSwapRouter = IPYESwapRouter(0xEaAdD17983842db731Bd4b4dC0DbD005F4444eB6);
...
// File: MAGIC-Token/contracts/MAGIC.sol
...
contract MAGIC is IPYE, Context, ERC20, ERC20Snapshot, Ownable {
...
//--------------------------------------BEGIN TOKEN PARAMS---------|
    ...
    string constant _name = "BullTest";
    string constant _symbol = "BTEST";
    uint8 constant _decimals = 9;
    uint minimumBuyBackThreshold = _tTotal / 1000000;
...
//--------------------------------------BEGIN BUYBACK VARIABLES---------|
    ...
    uint256 public autoBuybackAmount = 1 * 10 ** 16;
    uint256 autoBuybackBlockPeriod;
    uint256 autoBuybackBlockLast;

    event BuybackMultiplierActive(uint256 duration);

//--------------------------------------BEGIN REWARD DISTRIB---------|
...
//--------------------------------------BEGIN CONSTRUCTOR AND RECEIVE FUNCITON---------|    
    ...
    constructor() ERC20("Magic", "MAGIC") {
        ...
        _defaultFees = Fees(
            ...
            200,
            200,
            0xAb705Bb28d885af07F14312F548C4f5E6BCE01a4
        );
...
//--------------------------------------BEGIN TOKEN GETTER FUNCTIONS---------|
    ...
    // Magic uses "_tTotal" variable to define total token supply, 
...
//--------------------------------------BEGIN APPROVAL & ALLOWANCE FUNCTIONS---------|
    ...
    // approve fxn overrides std. ERC-20 approve() fxn which declares address owner = _msgSender(), whereas Magic approve() fxn does not.
...
//--------------------------------------BEGIN FEE FUNCTIONS---------|
        ...
        if(tokenIndex < pairsLength) {
            ...
            if(allowanceT >= amount) {
                ...
                if(token != WBNB) {
                    ...
                    IERC20(WBNB).transfer(distributorAddress, reflectionFeeAmount);
                    try distributor.deposit(WBNB, reflectionFeeAmount) {} catch {}
                } else {
                    uint256 totalFee = getTotalFee();
    ...
    function renounceDevelopmentFee() external {
        require(msg.sender == _defaultFees.developmentAddress , "Not the dev address!");
        _defaultFees.developmentFee = 0;
        _updatePairsFee();
    }
...
//--------------------------------------BEGIN BUYBACK FUNCTIONS---------|
    ...
    // logic to purchase magic tokens
