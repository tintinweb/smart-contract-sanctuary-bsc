...
interface IStakeERC721EarnToken {
    ...
    event SetTokenPerSecond(address indexed user, uint256 tokenPerSecond);
    ...
    function setTokenPerSecond(uint256 _tokenPerSecond) external;
...
contract StakeERC721EarnToken is IStakeERC721EarnToken, ERC721Holder, Ownable, Pausable, ReentrancyGuard {
    ...
    function setMint(bool _isMint) public onlyOwner {
        isMint = _isMint;
    }
    ...
    function setTokenPerSecond(uint256 _tokenPerSecond) external override onlyOwner {
        require(tokenPerSecond != _tokenPerSecond, ' NOT NEED UPDATE');
        massUpdatePools();
        emit SetTokenPerSecond(_msgSender(), _tokenPerSecond);
        tokenPerSecond = _tokenPerSecond;
    }
    ...
    function getTotalReward(uint256 _from, uint256 _to) public view override returns (uint256 totalReward) {
        ...
        return _to.sub(_from).mul(tokenPerSecond);
    }
