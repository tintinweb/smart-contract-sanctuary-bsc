...
library Address{
 ...
 contract EssentialBot is Context, IERC20, Ownable {
    ...
    address public marketingWallet = 0x0c72EB4edd47ee7B1f696b69C59e52c9FA531A19;
    address public researchWallet = 0x0c72EB4edd47ee7B1f696b69C59e52c9FA531A19;



    string private constant _name = "Essential Bot";
    string private constant _symbol = "ESSEBOT";
    ...
    Taxes public taxes = Taxes(0, 7, 0, 1);
    Taxes public sellTaxes = Taxes(0, 8, 0, 2);
    ...
    constructor (address routerAddress) {
        ...
        allowedTransfer[researchWallet] = true;
    ...
    function setTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _research) public onlyOwner {
       taxes = Taxes(_rfi,_marketing,_liquidity,_research);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _research) public onlyOwner {
       sellTaxes = Taxes(_rfi,_marketing,_liquidity,_research);
        emit FeesChanged();
    }
    ...
    function _takeresearch(uint256 rresearch, uint256 tresearch) private {
        totFeesPaid.research +=tresearch;

        if(_isExcluded[address(this)])
        {
            _tOwned[address(this)]+=tresearch;
        }
        _rOwned[address(this)] +=rresearch;
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rresearch) = _getRValues2(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.tresearch = tAmount*temp.research/100;
        s.tTransferAmount = tAmount-s.tRfi-s.tMarketing-s.tLiquidity-s.tresearch;
        return s;
    }
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rMarketing, uint256 rLiquidity){
        ...
        uint256 rresearch = s.tresearch*currentRate;
        rTransferAmount =  rAmount-rRfi-rMarketing-rLiquidity-rresearch;
        return (rAmount, rTransferAmount, rRfi,rMarketing,rLiquidity);
    }
    
    function _getRValues2(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 rresearch) {

        if(!takeFee) {
          return(0);
        }

        rresearch = s.tresearch*currentRate;
        return (rresearch);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(tradingEnabled && block.number <= genesis_block + 5) {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tMarketing + s.tresearch);
        }
        if(s.rMarketing > 0 || s.tMarketing > 0) _takeMarketing(s.rMarketing, s.tMarketing);
        if(s.rresearch > 0 || s.tresearch > 0) _takeresearch(s.rresearch, s.tresearch);
        emit Transfer(sender, recipient, s.tTransferAmount);
        
    }

    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.marketing + temp.research) * 2;
        ...
        uint256 researchAmt = unitBalance * 2 * temp.research;
        if(researchAmt > 0){
            payable(researchWallet).sendValue(researchAmt);
        }
    }
