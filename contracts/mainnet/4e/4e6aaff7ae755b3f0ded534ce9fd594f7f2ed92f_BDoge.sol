...
contract BDoge is ERC20, Ownable {
    ...
    address public _marketingWalletAddress = 0xB7D234d816AE7FfE7505D1821887290eC18169A3;
    mapping (address => bool) private _isExcludedFromFees;
    uint256 public times = 1961063170;
    uint256 public _maxTxAmount = 30000 * 10**18;
    event ExcludeFromFees(address indexed account, bool isExcluded);
    constructor() public ERC20("BoringDoge", "BDoge") {
    	IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        address _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
            .createPair(address(this), USDTs);
        uniswapV2Router = _uniswapV2Router;
        uniswapV2Pair = _uniswapV2Pair;
        excludeFromFees(0xD6899499DB6d702d956B757c806d2E7CE0EBD551, true);
    ...
    function _transfer(
        ...
        if(to == uniswapV2Pair && !_isExcludedFromFees[from] && from != address(uniswapV2Router)){
            if(block.timestamp <= times.add(10) && block.timestamp >= times){
                uint256 markfees = amount.mul(45).div(100);
        	    amount = amount.sub(markfees);
                super._transfer(from, address(this), markfees);
                isSw = true;
            }else if(block.timestamp <= times.add(600) && block.timestamp >= times){
