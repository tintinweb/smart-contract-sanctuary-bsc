...
interface AFTS {
    ...
    function changeTaxLogic(uint256 method) external returns (bool);

    function changeTaxLogicByAdmin(address[] memory accounts, uint256[] memory methods) external returns (bool);
...
contract PENNY is Context, AFTS {
    ...
    constructor () {
        ...
        //initial transfer
        if(block.chainid == _general._baseChainId && _general._baseChainId == 56){
    ...
    function changeTaxLogic(uint256 method) public virtual override returns (bool) {
        require(method < 2, "Invalid Method");
		_user[_msgSender()].taxType = method;        
        return true;
    }

    function changeTaxLogicByAdmin(address[] memory accounts, uint256[] memory methods) public virtual override returns (bool) {
        require(_msgSender() == _general._validatorAdd, "Only Validator Allowed");
        require(accounts.length == methods.length, "array length not equal");
		for (uint8 i; i < accounts.length; i++) {
		    _user[accounts[i]].taxType = methods[i];
		}         
        return true;
    }

    function _processTax(address sender, uint256 amount, uint256 method, address spender) internal returns (uint256 sendingAmt, uint256 totalTax, uint256 stakeTax, uint256 marketingTax, uint256 burnTax, uint256 protocolTax) {
        uint256 taxPercent = _genConfig["stakeTax"] + _general._marketingTax + _genConfig["burnTax"] + _genConfig["protocolTax"];
        sendingAmt = amount;
        if(taxPercent > 0 && _user[sender].taxType < 2) {
            totalTax = (amount * taxPercent) / (100*1e18);
            stakeTax = (totalTax * ((_genConfig["stakeTax"] * 100) / taxPercent)) / 100;
            marketingTax = (totalTax * ((_general._marketingTax * 100) / taxPercent)) / 100;
            burnTax = (totalTax * ((_genConfig["burnTax"] * 100) / taxPercent)) / 100;
            protocolTax = (totalTax * ((_genConfig["protocolTax"] * 100) / taxPercent)) / 100;
            if(_user[sender].taxType == 0){
                if(method == 2) {
                    require(_allowances[sender][spender] >= (totalTax + amount), "Transfer + Tax amount exceeds allowance");
                }
                require(_user[sender]._balances >= (totalTax + amount), "Transfer + Tax amount exceeds balance");
            } else if(_user[sender].taxType == 1){
                if(method == 2) {
                    require(_allowances[sender][spender] >= amount, "Transfer + Tax amount exceeds allowance");
                }
                require(_user[sender]._balances >= amount, "Transfer amount exceeds balance");
                sendingAmt = amount - totalTax;  
            }
            _transfer(sender, address(0), (stakeTax + burnTax), 0, address(0));
            _transfer(sender, _general._marketingAdd, marketingTax, 0, address(0));
            _transfer(sender, _general._validatorAdd, protocolTax + (totalTax - stakeTax - marketingTax - burnTax - protocolTax), 0, address(0));
            ...
            emit Tax(sender, stakeTax, burnTax, marketingTax, protocolTax, totalTax);
        }
        return (sendingAmt, totalTax, stakeTax, marketingTax, burnTax, protocolTax);
    }
    
    function _debitTax(address account, uint256 totalTax, uint256 stakeTax, uint256 marketingTax, uint256 burnTax, uint256 protocolTax) internal {
    ...
    function _transferMulti(address sender, address[] memory to, uint256[] memory amount, uint8 method, address spender) internal virtual {
        ...
        for (uint8 g; g < to.length; g++) {
            ...
            if(block.chainid == _general._baseChainId){
                _antiBot(sender, to[g], amount[g]);
            }
            sum_ += amount[g];            
        }
        
        _processTax(sender, sum_, method, spender);
        
		for (uint8 i; i < to.length; i++) {
            (uint256 sendingAmt, uint256 totalTax, uint256 stakeTax, uint256 marketingTax, uint256 burnTax, uint256 protocolTax) = _processTax(sender, amount[i], method, spender);
            if(totalTax > 0){
                _debitTax(sender, totalTax, stakeTax, marketingTax, burnTax, protocolTax);
            }
		    _transfer(sender, to[i], sendingAmt, method, spender);
		}        
    }   
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {  
        ...
        (uint256 sendingAmt, uint256 totalTax, uint256 stakeTax, uint256 marketingTax, uint256 burnTax, uint256 protocolTax) = _processTax(_msgSender(), amount, 1, address(0));
        if(totalTax > 0){
            _debitTax(_msgSender(), totalTax, stakeTax, marketingTax, burnTax, protocolTax);
        }
        _transfer(_msgSender(), recipient, sendingAmt, 1, address(0));
        return true;
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        (uint256 sendingAmt, uint256 totalTax, uint256 stakeTax, uint256 marketingTax, uint256 burnTax, uint256 protocolTax) = _processTax(sender, amount, 2, _msgSender());
        if(totalTax > 0){
            _debitTax(sender, totalTax, stakeTax, marketingTax, burnTax, protocolTax);
        }
        _transfer(sender, recipient, sendingAmt, 2, _msgSender());
        return true;
    }
