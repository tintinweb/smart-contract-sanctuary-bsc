...
contract Highest is ERC20Detailed, Ownable, WhitelistedRole {
    ...
    address public reserveReceiver = 0x04215B38129d1A829015876c9Bde475EB9f00907;
    address public riskFreeValueReceiver = 0x9CbaA173f135BBe71ec93B2a7305A5Ee79C18139;
    ...
    uint256 public reserveFee = 5;
    uint256 public buyFeeRFV = 3;
    uint256 public sellFeeReserveAdded = 5;
    uint256 public sellFeeRFVAdded = 4;
    uint256 public totalBuyFee = liquidityFee.add(reserveFee).add(buyFeeRFV);
    uint256 public totalSellFee = totalBuyFee.add(sellFeeReserveAdded).add(sellFeeRFVAdded);
    ...
    constructor() ERC20Detailed("Highest","HGT", uint8(DECIMALS)) {
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountToReserve = contractTokenBalance.sub(amountToLiquify).sub(amountToRFV);
        ...
        if(amountToReserve > 0){
            _swapTokensForBNB(amountToReserve, reserveReceiver);
        }

        emit SwapBack(contractTokenBalance, amountToLiquify, amountToRFV, amountToReserve);
    }
    ...
    function setFeeReceivers(address _liquidityReceiver, address _reserveReceiver, address _riskFreeValueReceiver) external onlyOwner {
        liquidityReceiver = _liquidityReceiver;
        reserveReceiver = _reserveReceiver;
        riskFreeValueReceiver = _riskFreeValueReceiver;
    }

    function setFees(uint256 _liquidityFee, uint256 _riskFreeValue, uint256 _reserveFee, uint256 _sellFeeReserveAdded, uint256 _sellFeeRFVAdded, uint256 _feeDenominator) external onlyOwner {
        require(
            _liquidityFee <= MAX_FEE_RATE &&
            _riskFreeValue <= MAX_FEE_RATE &&
            _reserveFee <= MAX_FEE_RATE &&
            _sellFeeReserveAdded <= MAX_FEE_RATE &&
       ...
       reserveFee = _reserveFee;
        sellFeeReserveAdded = _sellFeeReserveAdded;
        sellFeeRFVAdded = _sellFeeRFVAdded;
        totalBuyFee = liquidityFee.add(reserveFee).add(buyFeeRFV);
        totalSellFee = totalBuyFee.add(sellFeeReserveAdded).add(sellFeeRFVAdded);
    ...
    event SwapBack(uint256 contractTokenBalance,uint256 amountToLiquify,uint256 amountToRFV,uint256 amountToreserve);
