...
contract GroAsia  {
    ...
    function start_staking(uint256 tokenQty, uint256 _programId, address referrer) public 
    {
		...
		if(isNew){
			...
			if(users[users[msg.sender].referrer].restRefIncome>0 && users[users[msg.sender].referrer].nextWithdrawDate>block.timestamp)
            {
                ...
                {
                  groToken.transfer(users[msg.sender].referrer, users[users[msg.sender].referrer].restRefIncome);                           
                  emit ReferralReward(users[msg.sender].referrer, msg.sender, 0, users[users[msg.sender].referrer].restRefIncome); 
    ...
    function real_estate_staking_admin(address _user, address referrer, uint256 tokenQty) public 
    {
		...
		if(isNew){
            ...
            if(users[users[_user].referrer].restRefIncome>0 && users[users[_user].referrer].nextWithdrawDate>block.timestamp)
            {
                if(users[users[_user].referrer].restRefIncome>refIncome)
                {
                  groToken.transfer(users[_user].referrer, refIncome);
                  users[users[_user].referrer].restRefIncome-=refIncome;
                  emit ReferralReward(users[_user].referrer, _user, 0, refIncome); 
                }
                else
                {
                  groToken.transfer(users[_user].referrer, users[users[_user].referrer].restRefIncome);                           
                  emit ReferralReward(users[_user].referrer, _user, 0, users[users[_user].referrer].restRefIncome); 
                  users[users[_user].referrer].restRefIncome=0;
                }                        
            } 
		}
