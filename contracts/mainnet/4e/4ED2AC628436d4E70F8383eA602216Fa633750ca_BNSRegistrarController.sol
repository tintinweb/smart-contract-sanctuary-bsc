...
contract BNSRegistrarController is Ownable {
   ...
   // NftWlInfo public nftWlInfo;
    ...
    function registerWithConfigAdm(string memory name, /*address owner, uint duration, bytes32 secret,*/ address resolver/*, address addr*/) public payable {
   ...
   /* function wlRegister(string calldata name, address owner, uint duration, bytes32 secret,uint wlNum,uint[] calldata tokenIds,bytes32[] calldata _merkleProof) external payable {
    ...
    //roundTime 1   16-20
    function wlTwitRegisterWithConfig(string calldata name, bytes32 secret,bytes32[] calldata _merkleProof, address[] calldata  ora /*address owner,address resolver, address addr*/) external payable {
        ...
        if (!reserveAdm[msg.sender])
            require(MerkleProofUpgradeable.verify(_merkleProof, wlMerkleRoot, vars.leaf),"Invalid Proof." );
        vars.owner = ora[0];
        vars.resolver = ora[1];
        vars.addr = ora[2];
    ...
    function wlRegisterWithConfig(string calldata name, bytes32 secret,bytes32[] calldata _merkleProof, address[] calldata  ora ) external payable { // ora:address owner,address resolver, address addr
    ...
    function registerWithConfig_34Num(string calldata name, bytes32 secret, address[] calldata  ora /*address owner,address resolver, address addr*/) external payable {
