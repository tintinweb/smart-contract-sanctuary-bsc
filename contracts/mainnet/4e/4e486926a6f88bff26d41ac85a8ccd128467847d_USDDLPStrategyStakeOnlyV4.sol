...
// File: contracts/strategies/USDDLPStrategyStakeOnlyV4.sol
...
interface DotDotStaker{
    ...
    function claim(address _receiver, address[] calldata _tokens, uint256 _maxBondAmount) external; // MaxBondAmount indicates amount of EPX to stake as DEPX
    function claimExtraRewards(address _receiver, address pool) external;
    function claimable(address _user, address[] calldata _tokens) external view returns (DotDotStaker.Amounts[] memory); // DDD x 3 = with bonus
    function userBalances(address _user, address token) external view returns (uint256);
}

interface DotDotLocker{
    function claim(address _user, address[] calldata _tokens) external returns (uint256[] memory claimedAmounts); // Claims fee tokens from staked DEPX
    function deposit(address _user, uint256 _amount) external returns (bool); // Deposits DEPX into the locker
    function initiateUnbondingStream(uint256 _amount) external returns (bool); // Starts stream of withdrawing DEPX
    function withdrawUnbondedTokens(address _receiver) external returns (bool); // Withdraw any unbonded DEPX
}
...
contract USDDLPStrategyStakeOnlyV4 is Ownable {
    using SafeMath for uint256;
    ...
    address constant USDD_ADDRESS = address(0xd17479997F34dd9156Deef8F95A52D81D265be9c);
    address constant BUSD_ADDRESS = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
    address constant DOTDOT_ADDRESS = address(0x84c97300a190676a19D1E13115629A11f8482Bd1);
    address constant DEPX_ADDRESS = address(0x772F317ec695ce20290b56466b3f48501ba81352);
    
    address constant PANCAKE_ROUTER = address(0x10ED43C718714eb63d5aA57B78B54704E256024E); // Router v2
    address constant USDD_BUSD_POOL_ADDRESS = address(0x408A61e158D7BC0CD339BC76917b8Ea04739d473); // USDD, BUSD
    address constant DOTDOT_STAKING_ADDRESS = address(0x8189F0afdBf8fE6a9e13c69bA35528ac6abeB1af);
    address constant DOTDOT_LOCKER_ADDRESS = address(0xd4F7b4BC46e6e499D35335D270fd094979D815A0);
    ...
    function setupWithdrawTokens() internal {
        // Start with USDDLPv2 (USDD/BUSD)
        IERC20 _token = IERC20(address(0xB343F4cDE5e2049857898E800CD385247e21836D));
        tokenList.push(
            TokenInfo({
                token: _token,
    ...
    function getLPInStaker(address _user) public view returns (uint256) {
        DotDotStaker staker = DotDotStaker(DOTDOT_STAKING_ADDRESS);
        uint256 amount = staker.userBalances(_user, address(tokenList[0].token));
        return amount;
    }
    ...
    function deposit(bool nonContract) public onlyZSBToken {
        ...
        if(nonContract == true){}
        stakeLP(); // This will stake any LP in the contract into the Ellipsis staker
    ...
    function withdraw(address _depositor, uint256 _share, uint256 _total, bool nonContract) public onlyZSBToken returns (uint256) {
        ...
        if(nonContract == true){
            if( _share > _total.mul(percentTradeTrigger).div(DIVISION_FACTOR)){
                harvestRewards(); // This will claim rewards then lock EPX as DEPX, sell DDD for more USDDLP and some for WBNB
            }
        }
    ...
    */
    ...
    function stakeLP() internal {
        ...
        staker.deposit(address(this), address(tokenList[0].token), _bal);
    }
    ...
    function unstakeLP() internal {
        ...
        DotDotStaker staker = DotDotStaker(DOTDOT_STAKING_ADDRESS);
        staker.withdraw(address(this), address(tokenList[0].token), _bal);
    }

    function checkHarvestWBNB() internal view returns (uint256) {
        // Returns the amount of WBNB returned from liquidating the rewards
        IERC20 ddd = IERC20(DOTDOT_ADDRESS);
        uint256 rewards = ddd.balanceOf(address(this)); // Get any stored DDD
        DotDotStaker staker = DotDotStaker(DOTDOT_STAKING_ADDRESS);
        address[] memory tokens = new address[](1);
        tokens[0] = address(tokenList[0].token);
        DotDotStaker.Amounts[] memory rewardList = staker.claimable(address(this), tokens);
        rewards = rewards.add(rewardList[0].ddd.mul(3)); // DDD x 3 since we will stake all our EPX
        
        if(rewards > 0){
            // Get via Pancakeswap
            TradeRouter router = TradeRouter(PANCAKE_ROUTER);
            address[] memory path = new address[](2);
            path[0] = DOTDOT_ADDRESS;
            path[1] = WBNB_ADDRESS;
    ...
    function harvestRewards() internal {
        ...
        tokens[0] = address(tokenList[0].token);
        DotDotStaker.Amounts[] memory rewardList = staker.claimable(address(this), tokens);
        if(rewardList[0].ddd > 0){
            staker.claim(address(this), tokens, uint256(-1)); // This will claim DDD and convert EPX to locked DEPX
        }
        TradeRouter router = TradeRouter(PANCAKE_ROUTER);

        // Convert DDD to WBNB
        IERC20 ddd = IERC20(DOTDOT_ADDRESS);
        uint256 rewards = ddd.balanceOf(address(this)); // Get any DDD here
        if(rewards > 0){
            address[] memory path = new address[](2);
            path[0] = DOTDOT_ADDRESS;
            path[1] = WBNB_ADDRESS;
            ddd.safeApprove(PANCAKE_ROUTER, 0);
            ddd.safeApprove(PANCAKE_ROUTER, rewards);
        ...
        // Now add this token to Ellipsis liquidity for USDD/BUSD
        ...
        if(rewards > 0){
            ...
            CurveLikeExchange ellipsis = CurveLikeExchange(USDD_BUSD_POOL_ADDRESS);
            IERC20(BUSD_ADDRESS).safeApprove(USDD_BUSD_POOL_ADDRESS, 0);
            IERC20(BUSD_ADDRESS).safeApprove(USDD_BUSD_POOL_ADDRESS, rewards);
            ellipsis.add_liquidity(depositAmounts, 1); // Will give us USDDLPv2 token            
        }
    }
    ...
    function governanceLockDEPX() external onlyGovernance {
        // This function locks any contract depx into dotdot
        DotDotLocker locker = DotDotLocker(DOTDOT_LOCKER_ADDRESS);
        IERC20 depx = IERC20(DEPX_ADDRESS);
        uint256 _bal = depx.balanceOf(address(this));
        if(_bal == 0){return;}
        depx.safeApprove(DOTDOT_LOCKER_ADDRESS, 0);
        depx.safeApprove(DOTDOT_LOCKER_ADDRESS, _bal);
        locker.deposit(address(this), _bal);
    }

    function governanceStartUnlockDEPX(uint256 _amount) external onlyGovernance {
        DotDotLocker locker = DotDotLocker(DOTDOT_LOCKER_ADDRESS);
        locker.initiateUnbondingStream(_amount);
    }

    function governanceFinishUnlockDEPX() external onlyGovernance {
        DotDotLocker locker = DotDotLocker(DOTDOT_LOCKER_ADDRESS);
        locker.withdrawUnbondedTokens(governance());
    }

    function governanceClaimDEPXFees(address _token) external onlyGovernance {
        DotDotLocker locker = DotDotLocker(DOTDOT_LOCKER_ADDRESS);
        address[] memory tokens = new address[](1);
        tokens[0] = address(_token);
        locker.claim(address(this), tokens);
        uint256 _bal = IERC20(_token).balanceOf(address(this));
        if(_bal == 0){return;}
        IERC20(_token).safeTransfer(governance(), _bal);
    }

    function governanceClaimExtraRewards() external onlyGovernance {
        DotDotStaker staker = DotDotStaker(DOTDOT_STAKING_ADDRESS);
        staker.claimExtraRewards(governance(), address(tokenList[0].token));
        // Tokens will go to governance
    }
