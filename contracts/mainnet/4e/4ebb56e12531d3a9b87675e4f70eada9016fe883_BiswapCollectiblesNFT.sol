...
import "@openzeppelin/contracts-upgradeable/utils/AddressUpgradeable.sol"; //todo use in hook
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function mint(address to, uint8 level) external onlyRole(TOKEN_MINTER_ROLE) {
        ...
        require(level <= MAX_LEVEL, "Maximum level exceeded");
        ...
        //Select token only if address not contract
        if (selectedToken[to] == 0 && to.code.length == 0) {
    ...
    function _beforeTokenTransfer(
        ...
        if(from != to && from != address(0)) transferHookHandler(msg.sender, tokenId, _tokens[tokenId].level);
    ...
    function reselectToken(address owner) private {
        ...
        if (balanceOf(owner) > 1) {
            for (uint i = 0; i < balanceOf(owner); i++) {
                if (tokenOfOwnerByIndex(owner, i) != oldTokenId) {
                    selectedToken[owner] = tokenOfOwnerByIndex(owner, i);
                    newTokenId = selectedToken[owner];
                    break;
                }
            }
    ...
    function transferHookHandler(address user, uint tokenId, uint level) private {
        for (uint i = 0; i < transferHookAddresses.length; i++) {
            (bool success, ) = transferHookAddresses[i].call(
                abi.encodeWithSignature("transferNFTHookReceive(address,uint256,uint256)", user, tokenId, level)
            );
            if (!success) emit HookError(transferHookAddresses[i]);
        }
    }
}
