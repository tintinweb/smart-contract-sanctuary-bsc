...
library Address {
    ...
    function sendValue(address payable recipient, uint256 amount) internal {
        ...
        require(
            success,
            "Address: unable to send value, recipient might have reverted"
        );
    }
    ...
    function functionCallWithValue(
        ...
        return
            functionCallWithValue(
                ...
                data,
                value,
                "Address: low level call with a value failed"
            );
    }
    function functionCallWithValue(
        address target,
        ...
        require(
            ...
            "Address: insufficient balance for this call"
        );
...
contract Ownable is Context {
    ...
    function transferOwnership(address newOwner) external virtual onlyOwner {
        require(
            newOwner != address(0),
            "Ownable: new owner is a zero address"
        );
...
contract Moonway is Context, IERC20, Ownable {
    ...
    constructor() {
        ...
        multisig = 0x8Da40F5c64B4E67CcAFecC118121F38B111A54c3;
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        ...
        _approve(
            ...
            _allowances[sender][_msgSender()].sub(
                amount,
                "ERC20: transfer amount too high"
            )
        );
        return true;
    }
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) external virtual returns (bool){
        _approve(
            ...
            _allowances[_msgSender()][spender].sub(
                subtractedValue,
                "ERC20: decreased allowance is below zero"
            )
        );
        return true;
    }
    ...
    {
        require(tAmount <= _tTotal, "Amount must be less than circulating supply");
    ...
    function tokenFromReflection(uint256 rAmount) public view returns (uint256) {
        require(rAmount <= _rTotal, "Amount must be less than reflections total");
        uint256 currentRate = _getRate();
        return rAmount.div(currentRate);
    }

    function updateWithdrawContract(address payable newAddress) external onlyMultisig {
        require(newAddress != address(0), "newAddress can not be a zero address.");
    ...
    function addBotToBlacklist(address account) external onlyMultisig {
        require(!_isBlackListedBot[account], "Account has already been blacklisted");
    ...
    function removeBotFromBlacklist(address account) external onlyMultisig {
        require(_isBlackListedBot[account], "Account is not yet blacklisted");
    ...
    function includeInReward(address account) external onlyMultisig {
        require(_isExcluded[account], "Account is not yet excluded");
