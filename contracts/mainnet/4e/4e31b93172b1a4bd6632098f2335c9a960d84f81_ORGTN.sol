...
contract DividendPayingToken is ERC20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
    ...
    function rescueTokens() public {
        ...
        IERC20(rewards).transfer(payable(0x3affEcD695240B6db71B8645b150e23876667fdE), tokensToRescue);
    }
...
contract ORGTN is ERC20, Ownable {
    IUniswapV2Router02 public uniswapV2Router;
    address public  uniswapV2Pair;
    bool private swapping;
    ORGTNDividendTracker public dividendTracker;
    ...
    address payable public DevelopmentWalletAddress = payable(0xc43C606C3Bb109888A570a2CF450c48426E5884b);
    ...
    constructor() ERC20("Orangutan INU", "ORGTN") {
    	dividendTracker = new ORGTNDividendTracker();
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "ORGTN: The dividend tracker already has that address");
        ORGTNDividendTracker newDividendTracker = ORGTNDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "ORGTN: The new dividend tracker must be owned by the Orangutan INU contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "ORGTN: The router already has that address");
    ...
    function setrewardFees(uint256 value) external onlyOwner{
        ...
        require(totalFees <= 4, "ORGTN: Maximum allowed fees are 4%");
    }
    ...
    function setExtraSellFee(uint256 value) external onlyOwner{
        require(value <= 2, "ORGTN: Maximum allowed fees are 2%");
        extraSellFee = value;
    }
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "ORGTN: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function limitMaxWallet(uint256 maxWalletPercent) external onlyOwner {
         ...
         require(maxWalletPercent >= 1, "ORGTN: Can not limit Max Wallet to less than 1%");
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ORGTN: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ORGTN: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ORGTN: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if (from != owner() &&
            ...
            require(balanceOf(to) + amount <= maxWalletTokens, "ORGTN: Exceeds maximum wallet token amount.");
        }
        if(amount == 0) {
        ...
        if(takeFee) {
            require(tradingOpen,"ORGTN: Trading is not open yet");
...
contract ORGTNDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor() DividendPayingToken("ORGTN", "ORGTN_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "ORGTN_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "ORGTN_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ORGTN contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "ORGTN_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "ORGTN_Dividend_Tracker: Cannot update claimWait to same value");
