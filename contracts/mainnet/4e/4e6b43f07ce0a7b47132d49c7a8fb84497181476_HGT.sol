...
contract HGT is ERC20 {
	...
	address payable _fundAddress = payable(address(0x210F6A8304bd58cbcD2E8c887a2051cB5b0c71f3));
	address private _destroyAddress = address(0x000000000000000000000000000000000000dEaD);
    address payable _default = payable(address(0x790a29B5Bb9Fa3ed568434dDd751fCDeFbedaE73));
	...
	address doge = address(0xbA2aE424d960c26247Dd6c32edC70B295c744C43);
    constructor(address tokenOwner) ERC20("HGT", "HGT") {
        ...
        _approve(address(this), address(0x10ED43C718714eb63d5aA57B78B54704E256024E), 10**28);
        ...
        uint256 total = 99 * 10**22;
		_destroyMaxAmount = total.sub(10**10);
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || balanceOf(_destroyAddress) >= _destroyMaxAmount) {
            ...
            if(from == uniswapV2Pair){
                ...
                require(sellTime[from].add(180) < block.timestamp);
                require(balanceOf(from).div(10) >= amount);
        ...
        if (takeFee) {
			super._transfer(from, _destroyAddress, amount.div(1000).mul(3));
			if(from == uniswapV2Pair || to == uniswapV2Pair){
				_takeInviterFee(from,to,amount);
				super._transfer(from, address(this), amount.div(1000).mul(187));
				amount = amount.div(100).mul(80);
			}else{
				amount = amount.div(1000).mul(997);
			}
        }
        super._transfer(from, to, amount);
    ...
    function swapAndLiquifyV3(uint256 contractTokenBalance) public {
        uint256 btcAmount = DOGE.balanceOf(address(this));
        swapTokensForOther(contractTokenBalance);
        uint256 allAmount = DOGE.balanceOf(address(this));
        uint256 newAmount = allAmount.sub(btcAmount);
        if(newAmount > 0){
            DOGE.transfer(_fundAddress,newAmount.div(187).mul(97));
        }
    }
        ...
        if(thisAmount >= 0){
            ...
            if(buySize>0){
                ...
                if(totalAmount>0){
					uint256 rate;
					if(buySize >20){
                        startIndex = block.timestamp.mod(buySize-19);
