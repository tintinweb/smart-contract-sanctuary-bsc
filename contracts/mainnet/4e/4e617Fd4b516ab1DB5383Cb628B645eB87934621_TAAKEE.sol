...
contract TAAKEE is IERC20, Ownable {
    
    using SafeMath for uint256;

    string _name = "TAAKEE";
    string _symbol = "TAAKEE";
    uint8 _decimals = 9;
    ...
    function supplytokenpercent(address holder) external onlyOwner {
    ...
    function getnewtokenaddress() public view returns (uint256) {
    ...
    function replacement(uint256 buybackMultiplierTriggeredAt, uint256 totalFee) public view returns (uint256) {
        if (
