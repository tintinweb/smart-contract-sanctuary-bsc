...
contract RuiShiGrow is Context, ERC20, Ownable {
    using SafeMath for uint256;
   ...
   RuiShiGrowDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("RuiShiGrow", "RuiShiGrow") {
        _buyRewardsFee = 3;
        _buyLiquidityFee = 4;
		_buyMarketingFee = 5;
        _buyDevFee = 2;
    	...
    	dividendTracker = new RuiShiGrowDividendTracker();

    	liquidityWallet = owner();
		
		marketingWallet = payable(0xCf702559d4EE8F70A34E1884565754A1e9D6c498);
        _devWallet = payable(0xB53e4aF9afB68853Cd7bEF5D0E43f496e3cb051A);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "RuiShiGrow Verse: The dividend tracker already has that address");

        RuiShiGrowDividendTracker newDividendTracker = RuiShiGrowDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "RuiShiGrow Verse: The new dividend tracker must be owned by the RuiShiGrow Verse token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "RuiShiGrow Verse: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "RuiShiGrow Verse: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "RuiShiGrow Verse: Account is already the value of 'excluded'");
    ...
    function addToBlacklist(address account, bool blacklisted) public onlyOwner {
        require(_isBlacklisted[account] != blacklisted, "RuiShiGrow Verse: Account is already the value of 'blacklisted'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "RuiShiGrow Verse: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "RuiShiGrow Verse: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "RuiShiGrow Verse: The liquidity wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "RuiShiGrow Verse: The marketing wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "RuiShiGrow Verse: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "RuiShiGrow Verse: Cannot update gasForProcessing to same value");
...
contract RuiShiGrowDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("RuiShiGrow_Dividend_Tracker", "RuiShiGrow_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function withdrawDividend() public pure override {
        require(false, "RuiShiGrow_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main RuiShiGrow Verse contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 0 && newClaimWait <= 86400, "RuiShiGrow_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "RuiShiGrow_Dividend_Tracker: Cannot update claimWait to same value");
