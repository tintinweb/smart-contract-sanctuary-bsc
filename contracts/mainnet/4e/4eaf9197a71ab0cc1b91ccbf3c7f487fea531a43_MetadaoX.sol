...
contract MetadaoX  {
  ...
  function addReferrer(address payable referrer) internal returns(bool){
   ...
   function addReferrerdownline(address  user,address payable referrer) internal returns(bool){
    //   require (referrer !=address(0),"address must be referrer");
    if (referrer == address(0)) {
      emit RegisteredRefererFailed(user, referrer, "Referrer cannot be 0x0 address");
      return false;
    } else if (isCircularReference(referrer,user)) {
      emit RegisteredRefererFailed(user, referrer, "Referee cannot be one of referrer uplines");
      return false;
    } else if (accounts[referrer].referrer == address(0)) {
      emit RegisteredRefererFailed(user, referrer, "Address have been registered upline");
      return false;
    }
