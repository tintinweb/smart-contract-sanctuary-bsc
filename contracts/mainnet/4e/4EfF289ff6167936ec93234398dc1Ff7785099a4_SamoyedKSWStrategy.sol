...
contract SamoyedKSWStrategy is StratManager, FeeManager, GasThrottler, ReentrancyGuard {
    ...
    function deposit() public whenNotPaused nonReentrant {
        ...
        if (wantBal > 0) {
            masterchef.deposit(address(want), wantBal);
        }
    }
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        if (wantBal < _amount) {
            masterchef.withdraw(address(want), _amount.sub(wantBal));
    ...
    function chargeFees() internal {
        // Charge fee 3.5% from MasterChef reward token
        uint256 amountToWbnb = output.balanceOf(address(this)).mul(35).div(1000);

        // swap reward token to WBNB
        ...
        uint256 platformFeeAmount = wbnbBalance.mul(platformFee).div(MAX_FEE);
        wbnb.safeTransfer(feeRecipient, platformFeeAmount);

        // send 15% of fee to DevFee wallet
        uint256 devFeeAmount = wbnbBalance.mul(devFee).div(MAX_FEE);
        wbnb.safeTransfer(devWallet, devFeeAmount);
    }

    // Store 50% of output to Raffle
    function _collectRaffle() internal {
        // Store 50% from reward token to Raffle
        uint256 amountToWbnb = output.balanceOf(address(this)).mul(raffleRatio).div(MAX_RAFFLE_RATIO);
        ...
        uint256 wbnbBalance = wbnb.balanceOf(address(this));
        IVaultRaffle(raffle).deposit(wbnbBalance);
    }
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount, , ) = masterchef.userInfo(address(want), address(this));
        return _amount;
    }
    ...
    function retireStrat() external {
        ...
        masterchef.emergencyWithdraw(address(want));
