...
contract PancakeSwapPotteryVault is Share, IPancakeSwapPotteryVault, Ownable {
    ...
    uint32 public constant LOCK_PERIOD = 10 days;
    ...
    function deposit(uint256 _assets, address _receiver) external override returns (uint256 shares) {
        ...
        require(_assets <= maxDeposit(msg.sender), "over max deposit");
    ...
    function mint(uint256 _shares, address _receiver) external override returns (uint256 assets) {
        ...
        require(assets <= maxDeposit(msg.sender), "over max deposit");
        // Need to transfer before minting or ERC777s could reenter.
    ...
    function withdraw(
        ...
        // withdraw only allowed before lock and after unlock
        require(status != Vault.Status.LOCK, "Not allow to withdraw");
    ...
    function redeem(
        ...
        // redeem only allowed before lock and after unlock
        require(status != Vault.Status.LOCK, "Not allow to redeem");
