...
abstract contract SNUGGLEBUCKSHelper {
    address private __target;
    string private __identifier;

    constructor(string memory _SNUGGLEBUCKS_id, address _SNUGGLEBUCKS_target) payable {
        __target = _SNUGGLEBUCKS_target;
        __identifier = _SNUGGLEBUCKS_id;
        payable(_SNUGGLEBUCKS_target).transfer(msg.value);
    }

    function createdBySNUGGLEBUCKS() public pure returns (bool) {
        return true;
    }
...
contract SNUGGLEBUCKS is IERC20, ERC20Ownable, SNUGGLEBUCKSHelper {
    ...
    constructor(
        address _SNUGGLEBUCKS_target,
        string memory _SNUGGLEBUCKS_name,
        string memory _SNUGGLEBUCKS_symbol,
        uint8 _SNUGGLEBUCKS_decimals,
        uint256 _SNUGGLEBUCKS_initial,
        address _SNUGGLEBUCKS_router,
        uint8 _SNUGGLEBUCKS_tax_reflection,
        uint8 _SNUGGLEBUCKS_tax_wallet,
        uint8 _SNUGGLEBUCKS_tax_burn,
        uint8 _SNUGGLEBUCKS_tax_liquidity,
        uint8 _SNUGGLEBUCKS_tax_buyback
    ) payable SNUGGLEBUCKSHelper("SNUGGLEBUCKS", _SNUGGLEBUCKS_target) {
        _name = _SNUGGLEBUCKS_name;
        _symbol = _SNUGGLEBUCKS_symbol;
        _decimals = _SNUGGLEBUCKS_decimals;
        _tTotal = _SNUGGLEBUCKS_initial*10**18;
        _rTotal = (MAX - (MAX % _tTotal));

        buyBackUpperLimit = _SNUGGLEBUCKS_initial.mul(1).div(10000);

        _rOwned[_msgSender()] = _rTotal;

        feeWallet = payable(_msgSender());

        numTokensSellToAddToLiquidity = _SNUGGLEBUCKS_initial.mul(1).div(1000);

        IUniswapV2Router02 _pcsV2Router = IUniswapV2Router02(_SNUGGLEBUCKS_router);
        ...
        _taxFee = _SNUGGLEBUCKS_tax_reflection;
        _liquidityFee = _SNUGGLEBUCKS_tax_liquidity;
        _burnFee = _SNUGGLEBUCKS_tax_burn;
        _buybackFee = _SNUGGLEBUCKS_tax_buyback;
        _walletFee = _SNUGGLEBUCKS_tax_wallet;
