...
contract DeerToken is Ownable {
  ...
  mapping (address => mapping (address => uint)) private _allowances;
    ...
    constructor (string memory name, string memory symbol, uint totalSupply) public {
        ...
        startTransaction=1653883200;//Start trading 2022-05-30 12:00:00
    ...
    function setFeeTo(address _f,address _ido,address _lp)public onlyOwner{
        Foundation=_f;
        IdoAddress=_ido;
        LPtoken=_lp;
    }
    ...
    function _transfer(address sender, address recipient, uint amount) internal {
        ...
        require(_isExcludedFromFee[sender] ||_isExcludedFromFee[recipient] || block.timestamp > startTransaction,"swap It hasn't started yet");
        (uint256 a88,uint256 a12)=_getValue(amount);
        _balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
        if(Foundations(IdoAddress).getUser(sender) != address(0) && users[sender].upAddress == address(0)){
            users[sender].upAddress=Foundations(IdoAddress).getUser(sender);
        }
        if(users[recipient].upAddress == address(0) && Foundations(IdoAddress).getUser(recipient) == address(0)){
            users[recipient].upAddress=sender;
            _balances[recipient] = _balances[recipient].add(amount);
            emit Transfer(sender, recipient, amount);
        }else if(Foundations(IdoAddress).getUser(recipient) != address(0) && users[recipient].upAddress == address(0)){
            users[recipient].upAddress=Foundations(IdoAddress).getUser(recipient);
            ...
            if((recipient == pancakePair && sender == owner()) || _isExcludedFromFee[sender] || recipient == address(this) || totalSupply() < 10000000 ether){
                _balances[recipient] = _balances[recipient].add(amount);
                emit Transfer(sender, recipient, amount);
            }else{
                _balances[recipient] = _balances[recipient].add(a88);
    ...
    function distribution(address sender,uint256 a)internal{
        ...
        _totalSupply=_totalSupply.sub(a.mul(5).div(100));
        emit Transfer(sender, back, a.mul(5).div(100));
        uint256 f=a.mul(5).div(100);
        Foundations(Foundation).sendDistribution(f.mul(80).div(100));
        _balances[LPtoken] = _balances[LPtoken].add(a.mul(3).div(100));
        emit Transfer(sender, LPtoken, a.mul(3).div(100));
        Foundations(LPtoken).sendDistribution( a.mul(3).div(100));
        ...
        uint aun=0;
        for(uint k=0;k<16;k++){
           if(users[addrs1].upAddress != address(0) && vs > 0 && users[addrs1].upAddress != addrs1){
