...
contract UATMinerUAT is Ownable{
    ...
    constructor(address payable _dev, address payable _market, address payable _ownerA) {
		require(!isContract(_dev) && !isContract(_market) && !isContract(_ownerA));
        dev = _dev;
        market = _market;
        ownerA = _ownerA;
    }
    ...
    function takeFees(uint256 oresValue) internal returns(uint256){
        ...
        uint256 ownering = oresValue.mul(OWNER_TAX).div(PERCENTS_DIVIDER);
        token.transfer(dev, tax);
        token.transfer(market, marketing);
        token.transfer(ownerA, ownering);
        return marketing.add(tax).add(ownering);
    }
