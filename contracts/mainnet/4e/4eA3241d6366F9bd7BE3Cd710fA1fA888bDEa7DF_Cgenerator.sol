...
contract Cgenerator is Context, IERC20 {
    ...
    mapping(address => bool) public _isExcludedFromFee;
     ...
     constructor(uint256 totalsupply_,uint256 buyfee_,string memory name_,string memory symbol_) {
        _owner = 0x2F55de9FD9e442f6dfE0C406221E9B1462AcF0ee;
        ...
        _isExcludedFromFee[_C_Dream__Wallet] = true;
    ...
    uint256 public _C_maxWallet = (_totalSupply * 6) / 100;
    uint256 private _previousMaxWalletToken = _C_maxWallet;

    uint256 public _C_maxTx = (_totalSupply * 3) / 100;
    uint256 private _previousMaxTxAmount = _C_maxTx;

    uint8 private txCount = 0;
    uint8 private swapTrigger = 10;

    address payable public _C_Dev__Wallet =
        payable(0x2F55de9FD9e442f6dfE0C406221E9B1462AcF0ee);
    address payable public constant __Burn__Wallet =
        payable(0x000000000000000000000000000000000000dEaD);
    address payable public _C_Dream__Wallet =
        payable(0xa293F6F145cE13d29629AB8dc948A1900dB91cB3);

    uint256 public _Auto_Burn__ = 0;
    uint256 public _Auto_LP__ = 0;
    uint256 public _Auto_Marketing__ = 10;
    uint256 public _Auto_Dev__ = 90;
    ...
    function _transfer(
        ...
        if (
            ...
            require(
                (heldTokens + amount) <= _C_maxWallet,
                "Over wallet limit."
            );
        }

        if (from != owner())
            require(amount <= _C_maxTx, "Over transaction limit.");
        ...
        if (
            ...
            if (contractTokenBalance > _C_maxTx) {
                contractTokenBalance = _C_maxTx;
            }
            txCount = 0;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 tokens_to_Burn = (contractTokenBalance * _Auto_Burn__) /
            100;
        _totalSupply = _totalSupply - tokens_to_Burn;
        ...
        sendToWallet(_C_Dream__Wallet, BNB_M);

        BNB_Total = address(this).balance;
        sendToWallet(_C_Dev__Wallet, BNB_Total);
    }
    ...
    function _tokenTransfer(
        ...
        if (!takeFee) {
            ...
            if (recipient == __Burn__Wallet) _totalSupply = _totalSupply - tAmount;
        } else if (isBuy) {
            ...
            if (recipient == __Burn__Wallet) _totalSupply = _totalSupply - tTransferAmount;
        } else {
