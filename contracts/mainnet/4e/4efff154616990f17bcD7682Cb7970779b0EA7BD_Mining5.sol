...
import "../v2/Referral2.sol";
import "../v2/Agent2.sol";
import "../Tools.sol";

abstract contract Agent4 {
    using SafeERC20 for IERC20;
...
import "../v2/Referral2.sol";
import "../v4/Agent4.sol";
import "../Tools.sol";

abstract contract Agent5 {
    using SafeERC20 for IERC20;

    Agent4 public immutable agent;
    Referral2 public immutable referral;
    address public immutable admin;
    IERC20 public immutable dot;
    ...
    constructor(address agent_, address referral_, address dot_, address bdot_) {
        agent = Agent4(agent_);
        referral = Referral2(referral_);
        admin = agent.admin();
        dot = IERC20(dot_);
    ...
    function setRebate(address account_, uint256 rebate_) external {
        if (rebate_ == 0 || rebate_ > 3000) revert();
        if (level(account_) == 0) revert();
        if (referral.parent(account_) != msg.sender) revert();
        if (rebate(msg.sender) < rebate_) revert();
        if (rebate(account_) >= rebate_) revert();
    ...
    function buy(address account_, uint256 level_, uint256 deadline_, bytes memory signature) external {
        if (block.timestamp > deadline_) revert();
        bytes memory message = abi.encode(account_, level_, deadline_);
        bytes32 hash = keccak256(
            abi.encodePacked("\x19Ethereum Signed Message:\n", Strings.toString(bytes(message).length), message)
    ...
    function _distribute(address account_, uint256 amount_, uint256 take_, uint256 index_) internal {
        index_++;
        if (index_ > 10) {
            dot.safeTransfer(admin, (amount_ * (3000 - take_)) / 10000);
            return;
        }

        dot.safeTransfer(account_, (amount_ * (rebate(account_) - take_)) / 10000);
        take_ = rebate(account_);
        if (take_ == 3000) return;
    ...
    function currentWithdraw(
        ...
        if (rate == 0) {
            ...
            dot.safeTransfer(account, principal + (interest * rate) / 10000);
...
contract Mining5 is Agent5, Pausable {
    using SafeERC20 for IERC20;
    ...
    constructor(address _agent, address _referral, address _dot, address _bdot) Agent5(_agent, _referral, _dot, _bdot) {
        feeTo = 0x39C5F6E6fB73Da8E6c464F5007B73e1D4Ed1546b;
        feeRate = 3000;
    }
    ...
    function fixedDeposit(uint256 _period, uint256 _amount) external {
        if (_amount == 0) revert();
