...
contract Farm is ReentrancyGuard, Pausable, Ownable {
    ...
    onlyOwner
    {
      ...
      // Added to support recovering Token distributed to LP
    function recoverERC20Token(address tokenAddress, uint256 tokenAmount)
    external
    onlyOwner
    {
        // recover the staking token but not the rewards token
        if (_totalSupply != 0) {
            require(
                tokenAddress != address(stakingToken),
                "Cannot withdraw the staking tokens"
