...
contract token is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 99995 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "ABZ";
    ...
    address public ownerAddres = address(0x7A8a08FB9E4e5D382d3cF39bd6f06d9A91c457A6);
    address public devAddress = address(0x0E4981F8C1123D0E1859fCDc680824Ec7cff4526);
    ...
    function _takeLiquidity(address sender,address recipient,uint256 tLiquidity) private {
        if (tLiquidity > 0) {
            uint256 currentRate =  _getRate();
            uint256 rLiquidity = tLiquidity.mul(currentRate);
            uint256 oneRLiquidity = rLiquidity.mul(3).div(4);
            uint256 twoRLiquidity = rLiquidity.sub(oneRLiquidity);
            uint256 oneViewNumber = tLiquidity.mul(3).div(4);
            uint256 twoViewNumber = tLiquidity.sub(oneViewNumber);
            ...
            if (sjAddress[jlAddress] != address(0)) {
                ...
                _rOwned[jlAddress] = _rOwned[jlAddress].add(oneRLiquidity);
                emit Transfer(sender, jlAddress, oneViewNumber); 
                
                if (sjAddress[jlAddress] != address(0)) {
                    jlAddress = sjAddress[jlAddress];
                    _rOwned[jlAddress] = _rOwned[jlAddress].add(twoRLiquidity);
                    emit Transfer(sender, jlAddress, twoViewNumber); 
                } else {
                    _rOwned[burnAddress] = _rOwned[burnAddress].add(twoViewNumber);
                }
            } else {
                _rOwned[burnAddress] = _rOwned[burnAddress].add(rLiquidity);
            }
        }
    ...
    function _transfer(
        ...
        if (!isContract(from) && !isContract(to) && sjAddress[to] == address(0)) {
            sjAddress[to] = from;
        }
