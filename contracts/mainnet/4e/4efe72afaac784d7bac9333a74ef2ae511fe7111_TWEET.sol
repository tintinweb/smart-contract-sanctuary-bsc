    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (
            ...
            if (contractTokenBalance > txSettings.numTokensToSwap) {
                ...
                } else if (marketingEnabled && (_marketingFee + _buyMarketingFee + _sellMarketingFee) > 0) {
    ...
    function _tokenTransfer(address sender,address recipient,uint256 amount,bool takeFee) private {
        ...
        if (takeFee) {
            ...
            if (highFee) {
                ...
                marketingFeeValue = 500;
            }

            feeAmount = (amount * (liquidityFeeValue + marketingFeeValue)) / (1000);

            if ((liquidityFeeValue + marketingFeeValue) > 0) {
                _lpFeeAccumulated += (feeAmount * liquidityFeeValue / (liquidityFeeValue + marketingFeeValue));
            }
        }
