...
contract YELLOWFLOKI is Context, IERC20, Ownable {
    ...
    string private _name = "Yellow Floki";
    string private _symbol = "YELLOWFLOKI";
    ...
    function changeBuyFees(uint256 buyburn, uint256 buymarketing, uint256 buytax, uint256 buyliquidity) external onlyOperator{
        _BUYburnFee = buyburn;
    ...
    function calculateBurnFee(uint256 _amount) private view returns (uint256) {
        if(buyFeesActive){
            return _amount.mul(_BUYburnFee).div(
                10**2
            );
        }
