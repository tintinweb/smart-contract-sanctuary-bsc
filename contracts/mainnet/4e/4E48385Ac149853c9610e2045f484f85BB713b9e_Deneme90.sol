...
contract Deneme90 is Context, IBEP20, Ownable {
    ...
    mapping (address => bool) private _isRegisteredMaxBuy;
    mapping (address => bool) private _isRegisteredMaxSell;
    mapping (address => uint256) private _maxBuyTxAmountByAddress;
    mapping (address => uint256) private _maxSellTxAmountByAddress;
    address[] private _excluded;
   
    string private _name = "deneme coin";
...
constructor (uint256 o2OneFee, uint256 o2TwoFee, uint256 cO2OneFee, uint256 cO2TwoFee, uint256 cO2ThreeFee,
    ...
    function setMaxBuyTxPercent(uint256 maxBuyTxPercent) external onlyOwner() {
        _maxBuyTxAmount = getMaxTxAmount(maxBuyTxPercent);
    }     
    function setMaxSellTxPercent(uint256 maxSellTxPercent) external onlyOwner() {
        _maxSellTxAmount = getMaxTxAmount(maxSellTxPercent);
    }

    function isRegisteredMaxBuySell(address account, bool isBuy) private view returns (bool){
        if(isBuy){
            return _isRegisteredMaxBuy[account];
        }else{
            return _isRegisteredMaxSell[account];
        }
    }
    function registerMaxBuyerSeller(address account, bool isBuyer) private {
        if (isBuyer){
            _isRegisteredMaxBuy[account] = true;
        }else{
            _isRegisteredMaxSell[account] = true;
        }
    }    
    function setMaxTxPercentByAddress(address account, uint256 percent, bool isBuy) private {
        if(isBuy){
            _maxBuyTxAmountByAddress[account] = getMaxTxAmount(percent);
        }else{
            _maxSellTxAmountByAddress[account] = getMaxTxAmount(percent);
        }
    }   
    function getMaxTxAmountByAddress(address account, bool isBuy) private view returns(uint256) {
        if(isBuy){
          return _maxBuyTxAmountByAddress[account];
        }else{
          return _maxSellTxAmountByAddress[account];
        }
    }
    function setMaxBuyTxPercentByAddressList(address[] calldata addresses, uint256 percent) external onlyOwner {
      for (uint256 i; i < addresses.length; ++i) {
        setMaxBuyTxPercentByAddress(addresses[i], percent);
      }
    }
    function setMaxBuyTxPercentByAddress(address account, uint256 percent) public  onlyOwner{
        if(!isRegisteredMaxBuySell(account, true)){
            registerMaxBuyerSeller(account, true);
        }
        setMaxTxPercentByAddress(account, percent, true);
    }
    function setMaxSellTxPercentByAddressList(address[] calldata addresses, uint256 percent) external onlyOwner {
      for (uint256 i; i < addresses.length; ++i) {
        setMaxSellTxPercentByAddress(addresses[i], percent);
      }
    }
    function setMaxSellTxPercentByAddress(address account, uint256 percent) public onlyOwner {
        if(!isRegisteredMaxBuySell(account, false)){
            registerMaxBuyerSeller(account, false);
        }
        setMaxTxPercentByAddress(account, percent, false);
    }
    function getMaxTxAmount(uint256 percent) private view returns(uint256){
        return _tTotal.mul(percent).div(10**2);
    }
    ...
    function checkMaxTXAmount(address sender, address recipient, uint256 amount, bool isBuy) internal view{
        if(sender != owner() || recipient != owner() || !_isNatureLover[recipient] || !_isNatureLover[sender]) {
            bool isRegistered = isRegisteredMaxBuySell(sender, isBuy);
            if(isRegistered){
                if (!isBuy) {
                    require(amount <= getMaxTxAmountByAddress(sender, false), "Transfer amount exceeds the maxSellTxAmount");
                } else {
                    if (_msgSender() == sender) {                    
                        require(amount <= getMaxTxAmountByAddress(sender, true), "Transfer amount exceeds the maxBuyTxAmount");
                    } 
                    if(_msgSender() == recipient){
                        require(amount <= getMaxTxAmountByAddress(recipient, true), "Transfer amount exceeds the maxBuyTxAmount");
                    }
                } 
            }
            if (!isBuy) {
