...
contract False is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    FalseDividendTracker public dividendTracker;
    ...
    constructor() ERC20("False", "False") {
        marketingWallet = payable(0xcC09B9ac2fAA28ab74af04598D30087CB3FF419E);
        devWallet = payable(0x5a03CeF2a1711Ab32772bee7945843dda3D9f78B);
        ...
        dividendTracker = new FalseDividendTracker(
            payable(this),
            router,
...
contract FalseDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
   ...
   False public FalseContract;
    ...
    constructor(
        ...
        defaultToken = token;
        FalseContract = False(mainContract);
    ...
    function _transfer(
        ...
        require(false, "FALSE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "FALSE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FALSE contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(FalseContract);

            uint256 prevBalance = FalseContract.balanceOf(address(this));
            ...
            {
                uint256 received = FalseContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    FalseContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
