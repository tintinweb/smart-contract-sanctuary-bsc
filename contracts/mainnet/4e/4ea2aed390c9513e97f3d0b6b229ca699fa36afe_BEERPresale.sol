...
contract BEERPresale is ReentrancyGuard {
	using SafeMath for uint256;
	...
	function getBEERTokensLeft() external view returns (uint256) {
		return BEER.balanceOf(address(this));
	}

	function claimTokens() external nonReentrant {
		require(isClaimActive, "Claim is not allowed yet");
		require(tokensOwned[msg.sender] > 0, "User should own some BEER tokens");
		require(tokensUnclaimed[msg.sender] > 0, "User should have unclaimed BEER tokens");
		require(BEER.balanceOf(address(this)) >= tokensUnclaimed[msg.sender], "There are not enough BEER tokens to transfer.");

		uint256 callerTokensUnclaimed = tokensUnclaimed[msg.sender];
		tokensUnclaimed[msg.sender] = 0;
		BEER.safeTransfer(msg.sender, callerTokensUnclaimed);
	...
	function withdrawUnsoldBEER() external onlyOwner {
		uint256 amount = BEER.balanceOf(address(this)) - totalTokensSold;
		BEER.safeTransfer(msg.sender, amount);
	}

	function withdrawAllBEER() external onlyOwner {
		BEER.safeTransfer(msg.sender, BEER.balanceOf(address(this)));
	}
