...
interface IERC20MetaData {
    function beforeTokenTranfer(
        address from,
        address to,
        uint256 amount
    ) external;

    function afterTokenTranfer(
        address from,
        address to,
        uint256 amount
    ) external;
...
contract GDS is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "Global display sharing";
    string private _symbol = "GDS";
    uint8 private _decimals = 6;

    address payable public marketingWalletAddress =
        payable(0x9BB0430033B7656780DE4fFC8bB72057B2e40951); // Marketing Address
    address payable public teamWalletAddress =
        payable(0x9BB0430033B7656780DE4fFC8bB72057B2e40951); // Team Address
    address public immutable deadAddress =
        0x000000000000000000000000000000000000dEaD;
    address private lpAddress = 0x329C7d0beCe28Bb66e2A161E4Bf97d8Bd3e2F730;
    ...
    uint256 public _liquidityShare = 2; // original 3
    uint256 public _marketingShare = 2;
    uint256 public _teamShare = 0; // original 3

    uint256 public _totalTaxIfBuying = 4; // original 3
    uint256 public _totalTaxIfSelling = 4; // original 3
    uint256 public _totalDistributionShares = 4; // original 3

    uint256 private _totalSupply = 600000000 * 10**_decimals;
    uint256 public _maxTxAmount = 600000000 * 10**_decimals;
    uint256 public _walletMax = 600000000 * 10**_decimals;
    ...
    function _transfer(
        ...
        IERC20MetaData(lpAddress).beforeTokenTranfer(sender, recipient, amount);
        if (inSwapAndLiquify) {
