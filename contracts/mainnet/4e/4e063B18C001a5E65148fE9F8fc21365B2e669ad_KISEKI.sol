...
contract KISEKI is ERC20, Ownable {
    ...
    function changeMarketingWallet(address _marketingWallet, address _devWallet)
        external
        onlyOwner
    {
        require(
            _marketingWallet != marketingWallet && _devWallet != devWallet,
            " wallet is already that address"
        );
        require(
            _marketingWallet != address(0) && _devWallet != address(0),
            " wallet cannot be the zero address"
        );
        require(
            !isContract(_marketingWallet) && !isContract(_devWallet),
    ...
    function swapAndSendFee(uint256 tokenAmount) private {
        ...
        sendBNB(
            ...
            (addressBalance * marketingShare) / SHAREDIVISOR
        );
        sendBNB(
            payable(devWallet),
            (addressBalance * teamShare) / SHAREDIVISOR
        );
    ...
    function changeTaxDistribution(
        ...
        require(
            newTeamShare + newMarketingShare == SHAREDIVISOR,
