...
contract FireToken is ERC20, ERC20Detailed  {
    ...
    constructor (   
        address[6] memory _contractAddrs,
        uint256 _totalAmount,
        uint256[5] memory buyFee,
        uint256[5] memory sellFee,
        address[4] memory _addrs,
        uint256[5] memory coinConfig
    )  ERC20Detailed("Gaink", "Gaink", 18) {
        changeAddr = _contractAddrs[0];
        bFreeAddr = _contractAddrs[1];
        uFreeAddr = _contractAddrs[2];
        bigAirDropAddr = _contractAddrs[3];
        marketAddr = _contractAddrs[4];
        rewardAddr = _contractAddrs[5];
        ...
        uint256 totalSupply = _totalAmount.mul(coinConfig[0]).div(10000)*10**18;
        _mint(rewardAddr, totalSupply);

        uint256 lpAmount = _totalAmount.mul(coinConfig[1]).div(10000)*10**18;
        _mint(_addrs[0], lpAmount);

        uint256 foundationAmount = _totalAmount.mul(coinConfig[2]).div(10000)*10**18;
        _mint(_addrs[1], foundationAmount);

        uint256 daoAmount = _totalAmount.mul(coinConfig[3]).div(10000)*10**18;
        _mint(_addrs[2], daoAmount);

        uint256 ecologyAmount = _totalAmount.mul(coinConfig[4]).div(10000)*10**18;
        _mint(_addrs[3], ecologyAmount);
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            if(from == capitalPoolAddr) { 
                ...
                require(locktime<=block.timestamp || (getNftWhites(to)==true && buyUamount <= 301*10**18), "Purchase after adding whitelist, please purchase 100U quota");
            ...
            if(to == capitalPoolAddr){
                ...
                if(balance.sub(amount) < totalBuyAmount) {
                    ...
                    uint256 burnAmount = amount1.mul(buyTokenBFee).div(buyMulFee);
                    ...
                    if(fireshare >= burnAmount) {
                        uint256 afterDecrAmount = fireshare.sub(burnAmount);
                        setBpoolFireshare(sender, afterDecrAmount);
                    }
                    if(fireshare < burnAmount) {
                        setBpoolFireshare(sender, 0);
                    }
                    if(totalBuyAmount2 >= amount1) {
                        uint256 afterSAmount = totalBuyAmount2.sub(amount1);
                        setBuyAmountList(sender, afterSAmount);
                    }
                    if (totalBuyAmount2 < amount1) {
                        setBuyAmountList(sender, 0);
                    }
    
                    uint256 toAmount = amount1.mul(sellMulFee).div(10000);
                    super._transfer(sender, capitalPoolAddr, toAmount); 
                } else {
        ...
        uint256 burnAmount1 = amount.mul(buyTokenBFee).div(buyMulFee);
        uint256 fireshare1 = getBpoolFireshare(from);

        if(balance1.sub(amount) < totalBuyAmount1 && to != blackHoleAddr) {
            if(fireshare1 >= burnAmount1) {
                uint256 afterDecrAmount = fireshare1.sub(burnAmount1);
                setBpoolFireshare(from, afterDecrAmount);
            }
            if(fireshare1 < burnAmount1) {
                setBpoolFireshare(from, 0);
            }
            if(totalBuyAmount1 >= amount) {
                uint256 afterSAmount = totalBuyAmount1.sub(amount);
                setBuyAmountList(from, afterSAmount);
            }
            if (totalBuyAmount1 < amount) {
