...
contract WithdrawlTestSer {
    ...
    string public strategyTitle = "WithdrawlTestSer";
    ...
    // An Array of all of the steps
    StepDetails[] public steps;
    // An array of all of the steps, reverted (For Withdrawls)
    StepDetails[] public stepsReverted;
    // An array of all of the steps' functions, reverted (For Withdrawls)
    string[] public stepsFunctionsReverted;
    ...
    function withdraw(uint256 _amount) public {
        ...
        bytes[] memory dynamicArr = new bytes[](4);
        bytes[4] memory fixedArr = [abi.encode(msg.sender), abi.encode(_amount), abi.encode(reverseFunctions), abi.encode(reverseSteps)];
    ...
    function withdraw_post(address user, uint256 _amount) public onlyExecutor {
        userShares[user] -= _amount;
        totalVaultShares -= _amount;
    }


    function callback_post(bytes[] memory _arguments) internal returns (bool){
        address[] memory _targets = abi.decode(_arguments[0], (address[]));
    ...
    function getTokens() public returns (address[] memory) {
        ...
        tokens[0] = 0x431e0cD023a32532BF3969CddFc002c00E98429d;
        return tokens;
    }
