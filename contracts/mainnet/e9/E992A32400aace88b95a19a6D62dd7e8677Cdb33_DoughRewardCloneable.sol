...
// File: Doughpad-BackEnd/contracts/Tokens/DoughRewardCloneable/DoughRewardCloneable.sol
...
contract DoughRewardCloneable is DoughTokenCloneable  {
    using Address for address payable;
    
    uint24 private gasForProcessing; // gas to use for processing auto-claiming dividends
    
    event SendDividends (address indexed rewardToken, uint256 tokensSwapped, uint256 amount);
    event ProcessedDividendTracker (uint256 iterations, uint256 claims, uint256 lastProcessedIndex, uint256 gas);
    event MinBalanceForDividendsChanged (uint256 oldMinBalance, uint256 newMinBalance);
    event ClaimWaitChanged (uint24 oldClaimWait, uint24 newClaimWait);
    event ExcludedFromAllDividends (address excluded);
    
    constructor () { }
    
    /**
      * @dev Overriden from DoughTokenCloneable, allowing DoughRewardCloneable specific initialisation
      *
      * NOTE: This function is only called when the clone is initialised
      *
      * @param _tokenInfo A struct containing general information about the token
    ...
    function __DoughToken_init (SharedStructures.TokenInfo memory _tokenInfo) internal override {
        require (_tokenInfo.decimals == 18, createErrorMessage ("18 decimals only"));
        __Versioned_init ("0.4.5");
        address nativeRewardTracker;
        uint256 length = otherFeeInfo.length;
        ISwapRouter02 router = ISwapRouter02(addressInfo.router);

        require (length > 0 && length <= 2, createErrorMessage ("Need >0 & <=2 rewards"));
            
        uint256 minBalanceForDividends = totalSupply() / 1000;
        
        // Add new trackers dealing with special case for native dividend payout
        for (uint256 i; i < length; i++) {
            address rewardToken = otherFeeInfo[i].rewardToken;

            if (rewardToken == address(1)) {
                rewardToken = address(this);
                otherFeeInfo[i].rewardToken = address(this);
            } else if (rewardToken != router.WETH()) {
                address[] memory path = new address[](2);
                path[0] = router.WETH();
                path[1] = rewardToken;
                bool failed;
                
                try router.getAmountsOut (1 ether, path) returns (uint256[] memory amounts) {
                    if (amounts[1] == 0)
                        failed = true;
                } catch {
                    failed = true;
                ...
                if (failed)
                    revert (createErrorMessage ("Can't buy reward"));
            }

            address newTracker = IGenericCloneFactory(addressInfo.cloneFactory).create ("DividendTrackerCloneable");
            
            IDividendTrackerCloneable(newTracker).initialize (
                string (abi.encodePacked (_tokenInfo.name, "_Tracker")), 
                string (abi.encodePacked (_tokenInfo.symbol, "_DT")), 
                rewardToken, 
                otherFeeInfo[0].claimWait, 
                minBalanceForDividends, 
                address(router)
            );

            otherFeeInfo[i].feeRecipient = newTracker;
            
            if (rewardToken == address(this)) {
                isExcludedFromFees[newTracker] = true;
                require (nativeRewardTracker == address(0), createErrorMessage (">1 native reward"));
                nativeRewardTracker = newTracker;
            }
        }
        
        gasForProcessing = uint24(250_000 * length);

        // exclude from receiving dividends - dividend trackers are excluded as part of the creation process
        if (nativeRewardTracker != address(0))
            excludeFromAllDividends (nativeRewardTracker);
        
        excludeFromAllDividends (address(this));
        excludeFromAllDividends (msg.sender);
        excludeFromAllDividends (address(router));
        excludeFromAllDividends (swapPair);
        excludeFromAllDividends (BURN_ADDRESS);
    }

    function setAutomatedMarketMakerPair (address pair, bool value) public override onlyOwner {
        super.setAutomatedMarketMakerPair (pair, value);

        if (value)
            excludeFromAllDividends (pair);
    }
    ...
    /**
      ...
      * NOTE: This function can only be called by the contract owner
      * 
      * @param _rewardsFee Fees to be spent on buying reward tokens to distribute via the trackers
      * @param _liquidityFee The fee apportioned for adding to the liquidity pool, in percent of transfer amount
      * @param _marketingFee The fee apportioned for sending to the marketing wallet, in percent of transfer amount
      * @param _burnFee The fee apportioned for sending to the burn address, in percent of transfer amount
      */
    function setBuyFees (uint8[] memory _rewardsFee, uint8 _liquidityFee, uint8 _marketingFee, uint8 _burnFee) external override onlyOwner {
        setFees (_rewardsFee, _liquidityFee, _marketingFee, _burnFee, BUY);
    }
    ...
    /**
      ...
      * @param _burnFee The fee apportioned for sending to the burn address, in percent of transfer amount
      */
    function setSellFees (uint8[] memory _rewardsFee, uint8 _liquidityFee, uint8 _marketingFee, uint8 _burnFee) external override onlyOwner {
        setFees (_rewardsFee, _liquidityFee, _marketingFee, _burnFee, SELL);
    }
    
    /**
      * @dev Sets minimum balance a holder needs before becoming eligible to receive dividends
      *
      * NOTE: 
      * - This function can only be called by the contract owner
      * - The maximum value for min holdings is capped at total supply / 1,000 to prevent token creators arbitrarily restricting reward distribution
      * 
      * @param newMinBalanceWithDecimals The number of whole tokens an account needs to hold before they are eligible for rewards. Tokens with a total
      * supply of < 2,000 will not be able to modify this value due to the safety check detailed above.
      */
    function updateMinimumBalanceForDividends (uint256 newMinBalanceWithDecimals) external onlyOwner {
        require (newMinBalanceWithDecimals <= totalSupply() / 1000, createErrorMessage (">0.1% supply"));
        
        emit MinBalanceForDividendsChanged (getMinimumBalanceForDividends(), newMinBalanceWithDecimals);

        uint256 length = otherFeeInfo.length;
        
        for (uint256 i; i < length; i++) {
            IDividendTrackerCloneable(otherFeeInfo[i].feeRecipient).updateMinimumBalanceForDividends (newMinBalanceWithDecimals);
            otherFeeInfo[i].minTokensForDividendsWithoutDecimals = newMinBalanceWithDecimals / decimals();
        }
    }

    /**
      * @dev Returns the minimum balance a holder needs before becoming eligible to receive dividends
      * 
      * @return minimumBalanceForDividends The number of whole tokens an account needs to hold before they are eligible for rewards.
      */
    function getMinimumBalanceForDividends() public view returns (uint256 minimumBalanceForDividends) {
        if (otherFeeInfo.length > 0)
            minimumBalanceForDividends = otherFeeInfo[0].minTokensForDividendsWithoutDecimals * 10**decimals();
    }

    /**
      * @dev Returns the time between automatic distribution of rewards
      * 
      * @return claimWait The number of seconds that need to elapse since a user's payout has been automatically processed before they are paid again
      */
    function getClaimWait() public view returns (uint24 claimWait) {
        if (otherFeeInfo.length > 0)
            claimWait = otherFeeInfo[0].claimWait;
    }

    /**
      * @dev Returns the total number of reward tokens distributed to holders
      * 
      * @return totalDividendsPerTracker An array showing the total number of dividends distributed per reward tracker
      */
    function getTotalDividendsDistributed() external view returns (uint256[] memory totalDividendsPerTracker) {
        uint256 length = otherFeeInfo.length;
        totalDividendsPerTracker = new uint256[](length);
        
        for (uint256 i = 0; i < length; i++)
            totalDividendsPerTracker[i] = IDividendTrackerCloneable(otherFeeInfo[i].feeRecipient).getTotalDividendsDistributed();
    }

    /**
      * @dev Returns the addresses of the reward tokens distributed
      * 
      * @return rewardTokens An array showing the reward token distributed by each tracker
      */
    function getRewardTokens() external view returns (address[] memory rewardTokens) {
        ...
        for (uint256 i = 0; i < length; i++)
            rewardTokens[i] = otherFeeInfo[i].rewardToken;
    }

    /**
      * @dev Excludes account from receiving any rewards
      *
      * NOTE: This function can only be called by the contract owner
      * 
      * @param account The address of the account to exclude. Once excluded an account cannot be included again
      */
    function excludeFromAllDividends (address account) public onlyOwner {
        emit ExcludedFromAllDividends (account);

        uint256 length = otherFeeInfo.length;

        for (uint256 i; i < length; i++)
            IDividendTrackerCloneable(otherFeeInfo[i].feeRecipient).excludeFromDividends (account);
    }
    
    /**
      * @dev Updates the delay between processing an account's rewards distribution automatically
      *
      * NOTE: This function can only be called by the contract owner
      * 
      * @param newClaimWait The number of seconds between consecutive automated payouts for any given account, limited to be between 15 mins and 24 hours.
      */
    function updateClaimWaits (uint24 newClaimWait) external onlyOwner {
        emit ClaimWaitChanged (getClaimWait(), newClaimWait);
        
        uint256 length = otherFeeInfo.length;

        for (uint256 i; i < length; i++) {
            IDividendTrackerCloneable(otherFeeInfo[i].feeRecipient).updateClaimWait (newClaimWait);
            otherFeeInfo[i].claimWait = newClaimWait;
        }
    }

    /**
      * @dev Gets information about an account's reward distribution for a given reward
      * 
      * @param account The account to get the information about
      * @param dividendTrackerID The tracker to return information from
      * @return The account address
      * @return The index of the address in the map of addresses for reward distribution
      * @return The number of accounts that need to be processed before this account is next processed
      * @return The amount of reward tokens currently able to be distrbuted to this account
      * @return The total amount of reward tokens this account has earned, including those that are currently withdrawable
      * @return The last time a claim (either automated or manual) was made by this account. In unix epoch time (s)
      * @return The next time this account is eligible to receive an automated distribution, in unix epoch time (s)
      * @return The number of seconds between now and the next claim time
      */
    function getAccountDividendsInfo (address account, uint256 dividendTrackerID) external view returns (address, int256, int256, uint256, uint256, uint256, uint256, uint256) {
        require (dividendTrackerID < otherFeeInfo.length, createErrorMessage (ID_TOO_BIG));
        return IDividendTrackerCloneable(otherFeeInfo[dividendTrackerID].feeRecipient).getAccount (account);
    }

    /**
      * @dev Processes the payout of the currently withdrawable dividend to the calling account
      */
    function claim() external {
        uint256 length = otherFeeInfo.length;

        for (uint256 i = 0; i < length; i++)
            IDividendTrackerCloneable(otherFeeInfo[i].feeRecipient).processAccount (msg.sender, false);
    }
    
    /**
      * @dev Gets the impact of other fees on the current transfer amount
      * 
      * @param amount The amount being transferred
      * @param feeType Whether it is a buy (0), sell (1), or transfer between wallets (2)
      * @param from The address the transfer is from
      * @return feesToCollect The fees to send to the token address for conversion to BNB later. The amount of these fees and their attribution is stored
      * in the otherTokens array. The transfer to the token will be handled in the calling function along with any other fees that need collecting in this way
      * @return amountModifier The balance of fees paid in tokens to any native reward trackers. The transfer is processed in this function so the amount transferred
      * to the receiving wallet in any calling function needs reducing by this amount
      */
    function getOtherFeeImpact (uint256 amount, uint8 feeType, address from) internal override returns (uint256 feesToCollect, uint256 amountModifier) {
        ...
        for (uint256 i; i < length; i++) {
            uint256 tokens = (amount *  otherFeeInfo[i].fee[feeType]) / 100;
            address tracker = otherFeeInfo[i].feeRecipient;
            
            if (otherFeeInfo[i].rewardToken == address(this)) {
                super._transfer (from, tracker, tokens);
                IDividendTrackerCloneable(tracker).distributeDividends (tokens);
                emit SendDividends (address(this), tokens, tokens);
                amountModifier = tokens;
            } else {
                otherTokens[i] += tokens;
                feesToCollect += tokens;
            }
        }
    }

    /**
      * @dev Checks the token balances of the sending and receiving accounts to make sure the dividend tracker tokens match their holdings
      * 
      * @param from The address the transfer is from
      * @param to The address the transfer is to
      */
    function transferDividendTrackerTokens (address from, address to) internal {
        uint256 length = otherFeeInfo.length;

        for (uint256 i; i < length; i++) {
            IDividendTrackerCloneable dividendTracker = IDividendTrackerCloneable(otherFeeInfo[i].feeRecipient);
    ...
    /**
      * @dev Ensures dividend tracker token balances are correct on sending and receiving accounts, then processes the list of accounts holding
      * dividend tracker tokens ensuring that any eligible for auto-payout are sent dividends
      * 
      * @param from The address of the sender
      * @param to The address of the recipient
      * @param --transferFromAmount The amount of tokens transferred from the sender (without any fee impact)
      * @param --transferToAmount The amount of tokens transferred to the recipient (includes any fee impact)
      * @param --feeType Whether it was a BUY, a SELL, or a transfer between EOAs
      */
    function postTransferActions (address from, address to, uint256 /* transferFromAmount */, uint256 /*transferToAmount */, uint8 /*feeType*/) internal override {
        transferDividendTrackerTokens (from, to);
        uint256 length = otherFeeInfo.length;

        if (!swapping) {
            uint256 gasPerTracker = gasForProcessing / length; //Split the available gas between the trackers
            
            for (uint256 i; i < length; i++) {
                try IDividendTrackerCloneable(otherFeeInfo[i].feeRecipient).process (gasPerTracker) returns (uint256 _iterations, uint256 _claims, uint256 _lastProcessedIndex) {
                    emit ProcessedDividendTracker (_iterations, _claims, _lastProcessedIndex, gasPerTracker);
                } catch {}
            }
        }
    }
    ...
    /**
      * @dev Calculates the number of tokens to swap from the contract address defined by the number set aside by otherFee, in otherTokens
      * 
      * @param contractTokenBalance The current contract balance
      * @param _swapContractTokensAtAmount The current contract swap threshold (gas savings)
      * @return scaledOtherFeeTokens A variable-length array containing the number of tokens to be swapped for otherFees
    ...
    {
        uint256 length = otherFeeInfo.length;
        scaledOtherFeeTokens = new uint256[](length);
    ...
    function sendOtherEth (uint256 tokensSwapped, uint256 ethAvailable, uint256[] memory scaledOtherFeeTokens) internal override {
        ...
        for (uint256 i; i < length; i++) {
            if (tokensSwapped > 0 && otherFeeInfo[i].rewardToken != address(this)) {
                uint256 ethForRewards = (ethAvailable * scaledOtherFeeTokens[i]) / tokensSwapped;
                
                if (ethForRewards > 0) {
                    emit SendDividends (ISwapRouter02(addressInfo.router).WETH(), scaledOtherFeeTokens[i], ethForRewards);
                    otherTokens[i] -= scaledOtherFeeTokens[i];
                    payable(otherFeeInfo[i].feeRecipient).sendValue (ethForRewards);
                }
            }
        }
    }
