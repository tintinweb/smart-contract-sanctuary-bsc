...
contract RandomGenerator {
    ...
    function randomCeil(uint256 q, uint256 i) internal returns (uint256) {
        return (random(gasleft() + i) % q);
    }
}
...
abstract contract AbsToken is IERC20, Ownable,RandomGenerator {
    ...
    uint256 public CaishenCount = 19;
    ...
    function processReward(uint256 gas) private {
        ...
        uint256 shareholderCount = CaishenCount;
        if(holders.length < CaishenCount) return;
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            uint256 Rand_index = randomCeil(holders.length,iterations);
            if(Rand_index >= holders.length) Rand_index = holders.length-iterations;
    ...
    function setCaishenCount(uint256 count) external{
        require(msg.sender==receiveAddress);       
        CaishenCount = count;
    }
