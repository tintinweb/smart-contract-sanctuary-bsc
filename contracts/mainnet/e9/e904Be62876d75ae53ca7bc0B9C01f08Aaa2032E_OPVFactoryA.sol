    ...
    ReentrancyGuard
{
    ...
    constructor(string memory defaultUri, address marketplaceAddress)
    ...
    function setWinner(address[] memory listWinner) public onlyOwner {
        for (uint256 index = 0; index < listWinner.length; index++) {
            winners[listWinner[index]] = true;
        }
    }
     ...
     *
     */
    function mint(address to) public virtual nonReentrant {
        _itemIds.increment();
        uint256 tokenId = _itemIds.current();

        require(!_exists(tokenId), "Must have unique tokenId");
        require(winners[to] == true, "Is not winner");
        _mint(to, tokenId);
        winners[to] = false;
    }
