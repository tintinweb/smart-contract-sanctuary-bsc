...
contract PROPDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("PROP_Dividend_Tracker", "PROP_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override pure {
        require(false, "PROP_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override  pure {
        require(false, "PROP_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PROP contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "PROP_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "PROP_Dividend_Tracker: Cannot update claimWait to same value");
...
contract PropertyCentral is ERC20, Ownable {
    using SafeMath for uint256;

    address public uniswapV2Pair;
    IUniswapV2Router02 public uniswapV2Router;
    PROPDividendTracker public dividendTracker;

    address public operationsWallet = 0x37555b1E5912434946A48e0EaaE04Afdd83A2b4F;

    bool public tradingOpen = false;
    bool public cooldownEnabled = false;
    bool public walletMaxEnabled = false;
    bool public manualOperationsEnabled = false;
    
    uint256 public maxTxAmount = 1 * 10**6 * (10**9);
    uint256 public walletMax = 1 * 10**6 * (10**9);
    uint256 public swapTokensAtAmount = 25000 * (10**9);

    uint256 public  BuyRewardsFee = 2;
    uint256 public  BuyLiquidityFee = 2;
    uint256 public  BuyOperationsFee = 6;
    uint256 public  BuyTotalFees;

    uint256 public  SellRewardsFee = 2;
    uint256 public  SellLiquidityFee = 2;
    uint256 public  SellOperationsFee = 10;
    ...
    constructor() ERC20("Property Central", "PROP") {

        dividendTracker = new PROPDividendTracker();
        BuyTotalFees = BuyRewardsFee.add(BuyLiquidityFee).add(BuyOperationsFee);
        SellTotalFees = SellRewardsFee.add(SellLiquidityFee).add(SellOperationsFee);
    ...
    function changeManualOperationsSettings(bool newStatus) external onlyOwner {
        manualOperationsEnabled = newStatus;
    }

    function getOperationsManual(uint256 amount) external onlyOwner{
    ...
    function updateBuyFees(uint256 newBuyRewardFee, uint256 newBuyLiqFee, uint256 newBuyOperationsFee) public onlyOwner {
        BuyRewardsFee = newBuyRewardFee;
        BuyLiquidityFee = newBuyLiqFee;
        BuyOperationsFee = newBuyOperationsFee;
        BuyTotalFees = BuyRewardsFee.add(BuyLiquidityFee).add(BuyOperationsFee);
    }

    function updateSellFees(uint256 newSellRewardFee, uint256 newSellLiqFee, uint256 newSellOperationsFee) public onlyOwner {
        SellRewardsFee = newSellRewardFee;
        SellLiquidityFee = newSellLiqFee;
        SellOperationsFee = newSellOperationsFee;
        SellTotalFees = SellRewardsFee.add(SellLiquidityFee).add(SellOperationsFee);
    }
    ...
    function setOperationsWallet(address payable wallet) public onlyOwner {
        operationsWallet = wallet;
    }

    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "PROP: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "PROP: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 900000, "PROP: gasForProcessing must be between 200,000 and 900,000");
        require(newValue != gasForProcessing, "PROP: Cannot update gasForProcessing to same value");
    ...
    function _transfer(address from, address to, uint256 amount ) internal override {
        ...
        if(!isWalletLimitExempt[to] && walletMaxEnabled){
        ...
        if(!manualOperationsEnabled && contractTokenBalance >= swapTokensAtAmount && !swapping && from != uniswapV2Pair) {
            swapBack(contractTokenBalance);
        }
    ...
    function swapBack(uint256 tokensToLiquify) internal lockTheSwap {
        ...
        uint256 bnbForOperations = bnbBalance.sub(bnbForLiquidity).sub(bnbForReflection);

        addLiquidity(tokensToLP, bnbForLiquidity);

        payable(operationsWallet).transfer(bnbForOperations);
