...
contract DecubateStaking is Ownable, InterestHelper, IDecubateStaking {
  ...
  mapping(uint256 => uint256[2]) private priceBuffer;
  ...
  function canUnstake(uint256 _pid, address _addr)
    public
    view
    override
    returns (bool)
    ...
    returns (uint256 value)
  {
    ...
    if (from < to) {
      ...
      value = (accrueInterest(user.totalInvested, rayValue, to.sub(from))).sub(
        user.totalInvested
      );
    }
  ...
  function _updateRatio(uint256 _pid) internal {
    ...
    if (
      pool.endDate > block.timestamp &&
      pool.lastUpdatedTime + timeGap <= block.timestamp
    ) {
      ...
      uint256 priceOfInput = priceInputAverage.mul(1e18); // Price of 1 input token in BNB
      uint256 priceOfReward = priceRewardAverage.mul(1e18); // Price of 1 reward token in BNB
   ...
   *
   */
  function getPrices(uint256 _pid)
    internal
    view
    returns (uint256 priceInput, uint256 priceReward)
  {
    Pool memory pool = poolInfo[_pid];
