    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            if(from == capitalPoolAddr) { 
                ...
                LFee = amount.mul(buyTokenBFee).div(10000);
                super._transfer(from, bFreeAddr, LFee);
                uint256 shareAmount = (getBpoolFireshare(to)).add(LFee);
                setBpoolFireshare(to, shareAmount);
                
                MFee = amount.mul(buyTokenChangeBFee).div(10000);
                super._transfer(from, changeAddr, MFee);
                uint256 userAutoAmountB = (getAutoBuyChangeBAmountList(to)).add(MFee);
