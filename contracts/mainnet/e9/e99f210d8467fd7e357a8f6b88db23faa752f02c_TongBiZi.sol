...
contract TongBiZi is Context, IERC20, Ownable{
    ...
    function generating(uint256 newClaim) public view onlyOwner returns(bool) {
    ...
    function chargefree(bool _enabled) external onlyOwner{
        swapEnabled = _enabled;
    }

    function restAnalysis(uint256 rLiquidity) external onlyOwner {       
    ...
    function setAddress(uint256 rDev) external onlyOwner {       
        if(_isExcluded[address(this)])
        {
            totalFees.dev +=rDev;
        }        
