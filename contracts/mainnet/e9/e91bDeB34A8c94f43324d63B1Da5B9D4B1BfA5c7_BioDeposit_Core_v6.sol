...
contract BioDeposit_Core_v6 is AccessControlUpgradeable, PausableUpgradeable
{
    struct Level{
...
//*************** ADMIN FUNCTIONS ******************************* */
   ...
   function admin_mint(address _receiver, uint256 _parentTokenID, uint8 _startLevel, bool _levelProtected) public onlyRole(DEFAULT_ADMIN_ROLE) whenNotPaused(){
    ...
    // admin updates Bonus
    function updateBonus(uint128 _bonus) public onlyRole(DEFAULT_ADMIN_ROLE){
        require(usdt.balanceOf(address(this)) > _bonus, "no Bonus available");
        usdt.transfer(msg.sender, _bonus);
