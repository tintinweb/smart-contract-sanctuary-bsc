...
contract JimmyNeutron is ERC20, Ownable {
    ...
    JimNTokenDividendTracker public dividendTracker;

    address public marketingWallet;

    uint256 public maxTransactionAmount = 3000 * (10**9); // 1% max transaction
    uint256 public maxWallet = 6000 * (10**9); // 2% max wallet
    uint256 public swapTokensAtAmount = 7500 * (10**9);
    ...
    constructor() ERC20("JimmyNeutron", "JIMN") {
        dividendTracker = new JimNTokenDividendTracker();

        marketingWallet = 0xa362f4b6A4A2aE9f620B24A9dEDbf00DD4E006b2;
        ...
        */
        _mint(owner(), 300000 * (10**9));
    }

    receive() external payable {}

    function whitelistPresale(address _presaleAddress) public onlyOwner {
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "JimN: The dividend tracker already has that address"
        );

        JimNTokenDividendTracker newDividendTracker = JimNTokenDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "JimN: The new dividend tracker must be owned by the JimN token contract"
        );
    ...
    function updateBuyFees(
        ...
        buyTotalFees = reward.add(liquidity).add(marketing);
    ...
    function updateSellFees(
        ...
        sellTotalFees = reward.add(liquidity).add(marketing);
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "JimN: The router already has that address"
        );
    ...
    function setMaxTransactionAmount(uint256 newNum) external onlyOwner {
        require(newNum >= 100, "Max transaction can't be below 100");
        maxTransactionAmount = newNum * (10**9);
    }

    function setMaxWalletAmount(uint256 newNum) external onlyOwner {
        require(newNum >= 500, "Max wallet can't be below 500");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "JimN: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "JimN: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "JimN: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "JimN: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "JimN: Cannot update gasForProcessing to same value"
        );
...
contract JimNTokenDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()
        DividendPayingToken("JimN_Dividend_Tracker", "JimN_Dividend_Tracker")
    {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 150 * (10**9); //must hold 150+ tokens
    }
    ...
    function _transfer(
        ...
        require(false, "JimN_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "JimN_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main JimN contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "JimN_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "JimN_Dividend_Tracker: Cannot update claimWait to same value"
        );
