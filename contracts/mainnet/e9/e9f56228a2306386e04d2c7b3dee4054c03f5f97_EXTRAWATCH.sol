...
contract EXTRAWATCH is IERC20, Context, Ownable {
    ...
    address private _______Marketing_Wallet = 0x111D173b962F8f28C24d2b80FDcd6ebd6AF189bE;
    address private _______Team_Wallet = 0x111D173b962F8f28C24d2b80FDcd6ebd6AF189bE;

    string private constant _name = "ExtraWatch";
    string private constant _symbol = "Ewatch";

    uint256 public _______max_Buy_Size = 1000000 * 10**9;
    uint256 public _______max_Sell_Size = 1000000 * 10**9;

    uint256 public _______max_Wallet_Size = 2000000  * 10**9;
    uint256 private swapTokensAtAmount = 400000 * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[_______Marketing_Wallet] = true;
        _isExcludedFromFee[_______Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[_______Marketing_Wallet] = true;
        allowedTransfer[_______Team_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= _______max_Buy_Size, "You are exceeding _______max_Buy_Size");
            require(balanceOf(to) + amount <= _______max_Wallet_Size, "You are exceeding _______max_Wallet_Size");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= _______max_Sell_Size, "You are exceeding _______max_Sell_Size");
            if(to != pair){
                require(balanceOf(to) + amount <= _______max_Wallet_Size, "You are exceeding _______max_Wallet_Size");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(_______Marketing_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(_______Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function _______Update_MarketingWallet(address newWallet) external onlyOwner{
        _______Marketing_Wallet = newWallet;
    }
    
    function _______Update_TeamWallet(address newWallet) external onlyOwner{
        _______Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        _______max_Buy_Size = maxBuy * 10**decimals();
        _______max_Sell_Size = maxSell * 10**decimals();
    }
    
    function _______Max_Wallet_Size(uint256 amount) external onlyOwner{
        _______max_Wallet_Size = amount * 10**decimals();
    }
