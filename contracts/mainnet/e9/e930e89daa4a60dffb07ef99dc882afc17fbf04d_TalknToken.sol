...
contract TalknToken is ERC20, Ownable {
  address public immutable pancakePair;

  string private constant _name = "Talkn";
  string private constant _symbol = "TLKN";
  uint8 private constant _decimals = 18;

  uint256 constant initialSupply = 5000000000 * (10**18);

  address public teamWallet = 0x792114AE4781bf7857885e2B22532af61BfA169C;
  ...
  constructor() ERC20(_name, _symbol) {
    address _pancakePair = pancakeFactory.createPair(address(this), wbnbTokenAddress);
  ...
  function excludeFromFee(address account, bool excluded) public onlyOwner {
    require(_isExcludedFromFee[account] != excluded, "TLKN: Cannot change to the same value");
  ...
  function _transfer(
    ...
    if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
      if (from == pancakePair || to == pancakePair) {
        ...
        super._transfer(from, teamWallet, feeAmount);
      } else {
