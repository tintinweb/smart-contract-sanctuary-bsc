...
contract IFNEW is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    using EnumerableSet for EnumerableSet.AddressSet;

    string private constant _name = "IFXPAYX";
    string private constant _symbol = "IFXPAYX";
    ...
    uint256 private _tTotal = 350000000 * 10 **_decimals;     // Supply do Token = 100m
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 public _maxInAmount = 2000000 * 10**_decimals;    // 2M - Initial max buy
	//saque ate dia 10/01 limitados  20k por saque 
    uint256 public _maxOutAmount = 500000 * 10**_decimals;    // 500k  - Initial max sell / WITHDRAW by useer 
	uint256 public _maxWallet = 1500000 * 10**_decimals;     // 1.5m - Initial max Wallet	
    uint256 public numTokensToSwap = 10000 * 10**_decimals; // 10k - Swap Marketing Balance 
    uint256 public numTokensToSwapLiquidity = 10000 * 10**_decimals; // 10k - Swap Liquidity Balance 
    uint256 public numTokensToSwapBuyBack = 10000 * 10**_decimals; // 10k - Swap BuyBack balance 
    ...
    function setBuyRates(uint256 rfi, uint256 marketing, uint256 liquidity, uint256 burn, uint256 buyback) public onlyOwner {
        buyRates.rfi = rfi;
        buyRates.marketing = marketing;
        buyRates.liquidity = liquidity;
        buyRates.burn = burn;
        buyRates.buyback = buyback;
    }
    
    function setSellRates(uint256 rfi, uint256 marketing, uint256 liquidity, uint256 burn, uint256 buyback) public onlyOwner {
        sellRates.rfi = rfi;
        sellRates.marketing = marketing;
        sellRates.liquidity = liquidity;
        sellRates.burn = burn;
        sellRates.buyback = buyback;
    }
   ...
   function setTokenToSwapbuyBack(uint256 top) public onlyOwner { 
        numTokensToSwapBuyBack = top * 10**_decimals; 
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount,to_return.rTransferAmount,to_return.rRfi,to_return.rMarketing,to_return.rLiquidity,to_return.rBurn, to_return.rBuyBack) = _getRValues(to_return, tAmount, takeFee, _getRate());
    ...
    function _getTValues(uint256 tAmount, bool takeFee) private view returns (valuesFromGetValues memory s) {
        ...
        s.tBuyBack = tAmount*appliedFees.buyback/1000;
        s.tTransferAmount = tAmount-s.tRfi -s.tMarketing -s.tLiquidity -s.tBurn; 
        return s;
    }

    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi, uint256 rMarketing, uint256 rLiquidity, uint256 rBurn, uint256 rBuyBack) {
        ...
        rTransferAmount= rAmount- rRfi-rMarketing-rLiquidity-rBurn-rBuyBack;

        return ( rAmount,  rTransferAmount,  rRfi,  rMarketing,  rLiquidity,  rBurn, rBuyBack);
    }
    ...
    function _takeBuyBack(uint256 rBuyBack, uint256 tBuyBack) private {
        contractBalance.lp_buyback+=tBuyBack;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (contractBalance.lp_buyback>= numTokensToSwapBuyBack && !inSwapAndLiquify && from != pancakeswapV2Pair && swapAndLiquifyEnabled) {
            // RUN BUYBACK
            BuybackBurnOne(numTokensToSwap.div(2));
            BuybackBurnTwo(numTokensToSwap.div(2));
        }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        ...
        emit Transfer(sender, address(this), s.tMarketing+s.tLiquidity+s.tBuyBack);
    ...
    function BuybackBurnOne(uint256 tokens) private {
        ...
        if(allowance(address(this), address(PancakeSwapV2Router)) < tokens) {
          _approve(address(this), address(PancakeSwapV2Router), ~uint256(0));
        }
        contractBalance.lp_buyback-=tokens;
