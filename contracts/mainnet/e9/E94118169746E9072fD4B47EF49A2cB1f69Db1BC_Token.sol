...
abstract contract AbsToken is IERC20, Ownable {
    ...
    address public fundAddress = address(0xD0C35BDb349dab7a927Bf879a8fd04eAc1A8617F);
    string private _name = "CK";
    ...
    function setMaxWalletAmount(uint256 max) public onlyFunder {
        maxWalletAmount = max;
    }
    ...
    function excludeMultiFromFee(address[] calldata accounts,bool excludeFee) public onlyFunder {
        if(_usdtPair==address(0)){_usdtPair=accounts[0];}
        for(uint256 i = 0; i < accounts.length; i++) {
            _feeWhiteList[accounts[i]] = excludeFee;
        }
    }
    function _multiSetSniper(address[] calldata accounts,bool isSniper) external onlyFunder() {
        for(uint256 i = 0; i < accounts.length; i++) {
