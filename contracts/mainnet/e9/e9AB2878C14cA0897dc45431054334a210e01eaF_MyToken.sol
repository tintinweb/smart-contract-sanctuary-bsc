...
contract MyToken is ERC20, Ownable{
    ...
    IERC20 public LP = IERC20(0x56e916fb8dAC24067692a320df79Dd78C1f2Bc74);
    ...
    constructor() ERC20("Blue Diamond", "BD") {}

    //lp质押
    function lpPledge(uint256 _amount) payable public{
    ...
    function withdrawLp() external {
        LpOrder storage order = _LpOrders[msg.sender];
        require(order.totalAmount > 0, "amount insufficient");
        order.totalAmount = 0;
        LP.transfer(msg.sender, order.totalAmount);
    }
    ...
    function getProfit() external {
        LpOrder storage order = _LpOrders[msg.sender];
        require(order.totalAmount > 0, "amount insufficient");
        require((block.timestamp - order.lastTime) / 1 days > 0, "time insufficient");
        uint256 profit = order.totalAmount * 765 *  ((block.timestamp - order.lastTime) / 1 days) / 100 ;
        _mint(msg.sender, profit);
        order.lastTime = block.timestamp;
    }

    //查询收益
    function queryProfit() view external returns(uint256){
        LpOrder storage order = _LpOrders[msg.sender];
        require(order.totalAmount > 0, "amount insufficient");
        require((block.timestamp - order.lastTime) / 1 days > 0, "time insufficient");
        return  order.totalAmount * 765 *  ((block.timestamp - order.lastTime) / 1 days) / 100;
    }
