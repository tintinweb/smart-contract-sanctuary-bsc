...
contract ServeThePeople is Context, IERC20, Ownable {
    ...
    address payable public              devWallet = payable(0x0A6c3EC6959b88F40652286cbe493097FfC2ba61);
    address payable public         treasuryWallet = payable(0xeC97171636982221c25E4cA70aB04eE060846D32);
    address payable public          rewardsWallet = payable(0x5b729f76A2E5a57951D3957D7cf9Bf7dC17fF331);
    address payable public        liquidityWallet = payable(0xdc78c9bE297e69e11Ae634969dE748a89085C4b7);

    string private                          _name = "Serve The People";
    ...
    uint256 public           treasuryFeeNumerator = 6;
    ...
    uint256 public               feeRateNumerator = 2;
    ...
    address public         nativeTokenTradingPair;
    ...
    uint256 public                 launchTXNLimit = _supply.mul(1).div(100);    // 1%
    uint256 public               releasedTXNLimit = _supply.mul(1).div(100);    // 1%

    uint256 public              launchWalletLimit = _supply.mul(15).div(1000);  // 1.5%
    uint256 public            releasedWalletLimit = _supply.mul(25).div(1000);  // 2.5%
    ...
    function initContract (uint256 _launchPhaseLength, uint256 _rampUpPhaseLength, uint256 _launchBuyCooldownTimer) external onlyOwner {
        ...
        nativeTokenTradingPair = IUniswapV2Factory(tradingRouter.factory()).createPair(
            address(this),
            tradingRouter.WETH()
        );

        _setAutomatedMarketMakerPair(nativeTokenTradingPair, true);
    ...
    function enableTrading () external onlyOwner {
        ...
        require(balanceOf(nativeTokenTradingPair) > 0, "Liquidity not provided yet");
    ...
    function _transferTokens(address from, address to, uint256 amount, uint256 fee_ratio_numerator, uint256 fee_ratio_denominator) private {
        ...
        if (
            ...
            if (!_excludedFromWalletLimits[to]) {
            ...
            // If people want to use a high gas limit in general that's there business
            require(tx.gasprice <= (phase == Phase.LAUNCH ? launchGasPriceLimit : MAX), "!GAS");
            ...
            // Not allowed!
            require(_walletLastTransferBlock[tx.origin] != block.number, "!TOOMANY");
    ...
    function resolveFeeCollection() private internalCollectionsLiquification {
        uint256 pairFeeRatioTokens = balanceOf(nativeTokenTradingPair).mul(feeRateNumerator).div(feeRateDenominator);
