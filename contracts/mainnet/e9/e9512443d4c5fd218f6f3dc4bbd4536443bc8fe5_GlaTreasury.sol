...
contract GlaTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SGLA }
    ...
    address public sGLA;
    uint public sGLAQueue; // Delays change to sGLA address
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sGLA ).balanceOf( msg.sender ); // Can only borrow against sGLA held
    ...
    /**
        ...
        @param _amount uint
     */
    function repayDebtWithGLA( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SGLA ) { // 9
