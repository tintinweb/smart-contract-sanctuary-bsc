...
Rules of the “100x Lottery”.
The rules are simple.

1) Imagine a game with rounds.
2) During an active round users may buy tickets.
3) 1 ticket costs 20 BUSD.
4) There are 500 tickets available per round.
5) Each user may buy max 20 tickets per round.
6) When all tickets are sold, the round ends.
7) The winners are determined (within 1 minute)
   and the prizes are claimable immediately.
8) A new round starts automatically.


There are 25 prizes per round:

10x BUSD 100 (5x the ticket price)
8x  BUSD 250 (12.5x the ticket price)
4x  BUSD 500 (25x the ticket price)
2x  BUSD 1,000 (50x the ticket price)
1x  BUSD 2,000 (100x the ticket price)

The prizes will be randomly distributed among
the 500 ticket holders. Each ticket has an equal
chance of winning a prize.

Total prizes per round: BUSD 9,000 (90%).

DETAILS: The total ticket sales per round is
BUSD 10,000. Since BUSD 9,000 is given out to
participants as prizes, this results in a
payout of 90%. Only 10% have to be used for
development and maintenance. Given that national
lotteries only pay out around 60% of their
revenue, blockchain-based Liquiddery® Games
offer significantly higher efficiencies.

CHANCE TO WIN:
Each round, the chance for winning a prize
is 1/20 for each of the 500 tickets.
...
rights as described above: Triggering the fallback
function to emergency end a round, or send back tokens
...
contract Liquiddery is VRFConsumerBaseV2, ConfirmedOwner, ReentrancyGuard {
    ...
    uint32 public constant TOTAL_NO_OF_TICKETS = 500; /// Tickets per round
    uint32 public constant MAX_TICKETS = 20;          /// limitation per address
        ...
        external notContract nonReentrant
    {
        ...
        lastNumberTotal = addrTicketsRound[msg.sender][currentRound];
    ...
    function distributeRound() external notContract nonReentrant {
        ...
        if (!randomError) {
            ...
            BUSD_TOKEN.transfer(maintenance_address, uint256(1000E18));
            ...
            /// RANDOM_ERROR: we need new random numbers for the same round and overwrite requestIdOfRound[currentRound]
            requestIdOfRound[currentRound] = requestRandomWords();          /// save returned ID in "requestIdOfRound"
    ...
    /// @notice Enables owner to emergency end the current round, no new round,
    ...
    function emergencyEndRound() external onlyOwner {
        ...
        totalUnclaimedBUSD += TICKET_PRICE.mul(uint256(soldTickets));
    ...
    function getWinnersOfRound(uint32 round) external view returns (uint32[25] memory nums, address[25] memory addr) {
        if (round < currentRound) {
            for (uint32 i = 0; i < 25; i++) {
                nums[i] = randomNumber[round][i];
                addr[i] = participants[round][ randomNumber[round][i] ];
            }
        }
    }

    function getTicketNumsOfAddrRound(address who, uint32 round) external view returns (uint32[] memory nums) {
        uint32 tickets = addrTicketsRound[who][round];
        nums = new uint32[](tickets);
        if (tickets > 0) {
            uint32 counter = 0;
            uint32 index = 0;
            while (counter < tickets) {
                if (participants[round][index] == who) {
                    nums[counter] = index;
                    counter++;
                }
                index++;
