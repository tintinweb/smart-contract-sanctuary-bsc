...
contract NBAToken is Context,IERC20,Ownable {
    ...
    // 卖出基金分红
    uint256 public _chuJijin = 5;
    // 卖出NFT分红
    uint256 public _chuNft = 5;
    // 卖出销毁
    uint256 public _chuXiaohui = 2;
    // 卖出董事分红
    uint256 public _chuDongshi = 3;
    // 最大卖出比例99.99%
    uint256 public MAX_RATIO = 9999;
    // 购买开关 true为打开够买。false为关闭，只能白名单购买
    bool public _buySwitch = false;
    //nft奖励列表
    uint256[] nftAbountList;

    // 基金地址
    address private _jijinAddress = 0x0044164Dd42AEA889Cd1a881184e6772De7E15c4;
    // NFT地址
    address private _chuNftAddress = 0x98ABfA1a18bCeF62dAF309109295683C6acF8109;
    // 销毁地址
    address private _xiaohuiAddress = 0x0000000000000000000000000000000000000000;
    // 董事分红地址
    address private _dongshiAddress = 0xA60734c3382DE831992f3ddF72C600Ee3Afe7Ae8;
  ...
  */
   ...
   */
    ...
    // 设置购买开关
    function setBuySwitch(bool p) public onlyOwner{
        if(_buySwitch != p){
            _buySwitch = p;
        }
    }
    ...
    function setNftAddress(address account) public onlyOwner {
        if(_chuNftAddress != account){
            _chuNftAddress = account;
        }
    }
    // 设置销毁地址
    ...
    function setLpAddress(address account) public onlyOwner {
        if(_dongshiAddress != account){
            _dongshiAddress = account;
        }
    }
    ...
    // 设置卖出基金比例
    function setChuJijinRate(uint256 fee) public onlyOwner{
        _chuJijin = fee;
    }
    // 设置卖出NFT分红比例
    ...
    // 设置卖出销毁比例
    function setChuDongshiRate(uint256 fee) public onlyOwner{
        _chuDongshi = fee;
    }
   ...
   */
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if(takeFee && automatedMarketMakerPairs[sender] && !_buySwitch){
            revert("Cannot buy at present");
        }
        ...
        if(takeFee && automatedMarketMakerPairs[recipient]){
            // 到账数量
            uint256 rate = 100;
            rate = rate.sub(_chuXiaohui).sub(_chuDongshi).sub(_chuNft).sub(_chuJijin);
            uint256 daozhang_num = rate.mul(amount).div(100);
            _balances[recipient] = _balances[recipient].add(daozhang_num);

            // 基金
            uint256 jijinFee = _chuJijin.mul(amount).div(100);
            _balances[_jijinAddress] = _balances[_jijinAddress].add(jijinFee);

            // nft
            uint256 nftFee = _chuNft.mul(amount).div(100);
            _balances[_chuNftAddress] = _balances[_chuNftAddress].add(nftFee);
            // 写入列表记录，线下系统查询并分润
            ...
            // 董事
            uint256 dongshiFee = _chuDongshi.mul(amount).div(100);
            _balances[_dongshiAddress] = _balances[_dongshiAddress].add(dongshiFee);
