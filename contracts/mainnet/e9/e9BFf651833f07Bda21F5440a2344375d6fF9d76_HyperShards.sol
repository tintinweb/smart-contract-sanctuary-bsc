...
contract HyperShards  is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    HyperShardsDividendTracker public dividendTracker;
    ...
    constructor() ERC20("HyperShards ", "HPR") {
        marketingWallet = payable(0x1d666D02bb9d1499fdC273e6a3f7148aa41F15AC);
        devWallet = payable(0xd2D05E59ce00b8873216c90C67345A974D10851c);
        ...
        dividendTracker = new HyperShardsDividendTracker(
            payable(this),
            router,
...
contract HyperShardsDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    HyperShards public HyperShardsContract;
    ...
    constructor(
        ...
        defaultToken = token;
        HyperShardsContract = HyperShards(mainContract);
    ...
    function _transfer(
        ...
        require(false, "HyperShards_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "HyperShards_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main HyperShards contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(HyperShardsContract);

            uint256 prevBalance = HyperShardsContract.balanceOf(address(this));
            ...
            {
                uint256 received = HyperShardsContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    HyperShardsContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
