...
contract ActionExecutorRegistry is ManagerRole, SafeTransfer, Registry {
    ...
    function setUseGatewayFallback(uint256 _gatewayType, bool _useFallback) external onlyManager {
        require(
            gatewayMap[_gatewayType] != address(0),
            "gateway-type"
        );
    ...
    function _setGateway(uint256 _gatewayType, address _gatewayAddress, bool _useFallback) private {
        require(
            ...
            "gateway-zero-address"
        );

        require(
            !isGatewayAddress[_gatewayAddress] || gatewayMap[_gatewayType] == _gatewayAddress,
