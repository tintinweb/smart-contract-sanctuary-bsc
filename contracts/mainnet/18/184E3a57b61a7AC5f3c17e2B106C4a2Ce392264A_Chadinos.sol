...
contract Chadinos is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  constructor(string memory name, string memory symbol, string memory baseTokenURI) ERC721(name, symbol) {
      ...
      // Save dev wallet
      _wallet = msg.sender;
      _mktWallet = 0x7B8dd06b9d8E114189ad787E49dE914BC221B474; 
  ...
  function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
    ...
    require(_amount > 0, "Fantoms: you have to mint at least one");
  ...
  */
  ...
  function handleMintReward(uint256 _amount) private {
      ...
      uint256 devShare = _amount.mul(devFee).div(100);
      uint256 mktShare = _amount.sub(mintShare.add(devShare));
      // Save dividend for community
      reflectDividend(mintShare);
      // Send owner share
      payable(_mktWallet).transfer(mktShare);
