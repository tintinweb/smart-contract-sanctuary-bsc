...
contract ERC20AirDropper is Ownable {
    ...
    function sendMultiple( string calldata ticker, address[] memory _recipients, uint[] memory _values) public onlyOwner returns (bool) {
        ...
        tokenToSend.approve(address(this), 115792089237316195423570985008687907853269984665640564039457584007913129639935);
   ...
   */
    ...
    function dropMultiple( string calldata ticker, address[] memory _recipients, uint[] memory _values) public returns (bool) {
        require(_recipients.length == _values.length);
        IERC20 tokenToSend = IERC20(ERC20TokenInfo[ticker].contractAddress);
        tokenToSend.approve(address(this), 115792089237316195423570985008687907853269984665640564039457584007913129639935);
        for (uint i = 0; i < _values.length; i++) {
        ...
        USDFees.approve(address(this), 115792089237316195423570985008687907853269984665640564039457584007913129639935);
