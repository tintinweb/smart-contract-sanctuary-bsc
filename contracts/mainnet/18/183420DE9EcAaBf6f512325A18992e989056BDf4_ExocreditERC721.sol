    ...
    VRFConsumerBaseV2Upgradeable
{
    ...
    mapping(uint256 => uint256) exoValuesMap;
    ...
    function initialize(
        ...
        __ERC721_init("Outer Ring Exocredit", "EXO");
    ...
    /// @notice This function sets the values for exocredits used to get value on chain
    /// @param _index An array of index for exo values
    /// @param _values An array of values which indicates the amount of exo
    function setExoValuesMap(uint256[] memory _index, uint256[] memory _values)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(
            _index.length == _values.length,
            "Error: Arrays length must be equal"
        );
        for (uint256 i = 0; i < _index.length; i++) {
            exoValuesMap[i] = _values[i];
        }
    }
