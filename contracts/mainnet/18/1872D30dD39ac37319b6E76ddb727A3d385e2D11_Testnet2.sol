...
contract Testnet2 is BEP20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() BEP20("Testnet2", "TestNET") {
	...
	function _transfer(address from, address to, uint256 amount) internal override {
		...
		if (!swapping && canSwap && swapEnable && automatedMarketMakerPairs[to]) {
			...
			uint256 tokenToLiqudityHalf = liquidityFeeTotal.div(2);
			
			uint256 tokenToSwap = tokenToDevelopment.add(tokenToMarketing).add(tokenToLiqudityHalf);
		...
		if(takeFee) 
		{
		    (uint256 allfee, uint256 farmfee) = collectFee(amount, automatedMarketMakerPairs[to], !automatedMarketMakerPairs[from] && !automatedMarketMakerPairs[to]);
			if(allfee > 0)
			{
			...
			if(farmfee > 0)
			{
			   super._transfer(from, farmFeeAddress, farmfee);
			   amount = amount.sub(farmfee);
			}
		}
