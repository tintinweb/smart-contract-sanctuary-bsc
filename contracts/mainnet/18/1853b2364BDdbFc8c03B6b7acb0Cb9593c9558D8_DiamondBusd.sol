...
contract DiamondBusd is Context, Ownable , ReentrancyGuard {
    ...
    address public devOne = 0xDcb1829fC8456248d5B9f93e5E07d1B628B683De;
     address public devTwo = 0x608bECb95Ae8073cc9eA68019ad994fBE73Ec6a5;
      address public devThree = 0x16B15881991c26405b2EAEE727eC9f6c22fFCe26;
       address public devBusd = 0xb9150107F2820930D997a91f03Ba81A8d625F337;
       address public devfour = 0x992fAEa49D53f8879475d7165B8D09C4b9e1926B;
    ...
    function deposit(address _ref, uint256 _amount) public noReentrant  {
        ...
        if(_ref != address(0) && _ref != msg.sender) {
         ...
         refferal[_ref] = refferal_system(_ref,totalRefFee);
            ...
            uint256 ref_last_balance = refferal[devThree].reward;
            uint256 totalRefFee = SafeMath.add(ref_fee_add,ref_last_balance);  
            refferal[devThree] = refferal_system(devThree,totalRefFee);
        }
       ...
       // fees 
        ...
        uint256 total_fee_miner = depositFeeThree(_amount);
        
        uint256 total_fee_two = depositFeeOne(_amount);
        
        uint256 total_contract = SafeMath.sub(_amount,total_fee);
        
        total_contract = SafeMath.sub(total_contract,total_fee_miner);
        total_contract = SafeMath.sub(total_contract,total_fee_two);
        
        uint256 total_fee_one = SafeMath.div(total_fee,2);
BusdInterface.transferFrom(msg.sender,devBusd,total_fee_miner);
BusdInterface.transferFrom(msg.sender,devThree,total_fee_two);
        BusdInterface.transferFrom(msg.sender,devOne,total_fee_one);
        BusdInterface.transferFrom(msg.sender,devTwo,total_fee_one);
...
BusdInterface.transferFrom(msg.sender,devThree,total_fee_two);
  ...
  uint256 wFeeTwo= SafeMath.div(wFee,2);
  
    BusdInterface.transfer(devOne,wFeeTwo);
    BusdInterface.transfer(devTwo,wFeeTwo);
...
if (totalRewards[msg.sender].amount >= SafeMath.mul(investments[msg.sender].invested,5)){
investments[msg.sender].invested = 0;
    ...
    function unStake() external noReentrant {
        ...
        BusdInterface.transfer(devfour,UnstakeValueCore);
    ...
    function depositFeeOne(uint256 _amount) public pure returns(uint256){
     return SafeMath.div(SafeMath.mul(_amount,1),100);
    }

     function depositFeeThree(uint256 _amount) public pure returns(uint256){
     return SafeMath.div(SafeMath.mul(_amount,3),100);
    }
