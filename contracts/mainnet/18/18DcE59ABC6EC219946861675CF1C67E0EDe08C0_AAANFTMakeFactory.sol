...
import "../interfaces/IAAANFTMintable.sol";
...
contract SmartMintableInitializable is ReentrancyGuard, Ownable {
    ...
    IAAANFTMintable public AAANFTToken;
    ...
    function initialize(
        ...
        isInitialized = true;
        AAANFTToken = IAAANFTMintable(_AAANFTToken);
        reserve = _reserve;
    ...
    function mint() external payable nonReentrant onlyWhitelist {
        ...
        remaning -= 1;
        AAANFTToken.batchMint(msg.sender, plotsCapacity);

        emit MintPlots(AAANFTToken.totalSupply() + 1, plotsCapacity);
    }

    modifier onlyWhitelist() {
        require(
            !hasWhitelistLimit ||
                AAANFTMakeFactory(SMART_MINNER_FACTORY).whitelist(msg.sender),
...
contract AAANFTMakeFactory is Ownable {
    uint256 private nonce;

    address public AAANFTToken;
    ...
    constructor(address _AAANFTToken) {
        AAANFTToken = _AAANFTToken;
    }
    ...
    function setAdmin(address admin, bool enable) external onlyOwner {
        require(admin != address(0), "AAANFTMakeFactory: address is zero");
    ...
    modifier onlyAdmin() {
        require(
            isAdmin[msg.sender],
            "AAANFTMakeFactory: caller is not the admin"
        );
        _;
    }
