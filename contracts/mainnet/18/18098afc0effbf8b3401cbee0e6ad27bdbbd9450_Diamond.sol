    ...
    AccessControlUpgradeable
{
    ...
    function initialize() initializer public {
        ...
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        lockTime = 90 days;
        tokenURIPrefix = "https://beta-apishibafriend.io/pet/";
        tokenURISuffix = "";
    }
    ...
    */
        ...
        payable
    {
        ...
        if (_currency == address(0)) {
            //Native currency
            require(msg.value >= getLatestPrice(petSale.price), "SHFStore: Not enough balance");
        } else{
            // IERC20 currency
            IERC20(_currency).transferFrom(msg.sender, address(this), petSale.price * 10**18);
        }
        petSale.bought++;
...
import "@openzeppelin/contracts/utils/math/SafeMath.sol";
import "./SHFContract.sol";
import "@openzeppelin/contracts/access/Ownable.sol"; // Access Control
import "@openzeppelin/contracts/security/ReentrancyGuard.sol";

/**
* @title Stacker Contract
* @author Hoang Daker Thanh, Vu Peara Quang
* @notice A contract that allow users to stack custom
*/
contract SHFStackingContract is Ownable{
    //Using safemath
    using SafeMath for uint256;

    IERC20 public tokenAddress;

    enum StakePlanStatus{  IN_PROGRESS, WITHDREW }
    ...
    */
    constructor(address payable externalContractAddress) {
        //TODO allow NFT contract
        tokenAddress = IERC20(externalContractAddress);
    }
    ...
    //     // (bool sent, bytes memory data) = targetContractAddress.complete{value: contractBalance}();
    //     (bool sent,) = address(tokenAddress).call({value: contractBalance}(abi.encodeWithSignature("complete()"));
    //     require(sent, "targetContractAddress.complete failed");
    // }

    /**
    * @notice Stake method that update the user's balance
    */
    function stake(uint256 planId, uint256 tokenAmount) external {

        require(contractPlans[planId].status == PlanStatus.ACTIVE , 'This plan is not acceptable');
        require(contractPlans[planId].minimumAmount <= tokenAmount , 'This plan is not acceptable');
        require(contractPlans[planId].maximumAmount >= contractPlans[planId].stakedAmount.add(tokenAmount) , 'Token amount is too large');
        ...
        userStakes[msg.sender].push(StakePlan(tokenAmount, planId, StakePlanStatus.IN_PROGRESS , block.timestamp));
    ...
    */
    function withdraw(uint256 stakeId) public{
        ...
        require(userStakes[msg.sender][stakeId].amount > 0, "You don't have balance to withdraw");
        require(userStakes[msg.sender][stakeId].status == StakePlanStatus.IN_PROGRESS , "Your stake plan must be completed");
        // Check stake plan can withdraw
        require(checkWithDrawAble(stakeId) == true, "Stake plan still in locktime");
        ...
        require(tokenAddress.transfer(msg.sender,withdrawAmount), "Failed to send user balance back to the user");
        emit WithDraw(msg.sender, withdrawAmount , stakeId);
    }

    function calculateInterest(uint256 stakeId ) public view returns(uint){
        uint256 planId;
    ...
    function checkWithDrawAble(uint256 stakeId ) public view returns(bool){
        uint256 withdrawTime;
        uint256 planId;
        planId = userStakes[msg.sender][stakeId].planId;
        withdrawTime = userStakes[msg.sender][stakeId].stakeTime.add(contractPlans[planId].period * 1 days) ;
    ...
    /*
        ...
        PlanStatus status;
        uint256 rate;
        uint256 minimum; // Minimum of token to be staked
    */
    function addPlan(uint256 period, uint256 rate , uint256 minimumAmount , uint256 maximumAmount ) external onlyOwner {
        uint256 maximumWithInterest = maximumAmount.mul(rate).div(100);
        require(period > 0 , "Plan period should be positive");
        require(rate > 0 , "Plan interest should be positive");
        require(minimumAmount >= 0 , "Plan minimum should be positive");
        require(maximumAmount >= minimumAmount , "Plan minimum should smaller than maximum");

        // require(tokenAddress.allowance(owner(), address(this)) >= maximumWithInterest, "Not enough tokens");
        tokenAddress.transferFrom(owner(), address(this), maximumWithInterest);
        contractPlans.push(ContractPlan(period, PlanStatus.ACTIVE, rate , minimumAmount, maximumAmount, 0));
    }

    function deactivePlan(uint256 planId ) external onlyOwner {
        require(contractPlans[planId].status == PlanStatus.ACTIVE ,'Plan not found');
        contractPlans[planId].status = PlanStatus.INACTIVE;
    }

    function activePlan(uint256 planId ) external onlyOwner {
        require(contractPlans[planId].status == PlanStatus.INACTIVE ,'Plan not found');
    ...
    function userStake() public view returns(StakePlan[] memory){
        StakePlan[] memory ret = new StakePlan[](userStakes[msg.sender].length);
        for (uint i = 0; i < userStakes[msg.sender].length; i++) {
            ret[i] = userStakes[msg.sender][i];
        }
        return ret;
    }

    function getAllPlans() public view returns (ContractPlan[] memory){
