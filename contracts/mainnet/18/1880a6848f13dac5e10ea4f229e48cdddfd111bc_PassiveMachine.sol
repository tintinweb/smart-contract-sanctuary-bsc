...
contract PassiveMachine is HADToken {
    ...
    uint256 public constant ticketPrice = 0.001e18;
...
event NewWinner(address indexed wallet, uint256 amount, uint256 timeStamp);
 ...
 function stake(uint256 _amount, address ref) external {
        ...
        require(_amount >= INVEST_MIN_AMOUNT , "You should stake at least 20 HAD.");
...
function random() private view returns(uint){
    ...
    function pickWinner_lotto() public {
        ...
        if(players.length <= 0) {
            uint256 lottoTimeStamp = block.timestamp + 1 hours;
            ...
            uint256 marketingFees = lottoBalance.mul(MARKETING_FEE).div(PERCENTS_DIVIDER);
            ...
            emit NewWinner(players[index], userReward, block.timestamp);
        }
     }
