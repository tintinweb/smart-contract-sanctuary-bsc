...
contract Mining is Ownable, ReentrancyGuard {
    ...
    // 上一期的可以领取时间
    uint256 private _lastCanTakeTime;
    ...
    // 每个地址可质押的总量，总池子的百分之10
    uint256 public canRatio = 10;
    ...
    constructor(
        ...
        _lastCanTakeTime = block.timestamp.add(oneIssueTime); // 上一期的可以领取时间，等于当前时间+时间。
        // 默认第一轮 第一期
        turn = 1;
        issue[turn] = 1;
        // 开始第一轮第一期
        _startIssue(turn, issue[turn], 1000*(10**18));
    }
    ...
    function setCanRatio(uint256 _canRatio) public onlyOwner {
        require(_canRatio > 0 && _canRatio < 100, "canRatio error");
        canRatio = _canRatio;
    }
    // 设置消耗水晶的百分比
    ...
    function setNowIssueEndTime(uint256 _newEndTime) public onlyOwner {
        ...
        require(_newEndTime > _nowIssueMsg.startTime && _newEndTime < _nowIssueMsg.canTakeTime, "time error 1");
    ...
    function _startIssue(uint256 _turn, uint256 _issue, uint256 _totalUsdt) private {
        ...
        _lastCanTakeTime = _lastCanTakeTime >= block.timestamp.add(oneIssueTime) ? _lastCanTakeTime : block.timestamp.add(oneIssueTime);
    ...
    function userDeposit(uint256 lpAmount) public nonReentrant {
        ...
        if(orderOwnerOf[_order] == address(0)) {
            ...
            userOrderMsg[_order] = OrderMsg({count: lpAmount, didNFT: _didNFT, takedAmount: 0, turnOf: turn, issueOf: _nowIssue, isTaked: 0});
        }else {
            // 不是第一次质押
        ...
        // 质押总量不能超过占比
        require(userOrderMsg[_order].count <= _nowIssueMsg.total.mul(canRatio).div(100), "deposit can ratio error");
