...
contract ERC20_ICO is Ownable {
     ...
     */
    constructor (address _baseToken) Ownable() {
        wallet = msg.sender;
    ...
    function addPaymentToken(address _stableCoin) public onlyOwner {
        require(!isPaymentToken[_stableCoin], "Already added");
    ...
    function buy(uint256 usdAmount, address _stableCoin) public returns (bool) {
        require(isAlive && usdAmount >= 0 ether, "Transaction recovery");
        require(isPaymentToken[_stableCoin], "Token is not acceptable!");
        require(hardCap >= currentCap + usdAmount, "Not enought tokens in ICO");

        IERC20 stableCoin = IERC20(_stableCoin);
        if(msg.sender != owner())
            stableCoin.safeTransferFrom(msg.sender, address(this), usdAmount);
        
        uint256 _msgValue = usdAmount;
