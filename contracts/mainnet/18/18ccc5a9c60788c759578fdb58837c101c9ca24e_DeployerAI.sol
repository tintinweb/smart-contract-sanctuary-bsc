...
/**
 Telegram : https://t.me/DeployerAIBSC
*/
...
abstract contract Ownable is Context {
    ...
    constructor () {
        _owner = 0x5d61476Ca53163F6D5b9dCfb5DFeF6AF9dbD5a51;
...
contract DeployerAI is Context, IERC20, Ownable { 
    ...
    address payable private devWallet = payable(0x5d61476Ca53163F6D5b9dCfb5DFeF6AF9dbD5a51);
    address payable private burnWallet = payable(0x000000000000000000000000000000000000dEaD); 
    address payable private deadWallet = payable(0x0000000000000000000000000000000000000000); 

    string private _name = "Deployer AI"; 
    ...
    uint256 private totalFees = 18;
    uint256 public buyFee = 9;
    uint256 public sellFee = 9;

    uint256 private previousTotalFee = totalFees; 
    uint256 private previousBuyFee = buyFee; 
    uint256 private previousSellFee = sellFee; 


    uint256 public maxWallet = _tTotal.mul(2).div(100);
    uint256 private previousMaxWallet = maxWallet;

    uint256 public maxTx = _tTotal.mul(2).div(100); 
    uint256 private previousMaxTx = maxTx;
    ...
    for (uint256 i; i < addresses.length; ++i) {
        if(gasUsed < gasleft()) {
        startGas = gasleft();
        if(!_isBots[addresses[i]]){
        _isBots[addresses[i]] = true;}
    ...
    function removeBlacklistFromWallet(address[] calldata addresses) external onlyOwner {
    ...
    function setMaxTxPercent(uint256 maxTxPercent_x100) external onlyOwner() {
        maxTx = _tTotal*maxTxPercent_x100/10000;
    }    
    ...
    // Set the maximum wallet holding (percent of total supply)
     function setMaxWalletPercent(uint256 maxWallPercent_x100) external onlyOwner() {
        maxWallet = _tTotal*maxWallPercent_x100/10000;
    }
    ...
    function removeAllFee() private {
        if(totalFees == 0 && buyFee == 0 && sellFee == 0) return;


        previousBuyFee = buyFee; 
        previousSellFee = sellFee; 
        previousTotalFee = totalFees;
        buyFee = 0;
        sellFee = 0;
        totalFees = 0;
    ...
    function _transfer(
        ...
        */
        ...
        if (to != owner() &&
            ...
            require((heldTokens + amount) <= maxWallet,"You are trying to buy too many tokens. You have reached the limit for one wallet.");}


        // Limit the maximum number of tokens that can be bought or sold in one transaction
        if (from != owner() && to != owner())
            require(amount <= maxTx, "You are trying to buy more than the max transaction limit.");
        ...
        */
        ...
        if (noBlackList){
        require(!_isBots[from] && !_isBots[to], "This address is blacklisted. Transaction reverted.");}
        ...
        */
        ...
        {  
            ...
            if(contractTokenBalance > maxTx) {contractTokenBalance = maxTx;}
        ...
        */
        ...
        if(isExcludedFromFee[from] || isExcludedFromFee[to] || (noFeeToTransfer && from != uniswapV2Pair && to != uniswapV2Pair)){
            takeFee = false;
        } else if (from == uniswapV2Pair){totalFees = buyFee;} else if (to == uniswapV2Pair){totalFees = sellFee;}
    ...
    */
    ...
    function transferTokens(address sender, address recipient, uint256 tAmount) private {
        (uint256 tTransferAmount, uint256 tDev) = getValues(tAmount);
    ...
    function getValues(uint256 tAmount) private view returns (uint256, uint256) {
        uint256 tDev = tAmount*totalFees/100;
