...
contract Pool is Ownable {
    ...
    IERC20 public _stablecoinToken;
    INFTCollection public _nftEighthsRoundContract;
    INFTCollection public _nftFourthRoundContract;
    INFTCollection public _nftSemifinalRoundContract;
    INFTCollection public _nftFinalRoundContract;
    ...
    function amountToClaim(uint8 roundId, uint256 nftId)
        public
        view
        returns (uint256)
    {
        require(roundId < 5 && roundId != 0, "Incorrect round id");
        if (_nftIdClaimedInRound[roundId][nftId]) {
            return 0;
        }
        if (roundId == 1) {
            if (
    ...
    function claim(uint8 roundId, uint256 nftId) external {
        validateNftId(roundId, nftId);
        uint256 totalAmount = amountToClaim(roundId, nftId);
        require(totalAmount != 0, "There is nothing to claim");

        require(roundId < 5 && roundId != 0, "Incorrect round id");
        _nftIdClaimedInRound[roundId][nftId] = true;
    ...
    function validateNftId(uint8 roundId, uint256 nftId) private view {
        if (1 == roundId) {
            require(
                msg.sender == _nftEighthsRoundContract.getOwnerNftId(nftId),
                "msg.sender does not own this nft"
    ...
    function onlyRound(Round expected) private view {
        require(
            actualRound == expected,
            "you can not perform this action in this round"
        );
    }
