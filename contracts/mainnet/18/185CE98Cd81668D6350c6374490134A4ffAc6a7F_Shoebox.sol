...
contract Shoebox is Ownable, ERC1155 {
    ...
    function mint(uint256 id, uint256 amount) external {
        ...
        _mint(mainAddr, id, amount, "");
    }

    function mintBatch(uint256[] memory ids, uint256[] memory amounts) external {
        require(msg.sender == mainAddr, "Address is not the main address");
        _mintBatch(mainAddr, ids, amounts, "");
    }

    function balanceShoeboxBatch(address account, uint256[] memory ids) external view returns(uint256[] memory, uint256[] memory) {
        address[] memory addrs = new address[](ids.length);
        // address[] memory addr;
        for (uint i = 0; i < ids.length; i++)
            addrs[i] = account;
        
        return (ids, balanceOfBatch(addrs, ids));
    }
