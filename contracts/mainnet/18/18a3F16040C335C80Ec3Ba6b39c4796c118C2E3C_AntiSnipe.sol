...
contract AntiSnipe is ERC20, ERC20Burnable,AccessControl,ERC20Permit,Ownable{
    ...
    uint256 public saleBurnPercentage = 5 * percision;
    uint256 public buyBurnPercentage = 0 * percision;
    address public taxCollecter;
    uint256  public maxTxAmount;
    constructor(uint256 initialSupply,string memory name,string memory symbol,address to,address _taxCollecter) ERC20(name, symbol)   ERC20Permit(name){
        _transferOwnership(to);
        _grantRole(DEFAULT_ADMIN_ROLE, to);
        _grantRole(tax_exempt, to);
        _grantRole(tax_exempt, _taxCollecter);
        taxCollecter = _taxCollecter;
    ...
    function transferFrom(address from,address to,uint256 amount) public virtual override  returns (bool) {
        ...
        if(saleBurnPercentage!=0 && !senderIsLiquidityPool && taxCollecter!= address(0) && receiverIsLiquidityPool && !hasRole(tax_exempt, from) ){
            
            tax = (amount * saleBurnPercentage)/(100 * 1e18);
            _transfer(from, taxCollecter, tax);
        }
        // buy from lp to user # note max tx amount will apply
        if(buyBurnPercentage!=0 && senderIsLiquidityPool && taxCollecter!= address(0) && !receiverIsLiquidityPool && !hasRole(tax_exempt, to)){
    ...
    function transfer(address to, uint256 amount) public virtual override returns (bool)  {
        ...
        // sell from user to lp
        if(saleBurnPercentage!=0 && !senderIsLiquidityPool && taxCollecter!= address(0) && receiverIsLiquidityPool && !hasRole(tax_exempt, owner)){
            
            tax = (amount * saleBurnPercentage)/(100 * 1e18);
            _transfer(owner, taxCollecter, tax);
        }
        return true;
    }
