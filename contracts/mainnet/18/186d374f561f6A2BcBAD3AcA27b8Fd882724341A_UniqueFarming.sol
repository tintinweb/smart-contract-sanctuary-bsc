...
contract UniqueFarming 
{
   ...
   constructor(address _PullAddress, address _DevAddress, address _controller)
   {
    ...
    function getFees(uint256 value) private view returns(uint256 devFee, uint256 vaultFee) 
    { 
        return (
            getUserIncomePercent(value, PullValuePercent),
            getUserIncomePercent(value, DevRewardPercent)
        );
    }
