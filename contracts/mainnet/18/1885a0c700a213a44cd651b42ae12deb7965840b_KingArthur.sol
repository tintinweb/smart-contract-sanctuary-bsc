...
contract KingArthur is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 888 * 10 ** _decimals;

    string private _name = "KingArthur";
    string private _symbol = "KingArthur";
    

    uint256 public _interFee = 30;
    ...
    uint256 public _reflowFee = 5;
    uint256 public buyTotalFee = 60;

    // nostart
    uint256 public restFee = 400;

    uint256 public smallest = 5 * 10 ** 16;
    ...
    address public MKAddress = address(0x8278a7f99Fd1bB65657535De7ef6F10b37DdF658);
    address public lpToken = address(0xbA415523F91aD255C9e31F97a838a3f6cc02b3C6);
    address public interAddress = address(0x62ba5cBcD22cD01c86Cb8D559603190c327d15A9);
    address public tokenReceiver;

    bool public swapsEnabled = true;
    bool public startSwitch = true;
    ...
    mapping(address => address) public relationAddress;
    ...
    function setSmallest(uint256 ft)external onlyFunder{
        smallest = ft;
    }
    ...
    function setFee(uint _inter, uint mk, uint lP, uint bf, uint burn, uint reflow, uint btf, uint rest) public onlyOwner {
        _interFee = _inter;
        _marketFee = mk;
        _lPFee = lP;
        _backflowFee = bf;
    ...
    function _initParam(uint256 tAmount,Param memory param) private view  {
        param.tInter = tAmount * _interFee / 1000;
    ...
    function _sellParam(uint256 tAmount,Param memory param) private view  {
        ...
        param.treflow = tAmount * _reflowFee / 1000;
        uint tFee = tAmount * buyTotalFee / 1000;
        param.tTransferAmount = tAmount.sub(tFee);
    }
    ...
    function _takeFee(Param memory param,address from, address to, uint amount) private {
      ...
      if( param.treflow > 0 ){
        _take(param.treflow, from, address(this));
      }
      if( param.tRest > 0 ){
        _take(param.tRest, from, MKAddress);
      }
      if( param.tInter > 0 ){
    ...
    function _takeInviterFee(address sender,address recipient,uint256 tAmount) private {
        ...
        for (int256 i = 0; i < 7; i++) {
            ...
            if (cur == address(0)) {
                ...
                _tOwned[interAddress] = _tOwned[interAddress].add(curTAmount);
                emit Transfer(sender, interAddress, curTAmount);
            } else {
                uint256 curTAmount = tAmount.div(10000).mul(rate);
    ...
    function _transfer(address from,address to,uint256 amount) private {
        ...
        if( 
            ...
            // LP
            swapAndAward(contractTokenBalance.mul(35).div(100));
            // MK
            swapAndMK(contractTokenBalance.mul(35).div(100));
            // repo
            swapAndRepo(contractTokenBalance.mul(30).div(100));
        ...
        if (!isContract(from) && !isContract(to)) {
          bool shouldSetInviter2 = relationAddress[to] == address(0) && relationAddress[from] != to && !isContract(from) && !isContract(to);
          if (shouldSetInviter2) {
              relationAddress[to] = from;
          }
          bool shouldSetInviter = inviter[from] == address(0) && !isContract(from) && !isContract(to) && relationAddress[from] == to;
          if (shouldSetInviter) {
              inviter[from] = to;
          }
        }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount,Param memory param) private {
        ...
        if(param.takeFee){
            _takeFee(param,sender,recipient,tAmount);
        }
    }
    ...
    function process(uint256 gas) private {
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            if (amount < 1 * 10 ** 3 || IERC20(address(this)).balanceOf(shareholders[currentIndex]) < smallest) {
                currentIndex++;
                iterations++;
                continue;
            }
