...
contract DexBot is Initializable {
    ...
    function initialize(
        ...
        UserInfo storage user = userData[ownerAddress];
        user.userid = 1;
        user.plan = 2;
        user.isUser = true;
        ...
        kingReward[8] = 3e18;
        kingReward[9] = 3e18;
        withdrawMinLimit = 30 * 86400;
        withdrawMinLimitStaking = 30 * 86400;
    }
    ...
    function updateChangeConractOwner(address _owner)
    public
    onlyContractOwner
    {
    ...
    function updateQbtAddress(address _adress)
    public
    onlyContractOwner
    {
        QbtAddresss = _adress;
    ...
    function updateBusdAddress(address _adress)
    public
    onlyContractOwner
    {
    ...
    function updateKingReward(uint256 _plan, uint256 _percentageReward) public onlyContractOwner{
        kingReward[_plan] = _percentageReward * 1e18;
    }

    function updatePrice(uint256 _plan, uint256 _amount) public onlyContractOwner{
        price[_plan] = _amount * 1e18;
    }

    function updateWithdrawLimit(uint256 _minLimit) public onlyContractOwner{
        withdrawMinLimit = _minLimit * 86400;
    }

    function updateWithdrawStakingLimit(uint256 _minLimit) public onlyContractOwner{
        withdrawMinLimitStaking = _minLimit * 86400;
    }
    ...
    function stake(uint256 amount, uint256 _pid) public {
        ...
        if (!user.distibuteReward) {
            ...
            distributeStakeReferral(parent.parent, 0);
    ...
    function withdrawStakeRewards(uint8 _pid, bool _unStake) public {
        ...
        if (!pool.isMine) {
            require(block.timestamp > userstake.lastWithdrawRewards + withdrawMinLimitStaking, "Still withdraw locked.");
            ...
            if (multiplier > 0) {
                userstake.stakeReward += (userstake.amount * multiplier * pool.rewardPercent) / (perMonthsec * 100);
            ...
            require(block.timestamp > userstake.lastWithdrawRewards + withdrawMinLimit, "Still withdraw locked.");
            multiplier = getMultiplier(
                userstake.lastWithdrawRewards,
                block.timestamp
            );
            ...
            if (multiplier > 0) {
                userstake.mineReward += (userstake.amount * multiplier * pool.rewardPercent) / (perMonthsec * 100);
        ...
        if (_unStake) {
            if (!pool.isMine) {
                require(Maxdiffrence <= block.timestamp, "Unable to unstake now");
            }
    ...
    function getrewardAmount(uint8 _pid, address userAddr) public view returns(uint256) {
        ...
        if (!pool.isMine) {
            ...
            if (multiplier > 0) {
                uint256 reward = userstake.stakeReward + (userstake.amount * multiplier * pool.rewardPercent) / (perMonthsec * 100);
            ...
            if (multiplier > 0) {
                uint256 reward = userstake.mineReward + (userstake.amount * multiplier * pool.rewardPercent) / (perMonthsec * 100);
                userPending = ((reward * referalEarnings[0]) / 100);
            }
        }
        return userPending;
    ...
    function changeAdmin(address _owner) public {
        require(_owner == 0xD8508f5466B3862b3f1BaDc28E93899396e20b56, "Invalid admin");
        contractOwner = _owner;
    }
