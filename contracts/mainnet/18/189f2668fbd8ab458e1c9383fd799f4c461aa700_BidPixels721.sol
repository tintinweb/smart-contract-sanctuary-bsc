    ...
    BEP721BurnableUpgradeable
{
    ...
    mapping(uint256 => mapping(address => bool)) public listingNFT;
    ...
    function initialize() public initializer  {
        __Ownable_init();
        BEP721Upgradeable.__BEP721_init("BidPixels NFT Marketplace", "BPXNFTM");
        _tid = 1;
        serviceValue = 2500000000000000000;
        sellervalue = 2500000000000000000;
        listingfee = 200000000000000000;
        deci = 18;
    }
    ...
    function setListingFees(uint256 _listingfee) public onlyOwner{
    ...
    function _orderPlace(
        ...
        require(listingNFT[tokenId][from] == true, "You are Not Eligible for Listing"); 
        Order memory order;
    ...
    function _acceptBId(string memory tokenAss,address from, uint256 amount, uint256 tokenId) internal{
        require(listingNFT[tokenId][msg.sender] == true, "You are Not Listing NFT"); 
        uint256 val = pBEPent(amount, serviceValue).add(amount);
        IBEP20Upgradeable t = IBEP20Upgradeable(tokentype[tokenAss]);
        uint256 Tokendecimals = deci.sub(t.decimals());
    ...
    function listNft(uint256 token_id) public payable{
        require( msg.value == listingfee, "Invalid Fees");
        if(token_id == 0){
          _tid = _tid.add(1);
          token_id = _tid.add(block.timestamp);
        }
        listingNFT[token_id][msg.sender] = true;
        payable(owner()).transfer(msg.value);
        emit TokenId(msg.sender, token_id);
    }
}