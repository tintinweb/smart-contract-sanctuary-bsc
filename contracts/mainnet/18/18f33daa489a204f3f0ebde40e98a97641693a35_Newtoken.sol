...
contract Newtoken is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 100_000_000_000_000_000_000_000 * (10 ** _decimals);
    ...
    uint256 public bossFee = 3;
    uint256 private _previousbossFee = bossFee;
    ...
    uint256 public totalFees = taxFee + operationsFee + marketingFee + burnFee + bossFee;

    address public marketingWallet = 0xA4Dd2605e02d7338FC2EE39Cf1B025b54e2148df;
    address public burnWallet = 0xA4Dd2605e02d7338FC2EE39Cf1B025b54e2148df;
    address public operationsWallet = 0xA4Dd2605e02d7338FC2EE39Cf1B025b54e2148df;
    address public bossWallet = 0xA4Dd2605e02d7338FC2EE39Cf1B025b54e2148df;    
    ...
    event bossWalletChanged(address bossWallet);
    ...
    constructor() 
    { 
        address newOwner = 0xA4Dd2605e02d7338FC2EE39Cf1B025b54e2148df;
        ...
        _isExcludedFromFees[bossWallet] = true;
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 tDevTeam = calculateburnFee(tAmount);
    ...
    function _takeCharityFundrasing(address sender, uint256 tTransferAmount, uint256 rTransferAmount, uint256 tAmount) private returns (uint256, uint256) {
        if(bossFee==0) {
            return(tTransferAmount, rTransferAmount); }
        uint256 tCharity = calculatebossFee(tAmount);
    ...
    function calculatebossFee(uint256 _amount) private view returns (uint256) {
        return _amount * bossFee / 100;
    }
    ...
    function removeAllFee() private {
        if(taxFee == 0 && operationsFee == 0 && bossFee == 0 && marketingFee == 0 && burnFee == 0) return;
        
        _previousTaxFee = taxFee;
        _previousoperationsFee = operationsFee;
        _previousbossFee = bossFee;
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 taxForSwap = burnFee + marketingFee + operationsFee + bossFee;
            if(taxForSwap > 0) {
                ...
                if(burnFee > 0) {
                    uint256 devTeamBNB = newBalance * burnFee / taxForSwap;
                    sendBNB(payable(burnWallet), devTeamBNB);
                ...
                if(bossFee > 0) {
                    uint256 charityBNB = newBalance * bossFee / taxForSwap;
                    sendBNB(payable(bossWallet), charityBNB);
    ...
    function changeburnWallet(address _burnWallet) external onlyOwner {
        require(_burnWallet != burnWallet, "Developer Team wallet is already that address");
        require(!isContract(_burnWallet), "Developer Team wallet cannot be a contract");
    ...
    function changebossWallet(address _bossWallet) external onlyOwner {
        require(_bossWallet != bossWallet, "Charity Fundrasing wallet is already that address");
        require(!isContract(_bossWallet), "Charity Fundrasing wallet cannot be a contract");
        bossWallet = _bossWallet;
        emit bossWalletChanged(bossWallet);
    }

    function setTaxFeePercent(uint256 _taxFee) external onlyOwner() {
        taxFee = _taxFee;
        totalFees = taxFee + marketingFee + operationsFee + burnFee + bossFee;
