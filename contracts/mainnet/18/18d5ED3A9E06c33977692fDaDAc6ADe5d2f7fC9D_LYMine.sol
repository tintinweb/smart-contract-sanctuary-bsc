...
contract LYMine is Ownable {
    ...
    function getUserLpTokenSnapshotBalance(address _lpToken, address _user)
        public
        view
        returns (uint256)
    {
        uint256 pid = getPid(lpToken);
        return userInfo[pid][_user].lpTokenSnapshotAmount;
    }
    ...
    function deposit(address _lpToken, uint256 _amount) public {
        bool islpTokenSnapshot=false;
        if (_lpToken==lpTokenSnapshot){
            _lpToken=lpToken;
            islpTokenSnapshot=true;
        }
        ...
        if (_amount > 0) {
            if (!islpTokenSnapshot){
                ...
                IERC20(lpTokenSnapshot).safeTransferFrom(
                                address(msg.sender),
                                address(this),
                                _amount
                );
                user.lpTokenSnapshotAmount = user.lpTokenSnapshotAmount.add(_amount);
                user.amount = user.amount.add(_amount);
            }
    ...
    function withdraw(address _lpToken,uint256 _amount) public {
        require(block.timestamp>=unLockTime,"unLockTime err");
        bool islpTokenSnapshot=false;
        if (_lpToken==lpTokenSnapshot){
            _lpToken=lpToken;
            islpTokenSnapshot=true;
        ...
        if (!islpTokenSnapshot){
            ...
            require(_amount<=user.lpTokenSnapshotAmount,"out of LP value");
        }
        ...
        if (!islpTokenSnapshot){
           ...
           IERC20(lpTokenSnapshot).safeTransfer(address(msg.sender),_amount);
           user.lpTokenSnapshotAmount=user.lpTokenSnapshotAmount.sub(_amount);
        }
        updatePool(pid);
