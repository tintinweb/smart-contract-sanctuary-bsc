...
contract Doraemiao is ERC20, Ownable {
	...
	constructor() ERC20("Doraemiao", "DOM") payable {
		
		uint256 totalSupply = 1e3 * 1e8 * 1e18;
		...
		taxWallet = address(0xd248AC0c0D2921C8fF55a930ee28F74aD9E2CcD9);
	...
	function updateFees(uint256 _transferFee) external onlyOwner {
		transferFee = _transferFee;
		require(transferFee <= 200, "Must keep fees at 20% or less");
	}
	...
	function swapBack() private {
		...
		bool success;
		(success,) = address(taxWallet).call{value : bnbBalance}("");
		
		emit Swap(contractBalance, bnbBalance);
	}
