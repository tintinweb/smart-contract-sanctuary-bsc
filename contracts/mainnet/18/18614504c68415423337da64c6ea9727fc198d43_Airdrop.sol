 ...
 */
abstract contract INFT is IERC721Enumerable {
...
contract Airdrop {
    ...
    function airdropNFTs(
        ...
        require(to.length == amount.length, "Data length does not match");
        for (uint256 i = 0; i < to.length; i++) {
            uint256[] memory Ids = new uint256[](amount[i]);
            (Ids, ) = INFT(nftAddr).tokensOfOwnerBySize(
                msg.sender,
                0,
                amount[i]
            );
            INFT(nftAddr).safeTransferFromBatch(msg.sender, to[i], Ids);
        }
    }
}