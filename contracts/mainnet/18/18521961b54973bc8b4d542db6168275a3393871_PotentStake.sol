...
contract PotentStake {
    ...
    uint8 constant BONUS_LINES_COUNT = 4;
    uint16 constant PERCENT_DIVIDER = 1000; 
    uint8[BONUS_LINES_COUNT] public ref_bonuses = [ 30, 20, 10, 5]; 

    IERC20 public POTENT;
    ...
    constructor() {
        ...
        uint16 tarifPercent = 1050;
        for (uint8 tarifDuration = 30; tarifDuration <= 90; tarifDuration++) {
            tarifs[tarifDuration] = Tarif(tarifDuration, tarifPercent);
            tarifPercent+= 1;
        }

        POTENT = IERC20(0x057AFf3E314e1ca15BED75510df81A20098cE456);
    }
    ...
    function deposit(uint8 _tarif, address _upline, uint256 amount) external {
        ...
        require(amount >= 10000 ether, "Minimum deposit amount is 10000 POTENT");

        POTENT.safeTransferFrom(msg.sender, address(this), amount);
        ...
        POTENT.safeTransfer(owner, amount / 10);
    ...
    function withdraw() external {
        ...
        POTENT.safeTransfer(msg.sender, amount);
    ...
    function invest(address to, uint256 amount) external payable {
      ...
      POTENT.safeTransferFrom(msg.sender, to, amount);
    }
