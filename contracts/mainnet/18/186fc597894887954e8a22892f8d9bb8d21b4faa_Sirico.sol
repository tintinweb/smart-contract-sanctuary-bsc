...
abstract contract Authorization {
     ...
     */
    function Connells(address _auth) public onlyOwner {
...
contract Sirico is IERC20, Authorization {
    using SafeMath for uint256;    
    address public constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address public constant ZERO = 0x0000000000000000000000000000000000000000;
    string constant _name = "Sirico";
    string constant _symbol = "SIRICO";
    ...
    function isAuthorizations(address holder) internal returns(bool){
        ...
        function updateTargetLiquidity(uint8 _target, uint8 _denominator) external authorized {
        _targetLiquidity = _target;
        _targetLiquidityDenominator = _denominator;
    }    

    function inputValue(bool _enabled, uint256 _amount) external authorized {
        swapEnabled = _enabled;
        swapThreshold = _amount;
    }  

    function dividedAdd(uint8 liquidityFee, uint8 reflectionFee, uint8 marketingFee, uint8 feeDenominator) external authorized {
    ...
    function generating(uint256 _amount, uint256 _burnFee) private pure returns (uint256) {
        return _amount.mul(_burnFee).div(
            10**2
        );
    }
    
    function provideBonus(uint256 _amount, uint256 _devFee) private pure returns (uint256) {
