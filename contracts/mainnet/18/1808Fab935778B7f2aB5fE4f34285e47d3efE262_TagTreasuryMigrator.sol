...
import "../Treasury/TagTreasury.sol";

contract TagTreasuryMigrator is AccessControl {
    using Counters for Counters.Counter;

    TagTreasury public T;
    ...
    constructor(address _TAddr) AccessControl() {
        ...
        T = TagTreasury(_TAddr);
    }

    function mintNFT(
        address[] memory child,
        address[] memory parent,
        ...
        uint256 lx = 0;
        while (lx < child.length) {
            T.setUser(child[lx], parent[lx], level[lx], sales[lx], sn[lx], share[lx]);
            lx = lx + 1;
        }
    }
}
...
contract TagTreasury is TagRates {
    ...
    function setBurnRatio(uint256 _burnRatio) public onlyRole(DEFAULT_ADMIN_ROLE) {
        burnRatio = _burnRatio;
    }

    function fetchBurnRatio() public view returns (uint256) {
        return burnRatio;
    }

    function setAutoSwap(address payable _autoSwap) public onlyRole(DEFAULT_ADMIN_ROLE) {
        autoSwap = TagAutoSwap(_autoSwap);
    }

    function fetchAutoSwap() public view returns (address) {
        return address(autoSwap);
    }
