 ...
 *SmartContract XSN 
 *Token deploy to BSC
 *Telegram english : https://t.me/sencoinen
 *Telegram francais : https://t.me/sencoin
 *website:https://sencoin.net 
*/
...
contract SencoinTOKEN is Context, IBEP20, Ownable {
    ...
    uint256 private _tTotal = 80000000 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private constant _name     = "SenCoin TOKEN";
    ...
    uint256 public  _liquidityFeeBuy = 5; // 5% tax for buy tokens

    // sell taxsellfee
    uint256 public  _taxFeeSell       = 0;
    uint256 public  _liquidityFeeSell = 5; // 5% tax for sell tokens
    
    uint256 public  _maxTxAmount     = _tTotal * 100 / 10000; // 1% of the total supply
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "XSN: approve from the zero address");
        require(spender != address(0), "XSN: approve to the zero address");
    ...
    function _transfer(
        ...
        require(from != address(0), "XSN: transfer from the zero address");
        require(to != address(0), "XSN: transfer to the zero address");
        require(amount > 0, "Transfer amount must be greater than zero");
        require(!_isBlacklisted[from], "XSN: Sender is blacklisted");
        require(!_isBlacklisted[to], "XSN: Reciever is blacklisted");
        ...
        */
        ...
        if (from == uniswapV2Pair &&
            opCooldownEnabled &&
            !isTimelockExempt[to]) {
            require(cooldownTimer[to] < block.timestamp,"Please wait for one min between two operations");
