    ...
    NFTMarketAuction
{
    ...
    modifier onlyContract(address addr) {
        require(
            addr.isContract(),
            "NFTMarketReserveAuction: not contract address"
        );
        _;
    }
    ...
    modifier onlyAdminOrNonPrimaryMarket(address nftContract, uint256 tokenId) {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, msg.sender) ||
                !getIsPrimary(nftContract, tokenId),
    ...
    modifier onlyValidReservePrice(uint256 reservePrice) {
        require(
            reservePrice > 0,
            "NFTMarketReserveAuction: Reserve price must be at least 1"
        );
        _;
    }
    ...
    NFTMarketCore
{
     ...
     */
    function _getIsPrimary(address nftContract, uint256 tokenId)
        private
        view
        returns (bool)
    {
        ...
        )
    {
        ...
        if (creatorSecondaryMarketFee > 0) {
            IERC20(aliceToken).safeTransfer(
                creatorSecondaryMarketFeeTo,
                creatorSecondaryMarketFee
            );
        }
    ...
    function markTokenFirstSaleCompleted(
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            nftContractToTokenIdToFirstSaleCompleted[nftContract][
                tokenId
            ] = true;
    ...
    function requireTokenNotAlreadyMarkedAsSold(
        address nftContract,
        uint256 tokenId
    ) private view {
        require(
