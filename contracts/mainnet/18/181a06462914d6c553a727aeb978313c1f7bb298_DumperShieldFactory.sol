...
// File: contracts/DumperShieldToken.sol
...
// File: contracts/DumperShield.sol
...
library TransferHelper {
    ...
    function safeApprove(address token, address to, uint value) internal {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), "1"); //APPROVE_FAILED
    ...
    function safeTransfer(address token, address to, uint value) internal {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), "2"); //TRANSFER_FAILED
    ...
    function safeTransferFrom(address token, address from, address to, uint value) internal {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), "3"); //TRANSFER_FROM_FAILED
    ...
    function safeTransferETH(address to, uint value) internal {
        ...
        require(success, "4"); //ETH_TRANSFER_FAILED
...
*/
    ...
    modifier onlyOwner() {
        ...
        require(owner() == msg.sender, "5"); //Ownable: caller is not the owner
...
// end debug
    ...
    function setLock(address token, address user, uint256 time) external returns(bool) {
        ...
        require(msg.sender == address(dumperShieldTokens[token]), "7"); //onlyDSToken
...

    {
        ...
        require(address(dsToken) != address(0), "8");   //Token is not shielded
    ...
    function transferBatch(address token, address[] calldata to, uint256[] calldata value) external returns (bool) {

        require(to.length == value.length, "9"); // wrong length
        ...
        for (uint i = 0; i < to.length; i++) {
            ...
            _transfer(token, msg.sender, to[i], value[i]);
    ...
    function confirmOrder(uint256 orderId) external {
        ...
        require(o.confirmatory == msg.sender, "10"); //OnlyConfirmatory

        require(o.status == OrderStatus.Restricted, "11"); //Wrong status
    ...
    function cancelOrder(uint256 orderId) external {
        ...
        require(msg.sender == o.seller || msg.sender == o.buyer, "12"); //can't cancel

        require(o.status == OrderStatus.Created || o.status == OrderStatus.Restricted, "13"); // user can cancel restricted order too.
    ...
    function buyOrder(uint256 orderId, uint256 paymentAmount) external payable nonReentrant {
        ...
        require(o.buyer == address(0) || o.buyer == msg.sender, "14"); //Wrong buyer

        require(o.status == OrderStatus.Created, "15"); //Wrong status
        ...
        if (value > 0) {
            ...
            if (token == address(0)) {
                ...
                require (value != 0, "16"); // tokenAmount 0
...

    {
        ...
        require(sendToken <= address(32), "17"); //onlyCoin

        require(sendAmount == msg.value, "18"); //wrong value

        require(address(dumperShieldTokens[token]) != address(0), "19");  // Token is not in dumper shield
        ...
        require(set.totalTokenA >= tokenAmountOut, "20"); //Not enough available tokens

        require(tokenAmountOut >= buyAmountMin, "21"); //Amount is less then minimum
...

    {
        ...
        require(sendToken <= address(32), "22"); //coin
    ...
    function withdrawToken(address token, uint256 amount) external {
        ...
        require(dsUnlockDate[token] < block.timestamp, "23"); //tokens is locked

        require(locked[token][msg.sender] < block.timestamp, "24"); // tokens is locked in voting



        address dsUser = dumperShieldUsers[msg.sender];

        require(dsUser != address(0), "25");  //user0
...

    {
        ...
        require(address(dumperShieldTokens[token]) == address(0), "26"); //Token already shielded

        require(unlockDate > block.timestamp, "27");  //unlockDate in past
        ...
        require(amounts[1] != 0, "28"); //wrong router
    ...
    function setUnlockDate(address token, uint256 newUnlockDate) external onlyVoting {
        ...
        require(newUnlockDate > block.timestamp, "27"); //unlockDate in past
    ...
    function putOnSale(address token, uint256 value) external {
        ...
        require(address(dumperShieldTokens[token]) != address(0), "29");  //Token is not shielded

        (,uint256 balance) = balanceOf(token, msg.sender);

        require(balance >= value, "30"); // not enough balance
    ...
    function claimFromSale(address token) external {
        ...
        require(address(dumperShieldTokens[token]) != address(0));  //Token is not shielded

        Splitter.SplitterSet storage set = tokensSale[token];

        require(set.deposits[msg.sender].shares != 0, "31"); //You have no tokens on sale
    ...
    function _updateBalanceAndTotal(address token, address user) internal override returns (uint256 realBalance) {
        ...
        require(dsUser != address(0), "32");  // User is not in dumper shield
    ...
    function _transfer(address token, address from, address to, uint256 value) internal returns (bool) {
        ...
        require(address(dsToken) != address(0));    //Token is not shielded

        require(locked[token][from] < block.timestamp, "34"); //tokens is locked in voting
        ...
        if (from == address(this)) {
            ...
            require(total >= value, "30");  // Not enough balance
...

    {
        ...
        require(sellAmount != 0, "35"); //amount 0

        require(address(dumperShieldTokens[tokenSell]) != address(0)); //Token is not shielded

        require(locked[tokenSell][seller] < block.timestamp, "36");   //tokens is locked in voting
        ...
        if (confirmatory != address(0)) {
            ...
            require(balance >= sellAmount, "37"); //Not enough tokens on restricted account
            ...
            require(balance >= sellAmount, "38");    // Not enough tokens
    ...
    function _updateLimits(address token, uint256 buyAmount) internal {
        ...
        if (limit != 0) {
            ...
            require(limit >= buyAmount, "39"); //OutOfLimit
