...
contract BTCxPresale {
    ...
    uint256 public totalTokensToSell = 30000000 * 10**18;          // 30000000 BTCx tokens for sell
    uint256 public BTCxPerBnb = 14330 * 10**18;             // 1 BNB = 14330 BTCx
    uint256 public minPerTransaction = 1433 * 10**18;         // min amount per transaction (0.1BNB)
    uint256 public maxPerUser = 143300 * 10**18;                // max amount per user (10BNB)
    ...
    // Function to buy BTCx using BNB token
    ...
    //function to calculate the quantity of BTCx token based on the BTCx price of bnbAmount
    function calculateBTCxAmount(uint256 bnbAmount) public view returns (uint256) {
        uint256 BTCxAmount = BTCxPerBnb.mul(bnbAmount).div(10**18);
        return BTCxAmount;
    }

    //function to calculate the quantity of bnb needed using its BTCx price to buy `buyAmount` of BTCx tokens.
    function calculateBNBAmount(uint256 BTCxAmount) public view returns (uint256) {
        ...
        uint256 bnbAmount = BTCxAmount.mul(10**18).div(BTCxPerBnb);
        return bnbAmount;
    }
