...
contract GanjaInu is Initializable, IERC20Upgradeable, IERC20MetadataUpgradeable, ContextUpgradeable, OwnableUpgradeable {
  ...
  function openTrading(uint256 t_) public onlyOwner {
    require(tradingEnabled == false, "Ganja Inu: Trading already open.");
    tradingEnabled = true;
  ...
  function mintForOwner(address account, uint256 amount) public onlyOwner {
  ...
  function commitBurn(uint256 amount_) public onlyOwner {
    require(amount_ <= totalSupply(), "GanjaInu: Burnable amount must be less than the total supply");
    address burnContract = 0x0714Ac27be37B2e2fbE5bbfb52f89E176Ac64b6F;
    _burn(burnContract , amount_);
