...
contract JackDelRio is Context, IERC20, Ownable {
    ...
    string private _name = "JackDelRio";
    string private _symbol = "JDR";
    ...
    function getJackDelRioVotes(address account) external view returns (uint256) {
        uint32 nCheckVotes = numCheckVotes[account];
        return nCheckVotes > 0 ? checkVotes[account][nCheckVotes - 1].vot : 0;
    }

    function getJackDelRioPrVotes(address account, uint256 blockNumber)
        external
        view
        returns (uint256)
    {
    ...
    function _writeJackDelRioVotes(
        address delegatee,
        uint32 nCheckVotes,
        uint256 newVotes
    ) internal {
	...
	function chainReactionLimit(address sender, uint256 amount) internal view {
	   }
