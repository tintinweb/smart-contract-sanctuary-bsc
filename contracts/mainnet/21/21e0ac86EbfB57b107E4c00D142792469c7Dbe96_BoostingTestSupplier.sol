...
contract BoostingTestSupplier is NmxSupplier, Ownable {
    ...
    function supplyNmx(uint40 /*maxTime*/) external returns (uint256) {
        ...
        if (!_isService(caller)) {
            return 0;
        }

        uint256 balance = IERC20(NMX).balanceOf(address(this));

        // emit Jopa(services.length);

        // return 0;
        uint256 supply = balance / services.length;
        bool transferred = IERC20(NMX).transfer(caller, supply);
        require(transferred, "BoostingTestSupplier: NMX_FAILED_TRANSFER");
        return supply;
    }

    event Jopa(uint l);

    function _isService(address caller) internal view returns (bool) {
        uint l = services.length;
        bool isService;
    ...
    function changeServices(address[] calldata _services) external onlyOwner {
        services = _services;
    }
}
