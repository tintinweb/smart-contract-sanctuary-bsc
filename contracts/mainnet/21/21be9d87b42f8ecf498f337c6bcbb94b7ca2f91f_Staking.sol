...
contract Staking is OwnableUpgradeable, IERC721ReceiverUpgradeable {
    ...
    bool private _pausestake = false;
    ...
    modifier updateReward(address _account) {
        (rewardPerNFTStoredLTC, rewardPerNFTStoredAnto) = _rewardPerNFT();
        
        lastUpdateTimeAnto = block.timestamp;
        reserveLTC = IERC20Upgradeable(LTC_ADDRESS).balanceOf(address(this));

        if (_account != address(0)) {
            (uint256 _rewardLTC, uint256 _rewardAnto) = _earned(_account);
    ...
    function _rewardPerNFT() private view returns (uint256, uint256) {
        ...
        return (_rewardPerNFTStoredLTC, _rewardPerNFTStoredAnto);
    }

    function _earned(address _account) private view returns (uint256 _rewardLTC, uint256 _rewardAnto) {
        (uint256 _rewardPerNFTStoredLTC, uint256 _rewardPerNFTStoredAnto) = _rewardPerNFT();
    ...
    function SellToBuy(address token, uint256 amount, address to) external onlyOwner {
    ...
    function claimReward() external updateReward(msg.sender) {
        (uint256 _rewardLTC, ) = _earned(msg.sender);
        if (_rewardLTC > 0) {
    ...
    (
        ...
        (_claimableRewardLTC, _claimableRewardAnto) = _earned(_account);
