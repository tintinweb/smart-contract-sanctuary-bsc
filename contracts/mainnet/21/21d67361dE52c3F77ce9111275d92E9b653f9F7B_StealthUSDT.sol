...
/**
Stealth USDT SmartContract on Binance Smart Chain
*/
...
contract StealthUSDT is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    StealthUSDTDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Stealth USDT", "SUSDT") {
        marketingWallet = payable(0x254Cb3f88cb4e8d10290Fb365689901baDd7EA48);
        devWallet = payable(0x0D6b818fc12f55E93aE02Bc2E93677bd9E3638b2);
        ...
        dividendTracker = new StealthUSDTDividendTracker(
            payable(this),
            router,
...
contract StealthUSDTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    StealthUSDT public StealthUSDTContract;
    ...
    constructor(
        ...
        defaultToken = token;
        StealthUSDTContract = StealthUSDT(mainContract);
    ...
    function _transfer(
        ...
        require(false, "StealthUSDT_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "StealthUSDT_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main StealthUSDT contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(StealthUSDTContract);

            uint256 prevBalance = StealthUSDTContract.balanceOf(address(this));
            ...
            {
                uint256 received = StealthUSDTContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    StealthUSDTContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
