...
contract ATM is ERC20, Ownable {
    ...
    uint256 public liquidityDividendFeeBuy = 250;
    uint256 public liquidityDividendFeeSell = 250;
    uint256 public liquidityDividendFeeTotal;

    uint256 public daoDividendFee = 50;
    ...
    mapping(address => uint) public accountLpTokenConvertTokenBs;
    ...
    function setDaoNft(IERC721Enumerable _daoNft) public onlyOwner {
        daoNft = _daoNft;
    }
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (daoDividendFee > 0 && (from == address(uniswapV2Pair) || to == address(uniswapV2Pair))) {
                uint256 daoDividendFees = amount.mul(daoDividendFee).div(10000);
    ...
    function _updateRewardPerLpStoredAndLpBalance(address from, address to) private {
        if(!isContract(from)){
            ...
            if(accountLpBalance < accountLpBalances[from] && lpTokenConvertTokenB(accountLpBalance) < dividendFeeNeedLpUsdt){
                accountLpBalances[from] = 0;
            } else {
                if((accountLpBalances[from] == 0 && lpTokenConvertTokenB(accountLpBalance) >= dividendFeeNeedLpUsdt)
                    || accountLpBalance >= accountLpBalances[from]
                    || lpTokenConvertTokenB(accountLpBalance) >= dividendFeeNeedLpUsdt) {
        ...
        if(!isContract(to)){
            ...
            if(accountLpBalance < accountLpBalances[to] && lpTokenConvertTokenB(accountLpBalance) < dividendFeeNeedLpUsdt){
                accountLpBalances[to] = 0;
            } else {
                if((accountLpBalances[to] == 0 && lpTokenConvertTokenB(accountLpBalance) >= dividendFeeNeedLpUsdt)
                || accountLpBalance >= accountLpBalances[to]
                    || lpTokenConvertTokenB(accountLpBalance) >= dividendFeeNeedLpUsdt) {
        ...
        if(to == address(uniswapV2Pair)) {
            uint deadLpBalance = getLpNoFeeBalance();
            if (IUniswapV2Pair(uniswapV2Pair).totalSupply().sub(deadLpBalance) == 0){
                return;
            }
            rewardPerLpStored = liquidityDividendFeeTotal.mul(1e18).div(IUniswapV2Pair(uniswapV2Pair).totalSupply().sub(deadLpBalance));
        }
    }
    ...
    function getLpReward(address account) public view returns(uint) {
        ...
        if(account == owner() || account == uniswapV2Pair) {
            return 0;
        }
        if(userRewardPerLpStored[account] == 0 || accountLpBalances[account] == 0
            || (accountLpBalance < accountLpBalances[account] && lpTokenConvertTokenB(accountLpBalance) < dividendFeeNeedLpUsdt)){
            return 0;
        }
        if(rewardPerLpStored < userRewardPerLpStored[account]) {
            return 0;
        }
    ...
    function getReward(address account) public view returns(uint) {
        return getDaoReward(account).add(getLpReward(account));
    }
