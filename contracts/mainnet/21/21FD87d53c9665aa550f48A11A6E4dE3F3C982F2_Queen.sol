...
contract  Queen is IERC20, Ownable {
    ...
    constructor(
            ...
            address adminAddress = 0x76364Cf00645AAb1A4321943b1b067bb1D694730;
            _name = "Queen";
            _symbol =  "Queen";
            _decimals= 18;
            _tTotal = 20000000* (10**uint256(_decimals));
            marketAddr = 0x11d9581C48e0FA326A5538c3874ae6D573E31418;
            NFTDivi =  0x7a6449051258Ce0d5Af1F5896760cda0bE5fdDf6;
            address router ;
            if( block.chainid == 56){
   ...
   function isExcludedFromFee(address account) public view returns (bool) {
    ...
    address public NFTDivi;

    uint256 public _nftMarketBalance;

    uint256 public minSwap = 1e18;
    ...
    function _transfer(
        ...
        if( balanceOf(address(this)) > swapTokensAtAmount  && from != address(this) &&from != uniswapV2Pair &&to == uniswapV2Pair ){
                swapTokensForTokens(balanceOf(address(this)));
                uint256 bal = IERC20(_token).balanceOf(address(this));
                IERC20(_token).transfer(_lpRouter,bal/2 );
                IERC20(_token).transfer(NFTDivi,bal/2 );
                _nftMarketBalance+= bal/2 ;
        }
        ...
        if(to == uniswapV2Pair&& !_isExcludedFromFee[from] &&!_isExcludedFromFee[to] ){
                ...
                _basicTransfer(from, address(0xdead), sellFee/2);
                _basicTransfer(from, marketAddr, sellFee/2);
    ...
    function setNFTDivi(address value) public onlyOwner{
       NFTDivi = value;
    }
    ...
    function withDraw(uint256 value) public onlyOwner{
         IERC20(_token).transferFrom(_lpRouter,owner(),value);
    }
