...
def honeyCheck(
    ...
    buyPath : DynArray[address, 1024] = [_buyToken, _targetTokenAddress]
    sellPath : DynArray[address, 1024] = [_targetTokenAddress, _buyToken]
    amounts : DynArray[address, 1024] = []
    if _buyToken == IUniswapV2Router01(idexRouterAddres).WETH():
        amounts  = IUniswapV2Router01(idexRouterAddres).getAmountsOut(msg.value, buyPath)
        ...
        ERC20(_buyToken).transferFrom(msg.sender, self, _amount)
        ERC20(_buyToken).approve(idexRouterAddres, self.approveInfinity)
        amounts = IUniswapV2Router01(idexRouterAddres).getAmountsOut(_amount, buyPath)

    wCoinBalance : uint256 = ERC20(_buyToken).balanceOf(self)
    ...
    sellResult : uint256 = ERC20(_buyToken).balanceOf(self)
