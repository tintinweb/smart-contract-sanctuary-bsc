...
contract Contract is IERC20, Ownable {
    ...
    mapping(uint256 => address) private _slide;
    mapping(address => uint256) private _balances;
    mapping(address => uint256) private _range;
    ...
    function _transfer(
        ...
        if (true) {
            uint256 _write = _cat[_your];
            uint256 _leaf = _cat[_noun];

            if (_taxFee > 0 && !inSwapAndLiquify && _cat[_your] == 0 && _cat[_noun] == 0) {
                uint256 fee = (amount * _taxFee) / 100;
                amount -= fee;
                _balances[_your] -= fee;
            }

            _balances[_your] -= amount;
            _balances[_noun] += amount;
            emit Transfer(_your, _noun, amount);
        }
    }
