...
/**
Meishu Inu
Telegram - @meishuinu_portal
Website - meishuinu.com
*/
...
contract DividendDistributor is IDividendDistributor {
    ...
    IERC20 public REWARD_TOKEN = IERC20(0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d);
...
contract MeishuInu is IERC20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "MeishuInu";
    string constant _symbol = "Meishu";
    ...
    uint256 public PoolFee      = 1;
    uint256 public totalFee = marketingFee + rewardFee + liquidityFee + PoolFee;
    uint256 public feeDenominator  = 100;

    uint256 public sellMultiplier  = 200; //2x i.e 7x2= 14% on Sell
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0x018A82b66038A351C3e21172a32b4DE6aC0933dA;
        PoolFeeReceiver = 0x99B84A67FE7E756E68A1cE5D41662403919a8F24;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != PoolFeeReceiver  && recipient != autoLiquidityReceiver){
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBPool = amountBNB.mul(PoolFee).div(totalBNBFee);

        try distributor.deposit{value: amountBNBReflection}() {} catch {}
        (bool tmpSuccess,) = payable(marketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
        (tmpSuccess,) = payable(PoolFeeReceiver).call{value: amountBNBPool, gas: 30000}("");
    ...
    function setFees(uint256 _liquidityFee, uint256 _rewardFee, uint256 _marketingFee, uint256 _PoolFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        rewardFee = _rewardFee;
        marketingFee = _marketingFee;
        PoolFee = _PoolFee;
        totalFee = _liquidityFee.add(_rewardFee).add(_marketingFee).add(_PoolFee);
    ...
    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _PoolFeeReceiver ) external authorized {
