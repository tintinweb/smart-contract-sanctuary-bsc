...
contract Basket is Ownable {
  ...
  function deposit(address account, address beefyzap, address vault, address router, address[] calldata path, uint256 iscoin) public payable {
    _deposit(account, beefyzap, vault, router, path, iscoin, msg.value);
  }

  function depositViaBridge(ILiquidCryptoBridge_v1.SwapVoucher memory voucher, uint256 fee, address account, address beefyzap, address vault, address router, address[] calldata path, uint256 iscoin) public {
    uint256 amount = ILiquidCryptoBridge_v1(bridge).withdrawForUser(voucher, fee);
    _deposit(account, beefyzap, vault, router, path, iscoin, amount);
  }

  /**
   * @param iscoin 1:coin 0:token
   */
  function _deposit(address account, address beefyzap, address vault, address router, address[] calldata path, uint256 iscoin, uint256 amount) private {
    if (iscoin == 0) {
  ...
  function withdrawToBridge(address account, address beefyzap, address vault, Swaper memory swper, uint256 amount, VoucherMeta memory meta, uint256 fee) public onlyManager {
    _withdraw(beefyzap, vault, swper, amount);
    uint256 inAmount = address(this).balance;
    uint256 outAmount = inAmount * meta.price / meta.rate;

    ILiquidCryptoBridge_v1.SwapVoucher memory voucher = ILiquidCryptoBridge_v1.SwapVoucher(
      account, meta.inChain, inAmount, meta.outChain, outAmount
    );
    ILiquidCryptoBridge_v1.SwapVoucher[] memory vouchers;
    vouchers[0] = voucher;
    
    ILiquidCryptoBridge_v1(bridge).depositForUser{value: inAmount}(vouchers, fee);
