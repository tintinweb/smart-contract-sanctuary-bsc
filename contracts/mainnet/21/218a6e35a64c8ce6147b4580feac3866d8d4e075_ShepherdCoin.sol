...
/**
 * Develop by HunterGemX
 * @ This contract was developed by https://t.me/huntergemX
 *If you want to have a similar smart-contract, please contact me on Telegram!
...
contract ShepherdCoin is IBEP20, Ownable {
    /*=== SafeMath ===*/
    using SafeMath for uint256;
    using Address for address;
    /*=== Adress ===*/
    address private burnadress = address(0); // Adress for Buyback
    address private internalOperationAddress; // internal address
    IUniswapV2Router02 public  uniswapV2Router; // Adress Router
    address public  uniswapV2Pair; // Pair TOKEN/BNB
    /*=== Mapping ===*/
    mapping (address => uint256) private _balance; // Holders Balance
    mapping (address => bool) public _excludeFromFee; // Dont pay fee
    mapping (address => bool) private automatedMarketMakerPairs; // Exchange
    mapping (address => mapping(address => uint256)) private _allowances; // subsidy
    mapping (address => bool) public isTimelockExempt; // No Waiting Time
       /*=== Unitary ===*/
    uint8 private _decimals = 18;
    uint256 private _decimalFactor = 10**_decimals; // Decimal fact
    uint256 private _tSupply = 100000000 * _decimalFactor; // Supply
    uint256 public buyFee = 10; // Buy tax
    uint256 private previousBuyFee; // store purchase Buy fee
    uint256 public sellFee = 10; // sell tax
    uint256 private previousSellFee; // store purchase sell fee
    uint256 private limitBurn = 10000000 * _decimalFactor;
    uint256 public liquidityPercent = 20; // Tax of Liquidy 20%
    uint256 public maxWalletBalance = 5000000 * _decimalFactor; // 5% of Supply
    uint256 public maxBuyAmount = 2000000 * _decimalFactor; // 2% of Supply
    uint256 public maxSellAmount = 2000000 * _decimalFactor; // 2% of Supply
    ...
    constructor() {
        ...
        address pairCreated = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(address(this), _uniswapV2Router.WETH()); // Pair TOKEN/BNB
        uniswapV2Router = _uniswapV2Router; //  Root
        uniswapV2Pair = pairCreated; // Pair
        _balance[owner()] = _tSupply; // Defines Owner 
        internalOperationAddress = owner(); // Defines operation Adress
        _excludeFromFee[owner()] = true; // Defines Owner as True to not pay fees
        _excludeFromFee[address(this)] = true; // Set Contract to True to not pay Fees
        _excludeFromFee[internalOperationAddress] = true; // Set internalOperationAddress to True to not pay Fees
        isTimelockExempt[owner()] = true; // Owner has no waiting time for buying and selling
        isTimelockExempt[address(this)] = true; // Contract Does not have waiting time for purchase and sale
    ...
    /*=== Public View ===*/
    function name() public view override returns(string memory) { return _name; } // Name of token
    function symbol() public view override returns(string memory) { return _symbol; } // Symbol token
    function decimals() public view override returns(uint8) { return _decimals; } // Decimal
    function totalSupply() public view override returns(uint256) { return _tSupply; } // Supply
    ...
    function _setRouterAddress(address router) private {
        ...
        address pairCreated = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(address(this), _uniswapV2Router.WETH()); //  Pair SHC/BNB
        uniswapV2Router = _uniswapV2Router; // root
        uniswapV2Pair = pairCreated; //  Pair
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        ...
        automatedMarketMakerPairs[pair] = value; // Boolean
    ...
    function removeAllFee() private {
        previousBuyFee = buyFee; // previus fee
        previousSellFee = sellFee; // previus fee
        buyFee = 0; // Taxa 0
        sellFee = 0; // Taxa 0
    }
    function restoreAllFee() private {
        buyFee = previousBuyFee; // back fee
        sellFee = previousSellFee; // back fee
    }  
    function liquify(uint256 contractTokenBalance, address sender) internal {
        
        if (contractTokenBalance >= numberOfTokensToSwapToLiquidity) contractTokenBalance = numberOfTokensToSwapToLiquidity; // Defines the Quantity of Tokens for
        bool isOverRequiredTokenBalance = ( contractTokenBalance >= numberOfTokensToSwapToLiquidity ); // Boolean
        ...
        if ( isOverRequiredTokenBalance && swapAndLiquifyEnabled && !inSwapAndLiquify && (!automatedMarketMakerPairs[sender]) ) {
            ...
            _swapAndLiquify(tokenLiquidity); // Add Liquidy
            _sendBNBToContract(toSwapBNB); // swap token of BNB
        }
    }
    function _swapAndLiquify(uint256 amount) private lockTheSwap {
        uint256 half = amount.div(2); // Divide to Add Liquidity for Sale
        ...
        _addLiquidity(otherHalf, newBalance); // add liquidity
        emit SwapAndLiquify(half, newBalance, otherHalf); //  Event of Swap
    }
    ...
    function _swapTokensForEth(uint256 tokenAmount) private {
        address[] memory path = new address[](2); // Path Memory to start the sale of Tokens
        path[0] = address(this); // Adress of contract
        path[1] = uniswapV2Router.WETH(); // Pair  (BNB)
        _approve(address(this), address(uniswapV2Router), tokenAmount); // Aprove Tokens
        uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount, // Balance for Swap
            0, // Amount BNB
            path, // Path [address(this), uniswapV2Router.WETH()]
            address(this), // Adress of Fee
    ...
    function _addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        (uint256 tokenAmountSent, uint256 ethAmountSent, uint256 liquidity) = uniswapV2Router.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount, // ballance of Liquidity
            0, // Slippage 0
            0, // Slippage 0
            owner(), // Owner Add Liquidy
            block.timestamp // Timestamp
        );
        emit LiquidityAdded(tokenAmountSent, ethAmountSent, liquidity); // Event of Liquidity
    }
    ...
    function changeAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        ...
        _setAutomatedMarketMakerPair(pair, value); // Define new
    }
    ...
    function changeAddress(address _internalOperationAddress) external onlyOwner {
        internalOperationAddress = _internalOperationAddress; // Define Adress of Operation
    }
    ...
    function defineExcluded(address account, bool isTrue) external onlyOwner {
        _excludeFromFee[account] = isTrue; // 
    }
    ...
    function setActiveCoolDown(bool _buyCooldownEnabled, bool _sellCooldownEnabled, uint256 _cooldownTimerInterval) external onlyOwner {
        require(_cooldownTimerInterval <= 3600, "Buy and Sell Limit cannot be greater than one Hour");
