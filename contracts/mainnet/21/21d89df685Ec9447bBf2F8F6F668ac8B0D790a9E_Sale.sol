...
contract Sale is Ownable {
    ...
    */
    ...
    // BUSD
    IBEP20 public busd;
    IBEP20 public tokenClaim;
    address public walletMarketing;
    uint256 public valueBUSD = 17850000000000000;
    uint256 public minBuy = 50000000000000000000;
    uint256 public maxBuy = 400000000000000000000;
    uint256 public numberUser;

    mapping(address => uint256) public contractSell;
    ...
    // Constructor
    constructor (IBEP20 _busd, IBEP20 _tokenClaim, address _walletMarketing)
    {
        busd = _busd;
        tokenClaim = _tokenClaim;
        walletMarketing = _walletMarketing;
    }

    function sellContract(uint256 amount) private {
        contractSell[address(this)] += amount;
    }

    function perUser(uint256 amount, address user) private {
        userBuy[user] += amount;
        uint256 limit = userBuy[user];
        require(limit <= maxBuy, "Limite Atingido");
    }

    function buyNft(uint256 amount) public {
        require(amount >= minBuy &&  amount <= maxBuy, "Value Error");
        ...
        IBEP20(tokenClaim).transfer(_msgSender(), calculateToken(amount));
        // Transfer os BUSD
        withdrawBUSD();
    }
    function calculateToken(uint256 bAmount) public view returns(uint256) {
        uint256 buy = bAmount.div(valueBUSD);
        return buy *10**18;
    }
    ...
    function addManual(uint256 amount, address user) external onlyOwner {
        ...
        // User Valor
        perUser(amount, user);
        IBEP20(tokenClaim).transfer(user, calculateToken(amount));
    }

    function totalRaised() public view returns(uint256 totalBUSD) {
        uint256 sell = contractSell[address(this)];
        return sell;
    }
