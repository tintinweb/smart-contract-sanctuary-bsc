...
contract BeanLabs is Context, Ownable {
    ...
    function getpotionsSinceLastHire(address adr) public view returns(uint256) {
        uint256 secondsPassed = min(potions_TO_Hire_1chemist, SafeMath.sub(block.timestamp, lastHire[adr]));
         ...
         if (lastreorganization > lastHire[adr]) {
            // Calculate reduction in potions produced due to last call to reorganize function
            uint256 reduction = SafeMath.mul(secondsPassed, Factory[adr]) * lastrebasepercent / 100;
            // Subtract reduction from total potions produced since last hire
    ...
    function getlastrebasepercent() public view returns (uint256) {
    ...
    function getlastburnpercent() public view returns (uint256) {
        return lastburnpercent;
    }
    ...
    function increaseLabLevel(address user) private {
        // Add 1 to the user's lab level
        labLevel[user] = labLevel[user] + 1;
    }


    function reorganize(uint rebasepercent) public onlyOwner {
        require(rebasepercent <= 50);
