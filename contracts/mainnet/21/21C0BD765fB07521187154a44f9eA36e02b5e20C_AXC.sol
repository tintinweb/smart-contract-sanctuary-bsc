...
contract AXC is ERC20, Ownable {
    uint256 public buyFee = 10;
    uint256 public sellFee = 10;
    
    uint256 public liquidityShare = 10;
    uint256 public phoenixShare = 10;
    uint256 public utilityShare = 40;
    uint256 public marketingShare = 40;

    address public marketingWallet = 0x7C68D67d2e592f0Ea84dCB131E6525C4827Ce275;
    address public phoenixWallet = 0xA14d9AEF57FB02FC9922B71485908d6Bd5365706;
    address public utilityWallet = 0x1af7E4407658752eC44CA74CCBC7cEF98D7B3D53;
    address public liquidityWallet = 0x84E6D97f5d10bbDBa599D18Afd2CE00548ED6A3D;
    ...
    mapping (address => bool) private _isExcludedFromSellFees;
    ...
    event UtilityWalletChanged(address teamWallet);
    ...
    constructor () ERC20("Axon Chains", "AXC") 
    {   
        ...
        _isExcludedFromFees[utilityWallet] = true;
        _isExcludedFromFees[liquidityWallet] = true;
        
        _mint(owner(), 250 * 10**6 * (10 ** 18));
        swapTokensAtAmount = totalSupply() / 50000;
    }

    receive() external payable {}

    function setEnableTrading(bool state) external onlyOwner{
        require(enableTrading != state, "Trading is already on that state!");
        enableTrading = state;
    }
    ...
    function excludeFromBuyFees(address account, bool excluded) external onlyOwner {
        require(_isExcludedFromBuyFees[account] != excluded, "Account is already the value of 'excluded' from Buy Fees");
        _isExcludedFromBuyFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }

    function excludeFromSellFees(address account, bool excluded) external onlyOwner {
        require(_isExcludedFromSellFees[account] != excluded, "Account is already the value of 'excluded' from Sell Fees");
        _isExcludedFromSellFees[account] = excluded;
    ...
    function updateFeeShares(uint256 _liquidityFeeShare, uint256 _phoenixFeeShare, uint256 _marketingFeeShare, uint256 _utilityFeeShare) external onlyOwner {
        require(_liquidityFeeShare + _phoenixFeeShare + _marketingFeeShare + _utilityFeeShare== 100, "Fee shares must add up to 100");
        liquidityShare = _liquidityFeeShare;
        phoenixShare = _phoenixFeeShare;
        marketingShare = _marketingFeeShare;
        utilityShare = _utilityFeeShare;
        
        emit FeeSharesUpdated(liquidityShare, phoenixShare, marketingShare, utilityShare);
    }
    ...
    function changeUtilityWallet(address _utilityWallet) external onlyOwner {
        require(_utilityWallet != utilityWallet, "Utility wallet is already that address");
        require(!isContract(_utilityWallet), "Utility wallet cannot be a contract");
        utilityWallet = _utilityWallet;
        emit UtilityWalletChanged(utilityWallet);
    }
    ...
    function _transfer(
        ...
        if(_isExcludedFromSellFees[from] || _isExcludedFromBuyFees[to]){
            takeFee = false;
        }
        ...
        if( canSwap &&
            ...
            swapping = true;
            if(liquidityShare > 0 || marketingShare > 0 || phoenixShare > 0 || utilityShare > 0) {
                swapAndSendWallets(contractTokenBalance);
            } 
           swapping = false;
        }
    ...
    function swapAndSendWallets(uint256 tokenAmount) private {
        ...
        uint256 newBalance = address(this).balance - initialBalance;
            ...
            if(liquidityShare > 0) {
                uint256 liquidityBnb = newBalance * liquidityShare / 100;
                sendBNB(payable(liquidityWallet), liquidityBnb);
            } 
            ...
            if(phoenixShare > 0) {
                uint256 phoenixBnb = newBalance * phoenixShare / 100;
                sendBNB(payable(phoenixWallet), phoenixBnb);
               
            } 

            if(utilityShare > 0) {
                uint256 utilityBnb = newBalance * utilityShare / 100;
                sendBNB(payable(utilityWallet), utilityBnb);
                
            } 
        emit SwapAndSendWallets(tokenAmount, newBalance);
    }
