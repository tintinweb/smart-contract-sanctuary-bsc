...
contract BT is Ownable {
    ...
    event NewPremiumSubscription(uint256 newPrice, address newPaymentToken);

    constructor(
        address _feeAddress,
        address _quoteTokenAddress,
        uint256 _premiumSubscriptionPrice
    ) {
        ...
        subscriptions.push(
            Subscription(
                true, // active
                _premiumSubscriptionPrice, // price
                _quoteTokenAddress, // paymentToken
                address(0), // Lps
                0 // without limit
            )
        );
    ...
    /**
     * @notice For new subscribers only as only a single payment is required upon purchase
     * @param _paymentToken The payment token
     * @param _premiumSubscriptionPrice: new prices
     */
    function updatePremiumSubscription(
        uint256 _premiumSubscriptionPrice,
        address _paymentToken
    ) external onlyOwner {
        Subscription storage subscription = subscriptions[PREMIUM_ID];
        subscription.priceInQuoteToken = _premiumSubscriptionPrice;
        subscription.paymentToken = _paymentToken;
        emit NewPremiumSubscription(_premiumSubscriptionPrice, _paymentToken);
    }
    ...
    function checkVoteParam(
        ...
        require(
            _validatorsLength >= _quorum,
            "_quorum should be equal to number of validators"
        );
    }
        ...
        validSubscriptionID(_subscriptionId)
    {
        ...
        uint256 priceInPaymentToken = getPriceInPaymentToken(
            paymentPlan.exchangePair,
            (
                (_firstPayment && _subscriptionId != PREMIUM_ID)
    ...
    function billPayment() external {
        ...
        uint256 amountInPaymentToken = (
            userTestament.subscriptionID == PREMIUM_ID
                ? 0
                : getPriceInPaymentToken(
                    userpaymentPlan.exchangePair,
                    (userpaymentPlan.priceInQuoteToken * DISCOUNT_BP) /
                        BASE_POINT
                )
        );
                ...
                for (uint256 i = 0; i < tokens.erc20Tokens.length; i++) {
                    ...
                    if (alreadyDone[address(tokens.erc20Tokens[i])] == false) {
                        ...
                        if (perShare == 0) {
                            ...
                            if (userTestament.subscriptionID == FREEMIUM_ID) {
                                uint256 feeAmount = (testamentOwnerBalance *
                                    FREEMIUM_FEE_BP) / BASE_POINT;
                                if (feeAmount > 0) {
