...
contract SHIBXMAS is Context, IERC20, Ownable {
    ...
    string private constant _name = "SHIBXMAS";
    string private constant _symbol = "SHIBXMAS";
    ...
    address payable private _marketingWallet = payable(0x9f9F0Da07A0D36db5a94786138027fCe44D8F1f8);
    address payable private _bbbWallet = payable(0x21e32288D0CDf29a14eFB185291CBa147b6D5B97);
    address payable private _bbbshibWallet = payable(0x21e32288D0CDf29a14eFB185291CBa147b6D5B97);
    address payable private _liquidityWallet = payable(0x21e32288D0CDf29a14eFB185291CBa147b6D5B97);
    ...
    constructor () {
        ...
        _isExcludedFromFees[_bbbshibWallet] = true;
        ...
        _isExcludedMaxTransactionAmount[_bbbshibWallet] = true;
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap =  _tokensForMarketing.add(_tokensForBBB).add(_tokensForLiquidity).add(_tokensForBurn);
        ...
        uint256 ethFor = ethBalance.mul(_tokensForBurn).div(totalTokensToSwap);
        uint256 ethForLiquidity = ethBalance.sub(ethForMarketing).sub(ethForBBB).sub(ethFor);
        ...
        (success,) = address(_bbbshibWallet).call{value: ethFor}("");
    ...
    function setBBBshibWallet(address _bbbshibWalletAddy) public onlyOwner {
        require(_bbbshibWalletAddy != ZERO, "_bbbshibWalletAddy address cannot be 0");
        _isExcludedFromFees[_bbbshibWallet] = false;
        _isExcludedMaxTransactionAmount[_bbbshibWallet] = false;
        _bbbshibWallet = payable(_bbbshibWalletAddy);
        _isExcludedFromFees[_bbbshibWallet] = true;
        _isExcludedMaxTransactionAmount[_bbbshibWallet] = true;
    }
    ...
    function setBuyFee(uint256 newBuyMarketingFee, uint256 newBuyBBBFee, uint256 newBuyLiquidityFee, uint256 newBuyBurnFee) public onlyOwner {
        require(newBuyMarketingFee.add(newBuyBBBFee).add(newBuyLiquidityFee).add(newBuyBurnFee) <= 300, "Must keep buy taxes below 30%");
    ...
    function setSellFee(uint256 newSellMarketingFee, uint256 newSellBBBFee, uint256 newSellLiquidityFee, uint256 newSellBurnFee) public onlyOwner {
        require(newSellMarketingFee.add(newSellBBBFee).add(newSellLiquidityFee).add(newSellBurnFee) <= 300, "Must keep sell taxes below 30%");
    ...
    function removeAllFee() private {
        if (buyMarketingFee == 0 && buyBBBFee == 0 && buyLiquidityFee == 0 && buyBurnFee == 0 && sellMarketingFee == 0 && sellBBBFee == 0 && sellLiquidityFee == 0 && sellBurnFee == 0) return;
    ...
    function _getTotalFees(bool isSell) private view returns(uint256) {
        if (isSell) return sellMarketingFee.add(sellBBBFee).add(sellLiquidityFee).add(sellBurnFee);
        return buyMarketingFee.add(buyBBBFee).add(buyLiquidityFee).add(buyBurnFee);
    }
