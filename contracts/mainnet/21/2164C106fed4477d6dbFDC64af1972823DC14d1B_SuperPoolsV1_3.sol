...
abstract contract Admin is Ownable {
    ...
    uint256[] public intervalDays = [1, 11, 21];
    uint256[] public dailyRateDays = [60, 50, 40];
    ...
    constructor(address _owner) Ownable(_owner) {
        stakeDuration = DAYS.mul(60);
        refPercentage = 1000000000000000000;
        optionableBenefit = 2;
    }
    ...
    function setDailyDistribution(
        address[3] memory stakedToken,
        address[3] memory rewardToken,
        uint256[3] memory dailyDistribution
    ) public onlyOwner {
        require(
    ...
    function updateSequence(
        address stakedToken,
        address[3] memory rewardTokenSequence
    ) public onlyOwner {
...
// File contracts/SuperPoolsV1_3.sol
...
contract SuperPoolsV1_3 is Admin {
    ...
    function setTokenInfos(
        ...
        uint256[3] memory _rewardsCaps
    ) public onlyOwner {
        uint256[3] memory tokensRPerPool = [
            _rewardsCaps[0].div(3),
            _rewardsCaps[1].div(3),
            _rewardsCaps[2].div(3)
        ];

        _addsTokens(tokenViewA, tokenViewB, tokenViewC);

        _setDistributionsPerDay(
            [tokenViewA.tokenAds, tokenViewB.tokenAds, tokenViewC.tokenAds],
            tokensRPerPool
        );
    ...
    function _setDistributionsPerDay(
        ...
        setDailyDistribution(
            [_tokenAddresses[0], _tokenAddresses[0], _tokenAddresses[0]],
            [_tokenAddresses[0], _tokenAddresses[1], _tokenAddresses[2]],
            [
        ...
        setDailyDistribution(
            [_tokenAddresses[1], _tokenAddresses[1], _tokenAddresses[1]],
            [_tokenAddresses[1], _tokenAddresses[2], _tokenAddresses[0]],
            [
        ...
        setDailyDistribution(
            [_tokenAddresses[2], _tokenAddresses[2], _tokenAddresses[2]],
            [_tokenAddresses[2], _tokenAddresses[0], _tokenAddresses[1]],
            [
    ...
    function _setSequencesTokens(address[3] memory _tokenAddresses) internal {
        updateSequence(
            _tokenAddresses[0],
            [_tokenAddresses[0], _tokenAddresses[1], _tokenAddresses[2]]
        );
    ...
    {
        // Local variables
        uint256[] memory rewardsToken = new uint256[](3);
    ...
    function _UserRewardCalculation(
        ...
        uint256[] memory tokensEarned = new uint256[](3);
