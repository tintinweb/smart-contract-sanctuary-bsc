...
contract LMAOAirdrop is ERC721, Ownable, ReentrancyGuard {
  ...
  uint256 publicSaleAmountLimit = 4;
  ...
  modifier isValidMerkleProof(bytes32[] calldata _proof){
    require(MerkleProof.verify(_proof, root, keccak256(abi.encodePacked(msg.sender))), "Not allowed origin");
    _;
  }
  ...
  function airdropMint(address account, bytes32[] calldata _proof) 
    external 
    payable 
    ...
    onlyAccounts{
      ...
      require(presaleM);
      require(!paused);
      require(presaleClaimed[msg.sender] + 1 <= presaleAmountLimit);
      require(_tokenIds.current() + 1 <= maxSupply);
  ...
  function publicSaleMint(uint256 _amount) 
    ...
    onlyAccounts
    {
        ...
        require(publicSaleClaimed[msg.sender] + _amount <= publicSaleAmountLimit);
        ...
        publicSaleClaimed[msg.sender] += _amount;
