...
contract NitroPool is Ownable {
    ...
    function BuySlot() public {
        ...
        uint doubledBalance = (XTO_balanceAfter - XTO_balanceBefore) * 2;

        require(XTO.balanceOf(address(this)) >= doubledBalance, "Not Enough XTO after Swap");
        s.amount = doubledBalance;
        XTO.safeTransferFrom(address(this), s.user, doubledBalance);

        totalDeposited += slotPrice;
        totalPaid += doubledBalance;
        uPaid.paid += doubledBalance;

        emit SlotPaid(_slotToPay, s.user, doubledBalance);
