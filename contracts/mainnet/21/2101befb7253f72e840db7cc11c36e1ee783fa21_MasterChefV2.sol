...
contract ReentrancyGuard {
    ...
    /**
     * @dev Prevents a contract from calling itCST, directly or indirectly.
...
/// @notice The (older) MasterChef contract gives out a constant number of CST tokens per block.
/// It is the only address with minting rights for CST.
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    ///
    ///   pending reward = (user share * pool.accCSTPerShare) - user.rewardDebt
    ///
    ...
    ///     CST rewards each pool gets.
    /// `accCSTPerShare` Accumulated CSTs per share, times 1e12.
    ...
    ///     the CST rewards to all the SMBSwap products.
    /// `totalBoostedShare` The total amount of user shares in each pool. After considering the share boosts.
    struct PoolInfo {
        uint256 accCSTPerShare;
    ...
    /// @notice Address of CST contract.
    IBEP20 public immutable CST;

    /// @notice The only address can withdraw all the burn CST.
    ...
    ///  @notice 10 CSTs per block in MCV1
    uint256 public constant MASTERCHEF_CST_PER_BLOCK = 10 * 1e18;
    uint256 public constant ACC_CST_PRECISION = 1e18;

    /// @notice Basic boost factor, none boosted user's boost factor
    uint256 public constant BOOST_PRECISION = 100 * 1e10;
    /// @notice Hard limit for maxmium boost factor, it must greater than BOOST_PRECISION
    uint256 public constant MAX_BOOST_PRECISION = 200 * 1e10;
    /// @notice total CST rate = toBurn + toRegular + toSpecial
    uint256 public constant CST_RATE_TOTAL_PRECISION = 1e12;
    /// @notice The last block number of CST burn action being executed.
    /// @notice CST distribute % for burn
    uint256 public CSTRateToBurn = 643750000000;
    /// @notice CST distribute % for regular farm pool
    uint256 public CSTRateToRegularFarm = 62847222222;
    /// @notice CST distribute % for special pools
    uint256 public CSTRateToSpecialFarm = 293402777778;
    ...
    event UpdatePool(uint256 indexed pid, uint256 lastRewardBlock, uint256 lpSupply, uint256 accCSTPerShare);
    event Deposit(address indexed user, uint256 indexed pid, uint256 amount);
    event Withdraw(address indexed user, uint256 indexed pid, uint256 amount);
    event EmergencyWithdraw(address indexed user, uint256 indexed pid, uint256 amount);

    event UpdateCSTRate(uint256 burnRate, uint256 regularFarmRate, uint256 specialFarmRate);
    ...
    /// @param _MASTER_CHEF The SMBSwap MCV1 contract address.
    ...
    /// @notice Deposits a dummy token to `MASTER_CHEF` MCV1. This is required because MCV1 holds the minting permission of CST.
    ...
    function init(IBEP20 dummyToken) external onlyOwner {
        ...
        // MCV2 start to earn CST reward from current block in MCV1 pool
    ...
    /// only for CST distributions within SMBSwap products.
    function add(
        uint256 _allocPoint,
        IBEP20 _lpToken,
        bool _isRegular,
        ...
        // stake CST token will cause staked token and reward token mixed up,
        // may cause staked tokens withdraw as reward token,never do it.
        require(_lpToken != CST, "CST token can't be added to farm pools");
    ...
    /// @notice View function for checking pending CST rewards.
    /// @param _pid The id of the pool. See `poolInfo`.
    ...
    function pendingCST(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 CSTReward = multiplier.mul(CSTPerBlock(pool.isRegular)).mul(pool.allocPoint).div(
                (pool.isRegular ? totalRegularAllocPoint : totalSpecialAllocPoint)
            );
            accCSTPerShare = accCSTPerShare.add(CSTReward.mul(ACC_CST_PRECISION).div(lpSupply));
        }

        uint256 boostedAmount = user.amount.mul(getBoostMultiplier(_user, _pid)).div(BOOST_PRECISION);
        return boostedAmount.mul(accCSTPerShare).div(ACC_CST_PRECISION).sub(user.rewardDebt);
    }

    /// @notice Update CST reward for all the active pools. Be careful of gas spending!
    ...
    /// @notice Calculates and returns the `amount` of CST per block.
    /// @param _isRegular If the pool belongs to regular or special.
    function CSTPerBlock(bool _isRegular) public view returns (uint256 amount) {
        if (_isRegular) {
            amount = MASTERCHEF_CST_PER_BLOCK.mul(CSTRateToRegularFarm).div(CST_RATE_TOTAL_PRECISION);
        } else {
            amount = MASTERCHEF_CST_PER_BLOCK.mul(CSTRateToSpecialFarm).div(CST_RATE_TOTAL_PRECISION);
        }
    }

    /// @notice Calculates and returns the `amount` of CST per block to burn.
    function CSTPerBlockToBurn() public view returns (uint256 amount) {
        amount = MASTERCHEF_CST_PER_BLOCK.mul(CSTRateToBurn).div(CST_RATE_TOTAL_PRECISION);
    }
    ...
    function updatePool(uint256 _pid) public returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (lpSupply > 0 && totalAllocPoint > 0) {
                ...
                pool.accCSTPerShare = pool.accCSTPerShare.add((CSTReward.mul(ACC_CST_PRECISION).div(lpSupply)));
            }
            pool.lastRewardBlock = block.number;
            poolInfo[_pid] = pool;
            emit UpdatePool(_pid, pool.lastRewardBlock, lpSupply, pool.accCSTPerShare);
        }
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            settlePendingCST(msg.sender, _pid, multiplier);
        }
        ...
        user.rewardDebt = user.amount.mul(multiplier).div(BOOST_PRECISION).mul(pool.accCSTPerShare).div(
            ACC_CST_PRECISION
        );
        poolInfo[_pid] = pool;
    ...
    /// @notice Harvests CST from `MASTER_CHEF` MCV1 and pool `MASTER_PID` to MCV2.
    ...
    /// @notice Send CST pending for burn to `burnAdmin`.
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function burnCST(bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }

        uint256 multiplier = block.number.sub(lastBurnedBlock);
        uint256 pendingCSTToBurn = multiplier.mul(CSTPerBlockToBurn());

        // SafeTransfer CST
        _safeTransfer(burnAdmin, pendingCSTToBurn);
        lastBurnedBlock = block.number;
    }

    /// @notice Update the % of CST distributions for burn, regular pools and special pools.
    /// @param _burnRate The % of CST to burn each block.
    /// @param _regularFarmRate The % of CST to regular pools each block.
    /// @param _specialFarmRate The % of CST to special pools each block.
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function updateCSTRate(
        uint256 _burnRate,
        ...
        require(
            ...
            "MasterChefV2: CST rate must be greater than 0"
        );
        require(
            _burnRate.add(_regularFarmRate).add(_specialFarmRate) == CST_RATE_TOTAL_PRECISION,
            "MasterChefV2: Total rate must be 1e12"
        );
        if (_withUpdate) {
            massUpdatePools();
        }
        // burn CST base on old burn CST rate
        burnCST(false);

        CSTRateToBurn = _burnRate;
        CSTRateToRegularFarm = _regularFarmRate;
        CSTRateToSpecialFarm = _specialFarmRate;

        emit UpdateCSTRate(_burnRate, _regularFarmRate, _specialFarmRate);
    }
    ...
    function updateBoostMultiplier(
        ...
        settlePendingCST(_user, _pid, prevMultiplier);

        user.rewardDebt = user.amount.mul(_newMultiplier).div(BOOST_PRECISION).mul(pool.accCSTPerShare).div(
            ACC_CST_PRECISION
        );
    ...
    /// @notice Settles, distribute the pending CST rewards for given user.
    /// @param _user The user address for settling rewards.
    /// @param _pid The pool id.
    /// @param _boostMultiplier The user boost multiplier in specific pool id.
    function settlePendingCST(
        address _user,
        uint256 _pid,
        ...
        uint256 accCST = boostedAmount.mul(poolInfo[_pid].accCSTPerShare).div(ACC_CST_PRECISION);
        uint256 pending = accCST.sub(user.rewardDebt);
        // SafeTransfer CST
        _safeTransfer(_user, pending);
    }

    /// @notice Safe Transfer CST.
    /// @param _to The CST receiver address.
    /// @param _amount transfer CST amounts.
    function _safeTransfer(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            // Check whether MCV2 has enough CST. If not, harvest from MCV1.
            if (CST.balanceOf(address(this)) < _amount) {
