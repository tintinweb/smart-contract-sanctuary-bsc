...
contract LobbyFomo is Ownable, ReentrancyGuard {
    ...
    function hmineInLobby() external view returns (uint256 _hmine) {
        uint256 _bal = IERC20(hmineToken).balanceOf(address(this));
        _hmine = _bal - hmineJackpot - hmineReward;
    }
    ...
    function withdrawHmine(uint256 _amount) external onlyOwner nonReentrant {
        uint256 _bal = IERC20(hmineToken).balanceOf(address(this));
        require(_amount <= _bal - hmineJackpot - hmineReward, "Invalid amount");
