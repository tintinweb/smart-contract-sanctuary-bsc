...
contract OTCUSDT is ERC20Detailed, Ownable {

    using SafeMath for uint256;
    using SafeMathInt for int256;

    event LogRebase(uint256 indexed epoch, uint256 totalSupply);
    event LogReward(address indexed sender, uint256 amount);

    string public _name = "OTUSDT  Token";
    string public _symbol = "OTUSDT";
    ...
    address REWARD = 0x000000000000000000000000000000000000eFef;
    ...
    address public OTCUSDTDaoAddress;
    uint256 public startTradingTime;
    uint256 public autoLiquidityInterval;
    constructor(address _swapRouter,address _OTCUSDTDaoAddress,address _autoLiquidityReceiver,address _firePit,uint256 _initSupply,uint256 _startTradingTime) ERC20Detailed(_name,_symbol, uint8(DECIMALS)) Ownable() {
        require(_swapRouter!=address(0),"invalid swap router address");
        require(_OTCUSDTDaoAddress!=address(0),"invalid OTCUSDT dao address");
        OTCUSDTDaoAddress = _OTCUSDTDaoAddress;
        usdtAddress = 0x55d398326f99059fF775485246999027B3197955;//IOTCUSDTDao(_OTCUSDTDaoAddress).payToken();
        ...
        _isFeeExempt[OTCUSDTDaoAddress] = true;
    ...
    returns (bool)
    {
      ...
      /*  if(to ==  REWARD)
        {
            uint256 gonAmount = value;
            _gonBalances[msg.sender] = _gonBalances[msg.sender].sub(
                     gonAmount);
            address[] memory _parents = IOTCUSDTDao(OTCUSDTDaoAddress).getRelations(to);
            _gonBalances[treasuryReceiver] = _gonBalances[treasuryReceiver].add(
                     gonAmount.mul(50).div(100));

            for (uint8 i=0;i<_parents.length;i++){
                uint256 _parentFee = gonAmount.mul(25).div(1000);
                if (i==0){
                    _parentFee = gonAmount.mul(20).div(100);
                }
                if(i==1){
                    _parentFee = gonAmount.mul(10).div(100);
                }
                if(_parents[i] == address(0))
                {
                    _parents[i] = OTCUSDTDaoAddress;
                }
            ...
            emit LogReward(msg.sender,gonAmount);
...
//        }
    ...
    function takeFee(
        ...
        if (sender != pair) {
            ...
            _gonBalances[OTCUSDTDaoAddress] = _gonBalances[OTCUSDTDaoAddress].add(
                gonAmount.div(feeDenominator).mul(daoFee)

            );
            //IOTCUSDTDao(OTCUSDTDaoAddress).setDaoReward(gonAmount.div(_gonsPerFragment).mul(daoFee).div(feeDenominator));
            _gonBalances[treasuryReceiver] = _gonBalances[treasuryReceiver].add(
                gonAmount.div(feeDenominator).mul(daoFee)
        ...
        if (sender == pair){
            ...
            address[] memory _parents = IOTCUSDTDao(OTCUSDTDaoAddress).getRelations(recipient);
            for (uint8 i=0;i<_parents.length;i++){
    ...
    function addLiquidity() internal swapping {
        ...
        if (treasuryReceiver!=address(0)&&autoLiquidityAmount>0){
            ...
            IOTCUSDT(treasuryReceiver).addLiquidity(autoLiquidityAmount);
    ...
    function swapBack() internal swapping {
        if (treasuryReceiver!=address(0)){
            IOTCUSDT(treasuryReceiver).swapBack();
        }
    ...
    function setOTCUSDTDaoAddress(address _address) external onlyOwner {
        require(_address!=address(0),"invalid address");

        OTCUSDTDaoAddress = _address;
        _isFeeExempt[OTCUSDTDaoAddress] = true;
    }

    function setOTCUSDTTreasuryAddress(address _address) external onlyOwner {
        require(_address!=address(0),"invalid address");
