...
abstract contract AbsToken is IERC20, Ownable {
    ...
    constructor (){
        ...
        holderRewardCondition = 5 * 10 ** _decimals;
    }
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if (swapAmount > 0) {
                ...
                uint256 transferLPDividendFee=isSell?_sellLPDividendFee:_buyLPDividendFee;
                if(transferLPDividendFee>0){
                    _takeTransfer(sender,address(this),swapAmount*transferLPDividendFee/swapFee);
                }
                uint256 transferFundFee=isSell?_sellFundFee:_buyFundFee;
                if(transferFundFee>0){
                    _takeTransfer(sender,fundAddress,swapAmount*transferFundFee/swapFee);
                }
                uint256 transferInviteFee=isSell?_sellInviteFee:_buyInviteFee;
                if(transferInviteFee>0){
                    address baseInviter=inviterAddress[isSell?sender:recipient];
                    if(baseInviter==address(0)){
                        _takeTransfer(sender,fundAddress,swapAmount*transferInviteFee/swapFee);
                    }else{
                        _takeTransfer(sender,baseInviter,swapAmount*transferInviteFee/swapFee);
                    }
                }
                _commonTransfer(_mktExtra,swapAmount);
