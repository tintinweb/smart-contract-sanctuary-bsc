...
contract ImperialToken is ERC20Detailed, Ownable {
    ...
    string public _name = "Imperial_Financial";
    ...
    uint256 public busdreward = 60;
 
    uint256 public sellFeel = 10;
    uint256 public sellFeet = 10;
   
    uint256 public totalFee =
        liquidityFee.add(treasuryFee).add(busdreward);     
    ...
    constructor() ERC20Detailed("Imperial_Financial", "IMPERIAL", uint8(DECIMALS)) Ownable() {
    ...
    function _transferFrom(
        ...
        if(!isDividendExempt[sender]){ try distributor.setShare(sender, gonAmount) {} catch {} }
        if(!isDividendExempt[recipient]){ try distributor.setShare(recipient, gonAmountReceived) {} catch {} }
    ...
    function takeFee(
        ...
        uint256 _busdreward  = busdreward;


        if (recipient == pair) {
            _totalFee =
             totalFee.add(sellFeet).add(sellFeel);
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_busdreward)
        );
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountbusdreward = amountETH.mul(treasuryFee).div(totalFee);
        try distributor.deposit{value: amountbusdreward}() {} catch {}
        ...
        (success, ) = payable(autoLiquidityReceiver).call{
            value: amountETH.mul(liquidityFee).div(
                totalFee
            ),
            gas: 30000
        }("");
    ...
    function setFees(
        ...
        require(totalFee < feeDenominator / 4,"The total fee amount exceed the limit");
    }
