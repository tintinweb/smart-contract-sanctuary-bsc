...
contract GymLevelPool is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    function seedUsers(address[] calldata _wallets, uint256[] calldata _levels) external onlyOwner {
    ...
    function seedOldQualifiedMembersUsers(address[] calldata _wallets, uint256[] calldata _levels) external onlyOwner {
        require(_wallets.length == _levels.length, "GymLevelPool: args length mismatch");

        for (uint256 i = 0; i < _wallets.length; ++i) {
            users[_wallets[i]].level = _levels[i];

            for (uint256 i = 22; i <= _levels[i]; ++i) {
                poolWallets[i].add(_wallets[i]);
            }
        }
    }
