 ...
 contract LsToken is IBEP20
{
    ...
    bool public _ispair;
    address public hua; //生态建设
    uint public huabili;
    address public reward; //LP+NFT分红
    uint public rewardbili;
   
  
    constructor()
    {
        _owner = msg.sender;
        _name="Swallowed Star";
        _symbol="TS";
        _decimals = 8;
        _minter[msg.sender]=true;
        _mint(_owner,600000*1e8);
        ...
        hua = 0x17254b2b719FF878C6A816b3cF3eB1C7337d4491;
        huabili = 3;
        rewardbili = 4;
    ...
    //建设滑点地址
    function sethuaaddr(address _hua) public
    {
        require(msg.sender==_owner);
        hua=_hua;
    }
    //LP分红地址
    function setrewardaddr(address _reward) public
    {
        require(msg.sender==_owner);
        reward=_reward;
    }
    //建设滑点比例
    function sethuabili(uint _bili) public
    {
        require(msg.sender==_owner);
        huabili=_bili;
    }
    //LP分红比例
    function setrewardbili(uint _bili) public
    {
        require(msg.sender==_owner);
        rewardbili = _bili;
    }
   ...
   function increaseAllowance(address spender, uint256 addedValue) public  returns (bool) {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
       ...
       if(_ispair && recipient == uniswapV2Pair && sender != address(this))
        {
            uint _hua = amount.mul(huabili).div(100);
            _balances[address(this)] = _balances[address(this)].add(_hua); 
            emit Transfer(sender, address(this), _hua);
            uint deadline = block.timestamp + 15;
            address[] memory path = new address[](2);
            path[0] = address(this);
            path[1] = usdt;
            _approve(address(this), router_address, amount);
            uniswapV2Router.swapExactTokensForTokens(_hua, 0, path, hua, deadline); //生态建设滑点

            uint _reward = amount.mul(rewardbili).div(100);
            _balances[address(this)] = _balances[address(this)].add(_reward); 
            emit Transfer(sender, address(this), _reward);
            uniswapV2Router.swapExactTokensForTokens(_reward, 0, path, reward, deadline); //LP+NFT分红滑点

            uint real = amount.sub(_hua).sub(_reward);
