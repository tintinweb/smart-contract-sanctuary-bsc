...
contract RaffleContract {
    ...
    // Private crate raffle
    function pcreateRaffle(uint _ticket_price, uint _raffle_end_balance) private {
        Raffles.push(Raffle({
            id: Raffles.length + 1,
            ticket_price: _ticket_price,
            raffle_end_balance: _raffle_end_balance,
    ...
    // create erc20 raffle
    function pcreateERC20Raffle(uint _ticket_price, uint _raffle_end_balance) private {
        Raffles.push(Raffle({
            id: Raffles.length + 1,
            ticket_price: _ticket_price,
            raffle_end_balance: _raffle_end_balance,
    ...
    function endRaffle(uint _raffle_id) public {
        ...
        if (Raffles[_raffle_id].erc20 == false) {
            ...
            // create same new raffle
            pcreateRaffle(Raffles[_raffle_id].ticket_price, Raffles[_raffle_id].raffle_end_balance);
        } else {
            ...
            token.transfer(owner, fee_amount);
            Winners.push(Winner(ticketHolders[winner_id], _raffle_id));
            // set raffle to inactive
            Raffles[_raffle_id].is_active = false;
            // set winner
            ...
            emit WinnerEvent(ticketHolders[winner_id]._Address, Raffles[_raffle_id].raffle_end_balance, _raffle_id);
            // create same new raffle
            pcreateERC20Raffle(Raffles[_raffle_id].ticket_price, Raffles[_raffle_id].raffle_end_balance);
        }
    }
    // force end raffle, ending raffle without any sending
    function forceEndRaffle(uint _raffle_id) public {
        // check if raffle exists
        if(Raffles.length <= _raffle_id) {
            revert("Raffle does not exist");
        }
