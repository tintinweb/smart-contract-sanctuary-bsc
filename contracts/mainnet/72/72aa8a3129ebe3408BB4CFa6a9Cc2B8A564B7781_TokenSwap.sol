...
contract TokenSwap {
    ...
    uint public rate;
    uint public totalPresaleAmountSold;
    ...
    function swap(address _receiver) payable public {
        require(isIcoRunning == true, "Ico not running");
        require(msg.sender == contractOwner || msg.sender == _receiver, "Not authorized");
        
        uint swapAmount = rate * msg.value;

        totalPresaleAmountSold += swapAmount;

        require(
            mainToken.allowance(contractOwner, address(this)) >= swapAmount,
            "Token allowance too low"
        );
        require(
            mainToken.allowance(contractOwner, address(this)) >= totalPresaleAmountSold,
            "Token allowance too low and greater than totalPresaleSold"
        );
        
        _safeTransferFrom(mainToken, contractOwner, _receiver, swapAmount);
    }
