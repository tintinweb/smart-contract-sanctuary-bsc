...
contract StakingNFT is Ownable, ReentrancyGuard {
    ...
    uint256 public dailyAPRPerNFT = 20000 * 1e18; // 20k Cow per NFT per day
    ...
    function setDailyAPRPerNFT(uint256 _dailyAPRPerNFT) external onlyOwner {
        dailyAPRPerNFT = _dailyAPRPerNFT;
    }
    ...
    function getCurrentInterestById(uint256 _id)
        public
        view
        returns (uint256 interest)
    {
        ...
        interest = totalDays.mul(dailyAPRPerNFT).sub(stakeDetail.claimedAmount);
        return interest;
    }
