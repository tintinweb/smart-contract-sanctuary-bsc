...
import '../interfaces/IMVPair.sol';
import '../interfaces/IMVFactory.sol';

library MVLibrary {
    using SafeMath for uint;

    uint256 constant SWAP_FEE = 3000;
    uint256 constant SWAP_FEE_TO_PAIR = 1700;
    uint256 constant SWAP_FEE_BASE = 1000000;
    uint256 constant SWAP_FEE_LP = 2000;

    // returns sorted token addresses, used to handle return values from pairs sorted in this order
    function sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
        require(tokenA != tokenB, 'MVLibrary: IDENTICAL_ADDRESSES');
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'MVLibrary: ZERO_ADDRESS');
    }
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                IMVFactory(factory).INIT_CODE_PAIR_HASH()
            ))));
    }
    ...
    function getReserves(address factory, address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB) {
        ...
        (uint reserve0, uint reserve1,) = IMVPair(pairFor(factory, tokenA, tokenB)).getReserves();
        (reserveA, reserveB) = tokenA == token0 ? (reserve0, reserve1) : (reserve1, reserve0);
    }

    // given some amount of an asset and pair reserves, returns an equivalent amount of the other asset
    function quote(uint amountA, uint reserveA, uint reserveB) internal pure returns (uint amountB) {
        require(amountA > 0, 'MVLibrary: INSUFFICIENT_AMOUNT');
        require(reserveA > 0 && reserveB > 0, 'MVLibrary: INSUFFICIENT_LIQUIDITY');
        amountB = amountA.mul(reserveB) / reserveA;
    }

    // given an input amount of an asset and pair reserves, returns the maximum output amount of the other asset
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut) internal pure returns (uint amountOut) {
        require(amountIn > 0, 'MVLibrary: INSUFFICIENT_INPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'MVLibrary: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut) internal pure returns (uint amountIn) {
        require(amountOut > 0, 'MVLibrary: INSUFFICIENT_OUTPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'MVLibrary: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountsOut(address factory, uint amountIn, address[] memory path) internal view returns (uint[] memory amounts) {
        require(path.length >= 2, 'MVLibrary: INVALID_PATH');
...
import '@openzeppelin/contracts/math/SafeMath.sol';
import "../interfaces/IMVFactory.sol";
import "../libraries/MVLibrary.sol";
import "../interfaces/IMVPair.sol";
import '../interfaces/IOracle.sol';
import "../core/SafeOwnable.sol";
import '../token/MVToken.sol';
...
contract SwapMining is SafeOwnable {
    ...
    // MV tokens created per block
    uint256 public rewardPerBlock;
    // The block number when MV mining starts.
    ...
    // factory address
    IMVFactory public factory;
    // token address
    MVToken public mvToken;
    ...
    constructor(
        ...
        require(address(_mvToken) != address(0), "illegal address");
        mvToken = _mvToken;
    ...
    struct PoolInfo {
        ...
        uint256 allocMVAmount; // How many MVs
    ...
    function setMVerBlock(uint256 _newPerBlock) public onlyOwner {
        massMintPools();
        rewardPerBlock = _newPerBlock;
    }

    // Only tokens in the whitelist can be mined MV
    ...
    function getMVReward(uint256 _lastRewardBlock) public view returns (uint256) {
    ...
    function mint(uint256 _pid) public returns (bool) {
        ...
        uint256 blockReward = getMVReward(pool.lastRewardBlock);
        if (blockReward <= 0) {
            return false;
        }
        // Calculate the rewards obtained by the pool based on the allocPoint
        uint256 mvReward = blockReward.mul(pool.allocPoint).div(totalAllocPoint);
        // Increase the number of tokens in the current pool
        pool.allocMVAmount = pool.allocMVAmount.add(mvReward);
    ...
    function swap(address account, address input, address output, uint256 amount) public onlyRouter returns (bool) {
        ...
        address pair = MVLibrary.pairFor(address(factory), input, output);
    ...
    function getQuantity(address outputToken, uint256 outputAmount, address anchorToken) public view returns (uint256) {
        ...
        } else if (IMVFactory(factory).getPair(outputToken, anchorToken) != address(0)) {
    ...
    function takerWithdraw() public {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            if (user.quantity > 0) {
                ...
                uint256 userReward = pool.allocMVAmount.mul(user.quantity).div(pool.quantity);
                pool.quantity = pool.quantity.sub(user.quantity);
                pool.allocMVAmount = pool.allocMVAmount.sub(userReward);
                user.quantity = 0;
                user.blockNumber = block.number;
        ...
        mvToken.mintFor(msg.sender, userSub);
    }
    ...
    function getUserReward(uint256 _pid, address _user) public view returns (uint256, uint256){
        ...
        if (user.quantity > 0) {
            uint256 blockReward = getMVReward(pool.lastRewardBlock);
            uint256 mvReward = blockReward.mul(pool.allocPoint).div(totalAllocPoint);
            userSub = userSub.add((pool.allocMVAmount.add(mvReward)).mul(user.quantity).div(pool.quantity));
        }
        //MV available to users, User transaction amount
    ...
    function getPoolInfo(uint256 _pid) public view returns (address, address, uint256, uint256, uint256, uint256){
        require(_pid <= poolInfo.length - 1, "SwapMining: Not find this pool");
        PoolInfo memory pool = poolInfo[_pid];
        address token0 = IMVPair(pool.pair).token0();
        address token1 = IMVPair(pool.pair).token1();
        uint256 mvAmount = pool.allocMVAmount;
        uint256 blockReward = getMVReward(pool.lastRewardBlock);
        uint256 mvReward = blockReward.mul(pool.allocPoint).div(totalAllocPoint);
        mvAmount = mvAmount.add(mvReward);
        //token0,token1,Pool remaining reward,Total /Current transaction volume of the pool
        return (token0, token1, mvAmount, pool.totalQuantity, pool.quantity, pool.allocPoint);
    }
}
...
contract MVToken is ERC20Capped, SafeOwnable {
    using SafeMath for uint256;
    ...
    constructor() ERC20Capped(MAX_SUPPLY) ERC20("MV", "MV") {
        ...
        external
    {
        ...
        require(signatory != address(0), "MV::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "MV::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "MV::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "MV::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying MVs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "MV::_writeCheckpoint: block number exceeds 32 bits");
