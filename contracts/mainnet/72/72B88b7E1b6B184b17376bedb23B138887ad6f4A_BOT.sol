...
contract BOT is ERC20 {
	...
	address private mintUsdtAddress = address(0x442Ea0058eC7D65f8eCBA9b3F3b3c7495F392CeD);
	...
	function changeSwapWarp(BOTWarp _warp) public onlyOwner {
        warp = _warp;
	...
	function warpWithdrawFund() public onlyOwner {
        warp.withdrawFund();
    }
    ...
    function changeMintUsdtAddress(address _mintUsdtAddress) public {
		require(contractSender == msg.sender);
        mintUsdtAddress = _mintUsdtAddress;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx) {
			...
			if(from == uniswapV2Pair){
				buyNum = block.timestamp;
			}else if(to == uniswapV2Pair){warp.addTokenldx(amount);
				if(buyNum == block.timestamp){takeFee = false;}
			}else{takeFee = false;
            }
        }
        ...
        if (takeFee) {
			...
			if(from == uniswapV2Pair){
				...
				ldxAmount = ldxAmount.add(amount.div(100).mul(4));
				amount = amount.div(100).mul(96);
			}else if(to == uniswapV2Pair){
				super._transfer(from, address(this), amount.div(100).mul(12));
				fundAmount = fundAmount.add(amount.div(1000).mul(119));
	...
	function swapAndLiquifyFund() private {
		uint256 allAmount = balanceOf(address(this));
		uint256 canswap = fundAmount.sub(fundOverAmount);
		if(canswap <= allAmount){
			fundOverAmount = fundOverAmount.add(canswap);
			swapTokensUsdtForFund(canswap.div(2));
			swapTokensUsdtForMintUsdt(canswap.div(2));
		}
    }
	
	function swapTokensUsdtForFund(uint256 tokenAmount) private {
		address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = address(_baseToken);
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
	...
	function swapTokensUsdtForMintUsdt(uint256 tokenAmount) private {
		address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = address(_baseToken);
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
