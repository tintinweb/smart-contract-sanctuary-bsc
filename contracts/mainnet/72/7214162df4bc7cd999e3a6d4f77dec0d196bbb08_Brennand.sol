...
contract Brennand is Context, Ownable {
    using SafeMath for uint256;

    string _name = "Brennand";
    ...
    function swapModernToFee(address _token, uint256 tokens) private  {
    ...
    function threesetMarketing(uint8 marketingFee) external onlyOwner {
        marketingFees = marketingFee;
    }
  
    function onsetsetLpReward(uint8 rewardFee) external onlyOwner {
        lpRewardFees = rewardFee;
    }

    function twosetTxPercent(uint256 maxTxPercent) external onlyOwner {
        _maxTxAmount = _totalSupply.mul(maxTxPercent).div(100);
    }
    
    function uncalCurrentSupply(uint256 _rTotal, uint256 _tTotal) private pure returns(uint256, uint256) {
	...
	function uppaddition(uint256 _amount, uint256 _taxFee) private pure returns (uint256) {
