...
contract FishPondMiner is Context, Ownable {
    using SafeMath for uint256;

    event Buy(address indexed adr, uint256 fish, uint256 amount);
    event Sell(address indexed adr, uint256 fish, uint256 amount, uint256 penalty);
    ...
    */
    ...
    uint256 private FISH_TO_HATCH_1FISHER = 576000; //for final version should be seconds in a day
    ...
    mapping (address => uint256) private hiredFishers;
    ...
    function FeedFish(address ref) public {
        require(initialized);
        ...
        uint256 newFishers = SafeMath.div(fishUsed,FISH_TO_HATCH_1FISHER);
        hiredFishers[msg.sender] = SafeMath.add(hiredFishers[msg.sender],newFishers);
    ...
    function sellFish() public {
        ...
        uint256 sellTax = calculateSellTax(fishValue, msg.sender);
        ...
        withdrawals[msg.sender] += SafeMath.sub(fishValue,sellTax);
        payable (msg.sender).transfer(SafeMath.sub(fishValue,sellTax));

        // Push the timestamp
        sellsTimestamps[msg.sender].push(block.timestamp);

        emit Sell(msg.sender, fishValue, SafeMath.sub(fishValue,sellTax), penalty);
    }
    ...
    function getMarketFish() public view returns (uint256) {
        return marketFish;
    }
    ...
    function fishRewardsIncludingTaxes(address adr) public view returns(uint256) {
        uint256 hasFish = getMyFish(adr);
        (uint256 fishValue,) = calculateFishSellIncludingTaxes(hasFish, adr);
        return fishValue;
    }
    
    function BuyFish(address ref) public payable {
        require(initialized);
        uint256 fishBought = calculateBuyFish(msg.value,SafeMath.sub(address(this).balance,msg.value));

        uint256 fishBoughtFee = calculateBuyTax(fishBought);
        fishBought = SafeMath.sub(fishBought,fishBoughtFee);
        ...
        emit Buy(msg.sender, fishBought, msg.value);
    }
    ...
    function calculateFishSellIncludingTaxes(uint256 fish, address addr) public view returns(uint256, uint256) {
        uint256 totalTrade = calculateTrade(fish,marketFish,address(this).balance);
    ...
    function calculateBuyFish(uint256 eth,uint256 contractBalance) public view returns(uint256) {
        return calculateTrade(eth,contractBalance,marketFish);
    }
    
    function calculateBuyFishSimple(uint256 eth) public view returns(uint256) {
        return calculateBuyFish(eth,address(this).balance);
    }
    ...
    function getMyFishers(address adr) public view returns(uint256) {
        return hiredFishers[adr];
    }
    
    function getMyFish(address adr) public view returns(uint256) {
        return SafeMath.add(claimedFish[adr],getFishSinceLastHire(adr));
    }
    
    function getFishSinceLastHire(address adr) public view returns(uint256) {
        uint256 secondsPassed=min(FISH_TO_HATCH_1FISHER,SafeMath.sub(block.timestamp,lastHire[adr]));
        return SafeMath.mul(secondsPassed,hiredFishers[adr]);
    }
