...
contract TriKoin {
    ...
    function withdrawToken(uint256 _value) public returns (bool success) {
        ...
        require(tokens >= 0, "Value must be greater or equal to 0");
        require(
            senderBalance > tokens,
            "The contract has reached the maxmium limit."
        );
        require(
            fromAllowance >= tokens,
            "You are not allowed to withdraw TriKoins. Kindly approve the transaction first."
        );

        balanceOf[owner] = senderBalance - tokens;
        balanceOf[msg.sender] = receiverBalance + tokens;
        approvedToken[msg.sender] = fromAllowance - tokens;
