...
contract OrsoSwap is Ownable {
    ...
    function _swap(address _tokenA, address _tokenB, uint256 _amountIn, address[]memory path) internal {
        ...
        if (_tokenA == address(0)) {
            require (_amountIn >= msg.value, "Swap: Insufficient BNB");
            ...
            IBEP20(_tokenA).safeTransferFrom(msg.sender, address(this), _amountIn);
            uint fee = _amountIn * feePercent / 10000;
            IBEP20(_tokenA).safeTransfer(feeAccount, fee);
            uint amountToSwap = _amountIn - fee;
    ...
    function _swapBNBForTokens(uint _amount, address[]memory path) internal {
        IRouter(router).swapExactBNBForTokens{ value: _amount }(
            0,
            path,
            msg.sender,
            block.timestamp
    ...
    function _swapTokensForBNB(uint _amount, address[]memory path) internal {
        IRouter(router).swapExactTokensForBNB(
            _amount,
            0,
            path,
            msg.sender,
    ...
    function _approve(address _token, uint _amount) internal {
        IBEP20(_token).approve(address(router), _amount);
    }
    ...
    function emergencyWithdrawBNB(uint _amount) external onlyOwner {
