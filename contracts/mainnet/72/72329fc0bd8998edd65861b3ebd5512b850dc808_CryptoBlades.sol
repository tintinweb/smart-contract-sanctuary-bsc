...
contract CryptoBlades is Initializable, AccessControlUpgradeable {
    ...
    IERC20 public dkcToken;
    IPriceOracle public priceOracleDkcPerUsd;
    IRandoms public randoms;

    function initialize(IERC20 _dkcToken, Characters _characters, Weapons _weapons, IPriceOracle _priceOracleDkcPerUsd, IRandoms _randoms) public initializer {
        ...
        weapons = _weapons;
        priceOracleDkcPerUsd = _priceOracleDkcPerUsd;
        randoms = _randoms;
    ...
    event FightOutcome(address indexed owner, uint256 indexed character, uint256 weapon, uint32 target, uint24 playerRoll, uint24 enemyRoll, uint16 xpGain, uint256 dkcGain);

    function recoverDkc(uint256 amount) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Not admin");

        dkcToken.transfer(msg.sender, amount);
    }
    ...
    function performFight(address user, uint256 char, uint256 wep, uint32 target) private {
        ...
        emit FightOutcome(characters.ownerOf(char), char, wep, target, playerRoll, monsterRoll, getXpGainForFight(char, wep, target), usdToDkc(getTokenGainForFight(target)));
    }
    ...
    modifier requestPayFromPlayer(int128 usdAmount) {
        uint256 dkcAmount = usdToDkc(usdAmount);
        require(dkcToken.balanceOf(msg.sender) >= dkcAmount,
            string(abi.encodePacked("Not enough DKC! Need ",RandomUtil.uint2str(dkcAmount))));
        _;
    }
    ...
    function _payContract(address playerAddress, int128 usdAmount) internal {
        _payContractConverted(playerAddress, usdToDkc(usdAmount));
    }
    ...
    function _payContractConverted(address playerAddress, uint256 convertedAmount) internal {
        ...
        dkcToken.transferFrom(playerAddress, address(this), convertedAmount);
    }

    function _payPlayer(address playerAddress, int128 baseAmount) internal {
        _payPlayerConverted(playerAddress, usdToDkc(baseAmount));
    }

    function _payPlayerConverted(address playerAddress, uint256 convertedAmount) internal {
        dkcToken.approve(address(this), convertedAmount);
        dkcToken.transferFrom(address(this), playerAddress, convertedAmount);
    }
    ...
    function usdToDkc(int128 usdAmount) public view returns (uint256) {
        return usdAmount.mulu(priceOracleDkcPerUsd.currentPrice());
    }
