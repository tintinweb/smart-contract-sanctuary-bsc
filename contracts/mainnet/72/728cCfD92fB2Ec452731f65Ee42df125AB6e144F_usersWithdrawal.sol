...
contract usersWithdrawal is Ownable {
    ...
    // Reward user with AVR tokens
    function rewardUser(address to, uint256 amount) public avrInstanceInited onlyOwner{
        _avrToken.transfer(to, amount);
        emit userRewarded(to, amount);
    }
    

    function getBalance() public view avrInstanceInited returns(uint256){
        return _getBalanceOf(address(this));
    }

    function _getBalanceOf(address eoaAddress) private view avrInstanceInited returns(uint256) {
        return _avrToken.balanceOf(eoaAddress);
    }
