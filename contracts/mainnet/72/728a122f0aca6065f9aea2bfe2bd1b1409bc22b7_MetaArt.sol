...
contract MetaArt is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    MetaArtDividendTracker public dividendTracker;
    ...
    address payable public _marketingWalletAddress = payable(0x27957eBbC007661047Cd79eab168A3F40DB6E7E6);
    ...
    constructor() BEP20("MetaArt", "MART") 
    {
        dividendTracker = new MetaArtDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "MetaArt: The dividend tracker already has that address");
        MetaArtDividendTracker newDividendTracker = MetaArtDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "MetaArt: The new dividend tracker must be owned by the MetaArt token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "MetaArt: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MetaArt: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "MetaArt: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MetaArt: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MetaArt: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MetaArt: Cannot update gasForProcessing to same value");
    ...
    {
        require(_percentage<=90, "Too high Fee");
    ...
    function _transfer(address from, address to, uint256 amount) internal open(from, to) override 
    {
        ...
        !automatedMarketMakerPairs[from] && from != owner()) 
        {
            ...
            uint256 swapableFees = totalFees.sub(BUSDRewardsFee);
...
contract MetaArtDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("MetaArt_Dividend_Tracker", "MetaArt_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200_000 * (10**18); //must hold 200000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "MetaArt_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "MetaArt_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MetaArt contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "MetaArt_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "MetaArt_Dividend_Tracker: Cannot update claimWait to same value");
