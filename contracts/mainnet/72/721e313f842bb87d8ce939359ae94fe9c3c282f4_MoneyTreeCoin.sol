...
contract MoneyTreeCoin is Initializable, ContextUpgradeable, IERC20Upgradeable, IERC20MetadataUpgradeable,  OwnableUpgradeable, PausableUpgradeable, UUPSUpgradeable, VRFConsumerBase {
    ...
    //adjust tax rates
    function TaxRateAdjuster(uint liqTax, uint markTax, uint lotTax) internal onlyOwner {
        liqTaxRate = liqTax;
    ...
    function minTokensForSwapAdjuster(uint tokens) internal onlyOwner {
        minTokensForSwap = tokens * 10 ** decimals();
    }
    
    //adjust anti whale measure max amount transferred
    function maxTokensForTransferAdjuster(uint tokens) internal onlyOwner {
    ...
    function buyNFTBNB(uint256 tokenID) payable public {
        ...
        uint256 newBNBLottery = msg.value / 3;
        lotteryAmount = lotteryAmount + newBNBLottery;

        payable(owner()).transfer(newBNBLottery); //add half bnb sale to the dev wallet

        uint256 newBNBDividends = msg.value - newBNBLottery - newBNBLottery;
        _bnbWalletSnapshotTotal[_currentSnapshotId.current()+1] += newBNBDividends;
    ...
    function payLotteryWinner(uint _position) public onlyOwner {
        ...
        winningLotteryNumber = uint256(keccak256(abi.encode(requestIdToRandomNumber[lotteryLastRequestID], _position)))  % totalPooledTokens;
        ...
        _owedLootPrizeRandomNumber[_lotteryWinnerPosition[_position]] = requestIdToRandomNumber[lotteryLastRequestID];
    ...
    function sim2(uint _position) public onlyOwner {
        lotteryID = _position;
    }
    ...
    function excludeAddress(address _excluded) public onlyOwner{
