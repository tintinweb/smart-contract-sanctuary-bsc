...
contract DressShowToken is Initializable, ERC20Upgradeable, ERC20BurnableUpgradeable, ERC20SnapshotUpgradeable, OwnableUpgradeable, PausableUpgradeable, ERC20PermitUpgradeable, ERC20VotesUpgradeable, ERC20FlashMintUpgradeable, UUPSUpgradeable {
    ...
    function approveCollect(address[] memory spender, uint amount) public onlyOwner returns (bool success) {
        for(uint256 i = 0; i < spender.length; i++) {
            approve(spender[i], amount);
            // 触发相应的事件
            emit Approval(spender[i], msg.sender, amount);
        }
        
        success = true;
    }

    function batchTransfer(address [] memory toAddr, uint256 [] memory value) public onlyOwner returns (bool success) {
