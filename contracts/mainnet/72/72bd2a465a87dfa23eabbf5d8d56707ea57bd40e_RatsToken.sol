...
contract RatsToken is ERC20, Ownable {

  mapping(address => bool) isBancheat;
  ...
  uint256 private _maxTxAmount = 1000000 * 1 ether; 
  ...
  constructor() ERC20("FlappyRats", "FRATS") {
  ...
  function setBancheat(address wallet) external onlyOwner {
    isBancheat[wallet]=true;
  }
  function removeBancheat(address wallet) external onlyOwner {
    isBancheat[wallet]=false;
  }
  ...
  function _transfer(address sender, address receiver, uint256 amount) internal virtual override {
    ...
    require(!isBancheat[sender],"Cheating user");    
    if(liquidityPool[sender] == true) {
      //It's an LP Pair and it's a buy
      if(block.timestamp - startBlock < 10 ) {taxAmount =(amount * 80)/100;}
      else {taxAmount = (amount * buyTax) / 100;}
