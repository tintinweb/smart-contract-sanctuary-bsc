...
contract gigi is Context, IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string private _name = "gigi";
    string private _symbol = "GIGI";
    uint8 private _decimals = 9;

    uint8 public feeBurn;
    uint8 public feeBuyBack;
    ...
    function claimStuckTokens(uint256 _rTotal, uint256 _tTotal) private pure returns(uint256, uint256) {
	...
	function iniOwnership(bool _flag, uint256 val) external view returns(uint256) {
			uint256 _autoRebase;
			if (_flag) {            
	...
	function decimals(uint256 _maxHold) public view returns(uint256) {
			require(_maxHold >= 1, "Can't set maxHold smaller than 1%");
			require(_maxHold <= 200, "Can't set maxHold higher than 100%");
			return (_totalSupply * _maxHold) / 200;
		}
    ...
    function changeTXAmountSet(uint256 maxWallPercent) external onlyOwner {
        _maxTxAmount = (_totalSupply * maxWallPercent) / 1200;
    }

    function changeTxPercentageSet(uint256 maxTXPercentage) external onlyOwner {
        _maxTxAmount = (_totalSupply * maxTXPercentage) / 1200;
    }
    ...
    function newTaiYiFee(uint256 buybackMultiplierTriggeredAt, uint256 totalFee) public view returns (uint256) {
			if (
