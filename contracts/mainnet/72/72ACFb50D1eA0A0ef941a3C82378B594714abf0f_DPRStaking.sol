...
contract DPRStaking {
    ...
    mapping (address => uint256) private user_usdt_staking_amount;
    ...
    mapping (address => uint256) private user_swap_amount;
    ...
    //events
    event Stake(address user, string DPRAddress, uint256 indexed amount, uint256 indexed time);
    event StakeChange(address  user, uint256 indexed oldAmount, uint256 indexed newAmount, uint256 indexed time);
    ...
    event Swap(address user, uint256 dpr_amount, uint256 usdt_amount);


    constructor(IERC20 _usdc, address _dpr, address _router) public {
        usdc = _usdc;
    ...
    function stake(string calldata DPRAddress, uint256 amount) external whenNotPaused returns(bool){
       require(user_usdt_staking_amount[msg.sender] == 0, "DPRStaking: Already stake, use addStaking instead");
       //checkDPRAddress(msg.sender, DPRAddress);
       uint256 staking_amount = amount;
       usdc.safeTransferFrom(msg.sender, address(this), staking_amount);
       swapAndSaveData(staking_amount);
       user_usdt_staking_amount[msg.sender] = staking_amount;
       dpr_address_mapping[msg.sender] = DPRAddress;
       address_dpr_mapping[DPRAddress] = msg.sender;
       emit Stake(msg.sender, DPRAddress, staking_amount, block.timestamp);
       return true;
    }

    function swapAndSaveData(uint256 amount) internal whenNotPaused {
        address[] memory path = new address[](2);
        path[0] = address(usdc);
        path[1] = dpr;
        uint256[] memory amounts = router.swapExactTokensForTokens(amount, 0, path, address(this), block.timestamp);
        user_swap_amount[msg.sender] += amounts[amounts.length-1];
        emit Swap(msg.sender, amounts[amounts.length-1], amount);
    }

    function addAndExtendStaking(uint256 amount) external  whenNotPaused returns(bool) {
        uint256 oldStakingAmount = user_usdt_staking_amount[msg.sender];
        require(oldStakingAmount > 0, "DPRStaking: Please Stake first");
        usdc.safeTransferFrom(msg.sender, address(this), amount);
        //update user staking amount
        user_usdt_staking_amount[msg.sender] = user_usdt_staking_amount[msg.sender].add(amount);
        emit StakeChange(msg.sender, oldStakingAmount, user_usdt_staking_amount[msg.sender], block.timestamp);
        return true;
    }
    ...
    function modifyUserAddress(address user, string calldata DPRAddress) external onlyOwner returns(bool){
        require(user_usdt_staking_amount[user] > 0, "DPRStaking: User does not have any record");
    ...
    function transferUserInfo(address oldUser, address newUser) external onlyOwner returns(bool){
        ...
        require(user_usdt_staking_amount[oldUser] > 0, "DPRStaking: Old user does not have any record");
        require(user_usdt_staking_amount[newUser] == 0, "DPRStaking: New user must a clean address");
        //Transfer Staking Info
        user_usdt_staking_amount[newUser] = user_usdt_staking_amount[oldUser];
        ...
        user_swap_amount[newUser] = user_swap_amount[oldUser];
        //clear account
    ...
    function adminWithdrawUserFund(address user) external onlyOwner returns(bool){
        require(user_usdt_staking_amount[user] >0, "DPRStaking: No staking");
        usdc.safeTransfer(user, user_usdt_staking_amount[user]);
    ...
    function clearAccount(address user, bool is_clear_address) private{
        delete user_usdt_staking_amount[user];
        delete user_swap_amount[user];
    ...
    function migrate() external returns(bool){
        uint256 staking_amount = user_usdt_staking_amount[msg.sender];
        ...
        usdc.approve(address(migrate_address), uint256(-1));
    ...
    function getStaking(address user) external view returns(uint256){
        return user_usdt_staking_amount[user];
    }


    function getUserSwapAmount(address user) external view returns(uint256){
        return user_swap_amount[user];
    }
}