...
abstract contract ERC721Enumerable is ERC721, IERC721Enumerable {
    ...
    returns (uint[] memory)
    {
        uint balanceOf = ERC721.balanceOf(owner);
        uint[] memory result = new uint[](balanceOf);

        for (uint i; i < balanceOf; i++) {
...
contract MISADITEM is ERC721Enumerable, MISADAUTH {
    struct LevelUpPrice {
        uint pair;
        uint misad;
    }
    ...
    // Base URI
    string private _baseUriExtended = "https://api.wingofmisadventure.com/api/item/v1/";

    constructor() ERC721("Wing of Misadventure Item", "MISADITEM") MISADAUTH(msg.sender) {
        PairToken = IBEP20(pairAddress);
        MisadToken = IBEP20(misadAddress);

        //Add Genesis Items (ID: 0)
        addCardSet(1000);
    ...
    function _mintCardsOfPack(address user, uint8 packId, uint8 numOfCards) internal {
        require(_items.length == currentIndex, "Error length");

        uint itemId = currentIndex;

        for (uint8 i; i < numOfCards; i++) {
            packIdOfToken[itemId] = packId;
            _safeMint(user, itemId);
            _createItem(0, 0);

            emit CardMinted(user, itemId, packId);
            itemId += 1;
        }
    }

    function levelUp(uint itemId) external {
        require(ownerOf(itemId) == msg.sender, "!owner");

        Item storage item = _items[itemId];
        LevelUpPrice memory levelData = levelUpPrices[item.level];
        uint8 newLevel = item.level + 1;

        require(newLevel <= maxLevel, "Max level");
        require(newLevel == 1 || item.rarity > 0, "Wait for summoned");
        ...
        if(newLevel == 1){
            ...
            try IMisadReflect(reflectAddress).registerNewMint( itemId ) {} catch {}

            emit SummonItem(itemId);
        }else if(newLevel <= 5){
            item.level = newLevel;
            emit UserLevelUp(itemId, newLevel);
        }else{
            emit WaitingLevelUp(itemId, newLevel);
        }
    }

    function getItem(uint itemId) external view returns (uint8 level, uint8 rarity) {
    ...
    function adminSetAttribute(uint itemId, string memory attribute, string memory value) public {
        authorizedFor(Permission.ManageAttributes);

        require(attributeExists[attribute], "!Attribute");

        nftAttributes[itemId][attribute] = value;

        emit AttributeChanged(itemId, attribute, value);
    }
    ...
    function adminKillItem(uint itemId) external {
        authorizedFor(Permission.Mint);

        safeTransferFrom(ownerOf(itemId), deadAddress, itemId);

        try IMisadReflect(reflectAddress).updateBurnedToken( itemId ) {} catch {}

        emit AdminKillItem(itemId);
    }

    function adminSetLevel(uint itemId, uint8 newLevel) external {
        authorizedFor(Permission.ManageAttributes);
        ...
        emit AdminSetLevel(itemId, newLevel);
    }

    function adminSetRarity(uint itemId, uint8 rarity) external {
        authorizedFor(Permission.ManageAttributes);
    ...
    function _createItem(uint8 level, uint8 rarity) internal {
        _items.push(Item(level, rarity));
    }
    ...
    function _singleMint(address recipient, uint8 packId, uint8 level, uint8 rarity) internal {
        ...
        _createItem(level, rarity);

        cardSets[packTypes[packId].cardSetId].minted++;
        emit CardMinted(recipient, itemId, packId);
    }

    event CardMinted(address recipient, uint itemId, uint8 packId);
    ...
    event AttributeChanged(uint itemId, string attributeName, string newValue);
    event SummonItem(uint indexed itemId);
    event AdminKillItem(uint indexed itemId);
    event AdminSetLevel(uint indexed itemId, uint8 newLevel);
    event UserLevelUp(uint indexed itemId, uint8 newLevel);
    event WaitingLevelUp(uint indexed itemId, uint8 newLevel);
}