...
contract UniswapReward is LPTokenWrapper{
    ...
    uint256 public _usdtAmount = 150*1e15;
    ...
    constructor(address new_plusAddr,address new_lpAddr) public {
         _plusAddr = new_plusAddr;
         _lpAddr = new_lpAddr;
    }
    ...
    function govSetRewards(uint amount) external onlyGovernance checkHalve {
      _rewardRate = amount;
        if (totalSupply() == 0) {
            return;
        }
    ...
    function qualified(address account) public view returns (bool) {
        uint lpgot = IPancakePair(_lpAddr).balanceOf(account);
        uint lptotal = IPancakePair(_lpAddr).totalSupply();
        (uint112 reserve0, uint112 reserve1,) = IPancakePair(_lpAddr).getReserves();
        uint usdtgot = IPancakePair(_lpAddr).token0() == _USDT?lpgot.mul(reserve0).div(lptotal):lpgot.mul(reserve1).div(lptotal);

        return usdtgot >= _usdtAmount;
    }
    ...
    function stake(uint256 amount)
        ...
        checkStart
    {
        require(amount >0 && amount<=15,"amount err");
        require(qualified(msg.sender),"qualified err");
        stakeinternal(amount * 100*1e15);
        ...
        checkHalve
    {
        ...
        if(_lastStakedTime[msg.sender].add(30 days) > block.timestamp)
        {
            _balances[msg.sender] = _balances[msg.sender].mul(95).div(100);
        }

        IERC20(_USDT).safeTransfer( msg.sender, _balances[msg.sender]);
    ...
    function getReward() public checkHalve {
        ...
        if (reward > 0) {
            ...
            IERC20(_USDT).safeTransfer(msg.sender, reward );
    ...
    modifier checkStart() {
        require(block.timestamp > _startTime && block.timestamp < _startTime.add(24 hours), "not start");
        _;
    }
