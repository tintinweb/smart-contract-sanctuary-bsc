...
// File: contracts/interfaces/IWeedSwapRouter01.sol


pragma solidity >=0.6.2;

interface IWeedSwapRouter01 {
...
// File: contracts/interfaces/IWeedSwapPair.sol


pragma solidity >=0.5.0;

interface IWeedSwapPair {
    event Approval(address indexed owner, address indexed spender, uint256 value);
    event Transfer(address indexed from, address indexed to, uint256 value);
...
// File: contracts/interfaces/IWeedSwapFactory.sol


pragma solidity >=0.5.0;

interface IWeedSwapFactory {
...
// File: contracts/libraries/WeedSwapLibrary.sol
...
library WeedSwapLibrary {
    using SafeMath for uint256;

    // returns sorted token addresses, used to handle return values from pairs sorted in this order
    function sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
        require(tokenA != tokenB, "WeedSwapLibrary: IDENTICAL_ADDRESSES");
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), "WeedSwapLibrary: ZERO_ADDRESS");
    }
            ...
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        hex"408071b74013792c9066b994a74ef077cb61befd89aab6d4b547f34325942f9d" // init code hash
                    )
                )
            )
        );
    ...
    function getReserves(
        ...
        (uint256 reserve0, uint256 reserve1, ) = IWeedSwapPair(pairFor(factory, tokenA, tokenB)).getReserves();
    ...
    function quote(
        ...
        require(amountA > 0, "WeedSwapLibrary: INSUFFICIENT_AMOUNT");
        require(reserveA > 0 && reserveB > 0, "WeedSwapLibrary: INSUFFICIENT_LIQUIDITY");
...
// File: contracts/WeedSwapRouter01.sol
