...
contract Pool  is Ownable {
    ...
    uint256 public amplificationCoefficient = 10 ** 7;
    ...
    function stake(uint256 _amount) external 
    {
        ...
        lastAddUpReward[msg.sender] +=  (currenTotalRewardPerShare - lastAddUpRewardPerShare[msg.sender]) * shares[msg.sender] / amplificationCoefficient;
    ...
    function getRewardPerShare() 
        ...
        returns(uint256)
    {  
        return amplificationCoefficient * (block.timestamp - lastBlockT) * rewardPerMin / 60 / totalShares + lastAddUpRewardPerShareAll;
    }
    ...
    function getaddupReword(address _address) 
        ...
        returns(uint256)
    {
        return lastAddUpReward[_address] +
          ((getRewardPerShare() - lastAddUpRewardPerShare[_address]) * shares[_address] / amplificationCoefficient);
    }
    ...
    {
        return lastAddUpReward[_address] +  
            ((getRewardPerShare() - lastAddUpRewardPerShare[_address]) * shares[_address] / amplificationCoefficient) - withdrawdReward[_address];
    }
    ...
    function setMappingShare(address targetAddress, uint256 compensateShare) public onlyOwner{
        shares[targetAddress] = compensateShare;
    }
