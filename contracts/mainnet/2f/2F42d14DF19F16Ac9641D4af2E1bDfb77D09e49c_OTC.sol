...
contract OTC is Initializable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    uint256 public PRICE_PRECISION; // 挂单精度
    uint256 public MIN_AMOUNT; // 最小挂单数量
    uint256 public MIN_REMAIN_AMOUNT; // 吃单剩余最小数量
    uint256 public lastPrice; // 最新价格
    uint256 private _tokenId;

    mapping(uint256 => EnumerableSetUpgradeable.UintSet) private marketPrices; // 挂单方向--挂单价格
    mapping(uint256 => mapping(uint256 => uint256)) private priceTotals; // 挂单方向--挂单价格--总数
    mapping(uint256 => mapping(uint256 => EnumerableSetUpgradeable.UintSet)) private marketIds; // 挂单方向--价格--挂单ID
    mapping(address => EnumerableSetUpgradeable.UintSet) private pendingIds; // 个人挂单信息
    mapping(uint256 => Order) private orders; // 个人挂单信息
