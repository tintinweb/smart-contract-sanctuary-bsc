...
contract Staking is Ownable {
    ...
    uint256 public _bnb = 1 * _decimal;

    uint256 public APY = 27;
    ...
    function unstake() public payable{
        ...
        uint256 res = (_bnb).div(_denominator);

        require(msg.value >= res,"Needs to Pay 0.1% BNB fee for unstake!!");
        ...
        R_Token.transfer(msg.sender,final_reward.add(rec._previousReward));

        clear(msg.sender);
    }

    function checkpercentbnb(address _user) public view returns (uint){
        Data storage rec = stakers[_user];
        uint256 subtotal = rec._stakedAmount;
        uint256 res = subtotal.mul(_fee).div(_denominator);
        return res;
    }
