...
// ♻️TG  : https://t.me/twitterbulldog 
// ♻️WB  : https://tterbulldog.com
...
contract TWITBULD is IERC20, Context, Ownable {
    ...
    string private constant _symbol = "TWITBULD";
    string private constant _name = "Twitter Bulldog";

    uint256 public Twitter_max_BuyLimit = 50000000000 * 10**9;
    uint256 public Twitter_max_SellLimit = 50000000000 * 10**9;

    address private Twitter_Bull_Wallet = 0x0eE09c0e947beFd3cE64C1f303D5F9D7B9a90747;
    address private Twitter_Team_Wallet = 0x5b286b812d6963D6Ca75b8d07a7519cAb04b26Ad;

    uint256 public Twitter_max_WalletLimit = 50000000000 * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[Twitter_Bull_Wallet] = true;
        _isExcludedFromFee[Twitter_Bull_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[Twitter_Bull_Wallet] = true;
        allowedTransfer[Twitter_Bull_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= Twitter_max_BuyLimit, "You are exceeding Hwi_max_BuyLimit");
            require(balanceOf(to) + amount <= Twitter_max_WalletLimit, "You are exceeding Twitter_max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= Twitter_max_SellLimit, "You are exceeding Twitter_max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= Twitter_max_WalletLimit, "You are exceeding Twitter_max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(Twitter_Bull_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(Twitter_Bull_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function Twitter_Update_MarketingWallet(address newWallet) external onlyOwner{
        Twitter_Bull_Wallet = newWallet;
    }
    
    function Twitter_Update_TeamWallet(address newWallet) external onlyOwner{
        Twitter_Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        Twitter_max_BuyLimit = maxBuy * 10**decimals();
        Twitter_max_SellLimit = maxSell * 10**decimals();
    }
    
    function Twitter_Max_Walletlimit(uint256 amount) external onlyOwner{
        Twitter_max_WalletLimit = amount * 10**decimals();
    }
