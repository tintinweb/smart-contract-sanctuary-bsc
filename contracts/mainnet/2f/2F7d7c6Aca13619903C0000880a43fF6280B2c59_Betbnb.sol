 ...
 * @title Betbnb
 */
contract Betbnb is OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint256 public minBetbAmount; // minimum betb token condition amount (denominated in wei)
    ...
    event NewMinBetbAmount(uint256 indexed epoch, uint256 minBetbAmount);
    event NewTreasuryFee(uint256 indexed epoch, uint256 treasuryFee);
    event NewOperatorAddress(address operator);
    event NewBetbAddress(address betb);
    ...
    function initialize (
        ...
        address _betbAddress,
        uint256 _intervalSeconds,
        uint256 _bufferSeconds,
        uint256 _minBetAmount,
        uint256 _minBetbAmount,
    ...
    function betBear(uint256 epoch) external payable whenNotPaused nonReentrant notContract {
        ...
        if(minBetbAmount > 0) {
            require(IERC20Upgradeable(betbAddress).balanceOf(msg.sender) >= minBetAmount, "BetB token Balance must be greater than minBetbAmount");
        }
    ...
    /**
     * @notice Set minBetbtokenAmount
     * @dev Callable by admin
     */
    function setMinBetbtokenAmount(uint256 _minBetbAmount) external whenPaused onlyAdmin {
        minBetbAmount = _minBetbAmount;

        emit NewMinBetbAmount(currentEpoch, minBetbAmount);
    }
    ...
    /**
     * @notice Set betb token address
     * @dev Callable by admin
     */
    function setBetbAddress(address _betbAddress) external onlyAdmin {
        require(_betbAddress != address(0), "Cannot be zero address");
        betbAddress = _betbAddress;

        emit NewBetbAddress(_betbAddress);
    }
