...
contract PeghubZap is ContextUpgradeable {
    modifier onlyOwner() {
    ...
    struct PairInfo {
        ...
        IUniswapV2Pair pairAddress; // Address of LP pair for vault.
        address baseToken; // Address of other asset in pair.
        address baseTokenPair; // Address of basetoken pair.
        IRouter baseTokenRouter; // Address of router to get the base token
        bool isEnabled;
    }

    struct TokenInfo {
        IERC20Upgradeable token; // Address of token.
    ...
    mapping(address => uint256) private pairLookup;

    mapping(address => uint256) private tokenLookup;

    mapping(address => uint256) private vaultLookup;

    constructor(address _dev, address _beefyUniZapV2) initializer {}

    function initialize(address _dev, address _beefyUniZapV2)
        public
        initializer
    {
        dev = _dev;
    ...
    function beefInETH(address beefyVault, uint256 tokenAmountOutMin)
        ...
        payable
    {
        // require(msg.value >= 1000, "PegHubzap: Insignificant input amount");
        // (IBeefyVault vault, ) = _getVaultPair(beefyVault);

        IWETH(WETH).deposit{value: msg.value}();
        beefIn(beefyVault, tokenAmountOutMin, WETH, msg.value);
        // PairInfo memory basePairInfo = pairInfo[vaultLookup[beefyVault]];
        // address baseToken = basePairInfo.baseToken;

        // uint256 wethBalance = IERC20Upgradeable(WETH).balanceOf(address(this));
        // if (baseToken == WETH) {
        //     _approveTokenIfNeeded(WETH, address(beefyUniV2Zap));
        ...
        //     );
        // } else {
        //     uint256 baseBalance = swap(baseToken, 0, WETH, wethBalance);
        //     _approveTokenIfNeeded(WETH, address(beefyUniV2Zap));
        //     beefyUniV2Zap.beefIn(
        //         beefyVault,
        //         tokenAmountOutMin,
    ...
    function beefIn(
        ...
        if (token0 == tokenIn || token1 == tokenIn) {
            ...
            PairInfo memory basePairInfo = pairInfo[vaultLookup[beefyVault]];
            address baseToken = basePairInfo.baseToken;
            uint256 _baseBalance = swap(baseToken, 0, tokenIn, tokenInAmount);
            beefyUniV2Zap.beefIn(
                beefyVault,
                tokenAmountOutMin,
                baseToken,
    ...
    function swap(
        ...
        returns (
            ...
            uint256 _baseBalances
        )
    {
        require(tokenInAmount >= 1000, "PegHubzap: Insignificant input amount");
        require(
            IERC20Upgradeable(tokenIn).allowance(_msgSender(), address(this)) >=
                tokenInAmount,
            "PegHubzap: Input token is not approved"
        );
        ...
        uint256 baseBalance = IERC20Upgradeable(tokenOut).balanceOf(
            address(this)
        );

        if (baseBalance > 0) {
            uint256 baseFeeTotal = baseBalance.mul(1).div(100);
            IERC20Upgradeable(tokenOut).safeTransfer(feeReceiver, baseFeeTotal);
            baseBalance = baseBalance.sub(baseFeeTotal);
        }
        return baseBalance;
    }
        ...
        )
    {
        ...
        if (tokenIn == WETH) {
            if (baseToken == WETH) {
                baseTokenAmount = fullInvestmentIn;
            } else {
                baseTokenAmount = estimateSwapToBaseToken(
                    WETH,
                    baseToken,
                    fullInvestmentIn
                );
            ...
            baseTokenAmount = estimateSwapToWETH(tokenIn, fullInvestmentIn);
        } else {
            baseTokenAmount = estimateSwapToBaseToken(
                tokenIn,
                baseToken,
        ...
        (swapAmountIn, swapAmountOut, swapTokenOut) = beefyUniV2Zap
            .estimateSwap(beefyVault, baseToken, baseTokenAmount);
    }
    ...
    function estimateSwapToBaseToken(
        ...
        PairInfo memory basePairInfo = pairInfo[vaultLookup[beefyVault]];
        address baseToken = basePairInfo.baseToken;
        IUniswapV2Pair basePair = IUniswapV2Pair(basePairInfo.baseTokenPair);
        bool isBaseInputA = basePair.token1() == baseToken;
        require(
            isBaseInputA || basePair.token0() == baseToken,
            "PegHubZap: Input token not present in liqudity pair"
        );
        ...
        return
            _getQuote(
                ...
                baseReserveA,
                baseReserveB,
                address(basePairInfo.baseTokenRouter)
            );
    }
    ...
    function addPair(
        address _beefyVault,
        address _baseToken,
        address _baseTokenRouter,
        bool _isEnabled
    ) public onlyOwner {
        checkTokenDuplicate(_beefyVault);

        (IBeefyVault vault, IUniswapV2Pair pair) = _getVaultPair(_beefyVault);
        address _factory = IRouter(_baseTokenRouter).factory();
        IUniswapV2Pair basePair = IUniswapV2Pair(
            IFactory(_factory).getPair(WETH, _baseToken)
        );
        PairInfo memory _pairInfo = PairInfo({
            beefyVault: vault,
            pairAddress: pair,
            baseToken: _baseToken,
            baseTokenPair: address(basePair),
            baseTokenRouter: IRouter(_baseTokenRouter),
            isEnabled: _isEnabled
        });
        pairInfo.push(_pairInfo);
        pairLookup[_baseToken] = totalPairs;
        vaultLookup[_beefyVault] = totalPairs;
        _giveAllowance(_baseToken, address(beefyUniV2Zap));
        totalPairs++;
    }

    // Add a new token to to be used as input. Can only be called by the owner.
    function addToken(address _token, bool _isEnabled) public onlyOwner {
        checkTokenDuplicate(_token);
        address _factory = IRouter(pcsRouter).factory();
        address pair = IFactory(_factory).getPair(WETH, _token);
    ...
    function _swap(
        ...
        if (tokenIn == WETH) {
            ...
            path[0] = tokenIn;
            path[1] = WETH;
            _approveTokenIfNeeded(path[0], address(pcsRouter));
            ...
            wethAmount = IERC20Upgradeable(WETH).balanceOf(address(this));
        }
        PairInfo memory basePairInfo = pairInfo[pairLookup[tokenOut]];
        address baseToken = basePairInfo.baseToken;
        IUniswapV2Pair basePair = IUniswapV2Pair(basePairInfo.baseTokenPair);
        IRouter baseRouter = basePairInfo.baseTokenRouter;
        bool isBaseInputA = basePair.token1() == baseToken;
        require(
            isBaseInputA || basePair.token0() == baseToken,
            "PegHubZap: Output token not present in liqudity pair"
        );
        address[] memory basePath = new address[](2);
        basePath[0] = WETH;
        basePath[1] = tokenOut;
    ...
    function _giveAllowance(address _address, address _router) internal {
        IERC20Upgradeable(_address).approve(_router, 0);
        IERC20Upgradeable(_address).approve(_router, uint256(1e50));
    }
    ...
    function _getVaultPair(address beefyVault)
        private
        pure
        returns (IBeefyVault vault, IUniswapV2Pair pair)
    {
    ...
    function checkPairDuplicate(address _beefyVault) internal view {
        uint256 length = totalPairs;
        for (uint256 pid = 0; pid < length; ++pid) {
            require(
                address(pairInfo[pid].beefyVault) != _beefyVault,
