...
function withdraw () public {
 ...
 if (user.DEP_activatesRefer >= 2  && user.DEP_activatesRefer >= block.timestamp.add(1 minutes)/*&& user.checkpointLVR1RefBonus >= 5 minutes*/)
 
{
 	...
 	function DEP_activatesRefer_Timers(address userAddress) public view returns (bool) {
		User storage user = users[userAddress];



if(user.DEP_activatesRefer >= block.timestamp.add(5 minutes))
{
    user.RERE == true;
}
if(user.DEP_activatesRefer <= block.timestamp.add(5 minutes))
