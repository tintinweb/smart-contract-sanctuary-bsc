...
contract KashiPairWrapperLogic is Initializable, IFlashBorrower{
     ...
     */
    function initialize(
        address[8] calldata addrParams_,
    ...
    function _selfLiquidatePaySpecific(
        ...
        if (collateral != IERC20(WETH)) {
            ...
            /// @dev {address.transfer} can't be used in proxy mechanism
            IWETH(WETH).transfer(wethRefunder, _leftCollateralAmount);
            IWETH(wethRefunder).withdraw(_leftCollateralAmount);
    ...
    function _processFee(
        ...
        if (feeInfo.isAUTO) {
            ...
            if (assetAddr == WETH) {
                /// @dev {address.transfer} can't be used in proxy mechanism
                IWETH(WETH).transfer(wethRefunder, feeAmount);
                IWETH(wethRefunder).withdraw(feeAmount);
