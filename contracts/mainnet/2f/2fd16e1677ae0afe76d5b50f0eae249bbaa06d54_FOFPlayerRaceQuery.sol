...
contract FOFPlayerRaceQuery is Ownable {
    ...
    address public FOF = 0xa1Cf6214D17771B25DAF2aA7802c159F40921119;

    address public pancakeswapV2Pair = 0x3559179416b6F1ae70cD87c20C5b499c1F1A9930;

    uint256 public rankRaceUSDTReward = 3 * 10 ** 18;

    uint256 public settlementUSDTReward = 2 * 10 ** 18;

    uint256 public onceFuelToRaceTimes = 5;
    ...
    function setSettlementUSDTReward(uint256 _settlementUSDTReward) public onlyOwner {
        settlementUSDTReward = _settlementUSDTReward;
    }

    function setRankRaceUSDTReward(uint256 _rankRaceUSDTReward) public onlyOwner {
        rankRaceUSDTReward = _rankRaceUSDTReward;
    }

    function setOnceFuelToRaceTimes(uint256 _times) public onlyOwner {
        onceFuelToRaceTimes = _times;
    }
    ...
    function rankRaceRewards(uint256 reward) public view returns(uint256[] memory rewards) {
        ...
        uint256 reward2 = USDTEqualsToToken(rankRaceUSDTReward);
    ...
    function ticketDivided(uint256 ticketPrice) public pure returns(uint256 rank,uint256 repo,uint256 maket,uint256 league){
        rank = ticketPrice.mul(70).div(100);
        repo = ticketPrice.mul(12).div(100);
        league = ticketPrice.mul(5).div(100);
        maket = ticketPrice.sub(rank).sub(repo).sub(league * 3);
        
        return (rank,repo,maket,league);
    }

    function fuelDivided(uint256 fuelPrices) public pure returns(uint256 maket,uint256 burn){
        maket = fuelPrices.mul(10).div(100);
        // uint256 union = fuelPrices.mul(5).div(100);
        burn = fuelPrices.sub(maket * 4);
    ...
    uint256[] memory vehicleNums, uint256[3] memory raceMembersIndexs, uint256 raceType) {
        ...
        if(block.timestamp < FPRD.raceTime(2, FPRD.raceIds(2)) &&  block.timestamp + 72 hours > FPRD.raceTime(2, FPRD.raceIds(2))){
            raceType = 2;
        }else if(block.timestamp < FPRD.raceTime(3, FPRD.raceIds(3)) &&  block.timestamp + 72 hours > FPRD.raceTime(3, FPRD.raceIds(3))){
            raceType = 3;
        }
    ...
    function leaguePool(uint256 raceType) public view returns(uint256 reward, uint256 fofReward){

        if(raceType == 2){

            reward = FPC1.leagueAddress().balance.mul(48).div(100);

            fofReward = ERC20(FOF).balanceOf(FPC1.leagueAddress()).mul(48).div(100);
        }

        if(raceType == 3){

            reward = FPC1.leagueAddress().balance.mul(32).div(100) + FPRD.raceReward(3,FPRD.raceIds(raceType));

            fofReward = ERC20(FOF).balanceOf(FPC1.leagueAddress()).mul(32).div(100) + FPRD.fofRaceReward(3,FPRD.raceIds(raceType));
        }

        return (reward,fofReward);
    }
    ...
    function raceRanking(uint256 raceType,uint256 raceId) public view returns(uint256[] memory vehicleIds) {
        ...
        if(raceType == 1){
            ...
            // for(i = 0; i < rankRulesToRaceIds.length; i++){
            //     if(raceId < rankRulesToRaceIds[i] && i.mod(2) == 1){
            //         return vehicleIds0;
            //     }
            // }
        }else{
    ...
    function checkCanSignUpRank(address user, uint256 raceId, uint256 vehicleId) public view returns(uint256) {
        ...
        if(FPRD.userTodayRaceTimes(user) >= onceFuelToRaceTimes * 2 && FPRD.userLastRaceTime(user).div(24 hours) == block.timestamp.div(24 hours)) return 8;//"You have run out of game times for today"
    ...
    function checkCanBuyFuel(address user) public view returns(uint256) {
        ...
        if(FPRD.userTodayRaceTimes(user) >= onceFuelToRaceTimes * 2 && FPRD.userLastRaceTime(user).div(24 hours) == block.timestamp.div(24 hours)) return 3;
