...
🙇‍♀️ 20% Burn
🙇‍♀️ 5% Marketing
🙇‍♀️ 20% Future Partnership , (Preparing Supply for listing in 200 CEX)
...
Tax Buy and Sell , 10/10-13
Use Slippage 9-15 (depend on volume)
...
*Note : Tax will be lowered gradually
...
contract CityGirl is ERC20, Auth {
    ...
    event SellFeesChanged(uint256 _liquidityFee, uint256 _MarketingFee,uint256 _opsFee, uint256 _devFee, uint256 _StakeFee);
    event BuyFeesChanged(uint256 _liquidityFee, uint256 _MarketingFee,uint256 _opsFee, uint256 _devFee, uint256 _StakeFee);
    event TransferFeeChanged(uint256 _transferFee);
    event SetFeeReceivers(address _liquidityReceiver, address _MarketingReceiver,address _operationsFeeReceiver, address _devFeeReceiver, address _StakeFeeReceiver);
    ...
    string constant private _name = "CityGirl";
    string constant private _symbol = "CityGirl";
    ...
    uint256 private sellFeeStake = 2;
    ...
    //totalFees
    uint256 private totalBuyFee = liquidityFee.add(MarketingFee).add(opsFee).add(devFee).add(StakeFee);
    uint256 private totalSellFee = sellFeeLiquidity.add(sellFeeMarketing).add(sellFeeOps).add(sellFeeDev).add(sellFeeStake);

    uint256 private feeDenominator  = 100;

    address private autoLiquidityReceiver =0xD2Ef6e93436A902A7119C2a5f0D8F39891b83CA8;
    address private MarketingFeeReceiver =0x70F79Ff86c85b66c032AEa54A0a5c0347048417b;
    address private operationsFeeReceiver =0xD2Ef6e93436A902A7119C2a5f0D8F39891b83CA8;
    address private devFeeReceiver =0xD2Ef6e93436A902A7119C2a5f0D8F39891b83CA8;
    address private StakeFeeReceiver =0xad60155DfEBaB58993c59216e7a934cF9a69F7B5;
    address private StakeRewardFeeReceiver =0x6f6c12286926Eb1873471017024f682E49f0c282;
    ...
    function takeBuyFee(address sender, address recipient, uint256 amount) internal returns (uint256){
        ...
        uint256 feeAmount = amount.mul(totalBuyFee.sub(StakeFee)).div(feeDenominator);
        uint256 StakeFeeAmount = amount.mul(StakeFee).div(feeDenominator);
        uint256 totalFeeAmount = feeAmount.add(StakeFeeAmount);

        _balances[address(this)] = _balances[address(this)].add(feeAmount);
        emit Transfer(sender, address(this), feeAmount);

        if(StakeFeeAmount > 0) {
            _balances[StakeFeeReceiver] = _balances[StakeFeeReceiver].add(StakeFeeAmount/2);
            emit Transfer(sender, StakeFeeReceiver, StakeFeeAmount);
        }
    ...
    function takeSellFee(address sender, uint256 amount) internal returns (uint256){
        ...
        uint256 feeAmount = amount.mul(totalSellFee.sub(sellFeeStake)).div(feeDenominator);
        uint256 StakeFeeAmount = amount.mul(sellFeeStake).div(feeDenominator);
        uint256 totalFeeAmount = feeAmount.add(StakeFeeAmount);
        ...
        if(StakeFeeAmount > 0) {
            _balances[StakeFeeReceiver] = _balances[StakeFeeReceiver].add(StakeFeeAmount/2);
            _balances[StakeRewardFeeReceiver] = _balances[StakeRewardFeeReceiver].add(StakeFeeAmount/2);
            emit Transfer(sender, StakeFeeReceiver, StakeFeeAmount/2);
            emit Transfer(sender, StakeRewardFeeReceiver, StakeFeeAmount/2);
        }
    ...
    function swapBack() internal swapping {
        ...
        uint256 realTotalFee =totalBuyFee.add(totalSellFee).sub(StakeFee).sub(sellFeeStake);
    ...
    function setBuyFees(uint256 _liquidityFee, uint256 _MarketingFee, uint256 _opsFee, uint256 _devFee, uint256 _StakeFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        MarketingFee = _MarketingFee;
        opsFee = _opsFee;
        devFee = _devFee;
        StakeFee = _StakeFee; 
        totalBuyFee = _liquidityFee.add(_MarketingFee).add(_opsFee).add(_devFee).add(StakeFee);
        feeDenominator = _feeDenominator;
        require(totalBuyFee <= maxFee, "Fees cannot be higher than Maxfee");

        emit BuyFeesChanged(_liquidityFee, _MarketingFee,_opsFee, _devFee, _StakeFee);
    }

    function setSellFees(uint256 _liquidityFee, uint256 _MarketingFee,uint256 _opsFee, uint256 _devFee, uint256 _StakeFee, uint256 _feeDenominator) external authorized {
        sellFeeLiquidity = _liquidityFee;
        sellFeeMarketing = _MarketingFee;
        sellFeeOps = _opsFee;
        sellFeeDev = _devFee;
        sellFeeStake = _StakeFee;
        totalSellFee = _liquidityFee.add(_MarketingFee).add(_opsFee).add(_devFee).add(_StakeFee);
        feeDenominator = _feeDenominator;
        require(totalSellFee <= maxFee, "Fees cannot be higher than Maxfee%");

        emit SellFeesChanged(_liquidityFee, _MarketingFee,_opsFee, _devFee, _StakeFee);
    }
    ...
    function setFeeReceivers(address _autoLiquidityReceiver, address _MarketingFeeReceiver,address _operationsFeeReceiver, address _devFeeReceiver, address _StakeFeeReceiver) external authorized {
        require(_autoLiquidityReceiver != address(0) && _MarketingFeeReceiver != address(0) && _operationsFeeReceiver != address(0) && _devFeeReceiver != address(0) && _StakeFeeReceiver != address(0), "Zero Address validation" );
        ...
        emit SetFeeReceivers(_autoLiquidityReceiver, _MarketingFeeReceiver, _operationsFeeReceiver, _devFeeReceiver, _StakeFeeReceiver);
    }
