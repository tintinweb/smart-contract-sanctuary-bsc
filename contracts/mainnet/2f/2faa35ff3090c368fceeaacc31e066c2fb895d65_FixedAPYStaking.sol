...
contract FixedAPYStaking is Ownable, ReentrancyGuard {
    ...
    uint256 public monthlyWithdrawFee = 0.02 ether;

    uint256 public minTokensToStake = 500;
    ...
    constructor(address _penaltyReceiver, address _stakingToken, uint256 _apy, uint256 lockDays, uint256 exitPenalty) {
        stakingToken = IBEP20(_stakingToken); // make sure to update for mainnet
        rewardToken = IBEP20(_stakingToken);

        bnbReceiver = _penaltyReceiver;

        apy = _apy;
        lockDuration = lockDays * 1 days;
        exitPenaltyPerc = exitPenalty;
    ...
    function updateMonthlyWithdrawFee(uint256 amountInWei) external onlyOwner {
        require(amountInWei <= 0.1 ether, "amount too high");
        monthlyWithdrawFee = amountInWei;
    }

    function updateMinTokensToStake(uint256 amountInTokens) external onlyOwner {
        minTokensToStake = amountInTokens * (10 ** stakingToken.decimals());
    }

    function setBnbReceiver(address wallet) external onlyOwner {
        require(wallet != address(0), "can't set to 0 address");
        bnbReceiver = wallet;
    }
    ...
    function deposit(uint256 _amount) public nonReentrant {
        ...
        if(user.amount == 0){
            require(_amount >= minTokensToStake, "Must stake minimum amount of tokens");
        }
    ...
    function withdraw() public nonReentrant {
        ...
        require(holderUnlockTime[msg.sender] <= block.timestamp, "May not do normal withdraw early if penalty is active");
    ...
    function getWithdrawPenaltyFee(address holder) public view returns (uint256){
        if(holderUnlockTime[holder] >= block.timestamp){
            return monthlyWithdrawFee + ((holderUnlockTime[holder]-block.timestamp)/30 days * monthlyWithdrawFee);
        } else {
            return 0;
        }
    }
    ...
    function emergencyWithdraw() external payable nonReentrant {
        ...
        if(holderUnlockTime[msg.sender] >= block.timestamp){
            require(msg.value >= getWithdrawPenaltyFee(msg.sender), "Not enough BNB sent to unlock");
        }
        PoolInfo storage pool = poolInfo[0];
        UserInfo storage user = userInfo[msg.sender];
        uint256 _amount = user.amount;
        ...
        user.amount = 0;
        user.rewardDebt = 0;
        (success,) = address(bnbReceiver).call{value: address(this).balance}("");
