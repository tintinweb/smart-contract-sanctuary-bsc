 ...
 */
contract BEP20 is Context, IBEP20, Ownable, AccessControl {
...
// Cherry with Governance.
contract Cherry is BEP20 {
    ...
    constructor(address minter, address burner, Apple _apple, address marketing) public BEP20('Cherry', 'CHERRY') {
        _setupRole(MINTER_ROLE, minter);
        _setupRole(BURNER_ROLE, burner);
        _setupRole(FEE_SETTER_ROLE, msg.sender);
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
    ...
    function setMarketingFee(uint256 _percent) public {
        ...
        require(_percent <= 10000, "CHERRY: INVALID_MARKETING_FEE");
        marketingFee = _percent;
    }

    function setMarketingAddress(address _marketing) public {
        require(hasRole(FEE_SETTER_ROLE, msg.sender), "CHERRY: NOT_ALLOWED");
        require(_marketing != address(0), "CHERRY: INVALID_ADDRESS");
        marketingAddress = _marketing;
    }

    // Safe apple transfer function, just in case if rounding error causes pool to not have enough apples.
    function safeAppleTransfer(address _to, uint256 _amount) public onlyOwner {
