...
contract TSLRouter is IPancakeRouter02 {
    using SafeMath for uint256;
        ...
        returns (uint256[] memory amounts)
    {
        ...
        _swapForUSDT(amountIn.mul(fee) / 1000, pathForUSDT);
    }
        ...
        returns (uint256[] memory amounts)
    {
        ...
        _swap(amounts, path, to);
        _swapForUSDT(amountInMax.mul(fee) / 1000, pathForUSDT);
    }
        ...
        returns (uint256[] memory amounts)
    {
        ...
        _swap(amounts, path, to);
        uint256[] memory amountsForUSDT = PancakeLibrary.getAmountsOut(
            factory,
            msg.value.mul(fee) / 1000,
            pathForUSDT
        );
        assert(
            IWETH(WETH).transfer(
                PancakeLibrary.pairFor(factory, pathForUSDT[0], pathForUSDT[1]),
                amountsForUSDT[0]
            )
        );
        _swap(amountsForUSDT, pathForUSDT, vault);
    }
        ...
        returns (uint256[] memory amounts)
    {
        ...
        assert(
            IWETH(WETH).transfer(
                PancakeLibrary.pairFor(factory, path[0], path[1]),
                amounts[0] + amounts[0].mul(fee) / 1000
            )
        );
        _swap(amounts, path, to);
        // update
        uint256[] memory amountsForUSDT = PancakeLibrary.getAmountsOut(factory, amounts[0].mul(fee) / 1000, pathForUSDT);
        _swap(amountsForUSDT, pathForUSDT, vault);
        // refund dust eth, if any
        if (msg.value > amounts[0] + amounts[0].mul(fee) / 1000)
            TransferHelper.safeTransferETH(
                msg.sender,
                msg.value - amounts[0] - amounts[0].mul(fee) / 1000
            );
    }
    ...
    function _swapForUSDT(uint256 amountForUSDT, address[] memory pathForUSDT)
        internal
        virtual
    {
        if (pathForUSDT[0] == usdt) {
            TransferHelper.safeTransferFrom(
                usdt,
                msg.sender,
                vault,
