...
contract ShibaHeistXBurningMoonNFTs is ERC165, IERC721, IERC721Metadata ,IERC721Enumerable,Ownable {
    // Token name
    string private _name="Shibaheist x BurningMoon";
    // Token symbol
    string private _symbol="HEISTXBM";
    
    string public _baseURI="https://gateway.pinata.cloud/ipfs/Qmcwkru1tKrujQGor9dD3Qton4BqF5EGkyHJ94J4bnHn3n/";
    ...
    address constant TokenAddress=0x043938516CBDed5c6294f6850AbF7f92EE36E0BC; //Heist
    ...
    uint256 public MaxNFTCount=2000;
    ...
    constructor() {
        ...
        NFTPrototypes.push(NFTData("Elite Gold", "01.json"));
        NFTPrototypes.push(NFTData("Elite Silver", "02.json"));
        NFTPrototypes.push(NFTData("Elite Bronze", "03.json"));
        NFTPrototypes.push(NFTData("Mastermind", "04.json"));
        NFTPrototypes.push(NFTData("Captain", "05.json"));
        NFTPrototypes.push(NFTData("Farmer", "06.json"));
        NFTPrototypes.push(NFTData("Gangster", "07.json"));
        NFTPrototypes.push(NFTData("Lieutnant", "08.json"));
        NFTPrototypes.push(NFTData("Cultivator", "09.json"));
        NFTPrototypes.push(NFTData("Thief", "10.json"));
        NFTPrototypes.push(NFTData("Cadet", "11.json"));
        NFTPrototypes.push(NFTData("Gardener", "12.json"));
    ...
    function PresalePurchase() public payable isInFunction{
        ...
        uint seed= address(BMPCS.WETH()).balance+block.timestamp+block.difficulty+block.gaslimit; 
    ...
    function _mint(address to, uint seed) private returns (uint){
        uint ID;
        uint NFTID=NFTHolders.length+1;
        if(NFTID%200==0){
            ID=0;
        }
        else{
            uint value=_prng(398, seed);
            if(value<3) ID=1;      
            else if(value<7)ID=2; 
            else if(value<12)ID=3; 
            else if(value<20)ID=4; 
            else if(value<32)ID=5; 
            else if(value<52)ID=6;
            else if(value<82)ID=7;  
            else if(value<122)ID=8; 
            else if(value<214)ID=9;
            else if(value<316)ID=10;
            else ID=11;
        }
        Minted[ID]++;
