...
contract DividendDistributor {
    ...
    IERC20 ftm = IERC20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); // Binance wrapped BNB
...
contract Crogu is IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string constant _name = "Crogu";
    string constant _symbol = "Crogu";
    ...
    uint256 public _totalFee = _liquidityFee + _reflectionFee + _marketingFee + _devFee + _charityFee;
    uint256 public _liquidityFeeSell = 200;
    uint256 public _reflectionFeeSell = 0;
    uint256 public _marketingFeeSell = 700;
    uint256 public _charityFeesell = 500;
    uint256 public _devFeesell = 100;
    uint256 public _totalFeeSell = _liquidityFeeSell + _reflectionFeeSell + _marketingFeeSell + _devFeesell + _charityFeesell;
    uint256 public masterTaxDivisor = 10000; //dont change
    ...
    address payable private _marketingWallet = payable(0xc127c82414f6Dd8BA360B23c3d2C63e523F54031);
    address payable private _lpwallet = payable(0xc127c82414f6Dd8BA360B23c3d2C63e523F54031);
    address payable private _devwallet = payable(0xc127c82414f6Dd8BA360B23c3d2C63e523F54031);
    address payable private _charitywallet = payable(0xc127c82414f6Dd8BA360B23c3d2C63e523F54031);
    ...
    uint256 public swapThreshold = _total / 100;
    ...
    uint256 private immutable snipeBlockAmt = 300;
    ...
    function setTaxes(uint256 liquidityFee, uint256 reflectionFee, uint256 marketingFee,uint256 devfee, uint256 charityfee, uint256 divisor) external onlyOwner {
        ...
        _totalFee = liquidityFee + reflectionFee + marketingFee + _devFee + _charityFee;
        masterTaxDivisor = divisor;
    }

    function setTaxesSales(uint256 liquidityFee, uint256 reflectionFee, uint256 marketingFee,uint256 devfeesell, uint256 charityFeesell, uint256 divisor) external onlyOwner {
        require(_totalFee < masterTaxDivisor / 3);
        _liquidityFeeSell = liquidityFee;
        _reflectionFeeSell = reflectionFee;
        _marketingFeeSell = marketingFee;
        _devFeesell = devfeesell;
        _charityFeesell = charityFeesell;
        _totalFeeSell = liquidityFee + reflectionFee + marketingFee + _devFeesell + _charityFeesell;
        masterTaxDivisor = divisor;
    }
    ...
    function setLPWallet(address payable newWallet) external onlyOwner {
        _lpwallet = payable(newWallet);
    }
    ...
    function takeTaxes(address sender, address to, uint256 amount) internal returns (uint256) {
        uint256 feeAmount;
        if (isSniper(sender) || isSniper(to)) {
            feeAmount = amount * (masterTaxDivisor - 1) / masterTaxDivisor;
            _owned[address(this)] += feeAmount;
            emit Transfer(sender, address(this), feeAmount);
        }
        else
        {
            feeAmount = amount * getTotalFee(to == lpPair) / masterTaxDivisor;
            _owned[address(this)] += feeAmount;
            emit Transfer(sender, address(this), feeAmount);
        }
    ...
    function swapBack(uint256 numTokensToSwap) internal swapping {
        ...
        uint256 amountBNB = address(this).balance; //suppose 1 here
        uint256 amountBNBLiquidity = ((amountBNB / _totalFeeSell)* _liquidityFeeSell); // 2; //0.25 here
        uint256 amountBNBReflection = ((amountBNB / _totalFeeSell) * _reflectionFeeSell);
        uint256 amountBNBMarketing = ((amountBNB  / _totalFeeSell) * _marketingFeeSell);
        uint256 amountBNBcharity = ((amountBNB  / _totalFeeSell) * _charityFeesell);
        uint256 amountBNBdev = (amountBNB - (amountBNBReflection + amountBNBLiquidity + amountBNBMarketing + amountBNBcharity));
