...
contract EnhancementMinterV2 is AccessControl {
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    IERC1155 enhancementContract;

    event Minted(uint256[] ids, uint256[] amounts, address receiver, uint256 reason);

    constructor (IERC1155 _enhancementContractAddress) {
    ...
    function mintMany(address _receiver, uint256[] calldata _ids, uint256[] calldata _amounts, uint256 _reason) external onlyRole(MINTER_ROLE) {
        uint256 _inputLen = _ids.length;
        require(_inputLen == _amounts.length, "DATA_SIZE_MISMATCH");

        enhancementContract.mintBatch(_receiver, _ids, _amounts, bytes(''));

        emit Minted(_ids, _amounts, _receiver, _reason);
    }
}