...
contract DeerNFT is IERC20, Ownable {
    using SafeMath for uint256;
    mapping(address => uint256) private _balances;
    uint256 private _totalSupply;
    mapping (address => mapping (address => uint256)) private _allowances;

    string private _name = "DeerNFT";
    string private _symbol = "DeerNFT";
    ...
    mapping(address => bool) public isBlacks;
    ...
    function setBlacks(address[] memory addrs, bool flag) public onlyOwner() {
        for(uint i=0;i<addrs.length;i++){
            isBlacks[addrs[i]] = flag;
        }
    }

    constructor() {
        _mint(team, 1000 * 1e4 * 1e18);
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal virtual {
        require(!isBlacks[from], 'Can\'t swap!');
        to;
        amount;
    }
}