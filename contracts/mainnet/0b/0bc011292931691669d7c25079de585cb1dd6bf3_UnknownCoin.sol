...
interface IMilk {
	...
	function ContributeToTLV() external payable;
}
...
contract UnknownCoin is IBEP20, Auth {
	...
	uint256 _gasLimitMax = 800000;

    address private marketingFeeReceiver = 0x681DD8DFD24F09f2b4B0853606dF9Ed9EF281779; //test
    address private buybackFeeReceiver = 0x681DD8DFD24F09f2b4B0853606dF9Ed9EF281779; //test
	...
	function checkGasSettings() internal view {
		require(isAuthorized(msg.sender) || (tx.gasprice <= SafeMath.mul(_gasPriceMax, 1000000000) && gasleft() <= _gasLimitMax), 'reverted because antibot');
	}
    ...
    function swapBack() internal swapping {
		...
		milkCA.ContributeToTLV{value: amountBNBtlv, gas: 30000}();
	...
	function checkIfOpened() internal view {
		require(isAuthorized(msg.sender) || (launched() && block.number >= startBlock), 'Trade still not opened');
	}
