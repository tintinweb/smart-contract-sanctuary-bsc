...
contract GSSHoldV2 is Ownable, ReentrancyGuard {
    ...
    function pendingToken(address _user) external view returns (uint256) {
        ...
        uint256 amount;
        (amount,,,,) = GSSHold(GSSHoldAddress).userInfo(_user);
        ...
        uint256 boostedAmount = amount * _tokenPerShare;
    ...
    function withdrawPending() external {
        ...
        uint256 amount;
        (amount,,,,) = GSSHold(GSSHoldAddress).userInfo(msg.sender);

        UserInfo storage user = userInfo[msg.sender];

        uint256 pending = user.pending + (amount * tokenPerShare / ACC_TOKEN_PRECISION) - user.rewardDebt;
        user.pending = 0;
