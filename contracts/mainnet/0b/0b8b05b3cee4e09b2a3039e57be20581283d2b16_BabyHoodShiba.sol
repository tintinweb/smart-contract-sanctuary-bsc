...
contract BabyHoodShiba is Context, ERC20, Ownable {
    ...
    constructor() ERC20 ("Baby HoodShiba", "BHS") {
        ...
        maxBuyPercent = 200; //1% max buy
        maxSellPercent = 100; //1% max sell

        TotalSupply = 1000000000000000; //Set the total token supply

        sendTokens = false; //False to send ETH to marketing, true to send tokens to marketing

        liquidityWallet = owner();
        marketingWallet = address(payable(0xB85bBAdA1a4E2dB14966D3f4927CC61ad6a13127));
        developmentWallet1 = address(payable(0xC6EeE21b5474766F9671C17eE8435Bc02CaDAC1B));
        developmentWallet2 = address(payable(0x1eb5F14c1f62603C841910031f0A9dD8db47a43c));
        developmentWallet3 = address(payable(0x8DB2da1Fd6AdC5a92e4e27aDFAaca102a1a28db0));
        ...
        _isExcludedFromContractBuyingLimit[address(uniswapV2Router)] = true;
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Baby HoodShiba: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Baby HoodShiba: Account is already the value of 'excluded'");
    ...
    function updateIsElon(address account, bool elon) public onlyOwner {
        require(_isElon[account] != elon, "Baby HoodShiba: Account is already the value of 'elon'");
    ...
    function addToBlacklist(address account, bool blacklisted) public onlyOwner {
        require(_isBlacklisted[account] != blacklisted, "Baby HoodShiba: Account is already the value of 'blacklisted'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Baby HoodShiba: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Baby HoodShiba: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "Baby HoodShiba: The liquidity wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "Baby HoodShiba: The marketing wallet is already this address");
    ...
    function _transfer(
		...
		if(canSwap && !swapping && automatedMarketMakerPairs[to] && !_isExcludedFromFees[to] && !_isExcludedFromFees[from]) {
            ...
            uint256 developmentSplit = developmentAmount.div(3);


            (bool success, ) = developmentWallet1.call{value: developmentSplit}("");
