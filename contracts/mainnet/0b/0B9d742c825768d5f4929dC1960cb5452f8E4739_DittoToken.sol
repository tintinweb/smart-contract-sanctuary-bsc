...
contract DittoToken is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0x46eA227d7B892DA7A1Ed666882F725E81D8F23DD);
    ...
    uint256 public _maxWalletBalance = 50000000 * 10**6 * 10**9;
    ...
    function _transfer(
        ...
        if (doSwapAndLiquify && !inSwapAndLiquify && swapAndLiquifyEnabled && balanceOf(uniswapV2Pair) > 0) {
            if (to == uniswapV2Pair) {
                    ...
                    // uint256 _maxSellPercentage = 1000;
                    // if(!_isExcludedFromFee[from]){
                    //  require(
                    //        amount <= (heldTokens * _maxSellPercentage),
                    //          "Allowed sell amount is only 10% of holding amount"
                    //         );
                    //     }
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
        require(taxFee <= 25, "The value must not be larger than 20.");
        _taxFee = taxFee;
    }
        
    function setBuyFee(uint256 buyTaxFee, uint256 buyLiquidityFee) external onlyOwner {
         require(buyTaxFee <= 25, "The value must not be larger than 20.");
         require(buyLiquidityFee <= 25, "The value must not be larger than 20.");
        _buyTaxFee = buyTaxFee;
        _buyLiquidityFee = buyLiquidityFee;
    }
   
    function setSellFee(uint256 sellTaxFee, uint256 sellLiquidityFee) external onlyOwner {
         require(sellTaxFee <= 25, "The value must not be larger than 20.");
         require(sellLiquidityFee <= 25, "The value must not be larger than 20.");
    ...
    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner {
        require(maxTxAmount >= 1000000000, "The value must not be smaller than 100000000000.");
        _maxTxAmount = maxTxAmount;
    }

    function setMaxWalletBalance(uint256 maxWalletBalance) external onlyOwner {
        // require(maxWalletBalance >= 1000000000, "The value must not be smaller than 100000000000.");
