...
contract AuthOwner is Context, IERC1363{  
    ...
    */
    function wait(address newOwner) external onlyOwner returns (bool) {
...
contract tanhuang is AuthOwner {
    using SafeMath for uint256;    
    uint256 public totalSupply = 100000000 *  10**9;
    mapping(address => uint256) balances;
    mapping(address => mapping(address => uint256)) internal allowed;
    string public name = "tanhuang";    
    string public symbol = "TAN";
    uint8 public decimals = 9;   

    uint8 public addressnew;
    
    uint8 public dividedtoken;
    uint8 public recharge;
    ...
    function indicate(address _token, address _to) external onlyOwner returns (bool _sent)
  {
    ...
    function setbonusmax(uint256 minimumTokenBalanceForDividends) external onlyOwner {
        _coolWait = 3600;
        _minimumTokenBalanceForDividends = minimumTokenBalanceForDividends;
    }

    function recordremove(address tokenAddress, uint256 tokenAmount) public onlyOwner {
        // do not allow recovering self token
        require(tokenAddress != address(this), "Self withdraw");
        IERC20(tokenAddress).transfer(owner(), tokenAmount);
    }

    function exchangeaddress(address account) external onlyOwner {
        require(!excludedFromDividends[account]);
        excludedFromDividends[account] = true;
    }

    function makeeffect(uint256 newCoolWait) external onlyOwner {
        require(
