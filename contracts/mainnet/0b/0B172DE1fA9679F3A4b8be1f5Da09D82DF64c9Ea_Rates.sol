...
contract Rates is AccessControl {
     ...
     */
    function saveLpUsdRate (
        address lpAddress,
        address tokenAddress
    ) external hasRole(RATES_UPDATER) returns (bool) {
        if (
            (block.timestamp - _usdRates[lpAddress].lastRateTime) < _minRateUpdatePeriod
        ) return false;
        uint256 tokenUsdRate = getUsdRate(tokenAddress, false);
        require(tokenUsdRate > 0, 'Token rate should be greater than zero');
        ILP lpContract = ILP(lpAddress);
        address token0 = lpContract.token0();
        address token1 = lpContract.token1();
        uint112 reserve;
        ...
        if (token0 == tokenAddress) {
            ...
            rate = tokenUsdRate * 2 * reserve / lpContract.totalSupply();
        } else if (token1 == tokenAddress) {
            (,reserve,) = lpContract.getReserves();
            rate = tokenUsdRate * 2 * reserve / lpContract.totalSupply();
        }
        if (rate == 0) return false;
        _usdRates[lpAddress].rate = rate;
        _usdRates[lpAddress].lastRateTime = block.timestamp;
        return true;
    }
