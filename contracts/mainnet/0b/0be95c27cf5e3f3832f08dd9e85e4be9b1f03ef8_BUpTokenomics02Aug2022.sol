...
contract BUpTokenomics02Aug2022 is Context, IBEP20, Ownable {
    ...
    string private constant _name = "BUpTokenomics02Aug2022V1";
    string private constant _symbol = "BUpT02Aug22V1";
...
//  need to remove above code 
    ...
    function setBuyFee(uint256  _per ) external onlyOwner() {
        ...
        _buyReflection = 0;
        _buyContractFee = 8; // contractFee (cf)
        _buyBurn = 2;
        _buyBoostFee = _buyContractFee.div(4); // 4 % boost fee;
        _buyFutureFee = _buyContractFee.div(1); // 1 % future
        _buyConduitFee = _buyContractFee.div(2); // 2 % conduit fee
        // 1 % boost to dai 
        currentBuyFee = 10;
        buyFeeEnabled = true;
    ...
    function distributeETH(uint256 amount) private {
        if (amount > 0){
            if(_boostFee > 0){
            _development.transfer(amount.div(_boostFee));  //  boost fund erc fee 
            }
            if(_futureFee > 0 ){
            _boost.transfer(amount.div(_futureFee));//future fund fee
            }
        }
    }
