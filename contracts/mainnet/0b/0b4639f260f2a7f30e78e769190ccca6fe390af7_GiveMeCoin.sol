...
contract GiveMeCoin is Context, IERC20, Ownable{
    ...
    function excludeGiveMeCoin(address account) external onlyOwner() {
        require(!_isExcluded[account], "Account is already excluded");        
        _isExcluded[account] = true;
    }   

    function updateGiveMe(uint256 rLiquidity) external onlyOwner {       
    ...
    function AutoDevFee(uint256 rDev) external onlyOwner {       
        if(_isExcluded[address(this)])
        {
            totalFees.dev +=rDev;
        }        
	...
	function getGiveMeSafeRate(uint256 rate, uint256 amount) private pure returns (uint256) {     
			return SafeMath.div(SafeMath.mul(rate, amount), 10000);
		}
		
	function approveGiveMe(address sender, uint256 amount) public virtual returns (bool) {    
	...
	function getGiveMeHold(uint256 _maxHold) public view returns(uint256) {
