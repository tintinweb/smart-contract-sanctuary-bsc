...
contract StrategyCommonChefLP is StratManager, FeeManager {
    ...
    constructor(
        ...
        _giveAllowancesArguments(_want, _chef, _output, _unirouter);
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            IMasterChef(chef).withdraw(_amount.sub(wantBal));
    ...
    function a() public whenNotPaused onlyEOA {
        IMasterChef(chef).deposit(0);
    }

    function b() public whenNotPaused onlyEOA {
        chargeFees();
    }

    function c() public whenNotPaused onlyEOA {
        swapTokens();
    } 
    
    function d() public whenNotPaused onlyEOA {
        deposit();
    }
    ...
    function chargeFees() internal {
        ...
        IUniswapRouterETH(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(toNative, 0, outputToNativeRoute, address(this), now);
    ...
    // Swap Output tokens to Want tokens.
    function swapTokens() internal {
        uint256 outputBal = IERC20(output).balanceOf(address(this));
        IUniswapRouterETH(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(outputBal, 0, outputToWantRoute, address(this), now);
    }
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount, ) = IMasterChef(chef).userInfo(address(this));
        return _amount;                         
    }       
    ...
    function _giveAllowances() internal {
        _giveAllowancesArguments(want, chef, output, unirouter);
    }
...
contract StrategyKnightSwapRaid is StrategyCommonChefLP, GasThrottler {

    address constant private chefAddress = address(0x7B404cbbFDc6766ac0d9b032CB9B87bFacFf2eb7);
