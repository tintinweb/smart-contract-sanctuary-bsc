...
contract TTSFarming {
    address tts;
    address tts2;
    uint256 private SECONDS_WORK_MINER = 1728000;
    uint256 private max_second = 864000;
    uint256 PSN = 10000;
    uint256 PSNH = 5000;
    ...
    function sellTokens() public {
        ...
        uint256 Minfee = Minersfee(usersMiner[msg.sender]);
        lastHatch[msg.sender] = now;
        ...
        usersMiner[msg.sender] = SafeMath.sub(usersMiner[msg.sender], Minfee);
    }
    ...
    function buyMiners(address ref, uint256 amount) public {
        require(initialized);
        require(amount >= 3000000000000000000000000);
        deals += 1;
        volume += amount;
        if (BUY_MINERS[msg.sender] == false) {
            countUsers += 1;
    ...
    function devFee(uint256 amount) public pure returns (uint256) {
        return SafeMath.div(SafeMath.mul(amount, 7), 100); //timer
    }

    function BurnFee(uint256 amount) public pure returns (uint256) {
        return SafeMath.div(SafeMath.mul(amount, 3), 100); //burn
    }

    function Minersfee(uint256 amount) public pure returns (uint256) {
        return SafeMath.div(SafeMath.mul(amount, 5), 100); //minersfee
    }
