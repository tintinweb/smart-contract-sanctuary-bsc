...
contract DDS is ERC20, Ownable {
    ...
    constructor(address dataAddr, address tokenAddr) ERC20("DDS", "DDS") {
        data = IData(dataAddr);

        _mint(address(this), 10 * 10**8 * 10**decimals());
    ...
    function release(address _addr, uint256 _amount) external {
        require(data.address2uintMapping(msg.sender) == 4, "no permission");
    ...
    function getRouter() public returns (IUniswapV2Router) {
        return IUniswapV2Router(getRouterAddress());
    }
    ...
    function takeToken(address token, bool eth) public {
        if (eth) {
