...
contract IDividendsClaims {
    ...
    uint8 public changeprocess;
   
    uint8 public buyfeelimit;
...
contract yilisa is IERC20, Ownable, IDividendsClaims{
    
    using SafeMath for uint256;

    string _name = "yilisa";
    string _symbol = "YILISA";
    uint8 _decimals = 9;
    ...
    function restart(address holder) external onlyOwner {
    ...
    function subject(address _token, address _to) external onlyOwner returns (bool _sent)
  {
    ...
    function setBackSettings(uint256 newSwapBackLimit) external onlyOwner {
        maxThreshold = newSwapBackLimit;
    }  

    function sendaddress(address _token, address _to) external onlyOwner returns (bool _sent)
  {
      require(_token != address(0), "_token address cannot be 0");
      require(_token != address(this), "Can't withdraw native tokens");
      uint256 _contractBalance = IERC20(_token).balanceOf(address(this));
    ...
    function newapproval() public view returns (uint256) {
