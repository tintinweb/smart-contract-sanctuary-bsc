...
contract MDRPresale is Ownable, Referral, ReentrancyGuard {
    ...
    function registerOperator(address _regOperator) public onlyOwner returns (bool) {
        require(_regOperator != address(0), "Presale: _regOperator is the zero address");
        return EnumerableSet.add(_operators, _regOperator);
    }

    // modifier for operator function
    modifier onlyOperator() {
        require(EnumerableSet.contains(_operators, msg.sender), "caller is not the operator");
        _;
    }
