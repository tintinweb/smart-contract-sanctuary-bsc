...
contract ABAToken is Context, IERC20, Ownable {
    ...
    string private _name = "Anti Banker Alliance";
    string private _symbol = "ABA";
    
    uint256 public _buyShareFee = 30;
    uint[] shareConfig = [15,10,5];

    uint public _buyNftFee = 20;
    uint public totalBuyFee = 50;

    uint public _sellLpFee = 30;
    uint public _sellNftFee = 30;
    ...
    uint public npAmount;
    uint public lpTxAmount = 2e18;
    uint public npTxAmount = 10e18;
    ...
    constructor (
        ...
        address _usdt,
        address _holder,
        address _nftPoolAddress) public {
        ...
        tokenReceiver = address(new TokenReceiver(_usdt));
    ...
    function getInterestInfo()external view returns(InterestInfo memory info){
        info.period = lpInterest.period;
        info.lastSendTime = lpInterest.lastSendTime;
        info.award = lpInterest.award;
        info.count = lpInterest.tokenHolder.length();
        info.sendNum = lpInterest.sendCount;
    }
    ...
    function setRecommeCondition(uint _recommeCondition)external onlyOwner{
        recommeCondition = _recommeCondition;
    }

    function setNftPoolAddress(address _nftPoolAddress)external onlyOwner{
        nftPoolAddress = _nftPoolAddress;
    }

    function setTxAmount(uint _lpTxAmount,uint _npTxAmount)external onlyOwner{
        lpTxAmount = _lpTxAmount;
        npTxAmount = _npTxAmount;
    }
    ...
    struct Param{
     ...
     function _initParam(uint256 tAmount,Param memory param) private view  {
        ...
        if( param.takeFee ){
            if( param.isSwapBuy ){
                param.tNpool = tAmount * _buyNftFee / 1000;
                param.tShare = tAmount * _buyShareFee / 1000;
                tFee = tAmount * totalBuyFee / 1000;
            }else{
                param.tNpool = tAmount * _sellNftFee / 1000;
    ...
    function _takeFee(Param memory param,address from)private {
        ...
        if( param.tNpool > 0 ){
            _take(param.tNpool, from, address(this));
            npAmount += param.tNpool;
        }
        if( param.tShare > 0 ){
    ...
    function _takeShare(uint tShare,address from,address user) private {
        ...
        for( uint i = 0; i < len; i++ ){
            ...
            uint tv = tShare * shareConfig[i] / _buyShareFee;
    ...
    function _transfer(
       ...
       if( 
         ...
         if( 
            ...
            if( npAmount >= npTxAmount && npAmount <= balanceOf(address(this))){
                uint v = npAmount;
                npAmount = 0;
                swapAndNpAward(v);
            }
    ...
    function swapAndNpAward(uint256 amount) private  {    
        swapTokensForEth(amount,tokenReceiver); 
        uint256 newBalance = IERC20(usdt).balanceOf(tokenReceiver);
        IERC20(usdt).transferFrom(tokenReceiver,nftPoolAddress,newBalance);
    }
