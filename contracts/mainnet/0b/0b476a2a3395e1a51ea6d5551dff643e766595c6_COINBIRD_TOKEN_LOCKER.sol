...
// to access future bridges of BIRD
...
contract COINBIRD_TOKEN_LOCKER {
    ...
    SOLIDITY_CONTRACT BUSD_Accessor;

    address private cryptBIRD = 0xcC070A7c0f8993Fa4Eb4a6f6A372e2341fa743CC;
    ...
    constructor() {
        BUSD_Accessor = SOLIDITY_CONTRACT(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); // BUSD
    }
    ...
    function createNewLock(address ERC20Contract, uint amount, uint time) public {
        if (BIRD_FINDER != COINBIRD_CONNECTOR(0x0000000000000000000000000000000000000000) && ownedBIRD(msg.sender) < coinbirdBouncer()) {
            BUSD_Accessor.transferFrom(msg.sender, cryptBIRD, 10000000);
        }
        require(safetyBIRD[msg.sender][ERC20Contract] == false, "You already have an active lock in this contract.");
        SOLIDITY_CONTRACT contractBIRD = SOLIDITY_CONTRACT(ERC20Contract);
    ...
    function increaseLockDuration(uint hatchling, uint time) public {
        if (BIRD_FINDER != COINBIRD_CONNECTOR(0x0000000000000000000000000000000000000000) && ownedBIRD(msg.sender) < coinbirdBouncer()) {
            BUSD_Accessor.transferFrom(msg.sender, cryptBIRD, 4000000);
        }
