...
contract MoDApproveProxy is AccessControl {
    ...
    constructor (IResources _resources, IItems _items, IPlanetPlot _planetPlot, IERC20 _DAR, address _default_admin) {
    ...
    function DAR_transferFrom(address _sender, address _recipient, uint256 _amount) external onlyRole(HANDLER) {
        DAR.transferFrom(_sender, _recipient, _amount);
    }

    function item_safeTransferFrom(address _from, address _to, uint _tokenId, bytes memory _data) external onlyRole(HANDLER) {
        items.safeTransferFrom(_from, _to, _tokenId, _data);
    }

    function resources_safeTransferFrom(address _from, address _to, uint _id, uint _amount, bytes memory _data) external onlyRole(HANDLER) {
        resources.safeTransferFrom(_from, _to, _id, _amount, _data);
    }

    function resources_safeBatchTransferFrom(address _from, address _to, uint[] calldata _ids, uint[] calldata _amounts, bytes memory _data) external onlyRole(HANDLER) {
        resources.safeBatchTransferFrom(_from, _to, _ids, _amounts, _data);
    }

    function planetPlot_safeTransferFrom(address _from, address _to, uint _tokenId, bytes memory _data) external onlyRole(HANDLER) {
        planetPlot.safeTransferFrom(_from, _to, _tokenId, _data);
    }

    function item_burn(address _from, uint256[] memory _burnItemIds) external onlyRole(HANDLER) returns (bool){
        return(items.burn(_from, _burnItemIds));
    }

    function item_mint(address _to, uint256[] memory _mintTypes) external onlyRole(HANDLER) returns (bool){
        return(items.mint(_to, _mintTypes));
    }

    function item_burnAndMint(address _fromTo, uint256[] memory _mintTypes, uint256[] memory _burnItemIds) external onlyRole(HANDLER) returns (bool){
        return(items.burnAndMint(_fromTo, _mintTypes, _burnItemIds));
    }

    function resources_burnBatch(address _from, uint256[] memory _ids, uint256[] memory _amounts) external onlyRole(HANDLER) returns (bool) {
        return(resources.burnBatch(_from, _ids, _amounts));
    }

    function resources_mintBatch(address _to, uint256[] memory _ids, uint256[] memory _amounts) external onlyRole(HANDLER) returns (bool) {
        return(resources.mintBatch(_to, _ids, _amounts));
    }
}
