...
// MiningV2合约
contract MiningV4 is Ownable, ReentrancyGuard {
    ...
    // token地址
    address public ete;
    address public eteUsdtLp;   // ete-usdt-lp，用于计算ete价值和分红。
    address public depositLp;   // 质押的LP。必须是ete-其它token-lp, 本合约使用的是ete-weth-lp。
    address public eteDIDNFT;
    address public team;
    address public leader;
    address public linkedin;
    // 每个地址可质押的总量，总池子的万分之200
    uint256 public canRatio = 200;
    ...
    constructor(
        ...
        oneIssueTime = _dayTime * 5;

        ete = ete_;
        eteUsdtLp = eteUsdtLp_;
        depositLp = depositLp_;
        ...
        issue[turn] = 1;
        // 开始第一轮第一期
        _startIssue(turn, issue[turn], 5000*(10**18));
    }
    ...
    // 设置depositLp
    function setDepositLp(address _depositLp) public onlyOwner {
        require(_depositLp != address(0), "0 address error");
        depositLp = _depositLp;
    }
    // 设置eteDIDNFT
    ...
    // 计算质押币的价格，deposit-LP价值的U。。(1e10)个Lp的价值的U。depostLP必须是ete-其它token-lp。
    function getLpPrice() public view returns(uint256) {
        // 开始计算
        address token0 = IUniswapV2Pair(depositLp).token0();
        uint256 totalSupply = IUniswapV2Pair(depositLp).totalSupply();
        (uint256 reserve0, uint256 reserve1, ) = IUniswapV2Pair(depositLp).getReserves();
        uint256 eteAmount = token0 == ete ? reserve0 : reserve1;
        // 计算ete储备量价值的u
        uint256 eteAmountPriceUsdt = eteAmount.mul(getETEPrice()).div(1e18);
        // ete储备量价值多少个u，就说明其它token也价值这么多u，所以乘以就是全部lp价值的u，然后除以全部的lp就是每个lp价值的u。
        uint256 price10 = eteAmountPriceUsdt.mul(2).mul((1e10)).div(totalSupply);
        require(price10 > 0, "lp price error");
        return price10;
    }

    // 计算ETE价值多少个USDT。(1e18)个ETE等于多少个USDT
    function getETEPrice() public view returns(uint256) {
        ...
        return reserveUsdt.mul((1e18)).div(reserveEte);
    }
    ...
    function userDeposit(uint256 lpAmount) public nonReentrant {
        ...
        // 转账
        TransferHelper.safeTransferFrom(depositLp, _user, address(this), lpAmount);
        // 触发质押事件
    ...
    function userTake(bytes32 order) public nonReentrant {
        ...
        if(_orderIssueMsg.status == 4) {
            // 提取本金
            TransferHelper.safeTransfer(depositLp, msg.sender, _orderMsg.count);
