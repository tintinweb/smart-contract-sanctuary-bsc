...
import "./IERC20.sol";
import "./BigpadLaunchpad.sol";
import "./BigpadInfo.sol";
import "./BigpadLaunchpadLiquidityLock.sol";

interface IUniswapV2Factory {
    function getPair(address tokenA, address tokenB) external view returns (address pair);
}

contract BigpadFactory {
    using SafeMath for uint256;

    event PresaleCreated(bytes32 title, uint256 bigpadId, address creator);

    IUniswapV2Factory private constant uniswapFactory =
        IUniswapV2Factory(address(0xcA143Ce32Fe78f1f7019d7d551a6402fC5350c73));
    address private constant wethAddress = address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);

    BigpadInfo public immutable bigpad;

    constructor(address _bigpadInfoAddress) public {
        bigpad = BigpadInfo(_bigpadInfoAddress);
    }
    ...
    function createPresale(
        ...
        BigpadLaunchpad presale = new BigpadLaunchpad(address(this), bigpad.owner());
        ...
        BigpadLaunchpadLiquidityLock liquidityLock = new BigpadLaunchpadLiquidityLock(
                IERC20(pairAddress),
                msg.sender,
                _uniInfo.liquidityAddingTime + (_uniInfo.lpTokensLockDurationInDays * 1 days)
            );

        uint256 bigpadId = bigpad.addPresaleAddress(address(presale));
        presale.setbigpadInfo(address(liquidityLock), bigpad.getDevFeePercentage(), bigpadId);

        emit PresaleCreated(_stringInfo.saleTitle, bigpadId, msg.sender);
    }
}