...
contract TokenTemplate is ERC20,Ownable {
    ...
    uint public sell = 4; //2% 交易费卖出2% 50%销毁 50%回到cale-lp产矿
    ...
    function getPool() external returns(address){
        ...
        //获取流动性池中两种货币的量
        //uint balanceA=IPancakeFactory(A).balanceOf(pair);
        //Uint balanceB=IPancakeFactory(B).balanceOf(pair);
        //计算A对B的价格
        //uint price=balanceA.div(balanceB);
        //address pair=IPancakeFactory(pancakeToken).getPair(A,B);
