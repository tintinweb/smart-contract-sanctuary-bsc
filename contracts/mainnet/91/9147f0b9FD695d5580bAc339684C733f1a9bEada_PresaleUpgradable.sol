...
abstract contract Referral is Initializable, OwnableUpgradeable {
    ...
    function addReferrer(address payable referrer) internal returns (bool) {
        ...
        parentAccount.referee.push(msg.sender);
        ...
        returns (uint256)
    {
        ...
        for (uint i; i < levelRate.length; i++) {
            ...
            if (
                ...
                uint c = value.mul(referralBonus).div(LevelDecimals);
...
contract PresaleUpgradable is Referral, UUPSUpgradeable, PausableUpgradeable {
    ...
    function AddReferrerAdmin(address _referree, address payable referrer)
        external
        onlyOwner
        returns (bool)
    {
