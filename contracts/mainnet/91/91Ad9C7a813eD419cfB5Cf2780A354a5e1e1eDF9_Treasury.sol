    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize(        
        ...
        require(_eagle != address(0),"initialize: Invalid address");
    ...
    function validatePayout() external{
        uint256 balance = IERC20Upgradeable(eagle).balanceOf(address(this));
        uint256[] memory getEagleAmountOneETH = IUniswapV2Router(router).getAmountsOut(totalETH, path);
        uint256 totalAmount = getEagleAmountOneETH[1].mul(155).div(10);
        if(balance >= totalAmount && IGovernance(dao).nextInvesteeFund()<IGovernance(dao).nextInvestee()){
            fundInvestee(totalAmount,getEagleAmountOneETH[1].mul(25).div(10));
        }
    }
    ...
    function fundInvestee(uint256 totalAmount,uint burnAmount) internal nonReentrant{
        ...
        IERC20Upgradeable(eagle).transfer(DEAD_ADDRESS,burnAmount);
        IERC20Upgradeable(eagle).transfer(investee,totalAmount.sub(burnAmount));
    }
