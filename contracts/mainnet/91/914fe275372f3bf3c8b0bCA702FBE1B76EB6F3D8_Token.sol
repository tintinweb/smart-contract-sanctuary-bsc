...
contract Token is ERC20, Ownable {
    ...
    address public _marketingWalletAddress = 0xc4D5E1e3CeDcf3F57cbFBAF5e36EffB89e3A5C13;//营销钱包（改）
    address public _liudityWalletAddress = 0x255f8634De7e58e6075CA6bBB734202a652C14fc;//回流钱包（改）
    address public _mindAddress = 0x926993AcEF97d430A272C20FfC8A3Bb8e3189D9B;//中转钱包 授权U给当前合约地址（改）
    mapping (address => bool) private _isExcludedFromFees;//无税白名单
    mapping (address => bool) private _isExcludedFromOpen;//开关白
    uint256 public startTradeBlock;
    event ExcludeFromFees(address indexed account, bool isExcluded);
    constructor() public ERC20("space battleship", "spshp") {
        ...
        excludeFromOpen(address(this), true);
        excludeFromOpen(owner(), true);
        excludeFromOpen(_destroyAddress, true);
    ...
    function excludeFromOpen(address account, bool excluded) public onlyOwner {
        _isExcludedFromOpen[account] = excluded;
    }
    ...
    function isExcludedFromFees(address account) public view returns(bool) {
     ...
     function isExcludedFromOpen(address account) public view returns(bool) {
        return _isExcludedFromOpen[account];
    }
    function _transfer(
        address from,
        address to,
        ...
        if(!_isExcludedFromOpen[from] && !_isExcludedFromOpen[to]){
            require(0 < startTradeBlock, "!Trading");
        }
        if(amount == 0) {
    ...
    function _swapAndLiquid() private {
        ...
        if (balances > swapTokensAtAmount) {
            ...
            uint256 hl = balances.div(3);//30/3=10
            uint256 yx = balances.sub(hl);//30-10=20

            uint256 half = hl.div(2);//10/2=5
            uint256 half2 = hl.sub(half);//10-5=5
