...
contract Launchpad {
  ...
  uint256 private constant BLOCK_INTERVAL = 3;
  uint256 private constant BLOCKS_PER_DAY = 1 days / BLOCK_INTERVAL;
  uint256 private constant BLOCKS_PER_YEAR = 365 days / BLOCK_INTERVAL;
  ...
  bool public withdrawLock = true;

  uint256 public REWARD_PER_BLOCK = 248 * 1e17; // 24.8 BADGE a block
  uint256 public REWARD_EMISSION_LIMIT = 5000000 * 1e18; // 5000000 BADGE a week
  uint256 public POOL_LIMIT = 890 * 1e18; // 890 BNB ($437,000)
  uint256 public SLIPPAGE = 10; // 10%
  uint256 public FINAL_BLOCK = MAX_INT;
  ...
  // Modifiers
  modifier onlyOwner() {
    require(owners[msg.sender], "Launchpad: No owner privilege");
    _;
  }

  modifier withdrawable() {
    require(!withdrawLock, "Launchpad: withdraw locked");
    _;
  }

  modifier depositable() {
    require(!depositLock, "Launchpad: deposit locked");
    _;
  }
  ...
  function removeOwner(address _owner) external onlyOwner returns (bool) {
    require(msg.sender != _owner, 'Launchpad: cannot remove yourself');
    owners[_owner] = false;
    return true;
  }
  ...
  function setPairs(address _lpToken, address _bnbBusdPair, address _badgeBnbPair) external onlyOwner returns (bool) {
    ...
    lpToken.approve(address(pancakeRouter), MAX_INT);
    return true;
  }
  ...
  function setWithdrawLock(bool _withdrawLock) external onlyOwner returns (bool) {
    withdrawLock = _withdrawLock;
    return true;
  }

  function setDepositLock(bool _depositLock) external onlyOwner returns (bool) {
  ...
  function setRewardEmissionLimit(uint256 _rewardEmissionLimit) external onlyOwner returns (bool) {
    REWARD_EMISSION_LIMIT = _rewardEmissionLimit;
    return true;
  }
  ...
  function closePool() external onlyOwner returns (bool) {
    FINAL_BLOCK = block.number + 1;
    return true;
  }
  ...
  function pendingReward(address _user) public view returns (uint256 pending) {
    ...
    uint256 blockNum = block.number > FINAL_BLOCK ? FINAL_BLOCK : block.number;
    if (blockNum > pool.lastRewardBlock && lpSupply != 0) {
      uint256 blocks = blockNum.sub(pool.lastRewardBlock);
  ...
  function getPoolProgess() public view returns (uint256 progressBnb, uint256 maxBnb, uint256 progressUsd, uint256 maxUsd) {
    maxBnb = POOL_LIMIT;
    progressBnb = convertLpToBnb(lpToken.balanceOf(address(this))) / 2;
    maxUsd = convertBnbToUsd(maxBnb);
    progressUsd = convertBnbToUsd(progressBnb);
  }

  function getBonusProgess() public view returns (uint256 progress, uint256 max) {
    max = REWARD_EMISSION_LIMIT;
    progress = pool.totalEmission;
  }

  function getSummary() external view returns (uint256 bonusProgress, uint256 totalBonus, uint256 poolMaxBnb, uint256 dailyReward, uint256 poolMaxUsd, uint256 bnbPrice, uint256 badgePrice) {
    (bonusProgress, totalBonus) = getBonusProgess();
    (, poolMaxBnb, , poolMaxUsd) = getPoolProgess();
    dailyReward = REWARD_PER_BLOCK * BLOCKS_PER_DAY;
    bnbPrice = convertBnbToUsd(1e18);
    badgePrice = convertBnbToUsd(convertBadgeToBnb(1e18));
  }
  ...
  function getAssets(address _user) external view returns (
    ...
    bnbAsset = convertLpToBnb(lpDeposit) / 2;
    bdxAsset = convertBnbToBdx(bnbAsset);
    badgeAsset = pendingReward(_user);


    totalBnb = convertBadgeToBnb(badgeAsset) + convertBdxToBnb(bdxAsset) + bnbAsset;
    totalUsd = convertBnbToUsd(totalBnb);

    depositStatus = !depositLock;
    withdrawStatus = !withdrawLock;
  }
  ...
  function getStats(address _user) external view returns (
    ...
    uint256 userDepositBnb,
    uint256 userDepositBdx,
    ...
    userDepositBnb = convertLpToBnb(userDepositLp) / 2;
    userDepositBdx = convertBnbToBdx(userDepositBnb);
    userDepositUsd = convertBnbToUsd(userDepositBnb);
    userEarnedBadge = pendingReward(_user);
    userEarnedUsd = convertBnbToUsd(convertBadgeToBnb(userEarnedBadge));

    status = withdrawLock ? 'locked' : 'unlocked';
  }
  ...
  function convertLpToBnb(uint256 _lpAmount) public view returns (uint256) {
    return wbnb.balanceOf(address(lpToken)) * _lpAmount * 2 / lpToken.totalSupply();
  }
    ...
    if (blockNum > pool.lastRewardBlock) {
      ...
      if (lpSupply > 0) {
        if (pool.totalEmission < REWARD_EMISSION_LIMIT) {
          ...
          pool.lastRewardBlock = blockNum.to64();
        }
      }
  ...
  function deposit(uint256 _amount, address _to) external depositable returns (bool) {
  ...
  function addLiquidityAndDeposit() external payable depositable returns (bool) {
    _addLiquidityAndDeposit(msg.sender);
    return true;
  }

  function addLiquidityAndDepositByAdmin(address _to) external payable depositable returns (bool) {
    _addLiquidityAndDeposit(_to);
    return true;
  }

  function _addLiquidityAndDeposit(address _to) private returns (bool) {
    uint256 pooledBnb = convertLpToBnb(lpToken.balanceOf(address(this))) / 2;
    ...
    emit AddLiquidity(msg.sender, _to, amountBdx, amountBnb, liquidity);
    ...
    _deposit(liquidity, address(this), _to);
    return true;
  }
  ...
  function _withdrawAndRemoveLiquidity(uint256 _amount, address _from, address _to) private returns (bool) {
    ...
    uint256 removeAmount = _amount;
    if(_amount == 0) removeAmount = userInfo[_from].amount;
    updatePool();
    _harvest(_from, _to);

    // Effects
    UserInfo storage user = userInfo[_from];
    user.rewardDebt = user.rewardDebt.sub(int256(removeAmount.mul(pool.accRewardPerShare) / REWARD_PRECISION));
    user.amount = user.amount.sub(removeAmount);
    ...
    (uint amountBdx, uint amountBnb) = pancakeRouter.removeLiquidityETH(
      ...
      address(this),
      block.timestamp + 1 hours);
    bdx.transfer(_to, amountBdx);
    payable(_to).transfer(amountBnb);

    emit RemoveLiquidity(msg.sender, _to, amountBdx, amountBnb, removeAmount);
    return true;
  }
