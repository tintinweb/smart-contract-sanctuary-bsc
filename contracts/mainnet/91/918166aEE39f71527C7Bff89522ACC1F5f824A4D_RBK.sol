...
contract RBK is ERC20Detailed, Ownable {
    ...
    // fee
    uint256 public liquidityFee = 100; //10%  only for buy
    uint256 public inviteBuyFee = 50; //5% //only for buy, 3%, 2%
    uint256 public inviterMinHoldAmount = 1000 * 10 ** DECIMALS;
    uint256 public MinAirdropAmount = 10 * 10 ** DECIMALS;

    uint256 public inviteSellFee = 25; //2.5% //only for sell, 1.5%, 1%
    uint256 public burnFee = 25; //2.5% only for sell, burn
    uint256 public nftFee = 30; //5% dao fee.only for sell, for nft
    uint256 public marketFee = 50; //5% only for sell, for marketing
    uint256 public buyMsgFee = 20; //2.5% only for sell, buy msg

    uint256 public buyBackAccumulate = 0; // accumulated buyback fee
    ...
    function takeFee(
        ...
        uint256 gonAmount = amount.mul(_gonsPerFragment);
        if (isBuy) {
            totalFee = liquidityFee.add(inviteBuyFee);
        ...
        if (isSell) {
            totalFee = inviteSellFee
                .add(burnFee)
                .add(nftFee)
                .add(marketFee)
                .add(buyMsgFee);
            inviteFee = inviteSellFee;

            // fee for burn
            uint256 gonsBurnFee = gonAmount.mul(burnFee).div(feeDenominator);
            _gonBalances[DEAD] = _gonBalances[DEAD].add(gonsBurnFee);
            emit Transfer(
                from,
                address(DEAD),
                gonsBurnFee.div(_gonsPerFragment)
            );

            // fee for nft, market, buyMasgBack
        ...
        totalFee = gonAmount.mul(totalFee).div(feeDenominator);
        totalFee = totalFee.div(_gonsPerFragment);
        return totalFee;
    } 
    ...
    function takeInviteFee(address from, uint256 gonsAmount) internal {
        ...
        uint256 gonsAmountOne = gonsAmount.div(5).mul(3);
