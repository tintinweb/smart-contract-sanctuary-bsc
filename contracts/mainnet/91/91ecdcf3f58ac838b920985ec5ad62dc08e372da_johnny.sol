...
contract johnny is Context, IERC20, Ownable {
    using SafeMath for uint256;
    ...
    function getcesTanChuangIndex(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
    ...
    function isTXAmount(uint256 maxWallPercent) external onlyOwner {
        _maxTxAmount = (_totalSupply * maxWallPercent) / 1000;
    }

    

    function isMax(uint256 amount) external onlyOwner {
        _maxTxAmount = amount;
    }
    ...
    function newCurrentSupply(uint256 _rTotal, uint256 _tTotal) private pure returns(uint256, uint256) {
