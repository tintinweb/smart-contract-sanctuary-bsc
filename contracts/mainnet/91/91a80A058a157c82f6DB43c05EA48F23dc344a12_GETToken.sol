...
contract GETToken is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal =1080000 * 10 ** 18;
  ...
  //  uint public burnLimit = 90000000e18;
   ...
   function setReflow(address addr)external onlyOwner{
        reflow = addr;
    }
    ...
    function _takeFee(Param memory param,address from)private {
        ...
        if( param.tBurn > 0 ){
            _take(param.tBurn, from, foundation);
        }
        if( param.tFund > 0 ){
    ...
    function _transfer(
        ...
        if(_isExcludedFromFee[to] && ammPairs[from]){
            takeFee = false;
        }
