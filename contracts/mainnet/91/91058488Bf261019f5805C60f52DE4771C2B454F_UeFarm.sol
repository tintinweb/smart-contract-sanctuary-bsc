...
contract UeFarm {
    ...
    function deposit(uint _amount) public {
        ...
        for (uint i=0;i<9;++i) {
            ...
            if (_inviter == address(0)) i = 9;
                ...
                user1.rewardDebt = add(user1.rewardDebt,int256(mul(_amount*10/100,acclpPerShare)/1e18));
                user1.share2 = add(user1.share2,_amount*10/100);
            }
            else if (i==1 && !_inviter.isContract()) {
                user1.rewardDebt = add(user1.rewardDebt,int256(mul(_amount*8/100,acclpPerShare)/1e18));
                user1.share2 = add(user1.share2,_amount*10/100);
            }
            else if (i==2 && !_inviter.isContract()) {
                user1.rewardDebt = add(user1.rewardDebt,int256(mul(_amount*5/100,acclpPerShare)/1e18));
                user1.share2 = add(user1.share2,_amount*5/100);
            }
            else if (i==3 && !_inviter.isContract()) {
                user1.rewardDebt = add(user1.rewardDebt,int256(mul(_amount*3/100,acclpPerShare)/1e18));
                user1.share2 = add(user1.share2,_amount*3/100);
            }
            else if ((i==4 || i == 5) && !_inviter.isContract()) {
                user1.rewardDebt = add(user1.rewardDebt,int256(mul(_amount*2/100,acclpPerShare)/1e18));
                user1.share2 = add(user1.share2,_amount*2/100);
            }
            else if ((i==6 || i == 7 || i == 8) && !_inviter.isContract()) {
                user1.rewardDebt = add(user1.rewardDebt,int256(mul(_amount*1/100,acclpPerShare)/1e18));
    ...
    function harvest() public returns (uint256) {
        ...
        if (ue.balancePrice(msg.sender)) {
            ...
            uint256 _pendinglot1 = toUInt256(sub(int256(accumulatedlot),user.rewardDebt));
            user.rewardDebt = int(accumulatedlot1);
    ...
    function withdraw(uint256 _amount) public {
        ...
        for (uint i=0;i<9;++i) {
            ...
            if (_inviter == address(0)) i = 9;
                ...
                user1.rewardDebt = sub(user1.rewardDebt,int256(mul(_amount*10/100,acclpPerShare)/1e18));
                if (user1.share2 > _amount*10/100)  user1.share2 = sub(user1.share2,_amount*10/100);
                else user1.share2 = 0;
            }
            else if (i==1 && !_inviter.isContract()) {
                user1.rewardDebt = sub(user1.rewardDebt,int256(mul(_amount*8/100,acclpPerShare)/1e18));
                if (user1.share2 > _amount*8/100)  user1.share2 = sub(user1.share2,_amount*8/100);
                else user1.share2 = 0;
            }
            else if (i==2 && !_inviter.isContract()) {
                user1.rewardDebt = sub(user1.rewardDebt,int256(mul(_amount*5/100,acclpPerShare)/1e18));
                if (user1.share2 > _amount*5/100)  user1.share2 = sub(user1.share2,_amount*5/100);
                else user1.share2 = 0;
            }
            else if (i==3 && !_inviter.isContract()) {
                user1.rewardDebt = sub(user1.rewardDebt,int256(mul(_amount*3/100,acclpPerShare)/1e18));
                if (user1.share2 > _amount*3/100)  user1.share2 = sub(user1.share2,_amount*3/100);
                else user1.share2 = 0;
            }
            else if ((i==4 || i == 5) && !_inviter.isContract()) {
                user1.rewardDebt = sub(user1.rewardDebt,int256(mul(_amount*2/100,acclpPerShare)/1e18));
                if (user1.share2 > _amount*2/100)  user1.share2 = sub(user1.share2,_amount*2/100);
                else user1.share2 = 0;
            }
            else if ((i==6 || i == 7 || i == 8) && !_inviter.isContract()) {
                user1.rewardDebt = sub(user1.rewardDebt,int256(mul(_amount*1/100,acclpPerShare)/1e18));
    ...
    function beharvest1(address usr) public view returns (uint256) {
        ...
        uint256 accumulatedlot = mul((user.amount*67/100+user.share2),_acclotPerShare)/1e18;
        uint256 _pendinglot = toUInt256(sub(int256(accumulatedlot),user.rewardDebt));
        return _pendinglot;
    }
    ...
    function teamRewards(address usr) public view returns (uint256[] memory) {
        uint n = tokens.length;
