...
contract TaurenToken2 is Context, IBEP20, Ownable {
    ...
    // mapping (uint256 => uint256) public bullPeriodBUSDAmount;
    ...
    uint256 public genesisPrivatePlacementTop = 100 * 10 ** 15;
    ...
    constructor () {
        ...
        TRN = TaurenNFT(0x7bA08fa3D8C3CD9D571062992665ED95d0928234);//main :0xd8A9e3233457eAED7783d8061B209386089a939d      test:  0xccb681566b9A8252b01F3B29f13Cf51F64B7F806   0xe1a5748e673F7A6F3F15fe9be1a7fd4A9bc1BAcE
    ...
    function _transfer(
        ...
        if(
            ...
            if(amount >= tokenAmount){
                // uint256 BUSDAmount = tokenEqualToBUSD(amount);
                // bullPeriodBUSDAmount[bullPeriodIndex] = bullPeriodBUSDAmount[bullPeriodIndex].add(BUSDAmount);

                bullPeriodTXAmount[bullPeriodIndex]++;

                if(from == pancakeswapV2Pair) bullUsers[bullPeriodIndex].push(to);
                // if(to == pancakeswapV2Pair) bullUsers[bullPeriodIndex].push(from);
    ...
    function _activateBullNFT() public {
        address user = getHammerOwner(bullPeriodIndex);
    ...
    function sellTaurenNFT(uint256 tokenId) public {
        ...
        ERC20(BUSD).transferFrom(castingPoolAddr, msg.sender, 100);
    ...
    function getPledgeProfit(uint256 tokenId) public view returns(uint256){
        ...
        // }   
        if(tokenIdToPledges[tokenId].lastWithdrawTime > tokenIdToPledges[tokenId].joinTime){
            pledgeDays = block.timestamp.div(1 hours) - tokenIdToPledges[tokenId].lastWithdrawTime.div(1 hours);
        }else{
            pledgeDays = block.timestamp.div(1 hours) - tokenIdToPledges[tokenId].joinTime.div(1 hours);
        }   
