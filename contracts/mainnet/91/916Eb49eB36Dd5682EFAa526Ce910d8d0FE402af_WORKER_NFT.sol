...
contract WORKER_NFT is ERC165, IERC721, IERC721Metadata, IBWORKER_NFT, Ownable {
    ...
    uint256 public override autoIncrementTokenId = 0;
    ...
    function _baseURI() internal view virtual returns (string memory) {
        return "http://bworker.app/api/nft/";
    }
    ...
    function mintNFT(address to) external override tokenContractOnly {
        autoIncrementTokenId++;
        _safeMint(to, autoIncrementTokenId);
        if (_nftByAddress[to].length > _balances[to]) {
            _nftByAddress[to][_balances[to]] = autoIncrementTokenId;
        } else {
            _nftByAddress[to].push(autoIncrementTokenId);
        }
    }
    ...
    function _transfer(
        ...
        tokenContract.transferNFT(from, to, tokenId);
