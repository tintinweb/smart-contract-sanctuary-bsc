...
/**
 * @title FoxtrotVendingMachine
 * @dev A vending machine that allows users to buy products using ERC20 tokens.
 * @author Michael Araque
 */

/// @custom:security-contact [email protected]
contract FoxtrotVendingMachineV1 is TokenStore, Ownable, ERC1155, ReentrancyGuard {
	uint256 private _globalSupply;
	string private _name;
	string private _symbol;
	address public cashCollector;

	event AddedProductToVendingMachine(uint256 indexed trackerId);
	event DispenseProduct(uint256 indexed capsuleId, uint256 amount, address buyer);
	event DispenseFreeProduct(uint256 indexed capsuleId, uint256 amount, address buyer);
	event UpdateCashCollector(address indexed cashCollector);
	event CapsuleRemoved(uint256 indexed capsuleId);
	event CapsuleUpdated(Product product, uint256 indexed capsuleId, uint256 index);

	mapping(uint256 => uint256) private _tokenSupply;

	constructor(address nativeTokenWrapper, address _cashCollector) ERC1155("") TokenStore(nativeTokenWrapper) {
		_name = "Foxtrot Command Vending Machine";
		_symbol = "FCVM";
		cashCollector = _cashCollector;
	}
	...
	function tokenSupplyOfToken(uint256 _capsuleId) public view virtual returns (uint256) {
		return _tokenSupply[_capsuleId];
	}

	/**
	 * @notice get the URI of a token
	 * @param _capsuleId the id of the token
	 * @return string URI of the token
	 */
	function uri(uint256 _capsuleId) public view override returns (string memory) {
		return _getUri(_capsuleId);
	}

	/**
	 * @dev Adds an item to the vending machine.
	 * @param _products List of products to be added to the vending machine.
	 * @param _paymentMethods List of payment methods accepted for the product.
	 * @param _capsuleUri URI to retrieve more information about the product.
	 * @return trackerId Unique identifier for the added item in the vending machine.
	 */
	function addItemToVendingMachine(
		Product[] calldata _products,
		Payment[] calldata _paymentMethods,
		bool singlePayment,
		uint256 _capsuleId,
		string memory _capsuleUri
	) public onlyOwner returns (uint256 trackerId) {
		require(_products.length != 0, "FCVM: InvProdLen");
		require(_paymentMethods.length != 0, "FCVM: InvPayLen");

		trackerId = _capsuleId;

		if (!singlePayment) {
			for (uint256 i = 0; i < _paymentMethods.length; i++) {
				if (_paymentMethods[i].requiredAmount == 0) {
					revert("FCVM: InvPayNonSep");
				}
			}
		}
		...
		emit AddedProductToVendingMachine(trackerId);
	}

	/**
	 * @dev Allows a user to buy a product from the vending machine and
	 * 		pay with multiple payment methods at the same time.
	 * @param _capsuleId Unique identifier of the product to be purchased.
	 * @param _amount Amount of the product to be purchased.
	 */
	function buyProductWithMultiPayments(uint256 _capsuleId, uint256 _amount) public {
		require(!hasSinglePayment(_capsuleId), "FCVM: HaveSepPay");
		uint256 count = getPaymentCountOfBundle(_capsuleId);

		for (uint256 i = 0; i < count; i++) {
			Payment memory payment = getPaymentOfBundle(_capsuleId, i);
			uint256 balance = _balanceOf(msg.sender, payment);

			if (balance < payment.requiredAmount * _amount) {
				revert("FCVM: InsBalAmount");
			}
			...
			_payProduct(
				...
				Payment({
					...
					assetContract: payment.assetContract,
					tokenId: payment.tokenId,
					requiredAmount: payment.requiredAmount * _amount
				})
			);
		}

		_dispenseItems(_capsuleId, _amount, msg.sender);
		emit DispenseProduct(_capsuleId, _amount, msg.sender);
	}

	/**
	 * @dev Allows a user to buy a product from the vending machine with a single payment of the list.
	 * @param _capsuleId Unique identifier of the product to be purchased.
	 * @param _amount Amount of the product to be purchased.
	 * @param _specificPaymentMethod Payment method to be used for the purchase.
	 */
	function buyProductWithSinglePayment(
		uint256 _capsuleId,
		uint256 _amount,
		Payment calldata _specificPaymentMethod
	) public {
		require(hasSinglePayment(_capsuleId), "FCVM: NotHaveSepPay");

		uint256 count = getPaymentCountOfBundle(_capsuleId);

		bool isPaymentMethodAvailable = false;
		for (uint256 i = 0; i < count; i++) {
			Payment memory payment = getPaymentOfBundle(_capsuleId, i);
			if (
				payment.assetContract == _specificPaymentMethod.assetContract &&
				payment.tokenId == _specificPaymentMethod.tokenId
			) {
		...
		require(isPaymentMethodAvailable, "FCVM: InvPayMethod");
		...
		for (uint256 i = 0; i < count; i++) {
			...
			if (
				...
				if (payment.requiredAmount == 0) {
					emit DispenseFreeProduct(_capsuleId, _amount, msg.sender);
				}
			}
		}
	...
	/**
	 * @dev Allow the VM Owner to update the capsule 
	 *		in the vending machine.
	 * @param _productToBind Product to be updated.
	 * @param _capsuleId Unique identifier of the product to be updated.
	 * @param _index Index of the product to be updated.
	 */
	function updateProductInCrate(
		Product memory _productToBind,
		uint256 _capsuleId,
		uint256 _index
	) public onlyOwner {
		_updateTokenInBundle(_productToBind, _capsuleId, _index);
		emit CapsuleUpdated(_productToBind, _capsuleId, _index);
	}

	/**
	 * @dev Allow the VM Owner to delete the capsule
	 *		and recover the tokens in a specific address.
	 * @param _recipient The address that will receive the tokens.
	 * @param _capsuleId Unique identifier of the product to be deleted.
	 */
	function deleteCrate(address _recipient, uint256 _capsuleId) public onlyOwner {
		require(_capsuleId != 0, "FCVM: InvCapsuleId");
		_releaseTokens(_recipient, _capsuleId);
		emit CapsuleRemoved(_capsuleId);
	}

	/**
	 * @dev Updates the address of the cash collector in the vending machine.
	 * @param newAddress The new address for the cash collector.
	 */
	function updateCashCollectorAddress(address newAddress) external onlyOwner {
		require(cashCollector != newAddress, "FCVM: SmeAddrsAsActual");
		cashCollector = newAddress;

		emit UpdateCashCollector(newAddress);
	}
