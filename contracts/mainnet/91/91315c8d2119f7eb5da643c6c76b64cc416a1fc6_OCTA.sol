...
contract OCTA is Context, IERC20, Ownable {
    ...
    string private _name = "Octagram";
    string private _symbol = "OCTA";
    uint8 private _decimals = 18;
    bool public market_active;
    mapping (address => bool) public _isBlacklisted;
    mapping (address => bool) public premarket_user;

    address payable public DevelopmentWalletAddress = payable(0x2047d7F53F6FC7b1a2eEDDE1bD4855E536c79976);
    address payable public MarketingWalletAddress = payable(0xD19745cEE12670bFB90BE35d6fd6B65AA3Bb6F54);
    ...
    event stepLiquify(bool overMinimumTokenBalanceStatus,bool inSwapAndLiquifyStatus, bool isDevelopmentPair_sender, bool swapAndLiquifyEnabledStatus);
    event stepFee(bool p);

    event MarketingGetBnb(uint256 amount);
    event DevelopmentGetBnb(uint256 amount);
    ...
    constructor () {
        ...
        _totalTaxIfBuying = _buyLiquidityFee.add(_buyDevelopmentFee).add(_buyMarketingFee);
        _totalTaxIfSelling = _sellLiquidityFee.add(_sellDevelopmentFee).add(_sellMarketingFee);
        _totalDistributionShares = _liquidityShare.add(_DevelopmentShare).add(_MarketingShare);
    ...
    function setBuyTaxes(uint256 newLiquidityTax, uint256 newDevelopmentTax, uint256 newMarketingTax) external onlyOwner() {
    ...
    function setSelTaxes(uint256 newLiquidityTax, uint256 newDevelopmentTax, uint256 newMarketingTax) external onlyOwner() {
    ...
    function setDistributionSettings(uint256 newLiquidityShare, uint256 newDevelopmentShare, uint256 newMarketingShare) external onlyOwner() {
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        emit DevelopmentGetBnb(amountBNBDevelopment);
    ...
    function setBuyFee(uint256 buyLiquidityFee, uint256 buyDevelopmentFee,uint256 buyMarketingFee)
        external
        onlyOwner
    {
        ...
        require(_buyLiquidityFee + _buyDevelopmentFee + _buyMarketingFee <= 15, "Must keep taxes below 15%");
    }

    function setSellFee(uint256 sellLiquidityFee, uint256 sellDevelopmentFee,uint256 sellMarketingFee)
        external
        onlyOwner
    {
        ...
        require(_sellLiquidityFee + _sellDevelopmentFee + _sellMarketingFee <= 15, "Must keep taxes below 20%");
    }
}