...
interface IHPoolManager {
    ...
    function addProtocolFeeWETH(uint256 amount) external;
 ...
 * Github: madjarevicn
 */
contract HPoolToken is PausableUpgradeable, IERC20 {
...
contract HPool is HordUpgradable, HPoolToken {
    ...
    event EmergencyWithdrawStateChanged(bool isActive);
    event EmergencyWithdrawExecuted(address user, uint256 amount);
    ...
    */
    ...
    function setEmergencyMode(bool isActive) external onlyHordCongress {
        isEmergencyMode = isActive;
        emit EmergencyWithdrawStateChanged(isActive);
    }
    ...
    */
    ...
    returns (uint256,uint256)
    {
        ...
        if(totalCashoutValueUSDBeforeFees > initialDepositUSD) {
            ...
            championFeeWETH = getChampionFee(profitETH);
            // Compute protocol Fee in ETH
            protocolFeeWETH = getProtocolFee(profitETH);
        }

        // Total exit fee USD
        uint256 exitFeeUSD = hordConfiguration.exitFeeAmount(totalCashoutValueUSDBeforeFees);
        uint256 exitFeeWETH = hPoolManager.convertUSDValueToETH(exitFeeUSD);

        // Add into state accounting
        hPool.availableToClaimProtocolFee = hPool.availableToClaimProtocolFee.add(protocolFeeWETH).add(exitFeeWETH);
        hPool.availableToClaimChampionSuccessFee = hPool.availableToClaimChampionSuccessFee.add(championFeeWETH);

        hPool.totalProtocolFee = hPool.totalProtocolFee.add(protocolFeeWETH).add(exitFeeWETH);
        hPool.totalChampionSuccessFee = hPool.totalChampionSuccessFee.add(championFeeWETH);

        // Fire event that Fees are taken
        emit FeesTaken(championFeeWETH, protocolFeeWETH, exitFeeWETH);

        // Return profit made by this pool in USD and totalETH after fees
        return (
            profitUSD,
            totalETHReceived.sub(championFeeWETH.add(protocolFeeWETH).add(exitFeeWETH))
        );
    }
    ...
    */
    ...
    onlyMaintainer
    {
        ...
        uint256 amountWETHForChampion = totalETHReceived.mul(championShare).div(one);
        // Compute amount of ETH for users
        uint256 totalUsersLiquidatedETH = totalETHReceived.sub(amountWETHForChampion);

        // Compute amount of ETH which will be used to buy hord for champion
        uint256 championETHForHord = hordRatio.mul(amountWETHForChampion).div(one);
        amountWETHForChampion = amountWETHForChampion.sub(championETHForHord);
    ...
    function transferFundsToChampionAndEndPool(
        uint256 amountWETHForChampion,
        uint256 championTotalHord,
        uint256 championHPoolTokensAmount
    )
    internal
    {
        uint256 totalChampionWETH = hPool.availableToClaimChampionSuccessFee.add(amountWETHForChampion);
        // Reset champion fee to 0
        hPool.availableToClaimChampionSuccessFee = 0;
        // transfer total WETH (include championFee) worth to champion
        transferWETH(championAddress, totalChampionWETH);
    ...
    */
    ...
    whenNotPaused
    {
        ...
        emit EmergencyWithdrawExecuted(msg.sender, hPoolTokensAmount);
    }
    ...
    external
    {
        ...
        transferWETH(msg.sender, amount);
    ...
    /**
         ...
         * @param           amount is amount that maintainer wants to collect.
     */
    ...
    onlyMaintainer
    {
        ...
        hPoolManager.addProtocolFeeWETH(amount);
