...
contract WeLand is ERC721Enumerable, Ownable {
    using Strings for uint256;

    IERC20 public WeLandToken;

    uint256 public price = 0.02 ether;
    uint256 public welandPrice = 200 ether;
    ...
    function mintWithWeland(uint256 _amount) public payable {
        require(publicSaleStarts <= block.timestamp, "sale it's not open yet");       
        WeLandToken.transferFrom(msg.sender, address(this), welandPrice);
    ...
    function withdrawWeland() public onlyOwner {
        uint256 balance = WeLandToken.balanceOf(address(this));
        WeLandToken.transfer(msg.sender, balance);
    }
    ...
    function changeValues(uint256 _price, uint256 _welandPrice, uint256 _walletLimit)
        public
        onlyOwner
    {
        price = _price;
        welandPrice = _welandPrice;
