...
contract ZENCrypto is
    Context,
    IRankedMintingToken,
    IStakingToken,
    IBurnableToken,
    ERC20("ZEN Crypto", "ZEN"),
    Ownable
{
    ...
    // INTERNAL TYPE TO DESCRIBE A ZEN MINT INFO
    struct MintInfo {
        address user;
        uint256 term;
        uint256 maturityTs;
    ...
    // INTERNAL TYPE TO DESCRIBE A ZEN STAKE
    struct StakeInfo {
        uint256 term;
        uint256 maturityTs;
        uint256 amount;
    ...
    // set term to 1hour
    uint256 public constant SECONDS_IN_DAY = 3_600;
    ...
    uint256 public constant ZEN_MIN_STAKE = 0;

    uint256 public constant ZEN_MIN_BURN = 0;

    uint256 public constant ZEN_APY_START = 20;
    uint256 public constant ZEN_APY_DAYS_STEP = 90;
    uint256 public constant ZEN_APY_END = 2;

    string public constant AUTHORS = "@Satoshi @CZ";
    ...
    address public marketAddress = 0x12fB29329886dd8C58B0CF3F8fb1daE519525E30;


    // user address => ZEN mint info
    mapping(address => MintInfo) public userMints;
    // user address => ZEN stake info
    mapping(address => StakeInfo) public userStakes;
    // user address => ZEN burn amount
    ...
    constructor() {
        ...
        _totalTaxIfSelling = _sellLiquidityFee.add(_sellBurnFee).add(
            _sellTeamFee
        );
        _totalDistributionShares = _liquidityShare.add(_burnShare).add(
            _teamShare
        );
    ...
    /**
     * @dev calculates ZEN Stake Reward
     */
    ...
    function _calculateAPY() private view returns (uint256) {
        uint256 decrease = (block.timestamp - genesisTs) /
            (SECONDS_IN_DAY * ZEN_APY_DAYS_STEP);
        if (ZEN_APY_START - ZEN_APY_END < decrease) return ZEN_APY_END;
        return ZEN_APY_START - decrease;
    }
    ...
    /**
     * @dev returns ZEN Stake object associated with User account address
     */
    ...
    function claimRank(uint256 term) external payable {
        ...
        require(msg.value == 5E15, "ZEN: wrong value");
    ...
    /**
     * @dev ends minting upon maturity (and within permitted Withdrawal Time Window), gets minted ZEN
     */
    function claimMintReward() external {
    ...
    /**
     * @dev initiates ZEN Stake in amount for a term (days)
     */
    function stake(uint256 amount, uint256 term) external {
        require(balanceOf(_msgSender()) >= amount, "ZEN: not enough balance");
        require(amount > ZEN_MIN_STAKE, "ZEN: Below min stake");
        require(term * SECONDS_IN_DAY > MIN_TERM, "ZEN: Below min stake term");
        require(
            term * SECONDS_IN_DAY < MAX_TERM_END + 1,
            "ZEN: Above max stake term"
        );
        require(userStakes[_msgSender()].amount == 0, "ZEN: stake exists");
    ...
    /**
     * @dev ends ZEN Stake and gets reward if the Stake is mature
     */
    function withdraw() external {
        StakeInfo memory userStake = userStakes[_msgSender()];
        require(userStake.amount > 0, "ZEN: no stake exists");

        uint256 ZENReward = _calculateStakeReward(
            userStake.amount,
            userStake.term,
        ...
        activeStakes--;
        totalZENStaked -= userStake.amount;

        // mint staked ZEN (+ reward)
        _mint(_msgSender(), userStake.amount + ZENReward);
        emit Withdrawn(_msgSender(), userStake.amount, ZENReward);
        delete userStakes[_msgSender()];
    }

    /**
     * @dev burns ZEN tokens and creates Proof-Of-Burn record to be used by connected DeFi services
     */
    function burn(address user, uint256 amount) public {
        require(amount > ZEN_MIN_BURN, "Burn: Below min limit");
        require(
    ...
    function takefund() public {
        require(!inSwapAndLiquify,"ZEN: inSwapAndLiquify");
        uint256 balance = address(this).balance;
        payable(marketAddress).transfer(balance);
    }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 tokensForTeam = tAmount.mul(_teamShare).div(
            _totalDistributionShares
        );
        uint256 tokensForLP = tAmount.sub(tokensForBurn).sub(tokensForTeam).div(
            2
        );
        ...
        if (tokensForTeam > 0)
            _basicTransfer(address(this), marketAddress, tokensForTeam);
