...
contract MBGBToken is ERC20, ERC20Burnable, Ownable {
    ...
    constructor(address _market, address _owner) ERC20("MBGBToken", "MBGB") {
        _mint(address(this), 380000000000 * 10**decimals());
    ...
    function queryDays(uint32 _last) public view returns (uint32) {
        return (uint32(block.timestamp) - _last) / uint32(1 minutes);
    }
    ...
    {
        // (uint112 _reserve0, uint112 _reserve1, ) = IPancakeSwapV2Pair(
        //     uniswapUsdtV2Pair
        // ).getReserves();
    ...
    function updateTrend() internal {
        ...
        if (price > 0) {
            if (trend.length > 0) {
                ...
                if ((uint32(block.timestamp) - tLast.time) < 1 minutes) {
                    return;
                }
            }
