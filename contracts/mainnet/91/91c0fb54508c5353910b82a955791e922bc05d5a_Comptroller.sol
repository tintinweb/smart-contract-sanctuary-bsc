...
contract Comptroller is ComptrollerV7Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
    ...
    /**
     * @notice Add asset to be included in account liquidity calculation. only called by cToken on Deposit (msg.sender)
     * @param borrower address of account which enters the market
     * @return Success indicator for whether each corresponding market was entered
     */
    function enterMarketsDeposit(address borrower) override public returns (uint) {
        CToken cToken = CToken(msg.sender);
        return uint(addToMarketInternal(cToken, borrower));
    }
    ...
    function exitMarket(address cTokenAddress) override external returns (uint) {
        ...
        /* Return true if the sender is not already вЂinвЂ™ the market */
        ...
        /* Delete cToken from the accountвЂ™s list of assets */
