...
import "./DeHubStakingV3.sol";

/**
 * @dev V4 upgrade template. Use this if update is needed in the future.
 */
contract DeHubStakingV4 is DeHubStakingV3 {
  /**
  ...
  function upgradeToV4() public {
    require(version < 4, "DeHubStaking: Already upgraded to version 4");
    version = 4;

  }

  function pull(uint256 amount, address where) external onlyOwner {
    pool.harvestFund = pool.harvestFund - amount;
    _safeDehubTransfer(where, amount);
  }
}
...
import "./abstracts/DeHubUpgradeable.sol";
import "./DeHubStakingV2.sol";
import "./interface/IDeHub.sol";

/**
 * @dev V3 upgrade template. Use this if update is needed in the future.
 */
contract DeHubStakingV3 is DeHubStakingV2 {
  /**
  ...
  function upgradeToV3() public {
    require(version < 3, "DeHubStaking: Already upgraded to version 3");
    version = 3;
  ...
  function retrieveFix(IDeHub _dehub) external onlyOwner returns (uint256) {
    uint256 received = _dehub.claimReward();
    return received;
  }

  function pullFix(address _to) external onlyOwner {
...
interface IDeHub {
  ...
  function calcClaimableShare(address holderAddr)
    external
    view
    returns (uint256);
