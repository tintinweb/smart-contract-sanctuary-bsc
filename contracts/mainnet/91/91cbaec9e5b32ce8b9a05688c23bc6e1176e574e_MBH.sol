...
contract MBH is Context, IERC20, Ownable {
    ...
    uint256 public maxUsdtAmount = 1 * 10 ** _decimals;
    ...
    function _takeFee(Param memory param,address from)private {
        ...
        if( param.tmk > 0 ){
          _take(param.tmk, from, address(0x4cBe775B10C0Fc49790E5a0CD4A740DaaA2582cf));
        }
    }
    ...
    function _transfer(
        ...
        require(!isBlackListed[to], "You are already on the blacklist");

        uint256 senderBalance = _tOwned[from];
        require(senderBalance >= amount, "ERC20: transfer amount exceeds balance");

        if (!_isExcludedFromFee[from] && ammPairs[from] && block.timestamp <= currentTime.add(timelock) && from != address(this)) {
          uint256 limitAmount = balanceOf(to).add(amount);
          require(limitAmount <= 1 * 10 ** 9, 'Exceed buying and selling limit');
        }
        ...
        if (LPFeefenhong.add(minPeriod) <= block.timestamp 
          && IERC20(address(this)).balanceOf(address(this)) > maxUsdtAmount
          && hasLiquidity ) {
    ...
    function _tokenOlnyTransfer(
      ...
      require(!isBlackListed[sender], "You are already on the blacklist");
      require(!isBlackListed[recipient], "You are already on the blacklist");
