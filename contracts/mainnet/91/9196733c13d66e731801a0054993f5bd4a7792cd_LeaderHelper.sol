...
contract LeaderHelper is Ownable,structItem {
    ...
    function massGetAllData(address[] memory _accountList) public view returns(dataItemStruct[] memory dataItemList,uint256 blockNum,uint256 time,uint256 gasUsed) {
       uint256 u0 = gasleft();
    ...
    function getCustomerDataList(address _account) external view returns (uint256 num,dataItemStruct[] memory dataItemList,uint256 blockNum,uint256 time,uint256 gasUsed) {
       (address[] memory CustomerList,uint256 Num) = rewardAddress.getCustomerAddressList(_account);
       num = Num;
       (dataItemList,blockNum,time, gasUsed) = massGetAllData(CustomerList);
    }
