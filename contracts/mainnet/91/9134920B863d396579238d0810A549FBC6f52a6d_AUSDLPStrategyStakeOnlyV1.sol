...
// File: contracts/strategies/AUSDLPStrategyStakeOnlyV1.sol
...
interface AlpacaStaker{
    function deposit(address _for, uint256 _pid, uint256 _amount) external; // No auto harvest
    function withdrawAll(address _for, uint256 _pid) external; // This will auto harvest
...
contract AUSDLPStrategyStakeOnlyV1 is Ownable {
    using SafeMath for uint256;
    ...
    address constant AUSD_ADDRESS = address(0xDCEcf0664C33321CECA2effcE701E710A2D28A3F);
    address constant BUSD_ADDRESS = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
    address constant ALPACA_ADDRESS = address(0x8F0528cE5eF7B51152A59745bEfDD91D97091d2F);
    ...
    address constant AUSDLP_POOL_ADDRESS = address(0xa74077EB97778F4E94D79eA60092D0F4831d05A6);
    address constant POOL_3EPS_ADDRESS = address(0x160CAed03795365F3A589f10C379FfA7d75d4E76); // BUSD/USDC/USDT
    address constant ALPACA_STAKING_ADDRESS = address(0xA625AB01B08ce023B2a342Dbb12a16f2C8489A8F);
    uint256 constant LP_POOL_ID = 25;
    ...
    function setupWithdrawTokens() internal {
        // Start with AUSDLP
        IERC20 _token = IERC20(address(0xae70E3f6050d6AB05E03A50c655309C2148615bE));
        tokenList.push(
            TokenInfo({
                token: _token,
    ...
    function getLPInStaker(address _user) public view returns (uint256) {
        AlpacaStaker staker = AlpacaStaker(ALPACA_STAKING_ADDRESS);
        (uint256 amount, , ,) = staker.userInfo(LP_POOL_ID, _user);
        return amount;
    }
    ...
    function withdraw(address _depositor, uint256 _share, uint256 _total, bool nonContract) public onlyZSBToken returns (uint256) {
        ...
        if(nonContract == true){
            if( _share > _total.mul(percentTradeTrigger).div(DIVISION_FACTOR)){
                harvestRewards(); // This will claim rewards then sell ALPACA for AUSDLP and some for WBNB
            }
        }
    ...
    */
    ...
    function stakeLP() internal {
        ...
        staker.deposit(address(this), LP_POOL_ID, _bal);
    }

    function unstakeLP() internal {
        // Removes all LP from the pool, will also pull unclaimed ALPACA
        if(getLPInStaker(address(this)) == 0){ return; } // Nothing is staked
        AlpacaStaker staker = AlpacaStaker(ALPACA_STAKING_ADDRESS);
        staker.withdrawAll(address(this), LP_POOL_ID);
    }

    function emergencyUnstakeLP() internal {
        // Removes all LP from the pool, will also pull unclaimed ALPACA
        AlpacaStaker staker = AlpacaStaker(ALPACA_STAKING_ADDRESS);
        staker.emergencyWithdraw(LP_POOL_ID);
    }

    function checkHarvestWBNB() internal view returns (uint256) {
        // Returns the amount of WBNB returned from liquidating the rewards
        IERC20 alp = IERC20(ALPACA_ADDRESS);
        uint256 rewards = alp.balanceOf(address(this)); // Get any stored ALPACA
        AlpacaStaker staker = AlpacaStaker(ALPACA_STAKING_ADDRESS);
        rewards = rewards.add(staker.pendingAlpaca(LP_POOL_ID, address(this)));
    ...
    function harvestRewards() internal {
        // This will liquidate rewards to BUSD, convert some to WBNB then take remaining BUSD and convert to AUSDLP
        AlpacaStaker staker = AlpacaStaker(ALPACA_STAKING_ADDRESS);
        if(staker.pendingAlpaca(LP_POOL_ID, address(this)) > 0){
            staker.harvest(LP_POOL_ID);
        }
        TradeRouter router = TradeRouter(PANCAKE_ROUTER);

        // Convert ALPACA to BUSD
        IERC20 alp = IERC20(ALPACA_ADDRESS);
        uint256 rewards = alp.balanceOf(address(this)); // Get any ALPACA here
        if(rewards > 0){
            address[] memory path = new address[](2);
            path[0] = ALPACA_ADDRESS;
            path[1] = BUSD_ADDRESS;
            alp.safeApprove(PANCAKE_ROUTER, 0);
            alp.safeApprove(PANCAKE_ROUTER, rewards);
        ...
        if(rewards > 0){
            // Convert a percentage of rewards to WBNB
            rewards = rewards.mul(DIVISION_FACTOR.sub(percentStakeDepositor)).div(DIVISION_FACTOR);
            address[] memory path = new address[](2);
            path[0] = BUSD_ADDRESS;
            path[1] = WBNB_ADDRESS;
            IERC20(BUSD_ADDRESS).safeApprove(PANCAKE_ROUTER, 0);
            IERC20(BUSD_ADDRESS).safeApprove(PANCAKE_ROUTER, rewards);
            router.swapExactTokensForTokens(rewards, 1, path, address(this), now.add(60));
        }
        ...
        if(rewards > 0){
            ...
            CurveLikeExchange ellipsis = CurveLikeExchange(AUSDLP_POOL_ADDRESS);
            IERC20(TOKEN_3EPS_ADDRESS).safeApprove(AUSDLP_POOL_ADDRESS, 0);
            IERC20(TOKEN_3EPS_ADDRESS).safeApprove(AUSDLP_POOL_ADDRESS, rewards);
            ellipsis.add_liquidity(depositAmounts, 1); // Will give us AUSDLP token            
        }
    }
    ...
    function checkAndSwapToken(address _executor) internal {
        ...
        harvestRewards(); // Will generate more AUSDLP and WBNB
