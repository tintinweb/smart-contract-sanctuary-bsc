...
contract RewardAdapter is AccessControl, Pausable {

    bytes32 public constant HANDLER = keccak256("HANDLER");

    IPlanetPlot public planetPlot;

    constructor (IPlanetPlot _planetPlot) {
        planetPlot = _planetPlot;
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
    }

    function setPlayerPlanetPass(address _renter, uint[] memory _planetIds) external onlyRole(HANDLER) whenNotPaused(){
        IPlanetPlot(planetPlot).setPlayerPlanetPass(_renter, _planetIds);
    }

    function pauseHandler() external {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "DEFAULT_ADMIN_ROLE required");
        _pause();
    }
    function unpauseHandler() external {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "DEFAULT_ADMIN_ROLE required");
        _unpause();
    }
}
