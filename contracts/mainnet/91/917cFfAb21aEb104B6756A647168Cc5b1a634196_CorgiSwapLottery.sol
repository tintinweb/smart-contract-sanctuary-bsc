...
// File: contracts/CorgiSwapLottery.sol

pragma solidity ^0.8.4;
pragma abicoder v2;

/** @title CorgiSwap Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract CorgiSwapLottery is ReentrancyGuard, ICorgiSwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInCoris = 5000 ether;
    uint256 public minPriceTicketInCoris = 0.05 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInCoris;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] corisPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInCoris;
        uint32 finalNumber;
    }
     ...
     */
    constructor(address _corisTokenAddress, address _randomGeneratorAddress) {
        ...
        nonReentrant
    {
        ...
        // Calculate number of CORIS to this contract
        uint256 amountCorisToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInCoris,
            _ticketNumbers.length
        );

        // Transfer CORIS tokens to this contract
        corisToken.safeTransferFrom(address(msg.sender), address(this), amountCorisToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInCoris += amountCorisToTransfer;
    ...
    function claimTickets(
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInCorisToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        corisToken.safeTransfer(msg.sender, rewardInCorisToTransfer);

        emit TicketsClaim(msg.sender, rewardInCorisToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInCoris) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in CORIS for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].corisPerBracket[j] =
                ...
                // A. No CORIS to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].corisPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInCoris - amountToShareToWinners);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        _lotteries[_lotteryId].amountCollectedInCoris += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInCoris >= minPriceTicketInCoris) && (_priceTicketInCoris <= maxPriceTicketInCoris),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInCoris: _priceTicketInCoris,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            corisPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInCoris: pendingInjectionNextLottery,
            finalNumber: 0
        });
     ...
     * @dev Only callable by owner
     * @param _minPriceTicketInCoris: minimum price of a ticket in CORIS
     * @param _maxPriceTicketInCoris: maximum price of a ticket in CORIS
     */
    function setMinAndMaxTicketPriceInCoris(uint256 _minPriceTicketInCoris, uint256 _maxPriceTicketInCoris)
        external
        onlyOwner
    {
        require(_minPriceTicketInCoris <= _maxPriceTicketInCoris, "minPrice must be < maxPrice");

        minPriceTicketInCoris = _minPriceTicketInCoris;
        maxPriceTicketInCoris = _maxPriceTicketInCoris;
    }
     ...
     * @param _priceTicket price of a ticket (in CORIS)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].corisPerBracket[_bracket];
        } else {
            return 0;
        }
    }
