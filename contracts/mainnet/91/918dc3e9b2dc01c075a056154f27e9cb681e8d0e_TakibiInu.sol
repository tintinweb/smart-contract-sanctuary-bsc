...
contract TakibiInu is Context, IERC20, Ownable {
    ...
    address private MarketingWallet = 0x68A0c84D83Fb68E27ca7046E40c2cE8f19457b80;
    address private OperationsWallet = 0xdb42c9d5bAb8a40D8e15fE9F23730b7BB288dEBD;
    ...
    Taxes public buyTaxes = Taxes(0, 25, 15, 25, 15);
    Taxes public sellTaxes = Taxes(0, 50, 30, 50, 30);
    ...
    function setBuyTaxes(uint256 _Reflections, uint256 _Marketing, uint256 _Liquidity, uint256 _Operations, uint256 _Burn) public onlyOwner {
        require(_Reflections+(_Marketing)+(_Liquidity)+(_Operations)+(_Burn) <=22, "Buy Taxes cannot be higher than 22%");
        buyTaxes = Taxes(_Reflections,_Marketing,_Liquidity,_Operations,_Burn);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _Reflections, uint256 _Marketing, uint256 _Liquidity, uint256 _Operations, uint256 _Burn) public onlyOwner {
        require(_Reflections+(_Marketing)+(_Liquidity)+(_Operations)+(_Burn) <=22, "Sell Taxes cannot be higher than 22%");
       
       sellTaxes = Taxes(_Reflections,_Marketing,_Liquidity,_Operations,_Burn);
        emit FeesChanged();
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell, bool useLaunchTax) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rReflections, to_return.rMarketing, to_return.rLiquidity, to_return.rOperations, to_return.rBurn) = _getRValues(to_return, tAmount, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell, bool useLaunchTax) private view returns (valuesFromGetValues memory s) {
        ...
        s.tOperations = tAmount*temp.Operations/1000;
        s.tBurn = tAmount*temp.Burn/1000;
        s.tTransferAmount = tAmount-s.tReflections-s.tMarketing-s.tLiquidity-s.tOperations-s.tBurn;
        return s;
    }

    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rReflections,uint256 rMarketing, uint256 rLiquidity, uint256 rOperations, uint256 rBurn){
        ...
        rTransferAmount =  rAmount-rReflections-rMarketing-rLiquidity-rOperations-rBurn;
        return (rAmount, rTransferAmount, rReflections,rMarketing,rLiquidity,rOperations,rBurn);
    }
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.Liquidity + temp.Marketing + temp.Operations + temp.Burn) * 2;
    ...
    function updateOperationsWallet(address newWallet) external onlyOwner{
        ...
        _isExcluded[OperationsWallet];
        _isExcludedFromFee[OperationsWallet];
    }
