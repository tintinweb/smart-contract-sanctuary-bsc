...
contract TRC721MetadataMintable is TRC721, TRC721Metadata, MinterRole {
	...
	uint256 public nftIndexId;
    function mintAdminWithTokenURI(address user) public onlyMinter returns (bool) {
        return _mintAdminWithTokenURI(user);
    }
	
	function _mintAdminWithTokenURI(address user) private returns (bool) {
        _mint(user, nftIndexId);
        _setTokenURI(nftIndexId, "https://bafkreidowmylu6riosmq3a2nmm6vxvn2jzuqzdb7vu2nypyiznpfurddgq.ipfs.nftstorage.link");
		nftIndexId = nftIndexId+1;
        return true;
    }
	
	function mintAdminWithTokenURI2Users(address[] memory users) public onlyMinter {
		uint256 size = users.length;
        for(uint256 i=0;i<size;i++){
            _mintAdminWithTokenURI(users[i]);
        }
    }
}
...
contract YMNFT is TRC721, TRC721Enumerable, TRC721MetadataMintable {
    constructor() public TRC721Metadata("YM LC NFT", "YM LC NFT") {
       
    }

    function adminburn(address user, uint256 tokenId) public onlyMinter returns(bool){
        _burn(user,tokenId);
        return true;
    }
}