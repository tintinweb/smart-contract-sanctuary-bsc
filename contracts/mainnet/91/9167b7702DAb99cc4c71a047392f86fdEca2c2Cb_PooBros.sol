...
contract PooBros is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    address public RewardToken = 0xa1611E8D4070dee36EF308952CF34255c92a01c5;
    address public FeeReceiver = 0xeAd460fd893Cb18ddC83bE126a089253A19e21eB;
    address public FeeReceiver2 = 0xAc7FCbde08776573BC85728EBADfC52De477de84;
    ...
    function mint(uint256 _mintAmount) public payable {
        // Checks
        require(!paused, "PooBros: minting has not started yet");
        require(_mintAmount > 0, "PooBros: you have to mint at least one");
        require(
            _mintAmount <= maxMintAmount,
            "PooBros: you can only mint 10 at a time"
        );
        require(
            lastSupply >= _mintAmount,
            "PooBros: the collection is sold out"
        );
        if (msg.sender != owner()) {
            require(
                msg.value >= cost * _mintAmount,
                "PooBros: total cost isn't match"
            );
        }
        // Minting
    ...
    {
        // Checks
        require(_winner != address(0), "PooBros: zero address");
        require(_amount > 0, "PooBros: you have to mint at least one");
        require(lastSupply != 0, "PooBros: the collection is sold out");
    ...
    function tokenURI(uint256 _id)
        ...
        returns (string memory)
    {
        require(_exists(_id), "PooBros: URI query for nonexistent token");
    ...
    function handleMintReward(uint256 _amount) private {
        // Split payment
        uint256 mintShare = _amount.mul(40).div(100);
        uint256 devShare = _amount.mul(45).div(100);
        uint256 devShare2 = _amount.sub(mintShare).sub(devShare);
        // Save dividend for community
        reflectDividend(mintShare);
        // Send owner share
        payable(FeeReceiver).transfer(devShare);
        payable(FeeReceiver2).transfer(devShare2);
    }
