...
contract OracleBTC is Epoch {
    ...
    function consult(address _token, uint256 _amountIn) external view returns (uint144 amountOut) {
        _amountIn = _amountIn.mul(10000);
        if (_token == token0) {
            amountOut = price0Average.mul(1e22).decode144();
        } else {
    ...
    function twap(address _token, uint256 _amountIn) external view returns (uint144 _amountOut) {
        ...
        _amountIn = _amountIn.mul(10000);
        if (_token == token0) {
            _amountOut = FixedPoint.uq112x112(uint224((price0Cumulative - price0CumulativeLast) / timeElapsed)).mul(1e22).decode144();
