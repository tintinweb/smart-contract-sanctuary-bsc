...
contract NFTVoting is Ownable {
    ...
    function createtNewEvent(IERC721 _votingAddress, string[] memory _choices, string[] memory _details) external onlyOwner {
       totalvotingEvents++;
       ...
       votingevent.votingAddress = _votingAddress;
        ...
        for(uint i=0; i<_details.length; i++) {
            votingevent.details.push(bytes32(abi.encodePacked(_details[i])));
    ...
    function getOptionsofEvent(uint eventId) public view isEvent(eventId) returns(string[] memory) {
     ...
     function getDetailsofEvent(uint eventId) public view isEvent(eventId) returns(string[] memory) {
        VotingDetails storage vd = votingeventDetails[eventId];
        bytes32[] memory deatilsLocal = vd.details;       
        string[] memory alldetails = vd.totalVotes.getAllchoices(deatilsLocal);

        return alldetails;
    }   

    function getEventDetails(uint eventId) public view isEvent(eventId) returns (IERC721, string[] memory,string[] memory, uint[] memory, bool ) {
       VotingDetails storage vd = votingeventDetails[eventId];
       string[] memory allchoices = getOptionsofEvent(eventId);
       string[] memory alldetails = getDetailsofEvent(eventId);
       return (vd.votingAddress,allchoices,alldetails,vd.totalVotes,vd.isVotinglive);
    }
