...
/**
 *Submitted for verification at FtmScan.com on 2022-07-23
*/
...
interface IRewarder {
    function onReward(uint256 pid, address user, address recipient, uint256 licoAmount, uint256 newTokenAmount) external;
    function pendingTokens(uint256 pid, address user, uint256 licoAmount) external view returns (IERC20[] memory, uint256[] memory);
}
...
// MasterChef is the master of LICO. He can make LICO and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once LICO is sufficiently
...
contract MasterChefV3 is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of LICOs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accLicoPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws tokens to a pool. Here's what happens:
        //   1. The pool's `accLicoPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. LICOs to distribute per block.
        uint256 totalStaked;      // Amount of tokens staked in given pool
        uint256 lastRewardTime;   // Last timestamp LICOs distribution occurs.
        uint256 accLicoPerShare;  // Accumulated LICOs per share, times 1e12. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
    }

    // The LICO TOKEN!
    MockToken public lico;
    // Dev address.
    address public devaddr;
    // Dev fee percentage.
    uint256 public devFee = 100;
    // LICO tokens created per second.
    uint256 public licoPerSecond;
    // Deposit Fee address
    ...
    // The block number when LICO mining starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 licoPerSecond);
    ...
    event LogUpdatePool(uint256 indexed pid, uint256 lastRewardBlock, uint256 stakeSupply, uint256 accLicoPerShare);

    constructor(
        MockToken _lico,
        address _devaddr,
        address _feeAddress,
        uint256 _licoPerSecond,
        uint256 _startTime
    ) {
        lico = _lico;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        licoPerSecond = _licoPerSecond;
    ...
    // Update the given pool's LICO allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending LICOs on frontend.
    function pendingLico(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accLicoPerShare = pool.accLicoPerShare;
        uint256 stakeSupply = pool.totalStaked;
        if (block.timestamp > pool.lastRewardTime && stakeSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardTime, block.timestamp);
            uint256 licoReward = multiplier.mul(licoPerSecond).mul(pool.allocPoint).div(totalAllocPoint);
            accLicoPerShare = accLicoPerShare.add(licoReward.mul(1e12).div(stakeSupply));
        }
        return user.amount.mul(accLicoPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardTime, block.timestamp);
        uint256 totalLico = multiplier.mul(licoPerSecond).mul(pool.allocPoint).div(totalAllocPoint);
        if (totalLico == 0) return;

        if(lico.totalSupply() + totalLico > lico.cap()) {
            totalLico = lico.cap() - lico.totalSupply();
            if(totalLico == 0 && licoPerSecond != 0)
                return _updateEmissionRate(0);
        }

        uint256 forDevs = totalLico.mul(devFee).div(1000);
        uint256 licoReward = totalLico.sub(forDevs);
        lico.mint(devaddr, forDevs);
        lico.mint(address(this), licoReward);
        pool.accLicoPerShare = pool.accLicoPerShare.add(licoReward.mul(1e12).div(stakeSupply));
        pool.lastRewardTime = block.timestamp;
        emit LogUpdatePool(_pid, pool.lastRewardTime, stakeSupply, pool.accLicoPerShare);
    }

    // Deposit tokens to MasterChef for LICO allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            pending = user.amount.mul(pool.accLicoPerShare).div(1e12).sub(user.rewardDebt);
            if (pending > 0) {
                safeLicoTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accLicoPerShare).div(1e12);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accLicoPerShare).div(1e12).sub(user.rewardDebt);
        if (pending > 0) {
            safeLicoTransfer(msg.sender, pending);
        }
        if (_amount > 0) {
    ...
    // Safe lico transfer function, just in case if rounding error causes pool to not have enough LICOs.
    function safeLicoTransfer(address _to, uint256 _amount) internal {
        uint256 licoBal = lico.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > licoBal) {
            transferSuccess = lico.transfer(_to, licoBal);
        } else {
            transferSuccess = lico.transfer(_to, _amount);
        }
        require(transferSuccess, "safeLicoTransfer: transfer failed");
    }
    ...
    function _updateEmissionRate(uint256 _licoPerSecond) internal {
        require(_licoPerSecond <= MAX_EMISSION_RATE, "Updated emissions are more than maximum rate");
        licoPerSecond = _licoPerSecond;
        emit UpdateEmissionRate(msg.sender, _licoPerSecond);
    }

    function updateEmissionRate(uint256 _licoPerSecond) public onlyOwner {
        _updateEmissionRate(_licoPerSecond);
        massUpdatePools();
    }    
...
// File: contracts/RewarderTimer.sol
...
contract RewarderTimer is IRewarder,  Ownable, Initializable{
    using SafeMath for uint256;
    ...
    /// Also known as the amount of rewards to distribute per second.
    struct PoolInfo {
    ...
    event LogInit(IERC20 rewardToken, uint256 rewardPerSecond, address MASTER_CHEF);

    function initialize (IERC20 _rewardToken, uint256 _rewardPerSecond, address _MASTER_CHEF)  external initializer {
        rewardToken = _rewardToken;
        rewardPerSecond = _rewardPerSecond;
        MASTER_CHEF = _MASTER_CHEF;
        emit LogInit(_rewardToken, _rewardPerSecond, _MASTER_CHEF);
    }

    function onReward(uint256 pid, address _user, address to, uint256, uint256 lpToken) external onlyMC override {
        ...
        if (user.amount > 0) {
            ...
            if (rewardBalance() > pending)  safeTransferRewardInternal(to, pending);
            else safeTransferRewardInternal(to, rewardBalance());
        }
        user.amount = lpToken;
    ...
    /// @param _rewardPerSecond The amount of rewards to be distributed per second.
    ...
    function updatePool(uint256 pid) public returns (PoolInfo memory pool) {
        pool = poolInfo[pid];
        if (block.timestamp > pool.lastRewardTime) {
            uint256 lpSupply = getLpSupply(pid);
            if (lpSupply > 0) {
                uint256 time = block.timestamp.sub(pool.lastRewardTime);
                uint256 reward = time.mul(rewardPerSecond).mul(pool.allocPoint) / totalAllocPoint;
    ...
    function getSecondsToEmptyRewards() public view returns (uint256) {
        return getUnharvestedRewards() / rewardPerSecond;
    }

    function getLpSupply(uint256 pid) internal view returns (uint256 lpSupply) {
        (IERC20 stakeToken,,,,,,) = MasterChefV3(MASTER_CHEF).poolInfo(pid);
        lpSupply = stakeToken.balanceOf(MASTER_CHEF);
    }
