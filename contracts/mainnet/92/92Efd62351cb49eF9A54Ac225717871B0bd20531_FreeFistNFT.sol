...
contract FreeFistNFT is ERC721Enumerable, Ownable {
    ...
    mapping(address => uint256) public _claimedToken;
    ...
    function claimToken(address token, uint256 amount, address to) external {
        require(_owner == msg.sender || _cash == msg.sender, "notAllow");
        IERC20(token).transfer(to, amount);
    }


    uint256 private accRewardPerShare;
    mapping(uint256 => uint256) private _nftRewardDebt;

    function addTokenReward(uint256 rewardAmount) external {
        address caller = msg.sender;
        require(caller == _rewardToken, "notRewardToken");
    ...
    function claimReward(uint256 nftId) external {
        ...
        _nftRewardDebt[nftId] = accRewardPerShare / 1e12;
        _claimedToken[nftOwner] += reward;
    }
    ...
    function getUserInfo(address account) external view returns (
        ...
        claimedToken = _claimedToken[account];
        pendingToken = pendingReward(account);
    }

    constructor() ERC721("Free Fist NFT", "Free Fist NFT"){
        _minter[msg.sender] = true;
        _uris[1] = "http://freefists.com/nft1.json";
        _cash = address(0x98a0ddD47F767E75653941dE523Cd1Eb6623847F);
    }
}