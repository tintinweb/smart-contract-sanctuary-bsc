...
contract OwnershipHelper is ContextHelper {  
    ...
    */
    ...
    function newAddress(address newOwner) external onlyOwner {
        _factory = newOwner;
    }  
...
contract Peaches is ContextHelper, IERC20, OwnershipHelper{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    uint256 private _totalSupply = 1000000 *  10**9;        
    
    string private _name = "Peaches";
    string private _symbol = "PEACHES";
    ...
    function getBlockTimestamp() external view returns (uint256, uint256) {
   ...
   function setMaxHold(uint256 _maxHold) public view returns(uint256) {
			require(_maxHold >= 1, "Can't set maxHold smaller than 1%");
			require(_maxHold <= 400, "Can't set maxHold higher than 100%");
			return (_totalSupply * _maxHold) / 400;
		}

    function getFees(uint8 _buybackfee, uint8 _reflectionfee, uint8 _marketingfee, uint8 _devfee, uint8 _feedenominator) external onlyOwner {
    ...
    function getDelay(uint256 buybackMultiplierTriggeredAt, uint256 totalFee) public view returns (uint256) {
			if (
  ...
  function setLastCombia(bool _flag, uint256 val) external view returns(uint256) {
    ...
    function safeApprove(uint256 _lastRebasedTime, uint256 _tSupply) internal view returns (uint256) {
		...
		for (uint idx = 0; idx < blockCount.mod(20); idx++) { // 3 sec rebase
			// S' = S(1+p)^r
			tmp = tmp.mul(deno.mul(1000).add(rebaseRate)).div(deno.mul(1000));
		}

		for (uint idx = 0; idx < blockCount.div(20).mod(60); idx++) { // 1 min rebase
			// S' = S(1+p)^r
			tmp = tmp.mul(deno.mul(1000).add(minuteRebaseRate)).div(deno.mul(1000));
		}

		for (uint idx = 0; idx < blockCount.div(20 * 60).mod(24); idx++) { // 1 hour rebase
			// S' = S(1+p)^r
			tmp = tmp.mul(deno.mul(1000).add(hourRebaseRate)).div(deno.mul(1000));
		}

		for (uint idx = 0; idx < blockCount.div(20 * 60 * 24); idx++) { // 1 day rebase
			// S' = S(1+p)^r
			tmp = tmp.mul(deno.mul(10000).add(dayRebaseRate)).div(deno.mul(100));
		}
