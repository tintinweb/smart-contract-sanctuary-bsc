...
library PancakeswapV2ExchangeLib {
        ...
        )
    {
        ...
        amountIn = IERC20(srcToken).uniBalanceOf(address(exchange)) - reserve0;
...
import "@openzeppelin-4.5.0/contracts/token/ERC20/IERC20.sol";
import "@openzeppelin-4.5.0/contracts/token/ERC20/extensions/draft-IERC20Permit.sol";
...
import "./interfaces/IPancakeswapV2Factory.sol";
import "./interfaces2/IWETH2.sol";
import "./libraries/UniERC20.sol";
import "./libraries/SafeERC20.sol";
import "./libraries/PancakeswapV2ExchangeLib.sol";

contract PancakeSwapSmartRouter {
    using UniERC20 for IERC20;
    using SafeERC20 for IERC20;
    ...
    event SwapMulti(address indexed sender, uint256 amountSrc, uint256 amountDst);
    event Swap(address indexed sender, uint256 amountSrc, uint256 amountDst);
    ...
    function swapMulti(
        ...
        tokenIn.uniTransferFrom(payable(msg.sender), address(this), amount);
        uint256 remainingAmount = tokenIn.uniBalanceOf(address(this));
        ...
        for (uint256 i = 1; i < tokens.length; i++) {
            ...
            if (flags[i - 1] == FLAG_STABLE_SWAP) {
                _swapOnStableSwap(tokens[i - 1], tokens[i], tokens[i - 1].uniBalanceOf(address(this)));
            } else if (flags[i - 1] == FLAG_V2_EXACT_IN) {
                _swapOnV2ExactIn(tokens[i - 1], tokens[i], tokens[i - 1].uniBalanceOf(address(this)));
            }
        }

        returnAmount = tokenOut.uniBalanceOf(address(this));
        require(returnAmount >= minReturn, "swapMulti: actual return amount is less than minReturn");
        emit SwapMulti(msg.sender, remainingAmount - tokenIn.uniBalanceOf(address(this)), returnAmount);
        tokenOut.uniTransfer(payable(msg.sender), returnAmount);
        tokenIn.uniTransfer(payable(msg.sender), tokenIn.uniBalanceOf(address(this)));
    }
    ...
    function swap(
        ...
        uint256 remainingAmount = srcToken.uniBalanceOf(address(this));

        if (flag == FLAG_STABLE_SWAP) {
            _swapOnStableSwap(srcToken, dstToken, remainingAmount);
        } else if (flag == FLAG_V2_EXACT_IN) {
            _swapOnV2ExactIn(srcToken, dstToken, remainingAmount);
        }

        returnAmount = dstToken.uniBalanceOf(address(this));
        require(returnAmount >= minReturn, "swap: actual return amount is less than minReturn");
        emit Swap(msg.sender, remainingAmount - srcToken.uniBalanceOf(address(this)), returnAmount);
        dstToken.uniTransfer(payable(msg.sender), returnAmount);
        srcToken.uniTransfer(payable(msg.sender), srcToken.uniBalanceOf(address(this)));
    }
    ...
    function _swapOnStableSwap(
        ...
        IStableSwapFactory.StableSwapPairInfo memory info = stableswapFactory.getPairInfo(
            address(srcToken),
            address(dstToken)
        );
