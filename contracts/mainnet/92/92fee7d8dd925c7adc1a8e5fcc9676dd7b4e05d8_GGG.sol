...
contract GGG is Owner {
	...
	function Calculation(DataArr memory tmp,uint num,address _addr) internal view returns(uint is_win,uint win_amount) {
			...
			if(num==2){
				if(tmp.num==2222 || tmp.num==23  || tmp.num==12 ){
				 ...
				 if(tmp.num==102 || tmp.num==302  || tmp.num==403 || tmp.num==100  ||tmp.num==300){
					is_win=1;
					win_amount=tmp.amount;
				 }
				 if(tmp.num==200 ){
	...
	function setChildFee(address _admin_addr,address _addr,uint _val) external     {
		...
		uint tmp=AdminUserFeeLists[_admin_addr][ParentAddressLists[msg.sender]];
		if(tmp==0){
			tmp=AdminAdminFeeLists[_admin_addr];
		}
