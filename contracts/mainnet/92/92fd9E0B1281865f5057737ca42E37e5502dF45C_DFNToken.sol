...
contract DFNToken is Context, IERC20, Ownable {
    ...
    uint public lpTxAmount = 50e18;

    address public lpAwardToken;
    ...
    function setIsExcludedFromFee(address account,bool isEX) public onlyOwner {
       _isExcludedLpAward[account] = isEX;
    }

    function setIsExcludedLpAward(address owner,bool isEx)external onlyOwner{
        _isExcludedLpAward[owner] = isEx;
    }
    ...
    function _transfer(
        ...
        if( 
            ...
            ||  isAddLiquidity || ( !ammPairs[from] && !ammPairs[to])){
        ...
        if ( !_isExcludedLpAward[fromAddress] && !ammPairs[fromAddress] ) {
            setEst(fromAddress);
        }
        if (!_isExcludedLpAward[toAddress] && !ammPairs[toAddress] ) {
            setEst(toAddress);
        }
        fromAddress = from;
        toAddress = to;
    ...
    function swapTokensForAward(uint256 tokenAmount,address to) private {
        ...
        lpInterest.award = IERC20(lpAwardToken).balanceOf(to);
    }
    ...
    function processEst() private {
        ...
        while (sendedCount < sendCountLimit && iterations < shareholderCount) {
            ...
            if (
                IERC20(lpAwardToken).balanceOf(address(this)) < amount 
                ||  surplusAmount < amount ) break;

            if (amount >= 1e3) {
                surplusAmount -= amount;
                IERC20(lpAwardToken).transfer(shareholder, amount);
            }
            sendedCount ++;
            iterations++;
            index ++;
    ...
    function checkLpAwardCondition(address owner)internal view returns(bool){
        uint lpBal = lpInterest.token.balanceOf(owner);
        return lpBal >= lpAwardCondition.lpHoldAmount 
