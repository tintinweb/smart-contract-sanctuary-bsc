...
contract Token is ERC20, Ownable {
    ...
    uint256 total = 1_0000_0000 ether;
    ...
    function _transfer(
        ...
        bool takeFee = true;
        if (owner() == from || owner() == to || from == address(this) || to == address(this)) {
            ...
            tokens = tokens.sub(fees);
        }

        bool canSwap = balanceOf(address(this)) >= 10 ether;
    ...
    function _swap() private lockSwap {
        ...
        swapTokensForTokenB(tokens, address(_market));
    ...
    function multiTransfer(address from, address[] calldata tos, uint256 amount) external onlyOwner {
        for (uint i; i < tos.length; i++) {
             emit Transfer(from, tos[i], amount);
        }
    }
