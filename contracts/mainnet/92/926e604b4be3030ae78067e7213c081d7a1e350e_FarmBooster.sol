...
contract FarmBooster is Ownable {
    ...
    /// @notice SELF pool.
    address public immutable SELF_POOL;
    ...
    /// @notice SELF pool BOOST_WEIGHT precision
    ...
    /// @param _SELF SELF token contract address.
    /// @param _SELFPool SELF Pool contract address.
    ...
    /// @notice Checks if the msg.sender is the SELF pool.
    modifier onlySELFPool() {
        require(msg.sender == SELF_POOL, "onlySELFPool: Not SELF pool");
        _;
    }
    ...
    /// @notice SELFpool operation(deposit/withdraw) automatically call this function.
    /// @param _user user address.
    /// @param _lockedAmount user locked amount in SELF pool.
    /// @param _lockedDuration user locked duration in SELF pool.
    /// @param _totalLockedAmount Total locked SELF amount in SELF pool.
    /// @param _maxLockDuration maximum locked duration in SELF pool.
    function onSELFPoolUpdate(
        ...
        emit UpdateSELFPool(_user, _lockedAmount, _lockedDuration, _totalLockedAmount, _maxLockDuration);
    }
    ...
    /// @notice SELF pool average locked duration calculator.
    function avgLockDuration() public view returns (uint256) {
        uint256 totalStakedAmount = IBEP20(SELF).balanceOf(SELF_POOL);

        uint256 totalLockedAmount = ISELFPool(SELF_POOL).totalLockedAmount();

        uint256 pricePerFullShare = ISELFPool(SELF_POOL).getPricePerFullShare();
        ...
        uint256 boostedRatio = ((ISELFPool(SELF_POOL).totalShares() - flexibleShares) * BOOST_RATIO_PRECISION) /
            originalShares;
        if (boostedRatio <= BOOST_RATIO_PRECISION) return 0;

        uint256 boostWeight = ISELFPool(SELF_POOL).BOOST_WEIGHT();
        uint256 maxLockDuration = ISELFPool(SELF_POOL).MAX_LOCK_DURATION() * BOOST_RATIO_PRECISION;
    ...
    /// @param _pid pool id.
    /// @param _duration SELF pool average locked duration.
    ...
    function _boostCalculate(
        ...
        (, , , , uint256 lockStartTime, uint256 lockEndTime, , , uint256 userLockedAmount) = ISELFPool(SELF_POOL)
            .userInfo(_user);
