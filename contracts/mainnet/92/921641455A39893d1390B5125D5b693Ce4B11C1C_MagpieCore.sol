    ...
    IMagpieCore
{
    ...
    function _wrapAssets(SwapArgs memory swapArgs)
        private
        returns (SwapArgs memory newSwapArgs)
    {
        ...
        returns (uint256[] memory amountOuts)
    {
        ...
        require(
            ...
            "MagpieCore: invalid fromAssetId - toAssetId"
        );
    ...
    function _intermediarySwap(
        ...
        require(
            ...
            "MagpieCore: invalid intermediary amountIn"
        );
    ...
    function updateConfig(Config calldata _config) external override onlyOwner {
        ...
        require(
            _config.hyphenLiquidityPool != address(0),
            "MagpieCore: invalid hyphenLiquidityPool"
        );
        require(
        ...
        )
    {
        ...
        require(
            config.intermediaries.includes(args.swapArgs.getToAssetId()) ==
                true,
...

        {
            bytes memory payloadOut = bytes.concat(
                ...
                abi.encodePacked(
                    ...
                    args.payload.destGasTokenAmountOutMin,
                    block.chainid,
                    amountOut,
                    useTokenBridge
                )
            ...
            coreSequence = IWormholeCore(config.coreBridgeAddress)
                .publishMessage(
                    uint32(block.timestamp % 2**32),
                    payloadOut,
        ...
        returns (uint256[] memory amountOuts)
    {
        ...
        if (payload.useTokenBridge) {
            IWormhole(config.tokenBridgeAddress).completeTransfer(
        ...
        require(
            swapArgs.getAmountIn() <= payload.amountIn,
        ...
        if (payload.destGasTokenAmount > 0) {
            ...
            require(
                args.gasTokenSwapArgs.getToAssetId().isNative(),
                "MagpieCore: invalid toAssetId"
            );
            require(
            ...
            require(
                args.gasTokenSwapArgs.amountOutMin >= payload.destGasTokenAmountOutMin,
                "MagpieCore: invalid amountOutMin"
            );
...
interface IMagpieCore {
    ...
    function swapIn(SwapInArgs calldata swapArgs, bool tokenBridge)
        external
        payable
        returns (
            uint256[] memory amountOuts,
    ...
    event SwappedOut(SwapOutArgs args, uint256[] amountOuts, address caller);
...
library LibSwap {
    ...
    function getFromAssetId(IMagpieRouter.SwapArgs memory self)
        internal
        pure
        returns (address)
    {
    ...
    function getToAssetId(IMagpieRouter.SwapArgs memory self)
        internal
        pure
        returns (address)
    {
...
contract MagpieRouter is ReentrancyGuard, Ownable, IMagpieRouter {
    ...
    function _swap(SwapArgs memory swapArgs)
        internal
        returns (uint256[] memory amountOuts)
    {
        ...
        for (uint256 i = 0; i < swapArgs.routes.length; i++) {
            ...
            require(
                fromAssetId == swapArgs.assets[firstHop.path[0]],
                "MagpieRouter: invalid from asset"
            );
            require(
                toAssetId ==
                    swapArgs.assets[lastHop.path[lastHop.path.length - 1]],
                "MagpieRouter: invalid to asset"
            );
            uint256 beforeSwapBalance = swapArgs.getToAssetId().getBalance();
            _swapRoute(route, swapArgs.assets, swapArgs.deadline);
            uint256 afterSwapBalance = swapArgs.getToAssetId().getBalance();
            amountOuts[i] = afterSwapBalance - beforeSwapBalance;
        }
        ...
        require(
            toAssetId.getBalance() == startingBalance + amountOut,
    ...
    function _swapUniswapV2(
        ...
        uint256[] memory amounts = IUniswapV2Router02(amm.id)
