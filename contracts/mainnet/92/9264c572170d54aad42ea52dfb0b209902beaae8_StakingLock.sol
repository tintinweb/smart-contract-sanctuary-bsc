...
interface IBEP20 {
    ...
    function _maxOwnerShip() external view returns (uint256);
...
contract StakingLock {
    mapping(address => uint256) public withdrawedInfos;
    mapping(address => UserStakingInfo) public userPrivateInfos;
    ...
    uint256 public timePeriod = 432000;
    ...
    function userInfoAsObject(address account) public view returns (UserStakingInfo memory info) {
        ...
        info.amount = amount - withdrawedAmount;
        info.stakedDate = stakedDate;
    }
    ...
    function claimAmount(uint256 _amount) public noReentrant {
        (uint256 amount, uint256 stakedDate) = userInfos(msg.sender);
        require( amount >= _amount, "Ermm!");

        uint256 ownershipLeft =  bep20Token._maxOwnerShip() - bep20Token.balanceOf(msg.sender);

        require(_amount <= ownershipLeft, "Ermm!");
        require(stakedDate + timePeriod <= block.timestamp, "Not yet!");

        withdrawedInfos[msg.sender] = withdrawedInfos[msg.sender] + _amount;
    ...
    function claimAll() public noReentrant {
        ...
        if(amount > bep20Token._maxOwnerShip() - bep20Token.balanceOf(msg.sender))
        {
            amount = bep20Token._maxOwnerShip() - bep20Token.balanceOf(msg.sender);
        }

        require(amount > 0, "Ermm!");

        require(stakedDate + timePeriod <= block.timestamp, "Not yet!");

        withdrawedInfos[msg.sender] = withdrawedInfos[msg.sender] + amount;
        bep20Token.transfer(msg.sender, amount);
        emit TokenUnstaked(msg.sender, amount);
    }

    function getUserShare(address staker)
        external
        view
        returns (uint256 amount)
    {
        (uint256 stakedAmount, ) = userInfos(staker);
        amount = stakedAmount.mul(profit).div(profitDenominator);
    }
