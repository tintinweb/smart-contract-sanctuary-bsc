...
// SuppoinuStaking: 6 months staking period
...
contract SuppoinuStaking7 is Ownable, IStakingLockable {
  ...
  function deposit(uint256 amount) external {
    ...
    require(!(user.amount == 0 && amount < 100000000000 * decimals), 'First time deposit not enough amount');
  ...
  // update base on contract Apr 180 days
  function updateUserApr(address account) internal {
    UserInfo storage user = userInfo[account];
    if(user.amount < 100000000000 * decimals){
      user.apr = 0;
    }else if(user.amount >= 100000000000 * decimals && user.amount <= 300000000000 * decimals){
      user.apr = 200;
    }else if(user.amount > 300000000000 * decimals && user.amount <= 600000000000 * decimals){
      user.apr = 400;
    }else if(user.amount > 600000000000 * decimals){
      user.apr = 600;
    }
  }
