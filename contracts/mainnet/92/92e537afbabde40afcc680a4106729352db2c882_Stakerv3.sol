...
contract RewardTokenUpgradeable is BEP20Upgradeable('RewardToken', 'RWD') {
...
contract Stakerv3 is Initializable, OwnableUpgradeable {
    using SafeMathUpgradeable for uint256;
    ...
    struct PoolInfo {
        ...
        RewardTokenUpgradeable rwdToken; //RewardToken
    ...
    // The Reward TOKEN!
    RewardTokenUpgradeable public rwd;
    // Dev address.
   ...
   // uint256 public intervalBlock = 86400;
    ...
    event Claim(address indexed user, uint256 indexed pid, uint256 amount);
     ...
     function initialize() public initializer  {
        devaddr = 0x7266A06d646f4C669D21EA60B46670cAfE8f216c;
        feeAddress = 0x7266A06d646f4C669D21EA60B46670cAfE8f216c;
        startBlock = 16051043;
        poolFee = 0.05 ether;
    ...
    function add(uint256 _allocPoint, IBEP20Upgradeable _lpToken, uint16 _depositFeeBP, bool _withUpdate,RewardTokenUpgradeable _rwdToken,uint256 _lockedPeriod,bool _isFixed,string calldata _logo,uint256 _emisson,uint256 _interval) public payable  {
       ...
       if(!_isFixed)
        ...
        require(msg.value == poolFee,"Invaild Fee");
    ...
    function set(uint256 _pid, uint256 _allocPoint, uint16 _depositFeeBP, bool _withUpdate,RewardTokenUpgradeable _rwdToken,uint256 _lockedPeriod,string calldata _logo,uint256 _interval) public onlyOwner {
    ...
    function claimFixed(address _user,uint256 pid) internal{
        ...
        uint256 currentTime = !user.isClaimed ? (block.timestamp > user.userLockedPeriod && ( user.isClaimed || user.lastRewardBlock < user.userLockedPeriod)  ? user.userLockedPeriod : block.timestamp) : user.lastRewardBlock;
        uint256 multiplier = (getMultiplier(user.lastRewardBlock, currentTime)).div(pool.intervalBlock); // mul(3)  difference in seconds and converted into day
        uint256 tostReward = (user.amount.mul(pool.emissionPerBlock).div(10000)).mul(multiplier).mul(pool.allocPoint.div(100)); // TostPerBlock = 20 i.e 0.2 %
        pending = (tostReward);
