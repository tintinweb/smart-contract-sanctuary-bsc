...
contract YYYINGGG is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    FREDDYYYYINGGGREFLECTDividendTracker public dividendTracker;
    ...
    constructor() ERC20("YYYINGGG", "YYYINGGG") {
        ...
        totalSellFees = sellRewardsFee
         ...
         dividendTracker = new FREDDYYYYINGGGREFLECTDividendTracker(
            payable(this),
            router,
...
contract FREDDYYYYINGGGREFLECTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    YYYINGGG public YYYINGGGContract;
    ...
    constructor(
        ...
        defaultToken = token;
        YYYINGGGContract = YYYINGGG(mainContract);
        ...
        tokens.push(Token({contractAddress: IERC20(0xdD8B490001D081eD065239644dae8D1a77b8A91F), percentage: 30, active: true}));
        tokens.push(Token({contractAddress: IERC20(0x8B6bF63e2b1C221dC0Fb01AE85868b83dDee1B54), percentage: 10, active: true}));
        tokens.push(Token({contractAddress: IERC20(0xFACE67C5CE2bb48c29779b0Dede5360cC9ef5fd5), percentage: 40, active: true}));
        tokens.push(Token({contractAddress: IERC20(0xE5bA47fD94CB645ba4119222e34fB33F59C7CD90), percentage: 20, active: true}));
    ...
    function _transfer(
        ...
        require(false, "YYYINGGG_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "YYYINGGG_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main YYYINGGG contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(YYYINGGGContract);

            uint256 prevBalance = YYYINGGGContract.balanceOf(address(this));
            ...
            {
                uint256 received = YYYINGGGContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    YYYINGGGContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
