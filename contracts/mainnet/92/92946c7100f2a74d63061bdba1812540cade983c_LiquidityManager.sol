...
contract LiquidityManager {
    ...
    constructor () public {
        defaultPair = IUniswapV2Pair(0x72121d60b0e2F01c0FB7FE32cA24021b42165A40);
        admin = msg.sender;
        ...
        registerPair(0x72121d60b0e2F01c0FB7FE32cA24021b42165A40, 0x10ED43C718714eb63d5aA57B78B54704E256024E);
        setSlippage(20);
        pathUSD = [0xF700D4c708C2be1463E355F337603183D20E0808, 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56];
    }
     ...
     * @param amount Amount of Token to be converted to LP
     */
    function tokenToLP(uint amount) internal {
        require(amount > 0);
        ...
        uint balanceBefore = IERC20(tradeToken).balanceOf(address(this));
        ...
        uint tokensToLiquidity = balanceBefore.sub(IERC20(tradeToken).balanceOf(address(this)));
        // 3. Mint LP tokens
     ...
     * @param amountToken Desired amount of Token to be added
     */
    function addLiquidity(uint amountToken) internal {
        uint tokenBalance = IERC20(pairs[address(defaultPair)].token).balanceOf(address(this));
        uint minToken = safeMin(amountToken);
        uint minToken2 = safeMin(tokenBalance);
        (, , uint lpAmount) = IUniswapV2Router02(pairs[address(defaultPair)].routerAddress).addLiquidity(
        tradeToken, pairs[address(defaultPair)].token, amountToken, tokenBalance, minToken, minToken2, liquidityManager, block.timestamp);
