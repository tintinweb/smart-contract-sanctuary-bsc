    ...
    Pausable
{
    ...
    //save stakingFeePercentage
    uint256 public lessThan2Weeks = 25;
    uint256 public lessThan4Weeks = 15;
    uint256 public lessThan6Weeks = 10;
    uint256 public lessThan8Weeks = 5;
    uint256 public moreThan8Weeks = 0;
    ...
    constructor(
        ...
        xfuelToken = _xfuelToken;
        busd = _busd;
        name = _name;
        rewardsDistribution = _ownerAddress;
    ...
    function updateFeeCollector(address _feeCollectorAddress)
        external
        onlyOwner
    {
    ...
    function addGrandAmount(uint256 _amount) external onlyOwner {
        xfuelToken.transferFrom(msg.sender, address(this), _amount);
    ...
    function setLessThan2Weeks(uint256 feePercentage) external onlyOwner {
        lessThan2Weeks = feePercentage;
    }

    function setLessThan4Weeks(uint256 feePercentage) external onlyOwner {
        lessThan4Weeks = feePercentage;
    }

    function setLessThan6Weeks(uint256 feePercentage) external onlyOwner {
        lessThan6Weeks = feePercentage;
    }

    function setLessThan8Weeks(uint256 feePercentage) external onlyOwner {
        lessThan8Weeks = feePercentage;
    }

    function setMoreThan8Weeks(uint256 feePercentage) external onlyOwner {
        moreThan8Weeks = feePercentage;
    }

    function checkFee(address account, uint256 amount)
        public
        view
        returns (uint256)
    {
        uint256 percentage;
        uint256 tempTimeStamp = timeStampDiff(account) / 60;
        if (tempTimeStamp <= 20160) {
            // less than 2 weeks.
            percentage = lessThan2Weeks;
        } else if (tempTimeStamp <= 40320) {
            // less than 4 weeks.
            percentage = lessThan4Weeks;
        } else if (tempTimeStamp <= 60480) {
            // less than 6 weeks.
            percentage = lessThan6Weeks;
        } else if (tempTimeStamp <= 80640) {
            // less than 8 weeks.
            percentage = lessThan8Weeks;
        } else if (tempTimeStamp <= 100800) {
            // more than 8 weeks.
            percentage = moreThan8Weeks;
        }
        ...
        updateReward(msg.sender)
    {
        ...
        xfuelToken.transferFrom(msg.sender, address(this), amount);
        ...
        updateReward(msg.sender)
    {
        ...
        uint256 charges = checkFee(msg.sender, _amount);
        if (charges > 0) {
            xfuelToken.transfer(feeCollector, charges);
        }
        _totalSupply = _totalSupply - _amount;
        _balances[msg.sender] = _balances[msg.sender] - _amount;
        uint256 withdrawAmount = _amount - charges;
        xfuelToken.transfer(msg.sender, withdrawAmount);
        // Updating current timestamp.
        stakedTimeStamp[msg.sender] = block.timestamp;
    ...
    function getReward() public nonReentrant updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            xfuelToken.transfer(msg.sender, reward);
