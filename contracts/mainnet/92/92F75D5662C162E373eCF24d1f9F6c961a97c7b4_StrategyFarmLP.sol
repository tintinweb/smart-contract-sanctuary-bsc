...
contract StrategyFarmLP is StratManager {
    ...
    function withdraw(address account, uint256 share, uint256 totalShares) public virtual {
        require(msg.sender == vault, "!vault");

        uint256 amount = balanceOf() * share / totalShares;
        uint256 wantBal = IERC20(want).balanceOf(address(this));

        if (wantBal < amount) {
            IMasterChef(masterchef).withdraw(poolId, amount - wantBal);
    ...
    function harvestStage1() external virtual whenNotPaused onlyEOA {
        IMasterChef(masterchef).deposit(poolId, 0);
    }

    function harvestStage2() external virtual whenNotPaused onlyEOA {
        addLiquidity();
    }

    function harvestStage3() external virtual whenNotPaused onlyEOA {
        deposit();
    }
    ...
    function removeLiquidity(uint256 amount) internal returns (uint256 amountToken0, uint256 amountToken1) {
    ...
    function addLiquidity() internal {
        ...
        uint256 outputHalf = outputBal.div(2);
        ...
        uint256 lp0Bal = chargeFees(lpToken0);

        if (lpToken1 != output) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(
                outputBal.sub(outputHalf),
                0,
                outputToLp1Route,
                address(this),
                block.timestamp
        ...
        uint256 lp1Bal = chargeFees(lpToken1);
...
contract StratManager is Ownable, Pausable {
    ...
    uint256 public gravisFee;
    address public gravisFeeRecipient;
    ...
    event ChargedFees(uint256 callFees, uint256 gravisFees, uint256 strategistFees);
    ...
    constructor(
        ...
        gravisFee = _gravisFee;
        gravisFeeRecipient = _gravisFeeRecipient;
    ...
    function setGravisFee(uint256 _gravisFee) external onlyOwner {
        gravisFee = _gravisFee;
    }
    ...
    function chargeFees(address _token) internal returns (uint256) {
        IERC20 token = IERC20(_token);
        uint256 allBal = token.balanceOf(address(this));
        uint256 feeBal = allBal.mul(poolFee).div(MAX_FEE);

        uint256 callFeeAmount = feeBal.mul(callFee).div(MAX_FEE);
        if (callFeeAmount > 0)
            token.safeTransfer(callFeeRecipient, callFeeAmount);

        uint256 gravisFeeAmount = feeBal.mul(gravisFee).div(MAX_FEE);
        if (gravisFeeAmount > 0)
            token.safeTransfer(gravisFeeRecipient, gravisFeeAmount);

        uint256 strategistFeeAmount = feeBal.sub(callFeeAmount).sub(gravisFeeAmount);
        if (strategistFeeAmount > 0)
            token.safeTransfer(strategistFeeRecipient, strategistFeeAmount);

        emit ChargedFees(callFeeAmount, gravisFeeAmount, strategistFeeAmount);
        return allBal.sub(feeBal);
    }
