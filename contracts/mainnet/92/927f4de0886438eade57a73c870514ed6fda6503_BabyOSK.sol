...
contract BabyOSK is Context, IERC20, Ownable {
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if (IERC20(fistAddress).allowance(wrapRouter, address(this)) >= amount) {
            // Marketing
            uint256 amountMarketing = amount.mul(_marketingShare).div(_totalDistributionShares);
            IERC20(fistAddress).transferFrom(wrapRouter, marketingWalletAddress, amountMarketing);
            // team
            uint256 amountTeam = amount.sub(amountMarketing);
            IERC20(fistAddress).transferFrom(wrapRouter, teamWalletAddress, amountTeam);
        }
