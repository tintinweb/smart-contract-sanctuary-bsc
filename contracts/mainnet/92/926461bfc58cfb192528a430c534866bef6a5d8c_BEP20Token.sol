...
contract BEP20Token is Context, IBEP20, Ownable {
  ...
  event NewSettings(uint256 l, uint8 pf, uint256 mintFee, uint256 minPrice);
  ...
  constructor() public {
    ...
    _symbol = "SBTC";
    _decimals = 18;
    settings(3000, 11, 2, 1e20);
    feeA = 3;
    feeC = 80;
    initContracts(0x9cB501E79C2eA02F2fF729B2968566b97d8Dede5, 0x773C81a75F6cC86093B745F9819575591eE616f5);
    entry[_msgSender()] = 0;
    lastUpdate = now;
  ...
  function settings(uint256 _l, uint8 _profitLimit, uint8 _mintFee, uint256 _minPrice) public onlyOwner {
    require(_l > 0);
    ...
    mintFee = _mintFee;
    minPrice = _minPrice;
    emit NewSettings(_l, _profitLimit, _mintFee, _minPrice);
  }
  ...
  function getGain() internal view returns (uint256) {
    ...
    uint256 p = (((c / 1e10) ** 2) / (getMA() / 1e10) * 1e10 + c) / 2;
    if(p > c) p = (c * 4 + p) / 5;
    uint256 g = getMA2().add(p).add(minPrice);
  ...
  function nextDecrease() internal view returns (uint256) {
    ...
    return temp * getTimePassed(lastUpdate) * l * feeC / (c * 100);
  }
