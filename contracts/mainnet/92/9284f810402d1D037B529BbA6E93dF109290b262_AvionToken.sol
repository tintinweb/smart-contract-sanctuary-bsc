...
import "./interfaces/ILMSRouter.sol";
...
contract AvionToken is ERC20Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable  {
    ...
    function clearStuckBalanceBUSD(address _receiver) external onlyOwner {
    ...
    function buyPresale(uint256 amount) external {
        require(block.timestamp >= 1657220400, "Not yet in presale period");
        IERC20(busdToken).transferFrom(msg.sender, address(this), amount / 2);
        _gonBalances[msg.sender] += amount.mul(getYield(msg.sender));
    }
