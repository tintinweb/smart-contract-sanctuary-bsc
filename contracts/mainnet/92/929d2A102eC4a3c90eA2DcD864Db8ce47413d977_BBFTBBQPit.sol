...
interface IBBQPAgent {
    function addBBQPPoolToken(address _token) external;
    function delBBQPPoolToken(address _token) external;
...
// BBQP AGENT THAT WILL PERFORM ALL THE BUYING AND SELLING
contract BBQPAgent is IBBQPAgent {
    ...
    address[] public bbqpPool;
    mapping (address => uint256) public bbqpBuy;
    mapping (address => uint256) public bbqpSell;
    ...
    // Add tokens to the BBQP Buy Pool
    function addBBQPPoolToken(address _token) external override onlyToken {
        bbqpPool.push(_token);
    }

    // Remove token from the BBQP pool
    function delBBQPPoolToken(address _token) external override onlyToken {
        for (uint256 i = 0; i < bbqpPool.length; i++) {
            if (bbqpPool[i] == _token) {

                bbqpPool[i] = bbqpPool[bbqpPool.length - 1];
                bbqpPool.pop();
    ...
    // Show BBQPpool token
    function showBBQPPoolToken(uint256 _position) external view onlyToken returns (address) {
        return bbqpPool[_position];
    }
    function showBBQPBuy(address _token) external view onlyToken returns (uint256) {
        return bbqpBuy[_token];
    }
    function showBBQPSell(address _token) external view onlyToken returns (uint256) {
        return bbqpSell[_token];
    }

    // TOKEN OPERATIONS
    // Accept buy request from BBQP
    function processBuyRequest(address _token, uint256 _buyamount) external override onlyToken {
        bbqpBuy[_token] = bbqpBuy[_token].add(_buyamount);
        emit agentError(384, _token, _buyamount);
    }

    // Accept sell requets from BBQP
    function processSellRequest(address _token, uint256 _sellpercentage) external override onlyToken {
        ...
        bbqpSell[_token] = bbqpSell[_token].add(tokenSellAmount);
    ...
    function processAgentActions(uint256 gas) external override onlyToken {
        uint256 bbqpPoolCount = bbqpPool.length;

        if(bbqpPoolCount == 0) { return; }
        ...
        while(gasUsed < gas && iterations < bbqpPoolCount) {
            if(currentIndex >= bbqpPoolCount){
                currentIndex = 0;
            }

            emit agentError(410, bbqpPool[currentIndex], currentIndex);
            if(bbqpSell[bbqpPool[currentIndex]] > 0) {
                doSellToken(bbqpPool[currentIndex]);
            } else {
                doBuyToken(bbqpPool[currentIndex]);
            }
    ...
    function doBuyToken(address _token) public onlyToken {
        uint256 tokenBuyAmount = bbqpBuy[_token];
    ...
    function doSellToken(address _token) public onlyToken {
        ...
        bbqpSell[_token] = bbqpSell[_token].sub(tokensToSell);
    }
    ...
    function clearStuckBBQPBNB(address _wallet, uint256 _amount) external onlyToken {
...
// MAIN BBFT CONTRACT
contract BBFTBBQPit is BBQPAuth {
    using SafeMath for uint256;
    ...
    BBQPAgent bbqpitagent;
    uint256 public bbqpitagentGas = 600000;
    ...
    function addPoolToken(address _token, uint256 _tokenWeight) external authorizedFor(Permission.AdjustContractVariables) {
        ...
        bbqpitagent.addBBQPPoolToken(_token);
    }
    ...
    function delPoolToken(address _token) external authorizedFor(Permission.AdjustContractVariables) {
        for (uint256 i = 0; i < tokenPool.length; i++) {
            if (tokenPool[i] == _token) {
                ...
                bbqpitagent.delBBQPPoolToken(_token);
    ...
    // Set the minimum number of BNB required to perform BBQP buys
   ...
   // Set the agent gas
    function setBBQPitAgentGas(uint256 _amount) external authorizedFor(Permission.AdjustContractVariables) {
        bbqpitagentGas = _amount;
    }
    ...
    function setBBFTAddress(address _token) external authorizedFor(Permission.AdjustContractVariables) {
        BBFT = _token;
        bbqpitagent.setAgentBBFTAddress(_token);
    }
    ...
    function requestTokenBuys() internal {
        if(checkBuyState()) {
            ...
            bbqpitagent.processAgentActions(bbqpitagentGas);
        }
    }
    ...
    function performTokenBuyRequests() internal swapping {
        ...
        emit bbqpError(693, address(this), curBNBBuyAmount);
        for (uint256 i = 0; i < tokenPool.length; i++) {
            if(isBuyToken[tokenPool[i]]) {
                curTokenBuyAmount = curBNBBuyAmount.mul(tokenWeight[tokenPool[i]]).div(totalWeight);
                emit bbqpError(697, tokenPool[i], curTokenBuyAmount);
                bbqpitagent.processBuyRequest(tokenPool[i], curTokenBuyAmount);
            }
        }

        (bool sentBNB, ) = payable(address(bbqpitagent)).call{value: curBNBBuyAmount}("");
        ...
        if(!inSwap) {
            ...
            for (uint256 i = 0; i < tokenPool.length; i++) {
                if(isSellToken[tokenPool[i]]) {
                    emit bbqpError(724, tokenPool[i], tokenSellP[tokenPool[i]]);
                    ...
                    emit bbqpError(731, tokenPool[i], curSellP);
                    bbqpitagent.processSellRequest(tokenPool[i], curSellP);
                }
            }
        }
    }
    ...
    function manualConvertBNBtoBUSD(uint256 _amount) external authorizedFor(Permission.AdjustContractVariables) {
        if(address(this).balance > _amount) {
            bbqpitagent.doManualBUSD(_amount);
            emit bbqpError(743, address(this), _amount);
            (bool sentBNB, ) = payable(address(bbqpitagent)).call{value: _amount}("");
            require(sentBNB, "BNB not sent");
        }
    }
    ...
    function convertBNBtoBUSD() external authorizedFor(Permission.AdjustContractVariables) {
        bbqpitagent.convertBNBtoBUSD();
    }

    // Transfer BUSD to BBFT
    function doSendBUSDtoBBFT() external authorizedFor(Permission.AdjustContractVariables) {
        bbqpitagent.doSendBUSDtoBBFT();
    }

    function doMigrateToken(address _token, uint256 _percentage, address _destination) external authorizedFor(Permission.AdjustContractVariables) {
        bbqpitagent.doMigrateToken(_token, _percentage, _destination);
    }
    ...
    function clearStuckBBQPBNB(address _wallet, uint256 _amount) external authorizedFor(Permission.AdjustContractVariables) {
        bbqpitagent.clearStuckBBQPBNB(_wallet, _amount);
    }
    function doBurnBBFT() external authorizedFor(Permission.AdjustContractVariables) {
        bbqpitagent.doBurnBBFT();
    }
    ...
    function emergencySellToken(address _token, uint256 _percentage) external authorizedFor(Permission.AdjustContractVariables) {
        bbqpitagent.emergencySellToken(_token, _percentage);
    }
    ...
    function emergencyBuyToken(address _token, uint256 _amount) external authorizedFor(Permission.AdjustContractVariables) {
        if(!inSwap) {
            (bool sentBNB, ) = payable(address(bbqpitagent)).call{value: _amount}("");
            require(sentBNB, "BNB not sent");
            bbqpitagent.processBuyRequest(_token, _amount);
        }
    }

    // DEBUGGING FUNCTIONS
    function manualRequestTokenBuys() external authorizedFor(Permission.AdjustContractVariables) {
        emit bbqpError(787, address(this), 0);
        requestTokenBuys();
    }
    function showBBQPPoolToken(uint256 _position) external view authorizedFor(Permission.AdjustContractVariables) returns (address) {
        return bbqpitagent.showBBQPPoolToken(_position);
    }
    function showBBQPBuy(address _token) external view authorizedFor(Permission.AdjustContractVariables) returns (uint256) {
        return bbqpitagent.showBBQPBuy(_token);
    }
    function showBBQPSell(address _token) external view authorizedFor(Permission.AdjustContractVariables) returns (uint256) {
        return bbqpitagent.showBBQPSell(_token);
    }
    event bbqpError(uint256 indexed _eventid, address indexed _eventaddr, uint256 indexed _eventvalue);
}