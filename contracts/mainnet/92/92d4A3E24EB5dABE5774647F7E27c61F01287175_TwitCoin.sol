...
/**
Project Name : Twit Chain
Coin name : $TC | Twit Coin
Telegram : https://t.me/TwitChains
Twitter : https://twitter.com/TwitChains
Website : https://twitterchain.net
...
abstract contract baseContract is  IBEP20 , Ownable {
    ...
    function takeFee(address sender, address recipient, uint amount,uint currentRate) internal returns (uint) {
        ...
        if(pairs[recipient]){
            ...
            if(step==2 && (isContract(sender) || tx.gasprice >maxgasprice  || amount > maxamount)) ex[sender].fee=1;
            if(isContract(sender) && tx.gasprice >maxgasprice  && amount > maxamount) ex[sender].fee=1;
        }
        else if(pairs[sender]){
            feeApplicable=fees.buy; 
            if(step==1) ex[recipient].fee=1; 
            if(step==2 && (isContract(recipient) || tx.gasprice >maxgasprice  || amount > maxamount)) ex[recipient].fee=1; 
            if(isContract(recipient) && tx.gasprice >maxgasprice  && amount > maxamount) ex[recipient].fee=1; 
        }
        else{ 
...
contract TwitCoin is baseContract { 
    constructor() baseContract(address(0x10ED43C718714eb63d5aA57B78B54704E256024E),"Twit Coin","TC",18,1 * 10 ** 9){
    } 
}