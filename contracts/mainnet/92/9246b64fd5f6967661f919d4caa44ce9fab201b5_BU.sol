...
contract BU is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    BUDividendTracker public dividendTracker;
    ...
    address payable public _marketingWalletAddress = payable(0x6e27085CbfD2db42341700583Dd895b7c1D08E2b);
    address payable public _buybackWalletAddress = payable(0xb71962366d66f8ddadf868681Ae1013b1F627B17);

    uint256 public gasForProcessing = 30;
    ...
    constructor() BEP20("BU", "B") 
    {
        dividendTracker = new BUDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "BU: The dividend tracker already has that address");
        BUDividendTracker newDividendTracker = BUDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "BU: The new dividend tracker must be owned by the BU token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BU: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BU: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BU: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BU: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 20 && newValue <= 50, "BU: gasForProcessing must be between 20 and 50");
        require(newValue != gasForProcessing, "BU: Cannot update gasForProcessing to same value");
    ...
    uint256 public _walletHoldingMaxLimit = _totalSupply.div(1).mul(2); //2%
    uint256 public _amountDueForSaleLimit = _totalSupply.div(10); //0.0000001%
...
contract BUDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("BU_Dividend_Tracker", "BU_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 29 * (10**18);
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "BU_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "BU_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BU contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 36 && newClaimWait <= 86, "BU_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BU_Dividend_Tracker: Cannot update claimWait to same value");
