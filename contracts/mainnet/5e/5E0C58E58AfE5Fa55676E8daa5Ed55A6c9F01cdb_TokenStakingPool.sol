...
contract TokenStakingPool is Ownable ,ReentrancyGuard{
        ...
        for (uint256 _pid = 0; _pid < len; _pid++) {
            ...
            if( user.amount > 0 ){
                if (isMultLP(address(pool.lpToken))) {
                    ...
                    tokenAmountTotal = tokenAmountTotal.add(tokenAmount);
                } else {
                    uint256 iTokenAmount = pendingIToken(_pid, _user);
                    iTokenAmountTotal = iTokenAmountTotal.add( iTokenAmount );
                }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public notPause returns(uint256){
        PoolInfo storage pool = poolInfo[_pid];
        if (isMultLP(address(pool.lpToken))) {
            _amount = withdrawITokenAndToken(_pid, _amount, msg.sender);
        } else {
    ...
    function withdrawITokenAndToken(uint256 _pid, uint256 _amount, address _user) private returns(uint256){
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
