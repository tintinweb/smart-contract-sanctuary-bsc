...
contract token is ERC20,WhiteList {
    using SafeMath for uint256;
    using Address for address;
    ...
    address private _marketWallet = address(0xAC4c0133dBc2eC5B7d76DBc1EeB93B7594E96E37);

    //name,symbol
    string internal tokenName = "CM";
    string internal tokenSymbol = "CM";
    uint256 constant total  =  1999 * 10**18;
    ...
    // IERC20 public RewardToken = IERC20(0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684);
    ...
    constructor(address tokenOwner) ERC20(tokenName, tokenSymbol) { 
        ...
        excludeFromFees(_marketWallet);
        setWhiteList(_marketWallet, true);
        setWhiteList(tokenOwner, true);
    ...
    function setNormal(bool _enable) public onlyOwner {
        normal= _enable;
    }
    ...
    function _transfer(
        ...
        if(!isWhiteListed(from)){
            require(balanceOf(from).sub(amount) >= 5 * 10**14,"min 0.0005");
        }

        if(!isWhiteListed(to)){
            require(amount <= 2 * 10**18,"max 2");
            require(balanceOf(to).add(amount) <= 2 * 10**18,"max 2");
        }
        ...
        if (takeFee) {
            // super._transfer(from, _marketWallet, amount.div(100).mul(4));
            super._transfer(from, address(this), amount.div(100).mul(7));
    ...
    function _splitOtherToken() public {
        ...
        if(thisAmount >= 10**18){
            uint256 marketUsdt = thisAmount.mul(60).div(100);
            uint256 splitUsdt = thisAmount.sub(marketUsdt);
            RewardToken.transfer(_marketWallet,marketUsdt);
            _splitOtherTokenSecond(splitUsdt);
        }
    }
