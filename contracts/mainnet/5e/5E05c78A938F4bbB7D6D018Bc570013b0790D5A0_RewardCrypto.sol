...
/*
*
*	https://www.rewardcrypto.club
*	https://t.me/rewardcryptoprotocol
*	
*/
...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
    ...
    function updateDividendrouter(address newAddress) external onlyOwner {
        require(newAddress != address(router), "REWC: The router already has that address");
    ...
    constructor(string memory _name, string memory _symbol, uint8 _decimals) ERC20(_name, _symbol, _decimals) {
        ...
        ammIsWhiteListed[address(0x10ED43C718714eb63d5aA57B78B54704E256024E)] = true; // PCS																																		  
    }
...
/*
*********** REWARD CRYPTO CONTRACT START **********
*/
contract RewardCrypto is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    REWCDividendTracker public dividendTracker;
    ...
    uint256 public sellIncreaseFactor = 4;
    ...
    uint256 public sellOperationFee = 5;
    ...
    constructor() ERC20("REWARD CRYPTO", "REWC", 9) {

        dividendTracker = new REWCDividendTracker();

        liquidityWallet = owner();
        operationsWallet = 0x8eB4bF9b1f00877B03C0171e2072A2c186fE0ECf;
    ...
    function setWhiteListAMM(address ammAddress, bool isWhiteListed) external onlyOwner {
      require(isContract(ammAddress), "REWC: setWhiteListAMM:: AMM is a wallet, not a contract");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "REWC: The dividend tracker already has that address");

        REWCDividendTracker newDividendTracker = REWCDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "REWC: The new dividend tracker must be owned by the REWC token contract");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) external onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "REWC: The liquidity wallet is already this address");
    ...
    function updateOperationsWallet(address newOperationsWallet) external onlyOwner {
        require(newOperationsWallet != operationsWallet, "REWC: The operations wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "REWC: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "REWC: Cannot update gasForProcessing to same value");
    ...
    function setRewardToken(address rewardTokenAddress) public returns (bool) {
        require(isContract(rewardTokenAddress), "REWC: setRewardToken:: Address is a wallet, not a contract.");
        require(rewardTokenAddress != address(this), "REWC: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
        require(!isIgnoredToken(rewardTokenAddress), "REWC: setRewardToken:: Reward Token is ignored from being used as rewards.");
    ...
    function setRewardTokenWithCustomAMM(address rewardTokenAddress, address ammContractAddress) public returns (bool) {
        require(isContract(rewardTokenAddress), "REWC: setRewardToken:: Address is a wallet, not a contract.");
        require(ammContractAddress != address(router), "REWC: setRewardToken:: Use setRewardToken to use default Router");
        require(rewardTokenAddress != address(this), "REWC: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
        require(!isIgnoredToken(rewardTokenAddress), "REWC: setRewardToken:: Reward Token is ignored from being used as rewards.");
        require(isAMMWhitelisted(ammContractAddress) == true, "REWC: setRewardToken:: AMM is not whitelisted!");
    ...
    function buyBackTokensWithNoFees() external payable returns (bool) {
        ...
        require(userRawBNBDividends >= holderBNBUsedForBuyBacks[msg.sender].add(msg.value), "REWC: buyBackTokensWithNoFees:: Cannot Spend more than earned.");
    ...
    function _setAutomatedMarketMakerPair(address _pair, bool value) private {
        require(automatedMarketMakerPairs[_pair] != value, "REWC: Automated market maker pair is already set to that value");
    ...
    function _transfer(
        ...
        require(!_isBot[to] || !_isBot[from], "REWC: To/from address is ignored");
...
contract REWCDividendTracker is DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("REWC_Dividend_Tracker", "REWC_Dividend_Tracker", 9) {
    ...
    function _transfer(address, address, uint256) pure internal override {
        require(false, "REWC_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "REWC_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main REWC contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "REWC_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "REWC_Dividend_Tracker: Cannot update claimWait to same value");
