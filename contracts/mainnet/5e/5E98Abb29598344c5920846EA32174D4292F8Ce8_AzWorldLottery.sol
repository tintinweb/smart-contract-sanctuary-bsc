...
contract AzWorldLottery is ReentrancyGuard, IAzWorldLottery, Ownable {
    ...
    uint256 public maxPriceTicketInazw = 50 ether;
    uint256 public minPriceTicketInazw = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInazw;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] azwPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInazw;
        uint32 finalNumber;
    }
     ...
     * @param _azwTokenAddress: address of the azw token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _azwTokenAddress, address _randomGeneratorAddress) {
        azwToken = IERC20(_azwTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of azw to this contract
        uint256 amountazwToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInazw,
            _ticketNumbers.length
        );

        // Transfer azw tokens to this contract
        azwToken.safeTransferFrom(address(msg.sender), address(this), amountazwToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInazw += amountazwToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInazwToTransfer
        uint256 rewardInazwToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInazwToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        azwToken.safeTransfer(msg.sender, rewardInazwToTransfer);

        emit TicketsClaim(msg.sender, rewardInazwToTransfer, _lotteryId, _ticketIds.length);
    }
    ...
    /**
     * @notice Draw the final number, calculate reward in azw per group, and make lottery claimable
     * @param _lotteryId: lottery id
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInazw) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in azw for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].azwPerBracket[j] =
                ...
                // A. No azw to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].azwPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInazw - amountToShareToWinners);

        // Transfer azw to treasury address
        azwToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in azw token
     * @dev Callable by owner or injector address
     */
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        require(_lotteries[_lotteryId].status == Status.Open, "Lottery not open");

        azwToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInazw += _amount;
     ...
     * @param _priceTicketInazw: price of a ticket in azw
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInazw >= minPriceTicketInazw) && (_priceTicketInazw <= maxPriceTicketInazw),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInazw: _priceTicketInazw,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            azwPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInazw: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(azwToken), "Cannot be azw token");
    ...
    /**
     * @notice Set azw price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInazw: minimum price of a ticket in azw
     * @param _maxPriceTicketInazw: maximum price of a ticket in azw
     */
    function setMinAndMaxTicketPriceInazw(uint256 _minPriceTicketInazw, uint256 _maxPriceTicketInazw)
        external
        onlyOwner
    {
        require(_minPriceTicketInazw <= _maxPriceTicketInazw, "minPrice must be < maxPrice");

        minPriceTicketInazw = _minPriceTicketInazw;
        maxPriceTicketInazw = _maxPriceTicketInazw;
    }
     ...
     * @param _priceTicket price of a ticket (in azw)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].azwPerBracket[_bracket];
        } else {
            return 0;
        }
    }
