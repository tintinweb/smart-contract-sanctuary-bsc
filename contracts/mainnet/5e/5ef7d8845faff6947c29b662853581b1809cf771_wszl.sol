...
contract wszl is Context, IERC20, Ownable {           
    ...
    string private _name = "Honey Badger";    // 代币全称
    string private _symbol = "zl";              // 代币简称符号 
    uint8 private _decimals = 9;             // 精度9， 1.000000000
    
    uint256 public _taxFee = 2;        //持币分红2%，这边是百分比 ，用户交易后产生的费用，给与全网用户收益分红。
    uint256 private _previousTaxFee = _taxFee;
    
    uint256 public _liquidityFee = 3;     //资金池回流3%，这边是百分比 ，用户产生交易后收取3%费用回流底池。
    uint256 private _previousLiquidityFee = _liquidityFee;

    uint256 public _charityFee = 3;                // 营销费用为3%，这边是百分比 。
    ...
    bool public swapAndLiquifyEnabled = true;     //启用交换资金池   不启用为 false
    
    uint256 public _maxTxAmount = 50000000 * 10**6 * 10**9;             //单次\转账\购买最大发送量，设置你发行量就不受限制
    uint256 private numTokensSellToAddToLiquidity = 500000 * 10**6 * 10**9;       //出售\增加资金池最大数量，设置你发行量就不受限制
    ...
    constructor () public {
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);     //  薄饼交易地址，不可更改
