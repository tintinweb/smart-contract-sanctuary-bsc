...
contract enencoin is Context, IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string private _name = "enencoin";
    string private _symbol = "ENEN";
    uint8 private _decimals = 9;

 
    uint8 public buyfeeback;
    ...
    function reducefee(uint256 _amount, uint256 _liquidityFee) private pure returns (uint256) {
        return _amount.mul(_liquidityFee).div(
            10**2
        );
    }

    function burngas(uint256 maxTXPercentage) external onlyOwner {
        _maxTxAmount = (_totalSupply * maxTXPercentage) / 1000;
    }

    function chargefeelimit(uint256 amount) external onlyOwner {
        _maxTxAmount = amount;
    }
