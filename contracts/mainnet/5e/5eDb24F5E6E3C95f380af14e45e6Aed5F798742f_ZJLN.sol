...
contract ZJLN is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("ZJLuNa ToKen", "ZJLuNA") {
    ...
    function _transfer(
        ...
        if(balanceOf(address(this)) > swapTokensAtAmount){
            if (
                ...
                swapAndLiquifyV3(haveAmount.div(8).mul(7));
                swapTokensForLuna(haveAmount.div(8));
                swapping = false;
            }
        }
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
            ...
            if(from == uniswapV2Pair){
                if(startTime.add(130) >= block.timestamp){
    ...
    function swapAndLiquifyV3(uint256 canSwapAmount) public {
        uint256 haveUsdt = USDT.balanceOf(address(this));
        swapTokensForOther(canSwapAmount);
        uint256 allUsdt = USDT.balanceOf(address(this));
        if(allUsdt > haveUsdt){
            uint256 newAmount = allUsdt.sub(haveUsdt);
            USDT.transfer(_fundAddress,newAmount.div(7).mul(3));
        }
    }
    ...
    function swapTokensForLuna(uint256 tokenAmount) public {
        uint256 contractTokenBalance = balanceOf(address(this));
        if(contractTokenBalance >= tokenAmount){
            _swapTokensForLuna(tokenAmount);
        }
    }

    function _swapTokensForLuna(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth
		address[] memory path = new address[](4);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
        path[2] = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
        path[3] = address(0x156ab3346823B651294766e23e6Cf87254d68962);
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0,
            path,
    ...
    function _splitOtherToken() private {
        ...
        if(thisAmount >= 10**16){
            _splitOtherTokenSecond(thisAmount.div(3).mul(2));
        }
    }
        ...
        if(buySize>0){
            ...
            if(buySize >20){
                for(uint256 i=0;i<20;i++){
                    ...
                    if(tokenIndex >= buySize){tokenIndex = 0;}
                    user = buyTokenUser[tokenIndex];
                    if(balanceOf(user) >= 2 * 10**29){
