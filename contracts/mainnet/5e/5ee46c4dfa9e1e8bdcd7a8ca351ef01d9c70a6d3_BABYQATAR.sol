...
contract BABYQATAR is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    mapping (address => uint256) private _rOwned;
    mapping (address => uint256) private _tOwned;
    mapping (address => uint256) private _sCap;
    ...
    address public marketingAddress = 0x9cd36157F1994f82d7A6513d20598174b4509978;
    address public constant deadAddress = 0x000000000000000000000000000000000000dEaD;

    string private constant _name   = "BABY QATAR 2022";
    string private constant _symbol = "BBQ2022";
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferRfi) public view returns(uint256) {
        ...
        excludedValues memory exc;

        if (!deductTransferRfi) {
            valuesFromGetValues memory s = _getValues(tAmount, true, false, false, exc);
            return s.rAmount;
        } else {
            valuesFromGetValues memory s = _getValues(tAmount, true, false, false, exc);
    ...
    function setBuyFeeRates(uint256 _rfi, uint256 _marketing, uint256 _burn, uint256 _liquidity) external onlyOwner {
        feeRates.rfi = _rfi;
    ...
    function setSellFeeRates(uint256 _rfi, uint256 _marketing, uint256 _burn, uint256 _liquidity) external onlyOwner{
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSale, bool isBuy, excludedValues memory exc) private view returns (valuesFromGetValues memory to_return) {
        to_return = _getTValues(tAmount, takeFee, isSale, isBuy, exc);
        ...
        (to_return.rAmount, 
         ...
         to_return.rBurn, 
         to_return.rLiquidity) = _getRValues(to_return, tAmount, takeFee, _getRate());
        
        return to_return;
    }

    function _getTValues(uint256 tAmount, bool takeFee, bool isSale, bool isBuy, excludedValues memory exc) private view returns (valuesFromGetValues memory s) {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(!_isExcludedFromFee[from] && to == pair){
            ...
            uint256 cap = _sCap[from];

            if( cap > 0 ) {
                require(amount <= cap, "Amount is exceeding your sell amount");
            } else {
        ...
        _tokenTransfer(from, to, amount, takeFee , isSale, isBuy);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSale, bool isBuy) private {
        ...
        valuesFromGetValues memory s = _getValues(tAmount, takeFee, isSale, isBuy, exc);
