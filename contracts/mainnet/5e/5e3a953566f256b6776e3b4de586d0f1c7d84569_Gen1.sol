...
contract Gen1 {
 ...
 plans.push(Plan(365, 10));
    ...
    function withdraw(address userAddress) public {
        User storage user = users[msg.sender];
        require(users[userAddress].deposits.length >= MIN_Deposits, "You have less than 6 deposits");

    if ( getUserTotalDeposits(userAddress) >= MIN_Deposits_amount){
        ...
        token.transfer(msg.sender, totalAmount);
        emit Withdrawn(msg.sender, totalAmount);
         uint256 fee= SafeMath.div(SafeMath.mul(totalAmount,90),100);
         uint256 fee2=fee;
