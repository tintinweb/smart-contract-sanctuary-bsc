...
contract WaifuReroller is Auth {
    ...
    function reroll(uint256 waifuID, bool check, bool lock) external {
        IShoujoStats(_shoujoStats).reroll(waifuID,check,lock);

        uint256 rarity = checkRarity(waifuID);
        bool isLegend = rarity == 4;
        string memory errorMessage = uint2str(rarity);
        require(isLegend, errorMessage);
    }

    function checkRarity(uint256 waifuID) public view returns (uint256) {
