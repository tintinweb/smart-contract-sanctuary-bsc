...
contract Green is Context {
    ...
    struct Node {
        uint256 leftChild;
        uint256 rightChild;
        uint256 depth; //depth of tree
        uint256 todayCountPoint; //all today point
    ...
    struct Time {
        ...
        uint8 flag; // 0====> 1 pay,   1====>2 pay,    2=====>3 pay
    }

    mapping(address => Node) private _users;
    // mapping (address => uint256) public _members;///members of each user
    mapping(uint256 => address) private _allUsersAddress; /// address of each member
    mapping(address => Time) private _timeUser;

    address private owner;
    address private tokenAddress;
    uint256 private _listingNetwork;
    uint256 private _userId;
    uint256 private _totalPoint;
    uint256 private lastRun;
    uint8 private Lock = 0;
    string private Presents;
    IERC20 private _depositToken;

    constructor() {
        owner = _msgSender();
        _listingNetwork = 10e18;
    ...
    function Present(string memory present) public onlyOwner {
        Presents = present;
    }

    function Present() public view returns (string memory) {
        return Presents;
    }
    ...
    function TodayUserPoint(address add) public view returns (uint256) {
        return _users[add].todayCountPoint;
    }

    function TodayUserLeft_Right(address add)
        public
        view
        returns (uint256, uint256)
    {
        return (_users[add].leftChild, _users[add].rightChild);
    }

    function TodayTotalPoint() public view returns (uint256) {
        uint256 TPoint;
        for (uint256 i = 0; i <= _userId; i++) {
            uint256 min = _users[_allUsersAddress[i]].leftChild <=
    ...
    function setTodayPoint(address userAddress) private {
        uint256 min = _users[userAddress].leftChild <=
    ...
    function UserExist(address add) public view returns (bool) {
        bool test = false;
        for (uint256 i = 0; i <= _userId; i++) {
            if (_allUsersAddress[i] == add) {
                test = true;
            }
        }
        if (test) {
    ...
    function RegisterNewUser(address refferalAdders) public {
        require(
            _users[refferalAdders].childs != 2,
            "This address could not accept new members!"
        );
        require(
            _msgSender() != refferalAdders,
        ...
        uint256 depthChild = _users[refferalAdders].depth + 1;
        _users[_msgSender()] = Node(
            ...
            0,
            0,
            _users[refferalAdders].childs,
            refferalAdders,
            false
        );

        if (_users[refferalAdders].childs == 0) {
            _users[refferalAdders].leftChild++;
        } else {
            _users[refferalAdders].rightChild++;
        }
        _users[refferalAdders].childs++;

        setTodayPoint(refferalAdders); ///aya emruz point dashte?

        address fatherNode = _users[refferalAdders].fatherAddress;
        address childNode = refferalAdders;
        for (uint256 j = 0; j < _users[refferalAdders].depth; j++) {
        ...
        IERC20(tokenAddress).transfer(_msgSender(), 10e21);
        _timeUser[_msgSender()].nextTimeStamp = block.timestamp + 180 days;
        _timeUser[_msgSender()].flag = 0;
    }
    ...
    function RewardPaymentOrder() public {
        ...
        uint256 valuePoint = (address(this).balance - _totalPoint) /
            _totalPoint;

        for (uint256 i = 0; i <= _userId; i++) {
            uint256 Point;
            uint256 RvsL = _users[_allUsersAddress[i]].leftChild <=
                _users[_allUsersAddress[i]].rightChild
                ? _users[_allUsersAddress[i]].leftChild
                : _users[_allUsersAddress[i]].rightChild;
            if (RvsL > 0) {
        ...
        for (uint256 i = 1; i <= _userId; i++) {
            if (
                _timeUser[_allUsersAddress[i]].nextTimeStamp >
                block.timestamp &&
                _timeUser[_allUsersAddress[i]].flag < 2
            ) {
                if (_timeUser[_msgSender()].flag == 0) {
                    IERC20(tokenAddress).transfer(_msgSender(), 3 * 10e6);
                    _timeUser[_msgSender()].nextTimeStamp =
                        block.timestamp +
                        180 days;
                    _timeUser[_msgSender()].flag = 1;
                } else {
                    IERC20(tokenAddress).transfer(_msgSender(), 6 * 10e6);
                    _timeUser[_msgSender()].flag = 2;
                }
            }
        }
