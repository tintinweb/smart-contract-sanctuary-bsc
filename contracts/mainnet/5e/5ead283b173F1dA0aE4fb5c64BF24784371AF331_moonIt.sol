...
contract moonIt is IERC20, Ownable {
    ...
    string private _name = "MoonIt1";
    string private _symbol = "Moon1";
    ...
    uint256 public _liquidityFee = 0; // Liquidity Fee + Marketing Fee + Dev Fee 
    ...
    event UpdatedFees(uint256 liquidityFee, uint256 burnFee, uint256 taxFee);
    ...
    /**
     * @dev Update the fees for the contract. Liquidity fee must be set to LP fee PLUS BurnFee.
     * e.g Burn = 1, Liquidity = 1.... Liquidity MUST be set to 2 (1+1)
     */
    function updateFees(uint256 LiquidityFee, uint256 BurnFee, uint256 TaxFee) public onlyOwner {
        require(LiquidityFee + TaxFee <= 6, "Total taxes must be under 12%");
        _liquidityFee = LiquidityFee;
        _burnFee = BurnFee;
        _taxFee = TaxFee;

        emit UpdatedFees(_liquidityFee, _burnFee, _taxFee);
    }
