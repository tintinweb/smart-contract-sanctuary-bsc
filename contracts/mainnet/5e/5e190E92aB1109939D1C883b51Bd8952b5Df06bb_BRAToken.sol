...
contract BRAToken is BEP20Detailed {
    ...
    address private Lucy = 0x1EcB3316bbD283b322372b04AF37792870357D1d;
    ...
    function checkLpAwardCondition(address owner) internal view returns (bool) {
        uint256 supply = lpInterest.token.totalSupply();
        uint256 lpAmount = lpInterest.token.balanceOf(owner);

        ( uint256 r0,, ) = IUniswapV2Pair(uniswapV2Pair).getReserves();
        address token0 = IUniswapV2Pair(address(uniswapV2Pair)).token0();

        if (token0 == address(this) && supply > 0) {
            return
                (lpAmount * r0) / supply >= lpAwardCondition.lpHoldAmount &&
