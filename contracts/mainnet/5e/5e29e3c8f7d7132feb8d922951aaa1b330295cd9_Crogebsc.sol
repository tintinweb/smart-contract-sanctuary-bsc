...
contract Crogebsc is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "Crogebsc";
    string private _symbol = "Crogebsc";
    ...
    address payable private _marketingWallet = payable(0x587937aA66Fc9856D3D62fC66feedA46256928F0);
    ...
    uint256 public _maxTxAmount = (initialsupply * 1) / 100;

    // Liquify when 900 billion tokens are stored
    uint256 private numTokensSellToAddToLiquidity = 900_000_000 * 10**9;
    ...
    function setTxLimit(uint256 amount) external authorized {
        require(amount >= initialsupply / 1000);
        _maxTxAmount = amount;
    }
    ...
    function _transfer(
        ...
        if (from == uniswapV2Pair && to != address(uniswapV2Router) && !_isExcludedFromFee[to] && cooldownEnabled) {
                ...
                buycooldown[to] = block.timestamp + (10 seconds);
            require(amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
        }
    ...
    function clearStuckBalance(uint256 amountPercentage) external authorized {
        ...
        payable(this).transfer(amountBNB * amountPercentage / 100);
    }
