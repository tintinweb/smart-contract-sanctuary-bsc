...
contract CryptoDrift_V3 is ERC721Upgradeable, ERC721EnumerableUpgradeable, AccessControlUpgradeable,PausableUpgradeable, ERC721URIStorageUpgradeable,UUPSUpgradeable {
    ...
    function useMysteryKey(uint256 count) external {
        ...
        for(uint256 i=0;i < count;i++){
            ...
            if(carType > 23 && carType < 29){
                if((carGenesis2SaleSupply - 1) < 0){
                    carType = 1;
                }else{
                    carGenesis2SaleSupply = carGenesis2SaleSupply - 1;
                }
            }
            CAR_TYPES[id] = carType;
            _safeMint(msg.sender, id);
