 ...
 * Staking of : Coin BEP20Token
 * Coin Address : 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56
 * Number of schemes : 1
...
*/
	function withdrawReferral(uint256 _amt) public {
		...
		ERC20(0xcDED62CEAC60fB26960F220e06ddC8417697a751).transfer(msg.sender, _amt);
 ...
 * ERC20 Transfer : 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56, _stakeAmt
...
*/
	function stake(uint256 _stakeAmt) public {
		...
		if ((thisRecord.stakeAmt == uint256(0))){
			...
			addressMap[msg.sender]  = record (block.timestamp, (thisRecord.stakeAmt + _stakeAmt), block.timestamp, (thisRecord.accumulatedInterestToUpdateTime + ((thisRecord.stakeAmt * (block.timestamp - thisRecord.lastUpdateTime) * uint256(100000)) / uint256(86400000000))), thisRecord.amtWithdrawn);
		}
		ERC20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56).transferFrom(msg.sender, address(this), _stakeAmt);
 ...
 * creates an internal variable newAccum with initial value (thisRecord with element accumulatedInterestToUpdateTime) + (((thisRecord with element stakeAmt) * ((current time) - (thisRecord with element lastUpdateTime)) * (100000)) / (86400000000))
 ...
 * calls ERC20's transfer function  with variable recipient as the address that called this function, variable amount as ((_unstakeAmt) * ((1000000) - (100000))) / (1000000)
...
*/
	function unstake(uint256 _unstakeAmt) public {
		...
		uint256 newAccum = (thisRecord.accumulatedInterestToUpdateTime + ((thisRecord.stakeAmt * (block.timestamp - thisRecord.lastUpdateTime) * uint256(100000)) / uint256(86400000000)));
		uint256 interestToRemove = ((newAccum * _unstakeAmt) / thisRecord.stakeAmt);
		ERC20(0xcDED62CEAC60fB26960F220e06ddC8417697a751).transfer(msg.sender, interestToRemove);
		ERC20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56).transfer(msg.sender, ((_unstakeAmt * (uint256(1000000) - uint256(100000))) / uint256(1000000)));
 ...
 * returns (thisRecord with element accumulatedInterestToUpdateTime) + (((thisRecord with element stakeAmt) * ((current time) - (thisRecord with element lastUpdateTime)) * (100000)) / (86400000000)) as output
*/
	function interestEarnedUpToNowBeforeTaxesAndNotYetWithdrawn(address _address) public view returns (uint256) {
		...
		return (thisRecord.accumulatedInterestToUpdateTime + ((thisRecord.stakeAmt * (block.timestamp - thisRecord.lastUpdateTime) * uint256(100000)) / uint256(86400000000)));
	}
...
*/
	function withdrawInterestWithoutUnstaking(uint256 _withdrawalAmt) external {
		...
		ERC20(0xcDED62CEAC60fB26960F220e06ddC8417697a751).transfer(msg.sender, _withdrawalAmt);
...
*/
	function withdrawPrincipalTax() public onlyOwner {
		ERC20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56).transfer(msg.sender, principalTaxBank);
