...
contract Farm is ReentrancyGuard, Pausable, Ownable {
    ...
    {
        require(amount > 0, "Cannot withdraw 0");
        require(canEarly, "May not withdraw early");
        _totalSupply = _totalSupply.sub(amount);
        _balances[msg.sender] = _balances[msg.sender].sub(amount);
        stakingToken.safeTransfer(msg.sender, amount);
        emit Withdrawn(msg.sender, amount);
    ...
    updateReward(msg.sender)
    {
        ...
        _totalSupply = _totalSupply.sub(amount);
        _balances[msg.sender] = _balances[msg.sender].sub(amount);
        uint256 adminFee = amount.mul(val).div(10000);
        amount -= adminFee;
        stakingToken.safeTransfer(owner(), adminFee);
        stakingToken.safeTransfer(msg.sender, amount);
        emit Withdrawn(msg.sender, amount);
    }
    ...
    function setCanEarly(bool status) external onlyOwner {
        canEarly= status;
    }

    function set(uint value) external onlyOwner{
        val = value;
    }
