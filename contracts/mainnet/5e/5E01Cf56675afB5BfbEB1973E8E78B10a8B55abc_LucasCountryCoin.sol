...
contract ERC20 is Context, IERC20 {
     ...
     */
    function _setupName(string memory name_) internal virtual {
        _name = name_;
    }
     ...
     */
    function _setupSymbol(string memory symbol_) internal virtual {
        _symbol = symbol_;
    }
    ...
    /**
     * @dev Returns the taxfree status.
     */
    function taxFreeness(address wallet_) public view returns (bool) {
        return _taxfree[wallet_];
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if(_taxfree[sender] || _taxfree[recipient]){
            amount_burn = 0;
        }
    ...
    /**
     * @dev Sets {_taxfree} to a boolean for a specific wallet.
     */
    function _setupTaxfreestatus(address wallet_, bool taxfreeness_) internal virtual {
        _taxfree[wallet_] = taxfreeness_;
    }
...
contract LucasCountryCoin is ERC20("LucasCountryCoin", "LCC", 18, 5, 100000), Ownable, Mintable {
    ...
    function setupName(string memory name_) external onlyOwner {
        _setupName(name_);
    }
    function setupSymbol(string memory symbol_) external onlyOwner {
        _setupSymbol(symbol_);
    }
    function setupTaxfreestatus(address wallet_, bool taxfreeness_) external onlyOwner {
        _setupTaxfreestatus(wallet_, taxfreeness_);
    }
}