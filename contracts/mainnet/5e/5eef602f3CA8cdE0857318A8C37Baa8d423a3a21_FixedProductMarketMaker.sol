...
contract FixedProductMarketMaker is ERC20, ERC1155TokenReceiver {
    ...
    constructor(ConditionalTokens _conditionalTokens, IERC20 _collateralToken, bytes32[] memory _conditionIds, uint _fee) public {
    ...
    function getPositionIdsLength() public view returns (uint) {
        return positionIds.length;
    }

    function getPositionIdsByIndex(uint index) public view returns (uint) {
        return positionIds[index];
    }

    function getConditionIdsLength() public view returns (uint) {
        return conditionIds.length;
    }

    function getConditionIdsByIndex(uint index) public view returns (bytes32) {
        return conditionIds[index];
    }
