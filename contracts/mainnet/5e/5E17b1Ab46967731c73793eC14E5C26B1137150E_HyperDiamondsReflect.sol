...
/**
 Telegram: https://t.me/HyperDiamondsReflect
...
contract HyperDiamondsReflect  is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    HyperDiamondsReflectDividendTracker public dividendTracker;
    ...
    uint256 public cooldowntimer = 120; //default cooldown 120s
    ...
    constructor() ERC20("HyperDiamondsReflect ", "HYPER2.0") {
        ...
        dividendTracker = new HyperDiamondsReflectDividendTracker(
            payable(this),
            router,
            0xd7986962338b0Dd2f05a8688A0e3B03Ebc9554c7,
            "HyperDiamondsReflectTRACKER",
            "HYPER2.0TRACKER"
        );
        ...
        uint256 totalTokenSupply = (100000_000) * (10**18);
        _mint(owner(), totalTokenSupply); // only time internal mint function is ever called is to create supply
        maxWallet = totalTokenSupply / 4000; // 0.025%
        swapTokensAtAmount = totalTokenSupply / 4000; // 0.025%;
    ...
    function setmaxWallet(uint256 value) external onlyOwner {
        value = value * (10**18);
        require(value >= _totalSupply / 4000, "max wallet cannot be set to less than 0.025%");
        maxWallet = value;
    }
...
contract HyperDiamondsReflectDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    HyperDiamondsReflect public HyperDiamondsReflectContract;
    ...
    constructor(
        ...
        defaultToken = token;
        HyperDiamondsReflectContract = HyperDiamondsReflect(mainContract);
    ...
    function _transfer(
        ...
        require(false, "HyperDiamondsReflect_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "HyperDiamondsReflect_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main HyperShards contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(HyperDiamondsReflectContract);

            uint256 prevBalance = HyperDiamondsReflectContract.balanceOf(address(this));
            ...
            {
                uint256 received = HyperDiamondsReflectContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    HyperDiamondsReflectContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
