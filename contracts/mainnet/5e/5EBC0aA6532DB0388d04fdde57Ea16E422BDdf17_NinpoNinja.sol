...
contract NinpoNinja is Context, IERC20, Ownable, Pausable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "Ninpo Ninja";
    string private _symbol = "Ninja";
    uint8 private _decimals = 18;

    address payable public FeeAddress = payable(0x2459B31a40f005E996463a3FD18c3fFB0b14937B);
    ...
    function updateFee(uint256 newTxfee, uint256 newBurnFee) external onlyOwner() {
        txFee = newTxfee;
        burnFee = newBurnFee;
    }
    ...
    function _transfer(address sender, address recipient, uint256 _value) private returns (bool) {
        ...
        require(_balances[recipient] + _value <= _walletMax, "Exceeds maximum wallet token amount" );

        require(_value <= _balances[sender]);
    ...
    function withdrawForeignToken(address _tokenContract, uint256 _amount) external onlyOwner{
