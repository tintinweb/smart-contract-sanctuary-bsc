...
contract TechTreesCoin is ERC20, Ownable, AccessControl {
    ...
    constructor(address _royaltyReceiver) ERC20("Tech Trees Coin", "TTC") {
    ...
    function transfer(address to, uint256 amount) public override returns (bool) {
        ...
        require(enableTransfer || transferWhitelist[from] || from == owner(), "Unable to transfer");
        require(!transferBlist[from], "Unable to transfer");

        if (royaltyWhitelist[from] || royaltyWhitelist[to]) {
            _transfer(from, to, amount);
        } else {
    ...
    function setEnableTransfer(bool _enableTransfer, address _uniswapV2Pair) external onlyOwner {
