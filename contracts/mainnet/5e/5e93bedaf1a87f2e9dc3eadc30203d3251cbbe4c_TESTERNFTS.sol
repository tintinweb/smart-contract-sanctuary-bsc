...
contract TESTERNFTS is Auth, ERC721, ERC721Enumerable {
    ...
    uint256 ops_delta = 200;
    uint256 opp_delta = 125;
    uint256 opr_delta = 125;
    uint256 opt_delta = 250;
    ...
    // bnb price per mint
    uint256 pLevelOne = 5000000000000000;
    uint256 pLevelTwo = 7500000000000000;
    uint256 pLevelThree = 10000000000000000;
    uint256 pLevelFour = 12500000000000000;
    uint256 pLevelFive = 15000000000000000;

    // token price per mint
    uint256 tLevelOne = 3000000000000000;
    uint256 tLevelTwo = 5000000000000000;
    uint256 tLevelThree = 8000000000000000;
    uint256 tLevelFour = 10000000000000000;
    uint256 tLevelFive = 12000000000000000;

    //set specifics of token
    uint256 tokenTax = 9;
    uint256 taxDenominator = 100;
    address tokenAddress = 0xb6Fde0eC0f69425AB54672dbeeDAeFb1cEcD3101;
    ...
    // balances
    uint public _balanceDistributor;
    uint public _balanceBNBDistributor;
    uint public _balanceReflections;
    ...
    function _allEarnedReflection(address tokenOwner) public view returns (uint) {
        ...
        uint taxinverse = taxDenominator - tokenTax;
        uint taxedEarned = _allEarned * taxinverse / taxDenominator;
        return taxedEarned;
    }

    function _allNonTaxEarnedReflection(address tokenOwner) public view returns (uint) {
        uint[] memory _tokens = _tokensByOwner(tokenOwner);
        if (_tokens.length == 0) {
            return 0;
        }
    ...
    function _claimAllMintRewards(address tokenOwner) internal {
        ...
        IBEP20(token).transfer(_msgSender(), _allEarned);
    ...
    function teamTokenPayout() public authorized {
        uint p = _balanceDistributor;
        if(p > 0){
        IBEP20(token).transfer(mark, (p*mark_delta)/den_delta);
        IBEP20(token).transfer(alpha, (p*ops_delta)/den_delta);
        IBEP20(token).transfer(delta, (p*opp_delta)/den_delta);
        IBEP20(token).transfer(omega, (p*opr_delta)/den_delta);
        IBEP20(token).transfer(lottery, (p*opt_delta)/den_delta);
        _balanceDistributor = 0;}
    }

    function teamBNBPayout() public authorized {
        uint p = _balanceBNBDistributor;
        if(p > 0){
        ...
        payable(lottery).transfer((p*opt_delta)/den_delta);
        _balanceBNBDistributor = 0;}
    }

    function teamTotalPayout() public authorized {
        uint b = _balanceBNBDistributor;
        if(b > 0){
        payable(mark).transfer((b*mark_delta)/den_delta);
        payable(alpha).transfer((b*ops_delta)/den_delta);
        payable(delta).transfer((b*opp_delta)/den_delta);
        payable(omega).transfer((b*opr_delta)/den_delta);
        payable(lottery).transfer((b*opt_delta)/den_delta);
        _balanceBNBDistributor = 0;}
        uint p = _balanceDistributor;
        if(p > 0){
        IBEP20(token).transfer(mark, (p*mark_delta)/den_delta);
    ...
    function setDeltas(uint256 _mark, uint256 _ops, uint256 _opp, uint256 _opr, uint256 _opt) external authorized {
        mark_delta = _mark;
        ops_delta = _ops;
        opp_delta = _opp;
        opr_delta = _opr;
    ...
    function getreflection() public view returns (uint256) {
        uint256 reflection;
        uint256 taxinverse = taxDenominator - tokenTax;
        uint256 tPrice = calculatetokenPrice() * taxinverse / taxDenominator;
        reflection = (tPrice * _mintSharesAllotment) / (_mintDenominator);
        return reflection;
    }
     ...
     */
    function mintTokenBNB(address recipient, uint amount, uint payment) internal {
        _splitMintFees(payment, recipient);
        uint tokenId;
        for (uint i = 0; i < amount; i++) {
            ...
            if (payment > 0) {
                _totalDividend += (_userDeposit[recipient] / tokenId);}
    ...
    function _splitMintFees(uint payment, address recipient) internal {
        uint256 cBalance = token.balanceOf(address(this));
        if (payment > 0) {
            uint reflect = (payment * _mintSharesAllotment) / (_mintDenominator);
            buyTokens(reflect);
            uint256 aBalance = token.balanceOf(address(this));
            uint256 _reflect = aBalance - cBalance;
            _userDeposit[recipient] = _reflect;
            _balanceReflections += _reflect;
            _balanceBNBDistributor += payment - reflect;
        }
    }

    function _splitTokenMintFees(uint payment) internal {
        IBEP20(token).transferFrom(_msgSender(), address(this), payment);
        uint256 taxinverse = taxDenominator - tokenTax;
        uint256 tPayment = payment * taxinverse / taxDenominator;
        if (tPayment > 0) {
            uint reflect = (tPayment * _mintSharesAllotment) / (_mintDenominator);
            _balanceReflections += reflect;
            _balanceDistributor += tPayment - reflect;
        }
    }

    function mintBNB(uint amount) public payable whenMintingAllowed {
        ...
        mintTokenBNB(_msgSender(), amount, msg.value);
    }

    function mintToken(uint amount, uint256 tokenAmt) public whenMintingAllowed {
        require(((_tokenIds.current() + amount) <= _maxSupply) && (amount <= _maxMintPerTx) && (token.balanceOf(_msgSender()) >= tokenAmt) && (tokenAmt >= calculatetokenPrice() * amount) && _mintedWallet[_msgSender()] + amount <= _maxMintPerWallet, "Mint failed");
        mintToken(_msgSender(), amount, tokenAmt);
    }
    ...
    function setBNBPrices(uint256 _plevel1, uint256 _plevel2, uint256 _plevel3, uint256 _plevel4, uint256 _plevel5) external authorized {
        pLevelOne = _plevel1;
        pLevelTwo = _plevel2;
    ...
    function setTokenPrices(uint256 _tlevel1, uint256 _tlevel2, uint256 _tlevel3, uint256 _tlevel4, uint256 _tlevel5) external authorized {
        tLevelOne = _tlevel1;
        tLevelTwo = _tlevel2;
        tLevelThree = _tlevel3;
        tLevelFour = _tlevel4;
        tLevelFive = _tlevel5;
    }

    function setTokenTax(uint256 _tax, uint256 _denominator) external authorized {
        tokenTax = _tax;
        taxDenominator = _denominator;
    }
    ...
    function rescueBEP20(address _token, address _rec, uint256 _amt) external authorized {
        ...
        IBEP20(_token).transfer(_rec, tamt * (_amt) / (100));
    }
    ...
    function cBalances() public view returns (uint, uint, uint) {
        return (address(this).balance, _balanceReflections, _balanceBNBDistributor);
    }

    function tBalances() public view returns (uint, uint, uint) {
        return (token.balanceOf(address(this)), _balanceReflections, _balanceDistributor);
    }
