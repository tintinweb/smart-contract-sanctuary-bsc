...
// File: contracts\SingleFitswapNft.sol

// Single FitSwap token

pragma solidity ^0.8.0;


contract SingleFitswapNft is ERC721 {
    ...
    constructor() ERC721("SingleFitswapNft","SingleFitswapNft") {
        factory = msg.sender;
    }
...
// File: contracts\SingleFitswapNftFixed.sol

// Single FitSwap Fixed Price Marketplace contract
...
contract SingleFitswapNftFixed is Ownable, ERC721Holder {
    using SafeMath for uint256;
	using EnumerableSet for EnumerableSet.AddressSet;
	...
	function createCollection(string memory _name, string memory _uri, bool bPublic) public returns(address collection) {
		bytes memory bytecode = type(SingleFitswapNft).creationCode;
        ...
        ISingleFitswapNft(collection).initialize(_name, _uri, msg.sender, bPublic);
    ...
    function singleList(address _collection, uint256 _tokenId, address _tokenAdr, uint256 _price) OnlyItemOwner(_collection,_tokenId) public {
		...
		ISingleFitswapNft nft = ISingleFitswapNft(_collection);        
    ...
    function singleDelist(uint256 _id) external {        
        ...
        ISingleFitswapNft(pairs[_id].collection).safeTransferFrom(address(this), pairs[_id].owner, pairs[_id].tokenId);        
    ...
    function singleBuy(uint256 _id) external payable {
		...
		ISingleFitswapNft(pairs[_id].collection).safeTransferFrom(address(this), msg.sender, pair.tokenId);
	...
	modifier OnlyItemOwner(address tokenAddress, uint256 tokenId){
        ISingleFitswapNft tokenContract = ISingleFitswapNft(tokenAddress);
