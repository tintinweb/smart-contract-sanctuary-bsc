...
contract VeCakeStaker is ERC20, ReentrancyGuard, CakeChefManager {
    ...
    function harvestAndDepositCake() public {
        ...
        if (totalCakes() > 0) {
            ...
            if (cakeBalance > required) {
                ...
                veCake.deposit(timelockableCakes, DURATION);
            } else {
                uint256 minDeposit = veCake.MIN_DEPOSIT_AMOUNT();
                if (cakeBalance > minDeposit) {
                    veCake.deposit(minDeposit, DURATION);
                }
            }
        } 
    }
    ...
    function balanceOfCakeInVe() public view returns (uint256) {
        (uint256 shares,,,,,,,,) = veCake.userInfo(address(this));
        return shares * veCake.getPricePerFullShare();
    }
    ...
    function lockInfo() external view returns (uint256 endLock, uint256 lockRemaining) {
...
contract CakeChefManager is Ownable, Pausable {
    ...
    bytes32 public id = bytes32("cake.eth");
