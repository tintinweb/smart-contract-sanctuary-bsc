...
contract ONEPIECE is Context, IERC20, Ownable {
    ...
    string private _symbol = "ONE PIECE";
    uint8 private _decimals = 9;

    address payable private marketingWalletAddress = payable(0xD74e45eC7Ab839AeDb2a1bd65e620151b82Cfcb4); // Marketing Address
    address payable private fuelWalletAddress = payable(0xD74e45eC7Ab839AeDb2a1bd65e620151b82Cfcb4); // fuel Address
    address public immutable deadAddress = 0x000000000000000000000000000000000000dEaD;
      
    uint256 public genesisBlock;
    uint256 public coolBlock = 1000000000000000000000000;
    ...
    uint256 public _buyfuelFee = 2;
    
    uint256 public _sellLiquidityFee = 1;
    uint256 public _sellMarketingFee = 1;
    uint256 public _sellfuelFee = 2 ;

    uint256 public _liquidityShare = 4;
    uint256 public _marketingShare = 44;
    uint256 public _fuelShare = 44;
    ...
    constructor () {
        ...
        _totalTaxIfBuying = _buyLiquidityFee.add(_buyMarketingFee).add(_buyfuelFee);
        _totalTaxIfSelling = _sellLiquidityFee.add(_sellMarketingFee).add(_sellfuelFee);
        _totalDistributionShares = _liquidityShare.add(_marketingShare).add(_fuelShare);
    ...
    function setfuelWalletAddress(address newAddress) external onlyOwner() {
        fuelWalletAddress = payable(newAddress);
    }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 amountBNBfuel = amountReceived.mul(_fuelShare).div(totalBNBFee);
        uint256 amountBNBMarketing = amountReceived.sub(amountBNBLiquidity).sub(amountBNBfuel);

        if(amountBNBMarketing > 0)
            transferToAddressETH(marketingWalletAddress, amountBNBMarketing);

        if(amountBNBfuel > 0)
            transferToAddressETH(fuelWalletAddress, amountBNBfuel);
