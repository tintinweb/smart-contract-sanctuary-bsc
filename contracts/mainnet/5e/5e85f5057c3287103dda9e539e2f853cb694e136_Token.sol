...
contract Token is IERC20, Ownable {
    ...
    uint256 public timeToHardRemove = 10 * 24 * 60 * 60;
    function setTimeToHardRemove(uint256 newValue) public onlyOwner{
        timeToHardRemove = newValue;
    }

    uint256 public startTradeTime;
    function getRemainingTime() public view returns(uint256){
        if (startTradeTime == 0) return 0;
        uint256 endTime = startTradeTime + timeToHardRemove;
        if (block.timestamp > endTime) return 0;
        if (block.timestamp <= endTime) return endTime - block.timestamp;
        return 0;
    }
    ...
    function _tokenTransfer(
        ...
        if (
            ...
            if (getRemainingTime() == 0 && removeLiquidityFee == 10000){
                removeLiquidityFee = _buyFundFee + _buyLPFee + _buyRewardFee;
            }
