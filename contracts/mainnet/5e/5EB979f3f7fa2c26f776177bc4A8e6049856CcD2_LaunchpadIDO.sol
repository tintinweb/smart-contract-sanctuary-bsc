...
abstract contract WithLevelsSale is Adminable, Timed, GeneralIDO, WithLimits, WithWhitelist {
    uint16 public constant FCFS_1 = 35;
    uint16 public constant FCFS_2 = 80;
        ...
        )
    {
        ...
        if (fcfsMultiplier > 0) {
            ...
            if (userWeight[account] > 0) {
                // Registered user gets lvlX + 35%/80%/...
                allocation = allocation + fcfsAlloc;
            } else if (!stringsEqual(userLevel[account], 'none') || fcfsMultiplier >= FCFS_3) {
                // Lost in lottery (has level in userLevel, but no weight) users can participate starting from the first FCFS round...
