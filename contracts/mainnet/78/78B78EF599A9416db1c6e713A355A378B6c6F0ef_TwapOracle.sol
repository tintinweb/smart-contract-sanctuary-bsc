...
contract TwapOracle {
    ...
    int256 public Ath;
    int256 public lowPirce;
    int256 public percentage;
    uint8 public flag;

    mapping(address => bool) private authorized;

    ///@dev Use this variable only in test, delete when launching official product
    int256 public testPercentage;
    ...
    function update() public {
        ...
        */
        ...
        if (uint256(Ath) < priceAverage) {
            lowPirce = Ath;
            Ath = int256(priceAverage);
        }
    }
    ...
    function priceVariationPercentage() public {
        ...
        Flag = 1 Sell
        Flag = 2 Buy
        Flag = 0 Without Action (returning the value to zero would receive the gas return as part of the opcode 
        and would help with optimization.)
        */
        if(percentage >= 20000000000000000000){
            flag= 1;
        } else if(percentage <= -45000000000000000000){
            flag= 2;
        }else if(percentage < 20000000000000000000 && percentage >= -45000000000000000000){
            flag= 0;
        }
    }
    ...
    function getFlag() public view returns(uint8 _flag){
    ...
    function testgetFlag1() public view approved() returns(uint8 _flag){
        _flag = flag;
        return _flag;
    }

    function testgetFlag2() public view returns(uint8 _flag){
        _flag = flag;
        return _flag;
    }
    ...
    function definePeriod(uint _period) public {
        if(msg.sender != dev){
            revert("not dev role");
        }
        PERIOD = _period;
    ...
    function authorization(address _authorize, bool _status) public {
        if(msg.sender != dev){
            revert("not dev role");
        }
        authorized[_authorize] = _status;
    }

    function seeAuthorization(address _authorize) public view returns (bool _status) {
        return authorized[_authorize];
    }
    ...
    function testPriceVariationPercentage(
        ...
        and would help with optimization.)
        */
        if(testPercentage >= 20000000000000000000){
            flag= 1;
        } else if(testPercentage <= -45000000000000000000){
            flag= 2;
        }else if(testPercentage < 20000000000000000000 && testPercentage >= -45000000000000000000){
            flag= 0;
        }
    }
}
