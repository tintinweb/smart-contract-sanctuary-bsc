...
contract MRstakePool3 is Context, Ownable {
    ...
    function claimRewardToken() public {
        ...
        uint256 amountToWithdraw = _claimableRewardToken(_msgSender());
        ...
        _rewardToken.transfer(_msgSender(), amountToWithdraw);
        emit RewardTokenWithdraw(_msgSender(), amountToWithdraw);
    }
    ...
    function deposit(uint256 amount, uint256 apyTime, bytes32 _hashedMessage, uint8 _v, bytes32 _r, bytes32 _s) public {
        require(verifyMessage(_hashedMessage, _v, _r, _s, _msgSender()), "Signed message does not match");
        if(pool.stakeSize > 0) {
    ...
    function claimStaking() public {
        ...
        _claimableRewardToken(_msgSender());
        _updateStakerSize(_msgSender(), stakers[_msgSender()]);
