...
contract Spooky is IERC20 {
    ...
    uint256 constant private startingSupply = 1_000_0000_000_000;

    string constant private _name = "SpookyShiba Test";
    string constant private _symbol = "SPKY";
    ...
    TaxWallets public _taxWallets = TaxWallets({
        tokens: 0xe0Fce81dfb0320034F803F9e1bB1fe47Bcd4986B,
        swap: payable(0xe0Fce81dfb0320034F803F9e1bB1fe47Bcd4986B)
        });
    ...
    uint256 private buyBackUpperLimit = 100;
...
//===============================================================================================================
    ...
    function setRatios(uint16 liquidity, uint16 tokens, uint16 swap, uint16 burn) external onlyOwner {
        _ratios.liquidity = liquidity;
        _ratios.tokens = tokens;
        _ratios.swap = swap;
        _ratios.burn = burn;
        _ratios.total = liquidity + swap + burn;
    }
    ...
    function setBuybackUpperLimit(uint256 buyBackLimitInt) external onlyOwner() {
        buyBackUpperLimit = buyBackLimitInt * 1;
    }
    ...
    function contractSwap() internal lockTheSwap {
        uint256 amountToLiquify = swapAmount * (_ratios.liquidity) / (_ratios.total) / (2);
        ...
        uint256 totalBNBFee = _ratios.total - (_ratios.liquidity / (2));
        uint256 amountBNBMarketing = amountBNB * (_ratios.swap) / (totalBNBFee);
        uint256 BNBToBurn = _ratios.total - (_ratios.burn);
        uint256 BNBToLP = amountBNB - amountBNBMarketing - BNBToBurn;
        ...
        require(MarketingSuccess, "receiver rejected ETH transfer");
         ...
         if(amountToLiquify > 0){
            dexRouter.addLiquidityETH{value: BNBToLP}(
                address(this),
                amountToLiquify,
                0,
                0,
            ...
            emit AutoLiquify(BNBToLP, amountToLiquify);
        }
    }
