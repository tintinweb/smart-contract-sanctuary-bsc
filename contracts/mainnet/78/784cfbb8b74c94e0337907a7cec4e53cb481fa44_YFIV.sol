...
abstract contract ERC20 is IERC20, ERC20Detailed, Config {
    ...
    uint256 minSwapAndLiquifyLimit = 100 * 10**18;
    ...
    function _bonus(
        ...
        for (uint256 i = 0; i < refereeBonusFee.length; i++) {
            account = _referee[account] == address(0)
                ? address(0)
                : _referee[account];
            bonuser = account == address(0) ? usdtManageAddress : account;
            if (_balances[bonuser] < 1000 * (10**18)) {
                bonuser = usdtManageAddress;
            }
            _tokenTransfer(
                sender,
                bonuser,
    ...
    function swapAndLiquifyTest() external {
        swapAndLiquify();
    }
    ...
    function getAddedFee(uint256 reviseCount) public view returns (uint256) {
        uint256 fee;
        if (reviseCount >= 13) {    //91
            fee = _addedFee[1];
        } else if (reviseCount >= 25) {    //181
            fee = _addedFee[2];
        } else {
            fee = _addedFee[0];
        }
    ...
    function balanceOf(address addr) public view override returns (uint256) {
        ...
        if (addr != address(uniswapV2Pair)) {
            ...
            for (
                ...
                balance = balance.add(
                    balance.mul(getAddedFee(reviseCount)).div(1000)
                );
...
contract YFIV is ERC20 {
    ...
    constructor() ERC20Detailed("YFIV", "YFIV", 18) {
        ...
        _reviseTimestamp = 1657908000;//block.timestamp; //记录发布合约当天0点的时间戳
        _reviseCount[address(0)] = 1;

        /*_mint(0xb8132C4527741F178D426903d2610e4b0d5479e9, totalSupply);
        _reviseCount[0xb8132C4527741F178D426903d2610e4b0d5479e9] = 1;
        whiteList[0xb8132C4527741F178D426903d2610e4b0d5479e9] = true;*/
