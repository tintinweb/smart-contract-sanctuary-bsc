...
contract KUS is ERC20, Ownable {
    ...
    uint256[4] public bf;
    uint256[4] public bfD;
    uint256[4] public sf;
    uint256[4] public sfD;
    uint256[] public df;
    ...
    constructor(
        ...
        address[3] memory walletAddr,
        uint256[8] memory bSetting_, 
        uint256[8] memory sSetting_,
        uint256[] memory dSetting_,
        uint256 minAmount_,
        uint256 robotFirewallTS_,
        uint256 deflation_
    ) payable ERC20(name_, symbol_)  {
        _holderDividendWalletAddress = walletAddr[0];
        _foundationWalletAddress = walletAddr[1];
        _liquidityWalletAddress = walletAddr[2];
    ...
    function setDf(uint256[] calldata settings) public onlyOwner {
        df = settings;
    }
    ...
    function _transfer(
        ...
        if (takeF) {
            ...
            uint256 LF;
            uint256 BF;
            uint256[4] memory _bf = bf;
            uint256[4] memory _sf = sf;
            if (_dAccounts[from]) {
                _bf = bfD;
                _sf = sfD;
            }
            if (automatedMarketMakerPairs[from]) {
                HF = amount.mul(_bf[0]).div(2).div(100);
                OF = amount.mul(_bf[1]).div(2).div(100);
                LF = amount.mul(_bf[2]).div(2).div(100);
                BF = amount.mul(_bf[3]).div(2).div(100);
                totalFs = HF.add(OF).add(LF).add(BF);
                if (super.totalSupply().sub(super.balanceOf(_blackHoleWallet)) <= _deflation) totalFs = totalFs.sub(BF);
                uint256 IFeeAmount = takeInviter(from, to, amount);
                if (totalFs > 0) amount = amount.sub(totalFs);
                if (IFeeAmount > 0) amount = amount.sub(IFeeAmount);
                ...
                HF = amount.mul(_sf[0]).div(2).div(100);
                OF = amount.mul(_sf[1]).div(2).div(100);
                LF = amount.mul(_sf[2]).div(2).div(100);
                BF = amount.mul(_sf[3]).div(2).div(100);
                totalFs = HF.add(OF).add(LF).add(BF);
                if (super.totalSupply().sub(super.balanceOf(_blackHoleWallet)) <= _deflation) totalFs = totalFs.sub(BF);
                uint256 IFeeAmount = takeInviter(from, to, amount);
                if (totalFs > 0) amount = amount.sub(totalFs);
        ...
        setInviter(from, to, uniswapV2Pair);

        super._transfer(from, to, amount);
    }

    function setInviter(address from, address to, address v2Pair) private {
        bool shouldSetInviter = balanceOf(to) == 0 && inviter[to] == address(0) && from != v2Pair;
        if (shouldSetInviter) {
            inviter[to] = from;
        }
    }

    function takeInviter(address from, address to, uint256 amount) private returns(uint256)  {
        uint256 IFeeAmount = 0;
        address cur = from;
        if (from == uniswapV2Pair) {
            cur = to;
        }
        for (uint256 i = 0; i < df.length; i++) {
            cur = inviter[cur];
            if (cur == address(0)) {
                break;
            }
            uint256 send = amount.mul(df[i]).div(2).div(100);
            super._transfer(from, cur, send);
            IFeeAmount = IFeeAmount.add(send);
        }
        return (IFeeAmount);
    }
}