...
contract testBul is ERC20Burnable, Operator {
    ...
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 17000 ether;
     // Distribution for team wallet
    uint256 public constant INITIAL_TEAM_WALLET_DISTRIBUTION = 15000 ether;
    ...
    /**
     * @notice Constructs the DIGIT ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Test DIGIT", "bul") {
        // Mints 1 DIGIT to contract creator for initial pool setup
    ...
    function _getDigitPrice() internal view returns (uint256 _digitPrice) {
        try IOracle(digitOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Digital: failed to fetch DIGIT price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _digitPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_digitPrice >= taxTiersTwaps[tierId]) {
    ...
    function setDigitOracle(address _digitOracle) public onlyOperatorOrTaxOffice {
        require(_digitOracle != address(0), "oracle address cannot be 0 address");
        digitOracle = _digitOracle;
    }
    ...
    /**
     * @notice Operator mints DIGIT to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of DIGIT to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentDigitPrice = _getDigitPrice();
            currentTaxRate = _updateTaxRate(currentDigitPrice);
            if (currentDigitPrice < burnThreshold) {
                burnTax = true;
            }
        }
