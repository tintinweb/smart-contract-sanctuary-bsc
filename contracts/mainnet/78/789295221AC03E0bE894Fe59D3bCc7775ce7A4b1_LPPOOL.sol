...
contract LPPOOL is Ownable {
    ...
    struct UserInfo {
        uint256 amount; // How many LP or TOKEN tokens the user has provided.
        uint256 rewardDebt;
        
        uint256 assumedRusdDividends;
        uint256 rusdDividendsDebt;
        uint256 unlockTime;
    }
    ...
    uint256 public totalAssumedRusdDividends;

    uint256 public totalDeptRusdDividends;

    uint256 public totalDeptRusdDividendsByRemoveLp;
    ...
    // The block number when mintTokenAddress mining end.
    ...
    event WithdrawRusdDividends(address indexed sender, address indexed recipient, uint256 amount);
    ...
    constructor(address _Minttoken) public {
        mintTokenAddress = _Minttoken;
    }

    // ============ Modifiers ============

    modifier lpTokenExist(address _lpToken,uint256 _period) {
        require(lpTokenRegistry[_lpToken][_period] > 0, "Airdrop: LP token not exist");
        _;
    }

    modifier lpTokenNotExist(address _lpToken,uint256 _period) {
        require(
            lpTokenRegistry[_lpToken][_period] == 0,
    ...
    function getPid(address _lpToken,uint256 _period)
        public
        view
        lpTokenExist(_lpToken,_period)
        returns (uint256)
    {
        return lpTokenRegistry[_lpToken][_period] - 1;
    }

    function getUserLpBalance(address _lpToken, address _user,uint256 _period)
        public
        view
        returns (uint256)
    {
        uint256 pid = getPid(_lpToken,_period);
    ...
    {
        uint256 pid = getPid(_lpToken,_period);
        return userInfo[pid][_user].unlockTime;
    }
    ...
    function newAirdrop(
        ...
        uint256 newRemaining = remainingAmount.add(_tokenAmount);
        require(
            _tokenAmount > 0,
        ...
        endBlock = _startBlock.add(remainingAmount.div(tokenPerBlock));
        updatePoolLastRewardBlock(_startBlock);
        emit NewAirdrop(msg.sender, _tokenAmount, _startBlock, _newPerBlock);
    }
    ...
    function addLpToken(
        ...
        uint256 _period,
        uint256 _allocPoint
    ) public lpTokenNotExist(_lpToken,_period) onlyOwner {
        ...
        lpTokenRegistry[_lpToken][_period] = poolInfos.length;
    }
    ...
    function getPendingReward(address _lpToken, address _user,uint256 _period)
        external
        view
        returns (uint256)
    {
        ...
        return (user.amount.mul(accTOKENPerShare).div(1e12).sub(user.rewardDebt)).mul(70).div(100);
    }
    ...
    function getRemainingAmount() external view returns (uint256) {
        ...
        for (uint256 pid = 0; pid < poolInfos.length; ++pid) {
         ...
         uint256 tokenReward =multiplier
                ...
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            remaining = remaining.sub(tokenReward);
        }
        return remaining;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 tokenReward =multiplier
                ...
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
        remainingAmount = remainingAmount.sub(tokenReward);
    ...
    function deposit(address _lpToken, uint256 _amount,uint256 _period,address _referrerAddress) public {
        uint256 pid = getPid(_lpToken,_period);
        PoolInfo storage pool = poolInfos[pid];
        UserInfo storage user = userInfo[pid][msg.sender];
        require(user.amount==0,"you have pledged");
        updatePool(pid);
        ...
        if (user.amount > 0) {
            ...
            if (pendingAmount > 0) {
                safeTOKENTransfer(msg.sender, pendingAmount.mul(70).div(100), pool.tokenAmount);
                dividendsToReferrer(msg.sender,pendingAmount.mul(30).div(100),pool.tokenAmount,pid);
            }
        }
        if (_amount > 0) {
        ...
        user.unlockTime =block.timestamp.add(_period.mul(1 days));
        emit Deposit(msg.sender, pid, _amount);
    }

    function withdraw(address _lpToken,uint256 _period) public {
        uint256 pid = getPid(_lpToken,_period);
        PoolInfo storage pool = poolInfos[pid];
        UserInfo storage user = userInfo[pid][msg.sender];
        require(user.unlockTime<block.timestamp&&user.unlockTime!=0,"unLockTime err");
        updatePool(pid);
        ...
        if (user.amount > 0) {
            if (pool.lpToken == mintTokenAddress) {
                pool.tokenAmount = pool.tokenAmount.sub(user.amount);
            }
    ...
    function claim(address _lpToken,uint256 _period) public {
        uint256 pid = getPid(_lpToken,_period);
        if (
    ...
    function dividendsToReferrer(address from,uint256 Amount,uint256 poolTokenAmount,uint256 pid)private{
        uint8 i=1;
        ...
        while (true) {
            ...
            if(userInfo[pid][referalAddress].amount==0){
