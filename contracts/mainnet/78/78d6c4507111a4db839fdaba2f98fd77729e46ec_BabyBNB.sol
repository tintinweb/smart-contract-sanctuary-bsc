...
contract BabyBNBDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("BabyBNB_Dividend_Tracker", "BabyBNB_Dividend_Tracker") {
        claimWait = 3600;
...
contract BabyBNB is ERC20, Ownable {
    ...
    function setBuyFees(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _marketFee, uint256 _devFee, uint256 _buybackFee) 
        public onlyOwner {
        BNBRewardsBuyFee = _bnbRewardFee;
        liquidityBuyFee = _liquidityFee;
        marketBuyFee = _marketFee;
        devBuyFee = _devFee;
        buybackBuyFee = _buybackFee;
        totalBuyFees = BNBRewardsBuyFee.add(liquidityBuyFee).add(marketBuyFee).add(devBuyFee).add(buybackBuyFee); // total buy fee
    }
    
    function setSellFees(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _marketFee, uint256 _devFee, uint256 _buybackFee) 
        public onlyOwner {
        BNBRewardsSellFee = _bnbRewardFee;
        liquiditySellFee = _liquidityFee;
        marketSellFee = _marketFee;
        devSellFee = _devFee;
        buybackSellFee = _buybackFee;
        totalSellFees = BNBRewardsSellFee.add(liquiditySellFee).add(marketSellFee).add(devSellFee).add(buybackSellFee); // total sell fee
    }
    ...
    constructor() ERC20("BabyBNB", "BabyBNB") 
    {
        ...
        marketWallet = payable(0xd3488350E5530E0a45E5A3b8fb7279147Bc582A0);
        devWallet = payable(0x2a4aE3948c523258a51c27091b46Ffb3FA8124bc);
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BabyBNB: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BabyBNB: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // worthy market fee
        uint256 marketAmount = deltaBalance.mul(marketSellFee).div(totalSellFees);
        if (marketAmount > 0) {
    	    marketWallet.transfer(marketAmount);
	    }
        
        // worthy dev fee
        uint256 devAmount = deltaBalance.mul(devSellFee).div(totalSellFees);
        if (devAmount > 0) {
