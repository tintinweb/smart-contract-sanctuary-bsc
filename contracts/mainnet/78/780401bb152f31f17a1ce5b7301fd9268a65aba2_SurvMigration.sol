...
contract SurvMigration is Ownable {
    using SafeMath for uint256;
    ...
    uint16 public migrationRate = 100; // Migrate 1 SURV to 1 SURV2.0
    uint256 public startAt = 1669921200; // Dec 1, 2022 07:00:00 PM GMT
    uint256 public maxLimit = 1e18; // Max migration limit per user
    ...
    function migrate(uint256 _fromAmount) external {
        ...
        fromToken.safeTransferFrom(_msgSender(), address(this), _fromAmount);
        _fromAmount = fromToken.balanceOf(address(this)).sub(balanceBefore);

        uint256 userMigratedAmount = userInfo[_msgSender()];

        require(
            whitelist[_msgSender()] ||
                userMigratedAmount.add(_fromAmount) <= maxLimit,
            "Exceeds limitation"
        );
        ...
        require(
            toToken.balanceOf(address(this)) >= toAmount,
    ...
    /**
     * @notice Set migration token pairs
     * @dev Only owner can call this function
     */
    function setMigrationTokenPair(IBEP20 _fromToken, IBEP20 _toToken)
        external
        onlyOwner
    {
        require(address(_fromToken) != address(0), "Invalid from token");
    ...
    /**
     * @notice Set new airdrop multiply
     * @dev Only owner can call this function
     */
    function setMigrationRate(uint16 _rate) external onlyOwner {
        require(_rate > 0, "Invalid migration rate");
        migrationRate = _rate;
    }
    ...
    /**
     * @notice Include / Exclude account in whitelist
     * @dev Only owner can call this fundtion
     */
    function includeWhitelist(address _account, bool _whitelisted)
        external
        onlyOwner
    {
    ...
    /**
     * @notice Include / Exclude several accounts in whitelist
     * @dev Only owner can call this fundtion
     */
    function includeBulkWhitelist(
     ...
     * @dev Only owner can call this function
     */
    function recoverTokens(IBEP20 _token, uint256 _amount) external onlyOwner {
        uint256 contractBalance = _token.balanceOf(address(this));
        require(contractBalance >= _amount, "Too much amount");
