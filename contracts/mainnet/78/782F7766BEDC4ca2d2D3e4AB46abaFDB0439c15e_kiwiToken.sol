...
contract kiwiToken is Context, IERC20, Ownable {
    ...
    string private _name = "Kiwi";
    string private _symbol = "Kiwi";
    uint8 private _decimals = 18;
    
    uint256 public _invitBuyFee = 6;
    
    uint256 public _devSellFee = 1;
    uint256 public _poolSellFee = 3;
    uint256 public _lpSellFee = 2;
    
    IWrap public usdtWrap;
    IWrap public sifeWrap;
    
    address public burnAddress = address(0x000000000000000000000000000000000000dEaD);
    address public ownerAddres = address(0x1eBeF3c407C4Ba93F6027EB72930d5BE6b2236d9);
    address public devAddress = address(0x90b027720aa431343A39fA8d69164b23145355de);
    address public poolAddress = address(0x000000000000000000000000000000000000dEaD);
    
    ExternalToken public externalToken = ExternalToken(address(0x7f3f6359A83b4B54F5a24DbB83D35056dDBa3a23));
    
    address public usdtToken = address(0x55d398326f99059fF775485246999027B3197955);
    address public sifeToken = address(0x000000000000000000000000000000000000dEaD);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 addTokenBalance = IERC20(sifeToken).balanceOf(address(this));
        uint256 addUsdtBalance = IERC20(usdtToken).balanceOf(address(this));
        addLiquidity(addTokenBalance, addUsdtBalance);
    ...
    function swapTokensForUsdt(uint256 otherHalf) private lockTheSwap {
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = usdtToken;
        
        _approve(address(this), address(uniswapV2Router), otherHalf.mul(100));
    ...
    function addLiquidity(uint256 tokenAmount, uint256 tokenBmount) public {
        ...
        IERC20(sifeToken).approve(address(uniswapV2Router), tokenAmount.mul(100));
        IERC20(usdtToken).approve(address(uniswapV2Router), tokenBmount.mul(100));
        // add the liquidity
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        if (!takeFee) {
            ...
            if (sender == uniswapV2Pair) { // 购买
                ...
                uint256 newAmount = amount.sub(invitBuyAmount);
                ...
                uint256 lpSellAmount = amount.mul(_lpSellFee).div(100);
                uint256 devSellAmount = amount.mul(_devSellFee).div(100); 
                uint256 newAmount = amount.sub(poolSellAmount).sub(lpSellAmount).sub(devSellAmount);
                _rOwned[recipient] = _rOwned[recipient].add(newAmount);
                _rOwned[poolAddress] = _rOwned[poolAddress].add(poolSellAmount);
                _rOwned[address(this)] = _rOwned[address(this)].add(lpSellAmount);
                _rOwned[devAddress] = _rOwned[devAddress].add(devSellAmount);
                emit Transfer(sender,recipient, newAmount);
                emit Transfer(sender,poolAddress, poolSellAmount);
                emit Transfer(sender,address(this), lpSellAmount);
                emit Transfer(sender,devAddress, devSellAmount);
            } else {
                _rOwned[recipient] = _rOwned[recipient].add(amount);
                emit Transfer(sender,recipient, amount);
            }
    ...
    function buyFeeDist(address sender,address recipient,uint256 invitBuyAmount) private {
        uint256 oneAmount = invitBuyAmount.div(6);
        ...
        for (uint256 i=0; i < 2; i++) {
            if (sjAddress != address(0)) {
                ...
                _rOwned[poolAddress] = _rOwned[poolAddress].add(syAmount);
                break;
            }
        }
    ...
    function setFee(uint256 iBuyFee,uint256 dSellFee,uint256 pSellFee,uint256 lSellFee) public onlyOwner {
        _invitBuyFee = iBuyFee;
        _devSellFee = dSellFee;
        _poolSellFee = pSellFee;
        _lpSellFee = lSellFee;
    }
    ...
    function setIWrap(address _usdtWrap, address _sifeWrap) public onlyOwner {
        usdtWrap = IWrap(_usdtWrap);
        sifeWrap = IWrap(_sifeWrap);
    }
    
    function setSifeToken(address _sifeToken) public onlyOwner {
        sifeToken = _sifeToken;
    }
