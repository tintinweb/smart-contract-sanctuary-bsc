...
contract CyberSale is Ownable {
    ...
    ERC20 public CyberLink = ERC20(0xC57D5be2E77845AAC76bC8CA1943Fc0dbE894861);
    ERC20 public Busd = ERC20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
    bool private withdrawn = false;
    address[] public joinList;

    uint256 public SaleS = 1664017200;
    uint256 public SaleE = 1664575200;
    ...
    function deposit(uint8 saleID, uint256 amount) public {
        require(block.timestamp > SaleS && block.timestamp < SaleE, "Out of time");
        if(saleID == 0){
            require(amount >= Sale0MinBuy && amount.div(10 ** 18).mul(Sale0TokensPerBusd) <= Sale0TokenAlloc.sub(totalBoughtIn(0)));
            Busd.transferFrom(msg.sender, address(this), amount);
            sale0DataMap[msg.sender] = sale0Data(sale0DataMap[msg.sender].totalBought.add(amount.div(10 ** 18).mul(Sale0TokensPerBusd)), sale0DataMap[msg.sender].currentBalance.add(amount.div(10 ** 18).mul(Sale0TokensPerBusd)), 0);

        } else {
            require(amount >= Sale1MinBuy && amount.div(10 ** 18).mul(Sale1TokensPerBusd) <= Sale1TokenAlloc.sub(totalBoughtIn(1)));
            Busd.transferFrom(msg.sender, address(this), amount);
            sale1DataMap[msg.sender] = sale1Data(sale1DataMap[msg.sender].totalBought.add(amount.div(10 ** 18).mul(Sale1TokensPerBusd)), sale1DataMap[msg.sender].currentBalance.add(amount.div(10 ** 18).mul(Sale1TokensPerBusd)), 0);
        }
    ...
    function claimBatch(uint8 saleID) public {
        require(block.timestamp >= SaleE + 2592000 && isPresaler(msg.sender));
        uint256 batch;
        if(saleID == 0){
            require(sale0DataMap[msg.sender].lastClaim + 2592000 <= block.timestamp && sale0DataMap[msg.sender].currentBalance > 0);
            ...
            require(sale1DataMap[msg.sender].lastClaim + 2592000 <= block.timestamp && sale1DataMap[msg.sender].currentBalance > 0);
