...
contract PiBridgeTesseractNFT is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, AccessControl, ERC721Burnable {
    ...
    mapping(address => bool) public userMintedMap;

    event WhitelistMinted(address indexed to, uint256 indexed tokenId);

    constructor() ERC721("PiBridge Tesseract NFT", "PiBTNFT") {
    ...
    function safeMintWhiteList() public onlyRole(WHITELIST_ROLE) validTime {
        require (totalSupply() < maxSupply);
        require(userMintedMap[msg.sender] == false, "You have already minted!");
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
