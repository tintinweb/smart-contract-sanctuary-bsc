...
contract FeeReceiver is Ownable, IFeeReceiver{
    ...
    uint[] public amounts;
    mapping(address=>address) bonusReceiver;
    uint DENOMINATOR;
    ...
    function receiveFees(address Token) external payable{
        address _bonusReceiver=bonusReceiver[Token];
        bool sent;
        if(_bonusReceiver!=address(0)){
            (sent,)=_bonusReceiver.call{value:msg.value*125/1000}("");
        }
