...
contract NodeType is INodeType, OwnersUpgradeable, NodeRewards {
    ...
    function _hotfixSubstractGRPBatch(
        uint256[] calldata tokenIds,
        uint256[] calldata substractLifetime,
        uint256[] calldata newPlotAdditional
    ) public onlyOwners {
        require(tokenIds.length == substractLifetime.length);
        require(tokenIds.length == newPlotAdditional.length);

        for (uint256 i = 0; i < tokenIds.length; i++) {
            Node storage node = userOf[tokenIdToOwner[tokenIds[i]]].values[tokenIds[i]];
            node.lastLifetime -= substractLifetime[i];
            node.plotAdditionalLifetime = newPlotAdditional[i];
        }
    }
}
