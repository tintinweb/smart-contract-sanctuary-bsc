...
contract ElonsBadDay is IBEP20, Auth {
    ...
    uint256[] public _percentages = [20,20,20,20,10,10,0];                                // wallet(s) to receive liquidity fee split
    address payable[] public _wallets = [                                                // wallet(s) to receive liquidity fee split
        ...
        payable(0xc097cf7440de0bc62C9C6E887c9E14B9f3B2906d),                             // Marketing Wallet!                                                                                   
    ...
    function swapBack() internal swapping {
        ...
        uint256 BNBBalance = amountBNB.mul(marketingFee).div(totalBNBFee);
         
        if(CharityEnabled){

         // send up to but not including the last wallet
        for(uint256 i=0; i<_wallets.length-1; i++){
            uint256 amountToWallet = calculateAmount(BNBBalance, _percentages[i]);
          _wallets[i].transfer(amountToWallet);
          }
        }else{
        payable(marketingFeeReceiver).transfer(BNBBalance);
        } 
    ...
    function setBuybackMultiplierSettings(uint256 numerator, uint256 denominator, uint256 length, uint256 _Fees1) external authorized {
        require(numerator / denominator <= 2 && numerator > denominator);
        require(_Fees1 <= 18000 );//Used during buyback period for sells
