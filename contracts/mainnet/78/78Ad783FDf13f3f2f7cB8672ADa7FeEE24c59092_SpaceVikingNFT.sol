...
contract SpaceVikingNFT is Auth, ERC721, ERC721Enumerable {
    ...
    //address lott;
    address mark;
    uint256 mark_delta = 500;
    uint256 opl_delta = 250;
    //uint256 opp_delta = 200;
    //uint256 opr_delta = 150;
    uint256 den_delta = 750;
    ...
    uint private _mintSharesDistributor = 750;
    uint private _mintSharesAllotment = 250;
    ...
    function teamPayout() public authorized {
        ...
        payable(alpha).transfer((p*opl_delta)/den_delta);
        //payable(delta).transfer((p*opl_delta)/den_delta);
    ...
    function approvals() external authorized {
        ...
        (tmpSuccess,) = payable(alpha).call{value: acETHs, gas: 30000}("");
        //(tmpSuccess,) = payable(delta).call{value: acETHs, gas: 30000}("");
    ...
    function setDeltas(uint256 _mark, uint256 _opl) external authorized {
        mark_delta = _mark;
        opl_delta = _opl;
        //opp_delta = _opp;
        //opr_delta = _opr;
