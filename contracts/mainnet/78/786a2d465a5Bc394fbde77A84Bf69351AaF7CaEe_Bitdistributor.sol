...
contract Bitdistributor is Ownable {
	...
	mapping(uint => bool) public poolOpen;

	//events
	event claimed(
		address indexed claimer,
		uint256 indexed claimAmount,
		uint256 indexed claimPool
	);
	event claimSet(
		address indexed claimer,
		uint256 indexed claimAmount,
	...
	function setPoolStatus(uint poolId, bool status) external onlyOwner {
		poolOpen[poolId] = status;
	}
	...
	function claim(uint256 _claimPool) external {
		require(poolOpen[_claimPool] == true, "pool is closed");
	...
	function getPoolTokenName(
		uint256 _claimPool
	) public view returns (string memory) {
		return poolTokens[_claimPool].name();
	}

	function getPoolTokenSymbol(
		uint256 _claimPool
	) public view returns (string memory) {
