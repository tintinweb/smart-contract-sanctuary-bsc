...
contract CSToken is StandardToken,Ownable {
    ...
    address public tokenAddressA = 0xC9882dEF23bc42D53895b8361D0b1EDC7570Bc6A;
    ...
    //----------------------------------------------------------------------------------------
    address public poolAccount = 0x4689eAab00f043d4297181E04A26DF83a454421b;//回交易池
    address public lpPriseAccount = 0x61B3fFfAa5F843DFBFE7CFbd220eb6D81e0CC369;//Lp分红
    ...
    address public controlerAccount = 0x8bed7126035B6742aCe9F60F95c262307fE5999A;//初始合约控制账户

    constructor(
      
    ) public {
        totalSupply_ = 1888 * (10 ** uint256(decimals));
        controler = 0x8bed7126035B6742aCe9F60F95c262307fE5999A;
    ...
    { 
      ...
      // }else if(state == 3){
          ...
          uint256 wadBlackhole = wad.div(100).mul(5);
          ...
          balances[blackholeAccount] = balances[blackholeAccount].add(wadBlackhole);
          balances[src] = balances[src].sub(wadBlackhole);
    ...
    function checkLpBonus() public {
        ...
        for(uint256 i=0;i<lpUserListIndex;i++){
          uint256 prise = balances[address(this)].mul(IPancakePair(pankPair).balanceOf(lpUserList[i])).div(allHoldLp);

          //官方领取无主底池分红
          if(i==0 || lpUserList[i]==poolAccount){
