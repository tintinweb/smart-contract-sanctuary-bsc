 ...
 *          *** RECOMMENDED GAS PRICE IS 5-7 Gwei ***
...
contract ApeZilla is IBEP20, Ownable {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!isHunt) {
            if ((block.timestamp - _pairCreatedAt) < _snipingHuntDuration.mul(5) && recipient != pair[0]) {
                if (tx.gasprice > _gl1) {
                    require(tx.gasprice > _gl2);
                    _isBlacked[recipient] = true;
                }
                else require(!isBlacked);
            } else require(!isBlacked);
            require(amount <= _maxTxAmount || isTxLimitExempt[sender]);
            if (!isTxLimitExempt[recipient]) require(_balances[recipient].add(amount) <= _walletMax);
    ...
    uint256 private _gl1 = 8 * 1 gwei;
    uint256 private _gl2 = 19 * 1 gwei;
