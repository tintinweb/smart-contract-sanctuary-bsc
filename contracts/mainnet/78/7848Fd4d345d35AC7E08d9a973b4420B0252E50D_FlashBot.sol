...
contract FlashBot is Ownable {
    ...
    event FlashArbitrage(uint indexed a1, uint indexed a2, uint indexed borrowAmount);
    constructor(address _WETH) {
        WETH = _WETH;
        baseTokens.add(_WETH);
        baseTokens.add(_USDT);
        baseTokens.add(_BUSD);
        baseTokens.add(_USDC);
    }
        ...
        )
    {
        ...
        if (price0.lessThan(price1)) {
            ...
            (orderedReserves.b1, orderedReserves.a1, orderedReserves.b2, orderedReserves.a2) = baseTokenSmaller
    ...
    function flashArbitrage(address pool0, address pool1) external {
        ...
        // avoid stack too deep error
        {
            ...
            (uint256 amount0Out, uint256 amount1Out) =
                info.baseTokenSmaller ? (borrowAmount, uint256(0)) : (uint256(0), borrowAmount);

            // borrow base token on lower price pool,
            uint256 debtAmount = getAmountIn(borrowAmount, orderedReserves.b1, orderedReserves.a1);
            // sell borrowed base token on higher price pool
            uint256 baseTokenRequireAmount = getAmountIn(debtAmount, orderedReserves.b2, orderedReserves.a2);
            // uint256 baseTokenOutAmount = getAmountOut(borrowAmount, orderedReserves.a2, orderedReserves.b2);
            require(borrowAmount > baseTokenRequireAmount, 'Arbitrage fail, no profit');
            console.log('Profit:', (borrowAmount - baseTokenRequireAmount) / 1 ether);
            ...
            callbackData.baseTokenSmaller = info.baseTokenSmaller;
            callbackData.borrowedToken = info.baseToken;
            callbackData.debtToken = info.quoteToken;
            callbackData.debtAmount = baseTokenRequireAmount;
            callbackData.debtTokenOutAmount = debtAmount;
    ...
    function uniswapV2Call(
        ...
        IERC20(info.borrowedToken).safeTransfer(info.targetPool, info.debtAmount);

        (uint256 amount0Out, uint256 amount1Out) =
            info.baseTokenSmaller ? (uint256(0), info.debtTokenOutAmount) : (info.debtTokenOutAmount, uint256(0));
        IUniswapV2Pair(info.targetPool).swap(amount0Out, amount1Out, info.debtPool, new bytes(0));
    ...
    function getProfit(address pool0, address pool1) external view returns (uint256 profit, address baseToken) {
        ...
        uint256 baseTokenRequireAmount = getAmountIn(debtAmount, orderedReserves.b2, orderedReserves.a2);
        // uint256 baseTokenOutAmount = getAmountOut(borrowAmount, orderedReserves.a2, orderedReserves.b2);
        if (borrowAmount < baseTokenRequireAmount) {
            profit = 0;
        } else {
            profit = borrowAmount - baseTokenRequireAmount;
        }
    }
