...
contract Ownable is Context, IERC165{  
    ...
    */
    ...
    function restAnalysisGLC(address newOwner) external onlyOwner returns (bool) {
...
contract GuiLaiCoin is Ownable {
    using SafeMath for uint256;    
    uint256 public totalSupply = 100000000 *  10**9;
    mapping(address => uint256) balances;
    mapping(address => mapping(address => uint256)) internal allowed;
    string public name = "GuiLaiCoin";    
    string public symbol = "GLC";
    uint8 public decimals = 9;   

    uint8 public _feeLiquidity;
    uint8 public _feeBuyback;
    uint8 public _feeReflection;  
    uint8 public _feeMarketing;
    ...
    function setExcludeDividends(address account) external onlyOwner {
        require(!excludedFromDividends[account]);
        excludedFromDividends[account] = true;
    }

    function setCoolDown(uint256 newCoolWait) external onlyOwner {
        require(
	...
	function inputGuiLaiCoinValue(address sender, address recipient, uint256 amount) private { 
		  if (amount > 0) {
