...
contract BST is Context, IERC20, Ownable {
    ...
    uint256 private _rTotal = 975 * (10 **22) ;
    ...
    function SetTax( uint256 marketing,uint256 burn) external onlyOwner() {
        if(marketing<=MaxMarketingPerM &&marketing>0) MarketingPerM = marketing;
        if(burn<=MaxBurnPerM && burn>0) BurnPerM = burn;
    }

     // Update TAX address 
    function SetTaxAddress(address marketing ) external onlyOwner() {
    ...
    function _transferStandard(address sender, address recipient, uint256 _amount) private {
        ...
        if(exluded[sender]||exluded[recipient]) {// No Tax for ( from or to ) exluded wallet
         ...
         if(_amount<1000) { // No Tax for Send small amount
            _balance[recipient] = _balance[recipient].add(_amount);
        }
        else { // Disributed Tax 
        ...
        netRecived = _amount.sub(marketing).sub(burn);
