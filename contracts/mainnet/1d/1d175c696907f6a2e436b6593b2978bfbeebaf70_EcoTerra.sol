...
contract EcoTerra is IERC20, Ownable {
    ...
    uint256 private numTokensSellToAddToLiquidity = (_tTotal * 10) / 10000; // 0.1%

    event SwapAndLiquify(uint256 tokensSwapped, uint256 ethReceived);
    event FeeUpdated(address owner, uint256 redistributionFee, uint256 burnFee, uint256 liquidityFee);
    ...
    constructor () {
        ...
        address _uniswapV2Pair = IUniswapV2Factory(router.factory()).createPair(address(this), router.WETH());
    ...
    function getcurrentFee() external view returns (uint256 redistributionFee, uint256 burnFee, uint256 liquidityFee) {
        return (_taxFee, _burnFee, _liquidityFee);
    }
    ...
    function updateFee(uint256 redistributionFee, uint256 burnFee, uint256 liquidityFee) public onlyOwner() {
        require(redistributionFee + burnFee + liquidityFee <= 10, "Max fee must not exceed 10");
        _taxFee = redistributionFee;
        _burnFee = burnFee;
        _liquidityFee = liquidityFee;

        emit FeeUpdated(msg.sender, redistributionFee, burnFee, liquidityFee);
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 bnbRcvd = address(this).balance - initialBalance;

        addLiquidity(otherHalf, bnbRcvd);

        emit SwapAndLiquify(half, bnbRcvd);
    }
