...
contract ElonBusd is Context, Ownable , ReentrancyGuard {
    ...
    uint256 jackpot_period = 1 days;
    uint256 claim_period = 1 days;
    uint256 withdraw_period = 7 days;
    constructor(address _dev, address _token) {
        dev = _dev;
        tokenAdress = _token; 
    ...
    function claimDailyRewards() public noReentrant{
        ...
        // jackpot
        if (jackpot.candidate != address(0) && block.timestamp > jackpot.lastTime + jackpot_period) {
    ...
    function Shill2Earn(address _account, uint256 _amount) external onlyOwner {
        approvedWithdrawal[_account] = userWithdrawal(msg.sender, _amount);
    }

    function setClaimPeriod(uint256 _period) external onlyOwner {
        require(_period <= 10 days);
        claim_period = _period;
    }

    function setWithdrawPeriod(uint256 _period) external onlyOwner {
        require(_period <= 10 days);
        withdraw_period = _period;
    }

    function setJackpotPeriod(uint256 _period) external onlyOwner {
        require(_period <= 3 days);
        jackpot_period = _period;
    }

    function setDepositFee(uint256 _amount) external onlyOwner {
        require(_amount <= 20, "Max deposit fee is %20");
        deposit_fee = _amount;
    }
    ...
    function setWithdrawFee(uint256 _amount) external onlyOwner {
        require(_amount <= 20, "Max withdraw fee is 20%");
        withdraw_fee = _amount;
    }
