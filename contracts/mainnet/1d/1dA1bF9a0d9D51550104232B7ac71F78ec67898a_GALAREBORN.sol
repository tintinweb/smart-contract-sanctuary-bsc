...
/**
GALA REBORN SmartContract on Binance Smart Chain
*/
...
contract GALAREBORN is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    GALAREBORNDividendTracker public dividendTracker;
    ...
    constructor() ERC20("GALA REBORN", "REBORN") {
        marketingWallet = payable(0x8a96A0e3B3166aC38C91E208D87ED9bC89289f30);
        devWallet = payable(0x0A0685bB3EB386828c895EC35c2f84a783Ab1e18);
        ...
        dividendTracker = new GALAREBORNDividendTracker(
            payable(this),
            router,
...
contract GALAREBORNDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    GALAREBORN public GALAREBORNContract;
    ...
    constructor(
        ...
        defaultToken = token;
        GALAREBORNContract = GALAREBORN(mainContract);
    ...
    function _transfer(
        ...
        require(false, "GALAREBORN_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "GALAREBORN_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main GALAREBORN contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(GALAREBORNContract);

            uint256 prevBalance = GALAREBORNContract.balanceOf(address(this));
            ...
            {
                uint256 received = GALAREBORNContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    GALAREBORNContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
