...
// File: my/new_cow/grade/ICowAttack.sol



pragma solidity ^0.8.0;

interface ICowAttack {
...
// File: @openzeppelin/cont[email protected]/token/ERC721/presets/ERC721PresetMinterPauserAutoIdUpgradeable.sol
...
contract WolfAttackCow is WolfGameStorage {
    ...
    function attack(uint wolfId,uint cowId,uint deadline,bytes calldata signature) external nonReentrant {
        address cowAttack=paramAddress["cowAttack"];
        if(cowAttack!=address(0)) {
            ICowAttack(cowAttack).cowAttack(deadline,wolfId,cowId,signature);
        }
