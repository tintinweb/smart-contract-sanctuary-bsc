...
contract NftItems is ERC721URIStorage, AccessControl {
    ...
    mapping (uint => ItemType) public nftList;

    event Minted(uint256 _itemType, uint256 _tokenId, address _to);

    constructor(string memory name, string memory symbol, string memory _url) ERC721(name, symbol) {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        url = _url;
    }

    function mint(address _to, uint _itemType) public onlyMinters {
        ...
        string memory link = string(bytes.concat(bytes(url), bytes(Strings.toString(_itemType)), '/', bytes(newId)));
    ...
    function setItemPrice(uint _itemType, uint _price) public onlyAdmin {
        nftList[_itemType].price = _price;
    }

    function removeItemType(uint _itemType) public onlyAdmin {
        delete nftList[_itemType];
    }

    function getItemPrice(uint _itemType) external view returns(uint) {
        require(nftList[_itemType].price != 0, "Sorry, we don't have item with this type id");
