...
// File: contracts/goodrollcryptoNFTpass.sol
...
contract MyToken is ERC1155, Ownable {
    ...
    */ 
    ...
    uint[3] public pricePerNFT; // 3 cajas 3 precios, de menor a mayor. 
    ...
    mapping(address => packOpen) packOpens;
    event FoundsOut(address,uint,address); // withdraw found of this contract(msg.sender, total, address founds)


    constructor() ERC1155("https://bafybeihk5szkv43pjhobvto5fuav5srksgleeftbt5n3jdbgxqlhoyecx4.ipfs.nftstorage.link/") {
        pricePerNFT[0] = 10000000000000000; //seteo de la caja
        pricePerNFT[1] = 20000000000000000;
        pricePerNFT[2] = 30000000000000000;
        maxMint = 1333;
    }
    ...
    function nftURI(uint256 _idNft) public view  returns (string memory) {

        require(_idNft >= 0, "Id not found" );
        require(_idNft < 5, "Id not found");

        string memory currentBaseURI = uri(1);
    ...
    function rewardMint(address account,uint idList) public onlyOwner {
              mint(account,idList);
    }

    function mint(address account,uint idList)
        internal
    {
        ...
        packOpens[account].idPack.push(idList);
        packOpens[account].idNft.push(id);
        _mint(account, id, 1, "");
    }

    function getMintedPacked(address _address) public view returns (uint[] memory,uint[] memory){
        return (packOpens[_address].idPack,packOpens[_address].idNft);
    }

    function getAddressOfMinted() public view  returns (address[] memory,address[] memory, address[] memory ){
        return (listLow, listMid, listHigh);
    }


  

    function currentNfts() public view returns(uint){
