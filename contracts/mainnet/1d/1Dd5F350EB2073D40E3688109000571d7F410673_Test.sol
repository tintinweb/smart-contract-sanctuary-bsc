...
contract Test  is ERC20, Ownable {
    ...
    uint256 public _maxWalletAmount = 2 * _totalSupply / 100; // 2%
    ...
    // address constant routerAdress = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;    
    address constant routerAdress = 0x10ED43C718714eb63d5aA57B78B54704E256024E;    
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (recipient != pair && recipient != DEAD && recipient != ZERO) {
            require(amount <= _maxTxAmount, "Excessive transfer amount");
            require(_balances[recipient] + amount <= _maxWalletAmount, "Excessive receiver token holdings");
        }           
    ...
    function takeFee(address sender, uint256 amount) internal returns (uint256) {
        uint256 feeAmount = sender == pair ? 
            amount * buyFee / feeDenominator : amount * sellFee / feeDenominator;                        
    ...
    function swapBack() internal swapping {
        ...
        uint256 tokensToSwap = tokenBalance >= _maxTxAmount ? _maxTxAmount : tokenBalance;
    ...
    function openTrading(uint256 numdeadBlocks, bool _antiBotActive) external onlyOwner {
        // require(!tradingOpened, "Trading already enabled");
        // require(numdeadBlocks <= 3, "Max 3(+1) dead blocks"); ////////////////////////////////////
        tradingOpened = true;
        antiBotActive = _antiBotActive;
        finalDeadBlock = block.number + numdeadBlocks;
    }
    
    function setMaxAmount(uint256 maxWalletAmount) external onlyOwner {
        require(maxWalletAmount >= _totalSupply / 100, "MaxWalletAmount needs to be higher than 1% of total supply");
        _maxWalletAmount = maxWalletAmount;
    }
    
    function setMaxTx(uint256 maxTxAmount) external onlyOwner {
        require(maxTxAmount >= _totalSupply / 100, "MaxTxAmount needs to be higher than 1% of total supply");
        _maxTxAmount = maxTxAmount;
    }

    function setFee(uint256 buyFeePercent, uint256 sellFeePercent) external onlyOwner {
    ...
    function manualMarkSniper(address account) external onlyOwner {
