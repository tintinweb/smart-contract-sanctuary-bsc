...
contract MirrorProtocolBNBDistributor is UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuard {
    ...
    function processBNBSwap() internal {
        uint256 rwdAmount = address(this).balance.sub((totalDividends.sub(totalDistributed)));
        if(rwdAmount > 0) {
    ...
    function distributeDividend(address shareholder_) internal nonReentrant {
        if(shares[shareholder_].amount == 0){ return; }

        uint256 amount = getUnpaidEarnings(shareholder_);
        if(amount > 0){
            (bool rwdSent, ) = payable(shareholder_).call{value: amount}("");
            if(rwdSent) {
