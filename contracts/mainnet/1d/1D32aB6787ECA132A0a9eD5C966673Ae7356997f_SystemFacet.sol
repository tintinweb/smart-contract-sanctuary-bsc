...
contract SystemFacet {
    ...
    event AddToBlacklist(address sender, address account, uint256 timestamp);
    event RemoveFromBlacklist(address sender, address account, uint256 timestamp);
    ...
    function addToBlacklist(address account) external {
        LibDiamond.enforceIsContractOwner();
        s.blacklist[account] = 1;
        emit AddToBlacklist(msg.sender, account, block.timestamp);
    }

    function removeFromBlacklist(address account) external {
        LibDiamond.enforceIsContractOwner();
        s.blacklist[account] = 0;
        emit RemoveFromBlacklist(msg.sender, account, block.timestamp);
    }

    function blacklist(address account) public view returns (uint8) {
