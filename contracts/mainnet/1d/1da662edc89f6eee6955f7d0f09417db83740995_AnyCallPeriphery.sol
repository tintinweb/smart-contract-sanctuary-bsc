...
contract AnyCallBase is Initializable {
  ...
  event MyLog(bytes4 extractedSelector, bytes4 staticSelector);
    ...
    external returns (bool success, bytes memory result) 
  {
    ...
    emit MyLog(selector, this.anyExecute.selector);
...
// File contracts/AnyCallPeriphery.sol

pragma solidity ^0.8.0;


/**
The contract is deployed on the periphery (gnosis) network.
 */
contract AnyCallPeriphery is Initializable, AnyCallBase {
  ...
  ) internal override returns(bytes memory) 
  {
    ...
    bytes memory responsePayload = new bytes(10);
    if (requestCode == 1) {
      responsePayload[0] = bytes1(uint8(0x1));
    } else if (requestCode == 2) {
      responsePayload[0] = bytes1(uint8(0x2));
    } else {
      responsePayload[1] = bytes1(uint8(3));
    }
    emit ResponsePayload(responsePayload);
    return responsePayload;
  }
}