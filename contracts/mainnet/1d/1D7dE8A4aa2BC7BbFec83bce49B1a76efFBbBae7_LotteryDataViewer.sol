...
contract LotteryV2 is VRFConsumerBaseV2, ReentrancyGuard, DevFaucetIntegration {
    ...
    function calculateRewardsPerRound(
        ...
        returns (
            ...
            uint256[] memory _tokensToClaim
        )
    {
        _tokensToClaim = new uint256[](bonusAddresses.length);
        require(
        ...
        if (nonWinners > 0) {
            ...
            if (roundBonus.bonusToken != address(0)) {
                _tokensToClaim[bonusTokenIndex[roundBonus.bonusToken]] += (
                    nonWinners.mul(
                        getBonusReward(
        ...
        if (winners > 0) {
            for (uint256 j = 0; j < winners; j++) {
                ...
                _tokensToClaim[0] += checkTicketRequirements(
                    ticketId,
                    matchBatch,
                    roundChecked
                );
    ...
    function checkTicketRequirements(
        ...
        require(_matchBatch > 0 && _matchBatch < 7, "M1"); // dev: Minimum match 1
        require(
