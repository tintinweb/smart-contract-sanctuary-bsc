...
abstract contract IERC20Staking is ReentrancyGuard, Ownable {
    ...
    function stake(uint256 _stakingId, uint256 _amount,address _address,uint256 key) public virtual;
...
contract RegentStake is IERC20Staking {
    ...
    constructor(address _stakingToken,uint256 _key) IERC20Staking(_stakingToken) {
        key=_key;
        plans[0].apr = 12;
    ...
    function referralStake(uint256 _stakingId, uint256 _amount,address _stakerAddress, address _referrer,uint256 _key) public {
        require(_referrer!=_stakerAddress, "You can't refer yourself");

        if(referrals[_stakerAddress].referrer == address(0) && getTotalStakedAmount(_referrer) >= minTokenForReferral) {
            referrals[_stakerAddress].referrer = _referrer;
            referrals[_referrer].referees.push(_stakerAddress);
        }

        stake(_stakingId, _amount,_stakerAddress,_key);
    }

    function stake(uint256 _stakingId, uint256 _amount, address _address,uint256 _key) public override {
        require(key==_key,"Validation failed");
        require(_amount > 0, "Staking Amount cannot be zero");
        require(
            IERC20(stakingToken).balanceOf(_address) >= _amount,
        ...
        IERC20(stakingToken).transferFrom(_address, address(this), _amount);
        ...
        uint256 stakelength = stakes[_stakingId][_address].length;
        
        if(stakelength == 0) {
            plan.stakesCount += 1;
        }

        stakes[_stakingId][_address].push();
        
        Staking storage _staking = stakes[_stakingId][_address][stakelength];
        ...
        if(referrals[_address].referrer != address(0)) {
            address _referrer = referrals[_address].referrer;
            
            ReferralStake storage _referralStake = referrals[_referrer].referralStakes[_address].push();
