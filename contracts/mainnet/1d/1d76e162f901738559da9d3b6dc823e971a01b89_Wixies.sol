...
/**
 ...
 Telegram: https://t.me/wixies

 Website: https://wixies.tech 
 
 Twitter: https://twitter.com/GeniusMarket?t=9tWY4aBEm-0348RJHCuVyQ&s=35



 Supply 2,000,000,000
 Taxes 5% Buy 5% Sell
 Launch January 21st 2023
...
contract Wixies  is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    WixiesDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Wixies", "Wixies") {
        marketingWallet = payable(0x6A2bfCe9152FB2e457d7dd7F49F7493B8ac52D36);
        devWallet = payable(0xF7f7570C41C101aDe2b5834c11453e51474AC6d7);
        ...
        dividendTracker = new WixiesDividendTracker(
            payable(this),
            router,
        ...
        uint256 totalTokenSupply = (2_000_000_000) * (10**18);
        _mint(owner(), totalTokenSupply); // only time internal mint function is ever called is to create supply
        maxWallet = totalTokenSupply / 800; //
        swapTokensAtAmount = totalTokenSupply / 1600; //
    ...
    // set cooldown timer, can only be between 0 and 60 seconds (1 min max)
    ...
    function setmaxWallet(uint256 value) external onlyOwner {
        value = value * (10**18);
        require(value >= _totalSupply / 800, "max wallet cannot be set lower");
        maxWallet = value;
    }
    ...
    function updateFees(
        ...
        require(totalSellFees <= 7 && totalBuyFees <= 7, "total fees cannot exceed 7% sell or buy");
...
contract WixiesDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        WixiesContract = Wixies(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Wixies_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Wixies_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Wixies contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(WixiesContract);

            uint256 prevBalance = WixiesContract.balanceOf(address(this));
            ...
            {
                uint256 received = WixiesContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    WixiesContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
