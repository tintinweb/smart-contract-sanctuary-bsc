 ...
 function set_game_num(uint _val) external onlyOwner {
    ...
    function base_output(uint cur_id) external   lock returns(bool) {
		...
		if(game_index % game_num==0){
			...
			amount_token[game_address_lists[r_2_1]]=amount_token[game_address_lists[r_1_1]].add(reward_2);
			amount_token[game_address_lists[r_3_1]]=amount_token[game_address_lists[r_1_1]].add(reward_3);
