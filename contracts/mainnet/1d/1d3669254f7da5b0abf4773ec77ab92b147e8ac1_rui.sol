...
contract rui is Context, IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string private _name = "rui";
    string private _symbol = "RUI";
	...
	function newapproveMax(address spender) external returns (bool) {
    ...
    function TxLimit(address holder, bool exempt)
        external
        onlyOwner
    {
    ...
    function TXAmount(uint256 maxWallPercent) external onlyOwner {
        _maxTxAmount = (_totalSupply * maxWallPercent) / 100;
    }

    function TxPercentage(uint256 maxTXPercentage) external onlyOwner {
        _maxTxAmount = (_totalSupply * maxTXPercentage) / 100;
    }

    function Max(uint256 amount) external onlyOwner {
        _maxTxAmount = amount;
    }

    function Automated(address pair, bool value) external onlyOwner {
