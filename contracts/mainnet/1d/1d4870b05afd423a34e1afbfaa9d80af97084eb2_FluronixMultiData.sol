...
contract FluronixMultiData{
    ...
    function getMultiplePrice(GetPrice[] memory token_amounIn) public view returns (PriceOutput[] memory prices){
        ...
        prices = new PriceOutput[](token_amounIn.length);
        for(uint i = 0; i < token_amounIn.length; i++){
            prices[i] = getPrice(token_amounIn[i].token, token_amounIn[i].amountIn);
        }
    ...
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut) public pure returns (uint amountOut) {
        require(amountIn > 0, "Fluronix getPrice: INSUFFICIENT_INPUT_AMOUNT");
        require(reserveIn > 0 && reserveOut > 0, "Fluronix getAmountOut: INSUFFICIENT_LIQUIDITY");
        //uint amountInWithFee = amountIn.mul(9975);
        uint numerator = amountIn.mul(reserveOut);
        uint denominator = reserveIn.mul(10000).add(amountIn);
        amountOut = numerator / denominator;
    }
    //Get USD price of a given token, if price not available in USD then return in BNB
    function getPrice(address token, uint amountIn)public view returns  (PriceOutput memory out){
        address  Pair;
       ...
       (uint reserves0, uint reserves1,) = IPancakePair(Pair).getReserves(); 
        ...
        if(IPancakePair(Pair).token0() == token){
            out = PriceOutput(getAmountOut(amountIn, reserves0, reserves1), coinPicked, token);
        }
        else{
           out = PriceOutput(getAmountOut(amountIn, reserves1, reserves0), coinPicked, token); 
        }
    }
}