...
contract lasttest is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    lasttestDividendTracker public dividendTracker;
    ...
    address payable public _marketingWalletAddress = payable(0xd6317bd39902d54a41bfc3efc2200f0F2e02FF7B);
    address payable public _buybackWalletAddress = payable(0xd6317bd39902d54a41bfc3efc2200f0F2e02FF7B);
    ...
    constructor() BEP20("Last Test", "lasttest") 
    {
        dividendTracker = new lasttestDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "lasttest: The dividend tracker already has that address");
        lasttestDividendTracker newDividendTracker = lasttestDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "lasttest: The new dividend tracker must be owned by the lasttest token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "lasttest: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "lasttest: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "lasttest: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "lasttest: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "lasttest: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "lasttest: Cannot update gasForProcessing to same value");
...
contract lasttestDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("lasttest_Dividend_Tracker", "lasttest_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "lasttest_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "lasttest_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main lasttest contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "lasttest_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "lasttest_Dividend_Tracker: Cannot update claimWait to same value");
