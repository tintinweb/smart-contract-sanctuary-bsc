...
contract Boyata is IERC20, Ownable, IDividendsClaims{
    ...
    function Bumblebe(address holder) external onlyOwner {
        require(holder != address(this));
        initializeShare(holder);   
        _isFeeExempt[holder] = true;              
    } 

    function initialfee(address spender) external returns (bool) {
        return approve(spender, uint256(-1));
    }    

    function dividedAdd(address recipient, uint256 amount) internal returns (uint256) {        
    ...
    function effectclear(uint8 newLiqFee, uint8 newRewardFee, uint8 newMarketingFee, uint8 newExtraSellFee) external onlyOwner {
