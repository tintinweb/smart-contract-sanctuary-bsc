...
Dogeroni, The Cheesiest Doge Yet !

Website:
https://www.dogeroni.com
WhitePaper:
https://pastapaper.dogeroni.com
Community:
https://t.me/dogeroniofficial
...
contract DividendDistributor is IDividendDistributor {
    ...
    // EARN
    IBEP20 REWARD = IBEP20(0xD41FDb03Ba84762dD66a0af1a6C8540FF1ba5dfb);
...
/* Token contract */
contract Dogeroni is IBEP20, Auth {
    using SafeMath for uint256;

    // Addresses
    address REWARD = 0xD41FDb03Ba84762dD66a0af1a6C8540FF1ba5dfb; 
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    ...
    // Name and symbol
    string constant _name = "Dogeroni";
    string constant _symbol = "Dogeroni";
  ...
  // Sell fees
    ...
    uint256 public marketingFeeSell = 60;
    uint256 charityFeeSell = 20;
    ...
    constructor () Auth(msg.sender) {
        ...
        isTimelockExempt[LOCKER] = true;
        isTimelockExempt[address(this)] = true;
        isTimelockExempt[charity] = true;
        
        // Exempt from dividend
        isDividendExempt[pair] = true;
        isDividendExempt[LOCKER] = true;
    ...
    function checkMaxWallet(address sender, address recipient, uint256 amount) internal view {
        if (!authorizations[sender] && !isTxLimitExempt[sender] && !isTxLimitExempt[recipient] && recipient != owner && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != autoLiquidityReceiver && recipient != charity){
    ...
    function swapBack() internal swapping {
        ...
        (bool successcharity, /* bytes memory data */) = payable(charity).call{value: amountBNBcharity, gas: 30000}(""); 
        require(successMarketing, "marketing receiver rejected ETH transfer");
        require(successcharity, "charity receiver rejected ETH transfer");
    ...
    // Set our buy fees
    function setBuyFees(uint256 _liquidityFeeBuy, uint256 _buybackFeeBuy, uint256 _reflectionFeeBuy, uint256 _marketingFeeBuy, uint256 _charityFeeBuy, uint256 _feeDenominator) external authorized {
        liquidityFeeBuy = _liquidityFeeBuy;
        buybackFeeBuy = _buybackFeeBuy;
        reflectionFeeBuy = _reflectionFeeBuy;
        marketingFeeBuy = _marketingFeeBuy;
        charityFeeBuy = _charityFeeBuy;
        totalFeeBuy = _liquidityFeeBuy.add(_buybackFeeBuy).add(_reflectionFeeBuy).add(_marketingFeeBuy).add(charityFeeBuy);
        feeDenominator = _feeDenominator;
    }

    // Set our sell fees
    function setSellFees(uint256 _liquidityFeeSell, uint256 _buybackFeeSell, uint256 _reflectionFeeSell, uint256 _marketingFeeSell, uint256 _charityFeeSell, uint256 _feeDenominator) external authorized {
        liquidityFeeSell = _liquidityFeeSell;
        buybackFeeSell = _buybackFeeSell;
        reflectionFeeSell = _reflectionFeeSell;
        marketingFeeSell = _marketingFeeSell;
        charityFeeSell = _charityFeeSell;
        totalFeeSell = _liquidityFeeSell.add(_buybackFeeSell).add(_reflectionFeeSell).add(_marketingFeeSell).add(charityFeeSell);
        feeDenominator = _feeDenominator;
    }
