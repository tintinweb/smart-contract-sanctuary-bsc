...
contract RewardClaim is ReentrancyGuard,Ownable{
    ...
    modifier notPause() {
        require(!pause, "OPENOCEAN: PAUSING_NOW");
        _;
    }
    ...
    function AddTokenToPool(address token, uint256 amount_) external{
    ...
    function adminWithdrew(address token, uint256 amount_) external onlyOwner{
        require(IERC20(token).balanceOf(address(this)) >= amount_,"OPENOCEAN: WITHDREW_TOO_MUCH");
        IERC20(token).transfer(msg.sender,amount_);
    }

    function claim(rewardClaim memory input) external nonReentrant notPause{
        ...
        IERC20 _token = IERC20(input.token);
        require(_token.balanceOf(address(this)) >= input.amount,"TOKEN_IS_NOT_ENOUGHT_PLZ_CONTACT_ADMIN");
        _token.transfer(msg.sender,input.amount);
        emit logGasRefund(input.user, input.Hash, input.amount, input.nonce);
    }

    function _verifyInputSignature(rewardClaim memory input) internal virtual {
        bytes32 hash = keccak256(abi.encode(input.user, input.amount, input.nonce, input.chainID));
        require(hash == input.Hash,"WRONG_ENCODE");
        require(block.chainid == input.chainID, "OPENOCEAN: WRONG_CHAIN");
