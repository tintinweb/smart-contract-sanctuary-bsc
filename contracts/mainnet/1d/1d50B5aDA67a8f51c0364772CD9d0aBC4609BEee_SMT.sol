...
contract SMT is Context, IBEP20, Ownable {
    ...
    */
    ...
    function setBNBInfo(uint256 _price, uint256 _decimal) 
        public onlyOperator 
    {
        _tokenPriceByBNB = _price;
        _bnbDec = _decimal;
        emit UpdatedBNBInfo(_price, _decimal);
    }

    /**
     * @dev Sets buying token info.
     */
    function setBuyingTokenInfo(
        address _assetToken,
        uint256 _price,
        uint256 _decimal
    ) public onlyOperator {
        require(_assetToken != address(0x0), "asset token can't be zero address");
        _mapBuyingToken[_assetToken] = BuyingTokenInfo(_price, _decimal);
        emit UpdatedBuyingTokenInfo(_assetToken, _price, _decimal);
    }
    ...
    */
    function setRouter(
        ...
        require(address(_router) != address(0x0), "Router address can't be zero address");
        _uniswapV2Router = _router;
        emit UpdatedExchangeRouter(_router);
    }
    ...
    */
    function createBNBPair() public onlyOperator {
        require(_uniswapV2Router != address(0x0), "Router address can't be zero address");
    ...
    */
    function createPair(address _assetToken) public onlyOperator {
        require(address(_uniswapV2Router) != address(0x0), "Router address can't be zero address");
        require(address(_assetToken) != address(0x0), "Asset token address can't be zero address");
        address pairAsset = IUniswapV2Factory(IUniswapV2Router02(_uniswapV2Router).factory())
            .createPair(address(this), _assetToken);
