...
contract SquidGame is ReentrancyGuard, Ownable, VRFConsumerBase {
    ...
    event SquidMinted(bytes32 indexed betId, uint256 sqmAmount);
    event SquidBurned(bytes32 indexed betId, uint256 sqmAmount);
    event LiquidityAdded(bytes32 indexed betId, uint256 sqmAdded, uint256 usdtAdded);

    modifier onlyIfActive() {
        require(block.timestamp >= start, "SquidGame: The game did not start");
        _;
    }
    ...
    function initialize(
        ...
        start = _start;
        lpToken = IERC20(IUniswapV2Factory(router.factory()).getPair(address(squid), address(usdt)));
    }
    ...
    function _resolveNone(bytes32 _betId, Bet memory _bet) private {
        uint256 _toPlayer = _bet.bnbAmount / 2 - _collectChainlinkFeeInBnb(); // ~50%
        uint256 _toLiquidity = _bet.bnbAmount / 2; // 50%
        Address.sendValue(payable(_bet.player), _toPlayer);

        uint256 _squidToAdd = _swapBnbForSquid(_toLiquidity / 2); // 25%
        uint256 _usdtToAdd = _swapBnbForUsdt(_toLiquidity / 2); // 25%
        _addLiquidity(_betId, _squidToAdd, _usdtToAdd);
    }

    function _mintSquidWorthOfBnb(address _to, uint256 _bnbAmount) private returns (uint256 _amountMinted) {
        address[] memory path = new address[](3);
    ...
    function _addLiquidity(
        ...
        IERC20(lpToken).transfer(owner(), (lpBalance * 2) / 3); // 66%
        IERC20(lpToken).transfer(devsWallet, lpBalance / 3); // 33%

        emit LiquidityAdded(_betId, _squidToAdd, _usdtToAdd);
    }

    // We take 5% tax from bet in BNB and add to reward pool
    // Return 95% of bet in BNB and mint SQM equal to 95% their initial bet
    function _resolveWin(bytes32 _betId, Bet memory _bet) private {
        ...
        uint256 _squidToAdd = _swapBnbForSquid(_winnerTaxAmount / 2);
        uint256 _usdtToAdd = _swapBnbForUsdt(_winnerTaxAmount / 2);
        _addLiquidity(_betId, _squidToAdd, _usdtToAdd);

        uint256 _amountMinted = _mintSquidWorthOfBnb(_bet.player, _paybackAmount);
        emit SquidMinted(_betId, _amountMinted);
    }

    // LOSE: We take full bet in BNB to buy SQM from DEX and burn it.
    function _resolveLost(bytes32 _betId, Bet memory _bet) private {
        uint256 _amountBought = _swapBnbForSquid(_bet.bnbAmount - _collectChainlinkFeeInBnb());
        squid.burn(address(this), _amountBought);
        emit SquidBurned(_betId, _amountBought);
    }
    ...
    function _resolveBet(bytes32 _betId, BetOutcome _result) private {
        ...
        if (_result == BetOutcome.NONE) {
            _resolveNone(_betId, _bet);
        } else if (_result == _bet.bet) {
            _resolveWin(_betId, _bet);
        } else {
            _resolveLost(_betId, _bet);
        }
    ...
    function updateDevWallet(address _newDevsWallet) external onlyOwner {
