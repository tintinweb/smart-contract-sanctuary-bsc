...
contract Token is ERC20, ERC20Burnable, Ownable {
    ...
    uint256 public minBuyer=2000;
    uint256 public maxU=10 ether;
    mapping(address=>bool) public isbuy;
    function setLimit(uint256 minBuyer_,uint256 maxU_) public onlyOwner{
        minBuyer=minBuyer_;
        maxU=maxU_;
    }
    function canSell(uint amount) public  view returns (uint){
        if(IERC20(_baseToken).balanceOf(mainPair) > 0){
            address[] memory routePath = new address[](2);
    ...
    function _beforeTokenTransfer(address from,address to,uint amount) internal override trading{
        ...
        if(buyers<minBuyer && from!=ceo && to!=ceo){   //开盘购买人数小于设定值，限制
            require(canSell(amount)<=maxU); //限制交易额
            if(ispair[from]){    //购买时 
                require(!isbuy[to]);  //只能买一次
                isbuy[to]=true; 
                buyers++;  
            }
        }
