...
// File: contracts\TYZENMaker.sol

pragma solidity 0.6.12;


// This contract handles "serving up" rewards for xTYZEN holders by trading tokens collected from fees for TYZEN.
contract TYZENMaker  is Ownable {
    using SafeMath for uint;
    using SafeERC20 for IERC20;
    IUniswapV2Router02 public routerv2;
    IUniswapV2Factory public factory;
    address public tyzenSafe;
    address public tyzen;
    address public weth;
    ...
    function removeLiqidity(address token0, address token1, uint amount) public {
        require(operators[msg.sender], 'TYZENMaker: no permistion');
    ...
    function marketBuyTYZENWithETH(address[] calldata path, uint amount, uint deadline) public {
      require(operators[msg.sender], 'TYZENMaker: no permistion');
      require(path[path.length - 1] == tyzen, 'TYZENMaker: wrong path');
      for (uint i = 1; i < path.length - 1; i++) {
        require(acceptTokens[path[i]], 'TYZENMaker: wrong path');
      }
    ...
    function marketBuyTYZENWithToken(address[] calldata path, uint amount, uint deadline) public {
      require(operators[msg.sender], 'TYZENMaker: no permistion');
      address token = path[0];
      if (token == tyzen) {
          IERC20(token).safeTransfer(tyzenSafe, amount);
      } else  {
        require(path[path.length - 1] == tyzen, 'TYZENMaker: wrong path');
        for (uint i = 1; i < path.length - 1; i++) {
          require(acceptTokens[path[i]], 'TYZENMaker: wrong path');
    ...
    function convert(address token0, address token1) public {
        ...
        if (IERC20(weth).balanceOf(address(this)) > 0) {
          ...
          _swap(weth, tyzen, IERC20(weth).balanceOf(address(this)));
        }

        if (IERC20(tyzen).balanceOf(address(this)) > 0) {
          IERC20(tyzen).safeTransfer(tyzenSafe, IERC20(tyzen).balanceOf(address(this)));
        }
    }
    ...
    function _toBaseToken(address token, uint amountIn) internal {
      if (token == tyzen) return;

      if (_canSwap(token, tyzen)) {
        _swap(token, tyzen, amountIn);
      }
