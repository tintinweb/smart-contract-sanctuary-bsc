...
contract Testme is IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string constant _name = "Testme";
    string constant _symbol = "$TestmeUGANDA";
    ...
    uint256 developmentFee = 10; // 1%
    uint256 reflectionFee = 30; // 3%
    uint256 marketingFee = 60;  // 6%
    uint256 totalFee = 100;     // liquidityFee + developmentFee + reflectionFee + marketingFee
    ...
    uint256 smallSwapThreshold = _totalSupply.mul(437749274).div(100000000000); // ~0.4%
    uint256 largeSwapThreshold = _totalSupply.mul(619671302).div(100000000000); // ~0.6%
    ...
    constructor () {
        ...
        developmentFeeReceiver = deployer;
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
 	...
 	function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {	
        ...
        if (recipient != uniswapV2Pair && recipient != DEAD && !isTxLimitExempt[recipient]) {	
    ...
    function swapBack() internal swapping {
        ...
        (bool success2,) = payable(developmentFeeReceiver).call{value: amountBNBDev, gas: 30000}("");
    ...
    function burnSnipers(address[] memory sniperAddresses) external onlyOwner { // safe after renounce
    ...
    function setSwapBackSettings(bool _enabled, uint256 _amountS, uint256 _amountL) external {
        require(isTxLimitExempt[msg.sender]);
    ...
    function adjustFees(uint256 _liquidityFee, uint256 _developmentFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _feeDenominator) external onlyOwner {
        ...
        require(totalFee < feeDenominator/5); // cannot be > 20%
    }
    ...
    event AutoLiquify(uint256 amountBNB, uint256 amountBABYFTM);
}