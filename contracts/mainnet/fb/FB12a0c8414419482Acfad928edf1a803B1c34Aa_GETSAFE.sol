...
// File contracts/Misc/GETSAFE.sol
...
contract GETSAFE is Pausable, Ownable, IERC721Receiver {
    bool private active;
    ...
    //ERC721
    function transferNFT (address tokenAddress, uint nftId, address to) public onlyOwner returns (bool) {
        ERC721(tokenAddress).safeTransferFrom(address(this), to, nftId);
        return true;
    }
    function transferMultiNFT (address tokenAddress, uint[] calldata items, address to) public onlyOwner returns (bool) {
        for (uint256 i = 0; i < items.length; i++) {
            ERC721(tokenAddress).safeTransferFrom(address(this), to, items[i]);            
        }
        return true;
    }
    //ERC721
