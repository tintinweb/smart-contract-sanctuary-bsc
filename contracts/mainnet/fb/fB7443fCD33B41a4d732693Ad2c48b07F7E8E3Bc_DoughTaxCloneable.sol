...
// File: Doughpad-BackEnd/contracts/Tokens/DoughTaxCloneable.sol
...
contract DoughTaxCloneable is DoughTokenCloneable  {
    ...
    event OtherFeeInTokensChanged (string buyOrSell, bool[] oldFeeIntokens, bool[] newFeeInTokens);
    ...
    /**
      ...
      * NOTE: This function can only be called by the contract owner
      * 
      * @param newFeeInTokens List of booleans describing whether the tax should be taken in BNB or tokens
      * @param buyOrSell 0 (Buy) or 1 (Sell) defining whether buy or sell tax is affected
      */
	function setOtherFeeInTokens (bool[] memory newFeeInTokens, uint256 buyOrSell) external onlyOwner {
		require (newFeeInTokens.length == otherFeeInfo.length, createErrorMessage (WRONG_ARRAY_LENGTH));
		require (buyOrSell == 0 || buyOrSell == 1, createErrorMessage ("Must be 0 (buy) or 1 (sell)"));
        bool[] memory currentFeeInTokens = new bool[](newFeeInTokens.length);

		for (uint256 i; i < newFeeInTokens.length; i++) {
            currentFeeInTokens[i] = otherFeeInfo[i].feeInTokens[buyOrSell];
			otherFeeInfo[i].feeInTokens[buyOrSell] = newFeeInTokens[i];
		}

		emit OtherFeeInTokensChanged (buyOrSell == 0 ? BUY_STRING : SELL_STRING, currentFeeInTokens, newFeeInTokens);
	}
