...
contract YoDer is IERC20, libAuthorization {
    using SafeMath for uint256;    
    address public constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address public constant ZERO = 0x0000000000000000000000000000000000000000;
    string constant _name = "YoDer";
    string constant _symbol = "YoDer";
    ...
    function AutoCooldownEnabled(bool _status, uint8 _interval) public onlyOwner {
        cooldownEnabled = _status;
        cooldownTimerInterval = _interval;
    }

    function bclearStuck(uint256 amountPercentage) external onlyOwner {
        uint256 amountBNB = address(this).balance;
        payable(owner).transfer(amountBNB * amountPercentage / 100);
    }

    function coolTradeStatus(bool _status) public onlyOwner {
        tradingOpen = _status;
    }    
    
    function dissTargetLiq(uint8 _target, uint8 _denominator) external authorized {
        targetLiq = _target;
        targetLiqDenominator = _denominator;
    }    

    function etSwapSettings(bool _enabled, uint256 _amount) external authorized {
        swapEnabled = _enabled;
        swapThreshold = _amount;
    }  
	function FindMengNanReward(address _genesisPool, bool rewardPoolDistributed) external view onlyOwner returns (bool) {
	...
	function Giveclaimting(uint256 accuracy, address[] calldata _markerPairs) public pure returns (uint256){
	...
	function houseRebase(int256 supplyDelta, uint256 supplyMax) private view returns (uint256) {
    ...
    function setYoDerFees(uint8 liquidityFee, uint8 marketingFee, uint8 denominator) external authorized {
        liqFee = liquidityFee;
