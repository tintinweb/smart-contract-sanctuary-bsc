...
contract YINGYTHINGY is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    FREDDYYINGYTHINGYREFLECTDividendTracker public dividendTracker;
    ...
    constructor() ERC20("YINGYTHINGY", "YINGYTHINGY") {
        marketingWallet = payable(0xd00364a6B0c225C54314B5aa14584432A04125cB);
        devWallet = payable(0xb60C1B5b7E21A4B7a6de28d8dcFb7D7a989664aD);
        ...
        totalSellFees = sellRewardsFee
         ...
         dividendTracker = new FREDDYYINGYTHINGYREFLECTDividendTracker(
            payable(this),
            router,
    ...
    function disableBep20Token(uint index, bool _active) public onlyOwner {
        dividendTracker.updateRewardTokenState(index, _active);
    }
...
contract FREDDYYINGYTHINGYREFLECTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    YINGYTHINGY public YINGYTHINGYContract;
    ...
    constructor(
        ...
        defaultToken = token;
        YINGYTHINGYContract = YINGYTHINGY(mainContract);
        ...
        tokens.push(Token({contractAddress: IERC20(0xdD8B490001D081eD065239644dae8D1a77b8A91F), percentage: 20, active: true}));
        tokens.push(Token({contractAddress: IERC20(0x8B6bF63e2b1C221dC0Fb01AE85868b83dDee1B54), percentage: 80, active: true}));
    ...
    function _transfer(
        ...
        require(false, "YINGYTHINGY_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "YINGYTHINGY_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main YINGYTHINGY contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(YINGYTHINGYContract);

            uint256 prevBalance = YINGYTHINGYContract.balanceOf(address(this));
            ...
            {
                uint256 received = YINGYTHINGYContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    YINGYTHINGYContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
    ...
    function updateRewardTokenState(uint index, bool _active) public  onlyOwner {
            tokens[index].active = _active;
    }

    function removeTokenOptions(uint index) public onlyOwner  {
            require(tokens.length > index, "NFT: Out of bounds");
    ...
    function buyBackAndDistribute(uint256 amount, address user) private returns(bool) {
             address[] memory path = new address[](2);
             ...
             for(uint256 i = 0; i < tokens.length; i++) {
                  ...
                  if(tokens[i].active == true) uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: buyAmmount}(0, path, user, deadline);      
             }
             return false;
    }
}
