...
// File: contracts/strategies/USDDLPStrategyStakeOnlyV2.sol
...
contract USDDLPStrategyStakeOnlyV2 is Ownable {
    using SafeMath for uint256;
    ...
    address constant USDDLP_POOL_ADDRESS = address(0xC2cF01F785C587645440ccD488B188945C9505e7); // USDD, BUSD, USDC, USDT
    ...
    function withdraw(address _depositor, uint256 _share, uint256 _total, bool nonContract) public onlyZSBToken returns (uint256) {
        ...
        if(nonContract == true){
            if( _share > _total.mul(percentTradeTrigger).div(DIVISION_FACTOR)){
                harvestRewards(); // This will claim rewards then sell EPX && USDD for USDDLP and some for WBNB
            }
        }
    ...
    */
    ...
    function checkHarvestWBNB() internal view returns (uint256) {
        ...
        // Next get the amount of USDD rewards
        uint256 usddAmount = EllipsisLP(address(tokenList[0].token)).earned(address(this), USDD_ADDRESS);
        usddAmount = usddAmount.add(IERC20(USDD_ADDRESS).balanceOf(address(this)));
        if(usddAmount > 0){
            // Convert all of it to BUSD then some to WBNB
            uint256 outAmount = CurveLikeExchange(USDDLP_POOL_ADDRESS).get_dy_underlying(0, 1, usddAmount);
            outAmount = outAmount.mul(DIVISION_FACTOR.sub(percentStakeDepositor)).div(DIVISION_FACTOR);
            // Then convert from BUSD to WBNB
            TradeRouter router = TradeRouter(PANCAKE_ROUTER);
            address[] memory path = new address[](2);
            path[0] = BUSD_ADDRESS;
            path[1] = WBNB_ADDRESS;
            uint256[] memory estimates = router.getAmountsOut(outAmount, path);
            outAmount = estimates[estimates.length - 1];
            rewards = rewards.add(outAmount); // Add to the total WBNB
        }
    ...
    function harvestRewards() internal {
        ...
        // Now handle USDD rewards
        EllipsisLP(address(tokenList[0].token)).getReward();
        uint256 usddAmount = IERC20(USDD_ADDRESS).balanceOf(address(this));
        if(usddAmount > 0){
            // Convert all of it to BUSD then part of it to WBNB
            CurveLikeExchange curve = CurveLikeExchange(USDDLP_POOL_ADDRESS);
            IERC20(USDD_ADDRESS).safeApprove(address(curve), 0);
            IERC20(USDD_ADDRESS).safeApprove(address(curve), usddAmount);
            uint256 _bal = IERC20(BUSD_ADDRESS).balanceOf(address(this));
            curve.exchange_underlying(0, 1, usddAmount, 1);
            uint256 outAmount = IERC20(BUSD_ADDRESS).balanceOf(address(this)).sub(_bal);
            // Take a piece of this BUSD and convert to WBNB
            outAmount = outAmount.mul(DIVISION_FACTOR.sub(percentStakeDepositor)).div(DIVISION_FACTOR);
            address[] memory path = new address[](2);
            path[0] = BUSD_ADDRESS;
            path[1] = WBNB_ADDRESS;
            IERC20(BUSD_ADDRESS).safeApprove(PANCAKE_ROUTER, 0);
            IERC20(BUSD_ADDRESS).safeApprove(PANCAKE_ROUTER,outAmount);
            router.swapExactTokensForTokens(outAmount, 1, path, address(this), now.add(60));
        }
