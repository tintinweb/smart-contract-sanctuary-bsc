...
interface IPandoPool {
    function lastAllocatedTime() external returns(uint256);
    function allocateDay() external returns(uint256);
    function minAllocateInterval() external returns(uint256);
    function migrateTo(IERC20[] memory _tokens) external;
    function migrateFrom(IERC20[] memory _tokens) external;
}
...
import "../interfaces/IPandoPool.sol";
...
contract PandoPoolV2 is Ownable, Pausable {
    ...
    //migrate
    address public previousPandoPool;
    address public nextPandoPool;

    constructor (address[] memory _tokens, uint256[] memory _allocationDailyPercent, address _previousPandoPool) {
        if(_previousPandoPool != address(0)) {
            lastAllocatedTime = IPandoPool(_previousPandoPool).lastAllocatedTime();
            allocateDay = IPandoPool(_previousPandoPool).allocateDay();
            minAllocateInterval = IPandoPool(_previousPandoPool).minAllocateInterval();
            previousPandoPool = _previousPandoPool;
        } else {
            lastAllocatedTime = 0;
            allocateDay = 1;
        }
    ...
    modifier onlyMigrator() {
        require(msg.sender == nextPandoPool || msg.sender == owner(), "PandoPool: only migrator");
        _;
    }
    ...
    function setOtherPandoPool(address _previous, address _next) external onlyOwner {
        previousPandoPool = _previous;
        nextPandoPool = _next;
        emit SetOtherPandoPool(_previous, _next);
    }

    function migrateTo(IERC20[] memory _tokens) external onlyMigrator whenPaused {
        if(nextPandoPool != address(0) && _tokens.length > 0) {
            for(uint256 i = 0; i < _tokens.length; i++) {
                IERC20 _token = _tokens[i];
                uint256 _amount = _token.balanceOf(address(this));
                _token.safeTransfer(nextPandoPool, _amount);
                emit Migrate(address(this), nextPandoPool, address(_token), _amount);
            }
        }
    }

    function migrateFrom(IERC20[] memory _tokens) external onlyOwner {
        if(previousPandoPool != address(0) && _tokens.length > 0) {
            IPandoPool(previousPandoPool).migrateTo(_tokens);
        }
    }
    ...
    event Migrate(address _from, address _to, address _token, uint256 _amount);
    event SetOtherPandoPool(address _previous, address _next);
}