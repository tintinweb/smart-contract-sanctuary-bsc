    ...
    PriceControl
{
    ...
    event BuyLootBox(uint8 tier, uint256 timestamp, uint8 paymentMethod);
    event FreeLootBox(uint8 tier, uint256 timestamp);
    event DiscountLootBox(uint8 tier, uint256 timestamp);
    ...
    function setFreeWhiteList(
        ...
        require(
            _whiteListed.length == _tiers.length,
            "Error: Arrays length must be equal"
        );
    ...
    function buyLootBox(
        ...
        emit BuyLootBox(tier, block.timestamp, paymentMethod);
    }
    ...
    function lootBoxFreeWhiteList(address _whiteListed) external {
        FreeWhiteList storage freeWhiteList = mapOfFreeWhiteList[msg.sender];
        ...
        emit FreeLootBox(freeWhiteList.tier, block.timestamp);
    }
    ...
    function lootBoxDiscountWhiteList(address _whiteListed) external {
        ...
        _checkAndControlSupply(discountWhiteList.tier);
        discountWhiteList.status = false;
        uint256 price = getPriceByTier(discountWhiteList.tier) -
            ((getPriceByTier(discountWhiteList.tier) *
        ...
        emit DiscountLootBox(discountWhiteList.tier, block.timestamp);
    }
    ...
    function _clanBadgeGeneration() internal {
        IClanBadge(clanBadge).safeMint(
            msg.sender,
            "https://outerringmmo.mypinata.cloud/ipfs/Qmc361pFw2PHX36ABoJn54MJtMRJmSifVuidFMMCEHczmN"
        );
    }
    ...
    function _paymentWithToken(uint256 price) internal {
        uint256 priceToPay = price - ((price * discount) / 10000);
