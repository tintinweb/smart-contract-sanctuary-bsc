...
import "./RescueTokens.sol";

contract MutantFroggoTokenClaim is Ownable, AccessControl, RescueTokens {
    ...
    function claim(uint256 deadline, uint256 amount, uint8 v, bytes32 r, bytes32 s) external {
        require(claimEnabled, "MutantFroggo: Claim disabled");
        require(!userClaim[msg.sender], "MutantFroggo: Already Claimed!");
        require(deadline >= block.timestamp, "MutantFroggo: Expired");
        ...
        require(hasRole(CLAIM_SIGNER_ROLE, signer), "MutantFroggo: Invalid Signature");
    ...
    function setSignerRoleAdmin(address admin) external onlyOwner {
        require(admin != address(0), "MutantFroggo: Invalid admin");
        _setupRole(DEFAULT_ADMIN_ROLE, admin);
    }

    function grandRewardSignerRole(address account) external onlyOwner {
        require(account != address(0), "MutantFroggo: Invalid account");
...
contract RescueTokens is Ownable {
    using SafeERC20 for IERC20;
