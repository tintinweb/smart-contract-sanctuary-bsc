...
//TG: https://t.me/NinjutsuInuPortal
...
contract NINJUTSUINU is Context, IERC20, Ownable {
    ...
    uint8 public cooldownTimerInterval = 2; // In Seconds
    ...
    address public marketingWallet = 0x8Def2c6536a93E038096CA69e0Bb7720701a3322;
    ...
    Taxes public taxes = Taxes(1,1,1,0);
...
function blackListAddress(address addr) external onlyOwner returns (bool) {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        require(amount <= balanceOf(from),"excede your balance");
        
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            if(to != pair) require(balanceOf(to) + amount <= maxWalletAmount, "maxWalletAmount");
      ...
      //blacklist
        ...
        buyCooldownEnabled &&
            !isTimelockExempt[to]) {
            require(cooldownTimer[to] < block.timestamp,"cooldown");
    ...
    function rescueBNB(uint256 weiAmount) external onlyOwner{
        require(address(this).balance >= weiAmount, "ins. BNB bal.");
