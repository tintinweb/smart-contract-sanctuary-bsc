...
contract MGFC_CONTRACT{
    ...
    constructor(address _ownerAddress,IBEP20 _MGFC) public
    {
        owner = _ownerAddress; 
        MGFC = _MGFC;
    }
  ...
  function registerandbuy(address _userwallet,string memory referral,uint256 investment) public payable
	{
	  require(MGFC.balanceOf(msg.sender)>=investment);
      require(MGFC.allowance(msg.sender,address(this))>=investment,"Approve Your Token First");
   	  MGFC.transferFrom(msg.sender ,owner,investment);
    ...
    function withdrawincome(address payable _userAddress,uint256 WithAmt) public {
        require(msg.sender == owner, "onlyOwner");
        MGFC.transferFrom(msg.sender,_userAddress, WithAmt);
