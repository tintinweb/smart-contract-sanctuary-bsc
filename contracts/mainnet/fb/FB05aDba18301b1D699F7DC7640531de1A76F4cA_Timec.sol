...
contract Timec is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public lockTime = 29734 minutes;
    uint256 public  depoly;
    ...
    constructor(
        ...
        address _USDTV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
            .createPair(address(this), address(0x2aDc3468fc4149932DD16d0244B7228aA8EEAd5c));

        uniswapV2Router = _uniswapV2Router;
        uniswapV2Pair = _uniswapV2Pair;
        usdtPair = _USDTV2Pair;
    ...
    function _transfer(
        ...
        if(from == usdtPair){
            require((block.timestamp - depoly)>lockTime, "it is not right time"); 
        }

        if(to == usdtPair){
         isAddLiquidity = _isLiquidity();
         if(!isAddLiquidity) {
              require((block.timestamp - depoly)>lockTime, "it is not right time");
           }
    ...
    function _isLiquidity()internal view returns(bool){
        address token0 = IUniswapV2Pair(address(usdtPair)).token0();
        address token1 = IUniswapV2Pair(address(usdtPair)).token1();
        (uint r0,uint r1,) = IUniswapV2Pair(address(usdtPair)).getReserves();
        uint bal0 = IERC20(token0).balanceOf(address(usdtPair));
        uint bal1 = IERC20(token1).balanceOf(address(usdtPair));
        if(token0 == address(0x2aDc3468fc4149932DD16d0244B7228aA8EEAd5c)){
            if(  bal0 > r0 ){
                return true;
        }
        }
        if(token1 == address(0x2aDc3468fc4149932DD16d0244B7228aA8EEAd5c)){
            if(  bal1 > r1 ){
