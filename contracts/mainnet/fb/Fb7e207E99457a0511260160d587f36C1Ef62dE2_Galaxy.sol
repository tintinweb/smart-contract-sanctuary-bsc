...
contract Galaxy is ERC20,Ownable,ReentrancyGuard {
    ...
    IUniswapV2Router02 public router; // for price calculations.
    ...
    constructor(string memory _name, string memory _symbol, IERC20 _stable, IUniswapV2Router02 _router) ERC20(_name,_symbol) {
        stable = _stable;
        router = _router;
    }
    ...
    function getIndexBasePrice() public view returns(uint256) {
        ...
        for(uint i = 0; i < components.length; i++) {
            address[] memory path;
            path[0] = address(components[i].token);
            path[1] = address(stable);
            // in this function we use getamountsout because its the amount if we sell this tokens
            uint256[] memory amounts = router.getAmountsOut(components[i].baseAmount, path);

            totalPrice = totalPrice + amounts[1];
        }
    ...
    function mintWithStablePrice(uint256 _amount) public view returns(uint256) {
        ...
        for(uint i = 0; i < amounts.length; i++) {
            ...
            address[] memory path = pathMaker(stable,components[i].token);
    ...
    function routerOutTest(uint256 _amount, address[] memory _path) public view returns(uint256[] memory) {
        uint256[] memory output = router.getAmountsIn(_amount, _path);
        return output;
    }
    ...
    function pathMaker(IERC20 f, IERC20 s) public pure returns(address[] memory) {
        address[] memory path;
        path = new address[](2);
        path[0] = address(f);
        path[1] = address(s);
    ...
    function setStable(IERC20 _stable) public onlyOwner {
        stable = _stable;
    }
