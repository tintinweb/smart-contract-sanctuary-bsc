...
contract FRT is IBEP20, Auth {
    
    using SafeMath for uint256;

    string constant _name = "Fart";
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingWallet = 0x09b2336087a76490cc000d4096f666d2c1A9DBb8; 
        fartWallet = 0xE69690c63Fe53964643467A87327B0f381BFC530; 
        
        isFeeExempt[marketingWallet] = true;
        isTxLimitExempt[marketingWallet] = true;

        totalFee = (liquidityFee.add(marketingFee).add(rewardsFee).add(fartFee));
    ...
    onlyOwner
    {
        for (uint256 i; i < addresses.length; ++i) {
            ...
            if(status){
                dividendDistributor.setShare(addresses[i], 0);
            }else{
    ...
    function setMode(bool _bool) external authorized {
        checkOn = _bool;
    }
    ...
    function changeFees(uint256 newLiqFee, uint256 newRewardFee, uint256 newMarketingFee, uint256 newFartFee, uint256 extraSellFee) external authorized {
    ...
    function changeFeeReceivers(address newLiquidityReceiver, address newMarketingWallet, address newFartWallet) external authorized {
        autoLiquidityReceiver = newLiquidityReceiver;
        marketingWallet = newMarketingWallet;
        fartWallet = newFartWallet;
    }
    ...
    function swapBack() internal lockTheSwap {
        ...
        uint256 amountBNBFart = amountBNB.mul(fartFee).div(totalBNBFee);
        ...
        (bool tmpSuccess1,) = payable(fartWallet).call{value: amountBNBFart, gas: 30000}("");
