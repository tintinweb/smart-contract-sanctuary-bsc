...
contract BasicBridge is EternalStorage, Validatable, Ownable, OwnedUpgradeability {
    ...
    function getCollectedFee() public view returns(uint){
        return  uintStorage[keccak256(abi.encodePacked("collectedFee"))];
    }

    function addCollectedFee(uint _amount) internal {
        uintStorage[keccak256(abi.encodePacked("collectedFee"))]=getCollectedFee().add(_amount);
    }

    function setZeroCollectedFee() internal {
        uintStorage[keccak256(abi.encodePacked("collectedFee"))]= 0;
    }

    function claimFeeToken(address _token,address _to) public onlyOwner {
        require(_to != address(0));
        if (_token == address(0)) {
            _to.transfer(address(this).balance);
            return;
        ...
        ERC20Basic token = ERC20Basic(_token);
        uint256 balance = getCollectedFee();
        require(token.transfer(_to, balance));
        setZeroCollectedFee();
    }
...
contract ForeignBridgeErcToErc is BasicBridge, BasicForeignBridge, FeeManager {
    ...
    function onExecuteMessage(address _recipient, uint256 _amount) internal returns(bool){
        ...
        if (feePercent() == 0) {
            ...
            super.addCollectedFee(entireValidatorValue);
