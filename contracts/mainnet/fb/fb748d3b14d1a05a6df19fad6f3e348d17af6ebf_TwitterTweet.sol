...
library Address{
 ...
 contract TwitterTweet is IERC20, Context, Ownable {
    ...
    string private constant _symbol = "TITIT";
    string private constant _name = "TwitterTweet";

    uint256 public TITIT_max_BuyLimit = 25000000 * 10**9;
    uint256 public TITIT_max_SellLimit = 25000000 * 10**9;

    address private TITIT_Wallet = 0xbAD8Cf4C7f3c95e738e28bC8D380AcA6b2a283ea;
    address private TITIT_Team_Wallet = 0xbAD8Cf4C7f3c95e738e28bC8D380AcA6b2a283ea;

    uint256 public TITIT_max_WalletLimit = 25000000 * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[TITIT_Wallet] = true;
        _isExcludedFromFee[TITIT_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[TITIT_Wallet] = true;
        allowedTransfer[TITIT_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= TITIT_max_BuyLimit, "You are exceeding TITIT_max_BuyLimit");
            require(balanceOf(to) + amount <= TITIT_max_WalletLimit, "You are exceeding TITIT_max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= TITIT_max_SellLimit, "You are exceeding TITIT_max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= TITIT_max_WalletLimit, "You are exceeding TITIT_max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(TITIT_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(TITIT_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function TITIT_Update_MarketingWallet(address newWallet) external onlyOwner{
        TITIT_Wallet = newWallet;
    }
    
    function TITIT_Update_TeamWallet(address newWallet) external onlyOwner{
        TITIT_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        TITIT_max_BuyLimit = maxBuy * 10**decimals();
        TITIT_max_SellLimit = maxSell * 10**decimals();
    }
    
    function TITIT_Max_Walletlimit(uint256 amount) external onlyOwner{
        TITIT_max_WalletLimit = amount * 10**decimals();
    }
