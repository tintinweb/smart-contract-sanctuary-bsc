...
contract SurgeForkLottery is ReentrancyGuard {
    ...
    address private buybackwallet = 0x5d3bd308E16f818409A274876D52D84d23Acb768;
    address private dev = 0x4d95EE4B36226Eea67b166310a6496F607B0Ea89;
    ...
    constructor() {
        ...
        ticketPrice = 300 * 10**9;

        maxParticipantNumbers1 = 5;
        ticketPrice1 = 500 * 10**9;

        maxParticipantNumbers2 = 4;
        ticketPrice2 = 2000 * 10**9;

        tokenAdress = 0x9f19c8e321bD14345b797d43E01f0eED030F5Bff;
        BusdInterface = IERC20(tokenAdress);
    }

    event logshh(uint256 _id, uint256 _value);

    function setBusdInterface(address _tokenAddress) external onlyOwner {
    ...
    function joinLottery(uint256 _amount) external notOwner noReentrant {
        ...
        bool chk = BusdInterface.transferFrom(
            msg.sender,
            address(this),
            _amount
        );
        ...
        )
    {
        ...
        uint256 lHm1 = 0;
        uint256 lHm2 = 0;
        uint256 arrayLength = participants.length;
        if (arrayLength != 0) {
        ...
        uint256 arrayLength1 = participants1.length;
        if (arrayLength1 != 0) {
            for (uint256 i = 0; i < arrayLength1; i++) {
                // do something
        ...
        uint256 arrayLength2 = participants2.length;
        if (arrayLength2 != 0) {
    ...
    function pickwinner() internal {
        ...
        uint256 BuyBackFee = SafeMath.div(
            SafeMath.mul(contractBalance, 30),
            100
        ); //buyback fee
        uint256 devFee = SafeMath.div(SafeMath.mul(contractBalance, 0), 100); //set dev fee

        BusdInterface.transfer(buybackwallet, BuyBackFee);
        uint256 winnerAmount = SafeMath.sub(contractBalance, BuyBackFee);
        winnerAmount -= devFee;
    ...
    function pickwinner1() internal {
        ...
        uint256 devFee = SafeMath.div(SafeMath.mul(contractBalance, 0), 100);
