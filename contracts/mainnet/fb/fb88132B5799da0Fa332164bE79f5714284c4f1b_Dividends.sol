...
// 基类合约
    contract Base {
        ...
        Erc20Token constant internal FTE    = Erc20Token(0x0b74080FE228C986De9bba11f6887907Ce18A37B); 
...
contract Dividends is Base {
  ...
  function Recharge(uint256 Quantity,uint256 tokenType) public payable {
    ...
    else
      {
        ...
        for (uint256 i=0; i<15; i++) {
            ...
            if(add != address(0)){
                ...
                player.transfer(Quantity.mul(BL[i]).div(100));        
            }
        } 
    ...
    function WithdrawalOperatorFTE(address Addrs,uint256 Quantity) public onlyOperator {
        require(FTE.balanceOf(address(this)) >= Quantity, "404");
        FTE.transfer(Addrs, Quantity);
    }        
    function Withdrawal(uint256 Quantity,uint256 tokenType) public  {
    }
    function UpgradeBroker(uint256 Quantity) public   {
    }  
    function BlindBoxOpen() public   {
    }  
    function pledge(uint256 NFTID,uint256 Amountofenergy,uint256 tokenType,uint256 Type1,uint256 Type2) public{
    }
    ...
    function NFTBuy(uint256 tokenType,uint256 Quantity) public   {
    }
    function RenewTheContract(uint256 NFTID,uint256 tokenType) public   {
    }
