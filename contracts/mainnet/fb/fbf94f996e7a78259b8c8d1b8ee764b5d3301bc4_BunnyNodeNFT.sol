...
contract BunnyNodeNFT is ERC721, ERC721Enumerable, AccessRole {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIdCounter;

    string  public baseURI          = "https://static.bunnyswap.app/nft/node/";
    uint256 public maxSupply        = 1000;
    uint256 public nodeUsdtPrice    = 3000 * 10 ** 18;
    address public usdtAddress      = 0x55d398326f99059fF775485246999027B3197955;
    address public marketAddress    = 0x170f56a6f54AEf01335B5BDD2354B19c7985785B;

    event PurchaseNFT(address to, uint256 tokenId, uint256 price);
    event TokenBaseURIUpdated(string uri);

    constructor() ERC721("BunnyNodeToken", "BNT") {
        _tokenIdCounter.increment(); // 1
    }

    function safeMint(address to) public {
        // pay
        IERC20(usdtAddress).transferFrom(msg.sender, marketAddress, nodeUsdtPrice);

        // mint
        uint256 tokenId = _tokenIdCounter.current();
        require(tokenId <= maxSupply, "MINT_LIMIT");
        _tokenIdCounter.increment();
        _safeMint(to, tokenId);

        emit PurchaseNFT(to, tokenId, nodeUsdtPrice);
    }

    function safeBatchMint(address to, uint256 quantity) public fullAccessRight {
    ...
    function burn(address to, uint256 tokenId) public fullAccessRight {
        require(_ownerOf(tokenId) == to, "error owner");
        _burn(tokenId);
    }
    ...
    function isNftExist(uint256 tokenId) public view returns (bool) {
