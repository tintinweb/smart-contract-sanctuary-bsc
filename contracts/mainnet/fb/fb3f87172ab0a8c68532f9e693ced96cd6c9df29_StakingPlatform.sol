...
interface IStakingPlatform {
     ...
     * @dev `block.timestamp` higher than `_userStartTime[_msgSender()] + lockupDuration`
     ...
     * @dev `block.timestamp` must be higher than `_userStartTime[_msgSender()] + lockupDuration`
...
contract StakingPlatform is IStakingPlatform, Ownable {
    ...
    /**
     * @notice function that allows a user to withdraw its initial deposit
     * @dev must be called only when `block.timestamp` >= `_userStartTime[_msgSender()] + lockupDuration`
     * @dev `block.timestamp` higher than `_userStartTime[_msgSender()] + lockupDuration`
     * withdraw reset all states variable for the `msg.sender` to 0, and claim rewards
     * if rewards to claim
     */
