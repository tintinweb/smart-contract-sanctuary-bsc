...
contract SquidFeeBatchV3 is Initializable, OwnableUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    IERC20Upgradeable public wNative;
    IERC20Upgradeable public sqdi;
    ...
    address[] public wNativeToSqdiRoute;
    address[] public wNativeToStableRoute;

    bool public splitTreasury;
    bool public sendHarvesterGas;

    event Harvest(uint256 totalHarvested, uint256 stablesHarvested, uint256 sqdiHarvested, uint256 nativeNotified, uint256 time);
    ...
    event NewSqdiRoute(address[] oldRoute, address[] newRoute);
    ...
    function initialize(
        ...
        wNativeToSqdiRoute = _sqdiRoute;
        wNativeToStableRoute = _stableRoute;
    }

    // Main function. Divides Squid's profits.
    function harvest() public {
        ...
        if (splitTreasury) {
            ...
            sqdiAmt = nativeAvailable - stableAmt;
            IUniswapRouter(unirouter).swapExactTokensForTokens(stableAmt, 0, wNativeToStableRoute, treasury, block.timestamp);
            IUniswapRouter(unirouter).swapExactTokensForTokens(sqdiAmt, 0, wNativeToSqdiRoute, treasury, block.timestamp);
        } else {
        ...
        emit Harvest(wNativeBal, stableAmt, sqdiAmt, rewardPoolAmount, block.timestamp);
    }
    ...
    function setRoute(address[] calldata _route, bool _stableRoute) external onlyOwner {
        ...
        if(_stableRoute) {
            ...
            require(_route[_route.length - 1] == address(sqdi), "!sqdi");

            emit NewSqdiRoute(wNativeToSqdiRoute, _route);
            wNativeToSqdiRoute = _route;
        }
    }
