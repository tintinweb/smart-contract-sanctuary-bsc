...
//@title TransferProxy is used for communicating with Camly721, tokens(WETH, WBNB, WMATIC,...).
...
contract CamlyProxy is AccessControl, ICamlyMarketProxy {
    ...
    //@param token, Camly721 address @dev see {IERC721}.
    ...
    //@require - caller must be an Operator Address
    ...
    //@param from, address of user,who's sending the tokens.
    //@param to, address of user,who's receiving the tokens.
...
interface ICamlyMarketProxy {
    ...
    //@notice the function transfers Camly721 NFTs to users.
    //@param token, Camly721 address @dev see {IERC721A}.
    //@param from, seller address, NFTs to be transferrred from this address.
    //@param to, buyer address, NFTs to be received to this address.
    //@param tokenId, unique NFT id to be transfer.
