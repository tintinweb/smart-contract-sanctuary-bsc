...
// SPDX-License-Identifier: Unlicensed
    // Bambo Finance Start From Here
  // Dual Elements - BAM Powered - Dual Liquidity
// Let's Get Onboard °°°°°°°°°°°°°°°°°°°°°
...
contract BAM is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    uint256 private _tTotalDistributedToken;
      // Dual Bambo Finance - Together - We Build The Future
    // Imagination Into Reality
  // Ignition Into Metaverse - Explore The Universe 
// Cold Cranking - Hot Start - Hold On Tight
    string private _name = "Bambo Finance";
    ...
    address public dev_future = payable(0xff6E5AFD0f3B13DC68b3cb9B08f24921c198a420);
    address public dual_Liquidity = payable(0x6B134741057C1f22bfaf8077932F6C3a0Dc4411D);
    ...
    constructor () {
        ...
        _excludedFromFee[dev_future] = true;
        _excludedFromFee[dual_Liquidity] = true;
    ...
    function setdev_future(address newDev) external onlyOwner {
        require(newDev != address(0), "newDev can not be a zero address");
        dev_future = payable(newDev);

    }
    
    function setDual_Liq(address newDual_Liq) external onlyOwner {
        require(newDual_Liq != address(0), "Dual_Liq can not be a zero address");
        dual_Liquidity = payable(newDual_Liq);

    }
    
    function setBuyFeePercent(uint256 reflect, uint256 dev, uint256 liquid) public onlyOwner {
        require(reflect.add(dev).add(liquid) <= 15, "Fee limitation is 15% anti rugged, you're in good hand");
        _bReward = reflect;
        _bDevfee = dev;
        _bRfuel = liquid;
    }
    ...
    function setSellFeePercent(uint256 reflect, uint256 dev, uint256 liquid) public onlyOwner {
        require(reflect.add(dev).add(liquid) <= 20, "Fee limitation is 20% anti rugged, you're in good hand");
        _sReward = reflect;
        _sDevfee = dev;
        _sRfuel = liquid;
    }
    ...
    function _takeLiquidNDevFee(uint256 tLiquidNDevFee,address sender) private {
        ...
        _sendFee(sender, dev_future, tDev);
