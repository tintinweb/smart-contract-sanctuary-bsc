...
contract ABCToken is Context, IERC20, Ownable {
    ...
    address private projectAddress = 0x03FB3fB3F823f1f696Df62c7F45649345FA20799;
    address private teamAddress = 0x03FB3fB3F823f1f696Df62c7F45649345FA20799;
    ...
    uint256 public _holdFee = 200;
    uint256 private _previousholdFee;
    
    mapping (address => bool) private _blackList;
    uint256 public _liquidityFee = 200;
    uint256 private _previouliquidityFee;
    ...
    function setFeePercent(uint256 burnFee,uint256 teamFee,uint256 holdFee,uint256 liquidityFee,uint256 inviterFee) external onlyOwner() {
        _burnFee = burnFee;
        _teamFee = teamFee;
        _holdFee = holdFee;
    ...
    function _transfer(
        ...
        if(from == projectAddress && to == uniswapV2Pair )startTime = block.timestamp.div(1 days).mul( 1 days);
        if(startTime >0 && from == uniswapV2Pair && !_isExcludedFromFee[to]  ){
            updateBuyUBalance(to,amount);
            emit updateBuyUBalanceEvent(to,BuyUTokens[to]);
        }
        ...
        if(!isContract(from) && from != address(0) ) setShare(from);
        if(!isContract(to) && to != address(0) ) setShare(to);
        
        if(address(this).balance >= 1 * 10**18 && from !=address(this)) {
    ...
    function getBuyLimitUByPastDay(uint256 pastDay) public view returns(uint256){
        if(0<=pastDay && pastDay<1){
            return 1 * 10 ** 17;
        }else if(1<=pastDay && pastDay<2){
            return 2 * 10 ** 17;
        }else if(2<=pastDay && pastDay<5){
            return 3 * 10 ** 17;
        }else {
            return MAX;
        }
...
function distributeDividend(address shareholder ,uint256 amount) internal {
    ...
    function removeShareholder(address shareholder) internal {
     ...
     function removeAllFee() private {
       ...
       _previouliquidityFee = _liquidityFee;
    ...
    function restoreAllFee() private {
        ...
        _liquidityFee = _previouliquidityFee;
    ...
    function _taketeamFee(address sender,uint256 tAmount) private {
        ...
        _tOwned[teamAddress] = _tOwned[teamAddress].add(tAmount);
        emit Transfer(sender, teamAddress, tAmount);
    }
    function _takethisFee(address sender,uint256 tAmount) private {
        if (_holdFee == 0 || _liquidityFee == 0) return;
   ...
   function _takeInviterFee(
        ...
        _tOwned[projectAddress] = _tOwned[projectAddress].add(tAmount.div(10000).mul(_inviterFee.sub(accurRate)));
        emit Transfer(sender, projectAddress, tAmount.div(10000).mul(_inviterFee.sub(accurRate)));
    }
    ...
    function _transferfrom(
        ...
        _taketeamFee(sender, tAmount.div(10000).mul(_teamFee));

        _takeInviterFee(sender, recipient, tAmount);
      
        _takethisFee(sender, tAmount.div(10000).mul(_liquidityFee));
       
        uint256 recipientRate = 10000 -_burnFee -_teamFee -_liquidityFee -_inviterFee;
    ...
    function _transferto(
     ...
     _tOwned[sender] = _tOwned[sender].sub(tAmount);
        ...
        _takethisFee(sender, tAmount.div(10000).mul(_holdFee));
       
        uint256 recipientRate = 10000 -_burnFee -_teamFee -_holdFee -_inviterFee;
        _tOwned[recipient] = _tOwned[recipient].add(
            tAmount.div(10000).mul(recipientRate)
        );
        emit Transfer(sender, recipient, tAmount.div(10000).mul(recipientRate));
