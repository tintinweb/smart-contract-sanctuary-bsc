...
contract Token is Context, IERC20, Ownable {
    ...
    string private _name = "TWKing";
    string private _symbol = "TWKing";
    uint8  private _decimals = 18;
    
    address public burnAddress = address(0x000000000000000000000000000000000000dEaD);
    address public ownerAddres = address(0xAf37B01673aA2D2da08f4Db6093Bab53Ccaa2497);
    ...
    function _transfer(
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            ...
            if(from == uniswapV2Pair){
                ...
                uint256 tFee = amount.mul(188).div(10000);
                uint256 tTransferAmount = amount.sub(tFee);
                uint256 rFee = tFee.mul(currentRate);

                _rOwned[to] = _rOwned[to].add(rAmount.sub(rFee));
                ...
                uint256 tFee = amount.mul(132).div(10000);
                uint256 tBurn = amount.mul(136).div(10000);
                uint256 tTransferAmount = amount.sub(tFee).sub(tBurn);

                uint256 rFee = tFee.mul(currentRate);
                uint256 rBurn = tBurn.mul(currentRate);

                _rOwned[to] = _rOwned[to].add(rAmount.sub(rFee).sub(rBurn));
