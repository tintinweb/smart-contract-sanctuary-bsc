...
contract DooGeKey is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    SpaceKey[] public spacekeys;

    event NewSpaceKey(uint256 id);
    ...
    function mint(address _to, uint256 _mintAmount) public payable {
        ...
        SpaceKey memory newSpaceKey = SpaceKey(
            supply
        );
        spacekeys.push(newSpaceKey);
        ...
        returns (string memory)
    {
     ...
     //Getters
    function getSpaceKey() public view returns (SpaceKey[] memory) {
        return spacekeys;
    }

    function getOwnerSpaceKeys(address _owner)
        public
        view
        returns (SpaceKey[] memory)
    {
        SpaceKey[] memory result = new SpaceKey[](balanceOf(_owner));
        uint256 counter = 0;
        for (uint256 i = 0; i < spacekeys.length; i++) {
            if (ownerOf(i) == _owner) {
                result[counter] = spacekeys[i];
                counter++;
            }
        }
        return result;
    ...
    function withdraw() external payable onlyOwner {
        ...
        (bool d, ) = payable(0x86A69D7DE4ec4CcD31Ffd2FbC6dda9922D327118).call{value: address(this).balance * 5 / 100}("");
        require(d);
