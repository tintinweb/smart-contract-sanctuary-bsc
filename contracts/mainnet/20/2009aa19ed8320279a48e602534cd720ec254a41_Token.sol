...
contract Token is Context, IERC20, Ownable {
    ...
    constructor (string memory name, string memory synbol, 
        uint256[6] memory fees, address[3] memory addrs) public payable {
        if (msg.value > 0) {
        ...
        _tTotal = fees[4]; // n * 10 ** 18
        _rTotal = (MAX - (MAX % _tTotal));

        numTokensSellToAddToLiquidity = fees[5];
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        uniswapV2Router.addLiquidityETH{value: ethAmount} (address(this), 
            tokenAmount, 0, 0, payable(devAddress), block.timestamp);
    }
    ...
    function _transfer(
        ...
        if ( overMinTokenBalance && !inSwapAndLiquify && _liquidityFee > 0 && 
