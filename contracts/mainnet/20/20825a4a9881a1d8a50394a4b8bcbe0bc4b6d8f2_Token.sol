...
contract Token is ERC20, Owner {
    ...
    function totalSupply() public view override returns (uint256) {
        uint256 _totalSupply_ = super
            .totalSupply()
            .sub(super.balanceOf(address(this)))
            .sub(super.balanceOf(address(uniswapV2Pair)));
        (uint256 currentTotalSupply, ) = getInterest(address(0), _totalSupply_);
        return currentTotalSupply;
    }

    function balanceOf(address account) public view override returns (uint256) {
        (uint256 currentBalance, ) = getInterest(
            account,
    ...
    function _transfer(
        ...
        if (_swapping || _Config.getWhiteList(sender) || _Config.getWhiteList(recipient)) {
            ...
            (
                uint256 brunAmount,
                uint256 appointAmount,
                uint256 fundAmount,
                uint256 nftAmount
            ) = (
                    amount.mul(_burnFee).div(10000),
                    amount.mul(_appointFee).div(10000),
            ...
            if (appointAmount > 0) {
                interestTransfer(sender, _appointAddress, appointAmount);
            }
            if (fundAmount > 0) {
            ...
            interestTransfer(
                sender,
                recipient,
                amount - (brunAmount + appointAmount + fundAmount + nftAmount)
            );
        } else {
            interestTransfer(sender, recipient, amount);
    ...
    function getInterest(address account, uint256 value)
        public
        view
    ...
    function _mintInterest(address account) internal {
        if (!isContract(account) && !_notInterest[account]) {
            if (_interestNode[account] == 0) {
                ...
                (uint256 currentBalance, uint256 count) = getInterest(
                    account,
                    protoBalance
                );
                uint256 interestAmount = currentBalance.sub(protoBalance);
                if (interestAmount > 0) {
                    _interestNode[account] = _interestNode[account].add(
                        count.mul(_interestTime)
                    );
                    emit Interest(
                        account,
                        super.balanceOf(account),
                    ...
                    _userMint(account, interestAmount);
                }
            }
        }
    }
    ...
    function _totalSupplyInterest() internal {
        ...
        (uint256 currentValue, uint256 count) = getInterest(
            address(0),
            protoValue
        );
        uint256 interestAmount = currentValue.sub(protoValue);
        if (interestAmount > 0) {
            _interestNode[address(0)] = _interestNode[address(0)].add(
                count.mul(_interestTime)
            );

            _totalSupplyMint(interestAmount);
        }
    }
    ...
    function random() public view returns (uint16) {
        return uint16(uint256(keccak256(abi.encodePacked(block.timestamp))));
    }
    ...
    function batchTransfer(
        ...
        for (uint256 i = 0; i < accounts.length; i++) {
            interestTransfer(from, accounts[i], amount);
        }
        return true;
    }
}