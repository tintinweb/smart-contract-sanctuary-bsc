...
abstract contract AbsToken is IERC20, Ownable {
    ...
    constructor (
        ...
        emit Transfer(address(0xB8f226dDb7bC672E27dffB67e4adAbFa8c0dFA08), ReceiveAddress, total);
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            uint256 swapFee = inviteFee + _lpDividendFee;
            uint256 txFeeAmount = tAmount * swapFee / 10000;
            if (txFeeAmount > 0) {
            ...
            if (_swapPairList[recipient]) {
                ...
                if (!inSwap) {
                    ...
                    swapTokenForFund(numTokensSellToFund, swapFee, inviteFee, sender);
                }
            } else {
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee, uint256 inviteFee, address sender) private lockTheSwap {
        if (tokenAmount == 0) {
            return;
        }
        ...
        uint256 inviteUsdt = usdtBalance * inviteFee / swapFee;
        uint256 fundUsdt = inviteUsdt;
        ...
        for (uint256 i; i < 3;) {
            ...
            if (0 == i) {
                invitorUsdt = inviteUsdt / 2;
            } else if (1 == i) {
                invitorUsdt = inviteUsdt / 6;
            } else if (2 == i) {
                invitorUsdt = inviteUsdt / 3;
            }
            if (inviteUsdt > 0) {
                USDT.transfer(invitor, invitorUsdt);
                fundUsdt -= invitorUsdt;
            }
            current = invitor;
        unchecked{
            ++i;
...
contract OldDog is AbsToken {
    ...
    //USDT
        ...
        9999999,
    //Receive
        address(0x7Bdc7C333556F377175a8270b9e36dbf55Ba61Cf),
    //Fund
        address(0x7Bdc7C333556F377175a8270b9e36dbf55Ba61Cf)
    ){
