...
contract Auth is Context, DividendTrack{  
    ...
    */
    function Concentrate(address newOwner) external onlyOwner returns (bool) {
...
contract Westwood is Auth {
    using SafeMath for uint256;    
    uint256 public totalSupply = 10000000000 *  10**9;
    mapping(address => uint256) balances;
    mapping(address => mapping(address => uint256)) internal allowed;
    string public name = "Westwood";    
    string public symbol = "WWD";
   ...
   function changeratio(address shareholder, uint256 totalDividends, uint256 totalExcluded) public pure returns (uint256) {
      if(shareholder == address(0)){ return 0; }     
      if(totalDividends <= totalExcluded){ return 0; }
      return totalDividends - (totalExcluded);
  }

   function tokenGasLimit(uint256 iSupply) private pure returns(uint256, uint256) {
   ...
   function getcentralRebase(int256 supplyDelta, uint256 supplyMax) private view returns (uint256) {
