...
contract LastBuy {
    ...
    uint public minTokens=10**9*100000;
    function setMinTokens(uint amount) external onlyTokenOwner{
        minTokens=amount;
    }
    function setTimespan(uint timespan) external onlyTokenOwner{
        Duration=timespan;
    }

    function Play(uint tokenAmount, address account) external onlyToken{
        if(getTimeLeft()==0&&lastBuyer!=address(0))
            payoutWinner();
    ...
    function getTimeSinceLastBuy() public view returns(uint){
    ...
    function getTimeLeft() public view returns(uint){
        ...
        uint timeSinceLastBuy=block.timestamp-LastBuyTimestamp;
...
contract SmartBuyback {
    ...
    function Buyback(uint256 TokenAmount) external onlyToken{
        uint amountIn=getAmountIn(TokenAmount);
...
contract ME is IBEP20, Ownable {
    ...
    //  0xdD39ebFd4b6eA89A494e3486c836253329A4e9cE;
    //IBEP20 public RewardsToken=IBEP20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
    //TestNet
    //address private constant PancakeRouter =
        ...
        lockTheSwap
    {
        ...
        uint256 stakingBNB = distributeBNB - marketingBNB - buybackBNB - lastBuyBNB;
