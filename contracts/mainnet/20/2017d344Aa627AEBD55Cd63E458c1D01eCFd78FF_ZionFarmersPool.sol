...
contract ZionFarmersPool is Ownable, Pausable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 public rewardRate = uint256(0.00015 ether);
    ...
    constructor(IERC20 _rewardsToken, IDigitalaxNFT _parentNFT) {
    ...
    function stakeAll() external {
        ...
        for (uint256 i = 0; i < balance; i++) {
            ...
            if (allowedTokens[_tokenId]) _tokenIds[i] = _tokenId;
        }
    ...
    function stake(uint256 _tokenId) external {
        ...
        require(allowedTokens[_tokenId], "error: not allowed NFT!");
        _stake(_tokenId);
    }

    function _stake(uint256 _tokenId)
        internal
        nonReentrant
        whenNotPaused
        updateReward(false)
    ...
    function claimable(address _user) external view returns (uint256) {
        ...
        return
            amount.mul(curAccPerShare).div(1e12).sub(user.rewardDebt).add(
                user.pendingRewards
            );
    }
