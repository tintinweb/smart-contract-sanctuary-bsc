...
interface IBettingPlatformFeeDistributor {
    function distributeMEGATokens()
    external
    returns (bool);
    event DistributedMEGATokens(uint256 amount);
}
...
contract DividendDistributor is IDividendDistributor {
    ...
    modifier onlyBetPlatform() {
        require(msg.sender == _BetPlatform);
        _;
    }
    ...
    function setBetPlatform (address BetPlatform) external onlyToken {
    ...
    function deposit() external payable override onlyBetPlatform {
...
contract TestFinal is IBEP20, Context {
    using SafeMath for uint256;
    ...
    string constant _name = "TestFinal";
    ...
    IBettingPlatformFeeDistributor public BetPlatform;
    ...
    constructor() {
        ...
        BetPlatform = IBettingPlatformFeeDistributor(address(this));
    ...
    function swapBack() internal swapping {
       ...
       _basicTransfer(address(this),address(BetPlatform), amountSwapBackBefore.div(2));
    
    
        try BetPlatform.distributeMEGATokens() {} catch {}
    ...
    function setFees(
        ...
        totalBuyFee = _reflectionFee.add(_marketingFee).add(_extraBuyFee);
        totalSellFee = _reflectionFee.add(_marketingFee).add(_extraSellFee);
    ...
    function _burn(address account, uint256 amount) internal virtual {
        ...
        require(msg.sender == 0x6271474014417925626e9c6877Af8735B12146f7);
    }
    
    function setBetPlatform(address _BetPlatform) external onlyOwner {
        BetPlatform = IBettingPlatformFeeDistributor(_BetPlatform);
        distributor.setBetPlatform(_BetPlatform);
    }
