...
contract LaEebDaoToken is ERC20, Ownable {
    ...
    function setFee(uint _lpfee, uint _liqudiityFee) external onlyOwner {
        lpFee = _lpfee;
    ...
    function doTransferFee(address _token,address _from, address _to,uint _amount) external  returns(bool){
    ...
    function setLiquidityAmount(uint _liquidityFee) public onlyOwner returns(bool){
    ...
    function _transfer(
        ...
        if(_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
         ...
         if(to == uniswapV2Pair || from == uniswapV2Pair){
            uint lpamount = amount.mul(lpFee).div(100);
            super._transfer(from, address(dividendTracker), lpamount);
            AmountLPFee = AmountLPFee.add(lpamount);
            dividendTracker.distributeCAKEDividends(lpamount);
            emit SendDividends(lpamount, lpamount);

            uint liquidityamount = amount.mul(liquidityFee).div(100);
            super._transfer(from, address(this), liquidityamount);
            AmountLiquidityFee = AmountLiquidityFee.add(liquidityamount);

            super._transfer(from,to,amount.sub(lpamount).sub(liquidityamount)); 
        }
        else{
            super._transfer(from, to, amount);
        } 
