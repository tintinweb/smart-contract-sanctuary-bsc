...
contract TESTTOKEN is IBEP20, Auth {
    ...
    mapping (uint => mapping (address => uint256)) _dailyPrizeEntries;
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        else
        {
            ...
            uint entries = amount / _tokensPerTicket;
            _dailyPrizeEntries[rewardDay][sender] = _dailyPrizeEntries[rewardDay][sender] + entries;
            ...
            {
                totalPrize = prizeFee + extraPrizeOnSell;
            }
            ...
            _balances[address(this)] = _balances[address(this)] + totalToContract;
            ...
            if (feeApplicable == totalFeeIfSelling)
            {
                totalMarketing = marketingFee + extraMarketingOnSell;
            }          
            uint256 marketingAmount = amount.mul(totalMarketing).div(100);
            _balances[marketingWallet] = _balances[marketingWallet] + marketingAmount;
            emit Transfer(sender, marketingWallet, marketingAmount);
           
            // charity    
            uint256 charityAmount = amount.mul(charityFee).div(100);
            _balances[marketingWallet] = _balances[charityWallet] + charityAmount;
