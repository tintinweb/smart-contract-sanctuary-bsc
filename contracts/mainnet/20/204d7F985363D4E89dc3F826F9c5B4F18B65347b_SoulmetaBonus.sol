...
contract SoulmetaBonus is DivestorUpgradeable {
    ...
    bool public isOpen;
    // 历史总数据
    // DayTokenTotal public totalInput;
    // 处理历史数据
    // bool public isTransferData;
    ...
    function initialize(address usdtAddr_, address soul1155_, address soulToken_, address lpAddr_, address soulController_) public initializer {
        __Divestor_init();
        addr = AddrCollection({
    ...
    function setMeta(address usdtAddr_, address soul1155_, address soulToken_, address lpAddr_, address soulController_, bool isOpen_) public onlyOwner {
        addr = AddrCollection({
        usdtAddr : IERC20Upgradeable(usdtAddr_),
        soul1155Addr : ISoul1155(soul1155_),
        soulTokenAddr : ISoulToken(soulToken_),
    ...
    function _getSameDay() private returns (uint) {
        ...
        if (lastSettleDay != tempQuotient) {
            if (tempQuotient < day) {
                ...
                } else if (lastSettleDay < day) {// tempQuotient >= day
                    for (uint i = from; i <= to; i++) {
                        totalDayToken[day].soul += totalDayToken[i].soul;
                        totalDayToken[day].nft += totalDayToken[i].nft;
                    }
                    totalDayToken[day].lp += addr.lpAddr.balanceOf(address(this));
...
//    }
    ...
    function inputNftApi(uint8 type_, uint[] memory tokenIds) public {
        ...
        // 历史总量
        // totalInput.nft += tokenIds.length;
    ...
    function receiveTokenApi() public {
        ...
        UserInfo storage userInfo = userInfos[_msgSender()];
        // uint lp = userInfo.lp - (dayToDayToken[sameDay][_msgSender()].lp - dayToDayToken[sameDay-1][_msgSender()].lp);
    ...
    function searchUserBonusApi(address player_) view public returns (UserInfo memory, DayTokenTotal memory, uint, uint) {
        ...
        uint count = tempQuotient - lastSettleDay_ + 1; // 数组大小
        ...
        if (userInfo.firstDay == 0) {
            ...
            // 栈太深
            (total, one) = _getSettleData(player_, lastSettleDay_, count);

            if (lastSettleDay != tempQuotient) {
                if (tempQuotient >= day) {
                    if (lastSettleDay >= day) {
                        for (uint i = lastSettleDay + 1 - lastSettleDay_; i <= tempQuotient - lastSettleDay_; i++) {
                            total[i].soul += total[i - 1].soul;
                            total[i].nft += total[i - 1].nft;
                            total[i].lp += total[i - 1].lp;
                        }
                    } else if (lastSettleDay < day && from >= lastSettleDay_ && to >= lastSettleDay_) {
                        for (uint i = from - lastSettleDay_; i <= to - lastSettleDay_; i++) {
                            total[day].soul += total[i].soul;
                            total[day].nft += total[i].nft;
                        }
                        total[day].lp += addr.lpAddr.balanceOf(address(this));
                        if (tempQuotient > day) {
                            for (uint i = day + 1 - lastSettleDay_; i <= tempQuotient - lastSettleDay_; i++) {
                                total[i].soul += total[i - 1].soul;
                                total[i].nft += total[i - 1].nft;
                                total[i].lp += total[i - 1].lp;
                            }
            ...
            if (tempQuotient != lastSettleDay_) {
                if (tempQuotient >= day) {
                    if (lastSettleDay_ >= day) {
                        for (uint i = 1; i <= tempQuotient - lastSettleDay_; i++) {
                            one[i].soul += one[i - 1].soul;
                            one[i].nft += one[i - 1].nft;
                            one[i].lp += one[i - 1].lp;
                        }
                    } else if (lastSettleDay_ < day && from >= lastSettleDay_ && to >= lastSettleDay_) {
                        for (uint i = from - lastSettleDay_; i <= to - lastSettleDay_; i++) {
                            one[day].soul += one[i].soul;
                            one[day].nft += one[i].nft;
                        }
                        one[day].lp += userInfo.lp;
                        if (tempQuotient > day) {
                            for (uint i = day + 1 - lastSettleDay_; i <= tempQuotient - lastSettleDay_; i++) {
                                one[i].soul += one[i - 1].soul;
                                one[i].nft += one[i - 1].nft;
        ...
        UserInfo memory nowUserInfo = userInfos[player_];
        DayTokenTotal memory pastDay1 = DayTokenTotal(0, 0, 0);
        if (tempQuotient > lastSettleDay_ + 1) {
            pastDay1 = one[tempQuotient - 1 - lastSettleDay_];
        }

        uint lp = nowUserInfo.lp;

        UserInfo memory returnUserinfo = UserInfo(
            pastDay1.soul,
            pastDay1.nft,
            pastDay1.lp,
            nowUserInfo.accumulate - nowUserInfo.cumulative + notGet,
            nowUserInfo.cumulative,
            0,
            0
        );

        pastDay1 = DayTokenTotal(0, 0, 0);
        if (tempQuotient > lastSettleDay_ + 1) {
            pastDay1 = total[tempQuotient - 1 - lastSettleDay_];
        }

        DayTokenTotal memory returnTotal = DayTokenTotal(
            pastDay1.soul,
            pastDay1.nft,
            pastDay1.lp
        );

        return (returnUserinfo, returnTotal, bonusPool[tempQuotient - 1], lp);
    }

    // 处理结算memory数据
    function _getSettleData(address player_, uint lastSettleDay_, uint count) view private returns(DayTokenTotal[] memory, DayTokenTotal[] memory) {
        DayTokenTotal[] memory total = new DayTokenTotal[](count);
        DayTokenTotal[] memory one = new DayTokenTotal[](count);
