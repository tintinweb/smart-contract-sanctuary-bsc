...
contract LFWUtils {
    ...
    function getPending(address _scAddress, address _userAddress) public view returns(uint256 myPending) {
        myPending = IPancakeInterface(_scAddress).pendingReward(_userAddress);
    
    }

    function getBlockCD(address _scAddress) public view returns(uint256 myCD) {
        myCD = IPancakeInterface(_scAddress).bonusEndBlock() - IPancakeInterface(_scAddress).lastRewardBlock();
    }
}