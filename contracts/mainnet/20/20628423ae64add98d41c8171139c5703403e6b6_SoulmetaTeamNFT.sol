...
contract SoulmetaTeamNFT is DivestorUpgradeable, ERC721EnumerableUpgradeable{
    ...
    function newTeamBatch(address[] calldata leaders_, bool toMe_) public onlyOwner returns (bool) {
        for (uint i = 0; i < leaders_.length; i++) {
            newTeam(leaders_[i], toMe_);
        }
        return true;
    }

    function newTeam(address leader_, bool toMe_) public onlyOwner returns (bool) {
