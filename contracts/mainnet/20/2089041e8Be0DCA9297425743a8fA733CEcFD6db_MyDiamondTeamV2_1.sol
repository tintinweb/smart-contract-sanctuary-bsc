...
contract MyDiamondTeamV2_1 {
    ...
    function _deposit(address _addr, uint256 _amount) private {
        ...
        if(users[_addr].upline != address(0)) {
			...
			if(users[users[_addr].upline].checkpoint > 0 && users[users[_addr].upline].deposit_amount.add(refBonus) < this.maxReinvestOf(users[users[_addr].upline].total_direct_deposits)) {
...
/*
    ...
    function SET_MAX_REINVEST_MULTIPLIER(uint256 value) external {
        require(msg.sender == owner, "Admin use only");
        MAX_REINVEST_MULTIPLIER = value;
    }

    /*function UpgradeUsersForSustainabilityUpgrade(address _addr, address _addr2, address _addr3) external {
        require(msg.sender == owner, "Admin use only");
        ...
        for(uint256 i = 0; i < members.length; i++){
            ...
            address userAddr = members[i];
            if(userAddr == address(0)){
                continue;
            }
            uint256 real_deposits = users[userAddr].total_direct_deposits;
            uint256 acc_deposits = users[userAddr].deposit_amount;

            if(acc_deposits > (real_deposits * 5)){
                // remember the old value
                usersRealDepositsBeforeMigration[userAddr] = users[userAddr].total_direct_deposits;

                // new real deposits is accumulated deposits divided by 4.5
                users[userAddr].total_direct_deposits = acc_deposits.mul(10).div(45); 
            } 
        }
    }*/
}
