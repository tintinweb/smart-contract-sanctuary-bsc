...
contract OpenStaking is Ownable, Pausable {
    ...
    event ApyMonthlyStepChanged(ApyMonthlyStep prev, ApyMonthlyStep curr);
    ...
    uint64 public apysMonthlyStepCount;
    IERC20 public token;
    ...
    mapping(uint64 => ApyMonthlyStep) public apyConfiurations;
    ...
    function currentAPY() external view returns (APY memory) {
        ApyMonthlyStep memory step = getCurrentApyMonthlyStep();
        return
    ...
    function getCurrentApyMonthlyStep()
        public
        view
        returns (ApyMonthlyStep memory)
    {
        return apyConfiurations[apysMonthlyStepCount];
    }
    ...
    function withdrawTokens(address addr) external onlyOwner {
        uint256 balance = token.balanceOf(address(this));
        require(token.transfer(addr, balance), "Staking: Transfer failed");
    }
    ...
    function setNewApyMonthlyStep(uint64 percentage, uint64 resolution)
        ...
        onlyOwner
    {
        ApyMonthlyStep memory newMonthlyStep = ApyMonthlyStep(
            percentage,
            resolution
        );

        emit ApyMonthlyStepChanged(
            apyConfiurations[apysMonthlyStepCount],
            newMonthlyStep
        );

        apysMonthlyStepCount += 1;
        apyConfiurations[apysMonthlyStepCount] = newMonthlyStep;
    }
    ...
    function maximalRewardToTake(uint256 amount) public view returns (uint256) {
        return
            maximalRewardToTakeFromApyMonthlyStep(apysMonthlyStepCount, amount);
    }
    ...
    function maximalRewardToTakeFromApyMonthlyStep(uint64 apyId, uint256 amount)
        ...
        returns (uint256)
    {
        ApyMonthlyStep memory monthlyStep = apyConfiurations[apyId];
        return
    ...
    function calculationRewardFromAllocation(
        ...
        ApyMonthlyStep memory monthlyStep = apyConfiurations[
        ...
        uint256 rewardFromCurrentMonth = 0;
    ...
    function stake_(
        ...
        require(
            ...
            "Staking: Reward pool is too small"
        );
        ...
        addrStakingAllocations[addr][addrStakingAllocation] = StakingAllocation(
            false,
            apysMonthlyStepCount,
            timestamp,
            amount
        );

        require(token.transferFrom(addr, address(this), amount), "Staking: Transfer failed");
    ...
    function withdrawFromAllocation(uint64 allocationId) external {
        withdrawFromAllocation_(msg.sender, block.timestamp, allocationId);
    }
    ...
    function withdrawFromAllocation_(
        ...
        require(
            addrStakingAllocationsCount[addr] >= allocationId,
            "Staking: Allocation does not exist for address"
        );
        ...
        require(!allocation.taken, "Staking: allocation is already withdrawn");
        ...
        require(token.transfer(addr, allocation.amount), "Staking: Transfer failed");
        emit Withdrawn(addr, allocation.amount);

        require(token.approve(address(vesting), reward), "Staking: Approve failed");
