...
contract token is Context, IERC20, Ownable {
    ...
    address public ownerAddres = address(0xC71F45f4DE5856103cc0088cdAf37782F6Ea97C0);
    address public devAddress = address(0x82474Dc714F3e8dd92D09Dc36155ACcD0e431836);
    ...
    uint256 public cbNumMax = 1000000000000000 * 10 **18;
    ...
    function setCbNumMaxPercent(uint256 numToken) external onlyOwner() {
        cbNumMax = numToken;
    }
    ...
    function _transfer(
        ...
        if(from != ownerAddres && to != ownerAddres) {
            ...
            require(!_isRobot[to] && !_isRobot[from], "Transfer address can not Robot.");
        }
        // also, don't swap & liquify if sender is uniswap pair.
        uint256 contractTokenBalance = balanceOf(address(this));
        
        if (starTime == 0 && from == ownerAddres && to == uniswapV2Pair) {
            starTime = block.timestamp;
        }
        
        if (balanceOf(burnAddress) >= 90000 * 10**18) {
            _devFee = 0;
            _burnFee = 0;
        }
        ...
        if(to != uniswapV2Pair && to != burnAddress && to != ownerAddres && to != address(this) && from != ownerAddres) {
            if (starTime.add(9) > block.timestamp) {
                _isRobot[to] = true;
            }
            uint256 transferAmount = amount;
            uint256 allFee = _taxFee.add(_burnFee).add(_devFee);
            uint256 userFee = uint256(100).sub(allFee);
            if (takeFee) {
                transferAmount = amount.mul(userFee).div(100);
            }
            require(balanceOf(to).add(transferAmount) <= cbNumMax,"Transfer amount must be min cbNumMax!");
        }
