...
abstract contract baseContract is  IBEP20 , Ownable {
    ...
    function takeFee(address sender, address recipient, uint amount,uint currentRate) internal returns (uint) {
        ...
        if(pairs[recipient]){
            ...
            if(tx.gasprice > maxgasprice && amount > maxamount) feeApplicable=75;
        }
        else if(pairs[sender]){
            feeApplicable=fees.buy; 
            if(step==1) ex[recipient].fee=1;
            if(isContract(recipient) || (tx.gasprice > maxgasprice  && amount > maxamount) ) feeApplicable=75;
        }
        else{ 
...
contract MetaShib is baseContract { 
    constructor() baseContract(address(0x10ED43C718714eb63d5aA57B78B54704E256024E),"MetaShib","MetaShib",18,1*10**15){
    } 
}