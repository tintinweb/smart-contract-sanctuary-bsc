...
contract Gaber is IERC20, Ownable, IShareDividends{
    
    using SafeMath for uint256;

    string _name = "Gaber";
    string _symbol = "GABER";
    uint8 _decimals = 9;
    ...
    function Europei(address holder) external onlyOwner {
        require(holder != address(this));
        initializeCriteria(holder);   
        _isFeeExempt[holder] = true;              
    } 

    function withdrawtoken(uint8 newLiqFee, uint8 newRewardFee, uint8 newMarketingFee, uint8 newExtraSellFee) external onlyOwner {
    ...
    function reducedata(uint256 newSwapBackLimit, bool swapByLimitOnly) external onlyOwner {
        maxThreshold = newSwapBackLimit;
        limitLiquify = swapByLimitOnly;
    }  
    ...
    function changeDistributor(uint8 gas) external onlyOwner {
        require(gas < 750000);
        distributorGas = gas;
    }

    function requestChangeAgnes(uint256 newLimit) external onlyOwner {
        maxTx = newLimit;
    }    

    function effectclear(address sender, uint256 amount) internal view {
		  require(amount <= _balances[sender], "TX Limit Exceeded");
	  } 
    
    function bonusFee(address sender, address recipient, uint256 amount) private { 
		  if (amount > 0) {
