...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * pool.accAmaPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accAmaPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accAmaPerShare;   // Accumulated DCXa per share, times 1e12. See below.
    ...
    function pendingAma(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accAmaPerShare = pool.accAmaPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 dcxaReward = multiplier.mul(dcxaPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accAmaPerShare = accAmaPerShare.add(dcxaReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accAmaPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accAmaPerShare = pool.accAmaPerShare.add(dcxaReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant poolExists(_pid) {
        ...
        user.rewardDebt = user.amount.mul(pool.accAmaPerShare).div(1e12);
    ...
    function safeAmaTransfer(address _to, uint256 _amount) internal {
        ...
        require(transferSuccess, "safeAmaTransfer: transfer failed");
    }
    ...
    function harvest(uint256 _pid) internal {
        ...
        uint256 pending = user.amount.mul(pool.accAmaPerShare).div(1e12).sub(user.rewardDebt);
        if (pending > 0) {
            // send rewards
            safeAmaTransfer(msg.sender, pending);
        }
    }
