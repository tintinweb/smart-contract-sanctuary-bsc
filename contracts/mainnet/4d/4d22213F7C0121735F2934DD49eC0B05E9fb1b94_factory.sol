...
contract StakeToken is ERC20 {
   ...
   * @param _stakeEndDate Unix timestamp that the staking is finished.
  ...
  constructor(
    ...
    require(
      _stakeEndDate > block.timestamp,
      'staking end time must be after now'
    );

    uint256 _perBlockAmount = _rewardSupply.div(_stakeEndDate - block.timestamp);
...
contract factory {
  ...
  address public farmingContractNFT;
  address public farmingContractToken;

  bool public isCreatedNFTPool;
  bool public isCreatedTokenPool;
  ...
  constructor()
  {
      isCreatedNFTPool = false;
      isCreatedTokenPool = false;
  }

  function getFarmingContractNFT() external view returns (address) {
    return farmingContractNFT;
  }

  function getFarmingContractToken() external view returns (address) {
    return farmingContractToken;
  }

  function isCreatedNFTStakingPool() external view returns ( bool ) {
    return isCreatedNFTPool;
  }

  function isCreatedTokenStakingPool() external view returns ( bool ) {
    return isCreatedTokenPool;
  }
  ...
  function createNewTokenContract(
    ...
    address _stakedTokenAddy,
    uint256 _supply,
    uint256 _stakingExistTimeSeconds,
    uint256 _timelockSeconds,
    bool isNFT
  ) external {
    if (isNFT)
      require(!isCreatedNFTPool, "already exist pool");
    else
      require(!isCreatedTokenPool, "already exist pool");
    ...
    if (isNFT)
    {
       StakeToken _contract = new StakeToken(
        ...
        _stakedTokenAddy,
        msg.sender,
        _stakingExistTimeSeconds,
        _timelockSeconds,
        true
      );
      farmingContractNFT = address(_contract);
      isCreatedNFTPool = true;
      ...
      farmingContractToken = address(_contract);
      isCreatedTokenPool = true;
  ...
  function removeTokenContract(bool isNFT) external {
    if (isNFT)
    {
      StakeToken _contract = StakeToken(farmingContractNFT);
      require(
      ...
      StakeToken _contract = StakeToken(farmingContractToken);
      require(
        msg.sender == _contract.tokenOwner(),
        'user must be the original token owner to remove tokens'
      );
  ...
  function getTotalRewardPaid(address _userAddy) external view returns (uint256[] memory) {
    ...
    if (isCreatedNFTPool) {
      StakeToken _contractNFT = StakeToken(farmingContractNFT);
      _ret[0] = _contractNFT.calcHarvestTot(_userAddy); //claimable reward amount in nft staking pool
    }
    
    if (isCreatedTokenPool) {
      StakeToken _contractToken = StakeToken(farmingContractToken);
      _ret[1] = _contractToken.calcHarvestTot(_userAddy); //claimable reward amount in token staking pool
    }    
