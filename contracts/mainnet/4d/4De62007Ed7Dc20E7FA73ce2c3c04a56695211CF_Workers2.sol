...
contract Workers2 is ERC721Enumerable, Ownable, WorkerFactoryv3, PriceConsumerWTN {
    ...
    function setXCurrentGen(uint8 _gen) external onlyOwner {
        currentGen = _gen;
    }

    function setXCurrentGenQuantity(uint256 _quant) external onlyOwner {
        currentGenQuantity = _quant;
    }

    function setXNextRewardTax(uint8 _value) external onlyOwner {
        nextRewardsTax = _value;
    }

    function setXBurnTax(uint8 _value) external onlyOwner {
        burnTax = _value;
    }

    function setXExchangeAddress(address _address) external onlyOwner {
        exchangeAddress = _address;
    }

    function setXNextRewardsAddress(address _address) external onlyOwner {
        nextRewardsAddress = _address;
    }

    function setXSacrificeRitualAddress(address _address) external onlyOwner {
        sacrificeRitualAddress = _address;
    }

    function setXHospitalAddress(address _address) external onlyOwner {
        HospitalAddress = _address;
    }

    function setXTempleAddress(address _address) external onlyOwner {
