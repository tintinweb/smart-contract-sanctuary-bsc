...
contract paySystem is Ownable {
    ...
    // withdrawCHDId => status
    mapping(uint => bool) public withdrawCHDIdOf;
    uint public totalWithdrawCHD;
    mapping(address => uint) public withdrawCHDOf;
    ...
    uint public CHDRate = 0;
    address public secretSigner;


    event WithdrawReward(address indexed user, uint amount, uint withdrawRewardId);
    event WithdrawCHD(address indexed user, uint amount, uint withdrawCHDId);
    constructor (address _token) {
        CHD_ADDRESS = _token;
    }

    function setAToken(address _token) external onlyOwner {
        CHD_ADDRESS = _token;
    }
    ...
    function setCHDRate(uint _rate) external onlyOwner {
        CHDRate = _rate;
    }
    ...
    function withdrawCHD(uint _withdrawCHDId, address _to, uint _amount, uint8 _v, bytes32 _r, bytes32 _s) external {
        require(!withdrawCHDIdOf[_withdrawCHDId], "withdrawCHD: invalid withdrawCHDId");
        require(address(0) != _to, "withdrawCHD: address(0) != _to");
        require(0 < _amount, "withdrawCHD: 0 < _amount");
        require(address(0) != secretSigner, "withdrawCHD: address(0) != secretSigner");
        bytes32 msgHash = keccak256(abi.encodePacked(_withdrawCHDId, _to, _amount));
        require(ecrecover(msgHash, _v, _r, _s) == secretSigner, "withdrawCHD: incorrect signer");

        uint fee = _amount * extractRate / _decimals;
        // transfer CHD token
        IERC20(CHD_ADDRESS).safeTransfer(_to, _amount - fee);

        // update totalWithdrawCHD & withdrawCHDOf & alreadyMine
        withdrawCHDOf[_to] += _amount;
        totalWithdrawCHD += _amount;

        // update _withdrawCHDId
        withdrawCHDIdOf[_withdrawCHDId] = true;
        
        //

        emit WithdrawCHD(_to, _amount, _withdrawCHDId);
    }
