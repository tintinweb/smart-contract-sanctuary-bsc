...
contract UniswapV2Adapter is AdapterBase {
    ...
    function _executeCall(
        ...
        SwapArgs memory swapArgs = abi.decode(args, (SwapArgs));
        require(tokenIn == swapArgs.path[0], "TokenIn does not match path");
        IUniswapV2Router02(target).swapExactTokensForTokens(  //todo support native token
            amountIn,
            swapArgs.minAmountOut,
            swapArgs.path,
            swapArgs.receiver,
            swapArgs.deadline
        );
