...
contract StrategyPlatypus is ReentrancyGuard, Ownable, CoreRef {
    ...
    address public inputTokenAddress;
    address public wavaxAddress;
    address public ptpDaiLPToken;
    address public ptpDaiProxyAddress;
    address public ptpAddress;
    address public ptptrollerAddress;
    address public dexRouterAddress;
    address[] public baseTokenToInputTokenPath;
    uint256 public swapSlippage;
    address public oracle;
    bool public isPtpReward = true;
    ...
    constructor(
        ...
        address _inputTokenAddress,
        address _wavaxAddress,
        address _ptpDaiLPToken,
        address _ptpDaiProxyAddress,                          
        address _ptpAddress,
        address _ptptrollerAddress,
        address _dexRouterAddress,
        address[] memory _baseTokenToInputTokenPath,
        uint256 _swapSlippage,
        address _oracle
        ...
        inputTokenAddress = _inputTokenAddress;
        wavaxAddress = _wavaxAddress;
        ptpDaiLPToken = _ptpDaiLPToken;
        ptpDaiProxyAddress = _ptpDaiProxyAddress;
        ptpAddress = _ptpAddress;
        ptptrollerAddress = _ptptrollerAddress;
        dexRouterAddress = _dexRouterAddress;
        baseTokenToInputTokenPath = _baseTokenToInputTokenPath;
        swapSlippage = _swapSlippage;
        oracle = _oracle;
    ...
    function _deposit(uint256 _depositAmt) internal {
        IERC20(inputTokenAddress).safeApprove(ptpDaiProxyAddress, _depositAmt);
        uint256 mintedLiquidity = IPtpDaiProxy(ptpDaiProxyAddress).deposit(
            inputTokenAddress,
            _depositAmt,
            address(this),
            block.timestamp.add(600)
        ...
        // stake to the pool
        IERC20(ptpDaiLPToken).safeApprove(ptptrollerAddress, mintedLiquidity);
        // pool ID 2 is DAI.e pool
        IPtptroller(ptptrollerAddress).deposit(2, mintedLiquidity);
        depositedLiquidity += mintedLiquidity;
    }

    function withdraw() public onlyMultistrategy nonReentrant {
        // Unstake
        IPtptroller(ptptrollerAddress).withdraw(2, depositedLiquidity);
        // Reset deposited LP
        depositedLiquidity = 0;

        uint256 withdrawAmountToBe = IERC20(ptpDaiLPToken).balanceOf(address(this));
        // Approve LP token
        IERC20(ptpDaiLPToken).safeApprove(ptpDaiProxyAddress, withdrawAmountToBe);
        uint256 withdrawnAmount = IPtpDaiProxy(ptpDaiProxyAddress).withdraw(
            inputTokenAddress,
            withdrawAmountToBe,
            (withdrawAmountToBe * 99) / 100,
            address(this),
            block.timestamp.add(600)
        );
        ...
        if (isPtpReward == true) {
            uint256[] memory claimInputs = new uint256[](3);
            claimInputs[0] = uint256(20);
            claimInputs[1] = uint256(1);
            claimInputs[2] = uint256(2);
            IPtptroller(ptptrollerAddress).multiClaim(claimInputs);
    ...
    function _calculateMinReturn(uint256 _amount, address _rewardTokenAddress)
        internal
        view
    ...
    function inCaseTokensGetStuck(
        ...
        require(_token != inputTokenAddress, "!safe");
        require(_token != ptpDaiProxyAddress, "!safe");
...
contract StrategyWonderland is ReentrancyGuard, Ownable, CoreRef {
    ...
    function _expectedStakingReturn(uint256 _type, uint256 _depositAmt, uint256 _day) internal view returns(uint256) {
        ...
        r.epochPerDay = 3;
        r.trancheDuration = ITrancheMaster(trancheMasterAddress).duration();
        if (_type == 0) {
            ...
            // 1003 / 1000 refer to 0.03% fee for dex trade
...
contract TrancheMasterAuto is ITrancheMasterAuto, CoreRef, ReentrancyGuard {
    ...
    uint256 public constant PercentageScale = 1e18;
    uint256 private constant MaxAPY = 100000;
    uint256 private constant MaxFee = 20000;
    ...
    function _processExit() internal {
        ...
        for (uint256 i = 0; i < tranches.length - 1; i++) {
            ...
            if (capital > principal) {
                uint256 fee = capital.sub(principal).mul(senior.fee).div(PercentageParamScale);
                producedFee = producedFee.add(fee);
                capital = capital.sub(fee);
            }
...

        {
            ...
            capital = restCapital;
            if (capital > principal) {
                uint256 fee = capital.sub(principal).mul(junior.fee).div(PercentageParamScale);
                producedFee = producedFee.add(fee);
                capital = capital.sub(fee);
            }
            cycleExchangeRate = _calculateExchangeRate(capital, principal);
