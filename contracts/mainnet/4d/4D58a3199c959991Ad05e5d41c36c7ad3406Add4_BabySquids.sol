...
contract BabySquids is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  IERC20 public tokenAddress =
    IERC20(0x3F5c18ED4A7EA28066949582200519AB68287b76); // BSquidgrow
  uint256 public rate = 10000000 * 10**18; //0.01% of BSquidgrow Total Supply

  address private _BabySquidsBurn = 0x000000000000000000000000000000000000dEaD;
  ...
  function mint(uint256 _mintAmount) public {
    // Checks
    require(!paused, "BabySquids: minting has not started yet");
    require(_mintAmount > 0, "BabySquids: you have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "BabySquids: you can only mint 5 at a time");
    require(lastSupply >= _mintAmount,  "BabySquids: the collection is sold out");
    uint256 balance = tokenAddress.balanceOf(msg.sender);
    if (msg.sender != owner()) {
        require(balance >= _mintAmount.mul(rate), "BabySquids: total cost isn't match");
    }
    tokenAddress.transferFrom(msg.sender, _BabySquidsBurn, _mintAmount.mul(rate));
    // Minting
  ...
  function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
    // Checks
    require(_winner != address(0), "BabySquids: zero address");
    require(_amount > 0, "BabySquids: you have to mint at least one");
    require(lastSupply != 0,  "BabySquids: the collection is sold out");
  ...
  function tokenURI(uint256 _id) public view virtual override returns (string memory){
    require(_exists(_id),"BabySquids: URI query for nonexistent token");
