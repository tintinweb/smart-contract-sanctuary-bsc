...
contract CHNC is Context, IERC20, Ownable {
    ...
    bool public enableMarketing = false;
    ...
    */
    address payable private Wallet_Buyback =
        payable(0x41a94caeb3217B40e089a386F37e5cD75B04a9Ce);
    address payable private Wallet_Marketing =
        payable(0x0bf55E0B46fbFfE7618EFd06B1F2668D0c825761);
    ...
    */
    ...
    string private _name = "ooooooo";
    string private _symbol = "oo";
    ...
    */
    ...
    function buyback_Switch(bool true_or_false) public onlyOwner {
        enableBuyback = true_or_false;
    }

    function addLiquidity_Switch(bool true_or_false) public onlyOwner {
        enableAddliquidity = true_or_false;
    }

    function addMarketing_Switch(bool true_or_false) public onlyOwner {
        enableMarketing = true_or_false;
    }
    ...
    */
    ...
    function _transferTokens(
        ...
        if (tBuyback > 0 && enableBuyback) {
            swapBNBToWallet(tBuyback);
        }

        if (tLiquidity > 0 && enableAddliquidity) {
            swapAndLiquify(tLiquidity);
        }

        if (tMarketing > 0 && enableMarketing) {
            swapBNBToWallet(tMarketing);
        }
    ...
    {
        uint256 totalFee = (tAmount * _TotalFee) / 100;
        uint256 tBuyback = (totalFee * 60) / 100;
        uint256 tLiquidity = (totalFee * 10) / 100;
        uint256 tMarketing = (totalFee * 30) / 100;

        uint256 tTransferAmount = tAmount.sub(totalFee);
        return (tTransferAmount, tBuyback, tLiquidity, tMarketing);
    }

    function swapBNBToWallet(uint256 tokens) private {
        ...
        // swap tokens for ETH
        swapTokensForBNB(tokens); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered

        // how much ETH did we just swap into?
        // uint256 newBalance = address(this).balance.sub(initialBalance);
        // sendToWallet(wallet, newBalance);
    }
