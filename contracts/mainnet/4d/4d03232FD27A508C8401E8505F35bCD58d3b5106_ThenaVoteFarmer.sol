...
// File contracts/interfaces/IThenaVoter.sol
...
interface IThenaVoter is IVoter {
...
// File contracts/interfaces/IThenaBribe.sol
...
// File contracts/interfaces/IThenaInternalBribe.sol
...
interface IThenaInternalBribe is IBribe {
...
// File contracts/interfaces/IThenaExternalBribe.sol
...
// Bribes pay out rewards for a given pool based on the votes that were received from the user (goes hand in hand with Voter.vote())
interface IThenaExternalBribe is IBribe {
    // address public immutable voter; // only voter can modify balances (since it only happens on vote())
...
// File contracts/interfaces/IThenaWrappedExternalBribeFactory.sol


pragma solidity ^0.8.0;

interface IThenaWrappedExternalBribeFactory {
    function oldBribeToNew(address) external view returns (address);
}


// File contracts/interfaces/IThenaRewardsDistributor.sol


pragma solidity 0.8.13;

interface IThenaRewardsDistributor {
...
interface IThenaRouter is IRouter {
    struct route {
        address from;
        address to;
        bool stable;
...
interface IThenaGauge is IGauge {
    /// STATE VARIABLES ///
...
// File contracts/ThenaVoteFarmer.sol
...
error ThenaVoteFarmer__Unauthorized();
error ThenaVoteFarmer__WrongInput();
error ThenaVoteFarmer__VoteCooldown();
error ThenaVoteFarmer__TokenExpiresTooEarly();
error ThenaVoteFarmer__TokenTooSmall();
error ThenaVoteFarmer__OverNftCap();

/// @dev Vote for thena pairs, get rewards and grow veTHE
contract ThenaVoteFarmer is UUPSUpgradeable, AccessControlEnumerableUpgradeable, PausableUpgradeable {
    ...
    uint256 public constant MAX_TIME = 2 * 365 * 86400; // Max lock duration
    uint256 public constant MIN_AMOUNT = 2 * 1e19;
    address public constant THE = 0xF4C8E32EaDEC4BFe97E0F595AdD0f4450a863a11;
    address public constant VETHE = 0xfBBF371C9B0B994EebFcC977CEf603F7f31c070D;
    IThenaVoter public constant THENA_VOTER = IThenaVoter(0x62Ee96e6365ab515Ec647C065c2707d1122d7b26);
    address public constant THENA_ROUTER = 0x20a304a7d126758dfe6B243D0fc515F83bCA8431;
    IThenaRewardsDistributor public constant THENA_REWARDS_DISTRIBUTOR = IThenaRewardsDistributor(0xE9fE83aA430Ace4b703C299701142f9dFdde730E);
    IThenaWrappedExternalBribeFactory public constant THENA_WEBF = IThenaWrappedExternalBribeFactory(0x4BF5A42202927B0263D2279E509b3dA05A6235bC);
    ...
    struct RewardInfo {
        ...
        uint256 totalReceivedThena; // How much the above was worth in THE
    }
    ...
    EnumerableSetUpgradeable.AddressSet private rewards; // Rewards (to swap to THE)
    ...
    function __ThenaVoteFarmer_init() internal onlyInitializing {
    ...
    function initialize() public initializer {
        __ThenaVoteFarmer_init();
        manager = IThenaVoteManager(msg.sender);


        // Somewhat unsafe, saves gas
        IERC20Upgradeable(THE).safeIncreaseAllowance(VETHE, type(uint256).max);
    }
    ...
    function harvest() external onlyRole(MANAGER) {
        ...
        (uint256 balanceBefore, uint256 votingPowerBefore) = _getThenaBalance();
        increaseDurationAll();
        claimFees();
        swapRewards(); //into THE
        chargeFees(); //charge fees on THE
        compoundThena(); //deposits proper THE into veNFTs
        rebase();
        (uint256 balanceAfter, uint256 votingPowerAfter) = _getThenaBalance();
    ...
    function deposit(uint256 _tokenId, address _owner) external whenNotPaused onlyRole(MANAGER) {
        ...
        if (!IThenaVotingEscrow(VETHE).isApprovedOrOwner(address(this), _tokenId)) {
            revert ThenaVoteFarmer__Unauthorized();
        }
        if (tokenIds.length >= nftCap) {
            revert ThenaVoteFarmer__OverNftCap();
        }
        // Claimable will revert should the NFT be "broken" following a merge
        THENA_REWARDS_DISTRIBUTOR.claimable(_tokenId);
    ...
    /// @notice For each token, try to swap to THE
    /// @dev To prepare for the incoming compounding, should store the amount of THE gotten
    function swapRewards() public onlyRole(MANAGER) {
        uint256 rewardBal;
        uint256 thenaBalBefore;
        uint256 thenaBalAfter;
        for (uint256 i; i < rewards.length(); i = _uncheckedInc(i)) {
            rewardBal = IERC20Upgradeable(rewards.at(i)).balanceOf(address(this));
            thenaBalBefore = IERC20Upgradeable(THE).balanceOf(address(this));
            _swap(rewards.at(i), THE, rewardBal);
            thenaBalAfter = IERC20Upgradeable(THE).balanceOf(address(this));
            tokenToRewardInfo[rewards.at(i)].totalReceivedThena = thenaBalAfter - thenaBalBefore;
        }
    }

    /// @notice Distribute available THE to grow the veNfts
    /// @dev THE for 1 veNft = (nftReward1Share * 1e18 / reward1ReceivedTotal) + ()
    function compoundThena() public onlyRole(MANAGER) {
        uint256 veNftRewardShare;
        uint256 totalReward;
        uint256 totalRewardThena;
        uint256 veNftTotalThena;
        ...
        for (uint256 i; i < tokenIds.length; i = _uncheckedInc(i)) {
            for (uint256 j = 0; j < rewards.length(); j = _uncheckedInc(j)) {
                ...
                //total amount of reward expressed in THE
                totalRewardThena = tokenToRewardInfo[rewards.at(j)].totalReceivedThena;
                if (totalReward != 0) {
                    veNftTotalThena += (veNftRewardShare * totalRewardThena) / totalReward;
                }
            }
            if (veNftTotalThena != 0) {
                IThenaVotingEscrow(VETHE).increase_amount(tokenIds[i], veNftTotalThena);
                veNftTotalThena = 0;
            }
        }
    }
    ...
    function rebase() public onlyRole(MANAGER) {
        THENA_REWARDS_DISTRIBUTOR.claim_many(tokenIds);
    }
    ...
    function resetBalancesAll() external onlyRole(MANAGER) {
        for (uint256 i; i < rewards.length(); i = _uncheckedInc(i)) {
            ...
            tokenToRewardInfo[rewards.at(i)].totalReceivedThena = 0;
        }
    }
    ...
    function synchronize() external onlyRole(MANAGER) {
        ...
        for(uint256 i; i < gaugesLength; i = _uncheckedInc(i)) {
            ...
            pairs.push(IThenaGauge(gauges[i]).TOKEN());
        }
        // Rewards from getRewards + swap path from rewards to THE
        ...
        for (uint256 i; i < rewardsArray.length; i = _uncheckedInc(i)) {
            ...
            swapPath[rewardsArray[i]][THE] = manager.getSwapPath(rewardsArray[i],THE);
        }

        // Swap path from THE to BUSD
        swapPath[THE][BUSD] = manager.getSwapPath(THE,BUSD);
    }
    ...
    function _vote(
        ...
        uint256 lastVoted = THENA_VOTER.lastVoted(_tokenId);
        uint256 votingPower = IThenaVotingEscrow(VETHE).balanceOfNFT(_tokenId);

        if ((block.timestamp / EPOCH) * EPOCH > lastVoted && votingPower != 0) {
            THENA_VOTER.vote(_tokenId, _pairs, _weights);
        }
    }
    ...
    function _claimFees(uint256 _tokenId) internal {
        ...
        for (uint256 i; i < gauges.length; i = _uncheckedInc(i)) {
            ...
            address iBribe = THENA_VOTER.internal_bribes(gauges[i]); //swap fees
            address eBribe = THENA_VOTER.external_bribes(gauges[i]); //bribes
            // address eBribe = THENA_WEBF.oldBribeToNew(oldEBribe); //bribes

            // Construct rewards for Internal Bribe, External Bribe
            uint256 iRewardsLen = IThenaInternalBribe(iBribe).rewardsListLength();
            uint256 eRewardsLen = IThenaExternalBribe(eBribe).rewardsListLength();
            ...
            for (uint256 j; j < iRewardsLen; j = _uncheckedInc(j)) {
                iRewards[j] = IThenaInternalBribe(iBribe).rewardTokens(j);
                iRewardsBalBefore[j] = IERC20Upgradeable(iRewards[j]).balanceOf(address(this));
            }
            IThenaInternalBribe(iBribe).getReward(_tokenId, iRewards); //get rewards all at the same time
            for (uint256 j; j < iRewardsLen; j = _uncheckedInc(j)) {
            ...
            for (uint256 j; j < eRewardsLen; j = _uncheckedInc(j)) {
                eRewards[j] = IThenaExternalBribe(eBribe).rewardTokens(j);
                eRewardsBalBefore[j] = IERC20Upgradeable(eRewards[j]).balanceOf(address(this));
            }
            IThenaExternalBribe(eBribe).getReward(_tokenId, eRewards);
            for (uint256 j; j < eRewardsLen; j = _uncheckedInc(j)) {
    ...
    function _increaseDuration(uint256 _tokenId) internal {
        uint256 newUnlockTime = IThenaVotingEscrow(VETHE).locked__end(_tokenId) + EPOCH - block.timestamp;
        if (((block.timestamp + newUnlockTime) / EPOCH * EPOCH) > block.timestamp + MAX_TIME) {
            // If somehow it still exceeds the max lock, skip
            return;
        }
        IThenaVotingEscrow(VETHE).increase_unlock_time(_tokenId, newUnlockTime);
    }

    function _chargeFees() internal {
        IERC20Upgradeable busd = IERC20Upgradeable(BUSD);
        uint256 busdBalBefore = busd.balanceOf(address(this));
        uint256 toSwap = (IERC20Upgradeable(THE).balanceOf(address(this)) * totalFee) / PERCENT_DIVISOR;
        _swap(THE, BUSD, toSwap);
        ...
        for (uint256 i; i < rewards.length(); i = _uncheckedInc(i)) {
            tokenToRewardInfo[rewards.at(i)].totalReceivedThena -=
                (tokenToRewardInfo[rewards.at(i)].totalReceivedThena * totalFee) /
                PERCENT_DIVISOR;
        }
    }
    ...
    function _swap(
        ...
        uint256 output;
        bool useStable;
        IThenaRouter router = IThenaRouter(THENA_ROUTER);
        address[] storage path = swapPath[_from][_to];
        IThenaRouter.route[] memory routes = new IThenaRouter.route[](path.length - 1);
        ...
        for (uint256 i; i < routes.length; i = _uncheckedInc(i)) {
            ...
            routes[i] = IThenaRouter.route({from: path[i], to: path[i + 1], stable: useStable});
    ...
    function _removeRevokedNfts() internal {
        for (uint256 i; i < tokenIds.length;  i = _uncheckedInc(i)) {
            uint256 lockEnd = IThenaVotingEscrow(VETHE).locked(tokenIds[i]).end;
            if ((!IThenaVotingEscrow(VETHE).isApprovedOrOwner(address(this), tokenIds[i])) || lockEnd < block.timestamp) {
    ...
    function _getThenaBalance() internal view returns (uint256 lockedTotal, uint256 votingPowerTotal) {
        IThenaVotingEscrow.LockedBalance memory locked;
        for (uint256 i; i < tokenIds.length;  i = _uncheckedInc(i)) {
            locked = IThenaVotingEscrow(VETHE).locked(tokenIds[i]);
            votingPowerTotal += IThenaVotingEscrow(VETHE).balanceOfNFT(tokenIds[i]);
            if (locked.amount >= 0) {
    ...
    function _requireMinimumLockDuration(uint256 _tokenId) internal view {
        if (IThenaVotingEscrow(VETHE).locked__end(_tokenId) <= block.timestamp + EPOCH) {
            revert ThenaVoteFarmer__TokenExpiresTooEarly();
        }
    }

    function _requireMinimumAmount(uint256 _tokenId) internal view {
        if (uint128(IThenaVotingEscrow(VETHE).locked(_tokenId).amount) < MIN_AMOUNT) {
            revert ThenaVoteFarmer__TokenTooSmall();
        }
    }

    function _requireOwnerOf(uint256 _tokenId, address _owner) internal view {
        if (!IThenaVotingEscrow(VETHE).isApprovedOrOwner(_owner, _tokenId)) {
            revert ThenaVoteFarmer__Unauthorized();
        }
    }
