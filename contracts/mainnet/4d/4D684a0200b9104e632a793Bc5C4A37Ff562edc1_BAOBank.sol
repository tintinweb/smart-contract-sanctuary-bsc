...
contract BAOBank{
    ...
    mapping(address=>uint256)public sBAO;
    ...
    constructor (address _DEX,address pancakeswap) public {
        ...
        price=20;
        sTime=86400;
        BAO=0x1b7cc2E9DfeAdc0AA3c283D727C50DF84558dA59;
        USDT=0x55d398326f99059fF775485246999027B3197955;
        tokenEx(_DEX).approve(address(pancakeswap), 2 ** 256 - 1);
        tokenEx(0x1b7cc2E9DfeAdc0AA3c283D727C50DF84558dA59).approve(address(0x10ED43C718714eb63d5aA57B78B54704E256024E), 2 ** 256 - 1);
    }
    ...
    function payToken(address up)public{
        ...
        uint _BAOss=getBaos();
        uint _BAO=_BAOss*97/100;
        tokenEx(BAO).transferFrom(msg.sender,address(this),_BAOss);
        ...
        tokenEx(BAO).transfer(address(0x000000000000000000000000000000000000dEaD),_BAO *30/100);
        toDEX(_BAO*70/100);
    ...
    function toDEX(uint _bao)public {
        uint baoTousdt=_bao;
        address[] memory path = new address[](3);
        path[0]=0x3Cd82B316bCE3CcCbb2A1cA3741a4603B8feBfFA;
        path[1]=0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
        path[2]=0x55d398326f99059fF775485246999027B3197955;
        IRouter(0x1b7cc2E9DfeAdc0AA3c283D727C50DF84558dA59).swapExactTokensForTokensSupportingFeeOnTransferTokens(baoTousdt,0,path,address(this),block.timestamp+100);
    }
    ...
    function getBao(address addr)private view returns(uint){
        uint _time;
        uint _BAO;
        if(block.timestamp > isTime[msg.sender] + sTime){
            _time=(block.timestamp-isTime[addr])/sTime;
            _BAO=price * _time * inBNB[msg.sender]/100;   
        }else{
            _BAO=0;
        }
        return _BAO;
    ...
    function getUser(address addr)public view returns(address,uint,uint,uint,uint,uint,uint){
        ...
        return (_addr,_BAO,_time,sUSDT[_addr],sBAO[_addr],TeamMembers[_addr],tokenEx(USDT).balanceOf(address(this)));   
    }
    function getBaos()public  view returns (uint){
        address[] memory path = new address[](3);
        uint[] memory amount;
        path[0]=0x55d398326f99059fF775485246999027B3197955;
        path[1]=0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
        path[2]=0x3Cd82B316bCE3CcCbb2A1cA3741a4603B8feBfFA;
        amount=IRouter(0x1b7cc2E9DfeAdc0AA3c283D727C50DF84558dA59).getAmountsOut(100 ether,path); 
        return amount[2];
    }
    function getBAOUp(address addr)public view returns (address){        
