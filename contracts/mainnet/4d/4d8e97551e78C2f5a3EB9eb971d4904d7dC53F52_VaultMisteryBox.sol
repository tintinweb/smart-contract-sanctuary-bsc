    ...
    AccessControlUpgradeable
{
    ...
    function _swapPaymentTokenToBUSD() internal {
        ...
        uint256[] memory minAmounts = router.getAmountsOut(
            (paymentToken.balanceOf(address(this)) * 2500) / 10000,
            path
        );
    ...
    function _swapBUSDToGQToken() internal {
        address[] memory path = new address[](2);
        path[0] = address(busd);
        path[1] = address(gqToken);
        uint256 quantityBUSDToGQ = (busd.balanceOf(address(this)) * 5000) /
            10000;
        uint256[] memory minAmounts = router.getAmountsOut(
            quantityBUSDToGQ,
            path
    ...
    function _addLiquidityGQBUSD() internal {
        address[] memory path = new address[](2);
        path[0] = address(busd);
        path[1] = address(gqToken);
        uint256 minQuantityBUSD = (busd.balanceOf(address(this)) * 9500) /
            10000;
        uint256 minQuantityGQ = (gqToken.balanceOf(address(this)) * 9500) /
            10000;
        ...
        router.addLiquidity(
            ...
            gqToken.balanceOf(address(this)),
            minQuantityBUSD,
            minQuantityGQ,
            address(this),
            block.timestamp
