...
// File contracts/Mining/Activities.sol
...
contract ACTIVITIES is Ownable {
    ...
    uint32 max_shares_steps = 20000;
    uint32 max_shares_calories = 20000;
    uint32 max_shares_distance = 20000;
    uint32 max_shares_workouts = 20000;
    uint32 max_shares_sleep = 20000;
    ...
    function computeAllShares (
        ...
        workouts_list;
        uint total = 0;
        uint total_steps = data[0]+data[1]+data[2]+data[3];
        {
            total = computeIntensityStepsShares(tokenId, data[0], data[1], data[2], data[3]) + computeDailyStepsShares(tokenId, total_steps);
        }
        {
            total = total + distanceComputeShares(tokenId, data[4]) + caloriesComputeShares(tokenId, data[5]) + workoutsComputeShares(tokenId, data[6]) + sleepComputeShares(tokenId, data[7]);
        }
        {
...
// File contracts/MiningV2/Mining.sol
...
contract MINING is Pausable, Ownable {
    ...
    mapping ( uint256 => shareData ) public  submitted;
    mapping ( uint256 => shareData ) public  claimed;
    mapping ( uint256 => submittedData[] ) public  submitterByNft;


    uint public miningDate;
    uint public miningDays;
    uint private startingDate;
    address private dateServer;
    ...
    constructor(address _stake, address _airdrops, address _activities) Ownable() {
        stakeCon = STAKE(_stake);
    ...
    function submit (
        ...
        uint[] calldata workouts,
        uint32[] calldata date
        ...
        uint _date = dateRef(date[1], date[2], date[3]);
        require( isValidDateToSubmit(date[0]), "Submission outdated");
        require(msg.value >= submissionFees, "Not enough BNB sent; check fees!");
        require(keccak256(abi.encodePacked(_tkn)) == tknHash, "App not allowed");
        require(isValidSubmitter(tokenId, owner, msg.sender, date[0]), "Invalid submitter");
        uint track = trackId(tokenId, date[1], date[2], date[3]);
        require(!isAlreadySubmitted(track), "Already submitted");
        shareData memory fin_data = shareData(owner, tokenId, activitiesCon.computeAllShares(tokenId, data, workouts), date[1], date[2], date[3], date[0]);
        totalDailyShares[date[0]] = totalDailyShares[date[0]] + fin_data.shares;
        submitted[track] = fin_data;
        submitterByNft[tokenId].push(submittedData(fin_data.shares, date[0],date[1], date[2], date[3]));
        setLastDate(date[0], _date);
        emit submitEvent(
            track,
            tokenId,
            owner,
    ...
    function claim (string memory _tkn, uint256 tokenId) payable whenNotPaused public returns ( bool ) {
        require(msg.value >= miningFees, "Not enough BNB sent; check fees!");
        require(keccak256(abi.encodePacked(_tkn)) == tknHash, "App not allowed");
        uint32 ratio_multiplier = 1000000;
        (address owner,,,,,) = stakeCon.ownerOf(tokenId);
        uint n = submitterByNft[tokenId].length;
        submittedData[] memory _array = new submittedData[](n);
        for (uint256 index = 0; index < n; index++) {
            _array[index] = submitterByNft[tokenId][index];
        }
        uint32 totalRatio = 0;
        uint totalShares = 0;
        for (uint256 index = 0; index < n; index++) {
            uint _days = _array[index]._days;
            if(isValidDateToMine(_days)){
                uint32 ratio = uint32((_array[index].shares*ratio_multiplier)/totalDailyShares[_days]);
                totalRatio = totalRatio + ratio;
                removeFromSubmitterByNft(tokenId, index);
                uint track = trackId(tokenId, _array[index].day, _array[index].month, _array[index].year);
                claimed[track] = submitted[track];
                delete submitted[track]; 
            }
        }
        airdropsCon.claimTokens(owner, totalRatio);
        emit claimEvent(
            tokenId,
            owner,
            totalShares
    ...
    function addDailyShares (uint32 _day, uint _shares) public onlyAllowedContract returns (bool) {
        totalDailyShares[_day] = totalDailyShares[_day] + _shares;
        return true;
    }

    function removeFromSubmitterByNft(uint tokenId, uint index) private {
        for(uint i = index; i < submitterByNft[tokenId].length-1; i++){
        submitterByNft[tokenId][i] = submitterByNft[tokenId][i+1];
        }
        submitterByNft[tokenId].pop();
    }
    function isValidSubmitter (uint tokenId, address owner, address intWal, uint date_days) public view returns (bool) {
        (address _owner, address _int_wallet,,,, uint32 _days) = stakeCon.ownerOf(tokenId);
        if(owner == _owner && intWal == _int_wallet && date_days >= _days){
            return true;
        }
        return false;
    }
    function isAlreadySubmitted (uint _trackId) public view returns (bool) {
        uint tk = submitted[_trackId]._tokenId;
        return tk!=0;
    }


    function getPendingClaimsNbr (uint tokenId) public view returns (uint) {
        return submitterByNft[tokenId].length;
    }
    function getPendingClaims (uint tokenId) public view returns (submittedData[] memory _array) {
        return submitterByNft[tokenId];
    }

    function getSubPerDay (uint tokenId, uint32 day, uint32 month, uint32 year) public view returns (shareData memory) {
        uint ref = trackId(tokenId, day, month, year);
        return submitted[ref];
    }
    function getClaimPerDay (uint tokenId, uint32 day, uint32 month, uint32 year) public view returns (shareData memory) {
        uint ref = trackId(tokenId, day, month, year);
        return claimed[ref];
    }
    function getDayShares (uint _day) public view returns (uint) {
        return totalDailyShares[_day];
    }
    function isValidDateToSubmit (uint _days) public view returns (bool) {
        if(_days > miningDays) return true;
        return (miningDays - _days) < daysBuffer;
    }
    function isValidDateToMine (uint _days) public view returns (bool) {
        if(_days > miningDays) return false;
        if(_days < startingDate) return false;
        return (miningDays - _days) > (daysBuffer - 1);
    }
    ...
    function trackId (uint256 tknId, uint32 day, uint32 month, uint32 year) private pure returns (uint256) {
        return (tknId*100000000) + dateRef(day, month, year);
    }

    function dateRef (uint32 day, uint32 month, uint32 year) private pure returns (uint256) {
    ...
    function setLastDate (uint _days, uint _date) private returns (bool) {
        if(_days > miningDays){
            miningDays = _days;
            miningDate = _date;
        }
    ...
    function setDate (uint _days, uint _date) public onlyOwner returns (bool) {
        miningDays = _days;
        miningDate = _date;
        return true;
    }
    function setStartingDate () public onlyOwner view returns (uint) {
        return startingDate;
    }
    function setStartingDate (uint _days) public onlyOwner returns (bool) {
        startingDate = _days;
        return true;
    }
    function getDate () public view returns (uint _date, uint _days) {
        _date = miningDate;
        _days = miningDays;
    }
    function setDateServer(address _addr) public onlyOwner returns (bool) {
        dateServer = _addr;
        return true;
    }
    function setDateFromServer (uint _days, uint _date) public returns (bool) {
        require(msg.sender == dateServer, "not authorized");
        miningDays = _days;
        miningDate = _date;
        return true;
    }
    function setActivitiesContract (address _addr) public onlyOwner returns (bool) {
        activitiesCon = ACTIVITIES(_addr);
        return true;
    }
    function setStakeContract (address _addr) public onlyOwner returns (bool) {
        stakeCon = STAKE(_addr);
        return true;
    }
    function setAirdropsContract (address _addr) public onlyOwner returns (bool) {
        airdropsCon = AIRDROPS(_addr);
        return true;
    }
    function setDaysBuffer (uint _days) public onlyOwner returns (bool) {
        daysBuffer = _days;
        return true;
    }
    function getDaysBuffer () public view returns (uint) {
        return daysBuffer;
    }
    function getBalance() public view returns(uint) {
        return address(this).balance;
