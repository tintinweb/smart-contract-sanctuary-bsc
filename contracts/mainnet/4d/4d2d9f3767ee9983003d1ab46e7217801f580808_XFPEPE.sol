...
contract XFPEPE is ERC721, Ownable {
    using Strings for uint256;
    using Counters for Counters.Counter;

    Counters.Counter private supply;

    string public uriPrefix = "https://xrpfarm.app/api/";
    ...
    constructor() ERC721("XF Pepe", "XFPEPE") {}

    modifier mintRequire(uint256 _mintAmount) {
        require(
            supply.current() + _mintAmount <= currentSupply,
            "Max supply exceeded!"
        );
        _;
    }
    ...
    function updateSupply(uint256 newSupplyIncrement) external onlyOwner {
        currentSupply += newSupplyIncrement;
        require(currentSupply <= maxSupply, "current supply > maximum supply");
    }
    ...
    function mintTo(address _to) public onlyOwner mintRequire(1) {
        _mint(_to);
    }
