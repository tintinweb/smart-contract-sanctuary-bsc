...
import "../SpritzPay_V1.sol";

contract SpritzPay_V1_Staging is SpritzPay_V1 {}
...
import "./lib/SpritzPay_Storage.sol";
    ...
    WETHUpgradeable
{
    ...
    function payWithToken(
        ...
        IERC20 erc20 = IERC20(paymentTokenAddress);

        bool transferSuccess = erc20.safeTransferFrom(_msgSender(), paymentRecipient, paymentTokenAmount);
        if (!transferSuccess) {
    ...
    function setV3Router(address newRouterAddress) external onlyOwner {
        if (newRouterAddress == address(0)) revert SetZeroAddress();
        v3RouterAddress = newRouterAddress;
    }

    function setV2Router(address newRouterAddress) external onlyOwner {
        if (newRouterAddress == address(0)) revert SetZeroAddress();
        v2RouterAddress = newRouterAddress;
    }
...
contract SpritzPay_Storage {
    ...
    address internal v3RouterAddress;

    address internal v2RouterAddress;
