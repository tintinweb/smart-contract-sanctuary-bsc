...
contract CubicChicken is ERC20, Ownable {
    ...
    constructor(
        ...
        address owner,
        address mintToAddr
    ) ERC20('Cubic Chicken', 'CUBC') {
        require(
            feeAccount != address(0),
            'Cubic Chicken: fee account from the zero address'
        );
        require(feePercent <= PRECISION, 'Cubic Chicken: incorrect fee percent');
        require(owner != address(0), 'Cubic Chicken: incorrect owner address');
        require(mintToAddr != address(0), 'Cubic Chicken: incorrect mintToAddr address');
        if (_msgSender() != owner) {
    ...
    function _transfer(
        ...
        require(sender != address(0), 'Cubic Chicken: transfer from the zero address');
        require(recipient != address(0), 'Cubic Chicken: transfer to the zero address');
