...
/**
 * @title Baked Potatoes NFT Contract
 */
contract BakedPotatoes is ERC721Base, ReentrancyGuard {
    ...
    uint256 public immutable maxPerWallet = 5;
    uint256 public immutable maxSupply;

    uint256 public immutable reservedTokens = 20;

    uint256 public price = 0.2 ether;

    bool public saleIsActive = false;

    address public immutable walletArtist;      // 40%
    address public immutable walletFounder;     // 30%
    address public immutable walletDevelopment; // 15%
    address public immutable walletDev1;        // 7.5%
    address public immutable walletDev2;        // 7.5%
        ...
        ERC721Base(_name, _symbol, _baseURI)
    {
        ...
        walletArtist = _artist;
        walletFounder = _founder;
        walletDevelopment = _development;
        walletDev1 = _dev1;
        walletDev2 = _dev2;
    }
    ...
    function mint(uint256 _amount) external nonReentrant payable {
        ...
        require(balanceOf(msg.sender) + _amount <= maxPerWallet, "Exceeds max per wallet");
     ...
     * @param _amount: amount of nfts to mint
     */
    function mintFor(address _to, uint256 _amount) external nonReentrant payable {
        require(saleIsActive, "Sale not started yet");
        require((totalSupply() + _amount) <= maxSupply, "Exceeds max supply");
        require(_amount > 0 && _amount <= maxMintPerTx, "Invalid amount");
        require(msg.value == price * _amount, "Value sent != price");
        require(balanceOf(_to) + _amount <= maxPerWallet, "Exceeds max per wallet");
    ...
    function startSale() external onlyOwner {
        if (totalSupply() == 0) {
            // reserve some NFTs for team and giveaways
            _mintTokens(owner(), reservedTokens);
        }
        saleIsActive = true;
    }
    ...
    function withdraw() external nonReentrant {
        require(owner() == msg.sender || walletFounder == msg.sender 
            || walletArtist == msg.sender || walletDevelopment == msg.sender 
            || walletDev1 == msg.sender || walletDev2 == msg.sender, "Not authorized");

        uint256 balance = address(this).balance;
        payable(walletArtist).transfer(balance * 40 / 100);      // 40%
        payable(walletDevelopment).transfer(balance * 15 / 100); // 15%
        payable(walletDev1).transfer(balance * 75 / 1000);       // 7.5%
        payable(walletDev2).transfer(balance * 75 / 1000);       // 7.5%
        payable(walletFounder).transfer(address(this).balance);  // rest = 30%
    }
