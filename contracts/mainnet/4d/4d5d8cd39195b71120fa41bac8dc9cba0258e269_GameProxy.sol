...
contract GameProxy is Auth {
  ...
  event FarmPointConditionUpdated(uint farmPointCondition, uint timestamp);

  function initialize(address _mainAdmin, address _pve, address _pvp, address _dual, address _farm) public initializer {
    super.initialize(_mainAdmin);
    gamePve = IFOTAGame(_pve);
    gamePvp = IFOTAGame(_pvp);
    gameDual = IFOTAGame(_dual);
    farm = IFarm(_farm);
    farmPointCondition = 1000e18;
  }

  function validateInviter(address _inviter) external view returns (bool) {
    (,,uint point,,,) = farm.farmers(_inviter);
    return point >= farmPointCondition;
  }

  function getTotalPVEWinInDay(address _user) external view returns (uint) {
    return gamePve.getTotalWinInDay(_user);
  }

  function getTotalPVPWinInDay(address _user) external view returns (uint) {
    return gamePvp.getTotalWinInDay(_user);
  }

  function getTotalDUALWinInDay(address _user) external view returns (uint) {
    return gameDual.getTotalWinInDay(_user);
  }

  function setGameAddresses(address _pve, address _pvp, address _dual, address _farm) external onlyMainAdmin {
    require(_pve != address(0) && _pvp != address(0) && _dual != address(0) && _farm != address(0), "Invalid address");
    gamePve = IFOTAGame(_pve);
    gamePvp = IFOTAGame(_pvp);
    gameDual = IFOTAGame(_dual);
    farm = IFarm(_farm);
  ...
  function setFarmPointCondition(uint _farmPointCondition) external onlyMainAdmin {
    farmPointCondition = _farmPointCondition;
    emit FarmPointConditionUpdated(farmPointCondition, block.timestamp);
  }
}
