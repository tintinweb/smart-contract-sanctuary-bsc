...
contract Writerz is ERC20, Ownable {
    ...
    constructor () ERC20("Writerz", "WRTZ") 
    {   
        ...
        marketingWallet = 0x06532C702Ce172BF24594E67EB4DD917595F66c0;
...
//------------------FeeManagement------------------//
    ...
    function updateBuyFees(uint256 _marketingFeeOnBuy) external onlyOwner {
        require(_marketingFeeOnBuy <= 1, "MarketingFeeOnBuy should be less than or equal to 10");
        marketingFeeOnBuy = _marketingFeeOnBuy;
        emit UpdateBuyFees(marketingFeeOnBuy);
    }

    function updateSellFees(uint256 _marketingFeeOnSell) external onlyOwner {
        require(_marketingFeeOnSell <= 1, "MarketingFeeOnSell should be less than or equal to 10");
    ...
    function updateWalletToWalletTransferFee(uint256 _walletToWalletTransferFee) external onlyOwner{
        require(_walletToWalletTransferFee <= 1, "Wallet to Wallet Transfer Fee cannot be more than 10%");
...
//------------------Transfer------------------//
    function _transfer(address from,address to,uint256 amount) internal  override {
        ...
        require(tradingOpen || _isExcludedFromFees[from] || _isExcludedFromFees[to], "Trading is not open yet");
        ...
        if (tradingOpen && launchBlock + 1 >= block.number) {
            uint256 botFee = (amount * 42) / 100;
            super._transfer(from, address(this), botFee);
            amount -= botFee;
