...
contract CBGPool is Ownable {
    ...
    uint[] public _layerHashRate = [54,29,23,17,11,6,29,23,17,11,6,29,23,17,11,6,29,23,17,11,6];
    ...
    function updatePool(address _lpAddress) public {
    ...
    function pending(address _user,address _lpAddress) internal view returns (uint256){
        ...
        if(_user != deadAddress){
            return user.inviteReward.add(pendingAmount.mul(1e12).mul(userPercent).div(1000).div(1e12));
        }else{
    ...
    function deposit(address _lpAddress,address _user, uint256 _amount) internal returns (uint256) {
        ...
        if(_user != deadAddress && pendingReward > 0){
            uint256 userPercentAmount = pendingReward.mul(1e12).mul(userPercent).div(1000).div(1e12);

            distributeReward(_user,pendingReward.sub(userPercentAmount),_lpAddress);
            pendingReward = userPercentAmount;

        }
        pendingReward = user.inviteReward.add(pendingReward);
        user.inviteReward = 0;
    ...
    function distributeReward(address _user,uint256 _amount,address _lpAddress) internal {
        ...
        for (uint j = 0; j < _layerHashMax; j++) {
            ...
            if(isDistributeReward(cur)){
                ...
                userInfoMap[_lpAddress][cur].inviteReward = userInfoMap[_lpAddress][cur].inviteReward.add(inviteAmount);
    ...
    function updatePoolAllocPoint()  public onlyOwner {
        if(autoAdjustAlloc){
            massUpdatePools();
            totalAllocPoint = 0;
            uint256 length = lpAddressList.length;
...
contract CBG is CBGPool {
    ...
    uint256 private maxSupply = 21000000 * 1e18;
    uint256 public mintSupply;

    uint private _technologyRate = 5;
    address private _technologyAddr= address(0xd4C14436c2FfB4786773004489644cb1Cf829663);

    uint private _daoFundRate = 5;
    address private _daoFundAddr= address(0x95C76Df078ebB0B7A8920Bcd114bE1fa8736Ce7A);

    uint private _daoRate = 20;
    address private _daoAddr= address(0xf3cCd62a7567B55Ec5BC87D99a6830f80aA802d8);
    ...
    function mint(address account, uint256 amount) external onlyOwner{
        // require(mappingToken.balanceOf(address(this))>= amount.add(mintSupply));
        _mint(account,amount);

    }


    function _takeBuyFee(uint256 tAmount,address from) internal returns(uint allocatedAmount){
        uint curTAmount = 0;
    ...
    function _takeSellFee(uint256 tAmount,address from) internal returns(uint allocatedAmount){
        uint curTAmount = 0;
        // if(_liquidityRate>0) {
        //     curTAmount = tAmount.div(1000).mul(_liquidityRate);
        //     _balances[address(this)] = _balances[address(this)].add(curTAmount);
    ...
    function _checkPool(address src) internal {
        ...
        if(pendingReward > 0){
            if(src == deadAddress){
                ...
                _mint(deadAddress,dAmount);
                if(pendingReward > dAmount){
                    _mint(treasuryAddress,pendingReward.sub(dAmount));
                }
            }else{
                _mint(src,pendingReward);
            }
        }
    ...
    function _sellFee(address sender, address recipient, uint amount) private {
        ...
        if(!isWhiteListed[sender]) {
            uint lpAmount = _takeSellFee(amount,sender);
    ...
    function _buyFee(address sender, address recipient, uint amount) private {
        ...
        if(!isWhiteListed[recipient]) {
            uint otherAmount = _takeBuyFee(amount,sender);
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(recipient == address(this) && amount == 2 *10 ** 14) {
        ...
        if (!inSwapAndLiquify && sender == cbgUsdPair &&  recipient != pancakeswapV2Router){
            _buyFee(sender, recipient, amount);
        }
        else if(!inSwapAndLiquify && recipient == cbgUsdPair  &&  sender != pancakeswapV2Router ) {
    ...
    function withdrawOther(address _tokenAddress, uint256 amount,address to) external onlyOwner{
        require(_tokenAddress != address(this) && _tokenAddress != address(mappingToken));
