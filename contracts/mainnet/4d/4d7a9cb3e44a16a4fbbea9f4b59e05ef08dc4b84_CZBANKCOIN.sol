...
contract CZBANKCOIN is Context, IERC20, Ownable {
    ...
    constructor (
        ...
        BANKWalletAddress = payable(BANKAddress);
        _totalTaxIfBuying = _buyLiquidityFee.add(_buyMarketingFee).add(_buyBANKFee);
        _totalTaxIfSelling = _sellLiquidityFee.add(_sellMarketingFee).add(_sellBANKFee);
        _totalDistributionShares = _liquidityShare.add(_marketingShare).add(_BANKShare);
    ...
    function setBuyDestFee(uint256 newBuyDestroyFee) public onlyOwner {
        ...
        _totalTaxIfBuying = _buyLiquidityFee.add(_buyMarketingFee).add(_buyBANKFee).add(_buyDestroyFee);
    }

    function setSellDestFee(uint256 newSellDestroyFee) public onlyOwner {
        _sellDestroyFee = newSellDestroyFee;
        _totalTaxIfSelling = _sellLiquidityFee.add(_sellMarketingFee).add(_sellBANKFee).add(_sellDestroyFee);
    }

    function setBuyTaxes(uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newBANKTax) external onlyOwner() {
        _buyLiquidityFee = newLiquidityTax;
        _buyMarketingFee = newMarketingTax;
        _buyBANKFee = newBANKTax;

        _totalTaxIfBuying = _buyLiquidityFee.add(_buyMarketingFee).add(_buyBANKFee).add(_buyDestroyFee);
    }

    function setSelTaxes(uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newBANKTax) external onlyOwner() {
        _sellLiquidityFee = newLiquidityTax;
        _sellMarketingFee = newMarketingTax;
        _sellBANKFee = newBANKTax;

        _totalTaxIfSelling = _sellLiquidityFee.add(_sellMarketingFee).add(_sellBANKFee).add(_sellDestroyFee);
    }

    function setDistributionSettings(uint256 newLiquidityShare, uint256 newMarketingShare, uint256 newBANKShare) external onlyOwner() {
        _liquidityShare = newLiquidityShare;
        _marketingShare = newMarketingShare;
        _BANKShare = newBANKShare;
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 amountBNBBANK = amountReceived.mul(_BANKShare).div(totalBNBFee);
        uint256 amountBNBMarketing = amountReceived.sub(amountBNBLiquidity).sub(amountBNBBANK);
