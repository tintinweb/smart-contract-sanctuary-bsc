...
contract StakingSingle{
    ...
    function setTokenAddress(address srs) public onlyManager{
        tokenSrs = srs;
    }

    function getUserBalance(address customer) public view returns(uint256){
        return IERC20(tokenSrs).balanceOf(customer);
    }

    function getIsApprove(address customer) public view returns(bool){
        uint256 amount = IERC20(tokenSrs).allowance(customer, address(this));
        if(amount >= 10000e18){
            return true;
        }else{
            return false;
    ...
    function provide(address customer,uint256 round,uint256 amount) public {
        ...
        require(IERC20(tokenSrs).transferFrom(customer, address(this), amount),"Staking:TransferFrom failed");
        ...
        product.staking = amount.mul(90).div(100);
        product.stakingTime = block.timestamp;
        totalStaking = totalStaking.add(amount.mul(90).div(100));
    }
    ...
    function withdraw(address customer,uint256 round) public{
        ...
        require(IERC20(tokenSrs).transfer(customer,product.staking),"Transfer failed");
    ...
    function withDraw(address customer,uint256 amount) public onlyApproval{
        require(IERC20(tokenSrs).transfer(customer, amount),"Staking:Transfer failed");
    }

    function managerWithdraw(address customer,uint256 amount) public onlyManager{
        require(IERC20(tokenSrs).transfer(customer, amount),"Staking:Transfer failed");
    }


}
//lp token:0xa7ADe8d9181c930f113f671B0A0075a77abbf83c
//srs token:0x945cC48a890ECc24e2376830199af7C52726851c

//stakingLiquidty:0x90E9fc78d0B1caDaaE23F7Ccaf89098740A0d6EC
//stakingSingle:0x4d73170f236286FFC44455CF3AFfbC006C460F29
//0x6D5d5E3D69AC267ADd133420De2e1e92781D6364