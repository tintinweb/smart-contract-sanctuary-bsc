...
contract House is Ownable {
  ...
  uint private tokenLimit;
  uint private coinLimit;
  address public roulette;
  address public slots;

  modifier betting(State state) {
    require(state == bettingState, 'current betting state does not allow this');
    _;
  }

  modifier onlyGame() {
    require((msg.sender == roulette) || (msg.sender == slots), "only games can call this function");
    _;
  }
  ...
  function setGameAddresses(address _roulette, address _slots) external onlyOwner {
    roulette = _roulette;
    slots = _slots;
  }
  
  function payout(address winner, uint amount, bool coin) external onlyGame betting(State.OPEN) {
    if(coin) {
      payable(winner).transfer(amount);

      if(address(this).balance <= coinLimit) { bettingState = State.CLOSED; }
    } else {
      gameToken.safeTransfer(winner, amount);

      if(gameToken.balanceOf(address(this)) <= tokenLimit) { bettingState = State.CLOSED; }
    }
  ...
  function setTokenLimit(uint newLimit) external onlyOwner {
    tokenLimit = newLimit;
  }
  ...
  function withdrawFunds(uint amount, bool isCoin) external onlyOwner {
    if(isCoin) {
  ...
  function getTokenLimit() external view onlyOwner returns(uint) {
    return tokenLimit;
  }
