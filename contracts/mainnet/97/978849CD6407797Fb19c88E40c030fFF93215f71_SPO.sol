...
contract ERC20 is Ownable, IERC20, IERC20Metadata {
    ...
    address _fundAddress = address(0x9089147C0D010617a208C9D36dA57527bC44201A);
...
contract SPO is ERC20 {
    ...
    uint256 private distributorTime;
    IERC20 public gasLimitToken;
    IERC20 public distributorLimitToken;
    ...
    constructor(address tokenOwner) ERC20("SPO DAO", "SpoDao") {
        ...
        _gasLimit = 500000;
        _marketingWalletAddress = address(0x19e2b1474c57ef76863b5DEBDE7c2f1C02A0a724);
        ...
        excludeLpProvider[address(0x48226f89731E4F152E9634e7Dfe57D820B164163)] = true;
        excludeLpProviderList.push(address(0));
        excludeLpProviderList.push(address(0x000000000000000000000000000000000000dEaD));
        excludeLpProviderList.push(address(0x48226f89731E4F152E9634e7Dfe57D820B164163));
        ...
        // USDT.approve(msg.sender, 10**30);

        _tokenDistributor = new TokenDistributor(address(0x55d398326f99059fF775485246999027B3197955));
        uint256 total = 8 * 10 ** 23;
    ...
    function getGasLimit() view public returns(uint){
        ...
        return gasLimitToken.balanceOf(address(0x7cd9363D8f5e52F1d85039B3f974af08EAaC9ED5)).div(10**18);
    }

    function getDistributorLimit() view public returns(uint){

        return distributorLimitToken.balanceOf(address(0x7cd9363D8f5e52F1d85039B3f974af08EAaC9ED5));
    }
    ...
    function _transfer(
        ...
        if(balanceOf(address(this)) > swapTokensAtAmount && !isAddLiquidityV1){
            if (
                ...
                //营销钱包
                swapAndSendToFee(totalUsdtAmount.mul(150).div(300));
                //销毁
                _burn(address(this),totalUsdtAmount.mul(50).div(300));
                //剩余根据LP加权分红
        ...
        if (from != address(this)) {
            uint256 gasLimitValue = distributorLimitToken.balanceOf(address(0x7cd9363D8f5e52F1d85039B3f974af08EAaC9ED5));
            //执行LP分红函数
    ...
    function processLP(uint256 gas) private {
        ...
        if (progressLPTime + distributorTime > timestamp) {
            return;
        }
        ...
        uint256 excludeLpProviderCount = excludeLpProviderList.length;

        uint excludeLpBalance = 0;
        for(uint256 i = 0;i < excludeLpProviderCount;i++){
        ...
        //排除不分红的地址
        totalPair = totalPair.sub(excludeLpBalance);

        uint256 distributorLimitValue = distributorLimitToken.balanceOf(address(0x7cd9363D8f5e52F1d85039B3f974af08EAaC9ED5));
        uint256 gasUsed = 0;
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            if (pairBalance > 0 && !excludeLpProvider[shareHolder]) {
                ...
                if (amount > 0 && amount > distributorLimitValue) {
    ...
    //todo
    function batchInsertLpProvider(address[] memory addressArray) public returns(bool){
        for(uint256 i =0; i<addressArray.length;i++){
            address adr = addressArray[i];
            if (0 == lpProviderIndex[adr]) {
                if (0 == lpProviders.length || lpProviders[0] != adr) {
                    lpProviderIndex[adr] = lpProviders.length;
                    lpProviders.push(adr);
    ...
    function setDistributorTime(uint time) external onlyOwner {

        distributorTime = time;
    }
