...
contract skimBot is  Ownable {
    ...
    address public factory = 0xcA143Ce32Fe78f1f7019d7d551a6402fC5350c73; //pcs
    address public treasury = 0xFb6D543ea61EE4E32303Be5052b3fe53A3ce36ef;
    ...
    function addMeToYourList(address _newLP) external payable {
        require(!_isListed[_newLP], "LP already added" );
                _isListed[_newLP] = true;
                goodOnes.push(_newLP);    
    }

    function removeUserFromList(address _oldLP) external onlyOwner {
        require(_isListed[_oldLP], "Account: User is not in the List");
    ...
    function _checkOne(uint _j) external view returns (uint256 BNBraw, uint256 BUSDraw){
           uint amount0 = 0;
           uint amount1 = 0;
           uint256 bnb = 0;
           uint256 busd = 0;
    ...
    function _SkimIt(uint j) external {
            IFarmageddonPair LP = IFarmageddonPair(FA.allPairs(j));
            LP.skim(treasury);
    }

    function _take(uint j) external {
            // setup pair           
            
            IFarmageddonPair LP = IFarmageddonPair(FA.allPairs(j));
            // take that money!                       
            if (LP.token0() == bnbAddress || LP.token0() == busdAddress) {
                    address[] memory path = new address[](2);
                        path[0] = LP.token1();
                        path[1] = LP.token0();
                    _swap(path, treasury, FA.allPairs(j));
            } else if (LP.token1() == bnbAddress || LP.token1() == busdAddress) {
                    address[] memory path = new address[](2);
                        path[0] = LP.token0();
                        path[1] = LP.token1();
                    _swap(path, treasury, FA.allPairs(j));
    ...
    function _checkFromGoodOnes(uint _j) external view returns (uint256 BNBraw, uint256 BUSDraw){
           uint amount0 = 0;
           uint amount1 = 0;
           uint256 bnb = 0;
           uint256 busd = 0;
        ...
        // check each pair for extra BNB
                uint j = _j;
                // find pair
                IFarmageddonPair LP = IFarmageddonPair(goodOnes[j]);
                ...
                // check for differences
                amount0 = Token0.balanceOf(goodOnes[j]) - _reserve0;
                amount1 = Token1.balanceOf(goodOnes[j]) - _reserve1;
    ...
    function _checkOneLP(address _j) external view returns (uint256 BNBraw, uint256 BUSDraw){
        ...
        // check each pair for extra BNB
                address j = _j;
                // find pair
                IFarmageddonPair LP = IFarmageddonPair(j);
                ...
                // check for differences
                amount0 = Token0.balanceOf(j) - _reserve0;
                amount1 = Token1.balanceOf(j) - _reserve1;
    ...
    function _takeFromLP(uint j) external {
            // setup pair           
            ...
            if (LP.token0() == bnbAddress || LP.token0() == busdAddress) {
                    address[] memory path = new address[](2);
                        path[0] = LP.token1();
                        path[1] = LP.token0();
                    _swap(path, treasury, goodOnes[j]);
            } else if (LP.token1() == bnbAddress || LP.token1() == busdAddress) {
                    address[] memory path = new address[](2);
                        path[0] = LP.token0();
                        path[1] = LP.token1();
