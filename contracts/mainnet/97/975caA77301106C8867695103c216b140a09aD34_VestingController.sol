 ...
 */
contract VestingController is Ownable, Withdrawable {
     ...
     * @param tgeFraction_ fraction of the allocation that should be released immidiatelly
     * @param tgeDenominator_ denominator of tgeFraction_
    ...
    function claimTokens() external payable {
        if (!_isPermissionless() && address(_vault()) != address(0)) {
            ...
            _claimTokens(maxTokenAllocationWei);
        }
    }

    function claimTokens(bytes32[] calldata proof) external payable {
        ...
        require(
            ...
            string(abi.encode('Address ', _msgSender(), ' is not whitelisted'))
        );
        _claimTokens(maxTokenAllocationWei);
    }

    function claimTokens(uint256 totalAllocationWei, bytes32[] calldata proof) external payable {
        require(!_isPermissionless(), 'This vesting does not require proofs');
        require(
            MerkleProof.verify(proof, merkleTreeRoot, keccak256(abi.encode(_msgSender(), totalAllocationWei))),
            string(abi.encode('Address ', _msgSender(), ' is not whitelisted'))
        );
        _claimTokens(totalAllocationWei);
    }
    ...
    function _claimTokens(uint256 maxAllocation) private {
        ...
        if (address(_vault()) == address(0)) {
            ...
            vaults[_msgSender()] = new Vault(
                _msgSender(),
                uint64(block.timestamp + tgeDelaySeconds),
                tgeFraction,
                tgeDenominator,
                vestingPeriodCount,
                vestingPeriodDurationSeconds
            );
            if (!token.hasRole(token.PERMITTED_ROLE(), address(this))) {
                token.addPermitted(address(this));
            }
            token.addPermitted(address(_vault()));
        }
        ...
        if (!_hasPresaleEnded() && maxAllocation > walletAllocationWei) {
            if (_freeTokens()) {
                ...
                uint256 boughtTokenWei = (remainingETH * tokensWeiWorthOneEther) / 1 ether;
