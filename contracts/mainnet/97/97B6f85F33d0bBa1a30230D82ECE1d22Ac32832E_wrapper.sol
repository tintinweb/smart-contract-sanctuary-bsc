...
contract wrapper is IWrapper, Auth {
    ...
    mapping (address => bool) public isContractDividendExempt;
    ...
    function setisContractDividendExempt(address holder, bool exempt) public authorized {
        isContractDividendExempt[holder] = exempt;
        if(exempt){processShare(holder, 0);}
        else{processShare(holder, IERC20(token).balanceOf(holder)); }
    }

    function setShare(address sender, uint256 sbalance, address recipient, uint256 rbalance) external override onlyToken {
        if(!isDividendExempt[sender] && (!isContract(sender) || isContractDividendExempt[sender])){processShare(sender, sbalance);}
        if(!isDividendExempt[recipient] && (!isContract(recipient) || isContractDividendExempt[recipient])){processShare(recipient, rbalance); }
    }
