...
contract Cusd is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 96h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 13740 ether;
    uint256 public constant AIRDROP_DISTRIBUTION = 3000 ether;
    ...
    uint16 public antiBotTimeDelayed = 3;
    ...
    mapping (address => uint256) public lastSoldTime;
    ...
    /**
     * @notice Constructs the CUSD BEP-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Cex USD", "CUSD") {
        // Mints 1 CUSD to contract creator for initial pool setup
    ...
    function _getCusdPrice() internal view returns (uint256 _cusdPrice) {
        try IOracle(cusdOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Cusd: failed to fetch CUSD price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _cusdPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_cusdPrice >= taxTiersTwaps[tierId]) {
    ...
    function setCusdOracle(address _cusdOracle) public onlyOperatorOrTaxOffice {
        require(_cusdOracle != address(0), "oracle address cannot be 0 address");
        cusdOracle = _cusdOracle;
    }
    ...
    /**
     * @notice Operator mints CUSD to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of CUSD to mint to
    ...
    function setBlackList(address _address, bool _boolean) public onlyOwner {
        require(_address != address(0), "zero");
        blackList[_address] = _boolean;
    }
    ...
    function transferFrom(
        ...
        require(!blackList[sender] && !blackList[recipient], "Blocked address");
       
        if(!whiteList[sender]){
          require(lastSoldTime[sender] + antiBotTimeDelayed < block.timestamp, "Error");
          lastSoldTime[sender] = block.timestamp;
        }
        if(sender != recipient && !whiteList[recipient]){
            require(lastSoldTime[recipient] + antiBotTimeDelayed < block.timestamp, "Error");
            lastSoldTime[recipient] = block.timestamp;
        }

        if (autoCalculateTax) {
            uint256 currentCusdPrice = _getCusdPrice();
            currentTaxRate = _updateTaxRate(currentCusdPrice);
            if (currentCusdPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function transfer( address recipient, uint256 amount) public override returns (bool){
        require(!blackList[msg.sender] && !blackList[recipient], "Blocked address");
        if(!whiteList[msg.sender]) {
            require(lastSoldTime[msg.sender] + antiBotTimeDelayed < block.timestamp, "Error");
            lastSoldTime[msg.sender] = block.timestamp;
        }

        if(msg.sender != recipient && !whiteList[recipient]) {
            require(lastSoldTime[recipient] + antiBotTimeDelayed < block.timestamp, "Error");
            lastSoldTime[recipient] = block.timestamp;
        }
    ...
    function setAntibotTimeDelayed(uint16 _antiBotTime) public onlyOwner {
        antiBotTimeDelayed = _antiBotTime;
    }
    ...
    function setWhiteList(address _address, bool _boolean) public onlyOwner {
        require(_address != address(0), "zero");
        whiteList[_address] = _boolean;
    }
    /**
    ...
    function distributeReward(
        ...
        require(_airdrop != address(0), "!_genesisPool");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_airdrop, AIRDROP_DISTRIBUTION);
   }
