...
contract Bitchemical is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("Bitchemical", "BCHEC") {

    	marketingWallet = 0x1F8D41dF72F183b745356FeE61F930DEeB1F53e8; // marketing wallet
    	rewardsWallet = 0x83aF9cd324b20D767179F47F00e59E83d1eaaC99; // rewards wallet
    ...
    function updateFees(uint8 _buyBuyBackFee, uint8 _buyMarketingFee, uint8 _buyRewardsFee, uint8 _sellBuyBackFee, uint8 _sellMarketingFee, uint8 _sellRewardsFee) external onlyOwner {
        require(_buyBuyBackFee + _buyMarketingFee + _buyRewardsFee <= 45, "buy fee must be less than 45%"); // on buy marketing fees goes to marketing address you can change 45 to whatever percent you want.
            ...
            if (!swapping && canSwap) {
                ...
                if (marketingEnabled) {
                    if(block.timestamp < _firstBlock + (1 days)) {
                        ...
                        address payable addr = payable(0x88Ff11649889769Bc40C299027a6a653c42f2cC2); // dev fee lasts for one day only
