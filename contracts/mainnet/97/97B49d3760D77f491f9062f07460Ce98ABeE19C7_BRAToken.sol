...
contract BEP20 is Context, Ownable, IBEP20 {
    ...
    address internal constant A = 0x8fdd463A9bBa1cf78Bb6A2Ae07094B19331CC8E4;
    address internal constant B = 0x571c395298029FDb74A69870343710CCAE0e5585;
...
contract BRAToken is BEP20Detailed {
    ...
    constructor(address _Lucy, address ExtenAddress) BEP20Detailed("BRA2", "BRA2", 18) {
        Lucy = _Lucy;
        ...
        lpInterest.minAward = 10**18;
        lpInterest.period = 1;
        lpInterest.sendCount = 50;

        lpAwardCondition = LpAwardCondition(20*10**18,20*10**18);
    }
    ...
    function _transfer(
        ...
        if(_balances[sender] < (amount + Min)){
        ...
        if (recipient == uniswapV2Pair && !senderAllowSell && !isAddLiquidity) {
    ...
    function checkLpAwardCondition(address owner,bool isAddLiquidity,bool isDelLiquidity ,uint256 amount ) internal view returns (bool) {
        ...
        if( token1 == address(this) && balanceOf(owner) >= lpAwardCondition.balHoldAmount && supply > 0  &&  r1 > 0){
            ...
            uint lpHoldAmount_ = amount/r1*r0;
            if(isAddLiquidity){
