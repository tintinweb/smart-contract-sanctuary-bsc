...
contract IDO is Ownable {
  ...
  uint256 public constant CLAIM_RATIO = 1;
  ...
  function buy(
    ...
    if (
      ...
      uplineToBeSet != address(0)
    ) {
      IDOUsers[uplineToBeSet].downlines.push(msg.sender);
      IDOUsers[msg.sender].uplines[0] = uplineToBeSet;

      for (uint i = 0; i < 4; i++) {
        if (IDOUsers[uplineToBeSet].uplines[i] == address(0)) break;

        IDOUsers[msg.sender].uplines[i + 1] = IDOUsers[uplineToBeSet].uplines[
          i
        ];
      }
    }
