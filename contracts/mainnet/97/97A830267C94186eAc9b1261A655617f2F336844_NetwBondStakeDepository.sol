...
contract NetwBondStakeDepository is Ownable {
    ...
    address public immutable NETW; // intermediate reward token from treasury
    address public immutable sNETW; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints NETW when receives principle
    ...
    struct Bond {
        uint gonsPayout; // sNETW gons remaining to be paid
        uint netwPayout; // netw amount at the moment of bond
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 NETW ( underflow protection )
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) NETW
         */
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            IERC20( NETW ).safeTransfer( DAO, fee ); 
        }
        ...
        IERC20( NETW ).approve( staking, payout );
        IStaking( staking ).stake( payout, address(this) );
        
        uint stakeGons=ISNETW(sNETW).gonsForBalance(payout);

        // depositor info is stored
        _bondInfo[ _depositor ] = Bond({ 
            gonsPayout: _bondInfo[ _depositor ].gonsPayout.add( stakeGons ),
            netwPayout: _bondInfo[ _depositor ].netwPayout.add( payout ),
    ...
    function redeem( address _recipient, bool _stake) external returns ( uint ) {        
        ...
        uint _amount = ISNETW(sNETW).balanceForGons(info.gonsPayout);
        emit BondRedeemed( _recipient, _amount, 0 ); // emit bond data
        IERC20( sNETW ).transfer( _recipient, _amount ); // pay user everything due
        return _amount;
    }
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( NETW ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    /**
     *  @notice return bond info with latest sNETW balance calculated from gons
     *  @param _depositor address
     *  @return payout uint
     *  @return vesting uint
    ...
    function bondInfo(address _depositor) public view returns ( uint payout,uint vesting,uint lastTime,uint pricePaid ) {
        ...
        payout=ISNETW(sNETW).balanceForGons(info.gonsPayout);
    ...
    /**
     *  @notice calculate current ratio of debt to NETW supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = IERC20( NETW ).totalSupply();
    ...
    /**
     *  @notice calculate amount of NETW available for claim by depositor
     *  @param _depositor address
     *  @return pendingPayout_ uint
     */
    function pendingPayoutFor( address _depositor ) external view returns ( uint pendingPayout_ ) {
        uint percentVested = percentVestedFor( _depositor );
        uint payout = ISNETW(sNETW).balanceForGons(_bondInfo[ _depositor ].gonsPayout);
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or NETW) to the DAO
     *  @return bool
     */
    function recoverLostToken( address _token ) external returns ( bool ) {
        require( _token != NETW );
        require( _token != sNETW );
