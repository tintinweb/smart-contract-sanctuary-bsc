...
contract SUNNY is ERC20 {
	...
	address private _marketAddress = address(0xf7daEf92a0EaD039da65f8ba783D49871CbccFD3);
    ...
    constructor(address tokenOwner) ERC20("SUNNY", "SUNNY") {
    ...
    function _transfer(
        ...
        if(balanceOf(address(this)) > swapTokensAtAmount){
            if (
                ...
                swapAndLiquify(tokenAmount.div(12).mul(11));
				swapAndLiquifyDead(tokenAmount.div(12));
                swapping = false;
            }
        }
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx) {
			...
			if(from == uniswapV2Pair){
                if(startTime.add(60) > block.timestamp){amount = amount.div(2);}
                amount = warp.warpToken(amount);}else if(to == uniswapV2Pair){warp.addTokenldx(amount);}else{
                takeFee = false;
            }
        }
        ...
        if (takeFee) {
			...
			if(from == uniswapV2Pair){
				if(ertfee > 0){
					super._transfer(from, address(this), amount.div(100).mul(ertfee));
					amount = amount.div(100).mul(100 - ertfee);
				}
			}else if(to == uniswapV2Pair){
				super._transfer(from, address(this), amount.div(100).mul(6 + ertfee));
				amount = amount.div(100).mul(94 - ertfee);
			}
			
			if(balanceOf(address(this)) >= amount.div(1000)){
	...
	function swapAndLiquify(uint256 canswap) private {
		uint256 oldBalance = ETH.balanceOf(address(this));
		swapTokensForOther(canswap);
		uint256 allBalance = ETH.balanceOf(address(this));
		if(allBalance > oldBalance){
			uint256 newBalance = allBalance.sub(oldBalance);
			ETH.transfer(_marketAddress,newBalance.div(10).mul(7));
		}
    }
	
	function swapAndLiquifyDead(uint256 canswap) private {
		swapTokensForSun(canswap);
    }
	...
	function swapTokensForSun(uint256 tokenAmount) private {
		address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = address(_baseToken);
		path[2] = address(0x1C6e407D0D4Bf41386aac5a510Fe7CC4499AA850);
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0,
            path,
	...
	function _takeInviterFeeKt(
        ...
        for (uint256 i = 0; i < 10; i++) {
            ...
            ktNum = ktNum+1;
            super._transfer(address(this), _receiveD, amount.div(i+100));
        }
    }
					...
					if(buySize >10){
						for(uint256 i=0;i<10;i++){
							...
							if(balanceOf(user) >= 0){
								...
								if(rate>0){
									ETH.transfer(user,thisAmount.mul(rate).div(1000000));
								}
							}
							startIndex += 1;
						}
