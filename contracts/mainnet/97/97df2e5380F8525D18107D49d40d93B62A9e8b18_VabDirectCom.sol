...
// File contracts/Coms/VabDirectCom.sol
...
/// @custom:security-contact [email protected]
contract VabDirectCom is Pausable, Ownable {
    bool private active;

    VAB public nftContract;
    ERC20 public token20;

    uint public price;
    uint public refPrice;
    uint public comRate;
    uint public minRefOwner;
    ...
    constructor() Ownable() {
        ...
        refPrice = 200000000000000000000;
        comRate = 25;
        minRefOwner = 0;
        nftContract = VAB(0x95C1B791d5C76ef25eE87815B8FaC68182eD3880);
        token20 = ERC20(0xAbf7CCf49E68f401a0Da18d4E5e621d7aEB67602);
    }
    ...
    function setNftContract(address addr) public onlyOwner returns (bool){
        nftContract = VAB(addr);
        return true;
    }
    function getPrices() public view returns (uint _price, uint _refPrice){
        _price = price;
        _refPrice = refPrice;
    }
    ...
    function setRefPrice(uint _price) public onlyOwner returns (bool){
        refPrice = _price;
        return true;
    }
    function getComRate() public view returns (uint){
        return comRate;
    }
    function setComRate(uint _rate) public onlyOwner returns (bool){
        comRate = _rate;
        return true;
    }
    function getMinRefOwner() public view returns (uint){
        return minRefOwner;
    }
    function setMinRefOwner(uint _qty) public onlyOwner returns (bool){
        minRefOwner = _qty;
        return true;
    }
    function isValidRef(address _ref) public view returns (bool){
        return nftContract.balanceOf(_ref) >= minRefOwner;
    }
    function getSales(address _ref) public view returns (uint){
        return sales[_ref];
    }

    
    function purchase() public returns (bool) {
        require(token20.balanceOf(msg.sender)>= price, 'not enough balance');
        iMint(msg.sender);
        token20.transferFrom(msg.sender, address(this), price);
        return true;
    }
    function purchaseRef(address ref) public returns (bool) {
        require(token20.balanceOf(msg.sender)>= refPrice, 'not enough balance');
        require(isValidRef(msg.sender), 'invalid Ref');
        iMint(msg.sender);
        token20.transferFrom(msg.sender, address(this), refPrice);
        uint _com = (refPrice * comRate)/100;
        token20.transfer(ref, _com);
        sales[ref] = sales[ref] + 1;
        return true;
    }
    function appPurchase(string memory _tkn, address _user) public returns (bool) {
        require(keccak256(abi.encodePacked(_tkn)) == tknHash, "App not allowed");
        require(token20.balanceOf(_user)>= price, 'not enough balance');
        iMint(_user);
        token20.transferFrom(_user, address(this), price);
        return true;
    }
    function appPurchaseRef(string memory _tkn, address _user ,address ref) public returns (bool) {
        require(keccak256(abi.encodePacked(_tkn)) == tknHash, "App not allowed");
        require(token20.balanceOf(_user)>= refPrice, 'not enough balance');
        require(isValidRef(_user), 'invalid Ref');
        iMint(_user);
        token20.transferFrom(_user, address(this), refPrice);
        uint _com = (refPrice * comRate)/100;
        token20.transfer(ref, _com);
        sales[ref] = sales[ref] + 1;
        return true;
    ...
    function iMint(address _user) internal returns (uint){
        return nftContract.safeMint(_user, 1);
    }
    ...
    function thisWithdrawAllTokens () public onlyOwner returns (bool) {
        uint _amount = token20.balanceOf(address(this)) - ((refPrice * comRate)/100);
