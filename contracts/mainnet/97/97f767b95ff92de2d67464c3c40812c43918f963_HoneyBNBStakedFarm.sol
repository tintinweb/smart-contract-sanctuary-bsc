    ...
    PausableUpgradeable
{
    ...
    function stakeFromEth(
        ...
        IERC20Upgradeable(LPToken.token0()).transfer(
            msg.sender,
            unusedToken0
        );

        // Send back unused Token 0
        IERC20Upgradeable(LPToken.token1()).transfer(
            msg.sender,
            unusedToken1
        );
    ...
    function unstakeToEth(
        ...
        uint256 ethAmount = DEX.convertPairLpToEth(
            address(LPToken),
            amount
        );
    ...
    function claimRewards() public whenNotPaused {
        ...
        participantData[msg.sender].masterchefRewardMask = masterchefRoundMask;

        if (rewardTokensToTransfer > 0) {
            participantData[msg.sender]
                .claimedRewardTokens += rewardTokensToTransfer;

            IERC20Upgradeable(address(RewardToken)).safeTransfer(
                msg.sender,
                rewardTokensToTransfer
            );
            emit ClaimRewardTokens(msg.sender, rewardTokensToTransfer);
        }
    }
