...
contract StakingV2 is OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable  {

    IERC20Upgradeable public usdt;
    IUniswapV2Router public router;
    IWETH public weth;
    uint8 public stakeFeePercent; //initialized to 5
    uint8 public withdrawFeePercent; //initialized to 5
    uint8 public maxRate5; //initialized to 5
    uint8 public maxRate10; //initialized to 10
    ...
    */
    ...
    function initialize(address _usdt, address _router, address _weth) initializer public {
        __Ownable_init();
        __Pausable_init();
        usdt = IERC20Upgradeable(_usdt);
        router = IUniswapV2Router(_router);
    ...
    function setLevelStakeAmount(uint8 level, uint _stakeAmount) public onlyOwner {
        require(level >=0 && level <= 10, "Level should be between 0 and 10");
        stakeAmounts[level] = _stakeAmount;
    }
    ...
    function changeWithdrawFeePercent(uint8 _newFee) public onlyOwner {
        require(_newFee >= 1 && _newFee <=99, "withdraw fee percent should be between 1 and 99 percent");
        withdrawFeePercent = _newFee;
    }

    function changeStakeFeePercent(uint8 _newFee) public onlyOwner {
        require(_newFee >= 1 && _newFee <=99, "stake fee percent should be between 1 and 99 percent");
        stakeFeePercent = _newFee;
    }
    ...
    function stakeUsdt(address _ref, uint rank0stakeAmount) public whenNotPaused nonReentrant {
        address owner = owner();
        require(inputToken == TokenType.usdt, "You should stake with ether");
        require(_ref != address(0), "you should have referral address");
        require(_ref != _msgSender() || _ref == owner, "You cant put your address as inviter !");
        require(stakers[_ref].parent != address(0) || _ref == owner, "The referal address is not a staker in our app");
        uint8 rank = stakers[_msgSender()].rank;
        uint stakeAmount = rank == 0 ? rank0stakeAmount : stakeAmounts[rank];
        require(stakeAble(_msgSender()), "You can not stake again until your rank be increased" );
        if(rank == 0){
            require(stakeAmount >= stakeAmounts[0] && stakeAmount <= stakeAmounts[1], "For first rank stake amount should be between stakeAmounts[0] and stakeAmount[1]");
        }
        //check that user sent enough token
        uint fee;
        
        fee = stakeFeePercent * stakeAmount / 100;
       
        uint requireAmount = stakeAmount + fee;
        SafeERC20Upgradeable.safeTransferFrom(usdt, _msgSender(), address(this), requireAmount);
        
        //transfer owner the share and fee
        uint adminShare = 9 * stakeAmount / 100;
        uint ownerShare = adminShare + fee;
        SafeERC20Upgradeable.safeTransfer(usdt, owner, ownerShare);

        _addToUserList(_msgSender());

        //declare the parents
        address[] memory parents = getParents(_msgSender(), _ref);
        stakers[_msgSender()].parent = parents[0];
        
        _giveParentsReward(parents, stakeAmount);
        //completing user data
        if(rank == 0 && rank0stakeAmount == stakeAmounts[1]){
            stakers[_msgSender()].rank = 1;
        }
        stakers[_msgSender()].active = true;
        stakers[_msgSender()].earn = 0;
        stakers[_msgSender()].contractEndTime = block.timestamp + 1080 days;
    }
    

    function stakeEth(address _ref, uint rank0stakeAmount) public payable whenNotPaused nonReentrant {
        address owner = owner();
        require(inputToken == TokenType.eth, "You should stake with eth");
        require(_ref != address(0), "you should have referral address");
        require(_ref != _msgSender() || _ref == owner, "You can not put your address as inviter !");
        require(stakers[_ref].parent != address(0) || _ref == owner , "The referal address is not a staker in our app");
        uint8 rank = stakers[_msgSender()].rank;
        uint stakeAmount = rank == 0 ? rank0stakeAmount : stakeAmounts[rank];
        require(stakeAble(_msgSender()), "You can not stake again until your rank be increased" );
        ...
        //check that user sent enough token
        uint fee = stakeFeePercent * stakeAmount / 100;
    ...
    function withdrawRewardsUsdt(uint256 _amount) public nonReentrant {
        address owner = owner();
        require(outputToken == TokenType.usdt, "You should withdraw your tokens using ether");
        require(pendingRewards[_msgSender()] >= 0, "You dont have any reward to withdraw");
        require(pendingRewards[_msgSender()] >= _amount, "You dont enough reward to withdraw");
        require(stakers[_msgSender()].freezed == false, "You can not withdraw because you are freezed");
        uint adminFee = withdrawFeePercent*_amount/100;
        uint userReward = _amount - adminFee;
        pendingRewards[_msgSender()] -= _amount;
        totalPending -= _amount;
        SafeERC20Upgradeable.safeTransfer(usdt, owner, adminFee);
        SafeERC20Upgradeable.safeTransfer(usdt, _msgSender(), userReward);
    }


    function withdrawRewardsEther(uint256 _usdtAmount) public nonReentrant {
        address owner = owner();
        ...
        path[0] = address(usdt);
        path[1] = address(weth);
        require(pendingRewards[_msgSender()] >= _usdtAmount, "You dont enough reward to withdraw");
        uint ethAmountsOut = getAmountEthOut(_usdtAmount);
        uint adminFee = withdrawFeePercent*ethAmountsOut/100;
        uint userReward = ethAmountsOut - adminFee;
        pendingRewards[_msgSender()] -= _usdtAmount;
        ...
        payable(_msgSender()).transfer(userReward);
    }
    ...
    function withdrawAdmin_usdt(uint _amount) public onlyOwner {
        ...
        SafeERC20Upgradeable.safeTransfer(usdt, _msgSender(), _amount);
    }
