...
contract CeocoinIdo is Modifier, Util {
    ...
    constructor() {
        poundage = 50;
        normalBuyPrice = 2000000000000000;
        totalNormalNumber = toWei(100000000, 18);
    ...
    function setPoundage(uint256 _poundage) public onlyOwner {
        poundage = _poundage;
    }
    ...
    function buyNormal(uint256 amountToWei, address _address) public isRunning nonReentrant returns (bool) {
        ...
        uint256 actualAmount = amountToWei.sub(amountToWei.mul(poundage).div(1000));
        uint256 buyNumbers = toWei(actualAmount, 18).div(normalBuyPrice);
