...
contract GHOSTINU is IERC20, Context, Ownable {
    ...
    address private XGhost__Inu____Wallet = 0x60772Eb0A73a09dD14210733Af1a6993779c30ce;
    address private XGhost__Inu___Team_Wallet = 0x60772Eb0A73a09dD14210733Af1a6993779c30ce;
    
    string private constant _symbol = "GHOST";
    string private constant _name = "Ghost Inu";

    uint256 public XGhost__Inu___max_WalletLimit = 2000000  * 10**9;
    uint256 public swapTokensAtAmount = 300000 * 10**9;

    uint256 public XGhost__Inu___max_BuyLimit = 2000000 * 10**9;
    uint256 public XGhost__Inu___max_SellLimit = 1000000 * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[XGhost__Inu____Wallet] = true;
        _isExcludedFromFee[XGhost__Inu___Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[XGhost__Inu____Wallet] = true;
        allowedTransfer[XGhost__Inu___Team_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= XGhost__Inu___max_BuyLimit, "You are exceeding XGhost__Inu___max_BuyLimit");
            require(balanceOf(to) + amount <= XGhost__Inu___max_WalletLimit, "You are exceeding XGhost__Inu___max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= XGhost__Inu___max_SellLimit, "You are exceeding XGhost__Inu___max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= XGhost__Inu___max_WalletLimit, "You are exceeding XGhost__Inu___max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(XGhost__Inu____Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(XGhost__Inu___Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function XGhost__Inu___Update_MarketingWallet(address newWallet) external onlyOwner{
        XGhost__Inu____Wallet = newWallet;
    }
    
    function XGhost__Inu___Update_TeamWallet(address newWallet) external onlyOwner{
        XGhost__Inu___Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        XGhost__Inu___max_BuyLimit = maxBuy * 10**decimals();
        XGhost__Inu___max_SellLimit = maxSell * 10**decimals();
    }
    
    function XGhost__Inu___Max_Walletlimit(uint256 amount) external onlyOwner{
        XGhost__Inu___max_WalletLimit = amount * 10**decimals();
    }
