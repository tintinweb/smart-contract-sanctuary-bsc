...
contract token is Context, IERC20, Ownable {
    ...
    string private _name = '无名币';
    string private _symbol = 'WMB';
    uint8 private _decimals = 18;
    uint256 private _totalSupply = 1999999 * 10 **18;
    ...
    address public _ownerAddress = address(0xcF8197AE41cbF2b542cf142Dc363bf2e89b2daaa);
    ...
    function _transfer(address from, address to, uint256 value) internal {
        ...
        if (from == uniswapV2Pair || to == uniswapV2Pair) { // buy or sell
            uint256 lpFeeNumber = value.mul(lpFee).div(100);
            uint256 burnFeeNumber = value.mul(burnFee).div(100);
            uint256 newValue = value.sub(lpFeeNumber).sub(burnFeeNumber);
            _balances[to] = _balances[to].add(newValue);
            _balances[address(this)] = _balances[address(this)].add(lpFeeNumber);
            _balances[_burnAddress] = _balances[_burnAddress].add(burnFeeNumber);
            emit Transfer(from,to, newValue);
            emit Transfer(from,address(this), lpFeeNumber);
            emit Transfer(from,_burnAddress, burnFeeNumber);
        } else {
    ...
    uint256 public holderRewardCondition = 100 * 10 ** 18;
    ...
    function processReward(uint256 gas) private {
        ...
        IERC20 usdt = IERC20(address(this));
