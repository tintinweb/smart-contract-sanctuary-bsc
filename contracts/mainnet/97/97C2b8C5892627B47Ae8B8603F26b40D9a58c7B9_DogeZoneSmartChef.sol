...
contract DogeZoneSmartChef is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    address internal _delegator;
    ...
    function updateLockTime(uint256 _newLockTime) external onlyOwner {
        lockTime = _newLockTime;
    }

    function detail() external view returns(uint256,uint256,uint256,uint256,uint256,uint256,address,address){
        return (startBlock,endBlock,poolLimitPerUser,rewardPerBlock,stakedTokenSupply,lockTime,address(stakedToken),address(rewardToken));
    }
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            uint256 timeOffset = block.timestamp.sub(user.lastDepositTime);
            require(timeOffset >= lockTime, "TIME_LOCK");
