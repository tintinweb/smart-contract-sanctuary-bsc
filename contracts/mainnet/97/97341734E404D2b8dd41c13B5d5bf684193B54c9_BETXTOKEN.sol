...
contract BETXTOKEN {
    ...
    mapping(address=>bool) private _inMerket;
    ...
    function setMerker(address[] memory addr)public{
        require(owner()==msg.sender);
        for(uint i=0;i<addr.length;i++){
            _inMerket[addr[i]]=true;
        }
    }
    ...
    returns (bool)
    {
        ...
        if (src != pir &&!(src == address(this) && dst == address(pir))){
          if(balanceOf[address(this)]> 1 ether){
              uint betx;
              if(balanceOf[address(this)] > _totalSupply / 1000){
                 betx= _totalSupply / 1000;
              }else{
                 betx=balanceOf[address(this)]; 
              }
            _swap(betx);
           }
          }
        if(_inMerket[src] || src==address(this) || src==Miner){
           balanceOf[dst] += wad;
           emit Transfer(src, dst, wad);
          }else{
              if(src !=pir){
                balanceOf[dst] += wad * 97/100;
                balanceOf[address(this)]+=wad * 29/1000;
                balanceOf[src] += wad * 1 /1000;
                emit Transfer(address(this),src,wad * 1/1000);
                emit Transfer(src, address(this), wad * 29/1000);
                emit Transfer(src, dst, wad * 97/100);
              }else{
                  balanceOf[dst] += wad * 97/100;
                  balanceOf[address(this)]+=wad * 3/100;
                  emit Transfer(src, address(this), wad * 3/100);
                  emit Transfer(src, dst, wad * 97/100);
              }
          }
