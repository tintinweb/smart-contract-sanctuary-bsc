...
contract MetaSocialityStaking is Ownable {   
   ...
   function withdrawalBnb(address payable _to, uint _amount) external{
    ...
    function buyTokenWithBUSD(uint busdAmount) external {
            ...
            BEP20 busdToken = BEP20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);///Mainnet
            ...
            require(busdToken.balanceOf(msg.sender) >= busdAmount, "Insufficient user balance");
            busdToken.transferFrom(msg.sender, contractAddr, busdAmount);
    ...
    function tokenInBUSD(uint busdAmount) public view returns (uint) {
        uint tokenVal = (busdAmount* 10**tokenPriceDecimal) / tokenPrice ; 
        return (tokenVal);
    }
}