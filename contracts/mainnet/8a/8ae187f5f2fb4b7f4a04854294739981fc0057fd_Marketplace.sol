...
import "../token/LegendVerseICO.sol";
    ...
    MarketplaceStorage
{
    ...
    LegendVerseICO public icoContract;
    ...
    function setIcoContract(address _icoContract) external onlyOwner {
        icoContract = LegendVerseICO(_icoContract);
    }
    ...
    function createItem(
        ...
        require(sellerAddress == assetOwner, "Only the owner can create item");
        require(
        ...
        emit ItemCreated(itemId, assetId, assetOwner, nftAddress, price);
    }
    ...
    function buyItem(
        ...
        require(price >= item.price, "value need equal price");
        require(
            seller == nftRegistry.ownerOf(assetId),
            "The seller is no longer the owner"
        );
        uint256 lockedBalance = icoContract.getLockedBalance(_msgSender());
        if (lockedBalance >= price) {
            icoContract.decreaseLockedBalance(_msgSender(), price);
        } else {
            icoContract.decreaseLockedBalance(_msgSender(), lockedBalance);
            uint256 remainAmount = price - lockedBalance;
            currency.transferFrom(_msgSender(), seller, remainAmount);
        }
    ...
    function sellItem(
        ...
        emit SellItemSuccessful(nftAddress, item.id, assetId, price, seller);
    }
    ...
    function delistItem(address nftAddress, uint256 assetId) public {
        ...
        emit DelistItemSuccessful(nftAddress, item.id, assetId, deleteBy);
    }
    ...
    function offerItem(
        ...
        _requireERC721(nftAddress);
        currency.transferFrom(_msgSender(), address(this), offerPrice);
    ...
    function cancleOfferItem(address nftAddress, uint256 assetId) public {
        ...
        currency.transfer(_msgSender(), offerPrice);
    ...
    function approveOfferItem(
        ...
        ItemOffer memory offerData = itemOffers[nftAddress][assetId][offerer];
        uint256 offerPrice = offerData.offerPrice;
        currency.transferFrom(address(this), seller, offerPrice);
...
import "./ILegendVerseICO.sol";

contract LegendVerseICO is Initializable, ILegendVerseICO, ContextUpgradeable, OwnableUpgradeable {
    using SafeMathUpgradeable for uint256;

    using SafeERC20Upgradeable for IERC20Upgradeable;
    IERC20Upgradeable public tokenBUSD;
    IERC20Upgradeable public tokenLegendVerse;
    ...
    function initialize(
        ...
        address _tokenLegendVerseContract,
        ...
        require(
            _tokenLegendVerseContract != address(0),
            "invalid contract address"
        );
        ...
        tokenLegendVerse = IERC20Upgradeable(_tokenLegendVerseContract);
    ...
    modifier onlyAdmin() {
        require(msg.sender == adminAddress, "not Admin contract");
        _;
    }

    modifier onlyMarketpace() {
        require(msg.sender == marketplaceAddress, "not Marketplace contract");
        _;
    }
    ...
    function setAdmin(address _adminAddress) external onlyAdmin {
        ...
        emit SetAdmin(oldAdmin, _adminAddress);
    }

    function setMarketAddress(address _address) external onlyAdmin {
    ...
    function buyByBUSD(uint256 amount) external {
        ...
        lockedBalance[sender] += buyAmountToken;
    ...
    function buyByUSDT(uint256 amount) external {
        _checkBlackList(_msgSender());
        _precheckBuy(amount);
        uint256 buyAmountToken = amount.mul(totalAmountPerBUSD);
        address sender = _msgSender();
    ...
    function buyByBNB() external payable {
        _checkBlackList(_msgSender());
    ...
    function claimBUSD() external onlyAdmin {
        ...
        tokenBUSD.transfer(_msgSender(), remainAmountToken);
        emit Claim(_msgSender(), remainAmountToken);
    }

    function claimUSDT() external onlyAdmin {
        uint256 remainAmountToken = tokenUSDT.balanceOf(address(this));
        tokenUSDT.transfer(_msgSender(), remainAmountToken);
        emit Claim(_msgSender(), remainAmountToken);
    }

    function claimToken() external onlyAdmin {
        uint256 remainAmountToken = tokenLegendVerse.balanceOf(address(this));
        tokenLegendVerse.safeTransfer(_msgSender(), remainAmountToken);
        emit Claim(_msgSender(), remainAmountToken);
    }

    function setContractToken(address contractToken) external onlyOwner {
        tokenLegendVerse = IERC20Upgradeable(contractToken);
    }

    function setUSDTContractToken(address contractToken) external onlyOwner {
        require(contractToken != address(0), "invalid USDT contract");
        tokenUSDT = IERC20Upgradeable(contractToken);
    }
    ...
    function setRoundInfo(
        ...
        require(_totalAmountPerBUSD > 0, "invalid rate BUSD");
    ...
    function getLockedBalance(address _address) public view virtual override returns (uint256) {
        return lockedBalance[_address];
    }

    function decreaseLockedBalance(address _address, uint256 _amount) public virtual onlyMarketpace override returns (bool) {
         uint256 currentBalance = lockedBalance[_address];
        require(currentBalance >= _amount, "decreased locked balance below zero");
        lockedBalance[_address] -= _amount;

        emit ChangeLockedBalance(_msgSender(), _address, _amount);
        return true;
    }
}
...
interface ILegendVerseICO {

    function getLockedBalance(address _address) external view returns (uint256);

    function decreaseLockedBalance(address _address, uint256 _amount) external returns (bool);

    // EVENTS
    event BuyEvent(
        uint256 priceInBUSD,
        uint256 amountInputToken,
        uint256 amountOuputToken
    );

    event SetAdmin(address indexed old, address indexed admin);

    event Claim(address indexed receiver, uint256 value);

    event ChangeLockedBalance(address indexed sender, address indexed account, uint256 value);
}
