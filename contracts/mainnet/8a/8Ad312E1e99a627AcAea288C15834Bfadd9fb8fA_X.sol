...
contract X is BEP20 { 
	...
	function toggleExchangesList(address account) external onlyOperator returns (bool) {
	...
	function toggleBlackList(address account) external onlyOperator returns (bool) {
		blackList[account] = !blackList[account];	
		return blackList[account];
	}

	function toggleSwapAndLiquifyEnabled() external onlyOperator returns (bool) {
