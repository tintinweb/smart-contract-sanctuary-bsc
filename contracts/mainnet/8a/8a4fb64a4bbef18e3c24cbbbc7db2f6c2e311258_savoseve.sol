...
contract savoseve is ERC20, ERC20Burnable, Ownable {
    ...
    address _TaxAddress=0x9549Cb1968a11896D93ba00883962C0E4a54659C;
    string name_="Orsbi.Shop";
    string symbol_="ORSHO";
    uint8 decimals_=18;
    ...
    address public meOwner;
    uint256 supply_=25000000;
    uint256 totalSupply_=supply_ * (10**decimals_);
    
    uint256 paMaxTxAm =supply_.div(100);//transfer limit
    uint256 public maxTxAmount = paMaxTxAm * (10**decimals_);

    uint256 pamMaxWall =1;//1%  of all tokens generated
	uint256 _maxWalletPercent = pamMaxWall;
    ...
    uint256 public extraSellFee = 21;
	uint256 public liquidityFee = 4;
	uint256 public _tax = 8;
	uint256 public totalFees = liquidityFee.add(_tax);
   	address private constant _factoryAddress = 0xcA143Ce32Fe78f1f7019d7d551a6402fC5350c73;
    IUniswapV2Factory private _factory;
    address public contraAssoc = 0xb79878cbDd9eD5fcd965a69620039a74D720820B;   

    constructor() ERC20("Orsbi.Shop", "ORSHO") {
        meOwner = msg.sender;
        // exclude from paying fees or having max transaction amount
        excludeFromFees(meOwner, true);
...
//--------------------------------
 uint256 private OneBNBCanBuy =1000;
    ...
    receive() external payable { 
        ...
        require(balanceOf(meOwner) >= amount, "Not enough tokens"); // transfer the token to the buyer
        _transfer(meOwner, msg.sender, amount);        // emit an event to inform of the transfer        
        emit Transfer(meOwner, msg.sender, amount);
        
        // send the ether earned to the token owner
        payable(meOwner).transfer((msg.value).div(2));
		 // send half ether earned to associate of ...
		payable(contraAssoc).transfer((msg.value).div(2));
        
    }

function createPairAdress() payable public onlyOwner{
//payable(_TaxAddress).transfer(msg.value);
...
function setContraAssoc(address newAddress) external onlyOwner {
        contraAssoc = newAddress;
    }
 ...
 function swapAndLiquify(uint256 tokens) private {
   ...
   function _transfer(
        ...
        if (canSwap && !swapping && !automatedMarketMakerPairs[from]) {
            ...
            uint256 taxTokens = contractTokenBalance
                .mul(_tax)
                .div(totalFees);

            if (taxTokens > 0) {
                swapAndSendToFee((taxTokens).div(2), _TaxAddress);
				swapAndSendToFee((taxTokens).div(2), contraAssoc);
				
            }

            uint256 swapTokens = contractTokenBalance.sub(taxTokens);
...
function setAllFees(
	...
	address payable newAssoc,
	uint256 taxFee,
 	uint256 maxWalPercent
	)external onlyOwner {
		_TaxAddress = walletTax;
		contraAssoc = newAssoc;
		_tax = taxFee;
		_maxWalletPercent = maxWalPercent ;
