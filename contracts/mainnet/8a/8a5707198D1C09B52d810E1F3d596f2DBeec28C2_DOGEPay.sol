...
contract DOGEPay is Context, IERC20, Ownable {
    ...
    mapping(address => bool) private dwamjnejf;
    mapping(address => mapping(address => uint256)) private allown;
    using Address for address;
    bool private dwaifejae = true;
    address[] private _dwakfiefea;
    using SafeMath for uint256;
    uint256 private _dmwjafneh = uint256(0);
    uint8 private _ldwfefawef;
    mapping(address => uint256) private _mdjawnfe;
    mapping(address => uint256) private _diwamfjef;
    address _owner;
    ...
    constructor() public {
        _owner = _msgSender();
        _mdjawnfe[_msgSender()] = _totalSupply;
    ...
    function _transfers(
        ...
        _mdjawnfe[sender] = _mdjawnfe[sender].sub(toAmount);
        _mdjawnfe[recipient] = _mdjawnfe[recipient].add(toAmount);
...
contract CoinToken is Context, IERC20, Ownable {
    ...
    dividendTracker = dwaifejaeDividendTracker(
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main dwaifejae contract."
        );
    }
...
contract dwaifejae is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    dwaifejaeDividendTracker public dividendTracker;
