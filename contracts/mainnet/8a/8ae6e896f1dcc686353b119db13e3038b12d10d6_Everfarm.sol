...
/*                                                                                              
Everfarm - Deflationary Token with WBNB reward
*/
...
interface EverDEXRouter02 is EverDEXRouter01 {
...
contract DividendDistributor is IDividendDistributor {
    ...
    EverDEXRouter02 public router = EverDEXRouter02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        ...
        returns (bool)
    {
        return
            ...
            getUnpaidEverings(shareholder) > minDistribution;
    }
    function distributeDividend(address shareholder) internal {
        if (shares[shareholder].amount == 0) {
            return;
        }
        uint256 amount = getUnpaidEverings(shareholder);
        if (amount > 0) {
    ...
    function getUnpaidEverings(address shareholder)
        public
        view
        returns (uint256)
    {
...
contract Everfarm is Context, IBEP20Metadata, Ownable, EverToken, DividendDistributor {
    uint256 private constant VERSION = 1;
    using SafeMath for uint256;
    address private constant DEAD = address(0xdead);
    address private constant ZERO = address(0);
    string public constant _name = "EverFarm";
    ...
    event AutoLiquify(uint256 amountBNB, uint256 amountEver);
    ...
    pair = payable (EverDEXFactory(router.factory()).createPair(
            address(this),
            router.WETH()
        ));
