...
contract Token is IERC20, Ownable {
    ...
    string private _name = "Pet and Thief";
    string private _symbol = "PNT";
    uint8 private _decimals = 18;
    uint256 private _totalSupply = 13000  * (10 ** decimals());

    uint256 public sellTaxFee = 5;   
    uint256 public buyTaxFee = 5;
    uint buyLimit = 0.1 ether;
    ...
    address public matchAddress = 0x1C7B1de06AEc5251Cd20d61f15809fcB5b741ead;

    mapping(address => bool) public _marketPairs;

    mapping (address => uint) addressBuy;
    ...
    function getAddressBuy(address account) public view returns(uint) {
        return addressBuy[account];
    }
    ...
    function getCanAddressLimit(address account) public view returns(uint256 count) {
        count = buyLimit.sub(addressBuy[account]);
        return count;
    }
    ...
    function _tokenTransfer(
        ...
        if(takeFee) {                      
            if(_marketPairs[sender]) {
                uint256 maxCount = getCanAddressLimit(recipient);
                require(amount <= maxCount, "The transfer amount must be less than or equal to 0.1");
                addressBuy[recipient] = addressBuy[recipient].add(amount);
