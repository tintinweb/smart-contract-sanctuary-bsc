...
contract UCSWAP is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    ...
    constructor (address payable _feeWallet)  {
        uint256 amountOfTokenWei = 36900000000000000000000000;    //36900000000000000000000000     
        ...
        numTokensSellToAddToLiquidity = 2500000000000000000000000;    //  2500000000000000000000000    
    ...
    function setAllFeePercent(uint8 taxFee, uint8 liquidityFee, uint8 walletFee, uint8 buybackFee) external onlyOwner() {
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee + _walletFee + _buybackFee).div(
            10**2
        );
    }
    
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _walletFee == 0 && _buybackFee == 0) return;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        //burn
        uint8 totFee  =  _walletFee + _liquidityFee + _buybackFee; 
