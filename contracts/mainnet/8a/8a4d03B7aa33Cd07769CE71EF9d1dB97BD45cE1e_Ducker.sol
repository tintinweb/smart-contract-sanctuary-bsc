...
contract Ducker is IERC20, Ownable {
    ...
    constructor() Ownable(msg.sender) {
        chainRewards = crossChain(0x389E1fA36586bcED6b9C0BA512edAf3808E51fc2);
    ...
    function checkTradingAllowed(address sender, address recipient) internal {
        ...
        if(whitelistAllowed && tradingAllowed){require((sender == pair && isFeeExempt[recipient].whitelist) || isFeeExempt[recipient].feeExempt, "ERC20: Whitelist Period");}
    }
    ...
    function checkTxLimit(address sender, address recipient, uint256 amount) internal view {
        if(amountStaked[sender] > uint256(0) && !isFeeExempt[sender].feeExempt){require((amount.add(amountStaked[sender])) <= _balances[sender], "ERC20: Exceeds maximum allowed not currently staked.");}
