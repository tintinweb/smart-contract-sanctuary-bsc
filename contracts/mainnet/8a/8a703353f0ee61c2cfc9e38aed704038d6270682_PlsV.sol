...
contract PlsV is Context, ERC20, Ownable {
    using SafeMath for uint256;
   ...
   PlsVDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("PlsV", "PLSV") {
		...
		contractTokenBalanceAmount = 250000000 * 9**9;
        
        swapTokensAtAmount = 250000000 * (9**9);
        _maxWallet = 1500000000 * (9**9);
        _maxBuy = 1500000000 * (9**9);
        _maxSell = 1500000000 * (9**9);
    	...
    	dividendTracker = new PlsVDividendTracker();
	...
	function updateSwapAmount(uint256 amount) public onlyOwner {
	    contractTokenBalanceAmount = amount * (9**9);
	    swapTokensAtAmount = amount * (9**9);
	}

    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "PlsV: The dividend tracker already has that address");

        PlsVDividendTracker newDividendTracker = PlsVDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "PlsV: The new dividend tracker must be owned by the PlsV token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "PlsV: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "PlsV: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "PlsV: Account is already the value of 'excluded'");
    ...
    function addToBlacklist(address account, bool blacklisted) public onlyOwner {
        require(_isBlacklisted[account] != blacklisted, "PlsV: Account is already the value of 'blacklisted'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "PlsV: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "PlsV: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "PlsV: The liquidity wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "PlsV: The marketing wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "PlsV: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "PlsV: Cannot update gasForProcessing to same value");
...
contract PlsVDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("PlsV_Dividend_Tracker", "PlsV_Dividend_Tracker") {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 1 * (9**9);
        isAuth[owner()] = true;
    }
    ...
    function withdrawDividend() public pure override {
        require(false, "PlsV_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PlsV contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 0 && newClaimWait <= 86400, "PlsV_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "PlsV_Dividend_Tracker: Cannot update claimWait to same value");
