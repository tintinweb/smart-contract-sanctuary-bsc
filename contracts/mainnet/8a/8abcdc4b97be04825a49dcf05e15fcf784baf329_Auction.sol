...
interface ISwapFeeRewardWithMB {
    function accrueMBFromAuction(
        address account,
        address fromToken,
        uint256 amount
    ) external;
...
contract Auction is ReentrancyGuard, Ownable, Pausable, IERC721Receiver {
    ...
    mapping(address => bool) public nftForAccrualMB; //add tokens on which MonkeyBoost is accrual
    ...
    ISwapFeeRewardWithMB feeRewardMB;
    bool public feeRewardMBIsEnabled = true;
    ...
    constructor(
        ...
        ISwapFeeRewardWithMB feeRewardMB_
    ) {
    ...
    function updateSettings(
        ...
        uint256 bidIncrRate_,
        address treasuryAddress_,
        ISwapFeeRewardWithMB _feeRewardMB
    ) public onlyOwner {
        prolongationTime = prolongationTime_;
        extendEndTimestamp = extendEndTimestamp_;
        minAuctionDuration = minAuctionDuration_;
    ...
    function addNftForAccrualMB(address _nft) public onlyOwner {
        require(_nft != address(0), "Address cant be zero");
        nftForAccrualMB[_nft] = true;
        emit NFTAccrualListUpdate(_nft, true);
    }

    function delNftForAccrualMB(address _nft) public onlyOwner {
        require(_nft != address(0), "Address cant be zero");
        delete nftForAccrualMB[_nft];
    ...
    function enableMBFeeReward() public onlyOwner {
        feeRewardMBIsEnabled = true;
    }

    function disableMBFeeReward() public onlyOwner {
        feeRewardMBIsEnabled = false;
    }
    ...
    function _collect(uint256 id) internal {
        ...
        if (fee > 0) {
            ...
            if (
                feeRewardMBIsEnabled && nftForAccrualMB[address(inv.pair.nft)]
            ) {
                feeRewardMB.accrueMBFromAuction(
                    inv.bidder,
