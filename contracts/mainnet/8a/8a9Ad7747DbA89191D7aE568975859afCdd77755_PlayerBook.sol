...
contract PlayerBook is Governance, IPlayerBook {
    ...
    uint256[] public _referRewardRate = [2000,2000,2000,2000,2000];     //todo
    uint256[] public _BNBreferRewardRate = [2000,2000,2000,2000,2000];     //todo
    // base rate
    ...
    function seize(IERC20 asset) onlyGovernance external returns (uint256 balance) {
        balance = asset.balanceOf(address(this));
        asset.safeTransfer(msg.sender, balance);
    }

    function seizeEth() onlyGovernance external  {
        uint256 _currentBalance =  address(this).balance;
        msg.sender.transfer(_currentBalance);
    }

    function setStakeAddr(address _stakeAddr) onlyGovernance external  {
        ...
        returns (uint256)
    {
        ...
        emit eveSettle( pID,affID,affID,affReward,affReward,amount);

        return fee;
    }

    function BNBsettleReward(address from, uint256 amount)
        isRegisteredPool()
        validAddress(from)    
        external
        returns (uint256)
    {
        ...
        _plyr[pID].BNBamount = _plyr[pID].BNBamount.add( amount);

        for(uint i=0;i<_BNBreferRewardRate.length;i++)
        {
            affID = _plyr[pID].laff;
            affReward = (amount.mul(_BNBreferRewardRate[i])).div(_baseRate);
            if(affID <= 0){
                affID = _pIDxName[_defaulRefer];
            }
            _plyr[affID].BNBrreward = _plyr[affID].BNBrreward.add(affReward);
            _totalReferReward = _totalReferReward.add(affReward);
            fee = fee.add(affReward);
        ...
        public
    {
        ...
        require(stakeCon(stakeAddr).balanceOfbnb(msg.sender)>0,"not stake");
        ...
        public
    {
        address addr = msg.sender;
        uint256 pid = _pIDxAddr[addr];
        uint256 reward = _plyr[pid].BNBrreward;

        require(reward > 0,"only have reward");
        require(stakeCon(stakeAddr).balanceOfbnb(msg.sender)>0,"not stake");

        IERC20(rewardToken).approve(address(_uniswapV2Router),uint(-1));
        ...
        uint[] memory punkamounts;
         address[] memory callbackpath = new address[](3);
            ...
            callbackpath[1] = usdt;
            callbackpath[2] = weth;
        punkamounts = _uniswapV2Router.swapExactTokensForETH(reward, 0, callbackpath, msg.sender, block.timestamp.add(1800));
        
        //reset
        _plyr[pid].allReward = _plyr[pid].allReward.add(reward);
        _plyr[pid].BNBrreward = 0;
        ...
        returns (uint256,uint256,uint256,uint256)
    {
        ...
        return (_plyr[pID].rreward,_plyr[pID].BNBrreward,_plyr[pID].lv1Count,_plyr[pID].lv2Count);
    }
