...
contract Dogflip is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "Dogflip";
    string private _symbol = "Dogflip";
    ...
    function _transfer(
        ...
        if (recipient == uniswapPair && !isTxLimitExempt[sender]) {
            require(
                amount <= _maxTxAmount,"transfer amount exceeds the maxTransactionAmount."
            );
            require(
                amount + balanceOf(recipient) <= _walletMax,"transfer amount exceeds the maxWalletAmount."
            );
