...
contract FairLaunch is IFairLaunch, Ownable, ReentrancyGuard {
  ...
  struct PoolInfo {
    ...
    uint256 accLvrgdPerShare; // Accumulated ALPACAs per share, times 1e12. See below.
    uint256 accLvrgdPerShareTilBonusEnd; // Accumated ALPACAs per share until Bonus End.
  }
  ...
  */
  ...
  function emergencyWithdraw(uint256 _pid) external nonReentrant {
    ...
    // Safe lvrgd transfer function, just in case if rounding error causes pool to not have enough ALPACAs.
  function safeLvrgdTransfer(address _to, uint256 _amount) internal {
    uint256 lvrgdBal = lvrgd.balanceOf(address(this));
    if (_amount > lvrgdBal) {
      require(lvrgd.transfer(_to, lvrgdBal), "failed to transfer ALPACA");
    } else {
      require(lvrgd.transfer(_to, _amount), "failed to transfer ALPACA");
    }
  }
