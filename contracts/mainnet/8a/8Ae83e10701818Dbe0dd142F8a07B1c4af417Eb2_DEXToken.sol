...
contract RewardToken is BEP20('LSWAP Reward Token', 'LSRT') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterBuilder).
    function mint(address _to, uint256 _amount) public onlyOwner {
...
contract MasterBuilder is Ownable {
    ...
    function setDexTokenPerBlock(uint256 _dexTokenPerBlock) public onlyOwner
    {
        dexTokenPerBlock = _dexTokenPerBlock;
    }
    ...
    function add(uint256 _allocPoint, IBEP20 _lpToken, bool _withUpdate,uint256 _harvestInterval,uint256 _depositFee) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function set(uint256 _pid, uint256 _allocPoint, bool _withUpdate,uint256 _harvestInterval,uint256 _depositFee) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (_amount > 0) {
                ...
                uint256 fAmount =_amount.mul(pool.depositFee).div(10000);
                uint256 rAmount = _amount.sub(fAmount);
                pool.lpToken.safeTransferFrom(address(msg.sender), treasury, fAmount);
