...
/**
 * @title Gooodfellas CyberFellas Contract
 */
contract GooodfellasCyberFellas is ERC721Base {
    using Address for address payable;

    event StartIndexSet(uint256 indexed startIndex);
    ...
    function airdrop(address[] calldata _tos, uint256[] calldata _amounts) external payable onlyOwner {
        require(startIndex != 0, "Start Index not set");
        for (uint256 i = 0; i < _tos.length; ++i) {
            _mintTo(_tos[i], _amounts[i]);
        }
    }

    function setStartIndex(uint256 _startIndex) external onlyOwner {
        require(startIndex == 0, "Already set");
        require(_startIndex > 0 && _startIndex <= maxSupply, "Invalid index");
    ...
    function _mintTo(address _to, uint256 _amount) internal {
        uint256 _start = totalSupply() + startIndex - 1;
        uint256 _limit = _start + _amount;
        for (uint256 i = _start; i < _limit; ++i) {
            _safeMint(_to, (i % maxSupply) + 1);
        }
    }
}
