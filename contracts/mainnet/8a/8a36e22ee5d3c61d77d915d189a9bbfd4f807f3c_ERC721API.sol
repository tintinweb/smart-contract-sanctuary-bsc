...
contract  ERC721API {
  ...
  function ERC721TokenIds( 
    ...
    uint256 from_,
    uint256 to_
  ) public view returns (address[] memory ids, uint256 balance) {
    uint256 _to = to_;
    balance = erc721_.totalSupply();
    if (balance < _to) _to = balance;
    ids = new address[](_to - from_);
    for (uint256 i = 0; i < ids.length; i++) {
      ids[i] = erc721_.ownerOf(from_ + i);
    }
  }
