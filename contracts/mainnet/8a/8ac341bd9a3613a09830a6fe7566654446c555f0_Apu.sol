...
contract Apu is IBEP20, Ownable
{
    ...
    uint256 public constant InitialSupply= 300000000000 * 10**_decimals;
    ...
    //
    address public TeamWallet=payable(0x851847fAc81f2B1bBC6300966E5986BC44c1f0Ae);
    address public Buyback=payable(0x150Bdc3c664DC5A9cC7D989d669bE8D235359b8b);
    //TestNet
	...
	uint256 private antiWhale = 3000000000 * 10**_decimals;
    ...
    function TeamWithdrawALLMarketingBNB() external onlyOwner{
        ...
        payable(Buyback).transfer((amount-(amount*50) / 100));
        emit Transfer(address(this), TeamWallet, (amount*50) / 100);
        emit Transfer(address(this), Buyback, (amount-(amount*50) / 100));
    } 
    ...
    function TeamChangeBuyback(address newBuyback) external onlyOwner{
      require(newBuyback != address(0),
      "Error: Cannot be 0 address.");
      Buyback=payable(newBuyback);
      emit ContractChangedAddress(Buyback);
    }
