...
contract SCUBA is Context, IBEP20, Ownable {
    ...
    string private constant _name = "Scuba Inu";
    string private constant _symbol = "SCUBA";
    ...
    constructor (address payable marketing, address payable _charity) {
        _rOwned[_msgSender()] = _rTotal;
		
		_marketingWallet = marketing;
		_charityWallet = _charity;
   ...
   function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
		...
		uint256 tCharity = calculateCharityFee(amount);
		if(tMarketing > 0){
        ...
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, amount.sub(tMarketing).sub(tCharity));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, amount.sub(tMarketing).sub(tCharity));
        }  else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, amount.sub(tMarketing).sub(tCharity));
        } else {
            _transferStandard(sender, recipient, amount.sub(tMarketing).sub(tCharity));
        }
