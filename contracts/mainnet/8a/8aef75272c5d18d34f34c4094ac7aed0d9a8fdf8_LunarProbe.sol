...
contract LunarProbe is ERC20 {
    ...
    address _fundAddress = address(0x9A3AB5c498C661208E2Ec76305D863183B3293C9);
	...
	uint256 public fundRewardAmount;
	uint256 public fundRewardOverAmount;

    constructor(address tokenOwner) ERC20("Lunar Probe", "Lunar Probe") {
		...
		nftContract = UsdtWarp(0x10ED43C718714eb63d5aA57B78B54704E256024E);//nft;
        uint256 total = 1128 * 10**22;
		startTime = total;
	...
	bool public isNftReward;
	function changeNftRewardStats() public onlyOwner {
        isNftReward = !isNftReward;
    }
    ...
    function getNowPrice() public view returns(uint256){
        ...
        uint256 poolLunarp = balanceOf(uniswapV2Pair);
        if(poolLunarp > 0){
            return poolUsdt.mul(10**10).div(poolLunarp);
        }
        return 0;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx) {
			...
			if(from == uniswapV2Pair){
				if(startTime.add(300) > block.timestamp){
					super._transfer(from, address(0xbfFA70E09f0036680e6aA6f4FE272290A6a688A8), amount.div(10).mul(3));
					amount = amount.div(10).mul(7);
				}
				require(balanceOf(to).add(amount) <= 11280 * 10**18,"max");
            }else if(to == uniswapV2Pair){
				if(amount == balanceOf(from)){amount = amount.sub(10**14);}
            }else{
				if(amount == balanceOf(from)){amount = amount.sub(10**14);}
				require(balanceOf(to).add(amount) <= 11280 * 10**18,"max");
                takeFee = false;
        ...
        if (takeFee) {
			...
			fundRewardAmount = fundRewardAmount.add(amount.div(50));
			
			if(amount >= 128 * 10**18){
				_takeInviterGpFee(from, amount);
			}else{
				super._transfer(from, address(this), amount.div(200).mul(3));
				nftRewardAmount = nftRewardAmount.add(amount.div(200).mul(3));
			}
			...
			if(from == uniswapV2Pair){
                ...
                if(fundrate > 0){
                    super._transfer(from, _destroyAddress, amount.div(100).mul(fundrate));
                    amount = amount.div(100).mul(94-fundrate);
                }else{
                    amount = amount.div(100).mul(94);
                }
            }
	...
	mapping(uint256 => uint256) nftIdReciverAmount;
	function nftReciverAmount(address user) private {
		if(isNftReward){
			uint256 userTotal = nftRewardAmount.div(128);
			uint256 nftNum = nftContract.balanceOf(user);
            if(nftNum == 0){return ;}
			uint256 nftFristId = nftContract.tokenOfOwnerByIndex(user,0);
			uint256 overAmount = nftIdReciverAmount[nftFristId];
			if(userTotal > overAmount){
				uint256 sendAmount = userTotal.sub(overAmount);
				nftIdReciverAmount[nftFristId] = nftIdReciverAmount[nftFristId].add(sendAmount);
				super._transfer(address(this), user, sendAmount);
			}
		}
	}
    ...
    function swapTokensForOther(uint256 tokenAmount) private {
		...
		USDT.transfer(_fund1Address,uamount.div(2));
		USDT.transfer(_fund2Address,uamount.div(2));
    }
    ...
    function swapAndLiquify() private {
		...
		uint256 fundRemain = fundRewardAmount.sub(fundRewardOverAmount);
		if(thisAmount > fundRemain && fundRemain > 10**19){
			fundRewardOverAmount = fundRewardOverAmount.add(fundRemain);
			swapTokensForOther(fundRemain);
		}
    }
    ...
    function splitOtherToken() private {
        ...
        if(thisAmount > nftRemain + fundRemain){
			thisAmount = thisAmount - nftRemain - fundRemain;
	...
	function _takeInviterGpFee(
		...
		if(size >= 5){
			for (uint256 i = 0; i < 5; i++) {
				super._transfer(sender, buyUser[size - 1 - i], amount.div(1000).mul(3));
			}
		}else{
			for (uint256 i = 0; i < size; i++) {
				super._transfer(sender, buyUser[i], amount.div(1000).mul(3));
			}
			super._transfer(sender, address(0x3af022A4A7Aa8C121E11eB7Eb79a4C9ceBada277), amount.div(1000).mul(3 * (5 - size)));
		}
    }
}