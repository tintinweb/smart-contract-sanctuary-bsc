...
contract TBOSS is ERC20Detailed, Ownable {
    ...
    uint256 public _maxTxAmount = _totalSupply * maxtx / 100;
    uint256 public _maxWalletToken =_totalSupply * maxWallet / 100;

    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => bool) public blacklist;

    constructor() ERC20Detailed("Twittelon BOSS", "TBOSS", uint8(DECIMALS)) Ownable() {
        ...
        teamReceiver = 0xc1E832e74E0c83dCF0e91F1FF57E217736232d49;
        marketingReceiver = 0x973De6AbDFE3C3ed2e9194bb8B2F4F076836Fe94;  
        admin = 0x15C0312d6e9E91107cf2f5C132BD310B1bCa927a;
    ...
    function takeFee(
        ...
        _balances[address(this)] = _balances[address(this)].add(
            amount.div(feeDenominator).mul(_marketingFee.add(_teamFee))
        );
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(marketingReceiver).call{
            value: amountETH.mul(marketingFee).div(
                marketingFee.add(teamFee)
            ),
            gas: 30000
        }("");
        (success, ) = payable(teamReceiver).call{
            value: amountETH.mul(teamFee).div(
                marketingFee.add(teamFee)
            ),
            gas: 30000
        }("");
    ...
    function withdrawAllTomarketing() external swapping onlyOwner {
        ...
        uint256 amountToSwap = _balances[address(this)];
        require( amountToSwap > 0,"There is no TBOSS token deposited in token contract");
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();
        router.swapExactTokensForETHSupportingFeeOnTransferTokens(
