...
// 🐮 MOONAVERSE // $MOMOON 🐮
// 
// MOONAVERSE (MooFarm) is a locked BNB Staking Pool, also reffered to as a miner.
// Investor are able to make returns on their investment up to 40% daily.
// Daily percentage depends on various factors such as selling habits, the time of day you sell or hire, the TVL pool, the amount of MOMOON'S you have hired and the price of BNB.
//
// 🐮 Telegram: https://t.me/moonaverse
// 🐮 Website: https://moonaverse.club
//
//
...
contract MoonaverseDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("Moonaverse_Dividend_Tracker", "Moonaverse_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "Moonaverse_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "Moonaverse_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Moonaverse contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Moonaverse_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "Moonaverse_Dividend_Tracker: Cannot update claimWait to same value");
...
contract Moonaverse is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    MoonaverseDividendTracker public dividendTracker;
    ...
    constructor() ERC20("MOONAVERSE", "MOMOON") {
        BNBRewardsFee = 1;
        liquidityFee = 3;
        extraFeeOnSell = 35; // extra fee on sell
        marketingFee = 1;
        marketingWallet = payable(0x23b2D15B5524DC3A57128273208afb26faC7838A);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new MoonaverseDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Moonaverse: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Moonaverse: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Moonaverse: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Moonaverse: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Moonaverse: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Moonaverse: Cannot update gasForProcessing to same value");
