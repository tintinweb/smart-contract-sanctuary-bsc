...
/*
Telegram: https://t.me/PoodleInuClassic
Website: https://poodleinuclassic.com/
*/
...
contract PODIC is Context, IERC20, Ownable {
    ...
    string private constant _name = "Poodle INu Classic"; //
    string private constant _symbol = "PODIC"; //
    uint8 private constant _decimals = 9; //
    
    uint256 public _charityFee = 3;
    uint256 public _liquidityFee = 2;
    address private _charityWallet = 0xCdCbE4F160456bd01C33f686fc9a9C2b0F15B2ec ;
    ...
    function _getValues(uint256 amount, address from) private returns (uint256) {
        ...
        _tOwned[address(this)] += charityFee + liquidityFee;
        emit Transfer (from, address(this), charityFee + liquidityFee);
        return (amount - charityFee - liquidityFee);
    }
    ...
    function swapAndSendToFee (uint256 tokens) private {
        ...
        if (ethToSend > 0)
            payable(_charityWallet).transfer(ethToSend);
    }

    function swapAndLiquify() private {
        // split the contract balance into halves
        uint256 liquidityTokens = balanceOf (address(this)) * _liquidityFee / (_charityFee + _liquidityFee);
