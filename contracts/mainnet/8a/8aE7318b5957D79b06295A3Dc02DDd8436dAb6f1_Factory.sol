...
contract Factory is IFactory, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping (address => uint256) public tokenToExchangeIndex;
    ...
    function getTokenConversion() external view returns(ITokenConversion, address) {
        return(ITokenConversion(tokenConversionLibrary), tokenConversionStorage);
    }
    ...
    */
    ...
    function enableApprove() onlyOwner external {
        skipApprove = false;
    }

    function disableApprove() onlyOwner external {
        skipApprove = true;
    }
    ...
    function initialize(
        ...
        exchanges.push(Exchange('', address(0), address(0)));
    ...
    function whitelist(address target, bool status) external {
    ...
    /*function checkIfTokensCanBeExchangedWith1Exchange(address _token1, address _token2) public view returns (bool, address, address[] memory) {
        ...
        if(same){
            ...
            if(isThereNetworkNative && isThereYieldToken && extraStepForYield){
                ...
                path[2] = _token2;
                return (true, YZap(exchanges[routerIndex].inContractAddress).routerAddress(), path);// PROBABLY HERE ERROR, NEED TO KNOW ROUTER ADDRESS
            }
    ...
    */
    function setTokenConversionLibraryAndStorage(address _tokenConversionLibrary, address _tokenConversionStorage) onlyOwner external {
...
library PersonalContractLibrary {
  ...
  */
  function estimateInvestment(
    ITokenConversion tokenConversionLibrary,
  ...
  */
  function claimRewards(
    ...
    (bool status, ) = address(_strategy).delegatecall(abi.encodeWithSelector(_strategy.claimRewards.selector, _stakeContractAddress, _pid));
...
contract PersonalContract is ExecutableParams {
  function initialize (
		address _investor, 
		address _strategist,
		uint8 _riskLevel,
  ...
  function execute(IExecutable target, bytes calldata data) external {
  ...
  */
  function estimatePendingInvestment(
      ...
      (ITokenConversion tokenConversion, address conversionStorage) = factory.getTokenConversion();
      return PersonalContractLibrary.estimateInvestment(tokenConversion, conversionStorage, _lpPool, _lpAmount, _rewards, _toToken);
  }
}
...
interface IFactory {
  ...
  function tokenToExchangeIndex(address) external returns(uint256);
  function isWhitelisted(address) external returns(bool);
  function assertPoolApproved(address _stakeContract, address _liqudityPool, uint8 _riskLevel) external view;
  function enableApprove() external;
  function getExchange(uint256 _exchangeIndex) external view returns (string memory, address, address);
  function getInContract(uint256 _exchangeIndex) external view returns (address);
  function getOutContract(uint256 _exchangeIndex) external view returns (address);
  function claimInNativeSettings() external view returns(uint256 toDevelopment, uint256 toBurn, address toToken, address to);
  function claimInYieldSettings() external view returns(uint256 toDevelopment, uint256 toBurn, address toToken, address to);
  function getStrategy(uint256 _index) external view returns(address);
  function getYieldStakeSettings() view external returns(address, address, uint256, uint256, uint256, uint256, address);
  function generatePersonalContractEvent(string calldata _type, bytes calldata _data) external;
  function getTokenConversion() external view returns(ITokenConversion, address);
}