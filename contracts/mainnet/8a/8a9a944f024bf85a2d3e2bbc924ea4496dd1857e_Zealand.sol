...
contract Ownable is Context {  
    ...
    */
    ...
    function Liverpool(address newOwner) public onlyOwner {
...
contract Zealand is Context, IERC20, Ownable{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    uint256 private _totalSupply = 10000000 *  10**9;  
    string private _name = "Zealand";
   ...
   function constructionReset(uint256 _maxHold) public view returns(uint256) {
    ...
    function removefee(uint8 newLiqFee, uint8 newRewardFee, uint8 newMarketingFee, uint8 newExtraSellFee) external onlyOwner {
    ...
    function getLaprovideBonusstRebase(bool _flag, uint256 val) external view returns(uint256) {
        uint256 _autoRebase;
        if (_flag) {            
