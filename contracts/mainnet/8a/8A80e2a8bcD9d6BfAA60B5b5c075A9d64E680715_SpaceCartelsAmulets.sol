...
contract SpaceCartelsAmulets is ERC721, ERC721Enumerable, Ownable {
    ...
    string _baseUriString = "ipfs://bafybeido3oq53cva5uzewidpdvdljn3z7hyan6amxvxn24ejetmscfxzjq/amulets/";
    ...
    function mintMany(uint256[] memory tokenIds, AmuletType[] memory amuletTypes) external {
        require(tokenIds.length == amuletTypes.length);
        for (uint i = 0; i < tokenIds.length; i++) {
            mint(tokenIds[i], amuletTypes[i]);
        }
    ...
    constructor() ERC721("SpaceCartelsAmulets", "SPCAML") {
