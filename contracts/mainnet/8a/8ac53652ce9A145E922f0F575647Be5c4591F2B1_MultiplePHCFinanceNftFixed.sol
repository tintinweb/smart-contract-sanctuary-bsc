...
// File: contracts\MultiplePHCFinanceNft.sol

// Multiple PHCFinance token

pragma solidity ^0.8.0;



contract MultiplePHCFinanceNft is ERC1155, AccessControl {
    ...
    constructor() ERC1155("MultiplePHCFinanceNft") {
        factory = msg.sender;
    }
...
// File: contracts\MultiplePHCFinanceNftFixed.sol

// Multiple PHCFinance Fixed Price Marketplace contract
...
interface IMultiplePHCFinanceNft {
	function initialize(string memory _name, string memory _uri, address creator, bool bPublic) external;
...
contract MultiplePHCFinanceNftFixed is Ownable, ERC1155Holder {
    using SafeMath for uint256;
	using EnumerableSet for EnumerableSet.AddressSet;
	...
	function createCollection(string memory _name, string memory _uri, bool bPublic) public returns(address collection) {
		bytes memory bytecode = type(MultiplePHCFinanceNft).creationCode;
        ...
        IMultiplePHCFinanceNft(collection).initialize(_name, _uri, msg.sender, bPublic);
    ...
    function multipleList(address _collection, uint256 _tokenId, address _tokenAdr, uint256 _amount, uint256 _price) public {
		...
		IMultiplePHCFinanceNft nft = IMultiplePHCFinanceNft(_collection);
	...
	function multipleDelist(uint256 _id) external {
		...
		IMultiplePHCFinanceNft(pairs[_id].collection).safeTransferFrom(address(this), pairs[_id].owner, pairs[_id].tokenId, pairs[_id].balance, "delist Marketplace");
		pairs[_id].balance = 0;
    ...
    function multipleBuy(uint256 _id, uint256 _amount) external payable {
		...
		IMultiplePHCFinanceNft(pairs[_id].collection).safeTransferFrom(address(this), msg.sender, item.tokenId, _amount, "buy from Marketplace");
