...
contract DXTStaking is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    address public marketingWallet = 0x23940C383baFB01503585dcbC3b9548e87FC385a;
    // The token TOKEN!
    IBEP20 public token;
    // Info of each pool.
    ...
    uint256 public earlyWithdrawFee = 40;
    ...
    constructor() {
        uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); //Pancake Mainnet
    	//uniswapV2Router = IUniswapV2Router02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1); //Pancake Testnet

        token = IBEP20(0x28920776EdF8a4f612c81866D1F3abFb69A9791A);
    ...
    function add(uint256 _apy, bool _withUpdate,
        uint256 _withdrawLockPeriod ) external onlyOwner {
        require(_withdrawLockPeriod <= 365 days, "withdraw lock must be less than 365 days");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function pendingToken(uint256 _pid, address _user) public view returns (uint256) {
        ...
        if (myBlock > pool.lastRewardBlock && tokenSupply != 0 ) {
            ...
            uint256 tokenPerBlock = tokenSupply.mul(pool.apy).div(100).div(10512000);
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accTokenPerShare = pool.accTokenPerShare.add(tokenReward.mul(1e12).div(tokenSupply));
        pool.lastRewardBlock = myBlock;
    }
    ...
    function withdraw(uint256 _pid) external nonReentrant {
        ...
        if(!canHarvest(_pid, msg.sender)){
            ...
            uint256 totalAmount = penaltyAmount.add(user.rewardLockedUp);
            if(totalAmount > 0){
                swapTokensForBnb(totalAmount);
    ...
    function setEarlyWithdrawFee(uint256 _earlyFeePercent)  external onlyOwner {
        require(_earlyFeePercent <= 40, "Max is 40 percent");
        earlyWithdrawFee = _earlyFeePercent;
    }

    function setMarketingWallet(address _marketingWallet)  external onlyOwner {
        require(_marketingWallet != address(0), "Marketing wallet can't be address 0");
    ...
    function recoverTreasure( IBEP20 recoverToken, uint256 amount) external onlyOwner{
        ...
        if(recoverToken == token){
            require(amount <= treasure, "Withdraw rewards");
            treasure = treasure.sub(amount);
        }
