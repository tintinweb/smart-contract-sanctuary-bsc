...
contract NerdRebaseFragment {
    ...
    {
        uint256 _nfv = accounting[0] + accounting[2] + team[5];
        if (_nfv == 0) _nfv = 1;
        int256 playable = (int256)(_nfv) - (int256)(claims[1] + claims[2]);
        _percent = (playable * 100_0000) / (int256)(_nfv + team[5]);
...
contract NerdFaucetV4 is Ownable {
    ...
    NerdRebaseFragment public fragment;
    ...
    constructor(
        ...
        fragment = new NerdRebaseFragment();
    }
        ...
        )
    {
        ...
        if (!user.done) {
            (, _rebasePayout, , _nerdPercent) = useRebaseFragment(_user);
    ...
    function useRebaseFragment(address _user)
        public
        view
        returns (
            uint256 _totalRebase,
        ...
        )
    {
        ...
        return
            fragment.getNerdAdjustedRebase(
                [
                    acc.netFaucet,
                    acc.deposits,
                    acc.rolls,
                    acc.rebaseCompounded,
                    acc.airdrops_rcv,
                    acc.accFaucet,
                    acc.accRebase,
                    acc.rebaseCount,
                ...
                [
                    ...
                    tm.structure,
                    tm.maxDownline,
                    tm.referralsToUpdate,
                    tm.leaders
                ],
                [
                    cl.faucetClaims,
                    cl.rebaseClaims,
                    cl.faucetEffectiveClaims,
                    cl.boostEnd,
                    uint256(cl.boostLvl)
                ],
                [_user, tm.upline],
    ...
    function firstUpline(address _user, address _upline) internal {
        ...
        require(_upline != address(0), "FU0"); // dev: upline address 0
        ...
        if (
            ...
            revert("FUX"); // dev: Invalid user for upline
        }
    }
    ...
    */
    ...
    function updateGovTk(address _newFlm) external onlyOwner {
        govToken = _newFlm;
        emit LogEvent("GV_U", 0, _newFlm);
    }

    function changeFragment(address _newFragment) external onlyOwner {
        fragment = NerdRebaseFragment(_newFragment);
    }
}