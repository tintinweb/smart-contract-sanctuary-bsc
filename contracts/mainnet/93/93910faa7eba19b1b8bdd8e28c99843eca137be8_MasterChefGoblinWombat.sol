...
// File: contracts/MasterChefGoblinWombat.sol
...
contract MasterChefGoblinWombat is Initializable, ReentrancyGuardUpgradeSafe, OwnableUpgradeable, Goblin {
    /// @notice Libraries
    ...
    IUniswapV2Pair public wombatLpToken;
    address public weth;
    ...
    function initialize (
        ...
        wombatLpToken.approve(address(wombatRouter), uint256(-1)); // 100% trust in the farming
    ...
    function reinvest() public onlyEOA nonReentrant {
        ...
        rewardPath[1] = address(weth);
        router.swapExactTokensForETH(reward.sub(bounty), 0, rewardPath, address(this), now);
    ...
    function health(uint256 id) external view returns (uint256) {
        ...
        (amount[0], ) = wombatRouter.quotePotentialDeposit(fToken, shareToBalance(shares[id]));
