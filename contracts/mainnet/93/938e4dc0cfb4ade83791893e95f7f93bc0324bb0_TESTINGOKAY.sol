...
contract TESTINGOKAY is Context, IERC20 { 
    ...
    // Token Information
    string private constant _tokenName = "TESTINGOKAY Inu";
    string private constant _tokenSymbol = "TESTINGOKAY";
    ...
    bool public autoLiquidityAdd = false;
    ...
    // Used to enable/disable adding liquidity to the LP.
    function autoLiquidityEnabled(bool isEnabled) public {
        require(msg.sender == walletMarketing, "Error: Swap & Liquify can only be enabled/disabled by the marketing team.");

        autoLiquidityAdd = isEnabled;
    }

    // Used to modify the Marketing/Burn/LP percentage split.
    function percentageSplitChanger(uint256 mSplit, uint256 bSplit, uint256 lpSplit) public {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // Marketing Split
        uint256 MarketingSize = SplitMarketing * 100 / (SplitLP + SplitMarketing);
        uint256 MarketingBNB = TotalBNB * MarketingSize / 100;

        // Liquidity Add
        if (autoLiquidityAdd) {
            addLiquidity(tokensLP, (TotalBNB - MarketingBNB));
            emit SwapAndLiquify(tokensLP, (TotalBNB - MarketingBNB), tokensLP);
        }
