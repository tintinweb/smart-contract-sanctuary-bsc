...
contract EARNER is Context, IERC20, Ownable {
    ...
    string private constant _name = "Earner";
    string private constant _symbol = "EARNER";
    ...
    function setBuyTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity) public onlyOwner {
        require((_rfi + _marketing + _liquidity) <= 5, "Buy Fees can't exceed 5%");
    ...
    function setSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity) public onlyOwner {
        require((_rfi + _marketing + _liquidity) <= 5, "Sell Fees can't exceed 5%");
    ...
    function updateSwapTokensAtAmount(uint256 amount) external onlyOwner{
        require(amount <= 1_000_000_000, "Cannot set swap threshold amount higher than 1% of tokens");
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        require(maxBuy >= 1_000_000_000, "Cannot set max buy amount lower than 1% of total supply");
        require(maxSell >= 1_000_000_000, "Cannot set max sell amount lower than 1% of total supply");
        maxBuyLimit = maxBuy * 10**decimals();
        maxSellLimit = maxSell * 10**decimals();
    }
    
    function updateMaxWalletlimit(uint256 amount) external onlyOwner{
        require(amount >= 1_000_000_000, "Cannot set max wallet amount lower than 1% of total supply");
