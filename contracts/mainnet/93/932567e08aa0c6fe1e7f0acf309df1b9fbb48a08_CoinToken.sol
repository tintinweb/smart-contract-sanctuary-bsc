...
contract CoinToken is Context, IERC20, Ownable {
  ...
  function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
    ...
    if(_burnFee > 0){
      uint256 toFee = _burnFee.add(_taxFee);
      uint256 burnAmount = tFee.mul(_burnFee).div(toFee);
      _tOwned[deadAddress] = _tOwned[deadAddress] + burnAmount;
  ...
  function calculateTaxFee(uint256 _amount) private view returns (uint256) {
    uint256 totalFee = _taxFee + _burnFee;
