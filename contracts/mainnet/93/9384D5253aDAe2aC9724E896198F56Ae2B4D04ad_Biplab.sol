...
contract Biplab is Context, IBEP20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    mapping (address => uint256) private _rOwned; //Change to private during real deployment
    mapping (address => uint256) private _tOwned; //Change to private during real deployment 
    ...
    //Can be called with Fees or without fees.
    function burn(uint256 _value, bool withFees) public {
        ...
        if (_isExcluded[msg.sender]) {
            _burnFromExcluded(msg.sender, _value); //If caller is an excluded account
        
        } else {
            _burnStandard(msg.sender, _value);  //Standard burn
        }
        restoreAllFee();
    ...
    function _burnStandard(address sender, uint256 tAmount) internal {
        ...
        (uint256 rAmount,,uint256 rFee, uint256 tTransferAmount, uint256 tFee,, uint256 tCharity) = _getValues(tAmount);
