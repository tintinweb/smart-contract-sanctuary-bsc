...
contract SharedPlan is Ownable {
    ...
    IERC20 public QSQToken = IERC20(0xDb091747B8d9BAe28011D9D4DE84050D1feCBBeD);
    IERC20 public usdtToken = IERC20(0x55d398326f99059fF775485246999027B3197955);
    mapping (uint8 => uint256) public comboMap;    // 套餐组合

    address platformAddress34 = 0x2d702aBeD72fD06Fb266966247c850259Bdcee53;
    address platformAddress46 = 0x9AEA43C2F194B60691c43E0532fd5E51492870B3;

    address public usdtAddress = 0x55d398326f99059fF775485246999027B3197955;
    address public QSQAddress = 0xDb091747B8d9BAe28011D9D4DE84050D1feCBBeD;
    ...
    constructor (IERC20 _QSQToken,address _QSQAddress) public {
        QSQToken = _QSQToken;
        QSQAddress = _QSQAddress;
    }

    function setQSQToken (IERC20 _QSQToken, address _QSQAddress) public onlyOwner {
        QSQToken = _QSQToken;
        QSQAddress = _QSQAddress;
    }
    ...
    function buyIdentity (uint8 _key) public {
        ...
        // 34% 给平台
        usdtToken.transfer(platformAddress34, (_amount * 34)/100);
        // 19.8% 组LP 给用户锁仓
        uint256 add198 = (_amount * 198)/1000;
        uint256 addLPUsdt = add198/2;
        // usdtToken.transferFrom(msg.sender, address(this), add198);
        swapTokensForU(addLPUsdt, address(this));
        usdtToken.approve(address(_router), _amount);
    ...
    function swapTokensForU(uint256 tokenAmount, address _to) public {
    ...
    function setReleaseTimestamp (uint256 _releaseTimestamp) public {
        startReleaseTimestamp = _releaseTimestamp;
    }
    
    function withdrawLP() public {
        UserInfo memory userinfo = userinfoMap[msg.sender];
        uint256 _release = (startReleaseTimestamp - userinfo.lpLockTime) / 7 days;
        uint256 _releaseAmount = getReleaseLP(msg.sender);
    ...
    function getReleaseLP (address _address) public view returns (uint256 _releaseAmount) {
        UserInfo memory userinfo = userinfoMap[_address];
        uint256 _release = (startReleaseTimestamp - userinfo.lpLockTime) / 7 days;
        if (_release > 40) {
            _release = 40;
        }
        _releaseAmount = ((userinfo.lpLockAmount * 25 * _release) / 1000) - userinfo.lpReceived;
    }

    function getUserInfo() public view returns(uint256 _level, uint256 _lpLockAmount, uint256 _lpReceived, uint256 _lpLockTime, uint256 _releaseLP) {
        UserInfo memory userinfo = userinfoMap[msg.sender];
        _level = userinfo.level;                // 用户级别
        _lpLockAmount = userinfo.lpLockAmount;  // LP锁仓总量
        _releaseLP = getReleaseLP(msg.sender);  // 已释放数量
        _lpReceived = userinfo.lpReceived;      // 已领取数量
        _lpLockTime = userinfo.lpLockTime;      // lp锁仓时间
    }

    function getUserInfoOther(address _address) public view returns(uint256 _level, uint256 _lpLockAmount, uint256 _lpReceived, uint256 _lpLockTime, uint256 _releaseLP) {
        UserInfo memory userinfo = userinfoMap[_address];
        _level = userinfo.level;                // 用户级别
        _lpLockAmount = userinfo.lpLockAmount;  // LP锁仓总量
        _releaseLP = getReleaseLP(_address);  // 已释放数量
        _lpReceived = userinfo.lpReceived;      // 已领取数量
        _lpLockTime = userinfo.lpLockTime;      // lp锁仓时间
    }
