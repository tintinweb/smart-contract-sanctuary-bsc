...
import "./IAlterverseCollection.sol";
import "./ERC2981PerTokenRoyalties.sol";

contract AlterverseCollection is
    IAlterverseCollection,
    AccessControl,
    ERC721Enumerable,
    ERC721URIStorage,
    ERC2981PerTokenRoyalties
{
    ...
    modifier onlyController(address controller_) {
        ...
        require(controller() == controller_, "AlterverseCollection: not a controller address.");
        _;
    }
    ...
    /**
     * @dev See {IAlterverseCollection-tokenIds}.
     */
    ...
    /**
     * @dev See {IAlterverseCollection-ownersHistory}.
     */
    ...
    /**
     * @dev See {IAlterverseCollection-locked}.
     */
    ...
    /**
     * @dev See {IAlterverseCollection-setRoomInfos}.
     */
    function setCollectionInfos(uint256 tokenId, bool locked_) external onlyController(_msgSender()) {
        // solhint-disable-next-line reason-string
        require(tokenId != 0, "AlterverseCollection: tokenId is the zero value");
        Collection storage collection = _collections[tokenId];
        collection.locked = locked_;
    }

    /**
     * @dev See {IAlterverseCollection-setController}.
     */
    function setController(address controller_) external onlyController(_msgSender()) {
        // solhint-disable-next-line reason-string
        require(controller_ != address(0), "AlterverseCollection: controller is the zero address");
        // solhint-disable-next-line reason-string
        require(controller_ != controller(), "AlterverseCollection: controller already updated");
        _controller = controller_;
        emit ControllerChanged(controller_);
    }

    /**
     * @dev See {IAlterverseCollection-lockTokenId}.
     */
    function lockTokenId(uint256 tokenId) external onlyController(_msgSender()) {
        // solhint-disable-next-line reason-string
        require(_exists(tokenId), "AlterverseCollection: change for nonexistent token");
        _collections[tokenId].locked = true;
        emit TokenIdLocked(tokenId, true);
    }

    /**
     * @dev See {IAlterverseCollection-releaseLockedTokenId}.
     */
    function releaseLockedTokenId(uint256 tokenId) external onlyController(_msgSender()) {
        // solhint-disable-next-line reason-string
        require(_exists(tokenId), "AlterverseCollection: change for nonexistent token");
        // solhint-disable-next-line reason-string
        require(_collections[tokenId].locked == true, "AlterverseCollection: tokenId not locked");
        _collections[tokenId].locked = false;
        emit TokenIdReleased(tokenId, false);
    }

    /**
     * @dev See {IAlterverseCollection-controller}.
     */
    ...
    /**
     * @dev See {IAlterverseCollection-replicas}.
     */
    ...
    /**
     * @dev See {IAlterverseCollection-baseURI}.
     */
    ...
    /**
     * @dev See {IAlterverseCollection-exists}.
     */
    ...
    /**
     * @dev See {IAlterverseCollection.setBaseURI}.
     */
    ...
    /**
     * @dev See {IAlterverseCollection.setTokenURI}.
     */
    function setTokenURI(uint256 tokenId, string memory tokenURI_) public onlyRole(DEFAULT_ADMIN_ROLE) {
        // solhint-disable-next-line reason-string
        require(_exists(tokenId), "AlterverseCollection: change uri for nonexistent token");
        _setTokenURI(tokenId, tokenURI_);
    }

    /**
     * @dev See {IAlterverseCollection-mint}.
    ...
    function mint(
        ...
        // solhint-disable-next-line reason-string
        require(totalSupply() < replicas(), "AlterverseCollection: all tokens already minted.");
    ...
    function bulkSafeMint(
        ...
        // solhint-disable-next-line reason-string
        require(tos.length > 0, "AlterverseCollection: empty tos");
        // solhint-disable-next-line reason-string
        require(mTokenIds.length == tos.length, "AlterverseCollection: tokenIds length mismatched");
    ...
    /**
     * @dev See {IAlterverseCollection-safeMint}.
     */
    function safeMint(
        address to,
        uint256 tokenId,
