...
// SPDX-License-Identifier: Unlicensed
    ...
    contract CYFpublicsales is ReentrancyGuard, PriceConsumerV3{
        ...
        uint256 internal _liquidityBNB;
        uint256 internal _liquidityUSD;
        uint256 internal _devFeeBNB; 
        uint256 internal _devFeeUSD; 
        uint256 internal _yieldfarmSupportBNB;
        ...
        address internal immutable yieldfarm_ = address(0xdA95522baC148AFACD69D694FC5132eE97AA3e62);
        ...
        function dispatach(uint256 _amount, bool _bnb) private{
            ...
            if(_bnb){
                ...
                _liquidityBNB += _liquidityfunds;
                _yieldfarmSupportBNB += _yieldfarmsupport;
        ...
        function registerPlayer(address _refBy) internal{
            if(players[msg.sender]._sponsor == address(0)){
                address _upline = dev_;
                // address _upline = _refBy; // testing purpose
                if(_refBy != address(0) && _refBy != msg.sender && players[_refBy]._allocation > 0){
        ...
        function dispatachFunds() public{
            require(_yieldfarmSupport_ > 0 || _liquidityBNB > 0 || _liquidityUSD > 0, 'NotWallowed');
            if(_yieldfarmSupport_ > 0){
                swapAndLiquify(_yieldfarmSupport_);
                _yieldfarmSupport_ = 0;
            }
            if(_yieldfarmSupportBNB > 0){
                payable(yieldfarm_).transfer(_yieldfarmSupportBNB);
                _yieldfarmSupportBNB = 0;
            }
            if(_liquidityBNB > 0){
                payable(liquidity_).transfer(_liquidityBNB);
                _liquidityBNB = 0;
            }
            if(_liquidityUSD > 0){
                _usdToken.transfer(liquidity_, _liquidityUSD);
                _liquidityUSD = 0;
            }
        }
