...
contract Deposit is Ownable, Pausable {
    ...
    function updateTokens(address[] memory _tokens, bool[] memory _valids)
        external
        onlyOwner
    {
    ...
    function deposit(address _token, uint256 _amount) external whenNotPaused {
        ...
        tokens[_token].userDeposit[_msgSender()] += _amount;

        emit Desposit(_msgSender(), _token, _amount, block.timestamp);
    }

    function getDeposit(address _token, address _user)
        public
        view
        returns (uint256)
    {
