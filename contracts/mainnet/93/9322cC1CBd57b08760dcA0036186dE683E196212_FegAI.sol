...
Telegram : https://t.me/FEGAiPortal
*/
...
contract FegAI is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    FegAIDividendTracker public dividendTracker;
    ...
    constructor() ERC20("FEG AI", "FEGAI") {
        marketingWallet = payable(0x48897D7CA1cc7a5567a73d717163C5C8FCa0cA11);
        devWallet = payable(0x48897D7CA1cc7a5567a73d717163C5C8FCa0cA11);
        ...
        dividendTracker = new FegAIDividendTracker(
            payable(this),
            router,
...
contract FegAIDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        FegAIContract = FegAI(mainContract);
    ...
    function _transfer(
        ...
        require(false, "FegAI_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "FegAI_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FegAI contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(FegAIContract);

            uint256 prevBalance = FegAIContract.balanceOf(address(this));
            ...
            {
                uint256 received = FegAIContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    FegAIContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
