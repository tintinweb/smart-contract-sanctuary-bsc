...
contract RangoHyphen is IRangoHyphen, BaseProxyContract {
    ...
    function updateHyphenAddress(address _address) public onlyOwner {
        updateHyphenAddressInternal(_address);
    }
    ...
    function hyphenBridge(
        HyphenBridgeRequest memory _request,
        address _token,
        uint256 _amount
        ...
        if (_token == NULL_ADDRESS)
            require(_amount <= msg.value, "Insufficient ETH sent to RangoHyphen");
        else
