...
contract VestingRedeemer is OwnableBase {
    ...
    function claim(bool isStake) public {
        ...
        if(isStake){
            IERC20(vestingToken).approve(address(kroStaking),redeemableAmountFromInitial);
            require(kroStaking.stake(redeemableAmountFromInitial,msg.sender),"no staking");
        }else{
    ...
    function setKroStaking(address _kroStaking) public onlyOwner {
        kroStaking = IKroStaking(_kroStaking);
    }
