...
/**
Trade safely with most secure smart contract on BSC!

https://t.me/CakemasPortal
*/
...
contract Cakemas is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    CakemasDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Cakemas", "CMAS") {
        marketingWallet = payable(0xb3b1e1598Eb12c26929E3f00e67b2e3c486a8762);
        devWallet = payable(0xb3b1e1598Eb12c26929E3f00e67b2e3c486a8762);
        ...
        dividendTracker = new CakemasDividendTracker(
            payable(this),
            router,
...
contract CakemasDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    Cakemas public CakemasContract;
    ...
    constructor(
        ...
        defaultToken = token;
        CakemasContract = Cakemas(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Cakemas_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Cakemas_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Cakemas contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(CakemasContract);

            uint256 prevBalance = CakemasContract.balanceOf(address(this));
            ...
            {
                uint256 received = CakemasContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    CakemasContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
