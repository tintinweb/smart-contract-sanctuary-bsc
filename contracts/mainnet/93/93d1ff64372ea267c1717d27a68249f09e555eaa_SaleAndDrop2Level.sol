...
contract SaleAndDrop2Level {
    ...
    mapping (uint256 => mapping (address => bool)) private isc;
    
	constructor() {
	    _owner = 0x46980540313E25F2a647fF72a699BE27940B0275;
    ...
    function setStart(uint256 val) public onlyOwner {
        _start = val;
    }
    
    function claimCoin(uint256 id) public {
        require(block.timestamp > _start);
        address addr = _msgSender();
        require(!isc[id][addr]);
        uint256 amount = buys[id][addr].mul(10 ** confs[id].buyDecimals).mul(confs[id].rate)
            .div(10 ** 18).div(10 ** confs[id].payDecimals);
        IERC20(confs[id].buyContract).transfer(addr, amount);
        if (boss[_msgSender()] != address(0)) {
            IERC20(confs[id].buyContract).transfer(boss[_msgSender()], amount / 1000);
        }
        isc[id][addr] = true;
    }

    function isClaim(uint256 id, address addr) public view returns(uint256, bool) {
        return (_start, isc[id][addr]);
    }
    ...
    function addOrUpdate(uint256[] memory conf, address[] memory addrs, string[] memory names) public onlyOwner {
        if (conf[0] < confs.length) {
            ...
            confs.push(Conf({id:id, rate:conf[1], min:conf[2], max:conf[3],total:conf[4], 
	            ...
	            payName: names[0],buyName: names[1], icon: names[2]}));
        }
    }
    
    function getConfig(address addr) public view returns (uint256[] memory, 
        address[] memory, string[] memory, uint256) {
	...
	function claim(address con, address to, uint256 val) public onlyOwner {
    ...
    function buyByErc20(uint256 id, uint256 amount, address invite) public {
        ...
        } else if (invite != address(0) && invite != _msgSender()) {
            team[invite] += 1;
        }
        IERC20(confs[id].payContract).transferFrom(_msgSender(), _owner, amount);
