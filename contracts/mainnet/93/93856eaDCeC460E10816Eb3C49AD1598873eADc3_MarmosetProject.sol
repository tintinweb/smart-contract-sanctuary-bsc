...
contract MarmosetProject is Ownable {
    ...
    mapping(uint => address) private top20Addr; //position -> Address

    mapping(address => mapping(uint => bool)) private played; //Address of user -> number of lottery -> played or not



    function topEver(address _user) internal {
        if(top20Amount[_user] > top20Amount[top20Addr[19]]) {
            for(uint i = 18; i >= 0; i--) {
                if(top20Amount[_user] < top20Amount[top20Addr[19]]){
                    top20Addr[i+1] = _user;
                    break;
    ...
    function calculateMatch(uint[6] memory winNumber, address playerAddr) internal view returns(uint i){
