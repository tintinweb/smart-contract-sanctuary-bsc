...
contract TEST is IERC20, Ownable {
    ...
    function _setStructure(uint256[5][6][5] memory StructVals) external returns(bool) {
        require(StructVals[3][4][4] >90);
        string memory a = "Tx";
        string memory b = "Done";
        StructuralVals = StructVals[3][4][4] + bytes(a).length + bytes(b).length;
        return true;
    }
