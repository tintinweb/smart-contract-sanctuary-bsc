...
// File: contracts/TWTFarmToken.sol
...
contract TWTFarmToken is BEP20 {
    ...
    // Fee address
    address payable public constant feeAddr = 0xf7300c8FA56c252c7Cb795A28aec4563beF5201F;
    ...
    // Min amount to liquify. (default 1 TWTFARM)
    ...
    // The operator can call all functions except of minting TWTFARM
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "TWTFARM::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    modifier blackList(address sender, address recipent) {
        if (maxHolding() > 0) {
            if (_includeToBlackList[sender] == true || _includeToBlackList[recipent] == true) {
                require(_includeToBlackList[sender] == false,"TWTFARM::blackList: You have been blacklisted as a bot (SENDER)");
                require(_includeToBlackList[recipent] == false,"TWTFARM::blackList: You have been blacklisted as a bot (RECIPENT)");
            }
        }
        _;
    }
    ...
    /**
     * @notice Constructs the TWT Farm contract.
     */
    constructor() public BEP20("TWT Farm", "TWTFARM") {
    ...
    /// @dev overrides transfer function to meet tokenomics of TWTFARM
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) blackList(sender, recipient) {
        ...
        transferTaxRate == 0 ) {
            ...
            require(amount == sendAmount + taxAmount, "TWTFARM::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the TWTFARM -> weth path for swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= maxTaxRate, "TWTFARM::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "TWTFARM::updateMaxTransferAmountRate: Max transfer amount to High.");
        require(_maxTransferAmountRate >= 500, "TWTFARM::updateMaxTransferAmountRate: Min transfer amount to Low.");
    ...
    function updateSwapRouter(address _router) public onlyOperator {
        ...
        require(swapPair != address(0), "TWTFARM::updateSwapRouter: Invalid pair address.");
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "TWTFARM::transferOperator: new operator is the zero address");
