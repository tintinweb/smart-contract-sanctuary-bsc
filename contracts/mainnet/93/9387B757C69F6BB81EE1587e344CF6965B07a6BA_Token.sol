...
contract Token is ERC20, Ownable {
    ...
    address public _devWalletAddress = 0x18Ab9eabe358cfe23d878eb2872f229c9bBFDd75;//1生态资金钱包3
    address public _marketingWalletAddress1 = 0x81499596214B9de65837AFA3c7aC5Ae58BAd08e7;//0.7营销钱包1
    address public _marketingWalletAddress2 = 0xCD3b975B59506CD3568ca59bfbbe92952CB958C7;//1.3营销钱包2
    // address public _marketingWalletAddress3 = 0xA88d1A2b9DAa5c336964B62d173a553349bC8F70;//0.5营销钱包
    address public _mindWalletAddress = 0x318B442Df63a30906fc640BCd392fCD0554b27c0;//中转钱包（授权U给合约）
    ...
    constructor() public ERC20("331", "331") {
    ...
    function _swapAndLiquid() private {
        ...
        if(fistval > 0){
            uint256 dr = fistval.mul(30).div(60);
            uint256 dr0 = fistval.mul(10).div(60);
            uint256 dr1 = fistval.mul(7).div(60);
            uint256 dr2 = fistval.mul(13).div(60);
            // uint256 dr3 = fistval.sub(dr).sub(dr0).sub(dr1).sub(dr2);
            IERC20(USDTs).transfer(address(dividendTracker), dr);
            IERC20(USDTs).transfer(_devWalletAddress, dr0);
            IERC20(USDTs).transfer(_marketingWalletAddress1, dr1);
            IERC20(USDTs).transfer(_marketingWalletAddress2, dr2);
            // IERC20(USDTs).transfer(_marketingWalletAddress3, dr3);
            return;
        }
