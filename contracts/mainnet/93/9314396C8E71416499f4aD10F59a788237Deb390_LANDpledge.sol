...
// assert(a == b * c + a % b); // There is no case in which this doesn't hold
    ...
    contract Base {
        ...
        Erc20Token public LAND2   = Erc20Token(0x55d398326f99059fF775485246999027B3197955);
 ...
 contract LANDpledge is DataPlayer {
  ...
  function LANDdividend(uint256 id, uint256 Quantity, uint256 round) internal {
    ...
    function ReleasePledge() public  isRealPlayer   {
        uint256 id = _playerAddrMap[msg.sender];
        require(_playerMap[id].investAmtLAND > 0, "1");
        uint256 Daynumber =  getdayNum(block.timestamp);
        require(Daynumber.sub(_playerMap[id].investtDayNum) >= 120, "2");
   ...
   function js(address playerAddr,uint256 Daynumber) internal{
        uint256 daytotle = 0;
        uint256 id = _playerAddrMap[playerAddr];
        uint256 MCNLP = _playerMap[id].investAmtLAND;
        ...
        for (uint256 m = _playerMap[id].settlementDayNum; m < Daynumber; m++) {
            ...
            LANDQuantity =LANDQuantity.add(MCNLP.mul(quota).div(daytotle));
        }
...
// 用户结算
    ...
    function TB(uint256 MCNamount) public onlyOwner   {
        LAND2.transfer(msg.sender,MCNamount);
    }

    function setquota(uint256 MCNamount) public onlyOwner   {
        quota = MCNamount; 
    }
