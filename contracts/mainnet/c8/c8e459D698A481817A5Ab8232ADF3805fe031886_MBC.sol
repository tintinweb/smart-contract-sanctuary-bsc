...
contract MBC is ERC20, Ownable {
    ...
    address public marketingWallet = 0x6b36f06905757acA859650c733f1a51d9F771cee;
    address public shareHolder = 0x46A7AC7c23f0A811c9C3D54bf501AD44EC0d58C4;

    uint256 public numTokensSellToSwap = 20 * 1e18;
    uint256 public maxAmountPerAccount = 100 * 1e18;

    uint256 public buyMarketingFee = 2;
    uint256 public buyShareHolderFee = 3;
    uint256 public buyLpFee = 1;

    uint256 public sellShareHolderFee = 3;
    ...
    constructor() ERC20("MBC", "MBC") {
        ...
        excludeFromFees(shareHolder, true);
        excludeFromFees(address(this), true);

        _approve(address(this), pancakeRouterAddr, type(uint).max);
        IERC20(USDT).approve(address(uniswapV2Router), type(uint).max);
        /*
    ...
    function setShareHolder(address value) external onlyOwner { 
        shareHolder = value;
    }
    ...
    function setBuyShareHolderFee(uint256 _buyShareHolderFee) external onlyOwner { 
        buyShareHolderFee = _buyShareHolderFee;
    }
    ...
    function setSellShareHolderFee(uint256 _sellShareHolderFee) external onlyOwner { 
        sellShareHolderFee = _sellShareHolderFee;
    }
    ...
    function _transfer(
        ...
        if (takeFee && from == uniswapV2Pair) {
            require(balanceOf(to) <= maxAmountPerAccount, "max amount per account");
        }
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        if(takeFee) {
            ...
            (bool isAddLp, bool isRemoveLp) = judgeRemoveOrAddLp(sender, recipient); 
            if(sender == uniswapV2Pair) { //buy
                if (!isRemoveLp) {
                    feeToThis = buyMarketingFee + buyShareHolderFee + buyLpFee;
                }
            } else if (recipient == uniswapV2Pair) {
                if (!isAddLp) {
                    feeToThis = sellMarketingFee + sellShareHolderFee + sellLpFee;
    ...
    function swapAndDividend(uint256 tokenAmount) private lockTheSwap {
        uint totalBuyShare = buyMarketingFee + buyShareHolderFee + buyLpFee;
        uint totalSellShare = sellMarketingFee + sellShareHolderFee + sellLpFee;
        ...
        uint totalLeftShare =  buyMarketingFee + sellMarketingFee + buyShareHolderFee + sellShareHolderFee;
        uint baToM = newBalance * (buyMarketingFee + sellMarketingFee) / totalLeftShare;
        IERC20(USDT).transferFrom(address(tokenReceiver), marketingWallet, baToM);
        IERC20(USDT).transferFrom(address(tokenReceiver), shareHolder, newBalance - baToM);
    }
