...
contract X8MultiSwap {
    ...
    event FeeApplied(uint256 amount, uint256 n, uint256 d, uint256 amountFeeApplied);
    ...
    function swap(
        ...
        for (uint256 i = 0; i < pairs.length; i++) {
            ...
            uint256 pairFeeD = fds[i]; // gas savings
            address swapTo = i < pairs.length - 1 ? pairs[i + 1] : msg.sender; // swap to next pool or sender if final swap

            uint256 amountToSwapFeeApplied = (amountToSwap *
                (pairFeeD - fns[i])) / pairFeeD;
            emit FeeApplied(amountToSwap, fns[i], pairFeeD, amountToSwapFeeApplied);

            (
                address nextTokenToSwap,
                uint256 amount0Out,
                uint256 amount1Out
            ) = calcSwap(tokenToSwap, amountToSwapFeeApplied, pair);

            pair.swap(amount0Out, amount1Out, swapTo, new bytes(0));
            emit SubSwap(address(pair), tokenToSwap, nextTokenToSwap, amountToSwapFeeApplied, amount0Out, amount1Out);
    ...
    )
{
    ...
    if (t0 == tokenIn) {
        return (t1, 0, (amountIn * r1) / (r0 + amountIn));
    } else {
        return (t0, (amountIn * r0) / (r1 + amountIn), 0);
    }
}