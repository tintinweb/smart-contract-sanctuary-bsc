...
contract SupportAssistExt is Ownable {
    ...
    uint256 public fundTokensAtAmount = 10 * 1e18;
    ...
    function deposit(uint256 _ftype, uint256 amount0) external {
        ...
        if (_inviter != address(0)) {
            ...
            if (_speedtime > 0) {
                uData._speed(_inviter, _speedtime);
            }
        }
    ...
    function getRewardMint(address _account) public {
        uint256 _reward = lpRewardMint.getReward(_account);
        if (_reward > 0) {
            uint256 _lp = (_reward * fhratio[0]) / 1000;
            if (_lp > 0) {
                lpRewardMint.executeReward(_lp);
            }

            uint256 _burn = (_reward * fhratio[1]) / 1000;
            uint256 _income = _reward - _lp - _burn;
            USDT.transfer(_account, _income);
            emit LPReward(_account, _reward, _income, _lp, _burn);
        }
    }
        ...
        returns (uint256[8] memory data)
    {
        ...
        data[7] = 0;
        if (address(lpRewardMint) != address(0)) {
            data[7] = lpRewardMint.earned(_account);
        }
        return data;
    }
    ...
    function _afterTokenTransfer(
        ...
        if (_fromtype == 0) {
            ...
            bool canswap = swapTokensAmount > swapTokensAtAmount &&
                tokenB.balanceOf(address(this)) >= swapTokensAmount;
            if (!_swapping && canswap) {
                _swapping = true;
                uint256 swapamount = swapTokensAmount;
                swapTokensAmount = 0;
                ...
                if (amounts[1] > 0) {
                    lpRewardMint.executeReward(amounts[1]);
                }
                _swapping = false;
            }
        }
