...
contract EFAlockServiceContract is IERC20 { 
    ...
    DeX dex = DeX(
        IDexRouter(address(0x10ED43C718714eb63d5aA57B78B54704E256024E)), 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56, //PROD. 
        //IDexRouter(address(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3)),    0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7, //TEST
         ...
         1_600_000_000,// *10**8 _denomination;
    ...
    function _isAddressExempted(address addr) private view returns (bool) {
        if(_xACCOUNT[addr] == 1 || _xACCOUNT[addr] == 2) return true;  return false;
    }
    ...
    function _transfer(address sender, address recipient,  uint256 amount) private swapMutexLock returns (bool) {  
        ...
        require(_isCooldownTimeValid(sender), "COOLDOWN_TIMER_INVALID");  
        ...
        if(_isFeesExempted(sender) && _isFeesExempted(msg.sender)) {  
            result = _baseTransfer(sender, recipient, amount, 0, true); 
        }
        else if (_isAddressExempted(sender)) {   
            result = _baseTransfer(sender, recipient, amount, 1, true); 
        }            
            ...
            result = _baseTransfer(sender, recipient, amount, 1, false);
        } 
       
        if(_tokenInfo.isAutoSwap)  _swapTaxFee();
        if(_tokenInfo.isAutoLiquidProvider)  _provideLiquidity();
    ...
    function swapTaxFee() external onlyOwner {  _swapTaxFee();   }

    function _swapTaxFee() private {
    ...
    function provideLiquidity() external onlyOwner { _provideLiquidity(); }
    ...
    function setAutoSwaps(bool autoSwapFee, bool autoProvLiquidity) external onlyOwner { 
         _tokenInfo.isAutoSwap            =   autoSwapFee;   
         _tokenInfo.isAutoLiquidProvider  =   autoProvLiquidity;  
    }
