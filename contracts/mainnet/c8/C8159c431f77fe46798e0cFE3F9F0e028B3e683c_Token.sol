...
library Address {
    /**
     * @dev Returns true if `accaunt` is a contract.
     *
     * [IMPORTANT]
     * ====
     * It is unsafe to assume that an address for which this function returns
     * false is an externally-owned accaunt (EOA) and not a contract.
     *
     * Among others, `isContract` will return false for the following
     * types of addresses:
     *
     *  - an externally-owned accaunt
...
interface IBEP20 {
    ...
    /**
     * @dev Returns the annmount of tokens owned by `accaunt`.
     */
    function balanceOf(address accaunt) external view returns (uint256);

    /**
     * @dev Moves `annmount` tokens from the caller's accaunt to `recipient`.
     *
    ...
    /**
     * @dev Emitted when `value` tokens are moved from one accaunt (`from`) to
     * another (`to`).
     *
 ...
 * manner, since when dealing with GSN meta-transactions the accaunt sending and
 ...
 * there is an accaunt (an owner) that can be granted exclusive access to
 * specific functions.
 *
 * By default, the owner accaunt will be the one that deploys the contract. This
...
contract Ownable is Context {
    ...
    /**
     * @dev Throws if called by any accaunt other than the owner.
     */
    modifier onlyOwner() {
    ...
    /**
     * @dev Transfers ownership of the contract to a new accaunt (`newOwner`).
...
contract Token is IBEP20, HasForeignAsset {
    ...
    /** @dev Creates `annmount` tokens and assigns them to `accaunt`, increasing
     * the total supply.
     *
    ...
    /**
     * @dev Destroys `annmount` tokens from `accaunt`, reducing the
     * total supply.
     *
     ...
     * Requirements
     *
     * - `accaunt` cannot be the zero address.
     * - `accaunt` must have at least `annmount` tokens.
     */
