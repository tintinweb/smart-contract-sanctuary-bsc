    ...
    ReentrancyGuard
{
    ...
    {
        design = IHouseDesign(contractAddress);
    ...
    /** Mints a token. */
    function mint(uint256 rarity) external whenNotPaused nonReentrant {
        // Check global limit.
        uint256[] memory mintLimits = design.getMintLimits();
        require(rarity < mintLimits.length, "Invalid rarity");
        require(
            tokenByRarity[rarity] < mintLimits[rarity],
            "Global limit reached"
        );

        // Transfer coin token.
        address to = msg.sender;
        coinToken.transferFrom(to, vault, design.getMintCost(rarity));
        ...
        _safeMint(to, id);
        emit TokenCreated(to, id, tokenDetails[id]);
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (from == address(0)) {
            ...
            ++tokenByRarity[HouseDetails.decodeRarity(details)];
        } else {
            // Transfer or burn.
            // Swap and pop.
            uint256[] storage ids = tokenIds[from];
            uint256 index = HouseDetails.decodeIndex(tokenDetails[id]);
            uint256 lastId = ids[ids.length - 1];
            ids[index] = lastId;
            ids.pop();

            // Update index.
            tokenDetails[lastId] = HouseDetails.setIndex(
                tokenDetails[lastId],
                index
            );
        }
        if (to == address(0)) {
            ...
            ids.push(id);
            tokenDetails[id] = HouseDetails.setIndex(tokenDetails[id], index);
