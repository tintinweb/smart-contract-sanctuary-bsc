...
contract HacLottery is VRFConsumerBaseV2, Ownable, Pausable {
    ...
    address public tokenAddress = 0x4a04dB284EB0b238CD282CB248fC66DDd5fD300E;
    IERC20 token;

    address public blackHoleAddress =
        0x0235f90cA7a5C321Ad1CAE4c984A2abe910EB3e3;

    uint256 public maxTicketsPerPlayer = 100;
    mapping(uint256 => mapping(address => uint256)) public numTicketsBought;

    uint256 public winnerShare = 1;
    uint256 public blackHoleShare = 999;
    ...
    function enter(uint256 tickets) external whenNotPaused {
        ...
        token.transferFrom(msg.sender, address(this), tickets * 10 ether);
    }

    function start() external onlyOwner {
        endTimes[lotteryID] = block.timestamp + 2 weeks;
        _unpause();
    }
    ...
    function payoutLottery() external {
        ...
        if (players.length > 0) {
            uint256 totalAmount = players.length * 10 ether;
