...
//Dill SMART  CONTRACT - 2023

contract Dill {
//============= public variables ==============
    string constant public name = "Dill";
    string constant public symbol = "DLL";
...
//============= Events ==============
	...
	function burn(uint256 _amountTokens) external {
		...
		data.holders[msg.sender].balance -= _amountTokens;
	...
	function approve(address _spender, uint256 _amountTokens) external returns (bool) {
		data.holders[msg.sender].allowance[_spender] = _amountTokens;
	...
	function _transfer(address _sender, address _receiver, uint256 _amountTokens) internal returns (uint256) {
		...
		data.holders[_sender].balance -= _amountTokens;
	...
	function transferFrom(address _sender, address _receiver, uint256 _amountTokens) external returns (bool) {
		require(data.holders[_sender].allowance[msg.sender] >= _amountTokens);
		data.holders[_sender].allowance[msg.sender] -= _amountTokens;
	...
	function disableBurning(address _holder, bool _status) public {
		...
		data.holders[_holder].burningDisabled = _status;
	...
	function getRewards(address _holder) public view returns (uint256) {
		return uint256(int256(data.scaledPayoutPerToken * data.holders[_holder].amountStaked) - data.holders[_holder].scaledPayout) / DEFAULT_SCALAR_VALUE;
	}
	...
	function isBurningDisabled(address _holder) public view returns (bool) {
		return data.holders[_holder].burningDisabled;
	}
	...
	function _staking(uint256 _amount) internal {
		...
		data.holders[msg.sender].amountStaked += _amount;
		data.holders[msg.sender].scaledPayout += int256(_amount * data.scaledPayoutPerToken);
