...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        bool governance; // mint gSTEEMFI if true
    }

    // SteemFi TOKEN!
    ISteemFiToken public steemFiToken;
    // Governance Token
    gSTEEMFIToken public gSTEEMFI;
    // Dev address.
    address public dev1;
    address public dev2;
    // Deposit Fee address
    address public feeAddress;
    // SteemFi tokens created per block.
    uint256 public tokenPerBlock;
    // Bonus muliplier for early steemFiToken makers.
    ...
    constructor(
        ISteemFiToken _token,
        gSTEEMFIToken _xtoken,
        uint256 _startBlock,
    ...
    function pendingToken(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 steemFiTokenReward = multiplier.mul(tokenPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accTokenPerShare = accTokenPerShare.add(steemFiTokenReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 totalDevReward = steemFiTokenReward.div(10);

        steemFiToken.mint(dev1, totalDevReward.div(2));
        steemFiToken.mint(dev2, totalDevReward.div(2));

        steemFiToken.mint(address(this), steemFiTokenReward);

        pool.accTokenPerShare = pool.accTokenPerShare.add(steemFiTokenReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for SteemFi Tokens allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(steemFiToken)) {
                uint256 transferTax = _amount.mul(steemFiToken.transferTaxRate()).div(10000);
            ...
            if (pool.depositFeeBP > 0) {
                ...
                if(pool.governance) {
                    gSTEEMFI.mint(msg.sender, _amount.sub(depositFee)); // gSTEEMFI mint
                }
            } else {
                user.amount = user.amount.add(_amount);

                if(pool.governance) {
                    gSTEEMFI.mint(msg.sender, _amount);
                }
            }
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if(pool.governance) {
            gSTEEMFI.burn(msg.sender, _amount); // if user transfer their gSTEEMFI it causes error.
        }
        user.rewardDebt = user.amount.mul(pool.accTokenPerShare).div(1e12);
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
        ...
        if(pool.governance) {
            gSTEEMFI.burn(msg.sender, amount);
        }
    ...
    // Pay or lockup pending SteemFi Tokens.
    ...
    // Safe steemFiToken transfer function, just in case if rounding error causes pool to not have enough SteemFi Tokens.
    function safeTokenTransfer(address _to, uint256 _amount) internal {
        uint256 steemFiBalance = steemFiToken.balanceOf(address(this));
        if (_amount > steemFiBalance) {
            steemFiToken.transfer(_to, steemFiBalance);
        } else {
            steemFiToken.transfer(_to, _amount);
        }
    }
