...
contract PeaceGigaFactory is Ownable,ERC1155Receiver {
    ...
    modifier onlyNFTSeller(uint256 _tokenID) {
        ...
        require(_allSaleInfo[_tokenIndex].currentOwner == msg.sender || owner() == msg.sender, "not NFT owner");
        _;
    }
    ...
    constructor(address _nftAddress) {
        ...
        addressInfo.teamAddress = payable(0xbAB8E9cA493E21d5A3f3e84877Ba514c405be0e1);
    ...
    function setNFTCardInfo (uint _id, string memory _symbol, string memory _uri, uint256 _busd, uint256 _sup) external onlyOwner {
        ...
        emit SetNFTCardInfo (_id, _uri, _busd, _sup);
    }
    ...
    function getRewardInfoByNFT(uint256 _nftID) view private returns(uint256, uint256){
        ...
        if (block.timestamp - nftCreatedTime <= ONE_DAY_TIME * 80) {
            rewardPercent += (block.timestamp - nftClaimedTime) * nftPrice * royaltyInfo.PeaceGigaROI / 10000 / ONE_DAY_TIME;
            ...
            if (nftCreatedTime + ONE_DAY_TIME * 80 >= nftClaimedTime) {
                rewardPercent += (nftCreatedTime + ONE_DAY_TIME * 80 - nftClaimedTime) * nftPrice * royaltyInfo.PeaceGigaROI / 10000 / ONE_DAY_TIME;
            }
            rewardPercent += (block.timestamp - (nftCreatedTime + ONE_DAY_TIME * 80)) * nftPrice * royaltyInfo.OneYearROI / 10000 / ONE_DAY_TIME;
            currentROI = royaltyInfo.OneYearROI;
        }
        else if (block.timestamp - nftCreatedTime <= ONE_DAY_TIME * 365 * 2) {
            if (nftCreatedTime + ONE_DAY_TIME * 80 >= nftClaimedTime) {
                rewardPercent += (nftCreatedTime + ONE_DAY_TIME * 80 - nftClaimedTime) * nftPrice * royaltyInfo.PeaceGigaROI / 10000 / ONE_DAY_TIME;
                rewardPercent += (ONE_DAY_TIME * 285) * nftPrice * royaltyInfo.OneYearROI / 10000 / ONE_DAY_TIME;
            }
            else if (nftCreatedTime + ONE_DAY_TIME * 365 >= nftClaimedTime) {
                rewardPercent += (nftCreatedTime + ONE_DAY_TIME * 365 - nftClaimedTime) * nftPrice * royaltyInfo.OneYearROI / 10000 / ONE_DAY_TIME;
            }
            rewardPercent += (block.timestamp - (nftCreatedTime + ONE_DAY_TIME * 365)) * nftPrice * royaltyInfo.TwoYearROI / 10000 / ONE_DAY_TIME;
            currentROI = royaltyInfo.TwoYearROI;
        }

        return (rewardPercent, currentROI);
    }
    ...
    function claimByNFT(uint256 _tokenID) external onlyNFTOwner(_tokenID) {
        ...
        (uint256 nftReward, uint256 nftROI) = getRewardInfoByNFT(_tokenID);
        ...
        _busdToken.transfer(msg.sender, nftReward * (royaltyInfo.totalPercent - royaltyInfo.teamPercent) / royaltyInfo.totalPercent);
        _busdToken.transfer(addressInfo.teamAddress, nftReward * royaltyInfo.teamPercent / royaltyInfo.totalPercent);
    ...
    function claimAll() external {
        ...
        for(uint i=0; i<nftCount; i++) {
            (uint256 nftReward, uint256 nftROI) = getRewardInfoByNFT(_nftIDsOfUser[msg.sender][i]);
            rewards += nftReward;
            nftROI = 0;
        ...
        _busdToken.transfer(msg.sender, rewards * (royaltyInfo.totalPercent - royaltyInfo.teamPercent) / royaltyInfo.totalPercent);
        _busdToken.transfer(addressInfo.teamAddress, rewards * royaltyInfo.teamPercent / royaltyInfo.totalPercent);
    ...
    function getAllNFTInfos () view external returns (NFTInfos memory){
        ...
        for(uint i=0; i<nftIDs.length; i++) {
            ...
            (rwInfo.nftRevenue[rwIndex], rwInfo.currentROI[rwIndex]) = getRewardInfoByNFT(nftID);
    ...
    function createSaleReal(uint256 _tokenID, uint _price, uint8 _kind) external onlyNFTOwner(_tokenID) returns (bool) {
        require (_pauseService == false, "Service is stopped.");
        ...
        _allSaleInfo.push (SaleInfo(_tokenID, _uriFromId[_tokenID], mkNFT.creators(_tokenID), msg.sender, _price, _kind));
    ...
    function buyNow(uint256 _tokenID) payable external nonReentrant{
        ...
        _allSaleInfo[_tokenIndex].currentOwner = msg.sender;

        emit BuyNow(_allSaleInfo[_tokenIndex].currentOwner, _allSaleInfo[_tokenIndex].currentOwner, msg.sender, _allSaleInfo[_tokenIndex].salePrice, _allSaleInfo[_tokenIndex].tokenHash, _tokenID, royaltys);
    }
    ...
    function setRoyalty(uint256 _ra, uint256 _ta, uint256 _da, uint256 _tda, uint256 _ohr, uint256 _oyr, uint256 _tyr) external onlyOwner {
        royaltyInfo.rewardPercent = _ra;
        royaltyInfo.treasuryPercent = _ta;
        royaltyInfo.teamPercent = _da;
    ...
    event SetNFTCardInfo(uint infoID, string uri, uint256 usdc, uint256 sup);
