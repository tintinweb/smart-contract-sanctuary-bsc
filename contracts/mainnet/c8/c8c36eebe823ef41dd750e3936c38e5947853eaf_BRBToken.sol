...
contract BRBToken is ERC20("Blue Ribbon", "BRB"), Ownable {
    ...
    uint256 private _totalSupply = 22000 * (10**_decimals);
    ...
    uint256 public stakePoolFee = 20;
    uint256 public burnFee = 5;
    uint256 public lpFee = 10;
    bool inSwapAndLiquify;


    mapping (address => bool) internal isBrber;
    ...
    address public stakePoolAddress = 0xF3874AF7B6d59d35D8b651aB9c39dee4a90913fA;
    address public lpReceiverAddress = 0x27E1f8611D9e40E2d838467430FA5ca31f79F437;
    address public desilterWallet = 0x5A95fB0981020C4B8427A1c835e460d0F3262fa1;
    ...
    constructor(address router_) {
        ...
        liquidityWallet = 0xe01AC4199f55aC20c918bA307347fa8d0Da00303;
    ...
    function updateBrber(address account, bool enabled) public onlyOwner {
        isBrber[account] = enabled;
    }
    ...
    function setStakePool(address _add) public onlyOwner {
        stakePoolAddress = _add;
    }
    ...
    function _takeFees(address sender, address recipient, FeeInfo memory _feeInfo) private {
       ...
       if (_feeInfo.tBurn > 0) {
            uint256 endTotal =  2200 * 10**decimals();
            if (totalSupply().sub(_feeInfo.tBurn).sub(balanceOf(deadWallet)) < endTotal) {
                if (totalSupply().sub(balanceOf(deadWallet)) >= endTotal){
        ...
        if (_feeInfo.tStake > 0){
            _approve(address(this),stakePoolAddress, _feeInfo.tStake);
            _tOwned[address(this)] = _tOwned[address(this)].add(_feeInfo.tStake);
            IPool(stakePoolAddress).sendAmount(_feeInfo.tStake);
        }
        if (_feeInfo.tLp > 0) {
            _distributionFees(address(this),  _feeInfo.tLp);
        }
        ...
        address taxer = sender;
            ...
            _distributionFees(referrer2, _feeInfo.tGen);
            _distributionFees(referrer3, _feeInfo.tGen);
        }
    }
        ...
        )
    {
        ...
        if (_isBuy || _isSell) {
            ...
            _feeInfoTmp.tStake = tAmount.mul(stakePoolFee).div(1000);
            _feeInfoTmp.tBurn = tAmount.mul(burnFee).div(1000);
            _feeInfoTmp.tLp = tAmount.mul(lpFee).div(1000);
        
        
        _feeInfoTmp.tAmount = tAmount;
        tTransferAmount = tTransferAmount.sub(_feeInfoTmp.tGen * 3).sub(_feeInfoTmp.tStake).sub(_feeInfoTmp.tBurn).sub(_feeInfoTmp.tLp);
    ...
    function _transfer(
        ...
        if (_referrerByAddr[recipient] == address(0) && amount >= 10000000000000 && !sender.isContract() && !recipient.isContract()) {
        ...
        if (_isBuy || _isSell){
            ...
            if (!inSwapAndLiquify && balanceOf(address(this)) > 0) {
                    swapAndLiquify();
            }
        }
    }
    ...
    function Sweep() external {
     ...
     function swapAndLiquify() private lockTheSwap {
        ...
        uint256 otherHalf = balanceOf(address(this)).sub(half);
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH(); //uniswapV2Router.WETH();
    ...
    function burn(uint256 amount) public {
       _tokenTransferNoFee(msg.sender, deadWallet, amount);
    }
    function inviter(address _add) public view returns(address) {
        return _referrerByAddr[_add];
    }
