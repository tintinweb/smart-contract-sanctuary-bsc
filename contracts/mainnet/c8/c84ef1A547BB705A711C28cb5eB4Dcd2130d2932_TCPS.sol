...
contract TCPS is IERC20, Ownable {
    ...
    address public fundAddress = address(0x1706417bB546859E5Dd085A91E5f1A86a404F3cd);
    address public lpAddress = address(0x97AEB043D7e3a785aa7190897B812fb34065Da99);
    ...
    uint256 public numTokensSellToAddToLiquidity = 9 * 10**18;
    ...
    function setMin(uint256 _num) public onlyOwner {
    ...
    function _transfer(
        ...
        if (from==uniswapV2Pair && isRoute[to]){
            ...
            if(block.timestamp<startTime){
                require(_isExcludedFromFee[from] || _isExcludedFromFee[to],"addr error");
            }
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]  || balanceOf(_destroyAddress)>=930000*10**18) {
            takeFee = false;
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 funAmount=contractTokenBalance.div(3);
        swapTokensForUsdt(funAmount);

        contractTokenBalance=contractTokenBalance.sub(funAmount);
