...
contract PingPongDemo is CCIPReceiverInterface, OwnerIsCreator {
    ...
    constructor(
        CCIPRouterInterface receivingRouter,
    ...
    {
        require(
            message.sourceChainId == s_counterpartChainId,
            "wrong source chain"
        );
        require(
            abi.decode(message.sender, (address)) == s_counterpartAddress,
            "wrong sender"
        );
    ...
    function getRouters()
        external
        view
        returns (CCIPRouterInterface, CCIPRouterInterface)
    {
