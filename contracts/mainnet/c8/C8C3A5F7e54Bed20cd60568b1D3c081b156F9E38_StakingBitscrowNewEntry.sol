...
contract StakingBitscrowNewEntry {
    ...
    modifier checkPeriod(){
        require(timestart[msg.sender] != 0);
    ...
    function changeSeats(uint amount) external onlyOwner{
        require(_tokenaavailableforrewards * 1000 / rewardsrate / Cost >= amount);
        _seatsLeft = amount;
    }
    ...
    function timeLeftTillUnstake(address user) external view returns(uint256){
        if(block.timestamp >= timestart[msg.sender] + timePeriod) {
            return 0;
        }else {
