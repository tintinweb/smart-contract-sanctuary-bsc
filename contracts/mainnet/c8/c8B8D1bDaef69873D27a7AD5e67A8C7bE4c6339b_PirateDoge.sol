...
/*
-------EPICALLS TEAM NEW PROJECT -----------

*//*https://t.me/piratedogebsc
*/
...
contract PirateDoge is IERC20, Ownable {
    ...
    string _name = "Pirate Doge";
    string _symbol = "PIRATEDOGE";
    ...
    constructor () {
        ...
        _marketingFeeReceiver = deployer;
        teamFeeReceiver = 0xfbf2D907e2bFF2Dc397b927b6339323130C01dD9;
    ...
    function viewFees() external view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        return (FeeLiquidity, _marketingFee, giveawayFee, reflectionFee, totalFee, feeDenominator);
    }
    ...
    function clearBalance() external {
        ...
        (bool success,) = payable(_marketingFeeReceiver).call{value: address(this).balance, gas: 30000}("");
        require(success);
    }

    function changeMaxTx(uint256 maxTx) external onlyOwner {
        _maxTxAmount = 10000000000000000000 * 10 ** _decimals * maxTx / 1000;
    }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountToLiquify = swapThreshold.mul(FeeLiquidity).div(_totalFee).div(2);
        ...
        uint256 totalBNBFee = _totalFee.sub(FeeLiquidity.div(2));
        uint256 amountBNBLiquidity = amountBNB.mul(FeeLiquidity).div(totalBNBFee).div(2);
        ...
        if (amountBNBMarketing.add(amountBNBGiveaway) > 0) {
            (bool market,) = payable(_marketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
    ...
    function changeFees(uint256 _FeeLiquidity, uint256 _reflectionFee, uint256 __marketingFee, uint256 _giveawayFee) external onlyOwner {
        FeeLiquidity = _FeeLiquidity;
        reflectionFee = _reflectionFee;
        _marketingFee = __marketingFee;
        giveawayFee = _giveawayFee;
        totalFee = FeeLiquidity.add(reflectionFee).add(_marketingFee).add(giveawayFee);
    ...
    function setFeeReceivers(address __marketingFeeReceiver, address _liquidityReceiver) external {
        require(isTxLimitExempt[msg.sender]);
