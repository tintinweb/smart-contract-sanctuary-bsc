...
contract MirrorProtocolDistributor is UUPSUpgradeable, OwnableUpgradeable {
    ...
    modifier onlyToken() {
        require(msg.sender == parentToken); _;
    }
    ...
    function setShare(address shareholder_, uint256 amount_) external onlyToken {
        if(shares[shareholder_].amount > 0 && !isPaused){
    ...
    function process(uint256 gas_) external onlyToken {
        require(!isPaused, "Rewards are paused");
    ...
    function processBNBSwap() internal {
        uint256 bnbAmount = 0;
        if(rewardToken == WBNB) {
            if(address(this).balance.sub((totalDividends.sub(totalDistributed))) > 0) {
                bnbAmount = address(this).balance.sub((totalDividends.sub(totalDistributed)));
            }
        }
        else {
            ...
            rewardRouter.swapExactETHForTokensSupportingFeeOnTransferTokens{value: swapBNBThreshold}(
                0,
                path,
                address(this),
                block.timestamp
            ...
            bnbAmount = IERC20(rewardToken).balanceOf(address(this)).sub(balanceBefore);
        }

        if(bnbAmount > 0) {
            totalDividends = totalDividends.add(bnbAmount);
            dividendsPerShare = dividendsPerShare.add(dividendsPerShareAccuracyFactor.mul(bnbAmount).div(totalShares));
        }
    }
