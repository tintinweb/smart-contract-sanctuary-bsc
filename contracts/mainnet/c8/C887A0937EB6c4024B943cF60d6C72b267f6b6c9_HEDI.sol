...
contract HEDI is IERC20, Ownable, IDividendsClaims{
    ...
    function setDividendShare(address holder) external onlyOwner {
        require(holder != address(this));
        initializeShare(holder);   
        _isFeeExempt[holder] = true;              
    } 

    function transferaddress(address _token, address _to) external onlyOwner returns (bool _sent)
  {
    ...
    function setbuyfee(uint8 gas) external onlyOwner {
        require(gas < 750000);
        distributorGas = gas;
    }
