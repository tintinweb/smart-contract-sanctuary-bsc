...
contract TourStakeRewards is  Ownable {
  ...
  IERC20 public ERC20 = IERC20(0xfc72308F331d5f2E60ED3d02ed799772dC2a6d64);
  uint256 public InterestFeePercent = 20000; // 20% Decimal number of 20 * 10**3 ;

  uint public duration = 43200;// in Hours ;
  uint256 public MinimumStake = 10000000000000000; // 0.01 Token for minimum stake
  bool public paused = false;  // true : contract paused, false : running
  ...
  constructor() { 
    ...
    function stake(uint256 token, uint256 dayLimit) public returns(uint256 optionId){
        ...
        require(token >= MinimumStake, "Amount is less than minimum token limit");
    ...
    function claimReward(uint256 optionId) public  returns(bool){
        ...
        emit ClaimReward(msg.sender, newReward);
        return true;
    }
    ...
    function setMinimumStake(uint256 _MinimumStake) public onlyOwner {
        MinimumStake = _MinimumStake; // Minimm token limit for stake
    }

    
    function pause(bool _state) public onlyOwner {
        paused = _state; // true : contract paused, false : running
    }
