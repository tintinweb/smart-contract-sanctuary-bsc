...
// She is still and will forever be daddy's most adorable and tiniest rat.
...
contract TINYRAT_BSC is BEP20, Auth {
    ...
    string public constant name             = "TINYRAT BSC";
    string public constant symbol           = "TINY";
    uint8 public constant decimals          = 9;

    uint256 public constant totalSupply     = 2957912 * 10**decimals;
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver                = 0xE7f96E1eA765a6C87fDdBBaD1162EBC25A9a2718;
    ...
    // switch Trading
    function tradingStatus(bool _status, bool _antibot) external onlyOwner {
        if(!_status || _antibot){
    ...
    function tradingStatus_launchmode(uint256 confirm) external onlyOwner {
        require(confirm == 2957912,"Accidental Press"); // just paranoid
    ...
    function _updatefees() internal {
        require(totalFee.mul(buyMultiplier).div(100) < 100, "Buy tax cannot be more than 99%");
        require(totalFee.mul(sellMultiplier).div(100) < 19, "Sell tax cannot be more than 18%");
        require(totalFee.mul(transferMultiplier).div(100) < 7, "Transfer Tax cannot be more than 6%");
