...
// File contracts/StarGold.sol

pragma solidity 0.6.12;


contract StarGold is ERC20Burnable, Operator {
    ...
    // AIRDROP
    uint256 public constant INITIAL_GENESIS_POOL_AIRDROP = 20000 ether;
    ...
    /**
     * @notice Constructs the StarGold ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("StarGold", "StarGold") {
        // Mints 1 StarGold to contract creator for initial pool setup
    ...
    function _getStarGoldPrice() internal view returns (uint256 _StarGoldPrice) {
        try IOracle(StarGoldOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("StarGold: failed to fetch StarGold price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _StarGoldPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_StarGoldPrice >= taxTiersTwaps[tierId]) {
    ...
    function setStarGoldOracle(address _StarGoldOracle) public onlyOperatorOrTaxOffice {
        require(_StarGoldOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints StarGold to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of StarGold to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentStarGoldPrice = _getStarGoldPrice();
            currentTaxRate = _updateTaxRate(currentStarGoldPrice);
            if (currentStarGoldPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_airdropWallet != address(0), "!_genesisPool");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_airdropWallet, INITIAL_GENESIS_POOL_AIRDROP);
    }
