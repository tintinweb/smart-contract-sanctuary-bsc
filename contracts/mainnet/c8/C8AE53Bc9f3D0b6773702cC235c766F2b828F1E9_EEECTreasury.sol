...
contract EEECTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SEEEC }
    ...
    address public sEEEC;
    uint public sEEECQueue; // Delays change to sEEEC address
    ...
    /**
        @notice allow approved address to deposit an asset for EEEC
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint EEEC needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( EEEC ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn EEEC for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IEEECERC20( EEEC ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sEEEC ).balanceOf( msg.sender ); // Can only borrow against sEEEC held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with EEEC
        @param _amount uint
     */
    function repayDebtWithEEEC( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IEEECERC20( EEEC ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, EEEC, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( EEEC ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( EEEC ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns EEEC valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match EEEC decimals
            value_ = _amount.mul( 10 ** IERC20( EEEC ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SEEEC ) { // 9
            sEEECQueue = block.number.add( blocksNeededForQueue );
        } else return false;
