...
contract BABYTOKEN is ERC20, Ownable, BaseToken {
    ...
    uint256 public kill = 2;
    uint256 public counter = 0;

    address public _marketingWalletAddress = 0xB30ed9d941e646b5D04aEaC4E8aADe40539a217f;
    address public _marketingWalletAddress1 = 0x3516818227dE8E2Bec0898923FBefF753859440F;
    ...
    constructor(
        ...
        rewardToken = 0xC9882dEF23bc42D53895b8361D0b1EDC7570Bc6A;
        require(
        ...
        dividendTracker = BABYTOKENDividendTracker(
            payable(Clones.clone(0x86F9e19D618275B4EC4a6caFA27718fdF3c2a833))
        );
    ...
    //添加白名单，无费率  【地址，true】 true为添加 false为移除
    ...
    //修改营销钱包1地址
    function setMarketing1Wallet(address payable wallet) external onlyOwner {
    ...
    function setTokenRewardsFee(uint256 value) external onlyOwner {
        ...
        totalFees = tokenRewardsFee.add(liquidityFee).add(marketingFee).add(marketingFee1);
        require(totalFees <= 25, "Total fee is over 25%");
    }
    ...
    //修改杀死地址数量
    function setMarketingFee1(uint256 _target) external onlyOwner {
        kill = _target;
    }
    ...
    function _transfer(
        ...
        require(swapEnable, "swap is no open");
    ...
    function swapAndSendToFee(uint256 tokens) private {
        ...
        uint256 marketAmount = newBalance.mul(marketingFee).div(marketingFee.add(marketingFee1));
        IERC20(rewardToken).transfer(_marketingWalletAddress, marketAmount);
        IERC20(rewardToken).transfer(_marketingWalletAddress1, newBalance.sub(marketAmount));
    }
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        uint256 initialBalance = rewardToken.balance;

        // swap tokens for ETH
        swapTokensForCake(half); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered

        // how much ETH did we just swap into?
        uint256 newBalance = rewardToken.balance.sub(initialBalance);
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        _approve(rewardToken, address(uniswapV2Router), ethAmount);
        _approve(address(this), address(uniswapV2Router), tokenAmount);
