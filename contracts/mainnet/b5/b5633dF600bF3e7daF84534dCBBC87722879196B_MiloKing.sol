...
contract TokenTracker {
    constructor(
        ...
        IERC20(usdt).approve(msg.sender, amount);
    }
}

contract MiloKing is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public _market = 0xA0b24AE93cc055733aD43Af7E4EEB91b5E0762af;
    ...
    uint256 private buyDividendFee = 3;
    uint256 private sellDividendFee = 3;
    ...
    uint256 public do_amount = (1 * 10**18);
    ...
    constructor() ERC20("MiloKing", "MiloKing") {
        _totalSupply = 6900000000 * (10**18);

        dividendTracker = new dDividendTracker();
        tokenReceiver = address(
            new TokenTracker(address(dToken), _usdt, 10**18 * 10**18)
        );
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        uint256 initialBalance = IERC20(_usdt).balanceOf(tokenReceiver);

        // swap tokens for ETH
        swapTokensForUSDT(half, tokenReceiver); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered
        // how much ETH did we just swap into?
        uint256 newBalance = IERC20(_usdt).balanceOf(tokenReceiver) -
            initialBalance;
        IERC20(_usdt).transferFrom(tokenReceiver, address(this), newBalance);
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        _approve(address(this), address(uniswapV2Router), tokenAmount);
        // add the liquidity
        IERC20(_usdt).approve(address(uniswapV2Router), ethAmount);
...
contract dDividendTracker is Ownable, DividendPayingToken {
    ...
    {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 69000000 * (10**18); //must hold tokens for dividen
    }
