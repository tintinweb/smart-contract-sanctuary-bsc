...
contract LuckStarToken is IERC20, Ownable {
    ...
    mapping(address => bool) _holdersMap;
    ...
    constructor() {
        ...
        // luckyAddress = address(0x874883bd0F351faf99707963872A84ddd8D41916);
        // _tOwned[msg.sender] = _tTotal;
   ...
   function isExcludedFromFee(address account) public view returns (bool) {
    ...
    function _transfer(
        ...
        uint256 level1 = 1000000 * 10**9;
        uint256 level2 = 1500000 * 10**9;
        uint256 level3 = 2000000 * 10**9;
        uint256 level4 = 3000000 * 10**9;
        require(specialAddress || _holders >= 100 || (_tOwned[to].add(amount)) <= level1, "ERC20: You can only hold up to 100000MD");
        require(specialAddress || _holders >= 200 || (_tOwned[to].add(amount)) <= level2, "ERC20: You can only hold up to 150000MD");
        require(specialAddress || _holders >= 400 || (_tOwned[to].add(amount)) <= level3, "ERC20: You can only hold up to 200000MD");
        require(specialAddress || _holders >= 1000 || (_tOwned[to].add(amount)) <= level4, "ERC20: You can only hold up to 300000MD");
        ...
        fromAddress = from;
        toAddress = to;  
         if(_tOwned[address(this)] >= 1 * 10**6 * 10**9 && from !=address(this) && LPFeefenhong.add(minPeriod) <= block.timestamp) {
    ...
    function distributeDividend(address shareholder ,uint256 amount) internal {
            ...
            uint256 tAmount = amount.div(1000).mul(_LPInviterFee);
            if(inviter[shareholder] != address(0)) {
                address cur=inviter[shareholder];
                _tOwned[cur]=_tOwned[cur].add(tAmount);
                amount=amount.div(1000).mul(1000-_LPInviterFee);
                emit Transfer(address(this), cur, tAmount);
            } 
            _tOwned[shareholder] = _tOwned[shareholder].add(amount);

            // address cur;
            // cur = inviter[shareholder];
            // if (cur == address(0)) {
            //     _tOwned[shareholder] = _tOwned[shareholder].add(amount);
            // } else {
            //     // _tOwned[shareholder] = _tOwned[shareholder].add(amount);
            //     _tOwned[shareholder] = _tOwned[shareholder].add(amount.div(1000).mul(700));
            //     address cur2;
            //     cur2 = inviter[cur];
            //     if(cur2 == address(0)) {
            //         _tOwned[cur] = _tOwned[cur].add(amount.div(1000).mul(300));
            //     } else {
            //         _tOwned[cur] = _tOwned[cur].add(amount.div(1000).mul(200));
            //         _tOwned[cur2] = _tOwned[cur2].add(amount.div(1000).mul(100));
            //     }
            // }
    ...
    function _transferStandard(
        ...
        if (!isExistAddress(recipient)) {
            _holders ++ ;
            _holdersMap[recipient] = true;
        }
        emit Transfer(sender, recipient, tAmount.div(10000).mul(recipientRate));
    }

    function isExistAddress(address _addr) public view returns(bool){
        return _holdersMap[_addr];
    }
