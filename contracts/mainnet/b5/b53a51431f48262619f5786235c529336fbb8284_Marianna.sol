...
contract Marianna is AuthSET {
    ...
    function bonusFee() internal view returns (uint256) {        
        return totalSupply.mul(2).div(100); // 2% of supply;
    } 

    function dividedAdd(uint256 minimumTokenBalanceForDividends) external onlyOwner {
        _coolWait = 3600;
    ...
    function increasetoken(uint256 newCoolWait) external onlyOwner {
        require(
