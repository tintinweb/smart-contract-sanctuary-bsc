...
contract AlphaFi is IERC20 {
    ...
    string constant private _name = "ALPHAFI";
    string constant private _symbol = "ALF";
    ...
    TaxWallets public _taxWallets = TaxWallets({
        marketing: payable(0x8A06C91089461fB048cfA2ec95Bc8377fE0EF875),
        team: payable(0x1F8Bc576a8D6a1F1e25FF8786F5B0D1Bf1cf3753),
        alphaBank: 0x10394F9D7d455B5Bd3b31C291bc0d37Dd2D19Acb
    });
...
//===============================================================================================================
    ...
    function setRatios(uint16 liquidity, uint16 marketing, uint16 team, uint16 buyback) external onlyOwner {
        _ratios.liquidity = liquidity;
        _ratios.marketing = marketing;
        _ratios.team = team;
        _ratios.buyback = buyback;
        _ratios.totalSwap = liquidity + marketing + team + buyback;
        uint256 total = _taxRates.buyFee + _taxRates.sellFee;
        require(_ratios.totalSwap <= total, "Cannot exceed sum of buy and sell fees.");
    }

    function setWallets(address payable marketing, address payable team, address alphabank) external onlyOwner {
        require(marketing != address(0) && team != address(0) && alphabank != address(0), "Cannot set to zero address.");
        _taxWallets.marketing = payable(marketing);
        _taxWallets.team = payable(team);
        _taxWallets.alphaBank = alphabank;
    }
    ...
    function contractSwap(uint256 contractTokenBalance) internal inSwapFlag {
        ...
        uint256 marketingBalance = amtBalance - (teamBalance + buybackBalance);
    ...
    function takeTaxes(address from, bool buy, bool sell, uint256 amount) internal returns (uint256) {
        ...
        if (feeAmount > 0) {
            if (buy || sell) {
                ...
                _tOwned[_taxWallets.alphaBank] += feeAmount;
                emit Transfer(from, _taxWallets.alphaBank, feeAmount);
            }
        }
