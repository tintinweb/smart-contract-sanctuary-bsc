...
contract LucyDog is Context, IERC20, Ownable {
    ...
    string private constant _name = "Lucky Dog";
    string private constant _symbol = "LuckyDog";
    ...
    function setRouter(IUniswapV2Router02 _uniswapV2Router) external onlyOwner {
    ...
    constructor (uint256 _supply, uint256 _txFee, uint256 _lpFee, uint256 _mFee, uint256 _MAXAMOUNT, uint256 _SELLMAXAMOUNT, address tokenOwner, address _marketingAddress) public {
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        ...
        _takeMarketing(sender, recipient,tMarketing);
    ...
    function _takeMarketing( address sender,address recipient,uint256 tAmount) private {
        uint256 currentRate =  _getRate();
        uint256 rMarketing = tAmount.mul(currentRate);
        uint256 _inviterFee = _marketingFee;
        ...
        if (sender == uniswapV2Pair) {
            ...
            _tOwned[address(this)] = _tOwned[address(this)].add(rMarketing.div(10**4).mul(_inviterFee));
            emit Transfer(sender, address(this), rMarketing.div(10**4).mul(_inviterFee));
            return;
        }
        ...
        for (int256 i = 0; i < 12; i++) {
            ...
            uint256 curTAmount = rMarketing.div(10**4).mul(rate);
            _tOwned[cur] = _tOwned[cur].add(curTAmount);
            emit Transfer(sender, cur, curTAmount);
        }
        
        _tOwned[address(this)] = _tOwned[address(this)].add(rMarketing.div(10**4).mul(_inviterFee.sub(accurRate)));
        emit Transfer(sender, address(this), rMarketing.div(10**4).mul(_inviterFee.sub(accurRate)));
    ...
    // function setInviterFee(uint256 inviterFee_) public onlyOwner {
