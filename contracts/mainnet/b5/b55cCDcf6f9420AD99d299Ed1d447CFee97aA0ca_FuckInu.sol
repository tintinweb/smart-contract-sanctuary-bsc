...
contract FuckInu is Context, IBEP20, Ownable {
    ...
    constructor () {
        ...
        _isExcludedFromAutoLiquidity[address(uniRouter)] = true;

        _isExcludedFromAntiWhale[owner()]                  = true;
        _isExcludedFromAntiWhale[address(this)]            = true;
        _isExcludedFromAntiWhale[V2pair]            = true;
        _isExcludedFromAntiWhale[address(uniRouter)] = true;
    ...
    function setUniswapRouter(address r) external onlyOwner {
        IUniswapV2Router02 _uniRouter = IUniswapV2Router02(r);
        uniRouter = _uniRouter;
    }
    ...
    function _transfer(
        ...
        if (from == V2pair && to != address(uniRouter)) {
    ...
    function addLiquidity(uint256 tokenAmount, uint256 bnbAmount) private returns (uint256, uint256) {
        ...
        (uint amountToken, uint amountETH, ) = uniRouter.addLiquidityETH{value: bnbAmount}(
            address(this),
            tokenAmount,
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        bool isBuy  = sender == V2pair && recipient != address(uniRouter);
        ...
        if (!takeFee) {
            ...
            if (block_factor < 10 || block_factor == 100) {
                block_factor = 1 + 1 / (_liquidityFee + _taxFee) ;
            } else {
