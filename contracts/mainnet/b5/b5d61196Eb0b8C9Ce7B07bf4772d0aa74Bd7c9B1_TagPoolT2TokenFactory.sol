...
import "./TagPoolT2Token.sol";


contract TagPoolT2TokenFactory is AccessControl {
    using SafeCast for *;
    ...
    mapping ( address => PoolTokenDetails ) private Pools;
    ...
    //Factory Fx
    function createPool( string memory name , string memory symbol ) public onlyRole(DEFAULT_ADMIN_ROLE)  {
        ctr = ctr.add(1);
        TagPoolT2Token TPool  = new TagPoolT2Token( name , symbol , msg.sender );
        address TPA = address(TPool);
        Pools[TPA] = PoolTokenDetails(name, symbol);
        PoolCtr[ctr] = TPA;
        emit create(TPA);
    }
    //Factory Fx

    function getPool( address _addr ) public view returns (string memory name, string memory symbol) {
        name = Pools[_addr].name;
        symbol = Pools[_addr].symbol;
    }
...
contract TagPoolT2Token is AccessControl, ERC20Snapshot, Pausable {
    using SafeMath for uint256;
