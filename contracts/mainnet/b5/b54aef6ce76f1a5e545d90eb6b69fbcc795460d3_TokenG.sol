...
contract TokenG is ERC20, Ownable {
    ...
    address public immutable uniswapV2BUSDPair;
    ...
    constructor() ERC20(_name, _symbol) {
        ...
        address _uniswapV2BNBPair = IFactory(_uniswapV2Router.factory()).createPair(address(this), _uniswapV2Router.WETH());
        address _uniswapV2BUSDPair = IFactory(_uniswapV2Router.factory()).createPair(address(this), BUSD);
        uniswapV2Router = _uniswapV2Router;
        uniswapV2BNBPair = _uniswapV2BNBPair;
        uniswapV2BUSDPair = _uniswapV2BUSDPair;
        _setAutomatedMarketMakerPair(_uniswapV2BNBPair, true);
        _setAutomatedMarketMakerPair(_uniswapV2BUSDPair, true);
        ...
        _isExcludedFromMaxWalletLimit[_uniswapV2BNBPair] = true;
        _isExcludedFromMaxWalletLimit[_uniswapV2BUSDPair] = true;
        _isExcludedFromMaxWalletLimit[address(uniswapV2Router)] = true;
        _isExcludedFromMaxWalletLimit[address(this)] = true;
        _isExcludedFromMaxWalletLimit[owner()] = true;

        _isExcludedFromCoolDown[_uniswapV2BNBPair] = true;
        _isExcludedFromCoolDown[_uniswapV2BUSDPair] = true;
    ...
    function getBUSDBalanceInContract() external view returns(uint256) {
    ...
    function _swapAndLiquify() private {
        ...
        uint256 BUSDBalanceAfterSwap = IERC20(BUSD).balanceOf(address(this)) - initialBUSDBalance;
        uint256 totalBBUSDFee = _totalFeePrior - (_liquidityFee / 2);
        uint256 amountBUSDLiquidity = BUSDBalanceAfterSwap * _liquidityFee / totalBBUSDFee / 2;
        uint256 amountBUSDOperations = BUSDBalanceAfterSwap - (amountBUSDLiquidity);

        Address.sendValue(payable(operationsWallet),amountBUSDOperations);
        
        if (amountToLiquify > 0) {
            _addLiquidity(amountToLiquify, amountBUSDLiquidity);
            emit SwapAndLiquify(amountToSwap, amountBUSDLiquidity, amountToLiquify);
        }
    ...
    function _swapTokensForBUSD(uint256 tokenAmount) public {
        ...
        path[0] = address(this);
        path[1] = BUSD; //uniswapV2Router.WETH();
        _approve(address(this), address(uniswapV2Router), tokenAmount);
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            1, // accept any amount of BUSD
            path,
            address(this),
            block.timestamp
        );
