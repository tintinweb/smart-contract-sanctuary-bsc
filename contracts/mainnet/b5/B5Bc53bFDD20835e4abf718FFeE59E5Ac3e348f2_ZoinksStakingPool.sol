...
contract ZoinksStakingPool is Ownable, ReentrancyGuard {
    ...
    // The reward token
    IERC20 public rewardToken; //reward zoinks token
    IERC20 public rewardSnacksToken;
    ...
    uint256 public totalStakedZoinks; // Total staked amount = totalSupply() - reward
    uint256 public totalRewardDebtZoinks; // Total Reward debt of zoinks
    uint256 public totalRewardDebtSnacks; // Total Reward debt of snacks
    ...
    constructor(address _seniorage) {
        seniorageAddress = _seniorage; // Ex.: 0x467927774B59F7cB023863b07960669f958EC19a
    }
    ...
    function deposit(uint256 amount) external {
        ...
        if (amount > 0) {
            ...
            totalStakedZoinks = totalStakedZoinks + amount;
            stakedToken.safeTransferFrom(msg.sender, address(this), amount);
            user.lastDepositDate = block.timestamp;
            addUser();
        }
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (block.timestamp < user.lastDepositDate + 1 days) {
            stakedToken.safeTransfer(seniorageAddress, _amount.div(2));
            stakedToken.safeTransfer(msg.sender, _amount.div(2));
    ...
    function emergencyWithdraw() external nonReentrant {
        ...
        if (block.timestamp < user.lastDepositDate + 1 days) {
            ...
            stakedToken.safeTransfer(msg.sender, amountToTransfer.div(2));
        } else {
    ...
    function updatePool() external {
        // ! who is actor to give acces?

        require(
            totalStakedZoinks > 0,
            "0 staked zoinks"
        );
        uint256 rewardZoinks = rewardToken.balanceOf(address(this)) -
            totalRewardDebtZoinks;
        ...
        if (rewardZoinks > 0) {
            for (uint256 i = 0; i < userAddressList.length; i++) {
                userInfo[userAddressList[i]].rewardDebt += rewardZoinks
                    .mul(userInfo[userAddressList[i]].amount)
                    .div(totalStakedZoinks);
            }

            totalRewardDebtZoinks = totalRewardDebtZoinks.add(rewardZoinks);
        }

        uint256 rewardSnacks = rewardSnacksToken.balanceOf(address(this)) -
            totalRewardDebtSnacks;
        ...
        if (rewardSnacks > 0) {
            for (uint256 i = 0; i < userAddressList.length; i++) {
                userInfo[userAddressList[i]].rewardDebtSnacks += rewardSnacks
                    .mul(userInfo[userAddressList[i]].amount)
                    .div(totalStakedZoinks);
            }
    ...
    function rewardTransfer() internal {
        ...
        if (rewardDebt > 0) {
            rewardToken.safeTransfer(msg.sender, rewardDebt);
        ...
        if (rewardDebtSnacks > 0) {
            rewardSnacksToken.safeTransfer(msg.sender, rewardDebtSnacks);
