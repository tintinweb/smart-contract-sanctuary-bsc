...
contract SphynxMasterChef is Ownable {
	...
	struct PoolInfo {
		...
		uint256 accSphynxPerShare; // Accumulated Sphynxs per share, times 1e18. See below.
	}
	...
	function pendingSphynx(uint256 _pid, address _user) external view returns (uint256) {
		...
		if (block.number > pool.lastRewardBlock && lpSupply != 0) {
			...
			accSphynxPerShare = accSphynxPerShare.add(sphynxReward.mul(1e18).div(lpSupply));
		}
		return user.amount.mul(accSphynxPerShare).div(1e18).sub(user.rewardDebt);
	}
	...
	function updatePool(uint256 _pid) public {
		...
		pool.accSphynxPerShare = pool.accSphynxPerShare.add(sphynxReward.mul(1e18).div(lpSupply));
		pool.lastRewardBlock = block.number;
	}
	...
	function deposit(uint256 _pid, uint256 _amount) public {
		...
		updatePool(_pid);
		if (user.amount > 0) {
			uint256 pending = user.amount.mul(pool.accSphynxPerShare).div(1e18).sub(user.rewardDebt);
			if (pending > 0) {
		...
		user.rewardDebt = user.amount.mul(pool.accSphynxPerShare).div(1e18);
