...
contract LUPIN is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
//custom
    IUniswapV2Router02 public uniswapV2Router;
//string
    string private _name = "LUPIN QUEST";
    string private _symbol = "LUPIN";
//bool
...
//address
    ...
    address public _MarketingWalletAddress = 0x1F9c789309c673e0d0Ec26D475c6a6753A1A16F0;
    address public _RndWalletAddress = 0x1F9c789309c673e0d0Ec26D475c6a6753A1A16F0;
...
//uint
    ...
    uint public buyRndFee = 3;
    uint public sellRndFee = 3;
    
    uint public buyFee = buyReflectionFee + buyLiqFee + buyMarketingFee + buyRndFee;
    uint public sellFee = sellReflectionFee + sellLiqFee + sellMarketingFee + sellRndFee;
    uint public buySecondsLimit = 5;
    uint public minimumTokensBeforeSwap = 7000000 * (10**9); 
    ...
    uint private _MnmReflectionFee;
    uint private _MnmLiqFee;
    uint private _MnmMarketingFee;
    uint private _MnmRndFee;
    uint private _ReflectionFee;
    uint private _LiqFee;
    uint private _MarketingFee;
    uint private _RndFee;
...
// constructor
    constructor() {
        ...
        excludedFromFees[_RndWalletAddress] = true;
    ...
    function setSwapToBNB(bool liq, bool swap) external onlyOwner {
        addLiq = liq;
        swapBnbActive = swap;
    }
    function setFees() private {
        buyFee = buyReflectionFee + buyLiqFee + buyMarketingFee + buyRndFee;
        sellFee = sellReflectionFee + sellLiqFee + sellMarketingFee + sellRndFee;
    }
    function setBuyFee(uint buyReflection, uint buyLiq, uint buyMarketing, uint buyRnd) external onlyOwner() {
        buyReflectionFee = buyReflection;
        buyLiqFee = buyLiq;
        buyMarketingFee = buyMarketing;
        buyRndFee = buyRnd;
        setFees();
    }
    function setSellFee(uint sellReflection, uint sellLiq, uint sellMarketing, uint sellRnd) external onlyOwner() {
        sellReflectionFee = sellReflection;
        sellLiqFee = sellLiq;
    ...
    function _getValues(uint256 tAmount) private view returns (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tMarketing, uint256 tRnd) {
        (tTransferAmount, tFee, tLiquidity, tMarketing, tRnd) = _getTValues(tAmount);
        (rAmount, rTransferAmount, rFee) = _getRValues(tAmount, tFee, tLiquidity, tMarketing, tRnd, _getRate());
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tLiquidity, tMarketing, tRnd);
    }

    function _getTValues(uint256 tAmount) private view returns (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tMarketing, uint256 tRnd) {
        tFee = calculateReflectionFee(tAmount);
        tLiquidity = calculateLiquidityFee(tAmount);
        tMarketing = calculateMarketingFee(tAmount);
        tRnd = calculateRndFee(tAmount);
        tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tMarketing).sub(tRnd);
        return (tTransferAmount, tFee, tLiquidity, tMarketing, tRnd);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tMarketing, uint256 tRnd, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 rMarketing = tMarketing.mul(currentRate);
        uint256 rRnd = tRnd.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity).sub(rMarketing).sub(rRnd);
    ...
    function _takeRnd(uint256 tRnd) private {
        uint256 currentRate =  _getRate();
        uint256 rRnd = tRnd.mul(currentRate);
        _rOwned[address(this)] = _rOwned[address(this)].add(rRnd);
        if(_isExcluded[address(this)])
    ...
    function setMnmFees() private {
        _MnmReflectionFee = _ReflectionFee;
        _MnmMarketingFee = _MarketingFee;
         _MnmRndFee = _RndFee;
        _MnmLiqFee = _LiqFee;
    }
    ...
    function restoreFees() private {
        _ReflectionFee = _MnmReflectionFee;
        _LiqFee = 0;
        _MarketingFee = _MnmMarketingFee;
         _RndFee = _MnmRndFee;
    }
    ...
    function sendToWallet(uint amount) private {
        ...
        uint256 Rnd_part = amount * sellRndFee / 100;
        (bool success2, ) = payable(_RndWalletAddress).call{value: Rnd_part, gas: 30000}("");
...
//set functions
 ...
 function setrndAddress(address _value) external onlyOwner {
        _RndWalletAddress = _value;
    }
...
// operational functions
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tMarketing, uint256 tRnd) = _getValues(tAmount);
