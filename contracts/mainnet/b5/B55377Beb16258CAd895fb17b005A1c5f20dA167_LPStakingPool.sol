...
contract LPStakingPool is Ownable {
    ...
    function getLiquidityUSDT() external view returns (uint256) {
        ...
        uint256 total = _calculateLiquidity() + address(pairToken).balance;
        ...
        if (total == 0) {
            ...
            uint256[] memory amountOutMins = IUniswapV2Router02(
                uniswapV2Router).getAmountsOut(total, path);
    ...
    function _calculateLiquidity() private view returns (uint256) {
        uint256 pool = IERC20(address(this)).balanceOf(address(pairToken));
        ...
        if (pool == 0) {
            ...
            uint256[] memory amountOutMins = IUniswapV2Router02(
                uniswapV2Router).getAmountsOut(pool, path);
