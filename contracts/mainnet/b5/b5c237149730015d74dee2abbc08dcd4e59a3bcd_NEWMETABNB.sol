...
contract NEWMETABNB {
	...
	uint[] public					REFERRAL_PERCENTS				= [150, 100, 70, 30, 20];	
	uint constant public			PROJECT_FEE						= 100;					
	uint constant public			MARKETING_FEE					= 150;					
	uint constant public			MAX_WITHDRAW_AMOUNT				= 5 ether;				
	...
	uint constant public			INSURANCE_PERCENT				= 200;					
	uint constant public			INSURANCE_LOWBALANCE_PERCENT	= 250;					
	uint constant public			REINVEST_PERCENT				= 100;					
	...
	constructor(address payable _walletProject, address payable _walletMarketing) {
		...
		INSURANCE_CONTRACT = payable(new INSURANCE(_walletProject));
