 ...
 */
contract FireCatRecommend is IFireCatRecommend, FireCatTransfer, ModifyControl {
    ...
    /// @inheritdoc IFireCatRecommend
    function addReward(address user, uint256 addAmount) external onlyRole(FIRECAT_VAULT) returns (uint256) {
        require(IERC20(rewardToken).balanceOf(msg.sender) >= addAmount, "RES:E01");
        uint totalRewardNew;
        uint actualAddAmount;

        actualAddAmount = doTransferIn(rewardToken, msg.sender, addAmount);
        // totalRewardNew + actualAddAmount
        totalRewardNew = totalReward.add(actualAddAmount);

        /* Revert on overflow */
        require(totalRewardNew >= totalReward, "RES:E02");

        totalReward = totalRewardNew;
        _userReward[user] = _userReward[user].add(actualAddAmount);

        emit AddReward(user, actualAddAmount, totalRewardNew);
