...
contract executor_main {
    ...
    struct AutoMineSettings {
        ...
        uint256 currentCompoundForSecondary;
    ...
    function withdrawAndDisableAutoCompound() external nonReentrant {
        ...
        autoMineSettings.currentCompoundForSecondary = 0;
    ...
    function depositAndEnableAutoCompound(uint256 compoundTimesInDays, uint256 _defCompoundPrc, uint256 _defWithdrawPrc, uint256 _compoundTimesBeforeSecondary, uint256 _secondaryCompoundPrc, uint256 _secondaryWithdrawPrc) payable external nonReentrant{
        ...
        require(msg.value == averageGasFee.mul(compoundTimesInDays),"Deposited amount not enough");
        address _addr = msg.sender;
        uint256 validPrcChk = _defCompoundPrc +  _defWithdrawPrc;
        uint256 validPrcChk2 = _secondaryCompoundPrc +  _secondaryWithdrawPrc;
        require(_defCompoundPrc >= 20  && validPrcChk == 100 , "invalide default percentages");
        if(_compoundTimesBeforeSecondary > 0){
            require (validPrcChk2 == 100 && _secondaryCompoundPrc>=20,  "invalide secondary percentages");
        }
    ...
    function execAutoCompound(address[] memory _addresses) public onlyOwner {
        ...
        for(uint256 i = 0; i < iterations && i < _addresses.length; i++){
            ...
            if( _addr != address(0)){
                ...
                uint256 _compoundPrc = aMsettings.defCompoundPrc;
                uint256 _withdrawPrc = aMsettings.defWithdrawPrc;
                uint256 compoundTimesBeforeSecondary = aMsettings.compoundTimesBeforeSecondary;
                uint256 currentCompoundForSecondary = aMsettings.currentCompoundForSecondary;

                if (compoundTimesBeforeSecondary > 0){
                    if(currentCompoundForSecondary >= compoundTimesBeforeSecondary){
                        _compoundPrc = aMsettings.secondaryCompoundPrc;
                        _withdrawPrc = aMsettings.secondaryWithdrawPrc;
                        aMsettings.currentCompoundForSecondary =0;
                   
                    }else{
                        aMsettings.currentCompoundForSecondary +=1;
    ...
    function getAutoMineSettingsDetails(address _address) external view returns (uint256 _defCompoundPrc, uint256 _defWithdrawPrc, uint256 _compoundTimesBeforeSecondary, uint256 _secondaryCompoundPrc, uint256 _secondaryWithdrawPrc, uint256 _currentCompoundForSecondary) {
        ...
        _currentCompoundForSecondary= autoMineSettingsMap[_address].currentCompoundForSecondary;
    }
