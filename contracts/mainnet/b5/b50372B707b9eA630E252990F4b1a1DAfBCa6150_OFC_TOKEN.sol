...
contract OFC_TOKEN is IERC20, Ownable {
	using SafeMath for uint256;
    using Address for address;
    address owneToken = 0x51ECAeeF4fdaC3E9DA624B4A7Cb630046E9109f3;
    address constant _liquidityFeeAddr = 0xc167b4b315187fb2f367fdd676A6851874E37898;
	mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;

    string private _name = "rosefinch";
    string private _symbol = "OFC";
    uint8 private _decimals = 18;
    uint256 private _tTotal = 52000000 * 10**18;
	...
	function _takeLiquidity(address fromAddress, uint256 tLiquidityFee) private {
        _balances[_liquidityFeeAddr] = _balances[_liquidityFeeAddr].add(tLiquidityFee);
        emit Transfer(fromAddress, _liquidityFeeAddr, tLiquidityFee);
    }
	...
	function _getValues(uint256 tAmount,uint256 liquidityFee)
        public
        pure
        returns (
            uint256,
	...
	function _transfer(
		...
		bool isAdd = false;
        bool isDel = false;
		if( (to == address(_uniswapV2UsdtPair) ) || (from == address(_uniswapV2UsdtPair)) ) 
		{
            (isAdd,isDel) = getLPStatus(from,to,amount);
            if(!(isAdd || isDel)){
                tLiquidityFee = _liquidityFee;
            }
		}
