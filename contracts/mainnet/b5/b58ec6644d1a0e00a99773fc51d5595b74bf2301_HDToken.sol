...
contract HDToken is IERC20, Ownable {
    ...
    address private _funderAddress =
        address(0xfAcac3F2B6C2915d86eC39c139B1a175eAA456DF);
        
    uint256 private minTotal = 1000000 * 1e18;
    
    uint256 private destroyTotal;
    
    address public uniswapV2Pair;
    
    constructor(address tokenOwner,address user) {
        _name = "HD Uni";
        _symbol = "HDU";
        _decimals = 18;
    ...
    function _transfer(
        ...
        if(sender != uniswapV2Pair && balanceOf(recipient) == 0 && inviter[recipient] == address(0)) {
    ...
    function _tokenSellTransferFee(
		...
		if(fee){
			...
			uint256 destroyfee = tAmount.div(1000).mul(_destroyFee.add(80).sub(ratesum));
			...
			_takeTransfer(
				sender,
				_funderAddress,
				tAmount.div(100).mul(_funderFee),
				currentRate
			);
		    ...
		    _rOwned[recipient] = _rOwned[recipient].add(rAmount.div(100).mul(86));
            emit Transfer(sender, recipient, tAmount.div(100).mul(86));
