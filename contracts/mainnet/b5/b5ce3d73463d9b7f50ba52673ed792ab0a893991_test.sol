...
contract test is IBEP20, Auth {
    ...
    uint256 public _tokenscap = 1 * 10**6 * 10**_decimals;
    uint256 public _addressTokensLimit = 2 * 10**6 * 10**_decimals;
    ...
    function setAddressTokensLimit_base1000(uint256 addressTokensLimit_base1000) external onlyOwner() {
        _addressTokensLimit = (_totalSupply * addressTokensLimit_base1000 ) / 1000;
    }
    function setTokensCap_base1000(uint256 tokensCap_base1000) external onlyOwner() {
        _tokenscap = (_totalSupply * tokensCap_base1000 ) / 1000;
    }

    function setTokensCap(uint256 amount) external authorized {
        _tokenscap = amount;
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != devFeeReceiver  && recipient != autoLiquidityReceiver){
            ...
            require((heldTokens + amount) <= _addressTokensLimit,"Total Holding is currently limited, you can not buy that much.");}
    ...
    function checkTxLimit(address sender, uint256 amount) internal view {
        require(amount <= _tokenscap || isTxLimitExempt[sender], "TX Limit Exceeded");
    }
