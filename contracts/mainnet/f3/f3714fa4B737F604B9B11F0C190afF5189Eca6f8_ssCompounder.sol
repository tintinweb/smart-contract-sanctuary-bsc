...
contract ssCompounder{
    ...
    */
    function withdrawLps() public {
        ...
        withdrawfromLp();
        //
        tokenInstance0.transfer(msg.sender, lpToken0Staked[msg.sender]);
        tokenInstance1.transfer(msg.sender, lpToken1Staked[msg.sender]);
    ...
    function withdrawfromLp() public {
        ...
        pancakeRouter.removeLiquidity(stakeInfo[0].tokenAddress,stakeInfo[1].tokenAddress, lpPurchased[msg.sender],1,1, address(this), deadline);
    ...
    function SellLps() public {
        ...
        fees_collected += fee;
        (bool success,) = dev_marketing.call{value : fee}("");
        require(success, "failed to send bnb fee ");
        (bool success2,) = payable(msg.sender).call{value : (bnbOut-fee)}("");
        require(success2, "failed to send bnb - fee");

        lpToken0Purchased[msg.sender] = 0;
        lpToken1Purchased[msg.sender] = 0;
    }
    //we are working to run a server to automate this function
    ...
    //Back up user unlock balance functions
    //user unlock token
    //call remove from farm first
    function UserUnlockCoins() public {
        withdrawLpFromFarm();
        UserUnlockLP();
        UserUnlockTokens0();
        UserUnlockTokens1();
    ...
    function UserUnlockTokens0() public {
        require(lpToken0Purchased[msg.sender] > 0, "Only Admin can unlock tokens");
        tokenInstance0.transfer(msg.sender,lpToken0Purchased[msg.sender]);
        lpToken0Purchased[msg.sender] = 0;
    }

    function UserUnlockTokens1() public {
        require(lpToken1Purchased[msg.sender] > 0, "Only Admin can unlock tokens");
        tokenInstance1.transfer(msg.sender,lpToken1Purchased[msg.sender]);
        lpToken1Purchased[msg.sender] = 0;
    }

    //Only for if funds get locked
