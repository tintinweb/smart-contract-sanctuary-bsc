...
contract PSHP is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "PSHP";
    string private _symbol = "PSHP";
    uint8 private _decimals = 9;

    address payable public marketingWalletAddress = payable(0x96549caaaB9F4aFE79dD2D3C8C186Eadf50480fF); // Marketing Address
    ...
    function setSellTaxes(uint256 newLiquidityTax, uint256 newMarketingTax) external onlyOwner() {
        ...
        _totalTaxIfSelling = _sellLiquidityFee.add(_sellMarketingFee);
        require(_totalTaxIfSelling <= 100, "Sell tax must be less than 10%");
    }
    ...
    function setDistributionSettings(uint256 newLiquidityShare, uint256 newMarketingShare) external onlyOwner() {
        ...
        _totalDistributionShares = _liquidityShare.add(_marketingShare);
        require(_totalDistributionShares <=100, "Distribution Shares must be less than 20%");
    }
