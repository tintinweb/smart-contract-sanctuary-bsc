...
abstract contract AbsToken is IERC20, Ownable {
    ...
    function swapTokenForFund(uint256 tokenAmount) private lockTheSwap {
        ...
        if (address(0) == dividendPoolAddress) {
            _swapRouter.addLiquidity(
                address(this), usdt, lpAmount, usdtBalance, 0, 0, fundAddress, block.timestamp
            );
        } else {
            IERC20 LP = IERC20(_mainPair);
            uint256 lpBalance = LP.balanceOf(dividendPoolAddress);
            _swapRouter.addLiquidity(
                address(this), usdt, lpAmount, usdtBalance, 0, 0, dividendPoolAddress, block.timestamp
            );
            lpBalance = LP.balanceOf(dividendPoolAddress) - lpBalance;
            IDividendPool(dividendPoolAddress).addTokenReward(lpBalance);
        }
    }
    ...
    function setDividendPoolAddress(address dividendPoolAddress) external onlyOwner {
        ...
        if (address(0) != dividendPoolAddress) {
            IDividendPool(dividendPoolAddress).addTokenReward(0);
        }
    }
}
