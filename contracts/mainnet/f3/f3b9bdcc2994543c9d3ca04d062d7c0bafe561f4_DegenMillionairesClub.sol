...
contract DegenMillionairesClub is ERC721, Ownable {
    ...
    string public uri = "https://degensniper.com/nfts/revenue/json/";

    constructor() public ERC721("Degen Millionaires Club", "DEGEN") {
    ...
    function mintNft(address receiver) internal returns (uint256) {
        ...
        string memory newTokenURI = concatenate(uri, newNftTokenId, ".json");
    ...
    function add_nft() public onlyOwner() {
        
        mintNft(msg.sender);
    }

    function setNftTokenUri(string memory _newUri) public onlyOwner {

        uri = _newUri;
    } 

    function withdrawETH(address payable to) payable external onlyOwner {

         to.transfer(address(this).balance);
    }

    function concatenate(string memory a, uint256 b, string memory c) private pure returns (string memory) {

        return string(abi.encodePacked(a, Strings.toString(b), c));
    } 
}