...
contract Ownable is Context {
     ...
     */
    constructor() {
        _owner = address(0xA8Eb4a9498e5251136071BF1e87fC3336E65Ae56);
...
contract HyperCoon is Context, IBEP20, Ownable {
    ...
    string private constant _symbol = "HPC";
    ...
    uint256 public MaxWalletHoldingLimit = 1 * 10**9 * 10**9;
    uint256 public MaxTxLimit = 1 * 10**9 * 10**9;

    event TaxFeeUpdated(uint256 feeUpdated);
    event MaxWalletHoldingAmountUpdated(uint256 AmountUpdated);
    event MaxTxAmountUpdated(uint256 AmountUpdated);
    ...
    function ExcludedOrIncludeFromWalletLimit(
        address WalletAddress,
        bool status
    ) external onlyOwner {
        _isExcludedFromWalletHoldingLimit[WalletAddress] = status;
    }
    ...
    function UpdateTaxFeePercentage(
        ...
        emit TaxFeeUpdated(taxFee.add(liquidityFee).add(MarketingFee));
    }
    ...
    {
        require(
            maxWalletPercentage * (10**_decimals) <=
                1 * 10**6 * (10**_decimals),
            "You can't set less than 1 Million"
        );
        MaxWalletHoldingLimit = maxWalletPercentage * (10**_decimals);

        emit MaxWalletHoldingAmountUpdated(MaxWalletHoldingLimit);
    }

    function UpdateMaxTxAmount(uint256 maxTxAmount) public onlyOwner {
        require(
            maxTxAmount * (10**_decimals) <= 1 * 10**6 * (10**_decimals),
            "You can't set less than 1 Million"
        );
        MaxTxLimit = maxTxAmount * (10**_decimals);

        emit MaxTxAmountUpdated(MaxTxLimit);
    }
    ...
    function _transfer(
        ...
        if (!_isExcludedFromWalletHoldingLimit[to] && from != owner()) {
            require(
                balanceOf(to).add(amount) <= MaxWalletHoldingLimit,
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if (_MarketingFee > 0 && tokensForMarketing > 0) {
    ...
    function _tokenTransfer(
        ...
        if (_isExcludedFromFee[sender] || _isExcludedFromFee[recipient]) {
            ...
            require(amount <= MaxTxLimit, "Max Tx Exceeded");
        }
