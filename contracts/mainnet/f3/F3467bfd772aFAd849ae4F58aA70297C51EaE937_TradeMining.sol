...
contract TradeMining is Ownable {
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    //=========== External functions ============//
    function harvest(bytes memory _signature, address _user, uint _amount, uint _expire, address _to)
        external
    {
        ...
        address _receiver;
        {
            ...
            transferReward(_receiver, _amount);
        }
        emit Harvest(_user, _amount, _nonce, _receiver);
    }

    function transferReward(address _receiver, uint256 _amount) public {
    ...
    function depositPendingPan(uint _amount) external {
        PAN.safeTransferFrom(msg.sender, address(this), _amount);
    }
