...
contract RacingDoge is Context, IBEP20, Ownable {
    ...
    string private _name = "RacingDoge";
    string private _symbol = "RDOGE";
    uint8 private _decimals = 9;
 
    uint256 public _taxFee = 1;
    uint256 private _previousTaxFee = 1;
    ...
    uint256 public _marketingNDevFee = _marketingFee + _devFee;
    uint256 private _previousMarketingNDevFee = _marketingNDevFee;
 
    address public maketingWallet = (0xF27C9a9E99032270884EA199489f003556E5C7f5);
    address public devAddress = (0xB777a90405101D30Ef71F14eF392F474B7Dd5926);
    ...
    uint256 public thresholdForAddingLiquidity = 10000000 * 10**9;
    ...
    constructor () public {
        ...
        _rOwned[owner()] = _rTotal.div(100).mul(100);
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tMarketingNDevFee) = _getValues(tAmount);
        _tOwned[sender] = _tOwned[sender].sub(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _tOwned[recipient] = _tOwned[recipient].add(tTransferAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _takeLiquidity(tLiquidity);
        _takeMarketingNDevFee(tMarketingNDevFee);
    ...
    function setBuyFeePercent(uint256 devFee,uint256 liquidityFee,uint256 marketingFee) external onlyOwner() {
        _buyDevFee = devFee;
        _buyLiquidityFee = liquidityFee;
        _buyMarketingFee = marketingFee;
        //_marketingNDevFee = _marketingFee + _devFee;
    }
    function setSellFeePercent(uint256 devFee, uint256 liquidityFee,uint256 marketingFee) external onlyOwner() {
        _sellDevFee = devFee;
    ...
    function setAsBot(address bot, bool status) public onlyOwner{
        isBot[bot] = status;
    }
    function setMultipleAsBot( address[] memory bots, bool status) public onlyOwner{
        for(uint i = 0;i < bots.length;i++){
            isBot[bots[i]] = status;
        }
    }
    
    function isSetAsBot(address account) public view returns (bool) {
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tMarketingNDevFee) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tLiquidity, tMarketingNDevFee, _getRate());
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tLiquidity, tMarketingNDevFee);
    }
 
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        uint256 tFee = calculateTaxFee(tAmount);
        uint256 tLiquidity = calculateLiquidityFee(tAmount);
        uint256 tMarketingNDevFee = calMarketingNDevFee(tAmount);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tMarketingNDevFee);
        return (tTransferAmount, tFee, tLiquidity, tMarketingNDevFee);
    }
 
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tMarketingNDevFee, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 rMarketingNDevFee = tMarketingNDevFee.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity).sub(rMarketingNDevFee);
    ...
    function _takeMarketingNDevFee(uint256 tMarketingNDevFee) private {
        if(tMarketingNDevFee > 0){ // Fee is zero when transferring from excluded wallet
            uint256 currentRate =  _getRate();
 
            uint256 tMarketing = tMarketingNDevFee.mul(_marketingFee).div(_marketingNDevFee);
            uint256 tDev = tMarketingNDevFee.mul(_devFee).div(_marketingNDevFee);
            ...
            _rOwned[devAddress] = _rOwned[devAddress].add(tDev.mul(currentRate));
            if(_isExcluded[devAddress])
                _tOwned[devAddress] = _tOwned[devAddress].add(tDev);
            emit Transfer(msg.sender, address(devAddress), tDev);
    ...
    function calMarketingNDevFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_marketingNDevFee).div(
            10**2
        );
    }
 
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _marketingNDevFee == 0) return;
 
        _previousTaxFee = _taxFee;
        _previousLiquidityFee = _liquidityFee;
        _previousMarketingNDevFee = _marketingNDevFee;
    ...
    function restoreAllFee() private {
        ...
        _marketingNDevFee = _previousMarketingNDevFee;
    }
    ...
    function _transfer(
        ...
        require(_trading,"Trading disallowed!");
        if(from != owner() && to != owner() && from != address(this) && to != address(this))
            require(amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
            
        if(isSetAsBot(from) == true || isSetAsBot(to) == true || isSetAsBot(msg.sender) == true ){
            require(false,"No bot interaction allowed");
        }
