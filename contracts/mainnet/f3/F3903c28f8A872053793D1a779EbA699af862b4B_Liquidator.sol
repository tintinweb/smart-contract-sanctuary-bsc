...
contract Liquidator is IPancakeCallee {
    ...
    function pancakeCall(address sender, uint amount0, uint amount1, bytes calldata data) override external {
        ...
        uint swapAmountMax = seized_uUnits * 105 / 100;
        ...
        IERC20(seizeUToken).safeApprove(ROUTER, swapAmountMax);
        ...
        // IPancakeRouter(ROUTER).swapTokensForExactTokens(debt, seized_uUnits, path, address(this), now + 1 minutes);
        IPancakeRouter(ROUTER).swapTokensForExactTokens(debt, swapAmountMax, path, address(this), now + 1 minutes);
