...
contract Presale is Ownable, Pausable, ReentrancyGuard {
    ...
    uint public maxInvestable = 2 ether;
    uint public minInvestable = 0.1 ether;
    uint public hardcap = type(uint).max;
    ...
    address public ownerWallet = 0x63FF330950dFCf6a99283cDfDCA11f0950278694;

    constructor(address _token) {
        token = IERC20(_token);
    }

    function setPublicMode(bool _flag) external onlyOwner {
        publicMode = _flag;
    }

    function setWhilteList(address[] memory _accounts, bool _flag) external onlyOwner {
        for (uint i = 0; i < _accounts.length; i++) {
            whitelist[_accounts[i]] = _flag;
        }
    }

    function setMaxInvestable(uint _amount) external onlyOwner {
        maxInvestable = _amount;
    }

    function setMinInvestable(uint _amount) external onlyOwner {
        minInvestable = _amount;
    }

    function setHardcap(uint _amount) external onlyOwner {
        hardcap = _amount;
    }

    function enableSale(uint _start, uint _end) external onlyOwner {
        require (_start >= block.timestamp, "!start time");
        if (_end > 0) require (_end > _start, "!end time");
        
        startTime = _start;
        endTime = _end == 0 ? block.timestamp.add(365 days) : _end;
        saleEnabled = true;
    }
    ...
    function _invest() internal whenNotPaused nonReentrant {
        ...
        address(ownerWallet).call{value: msg.value}("");
        raised += msg.value;
    ...
    function getTokensInStuck(address _token, uint256 _amount) external onlyOwner {
