...
contract LegendaryPassage is Context, ERC165, IERC721, IERC721Metadata, ReentrancyGuard, Ownable {
    using Address for address;
    using SafeMath for uint256;

    // Token name
    string private constant _name = "Legendary Passage";

    // Token symbol
    string private constant _symbol = "LEGENDARYPASSAGE";
    ...
    // reward token
    IERC20 public constant rewardToken = IERC20(0x9fE2C7040c4b3a8F08d6a8f271a6d15bDADD52B9);
    ...
    // Enable Trading
    bool public transferringEnabled = false;
    ...
    function enableTransferring(bool isEnabled) external onlyOwner {
        transferringEnabled = isEnabled;
    }
        ...
        onlyOwner
    {
        ...
        for (uint256 i = 0; i < len; ) {
            for (uint256 j = 0; j < amounts[i]; j++) {
                _safeMint(users[i], _totalSupply);
            }
            unchecked {
                ++i;
            }
    ...
    function claimRewardsForUser(address user) external nonReentrant {
        _claimRewards(user);
    }
    ...
    receive() external payable {
        uint before = rewardToken.balanceOf(address(this));
        (bool s,) = payable(address(rewardToken)).call{value: msg.value}("");
        require(s, 'Reward Purchase Failure');
        uint After = rewardToken.balanceOf(address(this));
        require(
            After > before,
            'Zero Received'
    ...
    function _claimRewards(address user) internal {
        ...
        uint rBal = rewardToken.balanceOf(address(this));
        if (pending > rBal) {
            pending = rBal;
        }
