...
contract IKI is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    IKIDividendTracker public dividendTracker;
    ...
    constructor() BEP20("IKI", "K") 
    {
        dividendTracker = new IKIDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "IKI: The dividend tracker already has that address");
        IKIDividendTracker newDividendTracker = IKIDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "IKI: The new dividend tracker must be owned by the IKI token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "IKI: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "IKI: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "IKI: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "IKI: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "IKI: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "IKI: Cannot update gasForProcessing to same value");
...
contract IKIDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("IKI_Dividend_Tracker", "IKI_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 100 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "IKI_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "IKI_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main IKI contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "IKI_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "IKI_Dividend_Tracker: Cannot update claimWait to same value");
