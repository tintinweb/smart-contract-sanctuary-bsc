...
contract BitXGoldSwap is OwnableUpgradeable, AccessControlUpgradeable {
  ...
  function swapAffiliate(address _affliator, uint256 _amount) public returns (bool) {
    require(initialized == true, "Trade is not started yet");
    require(referrals[_affliator] == true, "Invalid referral's address");
    require((BitX.balanceOf(address(this)) > _amount), "Insufficient Amount in contract");
    ...
    USDT.transferFrom(msg.sender, owner(), rewardamount);
    BitX.transfer(msg.sender, _amount);
    emit Trade(_amount);
    return true;
  }
  ...
  function withdrawBitX(IERC20 _BitX) public onlyOwner returns (bool) {
    ...
    _BitX.transferFrom(address(this), msg.sender, _BitX.balanceOf(address(this)));
