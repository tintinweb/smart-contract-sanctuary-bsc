...
Buy tax(10%) and Sell tax (15%)
    ...
    (max 6 choice tokens)
    The Donation pot is paid from transfer tax instead of large token holdings 
    ...
    Some functions are reserved for a main wallet and marketing wallet or workhelperwallet
...
ORIGINAL TOKENOMICS
Supply
Max Supply: 1_000_000_000_000_000*(10**9); //1 qua tokens 
Buy tax
Tax: 10% (slippage 11%)
4.4% Payouts in  $VICTORY, $BUSD, $BNB, $GLOWV2, and more added later
3% Auto buyback 
1% Gas Refunds 
1% Marketing in $Bnb
.2% Donation
...
Payouts
→ all the sellers tax is given back in multiple tokens → no $VICTORY price dump from selling the sells tax
Out of the seling tax:
30% holders in $VICTORY, $BNB, $BUSD & $GLOWV2 and more added later
39% liquidity to pancakeswap 
19% Marketing
2% lottery
2% Donation in bnb
...
contract  VictoryToken is IBEP20, ReentrancyGuard {
    string public constant symbol = "VICTORY";
    string public constant name = "Victory Token"; 
    uint8 public constant decimals = 9;

    
    uint256 constant initialSupply = 1_000_000_000_000_000*(10**9); //1 qua tokens
...
//   uint32 public PromoPerc;   
 ...
 //Tax reserves and pots kept public for maximum transparency  
    ...
    uint256 private DonationPotBNBcalc;
...
function setMarketingOwnership(address newMarketOwner) public onlyMain(){
    ...
    constructor() {
        ...
        UpdateRegister(0x8c62b5c0Ffe471bb8852E54FD1bFC133234affCB, false);
        _isDonation[0x8c62b5c0Ffe471bb8852E54FD1bFC133234affCB] = true;
        N_Donations++;
        DonationList.push(0x8c62b5c0Ffe471bb8852E54FD1bFC133234affCB);
...
//function called when a new address is sent tokens
  function UpdateRegister(address recipient, bool ExcludedfromTax) private
   ...
   function isDonation(address addr) public view returns (bool)
   {
       return _isDonation[addr];
   }
    ...
    function _PayTeam() private {
     ...
     if(SendBNBfromContract(MarketingBNB, MarketingWallet)==true) 
        ...
        //Donation
         ...
         uint256 L = DonationList.length;
         if((L>0)&&(DonationPotBNB > 0))
           { uint256 Donationshare = DonationPotBNB / N_Donations;
              DonationPotBNB = 0;
            for(uint32 pos = 0 ; pos < L; pos++)
             if(_isDonation[DonationList[pos]])
                SendBNBfromContract(Donationshare, DonationList[pos]);
        }
        }
}

    
       function addtoDonationList(address addr) public onlyAdmin()
    {
        if(_isDonation[addr] == false)
        {
            if(_isregistered[addr] == false) 
            UpdateRegister(addr, false);
        
        _isDonation[addr] = true;
        N_Donations++;
        DonationList.push(addr);
        }
    }
    
        function removefromDonationList(address addr) public onlyAdmin()
    {
        if(_isDonation[addr] == true)
        {
        N_Donations--;
        _isDonation[addr] = false;
        }
    }
...
{   //payouts
    ...
    if(SellTaxBNBPot>10000)  //skip if pot is empty
    {
      ...
      //2% total tax * 100/79.5     = 2.52 % bnb tax portion
        ...
        //2% Donation in bnb     
            DonationPotBNB += SellTaxBNBPot * 252 / 10000;
        
            //39% lq to pancake lq 
     {       
          //  netamount -= LQTax;
     LQPotBNB += SellTaxBNBPot * 2703 / 10000; // half of 39% liquidity tax converted to BNB, rest is tokens on contract
...
function DistributeBuyTax() internal
{
     ...
     {
        //34% buybackpot
        BuybackPotBNB += BuyTaxBNBPot * 34 / 100;
        ...
        DonationPotBNB += BuyTaxBNBPot * 2 / 100;
        BuyTaxBNBPot = 0;
     }    
}
...
{
  uint256 reservedBNBs = LQPotBNB + BuybackPotBNB + HolderPotBNB + DonationPotBNB;
...
function _StartRewardCalculation() private {
   ...
   DonationPotBNBcalc = DonationPotBNB;
