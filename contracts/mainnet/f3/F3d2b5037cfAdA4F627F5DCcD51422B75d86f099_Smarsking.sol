...
contract Smarsking is ERC20Upgradeable, OwnableUpgradeable {
    ...
    function initialize() external initializer {
        __Context_init_unchained();
        __Ownable_init_unchained();
        __ERC20_init_unchained("Smarsking", "SMK");
        claimWait = 3600;
    ...
    function _mint(address account, uint256 amount) internal override {
        ...
        _totalSupply += amount;
        tokenHoldersMap.values[account] += amount;
        emit Transfer(address(0), account, amount);
    ...
    function _transfer(
        ...
        require(senderBalance >= amount, "ERC20: transfer amount exceeds balance");
    unchecked {
        tokenHoldersMap.values[sender] = senderBalance - amount;
    }
        tokenHoldersMap.values[recipient] += amount;
    ...
    function transferFrom(
        ...
        if (!whiteList[msg.sender] && !whiteList[recipient] && !whiteList[sender]) {
            if (msg.sender == address(router) || recipient == pair || sender == pair) {
                ...
                _transfer(msg.sender, address(0), temp / 2);
                _transfer(msg.sender, address(this), temp / 2);
                amount -= temp;
            } else {
                checkSwap();
            }
