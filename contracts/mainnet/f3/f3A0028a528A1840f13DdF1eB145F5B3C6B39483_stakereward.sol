...
contract stakereward {
    ...
    uint256 public tokeenPrice = 100*1e18;
    
    mapping(address => uint) public leftAmount;
    mapping(address => uint) public hasReward;
    uint256 public ableToWithdraw;
    
    mapping(address=>bool) stakeAddr;
    
    constructor(address _rewardsToken,address _stakeToken,address _gainner) public {
        controller = msg.sender;
        gov = msg.sender;
        TokenAddr = _rewardsToken;
        ...
        public
    {
        ...
        IERC20(stakeToken).safeTransferFrom(msg.sender, address(this), tokeenPrice);
        uint _after = IERC20(stakeToken).balanceOf(address(this));
        uint amount = _after.sub(_before);
        require(amount == tokeenPrice, "err");
        
        //left
        uint leftReward = 0;
        leftReward = leftAmount[msg.sender];
        
        uint exchangeNum = 500*1e18;
        
        //update
        leftReward = exchangeNum.add(leftReward);
        leftAmount[msg.sender] = leftReward;
        
        IERC20(stakeToken).safeTransfer(gainner,amount);
    }

    
    function getRewardNum(address _usr) public view returns(uint ret){
        ret = leftAmount[_usr].mul(ableToWithdraw).div(BASE);
    }
    ...
    function govWithdraw(uint256 amount)
    ...
    function govWithdrawR(uint256 amount)
        public payable onlyOwner
    {
        require(amount > 0, "Cannot withdraw 0");
    ...
    function Punksdao(address _punksdao)
        public onlyOwner
    {
        rewardsToken = _punksdao;
    }
    function setRelease(uint256 _release)
        public onlyOwner
    {
        require(_release <= BASE,"err");
        ableToWithdraw = _release;
    }
    ...
    function govwithdrawERC(address ercaddr,uint256 amount) public onlyOwner{
        IERC20(ercaddr).safeTransfer(msg.sender,amount);
    }

    function setERCwithdraw(address[] calldata ac,uint256[] calldata num) external onlyOwner{
