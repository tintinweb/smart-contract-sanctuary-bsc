...
contract FCN is IERC20, Ownable {
    ...
    address public marketAddress = 0x6BC7b25B6aeDf713b698CFCd104E2030138772d2;

    string private _name = "FCN";
    string private _symbol = "FCN";
    uint8 private _decimals = 18;

    uint256 public technologyFee = 300;
    uint256 private _previoustechnologFee;
    ...
    uint256 private _tTotal = 8500 * 10**18;
    ...
    address public technologyAddr = 0x2015613524FAd91c936A4702E8C0b19d05451B58;
    ...
    function settechnologyAddr(address Address)
        public
        onlyOwner{
            technologyAddr = Address;
    }
    ...
    function setWhiteKG(bool trueOrFalse)
        public
        onlyOwner{
 ...
 function setWhiteList(address account) public onlyOwner {
    ...
    function removeAllFee() private {
        _previoustechnologFee = technologyFee;
    ...
    function restoreAllFee() private {
        technologyFee = _previoustechnologFee;
    ...
    function _transfer(
        ...
        if (from == uniswapV2Pair||from == uniswapV2Pair) {
            if(WhiteList){
                require(_isWhiteList[to] , "OnlyIsWhiteList");
            }
...
struct SendData {
    ...
    function _taketechnologyFeeFee(
        address sender,
        uint256 tAmount
    ) private {
         if (technologyFee == 0) return;

        _tOwned[address(0)] = _tOwned[address(technologyAddr)].add(tAmount);

        emit Transfer(sender, address(technologyAddr), tAmount);
    }
    ...
    function _takeInviterFee(
        ...
        _tOwned[address(marketAddress)] = _tOwned[address(marketAddress)].add(tAmount.div(10000).mul(_inviterFee.sub(accurRate)));
        emit Transfer(sender, address(marketAddress), tAmount.div(10000).mul(_inviterFee.sub(accurRate)));
    }
    ...
    function _transferStandard(
        ...
        _taketechnologyFeeFee(sender, tAmount.div(10000).mul(technologyFee));
