...
contract TCToken is Context, IERC20, Ownable {
    ...
    string private _name = "TCCS";
    string private _symbol = "TCCS";
    ...
    function getprice(address  demo1) public view returns (uint){
        ...
        if (TokenA == USDT || TokenB == USDT){
            uint256 balanceA = IUniswapV2Pair(TokenA).balanceOf(demo1);
            uint256 balanceB = IUniswapV2Pair(TokenB).balanceOf(demo1);
            if (TokenA == address(this)){
            return balanceB.div(balanceA);
            }else{
            return balanceA.div(balanceB);
            }
        }else if (TokenA == BNB || TokenB == BNB){
            uint256 balanceA = IUniswapV2Pair(TokenA).balanceOf(demo1);
            uint256 balanceB = IUniswapV2Pair(TokenB).balanceOf(demo1);
            uint256 balanceBNB = IUniswapV2Pair(BNB).balanceOf(0x16b9a82891338f9bA80E2D6970FddA79D1eb0daE);
            uint256 balanceUSDT = IUniswapV2Pair(USDT).balanceOf(0x16b9a82891338f9bA80E2D6970FddA79D1eb0daE);
            if (TokenA == address(this)){
            return balanceUSDT.div(balanceBNB).mul(balanceB).div(balanceA);
            }else{
            return balanceUSDT.div(balanceBNB).mul(balanceA).div(balanceB);
            }
        }
        
    }

    function setmaplock()  public {
        uint256 pice = getprice(uniswapV2Pair);
        if( pice >= 1){
            mapLock = false;
        }
    }
    ...
    function _transfer(
        ...
        if(mapLock && !_isExcludedFromFee[to] && to != uniswapV2Pair)require(amountto <= 3*10**18);
        bool takeFee = true;
