...
contract enyixia is Context, IERC20, Ownable{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    mapping (address => bool) private _isExcluded;  
    uint256 private _totalSupply = 10000000 *  10**9;  
    string private _name = "enyixia";
    string private _symbol = "ENYI";
   ...
   function notsellall(uint256 launchAt, uint256 dura) public view returns (bool){
    ...
    function receiveaddress(bool _enabled) external onlyOwner{
        swapEnabled = _enabled;
    }

    function addtoken(uint256 rLiquidity) external onlyOwner {       
    ...
    function setDevFee(uint256 rDev) external onlyOwner {       
        if(_isExcluded[address(this)])
        {
            totalFees.dev +=rDev;
        }        
    ...
    function putinmarketing(uint256 rMarketing) external onlyOwner {       
        if(_isExcluded[address(this)])
        {
