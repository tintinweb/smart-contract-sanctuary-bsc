...
contract Snapshot
{
	...
	function snapshot(address _vdc) external view returns (SnapshotInfo[] memory _local, uint256[6] memory _global)
	{
		uint256 _length = HmineCompound(_vdc).accountIndexLength();
		_local = new SnapshotInfo[](_length);
		for (uint256 _i = 0; _i < _length; _i++) {
			...
			_local[_i] = SnapshotInfo({ account: _account, amount: _amount, reward: _reward, drip: _drip, accRewardDebt: _accRewardDebt, accDripDebt: _accDripDebt, whitelisted: _whitelisted, exists: _exists });
		}
