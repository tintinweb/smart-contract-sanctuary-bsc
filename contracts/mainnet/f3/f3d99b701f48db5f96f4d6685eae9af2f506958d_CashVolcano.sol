...
contract CashVolcano {
    using SafeMath for uint256;
    uint256 constant launch = 1662368375;
    ...
    constructor() {
            owner = msg.sender;
            PercsKey[10] = DivPercs(604800, 15);    //7 days in seconds 1.5%
            PercsKey[20] = DivPercs(864000, 25);    //10 days in seconds 2.5%
            PercsKey[30] = DivPercs(1296000, 35);   //15 days in seconds 3.5%
            PercsKey[40] = DivPercs(1728000, 45);   //20 days in seconds 4.5%
            PercsKey[50] = DivPercs(2160000, 55);   //25 days in seconds 5.5%
            FeesKey[10] = FeesPercs(604800, 150);   //7 days in seconds 15%
            FeesKey[20] = FeesPercs(864000, 100);   //10 days in seconds 10%
            FeesKey[30] = FeesPercs(1728000, 80);  //20 days in seconds 8%
            FeesKey[40] = FeesPercs(2160000, 60);  //25 days in seconds 6%
            FeesKey[50] = FeesPercs(2592000, 60);  //30 days in seconds 6%
    ...
    function withdrawInitial(uint256 keyy) external {
        ...
        if (currDays < FeesKey[10].daysInSeconds){ // LESS THAN 7 DAYS STAKED
            uint256 minusAmt = initialAmt.mul(FeesKey[10].feePercentage).div(percentdiv); //15% fee
        ...
        } else if (currDays >= FeesKey[10].daysInSeconds && currDays < FeesKey[20].daysInSeconds){ // BETWEEN 7 and 10 DAYS
        ...
        } else if (currDays >= FeesKey[20].daysInSeconds && currDays < FeesKey[30].daysInSeconds){ // BETWEEN 10 and 20 DAYS
            uint256 minusAmt = initialAmt.mul(FeesKey[30].feePercentage).div(percentdiv); //8% fee
        ...
        } else if (currDays >= FeesKey[30].daysInSeconds && currDays < FeesKey[40].daysInSeconds){ // BETWEEN 20 and 25 DAYS
            uint256 minusAmt = initialAmt.mul(FeesKey[40].feePercentage).div(percentdiv); //6% fee
        ...
        } else if (currDays >= FeesKey[40].daysInSeconds && currDays < FeesKey[50].daysInSeconds){ // BETWEEN 25 and 30 DAYS
            uint256 minusAmt = initialAmt.mul(FeesKey[40].feePercentage).div(percentdiv); //6% fee
