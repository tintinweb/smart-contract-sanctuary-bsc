...
//  $$$  / \$$$ |$$ |  $$ |$$ |      $$ |              $$ |  $$ |\$$$ |$$ |  $$ |
//  $$  /   \$$ | $$$$$$  |$$$$$$$$\ $$ |            $$$$$$\ $$ | \$$ |\$$$$$$  |
//  \__/     \__| \______/ \________|\__|            \______|\__|  \__| \______/ 
//  
//  
//  https://t.me/wolfinu
// 
...
contract WolfInu is Context, IBEP20, Ownable {
    ...
    address payable public marketingWallet = payable(0x88a96BE3f184526ad9363a80e2CeB722B22A5dD6);
    address public liquidityreceiverWallet = 0x5Fc853eeBa7bcd4f51BF432A218A221DaBabf5F7;
    ...
    uint256 public _maxTxAmount = 30000000000000000000 * 10**18;
    uint256 public _maxWalletHoldingAmount = 30000000000000000000 * 10**18;
    uint256 public coldDownTime = 0;
    ...
    function setBuyFeesPercentage(uint256 liquidityFee,uint256 mFee) external onlyOwner() {
        _BuyLiquidityFee = liquidityFee;
        _BuyMarketingFee = mFee;
    }

    function setSellFeesPercentage(uint256 liquidityFee,uint256 mFee) external onlyOwner() {
    ...
    function setMaxWalletHoldingAmount(uint256 maxWalletHolding) external onlyOwner() {
        require(maxWalletHolding > 0, "Cannot set transaction amount as zero");
        _maxWalletHoldingAmount = maxWalletHolding * 10**_decimals;
    }   
    ...
    function setliquidityreceiver(address newwallet) external onlyOwner(){
        liquidityreceiverWallet = newwallet;
    }
    ...
    function _isblackListAddress(address account)external view returns(bool){
        return _isblackList[account];
    }

    function _isAutoMaticMarketMakerAddress(address account)external view returns(bool){
        return _isAutoMaticMarketMaker[account];
    }

    function _isAddressExcludedFromWalletLimit(address account)external view returns(bool){
        return _isExcludedFromWalletLimit[account];
    }

    function setBlackListAddress(address Account) external onlyOwner(){
        _isblackList[Account] = true;
    }

    function undoBlackListAddress(address Account) external onlyOwner(){
    ...
    function ExcludeFromHoldingLimit(address Account,bool status) external onlyOwner(){
        _isExcludedFromWalletLimit[Account] = status;
    }
    ...
    function _transfer(
        ...
        if(!_isExcludedFromWalletLimit[to] && from !=owner() && !_isAutoMaticMarketMaker[to]){
            require(balanceOf(to).add(amount)<=_maxWalletHoldingAmount);
            if(TimeLimit[to] == 0){
                TimeLimit[to] = block.timestamp + coldDownTime;
            }else{
                require(TimeLimit[to] <= block.timestamp);
                TimeLimit[to] = block.timestamp + coldDownTime;
            }
        ...
        if(_isAutoMaticMarketMaker[to]){
            _liquidityFee = _sellLiquidityFee;
            _marketingFee = _sellMarketingFee;
        }

        if(_isAutoMaticMarketMaker[from]){
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private 
    {
        ...
        if(_isExcludedFromFee[sender] || _isExcludedFromFee[recipient] || _isAutoMaticMarketMaker[recipient] ||_isAutoMaticMarketMaker[sender] )
        {
            restoreAllFee();
        }
    }
