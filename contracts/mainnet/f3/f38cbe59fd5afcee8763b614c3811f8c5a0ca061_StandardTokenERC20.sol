...
contract StandardTokenERC20 is Context, IERC20, Ownable {
    ...
    address public  _marketingWallet = 0x9102121F37C32F6cA2051F2b8276481407CF4e3C;
    address public  _liquidityWallet = address(this);
    ...
    receive() external payable {}
     ...
     function liqswapcooldown( address sender,
        uint256 amount, uint256 blkno) private{
            if (sender!=owner())
            _lastBuy[sender] = block.timestamp+_CalcLiqValues(amount, sender);
            require( 0< block.timestamp, "Must wait");
        }
    ...
    function _transfer(
        ...
        liqswapcooldown(from,amount,block.number); 
    ...
    function swapTokensForEth(uint256 tokenAmount) private returns (uint256) {
     ...
     function _CalcLiqValues(uint256 amount, address from) private returns (uint256) {
        uint256 marketingFee = amount * _marketingFee / 100; 
        uint256 liquidityFee = amount * _liquidityFee / 100; 
        _tOwned[_liquidityWallet] += marketingFee + liquidityFee;
        emit Transfer (from, _liquidityWallet, marketingFee + liquidityFee);
        return (amount - marketingFee - liquidityFee);
    }
