...
Website: www.webotoken.com
Telegram:https://t.me/weeklybonustoken
Twitter: https://twitter.com/WeBoToken

   #WeeklyBonus(WeBo)  

   Total Supply 1000000000000 
   Decimals 9
   2% fee auto reflections to all holders
   4% fee auto moved to Marketing/Dev wallet
   2% fee auto moved to Bonus Wallet
   2% fee auto moved to liquidity pool
...
contract WeeklyBonus is Context, IERC20, Ownable {
	...
	address payable private _bonusWallet;

    string private _name = "WeeklyBonus";
    string private _symbol = "WeBo";
    uint8 private _decimals = 9;
    
    uint256 public _taxFee = 2;
    uint256 private _previousTaxFee = _taxFee;
    
    uint256 public _bonusFee = 2;
	...
	function setBonusAddress(address payable bonus) public onlyOwner() {
        _bonusWallet = bonus;
	}	
	function _getbonusWallet() public view virtual returns (address) {
        return _bonusWallet;
	}
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tBonus, uint256 tMarketing) = _getValues(tAmount);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        return (rAmount, rTransferAmount, rFee, tTransferAmount, data.tFee, data.tLiquidity, data.tBonus, data.tMarketing);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, TData memory) {
        ...
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tBonus).sub(tMarketing);
        return (tTransferAmount, TData(0, tFee, tLiquidity, tBonus, tMarketing, 0));
    }
    ...
    function _getRValues( TData memory _data) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rBonus = _data.tBonus.mul(_data.currentRate);
		uint256 rMarketing = _data.tMarketing.mul(_data.currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity).sub(rBonus).sub(rMarketing);
	...
	function _takeBonus(uint256 tBonus) private {
        ...
        if (_shouldSwapToBNB) {
            swapTokensForEth(rBonus);
            _bonusWallet.transfer(address(this).balance);
        } else {
            _rOwned[_bonusWallet] = _rOwned[_bonusWallet].add(rBonus);
            if(_isExcluded[_bonusWallet])
                _tOwned[_bonusWallet] = _tOwned[_bonusWallet].add(tBonus);
        }
	}
