...
contract Consumption is Ownable {
    ...
    IConsumptionNFT nft = IConsumptionNFT(0x87A192FB59516C2feB245dcB45f578f2d9DdA898);
    ...
    mapping(address => uint) private userBurn;
    ...
    event NewOrder(address indexed user, uint32 indexed tid, uint amount, uint stime);
    event TakeBack(address indexed user, uint amount, uint stime);
    ...
    function purchase(uint pay) external {
        ...
        OrderInfo storage o = orderInfo[orders];
        o.user = msg.sender;
        o.tid = orders;
        o.amount = amount;
        o.stime = block.timestamp;
        // o.ntime = block.timestamp + 14 * SECONDS_PER_DAY;
        o.ntime = block.timestamp + 10 * 60;
        o.reward = 0;


        userOrders[msg.sender].push(orders);
        emit NewOrder(msg.sender, orders, amount, block.timestamp);
    ...
    function takeBack() external {
        uint256 totalRewards = 0;
        for (uint i = 0; i < userOrders[msg.sender].length; i++) {
            uint32 ind = userOrders[msg.sender][i];
            OrderInfo memory o = orderInfo[ind];
            if (block.timestamp >= o.ntime && o.reward < o.amount) {
                (uint di, uint rate) = _earned(o.stime, block.timestamp);
                if (rate == 25) {
                    totalRewards += o.amount - o.reward;
                    orderInfo[userOrders[msg.sender][i]].reward += o.amount - o.reward;
                } else {
                    totalRewards += o.amount * rate / 100;
                    orderInfo[userOrders[msg.sender][i]].reward += o.amount * rate / 100;
                }

                // orderInfo[userOrders[msg.sender][i]].ntime = o.stime + (di + 1) * 14 * SECONDS_PER_DAY;
                orderInfo[userOrders[msg.sender][i]].ntime = o.stime + (di + 1) * 10 * 60;
                
            }
        }
        nft.mint(0, msg.sender, uint32(totalRewards));
        emit TakeBack(msg.sender, totalRewards, block.timestamp);
    }
    ...
    function earned(address user) public view returns (uint256) {
        ...
        for (uint i = 0; i < userOrders[user].length; i++) {
            uint32 ind = userOrders[user][i];
            OrderInfo memory o = orderInfo[ind];
            if (block.timestamp >= o.ntime && o.reward < o.amount) {
                (, uint rate )= _earned(o.stime, block.timestamp);
                if (rate == 25) {
                    totalRewards += o.amount - o.reward;
                } else {
                    totalRewards += o.amount * rate / 100;
    ...
    function _earned(uint fromTimestamp, uint toTimestamp) private pure returns (uint, uint) {
        uint diffTimestamp = toTimestamp - fromTimestamp;
        // uint i = diffTimestamp / (14 * SECONDS_PER_DAY);
        uint i = diffTimestamp / (10 * 60);
        if (i == 1) return (i, 15);
        if (i == 2 || i == 3 || i == 4) return (i, 20);
        if (i == 5) return (i, 25);
        return (i, 0);
    }

    function getUserNftInfo(address user) public view returns (uint released, uint noReleased) {
        for (uint i = 0; i < userOrders[user].length; i++) {
            uint32 ind = userOrders[user][i];
            OrderInfo memory o = orderInfo[ind];
            released += o.reward;
            noReleased += o.amount - o.reward;
        }

        if (released > userBurn[user]) {
            released -= userBurn[user];
        } else {
            released = 0;
        }
    }
    ...
    function getFeeMarket() public view returns (address) {
        return feeMarket;
    }
