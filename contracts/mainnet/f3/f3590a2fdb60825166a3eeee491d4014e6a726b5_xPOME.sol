...
contract xPOME {
  string public name = "xPOME Token";
  string public symbol = "xPOME";
  ...
  modifier onlyOwner() {
    require(owners[msg.sender], "xPOME Token: No owner privilege");
    _;
  }

  modifier onlyMinter() {
    require(minters[msg.sender], "xPOME Token: No minter privilege");
    _;
  }

  modifier Transferable(address _from, address _to) {
    require(allUnfrozen || fromWhitelist[_from] || toWhitelist[_to], "xPOME Token: Transfer restricted");
     _;
  }
  ...
  function removeOwner(address _owner) external onlyOwner returns (bool) {
    require(_owner != msg.sender, "xPOME Token: Cannot remove yourself");
    owners[_owner] = false;
    return true;
  }
  ...
  function transferFrom(address _from, address _to, uint256 _amount) external Transferable(_from, _to) returns (bool) {
    if (approvedAll[msg.sender]) {
      ...
      require(_amount <= allowed[_from][msg.sender], "xPOME Token: insufficient allowance");
  ...
  function _transfer(address _from, address _to, uint256 _amount) private returns (bool) {
    require(_from != address(0), 'xPOME Token: from address not to be zero');
    require(_to != address(0), 'xPOME Token: to address not to be zero');
    require(_amount <= balances[_from], 'xPOME Token: insufficient balance');
