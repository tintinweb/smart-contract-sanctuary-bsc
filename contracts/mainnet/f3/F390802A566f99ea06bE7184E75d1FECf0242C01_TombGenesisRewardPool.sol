...
contract TombGenesisRewardPool {
    ...
    // MAINNET
    uint256 public tombPerSecond;
    uint256 public runningTime = 2 days; // 2 days
    uint256 public constant TOTAL_REWARDS = 200 ether; //old: 25000
    // END MAINNET
    ...
    constructor(
        ...
        tombPerSecond = TOTAL_REWARDS.div(runningTime);
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        UserInfo storage dao = userInfo[_pid][daoFundAddress];
        updatePool(_pid);
        if (dao.amount > 0) {
        ...
        if (_amount > 0) {
            ...
            dao.amount = dao.amount.add(_amount);
        }
        dao.rewardDebt = user.amount.mul(pool.accTombPerShare).div(1e18);
