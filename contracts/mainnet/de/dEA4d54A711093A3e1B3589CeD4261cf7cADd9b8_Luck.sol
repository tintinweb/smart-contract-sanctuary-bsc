...
// File: F22/lib/IUniswapV2Pair.sol
...
contract Luck is Ownable,ERC20{
    ...
    constructor(address _bnb, address _platAddress, address _receiveAddress, address _router) ERC20('Test','Test'){
        ...
        whites[platAddress] = true;
        whites[receiveAddress] = true;
        whites[address(this)] = true;
        whites[address(0x00)] = true;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        if(!whites[sender] && !whites[recipient] && sender != address(router)){
    ...
    function setWhite(address[] memory whiteList, bool flag) public onlyOwner{
        for(uint i=0;i<whiteList.length;i++){
            whites[whiteList[i]] = flag;
        }
    }
    ...
    function setOneDay(uint _oneDay) public onlyOwner{
        oneDay = _oneDay;
    }
}