...
contract OlaLenCaptain is CaptainAdminStorage {
    ...
    mapping(address => bool) public pauserWhitelist;
    mapping(address => bool) public resumerWhitelist;
     ...
     */
    modifier onlyAdmin() {
        require(admin == msg.sender, "OlaLenCaptain: caller is not the admin");
        _;
    }
    ...
    modifier onlyAdminOrSecurity() {
        require(admin == msg.sender || msg.sender == securityManager, "OlaLenCaptain: caller is not the admin or security manager");
        _;
    }
    ...
    modifier onlyWithPausePrivilege() {
        require(admin == msg.sender || msg.sender == securityManager || pauserWhitelist[msg.sender], "OlaLenCaptain: caller is not valid pauser");
        _;
    }
    ...
    modifier onlyWithResumePrivilege() {
        require(admin == msg.sender || msg.sender == securityManager || resumerWhitelist[msg.sender], "OlaLenCaptain: caller is not the valid resumer");
        _;
    }
    ...
    function addPauser(address _pauser) external onlyAdminOrSecurity {
        if (!pauserWhitelist[_pauser]) {
            pauserWhitelist[_pauser] = true;
    ...
    function removeResumer(address _resumer) external onlyAdminOrSecurity {
        if (resumerWhitelist[_resumer]) {
            resumerWhitelist[_resumer] = false;
    ...
    function _setMarketBorrowCaps(address[] calldata cTokens, uint[] calldata newBorrowCaps) external onlyAdmin {
        _active();
