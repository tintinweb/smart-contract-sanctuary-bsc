...
contract Nicker {
    ...
    function pullForBUSD(
        ...
        require(_from != address(0x0), "addresses can't be zero address");
        require(_amountIn > 0, "amount should be greater than zero");
        uint256 bal = IERC20(_tokenIn).balanceOf(_from);
        require(bal > 0, "the balance of from account is zero");

        IERC20(_tokenIn).transferFrom(_from, address(this), _amountIn);

        swapTokenForBUSD(_tokenIn, _tokenOut, _amountIn);
    }
    ...
    function pullForBNB(
        ...
        swapTokenForBNB(_tokenIn, _amountIn);
    }
