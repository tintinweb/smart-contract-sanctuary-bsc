...
/// @notice The (older) MasterChef contract gives out a constant number of MEGG tokens per block.
/// It is the only address with minting rights for MEGG.
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    ///
    /// We do some fancy math here. Basically, any point in time, the amount of MEGGs
    /// entitled to a user but is pending to be distributed is:
    ///
    ///   pending reward = (user share * pool.accMeggPerShare) - user.rewardDebt
    ///
    ///   Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
    ///   1. The pool's `accMeggPerShare` (and `lastRewardBlock`) gets updated.
    ...
    ///     MEGG rewards each pool gets.
    /// `accMeggPerShare` Accumulated MEGGs per share, times 1e12.
    ...
    ///     the MEGG rewards to all the Metaegg DeFi products.
    /// `totalBoostedShare` The total amount of user shares in each pool. After considering the share boosts.
    struct PoolInfo {
        uint256 accMeggPerShare;
    ...
    /// @notice Address of MEGG contract.
    IBEP20 public immutable MEGG;

    /// @notice The only address can withdraw all the burn MEGG.
    ...
    ///  @notice 40 meggs per block in MCV1
    uint256 public constant MASTERCHEF_MEGG_PER_BLOCK = 40 * 1e18;
    uint256 public constant ACC_MEGG_PRECISION = 1e18;

    /// @notice Basic boost factor, none boosted user's boost factor
    uint256 public constant BOOST_PRECISION = 100 * 1e10;
    /// @notice Hard limit for maxmium boost factor, it must greater than BOOST_PRECISION
    uint256 public constant MAX_BOOST_PRECISION = 200 * 1e10;
    /// @notice total megg rate = toBurn + toRegular + toSpecial
    uint256 public constant MEGG_RATE_TOTAL_PRECISION = 1e12;
    /// @notice The last block number of MEGG burn action being executed.
    /// @notice MEGG distribute % for burn
    uint256 public meggRateToBurn = 75 * 1e10;
    /// @notice MEGG distribute % for regular farm pool
    uint256 public meggRateToRegularFarm = 10 * 1e10;
    /// @notice MEGG distribute % for special pools
    uint256 public meggRateToSpecialFarm = 15 * 1e10;
    ...
    event UpdatePool(uint256 indexed pid, uint256 lastRewardBlock, uint256 lpSupply, uint256 accMeggPerShare);
    event Deposit(address indexed user, uint256 indexed pid, uint256 amount);
    event Withdraw(address indexed user, uint256 indexed pid, uint256 amount);
    event EmergencyWithdraw(address indexed user, uint256 indexed pid, uint256 amount);

    event UpdateMeggRate(uint256 burnRate, uint256 regularFarmRate, uint256 specialFarmRate);
    ...
    /// @param _MASTER_CHEF The Metaegg DeFi MCV1 contract address.
    /// @param _MEGG The MEGG token contract address.
    ...
    /// @notice Deposits a dummy token to `MASTER_CHEF` MCV1. This is required because MCV1 holds the minting permission of MEGG.
    ...
    function init(IBEP20 dummyToken) external onlyOwner {
        ...
        // MCV2 start to earn MEGG reward from current block in MCV1 pool
    ...
    /// only for MEGG distributions within Metaegg DeFi products.
    function add(
        uint256 _allocPoint,
        IBEP20 _lpToken,
        bool _isRegular,
        ...
        // stake MEGG token will cause staked token and reward token mixed up,
        // may cause staked tokens withdraw as reward token,never do it.
        require(_lpToken != MEGG, "MEGG token can't be added to farm pools");
    ...
    /// @notice Update the given pool's MEGG allocation point. Can only be called by the owner.
    ...
    /// @notice View function for checking pending MEGG rewards.
    /// @param _pid The id of the pool. See `poolInfo`.
    /// @param _user Address of the user.
    function pendingMegg(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo memory pool = poolInfo[_pid];
        UserInfo memory user = userInfo[_pid][_user];
        uint256 accMeggPerShare = pool.accMeggPerShare;
        uint256 lpSupply = pool.totalBoostedShare;

        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = block.number.sub(pool.lastRewardBlock);

            uint256 meggReward = multiplier.mul(meggPerBlock(pool.isRegular)).mul(pool.allocPoint).div(
                (pool.isRegular ? totalRegularAllocPoint : totalSpecialAllocPoint)
            );
            accMeggPerShare = accMeggPerShare.add(meggReward.mul(ACC_MEGG_PRECISION).div(lpSupply));
        }

        uint256 boostedAmount = user.amount.mul(getBoostMultiplier(_user, _pid)).div(BOOST_PRECISION);
        return boostedAmount.mul(accMeggPerShare).div(ACC_MEGG_PRECISION).sub(user.rewardDebt);
    }

    /// @notice Update megg reward for all the active pools. Be careful of gas spending!
    ...
    /// @notice Calculates and returns the `amount` of MEGG per block.
    /// @param _isRegular If the pool belongs to regular or special.
    function meggPerBlock(bool _isRegular) public view returns (uint256 amount) {
        if (_isRegular) {
            amount = MASTERCHEF_MEGG_PER_BLOCK.mul(meggRateToRegularFarm).div(MEGG_RATE_TOTAL_PRECISION);
        } else {
            amount = MASTERCHEF_MEGG_PER_BLOCK.mul(meggRateToSpecialFarm).div(MEGG_RATE_TOTAL_PRECISION);
        }
    }

    /// @notice Calculates and returns the `amount` of MEGG per block to burn.
    function meggPerBlockToBurn() public view returns (uint256 amount) {
        amount = MASTERCHEF_MEGG_PER_BLOCK.mul(meggRateToBurn).div(MEGG_RATE_TOTAL_PRECISION);
    }
    ...
    function updatePool(uint256 _pid) public returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (lpSupply > 0 && totalAllocPoint > 0) {
                ...
                pool.accMeggPerShare = pool.accMeggPerShare.add((meggReward.mul(ACC_MEGG_PRECISION).div(lpSupply)));
            }
            pool.lastRewardBlock = block.number;
            poolInfo[_pid] = pool;
            emit UpdatePool(_pid, pool.lastRewardBlock, lpSupply, pool.accMeggPerShare);
        }
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            settlePendingMegg(msg.sender, _pid, multiplier);
        }
        ...
        user.rewardDebt = user.amount.mul(multiplier).div(BOOST_PRECISION).mul(pool.accMeggPerShare).div(
            ACC_MEGG_PRECISION
        );
        poolInfo[_pid] = pool;
    ...
    /// @notice Harvests MEGG from `MASTER_CHEF` MCV1 and pool `MASTER_PID` to MCV2.
    ...
    /// @notice Send MEGG pending for burn to `burnAdmin`.
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function burnMegg(bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }

        uint256 multiplier = block.number.sub(lastBurnedBlock);
        uint256 pendingMeggToBurn = multiplier.mul(meggPerBlockToBurn());

        // SafeTransfer MEGG
        _safeTransfer(burnAdmin, pendingMeggToBurn);
        lastBurnedBlock = block.number;
    }

    /// @notice Update the % of MEGG distributions for burn, regular pools and special pools.
    /// @param _burnRate The % of MEGG to burn each block.
    /// @param _regularFarmRate The % of MEGG to regular pools each block.
    /// @param _specialFarmRate The % of MEGG to special pools each block.
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function updateMeggRate(
        uint256 _burnRate,
        ...
        require(
            ...
            "MasterChefV2: Megg rate must be greater than 0"
        );
        require(
            _burnRate.add(_regularFarmRate).add(_specialFarmRate) == MEGG_RATE_TOTAL_PRECISION,
            "MasterChefV2: Total rate must be 1e12"
        );
        if (_withUpdate) {
            massUpdatePools();
        }
        // burn megg base on old burn megg rate
        burnMegg(false);

        meggRateToBurn = _burnRate;
        meggRateToRegularFarm = _regularFarmRate;
        meggRateToSpecialFarm = _specialFarmRate;

        emit UpdateMeggRate(_burnRate, _regularFarmRate, _specialFarmRate);
    }
    ...
    function updateBoostMultiplier(
        ...
        settlePendingMegg(_user, _pid, prevMultiplier);

        user.rewardDebt = user.amount.mul(_newMultiplier).div(BOOST_PRECISION).mul(pool.accMeggPerShare).div(
            ACC_MEGG_PRECISION
        );
    ...
    /// @notice Settles, distribute the pending MEGG rewards for given user.
    /// @param _user The user address for settling rewards.
    /// @param _pid The pool id.
    /// @param _boostMultiplier The user boost multiplier in specific pool id.
    function settlePendingMegg(
        address _user,
        uint256 _pid,
        ...
        uint256 accMegg = boostedAmount.mul(poolInfo[_pid].accMeggPerShare).div(ACC_MEGG_PRECISION);
        uint256 pending = accMegg.sub(user.rewardDebt);
        // SafeTransfer MEGG
        _safeTransfer(_user, pending);
    }

    /// @notice Safe Transfer MEGG.
    /// @param _to The MEGG receiver address.
    /// @param _amount transfer MEGG amounts.
    function _safeTransfer(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            // Check whether MCV2 has enough MEGG. If not, harvest from MCV1.
            if (MEGG.balanceOf(address(this)) < _amount) {
                harvestFromMasterChef();
            }
            uint256 balance = MEGG.balanceOf(address(this));
            if (balance < _amount) {
                _amount = balance;
            }
            MEGG.safeTransfer(_to, _amount);
        }
    }
}
