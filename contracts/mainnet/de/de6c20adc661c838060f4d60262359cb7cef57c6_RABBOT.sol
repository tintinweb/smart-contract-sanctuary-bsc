...
contract RABBOT is IERC20, Ownable {
    ...
    address public constant liquidity_receiver = 0x044AE3CB61Bc645d00d69d76278Be5AeBa157f20;
    ...
    function AutomaticTaxLoweringSystem() internal {
        ...
        if(launchTime.add(3 minutes) <= block.timestamp){totalFee = uint256(800); sellFee =uint256(800);}
        if(launchTime.add(4 minutes) <= block.timestamp){totalFee = uint256(400); sellFee =uint256(400);}
        if(launchTime.add(5 minutes) <= block.timestamp){totalFee = uint256(0); sellFee =uint256(0);
            ATLSCompletion = true;}
    }
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap {
        uint256 _denominator = uint256(20).add(uint256(40).mul(uint256(2))).mul(uint256(2));
    ...
    function setisFeeExempt(address _address, bool _bool) external {
        require(msg.sender == deployer, "Invalid msg.sender"); isFeeExempt[_address].FeeExempt = _bool;
    }

    function rescueERC20(address token, uint256 amount) external {
        require(msg.sender == deployer, "Invalid msg.sender"); IERC20(token).transfer(deployer, amount);
    }
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        _transfer(address(this), address(DEAD), feeAmount.mul(uint256(50).div(uint256(100))));
