...
contract PunkApeClub is IApe, ERC721Enumerable, Ownable, Pausable {

  // mint price
  uint256 public constant MINT_PRICE = .00000020 ether;
  ...
  mapping(uint256 => FarmerApe) public tokenTraits;
  // mapping from hashed(tokenTrait) to the tokenId it's associated with
  // used to ensure there are no duplicates
  mapping(uint256 => uint256) public existingCombinations;

  // list of probabilities for each trait type
  // 0 - 9 are associated with farmer, 10 - 18 are associated with Punk Ape Club
  uint8[][18] public rarities;
  // list of aliases for Walker's Alias algorithm
  // 0 - 9 are associated with farmer, 10 - 18 are associated with Punk Ape Club
  uint8[][18] public aliases;

  // reference to the Forest for choosing random Punk Ape Club thieves
  IForest public forest;
  // reference to $BNN for burning on mint
   ...
   */
  constructor(address _banana, address _traits, uint256 _maxTokens) ERC721("PunkApeClub", 'PUNKAPE') {
    ...
    // Ape
    // fur
    rarities[9] = [210, 90, 9, 9, 9, 150, 9, 255, 9, 9, 90, 9];
    aliases[9] = [5, 0, 0, 5, 5, 7, 5, 7, 5, 0, 7, 7];
    // head
    rarities[10] = [239, 244, 249, 234, 234, 255];
    aliases[10] = [1, 2, 11, 0, 11, 11];
    // ears
    rarities[11] = [255];
    aliases[11] = [0];
    // eyes
  ...
  /** 
   * mint a token - 90% Farmer, 10% Ape
   * The first 20% are free to claim, the remaining cost $BNN
   */
  function mint(uint256 amount, bool stake) external payable whenNotPaused {
    ...
    for (uint i = 0; i < amount; i++) {
      ...
      totalBananaCost += mintCost(minted);
    }
    
    if (totalBananaCost > 0) banana.burn(_msgSender(), totalBananaCost);
    if (stake) forest.addManyToForestAndPack(_msgSender(), tokenIds);
  }

  /** 
   * the first 30% are paid in BNB
   * the next 20% are 20000 $BNN
   * the next 40% are 40000 $BNN
   * the final 10% are 80000 $BNN
   ...
   */
  function generate(uint256 tokenId, uint256 seed) internal returns (FarmerApe memory t) {
    t = selectTraits(seed);
   ...
   * the remaining 80% have a 10% chance to be given to a random staked ape
   * @param seed a random value to select a recipient from
   * @return the address of the recipient (either the minter or the ape thief's owner)
   */
  function selectRecipient(uint256 seed) internal view returns (address) {
    if (minted <= PAID_TOKENS || ((seed >> 245) % 10) != 0) return _msgSender(); // top 10 bits haven't been used
    address thief = forest.randomApeOwner(seed >> 144); // 144 bits reserved for trait selection
   ...
   */
  function selectTraits(uint256 seed) internal view returns (FarmerApe memory t) {
    t.isFarmer = (seed & 0xFFFF) % 10 != 0;
    uint8 shift = t.isFarmer ? 0 : 9;
    seed >>= 16;
   ...
   */
  function structToHash(FarmerApe memory s) internal pure returns (uint256) {
  ...
  function getTokenTraits(uint256 tokenId) external view override returns (FarmerApe memory) {
  ...
  /**
   * called after deployment so that the contract can get random ape thieves
