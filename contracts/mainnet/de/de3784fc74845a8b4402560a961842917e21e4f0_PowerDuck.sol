...
contract PowerDuck is Context, IERC20, Ownable {

    using SafeMath for uint256;
    using Address for address;

    IERC20 public gegg_token;
    ...
    uint256 private _tTotal = 250000000000000000000000000000000;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Power Duck";
    string private _symbol = "DUCK";
    uint8 private _decimals = 18;

    address payable public _mwallet = 0x36E58a20C7b96B3503Dc9F2c6aba3af1f4876c83;
    address payable public _devwallet = 0x19F1AFbCc0692B0e69E792F21827705Bba293D4f;

    //buy tax
    uint256 public _maxBuyTax = 8;   
    uint256 public _currentBuyTax = 8;   
    uint256 public _totalBuyTax = 8;  
    uint256 private _previousRefBuyTax = 8;

    //sell tax
    uint256 public _maxSellTax = 8;  
    uint256 public _currentSellTax = 8;   
    uint256 public _totalSellTax = 8;   
    uint256 private _previousRefSellTax = 8;

    uint256 public _refPer = 0; 

    uint256 public autoLiqPer = 2500;  
    uint256 public burnPer = 0; 
    uint256 public devPer = 3750;  
    uint256 public marketingPer = 3750;  
    uint256 public backPer = 0;  

    uint256 public _maxHoldAmount = 6250000000000000000000000000000; 
    uint256 public _maxTransAmount = 6250000000000000000000000000000; 
    uint256 public _minTokensForLiquidity = 250000000000000000000000000000; 
    ...
    constructor () public {
        ...
        if(_mwallet == _devwallet) {
            excludeAccount(_mwallet);
        } else {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(!_tradingOpen && !_isExcluded[sender] && !_isExcluded[recipient]) {
    ...
    function _setGeggToken(address new_contract) public onlyOwner() {

        gegg_token = IERC20(new_contract);
    }
    ...
    function _setTokenomics(uint256 _autoLiqPer, uint256 _burnPer, uint256 _devPer, uint256 _marketingPer, uint256 _backPer) external onlyOwner() {

        uint256 total = _autoLiqPer.add(_burnPer).add(_devPer).add(_marketingPer).add(_backPer);

        if(total != 100) {
            revert("Tokenomics must equal 100%");
        }

        autoLiqPer = _autoLiqPer.mul(100);
        burnPer = _burnPer.mul(100);
        devPer = _devPer.mul(100);
        marketingPer = _marketingPer.mul(100);
        backPer = _backPer.mul(100);
    }
    ...
    function _doTokenomics() public lockLiquidities {
        ...
        if(amount >= _minTokensForLiquidity && _lockLiquiditiesEnabled == true) {

            uint256 liqAmount = 0;
            uint256 burnAmount = 0;
            uint256 backAmount = 0;
            
            liqAmount = _findPercent(amount, autoLiqPer);
            burnAmount = _findPercent(amount, burnPer);
            backAmount = _findPercent(amount, backPer);

            uint256 remain_balance = amount.sub(burnAmount).sub(backAmount);
    ...
    function _doLiquidity(uint256 liqTokensAmount, uint256 fullBalance, uint256 burnAmount, uint256 backAmount) private {
        ...
        if(bnbNewBalance > 0) {
            ...
            devBnbAmount = _findPercent(bnbNewBalance, devPer);
            marketingBnbAmount = _findPercent(bnbNewBalance, marketingPer);
    ...
    function _doMarketing(uint256 amount) private {
        ...
        Address.sendValue(_mwallet, amount);
    }
    ...
    function manualWithdrawTokens() public onlyOwner() {
         ...
         if(balanceOf(address(this)) > 0) {
            this.transfer(_mwallet, balanceOf(address(this)));
         }
    }
