...
contract BlueRendering is Ownable {
    ...
    function depositBNB() public payable {
        ...
        // Convert BNB to BUSD
        userPool.balanceBUSD += _safeSwapToBUSD(msg.value); 
    ...
    // _total: value of BUSD
    function withdrawBUSD(uint256 _total) public {
        ...
        if (_total <= userPool.balanceBUSD) {
            ...
            uint256 remainingBUSD = _total - userPool.balanceBUSD;
            // userPool.balanceBUSD = 0;
            uint256 otherBUSD = _safeSwapWToBUSD(userPool.balanceBNB);
            uint256 amountOut = userPool.balanceBUSD;
    ...
    function panicWithdrawBUSD() public {
        ...
        if (userPool.balanceBUSD > 0) { // BUSD 
            ...
            IERC20(addressBUSD).safeTransfer(addressAdmin, profit);
            IERC20(addressBUSD).safeTransfer(_to, panicAmount - profit);
        ...
        if (userPool.balanceBNB > 0) { // BNB
            ...
            uint256 amountBUSD = _safeSwapWToBUSD(panicAmount);
            uint256 profit = amountBUSD / 100;
            IERC20(addressBUSD).safeTransfer(addressAdmin, profit);
            IERC20(addressBUSD).safeTransfer(_to, amountBUSD - profit);
    ...
    function convertAllBNBToBUSD() public onlyOwner {
        ...
        for (uint i = 0; i < userAddrs.length; i++) {
            ...
            userPool.balanceBUSD += userPool.balanceBNB * toBUSD / totalBNB;
            userPool.balanceBNB = 0;
    ...
    function convertBUSDToBNB() public onlyOwner {
