...
contract GEISHA is Context, IERC20, Ownable {
    ...
    address private Geisha_Marketing_Wallet = 0x49EDAFFa3aa077bc1db9163C1c2C6C3583d3cD33;
    address private Geisha_Team_Wallet = 0x49EDAFFa3aa077bc1db9163C1c2C6C3583d3cD33;

    uint256 public Geisha_max_WalletLimit = 10000000000 * 10**9; //1%
    uint256 public swapTokensAtAmount = 3000000000 * 10**9; //0.3%
    uint256 public Geisha_max_BuyLimit = 5000000000 * 10**9; // 0.5%
    uint256 public Geisha_max_SellLimit = 5000000000 * 10**9; //0.5%
    ...
    Taxes public taxes = Taxes(0, 44, 0, 1);
    Taxes public sellTaxes = Taxes(0, 44, 0, 1);
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[Geisha_Marketing_Wallet] = true;
        _isExcludedFromFee[Geisha_Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[Geisha_Marketing_Wallet] = true;
        allowedTransfer[Geisha_Team_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= Geisha_max_BuyLimit, "You are exceeding Geisha_max_BuyLimit");
            require(balanceOf(to) + amount <= Geisha_max_WalletLimit, "You are exceeding Geisha_max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= Geisha_max_SellLimit, "You are exceeding Geisha_max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= Geisha_max_WalletLimit, "You are exceeding Geisha_max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(Geisha_Marketing_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(Geisha_Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function Geisha_Update_MarketingWallet(address newWallet) external onlyOwner{
        Geisha_Marketing_Wallet = newWallet;
    }
    
    function Geisha_Update_TeamWallet(address newWallet) external onlyOwner{
        Geisha_Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        Geisha_max_BuyLimit = maxBuy * 10**decimals();
        Geisha_max_SellLimit = maxSell * 10**decimals();
    }
    
    function Geisha_Max_Walletlimit(uint256 amount) external onlyOwner{
        Geisha_max_WalletLimit = amount * 10**decimals();
    }
