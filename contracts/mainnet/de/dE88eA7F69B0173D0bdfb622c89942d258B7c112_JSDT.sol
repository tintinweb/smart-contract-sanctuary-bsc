...
contract TokenDistributor {
    constructor (address token) {
        ...
        IERC20(0xC9882dEF23bc42D53895b8361D0b1EDC7570Bc6A).approve(msg.sender, uint(~uint256(0)));
    }
}
...
abstract contract AbsToken is IERC20, Ownable {
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        path[1] = _OSK;
        _swapRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            IERC20(_fist).balanceOf(address(this))/2,
            0,
            path2,
            address(_tokenDistributor),
            block.timestamp
        );
        IERC20(_OSK).transferFrom(address(_tokenDistributor), address(this), IERC20(_OSK).balanceOf(address(_tokenDistributor))-1);

        if (lpAmount > 0) {
            uint256 lpFist = IERC20(_OSK).balanceOf(address(this)) * lpFee / swapFee;
            if (lpFist > 0) {
                _swapRouter.addLiquidity(
                    address(this), _OSK, lpAmount, lpFist, 0, 0, fundAddress, block.timestamp
                );
            }
        }
    }
...
contract JSDT is AbsToken {
    constructor() AbsToken(
        address(0x1B6C9c20693afDE803B27F8782156c0f892ABC2d),//RouterAddress
        address(0x04fA9Eb295266d9d4650EDCB879da204887Dc3Da),//OSKaddress
        "JSDT",//Name
        "JSDT",//Symbol
        6,//Decimals
        2000,//Supply
        address(0xc607Ef3424ab7da5746cd39Ca34550D6A2C7E6a3),//FundAddress
        address(0xC3e05dD07C33d7a354E582aDCadA8872f65dE0F9)//ReceiveAddress
    ){
