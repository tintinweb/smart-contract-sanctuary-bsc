...
contract BellasNFTs is ERC20, Ownable {
    ...
    uint256 private totalFees = liquidityFee.add(advertisementFee).add(cexFee).add(devFee);
    ...
    constructor() ERC20("BellasNFTs", "BEL") {
    	IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);//0x10ED43C718714eb63d5aA57B78B54704E256024E); // pancakeswap v2 router address
    ...
    function _transfer(
     ...
     function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        advWallet.transfer(address(this).balance.mul(advertisementFee).div(totalFees.sub(liquidityFee)));
        cexWallet.transfer(address(this).balance.mul(cexFee).div(totalFees.sub(liquidityFee)));
    ...
    function decreaseTax(uint256 _liquidityFee, uint256 _advFee, uint256 _cexFee, uint256 _devFee) public onlyOwner {
        require(_liquidityFee.add(_advFee).add(_cexFee).add(_devFee) <= 10, "tax too high");
