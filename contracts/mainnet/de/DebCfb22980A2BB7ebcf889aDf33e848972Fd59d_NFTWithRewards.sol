...
contract NFTWithRewards is Context, ERC165, IERC721, IERC721Metadata, ReentrancyGuard, Ownable {
    using Address for address;
    using SafeMath for uint256;

    // Token name
    string private constant _name = "Enter Name Here";

    // Token symbol
    string private constant _symbol = "Enter Name Here";
    ...
    uint256 public cost = 25_000 * 10**18;
    ...
    constructor(address token_, address rewardToken_, address rewardTokenSwapper_) {
        token = IERC20(token_);
        rewardToken = IERC20(rewardToken_);
        rewardTokenSwapper = rewardTokenSwapper_;
    }
    ...
    function setRewardTokenSwapper(address rewardTokenSwapper_) external onlyOwner {
        rewardTokenSwapper = rewardTokenSwapper_;
    }
    ...
    function mint(uint256 numberOfMints) external nonReentrant {
        ...
        uint reqCost = cost * numberOfMints;
        uint received = _transferIn(token, reqCost);
        require(
            received >= reqCost,
            'Invalid Received'
        );
    ...
    receive() external payable {
        uint before = rewardToken.balanceOf(address(this));
        (bool s,) = payable(rewardTokenSwapper).call{value: address(this).balance}("");
        require(s);
        uint After = rewardToken.balanceOf(address(this));
        require(After > before, 'Zero Received');
        _register(After - before);
    }
    ...
    function _transferIn(IERC20 token_, uint256 amount) internal returns (uint256) {
        uint before = token_.balanceOf(address(this));
        require(
            token_.transferFrom(
                msg.sender,
                address(this),
        ...
        uint After = token_.balanceOf(address(this));
        require(
            After > before,
            'Zero Received'
        );
    ...
    function _register(uint256 amount) internal {
        ...
        totalRewards += amount;
        if (_totalSupply > 0) {
            dividendsPerNFT += ( amount * PRECISION ) / _totalSupply;
        }
    ...
    */
    ...
    function getCumulativeDividends(uint256 share) internal view returns (uint256) {
        return share.mul(dividendsPerNFT).div(PRECISION);
    }
