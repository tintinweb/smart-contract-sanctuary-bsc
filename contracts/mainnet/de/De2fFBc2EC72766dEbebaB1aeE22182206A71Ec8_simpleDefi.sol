...
contract simpleDefi is Storage, Ownable, AccessControl {
    ...
    function performLiquidation(address _user, bool _sendfunds) internal lockFunction returns (uint) {    
        ...
        if (_units > 0) {
            ...
            (uint amount0, uint amount1) = poolUtil.removeLiquidity(iData, exchangeInfo, _units);
    ...
    function do_harvest(uint revert_trans) private returns (uint) {    
        ...
        uint reward = swap(pendingCake,exchangeInfo.rewardToken, WBNB_ADDR);
        uint gasRecovery = (lastGas * tx.gasprice);
        ...
        uint finalReward = reward - gasRecovery;
        emit sdFeeSent(address(0),"GASRECOVERY", gasRecovery,reward);

        uint feeAmount = poolUtil.distContrib(iData, mHolders, [finalReward, pendingCake], beaconContract) + gasRecovery;

        if(feeAmount > 0 && _bal > 0) {
            payable(address(feeCollector)).transfer(feeAmount);
            emit sdFeeSent(address(0), "HARVESTFEE", feeAmount - gasRecovery,pendingCake);
        }
    ...
    function userInfo(address _user) external view returns (uint lastDeposit,uint units, uint amount,uint _pendingReward, uint _accumulatedRewards) {   
        (amount,lastDeposit,units, _accumulatedRewards) = poolUtil.getUserInfo(iData, mHolders,exchangeInfo.chefContract,_user);
