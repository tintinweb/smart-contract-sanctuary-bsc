...
contract ElonsBadDay is IBEP20, Auth {
    ...
    uint256 public _maxTxAmount = _totalSupply * 6 / 100; // 6.0%
    uint256 public _maxWallet =_totalSupply * 3 / 100; // 3.0%
    ...
    uint256 totalFee1 = 700;

    uint256 reflection1Fee = 100;
    uint256 reflection2Fee = 100;
    uint256 reflection3Fee = 100;
    uint256 totalFee2 = 300;

    uint256 feeDenominator = 10000;
    uint256 totalFee = totalFee1.add(totalFee2);
    ...
    address payable[] public _wallets = [                                                // wallet(s) to receive liquidity fee split
        payable(0x0651AeB95c05EAc05f906a09D784902E1F8f76Bd),                             // Team Member Wallet 1!
        payable(0x819296deA78175e7eb3f44B4864e9365c8e102ac),                             // Team Member Wallet 2!
        payable(0xf214c42B776acC5253f8d774f293dce68B772f15),                             // Team Member Wallet 3!
        payable(0xa5771be34353feA5a20a6e58d0365b844315b4e7),                             // Team Member Wallet 4!
        payable(0x148F7552cf0c943d726e46eE4f95A317d2e67a68),                             // Marketing Wallet!
        payable(0xc097cf7440de0bc62C9C6E887c9E14B9f3B2906d)                              // Charity Wallet!        
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBReflection1 = amountBNB.mul(reflection1Fee).div(totalBNBFee);
        uint256 amountBNBReflection2 = amountBNB.mul(reflection2Fee).div(totalBNBFee);
        uint256 amountBNBReflection3 = amountBNB.mul(reflection3Fee).div(totalBNBFee);
        uint256 amountBNBMarketing = amountBNB.mul(marketingFee).div(totalBNBFee);

        try distributor.deposit{value: amountBNBReflection}() {} catch {}
        try distributor1.deposit{value: amountBNBReflection1}() {} catch {}   
        try distributor2.deposit{value: amountBNBReflection2}() {} catch {}
        try distributor3.deposit{value: amountBNBReflection3}() {} catch {}
        if(CharityEnabled){

            uint BNBBalance = amountBNBMarketing;
    ...
    function setCharityenabled(bool _enabled) external authorized {
    ...
    function setFees1(uint256 _liquidityFee, uint256 _buybackFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        buybackFee = _buybackFee;
        reflectionFee = _reflectionFee;
        marketingFee = _marketingFee;
        totalFee1 = _liquidityFee.add(_buybackFee).add(_reflectionFee).add(_marketingFee);
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator/4);
    }
    function setFees2(uint256 _reflection1Fee, uint256 _reflection2Fee, uint256 _reflection3Fee) external authorized {
      
        reflection1Fee = _reflection1Fee;
        reflection2Fee = _reflection2Fee;
        reflection3Fee = _reflection3Fee;
        totalFee2 = _reflection1Fee.add(_reflection2Fee).add(_reflection3Fee);
        require(totalFee < feeDenominator/4);
    }
