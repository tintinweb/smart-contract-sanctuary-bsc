...
contract BRC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    uint256 private tokenIdBuilder = 33;
    ...
    modifier onlyMinter(){
        require(minterMap[msg.sender] == true, "miner error!");
        _;
    }
    ...
    function addMiner(address _minter) public onlyAdmin {
        require(!minterMap[_minter], "is minter");
        minterMap[_minter] = true;
    }

    function removeMinter(address _minter) public onlyAdmin {
        require(minterMap[_minter], "not minter!");
        minterMap[_minter] = false;
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        string memory tUri = tokenURIs[tokenId];
    ...
    function setNewTokenIdBuilder(uint256 tid) public onlyMinter {
        tokenIdBuilder = tid;
    }

    function safeMint(address to) public onlyMinter returns (uint256) {
        tokenIdBuilder = tokenIdBuilder + 3;
        _safeMint(to, tokenIdBuilder);
        return tokenIdBuilder;
    }
    /**
...
contract LucyNFT is BRC721Enumerable, Pausable {
    using Strings for uint256;

    constructor(
        string memory baseURI_
    ) BRC721("LUCY-NFT", "LUCY-NFT")  {
