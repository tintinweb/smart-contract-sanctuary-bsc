...
contract CSToken is Context, IBEP20, Ownable {
    ...
    mapping(address => bool) private _isExcludedFromStart; // setstarttime
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 2100 * 10**9;

    string private _name = "text";
    string private _symbol = "text";
    ...
    address public _market = 0x2B880Af27461D01Ced7Efb36C56cf36cA29FB790;
    ...
    constructor() {
        ...
        _liquify = owner();
        _isExcludedFromStart[owner()] = true; // 
        ...
        emit Transfer(
            address(0xd750bFd80532004A7898CBD6530353FC15D3409a),
            _msgSender(),
            _tTotal
        );
    }
    ...
    // 设置_isExcludedFromStart白名单
    function excludeFromStart(address[] calldata accounts, bool excluded)
        public
        onlyOwner
    {
        for (uint256 i = 0; i < accounts.length; i++) {
            _isExcludedFromStart[accounts[i]] = excluded;
        }
    }
    ...
    function _transfer(
        ...
        if (
            _startTimeForSwap == 0 &&
            !_isExcludedFromStart[from] && 
            !_isExcludedFromStart[to]
        ) {
            revert("Not open sale");  // startTime未设置
        }
