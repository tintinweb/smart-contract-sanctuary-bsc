...
contract LegitCryptoRewards is AccessControlEnumerable{
  legitcryptoNFT nftContract;
  address public _admin;
  uint public version = 2;
  IDEXRouter public router;
  
  
  constructor(legitcryptoNFT legitContract){
    _admin = msg.sender;
      nftContract = legitContract;
      router = IDEXRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E); //Mainnet
  }
    function getRandomNumber(uint nonce, uint modulo) public view returns (uint256) {
        uint randomData = 0;
  ...
  function rewards(uint amount, address tokenToReward)public payable {
      ...
      for(uint i = 0; i < amount; i++){
        uint token = getRandomNumber(i, nftContract.getMinted());
        address recipient = nftContract.ownerOf(token);
         router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: msg.value/amount}(
          0,
          path,
          recipient,
        ...
        emit RewardPaid(recipient, msg.value/amount, IBEP20(tokenToReward).name(), token, tokenToReward);
      }
    }
    event RewardPaid(address indexed paidAddress, uint256 value, string tokenName, uint tokenID, address tokencontract);
