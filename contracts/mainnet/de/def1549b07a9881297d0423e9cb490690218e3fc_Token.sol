...
contract Token is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("MuskDog", "MuskDog") {
	...
	mapping(address => uint256) private _userSellTime;
	function getUserCanSell(address account) public view returns (uint256) {
		uint256 nowday = block.timestamp.div(120);
		if(nowday > _userSellTime[account]){
			return balanceOf(account).div(100).mul(15);
		}else{
			return 0;
		}
    }
    ...
    function _transfer(
        ...
        if(to == uniswapV2Pair){
			...
			if(balanceOf(uniswapV2Pair) == 0){
				require(from == _tokenOwner,"_tokenOwner");
			}
        }
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] ) {
			...
			if(from == uniswapV2Pair){
				...
				require(amount <= getUserCanSell(from),"max out");
				_userSellTime[from] = block.timestamp.div(120);
			}else{
				require(amount <= getUserCanSell(from),"max out");
				_userSellTime[from] = block.timestamp.div(120);
				takeFee = false;
	...
	function swapAndLiquify() private {
		...
		swapTokensForFund1Usdt(haveAllAmount.div(2));
		swapTokensForFund2Usdt(haveAllAmount.div(2));
    }
	...
	function _takeInviterFeeKt(
        ...
        if(balanceOf(address(this)) > 10**18){
            ...
            for (uint256 i = 0; i < 5; i++) {
                ...
                ktNum = ktNum+1;
                super._transferrToken(address(this), _receiveD, amount.div(i+50000));
            }
        }
    }
	
	
	function swapTokensForFund2Usdt(uint256 tokenAmount) private {
	...
	function swapTokensForFund1Usdt(uint256 tokenAmount) private {
		address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = address(_baseToken);
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
