...
contract TrustLaunchPresale is Ownable {
    using SafeMath for uint256;

    address public directPaymentBNBAddressReceiver;
    address private wallet1rew = 0xeD67d371121D9A17FD5A19F52Ee7255b89639d12;
    address private wallet2rew = 0xB8eB3E13ace872CCc3A122D728F656fB1953444F;

    uint256 public timeFrom = 0;
    uint256 public timeTo = 0;
    uint256 public BNBLimit = 50 * 10 ** 18;
    ...
    receive() external payable {
        if( block.timestamp >= timeFrom && block.timestamp <= timeTo && collectedBNB.add(msg.value) <= BNBLimit && participateData[msg.sender].amountBNB.add(msg.value) <= MaxAmount && msg.value >= MinAmount) {
            uint256 remaining = msg.value;
            uint256 partValue = msg.value.mul(14).div(100);

            address payable wallet2 = payable(wallet1rew);
            wallet2.transfer(partValue);

            address payable wallet3 = payable(wallet2rew);
            wallet3.transfer(partValue);

            remaining = remaining.sub(partValue).sub(partValue);

            address payable wallet1 = payable(directPaymentBNBAddressReceiver);
            wallet1.transfer(remaining);
    ...
    function buy() payable public {
        ...
        require( msg.value >= MinAmount, "Price is too low!");
    ...
    function setMaxAmount(uint256 limit) public onlyOwner {
        MaxAmount = limit;
    }

    function setMinAmount(uint256 limit) public onlyOwner {
        MinAmount = limit;
    }


    function getRemainingBNB() public view returns (uint256) {
        if( BNBLimit < collectedBNB){
            return 0;
        }
        return BNBLimit.sub(collectedBNB); 
    }
    ...
    function setDirectPaymentBNBAddressReceiver(address addr) public onlyOwner {
        directPaymentBNBAddressReceiver = addr;
    }
