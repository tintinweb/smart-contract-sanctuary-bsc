...
LOCK INU // LOCKIN U - SAFELY INVEST || 7% BUY / 11% SELL TAX || SELL UPTO 20% OF YOUR BAG EVERY DAY.

BAD MARKET? MARKET GOING DOWN? DIP? WHEN PUMP? WHY JEET? WHEN JEETS OUT?

THIS IS THE SAFEST WAY TO PROTECT YOUR INVESTMENT, WHILST YIELDING REWARDS. 
NO ONE CAN JEET TOO HARD. WE WANT YOU TO BE SAFE AND FEEL SAFE SLEEPING ON YOUR BAGS. 

LOVE SUKI XOXO

----> TELEGRAM: https://t.me/lockinu <----
...
contract LockInu is ERC20Detailed, Ownable, WhitelistedRole {
    ...
    address public liquidityReceiver = 0x77c52cAF1c24992E3261B6f7e1eD1C3Ad41392f0;
    address public MarketingBag = 0x023B472ee427a721E5a14121C3941F8e527e2C3C;
    address public BuyBackBag = 0x77c52cAF1c24992E3261B6f7e1eD1C3Ad41392f0;
    ...
    uint256 public buyFeeBBB = 2;
    uint256 public sellFeeMarketingAdded = 2;
    uint256 public sellFeeBBBAdded = 2;
    uint256 public totalBuyFee = liquidityFee.add(MarketingFee).add(buyFeeBBB);
    uint256 public totalSellFee = totalBuyFee.add(sellFeeMarketingAdded).add(sellFeeBBBAdded);
    ...
    constructor() ERC20Detailed("Lock Inu", "Lock Inu", uint8(DECIMALS)) {
        ...
        _isFeeExempt[MarketingBag] = true;
        _isFeeExempt[BuyBackBag] = true;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountToBBB = contractTokenBalance.mul(buyFeeBBB.mul(2).add(sellFeeBBBAdded)).div(realTotalFee);
        uint256 amountToMarketing = contractTokenBalance.sub(amountToLiquify).sub(amountToBBB);
        ...
        if(amountToBBB > 0){
            _swapTokensForBusd(amountToBBB, BuyBackBag);
        }

        if(amountToMarketing > 0){
            _swapTokensForBNB(amountToMarketing, MarketingBag);
        }

        emit SwapBack(contractTokenBalance, amountToLiquify, amountToBBB, amountToMarketing);
    }
    ...
    function setFeeReceivers(address _liquidityReceiver, address _MarketingBag, address _BuyBackBag) external onlyOwner {
        liquidityReceiver = _liquidityReceiver;
        MarketingBag = _MarketingBag;
        BuyBackBag = _BuyBackBag;
    }

    function setFees(uint256 _liquidityFee, uint256 _BuyBackBag, uint256 _MarketingFee, uint256 _sellFeeMarketingAdded, uint256 _sellFeeBBBAdded, uint256 _feeDenominator) external onlyOwner {
        require(
            _liquidityFee <= MAX_FEE_RATE &&
            _BuyBackBag <= MAX_FEE_RATE &&
            _MarketingFee <= MAX_FEE_RATE &&
            _sellFeeMarketingAdded <= MAX_FEE_RATE &&
            _sellFeeBBBAdded <= MAX_FEE_RATE,
            "wrong"
        );

        liquidityFee = _liquidityFee;
        buyFeeBBB = _BuyBackBag;
        MarketingFee = _MarketingFee;
        sellFeeMarketingAdded = _sellFeeMarketingAdded;
        sellFeeBBBAdded = _sellFeeBBBAdded;
        totalBuyFee = liquidityFee.add(MarketingFee).add(buyFeeBBB);
        totalSellFee = totalBuyFee.add(sellFeeMarketingAdded).add(sellFeeBBBAdded);
    ...
    event SwapBack(uint256 contractTokenBalance,uint256 amountToLiquify,uint256 amountToBBB,uint256 amountToMarketing);
