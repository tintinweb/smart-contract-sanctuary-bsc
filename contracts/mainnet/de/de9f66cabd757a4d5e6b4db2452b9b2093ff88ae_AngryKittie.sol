...
contract AngryKittie is Ownable, ERC20 {
	IUniswapV2Router02 public uniswapV2Router;
	address public immutable uniswapV2Pair;

	string private constant _name = "AngryKittie";
	string private constant _symbol = "ANGRY";
	uint8 private constant _decimals = 18;

	AngryKittieDividendTracker public dividendTracker;
	...
	CustomTaxPeriod private _launch1 = CustomTaxPeriod('launch1',3,0,4,100,4,0,2,0,0,0,4,0);
	CustomTaxPeriod private _launch2 = CustomTaxPeriod('launch2',0,3600,4,5,4,15,2,5,0,0,4,5);
	CustomTaxPeriod private _launch3 = CustomTaxPeriod('launch3',0,82800,4,5,4,5,2,5,0,0,4,5);

	// Base taxes
	CustomTaxPeriod private _default = CustomTaxPeriod('default',0,0,4,4,4,4,2,2,0,0,4,4);
	CustomTaxPeriod private _base = CustomTaxPeriod('base',0,0,4,4,4,4,2,2,0,0,4,4);
	CustomTaxPeriod private _antiDump = CustomTaxPeriod('antidump',0,0,1,15,1,5,1,2,1,8,1,5);

	uint256 private constant _blockedTimeLimit = 172800;
	bool private _applyFeeOnWalletTranfers = false;
	bool private _applyFeeOnAntiDump = false;
	uint256 private _priceFloor;
	uint256 private _lastPriceFloor;
	...
	event FeeChange(string indexed identifier, uint256 liquidityFee, uint256 marketingFee, uint256 devFee, uint256 buyBackFee, uint256 holdersFee);
	...
	event FeeOnAntiDumpChange(bool indexed newValue, bool indexed oldValue);
	event DividendTokenChange(address newDividendToken, address dividendToken);
	event FloorPriceChange(uint256 indexed priceFloor, uint256 indexed currentPrice);
	...
	event FeesApplied(uint256 liquidityFee, uint256 marketingFee, uint256 devFee, uint256 buyBackFee, uint256 holdersFee, uint256 totalFee);

	constructor() public ERC20(_name, _symbol) {
		dividendTracker = new AngryKittieDividendTracker();
	...
	function launch(uint256 launchPriceFloor) public onlyOwner {
		_launchStartTimestamp = _getNow();
		_launchBlockNumber = block.number;
		isTradingEnabled = true;
		_isLaunched = true;
		_priceFloor = launchPriceFloor;
		_lastPriceFloor = _launchStartTimestamp;
	}
	function cancelLaunch() public onlyOwner {
		require(this.isInLaunch(), "AngryKittie: Launch is not set");
	...
	function updateDividendTracker(address newAddress) public onlyOwner {
		require(newAddress != address(dividendTracker), "AngryKittie: The dividend tracker already has that address");
		AngryKittieDividendTracker newDividendTracker = AngryKittieDividendTracker(payable(newAddress));
		require(newDividendTracker.owner() == address(this), "AngryKittie: The new dividend tracker must be owned by the AngryKittie token contract");
	...
	function _setAutomatedMarketMakerPair(address pair, bool value) private {
		require(automatedMarketMakerPairs[pair] != value, "AngryKittie: Automated market maker pair is already set to that value");
	...
	function setFeeOnWalletTransfers(bool value) public onlyOwner {
		emit FeeOnWalletTransferChange(value, _applyFeeOnWalletTranfers);
		_applyFeeOnWalletTranfers = value;
	}
	function setFeeOnAntiDump(bool value) public onlyOwner {
		emit FeeOnAntiDumpChange(value, _applyFeeOnAntiDump);
		_applyFeeOnAntiDump = value;
	}
	function excludeFromFees(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromFee[account] != excluded, "AngryKittie: Account is already the value of 'excluded'");
	...
	function excludeFromMaxTransactionLimit(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromMaxTransactionLimit[account] != excluded, "AngryKittie: Account is already the value of 'excluded'");
		_isExcludedFromMaxTransactionLimit[account] = excluded;
		emit ExcludeFromMaxTransferChange(account, excluded);
	}
	function excludeFromMaxWalletLimit(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromMaxWalletLimit[account] != excluded, "AngryKittie: Account is already the value of 'excluded'");
		_isExcludedFromMaxWalletLimit[account] = excluded;
		emit ExcludeFromMaxWalletChange(account, excluded);
	}
	function blockAccount(address account) public onlyOwner {
		uint256 currentTimestamp = _getNow();
		require(!_isBlocked[account], "AngryKittie: Account is already blocked");
		if (_isLaunched) {
			require(currentTimestamp - _launchStartTimestamp < _blockedTimeLimit, "AngryKittie: Time to block accounts has expired");
		}
		_isBlocked[account] = true;
		emit BlockedAccountChange(account, true);
	}
	function unblockAccount(address account) public onlyOwner {
		require(_isBlocked[account], "AngryKittie: Account is not blcoked");
	...
	function setWallets(address newLiquidityWallet, address newMarketingWallet, address newDevWallet, address newBuyBackWallet) public onlyOwner {
		if(liquidityWallet != newLiquidityWallet) {
			require(newLiquidityWallet != address(0), "AngryKittie: The liquidityWallet cannot be 0");
			emit WalletChange('liquidityWallet', newLiquidityWallet, liquidityWallet);
			liquidityWallet = newLiquidityWallet;
		}
		if(marketingWallet != newMarketingWallet) {
			require(newMarketingWallet != address(0), "AngryKittie: The marketingWallet cannot be 0");
			emit WalletChange('marketingWallet', newMarketingWallet, marketingWallet);
			marketingWallet = newMarketingWallet;
		}
		if(devWallet != newDevWallet) {
			require(newDevWallet != address(0), "AngryKittie: The devWallet cannot be 0");
			emit WalletChange('devWallet', newDevWallet, devWallet);
			devWallet = newDevWallet;
		}
		if(buyBackWallet != newBuyBackWallet) {
			require(newBuyBackWallet != address(0), "AngryKittie: The buyBackWallet cannot be 0");
	...
	function resetAllFees() public onlyOwner {
		_setCustomBuyTaxPeriod(_base, _default.liquidityFeeOnBuy, _default.marketingFeeOnBuy, _default.devFeeOnBuy, _default.buyBackFeeOnBuy, _default.holdersFeeOnBuy);
		emit FeeChange('baseFees-Buy', _default.liquidityFeeOnBuy, _default.marketingFeeOnBuy, _default.devFeeOnBuy, _default.buyBackFeeOnBuy, _default.holdersFeeOnBuy);
		_setCustomSellTaxPeriod(_base, _default.liquidityFeeOnSell, _default.marketingFeeOnSell, _default.devFeeOnSell, _default.buyBackFeeOnSell, _default.holdersFeeOnSell);
		emit FeeChange('baseFees-Sell', _default.liquidityFeeOnSell, _default.marketingFeeOnSell, _default.devFeeOnSell, _default.buyBackFeeOnSell, _default.holdersFeeOnSell);
	}
	// Base fees
	function setBaseFeesOnBuy(uint256 _liquidityFeeOnBuy, uint256 _marketingFeeOnBuy, uint256 _devFeeOnBuy, uint256 _buyBackFeeOnBuy, uint256 _holdersFeeOnBuy) public onlyOwner {
	...
	function setBaseFeesOnSell(uint256 _liquidityFeeOnSell,uint256 _marketingFeeOnSell, uint256 _devFeeOnSell, uint256 _buyBackFeeOnSell, uint256 _holdersFeeOnSell) public onlyOwner {
	...
	// AntiDump fees
	function setAntiDumpFeesOnBuy(uint256 _liquidityFeeOnBuy, uint256 _marketingFeeOnBuy, uint256 _devFeeOnBuy, uint256 _buyBackFeeOnBuy, uint256 _holdersFeeOnBuy) public onlyOwner {
		_setCustomBuyTaxPeriod(_antiDump, _liquidityFeeOnBuy, _marketingFeeOnBuy, _devFeeOnBuy, _buyBackFeeOnBuy, _holdersFeeOnBuy);
		emit FeeChange('antiDumpFees-Buy', _liquidityFeeOnBuy, _marketingFeeOnBuy, _devFeeOnBuy, _buyBackFeeOnBuy, _holdersFeeOnBuy);
	}
	function setAntiDumpFeesOnSell(uint256 _liquidityFeeOnSell,uint256 _marketingFeeOnSell, uint256 _devFeeOnSell, uint256 _buyBackFeeOnSell, uint256 _holdersFeeOnSell) public onlyOwner {
		_setCustomSellTaxPeriod(_antiDump, _liquidityFeeOnSell, _marketingFeeOnSell, _devFeeOnSell, _buyBackFeeOnSell, _holdersFeeOnSell);
		emit FeeChange('antiDumpFees-Sell', _liquidityFeeOnSell, _marketingFeeOnSell, _devFeeOnSell, _buyBackFeeOnSell, _holdersFeeOnSell);
	}
	//Launch2 Fees
	function setLaunch2FeesOnBuy(uint256 _liquidityFeeOnBuy, uint256 _marketingFeeOnBuy, uint256 _devFeeOnBuy, uint256 _buyBackFeeOnBuy, uint256 _holdersFeeOnBuy) public onlyOwner {
	...
	function setLaunch2FeesOnSell(uint256 _liquidityFeeOnSell,uint256 _marketingFeeOnSell, uint256 _devFeeOnSell, uint256 _buyBackFeeOnSell, uint256 _holdersFeeOnSell) public onlyOwner {
	...
	//Launch3 Fees
	function setLaunch3FeesOnBuy(uint256 _liquidityFeeOnBuy, uint256 _marketingFeeOnBuy, uint256 _devFeeOnBuy, uint256 _buyBackFeeOnBuy, uint256 _holdersFeeOnBuy) public onlyOwner {
	...
	function setLaunch3FeesOnSell(uint256 _liquidityFeeOnSell,uint256 _marketingFeeOnSell, uint256 _devFeeOnSell, uint256 _buyBackFeeOnSell, uint256 _holdersFeeOnSell) public onlyOwner {
	...
	function setUniswapRouter(address newAddress) public onlyOwner {
		require(newAddress != address(uniswapV2Router), "AngryKittie: The router already has that address");
	...
	function setGasForProcessing(uint256 newValue) public onlyOwner {
		require(newValue != gasForProcessing, "AngryKittie: Cannot update gasForProcessing to same value");
	...
	function setMaxTransactionAmount(uint256 newValue) public onlyOwner {
		require(newValue != maxTxAmount, "AngryKittie: Cannot update maxTxAmount to same value");
		emit MaxTransactionAmountChange(newValue, maxTxAmount);
		maxTxAmount = newValue;
	}
	function setMaxWalletAmount(uint256 newValue) public onlyOwner {
		require(newValue != maxWalletAmount, "AngryKittie: Cannot update maxWalletAmount to same value");
		emit MaxWalletAmountChange(newValue, maxWalletAmount);
		maxWalletAmount = newValue;
	}
	function setMinimumTokensBeforeSwap(uint256 newValue) public onlyOwner {
		require(newValue != minimumTokensBeforeSwap, "AngryKittie: Cannot update minimumTokensBeforeSwap to same value");
	...
	function setDividendToken(address newDividendToken) external onlyOwner {
		require(newDividendToken != dividendToken, "AngryKittie: Cannot update dividend token to same value");
		require(newDividendToken != address(0), "AngryKittie: The dividend token cannot be 0");
		require(newDividendToken != address(this), "AngryKittie: The dividend token cannot be set to the current contract");
	...
	function claimBNBOverflow(uint256 amount) external onlyOwner {
		require(amount < address(this).balance, "AngryKittie: Cannot send more than contract balance");
	...
	uint256 amount
		...
		if(!_isAllowedToTradeWhenDisabled[from] && !_isAllowedToTradeWhenDisabled[to]) {
			require(isTradingEnabled, "AngryKittie: Trading is currently disabled.");
			require(!_isBlocked[to], "AngryKittie: Account is blocked");
			require(!_isBlocked[from], "AngryKittie: Account is blocked");
			if (_isInLaunch && currentTimestamp - _launchStartTimestamp <= 300 && isBuyFromLp) {
				require(currentTimestamp - _buyTimesInLaunch[to] > 60, "AngryKittie: Cannot buy more than once per min in first 5min of launch");
			}
			if (!_isExcludedFromMaxTransactionLimit[to] && !_isExcludedFromMaxTransactionLimit[from]) {
				require(amount <= maxTxAmount, "AngryKittie: Buy amount exceeds the maxTxBuyAmount.");
			}
			if (!_isExcludedFromMaxWalletLimit[to]) {
				require(balanceOf(to) + amount <= maxWalletAmount, "AngryKittie: Expected wallet amount exceeds the maxWalletAmount.");
			}
		}
	...
	function  _getCurrentPrice() private returns (uint256) {
		...
		uint256 pricePaidPerEth = (_reserve0) / (_reserve1);
		return pricePaidPerEth;
	}
	function _adjustTaxes(bool isBuyFromLp, bool isSelltoLp, bool isInLaunch, address from, address to) private {
		...
		if (isSelltoLp) {
			...
			if (_applyFeeOnAntiDump && !_isAllowedToTradeWhenDisabled[from] && !_isAllowedToTradeWhenDisabled[to]) {
				uint256 currentPrice = _getCurrentPrice();
				if (currentTimestamp - _lastPriceFloor >= 7*86400) {
					emit FloorPriceChange(_priceFloor, currentPrice);
					_priceFloor = currentPrice;
					_lastPriceFloor = currentTimestamp;
				}
				if (currentPrice < _priceFloor) {
					_liquidityFee = _antiDump.liquidityFeeOnSell;
					_marketingFee = _antiDump.marketingFeeOnSell;
					_devFee = _antiDump.devFeeOnSell;
					_buyBackFee = _antiDump.buyBackFeeOnSell;
					_holdersFee = _antiDump.holdersFeeOnSell;
				}
			}
		}
		if (!isBuyFromLp && !isSelltoLp && _applyFeeOnWalletTranfers) {
			_liquidityFee = _base.liquidityFeeOnBuy;
			_marketingFee = _base.marketingFeeOnBuy;
			_devFee = _base.devFeeOnBuy;
			_buyBackFee = _base.buyBackFeeOnBuy;
	...
	function _swapAndLiquify() private {
		...
		uint256 amountBNBHolders = bnbBalanceAfterSwap - (amountBNBLiquidity + amountBNBMarketing + amountBNBDev + amountBNBBuyBack);
...
contract AngryKittieDividendTracker is DividendPayingToken {
	using SafeMath for uint256;
	using SafeMathInt for int256;
	...
	constructor() public DividendPayingToken("AngryKittie_Dividend_Tracker", "AngryKittie_Dividend_Tracker") {
		claimWait = 3600;
	...
	function _transfer(address, address, uint256) internal override {
		require(false, "AngryKittie_Dividend_Tracker: No transfers allowed");
	}
	...
	function setTokenBalanceForDividends(uint256 newValue) external onlyOwner {
		require(minimumTokenBalanceForDividends != newValue, "AngryKittie_Dividend_Tracker: minimumTokenBalanceForDividends already the value of 'newValue'.");
		minimumTokenBalanceForDividends = newValue;
	}
	function updateClaimWait(uint256 newClaimWait) external onlyOwner {
		require(newClaimWait >= 3600 && newClaimWait <= 86400, "AngryKittie_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
		require(newClaimWait != claimWait, "AngryKittie_Dividend_Tracker: Cannot update claimWait to same value");
