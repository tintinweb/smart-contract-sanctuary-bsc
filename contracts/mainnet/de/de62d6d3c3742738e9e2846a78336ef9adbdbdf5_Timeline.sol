...
contract Timeline is IERC20, Context, Ownable {
    ...
    string private constant _symbol = "TL";
    string private constant _name = "Timeline";

    uint256 public Timeline_max_BuyLimit = 10000000 * 10**9;
    uint256 public Timeline_max_SellLimit = 10000000 * 10**9;

    address public Timeline_Wallet = 0xa1a3f5D5fd312fbcFb6f2A81f24cf95e009b73b5;
    address public Timeline_Team_Wallet = 0xE4736bb8B8ae2dd38001Cdea7cf468E99DAB5814;

    uint256 public Timeline_max_WalletLimit = 30000000 * 10**9;
    ...
    Taxes public sellTaxes = Taxes(0, 7, 1, 1);
    Taxes public taxes = Taxes(0, 7, 1, 1);
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[Timeline_Wallet] = true;
        _isExcludedFromFee[Timeline_Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[Timeline_Wallet] = true;
        allowedTransfer[Timeline_Team_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= Timeline_max_BuyLimit, "You are exceeding Timeline_max_BuyLimit");
            require(balanceOf(to) + amount <= Timeline_max_WalletLimit, "You are exceeding Timeline_max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= Timeline_max_SellLimit, "You are exceeding Timeline_max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= Timeline_max_WalletLimit, "You are exceeding Timeline_max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(Timeline_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(Timeline_Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function Timeline_Update_MarketingWallet(address newWallet) external onlyOwner{
        Timeline_Wallet = newWallet;
    }
    
    function Timeline_Update_TeamWallet(address newWallet) external onlyOwner{
        Timeline_Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        Timeline_max_BuyLimit = maxBuy * 10**decimals();
        Timeline_max_SellLimit = maxSell * 10**decimals();
    }
    
    function Timeline_Max_Walletlimit(uint256 amount) external onlyOwner{
        Timeline_max_WalletLimit = amount * 10**decimals();
    }
