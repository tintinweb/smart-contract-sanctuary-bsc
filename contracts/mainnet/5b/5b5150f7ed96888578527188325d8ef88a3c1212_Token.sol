...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
	...
	function distributeDividends_KICT(uint256 _amount) public{
		require(msg.sender == KICT);
		require(totalSupply() > 0);
	...
	function _withdrawDividendOfUser(address user) internal returns (uint256) {
		...
		if (_withdrawableDividend > 0) {
			...
			(bool success, bytes memory data) = KICT.call(abi.encodeWithSelector(0xa9059cbb, user, _withdrawableDividend));
...
contract KICTDividendTracker is DividendPayingToken, Ownable {
	using SafeMath for uint256;
	using SafeMathInt for int256;
	...
	constructor() DividendPayingToken("KICT_DividendTracker", "KICT_DividendTracker") {
		claimWait = 0;
	...
	function setKICT(address _KICT) external onlyOwner{
		KICT = _KICT;
	}

	function _transfer(address, address, uint256) internal pure override {
		require(false, "KICTDividendTracker: No transfers allowed");
	}

	function withdrawDividend() public pure override {
		require(false, "KICTDividendTracker: withdrawDividend disabled. Use the 'claim' function on the main KICT contract.");
	}
...
contract Token is ERC20, Ownable {
	...
	KICTDividendTracker public dividendTracker;
    uint256 public tradingStratTime;
	address public marketingAddress = 0xBE7C0F93F7a61763b3E52CB6583c0fb49151c7Be;
    address public receiveAddress = 0x41f48E2c2739c10C16CF0b1312233EB5f0e18a74;
	
	uint256 public swapTokensAtAmount = 3* 10 ** decimals();
	uint256 public dividendFee = 15;  
	...
	constructor() ERC20("KICT", "KICT") {
		dividendTracker = new KICTDividendTracker();
		...
		dividendTracker.setKICT(address(this));
		...
		_mint(receiveAddress, 30000 * (10**decimals()));
		
		
	}
	function updatePancakeRouter(address newAddress) public onlyOwner {
		require(newAddress != address(pancakeRouter), "KICT: The router already has that address");
	...
	function updateDividendTracker(address newAddress) public onlyOwner {
		require(newAddress != address(dividendTracker), "KICT: The dividend tracker already has that address");

		KICTDividendTracker newDividendTracker = KICTDividendTracker(payable(newAddress));

		require(newDividendTracker.owner() == address(this), "KICT: The new dividend tracker must be owned by the KICT token contract");
	...
	function excludeFromFees(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromFees[account] != excluded, "KICT: Account is already the value of 'excluded'");
	...
	function excludeFromPeriodLimit(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromPeriodLimit[account] != excluded, "KICT: Account is already the value of 'excluded'");
	...
	function excludeFromMaxTxLimit(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromMaxTxLimit[account] != excluded, "KICT: Account is already the value of 'excluded'");
	...
	function updateGasForProcessing(uint256 newValue) public onlyOwner {
		require(newValue >= 200000 && newValue <= 500000, "KICT: gasForProcessing must be between 200,000 and 500,000");
		require(newValue != gasForProcessing, "KICT: Cannot update gasForProcessing to same value");
	...
	function _transfer(
		...
		if(takeFee) {
			...
			if (from == pancakePair) {
			// buy
				require(balanceOf(to).add(amount) <= 100 * 10**decimals(),"over hold limit");
				uint256 forDividends = amount.mul(dividendFee).div(1000);
				super._transfer(from, address(dividendTracker), forDividends);
       			dividendTracker.distributeDividends_KICT(forDividends);
			 	emit SwapAndSendTo(forDividends, "DIVIDENDS");
                uint256 forMarketing = amount.mul(marketingFee).div(1000);
