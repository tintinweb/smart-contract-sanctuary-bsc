...
contract RichSunak is ERC20, Ownable {
    ...
    uint256 public  totalFees      = rewardFee + marketingFee + jackpotFee;

    address public marketingWallet  = 0x8956F049b6bF41A480226427A432cd189591f556;
    address public jackpotWallet    = 0x2A0456937A2168F59c2a69237161FB4A998A3aCb;
    ...
    event UpdateFees( uint256 marketingFee, uint256 rewardFee, uint256 jackpotFee);
    ...
    constructor() ERC20("Rich Sunak", "$RICHI") 
    {

        _mint(owner(), 70_000_000 * (10 ** 18));
        ...
        _isExcludedFromFees[jackpotWallet] = true;
    ...
    function updateFees(uint256 _marketingFee, uint256 _rewardFee, uint256 _jackpotFee) external onlyOwner {
        marketingFee = _marketingFee;
        rewardFee    = _rewardFee;
        jackpotFee    = _jackpotFee;

        totalFees = _rewardFee + _marketingFee + _jackpotFee;
        
        require(totalFees <= 10, "Fees cannot exceed 10%");
        emit UpdateFees(marketingFee, rewardFee, jackpotFee);
    }
    ...
    function _transfer(
        ...
        if(antibotEnabled){
            if (launchedAt == 0 && to == (uniswapV2Pair)) {
                ...
                if(from == uniswapV2Pair) {
                    ...
                    require(amount <= 70_000 * decimals(), "Max sell amount at launch is 0.1%");
        ...
        if( canSwap &&
            ...
            uint256 marketingShare = (marketingFee * newBalance) / (marketingFee + rewardFee);
        ...
        uint256 _totalFeesToContract = _totalFeesMarketing + _totalFeesReward;
        uint256 _amount = amount;

        if( _totalFeesToContract > 0) {
            uint256 fees = (_amount * _totalFeesToContract) / 100;
            amount -= fees;
        ...
        if (_jackpotFee > 0){
            uint256 _fees = (_amount * jackpotFee) / 100;
            amount -= _fees;

            super._transfer(from, jackpotWallet, _fees);
        }
