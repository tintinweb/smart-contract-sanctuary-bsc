...
contract AuctionWithBidding is Auction {
    ...
    constructor(bytes32 lotBatchRoot_, address token_) {
    ...
    function bid(
    ...
    ) external onlyWhitelisted(whitelistProof) onlyAvailableLots(lotKey, lotBatchProof) {
        ...
        if (amount != 0) token.safeTransferFrom(_msgSender(), address(this), amount);

        if (topBid.amount != 0) token.safeTransfer(topBid.bidder, topBid.amount);
...
import "./AuctionWithBidding.sol";

contract FameMMAVIPAuctionTest is AuctionWithBidding {
    constructor(bytes32 lotBatchRoot, address token) AuctionWithBidding(lotBatchRoot, token) {
        //
    }
}
