...
/*
Website: https://animalinu.co/
Telegram: https://t.me/Animalinu
Twitter : https://twitter.com/AnimalInu
*/
...
contract AnimalInu {
    ...
    string  private _name = 'Animal INU';
    ...
    uint256 public _addLiquidityAmount;
    ...
    mapping (address => bool) private _sniped;
    ...
    constructor () {
        ...
        setNoLimits(address(this));
        setNoLimits(_owner);
        setNoLimits(uniswapPair);
        setNoLimits(uniswapRouter);
        setWallets(msg.sender);
        
        setLimits(30000, 30000, 30000);
        setFees(3, 1, 2, 2);
    }
    
    function setNoLimits(address user) public virtual onlyOwner {
    ...
    function setWallets(address _devWallet) public virtual onlyOwner {
        devWallet = _devWallet;
    }
    
    function setLimits(uint256 maxWalletLimit, uint256 maxTxAmount, uint256 addLiquidityAmount) public virtual onlyOwner {
        _maxWalletLimit = maxWalletLimit * 10**_decimals;
        _maxTxAmount = maxTxAmount * 10**_decimals;
        // max transaction: %0.05.
        require(_maxTxAmount >= _totalSupply / 2000, 'max transaction!');
        _addLiquidityAmount = addLiquidityAmount * 10**_decimals;
    }
    ...
    function setFees(uint8 _devFeeBuy, uint8 _liquidityFeeBuy, uint8 _devFeeSell, uint8 _liquidityFeeSell) public virtual onlyOwner {
        // max fee: 15%.
        require(_devFeeBuy + _liquidityFeeBuy <= 15, 'max fee!');
        // max fee: 15%.
        require(_devFeeSell + _liquidityFeeSell <= 15, 'max fee!');
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        require(!_sniped[sender] && !_sniped[recipient], 'sniper detected!');
        ...
        if(!_excludedMaxWallet[recipient]) {
            require(balanceOf(recipient) + amount <= _maxWalletLimit, 'max wallet!');
        }
        
        uint256 senderBalance = balanceOf(sender);
        require(senderBalance >= amount, 'balance not enough!');
    ...
    function swapAddLiquidity() internal virtual {
        ...
        if(!_inSwap && tokenBalance >= _addLiquidityAmount) {
            _inSwap = true;
