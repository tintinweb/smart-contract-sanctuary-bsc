...
contract BearishAI is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BearishAIDividendTracker public dividendTracker;

    address payable public marketingWallet;
    address payable public buybackfeeWallet;
    ...
    uint256 public buyBuybackfeeFee;
    uint256 public sellBuybackfeeFee;
    ...
    event UpdateFees(
        ...
        uint256 buyBuybackfeeFee,
        uint256 sellBuybackfeeFee
    );
    ...
    constructor() ERC20("BearishAI", "BearishAI") {
        marketingWallet = payable(0x9Bdc2A3739da1c9d783d7099Ff15A4a9c45885f0);
        buybackfeeWallet = payable(0x9Bdc2A3739da1c9d783d7099Ff15A4a9c45885f0);
        ...
        totalBuyFees = buyRewardsFee
            ...
            .add(buyBuybackfeeFee);
        totalSellFees = sellRewardsFee
            .add(sellLiquidityFee)
            .add(sellMarketingFees)
            .add(sellBuybackfeeFee);

        dividendTracker = new BearishAIDividendTracker(
            payable(this),
            router,
        ...
        _isExcludedFromFees[address(buybackfeeWallet)] = false;
    ...
    function updateFees(
        ...
        uint256 RewardsBuy,
        uint256 RewardsSell,
        uint256 buybackfeeBuy,
        uint256 buybackfeeSell
    ) public onlyOwner {
        buyDeadFees = deadBuy;
        ...
        buyBuybackfeeFee = buybackfeeBuy;
        sellBuybackfeeFee = buybackfeeSell;
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            if (!isBuying && !isSelling) {
                ...
                buybackfeeFees = sellBuybackfeeFee;
                ...
                buybackfeeFees = buyBuybackfeeFee;
            ...
            uint256 totalFees = RewardsFee
                .add(liquidityFee + marketingFees + buybackfeeFees);
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        if (buyRewardsFee > 0) {
            ...
            .div(totalAmount)
            .mul(buyRewardsFee)
            .div(buyRewardsFee + buyMarketingFees + buyBuybackfeeFee);
        }
        if (sellRewardsFee > 0) {
            ...
            .div(totalAmount)
            .mul(sellRewardsFee)
            .div(sellRewardsFee + sellMarketingFees + sellBuybackfeeFee);
        }
        ...
        uint256 _completeFees = sellMarketingFees.add(sellBuybackfeeFee) +
            buyMarketingFees.add(buyBuybackfeeFee);
        ...
        uint256 buybackfeePayout = buyBuybackfeeFee.add(sellBuybackfeeFee) * feePortions;

        if (marketingPayout > 0) {
            (successOp1, ) = address(marketingWallet).call{value: marketingPayout}("");
        }
        if (buybackfeePayout > 0) {
            (successOp2, ) = address(buybackfeeWallet).call{value: buybackfeePayout}("");
        }

        emit SendDividends(
            dividends,
            marketingPayout + buybackfeePayout,
...
contract BearishAIDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    BearishAI public BearishAIContract;
    ...
    constructor(
        ...
        defaultToken = token;
        BearishAIContract = BearishAI(mainContract);
    ...
    function _transfer(
        ...
        require(false, "BearishAI_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "BearishAI_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BearishAI contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(BearishAIContract);

            uint256 prevBalance = BearishAIContract.balanceOf(address(this));
            ...
            {
                uint256 received = BearishAIContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    BearishAIContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
