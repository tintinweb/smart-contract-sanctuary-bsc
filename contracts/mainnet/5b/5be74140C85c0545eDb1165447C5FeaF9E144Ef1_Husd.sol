...
contract Husd is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the HUSD BEP-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("High USD", "HUSD") {
        // Mints 1 HUSD to contract creator for initial pool setup
    ...
    function _getHusdPrice() internal view returns (uint256 _HusdPrice) {
        try IOracle(HusdOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Husd: failed to fetch HUSD price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _HusdPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_HusdPrice >= taxTiersTwaps[tierId]) {
    ...
    function setHusdOracle(address _HusdOracle) public onlyOperatorOrTaxOffice {
        require(_HusdOracle != address(0), "oracle address cannot be 0 address");
        HusdOracle = _HusdOracle;
    }
    ...
    /**
     * @notice Operator mints HUSD to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of HUSD to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentHusdPrice = _getHusdPrice();
            currentTaxRate = _updateTaxRate(currentHusdPrice);
            if (currentHusdPrice < burnThreshold) {
                burnTax = true;
            }
        }
