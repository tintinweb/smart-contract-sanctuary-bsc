...
contract AgentManager is Ownable {
    ...
    function removeZeroList(uint256 _pid) internal {
        DebtInfo[] storage debtList = debtInfo[_pid];
        uint256 index = 0;
    ...
    function compound(uint256 _pid) public {
        UserInfo storage user = userInfo[_pid][msg.sender];
        updateUser(_pid, msg.sender);
        user.amount = user.amount.add(user.pending);
        user.pending = 0;
        emit Compound(msg.sender, _pid, user.pending);
    }
    ...
    function UpdateAgents(uint256[] memory _tids, uint256[] memory _depositAmount, uint256[] memory _withdrawAmount, uint256[] memory _unlockAmount) public onlyOwner{
        for (uint256 i = 0; i < _tids.length; i++) {
            TargetInfo memory target = targetInfo[_tids[i]];
            ...
            // deposit
            j = 0;
            while(j < agentList.length && _depositAmount[i] > 0) {
                IAgent agent = IAgent(agentList[j]);
                uint256 available = agent.availableDeposit(_depositAmount[i]);
                if(available > 0) {
                    available = available > _depositAmount[i] ? _depositAmount[i] : available;
                    IERC20(target.depositToken).transfer(address(agent), available);
                    agent.deposit();
                    _depositAmount[i] = _depositAmount[i].sub(available);
                }
                j++;
            }
            if(_depositAmount[i] > 0) {
                _createNewAgent(_tids[i], _depositAmount[i]);
            }
            // remove blank agent
            j = 0;
