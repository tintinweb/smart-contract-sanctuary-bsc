...
contract abc is IERC20,Ownable  {
    ...
    mapping (address => bool) public whiteBuyers;
    
    string private _name="abc";
    string private _symbol="abc";
    uint8 private _decimals=18;

    uint256 public nftBonusFee=2;
    uint256 public addLpFee =2;
    uint256 public shareLpFee=5 ;
    uint256 public consensusFundFee =2;
    uint256 public operationFee =5;
    uint public nodeForceFee=2;


    address public nftBonus; //2%
    address public consensusFund;
    address public operation;
    address public nodeForce;
    ...
    uint256 private _tTotal=210000000e18;
    ...
    function setAddrs(address _nftBonus,address _consensusFund,address _operation,address _node)external onlyOwner {
        nftBonus = _nftBonus;
        consensusFund = _consensusFund;
        operation = _operation;
        nodeForce = _node;
    }

    function changeStartBuy() external onlyOwner {
        startBuy = !startBuy;
    }

    function changeStartSell() external onlyOwner {
        startSell = !startSell;
    }
    ...
    function setDividExempt(address[] calldata _users,bool _v) external onlyOwner  {
    ...
    function setExcludeFromFee(address[] calldata _accounts,bool _v) external onlyOwner  {
       ...
       function setWhiteBuyers(address[] calldata _accounts,bool _v) external onlyOwner  {
        for(uint i=0;i< _accounts.length;i++){
            whiteBuyers[_accounts[i]] = _v;
        }
    }
    ...
    function setInitialize(
        uint _shareAmount,
        uint _addTokenAmount,
        uint _minShareLp,
        uint _minSharePeriod,
        uint _gas,
        address _temp
     ) external onlyOwner {

        shareAtAmount = _shareAmount;
        addTokensAtAmount = _addTokenAmount;
        minShareLp = _minShareLp;
        minPeriod = _minSharePeriod;
        distributorGas = _gas;
    ...
    function _transfer(
        ...
        if (from == uniswapV2Pair){ //buy
            require(startBuy || whiteBuyers[to],"not start");
    ...
    function _tokenTransfer(
        ...
        if(isSell){
            ...
            _takeAddLPFee(sender, tAmount.div(100).mul(addLpFee));
            _takeNodeForceFee(sender,tAmount.div(100).mul(nodeForceFee));

        }else{
            _takeShareLPFee(sender, tAmount.div(100).mul(shareLpFee));
            _takeNftBonusFee(sender,tAmount.div(100).mul(nftBonusFee));
            _takeConsensusFundFee(sender,tAmount.div(100).mul(consensusFundFee));

        }


        _tOwned[recipient] = _tOwned[recipient].add( tAmount.div(100).mul(91));
        emit Transfer(sender, recipient,tAmount.div(100).mul(91));
    }



     function _takeConsensusFundFee(
        address sender,
        uint256 tAmount
    ) private {
        _tOwned[consensusFund] = _tOwned[consensusFund].add(tAmount);
         emit Transfer(sender, consensusFund, tAmount);
    }
    ...
    function _takeOperationFee(
        ...
        _tOwned[operation] = _tOwned[operation].add(tAmount);
         emit Transfer(sender, operation, tAmount);
    }

  function _takeNodeForceFee(
        address sender,
        uint256 tAmount
    ) private {
        _tOwned[nodeForce] = _tOwned[nodeForce].add(tAmount);
         emit Transfer(sender, nodeForce, tAmount);
    }

     function _takeNftBonusFee(
        address sender,
        uint256 tAmount
    ) private {
        _tOwned[nftBonus] = _tOwned[nftBonus].add(tAmount);
         emit Transfer(sender, nftBonus, tAmount);
    }
