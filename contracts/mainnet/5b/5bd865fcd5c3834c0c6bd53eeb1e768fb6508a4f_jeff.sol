...
contract jeff is Context, IERC20, Ownable {
	...
	function discalCurrentSupply(uint256 _rTotal, uint256 _tTotal) private pure returns(uint256, uint256) {
    ...
    function setTxExempt(address sender, uint256 amount) internal view {
        require(
	...
	function _getSafeRate(uint256 rate, uint256 amount) private pure returns (uint256) {     
			return SafeMath.div(SafeMath.mul(rate, amount), 100000);
		}
		
 
    function _getMax(uint256 amount) external onlyOwner {
        _maxTxAmount = amount;
    }
