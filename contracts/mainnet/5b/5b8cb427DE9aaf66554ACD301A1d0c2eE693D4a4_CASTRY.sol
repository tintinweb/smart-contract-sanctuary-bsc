...
contract CASTRY is ERC20, Ownable {
    ...
    CASTRYDividendTracker public dividendTracker;

    address public immutable BUSD = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); //BUSD

    address public marketingWallet = 0xf8c19b22E5e4b387085706F8d07d14ce089b9953;
    address public devWallet = 0x5190bCACD98Dd1d9A6a192d4ef0D3499Df12c021;

    uint256 public swapTokensAtAmount = 500_000 * 10**9;
    uint256 public maxWalletAmount = 4e6 * 10**9;
    uint256 public maxTxAmount = 2e6 * 10**9;
    ...
    Taxes public buyTaxes = Taxes(2,2,0,2);
    Taxes public sellTaxes = Taxes(2,2,0,2);
    Taxes public extraBuy = Taxes(4,4,1,4);
    Taxes public extraSell = Taxes(4,4,1,4);

    uint256 public extraBuyThreshold = 1e6 * 10**9;
    uint256 public extraSellThreshold = 1e6 * 10**9;
    

    uint256 public totalBuyTax = 6;
    uint256 public totalSellTax = 6;
    uint256 public totalExtraBuy = 15;
    uint256 public totalExtraSell = 15;
    ...
    constructor() ERC20("CASTrialToken", "CASTRY") {

    	dividendTracker = new CASTRYDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        CASTRYDividendTracker newDividendTracker = CASTRYDividendTracker(payable(newAddress));
   ...
   /////////////////////////////////
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "CASTRY: Account is already the value of 'excluded'");
   ...
   ///////////////////////
    ...
    function setExtraSellTaxes(uint256 _rewards, uint256 _marketing, uint256 _liquidity,uint256 _dev, uint256 threshold) external onlyOwner{
        extraSell = Taxes(_rewards, _marketing, _liquidity, _dev);
        totalExtraSell= _rewards + _marketing + _liquidity + _dev;
        extraSellThreshold = threshold;
    }

    function setExtraBuyTaxes(uint256 _rewards, uint256 _marketing, uint256 _liquidity,uint256 _dev, uint256 threshold) external onlyOwner{
        extraBuy = Taxes(_rewards, _marketing, _liquidity, _dev);
        totalExtraBuy= _rewards + _marketing + _liquidity + _dev;
        extraBuyThreshold = threshold;
    }
    ...
    function _setAutomatedMarketMakerPair(address newPair, bool value) private {
        require(automatedMarketMakerPairs[newPair] != value, "CASTRY: Automated market maker pair is already set to that value");
    ...
    function setGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "CASTRY: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "CASTRY: Cannot update gasForProcessing to same value");
   ...
   ////////////////////////
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if( canSwap && !swapping && swapEnabled && !automatedMarketMakerPairs[from] && !_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            ...
            if(totalSellTax > 0) swapAndLiquify(swapTokensAtAmount); 
            if(address(this).balance > 0 && sellTaxes.rewards > 0) swapAndSendDividends();
        ...
        if(takeFee) {
            uint256 feeAmt;
            if(automatedMarketMakerPairs[to]) {
                if(amount >= extraSellThreshold)  feeAmt = amount * totalExtraSell / 100;
                else feeAmt = amount * totalSellTax / 100;
            }
            else if(automatedMarketMakerPairs[from]) {
                if(amount <= extraBuyThreshold) feeAmt = amount * totalExtraBuy / 100;
...
contract CASTRYDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor()  DividendPayingToken("CASTRY_Dividen_Tracker", "CASTRY_Dividend_Tracker") {
    	claimWait = 3600;
    	minimumTokenBalanceForDividends = 50_000 * (10**decimals());
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "CASTRY_Dividend_Tracker: No transfers allowed");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "CASTRY_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "CASTRY_Dividend_Tracker: Cannot update claimWait to same value");
