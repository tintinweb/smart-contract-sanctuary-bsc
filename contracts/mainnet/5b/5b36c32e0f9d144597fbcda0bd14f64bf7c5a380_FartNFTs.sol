...
contract FartNFTs is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  constructor(string memory name, string memory symbol, string memory baseTokenURI) ERC721(name, symbol) {
      ...
      // Save dev wallet
      _wallet = 0x09b2336087a76490cc000d4096f666d2c1A9DBb8;
      _regularMint();
  }
  ...
  function mint(uint256 _mintAmount) public payable {
    // Checks
    require(!paused, "Fart: minting has not started yet");
    require(_mintAmount > 0, "Fart: you have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "Fart: you can only mint 10 at a time");
    require(lastSupply >= _mintAmount,  "Fart: the collection is sold out");
    if (msg.sender != owner()) {
      require(msg.value >= cost * _mintAmount, "Fart: total cost isn't match");
    }
    // Minting
  ...
  function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
    // Checks
    require(_winner != address(0), "Fart: zero address");
    require(_amount > 0, "Fart: you have to mint at least one");
    require(lastSupply != 0,  "Fart: the collection is sold out");
  ...
  function tokenURI(uint256 _id) public view virtual override returns (string memory){
    require(_exists(_id),"Fart: URI query for nonexistent token");
