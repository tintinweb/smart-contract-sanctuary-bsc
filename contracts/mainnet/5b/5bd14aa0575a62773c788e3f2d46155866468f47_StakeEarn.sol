...
contract StakeEarn is Ownable, ReentrancyGuard {
    ...
    function initialize(uint256 _rewardRate, uint256 _minStakePerUser) external onlyOwner {
    ...
    function deposit(uint256 _amount) external nonReentrant {
		...
		if (_amount > 0) {
			 ...
			 user.rate = rewardRate;
			 user.rewardRemaining = user.rewardRemaining.add(pendingreward(msg.sender));
    ...
    function withdraw() public nonReentrant {
		...
		uint256 pending = user.rewardRemaining.add(pendingreward(msg.sender));
	...
	function pendingreward(address _user) public view returns (uint256) {
		...
		if(user.amount > 0)
		{
			...
			uint256 reward = user.amount.mul(user.rate).div(10000).mul(eTime.sub(sTime).div(TIME_STEP));
			return reward;
		}
		else
		{
	...
	function getUserInfo(address userAddress) public view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        return (user.amount, user.rewardRemaining, user.rewardWithdrawl, user.startTime, user.rate);
    }
