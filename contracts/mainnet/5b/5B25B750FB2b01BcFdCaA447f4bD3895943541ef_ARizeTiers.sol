...
contract ARizeTiers is Ownable {
  using SafeMath for uint256;
   ...
   * @param {minLimit} Minimum amount of arz to be staked to join tier
   * @param {maxLimit} Maximum amount of arz to be staked to join tier
   *
   */
  struct Tier {
    uint256 minLimit;
  ...
  IDecubateMasterChef private compoundStakingContract; //Staking contract instance
  DCBVault private compounderContract; //Staking contract instance
  address private tokenAddress; //Token instance

  Tier[] public tierInfo; //Tier storage

  constructor(
    IDecubateMasterChef _stakingContract,
    DCBVault _vault,
    address _token
  ) {
   ...
   *
   */
  function setCompoundingStakingContract(IDecubateMasterChef _stakingContract)
    external
    onlyOwner
  {
    compoundStakingContract = IDecubateMasterChef(_stakingContract);
  ...
  /**
   *
   * @dev set address of arz token contract
   *
   */
  ...
  /**
   *
   * @dev Get total amount of arz staked by a user
   *
   * @param addr Address of the user
   *
   * @return amount Total amount of arz staked
   */

  function getTotalDeposit(address addr) public view returns (uint256 amount) {
    uint256 tempAmt;
    uint256 compound_len = compoundStakingContract.poolLength();

    for (uint256 i = 0; i < compound_len; i++) {
    ...
    )
  {
    ...
    if (!flag && totalDeposit > tierInfo[len - 1].maxLimit) {
      ...
      // 80,0000+ ARZ. The max limit of the tier should be set
      // 159,999 ARZ and when the limit is passed the compounding
