...
contract FantomHouseBusd is VRFConsumerBaseV2, Ownable, ReentrancyGuard {
    ...
    IERC20 private busd = IERC20(BUSD_ADDRESS);
    ...
    function depositHouse(uint256 _amount, address _referral) external {
        require(depositors.length <= 50, "Depositor club is already full! Please visit Discord for more information.");
    ...
    function refund() external nonReentrant {
        uint256 _betIdIndex;
        for (uint256 i = startBetIdIndex; i < ids.length; i++) {
            Bet memory _bet = bets[ids[i]];
            if (block.number >= _bet.block + 400) {
                if (_bet.pending == 1) {
    ...
    function bet(uint256 _betAmount, uint256 _faceSide, address _referral) external nonReentrant {
        ...
        if (_lastBetBlock != 0 && block.number - _lastBetBlock < 5) {
        ...
        busd.safeTransferFrom(_sender, address(this), _betAmount);
    ...
    function returnDepositAndRemoveDepositor(uint256 _length, bool _shouldRemoveDepositor) internal {
        for (uint i = 0; i < _length; i++) {
            ...
            busd.safeTransfer(_depositor, _depositAmount);
