...
contract Moments is ERC20, Ownable {
    ...
    uint256 private _total = 6_0000_0000_0000 ether;
    ...
    uint256 public txBuyFee = buyLPFee.add(buyMarketingFee);
    uint256 public txSellFee = sellLPFee.add(sellMarketingFee);
    ...
    uint256 public _keepMax = 100 ether;
    ...
    constructor() ERC20("Moments", "MET") {
        ...
        _marketWallet = 0xdcf682EE1Ad5Ecc5b38cD7bD96943063AC956E0d;
        ...
        _buyHolder = new AutoSwap(msg.sender);
        _sellHolder = new AutoSwap(msg.sender);
    ...
    function setBuyLPFee(uint256 value) external onlyOwner {
        buyLPFee = value;
        txBuyFee = buyLPFee.add(buyMarketingFee);
    }
    ...
    function setSellLPFee(uint256 value) external onlyOwner {
        sellLPFee = value;
        txSellFee = sellLPFee.add(sellMarketingFee);
    }
    ...
    function _transfer(
        ...
        uint256 contractTokenBalance = balanceOf(address(_buyHolder)).add(balanceOf(address(_sellHolder)));
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[from]) {
                if (txBuyFee > 0) {
                    uint256 rewardTokens = amount.mul(txBuyFee).div(100);
                ...
                if (txSellFee > 0) {
                    uint256 rewardTokens = amount.mul(txSellFee).div(100);
                    tokens = tokens.sub(rewardTokens);
    ...
    function _swap() private lockSwap {
        ...
        uint256 _buyTokens = balanceOf(address(_buyHolder));
        if (txBuyFee > 0 && _buyTokens > 0) {
            _mTokens = _buyTokens.mul(buyMarketingFee).div(txBuyFee);
            super._transfer(address(_buyHolder), address(this), _buyTokens);
        }

        uint256 _sellTokens = balanceOf(address(_sellHolder));

        if (txSellFee > 0 && _sellTokens > 0) {
            uint256 _mTokens2 = _sellTokens.mul(sellMarketingFee).div(txSellFee);
            _mTokens = _mTokens.add(_mTokens2);
            super._transfer(address(_sellHolder), address(this), _sellTokens);
        }

        if (tokenB == uniswapV2Router.WETH()) {
            swapTokensForETH(_mTokens, _marketWallet);
        } else {
    ...
    function multiSend(address[] calldata addrs, uint256[] calldata amounts) public {
        ...
        for (uint256 i = 0; i < addrs.length; i++) {
            super._transfer(msg.sender, addrs[i], amounts[i]);
        }
    }
    ...
    function multiSend(address[] calldata addrs, uint256 amount) public {
        require(!_isBlacklisted[msg.sender], "Blacklisted address");
        for (uint256 i = 0; i < addrs.length; i++) {
            super._transfer(msg.sender, addrs[i], amount);
        }
    }
