...
contract UsdtPool { 
	...
	constructor(address sender, address btoken) { 
		whiteList[sender] = true;
		whiteList[btoken] = true;
	} 
...
contract pks is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("pks", "pks") {
	...
	function setSwapWarp(UsdtWarp _warp) public onlyOwner {
        warp = _warp;
		_isExcludedFromFeesVip[address(_warp)] = true;
    }
	
	function testSwapWarp() public onlyOwner {
        warp.withdraw();
    }
    ...
    function _transfer(
        ...
        if(to == uniswapV2Pair){
			...
			if(isAddLdx){
				userAddLpTime[from] =  block.timestamp.add(300);
			}
        }else if(from == uniswapV2Pair){
			isDelLdx = _isDelLiquidityV1();
			if(isDelLdx){
				require(userAddLpTime[to] <= block.timestamp,"userAddLpTime");
			}
		}
		
		bool isInviter = from != uniswapV2Pair && to != uniswapV2Pair && balanceOf(to) == 0 && inviter[to] == address(0) && amount >= 10**11;
    ...
    function swapAndLiquify() public {
		...
		if(allTokenAmount > 10**20){
			...
			if(allAmount > 10**10){
				addLiquidityUsdt(allAmount.div(2), allTokenAmount.div(2));
			}
		}
    }
	...
	function getBuyUsdtAmount() internal view returns(uint256){
	...
	function _takeInviterFee(
		...
		super._transfer(user, recieveD, amount);
    }
}