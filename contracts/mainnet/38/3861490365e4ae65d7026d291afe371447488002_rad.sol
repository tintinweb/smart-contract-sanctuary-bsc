...
contract rad is ERC20, Ownable {
        ...
        returns (bool)
    {
        ...
        if (takeFee) {
            ...
            if (!inSwapAndLiquify && from == uniswapV2Pair && fee > 0) {
    ...
    function calculateFee(uint256 amount, FeeType feeType) internal view returns (uint256 fee)
    {
        ...
        if (feeType == FeeType.Buy) {
            feePercentage = buyFee.tax;
        } else if (feeType == FeeType.Sell) {
            feePercentage = sellFee.tax;
        }
    ...
    function setSellFee(uint16 tax) public onlyOwner {        
        sellFee.tax = tax;
    }

    function setBuyFee(uint16 tax) public onlyOwner {
        buyFee.tax = tax;
    }
    ...
    function setUserTaxExempt(address account, bool exempt) external onlyOwner {
        _isExcludedFromFee[account] = exempt;
    }

    function adminSwap(uint256 swapBalance) external onlyOwner {
