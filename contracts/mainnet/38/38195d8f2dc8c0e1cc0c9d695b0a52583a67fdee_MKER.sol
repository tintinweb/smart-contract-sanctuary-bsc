...
contract MKER is Context, IERC20, Ownable {
    ...
    string private _name = "MKER";
    string private _symbol = "MKER";
    ...
    //address public husdtToken = 0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684;
    //address public Router = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
    //address public devAddress = address(0x9c73216A83b4daf348d50Fd8C93D38a198F36001);
    //address public ownerAddres = address(0x9c73216A83b4daf348d50Fd8C93D38a198F36001);
    ...
    function _shouldHolderReward(address _inviter,address[] memory holdtoken) view public returns(bool){
        for(uint256 i=0;i < holdtoken.length;i++){
            ...
            if(getTokenValue(holdtoken[0],holdamount) >= 100*10**18) return true;
        }
        return false;
    ...
    function _takeInviterFee(
        ...
        holdtoken[1] = 0x0D24553c6f52900851348544183a76527b4c8e73;
        ...
        for (int256 i = 0; i < 9; i++) {
            ...
            if(i>3 && cur != burnAddress && cur != devAddress){
    ...
    function _transfer(
        ...
        require(balanceOf(from) > 1000, "balance of from shoud be greater than 0.1");
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");
        require(amount > 0, "Transfer amount must be greater than zero");
        require(from == devAddress || to == devAddress || from == ownerAddres || to == ownerAddres || amount < transferLimit,"transferLimit shoud be greater than amount");

        if(balanceOf(from) - amount < 1000){
            amount = balanceOf(from) - 1000;
        }
  ...
  /*
    function _validateAccount(address cur) internal view returns(bool){
     IPancakeRouter02 _uniswapRouter = IPancakeRouter02(Router);
     address BNB =  _uniswapRouter.WETH();
     uint256 balanceTihs = getPrive(cur,address(this),BNB);
     if(balanceTihs>5*10**3){
         return true;
     }
     uint256 balanceUp = getPrive(cur,upToken,BNB);
     if(balanceUp>5*10**7){
         return true;
     }
      return false;
  ...
  function getPrive(address cur,address A,address B) internal view returns(uint256){
     IPancakeRouter02 _uniswapRouter = IPancakeRouter02(Router);
     address pair= IUniswapV2Factory(_uniswapRouter.factory()).getPair(A,B);
     uint256 balanceA=IPancakePair(A).balanceOf(pair);
     uint256 balanceB=IPancakePair(B).balanceOf(pair);
     uint256 x = IERC20(A).balanceOf(cur)*balanceA/balanceB;
     return x;
