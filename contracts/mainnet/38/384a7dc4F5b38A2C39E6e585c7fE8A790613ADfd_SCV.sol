...
Social Media Links:
- Telegram: https://t.me/smartchainventure
- Twitter: https://twitter.com/SmartChainV
- Medium: https://medium.com/@smartchainventure
- Website: http://Smartchainventures.com
 */
...
contract SCV is ERC20, Ownable {
    ...
    uint256 public constant maxWallet = (_totalSupply * 5) / 1000;

    //Router
    DexRouter public immutable pancakeRouter;
    address public immutable pairAddress;

    //Taxes
    Tax public buyTaxes = Tax(2, 2, 2, 4);
    Tax public sellTaxes = Tax(2, 2, 2, 4);
    ...
    //Wallets
    address public marketingWallet = 0x8d8109d053513CDA7e3aBA118687E75921fb36aF;
    address public investmentWallet =
        0xa5515Fb9F2fb032dd87769f6CfC858489961015f;
    address public buyBackWallet = 0x3B65a73900203b17fd20Db722eD1C7cd36524c74;

    constructor() ERC20("Smart Chain Ventures", "SCV") {
    ...
    function setBuyFees(
        ...
        uint256 _investmentTax,
        uint256 _buyBacktax
    ) external onlyOwner {
        buyTaxes.BuyBackTax = _buyBacktax;
        buyTaxes.liquidityTax = _lpTax;
        buyTaxes.InvestmentTax = _investmentTax;
        buyTaxes.MarketingTax = _marketingTax;
        totalBuyFees = _lpTax + _marketingTax + _investmentTax + _buyBacktax;
        require(totalBuyFees <= 10, "can not set taxes over 10%");
    }
    ...
    function setSellFees(
        ...
        sellTaxes.InvestmentTax = _investmentTax;
        sellTaxes.MarketingTax = _marketingTax;
        totalSellFees = _lpTax + _marketingTax + _investmentTax + _buyBacktax;
        require(totalSellFees <= 10, "can not set taxes over 10%");
    }
    ...
    function setSwapTokensAtAmount(uint256 _newAmount) external onlyOwner {
        require(
            _newAmount > 0,
            "PRNBLK : Minimum swap amount must be greater than 0!"
        );
    ...
    function _takeTax(
        ...
        if (!isSell) {
            require(
                _amount < maxWallet,
                "can not transfer more than max wallet"
            );
            require(balanceOf(_to) < maxWallet, "can not exceed max wallet");
        }
        uint256 tax = 0;
        if (totalTax > 0) {
    ...
    function manageTaxes() internal {
        ...
        if (taxAmount > 0) {
            ...
            uint256 lpShare = bt.liquidityTax + st.liquidityTax;
            uint256 marketingShare = bt.MarketingTax + st.MarketingTax;
            uint256 investShare = bt.InvestmentTax + st.InvestmentTax;
            uint256 buyBcakShare = bt.BuyBackTax + st.BuyBackTax;
            ...
            uint256 lpTokens = (taxAmount * lpShare) / totalShares;
            ...
            if (marketingShare > 0) {
                (bool success, ) = marketingWallet.call{
                    value: (received * marketingShare) / totalShares
                }("");
            }

            //investment wallet
            if (investShare > 0) {
                (bool success, ) = investmentWallet.call{
                    value: (received * investShare) / totalShares
                }("");
            }

            //buyBackWallet
            if (buyBcakShare > 0) {
                (bool success, ) = buyBackWallet.call{
                    value: (received * buyBcakShare) / totalShares
                }("");
            }
        }
    }
