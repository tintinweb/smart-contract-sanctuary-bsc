...
contract KshareRewardPool is ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // The time when kshare mining starts and ends.
    ...
    uint256 public immutable ksharePerSecond;
    ...
    constructor(
        ...
        ksharePerSecond = TOTAL_REWARDS.div(runningTime);
    }
    ...
    function pendingShare(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 accKsharePerShare = poolToken.accKsharePerShare;
        ...
        if (block.timestamp > poolToken.lastRewardTime && tokenSupply != 0) {
            ...
            uint256 _kshareReward = _generatedReward.mul(poolToken.allocPoint).div(totalAllocPoint);
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            ...
            //Add (rewards/tokensInPool) to the accumulated Kshare per Share. Based in ether - hence .div(1e18) later on.
            poolToken.accKsharePerShare = poolToken.accKsharePerShare.add(_kshareReward.mul(1e18).div(tokenSupply));
        }
        poolToken.lastRewardTime = block.timestamp;
    }
    ...
    function _deposit(
        ...
        uint256 _pending = user.amount.mul(poolToken.accKsharePerShare).div(1e18).sub(user.rewardDebt);
        if (_pending > 0) {
            safeKshareTransfer(_recipient, _pending);
        ...
        user.rewardDebt = user.amount.mul(poolToken.accKsharePerShare).div(1e18);
