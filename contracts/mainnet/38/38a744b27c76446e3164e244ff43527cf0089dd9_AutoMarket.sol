...
// File: contracts\libs\pancakeswap-peripheral\contracts\interfaces\IPancakeRouter01.sol
...
// File: contracts\libs\pancakeswap-peripheral\contracts\interfaces\IPancakeRouter02.sol
...
// File: contracts\libs\interfaces\IAuthorizedList.sol
...
// File: contracts\libs\AuthorizedList.sol
...
// File: contracts\libs\LPSwapSupport.sol
...
abstract contract LPSwapSupport is AuthorizedList {
    ...
    IERC20 public BUSD;
    TempRewardReceiver private tempRewardReceiver;
    ...
    /**Swap Token to BUSD */
    function swapTokensForCurrency(address tokenAddress, uint256 tokenAmount) internal {
        swapTokensForCurrencyAdv(tokenAddress, tokenAmount, address(this));
    }

    /** Swap BUSD to Token */
    function swapBusdForToken(uint256 busdAmount, address tokenAddress) internal {
        swapBusdForTokenAdv(address(BUSD), tokenAddress, busdAmount, address(this));
    }

    function swapBusdForTokenAdv(
        address busdAddress,
        ...
        IERC20(busdAddress).approve(address(pancakeRouter), busdAmount);

        swapTokens(busdAddress, tokenAddress, busdAmount, destination);
    }
...
// File: contracts\AutoMarket.sol

pragma solidity ^0.6.12;
contract AutoMarket is LPSwapSupport {
    ...
    // Add event log for pushSwap
    event PushSwap(address indexed owner, uint256 amount);
    event WithdrawToken(address indexed spender, uint256 amount);

    constructor(address routerAddress, address _busd, uint256 _amount, address _token) public {
        swapsEnabled = false;
    ...
    function pushSwap() external authorized {
    ...
    function selectSwapEvent() private lockTheSwap {
        ...
        require(BUSD.allowance(_msgSender(), address(this)) >= busdAmount, "PushSwap: busd amount exceeds allowance");

        BUSD.transferFrom(_msgSender(), address(this), busdAmount);
        
        // User Amount
        users[_msgSender()] = users[_msgSender()].add(busdAmount);
        totalAmount = totalAmount.add(busdAmount);

        uint256 contractBUSDBalance = BUSD.balanceOf(address(this));
    
        swapBusdForToken(contractBUSDBalance, address(Token));

        // uint256 contractTokenBalance = Token.balanceOf(address(this));

        // swapTokensForCurrency(address(Token), contractTokenBalance);

        // Withdraw BUSD to msg sender
        // uint256 amount = BUSD.balanceOf(address(this));
        // BUSD.transfer(_msgSender(), amount);

        emit PushSwap(_msgSender(), contractBUSDBalance);
    } 

    function updateTokenAddress(address newAddress) public authorized {
        require(newAddress != address(0));
        Token = IERC20(newAddress);
    } 

     function withdrawToken() external authorized{
        uint256 tokenBalance = Token.balanceOf(address(this));
        Token.transfer(_msgSender(), tokenBalance);
        emit WithdrawToken(_msgSender(), tokenBalance);
    } 

    function updateBusdAmount(uint256 newBusdAmount) external onlyOwner {
        require(newBusdAmount> 0, "Swap busd amount is not less than zero");
        busdAmount = newBusdAmount;
    }
