...
contract DividendDistributor is IDividendDistributor,Ownable {
    ...
    constructor (address ceo_,address rewardToken_) {
    ...
    function setPair(address pair,bool tf) public override { 
    ...
    function setDistributionCriteria(uint newMinPeriod, uint newMinDistribution) external override onlyOwner{ 
    ...
    function recoverERC20(address tokenAddress,address  to, uint tokenAmount)external onlyOwner{
        
        IERC20(tokenAddress).transfer(to, tokenAmount);
    }
    function recoverBNB(address  to,uint tokenAmount) external onlyOwner{
        payable(to).transfer(tokenAmount);
    }
    function getTokenForUserLp(address account) public view returns (uint token0) {
        if(pairs.length>0){
            for (uint index = 0; index < pairs.length; index++) {
                token0=token0.add(getTokenForPair(pairs[index],account));
            }  
        }
    } 
    function getTokenForPair(address pair,address account) public view returns (uint token0) {
        uint all=rewardToken.balanceOf(pair);
        uint lp=IERC20(pair).balanceOf(account);
        if(lp>0) token0=all.mul(lp).div(IERC20(pair).totalSupply());
    }
    ...
    function setShare(address wait) external override{
        require(_msgSender() == _token);
        if(pending.length()>=4){  
...
contract Token is ERC20, ERC20Burnable, Ownable {
    ...
    constructor(string memory name_,string memory symbol_,uint total_) ERC20(name_, symbol_) {
        ...
        allot=new DividendDistributor(ceo,_baseToken);
