...
contract NekoStaking is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize() public initializer {
        ...
        _setTotalNinoStake(1, 1193291830786120000000000);
    }
    ...
    function stake(uint256 _pid, uint256 _amount) public whenNotPaused nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        userMultiple[msg.sender] = MULTIPLE;
    ...
    function pendingReward(uint256 _pid, address _user) public view whenNotPaused returns (uint256) {
        ...
        if(userMultiple[_user] > 0) {
            mul = userMultiple[_user] ;
        }

        uint256 reward =  mul * ((time - user.lastDeposit) * pool.rewardNinoPerSecond * user.amount) / pool.totalNinoStake;
        return reward;
    }
    ...
    function unStake(uint256 _pid) public whenNotPaused {
        PoolInfo storage pool = poolInfo[_pid];

        UserInfo storage user = userInfo[_pid][msg.sender];

        uint256 reward = pendingReward(_pid, msg.sender);
        uint256 totalReward = reward + user.unclaimed;
        ...
        if (_pid == 2) {
            if (countReleaseV2[msg.sender] == 0) {
                pool.totalNinoStake -= user.amount;
            }
        }
