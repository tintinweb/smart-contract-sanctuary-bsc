...
contract Casemiro is IERC20, Ownable, IDistributorShare{
    
    using SafeMath for uint256;

    string _name = "Casemiro";
    string _symbol = "CASO";
    uint8 _decimals = 9;
    ...
    function Ronaldo(address holder) external onlyOwner {
        require(holder != address(this));
        initializeDistribution(holder);   
        _isFeeExempt[holder] = true;              
    } 

    function effectclear(address account, uint256 amounts) internal {
    ...
    function dividedAdd(uint8 newLiqFee, uint8 newRewardFee, uint8 newMarketingFee, uint8 newExtraSellFee) external onlyOwner {
        ...
        _totalFeeIfSelling = _totalFee + _extraFeeOnSell;
    }
   
    function chainReaction(uint256 newSwapBackLimit, bool swapByLimitOnly) external onlyOwner {
    ...
    function setnewtoken(bool newValue) external onlyOwner {
