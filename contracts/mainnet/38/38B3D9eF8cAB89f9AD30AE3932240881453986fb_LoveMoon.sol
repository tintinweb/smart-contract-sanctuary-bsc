...
contract LoveMoon is Context, IERC20, Ownable {
    ...
    string public _name = "LoveMoon";
    string public _symbol ="LoveMoon";
    uint8  public _decimals = 9;

    uint256 public _totalSupply = 1000000000000* 10**9;
    uint256 public _minimumTokensBeforeSwap = 2 *10E8* 10**9;
    
    address payable private marketingWalletAddress = payable(0x0C005293C2f875eE76a943137b9ADa35904aa09B); // Marketing Address
   ...
   event BlacklistMultipleAddresses(address[] accounts, bool value);
    ...
    constructor ( ) {
        ...
        isExcludedFromFee[liquidityReceiver] = true;      
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if((ammPairs[sender]  || ammPairs[recipient]) && swapstatus == false){
            require(whitelist[sender] || whitelist[recipient],"only whitelist!");
        }
        ...
        else
        {
            ...
            if (overMinimumTokenBalance && !inSwapAndLiquify && !ammPairs[sender] && swapAndLiquifyEnabled)
            {
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 totalForFee = _totalDistributionShares.sub(_liquidityShare.div(2));

        uint256 amountForLiquidity = amountReceived.mul(_liquidityShare).div(totalForFee).div(2);
        uint256 amountForMarketing = amountReceived.sub(amountForLiquidity);

        if(amountForMarketing > 0)
            transferToAddressETH(marketingWalletAddress, amountForMarketing);


        if(amountForLiquidity > 0 && tokensForLP > 0)
