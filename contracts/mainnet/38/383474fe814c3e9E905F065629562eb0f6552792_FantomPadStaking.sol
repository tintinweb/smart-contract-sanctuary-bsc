...
function _hash(Signer memory whitelist) internal view returns (bytes32) {
        return _hashTypedDataV4(keccak256(abi.encode(
                keccak256("Signer(address userAddress,address contractAddress,uint256 id,uint256 timestamp)"),
...
pragma solidity ^0.8.15;
    ...
    contract FantomPadStaking is OwnableUpgradeable,whitelistChecker {
        ...
        function withdrawStakedTokens(uint poolId,Signer memory _signer) external {
        ...
        function getReward(address _user, uint _poolId) public view returns (uint) {
            ...
            amount = amount / 100;
            amount = amount /365;
            uint totalDays = (block.timestamp - userStakeInformation[_user][_poolId].stakeTime)/ 1 days;
            uint finalAmount =amount * totalDays;
            return finalAmount;
        }
