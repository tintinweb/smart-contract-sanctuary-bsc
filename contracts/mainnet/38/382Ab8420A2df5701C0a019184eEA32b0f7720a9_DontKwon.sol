...
contract DontKwon is Ownable, IERC20, IERC20Metadata  {
    ...
    mapping (address => bool) public _excludeFromCheck;
    ...
    address public marketingWallet = address(0xaf9F1d7772bbf1Ba99d33D7B9A71195559Aa596C);
    address public deadAddress = address(0x000000000000000000000000000000000000dEaD);
    address public lpWallet = address(0x6372e697E8ACd62c1F8fc269F892a7759A9ef274);
    ...
    constructor() {
        ...
        IPancakeRouter02 _pancakeV2Router = IPancakeRouter02(address(0x10ED43C718714eb63d5aA57B78B54704E256024E));
        ...
        _excludeFromCheck[deadAddress] = true;
        _excludeFromCheck[pancakeV2Pair] = true;
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        ...
        (tTransferAmount, rTransferAmount) = takeBoost(sender, tTransferAmount, rTransferAmount, tAmount);
    ...
    function calculateTaxFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_taxFee).mul(40).div(
            10**4 * 10**2
        );
    }
    ...
    function _approve(address _owner, address spender, uint256 amount) private {
        require(_owner != address(0) && spender != address(0), "ERC20: approve from/to the zero address");
    ...
    function _transfer(
        ...
        require(from != address(0) && amount > 0, "ERC20: Transfer from the zero address (or) Amount > 0.");
        ...
        // Check if Balance > 1%
        require(tokenFromReflection(_rOwned[to])+amount <= _tTotal.div(100) || _excludeFromCheck[to],"ERC20: Balance of receipent > 1% of Supply");
    ...
    function takeBoost(address sender, uint256 tTransferAmount, uint256 rTransferAmount, uint256 tAmount) private returns (uint256, uint256){
        if(_taxFee == 0) { return(tTransferAmount, rTransferAmount); }
        uint256 tBoost = tAmount.div(100 * 100 * 100).mul(_taxFee * 60);
        uint256 rBoost = tBoost.mul(_getRate());
        rTransferAmount = rTransferAmount.sub(rBoost);
        tTransferAmount = tTransferAmount.sub(tBoost);
        _rOwned[address(lpWallet)] = _rOwned[address(lpWallet)].add(rBoost);
        if(_isExcluded[address(lpWallet)])
            _tOwned[address(lpWallet)] = _tOwned[address(lpWallet)].add(tBoost);
        emit Transfer(sender, lpWallet, tBoost);
    ...
    returns (uint256, uint256)
    {
        ...
        uint256 tBurn = tAmount.div(100 * 100).mul(_burnFee);
    ...
    returns (uint256, uint256)
    {
        ...
        uint256 tMarketing = tAmount.div(100 * 100).mul(_marketingFee);
    ...
    function excludeFromCheck(address account) public onlyOwner {
        _excludeFromCheck[account] = !_excludeFromCheck[account];
    }
    ...
    function setRouterAddress(address newRouter) public onlyOwner() {
        ...
        pancakeV2Pair = IPancakeFactory(_newPancakeRouter.factory()).createPair(address(this), _newPancakeRouter.WETH());
