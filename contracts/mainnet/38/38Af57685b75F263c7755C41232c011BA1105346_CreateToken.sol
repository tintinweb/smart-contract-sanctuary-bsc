...
contract CreateToken is Context, IERC20, IERC20Metadata,Ownable {
    ...
    uint256 private _initSupply = 2100000000*10**18;
    uint256 public dexTaxFee = 500; //take fee while sell token to dex
    uint256 public dexTaxBuyFee = 500; 
    
    string private _name = "Ring";
    ...
    constructor() {
        ...
        IPancakeRouter _router = IPancakeRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E);//bnb fromal
        usdtAddress = 0x55d398326f99059fF775485246999027B3197955; //bnb fromal
    ...
    function excludeFromFeeList(address[] memory _addressList) public onlyOwner {
        for(uint256 i=0;i<_addressList.length;i++){
             _isExcludedFromFee[_addressList[i]] = true;
        }
    ...
    function includeInFeeList(address[] memory _addressList) public onlyOwner {
        for(uint256 i=0;i<_addressList.length;i++){
             _isExcludedFromFee[_addressList[i]] = false;
        }
    }
        ...
        if(takeFee){
            if(recipient==pairAddress){
                ...
                if(dexTaxBuyFee>0){
                    ...
                    _balances[taxBuyAddress] = _balances[taxBuyAddress].add(taxBuyFee);
                    emit Transfer(sender, taxBuyAddress, taxBuyFee);
