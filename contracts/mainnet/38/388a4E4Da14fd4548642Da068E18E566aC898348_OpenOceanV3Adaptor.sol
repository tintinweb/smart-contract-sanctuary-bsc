...
contract OpenOceanV3Adaptor is IDexAggregator, Ownable {
    ...
    // TODO: Add setter of router

    receive() external payable {}

    function swap(SwapDescription calldata desc, bytes calldata data) external override payable returns (uint256) {
    // function swap(SwapDescription calldata desc, bytes calldata data) external override payable returns (uint256) {
        ...
        Address.functionCallWithValue(address(aggregator), data, msg.value, "call to open ocean v3 failed");
    ...
    function quit() external onlyOwner {
