...
contract FRAGMENTOFFREE is ERC1155,Ownable{
    ...
    constructor() ERC1155("https://freepeopledao.xyz/FRAGMENTMETEDATA/fragments.json") {
        name = "FRAGMENTS OF FREE";
        symbol = "FRAGMENTS";
    }

    modifier onlyMiner() {
        require(miner[msg.sender]==true||owner()==_msgSender(), "Ownable: caller is not the miner");
        _;
    }
    function setMiner(address _addr) public onlyOwner() {
        miner[_addr]=true;
    }

    function setURL(uint _tokenId,string memory _tokenUrl) public onlyOwner{
        _tokenURIs[_tokenId] = _tokenUrl;
    }

    function mint(address _recipient,uint _amount, uint _tokenId) public onlyMiner returns(uint _mintTokenId){
    ...
    function Burn(uint id,uint _amount) public {
