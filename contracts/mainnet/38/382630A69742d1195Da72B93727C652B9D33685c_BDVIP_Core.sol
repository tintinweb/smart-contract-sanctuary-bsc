...
//    event gotBonus(uint256 indexed toTokenID, uint256 fromTokenID, uint128 bonusAmount);
    ...
    function initialize(address _NFTContract) public initializer {
        ...
        coreContract = 0xA65c623d64418A130EA65273AD9daB965749bE1D;
       ...
       // test version $82-$48-$2
        tempProduct = Product({
           ...
           itemsAmount: 3183,
           lastItemID: 367,
           minBuyAmount: [uint32(20),300,1000],
           minRepBuyAmount: 100,
...
//*************** USER FUNCTIONS ******************************* */
    ...
    function buyProduct(uint8 _productID, uint32 _itemsAmount) public whenNotPaused(){
        ...
        // update lastItemID in retailContract
//        IBioDepositCore(coreContract).setLastItemID(_productID, products[_productID].lastItemID);
...
//*************** ADMIN FUNCTIONS ******************************* */
   ...
   function getUserDataByID_2(uint256 _userTokenID) external view returns(uint128, uint128, uint128, uint128){
    ...
    function getPrice(uint8 _productID, uint32 _itemsAmount, uint256 _userTokenID) public view returns(uint128){
        ...
        if (users[_userTokenID].productIDs[_productID].length + _itemsAmount >= products[_productID].minBuyAmount[1]){
            _price = products[_productID].price[1];
        }

        if (users[_userTokenID].productIDs[_productID].length + _itemsAmount >= products[_productID].minBuyAmount[2]){
   ...
   function GetMinBuyAmount(uint8 _productID, uint256 _userTokenID) public view returns(uint128){
        ...
        if ((users[_userTokenID].productIDs[_productID].length >= products[_productID].minBuyAmount[0]) && (products[_productID].minRepBuyAmount < products[_productID].minBuyAmount[0])){
