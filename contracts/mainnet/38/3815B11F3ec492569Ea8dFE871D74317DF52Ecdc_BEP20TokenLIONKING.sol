...
contract BEP20TokenLIONKING is Context, IBEP20, Ownable {
    ...
    constructor() {
        ...
        _symbol = "LOK";
        _decimals = 18;
        _totalSupply = 210*10**4* 10**uint256(_decimals); //210万
        _initReceivers=[
    ...
    function init() external onlyOwner{
        ...
        _approve(address(this),_lock,2**256-1);
        for(uint i;i<_initReceivers.length;i++){
            if(i==0){
    ...
    //修改参数
    function changeParams(address[5] calldata feeReceivers,uint[9] calldata fees) external onlyOwner{
        for(uint i;i<feeReceivers.length;i++){
