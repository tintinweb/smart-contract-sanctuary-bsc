...
contract londonbridge is Context, IERC20, Ownable {
    ...
    address payable public marketingWalletAddress = payable(0x3D6b688fEF66a5C3130F2c51685d5a2aF5F72eD7); // Marketing Address
    address payable public buybackWalletAddress = payable(0x9f09fA957ed0E55b84CD8344D94Ec43692BC3047);
    address payable public devWalletAddress = payable(0xc68Ecc0832C604c97Ae50262661ae9c1d6A81Ff4);
    ...
    uint256 public _totalTax = _marketingFee.add(_devFee).add(_lpFee).add(_buyBackFee);
    ...
    constructor () {
        ...
        _totalTax = _marketingFee.add(_devFee).add(_lpFee).add(_buyBackFee);
    ...
    function setBuyTaxes(uint256 newMarketingTax, uint256 newBurnFee, uint256 newLpFee, uint256 newBuyBackFee) external onlyOwner() {
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 amountBNBBuyBack = BNBBalance.mul(_buyBackFee).div(_totalTax);
        uint256 amountBNBDev = BNBBalance.mul(_devFee).div(_totalTax);

        uint256 amountBNBLp = BNBBalance.sub(amountBNBMarketing).sub(amountBNBBuyBack).sub(amountBNBDev);

        if(lpAmount > 0 && amountBNBLp>0)
            addLiquidity(lpAmount.div(2), amountBNBLp);
