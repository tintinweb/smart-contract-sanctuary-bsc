...
contract PowerBUSD is Context, Ownable, IERC20 {
    using SafeMath for uint256;
	using SafeERC20 for IERC20;
    
    uint8 private _decimals = 18;
    string private _symbol = "PBT";
    string private _name = "Power BUSD Token ";
    uint256 private _totalSupply =  1000000000 * 10**18;
          
    uint256 constant project = 50; // 5% deposit fee to wallet 3
    uint256 constant development = 50; // 5% deposit fee to wallet 2
    uint256 constant hardDays = 1 days;
    uint256 constant refPercentage = 50; // 5% referral fee /reward
    uint256 constant minInvest = 50 ether; // 50 BUSD min deposit
    uint256 constant percentDivider = 1000;
    uint256 constant maxRewards = 1000 ether; // 1000 BUSD max reward
    uint256 constant dailyPrc = 30; // 3% daily ROI
    ...
    constructor(address addr1, address addr2, address addr3) {
        ...
        //_balances[proj] = 250000000 * 10**18; <-- removed this
        //_balances[part] = 250000000 * 10**18; <-- removed this
        emit Transfer(address(0), address(this), _balances[address(this)]);  
        //emit Transfer(address(0), proj, _balances[proj]);	 <-- removed this
        //emit Transfer(address(0), part, _balances[part]); <-- removed this
    }
    ...
    function launch() public onlyOwner {
        ...
        //invest(addr, amount); <-- removed this
    }
    ...
    function invest(address ref, uint256 amount) public isInitialized {
        ...
        BUSD.safeTransfer(proj, devStakeFee); // 5% to wallet 2
        BUSD.safeTransfer(part, projStakeFee); // 5% to wallet 3
    ...
    function withdrawInitial(uint256 key) public isInitialized {
        ...
        //BUSD.safeTransfer(dev, devStakeFee); <-- removed this, stays in contract
        //BUSD.safeTransfer(proj, projStakeFee); <-- removed this, stays in contract
        BUSD.transfer(msg.sender, refundAmount.sub(devStakeFee + projStakeFee));
        user.activeStakesCount--;
        user.totalInvested -= amount;
    ...
    function sellTokens(uint256 amount) external {       
        ...
        BUSD.safeTransfer(msg.sender, cvtAmt.sub(devFee + projFee));
        BUSD.safeTransfer(dev, devFee); // 5% to wallet 1
        BUSD.safeTransfer(proj, projFee); // 5% to wallet 2
    }
