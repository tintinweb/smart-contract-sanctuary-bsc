...
contract VSLWalletTimelock {
    ...
    mapping(address => uint256) public timeWithdrawForUsers;
    ...
    event AllocationPerformed(address recipient, uint256 amount, uint256 expireTime);
    ...
    function finalizeAllIncomingDeposits() public onlyOwner incomingDepositsStillAllowed {
    ...
    function depositTokens(address recipient, uint256 amount, uint256 expireTime) public onlyOwner incomingDepositsStillAllowed {
        require(recipient != address(0), "ERC20: transfer to the zero address");
        require(expireTime > 0, "Expire Time must be large than 0");
    
        uint256 timePeriod = block.timestamp.add(expireTime);    
        if ( timeWithdrawForUsers[recipient] != uint256(0)) {
            timePeriod = timeWithdrawForUsers[recipient];
        }
        balances[recipient] = balances[recipient].add(amount);
        timeWithdrawForUsers[recipient] = timePeriod;
        emit AllocationPerformed(recipient, amount, timePeriod);
    }
    ...
    function transferTimeLockedTokensAfterTimePeriod(address to, uint256 amount) public noReentrant {
        ...
        if (block.timestamp >= timeWithdrawForUsers[msg.sender]) {
            alreadyWithdrawn[to] = alreadyWithdrawn[to].add(amount);
            delete timeWithdrawForUsers[to];
