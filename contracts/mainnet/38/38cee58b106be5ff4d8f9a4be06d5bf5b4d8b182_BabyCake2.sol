...
contract BabyCake2 is Context, IERC20, Ownable {
    ...
    constructor (address _marketing, address _treasury) {
        ...
        _walletMax =30000000 * 10**_decimals;
        ...
        BurnAddress = payable(deadAddress);
        _buyLiquidityFee = 3;
        _buyBurnFee = 1;
        _buyMarketingFee = 7;
        _buyTreasuryFee = 1;
        ...
        _totalTaxIfBuying = _buyLiquidityFee.add(_buyMarketingFee).add(_buyTreasuryFee).add(_buyBurnFee);
        _totalTaxIfSelling = _sellLiquidityFee.add(_sellMarketingFee).add(_sellTreasuryFee).add(_sellBurnFee);
        _totalDistributionShares = _liquidityShare.add(_marketingShare).add(_treasuryShare).add(_BurnShare);
    ...
    function setBuyTaxes(uint256 newLiquidityTax, uint256 newBurnTax, uint256 newMarketingTax, uint256 newTeamTax) external onlyOwner() {
    ...
    function setSellTaxes(uint256 newLiquidityTax, uint256 newBurnTax, uint256 newMarketingTax, uint256 newTeamTax) external onlyOwner() {
    ...
    function setDistributionSettings(uint256 newLiquidityShare, uint256 newMarketingShare, uint256 newBurnshare, uint256 newTeamShare) external onlyOwner() {
