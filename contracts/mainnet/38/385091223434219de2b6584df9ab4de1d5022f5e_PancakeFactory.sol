...
contract PancakeERC20 is NewPancakeERC20 {
...
interface NewPancakeCallee {
    function pancakeCall(address sender, uint amount0, uint amount1, bytes calldata data) external;
}

contract PancakePair is NewPancakePair, PancakeERC20 {
    using SafeMath  for uint;
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        address feeTo = NewPancakeFactory(factory).feeTo();
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        if (data.length > 0) NewPancakeCallee(to).pancakeCall(msg.sender, amount0Out, amount1Out, data);
        balance0 = IERC20(_token0).balanceOf(address(this));
        balance1 = IERC20(_token1).balanceOf(address(this));
        }
...
contract PancakeFactory is NewPancakeFactory {
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        ...
        NewPancakePair(pair).initialize(token0, token1);
