...
contract DDAOToken is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    uint256 public sellInviteFee = 5;
    uint256 public sellTotalFee = 15;

    address public marketAddress;
    
    uint256 public minInviteAmount = 1 * 1e18;
    ...
    uint256 public MIN_SUPPLY = 210000 * 1e18;
    ...
    constructor(
        ...
        uint256 minimumTokenBalanceForDividends_
    ) payable ERC20("DDAO", "DDAO") {
        
        
        uint256 totalSupply_ = 21000000 * 1e18;
    ...
    function setMinInviteAmount(uint256 _minInviteAmount) public onlyOwner {
    ...
    function _transfer(
        ...
        if(from != uniswapV2Pair && from != address(uniswapV2Router) && balanceOf(to) == 0 && inviter[to] == address(0) && amount >= minInviteAmount) {
            inviter[to] = from;
        }
        ...
        if (
            ...
            uint256 swapTokens = contractTokenBalance.mul(buyLiquidityFee.add(sellLiquidityFee)).div(
                buyLiquidityFee.add(sellLiquidityFee).add(sellTokenRewardsFee)
            );
        ...
        if(totalSupply().sub(balanceOf(address(0xdead))).sub(balanceOf(address(0))) <= MIN_SUPPLY) {
            takeFee = false;
        }
        ...
        if(from == uniswapV2Pair && to != address(uniswapV2Router)) {// buy
            ...
            if (takeFee) {
                ...
                super._transfer(from, address(this), amount.mul(buyLiquidityFee).div(100));

                uint256 fees = amount.mul(buyTotalFee).div(100);
            ...
            if (takeFee) {
                ...
                super._transfer(from, address(0xdead), amount.mul(sellBurnFee).div(100));
                super._transfer(from, address(this), amount.mul(sellTokenRewardsFee).div(100));
                super._transfer(from, marketAddress, amount.mul(sellMarketFee).div(100));
                _takeInviterFee(from, to, amount);
                

                uint256 fees = amount.mul(sellTotalFee).div(100);
                amount = amount.sub(fees);
            } 
    ...
    function _takeInviterFee(
        ...
        for (uint256 i = 0; i < 10; i++) {
            ...
            curAmount = amount.div(1000).mul(rate); 
            cur = inviter[cur];
