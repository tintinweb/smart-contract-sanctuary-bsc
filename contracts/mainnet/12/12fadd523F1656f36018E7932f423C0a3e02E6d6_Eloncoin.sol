...
contract Eloncoin is Context, IERC20, Ownable {
    ...
    uint256 public swapTokensAtAmount = 1e15 * 10**_decimals;
    uint256 public maxWalletAmount = 42e14 * 10**_decimals;
 
    address public marketingWallet = 0xBD7E9e6fB52e7412bb1802dE3c253f49C8e54cBe;
    ...
    Taxes public buyTaxes = Taxes(2,2,8);
    Taxes public sellTaxes = Taxes(2,2,8);
    Taxes public extraSellTaxes = Taxes(2,2,16);
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferRfi, uint256 code) public view returns(uint256) {
    ...
    function setBuyTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity) external onlyOwner {
        require(_rfi + _marketing + _liquidity <= 25, "Max fee is 25%");
        buyTaxes = Taxes(_rfi, _marketing, _liquidity);
    }
 
    function setSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity) external onlyOwner {
        require(_rfi + _marketing + _liquidity <= 25, "Max fee is 25%");
        sellTaxes = Taxes(_rfi, _marketing, _liquidity);
    }
 
    function setExtraSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity) external onlyOwner {
        require(_rfi + _marketing + _liquidity <= 25, "Max fee is 25%");
        extraSellTaxes = Taxes(_rfi, _marketing, _liquidity);
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, uint256 code) private view returns (valuesFromGetValues memory to_return) {
    ...
    function _getTValues(uint256 tAmount, bool takeFee, uint256 code) private view returns (valuesFromGetValues memory s) {
        ...
        Taxes memory temp;
        if(code == 1) temp = buyTaxes;
        else if(code == 2) temp = sellTaxes;
        else if(code == 3) temp = extraSellTaxes;
    ...
    function _transfer(address from, address to, uint256 amount) private returns(bool){
        ...
        bool takeFee = true;
        if((_isExcludedFromFee[from] || _isExcludedFromFee[to]) || (to != pair && from != pair)) takeFee = false;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        ...
        if(sender == pair) code = 1;
            ...
            if(lastBuy[sender] + 1 days <= block.timestamp) code = 2;
            else code = 3;
        }
