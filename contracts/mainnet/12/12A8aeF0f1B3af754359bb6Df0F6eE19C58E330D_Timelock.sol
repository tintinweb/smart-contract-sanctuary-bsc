...
contract Timelock is ITimelock, Ownable {
    ...
    //For RewardRouter:
    function setTokenConfig(address _target, address _token, uint256 _token_decimal, address _elp_n,bool _isStable) external onlyKeeperAndAbove {
        ITimelockTarget(_target).setTokenConfig(_token, _token_decimal, _elp_n, _isStable);
    }
    function setELPn(address _target, address _elp_n, uint256 _elp_n_weight, address _stakedELPnVault, uint256 _elp_n_decimal, address _stakedElpTracker) external onlyKeeperAndAbove{
        ITimelockTarget(_target).setELPn(_elp_n, _elp_n_weight, _stakedELPnVault, _elp_n_decimal, _stakedElpTracker);
    }

    //For PriceFeed:
    function setPositionRouter(address _target, address[] memory _positionRouters) external onlyKeeperAndAbove {
        ITimelockTarget(_target).setPositionRouter(_positionRouters);
    }
...
interface ITimelockTarget {
    ...
    //RewardRouter:
    function setTokenConfig(address _token, uint256 _token_decimal, address _elp_n,bool _isStable) external;
    function setELPn(address _elp_n, uint256 _elp_n_weight, address _stakedELPnVault, uint256 _elp_n_decimal, address _stakedElpTracker) external;
