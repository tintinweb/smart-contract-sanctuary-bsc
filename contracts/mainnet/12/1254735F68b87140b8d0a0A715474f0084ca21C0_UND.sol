...
contract UND is ERC20, Ownable {
   ...
   function setDeadFee(uint256 val) public onlyOwner {
    ...
    function _transfer(
        ...
        if(_isExcludedFromFees[from] || _isExcludedFromFees[to] || (to != uniswapV2Pair && from != uniswapV2Pair && from != address(uniswapV2Router) && to != address(uniswapV2Router))) {
            takeFee = false;
        }
