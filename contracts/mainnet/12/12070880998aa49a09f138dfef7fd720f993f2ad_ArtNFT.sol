...
// File: @openzeppelin/[email protected]/interfaces/IERC165Upgradeable.sol
...
// File: @openzeppelin/[email protected]/token/common/ERC2981Upgradeable.sol
...
contract ArtNFT is Initializable, PausableUpgradeable, AccessControlUpgradeable, ERC2981Upgradeable, ERC721URIStorageUpgradeable {
  ...
  /**
    * @dev Emitted when `baseURI` changes.
    */
  event NewBaseURI(string uri);

  /**
    * @dev Emitted when `mintingPrice` changes.
    */
  event NewMintingPrice(uint256 price);

  /**
    * @dev Emitted when fees are withdraw from contract.
    */
  event Withdraw(
   ...
   *
   * NOTE: used as a temporal gateway in order to get JSON data on Android devices correctly.
   */
  ...
  /**
    * @dev Changes the minting price.
    *
    * Requirements:
    *
    * - the caller must have the 'minter' role.
    */
  function setMintingPrice(uint256 price) public onlyRole(MINTER_ROLE) {
    mintingPrice = price;
    emit NewMintingPrice(price);
  }
  ...
  /**
    * @dev Mints a certain `quantity` of some ipfs `uri` NFT. Where `to` is the address owner of the NFTs
    * and `fee numerator` is used to calc which fraction of the sold price will go to the artist (`to` address).
    * 100 points of `fee numerator` are equal to 1% of sold price as fee.
    *
    * Requirements:
    *
    * - the contract must not be paused.
    * - the caller must send minting price value.
    * - `fee numerator` should be less than or equals 1000.
    */
  function mint(address to, string calldata uri, uint96 feeNumerator, uint96 quantity) public payable whenNotPaused {
    require(msg.value >= mintingPrice, "Price does not match");
    require(feeNumerator <= 1000, "Artist fee should be up to 10% of sold price");
  ...
  /**
    * @dev Burns `tokenId`. See {ERC721URIStorage-_burn}.
    *
    * Requirements:
    *
