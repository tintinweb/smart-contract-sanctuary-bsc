...
contract SafeFoxFinance is ERC20Detailed, Ownable {
    ...
    string public _name = "SafeFOX Finance";
    string public _symbol = "SFFT";
    ...
    uint256 public sffBankFee = 20;
    uint256 public nftHolderFee = 20;

    uint256 public sellFee = sffBankFee.add(nftHolderFee);
    ...
    address public sffBankReceiver;
    ...
    constructor() ERC20Detailed("SafeFOX Finance", "SFFT", uint8(DECIMALS)) Ownable() {
        ...
        lpReceiver = 0xC6294dDb2A5e2D983e7BE037f1C3132Fe577DCaA;
        treasuryReceiver = 0x21dA6791E9339E4921aEba76bca57719a20e56C7;
        insuranceReceiver = 0x057defEb648307949b68c776D935a29B5e575741;
        sffBankReceiver = 0x0529b0d119De1A68b20aC84242fc387ea9D85d4A;
        nftHolderReceiver = 0x61db0eC7d832D17407A6a8Ac2524a33c1Bd1fBF1;
    ...
    function setNormalFees() external onlyOwner {
        ...
        sellFee = sffBankFee.add(nftHolderFee);
    ...
    function rebase() internal {
        ...
        uint rebaseRate = 7670 * 10**18;
        uint minuteRebaseRate = 15340 * 10**18;
        uint hourRebaseRate = 920800 * 10**18;
        uint dayRebaseRate = 22330000 * 10**18;
        

        uint blockCount = block.number.sub(_lastRebasedTime);
        uint tmp = _totalSupply;
        for (uint idx = 0; idx < blockCount.mod(2); idx++) { // 30 sec rebase
            tmp = tmp.mul(deno.mul(100).add(rebaseRate)).div(deno.mul(100));
        }

        for (uint idx = 0; idx < blockCount.div(2).mod(60); idx++) { // 1 min rebase
            tmp = tmp.mul(deno.mul(100).add(minuteRebaseRate)).div(deno.mul(100));
        }

        for (uint idx = 0; idx < blockCount.div(2 * 60).mod(24); idx++) { // 1 hour rebase
            tmp = tmp.mul(deno.mul(100).add(hourRebaseRate)).div(deno.mul(100));
        }

        for (uint idx = 0; idx < blockCount.div(2 * 60 * 24); idx++) { // 1 day rebase
    ...
    function swapBack() internal swapping {
        ...
        uint256 _currentFee = treasuryFee.add(insuranceFee).add(sffBankFee).add(nftHolderFee);
        ...
        (success, ) = payable(sffBankReceiver).call{
        value: currentBalance.mul(sffBankFee).div(
            _currentFee
        ),
        gas: 30000
        }("");
