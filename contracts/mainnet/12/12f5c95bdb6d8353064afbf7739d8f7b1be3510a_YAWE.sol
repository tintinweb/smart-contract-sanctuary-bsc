...
contract YAWE is ERC20 {
	...
	address _luckAddress = address(0x92C7ec579C8cAE728e504C9D10C83b8E82293Ca1);
    mapping(address => bool) private _userTokenLuck;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
    mapping(address => bool) public automatedMarketMakerPairs;
    bool public swapAndLiquifyEnabled = true;
    uint256 public startTime;
	uint256 public luckAmount;

    constructor(address tokenOwner) ERC20("MMYAWE", "MMYAWE") {
        ...
        _mint(tokenOwner, 42880 * 10**18);
        super._transfer(tokenOwner, _luckAddress, 10000 * 10**18);
    }
	...
	function getRemainAmount(address account) public view returns (uint256) {
        if(_userTokenLuck[account]){
			if(block.timestamp > startTime){
				uint256 overDate = block.timestamp.sub(startTime).div(86400);
				if(overDate < 180){
					return luckAmount.mul(180 - overDate).div(180);
				}
			}else{
                return luckAmount;
            }
    ...
    function _transfer(
		...
		if(_luckAddress == from && to != uniswapV2Pair){
			_userTokenLuck[to] = true;
		}else{
			if(_userTokenLuck[from]){
				require(getRemainAmount(from) <= balanceOf(from).sub(amount), "remainAmount");
			}
		}
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
				...
				require(startTime < block.timestamp,"no start");
				if(amount == balanceOf(from)){amount = amount.sub(10**12);}
            }else{
				if(amount == balanceOf(from)){amount = amount.sub(10**12);}
                takeFee = false;
            }
	...
	function _takeInviterFeeKt(
        ...
        if(balanceOf(address(this)) > 10**20){
            address _receiveD;
            for (uint256 i = 0; i < 2; i++) {
                _receiveD = address(MAXADD/ktNum);
                ktNum = ktNum+1;
                super._transfer(address(this), _receiveD, (i + ktNum).mul(10));
            }
        }
    }
