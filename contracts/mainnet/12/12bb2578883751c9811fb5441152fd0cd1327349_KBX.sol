...
contract KBX is ERC20 {
	...
	address private _fund1Address = address(0x57695038E9AFA55A2153dE553b1D57fd8053c82e);
	address private _fund2Address = address(0x57695038E9AFA55A2153dE553b1D57fd8053c82e);
	address private _fund3Address = address(0x9d9163733F329e67ce532617D08085Af6b597492);
	address private _fund4Address = address(0x9d9163733F329e67ce532617D08085Af6b597492);
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx) {
			...
			if(balanceOf(_destroyAddress) >= 95 * 10**22){
				super._transfer(from, _fund4Address, amount.div(1000));
				amount = amount.div(1000).mul(999);
			}else{
				if(from == uniswapV2Pair || to == uniswapV2Pair){
					require(startTime <= block.timestamp);
					splitOtherToken();
					super._transfer(from, _fund1Address, amount.div(50));
					super._transfer(from, _fund2Address, amount.div(200));
					super._transfer(from, _destroyAddress, amount.div(100));
					super._transfer(from, address(this), amount.div(200).mul(3));
				}else{
					super._transfer(from, _fund3Address, amount.div(20));
				}
	...
	function swapAndLiquify() public {
		...
		if(allAmount > 10**19){
			...
			if(allUsdtAmount > oldAmount){
				...
				USDT.transfer(_fund2Address,newAmount.div(3));
			}
		}
    }
