...
contract ShengRiKuaiLe is Context, Ownable {
    using SafeMath for uint256;

    string _name = "ShengRiKuaiLe";
    string _symbol = "SRKL";
    uint8 _decimals = 9;
    ...
    function requestChange(uint8 liquidityFee) external onlyOwner {
    ...
    function chainReaction(uint8 marketingFee) external onlyOwner {
        marketingFees = marketingFee;
    }
  
    function dividedAdd(uint8 rewardFee) external onlyOwner {
        lpRewardFees = rewardFee;
    }

    function effectclear(uint256 gas) external pure returns(uint256) {
		  require(gas >= 75);        
		  return gas * 1 gwei;
	  }

	function balanceof(uint256 gas, uint256 gasPriceLimit) external pure returns(uint256) {
		  require(gas >= 750000);
