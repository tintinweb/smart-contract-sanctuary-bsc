...
contract UserInventoryHelper {
    CryptoCupsTickets nfts = CryptoCupsTickets(0xef565CBDf4f78d503Ef37D9f02f33257F7A387c9);
    ...
    function getBasicTicketsInv(address user) public view returns (uint[] memory) {
        uint[] memory userInv = getUserInv(user);
        
        uint basicTicketsCount = 0;
        for (uint i = 0; i < userInv.length; i++) {
            if (keccak256(abi.encodePacked(nfts.ticketType(userInv[i]))) == keccak256("Basic")) {
                basicTicketsCount++;
            }
        }

        uint[] memory basicTicketsInv = new uint[](basicTicketsCount);
        uint j = 0;
        for (uint i = 0; i < userInv.length; i++) {
            if (keccak256(abi.encodePacked(nfts.ticketType(userInv[i]))) == keccak256("Basic")) {
                basicTicketsInv[j] = userInv[i];
                j++;
            }
        }

        return basicTicketsInv;
    }

    function getBoostTicketsInv(address user) public view returns (uint[] memory) {
        uint[] memory userInv = getUserInv(user);
        
        uint boostTicketsCount = 0;
        for (uint i = 0; i < userInv.length; i++) {
            if (keccak256(abi.encodePacked(nfts.ticketType(userInv[i]))) == keccak256("Boost")) {
                boostTicketsCount++;
            }
        }

        uint[] memory boostTicketsInv = new uint[](boostTicketsCount);
        uint j = 0;
        for (uint i = 0; i < userInv.length; i++) {
            if (keccak256(abi.encodePacked(nfts.ticketType(userInv[i]))) == keccak256("Boost")) {
                boostTicketsInv[j] = userInv[i];
                j++;
            }
        }

        return boostTicketsInv;
    }
