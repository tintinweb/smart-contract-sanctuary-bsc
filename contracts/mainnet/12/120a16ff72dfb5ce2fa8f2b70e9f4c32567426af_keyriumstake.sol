...
contract keyriumstake is Ownable {
    ...
    // Fees address...
    address public devAddress = 0x1B5b311BA04d0a419C2550173BFF299E4b514C4a;
    
    // Keyrium token contract...
    address public Keyrium = 0x11A404218439A4F5b4606E198A3EfD1Bcf9d4749;
    ...
    function updateAccount(address account) private {
        ...
        uint256 conbalance = Token(Keyrium).balanceOf(address(this));
        ...
        if (pendingDivs != 0) {
            ...
            Token(Keyrium).transfer(account, pendingDivs);
    ...
    function deposit(uint256 amountToStake, uint256 _timeperiod) public {
       ...
       require(_timeperiod == 7 || _timeperiod == 15, "Invalid Time Period. it must be either 7 or 15"); 
        ...
        Token(Keyrium).transferFrom(msg.sender, address(this), amountToStake);
    ...
    function withdraw(uint256 amountToWithdraw) public {
        ...
        if (_lastClaimedTime >= MinimumWithdrawTime) {
            require(Token(Keyrium).transfer(msg.sender, amountToWithdraw), "Could not transfer tokens.");
        }
        
        if (_lastClaimedTime < MinimumWithdrawTime) {
            uint256 WithdrawFee = amountToWithdraw.div(1e3).mul(750);
            uint256 amountAfterFee = amountToWithdraw.sub(WithdrawFee);
            require(Token(Keyrium).transfer(msg.sender, amountAfterFee), "Could not transfer tokens.");
            require(Token(Keyrium).transfer(devAddress, WithdrawFee), "Could not transfer tokens.");
        }}
    ...
    function transferAnyERC20Tokens(address _tokenAddress, address _to, uint256 _amount) public onlyOwner {
        require(_tokenAddress != Keyrium, "You can't transfer Keyrium token.");
