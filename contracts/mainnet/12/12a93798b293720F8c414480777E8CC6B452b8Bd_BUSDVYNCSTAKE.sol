    ...
    OwnableUpgradeable
{
    ...
    function initialize() public initializer {
        ...
        dataAddress = 0x2486C1560C34598Bd22783C73a10265837B219ca;
        data = GetDataInterface(dataAddress);
        TreasuryAddress = 0xe6D3c59e945202EAA617fA609b2275c11353f5D0;
        treasury = TreasuryInterface(TreasuryAddress);
        migrateAddress = 0x03A6985cb3fAab9ac16882e6E9d0E8C9Fb2503Fb; // old busd pool address
        ...
        lpToken = 0x67Cd8873a1fD5Cbb3772d89b00a9DD2665C0bF7e;
        ...
        returns (uint256 _compoundedReward)
    {
        ...
        if (userInfo[user].lastStakeUnstakeTimestamp < aprChangeTimestamp) {
            if (isAprIncrease == false) {
                _compoundedReward =
                    _compoundedReward -
                    ((userInfo[user].autoClaimWithStakeUnstake *
    ...
    function migrateStaking() public {
        ...
        userInfo[staker].lpAmount = userInfo[staker].lpAmount + user.lpAmount;
        ...
        onlyOwner
    {
        for (uint256 i = 0; i < _stakers.length; i++) {
            ...
            require(stopMigrate[staker] == false, "already migrated");

            uint256 _compoundReward = compoundedReward(staker);
            VyncMigrate.userInfoData memory user = migrate.userInfo(staker);

            userInfo[staker].lpAmount =
                userInfo[staker].lpAmount +
                user.lpAmount;
            userInfo[staker].stakeBalanceWithReward =
                userInfo[staker].stakeBalanceWithReward +
     ...
     function manualMigrateByOwner(
       ...
       userInfo[staker].lpAmount =
                userInfo[staker].lpAmount + lpamount;
        userInfo[staker].stakeBalanceWithReward =
            userInfo[staker].stakeBalanceWithReward +
            stakeBalance +
            compoundAmount +
