...
import "./interfaces/IReflexStaking.sol";
import "./DividendDistributor.sol";

contract ReflexV2 is Initializable, UUPSUpgradeable, IERC20MetadataUpgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    uint256 public antiWhaleSellLimit;
    ...
    event TokenFromContractTransferred(address contractAddress, address toAddress, uint amount);
    event BnbFromContractTransferred(uint amount);

    function initialize(address router_, address _marketingFeeReceiver, address _stakingFeeReceiver) public initializer {
    ...
    function initializeParams() internal {
      ...
      swapEnabled = true;
      swapThreshold = 200000000000 * 10**_decimals;
      autoBuybackEnabled = true;
      autoBuybackThreshold = 1 * 10**18;
      autoBuybackBlockPeriod = 200;
      autoBuybackBlockLast = block.number;
      distributorGas = 500000;
      antiWhaleEnabled = true;
      antiWhaleSellLimit = 100000000000 * 10**_decimals;
    }
    ...
    function getPaidAndUnpaidDividend(address shareholder) external view returns (uint256, uint256) {
      return (distributor.getPaidEarnings(shareholder), distributor.getUnpaidEarnings(shareholder));
    }
    ...
    function transferFrom(address from, address to, uint256 amount) external virtual override whenNotPaused returns (bool) {
    ...
    function triggerAutoBuyback() external onlyOwner {
      swapEthForTokens(autoBuybackThreshold, DEAD);
      autoBuybackBlockLast = block.number;
    }
    ...
    function setFees(uint256 _liquidityFee, uint256 _buybackFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _stakingFee, uint256 _feeDenominator) public onlyOwner {
    ...
    function setStakeAddress(address _staking) external onlyOwner {
    ...
    function setStakingFeeReceiver(address _stakingFeeReceiver) external onlyOwner {
      require(_stakingFeeReceiver != address(0), "Zero address");
      stakingFeeReceiver = _stakingFeeReceiver;
    }

    function setStakingStatus(bool enable) external onlyOwner {
      stakingEnabled = enable;
    }

    function setAutoBuyBackStatus(bool enable) external onlyOwner {
    ...
    function setAntiWhaleStatus(bool enable) external onlyOwner {
      antiWhaleEnabled = enable;
    }

    function setSwapThreshold(uint256 thresholdInWei) external onlyOwner {
      require(thresholdInWei > 0, "Must be greater than 0");
      swapThreshold = thresholdInWei;
    }

    function setBuyBackThreshold(uint256 thresholdInWei) external onlyOwner {
      require(thresholdInWei > 0, "Must be greater than 0");
      autoBuybackThreshold = thresholdInWei;
    }

    function setBuyBackBlockPeriod(uint256 blocks) external onlyOwner {
      require(blocks > 0, "Must be greater than 0");
    ...
    function includeInFee(address account) external onlyOwner {
      ...
      require(isFeeExempt[account] != false, "Already in fee");
      isFeeExempt[account] = false;
    }

    function excludeFromFee(address account) external onlyOwner {
      require(account != address(0), "Zero address");
      require(isFeeExempt[account] != true, "Already excluded");
    ...
    function includeAllInBlacklist(address[] memory accounts) external onlyOwner {
      for (uint256 index = 0; index < accounts.length; index++) {
        if(!isBlacklisted[accounts[index]]) {
          isBlacklisted[accounts[index]] = true;
        }
      }
    }

    function excludeAllFromBlacklist(address[] memory accounts) external onlyOwner {
      for (uint256 index = 0; index < accounts.length; index++) {
        if(isBlacklisted[accounts[index]]) {
          isBlacklisted[accounts[index]] = false;
        }
      }
    }

    function setIsDividendExempt(address holder, bool exempt) external onlyOwner {
      require(holder != address(this) && holder != pair, "Cant exempt");
    ...
    function setAntiWhaleSellLimit(uint256 amount) external onlyOwner {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
      ...
      if (stakingEnabled) {
        require(_balances[sender] - amount >= stake.stakedTokens(sender), "Cannot send staked token");
      }

      if (inSwap) {
        _basicTransfer(sender, recipient, amount);
      } else {
        if (antiWhaleEnabled && recipient == pair && sender != owner()) {
          require(amount <= antiWhaleSellLimit, "Limit exceeds");
        }

        if (shouldSwapBack()) swapBack();
        if (shouldAutoBuyback()) autoBuyback();
    ...
    function swapBack() internal swapping {
      ...
      emit Transfer(address(this), stakingFeeReceiver, stakingTokenAmount);
      ...
      if (liquidityTokenAmount > 0) {
        ...
        emit AutoLiquify(liquidityBnbAmount, liquidityTokenAmount);
      }
    }
...
contract DividendDistributor is IDividendDistributor {
    ...
    IERC20MetadataUpgradeable public rewardToken =
        IERC20MetadataUpgradeable(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
    IDexRouter public router;
