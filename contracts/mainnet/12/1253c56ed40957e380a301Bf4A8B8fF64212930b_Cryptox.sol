...
contract Cryptox is Context, IBEP20, Ownable {
    ...
    function getTimeToSellOrBuy(address wallet) public view returns(uint256 timeBuy, uint256 timeSell) {

        uint256 timeToBuy = (getFromLastPurchaseBuy(wallet) + buyPerSecond) - block.timestamp;
        uint256 timeToSell = (getFromLastSell(wallet) + sellPerSecond) - block.timestamp;
