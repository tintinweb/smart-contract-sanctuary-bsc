...
contract HFK is ERC20Rebase, Ownable {
    using SafeMath for uint; 
    MktCap public mkt;
    ...
    function _afterTokenTransfer(address from,address to,uint amount) internal override trading{
        ...
        if(ispair[from] && isLimit){
            require(amount<=totalSupply().mul(3).div(100),"is open limit"); 
            require(balanceOf(to)<=totalSupply().mul(3).div(100),"is open limit");  
        }
    ...
    function balanceOf(address account) public view virtual override returns (uint256) {
        ...
        if(account==address(0) || account==ceo)return balance;
