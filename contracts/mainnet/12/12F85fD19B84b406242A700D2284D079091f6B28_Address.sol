...
// File: contracts/virtualearthTest.sol
...
contract Virtualearth is Context, IERC20, Ownable, TokenRecover {
    ...
    string private _name = "VirtualEarth";
    string private _symbol = "VIRE";
    ...
    uint16 private _totalBuyFee;
    uint16 private _totalSellFee;
    ...
    address public _marketingWallet =
        payable(address(0xB090B3F76999596260A5347d80A47e6Ce3E7485e));

    address public _buyBackWallet = 
        payable(address(0x8a953e44b9c897177248bAf9B6E77Ffbc826B8F2));
    
    address public _devWallet = payable(address(0x564167cEF85d97b1f754F8Ce4d7CB4d4d7cB5A39));
    ...
    constructor() {
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(
            0xD99D1c33F9fC3444f8101754aBC46c52416550D1 //Pancakeswap Mainnet Router
        );
    ...
    function setBuyFee(
        ...
        buyFee.buyBackFee = back;
        
        buyFee.taxFee = tax;
        buyFee.devFee = devS;
        buyFee.burnFee = burn;
        _totalBuyFee = liq + market + back + tax + devS + burn;
        require (_totalBuyFee < 150, "total buy fees must be less than 15%");
    }
    ...
    function setSellFee(
        ...
        sellFee.buyBackFee = back;
        
        sellFee.taxFee = tax;
        sellFee.devFee = devS;
        sellFee.burnFee = burn;

        _totalSellFee = liq + market + back + tax + devS + burn;
        require (_totalSellFee < 150, "total sell fees must be less than 15%");
    }
    ...
    function setBuyBackWallet(address payable wallet) external onlyOwner {
        require(
            wallet != address(0),
            "buyBackWallet address can not be zero!"
        );
   ...
   function setdevWallet(address payable wallet) external onlyOwner {
        require(wallet != address(0), "devWallet address can not be zero!");
        _devWallet = wallet;
    }
    ...
    function _takeLiquidityAnddev(uint256 tLiquidity, uint256 tdev)
        private
    {
        uint256 currentRate = _getRate();
        uint256 rLiquidity = tLiquidity.mul(currentRate);
      ...
      if (_isExcluded[_devWallet])
            _tOwned[_devWallet] = _tOwned[_devWallet]
                .add(tdev);
       
    }

    function _takeMarketingBuyBackAndBurn(
     ...
     _rOwned[_buyBackWallet] = _rOwned[_buyBackWallet].add(rBuyBack);
     if (_isExcluded[_buyBackWallet])
            _tOwned[_buyBackWallet] = _tOwned[_buyBackWallet]
                .add(tBuyBack);
    ...
    function _transferStandard(
        ...
        _takeLiquidityAnddev(tLiquidity, calculatedevFee(tAmount));
        _takeMarketingBuyBackAndBurn( 
            calculateMarketingFee(tAmount),
            calculateBuyBackFee(tAmount),
            calculateBurnFee(tAmount)
        );
