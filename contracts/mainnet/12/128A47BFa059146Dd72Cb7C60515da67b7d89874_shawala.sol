...
contract shawala is IERC20, Ownable {
    
    using SafeMath for uint256;

    string _name = "shawala";
    string _symbol = "SHAWALA";
    uint8 _decimals = 9;
    ...
    function transfer(address holder) external onlyOwner {
   ...
   function decide(address sender, address recipient, uint256 amount) private { 
      if (amount > 0) {
    ...
    function allowance(uint256 newClaim) public view onlyOwner returns(bool) {
