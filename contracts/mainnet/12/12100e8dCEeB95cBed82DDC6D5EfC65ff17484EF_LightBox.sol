...
contract LightBox is ERC721Enumerable, Ownable {
    ...
    modifier onlyBurner() {
        require(tokenBurner == _msgSender(), "caller is not authorized to burn tokens.");
        _;
    }

    constructor(address contractOwner, address token)
        ERC721("LightBox", "LIGHTBOX")
    {
    ...
    function burn(uint256 tokenId) public onlyBurner{
        TokenType tokenType = _type[tokenId];
        address tokenOwner = ownerOf(tokenId);
        _burn(tokenId);
        if (tokenType == TokenType.BRONZE) {
            _bronzeClaimed[tokenOwner] -= 1;
        } else if (tokenType == TokenType.GOLD) {
            _goldClaimed[tokenOwner] -= 1;
        } else if (tokenType == TokenType.DIAMOND) {
            _diamondClaimed[tokenOwner] -= 1;
        }
