...
♻️ Website: https://voodoobsc.com
📱Telegram Group: https://t.me/voodoobsc
🐦 Twitter: https://twitter.com/VoodooBSC
...
library Address{
 ...
 contract Voodoo is Context, IERC20, Ownable {
    ...
    uint256 public swapTokensAtAmount = 30_000_000_000_000 * 10**9;
    uint256 public maxBuyLimit = 1_000_000_000_000_000 * 10**9;
    uint256 public maxSellLimit = 1_000_000_000_000_000 * 10**9;
    uint256 public maxWalletLimit = 2_000_000_000_000_000 * 10**9;
    
    uint256 public genesis_block;
    
    address public marketingWallet = 0x8eBc5872a8381C3ca38ACdCD1ae60Ed7D30c9286;
    address public VoodooWallet = 0x196Fb5c1714bbF2227f59425412f56642a18F0c8;
    ...
    Taxes public taxes = Taxes(0, 6, 2, 1);
    Taxes public sellTaxes = Taxes(0, 6, 2, 1);
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[VoodooWallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[marketingWallet] = true;
        allowedTransfer[VoodooWallet] = true;
    ...
    function setTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _Voodoo) public onlyOwner {
       taxes = Taxes(_rfi,_marketing,_liquidity,_Voodoo);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _Voodoo) public onlyOwner {
       sellTaxes = Taxes(_rfi,_marketing,_liquidity,_Voodoo);
        emit FeesChanged();
    }
    ...
    function _takeVoodoo(uint256 rVoodoo, uint256 tVoodoo) private {
        totFeesPaid.Voodoo +=tVoodoo;

        if(_isExcluded[address(this)])
        {
            _tOwned[address(this)]+=tVoodoo;
        }
        _rOwned[address(this)] +=rVoodoo;
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rVoodoo) = _getRValues2(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.tVoodoo = tAmount*temp.Voodoo/100;
        s.tTransferAmount = tAmount-s.tRfi-s.tMarketing-s.tLiquidity-s.tVoodoo;
        return s;
    }
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rMarketing, uint256 rLiquidity){
        ...
        uint256 rVoodoo = s.tVoodoo*currentRate;
        rTransferAmount =  rAmount-rRfi-rMarketing-rLiquidity-rVoodoo;
        return (rAmount, rTransferAmount, rRfi,rMarketing,rLiquidity);
    }
    
    function _getRValues2(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 rVoodoo) {

        if(!takeFee) {
          return(0);
        }

        rVoodoo = s.tVoodoo*currentRate;
        return (rVoodoo);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tMarketing + s.tVoodoo);
        }
        if(s.rMarketing > 0 || s.tMarketing > 0) _takeMarketing(s.rMarketing, s.tMarketing);
        if(s.rVoodoo > 0 || s.tVoodoo > 0) _takeVoodoo(s.rVoodoo, s.tVoodoo);
        emit Transfer(sender, recipient, s.tTransferAmount);
        
    }

    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.marketing + temp.Voodoo) * 2;
        ...
        uint256 VoodooAmt = unitBalance * 2 * temp.Voodoo;
        if(VoodooAmt > 0){
            payable(VoodooWallet).sendValue(VoodooAmt);
        }
    }
    ...
    function updateVoodooWallet(address newWallet) external onlyOwner{
        VoodooWallet = newWallet;
    }
