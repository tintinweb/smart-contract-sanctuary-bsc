...
contract IDO3 is Ownable {
    ...
    event SetTimeAndPrice(uint256 price, uint256 buyStartTime, uint256 buyEndTime, uint256 endTime);

    constructor(address _USDTToken, address _fundAddress) {
        USDTToken = _USDTToken;
        price = 6 * (10 ** IERC20Metadata(USDTToken).decimals()) / 10;
        fundAddress = _fundAddress;
    }

    function setTimeAndPrice(uint256 _price, uint256 _buyStartTime, uint256 _buyEndTime, uint256 _endTime) external onlyOwner {
        price = _price;
        buyStartTime = _buyStartTime;
        buyEndTime = _buyEndTime;
        endTime = _endTime;
        emit SetTimeAndPrice(_price, _buyStartTime, _buyEndTime, _endTime);
    }
    ...
    function buy(uint256 money, address _superior) external {
        ...
        if (superior == address(0) && _superior == address(0)) {
            superior = _superior;
        }
        uint256 fee1 = 0;
        uint256 fee2 = 0;
        ...
        GSS(GSSToken).transfer(msg.sender, _amount);
...
contract GSS is ERC20Basics, Ownable {
    ...
    constructor(address _USDTToken, address _IDOToken, address _GSSHoldToken, address _marketing, address _community, address _lp, address _uniswapV2RouterAddress)
...
contract IDO is Ownable {
    ...
    uint256 public endTime = 1660060800;
