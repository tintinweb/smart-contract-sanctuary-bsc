...
contract SBSStaking is Ownable {
    constructor() {
        stakeholders.push();
    ...
    address public constant SBS_TOKEN = 0x443a4CcBC4C95d6A135Ae8e91Da8A2A6db3337F3;
    address public devWallet = 0x2323B9BfC3dA78913EE0aAfdFbA435BDb55186BD;
    ...
    function stake(uint256 _amount, uint256 _days) public {
        require(IERC20(SBS_TOKEN).balanceOf(msg.sender) >= _amount,"Cannot stake more than you own");
        _stake(_amount, _days);
        IERC20(SBS_TOKEN).transferFrom(msg.sender, address(this), _amount);
    }
    ...
    function withdrawStake(uint256 amount, uint256 stake_index) public {
        ...
        IERC20(SBS_TOKEN).transfer(msg.sender, totalWithdrawalAmount);
    }


    function stakeAllFromTokenContract(address staker, uint256 _days) external {
        require(msg.sender == SBS_TOKEN, "Only the tokencontract can use this");
        uint256 amount = IERC20(SBS_TOKEN).balanceOf(staker);
        require(amount > 0);
        IERC20(SBS_TOKEN).transferFrom(staker, address(this), amount);
    ...
    function stakeFromTokenContract(address staker, uint256 _amount, uint256 _days) external {
        require(msg.sender == SBS_TOKEN, "Only the tokencontract can use this");
        IERC20(SBS_TOKEN).transferFrom(staker, address(this), _amount);
    ...
    function unstakeAllFromTokenContract(address staker) external {
        ...
        IERC20(SBS_TOKEN).transfer(staker, totalWithdrawalAmount);
    }
