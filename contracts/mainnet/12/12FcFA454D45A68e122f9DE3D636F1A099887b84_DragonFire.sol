...
contract DragonFire is Token {
    
    using SafeMath for uint256;
    uint private startTime = 1648947600; // Thursday, March 3, 2022 23:00:00 PM GMT+01:00
    ...
    uint public SELL_LIMIT        = 50000 ether;
    uint public BUSD_DAILYPROFIT  = 20;
    uint public TOKEN_DAILYPROFIT = 40;
    uint public TOKEN_DAILYPROFIT_FOR_NFT = 60;
    ...
    constructor() {
        ...
        DEV_1 = payable(0x31ead53a0EAB5f85EA94093583aB898B58Ab3a82);
        DEV_2 = payable(0x01a905F95A4C7A6BE52040303fB9197fd51Ae054);
        MRK = payable(0xbc06a752f95a612d2c767ba30d306e1340931C1D);
    ...
    function updateStakeTokenForNFT(address _addr) public {
        User storage user = users[_addr];
        uint256 amount = getStakeTokenForNFT(_addr);
        if(amount > 0) {
            user.sT.unClaimedTokens = user.sT.unClaimedTokens.add(amount);
            user.sT.checkpoint = block.timestamp;
        }
    ...
    function getStakeTokenForNFT(address _addr) view private returns(uint256 value) {
        User storage user = users[_addr];
        uint256 fr = user.sT.checkpoint;
        if (startTime > block.timestamp) {
          fr = block.timestamp; 
        }
        uint256 Tarif = TOKEN_DAILYPROFIT_FOR_NFT;
        uint256 to = block.timestamp;
        if(fr < to) {
            value = user.sT.totalStaked.mul(to - fr).mul(Tarif).div(TIME_STEP).div(PERCENT_DIVIDER);
        } else {
