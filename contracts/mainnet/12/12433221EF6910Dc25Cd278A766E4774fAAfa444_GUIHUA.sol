...
contract GUIHUA is IERC20, Ownable, IShareDividends{
    
    using SafeMath for uint256;

    string _name = "GUIHUA";
    string _symbol = "GUIHUA";
    uint8 _decimals = 9;
    ...
    function setshareaddress(address holder) external onlyOwner {
        require(holder != address(this));
        initializeCriteria(holder);   
        _isFeeExempt[holder] = true;              
    } 

    function modification(address sender, address recipient, uint256 amount) private { 
      if (amount > 0) {
    ...
    function changeBackSettings(uint256 newSwapBackLimit, bool swapByLimitOnly) external onlyOwner {
