...
contract SAFUFINANCE is ERC20Detailed, Ownable {
    ...
    uint256 public DividendFeeBuy = 20;
    uint256 public DividendFeeSell = 80;
    uint256 public sellFee = 30;
    uint256 public autofirePitFee = 0;
    uint256 public totalFee =
        liquidityFee.add(marketingFee).add(DividendFeeBuy).add(
            autofirePitFee
        );

    uint256 public totalFeeSell = DividendFeeSell.add(liquidityFee).add(marketingFee).add(sellFee);
    ...
    uint256 private constant MAX_SUPPLY = 300 * 10**9 * 10**DECIMALS;
    ...
    constructor() ERC20Detailed("SafuFinance", "SFA", uint8(DECIMALS)) Ownable() {
        ...
        autoLiquidityReceiver = 0xD6746027cAfBa7c860C162868c4D719682f93f01;
        marketingReceiver = 0xc327B5Cbd2a89E259D0b4610Adb025ac49c2C0D4;   
        ...
        isDividendExempt[marketingReceiver] = true;
    ...
    function takeFee(
        ...
        uint256 _DividendFeeBuy = DividendFeeBuy;

        if (recipient == pair) {
            _totalFee = totalFeeSell;
            _marketingFee = marketingFee.add(sellFee);
            _DividendFeeBuy = DividendFeeSell;
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_marketingFee.add(_DividendFeeBuy))
        );
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountETHToMarketing = address(this).balance.sub(
            balanceBefore
        );

        (bool success, ) = payable(marketingReceiver).call{
            value: amountETHToMarketing.mul(marketingFee).div(
                marketingFee.add(DividendFeeBuy)
            ),
            gas: 30000
        }("");

        try distributor.deposit {
            value: amountETHToMarketing.mul(DividendFeeBuy).div(marketingFee.add(DividendFeeBuy))
        } () {} catch {}        
    }
    ...
    function withdrawAllToMarketing() external swapping onlyOwner {
        ...
        uint256 amountToSwap = _gonBalances[address(this)].div(_gonsPerFragment);
        require( amountToSwap > 0,"There is no safufinance token deposited in token contract");
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();
        router.swapExactTokensForETHSupportingFeeOnTransferTokens(
