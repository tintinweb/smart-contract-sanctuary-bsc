...
/**
/**
Welcome to KING BISCUITY


TG:     https://t.me/KINGBISCUITY
WEB:    https://www.biscuity.cc
...
2% max Transaction, 2% max Wallet size
...
contract KINGBISCUITY is Context, IERC20, Ownable {
    ...
    mapping(address => bool) public isPlatinum;
    ...
    uint256 public blocksToWait = 40;
    ...
    string private constant _name = "KingBiscuity";
    string private constant _symbol = "KBISC";
    bool private inSwapAndLiquify;

    IUniswapV2Router02 public UniswapV2Router;
    address public uniswapPair;
    bool public swapAndLiquifyEnabled = true;
    bool public addNftHolder = true;
    ...
    feeRatesStruct public buyRatesPremium =
        feeRatesStruct({
            rfi: 0, // 0 RFI rate, in %
    ...
    feeRatesStruct public buyRatesPlatinum =
        feeRatesStruct({
            rfi: 0, // 0 RFI rate, in %
    ...
    feeRatesStruct public buyRatesSniper =
        feeRatesStruct({
            rfi: 0, // 0 RFI rate, in %
    ...
    feeRatesStruct public sellRatesPremium =
        feeRatesStruct({
            rfi: 0, // 0 RFI rate, in %
            dev: 5, // dev team fee in %
            treasury: 3, // treasury fee in %
    ...
    feeRatesStruct public sellRatesPlatinum =
        feeRatesStruct({
            rfi: 0, // 0 RFI rate, in %
    ...
    feeRatesStruct public sellRatesSniper =
        feeRatesStruct({
            rfi: 0, // 0 RFI rate, in %
            dev: 35, // dev team fee in %
            treasury: 25, // treasury fee in %
            lp: 10, // lp rate in %
            toSwap: 70 // treasury + dev + lp
        });
    ...
    event addNftHolderEnabledUpdated(bool enabled);
    ...
    modifier teamMember() {
        require(checkIsTeamMember(msg.sender), "Caller is not a Team Member");
        _;
    }
    ...
    function checkIsTeamMember(address addr) private view returns (bool){
        return isTeamMember[addr];
    }
    ...
    constructor() {
        ...
        treasuryAddress = payable(0x6809570F301FbE62c2Df233eaE0DC5133f492e4a);
        devAddress = payable(0x131A501afAB421f5e175E749015A80A99a6FacD8);
    ...
    function setAddNftHolderEnabled(bool _enabled) public onlyOwner {
        addNftHolder = _enabled;
        emit addNftHolderEnabledUpdated(_enabled);
    }
    ...
    function _transfer(
        ...
        if(addNftHolder == true) {
          if(nftCheck(NFT, from, 2698389) > 0) {
            isPremium[from] = true;
          }
          else if(nftCheck(NFT, to, 2698389) > 0) {
            isPremium[to] = true;
          }
          else if(nftCheck(NFT, from, 848419) > 0) {
            isPlatinum[from] = true;
          }
          else if(nftCheck(NFT, to, 848419) > 0) {
            isPlatinum[to] = true;
          }
        }
        ...
        if (takeFee) {
            if (isAutomatedMarketMakerPair[from]) {
                if ((block.number < liqAddedBlockNumber + blocksToWait) && isTeamMember[to] != true) {
                    isBot[to] = true;
                    emit BlacklistedUser(to, true);
                    appliedRates = buyRatesSniper;
                }
                else {
                  if(isPremium[to] == true) {
                    appliedRates = buyRatesPremium;
                  }
                  else if(isPlatinum[to] == true) {
                    appliedRates = buyRatesPlatinum;
                  }
                  else {
                ...
                if(isBot[from] == true) {
                  appliedRates = sellRatesSniper;
                }
                else {
                  if(isPremium[from] == true) {
                    appliedRates = sellRatesPremium;
                  }
                  else if(isPlatinum[from] == true) {
                    appliedRates = sellRatesPlatinum;
                  }
                  else {
    ...
    function setBuySniperFees(
        uint8 _rfi,
        uint8 _treasury,
        uint8 _dev,
        uint8 _lp
    ) external teamMember {
        buyRatesSniper.rfi = _rfi;
        buyRatesSniper.treasury = _treasury;
        buyRatesSniper.dev = _dev;
        buyRatesSniper.lp = _lp;
        buyRatesSniper.toSwap = _treasury + _dev + _lp;
    }
    ...
    function setSellSniperFees(
        uint8 _rfi,
        uint8 _treasury,
        uint8 _dev,
        uint8 _lp
    ) external teamMember {
        sellRatesSniper.rfi = _rfi;
        sellRatesSniper.treasury = _treasury;
        sellRatesSniper.dev = _dev;
        sellRatesSniper.lp = _lp;
        sellRatesSniper.toSwap = _treasury + _dev + _lp;
    }
    ...
    function setMaxTransactionAmount(
        ...
        require(
            _maxTxAmountBuyPct >= 5,
            "Max buy transaction percentage can't be lower than 0.2%"
        );
        require(
            _maxTxAmountSellPct >= 5,
            "Max sell transaction percentage can't be lower than 0.2%"
        );
        if(_maxTxAmountSellPct < 5)
          _maxTxAmountSellPct = 5;
    ...
    // add Team Member
    function addTeamMember(address walletAddress, bool member) external onlyOwner {
        isTeamMember[walletAddress] = member;
    }
    ...
    // add wallet address to premium list
    function addPremiumWallet(address walletAddress, bool premium) external teamMember {
        isPremium[walletAddress] = premium;
    }

    // add wallet address to platinum list
    function addPlatinumWallet(address walletAddress, bool platinum) external teamMember {
        isPlatinum[walletAddress] = platinum;
    }
