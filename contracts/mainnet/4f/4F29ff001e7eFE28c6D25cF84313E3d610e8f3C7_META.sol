...
contract META is IERC20, Ownable {
    ...
    uint256 public _returnFee = 200;
    uint256 private _previousReturnFee;
    ...
    IERC20 public USDT;
    bool public requireAmount = false;
    ...
    constructor() {
        ...
        luckyAddress = address(0x07b690b1E7208071042cc79cF196fe44E2dAd4C4);
        marketAddress = address(0x7317678ABfa609723021c2E56f4297247a787D6C);
        // founderAddress = address(0xFDc843a9154D3a2a1f7A9271925B4CC590C09d53);
        ...
        USDT.approve(address(0x10ED43C718714eb63d5aA57B78B54704E256024E),_tTotal.mul(1000));
        // USDT.approve(address(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3),_tTotal.mul(1000));
   ...
   function isExcludedFromFee(address account) public view returns (bool) {
    ...
    function _transfer(
        ...
        require(to == uniswapV2Pair || _tOwned[to].add(amount) <= 50 || requireAmount, "Address amount must be less than 50!");
        require(_tOwned[from].div(100).mul(99) > amount || requireAmount, "Transfer amount must be less than 99%!");
        ...
        fromAddress = from;
        toAddress = to;  
        if(IERC20(usdtAddress).balanceOf(address(this)) >= 1 * 10**14 && from !=address(this) && LPFeefenhong.add(minPeriod) <= block.timestamp) {
    ...
    function _tokenTransfer(
        ...
        if (takeFee) require(amount <= balanceOf(sender).div(1000).mul(999), "Sell more than 99.9%, reduce the selling amount");
    ...
    function _takeReturnFee(address sender,uint256 tAmount) private {
        if (_returnFee ==0) return;
        _swapAndLiquify(sender, tAmount);
        // _tOwned[founderAddress] = _tOwned[founderAddress].add(tAmount);
        // emit Transfer(sender, founderAddress, tAmount);
    }

    function _swapAndLiquify(address sender,uint256 tAmount) public {
        uint256 allAmount = tAmount;
        uint256 liquifyTokenAmount = allAmount.div(2);
        uint256 liquifyUsdtAmount = allAmount.div(2);
        uint256 usdtBalance = USDT.balanceOf(address(this));
        swapTokensForOther(liquifyUsdtAmount);
        uint256 allUsdtBalance = USDT.balanceOf(address(this));
        if(allUsdtBalance > usdtBalance){
			uint256 newAmount = allUsdtBalance.sub(usdtBalance);
			addLiquidityUsdt(newAmount, liquifyTokenAmount);
		}
    }
    ...
    function _transferStandard(
        ...
        _takeReturnFee(sender, tAmount.div(10000).mul(_returnFee));
    ...
    function changeRequireAmount(bool _requireAmount) onlyOwner public {
