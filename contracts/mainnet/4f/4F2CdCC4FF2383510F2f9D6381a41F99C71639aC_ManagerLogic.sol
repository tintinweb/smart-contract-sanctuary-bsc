...
import "../invitationCenter/InvitationCenterInterface.sol";
import "../dependant/holders/HERC721HolderLogic.sol";
import "../helper/Helper.sol";
import "../dependant/helperLibrary/ConstantLibrary.sol";

import "hardhat/console.sol";

contract ManagerLogic is Delegate, ManagerLayout,
OwnableLogic,
...
ManagerInterface {
    ...
    modifier onlyRegistered(address who){
        require(InvitationCenterInterface(invitationCenter()).isRegistered(who), "not registered");
        _;
    }

    modifier setRegistry(address who){

        if (!_reactedAddress.contains(who)) {
            _reactedAddress.add(who);
        }
        _;

    }

    function stake(uint256 planCode, uint256 capitalAmount, bytes32 inviterCode) override external setRegistry(msg.sender) {

        {
            InvitationCenterInterface invitation = InvitationCenterInterface(invitationCenter());
            if (invitation.isRegistered(msg.sender)) {
                require(inviterCode == ConstantLibrary.ZERO_BYTES, "you have registered, inviterCode should be empty");
            } else {
                //new comes
                invitation.registerInviterAuth(msg.sender, inviterCode);
        ...
        {
            require(globalConfig.minimalBnb <= capitalAmount, "minimal");
            Helper.takeFund(address(0), msg.sender, capitalAmount);


            //send to fee collector
            uint256 fee = capitalAmount.mul(globalConfig.stakeFeeRate).div(ConstantLibrary.THOUSAND);
            Helper.sendFund(address(0), feeCollector(), fee);

        }

        {
            ManagerType.PlanConfig storage plan = _planConfig[planCode];
            require(plan.planCode == planCode, "unknown plan");

            ManagerType.CalcPlanRet memory info = calcPlan(planCode, capitalAmount);

            _recordIdCounter.increment();
            uint256 recordId = _recordIdCounter.current();

            _recordDetails[recordId] = ManagerType.Record(
                capitalAmount, //capital
                block.timestamp, //start
                block.timestamp.add(info.duration), //end
                block.timestamp, //lastUpdate
                info.isFixed, //isFixed
                info.ratePerThousand,
                info.RoI
            );

            _liveRecords[msg.sender].add(recordId);
        }


        {
            //invitation bonus
            address[] memory inviters = InvitationCenterInterface(invitationCenter()).getInviters(msg.sender);
            ...
            for (uint256 i = 0; i < inviters.length; i++) {
                ...
                uint256 bonus = capitalAmount.mul(globalConfig.bonusRate[i]).div(ConstantLibrary.THOUSAND);
                _people[inviter].bonusRoI = _people[inviter].bonusRoI.add(bonus);
            }
        }
    }

    function _fixRate(uint256 when) view internal returns (uint256){
        ManagerType.GlobalConfig storage globalConfig = _globalConfig[0];

        if (globalConfig.startTimestamp <= when) {
            return when.sub(globalConfig.startTimestamp).div(1 days).mul(globalConfig.rateIncreasePerDay);
        }
        return 0;
    }

    function calcPlan(uint256 planCode, uint256 capitalAmount) view public returns (ManagerType.CalcPlanRet memory){

        ManagerType.CalcPlanRet memory ret = ManagerType.CalcPlanRet(0, 0, 0, false);

        ManagerType.PlanConfig storage plan = _planConfig[planCode];
        require(plan.planCode != ManagerType.PLAN_UNKNOWN, "unknown plan");

        uint256 lastDays = plan.duration.div(1 days);

        ret.duration = plan.duration;
        ret.isFixed = plan.isFixed;
        ret.ratePerThousand = plan.interestRatePerThousandPerDay.add(_fixRate(block.timestamp));

        if (plan.isFixed == true) {

            uint256 RoI = 0;
            for (uint256 day = 0; day < lastDays; day++) {
                RoI = RoI.add(
                    capitalAmount.add(RoI).mul(ret.ratePerThousand).div(ConstantLibrary.THOUSAND)
                );
            }
            ret.RoI = RoI;

        } else {
            ret.RoI = capitalAmount.mul(lastDays).mul(ret.ratePerThousand).div(ConstantLibrary.THOUSAND);
    ...
    function claim() override external onlyRegistered(msg.sender) setRegistry(msg.sender) {

        ManagerType.GlobalConfig storage globalConfig = _globalConfig[0];
        ManagerType.Person storage person = _people[msg.sender];
        uint256[] memory liveRecords = _liveRecords[msg.sender].values();

        uint256 totalRoIClaim = 0;
        for (uint256 i = 0; i < liveRecords.length; i++) {
            uint256 recordId = liveRecords[i];
            ManagerType.Record storage record = _recordDetails[recordId];

            if (record.isFixed) {
                if (record.end <= block.timestamp) {
                    record.lastUpdate = block.timestamp;
                    totalRoIClaim = totalRoIClaim.add(record.RoI);

                    _liveRecords[msg.sender].remove(recordId);
                    _deadRecords[msg.sender].add(recordId);

                }
            } else {

                uint256 from = Math.max(record.start, record.lastUpdate);
                uint256 to = Math.min(block.timestamp, record.end);
                ...
                if (to > from) {
                    uint256 span = to.sub(from);

                    uint256 amount = record.RoI.mul(span).div(record.end.sub(record.start));

                    totalRoIClaim = totalRoIClaim.add(amount);

                }
            }

        }
        person.RoIClaimed = person.RoIClaimed.add(totalRoIClaim);

        //
        {
            if (person.bonusRoI > 0) {

                totalRoIClaim = totalRoIClaim.add(person.bonusRoI);

                person.bonusClaimed = person.bonusClaimed .add(person.bonusRoI);
                person.bonusRoI = 0;
            }
        }


        require(totalRoIClaim > 0, "no profit or bonus to claim");

        uint256 contractBalance = address(this).balance;
        if (contractBalance < totalRoIClaim) {
            totalRoIClaim = contractBalance;
        }

        //take fee
        uint256 fee = totalRoIClaim.mul(globalConfig.claimFeeRate).div(ConstantLibrary.THOUSAND);
        Helper.sendFund(address(0), feeCollector(), fee);

        Helper.sendFund(address(0), msg.sender, totalRoIClaim.sub(fee));

    }

    function getGlobalConfig() override view external returns (ManagerType.GlobalConfig memory){
        return _globalConfig[0];
    }

    function getPerson(address who) override view external returns (ManagerType.Person memory){
        return _people[who];
    }

    function liveRecordsLength(address who) override view external returns (uint256){
        return _liveRecords[who].length();
    }

    function liveRecordsAt(address who, uint256 index) override view external returns (uint256){
        return _liveRecords[who].at(index);
    }

    function liveRecordsContains(address who, uint256 input) override view external returns (bool){
        return _liveRecords[who].contains(input);
    }

    function liveRecordsValues(address who) override view external returns (uint256[] memory){
        return _liveRecords[who].values();
    }

    //--

    function deadRecordsLength(address who) override view external returns (uint256){
        return _deadRecords[who].length();
    }

    function deadRecordsAt(address who, uint256 index) override view external returns (uint256){
        return _deadRecords[who].at(index);
    }

    function deadRecordsContains(address who, uint256 input) override view external returns (bool){
        return _deadRecords[who].contains(input);
    }

    function deadRecordsValues(address who) override view external returns (uint256[] memory){
        return _deadRecords[who].values();
    }

    //--

    function recordDetails(uint256 input) override view external returns (ManagerType.Record memory){
        return _recordDetails[input];
    }
}
...
import "./ManagerEvent.sol";

import "./ManagerEvent.sol";

interface ManagerInterface is
OwnableInterface,
CrownNameServiceRefInterface,
ManagerEvent
{
    function stake(uint256 planCode, uint256 capitalAmount, bytes32 inviterCode) external;

    function calcPlan(uint256 planCode, uint256 capitalAmount) view external returns (ManagerType.CalcPlanRet memory);

    function claim() external;

    function getGlobalConfig() view external returns (ManagerType.GlobalConfig memory);

    function getPerson(address who) view external returns (ManagerType.Person memory);

    function liveRecordsLength(address who) view external returns (uint256);

    function liveRecordsAt(address who, uint256 index) view external returns (uint256);

    function liveRecordsContains(address who, uint256 input) view external returns (bool);

    function liveRecordsValues(address who) view external returns (uint256[] memory);

    //--

    function deadRecordsLength(address who) view external returns (uint256);

    function deadRecordsAt(address who, uint256 index) view external returns (uint256);

    function deadRecordsContains(address who, uint256 input) view external returns (bool);

    function deadRecordsValues(address who) view external returns (uint256[] memory);

    //--

    function recordDetails(uint256 input) view external returns (ManagerType.Record memory);
}
...
import "../dependant/preclude/Preclude.sol";
...
library Helper {
    ...
    function _genNewRandomness(uint256 oldRandomness, bytes32 reason) pure internal returns (uint256){
        return uint256(keccak256(abi.encode(oldRandomness, reason)));
    }
    ...
    function _fitRangeInclude(uint256 input, uint256 min, uint256 max) pure internal returns (uint256){
        ...
        if (min == max) {
            ...
            uint256 range = max.sub(min).add(1);
            uint256 number = input.mod(range);
            return min.add(number);
        }
    }
    ...
    function takeFund(address token, address from, uint256 amount) internal {
        if (token != address(0)) {
            IERC20(token).safeTransferFrom(from, address(this), amount);
        } else {
            require(msg.sender == from, "you can't take native token from one who is not the msg.sender");
            require(msg.value >= amount, "native token enough");
        }
    }

    function sendFund(address token, address to, uint256 amount) internal {
        if (token != address(0)) {
