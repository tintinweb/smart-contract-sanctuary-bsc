...
contract NFTTemplateManager is ERC721Enumerable ,Ownable {
    ...
    uint256 public _defaultTemplateId = 0;  // Modify to the templateId that you want
    uint256 public _currentNextId;       // The NFT id that you want to start with. Auto increment here.
    ...
    function mintTo(address to, uint size) external onlyLaunchpad {
        ...
        for (uint256 i=1; i <= size; i++) {
            _mint(to, _defaultTemplateId);
            LAUNCH_SUPPLY++;
        }
    }

    function changeLaunchMaxSupply(uint256 newnubmer) external onlyOwner {
        require(newnubmer > LAUNCH_MAX_SUPPLY, "LAUNCH_MAX_SUPPLY can only increase");
        LAUNCH_MAX_SUPPLY = newnubmer;
    }
    // ----Binance Launch Pad----

    
    string public _baseUri;
    // Use 0 = "" as default if you do not want to use it.
    ...
    constructor (string memory name_, string memory symbol_, string memory baseUri_, address launchpad, uint256 maxSupply) {
        _name = name_;
        _symbol = symbol_;
        _baseUri = baseUri_;
        LAUNCHPAD = launchpad;
    ...
    function _mint(address _to, uint256 _uriTemplateId) internal returns (uint256){
    ...
    function safeMint(address _to, uint256 _uriTemplateId) public onlyMinter {
        uint256 _id = _mint(_to, _uriTemplateId);
        if (_to.isContract()) {
    ...
    function safeBatchMint(address[] calldata _to, uint256 _uriTemplateId) external onlyMinter {
        ...
        for (uint256 i = 0; i < _to.length; ++i) {
            ...
            uint256 _id = _mint(to, _uriTemplateId);
            if (to.isContract()) {
                require(_checkOnERC721Received(address(0), to, _id, ''), "ERC721: transfer to non ERC721Receiver implementer");
            }
        }
    }
    ...
    function queryMultipleTokenTemplate(uint256[] calldata _templateId) external view returns(string[] memory){
        ...
        for(uint256 n; n < _templateId.length; ++n){
            templateStrings[_templateId[n]] = _uriTemplate[n];
        }
