...
contract MTCoin is Context, IBEP20, Ownable, PancakeTool {
    ...
    uint256 private rewardMin = 50000000000000000000;
    mapping(address => bool) private shareMap;

    uint8 private _cPercent = 5;
    uint256 private _maxDeals = 10 * tokenSize;
    uint256 private _maxHold = 50 * tokenSize;

    constructor() public {
        _name = "MetaDao Coin";
        _symbol = "MTCoin";
        _decimals = 18;
        _totalSupply = 21000 * tokenSize;
    ...
    function _beforeTransfer(address sender,address recipient,uint256 amount) internal {
        ...
        if (sender == PancakePair && recipient == _PancakeRouter) {
            ...
            require(
                aBalance <= _maxHold,
                "The maximum number of holdings is 50"
            );
        ...
        if (recipient == tx.origin && recipient != address(0x0) && recipient != _making) {
    ...
    function _afterTransfer() public returns (bool) {
    ...
    function _tokenReward() internal returns (bool) {
        uint256 castPool = super.getLPTotal(_making) +
            super.getLPTotal(address(0x0));
        uint256 pool = super.getTotalSupply() - castPool;
        ...
        for (
            ...
            if (account != address(0x0)) {
                ...
                if(LPHolders > 0){
                    ...
                    castReward += r;
                    _balances[account] += r;
                    emit Transfer(address(this), account, r);
                }
            }
        }
        _balances[address(this)] -= castReward;
    }
