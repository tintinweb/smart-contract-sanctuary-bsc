...
contract whalefogBNB is MerkleTreeWithHistory, ReentrancyGuard {
  ...
  function _processWithdraw(uint256 _amount, address payable _recipient, address payable _relayer, uint256 _fee) internal {
     ...
     uint256 reamount = _amount.sub(relayamount); 
      ...
      (bool success1, ) = _recipient.call{value: reamount.sub(_fee)}("");
      require(success1, "Address: unable to send value, recipient may have reverted");
      //_recipient.transfer(reamount.sub(_fee));
      //token.transfer(_recipient, reamount - _fee);

      if (relayamount > 0) {
        (bool success2, ) = _relayer.call{value: relayamount}("");
        require(success2, "Address: unable to send value, relayer may have reverted");
      ...
      if (_fee > 0) {
        ...
        (bool success3, ) = _relayer.call{value: _fee}("");
        require(success3, "Address: unable to send value, relayer may have reverted");
