...
abstract contract AbsToken is IERC20, Ownable {
    ...
    constructor (
        ...
        _numToSell = 5 * 10 ** Decimals;

        lpHolderRewardCondition = 10 * 10 ** Decimals;
    }
            ...
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                ...
                if (_swapPairList[to]) {
                    if (!inSwap) {
                        ...
                        if (contractTokenBalance >= _numToSell) {
                            uint256 swapFee = _fundFee + _lpDividendFee;
                            swapTokenForFund(_numToSell, swapFee);
                        }
                    }
                }
                takeFee = true;
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        uint256 dividendAmount = usdtBalance * _lpDividendFee / swapFee;
        if (dividendAmount > 0) {
            USDT.transferFrom(tokenDistributor, address(this), dividendAmount);
        }
    }
    ...
    function setNumToSell(uint amount) external onlyFunder {
        _numToSell = amount * 10 ** _decimals;
    }

    uint256 public currentLPIndex;
    uint256 public lpHolderRewardCondition;
    uint256 public progressLPRewardBlock;
    uint256 public _progressLPBlockDebt = 200;
    mapping(address => uint256) public _progressLPRewardTime;

    function processLPReward(uint256 gas) private {
        uint256 blockNum = block.number;
        if (progressLPRewardBlock + _progressLPBlockDebt > blockNum) {
            return;
        }
        ...
        if (balance < lpHolderRewardCondition) {
            return;
        }
        balance = lpHolderRewardCondition;
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            if (_progressLPRewardTime[shareHolder] + 28800 <= blockNum) {
    ...
    function setLPHolderRewardCondition(uint256 amount) external onlyFunder {
        lpHolderRewardCondition = amount;
    }

    function setLPProgressBlockDebt(uint256 progressBlockDebt) external onlyFunder {
        _progressLPBlockDebt = progressBlockDebt;
    }
...
contract HUPToken is AbsToken {
    ...
    //USDT
        ...
        21000,
    //Fund
        address(0x93Ad9BeCfC95D2ed03371c02759610512472Ed3b),
    //Receive
        address(0xAD4b9A6960eA1cB183F4e9659760f10e17436D47)
    ){
