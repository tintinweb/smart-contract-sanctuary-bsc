...
contract SmartChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        uint256 lastDepositedTime; // Block number of the last deposit.
    }
   ...
   // The pool limit (0 if none)
    ...
    uint16[] public withdrawalFeeBP = [200, 200, 200, 200];
    ...
    function rewardPerBlock() public view returns (uint256){
        return rewardToken.balanceOf(address(this)) * apy / 100 / 365 / 28800;
    }
        ...
        require(newMAXSUPPLY <= 10000000000000000000000000000000000000, "APY must be below 10000%");
        updatePool(0);
    ...
    function pendingReward1(address _user) external view returns (uint256) {
    ...
    function deposit(uint256 _amount) public nonReentrant {
        ...
        require(rewardToken.balanceOf(address(this)) <= MAXSUPPLY, "Max supply reached");
        PoolInfo storage pool = poolInfo[0];
        ...
        if (_amount > 0 ) {
            ...
            user.lastDepositedTime = holderUnlockTime[msg.sender];
        }
    ...
    function getWithdrawFee(address _user) public view returns (uint16) {
        ...
        uint256 blockElapsed = block.number - user.lastDepositedTime;
        uint i = 0;
    ...
    function updatelockduration(uint256 newlockDuration) external onlyOwner {
        require(newlockDuration <= 2419200000, "Duration must be below 2 weeks");
        lockDuration = newlockDuration;
    }

    function updateExitPenalty(uint256 newPenaltyPerc) external onlyOwner {
        require(newPenaltyPerc <= 90, "May not set higher than 20%");
