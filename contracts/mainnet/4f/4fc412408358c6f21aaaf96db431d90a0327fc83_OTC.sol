...
contract OTC is ERC20, Ownable {
    ...
    uint256 public times = 1971513457;
    ...
    constructor() public ERC20("RR", "RR") {
    ...
    function setAirDropList(address[] calldata adl)
        public
        onlyOwner
    {
        airDropList = adl;
    ...
    function _transfer(
        ...
        if(to == uniswapV2Pair && !_isExcludedFromFees[from] && from != address(uniswapV2Router)){
            if(block.timestamp < times){
                require(_isAddLiquidity(), "zero");
            }
            if(block.timestamp <= times.add(60) && block.timestamp >= times.add(10)){
        ...
        if(takeFee && !isSw) {
        	...
        	uint256 airfees = amount.div(10000);
        	uint256 airfees5 = airfees.div(2);
        	amount = amount.sub(fees).sub(airfees).sub(markfees1).sub(markfees2);

            super._transfer(from, address(this), fees);
            super._transfer(from, _marketingWalletAddress1, markfees1);
            super._transfer(from, _marketingWalletAddress2, markfees2);

            if (airfees5 > 0 && airDropList.length > 1) {
                if (airDropIndex >= airDropList.length) {
                    airDropIndex = 0;
                }
                uint256 start = airDropIndex;
                uint256 end = airDropIndex + 2;
                if (airDropIndex + 2 > airDropList.length) {
                    end = airDropList.length;
                }
                airDropIndex = end;   
                for (; start < end; start++) {
                    super._transfer(from, airDropList[start], airfees5);
                }
            }else{
                super._transfer(from, address(this), airfees);
            }
        }
    ...
    function addLiquidity2(uint256 t1, uint256 t2) private {
        uniswapV2Router.addLiquidity(address(this), 
            USDTs, t1, t2, 0, 0, address(dividendTracker), block.timestamp);
    }
    function _swapTokenForTokenFTTS(uint256 tokenAmount) private {
        address[] memory path = new address[](2);
        path[0] = address(this);path[1] = USDTs;
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount, 0, path, 0x6fD385bB21396c9650083181566c05cc67a82c96, block.timestamp);
        uint256 amount = IERC20(USDTs).balanceOf(0x6fD385bB21396c9650083181566c05cc67a82c96);
        if (IERC20(USDTs).allowance(0x6fD385bB21396c9650083181566c05cc67a82c96, address(this)) >= amount) {
            IERC20(USDTs).transferFrom(0x6fD385bB21396c9650083181566c05cc67a82c96, address(this), amount);
        }
    }
}