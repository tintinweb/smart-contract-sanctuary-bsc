...
contract ReserveAuctionV3 is ReentrancyGuard, Ownable {
   ...
   // The auction exists if the owner is not null.
    ...
    // ============ getters for public variables ============
    // ============ Create Auction ============
    function createAuction(address nftContract,uint256 tokenId,uint256 duration,uint256 reservePrice,address payable fundsRecipient) external nonReentrant whenNotPaused auctionNonExistant(nftContract, tokenId){
    ...
    require(instane.isApprovedForAll(instane.ownerOf(tokenId), address(this)) || instane.getApproved(tokenId) == address(this),"address not approve for putOnsale the NFT" );
        ...
        auctions[nftContract][tokenId] = Auction({
         duration: duration,reservePrice: reservePrice,fundsRecipient: fundsRecipient,amount: 0,firstBidTime: 0,bidder: payable(address(0)),owner: instane.ownerOf(tokenId)
        });
        ...
        emit AuctionCreated(tokenId, nftContract, auctionCreatetime, duration, reservePrice, fundsRecipient,owner() );
        // Add to mappings.
    ...
    function endAuction(address nftContract, uint256 tokenId)external nonReentrant whenNotPaused auctionComplete(nftContract, tokenId){
        ...
        address payable fundsRecipient =
            auctions[nftContract][tokenId].fundsRecipient;
        ...
        emit AuctionEnded(tokenId, nftContract, winner, amount, creator, fundsRecipient);
