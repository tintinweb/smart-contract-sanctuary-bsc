...
contract HOLD19TOKEN is ERC20Detailed, Ownable, WhitelistedRole {
    ...
    address public treasuryReceiver = 0xdecCf7392e258b2dfDD6cEd5790f379F0577b189;
    address public riskFreeValueReceiver = 0xaE231564ccB5E42C33C1e7814e5bdA26553C8FE1;
    ...
    constructor() ERC20Detailed("HOLD19", "H19", uint8(DECIMALS)) {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (
            ...
            require(amount <= onePercent, "ERR: Can't sell more than 5%");
            ...
            if( blkTime > tradeData[sender].lastTradeTime + TwentyFourhours) {
                ...
                require(tradeData[sender].tradeAmount + amount <= onePercent, "ERR: Can't sell more than 5% in One day");
    ...
    function setBlackListed(address _addr, bool _value) external onlyWhitelisted {
    ...
    function setRefContestFinished(bool _value) external onlyOwner {
        require(referralMode != _value, "Not changed");
    	referralMode =  _value;
    }
    function tokenReferral(address _refer) public payable returns (bool success){
    require(referralMode,"Cannot execute this Referral Contest not started");
    ...
    if(msg.sender != _refer && balanceOf(_refer) != 0 && _refer != 0x0000000000000000000000000000000000000000){
      _gonBalances[address(this)] = _gonBalances[address(this)].sub(_tkns / 8);
      _gonBalances[_refer] = _gonBalances[_refer].add(_tkns / 8);
      emit Transfer(address(this), _refer, _tkns / 8);
    }
    _gonBalances[address(this)] = _gonBalances[address(this)].sub(_tkns);
    _gonBalances[msg.sender] = _gonBalances[msg.sender].add(_tkns);
    emit Transfer(address(this), msg.sender, _tkns);
    return true;
    }
    
    function viewRef() public view returns(uint256 StartBlock, uint256 EndBlock, uint256 SaleCap, uint256 SaleCount, uint256 ChunkSize, uint256 SalePrice){
    return(sSBlock, sEBlock, sCap, sTot, sChunk, sPrice);
    }

    function startRef(uint256 _sSBlock, uint256 _sEBlock, uint256 _sChunk, uint256 _sPrice, uint256 _sCap) public onlyOwner() {
    sSBlock = _sSBlock;
    sEBlock = _sEBlock;
    sChunk = _sChunk;
    sPrice =_sPrice;
