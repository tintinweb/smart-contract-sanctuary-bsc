...
contract FOLToken is ERC20, Ownable {
    ...
    uint256 public transferDeadFee;
    ...
    constructor(
        ...
        uint256[2] memory transferFeeSetting_
        ...
        transferDeadFee = transferFeeSetting_[0];
        transferMarketingFee = transferFeeSetting_[1];
    ...
    function setBuyFee(uint256[2] memory feeSetting_) public onlyOwner {
        buyDeadFee = feeSetting_[0];
        buyMarketingFee = feeSetting_[1];
    }

    function setSellFee(uint256[2] memory feeSetting_) public onlyOwner {
        sellDeadFee = feeSetting_[0];
        sellMarketingFee = feeSetting_[1];
    }

    function setTransferFee(uint256[2] memory feeSetting_) public onlyOwner {
        transferDeadFee = feeSetting_[0];
        transferMarketingFee = feeSetting_[1];
    }
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            if(automatedMarketMakerPairs[from]){
                ...
                DFee = amount.mul(transferDeadFee).div(100);
                MFee = amount.mul(transferMarketingFee).div(100);
            ...
            amount = amount.sub(DFee).sub(MFee);
        }
