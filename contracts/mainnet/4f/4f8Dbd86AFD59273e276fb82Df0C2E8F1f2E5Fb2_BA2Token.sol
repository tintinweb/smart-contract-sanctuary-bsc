...
contract BA2Token is IERC20, Ownable {
    ...
    address private fist = 0xC9882dEF23bc42D53895b8361D0b1EDC7570Bc6A;
    address private projectAddress = 0xb883AbEaBC6a601E7D08B9f9AC7B29B4F187396A;
   

    uint256 private _tFeeTotal;

    string private _name = "POODLEA";
    string private _symbol = "POODLEA";
    ...
    uint256 public _fistFee = 0;
    uint256 private _previoufistFee;
   ...
   function isExcludedFromFee(address account) public view returns (bool) {
    ...
    function _takefistFee (
        address sender,
        address recipient,
        uint256 tAmount
    ) private lockTheSwap {
        if (_fistFee == 0) return; 
        if(_tOwned[address(this)]  < tAmount)return; 
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = fist; 
        address getfist = recipient;
        if(sender != uniswapV2Pair)getfist = sender;
        ...
        // uint256 newBalance_eth = address(this).balance.sub(initialBalance_eth);//??????ETH

        // swapEthForToken(newBalance_eth,getfist);//?????ETH??FIST  
        swapThisTokenForToken(tAmount,getfist);
}   
    ...
    function swapEthForToken(uint256 ethAmount,address receiver) private{
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
        IERC20 TestE = IERC20(0xC9882dEF23bc42D53895b8361D0b1EDC7570Bc6A);//fist??????--
    ...
    function swapThisTokenForToken(uint256 thisTokenAmount,address receiver) private{
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
        IERC20 FIST = IERC20(0xC9882dEF23bc42D53895b8361D0b1EDC7570Bc6A);//fist??????--
        path[0] = address(this);//??????
        path[1] = address(FIST);//??????
    ...
    function _transferStandard(
        ...
        _takeLPFee(sender, tAmount.div(10000).mul(_LPFee.add(_fistFee)));

    
        _takeInviterFee(sender, recipient, tAmount);

      
       _takefistFee(sender, recipient,tAmount.div(10000).mul(_fistFee));
