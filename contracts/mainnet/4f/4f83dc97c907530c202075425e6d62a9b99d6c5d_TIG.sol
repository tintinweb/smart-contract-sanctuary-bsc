...
abstract contract Ownable {
    ...
    constructor () {
        _owner = 0x8064cF2CC4fbde85F18090f9015c3A8c5824ea70;
...
contract TIG is Ownable, IBEP20 {
    ...
    uint256 public feeToLPDividend = 1;
    uint256 public feeToMarketing = 2;
    uint256 public minAmountToSwap = 3000*(1e18);
    ...
    address public marketingAddr = 0x2C79A1cddDc45821FcAb84903345BEEa8099f99B;
    ...
    function _transfer(address from, address to, uint256 amount) internal {
        ...
        require(!_isBlackList[from], "blacklist users");
        if(!from.isContract() && amount > _balances[from].mul(99).div(100)) {
            amount = amount.sub(1);
        }
        ...
        if(from == pair && block.timestamp.sub(timeOfLiquidityAdded) <= 12) {
    ...
    function swapAndDividend(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 balancesToLPHolders = balances.mul(feeToLPDividend).div(feeToLPDividend.add(feeToMarketing));
        dividendToLPHolders(balancesToLPHolders);
    ...
    function dividendToLPHolders(uint256 amount) private {
        ...
        for(uint256 i = 0; i < lpHolders.length; i++){
            ...
            if(LPAmount > 0) {
                ...
                IBEP20(USDT).transferFrom(address(_usdtReceiver), cur, difidendAmount);
            }
        }
    }
    ...
    function swapTokensForUSDT(uint256 tokenAmount) private {
        ...
        _approve(address(this), address(pancakeRouterAddr), tokenAmount);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if(takeFee) {
            if(feeToLPDividend > 0) {
                uint256 feeToLPDividendAmount = amount.mul(feeToLPDividend).div(100);
                _balances[address(this)] = _balances[address(this)].add(feeToLPDividendAmount);
                emit Transfer(sender, address(this), feeToLPDividendAmount); 
                totalFee = totalFee.add(feeToLPDividendAmount);
            }
            if(feeToMarketing > 0) {
                uint256 feeToMarketingAmount = amount.mul(feeToMarketing).div(100);
                _balances[address(this)] = _balances[address(this)].add(feeToMarketingAmount);
                emit Transfer(sender, address(this), feeToMarketingAmount); 
                totalFee = totalFee.add(feeToMarketingAmount);
            }
        }
    ...
    function setMinAmountToSwap(uint256 value) external onlyOwner() { 
        minAmountToSwap = value;
    }
    function setFeeToMarketing(uint256 value) external onlyOwner() { 
        feeToMarketing = value;
    }
    function setFeeToLPDividend(uint256 value) external onlyOwner() { 
        feeToLPDividend = value;
    }
