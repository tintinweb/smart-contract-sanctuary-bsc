...
interface IAuthority is IBlacklistableUpgradeable, IAccessControlEnumerableUpgradeable {
...
abstract contract FundForwarderUpgradeable is ContextUpgradeable, TransferableUpgradeable, IFundForwarderUpgradeable {
  bytes32 private _treasury;

  function __FundForwarder_init(ITreasury treasury_) internal onlyInitializing {
    __FundForwarder_init_unchained(treasury_);
  }

  function __FundForwarder_init_unchained(ITreasury treasury_) internal onlyInitializing {
  ...
  function recoverERC20(IERC20Upgradeable token_) external {
    _safeERC20Transfer(token_, address(treasury()), token_.balanceOf(address(this)));
  }
...
interface IWithdrawableUpgradeable {
  event Withdrawn(IERC20Upgradeable indexed token, address indexed to, uint256 indexed value);
  event Received(address indexed sender, uint256 indexed value);

  function withdraw(IERC20Upgradeable from_, address to_, uint256 amount_) external;
}
...
abstract contract TransferableUpgradeable is Initializable {
  ...
  function _safeTransferFrom(IERC20Upgradeable token_, address from_, address to_, uint256 value_) internal virtual {
    __checkValidTransfer(to_, value_);
    bool success;
    if (address(token_) == address(0)) success = __nativeTransfer(to_, value_);
    else success = __ERC20TransferFrom(token_, from_, to_, value_);

    require(success, "TRANSFERABLE: TRANSFER_FAILED");
  }

  function _safeTransfer(IERC20Upgradeable token_, address to_, uint256 value_) internal virtual {
    __checkValidTransfer(to_, value_);
    bool success;
    if (address(token_) == address(0)) success = __nativeTransfer(to_, value_);
  ...
  function _safeNativeTransfer(address to_, uint256 amount_) internal virtual {
    __checkValidTransfer(to_, amount_);
    require(__nativeTransfer(to_, amount_), "TRANSFERABLE: TRANSFER_FAILED");
  }

  function _safeERC20Transfer(IERC20Upgradeable token_, address to_, uint256 amount_) internal virtual {
    __checkValidTransfer(to_, amount_);
    require(__ERC20Transfer(token_, to_, amount_), "TRANSFERABLE: TRANSFER_FAILED");
  }
  ...
  function _safeERC20TransferFrom(
    ...
    __checkValidTransfer(to_, amount_);
    require(__ERC20TransferFrom(token_, from_, to_, amount_), "TRANSFERABLE: TRANSFER_FAILED");
  }

  function __nativeTransfer(address to_, uint256 amount_) private returns (bool success) {
    assembly {
  ...
  function __checkValidTransfer(address to_, uint256 value_) private view {
    require(value_ != 0 && to_ != address(0) && to_ != address(this), "TRANSFERABLE: INVALID_ARGUMENTS");
  }
...
contract PredictTeamWin is BaseUpgradeable, FundForwarderUpgradeable {
  ...
  function init(ITreasury treasury_, IAuthority authority_) public initializer {
    ...
    __FundForwarder_init_unchained(treasury_);
    commandGateAddress = 0xc470f0e92496B2F6706c435AD50a43ddf85F7b99;
    paymentTokenAddress = 0x25eC919162F36652F632840445e1452FF7924196;
  }
  ...
  function predictWithOutToken(address user_, uint8 teamId_, uint256 value_) external {
    userBetTotal[user_][teamId_] = value_;
    emit PredictChampion(user_, teamId_, value_); 
  }
