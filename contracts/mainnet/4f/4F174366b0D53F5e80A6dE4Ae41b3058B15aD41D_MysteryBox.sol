...
interface IMintImplement {
    ...
    function airdrop(address to, uint256 tokenId, bytes calldata data) external;
}
...
contract MysteryBox is ERC721EnumerableUpgradeable, SafeOwnable, Verifier, ReentrancyGuardUpgradeable {
    using StringsUpgradeable for uint256;
    ...
    function verifyMintParam(address account, uint128 period, uint128 round, uint256[] calldata tokenIds, address nftAddress, uint8 v, bytes32 r, bytes32 s) internal view returns (bool){
    ...
    function verifyOpenBlindBoxParam(address account, uint256 tokenId, uint256 beastTokenId, address nftAddress, uint8 v, bytes32 r, bytes32 s) internal view returns (bool){
        bytes32 hash = keccak256(abi.encodePacked(account, "_", tokenId, "_", beastTokenId, "_", block.chainid, "_", nftAddress));
        bytes32 cHash = convertHash(hash);
        return verifier == cHash.recover(v, r, s);
    }

    function verifyAirdropParam(address account, uint256 tokenId, bytes calldata data, address nftAddress, uint8 v, bytes32 r, bytes32 s) internal view returns (bool){
        bytes32 dHash = keccak256(data);
        bytes32 hash = keccak256(abi.encodePacked(account, "_", tokenId, "_", dHash, "_", block.chainid, "_", nftAddress));
        bytes32 cHash = convertHash(hash);
        return verifier == cHash.recover(v, r, s);
    }
    ...
    function airdrop(address to, uint256 tokenId, bytes calldata data, uint8 v, bytes32 r, bytes32 s) external {
        require(verifyAirdropParam(to, tokenId, data, address(this), v, r, s), "Airdrop: access verify failed");

        mintImplement.airdrop(to, tokenId, data);
    }
