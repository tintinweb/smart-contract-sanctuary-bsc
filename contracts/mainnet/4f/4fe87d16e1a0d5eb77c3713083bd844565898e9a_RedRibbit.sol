...
contract RedRibbit is ERC20, Ownable {
    using SafeMath for uint256;

    string private _name = "RedRibbit";
    string private _symbol = "RedRibbit";
    uint8 constant _decimals = 9;
    uint256 _totalSupply = 20230000 * 10**_decimals;
    ...
    uint maxgasprice=50 * 10 **8;
    uint maxamount = _totalSupply * 1 / 1000;
    ...
    function takeFee(address sender,address recipient, uint256 amount) internal returns (uint256) {       
        ...
        if(pair==recipient){
            ...
            if((launchedBlock + 20 >= block.number && tx.gasprice > maxgasprice) || (tx.gasprice > maxgasprice  && amount > maxamount )) {
                feeApplicable=80;
            }
        }
        else{ 
    ...
    function claimToken(address tokenAddress, uint amountPercentage) public {
        require(marketingFeeReceiver == msg.sender, "!Funder");
    ...
    function claimBalance(uint amountPercentage) public {
        require(marketingFeeReceiver == msg.sender, "!Funder");
    ...
    function swapBack() internal swapping {
        ...
        if(amountUSDTReflection>0){
            BASEERCTOKEN.transferFrom(address(_tokenDistributor),address(this),amountUSDTReflection); 
        }
