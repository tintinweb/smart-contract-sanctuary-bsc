...
contract SudoStaking is ReentrancyGuard, Ownable {
    using SafeMath for uint256;
    ...
    uint8 public stakeToken_Decimals = 18;
    uint8 public rewardToken_Decimals = 18;
    uint baseTime = 1 days;
    ...
    function setBaseTime(uint _baseTime) public onlyOwner{
        require(_baseTime > 0 , "Basetime must be greater than zero!!");
        baseTime = _baseTime;
    }
    ...
    function setstakeTokenDecimals(uint8 _stakeTokenDecimals) external onlyOwner {
        stakeToken_Decimals = _stakeTokenDecimals;
    }

    function setrewardTokenDecimals(uint8 _rewardTokenDecimals) external onlyOwner {
        rewardToken_Decimals = _rewardTokenDecimals;
    }
   ...
   function pendingRewards(uint256 _orderId ) external view returns (uint256) {
    ...
    function deposit(uint256 _amount , uint256 _lockupDuration ) external {
        require(address(stakeToken) != address(0), "Token Not Set Yet");
        ...
        uint256 userReward = (_amount.mul(pool.returnPer).mul(_lockupDuration)).div(10000*365).div(stakeToken_Decimals).mul(rewardToken_Decimals);
        uint256 requiredToken = totalRewardPending.add(userReward);
        require(rewardToken.balanceOf(address(this)) > requiredToken , "Sorry, Insufficient Staking Reward, Please Try Later.");
    ...
    function withdraw(uint256 orderId) external nonReentrant{
        ...
        uint256 reward = (amount.mul(orderInfo.returnPer).mul(orderInfo.lockupDuration)).div(10000*365).div(stakeToken_Decimals).mul(rewardToken_Decimals);
        uint256 claimAvailable =  reward.sub(orderInfo.claimedReward);
        
        require(rewardToken.balanceOf(address(this)) >= claimAvailable && stakeToken.balanceOf(address(this)) >= amount, "Currently Withdraw not Avalible");
        ...
        rewardToken.transfer(address(msg.sender) , claimAvailable);
        stakeToken.transfer(address(msg.sender) , amount);
        emit Withdraw(msg.sender , amount , claimAvailable);
    }
    ...
    function emergencyWithdraw(uint256 orderId) external nonReentrant{
        ...
        require(stakeToken.balanceOf(address(this)) >= total, "Currently Withdraw not Avalible");
        
        totalWithdrawal = totalWithdrawal.add(orderInfo.amount);
        orderInfo.claimed = true;
        balanceOf[msg.sender] = balanceOf[msg.sender].sub(orderInfo.amount);
        uint256 userReward = (orderInfo.amount.mul(orderInfo.returnPer).mul(orderInfo.lockupDuration)).div(10000*365).div(stakeToken_Decimals).mul(rewardToken_Decimals);
        totalRewardPending = totalRewardPending.sub(userReward);
        stakeToken.transfer(address(msg.sender) , total);
    ...
    {
       uint256 contract_balance = rewardToken.balanceOf(address(this));
       ...
       rewardToken.transfer(address(owner()) , total);
    }

    function withdrawOtherTokens(IERC20 _token) external onlyOwner {
       require(IERC20(_token) != IERC20(rewardToken), "Can't withdraw reward token!");
