...
contract CFT is ERC20, Ownable, LockToken {
    ...
    address public  marketingWallet = 0xB0cBa790257AF0772fe7dc2A28cF0aeF71e59eFa;
    address public  treasuryWallet = 0x1037123a49F79Ff3a4f15dF34843a96F03897000;
    ...
    constructor() ERC20("Childrens Fund Token", "CFT") {
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFees[from] && !_isExcludedFromFees[to] && to==uniswapV2Pair) {
        	uint256 treasuryTokens = amount.mul(sellTreasuryFee).div(100);
            ...
            if (sellTreasuryFee>0) {
                super._transfer(from, treasuryWallet, treasuryTokens); 
            }
            ...
            amount = amount.sub((treasuryTokens).add(marketingTokens).add(burnTokens));
     ...
     function setMaxWalletToken(uint256 _maxToken) external onlyOwner {
          ...
          require(maxWalletToken >= totalSupply().div(1000), "value too low");
  	}

     function updateFees(uint256 _sellTreasuryFee, uint256 _sellMarketingFee, uint256 _sellBurnFee) public onlyOwner {
        require(_sellTreasuryFee.add(_sellMarketingFee).add(_sellBurnFee) <= 10, "Fees too high");
    ...
    function setTreasuryWallet(address _newAddress) public onlyOwner {
     ...
     function setMaxTxAmount(uint256 newAmount) public onlyOwner {
        ...
        require(maxTxAmount >= totalSupply().div(1000), "value too low");
    }
