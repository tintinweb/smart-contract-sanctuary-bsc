...
contract GOTAMA is Context, IERC20, Ownable {
    ...
    address payable public marketingAddress = payable(0xF5F12D8AEFbf8ccF1c2109e1f9376cBAcc58be05); // Marketing Address
    ...
    uint256 public maxBal =1000000000000000e9;
  ...
  function ExcludeLimitcheck(address _addr,bool _status) public onlyOwner() {
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair && to != address(uniswapV2Router) && !_isExcludedFromFee[to]) {
            ...
            if (block.timestamp == launchTime) {
             ...
             if(!Limtcheck[to]){ require(amount <= maxTX,"Tx Limit Exceed");
         }
         if(!Limtcheck[to]){ require(balanceOf(to).add(amount) <= maxBal,"Tx Limit Exceed");
         }
    ...
    function setmaxTX(uint256 amount) external {
     ...
     function setmaxBal(uint256 amount) external {
        require(msg.sender==owner());
        maxBal = amount;
    }
