...
contract ClearStaking is Ownable {
   ...
   reserveWallet = _reserveWallet;
    ...
    function _stake(
        ...
        require(_amount + user.total_invested <= maxDepositAmount, "Max limit of deposit reached");
    ...
    function unStake(uint8 _pid, uint256 _amount) external returns (bool) {
       ...
       if (block.timestamp.sub(user.depositTime) < pool.lockPeriodInDays.mul(1 days)) {
        ...
        safeTransferFrom(reserveWallet, msg.sender, _amount - penality);
...

       {
        ...
        safeTransferFrom(reserveWallet, msg.sender, _amount);
       }
