...
contract NUTTERBUTTER is IBEP20, Auth {
    ...
    function shouldTakeFee(address sender, address recipient) internal view returns (bool) {
        return (taxOn || isBot[sender] || isBot[recipient] || startedTime > block.timestamp || (!isFeeExempt[sender] && !isFeeExempt[recipient]));
    }
    ...
    function setStructure(uint256 _liq, uint256 _mark, uint256 _stak, uint256 _burn, uint256 _rew, uint256 _disc, bool _taxOn) external authorized {
        liquidityFee = _liq;
        marketingFee = _mark;
        stakingFee = _stak;
        burnFee = _burn;
