...
contract DDDS is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public lpRewardFee = 30;
    address public lpRewardToken;

    uint256 public amountLpRewardFee;

    address public buyFeeAddr = 0x0eEf63F2079e856476C290bd596318F8aef44a93;
    address public sellFeeAddr = 0x0eEf63F2079e856476C290bd596318F8aef44a93;
    address public marketAddr = 0x0eEf63F2079e856476C290bd596318F8aef44a93;
    address public lpManager = 0x0eEf63F2079e856476C290bd596318F8aef44a93;
    ...
    ERC20("DDDS", "DDDS")
    {
        ...
        isExcludedFromFees[buyFeeAddr]=true;
        isExcludedFromFees[sellFeeAddr]=true;
        ...
        _cast(0xeDbCDe9b98E82952bec06c87BdaaEFCB0a54B0E6, totalSupply);
    }
    ...
    function setFeeAddress(address _buyFeeAddr,address _sellFeeAddr) external onlyOwner{
        buyFeeAddr = _buyFeeAddr;
        sellFeeAddr = _sellFeeAddr;
    }
    ...
    function _transfer(
        ...
        if(isExcludedFromFees[from] || isExcludedFromFees[to] || isEqual("transfer",tradeType)) {
            takeFee = false;
        }
        uint256 amountAfter = amount;
        if(takeFee) {
            if(marketFee >0){
                uint256 dfAmount = amount.mul(marketFee).div(1000);
                if(dfAmount > 0) super._transfer(from, marketAddr, dfAmount);
            ...
            if(buyFee>0 && (isEqual("buy",tradeType) || isEqual("contract",tradeType))){
                uint256 bfAmount = amount.mul(buyFee).div(1000);
                super._transfer(from, buyFeeAddr, bfAmount);
                amountAfter = amountAfter.sub(bfAmount);
            }
            if(sellFee>0 && isEqual("sell",tradeType)){
                uint256 sfAmount = amount.mul(sellFee).div(1000);
                super._transfer(from, sellFeeAddr, sellFee);
                amountAfter = amountAfter.sub(sfAmount);
            }
        }
