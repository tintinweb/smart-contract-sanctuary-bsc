...
contract DividendDistributor is ReentrancyGuard {
    ...
    function setRewardToken(address newReward) external onlyToken {
        require(
            ...
            newReward == 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56 || // BUSD
            newReward == 0x55d398326f99059fF775485246999027B3197955 || // USDT
...
contract AutoReward is ERC20, Ownable {
    IDexRouter public constant ROUTER = IDexRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E);
    DividendDistributor public immutable distributor;
    address public immutable pair;
    
    address public adminWallet;
    address public ibagWallet;
    address public presaleAddress;

    address public lastEligibleBuyer;
    uint256 public lastBuyBlock;
    uint256 public incentiveMinBuy = 10**17;
    uint256 public incentiveBlockLimit = 200;
    uint256 public incentiveHardCap = 77 ether;
    uint256 public incentiveHardCapDenominator = 2;

    address public previousWinner;
    uint256 public previousIncentive;
    uint256 public totalIncentivePaid;

    uint256 public buyTax = 1400;
    uint256 public sellTax = 1400;
    uint256 public transferTax = 0;
    uint256 public incentiveShare = 140;
    uint256 public rewardShare = 430;
    uint256 public liquidityShare = 70;
    uint256 public ibagShare = 360;
    ...
    event DepositIbag(address account, uint256 amount);
    event DepositRewards(uint256 amount);
    event AutoLiquidity(uint256 amountBNB, uint256 amountTokens);
    event AwardIncentive(address winner, uint256 incentive);
    ...
    event SetShares(uint256 liquidityShare, uint256 ibagShare, uint256 rewardShare, uint256 incentiveShare);
    event TriggerSwapBack(uint256 tokenAmount);
    event SetSwapBackSettings(bool enabled, bool swapAll, uint256 tokenAmount);
    event SetIncentiveSettings(uint256 minBuy, uint256 blockLimit, uint256 hardCap, uint256 hardCapDenominator);
    event SetTransferGas(uint256 transferGas);
    event SetAdminWallet(address adminWallet);
    event SetIbagWallet(address ibagWallet);
    ...
    constructor(address ibag) ERC20("AutoReward", "AUTO") {
        require(ibag != address(0), "New ibag wallet is the zero address");
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        // Incentive
        if (tradingEnabled && address(this).balance >= incentiveHardCap) {
            _handleIncentiveSizeLimit();
        }
        if (lastBuyBlock != 0 && block.number - lastBuyBlock >= incentiveBlockLimit) {
            _handleIncentive();
        }
        if (sender == pair && isEligibleForIncentive(amount)) {
    ...
    function _swapBack(uint256 tokenAmount) private swapping {
        ...
        uint256 amountBNBIbag = amountBNB * ibagShare / totalBNBShares;
        uint256 amountBNBRewards = amountBNB * rewardShare / totalBNBShares;

        (bool ibagSuccess,) = payable(ibagWallet).call{value: amountBNBIbag, gas: transferGas}("");
        if (ibagSuccess) { emit DepositIbag(ibagWallet, amountBNBIbag); }
    ...
    function isEligibleForIncentive(uint256 amount) public view returns (bool) {
        if (incentiveMinBuy == 0) { return true; }

        address[] memory path = new address[](2);
        path[0] = ROUTER.WETH();
        path[1] = address(this);

        uint256 tokensOut = ROUTER.getAmountsOut(incentiveMinBuy, path)[1];
        return amount >= tokensOut;
    }

    function _handleIncentiveSizeLimit() private {
        uint256 rewardBNB = address(this).balance / incentiveHardCapDenominator;
        try distributor.deposit{value: rewardBNB}() {
            emit DepositRewards(rewardBNB);
        } catch {}
    }

    function _handleIncentive() private {
        uint256 incentive = address(this).balance;
        address winner = lastEligibleBuyer;

        lastEligibleBuyer = address(0);
        lastBuyBlock = 0;

        totalIncentivePaid += incentive;
        previousIncentive = incentive;
        previousWinner = winner;

        (bool success,) = payable(winner).call{value: incentive, gas: transferGas}("");
        if (success) {
            emit AwardIncentive(winner, incentive);
        } else {
            // If incentive winner is smart contract that can't receive BNB, incentive is distributed manually.
            (bool ibagSuccess,) = payable(ibagWallet).call{value: incentive, gas: transferGas}("");
            if (ibagSuccess) { emit DepositIbag(ibagWallet, incentive); }
        }
    }
    ...
    function setShares(
        ...
        uint256 newIbagShare,
        uint256 newRewardShare,
        uint256 newIncentiveShare
    ) external onlyOwner {
        liquidityShare = newLiquidityShare;
        ibagShare = newIbagShare;
        rewardShare = newRewardShare;
        incentiveShare = newIncentiveShare;
        totalShares = liquidityShare + ibagShare + rewardShare + incentiveShare;
        require(totalShares > 0, "totalShares must be positive number");
        emit SetShares(liquidityShare, ibagShare, rewardShare, incentiveShare);
    }
    ...
    function setIncentiveSettings(
        ...
        incentiveMinBuy = minBuy;
        incentiveBlockLimit = blockLimit;
        incentiveHardCap = hardCap;
        incentiveHardCapDenominator = hardCapDenominator;
        emit SetIncentiveSettings(minBuy, blockLimit, hardCap, hardCapDenominator);
    }
    ...
    function setIbagWallet(address newWallet) external onlyOwner {
        require(newWallet != address(0), "New ibag wallet is the zero address");
        ibagWallet = newWallet;
        emit SetIbagWallet(ibagWallet);
    }
}