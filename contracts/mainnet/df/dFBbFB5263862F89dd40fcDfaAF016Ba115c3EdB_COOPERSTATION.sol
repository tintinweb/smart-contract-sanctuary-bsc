...
contract COOPERSTATION is Context, IERC20, Ownable {
    ...
    string private constant _name = "Cooper Station";
    string private constant _symbol = "COOPER";

    address public Cooper_Marketing_Wallet = 0x93B2f9595D21a17a00FAA89e0B0c8b0dE7166666;
    address private Cooper_Team_Wallet = 0x7f3658bf2D80Dd3DFaCb32C069dfD2Ed13637736;

    // Supply Holder Configuration
    uint256 public Cooper_max_WalletLimit = 20000000000 * 10**9; //2%
    uint256 public swapTokensAtAmount = 3000000000 * 10**9; //0.5%
    uint256 public Cooper_max_BuyLimit = 10000000000 * 10**9;
    uint256 public Cooper_max_SellLimit = 10000000000 * 10**9;
    ...
    Taxes public taxes = Taxes(0, 24, 0, 1);
    Taxes public sellTaxes = Taxes(0, 24, 0, 1);
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[Cooper_Marketing_Wallet] = true;
        _isExcludedFromFee[Cooper_Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[Cooper_Marketing_Wallet] = true;
        allowedTransfer[Cooper_Team_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= Cooper_max_BuyLimit, "You are exceeding Cooper_max_BuyLimit");
            require(balanceOf(to) + amount <= Cooper_max_WalletLimit, "You are exceeding Cooper_max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= Cooper_max_SellLimit, "You are exceeding Cooper_max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= Cooper_max_WalletLimit, "You are exceeding Cooper_max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(Cooper_Marketing_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(Cooper_Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function Cooper_Update_MarketingWallet(address newWallet) external onlyOwner{
        Cooper_Marketing_Wallet = newWallet;
    }
    
    function Cooper_Update_TeamWallet(address newWallet) external onlyOwner{
        Cooper_Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        Cooper_max_BuyLimit = maxBuy * 10**decimals();
        Cooper_max_SellLimit = maxSell * 10**decimals();
    }
    
    function updateCooper_Max_Walletlimit(uint256 amount) external onlyOwner{
        Cooper_max_WalletLimit = amount * 10**decimals();
    }
