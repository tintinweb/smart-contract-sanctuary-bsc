...
contract GymAccountant is IAccountant, OwnableUpgradeable {
    ...
    modifier onlyRelatedContracts() {
        require(msg.sender == gymMLMContractAddress, "GymAccountant:: Only related accounts can call the method");
        _;
    }
    ...
    function claimPendingRewards(uint32 _type) external {
        ...
        uint256 _pendingRewards = IGymMLM(gymMLMContractAddress).getPendingRewards(msg.sender, _type);
     ...
     */
    function updateBorrowedAmount(address _userAddress, uint256 _amount, bool _increase, uint32 _type) public onlyRelatedContracts {
     ...
     */
    function getUserBorrowedAmount(address _userAddress, uint32 _type) public view onlyRelatedContracts returns (uint256 amount) {
        if (_type == 1) {
     ...
     */
    function _claimPendingRewards(address _userAddress, uint256 _pendingRewards, uint32 _type) private {
     ...
     */
    function _updateBorrowedAmount(address _userAddress, uint256 _amount, bool _increase, uint32 _type) private {
        if (_increase) {
            if (_type == 1) {
        ...
        uint256 returnAmount = _type == 1 ? borrowedAmount[_userAddress] : borrowedAmountByType[_type][_userAddress];
