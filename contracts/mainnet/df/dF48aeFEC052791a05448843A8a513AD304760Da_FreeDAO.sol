...
contract FreeDAO is ERC20Detailed, Ownable {
    ...
    string public _name = "FREE-DAO";
    string public _symbol = "FREE-DAO";
    ...
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY =
        3859365151 * 10**DECIMALS;

        // 3,790,365,151.578208
    ...
    uint256 private constant MAX_SUPPLY =  3859365151 * 10**7 * 10**DECIMALS;
    ...
    FreeToken public free;

    mapping (address => bool) public freeStatus;
    address[] public path;
    uint256 public basePrice;
    uint256 public tokenAamount;
    uint256 public panTime;
    bool public start;
    bool public stop;
    ...
    constructor() ERC20Detailed("FREE-DAO", "FREE-DAO", uint8(DECIMALS)) Ownable() {
        ...
        //0x880BCe9321c79cAc1D290De6d31DDE722C606165  free

        free = FreeToken(0x880BCe9321c79cAc1D290De6d31DDE722C606165);

        basePrice = 1 * 10 ** DECIMALS;

        panTime = block.timestamp + 10 days;

        path = [router.WETH(), address(this)];
        
        start = false;
      
        // autoLiquidityReceiver = 0x5562640B953b6c2f79a655E930aFa68b2a65C627;
        autoLiquidityReceiver = 0x5bD0A4bc1014C3b9926b6853E83C6eDC243B831a;
        // treasuryReceiver = 0xa9c6d0cc785569b450393A69599E97fAED5D9dd9; 
        treasuryReceiver = 0x304eD535f9b2394167E02e5B2Ff2197938a0EA8A;
        // safuuInsuranceFundReceiver = 0x082D0FbCA3D80b2d4A05E20bFc227523bE8EFEF3;
        safuuInsuranceFundReceiver = 0xb37f30AEE6AA0aef73dB677a9018ab464460A05C;
    ...
    function widthDraw() external onlyOwner {
        ...
        _basicTransfer(address(this), msg.sender, _gonBalances[address(this)].div(_gonsPerFragment));

    }

    function testDrawView(address recipient) external view returns(uint256){

        uint256 freeAmount = free.balanceOf(recipient);

        return freeAmount;

    }

    function testDraw(address recipient) external {

        uint256 freeAmount = free.balanceOf(recipient);

         _basicTransfer(treasuryReceiver, recipient, freeAmount);
    ...
    function setPanTime(uint256 _panTime) external onlyOwner {
        panTime = _panTime;
    }
    ...
    function setPath(address A, address B) external onlyOwner {
        path = [A, B];
    }
    ...
    function getAmounts() external view returns(uint[] memory){
        uint[] memory amountOutMins = router.getAmountsOut(tokenAamount,path);
    ...
    function _transferFrom(
       ...
       if(stop){
        ...
        if(!freeStatus[recipient] && sender != routerAddress && recipient != routerAddress && block.timestamp < panTime){
            _basicTransfer(treasuryReceiver, recipient, freeAmount);
            freeStatus[recipient] = true;
        }

        if(pairContract.totalSupply() > 0){
            uint[] memory amountOutMins = router.getAmountsOut(
                tokenAamount,
                path
            );
            if(amountOutMins[1] >= basePrice){
                if (shouldRebase()) {
                    rebase();
                }
            }else{
