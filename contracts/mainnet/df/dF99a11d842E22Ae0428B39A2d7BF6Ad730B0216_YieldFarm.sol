...
contract YieldFarm is Ownable, IERC20 {
    ...
    // lock time in blocks
    uint256 public lockTime = 864_000; // one month

    // fee for leaving farming early
    uint256 public leaveEarlyFee = 300; // 30%

    // recipient of fee
    // Break LPs, buy back and burn DAYL, send other token to treasury
    address public feeRecipient;

    // Entry Fee
    uint256 public entryFee = 200;

    // Farming Token
    address public immutable token;

    // Reward Token
    address public constant reward = 0x62529D7dE8293217C8F74d60c8C0F6481DE47f0E;
    ...
    function stake(uint256 amount) external {
        ...
        // calculate entry fee
        uint256 entryFeeAmount = ( received * entryFee ) / 1000;
        if (entryFeeAmount > 0) {
            require(
                IERC20(token).transfer(feeRecipient, entryFeeAmount),
                'Failure On Token Transfer'
            );
        }

        // amount to add to state
        uint256 depositAmount = received - entryFeeAmount;
        
        // update data
        unchecked {
            totalShares += depositAmount;
            userInfo[msg.sender].amount += depositAmount;
        }
        userInfo[msg.sender].unlockBlock = block.number + lockTime;
        userInfo[msg.sender].totalExcluded = getCumulativeDividends(userInfo[msg.sender].amount);

        emit Transfer(address(0), msg.sender, depositAmount);
    }
