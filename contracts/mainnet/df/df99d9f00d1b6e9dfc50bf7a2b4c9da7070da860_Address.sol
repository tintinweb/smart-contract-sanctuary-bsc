...
// File: contracts/ONUS-launchpad-v2.sol
...
contract ONUSLaunchpad is Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 _projectId, uint256 _amount) external nonReentrant {
        ...
        require(
            block.timestamp <= projectAtId[_projectId].depositCondition.endTimestampDeposit,
            "Deposit time is ended"
        );
        require(
            userStakeRequirementAtId[_projectId][msg.sender]
            >=projectAtId[_projectId].minStakedToken,
            "Staked token amount do not meet requirement"
        );
        require(
            _amount.add(userDepositAtId[_projectId][msg.sender]) >= projectAtId[_projectId].minCurrencyPerUser,
            "User amount below minimum"
        );
        require(
            _amount.add(userDepositAtId[_projectId][msg.sender]) <= projectAtId[_projectId].maxCurrencyPerUser,
            "User amount above maximum"
        );

        if (_amount > 0) {
            userDepositAtId[_projectId][msg.sender] += _amount;
            projectAtId[_projectId].currencyToken.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    function withdraw(uint256 _projectId, uint256 _amount) external nonReentrant {
        ...
        require(
            _amount.sub(userDepositAtId[_projectId][msg.sender]) >= projectAtId[_projectId].minCurrencyPerUser,
            "User amount below minimum"
        );

        if (_amount > 0) {
            userDepositAtId[_projectId][msg.sender] -= _amount;
            projectAtId[_projectId].currencyToken.safeTransfer(msg.sender, _amount);
            projectAtId[_projectId].totalUserDeposit -= _amount;
        }
    ...
    function claimTGE(uint256 _projectId) external nonReentrant {
        require(
            projectAtId[_projectId].isPausedProject != true,
            "This project is paused!"
        );
        require(
            userStakeRequirementAtId[_projectId][msg.sender] >= projectAtId[_projectId].minStakedToken,
            "You did not stake enough to buy"
        );
        require(
        ...
        require(
            block.timestamp >= projectAtId[_projectId].tgeCondition.startTimestampTGE,
            "TGE not release"
        );

        // calc token allocation
        uint256 percentageAllocation = (userDepositAtId[_projectId][msg.sender].mul(10**18).div(projectAtId[_projectId].totalUserDeposit)).min(
            userDepositAtId[_projectId][msg.sender].div(projectAtId[_projectId].totalSaleToken.mul(projectAtId[_projectId].pricePerSaleToken).div(10**36))
        );
        uint256 userRemaining = userDepositAtId[_projectId][msg.sender].sub(percentageAllocation.mul(projectAtId[_projectId].totalSaleToken.mul(projectAtId[_projectId].pricePerSaleToken).div(10**36)));

        projectAtId[_projectId].stakedToken.safeTransfer(msg.sender, userStakeRequirementAtId[_projectId][msg.sender]);

        if (userRemaining > 0) {
            projectAtId[_projectId].currencyToken.safeTransfer(msg.sender, userRemaining);
        }
        // calc token can claim TGE
        uint256 amountUser = percentageAllocation.mul(projectAtId[_projectId].totalSaleToken).div(10**18);
        uint256 amountClaimTGE = amountUser.mul(projectAtId[_projectId].tgeCondition.releaseTGE).div(projectAtId[_projectId].PRECISION_FACTOR);

        projectAtId[_projectId].saleToken.safeTransfer(msg.sender, amountClaimTGE);
        // Set userIsClaimTGE
    ...
    function claimStage(uint256 _projectId, uint256 _stageIndex) external nonReentrant {
        require(projectAtId[_projectId].isPausedProject != true, "This contract is pause!");
        ...
        uint256 amountUser = percentageAllocation.mul(projectAtId[_projectId].totalSaleToken).div(10**18);
        uint256 amountClaimByStage = amountUser.mul(stage.stageRelease).div(projectAtId[_projectId].PRECISION_FACTOR);

        projectAtId[_projectId].saleToken.safeTransfer(msg.sender, amountClaimByStage);
    ...
    function setTGE(uint256 _projectId, TgeCondition calldata _tgeCondition) external onlyOwner {
        ...
        require(
            _tgeCondition.startTimestampTGE > projectAtId[_projectId].depositCondition.endTimestampDeposit,
        ...
        require(
            totalPercentage.add(_tgeCondition.releaseTGE) == projectAtId[_projectId].PRECISION_FACTOR,
