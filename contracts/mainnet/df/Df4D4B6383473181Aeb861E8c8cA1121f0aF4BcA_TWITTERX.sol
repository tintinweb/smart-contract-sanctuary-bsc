...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20  hurricane = IBEP20(0xF33b6B3DD26EE0B7c0Ba1Ce3239A1Ce15B94934C);
    ...
    function deposit() external payable override onlyToken {
        uint256 balanceBefore = hurricane .balanceOf(address(this));

        address[] memory path = new address[](2);
        path[0] = WBNB;
        path[1] = address(hurricane);
        ...
        uint256 amount = hurricane.balanceOf(address(this)).sub(balanceBefore);
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            hurricane.transfer(shareholder, amount);
...
contract TWITTERX is IBEP20, Auth {
    ...
    string constant _name = "TWITTERX";
    string constant _symbol = "TWITTER-X";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 100000000 * 10**0 * (10 ** _decimals);

    uint256 public _maxBuyTxAmount = _totalSupply * 30 / 10000; 
    uint256 public _maxSellTxAmount = _totalSupply * 30 / 10000; 
