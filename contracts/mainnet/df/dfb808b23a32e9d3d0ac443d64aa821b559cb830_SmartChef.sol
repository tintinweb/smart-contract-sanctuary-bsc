...
contract SmartChef is Ownable {
    ...
    function deposit(uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accHandPerShare).div(1e12).sub(offset);
    ...
    function withdraw(uint256 _amount) public {
        ...
        if(pending > 0) {
            require(handIds.length >= pending, "harvest: not enough Hands. emergencyWithdraw tokens!");
