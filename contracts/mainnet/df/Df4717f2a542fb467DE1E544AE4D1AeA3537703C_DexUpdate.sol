...
contract DexUpdate is Ownable, DexStorage, SecondDexStorage {
     ...
     event WithdrawNFT(uint256 tokenId, address collection, address to, bool onSale);
 ...
 function withdrawNFT(address to, uint256[] calldata tokenId, address collection) external {
    require(msg.sender == nonCryptoNFTVault, "not authorized");

    bool valid = true;
   
    for(uint256 i = 0; i< tokenId.length; i++){
        uint256 _tokenId = tokenId[i];
       if(INFT(collection).ownerOf(_tokenId) == address(this)){
           if(_saleTokens[_tokenId][collection].seller != msg.sender){
               valid = false;
               break;
           }
           delete _saleTokens[_tokenId][collection];
           INFT(collection).transferFrom(address(this), to, _tokenId);
         emit WithdrawNFT(_tokenId, collection, to, true);
       } else {
           if( INFT(collection).ownerOf(_tokenId) != nonCryptoNFTVault){
           valid = false;
           break;
        }
         INFT(collection).transferFrom(msg.sender, to, _tokenId);
         emit WithdrawNFT(_tokenId, collection, to, false);
       }

       
        delete _nonCryptoOwners[_tokenId][collection];
    }
    require(valid, "no owner");
