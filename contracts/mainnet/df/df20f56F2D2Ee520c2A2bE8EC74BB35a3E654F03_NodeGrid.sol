...
contract NodeGrid is ERC20Upgradeable {
    // using SafeMath for uint256;
    function initialize() public initializer {
        __ERC20_init("NodeGrid", "NGRID");
        ...
        if(address(uniswapV2Router)!=address(0)) {
            if(address(uniswapV2Pair)!=address(0)) {
                ...
                if(priceTimestampLast==0)  {
                    priceTimestampLast = blockTimestamp;
                }
                uint32 timeElapsed = blockTimestamp - priceTimestampLast;
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual override {
        ...
        if (!isExcludedFromFee[from] && !isExcludedFromFee[to]) {
            ...
            if(taxAmount>0) {
                uint256 operatorFeeAmount = taxAmount * operatorFee / 100;
        ...
        _update();
        // console.log("price", getAmountOut(1 ether));
    }
    ...
    function getAmountOut(uint256 _amount) public view returns (uint256) {
        ...
        if(timeElapsed > 0) {
            IUniswapV2Pair pair = IUniswapV2Pair(uniswapV2Pair);
            uint256 priceCumulative = pair.token0()==address(this) ? pair.price0CumulativeLast() : pair.price1CumulativeLast();
            (uint256 reserve0, uint256 reserve1, uint32 blockTimestamp) = pair.getReserves();
            if(pair.token1()==address(this))
                (reserve0, reserve1) = (reserve1, reserve0);
            priceCumulative += (reserve1 << 112) * (block.timestamp - blockTimestamp) / reserve0;
            uint256 priceAverageExtra = 0;
            if(PRICE_PERIOD > timeElapsed)
                priceAverageExtra = (priceAverage * (PRICE_PERIOD - timeElapsed) + priceCumulative - priceCumulativeLast) / PRICE_PERIOD;
            else
