...
contract BonkyBaby is Context, IBEP20, Ownable {
    ...
    uint256 private _tTotal = 1000000000000 * 10 ** 12;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    uint256 private maxWallet;

    string private _name = "Bonky Baby";
...
//Remove from Blacklist
    ...
    function setSellFees(uint taxFee, uint liq_fee) external onlyOwner {
        _taxFeeBuy       = taxFee;
        _liq_feeBuy = liq_fee;
    }

    function setBuyFees(uint taxFee, uint liq_fee) external onlyOwner {
        _taxFeeSell       = taxFee;
    ...
    function setLiquidityReceiver(uint256 teamFee, uint256 marketingFeeVariable) external onlyOwner {
