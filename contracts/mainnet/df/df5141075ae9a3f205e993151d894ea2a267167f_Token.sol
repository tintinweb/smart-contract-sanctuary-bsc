...
contract Token is ERC20, Ownable {
    ...
    uint256 private totalswapfee = liquidityFee+marketingFee+foundationFee*2;
    address public lpRewardToken; 
    ...
    address payable public foundationWalletAddress; 
    ...
    constructor(
        ...
        swapTokensMaxAmount = swapTokensAtAmount * 10;
        ...
        marketingWalletAddress = payable(marketingWalletAddr_);
        foundationWalletAddress = payable(foundationWalletAddress_);
    ...
    function setFoundationWallet(address addr) public onlyOwner {
        foundationWalletAddress = payable(addr);
    }
    ...
    function takeAllFee(address from,uint256 amount) private returns(uint256 amountAfter) {
        ...
        uint256 totalLiFee = MFee+FFee+FFee+LFee;
        AmountLiquidityFee += totalLiFee;


        uint256 LPFee = amount.mul(lpRewardFee.sub(2)).div(100);
        AmountLpRewardFee += LPFee;
        amountAfter = amountAfter.sub(LPFee).sub(FFee);
  ...
  /*  function swapAndLiquify(uint256 tokens) private {
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        uint256 BNBfee2 = totalswapfee*2-liquidityFee;
        
        uint256 amountLiquidity = amountReceived.div(BNBfee2);
        uint256 amountFoundation = amountReceived.mul(marketingFee).div(BNBfee2).mul(3).div(2);
        uint256 amountMarketing = amountReceived.mul(foundationFee).div(BNBfee2).mul(3).div(2);
        uint256 amount = amountReceived - amountLiquidity - amountFoundation - amountMarketing;

        if(amountFoundation > 0){
            transferToAddressETH(foundationWalletAddress, amountFoundation);
        }
        if(amountMarketing > 0){
            transferToAddressETH(marketingWalletAddress, amountMarketing);
        }
        if(amount > 0){
            transferToAddressETH(LPtokenAddress, amount);
        }
        if(amountLiquidity > 0 && tokensForLP > 0){
            addLiquidity(tokensForLP, amountLiquidity);
        }
        emit SwapAndLiquify(tokensForLP, amountLiquidity, tokensForLP);
    }
