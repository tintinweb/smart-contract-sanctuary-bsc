...
contract BlowJob is ERC20, Ownable {
    ...
    BlowJobDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Blow Job", "$BJ") {
        marketingWallet = payable(0x844B6B64f0Ea09A365076432d7c583610b677e0b);
        devWallet = payable(0x844B6B64f0Ea09A365076432d7c583610b677e0b);
        ...
        dividendTracker = new BlowJobDividendTracker(
            payable(this),
            router,
...
contract BlowJobDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    BlowJob public BlowJobContract;
    ...
    constructor(
        ...
        defaultToken = token;
        BlowJobContract = BlowJob(mainContract);
    ...
    function _transfer(
        ...
        require(false, "BlowJob_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "BlowJob_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BlowJob contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(BlowJobContract);

            uint256 prevBalance = BlowJobContract.balanceOf(address(this));
            ...
            {
                uint256 received = BlowJobContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    BlowJobContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
