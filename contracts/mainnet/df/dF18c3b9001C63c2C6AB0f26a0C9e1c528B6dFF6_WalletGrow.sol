...
contract WalletGrow is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("WalletGrow", "Wallet") {
        ...
        maxTransactionBuyAmount = totalSupply * 5 / 1000; // 1% maxTransactionAmountTxn
        maxTransactionSellAmount = totalSupply * 5 / 1000; // 1% maxTransactionAmountTxn
        ...
        _fees.buyTreasuryFee = 2;
        _fees.buyTotalFees = _fees.buyMarketingFee + _fees.buyLiquidityFee + _fees.buyTreasuryFee;

        _fees.sellMarketingFee = 6;
        _fees.sellLiquidityFee = 3;
        _fees.sellTreasuryFee = 2;
        _fees.sellTotalFees = _fees.sellMarketingFee + _fees.sellLiquidityFee + _fees.sellTreasuryFee;


        marketingWallet = address(0x19b8Cf9b94Aa24e8dB5D68b2aeeAe3FbA5Bc05Ec); // set as marketing wallet
        treasuryWallet = address(0xb980C4aC3d31507F0097a3a265E75b5431560392); // set as treasury wallet
    ...
    function updateMaxTxnAmount(uint256 newMaxBuyAmount, uint256 newMaxSellAmount) external onlyOwner {
        require((totalSupply() * newMaxBuyAmount) / 1000 >= ((totalSupply() * 15) / 100), "Cannot set maxTransactionAmounts higher than 15%");
        require((totalSupply() * newMaxSellAmount) / 1000 >= ((totalSupply() * 15) / 100), "Cannot set maxTransactionAmounts higher than 15%");
    ...
    function updateBuyFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _treasuryFee) external onlyOwner {
        ...
        _fees.buyTreasuryFee = _treasuryFee;
        _fees.buyTotalFees = _fees.buyMarketingFee + _fees.buyLiquidityFee + _fees.buyTreasuryFee;
    ...
    function updateSellFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _treasuryFee) external onlyOwner {
        ...
        _fees.sellTreasuryFee = _treasuryFee;
        _fees.sellTotalFees = _fees.sellMarketingFee + _fees.sellLiquidityFee + _fees.sellTreasuryFee;
        require(_fees.sellTotalFees <= 15, "Must keep fees at 15% or less");
    }

    function updateAntiSnipingFees(uint256 _marketingBuyFee, uint256 _liquidityBuyFee, uint256 _treasuryBuyFee, uint256 _marketingSellFee, uint256 _liquiditySellFee, uint256 _treasurySellFee) external onlyOwner{
        _firstFees.sellMarketingFee = _marketingSellFee;
        _firstFees.sellLiquidityFee = _liquiditySellFee;
        _firstFees.sellTreasuryFee = _treasurySellFee;
        _firstFees.sellTotalFees = _firstFees.sellMarketingFee + _firstFees.sellLiquidityFee + _firstFees.sellTreasuryFee;
        require(_firstFees.sellTotalFees <= 15, "Must keep fees at 15% or less");

        _fees.buyMarketingFee = _marketingBuyFee;
        _fees.buyLiquidityFee = _liquidityBuyFee;
        _fees.buyTreasuryFee = _treasuryBuyFee;
        _fees.buyTotalFees = _fees.buyMarketingFee + _fees.buyLiquidityFee + _fees.buyTreasuryFee;
        require(_fees.buyTotalFees <= 15, "Must keep fees at 15% or less");
    }
    ...
    function updateWallets(address newMarketingWallet, address newTreasuryWallet) external onlyOwner{
    ...
    function _transfer(
        ...
        if (takeFee) {
            // check if trade is in 30 minutes
            bool isAnti = (launchedAtTime + 1800) > block.timestamp;
            if (isAnti) {
                _storeFees = _fees;
                if ((launchedAtBlock + 4) >= block.number) {
                    // 4 block taxes
                    _fees.buyMarketingFee = 99;
                    _fees.buyLiquidityFee = 0;
                    _fees.buyTreasuryFee = 0;
                    _fees.buyTotalFees = _fees.buyMarketingFee + _fees.buyLiquidityFee + _fees.buyTreasuryFee;

                    _fees.sellMarketingFee = 99;
