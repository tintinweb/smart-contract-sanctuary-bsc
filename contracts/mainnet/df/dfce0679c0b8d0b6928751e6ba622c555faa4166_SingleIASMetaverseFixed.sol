...
// File: contracts\SingleIASMetaverseFixed.sol

// Single IASMetaverse Fixed Price Marketplace contract
...
contract SingleIASMetaverseFixed is Ownable, ERC721Holder {
    using SafeMath for uint256;
	using EnumerableSet for EnumerableSet.AddressSet;
	...
	function createCollection(string memory _name, string memory _uri, bool bPublic) public returns(address collection) {
		bytes memory bytecode = type(SingleIASMetaverse).creationCode;
        ...
        ISingleIASMetaverse(collection).initialize(_name, _uri, msg.sender, bPublic);
    ...
    function singleList(address _collection, uint256 _tokenId, address _tokenAdr, uint256 _price) OnlyItemOwner(_collection,_tokenId) public {
		...
		ISingleIASMetaverse nft = ISingleIASMetaverse(_collection);        
    ...
    function singleDelist(uint256 _id) external {        
        ...
        ISingleIASMetaverse(pairs[_id].collection).safeTransferFrom(address(this), pairs[_id].owner, pairs[_id].tokenId);        
    ...
    function singleBuy(uint256 _id) external payable {
		...
		ISingleIASMetaverse(pairs[_id].collection).safeTransferFrom(address(this), msg.sender, pair.tokenId);
	...
	modifier OnlyItemOwner(address tokenAddress, uint256 tokenId){
        ISingleIASMetaverse tokenContract = ISingleIASMetaverse(tokenAddress);
