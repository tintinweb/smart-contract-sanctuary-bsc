...
contract DexTreasury is Ownable {
    ...
    address public immutable payoutUSDEX;
    address public immutable payoutDexSHARE;
    ...
    constructor(address _payoutUSDEX, address _payoutDexSHARE, address _initialOwner) {
        require( _payoutUSDEX != address(0) );
        payoutUSDEX = _payoutUSDEX;
        require( _payoutDexSHARE != address(0) );
        payoutDexSHARE = _payoutDexSHARE;
    ...
    function depositUSDEX(address _principleTokenAddress, uint _amountPrincipleToken, uint _amountPayoutUSDEX) external {
        ...
        IERC20(payoutUSDEX).safeTransfer(msg.sender, _amountPayoutUSDEX);
    }
    ...
    *   @return value_ uint
     */
    function valueOfUSDEX( address _principleTokenAddress, uint _amount ) public view returns ( uint value_ ) {
        // convert amount to match payout token decimals
        value_ = _amount.mul( 10 ** IERC20( payoutUSDEX ).decimals() ).div( 10 ** IERC20( _principleTokenAddress ).decimals() );
    }

    function valueOfDexSHARE( address _principleTokenAddress, uint _amount ) public view returns ( uint value_ ) {
        // convert amount to match payout token decimals
        value_ = _amount.mul( 10 ** IERC20( payoutDexSHARE ).decimals() ).div( 10 ** IERC20( _principleTokenAddress ).decimals() );
    }
