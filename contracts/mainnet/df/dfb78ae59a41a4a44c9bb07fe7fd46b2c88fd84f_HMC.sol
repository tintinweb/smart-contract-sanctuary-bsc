...
contract HMC is IERC20, Ownable {
    ...
    uint256 public _fundFee2;
    uint256 public _fundFee3;
    uint256 public _fundFee4;
    uint256 public _fundFee5;
    uint256 public _fundFee6;

    address private fundAddress = address(0x16A76fb87C017e4882BABe6f821B12E7BA3B79ed);
    address private fundAddress2 = address(0xd29adb16E3e4B06417EC9b6fd52bD4B9f20BC42b);
    address private fundAddress3 = address(0x71fa28727e6C0ea44d553b9A2798E7A8a32af44E);
    address private fundAddress4 = address(0x4680eacB5a20c4864555d6F9e51e9fA7D5c066D6);
    address private fundAddress5 = address(0x0914A1000E65a588a3d0911Dc2b827A49E88fD9F);
    address private fundAddress6 = address(0x2D71ebBa6F8464c8e09aF953CdF9A3BcDd614f9B);
        ...
        if (takeFee) {
            ...
            if (sender==uniswapV2Pair && isRoute[recipient]){
                ...
                _takeTransfer(
                    sender,
                    fundAddress2,
                    tAmount.div(1000).mul(_fundFee2.mul(multiple)),
                    currentRate
                );
                _takeTransfer(
                    sender,
                    fundAddress3,
                    tAmount.div(1000).mul(_fundFee3.mul(multiple)),
                    currentRate
                );
                _takeTransfer(
                    sender,
                    fundAddress4,
                    tAmount.div(1000).mul(_fundFee4.mul(multiple)),
                    currentRate
                );
                _takeTransfer(
                    sender,
                    fundAddress5,
                    tAmount.div(1000).mul(_fundFee5.mul(multiple)),
                    currentRate
                );
                ...
                if (sender!=uniswapV2Pair && isRoute[sender]){
                    ...
                    _takeTransfer(
                        sender,
                        fundAddress6,
                        tAmount.div(1000).mul(_fundFee6.mul(multiple)),
                        currentRate
                    );
            ...
            rate = _fundFee.mul(multiple) + _fundFee2.mul(multiple) + _fundFee3.mul(multiple) + _fundFee4.mul(multiple) + _fundFee5.mul(multiple) + _fundFee6.mul(multiple);
        }
