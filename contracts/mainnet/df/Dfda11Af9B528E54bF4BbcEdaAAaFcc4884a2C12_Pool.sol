...
contract Pool{
    ...
    function sendbnb(address  u,uint256 am) public  returns (bool success){
        require(msg.sender==owner);
        return address(uint160(u)).send(am);
     }

    function invest(uint256 am) public payable returns (bool success){
        return  token(tokenaddr).transferFrom(msg.sender,address(this),am);
    }
}