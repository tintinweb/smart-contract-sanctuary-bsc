...
contract Smoky_Inu_Token is ST20, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    string public _name = "Smoky Inu";
    string public _symbol = "SMOKY";
    ...
    uint256 public insuranceFee = 30;
    uint256 public blackholeFee = 30;
    uint256 public sellFee = 40;
    uint256 public totalFee = liquidityFee.add(treasuryFee).add(insuranceFee).add(blackholeFee);
    ...
    address public blackholeReceiver;
    ...
    constructor() ST20("Smoky Inu", "SMOKY", uint8(DECIMALS)) Ownable() {
        router = IPancakeSwapRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E); 
        pair = IPancakeSwapFactory(router.factory()).createPair(router.WETH(), address(this));
        liquidityReceiver = 0x831372691160F5987a17Cfb98b8ed533e77b9159;
        treasuryReceiver = 0x4EaE5aCA23b325B5e0D9d43786316d33444C0389; 
        insuranceReceiver = 0x0A08c5405862B118fa0cC4eAc69302F30b4C123b;
        blackholeReceiver = 0x000000000000000000000000000000000000dEaD;
    ...
    function takeFee(address sender, address recipient, uint256 gonAmount) internal returns (uint256) {
        ...
        _gonBalances[blackholeReceiver] = _gonBalances[blackholeReceiver].add(gonAmount.div(feeDenominator).mul(blackholeFee));
        _gonBalances[address(this)] = _gonBalances[address(this)].add(gonAmount.div(feeDenominator).mul(_treasuryFee.add(insuranceFee)));
        _gonBalances[liquidityReceiver] = _gonBalances[liquidityReceiver].add(gonAmount.div(feeDenominator).mul(liquidityFee));
        emit Transfer(sender, address(this), feeAmount.div(_gonsPerFragment));
        return gonAmount.sub(feeAmount);
    }
    function addLiquidity() internal swapping {
        uint256 autoLiquidityAmount = _gonBalances[liquidityReceiver].div(_gonsPerFragment);
        _gonBalances[address(this)] = _gonBalances[address(this)].add(_gonBalances[liquidityReceiver]);
    ...
    function setFeeReceivers(address _liquidityReceiver, address _treasuryReceiver, address _insuranceReceiver, address _blackholeReceiver) external onlyOwner {
        liquidityReceiver = _liquidityReceiver;
        treasuryReceiver = _treasuryReceiver;
        insuranceReceiver = _insuranceReceiver;
        blackholeReceiver = _blackholeReceiver;
    }
