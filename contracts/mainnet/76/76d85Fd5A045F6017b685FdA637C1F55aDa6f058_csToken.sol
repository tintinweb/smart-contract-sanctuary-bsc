...
contract csToken is ERC20, SafeOwnable {
    using SafeMath for uint256;
    using Address for address;

    string private name_ = "cs9";
    string private symbol_ = "cs9";
    uint8 private decimals_ = 18;
    uint256 private totalSupply_ = 100000000 * 10**decimals_;

    uint256 private minimumTokensBeforeSwap = 1 * 10**decimals_;
    ...
    constructor() ERC20(name_, symbol_, totalSupply_) {
        _setupDecimals(decimals_);

        usdtToken = IERC20(USDT);

        IRouter _router = IRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        liquidity = IFactory(_router.factory()).createPair(USDT, address(this));
        isMarketPair[address(liquidity)] = true;
    ...
    function setMinSwapAndLiquifyLimit(uint256 min) external onlyOwner {
        ...
        minimumTokensBeforeSwap = min * 10**decimals_;
    }

    function setLiquidityRatio(uint256 marketing_, uint256 liquidity_)
        external
        onlyOwner
    {
    ...
    function setLiquidityFee(uint256 amo1, uint256 amo2) external onlyOwner {
        _buyFee = amo1;
        _sellFee = amo2;
    }
    ...
    function calculateFee(
        ...
        if (isMarketPair[from]) {
            //is buy
            liquidityFee = realAmount.mul(_buyFee).div(1000);
        } else if (isMarketPair[to]) {
            //is sell
            liquidityFee = realAmount.mul(_sellFee).div(1000);
        } else {
            //other
            liquidityFee = amount.mul(_buyFee).div(1000);
        }
    ...
    function swapAndLiquify() private lockTheSwap {
        ...
        uint256 feeAmount = USDTBalance.mul(_marketing).div(_total);
        if (feeAmount > 0) {
            transferToAddressUsdt(marketingWalletAddress, feeAmount);
        }
        uint256 usdtAmount = USDTBalance.sub(feeAmount);
        if (usdtAmount > 0) {
            transferToAddressUsdt(liquidityContractAddress, usdtAmount);
        }
    }
