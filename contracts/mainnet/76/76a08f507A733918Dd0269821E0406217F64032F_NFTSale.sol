...
contract NFTSale is ReentrancyGuard, Ownable {
    ...
    function getAmountOfAzwToPay(address buyer, uint256 _amountOfBox, uint8 roundId) public view returns(uint256){
        (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast) = azwPair.getReserves();
        if(roundWhiteList[roundId][buyer]){
            return _amountOfBox * ((_purchaseRounds[roundId].priceInBnb * _purchaseRounds[roundId].whitelistDiscountPercent / 100) / (reserve1 / reserve0));
        }
        else{
        ...
        roundAvailable(_roundId) preventContractCalls
    {
        ...
        if(useAzw){
            uint256 amountOfAzwToPay = getAmountOfAzwToPay(msg.sender,_mintAmount, _roundId);
