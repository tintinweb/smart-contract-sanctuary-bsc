...
contract SquidGame is ReentrancyGuard, Ownable, VRFConsumerBase {
    ...
    uint256 private constant MAX_UNIQUE_WINNERS = 5_000;
    ...
    uint256 private constant DISTRIBUTION_COOLDOWN = 48 hours;
    uint256 private constant ORACLE_PERIOD = 1 hours;
    uint256 private constant REIMBURSE_PERIOD = 2 hours;
    ...
    uint256 public constant TOP_WINNERS_REWARD_SHARE = 0.9e18; // 90%
    uint256 public constant DEVS_REWARD_SHARE = 0.1e18; // 10%
    ...
    event BetPlaced(bytes32 indexed betId, address indexed player, uint256 bnbAmount, BetOutcome bet);
    event BetResolved(bytes32 indexed betId, BetOutcome result);
    event RewardDistribuited(address indexed player, uint256 reward);
    ...
    function initialize(IOracle _oracle, address _devsWallet, uint256 _start, uint256 _end) public initializer {
        require(address(_oracle) != address(0), "SquidGame: Oracle is null");
    ...
    function placeBet(BetOutcome _bet) external payable nonReentrant onlyIfActive {
        ...
        require(
            _isWinnerExists(_player) || winners.length < MAX_UNIQUE_WINNERS,
    ...
    function reimburse(bytes32 _betId) external onlyOwner {
        Bet storage _bet = bets[_betId];
        require(_bet.player != address(0), "SquidGame: Invalid bet id");
        require(
            block.timestamp >= _bet.timestamp + REIMBURSE_PERIOD,
            "SquidGame: This bet is not elegible for reimbursment"
        );
        require(_bet.result == BetOutcome.PENDING, "SquidGame: This bet was resolved");

        _bet.result = BetOutcome.REIMBURSED;
        Address.sendValue(payable(_bet.player), _bet.bnbAmount);
        emit BetResolved(_betId, _bet.result);
    }
    ...
    function _collectChainlinkFeeInBnb() private returns (uint256 _chainlinkFeeInBnb) {
        _chainlinkFeeInBnb = oracle.consultLinkForBnb(CHAINLINK_FEE);
        Address.sendValue(payable(owner()), _chainlinkFeeInBnb);
    }
    ...
    function _resolveNone(Bet memory _bet) private {
        uint256 _toPlayer = _bet.bnbAmount / 2 - _collectChainlinkFeeInBnb(); // ~50%
    ...
    function _createOrUpdateWinner(address _address, uint256 _score) private {
        if (!_isWinnerExists(_address)) {
            ...
            uint256 _winnerId = winnerId[_address];
            winners[_winnerId] = Winner({
                player: _address,
                score: winners[_winnerId].score + _score,
                reward: 0,
                claimed: false
            });
        }
    ...
    function _resolveWin(Bet memory _bet) private {
        ...
        uint256 _paybackAmount = _bet.bnbAmount - _winnerTaxAmount - _collectChainlinkFeeInBnb();
    ...
    function _resolveLost(Bet memory _bet) private {
        uint256 _amountBought = _swapBnbForSquid(_bet.bnbAmount - _collectChainlinkFeeInBnb());
    ...
    function _quickSort(Winner[] memory arr, int256 left, int256 right) private view {
        int256 i = left;
        int256 j = right;
        if (i == j) return;
    ...
    // Distribuite rewards
    // This function will use all SQM that its hold as pot
    // The pool will be split among 1) Top winners, 2) Devs
    ...
    function distribute() external nonReentrant onlyIfIsOver {
        require(
            _msgSender() == owner() || block.timestamp > end + DISTRIBUTION_COOLDOWN,
            "SquidGame: need to wait 48 hours cool down"
        );
        ...
        // 4. Split all BNB in 2 parts A) 90% to top winner B) 10% to devs
    ...
    function updateBnbForSquidSwapSlippage(uint256 _newSlippage) external onlyOwner {
        require(_newSlippage <= 1e18, "SquidGame: slippage too high");
        bnbForSqmSwapSlippage = _newSlippage;
    }

    function updateBnbForUsdtSwapSlippage(uint256 _newSlippage) external onlyOwner {
        require(_newSlippage <= 1e18, "SquidGame: slippage too high");
        bnbForUsdtSwapSlippage = _newSlippage;
    }

    function updateBnbForSqmMintSlippage(uint256 _newSlippage) external onlyOwner {
