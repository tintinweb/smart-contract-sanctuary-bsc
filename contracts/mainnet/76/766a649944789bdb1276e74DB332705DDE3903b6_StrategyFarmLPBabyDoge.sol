...
// INTERNAL FUNCTIONS
  ...
  function _outputBalance() internal override returns (uint256) {
    ...
    if (output == BABYDOGE) {
      uint256 coinFee = IBabyDogeCoin(output)._taxFee();
      coinFee+= IBabyDogeCoin(output)._liquidityFee();

      outputHalf = outputHalf * (MAX_BABYDOGE_FEE - coinFee) / MAX_BABYDOGE_FEE;
    }
...
// INTERNAL FUNCTIONS
    ...
    function _removeLiquidity(uint256 amount) internal returns (
