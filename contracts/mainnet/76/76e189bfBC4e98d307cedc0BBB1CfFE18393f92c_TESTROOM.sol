...
contract ShareWrapper {
    ...
    function withdraw(uint256 amount) public virtual {
        ...
        require(masonShare >= amount, "Urinal: withdraw request greater than staked amount");
...
contract TESTROOM is ShareWrapper, ContractGuard {
    using SafeERC20 for IERC20;
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "TESTROOM: caller is not the operator");
        _;
    }

    modifier masonExists {
        require(balanceOf(msg.sender) > 0, "TESTROOM: The mason does not exist");
        _;
    }
    ...
    modifier notInitialized {
        require(!initialized, "TESTROOM: already initialized");
        _;
    }
    ...
    function stake(uint256 amount) public override onlyOneBlock updateReward(msg.sender) {
        require(amount > 0, "TESTROOM: Cannot stake 0");
        super.stake(amount);
    ...
    function withdraw(uint256 amount) public override onlyOneBlock masonExists updateReward(msg.sender) {
        require(amount > 0, "TESTROOM: Cannot withdraw 0");
        require(masons[msg.sender].epochTimerStart.add(withdrawLockupEpochs) <= treasury.epoch(), "Urinal: still in withdraw lockup");
        claimReward();
    ...
    function claimReward() public updateReward(msg.sender) {
        ...
        if (reward > 0) {
            require(masons[msg.sender].epochTimerStart.add(rewardLockupEpochs) <= treasury.epoch(), "Urinal: still in reward lockup");
            masons[msg.sender].epochTimerStart = treasury.epoch(); // reset timer
    ...
    function allocateSeigniorage(uint256 amount) external onlyOneBlock onlyOperator {
        require(amount > 0, "TESTROOM: Cannot allocate 0");
        require(totalSupply() > 0, "TESTROOM: Cannot allocate when totalSupply is 0");
