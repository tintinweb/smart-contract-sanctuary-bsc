...
contract WorldStreamFinanceV2 is ERC20, Ownable {
    ...
    address public marketingWalletContratv1 = 0xEa960961c33E95A6E6985EDCe8bDDE05EBc4425c; // Contract V1 marketing wallet
    address public CEOWalletContratv1 = 0xBB625df1F46B6A6f4867EE156167a2005F0325DD; // Contract V1 CEO wallet
    address public CTOWallet1Contratv1 = 0xC6E39D58614a70C3Dbec5493F08204F9ECe2ab40; // Contract V1 CTO wallet
    address public CTOWallet2Contratv1 = 0xD20469C7bbe16F3eb33A4DC4aa26DC288d02e189; // Contract V1 CTO wallet
    ...
    constructor() ERC20("World Stream Finance V2", "Stream"){
        ...
        uint256 totalSupply = 100000000 * (10**3);
        
        maxWallet = totalSupply * 1;
        swapTokensAtAmount = 54323 * (10**3);
        ...
        */
        ...
        _mintDeploy(address(0), owner(), 100000000 * (10**3));
        _transferDeploy(owner(), marketingWalletContratv1, 2835466 * (10**3));
        _transferDeploy(owner(), CEOWalletContratv1, 2347161 * (10**3));
        _transferDeploy(owner(), CTOWallet1Contratv1, 1379965 * (10**3));
        _transferDeploy(owner(), CTOWallet2Contratv1, 1175176 * (10**3));
        _burnDeploy(marketingWalletContratv1, address(0), 2835466 * (10**3)); //burn tokens of marketing wallet
        _burnDeploy(CEOWalletContratv1, address(0), 2347161 * (10**3)); //burn tokens of CEO wallet
        _burnDeploy(CTOWallet1Contratv1, address(0), 1379965 * (10**3)); //burn tokens of CTO wallet 
        _burnDeploy(CTOWallet2Contratv1, address(0), 1175176 * (10**3)); //burn tokens of CTO wallet
        _burnDeploy(owner(), address(0), 64018163 * 10**3);
    }
    ...
    function updateFees(uint256 _marketingFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _buybackFee, uint256 _developmentFee, uint256 _stakingFee, uint256 _burnFee, uint256 _nftHolderFee, uint256 _feeDivisor) external onlyOwner {
        ...
        totalFees = marketingFee.add(rewardsFee).add(liquidityFee).add(buybackFee).add(developmentFee).add(burnFee).add(stakingFee).add(nftHolderFee);
    }
    ...
    function MigrateStreamV2 (address[] memory holder, uint256[] memory tokens, uint256 totalTokensToMigrate) external onlyOwner {
        
        emit Transfer(msg.sender, address(this), totalTokensToMigrate);
    ...
    function airdrop (address[] memory addresses, uint256[] memory tokens, uint256 totalAirdrop) external onlyOwner {
        ...
        emit Transfer(msg.sender, address(this), totalAirdrop);
    ...
    function witdrawTokens(address tokenAddr, address to, uint amount) public onlyOwner {
        IERC20(tokenAddr).transfer(to, amount);
    }
    function selfD () public {
...
contract DividendTracker is DividendPayingToken {
    ...
    constructor() {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 10000 * 10**3; //must hold 20000+ tokens to receive dividends
    }
