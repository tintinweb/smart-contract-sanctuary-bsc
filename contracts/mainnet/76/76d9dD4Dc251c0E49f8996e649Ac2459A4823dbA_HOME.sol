...
abstract contract LpFee is Owned, DexBase, ERC20 {
    uint256 public constant buyLpFee = 4;
    uint256 public constant sellLpFee = 6;
    address constant fundAddress = 0x6a6085A2D2C974CBB1Cc705e93952Ddd54cbfaf6;
    address constant marketAddress = 0x63d70f049C1893C4De9F979D0AA0F11445AD5909;
    ...
    function swapAndToDividend() internal lockTheSwap {
        ...
        uint256 swapToMarket = (theSwapAmount * 4) / 10;
        uint256 swapToLp = swapToMarket;
        uint256 swapToFund = theSwapAmount - swapToMarket - swapToLp;
        try
            distributor.transferUSDT(IERC20(USDT), address(this), swapToLp)
        {} catch {}
        try
            distributor.transferUSDT(IERC20(USDT), marketAddress, swapToMarket)
        {} catch {}
        try
...
contract HOME is ExcludedFromFeeList, BlackList, LpFee {
    uint256 private constant _totalSupply = 21000 * 1e18;

    uint256 public launchedAt;
    uint256 public launchedAtTimestamp;

    constructor() ERC20("HOME", "HOME", 18) LpFee(1 * 1e18) {
    ...
    function kaipan() external onlyOwner {
        launch();
    }
    ...
    function takeFee(address sender, uint256 amount)
        ...
        returns (uint256)
    {
        if (launchedAt + 600 >= block.number && sender != uniswapV2Pair) {
