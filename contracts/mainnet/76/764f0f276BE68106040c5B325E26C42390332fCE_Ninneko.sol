...
contract Ninneko is Initializable, ERC721Upgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    mapping(address => uint256) public userVolumeTrade;
    ...
    function _makeTransaction(
        ...
        if (isRecordVolumeTrade) {
            userVolumeTrade[_buyer] = userVolumeTrade[_buyer] + _price;
        }
    ...
    function setPriceRebornPet(uint256 _rebornFee) external onlyOwner {
        rebornFee = _rebornFee;
    }
    ...
    function getVolumeTrade(address[] memory _address) external view returns (uint256[] memory) {
        uint256[] memory result = new uint256[](_address.length);
        for (uint256 i = 0; i < _address.length; i++) {
            address user = _address[i];
            result[i] = userVolumeTrade[user];
        }
        return result;
    }
    ...
    function reborn(
        ...
        uint256 allowanceNino = NINOToken.allowance(msg.sender, address(this));
        require(allowanceNino >= rebornFee, "Token payment allowance is insufficient");
        //
