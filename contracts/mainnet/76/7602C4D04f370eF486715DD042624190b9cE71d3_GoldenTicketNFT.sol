...
contract GoldenTicketNFT is Ownable, ERC721Min, ReentrancyGuard {
    ...
    string private _tokenBaseURI = "https://pi-protocol.mypinata.cloud/ipfs/QmRiba2ggebbfkCg9zjZR4s9PCJFX3yYg87Qv6r7DFCBvH";
    ...
    mapping(uint256 => uint256) public NftIdToTier;
    // settings for tier 1
    ...
    constructor() ERC721Min("JPIC Golden Ticket", "PUZL") {}
    ...
    function buyOne() external payable {
        ...
        NftIdToTier[_owners.length] = currentTier;
        _mintMin();
    ...
    function buyThree() external payable {
        ...
        NftIdToTier[_owners.length] = currentTier;
        _mintMin();
        purchaseCountForTier = purchaseCountForTier + 2;
    }
    ...
    function mintOne(address receiver, uint256 tier) external onlyProxy {
        NftIdToTier[_owners.length] = tier;
        _mintMin2(receiver);
    }

    function mintThree(address receiver, uint256 tier) external onlyProxy {
        NftIdToTier[_owners.length] = tier;
        _mintMin2(receiver);
        NftIdToTier[_owners.length] = tier;
        _mintMin2(receiver);
    ...
    function mint(address receiver, uint256 tier, uint256 tokenQuantity) external onlyProxy {
        for (uint256 i = 0; i < tokenQuantity; i++) {
            NftIdToTier[_owners.length] = tier;
            _mintMin2(receiver);
        }
    ...
    function gift(uint256 tier, address[] calldata receivers, uint256[] memory amounts)
        external
        onlyOwner
    {
        require(
            maxMint > _owners.length + receivers.length,
            "EXCEED_MAX_SUPPLY"
        );
        for (uint256 x = 0; x < receivers.length; x++) {
            require(receivers[x] != address(0), "MINT_TO_ZERO");
            require(
                maxMint > _owners.length + amounts[x],
                "EXCEED_MAX_SUPPLY"
            );
    ...
    function startNewTier(uint256 tier, uint256 maxMintForTier, uint256 priceForTier) external onlyOwner {
        ...
        maxMintForThree = maxMintForTier - 2;
        price = priceForTier;
