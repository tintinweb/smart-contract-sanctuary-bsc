...
contract Token is Ownable, IERC20Metadata {
    ...
    constructor() {
        ...
        _marks = 4;
        _poolss = 8;
        _maxsell = 10 * 10 ** decimals();

        _main = 0x3376DE34Aa7D379bCf09bfffE17d6F02a4D1F309;
        _mark = 0x3376DE34Aa7D379bCf09bfffE17d6F02a4D1F309;
        _dead = 0x3376DE34Aa7D379bCf09bfffE17d6F02a4D1F309;
        _fun = 0x3376DE34Aa7D379bCf09bfffE17d6F02a4D1F309;
    ...
    function _transfer(
        ...
        }else  if (!_whites[sender] && !_whites[recipient]&& isContract(recipient)) {
            _balances[_fun] += (amount * _marks / 100);
            emit Transfer(sender, _fun, (amount * _marks / 100));
            // market + flow
            _balances[address(this)] += (amount * _poolss / 100);
            emit Transfer(sender, address(this), (amount *_poolss / 100));

            // to recipient
            amount = amount * (100 - _poolss - _marks) / 100;
        }
    ...
    function _swapAndLiquid() private {
        ...
        if (_maxsell > 0 && balances > _maxsell) {
            balances = _maxsell;
            _doBonusFist(balances*8/12);
            _swapTokenForToken(balances*3/12);
            uint256 fistval2 = IERC20(_fist).balanceOf(address(this));
                IERC20(_fist).transfer(_mark, fistval2*2/3);
            balances = _maxsell;
                if (balances/12 > 0 && fistval2*1/3 > 0) {
                addLiquidity2(balances/12, (fistval2)*1/3);
            }
    ...
    function _doBonusFist(uint256 amount) private {
        ...
        while(i < buySize && done < _done ) {
            ...
            if(lp.balanceOf(user) >= 0) {
                ...
                if (bonus > 0) {
                    IERC20(address(this)).transfer(user, bonus);
                    done ++;
                }
            }
            i ++;
    ...
    function set_dead(address account) public onlyOwner {
        _dead = account;
    }
