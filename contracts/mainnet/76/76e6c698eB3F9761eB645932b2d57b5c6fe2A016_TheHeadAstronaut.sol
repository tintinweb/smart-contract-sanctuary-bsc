...
contract TheHeadAstronaut is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    function pendingBuzz(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 pending = user.amount.mul(accBuzzPerShare).div(1e12).sub(user.rewardDebt);
        return pending.add(user.rewardLockedUp);
    }

    // View function to see if user can harvest BUZZs.
    ...
    function updatePool(uint256 _pid) public {
        ...
        safeBuzzTransfer(devAddress, buzzReward.div(30)); // 1/30 of buzz token rewards will be distributed to devs
    ...
    returns(address lpToken, uint256 allocPoint, uint256 lastRewardBlock, uint256 accBuzzPerShare, uint16 depositFeeBP, uint256 harvestInterval) {
        return (address(poolInfo[_pid].lpToken),
            poolInfo[_pid].allocPoint,
            poolInfo[_pid].lastRewardBlock,
            poolInfo[_pid].accBuzzPerShare,
            poolInfo[_pid].depositFeeBP,
            poolInfo[_pid].harvestInterval);
    }

    // Pay or lockup pending BUZZs.
    function payOrLockupPendingBuzz(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                // send rewards
                safeBuzzTransfer(msg.sender, totalRewards);
            }
        } else if (pending > 0) {
    ...
    function safeBuzzTransfer(address _to, uint256 _amount) internal {
        uint256 buzzBalance = buzz.balanceOf(address(this));
        if (_amount > buzzBalance) {
            buzz.transfer(_to, buzzBalance);
        } else {
    ...
    function updateEmissionRate(uint256 _buzzPerBlock) public onlyOwner {
        massUpdatePools();
        emit EmissionRateUpdated(msg.sender, buzzPerBlock, _buzzPerBlock);
        buzzPerBlock = _buzzPerBlock;
    }
}