...
contract DF1 is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // FIRST INITIAL MINT = 160,000 TOKENS
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 1 ether;  // for the first genesis pools
    uint256 public constant INITIAL_DF1_POOL_DISTRIBUTION = 150000 ether; // for TOKEN-WBNB LP -> TOKEN pool
    uint256 public constant INITIAL_AIRDROP_WALLET_DISTRIBUTION = 9999 ether; // for the airdrops wallet
     ...
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("DF1", "DF1") {
        require(_taxRate < 10000, "Tax cannot equal or be more than 100%");
        require(_taxCollectorAddress != address(0), "Tax collector address must be non-zero address");

        excludeAddress(address(this));

        // Mint 1 token to the deployer for initial LP setup
    ...
    function _getDf1Price() internal view returns (uint256 _df1Price) {
        try IOracle(df1Oracle).consult(address(this), 1e18) returns (uint144 _price) {
    ...
    function _updateTaxRate(uint256 _df1Price) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_df1Price >= taxTiersTwaps[tierId]) {
    ...
    function setDf1Oracle(address _df1Oracle) public onlyOperatorOrTaxOffice {
        require(_df1Oracle != address(0), "oracle address cannot be 0 address");
        df1Oracle = _df1Oracle;
    }
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentDf1Price = _getDf1Price();
            currentTaxRate = _updateTaxRate(currentDf1Price);
            if (currentDf1Price < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_df1Pool != address(0), "!_df1Pool");
        require(_airdropWallet != address(0), "!_airdropWallet");

        rewardPoolDistributed = true;

        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_df1Pool, INITIAL_DF1_POOL_DISTRIBUTION);
