...
contract USDTSeeds is Ownable {
	...
	constructor(address payable wallet) public {
		...
		plans.push(Plan(7, 380));
        plans.push(Plan(15, 420));
        plans.push(Plan(30, 480)); 
		plans.push(Plan(60, 550));
        plans.push(Plan(90, 700)); 
	...
	function getUserDividends(address userAddress) public view returns (uint256) {
		...
		for (uint256 i = 0; i < user.deposits.length; i++) {
		    ...
		    if ( finish <= block.timestamp) {
                sumAmount =sumAmount.add(user.deposits[i].amount) ;
                uint256 share2 = user.deposits[i].amount.mul(plans[user.deposits[i].plan].percent).mul(plans[user.deposits[i].plan].time).div(PLANPER_DIVIDER);
        ...
        if(sumAmount>0){
            totalAmount=sumAmount+totalAmount-getUserTotalWithdrawn(userAddress)+getUserTotalSeedWithdrawn(userAddress);
        }
		return totalAmount;
	}
