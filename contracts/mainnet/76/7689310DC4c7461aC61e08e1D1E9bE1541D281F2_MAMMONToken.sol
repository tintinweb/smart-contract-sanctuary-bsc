...
contract MAMMONToken is Context, IBEP20, Ownable {
    // from:https://mofo.app
    ...
    string private  _name = "MAMMON";
    string private  _symbol = "MAMMON";
    uint8 private  _decimals = 18;
    uint256 private _tTotal = 100000 * 10**6 * 10**_decimals;

    uint8 public constant MAX_DRAW_BASE = 100; // base is 100

    uint256 public _drawNumerator = 30; // probability 30 / 100, _drawNumerator < MAX_DRAW_BASE
 ...
 constructor (        
        ...
        uint256[10] memory _feeSettings,
        uint256 drawNumerator_,
        ...
        _fomoSpacedTime=_feeSettings[9]; // default:5min=5*60=300
        _txgoldFee = _feeSettings[0]; //default 300 1.5%
        _marketingFee = _feeSettings[1]; //default 150 1.5%
        _burnFee = _feeSettings[2]; //default 50 0.5%
        _fomoFee = _feeSettings[3]; //default 50 0.5%
        _liquidityFee = _feeSettings[4]; //default 300 3%
        require((_txgoldFee + _marketingFee + _burnFee + _fomoFee) < 10000, "BEP20: The sum should be less than 10000");
        
        _numTokensSellToAddToLiquidity = _tTotal.div(10**7); //numTokensSellToAddToLiquidity_;
        _antiWhaleThreshold = _feeSettings[6] * 10**_decimals; // antiWhaleThreshold_=_maxTxAmount*5
        _maxTxAmount = _feeSettings[7] * 10**_decimals;// maxTxPercent default _tTotal
        _drawNumerator = drawNumerator_; // default 22 probability 22 / 100, _drawNumerator < MAX_DRAW_BASE
        require(_drawNumerator <= MAX_DRAW_BASE, "BEP20: drawNumerator should not be greater than MAX_DRAW_BASE");
        _daySellNumber = _feeSettings[8]; // default 10
        ...
        _txgoldWallet = payable(_wallet[0]); // txgoldWallet_
        _marketingWallet = payable(_wallet[1]);// marketingWallet_

        //exclude owner and this contract from fee
        _isExcludedFromFee[owner()] = true;
        _isExcludedFromFee[address(this)] = true;
        _isExcludedFromFee[_txgoldWallet] = true;
        ...
        _isExcludedFromAntiWhale[_txgoldWallet] = true;
        ...
        _isExcludedSendDraw[_txgoldWallet] = true;
        _isExcludedSendDraw[_marketingWallet] = true;
    ...
    function setDrawNumerator(uint256 drawNumerator) public onlyOwner {
