...
contract ABC is Base{
    ...
    function pledge(uint256 quantity,address superior) public {
        ...
        introducePrize(  quantity.div(10),_playerMap[id].superior,5);
    ...
    function settleStaticABC() public {
        ...
        uint256 dayd = lsrewardABC.mul(3).div(1000);
    ...
    function introducePrize(uint256 quantity,address superior,uint256 round) internal {
        ...
        if(round>0&&id>0){
            if(_playerMap[id].produceABC > 0){
                _playerMap[id].produceABC = _playerMap[id].produceABC.add(quantity);
                _playerMap[id].pledgeABCTZ = _playerMap[id].pledgeABCTZ.add(quantity);
