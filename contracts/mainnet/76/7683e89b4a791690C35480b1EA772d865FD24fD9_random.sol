...
contract random is Governance {
    ...
    uint[] public typeRate =[9000,9700];
    uint[] public typeAmounts =[1000*1e18,2000*1e18,3000*1e18];
    uint[] public priceAmounts =[1000*1e18,2000*1e18,3000*1e18];
    string[] public urls = ["QmWyokhS2iSZQioz5Cz2DgeRKmHkCjtgKQNZgkJ6tqz8Pi",
                          "QmezE95edSD8PxRufBfFuaRoinR1DbJV61RyQznDh7tAwW",
                          "QmS7sYHAHkd5jz4EXb1ZhWQ85qJGQf8euh9UdcE8rLgA4t"];
    ...
    function getRandom() external onlylock {
        ...
        nfttoken(_nfttoken).mint(msg.sender,true,typeAmounts[_parame],urls[_parame]);

        randomNumber = getRandom4RepeatHash(randomNumber.mod(1000000));
        ranlock = true;
    }

    function buyNFT(uint _type) external {
      require(_type >=0 && _type<=2,"_type err");

      IERC20(stakeToken).safeTransferFrom(msg.sender,withdrawAddr,priceAmounts[_type]);

      nfttoken(_nfttoken).mint(msg.sender,false,typeAmounts[_type],urls[_type]);
    ...
    function setAmounts( uint[] calldata new_typeAmounts) external onlyGovernance {
        typeAmounts = new_typeAmounts;
    }

    function setpriceAmounts( uint[] calldata new_priceAmounts) external onlyGovernance {
        priceAmounts = new_priceAmounts;
    }
