...
library Address{
 ...
 contract YourSurprise is Context, IBEP20, Ownable {
    ...
    address public promoprizesReceiver           = 0xFe45674eb8FFF0013477527BFF9f6507EfCE50DF;
    address public devReceiver                   = 0x77F678b7112ED2dF788d887ad1FF5E88bDFbE8B1;

    string private constant _name                = "YourSurprise";
    string private constant _symbol              = "YSURPRISE";
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[promoprizesReceiver] = true;
        _isExcludedFromFee[devReceiver]         = true;
        
        allowedTransfer[address(this)]          = true;
        allowedTransfer[owner()]                = true;
        allowedTransfer[pair]                   = true;
        allowedTransfer[promoprizesReceiver]    = true;
        allowedTransfer[devReceiver]            = true;
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rPromoprizes, uint256 rLiquidity){
        ...
        rPromoprizes            = s.tPromoprizes*currentRate;
        rLiquidity              = s.tLiquidity*currentRate;
        uint256 rDev            = s.tDev*currentRate;
        rTransferAmount         = rAmount-rRfi-rPromoprizes-rLiquidity-rDev;
        return (rAmount, rTransferAmount, rRfi,rPromoprizes,rLiquidity);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to] && block.number <= genesis_block + 0) {
            require(to != pair, "Sells are not allowed for the first 2 blocks.");
        }
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(promoprizesAmt > 0){
            payable(promoprizesReceiver).sendValue(promoprizesAmt);
        }
        uint256 devAmt = unitBalance * 2 * temp.dev;
        if(devAmt > 0){
            payable(devReceiver).sendValue(devAmt);
        }
    }
    ...
    function swapTokensForBNB(uint256 tokenAmount) private {    //Generate the Pancakeswap pair path of token -> WETH
