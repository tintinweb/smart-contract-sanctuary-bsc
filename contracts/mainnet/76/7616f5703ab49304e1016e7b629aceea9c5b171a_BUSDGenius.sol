...
if (block.timestamp >= (users[userAddress].checkpoint+ONE_WEEK)) {
   ...
   revert("NOT TIME");
    ...
    function blockinfo2() public view returns (uint256)
    {
        return block.timestamp.mul(10).div(ONE_WEEK).div(10);
    }

    function blockinfo3(uint8 plan) public view returns (uint256)
    {
        return  plans[plan].time;
    }
    function blockinfo4(address userAddress) public view returns (uint256)
    {
        return  users[userAddress].checkpoint+ONE_WEEK;
    }

    function blockinfo5() public view returns (uint256)
    {
        uint256 times = (users[msg.sender].checkpoint+ONE_WEEK);
        return  times;
    }
