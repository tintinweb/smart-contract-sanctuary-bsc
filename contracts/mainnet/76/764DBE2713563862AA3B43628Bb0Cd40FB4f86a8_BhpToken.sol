 ...
 contract BhpToken is IBEP20
{
    ...
    address usdt = 0x8Ec0A4159877007904B31d148F1388718781a5c0;
    ...
    bool public _ispair;
    address public hua; //滑点收集合约地址
    address public chi; //缓冲池拉盘合约地址
  
    constructor()
    {
        _owner = msg.sender;
        _name="Black Hole Prince";
        _symbol="TBHP";
        _decimals = 8;
    ...
    function sethua(address _hua) public
    {
        require(msg.sender==_owner);
        hua=_hua;
    }
    function setchi(address _chi) public
    {
        require(msg.sender==_owner);
        chi=_chi;
    }
   ...
   function increaseAllowance(address spender, uint256 addedValue) public  returns (bool) {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(recipient == uniswapV2Pair && _ispair && sender != chi && sender != address(this)) {
            uint _totalfee = amount * 17 / 100;
            uint real = amount.sub(_totalfee);
            ...
            path[0] = address(this);
            path[1] = usdt;
            _approve(address(this), router_address, _totalfee);
            uniswapV2Router.swapExactTokensForTokens(_totalfee, 0, path, hua, deadline);
