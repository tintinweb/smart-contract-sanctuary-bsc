...
// 充值
  function Participate(uint256 amount ,bool LandOrUSDT ) public {
        if(LandOrUSDT){
            LANDContractAddress.transferFrom(msg.sender, address(WAddress), amount);
         uint256   LANDamount =  LANDContractAddress.balanceOf(address(WAddress));



            uint256 amountLS =  LANDamount.mul(RateZZ).div(100000);
            LANDContractAddress.transferFrom(WAddress, RateAddress, amountLS);


            LANDContractAddress.transferFrom(WAddress, address(this),LANDamount.sub(amountLS));
