...
contract ERC721NFTMarket is ERC721Holder, Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        require(_BUSDAddress != address(0), "Operations: BUSD address cannot be zero");
        ...
        assetAccept[BUSD] = true;
        PancakeRouter = _PancakeRouteAddress;

        minimumAskPrice = _minimumAskPrice;
        maximumAskPrice = _maximumAskPrice;
    }
    function setAssetAccept(address[] memory assets, bool _status) external onlyOwner {
    ...
    /**
     * @notice Buy token with BUSD by matching the price of an existing ask order
     * @param _collection: contract address of the NFT
     * @param _tokenId: tokenId of the NFT purchased
     * @param _price: price (must be equal to the askPrice set by the seller)
     */
    function buyTokenUsingBUSD(
        address _collection,
        uint256 _tokenId,
        uint256 _price,
        string memory _hash
        ...
        IERC20(BUSD).safeTransferFrom(address(msg.sender), address(this), amounts[0]);
        IERC20(BUSD).approve(PancakeRouter, amounts[0]);
    ...
    function buyTokenUsingAsset2Seller(
        ...
        uint256 _price,
        string memory _hash,
        address _acceptAsset
    ) external nonReentrant {
        require(assetAccept[_acceptAsset], 'ERC721NFTMarket::buyTokenUsingAsset2Seller: invalid acceptAsset');
        address[] memory path = new address[](2);
        path[0] = _acceptAsset;
        path[1] = WBNB;
        uint[] memory amounts = IPancakeRouter(PancakeRouter).getAmountsIn(_price, path);
    ...
    function buyTokenUsingAsset2SellerWithoutBNBPair(
        address _collection,
        uint256 _tokenId,
        uint256 _price,
        string memory _hash,
