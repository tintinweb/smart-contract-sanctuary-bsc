...
contract Hakan is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    {
        require(newAddress != address(dividendTracker), "Hakan: The dividend tracker already has that address");
        HakanDividendTracker newDividendTracker = HakanDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "Hakan: The new dividend tracker must be owned by the Hakan token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Hakan: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Hakan: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Hakan: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Hakan: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Hakan: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Hakan: Cannot update gasForProcessing to same value");
...
contract HakanDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor() DividendPayingToken("Hakan_Dividend_Tracker", "Hakan_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 1 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "Hakan_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "Hakan_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Hakan contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Hakan_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Hakan_Dividend_Tracker: Cannot update claimWait to same value");
