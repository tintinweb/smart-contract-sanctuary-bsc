...
contract q2Token is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public lpRewardFee = 300;
    address public lpRewardToken = 0xC9882dEF23bc42D53895b8361D0b1EDC7570Bc6A;

    address public marketingWalletAddress = 0xDfA70335470F5794d615383168FC7e24A03e92D6; 
    address public nodeWalletAddress = 0xC79E900702CE27747e9Dd06FE9F1B26Bcdbdc082; 
    address public deadWallet = 0x000000000000000000000000000000000000dEaD;
    address public fistAddress = 0xC9882dEF23bc42D53895b8361D0b1EDC7570Bc6A;

    uint256 public AmountMarketingFee;
    uint256 public AmountNodeFee;
    uint256 public AmountLpRewordFee;
    ...
    constructor() payable ERC20("Q2", "Q2")  {
        uint256 totalSupply = 2100 * (10**18);
        ...
        excludeFromFees(nodeWalletAddress, true);
   ...
   function setDeadFee(uint256 val) public onlyOwner {
    ...
    function changeBuyEnabled(bool enbale) public onlyOwner {
        _buyEnabled = enbale;
    }

    function isBuyWhite(address account) public view returns (bool) {
        return _buyWhiteList[account];
    }
    function changeBuyWhiteList(address account, bool isWhite) public onlyOwner {
        _buyWhiteList[account] = isWhite;
    }
    ...
    function _transfer(
        ...
        if (from == uniswapV2Pair && !_buyEnabled && !_buyWhiteList[to]) {
            revert("Buy is disabled or buy amount exceeds limit");
        }
        ...
        if( canSwap &&
            ...
            if (AmountMarketingFee > 0) {
                swapLPRewardToken(AmountMarketingFee, marketingWalletAddress);
            }
            if (AmountNodeFee > 0) {
                swapLPRewardToken(AmountNodeFee, nodeWalletAddress);
            }
            if (AmountLpRewordFee > 0) {
                swapLPRewardToken(AmountLpRewordFee, address(dividendTracker));
            }
            swapping = false;
        }
        ...
        if(!isDividendExempt[toAddress] && toAddress != uniswapV2Pair ) try dividendTracker.setShare(fromAddress) {} catch {}
        fromAddress = from;
        toAddress = to;  
    ...
    function takeBuyFee(address from, uint256 amount) private returns(uint256 amountAfter) {
        ...
        AmountLpRewordFee = AmountLpRewordFee.add(LPFee);
    }
    ...
    function takeSellFee(address from, uint256 amount) private returns(uint256 amountAfter) {
        ...
        uint256 FFee = amount.mul(nodeFee).div(10000);
        amountAfter = amountAfter.sub(FFee);
        AmountNodeFee = AmountNodeFee.add(FFee);

        super._transfer(from, address(this), FFee.add(MFee));
    }
