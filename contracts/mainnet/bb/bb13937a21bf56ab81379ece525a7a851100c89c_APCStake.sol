...
contract APCStake is Initializable,OwnableUpgradeable,ReentrancyGuardUpgradeable{
  ...
  function getUserDeposits(address addr) public returns (Deposit[] memory){
  ...
  function stake(uint lpAmount) public {
    ...
    userDeposits[msg.sender].push(Deposit(msg.sender,lpAmount,amountAPC,amountUSDT,0,block.timestamp,block.timestamp,0));
  ...
  function fecthRewards(address addr) public view returns (uint musdAmount){

    for(uint i=0;i<userDeposits[addr].length;i++){
      uint tmpAmountUSDT= userDeposits[addr][i].amountUSDT * 2;
      if(tmpAmountUSDT==0){
        continue;
      }
      uint duration = (block.timestamp - userDeposits[addr][i].creatTime).div(24*60*60);

      uint totalReward = tmpAmountUSDT.mul(3).mul(duration).div(1000);
      uint depositWithdrawAmount = totalReward.sub(userDeposits[addr][i].withdrawn);
  ...
  function exit() public {
    for(uint i=0;i < userDeposits[msg.sender].length;i++){
      ...
      totalStaked -= userDeposits[msg.sender][i].amount;
      userDeposits[msg.sender][i].amount = 0;
      userDeposits[msg.sender][i].exitTime = block.timestamp;
    }
    withdraw();
  }
