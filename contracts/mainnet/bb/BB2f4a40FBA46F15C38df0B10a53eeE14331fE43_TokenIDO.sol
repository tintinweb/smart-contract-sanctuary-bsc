...
contract TokenIDO is Ownable, ReentrancyGuard {
    ...
    function witheListSale(uint256 amount) internal {
        ...
        require(MaxWhitelistSinglePayInAmount >= amount + whitelistPayInAmountCounter[msg.sender], "Exceeds the whitelist purchase limit");
    ...
    function commonSale(uint256 amount) internal {
        require(MaxCommonSinglePayInAmount >= amount + commonPayInAmountCounter[msg.sender], "Exceeds the public purchase limit");
    ...
    function buy(uint256 amount) nonReentrant external {
        require(saleStarted, "IDO is not open");
        require(block.timestamp >= IDOStartTimestamp, "IDO is not open");
        require(
