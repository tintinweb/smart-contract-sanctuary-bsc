...
contract FlexiPunkTLD is IBasePunkTLD, ERC721, Ownable, ReentrancyGuard {
  ...
  function tokenURI(uint256 _tokenId) public view override returns (string memory) {
    ...
    return IFlexiPunkMetadata(metadataAddress).getMetadata(fullDomainName, _tokenId);
  }
...
interface IFlexiPunkMetadata {
  ...
  function getMetadata(string calldata _fullDomainName, uint256 _tokenId) external view returns(string memory);
