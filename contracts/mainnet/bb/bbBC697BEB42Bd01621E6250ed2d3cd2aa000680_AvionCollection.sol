...
contract AvionCollection is ERC721EnumerableUpgradeable, OwnableUpgradeable {
    ...
    function mint(uint256 _amount) external {
        ...
        _mint(_amount, getPrice() * _amount);
    ...
    function mintToWallet(uint256[] calldata _tier, address wallet) external onlyOwner {
        for (uint256 i = 0; i < _tier.length; ++i) {
            mintInternal(_tier[i], wallet);
        }
    }

    function burn(uint256 _tier, uint256 _amount) external onlyOwner {
        for (uint256 i = 0; i < _amount; ++i) {
            mintInternal(_tier, address(0));
        }
    }
    ...
    */
    ...
    function setNftTier(uint256[] memory _tokenID, uint256[] memory _tier) external onlyOwner {
        for (uint i = 0; i < _tier.length; i++) {
            tokenTier[_tokenID[i]] = _tier[i];
        }
    }
