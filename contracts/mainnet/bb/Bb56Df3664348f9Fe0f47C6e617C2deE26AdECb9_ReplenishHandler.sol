...
contract ReplenishHandler is AccessControl, Pausable {
    ...
    uint public replenishCooldown = 12 hours;
    ...
    uint public divideMultiplier = 1;

    mapping(uint => EventRecipe) private eventRecipes;
    mapping(uint => UpgradeRecipe) private upgradeRecipes;
    ...
    event ReplenishPlot(address owner, uint plotId, uint digAmount, uint[] rIds, uint[] rAmounts);
    ...
    event UpgradeRecipeCreated(uint recipeId);
    event UpgradeRecipeRemoved(uint recipeId); 
    event StartPlotUpgrade(address owner, uint plotId, uint upgradeId);

    constructor (
        IPlanetPlot _planetPlot,  
        IPlotState _plotState,
        IReplenishState _replenishState,
    ...
    function addPlotEventRecipe(
        ...
        require(eventRecipes[_recipeId].digAmount == 0, "Event recipe already exists");
        require((_RIIds.length > 0 || _DIAmount > 0) && _digAmount > 0, "Input failure");
    ...
    function removePlotEventRecipe(uint _recipeId) external onlyRole(GAME_CONTROL){
        require(eventRecipes[_recipeId].digAmount != 0, "Event recipe does not exist");
        delete eventRecipes[_recipeId];
        emit EventRecipeRemoved(_recipeId);
    }

    function setPlotEvent(uint _plotId, uint _recipeId) external whenNotPaused onlyPlotOwner(_plotId) {
        require(eventRecipes[_recipeId].digAmount != 0, "Event recipe does not exist");

        if(eventRecipes[_recipeId].RIIds.length > 0) {
            MoDApproveProxy.resources_burnBatch(msg.sender, eventRecipes[_recipeId].RIIds, eventRecipes[_recipeId].RIAmounts);
        }
        if(eventRecipes[_recipeId].DIAmount > 0) {
            MoDApproveProxy.DAR_transferFrom(msg.sender, MoDTaxAccount, eventRecipes[_recipeId].DIAmount);
        }

        emit PlotEvent(_plotId, eventRecipes[_recipeId].eventId, eventRecipes[_recipeId].digAmount);
    }

    function addUpgradeRecipe(
        uint _recipeId,
        uint _upgradeId,
        uint[] calldata _RIIds,
        uint[] calldata _RIAmounts,
        uint[] calldata _IIPrefixes,
        uint _DIAmount
         ) external onlyRole(GAME_CONTROL){
        require(upgradeRecipes[_recipeId].upgradeId == 0, "Upgrade recipe already exists");
        require(_RIIds.length != 0 || _DIAmount != 0 || _IIPrefixes.length != 0, "Input failure");
        require(_upgradeId > 0, "upgradeId can not be 0");
        upgradeRecipes[_recipeId] = UpgradeRecipe({
            upgradeId: _upgradeId,
            RIIds: checkResourceIds(_RIIds),
            RIAmounts: validAmountArray(_RIAmounts, _RIIds.length),
            IIPrefixes: checkItemPrefixes(_IIPrefixes),
            DIAmount: _DIAmount
        });

        emit UpgradeRecipeCreated(_recipeId);
    }
        
    function removeUpgradeRecipe(uint _recipeId) external onlyRole(GAME_CONTROL) recipeExists(_recipeId) {
        delete upgradeRecipes[_recipeId];
        emit UpgradeRecipeRemoved(_recipeId);
    }

    function startPlotUpgrade(uint _plotId, uint _recipeId, uint[] calldata _tokenIds) external whenNotPaused onlyPlotOwner(_plotId) recipeExists(_recipeId) {
        if(upgradeRecipes[_recipeId].RIIds.length > 0) {
            MoDApproveProxy.resources_burnBatch(msg.sender, upgradeRecipes[_recipeId].RIIds, upgradeRecipes[_recipeId].RIAmounts);
        }

        if (upgradeRecipes[_recipeId].IIPrefixes.length > 0){
            itemInputCheck(_recipeId, _tokenIds);
            MoDApproveProxy.item_burn(msg.sender, _tokenIds);
        }

        if(upgradeRecipes[_recipeId].DIAmount != 0){
            MoDApproveProxy.DAR_transferFrom(msg.sender, MoDTaxAccount, upgradeRecipes[_recipeId].DIAmount);
        }
        replenishState.setUpgradeRequired(_plotId, true);
        emit StartPlotUpgrade(msg.sender, _plotId, upgradeRecipes[_recipeId].upgradeId);
    }
    ...
    function replenishUpgradedPlot(
        ...
        bytes memory message = abi.encode(msg.sender, _plotId, _resourceIds, _resourceAmounts, internalNonce[msg.sender]);
        ...
        replenishState.setPlotResources(_plotId, _resourceIds, _resourceAmounts);
        replenishState.setUpgradeRequired(_plotId, false);
        replenishPlot(_plotId);
    }

    function replenishPlot(uint _plotId) public whenNotPaused onlyPlotOwner(_plotId) {
        require(replenishState.upgradeRequired(_plotId) == false, "Plot upgrade required");
        require(plotCooldown[_plotId] + replenishCooldown < block.timestamp, "Plot in cooldown");
        uint plotMax = plotState.getMax(_plotId);
        uint digAmount =  plotMax - plotState.getLeft(_plotId);

        (uint[] memory rIds, uint[] memory rAmounts) = getReplenishCost(_plotId, digAmount);
        MoDApproveProxy.resources_burnBatch(msg.sender, rIds, rAmounts);

        plotState.setLeft(_plotId, plotMax);
        plotCooldown[_plotId] = block.timestamp;
        emit ReplenishPlot(msg.sender, _plotId, digAmount, rIds, rAmounts);
    }

    function setResourceMultiplier(uint[] calldata _resourceId, uint[] calldata _multiplier) external onlyRole(GAME_CONTROL) {
        require(_resourceId.length == _multiplier.length, "Array length mismatch");
        for (uint256 i = 0; i < _resourceId.length; i++) {
            resourceMultiplier[_resourceId[i]] = _multiplier[i];
        }
    }

    function setResourceBaseline(uint[] calldata _resourceId, uint[] calldata _baseline) external onlyRole(GAME_CONTROL) {
        require(_resourceId.length == _baseline.length, "Array length mismatch");
        for (uint256 i = 0; i < _resourceId.length; i++) {
    ...
    function setPlotResources(uint _plotId, uint[] calldata _rIds, uint[] calldata _rAmounts) external onlyRole(GAME_CONTROL) {
        replenishState.setPlotResources(_plotId, _rIds, _rAmounts);
    }

    function setPlotResourcesBatch(uint[] calldata _plotIds, uint[][] calldata _rIds, uint[][] calldata _rAmounts) external onlyRole(GAME_CONTROL) {
        replenishState.setPlotResourcesBatch(_plotIds, _rIds, _rAmounts);
    }

    function getReplenishCost(uint _plotId, uint _digsToReplenish) public view returns (uint[] memory resourceIds, uint[] memory resourceAmounts) {
        (uint[] memory rIds, uint[] memory rAmounts) = replenishState.getPlotResources(_plotId);
        uint length = rIds.length;
        resourceIds =  new uint[](length + 1);
        resourceAmounts = new uint[](length + 1);
        for (uint256 i = 0; i < length; i++) {
            resourceIds[i] = rIds[i];
            resourceAmounts[i] = ((resourceBaseline[rIds[i]] + (rAmounts[i] * resourceMultiplier[rIds[i]])) * _digsToReplenish) / divideMultiplier;
        }
    ...
    function checkResourceIds(uint[] memory _rIds) internal view returns (uint[] memory){
        if(_rIds.length > 0){
            ...
            for (uint256 i = 0; i < _rIds.length; i++) {
                ...
                for (uint256 j = 0; j < _rIds.length; j++) {
                    if(i != j){
                        require(_rIds[i] != _rIds[j], "Can not have the same _rId input multiple times");
                    }
                }
            }
        }
     ...
     function checkItemPrefixes(uint[] memory _iPrefixes) internal view returns (uint[] memory){
        if(_iPrefixes.length > 0){   
            for (uint256 i = 0; i < _iPrefixes.length; i++) {
                require(items.prefixExists(_iPrefixes[i]), "Invalid item prefix");
            }
        }
        return _iPrefixes;
    }
    ...
    function itemInputCheck(uint _recipeId, uint[] memory _tokenIds) internal view {
        require(upgradeRecipes[_recipeId].IIPrefixes.length == _tokenIds.length, "Invalid tokenId amount");
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            require(upgradeRecipes[_recipeId].IIPrefixes[i] == prefixStrip(_tokenIds[i]), "Invalid tokenId prefix");
        }
    }

    function updateReplenishCooldown(uint _replenishCooldown) external onlyRole(GAME_CONTROL) {
        replenishCooldown = _replenishCooldown;
    }

    function updateServer(address _serverPubKey) external onlyRole(GAME_CONTROL) {
    ...
    function pauseHandler() external onlyRole(GAME_CONTROL) {
        _pause();
    }
    
    function unpauseHandler() external onlyRole(GAME_CONTROL) {
        _unpause();
    }

    modifier onlyPlotOwner(uint _plotId) {
        require(planetPlot.ownerOf(_plotId) == msg.sender, "Sender not owner of plot");
        _;
    }

    modifier recipeExists(uint _recipeId) {
        require(upgradeRecipes[_recipeId].upgradeId > 0, "Upgrade recipe does not exist");
        _;
    }
}
