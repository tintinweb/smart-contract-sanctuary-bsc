...
contract GTest is Ownable, ERC20 {
	IUniswapV2Router02 public uniswapV2Router;
	address public immutable uniswapV2Pair;

	string private constant _name = "GTest Token";
	string private constant _symbol = "GT";
	uint8 private constant _decimals = 18;

	GTestDividendTracker public dividendTracker;
	...
	constructor() ERC20(_name, _symbol) {
		...
		dividendTracker = new GTestDividendTracker();
	...
	function cancelLaunch() external onlyOwner {
		require(this.isInLaunch(), "GTest: Launch is not set");
	...
	function setSellCoolDownPeriod(uint256 newCoolDown) external onlyOwner {
		require(newCoolDown != sellCoolDownPeriod, "GTest: sellCoolDownPeriod is already set to the value of newCoolDown");
	...
	function setSproutHour() external onlyOwner {
		require(!this.isInSproutHour(), "GTest: Sprout Hour is already set");
		require(isTradingEnabled, "GTest: Trading must be enabled first");
		require(!this.isInLaunch(), "GTest: Must not be in launch period");
		emit SproutHourChange(true, false);
		_sproutHourStartTimestamp = _getNow();
	}
	function cancelSproutHour() external onlyOwner {
		require(this.isInSproutHour(), "GTest: Sprout Hour is not set");
		emit SproutHourChange(false, true);
		_sproutHourStartTimestamp = 0;
	}
	function _setAutomatedMarketMakerPair(address pair, bool value) private {
		require(automatedMarketMakerPairs[pair] != value, "GTest: Automated market maker pair is already set to that value");
	...
	function excludeFromFees(address account, bool excluded) external onlyOwner {
		require(_isExcludedFromFee[account] != excluded, "GTest: Account is already the value of 'excluded'");
	...
	function excludeFromMaxTransactionLimit(address account, bool excluded) external onlyOwner {
		require(_isExcludedFromMaxTransactionLimit[account] != excluded, "GTest: Account is already the value of 'excluded'");
		_isExcludedFromMaxTransactionLimit[account] = excluded;
		emit ExcludeFrom('maxTransaction', account, excluded);
	}
	function excludeFromMaxWalletLimit(address account, bool excluded) external onlyOwner {
		require(_isExcludedFromMaxWalletLimit[account] != excluded, "GTest: Account is already the value of 'excluded'");
	...
	function excludeFromSellCoolDown(address account, bool excluded) external onlyOwner {
		require(_isExcludedFromSellCoolDown[account] != excluded, "GTest: Account is already the value of 'excluded'");
	...
	function blockAccount(address account) external onlyOwner {
		...
		require(!_isBlocked[account], "GTest: Account is already blocked");
		if (_isLaunched) {
			require(currentTimestamp - _launchStartTimestamp < _blockedTimeLimit, "GTest: Time to block accounts has expired");
		}
		_isBlocked[account] = true;
		emit BlockedAccountChange(account, true);
	}
	function unblockAccount(address account) external onlyOwner {
		require(_isBlocked[account], "GTest: Account is not blcoked");
	...
	function setFeeOnSelectedWalletTransfers(address account, bool value) external onlyOwner {
		require(_feeOnSelectedWalletTransfers[account] != value, "GTest: The selected wallet is already set to the value ");
	...
	function setWallets(address newLiquidityWallet, address newMarketingWallet, address newBuyBackWallet) external onlyOwner {
		if(liquidityWallet != newLiquidityWallet) {
			require(newLiquidityWallet != address(0), "GTest: The liquidityWallet cannot be 0");
			emit WalletChange('liquidityWallet', newLiquidityWallet, liquidityWallet);
			liquidityWallet = newLiquidityWallet;
		}
		if(marketingWallet != newMarketingWallet) {
			require(newMarketingWallet != address(0), "GTest: The marketingWallet cannot be 0");
			emit WalletChange('marketingWallet', newMarketingWallet, marketingWallet);
			marketingWallet = newMarketingWallet;
		}
		if(buyBackWallet != newBuyBackWallet) {
			require(newBuyBackWallet != address(0), "GTest: The buyBackWallet cannot be 0");
	...
	function setUniswapRouter(address newAddress) external onlyOwner {
		require(newAddress != address(uniswapV2Router), "GTest: The router already has that address");
	...
	function setMaxWalletAmount(uint256 newValue) external onlyOwner {
		require(newValue != maxWalletAmount, "GTest: Cannot update maxWalletAmount to same value");
		emit MaxWalletAmountChange(newValue, maxWalletAmount);
		maxWalletAmount = newValue;
	}
	function setMinimumTokensBeforeSwap(uint256 newValue) external onlyOwner {
		require(newValue != minimumTokensBeforeSwap, "GTest: Cannot update minimumTokensBeforeSwap to same value");
	...
	function claimBNBOverflow(uint256 amount) external onlyOwner {
		require(amount < address(this).balance, "GTest: Cannot send more than contract balance");
	...
	function _transfer(
		...
		if(!_isAllowedToTradeWhenDisabled[from] && !_isAllowedToTradeWhenDisabled[to]) {
			require(isTradingEnabled, "GTest: Trading is currently disabled.");
			require(!_isBlocked[to], "GTest: Account is blocked");
			require(!_isBlocked[from], "GTest: Account is blocked");
			if (_isInLaunch && currentTimestamp - _launchStartTimestamp <= 300 && isBuyFromLp) {
				require((currentTimestamp - _buyTimesInLaunch[to]) > 60, "GTest: Cannot buy more than once per min in launch");
			}
			if (isSelltoLp && !_isExcludedFromSellCoolDown[from]) {
				require((currentTimestamp - _lastSells[from]) >= sellCoolDownPeriod, "GTest: Must wait sellCoolDownPeriod to sell again");
			}
			if (!_isExcludedFromMaxTransactionLimit[to] && !_isExcludedFromMaxTransactionLimit[from]) {
				require(amount <= maxTxAmount, "GTest: Amount > maxTxAmount.");
				if (isSelltoLp) {
					require(amount <= maxSellTxAmount, "GTest: Sell amount > maxSellTxAmount.");
				}
				if (isBuyFromLp) {
					require(amount <= maxBuyTxAmount, "GTest: Buy amount > maxBuyTxAmount.");
				}
			}
			if (!_isExcludedFromMaxWalletLimit[to] && isBuyFromLp) {
				require((balanceOf(to) + amount) <= maxWalletAmount, "GTest: Expected wallet amount > maxWalletAmount.");
			}
		}
...
contract GTestDividendTracker is DividendPayingToken {
	using SafeMath for uint256;
	using SafeMathInt for int256;
	...
	constructor() DividendPayingToken("GTest_Dividend_Tracker", "GTest_Dividend_Tracker") {
		claimWait = 3600;
	...
	function _transfer(address, address, uint256) internal pure override {
		require(false, "GTest_Dividend_Tracker: No transfers allowed");
	}
	function excludeFromDividends(address account) external onlyOwner {
	...
	function setTokenBalanceForDividends(uint256 newValue) external onlyOwner {
		require(minimumTokenBalanceForDividends != newValue, "GTest_Dividend_Tracker: minimumTokenBalanceForDividends already the value of 'newValue'.");
