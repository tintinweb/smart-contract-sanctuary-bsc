...
interface INFTFactory {
    function createFee(address nft) external view returns (uint256);

    function ownerBynft(address nft) external view returns (address);
}
...
contract NFTMarket is ERC721Holder, ERC1155Holder, Ownable, ReentrancyGuard {
    ...
    INFTFactory public nftERC721Factory;
    INFTFactory public nftERC1155Factory;

    address public immutable USDT;
    uint256 public constant TOTAL_MAX_FEE = 20; // 10% of a sale

    bytes4 public constant ERC721_INTERFACE_ID = 0x80ac58cd;
    bytes4 public constant ERC1155_INTERFACE_ID = 0xd9b67a26;

    address public treasuryAddress;
    uint256 public tradingFeeRate = 3;
    ...
    // Treasury Addresses are updated
    event NewTreasuryAddresses(address indexed treasury);
    event UpdateTradingFee(uint256 indexed tradingFee);
    ...
    constructor(
        ...
        address _nftERC721Factory,
        address _nftERC1155Factory,
        ...
        require(
            ...
            "Operations: USDT address cannot be zero"
        );
        require(
            _minimumAskPrice > 0,
        ...
        USDT = _usdtAddress;
        nftERC721Factory = INFTFactory(_nftERC721Factory);
        nftERC1155Factory = INFTFactory(_nftERC1155Factory);
    ...
    function buyTokenUsingUSDT(
        ...
        uint256 totalTradingFee = (totalPrice.mul(tradingFeeRate)) / 100;
        IERC20(USDT).safeTransferFrom(
            address(msg.sender),
            address(this),
            totalPrice.add(totalTradingFee)
        );

        _buyToken(_nft, _tokenId, _amount, _price);
    }
    ...
    function cancelAskOrder(
        ...
        require(
            _tokenIdsOfSellerForCollection[msg.sender][_nft].contains(_tokenId),
            "Order: Token not listed"
        );

        Ask storage askInfo = _askDetails[_nft][_tokenId];
        require(askInfo.amount >= _amount, "amount error");

        if (askInfo.nftType == NFTType.ERC721) {
        ...
        if (_amount < askInfo.amount) {
            askInfo.amount = askInfo.amount.sub(_amount);
        } else {
            // Adjust the information
            _tokenIdsOfSellerForCollection[msg.sender][_nft].remove(_tokenId);
            delete _askDetails[_nft][_tokenId];
            _askTokenIds[_nft].remove(_tokenId);
        }

        // Emit event
        emit AskCancel(_nft, msg.sender, _tokenId, _amount);
    }
    ...
    function claimPendingRevenue() external nonReentrant {
        ...
        IERC20(USDT).safeTransfer(address(msg.sender), revenueToClaim);
    ...
    function createAskOrder(
        ...
        require(
            !_tokenIdsOfSellerForCollection[msg.sender][_nft].contains(
                _tokenId
            ),
            "Order: Tokens are listed"
        );

        bool nftType = IERC721(_nft).supportsInterface(ERC721_INTERFACE_ID);
        require(
            nftType || IERC1155(_nft).supportsInterface(ERC1155_INTERFACE_ID),
            "Operations: Not NFT"
        );
        address creator;
        uint256 createFee;
        ...
        if (nftType) {
            ...
            _amount = 1;
            createFee = nftERC721Factory.createFee(_nft);
            creator = nftERC721Factory.ownerBynft(_nft);
        } else {
            IERC1155(_nft).safeTransferFrom(
                address(msg.sender),
                address(this),
            ...
            createFee = nftERC1155Factory.createFee(_nft);
            creator = nftERC1155Factory.ownerBynft(_nft);
        }

        // Adjust the information
        _tokenIdsOfSellerForCollection[msg.sender][_nft].add(_tokenId);
        _askDetails[_nft][_tokenId] = Ask({
            seller: msg.sender,
            nft: _nft,
            tokenId: _tokenId,
            amount: _amount,
            ...
            nftType: nftType ? NFTType.ERC721 : NFTType.ERC1155,
            creator: creator,
            createFee: createFee
        });
        ...
        // Emit event
        emit AskNew(_nft, msg.sender, _tokenId, _amount, _askPrice, createFee);
    }
    ...
    function modifyAskOrder(
        ...
        // Adjust the information
        _askDetails[_nft][_tokenId].price = _newPrice;

        // Emit event
        emit AskUpdate(_nft, msg.sender, _tokenId, _newPrice);
    }
    ...
    function recoverFungibleTokens(address _token) external onlyOwner {
        require(_token != USDT, "Operations: Cannot recover usdt");
    ...
    function recoverNonFungibleToken(
        ...
        if (IERC721(_token).supportsInterface(ERC721_INTERFACE_ID)) {
        ...
        if (IERC1155(_token).supportsInterface(ERC1155_INTERFACE_ID)) {
    ...
    function updateTradingFee(uint256 _tradingFee) external onlyOwner {
        ...
        emit UpdateTradingFee(tradingFeeRate);
    }
    ...
    {
        Ask memory askOrder = _askDetails[collection][tokenId];
        return _calculatePriceAndFeesForCollection(amount, askOrder);
    }
    ...
    function _buyToken(
        ...
        require(_askTokenIds[_nft].contains(_tokenId), "Buy: Not for sale");

        Ask memory askOrder = _askDetails[_nft][_tokenId];

        // Front-running protection
        require(_price == askOrder.price, "Buy: Incorrect price");
        require(msg.sender != askOrder.seller, "Buy: Buyer cannot be seller");
        require(_amount <= askOrder.amount, "Buy: Amount error");
        ...
        (
            ...
            uint256 tradingFee,
            uint256 creatorFee
        ) = _calculatePriceAndFeesForCollection(_amount, askOrder);

        // Transfer USDT
        IERC20(USDT).safeTransfer(askOrder.seller, netPrice);

        // Update pending revenues for treasury/creator (if any!)
        if (creatorFee != 0) {
            pendingRevenue[askOrder.creator] += creatorFee;
        }
        ...
        if (askOrder.nftType == NFTType.ERC721) {
        ...
        if (_amount == askOrder.amount) {
            _tokenIdsOfSellerForCollection[askOrder.seller][_nft].remove(
                _tokenId
            );
            delete _askDetails[_nft][_tokenId];
            _askTokenIds[_nft].remove(_tokenId);
        } else {
            Ask storage askO = _askDetails[_nft][_tokenId];
            askO.amount = askO.amount.sub(_amount);
        }
    ...
    {
        uint256 totalAskPrice = askOrder.price.mul(_amount);
        tradingFee = (totalAskPrice.mul(tradingFeeRate)) / 100;
        creatorFee = (totalAskPrice.mul(askOrder.createFee)) / 100;
        netPrice = askOrder.price - creatorFee;
