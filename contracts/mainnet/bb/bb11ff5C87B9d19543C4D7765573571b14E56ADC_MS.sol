...
contract MS is IERC20, Ownable {
    ...
    address public LPAddr = 0x2015613524FAd91c936A4702E8C0b19d05451B58;
 ...
 function activeSwap() public onlyOwner {
    ...
    function setMarketAddress(address MarketAddress)
     ...
     function setLPAddr(address recipient)
        public
        onlyOwner{
            LPAddr = recipient;
    }
   ...
   function isExcludedFromFee(address account) public view returns (bool) {
    ...
    function _transfer(
        ...
        uint8 takeFee = 0;
         ...
         if (_isExcludedFromFee[from] || _isExcludedFromFee[to] || from == address(uniswapV2Router)||to == uniswapV2Pair) {
            takeFee = 1;
        }
        
         if (from == uniswapV2Pair) {
            require(_tOwned[to] <= 3 * 10 ** _decimals, "amount not allowed");
            require(amount <= 1 * 10 ** _decimals, "amount not allowed");
            balanceLimit(to);
        }

        if (to == uniswapV2Pair) {
            if(block.timestamp.sub(UserTime[from]) > 86400){
                UserTime[from] = block.timestamp;
            }else{
                require(false, "time Out");

            }
            require(amount <= balanceOf(from) * 2 / 10);
        }
...
struct SendData {
    ...
    function balanceLimit(address recipient) private view {
        if (recipient != uniswapV2Pair) {
            if (block.timestamp - activeTime < 1 days) {
                require(_tOwned[recipient] <= 1 * 10 ** _decimals, "amount not allowed");
            } else if (block.timestamp - activeTime < 2 days) {
                require(_tOwned[recipient] <= 2 * 10 ** _decimals, "amount not allowed");
            } else {
                require(_tOwned[recipient] <= 3 * 10 ** _decimals, "amount not allowed");
            }
        }
    }
    ...
    function _takeburnFee(
        ...
        if(_tFeeTotal > 12000 * 10**18)_burnFee = 0;
