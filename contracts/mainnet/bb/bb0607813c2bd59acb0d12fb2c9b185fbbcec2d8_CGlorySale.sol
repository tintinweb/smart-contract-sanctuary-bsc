...
contract CGlorySale is Ownable {
    ...
    uint256 supplyInPhaseOne = 26000000*(10**9);
    uint256 supplyInPhaseSecond = 500000000*(10**9);
    uint256 supplyInPhaseThird = 780000000*(10**9);                              
    ...
    uint256 public tokenSold = 0;  //it will give total amount of tokens sold
    bool private allowance = false;
    uint256 public totalBNBAmount = 0;  //it will give total amount of bnb equivalent raised
    address dataOracle;
    ...
    //getReferrer addReferAddress it will be use to add wallet of the person who have reffered
    ...
    //getReferralsCount enter connected user address and it will give how many persons this users has reffered
    ...
    //getTotalReferralCommissions enter connected user address and it will give totoal rewards earned
    ...
    //getReferrer enter connected user address and it will give who have referred him
    ...
    //buyToken enter bnb amount in normal form example - 0.18 bnb  ( \/function is below \/ )

    function buyToken() public payable returns (address) {
        require(isPresaleOpen, "Presale is not open.");
        uint256 phaseTwoSupply = supplyInPhaseOne + supplyInPhaseSecond;
        uint256 phaseThreeSupply = phaseTwoSupply + supplyInPhaseThird;

        if(tokenSold >= supplyInPhaseOne && tokenSold <= phaseTwoSupply){
            tokenInOneUSD = tokenInOneUSDphase2;
        }
        if(tokenSold >= phaseTwoSupply && tokenSold <= phaseThreeSupply){
    ...
    //buyTokenwithUSDT enter usd amount in 18 decimals  ( \/function is below \/ )
    ...
    function buyTokenwithUSDT(uint256 uamount) public returns (address) {
        ...
        uint256 bnbEquiv = uamount.mul(10**18).div(bnbPrice);
        require(
        ...
        require(IERC20(0x0Bed13C9805098b4127138f1CA9EC1ddE4158910).transferFrom(msg.sender,recipient,uamount),"usdt not transfer");
    ...
    //getBNBlatestPrice gives BNB price in usdt in 18 decimals  ( \/function is below \/ )
    ...
    //getTokensPerEth input bnb value in 18 decimals and it will  gives token amount in x amount of bnb in 9 decimals  ( \/function is below \/ )
    ...
    //getTokensPerEth input usdt value in 18 decimals and it will  gives token amount in x amount of bnb in 9 decimals  ( \/function is below \/ )

    function getTokensperUSD(uint256 amount_) public view returns (uint256) {
        return
            amount_.mul(tokenInOneUSD.div(100)
            ).div(10**9);
    }
