...
interface IIFOPool {
    function getPoint(address _owner) external view returns (uint256) ;
    function burn(address _owner, uint256 _withdrawAmount) external ;
...
contract IFO is ReentrancyGuardUpgradeable {
    ...
    uint256 constant public HARVEST_PERIODS = 1; 
    ...
    function initialize(
      ...
      address _adminAddress
        ...
        uint256 _vestingBlockOffset = 1; // Block offset between vesting distributions
    ...
    function getMaxAmount(address _owner) public  view returns (uint256) {
        uint256 point = ifoPool.getPoint(_owner); //1e18==100%
        uint256 amount = point * raisingAmount / 1e18;
        return amount*2 - userInfo[msg.sender].amount; 
    }
    ...
    function depositInternal(uint256 _amount) internal {
        uint bnbAmount = getMaxAmount(msg.sender);
        require(_amount <= bnbAmount, "amount is greater than max amount");
        uint totalStaked = ifoPool.getStakeTotalDeposited(msg.sender, 0) ;
        ifoPool.burn(msg.sender, _amount * totalStaked / bnbAmount / 10  );
    ...
    //         "no refund to user"
    //     );
    //     uint256 _stakeTokenAmount = userInfo[msg.sender].amount;
    //     userInfo[msg.sender].amount = 0;
