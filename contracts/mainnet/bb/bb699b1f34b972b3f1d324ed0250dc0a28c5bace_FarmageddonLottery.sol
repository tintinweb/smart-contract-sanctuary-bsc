...
contract FarmageddonLottery is ReentrancyGuard, IPancakeSwapLottery, Ownable {
    ...
    mapping(address => uint256) raritySetting;
    ...
    uint256 public treasuryAmount= 0;       // uses all 3 as max. split the % of distribution between the 3.
    uint256 public partnerAmount = 0;
    uint256 public otherAmount= 0;

    uint256 public treasuryAmountOnBuys = 0;    // 100 = 1%
    uint256 public partnerAmountOnBuys = 0;     // 100 = 1%
    uint256 public otherAmountOnBuys = 0;       // 100 = 1%

    address public BNBtreasury = 0x9ac1A7751335656C44723B5E9Ed62571fc763c11; //    COMPOST
    uint256 public BNBfee = 300000000000000;  //                      .0003 bnb
    ...
    uint256 public constant MIN_LENGTH_LOTTERY = 1 seconds; // 1 hours
    uint256 public constant MAX_LENGTH_LOTTERY = 32 days + 5 minutes; // 31 days
    uint256 public constant MAX_TREASURY_FEE = 3000; // 30%

    IERC20 public cakeToken;
    IRandomNumberGenerator public randomGenerator = IRandomNumberGenerator(0x439381Ad77172E0F8876f2e24fA4D3891B77cB36);
    ...
    constructor(address _cakeTokenAddress) {
        ...
        operatorAddress = 0xE148978f037cB3B4ae04217f37945469560009b3;
        ...
        payable
    {
        ...
        // Transfer CAKE to treasury address and Other Addresses
        uint256 feeDiv  = 10000;
        uint256 toLottery = amountCakeToTransfer;
        if (treasuryAmountOnBuys > 0) {
            uint256 amountToTreasury = (amountCakeToTransfer * treasuryAmountOnBuys / feeDiv);
            toLottery -= amountToTreasury;
            cakeToken.safeTransferFrom(address(msg.sender), treasuryAddress, amountToTreasury);
        }

        if (partnerAmountOnBuys > 0){
            uint256 amountToPartner = (amountCakeToTransfer * partnerAmountOnBuys / feeDiv);
            toLottery -= amountToPartner;
            cakeToken.safeTransferFrom(address(msg.sender), partnerAddress, amountToPartner);            
        }
        
        if (otherAmount > 0){
            uint256 amountToother = (amountCakeToTransfer * partnerAmountOnBuys / feeDiv);
            toLottery -= amountToother;
            cakeToken.safeTransferFrom(address(msg.sender), otherAddress, amountToother);
        }
        // Transfer cake tokens to this contract
        cakeToken.safeTransferFrom(address(msg.sender), address(this), toLottery);
        ...
        nonReentrant
    {
       ...
       // Setup for transfers
        ...
        uint256 amountToother;
        
        // Transfer CAKE to treasury address and Other Addresses
        uint256 feeDiv  = (treasuryAmount + partnerAmount + otherAmount);
        
        if (treasuryAmount > 0) {
            amountToTreasury = (amountToWithdrawToTreasury * treasuryAmount / feeDiv);
            cakeToken.safeTransfer(treasuryAddress, amountToTreasury);
        }

        if (partnerAmount > 0){
            amountToPartner = (amountToWithdrawToTreasury * partnerAmount / feeDiv);
            cakeToken.safeTransfer(partnerAddress, amountToPartner);
        }
        
        if (otherAmount > 0){
            amountToother = (amountToWithdrawToTreasury - amountToTreasury - amountToPartner);
            cakeToken.safeTransfer(otherAddress, amountToother);
        }
    ...
    function setTreasuryAddresses(
        ...
        require(_partnerAddress != address(0), "Cannot be zero address");
        require(_otherAddress != address(0), "Cannot be zero address");

        treasuryAddress = _treasuryAddress;
        partnerAddress = _partnerAddress;
        otherAddress = _otherAddress;

        emit NewOperatorAndTreasuryAddresses( msg.sender, _treasuryAddress, _partnerAddress, _otherAddress);
    }
    ...
    function setTreasuryFeesOnDraws(uint256 _treasury, uint256 _partner, uint256 _other) external onlyOperator {
        treasuryAmount = _treasury;
        partnerAmount = _partner;
        otherAmount = _other;
    }

    function setTreasuryFeesOnBuys(uint256 _treasury, uint256 _partner, uint256 _other) external onlyOperator {
        treasuryAmountOnBuys = _treasury;
        partnerAmountOnBuys = _partner;
        otherAmountOnBuys = _other;
    }
    ...
    function getTotalNFTTicketsAvailable(address _user) public view returns (uint256 _total) {
        ...
        for(uint32 j = 0; j < collections.length; j++) {
            ...
            for(uint i = 0; i < tokenIds.length; i++) {
                if(NFTC.getTokenRarity(tokenIds[i]) <= raritySetting[collections[j]]) _total++;
            }
        }
        return (_total);
    }

    function checkReqs(uint256 howMany) internal {
        if (BNBfee > 0) {
            require(address(this).balance >= (BNBfee * howMany), "Not enough BNB");
        ...
        payable
    {
        ...
        // get total or Max available free tickets
        uint256 userTotal = getTotalNFTTicketsAvailable(msg.sender);
        require(userTotal > 0 && _howMany <= userTotal, "you dont have that many Free Tickets");
        ...
        emit TicketsPurchase(msg.sender, _lotteryId, userTotal);
    }
    ...
    function addCollection(address newCollection, uint256 _raritySetting) external onlyOwner{
        require(!isCollection[newCollection], "already added");
        collections.push(newCollection);
        raritySetting[newCollection] = _raritySetting;
    ...
    function removeCollection(address oldCollection) external onlyOwner{
        ...
        for(uint32 i = 0; i < collections.length; i++) {
            if(collections[i] == oldCollection){
                ...
                delete raritySetting[oldCollection];
            }
        }
