...
contract MBF_Router is IMBF_Router, Ownable {
    ...
    modifier ensure(uint deadline) {
        require(deadline >= block.timestamp, 'MBF_Router: EXPIRED');
        _;
    }

    constructor(address _factory, address _WBNB,uint256 _threshold) public {
        factory = _factory;
        WBNB = _WBNB;
    ...
    function pairFor(address tokenA, address tokenB) public view returns (address pair){
        pair = IMBF_Factory(factory).pairFor(tokenA, tokenB);
    }
    ...
    function setDefaultThreshold(uint256 _threshold) external onlyOwner{
        require(_threshold <= 10000, "MBF_Router: Numerical overflow");
        defaultThreshold = _threshold;
    }
    ...
    function multSetRestrictPair(address[] calldata _pairs) external onlyOwner {
        require(_pairs.length > 0, "MBF_Router: Length is zero");
        for (uint256 index = 0; index < _pairs.length; index++) {
            address _pair = _pairs[index];
            require(!restrict[_pair], "MBF_Router: Pair has been set");
            restrict[_pair] = true;
    ...
    function multDelRestrictPair(address[] calldata _pairs) external onlyOwner {
        ...
        for (uint256 index = 0; index < _pairs.length; index++) {
            address _pair = _pairs[index];
            require(restrict[_pair], "MBF_Router: Pair is not set");
    ...
    function _addLiquidity(
        ...
        if (IMBF_Factory(factory).getPair(tokenA, tokenB) == address(0)) {
            IMBF_Factory(factory).createPair(tokenA, tokenB);
        }
        (uint reserveA, uint reserveB) = IMBF_Factory(factory).getReserves(tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint amountBOptimal = IMBF_Factory(factory).quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'MBF_Router: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = IMBF_Factory(factory).quote(amountBDesired, reserveB, reserveA);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, 'MBF_Router: INSUFFICIENT_A_AMOUNT');
    ...
    function addLiquidity(
        ...
        liquidity = IMBF_Pair(pair).mint(to);
    }
    ...
    function removeLiquidity(
        ...
        address pair = pairFor(tokenA, tokenB);
        IMBF_Pair(pair).transferFrom(msg.sender, pair, liquidity);
        // send liquidity to pair
        (uint amount0, uint amount1) = IMBF_Pair(pair).burn(to);
        (address token0,) = IMBF_Factory(factory).sortTokens(tokenA, tokenB);
        (amountA, amountB) = tokenA == token0 ? (amount0, amount1) : (amount1, amount0);
        require(amountA >= amountAMin, 'MBF_Router: INSUFFICIENT_A_AMOUNT');
        require(amountB >= amountBMin, 'MBF_Router: INSUFFICIENT_B_AMOUNT');
    }
    ...
    function removeLiquidityWithPermit(
        ...
        IMBF_Pair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            (address token0,) = IMBF_Factory(factory).sortTokens(input, output);
            ...
            if(restrict[pair]) {
                (uint112 reserve0, uint112 reserve1,) = IMBF_Pair(pair).getReserves();
                uint256 pairThreshold = getThreshold(pair);
                require(amount0Out.mul(1e8).div(reserve0) <= pairThreshold && amount1Out.mul(1e8).div(reserve1)<= pairThreshold, "MBF_Router: A single transaction exceeds the limit");
            }
            address to = i < path.length - 2 ? pairFor(output, path[i + 2]) : _to;
            IMBF_Pair(pairFor(input, output)).swap(
    ...
    function swapExactTokensForTokens(
        ...
        amounts = IMBF_Factory(factory).getAmountsOut(amountIn, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'MBF_Router: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    function swapTokensForExactTokens(
        ...
        amounts = IMBF_Factory(factory).getAmountsIn(amountOut, path);
        require(amounts[0] <= amountInMax, 'MBF_Router: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, pairFor(path[0], path[1]), amounts[0]
        );
        _swap(amounts, path, to);
    ...
    returns (uint[] memory amounts)
    {
        require(path[0] == WBNB, 'MBF_Router: INVALID_PATH');
        amounts = IMBF_Factory(factory).getAmountsOut(msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'MBF_Router: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WBNB, 'MBF_Router: INVALID_PATH');
        amounts = IMBF_Factory(factory).getAmountsIn(amountOut, path);
        require(amounts[0] <= amountInMax, 'MBF_Router: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, pairFor(path[0], path[1]), amounts[0]
    ...
    returns (uint[] memory amounts)
    {
        require(path[0] == WBNB, 'MBF_Router: INVALID_PATH');
        amounts = IMBF_Factory(factory).getAmountsIn(amountOut, path);
        require(amounts[0] <= msg.value, 'MBF_Router: EXCESSIVE_INPUT_AMOUNT');
        IWBNB(WBNB).deposit{value : amounts[0]}();
        assert(IWBNB(WBNB).transfer(pairFor(path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        // refund dust eth, if any
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = IMBF_Factory(factory).sortTokens(input, output);
            IMBF_Pair pair = IMBF_Pair(pairFor(input, output));
            uint amountInput;
            uint amountOutput;
            {// scope to avoid stack too deep errors
                ...
                amountOutput = IMBF_Factory(factory).getAmountOut(amountInput, reserveInput, reserveOutput, input, output);
            }
            if (swapMining != address(0)) {
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        require(
            ...
            'MBF_Router: INSUFFICIENT_OUTPUT_AMOUNT'
        );
    }
    ...
    ensure(deadline)
    {
        ...
        require(amountOut >= amountOutMin, 'MBF_Router: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    function quote(uint256 amountA, uint256 reserveA, uint256 reserveB) public view override returns (uint256 amountB) {
        return IMBF_Factory(factory).quote(amountA, reserveA, reserveB);
    }
    ...
    function getAmountOut(uint256 amountIn, uint256 reserveIn, uint256 reserveOut, address token0, address token1) public view override returns (uint256 amountOut){
        return IMBF_Factory(factory).getAmountOut(amountIn, reserveIn, reserveOut, token0, token1);
    }
