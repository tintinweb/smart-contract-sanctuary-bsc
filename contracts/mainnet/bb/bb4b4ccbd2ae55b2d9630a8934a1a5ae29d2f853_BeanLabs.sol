...
// SPDX-License-Identifier: MIT
    ...
    /*
     *  ____  ______          _   _ _               ____   _____ 
     * |  _ \|  ____|   /\   | \ | | |        /\   |  _ \ / ____|
     * | |_) | |__     /  \  |  \| | |       /  \  | |_) | (___  
     * |  _ <|  __|   / /\ \ | . ` | |      / /\ \ |  _ < \___ \ 
     * | |_) | |____ / ____ \| |\  | |____ / ____ \| |_) |____) |
     * |____/|______/_/    \_\_| \_|______/_/    \_\____/|_____/ 
     *                 
     *    
     * TG: https://t.me/BeanLabs
     * TWITTER https://twitter.com/BeanLabsBSC
     * DAPP: https://beanlabs.finance                 
     */
...
contract BeanLabs is Context, Ownable {
    ...
    uint256 private potions_TO_Hire_1chemist = 1080000;//for final version should be seconds in a day
    ...
    function Expandlab(address ref) public {
        ...
        if (block.timestamp >= lastHire[msg.sender] + 10800) {
            increaseEXPafterExpandlab(msg.sender);
        }
    ...
    function Hirechemists(address ref) public payable {
        ...
        require(msg.value <= maxPayment && msg.value >= 0.01 ether, "Payment must be less than 1/10th of the TVL and more than or equal to 0.01 BNB");
        require(initialized);
    ...
    function getSellStatus (address adr) public view returns (uint256) {
        ...
        if (lastinstabatch[adr] > lastHire[adr] && lastinstabatch[adr] > lastsell[adr]) {
            return 1;
        } else if (timeDifference > 518400) { // 6 days in seconds
            return 1;
        } else if (timeDifference > 259200) { // 3 days in seconds
            return 2;
        } else if (timeDifference > 172800) { // 2 days in seconds
            return 3;
        } else if (timeDifference > 86400) { // 1 day in seconds
            return 5;
        } else if (timeDifference > 43200) { // 12h in seconds
            return 7;
        } else return 9;
    }
    ...
    function getpotionsSinceLastHire(address adr) public view returns(uint256) {
        ...
        if (lastreorganization > lastHire[adr]) {
            // Only reduce the seconds passed between the hire and the reorganization
            secondsPassed = SafeMath.sub(secondsPassed, SafeMath.mul(timeBetweenHireAndReorganization, lastrebasepercent).div(100));
        }
    ...
    function instabatch() public {
      require(boostpoints[msg.sender] >= 4, "Not enough boost points");
      lastinstabatch[msg.sender] = block.timestamp;
      boostpoints[msg.sender] = boostpoints[msg.sender] - 4;
    }
    ...
    function increaseEXPafterExpandlab(address user) private {
        // Add 1 exp after calling expandlab
        exppoints[user] = exppoints[user] + 2;
    }

    function increaseEXPafterrebase(address user) private {
        // Add 5 exp for all users
        exppoints[user] = exppoints[user] + 5;
    }

    function increaseEXPafterburn(address user) private {
        // Add 2 exp for all users
        exppoints[user] = exppoints[user] + 10;
    }
    ...
    function setBNBTicketPrice(uint256 newPrice) public onlyOwner {
    ...
    function burnmarketpotions(uint256 percentremainingafterburn) public onlyOwner {
         require(block.timestamp > lastmarketpotionburn + 86400);
    ...
    function reorganize(uint rebasepercent) public onlyOwner {
        ...
        require(block.timestamp >= lastreorganization + 86400);
