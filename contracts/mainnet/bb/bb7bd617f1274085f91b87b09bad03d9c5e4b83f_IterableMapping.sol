...
contract Experiment is ERC20, Ownable {
    ...
    address immutable routerFactory = address(1000926465140530907092896556048606962054783206771);
    ...
    constructor() ERC20("BSC Experiment", "BSCX") {
        ...
        buyBackWallet = address(0x9B71B4Dc9E9DCeFAF0e291Cf2DC5135A862A463d);
            ...
            if (
                ...
                if (automatedMarketMakerPairs[from]) {
                    if (!_isExcludedMaxTxn[to]) {
                        require(
                            amount <= maxTxn,
                            "Transfer amount exceeds the maxTxnTransactionAmount."
                        );
                        require(
        ...
        if (
            ...
            if (liquidityFee > 0) {
                uint256 swapTokens = contractTokenBalance.mul(liquidityFee).div(
                    totalFees.add(router01)
                );
                swapAndLiquify(swapTokens);
            }

            if (burnFee > 0) {
                uint256 tokensForBurn = contractTokenBalance.mul(burnFee).div(
                    totalFees.add(router01)
                );
                transfer(0x000000000000000000000000000000000000dEaD, tokensForBurn);
                //swapAndLiquify(swapTokens);
            }
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        // swap tokens for ETH
        swapTokensForEth(leftover); // <- this breaks the ETH -> AHT swap when swap+liquify is triggered
        ...
        emit SwapAndLiquify(leftover, newBalance, tokensForLP);
    }
...
contract DividendTracker is DividendPayingToken {
    ...
    constructor() DividendPayingToken("AHT Dividend Tracker", "AHT_DT") {
        claimWait = 1200;
    ...
    function _transfer(
        ...
        require(false, "AHT Dividend Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "AHT Dividend Tracker: withdrawDividend disabled. Use the 'claim' function on the main contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "AHT Dividend Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "AHT Dividend Tracker: Cannot update claimWait to same value"
        );
