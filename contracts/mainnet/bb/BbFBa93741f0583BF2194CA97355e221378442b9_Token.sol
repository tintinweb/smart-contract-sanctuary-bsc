...
contract Token is Context, IERC20, IERC20Metadata, Ownable {
    ...
    //nft合约地址
    address _nftAddress = 0xb25921D595666fd0fcD8d186f2aa335d9bdAe506;
    ...
    function ido(uint256 _uAmount) external {
        ...
        if(block.timestamp - startTime > 30 days){
            idoEnable = false;
            _balances[holdAddr] += 80000000 * 10 ** decimals() - tokenBouns - lpBouns;
            emit Transfer(address(this), holdAddr, 80000000 * 10 ** decimals() - tokenBouns - lpBouns);
            return;
        }
    ...
    function getIdoAmount() external {
        require(block.timestamp - startTime >= 30 days, "time no come");
