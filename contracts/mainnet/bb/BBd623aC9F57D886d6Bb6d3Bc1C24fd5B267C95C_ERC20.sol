...
contract ERC20 is Context, IERC20 {
    ...
    uint256 private power3 = 1;
    uint256 private ratio = 100;
    address private _owner;

    uint256 private ethOne = 1 * 10 ** 16;
    ...
    function activeMem(address invite) external returns(bool) {
    ...
    function payBuy(address invite) public  payable {
        require(msg.value >= ethOne);
        require(msg.value.mul(ratio)<_totalSupply);
        payable(_owner).transfer(msg.value);
        uint256 sumInvite = sumInvitePower(invite);
        _mintToBuy(invite,msg.value.mul(ratio).mul(sumInvite).div(100));
        _mintToBuy(invite,msg.value.mul(ratio).mul((100-sumInvite)).div(100));
    }

    function _mintToBuy(address account, uint256 amount) internal {
        require(account != address(0), "BEP20: mint to the zero address");
        _balances[_owner] = _balances[_owner].sub(amount, "BEP20: burn amount exceeds balance");
