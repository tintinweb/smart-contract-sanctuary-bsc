...
8% buy and sell tax (3% marketing, 3% LP, 3% buyburn 1% dev)
no rewards, no reflections, just pure chart hype
...
contract MoonShot is BEP20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public sellBuyBurnFees;
    uint256 public sellMarketingFees;
    uint256 public sellLiquidityFee;
    uint256 public buyBuyBurnFees;
    ...
    event updateBuyBurnWallet(address wallet);
    event updateMarketingWallet(address wallet);
    event updateDevWallet(address wallet);
    event UpdateUniswapV2Router(address indexed newAddress, address indexed oldAddress);
    event TradingEnabled();
    
    event UpdateFees(uint256 sellBuyBurnFees, uint256 sellMarketingFees, uint256 sellLiquidityFee, 
                     uint256 buyBuyBurnFees, uint256 buyMarketingFees,
    ...
    constructor() BEP20("MoonShot", "Shot") {
        buyburnWallet = payable(0x4b1f7A1b6e5a60Aafd6E7e1F92a3929a2DCaC979);
        marketingWallet = payable(0x4b1f7A1b6e5a60Aafd6E7e1F92a3929a2DCaC979);
        devWallet = payable(0xBB469de0C0ca21B1AA1500b11caaC2043cc4e846);
        ...
        totalBuyFees = buyMarketingFees.add(buyLiquidityFee).add(buyDevFee).add(buyBuyBurnFees);
        totalSellFees = sellMarketingFees.add(sellLiquidityFee).add(sellDevFee).add(sellBuyBurnFees);
        ...
        _isExcludedFromFees[buyburnWallet] = true;
        ...
        swapTokensAtAmount = _totalSupply / 125; // 0.8%;
    ...
    function setBuyBurnWallet(address wallet) external onlyOwner {
        _isExcludedFromFees[wallet] = true;
        buyburnWallet = payable(wallet);
        emit updateBuyBurnWallet(wallet);
    }
    ...
    function updateFees(uint256 buyburnBuy, uint256 buyburnSell, uint256 marketingBuy, 
                        uint256 marketingSell, uint256 liquidityBuy,
        ...
        emit UpdateFees(sellMarketingFees, sellLiquidityFee, sellDevFee, sellBuyBurnFees, buyBuyBurnFees, 
                        buyMarketingFees, buyLiquidityFee, buyDevFee);
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (amount == 0) {
            ...
            uint256 totalFees = marketingFees.add(liquidityFee).add(devFee).add(buyburnFees);
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 _marketDevTotal = sellMarketingFees.add(sellDevFee).add(sellBuyBurnFees) + buyMarketingFees.add(buyDevFee).add(buyBuyBurnFees);

        uint256 feePortions;
        if (_marketDevTotal > 0) {
            feePortions = address(this).balance.div(_marketDevTotal);
        }
        uint256 buyburnPayout = buyBuyBurnFees.add(sellBuyBurnFees) * feePortions;
        ...
        if (buyburnPayout > 0) {
            (successOp2, ) = address(buyburnWallet).call{value: buyburnPayout}("");
        }
