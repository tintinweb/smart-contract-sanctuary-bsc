...
contract ASTToken is Context, IERC20, Ownable {
    ...
    uint256 public _taxAmount = 500000;    // 0.5%
    ...
    address public fundAddress = address(0xE40fFB324D4119e73B9ed3612575f865C69F0a04);
    address public liquidAddress = address(0xE40fFB324D4119e73B9ed3612575f865C69F0a04);
    ...
    function setTaxAmount(uint256 amount) external onlyOwner {
        _taxAmount = amount;
    }
    ...
    function _reflectFee() private {
        ...
        if (diff > 86400 ) {
            if (_tTaxFeeTotal < _maxTaxFeeTotal) {
                ...
                if (_taxAmount <= balance) {
                    uint256 rTaxFee = _taxAmount.mul(_getRate());
                    _rOwned[address(this)] = _rOwned[address(this)].sub(rTaxFee);
