...
contract SifuNftStakingV2 is Ownable, ReentrancyGuard {
...
//end of variables for dapp
    ...
    uint256 public rewardsPerHour = 1000000000;
  ...
  mapping (address => uint256) public ownerToNFTokenCount;
    ...
    constructor() {
        ...
        nftCollection = IERC721(0x495821f24Cd129002Ec9ebF01c588f05DDf0baC0);
        
        rewardsToken = IERC20(0x185DEB1FdEf8A1e5B76A13CC5b712C05C3AdbB30);
        } 
        // else if (block.chainid == 97) {
        // nftCollection = IERC721(0x057D7E3f4a88efa75C436C5F4e0bB184a5196304);
...
//Variables have been defined now we move on the rest of the configuration	
    ...
    // value of now.
    function stake(uint256[] calldata _tokenIds, uint256[] calldata _rarity) external nonReentrant {
        ...
        for (uint256 i; i < len; ++i) {
            ...
            rarityAmount += _tokenIds[i] * _rarity[i];
    ...
    function multisendNft(uint256[] calldata _tokenIds, address[] calldata recipient) external nonReentrant onlyOwner{
        for (uint256 i; i < recipient.length; ++i) {
													   
            nftCollection.transferFrom(address(this), recipient[i], _tokenIds[i]);													  
            stakerAddress[_tokenIds[i]] = msg.sender;
