...
contract StakePoolTokenForBNB is States, ReentrancyGuard, Ownable {
    ...
    function setTokens(uint256 _earlyUstakeFee, IBEP20 _stakingToken, uint256 _lockPeriod, uint256 _earlyRewardUstakeFee, bool _earlyUstake) public onlyOwner {
    ...
    function setData(uint256 _days, uint256 _earlyUstakeFee, uint256 _earlyRewardUstakeFee) public payable onlyOwner {
        lockPeriod = _days;
        earlyUstakeFee = _earlyUstakeFee;        
        earlyRewardUstakeFee = _earlyRewardUstakeFee;
    }
    ...
    function _unstakeTokens() internal nonReentrant{
        User memory wUser = deposit[_msgSender()];
    ...
    function EarlyUnstakeTokens() internal nonReentrant{
        ...
        stakingToken.transfer(owner(), _fee);
    ...
    function calculateApy() public view returns (uint256){
        return address(this).balance.mul(10**18).div(totalStaked);
    }
...
contract FactoryStakingPool is Ownable{
    ...
    function createTokenForBNB(address ownerAddress, uint256 _earlyUstakeFee, IBEP20 _stakingToken, uint256 _lockPeriod, uint256 _earlyRewardUstakeFee, bool _earlyUstake) public onlyOwner returns (address)
    {
        StakePoolTokenForBNB toDeploy = new StakePoolTokenForBNB();
        toDeploy.setTokens(_earlyUstakeFee, _stakingToken, _lockPeriod, _earlyRewardUstakeFee, _earlyUstake);
