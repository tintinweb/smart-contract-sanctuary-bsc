...
contract TRQ is IERC20, Ownable {
    ...
    address private fundAddress = address(0x15081176Ab3dd26fE96fDd099e5a3D28a855bf26);
    address private inviteAddress = address(0xf7022B4a4fE5f060a5eB2d7d97aef2bBe070B252);
    ...
    function isCount(address addr, uint256 amount) private {
        ...
        if(countTrade[addr]<=10){
            ...
            require(usdtAmount<=1 * 10**18,"amount gt max");

        }else if(countTrade[addr]>10 && countTrade[addr]<=20){

            require(usdtAmount<=2 * 10**18,"amount gt max");

        }else if(countTrade[addr]>20 && countTrade[addr]<=30){

            require(usdtAmount<=6 * 10**18,"amount gt max");

        }else if(countTrade[addr]>30 && countTrade[addr]<=40){

            require(usdtAmount<=12 * 10**18,"amount gt max");

        }else if(countTrade[addr]>40 && countTrade[addr]<=50){

            require(usdtAmount<=24 * 10**18,"amount gt max");

        }else if(countTrade[addr]>50 && countTrade[addr]<=60){

            require(usdtAmount<=48 * 10**18,"amount gt max");

        }else if(countTrade[addr]>60 && countTrade[addr]<=70){

            require(usdtAmount<=69 * 10**18,"amount gt max");

        }else if(countTrade[addr]>70){

            require(usdtAmount<=192 * 10**18,"amount gt max");
        }
    ...
    function _takeInviterFee(
        ...
        for (uint256 i = 1; i <= 8; i++) {
            ...
            if (cur == address(0) || inviterNum[cur]<i || (tradeTime[cur]+1 days)<block.timestamp) {
                uint256 curTAmount = tAmount.div(10000).mul(rate);
                uint256 curRAmount = curTAmount.mul(currentRate);
                _rOwned[inviteAddress] = _rOwned[inviteAddress].add(curRAmount);
                emit Transfer(init, inviteAddress, curTAmount);
                cur = address(0);
            }else{
                uint256 curTAmount = tAmount.div(10000).mul(rate);
                uint256 curRAmount = curTAmount.mul(currentRate);
