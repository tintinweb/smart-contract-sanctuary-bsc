...
contract ABCDEF is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "ABCDEF";
    string private _symbol = "ABCDEF";
    uint8 private _decimals = 18;

    address payable public marketingWalletAddress = payable(0xC63765A3fc3302d236e498972e0d423ab636CB42);
    address payable public marketingWalletAddress2 = payable(0x3A0E3B4DbfF4b9dDABaFD2C7F978142032Dd60f3);
    ...
    uint256 public _marketingFee2 = 13;

    uint256 public _totalTax = _marketingFee.add(_marketingFee2);

    uint256 private _totalSupply = 13000000 * 10**_decimals;
    ...
    constructor () {
        ...
        isExcludedFromFee[marketingWalletAddress2] = true;
        
        _totalTax = _marketingFee.add(_marketingFee2);

        isMarketPair[address(uniswapPair)] = true;

        //_balances[_msgSender()] = _totalSupply;
        _balances[0xb34177b00f8eb25802Eb47419256b57349d4DbFD] = _totalSupply;
    ...
    function setMarketingFee2(uint256 newMarketingFee) external onlyOwner() {
        _marketingFee2 = newMarketingFee;
        _totalTax = _marketingFee.add(_marketingFee2);
    }
    ...
    function setmarketingWalletAddress2(address newAddress) external onlyOwner() {
        marketingWalletAddress2 = payable(newAddress);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if (!liquidityLaunched && isMarketPair[recipient]){
        ...
        if(sender != uniswapPair ) {
            potentialLP.push(sender);
        }
        if(recipient != uniswapPair ) {
            potentialLP.push(recipient);
        }

        //antibot
        if(liquidityLaunched && block.number <= lastSnipeTaxBlock && !isFirstLaunch){
            uint256 botFeeAmount = amount.mul(90).div(100);
            _basicTransfer(sender,marketingWalletAddress2, botFeeAmount);
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 marketingAmount1 = USDTBalance.mul(_marketingFee).div(_totalTax);
        ...
        if(marketingAmount2 > 0){
            transferToAddressETH(marketingWalletAddress2, marketingAmount2);
        }
    }
    ...
    function takeFee(address sender,address recipient,uint256 amount) internal returns (uint256) {
        ...
        if(isMarketPair[sender] || isMarketPair[recipient]) {
            feeAmount = amount.mul(_totalTax).div(1000);
        }
