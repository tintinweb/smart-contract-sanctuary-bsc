...
contract wGlobalDAOToken is ERC20 {
    ...
    constructor(address _sGLBD) ERC20("Wrapped sGLBD", "wsGLBD") {
    ...
    function wrap(uint256 _amount) external returns (uint256) {
        IERC20(sGLBD).transferFrom(msg.sender, address(this), _amount);

        uint256 value = sGLBDTowGLBD(_amount);
    ...
    function unwrap(uint256 _amount) external returns (uint256) {
        ...
        uint256 value = wGLBDTosGLBD(_amount);
        IERC20(sGLBD).transfer(msg.sender, value);
        return value;
    }

    /**
        @notice converts wGLBD amount to sGLBD
        @param _amount uint
        @return uint
     */
    ...
    /**
        @notice converts sGLBD amount to wGLBD
        @param _amount uint
        @return uint
     */
