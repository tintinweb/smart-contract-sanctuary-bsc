...
contract OrbFlexiblePool is Ownable, Pausable {
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint256 shares; // number of shares for a user
        uint256 lastDepositedTime; // keeps track of deposited time for potential penalty
        uint256 orbAtLastUserAction; // keeps track of orb deposited at the last user action
        uint256 lastUserActionTime; // keeps track of the last user action time
    }

    IERC20 public immutable token; // Orb token
    IOrbPool public immutable orbPool; //Orb pool
    ...
    //When call orbpool.withdrawByAmount function,there will be a loss of precision, so need to withdraw more.
    uint256 public withdrawAmountBooster = 10001; // 1.0001
    uint256 public performanceFee = 200; // 2%
    uint256 public withdrawFee = 10; // 0.1%
    uint256 public withdrawFeePeriod = 72 hours; // 3 days

    event DepositOrb(address indexed sender, uint256 amount, uint256 shares, uint256 lastDepositedTime);
    ...
    /**
     * @notice Constructor
     * @param _token: Orb token contract
     * @param _orbPool: OrbPool contract
    ...
    constructor(
        ...
        // Infinite approve
        IERC20(_token).safeApprove(address(_orbPool), type(uint256).max);
    }
    ...
    /**
     * @notice Deposits funds into the Orb Flexible Pool.
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in ORB)
     */
    function deposit(uint256 _amount) external whenNotPaused {
        ...
        if (user.shares > 0) {
            ...
            uint256 earnAmount = totalAmount - user.orbAtLastUserAction;
            ...
            if (currentPerformanceFee > 0) {
                ...
                if (_amount >= currentPerformanceFee) {
                    ...
                    orbPool.withdrawByAmount(withdrawAmount);
        ...
        user.orbAtLastUserAction = (user.shares * balanceOf()) / totalShares;
        user.lastUserActionTime = block.timestamp;

        emit DepositOrb(msg.sender, _amount, currentShares, block.timestamp);
    }

    /**
     * @notice Withdraws funds from the Orb Flexible Pool
    ...
    /**
     * @notice Withdraws from Orb Pool without caring about rewards.
     * @dev EMERGENCY ONLY. Only callable by the contract admin.
     */
    function emergencyWithdraw() external onlyAdmin {
        require(staking, "No staking orb");
        staking = false;
        orbPool.withdrawAll();
    }

    /**
     * @notice Withdraw unexpected tokens sent to the Orb Flexible Pool
     */
     ...
     * @dev It includes tokens held by the contract and held in OrbPool
     */
    function balanceOf() public view returns (uint256) {
        (uint256 shares, , , , , , , , ) = orbPool.userInfo(address(this));
        uint256 pricePerFullShare = orbPool.getPricePerFullShare();

        return token.balanceOf(address(this)) + (shares * pricePerFullShare) / 1e18;
    }

    /**
     * @notice Deposits tokens into OrbPool to earn staking rewards
     */
    function _earn() internal {
        uint256 bal = available();
        if (bal > 0) {
            orbPool.deposit(bal, 0);
        }
    }
}
