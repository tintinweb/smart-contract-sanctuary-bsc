...
contract SupremeLottery is VRFConsumerBaseV2, Ownable, Pausable {
    ...
    uint256 public supPerEntry = 100000;
    ...
    function setSupPerEntry(uint256 _sup) external onlyOwner {
        supPerEntry = _sup;
    }
    ...
    function previousWinner() external view returns (address) {
        if ( lotteryID == 1)
            return address(0x03861288cc5c2714e8c00Fc63ee6b0A0b306F3e9);
    ...
    function enter(uint256 tickets) external whenNotPaused {
        ...
        token.transferFrom(msg.sender, address(this), tickets * 10**18 *supPerEntry);
    }
    ...
    function payoutLottery() external onlyOwner{
        ...
        if (players.length > 0) {
            //uint256 totalAmount = players.length * 10**18 * supPerEntry;
