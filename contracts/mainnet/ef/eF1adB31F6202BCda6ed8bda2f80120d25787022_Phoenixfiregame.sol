...
contract Phoenixfiregame is Ownable, ReentrancyGuard {
    ...
    uint256[64] public _revealDates = [
                                        1659301200, 1662094800, 1662138000, 1662181200, 1662224400, 1662310800, 1662397200, 1662483600, 1662570000, 1662742800,
    ...
    function setLeader(address account, bool isLeader) external payable onlyOwner {
        ...
        if (users[account].isLeader && !isLeader) {
            ...
            require(msg.value >= _bonusPerUser, "You must send BNB for this user to receive bonus");
    ...
    function addLeaders(address[] calldata accounts) external payable onlyOwner {
        require(msg.value >= accounts.length * _bonusPerUser, "Insufficient BNB sent to add leaders");
    ...
    function processPayment(address account, uint256 level) internal {
        ...
        _bonusPerUser += toPool / getUsersReceivingBonus();
    ...
    function buyLevelByOwner(address _account, uint256 _level) public payable onlyOwner {
        ...
        if (!users[_account].isActive && !users[_account].isLeader) {
     ...
     *
     */
    function getUsersReceivingBonus() public view returns(uint256) {
        uint256 result = _activeUsers + _leaders.length;
        for (uint i = 0; i < _leaders.length; i++) {
    ...
    function getAvailableBonus(address account) public view returns(uint256) {
        ...
        return _bonusPerUser - users[account].claimed;
    }
