...
contract GymFarming is OwnableUpgradeable, ReentrancyGuardUpgradeable, RewardRateConfigurable {
        ...
        onlyMlmParticipant
    {
        ...
        if (
            !ICommissionActivation(0x3E1240E879b4613C7Ae6eE1772292FC80B9c259e)
                .getCommissionActivation(msg.sender, 2)
        ) {
            _activatePendingCommissions();
        }

        _updateUserInfo(_pid, msg.sender, liquidityData);
        IGymMLM(relationship).distributeCommissions(liquidityData.lpTokensReceived, _pid, 2, true, msg.sender);
    ...
    function depositFromOtherContract(
        ...
        _refundRemainderTokens(_from, poolBaseToken, result);
         ...
         IGymMLM(relationship).distributeCommissions(result.lpTokensReceived, _pid, 2, true, _from);
    ...
    function withdraw(uint256 _pid) external nonReentrant poolExists(_pid) onlyMlmParticipant {
        ...
        if (
            ...
            IGymMLM(relationship).distributeCommissions(withdrawAmount, _pid, 2, false, msg.sender);
        }
    ...
    function _deposit(
        ...
        IGymMLM(relationship).distributeCommissions(result.lpTokensReceived, _pid, 2, true, msg.sender);
