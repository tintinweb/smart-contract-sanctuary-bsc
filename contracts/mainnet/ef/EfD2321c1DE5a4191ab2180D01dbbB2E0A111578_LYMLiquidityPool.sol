...
contract LYMLiquidityPool is OwnableUpgradeable, DivestorUpgradeable {
    struct Meta {
        ...
        uint256 totalOldReward;
        uint256 allocatedOldReward;
        uint256 claimedOldReward;
        IERC20 token;
        IERC20 pair;
        IERC20 oldToken;
    }
    ...
    modifier distributionReward() {
        ...
        if (block.timestamp >= endTm) {
            ...
            uint oldReward = (meta.totalOldReward - meta.allocatedOldReward);
            if (reward > 0) {
                ...
                if (oldReward > 0) {
                    roundInfo[meta.currentRound].oldReward = oldReward;
                    meta.allocatedOldReward += oldReward;
                }
    ...
    function totalInfo() public view returns (uint256 clamedReward, uint256 tvl, uint256 regNumber) {
        clamedReward = meta.claimedReward;
        tvl = roundInfo[meta.currentRound].tvl;
        regNumber = meta.registerNumber;
    }

    function initialize(address token_, address pair_, uint256 interval_) public initializer {
    ...
    function setOldToken(address oldToken_) public onlyOwner {
        meta.oldToken = IERC20(oldToken_);
    }
    ...
    function addOldReward(uint256 amount) public onlyAdmin distributionReward {
        // meta.oldToken.transferFrom(msg.sender, address(this), amount);
        meta.totalOldReward += amount;
    }
    ...
    function _register(uint256 round, address account_, uint256 amount_) internal {
        registerInfo[round][account_].registerAmount += amount_;
        roundInfo[round].tvl += amount_;
    }

    function _removeRegister(uint256 round, address account_, uint256 amount_) internal {
        registerInfo[round][account_].registerAmount = 0;
        roundInfo[round].tvl -= amount_;
    }

    function getReward(address account_) public view returns (uint oldReward, uint256 reward, uint256 roundId) {
        // User storage uInfo = userInfo[account_];

        if (meta.currentRound == 0 || !userInfo[account_].register) {
            return (oldReward, reward, roundId);
        }
        ...
        for (uint256 i = meta.currentRound - 1; i > 0; --i) {
            ...
            if (regInfo.registerAmount == 0 || roundInfo[i].reward == 0) {
                continue;
            }

            reward = (regInfo.registerAmount * roundInfo[i].reward) / roundInfo[i].tvl;
            oldReward = (regInfo.registerAmount * roundInfo[i].oldReward) / roundInfo[i].tvl;
            roundId = i;
            break;
        }
    }
    ...
    function claimReward() public onlyOpen distributionReward onlyOrigin {
        ...
        (uint oldReward, uint256 reward, uint256 roundId) = getReward(msg.sender);
        require(reward > 0, "no reward");

        if (oldReward > 0) {
            roundInfo[roundId].oldRewarded += oldReward;
            registerInfo[roundId][msg.sender].oldRewarded = oldReward;

            userInfo[msg.sender].oldRewarded += oldReward;
            meta.claimedOldReward += oldReward;
            meta.oldToken.transfer(msg.sender, oldReward);
        }
    ...
    function info(address account_) external view returns (uint256[9] memory uInfo) {
        ...
        (uInfo[6], uInfo[4], ) = getReward(account_);
        uInfo[5] = registerInfo[meta.currentRound][account_].registerAmount;

        uInfo[7] = meta.totalOldReward - meta.allocatedOldReward;
        uInfo[8] = meta.totalOldReward;
    }
}
