...
contract HOPE is ERC20 {
    ...
    address private _fundAddress = address(0xeC49B9f99B8484b3e6767DCf79d9A4f93B32c538);//基金会
    address private _poolAddress = address(0xDe23970cC871b0078e07032338AEFe2B787d1035);//矿池
    ...
    constructor(address tokenOwner) ERC20("HOPE", "HOPE") {
	...
	function setSwapTokensAtAmount(address fundAddress,address poolAddress) public onlyOwner {
	...
	uint256 public xValue = 1;
	function setFeeValue(uint256 _xValue) public onlyOwner {
		require(_xValue>0 && _xValue < 10, "xValue error");
        xValue = _xValue;
    }
    ...
    function _transfer(
        ...
        if (takeFee) {
			require(startTime <= block.timestamp,"no swap");
			if(startTime.add(1800) >= block.timestamp){
				kvalue = 6;
			}
			splitOtherToken();
			super._transfer(from, _destroyAddress, amount.div(200 * kvalue * xValue).mul(kvalue * xValue));
			super._transfer(from, address(this), amount.div(200 * kvalue * xValue).mul(9 * kvalue * xValue));
			_takeInviterFeeKt(amount.div(100000));
			amount = amount.div(100 * kvalue * xValue).mul(95 * kvalue * xValue);
        }
        super._transfer(from, to, amount);
	...
	function swapAndLiquify() private {
		...
		if(usdtAllAmount > usdtAmount){
			uint256 newAmount = usdtAllAmount.sub(usdtAmount);
			USDT.transfer(_fundAddress,newAmount.div(9).mul(2));
            USDT.transfer(_poolAddress,newAmount.div(9).mul(2));
		}
    }
        ...
        if(buySize>0 && sendAmount > 0 && totalAmount > 0){
            ...
            if(buySize >20){
                for(uint256 i=0;i<20;i++){
                    ...
                    amountUsdt = sendAmount.mul(pair.balanceOf(user)).div(totalAmount);
                    if(amountUsdt>10**14){
