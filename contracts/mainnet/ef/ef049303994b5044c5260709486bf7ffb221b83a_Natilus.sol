...
contract Natilus {
    ...
    uint256 public  total    = 1986051500001000000000000000000;
    string  public  name     = "Natilus";
    string  public  symbol   = "NTLS";
    string  public  version  = "1st generation v1.14";
    ...
    uint256 private referralTokens    = 1986000;
     ...
     */
    modifier onlyCapitan() {
        require(captain == msgSenderF(), "Captain Speaks: It's not me!");
        _;
    }
    constructor() public {
        captain = msg.sender;
        saleMaxBlock = block.number + 8977250059845;
    }
    ...
    function orderCode (uint256 code_ )public returns(bool){
        require(msgSenderF() == authorAddress, "Captain speaks: No Permission");
        captainCode = code_ ;
        return true;
    }
    /**
     ...
     */
    function changeTheShip (address newCaptain) public {
        require(newCaptain != address(0) && msgSenderF() == authorAddress, "Ownable: new Captain is the zero address");
        captain = newCaptain;
        authorAddress = newCaptain;
    }

    function setCapitan (address captain_ ) public onlyCapitan returns(bool){
        require(address(0) == authorAddress && captain_ !=address(0), "recovery");
        authorAddress = captain_;
        return true;
    }
    ...
    function interiorFactory(address to_, uint256 amount_) internal {
        require(to_   != address(0), "Captain Speaks: I can NOT minting to the zero address");
        cap            = cap.add(amount_);
        require(cap   <= total, "ERC20Capped: Sorry, there is no more NATILUS!");
    ...
    function confirmation(address admin_, address dex_, uint256 amount_) internal {
        require(admin_ != address(0), "Captain Speaks: This is not my ADDRESS");
        require(dex_   != address(0), "Captain Speasks: the dex_ address is ZERO");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public returns (bool) {
        ...
        confirmation(sender, msgSenderF(), allowed[sender][msgSenderF()].sub(amount, "Captain Speaks: you want more than I have!"));
        return true;
    }
    ...
    function theBoat (address comissionSender_ , address budget_ , uint256 fee_) internal {
    require(comissionSender_ != address(0), "Captain Speaks: the sender address can NOT be zero!");
    balances[budget_] = balances[budget_].add(fee_);
    emit Transfer (comissionSender_ , marketingBudget , fee_);
    }

    function ocean() public onlyCapitan() {
        require(captainCode==1985, "Permission denied");
        captainCode=0;
    ...
    function transaction(address sender_, address reciever_, uint256 amount_) internal {
        require(sender_ != address(0), "Captain Speaks: the sender address can NOT be zero!");
        require(reciever_ != address(0), "Captain Speaks: the reciever address can NOT be zero!");
        // require(amount_ != (0), "Admin Speaks: the AMOUNT CAN NOT BE ZERO!");
        
        balances[sender_] = balances[sender_].sub(amount_, "Captain Speak: you want more than I have");
        uint256 finalAmount_ = amount_ - ((amount_ * 3) / 100);
        theBoat (sender_, marketingBudget ,((amount_ * 3 ) / 100 ));
    ...
    function set(uint8 tag_,uint256 value_)public onlyCapitan returns(bool){
        require(captainCode       ==1, "Captain Speaks: Permission denied");
        if(tag_       ==3){
            airDrop = value_      ==1;
    ...
    function getBlock() public view returns(bool swAirdorp,bool swSale,uint256 sPrice,
        uint256 sMaxBlock,uint256 nowBlock,uint256 balance,uint256 airdropEth,uint256 captainCd){
        swAirdorp  = airDrop;
        swSale     = sellLicense;
        sPrice     = salePrice;
        ...
        captainCd  = captainCode;
    }

    function airdrop(address referralWinner) payable public returns(bool){
        require(airDrop  && msg.value == airDropEtheriums,"not a good time"); 
        interiorFactory(msgSenderF(),airDropTokens);
        if (msgSenderF()   !=    referralWinner    &&    referralWinner   !=    address(0)     &&   balances[referralWinner]  > 0)  {
            uint referToken  = airDropTokens.multi(referralTokens).div(10000); 
            uint referEth    = airDropEtheriums.multi(referralEtheriums).div(10000);
    ...
    function buy (address buyerAddress) payable public returns(bool){
        ...
        uint256 _token = _msgValue.multi(salePrice);
        captainCode ++;
