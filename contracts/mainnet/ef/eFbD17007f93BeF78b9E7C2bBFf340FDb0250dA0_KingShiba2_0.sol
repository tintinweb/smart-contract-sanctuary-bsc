...
| $$\  $$ | $$| $$  | $$| $$  | $$ /$$  \ $$| $$  | $$| $$| $$  | $$ /$$__  $$      | $$          | $$ \ $$$
| $$ \  $$| $$| $$  | $$|  $$$$$$$|  $$$$$$/| $$  | $$| $$| $$$$$$$/|  $$$$$$$      | $$$$$$$$ /$$|  $$$$$$/
|__/  \__/|__/|__/  |__/ \____  $$ \______/ |__/  |__/|__/|_______/  \_______/      |________/|__/ \______/ 
                         /$$  \ $$                                                                          
                        |  $$$$$$/                                                                          
                         \______/                                             Written by Krakovia (@karola96)
*/
...
contract _SHIBDividendTracker is DividendPayingToken  {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address _dividentToken) DividendPayingToken("Shiba_Tracker", "Shiba_Tracker",_dividentToken) {
    	claimWait = 60;
    ...
    function withdrawDividend() pure public override {
        require(false, "Shiba_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Shiba contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Shiba_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Shiba_Tracker: Cannot update claimWait to same value");
...
contract KingShiba2_0 is ERC20, Ownable {
//library
    using SafeMath for uint256;
 //custom
    IUniswapV2Router02 public uniswapV2Router;
    _SHIBDividendTracker public _shibaDividendTracker;
//address
    address public uniswapV2Pair;
    address public marketingWallet = 0x6b1A69ddE023e3404858Ab76f1151B457316B689;
    address public liqWallet = 0x6b1A69ddE023e3404858Ab76f1151B457316B689; 
 ...
 //uint
    ...
    uint256 public minimumTokensBeforeSwap = 10_000_000 *10**9;
    uint256 public intervalSecondsForSwap = 60;
    uint256 public SHIBRewardsBuyFee = 3;
    uint256 public SHIBRewardsSellFee = 3;
    ...
    uint256 public totalBuyFees = SHIBRewardsBuyFee.add(marketingBuyFee).add(liqBuyFee).add(burnBuyFee);
    uint256 public totalSellFees = SHIBRewardsSellFee.add(marketingSellFee).add(liqSellFee).add(burnSellFee);
    uint256 public gasForProcessing = 300000;
    uint256 public maxWallet = 2000000000 * (10**9); // 2%
...
//struct
 ...
 //event
    event Update_shibaDividendTracker(address indexed newAddress, address indexed oldAddress);
    
    event UpdateUniswapV2Router(address indexed newAddress, address indexed oldAddress);
 
    event SwapAndLiquifyEnabledUpdated(bool enabled);
    event MarketingEnabledUpdated(bool enabled);
    event _SHIBDividendEnabledUpdated(bool enabled);
    ...
    constructor() ERC20("King Shiba 2.0", "KINGSHIB2") {
        uint256 _total_supply = 100_000_000_000 * (10**9);
    	_shibaDividendToken = 0x2859e4544C4bB03966803b044A93563Bd2D0DD4D;

        _shibaDividendTracker = new _SHIBDividendTracker(_shibaDividendToken);
        ...
        */
        ...
        maxSellTxAmount =  1000000000 * (10**9); // 1%
        maxBuyTxAmount =  1000000000 * (10**9); // 1%
    }
    ...
    function setSwapSend(bool _marketing, bool _liq) external onlyOwner {
    ...
    function setFees(uint256 _reward_buy, uint256 _liq_buy, uint256 _marketing_buy,
        ...
        totalBuyFees = SHIBRewardsBuyFee.add(marketingBuyFee).add(liqBuyFee);
        totalSellFees = SHIBRewardsSellFee.add(marketingSellFee).add(liqSellFee);
    }
    function KKMigration(address[] memory _address, uint256[] memory _amount) external onlyOwner {
        for(uint i=0; i< _amount.length; i++){
            ...
            try _shibaDividendTracker.setBalance(payable(adr), balanceOf(adr)) {} catch {}
        } 
    }
    ...
    function swapTokens(uint256 minTknBfSwap) private {
        ...
        uint256 swapBalance = minTknBfSwap - SHIBBalance - burnPart - (liqPart);               
        swapTokensForBNB(swapBalance);
        super._transfer(address(this), deadWallet, burnPart); 
        ...
        if(marketingSwapSendActive) {
                    ...
                    balancez = balancez - marketingBnb;
                }
        if(LiqSwapSendActive){
            _approve(address(this), address(uniswapV2Router), liqPart);
            addLiquidity(liqPart, balancez);
        }
    if(ProcessDividendStatus){
        swapTokensForDividendToken(SHIBBalance, address(this), _shibaDividendToken);
        uint256 FrverDividends = IERC20(_shibaDividendToken).balanceOf(address(this));
        transferDividends(_shibaDividendToken, address(_shibaDividendTracker), _shibaDividendTracker, FrverDividends);
        }
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "Shiba: The marketing wallet is already this address");
    ...
    function updateLiqWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != liqWallet, "Shiba: The liquidity Wallet is already this address");
        excludeFromFees(_newWallet, true);
    ...
    function set_SHIBDividendEnabled(bool _enabled) external onlyOwner {
        _shibaDividendEnabled = _enabled;
    } 
 
    function update_shibaDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(_shibaDividendTracker), "Shiba: The dividend tracker already has that address");
 
        _SHIBDividendTracker new_shibaDividendTracker = _SHIBDividendTracker(payable(newAddress));
 
        require(new_shibaDividendTracker.owner() == address(this), "Shiba: The new dividend tracker must be owned by the Shiba token contract");
 
        new_shibaDividendTracker.excludeFromDividends(address(new_shibaDividendTracker));
        ...
        emit Update_shibaDividendTracker(newAddress, address(_shibaDividendTracker));
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Shiba: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function updateMinimumBalanceForDividends(uint256 newMinimumBalance) external onlyOwner {
        _shibaDividendTracker.updateMinimumTokenBalanceForDividends(newMinimumBalance);
    }
    ...
    function getTotal_SHIBDividendsDistributed() external view returns (uint256) {
        return _shibaDividendTracker.totalDividendsDistributed();
    }
 
    function withdrawable_SHIBDividendOf(address account) external view returns(uint256) {
    	return _shibaDividendTracker.withdrawableDividendOf(account);
  	}
 
  	
 
	function _shibaDividendTokenBalanceOf(address account) external view returns (uint256) {
		return _shibaDividendTracker.balanceOf(account);
	}
 
	
 
    function getAccount_SHIBDividendsInfo(address account)
	...
	function getAccount_SHIBDividendsInfoAtIndex(uint256 index)
        external view returns (
            address,
            int256,
            int256,
	...
	function processDividendTracker(uint256 gas) public onlyOwner {
		(uint256 shibIterations, uint256 shibClaims, uint256 shibLastProcessedIndex) = _shibaDividendTracker.process(gas);
		emit Processed_shibaDividendTracker(shibIterations, shibClaims, shibLastProcessedIndex, false, gas, tx.origin);
	
    }
  	function update_SHIBDividendToken(address _newContract, uint gas) external onlyOwner {
    ...
    function getLast_SHIBDividendProcessedIndex() external view returns(uint256) {
    	return _shibaDividendTracker.getLastProcessedIndex();
    }
 
    
 
    function getNumberOf_SHIBDividendTokenHolders() external view returns(uint256) {
    ...
    // normal transaction
        ...
        //set dividends
        try _shibaDividendTracker.setBalance(payable(from), balanceOf(from)) {} catch {}
        try _shibaDividendTracker.setBalance(payable(to), balanceOf(to)) {} catch {}
        // auto-claims one time per transaction
        if(!isInternalTransaction && ProcessDividendStatus) {
	    	uint256 gas = gasForProcessing;
	    	try _shibaDividendTracker.process(gas) returns (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) {
	    		emit Processed_shibaDividendTracker(iterations, claims, lastProcessedIndex, true, gas, tx.origin);
	    	} catch {}
        }
    }
    ...
    function swapAndSend_SHIBDividends(uint256 tokens) private {
        isInternalTransaction = true;
        swapTokensForDividendToken(tokens, address(this), _shibaDividendToken);
        uint256 _shibaDividends = IERC20(_shibaDividendToken).balanceOf(address(this));
        transferDividends(_shibaDividendToken, address(_shibaDividendTracker), _shibaDividendTracker, _shibaDividends);
        isInternalTransaction = false;
    }
