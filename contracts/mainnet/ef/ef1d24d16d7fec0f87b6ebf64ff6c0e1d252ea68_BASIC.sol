...
contract BASIC is IERC20, Context, Ownable {
    ...
    string private constant _symbol = "BASIC LIFE INU";
    string private constant _name = "BASIC";

    uint256 public BASIC_max_BuyLimit = 10000000 * 10**9;
    uint256 public BASIC_max_SellLimit = 10000000 * 10**9;

    address public BASIC_LIFEINU_Wallet = 0x14E5208895CfA0301C57ddC4b12d05ea73502D65;
    address public BASIC_Team_Wallet = 0x14E5208895CfA0301C57ddC4b12d05ea73502D65;

    uint256 public BASIC_max_WalletLimit = 20000000 * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[BASIC_LIFEINU_Wallet] = true;
        _isExcludedFromFee[BASIC_Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[BASIC_LIFEINU_Wallet] = true;
        allowedTransfer[BASIC_Team_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= BASIC_max_BuyLimit, "You are exceeding BASIC_max_BuyLimit");
            require(balanceOf(to) + amount <= BASIC_max_WalletLimit, "You are exceeding BASIC_max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= BASIC_max_SellLimit, "You are exceeding BASIC_max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= BASIC_max_WalletLimit, "You are exceeding BASIC_max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(BASIC_LIFEINU_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(BASIC_Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function BASIC_Update_MarketingWallet(address newWallet) external onlyOwner{
        BASIC_LIFEINU_Wallet = newWallet;
    }
    
    function BASIC_Update_TeamWallet(address newWallet) external onlyOwner{
        BASIC_Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        BASIC_max_BuyLimit = maxBuy * 10**decimals();
        BASIC_max_SellLimit = maxSell * 10**decimals();
    }
    
    function BASIC_Max_Walletlimit(uint256 amount) external onlyOwner{
        BASIC_max_WalletLimit = amount * 10**decimals();
    }
