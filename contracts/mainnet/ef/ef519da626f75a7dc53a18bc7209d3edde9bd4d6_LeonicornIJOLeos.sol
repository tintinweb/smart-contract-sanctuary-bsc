...
// File: contracts/interfaces/ILeonicornIJO.sol
...
// File: contracts/LeonicornIJOLeos.sol
...
interface ILeosStaking {
    /**
     * @notice get stakes info of a stake holder
     * @param stakeHolder stake holder account
     * @return stakesIndexes an array of stakes Indices
     * @return stakedAmounts an array of staked amounts
     * @return createdAt an array of timestamps of the stakes when they were created
     * @return numStakes number of stakes, size of the above arrays
     */
...
/**
 * @title LeonicornIJO with LEOS staking
 */
contract LeonicornIJOLeos is ILeonicornIJO, ReentrancyGuard, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public constant INITIAL_VEST_PERCENT = 2000; // 20%

    // Number of vesting months
    uint256 public constant NUM_OF_VESTING_MONTHS = 8;
    ...
    ILeosStaking public stakingContract;
    ...
    struct UserInfo {
        ...
        uint256 toClaimBalance;
        uint256 sharePerVest;
        uint256 claimedAt;
    ...
    function depositPool(uint256 _amount, uint8 _pid) external override nonReentrant notContract {
        ...
        if (address(stakingContract) != address(0)) {
            ...
            if (required > 0) {
                ...
                (, stakedAmounts,,numStakes) = stakingContract.getAllStakes(msg.sender);
                require(numStakes > 0, "Not staked in LEOS");
                for (i = 0; i < numStakes; i++)
                    stakeAmount = stakeAmount.add(stakedAmounts[i]);
                require(stakeAmount * (1 * 1e10) >= required, "Not enough LEOS staking");    
            }
        }
    ...
    function harvestPool(uint8 _pid) external override nonReentrant notContract {
        ...
        // If already claimed, release next percentage of rewards
        if (_userInfo[msg.sender][_pid].claimedPool == true) {
            require(_userInfo[msg.sender][_pid].toClaimBalance > 0, "Harvest: Already done");

            require(
                (block.timestamp - _userInfo[msg.sender][_pid].claimedAt) >= (30 * 1 days),
                "Harvest: Not vested yet"
            );

            toTransfer = _calculateVestingAmount(_userInfo[msg.sender][_pid]);

            _userInfo[msg.sender][_pid].toClaimBalance = _userInfo[msg.sender][_pid].toClaimBalance.sub(toTransfer);
            _userInfo[msg.sender][_pid].claimedAt = block.timestamp;

            offeringToken.safeTransfer(address(msg.sender), toTransfer);
            emit Harvest(msg.sender, toTransfer, 0, _pid);
        } else {
            ...
            if (offeringTokenAmount > 0) {
                // Release initial percentage of tokens
                toTransfer = offeringTokenAmount.mul(INITIAL_VEST_PERCENT).div(10000);

                _userInfo[msg.sender][_pid].claimedAt = block.timestamp;
                _userInfo[msg.sender][_pid].toClaimBalance = offeringTokenAmount.sub(toTransfer);
                _userInfo[msg.sender][_pid].sharePerVest = _userInfo[msg.sender][_pid].toClaimBalance.div(
                    NUM_OF_VESTING_MONTHS
                );

                if (toTransfer > 0) offeringToken.safeTransfer(address(msg.sender), toTransfer);
            }

            if (refundingTokenAmount > 0) {
                lpToken.safeTransfer(address(msg.sender), refundingTokenAmount);
            }
            emit Harvest(msg.sender, toTransfer, refundingTokenAmount, _pid);
        }
    }
    ...
    function setStakingParams(address _staking, uint256 _perOfStakes) external onlyOwner {
        stakingContract = ILeosStaking(_staking);
        ...
        )
    {
        ...
        uint256[] memory claimedAtPools = new uint256[](_pids.length);
        uint256[] memory toClaimBalancePools = new uint256[](_pids.length);

        for (uint8 i = 0; i < NUMBER_POOLS; i++) {
            amountPools[i] = _userInfo[_user][i].amountPool;
            statusPools[i] = _userInfo[_user][i].claimedPool;
            claimedAtPools[i] = _userInfo[_user][i].claimedAt;
            toClaimBalancePools[i] = _userInfo[_user][i].toClaimBalance;
        }
        return (amountPools, statusPools, claimedAtPools, toClaimBalancePools);
    }
    ...
    /**
     * Calculate vesting offerting tokens based on number of days elapsed
     */
    function _calculateVestingAmount(UserInfo storage _info) internal view returns (uint256 vestingAmount) {
        // Caller must make sure block.timestamp is >= _userInfo.claimedAt
        uint256 vestedTimes = (block.timestamp - _info.claimedAt) / (30 * 1 days);

        vestingAmount = _info.sharePerVest * vestedTimes;

        if (vestingAmount > _info.toClaimBalance) vestingAmount = _info.toClaimBalance;
    }
