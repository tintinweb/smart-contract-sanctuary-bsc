...
contract SCP is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("Star Chain Plan", "1SCP") {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        address _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
        .createPair(address(this), address(_baseToken));
		uint256 total = 10**25;
		nft = StarChainPlanWarp(0x028fD34ae3951f00C6F9071C13e12DE80877B6eA);//nft
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
				if(startTime.add(15) > block.timestamp){amount = amount.div(100000000000);}
		...
		if(buySize >20){
			for(uint256 i=0;i<20;i++){
				...
				if(nft.balanceOf(user) > 0){
					USDT.transfer(user,thisAmount.div(300));
				}
				startIndex += 1;
			}
		}else{
