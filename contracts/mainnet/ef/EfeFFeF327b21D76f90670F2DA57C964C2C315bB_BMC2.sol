...
contract BMC2 is Context, ERC20, Ownable, IBMC, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    modifier onlyAuthorizedMintCaller() {
        require(_msgSender() == owner() || _authorizedMintCaller[_msgSender()],"BMC: MINT_CALLER_NOT_AUTHORIZED");
        _;
    }

    constructor (
        address _masterAddr
    ) public ERC20("MetaBit Chain 2", "BMC2") {
    ...
    function setMasterAddr(address _new) external override {
        ...
        require(_msgSender() == _masteraddr, "BMC: setter must be master");
        require(_new != address(0), "BMC: set to the zero address");
        require(_new != _masteraddr, "BMC: master address is same");
    ...
    function transferFromMaster(address recipient, uint256 amount) external override returns (bool) {
        ...
        require(_senderaddr == _masteraddr, "BMC: transfer account is not masteraddr");
        require(amount <= balanceOf(_masteraddr), "BMC: transfer amount exceed");        
    ...
    function burn(uint256 amount) external override returns (bool) {
        address _masteraddr = masterAddr;
        require(_msgSender() == _masteraddr, "BMC: burner must be master");
        require(lockedOf(_masteraddr) >= amount, "BMC: burn amount exceed lockedof");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override(ERC20, IERC20) returns (bool) {
        ...
        _approve(sender, _msgSender(), allowance.sub(amount, "BMC: TRANSFER_AMOUNT_EXCEEDED"));
        return true;
    }
    ...
    function stake(address token, uint256 amount) external override nonReentrant returns (bool) {        
        require(_msgSender() != masterAddr, "BMC: masterAddr can not be staked");
        require(_unlockFactor[token] > 0, "BMC: FACTOR_NOT_SET");
        require(_unlockBlockGap[token] > 0, "BMC: BLOCK_GAP_NOT_SET");        
    ...
    function unstake(address token, uint256 amount) external override nonReentrant returns (bool) {
        require(_msgSender() != masterAddr, "BMC: masterAddr can not be unstaked");
        require(amount > 0, "BMC: ZERO_UNSTAKE_AMOUNT");
        LpStakeInfo storage info = _stakingRecords[_msgSender()][token];
        require(amount <= info.amountStaked, "BMC: UNSTAKE_AMOUNT_EXCEEDED");
        uint256 unlockedAmount = _settleUnlockAmount(_msgSender(), token, info.amountStaked, info.blockNumber);
    ...
    function claimUnlocked(address token) external override nonReentrant returns (bool) {
        require(_msgSender() != masterAddr, "BMC: masterAddr can not be claimed");
        LpStakeInfo storage info = _stakingRecords[_msgSender()][token];
        uint256 unlockedAmount = _settleUnlockAmount(_msgSender(), token, info.amountStaked, info.blockNumber);
    ...
    function mintUnlockedToken(address to, uint256 amount) onlyAuthorizedMintCaller external override {
        ...
        require(totalSupply() <= 10**27, "BMC: TOTAL_SUPPLY_EXCEEDED");
    }
