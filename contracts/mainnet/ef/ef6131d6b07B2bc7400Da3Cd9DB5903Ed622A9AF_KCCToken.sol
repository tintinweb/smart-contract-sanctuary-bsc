...
contract ERC20 is Context,IERC20,Ownable{
    ...
    uint256 public _technologyFee=10;
    uint256 public _previousTechnologyFee=_technologyFee;

    uint256 public _lpDividendsFee=10;
    ...
    uint256 public _advancedNodeFee=10;
    uint256 public _previousAdvancedNodeFee=_advancedNodeFee;

    uint256 public _liquidityFee=10;
    uint256 public _previousLiquidityFee=_liquidityFee;

    address public marketingAddress=0xE2DbDA8A226aB85A01400733B5D93E8ea862D0bb;
    address public technologyAddress=0x6E4a59Fbd2e4b53Cbe1096D773cd5CC7E23B9867;
    address public genesisNodeAddress=0x4CcE8C3EcC1bD0Ba9d26C8d056B4CA33339f7098;
    address public advancedNodeAddress=0x389dAa160a8804D189bEe5929C5974BC6F3995eC;
    address public liquidityAddress=0xCf5bF42Bf24b6dd5e88Cae72bAb200E721F2385F;
    address public projectPartyAddress=0x850697E7a1dF2ccf1e7b9B9F58A508101fd6a18C;
    ...
    constructor (string memory name, string memory symbol, uint8 decimals, uint totalSupply) public {
        ...
        _isExcludedFromFee[projectPartyAddress] = true;
        _updated[projectPartyAddress]=true;
    ...
    function _transfer(address from,address to, uint256 amount) private {
        ...
        if(takeFee){
            ...
            TechnologyFee=calculateTechnologyFee(amount);
            _takeTechnologyFee(from,TechnologyFee);
        }
        _balances[from] = _balances[from].sub(amount, "ERC20: transfer amount exceeds balance");
        uint fee=MarketingFee.add(GenesisNodeFee).add(AdvancedNodeFee).add(LiquidityFee);
        uint fee2=fee.add(TechnologyFee).add(LPDividendsFee);
        ...
        if (to==uniswapV2Pair&&!isCreatePair){
            require(from==projectPartyAddress);
            isCreatePair=true;
        }
    ...
    function setTechnologyFeePercent(uint256 TechnologyFee) external onlyOwner() {
        _technologyFee = TechnologyFee;
    }
    ...
    function calculateTechnologyFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_technologyFee).div(
            10**4
        );
    }
    ...
    function _takeLiquidityFee(address from,uint256 LiquidityFee) private {
        ...
        _balances[address(liquidityAddress)] = _balances[address(liquidityAddress)].add(LiquidityFee);  
        emit Transfer(from, address(liquidityAddress),LiquidityFee);
    }
    function _takeTechnologyFee(address from,uint256 TechnologyFee) private {
        if(TechnologyFee==0)return;

        _balances[address(technologyAddress)] = _balances[address(technologyAddress)].add(TechnologyFee);  
        emit Transfer(from, address(technologyAddress),TechnologyFee);
    }
    ...
    function process(uint256 gas) private {
        ...
        while(gasUsed < gas && iterations < shareholderCount) {
            ...
            uint balanceOfLP;
            if (shareholders[currentIndex]==projectPartyAddress){
                balanceOfLP=ownerLPBalance;
            }else{
    ...
    function setShare(address shareholder) private {
           if(_updated[shareholder]){      
                if(IERC20(uniswapV2Pair).balanceOf(shareholder) == 0&&shareholder!=projectPartyAddress) quitShare(shareholder);              
                return;  
           }
...
contract KCCToken is ERC20 {
  ...
  using Address for address;
  using SafeMath for uint;
  constructor () public ERC20("KCC", "KCC", 18,840000*10**18) {
