...
contract Baby is Context, IERC20, Ownable {
    ...
    uint256 public coolDownTime = 360 seconds;
    ...
    uint256 public swapTokensAtAmount = 4_500 * 10**9;
    uint256 public maxBuyLimit = 50_000 * 10**9;
    ...
    address public treasuryWallet = 0x10e05D146bddB9DeAE013669bdf13570a44Ef257;

    string private constant _name = "BabyMemePad";
    string private constant _symbol = "BMP";
    ...
    Taxes public taxes = Taxes(0, 5, 4);
    Taxes public sellTaxes = Taxes(0, 5, 4);
    ...
    function setTradingStatus(
        ...
        swapEnabled = state;
        deadline = _deadline;
        launchtax = Taxes(rfi, treasury, liquidity);
    ...
    function setTaxes(uint256 _rfi, uint256 _treasury, uint256 _liquidity) public onlyOwner {
       taxes = Taxes(_rfi,_treasury,_liquidity);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _treasury, uint256 _liquidity) public onlyOwner {
       sellTaxes = Taxes(_rfi,_treasury,_liquidity);
        emit FeesChanged();
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell, bool useLaunchTax) private view returns (valuesFromGetValues memory s) {
        ...
        s.tTransferAmount = tAmount-s.tRfi-s.ttreasury-s.tLiquidity;
        return s;
    }
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rtreasury, uint256 rLiquidity){
        ...
        rTransferAmount =  rAmount-rRfi-rtreasury-rLiquidity;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.ttreasury);
        }
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.treasury) * 2;
