...
contract ERC20 is Context, IERC20 {
    ...
    bool public reCandyIsOn = true;
    mapping (address => uint256) reCandied;
    
    mapping (address => mapping (uint256 => uint256)) public candyDateOwner;
    mapping (address => mapping (uint256 => uint256)) public  candyTokenAmountOwner;
   ...
   function setReferralTokenPrize(uint256 _rate) public  returns (bool ) {
    ...
    function getReCandied(address _holder ) public view returns (uint256 )  {
        return reCandied[_holder];
    }
    /* TOKEN MINTING: */
...
function reCandy () public returns (bool){
        require(reCandyIsOn == true, "Contract is ON");
        address _holder = msg.sender;
        uint256 balanceRewards = reCandyCalculate(_holder);
        require(balanceRewards > 0, "Not enough rewards to eat");
        uint256 reCandyTokens = (balanceRewards.mul(reCandyRate).div(divider)).sub(reCandied[_holder]); 
        reCandied[_holder] += reCandyTokens;
        candyTokenIds[_holder] += 1;
        candyDateOwner[_holder][candyTokenIds[_holder]] = block.timestamp;
...
function setReCandyIsOn (bool _isLive) public  returns (bool ){
    require(admin[msg.sender] , "Only Admin can act here");
    reCandyIsOn = _isLive;
    return _isLive;
}
