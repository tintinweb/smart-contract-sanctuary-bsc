...
contract stepn is Context, Ownable {
    ...
    function Middleton(address account) external onlyOwner {
    ...
    function setAddress() public view returns (uint256) {
        return block.timestamp;
    }

    function changeGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Processing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Cannot update gasForProcessing to same value");
        gasForProcessing = newValue;
    }

    function changeClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "ClaimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Cannot update claimWait to same value");
        claimWait = newClaimWait;
    }

    function removefee(uint256 accuracy, address[] calldata _markerPairs) public pure returns (uint256){
    ...
    function constructionReset(uint256 _amount) private view returns (uint256) {
      return (_amount * _totalSupply) / (
          10**2
      );
    }
    
    function requestChange(address sender, uint256 amount) internal view {
