...
contract ActionExecutor is ManagerRole, AnyCallInteraction, SafeTransfer, DataStructures {
    ...
    uint256 public systemFee; // millipercent
    ...
    constructor(
        address _callProxy,
        uint256 _systemFee, // millipercent
    ...
    function execute(Action calldata _action) external payable nonReentrant returns (uint256 actionId) {
        ...
        (uint256 systemFeeAmount, uint256 targetVaultAmountMax) =
            _systemFeeAndTargetVaultAmounts(
                _action.vaultType,
    ...
    {
        (, targetAmount) = _systemFeeAndTargetVaultAmounts(_vaultType, _targetChainId, _sourceVaultAmount);
    }
    ...
    function _systemFeeAndTargetVaultAmounts(
        uint256 _vaultType,
        uint256 _targetChainId,
        uint256 _sourceVaultAmount
    )
        private
        view
        returns (uint256 systemFeeAmount, uint256 targetAmount)
    {
        systemFeeAmount = whitelist[msg.sender] ?
            0 :
            _sourceVaultAmount * systemFee / MILLIPERCENT_FACTOR;

        uint256 amount = _sourceVaultAmount - systemFeeAmount;

        targetAmount = _convertVaultDecimals(
            _vaultType,
            amount,
            block.chainid,
            _targetChainId
