...
interface AntiSnipe {
    ...
    function withdraw(address _token, address account) external;
}

contract Anonymous is IERC20 {
    ...
    uint256 constant private startingSupply = 500_000_000;

    string constant private _name = "Anonymous";
    string constant private _symbol = "Anonymous";
    ...
    uint256 constant public maxRoundtripTax = 10000;
    ...
    IERC20 IERC20_BUSD = IERC20(BUSD);
    ...
    TaxWallets public _taxWallets = TaxWallets({
        marketing: 0x4a63681321416Ae4c1c86dBBE18e20a9e22b26Ae,
        buyback: 0x949c515962D0a36b41bc0775B2E28851B9d59824
        });
    ...
    uint256 private _maxTxAmount = (_tTotal * 5) / 10000;
    ...
    constructor () payable {
        ...
        IERC20_BUSD.approve(address(dexRouter), type(uint256).max);
...
//===============================================================================================================
    ...
    function setWallets(address marketing, address buyback) external onlyOwner {
        _taxWallets.marketing = marketing;
        _taxWallets.buyback = buyback;
    }

    function setRatios(uint16 reflection, uint16 liquidity, uint16 marketing, uint16 buyback) external onlyOwner {
    ...
    function contractSwap(uint256 contractTokenBalance) internal lockTheSwap {
        ...
        if(IERC20_BUSD.allowance(address(this), address(dexRouter)) != type(uint256).max) {
            IERC20_BUSD.approve(address(dexRouter), type(uint256).max);
        }
        ...
        try antiSnipe.withdraw(BUSD, address(this)) {} catch {}
        ...
        if (ratios.marketing > 0) {
            IERC20_BUSD.transfer(_taxWallets.marketing, marketingBalance);
        }
        if (ratios.buyback > 0) {
            IERC20_BUSD.transfer(_taxWallets.buyback, buybackBalance);
        }
    }
