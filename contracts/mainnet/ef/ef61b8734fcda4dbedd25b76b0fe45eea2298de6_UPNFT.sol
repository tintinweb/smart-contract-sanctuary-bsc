...
abstract contract RoyaltiesAddon is ERC721 {
    ...
    function _setStakeAddress(address _stakeAddress) internal {
...
contract UPNFT is Ownable, Pausable, RoyaltiesAddon, ERC2981 {
    ...
    mapping ( uint256 => Ref ) public refInfo;
    ...
    IERC20 public tokenAddress = IERC20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);

    // metadata URI
    string public BASE_URI = "https://upjson.s3.us-west-1.amazonaws.com";
    uint256 public royaltiesFees = 10;

    // mint tracking
    uint256 public minted = 0; 

    uint256 public startTime = 1670281200; // Set start

    event onMinted(uint256 tokenId);


    constructor() ERC721("UP NFT Collection", "UP NFT") {}
        ...
        returns (string memory)
    {
        ...
        return
            string(
                abi.encodePacked(_baseURI(), "/upnft.json")
            );
    }
    ...
    function isApprovedForAll(address _owner, address _operator)
        ...
        returns (bool)
    {
        // Stake contract must be able to stake and unstake
        if (stakeAddress != address(0) && _operator == stakeAddress)
            return true;
    ...
    function setStakeAddress(address _stakeAddress) external onlyOwner {
        stakeAddress = _stakeAddress;
        super._setStakeAddress(_stakeAddress);
    }
    ...
    function setRef(uint256[] memory _id, Ref[] memory _user) external onlyOwner {
        for (uint256 i = 0; i < _id.length; i++) {
            refInfo[_id[i]].total = _user[i].total;
        }
    }
     ...
     */
    function withdrawNative(uint256 _amount) external payable onlyOwner {
    ...
    function _createNFT(address to, uint256 tokenId) internal {
        require(minted <= TOTAL, "cannot mint anymore vintners");
        _safeMint(to, tokenId);

        emit onMinted(tokenId);
    }

    function _createNFTs(uint256 qty, address to) internal {
        for (uint256 i = 0; i < qty; i++) {
            minted += 1;
            _createNFT(to, minted);
        }
    }
     ...
     * @dev Mint with BUSD
     */
    function Mint(uint256 qty, uint256 ref) external whenNotPaused {
        require(mintingStarted(), "cannot mint right now");

        require(qty > 0 && qty <= maxMint, "Exceeds number of mints allowed");
        require(
            (minted + qty) <= TOTAL,
            "Exceeds number of total mints allowed"
        );

        uint256 transactionCost = price * qty;

        if(ref > 0){
            transactionCost = (price - (price/10)) * qty;
        }

        require(
            tokenAddress.balanceOf(_msgSender()) >= transactionCost,
            "not enough BUSD"
        );

        tokenAddress.transferFrom(_msgSender(), address(this), transactionCost);

        refInfo[ref].total += qty;
