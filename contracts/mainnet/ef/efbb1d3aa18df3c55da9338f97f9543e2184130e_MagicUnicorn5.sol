...
contract MagicUnicorn5 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    MagicUnicorn5DividendTracker public dividendTracker;
    ...
    constructor() ERC20("MagicUnicorn5", "MU5") {
        ...
        dividendTracker = new MagicUnicorn5DividendTracker(payable(this), router, 0x17Bd2E09fA4585c15749F40bb32a6e3dB58522bA,
                                                   "MagicUnicorn5TRACKER", "MU5TRACKER");
...
contract MagicUnicorn5DividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    MagicUnicorn5 public MagicUnicorn5Contract;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        MagicUnicorn5Contract = MagicUnicorn5(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main MagicUnicorn5 contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            address[] memory path = new address[](2);
            path[0] = uniswapV2Router.WETH();
            path[1] = address(MagicUnicorn5Contract);

            uint256 prevBalance = MagicUnicorn5Contract.balanceOf(address(this));
            ...
            {
                uint256 received = MagicUnicorn5Contract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    MagicUnicorn5Contract.transfer(account, received);
                } 
                else {
                    success = false;
                }
