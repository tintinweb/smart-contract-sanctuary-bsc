...
contract HappyLeox is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    HappyLeoxDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Happy Leox", "LEOX") {
        marketingWallet = payable(0xDbCC3d99603D5fe640a10A930B0e3f3E23E2F7F0);
        devWallet = payable(0x4fbbe6B3d4C7AF087610F16A857E61E76E9e09BD);
        ...
        dividendTracker = new HappyLeoxDividendTracker(
            payable(this),
            router,
...
contract HappyLeoxDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    HappyLeox public HappyLeoxContract;
    ...
    constructor(
        ...
        defaultToken = token;
        HappyLeoxContract = HappyLeox(mainContract);
    ...
    function _transfer(
        ...
        require(false, "HappyLeox_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "HappyLeox_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main HappyLeoxcontract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(HappyLeoxContract);

            uint256 prevBalance = HappyLeoxContract.balanceOf(address(this));
            ...
            {
                uint256 received = HappyLeoxContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    HappyLeoxContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
