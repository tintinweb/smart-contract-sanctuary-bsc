...
contract AnitoGacha is Pausable, Ownable, ReentrancyGuard {
    ...
    constructor(address _anitoNFT, address _busdAddress, address _treasuryAddress){
        busdToken = ERC20(_busdAddress); //testnet
        treasuryAddress = _treasuryAddress;
        
        gachaWhitelistPrices[COMMON] = 35 * 10 ** 18; //$45
        gachaWhitelistPrices[EPIC] = 75 * 10 ** 18; //$85
        gachaWhitelistPrices[LEGENDARY] = 160 * 10 ** 18; //$175

        gachaPrices[COMMON] = 40 * 10 ** 18; //$45
        gachaPrices[EPIC] = 85 * 10 ** 18; //$85
        gachaPrices[LEGENDARY] = 175 * 10 ** 18; //$175

        wlMaxMintPerCategory[COMMON] = 619;
        wlMaxMintPerCategory[EPIC] = 763;
        wlMaxMintPerCategory[LEGENDARY] = 118;

        publicMaxMintPerCategory[COMMON] = 1621;
        publicMaxMintPerCategory[EPIC] = 747;
        publicMaxMintPerCategory[LEGENDARY] = 132;
    ...
    function setPrivateSaleStatus(bool _privateSaleStatus) public onlyOwner {
    ...
    function getRemainingGachas() public view returns(uint256 common,uint256 epic,uint256 legendary){
        if(privateSaleStatus && !publicSaleStatus){
            common = (wlMaxMintPerCategory[COMMON] - wlTotalMintedPerCategory[COMMON]);
            epic = (wlMaxMintPerCategory[EPIC] - wlTotalMintedPerCategory[EPIC]);
            legendary = (wlMaxMintPerCategory[LEGENDARY] - wlTotalMintedPerCategory[LEGENDARY]);
        }
    ...
    function setPublicSaleStatus(bool _publicSaleStatus) public onlyOwner {
        ...
        publicMaxMintPerCategory[COMMON] += (wlMaxMintPerCategory[COMMON] - wlTotalMintedPerCategory[COMMON]);
        publicMaxMintPerCategory[EPIC] += (wlMaxMintPerCategory[EPIC] - wlTotalMintedPerCategory[EPIC]);
        publicMaxMintPerCategory[LEGENDARY] += (wlMaxMintPerCategory[LEGENDARY]  - wlTotalMintedPerCategory[LEGENDARY]);
    }

    function addWhitelistAddress(address[] memory _whitelistedAddresses) public onlyOwner { 
    ...
    function buyGacha(uint256 gachaCategory, uint256 quantity) public nonReentrant {
        ...
        require(publicSaleStatus || privateSaleStatus,"sale not yet opened");

        if(!publicSaleStatus){
            require(privateSaleStatus && whitelistedAddresses[msg.sender],"address not whitelisted");
            require(whitelistAnitoLimit[msg.sender] + quantity <= 30,"max category reached");
            require(wlTotalMintedPerCategory[gachaCategory] + quantity <= wlMaxMintPerCategory[gachaCategory],"max category reached");
            whitelistAnitoLimit[msg.sender] += quantity;
            wlTotalMintedPerCategory[gachaCategory] += quantity;
        }else{
