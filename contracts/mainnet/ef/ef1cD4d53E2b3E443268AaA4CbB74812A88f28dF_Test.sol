...
contract Test is IERC20Metadata, Ownable {
    ...
    //OpenTrade
    uint256 public trade_open;

    //Tax
    address payable public tax_addr;
    uint256 public tax_buy = 8;
    uint256 public tax_sell = 8;

    //Ignore fee
    mapping(address => uint256) public ignore_fee;

    //Maxes
    uint256 public max_tx = 5000000 * 10**_decimals;            //0.5%
    uint256 public max_wallet = 10000000 * 10**_decimals;       //1%
    uint256 public _swapTokensAtAmount = _totalSupply / 1000;   //0.1%
    ...
    constructor() {
        ...
        tax_addr = payable(msg.sender);
        ignore_fee[msg.sender] = 1;
    ...
    //Set buy, sell tax
    function SetTaxes(uint256 _tax_buy, uint256 _tax_sell) public onlyOwner {
    ...
    //Set tax wallet
    function SetTaxAddr(address payable _tax_addr) public onlyOwner {
        tax_addr = _tax_addr;
    }

    //Add fee ignore to wallets
    function SetIgnoreFee(address[] calldata _input, uint256 _enabled)
        public
        onlyOwner
    {
        unchecked {
    ...
    function _transfer(
        ...
        require(trade_open != 0, "Trading is disabled");
    ...
    function swapTokensForEth(uint256 tokenAmount) private lockTheSwap {
        uint256 ethAmt = CalcPercent(tokenAmount, 85);
        ...
        addLiquidity(liqAmt, CalcPercent(amountETH, 15));
    }

    function sendETHToFee(uint256 amount) private {
        (bool success, ) = tax_addr.call{value: amount}("");
