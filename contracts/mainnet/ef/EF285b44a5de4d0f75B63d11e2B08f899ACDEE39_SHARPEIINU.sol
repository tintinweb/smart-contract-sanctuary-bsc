...
contract SHARPEIINU is ERC20, Ownable {
    ...
    constructor(
        ...
        address owner,
        address mintToAddr
    ) ERC20('SHARPEI INU', 'SPINU') {
        require(
            feeAccount != address(0),
            'SHARPEI: fee account from the zero address'
        );
        require(feePercent <= PRECISION, 'SHARPEI: incorrect fee percent');
        require(owner != address(0), 'SHARPEI: incorrect owner address');
        require(mintToAddr != address(0), 'SHARPEI: incorrect mintToAddr address');
        if (_msgSender() != owner) {
    ...
    function _transfer(
        ...
        require(sender != address(0), 'SHARPEI: transfer from the zero address');
        require(recipient != address(0), 'SHARPEI: transfer to the zero address');
