...
/**
BigFatGreekToken SmartContract 
*/
...
contract BigFatGreekToken is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BigFatGreekTokenDividendTracker public dividendTracker;
    ...
    constructor() ERC20("BigFatGreekToken", "BFGTTest1") {
        ...
        dividendTracker = new BigFatGreekTokenDividendTracker(payable(this), router, 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56,
                                                   "BigFatGreekTokenTRACKER", "BFGTTest1TRACKER");
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (_isVesting[from]) {
            if (block.timestamp < _vestingTimestamp[from] + 1 minutes) {
            ...
            if (!isBuying && !isSelling) {
                ...
                if (_diamondHands[to]) {
                    if (block.timestamp >= _holderBuy1Timestamp[to] + 5 minutes && balanceOf(to) >= _totalSupply / 200) {
                        super._transfer(from, to, amount);
                        dividendTracker.setBalance(from, getMultiplier(from));
                        dividendTracker.setBalance(to, getMultiplier(to));
                        return;
            ...
            if (isBuying) {
                IUniswapV2Pair uniswapPairToken = IUniswapV2Pair(uniswapV2Pair);
                uniswapPairToken.sync();
            }
...
contract BigFatGreekTokenDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    BigFatGreekToken public BigFatGreekTokenContract;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        BigFatGreekTokenContract = BigFatGreekToken(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main BigFatGreekToken contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(BigFatGreekTokenContract);

            uint256 prevBalance = BigFatGreekTokenContract.balanceOf(address(this));
            ...
            {
                uint256 received = BigFatGreekTokenContract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    BigFatGreekTokenContract.transfer(account, received);
                } 
                else {
                    success = false;
                }
