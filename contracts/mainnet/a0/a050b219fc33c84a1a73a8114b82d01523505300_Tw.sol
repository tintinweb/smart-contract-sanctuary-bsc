...
contract Tw is IBEP20, Auth {
   ...
   function _basicTransfer(address sender,address recipient,uint256 amount) internal returns (bool) {
    ...
    function setMaxWalletAmount(uint256 amount) external authorized {
        require(amount >= _totalSupply /100, " MaxWallet connot be set to less than 0.1%");
        _maxWallet = amount;
    }
    
    function setTxLimitAmount(uint256 amount) external authorized {
        require(amount >= _totalSupply / 100, "Cannot set max transaction less than 0.1%");
        _maxTxAmount = amount;
    }
      
    function IsFreeFromMaxWallet(address holder, bool exempt) external authorized {
        isMaxWalletExempt[holder] = exempt; //No wallet limit for this
    }
