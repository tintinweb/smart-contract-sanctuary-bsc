...
contract WittyP2P is ReentrancyGuard, Ownable {
    ...
    constructor (address _revenueAccount, address _witty)  {
    ...
    function buyer_refPayout(address _user, uint _tradeID, uint _favour, uint _amount, uint8 choice, address[] memory _ref) internal {
        ...
        admin_payout(_user, _tradeID, _favour, _amount, _convert, refAmt, choice);
    }

    function admin_payout(address _user, uint _tradeID,uint _favour,uint _amount, uint _convert, uint _refAmt, uint8 choice) internal {
        proposals storage _Trade = trade[_tradeID];
        
        if (choice == 0) {
            uint adminBal = _amount - _refAmt;
            if (_favour == 1){
                require(payable(revenueAccount).send(adminBal), "Admin payout failed");
                ...
                require(IERC20(_Trade.expectAddr).transfer(revenueAccount, adminBal), "Admin payout failed");
            ...
            require(IERC20(token).transfer(revenueAccount, adminBal), "Admin payout failed");
            adminRevenue[token] += adminBal;
            wittyBalance[_user] -= adminBal;
        }
    ...
    function updateRevenueAccount(address _revenueAccount) public onlyOwner {
        revenueAccount = _revenueAccount;
    }
