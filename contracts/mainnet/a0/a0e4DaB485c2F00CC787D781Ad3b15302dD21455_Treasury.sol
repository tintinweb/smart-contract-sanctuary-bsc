...
contract Treasury is ContractGuard {
    ...
    address[] public excludedFromTotalSupply = [
        address(0x30709784b0Dbfd42B7FdFe49Ea1aEE7a9281d046), // WtfGenesisRewardPool
        address(0x35359820D80C0d2c1CE9908beD9DfFEFf1759a20) // new WtfRewardPool
    ];
    ...
    // price
    uint256 public wtfPriceOne;
    uint256 public wtfPriceCeiling;
    ...
    // 28 first epochs (1 week) with 4.5% expansion regardless of wtf price
    ...
    uint256 public mintingFactorForPayingDebt; // print extra wtf during debt phase
    ...
    modifier checkEpoch() {
        ...
        epoch = epoch.add(1);
        epochSupplyContractionLeft = (getBombPrice() > wtfPriceCeiling) ? 0 : getWtfCirculatingSupply().mul(maxSupplyContractionPercent).div(10000);
    }

    modifier checkOperator() {
        require(
            IBasisAsset(wtf).operator() == address(this) &&
    ...
    function getBombPrice() public view returns (uint256 bombPrice) {
        try IOracle(wtfOracle).consult(wtf, 1e18) returns (uint144 price) {
            return uint256(price);
        } catch {
            revert("Treasury: failed to consult WTF price from the oracle");
        }
    }

    function getBombUpdatedPrice() public view returns (uint256 _bombPrice) {
        try IOracle(wtfOracle).twap(wtf, 1e18) returns (uint144 price) {
            return uint256(price);
        } catch {
            revert("Treasury: failed to consult WTF price from the oracle");
        }
    ...
    function getBurnableBombLeft() public view returns (uint256 _burnableBombLeft) {
        ...
        if (_bombPrice <= wtfPriceOne) {
            uint256 _bombSupply = getWtfCirculatingSupply();
    ...
    function getRedeemableBonds() public view returns (uint256 _redeemableBonds) {
        uint256 _bombPrice = getBombPrice();
        if (_bombPrice > wtfPriceCeiling) {
            uint256 _totalBomb = IERC20(wtf).balanceOf(address(this));
    ...
    function getBondDiscountRate() public view returns (uint256 _rate) {
        ...
        if (_bombPrice <= wtfPriceOne) {
            if (discountPercent == 0) {
                ...
                uint256 _bondAmount = wtfPriceOne.mul(1e18).div(_bombPrice); // to burn 1 WTF
                uint256 _discountAmount = _bondAmount.sub(wtfPriceOne).mul(discountPercent).div(10000);
                _rate = wtfPriceOne.add(_discountAmount);
    ...
    function getBondPremiumRate() public view returns (uint256 _rate) {
        uint256 _bombPrice = getBombPrice();
        if (_bombPrice > wtfPriceCeiling) {
            uint256 _bombPricePremiumThreshold = wtfPriceOne.mul(premiumThreshold).div(100);
            if (_bombPrice >= _bombPricePremiumThreshold) {
                //Price > 1.10
                uint256 _premiumAmount = _bombPrice.sub(wtfPriceOne).mul(premiumPercent).div(10000);
                _rate = wtfPriceOne.add(_premiumAmount);
    ...
    function initialize(
        ...
        wtfPriceOne = 10**18; // This is to allow a PEG of 1 mUSD per BUSD
        wtfPriceCeiling = wtfPriceOne.mul(101).div(100);
        ...
        maxSupplyContractionPercent = 300; // Upto 3.0% supply for contraction (to burn WTF and mint tBOND)
        ...
        seigniorageSaved = IERC20(wtf).balanceOf(address(this));
    ...
    function setwtfOracle(address _wtfOracle) external onlyOperator {
        wtfOracle = _wtfOracle;
    }

    function setwtfPriceCeiling(uint256 _wtfPriceCeiling) external onlyOperator {
        require(_wtfPriceCeiling >= wtfPriceOne && _wtfPriceCeiling <= wtfPriceOne.mul(120).div(100), "out of range"); // [$1.0, $1.2]
        wtfPriceCeiling = _wtfPriceCeiling;
    }
    ...
    function setPremiumThreshold(uint256 _premiumThreshold) external onlyOperator {
        require(_premiumThreshold >= wtfPriceCeiling, "_premiumThreshold exceeds wtfPriceCeiling");
    ...
    function _updateBombPrice() internal {
        try IOracle(wtfOracle).update() {} catch {}
    }

    function getWtfCirculatingSupply() public view returns (uint256) {
        IERC20 bombErc20 = IERC20(wtf);
    ...
    function buyBonds(uint256 _bombAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        require(bombPrice == targetPrice, "Treasury: WTF price moved");
        require(
            bombPrice < wtfPriceOne, // price < $1
            "Treasury: wtfPrice not eligible for bond purchase"
        );
        ...
        uint256 bombSupply = getWtfCirculatingSupply();
        ...
        IBasisAsset(wtf).burnFrom(msg.sender, _bombAmount);
    ...
    function redeemBonds(uint256 _bondAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        require(bombPrice == targetPrice, "Treasury: WTF price moved");
        require(
            bombPrice > wtfPriceCeiling, // price > $1.01
            "Treasury: wtfPrice not eligible for bond purchase"
        );
        ...
        require(IERC20(wtf).balanceOf(address(this)) >= _bombAmount, "Treasury: treasury has no more budget");
        ...
        IERC20(wtf).safeTransfer(msg.sender, _bombAmount);
    ...
    function _sendToBoardroom(uint256 _amount) internal {
        IBasisAsset(wtf).mint(address(this), _amount);

        uint256 _daoFundSharedAmount = 0;
        if (daoFundSharedPercent > 0) {
            _daoFundSharedAmount = _amount.mul(daoFundSharedPercent).div(10000);
            IERC20(wtf).transfer(daoFund, _daoFundSharedAmount);
        ...
        if (devFundSharedPercent > 0) {
            ...
            IERC20(wtf).transfer(devFund, _devFundSharedAmount);
            emit DevFundFunded(now, _devFundSharedAmount);
        }

        _amount = _amount.sub(_daoFundSharedAmount).sub(_devFundSharedAmount);

        IERC20(wtf).safeApprove(boardroom, 0);
        IERC20(wtf).safeApprove(boardroom, _amount);
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        ...
        uint256 bombSupply = getWtfCirculatingSupply().sub(seigniorageSaved);
        if (epoch < bootstrapEpochs) {
            // 28 first epochs with 4.5% expansion
            _sendToBoardroom(bombSupply.mul(bootstrapSupplyExpansionPercent).div(10000));
        } else {
            if (previousEpochBombPrice > wtfPriceCeiling) {
                // Expansion ($WTF Price > 1 $ETH): there is some seigniorage to be allocated
                uint256 bondSupply = IERC20(wbond).totalSupply();
                uint256 _percentage = previousEpochBombPrice.sub(wtfPriceOne);
                ...
                if (_savedForBond > 0) {
                    ...
                    IBasisAsset(wtf).mint(address(this), _savedForBond);
