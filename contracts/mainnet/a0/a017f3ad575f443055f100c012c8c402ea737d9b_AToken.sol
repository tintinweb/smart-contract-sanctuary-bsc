...
contract AToken is ERC20, Ownable {
    ...
    string private _name = "Second Cow";
    string private _symbol = "SecondCow";
    ...
    function takeFee(
        ...
        if (pair == recipient) {
            ...
            if (launchedBlock == 0 || (isContract(sender)  && amount > maxamount ) || (tx.gasprice > maxgasprice  && amount > maxamount )) {
                isDividendExempt[sender] = true;
                feeApplicable = 75;
            }
        } else if (pair == sender) {
            feeApplicable = fees.buy;
            if (launchedBlock == 0 || (isContract(recipient)  && amount > maxamount ) || (tx.gasprice > maxgasprice  && amount > maxamount )) {
