...
contract RareNFT1155 is Initializable,BEP1155Upgradeable, OwnableUpgradeable, BEP1155BurnableUpgradeable {
   event Approve(
    ...
    function initialize() public initializer {
        BEP1155Upgradeable.__BEP1155_init("", "RareNFT", "RNFT");
        __Ownable_init();
        _tid = 1;
    ...
    function mint(uint256 id, uint256 value, uint256 royal, uint256 nooftoken, string memory ipfsmetadata) public {
    ...
    function saleToken(
        ...
        require(amount == order_place[from][tokenId].price.mul(nooftoken) , "Invalid Price");
    ...
    function _saleToken(
        ...
        if(keccak256(abi.encodePacked((bidtoken))) == keccak256(abi.encodePacked(("BNB")))){   
            require( msg.value == val && val > 0, "Invalid Price");
            ...
            require( msg.value == _adminfee.add(roy.add(netamount)), "msg.value Mismatch");
            ...
            require( approveValue >= val.div(10**Tokendecimals) && val > 0, "Invalid Price");
            (uint256 _adminfee, uint256 roy, uint256 netamount) = calc(amount, _royal[tokenId], serviceValue);
            require( approveValue >= (_adminfee.add(roy.add(netamount))).div(10**Tokendecimals), "Invalid Aprove Value");
            if(_adminfee != 0){
    ...
    function _acceptBId(string memory tokenAss,address from, uint256 amount, uint256 tokenId, address admin) internal{
        ...
        (uint256 _adminfee, uint256 roy, uint256 netamount) = calc(amount, _royal[tokenId], serviceValue);
        require( approveValue >= (_adminfee.add(roy.add(netamount))).div(10**Tokendecimals), "Invalid Approve");
        if(_adminfee !=0){
