    ...
    Ownable
{
    ...
    function enterLottery(uint256 _numberOfEntries) external {
        ...
        uint256 totaltickets = ticketCounter[msg.sender] + _numberOfEntries;

        if (s_lotteryState != LotteryState.OPEN) {
            revert Lottery__notOpen();
        }

        require(totaltickets <= 10, "Per wallet only 10 Tickets allowed");
    ...
    function pickWinner() public {
        ...
        uint256 one = random() % s_players.length;
        uint256 two = random() % s_players.length;
        uint256 three = random() % s_players.length;
        s_lotteryState = LotteryState.CALCULATING;

        address payable firstplace = s_players[one];
        address payable secondplace = s_players[two];
        address payable thirdplace = s_players[three];
