...
contract Vault is ReentrancyGuard, IVault, Ownable {
    ...
    function _decreasePosition(
        ...
        if (!_isLong) {
            globalShortAveragePrices[
                _indexToken
            ] = getDecreaseNextGlobalShortAveragePrice(
                _indexToken,
                position.averagePrice,
                _sizeDelta);
        }
    ...
    function getNextGlobalShortAveragePrice(
        ...
        return (averagePrice.mul(size)).add(_sizeDelta.mul(_nextPrice)).div(nextSize);      
    }
    function getDecreaseNextGlobalShortAveragePrice(
        address _indexToken,
        uint256 _markPrice,
        uint256 _sizeDelta
    ) public view returns (uint256) {
        uint256 size = globalShortSizes[_indexToken];
        uint256 averagePrice = globalShortAveragePrices[_indexToken];
        uint256 _latestSize = size > _sizeDelta ? size.sub(_sizeDelta) : 0;
        return _latestSize > 0 ? (averagePrice.mul(size).sub(_sizeDelta.mul(_markPrice))).div(_latestSize): 0;
    }
