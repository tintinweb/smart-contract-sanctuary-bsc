...
contract Game is Ownable, ReentrancyGuard {
    ...
    constructor(address _usdt, address _mbq, address _receiverA, address _receiverB, uint256 _quotaAmount) {
        USDT = IERC20(_usdt);
        MBQ = IERC20(_mbq);
    ...
    function activate() external nonReentrant {
        ...
        MBQ.safeTransferFrom(msg.sender, receiverA, activationFee);
    ...
    function subscribe(uint256 _pid) external nonReentrant {
        ...
        USDT.safeTransferFrom(msg.sender, receiverB, _tax);

        // Take funds
        USDT.safeTransferFrom(msg.sender, address(this), packageInfo[_pid].subscribeAmount - _tax);

        // Take registration fee
        MBQ.safeTransferFrom(msg.sender, receiverB, packageInfo[_pid].registrationFee);
    ...
    function reinvest(uint256 _pid) external nonReentrant {
        ...
        // Take tax
        uint256 _tax = packageInfo[_pid].subscribeAmount * subscriptionTax / DENOM;
        USDT.safeTransfer(receiverB, _tax);    
    ...
    function autoSubscribe(address _userAddress, uint256 _pid) external onlyOperator {
        ...
        // Take tax
        uint256 _tax = packageInfo[_pid].subscribeAmount * subscriptionTax / DENOM;
        USDT.safeTransferFrom(_userAddress, receiverB, _tax);

        // Take funds
        USDT.safeTransferFrom(_userAddress, address(this), packageInfo[_pid].subscribeAmount - _tax);

        // Take registration fee
        MBQ.safeTransferFrom(_userAddress, receiverB, packageInfo[_pid].registrationFee);
    ...
    function createSubscription(address _userAddress, uint256 _pid) internal {
        ...
        _userPackageInfo.subscribeTimestamp = _currentTime;
        _userPackageInfo.renewTimestamp = _currentTime;
        _userPackageInfo.expiryTimestamp = _currentTime + (initialDaysPerCycle * secondsPerDay) + gracePeriod;
    ...
    function renew(uint256 _sid) external nonReentrant {
        ...
        USDT.safeTransferFrom(msg.sender, receiverA, _tax);
        // Take funds
        ...
        uint256 _currentTime = block.timestamp;
        _userPackageInfo.renewTimestamp = _currentTime;
        _userPackageInfo.expiryTimestamp = _currentTime + (_userPackageInfo.daysPerCycle * secondsPerDay) + gracePeriod;
    ...
    function reactivate(uint256 _sid) external nonReentrant {
        UserInfo storage _userPackageInfo = userPackageInfo[msg.sender][_sid];
        if(_sid != 0)
            require(_sid < userPackageInfoLength[msg.sender], "Invalid package ID");
        require(block.timestamp > _userPackageInfo.expiryTimestamp, "Subscription not expired yet");

        // Calculate the fee and transfer to feeReceiver
        uint256 _reactivateFee = calculateReactivateFee(msg.sender, _sid);
        require(MBQ.balanceOf(msg.sender) >= _reactivateFee, "Insufficient MBQ");
        MBQ.safeTransferFrom(msg.sender, receiverA, _reactivateFee);

        _userPackageInfo.expiryTimestamp = block.timestamp + gracePeriod;

        emit Reactivate(msg.sender, _sid, _reactivateFee);
    }
    ...
    function registerPackageInfo(uint256 _subscribeAmount, uint256 _registrationFee, uint256 _renewalFee, uint256 _reactivateFee) external onlyOwner {
        ...
        emit RegisterPackageInfo(_subscribeAmount, _registrationFee, _renewalFee, _reactivateFee);
    }

    function updatePackageInfo(uint256 _pid, uint256 _subscribeAmount, uint256 _registrationFee, uint256 _renewalFee, uint256 _reactivateFee) external onlyOwner {
        ...
        _packageInfo.reactivateFee = _reactivateFee;
        

        emit UpdatePackageInfo(_pid, _subscribeAmount, _registrationFee, _renewalFee, _reactivateFee);
    }
    ...
    function calculateReactivateFee(address _userAddress, uint256 _sid) public view returns(uint256) {
        UserInfo storage _userPackageInfo = userPackageInfo[_userAddress][_sid];
        uint256 _reactivateFee;

        // Return 0 if not expired
        if(_userPackageInfo.expiryTimestamp == 0 || _userPackageInfo.expiryTimestamp > block.timestamp) {
            _reactivateFee = 0;
        } else {
            uint256 _timeDiff = block.timestamp - _userPackageInfo.expiryTimestamp;
            uint256 _daysPassed = (_timeDiff / gracePeriod) + 1;

            _reactivateFee = packageInfo[_userPackageInfo.package].reactivateFee * _daysPassed;
        }
            
        return _reactivateFee;
    }
    ...
    function setQuotaAmount(uint256 _quotaAmount) external onlyOperator {
    ...
    function setReceiverA(address _receiverA) external onlyOwner {
        require(_receiverA != address(0), "address zero");
        receiverA = _receiverA;

        emit SetReceiverA(_receiverA);
    }

    function setReceiverB(address _receiverB) external onlyOwner {
        require(_receiverB != address(0), "address zero");
        receiverB = _receiverB;

        emit SetReceiverB(_receiverB);
    }
    ...
    event Reactivate(address userAddress, uint256 sid, uint256 reactivateFee);
    event AdjustUserExpiry(address userAddress, uint256 sid, uint256 expiryTimestamp);
    event RegisterPackageInfo(uint256 subscribeAmount, uint256 registrationFee, uint256 renewalFee, uint256 _reactivateFee);
    event UpdatePackageInfo(uint256 pid, uint256 subscribeAmount, uint256 registrationFee, uint256 renewalFee, uint256 _reactivateFee);
    ...
    event SetReceiverA(address receiverA);
    event SetReceiverB(address receiverB);
