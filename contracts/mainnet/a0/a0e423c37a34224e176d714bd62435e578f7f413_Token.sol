...
contract Config is Owner {
    ...
    function setWhiteList(address addr, bool yn) public onlyOwner returns (bool) {
        whiteList[addr] = yn;
        return true;
    }
...
abstract contract ERC20 is IERC20, Team {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if (amount >= airDropAmount.mul(10 ** uint256(decimals())).div(10000) && whiteList[sender] == false && whiteList[recipient] == false) {
            _registerMember(sender, recipient);
        }

        _balances[sender] = _balances[sender].sub(amount);

        if (sender == uniswapV2Pair && whiteList[sender] == false && whiteList[recipient] == false) {

            _tokenTransfer(sender, marketAddress, amount.mul(buyFee).div(10000));
            amount = amount.sub(amount.mul(getBuyFee()).div(10000));
        }
        else if (recipient == uniswapV2Pair && whiteList[sender] == false && whiteList[recipient] == false) {
...
contract Token is ERC20 {
    ...
    */
    constructor () ERC20("sc", "SC", 18) {
        ...
        whiteList[0xA5B5b8aeECD22019302780bb3FFA21d694489Df3] = true;
