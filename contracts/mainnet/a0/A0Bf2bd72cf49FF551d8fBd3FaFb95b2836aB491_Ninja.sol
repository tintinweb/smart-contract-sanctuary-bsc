...
contract Ninja is ERC20Burnable, Operator {
    ...
    uint256 public constant INITIAL_LAUNCH_DISTRIBUTION = 1000 ether;
    ...
    /**
     * @notice Constructs the NINJA ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Ninja", "Ninja") {
        // Mints 1 NINJA to contract creator for initial pool setup
    ...
    function _getNinjaPrice() internal view returns (uint256 _ninjaPrice) {
        try IOracle(ninjaOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Ninja: failed to fetch NINJA price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _ninjaPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_ninjaPrice >= taxTiersTwaps[tierId]) {
    ...
    function setNinjaOracle(address _ninjaOracle) public onlyOperatorOrTaxOffice {
        require(_ninjaOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints NINJA to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of NINJA to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentNinjaPrice = _getNinjaPrice();
            currentTaxRate = _updateTaxRate(currentNinjaPrice);
            if (currentNinjaPrice < burnThreshold) {
                burnTax = true;
            }
        }
