...
contract spicy is Context, Ownable, IERC20 {
    ...
    address payable private devAddress = payable(0);
    ...
    function takeTaxes(address from, bool isbuy, bool issell, uint256 amount, bool launchTax) internal returns (uint256) {
        uint256 fee;
        if (isbuy)  fee = buyfee;  else if (issell)  fee = sellfee;  else  fee = transferfee; 
        if (launchTax) {fee = LaunchTaxs;}
    ...
    function internalSwap(uint256 contractTokenBalance) internal inSwapFlag {
        ...
        function enableTrading(uint256 dead) external onlyOwner {
            require(dead < 5,"Can't be more than 5");
            require(!isTradingEnabled, "Trading already enabled");
            swapThreshold = (balanceOf(lpPair)) / 10000;
            isTradingEnabled = true;
            _deadline = block.number + dead;
        }
