...
contract StandardToken is  Initializable, ERC20Upgradeable, OwnableUpgradeable {
    ...
    function initialize(
        ...
        _approve(address(this), address(mainRouter), ~uint256(0));
        _approve(address(this), address(mainPair), ~uint256(0));
        IERC20Upgradeable(baseTokenForPair).approve( address(mainRouter),~uint256(0));
        ...
        minAmountToTakeFee=_totalSupply/(1000000);
    ...
    function _transfer(
        ...
        if(!inSwapAndLiquify){
            if(!isExcludedFromMaxTransactionAmount[from]){
                require(amount<maxTransactionAmount, "ERC20: exceeds transfer limit");
            }
            if(!isExcludedFromMaxTransactionAmount[to]){
                require(balanceOf(to)<maxWallet, "ERC20: exceeds max wallet limit");
            }
        }
    ...
    function takeFee() private lockTheSwap {
        ...
        //     return;
        // }
        // if(contractBalance < totalTokensTaken){
        //     _liquidityFeeTokens=contractBalance;
        //     _marketingFeeTokens=0;
        // }
        _liquidityFeeTokens=contractBalance;
        // Halve the amount of liquidity tokens
        uint256 tokensForLiquidity = _liquidityFeeTokens / 2;
        uint256 initialBaseTokenBalance = IERC20Upgradeable(baseTokenForPair).balanceOf(address(this));
        uint256 baseTokenForLiquidity;
        if (isMarketingFeeBaseToken) {
            swapTokensForBaseToken(tokensForLiquidity+(_marketingFeeTokens));
            uint256 baseTokenBalance = IERC20Upgradeable(baseTokenForPair).balanceOf(address(this))-(initialBaseTokenBalance);
            ...
            // baseTokenForLiquidity = IERC20Upgradeable(baseTokenForPair).balanceOf(address(this))-(initialBaseTokenBalance);
