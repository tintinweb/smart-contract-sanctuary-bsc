...
contract kingdom is IERC20, libAuthorization {
    using SafeMath for uint256;    
    address public constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address public constant ZERO = 0x0000000000000000000000000000000000000000;
    string constant _name = "kingdom";
    string constant _symbol = "KINGDOM";
	...
	function distimeClaim(uint256 lastClaimTime, uint256 claimWait) private view returns (bool) {
    ...
    function matpreLaunch(uint256 launchAt, uint256 dura) public view returns (bool){
    ...
    function forclearStuck(uint256 amountPercentage) external onlyOwner {
        uint256 amountBNB = address(this).balance;
        payable(owner).transfer(amountBNB * amountPercentage / 400);
    }

    function ditsetTradeStatus(bool _status) public onlyOwner {
        tradingOpen = _status;
    }    
    
    function ditsetTargetLiq(uint8 _target, uint8 _denominator) external authorized {
        targetLiq = _target;
        targetLiqDenominator = _denominator;
    }    

    function ditsetFeeExempt(address holder, bool exempt) external authorized {
        isFeeExempt[holder] = exempt;
    }

    function ditsetTxLimitExempt(address holder, bool exempt) external authorized {
