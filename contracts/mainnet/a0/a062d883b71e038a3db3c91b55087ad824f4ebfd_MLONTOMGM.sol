...
contract MLONTOMGM is ReentrancyGuard, Ownable{
   using SafeMath for uint256; 
    ...
    constructor(address _mlon, address _mgm, address _lppair, address _treasury, address _dao) public{
        mlon = IBEP20(_mlon);
        mgm = IBEP20(_mgm);
    ...
    function currentPrice() public view returns(uint256){
       ...
       uint256 mSupply= uint256(reserve0);
        uint256 usdtSupply = uint256(reserve1);
        return mSupply.div(usdtSupply);
    }
    ...
    function swapMLONToMGM(uint256 _amountIn, uint256 _amountOutMin) public nonReentrant{ 
        ...
        uint256 amountOutDao = amountOut.mul(DAOfee).div(100);
        uint256 amountOutTreasury = amountOut.mul(treasuryFee).div(100);
        uint256 amountOutBalance = amountOut.sub(amountOutDao);
        uint256 amountOutFinal = amountOutBalance.sub(amountOutTreasury);

        mgm.transfer(treasury, amountOutTreasury);
        mgm.transfer(dao, amountOutDao);
        mgm.transfer(msg.sender, amountOutFinal);
    ...
    function _getAmountOut(uint256 _amountIn) public view returns(uint256 amountOut){
        (uint112 reserve0, uint112 reserve1,) =  lppair.getReserves();
        amountOut = _amountIn.mul(uint256(reserve0)).div(uint256(reserve1));
        return amountOut;
    }


    function removeMGMLiquidity(uint256 _mgmAmount) public onlyOwner{
    ...
    function removeMLONiquidity(uint256 _mlonAmount) public onlyOwner{
