...
contract PoolFunctionality is Ownable, IPoolFunctionality {
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; ++i) {
            ...
            if (supportingFee) {
                ...
                amountOut = OrionPoolV2Library.getAmountOut(amountIn, reserveInput, reserveOutput);
            } else {
    ...
    function _addLiquidity(
        ...
        (uint256 reserveA, uint256 reserveB) = OrionPoolV2Library.getReserves(
            factory,
            tokenA,
            tokenB
        );
        if (reserveA == 0 && reserveB == 0) {
            ...
            uint256 amountBOptimal = OrionPoolV2Library.quote(
                amountADesired,
                reserveA,
                reserveB
            );
            if (amountBOptimal <= amountBDesired) {
                ...
                uint256 amountAOptimal = OrionPoolV2Library.quote(
                    amountBDesired,
                    reserveB,
                    reserveA
                );
    ...
    function increaseAllowances(address[] calldata tokens, address[] calldata tos, uint256[] calldata amounts) public onlyOwner {
        for (uint i = 0; i < tokens.length; ++i) {
            IERC20(tokens[i]).safeIncreaseAllowance(tos[i], amounts[i]);
        }
    }
}
...
library OrionMultiPoolLibrary {
    ...
    function getAmountOutUniversal(
        ...
        if (factoryType == IPoolFunctionality.FactoryType.UNISWAPLIKE) {
            ...
            amountOut = OrionPoolV2Library.getAmountOut(amountIn, reserveIn, reserveOut);
            ...
            revert("OrionPoolV2Library: FACTORY_UNSUPPORTED");
        }
    }
    ...
    function getAmountInUniversal(
        ...
        if (factoryType == IPoolFunctionality.FactoryType.UNISWAPLIKE) {
            (uint reserveIn, uint reserveOut) = getReserves(factory, from, to);
            amountIn = OrionPoolV2Library.getAmountIn(amountOut, reserveIn, reserveOut);
        } else if (factoryType == IPoolFunctionality.FactoryType.CURVE) {
