...
/**
 * Website - https://www.digitalmint.finance
 * Telegram - https://t.me/digitalmintportal
 * Medium - https://thedigitalmint.medium.com
 * Twitter - https://twitter.com/the_digitalmint
 * Facebook - https://www.facebook.com/digitalmintofficial
 * Instagram - https://www.instagram.com/the_digital_mint/
*/
 // Copyright The Digital Mint 2022
...
contract TheDigitalMintTest is ERC20Detailed, Ownable, WhitelistedRole {
    ...
    uint256 public rewardYield = 4583333;
    ...
    address public liquidityReceiver = 0xFa5b5ABe95188Bd4ca51b0158715989b0B0019c6;
    address public MintTreasuryReceiver = 0xFa5b5ABe95188Bd4ca51b0158715989b0B0019c6;
    address public MintVaultReceiver = 0xFa5b5ABe95188Bd4ca51b0158715989b0B0019c6;
    ...
    uint256 public totalBuyFee = liquidityFee.add(MintTreasuryFee).add(buyFeeDMV);
    uint256 public totalSellFee = totalBuyFee.add(sellFeeMintTreasuryAdded).add(sellFeeDMVAdded);
    ...
    uint256 public MintAntiDump = 600;
    ...
    constructor() ERC20Detailed("The Digital Mint", "MINT", uint8(DECIMALS)) {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (
            ...
            if( blkTime > tradeData[sender].lastTradeTime + MintAntiDump) {
                tradeData[sender].lastTradeTime = blkTime;
                tradeData[sender].tradeAmount = amount;
            }
            else if( (blkTime < tradeData[sender].lastTradeTime + MintAntiDump) && (( blkTime > tradeData[sender].lastTradeTime)) ){
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountToDMV = contractTokenBalance.mul(buyFeeDMV.mul(2).add(sellFeeDMVAdded)).div(realTotalFee);
        uint256 amountToMintTreasury = contractTokenBalance.sub(amountToLiquify).sub(amountToDMV);
        ...
        if(amountToMintTreasury > 0){
            _swapTokensForBNB(amountToMintTreasury, MintTreasuryReceiver);
        }

        emit SwapBack(contractTokenBalance, amountToLiquify, amountToDMV, amountToMintTreasury);
    }
    ...
    function setMintAntiDump(uint256 _time) external onlyOwner {
        MintAntiDump = _time;
    }
    ...
    function setFeeReceivers(address _liquidityReceiver, address _MintTreasuryReceiver, address _MintVaultReceiver) external onlyOwner {
        liquidityReceiver = _liquidityReceiver;
        MintTreasuryReceiver = _MintTreasuryReceiver;
        MintVaultReceiver = _MintVaultReceiver;
    }

    function setFees(uint256 _liquidityFee, uint256 _DigitalMintVaul, uint256 _MintTreasuryFee, uint256 _sellFeeMintTreasuryAdded, uint256 _sellFeeDMVAdded, uint256 _feeDenominator) external onlyOwner {
        require(
            _liquidityFee <= MAX_FEE_RATE &&
            _DigitalMintVaul <= MAX_FEE_RATE &&
        ...
        totalSellFee = totalBuyFee.add(sellFeeMintTreasuryAdded).add(sellFeeDMVAdded);
    ...
    event SwapBack(uint256 contractTokenBalance,uint256 amountToLiquify,uint256 amountToDMV,uint256 amountToMintTreasury);
