...
contract WorldCupGamble is Ownable, ReentrancyGuard {
    ...
    struct Bet {
        ...
        string idTeam; // id of the team - id home team or away team, if draw id = '0'
    ...
    struct Match {
        ...
        bool isClaimed; // Check if the owner has claimed the reward when no one make right bet
    }
    ...
    struct Player {
        ...
        Bet[] bets; // list of bets
        uint256 totalBet; // total bet of the player
    }
    ...
    function addTeam(string memory _id, string memory _name) public onlyOwner {
        require(bytes(teamId[_id]).length == 0, "Team already exists");
        teamId[_name] = _id;
        teamName[_id] = _name;
    ...
    function addMatch(
        ...
        require(matches[idMatch].startTime == 0, "Match already exists");

        require(
            bytes(teamName[_idHomeTeam]).length != 0 &&
                bytes(teamName[_idAwayTeam]).length != 0,
            "Team not exists"
        );
        ...
        matches[idMatch].isClaimed = false;
        totalMatch++;
    ...
    function createBet(
        ...
        require(
            compareStringsbyBytes(idTeam, _match.idHomeTeam) ||
                compareStringsbyBytes(idTeam, _match.idAwayTeam) ||
                compareStringsbyBytes(idTeam, "0"),
            "Invalid team"
        );
        if (compareStringsbyBytes(idTeam, _match.idHomeTeam)) {
            require(
                betCategory <= 1,
                "Your choose home team but you bet on another option"
            );
        }
        if (compareStringsbyBytes(idTeam, _match.idAwayTeam)) {
            require(
                betCategory >= 3,
                "Your choose away team but you bet on another option"
            );
        }
        if (compareStringsbyBytes(idTeam, "0")) {
            require(
                betCategory == 2,
                "Your choose draw team but you bet on another option"
            );
        }
        ...
        _match.bets.push(
            Bet(msg.sender, idMatch, idTeam, amount, betCategory, false)
        );
        ...
        playerMatchBets[msg.sender][idMatch].push(
            Bet(msg.sender, idMatch, idTeam, amount, betCategory, false)
        );
    ...
    function updateMatchScore(
        ...
        require(
            getCurrentTime() > matches[idMatch].startTime + 7200,
            "Can update after 2 hours"
        );
        require(
    ...
    function claim(string memory idMatch) public nonReentrant {
        ...
        if (
            _match.betCategoryAmount[_match.matchResult] == 0 &&
            !_match.isClaimed
        ) {
        ...
        for (uint256 i = 0; i < uint256(bets.length); i++) {
            ...
            if (bets[i].betCategory == _match.matchResult) {
                if (_match.totalLoseAmount > 0) {
                    ...
                    _token.safeTransfer(bets[i].player, bets[i].amount);
    ...
    function getMessageHash(
        ...
        return
            keccak256(
                abi.encodePacked(idMatch, homeScore, awayScore, homeId, awayId)
            ); // Create match key
    }
