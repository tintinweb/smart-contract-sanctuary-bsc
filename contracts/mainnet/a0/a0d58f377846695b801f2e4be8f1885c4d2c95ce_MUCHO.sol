...
contract MUCHO is IERC20, Ownable {
    ...
    address public pair;
    uint256 internal liquidityFee = 200;
    ...
    bool internal startSwap = false;
    uint256 internal swapThreshold = ( _totalSupply * 550 ) / 100000;
    uint256 internal _minTokenAmount = ( _totalSupply * 15 ) / 100000;
    modifier lockTheSwap {swapping = true; _; swapping = false;}
    uint256 internal minSells = 2;
    uint256 internal swapTimes; 
    bool internal swapping; 
    mapping(address => bool) public isFeeExempt;
    address internal liquidity_receiver; 
    address internal alpha_receiver;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        uint256 amountFee = shouldTakeFee(sender, recipient) ? getTotalFee(sender, recipient) : 0;
    ...
    function getTotalFee(address sender, address recipient) internal view returns (uint256) {
        if(sender != pair && recipient == pair){return sellFee;}
        if(sender != pair && recipient != pair){return transferFee;}
        return totalFee;
    }
    ...
    function rescueERC20(address _token, address _receiver, uint256 _percentage) external onlyOwner {
        ...
        IERC20(_token).transfer(_receiver, tokenAmt.mul(_percentage).div(100));
    }
