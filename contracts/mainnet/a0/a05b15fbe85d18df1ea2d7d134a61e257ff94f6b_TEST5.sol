...
contract TEST5 is ERC20, Ownable {
    ...
    TEST5DividendTracker public dividendTracker;
    ...
    constructor() ERC20("NAMETEST5", "TEST5") {
    	dividendTracker = new TEST5DividendTracker();
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "TEST5: The dividend tracker already has that address");
        TEST5DividendTracker newDividendTracker = TEST5DividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "TEST5: The new dividend tracker must be owned by the NAMETEST5 contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "TEST5: The router already has that address");
    ...
    function setrewardFees(uint256 value) external onlyOwner{
        ...
        require(totalFees <= 12, "TEST5: Maximum allowed fees are 12%");
    }
    ...
    function setExtraSellFee(uint256 value) external onlyOwner{
        require(value <= 1, "TEST5: Maximum allowed fees are 1%");
        extraSellFee = value;
    }
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "TEST5: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function limitMaxWallet(uint256 maxWalletPercent) external onlyOwner {
         ...
         require(maxWalletPercent >= 1, "TEST5: Can not limit Max Wallet to less than 1%");
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "TEST5: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "TEST5: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "TEST5: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if (from != owner() &&
            ...
            require(balanceOf(to) + amount <= maxWalletTokens, "TEST5: Exceeds maximum wallet token amount.");
        }
        if(amount == 0) {
        ...
        if(takeFee) {
            require(tradingOpen,"TEST5: Trading is not open yet");
...
contract TEST5DividendTracker is Ownable, DividendPayingToken {
    ...
    constructor() DividendPayingToken("TEST5", "TEST5_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "TEST5_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "TEST5_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TEST5 contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "TEST5_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "TEST5_Dividend_Tracker: Cannot update claimWait to same value");
