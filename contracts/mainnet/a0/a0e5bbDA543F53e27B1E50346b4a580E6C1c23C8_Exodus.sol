...
contract Exodus is ERC1155Supply, Ownable  {
    ...
    uint constant MAX_PAY_PER_PURCHASE = 50;
    uint public constant MAX_PAY = 30000000;
    uint private PriceWL = 0.33 ether; //BNB
    uint private Price = 0.33 ether;   //BNB
    ...
    uint256 public totalBlockNFT = 0;
...
// Withdraw addresses
    address public w1 = 0x66719ABdc5691B79186c1AeE869843047Cb11fAe;//80%
    address public w2 = 0xb3a3610832Ecf7dEf03b1C388838DC066789259C;//20%
    address public wStat = 0x245536E42c7DF4B736fa312cAF73b7295B5CC8AD;//direct sales statistics
    address wDev;
...
constructor(string memory uri, string memory _name, string memory _symbol, uint256 _StartSale, uint256 _modi, address _wDev) ERC1155(uri) {
    ...
    minraite[0] = 1; //min rating (initial value)
    yeild[0] = 0.000000015067708333 ether; //min 0.3945% day (initial value)
    mindate[0] = 259200; //min period 3 days (initial value)
    for (uint i = 1; i < 5; i++) {
...
// Mint for WhiteList
    ...
    require(totalSupply(TOKEN_ID) + numberOfTokens + totalBlockNFT <= MAX_PAY, "Purchase would exceed max supply of tokens");
