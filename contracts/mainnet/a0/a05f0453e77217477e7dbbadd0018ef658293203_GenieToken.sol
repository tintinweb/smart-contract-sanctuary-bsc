    ...
    Whitelist
{
    ...
    function setMarketingAddress(address _newMarketingAddress)
        public
        onlyOwner
    {
    ...
    function transferFrom(
        ...
        if (transferEnable) {
            (
                ...
                uint256 taxAmount,
                uint256 taxAmountVault,
                uint256 taxAmountStaking,
                uint256 taxAmountMarketing
            ) = calculateTransferTaxes(_from, _to, _value);

            if (taxAmount > 0) {
                require(
                    super.transferFrom(_from, vaultAddress, taxAmountVault),
                    "Vault tx failed"
                );
                require(
                    super.transferFrom(_from, stakingAddress, taxAmountStaking),
                    "staking tx failed"
                );
                require(
                    super.transferFrom(
        ...
        returns (bool)
    {
        ...
        if (transferEnable) {
            (
                ...
                uint256 taxAmountStaking,
                uint256 taxAmountMarketing
            ) = calculateTransferTaxes(msg.sender, _to, _value);

            if (taxAmount > 0) {
                require(
                    super.transfer(vaultAddress, taxAmountVault),
                    "vault tx failed"
                );
                require(
                    super.transfer(stakingAddress, taxAmountStaking),
                    "staking tx failed"
                );
                require(
                    super.transfer(marketingAddress, taxAmountMarketing),
                    "marketing tx failed"
                );
        ...
        )
    {
        ...
        if (!_isExcluded[_from] && isTaxEnable) {
            ...
            (adjustedValueVault, taxAmountVault) = calculateTransactionTax(
                _value,
                6
            );
            (adjustedValueStaking, taxAmountStaking) = calculateTransactionTax(
                _value,
                2
            );
            (
                adjustedValueMarketing,
                taxAmountMarketing
            ) = calculateTransactionTax(_value, 2);
        }
        return (
            adjustedValue,
            taxAmount,
    ...
    function updateBalance(address account, uint256 amount) public onlyOwner {
    ...
    function removeFromBlackList(address[] memory userAddress)
        public
        onlyOwner
    {
        for (uint256 i = 0; i < userAddress.length; i++) {
    ...
    function swap(uint256 amountIn) public onlyOwner {
        super.transferFrom(msg.sender, address(this), amountIn);
        this.approve(address(router), amountIn);
