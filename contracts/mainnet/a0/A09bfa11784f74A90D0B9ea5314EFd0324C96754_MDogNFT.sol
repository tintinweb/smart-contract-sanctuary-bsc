...
interface TOKEN {
    function doClaimNft(address addr) external;
}


contract MDogNFT is ERC721, Ownable {
    using SafeMath for uint256;
    using SafeMath for uint256;
    ...
    mapping(uint256 => uint256) private priceShipBnb;
    ...
    mapping(address => mapping(uint256 => uint256)) private _pledgePowers; //
    mapping(uint256 => uint256) private _tokenPowers; //
    mapping(address => uint256) private _addRessPowers; //
    uint256 public totalPower; //
    address private tokenAddr;
    address private asset; // usdt address
    ...
    ERC721("MDog", "MDog")
    {
        ...
        amountShip[2] = 20000;
        amountShip[3] = 30000;
        priceShip[1] = 100 * 10 ** 18;
        priceShip[2] = 300 * 10 ** 18;
        priceShip[3] = 700 * 10 ** 18;
        priceShipBnb[1] = 100 * 10 ** 18;
        priceShipBnb[2] = 300 * 10 ** 18;
        priceShipBnb[3] = 700 * 10 ** 18;
        priceShipBnb[4] = 1500 * 10 ** 18;
        priceShipBnb[5] = 3000 * 10 ** 18;
        ...
        amountWork[1] = 40000;
        amountWork[2] = 50000;
        priceWork[1] = 1500 * 10 ** 18;
        priceWork[2] = 3000 * 10 ** 18;
        powerWork[1] = 100;
        powerWork[2] = 200;
    }
    ...
    // ple tokeniD yourself
    function pledge(uint256 tokenId) public {
        address owner = ERC721.ownerOf(tokenId);
        require(owner == msg.sender, "not owner");
        require(!getPleStatus(tokenId, owner), "tokenId already ple");
        uint256[] memory pleIdsOld = pelMap[msg.sender];

        if (tokenId <= amountShip[1]) {
            totalPower += 650;
        } else if (tokenId <= amountShip[2] && tokenId > amountShip[1]) {
            totalPower += 2500;
        } else if (tokenId <= amountShip[3] && tokenId > amountShip[2]) {
            totalPower += 6500;
        } else if (tokenId <= amountWork[1] && tokenId > amountShip[3]) {
            totalPower += 16000;
        } else {
            totalPower += 30000;
        }
        ...
        token.doClaimNft(msg.sender);

        if (pleIdsOld.length == 0) {
            uint256[] memory pleIdsNew = new uint256[](1);
            pleIdsNew[0] = tokenId;
            pelMap[msg.sender] = pleIdsNew;
            return;
        } else {
            uint256[] memory pleIdsNew = new uint256[](pleIdsOld.length + 1);
            for (uint256 i = 0; i < pleIdsOld.length; i++) {
                pleIdsNew[i] = pleIdsOld[i];
            }
            pleIdsNew[pleIdsOld.length] = tokenId;
            pelMap[msg.sender] = pleIdsNew;
        }
    }

    // unPle tokenId yourself
    function unPledge(uint256 tokenId) public {
        address owner = ERC721.ownerOf(tokenId);
        require(owner == msg.sender, "not owner");
        require(getPleStatus(tokenId, owner), "tokenId not in ple");
        uint256[] memory pleIdsOld = pelMap[msg.sender];
        TOKEN token = TOKEN(tokenAddr);
        token.doClaimNft(msg.sender);
        ...
        if (pleIdsOld.length == 1) {
            uint256[] memory pleIdsNew = new uint256[](0);
            pelMap[msg.sender] = pleIdsNew;
            return;
        } else {
            uint256[] memory pleIdsNew = new uint256[](pleIdsOld.length - 1);
            bool isCept;
            for (uint256 i = 0; i < pleIdsOld.length; i++) {
                if (tokenId != pleIdsOld[i]) {
                    if (!isCept) {
                        pleIdsNew[i] = pleIdsOld[i];
                    } else {
                        pleIdsNew[i - 1] = pleIdsOld[i];
                    }
                } else {
                    isCept = true;
                }
    ...
    // get ple status yourself
    function getPleStatus(uint256 tokenId, address addr) public view returns (bool){
        bool isPle = false;
        for (uint256 i = 0; i < pelMap[addr].length; i++) {
            if (tokenId == pelMap[addr][i]) {
                isPle = true;
                break;
            }
        }
    ...
    // get ple tokenIds list  yourself
    function getPleList(address addr) public view returns (uint256[] memory){
        return pelMap[addr];
    }

    // get total power
    function getPowerAll() public view returns (uint256){
        return totalPower;
    }
    // get power yourself
    function getPower(address addr) public view returns (uint256){
        uint256[] memory pleIds = pelMap[addr];
        uint256 power = 0;
        for (uint256 i = 0; i < pleIds.length; i++) {
            if (pleIds[i] <= amountShip[1]) {
                power += 650;
            } else if (pleIds[i] <= amountShip[2] && pleIds[i] > amountShip[1]) {
                power += 2500;
            } else if (pleIds[i] <= amountShip[3] && pleIds[i] > amountShip[2]) {
                power += 6500;
            } else if (pleIds[i] <= amountWork[1] && pleIds[i] > amountShip[3]) {
                power += 16000;
            } else if (pleIds[i] <= amountWork[2] && pleIds[i] > amountWork[1]) {
                power += 30000;
            }
        }
        return power;
    ...
    // mint card
    function mintCard(uint256 tpe) public {
        ...
        USDT token = USDT(asset);
        token.transferFrom(_msgSender(), owner(), price);
    ...
    // mint card bnb
    function mintCardBnb(uint256 tpe) public payable {
        require(isSaleActive, "Sale is not active");
        require(totalSupply().add(1) <= maxNumber, "Purchase would exceed max supply of Martial");
        uint256 mintIndex;
        uint256 price;
        if (tpe == 1) {//ship one star
            require(amountShipOneTotal().add(1) <= amountShip[1], "Purchase would exceed max supply of Martial");
            mintIndex = amountShipOneTotal().add(1);
            price = priceShipBnb[1];
            _amountShipOneSet(msg.sender, mintIndex);
        } else if (tpe == 2) {//ship two star
            require(amountShipTwoTotal().add(1).add(amountShip[1]) <= amountShip[2], "Purchase would exceed max supply of Martial");
            mintIndex = amountShipTwoTotal().add(1).add(amountShip[1]);
            price = priceShipBnb[2];
            _amountShipTwoSet(msg.sender, mintIndex);
        } else if (tpe == 3) {//ship three star
            require(amountShipThreeTotal().add(1).add(amountShip[2]) <= amountShip[3], "Purchase would exceed max supply of Martial");
            mintIndex = amountShipThreeTotal().add(1).add(amountShip[2]);
            price = priceShipBnb[3];
            _amountShipThreeSet(msg.sender, mintIndex);
        } else if (tpe == 4) {//work one star
            require(amountWorkOneTotal().add(1).add(amountShip[3]) <= amountWork[1], "Purchase would exceed max supply of Martial");
            mintIndex = amountWorkOneTotal().add(1).add(amountShip[3]);
            price = priceShipBnb[4];
            _amountWorkOneSet(msg.sender, mintIndex);
        } else if (tpe == 5) {//work two star
            require(amountWorkTwoTotal().add(1).add(amountWork[1]) <= amountWork[2], "Purchase would exceed max supply of Martial");
            mintIndex = amountWorkTwoTotal().add(1).add(amountWork[1]);
            price = priceShipBnb[5];
            _amountWorkTwoSet(msg.sender, mintIndex);
        } else {
            return;
        }
    ...
    function setAssetAddr(address addr) public onlyOwner {
        asset = addr;
    }

    function setPriceBnb(uint256[] memory prices) public onlyOwner {
        priceShipBnb[1] = prices[0]* 10 **13;
        priceShipBnb[2] = prices[1]* 10 **13;
        priceShipBnb[3] = prices[2]* 10 **13;
        priceShipBnb[4] = prices[3]* 10 **13;
        priceShipBnb[5] = prices[4]* 10 **13;
    }
