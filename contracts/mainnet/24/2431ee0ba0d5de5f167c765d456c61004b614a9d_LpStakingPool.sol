...
contract LpStakingPool is USDTWrapper,Ownable {
    ...
    uint256 public totalOutput = 2100000e18;
    uint256 public totalDuration = 60 * 60 * 24 * 26000;

    uint256 public starttime;
    uint256 public withdrawntime;
    ...
    constructor() public {
        ...
        address inToken_ = 0x2619A3e3e3F3b0EFf5520F27bD6d5d2E780217ec;

        address outToken_ = 0x0bfE73A6BC8A20FC3916a8E86969C554627853e9;

        referrerAddress[msg.sender] = 0x372E5543BD10411766Ee47B0d490678996f585d5;

        uint256 starttime_ = 1678201200;

        deposits[0xB74387861EcF82b9BCFC4cE66aF69eF254a00B3c] = 800000e18;
    ...
    modifier checkWithdrawn() {
        require(block.timestamp >= withdrawntime, ' not start');
        _;
    }
    ...
    function exit() external {
        ...
        getReward(rewards[msg.sender]);
    }

    function getReward(uint256 amount) public updateReward(msg.sender) {
        require(amount <= rewards[msg.sender], "amount error");
        if (amount > 0) {
            rewards[msg.sender] = rewards[msg.sender] - amount;
            stakeOutToken.safeTransfer(msg.sender, amount);

            if (referrerAddress[msg.sender] != address(0)) {
                  stakeOutToken.safeTransfer(referrerAddress[msg.sender], amount * 10 /100);
                  if (referrerAddress[referrerAddress[msg.sender]] != address(0)) {
                         stakeOutToken.safeTransfer(referrerAddress[referrerAddress[msg.sender]], amount * 5 /100);
                  }
            }
      ...
      function setreferrerAddress(address readdr) external {
        ...
        require(referrerAddress[readdr] != address(0), "readdr is not null");
