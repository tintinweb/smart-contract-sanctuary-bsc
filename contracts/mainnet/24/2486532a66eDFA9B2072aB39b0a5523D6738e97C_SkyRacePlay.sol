...
contract SkyRacePlay is Ownable {
    ...
    function creatorWithdrawalMargin (uint256 _roomId, uint256 _amount, uint256 _withdrawalType) public {
        require(roomList[_roomId].oddsType == 2, "Types of abnormal");
        require(roomList[_roomId].creator == msg.sender, "Not Creator");
        require(roomList[_roomId].roomDeposit >= _amount, "Lack of balance[1]");
        require(roomList[_roomId].roomDeposit > 0, "Lack of balance[2]");

        uint256 _quizID = roomList[_roomId].quizID;
        uint256 _baseNum = 1000000000000000000;
        ...
        if (quizList[_quizID].result == 3) {
            ...
            if (_withdrawalType == 1) {
                ...
                require(_oneFinal * roomList[_roomId].resultTwoFixedOdds / _baseNum == roomList[_roomId].resultTwoTotal * roomList[_roomId].resultOneFixedOdds / _baseNum, "The amount is wrong [1]");
                ...
                require(roomList[_roomId].resultOneTotal * roomList[_roomId].resultTwoFixedOdds / _baseNum  == _twoFinal * roomList[_roomId].resultOneFixedOdds / _baseNum , "The amount is wrong [2]");
                platformToken.safeTransfer(msg.sender, _amount);
                roomList[_roomId].roomDeposit = roomList[_roomId].roomDeposit - _amount;
            }

            else if (_withdrawalType == 3) {
                require(quizList[_quizID].result == 1, "The result is unknown [1]");
                uint256 _oneFinal = roomList[_roomId].resultOneTotal + roomList[_roomId].roomDeposit - _amount;
                require(_oneFinal * roomList[_roomId].resultTwoFixedOdds / _baseNum  == roomList[_roomId].resultTwoTotal * roomList[_roomId].resultOneFixedOdds / _baseNum , "The amount is wrong [3]");

                uint256 _creatorPrincipal = roomList[_roomId].roomDeposit - _amount;
                uint256 _creatorWin = getWinAmount(_roomId, 1, _creatorPrincipal);
                if (_creatorWin >= roomList[_roomId].resultTwoTotal) {
                    _creatorWin = roomList[_roomId].resultTwoTotal;
                }

                platformToken.safeTransfer(msg.sender, (_creatorWin + roomList[_roomId].roomDeposit));

                roomList[_roomId].roomDeposit = 0;
            }

            else if (_withdrawalType == 4) {
                require(quizList[_quizID].result == 2, "The result is unknown [2]");
                uint256 _twoFinal = roomList[_roomId].resultTwoTotal + roomList[_roomId].roomDeposit - _amount;
                require(roomList[_roomId].resultOneTotal * roomList[_roomId].resultTwoFixedOdds / _baseNum  == _twoFinal * roomList[_roomId].resultOneFixedOdds / _baseNum , "The amount is wrong [4]");

                uint256 _creatorPrincipal = roomList[_roomId].roomDeposit - _amount;
                uint256 _creatorWin = getWinAmount(_roomId, 2, _creatorPrincipal);
                if (_creatorWin >= roomList[_roomId].resultOneTotal) {
                    _creatorWin = roomList[_roomId].resultOneTotal;
                }
