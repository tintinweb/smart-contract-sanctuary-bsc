...
contract NotAFatGreek is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    NotAFatGreekDividendTracker public dividendTracker;
    ...
    constructor() ERC20("NotAFatGreek", "NTFGTest") {
        marketingWallet = payable(0xC8218d77125b7c08076536EB0a08922130a7bdf1);
        devWallet = payable(0xcC178dDD4B9C799cf12B28f771C0B98fB46f95b2);
        ...
        dividendTracker = new NotAFatGreekDividendTracker(payable(this), router, 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56,
                                                   "NotAFatGreekTRACKER", "NTFGTestTRACKER");
...
contract NotAFatGreekDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    NotAFatGreek public NotAFatGreekContract;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        NotAFatGreekContract = NotAFatGreek(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main NotAFatGreek contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(NotAFatGreekContract);

            uint256 prevBalance = NotAFatGreekContract.balanceOf(address(this));
            ...
            {
                uint256 received = NotAFatGreekContract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    NotAFatGreekContract.transfer(account, received);
                } 
                else {
                    success = false;
                }
