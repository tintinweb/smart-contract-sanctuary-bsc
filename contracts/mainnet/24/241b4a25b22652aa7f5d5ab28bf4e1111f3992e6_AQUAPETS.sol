...
contract AQUAPETS is Context, IERC20, Ownable {
  ...
  uint256 public maxWalletAmount = _tTotal.mul(200).div(10000);
  string private _name = 'AquaPets';
  string private _symbol = 'AQP';
  uint8 private _decimals = 9;
  bool public antiwhaleEnabled = false;

// wallet to wallet fee
...
uint256 public buybackFeeSell = 2;
 ...
 uint256 public reflectFeeBuy = 0;
 uint256 public reflectFeeSell = 0;

 uint256 public totalFeeBuy = 10;
 uint256 public totalFeeSell = 14;

  address private _marketingWalletAddress = 0x158b2421a4F7e2767dA0217763c530A0A35dB658; //Affixed and created dynamically by the smart contract on construction time.
  address public _buybackWallet = 0x4AB25EEef1A4Ef411B50B75C47d07E8c88097C02;
  address public _charityWallet = 0xcaa0A3B73359754F2a9Eb0fa99e9dAc757cc8D00;
  address public _developmentWallet = 0x79FB653c9E13B0d3D1aCCBB296EE41d782719a4c;
  ...
  address public immutable AquaPetsUniswapV2Pair;
  ...
  uint256 private numTokensSellToAddToLiquidity = 1_000_000_000 * 10**9; //1 order of magnitude smaller than the max tx amount (so, div by 10, sort of)
  ...
  constructor() {
    ...
    AquaPetsUniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
 ...
 function setSellFees() private {
  ...
  function setDevelopmentWallet(address account) public onlyOwner {
      _developmentWallet = account;
  }
  ...
  function _transfer(
    ...
    if ((!launched() && to == AquaPetsUniswapV2Pair) || manualLaunch) {
      require(
        balanceOf(from) > 0,
    ...
    if (antiwhaleEnabled && ((from != owner() && to != owner()) || !(_isExcludedFromTxLimit[from]))) {
      require(
        amount <= _maxTxAmount,
        'GalaxyDoge: Transfer amount exceeds the maxTxAmount.'
      );
    }
    if(!_isExcludedFromMaxWalletLimit[from] && !_isExcludedFromMaxWalletLimit[to] && to != AquaPetsUniswapV2Pair) {
  ...
  function setSellFee(uint buyback, uint marketing, uint liquidity, uint reflect, uint charity, uint development) public onlyOwner {
      buybackFeeSell = buyback;
      marketingFeeSell = marketing;
      liquidityFeeSell = liquidity;
      reflectFeeSell = reflect;
      charityFeeSell = charity;
      developmentFeeSell = development;
      require(buyback + marketing + liquidity + reflect + charity + development <= 25, "max 25%");
  }

    function setBuyFees(uint buyback, uint marketing, uint liquidity, uint reflect, uint charity, uint development) public onlyOwner {
      buybackFeeBuy = buyback;
      marketingFeeBuy = marketing;
      liquidityFeeBuy = liquidity;
      reflectFeeBuy = reflect;
      charityFeeBuy = charity;
      developmentFeeBuy = development;
      require(buyback + marketing + liquidity + reflect + charity + development <= 25, "max 25%");
  }
  ...
  function _tokenTransfer(
    ...
    if(takeFee && sender == AquaPetsUniswapV2Pair) {
        // buy
        setBuyFees();
    } else if(takeFee && recipient == AquaPetsUniswapV2Pair) {
        // sell
        setSellFees();
    }
    ...
    if(takeFee && (sender == AquaPetsUniswapV2Pair || recipient == AquaPetsUniswapV2Pair)) {
        restoreAllFee();
    }
