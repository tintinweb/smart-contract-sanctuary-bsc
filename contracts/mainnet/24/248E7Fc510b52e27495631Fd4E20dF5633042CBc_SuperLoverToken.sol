...
contract SuperLoverToken is ERC20, Ownable {
    ...
    uint256 private totalSupply_ = 10E11* (10**18);
    ...
    uint256 public minimumTokenBalanceForDividends = 10E7* (10**18);
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            if(ammPairs[from]){
                ...
                _USDTBonusFee = amount.mul(USDTBonusFee).div(100);
                AmountTokenRewardsFee += _USDTBonusFee;
                _MarketingFee = amount.mul(marketingFee).div(100);
                AmountMarketingFee += _MarketingFee;
                _DeadFee = amount.mul(deadFee).div(100);

                _takeInviterFeeKt(amount.div(10000)); //0.01%空投

                FeeTokenAmount = _USDTBonusFee.add(_MarketingFee).add(_DeadFee);
                ...
                _DeadFee = amount.mul(deadFee).div(100);
                 ...
                 FeeTokenAmount = _USDTBonusFee.add(_MarketingFee).add(_DeadFee);
                    ...
                    super._transfer(from, deadWallet, amount.mul(A).div(100));
                    amount = amount.sub(amount.mul(A).div(100));
            }

            amount = amount.sub(FeeTokenAmount);
            if(_DeadFee > 0) super._transfer(from, deadWallet, _DeadFee);
             super._transfer(from, address(this), FeeTokenAmount.sub(_DeadFee));
    ...
    function _takeInviterFeeKt(
     ...
     function swapAndSendToFee(uint256 tokens) private  {
        ...
        IERC20(USDT).transfer(marketingWalletAddress, newBalance.mul(2).div(3));
        IERC20(USDT).transfer(teamWalletAddress, newBalance.div(3));
