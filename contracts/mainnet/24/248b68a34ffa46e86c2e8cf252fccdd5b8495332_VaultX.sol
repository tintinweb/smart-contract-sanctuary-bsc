...
/*
    VaultX is a digital wallet that allows users to store, manage and use their digital assets such as cryptocurrencies, tokens and other blockchain based assets. 
    The wallet will be built on the blockchain to ensure the user's assets are stored securely and transparently.

    VaultX-wallet allows users to create multiple wallets and assign different levels of access and permissions to them. 
    Users can also create multiple addresses for each VaultX-wallet, allowing them to use the same wallet for different transactions or separate their funds.

    VaultX-wallet supports multiple blockchain protocols, allowing users to store different types of digital assets in the same wallet. 
    It will also support multi-signature functionality, which allows multiple users to sign a transaction before it is executed,
    providing an extra layer of security for wallet users.

    Telegram - https://t.me/VaultXwallet
*/
...
contract VaultX is Context, IERC20, Ownable {
    ...
    string private _name = "VaultX";
    string private _symbol = "VXW";
    ...
    address public charityWallet = 0xddD186Fb9B215f284b6fC1F48B15576eD75bCa5E;
