...
contract InfiniteMonkey is Context, IERC20, IERC20Extented, Ownable {
    using SafeMath for uint256;
    string private constant _name = "Infinite Monkey";
    string private constant _symbol = "IFM";
    ...
    uint256 public _maxWallet = _tTotal.mul(3).div(200);
    ...
    uint256 public _buyBBackFee = 1;
    uint256 private _previousBuyBBackFee = _buyBBackFee;
    ...
    uint256 public _sellBBackFee = 1;
    uint256 private _previousSellBBackFee = _sellBBackFee;

    uint256 public _marketingPercent = 40;
    uint256 public _liquidityPercent = 40;
    uint256 public _buybackPercent = 20;
    ...
    address payable private _marketingAddress = payable(0x2c321774823Be48baCe981e08F2d7378e4CE2754);
    address payable private _liquidityAddress = payable(_msgSender());
    address payable private _buybackAddress = payable(0xE193d62393C004B50Cc5f37CCCB9752E0E74fA84);
    ...
    event PercentsUpdated(uint256 _marketingPercent, uint256 _liquidityPercent, uint256 _buybackPercent);
    event FeesUpdated(uint256 _buyLiquidityFee, uint256 _buyMarketingFee, uint256 _buyBBackFee, uint256 _buyBurnFee, uint256 _sellLiquidityFee, uint256 _sellMarketingFee, uint256 _sellBBackFee, uint256 _sellBurnFee);
    ...
    constructor() {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); //bsc test 0xD99D40B5Aade4F78e8285944Bd3Ff59788feD9fE);//bsc main net 0x10A4e305939955159ba1778d029b8e51114d88de);
    ...
    function setSellLimit(bool val) external onlyOwner() {
        sellLimit = val;
        emit UpdateSellLimit(val);
    }

    function setFreeTransfer(bool val) external onlyOwner() {
        freeTrf = val;
        emit UpdateNtr(val);
    }
    ...
    function _getValuesSell(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        (sellFees.tTransferAmount, sellFees.tLiquidity, sellFees.tMarketing) = _getTValuesSell(tAmount, _sellLiquidityFee, _sellMarketingFee.add(_sellBBackFee).add(_sellBurnFee));
        uint256 currentRate = _getRate();
    ...
    function _getValuesBuy(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        (buyFees.tTransferAmount, buyFees.tLiquidity, buyFees.tMarketing) = _getTValuesBuy(tAmount, _buyLiquidityFee, _buyMarketingFee.add(_buyBBackFee).add(_buyBurnFee));
        uint256 currentRate = _getRate();
    ...
    function setBot(address account, bool vl) external onlyOwner() {
        bots[account] = vl;
    }
    ...
    function setPercents(uint256 marketingPercent, uint256 liquidityPercent, uint256 bbp) external onlyOwner() {
        require(marketingPercent.add(liquidityPercent).add(bbp) == 100, "Sum of percents must equal 95");
        _marketingPercent = marketingPercent;
        _liquidityPercent = liquidityPercent;
        _buybackPercent = bbp;
        emit PercentsUpdated(_marketingPercent, _liquidityPercent, _buybackPercent);
    }

    function setTaxes(uint256 buyMarketingFee, uint256 buyLiquidityFee, uint256 buybbfee, uint256 buyburnfee, uint256 sellMarketingFee, uint256 sellLiquidityFee, uint256 sellbbfee, uint256 sellburnfee) external onlyOwner() {
        require(buyMarketingFee.add(buyLiquidityFee).add(buybbfee).add(buyburnfee) < 50, "Sum of sell fees must be less than 50");
        require(sellMarketingFee.add(sellLiquidityFee).add(sellbbfee).add(sellburnfee) < 50, "Sum of buy fees must be less than 50");
        _buyMarketingFee = buyMarketingFee;
        _buyLiquidityFee = buyLiquidityFee;
        _buyBBackFee = buybbfee;
        _buyBurnFee = buyburnfee;
        _sellMarketingFee = sellMarketingFee;
        _sellLiquidityFee = sellLiquidityFee;
        _sellBBackFee = sellbbfee;
        ...
        emit FeesUpdated(_buyMarketingFee, _buyLiquidityFee, _buyBBackFee, _buyBurnFee, _sellMarketingFee, _sellLiquidityFee, _sellBBackFee, _sellBurnFee);
    }

    function setPriceImpact(uint256 priceImpact) external onlyOwner() {
        require(priceImpact <= 10000, "max price impact must be less than or equal to 100");
    ...
    function setBuyBackAddress(address Address) external onlyOwner() {
        _buybackAddress = payable(Address);
    }
