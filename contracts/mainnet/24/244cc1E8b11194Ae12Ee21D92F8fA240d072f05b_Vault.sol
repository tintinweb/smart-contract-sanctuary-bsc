...
contract Vault is Ownable, Signers, VerifySigEIP712 {
    ...
    bytes4 private constant UNO_CALL_SELECTOR = 0x0502b1c5;
    bytes4 private constant UNO_TO_CALL_SELECTOR = 0xf78dc253;
    bytes4 private constant V3_CALL_SELECTOR = 0xe449022e;
    bytes4 private constant V3_TO_CALL_SELECTOR = 0xbc80f1a8;
    ...
    function cBridge(address _token, uint256 _amount, BridgeDescription calldata bDesc) external payable {
    ...
    function swapRouter(SwapData calldata _swap, bytes calldata _data) external payable {
        ...
        uint256 dstAmount = _swapStart(swapData.srcToken, msg.sender, swapData.amount, swapData.minReturnAmount, _data);
    ...
    function swapCBridge(SwapData calldata _swap, bytes calldata _data, BridgeDescription calldata bDesc) external payable {
        ...
        _isNativeDeposit(IERC20(swapData.srcToken), swapData.amount);
        uint256 dstAmount = _swapStart(swapData.srcToken, msg.sender, swapData.amount, swapData.minReturnAmount, _data);
        _feeStart(swapData.dstToken, dstAmount);
        _cBridgeStart(_swap.dstToken, dstAmount, bDesc);
    }

    function swapMultichain(SwapData calldata _swap, bytes calldata _data, MultiChainDescription calldata mDesc) external payable {
        SwapData calldata swapData = _swap;
        _isNativeDeposit(IERC20(swapData.srcToken), swapData.amount);
        uint256 dstAmount = _swapStart(swapData.srcToken, msg.sender, swapData.amount, swapData.minReturnAmount, _data);
        _feeStart(swapData.dstToken, dstAmount);
    ...
    function _feeStart(address dstToken, uint256 dstAmount) private {
    ...
    function _swapStart(address _token, address _user, uint256 _amount, uint256 minOut, bytes calldata _data) private returns (uint256 dstAmount) {
        bytes4 selector = bytes4(_data);
        (bool succ, bytes memory _data) = address(ROUTER).call{value: _amount}(_data);
        if (succ) {
            if (selector == SWAP_CALL_SELECTOR) {
                (uint256 returnAmount, uint256 spentAmount) = abi.decode(_data, (uint256, uint256));
                uint256 unspentAmount = _amount - spentAmount;
                if (unspentAmount > 0) {
                    if (!_isNative(IERC20(_token))) {
                        IERC20(_token).safeTransfer(_user, unspentAmount);
                    } else {
                        _safeNativeTransfer(_user, unspentAmount);
                    }
                }
                dstAmount = returnAmount;
            } else if (
                selector == RFQ_CALL_SELECTOR ||
                selector == RFQ_TO_CALL_SELECTOR ||
                selector == ORDER_CALL_SELECTOR ||
                selector == ORDER_TO_CALL_SELECTOR
            ) {
                (, uint256 returnAmount, ) = abi.decode(_data, (uint256, uint256, bytes32));
                dstAmount = returnAmount;
            } else if (
                selector == UNO_CALL_SELECTOR ||
                selector == UNO_TO_CALL_SELECTOR ||
                selector == V3_CALL_SELECTOR ||
                selector == V3_TO_CALL_SELECTOR ||
                selector == CLIPPER_CALL_SELECTOR ||
                selector == CLIPPER_TO_CALL_SELECTOR
            ) {
    ...
    function relaySwapRouter(
        ...
        uint256 dstAmount = _swapStart(sig.fromTokenAddress, sig.userAddress, tokenAmount, sig.minOut, _data);
