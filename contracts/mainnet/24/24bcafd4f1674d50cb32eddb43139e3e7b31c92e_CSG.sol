...
contract CSG is ERC20 {
    ...
    function _transfer(
        ...
        if (takeFee) {
			_splitOtherToken();
            super._transfer(from, address(this), amount.div(200).mul(14 + delldxRate));
    ...
    function swapAndLiquify(uint256 contractTokenBalance) public {
		...
		if(allUsdt > haveUsdt){
			...
			USDT.transfer(_fundAddress,newUsdt.div(11).mul(7));
		}
    }
	...
	function _takeInviterFee(
        ...
        for (int256 i = 0; i < 3; i++) {
            ...
            super._transfer(sender, _receiveD, amount.div(200).mul(rate));
        }
    }
}