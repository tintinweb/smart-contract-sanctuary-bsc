...
contract StorageV1 is Initializable, OwnableUpgradeable, PausableUpgradeable {
    ...
    modifier isLogicContract(address account) {
        require(logicContract == account, "is not  logicContract");
        _;
    }
    ...
    function interestFee() public {
        ...
        require(
            ...
            "contracrt hasn't enough for interest fee, please contact the administrator"
        );
    ...
    function addEarn(uint256 amount) external isLogicContract(msg.sender) {
        ...
        earnBLID[countEarns_].tdt = uint256(
            (int256(((block.timestamp) * earnBLID[countEarns_].usd)) -
                _dollarTime) / (1 ether) 
                ); // count delta of current token time and all user token time

        for (uint256 i = 0; i < countTokens_; i++) {
            tokenTime[tokens[i]] = int256(
                tokenDeposited[tokens[i]] * block.timestamp
            ); // count curent token time 
        }
    ...
    function _upBalanceByItarate(address account, uint256 iterate) external {
        ...
        uint sum=0;
        require(
            countEarns - userIterate >= iterate,
            "itarate more than you can itarate"
        );
        uint256 countTokens_ = countTokens;
        for (
            uint256 i = userIterate;
            i < iterate + userIterate;
            i++
        ) {
            for (uint256 j = 0; j < countTokens_; j++) {
                ...
                if (i == deposits[account].depositIterate[tokens[j]]) {
                    sum +=
                        (earnBLID[i].allBLID * uint256((int256( // all distibution BLID multiply to
                            (deposits[account].amount[tokens[j]] * earnBLID[i].rates[tokens[j]]) * earnBLID[i].timestamp
                                ) -
                            ...
                            )) / earnBLID[i].tdt / (1 ether);//div to delta of all users dollar time and all users dollar time if all users deposited in at the beginning distibution 
                } else {
                    sum +=
    ...
    function balanceEarnBLID(address account) public view returns (uint256) {
        ...
        uint256 sum = 0;
        for (
            uint256 i = deposits[account].iterate;
            i < countEarns_; 
            i++
        ) {
            for (uint256 j = 0; j < countTokens_; j++) {
    ...
    function balanceOf(address account) external view returns (uint256) {
        ...
        for (uint256 j = 0; j < countTokens_; j++) {
            sum += ((deposits[account].amount[tokens[j]] *
                uint256(
                    AggregatorV3Interface(oracles[tokens[j]]).latestAnswer()
                ) *
