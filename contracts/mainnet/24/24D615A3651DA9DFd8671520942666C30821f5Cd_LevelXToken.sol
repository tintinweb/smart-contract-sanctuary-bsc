...
contract LevelXToken is Initializable, Ownable, ERC20
{
	...
	uint256 constant DEFAULT_MINIMUM_REWARD_BALANCE_TO_SWAP = 0.01e18; // 0.01 BNB
	...
	function initialize(address _owner, string memory _name, string memory _symbol, address _router) public initializer
	{
		...
		excludeFromTradeFeeAsBuyer[_owner] = true;
		excludeFromTradeFeeAsSeller[_owner] = true;
	...
	{
		// swaps LVLX to BNB
		// should be protected by inswap_
	...
	function _swapBNBToReward(address _rewardToken, uint256 _bnbAmount) internal
	{
		...
		// protection is not necessary as it does not involve LVLX
		// (unless the swap path includes LVLX, which should not be the case)
		RewardInfo storage _rewardInfo = rewardInfo[_rewardToken];
