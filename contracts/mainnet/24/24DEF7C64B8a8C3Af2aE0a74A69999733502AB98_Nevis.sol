...
contract Nevis is Context, IERC20, Ownable {
    ...
    uint256 public _redistributionFee = 5;
    uint256 private _previousredistributionFee = _redistributionFee;
    
    uint256 public _NevisbankFee = 5;
    address public _NevisbankWallet;
    uint256 private _previousNevisbankFee = _NevisbankFee;

    uint256 public _NevisswapSellFee = 20;
    uint256 private _previousNevisswapsellFee = _NevisswapSellFee;

    address public _NevisRewardWallet = 0xDC3be9878ae4Ad7EFe787c2C42995316cb066C3f;
 ...
 function includeInDivident(address account) external onlyOwner() {
    ...
    function getTokenPrice() public view returns(uint[] memory amounts)
        {
            ...
            path[0] = busd;
            path[1] = address(this);
            return uniswapV2Router.getAmountsIn(1, path);
        }
    ...
    function distributerewards(uint256 amount) external payable onlyOwner{
        ...
        uint256 userreward ;
        (uint256[] memory _price)=getTokenPrice();
        uint256 holdingbusd=totalHoldings.mul(_price[0]);
        for(uint256 i = 0; i < investors.length; i++){
            User storage user = users[investors[i]];
            if (!_isExcludeddividend[investors[i]]) {
                uint256 nevisinbusd=_tOwned[investors[i]].mul(_price[0]);
    ...
    function setRewardFeePercent(uint256 redistributionFee) external onlyOwner() {
    ...
    function setNevisbankBuyFeePercent(uint256 NevisbankFee) external onlyOwner() {
        require(NevisbankFee < 6, "Invalid Fees");
        _NevisbankFee = NevisbankFee;
        emit setfees(NevisbankFee);
    }

    function setNevisbankSellFeePercent(uint256 NevisswapFee) external onlyOwner() {
        require(NevisswapFee < 21, "Invalid Fees");
        _NevisswapSellFee = NevisswapFee;
        emit setfees(NevisswapFee);
    }
    
    function setbusd(address newbusd) external onlyOwner() {
        busd = newbusd;
    }
