...
interface iNitroBUSDPool {
    ...
    function idToAddress(uint id) external view returns (address _user);
    function getReferralsByRound(address _user, uint _round) external view returns(uint refCount);
}

contract NitroBUSDPool is Ownable {

    address public constant NitroBUSDV1 = address(0x315F8C3cf88224Be79bDe4Cb9b26fD196112D376);
    iNitroBUSDPool NitroBUSD = iNitroBUSDPool(NitroBUSDV1);
    ...
    function initUsers(uint _from, uint _to) public onlyOwner() {
        require(_from > 1);
        require(_from < _to);
        for (uint i = _from; i <= _to; i++ ) {
            address _wallet = NitroBUSD.idToAddress(i);
            (uint256 _v1uid, address _v1sponsor, uint256 _v1refcount,,,,) = NitroBUSD.User(_wallet);
            uint _refcount = NitroBUSD.getReferralsByRound(_wallet, 1);

            Users storage u = User[_wallet];
            u.id =_v1uid;
            u.invitedBy =_v1sponsor;
            u.referrals = _v1refcount;                       
            u.roundToReferrals[currentRound] = _refcount;

            idToAddress[_v1uid] = _wallet;
            addressToId[_wallet] = _v1uid;
        }
        totalUsers = _to;
    }
