...
// File contracts/v1contracts/dependencies/WithOperator.sol
// License-Identifier: MIT

pragma solidity =0.6.12;

abstract contract WithOperator is Ownable {
    ...
    modifier onlyOperator() {
        require(operator() == _msgSender(), "WithOperator: caller is not the operator");
        _;
    }

    modifier onlyOwnerOrOperator() {
        require(operator() == _msgSender() || owner() == _msgSender(), "WithOperator: caller is not the owner nor the operator");
        _;
    }
...
abstract contract UpgradeableMiningBaseBefore is Proxiable, IUpgradeableMiningBase {
...
abstract contract UpgradeableMiningBase is UpgradeableMiningBaseBefore, WithOperator {}
...
contract MiningWithToken is UpgradeableMiningBase {
    string public constant contractName = "MiningWithToken MT";
    string public constant contractVersion = "2.5";
    ...
    function updateTarget(address _targetAddr, address[] calldata _path) external doReinvest(_path) onlyOwnerOrOperator {
        pools[0].stakeToken.approve(pools[0].target, 0);
        pools[0].target = _targetAddr;
    ...
    function setRouter(address _router) external onlyOwnerOrOperator {
        router = IPancakeRouter02(_router);
    }
    ...
    function setMinRelease(uint256 _minRelease) external onlyOwnerOrOperator {
        pools[0].minRelease = _minRelease;
    }
    ...
    function setPenaltyConfig(uint256 _penaltyRate, uint256 _penaltyEndTime) external onlyOwnerOrOperator {
        pools[0].penaltyRate = _penaltyRate;
        pools[0].penaltyEndTime = _penaltyEndTime;
    }

    function getPenaltyConfig() external view returns (uint256 _penaltyRate, uint256 _penaltyEndTime) {
        _penaltyRate = pools[0].penaltyRate;
        _penaltyEndTime = pools[0].penaltyEndTime;
    }

    function updatePenalty(uint256 prevAmount, uint256 curAmount, address user) internal {
        uint256 diff = prevAmount.sub(curAmount);
        uint256 penalty = diff.mul(pools[0].penaltyRate).mul(users[user].rewardSettled).div(prevAmount).div(RATE_BASE);
        users[user].rewardSettled = users[user].rewardSettled.sub(penalty);

        emit RewardPenalty(0, user, penalty, block.timestamp);
    }
    ...
    function setRewardPerBlock(uint256 _rewardPerBlock) external override onlyOwnerOrOperator {
        updatePool();
        pools[0].rewardPerBlock = _rewardPerBlock;
    }

    function setReceiver(address _receiverAddr) external override onlyOwnerOrOperator {
        pools[0].receiver = _receiverAddr;
    }
    ...
    function stake(uint256 _amount, address[] calldata _path) external override inited reGuard doReinvest(_path) {
        ...
        if (prevAmount > _amount && block.timestamp < pools[0].penaltyEndTime && pools[0].penaltyRate > 0) {
            updatePenalty(prevAmount, _amount, msg.sender);
        }
    ...
    function reinvestTarget(address[] calldata _path) external override onlyOwnerOrOperator doReinvest(_path) {}
