...
contract Vault is ReentrancyGuard, IVault {
    ...
    uint256 public constant USDJ_DECIMALS = 18;
    ...
    address public override usdj;
    address public override gov;

    uint256 public override whitelistedTokenCount;

    uint256 public override maxLeverage = 19 * 10000; // 19x

    uint256 public override liquidationFeeUsd;
    uint256 public override taxBasisPoints = 50; // 0.5%
    uint256 public override stableTaxBasisPoints = 20; // 0.2%
    uint256 public override mintBurnFeeBasisPoints = 25; // 0.25%
    uint256 public override swapFeeBasisPoints = 10; // 0.1%
    uint256 public override stableSwapFeeBasisPoints = 5; // 0.05%
    ...
    // usdjAmounts tracks the amount of USDJ debt for each whitelisted token
    mapping(address => uint256) public override usdjAmounts;

    // maxUsdjAmounts allows setting a max amount of USDJ debt for a token
    mapping(address => uint256) public override maxUsdjAmounts;
    ...
    // this value is used to calculate the redemption values for selling of USDJ
    ...
    event IncreaseUsdjAmount(address token, uint256 amount);
    event DecreaseUsdjAmount(address token, uint256 amount);
    ...
    {
        require(
            msg.sender == address(vaultController),
            "Vault: invalid vaultController"
        );
    ...
    function setTokenConfig(
        ...
        maxUsdjAmounts[_token] = _maxUsdjAmount;
    ...
    function setUsdjAmount(address _token, uint256 _amount) external override {
        _onlyGov();

        uint256 usdjAmount = usdjAmounts[_token];
        if (_amount > usdjAmount) {
            _increaseUsdjAmount(_token, _amount.sub(usdjAmount));
            return;
        }

        _decreaseUsdjAmount(_token, usdjAmount.sub(_amount));
    }

    // deposit into the pool without minting USDJ tokens
        ...
        returns (uint256)
    {
        ...
        uint256 usdjAmount = tokenAmount.mul(price).div(PRICE_PRECISION);
        usdjAmount = adjustForDecimals(usdjAmount, _token, usdj);
        _validate(usdjAmount > 0, 18);

        uint256 feeBasisPoints = vaultUtils.getBuyUsdjFeeBasisPoints(
            _token,
            usdjAmount
        );
        ...
        mintAmount = adjustForDecimals(mintAmount, _token, usdj);

        _increaseUsdjAmount(_token, mintAmount);
        _increasePoolAmount(_token, amountAfterFees);

        IUSDJ(usdj).mint(_receiver, mintAmount);
        ...
        returns (uint256)
    {
        ...
        uint256 usdjAmount = _transferIn(usdj);
        _validate(usdjAmount > 0, 20);

        updateCumulativeFundingRate(_token, _token);

        uint256 redemptionAmount = getRedemptionAmount(_token, usdjAmount);
        _validate(redemptionAmount > 0, 21);

        _decreaseUsdjAmount(_token, usdjAmount);
        _decreasePoolAmount(_token, redemptionAmount);

        IUSDJ(usdj).burn(address(this), usdjAmount);

        // the _transferIn call increased the value of tokenBalances[usdj]
        // usually decreases in token balances are synced by calling _transferOut
        // however, for usdj, the tokens are burnt, so _updateTokenBalance should
        // be manually called to record the decrease in tokens
        _updateTokenBalance(usdj);

        uint256 feeBasisPoints = vaultUtils.getSellUsdjFeeBasisPoints(
            _token,
            usdjAmount
        );
        ...
        emit SellUSDJ(_receiver, _token, usdjAmount, amountOut, feeBasisPoints);
    ...
    function swap(
        ...
        // adjust usdjAmounts by the same usdjAmount as debt is shifted between the assets
        uint256 usdjAmount = amountIn.mul(priceIn).div(PRICE_PRECISION);
        usdjAmount = adjustForDecimals(usdjAmount, _tokenIn, usdj);
        ...
        _increaseUsdjAmount(_tokenIn, usdjAmount);
        _decreaseUsdjAmount(_tokenOut, usdjAmount);
    ...
    function _decreasePosition(
        ...
        if (position.size != _sizeDelta) {
            ...
            vaultController.removePosition(key);
        }
        ...
        returns (uint256)
    {
        ...
        uint256 redemptionAmount = _usdjAmount.mul(PRICE_PRECISION).div(price);
        return adjustForDecimals(redemptionAmount, usdj, _token);
    }
    ...
    function adjustForDecimals(
        ...
        uint256 decimalsDiv = _tokenDiv == usdj
            ? USDJ_DECIMALS
            : tokenDecimals[_tokenDiv];
        uint256 decimalsMul = _tokenMul == usdj
            ? USDJ_DECIMALS
    ...
    function getTargetUsdjAmount(address _token)
        ...
        returns (uint256)
    {
        uint256 supply = IERC20(usdj).totalSupply();
        if (supply == 0) {
            return 0;
        }
    ...
    function _increaseUsdjAmount(address _token, uint256 _amount) private {
        usdjAmounts[_token] = usdjAmounts[_token].add(_amount);
        uint256 maxUsdjAmount = maxUsdjAmounts[_token];
        if (maxUsdjAmount != 0) {
            _validate(usdjAmounts[_token] <= maxUsdjAmount, 51);
        }
        emit IncreaseUsdjAmount(_token, _amount);
    }

    function _decreaseUsdjAmount(address _token, uint256 _amount) private {
        uint256 value = usdjAmounts[_token];
        // since USDJ can be minted using multiple assets
        // it is possible for the USDJ debt for a single asset to be less than zero
        // the USDJ debt is capped to zero for this case
        if (value <= _amount) {
            usdjAmounts[_token] = 0;
            emit DecreaseUsdjAmount(_token, value);
            return;
        }
        usdjAmounts[_token] = value.sub(_amount);
        emit DecreaseUsdjAmount(_token, _amount);
    }
