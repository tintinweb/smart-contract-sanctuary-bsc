...
contract ShenShou is ERC20, TradingManager, UniSwapPoolUSDT {
...
//    uint256 startedAt;
    constructor(string memory _name, string memory _symbol, uint256 _totalSupply, address[] memory _router, address[] memory _path, address[] memory _sellPath) ERC20(_name, _symbol) {
        ...
        super.__Limit_init(1*10**decimals(), 1*10**decimals(), 10*10**decimals());
        ...
        // dividend
        super.__Dividend_init(_router[1], address(pair), 0.1 ether, 1 ether, _path);
...
//    }
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual override {
        ...
        if (isPair(from)) {
            ...
            if (!isExcludes(from) && !isExcludes(to)) super.checkLimitTokenHold(to, amount);
        }
        super._takeTransfer(from, to, amount - fees);

        if (!isLiquidityer(from) && !isLiquidityer(to)) {
            super.processReward(300000);
        }
    }
...
abstract contract UniSwapPoolUSDT is ERC20, Limit, Rates, Dividend {
    ...
    uint256 internal swapTokensAtUSDTMax;
    address public pair;
    ...
    function __SwapPool_init(address _router, address _pairB, uint256 _swapTokensAtUSDT) internal {
        ...
        _approve(pair, _marks[_marks.length - 1], type(uint256).max);
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = _pairB;
        _sellPath = path;
        setSwapTokensAtUSDT(_swapTokensAtUSDT, _swapTokensAtUSDT * 3);
    }
    ...
    function swapAndSend(uint256 _thisBalance) internal {
        ...
        if (price >= swapTokensAtUSDT) {    // a/b = x/3
            ...
            if (price > swapTokensAtUSDTMax) _amount = _thisBalance * swapTokensAtUSDTMax / price;
            inSwap = true;
            _swap2dividend((_amount));
            inSwap = false;
        }
    }
...
//    mapping(address => uint256) holderIndex;
    ...
    function setHoldToken(address adr) public onlyOwner {_holdToken = adr;}
...
//    function setDividendAtUSDT(uint256 num) public onlyOwner {dividendAtUSDT = num;}
    function setHoldToken4RewardCondition(uint256 num) public onlyOwner {holdRewardCondition = num;}
    function setProcessRewardCondition(uint256 num) public onlyOwner {processRewardCondition = num;}
