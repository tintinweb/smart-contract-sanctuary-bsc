...
/**               
NEW YEAR CHAIN
Telegram : https://t.me/NewYearChain
Website : https://newyearchain.ink
...
contract NewYearChain is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    NewYearChainDividendTracker public dividendTracker;
    ...
    constructor() ERC20("NewYearChain", "NYC") {
        marketingWallet = payable(0x53079e02fa455800502e0dE0aD90Bd609BFD84Da);
        buybackfeeWallet = payable(0x29B5dB7DD562df2a1Dde3F60800920E9026026Fc);
        ...
        dividendTracker = new NewYearChainDividendTracker(
            payable(this),
            router,
...
contract NewYearChainDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    NewYearChain public NewYearChainContract;
    ...
    constructor(
        ...
        defaultToken = token;
        NewYearChainContract = NewYearChain(mainContract);
    ...
    function _transfer(
        ...
        require(false, "NewYearChain_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "NewYearChain_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main NewYearChain contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(NewYearChainContract);

            uint256 prevBalance = NewYearChainContract.balanceOf(address(this));
            ...
            {
                uint256 received = NewYearChainContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    NewYearChainContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
