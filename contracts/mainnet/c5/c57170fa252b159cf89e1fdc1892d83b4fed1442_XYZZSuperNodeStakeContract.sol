...
interface IInviteContract {
    ...
    function getMyInviter(address _addr) external view returns(address);

    function getMyInviteeAddress(address _addr) external view returns(address[] memory);
}
 ...
 */
contract XYZZSuperNodeStakeContract is Ownable {
    ...
    struct UserInfo {
        ...
        uint256 receivedReferralReward;
    ...
    struct PoolInfo {
        ...
        uint256 threshold;
        uint256 thresholdRateDenominator;
    ...
    uint256 public bonusPerReferral = 15; 
    ...
    mapping(uint256 => uint256) public currentAccTokenPerShare;
    ...
    constructor(
        ...
        inviteContract = IInviteContract(_inviteContract);
    }
    ...
    function add(IERC20 _lpToken,uint256 _tokenPerBlock,uint256 _threshold,uint256 _thresholdRateDenominator) public onlyOwner nonDuplicated(_lpToken) {
        require(_thresholdRateDenominator > 0,"add: Threshold denominator cannot be zero");
        poolInfo.push(PoolInfo({
            lpToken: _lpToken,
            tokenPerBlock: _tokenPerBlock,
            threshold: _threshold,
            thresholdRateDenominator: _thresholdRateDenominator,
            accTokenPerShare: 0,
    ...
    function set(uint256 _pid,IERC20 _newLpToken,uint256 _tokenPerBlock,uint256 _threshold,uint256 _thresholdRateDenominator) public onlyOwner {
        require(_thresholdRateDenominator > 0,"set: Threshold denominator cannot be zero");
        poolInfo[_pid].lpToken = _newLpToken;
        poolInfo[_pid].tokenPerBlock = _tokenPerBlock;
        poolInfo[_pid].threshold = _threshold;
        poolInfo[_pid].thresholdRateDenominator = _thresholdRateDenominator;
    }
    ...
    function pendingReward(uint256 _pid, address _user) public view returns (uint256 totalPending) {
        ...
        for (uint256 i = 0; i < user.depositInfo.length; i++) {
            if(user.depositInfo[i].status == true) {
                uint256 pending = user.depositInfo[i].amount.mul((accTokenPerShare.sub(currentAccTokenPerShare[user.depositInfo[i].depositBlock]))).div(1e12).sub(user.depositInfo[i].rewardDebt);
    ...
    function myPendingReward(uint256 _pid, address _user) internal returns (uint256 totalPending) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accTokenPerShare = pool.accTokenPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
    ...
    function myReferralReward(uint256 _pid,address _user) public view returns(uint256 totalReferralBonus){
        address[] memory myInviteeAddress = inviteContract.getMyInviteeAddress(_user);
        for (uint256 i = 0; i < myInviteeAddress.length; i++) {
            uint256 referralBonus = (pendingReward(_pid,myInviteeAddress[i]).add(userInfo[_pid][myInviteeAddress[i]].totalMintAmount)).mul(bonusPerReferral).div(referralRateDenominator);
            totalReferralBonus = totalReferralBonus.add(referralBonus);
        }
        return totalReferralBonus.sub(userInfo[_pid][_user].receivedReferralReward);
    }

    function receiveMyReferralReward(uint256 _pid) public {
        uint256 pendingReferralReward = myReferralReward(_pid,msg.sender);
        require(pendingReferralReward > 0, "receiveMyReferralReward: You have no referral rewards to claim");
        if(pendingReferralReward > 0){
            uint256 actualReferralReward = pendingReferralReward.div(10 ** 18 / 10 ** rewardTokenDecimals);
            safeTokenTransfer(msg.sender, actualReferralReward);
            userInfo[_pid][msg.sender].receivedReferralReward = userInfo[_pid][msg.sender].receivedReferralReward.add(pendingReferralReward);
        }
    }

    function judgmentThreshold(uint256 _pid,address _user) public view returns(bool isReached) {
        PoolInfo storage pool = poolInfo[_pid];
        uint256 totalLPTokenSupply = pool.lpToken.totalSupply();
        uint256 mylpTokenSupply = pool.lpToken.balanceOf(_user);
        uint256 proportion = mylpTokenSupply.mul(1e18).div(totalLPTokenSupply);
        if(proportion >= pool.threshold.mul(1e18).div(pool.thresholdRateDenominator)) {
            return true;
        } else {
            return false;
        }
    ...
    function deposit(uint256 _pid, uint256 _amount, uint256 _depositDays) public {
        ...
        require(inviteContract.getMyInviter(msg.sender) != address(0),"You haven't bound an inviter yet");
        require(judgmentThreshold(_pid,msg.sender), "deposit: The account has not reached the pledge threshold");
        require(_amount > 0, "deposit: input amount must be greater than 0");
        require(_depositDays == duration,"deposit: Incorrect pledge days");
        PoolInfo storage pool = poolInfo[_pid];
        ...
        if(_amount > 0) {
            ...
            user.depositInfo.push(DepositInfo({
                    ...
                    depositDays: _depositDays.mul(86400),
    ...
    function claim(uint256 _pid) public {
        ...
        require(IERC20(rewardToken).balanceOf(address(this)) > 0,"claim: The reward token balance of this contract is insufficient"); 
        updatePool(_pid);
        uint256 deservedReward = myPendingReward(_pid,msg.sender);
        require(deservedReward > 0,"claim: You have no reward tokens to claim");
        if(user.totalDepositAmount > 0 && deservedReward > 0 && IERC20(rewardToken).balanceOf(address(this)) > 0){
            uint256 actualReward = deservedReward.div(10 ** 18 / 10 ** rewardTokenDecimals);
            safeTokenTransfer(msg.sender, actualReward);
        }
        user.totalMintAmount = user.totalMintAmount.add(deservedReward);
    ...
    function withdraw(uint256 _pid) public {
        ...
        updatePool(_pid);
        uint256 deservedReward = myPendingReward(_pid,msg.sender);
        if (deservedReward > 0 && IERC20(rewardToken).balanceOf(address(this)) > 0) {
            uint256 actualReward = deservedReward.div(10 ** 18 / 10 ** rewardTokenDecimals);
            safeTokenTransfer(msg.sender, actualReward);
            emit Claim(msg.sender, _pid, deservedReward);
        }
    ...
    function getAllowedWithdrawAmount(uint256 _pid, address _user) public view returns (uint256 amount) {
        for (uint256 i = 0; i < userInfo[_pid][_user].depositInfo.length; i++) {
                if (block.timestamp > (userInfo[_pid][_user].depositInfo[i].lastDepositTime + userInfo[_pid][_user].depositInfo[i].depositDays) && userInfo[_pid][_user].depositInfo[i].status == true) {
                    amount = amount + userInfo[_pid][_user].depositInfo[i].amount;
                }
        }
        return amount;
    }
    ...
    function deleteWithdrawTX(uint256 _pid) private {
        for (uint256 i = 0; i < userInfo[_pid][msg.sender].depositInfo.length; i++) {
            if (block.timestamp > (userInfo[_pid][msg.sender].depositInfo[i].lastDepositTime + userInfo[_pid][msg.sender].depositInfo[i].depositDays) && userInfo[_pid][msg.sender].depositInfo[i].status == true) {
                userInfo[_pid][msg.sender].depositInfo[i].status = false;
            }
        }
    }
    ...
    function deleteAllWithdrawTX(uint256 _pid) private {
        for (uint256 i = 0; i < userInfo[_pid][msg.sender].depositInfo.length; i++) {
            if (userInfo[_pid][msg.sender].depositInfo[i].status == true) {
                userInfo[_pid][msg.sender].depositInfo[i].status = false;
            }
        }
    }
    ...
    function setRewardAddress(address _newRewardToken) public onlyOwner {
        rewardToken = _newRewardToken;
    }

    function setRewardTokenDecimals(uint8 _decimals) public onlyOwner {
