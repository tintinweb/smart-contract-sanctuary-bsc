...
/** 
 * @title Rareboard proxy contract to handle accepting bids on NFTKeyV2 marketplaces.
 * 
 * When configured for automatic commissions, this contract is meant to be owned by a multisig
 * consisting of both parties.
 *
 * To avoid the need for approvals and save on gas cost, this contract can be used by utilising the data field
 * of {IERC721-safeTransferFrom}: `erc721Address`.safeTransferFrom(`msg.sender`, address(this), abi.encode(`HEADER`, `bidder`, `value`)
 */
contract RareboardNFTKeyBidProxy is ERC721Holder, Pausable, Ownable, ReentrancyGuard {
    
    using SafeERC20 for IERC20;
    
    address public immutable MARKET;
    IERC20 public immutable WBNB;
    bytes32 public immutable HEADER = keccak256("RareboardNFTKeyBidProxy/acceptBidForToken");

    // Trading commissions relative to serviceFee, 1000 based. 500 = 50%
    uint256 public commission = 500;
    address public commissionReceiver;
    address public commissionSource;
    
    // Bid is matched by a trade
    event Trade(
    ...
    constructor(address _market, address _wbnb) {
        ...
        require(_wbnb != address(0), "Operations: WBNB address cannot be zero");
        MARKET = _market;
        WBNB = IERC20(_wbnb);
    }
    ...
    /**
     * @notice Accept the given bid.
     * Assumes this contract is approved for the given `erc721Address`.
     * To avoid approvals and safe a bit of gas, use `erc721Address`.safeTransferFrom(`msg.sender`, address(this), abi.encode(`HEADER`, `bidder`, `value`))
     */
    function acceptBidForToken(
        ...
        IERC721(erc721Address).safeTransferFrom(address(msg.sender), address(this), tokenId, abi.encode(HEADER, bidder, value));
    }

    /** 
     * @notice Accepts the given bid through this proxy.
     * @dev Expects the NFT to be owned by this contract
     */
    function _acceptBidForToken(
        ...
        (uint256 serviceFee, uint256 royaltyFee) = calculateFees(erc721Address, value);
        uint256 proceeds = value - serviceFee - royaltyFee;
        uint256 commissions = royaltyFee * commission / 1000;

        IERC721(erc721Address).approve(MARKET, tokenId);
        INFTKEYMarketplaceV2(MARKET).acceptBidForToken(erc721Address, tokenId, bidder, value);
        
        WBNB.safeTransfer(seller, proceeds);

        bool payCommission = commissionSource != address(0) && WBNB.allowance(commissionSource, address(this)) >= commissions;
        if (payCommission) {
            WBNB.safeTransferFrom(commissionSource, commissionReceiver, commissions);
        }

        emit Trade(erc721Address, tokenId, seller, bidder, value, commissions, payCommission);
    }

    /**
     * @dev Configure commissions.
     * When `_source` is not address(0), this contract will transfer the calculated commission from `_source' 
     * to `_receiver` on every trade automatically.
     * `_source` needs to approve this contract for this to work.
     */
    function setCommission(uint256 _commission, address _receiver, address _source) external onlyOwner {
        require(_source == address(0) || _receiver != address(0), "receiver not set");
        require(_commission < 1000, "Invalid commission");

        commission = _commission;
        commissionReceiver = _receiver;
        commissionSource = _source;

        emit CommissionChanged(_commission, _receiver, _source);
    }

    /**
     * @dev Pause operations, only owner.
     */
    ...
    /**
     * @dev Resume normal operations, only owner.
     */
    ...
    /**
     * @notice Calculate the service and royalty fee applied for a given collection and price by NFTKey.
     */
    function calculateFees(address erc721Address, uint256 value)
        public
        view
        returns (uint256 _serviceFee, uint256 _royaltyFee)
    {   
        uint256 _serviceFeeFraction = INFTKEYMarketplaceV2(MARKET).serviceFee();
        uint256 _royaltyFeeFraction = INFTKEYMarketplaceRoyalty(MARKET).royalty(erc721Address).feeFraction;
    ...
    function onERC721Received(
        ...
        require(data.length == 96, "Invalid data");
        (bytes32 header, address bidder, uint256 value) = abi.decode(data, (bytes32, address, uint256));
        require(header == HEADER, "Invalid header");

        _acceptBidForToken(msg.sender, tokenId, bidder, from, value);
