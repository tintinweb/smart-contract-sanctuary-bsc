...
contract WrapperTokenBridge is ReentrancyGuard
{
	...
	function deposit(uint256 _pid, uint256 _type, address _routeToken, uint256 _minAmountOut) external payable nonReentrant returns (uint256 _amountOut)
	{
		...
		if (_routeToken == wrappedToken) {
			...
			_path[1] = _routeToken;
			_amountIn = IUniswapV2Router(router).swapExactETHForTokens{value: _amountIn}(1, _path, address(this), block.timestamp)[_path.length - 1];
		}
		return _deposit(_pid, _type, _routeToken, _amountIn, _minAmountOut, address(0));
	}
	...
	function deposit(uint256 _pid, uint256 _type, address _routeToken, uint256 _minAmountOut, address _referral) external payable nonReentrant returns (uint256 _amountOut)
	{
		...
		return _deposit(_pid, _type, _routeToken, _amountIn, _minAmountOut, _referral);
	}
	...
	function deposit(uint256 _pid, uint256 _type, address _tokenIn, uint256 _amountIn, address _routeToken, bool _directRoute, uint256 _minAmountOut) external nonReentrant returns (uint256 _amountOut)
	{
		...
		if (_routeToken != _tokenIn) {
			...
			_amountIn = IUniswapV2Router(router).swapExactTokensForTokens(_amountIn, 1, _path, address(this), block.timestamp)[_path.length - 1];
		}
		return _deposit(_pid, _type, _routeToken, _amountIn, _minAmountOut, address(0));
	}
	...
	{
		_amountOut = _withdraw(_pid, _type, _routeToken, _amountIn);
		if (_routeToken == wrappedToken) {
	...
	function _deposit(uint256 _pid, uint256 _type, address _tokenIn, uint256 _amountIn, uint256 _minAmountOut, address _referral) internal returns (uint256 _amountOut)
	{
	...
	function _withdraw(uint256 _pid, uint256 _type, address _tokenOut, uint256 _amountIn) internal returns (uint256 _amountOut)
	{
		require(_type < 4, "invalid type");
		(address _poolToken,,,,,,,) = MasterChef(masterChef).poolInfo(_pid);
...
contract MasterChef is Initializable, Ownable, ReentrancyGuard
{
	...
	{
		{
			...
			require(_poolInfo.token == rewardToken, "invalid pid");
		}
		_amount = _updateReferral(msg.sender);
		if (_amount > 0) {
			allocReward -= _amount;
	...
	function compoundAll(uint256 _pid0) external nonReentrant returns (uint256 _amount)
	{
		{
			PoolInfo storage _poolInfo = poolInfo[_pid0];
			require(_poolInfo.token == rewardToken, "invalid pid");
