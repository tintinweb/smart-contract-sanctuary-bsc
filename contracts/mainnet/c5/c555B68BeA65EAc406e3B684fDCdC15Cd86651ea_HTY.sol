...
pragma solidity ^0.6.2;
contract HTY is ERC20, Ownable {
    using SafeMath for uint256;
    IUniswapV2Router02 public uniswapV2Router;
    address public  uniswapV2Pair;
    bool private swapping;
    address public USDTs = address(0x55d398326f99059fF775485246999027B3197955); 
    address public _marketingWalletAddress = 0xf3F4aBFdf9515a88e3C757577e2d2418F54bbF86;//DAO地址
    ...
    //HTY
    constructor() public ERC20("HTY", "HTY") {
    ...
    function _transfer(
        ...
        if(takeFee) {
            if(from == uniswapV2Pair){
                uint256 daofees = amount.mul(3).div(100);
                uint256 deadfees = amount.mul(3).div(100);
        	    amount = amount.sub(daofees).sub(deadfees);
                super._transfer(from, _marketingWalletAddress, daofees);
                super._transfer(from, _deadWalletAddress, deadfees);
            }else if(to == uniswapV2Pair){
                uint256 daofees = amount.mul(3).div(100);
                uint256 lpfees = amount.mul(3).div(100);
        	    amount = amount.sub(daofees).sub(lpfees);
                super._transfer(from, _marketingWalletAddress, daofees);
