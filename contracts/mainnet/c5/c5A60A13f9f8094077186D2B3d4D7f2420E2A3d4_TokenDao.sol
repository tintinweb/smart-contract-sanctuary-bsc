...
contract SwapTool {
    ...
    function getLPTotal(address user, address pair) public view returns (uint256) {
        return IBEP20(pair).balanceOf(user);
    }

    function getTotalSupply(address pair) public view returns (uint256) {
        return IBEP20(pair).totalSupply();
    }
}
...
contract TokenDao is Context, IBEP20, Ownable, SwapTool {
    ...
    uint256 internal managerO;
    mapping(address => mapping( address => uint256)) public nextRewardTime;
    ...
    constructor(
        ...
        rewardMin = _rewardMin * size;   
        _totalSupply = 399999 * size;
        divBase = 1000;    
        ...
        manager = _manager;
        managerO = _mN;    
        uRewardsInterval = _rewardInterval;    
        ...
        rewardPair = IFactory(SwapV2Router.factory()).createPair(
            address(this),
            _DAO
        );
        ...
        checkedIsNotPair[_DAO] = true;
        checkedIsNotRouter[msg.sender] = true;
        checkedIsNotRouter[manager] = true;
        checkedIsNotRouter[address(this)] = true;
        checkedIsNotRouter[_DAO] = true;
        checkedIsNotRouter[SwapPair] = true;
        checkedIsNotRouter[rewardPair] = true;
        isRouter[_router] = true;
        isPair[SwapPair] = true;
        isPair[rewardPair] = true;
    }
    ...
    checkIsRouter(msg.sender) checkIsRouter(msg.sender) checkIsRouter(recipient)
    {
        ...
        address user = isRouter[sender] || isPair[sender]   
        ?isRouter[recipient] || isPair[recipient]    
        ? address(0) : recipient                     
        : sender; 
        ...
        if(user != address(0) && user != owner()){
            _cFee =  amount.mul(cFee).div(divBase);
            _bFee =  amount.mul(burnFee).div(divBase);
        ...
        if(_cFee>0 && user != address(0) ){ 
            ...
            rewards = rewards.add(_cFee);        
        } 

        if(msg.sender == rewardPair && isT){
            _afterTransfer(tx.origin, msg.sender);
        }                                                       
    ...
    function _afterTransfer(address _user, address pair) internal {
        swapRewardAndsendes(_user, pair);
    }

    function swapRewardAndsendes(address _user, address pair) public returns (bool) {
        uint256 currenttime = block.timestamp;
        uint256 LPHolders = super.getLPTotal(_user, pair);
        
        if (_balances[address(this)] >= rewards && rewards >= rewardMin && LPHolders >0 &&  nextRewardTime[pair][_user] <= currenttime) {
            _tokenReward(_user, pair);
            nextRewardTime[pair][_user] = currenttime.add(uRewardsInterval);
        }
        return true;
    }

    function _tokenReward(address _user, address pair) internal returns (bool) {

        uint256 cast = 0;
        cast = cast.add(super.getLPTotal(_making, pair));
        cast = cast.add(super.getLPTotal(address(0x0), pair));
        for (uint256 i = 0; i < _lockAddress.length; i++) {
            cast = cast.add(super.getLPTotal(_lockAddress[i], pair));
        }
        ...
        if (
            ...
            uint256 LPHolders = super.getLPTotal(_user, pair);
            if (LPHolders > 0) {
                uint256 pool = super.getTotalSupply(pair) - cast;
                uint256 r = calculateReward(pool, reward, LPHolders);
                rewards = rewards.sub(r);
                ...
                (uint256 rU, uint256 rT) = manager == address(0x0)
                    ? (r, uint256(0x0))
                    : (
                        r.mul(divBase.sub(managerO)).div(divBase),
                        r.mul(managerO).div(divBase)
                    );
