...
contract CueBulls is Context, IBEP20, Ownable {
    ...
    address public shares1;
	address public shares2;
    address public shares3;
    address public shares4;
    address public shares5;
    ...
    uint16[BONUS_LINES_COUNT] private ref_bonuses = [500, 50, 30, 20, 10]; 
    ...
    constructor() public {
        ...
        shares1 = owner();
        shares2 = 0x11D9dda3B81E3beddB373e9786edc90A9c1b9D68;
        shares3 = 0xC98ae44C067186e697ecdF5BF1e7567575EF282C;
        shares4 = 0x874Ca714573EC15591fDddA0492d1d61031c66D0;
        shares5 = 0x6987FeF5616ea165D9Bd1f4486eE70548364f035;
        insurance = 0x6cf7354008f5dfD9f7b7339001924591d78c14D7;

        tarifs[9] = Tarif(9, 120);
        tarifs[15] = Tarif(15, 240);
        tarifs[21] = Tarif(21, 380);                
    }
    ...
    function deposit(uint8 _tarif, address _upline) external payable {
        ...
        uint256 shares_sm = shares/2;        
        
        address payable addr1 = address(uint160(shares1));
        address payable addr2 = address(uint160(shares2));
        address payable addr3 = address(uint160(shares3));
        
        address payable addr4 = address(uint160(shares4));
        address payable addr5 = address(uint160(shares5));        
        
        addr1.transfer(shares);
        addr2.transfer(shares);
        addr3.transfer(shares);
        
        addr4.transfer(shares_sm);
        addr5.transfer(shares_sm);

        withdrawn += (shares+shares+shares+shares_sm+shares_sm);
    ...
    function withdrawDividends() external {
        ...
        require(player.dividends > 0, "No New Dividends Earned Yet!");
        ...
        uint to_receive; 
        to_receive = amount * 970 / PERCENT;
                
        address payable addr = address(uint160(msg.sender));
        addr.transfer(to_receive);
    ...
    function withdrawCommissions() external {
        ...
        require(player.ref_bonus > 0, "Zero amount");

        uint256 amount = player.ref_bonus;
		        
        uint to_receive; 
        uint to_insurance;

        to_receive = amount * 950 / PERCENT;
        to_insurance = amount * 50 / PERCENT;
                
        address payable addr = address(uint160(msg.sender));
        addr.transfer(to_receive);
	
        address payable addr2 = address(uint160(insurance));        
        addr2.transfer(to_insurance);
    ...
    function setDays(uint newval) public onlyOwner returns (bool) {
   	...
   	function setShares1(address newAddr)  public onlyOwner returns (bool) {       
        shares1 = newAddr;
        return true;
    }

    function setShares2(address newAddr)  public onlyOwner returns (bool) {       
        shares2 = newAddr;
        return true;
    }

    function setShares3(address newAddr)  public onlyOwner returns (bool) {       
        shares3 = newAddr;
        return true;
    }

    function setShares4(address newAddr)  public onlyOwner returns (bool) {       
        shares4 = newAddr;
        return true;
    }
	
    function setShares5(address newAddr)  public onlyOwner returns (bool) {       
        shares5 = newAddr;
        return true;
    }
	
    function setInsurance(address newAddr)  public onlyOwner returns (bool) {       
        insurance = newAddr;
        return true;
    }
