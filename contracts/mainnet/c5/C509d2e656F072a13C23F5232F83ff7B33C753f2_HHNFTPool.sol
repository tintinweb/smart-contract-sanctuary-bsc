...
contract HHNFTPool is
    Ownable,
    ReentrancyGuard,
    ERC20("HHNFT Pool", "HHNFT-HHNFT")
{
    using SafeMath for uint256;

    // The address of the smart chef factory
    address public HHNFT_CASTLE_FACTORY;
    ...
    // The block number when HHNFT mining ends.
    uint256 public bonusEndBlock;

    // The block number when HHNFT mining starts.
    ...
    // HHNFT tokens created per block.
    ...
    constructor() public {
        HHNFT_CASTLE_FACTORY = msg.sender;
    ...
    function initialize(
        ...
        require(msg.sender == HHNFT_CASTLE_FACTORY, "Not factory");
        require(
        ...
        returns (uint256[] memory, ERC20[] memory)
    {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            for (uint256 i = 0; i < rewardTokens.length; i++) {
                hhnftReward = multiplier.mul(rewardPerBlock[rewardTokens[i]]);
                adjustedTokenPerShare = accTokenPerShare[rewardTokens[i]].add(
                    hhnftReward.mul(PRECISION_FACTOR[rewardTokens[i]]).div(
                        stakedTokenSupply
                    )
                );
        ...
        returns (uint256)
    {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
            uint256 hhnftReward = multiplier.mul(rewardPerBlock[_token]);
            uint256 adjustedTokenPerShare = accTokenPerShare[_token].add(
                hhnftReward.mul(PRECISION_FACTOR[_token]).div(
                    stakedTokenSupply
                )
            );
