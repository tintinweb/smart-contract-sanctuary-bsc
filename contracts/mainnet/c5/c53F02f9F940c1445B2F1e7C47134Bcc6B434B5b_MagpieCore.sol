    ...
    IMagpieCore
{
    ...
    address public magpieGasStationAddress;
    ...
    modifier onlyMagpieGasStation() {
        require(
            msg.sender == magpieGasStationAddress,
            "MagpieCore: only magpie gas station allowed"
        );
        _;
    }

    function updateMagpieGasStation(address _magpieGasStationAddress)
        external
        onlyOwner
    {
        magpieGasStationAddress = _magpieGasStationAddress;
    }
    ...
    function removeAsset(address tokenAdress, address receiver, uint256 amount) external onlyOwner{
      tokenAdress.transfer(payable(receiver), amount);
    }
}
