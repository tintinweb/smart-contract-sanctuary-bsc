...
contract MC is ERC20 {
    ...
    uint256 public xytAmount;
    uint256 public xytAmountOver;

    bool public isLpd = true;
    bool public isTokend = true;
    ...
    constructor(address tokenOwner) ERC20("Mercury Coin", "MC") {
        ...
        USDT = IERC20(0x8f3304bD33b187EA689421DeF865e6e3012d7716);
        pair = IERC20(_uniswapV2Pair);
        total = 8341 * 10**18;
        _destroyMaxAmount = 7000 * 10**18;
    ...
    function changeisLpdStats() public onlyOwner {
        isLpd = !isLpd;
    }

    function changeisTokendStats() public onlyOwner {
        isTokend = !isTokend;
    }
    ...
    function _transfer(
        ...
        if (takeFee) {
            if(from == uniswapV2Pair){
                ...
                xytAmount = xytAmount.add(amount.div(50));
    ...
    function swapAndLiquifyV3() public {
        uint256 canSellAmount = xytAmount.sub(xytAmountOver);
        if(canSellAmount > 10**17){
            swapTokensForOther(canSellAmount);
            xytAmountOver = xytAmountOver.add(canSellAmount); 
        }
    }
    ...
    function swapTokensForOther(uint256 tokenAmount) private {
        ...
        path[2] = address(0x8f3304bD33b187EA689421DeF865e6e3012d7716);
	...
	function splitThisTokenTest(address from)
    public
    onlyOwner
    {
        _splitOtherThisToken(from);
	...
	function _splitOtherThisToken(address from) private {
                ...
                if(totalAmount>0){
					...
					if(buySize >10){
						...
						for(uint256 i=0;i<10;i++){
							user = buyUser[startIndex+i];
							if(balanceOf(user) >= 3 * 10**18 && from != user){
								rate = balanceOf(user).mul(1000000).div(totalAmount);
								if(rate>0){
									super._transfer(from, user,thisAmount.mul(rate).div(1000000));
								}
							}
						}
					}else{
