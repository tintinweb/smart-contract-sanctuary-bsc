...
contract SquidGame is SquidGameBasic  {
 ...
 //uint256 checkpoint_ = users[_userAddress].activeMainLevels[level].user.checkpointU;
	...
	function getUserDepositInfo2(address _userAddress, uint8  level) public view returns(uint256 start, uint256 finish) {
	    User storage user = users[_userAddress];
...
uint256 pay_numb = paymentID(level);
	...
	function getUserDepositInfo3(address _userAddress, uint256 index) public view returns(uint256 start, uint256 finish) {
	    User storage user = users[_userAddress];
	...
	function getUserDepositInfo4(address _userAddress, uint8  level) public view returns(uint256 start, uint256 finish) {
	    User storage user = users[_userAddress];

//uint256 pay_numb = paymentID(level);

				start = user.deposits[users[_userAddress].deposits.length - 1].start;
				finish = user.deposits[users[_userAddress].deposits.length - 1].finish;

	}

	function getUserDepositInfo5(address _userAddress, uint8  level) public view returns(uint256 start, uint256 finish) {
	    User storage user = users[_userAddress];

//uint256 pay_numb = paymentID(level);
			level = user.deposits[users[_userAddress].deposits.length - 1].level;
