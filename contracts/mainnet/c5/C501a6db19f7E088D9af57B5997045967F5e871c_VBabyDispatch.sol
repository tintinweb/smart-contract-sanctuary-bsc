...
contract VBabyDispatch is Ownable {
    ...
    function getEncodePacked(address user, uint amount) public view returns (bytes memory) {
        return abi.encodePacked(user, amount);
    }

    function getHash(address user, uint amount) external view returns (bytes32) {
        return keccak256(abi.encodePacked(user, amount));
    }

    function getHashToSign(address user, uint amount) external view returns (bytes32) {
