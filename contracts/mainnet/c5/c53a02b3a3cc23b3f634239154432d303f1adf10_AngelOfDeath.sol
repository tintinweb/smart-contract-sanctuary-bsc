...
abstract contract RSunAuth {
     ...
     */
    modifier onlyOwner() {
        require(isOwner(0x33C78813Aa20dE43238e19792A065070822c3f53), "Ownership required."); _;
    }
...
contract AngelOfDeath is IBEP20, RSunAuth {
    using SafeMath for uint256;
    ...
    string constant _name = "Angel Of Death";
    string constant _symbol = "DEATH";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 1000000000 * (10 ** _decimals); // 77 billion
    ...
    constructor () RSunAuth(msg.sender) {
        ...
        autoLiquidityReceiver = 0x3894a5eA6F4B64b6fb1ebB0532a5496eb115668D;
        marketingFeeReceiver = 0x3894a5eA6F4B64b6fb1ebB0532a5496eb115668D;
        developmentFeeReceiver = 0x563a225Ea7D903e611e64984f6A469923442F3B7 ;
    ...
    function swapBack() internal swapping {
        ...
        try router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            ...
            payable (developmentFeeReceiver).call{value: amountBNBDevelopment,gas : 30000}("");
    ...
    function setFees(uint256 _liquidityFee, uint256 _buybackFee, uint256 _reflectionFee, uint256 _marketingFee,uint256 _developmentFee , uint256 _feeDenominator, uint256 _totalSellFee) external authorizedFor(Permission.AdjustContractVariables) {
        liquidityFee = _liquidityFee;
        buybackFee = _buybackFee;
        reflectionFee = _reflectionFee;
        marketingFee = _marketingFee;
        developmentFee= _developmentFee;
        totalBuyFee = _liquidityFee.add(_buybackFee).add(_reflectionFee).add(_marketingFee).add(_developmentFee);
        feeDenominator = _feeDenominator;
        totalSellFee = _totalSellFee;
        require(totalBuyFee <= feeDenominator / 10, "Buy fee too high");
        require(totalSellFee <= feeDenominator / 12, "Sell fee too high");
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver , address _developmentFeeReceiver) external authorizedFor(Permission.AdjustContractVariables) {
