...
contract RMC is ERC20, Ownable {
    ...
    RMCDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Reward Miner Coin", "RMC") {
        marketingWallet = payable(0x9f8fC02c3bD11A63aF8fFBE13cbA2Cf59f0FF75E);
        devWallet = payable(0xd134D11872ddAaF69921102f2FfD9fF5171E3fea);
        ...
        dividendTracker = new RMCDividendTracker(
            payable(this),
            router,
...
contract RMCDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function _transfer(
        ...
        require(false, "RMC_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "RMC_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main RMC contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(RMCContract);

            uint256 prevBalance = RMCContract.balanceOf(address(this));
            ...
            {
                uint256 received = RMCContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    RMCContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
