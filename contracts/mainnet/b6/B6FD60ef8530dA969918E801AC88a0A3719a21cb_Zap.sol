...
contract Zap is Ownable, ReentrancyGuard {
    ...
    function zapIntoFarmWithToken(
        ...
        //Deposit tokens into farms.
        amountOut = IERC20(_targetToken).balanceOf(address(this));
    ...
    */
    function _zapIntoToken(
        ...
        //Handle token taxes.
        if (_inputToken == mainToken || _inputToken == shareToken) {
        ...
        if (tokenType[_targetToken] == TokenType.LP) {
            ...
            if (_inputToken == token0 || _inputToken == token1) {
                ...
                address altToken = _inputToken == token0 ? token1 : token0;
                uint256 sellAmount = _amount.div(2);
                _increaseRouterAllowance(altToken, sellAmount);

                uint256 altTokenAmount = _swapTokensForTokens(
                ...
                _increaseRouterAllowance(_inputToken, _amount.sub(sellAmount));
                _increaseRouterAllowance(altToken, altTokenAmount);

                (,, amountOut) = ROUTER.addLiquidity(
                    _inputToken,
                    altToken,
                    _amount.sub(sellAmount),
                    altTokenAmount,
                    0,
                    0,
                    _recipient,
                ...
                emit AddLiquidity(_inputToken, altToken, _amount.sub(sellAmount), altTokenAmount, amountOut);
            //Otherwise swap the token for EVMOS and then add the liquidity from there.
                ...
                amountOut = _swapETHToLP(_targetToken, ethAmount, _recipient);
            }
