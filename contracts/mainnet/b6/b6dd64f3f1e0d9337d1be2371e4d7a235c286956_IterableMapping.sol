...
contract AFINADividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("AFINA_Dividend_Tracker", "AFINA_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**8); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "AFINA_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "AFINA_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main AFINA contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "AFINA_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "AFINA_Dividend_Tracker: Cannot update claimWait to same value");
...
contract AFINA is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    AFINADividendTracker public dividendTracker;

    uint256 public swapTokensAtAmount = 1000 * (10**8);
    ...
    function setFee(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _marketingFee, uint256 _DAOFee) public onlyOwner {
        ...
        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(DAOFee); // total fee transfer and buy
        require(totalFees <= 11, "Fee limit reached!!");
    }
   ...
   function setMarketingWallet(address payable _newMarketingWallet) public onlyOwner {
    ...
    function setDAOWallet(address payable _newDAOWallet) public onlyOwner {
        DAOWallet = _newDAOWallet;
    }
    
    

    constructor() ERC20("AFINA BSC", "AFINA") {
        BNBRewardsFee = 1;
        liquidityFee = 3;
        marketingFee = 3;
        DAOFee = 3;

        marketingWallet = payable(0x9C59D8b82485a044cDb7F6e4985bB449c47b2B79);
        DAOWallet = payable(0xCcA5D05b828FbFF32E79931c714E504C39aC75AB);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(DAOFee); // total fee transfer and buy

     
    	dividendTracker = new AFINADividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "AFINA: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "AFINA: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "AFINA: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "AFINA: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "AFINA: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "AFINA: Cannot update gasForProcessing to same value");
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 DAOAmount = deltaBalance.sub(bnbToAddLiquidityWith).div(totalFees.sub(liquidityFee)).mul(DAOFee);
        DAOWallet.transfer(DAOAmount);
