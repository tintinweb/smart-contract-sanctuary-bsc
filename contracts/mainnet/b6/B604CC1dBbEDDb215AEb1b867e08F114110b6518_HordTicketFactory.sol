...
contract HordTicketFactory is HordUpgradable, ERC1155PausableUpgradeable {
    ...
    onlyMaintainer
    {
        require(tokenId <= lastMintedTokenId, "AddTokenSupply: Firstly MINT token, then expand supply.");
    ...
    onlyMaintainer
    {
        ...
        for(uint256 i = startIndex; i < endIndex; i++) {
            require(suppliesToAdd[i] <= maxTickets[i], "MintNewHPoolNFT: Supply overflow.");
            require(tokenIds[i] == lastMintedTokenId.add(1), "MintNewHPoolNFT: Token ID is wrong.");
            require(maxTickets[i] < maxFungibleTicketsPerPool, "Exceed maxFungibleTicketsPerPool");

            tokenIdToMaxFungibleTicketsPerPool[tokenIds[i]] = maxTickets[i];
            tokenIdToMintedSupply[tokenIds[i]] = suppliesToAdd[i];

            _mint(address(hordTicketManager), tokenIds[i], suppliesToAdd[i], "0x0");
            emit MintedNewNFT(tokenIds[i], championIds[i], suppliesToAdd[i]);

            hordTicketManager.addNewTokenIdForChampion(tokenIds[i], championIds[i]);
            lastMintedTokenId = tokenIds[i];
        }
    ...
    onlyMaintainer
    {
        ...
        require(_maximalFungibleTicketsPerPoolForTokenId <= maxFungibleTicketsPerPool);
    ...
    /**
        * @notice  Maximal number of fungible tickets per Pool
    */
    function getMaxFungibleTicketsPerPool(
        uint256 tokenId
    )
    external
    view
