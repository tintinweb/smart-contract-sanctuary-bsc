...
contract BuffedElonDogeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("BuffedElonDoge_Dividend_Tracker", "BuffedElonDoge_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "BuffedElonDoge_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "BuffedElonDoge_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BELDOGE Inu contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BuffedElonDoge_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "BuffedElonDoge_Dividend_Tracker: Cannot update claimWait to same value");
...
contract BuffedElonDoge is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    BuffedElonDogeDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Buffed Elon Doge", "BELDOGE") {
        BNBRewardsFee = 1;
        liquidityFee = 4;
        extraFeeOnSell = 5; // extra fee on sell
        marketingFee = 3;
        marketingWallet = payable(0x20D3e4253DDF07208D1F6d9EDCD5B29553ebC8fA);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new BuffedElonDogeDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BuffedElonDoge: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BuffedElonDoge: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BuffedElonDoge: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BuffedElonDoge: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BuffedElonDoge: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BuffedElonDoge: Cannot update gasForProcessing to same value");
