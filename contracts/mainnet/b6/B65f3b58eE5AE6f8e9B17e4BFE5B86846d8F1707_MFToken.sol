...
contract MFToken is ERC20, Ownable {
    ...
    address public liquidityWalletAddress = 0xa9C911e31819f54B52fF0631BdC87284997Fea9a;
    address public rdWalletAddress = 0x6B4718874d9f4EE429E562e8baE4A1C843518C77;
    address public woodWalletAddress = 0x0825b1740Ea16a38C805a8F192Ef161317Ef45CA;
    address public lpRewardToken = 0xC9882dEF23bc42D53895b8361D0b1EDC7570Bc6A;

    uint256 private woodSwapAmount;
    ...
    bool private priceSwitch = true;
    bool public swapWoodEnabled = false;
    ...
    constructor(
        ...
        address initAddress1,
        address initAddress2,
        address initAddress3
    ) ERC20(name_, symbol_) {
        ...
        excludeFromFees(initAddress1, true);
        excludeFromFees(initAddress2, true);
        excludeFromFees(initAddress3, true);
        _mint(initAddress1, (totalSupply_ * 30) / 100);
        _mint(initAddress2, (totalSupply_ * 10) / 100);
        _mint(initAddress3, (totalSupply_ * 10) / 100);
        _mint(DEAD, (totalSupply_ * 50) / 100);
    }
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "MF: Account is already the value of 'excluded'"
        );
        excludedFromFees[account] = excluded;
    ...
    function swapToUSDT(uint256 tokenAmount) internal swapping {
    ...
    function swapToWood(uint256 tokenAmount) internal swapping {
        address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = usdtToken;
        path[2] = woodToken;
    ...
    function takeAutoWoodFee(address from, uint256 amount)
        internal
        returns (uint256)
    {
        ...
        if (feeAmount > 0) {
            if (swapWoodEnabled) {
                ...
                woodSwapAmount += feeAmount;
            } else {
                super._transfer(from, woodWalletAddress, feeAmount);
            }
        }
        return feeAmount;
    }

    function takeRDFee(address from, uint256 amount)
        internal
        returns (uint256)
    {
        uint256 feeAmount = amount.mul(sellRDFee).div(baseFee);
        if (feeAmount > 0) {
            if (swapEnabled) {
                super._transfer(from, address(this), feeAmount);
                swapToUSDT(feeAmount);
            } else {
                super._transfer(from, rdWalletAddress, feeAmount);
            }
        }
        return feeAmount;
    }

    function takeLPFee(address from, uint256 amount)
        internal
        returns (uint256)
    {
        uint256 feeAmount = amount.mul(sellLiquidityFee).div(baseFee);
        if (feeAmount > 0) {
            if (swapEnabled) {
                super._transfer(from, address(this), feeAmount);
    ...
    function takeFee(
        ...
        uint256 totalFee;
        if (from == tokenPair) {
            uint256 WFee = takeAutoWoodFee(from, amount);
            totalFee += WFee;
        } else if (to == tokenPair) {
            uint256 DFee = _burnToDead(from, sellDeadFee, amount);
            totalFee += DFee;
            uint256 RDFee = takeRDFee(from, amount);
            totalFee += RDFee;
            uint256 LPFee = takeLPFee(from, amount);
            totalFee += LPFee;
        } else {
        ...
        if (priceSwitch) {
            ...
            if (to == tokenPair) {
                ...
                uint256 allSellFee = downRate.add(totalSellFee);
                if (allSellFee > sellFeeLimit) {
                    downRate = sellFeeLimit.sub(totalSellFee);
                }
    ...
    function canSwap(address sender) internal view returns (bool) {
        return sender != tokenPair && swapWoodEnabled && woodSwapAmount > 0;
    }
    ...
    function _transfer(
        ...
        if (from == tokenPair) {
            if (block.timestamp < startTradingTime) {
                require(tradeWhitelist[to], "MF: only whitelist can buy");
            }
        }
        if (inSwap) {
        ...
        if (canSwap(from)) {
            uint256 startSwapAmount = woodSwapAmount;
            woodSwapAmount = 0;
            swapToWood(startSwapAmount);
        }
        uint256 allFee;
    ...
    function getMFPrice() public view returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = usdtToken;
    ...
    function setSwapWoodEnabled(bool swapEnabled_) public onlyOwner {
        swapWoodEnabled = swapEnabled_;
    }
    ...
    function setUintConfigs(uint256[3] memory configs) public onlyOwner {
        downCritical = configs[0];
        slipRate = configs[1];
        sellFeeLimit = configs[2];
    }

    function setTokens(address[6] memory configs) public onlyOwner {
        liquidityWalletAddress = configs[0];
        rdWalletAddress = configs[1];
        woodWalletAddress = configs[2];
        woodToken = configs[3];
        lpRewardToken = configs[4];
    }

    function setAllFees(uint256[5] memory configs) public onlyOwner {
        buyFee = configs[0];
        sellDeadFee = configs[1];
        sellLiquidityFee = configs[2];
        sellRDFee = configs[3];
        transferFee = configs[4];
        totalSellFee = sellDeadFee.add(sellLiquidityFee).add(sellRDFee);
    }
