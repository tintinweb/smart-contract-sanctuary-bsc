...
contract XQuantumpresale {
    ...
    uint256 public perDollarPrice_2;  //in decimals
    ...
    constructor(uint256 _price,address _presaleToken1 ,address _presaleToken2 , uint _perUsdtoBnb , address _usdt , address _busd){
        owner = msg.sender;
        perDollarPrice = _price;
        perDollarPrice_2 = _price;
        token = IERC20(_presaleToken1);
        token1 = IERC20(_presaleToken2);
    ...
    function ikeBalance(address _user) public view returns(uint){
        ...
        function ikeBalance2(address _user) public view returns(uint){
        return token1.balanceOf(_user);
    }
    ...
    function remainingToken1() public view returns(uint){
    ...
    function buyfromToken(uint _pid,address ref,uint token_option) public {

        require(!paused,"Presale is Paused!!");
        require(token_option>=0&&token_option<=1);
        if(token_option==0)
        {
            uint check = 1;   
        ...
        else if(token_option==1)
        {
            ...
            if(_pid == 1){
                ...
                uint temp = _amount;
            //  uint multiplier = temp * 10 **(token.decimals()) / 50000 ;
            uint multiplier = temp * 10 **(token1.decimals()) / (perDollarPrice_2 *10**12);

                token1.transfer(msg.sender,multiplier);
            ...
            // busd.transferFrom(msg.sender,address(this),_amount);
                ...
                uint temp = _amount;
                uint multiplier = temp * 10 **(token1.decimals()) / (perDollarPrice_2 *10**12); //(token.decimals() - usdt.decimals());
                token1.transfer(msg.sender,multiplier);
            }
            else {
                revert("Wrong Selection!!");
    ...
    function buyFromNative(address ref, uint token_option) public payable {
        ...
        {
        uint multiplier = (perDollarPrice_2 *10**12)  * equaltousd;
