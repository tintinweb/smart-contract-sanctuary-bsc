...
contract GPOBNB is BaseGPO {
    ...
    // Manually set the pinkAntibot address
    function setPinkAntiBot(address _pinkAntiBot) public onlyOwner {
        pinkAntiBot = IPinkAntiBot(_pinkAntiBot);
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual override {        
        ...
        // is not whitelisted OR from is pool and to isn't whitelisted
        if (!freeTrade && (!whitelistedWallets[from] || (from == authorizedPool && !whitelistedWallets[to]))) { 
    ...
    function performSwapAndDistribute() public {
        uint256 currBalance = balanceOf(address(this));
        _approve(address(this), address(swapRouter), currBalance);
...
____________________________
Description:
Base GoldPesa Option Contract (GPO) - GPO represents the option to purchase GPX at spot gold price + 1 %.
__________________________________
 */
abstract contract BaseGPO is ERC20Permit, Pausable, Ownable, AccessControl, GPOStructs {
    ...
    // Uniswap or PancakeSwap liquidity pool address
    ...
    // When freeTrade is true the token bypasses the hard cap on wallet along with the GPO fees and can be traded freely on any exchange
    ...
    bytes32 public constant AUTHORIZED_LOCK =
        keccak256("CAN_LOCK");
    // Defines an Access Control role called "AUTHORIZED_OPERATOR" which is granted to addresses that are allowed to mint GPOm tokens
    ...
    // Mapping which holds details of the wallet addresses which bypass transfer restrictions and fees
    mapping(address => bool) public whitelistedWallets;
    // Mapping which holds details of the amount of GPO tokens locked in the lockedWallets mapping 
    ...
    function switchFreeTrade() external onlyOwner {
     ...
     // Enables GoldPesa to turn swapping on and off
    ...
    // Enables GoldPesa to set the "feeOnSwap" distribution details. These fees are only for internal GoldPesa wallets
    ...
    // Additional checks before executing the _transfer function
    ...
    // Mint GPO tokens to a specific address
    function mintTo(address account, uint256 amount) external authorizedOperator {
        require(mintOnDemand);
        _mint(account, amount);
    }

     // Internal GPO mint function used by contract derived from GPO Base
    ...
    // Burns `amount` tokens from sender, granted that they are allowed to 
    function burn(uint256 amount) public virtual {
        _burn(_msgSender(), amount);
    }

    // Destroys `amount` tokens from `account`, deducting from the caller's allowance annd assuming the account has the right to burn
    function burnFrom(address account, uint256 amount) public virtual {
        _spendAllowance(account, _msgSender(), amount);
        _burn(account, amount);
    }

    // Manually set the authorized liquidity Pool address
