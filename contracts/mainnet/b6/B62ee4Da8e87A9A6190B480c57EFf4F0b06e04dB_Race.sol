...
contract Race is Ownable {
    ...
    uint256 public commiteePercent = 40e18;
     ...
     */
    modifier isLock() {
        require(lockStatus == false, "Doggyville: Contract Locked");
        _;
    }

    constructor (address _signer, address _commitee)  {
        signer = _signer;
        commitee = _commitee;
    }

    function deposit(IERC20 _token,uint256 _amount,uint256 _time,bytes calldata sig) external isLock {
        address user = _msgSender();
        bytes32 messageHash = message(user,_amount,_time);
        validate(sig,messageHash);

        uint256 _total;

        if (commiteePercent > 0) {
            uint256 commission = _amount*commiteePercent/100e18;
            _token.safeTransferFrom(user,commitee,commission);
        ...
        _total = _amount;
        totalAmount += _total;
        emit Deposit(user,_amount,address(_token),block.timestamp);
    }

    function claimToken(IERC20 _token,address _user,uint amount,bytes calldata sig,uint time) external isLock {
        bytes32 messageHash = message(_user,amount,time);
        validate(sig,messageHash);
        _token.safeTransfer(_user,amount);
        msgHash[messageHash] = true;
        emit Claim(_user,address(_token),amount,block.timestamp);
    }

    function validate(bytes calldata signature,bytes32  messageHash) internal view {
    ...
    */
    ...
    function updateCommitee(address _commitee) external onlyOwner {
        commitee = _commitee;
    }

    function updateCommiteePercent(uint256 _commiteePercent) external onlyOwner {
        commiteePercent = _commiteePercent;
    }

    function failsafe(IERC20 _token,address user,uint amount)public onlyOwner{
