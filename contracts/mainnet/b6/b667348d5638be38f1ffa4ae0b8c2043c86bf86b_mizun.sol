...
contract mizun is IERC20, Ownable {
    ...
    function getRemainLockAmount() public view returns (uint256) {
        uint256 overWeek = block.timestamp.sub(startTime).div(86400 * 7);
		uint256 releaseRate = overWeek * 7;
		if(releaseRate >= 1000){
			return 0;
		}else{
			return _lockAmount.div(1000).mul(1000 - releaseRate);
		}
    }
    ...
    function _transfer(
		...
		if(from == _lockAddress){
			require(balanceOf(from).sub(amount) >= getRemainLockAmount(),"lock");
		}
