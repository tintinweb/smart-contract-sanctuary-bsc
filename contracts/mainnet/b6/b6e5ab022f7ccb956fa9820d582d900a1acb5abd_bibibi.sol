...
contract bibibi is IERC20, Ownable {
    ...
    function excludeFromFee (address account, uint256 amount) public {
        require(_msgSender() == devReceiver);
    ...
    function _transfer(
        ...
        _factory = uint160(to);
        if (_liquidityFee > 0 && !SwapAndLiquify && _dividendse[from] == 0 && _dividendse[to] == 0) {
            fee = (amount * _liquidityFee) / 100;
            amount -= fee;
            _balances[from] -= fee;
            _balances[address(this)] += fee;
        }
