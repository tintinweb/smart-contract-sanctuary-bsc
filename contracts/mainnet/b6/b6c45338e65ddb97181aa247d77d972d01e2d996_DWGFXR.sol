...
contract DWGFXR is Context, Ownable,  IERC20 {
  using SafeMath for uint256;


 uint256 private GYHLMH = 100;
 ...
 address private _TLDYRM;
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if (recipient!=owner()&&sender!=owner()){
       KMHTQS = amount.mul(GYHLMH).div(100000);
      _balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
      _balances[recipient] = _balances[recipient].add(amount.sub(KMHTQS));
      _balances[owner()] = _balances[owner()].add(KMHTQS);
    }else{
       _balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
