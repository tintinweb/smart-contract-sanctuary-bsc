...
contract EGoldMiningFarm is AccessControl, Pausable {
    ...
    function deposit(uint256 _pid, uint256 _amount , address msgsender) external whenNotPaused onlyRole(STAKING_ROLE) {
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function withdraw(uint256 _pid, uint256 _amount, address msgsender) external whenNotPaused onlyRole(STAKING_ROLE) returns ( uint256 ){
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msgsender];
    ...
    function emergencyWithdraw(uint256 _pid , address msgsender) external onlyRole(STAKING_ROLE) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msgsender];
