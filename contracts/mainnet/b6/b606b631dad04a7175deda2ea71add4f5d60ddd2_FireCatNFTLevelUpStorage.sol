...
import {IFireCatGate} from "./interfaces/IFireCatGate.sol";
...
/**
 * @title FireCat's FireCatNFTLevelUpStorage contract
 * @notice main: isStakeQualified, setFireCatGate, setLevelUpRequrie
 * @author FireCat Finance
 */
contract FireCatNFTLevelUpStorage is FireCatAccessControl {
    address public fireCatGate;
    address public fireCatNFT;
    uint256[] public levelArray;
    uint256[] public stakeNumArray;
    uint256[] public payNumArray;
    address public levelUpPayToken;
    mapping(uint256 => uint256) public levelUpRequireStake;
    mapping(uint256 => uint256) public levelUpRequirePay;

    function initialize(address fireCatGate_, address fireCatNFT_) initializer public {
    ...
    * @dev access total staked from fireCatGate
    * @param tokenId_ address.
    * @return bool
    */
    function isStakeQualified(uint256 tokenId_) external view returns (bool) {
        uint256 tokenLevel = IFireCatNFT(fireCatNFT).tokenLevelOf(tokenId_);
        uint256 totalVaultStaked = IFireCatGate(fireCatGate).vaultStakedOf(tokenId_);
        if (totalVaultStaked >= levelUpRequireStake[tokenLevel]) {
            return true;
        }
        return false;
    }
    ...
    /**
    * @notice set fireCat contract address.
    * @param fireCatGate_ address
    */
    function setFireCatGate(address fireCatGate_) external onlyRole(DATA_ADMIN) {
        fireCatGate = fireCatGate_;
    }

    /**
    * @notice set the level up require config
    * @param levelArray_ uint256[]
    * @param stakeNumArray_ uint256[]
    * @param payNumArray_ uint256[]
    * @param levelUpPayToken_ address
    */
    function setLevelUpRequrie(
        uint256[] calldata levelArray_, 
        uint256[] calldata stakeNumArray_,
        uint256[] calldata payNumArray_,
        address levelUpPayToken_
    ) external onlyRole(DATA_ADMIN) {
        levelArray = levelArray_;
        stakeNumArray = stakeNumArray_;
        payNumArray = payNumArray_;
        levelUpPayToken = levelUpPayToken_;

        for (uint256 i = 0; i < stakeNumArray_.length; ++i) {
            levelUpRequireStake[levelArray_[i]] = stakeNumArray_[i];
            levelUpRequirePay[levelArray_[i]] = payNumArray_[i];
        }
    }
