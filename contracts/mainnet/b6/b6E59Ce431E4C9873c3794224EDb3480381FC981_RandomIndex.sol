...
contract RandomIndex is VRFConsumerBaseV2, Ownable {
...
//    bytes32 keyHash = 0xd4bb89654db74673a187bd804519e65e3f71a52bc55f11da7601a13dcf505314;
    ...
    uint256 public roundDuration = 6 hours;

    // ==================== Modifier ================================ //
    modifier onlyOperator {
        require(msg.sender == operator, 'RandomIndex: !operator');
        _;
    }
    ...
    function changeRoundId(uint256 _roundId) external onlyOwner {
        require(currentRoundId > _roundId, 'RandomIndex: !_roundId');
        currentRoundId = _roundId;
    }

    function changeRoundDuration(uint256 _value) external onlyOwner {
        require(_value > 0, 'RandomIndex: !_value');
        roundDuration = _value;
    }
    ...
    function requestRandomWords() external onlyOperator {
        ...
        require(!lockFullFill, "RandomIndex: Waiting for full fill!");
        require(block.timestamp >= lastUpdateResult + roundDuration, 'RandomIndex: < roundDuration');
    ...
    // ================= View functions ==================== //
    function _getRandomNumber(uint _n1, uint _n2, uint _n3, uint _roundId, uint _index) internal view returns (uint _number) {
    ...
    function getRandomIndex(uint _roundId, uint _maxIndex, uint _quantity) external view returns (uint[] memory _result, uint _blockNumber) {
        ...
        for (uint i = 0; i < _quantity; i++) {
            uint _index = _getRandomNumber(numbers[_roundId][0], numbers[_roundId][1], numbers[_roundId][2], _roundId, i) % _maxIndex;
