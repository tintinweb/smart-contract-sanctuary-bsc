...
contract PigPunkNFTs is Auth, ERC721, ERC721Enumerable {
    ...
    // mint shares
    uint private _mintSharesDistributor = 950;
    uint private _mintSharesAllotment = 50;
    uint256 private _mintDenominator = 1000;

    // bnb price per mint
    uint256 pLevelOne = 100000000000000000;
    uint256 pLevelTwo = 250000000000000000;
    uint256 pLevelThree = 350000000000000000;
    uint256 pLevelFour = 350000000000000000;
    uint256 pLevelFive = 350000000000000000;

    // token price per mint
    uint256 tLevelOne = 45000000000000000000;
    uint256 tLevelTwo = 110000000000000000000;
    uint256 tLevelThree = 160000000000000000000;
    uint256 tLevelFour = 160000000000000000000;
    uint256 tLevelFive = 160000000000000000000;

    //set specifics of token
    uint256 tokenTax = 6;
    uint256 taxDenominator = 1000;
    address tokenAddress = 0x5f75dbb28784414f94B0dFD732915086D6Cb5616;
    ...
    function setInternalAddresses(address _mark) external authorized {
        mark = _mark;
    }
    ...
    function contributeTokens(uint amount) external {
        require(token.balanceOf(_msgSender()) >= amount);
        uint tokenId;
        tokenId = _tokenIds.current();
        uint256 taxinverse = taxDenominator - tokenTax;
        uint256 tAmount = amount * taxinverse / taxDenominator;
        IBEP20(token).transferFrom(_msgSender(), address(this), tAmount);
        _balanceReflections += tAmount;
        _totalDividend += (tAmount / tokenId);
    }
    ...
    function setCanClaim(bool _enabled) external authorized {
        canClaim = _enabled;
    }
