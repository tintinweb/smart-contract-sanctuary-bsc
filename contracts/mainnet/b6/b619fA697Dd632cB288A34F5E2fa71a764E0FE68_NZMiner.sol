...
contract NZMiner is Ownable {
    ...
    function claim() external {
        ...
        userRewardInfo[msg.sender].accHasReward += userRewardInfo[msg.sender].accPendingReward;
        userRewardInfo[msg.sender].accPendingReward = 0;
        userRewardInfo[msg.sender].peerHasReward = userRewardInfo[msg.sender].peerPendingReward;
        userRewardInfo[msg.sender].peerPendingReward = 0;

        _claimInviteFee(msg.sender, totalRewards.div(10));
    ...
    function stake(uint256 amount, bool all) external {
        ...
        _inviteFee(msg.sender, amount.mul(a).div(10000));

        _peerFee(msg.sender, amount.mul(a).div(10000));
    ...
    function _inviteFee(address user, uint256 amount) private {
        address parent = user;
        for (uint256 i = 1; i <= 10; i++) {
            ...
            if (u.validInvAmount >= i) {
                userRewardInfo[parent].accTotalReward += amount.mul(rate).div(100);
            }
        }
    }
    ...
    function _claimInviteFee(address user, uint256 amount) private {
        ...
        for (uint256 i = 1; i <= 10; i++) {
            ...
            UserReward memory ur = userRewardInfo[parent];
            if (ur.accTotalReward.add(amount) >= ur.accHasReward) {
                amount = ur.accTotalReward.sub(ur.accHasReward);
            }
            
            userRewardInfo[parent].accPendingReward += amount;
        }
    }

    function _peerFee(address user, uint256 amount) private {
        address parent = user;
        for (uint256 i = 1; i <= 20; i++) {
            ...
            if (u.isPeer) {
                userRewardInfo[parent].peerTotalReward += amount.div(10);
                break;
            }
        }
    }
    ...
    function _claimPeerFee(address user, uint256 amount) private {
        ...
        for (uint256 i = 1; i <= 20; i++) {
            ...
            UserReward memory ur = userRewardInfo[parent];
            if (ur.peerTotalReward.add(amount) >= ur.peerHasReward) {
                amount = ur.peerTotalReward.sub(ur.peerHasReward);
            }
