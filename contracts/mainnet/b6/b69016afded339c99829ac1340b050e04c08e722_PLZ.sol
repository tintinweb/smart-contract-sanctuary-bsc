...
/*! PLZ.token.sol | SPDX-License-Identifier: MIT License */
...
contract PLZ is TRC20 {
    address public owner;
    bool public stopmint;

    modifier onlyOnwer() {
        require(msg.sender == owner, "PLZ: ACCESS_DENIED");
        _;
    }
    ...
    function mint(address to, uint256 value) external onlyOnwer {
        require(!stopmint, "PLZ: MINT_ALREADY_STOPED");

        _mint(to, value);
    }

    function burn(uint256 value) external onlyOnwer {
        require(balanceOf[msg.sender] >= value, "PLZ: INSUFFICEINT_FUNDS");
