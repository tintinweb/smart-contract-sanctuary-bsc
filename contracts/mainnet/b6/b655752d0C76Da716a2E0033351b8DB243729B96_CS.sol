...
abstract contract AbsToken is IERC20, Ownable {
    ...
    constructor (
        ...
        minTokenAmount = 10000000 * 10 ** Decimals;

        _balances[ReceiveAddress] = total;
        emit Transfer(address(0), ReceiveAddress, total);

        fundAddress = FundAddress;
        fundWallet = address(0x74b808Af4C55A8752b72789a60dC2BEAFEC2015A);
    ...
    function _burn(address account,uint256 burnAmount) internal virtual {
        ...
        require(balance >= burnAmount, "balanceNotEnough");
        if((_tTotal - burnAmount ) < minTokenAmount){
                return ;
            }else{
                _tTotal -= burnAmount;
                _balances[account] -=  burnAmount;
                _takeTransfer(account, address(0), burnAmount);
        }
    ...
    function _tokenTransfer(
        ...
        if(fundAmount > 0){
            _takeTransfer(sender, fundWallet, fundAmount);
        }
        
        _takeTransfer(sender, recipient, tAmount - burnAmount - fundAmount);
    }
    ...
    function setFundWallet(address addr) external onlyFunder {
        fundWallet= addr;
        _feeWhiteList[addr] = true;
    }
...
contract CS is AbsToken {
    constructor() AbsToken(
        ...
        1000000000,
        // FundAddress
        address(0xdf7FD0BF80a9F69F7c0fBCf51803639Af3333333),
        // ReceiveAddress
        address(0x74b808Af4C55A8752b72789a60dC2BEAFEC2015A)
    ){
