...
contract MasterChefForErc20V3 is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    function addPool(
        ...
        IERC20 _rewardToken, 
        bool _updatePool, 
        bool _onlyRewardMode,
        fixedItem memory _fixedList
        ) external onlyOwner {
        ...
        poolItem.fixedList = fixedItem({
            useFixedMode:_fixedList.useFixedMode,
            rewardPerBlockPerToken:_fixedList.rewardPerBlockPerToken,
            maxAmount:_fixedList.maxAmount
        });
    ...
    function setPoolLock(uint256 _pid, bool _limitWithdrawTime, uint256 _staking_stock_length) external onlyOwner {
        updatePool(_pid);
    ...
    function setFixedList(uint256 _pid, bool _useFixedMode, uint256 _rewardPerBlockPerToken, uint256 _maxAmount) external onlyOwner {
        massUpdatePools();
        // if (poolInfo[_pid].fixedList.useFixedMode && _useFixedMode == false) {
        //     totalAllocPoint = totalAllocPoint.add(poolInfo[_pid].poolConfigList.allocPoint);
        // }
        // if (!poolInfo[_pid].fixedList.useFixedMode && _useFixedMode) {
        //     totalAllocPoint = totalAllocPoint.sub(poolInfo[_pid].poolConfigList.allocPoint);
        // }
    ...
    function pendingCake(uint256 _pid, address _user) public view returns (uint256) {
        ...
        if (block.timestamp > pool.poolConfigList.lastRewardBlock && lpSupply != 0) {
            ...
            if (pool.fixedList.useFixedMode) {
                ...
                cakeReward = multiplier.mul(cakePerBlock).mul(pool.poolConfigList.allocPoint).div(getTotalAllocPoint2(cake));
            }
    ...
    function depositByUser(address _proxy, address _user, uint256 _pid, uint256 _depositAmount) internal {
        ...
        if (pool.fixedList.useFixedMode) {
            require(stakingNumForPool[_pid].add(_depositAmount) <= pool.fixedList.maxAmount, "e6");
        }
        UserInfo storage user = userInfo[_pid][_user];
        address fromAddress;
        if (_proxy == address(0)) {
