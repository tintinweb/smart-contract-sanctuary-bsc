...
contract Medusa is ReentrancyGuard {
    ...
    struct StakingParams {
        ...
        uint256 maxReward; // max reward possible daily with this plan
    }

    // Staking parameters for each plan
    StakingParams[5] public stakingParams;
    ...
    constructor(IERC20 _token) {
        ...
        stakingParams[0].minStake = 30 * (10 ** 18);
        stakingParams[0].maxWithdrawal = 50000 * (10 ** 18);
        stakingParams[0].rewardPercentage = 15;
        stakingParams[0].referralCount = 0;
        stakingParams[0].withdrawFee = 8;
        stakingParams[0].maxReward = 3 * (10 ** 18);

        stakingParams[1].minStake = 100 * (10 ** 18); 
        stakingParams[1].maxWithdrawal = 80000 * (10 ** 18);
        stakingParams[1].rewardPercentage = 20;
        stakingParams[1].referralCount = 2;
        stakingParams[1].withdrawFee = 6;
        stakingParams[1].maxReward = 10 * (10 ** 18);

        stakingParams[2].minStake = 1000 * (10 ** 18); 
        stakingParams[2].maxWithdrawal = 100000 * (10 ** 18);
        stakingParams[2].rewardPercentage = 35;
        stakingParams[2].referralCount = 5;
        stakingParams[2].withdrawFee = 4;
        stakingParams[2].maxReward = 100 * (10 ** 18);

        stakingParams[3].minStake = 10000 * (10 ** 18);
        stakingParams[3].maxWithdrawal = 300000 * (10 ** 18);
        stakingParams[3].rewardPercentage = 40;
        stakingParams[3].referralCount = 10;
        stakingParams[3].withdrawFee = 2;
        stakingParams[3].maxReward = 1000 * (10 ** 18);

        stakingParams[4].minStake = 20000 * (10 ** 18);
        stakingParams[4].maxWithdrawal = 800000 * (10 ** 18);
        stakingParams[4].rewardPercentage = 55;
        stakingParams[4].referralCount = 20;
        stakingParams[4].withdrawFee = 1;
        stakingParams[4].maxReward = 2000 * (10 ** 18);
    ...
    function stake(uint256 amount, address referralAddress) public noReentry {
        ...
        if (!stakedAmounts[msg.sender].active) {
            require(amount >= stakingParams[0].minStake , "First stake must be bigger then minimum");
        ...
        stakedAmounts[msg.sender].amount += amount;
        ...
        if (referralAddress != address(0) && referralAddress != msg.sender && stakedAmounts[msg.sender].referee == address(0) && stakedAmounts[referralAddress].referee != msg.sender) {
            ...
            stakedAmounts[referralAddress].amount += amount.div(1000).mul(150); // If null address rewards go to dead wallet
            stakedAmounts[stakedAmounts[referralAddress].referee].amount += amount.div(1000).mul(70); // If null address rewards go to dead wallet
            stakedAmounts[stakedAmounts[stakedAmounts[referralAddress].referee].referee].amount += amount.div(1000).mul(30); // If null address rewards go to dead wallet
        }
    ...
    function claimReward() public noReentry {
        ...
        uint256 rewardFee = reward.div(100).mul(stakingParams[stakedAmounts[msg.sender].lockUpPlan].withdrawFee);
        uint256 rewardUser = reward.sub(rewardFee);
        token.safeTransfer(msg.sender, rewardUser);
        token.safeTransfer(marketingWallet, rewardFee);
    ...
    function compound() public noReentry {
        ...
        stakedAmounts[msg.sender].amount += reward;
    ...
    // Update the max withdraw for each plan
    function updateMaxWithdraw(uint256 plan, uint256 amount) public {
        require(msg.sender == marketingWallet, "Not the owner");
        require(stakingParams[plan].maxReward < amount, "Max reward can only be increased not decreased");
        stakingParams[plan].maxReward = amount;
    }
    ...
    function displayEstimatedReward(address user) public view returns (uint256) {
        ...
        uint256 dailyReward = stakedAmount.mul(rewardPercentage).div(1000);

        // Check if the user has reached the maximum withdrawal amount
        uint256 maxWithdrawal = stakingParams[stakedAmounts[msg.sender].lockUpPlan].maxWithdrawal;
        uint256 totalWithdrawn = stakedAmounts[msg.sender].totalWithdrawn;
        if (totalWithdrawn.add(dailyReward) >= maxWithdrawal) {
            dailyReward = 0;
        }
        
        // Check if daily reward is higher then max reward
        if (stakingParams[lockUpPlan].maxReward <= dailyReward) {
            dailyReward = stakingParams[lockUpPlan].maxReward;
        }
