...
contract DVFDepositContract is Ownable, MessageSenderApp, MessageReceiverApp {
  ...
  function executeMessageWithTransfer(
      ...
      return this.executeProxiedWithdrawal(_token, _amount, _srcChainId, _message);
  }

  function executeProxiedWithdrawal(
    address _token,
    uint256 _amount,
    uint64 _srcChainId,
    bytes calldata _message
  ) external payable onlyOwner returns (ExecutionStatus) {
    ProxiedWithdrawalRequest memory message = abi.decode((_message), (ProxiedWithdrawalRequest));
    // require(_token == m.swap.path[0], "bridged token must be the same as the first token in destination swap path");
    IERC20Upgradeable(_token).safeTransfer(message.receiver, message.amount);
    emit ProxiedBridgedWithdrawalExecuted(message.receiver, _token, _amount, _srcChainId, message.nonce);
