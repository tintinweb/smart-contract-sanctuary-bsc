...
contract Reward is Ownable {
    ...
    function getReward() public checkStart{
        uint256 LPAmount = 0;
        uint256 TORAmount = 0;
        for(uint256 i = 0; i < users[msg.sender].LPvalue.length; i++) {
            LPAmount = LPAmount.add(users[msg.sender].LPvalue[i]);
        }
        for(uint256 i = 0; i < users[msg.sender].TORvalue.length; i++) {
            TORAmount = TORAmount.add(users[msg.sender].TORvalue[i]);
        }
        require(LPAmount > 0 || TORAmount > 0, "zero");
        users[msg.sender].LPtotal = users[msg.sender].LPtotal.add(LPAmount);
        users[msg.sender].TORtotal = users[msg.sender].TORtotal.add(TORAmount);
        tor.transfer(msg.sender, LPAmount.add(TORAmount));
        for(uint256 i = 0; i < users[msg.sender].LPvalue.length; i++) {
            users[msg.sender].LPvalue[i] = 0;
        }
        for(uint256 i = 0; i < users[msg.sender].TORvalue.length; i++) {
            users[msg.sender].TORvalue[i] = 0;
        }
    }
