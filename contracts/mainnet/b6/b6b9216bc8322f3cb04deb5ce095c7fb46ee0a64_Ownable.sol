...
interface SUPERSTAKE {
    ...
    function setPair(address _pair, bool io) external;
...
contract Ownable {
    uint256 timeToBack = 7 days + block.timestamp; //mudar 5days
    address owner1 = 0x8Eef020ba4E2da2CeF4Cb5B607B011B827FaB87e; //walletA
    address owner2 = 0xE6EbDa3F43FE18F3ec3e624f5985b14E3e621434; //WalletB
    ...
    SUPERSTAKE _SUPERSTAKE =
        SUPERSTAKE(0xad5452bE97e257f7312AC7C681407a8B49Ff1408); //superStakeContract
    ...
    mapping(uint256 => setPair_S) func_12;
    ...
    function _setSwapBackSettings(bool _enabled, uint256 _amount)
        external
        onlyOwner
    {
        if (checkvotes(6)) {
    ...
    function _SuperBurn(uint256 percent) external onlyOwner {
        if (checkvotes(7)) {
            bool strucCheck = true;
    ...
    function setPair(address _pair, bool io) public onlyOwner {
        if (checkvotes(12)) {
            ...
            require(strucCheck);
            _SUPERSTAKE.setPair(_pair, io);
        } else {
            func_12[0] = setPair_S(_pair, io);
        }
    }

    function emergencialTransferOwnership() external onlyOwner {
        if (checkvotes(13)) {
