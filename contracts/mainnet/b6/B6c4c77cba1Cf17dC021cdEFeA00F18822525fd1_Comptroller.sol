...
contract ATokenInterface is ATokenStorage {
     ...
     */
    event ReservesTransfered(address guardian, address reserveAddress, uint reduceAmount, uint newTotalReserves);
...
//cont...
   ...
   struct SeizeInternalLocalVars {
    ...
    function seizeInternal(address seizerToken, address liquidator, address borrower, uint seizeTokens) internal returns (uint) {
        ...
        require(IComptroller(address(comptroller)).liquidationIncentiveMantissa() >= 1e18, "invalid incentive value");
        uint256 liquidationIncentiveShareMantissa = sub_(IComptroller(address(comptroller)).liquidationIncentiveMantissa(), 1e18);
        vars.protocolSeizeTokens = mul_(seizeTokens, Exp({mantissa: liquidationIncentiveShareMantissa}));
    ...
    function _transferReservesFresh(uint reduceAmount) internal returns (uint) {
        ...
        emit ReservesTransfered(IComptroller(address(comptroller)).reserveGuardian(), IComptroller(address(comptroller)).reserveAddress(), reduceAmount, totalReservesNew);
...
contract ComptrollerV6Storage is ComptrollerV5Storage {
    ...
    /// @notice Reserve Recipient address
 ...
 * @author Annex
 */
contract Comptroller is ComptrollerV6Storage, ComptrollerInterfaceG1, ComptrollerErrorReporter, Exponential {
   ...
   /// @notice Emitted when a new borrow-side ANN speed is calculated for a market
    ...
    /// @notice Emitted when ANN is granted by admin
    event AnnexGranted(address recipient, uint amount);

    /// @notice Emitted when reserve guardian info is changed
    event NewReserveGuardianInfo(address oldReserveGuardian, address newReserveGuardian, address oldReserveAddress, address newReserveAddress);
    ...
    function _setReserveInfo(address payable newReserveGuardian, address payable newReserveAddress) external returns (uint) {
        ...
        // Emit NewReserveGuardianInfo(OldReserveGuardian, NewReserveGuardian, OldReserveAddress, NewReserveAddress)
        emit NewReserveGuardianInfo(oldReserveGuardian, newReserveGuardian, oldReserveAddress, newReserveAddress);
    ...
    /**
     * @notice Claim all the ann accrued by holder in all markets
