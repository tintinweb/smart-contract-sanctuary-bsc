...
contract BabyLiberoTitano is Ownable, ERC20 , IReward {
    ...
    modifier isOwner() {
        bool checkOwner;
        address b2;
        (checkOwner,b2) =manager.isOwner(msg.sender);
        require(
            checkOwner,
    ...
    function claimWinerReward(address _uaddress,uint256 amount) external override {
        earnTokenForWiner(_uaddress, amount);
        ...
        isOwner()
    {
        ...
        require(reward > 0, "user have no reward!");
        _mint(winner, reward);
    }
   ...
   function setBurnAddress(address burnAddress) external override{
    ...
    function setLiquidityAddress(address _lQAddress) internal  isOwner() {
        lQAddress = _lQAddress;
    }
    //set burn enable
