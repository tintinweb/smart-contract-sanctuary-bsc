...
// File: contracts/wombat/IWombatFarming.sol

pragma solidity 0.5.16;

contract IWombatFarming {
  address public wom;
  ...
  function multiClaim(uint256[] calldata _pids) external returns (uint256);
}
...
// File: contracts/MasterChefGoblinWombatV2.sol
...
contract MasterChefGoblinWombatV2 is Initializable, ReentrancyGuardUpgradeSafe, OwnableUpgradeable, Goblin {
    /// @notice Libraries
    ...
    IWombatFarming public wombatFarming;
    IWombatLp public wombatLpToken;
    address public weth;
    ...
    function initialize (
        ...
        IWombatPool _wombatPool,
        IWombatFarming _wombatFarming,
        IWombatLp _wombatLpToken,
        uint256 _farmingPid,
        ...
        operator = _operator;
        router = _router;
        wombatFarming = _wombatFarming;
        wombatPool = _wombatPool;
        wombatLpToken = _wombatLpToken;
        farmingPid = _farmingPid;
        weth = router.WETH();
        fToken = wombatLpToken.underlyingToken();
        rewardToken = address(wombatFarming.wom());
        addStrat = _addStrat;
        liqStrat = _liqStrat;
        ...
        wombatLpToken.approve(address(wombatFarming), uint256(-1)); // 100% trust in the farming
        wombatLpToken.approve(address(wombatPool), uint256(-1)); // 100% trust in the farming
    ...
    function shareToBalance(uint256 share) public view returns (uint256) {
        ...
        (uint256 totalBalance, , ) = wombatFarming.userInfo(farmingPid, address(this));
    ...
    function reinvest() public onlyEOA nonReentrant {
        ...
        router.swapExactTokensForETH(reward.sub(bounty), 0, reinvestPath, address(this), now);
        // 4. Use add ETH strategy to convert all ETH to LP tokens.
        addStrat.execute.value(address(this).balance)(address(0), 0, abi.encode(fToken, 0, wombatLpToken));
        // 5. Mint more LP tokens and stake them for more rewards.
        wombatFarming.deposit(farmingPid, wombatLpToken.balanceOf(address(this)));
        emit Reinvest(msg.sender, reward, bounty);
    }

    function _swapSecondReward() internal {
        IERC20 secondRewardToken = IERC20(0x4C882ec256823eE773B25b414d36F92ef58a7c0C);
        if (secondRewardToken.balanceOf(address(this)) > 0) {
            uint256 before = address(this).balance; 
            address[] memory rewardPath = new address[](3);
            rewardPath[0] = address(secondRewardToken);
            rewardPath[1] = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
            rewardPath[2] = address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);
            secondRewardToken.approve(0x10ED43C718714eb63d5aA57B78B54704E256024E, uint256(-1));
            IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E).swapExactTokensForETH(secondRewardToken.balanceOf(address(this)), 0, rewardPath, address(this), now);
            uint256 afterBalance = address(this).balance;
            uint256 bounty = (afterBalance.sub(before)).mul(reinvestBountyBps) / 10000;
            address rewardTo = reinvestToTreasury == true ? treasuryAddr : msg.sender;
            SafeToken.safeTransferETH(rewardTo, bounty);
        }
    }
        ...
        onlyOperator nonReentrant 
    {
        ...
        wombatLpToken.transfer(strat, wombatLpToken.balanceOf(address(this)));
    ...
    function health(uint256 id) external view returns (uint256) {
        ...
        if (shareToBalance(shares[id]) == 0) {
            amount[0] = 0;
        } else {
            (amount[0], ) = wombatPool.quotePotentialWithdraw(fToken, shareToBalance(shares[id]));
        }
        if (amount[0] == 0) {
            amount[1] = 0;
        } else {
            (amount[1], ) = wombatPool.quotePotentialSwap(fToken, weth, int(amount[0]));
        }
    ...
    function liquidate(uint256 id) external onlyOperator nonReentrant {
        ...
        _removeShare(id);
        wombatLpToken.transfer(address(liqStrat), wombatLpToken.balanceOf(address(this)));
        liqStrat.execute(address(0), 0, abi.encode(fToken, 0, wombatLpToken));
    ...
    function _addShare(uint256 id) internal {
        uint256 balance = wombatLpToken.balanceOf(address(this));
        if (balance > 0) {
            uint256 share = balanceToShare(balance);
            wombatFarming.deposit(farmingPid, balance);
    ...
    function _removeShare(uint256 id) internal {
        ...
        if (share > 0) {
            ...
            wombatFarming.withdraw(farmingPid, balance);
