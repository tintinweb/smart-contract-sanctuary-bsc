...
contract LockedSale is Ownable, ReentrancyGuard {
    ...
    function buyToken(uint256 _amount, IPancakePair _spendingToken) public onlySaleActive nonReentrant {
        ...
        uint256 lpAmount = discountedPrice.div(lpPrice);
        _spendingToken.transfer(receiver0, lpAmount.div(3) );
        _spendingToken.transfer(receiver1, lpAmount.div(3) );
        _spendingToken.transfer(receiver2, lpAmount.div(4) );
    ...
    // view functions
    function pathMaker2(IBEP20 _spendingToken) public view returns(address[] memory) {
        address[] memory path;
        path = new address[](2);
