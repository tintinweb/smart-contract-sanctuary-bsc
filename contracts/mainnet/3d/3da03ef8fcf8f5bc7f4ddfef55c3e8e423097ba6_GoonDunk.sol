...
library Address{
 ...
 contract GoonDunk is Context, IERC20, Ownable {
    ...
    address public WarChestWallet = 0x3ed4f7A4D42d5e8026D36C7C761E36c6549dF049;

    string private constant _name = "GoonDunk";
    string private constant _symbol = "$DUNK";
    ...
    Taxes public taxes = Taxes(0, 2, 1, 7);
    Taxes public sellTaxes = Taxes(0, 2, 1, 7);
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[WarChestWallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[BuyBackWallet] = true;
        allowedTransfer[WarChestWallet] = true;
    ...
    function setTaxes(uint256 _rfi, uint256 _WarChest, uint256 _liquidity, uint256 _BuyBack) public onlyOwner {
       taxes = Taxes(_rfi,_WarChest,_liquidity,_BuyBack);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _WarChest, uint256 _liquidity, uint256 _BuyBack) public onlyOwner {
       sellTaxes = Taxes(_rfi,_WarChest,_liquidity,_BuyBack);
        emit FeesChanged();
    }
    ...
    function _takeWarChest(uint256 rWarChest, uint256 tWarChest) private {
        totFeesPaid.WarChest +=tWarChest;

        if(_isExcluded[address(this)])
        {
            _tOwned[address(this)]+=tWarChest;
        }
        _rOwned[address(this)] +=rWarChest;
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi, to_return.rWarChest, to_return.rLiquidity) = _getRValues1(to_return, tAmount, takeFee, _getRate());
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.tWarChest = tAmount*temp.WarChest/100;
        s.tLiquidity = tAmount*temp.liquidity/100;
        s.tBuyBack = tAmount*temp.BuyBack/100;
        s.tTransferAmount = tAmount-s.tRfi-s.tWarChest-s.tLiquidity-s.tBuyBack;
        return s;
    }

    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rWarChest, uint256 rLiquidity){
        ...
        rWarChest = s.tWarChest*currentRate;
        rLiquidity = s.tLiquidity*currentRate;
        uint256 rBuyBack = s.tBuyBack*currentRate;
        rTransferAmount =  rAmount-rRfi-rWarChest-rLiquidity-rBuyBack;
        return (rAmount, rTransferAmount, rRfi,rWarChest,rLiquidity);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tWarChest + s.tBuyBack);
        }
        if(s.rWarChest > 0 || s.tWarChest > 0) _takeWarChest(s.rWarChest, s.tWarChest);
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.WarChest + temp.BuyBack) * 2;
        ...
        uint256 WarChestAmt = unitBalance * 2 * temp.WarChest;
        if(WarChestAmt > 0){
            payable(WarChestWallet).sendValue(WarChestAmt);
        }
