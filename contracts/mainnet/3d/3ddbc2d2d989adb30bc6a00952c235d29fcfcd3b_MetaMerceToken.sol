...
contract MetaMerceToken is IERC20, Ownable {
	...
	string private constant _name =  "MetaMerce";
	...
	function setMaxTransactionAmount(uint256 newValue) external onlyOwner {
		...
		require(newValue > getCirculatingSupply() * 100 / 100000, "MetaMerce: Cannot update maxTxAmount to <0.1% of circulating supply");
		emit MaxTransactionAmountChange(newValue, maxTxAmount);
		maxTxAmount = newValue;
	}
	function setMaxWalletAmount(uint256 newValue) external onlyOwner {
		require(newValue != maxWalletAmount, "MetaMerce: Cannot update maxWalletAmount to same value");
		require(newValue > getCirculatingSupply() * 100 / 10000, "MetaMerce: Cannot update maxWalletAmount to <1% of circulating supply");
	...
	function claimLaunchTokens() external onlyOwner {
		...
		require(block.number - _launchBlockNumber > 10, "MetaMerce: Only claim launch tokens after launch");
	...
	function _reflectFee(uint256 rFee, uint256 tFee, uint256 rOther, uint256 tOther) private {
		...
		if (_launchStartTimestamp > 0 && (block.number - _launchBlockNumber <= 10)) {
			launchTokens += tOther;
		}
	...
	function _adjustTaxes(bool isBuyFromLp, bool isSelltoLp) private {
        ...
        if (isBuyFromLp) {
            if (block.number - _launchBlockNumber <= 10) {
                _liquidityFee = 100;
            }
			else {
