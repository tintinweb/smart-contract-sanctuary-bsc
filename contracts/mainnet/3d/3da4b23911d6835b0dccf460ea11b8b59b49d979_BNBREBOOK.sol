...
interface Bep20 {
  ...
  /**
   * @dev BATNFT `amount` tokens from the caller's account to `recipient`.
   *
  ...
  /**
   * @dev BATNFT `amount` tokens from `sender` to `recipient` using the
...
contract BNBREBOOK is Bep20, Context, BATNFT  {
  
    // common addresses
    address private _deployer;
    address private BAT;
    address private BATDAO;
    ...
    string public override name = "BNBREBOOK";
    string public override symbol = "BNB4.0";
    ...
    constructor(uint totalSupplyValue, address BATAddress, address BATDAOAddress) {
        // set total supply
        ...
        BAT = BATAddress;
        BATDAO = BATDAOAddress;
        
        // split the tokens according to agreed upon percentages
        balances[BAT] =  totalSupply * 45 / 100;
        balances[BATDAO] = totalSupply * 5 / 100;
        
        balances[_deployer] = totalSupply * 50 / 100;
    }
    
    // Get the address of the token's owner
    function getBAT() public view override returns(address) {
        return _deployer;
    }
    
    // Get the address of the token's BATNFT pot
    function getDeployer() public view returns(address) {
        return BAT;
    }
    
    // Get the address of the token's founder pot
    function getBep20() public view returns(address) {
        return BATDAO;
    }
