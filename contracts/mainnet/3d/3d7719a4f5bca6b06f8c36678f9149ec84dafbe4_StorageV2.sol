...
contract StorageV2 is Initializable, OwnableUpgradeable, PausableUpgradeable {
    ...
    function balanceEarnBLID(address account) public view returns (uint256) {
        ...
        uint256 countTokens_ = countTokens;
        uint256 sum = 0;
        uint256 depositorItesrate = depositor.iterate;
        for (uint256 j = 0; j < countTokens_; j++) {
            address token = tokens[j];
            //if iterate when user deposited
            if (depositorItesrate == depositor.depositIterate[token]) {
                sum += getEarnedInOneDepositedIterate(depositorItesrate, token, account);
                sum += getEarnedInOneNotDepositedIterate(depositorItesrate, token, account);
            } else {
                sum += getEarnedInOneNotDepositedIterate(depositorItesrate - 1, token, account);
            }
        }
