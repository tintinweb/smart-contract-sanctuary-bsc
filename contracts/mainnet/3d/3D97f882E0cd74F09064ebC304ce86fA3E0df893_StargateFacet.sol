...
contract StargateFacet is IMiraidon, SwapperV2, ReentrancyGuard, Validatable {
    /// Storage ///
    bytes32 internal constant NAMESPACE =
        keccak256("com.miraidon.facets.stargate");
    ...
    /// @param callTo the address to send the tokens to the destination
    ...
    /// @notice Initialize local variable for the Stargate Adapter
    ...
    function initStargate(
        ...
        for (uint256 i = 0; i < poolIdConfigs.length; i++) {
            require(poolIdConfigs[i].token != address(0), GenericErrors.E14);
            s.poolId[poolIdConfigs[i].token] = poolIdConfigs[i].poolId;
        }

        for (uint256 i = 0; i < chainIdConfigs.length; i++) {
            s.layerZeroChainId[chainIdConfigs[i].chainId] = chainIdConfigs[i]
                .layerZeroChainId;
        }
    ...
    /// @notice contains the business logic for the bridge via Stargate bridge
    /// @param _bridgeData Data used purely for tracking and analytics
    /// @param _stargateData Data specific to Stargate Bridge
    function _startBridge(
        ...
        _stargateData.minAmountLD = LibFeeCollect.fix(
    ...
    function setStargatePoolId(address _token, uint16 _poolId) external {
        ...
        require(sm.initialized, GenericErrors.E21);

        sm.poolId[_token] = _poolId;
    ...
    function getStargatePoolId(address _token) private view returns (uint16) {
        Storage storage sm = getStorage();
        uint16 poolId = sm.poolId[_token];
        require(poolId != 0, GenericErrors.E57);
        return poolId;
    }
    ...
    function getLayerZeroChainId(
        ...
        require(chainId != 0, GenericErrors.E58);
        return chainId;
    }
