...
interface NFT721 {
    ...
    function getInfo(uint256 _tokenId) external view returns(address, string memory, string memory, string memory,uint256);
...
contract GAME is Owner {
    ...
    address public nft_addr = 0xF102FC23f3495EcF1C984d4DDbB972530903Ea51;
	...
	uint256 public buy_game_max = 800 * 10 **18;

 
	
	uint public base_output_amount=40000000000000000000;
	uint public speed_output_amount_1=250000000000000000000;
	uint public speed_output_amount_2=260000000000000000000;
	uint public speed_output_amount_3=280000000000000000000;
	uint public speed_output_amount_4=300000000000000000000;
    ...
    function set_speed_output_amount_1(uint _val) external onlyOwner {
        speed_output_amount_1 = _val;
    }
    function set_speed_output_amount_2(uint _val) external onlyOwner {
        speed_output_amount_2 = _val;
    }
    function set_speed_output_amount_3(uint _val) external onlyOwner {
        speed_output_amount_3 = _val;
    }
    function set_speed_output_amount_4(uint _val) external onlyOwner {
        speed_output_amount_4 = _val;
    }
    ...
    function base_output(uint cur_id) external payable lock returns(bool) {
        ...
        (ownerAddress, NFT_name, NFT_ymbol, NFT__tokenURI,NFT__grade_id) = NFT721(nft_addr).getInfo(cur_id);
    ...
    function speed_output(uint cur_id) external payable lock returns(bool) {
        ...
        (ownerAddress, NFT_name, NFT_ymbol, NFT__tokenURI,NFT__grade_id) = NFT721(nft_addr).getInfo(cur_id);
		if(NFT__grade_id==1){
			speed_output_amount=speed_output_amount_1;	
		}
		if(NFT__grade_id==2){
			speed_output_amount=speed_output_amount_2;	
		}
		if(NFT__grade_id==3){
			speed_output_amount=speed_output_amount_3;	
		}
		if(NFT__grade_id==4){
			speed_output_amount=speed_output_amount_4;	
		}
    ...
    function day_cal(uint256 _day) external onlyOwner returns(bool) {
        ...
        if (base_output_lists[_day] < all_coin) {
            ...
            IERC20(token_addr).transfer(0xF13d0c5E16C87f714699288887F8f2F319867e33, span);

        }
        if (speed_output_lists[_day] < all_coin) {
            uint256 span = all_coin-speed_output_lists[_day];

            span = span * speed_output_amount_1;
