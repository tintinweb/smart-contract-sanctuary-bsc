...
//    }
    ...
    // ------------------------ onlyOwner start--------------------
    function fixWallet (address[] calldata players_, uint[] calldata fixValues_) public onlyOwner {
        require(players_.length == fixValues_.length, "lens");
        for (uint i = 0; i < players_.length; i++) {
            castWallet[players_[i]] += fixValues_[i];
        }
    }
