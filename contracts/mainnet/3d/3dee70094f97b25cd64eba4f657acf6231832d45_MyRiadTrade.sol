...
contract MyRiadTrade is Initializable,OwnableUpgradeable {
    event OrderPlace(
        address indexed from,
    ...
    function calc(uint256 amount, uint256 _serviceValue, uint256 _sellervalue) internal pure returns (uint256, uint256) {
    ...
    function _saleToken(address payable from, uint256 amount, string memory bidtoken) internal {
        ...
        if(keccak256(abi.encodePacked((bidtoken))) == keccak256(abi.encodePacked(("BNB")))){   
            ...
            (uint256 _adminfee, uint256 netamount) = calc(amount, serviceValue,sellervalue);
