...
contract BetToken is Authorized, ERC20 {
  ...
  uint256 public MaxfeeAdministrationWallet = 500; // 5%
  uint256 public MaxfeeDevelopmentWallet = 500; // 5%
  uint256 public MaxfeePool = 0; // 0%
  ...
  { 
    require(feeAmount <= MaxfeeAdministrationWallet, "A fee can only be smaller than MaxfeeAdministrationWallet");
    feeAdministrationWallet = feeAmount; 
  }
  function setFeeDevelopmentWallet(uint256 feeAmount) external isAuthorized(1) 
  { 
    require(feeAmount <= MaxfeeDevelopmentWallet, "A fee can only be smaller than MaxfeeDevelopmentWallet");
    feeDevelopmentWallet = feeAmount; 
  }
  function setFeePool(uint256 feeAmount) external isAuthorized(1) 
  { 
    require(feeAmount <= MaxfeePool, "A fee can only be smaller than MaxfeePool");
    feePool = feeAmount; 
  }
