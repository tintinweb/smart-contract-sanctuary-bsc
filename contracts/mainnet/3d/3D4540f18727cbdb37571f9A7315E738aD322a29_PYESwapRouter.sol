...
library PYESwapLibrary {
    ...
    address constant factory = 0x92DEe215a090Ff9b99e5E0B3E96BFA6C4EE7409C;
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut, bool tokenFee, bool baseOut, uint totalFee) internal pure returns (uint amountIn) {
        require(amountOut > 0, 'PYESwapLibrary: INSUFFICIENT_OUTPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'PYESwapLibrary: INSUFFICIENT_LIQUIDITY');
        uint amountOutMultiplier = !baseOut && tokenFee ? 10000 - totalFee : 10000;
    ...
    function getAmountsIn(uint amountOut, address[] memory path, uint totalFee) internal view returns (uint[] memory amounts) {
        ...
        for (uint i = path.length - 1; i > 0; i--) {
            ...
            amounts[i - 1] = getAmountIn(amounts[i], reserveIn, reserveOut, baseToken != address(0), baseOut, totalFee);
        }
    }
