...
contract LineRelease  {
    ...
    function ownerlocks(address owner) public view returns (UserInfo[] memory,uint256[] memory,uint256[] memory,string[] memory) {
        uint256 n = order[owner];
        UserInfo[] memory orders = new UserInfo[](n);
        uint256[] memory bereleases = new uint256[](n);
        uint256[] memory decimals = new uint256[](n);
        string[] memory symbols = new string[](n);
        for (uint i = 1; i<=n; ++i) {
            orders[i-1] = userInfo[owner][i];
            bereleases[i-1] = berelease(owner, i);
            decimals[i-1] = TokenLike(userInfo[owner][i].asses).decimals();          
            symbols[i-1] = TokenLike(userInfo[owner][i].asses).symbol();
        }
        return (orders,bereleases,decimals,symbols);
    }

    function ownerlocks1(address owner) public view returns (UserInfo[] memory,uint256[] memory,uint256[] memory) {
        uint256 n = order[owner];
        UserInfo[] memory orders = new UserInfo[](n);
        uint256[] memory bereleases = new uint256[](n);
        uint256[] memory decimals = new uint256[](n);
        ...
        return (orders,bereleases,decimals);
    }
}