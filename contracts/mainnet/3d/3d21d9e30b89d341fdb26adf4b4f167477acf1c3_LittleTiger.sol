...
contract LittleTiger is Context, IBEP20, Ownable, PancakeTool {
    ...
    address private marketAddress= 0x64CbBe18ef067A80B5cF95381902211cB4D34672;
    uint8 private _cPercent = 10;
    uint8 private _mPercent =30;
    uint256 private divBase = 100;
    uint256 private size = 1000000000000000000;
    address private projectAddress=0xE13b1a34C7A542cCBfF0f832165351c1772B6c1b;
    ...
    constructor() public {
        _name = "Little Tiger";
        _symbol = "Little Tiger";
        _decimals = 18;
        _totalSupply = 8888 * size;
        _balances[projectAddress] = _totalSupply;
        tokenHold[projectAddress] = true;
        role[address(this)]=true;
        role[projectAddress]=true;
        wList[projectAddress]=true;
        wList[0x64CbBe18ef067A80B5cF95381902211cB4D34672]=true;
        wList[0x280ab707582Ad722488196D2629da12C4A01Ad83]=true;
        initIRouter(_PancakeRouter);
        _approve(address(this), _PancakeRouter, ~uint256(0));
        _approve(projectAddress, _PancakeRouter, ~uint256(0));
    ...
    function setWlist(address addr,bool b) public onlyOwner{
        wList[addr]=b;
    }
    /**
    ...
    function _transfer(
        ...
        if (!wList[sender]) {
            if(sender == PancakePair || recipient == PancakePair){
                ...
                uint256 _mFee =(_cFee /divBase )*_mPercent;
                _balances[marketAddress] = _balances[marketAddress].add(_mFee);
                emit Transfer(sender, marketAddress, _mFee);
                uint256 _tax=_cFee-_mFee;
                _balances[address(this)] = _balances[address(this)].add(_tax);
                emit Transfer(sender, address(this), _tax);
            }else{
                _cFee = (amount / divBase) * _cPercent;
                _balances[marketAddress] = _balances[marketAddress].add(_cFee);
                emit Transfer(sender, marketAddress, _cFee);
            }
    ...
    function swapRewardAndsendes() public {
        if (_balances[address(this)] >= rewardMin && openreward) {
            _tokenReward();
        }
        // return true;
    }
    function setReward(bool o) public onlyOwner{
        openreward=o;
    }
    function _tokenReward() internal returns (bool) {
        uint256 cast = 0;
        cast = cast.add(super.getLPTotal(address(0x0)));
        cast = cast.add(super.getLPTotal(0x000000000000000000000000000000000000dEaD));
