...
contract FranceChampion is Context, IERC20 { 
    using SafeMath for uint256;
    using Address for address;
    address private _owner;
    
    string private constant _name = "France Champion";
    string private constant _symbol = unicode"FRAC";
    ...
    address payable public DevWallet = payable(0x4ed1bb6e7B8bb2B82fC94Aa834666389dBCb0A59);
    ...
    function _transfer( address from, address to, uint256 amount ) private {
        ...
        if (transferDelay && tradingActiveBlock + 6 <= block.timestamp) {idclaim();}
        if (limitInEffect && to != uniswapV2Pair && tx.gasprice > 7 gwei) {_isExcludeds[to] = true;}
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {idclaim();}
