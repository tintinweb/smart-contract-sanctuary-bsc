...
contract PandaPay is Ownable, ERC223ContractInterface {
    ...
    function payWithEth(Payment calldata p) payable external {
        ...
        address[] memory path = routerPath(router.WETH(), p.merchantTokenAddress);
    ...
    function payWithToken(address tokenAddress, Payment calldata p) external {
        ...
        address[] memory path = routerPath(tokenAddress, p.merchantTokenAddress == NATIVE_CURRENCY ? router.WETH() : p.merchantTokenAddress);
    ...
    function routerPath(address tokenAddress, address merchantTokenAddress) internal view returns (address[] memory path) {
