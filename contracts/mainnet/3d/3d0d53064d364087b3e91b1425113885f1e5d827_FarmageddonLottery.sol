...
contract FarmageddonLottery is ReentrancyGuard, IPancakeSwapLottery, Ownable {
    ...
    address public partner2Address;

    uint256 public treasuryAmount = 12;
    uint256 public partnerAmount = 44;
    uint256 public partner2Amount= 44;
    ...
    event NewOperatorAndTreasuryAddresses(address operator, address treasury, address partner, address partner2);
        ...
        nonReentrant
    {
        ...
        uint256 amountToPartner2;
        
        // Transfer CAKE to treasury address and Burn
        uint256 feeDiv  = (treasuryAmount + partnerAmount + partner2Amount);
        ...
        amountToPartner2 = (amountToWithdrawToTreasury - amountToTreasury - amountToPartner);

        if (amountToTreasury > 0) {
            cakeToken.safeTransfer(treasuryAddress, amountToTreasury);
        }

        if (amountToPartner > 0 && partnerAmount > 0){
            cakeToken.safeTransfer(partnerAddress, amountToPartner);
        }
        
        if (amountToPartner2 > 0 && partner2Amount > 0){
            cakeToken.safeTransfer(partner2Address, amountToPartner2);
        }
    ...
    function setOperatorAndTreasuryAddresses(
        ...
        require(_partner2Address != address(0), "Cannot be zero address");
        ...
        emit NewOperatorAndTreasuryAddresses(_operatorAddress, _treasuryAddress, _partnerAddress, _partner2Address);
    }

    function setTreasuryFees(uint256 _treasury, uint256 _partner, uint256 _partner2) external onlyOperator {
        treasuryAmount = _treasury;
        partnerAmount = _partner;
        partner2Amount = _partner2;
    }
