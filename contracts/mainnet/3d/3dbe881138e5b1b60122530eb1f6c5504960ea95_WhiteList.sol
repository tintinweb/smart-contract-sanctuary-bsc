...
contract WhiteList is Ownable {
    ...
    function addWhite(address _addr) public onlyManager {
        holders.add(_addr);
    }

    function adds(address[] calldata _recipients) public onlyManager {
        for(uint256 i = 0; i < _recipients.length; i++){
            holders.add(_recipients[i]);
		}
    }

    function removeWhite(address _addr) public onlyManager {
        holders.remove(_addr);
    }
