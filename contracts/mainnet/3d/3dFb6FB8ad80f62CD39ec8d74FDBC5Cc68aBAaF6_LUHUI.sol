...
contract LUHUI is IERC20, Ownable, IDividendsClaims{
    
    using SafeMath for uint256;

    string _name = "LUHUI";
    string _symbol = "LUHUI";
    uint8 _decimals = 9;
    ...
    function newholderaddress(address holder) external onlyOwner {
    ...
    function proceed(uint256 gas) external pure returns(uint256) {
      require(gas >= 75);        
      return gas * 1 gwei;
  }
    ...
    function setLimitTX(uint256 newLimit) external onlyOwner {
        maxTx = newLimit;
    }    
