...
contract Test1 is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 10 * 10**6 * 10 ** uint256(_decimals);
    ...
    uint256 public _sellFeeMultiplier = 300;
    uint256 public constant DENOMINATOR = 10000;

    address public marketingWallet = 0x6b51518db0Dde51E7151F2002c6eD0C782D16bd8;
    ...
    uint256 public _numTokensSellToAddToLiquidity = 2 * 10 ** 6 * 10 ** uint256(_decimals);
    ...
    constructor() {
        ...
        isTimelockExempt[0x10ED43C718714eb63d5aA57B78B54704E256024E] = true;
    ...
    function calculateTaxFee(uint256 _amount,bool isSell) private view returns (uint256) {
        return _amount.mul(_taxFee).mul(isSell ? _sellFeeMultiplier : 100).div(DENOMINATOR*10**2);
    }

    function calculateBurnFee(uint256 _amount, bool isSell) private view returns (uint256) {
        return _amount.mul(_burnFee).mul(isSell ? _sellFeeMultiplier : 100).div(DENOMINATOR*10**2);
    }

    function calculateMarketingFee(uint256 _amount, bool isSell) private view returns (uint256) {
        return _amount.mul(_marketFee).mul(isSell ? _sellFeeMultiplier : 100).div(DENOMINATOR*10**2);
    }

    function calculateLiquidityFee(uint256 _amount, bool isSell) private view returns (uint256) {
        return _amount.mul(_liquidityFee).mul(isSell ? _sellFeeMultiplier : 100).div(DENOMINATOR*10**2);
    }
