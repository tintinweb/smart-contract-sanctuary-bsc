...
contract PandaTest is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    PandaTestDividendTracker public dividendTracker;

    address payable public DevANDMarketing;
    ...
    uint256 public sellDevANDMarketingFees;
    uint256 public sellLiquidityFee;
    uint256 public buyDeadFees;
    uint256 public buyDevANDMarketingFees;
    ...
    event UpdateFees(
        ...
        uint256 sellDevANDMarketingFees,
        uint256 sellLiquidityFee,
        uint256 sellRewardsFee,
        uint256 buyDeadFees,
        uint256 buyDevANDMarketingFees,
    ...
    constructor() ERC20("PandaTest", "Panda") {
        DevANDMarketing = payable(0x49cD1cd98Ae946B4d52590bb926cB383Af073Bf1);
        CharityWallet = payable(0xe72F8015A57E4fc2c04De4eEb4152a3d20A16E0f);
        address router = 0x10ED43C718714eb63d5aA57B78B54704E256024E;

        buyDeadFees = 1;
        sellDeadFees = 1;
        buyDevANDMarketingFees = 3;
        sellDevANDMarketingFees = 3;
        buyLiquidityFee = 1;
        sellLiquidityFee = 1;
        buyRewardsFee = 3;
        sellRewardsFee = 5;
        ...
        totalBuyFees = buyRewardsFee
            .add(buyLiquidityFee)
            .add(buyDevANDMarketingFees)
            .add(buyCharityFee);
        totalSellFees = sellRewardsFee
            .add(sellLiquidityFee)
            .add(sellDevANDMarketingFees)
            .add(sellCharityFee);

         dividendTracker = new PandaTestDividendTracker(
            payable(this),
            router,
        ...
        dividendTracker.excludeFromDividends(DevANDMarketing);
        dividendTracker.excludeFromDividends(owner());

        // exclude from paying fees or having max transaction amount
        _isExcludedFromFees[address(this)] = true;
        _isExcludedFromFees[address(dividendTracker)] = true;
        _isExcludedFromFees[address(DevANDMarketing)] = true;
    ...
    function updateFees(
        ...
        emit UpdateFees(
            sellDeadFees,
            sellDevANDMarketingFees,
            sellLiquidityFee,
            sellRewardsFee,
            buyDeadFees,
            buyDevANDMarketingFees,
            buyLiquidityFee,
            buyRewardsFee,
            buyCharityFee,
            sellCharityFee
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            if (!isBuying && !isSelling) {
                ...
                DevANDMarketingFees = sellDevANDMarketingFees;
                liquidityFee = sellLiquidityFee;
                CharityFees = sellCharityFee;
                ...
                DevANDMarketingFees = buyDevANDMarketingFees;
            ...
            uint256 totalFees = RewardsFee
                .add(liquidityFee + DevANDMarketingFees + CharityFees);
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        if (buyRewardsFee > 0) {
            ...
            .div(totalAmount)
            .mul(buyRewardsFee)
            .div(buyRewardsFee + buyDevANDMarketingFees + buyCharityFee);
        }
        if (sellRewardsFee > 0) {
            ...
            .div(totalAmount)
            .mul(sellRewardsFee)
            .div(sellRewardsFee + sellDevANDMarketingFees + sellCharityFee);
        }
        ...
        uint256 _completeFees = sellDevANDMarketingFees.add(sellCharityFee) +
            buyDevANDMarketingFees.add(buyCharityFee);

        uint256 feePortions;
        if (_completeFees > 0) {
            feePortions = address(this).balance.div(_completeFees);
        }
        uint256 marketingPayout = buyDevANDMarketingFees.add(sellDevANDMarketingFees) * feePortions;
        uint256 devPayout = buyCharityFee.add(sellCharityFee) * feePortions;

        if (marketingPayout > 0) {
            (successOp1, ) = address(DevANDMarketing).call{value: marketingPayout}("");
        }
        if (devPayout > 0) {
            (successOp2, ) = address(CharityWallet).call{value: devPayout}("");
        }
...
contract PandaTestDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    PandaTest public PandaTestContract;
    ...
    constructor(
        ...
        defaultToken = token;
        PandaTestContract = PandaTest(mainContract);
    ...
    function _transfer(
        ...
        require(false, "PandaTest_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "PandaTest_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PandaTest contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(PandaTestContract);

            uint256 prevBalance = PandaTestContract.balanceOf(address(this));
            ...
            {
                uint256 received = PandaTestContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    PandaTestContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
