...
contract HipTest is BEP20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() BEP20("hungrTest", "yTEST") {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (amount == 0) {
            ...
            deadFees = burnFee;
            uint256 _burntokens = amount.mul(deadFees).div(100);
            _tTotal = _tTotal.sub(_burntokens);

            amount = amount.sub(_burntokens);

            if (isSelling) {
                sellAmount = sellAmount.add(_burntokens);
            } else {
                buyAmount = buyAmount.add(_burntokens);
            }
