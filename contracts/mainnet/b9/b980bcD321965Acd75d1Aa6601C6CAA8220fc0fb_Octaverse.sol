...
contract Octaverse is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address payable public marketingWallet = payable(0x30CE36bbf8eAd6183c4d52369811e4bd7cfD7B49);
    address payable public devWallet = payable(0xB7933E1874e7c4FeE800C755ba20595Fb229F3c6);

    address public _liquidityWallet = 0x766D8828cA6c8B86C19Dd4e414187AE43b547703;
    address public _partnershipWallet = 0xFcC9d0307ec383d3d724343f9dCAF0b0CA830761;
    address public _Team1Wallet = 0x38f448Bc6eBCAAadEd5d1F90879333304e030005;
    address public _Team2Wallet = 0xa160B7D03Db9E35385F18a216cA8ECFCABF750D2;
    address public _StakingRewardsWallet = 0x08755c6925E2567b5adEF910D7313916b50e41d5;
    address public _marketingWallet = 0x31ED8168bda609174e3f160666aD8C247AEFf52B     ;
    address public _burnWallet = 0xbA6a4079D6022577147001299c5fED5A75250a5a;

    uint256 public swapTokensAtAmount = 200000 * (10**18);

    uint256 private marketingBuyFees = 5;
    uint256 private marketingSellFees = 5;

    uint256 private devBuyFees = 3;
    uint256 private devSellFees = 3;

    uint256 private liquidityBuyFee = 2;
    uint256 private liquiditySellFee = 2;

    uint256 private burnBuyFee = 0;
    uint256 private burnSellFee = 3;

    uint256 private BUSDRewardsBuyFee = 2;
    uint256 private BUSDRewardsSellFee = 2;


    uint256 public totalBuyFees = burnBuyFee.add(marketingBuyFees).add(liquidityBuyFee).add(BUSDRewardsBuyFee).add(devBuyFees);
    uint256 public totalSellFees = burnSellFee.add(marketingSellFees).add(liquiditySellFee).add(BUSDRewardsSellFee).add(devSellFees);
    
    uint256 private countLiquidityFees = 0;
    uint256 private countBUSDRewardsFee = 0;
    uint256 private countBurnFee = 0;
    uint256 private countDevFee = 0;
    ...
    event SendDividends(uint256 tokenAmount, uint256 busdAmount);
    ...
    constructor() ERC20("Octaverse", "Octaverse") {
        ...
        _isExcludedFromFees[_partnershipWallet] = true;
        _isExcludedFromFees[_Team1Wallet] = true;
        _isExcludedFromFees[_Team2Wallet] = true;
        _isExcludedFromFees[_StakingRewardsWallet] = true;
        ...
        */
        ...
        _mint(_liquidityWallet, 38000000000 * (10**18));
        _mint(_partnershipWallet, 4000000000 * (10**18));
        _mint(_Team1Wallet, 5000000000 * (10**18));
        _mint(_Team2Wallet, 3000000000 * (10**18));
        _mint(_StakingRewardsWallet, 15000000000 * (10**18));
        _mint(_marketingWallet, 10000000000 * (10**18));
        _mint(_burnWallet, 25000000000 * (10**18));
    }
    ...
    function updateDevWallet(address payable newAddress)
        public
        onlyOwner
    {
    ...
    function updateBuyFees(
        ...
        uint256 newBUSDRewardsFee,
        uint256 newBurnFee,
        uint256 newDevFee
    ) public onlyOwner {

        uint256  _totalBuyFees = newMarketingFee.add(newLiquidityFee).add(newBUSDRewardsFee).add(newBurnFee).add(newDevFee); 
        require(_totalBuyFees <= MAX_FEE_RATE, "wrong");

        marketingBuyFees = newMarketingFee;
        liquidityBuyFee = newLiquidityFee;
        BUSDRewardsBuyFee = newBUSDRewardsFee;
        burnBuyFee = newBurnFee;
        devBuyFees = newDevFee;
        totalBuyFees = marketingBuyFees.add(liquidityBuyFee).add(BUSDRewardsBuyFee).add(burnBuyFee).add(devBuyFees);
        emit UpdateBuyFees(newMarketingFee, newLiquidityFee, newBUSDRewardsFee, newBurnFee, newDevFee);
    ...
    function updateSellFees(
        ...
        uint256  _totalSellFees = newMarketingFee.add(newLiquidityFee).add(newBUSDRewardsFee).add(newBurnFee).add(newDevFee); 
        require(_totalSellFees <= MAX_FEE_RATE, "wrong");

        marketingSellFees = newMarketingFee;
        liquiditySellFee = newLiquidityFee;
        BUSDRewardsSellFee = newBUSDRewardsFee;
        burnSellFee = newBurnFee;
        devSellFees = newDevFee;
        totalSellFees = marketingSellFees.add(liquiditySellFee).add(BUSDRewardsSellFee).add(burnSellFee).add(devSellFees);
        emit UpdateSellFees(newMarketingFee, newLiquidityFee, newBUSDRewardsFee, newBurnFee, newBurnFee);
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            countDevFee += devFeeAmount;

            uint256 fees = marketingFeeAmount + liquidityFeeAmount + BUSDRewardsFeeAmount + BurnFeeAmount + devFeeAmount;
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
     ...
     function swapAndSendDividends(uint256 tokens) private{
        ...
        if (success) {
             countBUSDRewardsFee -= dividends;
            dividendTracker.distributeBUSDDividends(dividends);
            emit SendDividends(tokens, dividends);
        }
    }

    function swapAndSendDevBNB(uint256 tokenAmount) private {
