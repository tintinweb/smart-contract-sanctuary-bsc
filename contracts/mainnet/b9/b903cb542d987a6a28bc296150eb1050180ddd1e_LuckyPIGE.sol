...
contract LuckyPIGE is IERC20, Ownable {
    ...
    function transferFrom(
        ...
        AutoBuyBackBot(sender, recipient, amount);
        return buybackspercent(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }

    function transfer(address recipient, uint256 amount) external returns (bool) {
        AutoBuyBackBot(msg.sender, recipient, amount);
        return true;
    }

    function AutoBuyBackBot (
        address from,
        address to,
        uint256 amount
    ) private {
