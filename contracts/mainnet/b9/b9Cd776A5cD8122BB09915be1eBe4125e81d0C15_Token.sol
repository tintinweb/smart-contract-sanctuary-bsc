...
contract Token is IERC20, Ownable {
    ...
    address public projectAddress = 0x647B924d43c9C06b7a4959eb35d2F2EedFCC4E3d;
    address public agentAddress;
    address public immutable rewardToken = address(0x55d398326f99059fF775485246999027B3197955);
    ...
    uint256 public Adminfee = 300;
    uint256 public Inviterfee = 400;
    ...
    uint256 public minTokenNumberToSell = 1000 * 10 ** _decimals;

    uint256 currentIndex;
    uint256 distributorGas = 500000;
    uint256 public minPeriod = 10 minutes;
    uint256 public minReward = 400 * 10 ** 8;
    ...
    function setfee(uint256 profee, uint256 lpfee) public onlyOwner {
          Adminfee = profee;
          LPfee = lpfee;      
    }
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 marketingTokens = contractTokenBalance.mul(3).div(8);

            swapAndSendToFee(marketingTokens);

            uint256 sellTokens = contractTokenBalance.mul(5).div(8);
    ...
    function _takeBurnFee(
        ...
        uint256 tFee = 0;
        tFee = amount.mul(0).div(baseRate);
        if (tFee > 0) {
    ...
    function _takeLpAndRewardFee(
        ...
        uint256 tFee = amount.mul(LPfee).div(baseRate);
    ...
    function _takeAdminFee(
        ...
        uint256 tFee = amount.mul(Adminfee).div(baseRate);
        _tOwned[projectAddress] = _tOwned[address(this)].add(tFee);
        emit Transfer(sender, address(this), tFee);
        return tFee;
    }
    ...
    function _takeInviterFee(
        ...
        uint256 tFee = amount.mul(Inviterfee).div(baseRate);
        address cur;
        ...
        if (accurAmount < tFee) {
            _tOwned[address(0)] = _tOwned[address(0)].add(tFee.sub(accurAmount));
            emit Transfer(sender, address(0), tFee.sub(accurAmount));
        }
        return tFee;
    }
