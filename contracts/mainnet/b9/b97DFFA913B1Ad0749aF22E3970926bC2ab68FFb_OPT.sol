...
abstract contract baseContract is  IBEP20 , Auth {
    ...
    string public constant TOKEN_NAME = "OPT";
    string public constant TOKEN_SYMBOL = "OPT";  
    ...
    Allot public allot;
    function setAllot(uint marketing,uint liquidity,uint burn,uint rewards) external authorized {
    ...
    function setmx(uint  maxgasprice_,uint maxamount_) external authorized { 
    ...
    Limit public limit;
    function setLimit(uint  txMax,uint positionMax,bool whale,uint part) external authorized {
    ...
    function _initOk() external  authorized{
    ...
    //币找回
    function recoverBEP20(address tokenAddress, uint tokenAmount) public {
        require(msg.sender==ceo); 
        IBEP20(tokenAddress).transfer(msg.sender, tokenAmount);
    }
    function recoverBNB(uint tokenAmount) public {
        require(msg.sender==ceo);
...
contract  OPT  is baseContract { 
    ...
    function setEx(address[] calldata  users,uint8 fee,bool txLimit,bool positionLimit,bool reward) public authorized {
    ...
    function setAutoSwap(bool enabled, uint newSwapMin, uint newSwapMax, bool swapByLimitOnly,uint part) external authorized {
    ...
    constructor(  address dev ) payable Auth(msg.sender) { 
		...
		autoSwap=AutoSwap(true,true,5,500,100000);  
        marketingAddress = [ceo];
        marketingShare=[100];  

        fees=Fee(400,400,400,10000);
        allot=Allot(2,1,0,1,4); 
        step=3; 
        ...
        emit Transfer(address(dev), msg.sender, _tTotal); 
    }
}