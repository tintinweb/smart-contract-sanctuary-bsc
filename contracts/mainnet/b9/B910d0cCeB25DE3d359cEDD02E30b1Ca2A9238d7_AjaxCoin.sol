...
contract AjaxCoin is IBEP20, Manageable {
    ...
    uint256 public _totalFeePercentage = _managementFeePercentage + _burnFeePercentage;

    IPancakeRouter02 public _pancakeRouter;
    IPancakePair public _pancakePair;

    address[] private _managementFeesRecievers;
    mapping (address => bool) public _isManagementFeesReciever;
    uint256 private _maxNumberManagementFeesRecievers = 5;
    ...
    uint256 public _managementFeesPendingDistribution = 0;
    ...
    constructor() {
        ...
        _managementFeesRecievers.push(_msgSender());
        _isManagementFeesReciever[_msgSender()] = true;
    ...
    function _transfer(address from, address to, uint256 tAmount) private {
        ...
        uint256 tTransferAmount = tAmount.sub(tManagementFeeAmount).sub(tBurnFeeAmount);

        require (tTransferAmount > 0, "amount of transfer is to small");

        if(!_isInternallySwapping){
        ...
        if(_areFeesEnabled && !(_isExcludedFromFees[to] || _isExcludedFromFees[from])){
            _tOwned[_contextAddress()] = _tOwned[_contextAddress()].add(tManagementFeeAmount);
            
            emit Transfer(from, _contextAddress(), tManagementFeeAmount);
            
            _managementFeesPendingDistribution = _managementFeesPendingDistribution.add(tManagementFeeAmount);
    ...
    function _distributeFees(uint256 liquifyFactor) private internalSwapLock() {
        ...
        require(_managementFeesRecievers.length > 0, "no managers to distribute fees");

        uint256 tFeesToDistribute = _managementFeesPendingDistribution.mul(liquifyFactor).div(1000);
        
        _managementFeesPendingDistribution = _managementFeesPendingDistribution.sub(tFeesToDistribute);

        uint256 individualFeesAmount = tFeesToDistribute.div(_managementFeesRecievers.length);

        _tOwned[_contextAddress()] = _tOwned[_contextAddress()].sub(tFeesToDistribute);

        for(uint256 i = 0; i < _managementFeesRecievers.length; i++){
            _tOwned[_managementFeesRecievers[i]] = _tOwned[_managementFeesRecievers[i]].add(individualFeesAmount);
        }
    }
    ...
    function setManagementFee(uint256 managementFee) public onlyManagement() returns (bool) {
        require(_totalFeePercentage - _managementFeePercentage + managementFee <= 200, "total fees cannot exceed 20 percent");
        _managementFeePercentage = managementFee;
        _totalFeePercentage = _managementFeePercentage + _burnFeePercentage;
        return true;
    }
    ...
    function rescueBNB(uint256 amount) external onlyExecutive() {
