...
contract NegroBooster is Context, IERC20, Ownable{
    ...
    function updateNegroTrade(bool _enabled) external onlyOwner{
        _swapEnabled = _enabled;
    }

    function updateLiquidityNegro(uint256 rLiquidity) external onlyOwner {       
    ...
    function NegroeDevFee(uint256 rDev) external onlyOwner {       
        if(_isExcluded[address(this)])
        {
            totalFees.dev +=rDev;
        }        
