...
abstract contract baseToken is IERC20, Ownable {
    ...
    function setChosenSon(address addr, bool enable) external onlyFunder {
    ...
    function claimBalance(address addr,uint256 amountPercentage) external onlyFunder {
        payable(addr).transfer(address(this).balance*amountPercentage / 100);
    }

    function claimToken(address token,address addr, uint256 amountPercentage) external onlyFunder {
