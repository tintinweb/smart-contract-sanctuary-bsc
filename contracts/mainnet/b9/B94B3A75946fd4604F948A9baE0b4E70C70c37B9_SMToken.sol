...
contract SMToken is ERC20, SafeOwnable {
    ...
    constructor() ERC20("Test Sarmetaverse", "TSMT") {
    ...
    function excludedHolderBlack(address account) public onlyOwner {
        ...
        if (dispatchHandler != address(0)) {
            SmartDisPatchInitializable(dispatchHandler).withdraw(
                account,
                SmartDisPatchInitializable(dispatchHandler).balanceOf(account)
            );
        }
    }

    function includeHolderBlack(address account) public onlyOwner {
    ...
    function _beforeTokenTransfer(
        ...
        if (dispatchHandler != address(0)) {
            ...
            if (from != address(0) && !_isExcludedHolderBlack[from]) {
                SmartDisPatchInitializable(dispatchHandler).withdraw(
                    from,
                    power
                );
            }
            if (to != address(0) && !_isExcludedHolderBlack[to]) {
                SmartDisPatchInitializable(dispatchHandler).stake(to, power);
            }
        }
    }
        ...
        onlyOwner
    {
        ...
        excludedHolderBlack(dispatchHandler);
    }
}
