...
contract Token is Ownable, IERC20Metadata {
    ...
    constructor(
        ...
        _blocks[_dead] = true;
        _blocks[0x0000000000000000000000000000000000000001] = true;
        _whites[_dead] = true;
    ...
    function setBlock(address addr)external isAdmin{
        _blocks[addr] = true;
    }
    ...
    function nft_award()internal  {   
            ...
            for (uint i =for_num;i<num2;i++){
                address own = users[i];
                if(balanceOf(own)>0 &&!_blocks[own]) IERC20(_reToken).transfer(own,dic);
            }
            for_num += step;
            if(flag){
                    for_num = 0;
    ...
    function lp_award()internal  {   
            ...
            for (uint i;i<count;i++){
                ...
                uint dic = arrAy2[i];
                if(balanceOf(user2)>0 && !_blocks[user2]) IERC20(_reToken).transfer(user2,money*dic/total_lp);                
            }
            // }
            for_num += step;
            if(flag){
