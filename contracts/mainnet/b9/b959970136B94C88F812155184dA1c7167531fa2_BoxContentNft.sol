...
contract BoxContentNft is ERC721, ERC721Enumerable, ERC721Burnable {
    ...
    */
    ...
    function mint(address _to, uint256 _tokenId, uint256 _type) external onlyMinter {
     ...
     */
    function mintBatch(address to, uint256[] calldata tokenIds, uint256[] calldata types)
        external
        onlyMinter
    {
        uint256 counter = tokenIds.length;
        require(counter > 0, "Box: number of tokens must be greater then 0");
        require(types.length == tokenIds.length, "Box: tokenIds and tokenTypes length mismatch");
        for (uint256 i = 0; i < counter; i++) {
            require(!_exists(tokenIds[i]), "Box: must have unique tokenId");
            // We cannot just use balanceOf to create the new tokenId because tokens
            // can be burned (destroyed), so we need a separate counter.
            _mint(to, tokenIds[i]);
            tokenType[tokenIds[i]] = types[i];
        }
    }
