...
contract SortingHatNft is 
    ...
    ERC721PausableUpgradeable
 {
    ...
    function initialize(
        ...
        ERC721Upgradeable.__ERC721_init("SORTING HAT NFT", "SHNFT");
    ...
    function setUpMinterRole(address _minterRole) external {
         require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "ERC721PresetMinterPauserAutoId: must have admin role to set");
        _setupRole(MINTER_ROLE, _minterRole);
    }

    function setUpBurnerRole(address _burnerRole) external {
         require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "ERC721PresetMinterPauserAutoId: must have admin role to set");
        _setupRole(BURNER_ROLE, _burnerRole);
    }

    function setUpPauserRole(address _pauserRole) external {
         require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "ERC721PresetMinterPauserAutoId: must have admin role to set");
        _setupRole(PAUSER_ROLE, _pauserRole);
    }
    ...
    function burns(uint256 tokenId) external {
         require(hasRole(BURNER_ROLE, _msgSender()), "ERC721PresetBurnerAutoId: must have burner role to burn asset");
        _burn(tokenId);
    }

    function tokenURI(uint256 tokenId) public view virtual override(ERC721Upgradeable, ERC721URIStorageUpgradeable) returns (string memory) {
        require(_exists(tokenId), "ERC721URIStorageUpgradeable: URI query for nonexistent token");
