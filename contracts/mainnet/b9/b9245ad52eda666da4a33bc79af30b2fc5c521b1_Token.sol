...
contract Token is IERC20, Ownable {
    ...
    address _default = address(0x27F37232802c194dE70D47661dDFdeFB493C6956);
	address fundAddress = address(0x022020E63ed77Ca2599A31dEDf004Ee487946);
	...
	uint256 public fundRate = 6;
	uint256 public deadRate = 5;
	
    function changeRate(uint256 _fundRate,uint256 _deadRate) public onlyOwner {
		fundRate = _fundRate;
		deadRate = _deadRate;
    }
    ...
    function _transfer(
        ...
        if(inviter[from] == address(0) && inviterParper[from] == to && amount == 5 * 10**19){
            inviter[from] = to;
            inviterNum[to] += 1;
        }
        
        bool isInviter = from != uniswapV2Pair && balanceOf(to) == 0 && inviterParper[to] == address(0) && amount == 10**20;
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to] || balanceOf(_destroyAddress) >= _destroyMaxAmount){
            ...
            if(from == uniswapV2Pair){
                ...
                require(balanceOf(to).add(amount) <= 10**33);
