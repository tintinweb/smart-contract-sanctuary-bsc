...
contract SC2 is IBEP20, Auth {
    ...
    string constant _name = "SchrodingersCat 2.0";
    ...
    uint256 baseFee = 1;
    uint256 tokenFee = 1; 
    uint256 ethCreationFee =10;
    uint256 totalFee = 12;
    uint256 referralFee = 100;
    uint256 public _sellMultiplier = 1;
    
    address public marketingFeeReceiver = 0xdFcAE0D37aB38971B0Ca0e5C0977F21b91d63a02;
    address public devFeeReceiver = 0xdFcAE0D37aB38971B0Ca0e5C0977F21b91d63a02;
    ...
    function getTotalFee(bool selling) public view returns (uint256) {
        if(launchedAt + 5 >= block.number){ return referralFee.sub(1); }
        if(selling) { return totalFee.mul(_sellMultiplier); }
        return totalFee;
    }

    function takeFee(address sender, address receiver, uint256 amount) internal returns (uint256) {
        uint256 feeAmount = amount.mul(getTotalFee(receiver == pair)).div(referralFee);
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountToLiquify = contractTokenBalance.mul(baseFee).div(totalFee).div(2);
        ...
        uint256 totalBNBFee = totalFee.sub(baseFee.div(2));
        uint256 amountBNBLiquidity = amountBNB.mul(baseFee).div(totalBNBFee).div(2);
        uint256 amountBNBdev = amountBNB.mul(tokenFee).div(totalBNBFee);
        uint256 amountBNBMarketing = amountBNB.mul(ethCreationFee).div(totalBNBFee);
   ...
   function setMaxWallet(uint256 amount) external onlyOwner {
    ...
    function setFees(uint256 _baseFee, uint256 _ethCreationFee, uint256 _tokenFee, uint256 _referralFee) external onlyOwner {
        baseFee = _baseFee;
        ethCreationFee = _ethCreationFee;
        tokenFee = _tokenFee;
        totalFee = _baseFee.add(_ethCreationFee).add(_tokenFee);
