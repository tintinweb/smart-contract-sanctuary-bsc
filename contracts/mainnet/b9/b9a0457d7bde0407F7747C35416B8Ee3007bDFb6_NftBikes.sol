...
contract NftBikes is ERC721URIStorage, AccessControl {
    ...
    mapping (uint => BikeType) public nftList;
    ...
    function mint(address _to, uint _bikeType) public onlyMinters {
        require(nftList[_bikeType].price > 0, 'This bike type can`t be minted');
        if (nftList[_bikeType].limit > 0) {
            require(nftList[_bikeType].counter.current() < nftList[_bikeType].limit, 'The limit for this bike type has been reached.');
        }
        ...
        string memory link = string(bytes.concat(bytes(url), bytes(Strings.toString(_bikeType)), '/', bytes(newId)));
    ...
    function setBikePrice(uint _bikeType, uint _price) public onlyAdmin {
        nftList[_bikeType].price = _price;
    }

    function removeBikeType(uint _bikeType) public onlyAdmin {
        delete nftList[_bikeType];
    }

    function getBikePrice(uint _bikeType) external view returns(uint) {
        require(nftList[_bikeType].price != 0, "Sorry, we don't have bike with this type id");
