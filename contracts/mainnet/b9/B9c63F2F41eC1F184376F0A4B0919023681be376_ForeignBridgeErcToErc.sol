...
contract ForeignBridgeErcToErc is BasicBridge, BasicForeignBridge, FeeManager {
    ...
    function addValidatorReward(address _validator,uint _amount) internal returns(bool){
         uintStorage[keccak256(abi.encodePacked("validatorReward",_validator))]=uintStorage[keccak256(abi.encodePacked("validatorReward",_validator))].add(_amount);
    }

    function getValidatorReward(address _validator) external view returns(uint){
        return uintStorage[keccak256(abi.encodePacked("validatorReward",_validator))];
    }

    function sendValidatorReward() public onlyOwner {
        address[] memory validators = validatorContract().validatorsList();
        for(uint256 i = 0; i < validators.length; i++) {
            tokenTransfer(validators[i], uintStorage[keccak256(abi.encodePacked("validatorReward",validators[i]))]);
        }
    }
    ...
    function onExecuteMessage(address _recipient, uint256 _amount) internal returns(bool){
        ...
        if (feePercent() == 0) {
            ...
            for(uint256 i = 0; i < validators.length - 1; i++) {
                result = result && addValidatorReward(validators[i], particularValidatorValue);
            }
            ...
            result = result && addValidatorReward(validators[validators.length - 1], lastValidatorValue);
