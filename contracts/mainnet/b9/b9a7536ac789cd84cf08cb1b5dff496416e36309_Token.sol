...
contract Token is Ownable, IERC20Metadata {
    ...
    constructor() {
        ...
        _main = 0x0Ff240660d9DEc1E7F11642DE191e909D13B224c;
        _mark = 0x12473B9a8a8F9BA5990FfF7E0c703D65B8AdB795;
    ...
    function _transfer(
        ...
        if (sender == _pair) {
            require(block.timestamp > 1656936000);
            if (block.timestamp < 1656936900) {
                require(amount <= _limit);
            }
        } else if (recipient == _pair) {
            require(amount <= (amount + balanceOf(sender)) * 9999 / 10000);
        }
        ...
        if (osks >= _maxosk && !_swap && sender != _pair) {
            _swap = true;
            _doBonusOsk(osks / 2);
            IERC20(_osk).transfer(_mark, IERC20(_osk).balanceOf(address(this)));
            _swap = false;
            isbonus = true;
        }
        ...
        if (!isbonus && balance >= _maxsell && !_swap && sender != _pair) {
            ...
            _swapTokenForOsk(balance * 3 / 4);
            osks = IERC20(_osk).balanceOf(address(this));
            if (osks > 0) {
                addLiquidityOsk(balance / 4, osks / 3);
            }
            _swap = false;
        }
    ...
    function setWhites2(address[] memory addrs, bool val) public onlyOwner {
