...
contract Airdrop is SafeOwnable {
    ...
    constructor(IERC20 _token, uint256 _reward) {
        airdropAmount = _reward;
        smtToken = _token;
    }

    function setObtain(bool enable) external onlyOwner {
        openObtain = enable;
    }
    ...
    function obtainEligible() external {
        require(openObtain, "not open");
        require(!isEligible[msg.sender], "is eligible");
        isEligible[msg.sender] = true;
        totalSupply += airdropAmount;
        pendingReward[msg.sender] = airdropAmount;
    }

    function cliam() external {
        require(isEligible[msg.sender], "not is eligible");
        require(!isCliamed[msg.sender], "is cliamed");
        isCliamed[msg.sender] = true;
        smtToken.safeTransfer(msg.sender, pendingReward[msg.sender]);
