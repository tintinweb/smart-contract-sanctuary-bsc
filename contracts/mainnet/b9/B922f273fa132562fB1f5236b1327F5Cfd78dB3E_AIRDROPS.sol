...
// File contracts/Mining/Airdrops.sol
...
contract AIRDROPS is Pausable, Ownable {
    ...
    bool private active;
    mapping(address => tokenData) private listedTokens;
    ...
    function addAirdrop(address _addr,uint256 _supply,uint256 _split,bool _active)
        public
        onlyOwner
        whenNotPaused
        returns (bool)
    {
        require(_split > 0, "Splits should be > 0");
        ERC20 token = ERC20(_addr);
        string memory name = token.name();
        string memory symbol = token.symbol();
        uint256 balance = token.balanceOf(address(this));
        require(_supply <= balance, "Supply should be <= balance");
        listedTokens[_addr] = tokenData(_addr, name, symbol, _supply ,balance, _split, 0, _active);
        ...
        )
    {
        ...
        _supply = listedTokens[addr].balance;
        _balance = listedTokens[addr].balance;
        _split = listedTokens[addr].splitDays;
    ...
    function getBalances () public onlyOwner view returns (tokenBalanceData[] memory) {
    ...
    function claimTokens (address addr, uint32 ratio) public whenNotPaused onlyAllowedContract returns (bool) {
        ...
        for (uint256 index = 0; index < allTokens.length; index++) {
            ...
            if(tkn.active == true){
                uint256 part = tkn.supply/tkn.splitDays;
                uint256 amount = (part * ratio)/ratio_multiplier;
                ERC20(tkn.addr).transfer(addr, amount);
                tkn.claimed = tkn.claimed + amount;
                emit claimEvent(addr, tkn.addr, amount);
            }
        }
        return true;
    }
