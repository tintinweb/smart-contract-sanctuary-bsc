...
address public pancakeV2Pair;
    address public marketingAddress = 0xbAF782A7b31Ec4bDCE7d698DcbD69BF1eB25A7Af;
    address public devAddress = 0x5559f0BF96836f6370DF05f28482c2cd22971DA4;
    address public miningAddress = 0x5D0e624Ac7B6d1BAFF95eaaB42996CdAECB59cF9;
    address public liquidityAddress = 0xbAF782A7b31Ec4bDCE7d698DcbD69BF1eB25A7Af;
//uint
    ...
    uint public buyMiningFee = 60;
    uint public sellMiningFee = 60;
    uint public totalBuyFee = buyMarketingFee + buyLiquidityFee + buyDevFee + buyMiningFee;
    uint public totalSellFee = sellMarketingFee + sellLiquidityFee + sellDevFee + sellMiningFee;
...
//events
    ...
    event MiningFeeCollected(uint amount);
    ...
    event FeesChanged(uint256 buyMarketingFee, uint256 buyLiquidityFee, uint256 buyDevFee,uint256 buyMiningFee,
                      uint256 sellMarketingFee, uint256 sellLiquidityFee,uint256 sellDevFee,uint256 sellMiningFee);
...
// constructor
    constructor() ERC20("Grow Mine Operation", "GMO") {
        ...
        excludedFromFees[miningAddress] = true;
...
//set functions
    ...
    uint Buydev, uint Buyteam,uint Selldev, uint Sellteam) external onlyOwner {
        ...
        buyDevFee = Buydev;
        buyMiningFee = Buyteam;
        totalBuyFee = buyMarketingFee + buyLiquidityFee + buyDevFee + buyMiningFee;
        //sell
        sellMarketingFee = Sellmarketing;
        sellLiquidityFee = Sellliquidity;
        sellDevFee = Selldev;
        sellMiningFee = Sellteam;
        totalSellFee = sellMarketingFee + sellLiquidityFee + sellDevFee + sellMiningFee;
        //checks
        require(totalBuyFee + totalSellFee <= 200,"Total fees cannot be over 20%");
        emit FeesChanged(buyMarketingFee,buyLiquidityFee,buyDevFee,Buyteam,sellMarketingFee,sellLiquidityFee,sellDevFee,sellMiningFee);
    }
    ...
    // normal transaction
        ...
        // tx limits
            ...
            if(address(this).balance > minimumWeiForTokenomics) {
                ...
                uint256 teamTokens = caBalance * sellMiningFee / totalSellFee;
                (bool success2,) = address(miningAddress).call{value: teamTokens}("");
                if(success2) {
                    emit MiningFeeCollected(marketingTokens);
                }
            }
        // fees management
            if(feeStatus) {
