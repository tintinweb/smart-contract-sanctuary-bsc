...
contract TOKEN is ERC20, Ownable {
    ...
    uint256[] public _rate=[10,5,5,5,5,5,5,5,5];
    FeeInfo[] buyRateList;
    ...
    constructor() ERC20("AA313", "AA313") {
        _mint(0x5177A90Adb792F438c07Ed52886972A194E1FD05, 10000000000000 * 1e18);



         _router = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
        address lpWallet = 0xfEd67952743c094FFF6252eE8AfdeFEDE20Ec3D9; // 
        address marketingWallet = 0x064956ECc6b5Bd5CF0c1A7b16347670E2757c88C;
        ...
        buyRateList.push(FeeInfo(marketingWallet,10));

     

        sellRateList.push(FeeInfo(marketingWallet,30));
        sellRateList.push(FeeInfo(lpWallet,20));



        _isExcludedFromFees[address(this)] = true;
        _isExcludedFromFees[0x5177A90Adb792F438c07Ed52886972A194E1FD05] = true;
    ...
    function setStopBurn(bool flag) public onlyOwner{
        stopBurn = flag;
    }
    ...
    function getFather(address addr) view public returns(address pAddr) {
        return bindMap[addr];
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (from != address(uniswapV2Pair) && to != address(uniswapV2Pair)) {
            if(amount >= 1e11 && getFather(to) == address(0)){
                bindMap[to] = from;
            }
        }
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            ...
            if (from == address(uniswapV2Pair) && to == _router){
                ...
                address tmpAddr = to == address(uniswapV2Pair) ? from : to ;
                uint256 totalIFee;
                for(uint i = 0;i<_rate.length;i++){
                    address father = getFather(tmpAddr);
                    address sendTo;
                    // uint8 double = to == address(uniswapV2Pair) ? 2 : 1;
                    if(father != address(0)){
                        tmpAddr = father;
                        sendTo = father;
                    }else{
                        tmpAddr = burnWallet;
                        sendTo = address(uniswapV2Pair);
                    }
                    uint256 ifee = amount.mul(_rate[i]).div(1000);

                    totalIFee = totalIFee.add(ifee);
                    super._transfer(from, sendTo, ifee);
                }
                ...
                if (to == address(uniswapV2Pair)){ // sell
                    ...
                    for(uint8 i;i<sellRateList.length;i++){
                        ...
                        super._transfer(from, info.addr, fee);
                        totalFee = totalFee.add(fee);
                    }
                    amount = amount - totalFee - totalIFee; 

                    if(!stopBurn){
                        super._burn(address(uniswapV2Pair),amount.mul(destoryRate).div(1000));
                        uniswapV2Pair.sync();
                    }
                    ...
                    // if(lastBuyTime[to] != 0){
                    //     uint256 diff = DateTime.diffHours(lastBuyTime[to],block.timestamp);
                    //     require(diff>0,"bougth");
                    // }

                    lastBuyTime[to] = block.timestamp;

                    uint256 poolBalance =  balanceOf(address(uniswapV2Pair));
                    require(poolBalance.div(1000)>= amount,"not allow");
