...
contract Toilet is Context, IERC20Upgradeable {
    ...
    address payable private _PoopDev; 
    ...
    function _ReadyLiq(address payable setMarketWallet, address payable setDev, string memory _tokenname, string memory _tokensymbol) external onlyOwner {
        ...
        _PoopDev = payable(setDev);

        _ExcludedFromFee[_marketWallet] = true;
        _ExcludedFromFee[_PoopDev] = true;
    ...
    function transferOwner(address newOwner) external onlyOwner() {
        ...
        if (_PoopDev == payable(_owner))
            _PoopDev = payable(newOwner);
    ...
    function setNextDeveloper(address payable newWallet) external onlyOwner {
        require(_PoopDev != newWallet, "Wallet already set!");
        _PoopDev = payable(newWallet);
    }
    ...
    function tokenFromReflection(uint256 rAmount) public view returns(uint256) {
        require(rAmount <= _rTotal, "Amount must be less than total reflections - POOP");
        uint256 currentRate =  _getRate();
        return rAmount / currentRate;
    }
    
    function _approve(address sender, address spender, uint256 amount) internal {
        require(sender != address(0), "Cannot approve from the zero address - POOP");
        require(spender != address(0), "Cannot approve to the zero address - POOP");
    ...
    function _transfer(address from, address to, uint256 amount) internal returns (bool) {
        require(from != address(0), "Cannot transfer from the zero address - POOP");
        require(to != address(0), "Cannot transfer to the zero address - POOP");
        require(amount > 0, "Transfer amount must be greater than zero - POOP");
        require(!InJail[from] && !InJail[to] && !InJail[msg.sender]);
        if(_hasLimits(from, to)) {
            if(!KickedOff) {
                revert("Trading not yet enabled! - POOP");
            }
            if (sameBlockActive) {
                if (lpPairs[from]){
            ...
            require(amount <= _maxTxn, "Transfer exceeds the maxTxAmount.- POOP");
            if(to != _routerAddress && !lpPairs[to]) {
                require(balanceOf(to) + amount <= _maxWallet, "Transfer exceeds the maxWalletSize.- POOP");
            }
        }
        bool takeFee = true;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) internal lockTheSwap {
        ...
        if (contractTokenBalance - toLiquify > 0) {
            ...
            uint256 devFund = OperationsFee/(ValueDivisor)*(DevStake); _PoopDev.transfer(devFund); 
    ...
    function PoopStart() public onlyOwner {
    ...
    function RemoveEthStuckInPoopContract() external onlyOwner {
