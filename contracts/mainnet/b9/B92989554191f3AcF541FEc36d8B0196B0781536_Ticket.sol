...
contract Ticket is ITicket, ERC721, ERC721Enumerable, SafeTransfer, Ownable, Pausable {
	...
	function setWhitelist(address[] memory _accounts) public onlyOwner {
		...
		for (uint256 i = 0; i < _accounts.length; i++) {
			require(!_members.contains(_accounts[i]), "Ticket: existed account");
	...
	function safeMint(address to) public onlyOwner {
		require(_tokenIdSell <= (MAX_SUPPLY - SUPPLY_RESERVE), "Ticket: exceed supply");
	...
	function claimCard() public payable whenNotPaused {
		...
		if (senderInfo.isMember && !senderInfo.claimed) {
			require(currentIdAirdrop <= SUPPLY_AIRDROP, "Ticket: exceed airdrop");
			...
			require(currentIdSell <= (MAX_SUPPLY - SUPPLY_RESERVE), "Ticket: exceed supply");
			require(msg.value >= cardPrice, "Ticket: price invalid");
