...
contract SAPPLaunchPad is Ownable, ReentrancyGuard {
    ...
    //SAPPNftMarketplace
    address public SAPPNftMarketplace;
    ...
    constructor(address _SAPPNftMarketplace) {
        require(_SAPPNftMarketplace!= address(0),"Zero address");
        require(_SAPPNftMarketplace.isContract(),"Not contract");
        SAPPNftMarketplace = _SAPPNftMarketplace;
    }
    ...
    function addLaunchPad(
        ...
        require(ISAPPNftMarketplace(SAPPNftMarketplace).isOwnerOfRoot(_rootId,msg.sender),"Bad root token");
    ...
    function setWinners(uint24 launchpadId, address[] memory _winners) public onlyOwner() launchpadNotFound(launchpadId) {
        ...
        require(_winners.length <= ISAPPNftMarketplace(SAPPNftMarketplace).getMaxFragment(),"Too many fragment");
    ...
    function claim(uint24 launchpadId) external nonReentrant onlyEOA{
        ...
        ISAPPNftMarketplace(SAPPNftMarketplace).mintFragment(_msgSender(), launchpad.rootIdToken);
    ...
    function setAddressMarketplace(address _SAPPNftMarketplace) public onlyOwner(){
        require(_SAPPNftMarketplace!= address(0),"Zero address");
        require(_SAPPNftMarketplace.isContract(),"Not contract");
        SAPPNftMarketplace = _SAPPNftMarketplace;
    }
    ...
    function deleteLaunchpad(uint24 launchpadId) external onlyOwner() launchpadNotFound(launchpadId){
        ...
        ISAPPNftMarketplace(SAPPNftMarketplace).burnByLaunchpad(owner(),launchpad.rootIdToken);
        //set disable
        launchpadDisabled[launchpadId] = true;
        // set Emegency
        if(launchpad.startTime < block.timestamp && block.timestamp < launchpad.endTime){
