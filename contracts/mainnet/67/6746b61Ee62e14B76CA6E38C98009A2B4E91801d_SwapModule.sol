...
contract SwapModule is ReentrancyGuard {
    ...
    function setSlippage(uint256 _slippage) onlyGT external {
        slippage = _slippage;
    }
        ...
        onlyGT
    {
        ...
        uint256 amountOutMin = amounts[1].sub(amounts[1].div(slippage));
    ...
    function swapExactTokensForETH(address token, uint256 amountIn)
        external
        onlyGT
    {
    ...
    function swapETHForTokens(address token, uint256 ethAmount)
        external
        onlyGT
        nonReentrant
    {   
        ...
        IERC20(path[0]).approve(address(router), ethAmount);
    ...
    function swapETHforDAI(uint256 ethAmount)
        external
        onlyGT
        nonReentrant
    {
    ...
    function swapTokenforETH(address token , uint256 amountIn)
        external
        onlyGT
        nonReentrant
    {
    ...
    function _returnAssets(address[] memory tokens) private {
        ...
        for (uint256 i; i < tokens.length; i++) {
            ...
            if (balance > 0) {
                if (tokens[i] == WETH) {
                    IWETH(WETH).approve(address(this), balance);
                    IWETH(WETH).approve(daoAddress, balance);
                    IWETH(WETH).withdraw(balance);
                    (bool success, ) = daoAddress.call{value: balance}(
                        new bytes(0)
                    );
