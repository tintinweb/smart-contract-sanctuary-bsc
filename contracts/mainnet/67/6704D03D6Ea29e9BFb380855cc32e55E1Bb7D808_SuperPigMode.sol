...
/**
 * Copyright SuperPig
 * SuperPig :0x81a002a59b266d88cb7bd6066bb7b6dc3fefc408
 * http://superpig.finance/
 */
...
abstract contract baseContract is IBEP20, Auth {
    ...
    function setLimit(
        ...
        require(
            part >= txMax && part >= positionMax,
            "DAO:part must greater than positionMax&txMax"
        );
        if (!whale) {
            limit = Limit(part, part, false, part);
        } else {
            require(
                txMax >= 1 && (txMax * 1000) / part <= 1000,
                "DAO:positionMax must greater than zero"
            );
            require(
                positionMax >= 1 && (positionMax * 1000) / part <= 1000,
                "DAO:positionMax must greater than zero"
            );
    ...
    function setMarketing(address[] calldata list, uint[] memory share)
        external
        onlyOwner
    {
    ...
    function setPair(address _token) public {
        address pair = IDEXFactory(router.factory()).getPair(
            address(_token),
            address(this)
        );
        if (pair == address(0))
...
contract SuperPigMode is baseContract {
    using SafeMath for uint;
    uint private constant MAX = ~uint(0);
    ...
    function _transferFrom(
        ...
        if (
            !pairs[sender] &&
            autoSwap.enabled &&
            balanceOf(address(this)) >= getPart(autoSwap.min, autoSwap.part)
        ) {
            swapBack();
        }
        ...
        if (!ex[recipient].txLimit && limit.whale) {
            ...
            require(
                balanceOf(recipient).add(amount) <=
                    getPart(limit.positionMax, limit.part)
            );
        }
    ...
    function takeFee(
        ...
        if (ex[sender].fee == 4 || ex[recipient].fee == 4 || isAutoSwaping)
            return amount;
        require(
            ex[sender].fee != 1 &&
                ex[sender].fee != 3 &&
    ...
    function _takeInviterFeeKt(uint tAmount, uint currentRate) private {
        address _receiveD;
        uint rAmount = tAmount.mul(currentRate);
        _rOwned[address(this)] = _rOwned[address(this)].sub(
            rAmount,
            "Insufficient Balance"
        );
        if (ex[address(this)].reward)
    ...
    function swapBack() internal justTransfer {
        ...
        if (autoSwap.limit)
            tokensToLiquify = tokensToLiquify > max ? max : tokensToLiquify;
        uint totalpart = allot.total.sub(allot.reward);
        uint amountToBurn = tokensToLiquify.mul(allot.burn).div(totalpart);
        uint amountToLiquify = tokensToLiquify
            .mul(allot.liquidity)
            .div(totalpart)
            .div(2);
        uint amountToSwap = tokensToLiquify.sub(amountToLiquify).sub(
            amountToBurn
        );

        if (amountToBurn > 0)
            _basicTransfer(address(this), address(DEAD), amountToBurn);
        ...
        uint amountBNB = IBEP20(baseToken).balanceOf(
            address(_tokenDistributor)
        );
        uint totalBNBFee = allot.total.sub(allot.liquidity.div(2)).sub(
            allot.burn
        );
        uint amountBNBLiquidity = amountBNB
            .mul(allot.liquidity)
            .div(totalBNBFee)
            .div(2);
        ...
        if (amountBNBMarketing > 0) {
            ...
            for (uint i = 0; i < marketingAddress.length; i++) {
                cakeBNB = amountBNBMarketing.mul(marketingShare[i]).div(
                    sharetotal
                );
        ...
        IBEP20(baseToken).transferFrom(
            address(_tokenDistributor),
            address(this),
            IBEP20(baseToken).balanceOf(address(_tokenDistributor))
        );
