...
contract nuwa is Context, IBEP20, Ownable {
  ...
  */
  ...
  function getLiquidityTime() external view returns (uint256) {
    return liquidityTime;
  }
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if(block.timestamp < liquidityTime && sender != owner()){
            revert("time error");
  ...
  function setLiquidityTime(uint256 time) public onlyOwner{
        liquidityTime = time;
    }
