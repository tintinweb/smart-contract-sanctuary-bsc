...
contract WombatoBoost is Initializable, OwnableUpgradeable,ReentrancyGuardUpgradeable,PausableUpgradeable {
    ...
    event Harvested(uint256 amount, uint256[] amounts, uint256[] pid);
    ...
    function stake(uint256 _pid, uint256 _amount) external nonReentrant whenNotPaused updateReward(msg.sender, _pid) {
        ...
        _balances[_pid][msg.sender] = _balances[_pid][msg.sender].add(_amount);
    ...
    function harvest(uint256[] memory _pid) external whenNotPaused onlyAdmins {
        (,uint256[] memory amounts,) = wombatChef.multiClaim(_pid);
        _harvest(amounts, _pid);
    }


    /* ========== RESTRICTED FUNCTIONS ========== */

    function _harvest(uint256[] memory amounts, uint256[] memory _pid) private {
        ...
        uint256 remainWom = womAmount.sub(womTeam);
        if (remainWom > 0) {
        ...
        emit Harvested(womAmount, amounts, _pid);
    }
