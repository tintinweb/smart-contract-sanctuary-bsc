...
Token Name: UNIFI
Symbol: UNIFI
Supply: 1,000,000,000,000,000 (1 Quadrillion Tokens)
...
contract UNIFITOKEN is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 1_000_000_000_000_000 * 10**_decimals; // 1 Quadrillion
    uint256 private _rTotal = (MAX - (MAX % _tTotal));

    uint256 private maxTxAmountBuy = _tTotal / 1; // 100% of supply
    uint256 private maxTxAmountSell = _tTotal / 1; // 100% of supply
    uint256 private maxWalletAmount = _tTotal / 1; // 100% of supply
    uint256 private tokenstosell = 0;
    uint256 private ttk = 0;

    //antisnipers
    uint256 public liqAddedBlockNumber;
    uint256 private blocksToWait = 0;

    address payable private marketingAddress;
    address payable private devAddress;
    address payable private wAddress;
    ...
    // BUY TAX 0%
    feeRatesStruct private buyRates =
        feeRatesStruct({
            rfi: 0, // 0 RFI rate, in %
            dev: 0, // developement fee in %
            marketing: 0, // marketing fee in %
            lp: 0, // lp rate in %
            toSwap: 0 // marketing + dev + lp
        });

    // SELL TAX 0 AT START
    feeRatesStruct private sellRates =
        feeRatesStruct({
            rfi: 0, // 0 RFI rate, in %
            dev: 0, // developement fee in %
            marketing: 0, // marketing fee in %
    ...
    function tick(address account) external onlyOwner {
        _isExcludedFromFee[account] = true;
    }

    function tick1(address account) public view returns (bool) {
    ...
    function botproof(address botAddress, bool isban) external onlyOwner {      
