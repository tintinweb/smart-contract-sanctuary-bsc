...
contract Sharddrop is Context,Ownable {
  ...
  mapping (uint256 => uint256) public _droppedBNB;
  mapping (uint256 => address) public _droppedAccount;
  mapping (uint256 => uint256) public _droppedTokenid;
  ...
  function dropBNB() internal {
    if(address(this).balance>amountdrop){
      ...
      uint256 ranamount = _createRandomNum(10000);
      if(ran>0){
        address recipient = a.ownerOf(ran);
        (bool success, ) = recipient.call{ value : amountdrop.mul(ranamount).div(10000) }("");
        require(success,"transfer fail!");
        dropid = dropid.add(1);
        _droppedBNB[dropid] = amountdrop.mul(ranamount).div(10000);
        _droppedAccount[dropid] = recipient;
        _droppedTokenid[dropid] = ran;
      }
    }
  }
