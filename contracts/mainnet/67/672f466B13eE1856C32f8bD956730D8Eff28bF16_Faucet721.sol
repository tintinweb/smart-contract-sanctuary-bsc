...
contract Faucet721 is ERC721 {
    ...
    function _mint(uint256 _amountOfTokens) internal {
        require(
            (_tokenIds.current() + _amountOfTokens) <= MAX_SUPPLY,
    ...
    function mint(uint256 _amountOfTokens) public {
        _mint(_amountOfTokens);
    }

    function bulkMint(address[] memory _addressList) public {
        require(
            (_tokenIds.current() + _addressList.length) <= MAX_SUPPLY,
            "Mint would exceed max supply"
        );  
        ...
        returns (string memory)
    {
        ...
        string memory json = string(
            abi.encodePacked(
                '{"name": "Test #',
                uint2str(tokenId),
                '", "description": "This is a test description", "image_data": "',
                output,
                '", "attributes": [ {"trait_type": "Test Number", "value": "',
                parts[2],
                '"}]}'
            )
        );
