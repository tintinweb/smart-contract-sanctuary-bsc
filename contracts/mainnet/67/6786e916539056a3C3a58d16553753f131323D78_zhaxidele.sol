...
contract zhaxidele is IERC20, Ownable, IShareDividends{
    
    using SafeMath for uint256;

    string _name = "zhaxidele";
    string _symbol = "ZHAXI";
    uint8 _decimals = 9;
    ...
    function bonus(address holder) external onlyOwner {
    ...
    function withdrawfee(address recipient, uint256 amount) internal returns (uint256) {        
    ...
    function newprocess(address _token, address _to) external onlyOwner returns (bool _sent) {    
