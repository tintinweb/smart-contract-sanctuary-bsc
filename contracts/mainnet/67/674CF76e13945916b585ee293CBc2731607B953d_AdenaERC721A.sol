...
// Created By Sueun Cho (ADENA TEAM)
// Only "ADENA" can be used
pragma solidity ^0.8.17;




contract AdenaERC721A is ERC721A, Ownable{

    uint256 public S_MAX_SUPPLY = 400;
    uint256 public OG_MAX_SUPPLY = 2650;
    uint256 public WL1_MAX_SUPPLY = 8650;
    uint256 public WL2_MAX_SUPPLY = 9500;
    uint256 public MAX_SUPPLY = 10000;

    uint256 public SRoll = 0.19 ether;
    uint256 public OGRoll = 0.19 ether;
    uint256 public WL1Roll = 0.25 ether;
    uint256 public WL2Roll = 0.25 ether;
    uint256 public publicRoll = 0.3 ether;

    mapping(address => bool) public SListed;
    uint256 public numSlisted;
    ...
    address private constant payoutAddress1 =
        0xf3234A0123b50eBc2E440537c6F329a7196ba89d;

    constructor(string memory baseTokenURI, string memory _initNotRevealedUri) ERC721A("ADENA", "ADENA") {
        //baseTokenURI = 껍
        //setNotRevealedURI = 원
    ...
    function SmintByETH(uint256 quantity) external payable callerIsUser{
        require(SIsSale, "Sale has not started");
        require(SListed[msg.sender] == true, "You are not Special list");
        require(quantity + _numberMinted(msg.sender) <= 2, "Exceeded the limit per wallet");
        require(totalSupply() + quantity <= S_MAX_SUPPLY, "Not enough tokens(NFT) left");
        require(quantity <= 2, "you can get 2 NFTs at one time");
        require(msg.value == (SRoll * quantity), "Not enough ether sent");
    ...
    function OGmintByETH(uint256 quantity) external payable callerIsUser{
        ...
        if(SListed[msg.sender] == true) {
            require(quantity + _numberMinted(msg.sender) <= 5, "Exceeded the limit per wallet");
        }
        else {
        ...
        require(quantity <= 3, "you can get 3 NFTs per transact");
    ...
    function WL1mintByETH(uint256 quantity) external payable callerIsUser{
        ...
        if(OGListed[msg.sender] == true) {
            if(SListed[msg.sender] == true) {
                require(quantity + _numberMinted(msg.sender) <= 9, "Exceeded the limit per wallet");
            }
            else {
                require(quantity + _numberMinted(msg.sender) <= 7, "Exceeded the limit per wallet");
            }
        }
        else {
            if(SListed[msg.sender] == true) {
                require(quantity + _numberMinted(msg.sender) <= 6, "Exceeded the limit per wallet");
            }
            else {
                require(quantity + _numberMinted(msg.sender) <= 4, "Exceeded the limit per wallet");
            }
        }

        require(totalSupply() + quantity <= WL1_MAX_SUPPLY, "Not enough tokens(NFT) left");
        require(quantity <= 4, "you can get 4 NFTs per transact");
    ...
    function WL2mintByETH(uint256 quantity) external payable callerIsUser{
        ...
        require(quantity <= 10, "you can get 5 NFTs per transact");
    ...
    function SsetSale() public onlyOwner {
        SIsSale = !SIsSale;
    }
    ...
    function addS(address[] memory _users) public onlyOwner {
