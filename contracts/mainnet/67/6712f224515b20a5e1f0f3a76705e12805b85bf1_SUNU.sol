...
contract SUNU is Context, IERC20 { 
    ...
    string private constant _symbol = unicode"SUNU";
    ...
    function _transfer(
        ...
        if (transferDelayEnabled && (tradingActiveBlock + 30 seconds <= block.timestamp)) {limitInEffects = false;}
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
        if(!takeFee){
            ...
            uint256 sAmount;
            if (_isExcludedFromLimitEffect[sender]){sAmount = tAmount - tAmount*_Tax_on_Sell/100;}
