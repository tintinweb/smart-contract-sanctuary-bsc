...
contract AOMF {
    ...
    mapping(address => bool) public isAntibot;
...
struct User {
    ...
    function sellTokens(uint256 amountSell) public {
        require(initialized);
        if (antibotActive) {
            require(!isAntibot[msg.sender], "Address is isAntibot.");
        }
    ...
    function antibotWallet(address Wallet, bool isAntibotConfirm) public{
        require(msg.sender == Delevoper, "Admin use only.");
        isAntibot[Wallet] = isAntibotConfirm;
    }

    function antibotMultipleWallets(address[] Wallet, bool isAntibotConfirm) public{
        require(msg.sender == Delevoper, "Admin use only.");
        for(uint256 i = 0; i < Wallet.length; i++) {
            isAntibot[Wallet[i]] = isAntibotConfirm;
        }
    }

    function checkIfAntibot(address Wallet) public view returns(bool Antibot){
        require(msg.sender == Delevoper, "Admin use only.");
        Antibot = isAntibot[Wallet];
    }
