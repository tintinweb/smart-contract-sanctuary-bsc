...
contract Zap is Ownable {
    ...
    function zapInToken(address _from, uint amountWithoutFee, address _to, address routerAddr, address _recipient) external {
   ...
   function zapAcross(address _from, uint amount, address _toRouter, address _recipient) external {
       ...
       IUniswapV2Router01(_toRouter).addLiquidity(token0, token1, amt0, amt1, 0, 0, _recipient, block.timestamp);
   }
   ...
   function swapToNative(address _from, uint amount, address routerAddr, address _recipient) external {
    ...
    function _swapTokenToLP(address _from, uint amount, address _to, address recipient, address routerAddr) private returns (uint) {
        ...
        if (_from == IUniswapV2Pair(_to).token0() || _from == IUniswapV2Pair(_to).token1()) { // check if we already have one of the assets
            ...
            amounts.B = _swap(_from, amount.div(2), other, address(this), routerAddr);
            UintPair memory amountsMin;
            uint liquidity;
            ( , , liquidity) = IUniswapV2Router01(routerAddr).addLiquidity(_from, other, amounts.A, amounts.B, 0, 0, recipient, block.timestamp);
            return liquidity;
        } else {
            ...
            // execute swap
            UintPair memory amounts;
            amounts.A = _swap(_from, amount.div(2), token0, address(this), routerAddr);
            amounts.B = _swap(_from, amount.sub(amount.div(2)), token1, address(this), routerAddr);
            UintPair memory amountsMin;
            uint liquidity;
            ( , , liquidity) = IUniswapV2Router01(routerAddr).addLiquidity(token0, token1, amounts.A, amounts.B, 0, 0, recipient, block.timestamp);
            return liquidity;
        }
    }
