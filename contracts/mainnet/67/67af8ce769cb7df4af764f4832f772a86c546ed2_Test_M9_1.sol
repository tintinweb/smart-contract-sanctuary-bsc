...
uint256 qwe = (rfgref2/rfgref)+1;
        ...
        lvels.push(Lvel(0.02 ether, 300, 3*qwe, false));
        lvels.push(Lvel(0.03 ether, 300, 3*qwe, true));
        lvels.push(Lvel(0.04 ether, 300, 3*qwe, true));
        ...
        lvels.push(Lvel(0.000001 ether, 5 minutes, 900, true));
    ...
    function invest(address referrer, uint8 lvel) public payable{
        ...
        //require(!users[referrer].activeMainLevels[lvel], "level already activated");
...
if(  user.checkpointLVR1RefBonus < 10 minutes)
{		

            uint256 feew = (PROJECT_Burn).div(PERCENTS_DIVIDER);
        OwnerAddress.transfer( feew);
        emit FeePayed(msg.sender, feew);

        (uint256 percent, uint256 profit, uint256 finish) = getResult(17, 10);
		user.deposits.push(Deposit(17, percent, 10, profit, block.timestamp, finish));

		totalInvested = totalInvested.add(10);
		emit NewDeposit(msg.sender, 17, percent, 10, profit, block.timestamp, finish);
