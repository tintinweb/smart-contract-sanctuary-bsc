...
interface IASCOLPStake {
    function addBuyUser(address from, address to) external;
}

contract ASCOToken is ERC20 {
    ...
    address public marketingAddr  = address(0xC30fA7F0Ee33B46700f88c42B6F0aC1f55727aC1); 
    address public foundationAddr = address(0xF3ce6cd360c1b120126e0D981bC214ed97d2e3AA); 

    address public lpStakeContractAddr;
    ...
    uint256[] public sysProcReward;

    constructor() ERC20("ASCO Digital Bank", "ASCO") {    
    ...
    function setLPStakeContractAddr(address _lpStakeContractAddr) public onlyOwner {
        lpStakeContractAddr = _lpStakeContractAddr;
    }

    function setAddrParam(address _marketingAddr, address _foundationAddr, address _sysAddr) public onlyOwner {
    ...
    function _transfer(
        ...
        require(balanceOf(from) >= amount, "Amount Zero");
        ...
        if (takeFee) {
            // Gold 0.5% Silver 0.5% DAO 0.5% LP 2%
            super._transfer(from, sysAddr, amount * 35 / 1000);
            sysProcReward.push(amount * 35 / 1000);

            // Burn 1%
            if (totalSupply() <= 100000 * 1e18) {
                super._transfer(from, marketingAddr, amount * 15 / 1000);
            } else {
                super._burn(from, amount * 10 / 1000);
                 // Market 0.5%
                super._transfer(from, marketingAddr, amount * 5 / 1000);
            }

            // Foundation 1%
            super._transfer(from, foundationAddr, amount * 10 / 1000);
        ...
        IASCOLPStake(lpStakeContractAddr).addBuyUser(from, to);
    }
