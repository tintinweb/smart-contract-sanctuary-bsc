...
contract PrivateSale is Owned {
    ...
    uint256 public tokenRatePerEth = 300;
    uint256 public minETHLimit = 0.1 ether;
    uint256 public maxETHLimit = 20 ether;
    ...
    function deposit() public payable checkIfPaused {
        ...
        require(IDOT.transfer(msg.sender, tokenAmount), "Insufficient balance of presale contract!");
    ...
    function getTokensPerEth(uint256 amount) internal view returns(uint256) {
        return amount.mul(tokenRatePerEth).div(10**(uint256(18).sub(IDOT.decimals())));
    }
}