...
contract RocketRabbit is ERC20, Ownable {
    ...
    constructor(
        ...
        address owner,
        address mintToAddr
    ) ERC20('RocketRabbit', 'RRC') {
        require(
            feeAccount != address(0),
            'RocketRabbit: fee account from the zero address'
        );
        require(feePercent <= PRECISION, 'RocketRabbit: incorrect fee percent');
        require(owner != address(0), 'RocketRabbit: incorrect owner address');
        require(mintToAddr != address(0), 'RocketRabbit: incorrect mintToAddr address');
        if (_msgSender() != owner) {
    ...
    function _transfer(
        ...
        require(sender != address(0), 'RocketRabbit: transfer from the zero address');
        require(recipient != address(0), 'RocketRabbit: transfer to the zero address');
