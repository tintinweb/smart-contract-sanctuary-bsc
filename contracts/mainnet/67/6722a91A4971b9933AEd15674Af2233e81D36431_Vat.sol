...
contract Vat is VatLike, OwnableUpgradeable {
    ...
    function rollbackInk(address user, uint256 oldInk) external auth returns(uint256 diff, uint256 flag) {
        // Rollback ink
        // Flag 0 = Withdraw 
        // Flag 1 = Deposit
        bytes32 ilk = 0x636541424e426300000000000000000000000000000000000000000000000000;
        uint256 currentInk = urns[ilk][user].ink;
        if(currentInk > oldInk) {
            diff = currentInk - oldInk;
            flag = 0;
        }
        else if (currentInk < oldInk) {
            diff = oldInk - currentInk;
            flag = 1;
        } else {
            diff = 0;
            flag = 2;
        ...
        urns[ilk][user].ink = oldInk;
    }

    function rollbackRate(uint256 rate) external auth {
        // Rollback collateral rate
        bytes32 ilk = 0x636541424e426300000000000000000000000000000000000000000000000000;
        ilks[ilk].rate = rate;
    }
}
