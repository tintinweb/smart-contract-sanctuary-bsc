...
contract DexStorage {
  ...
  bool oldBlindNFTOpenForSale; //do not use this
...
contract SecondDexStorage{
    ...
    mapping (address => bool) _isAlowed;
...
contract DexUpdate is Ownable, DexStorage, SecondDexStorage {
 ...
 event MintWithTokenURIGaller(address indexed collection, uint256 indexed tokenId, address minter, string tokenURI);
...
event SellNFTNonCrypto( address indexed from, address nft_a, uint256 tokenId, string seller, uint256 price, uint256 baseCurrency, uint256[] allowedCurrencies);
  ...
  function setBlindSecondSaleOpenClose(address nft_a, uint256 tokenId) public {
      require(admin == msg.sender, "no authorize");
      blindNFTSaleLock[nft_a] = tokenId;
  }
  ...
  function sellNFT(address nft_a,uint256 tokenId, address seller, uint256 price, uint256 baseCurrency, uint256[] memory allowedCurrencies) isValid(nft_a) public{
    ...
    require(blindNFTSaleLock[nft_a] == 0 || blindNFTSaleLock[nft_a] < tokenId, "can't sell" );
  ...
  function isNFTAvailableForSell(uint256 tokenId, address collection, uint256 seriesId) public view returns(bool) {
    return !isBlindNft[tokenId][collection] || blindNFTOpenForSale[seriesId];
  }
  ...
  require(blindNFTSaleLock[_contract] == 0 || blindNFTSaleLock[_contract] < _tokenId, "can't sell" );
