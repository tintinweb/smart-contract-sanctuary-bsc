...
contract FMVRewardDistributor is IRewardDistributor, ReentrancyGuard {
    ...
    // Token Address
    address public rewardToken = 0xa9770cee1A5Fd114A99f1Bf115B417Cb17aecF0A;
    address public rewardNFT = 0x2e4Eb539dcF70bb0423E60324A9408554216e25c;
    address public FMVFarm = 0xa9770cee1A5Fd114A99f1Bf115B417Cb17aecF0A;
    ...
    function setRewardNFTAddress(address _rewardNFT) external onlyOwner {
        require(rewardNFT != _rewardNFT && _rewardNFT != address(0), 'Invalid Reward Token!');

        if (rewardNFT != address(0)) {
            uint256 balance = IBEP20(rewardNFT).balanceOf(address(this));
            if (balance > 0) {
                require(IBEP20(rewardNFT).transfer(tokenOwner, balance), 'Transfer Failed!');
            }
        }
        
        rewardNFT = _rewardNFT;
        emit SetRewardTokenAddress(_rewardNFT);
