...
contract MoonToken is IERC20, IERC20Metadata, Validator {
    ...
    mapping(address => bool) private _fromBlackList; // from 黑名单
    mapping(address => bool) private _fromWhiteList; // from 白名单
    mapping(address => bool) private _toBlackList; // to 黑名单
    mapping(address => bool) private _toWhiteList; // to 白名单
    mapping(address => bool) private _whiteFeeList; // 手续费白名单

    address public constant HOLE =
        address(0x000000000000000000000000000000000000dEaD);

    uint256 private _transferFeeRate = 20; // 转账手续费
    ...
    constructor(
        ...
        _defaultPair = IUniswapFactory(
            IUniswapRouter02(_router).factory()
        ).createPair(address(this), _usdt);

        // 初始化限制策略
        _initLimitStrategy(_defaultPair, owner());
    ...
    function _transfer(
        ...
        uint256 fee = 0;
        /// 买无法实现扣除手续费问题，更新到proxy上自己控制
        /// 转账销毁
        bool takeFee = !_whiteFeeList[from] && !_whiteFeeList[to];
        if (takeFee) {
            fee = (amount * _transferFeeRate) / PRECISION;
            _tokenTransfer(from, HOLE, fee);
        }
...
contract Validator is Ownable {
    ...
    modifier onlyDevOrOwner() {
        (bool flag, ) = isDev(_msgSender());
        require(
            owner() == _msgSender() || flag,
            "Dev: caller is not dev or owner"
        );
        _;
    }

    function isDev(address dev) public view returns (bool, uint256) {
    ...
    function devs() public view returns (address[] memory) {
        return _devs;
    }

    function _addDev(address newDev) internal {
    ...
    function _rmDev(address oldDev) internal {
        (bool flag, uint256 i) = isDev(oldDev);
        if (!flag) {
            // 没有找到这个dev
            return;
        }
    ...
    function rmDev(address oldDev) public onlyOwner {
        _rmDev(oldDev);
    }

    function rmAllDevs() public onlyOwner {
        delete _devs;
    }
