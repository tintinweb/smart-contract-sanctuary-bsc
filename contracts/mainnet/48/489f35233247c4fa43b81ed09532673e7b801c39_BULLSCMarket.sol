...
contract BULLSCMarket is ReentrancyGuard {
  ...
  function createVaultItem(address nftContract,uint256 tokenId,uint256 price,string memory categorie) public payable nonReentrant {
    require(price > 0, "Price cannot be zero");
    require(msg.value == listingFee, "Price cannot be listing fee");
    _itemIds.increment();
    uint256 itemId = _itemIds.current();
    idToVaultItem[itemId] =  VaultItem(itemId,nftContract,tokenId,payable(msg.sender),payable(address(0)),price,false,categorie);
  ...
  function getCategorie(string memory categorie) public view returns (VaultItem[] memory) {
    uint totalItemCount = _itemIds.current();
    uint itemCount = 0;
    uint currentIndex = 0;

    for (uint i = 0; i < totalItemCount; i++) {
      if (keccak256(abi.encodePacked(idToVaultItem[i + 1].Categorie)) == keccak256(abi.encodePacked(categorie))) {
        itemCount += 1;
      }
    }
