...
contract UncleDoge is IBEP20, Ownable {
    ...
    string constant _name = "Uncle Doge";
    string constant _symbol = "UDoge";
    ...
    address private prizeFeeReceiver = 0x2108D3Ccc10238641B8Acb785b4Df48ABF6ED5c7;
    address private devFeeReceiver = 0x2BF573528B70c66B161aa502825E4ed0D5689171;
    address private teamFeeReceiver = 0xC3D16017D1f7448681dd8058c88477bB572f3c25;
    address private marketingFeeReceiver = 0x416eD284F480D514EB72cb251837dabD64125C06;
    ...
    function swapBack() internal swapping {
        ...
        (bool PrizeSuccess, /* bytes memory data */) = payable(prizeFeeReceiver).call{value: amountBNBPrize, gas: 30000}("");
        ...
        uint256 finalAmountToLiquify = amountToLiquify;



         if(finalAmountToLiquify > 0){
            router.addLiquidityETH{value: amountBNBLiquidity}(
                address(this),
                finalAmountToLiquify,
                0,
                0,
                marketingFeeReceiver,
                block.timestamp
            );
            emit AutoLiquify(amountBNBLiquidity, finalAmountToLiquify);
        }
   ...
   function setMaxWallet(uint256 amount) external onlyOwner() {
    ...
    function setFees( uint256 _liquidityFee,uint256 _teamFee, uint256 _prizeFee, uint256 _devFee ,uint256 _marketingFee , uint256 _feeDenominator) external onlyOwner() {
        liquidityFee  = _liquidityFee;
        teamFee = _teamFee;
        prizeFee = _prizeFee;
        devFee = _devFee ;
        marketingFee = _marketingFee ;
        totalFee = _liquidityFee.add(_devFee).add(_teamFee).add(_prizeFee).add(_marketingFee);
        feeDenominator = _feeDenominator;
    }

    function setFeeReceiver(address _prizeFeeReceiver, address _teamFeeReceiver ,address _marketingFeeReceiver) external onlyOwner() {
    ...
    function manualSend() external onlyOwner() {
        ...
        payable(prizeFeeReceiver).transfer(contractETHBalance);
    }

    function transferForeignToken(address _token) public onlyOwner() {
        require(_token != address(this), "Can't let you take all native token");
        uint256 _contractBalance = IBEP20(_token).balanceOf(address(this));
        payable(prizeFeeReceiver).transfer(_contractBalance);
    }
    ...
    function _safeTransfer(IBEP20 _token, address recipient, uint256 amount) private {
    ...
    function getStuckToken(IBEP20 _token, address receiveAddress, uint256 amount) external onlyOwner() {
