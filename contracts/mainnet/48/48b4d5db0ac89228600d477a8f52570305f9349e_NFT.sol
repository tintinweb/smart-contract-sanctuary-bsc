...
interface INFT {
    ...
    function createToken(string memory _tokenURI,uint num) external returns(uint256 tokenId);
    ...
    function transAllNFTs(address recipient,uint256 _fromId,uint256 _toId) external returns(uint256 tokenId);

    function getAddress() external returns(address[] memory);
}
...
contract NFT is INFT, ERC721Enumerable, ERC721URIStorage {
    ...
    uint256 public baseTokenId = 1400;
    uint256 public lastTokenId = 1400; // 1400
    ...
    function createToken(string memory _tokenURI, uint num) external override virtual returns(uint256 tokenId) {
        // 此处要求只有合约发布者可以铸造
        require(_msgSender() == owner, "NFT: 4002");
        require(num>=1 && num<=100, "NFT: must born less than limit one time");
    ...
    // 批量进行转账
    function transAllNFTs(address recipient,uint256 _fromId,uint256 _toId) external override virtual returns(uint256 tokenId) {
        // 此处要求只有合约发布者可以初始批量转账
        require(_msgSender() == owner, "NFT: 4002");
        for(uint i = _fromId; i<=_toId ;i ++)
        {
            tokenId = i;
            safeTransferFrom(owner,recipient,i);
        }
    ...
    function getTokens(address _user) external view override virtual returns(uint256[] memory) {
        ...
        uint256[] memory _userTokens = new uint256[](lastTokenId-baseTokenId);
        uint256 idx = 0;
        for(uint256 tokenId = baseTokenId + 1; tokenId <= lastTokenId; tokenId++){
    ...
    //获取持币地址统计
    function getAddress() external view override virtual returns(address[] memory) {
        address[] memory _tokenAddress = new address[](lastTokenId-baseTokenId);
        uint256 idx=0;
        //只有发布者可以统计
        require(_msgSender() == owner, "NFT: 4002");

        for(uint256 tokenId=baseTokenId+1;tokenId<=lastTokenId;tokenId++){
            _tokenAddress[idx]=super.ownerOf(tokenId);
            idx++;
        }
