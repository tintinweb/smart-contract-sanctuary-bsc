...
// File: contracts/Gangster_Fee_Wallet.sol


pragma solidity ^0.8.7;


contract Gangster_Fee_Nft_Wallet is Ownable {
    ...
    function send() external payable {
        NFTamount = (address(this).balance / toAddress.length);
        for (uint256 i = 0; i < toAddress.length; i++) {
            payable(toAddress[i]).transfer(NFTamount);
        }
    }

    function addWalletsTo(
        uint256 _x,
        string memory name,
        address _add,
        address[] memory _addresses
    ) public {
        bool checkWinstatus = addWallet(_x, name, _add);

        if (checkWinstatus == true) {
            addWalletsToAirdrop(_addresses);
        }
    }
    ...
    function addWalletsToAirdrop(address[] memory _addresses) private {
