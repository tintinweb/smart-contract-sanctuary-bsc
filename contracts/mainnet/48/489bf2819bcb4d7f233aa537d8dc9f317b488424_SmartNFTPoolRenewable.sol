...
contract SmartNFTPoolRenewable is IERC721Receiver ,Ownable, ReentrancyGuard {
    ...
    */
    ...
    // output info for pools ( user balance , TokenId's (array), rewardPerDayPerToken, totalstaked, pendingreward, startblock, endblock )
    function getPoolInfo(address _user) external view returns (
                uint256 _balance,
                uint256[] memory _tokenIds,
                uint256 _rewardPerDatPerToken,
        ...
        UserInfo storage user = userInfo[_user];
        uint256 PendingReward = pendingReward(_user);
        uint256 rewardPerTokenPerDay = (rewardPerBlock * 28800).div(totalStaked);
        return (user.amount, user.tokenIds, rewardPerTokenPerDay, totalStaked, PendingReward, startBlock, bonusEndBlock);
    }
