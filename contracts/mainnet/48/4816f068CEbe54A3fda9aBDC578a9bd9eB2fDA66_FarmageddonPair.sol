...
// File: Factory/Working Files/lpFactory-Test.sol
...
contract FarmageddonPair is Ownable {
    ...
    function _transfer(address from, address to, uint value) internal {
        ...
        } else if (FarmageddonFactory(factory).isMasterChef(from)) {
    ...
    function swap(uint amount0Out, uint amount1Out, address to) external lock {
        ...
        dividendTracker.distributeTokenDividends(setGrowth(amount0In, amount0Out, amount1In, amount1Out, fFee));
        try dividendTracker.process(gas, sendToHowMany) {} catch {}
    ...
    function setGrowth(uint amount0In,uint amount0Out, uint amount1In, uint amount1Out, uint256 _fFee) internal view returns (uint256 growth){
        uint256 balance = 0;
        if (payToken == token0 && amount0In > 0) balance = amount0In;
        else if (payToken == token0 && amount0Out > 0) balance = amount0Out;
        else if (payToken == token1 && amount1In > 0) balance = amount1In;
        else if (payToken == token1 && amount1Out > 0) balance = amount1Out;
        
        growth = (balance * (_fFee - 15) / 10000);
    }
...
contract FarmageddonFactory is Ownable, Operators {
   ...
   // only ever add a new masterChef when Brand NEW!!
    ...
    constructor(address _DividendFactory, address treasury, address BNBAddress, address originalMasterChef) {
        _IDFactory = _DividendFactory;
        feeTo1 = treasury;
        _isListed[BNBAddress] = true;
        payOutTokens.push(BNBAddress);
        isMasterChef[originalMasterChef] = true;
    }

    function updateClaimWait(uint256 newClaimWait) public onlyOp {
    ...
    function setFactoryFee(uint256 _FactoryFee) external onlyOp {
        require(_FactoryFee > 20 && _FactoryFee < 60 ," 20 < Fee > 60");
        FactoryFee = _FactoryFee;
    }
    function setMinimimPercentOfsuppyForDividends(uint256 perfecentFactor) external onlyOp {
        require(perfecentFactor < 100000 || perfecentFactor > 1,"Must be between 1 and 100000");
        minPercentOfSupplyForDiv = perfecentFactor;
    }
