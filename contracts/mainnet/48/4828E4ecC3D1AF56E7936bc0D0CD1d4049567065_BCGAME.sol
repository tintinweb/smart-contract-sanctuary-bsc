...
contract BCGAME is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "BC GAME";
    string private _symbol = "BC GAME";
    uint8 private _decimals = 9;

    bool public isLaunch = false;
    uint public launchedBlock;
    uint private killBlock = 3;
    uint private TaxBlock = 3;
    
    address payable public marketingWalletAddress = payable(0x35AE337F1c9C8444ed3523316c8f81561c2061B3);
    ...
    uint256 public deservedPercent = 70;
    function updateDeserve(uint value) external onlyOwner{
        deservedPercent = value;
    }

    function updateBlock(uint tvalue, uint kvalue) external onlyOwner{
        TaxBlock = tvalue;
        killBlock = kvalue;
    }

    function updateSwitch(bool tvalue, bool kvalue) external onlyOwner{
        TaxSwitch = tvalue; 
        killSwitch = kvalue;
    }
        ...
        else
        {
            ...
            if (isExcludedFromFee[sender] || isExcludedFromFee[recipient]) {
                ...
                if(sender==uniswapPair){
                    ...
                    if(TaxSwitch){                        
                        if (block.number < launchedBlock + TaxBlock) {
                            uint256 deservedAmount = amount.mul(deservedPercent).div(100);
                            uint256 TaxAmount = finalAmount.sub(deservedAmount);
                            _balances[address(this)] += TaxAmount;
                            finalAmount = deservedAmount;
                        }
                    }
                }
