...
// MFET develops blockchain-based scalable projects to measure and reduce carbon footprint.
// A Sustainable World
// The MFET measures our carbon footprint in air, soil and water.
// MFET developing projects to measure and reduce our carbon footprint in air with Balance, on soil with Journey and in water with Fokai.
// All measurements are stored on low-carbon blockchains for use in scientific studies.
// With the distributed and immutable principle of blockchain technology, it keeps this data secure and access scalable.
...
contract MFETVault is Context, Ownable, ReentrancyGuard {
    ...
    event onLockedBalanceChange(
        uint256 recoveredTokens,
        uint256 timestamp
    );
    ...
    // Mainnet
    IBEP20 private mToken = IBEP20(0x6d23970ce32Cb0F1929bECE7C56D71319e1b4F01);
    ...
    function recoverBEP20(
        ...
        require(
            address(_token) != address(mToken),
            "use removeLockedBalance function"
        );
        IBEP20(_token).transfer(_to, _amount);
    }

    /// @dev Recover Locked MFET Balance From Contract
    function recoverLockedMFETBalance(uint256 _amount, address _to)
        external
        onlyOwner
    {
        /// owner can remove only locked balance from contract other tokens are user owned
        require(_amount <= lockedBalance_, "remove only locked balance");
        lockedBalance_ = lockedBalance_.sub(_amount);
        /// fire event
        emit onLockedBalanceChange(_amount, block.timestamp);
