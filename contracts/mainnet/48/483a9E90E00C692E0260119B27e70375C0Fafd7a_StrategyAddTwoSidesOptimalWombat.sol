...
contract StrategyAddTwoSidesOptimalWombat is Ownable, ReentrancyGuard, Strategy {
    ...
    constructor(IUniswapV2Router02 _router, address _goblin, ISwap _swap, IWombatRouter _wombatRouter) public {
        router = _router;
        goblin = _goblin;
        swap = _swap;
        ...
        nonReentrant 
    {
        ...
        if (baseToken.myBalance() > 0) {
            ...
            {
                baseToken.safeApprove(address(swap), 0);
                baseToken.safeApprove(address(swap), uint(-1));
                swap.swapQuery(baseToken.myBalance(), baseToken, fToken);
            }
        }
