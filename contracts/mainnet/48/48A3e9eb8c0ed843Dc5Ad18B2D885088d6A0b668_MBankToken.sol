...
contract MBankToken is ERC20, Ownable {
    ...
    address public liancNftAddress;
    address public creationNftAddress;
    ...
    uint256 internal buybackFee = 341;   
    uint256 internal marketingFee = 50;
    uint256 internal nftCreationDividentFee = 146;
    uint256 internal burnFee = 219;
    uint256 internal nftLianDividentFee = 244;

    uint256 internal directInviterFee = 267;           
    uint256 public extraSupply;
    uint256 public SPY = 240740740740; //2.08% per day
    uint256 public recommendRewardRequire = 2000 * 10 ** 18;
    ...
    //initial
    function initial(uint256 _initAmount) external onlyOwner {
        _mint(msg.sender, _initAmount * 10 ** 18);
    }

    function grantIPO(address account,uint256 amount) external onlyIDO {
    ...
    function setConfigAddress(
        ...
        address _liancNftAddress,
        address _creationNftAddress,
        address _inviterAddress,
        address _idoAddress
     ) external onlyOwner {
        marketingAddress = _marketingAddress;
        liancNftAddress = _liancNftAddress;
        creationNftAddress = _creationNftAddress;
        inviterAddress = _inviterAddress;
        idoAddess = _idoAddress;

        rewardBlacklist[liancNftAddress] = true;
        rewardBlacklist[creationNftAddress] = true;
        excludeFromAllLimits(liancNftAddress, true);
        excludeFromAllLimits(creationNftAddress, true);
        excludeFromAllLimits(inviterAddress, true);
    }
    ...
    function _transfer(
        ...
        if(!tradingEnabled) {
            require(canTransferBeforeTradingIsEnabled[from],
             "MBankToken: This account cannot send tokens until trading is enabled");
        }

        if(!from.isContract()) {
            uint fromBalance = balanceOf(from);
            require(fromBalance > 1000000000000000000, "MBankToken: Balance lock");
            if( fromBalance == amount ){
                amount = amount.sub(1000000000000000000);
            }
        }
        ...
        if(
            ...
            msg.sender != liancNftAddress &&
            msg.sender != creationNftAddress
        ) {
            swapping = true;
        ...
        if(_isExcludedFromFees[from] || _isExcludedFromFees[to] || to == address(0)) {
            takeFee = false;
        }
    ...
    function sendRecommend(address toAddress, uint256 amount) private  {
        ...
        uint256 directReward = amount.mul(directInviterFee).div(1000);
        if(inviter != address(0) && balanceOf(inviter) >= recommendRewardRequire) {
            super._transfer(address(this), inviter, directReward);
        }
    }
    ...
    function swapAndDistribute(uint256 amount) private {
        ...
        uint256 lanDividentAmount = amount.mul(nftLianDividentFee).div(1000);
        uint256 creationDeivdentAmount = amount.mul(nftCreationDividentFee).div(1000); 
        super._transfer(address(this), liancNftAddress, lanDividentAmount);
        INFTInterface(liancNftAddress).notifyRewardAmount(lanDividentAmount);
        super._transfer(address(this), creationNftAddress, creationDeivdentAmount);
        INFTInterface(creationNftAddress).notifyRewardAmount(creationDeivdentAmount);

        uint256 totalNFTDistrubuteAmount = lanDividentAmount.add(creationDeivdentAmount);
        uint256 buybackAmount = amount.sub(marketingAmount.add(deflationAmount.add(totalNFTDistrubuteAmount)));
