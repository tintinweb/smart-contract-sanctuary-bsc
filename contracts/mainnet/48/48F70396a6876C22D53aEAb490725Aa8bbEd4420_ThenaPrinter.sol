...
/*
     ...
     _______  __   __  _______  __    _  _______  _______  ______    ___   __    _  _______  _______  ______   
    |       ||  | |  ||       ||  |  | ||   _   ||       ||    _ |  |   | |  |  | ||       ||       ||    _ |  
    |_     _||  |_|  ||    ___||   |_| ||  |_|  ||    _  ||   | ||  |   | |   |_| ||_     _||    ___||   | ||  
      |   |  |       ||   |___ |       ||       ||   |_| ||   |_||_ |   | |       |  |   |  |   |___ |   |_||_ 
      |   |  |       ||    ___||  _    ||       ||    ___||    __  ||   | |  _    |  |   |  |    ___||    __  |
      |   |  |   _   ||   |___ | | |   ||   _   ||   |    |   |  | ||   | | | |   |  |   |  |   |___ |   |  | |
      |___|  |__| |__||_______||_|  |__||__| |__||___|    |___|  |_||___| |_|  |__|  |___|  |_______||___|  |_|    


    Welcome to ThenaPrinter!

    Buy & Sell tax used to purchase THENA tokens from the open market and distribute to holders.

    Use 14-18% slippage.

    Stats dashboard at https://thenaprinter.fi by tomorrow afternoon (1/6/2023)
...
// Dependency file: contracts/baby/ThenaPrinterDividendTracker.sol
    ...
    DividendPayingTokenOptionalInterface
{
    ...
    function distributeTHENADividends(uint256 amount) public onlyOwner {
...
contract ThenaPrinterDividendTracker is OwnableUpgradeable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ThenaPrinter contract."
        );
    }
...
// Root file: contracts/baby/ThenaPrinter.sol
...
// import "contracts/interfaces/IUniswapV2Factory.sol";
// import "contracts/interfaces/IUniswapV2Router02.sol";
// import "contracts/baby/ThenaPrinterDividendTracker.sol";
// import "contracts/BaseToken.sol";

contract ThenaPrinter is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    ThenaPrinterDividendTracker public dividendTracker;
    ...
    constructor(
        ...
        dividendTracker = ThenaPrinterDividendTracker(
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "ThenaPrinter: The dividend tracker already has that address"
        );

        ThenaPrinterDividendTracker newDividendTracker = ThenaPrinterDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "ThenaPrinter: The new dividend tracker must be owned by the ThenaPrinter token contract"
        );
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "ThenaPrinter: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "ThenaPrinter: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "ThenaPrinter: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "ThenaPrinter: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "ThenaPrinter: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "ThenaPrinter: Cannot update gasForProcessing to same value"
        );
    ...
    function swapAndSendToFee(uint256 tokens) private {
        uint256 initialTHENABalance = IERC20(rewardToken).balanceOf(
            address(this)
        );

        swapTokensForTHENA(tokens);
    ...
    function swapTokensForTHENA(uint256 tokenAmount) private {
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        if (success) {
            dividendTracker.distributeTHENADividends(dividends);
