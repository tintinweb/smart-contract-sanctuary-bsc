...
interface IMarket {
    ...
    struct MarketItem {
        ...
        address buyer;
        address seller;
        // bool sold; // Remove this in future
    }
...
contract Market is IMarket, Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        require(
            _fortiContract != address(0),
            "Invalid Forti address!"
        );
        require(
            _knoxContract != address(0),
            "Invalid Knox address!"
        );
        adminWallet = _adminWallet;
        bfkToken = IERC20(_tokenContract);
        fortiContract = IForti(_fortiContract);
        knoxContract = IInfinityCard(_knoxContract);
    }
    ...
    function listItem(
        ...
        uint256 _price,
        address _nftContract
    ) external override nonReentrant validContract(_nftContract) {
        require(
            !itemIdOnSale[_nftContract][_tokenId][_index] ||
    ...
    function batchSale(
        ...
        require(
            _supply > 0 
            && _supply <= (IERC1155(_nftContract).balanceOf(_seller, _categoryId) - nftSaleSupply[_nftContract][_categoryId]),
        ...
        while (_flag) {
            for (uint256 _index = 1; _index <= _end; _index++) {
                ...
                (bool _isSetOnSale, ) = ISale(_nftContract).setTokenForSale(
                    _categoryId,
                    _index,
                    _price,
                    _seller
    ...
    function makeSale(
        ...
        override
        nonReentrant
        validContract(_nftContract)
        isItemForSale(_tokenId, _index, _nftContract)
    {
        MarketItem storage _marketItem = nftMarketItems[_nftContract][_tokenId][
            _index
        ];
        ...
        isItemForSale(_tokenId, _index, _nftContract)
    {
        ...
        require(ISale(_nftContract).cancelSale(_tokenId, _index, _seller));
    ...
    function batchCancelSale(
        ...
        uint256 _supply,
        address _nftContract
    ) external onlyOwner validContract(_nftContract) {
        uint256[] memory _totalIdsOnSale = ISale(_nftContract)
            .getTotalIdsOnSale(_tokenId);
        require(
    ...
    function createAuction(
        ...
        require(
            _startingPrice != 0 &&
                (_reservePrice != 0 && _reservePrice >= _startingPrice) &&
                (_buyNowPrice != 0 && _buyNowPrice > _reservePrice),
            "Market: Invalid pricing!"
        );

        address _seller = msg.sender;
        (bool _isListed, string memory _message) = ISale(_nftContract)
            .setTokenForSale(_tokenId, _index, _startingPrice, _seller);
        if (!_isListed) revert(_message);

        auctionIds[_nftContract]++;
        if(_seller == owner()) nftSaleSupply[_nftContract][_tokenId]++;
        
        // uint256 _newMarketItemId = auctionIds[_nftContract];
        nftAuctionItems[_nftContract][_tokenId][_index] = Auction({
            // itemId: _newMarketItemId,
            tokenId: _tokenId,
            nftId: _index,
        ...
        isItemForSale(_tokenId, _index, _nftContract)
    {
        ...
        require(
            ...
            || (nftAuctionItems[_nftContract][_tokenId][_index].highestBid.bid != 0
                && nftAuctionItems[_nftContract][_tokenId][_index].highestBid.bid <
                nftAuctionItems[_nftContract][_tokenId][_index].reservePrice),
            "Market: Auction unavailable!"
        );
        require(
        ...
        isItemForSale(_tokenId, _index, _nftContract)
    {
        ...
        require(
            ISale(_nftContract).cancelSale(_tokenId, _index, _seller),
        ...
        if (_auction.highestBid.bid > 0) {
            ...
            // bfkToken.transfer(_auction.highestBid.bidder, _auction.highestBid.bid + _feeAmount);
        }
    ...
    function getActiveAuctions(address _nftContract)
        external
        view
        validContract(_nftContract)
    ...
    function fetchMarketItems(address _nftContract)
        external
        view
        returns (MarketItem[] memory _marketItem)
    {
        uint256 _tokenId = ISale(_nftContract).getTokenId();
    ...
    function viewAuctionItem(
        ...
        validContract(_nftContract)
        isItemForSale(_tokenId, _index, _nftContract)
        returns (Auction memory _auctionItem)
    {
    ...
    function _getActiveAuctions(
        ...
        for (uint256 _outer = 1; _outer <= _tokenId; _outer++) {
            ...
            for (uint256 _inner = 1; _inner <= supply; _inner++) {
                if (
                    // !nftAuctionItems[_nftContract][_outer][_inner].sold &&
    ...
    function _fetchMarketItems(uint256 _tokenId, address _nftContract)
        private
        view
        returns (MarketItem[] memory)
    {
        ...
        for (uint256 _outer = 1; _outer <= _tokenId; _outer++) {
            ...
            for (uint256 _inner = 1; _inner <= _supply; _inner++) {
                if (
                    // !nftMarketItems[_nftContract][_outer][_inner].sold &&
                    itemIdOnSale[_nftContract][_outer][_inner] &&
    ...
    function _removeEndedAuction(
        ...
        if (
            ...
            ISale(_nftContract).cancelSale(_tokenId, _index, msg.sender);
            if (_existingAuction.highestBid.bid > 0) {
                ...
                // bfkToken.transfer(_existingAuction.highestBid.bidder, _existingAuction.highestBid.bid);
            }
            if(_existingAuction.seller == owner()) nftSaleSupply[_nftContract][_tokenId]--;
            itemIdOnSale[_nftContract][_tokenId][_index] = false;
            // auctionId--;
            auctionIds[_nftContract]--;
            delete nftAuctionItems[_nftContract][_tokenId][_index];
