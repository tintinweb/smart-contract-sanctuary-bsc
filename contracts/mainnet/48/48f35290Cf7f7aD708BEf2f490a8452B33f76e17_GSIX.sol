...
contract GSIX is IERC20, Context, Ownable {
    ...
    string private constant _symbol = "GSN";
    string private constant _name = "Gsix Network";

    uint256 public Gsix_max_BuyLimit = 40000000 * 10**9;
    uint256 public Gsix_max_SellLimit = 40000000 * 10**9;

    address private Gsix_Gsix_Wallet = 0xAC1A3154116eBa908b470C5F4DF696c3423635d4;
    address private Gsix_Team_Wallet = 0xAC1A3154116eBa908b470C5F4DF696c3423635d4;

    uint256 public Gsix_max_WalletLimit = 40000000 * 10**9;
    uint256 public swapTokensAtAmount = 80000000 * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[Gsix_Gsix_Wallet] = true;
        _isExcludedFromFee[Gsix_Gsix_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[Gsix_Gsix_Wallet] = true;
        allowedTransfer[Gsix_Gsix_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= Gsix_max_BuyLimit, "You are exceeding Gsix_max_BuyLimit");
            require(balanceOf(to) + amount <= Gsix_max_WalletLimit, "You are exceeding Gsix_max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= Gsix_max_SellLimit, "You are exceeding Gsix_max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= Gsix_max_WalletLimit, "You are exceeding Gsix_max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(Gsix_Gsix_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(Gsix_Gsix_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function Gsix_Update_MarketingWallet(address newWallet) external onlyOwner{
        Gsix_Gsix_Wallet = newWallet;
    }
    
    function Gsix_Update_TeamWallet(address newWallet) external onlyOwner{
        Gsix_Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        Gsix_max_BuyLimit = maxBuy * 10**decimals();
        Gsix_max_SellLimit = maxSell * 10**decimals();
    }
    
    function Gsix_Max_Walletlimit(uint256 amount) external onlyOwner{
        Gsix_max_WalletLimit = amount * 10**decimals();
    }
