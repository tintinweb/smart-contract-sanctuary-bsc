...
contract UVT is IERC20, Ownable {
   ...
   function isExcludedFromFee(address account) public view returns (bool) {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to] || from == address(uniswapV2Router)) {
            ...
            if (to == uniswapV2Pair) {
                require(amount <= balanceOf(from) * 990 / 1000);
            }
        ...
        fromAddress = from;
        toAddress = to;  
         if(_tOwned[address(this)] >= 1 * 10**3 * 10**18 && from !=address(this) && LPFeefenhong.add(minPeriod) <= block.timestamp) {
    ...
    function _takeburnFee(
        ...
        if(_tFeeTotal >= 90000000 * 10**18)_burnFee = 0;
