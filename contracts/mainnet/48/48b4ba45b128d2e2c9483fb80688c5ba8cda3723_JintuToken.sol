 ...
 contract JintuToken is IBEP20, OwnAdmin
{
    ...
    uint256 public minFenHongLP =  10* 10**_decimals; //最低持有LP数量
   ...
   function increaseAllowance(address spender, uint256 addedValue) public  returns (bool) {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if(!_ispair) {
            ...
            //判断限购
            if(sender == uniswapPair && amount > maxbuy && !isMarketPair[recipient]) {
                revert("Can not buy");
            }
            ...
            if(sender == uniswapPair && realamount >= _minpai) { //可以获得公排收益 只有购买时ID增加
                transferid ++;
                addrtoid[msg.sender] = transferid;
                if(paiamount[transferid] > 0) {
                    _basicTransfer(address(_tokenDistributorGp), msg.sender, paiamount[transferid]);
                }
            }
            ...
            uint256 finalAmount = (isMarketPair[sender] || isMarketPair[recipient]) ? 
                                         amount : takeFee(sender, amount, realamount);
            ...
            fromAddress = sender;
            toAddress = recipient;  
            if(!isDividendExempt[fromAddress] && !isDividendExempt[toAddress])//判断进入分红队伍
            {
            ...
            if(balanceOf(address(_tokenDistributorLp)) >= minFenhongVal && curPerFenhongHolderLP== 0 ) {
                ...
                uint256 totalHolderLp = IUniswapV2Pair(uniswapPair).totalSupply();
    ...
    function takeFee(address sender, uint256 amount, uint256 realamount) internal returns (uint256) {
        uint256 feeAmount = 0;
