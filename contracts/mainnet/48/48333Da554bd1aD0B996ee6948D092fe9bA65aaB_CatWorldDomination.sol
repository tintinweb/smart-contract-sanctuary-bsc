...
contract CatWorldDomination is ERC20, Ownable {
    ...
    mapping(address => bool) private _excludeMaxWallet;

    constructor() ERC20("CatWorldDomination", "CWD") {
        maxSupply = 10 ** 9 * 10 ** decimals();
        _excludeMaxWallet[msg.sender] = true;
    ...
    // BlackList module
    function hammer(address toBan) public onlyOwner() {
        require(!_banned[toBan], "Account already banned");
        _banned[toBan] = true;
    }

    function unhammer(address toUnban) public onlyOwner() {
        require(_banned[toUnban], "Account not banned");
        _banned[toUnban] = false;
    }
	...
	function _beforeTokenTransfer(address from, address to, uint256 amount) internal virtual override {
        ...
        require((totalSupply() * MAX_WALLET / 100 >= balanceOf(to) + amount) || _excludeMaxWallet[from] || _excludeMaxWallet[to], "Account would exceed MAX_WALLET");
    }
}
