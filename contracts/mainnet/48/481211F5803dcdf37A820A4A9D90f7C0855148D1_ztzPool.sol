...
contract ztzPool is Ownable ,IERC721Receiver{
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        address NFTToken; 
        address referralRewardContract;
        uint256 allocPoint;
    ...
    address public ztzAddress=0x5b3C40E694bE20e0e8C300FD362d7140b20441D3;
    ...
    uint256 public rewardDividends=2;
    ...
    function setrewardDividends(uint _rewardDividends) public onlyOwner{
        rewardDividends=_rewardDividends;
    }
   ...
   function onERC721Received(address operator,address from,uint256 tokenId,bytes calldata data) external override returns (bytes4){}
    ...
    function newMining(
        ...
        tokenPerBlock = remainingAmount.div(201600);
        startBlock = _startBlock;
        unLockTime=now+(startBlock-block.number).mul(3)+7 days;
        // minePeriod=_minePeriod;
    ...
    function addLpToken(
        address _lpToken,
        address _NFTToken,
        address _referralRewardContract,
        uint256 _allocPoint
        ...
        poolInfos.push(
            PoolInfo({
                lpToken: _lpToken,
                NFTToken:_NFTToken,
                referralRewardContract:_referralRewardContract,
    ...
    function deposit(address _lpToken, uint256 _amount,address _referrer,uint256 tokenId) public {
        ...
        if (_amount > 0) {
            ...
            IERC721(pool.NFTToken).transferFrom(
                address(msg.sender),
                address(this),
                tokenId
            );
        ...
        if(referrer!=address(0)){
            uint256 ztzbyLp=consult(_lpToken,_amount);
            IERC20(mintTokenAddress).safeTransferFrom(
                            address(pool.referralRewardContract),
                            referrer,
                            ztzbyLp.mul(rewardDividends).div(100)
                        );

            referralRewards[referrer] = referralRewards[referrer].add(ztzbyLp.mul(rewardDividends).div(100));
        }
        emit Deposit(msg.sender, pid, _amount);
    }

    function getZTZByLp(uint256 _amount)internal {
        
    }

    
    function consult(address _lpToken, uint256 _amountIn) internal view returns (uint256 amountOut) {
        uint256 ztzBalance = IERC20(ztzAddress).balanceOf(_lpToken);
        uint256 wbnbBalance = IERC20(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c).balanceOf(_lpToken);

        uint256 ztz=ztzBalance.mul(1*10**18).div(wbnbBalance);
        uint256 lpmin=sqrt(ztz.mul(1*10**18));

        uint256 ztzbyLp=_amountIn.mul(1*10**18).div(lpmin).mul(ztz);
        
        return ztzbyLp.div(1*10**18);
    }
    ...
    function withdraw(address _lpToken)public {
        ...
        if (user.amount > 0) {
            ...
            for (uint i=0;i<user.tokenIdList.length;i++){
                IERC721(pool.NFTToken).transferFrom(
                    address(this),
                    address(msg.sender),
                    user.tokenIdList[i]
