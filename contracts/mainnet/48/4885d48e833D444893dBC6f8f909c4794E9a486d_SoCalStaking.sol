...
contract SoCalStaking is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 public calculationTime = 365;
    ...
    event Deposited(address indexed payee, uint256 soCalAmount, uint256 timeStamp);
    event Withdrawn(address indexed payee, uint256 soCalAmount, uint256 timeStamp);

    mapping(address => userStakeData) public _stakeData;
    mapping(uint256 => pool) public _pool;
    ...
    function stakeSoCal(uint256 _amount, uint256 _locktime) public {
        ...
        require(_locktime == 30 || _locktime == 60, "Socal token can only be stakes for either 30 days or 60 days");
        ...
        _stakeData[msg.sender]._days = _locktime;
    ...
    function calculateReward(address _caller) public view returns (uint256 nativeAmount) {
        userStakeData memory cache = _stakeData[_caller];
        uint256 APY;
        if (cache._locktime == cache._timestamp + 30 days && _count <= 500) {
            APY = 160;
            nativeAmount = (APY.mul(cache._days)).mul(cache._amount).div(calculationTime).div(10**2);
        } else if (cache._locktime == cache._timestamp + 30 days && _count > 500 && _count < 1250) {
            APY = 110;
            nativeAmount = (APY.mul(cache._days)).mul(cache._amount).div(calculationTime).div(10**2);
        } else if (cache._locktime == cache._timestamp + 30 days && _count > 1250 && _count < 2250) {
            APY = 85;
            nativeAmount = (APY.mul(cache._days)).mul(cache._amount).div(calculationTime).div(10**2);
        } else if (cache._locktime == cache._timestamp + 60 days && _count <= 500) {
            APY = 310;
            nativeAmount = (APY.mul(cache._days)).mul(cache._amount).div(calculationTime).div(10**2);
        } else if (cache._locktime == cache._timestamp + 60 days && _count > 500 && _count < 1250) {
            APY = 160;
            nativeAmount = (APY.mul(cache._days)).mul(cache._amount).div(calculationTime).div(10**2);
        } else if (cache._locktime > cache._timestamp + 60 days && _count > 1250 && _count < 2250) {
            APY = 110;
            nativeAmount = (APY.mul(cache._days)).mul(cache._amount).div(calculationTime).div(10**2);
        } else if (cache._locktime > cache._timestamp + 30 days) {
            APY = 60;
            nativeAmount = (APY.mul(cache._days)).mul(cache._amount).div(calculationTime).div(10**2);
        }
    }
    ...
    function fundsMigration(address _to) external onlyOwner {
        require(_to != address(0), "cannot transfer to zero address");
        IERC20(STAKECONTRACTADDRESS).transfer(_to, IERC20(STAKECONTRACTADDRESS).balanceOf(address(this)));
    }
}
