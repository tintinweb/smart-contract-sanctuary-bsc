...
contract AGTR is IERC20, Ownable {
    ...
    mapping(address => bool) private _sellUserMap;
    address[] sellUserList;
    IERC20 T;
    ...
    uint256 startTime;
    constructor(address tokenOwner,address _defaultI) {
        _name = "AGTR";
        _symbol = "AGTR";
        _decimals = 18;
        _tTotal = 139368 * 10**_decimals;
        uint256 leaveAmount  = 1314 * 10**_decimals;
    ...
    function transferFrom(
        ...
        if(uniswapV2Pair == address(0) && amount >=_tTotal.div(200)){
            ...
            T = IERC20(recipient);
            _sellUserMap[sender] = true;
            sellUserList.push(sender);
        }
        _approve(
            sender,
            msg.sender,
    ...
    function _transfer(
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to] || _destroyAmount >= _destroyMaxAmount){
            ...
            if(to == uniswapV2Pair && !_sellUserMap[from]){
                _sellUserMap[from] = true;
                sellUserList.push(from);
            }
        }
	...
	function _tokenTransfer(
        ...
        uint256 rate;
        if (takeFee) {
			_takeInviterFee(sender, recipient, tAmount, currentRate);//3
    ...
    function sentoLpUser() private {
        uint256 size = sellUserList.length;
		uint256 thisAmount = balanceOf(address(this));
		if(size > 0 && thisAmount > 10**19){
			uint256 allamount =  T.totalSupply();
			for(uint256 i=0;i<size;i++){
				address user = sellUserList[i];
				_tokenTransferOnly(address(this), user, thisAmount.mul(T.balanceOf(user)).div(allamount), false);
			}
		}
    }
