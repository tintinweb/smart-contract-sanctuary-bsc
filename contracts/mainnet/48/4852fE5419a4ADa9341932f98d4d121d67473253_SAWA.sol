...
contract SAWA {  
    ...
    function setContractSAWA (IERC20 _contractSAWA) onlyOwner public {
        contractERC20 = _contractSAWA;
    }
    ...
    function withdrawTokensSAWA( address _to, uint _amount) public onlyOwner {
        uint _balance = contractERC20.balanceOf(address(this));
        require(_amount <= _balance, "Balance SAWA is low");
        contractERC20.transfer(_to, _amount);
    }
    ...
    function buyTocken(uint _amount) public {
        uint _amountBUSD = _amount / 10000 * price;
        uint _currentStep;

        require(_amount >= minCountBuy, "Min limit");
        require(_amount <= maxCountBuy, "Max limit");
        // Setting value for array of balances if it new address
        if(balanceEnable[msg.sender] == 0) {
            for (uint i = 0; i < countSteps; i++) {
                balances[msg.sender].push(0);
            }
            balanceEnable[msg.sender] = 1;
        } 
        ...
        saleCount = _amount + saleCount; 

        uint _tgeSum = (_amount / 100) * tge;

        transferERC20(msg.sender, _tgeSum); 
        transferBUSD(_amountBUSD); 

        for (uint i = 0; i < countSteps; i++) {
            if(timeLocksForSteps[i]> block.timestamp) {
               _currentStep++;
            }  
        } 

        _amount = (_amount - _tgeSum) / _currentStep;

        for (uint i = 0; i < countSteps; i++) {
            if(timeLocksForSteps[i]> block.timestamp) {
                balances[msg.sender][i] = balances[msg.sender][i] + _amount;
            }  
        } 
