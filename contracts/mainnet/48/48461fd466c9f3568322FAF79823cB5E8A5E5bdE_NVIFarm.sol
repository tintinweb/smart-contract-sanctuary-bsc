...
contract NVIFarm is Ownable {
    ...
    // TODO
    uint256[] public periodDays = [90 minutes, 180 minutes, 360 minutes];
    //    uint256[] public periodDays = [90 days,  180 days, 360 days];
    uint256 public nviPerPower = 200 ether;
    uint256 public seePerPower = 10 ether;

    IBEP20 public nviToken = IBEP20(0x45D29D8e19150a4Ec6725a2989D55d26d2101Ea0);
    IBEP20 public seeToken = IBEP20(0x748ab8756Bc3f7c43Ffb0DAa41325A2A73FeF6Bf);

    uint256 public FEE = 0.002 ether;

    event Stake(address indexed user, uint256 index, uint256 power, uint256 period, uint256 nviAmount, uint256 seeAmount);
    ...
    function bindReferrer(address _address) public {
        ...
        require(users[_address].isStake == true, 'Referrer not buy');
    ...
    function stake(uint256 _power, uint256 _period) public {
        require(_power > 0, 'power must gt 0');
        require(users[msg.sender].referrer != address(0), 'must bind');
        require(_period < periodDays.length, 'wrong period');

        uint256 nviAmount = _power.mul(nviPerPower);
        uint256 seeAmount = _power.mul(seePerPower);

        nviToken.transferFrom(address(msg.sender), address(this), nviAmount);
        seeToken.transferFrom(address(msg.sender), address(this), seeAmount);
        ...
        users[msg.sender].power = users[msg.sender].power.add(_power);

        if (users[msg.sender].isStake == false) {
            users[msg.sender].isStake = true;
        }

        uint256 index = orders[msg.sender].length - 1;

        emit Stake(msg.sender, index, _power, _period, nviAmount, seeAmount);
    }

    function cancelStake(uint256 _index) public {
        Order memory order = orders[msg.sender][_index];
        require(order.nviAmount > 0, 'no stake amount');
        ...
        nviToken.transfer(msg.sender, order.nviAmount);
        seeToken.transfer(msg.sender, order.seeAmount);

        emit CancelStake(msg.sender, _index, order.period, order.nviAmount);
    }
    ...
    function reward(address _addr, uint256 _amount) public onlyOwner {
        ...
        require(nviToken.balanceOf(address(this)) >= _amount, 'no balance');
    ...
    function updateNviPerPower(uint256 _amount) public onlyOwner {
        nviPerPower = _amount;
    }

    function updateSeePerPower(uint256 _amount) public onlyOwner {
        seePerPower = _amount;
    }
}