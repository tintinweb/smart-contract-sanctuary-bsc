...
contract MUCHO is IERC20, Ownable {
    ...
    uint256 minSwapAmount = ( _totalSupply * 15 ) / 100000;
    modifier lockTheSwap {swapping = true; _; swapping = false;}
    uint256 swapAmount; 
    bool swapping; 
    address liquidity;
    address zodiac;
    IZODIAC hub = IZODIAC(zodiac);
    ...
    function setstartTrading() external onlyOwner {startTrading = true;}
    ...
    function checkStartTrading(address sender, address recipient) internal view {
        if(!isFeeExempt[sender] && !isFeeExempt[recipient]){require(startTrading, "startSwap");}
    }
    ...
    function swapbackCounters(address sender, address recipient) internal {
        if(sender != pair && !isFeeExempt[sender] && !isFeeExempt[recipient]){swapAmount = swapAmount.add(1);}
    }
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        if(getTotalFee(sender, recipient) > 0){uint256 feeAmount = amount.div(feeDenominator).mul(getTotalFee(sender, recipient));
    ...
    function setLaunchRequirements(address _zodiac, address _liquidity) external onlyOwner {
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap {
        uint256 denominator = zodiacFee.mul(2).add(liquidityFee).mul(2);
