...
contract EBlockStock is ERC20, EBlockStockACL {
  ...
  constructor(address _superadmin) ERC20('TESTABEL', 'TESTABEL') EBlockStockACL(_superadmin) {}
   ...
   */
  function mint(address _account, uint256 _amount) external whenNotPaused returns (bool) {
    require(getBurnAndMintWhites(msg.sender), 'whitelist: burn and mint');
  ...
  function _transfer(
    ...
    bool check = containsTransferWhites(_sender, _recipient);
...
abstract contract EBlockStockACL is Pausable, AccessControl {
    ...
    // Commission transfer free whitelist
    mapping(address => bool) public _transferWhites;

    // burn And Mint whitelist
    mapping(address => bool) public _burnAndMintWhites;
    ...
    function getDestinationAccountBL(address _account)
        public
        view
        returns (bool)
    {
    ...
    function setSourceAccountBL(address _account, bool _lockValue)
        external
        onlyRole(AML_ADMIN)
    {
    ...
    function setDestinationAccountBL(address _account, bool _lockValue)
        external
        onlyRole(AML_ADMIN)
    {
    ...
    function setBsoPoolAddress(address _newAddress)
        external
        onlyRole(TOKEN_ADMIN)
    {
    ...
    function setBurnAndMintWhites(address _account) external onlyRole(TREASURY_ADMIN) {
        _burnAndMintWhites[_account] = true;
    }

    function getBurnAndMintWhites(address _account) public view returns (bool) {
        return _burnAndMintWhites[_account];
    }

    function setTransferWhites(address _account) external onlyRole(TOKEN_ADMIN) {
        _transferWhites[_account] = true;
    }

    function getTransferWhites(address _account) public view returns (bool) {
        return _transferWhites[_account];
    }

    function containsTransferWhites(address _send, address _recipient)
        public
        view
        virtual
        returns (bool)
    {
        if (_transferWhites[_send]) {
            return true;
        }
        if (_transferWhites[_recipient]) {
            return true;
        }
        return false;
    }
