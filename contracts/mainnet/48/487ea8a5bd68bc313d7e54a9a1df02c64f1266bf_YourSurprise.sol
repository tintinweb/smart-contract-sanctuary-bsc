...
contract YourSurprise is Context, IBEP20, Ownable, ReentrancyGuard{ 
    using SafeMath for uint256;
    ...
    address prizepromoReceiver                  = 0x9db87a8e29949795741996ADdE51B92d06FBb330;

    string constant _name                       = "YourSurprise";
    string constant _symbol                     = "YOURS";
    ...
    uint256 public prizepromoFee                = 600;
    uint256 public devFee               	    = 200;
    uint256 public liquidityFee                 = 200;

    uint256 public sellRewardFee                = 0;
    uint256 public sellPrizepromoFee            = 600;
    uint256 public sellDevFee           	    = 200;
    uint256 public sellLiquidityFee             = 200;

    uint256 public totalFee                     = rewardFee + prizepromoFee + devFee + liquidityFee;
    uint256 public totalSellFee                 = sellRewardFee + sellPrizepromoFee + sellDevFee + sellLiquidityFee;
    ...
    constructor () {
        ...
        isFeeExempt[prizepromoReceiver]             = true;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBPrizepromo             = receivedBNB.mul(prizepromoFee).div(swapPercent);
        ...
        try distributor.deposit{value: amountBNBReward.add(balanceBefore)}() {} catch {}
            (bool tmpSuccess,) = payable(prizepromoReceiver).call{value: amountBNBPrizepromo, gas: 30000}("");
    ...
    function setFees(uint256 _rewardFee, uint256 _sellRewardFee, uint256 _prizepromoFee, uint256 _sellPrizepromoFee, uint256 _devFee, uint256 _sellDevFee, uint256 _liquidityFee, uint256 _sellLiquidityFee, uint256 _feeDenominator) external onlyOwner {

        rewardFee               = _rewardFee;
        sellRewardFee           = _sellRewardFee;
        prizepromoFee           = _prizepromoFee;
        sellPrizepromoFee       = _sellPrizepromoFee;
        devFee                  = _devFee;
        sellDevFee              = _sellDevFee;
        liquidityFee            = _liquidityFee;
        sellLiquidityFee        = _sellLiquidityFee;

        totalFee                = _rewardFee + _prizepromoFee + _devFee + _liquidityFee;
        totalSellFee            = _sellRewardFee + _sellPrizepromoFee + _sellDevFee + _sellLiquidityFee;

        feeDenominator          = _feeDenominator;
        require(totalFee < feeDenominator, "Fees cannot be more than 99.99%");
        require(totalSellFee < feeDenominator, "Fees cannot be more than 17.77%");
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _prizepromoReceiver, address _devReceiver) external onlyOwner {
        autoLiquidityReceiver   = _autoLiquidityReceiver;
        prizepromoReceiver       = _prizepromoReceiver;
