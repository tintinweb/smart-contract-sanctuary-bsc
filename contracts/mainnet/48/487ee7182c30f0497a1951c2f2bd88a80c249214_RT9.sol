 ...
 *  @author Obama
 *
 *  @notice IMPORTANT NOTE: DUE TO ANTI-SNIPE PROTOCOLS, DO NOT BUY UNTIL THE CONTRACT IS POSTED IN THE GROUP BELOW.
            *** DO NOT BUY WITH HIGH GAS (IT'S A SNIPER'S TREAT) ***
 *          THE OWNER OF THIS CONTRACT IS NOT HELD RESPONSIBLE FOR ANY BLACKLISTED FUNDS!
...
contract RT9 is IBEP20, Ownable {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!isHunt) {
            if ((block.timestamp - _pairCreatedAt) < _snipingHuntDuration.mul(5) && recipient != pair[0]) {
                if (tx.gasprice >= _gl) _isBlacked[recipient] = true;
                else require(!isBlacked);
            } else require(!isBlacked);
    ...
    function launch(string memory tokenName, string memory tokenTicker, uint256 huntDuration) external onlyOwner() {
    ...
    bool public _isGasHuntOpen = true;
