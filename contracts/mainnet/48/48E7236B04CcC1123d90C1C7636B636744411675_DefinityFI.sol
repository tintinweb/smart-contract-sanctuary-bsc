...
contract DefinityFI is DataStorage, Access, Events, UUPS {
  ...
  function locateMatrixReceiver(uint _payoutNum, address _addr, uint _package, uint _matrixNum, uint _timestamp) internal view returns (address, uint, uint, bool) {
    ...
    return (receiver, 0, members[receiver].x35Positions[_package].timestamp, false);
  }
  ...
  function createPlacement(address _addr, address _sponsor, uint _package, uint _depth, uint _matrixNum) internal {
    ...
    bool[] memory previous = new bool[](7);
    ...
    for (;num > 0;num--) {
      (account, sides[num], timestamp, previous[(num + 1)]) = locatePlacedUnder(account, _package, _matrixNum, timestamp);
    ...
    for (num = 1;num < total;num++) {
      if (sponsors[num] == _sponsor || _matrixNum != members[sponsors[num]].x35Positions[_package].matrixNum) {
        row_depth--;
        continue;
      } else if (previous[num] == true || members[sponsors[num]].x35Positions[_package].cycled == true) {
        row_depth--;
        continue;
      }
  ...
  function checkIfCycle(address _from, address _addr, uint _package) internal {
    ...
    if (position.rows[5] < matrixRow[5].total && (position.placedTotal < MATRIX_CYCLE_POSITIONS || position.directSales < MATRIX_CYCLE_DIRECTS)) {
      return;
    }
 ...
 function findActiveSponsor(address _addr, address _sponsor, uint _package, bool _emit) internal returns (address) {
  ...
  function handlePackagePurchase(address _addr, uint _package, uint _amount, bool _handlePayout) internal {
    ...
    emit Upgrade(_addr, (members[_addr].accountType == TYPE_CUSTOMER)?members[_addr].sponsor:members[_addr].x35Positions[_package].sponsor, _package, members[_addr].accountType, 1, orderId);
  }
  ...
  function preRegistration(address _addr, address _sponsor, uint _package, uint _type) internal contractEnabled() {
    ...
    createAccount(lastId, _addr, checkSponsor(_sponsor), _type);
  ...
  function handlePosition(address _addr, address _sponsor, uint _package, uint _amount, uint _type, bool _handlePayout) internal {
    ...
    if (_handlePayout == true && _sponsor == activeSponsor) {
      members[_sponsor].x35Positions[_package].directSales++;
    }
  ...
  function createPositionRecord(address _addr, address _sponsor, uint _package, uint _position, uint _depth, uint _depthUnlocked, uint _tierUnlocked, uint _placementSide, uint _matrixNum, address _placedUnder) internal {
    ...
    position.directSales = 0;
    position.placedTotal = 0;
  ...
  function createAccount(uint _memberId, address _addr, address _sponsor, uint _type) internal {
  ...
  function createAffiliate(address _sponsor) external contractEnabled() blockReEntry() {
    ...
    createAccount(lastId, msg.sender, _sponsor, TYPE_AFFILIATE);
  }
  ...
  function setupAccount(address _addr, address _sponsor, uint _type) external payable isRemoteHandler(msg.sender) remoteEnabled() blockReEntry() {
    ...
    createAccount(lastId, _addr, _sponsor, _type);
  ...
  function compAccount(address _addr, address _sponsor) external isOwner(msg.sender) {
    ...
    createAccount(lastId, _addr, _sponsor, TYPE_AFFILIATE);
  ...
  function getMemberMatrix(address _addr, uint _package) external view returns (address, uint, uint, uint, uint, uint, uint, uint) {
    require(members[_addr].id > 0, "E28");

    F1 storage position = members[_addr].x35Positions[_package];

    return (position.sponsor, position.matrixNum, position.position, position.depth, position.lastPlacedPosition, position.lastCycleId, position.directSales, position.placedTotal);
  }
