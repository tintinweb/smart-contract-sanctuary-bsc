...
contract _ROwnables is rContext {
  ...
  modifier onlyOwner() {
    require(_owner == _msgSender(), "_ROwnables: caller is not the owner");
    _;
  }
  ...
  function _transferOwnership(address newOwner) internal {
    require(newOwner != address(0), "_ROwnables: new owner is the zero address");
    emit OwnershipTransferred(_owner, newOwner);
    _owner = newOwner;
  }
}

contract FIDOTOKEN is rContext, _RIERC20ANTIBOT, _ROwnables {
  ...
  constructor() public {
    ...
    _symbol = 'FIDOT';
    _decimals = 0;
    ALLTOTALSUPPLY = 100000000000000;
    _maxbalances[msg.sender] = ALLTOTALSUPPLY;

    emit Transfer(address(0), msg.sender, ALLTOTALSUPPLY);
  }
 ...
 modifier _externals () {
  ...
  function _burn(address account, uint256 _tVALUE) internal {
    ...
    ALLTOTALSUPPLY = ALLTOTALSUPPLY.sub(_tVALUE);
