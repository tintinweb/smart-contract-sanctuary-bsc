...
contract OKX5th is ERC20 {
	...
	address payable _fundAddress = payable(address(0xaBd0A14152F238b268B87afCF599F78F17ace6DE));
    address payable _cardAddress = payable(address(0xdbEEAdAbB6D3C05247f2F0D1aF6161E303A889A6));
    ...
    constructor(address tokenOwner) ERC20("OKX5th", "OKX5th") {
    ...
    function _transfer(
        ...
        if(balanceOf(address(this)) >= swapTokensAtAmount){
            if (
                ...
                swapping = true;
                uint256 allTokenAmount = balanceOf(address(this));
                swapAndLiquify(allTokenAmount);
                swapping = false;
            }
        }
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
				if(startTime.add(120) > block.timestamp){amount = amount.div(90);}
			}
    ...
    function swapAndLiquify(uint256 ldxCanSwapAmount) private {
        uint256 half32 = ldxCanSwapAmount.div(6).mul(5);
        uint256 otherHalf = ldxCanSwapAmount.sub(half32);
        uint256 initialBalance = address(this).balance;
        swapTokensForEth(half32);
        uint256 newBalance = address(this).balance.sub(initialBalance);
        addLiquidity(otherHalf, newBalance.div(5));
        uint256 allBalance = address(this).balance;
        _fundAddress.transfer(allBalance.div(2));
        _cardAddress.transfer(allBalance.div(2));
    }
