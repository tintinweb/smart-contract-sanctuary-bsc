...
interface ERC721
{
    function  getAllPower(uint _tid) external view returns (uint256);
...
contract UniswapReward is LPTokenWrapper{
    ...
    constructor(address newuniswapV2Pair, address newNFTaddr,address _newrewardaddr,address inviteAddr30) public {
        uniswapV2Pair = newuniswapV2Pair;
        _NFTaddr = newNFTaddr;
        _rewardToken = IERC20(_newrewardaddr);
        inviteAddr = inviteAddr30;
    }
        ...
        checkStart
    {
        ...
        itemRecord._addr = msg.sender;
        itemRecord._amount = tokenids.length;
        itemRecord._time = block.timestamp;
        uint _tpower = 0;
        for(uint i=0;i<tokenids.length;i++)
        {
            ERC721(_NFTaddr).safeTransferFrom(msg.sender,address(this),tokenids[i]);
            uint _power = ERC721(_NFTaddr).getAllPower(tokenids[i]);
            _tpower = _tpower.add(_power);
            super.stake(_power);

            hasStake[msg.sender] = hasStake[msg.sender].add(_power);
            IPowerStrategy(pwrCtrl).addPwr(_power);
        }
        itemRecord._power =_tpower;
        govSetting.push(itemRecord);

        if(balanceOfPower(msg.sender) >0 && !addrExist[msg.sender])
        {
            addrProviders.add(msg.sender);
            addrExist[msg.sender] = true;
        }

        IPlayerBook(inviteAddr).stakeregister(msg.sender);
    ...
    function getPrice() public view returns(uint){
        ...
        return token0 == _usdt?r0.mul(1e5).div(r1):r1.mul(1e5).div(r0);
    }

    function availableReward(address account) public view returns(uint){
        uint256 reward = earned(account);
        uint _power = reward.mul(getPrice()).div(1e5);
        return _power >= balanceOfPower(account)?balanceOfPower(account).mul(1e5).div(getPrice()):reward;
    }
    ...
    function getReward() public updateReward(msg.sender) checkHalve checkStart {
        ...
        if (reward > 0) {
            ...
            uint _power = reward.mul(getPrice()).div(1e5);
            uint256 leftReward = _power >= balanceOfPower(msg.sender)?balanceOfPower(msg.sender).mul(1e5).div(getPrice()):reward;
            withdraw(_power >= balanceOfPower(msg.sender)?balanceOfPower(msg.sender):_power,msg.sender);
            ...
            if(balanceOfPower(msg.sender) <= 0 && addrExist[msg.sender])
            {
                addrProviders.remove(msg.sender);
                addrExist[msg.sender] = false;
            }
        ...
        updateReward(address(0))
    {
       uint256 addrCount = addrProviders.length();
        ...
        while (iterations < addrCount) {
            ...
            updateAll(curAddr);
            uint256 reward = earned(curAddr);

            uint _power = reward.mul(curPrice).div(1e5);
            uint256 leftReward = _power >= balanceOfPower(curAddr)?balanceOfPower(curAddr).mul(1e5).div(curPrice):reward;
            withdraw(_power >= balanceOfPower(curAddr)?balanceOfPower(curAddr):_power,curAddr);

            if(leftReward>0){
                _rewardToken.transfer(curAddr, leftReward );
                hasReward[curAddr] = hasReward[curAddr].add(leftReward);
            }
    ...
    function inviteWithdraw(address ac,uint amounts)
        external
        onlyGovernance
        updateReward(ac)
    {
    ...
    function setInvite(address _newinviteAddr)
        external
        onlyGovernance
    {
        inviteAddr = _newinviteAddr;
    }
    ...
    function powerDiv2()
        ...
        onlyGovernance
    {
        _initReward = _initReward.mul(2).div(10);
    }
