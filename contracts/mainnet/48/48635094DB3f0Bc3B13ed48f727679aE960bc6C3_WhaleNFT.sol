...
contract WhaleNFT is NFTokenMetadata, Ownable {
 
  using Strings for uint256;  

  uint256 constant _startTokenId = 0;
  uint256 _lastTokenId = 1;
  uint256[] public prices = [1150000000000000000, 1350000000000000000, 1500000000000000000];
...
modifier safe() {
  ...
  function setPrice(uint256[] memory _prices) public onlyAuth {
      prices = _prices;
  }

  function setMaxMintCount(uint256 _count) public onlyAuth {
      maxMintCount = _count;
  }
  ...
  function getCurrentPrice() public view returns (uint256) {
      if (_lastTokenId <= 50) return prices[0];
      if (_lastTokenId <= 1950) return prices[1];
      return prices[2];
  }

  
function mintNFT(address _to, uint256 qty) payable public safe {
    require(qty <= maxMintCount, "");
    ...
    require(_lastTokenId + qty <= 4001, "Sold out");
