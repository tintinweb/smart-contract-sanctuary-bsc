...
- The winners stake is increased by a percentage on win 
...
contract HoppaTournament {
    ...
    event WinnerPercentageChanged(uint newWinnerPercentage);
    ...
    function setWinnerPercentage(uint winnings) external onlyOwner {
        winnerPercentage = winnings;
        emit WinnerPercentageChanged(winnerPercentage);
    }
    ...
    function selectWinner(address _winner) public onlyGameMaker {
        ...
        require(totalTokensStaked > 0, "Nothing is staked.");
        ...
        uint256 stake = stakedTokens[ _winner ];
        uint256 winnerTakes = stake.mul( winnerPercentage ).div( 100 );
        stake = stake.add( winnerTakes );

        uint256 unstakeAmount = balance.mul( stake ).div( totalTokensStaked );
    ...
    function getPenaltyPercentage() external view returns (uint8) {
        return penaltyPercentage;
    }

    function getWinningsPercentage() external view returns (uint) {
    ...
    function estimateReward() external view returns (uint256) {
        uint256 balance =  IERC20(tokenContract).balanceOf(address(this));
        uint256 stake = stakedTokens[ msg.sender ];
        if( stake == 0 || totalTokensStaked == 0)
            return 0;

        uint256 winnerTakes = stake.mul( winnerPercentage ).div( 100 );
        stake = stake.add(winnerTakes);

        uint256 estimatedAmount = balance.mul( stake ).div( totalTokensStaked );
