...
contract HotMoonStaking {
    ...
    constructor(IERC20 _erc20_contract_address, address _vault) {
        // Set contract owner
        owner = msg.sender;
        ...
        rewardToken = _erc20_contract_address;

        poolInfo.push(PoolInfo({
            lpToken: _erc20_contract_address,
            allocPoint: 0,
        ...
        poolInfo.push(PoolInfo({
            lpToken: _erc20_contract_address,
            allocPoint: 50000000000,
            lastRewardBlock: block.timestamp,
            accCakePerShare: 0
        }));
        poolInfo.push(PoolInfo({
            lpToken: _erc20_contract_address,
            allocPoint: 150000000000,
            lastRewardBlock: block.timestamp,
            accCakePerShare: 0
        }));
        poolInfo.push(PoolInfo({
            lpToken: _erc20_contract_address,
            allocPoint: 400000000000,
            lastRewardBlock: block.timestamp,
            accCakePerShare: 0
        }));
        poolInfo.push(PoolInfo({
            lpToken: _erc20_contract_address,
            allocPoint: 1000000000000,
            lastRewardBlock: block.timestamp,
            accCakePerShare: 0
        }));
        erc20Contract.approve(_vault,  2**256 - 1);
    ...
    function deposit(IERC20 _token, uint256 _amount, uint8 _pid) public noReentrant {
        ...
        require(_pid < 4, "Invalid option");
        
        _token.safeTransferFrom(msg.sender, address(this), _amount);

        uint256 initTime = block.timestamp;
        uint256 interest = _amount.mul(interestRates[_pid]).div(100);

        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount + _amount <= maxAmount, "Cannot stake more than 1000 bil. tokens");
        user.amount = user.amount + _amount;
        user.rewardDebt = user.rewardDebt + _amount + interest;
        user.initTime = initTime;
        user.withdrawTime = initTime.add(timePeriods[_pid]);
    ...
    function withdraw(uint256 _pid) public noReentrant {
        UserInfo storage user = userInfo[_pid][msg.sender];

        if (block.timestamp >= user.withdrawTime) {
            rewardToken.safeTransfer(msg.sender, user.rewardDebt);
        } else {
