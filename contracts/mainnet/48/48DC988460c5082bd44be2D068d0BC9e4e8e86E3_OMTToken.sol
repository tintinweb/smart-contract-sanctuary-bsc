...
contract OMTToken is ERC20, Ownable {
    ...
    uint256 public totalFee = marketingFee + buybackFee + burnFee;
    ...
    event SetFees(uint256 marketingFee, uint256 buybackFee, uint256 burnFee);
    ...
    constructor(
        ...
        address usdt_
    ) ERC20("One More Token", "OMT") {
        marketingWallet = marketingWallet_;
        usdt = usdt_;
        updatePancakeRouter(pancakeRouter_);
        canTransferBeforeTradingIsEnabled[owner()] = true;
        _mint(_msgSender(), 50000000 ether);
    }
    ...
    function updateUsdtAddress(address newUsdt) public onlyOwner {
        require(
            ...
            "PatToken: The usdt already has that address"
        );
        usdt = newUsdt;
    ...
    function setFees(
        ...
        emit SetFees(_marketingFee, _buybackFee, _burnFee);
    }
    ...
    function _transfer(
        ...
        if (!tradingEnabled) {
            require(
                ...
                "PatToken: token not enabled for trading"
            );
        }
        ...
        if (tradingEnabled && !swaping && !isWhitelist) {
            swaping = true;
            {
                ...
                uint256 swapTokensAtAmount = buyBackAmount.add(
                    burnAmount.add(marketingAmount)
                );
    ...
    function mint(address to, uint256 amount) external onlyOwner {
        ...
        if (totalSupply < maxSupply) {
            amount = maxSupply.sub(totalSupply) < amount
                ? maxSupply.sub(totalSupply)
                : amount;
            _mint(to, amount);
        }
    }
