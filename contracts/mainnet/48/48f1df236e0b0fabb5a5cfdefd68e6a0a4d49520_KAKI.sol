...
contract KAKI is NFT {
    using SafeMath for uint256;

    //default resolver address
    address payable _defaultResolverAddress;
    ...
    uint public rentPrices = 5 * 10**18;
     ...
     */
    function setDefaultResolverAddress(address payable defaultResolverAddress_) external virtual onlyOwner {
        _defaultResolverAddress = defaultResolverAddress_;
    }

    function setDivAddress(address divAddress) external virtual onlyOwner {
        _divAddress = divAddress;
    }
    // ------------------------------------------------------send--------------------------------------------------- 
    ...
    function mint() external payable virtual {
        ...
        require(success, "015---send bnb to feeto address fail");
        //NFT
        uint256 tokenId = _addrMint();
        require(tokenId<10000,"token is no more than 10000");

        _tokenIdOfAddress[msg.sender] = tokenId;
        _AddressOfTokenId[tokenId] = msg.sender;
        IKeyResolver(_defaultResolverAddress).addDivNft(_msgSender(),tokenId);
        IDIVC(_divAddress).addDivNft(_msgSender(),tokenId);
    }
    ...
    function batchMint(address account, uint num) external payable virtual onlyWhitelist { 
        ...
        for(uint i = 0; i < num; i++) {
            ...
            IKeyResolver(_defaultResolverAddress).addDivNft(account, tokenId);
            IDIVC(_divAddress).addDivNft(account, tokenId);
        }
    }
    ...
    function transfer(address to, uint tokenId) public virtual {
        //NFT
        super.transferFrom(_msgSender(), to, tokenId);
        require(_transferName(_msgSender(), to, tokenId), "010---transferName fail");
    }

    
    function _transferName(address form_, address to_, uint256 tokenId) internal virtual returns (bool){
        require(_AddressOfTokenId[tokenId]==(msg.sender), "012---form_ is not the owner of name");
        IKeyResolver(_defaultResolverAddress).setDivNft(to_,tokenId);
        IDIVC(_divAddress).setDivNft(to_,tokenId);
        IDIVC(_divAddress).updateDivNft(form_,tokenId);
        return true;
    }
