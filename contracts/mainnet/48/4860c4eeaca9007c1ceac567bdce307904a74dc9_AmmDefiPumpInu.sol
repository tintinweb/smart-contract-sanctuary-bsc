...
This is for all who are unhappy with the whole AMMDEFI scandal. PumpINU is a safe haven for you.

🟠Pumped Developer 
🟠Angrily Active community 
...
Max Bag - 3% (300,000)
Max Buy - 3% (300,000)
Max sell - 1.5% (150,000)
Telegram: https://t.me/AMMdefiPINU
...
contract AmmDefiPumpInu is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    AmmDefiPumpInuDividendTracker private dividendTracker;
    ...
    // Anti Dump //
    uint256 private maxSellAmountPerCycle = 150000 * 10**18;
    ...
    mapping(address => UserLastSell) private userLastSell;
    ...
    bool private limitsInEffect = true; //boolean used to turn limits on and off
    ...
    constructor() ERC20("AmmDefiPumpInu", "AMMDefi") {
        marketingWallet = payable(0x5dDB078B1AA162515D127bCF4F76df3Ac94C1B0F);
        devWallet = payable(0x5dDB078B1AA162515D127bCF4F76df3Ac94C1B0F);
        ...
        dividendTracker = new AmmDefiPumpInuDividendTracker(
            payable(this),
            router,
            0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56,
            "AMMDefiPumpInuTRACKER",
            "AMMDefiPumpInuTRACKER"
        );
        ...
        maxWallet = 300000 * 10**18; // 3%
        maxTransactionAmount = 300000 * 10**18; // 3%
...
contract AmmDefiPumpInuDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    AmmDefiPumpInu public AmmDefiPumpInuContract;
    ...
    constructor(
        ...
        defaultToken = token;
        AmmDefiPumpInuContract = AmmDefiPumpInu(mainContract);
    ...
    function _transfer(
        ...
        require(false, "AmmDefiPumpInu_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "AmmDefiPumpInu_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main AmmDefiPumpInu contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(AmmDefiPumpInuContract);

            uint256 prevBalance = AmmDefiPumpInuContract.balanceOf(address(this));
            ...
            {
                uint256 received = AmmDefiPumpInuContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    AmmDefiPumpInuContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
