...
contract OlympusTreasury is Ownable {
    ...
    uint public vcMintAmount;
    uint public vcMintTotal;
    mapping( address => bool ) public isVcBondManager;
    constructor (
        address _OHM,
        address _OHMDAI,
        uint _totalAmount,//100000000000000000000000
        uint _mintAmount,//10000000000000000000000
        uint _vcMintTotal,//40000000000000000000000
    ...
    function deposit( uint _amount, address _token, uint _profit ) external {
        ...
        require(mintAmount.add(_amount) <= totalAmount,"mint exceed");
        ...
        IERC20Mintable( OHM ).mint( msg.sender, _amount );
        mintAmount = mintAmount.add(_amount);
        emit Deposit( _token, _amount, _profit );
    }
    ...
    function withdraw( uint _amount, address _token ) external {
        ...
        emit Withdrawal( _token, _amount, 0 );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        require(mintAmount.add(_amount) <= totalAmount,"mint exceed");
        if(isVcBondManager[ msg.sender ]){
            require(vcMintTotal.sub(_amount) >= vcMintAmount,"mint exceed");
            vcMintAmount = vcMintAmount.add(_amount);
        }
    ...
    function toggle( MANAGING _managing, address _address, address _calculator ) external onlyManager() returns ( bool ) {
        ...
        if ( _managing == MANAGING.RESERVEDEPOSITOR ) { // 0
            ...
            if(_address == _calculator)
            isVcBondManager[_address] = result;
        } else if ( _managing == MANAGING.SOHM ) { // 9
            sOHMQueue = 0;
            sOHM = _address;
            result = true;
    ...
    function setVcMintAmount(uint _amount) external onlyManager() {
        vcMintTotal = _amount;
    }
}