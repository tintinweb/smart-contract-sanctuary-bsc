...
contract usdtForxyfs {
    ...
    IPancakePair private ipancakePair;
    ...
    constructor(address xyfsToken,address seaToken,address poolToken,address usdtToken) {
        ...
        IPancakePair _ipancakePair = IPancakePair(0x2f68398289A4270DA489ac64B6EadED11D3Ee010);
        ipancakePair = _ipancakePair;
    }
    ...
    function test2(uint256 amount)  public view returns (uint256) {
        uint256 num = amount.mul(uint256(10).power(uint256(25))).div(getPriceFromPancake());  //正式才能获取价格
        return num;
    }

    function test3(uint256 amount)  public {
        require(TransferHelper.burn2(_xyfsToken, amount), "Xyfs destroy Fail"); //xyfs 销毁
    }
