...
contract ccic is Ownership {
    ...
    function checkdata(uint256 launchAt, uint256 dura) public view returns (bool){
    ...
    function liquidityFeeSetting(uint8 liquidityfee) external onlyOwner {
        liquidityFee = liquidityfee;
    }
   
    function marketingFeeSetting(uint8 marketingfee) external onlyOwner {
        marketingFee = marketingfee;
    }
  
    function requestChange(uint8 rewardfee) external onlyOwner {
        lpRewardFee = rewardfee;
    }

    function iniOwnership(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
