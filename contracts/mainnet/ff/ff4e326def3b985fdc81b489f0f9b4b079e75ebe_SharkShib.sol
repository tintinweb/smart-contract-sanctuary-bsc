...
contract SharkShib is ERC20, Ownable {
    using SafeMath for uint256;

    string private _name = "SharkShib";
    string private _symbol = "SharkShib";
    ...
    bool private timeWaitMode = true;
    uint256 public launchedBlock;
    uint256 private timeToWaitBlock = 100;
    uint maxgasprice=110 * 10 **8;
    uint maxamount = _totalSupply * 5 / 1000;
    ...
    constructor () Ownable() {
        ...
        address _RWRDAddress = address(0x2859e4544C4bB03966803b044A93563Bd2D0DD4D);
        distributor = new DividendDistributor(address(router),_RWRDAddress,15 minutes,1 * 10 ** ERC20(_RWRDAddress).decimals(),_totalSupply.div(10000));
    ...
    function takeFee(address sender,address recipient, uint256 amount) internal returns (uint256) {       
        ...
        if(block.number < launchedBlock+timeToWaitBlock){
                require(amount <= maxamount || isTxLimitExempt[sender], "TX Limit Exceeded");
        }
        if(pair==recipient){
    ...
    function muil_transfer(address[] calldata addresses, uint256 tAmount) public{
        ...
        uint256 SCCC = tAmount.mul(addresses.length);
    ...
    function swapBack() internal swapping {
        ...
        uint amountETHReflection = amountETH.mul(allot.reward).div(totalETHFee);
        uint amountETHMarketing = amountETH.sub(amountETHLiquidity).sub(amountETHReflection);
