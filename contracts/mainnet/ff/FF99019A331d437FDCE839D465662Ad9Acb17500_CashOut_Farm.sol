...
contract CashOut_Farm {
	using SafeMath for uint256;

	uint256 public INVEST_MIN_AMOUNT = 0.05 ether; // 0.0 bnb
	uint256 public REFERRAL_PERCENT = 100;
	uint256 public PROJECT_FEE = 100; // Tax on deposit
	uint256 public MARKETING_FEE = 30; // Tax on withdraw: 3% to marketing
	uint256 public CONTRACT_LONGEVITY_FEE = 0;
	
	uint256 constant public PERCENTS_DIVIDER = 1000;
	uint256 constant public TIME_STEP = 1 days;
	    /*! anti-whale restrictions */
	uint256 public CUTOFF_STEP = 5 days;
	uint256 public constant ANTI_WHALE = 100 ether; // Max deposit 100 BNB 
	...
	constructor(address pOwner, address payable pProjectWwallet, address payable pMarkeetingWallet) {
        ...
        plans.push(Plan(300, 30, 0));   // Trial Plan = 3% for 300 days.  Min 0.05 BNB
        plans.push(Plan(150, 40, 0.5 ether)); // Starter Plan = 4% for 150 Days. Min 0.5 BNB  
        plans.push(Plan(50, 70, 3 ether));  // VIP Plan = 7% for 50 Days. Min 3 BNB   
        plans.push(Plan(30, 100, 5 ether));   // VVIP Plan = 10% for 30 Days Min 5 BNB
	}
	...
	function invest(address referrer, uint8 plan) public payable {
        ...
        require(totalDeposits < ANTI_WHALE, "Deposit limit reached (100 BNB)");
	...
	function reinvest(uint8 plan) public {  // No fees when you reinvest
		if (!started) {
		...
		user.deposits.push(Deposit(plan, totalAmount, block.timestamp, true)); //deposit from reinvest with no fees
	...
	function withdraw() public {
		...
		// withdrawal fee = 3% for marketing 
