...
██████╔╝╚█████╔╝╚██████╔╝███████╗╚█████╔╝██║░░██║░░░██║░░░
╚═════╝░░╚════╝░░╚═════╝░╚══════╝░╚════╝░╚═╝░░╚═╝░░░╚═╝░░░
The kindest token on the BSC! Do.Only.Good.Everyday 🐱 🐕 
📱Telegram: https://t.me/Doge_CatBSC
🐦Twitter: https://twitter.com/DogeCat_BSC
🌎Website: https://www.doge-cat.xyz/
...
contract DogecatTokenDividendTracker is DividendPayingToken, Ownable {
  using SafeMath for uint256;
  using SafeMathInt for int256;
  ...
  constructor()
    DividendPayingToken("DogecatToken_Dividend", "DogecatToken_Dividend")
  {
    claimWait = 3600;
...
contract Dogecat is ERC20, Ownable {
  using SafeMath for uint256;
  // Events Declarations
  ...
  event UpdatecharityFee(uint256 indexed newValue, uint256 indexed oldValue);
  ...
  event TransferETHTocharityWallet(address indexed wallet, uint256 amount);
  event TransferTokensTocharityWallet(address indexed wallet, uint256 amount);
  ...
  DogecatTokenDividendTracker public dividendTracker;

  uint256 public immutable MIN_BUY_TRANSACTION_AMOUNT = 10 * (10**18); // 0.000001% of total supply
  uint256 public immutable MAX_BUY_TRANSACTION_AMOUNT = 100000000 * (10**18); // 100% of supply
  uint256 public immutable MIN_SELL_TRANSACTION_AMOUNT = 10 * (10**18); // 0.000001% of total supply
  uint256 public immutable MAX_SELL_TRANSACTION_AMOUNT = 100000000 * (10**18); // 100% of total supply
  uint256 public immutable MIN_SWAP_TOKENS_AT_AMOUNT = 20 * (1**18);
  uint256 public immutable MAX_SWAP_TOKENS_AT_AMOUNT = 5000000 * (10**18); 
  ...
  // DogecatGuard is active on default
  // bool internal DogecatGuardOffline = true;

  address payable public marketingWallet =
    payable(0x547F342072E6153f07ADF433D7F5F3D0A6FC8C37);
  address payable public devWallet =
    payable(0x13f232D9052B1c65c2721429FB7a43852fd3734A);
  address payable public charityWallet =
    payable(0x8916a8c12A2E5EBF0DEc337600efc49b3B821B55);
  ...
  constructor() ERC20("Dogecat", "DC") {
    dividendTracker = new DogecatTokenDividendTracker();
    ...
    excludeFromMaxWallet(charityWallet, true);
  ...
  // function isDogecatGuardEnabled() public view returns (bool) {
  //   return DogecatGuardOffline;
  // }
  ...
  // function shutdownDogecatGuard() external onlyOwner {
  //   require(!DogecatGuardOffline, "Dogecat guard is already offline");
  //   DogecatGuardOffline = true;
  // }
  ...
  function updateDividendTracker(address newAddress) public onlyOwner {
    ...
    DogecatTokenDividendTracker newDividendTracker = DogecatTokenDividendTracker(
      payable(newAddress)
    );
  ...
  function launch() external onlyOwner {
    marketingFee = 3; // normalize the marketing fee
    maxBuyTransactionAmount = 5000000 * (10**18); // 5% of supply
    maxSellTransactionAmount = 5000000 * (10**18); // 5% of supply
    maxWallet = 5000000 * (10**18); // 5% of supply
  }

  function noLimit() external onlyOwner {
    maxBuyTransactionAmount = 100000000 * (10**18);
  ...
  function updatecharityFee(uint256 newFee) external onlyOwner {
    require(charityFee != newFee, "new fee required");
    require(newFee <= MAX_CHARITY_FEE, "new fee too high");
    charityFee = newFee;
    _validateFees();
  }
  ...
  function _transfer(
    ...
    //   require(
    //     DogecatGuardOffline || isWhitelisted[to],
    //     "trading isnt enabled or account isnt whitelisted"
    //   );
    // } else if (to == uniswapV2Pair) {
    //   require(
    //     DogecatGuardOffline || isWhitelisted[from],
    //     "trading isnt enabled or account isnt whitelisted"
    //   );
    // }
    ...
    if (canSwap && !swapping && automatedMarketMakerPairs[to]) {
      ...
      uint256 liquidityAndTeamTokens = contractTokenBalance
        .mul(liquidityFee.add(marketingFee).add(devFee).add(charityFee))
        .div(totalFees);

      if(liquidityFee.add(marketingFee).add(devFee).add(charityFee) > 0){
  ...
  function swapAndLiquifyAndFundTeam(uint256 tokens) private {
    ...
    uint256 mktAndDevAndcharity = marketingFee.add(devFee).add(charityFee);

    uint256 marketingETH = newBalance.mul(marketingFee).div(
      mktAndDevAndcharity
    );
    uint256 charityETH = (newBalance.mul(charityFee).div(mktAndDevAndcharity))
      .div(2);

    uint256 devETH = (newBalance.mul(devFee).div(mktAndDevAndcharity)).add(
      charityETH
    );
