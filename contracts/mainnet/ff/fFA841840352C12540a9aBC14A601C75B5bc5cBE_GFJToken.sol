...
contract GFJToken is ERC20, SafeOwnable {
    using SafeMath for uint256;
    ...
    uint256 public buyFeePercent = (4 * BASE_RATIO) / 100;
    uint256 public sellDeadFeePercent = (2 * BASE_RATIO) / 100;
    uint256 public sellLpPercent = (2 * BASE_RATIO) / 100;
    uint256 public transferDeadFeePercent = (4 * BASE_RATIO) / 100;
    ...
    constructor() ERC20("Greenford Token", "GFJ") {
        _setupDecimals(18); 
        _mint(msg.sender, 1000000 * (10 ** uint256(decimals())));
        address _usdtToken = 0x55d398326f99059fF775485246999027B3197955;
        market = 0x3cadEd32394A136e67c90cfBbEdCc8f4eaa52fc7;
        lpReceiver = 0xD385328b92F845cd8c13B009924E7936a6ac8e24;
        usdtToken = IERC20(_usdtToken);
        IFactory factory = IFactory(0xcA143Ce32Fe78f1f7019d7d551a6402fC5350c73);
        liquidity = factory.createPair(_usdtToken, address(this)); 
    ...
    function setSellDeadFeePercent(uint256 percent) external onlyOwner{
        sellDeadFeePercent = percent;
    }

    function setSellLpPercent(uint256 percent) external onlyOwner{
        sellLpPercent = percent;
    }

    function setTransferDeadFeePercent(uint256 percent) external onlyOwner{
        transferDeadFeePercent = percent;
    }
    ...
    function calculateFee(
        ...
        if(from == liquidity){//buy
            marketFee = amount.mul(buyFeePercent).div(BASE_RATIO);
            ...
            deadFee = amount.mul(sellDeadFeePercent).div(BASE_RATIO);
            liquidityFee = amount.mul(sellLpPercent).div(BASE_RATIO);
        }else{
            deadFee = amount.mul(transferDeadFeePercent).div(BASE_RATIO);
        }
