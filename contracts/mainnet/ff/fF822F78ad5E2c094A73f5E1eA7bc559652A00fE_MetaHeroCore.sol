...
interface MetaHeroNFT {
    function getApproved(uint) external view returns (address);
    function ownerOf(uint) external view returns (address);
}


//Game core logic smart contract
contract MetaHeroCore is ERC721HolderUpgradeable, Ownable , ReentrancyGuard{
    using EnumerableSet for EnumerableSet.AddressSet;
    EnumerableSet.AddressSet private _governors;

    EnumerableSet.AddressSet private _blackList;

    MetaHeroNFT public metaHeroNFT ;

    address public profitToken ;
    address public gameToken;

    //define hero join time
	struct HeroInfo {
        uint heroId;
        uint joinTime;
    }
    
    mapping(address => HeroInfo[] ) public playerHeroes;

    // Control gaming
    bool public paused = false;

    event HeroJoin(address heroOwner, uint heroId, uint joinTime);
    event HeroExit(address heroOwner, uint heroId, uint exitTime );

    event GovernorAdded( address _user);
    event GovernorDeleted( address _user);
    event BlacklistAdded( address _user );
    event BlacklistDeleted( address _user );
    event GameStatusChanged( bool _paused );

    constructor(MetaHeroNFT _metaHeroNFT)  {
        metaHeroNFT = _metaHeroNFT;
    }

    function setMetaHeroCraftNFT( address _metaHeroNFT ) public onlyOwner {
        metaHeroNFT = MetaHeroNFT(_metaHeroNFT) ;
    }

    function setTokenAddress( address _profitToken, address _gameToken ) public onlyOwner {
        profitToken = _profitToken;
        gameToken = _gameToken;
    }

    function _isApprovedOrOwner(address _address, uint _heroIndex) internal view returns (bool) {
        return metaHeroNFT.getApproved(_heroIndex) == _address || metaHeroNFT.ownerOf(_heroIndex) == _address;
    }


    function joinWar(uint[] memory heroIds ) public notPause{
        for( uint index = 0; index < heroIds.length; index ++ ){
            uint _heroId = heroIds[ index ];
            if( metaHeroNFT.ownerOf( _heroId ) == msg.sender ){

                //transfer hero
                IERC721Upgradeable( address(metaHeroNFT)).safeTransferFrom( _msgSender(), address(this), _heroId );

                playerHeroes[ msg.sender ].push(HeroInfo({heroId: _heroId, joinTime: block.timestamp}));

                emit HeroJoin( msg.sender, _heroId, block.timestamp );
            }
        }
    }

    function exitWar(uint[] memory heroIds ) public notPause{
        for( uint index = 0; index < heroIds.length; index ++ ){
            uint _heroId = heroIds[ index ];

            HeroInfo[] memory _info = playerHeroes[ msg.sender ];

            for( uint i = _info.length - 1 ; i >= 0; i -- ){
                if( _info[ i ].heroId == _heroId ){
                    if( metaHeroNFT.ownerOf( _heroId ) ==  address(this) ){
                        //transfer hero
                        IERC721Upgradeable( address(metaHeroNFT)).safeTransferFrom( address(this), _msgSender(), _heroId );

                        for (uint j = i; j < _info.length - 1; j++) {
                            playerHeroes[msg.sender][ j ] = playerHeroes[msg.sender][ j + 1];
                        }

                        playerHeroes[msg.sender].pop();

                        emit HeroExit( msg.sender , _heroId, block.timestamp );
                    }
