...
contract MainToken is ERC20PresetMinterPauserUpgradeable, OwnableUpgradeable {
    ...
    function initialize(uint256 _unlockTime, uint256 _unlockPerSecond) initializer public {
        ...
        __ERC20PresetMinterPauser_init("STEP COIN", "STEP");
        __Ownable_init();
    }
    ...
    function transferBatchTokenLock(Airdrop[] memory airdrops) payer public {
        for (uint256 i = 0; i < airdrops.length; i++) {
            transferTokenLock(airdrops[i].wallet, airdrops[i].amount);
        }
    }
