...
contract TimelockStakeV3
{
    ...
    *  **********************************************************/
    ...
    function getLPPrice(address lpToken, uint256 amountOfLPToken, address priceInToken) public view returns (uint256 result)
    {
        uint lpDecimals = IERC20(lpToken).decimals();
        uint lpTotalSupply = IERC20(lpToken).totalSupply();
        ...
        // Calc: LPUnitPrice = (reserve0Price + reserve1Price) / lpTotalSupply
        uint256 LPUnitPrice = lpTotalSupply > 0 ? SafeMath.safeDivFloat((reserve0Price + reserve1Price), lpTotalSupply, lpDecimals) : reserve0Price + reserve1Price;
