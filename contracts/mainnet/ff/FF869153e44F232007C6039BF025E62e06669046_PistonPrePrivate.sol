...
contract PistonPrePrivate is OwnableUpgradeable {
    ...
    */
    ...
    function initialize() external initializer {
        ...
        token_BUSD = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
        BUSD = IERC20(token_BUSD);

        maxUserDeposits = 1600000000000000000000; // 1600 BUSD
        pistonTokenPrice = 0;
    ...
    function deposit(
        ...
        uint256 userSpecialLimit = whitelist[msg.sender]; // 0 = not whitelisted, 1 = whitelisted with maxUserDeposits, higher = special deposit limit
    ...
    function claim() external {
        ...
        uint256 myTokenAmount = myDeposits.mul(1 ether).div(pistonTokenPrice);
    ...
    function getHardCap() public view returns(uint256) {
        ...
        for(uint256 i = 0; i < spots; i++) {
            ...
            }else if(whitelist[whitelistedWallet] == 0){
                limit = 0;
            }
    ...
    function setPresaleWallet(address value) external onlyOwner{
    ...
    function addWalletToWhitelist(address wallet, uint256 value) external onlyOwner {
        require(value > 0, "zero limit not allowed");
    ...
    function resetWallet(address wallet) external onlyOwner{
        uint256 investedAmount = deposits[wallet];

        totalInvested_BUSD = totalInvested_BUSD.sub(investedAmount);            
        deposits[wallet] -= investedAmount;
        whitelist[wallet] = 0;
    }
