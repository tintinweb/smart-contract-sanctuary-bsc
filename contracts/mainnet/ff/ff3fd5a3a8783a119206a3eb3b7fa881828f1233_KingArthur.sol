...
contract KingArthur is Context, IERC20, Ownable {
    ...
    function _takeFee(Param memory param,address from, address to, uint amount) private {
      ...
      if( param.tRest > 0 ){
        _take(param.tRest, from, address(this));
      }
      if( param.tInter > 0 ){
    ...
    function _transfer(address from,address to,uint256 amount) private {
        ...
        if (!isContract(from) && !isContract(to)) {
            if (relationAddress[to] == address(0)) {
                relationAddress[to] = from;
            }
            bool shouldSetInviter = inviter[to] == address(0) && from != uniswapV2Pair && to != uniswapV2Pair && relationAddress[to] == from && relationAddress[from] == to;
            if (shouldSetInviter) {
                inviter[to] = from;
            }
        }
