...
contract Staking is IStaking, Initializable {
    ...
    /**
     ...
     @param _depositAmount[][] : EBSC amount required for different levels & tiers
     */
    function initialize(
        address _token,
        address _admin,
    ...
    */
    ...
    /**
     @dev Transferring reflection to Airdrop
     @param _amount Amount to transfer  
     */
    function transferReflection(uint256 _amount) internal {
        if (_amount != 0) {
    ...
    /**
    @dev returns user staked amount
    ...
    function stake(uint256 _level, uint256 _amount) external payable {
        ...
        if (s.lock == 4 && highestTier == 7) {
            if (prevLock != 4 || prevTier != 7) {
                IAirdrops(admin.airdrop()).setShareForBNBReward(
                    msg.sender,
                    s.amount
                );
    ...
    /**
     @dev Distribute BNB and EBSC rewards when epoch is reached
     */
    function distributeRewards() public {
        if (IAirdrops(admin.airdrop()).checkEpoch()) {
            IAirdrops(admin.airdrop()).setEpoch();
            uint256 reflection = getReflection();
    ...
    */
        ...
        onlyOperator
    {
        // for (uint8 i = 0; i < uint8(_depositAmount.length); i++) {
        //     for (uint8 j = 0; j < uint8(_depositAmount[i].length); j++) {
        //         tiers[i + 1][j + 1].amount = uint128(_depositAmount[i][j]);
        //     }
        // }
    }

    function setDepositByTier(uint256 _lockLevel, uint256 _tier, uint256 _amount) external onlyOperator {
        tiers[_lockLevel][_tier].amount = uint128(_amount);
    }

    function setAllocationByTier(uint256 _lockLevel, uint256 _tier, uint256 _amount) external onlyOperator {
        tiers[_lockLevel][_tier].allocations = uint128(_amount);
    }
