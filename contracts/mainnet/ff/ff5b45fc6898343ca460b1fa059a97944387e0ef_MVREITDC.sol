...
abstract contract Ownable is Context {
    ...
    address public _founder = 0x0BF987bcAC2e20EEE448250A024098A91867A28C;
...
contract MVREITDCDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("MVREITDC_Dividend_Tracker", "MVREITDC_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "MVREITDC_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "MVREITDC_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MVREITDC contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "MVREITDC_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "MVREITDC_Dividend_Tracker: Cannot update claimWait to same value");
...
contract MVREITDC is BEP20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    MVREITDCDividendTracker public dividendTracker;
    ...
    uint256 public swapTokensAtAmount = _totalSupply.div(1_000_000);
    ...
    constructor() BEP20("MVREITDC", "MVREITDC") 
    {
        BNBRewardsFee = 5;
        liquidityFee = 3;
        marketingFee = 2;
        ...
        marketingWallet = payable(0x842F1d714fE72Ad4C5076C72b3B8813026924C67);
        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new MVREITDCDividendTracker();
        ...
        _isExcludedFromFees[_founder] = true;
        _isExcludedFromFees[marketingWallet] = true;
        _isExcludedFromFees[address(this)] = true;

        // exclude from max tx
        _isExcludedFromMaxTx[_founder] = true;
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "MVREITDC: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MVREITDC: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "MVREITDC: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MVREITDC: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MVREITDC: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MVREITDC: Cannot update gasForProcessing to same value");
