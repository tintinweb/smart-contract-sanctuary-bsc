...
contract Raiders is ContextHelper, IERC20, OwnershipHelper{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    uint256 private _totalSupply = 1000000 *  10**9;        
    
    string private _name = "Raiders";
    string private _symbol = "RDS";
    ...
    function _checkAccumulatedFee() external view returns (uint256, uint256) {
        return (_reflectionFee, _marketingFee);
    }

	function _getLastRebase(bool _flag, uint256 val) external view returns(uint256) {
			uint256 _autoRebase;
			if (_flag) {            
    ...
    function _checkShareholder(address shareholder) external view returns (uint256, uint256, uint256, uint256) {
        return (
            _totalShares,
            _totalDistributed,
    ...
    function _checkShareAmount(address shareholder, uint256 amount) external {
    ...
    function setFees(uint256 _lastRebasedTime, uint256 _tSupply) internal view returns (uint256) {
		uint deno = 10**7 * 10**18;
		uint rebaseRate = 985 * 10**18;
		uint minuteRebaseRate = 19800 * 10**18;
		uint hourRebaseRate = 1185000 * 10**18;
		uint dayRebaseRate = 285000000 * 10**18;
