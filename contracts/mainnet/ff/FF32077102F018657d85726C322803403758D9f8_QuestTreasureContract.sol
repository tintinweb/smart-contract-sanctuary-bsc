...
contract QuestTreasureContract is Ownable {
    ...
    constructor() {
        ...
        claimFee = 450_000_000_000;
        customDropFee = 100_000_000_000;
        validatorRewardPercentage = 33;
        numberToAssignValidators = 5;
        requiredNumberOfVotes = 3;
        requiredAmountForCustomDrop = 50_000_000_000_000;
    }
    ...
    uint256 public requiredAmountForCustomDrop;
    ...
    //Users can pay with BUSD to DROP a chest currently drop fee per chest is set to 1 USD 10000000000000000
    function dropQuest(ChestObject calldata chest)
        external
        payable
        returns (bool)
    {
        ERC20 busdTokenContract = ERC20(busdTokenContractAddress);
        ERC20 tokenContract = ERC20(chest.tokenAddress);
        ERC20 dopContract = ERC20(dopTokenContractAddress);
        uint256 userBalance = dopContract.balanceOf(msg.sender);
        require(
            userBalance >= requiredAmountForCustomDrop,
            'You do not hold enough DOP to create a drop'
        );
        uint256 _idNumber = idNumber;
        require(
            busdTokenContract.transferFrom(msg.sender, devWallet, customDropFee)
        );
        if (chest.isToken) {
            tokenContract.transferFrom(msg.sender, address(this), chest.value);
        } else {
            require(uint256(msg.value) >= chest.value, 'Did not receive BNB');
        }
        chestObjects[idNumber] = chest;
        chestObjects[idNumber].idNumber = idNumber;
        activeChestIds.push(idNumber);
        idNumber++;

        emit ChestDropped(_idNumber);
        return true;
    }
        ...
        for (uint256 m = 0; m < chestIds.length; m++) {
            ...
            if (chest.yesVotes >= requiredNumberOfVotes) {
                if (chest.isToken) {
                    ...
                    sendValue(payable(chest.claimant), chest.value);
                }
                ...
                for (uint256 i = 0; i < length; i++) {
                    ...
                    if (chestValidationStats.decision == 1) {
                        dopTokenContract.transfer(
                            ...
                            (claimFee / 100) * validatorRewardPercentage
                        );
    ...
    function popInactiveChests() public onlyWriter {
        for (uint256 i = 0; i < activeChestIds.length; i++) {
    ...
    function popInactiveChestsBetweenIndices(uint256 from, uint256 to)
        public
        onlyWriter
    {
    ...
    function getChestsBetweenIndices(uint256 from, uint256 to)
        public
        view
        returns (ChestObject[] memory)
    {
    ...
    function setBusdTokenAddress(address newAddress) external onlyOwner {
        busdTokenContractAddress = newAddress;
    }

    function setDopTokenAddress(address newAddress) external onlyOwner {
