...
contract EgyptLand is ERC721, ERC721Enumerable, AccessControl, IERC721_EXT {
    ...
    string public uniqueURI;
    mapping (uint => string) public _tokenURIs;
    IERC20 public token;

    constructor() ERC721("Egypt Land", "Egypt Land") {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(MINTER_ROLE, msg.sender);
        _grantRole(OWNER_ROLE, msg.sender);
        token = IERC20(0x7430b4D4DEf23f2055f7df3679c6dd7bcEf11D12);
    }
    ...
    function tokensByOwner(address owner) external view  override returns (uint256[] memory) {
    ...
    function tokenURI(uint tokenId) public view virtual override returns (string memory) {
        ...
        string memory _uniqueURI;
        if (bytes(uniqueURI).length > 0) {
            _uniqueURI = uniqueURI;
        } else {
            _uniqueURI = _tokenURIs[tokenId];
        }
        return bytes(baseURI_).length > 0 ? string(abi.encodePacked(baseURI_, _uniqueURI)): "";
    }
    ...
    function setUniqueURI(string memory URI) public onlyOwner {
        uniqueURI = URI;
    }

    uint public dividendPerToken;
    uint public pendingDividend;
    mapping(address => uint) public dividendsBalanceOf;
    mapping(address => uint) public dividendsCreditedTo;
    mapping(address => uint) public totalWithdrawn;

    function update(address _user) internal {
        uint debit = dividendPerToken - dividendsCreditedTo[_user];
        dividendsBalanceOf[_user] += balanceOf(_user) * debit;
        dividendsCreditedTo[_user] = dividendPerToken;
    }
    ...
    function desposit(uint _amount) public {
        require(_amount > 0, "amount must be greater than 0");
        uint _oldBalance = token.balanceOf(address(this));
        token.transferFrom(msg.sender, address(this), _amount);
        uint _newBalance = token.balanceOf(address(this));
        uint deposit = _newBalance.sub(_oldBalance, "insufficient amount");
        deposit += pendingDividend;
        uint delta = deposit / totalSupply();
        if(delta == 0) {
            pendingDividend = deposit;
        }
        else {
            pendingDividend += delta;
        }
    }

    function withDraw() public {
        uint _tokenBalance = getTokenBalance();
        require(_tokenBalance > 0, "insufficient token balance");
        update(msg.sender);
        uint amount = dividendsBalanceOf[msg.sender];
        require(amount > 0, "you have no dividends");
        delete dividendsBalanceOf[msg.sender];
    ...
    function canWithdraw(address _user) public view returns (bool) {
        ...
        uint debit = dividendPerToken - dividendsCreditedTo[_user];
        uint _amount = balanceOf(_user) * debit;
        _amount += dividendsBalanceOf[_user];
        return _amount > 0;
    }
