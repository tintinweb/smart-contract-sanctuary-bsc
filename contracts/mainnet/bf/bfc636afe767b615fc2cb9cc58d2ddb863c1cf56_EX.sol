...
contract EX is ERC20, Ownable {
    ...
    EXDividendTracker public dividendTracker;
    ...
    constructor() ERC20("ELONX", "EX") {
        marketingWallet = payable(0xb582bA022346F37a6900E2A3Af12A4781A6FA3be);
        devWallet = payable(0x5AE3163c1EF89387C8fA21f7d722D5e58452934a);
        ...
        totalSellFees = sellRewardsFee
         ...
         dividendTracker = new EXDividendTracker(
            payable(this),
            router,
...
contract EXDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function _transfer(
        ...
        require(false, "EX_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "EX_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EX contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(EXContract);

            uint256 prevBalance = EXContract.balanceOf(address(this));
            ...
            {
                uint256 received = EXContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    EXContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
