...
// File: IAddLiquidityContract.sol
...
contract TTS is ERC20, Ownable {
    ...
    address public oldContract = 0x0EA5cB259a20C2ac68047770926eA8312C328588; ////@todo changes
    address private feewallet;
    ...
    uint256 private _tokenAirDrop = 2 * 10**10 * 10**18; // 4%
    ...
    constructor(
        ...
        feewallet = 0x7b990e9A845357c15162D68d96ffF7f4b8bE93A3;
        ...
        _mint(_farmingTreasury, _tokenFarmingTreasury);
    ...
    function _swapBack(uint256 amountToSwap) private {
        _getFee(false);
        router.swapExactTokensForTokens(amountToSwap, 0, path, feeWallet, block.timestamp);

        uint256 oldContractPart = (busd.balanceOf(feeWallet) * 500) / 825;
        uint256 liquidityPart = (busd.balanceOf(feeWallet) * 125) / 825;
        uint256 marketingPart = busd.balanceOf(feeWallet) - oldContractPart - liquidityPart;

        busd.transferFrom(feeWallet, oldContract, oldContractPart);
        busd.transferFrom(feeWallet, marketing, marketingPart);
        busd.transferFrom(feeWallet, address(addLiquidityContract), liquidityPart);
