...
contract TrialToken is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public maxSellTransactionAmount = 5000 * (10**9); // 0.5%
    uint256 public maxWalletBalance = 20000 * (10**9); // 2%
    uint256 public swapTokensAtAmount = 20000 * (10**9); // 2%

    // buy fees
    uint256 public buyDividendRewardsFee = 8;
    uint256 public buyStakingFee = 0;
    ...
    uint256 public sellStakingFee = 0;
    ...
    // Bot fees
    uint256 public sellDividendRewardsBotFee;
    uint256 public sellStakingBotFee;
    uint256 public sellLiquidityBotFee;
    uint256 public sellBurnBotFee;
    uint256 public sellMarketingBotFee;
    uint256 public sellTotalBotFees;
    ...
    constructor() ERC20("Trial Token", "TTK") {
        dividendTracker = new TTKTokenDividendTracker();

        stakingWallet = 0x648f87caE819795Ec781CCAeaF193c1Cc81de105;
        marketingWallet = 0xe80c4a903A5eA98F8F82cd171201922fe4700eE6;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "TTK: The dividend tracker already has that address"
        );
        ...
        require(
            ...
            "TTK: The new dividend tracker must be owned by the TTK token contract"
        );
    ...
    function updateSellBotFees(
        ...
        sellDividendRewardsBotFee = reward;
        sellStakingBotFee = staking;
        sellLiquidityBotFee = liquidity;
        sellBurnBotFee = burn;
        sellMarketingBotFee = marketing;
        sellTotalBotFees = reward
            .add(liquidity)
            .add(burn)
            .add(marketing)
            .add(staking);
        require(sellTotalBotFees < 100, "Fees too high");
    }

    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            newAddress != address(uniswapV2Router),
            "TTK: The router already has that address"
        );
    ...
    function updateStakingAddress(address newAddress) public onlyOwner {
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "TTK: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "TTK: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "TTK: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "TTK: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "TTK: Cannot update gasForProcessing to same value"
        );
    ...
    function _transfer(
        ...
        if (sobActive && !_bot[to]) {
            if (automatedMarketMakerPairs[from] && 
                ((block.number < startBlockNumber + blocksToWait) || (tx.gasprice > gasPriceLimit) || (lastTrade[to] == block.number)) &&
                !_isExcludedFromFees[to]) {
                _bot[to] = true;
        ...
        if (tradingIsEnabled && automatedMarketMakerPairs[to]) {
            ...
            if (!swapping && canSwap) {
                ...
                uint256 bnbSwap = buyStakingFee.add(
                    buyMarketingFee
                );
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[to]) {
                ...
                if (sobActive && _bot[from]) {
                    fees = amount.div(100).mul(sellTotalBotFees);
                }
                if (sellBurnFee > 0) {
    ...
    function swapAndSendBnb(uint256 tokenAmount, uint256 totalFee) private {
        ...
        transferToWallet(
            payable(stakingWallet),
            address(this).balance.div(totalFee).mul(buyStakingFee)
        );
        transferToWallet(
...
contract TTKTokenDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()
        DividendPayingToken("TTK_Dividend_Tracker", "TTK_Dividend_Tracker")
    {
        claimWait = 3600;
    ...
    function _transfer(
        ...
        require(false, "TTK_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "TTK_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TTK contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "TTK_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "TTK_Dividend_Tracker: Cannot update claimWait to same value"
        );
