...
contract BNBPosiVault is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, VestingScheduleLogic {
    ...
    */
    function getLPTokenByAmount0(uint256 amount) public view returns (uint256 lpNeeded) {
    ...
    function compound() external updateReward(address(0)) nonReentrant {
        ...
        if (_canCompound) {
            ...
            uint256 amountCollected = posi.balanceOf(address(this)) * 99 / 100;
    ...
    function pendingPositionNextCompound() public view returns (uint256){
        return posiStakingManager.pendingPosition(POSI_BNB_PID, address(this)).mul(5).div(100);
    }
    ...
    // @dev this function is for debug tracing
    // will remove in the next version
    function mockVestingTime(address user, uint8 freq, uint256 index, uint256 timestamp) public onlyOwner {
        _setVestingTime(user, freq, index, timestamp);
    }

    function migrateFarm() public onlyOwner {
        IPosiStakingManager oldStakingManager = IPosiStakingManager(0x0C54B0b7d61De871dB47c3aD3F69FEB0F2C8db0B);
        (uint256 stakingAmount,,,) = oldStakingManager.userInfo(POSI_BNB_PID, address(this));
        // withdraw old one
        oldStakingManager.withdraw(POSI_BNB_PID, stakingAmount);
        // deposit new one
        posiStakingManager.deposit(POSI_BNB_PID, stakingAmount, address(vaultReferralTreasury));
    }
    ...
    function claimVesting(VestingFrequencyHelper.Frequency freq, uint256 index) public override nonReentrant {
        super.claimVesting(freq, index);
    }

    function claimVestingBatch(VestingFrequencyHelper.Frequency[] memory freqs, uint256[] memory index) public override nonReentrant {
