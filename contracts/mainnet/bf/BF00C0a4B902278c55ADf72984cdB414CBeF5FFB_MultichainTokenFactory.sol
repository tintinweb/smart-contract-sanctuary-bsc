...
/*
 * The main purpose of the MultichainTokenFactory is to allow MultichainTokens that have the exact same
 * address on different blockchains in a more automated way such that BonfireTokenWrapper can trust the
 * outcome. There can only be one Token, but it acts differently on source or other chains.
 */

contract MultichainTokenFactory is IMultichainTokenFactory {
    address public constant wrapper = address(0xBF00C4F91B8bB3DD00aF4dD5922bBc8c7f3aCAF4);
    ...
    function _multichainTokenAddress(bytes32 salt)
        internal
        view
                ...
                uint256(
                    keccak256(
                        abi.encodePacked(
                            ...
                            address(this),
                            salt,
                            keccak256(type(MultichainToken).creationCode)
                        )
                    )
                )
            )
    ...
    function getMultichainToken(
        ...
        multichainToken = _multichainTokenAddress(salt);
        uint32 size;
        assembly {
            size := extcodesize(multichainToken)
        }
        if (size == 0)
            _createMultichainToken(
                creator,
                sourceToken,
                sourceMaxSupply,
                chainid,
    ...
    function _createMultichainToken(
        address creator,
        address sourceToken,
        uint256 sourceMaxSupply,
        uint256 chainid,
        ...
        MultichainToken(multichainToken).initialize(
            creator,
            sourceToken,
            sourceMaxSupply,
            wrapper,
        ...
        _finishCreation(creator, sourceToken, chainid, multichainToken);
    }
    ...
    function _finishCreation(
        ...
        IBonfireTokenTracker(tracker).registerToken(targetToken);
        IBonfireTokenWrapper(wrapper).addMultichainToken(targetToken);
        emit TokenCreation(creator, sourceToken, sourceChainid, targetToken);
    }
}
    ...
    IMultichainToken
{
    ...
    address public override wrapper; //TODO
