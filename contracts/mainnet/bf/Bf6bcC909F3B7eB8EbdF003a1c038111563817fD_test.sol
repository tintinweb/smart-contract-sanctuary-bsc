...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 bnb = IBEP20(0x0d17d0bEc371DC3e6774Dc11809E525132353c15);
...
contract test is IBEP20, Auth {
    ...
    address bnb = 0x2859e4544C4bB03966803b044A93563Bd2D0DD4D;
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBTeam = amountBNB.mul(CharityFee).div(totalBNBFee);

        try distributor.deposit{value: amountBNBReflection}() {} catch {}
        (bool tmpSuccess,) = payable(marketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
        (tmpSuccess,) = payable(marketingcReceiver).call{value: amountBNBGame, gas: 30000}("");
        (tmpSuccess,) = payable(CharityFeeReceiver).call{value: amountBNBTeam, gas: 30000}("");
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _marketingc, uint256 _CharityFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        reflectionFee = _reflectionFee;
        marketingFee = _marketingFee;
        marketingc = _marketingc;
        CharityFee = _CharityFee;
        totalFee = _liquidityFee.add(_reflectionFee).add(_marketingFee).add(_marketingc).add(_CharityFee);
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator/4);
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _marketingcReceiver, address _CharityFeeReceiver) external authorized {
