...
contract FireCatGate is IFireCatGate, FireCatTransfer, ModifyControl {
    ...
    */
    ...
    /// @inheritdoc IFireCatGate
    function setFireCatTreasury(address fireCatTreasury_) external onlyRole(DATA_ADMIN) {
        fireCatTreasury = fireCatTreasury_;
    ...
    /// @inheritdoc IFireCatGate
    function migrate(uint256 tokenId, address vaultFrom, address vaultTo) external beforeMigrate isBanned(msg.sender) {
        require(IFireCatNFT(fireCatNFT).ownerOf(tokenId) == msg.sender, "GATE:E00");
        ...
        IFireCatVault(vaultFrom).migrateOut(msg.sender, tokenId, stakeAmount);    
    ...
    /// @inheritdoc IFireCatGate
    function destroy(uint256 tokenId) external beforeDestroy isBanned(msg.sender) nonReentrant {
        require(IFireCatNFT(fireCatNFT).ownerOf(tokenId) == msg.sender, "GATE:E00");
        ...
        address treasuryToken = IFireCatTreasury(fireCatTreasury).treasuryToken();
        uint256 actualTreasuryAmount = 0;
        if (IFireCatTreasury(fireCatTreasury).treasuryOf(tokenId) > 0) {
            uint256 swapTeasuryAmount = IFireCatTreasury(fireCatTreasury).swapTreasury(tokenId);
        ...
        fireCatNFT.transferFrom(msg.sender, address(1), tokenId);
