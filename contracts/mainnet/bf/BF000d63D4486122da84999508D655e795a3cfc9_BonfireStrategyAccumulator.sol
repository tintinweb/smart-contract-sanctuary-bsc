 ...
 * Strategies are dedicated to a single Token and need to implement IBonfireStrategicCalls interface.
 * AlwaysStrategies will be executed at any call (up to maxCount many of them from first to last)
 * RandomStrategies will be executed randomly (up to the remaining from maxCount)
 * It's up to the admin to make sure that strategies are either 'always' or 'random', the contract doesn't check that.
 * The only problem with strategies occuring in both though is that the quote will most likely not be accurate anymore.
 */
