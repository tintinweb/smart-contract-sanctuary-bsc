...
contract FarmRank is Recoverable {
    using SafeERC20 for IERC20Ex;

    // profit to rank users
    function profit(address token, uint256 amount, address[] calldata rankAddrs) public onlyOwner {
        require(amount>0, "amount can not be zero");
        require(rankAddrs.length>0, "rankAddrs.length can not be zero");
        require(IERC20Ex(token).balanceOf(address(this)) >= amount, "balance insufficient");

        uint256 profitAmount = amount / rankAddrs.length;

        for(uint16 i = 0; i < rankAddrs.length; i++) {
            IERC20Ex(token).safeTransfer(rankAddrs[i], profitAmount);
        }
    }
}
