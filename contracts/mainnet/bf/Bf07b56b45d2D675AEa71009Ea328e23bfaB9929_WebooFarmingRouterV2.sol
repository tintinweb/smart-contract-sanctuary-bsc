...
import "./interfaces/IWebooFarmingV2.sol";

contract WebooFarmingRouterV2 is Context, Auth {
    ...
    constructor(address _routerDexAddress,address _wbnbAddress) Auth(msg.sender) {
    ...
    function addFarmingETH(address farmingAddress, uint256 amountA, address recipient) external payable {
        ...
        IERC20(IWebooFarmingV2(farmingAddress).tokenA()).transferFrom(_msgSender(),address(this),amountA);

        // add farming
        IERC20(IWebooFarmingV2(farmingAddress).tokenA()).approve(farmingAddress,amountA);
        IERC20(IWebooFarmingV2(farmingAddress).tokenB()).approve(farmingAddress,balanceEthDiff);
        IWebooFarmingV2(farmingAddress).addFarming(amountA,balanceEthDiff,recipient);
    }

    function addFarming(address farmingAddress, uint256 amountA, uint256 amountB, address recipient) external payable {
        // send weboo to router
        IERC20(IWebooFarmingV2(farmingAddress).tokenA()).transferFrom(_msgSender(),address(this),amountA);
        // send token to router
        IERC20(IWebooFarmingV2(farmingAddress).tokenB()).transferFrom(_msgSender(),address(this),amountB);
        // add farming
        IWebooFarmingV2(farmingAddress).addFarming(amountA,amountB,recipient);
    }

    function removeFarmingETH(address farmingAddress, uint256 amountRemove, address recipient) external {
        uint256 balanceTokenABefore = IERC20(IWebooFarmingV2(farmingAddress).tokenA()).balanceOf(address(this));
        uint256 balanceTokenBBefore = IERC20(IWebooFarmingV2(farmingAddress).tokenB()).balanceOf(address(this));

        //send LP to router
        IERC20(farmingAddress).transferFrom(_msgSender(),address(this),amountRemove);
        IWebooFarmingV2(farmingAddress).removeFarming(amountRemove);

        uint256 balanceTokenAAfter = IERC20(IWebooFarmingV2(farmingAddress).tokenA()).balanceOf(address(this));
        uint256 balanceTokenBAfter = IERC20(IWebooFarmingV2(farmingAddress).tokenB()).balanceOf(address(this));
        ...
        IERC20(IWebooFarmingV2(farmingAddress).tokenA()).transfer(recipient,balanceTokenADiff);
    ...
    function calculateShare(address farmingAddress) external {
        IWebooFarmingV2(farmingAddress).calculateShare();
