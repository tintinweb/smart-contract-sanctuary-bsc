...
// File contracts/v1contracts/dependencies/UpgradeableMiningV2Base.sol
// License-Identifier: MIT
...
interface IUpgradeableMiningV2Base {
    struct UserInfo {
...
abstract contract UpgradeableMiningV2Base is Proxiable, IUpgradeableMiningV2Base {
...
// File contracts/v1contracts/MiningWithTokenV2.sol
// License-Identifier: MIT
...
contract MiningWithTokenV2 is UpgradeableMiningV2Base {
    string public constant contractName = "MiningWithTokenV2";
    string public constant contractVersion = "1.0";

    function getStakeContractBalance() public view returns (uint256 _stakeBalance) {
        return getBalance(address (pools[0].stakeToken));
    }

    function getStakeTargetAmount() public view returns (uint256 _stakeBalance) {
        (_stakeBalance,,) = ITarget(pools[0].target).userInfo(pools[0].pid, address(this));
    }

    function getStakeTargetPending() public view returns (uint256 _stakeBalance) {
        return ITarget(pools[0].target).pendingCake(pools[0].pid, address(this));
    }

    function getStakeTotalBalance() public view returns (uint256 _stakeBalance) {
        return getStakeContractBalance().add(getStakeTargetAmount());
    }


    modifier doReinvest() {
        ITarget(pools[0].target).withdraw(pools[0].pid, getStakeTargetAmount());
        _;
        ITarget(pools[0].target).deposit(pools[0].pid,getStakeContractBalance());
    }
    ...
    function initialize(
        ...
        pools[0].stakeToken = _stakeToken;
        pools[0].rewardToken = _rewardToken;
    ...
    {
        _stakeToken = address(pools[0].stakeToken);
        _rewardToken = address(pools[0].rewardToken);
    ...
    function getTargetReward() public view override returns (uint256) {
        return getStakeTargetPending().add(getBalance(ITarget(pools[0].target).CAKE()));
    }

    function withdrawTargetReward(uint256 _amount)
    external
    override
    onlyOwner
    doReinvest
    {
        ...
        require(IERC20(ITarget(pools[0].target).CAKE()).transfer(pools[0].receiver, _amount), "Target reward transferfailed!!");
    }
