...
contract BonfireMetaRouter is IBonfireMetaRouter, Ownable {
    ...
    address public constant override WETH = address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);
    address public constant override tracker = address(0xBF0089F09D4D90BB51b098F09e05eb40C641627a);
    address public constant override logbook = address(0xBF002628fE935c3B81B7AD22D5135b1AEE7b6891);
    address public constant override wrapper = address(0xBF00C4F91B8bB3DD00aF4dD5922bBc8c7f3aCAF4);
    address public constant override paths = address(0xBF00dee4A780037E64909D7D82f7dB3F5beabB95);
    address public accumulator;
    uint256 public maxAccumulationCount;
    address public constant defaultToken = address(0x5e90253fbae4Dab78aa351f4E6fed08A64AB5590);
        ...
        for (uint256 i = 0; i < poolPath.length; i++) {
            if (poolPath[i] != wrapper) {
                ...
                (rA, rB) = IBonfirePair(poolPath[i]).token0() == tokenPath[i]
                    ? (rA, rB)
                    : (rB, rA);
                uint256 increase = ((rA + amount) * (rA + amount) * 1000) /
                    (rA * rA);
            ...
            address target = i < poolPath.length - 1
                ? poolPath[i + 1]
                : msg.sender;
            amount = IBonfireRouterPaths(paths).quote(
                poolPath[i:i],
                tokenPath[i:i + 1],
                amount,
                target,
    ...
    function setAccumulator(address _accumulator, uint256 _count)
        external
        onlyOwner
    {
    ...
    function _regularSwap(
        ...
        if (isWrapper(pool)) {
            ...
            (address t0, ) = BonfireTokenHelper.getProxyParameters(
                tokenPath[0]
            );
            (address t1, ) = BonfireTokenHelper.getProxyParameters(
                tokenPath[1]
            );
            if (t0 == tokenPath[1]) {
                //unwrap it
            ...
            if (isWrapper(target) && tokenPath.length > 2) {
                ...
                (address t2, ) = BonfireTokenHelper.getProxyParameters(
                    tokenPath[2]
                );
    ...
    function accumulate(address to, address bonusToken) internal {
