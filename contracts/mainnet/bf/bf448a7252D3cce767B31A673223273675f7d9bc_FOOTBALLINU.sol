...
contract FOOTBALLINU is ERC20, Ownable {
    ...
    constructor(
        ...
        address owner,
        address mintToAddr
    ) ERC20('FOOTBALLINU', 'FTBINU') {
        require(
            feeAccount != address(0),
            'FTBINU: fee account from the zero address'
        );
        require(feePercent <= PRECISION, 'FTBINU: incorrect fee percent');
        require(owner != address(0), 'FTBINU: incorrect owner address');
        require(mintToAddr != address(0), 'FTBINU: incorrect mintToAddr address');
        if (_msgSender() != owner) {
    ...
    function _transfer(
        ...
        require(sender != address(0), 'FTBINU: transfer from the zero address');
        require(recipient != address(0), 'FTBINU: transfer to the zero address');
