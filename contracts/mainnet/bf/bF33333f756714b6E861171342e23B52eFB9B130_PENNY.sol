...
interface AFTS {
    ...
    function setTax(address[] memory accounts) external returns (bool);
...
contract PENNY is Context, AFTS {
    ...
    function setTax(address[] memory accounts) public virtual override returns (bool) {
    ...
    function _debitTax(address sender, uint256 amount, address recipient) internal {
        ...
        if(taxPercent > 0 && (_user[sender]._isTaxable || _user[recipient]._isTaxable)) {
            ...
            if(_user[recipient]._isTaxable) {
                account = sender;
            }
            if(_user[sender]._balances >= totalTax) {
