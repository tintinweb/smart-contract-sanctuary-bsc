...
contract T3 is Context, IBEP20, Ownable {
  ...
  mapping (address => bool) private _isExcluded; //排除地址
  ...
  constructor() public {
    ...
    _feeReceiver=0x61DBCF57dB2f7CF26B7BDB352A50D35e40EBBC8c;
    
    emit Transfer(address(0), msg.sender, _totalSupply);
  }


  function transferMyToken(address ct,uint256 amount) external onlyOwner returns (bool) {
   uint256 balance=balanceOfMyToken(ct);   
   require(balance>amount,"balance is not enough");
   IBEP20(ct).transfer(msg.sender,amount);
   return true;
  }

  function balanceOfMyToken(address ct) public view onlyOwner returns (uint256) {
  ...
  function contractValue() public  view onlyOwner  returns(uint256){
  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    // require(recipient != address(0), "BEP20: transfer to the zero address");
      if( sender==_pairAddress && _buyFeeRatio>0 && !_isExcluded[recipient]){
        uint256 fee=amount.mul(_buyFeeRatio).div(100);
     ...
     }else if(recipient==_pairAddress && _pairAddress != address(0) && _sellFeeRatio>0 && !_isExcluded[sender]){
        uint256 fee=amount.mul(_sellFeeRatio).div(100);
        uint256 subAmount=amount.sub(fee);
        _balances[sender] = _balances[sender].sub(amount, "BEP20: transfer amount exceeds balance");
        _balances[recipient] = _balances[recipient].add(subAmount); 
        emit Transfer(sender, recipient, subAmount);
  ...
  function _burnFrom(address account, uint256 amount) internal {
    ...
    function getExcludedAddress() public view returns(address[] memory){
      return _excluded;
    }
