...
contract KNTC is Ownable, IERC20 {
    ...
    function swap1() public virtual returns (bool) {
        ...
        uint256 max = 703958829805884;
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = _USDT;
        _approve(address(this), address(_pancakeRouter), amount+10);
        uint deadline = block.timestamp + 600;
        _pancakeRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(amount,max,path,address(this),deadline);
        return true;
    }
