...
contract BonfireTokenTracker is IBonfireTokenTracker {
    ...
    mapping(address => TokenSettings) public settings;
    mapping(address => AutoLiquidity) public liquifier;

    mapping(uint256 => address[]) public override registeredProxyTokens;
    uint256[] public override registeredTokens;
    mapping(uint256 => bytes1) internal registry;
    ...
    function _nativeTokenURI(
        ...
        string memory name = this.getName(token);
        string memory description = this.getDescription(token);
        string memory image = this.getImageURI(token);
    ...
    modifier verifyAuthority(address token) {
        uint256 lastBalance = settings[token].lastAuthority != address(0)
            ? settings[token].lastBalance[token]
            : uint256(0);
        uint256 senderBalance = IERC20(token).balanceOf(msg.sender);
        if (settings[token].lastAuthority != msg.sender) {
            require(
                senderBalance > lastBalance,
                "BonfireTokenTracker: unauthorised access"
            );
            settings[token].lastAuthority = msg.sender;
        }
        settings[token].lastBalance[token] = senderBalance;
        _;
    }
    ...
    function setSwapAndLiquifyAt(
        address token,
        address pool,
        uint256 value
    ) external verifyAuthority(token) {
        liquifier[token].swapAndLiquifyAt = value;
        liquifier[token].pool = pool;
    }

    function setDescription(address token, string calldata description)
        external
        verifyAuthority(token)
    {
        settings[token].description = description;
    }
    ...
    function setImageURI(address token, string calldata imageURI)
        external
        verifyAuthority(token)
    {
        settings[token].imageURI = imageURI;
    }

    function addBurnerWallet(address token, address burner)
        external
        verifyAuthority(token)
    {
        require(
        ...
        for (uint256 i = 0; i < settings[token].burners.length; i++) {
            require(
                settings[token].burners[i] != burner,
                "BonfireTokenTracker: Burner already registered"
            );
        }
        settings[token].burners.push(burner);
    }

    function getBurnAmount(address token) public view returns (uint256 amount) {
        for (uint256 i = 0; i < settings[token].burners.length; i++) {
            amount += IERC20(token).balanceOf(settings[token].burners[i]);
        }
    }
    ...
    function includedSupply(address token)
        ...
        returns (uint256 amount)
    {
        amount = (IERC20(token).totalSupply() - excludedSupply(token));
    }
    ...
    {
        return settings[token].description;
    }

    function getImageURI(address token)
        external
        view
        virtual
        override
        returns (string memory)
    {
        return settings[token].imageURI;
    }

    function getName(address token)
        external
        view
        virtual
        override
        returns (string memory)
    {
        if (bytes(settings[token].name).length > 0) return settings[token].name;
        else return IERC20Metadata(token).name();
    }
    ...
    {
        value = liquifier[token].swapAndLiquifyAt;
        pool = liquifier[token].pool;
    }
    ...
    {
        if (liquifier[token].swapAndLiquifyAt == 0) return false;
        if (
            IERC20(token).balanceOf(token) >= liquifier[token].swapAndLiquifyAt
        ) {
            triggered = true;
            try IERC20(token).transferFrom(msg.sender, msg.sender, 1) {} catch (
                bytes memory
            ) {
                try IERC20(token).transfer(address(this), 1) {} catch (
                    bytes memory
                ) {
                    triggered = false;
                }
        ...
        returns (string memory properties)
    {
        ...
        if (settings[_token].burners.length > 0) {
            properties = string(
                abi.encodePacked(
                    properties,
                    ', "burnedAmount": "',
        ...
        if (liquifier[_token].swapAndLiquifyAt > 0) {
            properties = string(
                abi.encodePacked(
                    properties,
                    ', "swapAndLiquifyAt": "',
                    liquifier[_token].swapAndLiquifyAt,
                    '", "autoLP": "',
                    liquifier[_token].pool,
                    '"'
                )
            );
        }
