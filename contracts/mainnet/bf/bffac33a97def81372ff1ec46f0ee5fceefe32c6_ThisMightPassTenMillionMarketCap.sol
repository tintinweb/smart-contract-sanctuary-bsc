...
/*  This token might be number one... EVENTUALLY! This is a 100% community driven social experiment! The team will push this as far as possible but at the end of the day it comes down to how holders tereat the chart. The taxes will primarily go back to the holders with a 2% tax on transactions improving the liquidity pool, 4% going to marketing and expansion and the rest to holders in likely Bitcoin reflections or whatever asset leads the market.
    https://t.me/Eventuallyglobal
*/
...
contract ThisMightPassTenMillionMarketCap is Context, ERC20, Ownable {
    using SafeMath for uint256;
   ...
   EventuallyDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("This might pass Ten Million Market Cap", "Eventually") {
        _buyRewardsFee = 4;
        _buyLiquidityFee = 2;
		_buyMarketingFee = 5;
        _buyDevFee = 1;
    	...
    	dividendTracker = new EventuallyDividendTracker();

    	liquidityWallet = owner();
		
		marketingWallet = payable(0x14920884E7a6889EE0D984d6d8D170D3E37D617D);
        _devWallet = payable(0x786B284dE10172d803a342EE5f54b1278e737f31);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "Eventually: The dividend tracker already has that address");

        EventuallyDividendTracker newDividendTracker = EventuallyDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "Eventually: The new dividend tracker must be owned by the Eventually token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "Eventually: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Eventually: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Eventually: Account is already the value of 'excluded'");
    ...
    function addToBlacklist(address account, bool blacklisted) public onlyOwner {
        require(_isBlacklisted[account] != blacklisted, "Eventually: Account is already the value of 'blacklisted'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Eventually: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Eventually: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "Eventually: The liquidity wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "Eventually: The marketing wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Eventually: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Eventually: Cannot update gasForProcessing to same value");
...
contract EventuallyDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("Eventually_Dividend_Tracker", "Eventually_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function withdrawDividend() public pure override {
        require(false, "Eventually_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Eventually contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 0 && newClaimWait <= 86400, "Eventually_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Eventually_Dividend_Tracker: Cannot update claimWait to same value");
