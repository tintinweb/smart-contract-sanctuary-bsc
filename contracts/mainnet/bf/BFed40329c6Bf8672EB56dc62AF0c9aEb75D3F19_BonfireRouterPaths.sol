...
contract BonfireRouterPaths is IBonfireRouterPaths, Ownable {
    ...
    function setDefaultProxy(address sourceToken, address proxy)
        external
        onlyOwner
    {
    ...
    {
        address pool = bestWETHPool[token];
        if (pool == address(0)) {
            pool = _bestWETHPool(token);
        }
        if (pool != address(0)) {
    ...
    function updateBestWETHPool(address token) external {
        bestWETHPool[token] = _bestWETHPool(token);
    }

    function _bestWETHPool(address token)
        internal
        view
        ...
        )
    {
        ...
        if (
             ...
             */
            value = _wrapperQuote(token0, token1, amountIn, to);
