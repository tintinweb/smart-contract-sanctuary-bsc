...
contract miniteslaV2 is Context, IERC20, Ownable {
    ...
    string private _name = "miniteslaV2";
    string private _symbol = "MTSLA";
    uint8 private _decimals = 9;
    
    address public deadAddress = 0x000000000000000000000000000000000000dEaD;
    address payable public marketingWallet = payable(0xb5a0C8C2295CDB44f9effBdfb63e10F8dA4eC532);
    ...
    uint256 public minimumTokensBeforeSwap = 100000 * 10**9;
    uint256 public _maxBuyTxAmount = 1000000000 * 10**9;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (overMinimumTokenBalance && !inSwapAndLiquify && swapAndLiquifyEnabled && to == uniswapV2Pair) {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 liquidityTokenShare = contractTokenBalance.mul(_liquidityFee).div(_saleLiquidityFee.add(_saleMarketingFee));
    ...
    function setAllFees(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee, uint256 burnFee) private {
        require(taxFee.add(_liquidityFee).add(marketingFee).add(burnFee) <= 90, "tax too high");
        _taxFee = taxFee;
    ...
    external onlyOwner() {
        require(taxFee.add(_liquidityFee).add(marketingFee).add(burnFee) <= 99, "tax too high");
    ...
    function setMinimumTokensBeforeSwap(uint256 newAmt) external onlyOwner() {
     ...
     function setMaxBuyTxAmount(uint256 maxBuyTxAmount) external onlyOwner() {
        ...
        require(_maxBuyTxAmount >= totalSupply().div(400), "value too low");
    }
    
    function setMaxSellTxAmount(uint256 maxSellTxAmount) external onlyOwner() {
        _maxSellTxAmount = maxSellTxAmount;
        require(_maxSellTxAmount >= totalSupply().div(400), "value too low");
    }
    ...
    // recover stuck bep20 tokens
    function recoverStuckTokens(address tokenAddress, uint256 tokenAmount) public onlyOwner {
