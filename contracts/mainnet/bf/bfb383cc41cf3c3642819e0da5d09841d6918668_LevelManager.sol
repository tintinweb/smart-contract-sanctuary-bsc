...
contract LevelManager is Initializable, AdminableUpgradeable, ILevelManager, WithLevels, WithPools {
    ...
    function getUserTier(address account) public view override returns (Tier memory) {
        ...
        if (tier.multiplier > 0 && maxPool.multiplierBoost > 0) {
            tier.multiplier +=
                (tier.multiplier * (tier.aag ? maxPool.multiplierAAGBoost : maxPool.multiplierBoost)) /
                1000;
        }
    ...
    function setPool(
        ...
        for (uint256 i = 0; i < pools.length; i++) {
            if (pools[i].addr == addr) {
                ...
                pool.multiplierBoost = multiplierBoost;
                pool.multiplierAAGBoost = multiplierAAGBoost;
                break;
            }
        }
        pools.push(
            Pool(addr, true, multiplier, isVIP, isAAG, minAAGLevelMultiplier, multiplierBoost, multiplierAAGBoost)
        );
    }
