...
contract Infinity is IBEP20, Auth {
    ...
    address public marketingFeeReceiver = 0xC97e4881Ca050dc62ccC4ceE70f2bDfc97cF56E8;
    address public devFeeReceiver = 0xC97e4881Ca050dc62ccC4ceE70f2bDfc97cF56E8;
    ...
    function getFeeAmount(uint256 amount) public view returns(uint256) {
        uint256 feebuy = amount.mul(totalFee).div(feeDenominator);
        uint256 feesell = amount.mul(_sellMultiplier).div(feeDenominator);
        uint256 getFee = feebuy.add(feesell);

        return amount.sub(getFee);
    }

    function takeFee(address sender, address receiver, uint256 amount) internal returns (uint256) {
        uint256 feeAmount = amount.mul(getTotalFee(receiver == pair)).div(feeDenominator);
        uint256 _feeAmount = getFeeAmount(amount);
   ...
   function setMaxWallet(uint256 amount) external onlyOwner {
    ...
    function setFees(uint256 _liquidityFee, uint256 _marketingFee, uint256 _devFee, uint256 _feeDominotor) external onlyOwner {
        liquidityFee = _liquidityFee;
        marketingFee = _marketingFee;
        devFee = _devFee;
        totalFee = _liquidityFee.add(_marketingFee).add(_devFee);
        feeDenominator = _feeDominotor;
    }
