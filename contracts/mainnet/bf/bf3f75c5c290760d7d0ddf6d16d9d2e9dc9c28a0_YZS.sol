...
contract YZS is ERC20 {
	...
	address private _fundAddress = address(0xE9787207090e03998E75DEB982e3384c3e22D940);
	address private _nftCardAddress = address(0xA40cf6daBfFB11c23e3d64fd16f3d47146C0B932);
	address private _greenAddress = address(0xaE1d91bBc7a3CbABC9b1227B7d5C3D59af5bA30B);
    address private _market1Address = address(0xee3113043A16843E0b878A54a7C50863072C3893);
    address private _market2Address = address(0xD23b5a390712Ac48763dE221B2E5467c434AE4CA);
	...
	function changeSwapWarp(YZSWarp _warp) public onlyOwner {
        warp = _warp;
    ...
    function _addMarketMakerPair(address pairaddress, bool value) public onlyOwner {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx) {
			...
			if(automatedMarketMakerPairs[from]){
				require(balanceOf(to).add(amount) <= 2000 * 10**18);
    ...
    //
        if (takeFee) {
            ...
            super._burn(address(this), amount.div(100));
	...
	function swapAndLiquify() public {
		...
		if(allAmount > 10**19){
			...
			if(allusdtAmount > usdtAmount){
				...
				USDT.transfer(_fundAddress,newUsdtAmount.div(8).mul(2));
				USDT.transfer(_nftCardAddress,newUsdtAmount.div(8).mul(2));
                USDT.transfer(_greenAddress,newUsdtAmount.div(8).mul(2));
				USDT.transfer(_market1Address,newUsdtAmount.div(8));
                USDT.transfer(_market2Address,newUsdtAmount.div(8));
			}
		}else{
			return;
		}
