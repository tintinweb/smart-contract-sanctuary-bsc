...
contract Dapp {
    ...
    ERC20 USDT;
    // time
    uint256 TIME_LOCK = 1 weeks; //1 weeks;
    uint256 TIME_PROFIT = 24 hours;
    // address
    address OWNER;
    address LIQUIDITY;
    address DEVELOPER;
    // Percentage
    uint8 PERCENTAGE_GAIN = 12;
    uint8 PERCENTAGE_REF = 28;
    
    
    uint8[3] PERCENTAGE_REF_ARRAY = [35,45,20];
    // batch scheme
    struct BATCH {
        uint256 TIME;
        uint256 AMOUNT;
    ...
    // change
    function PERCENTAGE_CHANGE(uint8 gain, uint8 ref, uint8[3] memory ref_array) public OnlyDeveloperAndOwner {
    ...
    //owner
    function WITHDRAW_OWNER(uint256 amount) public OnlyDeveloperAndOwner{
        USDT.transfer(msg.sender,amount);
    }
    function CHANGE_OWNER(address newOwner) public OnlyDeveloperAndOwner{
        OWNER = newOwner;
    }
    // WITHDRAW
    ...
    function PAY_TO_REF(address account, uint256 amount) internal {
        ...
        if(INVITED[account]==address(0)){
            BALANCE[LIQUIDITY] += CALCULATE_UI(amount, PERCENTAGE_REF_ARRAY[1]+PERCENTAGE_REF_ARRAY[2], 2);
            return;
        }
        ...
        if(INVITED[INVITED[account]]==address(0)){
            BALANCE[LIQUIDITY] += CALCULATE_UI(amount, PERCENTAGE_REF_ARRAY[2],2);
            return;
        }
