...
contract MultichainTokenFactory is IMultichainTokenFactory {
    ...
    constructor(address _wrapper, address _tracker) {
        wrapper = _wrapper;
        tracker = _tracker;
    }
    ...
    function getMultichainToken(
        ...
        if (tokens[salt] == address(0)) {
            multichainToken = _createMultichainToken(
                sourceToken,
                sourceMaxSupply,
                chainid,
                name,
    ...
    function _createMultichainToken(
        ...
        uint8 decimals,
        bytes32 salt
    ) internal returns (address multichainToken) {
        ...
        tokens[salt] = multichainToken;
        _finishCreation(sourceToken, chainid, tokens[salt]);
    }
    ...
    function _finishCreation(
        ...
        emit TokenCreation(sourceToken, sourceChainid, targetToken);
    }
}
    ...
    IMultichainToken
{
    ...
    modifier bridgeOnly() {
        require(
            chainid != block.chainid &&
                IBonfireTokenWrapper(wrapper).bridge(msg.sender),
            "MultichainToken: bridge exclusive transaction"
        );
        _;
    }
    ...
    function initialize(
        ...
        require(totalSupply == 0, "MultichainToken: already initialized");
        ...
        multiplier = MAX_SHARES / _sourceMaxSupply;
    ...
    function mint(address to, uint256 _token) external override bridgeOnly {
    ...
    function burn(address from, uint256 _token) external override bridgeOnly {
        uint256 _shares = tokenToShares(_token);
        shares[from] -= _shares;
