...
interface IMutantContract {
    function getOriginTokenId(uint256 _mutantId) external view returns(uint256);
}
contract MutantChipRaceContract is IERC721Receiver, Ownable {
    using SafeERC20 for IERC20;
    ERC721Enumerable g_mutantNFTAddress;
    IERC20 g_craceToken;
    IChipRaceContract g_oldContract;
    IMutantContract g_mutant;
    struct TokenInfo {
        address owner;
        uint256 level;
        uint256 rewards;
    ...
    uint256 public g_mutantDoubling;
    ...
    constructor(address _mutantAddress, IERC20 _tokenAddress, address _oldChip ) {
        g_mutantNFTAddress = ERC721Enumerable(_mutantAddress);
        g_mutant = IMutantContract(_mutantAddress);
        g_craceToken = _tokenAddress;
        g_oldContract = IChipRaceContract(_oldChip);
    ...
    function restoreInfo(uint256 _tokenId) internal {
        ...
        bool canUpgrade;
        bool isEnter;
        uint256 origin = g_mutant.getOriginTokenId(_tokenId);
        (level, remainningTime, score, rewards, canUpgrade, isEnter) = g_oldContract.getInfo(origin);
        info.level = level;
        info.score = score;
    ...
    function getOwnerOf(uint256 _tokenId) external view returns(address) {
        return g_mutantNFTAddress.ownerOf(_tokenId);
    }
    function getInfo(uint256 _tokenId) external view returns(uint256 level, uint256 remainningTime, uint256 score, uint256 rewards, bool canUpgrade, bool isEnter) {
        if(g_tokenInfo[_tokenId].isRestored == false) {
            uint256 origin = g_mutant.getOriginTokenId(_tokenId);
            return g_oldContract.getInfo(origin);
        }
    ...
    function enterChipRace(uint256 _tokenId, uint256 _poolId) external {
        ...
        require(g_carType[g_mutantNFTAddress.tokenURI(_tokenId)] == _poolId, "You can't enter this pool with this NFT");
        require(g_mutantNFTAddress.getApproved(_tokenId) == address(this), "Require NFT ownership transfer approval");

        g_mutantNFTAddress.safeTransferFrom(msg.sender, address(this), _tokenId);
        TokenInfo storage info = g_tokenInfo[_tokenId];
    ...
    function claimRewards(uint256 _tokenId) external {
        ...
        uint256 carType = g_carType[g_mutantNFTAddress.tokenURI(_tokenId)];
        TokenInfo storage info = g_tokenInfo[_tokenId];
        uint256 amount = 0;
        uint256 times = 0;
        ...
        amount = g_minableAmount[carType][info.level] * times * g_eventDoubling * g_mutantDoubling / 10;
        ...
        g_mutantNFTAddress.safeTransferFrom(address(this), msg.sender, _tokenId);
    ...
    function setMutantDoubling(uint256 _doubling) external onlyOwner {
        g_mutantDoubling = _doubling;
    }
    ...
    function withdrawNFT(uint256 _tokenId) external onlyOwner {
        ...
        g_mutantNFTAddress.safeTransferFrom(address(this), owner, _tokenId);
