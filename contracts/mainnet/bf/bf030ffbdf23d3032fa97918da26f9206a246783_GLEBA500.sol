...
abstract contract Ownable is Context {
 ...
 contract GLEBA500 is ERC721Enumerable, Ownable   {
  using Strings for uint256;
  IBEP20 public Token;
  IBEP20 public BUSD;
  ...
  uint256 public MinitngPricein_token=100;    // token
  uint256 public MinitngPricein_bnb=100;    // bnb 
 ...
 /////////////////whitelist prices///////////////////////////// 
  ...
  function mint_with_token(uint256 _count ,uint256 amount,uint256 amount1) public
     {
  ...
  function mint_with_BNB(uint256 _count ) public payable
     {
        ...
        if(iswhitelist[msg.sender]==true){
            require(msg.value>=_count*WhitelistMintingPricein_BNB ,"please enter BNB1");
            require(_count<=Whitelist_MaxLimitPerTransaction,"Whitelist Max Limit Per Transaction Exceed!");
            for (uint i = 0; i < _count; i++) 
              {
                 uint mintIndex = totalSupply();
         ...
         mintedNFTs[msg.sender] += _count;
            ...
            require(msg.value>=_count*bnb  ,"please enter BNB2");
            require(_count<=MaxLimitPerTransaction,"Max Limit Per Transaction Exceed!");
            for (uint i = 0; i < _count; i++) 
              {
                 uint mintIndex = totalSupply();
  ...
  function mint_with_one_token(uint256 _count ,uint256 amount) public 
     {
        require(totalSupply()<=maxsupply,"Minitng Limit Exceeded!");
        require( paused == false  ,"paused by owner");
        ...
        uint256 token  = ValueinToken;
        uint256 z = _count*token;
        
        if(iswhitelist[msg.sender]==true){
        require(amount>=_count*WhitelistMinitngPricein_BUSD ,"please enter BNB1");
        require(_count<=Whitelist_MaxLimitPerTransaction,"Whitelist Max Limit Per Transaction Exceed!");
          for (uint i = 0; i < _count; i++) 
              {
                 uint mintIndex = totalSupply();
        ...
        require( amount >= z  ,"please enter BNB2");
        require(_count<=MaxLimitPerTransaction," Max Limit Per Transaction Exceed!");
  ...
  function mint_with_BUSD(uint256 _count ,uint256 amount) public 
     {
        ...
        require(_count<=Whitelist_MaxLimitPerTransaction,"Whitelist Max Limit Per Transaction Exceed!");
         ...
         mintedNFTs[msg.sender] += _count;
          ...
          require(amount>=_count*(MinitngPricein_token*1e18) ,"please enter BNB2");
        require(_count<=MaxLimitPerTransaction," Max Limit Per Transaction Exceed!");
          for (uint i = 0; i < _count; i++) 
              {
                 uint mintIndex = totalSupply();
...
/////////////////////////Whitelist value////////////////////////////////
  ...
  function setenablemintwithbnb() public onlyOwner{
      bnbmint=true;
  } 
  ...
  function setDisablemintwithbnb() public onlyOwner{
      bnbmint=false;
  } 
