 ...
 * Not associated with original EverSAFU token or team in any way.
 */
...
contract DividendDistributor is IDividendDistributor {
    ...
    function setMinimumSahresRequired(uint256 _minSharesRequired) external onlyToken {
...
contract testEverSAFUv2 is ERC20Detailed, Ownable {
    ...
    event minSharesRequiredChanged(uint256 minSharesRequired);

    constructor() ERC20Detailed("testEverSAFUv2", "TES2", uint8(DECIMALS)) Ownable() {
    ...
    function setMinimumSharesRequired(uint256 _minSharesRequired) external onlyOwner {
        ...
        emit minSharesRequiredChanged(_minSharesRequired);
    }
    ...
    function withdrawAllToTreasury() external swapping onlyOwner {
        ...
        uint256 amountToSwap = _gonBalances[address(this)].div(_gonsPerFragment);
        require( amountToSwap > 0,"There is no ES2 token deposited in token contract");
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();
        router.swapExactTokensForETHSupportingFeeOnTransferTokens(
