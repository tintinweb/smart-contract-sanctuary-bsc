...
contract CrowdFunding {
  ...
  struct Funding {
    ...
    bool success;                    // 活动是否成功，成功则 amount 含义为项目剩余的钱

    uint amount;                     // 当前已经筹集到的金额
    uint numFunders;                 // 参与记录数量
    uint numUses;                    // 使用请求数量
    mapping(uint => Funder) funders; // 参与记录具体信息
    mapping(uint => Use) uses;       // 所有的使用请求
  }

  uint public numFundings;                  // 活动项目数量
  mapping(uint => Funding) public fundings; // 所有的活动项目
