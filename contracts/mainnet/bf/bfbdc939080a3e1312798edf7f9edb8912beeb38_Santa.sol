...
contract Santa is IBEP20, Auth {
    ...
    string constant _symbol = "HoSanta";
    uint8 constant _decimals = 4;

    uint256 _totalSupply =  15 * 10**9 * 10**_decimals;
    ...
    uint256 public devandcharityFee = 2;
    ...
    function setFees(uint256 _liquidityFee, uint256 _devandcharityFee, uint256 _marketingFee, uint256 _projectFee, uint256 _utilityFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        devandcharityFee = _devandcharityFee;
        marketingFee = _marketingFee;
        projectFee = _projectFee;
        utilityFee = _utilityFee;
        totalFee = _liquidityFee.add(_devandcharityFee).add(_marketingFee).add(_projectFee).add(_utilityFee);
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator/2, "Fees cannot be more than 50%");
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _projectFeeReceiver, address _utilityFeeReceiver, address _devandcharityFeeReceiver) external authorized {
