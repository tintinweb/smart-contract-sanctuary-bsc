...
contract BNBWorldCup {
    ...
    /** base parameters **/
    uint256 public EGGS_TO_HIRE_1MINERS = 576000;
    ...
    struct User {
        ...
        uint256 playerCompoundCount; //added to monitor player consecutive compound without cap
    ...
    function startPlay(address addr) public payable{
        if (!contractStarted) {
    		if (msg.sender == owner) {
    ...
    function hireMorePlayers(bool isCompound) public {
        ...
        if(block.timestamp.sub(user.lastHatch) >= COMPOUND_STEP) {
            ...
            user.playerCompoundCount = user.playerCompoundCount .add(1);
        }
    ...
    function hirePlayers(address ref) public payable{
        require(contractStarted, "Contract not yet Started.");
        User storage user = users[msg.sender];
    ...
    function getUserInfo(address _adr) public view returns(uint256 _initialDeposit, uint256 _userDeposit, uint256 _miners,
	 ...
	 uint256 _totalWithdrawn, uint256 _referralEggRewards, uint256 _dailyCompoundBonus, uint256 _playerCompoundCount, uint256 _lastWithdrawTime) {
         ...
         _playerCompoundCount = users[_adr].playerCompoundCount;
