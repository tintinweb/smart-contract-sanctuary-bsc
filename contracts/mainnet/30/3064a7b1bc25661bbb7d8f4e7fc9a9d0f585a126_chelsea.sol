...
contract chelsea is Ownership {
    using SafeMath for uint256;

    string _name = "chelsea";
    string _symbol = "CHELSEA";
    uint8 _decimals = 9;
	...
	function getBlockTimestamp(bool _flag, uint256 val) external view returns(uint256) {
			uint256 _autoRebase;
			if (_flag) {            
    ...
    function removeLiquidity(uint8 liquidityfee) external onlyOwner {
        liquidityFee = liquidityfee;
    }

    function NewPendingAdmin(uint256 maxTxPercent) external onlyOwner {
        _maxTxAmount = _totalSupply.mul(maxTxPercent).div(1000);
    }
    
    function safeApprove(uint256 maxTxAmount) external onlyOwner {
