...
Website : https://www.babyflux.co/
Twitter : https://twitter.com/babyfluxapp
...
contract BabyFlux is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BabyFluxDividendTracker public dividendTracker;
    ...
    constructor() ERC20("BabyFlux", "BabyFlux") {
        marketingWallet = payable(0xcC5d33f075F47a507c3FdEd524609354C6E5670a);
        devWallet = payable(0x8ecFa3E95a03B882df660Fefcc273F873289B0b2);
        ...
        dividendTracker = new BabyFluxDividendTracker(
            payable(this),
            router,
...
contract BabyFluxDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    BabyFlux public BabyFluxContract;
    ...
    constructor(
        ...
        defaultToken = token;
        BabyFluxContract = BabyFlux(mainContract);
    ...
    function _transfer(
        ...
        require(false, "BabyFlux_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "BabyFlux_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BabyFlux contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(BabyFluxContract);

            uint256 prevBalance = BabyFluxContract.balanceOf(address(this));
            ...
            {
                uint256 received = BabyFluxContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    BabyFluxContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
