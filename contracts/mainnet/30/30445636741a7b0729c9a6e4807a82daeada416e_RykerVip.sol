...
contract RykerVip is Context, Ownable, ReentrancyGuard{
    ...
    struct CompoundDetails {
        ...
        bool isAutoCompound;
        bool isAutoCompoundAfterWithdraw;
    }
    ...
    event LateFundsDistribution(address indexed addr, uint256 amount);

    UserDeposit[] public userDeposits; 
    TossCoinHistory[] public tossCoinHistoryArr;
    LotteryHistory[] internal lotteryHistory;

    constructor(address payable _project, address payable _lateInvFund, address payable _mkt) {
		require(!isContract(_project) && !isContract(_lateInvFund) && !isContract(_mkt));
        project = _project;
    ...
    function startFarm(address addr) public payable onlyOwner{
        if (!contractStarted) {
    ...
    function autoCompound() public {
        ...
        for (uint256 i = 0; i < userDeposits.length; i++) {
            ...
            if(autoCompoundMap[userDeposits[i].walletAdress].isAutoCompound){
                User storage user = users[userDeposits[i].walletAdress];
                CompoundDetails storage compoundDetails = autoCompoundMap[userDeposits[i].walletAdress];

                /** if compound step has passed and if users compound count is less than or = to set auto compound count **/
                if(getCurTime().sub(user.lastHatch) >= COMPOUND_STEP && compoundDetails.currentCompoundCount < compoundDetails.numberOfAutoCompounds){
                    compoundAddress(true, userDeposits[i].walletAdress);
                    compoundDetails.currentCompoundCount = compoundDetails.currentCompoundCount.add(1);
                }
            }
        }
    }

    function setAutoCompound(bool isAutoCompound,uint256 _numberOfAutoCompounds, bool _isAutoCompoundAfterWithdraw) public {
        require(contractStarted, "Contract not yet Started.");
        ...
        require(_numberOfAutoCompounds > 0 && _numberOfAutoCompounds< 365, "number of autocompound is not valid");

        autoCompoundMap[msg.sender].isAutoCompound = isAutoCompound;
        autoCompoundMap[msg.sender].numberOfAutoCompounds = _numberOfAutoCompounds;
        autoCompoundMap[msg.sender].currentCompoundCount = 0;   //get the current compound count of user.
        autoCompoundMap[msg.sender].isAutoCompoundAfterWithdraw = _isAutoCompoundAfterWithdraw;
        autoCompound(); 
    }
    ...
    function compoundAddress(bool isCompound, address _address) internal{
        ...
        if(isCompound) {
            ...
            if(TOSS_COIN_ACTIVATED && getCurTime().sub(TOSS_COIN_START_TIME) >= COIN_TOSS_STEP && currentHeadsBetCount > 0 && currenttailsBetCount > 0){
    ...
    function sellProfit() public nonReEntrant{
        ...
        if(user.dailyCompoundBonus < COMPOUND_FOR_NO_TAX_WITHDRAWAL){
            eggValue = eggValue.sub(eggValue.mul(WITHDRAWAL_TAX).div(PERCENTS_DIVIDER));
             ...
             //reset the users compound counter
             user.dailyCompoundBonus = 0;
             autoCompoundMap[msg.sender].currentCompoundCount  = 0;  
        }
        ...
        if(LOTTERY_ACTIVATED && getCurTime().sub(LOTTERY_START_TIME) >= LOTTERY_STEP || participants >= MAX_LOTTERY_PARTICIPANTS || currentPot >= MAX_INCENTIVE_BALANCE){
            chooseWinner();
        }
        ...
        if(autoCompoundMap[msg.sender].isAutoCompoundAfterWithdraw){ //set autocompound true if configured to autocompound after sell
            setAutoCompound(true,autoCompoundMap[msg.sender].numberOfAutoCompounds, true);
        }
    }
    ...
    function _poolDeposits(address _addr, uint256 _amount) private {
        ...
        if(upline == address(0) || upline == owner() || users[upline].miners < 1) return;
    ...
    function _buyTickets(address userAddress, uint256 amount) private {
        ...
        if(userAddress == owner()) return; // owner do not buy tickets, does not have the chance to win
   ...
   /** will auto execute, when condition is met. buy, hatch and sell, can be triggered manually by admin if theres no user action. **/
    ...
    function payFees(uint256 eggValue,bool isSell) internal returns(uint256){
        ...
        project.transfer(tax);
        mkt.transfer(mktTax);
        lateInvFund.transfer(livTax);
        
        uint256 totTax =  tax + mktTax + livTax; // 7%
    ...
    function getCoinFlipHistory() view external returns(uint256[5] memory _round,  uint256[5] memory _winner,  uint256[5] memory _oddsWon,
	   uint256[5] memory _tossTimeStamp,  uint256[5] memory _winnersCount,  uint256[5] memory _totalPot) {
        ...
        for(uint8 i = 0; i < 5; i++) {
            ...
            _totalPot[i] = tossCoinHistoryArr[startingIndex].totalMinerPot;
            if(startingIndex == 0) break;
            startingIndex--;
        }
	}
    ...
    function checkAutoCompound(address _adr) public view returns(bool _isAuto, uint256 _currentCompound, uint256 _numberOfCompoundTostop, bool _autoCompoundAfterWith){
        _isAuto = autoCompoundMap[_adr].isAutoCompound;
        _currentCompound = autoCompoundMap[_adr].currentCompoundCount;
        _numberOfCompoundTostop = autoCompoundMap[_adr].numberOfAutoCompounds;
        _autoCompoundAfterWith = autoCompoundMap[_adr].isAutoCompoundAfterWithdraw;
    }
    ...
    function distributeLateInvestorFunds() public payable onlyOwner returns(uint256){ 
        ...
        for(uint256 i = 0; i < finalUserDeposits.length; i++){
            ...
            totalWithdrawn = totalWithdrawn.add(totalShareValue);
             ...
             emit LateFundsDistribution(_userDeposit.walletAdress, totalShareValue);
        }
    ...
    function isOverIncome(address _address) public view returns(bool _isOverIncome, uint256 _perCent){
            User storage user = users[_address];
             if(user.initialDeposit.mul(30).div(10) < user.totalWithdrawn){ 
                  _isOverIncome = true;
            }
    ...
    function SET_FIXED_YIELD_INCOME_PRCT(uint256 value) external onlyOwner(){
        ...
        require(getCurTime().sub(LAST_CHANGE_TIMESTAMP) >= TIME_STEP); // can only be changed once a day
        LAST_CHANGE_TIMESTAMP = getCurTime();
        FIXED_YIELD_INCOME_PRCT = value;
    }

    function SET_LATE_INVESTOR_PERCENT(uint256 value) external onlyOwner(){
        require(value >= 50 && value <= 200); /** min 5% max 20%**/
        require(getCurTime().sub(LAST_CHANGE_TIMESTAMP2) >= TIME_STEP); // can only be changed once a day
        LAST_CHANGE_TIMESTAMP2 = getCurTime();
        LATE_INVESTOR_PERCENT = value;
    }

    function DEP_REQUIRED_FOR_AUTOCOMP(uint256 value) external onlyOwner(){
        require(value >= 10 && value <= 30); /** min 0.1 max 0.3**/
        DEP_REQUIRED_AUTOCOMP = value * 1e16;
    }
     
    /** flip coin enabler **/ 
    function ENABLE_FLIPCOIN() external onlyOwner(){
        require(contractStarted);
        TOSS_COIN_ACTIVATED = true;
        TOSS_COIN_START_TIME = getCurTime();
    }

    function DISABLE_FLIPCOIN() external onlyOwner(){
        require(contractStarted);
        TOSS_COIN_ACTIVATED = false;
    } 

    /** lottery enabler **/
    function ENABLE_LOTTERY() external onlyOwner(){
        require(contractStarted);
        LOTTERY_ACTIVATED = true;
        LOTTERY_START_TIME = getCurTime();
    }

    function DISABLE_LOTTERY() external onlyOwner(){
        require(contractStarted);
