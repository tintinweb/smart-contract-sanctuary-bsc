    ...
    ReentrancyGuard
{
    ...
    uint256 public bnbAmountToFixGun;
        ...
        onlyOwner
    {
        ...
        require(address(_fishTreasuryWallet) != address(0), "Invalid FishTreasury wallet address");
        require(address(_lfwTreasuryWallet) != address(0), "Invalid LFWTreasury wallet address");
        require(address(_rewardSC) != address(0), "Invalid rewardSC address");
     ...
     * @param _fishTreasuryWallet: address of treasury wallet to transfer fund
     */
    function setFishTreasuryWallet(address _fishTreasuryWallet) public onlyOwner {
        fishTreasuryWallet = _fishTreasuryWallet;
    }
     ...
     * @param _lfwTreasuryWallet: lfw treasury wallet that we already used in LFWar game
     */
    function setLfwTreasuryWallet(address _lfwTreasuryWallet) public onlyOwner {
        lfwTreasuryWallet = _lfwTreasuryWallet;
    }
     ...
     * @param _rewardSC: reward pool SC to reward user in game
     */
    function setRewardSC(address _rewardSC) public onlyOwner {
        rewardSC = _rewardSC;
    }
     ...
     */
    function setBnbAmountToFixGuns(uint256 _bnbAmount) public onlyOwner {
        require( _bnbAmount != 0, "FishTreasury: Parameter must not be 0");
        bnbAmountToFixGun = _bnbAmount;
    }
     ...
     * @dev only call by owner
     * @param _lfwAmount: lfw amount used to buy diamond
     * @param _serverId: sever ID that user want to transfer diamond in
     */
    function swapDiamond(uint256 _lfwAmount, uint256 _serverId) external nonReentrant returns (uint256){
        require(_lfwAmount > 0, "Invalid number");

        uint256 balance = lfwToken.balanceOf(_msgSender());

        require(balance >= _lfwAmount, "Insufficient LFW Token in your wallet");

        lfwToken.transferFrom(_msgSender(), address(lfwTreasuryWallet), _lfwAmount);

        uint256 diamondAmount = _lfwAmount.mul(conversionRate);

        // game will handle diamond.div(conversionRate)
        emit SwapDiamond(_msgSender(), _lfwAmount, diamondAmount, _serverId);
    ...
    function mintNFT(uint256 _boxNumber) external payable nonReentrant {
        ...
        (bool sent, ) = payable(rewardSC).call{value: bnbToTransfer}("");
    ...
    function fixGun(
        ...
        payable(fishTreasuryWallet).transfer(bnbAmountToFixGun);
