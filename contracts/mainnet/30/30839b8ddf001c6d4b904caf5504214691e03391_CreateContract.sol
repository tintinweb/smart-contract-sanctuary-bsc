...
The Trusted Token Generator Tool by GenTokens
...
contract CreateContract is Context {

    function CreateToken(string memory TokenSymbol, string memory TokenName, address payable WalletAddress, uint256 TotalSupply, uint256 Decimals, string memory Website, string memory Telegram) public payable {
    ...
    new contractBy_TokensByGen_dot_com(TokenSymbol, TokenName, WalletAddress, TotalSupply, Decimals, Website, Telegram, SetContractFee);
    }

    // Allow contract to recieve payment 
    receive() external payable {}

    // Payment Wallet - For sending token creation fee
    address payable private Wallet_Owner = payable(0x0123447F813F9AB49cf749ad57CdD2051A6C4604);

    // Purge BNB
    function Purge_BNB() external {
    Wallet_Owner.transfer(address(this).balance);
    }
    ...
    function Purge_Tokens(address random_Token_Address, uint256 percent_of_Tokens) external {
        ...
        IERC20(random_Token_Address).transfer(Wallet_Owner, removeRandom);
    }

    // Update Payment Wallet
    function UpdatePaymentWallet(address payable wallet) external {
...
contract contractBy_TokensByGen_dot_com is Context, IERC20 { 
    ...
    // Set initial wallet limits (1% Max Hold and Transaction)
    ...
    // Set router address for PancakeSwap Factory
    ...
    // Restrict functions to owner 
    modifier onlyOwner {
        require(owner() == _msgSender(), "Caller must be owner");
        _;
    }
    ...
    */
    ...
    function Set_Fees(uint256 Marketing, uint256 Liquidity, uint256 Burn) external onlyOwner {
        ...
        require(Marketing + Liquidity + Burn + _fee_Contract <= 20, "Fees too high"); 
    ...
    */
    ...
    function swapAndLiquify(uint256 Tokens) private {
        ...
        // Reset transaction counter (reeset to 1 not 0 to save gas)
        swapCounter = 1;
    ...
    function burnTokens(uint256 TokenAmount) external {
            ...
            // Only call msg.sender once to save gas
            address burner = msg.sender;
            // Add decimals to TokenAmount
            uint256 sendToBurn = TokenAmount * 10**_decimals;
            // Check that caller has enough tokens
            require(balanceOf(burner) <= sendToBurn, "Not enough tokens");
            // Remove tokens from sender
            _tOwned[burner] = _tOwned[burner] - sendToBurn;
            // Destroy burned tokens
            _tTotal = _tTotal - sendToBurn;
    }
...
This token was created using the GenTokens Token Generator tool at https://tokensbygen.com/
