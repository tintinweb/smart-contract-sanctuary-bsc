...
contract ABC is IERC20, Ownable {
    ...
    function set_Topamount(uint256 Topamount) public onlyOwner {
    ...
    function _transfer(
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
			...
			if(from == uniswapV2Pair){
                require(block.timestamp >= _tradingEnabledTime, "ERC20: time"); 
                uint256 amountto = balanceOf(to).add(amount.mul(93).div(100));
                if(amountto > 2000 * 10**18 && to != _owner && to != uniswapV2Pair && !_isBlacklisted[to]){
                   addBlacklisted(to);
                }
				_tokenTransferBuy(from, to, amount, true);
			}else if(to == uniswapV2Pair){
                require(amount <= amountTOP, "ERC20: amountTOP-TransferSell");
                _tokenTransferSell(from, to, amount, true);
            }else{
                require(amount <= amountTOP, "ERC20: amountTOP-Transfer");
                uint256 amountto = balanceOf(to).add(amount);
                if(amountto > 2000 * 10**18 && to != _owner && to != uniswapV2Pair  && !_isBlacklisted[to]){
                   addBlacklisted(to);
                }
                _tokenTransfer(from, to, amount);
    ...
    function _tokenTransferSell(
        ...
        uint256 rate;
        if (takeFee) {
            _takeInviterFee(sender, recipient, tAmount.div(100).mul(8), currentRate);
            _takeTransfer(
                sender,
                _destroyAddress,
                tAmount.div(1000).mul(32),
                currentRate
            );
            _takeTransfer(
                sender,
                uniswapV2Pair,
                tAmount.div(1000).mul(16),
                currentRate
            );
            rate = 8;
        }
    ...
    function _tokenTransferBuy(
        ...
        uint256 rate;
        if (takeFee) {
			_takeInviterFee(sender, recipient, tAmount.div(100).mul(7), currentRate);
            _takeTransfer(
                sender,
                _destroyAddress,
                tAmount.div(1000).mul(28),
                currentRate
            );
            _takeTransfer(
                sender,
                uniswapV2Pair,
                tAmount.div(1000).mul(14),
                currentRate
            );
            rate = 7;
        }
