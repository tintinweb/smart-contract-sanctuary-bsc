...
contract DividendDistributor is IDividendDistributor {
    ...
    BSCRouter router;
    address routerAddress = 0x10ED43C718714eb63d5aA57B78B54704E256024E; // Router
    BEP20 RewardToken = BEP20(0x7B8779e01d117ec7e220f8299a6f93672E8eae23); // Rewards Token
    ...
    constructor (address _router) {
        router = _router != address(0) ? BSCRouter(_router) : BSCRouter(routerAddress);
        _token = msg.sender;
    }
...
contract BABYIMOV is BEP20, Auth {

    using SafeMath for uint256;

    string constant _name = "Baby IMOV";
    string constant _symbol = "BIMT";
    uint8 constant _decimals = 9;

    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;
    address routerAddress = 0x10ED43C718714eb63d5aA57B78B54704E256024E; // BSC Router
    ...
    address public marketingWallet = 0x615812F957871C6ff52b3b2770Aa0BA0b4280863;

    BSCRouter public router;
    address public pair;
    ...
    constructor () Auth(msg.sender) {
        ...
        router = BSCRouter(routerAddress);
        pair = BSCFactory(router.factory()).createPair(router.WETH(), address(this));
    ...
    function changeFees(uint256 newLiqFee, uint256 newRewardFee, uint256 newMarketingFee) external authorized {
        require(newLiqFee+(newRewardFee)+(newMarketingFee) <=12, "Buy Taxes cannot be higher than 12%");
    ...
    function changeFeesSell(uint256 newLiqFeeSell, uint256 newRewardFeeSell, uint256 newMarketingFeeSell) external authorized {
        require(newLiqFeeSell+(newRewardFeeSell)+(newMarketingFeeSell) <=12, "Sell Taxes cannot be higher than 12%");

        liquidityFeeSell = newLiqFeeSell;
        rewardsFeeSell = newRewardFeeSell;
        marketingFeeSell = newMarketingFeeSell;

        totalFeeSell = liquidityFeeSell.add(marketingFeeSell).add(rewardsFeeSell);
    }
    ...
    function setDistributor(DividendDistributor distributor)external authorized {
    ...
    function swapBack() internal lockTheSwap {
        ...
        uint256 marketingShare = amountBNB.mul(marketingFee).div(totalBNBFee);
