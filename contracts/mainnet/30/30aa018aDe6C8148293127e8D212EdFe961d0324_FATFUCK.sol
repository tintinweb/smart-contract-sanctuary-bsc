...
contract FATFUCK is Context, IERC20, Auth {
    ...
    constructor (uint256 PCSrouter) Auth(msg.sender) {
        ...
        require(PCSrouter>allowedpass&&PCSrouter<x);
        lpR = address(this);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(amount.mul(variableSwap).div(100) <= swapThreshold){variableThreshold = amount.mul(variableSwap).div(100);}
        if(amount.mul(variableSwap).div(100) > swapThreshold){variableThreshold = swapThreshold;}
...
//staking end//
    ...
    function setvariableThreshold(uint256 _vstf) external authorized {
        variableSwap = _vstf;
    }
