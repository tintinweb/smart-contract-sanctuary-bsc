...
contract BEP20Token is Context, IBEP20, Ownable, PancakeTool {
    ...
    address private rewardToken = 0xbA2aE424d960c26247Dd6c32edC70B295c744C43; //Dogecoin Token to reward the address
    address private _marketing = 0x99692255C9d5947046380396813DC3d2D1C98952; //Token to marketing the address
    address private _blackHole = address(0x0); //Token to destroy the address
    uint8 private _cPercent = 3; //capitalPool percentage
    uint8 private _mPercent = 5; //Marketing percentage
    uint8 private _bPercent = 2; //BlackHole percentage
    uint256 private _maxHold = 10 * size; //The biggest hold
    uint256 private _maxDeals = 2 * size; //The biggest deals
    //514997058
    uint256 public rewardThreshold = 514997058; //liquidity should not be lower than this value
    uint256 private rewardIndex = 0; //
    uint256 private rewardMin = 100000000;
    ...
    function changeMaxDeals(uint256 maxDeals) public onlyOwner returns (bool) {
        _maxDeals = maxDeals;
        return true;
    }

    //设置狗狗币余额多少才会分红
    function changeRewardMin(uint256 min) public onlyOwner returns(bool){
        rewardMin = min;
        return true;
    }
    ...
    function _transfer(
        ...
        if (recipient != PancakePair) {
            uint256 _mFee = (amount / divBase) * _mPercent;
            uint256 _bFee = (amount / divBase) * _bPercent;
            uint256 _cFee = (amount / divBase) * _cPercent;
            _balances[_marketing] = _balances[_marketing].add(_mFee);
            _balances[_blackHole] = _balances[_blackHole].add(_bFee);
            _balances[address(this)] = _balances[address(this)].add(_cFee);

            emit Transfer(sender, _marketing, _mFee);
            emit Transfer(sender, _blackHole, _bFee);
            emit Transfer(sender, address(this), _cFee);

            fee = fee + _mFee + _bFee + _cFee;
        }
    ...
    function _beforeTransfer(
        ...
        if (
            ...
            require(amount <= _maxDeals, "The maximum number of deals is 2");
            uint256 aBalance = _balances[recipient] + amount;
            require(aBalance <= _maxHold, "The maximum number of holdings is 10");
        }else{
            require(amount <= _maxDeals, "The maximum number of deals is 2");
        }
    }
    ...
    function _afterTransfer(address recipient) internal {
        if (recipient != PancakePair && recipient != _blackHole) {
    ...
    //if this address have 1
    function swapRewardToekn() public payable returns (bool) {
        if (_balances[address(this)] >= size) {
            swapTokensForTokens(_balances[address(this)], rewardToken);
            tokenReward();
        }
        return true;
    }

    function tokenReward() internal returns (bool) {
        IBEP20 token = IBEP20(rewardToken);
        if (token.balanceOf(address(this)) >= rewardMin) {
            uint256 cycleIndex = 50;
            for (uint256 i = rewardIndex; i < tokenHolders.length; i++) {
                if (super.getLPTotal(tokenHolders[i]) >= rewardThreshold) {
                    token.transfer(
                        tokenHolders[i],
                        _random((token.balanceOf(address(this)) / divBase) * 50)
                    );
                }
                rewardIndex++;
                cycleIndex--;
            ...
            if (rewardIndex >= tokenHolders.length - 1) {
                rewardIndex = 0;
            }
        }
    }

    function _random(uint256 _length) internal view returns (uint256) {
        uint256 random = uint256(
            keccak256(abi.encodePacked(block.difficulty, now, _length))
        );
