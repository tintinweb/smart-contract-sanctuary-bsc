...
library Address{
 ...
 contract EasyCrypto is Context, IERC20, Ownable {
    ...
    uint256 private initialsupply = 21000000000;
	uint256 private _tTotal = initialsupply * 10 ** _decimals; 
    uint256 private _rTotal = (MAX - (MAX % _tTotal));

    uint256 public swapTokensAtAmount = 21000000000 * 10**9;
    uint256 public maxBuyLimit = 21000000000 * 10**9;
    uint256 public maxSellLimit = 210000000 * 10**9;
    uint256 public maxWalletLimit = 21000000000 * 10**9;
    
    uint256 public genesis_block;
    
    address public marketingWallet = 0x20784FE9e866725BaC8181831c1b0F10d4b18dF8;
    address public developmentWallet = 0x20784FE9e866725BaC8181831c1b0F10d4b18dF8;

    string private constant _name = "EasyCrypto";
    string private constant _symbol = "EZY";
    ...
    Taxes public taxes = Taxes(6, 2, 2, 2);
    Taxes public sellTaxes = Taxes(6, 2, 2, 2);
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferBUSD) public view returns(uint256) {
        require(tAmount <= _tTotal, "Amount must be less than supply");
        if (!deductTransferBUSD) {
    ...
    function setTaxes(uint256 _BUSD, uint256 _marketing, uint256 _liquidity, uint256 _development) public onlyOwner {
       taxes = Taxes(_BUSD,_marketing,_liquidity,_development);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _BUSD, uint256 _marketing, uint256 _liquidity, uint256 _development) public onlyOwner {
       sellTaxes = Taxes(_BUSD,_marketing,_liquidity,_development);
        emit FeesChanged();
    }

    function _reflectBUSD(uint256 rBUSD, uint256 tBUSD) private {
        _rTotal -=rBUSD;
        totFeesPaid.BUSD +=tBUSD;
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rBUSD, to_return.rMarketing, to_return.rLiquidity) = _getRValues1(to_return, tAmount, takeFee, _getRate());
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.tBUSD = tAmount*temp.BUSD/100;
        s.tMarketing = tAmount*temp.marketing/100;
        s.tLiquidity = tAmount*temp.liquidity/100;
        s.tdevelopment = tAmount*temp.development/100;
        s.tTransferAmount = tAmount-s.tBUSD-s.tMarketing-s.tLiquidity-s.tdevelopment;
        return s;
    }

    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rBUSD,uint256 rMarketing, uint256 rLiquidity){
        ...
        rBUSD = s.tBUSD*currentRate;
        rMarketing = s.tMarketing*currentRate;
        rLiquidity = s.tLiquidity*currentRate;
        uint256 rdevelopment = s.tdevelopment*currentRate;
        rTransferAmount =  rAmount-rBUSD-rMarketing-rLiquidity-rdevelopment;
        return (rAmount, rTransferAmount, rBUSD,rMarketing,rLiquidity);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rBUSD > 0 || s.tBUSD > 0) _reflectBUSD(s.rBUSD, s.tBUSD);
    ...
    function Whitelistpresale(address[] memory accounts, uint256[] memory amounts) external onlyOwner{
