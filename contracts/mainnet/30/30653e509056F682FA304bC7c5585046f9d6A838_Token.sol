...
contract Token is IERC20, Ownable {
    ...
    address private sat = 0x83c77ca10B79529a6c755180E42c59070ddF404e;
    address private projectAddress = 0x97f4C800771307c610292136927F56AC090A39e1;
   

    uint256 private _tFeeTotal;

    string private _name = "TTN";
    ...
    uint256 distributorGas = 999;

    uint256 public minPeriod = 1 hours;

    uint256 public maxHold = 20 * 10**9;
    uint256 public maxBc = 3 * 10**9;
   ...
   function isExcludedFromFee(address account) public view returns (bool) {
    ...
    function setIsopen(bool _enabled) external onlyOwner(){
        isOpenTrade = _enabled;
    }
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFee[to] && to != uniswapV2Pair){
            require(balanceOf(to) + amount <= maxHold);
        }

        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            require(amount <= maxBc);
        }
        if (!_isExcludedFromFee[from] &&  to == uniswapV2Pair) {
            require(isOpenTrade == true);
        }
        ...
        fromAddress = from;
        toAddress = to;  
         if(_tOwned[address(this)] >= 0.2 * 10**9 && from !=address(this) && LPFeefenhong.add(minPeriod) <= block.timestamp) {
    ...
    function _takeburnFee(
        ...
        if(_tFeeTotal >= 999 * 10**9)_burnFee = 0;
        _tOwned[address(0)] = _tOwned[address(0)].add(tAmount);
        _tFeeTotal = _tFeeTotal.add(tAmount);
        emit Transfer(sender, address(0), tAmount);
    }

    function _takeprojectFee(address sender,uint256 tAmount) private {
        if (_projectFee == 0) return;
    ...
    function swapEthForToken(uint256 ethAmount,address receiver) private{
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
        IERC20 TestE = IERC20(0x83c77ca10B79529a6c755180E42c59070ddF404e);//sat代币地址--
    ...
    function swapThisTokenForToken(uint256 thisTokenAmount,address receiver) private{
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
        IERC20 SAT = IERC20(0x83c77ca10B79529a6c755180E42c59070ddF404e);//sat代币地址--
    ...
    function _transferStandard(
        ...
        _takeprojectFee(sender, tAmount.div(10000).mul(_projectFee));
