...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional, Ownable {
  ...
  address public DividendsToken = address(0x1AdA8bB610C59Aae25d8DD8f354282F5693CefB1);
...
contract OshiInu is Context, ERC20, Ownable {
    using SafeMath for uint256;
   ...
   OshiInuDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("OSHI INU", "OSHI") {
        _buyRewardsFee = 0;
        _buyLiquidityFee = 1;
		_buyMarketingFee = 6;
        _buyDevFee = 1;
    	...
    	dividendTracker = new OshiInuDividendTracker();

    	liquidityWallet = owner();
		
		marketingWallet = payable(0xD0DAD74c60B11B504215a4c5893BecCCF8d575b7);
        _devWallet = payable(0xb38106EEF7A0906BfF274Ff6E24634852D3EfC1c);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "OSHI INU: The dividend tracker already has that address");

        OshiInuDividendTracker newDividendTracker = OshiInuDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "OSHI INU: The new dividend tracker must be owned by the OSHI INU token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "OSHI INU: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "OSHI INU: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "OSHI INU: Account is already the value of 'excluded'");
    ...
    function addToBlacklist(address account, bool blacklisted) public onlyOwner {
        require(_isBlacklisted[account] != blacklisted, "OSHI INU: Account is already the value of 'blacklisted'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "OSHI INU: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "OSHI INU: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "OSHI INU: The liquidity wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "OSHI INU: The marketing wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "OSHI INU: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "OSHI INU: Cannot update gasForProcessing to same value");
...
contract OshiInuDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("OSHI_Dividend_Tracker", "OSHI_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function withdrawDividend() public pure override {
        require(false, "OSHI_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main OSHI INU contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 0 && newClaimWait <= 86400, "OSHI_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "OSHI_Dividend_Tracker: Cannot update claimWait to same value");
