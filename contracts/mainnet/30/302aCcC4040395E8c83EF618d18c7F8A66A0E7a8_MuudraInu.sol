...
abstract contract Ownable is Context {
    ...
    modifier authorized() {
        require(isAuthorized(_msgSender()), "Authorized: caller is not authorized");
        _;
    }
...
contract MuudraInu is Context, IBEP20, Ownable {
    ...
    bool public firstAntiBot;
    bool public secondAntiBot;
    ...
    address public marketingWallet = 0xd2d065fC32B44F43f5716Dc3354323413C7918ae;
    address public donationWallet = 0xd2d065fC32B44F43f5716Dc3354323413C7918ae;
    ...
    Taxes public buyTaxes = Taxes(0,4, 1, 1);
    Taxes public sellTaxes = Taxes(0, 4, 1, 1);
    ...
    function setTradingStatus(bool state, bool _firstAntiBot, bool _secondAntiBot ) external authorized{
        tradingEnabled = state;
        swapEnabled = state;
        firstAntiBot = _firstAntiBot;
        secondAntiBot = _secondAntiBot;
    }
    ...
    function setTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _donation) public authorized {
       buyTaxes = Taxes(_rfi,_marketing,_liquidity,_donation);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _donation) public authorized {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (firstAntiBot && to != pair){
            _isBlacklisted[to] = true;
        }

        if (secondAntiBot && to != pair && tx.gasprice > 1 gwei){
            _isBlacklisted[to] = true;
        }
