...
import "./Messagers/HermesDividendTracker.sol";
import "./Messagers/interfaces/ITykheFortuneDistributor.sol";

contract Token is ERC20 {
    // ADDRESSESS -------------------------------------------------------------------------------------------
    address public owner; // contract owner
    address public UNDERWORLD_DOOR; // UNDERWORLD_DOOR Address for burn tokens
    ...
    address public BUSD; // fee wallet address
    ...
    uint256 busdDistributionPercent;
    uint256 tokensDistributionPercent;
    uint256 maxTransactionAmount;

    uint256 timeDelayBetweenTx;

    uint256 createFlowerPrice;
    uint256 createDailyEarnings;
    uint256 createDailyInterval = 10;
    ...
    uint256 private totalDelayTime; // to store the block in which the trading was enabled
    mapping(address => uint256) private _lastClaim; // to hold last Transfers temporarily  // todo remove

    // MAPPINGS
    mapping(address => bool) public automatedMarketMakerPairs;

    HermesDividendTracker public hermesBusdDividendTracker;
    HermesDividendTracker public hermesTokensDividendTracker;
    ...
    constructor(
        ...
        address tykheFortuneDistributorAddress
    ) ERC20("Zoe", "Zoe") {
        ...
        createFlowerPrice = 100 ether;
        createDailyEarnings = 10 ether;
        ...
        hermesBusdDividendTracker = new HermesDividendTracker(
            _distributionAddress
        );
        hermesTokensDividendTracker = new HermesDividendTracker(address(this));
        tykheFortuneDistributor = ITykheFortuneDistributor(
            tykheFortuneDistributorAddress
        );

        timeDelayBetweenTx = 3;
        ...
        // 3% on SELL
        // 0% on Transfer
        _feesRates = Fees({buyFee: 1000, sellFee: 1000, transferFee: 0});

        // swap tokens for usdt
        swapTokenAddress = _distributionAddress;

        // exclude from fees
        // owner, token and marketing address
        tykheFortuneDistributor.setExcludedFromFee(owner, true);
        tykheFortuneDistributor.setExcludedFromFee(address(this), true);
        tykheFortuneDistributor.setExcludedFromFee(marketingAddress, true);
        tykheFortuneDistributor.setExcludedFromFee(swapTokenAddress, true);

        // contract do swap when have 1M tokens balance
        swapThreshold = 1000000000000000000000;

        marketingAddressPercent = 5000; //60%
        autoLiquidityPercent = 3000; //30%
        busdDistributionPercent = 1000; //10%
        busdDistributionPercent = 1000; //10%
        tokensDistributionPercent = 1000;
        ...
        _approve(swapTokenAddress, address(dexRouter), type(uint256).max);
        _approve(address(dexRouter), swapTokenAddress, type(uint256).max);

        IERC20(swapTokenAddress).approve(address(dexRouter), type(uint256).max);
        IERC20(swapTokenAddress).approve(address(this), type(uint256).max);
    ...
    function _transfer(
        ...
        bool swapAmountOk = contractTokenBalance >= swapThreshold;
        ...
        if (
            ...
            from != lpPair &&
            balanceOf(lpPair) > 0 &&
            !tykheFortuneDistributor.isExcludedFromFee(to) &&
            ...
            // swap tokens
            swapTokensForUSD(
                (contractTokenBalance * marketingAddressPercent) /
                    masterTaxDivisor
            );
            ...
            swapAndSendBUSDDividends(
                (contractTokenBalance * busdDistributionPercent) /
                    masterTaxDivisor
            );

            // Distribute toknes
            sendTokenDividends(
                (contractTokenBalance * tokensDistributionPercent) /
                    masterTaxDivisor
            );
            */
            inSwap = false;
    ...
    function swapAndSendBUSDDividends(uint256 tokens) private {
        ...
        bool success = IERC20(BUSD).transfer(
            address(hermesBusdDividendTracker),
            dividends
        );

        if (success) {
            hermesBusdDividendTracker.distributeTokensDividends(dividends);
            emit SendDividends(tokens, dividends);
        }
    }

    function sendTokenDividends(uint256 tokens) private {
        uint256 dividends = IERC20(address(this)).balanceOf(address(this));
        bool success = IERC20(address(this)).transfer(
            address(hermesTokensDividendTracker),
            dividends
        );

        if (success) {
            hermesTokensDividendTracker.distributeTokensDividends(dividends);
            emit SendDividends(tokens, dividends);
        }
    }
    ...
    function isExcludedFromFee(address account)
        ...
        returns (bool)
    {
        return tykheFortuneDistributor.isExcludedFromFee(account);
    }
    ...
    function excludeFromFee(address account, bool val)
        ...
        onlyOwner
    {
        tykheFortuneDistributor.setExcludedFromFee(account, val);
    }
    ...
    function getFlowersRewards(address account) public view returns (uint256) {
        uint256 totalUnclaimed = 0;
        uint256 totalFlowers = 0;

        for (uint256 typeIndex = 0; typeIndex < totalFlowers; typeIndex++) {
            totalUnclaimed += ((uint256(block.timestamp - _lastClaim[account]) *
                createDailyEarnings) / createDailyInterval);
        }

        return totalUnclaimed;
    }

    function claimFlowersRewards(address account) external {
        uint256 totalUnclaimed = getFlowersRewards(account);
    ...
    function claimBusd() external view {
        hermesBusdDividendTracker.withdrawDividend();
    }

    function getWithdrawableDividendOf(address account)
        public
        view
        returns (uint256)
    {
        return hermesBusdDividendTracker.withdrawableDividendOf(account);
    }
}
    ...
    IDividendPayingTokenOptional
{
    ...
    function distributeTokensDividends(uint256 amount) public {
        require(totalSupply() > 0);
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            bool success = IERC20(distributionTokenAddress).transfer(
                user,
                _withdrawableDividend
            );
...
contract HermesDividendTracker is DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address _distributionTokenAddress)
        DividendPayingToken(
            "TestToken_Dividend_Tracker",
            "TestToken_Dividend_Tracker",
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "TestToken_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TestToken contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "TestToken_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "TestToken_Dividend_Tracker: Cannot update claimWait to same value"
        );
