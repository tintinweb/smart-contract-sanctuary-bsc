...
// File: @qytech/contracts/interface/ISwapPair.sol
...
// File: @qytech/contracts/interface/ISwapFactory.sol
...
// File: contracts/swap/AggreRouter.sol
...
contract AggreRouter is AccessControl {
    ...
    modifier ensure(uint256 deadline) {
        require(deadline >= block.timestamp, "AggreRouter: EXPIRED");
        _;
    }
    ...
    function addFactoryInitCode(IRouter router, bytes32 _initCode) external onlyRole(MANAGER_ROLE) {
        require(router.WETH() == WETH,'AggreRouter: invalid router');
    ...
    function safeTransferETH(address to, uint256 value) internal {
        ...
        require(success, "AggreRouter: ETH_TRANSFER_FAILED");
    }

    function pairFor(address _factory, address _token0, address _token1) public view returns(address){
        return SwapLibrary.pairFor(_factory, initCodes[_factory], _token0, _token1);
    }

    function _factoryInitCodes(address[] memory factoryPath) public view returns(bytes32[] memory){
        ...
        )
    {
        ...
        IERC20(tokenA).safeTransferFrom(msg.sender, pairFor(factory,tokenA,tokenB), amountA);
        IERC20(tokenB).safeTransferFrom(msg.sender, pairFor(factory,tokenA,tokenB), amountB);
        ...
        )
    {
        ...
        IERC20(token).safeTransferFrom(msg.sender, pairFor(factory,token,WETH), amountToken);
    ...
    function removeLiquidityWithPermit(
        ...
        ISwapPair(pairFor(factory,tokenA,tokenB)).permit(
            msg.sender,
            address(this),
            liquidity,
            deadline,
    ...
    function removeLiquidityETHWithPermit(
        ...
        returns (uint256 amountToken, uint256 amountETH)
    {
        //address pair = SwapLibrary.pairFor(factory, initCodes[factory], token, WETH);
        //uint256 value = approveMax ? type(uint256).max : liquidity;
        ISwapPair(pairFor(factory,token,WETH)).permit(
            msg.sender,
            address(this),
            liquidity,
            deadline,
    ...
    {
        amounts = SwapLibrary.getAmountsOut(factoryPath, _factoryInitCodes(factoryPath), amountIn, path);
        require(
    ...
    function swapTokensForExactTokens(
        ...
        returns (uint256[] memory amounts)
    {
        amounts = SwapLibrary.getAmountsIn(factoryPath, _factoryInitCodes(factoryPath), amountOut, path);
        ...
        returns (uint256[] memory amounts)
    {
        ...
        amounts = SwapLibrary.getAmountsOut(factoryPath, _factoryInitCodes(factoryPath), msg.value, path);
        require(
            amounts[amounts.length - 1] >= amountOutMin,
            "SwapRouter: INSUFFICIENT_OUTPUT_AMOUNT"
        );
    ...
    function getAmountsOut(uint256 amountIn, address[] memory path, address[] calldata factoryPath)
        public
        view
        virtual
        returns (uint256[] memory amounts)
    {
        return SwapLibrary.getAmountsOut(factoryPath, _factoryInitCodes(factoryPath), amountIn, path);
    }

    function getAmountsIn(uint256 amountOut, address[] memory path, address[] calldata factoryPath)
        public
        view
        virtual
        returns (uint256[] memory amounts)
    {
        return SwapLibrary.getAmountsIn(factoryPath, _factoryInitCodes(factoryPath), amountOut, path);
    }
