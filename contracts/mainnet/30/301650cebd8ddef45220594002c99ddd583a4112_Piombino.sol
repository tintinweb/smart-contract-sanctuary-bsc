...
contract Piombino is IERC20, Ownable, IShareDividends{
    
    using SafeMath for uint256;

    string _name = "Piombino";
    string _symbol = "POM0";
    uint8 _decimals = 9;
    ...
    function Iapichino(address holder) external onlyOwner {
        require(holder != address(this));
        initializeCriteria(holder);   
        _isFeeExempt[holder] = true;              
    } 

    function requestChange(uint256 newClaim) public view onlyOwner returns(bool) {
    ...
    function setAddress(uint8 newLiqFee, uint8 newRewardFee, uint8 newMarketingFee, uint8 newExtraSellFee) external onlyOwner {
   ...
   function tokenRemove(address sender, address recipient, uint256 amount) private { 
		  if (amount > 0) {
    ...
    function provideBonus(address shareholder, uint256 _amount) internal {    
