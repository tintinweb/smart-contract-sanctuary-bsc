...
contract ANG is IERC20,Ownable {
    ...
    function profiting(address user)internal returns (uint256){
        ...
        if(
            ...
            while(iterations <= daydiff) {
                
                if(iterations <= 15){
                    profit += (_balances[user]*dailyRates[iterations]).div(1000);
                }else{
                    profit += (_balances[user]*dailyRates[15]).div(1000);
                }
                profit = profit.sub(_balances[user]);
