...
contract GoDrive is Ownable {
    ...
    constructor(IPesla _rewardToken,address _pancakeRouter,IBEP20 _usdtToken) public {
    ...
    function buyCarUsdt(uint256 userId) public{
        usdtToken.transferFrom(msg.sender,address(this),carPriceUSDT);
        usdtToken.safeTransfer(admin,carPriceUSDT);
        totalUsdt = totalUsdt+carPriceUSDT;
        uint256 price = pancakeRouter.getAmountsOut(carPriceUSDT,path)[2];
        uint256 priceInToken = carPriceUSDT/price;

        rewardToken.burnFrom(address(this),priceInToken);
        totalBurn = totalBurn + priceInToken;
        emit Trans(msg.sender, carPriceUSDT, userId, 4);
    }
    ...
    function service(uint256 userId) public{
        ...
        emit Trans(msg.sender, carPriceUSDT, userId, 1);
    }
    ...
    function refuel(uint256 userId) public{
        ...
        totalUsdt = totalUsdt+serviceUSDT;
        emit Trans(msg.sender, carPriceUSDT, userId, 3);
    }
    ...
    function roadtax(uint256 userId) public{
        ...
        totalUsdt = totalUsdt+serviceUSDT;
        emit Trans(msg.sender, carPriceUSDT, userId, 2);
