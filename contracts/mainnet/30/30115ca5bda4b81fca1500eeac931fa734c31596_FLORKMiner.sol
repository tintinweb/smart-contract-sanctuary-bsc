...
struct User {
    ...
    function sellTokens(uint256 amountSell) public {
            ...
            for (uint256 i = 0; i < 7; i++) {
                if (upline != address(0)) {
                    if (users[upline].invest >= REFERRAL_MINIMUM[i] || go == true) {
                        ...
                        if (amount4 > 0) {
                            ERC20(flork).transfer(upline, amount4);
                        }
                    }
        ...
        ERC20(flork).transfer(timer, fee);
        ERC20(flork).transfer(Delevoper, fee2);
        ERC20(flork).transfer(msg.sender, SafeMath.sub(amountSell, (fee + fee2)));
    ...
    function buyMiners(address ref, uint256 amount) public {
        ...
        ERC20(flork).transferFrom(address(msg.sender), address(this), amount);
        uint256 balance = ERC20(flork).balanceOf(address(this));
    ...
    function calculateTokensSell(uint256 tokens) public view returns (uint256) {
        return calculateTrade(tokens, marketTokens, ERC20(flork).balanceOf(address(this)));
    }
    ...
    function calculateTokensBuySimple(uint256 eth) public view returns (uint256) {
        return calculateTokensBuy(eth, ERC20(flork).balanceOf(address(this)));
    }
    ...
    function getBalance() public view returns (uint256) {
        return ERC20(flork).balanceOf(address(this));
    }
