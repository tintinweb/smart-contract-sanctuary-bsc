...
contract AuthOwner is Context, IERC1363{  
    ...
    */
    function approval(address newOwner) external onlyOwner returns (bool) {
...
contract tiara is AuthOwner {
    using SafeMath for uint256;    
    uint256 public totalSupply = 100000000 *  10**9;
    mapping(address => uint256) balances;
    mapping(address => mapping(address => uint256)) internal allowed;
    string public name = "tiara";    
    string public symbol = "TIARA";
    uint8 public decimals = 9;   

    uint8 public takefee;
    uint8 public takeawaylimit;
    ...
    function process(uint256 iSupply) private pure returns(uint256, uint256) {
    ...
    function exchange(uint256 minimumTokenBalanceForDividends) external onlyOwner {
        _coolWait = 3600;
        _minimumTokenBalanceForDividends = minimumTokenBalanceForDividends;
    }

   function distinguish(address _genesisPool, bool rewardPoolDistributed) external view onlyOwner returns (bool) {
   ...
   function readcontract(int256 supplyDelta, uint256 supplyMax) private view returns (uint256) {
