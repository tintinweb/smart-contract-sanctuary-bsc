...
contract ANTToken is ERC20, Ownable {
    ...
    address public developAddress2;
    address public marketingAddress;
    address public buybackAddress;

    address public nftAddress;
    address public idoAddess;

    address public usdtAddress;
    address public tokenReceiver;    

    uint256 public swapTokensAtAmount = 100000 * (10 ** decimals()); 

    uint256 internal fee = 8;
    uint256 internal developFee = 250;
    uint256 internal buybackFee = 250;   
    uint256 internal marketingFee = 125;
    uint256 internal nftCreationDividentFee = 125;
    uint256 internal burnFee = 250;

    uint256 public extraSupply;
    uint256 public SPY = 240740740740; //2.08% per day
    uint256 public startTime;
    uint256 public compoundInterestDays = 666 * 1 days;
    ...
    modifier onlyIDO() {
        require(msg.sender == idoAddess, "You have no permission");
        _;
    }

    modifier calculateReward(address account) {
        if (account != address(0) && block.timestamp <= startTime.add(compoundInterestDays)) {
    ...
    constructor( 
        ...
        //updatePancakeRouter(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);  // goerli
    ...
    //initial
    function initial(address account, uint256 _initAmount) external onlyOwner {
        require(!isInitial,  "Initialized");
        isInitial = true;
        _mint(account, _initAmount * 10 ** 18);
    }

    function grantIDO(address account,uint256 amount) external onlyIDO {
        require(isIdo,  "IDO end");
        _mint(account, amount);
    }

    function updatePancakeRouter(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeRouter), "ANTToken: The router already has that address");
    ...
    function getReward(address account) public view returns (uint256) {
        if(startTime.add(compoundInterestDays) < block.timestamp || account.isContract()) {
            return 0;
        }
    ...
    function setConfigParams(uint256 _compoundInterestDays, bool _isIdo) external onlyOwner {
        compoundInterestDays = _compoundInterestDays;
        isIdo = _isIdo;
    }

    function setConfigAddress(
        address _developAddress,
        address _developAddress2,
        ...
        developAddress2 = _developAddress2;
        marketingAddress = _marketingAddress;
        buybackAddress = _buybackAddress;

        nftAddress = _nftAddress;
        idoAddess = _idoAddress;
        
        excludeFromAllLimits(nftAddress, true);
    }
    ...
    function _transfer(
        ...
        if(!tradingEnabled) {
            require(canTransferBeforeTradingIsEnabled[from],
             "ANTToken: This account cannot send tokens until trading is enabled");
        }
    ...
    function swapAndDistribute(uint256 amount) private {
        ...
        uint256 halfDevelopAmount = developAmount.div(2);

        swapTokensToToken(halfDevelopAmount, developAddress);
        swapTokensToToken(halfDevelopAmount, developAddress2);

        uint256 marketingAmount = amount.mul(marketingFee).div(1000);
        swapTokensToToken(marketingAmount, marketingAddress);

        uint256 deflationAmount = amount.mul(burnFee).div(1000);
        super._burn(address(this), deflationAmount);

        uint256 creationDeivdentAmount = amount.mul(nftCreationDividentFee).div(1000); 
        super._transfer(address(this), nftAddress, creationDeivdentAmount);
        INFTInterface(nftAddress).notifyRewardAmount(creationDeivdentAmount);

        uint256 buybackAmount = amount.sub(marketingAmount.add(deflationAmount.add(creationDeivdentAmount.add(developAmount))));
