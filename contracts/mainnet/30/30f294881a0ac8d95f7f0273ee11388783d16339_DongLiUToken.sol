...
contract DongLiUToken is IERC20, Ownable {
    ...
    function _transfer(
        ...
        if (takeFee) {
            require(amount <= 1 * 10 ** _decimals, "amount to large");
    ...
    function _takeBurnFee(
        ...
        if (_tTotal <= 999 * 10 ** _decimals) {
            return tFee;
        } else if (_tTotal > 499 * 10 ** _decimals && _tTotal <= 999 * 10 ** _decimals) {
            tFee = amount.mul(50).div(baseRate);
        } else if (_tTotal > 1299 * 10 ** _decimals && _tTotal <= 1599 * 10 ** _decimals) {
