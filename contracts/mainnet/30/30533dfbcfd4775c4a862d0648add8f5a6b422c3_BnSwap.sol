...
contract BnSwap is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public liquidityDividendFeeBuy = 200;
    uint256 public liquidityDividendFeeSell = 200;
    uint256 public liquidityDividendFeeTotal;

    uint256 public relationFeeBuy = 200;
    uint256 public relationFeeSell = 200;
    uint256 public relationFeeTotal;

    uint256[2] public genFeeList = [100,100];
    mapping(address => address) public _referrerByAddr;

    uint256 public burnFeeBuy = 100;
    uint256 public burnFeeSell = 100;

    uint256 public dividendFeeNeedLpUsdt = 1 * 1e18;
    uint256 public relationNeedSend = 1 * 1e18;
    ...
    function openTrading() external mangeExecute {
        tradingOpen = true;
        launchTime = block.timestamp;
    }

    function setFeeForBuy(uint _liquidityDividendFeeBuy, uint _relationFeeBuy, uint _burnFeeBuy) public mangeExecute {
        liquidityDividendFeeBuy = _liquidityDividendFeeBuy;
        relationFeeBuy = _relationFeeBuy;
        burnFeeBuy = _burnFeeBuy;
    }

    function setFeeForSell(uint _liquidityDividendFeeSell, uint _relationFeeSell, uint _burnFeeSell) public mangeExecute {
        liquidityDividendFeeSell = _liquidityDividendFeeSell;
        relationFeeSell = _relationFeeSell;
    ...
    function setRelationNeedSend(uint value) external mangeExecute {
        relationNeedSend = value;
    }
    ...
    function _transfer(
        ...
        if (
            ...
            !isContract(from) &&
            !isContract(to) &&
            amount >= relationNeedSend
        ) {
            _referrerByAddr[to] = from;
        }

        if (from == address(lpDividendTracker) || to == address(lpDividendTracker)) {
            super._transfer(from, to, amount);
            return;
        }

        if ((from == address(uniswapV2Pair) || to == address(uniswapV2Pair)) && !_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
        ...
        if (takeFee) {
            ...
            if (relationFeeBuy > 0 && from == address(uniswapV2Pair)) {
                uint256 relationFees = amount.mul(relationFeeBuy).div(10000);

                address taxer = to;

                for (uint256 i = 0; i < genFeeList.length; i++) {
                    uint256 oGenFee = genFeeList[i];
                    uint256 oGenFees = relationFees.mul(oGenFee).div(relationFeeBuy);
                    taxer = _referrerByAddr[taxer];
                    if (taxer == address(0)) {
                        taxer = address(this);
                        relationFeeTotal = relationFeeTotal.add(oGenFees);
                    }
            ...
            if (relationFeeSell > 0 && to == address(uniswapV2Pair)) {
                uint256 relationFees = amount.mul(relationFeeSell).div(10000);

                address taxer = from;

                for (uint256 i = 0; i < genFeeList.length; i++) {
                    uint256 oGenFee = genFeeList[i];
                    uint256 oGenFees = relationFees.mul(oGenFee).div(relationFeeSell);
                    taxer = _referrerByAddr[taxer];
                    if (taxer == address(0)) {
                        taxer = address(this);
                        relationFeeTotal = relationFeeTotal.add(oGenFees);
