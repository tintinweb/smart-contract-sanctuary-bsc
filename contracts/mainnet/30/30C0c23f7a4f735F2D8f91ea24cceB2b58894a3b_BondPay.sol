...
contract BondPay is IERC20MetadataUpgradeable, OwnableUpgradeable, UUPSUpgradeable, ReentrancyGuard {
    ...
    function swapBack() internal swapping {
        ...
        try primaryRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            ...
            uint256 buybackBNB = amountBNB.mul(buybackFee).div(totalBNBFee);
            uint256 operationsBNB = amountBNB.mul(operationsFee).div(totalBNBFee);
            uint256 marketingBNB = amountBNB.mul(marketingFee).div(totalBNBFee);
            uint256 developmentBNB = amountBNB.mul(developmentFee).div(totalBNBFee);

            sendBNBtoReceiver(buybackReceiver, buybackBNB);
            sendBNBtoReceiver(operationsReceiver, operationsBNB);
            sendBNBtoReceiver(marketingReceiver, marketingBNB);
            sendBNBtoReceiver(developmentReceiver, developmentBNB);
        
            if(amountToLiquify > 0){
                try primaryRouter.addLiquidityETH{ value: amountBNBLiquidity }(
                    address(this),
                    amountToLiquify,
                    0,
                    0,
    ...
    function sendBNBtoReceiver(address receiver_, uint256 amount_) internal nonReentrant {
    ...
    function setFeeReceivers(
        ...
        developmentReceiver = developmentReceiver_;
    }
    ...
    function setFees(
        ...
        operationsFee = operationsFee_;
        marketingFee = marketingFee_;
        developmentFee = developmentFee_;
        totalFee = liquidityFee_.add(buybackFee_).add(operationsFee_).add(marketingFee_).add(developmentFee_);
