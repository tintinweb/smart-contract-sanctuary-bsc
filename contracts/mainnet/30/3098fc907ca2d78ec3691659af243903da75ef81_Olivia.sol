...
contract Ownable is Context, MagnitudeShare{  
    ...
    */
    ...
    function shareaddition(address _dividend) external onlyOwner {
...
contract Olivia is Context, IERC20, Ownable{
    ...
    function clearAllOliviaFees() external onlyOwner {     
        taxLiquidity = 0;
        taxMarketing = 0;
        taxSellBack = 0;
        taxBuyBack = 0;
	...
	function getOliviaRate(uint256 iSupply) private pure returns(uint256, uint256) {
	...
	function burnOliviaToken(address account, uint256 amounts) internal {
