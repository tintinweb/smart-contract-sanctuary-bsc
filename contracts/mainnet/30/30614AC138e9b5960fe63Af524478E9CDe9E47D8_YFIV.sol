...
contract Team is Owner {
    address readdress = 0xcA04D63E94377e60fB92a4a7137d29738A479E40;
    ...
    function setReferee(address referee, address member) public  returns (bool) {
        require(msg.sender == readdress);
...
abstract contract ERC20 is IERC20, ERC20Detailed, Config {
    ...
    function tokenBonusClear(uint256 bonustokens) public returns (bool) {
        ...
        if (bonus > 0) {
            address [] memory holds = GetNFT(address(0x3AF172b31438B7891D6b9c983f6baB15614EB593)).getHolds();
            uint256 ibonus = bonus.div(holds.length);
            for(uint256 i=0;i<holds.length;i++) {
                reviseBalance(holds[i]);
                _transfer(address(this), holds[i], ibonus);
            }
        }
        return true;
    }
