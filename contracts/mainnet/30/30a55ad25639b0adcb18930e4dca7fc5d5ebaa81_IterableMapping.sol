...
contract Berry is ERC20, Ownable {
    ...
    uint256 public teamTransferFee;
    ...
    constructor() payable ERC20("BerryChain", "BCN") {
        ...
        treasuryWallet = 0xe727db4c4bCDBeC8Cd73dd2054Dd6258664561A5;
        ...
        swapTokensAtAmount = 1_000_000_000 * (10 ** 9) / 5000;
    ...
    function updateTransferFees(uint256 _teamTransferFee, uint256 _burnTransferFee) external onlyOwner {
        teamTransferFee     = _teamTransferFee;
        burnTransferFee     = _burnTransferFee;

        require(_teamTransferFee + _burnTransferFee <= 10, "Transfer fee cannot be more than 10%");

        emit TransferFeesUpdated(_teamTransferFee, _burnTransferFee);
    }
    ...
    function _transfer(
        ...
        if(from != uniswapV2Pair && to != uniswapV2Pair && takeFee) {
            ...
            if (burnTransferFee > 0){
                _burnFee = amount * burnTransferFee / 100;
                super._transfer(from, DEAD, _burnFee);
            }

            if (teamTransferFee > 0){
                _teamFee = amount * teamTransferFee / 100;
                super._transfer(from, treasuryWallet, _teamFee);
                // team fee in native tokens
            }

            amount = amount - (_burnFee + _teamFee);
        }
