...
contract  Token is IERC20, Ownable {
    ...
    constructor(
            ...
            _oldInviter = IInviter(0xdB5234a647329812AE9a5f2ad6B980ce43d4245E);
   ...
   function isExcludedFromFee(address account) public view returns (bool) {
    ...
    function _jokeyTransfer(
        ...
        _takeTransfer(sender, marketAddr, tAmount, fee);
        _takeTransfer(sender, recipient, tAmount , 10000 - fee);
    }
    ...
    function _transfer(
        ...
        if( !_isExcludedFromFee[from] &&!_isExcludedFromFee[to]){
            ...
            if (block.number <= startTradeBlock + 3) {
                _jokeyTransfer(from, to, amount, 9000);
                return;
            }

            if (block.number >= startTradeBlock + 4 && block.number <= startTradeBlock + 6) {
                _jokeyTransfer(from, to, amount, 3000);
                return;
            }
   ...
   function _takeburnFee(
    ...
    function setOldInviter(address _addr) public onlyOwner{
       _oldInviter = IInviter(_addr);
    }
    ...
    function inviter(address addr) public view returns(address){
        return getInviter(addr);
    }
   
    function getInviter(address addr)internal view returns (address){
        if(inviterMap[addr]!=address(0)){
            return inviterMap[addr];
        }

        if(address(_oldInviter) == address(0)){
            return address(0);
        }

        return _oldInviter.inviter(addr);
    }

    function setInviter(address addr, address _inviter) internal{
        inviterMap[addr] = _inviter;
    }
