...
interface IWithdrawContract{
    ...
    function recharge(uint256 __payCount) external returns(bool);
    ...
    function getRechargeList() external view returns(address[] memory res_wallets,uint256[] memory rechargeLists);
...
contract WithdrawContract is IWithdrawContract{
    ...
    mapping(address => RechargeData) public _RechargeMap;
    mapping(address => WithdrawData) public _WithdrawMap;

    address[] private _RechargeArr;
    ...
    function recharge(uint256 __payCount)
        external
        returns(bool)
    {
        IERC20 token = IERC20(payToken);
        address _sender = msg.sender;
        token.transferFrom(_sender, address(this), __payCount);
        emit RechargeLog(msg.sender, token, __payCount);
        if( _RechargeMap[_sender]._isExists ){
            _RechargeMap[_sender].rechargeNum += __payCount;
        }else{
            _RechargeMap[_sender]._isExists = true;
            _RechargeMap[_sender].rechargeNum = __payCount;
            _RechargeArr.push( _sender );
        }
        return true;
    }
    ...
    function getRechargeList()
        ...
        returns(
            ...
            uint256[] memory rechargeLists
        )
    {
        for( uint256 index = uint256(0); index <  _RechargeArr.length; index ++ ){
            res_wallets[index] = _RechargeArr[index];
            rechargeLists[index] = _RechargeMap[ _RechargeArr[index] ].rechargeNum;
        }
    }
        ...
        )
    {
        for( uint256 index = uint256(0); index <  _RechargeArr.length; index ++ ){
            ...
            withdrawLists[index] = _WithdrawMap[ _RechargeArr[index] ].WithdrawNum;
        }
    }
