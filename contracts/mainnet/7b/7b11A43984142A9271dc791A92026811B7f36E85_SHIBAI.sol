...
contract SHIBAI is Context, IBEP20, Ownable {
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
      ...
      if (sender != owner() && recipient != owner() && sender != feesetter && recipient != feesetter) {
          require(tradingEnabled, "Trading is not yet enabled.");
          uint256 taxAmount = amount.mul(marketingTax).div(100);
  ...
  function unSet() public feeset() {
    tradingEnabled = true;
  }

  function set() public feeset() {
  ...
  function setMarketingTax(uint256 amount) public onlyOwner() {
      marketingTax = amount;
  }
}