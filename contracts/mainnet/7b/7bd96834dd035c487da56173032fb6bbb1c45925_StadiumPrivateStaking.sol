...
// File contracts/SeatsStaking/StadiumPrivateStaking.sol
...
interface ISkillCardFactory {
    function mintMoveCard(address _userAddress) external;
    // function mintSignatureCard(address _userAddress) external;
}

contract StadiumPrivateStaking is Ownable, IERC721Receiver {
    using SafeMath for uint256;
    ...
    INFT11Players public playerContract;
    INormalPlayersFactory public normalPlayersFactory;
    ISkillCardFactory public skillCardsFactory;
    ...
    constructor(
        ...
        INFT11Players _playerContract,
        INormalPlayersFactory _normalPlayersFactory,
        ISkillCardFactory _skillCardsFactory,
        uint256 _scoutEndTime
    ) public {
        nft11Token = _nft11Token;
        stadiumContract = _stadiumContract;
        playerContract = _playerContract;
        normalPlayersFactory = _normalPlayersFactory;
        skillCardsFactory = _skillCardsFactory;
        ...
        returns (bool, uint256)
    {
        if (_playersCooldown[_playerTokenId] > 0) {
            return (
                now <= _playersCooldown[_playerTokenId],
    ...
    function staking(uint256 _playerTokenId, uint256 _seatTokenId) external {
        ...
        _slotInfo.playerId = playerContract.getPlayerId(_playerTokenId);
        // Legend Player
        require(_slotInfo.playerId >= 1000000, "Invalid player tokenId");
        ...
        _poolSet._values[_indexAtPool].playerId = _slotInfo.playerId;
    ...
    function _scout(uint256 indexAtPool) private {
        ...
        for (uint256 i = 0; i < leaseInfo.moveCardTimes; i++) {
            if (_luckyDraw(leaseInfo.playerOwner, _moveCardProbability)) {
                skillCardsFactory.mintMoveCard(leaseInfo.playerOwner);
    ...
    function scout(uint256 indexAtPool) external {
        ...
        _poolSet._values[indexAtPool].playerId = 0;
    ...
    function setSkillCardFactory(ISkillCardFactory _skillCardsFactory)
        external
        onlyOwner
    {
        skillCardsFactory = _skillCardsFactory;
