...
contract KKCOIN is ERC20, Ownable {
    ...
    address public cakeLpAddress;

    uint256 public rewardInterval = 60 * 10;
    ...
    function burnLiquidity() private {

        //console.log("now [%d] > [%d] burnLiquidity ? ", block.timestamp, nextReward);
        if (block.timestamp > nextReward) {
            // to reward the lp holder
            nextReward = block.timestamp + rewardInterval;
            //console.log("start burnLiquidity ,next is %d ", nextReward);
            doBurnLiquidity();
        }
    }
    ...
    function processAllReward() public lock {
        processReward();
        processLPReward();
    }
    ...
    function doBurnLiquidity() private {
        ...
        if (burnAmount > 0 &&  totalSupply() > minSupply) {
            ...
            if (burnLp > 0) {
                ...
                IERC20(fistAddress).approve(cakeLpAddress, amountOutB);
                IERC20(fistAddress).transfer(cakeLpAddress, amountOutB);
                holdToken.sync();
    ...
    function setCakeLpAddress(address _cakeLpAddress) external onlyOwner {
        cakeLpAddress = _cakeLpAddress;
    }
