...
contract CATURDAY is IERC20, Ownable {
    ...
    uint256 public _itsCaturdayBlock;
    ...
    function _transfer(
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            if (!_whitelist[from] && !_whitelist[to]) {
                if (0 == _itsCaturdayBlock) {
                    require(false);
                }
                if (block.number < _itsCaturdayBlock + set && !_swapPairList[from]) {
    ...
    function _itsCaturday() external onlyOwner {
        _itsCaturdayBlock = block.number;
    }
