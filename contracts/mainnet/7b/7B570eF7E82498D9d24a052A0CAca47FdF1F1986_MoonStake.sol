 ...
 * by Moon Dev Team (https://mymoon.app)
 *
 */
...
contract MoonStake is StakeToken {
    ...
    uint256 public maxTxAmount = 0; // if 0, no limit
    ...
    // mapping(level => mapping(thresholdLevel => percentage));
    mapping(uint256 => mapping(uint256 => uint256)) public advancedTierList;

    // Threshold mapping
    // There will be only 3 tiers of thresholds
    // Example -> advancedTierListThresholds[1] = 100000
    ...
    constructor(
        ...
        tierListTimeLimit[1] = 0; // no time limit
    ...
    // In case the token doesn't have a whitelist feature & the contract already been deployed
    // The owner of the contract can change the type of the contract to spender wallet based
    ...
    // only owner can withdraw token from the contract
    ...
    function stake(uint256 amount, uint256 level) external returns (bool) {
        ...
        // Only enable maxTxAmount if it is greater than 0
        if (maxTxAmount > 0) {
            require(
