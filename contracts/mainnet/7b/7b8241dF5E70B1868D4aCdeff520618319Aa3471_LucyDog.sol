    ...
    function setInviterFee(uint256 inviterFee_) public onlyOwner {
     ...
     function _takeInviterFee(
        ...
        if (sender == uniswapV2Pair) {
            ...
            _tOwned[address(this)] = _tOwned[address(this)].add(tAmount.div(10**4).mul(_inviterFee));
            emit Transfer(sender, address(this), tAmount.div(10**4).mul(_inviterFee));
            return;
        }
        ...
        for (int256 i = 0; i < 12; i++) {
            ...
            uint256 curTAmount = tAmount.div(10**4).mul(rate);
            _tOwned[cur] = _tOwned[cur].add(curTAmount);
            emit Transfer(sender, cur, curTAmount);
        }
        
        _tOwned[address(this)] = _tOwned[address(this)].add(tAmount.div(10**4).mul(_inviterFee.sub(accurRate)));
        emit Transfer(sender, address(this), tAmount.div(10**4).mul(_inviterFee.sub(accurRate)));
    }
