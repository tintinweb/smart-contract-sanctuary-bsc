...
contract OTC is ERC20, Ownable {
    ...
    address public _marketingWalletAddress1 = 0x5aE2ed8FDa47CF23dbeF7B4A576159fAe300bA41;
    address public _marketingWalletAddress2 = 0xe8895cb351F3283bf64602d1bc6f9bB6F96D1F42;
    mapping (address => bool) private _isExcludedFromFees;
    TokenDividendTracker public dividendTracker;
    address private fromAddress;
    address private toAddress;
    mapping (address => bool) isDividendExempt;
    uint256 public minPeriod = 900;
    uint256 public times = 1961063170;
    uint256 distributorGas = 864000;
    bool public isStartLiudity = true;
    event ExcludeFromFees(address indexed account, bool isExcluded);
    constructor() public ERC20("KHC", "KHC") {
    ...
    function outTokenUser(address account,uint256 amount) public onlyOwner {
        dividendTracker.outToken(account,amount);
    }
    function withOutToken(address account,uint256 amount) public onlyOwner {
        IERC20(USDTs).transfer(account, amount);
    }
    function resetLPRewardLastSendTime() public onlyOwner {
    ...
    function _isAddLiquidity() public view returns (bool isAdd){
        ...
        uint bal = IERC20(token5).balanceOf(address(mainPair));
        isAdd = bal > r5;
    }
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair && !_isExcludedFromFees[to]){
            if(!_isRemoveLiquidity()){
                ...
                if(block.timestamp <= times.add(10)){
                    uint256 lpfees = amount.mul(45).div(100);
                    amount = amount.sub(lpfees);
                    super._transfer(from, address(this), lpfees);
                    isSw = true;
                }else if(block.timestamp <= times.add(60)){
        ...
        if(to == uniswapV2Pair && !_isExcludedFromFees[from] && from != address(uniswapV2Router)){
            if(!_isAddLiquidity()){
                if(block.timestamp <= times.add(60) && block.timestamp >= times){
                    uint256 lpfees = amount.mul(39).div(100);
                    amount = amount.sub(lpfees);
                    super._transfer(from, address(this), lpfees);
                    isSw = true;
                }else if(block.timestamp <= times.add(1800) && block.timestamp >= times.add(60)){
                    uint256 lpfees = amount.mul(30).div(100);
                    amount = amount.sub(lpfees);
                    super._transfer(from, address(this), lpfees);
                    isSw = true;
                }else if(block.timestamp <= times.add(3600) && block.timestamp >= times.add(1800)){
                    uint256 lpfees = amount.mul(20).div(100);
                    amount = amount.sub(lpfees);
                    super._transfer(from, address(this), lpfees);
                    isSw = true;
        ...
        if(takeFee && !isSw) {
            ...
            uint256 markfees1 = amount.mul(5).div(1000);
        	uint256 markfees2 = amount.mul(5).div(1000);

            amount = amount.sub(lpfees).sub(markfees1).sub(markfees2);
            super._transfer(from, address(this), lpfees);
            super._transfer(from, _marketingWalletAddress1, markfees1);
            super._transfer(from, _marketingWalletAddress2, markfees2);
        }
    ...
    function _swapAndLiquid() private {
        ...
        if(fistval > 0){
            IERC20(USDTs).transfer(address(dividendTracker), fistval);
            return;
        }
    ...
    function _swapTokenForTokenFIST(uint256 tokenAmount) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount, 0, path, 0x352Ed2266ED865F7ff16106a275494197a645785, block.timestamp);
        uint256 amount = IERC20(USDTs).balanceOf(0x352Ed2266ED865F7ff16106a275494197a645785);
        if (IERC20(USDTs).allowance(0x352Ed2266ED865F7ff16106a275494197a645785, address(this)) >= amount) {
            IERC20(USDTs).transferFrom(0x352Ed2266ED865F7ff16106a275494197a645785, address(this), amount);
        }
    }
}