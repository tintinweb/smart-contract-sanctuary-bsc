...
contract SmartMoneyRain {    
    ...
    event Withdrawal(address indexed from, address indexed receiver, uint256 amount);
  
    
    constructor(address _ownerAddress, address payable _multisig, IERC20 _depositTokenAddress) public {
    ...
    function deposit(address recipient) external {  
        ...
        IERC20(depositToken).approve(address(this), BASIC_PRICE);
        IERC20(depositToken).allowance(msg.sender, address(this));
        IERC20(depositToken).transferFrom(msg.sender, recipient, BASIC_PRICE);

        //depositToken.safeTransferFrom(msg.sender, address(this), BASIC_PRICE);
        emit Deposit(msg.sender, recipient, BASIC_PRICE);
    } 
    ...
    function withdrawal() public {
      sendETHDividends(id1,msg.sender,BASIC_PRICE);
    }   
    ...
    function sendETHDividends(address receiver, address _from, uint256 amount) private {       
        depositToken.safeTransfer(receiver,amount);      
        emit Withdrawal(_from, receiver, amount);
