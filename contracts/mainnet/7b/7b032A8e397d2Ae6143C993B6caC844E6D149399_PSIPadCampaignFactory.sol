...
contract PSIPadCampaignFactory is IPSIPadCampaignFactory, Initializable, OwnableUpgradeable {
    ...
    function changeOwnerByAdmin(uint256 campaignId, address previousOwner, address newOwner) external onlyOwner {
        _changeOwner(campaignId, previousOwner, newOwner);
    }
    function changeOwner(uint256 campaignId, address newOwner) external isOwner(campaignId) {
        _changeOwner(campaignId, msg.sender, newOwner);
    }
    function _changeOwner(uint256 campaignId, address previousOwner, address newOwner) private {
        try PSIPadCampaign(campaigns[campaignId]).transferOwnership(newOwner) {} catch {}

        userCampaigns[newOwner].push(campaignId);
        bool lockFound = false;
        for (uint256 idx = 0; idx < userCampaigns[previousOwner].length; idx++) {
            if (lockFound || userCampaigns[previousOwner][idx] == campaignId) {
                if (idx < userCampaigns[previousOwner].length - 1) {
                    userCampaigns[previousOwner][idx] = userCampaigns[previousOwner][idx + 1];
                }
                lockFound = true;
            }
        }

        require(lockFound, 'PSIPadCampaignFactory: OLD_OWNER_NOT_FOUND');
        userCampaigns[previousOwner].pop();
        emit OwnerChanged(campaignId, previousOwner, newOwner);
    }
}
...
contract PSIPadCampaign is IPSIPadCampaign, Initializable, OwnableUpgradeable {
    ...
    /**
     * @dev Transfers ownership of the contract to a new account (`newOwner`).
     * Can only be called by the current owner or the PSI pad factory.
     */
    function transferOwnership(address newOwner) public override onlyPSIPadFactoryOrOwner {
        require(newOwner != address(0), "Ownable: new owner is the zero address");
        _transferOwnership(newOwner);
    }
...
interface IPSIPadCampaignFactory {
    ...
    event OwnerChanged(uint256 indexed campaignId, address indexed oldOwner, address indexed newOwner);
