...
contract ExecuteClaimProfitV2 is UnstakeLogic, StakeLogic, IExecutable {
  ...
  */
  function claimProfitAndExit(
    ...
    uint256 initialAmount = investment.initialAmount;
    ...
    uint256 profit = withdrawn > initialAmount ? withdrawn - initialAmount : 0; 

    if (profit > 0) {
      calculateAndSendProfitInType(_wl.toTokenAddress, profit, _claimProfitType);
    }
  ...
  */
  function claimProfitInToken(
    ...
    ) = factory.getTokenConversionAndStorage();

    (uint256 estimatedLiquidity, uint256 estimatedRewards) = TradeLibrary.estimateInvestment(
      _wl.fromliquidityPoolAddress,
      _un.amount,
      _wl.trade,
      _rewards,
      token,
  ...
  */
  function convertTokenToToken(
    ...
    (
      ITokenConversionLibrary tokenConversion,
      ITokenConversionStorage conversionStorage
    ) = _factory.getTokenConversionAndStorage();
...
interface IStakeLogic {
  ...
  struct ProvideLiquiditySet {
    ...
    address router;
    address factory;
    IExchange.AddLiquidityTrade trade;
  }
...
interface IFactory {
  ...
  function getTokenConversionAndStorage() external view returns(
