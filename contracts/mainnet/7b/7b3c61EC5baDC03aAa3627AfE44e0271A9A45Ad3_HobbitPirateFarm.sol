...
contract HobbitPirateFarm is Initializable, Context, ReentrancyGuard{
    ...
    receive() external payable{
        require(
            _msgSender() == wrapper,
            "HobbitPirateFarm : Only accept from wrapper"
        );
    }
    ...
    function _initialize(
        ...
        poolInfo.equal1 = 10**IERC20Metadata(depo).decimals();
        poolInfo.equal2 = 10**IERC20Metadata(income).decimals();
    }
    ...
    function userWantfarmIn(
        ...
        if(deposit == wrapper && msg.value > 0){
            ...
            require(
                IERC20(deposit).allowance(
                    _msgSender(),
                    address(this)
                ) >= amountfarm,
                "HobbitPirateFarm : Insufficient allocation for this transaction!"
            );
            IERC20(deposit).safeTransferFrom(
                _msgSender(),
                address(this),
                amountfarm
            );
    ...
    function _rewardPerSecondCalculator(
        ...
        unchecked{
            ...
            tempEqual = tempEqual / (10 ** IERC20Metadata(reward).decimals());
...
interface IHobbitPirateFarmFactory {
  ...
  function FarmAddress(uint256) external view returns(address);
  function FarmPair(address, address) external view returns(address);
