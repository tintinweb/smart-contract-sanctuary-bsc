...
// File @openzeppelin/contracts-old/GSN/[email protected]
...
// File @openzeppelin/contracts-old/access/[email protected]
...
// File contracts/distribution/TeamRewardPool.sol
...
contract TeamRewardPool is Ownable {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    address public constant ESHARE = 0xDB20F6A8665432CE895D724b417f77EcAC956550;
    address[] public TEAM = [
        0x0422B490424ce909285646d8532EC6a463bF2D9E, // MIKE
        0x6ef300aA8147C25C3f11409CA0F075aC57c1a142, // MARTY
        0x27a9B10213cD659467CDFfE2Eb8eBaedbdA3A57e, // AJ
        0xE6F91F9acB9Df852D624Ec4Ea249c32258DE3617, // DEMI
        0xE7C6ac3500DCF85e9E856Bd3144ee2b83f6e0a01  // LUNIZ
    ];
    struct User {
        uint256 alloc_point;
    ...
    function set(address _user, uint256 _allocPoint) public onlyOwner {
        totalAllocPoint = totalAllocPoint.sub(users[_user].alloc_point).add(_allocPoint);
        users[_user].alloc_point = _allocPoint;
    }
    
    function setUser(address _account, User memory user) public onlyOwner {
        totalAllocPoint = totalAllocPoint.sub(users[_account].alloc_point).add(user.alloc_point);
        total_claimed = total_claimed.sub(users[_account].total_claims).add(user.total_claims);
        users[_account] = user;
    }
    ...
    function getDistributionRewards(address account) public view returns (uint256) {
        ...
        uint256 distribute = users[account].alloc_point.mul(newDividendPoints).div(MULTIPLIER);
    ...
    function getTotalRewards(address _user) public view returns (uint256) {
        return users[_user].alloc_point > 0 ? 
            getDistributionRewards(_user).add(
                    getEShareBalancePool()
                        .mul(users[_user].alloc_point)
                        .div(totalAllocPoint)
                )
            : 0;
    }
    ...
    function claim() public {
        ...
        if (_rewards > 0) {
            ...
            IERC20(ESHARE).safeTransfer(_sender, _rewards);
    ...
    function _disperse(uint256 amount) internal {
        if (amount > 0 ) {
            totalDistributePoints = totalDistributePoints.add(amount.mul(MULTIPLIER).div(totalAllocPoint));
    ...
    function _dripRewards() internal {
        uint256 drip = getEShareBalancePool();
    ...
    function getEShareBalance() public view returns (uint256) {
        return IERC20(ESHARE).balanceOf(address(this));
    }

     function getEShareBalancePool() public view returns (uint256) {
        return getEShareBalance().sub(total_rewards);
    }
}