...
contract Marketer is Swap{
    ...
    constructor() public {
        ...
        userEndTime[msg.sender] = 99999999999;
        _setVipType(0,300*10**18,30*86400);
        _setVipType(1,10000*10**18,365*86400);
    ...
    function buyToken(address[] calldata path, uint256 slippage)external payable vipVerify{
        ...
        if(tokenU != WETH){
			...
			predicted = pancakeRouter.getAmountsOut(amountIn, jumpPath)[1];
			...
			require( amountIn >= predicted.mul(100-slippage)/100,"did not get the corresponding amount[1]");	
		...
		predicted = pancakeRouter.getAmountsOut(amountIn, path)[1];
        ...
        require( tokenAmountOut >= predicted.mul(100-slippage)/100,"did not get the corresponding amount[2]");	
	...
	//  TOKENU - TOKEN
    function buyTokenForToken(address[] calldata path, uint256 amountIn, uint256 slippage)external vipVerify{
        require(path[0] != path[1], 'INVALID_PATH');
		if(slippage > 100){
			slippage = 100;
		}
		
		uint256 predicted = pancakeRouter.getAmountsOut(amountIn, path)[1];
        uint256 tokenBal = IERC20(path[1]).balanceOf(address(this));
		...
		require( tokenAmountOut >= predicted.mul(100-slippage)/100,"did not get the corresponding amount");				
