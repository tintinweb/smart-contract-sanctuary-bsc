...
contract StakeSPLSE is Ownable {
    using SafeMath for uint256;

    address nullAddress = 0x0000000000000000000000000000000000000000;

    address public splseNFTAddress;
    
    address public amoTokenAddress;

    uint256 internal maxPerNftReward = 30 * 10 ** 18;

    uint256 public secondRewardPerNFT =  1504629629629; 
    ...
    constructor(
        address _amoTokenAddress,
        address _splseNFTAddress
    ) {
        amoTokenAddress = _amoTokenAddress;
        splseNFTAddress = _splseNFTAddress;
    }
    ...
    function stakeByIds(uint256[] memory tokenIds) public {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                IERC721(splseNFTAddress).ownerOf(tokenIds[i]) == msg.sender &&
                    tokenIdToStaker[tokenIds[i]] == nullAddress,
                "This NFT must be stakable by you!"
            );
            
            IERC721(splseNFTAddress).transferFrom(
                msg.sender,
                address(this),
                tokenIds[i]
            );
    ...
    function unstakeByIds(uint256[] memory tokenIds) public {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if(perTokenReward > 0) {
                ...
                amoTotalRewards = amoTotalRewards.add(perTokenReward);
            }
        ...
        uint256 tokenBalance = IERC20(amoTokenAddress).balanceOf(address(this));
        if(tokenBalance > 0 && tokenBalance < amoTotalRewards) {
            amoTotalRewards = tokenBalance;
        }
        if(amoTotalRewards > 0 && tokenBalance >= amoTotalRewards) {
            IERC20(amoTokenAddress).transfer(msg.sender, amoTotalRewards);
            emit ClaimReward(msg.sender, amoTotalRewards);
        }
    ...
    function claimAll() public {
        ...
        require(amoTotalRewards > 0, "You have no AMO Token claim");
        uint256 tokenBalance = IERC20(amoTokenAddress).balanceOf(address(this));
        require(tokenBalance > 0, "The mine has been hollowed out");
        if(tokenBalance > 0 && tokenBalance < amoTotalRewards) {
            amoTotalRewards = tokenBalance;
        }
        if(amoTotalRewards > 0) {
            IERC20(amoTokenAddress).transfer(msg.sender, amoTotalRewards);
            emit ClaimReward(msg.sender, amoTotalRewards);
        }
    }
    ...
    function getAllRewards(address staker) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if (perNftTotalReward[tokenIds[i]].add(reward) >= maxPerNftReward) {
                reward = maxPerNftReward.sub(perNftTotalReward[tokenIds[i]]);
            }
            if(reward > 0) {
        ...
        returns (uint256)
    {
        ...
        if (perNftTotalReward[tokenId].add(reward) >= maxPerNftReward) {
            reward = maxPerNftReward.sub(perNftTotalReward[tokenId]);
        }
        return reward;
    }
