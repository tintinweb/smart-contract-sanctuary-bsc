...
contract Staking is AccessControlUpgradeable, PausableUpgradeable {
  ...
  event IncreaseRewardsPool(
   ...
   function initialize(IERC20 _TKN) public initializer {
        ...
        _setupRole(DEFAULT_ADMIN_ROLE, 0x54caCf374e912930C07eC0016d14Fb9dD7482CC8);
        _setupRole(DEFAULT_ADMIN_ROLE, 0x709183d4C457D972e407B12B544dfd18db79Bc2C);
        _setupRole(DEFAULT_ADMIN_ROLE, 0x56308d0A6ABE4d3ec88F0Bcef31aB568379bd197);      
    }
...
//      rewardsPool -= amount;
  ...
  function updateMax(uint256 _max) public onlyRole(DEFAULT_ADMIN_ROLE) {
    MAX_STAKES = _max;
  }
  ...
  function changeNFTBoost(uint256[5] memory newAPYBoosts) external onlyRole(DEFAULT_ADMIN_ROLE) {
    NFTStakeAPYBoosts = newAPYBoosts;
  }

  /// @dev Changes apy rates
  /// @param newRates is array of new rates
  function setRates(uint32[5] memory newRates) external onlyRole(DEFAULT_ADMIN_ROLE) {
    rates = newRates;
  }

  function setPeriods() external onlyRole(DEFAULT_ADMIN_ROLE) {
      periods = [30 days, 60 days, 90 days, 180 days, 360 days];
  }

    function setMaxStakes() external onlyRole(DEFAULT_ADMIN_ROLE) {
      MAX_STAKES = 100;
  }
