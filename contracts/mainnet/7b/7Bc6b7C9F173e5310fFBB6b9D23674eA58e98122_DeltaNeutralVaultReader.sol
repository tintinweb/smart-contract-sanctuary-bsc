...
contract DeltaNeutralVaultReader {
  ...
  function getCurrentState(IDeltaNeutralVault _deltaVault)
    external
    view
    returns (VaultStatus memory _stats)
  {
    IDeltaNeutralOracle _oracle = IDeltaNeutralOracle(
      _deltaVault.priceOracle()
    );
    ...
    (_stats.lpTokenPrice, _stats.lastPriceUpdate) = _oracle.lpToDollar(
      1e18,
      _stats.lpToken
    );
