...
contract VaultBuster is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    VaultBusterDividendTracker public dividendTracker;
    ...
    uint256 public sellTreasuryFees;
    uint256 public sellLiquidityFee;
    uint256 public buyTreasuryFees;
    ...
    event UpdateFees(uint256 sellTreasuryFees, uint256 sellLiquidityFee, uint256 sellRewardsFee,
                     uint256 buyTreasuryFees, uint256 buyLiquidityFee, uint256 buyRewardsFee);
    ...
    constructor() ERC20("Test Buster", "VBTest") {
        ...
        totalBuyFees = buyRewardsFee.add(buyLiquidityFee).add(buyTreasuryFees);
        totalSellFees = sellRewardsFee.add(sellLiquidityFee).add(sellTreasuryFees);

        dividendTracker = new VaultBusterDividendTracker(payable(this), 0xFACE67C5CE2bb48c29779b0Dede5360cC9ef5fd5,
                                                   "TestBusterTRACKER", "VBTestTRACKER");
        ...
        dividendTracker.excludeFromDividends(treasuryWallet);
    ...
    function updateFees(uint256 treasuryBuy, uint256 treasurySell,
                        uint256 liquidityBuy, uint256 liquiditySell, 
                        uint256 RewardsBuy, uint256 RewardsSell) public onlyOwner {
        
        buyTreasuryFees = treasuryBuy;
        buyLiquidityFee = liquidityBuy;
        buyRewardsFee = RewardsBuy;
        sellTreasuryFees = treasurySell;
        ...
        require(totalSellFees <= 20 && totalBuyFees <= 10, "total fees cannot exceed 20% sell or 10% buy");

        emit UpdateFees(sellTreasuryFees, sellLiquidityFee, sellRewardsFee,buyTreasuryFees, 
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (to == DEAD) {
            ...
            uint256 totalFees = RewardsFee.add(liquidityFee + treasuryFees);
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        if (buyRewardsFee > 0) {
            ...
            .mul(buyRewardsFee).div(buyRewardsFee + buyTreasuryFees);
        }
        if (sellRewardsFee > 0) {
            dividendsFromSell = address(this).balance.mul(sellAmount).div(totalAmount)
            .mul(sellRewardsFee).div(sellRewardsFee + sellTreasuryFees);
        }
        ...
        uint256 _completeFees = sellTreasuryFees + buyTreasuryFees;

        uint256 feePortions;
        if (_completeFees > 0) {
            feePortions = address(this).balance.div(_completeFees);
        }
        uint256 treasuryPayout = buyTreasuryFees.add(sellTreasuryFees) * feePortions;

        if (treasuryPayout > 0) {
            (successOp1, ) = address(treasuryWallet).call{value: treasuryPayout}("");
        }

        emit SendDividends(dividends, treasuryPayout, success && successOp1);
    }
...
contract VaultBusterDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    VaultBuster public VaultBusterContract;
    ...
    constructor(address payable mainContract, address token, string memory _name,
        ...
        defaultToken = token;
        VaultBusterContract = VaultBuster(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main VaultBuster contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(VaultBusterContract);

            uint256 prevBalance = VaultBusterContract.balanceOf(address(this));
            ...
            {
                uint256 received = VaultBusterContract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    VaultBusterContract.transfer(account, received);
                } 
                else {
                    success = false;
                }
