...
contract PRIDE is Context, IERC20, Ownable {
    ...
    uint public liqFeeBuy = 3;
    uint public liqFeeSell = 3;

    uint public reflectFeeBuy = 2;
    uint public reflectFeeSell = 2;

    uint public marketingFeeBuy = 5;
    uint public marketingFeeSell = 5;

    uint256 private totalFees = _taxFee.add(_liquidityFee).add(_marketingFee);

    address public deadAddress = 0x000000000000000000000000000000000000dEaD;

    address public marketingWallet = 0xA795f3B8f5Bf1e852bdDA1fE90C2B63D44BaAB44;
    ...
    uint256 public swapTokensAtAmountMax = 100000000 * 10**9;

    // Anti Whale
    bool public enableAntiwhale = true;
    uint256 private maxTransferAmountRate = 20;

    // Bot Settings
    bool public antibotSystemEnable = true;
    uint256 public _botIncreaseFee = 3;
    uint256 private timeDetectBotMinutes = 2;
    uint256 public timeAntiBot = 60 * timeDetectBotMinutes;
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 totalTaxForSwap = _marketingFee.add(_liquidityFee);
    ...
    function swapAndSendToFee(uint256 tokens) private  {
        ...
        address(marketingWallet).call{value: newBalance}("");
    }
    ...
    function setMaxTransferAmountRateDenominator1000(uint256 _val) public onlyOwner {
        require(_val > 0, "Max transaction amount cannot be lower than 0.1% of total supply");
    ...
    function setSellFee(uint market, uint liq, uint reflect) public onlyOwner {
        marketingFeeSell = market;
        liqFeeSell = liq;
        reflectFeeSell = reflect; 
        require(marketingFeeSell + liqFeeSell + reflectFeeSell <= 25, "Max 25%");
    }
    
    function setBuyFee(uint market, uint liq, uint reflect) public onlyOwner {
        marketingFeeBuy = market;
        liqFeeBuy = liq;
        reflectFeeBuy = reflect;
        require(marketingFeeBuy + liqFeeBuy + reflectFeeBuy <= 25, "Max 25%");
    }
