...
contract bigsea is ERC20, Ownable {
    ...
    uint256 private _total = 10_0000_0000 * (10**18);
    ...
    address public _marketWallet = 0xbdAd8153057CD53772Bf25B2A22089529fDF5172;
    address public _wallet2 = 0xbBce65861b573EE5E897fBe04f50A6994DF2A111;
    ...
    AutoSwap public _buyHolder;
    AutoSwap public _sellHolder;
    ...
    constructor() ERC20("big sea", "big sea") {
        if (block.chainid == 56) {
        ...
        _buyHolder = new AutoSwap(address(this));
        _sellHolder = new AutoSwap(address(this));

        automatedMarketMakerPairs[uniswapV2Pair] = true;
        
        excludeFromFees(address(_autoSwap), true);
        excludeFromFees(address(_buyHolder), true);
        excludeFromFees(address(_sellHolder), true);
    ...
    function setWallet2 (address wallet) external onlyOwner {
        _wallet2 = wallet;
    }
    ...
    function setMaxSwap(uint256 value) external onlyManager {
        _maxSwap = value;
    }

    function setTokenRewardsFee(uint256 value) external onlyManager {
        buyFee = value;
    }

    function setTokenRewardsSellFee(uint256 value) external onlyManager {
        sellFee = value;
    }
    ...
    function blacklistAddresss(address[] memory accounts, bool value) external onlyManager {
        for (uint256 i = 0; i < accounts.length; i++) {
    ...
    function _transfer(
        ...
        uint256 contractTokenBalance = balanceOf(address(this)).add(balanceOf(address(_sellHolder))).add(balanceOf(address(_buyHolder)));
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[from]) {
                ...
                if (buyFee > 0) {
                    ...
                    super._transfer(from, address(_buyHolder), rewardTokens);
                }
            } else {
                if (sellFee > 0) {
                    uint256 rewardTokens = amount.mul(sellFee).div(10000);
                    tokens = tokens.sub(rewardTokens);
                    super._transfer(from, address(_sellHolder), rewardTokens);
                }
            }
            super._transfer(from, to, tokens);
    ...
    function _swap() private lockSwap {
        ...
        if (!swapCheckPair) {
            super._transfer(address(_buyHolder), address(this), balanceOf(address(_buyHolder)));
            super._transfer(address(_sellHolder), address(this), balanceOf(address(_sellHolder)));
            swapTokensForTokenB(balanceOf(address(this)), _marketWallet);
            return;
        }

        uint256 _w2 = balanceOf(address(_buyHolder)).div(2).add(balanceOf(address(_sellHolder)).div(5));
        uint256 _w1 = balanceOf(address(_buyHolder)).add(balanceOf(address(_sellHolder))).sub(_w2);


        super._transfer(address(_buyHolder), address(this), balanceOf(address(_buyHolder)));
        super._transfer(address(_sellHolder), address(this), balanceOf(address(_sellHolder)));

        uint256 swapTokens = _w2.add(_w1);

        if (_maxSwap == 0) {
            if (swapTokens > balanceOf(uniswapV2Pair).div(1000)) {
                swapTokens = balanceOf(uniswapV2Pair).div(1000);
            }
        }
        ...
        uint256 _w1b = raleTokenB.mul(_w1).div(swapTokens);
        
        IERC20(tokenB).transfer(_marketWallet, _w1b);
        
        IERC20(tokenB).transfer(_wallet2, IERC20(tokenB).balanceOf(address(this)));
    }
