...
contract TheTransplantCoinv3 is ITheTransplanCoinv3, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    modifier lockUpgrade() {
        require(!_upgraded, "The Transplant Coin v3: Already upgraded");
        _;
        _upgraded = true;
    }
    modifier checkTierIndex(uint256 _index) {
        require(feeTiers.length > _index, "The Transplant Coin v3: Invalid tier index");
        _;
    }
    ...
    function initialize(address _router) public initializer {
        __Context_init_unchained();
        __Ownable_init_unchained();
        __TheTransplantCoinv3_init_unchained(_router);
    }
    function __TheTransplantCoinv3_init_unchained(address _router) internal initializer {
        _name = "The Transplant Coin v3";
        _symbol = "TPC";
        _decimals = 9;
        _tTotal = 2000000000000 * 10**9;
        _rTotal = (MAX - (MAX % _tTotal));
        _maxFee = 1000;
        // swapAndLiquifyEnabled = true;
        _maxTxAmount = 5000 * 10**6 * 10**9;
        numTokensSellToAddToLiquidity = 500000000000 * 10**9;
        ...
        __TheTransplantCoinv3_tiers_init();
        emit Transfer(address(0), _msgSender(), _tTotal);
    }
    function __TheTransplantCoinv3_tiers_init() internal initializer {
    ...
    function whitelistAddress(address _account, uint256 _tierIndex)
        ...
        preventBlacklisted(_account, "The Transplant Coin v3: Selected account is in blacklist")
    {
        require(_account != address(0), "The Transplant Coin v3: Invalid address");
        _accountsTier[_account] = _tierIndex;
    }

    function excludeWhitelistedAddress(address _account) public onlyOwner {
        require(_account != address(0), "The Transplant Coin v3: Invalid address");
        require(_accountsTier[_account] > 0, "The Transplant Coin v3: Account is not in whitelist");
    ...
    function checkFees(FeeTier memory _tier) internal view returns (FeeTier memory) {
        ...
        require(_fees <= _maxFee, "The Transplant Coin v3: Fees exceeded max limitation");
    ...
    function setEcoSystemFeeAddress(uint256 _tierIndex, address _ecoSystem)
        ...
        checkTierIndex(_tierIndex)
    {
        require(_ecoSystem != address(0), "The Transplant Coin v3: Address Zero is not allowed");
    ...
    function setOwnerFeeAddress(uint256 _tierIndex, address _owner) external onlyOwner checkTierIndex(_tierIndex) {
        require(_owner != address(0), "The Transplant Coin v3: Address Zero is not allowed");
    ...
    function _approve(
        ...
        private
        preventBlacklisted(owner, "The Transplant Coin v3: Owner address is blacklisted")
        preventBlacklisted(spender, "The Transplant Coin v3: Spender address is blacklisted")
    {
    ...
    function _transfer(
        ...
        private
        preventBlacklisted(_msgSender(), "The Transplant Coin v3: Address is blacklisted")
        preventBlacklisted(from, "The Transplant Coin v3: From address is blacklisted")
        preventBlacklisted(to, "The Transplant Coin v3: To address is blacklisted")
        isRouter(_msgSender())
    {
    ...
    function swapAndEvolve() public onlyOwner lockTheSwap {
        ...
        uint256 initialBalance = ITheTransplanCoinv3(address(this)).balanceOf(msg.sender);
        // swap BNB for Tokens
        swapBnbForTokens(half);

        // how much BNB did we just swap into?
        uint256 newBalance = ITheTransplanCoinv3(address(this)).balanceOf(msg.sender);
        uint256 swapeedToken = newBalance.sub(initialBalance);

        _approve(msg.sender, address(this), swapeedToken);
        ITheTransplanCoinv3(address(this)).transferFrom(msg.sender, address(this), swapeedToken);
    ...
    function migrate(address account, uint256 amount)
        external
        override
        preventBlacklisted(account, "The Transplant Coin v3: Migrated account is blacklisted")
    {
        require(migration != address(0), "The Transplant Coin v3: Migration is not started");
        require(_msgSender() == migration, "The Transplant Coin v3: Not Allowed");
    ...
    function withdrawToken(address _token, uint256 _amount) public onlyOwner {
        ITheTransplanCoinv3(_token).transfer(msg.sender, _amount);
    }
