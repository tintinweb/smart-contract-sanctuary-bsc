...
contract DoughnutFinance {
    ...
    function getMyDoughnuts(address adr) public view returns(uint256){
        return claimedDoughnuts[adr].add(getDoughnutsSinceLastHatch(adr));
    }
    ...
    function rebakeDoughnut(address ref) public onlyInitialized {
        ...
        uint256 doughnutsUsed=getMyDoughnuts(msg.sender);
    ...
    function eatDoughnut() public onlyInitialized {
        uint256 hasDoughnuts=getMyDoughnuts(msg.sender);
    ...
    function doughRewards(address adr) public view returns(uint256) {
        uint256 hasDough = getMyDoughnuts(adr);
        uint256 doughValue = calculateDoughnutSell(hasDough);
        return doughValue;
    }
}