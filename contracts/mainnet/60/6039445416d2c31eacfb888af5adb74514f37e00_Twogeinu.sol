...
contract DividendDistributor is IDividendDistributor {
    ...
    IERC20Extended public SQUID =
        IERC20Extended(0xBB5A9067A1419b994EC917878B3275be77BEF016);
    ...
    uint256 public minDistribution = 1 * (10**SQUID.decimals());
    ...
    function deposit() external payable override onlyToken {
        uint256 balanceBefore = SQUID.balanceOf(address(this));

        address[] memory path = new address[](2);
        path[0] = router.WETH();
        path[1] = address(SQUID);

        router.swapExactETHForTokensSupportingFeeOnTransferTokens{
            value: msg.value
        }(0, path, address(this), block.timestamp);

        uint256 amount = SQUID.balanceOf(address(this)).sub(balanceBefore);
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if (amount > 0) {
            ...
            SQUID.transfer(shareholder, amount);
...
// main contract
contract Twogeinu is IERC20Extended, Auth {
    using SafeMath for uint256;

    string private constant _name = "TwogeInu2.0";
    string private constant _symbol = "$Twoge2";
    uint8 private constant _decimals = 9;
    uint256 private constant _totalSupply = 1_000_000_000_000 * 10**_decimals;

    address public SQUID = 0xBB5A9067A1419b994EC917878B3275be77BEF016;
