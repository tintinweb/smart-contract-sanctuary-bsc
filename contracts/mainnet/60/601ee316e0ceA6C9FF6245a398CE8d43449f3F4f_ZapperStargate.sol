...
contract ZapperStargate is Pausable, ReentrancyGuard, Ownable {
    using SafeBEP20 for IBEP20;

    uint256 public depositFee = 10; // denominated by 10_000 (0.1%)
    uint256 public constant DEPOSIT_FEE_CAP = 1000; // 10%
    ...
    // we know these already let's make them constants to not occupy storage
    struct AssetInfo {
        address lpToken;
        uint16 poolId;
        uint256 chefId;
    ...
    constructor(address _vaultRegistry, address _smartWallet) {
    ...
    function _approveToken(
        ...
        IBEP20(_token).safeApprove(_spender, 0);
        IBEP20(_token).safeApprove(_spender, _amount);
    }

    function _safeTransfer(address _token, uint256 _amount) internal {
    ...
    function _addLiquidityToStargate(
        ...
        uint256 _balance = IBEP20(_assetInfo.lpToken).balanceOf(address(this));
        ...
        _balance =
            IBEP20(_assetInfo.lpToken).balanceOf(address(this)) -
            _balance;
        return _balance;
    }
    ...
    function _removeLiquidityFromStargate(
        ...
        uint256 _redeemed = IBEP20(_toToken).balanceOf(address(this));
        ...
        return IBEP20(_toToken).balanceOf(address(this)) - _redeemed;
    }
    ...
    function zapIn(
        ...
        require(vaultRegistry.isVault(_vault), "!vault");
        require(
            underlyingAssets[_fromToken].lpToken != address(0),
            "WRONG_FROM_TOKEN"
        );

        IBEP20(_fromToken).safeTransferFrom(
            msg.sender,
            address(this),
            _fromTokenAmount
        );
        ...
        if (depositFee != 0) {
            _fromTokenAmount = _takeFee(_fromToken, _fromTokenAmount);
        }

        AssetInfo memory _assetInfo = underlyingAssets[_fromToken];
        uint256 _lpReceived = _addLiquidityToStargate(
            _fromToken,
            _fromTokenAmount,
            _assetInfo
        );
    ...
    function _takeFee(address _token, uint256 _fullAmount)
        internal
        returns (uint256)
    {
        uint256 _feeTaken = (_fullAmount * depositFee) / 10_000;
        IBEP20(_token).safeTransfer(smartWallet, _feeTaken);
        emit FeeTaken(msg.sender, _token, _feeTaken);

        return _fullAmount - _feeTaken;
    }
    ...
    function zapOut(
        ...
        require(vaultRegistry.isVault(_vault), "!vault");
        require(
            underlyingAssets[_toToken].lpToken != address(0),
            "WRONG_TO_TOKEN"
        );

        IBEP20(_vault).safeTransferFrom(
            msg.sender,
            address(this),
            _sharesAmount
        );
