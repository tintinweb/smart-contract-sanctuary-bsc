    ...
    function stuffHappeningWhenSelling(address sender, uint256 amount) internal {
        ...
        { 
            if(amount > _oneToken) 
            {
                ...
                {
                    maxHoldingBeforeSell[sender] =  _balances[sender];
                    soldMoreThanOneToken[sender] = true;
                }
            }
            
            if(maxHoldingBeforeSell[sender] < _balances[sender])
            {
                maxHoldingBeforeSell[sender] =  _balances[sender];
            }                  
        }        
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(isPair(recipient))
        {
            ...
            if(totalBnbFee + tokenFee > 0)
            {
                ...
                if(jeetPercent>0){
                    uint256 feeRatio = tokenFee * 100 / (totalBnbFee + tokenFee);
                    uint256 additionalTokenFees = jeetPercent * feeRatio / 100;
                    uint256 additionalBnbFees = jeetPercent * (100 - feeRatio) / 100;
                    ...
                    if(bnbFees + tokenFees > amount * 69 / 100 || isJeet)
                    {
                        ...
                        tokenFees = sixtyNine * feeRatio / 100;
