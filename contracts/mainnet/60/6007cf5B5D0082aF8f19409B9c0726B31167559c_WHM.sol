...
contract WHM is Context, IERC20, Ownable {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if (!startFlag) {
            ...
            require(sender == _owner || recipient != uniswapPair,'no start!');   
        }
        
        if(launchedAt == 0 && isMarketPair[recipient] && sender == _owner){
            launchedAt = block.number;
        }

        if(isMarketPair[sender] && !isMarketPair[recipient] && block.number <= launchedAt + 3) {
            require(recipient == uniswapPair , "Buys not allowed for first 3 blocks");
        }
