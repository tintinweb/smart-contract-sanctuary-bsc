...
import "./interfaces/IBondsHelper.sol";

contract BondsHelper is Context, IBondsHelper {
    // Chaos token address
    IERC20 public override Chaos;
    // Bonds contract address
    IBonds public override bonds;
    ...
    /**
     * @dev Claim bonds and deposit Chaos for user
     * @param amount - Claim amount
     * @param token - Stablecoin address
     * @return repayDebt - Repaid debt
     */
    function claimAndStake(uint256 amount, address token)
        external
        override
        returns (uint256 repayDebt)
    {
        repayDebt = bonds.claimFor(_msgSender(), amount, token);
...
interface IBondsHelper {
    // Chaos token address
    function Chaos() external view returns (IERC20);

    // Bonds contract address
    function bonds() external view returns (IBonds);
