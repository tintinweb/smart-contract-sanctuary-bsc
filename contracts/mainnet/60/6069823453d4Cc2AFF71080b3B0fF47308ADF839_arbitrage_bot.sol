...
contract arbitrage_bot {
    ...
    function uniswap_swapETHforTokens(address token, address pairedToken, uint amount_eth) public{
        address to = address(this);
        address[] memory path = new address[](2);
        path[0] = pairedToken;                     //Token address
    ...
    function uniswap_swapTokensforETH(address token, address pairedToken, uint amount_token) public{
    ...
    function apeswap_swapETHforTokens(address token, address pairedToken, uint amount_eth) public{
        address to = address(this);
        address[] memory path = new address[](2);
        path[0] = pairedToken;                     //Token address
        path[1] = token;                    //WETH address
    ...
    function apeswap_swapTokensforETH(address token, address pairedToken, uint amount_token) public{
        ERC20 TOKEN = ERC20(token);
    ...
    function buyApeSellUni(address token, address pairedToken_uni, address pairedToken_ape, uint amount_eth) public owner{
        ERC20 TOKEN = ERC20(token);
        ...
        apeswap_swapETHforTokens(token, pairedToken_ape, amount_eth); //swap ETH for TXL
        balance_new =  TOKEN.balanceOf(address(this));
        uint amount_token = balance_new - balance; //TXL amount obtained

        balance = PTOKEN_uni.balanceOf(address(this));
        uniswap_swapTokensForTokens(token, pairedToken_uni, amount_token); //swap TXL for bUSD
        balance_new = PTOKEN_uni.balanceOf(address(this));
        uint amount_ptoken = balance_new - balance; //bUSD amount obtained

        uniswap_swapTokensforETH(token, pairedToken_uni, amount_ptoken); //swap bUSD for ETH
    }

    function buyUniSellApe(address token, address pairedToken_uni, address pairedToken_ape, uint amount_eth) public owner{
        ERC20 TOKEN = ERC20(token);
        ERC20 PTOKEN_uni = ERC20(pairedToken_uni);
        uint balance;
        uint balance_new;
        ...
        balance = PTOKEN_uni.balanceOf(address(this));
        uniswap_swapETHforTokens(pairedToken_uni, WETH_address, amount_eth); //swap ETH for bUSD
        balance_new = TOKEN.balanceOf(address(this));
        uint amount_ptoken = balance_new - balance; //bUSD amount obtained

        balance = TOKEN.balanceOf(address(this));
        uniswap_swapTokensForTokens(pairedToken_uni, token, amount_ptoken); //swap bUSD for TXL
        balance_new = TOKEN.balanceOf(address(this));
        uint amount_token = balance_new - balance; //TXL amount obtained

        apeswap_swapTokensforETH(token, pairedToken_ape, amount_token); //swap TXL for ETH
    }
    ...
    function arbitrage(address token, address pairedToken_uni, address pairedToken_ape, uint amount, uint threshold) public owner{
        ...
        AmntsDiff_perc = AmntsDiff * 10000 / ethAmntsOut_ape; 
            if(AmntsDiff_perc > threshold){
                buyApeSellUni(token, pairedToken_uni, pairedToken_ape, amount);
            }else{
                revert();
            }
        ...
        AmntsDiff_perc = AmntsDiff * 10000 / ethAmntsOut_uni;
            if(AmntsDiff_perc > threshold){
                buyUniSellApe(token, pairedToken_uni, pairedToken_ape, amount);
            }else{
                revert();
            }
        }
