...
contract contractToken is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "ApeMan";
    string constant _symbol = "ApeMan";
    ...
    uint256 public minReflection = _totalSupply / 10000;
    ...
    uint256 autoBuybackAmount = 0.5 * 10 ** 18;
    uint256 autoBuybackBlockPeriod = 1 hours;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(recipient != marketingFeeReceiver && recipient != DEAD){
        amountReceived = shouldTakeFee(sender) ? takeFee(sender, recipient, amount) : amount;}
        _balances[recipient] = _balances[recipient].add(amountReceived);

        uint256 amountsender = _balances[sender] >= minReflection ? _balances[sender]:0;
        uint256 amountrecipient = _balances[recipient] >= minReflection ? _balances[recipient]:0;

        if(!isDividendExempt[sender]){ try distributor.setShare(sender, amountsender ) {} catch {} }
        if(!isDividendExempt[recipient]){ try distributor.setShare(recipient, amountrecipient ) {} catch {} }

        if(sender != marketingFeeReceiver && recipient != marketingFeeReceiver && recipient != DEAD ){ try distributor.process(distributorGas) {} catch {}}
