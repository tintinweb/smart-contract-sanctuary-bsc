...
//    IToken public TOKEN;
    ...
    constructor(address _factory, address _systemCollection){
        FACTORY = IFactory(_factory);
        FEE_COLLECTOR = owner();
        supportedCollections[_systemCollection] = _systemCollection;
    }
    ...
    function buyAtFixedPrice(address _collection, uint256 _tokenId) public override payable {
        require(onSell[_collection][_tokenId][FIXED].startPrice != 0, "tokenId must be on sell");
        require(onSell[_collection][_tokenId][FIXED].startPrice == msg.value , "insufficient tokens to buy this NFT");
        ...
        delete onSell[_collection][_tokenId][FIXED]; // deleting upfront to avoid re-entrancy issues
//        TOKEN.transferFrom(msg.sender, FEE_COLLECTOR, fee); // collecting fee (TODO: collecting fee from seller amount not from buyer)
        payable(FEE_COLLECTOR).transfer(fee); // collecting fee in native currency
        // Collecting ROYALTY
        // ROYALTY if supported by collection
        if( IERC165(_collection).supportsInterface(type(IERC2981).interfaceId)){
            // distributing royalty
            (address receiver, uint256 royaltyAmount) = INFT(_collection).royaltyInfo(_tokenId, price);
//            TOKEN.transfer(receiver, royaltyAmount); // royalty transferred
            price -= royaltyAmount; // deducting royalty
            payable(receiver).transfer(royaltyAmount); // royalty transferred in native currency
        }
            price -= fee; // deducting fee
//            TOKEN.transferFrom(msg.sender, seller, price); // paying to seller
    ...
    function placeBid(address _collection, uint256 _tokenId) public override payable {
...
//        TOKEN.transferFrom(msg.sender, address(this), _bidAmount); // escrowing bid amount
        payable(address(this)).transfer(_bidAmount); // escrowing bid amount in native currency
    ...
    function claimAuction(address _collection, uint256 _tokenId) public override {
        ...
        // deleting upfront to avoid double processing / reentrancy
...
//        TOKEN.transfer(FEE_COLLECTOR, fee); // transferring fee
        ...
        if( IERC165(_collection).supportsInterface(type(IERC2981).interfaceId)){
            // distributing royalty
            (address receiver, uint256 royaltyAmount) = INFT(_collection).royaltyInfo(_tokenId, amount);
//            TOKEN.transfer(receiver, royaltyAmount); // royalty transferred
            amount -= royaltyAmount;
            payable(receiver).transfer(royaltyAmount); // royalty transferred in native currency
        }
        amount -= fee;
//        TOKEN.transfer(seller, amount); // paying to seller
        payable(seller).transfer(amount); // paying to seller in native currency
    ...
    // withdrawing exrowed funds should only be called at emergency other than this these funds should be automatically used to pay to seller & fee
    function withdraw(address to, uint amount ) onlyOwner external {
...
interface IMarketplace {
    ...
    function buyAtFixedPrice(address _collection, uint256 _tokenId) payable external;
    function placeBid(address _collection, uint256 _tokenId) payable external;
...
interface IFactory {
    function createCollection(string memory _name, string memory _symbol, string memory _baseURI, bool _royalty) external returns (address);
