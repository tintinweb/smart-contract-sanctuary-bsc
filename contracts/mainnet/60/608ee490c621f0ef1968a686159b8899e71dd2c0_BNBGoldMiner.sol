...
contract BNBGoldMiner is Ownable , ReentrancyGuard {
    ...
    constructor(address payable _dev, address payable _marketing) {
        dev = _dev;
    ...
    function withdrawal() public noReentrant {
        require(init, "Not Started Yet");
        require(weekly[msg.sender].deadline <= block.timestamp, "You can't withdraw");
    ...
    function addFomoJackReward(address winnerAddr, uint256 amount) public onlyOwner {
        uint256 addedAmount = amount.add(approvedWithdrawal[winnerAddr].amount);
        approvedWithdrawal[winnerAddr] = userWithdrawal(winnerAddr,addedAmount);
    }
    ...
    function feeTransfer(uint256 _amount) private {
        uint256 each = _amount.div(2);
        dev.transfer(each);
        marketing.transfer(each);
    }
    ...
    function setFeeAddress(address payable _dev, address payable _marketing) public onlyOwner {
        dev = _dev;
        marketing = _marketing;
    }
