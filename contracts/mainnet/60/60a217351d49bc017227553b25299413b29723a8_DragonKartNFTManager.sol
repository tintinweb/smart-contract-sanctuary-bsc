...
contract DragonKartNFTManager is AccessControlEnumerable, ItemFactoryManager {
    using EnumerableSet for EnumerableSet.UintSet;
    using Counters for Counters.Counter;
    ...
    event TokenMetaDataUpdated(uint256 indexed tokenId, uint256 artifacts);

    constructor(address nftCore_, address itemFactory_)
        ItemFactoryManager(itemFactory_)
    {
        ...
        _supportedBoxTypes.add(5); // material NFTs
        _supportedBoxTypes.add(6);
        _supportedBoxTypes.add(7);
        _supportedBoxTypes.add(11); // characters (includes TET)
        _supportedBoxTypes.add(12); // cars (includes TET)
        _supportedBoxTypes.add(14); // characters, cars, weapons (includes TET)
        ...
        _addComboType(103); // Combo characters, cars, weapons (includes TET)
        _addBoxTypeToCombo(103, 11); // characters (includes TET)
        _addBoxTypeToCombo(103, 12); // cars (includes TET)
        _addBoxTypeToCombo(103, 3); // weapons
    ...
    modifier onlyTokenOwner(uint256 tokenId_, address owner_) {
        require(
            nftCore.ownerOf(tokenId_) == owner_,
    ...
    modifier onlyMysteryBox(uint256 tokenId_) {
        ...
        require(_itemId == 0, "DragonKartNFT: token is already unboxed");
        _;
    }
    ...
    function _updateTokenMetaData(uint256 tokenId_, uint256 artifacts_)
        private
    {
        nftCore.updateTokenMetaData(tokenId_, artifacts_);
        emit TokenMetaDataUpdated(tokenId_, artifacts_);
    }
    ...
    function _updateTokenMetaData(
        ...
        nftCore.updateTokenMetaData(tokenId_, artifacts_, dna_);
        emit TokenMetaDataUpdated(tokenId_, artifacts_);
    }
    ...
    function _mint(address to_, uint256 boxType_) private returns (uint256) {
        ...
        _artifacts = _artifacts.setBits(0, 8, boxType_);

        // Add item level: size 1 byte
        _artifacts = _artifacts.setBits(8, 8, uint256(1));

        _updateTokenMetaData(_tokenId, _artifacts);
    ...
    function _mintSpecificToken(
        ...
        // Add item level: size 1 byte
        _artifacts = _artifacts.setBits(8, 8, level_);

        // mint specific item, useful with material, campaign NFTs
        if (itemType_ > 0 && itemId_ > 0) {
            // add item type: size 1 byte
            _artifacts = _artifacts.setBits(16, 8, itemType_);

            // add item id: size 2 byte
            _artifacts = _artifacts.setBits(24, 16, itemId_);

            uint256 _artifactsLength = itemFactory.artifactsLength(itemType_);

            for (uint256 _index = 0; _index < _artifactsLength; _index++) {
                // add artifact id
                uint256 _artifactId = itemFactory.artifactIdAt(
                    itemType_,
                    _index
                );
                _artifacts = _artifacts.setBits(
                    40 + _index * 24,
                    8,
                    _artifactId
                );
                ...
                uint256 _randomness = uint256(
                    keccak256(
                        abi.encodePacked(
                            blockhash(block.number - (_index + 1)),
                            itemType_,
                            _artifactId,
                            _index
                        )
    ...
    function mint(address to_, uint256 boxType_)
        ...
        returns (uint256)
    {
        return _mint(to_, boxType_);
    }

    /**
     * @dev Mint specific token, useful in Material NFTs use case
     * @param to_ recipent address
     * @param boxType_ box type id
     * @param level_ level
     * @param itemType_ item type id
     * @param itemId_ item id
     * Waring: make sure itemId_ belongs to itemType_
     */
    function mintSpecificToken(
        ...
        returns (uint256)
    {
        require(level_ > 0, "DragonKartNFT: level_ is 0");
        return _mintSpecificToken(to_, boxType_, level_, itemType_, itemId_);
    }
        ...
        returns (uint256[] memory)
    {
        ...
        for (uint256 _index = 0; _index < amount_; _index++) {
            _tokenIds[_index] = _mint(to_, boxType_);
        }
        return _tokenIds;
    }

    /**
     * @dev Mint specific tokens in batch, useful in Material NFTs use case
     * @param to_ recipent address
     * @param boxType_ box type id
     * @param level_ level
     * @param itemType_ item type id
     ...
     * Waring: make sure itemId_ belongs to itemType_
     */
    function mintSpecificTokenBatch(
        address to_,
        uint256 boxType_,
        uint256 level_,
        uint256 itemType_,
        ...
        returns (uint256[] memory)
    {
        ...
        uint256[] memory _tokenIds = new uint256[](amount_);
        for (uint256 _index = 0; _index < amount_; _index++) {
            _tokenIds[_index] = _mintSpecificToken(
                to_,
                boxType_,
                level_,
                itemType_,
    ...
    function unbox(uint256 tokenId_)
        ...
        onlyTokenOwner(tokenId_, _msgSender())
        onlyMysteryBox(tokenId_)
    {
        ...
        if (_boxType > 100) {
            ...
            _boxType = boxTypes[0];
            _artifacts = _artifacts.setBits(0, 8, _boxType);
            _updateTokenMetaData(tokenId_, _artifacts);
            for (uint256 _index = 1; _index < boxTypes.length; _index++) {
                _mint(_msgSender(), boxTypes[_index]);
            }
        } else {
            uint256 _requestId = _requestIdTracker.current();
            uint256 _randomness = uint256(
                keccak256(
                    abi.encodePacked(
            ...
            _artifacts = _artifacts.setBits(16, 8, _itemType); // add itemType
            _artifacts = _artifacts.setBits(24, 16, _itemId); // add itemId
            ...
            _updateTokenMetaData(tokenId_, _artifacts, _dna);
    ...
    function addBoxTypeToCombo(uint256 comboType_, uint256 boxType_)
        external
        onlyRole(MANAGER_ROLE)
    {
    ...
    {
        _updateTokenMetaData(tokenId_, artifacts_);
    }
}