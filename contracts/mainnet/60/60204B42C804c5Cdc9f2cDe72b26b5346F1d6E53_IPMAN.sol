...
contract IPMAN is IERC20, Context, Ownable {
    ...
    string private constant _symbol = "IPMAN";
    string private constant _name = "IPMAN";

    uint256 public Coosha_max_BuyLimit = 10000000 * 10**9;
    uint256 public Coosha_max_SellLimit = 10000000 * 10**9;

    address public Coosha_Aurora_Wallet = 0x44f919e8672d1185Dacacb0090bC81ec0A8B0601;
    address private Coosha_Team_Wallet = 0x44f919e8672d1185Dacacb0090bC81ec0A8B0601;

    uint256 public Coosha_max_WalletLimit = 20000000 * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[Coosha_Aurora_Wallet] = true;
        _isExcludedFromFee[Coosha_Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[Coosha_Aurora_Wallet] = true;
        allowedTransfer[Coosha_Team_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= Coosha_max_BuyLimit, "You are exceeding Coosha_max_BuyLimit");
            require(balanceOf(to) + amount <= Coosha_max_WalletLimit, "You are exceeding Coosha_max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= Coosha_max_SellLimit, "You are exceeding Coosha_max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= Coosha_max_WalletLimit, "You are exceeding Coosha_max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(Coosha_Aurora_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(Coosha_Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function Coosha_Update_MarketingWallet(address newWallet) external onlyOwner{
        Coosha_Aurora_Wallet = newWallet;
    }
    
    function Coosha_Update_TeamWallet(address newWallet) external onlyOwner{
        Coosha_Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        Coosha_max_BuyLimit = maxBuy * 10**decimals();
        Coosha_max_SellLimit = maxSell * 10**decimals();
    }
    
    function Coosha_Max_Walletlimit(uint256 amount) external onlyOwner{
        Coosha_max_WalletLimit = amount * 10**decimals();
    }
