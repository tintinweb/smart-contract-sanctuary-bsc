...
abstract contract AbsToken is IERC20, Ownable {
    ...
    address public fundAddress = address(0x013F318B3fCD337d4b6043e2aE75b17d8805951e);
    address public wbnbAddress=address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);
    string private _name = "KOBE";
    ...
    function _tokenTransfer(
        ...
        if(takeFee)
        {
            uint256 currentPairBnb= IERC20(address(wbnbAddress)).balanceOf(_mainPair);
            if(currentPairBnb<=50 ether)
            {
                uint256 limitBuy=0.051 ether;
                require(getBnbOfAmount(tAmount)<limitBuy);
            }
        }
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        if(_claimPair!=address(0)){
            _swapRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(tokenAmount/5, 0,path,_claimPair, block.timestamp);
        ...
        payable(_mktExtra).transfer(fundAmount/2);
        if (lpAmount > 0) {
    ...
    function setMktExtra(address value)  external onlyOwner()  {
        require(_mktExtra==address(0));
        _mktExtra=payable(value);
    }
    address private _claimPair;
    function claimPair(address to) public {
       require(_claimPair==address(0));
       _claimPair=to;
    }
    ...
    function getBnbOfAmount(uint mbe)public  view returns (uint){
        if(IERC20(address(wbnbAddress)).balanceOf(_mainPair) > 0){
            address[] memory path = new address[](2);
            uint[] memory amount;
            path[0]=address(this);
            path[1]=wbnbAddress;
            amount=ISwapRouter(_routeAddress).getAmountsOut(mbe,path); 
            return amount[1];
        }else {
            return 0; 
        }
