...
contract SpinHolderToken is ERC20 {
    ...
    function balanceOf(address account) public view override returns (uint256) {
        ...
        return farm_balance + pool_balance + spin_balance;
    }

    function getFarmBalance(address account) public view returns (uint256) {
        (uint112 reserve0, uint112 reserve1, uint32 timestamp) = IPancakePair(
            pool
        ).getReserves();

        uint256 lp_balance = ISpinStakable(farm).balanceOf(account);
        uint256 total_lp_supply = IERC20(LP).totalSupply();
