...
contract aptosNewSWAP is Ownable{
    ...
    constructor(IERC20 _usdt, IERC20 _aptos) public {
        usdt = _usdt;
        aptos = _aptos;

        _tTotal = 99999 * 10**18;
        uint256 leaveAmount = 499995 * 10**17;
        _destroyMaxAmount = _tTotal.sub(leaveAmount);
    }

    function swapAptos(uint256 _amount) public{
        // require(blacklist[msg.sender] == false);

        uint256 aptos_am = 0;
        if(_rateType == 1) {
            aptos_am = _amount * (_usdtRate / _wei);
        } else {
            aptos_am = _amount / (_usdtRate / _wei);
        }

        usdt.transferFrom(msg.sender ,address(this) ,_amount);
        aptos.transfer(msg.sender ,aptos_am.div(100).mul(95));
        aptos.transfer(tokenNode ,aptos_am.div(100).mul(4));
        _destroyAptos(aptos_am.div(100).mul(1));
    }
    ...
    function swapUsdt(uint256 _amount) public{
        ...
        uint256 u_amount = 0;
        if(_rateType == 1) {
            u_amount = _amount / (_usdtRate / _wei);    
        } else {
            u_amount = _amount * (_usdtRate / _wei);
        }

        aptos.transferFrom(msg.sender ,address(this) ,_amount);
        usdt.transfer(msg.sender ,u_amount.div(100).mul(95));
        aptos.transfer(tokenNode ,_amount.div(100).mul(4));
        _destroyAptos(_amount.div(100).mul(1));
    }

    function _destroyAptos(uint256 amount) public {
        require(amount > 0, "Destroy amount must be greater than zero");

        uint256 _destroyAmount = aptos.balanceOf(_destroyAddress);
        if(_destroyAmount < _destroyMaxAmount) {
            aptos.transfer(_destroyAddress ,amount);
        }
    }

    function setRate(uint256 usdt_rate, uint8 rate_type) public {
        require(manager[msg.sender] == true);
        _usdtRate = usdt_rate;
        _rateType = rate_type;
        emit eveSetRate(usdt_rate, rate_type);
    }
    ...
    function withdrawal(address recipient, uint256 amount) public onlyOwner {
        aptos.transfer(recipient, amount);
    }
    ...
    function setBlackList(address addr, bool stauts) public onlyOwner{
        blacklist[addr] = stauts;
    }
}