...
contract NFT721 is ERC721, ERC721Enumerable, Pausable, AccessControl {
    using Strings for uint256;
    ...
    event mintLog(uint256 indexed tokenId_, address indexed owner_);
    ...
    function _mintNft(
        ...
        if (tokenId_ >= currentTokenId) {
            currentTokenId += 1;
        }
        emit mintLog(tokenId_, owner_);
    }
    ...
    function burn(uint256 tokenId_) public {
        require(_isApprovedOrOwner(_msgSender(), tokenId_), "ERC721: caller is not token owner nor approved");
        _burn(tokenId_);
    }
