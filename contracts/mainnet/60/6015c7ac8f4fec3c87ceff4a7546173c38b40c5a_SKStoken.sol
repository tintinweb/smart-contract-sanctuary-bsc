...
contract SKStoken is ERC20, Ownable {
//custom
...
//address
    ...
    address public marketingAddress = 0x53245145D9C107d13880b242054A7888ba714c96;
    address public burnAddress = 0x79761B718896772f1a99626eC610E0c1F7108726;
    address public teamAddress = 0x53245145D9C107d13880b242054A7888ba714c96;
    address public liquidityAddress = 0x53245145D9C107d13880b242054A7888ba714c96;
//uint
    uint public buyMarketingFee = 4;
    uint public sellMarketingFee = 4;
    uint public buyLiquidityFee = 0;
    uint public sellLiquidityFee = 0;
    uint public buyBurnFee = 1;
    uint public sellBurnFee = 1;
    uint public buyTeamFee = 4;
    uint public sellTeamFee = 4;
    uint public totalBuyFee = buyMarketingFee + buyLiquidityFee + buyBurnFee + buyTeamFee;
    uint public totalSellFee = sellMarketingFee + sellLiquidityFee + sellBurnFee + sellTeamFee;
...
//events
    ...
    event BurnFeeCollected(uint amount);
    ...
    event FeesChanged(uint256 buyMarketingFee, uint256 buyLiquidityFee, uint256 buyBurnFee,uint256 buyTeamFee,
                      uint256 sellMarketingFee, uint256 sellLiquidityFee,uint256 sellBurnFee,uint256 sellTeamFee);
    event FeesAddressesChanged(address marketing, address liquidity, address team, address burn);
...
// constructor
    constructor() ERC20("SharkCoin", "SKS") {
        uint total_supply = 210_000_000_000 * 10 ** decimals();
        // set gvars
        IPancakeV2Router02 _pancakeV2Router = IPancakeV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        pancakeV2Router = _pancakeV2Router;
        maxSellTxAmount = total_supply / 210; // 1% supply
        maxBuyTxAmount = total_supply / 210; // 1% supply
        //spawn pair
        ...
        excludedFromFees[burnAddress] = true;
...
//set functions
    ...
    function setFeesAddress(address marketing, address liquidity, address team, address burn) external onlyOwner {
        marketingAddress = marketing;
        liquidityAddress = liquidity;
        teamAddress = team;
        burnAddress = burn;
        emit FeesAddressesChanged(marketing,liquidity,team,burn);
    }
    ...
    uint Buyburn, uint Buyteam,uint Sellburn, uint Sellteam) external onlyOwner {
        //buy
        buyMarketingFee = Buymarketing;
        buyLiquidityFee = Buyliquidity;
        buyBurnFee = Buyburn;
        buyTeamFee = Buyteam;
        totalBuyFee = buyMarketingFee + buyLiquidityFee + buyBurnFee + buyTeamFee;
        //sell
        sellMarketingFee = Sellmarketing;
        sellLiquidityFee = Sellliquidity;
        sellBurnFee = Sellburn;
        sellTeamFee = Sellteam;
        totalSellFee = sellMarketingFee + sellLiquidityFee + sellBurnFee + sellTeamFee;
        //checks
        require(totalBuyFee + totalSellFee <= 200,"Total fees cannot be over 20%");
        emit FeesChanged(buyMarketingFee,buyLiquidityFee,buyBurnFee,Buyteam,sellMarketingFee,sellLiquidityFee,sellBurnFee,sellTeamFee);
    }
    ...
    function setSwapAndLiquify(bool _state, uint _intervalSecondsForSwap, uint _minimumTokensBeforeSwap, uint _tokensToSwap) external onlyOwner {
        ...
        require(tokensToSwap <= totalSupply() / 2100,"token to swap limited to 0.1% supply");
    ...
    // normal transaction
        ...
        // tx limits
            ...
            if(address(this).balance > minimumWeiForTokenomics) {
                ...
                uint256 burnTokens = caBalance * sellBurnFee / totalSellFee;
                (bool success1,) = address(burnAddress).call{value: burnTokens}("");
                if(success1) {
                    emit BurnFeeCollected(burnTokens);
                }
