...
contract UnoFarmApeswap is Initializable, ReentrancyGuardUpgradeable {
    ...
    function initialize(address _lpPair, address _assetRouter) external initializer {
        ...
        IBEP20(lpPair).approve(address(MasterApe), type(uint256).max);
        IBEP20(lpPair).approve(address(apeswapRouter), type(uint256).max);
        IBEP20(rewardToken).approve(address(apeswapRouter), type(uint256).max);
        IBEP20(tokenA).approve(address(apeswapRouter), type(uint256).max);
        IBEP20(tokenB).approve(address(apeswapRouter), type(uint256).max);
    }
    ...
    function distribute(
        ...
        (,,reward) = apeswapRouter.addLiquidity(tokenA, tokenB, IBEP20(tokenA).balanceOf(address(this)), IBEP20(tokenB).balanceOf(address(this)), swapInfos[0].amountOutMin, swapInfos[1].amountOutMin, address(this), block.timestamp);
    ...
    function getPid(address _lpPair) internal view returns (uint256 _pid) {
        ...
        for (uint256 i = 0; i < MasterApe.poolLength(); i++) {
            if (MasterApe.poolInfo(i).lpToken == _lpPair) {
                _pid = i;
                poolExists = true;
                break;
            }
