...
// File: contracts/NeoGi Token.sol
...
contract NeoGiToken is ERC20, Ownable {
    ...
    //EVENTS
    event taxSaleChanged(uint256 tax);
    event maxTransferChanged(uint256 amount);
    event liquidityContractChanged(address rewardPool);
    event newLiquidityAddress(address wallet);
    event walletsExcludedMaxTransfer(address wallet);

    constructor() ERC20("NeoGi Token", "NEOGI") {
        _mint(msg.sender, 1000000000 * 10 ** decimals());
        tax = 0;
        maxTransfer = 600000 * 10 ** decimals();
        liquidityContract = owner();
        //PINKSALE ANTIBOT
        pinkAntiBot = IPinkAntiBot(0x8EFDb3b642eb2a20607ffe0A56CFefF6a95Df002);
        pinkAntiBot.setTokenOwner(msg.sender);
    }

    function setSaleTax(uint256 percent) external onlyOwner {
        require(percent <= 3, "Percent can't be > 3");
        tax = percent;
        emit taxSaleChanged(percent);
    }
    ...
    function setLiquidityContract(address wallet) external onlyOwner {
        require(wallet != address(0), "address variable can not be zero address");
        require(wallet != owner(), "Pool Reward can't be the owner");
        liquidityContract = wallet;
        emit liquidityContractChanged(liquidityContract);
    }

    function setLiquidityPool(address wallet) external onlyOwner {
        require(wallet != address(0), "address variable can not be zero address");
        require(!isIncludeFromFee[wallet], "This pair address is included");
        isIncludeFromFee[wallet] = true;
        emit newLiquidityAddress(wallet);
    }
    ...
    function _transfer(
        ...
        if (includeFee) {
            ...
            super._transfer(from, liquidityContract, taxAmount);
            amount -= taxAmount;
