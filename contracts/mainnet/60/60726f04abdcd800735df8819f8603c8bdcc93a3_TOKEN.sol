...
contract TOKEN is Context, IERC20, Ownable {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 sumOfTokenParts = _MarketingFee.add(_liquidityFee);
        //6

        uint256 partsOfTokenBalance = contractTokenBalance.div(sumOfTokenParts);


        uint256 tokenPart = (partsOfTokenBalance.mul(_liquidityFee)).div(2);
        //Tokens for liquidity, half of liquidity fee

        uint256 swapPart = contractTokenBalance.sub(tokenPart);
        //Tokens swapped for BNB liquidity + marketingBNB
