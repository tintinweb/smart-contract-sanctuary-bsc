...
contract StableBUSD is Context, Ownable , ReentrancyGuard {
    using SafeMath for uint256;
    uint256 public constant min = 10 ether;
    uint256 public constant max = 1000000 ether;
    uint256 public roi = 35;
    uint256 public constant fee = 4; //deposit fee
    uint256 public constant withdraw_fee = 2;
    uint256 public constant ref_fee = 15;
    address public dev = 0x63C89d585eaa09d52B5AbfEC63217631E9220B97;
    ...
    function userReward(address _userAddress) public view returns(uint256) {
        ...
        uint256 userWeeklyReturn = WeeklyRoi(userInvestment);

        // invested time

        uint256 claimInvestTime = weekly[_userAddress].startTime; //weekly profit
        uint256 claimInvestEnd = weekly[_userAddress].deadline; //weekly profit

        uint256 totalTime = SafeMath.sub(claimInvestEnd,claimInvestTime);

        uint256 value = SafeMath.div(userWeeklyReturn,totalTime);
        ...
        return totalEarned;
            ...
            return userWeeklyReturn;
        }
    }

    function withdrawal() public noReentrant {
    require(init, "Not Started Yet");    
    require(weekly[msg.sender].deadline <= block.timestamp, "You cant withdraw yet"); //withdraw weekly
    ...
    uint256 totalAmountToWithdraw = SafeMath.sub(aval_withdraw,wFee); // changed from aval_withdraw to aval_withdraw2
    ...
    uint256 totalAmount = SafeMath.add(amount,aval_withdraw); // it will add one of his half to total withdraw
    ...
    function unStake() external noReentrant {
        ...
        uint256 UnstakeValueCore = SafeMath.div(UnstakeValue,2); //50% investment
    ...
    function WeeklyRoi(uint256 _amount) public view returns(uint256) {
