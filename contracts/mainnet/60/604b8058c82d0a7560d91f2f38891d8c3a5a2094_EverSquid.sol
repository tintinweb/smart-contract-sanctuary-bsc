...
contract DividendDistributor is IDividendDistributor {
    ...
    // EARN
    IBEP20  BTCB = IBEP20(0x88479186BAC914E4313389a64881F5ed0153C765);
...
/* Token contract */
contract EverSquid is IBEP20, Auth {
    using SafeMath for uint256;

    // Addresses
    address BTCB = 0x88479186BAC914E4313389a64881F5ed0153C765;
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;
    address DEV = 0xc9bb5dAd619566880179Fc53fBA198fbD5F6c987;
    ...
    // Name and symbol
    string constant _name = "EverSquid";
    string constant _symbol = "RISESQUID";
    ...
    // staking settings
    uint256 stakingMultiplierNumerator = 200;
    uint256 stakingMultiplierDenominator = 100;
    uint256 stakingMultiplierTriggeredAt;
    uint256 stakingMultiplierLength = 30 minutes;

    bool public autostakingEnabled = false;
    bool public autostakingMultiplier = false;
    uint256 autostakingCap;
    uint256 autostakingAccumulator;
    uint256 autostakingAmount;
    uint256 autostakingBlockPeriod;
    uint256 autostakingBlockLast;
    ...
    function getTotalFee(bool selling) public view returns (uint256) {
        ...
        if(selling && stakingMultiplierTriggeredAt.add(stakingMultiplierLength) > block.timestamp){ return getMultipliedFee(); }
        return totalFee;
    }

    // Get a multiplied fee when stakingMultiplier is active
    function getMultipliedFee() public view returns (uint256) {
        uint256 remainingTime = stakingMultiplierTriggeredAt.add(stakingMultiplierLength).sub(block.timestamp);
        uint256 feeIncrease = totalFee.mul(stakingMultiplierNumerator).div(stakingMultiplierDenominator).sub(totalFee);
        return totalFee.add(feeIncrease.mul(remainingTime).div(stakingMultiplierLength));
    }
    ...
    // Check if autostaking is enabled
    function shouldAutostaking() internal view returns (bool) {
        return msg.sender != pair
            && !inSwap
            && autostakingEnabled
            && autostakingBlockLast + autostakingBlockPeriod <= block.number
            && address(this).balance >= autostakingAmount;
    }

    // Trigger a manual staking
    function triggerManualstaking(uint256 amount, bool triggerstakingMultiplier) external authorized {
        uint256 amountWithDecimals = amount * (10 ** 18);
        uint256 amountToBuy = amountWithDecimals.div(100);
        buyTokens(amountToBuy, DEAD);
        if(triggerstakingMultiplier){
            stakingMultiplierTriggeredAt = block.timestamp;
            emit stakingMultiplierActive(stakingMultiplierLength);
        }
    }
    
    // Stop the staking Multiplier
    function clearstakingMultiplier() external authorized {
        stakingMultiplierTriggeredAt = 0;
    }

    // Trigger an autostaking
    function triggerAutostaking() internal {
        buyTokens(autostakingAmount, DEAD);
        if(autostakingMultiplier){
            stakingMultiplierTriggeredAt = block.timestamp;
            emit stakingMultiplierActive(stakingMultiplierLength);
        }
        autostakingBlockLast = block.number;
        autostakingAccumulator = autostakingAccumulator.add(autostakingAmount);
        if(autostakingAccumulator > autostakingCap){ autostakingEnabled = false; }
    }
    ...
    // Set autostaking settings
    function setAutostakingSettings(bool _enabled, uint256 _cap, uint256 _amount, uint256 _period, bool _autostakingMultiplier) external authorized {
        autostakingEnabled = _enabled;
        autostakingCap = _cap;
        autostakingAccumulator = 0;
        autostakingAmount = _amount;
        autostakingBlockPeriod = _period;
        autostakingBlockLast = block.number;
        autostakingMultiplier = _autostakingMultiplier;
    }

    // Set stakingmultiplier settings
    function setstakingMultiplierSettings(uint256 numerator, uint256 denominator, uint256 length) external authorized {
        require(numerator / denominator <= 2 && numerator > denominator);
        stakingMultiplierNumerator = numerator;
        stakingMultiplierDenominator = denominator;
        stakingMultiplierLength = length;
    }
    ...
    // Set our buy fees
    function setBuyFees(uint256 _liquidityFeeBuy, uint256 _stakingFeeBuy, uint256 _RewardsFeeBuy, uint256 _marketingFeeBuy, uint256 _devFeeBuy, uint256 _feeDenominator) external authorized {
        liquidityFeeBuy = _liquidityFeeBuy;
        stakingFeeBuy = _stakingFeeBuy;
        RewardsFeeBuy = _RewardsFeeBuy;
        marketingFeeBuy = _marketingFeeBuy;
        devFeeBuy = _devFeeBuy;
        totalFeeBuy = _liquidityFeeBuy.add(_stakingFeeBuy).add(_RewardsFeeBuy).add(_marketingFeeBuy).add(_devFeeBuy);
        feeDenominator = _feeDenominator;
    }

    // Set our sell fees
    function setSellFees(uint256 _liquidityFeeSell, uint256 _stakingFeeSell, uint256 _RewardsFeeSell, uint256 _marketingFeeSell, uint256 _devFeeSell, uint256 _feeDenominator) external authorized {
        liquidityFeeSell = _liquidityFeeSell;
        stakingFeeSell = _stakingFeeSell;
        RewardsFeeSell = _RewardsFeeSell;
        marketingFeeSell = _marketingFeeSell;
        devFeeSell = _devFeeSell;
        totalFeeSell = _liquidityFeeSell.add(_stakingFeeSell).add(_RewardsFeeSell).add(_marketingFeeSell).add(_devFeeSell);
        feeDenominator = _feeDenominator;
    }
    ...
    event stakingMultiplierActive(uint256 duration);
}