...
contract TestVaToken is ERC20, Ownable {
    ...
    uint256 public transferDevelopmentFee;
    ...
    constructor() ERC20("TestT", "TD") {
        ...
        transferLiquidityFee = 2;
        transferDevelopmentFee = 7;
        transferBurnFee = 0;
        ...
        excludeFromFees_admin(newOwner, true);
        excludeFromFees_admin(address(this), true);
        excludeFromFees_admin(address(0xdead), true);
        excludeFromFees_admin(developmentWallet, true);
    ...
    function addWhiteListNoControl_admin(address addr) external onlyOwner {
        _isWhiteList[addr] = true;
    }

    function createPairTradingFactory_admin() external onlyOwner {
    ...
    function enableContractAddressTrading_admin(address addr, bool value)
        external
        onlyOwner
    {
        require(addr.isContract(), "Only contract address is allowed!");
        _isExcludedFromContractBuyingLimit[addr] = value;
    }

    // Enable Trading
    function enableTrading_admin(bool value) external onlyOwner {
        if (value) {
            tradingActive = true;
            swapEnabled = true;
    ...
    function enableTransferDelay_admin(bool value)
        external
        onlyOwner
        returns (bool)
    {
    ...
    function blackListAddress_admin(address addr)
        external
        onlyOwner
        returns (bool)
    {
    ...
    function setBlackListFee_admin(uint256 _fee)
        external
        onlyOwner
        returns (bool)
    {
    ...
    function setMaxTransactionAmount_admin(uint256 newNum) external onlyOwner {
        maxTransactionAmount = newNum * (10**18);
    }

    function setMaxWalletAmount(uint256 newNum_Token) external onlyOwner {
        maxWallet = newNum_Token * (10**18);
    }
    ...
    function setTransfer_Tax_admin(
    ...
    function excludeFromFees_admin(address account, bool excluded)
        public
        onlyOwner
    {
    ...
    function setAutomatedMarketMakerPair_admin(address pair, bool value)
        public
        onlyOwner
    {
        require(
    ...
    function setMarketingWallet_admin(address newMarketingWallet)
        external
        onlyOwner
    {
    ...
    function setDevelopmentWallet_admin(address newWallet) external onlyOwner {
    ...
    function _transfer(
        ...
        if (takeFee) {
            if (
                ...
                tokensForBurn += (fees * transferBurnFee) / transferTotalFees;
    ...
    function swapBack() private {
        ...
        if (tokensForBurn > 0) {
            BurnTokens = tokensForBurn.mul(contractBalance).div(
                totalTokensToSwap
            );

            _burn(address(0), BurnTokens);
