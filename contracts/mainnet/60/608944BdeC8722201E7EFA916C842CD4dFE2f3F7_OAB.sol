...
contract OAB is Context, IERC20, Ownable {
    ...
    string private _name = "OAB";
    string private _symbol = "OAB";
    ...
    uint256 public minTokenNum = 8 * 10 ** 15;
	
	// fee list end
	address public ownerAddress = address(0x3bA6A6169a88368657C83d673EC799d774D5fc2D);
    
	address public marketAddress = address(0xd74A160648d3a70F14f7CE3F90678D5E5c430adA);
	
	address public lpAddress = address(0xf70b4045177c3F77518BAD3cba17754E104e3084); // to share lp
	
	address public tokenAddress = address(0x288960de77d198fb57e6cbA9688C13c25220372A); // t
	...
	private {
		if (amount > 0) {
            if (type2 == 0){
                (uint256 poolFee, uint256 daoFee) = _getbuyFeeList (amount);
                _backToLp(sender, poolFee);
                _toDao(sender, daoFee);
            } else {
                (uint256 poolFee, uint256 daoFee, uint256 cardFee) = _getbuyFeeList2 (amount);
                _backToLp(sender, poolFee);
                _toDao(sender, daoFee);
    ...
    function _backToCard(address sender, uint amount) private{
        if (amount > 0){
			_rOwned[tokenAddress] = _rOwned[tokenAddress].add(amount);
			emit Transfer(sender, tokenAddress, amount);
		}
	}
	...
	function _getbuyFeeList(uint256 uAmount2) private view returns
        (uint256 poolFee, uint256 daoFee){
		poolFee = uAmount2.div(2);
		daoFee = uAmount2.div(2);
	}

    function _getbuyFeeList2(uint256 uAmount2) private view returns
        (uint256 poolFee, uint256 daoFee, uint256 cardFee){
		uint256 getAmount = uAmount2.div(5);
		poolFee = getAmount;
		daoFee = getAmount;
        cardFee = getAmount.mul(3);
	}
