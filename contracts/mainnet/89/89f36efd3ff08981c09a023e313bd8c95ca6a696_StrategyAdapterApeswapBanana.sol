...
contract AlphaStrategyApeswap is AlphaStrategyBase {
    ...
    function pendingXToken() public view virtual override returns (uint256) {
        ...
        return
            balance +
            stakedToken +
            getPendingCake(pidSecond) +
            getPendingCake(pidLp);
    }
    ...
    function enterBaseToken(uint256 baseTokenBalance)
        ...
        override
    {
        IMasterChef(masterChef).deposit(pidSecond, baseTokenBalance);
    }

    function lpBalance() public view override returns (uint256) {
        uint256 balance;

        (balance, ) = IMasterChef(masterChef).userInfo(pidLp, address(this));

        return balance;
    }

    function withdrawXTokenStaked(uint256 toWithdraw) internal override {
        uint256 balance = IMasterChef(masterChef).pendingCake(
            pidSecond,
            address(this)
        );

        if (balance > 0) {
            IMasterChef(masterChef).deposit(pidSecond, 0);
        }

        uint256 xBalance = IERC20Upgradeable(xBaseToken).balanceOf(
            address(this)
        );
        ...
        if (xBalance < toWithdraw) {
            ...
            IMasterChef(masterChef).leaveStaking(needToUnstake);
        }
    }
}
...
abstract contract AlphaStrategyBase is OwnableUpgradeable {
    ...
    function withdrawXTokenReward(address user) internal onlyVault {
        ...
        if (_xBaseTokenBalance < _pendingXBaseToken) {
            ...
            if (stakedXToken > 0) {
                uint256 needToWithdraw = _pendingXBaseToken -
                    _xBaseTokenBalance;
