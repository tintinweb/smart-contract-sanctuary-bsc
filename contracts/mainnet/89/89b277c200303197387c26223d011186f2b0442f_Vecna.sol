...
contract Vecna is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "Vecna";
    string private _symbol = "Vecna";
    uint8 private _decimals = 18;
     address public  deadAddress = 0x000000000000000000000000000000000000dEaD;
    address payable public marketingWalletAddress = payable(0x8B7667541ec83D37af7Cb9446baf3857E6787D29); 
    address payable public marketingAddress2 = payable(0x8B7667541ec83D37af7Cb9446baf3857E6787D29); 
    address public lpAddress = address(this);
    address public recipientLpAddress = address(0x8B7667541ec83D37af7Cb9446baf3857E6787D29);

    // test :0x8BaBbB98678facC7342735486C851ABD7A0d17Ca  正式 0xdc80909eF17Ae8163B1B616cC5151f6F9680A2c4
    address ethAddress = address(0x44Ac0579BAb9b047240778052F31418566e4f79B);
    //test 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3   正式薄饼地址 0x44Ac0579BAb9b047240778052F31418566e4f79B
    ...
    uint256 private minimumTokensBeforeSwap = 2*10**8 * 1**_decimals; 
    ...
    uint256 public _buyLpFee = 6;
    uint256 public _buyMarketingFee = 3;
    uint256 public _buyTotalFee = _buyDestoryFee.add(_buyBackLpFee).add(_buyLpFee).add(_buyMarketingFee);

    uint256 public _sellDestoryFee = 1;
    uint256 public _sellBackLpFee = 3;
    uint256 public _sellLpFee = 6;
    uint256 public _sellMarketingFee = 3;
    uint256 public _sellTotalFee = _sellDestoryFee.add(_sellBackLpFee).add(_sellLpFee).add(_sellMarketingFee);
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            fromAddress = sender;
            toAddress = recipient;  
           if(IERC20(ethAddress).balanceOf(address(this)) >= minEthVal && LPFeeTime.add(minPeriod) <= block.timestamp) {
                process(distributorGas) ;
                LPFeeTime = block.timestamp;
            }
            return true;
        }
    }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 TokenNum = tAmount.mul(500).div(1000);
        uint256 marketNum1 =  tAmount.mul(166).div(1000);
        uint256 marketNum2 =  tAmount.mul(83).div(1000);
        uint256 letfBackLpToken = tAmount.sub(backLpTokenNum).sub(TokenNum).sub(marketNum1).sub(marketNum2);
        swapTokensForEth(tAmount.sub(letfBackLpToken));
    
        uint256 amountBNB = address(this).balance;
        uint256 markentBNB = amountBNB.mul(2857).div(10000);
        uint256 BNB = amountBNB.mul(5714).div(10000);
        uint256 lpBNB = amountBNB.sub(markentBNB).sub(BNB);
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(isMarketPair[sender]) {//买入 取回
            ...
            uint256 lpNum = amount.mul(_buyLpFee).div(100);
