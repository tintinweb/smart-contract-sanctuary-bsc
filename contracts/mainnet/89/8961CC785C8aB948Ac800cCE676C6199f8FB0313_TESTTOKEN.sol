...
contract TESTTOKEN is IBEP20, Auth {
    ...
    uint256 public _tokensPerTicket = 1000;
    ...
    function tokensPerTicket() external view override returns (uint256) { return _tokensPerTicket; }
    ...
    function myTickets(address spender) external view returns (uint) {
        uint _myTickets = _dailyPrizeEntries[rewardDay][spender];
         return _myTickets; 
    }
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        else
        {
            ...
            if (block.timestamp >= _rewardDateTime)
            {
                ...
                uint index = randModulus(prizeEntries.length);     
                uint amountWon = _prizePot.div(2);       
                _prizePot = _prizePot.sub(amountWon); 
                emit Transfer(prizeEntries[index], address(this), amountWon);
                // clear entries
                delete prizeEntries;
                // start again with charity wallet and this user
                prizeEntries.push(charityWallet);

            }

            uint256 entries = amount / _tokensPerTicket;
            _dailyPrizeEntries[rewardDay][sender] = _dailyPrizeEntries[rewardDay][sender].add(entries);
