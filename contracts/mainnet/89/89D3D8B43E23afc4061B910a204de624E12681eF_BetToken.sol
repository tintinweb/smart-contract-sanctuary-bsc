...
contract BetToken is Authorized, ERC20 {
  address constant DEAD = 0x000000000000000000000000000000000000dEaD;
  address constant ZERO = 0x0000000000000000000000000000000000000000;
  address constant BUSD = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
  address constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;

  string constant _name = "BetToken";
  ...
  uint256 public _maxTxAmount = maxSupply / 1000;
  uint256 public _maxAccountAmount = maxSupply / 100;
  
  uint256 public totalBurned;

  // Fees
  uint256 public feeAdministrationWallet = 500; // 5%
  uint256 public feeDevelopmentWallet = 500; // 5%
  ...
  // Fee Setup
  function setFeeAdministrationWallet(uint256 feeAmount) external isAuthorized(1) 
  { 
    require(feeAmount <= 500, "A fee can only be smaller than 5%");
    feeAdministrationWallet = feeAmount; 
  }
  function setFeeDevelopmentWallet(uint256 feeAmount) external isAuthorized(1) 
  { 
    require(feeAmount <= 500, "A fee can only be smaller than 5%");
    feeDevelopmentWallet = feeAmount; 
  }
  function setFeePool(uint256 feeAmount) external isAuthorized(1) 
  { 
    require(feeAmount <= 0, "A fee can only be smaller than 0%");
    feePool = feeAmount; 
  }

  function setMaxTxAmountWithDecimals(uint256 decimalAmount) public isAuthorized(1) 
  { 
    require(_maxTxAmount >= maxSupply / 1000, "Amount must be bigger then 0.1% tokens"); // You must use decimals to work
    _maxTxAmount = decimalAmount; 
  }

  function setMaxAccountAmountWithDecimals(uint256 decimalAmount) public isAuthorized(1) 
  { 
    require(_maxAccountAmount >= maxSupply / 100, "Amount must be bigger then 1% tokens"); // You must use decimals to work
  ...
  constructor()ERC20(_name, _symbol) {
    ...
    administrationWallet = 0x37E7e242bD0f9020F881757A52956391BFb66A9F;
    developingWallet = 0xda921C7C80EcC8aD77C3656706fE3be0475AF9BD;
