...
contract SUCKIT is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    SUCKITNUDividendTracker public dividendTracker;
    ...
    constructor() ERC20("SUCKIT", "SUCKIT") {
        ...
        totalSellFees = sellRewardsFee
         ...
         dividendTracker = new SUCKITNUDividendTracker(
            payable(this),
            router,
...
contract SUCKITNUDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        SUCKITContract = SUCKIT(mainContract);
    ...
    function _transfer(
        ...
        require(false, "SUCKIT_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "SUCKIT_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SUCKIT contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(SUCKITContract);

            uint256 prevBalance = SUCKITContract.balanceOf(address(this));
            ...
            {
                uint256 received = SUCKITContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    SUCKITContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
