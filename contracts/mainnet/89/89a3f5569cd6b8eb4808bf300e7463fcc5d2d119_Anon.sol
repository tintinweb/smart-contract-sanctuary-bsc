...
contract Anon is Context, IERC20, Ownable {
    ...
    string private _symbol = "WeDontJeet";
    ...
    function swapAndLiquify(uint256 tokenBalance) private lockTheSwap {
        ...
        uint256 liquidityTokens = tokenBalance/3;
        if ((tokenBalance-liquidityTokens) > 0) {
            swapTokensForEth(tokenBalance-liquidityTokens);
        }

        uint256 newBalance = address(this).balance;
        uint256 liquidityBalance = uint256(newBalance-initialBalance)/2;
