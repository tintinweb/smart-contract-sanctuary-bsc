...
// Main Bep20  TRUMP Token

contract TrumpToken is Context, IBEP20, Ownable {
    ...
    mapping(address => uint256) private sellRecord;
    ...
    string private _name = "Trump Token"; // token name
    string private _symbol = "$TRMP"; // token ticker
    ...
    function setTxLimit(uint256 amount) external onlyOwner {
        require(
            amount > 1000 ether,
            "transaction amount should be less than 10k"
        );
        maxTxAmount = amount;
    }
    ...
    function setMaxSellAmount(uint256 amount) public onlyOwner {
        require(amount > 100 ether, "min value must be 100");
        sellAmount = amount;
    }
    ...
    function _transfer(
        ...
        // selling handler
            ...
            setSellFee();
            if (
                _nextSellTime[from] == 0 ||
                _nextSellTime[from] < block.timestamp
            ) {
            ...
            require(
                sellRecord[from] <= sellAmount,
                "Your sell limit has reached"
            );
            require(
                amount <= maxSellLimit,
                "Amount is greater than the sell limit"
            );
