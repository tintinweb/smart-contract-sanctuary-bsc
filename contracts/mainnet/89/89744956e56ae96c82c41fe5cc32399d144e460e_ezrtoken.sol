...
contract BUSDDividendTracker is DividendPayingToken  {
    ...
    constructor(address _dividentToken) DividendPayingToken("ezrtoken_Busd_Dividend_Tracker", "ezrtoken_Busd_Dividend_Tracker",_dividentToken) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000000 * (10**18); 
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "ezrtoken_Busd_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "ezrtoken_Busd_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ezrtoken contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "ezrtoken_Busd_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "ezrtoken_Busd_Dividend_Tracker: Cannot update claimWait to same value");
...
contract ezrtoken is ERC20, Ownable {
    ...
    uint256 public farmingFee;
    uint256 public previousFarmingFee;

    uint256 public totalFees = busdDividendRewardsFee.add(farmingFee).add(liquidityFee);
    
    uint256 public _busdDividendRewardsFeeBuy = 0;
    uint256 public _farmingFeeBuy = 4;
    uint256 public _LpFeeBuy = 4;

    uint256 public _busdDividendRewardsFeeSell= 0;
    uint256 public _LpFeeSell= 4;
    uint256 public _farmingFeeSell= 4;  
    ...
    event FarmingWalletUpdated(address indexed newFarmingWallet, address indexed oldFarmingWallet);
    ...
    constructor() ERC20("EZR Token", "EZR") {
        	
    	farmingWallet = 0xC07605C4C58ec1fE8Efa4a8dCe51F51879b208FF; 
        ...
        excludeFromFees(farmingWallet, true);
    ...
    receive() external payable {
  	...
  	function updateFarmingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != farmingWallet, "ezrtoken: The farming wallet is already this address");
        excludeFromFees(_newWallet, true);
        emit FarmingWalletUpdated(farmingWallet, _newWallet);
  	    farmingWallet = _newWallet;
  	}
    ...
    function setFarmingEnabled(bool _enabled) external onlyOwner {
        require(farmingEnabled != _enabled, "Can't set flag to same status");
        if (_enabled == false) {
            previousFarmingFee = farmingFee;
            farmingFee = 0;
    ...
    function updatebusdDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(busdDividendTracker), "ezrtoken: The dividend tracker already has that address");
 
        BUSDDividendTracker newbusdDividendTracker = BUSDDividendTracker(payable(newAddress));
 
        require(newbusdDividendTracker.owner() == address(this), "ezrtoken: The new dividend tracker must be owned by the ezrtoken token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "ezrtoken: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "ezrtoken: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ezrtoken: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "ezrtoken: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "ezrtoken: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "ezrtoken: Trading has not started yet");
        ...
        if(takeFee) {
            ...
            uint256 tmpFarmingRewardPercent;
            uint256 tmpBusdDividedRewardsInContract;
            uint256 tmpLpRewardInContract;

            tmpFarmingRewardPercent = amount.mul(farmingFee).div(100);
            ...
            fees = tmpFarmingRewardPercent.add(tmpBusdDividedRewardsInContract)
            ...
            super._transfer(address(this),farmingWallet,tmpFarmingRewardPercent);
        }
