...
contract LOON is ERC20Burnable, Operator {
    ...
    // Initial distribution for the day 2-5 LOON-WETH LP -> LOON pool
    uint256 public constant INITIAL_LOON_POOL_DISTRIBUTION = 100000 ether;
    ...
    /**
     * @notice Constructs the LOON ERC-20 contract.
     */
    constructor( address _taxCollectorAddress) public ERC20("Doubloon", "LOON") {
        ...
        // Mints 1 LOON to contract creator for initial pool setup
    ...
    function _getLOONPrice() internal view returns (uint256 _LOONPrice) {
        try IOracle(LOONOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("LOON: failed to fetch LOON price from Oracle");
        }
    }
    ...
    function setLOONOracle(address _LOONOracle) public onlyOperatorOrTaxOffice {
        require(_LOONOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints LOON to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of LOON to mint to
    ...
    function distributeReward(
        ...
        _mint(_loonPool, INITIAL_LOON_POOL_DISTRIBUTION);
    }
