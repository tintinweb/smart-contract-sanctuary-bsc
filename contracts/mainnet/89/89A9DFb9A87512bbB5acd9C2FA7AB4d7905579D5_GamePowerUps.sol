...
contract GamePowerUps is ERC1155, ERC1155Burnable, Ownable, ReentrancyGuard {
  ...
  Counters.Counter private powerCount; //total count of powerups
  ...
  mapping(uint256 => powerUpInfo) private powerUps;
  //mapping of tokenId to a bool to check weather the id exists or not
  mapping(uint256 => bool) private exists;

  uint256[] private tokenIds; // array of powerup ids/token ids
  string private baseURI;
  address private rewardAddress; //rewardAddress

  //constructor requires a token address
  constructor(IERC20 _token, string memory _baseURI,address _rewardAddress) ERC1155(_baseURI) {
    token = _token;
    baseURI = _baseURI;
  ...
  event BatchUsed(address indexed from, uint256[] tokenId, uint256[] quantity);
  ...
  //modifier for reward address
  modifier onlyRewardAddress() {
      require(msg.sender == rewardAddress, "Only Reward Address can call this function.");
      _;
  }
  ...
  function buyPowerUp(
    ...
    token.transferFrom(msg.sender, rewardAddress, cost);
  ...
  function usePowerUp(
    ...
    uint256 tokenId,
    uint256 quantity
  ) external existing(tokenId) tokenQuantity(quantity) onlyRewardAddress {
    require(
      balanceOf(user, tokenId) >= quantity,
      "You don't have enough powerups."
    );
    _burn(user, tokenId, quantity);
    emit Used(user, tokenId, quantity);
  }

 function useBatchPowerUp(
    address user,
    uint256[] memory tokenId,
    uint256[] memory quantity
  ) external onlyRewardAddress {
    for(uint256 i=0; i< tokenId.length; i++){
    require(
      balanceOf(user, tokenId[i]) >= quantity[i],
      "You don't have enough powerups."
    );}
    _burnBatch(user, tokenId, quantity);
    emit BatchUsed(user, tokenId, quantity);
  }
  ...
  //function to return the rewardAddress
