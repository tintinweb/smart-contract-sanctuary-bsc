...
// File: RebateTreasuryV2.sol

contract RebateTreasuryV2 is MultipleOperator, Pausable {
    ...
    bool public enableAllowances = true;
    ...
    struct UserInfo {
        uint256 allowance;
        uint256 allowanceMultiplier;
        bool multiplierIncrease;
        uint256 lastEpochBuy;
        bool useStaticAllowance;
    }
    mapping(address => UserInfo) public userInfo;
    uint256 public startEpochAllowance;
    ...
    function bond(address _token, uint256 _amount, bool _intoLP, uint256 _slippage) external {
        ...
        require(bondLimit > minAmount, "Error: Insufficient tokens to bond");
        ...
        if (enableAllowances == true) {
            ...
            //Reset allowance.
            if (user.useStaticAllowance == false && user.lastEpochBuy < currentEpoch) {
                if (user.multiplierIncrease == true) {
                    user.allowance = startEpochAllowance
                        .mul(BASIS_POINTS_DENOM.add(user.allowanceMultiplier))
                        .div(BASIS_POINTS_DENOM);
                } else {
                    user.allowance = startEpochAllowance
                        .mul(BASIS_POINTS_DENOM.sub(user.allowanceMultiplier))
                        .div(BASIS_POINTS_DENOM);
                }

            }

            //Truncate amount in to account for allowance.
            uint256 allowance = user.allowance;
            require(allowance > minAmount && rebateTokenAmount > minAmount, "Error (RebateTreasury): Paused.");
            if (allowance < rebateTokenAmount) {
                _amount = _amount.mul(allowance).div(rebateTokenAmount);
                user.allowance = 0;
            } else {
                user.allowance = user.allowance.sub(rebateTokenAmount);
            }
        }

        //Set the buy of the last epoch.
        user.lastEpochBuy = currentEpoch;
    ...
    function redeemAssetsForBuyback(address[] calldata tokens) external onlyOperator {        
    ...
    function setStaticBondPremium(uint256 _staticBondPremium) external onlyOperator {
        staticBondPremium = _staticBondPremium;
    }

    function setEnableStaticBondPremium(bool _enableStaticBondPremium) external onlyOperator {
        enableStaticBondPremium = _enableStaticBondPremium;
    }

    function setStartEpochAllowance(uint256 _startEpochAllowance) external onlyOperator {
        startEpochAllowance = _startEpochAllowance;
    }
    ...
    function setUserInfo(address _user, uint256 _allowance, uint256 _allowanceMultiplier, bool _multiplierIncrease, bool _useStaticAllowance) external onlyOperator {
        userInfo[_user] = UserInfo({
            allowance: _allowance,
            allowanceMultiplier: _allowanceMultiplier,
            multiplierIncrease: _multiplierIncrease,
            lastEpochBuy: userInfo[_user].lastEpochBuy,
            useStaticAllowance: _useStaticAllowance
        });
    }

    function setUserAllowance(address _user, uint256 _allowance) external onlyOperator {
        userInfo[_user].allowance = _allowance;
    }

    function setUserAllowanceMultiplier(address _user, uint256 _allowanceMultiplier, bool _multiplierIncrease) external onlyOperator {
        userInfo[_user].allowanceMultiplier = _allowanceMultiplier;
        userInfo[_user].multiplierIncrease = _multiplierIncrease;
    }
