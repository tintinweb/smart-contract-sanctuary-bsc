...
/*
https://t.me/ogpredict

https://twitter.com/og_predict
*/
...
contract OG is IERC20, Context, Ownable, ReentrancyGuard {
    event Bought(
    ...
    // Total Supply
    uint256 public constant _totalSupply = 5 * 10**8 * _DECMULTIPLIER;
    ...
    //Tax wallets
    address public teamWallet = 0x606380038BeaA60cE5914d673BFcf4B809CEDc1C;
    ...
    uint256 public taxThreshold = 100 * 10**_srgDecimals;
    ...
    function changeTxLimits(uint256 newLimit, uint256 newMaxTx)
        external
        onlyOwner
    {
        require(
            newLimit >= _totalSupply / 100 &&
                newMaxTx >= _totalSupply / 2 / 100,
    ...
    function _buy(
        ...
        if (launchedAt + 100 > block.number) {
            require(!isContract(buyer), "contracts cannot buy at launch");
        }
        ...
        //transfer tax at threshold
        if (taxBalance > taxThreshold) {
            bool temp1 = SRGIE.transfer(teamWallet, taxBalance);
            require(temp1);
            taxBalance = 0;
        }
    ...
    function changeFeeReceivers(address newTeamWallet) external onlyOwner {
        require(
