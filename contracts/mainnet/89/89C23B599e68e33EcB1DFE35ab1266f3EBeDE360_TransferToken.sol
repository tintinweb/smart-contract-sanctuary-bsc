...
contract TransferToken is Controller {
    ...
    function deposit(address _token, uint256 _amount) external whenNotPaused isValidToken(_token) {
        address user = _msgSender();
        IERC20(_token).transferFrom(user, receiver, _amount);
        emit Deposit(user, _token, _amount, block.timestamp);
    }

    function withdraw(address _token, uint256 _amount, uint256 _nonce, bytes memory _sign) external whenNotPaused isValidToken(_token) {
        address user = _msgSender();
        require(block.timestamp <= _nonce + requestExpire, "request expired");
        bytes32 hash = keccak256(abi.encodePacked(user, _token, _amount, _nonce));
        address signer = hash.toEthSignedMessageHash().recover(_sign);
        require(signer == validator, "invalid sign");
        IERC20(_token).transfer(user, _amount);
        emit Withdraw(user, _token, _amount, block.timestamp);
    }

    function addToken(address _token) external onlyManager {
        tokens[_token] = true;
    }

    function removeToken(address _token) external onlyManager {
        tokens[_token] = false;
    }
    ...
    function updateRequestExpire(uint256 _number) external onlyManager {
