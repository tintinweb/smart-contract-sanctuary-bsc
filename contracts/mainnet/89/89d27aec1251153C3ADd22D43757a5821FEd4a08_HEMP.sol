...
contract HEMP is IBEP20, Ownable {
    ...
    function setCouncil(address newCouncil) external onlyOwner {
        require(
            council != newCouncil,
            "HEMP: council address cannot be the same"
        );
    ...
    function setDividend(address newDividend) external onlyOwner {
        require(
            ...
            "HEMP: dividend address cannot be the same"
        );
    ...
    function _transfer(
        ...
        if (_shouldTakeFee(sender, recipient)) {
            ...
            require(
                ...
                "HEMP: transfer amount + fees exceeds balance"
            );
    ...
    /**
     * @dev Transfers the fee `amount` of HEMP tokens from the `sender` to the `feeWallet`
     */
