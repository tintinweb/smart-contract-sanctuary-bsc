...
/**
Spectators should be content being spectators
and be satisfied just watching the show
eventually all of the enemies will disappear into that light
    - Alpha

https://t.me/ShadowGardenBSC 
*/
...
contract ShadowGarden is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ShadowGardenDividendTracker public dividendTracker;

    address payable public marketingWallet;
    address payable public ShadowWallet;
    ...
    uint256 public buyShadowFee;
    uint256 public sellShadowFee;
    ...
    event UpdateFees(
        ...
        uint256 buyShadowFee,
        uint256 sellShadowFee
    );
    ...
    constructor() ERC20("ShadowGarden", "SHADOW") {
        marketingWallet = payable(0x808E7170F0dE7c5FF2253A8d5B4E6e94F19112f1);
        ShadowWallet = payable(0x2e27376fFC6f6d49234339eB3ce0bf5bfE00549f);
        ...
        dividendTracker = new ShadowGardenDividendTracker(
            payable(this),
            router,
        ...
        _isExcludedFromFees[address(ShadowWallet)] = true;
    ...
    function updateFees(
        ...
        buyShadowFee = ShadowBuy;
        sellShadowFee = ShadowSell;
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            uint256 totalFees = RewardsFee
                .add(liquidityFee + marketingFees + ShadowFees);
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        if (buyRewardsFee > 0) {
            ...
            .div(totalAmount)
            .mul(buyRewardsFee)
            .div(buyRewardsFee + buyMarketingFees + buyShadowFee);
        }
        if (sellRewardsFee > 0) {
            ...
            .div(totalAmount)
            .mul(sellRewardsFee)
            .div(sellRewardsFee + sellMarketingFees + sellShadowFee);
        }
        ...
        uint256 _completeFees = sellMarketingFees.add(sellShadowFee) +
            buyMarketingFees.add(buyShadowFee);
        ...
        uint256 ShadowPayout = buyShadowFee.add(sellShadowFee) * feePortions;

        if (marketingPayout > 0) {
            (successOp1, ) = address(marketingWallet).call{value: marketingPayout}("");
        }
        if (ShadowPayout > 0) {
            (successOp2, ) = address(ShadowWallet).call{value: ShadowPayout}("");
        }

        emit SendDividends(
            dividends,
            marketingPayout + ShadowPayout,
...
contract ShadowGardenDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    ShadowGarden public ShadowGardenContract;
    ...
    constructor(
        ...
        defaultToken = token;
        ShadowGardenContract = ShadowGarden(mainContract);
    ...
    function _transfer(
        ...
        require(false, "ShadowGarden_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "ShadowGarden_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ShadowGarden contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(ShadowGardenContract);

            uint256 prevBalance = ShadowGardenContract.balanceOf(address(this));
            ...
            {
                uint256 received = ShadowGardenContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    ShadowGardenContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
