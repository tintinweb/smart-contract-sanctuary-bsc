...
contract Luck is Ownable,ERC20{
    ...
    constructor(address _bnb, address _platAddress, address _receiveAddress, address _router) ERC20('Test','Test'){
        bnb = _bnb;
        platAddress = _platAddress;
        receiveAddress = _receiveAddress;
        _mint(msg.sender,202300*1e18);
        nowOutAmount = 202300*1e18;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        if(!delivers[sender] && !delivers[recipient] && sender != address(router)){
            ...
            super._transfer(sender,platAddress,platAmount);

            _burn(sender,burnAmount);

            super._transfer(sender,address(this),LPAmount);
            if(sender != address(pair) && balanceOf(address(this)) >= addLPAmount){
                uint balanceLast = payable(address(this)).balance;
                uint nowBalnace = balanceOf(address(this));
                _approve(address(this), address(router), nowBalnace);
                swap(nowBalnace/2,address(this));
                (,uint amountBNB) = payable (address(this)).balance.trySub(balanceLast);
                addLP(nowBalnace/2,amountBNB,receiveAddress);
            }
    ...
    function swap(uint amountIn, address to) public{
    ...
    function addLP(uint amount,uint amountBNB,address to) public payable {
        router.addLiquidityETH{value: amountBNB}(address(this),amount,0,0,to,block.timestamp+60);
    }

    function setAddress(address _plat, address _receive) public onlyOwner{
        platAddress = _plat;
    ...
    function transferOut(address token, uint amount) public onlyOwner{
