...
contract TowerOfGod is Context, IERC20, Ownable {
    ...
    uint256 public swapTokensAtAmount = 30_000_000 * 10**9;
    ...
    address public marketingWallet = 0x75b92d888A7162eC84ED0Ce5cdF8608a56AAF79a;
    address public buybackWallet = 0x9A73a5505ea838956bf6cE884877b558087819F6;
    address private operationsWallet = 0x578fa5e4ed31217d066ae3389D0256083bEaa32b;

    string private constant _name = "TowerOfGod";
    ...
    Taxes public buyTaxes = Taxes(0, 4, 1, 1, 1);
    ...
    function setEnableTrading(
        ...
        deadline = _deadline;
        launchtax = Taxes(rfi, marketing, liquidity, buyback, operations);
    ...
    function setBuyTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _buyback, uint256 _operations) public onlyOwner {
        require(_rfi+(_marketing)+(_liquidity)+(_buyback)+(_operations) <=22, "Buy Taxes cannot be higher than 22%");
        buyTaxes = Taxes(_rfi,_marketing,_liquidity,_buyback,_operations);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _buyback, uint256 _operations) public onlyOwner {
        require(_rfi+(_marketing)+(_liquidity)+(_buyback)+(_operations) <=22, "Buy Taxes cannot be higher than 22%");
        sellTaxes = Taxes(_rfi,_marketing,_liquidity,_buyback,_operations);
        emit FeesChanged();
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell, bool useLaunchTax) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rbuyback, to_return.roperations) = _getRValues2(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell, bool useLaunchTax) private view returns (valuesFromGetValues memory s) {
        ...
        s.tTransferAmount = tAmount-s.tRfi-s.tMarketing-s.tLiquidity-s.tbuyback-s.toperations;
        return s;
    }
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rMarketing, uint256 rLiquidity){
        ...
        rTransferAmount =  rAmount-rRfi-rMarketing-rLiquidity-rbuyback-roperations;
        return (rAmount, rTransferAmount, rRfi,rMarketing,rLiquidity);
    }
    
    function _getRValues2(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 rbuyback,uint256 roperations) {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to] && block.number <= genesis_block + 6) {
            require(to != pair, "Sells not allowed for first 6 blocks");
        }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tMarketing + s.tbuyback+ s.toperations);
        }
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.marketing + temp.buyback + temp.operations) * 2;
