...
// File: contracts/triathonBatchTransfer.sol
...
contract triathonBatchTransfer is Ownable{
    address public geon;
    ...
    constructor(address geonAddress,address triasAddress,address partAddress,address hullAddress,address shipAddress,address artworkAddress){
        geon = geonAddress;
        trias = triasAddress;
        part = partAddress;
        hull = hullAddress;
    ...
    function changeTokenAddress(address geonAddress,address triasAddress,address partAddress,address hullAddress,address shipAddress,address artworkAddress) external onlyOwner{
        geon = geonAddress;
        trias = triasAddress;
        part = partAddress;
        hull = hullAddress;
    ...
    function artworkTransfer (address to,uint256[] memory tokenIds) external{
        for(uint256 i=0;i<tokenIds.length;i++){
            IERC721(artwork).safeTransferFrom(msg.sender, to, tokenIds[i]);
        }
    }

    function artworkBatchTransfer (address[] memory to,uint256[] memory tokenIds) external{
        require(to.length == tokenIds.length,"length not equal");
        for(uint256 i=0;i<tokenIds.length;i++){
            IERC721(artwork).safeTransferFrom(msg.sender, to[i], tokenIds[i]);
        }
    }

    function tokenBatchTransfer (address tokenAddress,address[] memory to,uint256[] memory tokenIds) external{
        require(to.length == tokenIds.length,"length not equal");
        for(uint256 i=0;i<tokenIds.length;i++){
            IERC721(tokenAddress).safeTransferFrom(msg.sender, to[i], tokenIds[i]);
        }
    }
}