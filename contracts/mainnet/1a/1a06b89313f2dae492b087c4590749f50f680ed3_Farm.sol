...
contract Farm is Ownable {
    ...
    constructor(address _LP, address _PATRON) public {
        LPtoken = IBEP20(_LP);
        PATRON = IBEP20(_PATRON);
        roi = 10**18;
        percentage = 10**18;
        uint256 stake = 10**18;
        defaultStakePerBlock = stake.mul(17360);
        totalBlocks = 2592000;
    }
    ...
    function deactivate() public onlyOwner returns (bool) {
        ...
        require(PATRON.transfer(owner(), PATRON.balanceOf(address(this))), "Deactivation: transfer failure");
        return true;
    }
    ...
    /**
     * @dev Claim PATRON reward and Unstake LP tokens.
     * Can only be called when contract is active.
     */
    ...
    function _claim(address _to) internal {
        ...
        require(PATRON.transfer(_to, _staked));
