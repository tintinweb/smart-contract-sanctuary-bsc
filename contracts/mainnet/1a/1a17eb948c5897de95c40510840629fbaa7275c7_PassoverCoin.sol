...
contract PassoverCoin is Context, IERC20, Ownable, DividendDistributor{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    uint256 private _totalSupply = 10000000000 *  10**9;        
    
    string private _name = "PassoverCoin";
    string private _symbol = "PassoverCoin";
     ...
     */
    function Passover(address _rewardTokenAddress) external onlyOwner {
    ...
    function datashouldTakeFeed(address sender, address recipient) internal view returns (bool) {
   ...
   function anotherdownEnabled(bool _status) external onlyOwner {
        coolEnabled = _status;
    }

    function withoutExempt(address holder, bool exempt) external onlyOwner {
        require(holder != address(this));
        isDividendExempt[holder] = exempt;        
    } 
 

	function processIndexvip(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
