...
contract Nebula is Ownable{
    ...
    uint256[] public token_reward_amount = [8000, 16000, 320000, 64000, 128000];
    uint256[] public plan_price = [8, 16, 32, 64, 128];
    uint256[] public refferral_percents = [2, 4, 6, 10, 15];
    uint256[] public nft_integral = [10, 20, 40, 80, 160];

    address NGC_COIN = 0xbf4499F5A9f5A5a426dC69a983a3B3ab1478029C;
    address NGC_HOLDER = 0xfeD318D0De093e803203A4FB0bd5149084388888;
    ...
    function joinIDO(address referrer, uint8 plan) public  {
        ...
        if(referrer == address(0)){
            referrer = 0x52FEBf2F9Db04054a18227f339C4d38E3c899999;
        }
        ...
        uint256 usdtProfit = plan_price[plan]*1e18 * (refferral_percents[refPlan]/100);
        ...
        ngc_token.transferFrom(NGC_HOLDER, msg.sender, ngcRewardAmount);
    }
