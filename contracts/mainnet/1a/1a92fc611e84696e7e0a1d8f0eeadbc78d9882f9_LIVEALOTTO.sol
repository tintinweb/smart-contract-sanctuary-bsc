...
/**
LIVEALOTTO LLT TOKEN
*/

pragma solidity ^0.7.4;

/**
Complicated but necessary stuff
*/
library SafeMath {
...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 RWRD = IBEP20(0x0000000000000000000000000000000000000000); /* empty ca for no rewards */
    ...
    /**
	Mostly useless functions because there are no rewards, but some of it is needed for reflections
	*/
...
contract LIVEALOTTO is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "LIVEALOTTO";
    string constant _symbol = "LLT";
    uint8 constant _decimals = 4;

    uint256 _totalSupply = 100 * 100000 * 10**_decimals;
    ...
    uint256 public totalBuyFee     = marketingFee + reflectionFee + liquidityFee + teamFee;
    ...
    uint256 public totalSellFee        = reflectionSellFee + liquiditySellFee + marketingSellFee + teamSellFee;
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0xDEfE594ECf12083045F4bb849727C06db1B812B3;
        teamFeeReceiver = 0x25072D1406e01Da39b30D31477c3aB88b7AABf28;
    ...
    function takeFee(address sender, uint256 amount, bool isSell) internal returns (uint256) {
        ...
        uint256 feeAmount = amount.mul(totalSellFee).mul(multiplier).div(feeDenominator * 100);
...
/*OK*/
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _teamFee, uint256 _feeDenominator) external authorized {
        ...
        teamFee = _teamFee;
        totalBuyFee = _liquidityFee.add(_reflectionFee).add(_marketingFee).add(_teamFee);
        feeDenominator = _feeDenominator;
        require(totalBuyFee < feeDenominator/2, "Fees cannot be more than 50%");
    }
	
	function setSellFees(uint256 _liquiditySellFee, uint256 _reflectionSellFee, uint256 _marketingSellFee, uint256 _teamSellFee, uint256 _feeDenominator) external authorized {
        liquiditySellFee = _liquiditySellFee;
        reflectionSellFee = _reflectionSellFee;
        marketingSellFee = _marketingSellFee;
        teamSellFee = _teamSellFee;
        totalSellFee = _liquiditySellFee.add(_reflectionSellFee).add(_marketingSellFee).add(_teamSellFee);
        feeDenominator = _feeDenominator;
        require(totalSellFee < feeDenominator/2, "Fees cannot be more than 50%");
    }
...
// LIVEALOTTO LLT contract by Raghza 2022