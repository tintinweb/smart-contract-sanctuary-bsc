...
// File: contracts/MarketplaceAssetsHelper.sol
...
contract MarketplaceAssetsHelper is IMarketplaceAssets,Ownable {
    ...
    function getFullAssetIdAndTokenId(uint256 _from,uint256 _to) public view  returns(string[] memory,uint256[] memory,bool[] memory){
        require(_to >= _from , "_to Must be bigger than _from");
        uint256 count = _from;
        uint256 index = 0;
         if(_to > MarketAddress.MarketTokenCounter() ) {
            _to =uint256( MarketAddress.MarketTokenCounter() );
        }
        uint256 LoopSize = _to - _from + 1; 
        uint256 ArraySize = uint256(MarketAddress.getNFTTotalSupply() );

        string[] memory assetIds = new string[](ArraySize);
        uint256[] memory tokenIds = new uint256[](ArraySize);
        bool[] memory saleable = new bool[](ArraySize);

        for (uint256 i=0; i< LoopSize ; i++ ) {
            //mappingIndex start from Number 1 
            if(MarketAddress.getFullDataByTokenIdNew(count).tokenId != 0 ){
                assetIds[index]=MarketAddress.getFullDataByTokenIdNew(count).assetId ;
                tokenIds[index]=MarketAddress.getFullDataByTokenIdNew(count).tokenId ;
                saleable[index]=MarketAddress.getFullDataByTokenIdNew(count).saleable ;
                index++;
            }
            count++ ;
        }
    ...
    function getFullDataByType(uint256 _type) public view  returns(MainAssetsNew[] memory){
        uint256 counter=0;
        uint256 totalSupplyType= MarketAddress.getSupplyByType(_type) ;
        MainAssetsNew[] memory assets = new MainAssetsNew[](totalSupplyType);
        for (uint256 i=1 ; i<= MarketAddress.MarketTokenCounter() ; i++ ) {
            if(MarketAddress.getFullDataByTokenIdNew(i).asset_type == _type){
                assets[counter]  = MarketAddress.getFullDataByTokenIdNew(i) ;
                counter++;
            }
        }
        return assets;
    ...
    function getFullDataByOwner(address _owner) public view returns(MainAssetsNew[] memory){
        //fix bug
        //uint256 balanceOf = nftContract.balanceOf(_owner);
        uint256 balanceOf = MarketAddress.getSupplyByOwner(_owner);
        MainAssetsNew[] memory assets = new MainAssetsNew[](balanceOf);

        uint256 counter;
        counter=0;
        for(uint256 i=1;i<= MarketAddress.MarketTokenCounter() ; i++) {
            if(   MarketAddress.getFullDataByTokenIdNew(i).owner   == _owner  ){
                 assets[counter] = MarketAddress.getFullDataByTokenIdNew(i);
                counter++;
                //assets.push(MarketAddress.getFullDataByTokenIdNew(i));
            }
        }

        return assets;
    }
    function getFullData(uint256 _from,uint256 _to ) public view returns(MainAssetsNew[] memory) {
        require(_to >= _from , "_to Must be bigger than _from");
        uint256 count = _from;
        uint256 index = 0;
        if(_to > MarketAddress.MarketTokenCounter() ) {
        ...
        uint256 LoopSize = _to - _from + 1 + Burnded; 
        
        uint256 ArraySize = uint256(MarketAddress.getNFTTotalSupply() ) - Burnded;

        if(ArraySize > LoopSize ) {
            ArraySize = LoopSize;
        }

        MainAssetsNew[] memory assets = new MainAssetsNew[](ArraySize );
        for(uint256 i=0;i< LoopSize ; i++) {
            if( MarketAddress.getFullDataByTokenIdNew(count).tokenId != 0 ) {
                assets[index] = MarketAddress.getFullDataByTokenIdNew(count) ; 
                index++;
            }
            count++;
        }
