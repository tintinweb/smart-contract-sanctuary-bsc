...
contract APE is ERC20, Ownable {
    ...
    constructor(address dataAddr, address tokenAddr) ERC20("APE", "APE") {
        data = IData(dataAddr);
    ...
    function takeFee(
        ...
        uint256[] memory fee = calFee(user, amount);
    ...
    function calFee(address user, uint256 amount)
        internal
        view
        returns (uint256[] memory)
    {
        ...
        uint256 destroyAmount = amount
            .mul(data.string2uintMapping("destroyFeeRate"))
            .div(1000000)
            .div(100);

        uint256 totalFee = cFee.add(dFee).add(rFee).add(mFee).add(
            destroyAmount
        );

        result[0] = totalFee;
        result[1] = destroyAmount;
        return result;
    }
