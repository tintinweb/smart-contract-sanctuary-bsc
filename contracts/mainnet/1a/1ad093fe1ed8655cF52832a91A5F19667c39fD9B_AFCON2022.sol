...
contract DividendPayingToken is ERC20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
  ...
  address public constant ETH = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); //BUSD
...
pragma solidity ^0.8.8;
    ...
    contract AFCON2022 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    bool private canBlacklist = true;
    ...
    address private constant ETH = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); //BUSD

    //uint256 public swapTokensAtAmount = 100 * 10**6 * (10**9);
    uint256 private maxSupply = 177 * 10 ** 8 * (10**9);
    uint256 public swapTokensAtAmount = 2 * 10**7 * (10**9);
    uint256 public maxTxAmount = 2 * 10**7 * (10**9);

    uint256 public maxWalletTokenPerThousand = 10;
    uint256 public maxWalletToken = maxSupply.mul(maxWalletTokenPerThousand).div(1000);


    // marketing wallet 
    address payable public marketingWallet = payable(address(0x30cB5bD229C01372B09082A1A586Fb27262a72ff)); //marketing wallet

    // liquidity wallet
    address payable public liquidityWallet = payable(address(0xA12de3F6C2b271CBD3d5EC1F622559938BbCdf6E));
    ...
    constructor() ERC20("Afcon2022", "Afcon2022") {
      ...
      // IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);   //BSC Testnet
        ...
        /*
            ...
            and CANNOT be called ever again, mint total supply 770m.
        */
    ...
    // blacklist account address, true means blacklist the address, false means disable
    ...
    // disable the ability of blacklisting accounts
    ...
    function setLiquidityWallet(address payable newLiquidityWallet) external onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "2022: The liquidity wallet is already this address");
    ...
    function changeSellFees() external onlyOwner {
        uint256 dividendHoldersNum = dividendTracker.getNumberOfTokenHolders();

        if (dividendHoldersNum < 200) {
            setSellTax(850, 30, 30);
        } else if (dividendHoldersNum < 300) {
            setSellTax(720, 30, 30);
        } else if (dividendHoldersNum < 400) {
            setSellTax(590, 30, 30);
        } else if (dividendHoldersNum < 500) {
            setSellTax(460, 30, 30);
        } else if (dividendHoldersNum < 700) {
            setSellTax(330, 30, 30);
        } else if (dividendHoldersNum < 1000) {
            setSellTax(200, 30, 30);
        } else {
            setSellTax(70, 30, 30);
        }
    }
    ...
    function setMaxWalletTokenPerThousand(uint256 percent) external onlyOwner {
        maxWalletTokenPerThousand = percent;
        maxWalletToken = maxSupply.mul(maxWalletTokenPerThousand).div(1000);
    }
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 liquidityTokens = contractTokenBalance
                ...
                .div(totalFees);
            swapAndAddLiquidity(liquidityTokens);
        ...
        if (takeFee) {
            ...
            if (!automatedMarketMakerPairs[to]) {
                uint256 holdToken = balanceOf(to);
                require((holdToken + amount) <= maxWalletToken, "Total Holding is currently limited, you can not buy that much.");
            }
...
contract TOKENDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor()
        DividendPayingToken("Afcon2022_Dividend_Tracker", "Afcon2022_Dividend_Tracker")
    {
        claimWait = 3600;
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "TOKEN_Dividend_Tracker: claimWait must be updated to between 1 sec and 24 hours"
        );
        require(
