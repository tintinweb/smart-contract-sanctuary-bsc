...
contract ForthBoxMarketSwap is Ownable, ReentrancyGuard {
    ...
    function _planSell(uint256 contractTypeInt,address contractAddr,uint256 token_id,uint256 amount,address erc20Addr,uint256 price) internal{
        ...
        require(price > 10000,"ForthBoxMarketSwap: price too small !");
        {
            ...
            _mSwapData[_mSumCount].key = mTempKey;
        }
    ...
    function _sell(uint256 id,uint256 token_id_sell) internal{
        ...
        {
            _mSwapData[_mSumCount].id = _mSumCount;
            _mSwapData[_mSumCount].swapType = enumSwapType.eSell;
            _mSwapData[_mSumCount].stateType = enumStateType.eClosed;
            _mSwapData[_mSumCount].startAddr = startAddr;
    ...
    function _fix(uint256 id,uint256 price) internal{
        ...
        if(_mSwapData[id].swapType == enumSwapType.ePlanSell){
            require(price<_mSwapData[id].contractData.price,"plan sell fix price must smaller than old !");
        }
        if(_mSwapData[id].swapType == enumSwapType.ePlanBuy){
            require(price>_mSwapData[id].contractData.price,"plan sell fix price must bigger than old !");
        }   
        require(startAddr == _msgSender(),"ForthBoxMarketSwap:not owner!");
        {
            _mSwapData[_mSumCount].id = _mSumCount;
    ...
    function planSell(uint256 contractTypeInt,address contractAddr,uint256 token_id,uint256 amount,address erc20Addr,uint256 price,uint256 key) external nonReentrant {
    ...
    function planBuy(uint256 contractTypeInt,address contractAddr,uint256 token_id,uint256 amount,address erc20Addr,uint256 price,uint256 key)  external nonReentrant{
        require(mbStart, "ForthBoxMarketSwap: not start!");
    ...
    function buy(uint256 id,uint256 key) external nonReentrant{
        require(mbStart, "ForthBoxMarketSwap: not start!");
        require(isWhiteContract(_msgSender()), "ForthBoxMarketSwap: Contract not in white list!");
        mTempKey = key;
        _buy(id);
    ...
    function sell(uint256 id,uint256 token_id_sell,uint256 key) external nonReentrant{
        require(mbStart, "ForthBoxMarketSwap: not start!");
        require(isWhiteContract(_msgSender()), "ForthBoxMarketSwap: Contract not in white list!");
        mTempKey = key;
    ...
    function fix(uint256 id,uint256 price,uint256 key) external nonReentrant{
        require(isWhiteContract(_msgSender()), "ForthBoxMarketSwap: Contract not in white list!");
        mTempKey = key;
        _fix(id,price); 
        mTempKey = 0;   
    }
    function cancle(uint256 id,uint256 key) external nonReentrant{
        require(isWhiteContract(_msgSender()), "ForthBoxMarketSwap: Contract not in white list!");
        mTempKey = key;
        _cancle(id);   
        mTempKey = 0; 
    }
    function planSells(uint256 contractTypeInt,address contractAddr,uint256[] calldata token_ids,uint256[] calldata amounts,address erc20Addr,uint256[] calldata prices,uint256 key) external nonReentrant {
        require(mbStart, "ForthBoxMarketSwap: not start!");
    ...
    function planBuys(uint256 contractTypeInt,address contractAddr,uint256[] calldata token_ids,uint256[] calldata amounts,address erc20Addr,uint256[] calldata prices,uint256 key)  external nonReentrant{
        require(mbStart, "ForthBoxMarketSwap: not start!");
        require(mbStartPlanBuy, "ForthBoxMarketSwap: not  plan buy!");
        require(token_ids.length == amounts.length , "ForthBoxMarketSwap: length not equal!");
        require(token_ids.length == prices.length , "ForthBoxMarketSwap: length not equal!");
    ...
    function buys(uint256[] calldata ids,uint256 key) external nonReentrant{
        require(mbStart, "ForthBoxMarketSwap: not start!");
        require(isWhiteContract(_msgSender()), "ForthBoxMarketSwap: Contract not in white list!");
        ...
        bool bOk = false;
        for(uint256 i=0; i<ids.length; ++i) {
            if(!_checkPlanSell(ids[i])) continue;
            _buy(ids[i]);  
            bOk = true; 
        }
        mTempKey = 0; 
        require(bOk, "ForthBoxMarketSwap: non success!");
    }
    function sells(uint256[] calldata ids,uint256[] calldata token_id_sells,uint256 key) external nonReentrant{
        require(mbStart, "ForthBoxMarketSwap: not start!");
        require(isWhiteContract(_msgSender()), "ForthBoxMarketSwap: Contract not in white list!");
        ...
        bool bOk = false;
        for(uint256 i=0; i<ids.length; ++i) {
            if(!_checkPlanBuy(ids[i])) continue;
    ...
    function cancles(uint256[] calldata ids,uint256 key) external nonReentrant{
        require(isWhiteContract(_msgSender()), "ForthBoxMarketSwap: Contract not in white list!");
    ...
    function fixs(uint256[] calldata ids,uint256[] calldata prices,uint256 key) external nonReentrant{
        require(isWhiteContract(_msgSender()), "ForthBoxMarketSwap: Contract not in white list!");
