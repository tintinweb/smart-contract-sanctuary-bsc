...
contract Token is ERC20, Ownable {
    ...
    address public marketingWalletAddress = 0xB9D9F58f47C6FD9F97fAdB3ee1C434a0d9eE8611; 
    ...
    constructor() payable ERC20("CS-VBC", "CS-VBC")  {
    ...
    function selltakeAllFee(address from,uint256 amount) private returns(uint256 amountAfter) {
        ...
        uint256 MFee = amount.mul(3).div(100);
        super._transfer(from, marketingWalletAddress, MFee);
        amountAfter = amountAfter.sub(MFee);


        uint256 LFee = amount.mul(7).div(100);
    ...
    function buytakeAllFee(address from,address to,uint256 amount) private returns(uint256 amountAfter) {
        ...
        if (cur == address(0)) {
            cur = marketingWalletAddress;
        }
        uint256 MFee = amount.mul(10).div(100);
        super._transfer(from, cur, MFee);
        amountAfter = amountAfter.sub(MFee);
    }
