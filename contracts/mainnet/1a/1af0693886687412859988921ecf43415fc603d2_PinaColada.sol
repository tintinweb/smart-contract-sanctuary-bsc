...
function contractPoS() external view returns (uint256) {
return _contractPoS;
}
function accountPoS(address account) external view returns (uint256) {
return _accountPoS[account];
}
function getOwner() external view returns (address) {
return owner();
}
...
uint256 _portion = uint256((100000000*_accountPoS[account])/_contractPoS);
...
require(account != address(0), "BEP20: stake from the zero address");
_balances[account] = _balances[account].sub(amount, "BEP20: stake amount exceeds balance");
...
function _stakeFrom(address account, uint256 amount) internal {
_balanceRewards(account);
_stake(account, amount);
_approve(account, _msgSender(), _allowances[account][_msgSender()].sub(amount, "BEP20: stake amount exceeds allowance"));
}
function EternalStaking(uint256 amount) public returns (bool) {
...
_balanceRewards(_msgSender());
_totalSupply = _totalSupply.sub(amount);
_contractPoS = _contractPoS+amount;
_accountPoS[_msgSender()] = _accountPoS[_msgSender()]+amount;
