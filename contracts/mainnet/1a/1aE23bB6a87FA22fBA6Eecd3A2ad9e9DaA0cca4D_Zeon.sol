...
contract Zeon is ERC20Burnable, Operator {
    ...
    // Initial distribution for the day 2-5 ZEON-WETH LP -> ZEON pool
    // uint256 public constant INITIAL_ZEON_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the ZEON ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("ZEON", "ZEON") {
        // Mints 1 ZEON to contract creator for initial pool setup
    ...
    function _getZeonPrice() internal view returns (uint256 _zeonPrice) {
        try IOracle(zeonOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Zeon: failed to fetch ZEON price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _zeonPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_zeonPrice >= taxTiersTwaps[tierId]) {
    ...
    function setZeonOracle(address _zeonOracle) public onlyOperatorOrTaxOffice {
        require(_zeonOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints ZEON to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of ZEON to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentZeonPrice = _getZeonPrice();
            currentTaxRate = _updateTaxRate(currentZeonPrice);
            if (currentZeonPrice < burnThreshold) {
                burnTax = true;
            }
        }
