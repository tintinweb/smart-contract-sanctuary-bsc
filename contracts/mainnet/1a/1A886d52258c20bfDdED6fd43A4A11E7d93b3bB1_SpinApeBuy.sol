...
contract SpinApeBuy is ReentrancyGuard, Ownable {
    ...
    function buy(uint256 amount, address refBy) public nonReentrant {
        ...
        require(
            nextBuy <= maxBuy,
            "Error: Your minimum allocation is $200 and maximum is $1000"
        );
...
contract SpinApeRef is Ownable, ReentrancyGuard {
    ...
    function calculateFundRef(
        ...
        if (rfL1 != address(0)) {
            ...
            totalUsdNeed += this.calculatePercent(amount, 800);
            totalUsdNeedPerTransaction += this.calculatePercent(amount, 800);
        }
        if (rfL2 != address(0)) {
        ...
        if (rfL3 != address(0)) {
            totalUsdOfUser[rfL3] += this.calculatePercent(amount, 300);
