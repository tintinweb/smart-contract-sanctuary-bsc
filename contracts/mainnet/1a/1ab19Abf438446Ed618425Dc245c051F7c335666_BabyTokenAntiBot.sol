...
contract BabyTokenAntiBot is ERC20, Ownable {
    ...
    IWhaleswapRouter02 public whaleswapRouter02;
    ...
    constructor(
        ...
        uint256[2] memory tokenSetting, // totalSupply_,  minimumTokenBalanceForDividends_,
        address[5] memory addrs, // reward, router, marketing wallet,  dividendTracker, whaleAntiBot_
        uint256[3] memory feeSettings, // rewards, liquidity, marketing,
        uint256 feeEnableAntiBot_
    )  ERC20(name_symbol[0], name_symbol[1]) {
        rewardToken = addrs[0];
        ...
        swapTokensAtAmount = tokenSetting[0].mul(2).div(10**6); // 0.002%

        whaleAntiBot = IWhaleAntiBot(addrs[4]);
        // Enable using whaleAntiBot in this contract
        antiBotEnabled = false;
        _whaleAntiBot = addrs[4];
        ...
        dividendTracker.initialize(rewardToken, tokenSetting[1]);

        IWhaleswapRouter02 _whaleswapRouter02 = IWhaleswapRouter02(addrs[1]);
        // Create a whaleswap pair for this new token
        address _whaleswapV2Pair = IWhaleswapFactory(
            _whaleswapRouter02.factory()
        ).createPair(address(this), _whaleswapRouter02.WETH());
        whaleswapRouter02 = _whaleswapRouter02;
        whaleswapV2Pair = _whaleswapV2Pair;
        _setAutomatedMarketMakerPair(_whaleswapV2Pair, true);
        ...
        dividendTracker.excludeFromDividends(address(_whaleswapRouter02));
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        newDividendTracker.excludeFromDividends(address(whaleswapRouter02));
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            newAddress != address(whaleswapRouter02),
            "BABYTOKEN: The router already has that address"
        );
        emit UpdateUniswapV2Router(newAddress, address(whaleswapRouter02));
        whaleswapRouter02 = IWhaleswapRouter02(newAddress);
        address _whaleswapV2Pair = IWhaleswapFactory(
            whaleswapRouter02.factory()
        ).createPair(address(this), whaleswapRouter02.WETH());
        whaleswapV2Pair = _whaleswapV2Pair;
    }
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        path[0] = address(this);
        path[1] = whaleswapRouter02.WETH();

        _approve(address(this), address(whaleswapRouter02), tokenAmount);

        // make the swap
        whaleswapRouter02.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function swapTokensForCake(uint256 tokenAmount) private {
        ...
        // make the swap
        whaleswapRouter02.swapExactTokensForTokensSupportingFeeOnTransferTokens(
                tokenAmount,
                0,
                path,
                address(this),
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        whaleswapRouter02.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
...
import "../Tokens/DividendPayingToken.sol";
...
import "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
import "../utilities/ContextUpgradeable.sol";
