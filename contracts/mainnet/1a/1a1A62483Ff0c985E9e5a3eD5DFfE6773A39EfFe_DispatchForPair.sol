...
contract DispatchForPair is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    uint256 public constant UPGRADE_LOCK_DURATION = 90 * 24 * 60 * 60;
    ...
    function setRouter(IEMERouter _router) external onlyOwner {
        router = _router;
    }
    ...
    modifier updateDispatch(address account) {
        ...
        uint256 _value = balance.sub(reserve);
        if (_value > 0 && _totalSupply > 0) {
            reserve = balance;
            lastReward = _value;
        }
    }
    ...
    {
        require(msg.sender == address(router), "caller not is router");
        _stake(msg.sender, account, amount);
    }

    function withdraw(uint256 amount) external updateDispatch(msg.sender) {
        require(unlockTime[msg.sender] < block.timestamp, "not expired");
    ...
    function withdrawAndRemoveLiquidity(uint256 amount)
        external
        updateDispatch(msg.sender)
    {
        require(unlockTime[msg.sender] < block.timestamp, "not expired");
        _totalSupply = _totalSupply.sub(amount);
        _balances[msg.sender] = _balances[msg.sender].sub(amount);
        IERC20(stakedToken).approve(address(router), amount);
        router.removeLiquidity(
            IEMEPair(stakedToken).token0(),
            IEMEPair(stakedToken).token1(),
            amount,
            0,
            0,
            msg.sender,
    ...
    function _stake(
        ...
        unlockTime[account] = block.timestamp.add(UPGRADE_LOCK_DURATION);
        ...
        clzNft.stakedStorage(stakedToken, account, amount);
