...
contract CADTTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SCADT }
    ...
    address public sCADT;
    uint public sCADTQueue; // Delays change to sCADT address
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        IERC20Mintable( CADT ).mint( msg.sender, send_ );
    ...
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        ICADTERC20( CADT ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sCADT ).balanceOf( msg.sender ); // Can only borrow against sCADT held
    ...
    /**
        ...
        @param _amount uint
     */
    function repayDebtWithCADT( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        ICADTERC20( CADT ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, CADT, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( CADT ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( CADT ).totalSupply().sub( totalDebt ) );
    }
    ...
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match CADT decimals
            value_ = _amount.mul( 10 ** IERC20( CADT ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SCADT ) { // 9
            sCADTQueue = block.number.add( blocksNeededForQueue );
        } else return false;
