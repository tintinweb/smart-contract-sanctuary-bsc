...
contract BloomBlue is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "BLUETOKEN";
    string private _symbol = "BKBT";
    uint8 private _decimals = 9;

    uint public launchedBlock;
    uint public killblock = 3;
    bool public isLaunch = false;
    address payable public marketingWalletAddress = payable(0x876b72Bd7E7DE5fA05b4665502a7e2F4d7D232E5); 
    ...
    uint256 private minimumTokensBeforeSwap = 2000000000000000 * 5 **_decimals; 
...
function setWhiteList(address[] calldata ws,bool status) public onlyOwner{
    for (uint256 i=0;i<ws.length;i++){
        isTxLimitExempt[ws[i]] = status;
        isWalletLimitExempt[ws[i]] = status;
        isExcludedFromFee[ws[i]] = status;
    }
}
    ...
    function writeBlackListMulti(address[] calldata bs, bool isBlack) public onlyOwner {
        for(uint256 i= 0;i<bs.length;i++){
            writeBlackList(bs[i],isBlack);
        }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        {
            if(checkWalletLimit && !isTxLimitExempt[sender] && !isTxLimitExempt[recipient]) {
