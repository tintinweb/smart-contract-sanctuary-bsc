...
contract Token is ERC20, Ownable {
    ...
    address public buyFeeLiquidity;
    ...
    uint256 public maxTotalSupply = 210000 * 1e18;
    ...
    {
        uint256 initSupply = 2100000 * 10**uint256(decimals());
        _mint(address(this), initSupply);
    }

    function init(
        address _buyFeeRecipient,
        address _buyFeeLiquidity,
    ...
    function setLiquidity(address _liquidity) public onlyOwner {
        ...
        super._transfer(address(this), liquidity, _totalSupply.mul(25).div(100));
    }

    function setTechnology(address _technology) public onlyOwner {
        require(technology == address(0), "already exists");
        technology = _technology;
        super._transfer(address(this), technology, _totalSupply.mul(5).div(100));
    }

    function setPool(address _pool) public onlyOwner {
        require(pool == address(0), "already exists");
        pool = _pool;
        super._transfer(address(this), pool, _totalSupply.mul(70).div(100));
    }
    ...
    function _recipientAmountSubFee(
        ...
        if (pair == sender) {
            ...
            if (buyFeeRecipient != address(0)) {
                // 0.5 buy recipient
                recipientAmount = amount.mul(5).div(1000);
                _balances[buyFeeRecipient] = _balances[buyFeeRecipient].add(
                    recipientAmount
                );
                emit Transfer(sender, buyFeeRecipient, recipientAmount);
            }
            if (buyFeeLiquidity != address(0)) {
                // 1 buy recipient
                backLiquidityAmount = amount.div(100);
                _balances[buyFeeLiquidity] = _balances[buyFeeLiquidity].add(
                    backLiquidityAmount
                );
                emit Transfer(sender, buyFeeLiquidity, backLiquidityAmount);
            }
            if (pool != address(0)) {
                // 1.5% lp dividend
                poolInAmount = amount.mul(15).div(1000);
                _balances[pool] = _balances[pool].add(poolInAmount);
                emit Transfer(sender, pool, poolInAmount);
            }
        } else {
            // sell 8%
            if (totalSupply() > maxTotalSupply) {
            ...
            if (pool != address(0)) {
                // 3% lp dividend
                poolInAmount = amount.mul(3).div(100);
                _balances[pool] = _balances[pool].add(poolInAmount);
                emit Transfer(sender, pool, poolInAmount);
            }
            if (sellFeeRecipient != address(0)) {
                // 2% recipient
                recipientAmount = amount.mul(2).div(100);
        ...
        return
            amount.sub(burnAmount).sub(poolInAmount).sub(recipientAmount).sub(
                backLiquidityAmount
            );
    }
}
