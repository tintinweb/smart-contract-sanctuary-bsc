...
contract LXPLifeGateway is Context, Ownable {
    ...
    address vaultAddress; //Receive all wallet payments

    //Configuration
    address public constant LXPLIFE_CONTRACT = 0xc49339F6b59215800f3420b6aF87a8d564B715F8;
    uint public GRACEPERIOD = 172800; //Number of seconds before expiry anyone is allowed to renew
    uint256 public USD_RATE; //Gets the rate from core lxplife contract
    ...
    function buySubscription(uint productId) external payable {
        ...
        //Check if user is registered in LXPLife system first
        uint userId = getLXPLifeUserId(userAddress);
        require(userId > 0, "Register in LXPLife first");
        ...
        require(checkUserLevelInLXPLife(userAddress, uint8(products[productId].minLevelrequired)), "Upgrade to required level first");
        ...
        if(products[productId].toVault > 0) {
            if(!payable(vaultAddress).send(convertDollarToCrypto(products[productId].toVault.div(100)))) {
                payable(vaultAddress).transfer(convertDollarToCrypto(products[productId].toVault.div(100)));
            }
        ...
        for(uint i = 1; i <= products[productId].toSponsors.length; i++) {
            if(uplineAddress == getLXPLifeOwner()) {
                uplineAddress = getLXPLifeOwner();
            } else {
                uplineAddress = getLXPLifeUserUpline(uplineAddress);
            }
            ...
            emit newPayment(userId, getLXPLifeUserId(uplineAddress), productId, i);
        }
    ...
    function getLXPLifeUserId(address userAddress) public view returns (uint) {
	    bytes memory payload = abi.encodeWithSignature("users(address)", userAddress);
        (bool success, bytes memory data) = address(LXPLIFE_CONTRACT).staticcall(payload);
        require(success, "Could not connect to LXPlife Contract");
        (uint id, address referrer, uint partnersCount) = abi.decode(data, (uint, address, uint));
        return id;
	}
    
    function getLXPLifeUserUpline(address userAddress) public view returns (address) {
	    bytes memory payload = abi.encodeWithSignature("users(address)", userAddress);
        (bool success, bytes memory data) = address(LXPLIFE_CONTRACT).staticcall(payload);
        require(success, "Could not connect to LXPlife Contract");
        (uint id, address referrer, uint referralsCount) = abi.decode(data, (uint, address, uint));
        return referrer;
	}

    function checkUserLevelInLXPLife(address userAddress, uint8 level) public view returns (bool) {
    ...
    function getLXPLifeUSDRATE() public view returns (uint) {
    ...
    function getLXPLifeOwner() public view returns (address) {
    ...
    function adjustUSD(uint256 rate) external onlyOwner {
        USD_RATE = rate;
    ...
    function addNewProduct(uint cost, uint minLevelrequired, uint toVault, uint[] memory toSponsors, uint subscriptionTime) external onlyOwner {
    ...
    function editProduct(uint productId, bool status, uint cost, uint minLevelrequired, uint toVault, uint[] memory toSponsors, uint subscriptionTime) external onlyOwner {
