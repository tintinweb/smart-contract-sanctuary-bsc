...
contract BoredGamblers is IERC20 {
    ...
    struct TaxWallets {
        ...
        address payable maintenenceAndGiveaway;
    }

    TaxWallets public _taxWallets = TaxWallets({
        marketing: payable(0xB66139C2471092740DCB7A17e2edB64263886939),
        maintenenceAndGiveaway: payable(0x56AA41DDa5400859Ed9f1bd578F1CCdE05E3F86B)
        });
...
//===============================================================================================================
    ...
    function setRatios(uint16 liquidity, uint16 marketing, uint16 maintenenceAndGiveaway) external onlyOwner {
        _ratios.liquidity = liquidity;
        _ratios.marketing = marketing;
        _ratios.maintenenceAndGiveaway = maintenenceAndGiveaway;
        _ratios.totalSwap = liquidity + marketing + maintenenceAndGiveaway;
        uint256 total = _taxRates.buyFee + _taxRates.sellFee;
        require(_ratios.totalSwap <= total, "Cannot exceed sum of buy and sell fees.");
    }

    function setWallets(address payable marketing, address payable maintenenceAndGiveaway) external onlyOwner {
        _taxWallets.marketing = payable(marketing);
        _taxWallets.maintenenceAndGiveaway = payable(maintenenceAndGiveaway);
    }
    ...
    function contractSwap(uint256 contractTokenBalance) internal lockTheSwap {
        ...
        bool success;
        uint256 maintenenceAndGiveawayBalance = (amtBalance * ratios.maintenenceAndGiveaway) / ratios.totalSwap;
        uint256 marketingBalance = amtBalance - maintenenceAndGiveawayBalance;
        if (ratios.marketing > 0) {
            (success,) = _taxWallets.marketing.call{value: marketingBalance, gas: 35000}("");
        }
        if (ratios.maintenenceAndGiveaway > 0) {
            (success,) = _taxWallets.maintenenceAndGiveaway.call{value: maintenenceAndGiveawayBalance, gas: 35000}("");
        }
    }
