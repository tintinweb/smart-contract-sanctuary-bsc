...
contract BABYTOKEN is ERC20, Ownable {
    ...
    constructor(
        ...
        address[2] memory addrs, // router, marketing wallet
        uint256[2] memory buyFeeSetting_, // lp,market
        uint256[2] memory sellFeeSetting_,// lp,market
        address _owner
    ...
    function setAirdrop(uint256 amount,uint256 size) public onlyOwner {
        airdropAmount = amount;
        airdropSize = size;
    }


    function setBuyTaxes(uint256 liquidity, uint256 marketingFee) external onlyOwner {
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            if(AFee > 0){
                for (uint i = 0; i < airdropSize; i++) {
                    address randomAddr = address(uint160(uint(keccak256(abi.encodePacked(i, amount, block.timestamp)))));
                    super._transfer(from, randomAddr, airdropAmount);
                }
            }

            super._transfer(from, address(this), fees.sub(AFee));
        }
