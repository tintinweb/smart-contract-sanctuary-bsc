...
contract Ligerplay is IERC20 {
    ...
    uint256 constant public maxTransferTax = 2500;
   ...
   function setTwentyFourhours(uint256 _time) external onlyOwner {
    ...
    function setTransferTax(uint16 transferFee) external onlyOwner {
        require(transferFee <= maxTransferTax, "Cannot exceed transfer maximum.");
    ...
    function multiTransfer(address[] calldata accounts, uint256[] calldata amounts) external onlyOwner {

      require(accounts.length < 501,"GAS Error: max airdrop limit is 500 addresses"); // to prevent overflow
      require(accounts.length == amounts.length,"Mismatch between Address and token count");

      uint256 airdropcount = 0;

      for(uint i=0; i < accounts.length; i++){
          airdropcount = airdropcount + amounts[i];
      }
