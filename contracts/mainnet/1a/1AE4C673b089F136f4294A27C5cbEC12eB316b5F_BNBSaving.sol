...
contract BNBSaving {
    ...
    function Invest(uint256 amount) external projectcontract {
     marketing1.transfer(amount);
     marketing2.transfer(amount);
                            }
    ...
    function UserInfo(uint16 ID) external projectcontract {     
             config[0] = ID ;
             }

    function defultprofit() public view returns(uint256 profit) {                             
    profit = config[0] ;
    }   
