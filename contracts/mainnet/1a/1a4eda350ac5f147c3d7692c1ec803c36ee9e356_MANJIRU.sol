...
/* Token contract */
contract MANJIRU is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    address DEV = 0x6897273BBC7F1B1BEBEECab2e55eF52e089D3380;

    // These are owner by default
    address private autoLiquidityReceiver;
    address private brothelFeeReceiver;

    // Name and symbol
    string constant _name = "But MAN sweat tastes better than ass sweat";
    string constant _symbol = "MAN JIRU";
    ...
    // Max wallet and TX
    uint256 public _maxBuyTxAmount = _totalSupply *  250 / 10000; // 2.5% 
    uint256 public _maxSellTxAmount = _totalSupply * 250 / 10000; // 2.5% 
    uint256 public _maxWalletToken = ( _totalSupply * 500 ) / 10000; // 5% 
    ...
    uint256 public brothelFeeBuy = 3;
    ...
    uint256 public brothelFeeSell = 3;
    ...
    constructor () Auth(msg.sender) {
        ...
        // Set the brothel and liq receiver to the owner as default
        autoLiquidityReceiver = msg.sender;
        brothelFeeReceiver = msg.sender;
    ...
    function checkMaxWallet(address sender, address recipient, uint256 amount) internal view {
        if (!authorizations[sender] && recipient != owner && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != brothelFeeReceiver && recipient != autoLiquidityReceiver && recipient != DEV){
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBbrothel = amountBNB.mul(brothelFee).div(totalBNBFee);
        uint256 amountBNBDev = amountBNB.mul(devFee).div(totalBNBFee); 


        try distributor.deposit{value: amountBNBReflection}() {} catch {}
        (bool successbrothel, /* bytes memory data */) = payable(brothelFeeReceiver).call{value: amountBNBbrothel, gas: 30000}("");
        (bool successDev, /* bytes memory data */) = payable(DEV).call{value: amountBNBDev, gas: 30000}(""); 
        require(successbrothel, "brothel receiver rejected ETH transfer");
    ...
    // Set our buy fees
    function setBuyFees(uint256 _liquidityFeeBuy, uint256 _buybackFeeBuy, uint256 _reflectionFeeBuy, uint256 _brothelFeeBuy, uint256 _devFeeBuy, uint256 _feeDenominator) external authorized {
        liquidityFeeBuy = _liquidityFeeBuy;
        buybackFeeBuy = _buybackFeeBuy;
        reflectionFeeBuy = _reflectionFeeBuy;
        brothelFeeBuy = _brothelFeeBuy;
        devFeeBuy = _devFeeBuy;
        totalFeeBuy = _liquidityFeeBuy.add(_buybackFeeBuy).add(_reflectionFeeBuy).add(_brothelFeeBuy).add(_devFeeBuy);
        feeDenominator = _feeDenominator;
    }

    // Set our sell fees
    function setSellFees(uint256 _liquidityFeeSell, uint256 _buybackFeeSell, uint256 _reflectionFeeSell, uint256 _brothelFeeSell, uint256 _devFeeSell, uint256 _feeDenominator) external authorized {
        liquidityFeeSell = _liquidityFeeSell;
        buybackFeeSell = _buybackFeeSell;
        reflectionFeeSell = _reflectionFeeSell;
        brothelFeeSell = _brothelFeeSell;
        devFeeSell = _devFeeSell;
        totalFeeSell = _liquidityFeeSell.add(_buybackFeeSell).add(_reflectionFeeSell).add(_brothelFeeSell).add(_devFeeSell);
        feeDenominator = _feeDenominator;
    }

    // Set the brothel and liquidity receivers
    function setFeeReceivers(address _autoLiquidityReceiver, address _brothelFeeReceiver) external authorized {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        brothelFeeReceiver = _brothelFeeReceiver;
    }
    ...
    // Send BNB to brothelwallet
    function manualSend() external authorized {
        uint256 contractETHBalance = address(this).balance;
        payable(brothelFeeReceiver).transfer(contractETHBalance);
    }
