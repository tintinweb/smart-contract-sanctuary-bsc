...
import "../interfaces/IERC721MintableWithSlotsInitializable.sol";

/**
 * @title ERC721MintableWithSlots
 * ERC721MintableWithSlots - ERC721 contract that allows to mint some tokens with id range
 */
contract ERC721MintableWithSlots is ERC721Mintable, IERC721MintableWithSlotsInitializable{

    uint256 public slotStart;
    uint256 public slotEnd;

    function initialize(bytes memory initializerEncoded_) initializer override public {
        (string memory name_, string memory symbol_, string memory contractURI_, address owner_, uint256 slotStart_, uint256 slotEnd_, address system_) = decodeInitializer_ERC721MintableWithSlots(initializerEncoded_);
    ...
    function decodeInitializer_ERC721MintableWithSlots(bytes memory initializerEncoded_) override public pure returns (string memory, string memory, string memory, address, uint256,
        uint256, address){
        return abi.decode(initializerEncoded_, (string, string, string, address, uint256, uint256, address));
    }

    function encodeInitializerParams(string memory name_, string memory symbol_, string memory contractURI_, address owner_, uint256 slotStart_,
        uint256 slotEnd_, address system_) public pure returns (bytes memory) {
        return  abi.encode(
            name_,
            symbol_,
            contractURI_,
    ...
    function encodeInitializer(string memory name_, string memory symbol_, string memory contractURI_, address owner_, uint256 slotStart_,
        uint256 slotEnd_, address system_) override public pure returns (bytes memory) {
        bytes memory params = encodeInitializerParams(name_, symbol_, contractURI_, owner_, slotStart_, slotEnd_, system_);
...
interface IERC721MintableWithSlotsInitializable {

    function encodeInitializer(string memory name_, string memory symbol_, string memory contractURI_, address owner_, uint256 slotStart_,
        uint256 slotEnd_, address system_) external pure returns (bytes memory);
    function decodeInitializer_ERC721MintableWithSlots(bytes calldata initializerEncoded_) external pure returns (string memory, string memory, string memory, address, uint256,
        uint256, address);
