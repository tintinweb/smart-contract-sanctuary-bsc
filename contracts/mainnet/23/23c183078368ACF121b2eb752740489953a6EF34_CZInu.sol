...
contract CZInu is ERC20, Ownable {
    ...
    CZInuDividendTracker public dividendTracker;
    ...
    constructor() ERC20("CZ Inu", "CZ") {
        marketingWallet = payable(0x51128260CF9e94cBc53DCBB33Cc91b78d5EC0927);
        devWallet = payable(0x51128260CF9e94cBc53DCBB33Cc91b78d5EC0927);
        ...
        dividendTracker = new CZInuDividendTracker(
            payable(this),
            router,
...
contract CZInuDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        CZInuContract = CZInu(mainContract);
    ...
    function _transfer(
        ...
        require(false, "CZInu_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "CZInu_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CZInu contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(CZInuContract);

            uint256 prevBalance = CZInuContract.balanceOf(address(this));
            ...
            {
                uint256 received = CZInuContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    CZInuContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
