...
contract VRFSub is VRFConsumerBaseV2 {
  ...
  uint256 wordsThreshold = 1;
  bool requestPending;
  
  address private constant ROUTER = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
  address private constant SWAP = 0x1FCc3B22955e76Ca48bF025f1A6993685975Bb9e;
  ...
  address public constant pegLINK = 0xF8A0BF9cF54Bb92F17374d9e9A321E6a111a51bD;
  ...
  function swapAndTopLink() public onlyGames {
    ...
    swap(WETH, pegLINK, amountIn, 0, address(this));

    uint256 pegAmount = IERC20(pegLINK).balanceOf(address(this));
    unPeg(pegLINK, LINK, pegAmount);
  ...
  function swap(address _tokenIn, address _tokenOut, uint256 _amountIn, uint256 _amountOutMin, address _to) internal {
    ...
    IRouter(ROUTER).swapExactTokensForTokens(_amountIn, _amountOutMin, path, _to, block.timestamp);
  }

  function unPeg(address _tokenIn, address _tokenOut, uint256 _amountIn) internal {
    ISwap(SWAP).swap(_amountIn, _tokenIn, _tokenOut);
  }
