...
contract FlokiDoge is Context, IERC20, Ownable {
    ...
    uint256 public swapTokensAtAmount = 100_000_000_000 * 10**_decimals;
    uint256 public maxBuyAmount = 1_000_000_000_000_000 * 10**_decimals;
    
    address public marketingW;
    address public buybackW;
    address public lpRecipient;

    string private constant _name = "FlokiDoge";
    ...
    constructor() {
        ...
        _isExcludedFromFee[buybackW]=true;
        _isExcludedFromFee[marketingW]=true;

        swapEnabled = true;

        emit Transfer(address(0x7bB09e3Ffa999FE9366C5cd4160679899f1D5793), owner(), _tTotal);
    }
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap{
       // Split the contract balance into halves
        ...
        if(elonFundAmt > 0){
            payable(marketingW).sendValue(elonFundAmt);
        }

        uint256 czFundAmt = unitBalance * 2 * taxes.czFund;
        if(czFundAmt > 0){
            payable(buybackW).sendValue(czFundAmt);
        }
    ...
    function updateMarketingW(address newWallet) external onlyOwner{
        marketingW = newWallet;
    }

    function updateBuybackW(address newWallet) external onlyOwner{
        buybackW = newWallet;
    }
