...
contract  Bridge is Context {
    ...
    bytes32 private ownerMerkleRoot;
    ...
    modifier onlyTokenOwner(bytes32[] memory merkleProof) {
        require(
            MerkleProof.verify(
                merkleProof,
                ownerMerkleRoot,
    ...
    */
     ...
     * @dev fillSwap
     */
    function withdrawTokens(uint256 chainId, uint256 amount, bytes32[] calldata merkleProof) onlyTokenOwner(merkleProof) external returns (bool) {
        require(tokenAddresses[chainId] != address(0x0), "no dest token exist");
        uint256 tokenBalance = IERC20(tokenAddresses[chainId]).balanceOf(address(this));
        require( tokenBalance > 0, "no tokens to withdraw");
        require( tokenBalance >= amount, "can't overflow tokenbalance");
        IERC20(tokenAddresses[chainId]).transfer(msg.sender, amount);
        // IERC20(tokenAddresses[chainId]).safeTransferFrom(address(this), msg.sender, tokenBalance);
        return true;
    }
