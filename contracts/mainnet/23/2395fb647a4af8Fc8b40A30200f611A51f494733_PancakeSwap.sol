...
// ----------------------------------------------- PancakeSwap ---------------------------------------------------------
// ---------------------------------------------------------------------------------------------------------------
contract PancakeSwap is BEP20, Pausable { 
	using SafeMath for uint256; 
	...
	function addLockedLiquidity(uint256 tokenAmount, uint256 bnbAmount) public payable lockLiquidity onlyOperator {
		...
		require(bnbAmount <= address(this).balance, 'PancakeSwap: not enough BNB');
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
		require(!blackList[sender] && !blackList[recipient], 'PancakeSwap: transfer from/to blacklisted address');		
        require(sender != address(0), 'PancakeSwap: transfer from the zero address');
        require(recipient != address(0), 'PancakeSwap: transfer to the zero address');
		if ( paused ) {
		    require(operatorsList[sender], "PancakeSwap: sender not whitelist to transfer when paused");
		}
		...
		if ( 
            ...
            require(amount <= maxTransferAmount, "PancakeSwap: sell transfer amount exceeds the max allowed");
			if (
				taxPercent != 0 &&
        ...
        _balances[sender] = _balances[sender].sub(amount, 'PancakeSwap: transfer amount exceeds balance');
	...
	function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        ...
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, 'PancakeSwap: transfer amount exceeds allowance') );
        return true;
    }

	function lockLpTokens(uint256 unlockTimestamp) external onlyOwner {
		require(lpUnlockTimestamp <= block.timestamp, 'PancakeSwap: already locked');
    ...
    function setTaxPercent(uint256 newTaxPercent) external onlyOwner {
		require(newTaxPercent <= 1200, 'PancakeSwap: tax can`t be more than 12%');
	...
	function toggleExchangesList(address account) external onlyOwner returns (bool) {
		require(account != pancakeSwapV2Pair, 'PancakeSwap: pancakeSwapV2Pair can`t be removed from list');
	...
	// drop to list of recipients with different amounts for each	
   	function migrateAccounts(address[] memory recipients, uint256[] memory amounts) public onlyOperator returns (uint256 amountTotal) {
        ...
        require(cnt > 0 && cnt <= 255, 'PancakeSwap: number or recipients must be more then 0 and not much than 255');
        require(amounts.length == recipients.length, 'PancakeSwap: number or recipients must be equal to number of amounts');
        for ( uint i = 0; i < cnt; i++ ){
			require(amounts[i] != 0, 'PancakeSwap: you can`t drop 0');
	...
	function availableTokensToMint() public view returns (uint256) {
   	...
   	function mint(uint256 amount) public onlyOperator {
		require(_totalSupply.add(amount) <= MAX_SYPPLY, 'PancakeSwap: exceed max supply');
	...
	function recoverTokens(address token, uint256 amount) external onlyOwner {
        if (token == pancakeSwapV2Pair) {
			require(lpUnlockTimestamp <= block.timestamp, 'PancakeSwap: can`t withdraw LP tokens before unlock time');			
		}				
		IBEP20(token).safeTransfer(_msgSender(), amount);        
    }

	function recoverBnb(uint256 amount) external onlyOwner {		
		require(amount <= balanceOfBnb(), 'PancakeSwap: transfer amount exceeds BNB balance');
        (bool sent,) = _msgSender().call{ value: amount }("");
        require(sent, 'PancakeSwap: failed');       
    }
