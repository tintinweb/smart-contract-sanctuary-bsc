...
contract TTZQCS is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public totalFees = ETHRewardsFee.add(burnFee).add(marketingFee).add(lpFee);
    bool public isL;
    ...
    constructor(address receiveAddress) public ERC20("TTZQCS", "TTZQCS") {
    ...
    function setETHRewardsFee(uint256 value) external onlyOwner{
        ETHRewardsFee = value;
        totalFees = ETHRewardsFee.add(burnFee).add(marketingFee).add(lpFee);
    }
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            uint256 marketingTokens = contractTokenBalance.mul(marketingFee.add(lpFee)).div(totalFees);
    ...
    function swapAndSendToFee(uint256 tokens) private  {
        ...
        uint256 lpAmount = tokens * lpFee / (lpFee + marketingFee) / 2;
        swapTokensForETH(tokens-lpAmount);
        uint256 newBalance = (IERC20(ETH).balanceOf(address(this))).sub(initialETHBalance);
        uint256 lpAmountTwo = newBalance * lpFee / (lpFee + marketingFee) / 2;
        IERC20(ETH).transfer(_marketingWalletAddress, newBalance-lpAmountTwo);


        _approve(address(this),address(uniswapV2Router),lpAmount);
        IERC20(ETH).approve(address(uniswapV2Router),lpAmountTwo);
