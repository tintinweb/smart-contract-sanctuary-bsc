...
contract SmellyCatDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("SmellyCat_Dividend_Tracker", "SmellyCat_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**9); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "SmellyCat_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "SmellyCat_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SmellyCat contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SmellyCat_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "SmellyCat_Dividend_Tracker: Cannot update claimWait to same value");
...
contract SmellyCat is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    SmellyCatDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Smelly Cat", "SmellyCat") 
    {
        BNBRewardsFee = 2;
        liquidityFee = 1;
        marketingFee = 2;
        ...
        marketingWallet = payable(0x2a6b06dc2049b9D5E17FA0C5d144AF5737535bD5);
        developerWallet = payable(0xE6B44D22080524C92bEA4901615b1f9366637bbc);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(developerFee); // total fee transfer and buy

    	dividendTracker = new SmellyCatDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SmellyCat: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SmellyCat: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SmellyCat: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SmellyCat: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SmellyCat: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SmellyCat: Cannot update gasForProcessing to same value");
