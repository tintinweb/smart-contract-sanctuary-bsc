...
contract PresaleGenerator is Ownable {
    ...
    constructor() {
        PRESALE_FACTORY = IPresaleFactory(0x535127D7d2f41c4C31fcB524edf4F2b87bC9d030);
        PRESALE_SETTINGS = IPresaleSettings(0xe04FDE6F56B022B3103d5DBD5b68Ea61167C09c7);
    }
    ...
    function createPresale (
      ...
      uint256[11] memory uint_params
        ...
        params.earlyAllowanceRate = uint_params[5];
        ...
        params.liquidityPercent, params.listingRate, params.startTime, params.endTime, params.lockPeriod);
...
import "./interfaces/IPresaleSettings.sol";
import "./BotProtector.sol";


contract Presale is ReentrancyGuard, BotProtector {
  using SafeMath for uint256;
  ...
  IPYESwapFactory public PYE_SWAP_FACTORY;
  IWETH public WETH;
  mapping(address => BuyerInfo) public BUYERS;
  EnumerableSet.AddressSet private ALLOWLIST;
  mapping (address => uint256) private ALLOWLIST_LEVEL;
  ...
  constructor(address _presaleGenerator) {
    ...
    PYE_SWAP_FACTORY = IPYESwapFactory(0x130F7f261B086204c565c7c45025CAC128a88612);
    WETH = IWETH(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);
    PRESALE_SETTINGS = IPresaleSettings(0xe04FDE6F56B022B3103d5DBD5b68Ea61167C09c7);
    PRESALE_LOCK_FORWARDER = IPresaleLockForwarder(0x87E027aFb65fE870c027aA6C1bd624b80986adfE);
  ...
  function checkAllowed(address account) public view returns (bool) {
    ...
    if (STATUS.TOTAL_BASE_COLLECTED < EARLY_ACCESS_ALLOWANCE){
      if (block.timestamp >= PRESALE_INFO.START_TIMESTAMP - STATUS.LEVEL_1_ROUND_LENGTH) { // 30 minutes
        allowed = accessLevel >= 1;
      } else if (block.timestamp >= PRESALE_INFO.START_TIMESTAMP - STATUS.LEVEL_2_ROUND_LENGTH) { // 1 hour
        allowed = accessLevel >= 2;
      } else if (block.timestamp >= PRESALE_INFO.START_TIMESTAMP - STATUS.LEVEL_3_ROUND_LENGTH) { // 2 hour
        allowed = accessLevel >= 3;
      } else if (block.timestamp >= PRESALE_INFO.START_TIMESTAMP - STATUS.LEVEL_4_ROUND_LENGTH) { // 3 hour
  ...
  function userDeposit (uint256 _amount) external payable nonReentrant {
    ...
    _beforeUserDeposit(msg.sender, _amount);
  ...
  function forceFailIfPairExists () external {
    ...
    if (PRESALE_LOCK_FORWARDER.PYELabPairIsInitialised(address(PRESALE_INFO.S_TOKEN), address(PRESALE_INFO.B_TOKEN))) {
  ...
  function addLiquidity() external onlyPresaleOwner nonReentrant {
    ...
    uint256 baseLiquidity = STATUS.TOTAL_BASE_COLLECTED.sub(pyeLABBaseFee).mul(PRESALE_INFO.LIQUIDITY_PERCENT).div(1000);
    if (PRESALE_INFO.PRESALE_IN_ETH) {
    ...
    if (PRESALE_FEE_INFO.REFERRAL_FEE_ADDRESS != address(0)) {
        ...
        // Token fee
        uint256 referralTokenFee = pyeLABTokenFee.mul(PRESALE_FEE_INFO.REFERRAL_FEE).div(1000);
        TransferHelper.safeTransfer(address(PRESALE_INFO.S_TOKEN), PRESALE_FEE_INFO.REFERRAL_FEE_ADDRESS, referralTokenFee);
        pyeLABTokenFee = pyeLABTokenFee.sub(referralTokenFee);
    }
...
contract BotProtector is Context, Ownable {

    uint256 private _maxTxAmount;
    uint256 private _coolDownTime = 15 minutes;
    ...
    function _beforeUserDeposit(address _buyer, uint256 _amount) internal {
        if (_buyer != owner()) {
            ...
            require(buyCoolDown[_buyer] < block.timestamp, "PYELab Bot Protector: can't buy until cool down");

            if(_amount > _maxTxAmount) {
                _amount = _maxTxAmount;
                buyCoolDown[_buyer] = block.timestamp + _coolDownTime;
            }
        }
    }
    ...
    function setCoolDownTime(uint256 _amount) external onlyOwner {
        _coolDownTime = _amount;
    }
