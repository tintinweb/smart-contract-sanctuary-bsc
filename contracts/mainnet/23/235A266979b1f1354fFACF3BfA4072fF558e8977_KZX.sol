...
contract KZX is IERC20, Ownable {
    ...
    event UpdateLotteryFee(
        uint16 sellLotteryFee,
        uint16 buyLotteryFee
    );
    event UpdateRewardFee(
        uint16 sellRewardFee,
        uint16 buyRewardFee
    );
    event UpdateLotteryWallet(
        address lotteryWallet,
        bool isBNBForLotteryFee
    );
    ...
    function _transferStandard(
        ...
        _takeLiquidity(tLiquidity, tLottery, tBurn);
        ...
        )
    {
        ...
        rTransferAmount=rAmount-rFee-tLiquidity*currentRate-tLottery*currentRate-tBurn*currentRate;
    }
    ...
    function _takeLiquidity(uint256 tLiquidity, uint256 tLottery, uint256 tBurn) private {
        ...
        _rOwned[address(0xdead)]=_rOwned[address(0xdead)]+tBurn*currentRate;
        _tOwned[address(0xdead)]=_tOwned[address(0xdead)]+tBurn;
        _rOwned[address(this)] = _rOwned[address(this)]+rLiquidity+rLottery;
        if (_isExcluded[address(this)])
            _tOwned[address(this)] = _tOwned[address(this)]+tLiquidity+tLottery;
    }
    ...
    function updateLiquidityFee(
        ...
        require(
            _sellLiquidityFee+sellLotteryFee+sellRewardFee+sellBurnFee <= 300,
            "sell fee <= 30%"
        );
        require(
    ...
    function updateLotteryFee(
        uint16 _sellLotteryFee,
        uint16 _buyLotteryFee
    ) external onlyOwner {
        require(
            _sellLotteryFee+sellLiquidityFee+sellRewardFee+sellBurnFee <= 300,
            "sell fee <= 30%"
        );
        require(
    ...
    function updateRewardFee(
        ...
        require(
            _sellRewardFee+sellLiquidityFee+sellLotteryFee+sellBurnFee <= 300,
            "sell fee <= 30%"
        );
        require(
    ...
    function updateBurnFee(
        ...
        require(
            sellRewardFee+sellLiquidityFee+sellLotteryFee+_sellBurnFee <= 300,
            "sell fee <= 30%"
        );
        require(
    ...
    function takeFee() private lockTheSwap {
        ...
        if (isBNBForLotteryFee) {
            swapTokensForBNB(tokensForLiquidity+_lotteryFeeTokens);
            uint256 bnbBalance = address(this).balance-initialBNBBalance;
            uint256 bnbForLottery = bnbBalance*(_lotteryFeeTokens)/(
                tokensForLiquidity+_lotteryFeeTokens
            );
            bnbForLiquidity = bnbBalance - bnbForLottery;

            (bool success, )=address(lotteryWallet).call{value: bnbForLottery}("");
            if(success)
                _lotteryFeeTokens = 0;
