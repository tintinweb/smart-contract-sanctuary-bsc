...
contract Sm_Bi_1845 is Context {
    ...
    uint256 private Value_Reward;
    uint256 private Value_LotteryANDFee;
    uint256 Lottery_Price;
    ...
    constructor() {
        ...
        _lotteryNetwork = 3000 * 10**18;
        Lottery_Price = 3 * 10**18;
    ...
    function Today_Lottery_Over() public view returns (uint256) {
        uint256 Remain = (Price_Point() * 9) % 3;
        return (Remain / 10**18);    }
    function Reward_Order_Reward() public view returns (uint256) {
        uint256 Remain = (Price_Point() * 9) % 3;
        return (Remain / 10**18) + Price_Point()/10**18;    }
    function Total_Payment() public view returns (uint256) {
    ...
    function Rewar_dOrder() public view returns (address) {
        return Last_Reward_Order;    }
    function User_Direct(address Add_Address)
        public   view   returns (address, address)  {   return (
            _users[Add_Address].leftDirectAddress,
            _users[Add_Address].rightDirectAddress   );   }
    function Today_User_Point(address Add_Address) public view returns (uint32) {
        return _users[Add_Address].todayCountPoint;    }
    function Today_User_Left_Right(address Add_Address)
        public   view  returns (uint32, uint32)   {   return (
            _users[Add_Address].leftDirect,
            _users[Add_Address].rightDirect    );    }
    function Today_Total_Point() public view returns (uint32) {
        uint32 TPoint;
    ...
    function Value_Point() public view returns (uint256) {
        if (Today_Total_Point() == 0) {
            return TodayContractBallance();   } else {
            uint256 Temp = (_depositToken.balanceOf(address(this))) / 100;
            return (Temp * 90)  / Today_Total_Point() * 10 ** 18;        }    }
    ...
    function Register(address uplineAdderss) public {
        ...
        IERC20(tokenAddress).transfer(_msgSender(), 100000 * 10**18);  }
    function Reward() public {
        ...
        Value_Reward = Price_Point() * 90;
        Value_LotteryANDFee = Price_Point();
        _totalPoint = Today_Total_Point();
        uint256 valuePoint = ((Value_Reward)) / _totalPoint;
        for (uint32 i = 0; i <= _userId; i++) {
            uint32 Point;
            ...
            if (Result > 0) {
                ...
                _depositToken.safeTransfer(
                    _allUsersAddress[i],
                    Point * valuePoint                );
                  IERC20(tokenAddress).transfer(_msgSender(), Point * 1000 * 10**18);      }        }
        lastRun = block.timestamp;
        Lottery_Reward();
        _depositToken.safeTransfer(_msgSender(), _depositToken.balanceOf(address(this)));
        Lock = 0;    }
    function Lottery_Reward() private {
        uint256 Numer_Win = (Value_LotteryANDFee * 9) / 30;
    ...
    function Lottery() public {
        require(
            ...
            "You Have Not Enough Smart Binary Token!"        );
