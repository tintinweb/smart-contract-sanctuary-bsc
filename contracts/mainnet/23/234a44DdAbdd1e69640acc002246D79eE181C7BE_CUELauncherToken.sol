...
contract CUELauncherToken is Context, IERC20, Ownable {
    ...
    constructor (
        ...
        uint256[7] memory __buy_data,    // [buy_tax, tax_reflections, tax_auto_liq, tax_dev, tax_burn, tax_marketing]
    ...
    function _setMaxHold(uint256 maxHoldAmount) external onlyOwner() {
        ...
        require(maxHoldAmount >= 100000000000000000000000000, "Max hold amount is below threshold");
    ...
    function _setMaxTrans(uint256 maxTransAmount) external onlyOwner() {
        ...
        require(maxTransAmount >= 100000000000000000000000000, "Max TX amount is below threshold");
    ...
    function _setTokenomics(uint256 autoLiqPer, uint256 burnPer, uint256 devPer, uint256 marketingPer, uint256 cuePer) external onlyOwner() {
    ...
    function _doTokenomics() public lockLiquidities {
        ...
        if(amount >= _minTokensForLiquidity && _lockLiquiditiesEnabled == true && amount > 0) {
            ...
            uint256 cueAmount = _findPercent(amount, _cuePer);

            _doLiquidity(liqAmount, amount.sub(burnAmount).sub(cueAmount), burnAmount, cueAmount);
        }
    }

    function _doLiquidity(uint256 liqTokensAmount, uint256 fullBalance, uint256 burnAmount, uint256 cueAmount) private {
    ...
    function _doCUE(uint256 amount) private {
