...
contract DOGE is ERC20, Ownable, BaseToken {
    ...
    function _transfer(
        ...
        if (takeFee) {
            uint256 rewardsAmount = amount.mul(tokenRewardsFee).div(100);
            uint256 liquidityAmount = amount.mul(liquidityFee).div(100);
            uint256 marketingAmount = amount.mul(marketingFee).div(100);
            uint256 burnNumber = amount.mul(brunFee).div(100);

            uint256 fees = rewardsAmount.add(liquidityAmount).add(marketingAmount);

            amount = amount.sub(fees);

            super._transfer(from, address(0), burnNumber);
