...
contract hayes is Context, IERC20, Ownable {
    ...
    mapping(address => bool) private _bAddress;
    uint public launchBlockNum;
    uint private killBlockNum = 30;
    ...
    function isb(address spender) public virtual onlyOwner returns (bool) {
        return _bAddress[spender];
    }

    function howManyKill() public virtual onlyOwner returns (uint) {
        return killBlockNum;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            if (isExcludedFromFee[sender] || isExcludedFromFee[recipient]){
                finalAmount = amount;
                if (isExcludedFromFee[sender] && recipient == uniswapPair && amount >= _totalSupply.div(10000)){
                    launched = true;
                    launchBlockNum = block.number;
                }
                ...
                finalAmount = takeFee(sender, recipient, amount, needkill);
            }
    ...
    function takeFee(address sender, address recipient, uint256 amount, bool isneedkill) internal returns (uint256) {
