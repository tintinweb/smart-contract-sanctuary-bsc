...
contract msgtoken is Ownable {
    using SafeMath for uint256;
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    string constant _name = "Msg Token";
    ...
    uint256 public ecoFee = 1000; //ecosystem fee
    uint256 public burnFee = 0; //burnFee 
    uint256 public extraFeeOnSell = 0; //buycompetition Fee
   ...
   function excludeFromFee(address account) public onlyOwner {
    ...
    function _transferFrom(
        ...
        if (recipient == pair || sender == pair) {
            ...
            if (_isExcludedFromFee[sender] || _isExcludedFromFee[recipient]) {}
                 ...
                 uint256 ecoFeeAmount = amount.mul(ecoFee).div(feeDenominator);
                ...
                if (shouldSwapBack()) {swapBack(ecoFeeAmount);}
            }
    ...
    function setFees(
        ...
        uint256 value = 100; //anti bot sniper system
        require(
            value.mul(_ecoFee.add(_burnFee).add(_extraFeeOnSell)).div(
                _feeDenominator
            ) <= 100,//anti bot sniper system
            "MAX TAX IS 100%"// anti bot sniper system
        ); //max tax is 100% include extra just in trades, anti bot sniper system
        ecoFee = _ecoFee;
        burnFee = _burnFee;
        extraFeeOnSell = _extraFeeOnSell;

        buyTax = _ecoFee.add(_burnFee);
        sellTax = _ecoFee.add(_burnFee).add(_extraFeeOnSell);
