...
contract Acoge is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "Acoge Token";
    string private _symbol = "Acoge";
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(isMarketPair[sender]) {
            ...
            feeAmount = amount.mul(_transferFee).div(100);
             _balances[burnWalletAddress] = _balances[burnWalletAddress].add(feeAmount);
            emit Transfer(sender, burnWalletAddress, feeAmount);
        }
