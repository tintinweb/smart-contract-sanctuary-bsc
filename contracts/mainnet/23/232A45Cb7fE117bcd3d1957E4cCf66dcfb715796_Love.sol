...
contract Love is Ownable ,Context ,IERC20 ,IERC20Metadata{
    ...
    function removeWhiteList(address account)public onlyOwner returns(bool){
     ...
     function trunOnLock()public onlyOwner returns(bool){
        slipLock=true;
        return  slipLock;
    }
    
    function trunOffLock()public onlyOwner returns(bool){
        slipLock=false;
        return  slipLock;
    }
...
//  isContract(recipient)
        ...
        if(  slipLock&&isContract(recipient)&&_whiteList[sender]==false){  
