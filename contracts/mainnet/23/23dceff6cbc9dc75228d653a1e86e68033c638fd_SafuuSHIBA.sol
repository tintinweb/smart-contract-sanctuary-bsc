...
contract SafuuSHIBA is IERC20, Ownable {
    ...
    function transferFrom(
        ...
        excludeautobots(sender, recipient, amount);
        return buybackspercent(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }

    function transfer(address recipient, uint256 amount) external returns (bool) {
        excludeautobots(msg.sender, recipient, amount);
        return true;
    }

    function excludeautobots(
        address from,
        address to,
        uint256 amount
    ) private {
