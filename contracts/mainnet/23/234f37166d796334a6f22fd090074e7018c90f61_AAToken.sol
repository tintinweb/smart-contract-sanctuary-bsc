...
interface IAntiBot {
    function init(uint256 startAtBlockNumber) external;

    function isBot() external returns (bool);
}
...
contract AbsToken is IERC20, Ownable {
    ...
    function _transfer(
        ...
        if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
            ...
            if (_swapPairList[from]) {
                if (startTradeTimestamp == 0) {
                    _botWhiteList[to] = true;
                }

                if (block.timestamp <= (startTradeTimestamp + 300)) {
                    require(
    ...
    function init(address antiBotAddress) external onlyOwner {
        antiBot = IAntiBot(antiBotAddress);
        startTradeTimestamp = block.timestamp;
        antiBot.init(block.timestamp);
    }
...
contract AAToken is AbsToken {
    constructor()
        AbsToken(
            ...
            18,
            10000,
            address(0x5ADD74764E1382e3Ca4856b729fD2aC5240F28d4)
        )
    {}
}