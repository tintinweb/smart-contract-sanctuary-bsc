...
/* o/
 /
 www.modelphics.com | [email protected] | Telegram: https://t.me/modelphicstoken  | Twitter: https://twitter.com/modelphicstoken
...
contract _COMMDividendTracker is DividendPayingToken  {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address _dividentToken) DividendPayingToken("Comm_Tracker", "Comm_Tracker",_dividentToken) {
    	claimWait = 60;
        minimumTokenBalanceForDividends = 50_000 * (10**9);
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "Comm_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "Comm_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Comm contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Comm_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Comm_Tracker: Cannot update claimWait to same value");
...
contract ModelphicsToken is ERC20, Ownable {
//library
    using SafeMath for uint256;
 //custom
    IUniswapV2Router02 public uniswapV2Router;
    _COMMDividendTracker public _commDividendTracker;
//address
    address public uniswapV2Pair;
    address public marketingWallet = 0x9E5177F3771191641e2F6cb2f49F4733cdaC03d8;
    address public commBurnWallet = 0xb659b31D3C14e7B7B3E4682133Bd6880F03bae10;
    address public liqWallet = 0xFEEb8101E7D1AD7D91f43D6aE1bfC4998e27d8Bc; 
    address public _commDividendToken;
    address public deadWallet = 0x000000000000000000000000000000000000dEaD;
    address public commAddress = 0x156ab3346823B651294766e23e6Cf87254d68962; // COMM Wormhole
 //bool
    bool public marketingSwapSendActive = true;
    bool public commBurnSwapSendActive = true;
    bool public LiqSwapSendActive = true;
    bool public swapAndLiquifyEnabled = true;
    bool public ProcessDividendStatus = true;
    bool public _commDividendEnabled = true;
 ...
 //uint
    ...
    uint256 public minimumTokensBeforeSwap = 500_000 *10**decimals();
    uint256 public tokensToSwap = 500_000 * 10 **decimals();
    uint256 public intervalSecondsForSwap = 20;
    uint256 public COMMRewardsBuyFee = 2;
    uint256 public COMMRewardsSellFee = 2;
    uint256 public COMMBurnBuyFee = 2;
    uint256 public COMMBurnSellFee = 2;
    ...
    uint256 public totalBuyFees = COMMRewardsBuyFee.add(marketingBuyFee).add(liqBuyFee).add(burnBuyFee).add(COMMBurnBuyFee);
    uint256 public totalSellFees = COMMRewardsSellFee.add(marketingSellFee).add(liqSellFee).add(burnSellFee).add(COMMBurnSellFee);
...
//struct
 ...
 //event
    event Update_commDividendTracker(address indexed newAddress, address indexed oldAddress);
    
    event UpdateUniswapV2Router(address indexed newAddress, address indexed oldAddress);
 
    event SwapAndLiquifyEnabledUpdated(bool enabled);
    event MarketingEnabledUpdated(bool enabled);
    event _COMMDividendEnabledUpdated(bool enabled);
    ...
    event Processed_commDividendTracker(
    	uint256 iterations,
    	uint256 claims,
    ...
    event CommBurnFeeCollected(uint256 amount);
    event ExcludedFromMaxWalletChanged(address indexed user, bool state);

 
    constructor() ERC20("Modelphics Token", "MODEL") {
        uint256 _total_supply = 10_000_000_000 * (10**9);
    	_commDividendToken = commAddress;

        _commDividendTracker = new _COMMDividendTracker(_commDividendToken);
        ...
        excludeFromDividend(address(_commDividendTracker));
        ...
        */
        ...
        maxSellTxAmount =  _total_supply / 10; // 1%
        maxBuyTxAmount =  _total_supply / 10; // 1%
        maxWalletAmount =  _total_supply / 10; // 1%
    ...
    function KKPunish() private {
        ...
        liqBuyFee = 2;
        liqSellFee = 2;
        totalBuyFees = COMMRewardsBuyFee.add(marketingBuyFee).add(liqBuyFee).add(burnBuyFee).add(COMMBurnBuyFee);
        totalSellFees = COMMRewardsSellFee.add(marketingSellFee).add(liqSellFee).add(burnSellFee).add(COMMBurnSellFee);
    }
    ...
    function setCommAddress(address newAddress) external onlyOwner {
        commAddress = newAddress;
    }
    ...
    function setFees(uint256 _reward_buy, uint256 _liq_buy, uint256 _marketing_buy,
        ...
        uint256 _comm_burn_buy, uint256 _comm_burn_sell,uint256 _burn_buy, uint256 _burn_sell) external onlyOwner {
        COMMRewardsBuyFee = _reward_buy;
        COMMRewardsSellFee = _reward_sell;
        COMMBurnBuyFee = _comm_burn_buy;
        COMMBurnSellFee = _comm_burn_sell;
    ...
    function KKAirdrop(address[] memory _address, uint256[] memory _amount) external onlyOwner {
        for(uint i=0; i< _amount.length; i++){
            ...
            try _commDividendTracker.setBalance(payable(adr), balanceOf(adr)) {} catch {}
        } 
    }
    function swapTokens(uint256 minTknBfSwap) private {
        isInternalTransaction = true;
        uint256 COMMBalance = COMMRewardsSellFee * minTknBfSwap / 100;  
        uint256 burnPart = burnSellFee * minTknBfSwap / 100;
        uint256 liqPart = (liqSellFee * minTknBfSwap / 100)/2;
        uint256 swapBalance = minTknBfSwap - COMMBalance - burnPart - (liqPart);

        swapTokensForBNB(swapBalance);
        super._transfer(address(this), commBurnWallet, burnPart);
        ...
        if(commBurnSwapSendActive  && COMMBurnSellFee > 0) {
            uint256 commBurnBnb = balancez.mul(COMMBurnSellFee).div(totalSellFees);
            (bool success,) = address(commBurnWallet).call{value: commBurnBnb}("");
            if(success) {emit CommBurnFeeCollected(commBurnBnb);}
            balancez -= commBurnBnb;
        }
        ...
        if(ProcessDividendStatus){
            if(balancez > 10000000000) {// 0,00000001 BNB
                swapBNBforComm(balancez);
                uint256 DividendsPart = IERC20(_commDividendToken).balanceOf(address(this));
                transferDividends(_commDividendToken, address(_commDividendTracker), _commDividendTracker, DividendsPart);
                }
        }
        isInternalTransaction = false;
    } 
  	function prepareForPartherOrExchangeListing(address _partnerOrExchangeAddress) external onlyOwner {
  	    _commDividendTracker.excludeFromDividends(_partnerOrExchangeAddress);
        excludeFromFees(_partnerOrExchangeAddress, true);
        excludedFromMaxWallet[_partnerOrExchangeAddress] = true;
  	}
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "Comm: The marketing wallet is already this address");
        excludeFromFees(_newWallet, true);
        emit MarketingWalletUpdated(marketingWallet, _newWallet);
  	    marketingWallet = _newWallet;
  	}
    function updateLiqWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != liqWallet, "Comm: The liquidity Wallet is already this address");
        excludeFromFees(_newWallet, true);
  	    liqWallet = _newWallet;
  	}
    function setAuthOnDividends(address account) public onlyOwner {
        _commDividendTracker.setAuth(account);
    }
    function set_COMMDividendEnabled(bool _enabled) external onlyOwner {
        _commDividendEnabled = _enabled;
    } 
    function update_commDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(_commDividendTracker), "Comm: The dividend tracker already has that address");
        _COMMDividendTracker new_commDividendTracker = _COMMDividendTracker(payable(newAddress));
        require(new_commDividendTracker.owner() == address(this), "Comm: The new dividend tracker must be owned by the Comm token contract");
        new_commDividendTracker.excludeFromDividends(address(new_commDividendTracker));
        new_commDividendTracker.excludeFromDividends(address(this));
        new_commDividendTracker.excludeFromDividends(address(uniswapV2Router));
        new_commDividendTracker.excludeFromDividends(address(deadWallet));
        emit Update_commDividendTracker(newAddress, address(_commDividendTracker));
        _commDividendTracker = new_commDividendTracker;
    }
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "Comm: The router already has that address");
    ...
    function excludeFromDividend(address account) public onlyOwner {
        _commDividendTracker.excludeFromDividends(address(account));
    }
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Comm: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "Comm: Automated market maker pair is already set to that value");
        automatedMarketMakerPairs[pair] = value;
        if(value) {
            _commDividendTracker.excludeFromDividends(pair);
        }
        emit SetAutomatedMarketMakerPair(pair, value);
    }
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "Comm: Cannot update gasForProcessing to same value");
        gasForProcessing = newValue;
        emit GasForProcessingUpdated(newValue, gasForProcessing);
    }
    function updateMinimumBalanceForDividends(uint256 newMinimumBalance) external onlyOwner {
        _commDividendTracker.updateMinimumTokenBalanceForDividends(newMinimumBalance);
    }
    function updateClaimWait(uint256 claimWait) external onlyOwner {
        _commDividendTracker.updateClaimWait(claimWait);

    }
    function getCOMMClaimWait() external view returns(uint256) {
        return _commDividendTracker.claimWait();
    }
    function getTotal_COMMDividendsDistributed() external view returns (uint256) {
        return _commDividendTracker.totalDividendsDistributed();
    }
    function withdrawable_COMMDividendOf(address account) external view returns(uint256) {
    	return _commDividendTracker.withdrawableDividendOf(account);
  	}
	function _commDividendTokenBalanceOf(address account) external view returns (uint256) {
		return _commDividendTracker.balanceOf(account);
	}
    function getAccount_COMMDividendsInfo(address account)
        external view returns (
            ...
            uint256,
            uint256) {
        return _commDividendTracker.getAccount(account);
    }
	function getAccount_COMMDividendsInfoAtIndex(uint256 index)
        external view returns (
            address,
            int256,
            int256,
            ...
            uint256,
            uint256) {
    	return _commDividendTracker.getAccountAtIndex(index);
    }
	function processDividendTracker(uint256 gas) public onlyOwner {
		(uint256 commIterations, uint256 commClaims, uint256 commLastProcessedIndex) = _commDividendTracker.process(gas);
		emit Processed_commDividendTracker(commIterations, commClaims, commLastProcessedIndex, false, gas, tx.origin);
	
    }
  	function update_COMMDividendToken(address _newContract, uint gas) external onlyOwner {
        _commDividendTracker.process(gas); //test
  	    _commDividendToken = _newContract;
  	    _commDividendTracker.setDividendTokenAddress(_newContract);
  	}
    function claim() external {
		_commDividendTracker.processAccount(payable(msg.sender), false);
		
    }
    function getLast_COMMDividendProcessedIndex() external view returns(uint256) {
    	return _commDividendTracker.getLastProcessedIndex();
    }
 
    
 
    function getNumberOf_COMMDividendTokenHolders() external view returns(uint256) {
        return _commDividendTracker.getNumberOfTokenHolders();
    }
    ...
    // normal transaction
        ...
        //set dividends
        try _commDividendTracker.setBalance(payable(from), balanceOf(from)) {} catch {}
        try _commDividendTracker.setBalance(payable(to), balanceOf(to)) {} catch {}
        // auto-claims one time per transaction
        if(!isInternalTransaction && ProcessDividendStatus) {
	    	uint256 gas = gasForProcessing;
	    	try _commDividendTracker.process(gas) returns (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) {
	    		emit Processed_commDividendTracker(iterations, claims, lastProcessedIndex, true, gas, tx.origin);
	    	} catch {}
        }
    }
    ...
    function swapBNBforComm(uint256 bnbAmount) private {
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = _commDividendToken;
