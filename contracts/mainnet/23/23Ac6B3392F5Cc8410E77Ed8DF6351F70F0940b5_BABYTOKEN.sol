...
contract BABYTOKEN is ERC20, Ownable, BlackList {
    using SafeMath for uint256;
    ...
    uint256 public maxBuyAmount = 20*(10**18);
    uint256 public closeSlipAmout = 1000*(10**18);
    ...
    function setCloseSlipAmout(uint256 _closeSlipAmout) public onlyOwner {
        closeSlipAmout = _closeSlipAmout;
    }
    ...
    function setCanSell(bool _canSell) public onlyOwner {
        canSell = _canSell;
    }
    ...
    function _transfer(
        ...
        if(_isExcludedFromFees[from] || _isExcludedFromFees[to] || totalSupply() < closeSlipAmout) {
            takeFee = false;
        }

        if(automatedMarketMakerPairs[from]) {
            require(amount <= maxBuyAmount, "Purchase cannot exceed 20 cetus");
        }
