...
contract Prediction is Ownable, Pausable, ReentrancyGuard {
    ...
    function getSnapshot() external view returns (Snapshot memory) {
        Round memory lastRound;
        if (currentEpoch > 0) lastRound = rounds[currentEpoch - 1];

        return Snapshot({
            token: token,
            oracle: oracle,
            paused: paused(),
            genesisLockOnce: genesisLockOnce,
            genesisStartOnce: genesisStartOnce,
            ...
            oracleLatestRoundId: oracleLatestRoundId,
            oracleUpdateAllowance: oracleUpdateAllowance,
            lastRound: lastRound,
            currentRound: rounds[currentEpoch]
        });
    }
}