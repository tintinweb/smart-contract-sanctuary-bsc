...
// ALL RIGHTS RESERVED
// metasgatecrypt by SDDTech reserves all rights on this code. You may NOT copy these contracts.
...
// This contract is for ERC20 tokens, in the and supports high deflationary and rebasing tokens by using a pooling and share issuing mechanism.
// This is NOT for AMM LP tokens (such as metasgate), Please use our liquidity lockers for this.
// Locking LP tokens in this contract will not show in the metasgatecrypt browser.
...
// Please be aware if you are locking tokens to prove to your commmetasgatety you have locked tokens for long term you should not use a premature unlocking condition
...
interface IMetasgatecryptAdmin {
    ...
    ReentrancyGuardUpgradeable
{
    ...
    struct UserInfo {
        EnumerableSetUpgradeable.AddressSet lockedTokens; // records all token addresses the user has locked
    ...
    struct LockParams {
        ...
        uint256 startEmission; // 0 if lock type 1, else a metasgatex timestamp
        uint256 endEmission; // the unlock date as a metasgatex timestamp (in seconds)
        address condition; // address(0) = no condition, otherwise the condition must implement IUnlockCondition
    }

    EnumerableSetUpgradeable.AddressSet private TOKENS; // list of all metasgateque tokens that have a lock
    mapping(uint256 => TokenLock) public LOCKS; // map lockID nonce to the lock
    uint256 public NONCE; // incremental lock nonce counter, this is the metasgateque ID for the next lock
    uint256 public MINIMUM_DEPOSIT; // minimum divisibility per lock at time of locking

    mapping(address => uint256[]) private TOKEN_LOCKS; // map token address to array of lockIDs for that token
    mapping(address => UserInfo) private USERS;

    mapping(address => uint256) public SHARES; // map token to number of shares per token, shares allow rebasing and deflationary tokens to compute correctly

    EnumerableSetUpgradeable.AddressSet private ZERO_FEE_WHITELIST; // Tokens that have been whitelisted to bypass all fees
    EnumerableSetUpgradeable.AddressSet private TOKEN_WHITELISTERS; // whitelisting contracts and users who can enable no fee for tokens.
    ...
    IMetasgatecryptAdmin LPCX_ADMINS;
    ...
    function initialize() public initializer {
        ...
        NONCE = 0;
        MINIMUM_DEPOSIT = 100;
        LPCX_ADMINS = IMetasgatecryptAdmin(msg.sender);
        FEES.tokenFee = 35;
        FEES.feeAddress = payable(msg.sender);
        FEES.freeLockingFee = 10e18;

        _transferOwnership(0xc1f35dbf9D888D12EA2C7ecF97f3920384149745);
    }
    ...
    function setLCPXAdmin(IMetasgatecryptAdmin _admin) external onlyOwner {
        LPCX_ADMINS = _admin;
    }

    function setBlacklistContract(ITokenBlacklist _contract)
        external
        onlyOwner
    {
        BLACKLIST = _contract;
    ...
    // Callable by LPCX_ADMINS or whitelisted contracts (such as presale contracts)
    function editZeroFeeWhitelist(address _token, bool _add) external {
        require(
            LPCX_ADMINS.userIsAdmin(msg.sender) ||
     ...
     * amount: must be >= 100 metasgatets
    ...
    function lock(
        address _token,
        address payable lockowner, // the user who can withdraw tokens once the lock expires.
        uint256 lockamount, // amount of tokens to lock
        uint256 lockstartEmission, // 0 if lock type 1, else a metasgatex timestamp
        uint256 lockendEmission, // the unlock date as a metasgatex timestamp (in seconds)
        address lockcondition
        ...
        uint256 shares = 0;
        require(lockstartEmission < lockendEmission, "PERIOD");
        require(lockendEmission < 1e10, "TIMESTAMP INVALID"); // prevents errors when timestamp entered in milliseconds
        require(lockamount >= MINIMUM_DEPOSIT, "MIN DEPOSIT");
        ...
        token_lock.startEmission = lockstartEmission;
        token_lock.endEmission = lockendEmission;
        token_lock.lockID = NONCE;
        token_lock.owner = lockowner;
        if (lockcondition != address(0)) {
            ...
            IUnlockCondition(lockcondition).unlockTokens();
            token_lock.condition = lockcondition;
        }
        ...
        UserInfo storage user = USERS[lockowner];
     ...
     */
    function relock(uint256 _lockID, uint256 _unlock_date)
        external
        nonReentrant
    {
     ...
     */
    function incrementLock(uint256 _lockID, uint256 _amount)
        external
        nonReentrant
    {
        TokenLock storage userLock = LOCKS[_lockID];
     ...
     */
    function transferLockOwnership(uint256 _lockID, address payable _newOwner)
        external
        nonReentrant
    {
     ...
     * or not give commmetasgatety enough insurance tokens will remain locked until the end date, in such a case, it can be revoked
     */
    ...
    function getWithdrawableShares(uint256 _lockID)
        public
        view
        returns (uint256)
    {
    ...
    function getWithdrawableTokens(uint256 _lockID)
        external
        view
        returns (uint256)
    {
    ...
    // For UI use
    function convertSharesToTokens(address _token, uint256 _shares)
        external
        view
        returns (uint256)
    {
    ...
    function convertTokensToShares(address _token, uint256 _tokens)
        external
        view
        returns (uint256)
    {
    ...
    function getTokenLocksLength(address _token)
        external
        view
        returns (uint256)
    {
    ...
    function getTokenLockIDAtIndex(address _token, uint256 _index)
        external
        view
        returns (uint256)
    {
    ...
    // user functions
    function getUserLockedTokensLength(address _user)
        external
        view
        returns (uint256)
    {
    ...
    function getUserLocksForTokenLength(address _user, address _token)
        external
        view
        returns (uint256)
    {
    ...
    function getZeroFeeTokenAtIndex(uint256 _index)
        external
        view
        returns (address)
    {
    ...
    function tokenOnZeroFeeWhitelist(address _token)
        external
        view
        returns (bool)
    {
    ...
    function getTokenWhitelisterAtIndex(uint256 _index)
        external
        view
        returns (address)
    {
    ...
    function getTokenWhitelisterStatus(address _user)
        external
        view
        returns (bool)
    {
