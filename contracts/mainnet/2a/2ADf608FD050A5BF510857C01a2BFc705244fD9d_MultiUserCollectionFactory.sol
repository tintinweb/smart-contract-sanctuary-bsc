...
/**
 * @title MultiUserCollectionFactory
 * @dev Anyone can create a collection from the factory
 */
contract MultiUserCollectionFactory is Ownable, ReentrancyGuard {
    // The rigistry for user's created collections.
    MultiUserCollection[] collections;
    mapping(address => MultiUserCollection[]) collectionByOwner;

    event CreateERC1155Factory(
    ...
    function createCollection(
        ...
        MultiUserCollection newCollection = new MultiUserCollection(
            name,
            symbol,
            contractURI,
            tokenURIPrefix
    ...
    function getAllCollections()
        external
        view
        returns (MultiUserCollection[] memory)
    {
        return collections;
    }
...
contract MultiCollection is Ownable, SignerRole, ERC1155Base {
    ...
    function mint(uint256 id, uint8 v, bytes32 r, bytes32 s, Fee[] memory fees, uint256 supply, string memory uri, uint256 nonce) public {
...
contract MultiUserCollection is MultiCollection {
    ...
    ///        The URI must link to JSON object with various fields: `name, description, image, external_url, attributes`.
    ///        Can also contain another various fields.
    function mint(uint256 id, Fee[] memory fees, uint256 supply, string memory uri, uint256 nonce) onlyOwner public {
        require(!usedNonces[nonce], "Nonce should be valid");
        usedNonces[nonce] = true;
        _mint(id, fees, supply, uri);
        if (id > maxTokenId)
