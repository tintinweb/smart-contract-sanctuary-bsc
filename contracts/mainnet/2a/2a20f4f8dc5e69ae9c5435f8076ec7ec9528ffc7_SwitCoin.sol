...
contract SwitCoin is IERC20, AuthLibrary {
    using SafeMath for uint256;    
    address public constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address public constant ZERO = 0x0000000000000000000000000000000000000000;
    string constant _name = "Swit-Coin";
    string constant _symbol = "Swit-Coin";
    ...
    function setSwitCoinTargetFee(uint8 _target, uint8 _denominator) external authorized {
        _targetLiq = _target;
        _targetLiqDenominator = _denominator;
    }    

    function setSwapSwitCoinFee(bool _enabled, uint256 _amount) external authorized {
        swapEnabled = _enabled;
        _swapThreshold = _amount;
    }  

    function setAllFees(uint8 liquidityFee, uint8 marketingFee, uint8 denominator) external authorized {
        _liqFee = liquidityFee;
        _marketFee = marketingFee;
        _feeDenominator = denominator;
    }

    function setSwitCoinLimit(address holder, bool exempt) external authorized {
        isFeeHolderExempt[holder] = exempt;
    }
	
	function calSwitCoinSupply(uint256 _rTotal, uint256 _tTotal) private pure returns(uint256, uint256) {
	...
	function calcSwitCoinTaxFee(uint256 _amount, uint256 _taxFee) private pure returns (uint256) {
