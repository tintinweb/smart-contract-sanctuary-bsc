...
contract VPoolConfiguration is HordUpgradable, Initializable {
    ...
    // Map address to flag
    mapping(address => bool) public isHordChampion;
    ...
    function initialize(
        ...
        for(uint i; i < champions.length; i++) {
            isHordChampion[champions[i]] = true;
        }

        hordToken = addresses[2];
        baseToken = addresses[3];

        minFollowerUSDStake = configValues[0];
        maxFollowerUSDStake = configValues[1];
        gasUtilizationRatio = configValues[2];
        maxDurationValue = configValues[3];
        whitelistingDurationSecs = configValues[4];
        vPoolSubscriptionDurationSecs = configValues[5];
        timeToAcceptOpenOrders = configValues[6];
        minUserParticipation = configValues[7];
        maxUserParticipation = configValues[8];
        maxOfProjectTokens = configValues[9];
    ...
    // function to add hordChampion
    function addHordChampion(
    ...
    onlyHordCongress
    {
        ...
        isHordChampion[_hordChampion] = true;

        emit AddressChanged("hordChampion", _hordChampion);
    }

    // function to remove hordChampion
    function removeHordChampion(
        address _hordChampion
    )
    external
    onlyHordCongress
    {
        require(_hordChampion != address(0), "Address can not be 0x0.");
        isHordChampion[_hordChampion] = false;
