...
contract Anubit is OwnableUpgradeable, IERC20, UUPSUpgradeable {
    ...
    */
    ...
    modifier onlyService() {
        require(isService(msg.sender), "!SVC");
        _;
    }
    ...
    function calculateTaxFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(currentFees.refl).div(10**4);
    }

    function calculateBountyFee(uint256 _amount)
        private
        view
        returns (uint256)
    {
        return _amount.mul(currentFees.bounty).div(10**4);
    }
    ...
    function calculateLiquidityFee(uint256 _amount)
        ...
        returns (uint256)
    {
        return _amount.mul(currentFees.liq).div(10**4);
    }
    ...
    function _transfer(
        ...
        if (_antiWhaleProtection) {
            if (to != uniswapV2Pair) {
                ...
                if (
                    _allowedTxAmount[from] == 0 && _sellIntervalStart[from] == 0
                ) {
                    _allowedTxAmount[from] = _maxWhaleTxAmount;
    ...
    function getAnubitsForBNBFromSwap(address recipient, uint256 amount)
        private
    {
        address[] memory path = new address[](2);
    ...
    function multiLock(address[] memory wallets, bool lockWallet)
        external
        onlyOwner
    {
    ...
    function setMaxWhaleTxAmount(uint256 amount)
        external
        onlyOwner
        returns (uint256)
    {
    ...
    function setWhaleDiv(uint256 div) external onlyOwner returns (uint256) {
        ...
        uint256 bnbBalance = IERC20(uniswapV2Router.WETH()).balanceOf(
            uniswapV2Pair
        );
    ...
    function setMaxWalletAmount(uint256 amount)
        external
        onlyOwner
        returns (uint256)
    {
    ...
    function increaseLockTime(address _lockAccount, uint256 _secondsToIncrease)
        public
        onlyOwner
    {
    ...
    function lockedAccountDetails(address account)
        public
        view
        returns (
            uint256,
    ...
    function distributeBounty(address account, uint256 amount)
        public
        onlyService
    {
    ...
    function bountyAddressChange(address payable bountyAddress)
        external
        onlyOwner
    {
    ...
    function lolaMintSwap(uint256 amount, address bnbRecipient)
        external
        onlyLola
        lockTheSwap
    {
    ...
    function posMerchantAddLocation(address _address, bool _enable)
        external
        onlyPos
    {
