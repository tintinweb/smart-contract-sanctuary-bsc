...
contract MRStakingPoolsV4 is Context, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public minThreshold = 6000 * 10**18; // 6k token


    mapping(address => mapping(uint => Staker)) private stakers;
    ...
    constructor(IERC20 token_, IERC20 rewardToken_, address addr) Ownable(addr) {
        _token = token_;
    ...
    function _distributeDividends() internal {
        ...
        if(rBalance >= minThreshold) {
            ...
            try dividendDistributor.deposit(rBalance) {} catch {}
        }
    }

    function deposit(uint256 amount, uint256 apyTime, uint type_) public {
        uint poolNo = type_;
        uint ran;
        uint256 initialApy;
        ...
        if(isTaxPayable && owner() != _msgSender()) {
            uint256 depositTax = amount.mul(taxPayable).div(10**2);
            amount = amount.sub(depositTax);
        }


        uint256 rAmount = calculateReturn(_msgSender(), poolNo);
        ...
        staker.timeStakedFor = staker.timeStakedFor > block.timestamp ? staker.timeStakedFor.add(_stakeTimes(apyTime)) : _stakeTimes(apyTime);
        ...
        if(!isStaker[_msgSender()][poolNo]){
            addLockCount(poolNo, apyTime, amount);
            stakerSize[poolNo].push(staker);
        } else {
            updateLockCount(poolNo, initialApy, apyTime, initialAmount, amount.sub(rAmount));
    ...
    function _claimReturn(address account, uint poolNo) internal {
        ...
        if(poolShare[poolNo]>=returnAmount){poolShare[poolNo] -= returnAmount;}
    ...
    function updatePool(uint8 poolNo_, string memory rarity_, uint256 stakeSize_, uint256 minStake_, uint256 apyNoLock_, uint256 apy15Day_, uint256 apy30Day_) public onlyOwner {
        ...
        if(poolNo_ > 1 && stakeSize_ > pools[poolNo_].stakeSize && stakerSize[poolNo_].length >= pools[poolNo_].stakeSize) {
