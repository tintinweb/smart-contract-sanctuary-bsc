...
// File: contracts/SpeedcryptoDividendPool.sol
...
contract SpeedcryptoDividendPoolUpgradeable is OwnableUpgradeable, IERC721Receiver {
    ...
    function initialize() public initializer {
        // owner
        __Ownable_init();
        speedNFT = SpeedcryptoNFT(0x26f781728C492ba54C4a903FA3b3F4Ac60C63178);
        bunnyIdToPower[2] = 1;
        bunnyIdToPower[5] = 1;
        bunnyIdToPower[3] = 20;
        bunnyIdToPower[6] = 20;
    ...
    function stake(uint256 _tokenId) public {
        PoolInfo storage pool = getPoolInfo[cycle];
        require(block.number > pool.startBlock && block.number < pool.endBlock, "pool is not open");
        speedNFT.safeTransferFrom(msg.sender, address(this), _tokenId);
        uint256 bunnyId = speedNFT.getBunnyId(_tokenId);
    ...
    function unStake(uint256 _tokenId) public {
        ...
        speedNFT.safeTransferFrom(address(this), msg.sender, _tokenId);
        if (power == 1) {
            pool.srCount -= 1;
            addressInfo.srCount -= 1;
        } else if (power == 20) {
    ...
    function stakeList(uint256[] memory _tokenIds) public {
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            speedNFT.safeTransferFrom(msg.sender, address(this), _tokenIds[i]);
            uint256 bunnyId = speedNFT.getBunnyId(_tokenIds[i]);
            uint256 power = bunnyIdToPower[bunnyId];
            require(power > 0, "bonnyId error");
            if (power == 1) {
                pool.srCount += 1;
    ...
    function unStakeList(uint256[] memory _tokenIds) public {
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            speedNFT.safeTransferFrom(address(this), msg.sender, _tokenIds[i]);
            if (power == 1) {
                pool.srCount -= 1;
                addressInfo.srCount -= 1;
            } else if (power == 20) {
    ...
    function getNfts(address _address) public view returns (uint256[] memory, uint256[] memory) {
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            bunnyIds[i] = speedNFT.getBunnyId(ret[i]);
        }
        return (ret, bunnyIds);
    }
