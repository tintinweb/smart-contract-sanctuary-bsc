...
// File: contracts\IOpensaleReferral.sol



pragma solidity 0.6.12;

interface IOpensaleReferral {
    /**
     * @dev Record referral.
     */
...
// OdcToken with Governance.
contract OdcToken is BEP20 {
    ...
    // Min amount to liquify. (default 500 ODCs)
    uint256 public minAmountToLiquify = 500 ether;
    // The swap router, modifiable. Will be changed to Opensale's router when our own AMM release
    IUniswapV2Router02 public odcRouter;
    // The trading pair
    address public odcPair;
    // In swap and liquify
    ...
    event OpensaleRouterUpdated(address indexed operator, address indexed router, address indexed pair);
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "ODC::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    /**
     * @notice Constructs the ODC Token contract.
     */
    constructor() public BEP20("OpenDefiCoin", "ODC") {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        require(sender != address(0), "ODC::sender is not valid");
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(taxAmount == burnAmount + liquidityAmount, "ODC::transfer: Burn value invalid");

            // default 95% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "ODC::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the odc pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = odcRouter.WETH();

        _approve(address(this), address(odcRouter), tokenAmount);

        // make the swap
        odcRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        odcRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    // To receive BNB from odcRouter when swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "ODC::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(_burnRate <= 100, "ODC::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000000, "ODC::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
     ...
     * Can only be called by the current operator.
     */
    function updateOpensaleRouter(address _router) public onlyOperator {
        odcRouter = IUniswapV2Router02(_router);
        odcPair = IUniswapV2Factory(odcRouter.factory()).getPair(address(this), odcRouter.WETH());
        require(odcPair != address(0), "ODC::updateOpensaleRouter: Invalid pair address.");
        emit OpensaleRouterUpdated(msg.sender, address(odcRouter), odcPair);
    }
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "ODC::transferOperator: new operator is the zero address");
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "ODC::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "ODC::delegateBySig: invalid nonce");
        require(now <= expiry, "ODC::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "ODC::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying ODCs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "ODC::_writeCheckpoint: block number exceeds 32 bits");
    ...
    function addMinter(address _addMinter) public onlyOwner returns (bool) {
        require(_addMinter != address(0), "ODC: _addMinter is the zero address");
        return EnumerableSet.add(_minters, _addMinter);
    }

    function delMinter(address _delMinter) public onlyOwner returns (bool) {
        require(_delMinter != address(0), "ODC: _delMinter is the zero address");
    ...
    function getMinter(uint256 _index) public view onlyOwner returns (address){
        require(_index <= getMinterLength() - 1, "ODC: index out of bounds");
...
// File: contracts\ODCMasterChef.sol
