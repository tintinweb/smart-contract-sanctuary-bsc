...
contract GAYCATS is Context, IERC20, Ownable {
    ...
    address public stakingAddress = 0xD4381ac546622988841BC3151487A93C007245F7;
    address public marketingAddress = 0xD4381ac546622988841BC3151487A93C007245F7;
    address public devAddress = 0x369306c0889a796aD2BcE0F7c031cF5D3cbBde4A;

    address public constant deadAddress = 0x000000000000000000000000000000000000dEaD;

    string private constant _name = "GayCat";
    string private constant _symbol = "GAYCATS";
   ...
   function setMaxWalletPercent(uint256 maxWallPercent) external onlyOwner() {
    ...
    function setFeeRates(uint256 _rfi, uint256 _marketing, uint256 _staking, uint256 _devfee, uint256 _liquidity, uint256 _buyback) external onlyOwner {
        feeRates.rfi = _rfi;
        feeRates.marketing = _marketing;
        feeRates.staking = _staking;
        feeRates.devfee = _devfee;
        feeRates.liquidity = _liquidity;
        feeRates.buyback = _buyback;
        emit FeesChanged();
    }

    function setSellFeeRates(uint256 _rfi, uint256 _marketing, uint256 _staking, uint256 _devfee,uint256 _liquidity, uint256 _buyback) external onlyOwner{
        sellFeeRates.rfi = _rfi;
        sellFeeRates.marketing = _marketing;
        sellFeeRates.staking = _staking;
        sellFeeRates.devfee = _devfee;
    ...
    function _takedevfee(uint256 rdev, uint256 tdev) private {
        totFeesPaid.devfee +=tdev;
        if(_isExcluded[address(this)]){
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi,to_return.rmarketing, to_return.rdevfee ,to_return.rstaking, to_return.rLiquidity, to_return.rBuyback) = _getRValues(to_return, tAmount, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory s) {
        ...
        if(isSale){
            ...
            s.tdevfee = tAmount*sellFeeRates.devfee/1000;
            s.tLiquidity = tAmount*sellFeeRates.liquidity/1000;
            s.tBuyback = tAmount*sellFeeRates.buyback/1000;
            s.tTransferAmount = tAmount-s.tRfi-s.tmarketing-s.tstaking-s.tLiquidity-s.tBuyback;
        }
        else{
            s.tRfi = tAmount*feeRates.rfi/1000;
            s.tmarketing = tAmount*feeRates.marketing/1000;
            s.tstaking = tAmount*feeRates.staking/1000;
            s.tdevfee = tAmount*feeRates.devfee/1000;
            s.tLiquidity = tAmount*feeRates.liquidity/1000;
            s.tBuyback = tAmount*feeRates.buyback/1000;
            s.tTransferAmount = tAmount-s.tRfi-s.tmarketing-s.tstaking-s.tLiquidity-s.tBuyback-s.tdevfee;
        }
        return s;
    }

    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi, uint256 rmarketing,uint256 rstaking, uint256 rdevfee, uint256 rLiquidity, uint256 rBuyback) {
        ...
        rdevfee = s.tdevfee*currentRate;
        rLiquidity = s.tLiquidity*currentRate;
        rBuyback = s.tBuyback*currentRate;
        rTransferAmount =  rAmount-rRfi-rmarketing-rstaking-rLiquidity-rBuyback-rdevfee;
        return (rAmount, rTransferAmount, rRfi,rmarketing,rstaking, rdevfee, rLiquidity, rBuyback);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSale) private {
        ...
        _takedevfee(s.rdevfee,s.rdevfee);
        _takeLiquidity(s.rLiquidity,s.tLiquidity);
        _takestaking(s.rstaking, s.tstaking);
        _takeBuyback(s.rBuyback, s.tBuyback);
        emit Transfer(sender, recipient, s.tTransferAmount);
        emit Transfer(sender, address(this), s.tLiquidity + s.tmarketing + s.tBuyback + s.tdevfee);
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap{
        ...
        uint256 denominator= (feeRates.liquidity + feeRates.buyback + feeRates.marketing +feeRates.devfee) * 2;
        ...
        uint256 devfeeAmt = unitBalance * 2 * feeRates.devfee;
        if(devfeeAmt > 0){
          payable(devAddress).transfer(devfeeAmt);
        }
    }
