...
contract hd is Context, Ownable , ReentrancyGuard {
    ...
    function upgradeMiner(uint256 id) public {
        ...
        for (uint i=1; i<=minerId; i++) {
            if (miner[i].owner == msg.sender && miner[i].id == id && miner[i].lvl<8) {
    ...
    function deposit(address _ref, uint256 _amount) public noReentrant  {
        ...
        uint256 addref = SafeMath.add(balance[msg.sender].withdrawbalance,SafeMath.div(userCurrentInvestment,10));
        if(!checkAlready()){
            // chest
        ...
        balance[dev] = userBalance(dev,balance[dev].buybalance,addref);
        }



        // investment details
        uint256 newBalance = SafeMath.add(balance[msg.sender].buybalance,userCurrentInvestment);
        balance[msg.sender] = userBalance(msg.sender,newBalance,balance[msg.sender].withdrawbalance);
