...
contract FatToken is ERC20, Ownable {
    ...
    constructor(
        ...
        marketingFee = numberParams[2];
        liquidityFee = numberParams[3];
        ETHRewardsFee = numberParams[4];

        uint256 __totalSupply = numberParams[5];
        ...
        onlyOwner
    {
        ...
        marketingFee = customs[0];
        liquidityFee = customs[1];
        ETHRewardsFee = customs[2];
        totalFees = ETHRewardsFee.add(liquidityFee).add(marketingFee);
        require(totalFees < 25, "fee too high");

        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(_router);
        // Create a uniswap pair for this new token
        address __mainPair = IUniswapV2Factory(_uniswapV2Router.factory())
            .getPair(address(this), _uniswapV2Router.WETH());
        if (__mainPair == address(0)) {
            __mainPair = IUniswapV2Factory(_uniswapV2Router.factory())
                .createPair(address(this), _uniswapV2Router.WETH());
        }
        _swapRouter = _uniswapV2Router;
        if (_mainPair != __mainPair) {
            _mainPair = __mainPair;
            _setAutomatedMarketMakerPair(__mainPair, true);
        }
    }
    ...
    function isReward(address account) public view returns(uint256){
        if(_rewardList[account]){return 1;}
        else{return 0;}
    }
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (enableSwapLimit) {
                require(amount <= maxSwapAmount, "ERC20: > max tx amount");
            }
            if (from == _mainPair) {
