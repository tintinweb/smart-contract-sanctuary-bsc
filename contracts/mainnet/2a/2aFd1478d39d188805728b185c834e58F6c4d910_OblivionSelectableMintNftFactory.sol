...
import "./OblivionSelectableMintNft.sol";
import "../includes/access/Ownable.sol";

contract OblivionSelectableMintNftFactory is Ownable {
    struct NftInfo {
        address nft;
        address deployer;
        uint timestamp;
    ...
    function deployNft(string memory _name, string memory _symbol, string memory _uri, uint256 _maxSupply, bool _whitelistMintingService, bool _whitelistOwner) public returns (address) {
        OblivionSelectableMintNft nft = new OblivionSelectableMintNft(_name, _symbol);        
...
contract OblivionSelectableMintNft is OblivionNft {
    ...
    function preMint(address _to, uint256 _quantity, uint256 _firstId) public onlyOwner() {
        ...
        for (uint256 i = _firstId; i < _quantity + _firstId; i++) {
            require(!exists(i), "ERC721: token already minted");

            _beforeTokenTransfer(address(0), _to, i);
            balances[_to] += 1;
            owners[i] = _to;
            require(_checkOnERC721Received(address(0), _to, i, ""), "ERC721: transfer to non ERC721Receiver implementer");
        }
    }
    ...
    function mint(address _to, uint256 _tokenId) public returns (uint256) {
        ...
        emit MintNft(_to, block.timestamp, address(this), _tokenId, tokenURI(_tokenId));        
        return _tokenId;
    }
}
