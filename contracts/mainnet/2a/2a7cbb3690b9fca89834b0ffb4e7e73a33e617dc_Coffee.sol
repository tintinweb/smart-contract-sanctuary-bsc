...
contract Coffee is ERC20 {
	...
	address private _fundAddress = address(0x57195f5437532edB8B1d8923A546380a3a17D218);
    ...
    constructor(address tokenOwner) ERC20("Coffee Token", "Coffee") {
	...
	function changeSwapWarp(CoffeeWarp _warp) public onlyOwner {
        warp = _warp;
    ...
    function getLuck(address account) public view returns (uint256) {
        return _luckLdxAmount[account];
    }
    ...
    function _transfer(
        ...
        if(to == uniswapV2Pair){
			...
			if(isDelLdx && !_isExcludedFromFees[to]){
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx) {
			...
			if(from == uniswapV2Pair){
				...
				require(balanceOf(from).sub(amount) >= _luckLdxAmount[from],"error");
			}else{
                require(balanceOf(from).sub(amount) >= _luckLdxAmount[from],"error");
				takeFee = false;
            }
	...
	function swapAndLiquifyUsdt() public {
		...
		if(allAmount > 10**19){
			uint256 canswap = allAmount.div(10).mul(9);
			...
			if(allusdtAmount > usdtAmount){
				...
				USDT.transfer(_fundAddress,newUsdtAmount.div(9).mul(8));
				addLiquidityUsdt(newUsdtAmount.div(9),otherAmount);
			}
		}
    }
