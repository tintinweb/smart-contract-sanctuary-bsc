...
contract RIPPLENOMICS is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "RIPPLE NOMICS";
    string constant _symbol = "RIPPLENOMICS";
    ...
    uint256 public totalFee        = marketingFee + reflectionFee + liquidityFee + ecofee + burnFee;
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0xC5C7B1b8fB6112C9383B4959164F9Feb35d964Ee;
        ecofeeReceiver = 0xC5C7B1b8fB6112C9383B4959164F9Feb35d964Ee;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != ecofeeReceiver  && recipient != autoLiquidityReceiver && recipient != burnFeeReceiver){
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _ecofee, uint256 _burnFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        reflectionFee = _reflectionFee;
        marketingFee = _marketingFee;
        ecofee = _ecofee;
        burnFee = _burnFee;
        totalFee = _liquidityFee.add(_reflectionFee).add(_marketingFee).add(_ecofee).add(_burnFee);
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator/2, "Fees cannot be more than 50%");
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _ecofeeReceiver, address _burnFeeReceiver ) external authorized {
