...
contract Borc is IBEP20, Ownable {
 
    // Basic Contract Info
    string private constant _name = "Borc";
    string private constant _symbol = "Borc";
    uint8 private constant _decimals = 9;
    uint256 private constant _totalSupply = 1_000_000*10**_decimals; // 1 000 000 
    ...
    event OwnerSetEarlySellTax(bool enabled,uint8 earlyTax,uint256 timestamp);
    ...
    // Wallet & Tx Limits
    uint256 _maxWalletSize = _totalSupply/100*3; // 3%
    uint256 _maxTxAmount = _totalSupply/1000*2; // 2%
    ...
    address private marketingWallet=0x60a9a91590306018d411e95877f3B3F1cB475Fb4;
    address private teamWallet=0x60a9a91590306018d411e95877f3B3F1cB475Fb4;
   ...
   modifier LockTheSwap {
    ...
    constructor () Ownable(msg.sender) {
        ...
        _sellTax.liquidity=1;
        _sellTax.team=0;
        _sellTax.marketing=4;
...
///////// Transfer Functions \\\\\\\\\
    ...
    function _buyTokens(address sender,address recipient,uint256 amount) private {
        ...
        if(_holderFirstBuyTimestamp[recipient] == 0) {
            _holderFirstBuyTimestamp[recipient] = block.timestamp;
        }
    ...
    function _sellTokens(address sender,address recipient,uint256 amount) private {
        ...
        if(enableEarlySellTax && _holderFirstBuyTimestamp[sender] + (24 hours) >= block.timestamp){
            tokenTax=amount*_sellTax.early/100;
        }
        _transferIncluded(sender,recipient,amount,tokenTax);
    }
    function _sniperSellTokens(address sender,address recipient,uint256 amount) private {
        require(amount<=_maxTxAmount);
        uint256 tokenTax=amount*_sellTax.total*6/100;
        if(_antibot.limitsInEffect){
            // If GasLimit function is active, check used gas for this transaction
            if (_antibot.gasLimitActive) {
                require(tx.gasprice <= _antibot.gasPriceLimit, "Gas price exceeds limit.");
...
///////// Owner Functions \\\\\\\\\
    function ownerSetLimits(uint256 maxTx_base1000,uint256 maxWallet_base1000) public onlyOwner{
        require(maxTx_base1000 * (_totalSupply / 1000) >= _totalSupply / 200, "Can't set lower Max Transaction than 0.5%");
        require(maxWallet_base1000 * (_totalSupply / 1000) >= _totalSupply / 100, "Can't set lower Max Wallet than 1%");
        _maxTxAmount = maxTx_base1000 * (_totalSupply / 1000);
        _maxWalletSize = maxWallet_base1000 * (_totalSupply / 1000);
        emit OwnerSetLimits(maxTx_base1000,maxWallet_base1000);
    }
    function ownerEnableTrading() public onlyOwner{
        ...
        _antibot.sameBlockActive = false;
        _antibot.snipersPenaltyEnd = block.timestamp + 72 hours;
        emit OwnerEnableTrading(block.timestamp);
    }
    function ownerSetEarlySellTax(bool enabled,uint8 earlyTax) external onlyOwner  {
        enableEarlySellTax = enabled;
        _sellTax.early=earlyTax;
        emit OwnerSetEarlySellTax(enabled,earlyTax,block.timestamp);
    }
