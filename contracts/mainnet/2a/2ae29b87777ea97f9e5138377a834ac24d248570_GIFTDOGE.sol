...
contract GIFTDOGE is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    GDDividendTracker public dividendTracker;

    address constant public deadWallet = 0x000000000000000000000000000000000000dEaD;

    address public immutable ETH = address(0x55d398326f99059fF775485246999027B3197955); //USDT
  ...
  //  uint256 public extraFeeOnSell = 4;
    ...
    address public _marketingWalletAddress = 0xC41D495258846E77243aE36e0679Dc95EE86594e;
    address public _developmentWalletAddress = 0xD188A002118A319dAD552EB8Db78c089B43Fa2e5;
    ...
    constructor() ERC20("GIFT DOGE", "GD") {

    	dividendTracker = new GDDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "GD: The dividend tracker already has that address");

        GDDividendTracker newDividendTracker = GDDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "GD: The new dividend tracker must be owned by the GD token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "GD: The router already has that address");
    ...
    function excludeOrIncludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "GD: Account is already the value of 'excluded'");
    ...
    {
        require(_isExcludedFromMaxTx[account] != excluded, "GD: Account is already the value of 'excluded'");
    ...
    {
        require(_isExcludedFromMaxWallet[account] != excluded, "GD: Account is already the value of 'excluded'");
    ...
    function setSellFees(uint256 sellEthRewardFee, uint256 sellLiquidityFee, uint256 sellMarketingFee, uint256 sellDevFee) external onlyOwner
    {
      ...
      function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "GD: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 800000, "GD: gasForProcessing must be between 200,000 and 800,000");
        require(newValue != gasForProcessing, "GD: Cannot update gasForProcessing to same value");
...
contract GDDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("GD_Dividen_Tracker", "GD_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 20000 * (10**18); //must hold 20000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "GD_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "GD_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main GD contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "GD_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "GD_Dividend_Tracker: Cannot update claimWait to same value");
