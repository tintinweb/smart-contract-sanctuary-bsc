...
contract CINQMiner is Ownable, ReentrancyGuard {
    ...
    IBEP20 public token = IBEP20(0xc29982A00D375d068f9f3F96a66feDbdb11D869a);
    ...
    //Referral program
    uint256 public referralDepositPercent = 1;
    uint256 public referralCompoundPercent = 10;
    ...
    bool public enableMiner = false;
 
    constructor() {}

    function enable(bool enabled) public onlyOwner {
        enableMiner = enabled;
    }

    function setReferralDepositPercent(uint256 percent) public onlyOwner {
        referralDepositPercent = percent;
    }

    function setReferralCompoundPercent(uint256 percent) public onlyOwner {
        referralCompoundPercent = percent;
    }

    function deposit(uint256 amount, address ref) public nonReentrant {
        require(enableMiner == true, "Not enable yet");
        ...
        if(yourRef != address(0) && userInfos[yourRef].balance > 0){
            userInfos[yourRef].balance = userInfos[yourRef].balance.add(amount.mul(referralDepositPercent).div(100)); // referral on deposit
            refInfos[yourRef].refReward = refInfos[yourRef].refReward.add(amount.mul(referralDepositPercent).div(100));
        }
        ...
        if(info.lastClaim == 0){
            info.lastClaim = block.timestamp; // Note time for start claim count down
        }
    }
    ...
    function compound() public nonReentrant {        
        ...
        address yourRef = referrals[msg.sender];
        if(yourRef != address(0) && userInfos[yourRef].balance > 0){
            userInfos[yourRef].balance = userInfos[yourRef].balance.add(amount.mul(referralCompoundPercent).div(100)); // Referral on compounding
        }
    }
