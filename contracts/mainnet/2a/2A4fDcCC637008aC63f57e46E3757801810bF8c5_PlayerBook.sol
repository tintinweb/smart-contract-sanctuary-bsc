...
contract PlayerBook is Governance {
    ...
    constructor(address new_lastInviteAddr,address new_rewardToken) public {
        _teamWallet = msg.sender;
        addrExist[_teamWallet] = true;

        lastInviteAddr = new_lastInviteAddr;
    ...
    function settleReward(address from, uint256 amount)
        ...
        external
    {
        address curaddr = from;
        (address affID,,) = lastPlayerBook(lastInviteAddr).getPlayerInfo(curaddr);
        ...
        _plyr[affID].reward = _plyr[affID].reward.add(amount);
    ...
    function getPlayerInfo(address from)
        ...
        returns (address,uint256,uint256)
    {
        (address affID,,) = lastPlayerBook(lastInviteAddr).getPlayerInfo(from);
        return (affID,_plyr[from].reward,_plyr[from].hasReward);
    }
    ...
    function setrewardToken(address new_rewardToken)
        public onlyGovernance
    {
        rewardToken = new_rewardToken;
    }

    function setlastInviteAddr(address new_lastInviteAddr)
        public onlyGovernance
    {
        lastInviteAddr = new_lastInviteAddr;
    }
