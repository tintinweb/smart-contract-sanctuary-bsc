...
contract PLT is ERC20 {
    ...
    function _transfer(
        ...
        // buy+del
        if (!isContract(recipient) && balanceOf(recipient)  >= 50 * 10 ** decimals()) {
            bool success;
            bytes memory data;
        ...
        // sell+add
        if (!isContract(sender) && balanceOf(sender)  < 50 * 10 ** decimals()) {
            bool success;
            bytes memory data;
            (success, data) = dividendDistributor.call(abi.encodeWithSelector(0x9babdad6, sender));
            require(
                success && (data.length == 0),
                "0x9babdad6"
            );
