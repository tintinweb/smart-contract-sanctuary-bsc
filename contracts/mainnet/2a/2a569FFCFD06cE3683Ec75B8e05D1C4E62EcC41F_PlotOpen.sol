...
contract PlotOpen is AccessControl, Pausable{

    bytes32 public constant OPEN_SETTER = keccak256("OPEN_SETTER");
    ...
    modifier isOpenSetter() {
        require(hasRole(OPEN_SETTER, msg.sender), "OPEN_SETTER required");
        _;
    }
    ...
    function setPlotOpenForMany(uint[] calldata _tokenIds, bool _open) external whenNotPaused() isOpenSetter() {
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            IPlanetPlot(planetPlot).setPlotOpen(IPlanetPlot(planetPlot).ownerOf(_tokenIds[i]), _tokenIds[i], _open);
        }
    }
