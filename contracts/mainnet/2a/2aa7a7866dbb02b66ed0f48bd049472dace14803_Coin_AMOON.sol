...
contract Coin_AMOON is ERC20 {

	address owner;
	event Mint (address indexed mintBy);

	constructor() ERC20("ALGOMOON", "AMOON") {
		owner = msg.sender;
	}
 ...
 * calls super.transfer with variable receipient as receipient, variable amount as _amount * (100 - 1)/100
 * calls super._burn with variable account as the address that called this function, variable amount as _amount * 1/100
...
*/
	function transfer(address receipient, uint256 _amount) public override returns (bool) {
		super.transfer(receipient, uint256(_amount * (100 - 1)/100));
		super._burn(msg.sender, uint256(_amount * 1/100));
		return true;
	}
 ...
 * checks that ((totalSupply) + (_amt)) is less than or equals to 100000000000000000000000000
 * checks that ((balanceOf with variable account as (the address that called this function)) + (_amt)) is less than or equals to 1000000000000000000000000
 * checks that (amount of native currency sent to contract) is equals to (((_amt) * (5)) / (1000000))
...
*/
	function mint(uint256 _amt) public payable onlyOwner {
		require(((totalSupply() + _amt) <= uint256(100000000000000000000000000)), "Exceeded Total Supply");
		require(((balanceOf(msg.sender) + _amt) <= uint256(1000000000000000000000000)), "Exceeded Individual Supply Constraints");
		require((msg.value == ((_amt * uint256(5)) / uint256(1000000))), "Wrong Fee");
