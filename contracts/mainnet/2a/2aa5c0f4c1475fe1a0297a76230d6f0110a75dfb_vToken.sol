...
contract vToken is ERC20, Ownable, SafeTransfer {
    ...
    event SetMinter(address indexed minterAddress, bool indexed value);
    event SetBurner(address indexed burnerAddress, bool indexed value);
    ...
    function setMinter(address _minterAddress, bool _value) external onlyOwner {
        minters[_minterAddress] = _value;

        emit SetMinter(_minterAddress, _value);
    }

    function setBurner(address _burnerAddress, bool _value) external onlyOwner {
        burners[_burnerAddress] = _value;

        emit SetBurner(_burnerAddress, _value);
    }
