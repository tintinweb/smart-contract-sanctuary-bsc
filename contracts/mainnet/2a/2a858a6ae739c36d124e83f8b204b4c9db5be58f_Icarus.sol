...
contract Icarus is ERC20, Ownable {
    ...
    constructor() ERC20("Icarus", "BPHANTOM") {
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "Icarus: The marketing wallet is already this address");
    ...
    function mint(address to, uint256 _totalSupply) public onlyOwner {
    ...
    function updatefirstDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(firstDividendTracker), "Icarus: The dividend tracker already has that address");
 
        FIRSTDividendTracker newfirstDividendTracker = FIRSTDividendTracker(payable(newAddress));
 
        require(newfirstDividendTracker.owner() == address(this), "Icarus: The new dividend tracker must be owned by the Icarus token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "Icarus: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "Icarus: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Icarus: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "Icarus: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "Icarus: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "Icarus: Trading has not started yet");
...
contract FIRSTDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("Icarus_Dividend_Tracker", "Icarus_Dividend_Tracker", 0x8b3b45E48bE6C31366ffd9dD4F29C1edFFcbA97D) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 2000000 * (10**18); //must hold 2000000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "Icarus_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "Icarus_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Icarus contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Icarus_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Icarus_Dividend_Tracker: Cannot update claimWait to same value");
