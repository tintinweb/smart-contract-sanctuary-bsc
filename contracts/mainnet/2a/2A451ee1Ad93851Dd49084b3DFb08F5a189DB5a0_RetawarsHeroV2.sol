...
abstract contract RetaNftV2 is
  Context,
  AccessControlEnumerable,
  ERC721Enumerable,
  ERC721Burnable,
  ERC721Pausable
{
  ...
  uint32 private _allowedTokenAmount;
  bool private _isCheckingAllowedTokenAmount;
  ...
  constructor(
    ...
    _isCheckingAllowedTokenAmount = true;
  ...
  function allowedTokenAmount() public view returns (uint32) {
    return _allowedTokenAmount;
  }

  function setIsCheckingAllowedTokenAmount(bool value) public onlyOwner {
    _isCheckingAllowedTokenAmount = value;
  }

  function isCheckingAllowedTokenAmount() public view returns (bool) {
    return _isCheckingAllowedTokenAmount;
  }
  ...
  function _beforeTokenTransfer(
    ...
    if(_isCheckingAllowedTokenAmount && to != address(0) && to != owner() && !to.isContract()) {
...
interface IRWHeroSpawnerV2 {
  function spawn(address to, uint64 ms) external returns (uint256);
  function spawnPresaleV2(address to, uint64 ms) external returns (uint256);
  function spawnPresaleV1Referral(address to, uint64 ms, IRetawarsHero.Grade grade) external returns (uint256);
  function cloneV1(address to, uint64 ms, uint256 srcTokenId, IRetawarsHero.Hero memory srcHero) external returns (uint256);
}

interface IRWHeroGeneratorV2 {
  ...
  function generateByGrade(address sender, uint64 ms, IRetawarsHero.Grade grade, uint8 heroType) external returns (IRetawarsHero.Hero memory);
  function generatePresale2nd(address sender, uint64 ms) external returns (IRetawarsHero.Hero memory);
  function generateBonusStat(IRetawarsHero.Hero memory srcHero, address sender, uint64 ms) external returns (IRetawarsHero.Hero memory);
}


contract RetawarsHeroV2 is IRetawarsHero, IRWHeroSpawnerV2, RetaNftV2 {
  using SafeMath for uint256;
  
  mapping(uint256 => Hero) public heroes_;

  address private generator_;
  uint256 public v1TokenMax_;
  ...
  event Cloned(address indexed minter, address indexed tokenOwner, uint256 indexed tokenId, IRetawarsHero.Hero hero);
  ...
  constructor() 
    RetaNftV2("Retawars Hero", "RWHero", "https://api.retawars.com/hero/", 1000)
  {
  }

  function setGenerator(address generatorAddr) public onlyOwner {
    require(IRWHeroGeneratorV2(generatorAddr).getCaller() == address(this), "The generator's caller is wrong");
    generator_ = generatorAddr;
  }

  function setV1TokenMax(uint256 maxTokenId) public onlyOwner {
    v1TokenMax_ = maxTokenId;
  }

  modifier usingGenerator {
    require(generator_ != address(0), "The generator is empty");
    _;
  }

  function _baseTokenId() internal pure override returns (uint256) {
    return 100000;
  }
  ...
  function spawn(address to, uint64 ms)
    ...
    returns (uint256)
  {
    IRWHeroGeneratorV2 generator = IRWHeroGeneratorV2(generator_);

    Hero memory hero = generator.generate(to, ms);
    uint256 tokenId = super.mint(to);

    heroes_[tokenId] = hero;
  ...
  /**
    @dev `spawnPresaleV2` only called by RWHeroPresaleV2 contract
   */
  function spawnPresaleV2(address to, uint64 ms)
    external
    override
    onlyMinter
    usingGenerator
    ...
    returns (uint256)
  {
    ...
    Hero memory hero = generator.generatePresale2nd(to, ms);
    uint256 tokenId = super.mint(to);
  ...
  function spawnPresaleV1Referral(address to, uint64 ms, IRetawarsHero.Grade grade)
    external
    override
    onlyMinter
    usingGenerator
    ...
    returns (uint256)
  {
    ...
    Hero memory hero = generator.generateByGrade(to, ms, grade, 1);
    uint256 tokenId = super.mint(to);
  ...
  /**
    @dev `cloneV1` only called by RWHeroPresaleV2 contract
   */
  function cloneV1(address to, uint64 ms, uint256 srcTokenId, Hero memory srcHero)
    external
    override
    onlyMinter
    usingGenerator
    ...
    returns (uint256)
  {
    require(!_exists(srcTokenId), "existing token id");
    require(srcTokenId <= v1TokenMax_, "invalid token id to clone");

    IRWHeroGeneratorV2 generator = IRWHeroGeneratorV2(generator_);

    _mint(to, srcTokenId);
    heroes_[srcTokenId] = generator.generateBonusStat(srcHero, _msgSender(), ms);

    emit Cloned(_msgSender(), to, srcTokenId, srcHero);
    return srcTokenId;
  }
    ...
    returns (uint8)
  {
    ...
    Hero storage hero = heroes_[tokenId];
