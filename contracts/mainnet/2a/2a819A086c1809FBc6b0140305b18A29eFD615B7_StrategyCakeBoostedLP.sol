...
contract StrategyCakeBoostedLP is StratManager, FeeManager, GasThrottler {
    ...
    ) StratManager(_keeper, _strategist, _unirouter, _vault, _cashFeeRecipient) public {
        want = _want;
        poolId = _poolId;
        chef = _chef;
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        IERC20(native).safeTransfer(cashFeeRecipient, cashFeeAmount);
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount,,) = ICakeV2Chef(chef).userInfo(poolId, address(chef));
        return _amount;
    }

    // returns rewards unharvested
    function rewardsAvailable() public view returns (uint256) {
        return ICakeV2Chef(chef).pendingCake(poolId, address(chef));
    }
...
import "../interfaces/common/IGasPrice.sol";
