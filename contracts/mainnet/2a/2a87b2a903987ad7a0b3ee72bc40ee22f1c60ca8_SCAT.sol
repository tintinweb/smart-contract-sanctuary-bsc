...
contract ERC20DividendToken is Context, ERC20, Ownable {
    ...
    event SendFee(address indexed recipient, uint256 tokensSwapped, uint256 amount);

    constructor(string memory name_, string memory symbol_, uint256 totalSupply_, address routerV2_, address marketingAddress_, address devAddress_) ERC20(name_, symbol_) {
        liquidityFee = 5;
        marketingFee = 4;
        devFee = 6;
        totalFees = devFee.add(liquidityFee).add(marketingFee);
        require(totalFees <= 15, "Total fee is over 15%");

        marketingAddress = marketingAddress_;
        devAddress = devAddress_;
        
        minTokensBeforeSwap = 10_000 * (10**18);

        maxTxAmount = totalSupply_ * (10**18);
        ...
        // exclude from max wallet limit and max transaction limit
    ...
    /**
      * @dev Includes/Excludes an account from max limits.
      *
      * - `account` - address to change the state.
      * - `excluded` - true/false whether exclude/include in limits
      */
    ...
    /**
      * @dev Changes max transaction amount
      *
      * - `amount` - new max transaction amount in token wei's
      */
    ...
    /**
      * @dev Changes max wallet amount
      *
      * - `amount` - new max wallet amount in token wei's
      */
    ...
    /**
      * @dev Sets swap threshold for executing fees accumulated on contract balance
      *
      * - `amount` - new swap threshold in token wei's
      */
    ...
    /**
      * @dev Internal function used to create pair contract for DEX using routerV2 address provided
      *
      * - `newAddress` - address of RouterV2 contract deployed
      */
    ...
    /**
      * @dev Includes/Excludes an account from paying fees.
      *
      * - `account` - address to change the state.
      * - `excluded` - true/false whether exclude/include in paying fees
      */
    ...
    /**
      * @dev Includes/Excludes an accounts from paying fees.
      *
      * - `account[]` - array of addresses to change the state.
      * - `excluded` - true/false whether exclude/include all addresses in paying fees
      */
    ...
    /**
      * @dev Sets new marketing address which will receive makreting fee
      *
      * - `wallet` - address of new marketing wallet
      */
    ...
    /**
      * @dev Sets new dev address which will receive dev fee
      *
      * - `wallet` - address of new dev wallet
      */
    ...
    /**
      * @dev Sets new rate of liquidity fee
      *
      * - `value` - percent which will be taken from each transaction for liquidity
      */
    ...
    /**
      * @dev Sets new rate of marketing fee
      *
      * - `value` - percent which will be taken from each transaction for marketing
      */
    ...
    /**
      * @dev Sets new rate of dev fee
      *
      * - `value` - percent which will be taken from each transaction for dev
      */
    ...
    /**
      * @dev Adds/Removes the pair contract on which taxes will be charged
      *
      * - `pair` - address of pair contract deployed
      * - `value` - true/false whether pair is recognized by contract or not
      */
     ...
     *
     * This is an override for internal function {transfer}, and can be used to
     * e.g. implement automatic token fees, slashing mechanisms, etc.
     *
     * Emits a {Transfer} event.
     *
    ...
    function _transfer(
        ...
        // if the contract reached swap threshold - execute all taxes
        ...
        if (
            ...
            if (marketingFee > 0) {
                ...
                swapAndSendFee(marketingAddress, marketingTokens);
            }
            if (devFee > 0) {
                uint256 devTokens = contractTokenBalance.mul(devFee).div(totalFees);
                swapAndSendFee(devAddress, devTokens);
            }
            if (liquidityFee > 0) {
    ...
    /**
     * @dev Swaps `tokens` for ETH and sends it to `recipient`
     *
     * Emits {SendFee} event indicating the recipient address,
     * the amount of tokens swapped to ETH
     * and the amount of ETH sent.
     */
    function swapAndSendFee(address recipient, uint256 tokens) private {
        ...
        if (success) {
            emit SendFee(recipient, tokens, dividends);
        }
    }
}

contract SCAT is ERC20DividendToken {
    ...
    address private _marketingAddress = 0x26A82F90aC4f2e87d22d95767Cb237d916A255ba;
    address private _devAddress = 0xCD531757760E8b07fe880f9F1A0ED1c5950579c9;

    constructor () ERC20DividendToken("Smol Cat", "SCAT", _tokenSupply, _routerAddress, _marketingAddress, _devAddress) {
