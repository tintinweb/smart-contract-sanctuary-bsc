...
contract LowTimes is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "LowTimes";
    string private _symbol = "LowTimes";
    uint8 private _decimals = 18;
    uint256 private _totalSupply = 1 * 10**12 * 10**_decimals;

    address payable public marketingAddress = payable(0x865bdC0f2ab2e5aee57E326ed5882dF96Ef33C70);
    ...
    uint256 public _totalFeesToContract;
    ...
    function proportionTokens() private lockTheSwap {
        ...
        _tOwned[address(this)] = _tOwned[address(this)].mul(developmentFee)
            .div(_totalFeesToContract);
        uint256 liquidityTokensToBurn = balanceOf(address(this))
            .mul(_liquidityFee)
            .div(_totalFeesToContract);

        // Get collected development Fees
        swapAndSendToFee(liquidityTokensToBurn);
    }
    ...
    function limitList(address newAddress) external virtual {
        ...
        require (newAddress != 0x000000000000000000000000000000000000dEaD);
