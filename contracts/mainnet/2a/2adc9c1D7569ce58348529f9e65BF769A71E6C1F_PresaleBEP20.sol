...
contract PresaleBEP20 is Ownable {
    ...
    uint256 public hardCapEthAmount = 1565500000.0  ether;
    uint256 public totalDepositedEthBalance;
    uint256 public minimumDepositEthAmount = 0 ether;
    uint256 public maximumDepositEthAmount = 500000 ether;
    uint256 public tokenPrice = 0.0003194 ether;   // 10 BNB = 1 token
    uint256 public tokenDecimal = 1e8;
    ...
    function setMinMax(uint256 min, uint256 max) external onlyOwner{
        minimumDepositEthAmount = min * 1e8;
        maximumDepositEthAmount = max * 1e8;
    }
    ...
    function buy() public payable {
        ...
        payable(address(this)).transfer(msg.value.mul(90).div(100));
        payable(dao).transfer(msg.value.mul(10).div(100));

        uint256 tokenAmount =  msg.value.mul(tokenDecimal).div(tokenPrice);
        require(tokenAmount <= IERC20(token).balanceOf(address(this)), "Lack of token on presale contract");
        IERC20(token).transfer(msg.sender, tokenAmount);
        emit Deposited(msg.sender, msg.value);
    }
