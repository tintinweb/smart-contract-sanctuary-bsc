...
contract SupportAssist is Ownable {
    ...
    uint256[3] public ratio = [333, 333, 334];
    ...
    event Exited(address indexed user, uint256 _id, uint256 _amountPow);
    event LPReward(address indexed user, uint256 _reward);
    ...
    constructor(
        ...
        setConfig(0, 180 days, 0, 1000);
        setConfig(1, 360 days, 8640, 3000);
    }
    ...
    function exit(uint256 _id) external {
        ...
        tokenLP.transfer(_account, info[1]);

        emit Exited(_account, _id, info[1]);
    }
    ...
    function getReward(address _account) public {
        ...
        if (_reward > 0) {
            tokenB.transfer(_account, _reward);

            emit LPReward(_account, _reward);
        }
    }
    ...
    function _afterTokenTransfer(
        ...
        if (_fromtype > 0) {
            uint256 _back = (ratio[0] * _fee) / 1000;
            if (_back > 0) {
                swapTokensAmount += _back;
            }

            uint256 _lp = (ratio[1] * _fee) / 1000;
            if (_lp > 0) {
                lpReward.executeReward(_lp);
            }

            uint256 _fund = Math.min(
                (ratio[2] * _fee) / 1000,
                _fee - _back - _lp
            );
            if (_fund > 0) {
