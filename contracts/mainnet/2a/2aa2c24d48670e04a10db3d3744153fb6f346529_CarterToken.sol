...
contract CarterToken is Context, IBEP20 {
    ...
    IDEXRouter public _routerAPE;
    address public _pair;
    address public _pairAPE;

    uint256 public maxWalletBalance = 200_000_000 * 10 ** decimals(); // 2% of total supply
    uint256 public MaxTradeLimit = 50_000_000 * 10 ** decimals();     // 0.5% of total supply
    ...
    constructor() {
        ...
        _routerAPE = IDEXRouter(0xcF0feBd3f17CEf5b47b0cD257aCf6025c5BFf3b7); // ApeSwap Router MAINNET
        //_router = IDEXRouter(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3); // Pancake Router TESTNET
        //_routerAPE = IDEXRouter(0x3380aE82e39E42Ca34EbEd69aF67fAa0683Bb5c1); // ApeSwap Router TESTNET
        ...
        _pairAPE = IDEXFactory(_routerAPE.factory()).createPair(
            _routerAPE.WETH(),
            address(this)
        );

        setAutomatedMarketMakerPair(_pair, true);
        setAutomatedMarketMakerPair(_pairAPE, true);

        excludedFromTradeLimit(_owner, true);
    ...
    function addBotToList(address account) external onlyOwner() {
        require(
            account != address(_router) || account != address(_routerAPE),
            "We can not blacklist routers"
        );
