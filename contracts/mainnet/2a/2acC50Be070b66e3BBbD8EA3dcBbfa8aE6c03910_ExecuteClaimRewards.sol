...
contract ExecuteClaimRewards is UnstakeLogic, StakeLogic, IExecutable {
  enum ClaimRewardTypes {
    Native,
    Yield,
    Stake
  }

  event RewardsNativeClaimed(uint256 toBurn, uint256 toDevelopment, uint256 yieldAmount);
  event RewardsYieldClaimed(uint256 toBurn, uint256 toDevelopment, uint256 yieldAmount);
  event RewardsStaked(uint256 amount);
  ...
  */
  function execute(bytes calldata data) strategistOrInvestor external {
    (
      ...
      ClaimRewardTypes _claimRewardType,
      bool exit
    ) = abi.decode(data, (UnstakeSet, VaultToLiquiditySet, WithdrawLiquiditySet, address[], ClaimRewardTypes, bool));

    if (exit) {
      claimRewardsInType(_claimRewardType, _un.poolTemplate, _un.stakeContractAddress, _un.pid, _rewards);
      unstakeAndWithdrawLiquidity(_un, _vl, _wl, true);
  ...
  function claimRewardsInType(
    ClaimRewardTypes _claimProfitType,
    uint256 _poolTemplate,
    address _stakeContractAddress,
    uint256 _pid,
    address[] memory _rewards
  ) internal {
    IStrategy strategy = IStrategy(factory.getStrategy(_poolTemplate));
    PersonalContractLibrary.claimRewards(strategy, _stakeContractAddress, _pid);

    if (_claimProfitType == ClaimRewardTypes.Native) {
      claimRewardsInNative(_rewards);
    } else if (_claimProfitType == ClaimRewardTypes.Yield) {
      claimRewardsInYield(_rewards);
    } else if (_claimProfitType == ClaimRewardTypes.Stake) {
      stakeRewardsToYieldPool(_rewards);
    } else {
      revert();
    }
  }

  function claimRewardsInNative(address[] memory _rewards) internal {
    (uint256 toDevelopment, uint256 toBurn, address toToken, address to) = factory.claimInNativeSettings();
    for (uint256 i = 0; i < _rewards.length; i++) {
      claimRewardInNative(_rewards[i], toDevelopment, toBurn, toToken, to);
    }
  }

  function claimRewardInNative(
    address reward,
    ...
    toDevelopment = balance - ((balance * (percentageDecimals - toDevelopment)) / percentageDecimals);
    toBurn = balance - ((balance * (percentageDecimals - toBurn)) / percentageDecimals);

    PersonalContractLibrary.convertTokenToToken(factory, to, reward, toToken, toDevelopment, 1);

    uint256 yieldToBurn = PersonalContractLibrary.convertTokenToToken(
      factory,
      address(this),
      reward,
      yieldToken,
    ...
    uint256 toInvestor = IERC20(reward).balanceOf(address(this));
    IERC20(reward).transfer(investor, toInvestor);
    emit RewardsNativeClaimed(toBurn, toDevelopment, toInvestor);
  }

  function claimRewardsInYield(address[] memory _rewards) internal {
    (uint256 toDevelopment, uint256 toBurn, address toToken, address to) = factory.claimInYieldSettings();
    address _yieldToken = yieldToken;

    for (uint256 i = 0; i < _rewards.length; i++) {
      claimRewardInYield(_rewards[i], toDevelopment, toBurn, toToken, to, _yieldToken);
    }
  }

  function claimRewardInYield(
    address reward,
    uint256 toDevelopment,
    uint256 toBurn,
    address toToken,
    ...
    uint256 yieldAmount = PersonalContractLibrary.convertTokenToToken(
      ...
      reward,
      _yieldToken,
      balance - toDevelopment,
      1
    );

    uint256 toBurnYield = yieldAmount * toBurn / (balance - toDevelopment);
    yieldAmount -= toBurnYield;

    ERC20Burnable(_yieldToken).burn(toBurnYield);
    IERC20(_yieldToken).transfer(investor, yieldAmount);
    emit RewardsYieldClaimed(toBurn, toDevelopment, yieldAmount);
  }

  function stakeRewardsToYieldPool(address[] memory _rewards) internal {
    address token = investment.token;
    uint256 balance;
    for (uint256 i = 0; i < _rewards.length; i++) {
      uint256 rewardBalance = IERC20(_rewards[i]).balanceOf(address(this));
      if (rewardBalance > 0) {
        balance += PersonalContractLibrary.convertTokenToToken(
          factory,
          address(this),
          _rewards[i],
          token,
    ...
    stakeToYieldPool(token, balance);
  }

  /**
  @notice Stake tokens to yield liquidity pool
  @param profitToken token which profit collected
