...
// File: contracts/tokenRelaunch.sol
...
contract tokenRelaunch is Ownable {
    ...
    event tokensDeposited (address user, uint allocation, uint timestamp);
    event tokensClaimed (address user, uint allocation, uint timestamp);
    event vestingFunded (uint amount, uint timestamp);
    
    constructor ( address _acceptedToken, uint _startTime) {       
        acceptedToken = IERC20(_acceptedToken);
        startTime = _startTime;
    }

    function setTokenToDistribute(address _tokenToDistribute) external onlyOwner {
        tokenToDistribute = IERC20(_tokenToDistribute);
    }

    function claim () external {  
        require(isFunded(), "Vesting not funded yet");
        require(block.timestamp > startTime);
        require(userAllocation[msg.sender] > 0, "User doesn't have allocation");

        uint256 amountToTranfer = userAllocation[msg.sender];
        ...
        emit tokensClaimed(msg.sender, amountToTranfer, block.timestamp);    
    }

    function depositTokens(uint _amount) external {
        acceptedToken.transferFrom(msg.sender, address(this), _amount);

        userAllocation[msg.sender] = userAllocation[msg.sender] + _amount;
    ...
    function withdrawToken (address _tokenAddress) external onlyOwner {
        uint remaining = IERC20(_tokenAddress).balanceOf(address(this));
