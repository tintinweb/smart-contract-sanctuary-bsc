...
contract MPEToken is ERC20, Ownable {
    ...
    function _transfer(
        ...
        // require(swapPair != address(0), "ERC20: swap pair canot be zero address");
        ...
        if (
            ...
            feeTotal >= 10000000 * 10**18
        ) {
            fee = 0;
        } else {
            if (sender == swapPair || sender == address(0)) {
                if (isBlackOpen) _isBlacklisted[recipient] = true;
        ...
        if (fee > 0) super._transfer(sender, deadwallet, fee);
    }
}