...
contract SeedRound is Ownable {
  ...
  uint256 constant public               MIN_PURCHASE_AMOUNT = 1 ether; // <-- change to 1000
  uint256 constant public               ROUND_START_DATE = 1656620238;        // <-- change to 1657324800 09.07.22 00:00
  uint256 constant public               ROUND_END_DATE = 1658102400;          // 18.07.22 00:00
  uint256 constant public               LOCK_PERIOD = 10 seconds;             // <------------------------ change to 30 days

  // @notice                            token interfaces
  address public                        sqatAddress;
  ...
  modifier                              ifActive() {
    if (isActive == false || (block.timestamp < ROUND_START_DATE) || (block.timestamp > ROUND_END_DATE) || availableTreasury == 0) {
      isActive = false;
