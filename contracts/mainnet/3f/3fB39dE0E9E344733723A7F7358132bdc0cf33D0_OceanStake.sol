...
contract OceanStake is ReentrancyGuard {
    ...
    constructor(
        ...
        require(!Address.isContract(_dev) && !Address.isContract(_mrk));
        owner = msg.sender;
        dev = _dev;
        mrk = _mrk;

        //_tokenAddress is the addres of the token to be used for this contract
        //can be address(0) to use network's native token (ETH,BNB,Matic etc..)
        require(
        ...
        nonReentrant
    {
        ...
        if (tokenAddress != address(0)) {
            require(amount == 0); //prevent payments of network's native token
            amount = _amount;
    ...
    function _getCurrentPerc(address addr) private view returns (uint256) {
        return _getBasePerc() + _getHoldBonusPerc(addr) + _getBalancePerc();
    }
