...
contract Snowflake is IBEP20, Auth {
    ...
    address DEV = 0xEb8049b879933e257cE4FE856B17293602915EC2;
    ...
    uint256 public totalFee        = marketingFee + reflectionFee + liquidityFee + devFee + charityFee;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
       ...
       if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != devFeeReceiver  && recipient != autoLiquidityReceiver && recipient != charityFeeReceiver){
   ...
   function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _charityFee, uint256 _feeDenominator) external authorized {
        ...
        totalFee = _liquidityFee.add(_reflectionFee).add(_marketingFee).add(devFee).add(_charityFee);
