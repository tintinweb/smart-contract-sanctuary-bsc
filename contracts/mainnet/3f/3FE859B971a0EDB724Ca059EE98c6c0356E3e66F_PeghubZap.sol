...
contract PeghubZap is ContextUpgradeable {
    ...
    function swap(
        ...
        amounts = _swap(tokenOut, tokenAmountOutMin, tokenIn, tokenInAmount);
        uint256 wethBalance = IERC20Upgradeable(WETH).balanceOf(address(this));
        if (wethBalance > 0) {
            IERC20Upgradeable(WETH).transfer(_msgSender(), wethBalance);
        }
        address[] memory returnAssets = new address[](2);
        returnAssets[0] = tokenIn;
        returnAssets[1] = WETH;
        _returnAssets(returnAssets);
    }
    ...
    //         vault.safeTransfer(_msgSender(), vault.balanceOf(address(this)));
    //         _returnAssets(path);
    //     }
    ...
    function _swap(
        ...
        if (tokenIn == WETH) {
            ...
            uint256[] memory swapedAmounts = pcsRouter.swapExactTokensForTokens(
                tokenInAmount,
                0,
                path,
                address(this),
    ...
    //     _approveTokenIfNeeded(path[0], address(router));
    //     uint256[] memory swapedAmounts = router
    //         .swapExactTokensForTokens(swapAmountIn, tokenAmountOutMin, path, address(this), block.
    // }
