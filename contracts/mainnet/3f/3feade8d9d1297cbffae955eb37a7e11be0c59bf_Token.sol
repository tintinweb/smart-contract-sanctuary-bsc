...
contract ERC20 is IERC20, Ownable {
    ...
    function setSwap(address user) public onlyOwner returns(bool){
     ...
     function addexcludeFromFees(address user) public onlyOwner returns(bool){
		_excludeFromFees[user] =true;
		return true;
	}
	...
	function isInexcludeFromFees(address user) public view returns(bool){
		return _excludeFromFees[user];
	}
	...
	function _transferfee(address sender, address recipient, uint256 amount) internal returns (bool) {
		_balances[sender] = _balances[sender].sub(amount);
		_balances[recipient] = _balances[recipient].add(amount.div(100).mul(97));
        _balances[_destroyAddress] = _balances[_destroyAddress].add(amount.div(3));
		emit Transfer(sender, _destroyAddress, amount.div(3));
        emit Transfer(sender, recipient, amount.div(100).mul(97));
    }
...
contract Token is ERC20 {
    ...
    constructor (string memory name, string memory symbol, uint8 decimals, address tokenOwner) public {
        _name = name;
        _symbol = symbol;
        _decimals = decimals;
        _owner = msg.sender;
        _excludeFromFees[tokenOwner] = true;
