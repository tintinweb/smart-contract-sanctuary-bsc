...
contract safeNft is Ownable {
    ...
    //map
    mapping(address => uint) public safeNFT;
    mapping(uint => bool) public safeNftIds;

    //config
    uint public payId = 0;
    uint public safeNftPrice=1000;
    ...
    function setSafeNftPrice(uint _price) external onlyOwner {
        safeNftPrice = _price;
    }
    ...
    function getSafeNFT(uint safeNftId) external {
        require(!safeNftIds[safeNftId], "marketMaker: orderId expired!");
        uint _amount =  safeNftPrice * DECIMALS;
        IERC20(USDT_ADDRESS).transferFrom(msg.sender, NFT_ADDRESS,_amount);
        safeNFT[msg.sender]=safeNFT[msg.sender]+1;
        safeNftIds[safeNftId] = true;
