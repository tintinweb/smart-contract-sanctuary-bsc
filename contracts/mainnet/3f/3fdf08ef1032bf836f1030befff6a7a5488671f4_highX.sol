...
contract highX is ERC20Detailed, Ownable {
    ...
    string public _name = "high X ";
    string public _symbol = "HUX";
    ...
    uint256 public cafFee = 60;
    uint256 public afFee = 20;
    uint256 public PlatinumFee = 10;
    uint256 public burnFee = 10;

    uint256 public sellFee = PlatinumFee;
    uint256 public totalFee = liquidityFee.add(reserveFee).add(cafFee).add(afFee).add(burnFee);
    uint256 public feeDenominator = 1000;

    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

    address public afReceiver;
    address public PlatinumReceiver;

    address public lpReceiver;
    address public reserveReceiver;
    address public cafReceiver;
    ...
    constructor() ERC20Detailed("high X", "HUX", uint8(DECIMALS)) Ownable() {
        ...
        lpReceiver = 0xA5eebD2DfE8B867d9Bf7d107cadc4b93828B49a2;
        reserveReceiver = 0xB567a55af69C832A6Cc229b56003694905bd85ca;
        cafReceiver = 0x20aE516F261dc1e5Da7fC158AC35dD8Da1EF703c;
        afReceiver = 0xf4C190C4B386777ECB84fCc3F3eb16d6e784D42C;
        PlatinumReceiver = 0xCA3885b5b5DF6dF9bE0E1ff88D95C43eb054b9Af;
    ...
    function setFees(uint256 _liquidityFee, uint256 _reserveFee, uint256 _cafFee, uint256 _afFee, uint256 _PlatinumFee, uint256 _burnFee) external onlyOwner {
        ...
        cafFee = _cafFee;
        afFee = _afFee;
        PlatinumFee = _PlatinumFee;
        burnFee = _burnFee;

        sellFee = PlatinumFee;
        totalFee = liquidityFee.add(reserveFee).add(cafFee).add(afFee).add(burnFee);
    }
    ...
    function swapBack() internal swapping {
        ...
        uint256 _currentFee = reserveFee.add(cafFee).add(afFee).add(PlatinumFee);

        (bool success, ) = payable(reserveReceiver).call{
        ...
        (success, ) = payable(cafReceiver).call{
        value: currentBalance.mul(cafFee).div(
            _currentFee
        ),
        gas: 30000
        }("");

        (success, ) = payable(afReceiver).call{
        value: currentBalance.mul(afFee).div(
            _currentFee
        ),
        gas: 30000
        }("");

        (success, ) = payable(PlatinumReceiver).call{
        value: currentBalance.mul(PlatinumFee).div(
            _currentFee
        ),
        gas: 30000
        }("");
    ...
    function withdrawAllToreserve() external swapping onlyOwner {
    ...
    function setFeeReceivers(
        ...
        address _cafReceiver,
        address _PlatinumReceiver,
        address _afReceiver
    ) external onlyOwner {
        reserveReceiver = _reserveReceiver;
        cafReceiver = _cafReceiver;
        PlatinumReceiver = _PlatinumReceiver;
        afReceiver = _afReceiver;
