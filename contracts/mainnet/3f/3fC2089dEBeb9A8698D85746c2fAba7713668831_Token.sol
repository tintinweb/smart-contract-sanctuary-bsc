...
contract Token is ERC20, Ownable {
    ...
    address public marketWallet = 0xD6d4cEd17a6B2402b0E143BDA110587aCED6bff3;//改为自己的营销钱包

    uint256 public numTokensSellToSwap = 10000 * 1e18; //改兑换门槛
    
    uint256 public _buyLpFee = 10;  //改滑点，5对应0.5， 25对应2.5
    ...
    constructor() ERC20("LanternRabbit", "LanternRabbit") {  //改币名
        ...
        */
        _mint(owner(), 100000000000 * 1e18);  //改总量,现在是1亿
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        if(takeFee) {
            ...
            if(sender == uniswapV2Pair) { //buy
                totalFee = _buyLpFee + _buyMarketingFee;
            } else if (recipient == uniswapV2Pair) {
                totalFee = _sellLpFee + _sellMarketingFee;
            } 

            if(totalFee > 0) {
                uint256 feeAmount = amount * totalFee / 1000;
    ...
    function swapAndDividend(uint256 tokenAmount) private lockTheSwap {
        uint totalBuyShare = _buyLpFee + _buyMarketingFee;
        uint totalSellShare = _sellLpFee + _sellMarketingFee;
