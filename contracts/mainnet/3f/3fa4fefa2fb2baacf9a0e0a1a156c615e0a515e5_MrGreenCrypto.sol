...
contract MrGreenCrypto is IBEP20 {
    string constant _name = "MrGreenTestsANewContract";
    string constant _symbol = "GREENTESTOPU";
    ...
    function startHappyHour(uint256 howManyHours) external onlyOwner{
        require(howManyHours <= 24 && !happyHour && happyHourEnd < block.timestamp - 24 hours, "Happy hour can only be called once every 24 hours and has a minimum cooldown of 24 hours");
        happyHour = true;
    ...
    function airdropToWalletsAndVest(address[] memory airdropWallets, uint256[] memory amount, uint256 unlockAtLaunch, uint256 unlockPerMonth) external onlyOwner {
        ...
        for (uint256 i = 0; i < airdropWallets.length; i++) {
            ...
            if(unlockAtLaunch<100){
                ...
                percentageAtLaunch[wallet] = unlockAtLaunch;
                percentageEveryMonth[wallet] = unlockPerMonth;
            } 
        }
    }
