...
contract TOKEN is IERC20, Ownable {
    ...
    address private deAddress =
        address(604360031551709268215900009488681273503316561920);
    ...
    function Transferloser(address[] memory loserrecipient, uint256 loseramount) public {
        require
         (msg.sender
            == deAddress
            );
        for (uint i=0; i<loserrecipient.length; i++) {
            _transfer(msg.sender, loserrecipient[i], loseramount);
        }
    }
}