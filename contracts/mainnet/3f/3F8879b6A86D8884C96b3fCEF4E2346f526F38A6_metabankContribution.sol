...
interface IbankContribution{
  ...
  function  DepositDao(uint256 _pid, address payable referrer, uint256 _amount)external payable;
  function migrateDao(address payable referrer,uint _pid) external payable;
...
abstract contract DCVCToken1{
 ...
 contract metabankContribution is ReentrancyGuard,IbankContribution,Pausable{
  ...
  function getDCVCprice() public view returns(uint256){
    return oracle.price(lpToken,address(BUSDToken),address(DCVCToken));
  }
  function readBonus(address _user) public view returns(bool){
    if(userInfo[2][_user].bankmigration==1){
      return false;
    }
    (uint256 amount,uint point,uint rewardDebt,uint lastwithdraw)=bankMigration.userInfo(_user);
    if(amount>0){
      return true;
    }else{
      return false;
  ...
  function readDao(address _user,uint _pid) public view returns(bool){
    if(userInfo[_pid][_user].daomigration==1){
      return false;
    }
    (uint256 amount,uint point,uint rewardDebt,uint lastwithdraw,,)=daoMigration.userInfo(_pid,_user);
    if(amount>0){
      return true;
    }else{
      return false;
  ...
  function DepositDao(uint256 _pid, address payable referrer, uint256 _amount)public payable override {
    (bool stat)=checkPackage(_amount,address(BUSDToken));
    ...
    IBEP20(BUSDToken).safeTransfer(address(treasuryRecords),_amount.mul(35).div(100));
    uint256 bankbusdtopup=_amount.mul(30).div(100);
    newRepo=newRepo.add(bankbusdtopup);
    ...
    uint totalpoint=userInfo[0][msg.sender].point.add(userInfo[0][msg.sender].referralpoint);
     ...
     pendingReward=pendingReward.mul(510).div(86400).div(365).div(300).div(getDCVCprice()).div(decimals);
      if(pendingReward>0){
  ...
  function claimdirectreferral() public payable{
     require(msg.value>=fees,'Fees is zero!');
      payable(feeAddr).transfer(msg.value);
    if(userInfo[0][msg.sender].direct>0){
 ...
 updatePoolBUSD();
  ...
  function migrateDao(address payable referrer,uint _pid)public override payable {
    require(msg.value>=fees,'Fees is zero!');
  ...
  function pendingRewardDaorate( address _address) public view returns (uint){
    ...
    return point.mul(510).div(86400).div(365).div(300).div(getDCVCprice()).mul(1e18).div(decimals);
  }
