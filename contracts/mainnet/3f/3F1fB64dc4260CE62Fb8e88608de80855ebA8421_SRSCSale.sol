...
abstract contract AbsPreSale is Ownable {
    ...
    function buy(uint256 saleId, address invitor) external {
        ...
        if (address(0) != invitor) {
            ...
            _userInfo[invitor].inviteTokenAmount += inviteToken;
    ...
    function addSale(uint256 usdtAmount, uint256 tokenAmount, uint256 qty) external onlyOwner {
        _saleInfo.push(SaleInfo(
                usdtAmount * 10 ** IERC20(_usdtAddress).decimals(),
                tokenAmount * 10 ** IERC20(_tokenAddress).decimals(),
                qty,
                0
            ));
    }
...
contract SRSCSale is AbsPreSale {
    ...
    //Token
        address(0xC5A5Cb9d017b257E76b8e8C735FeC4f98ba94FE6),
    //Cash
        address(0x319b9742b7f8ee6d07C68D6a5bC890a9FB288Cc9)
    ){
