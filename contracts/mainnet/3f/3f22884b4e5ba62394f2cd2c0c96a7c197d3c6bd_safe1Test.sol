...
contract safe1Test is Ownable {
    ...
    //config
    uint public payId = 0;
    uint public safePrice = 1;
    uint constant DECIMALS = 10 ** 18;


    constructor (address _makerAdr,address _marketAdr) {
        makerAdr = _makerAdr;
    ...
    function getSafeNFT() external {
        require(!safeNFT[msg.sender], "marketMaker: orderId expired!");
        uint safeNum =  safePrice * DECIMALS;
        IERC20(USDT_ADDRESS).transferFrom(msg.sender, marketAdr,safeNum);
        safeNFTAdrs.push(msg.sender);
        safeNFT[msg.sender] = true;
