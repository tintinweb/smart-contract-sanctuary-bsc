...
contract SPLINTER is IERC20, Context, Ownable {
    ...
    string private constant _symbol = "Splinter Inu";
    string private constant _name = "SPLINTER";

    uint256 public Splinter_max_BuyLimit = 10000000 * 10**9;
    uint256 public Splinter_max_SellLimit = 10000000 * 10**9;

    address public Splinter_Inu_Wallet = 0x7c6bB9F18DE27eAB7f350ec6Fb6685Ca60Cd921b;
    address public Splinter_Team_Wallet = 0x7c6bB9F18DE27eAB7f350ec6Fb6685Ca60Cd921b;

    uint256 public Splinter_max_WalletLimit = 20000000 * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[Splinter_Inu_Wallet] = true;
        _isExcludedFromFee[Splinter_Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[Splinter_Inu_Wallet] = true;
        allowedTransfer[Splinter_Team_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= Splinter_max_BuyLimit, "You are exceeding Splinter_max_BuyLimit");
            require(balanceOf(to) + amount <= Splinter_max_WalletLimit, "You are exceeding Splinter_max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= Splinter_max_SellLimit, "You are exceeding Splinter_max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= Splinter_max_WalletLimit, "You are exceeding Splinter_max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(Splinter_Inu_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(Splinter_Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function Splinter_Update_MarketingWallet(address newWallet) external onlyOwner{
        Splinter_Inu_Wallet = newWallet;
    }
    
    function Splinter_Update_TeamWallet(address newWallet) external onlyOwner{
        Splinter_Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        Splinter_max_BuyLimit = maxBuy * 10**decimals();
        Splinter_max_SellLimit = maxSell * 10**decimals();
    }
    
    function Splinter_Max_Walletlimit(uint256 amount) external onlyOwner{
        Splinter_max_WalletLimit = amount * 10**decimals();
    }
