...
contract Municipality is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    /// @notice Get price for purchaseParcels for Gymnet use purposes
    function getPriceForPurchaseParcels (uint256 _parcelsCount) public view returns(uint256, uint256) {
        (uint256 rawParcelsPrice, ) = _getPriceForParcels(_parcelsCount);
        uint256 percentage;
        if(_parcelsCount >= 90) {
            percentage = 35187;
        } else if(_parcelsCount >= 35) {
            percentage = 28577;
        } else if(_parcelsCount >= 16) {
            percentage = 21875;
        } else if(_parcelsCount >= 3) {
            percentage = 16667;
        }
        uint256 purchaseParcelsPrice = _discountPrice(rawParcelsPrice, percentage);
        return (purchaseParcelsPrice, rawParcelsPrice);
    }
    ...
    function purchaseParcels(uint256 _parcelsAmount, uint256 _referrerId) external onlySaleActive notZeroAddress {
        ...
        INetGymStreet(netGymStreetAddress).addGymMlm(msg.sender, _referrerId);
        (uint256 purchaseParcelsPrice, ) = getPriceForPurchaseParcels(_parcelsAmount);
        _transferToContract(purchaseParcelsPrice);
        userToPurchasedAmountMapping[msg.sender] += purchaseParcelsPrice;
        _updateAdditionLevel(msg.sender);
        lastPurchase.dollarValue += purchaseParcelsPrice;
        _lastPurchaseDateUpdate(msg.sender);
        IAmountsDistributor(amountsDistributorAddress).distributeAmounts(
            purchaseParcelsPrice,
        ...
        emit PurchaseMade(msg.sender, PURCHASE_TYPE_PARCELS, nftIds, purchaseParcelsPrice);
    }

    /// @notice Get price for purchaseMiners for Gymnet use purposes
    function getPriceForPurchaseMiners(uint256 _minersCount) public view returns(uint256, uint256) {
    ...
    function purchaseMiners(uint256 _minersCount, uint256 _referrerId) external onlySaleActive notZeroAddress {
        ...
        INetGymStreet(netGymStreetAddress).addGymMlm(msg.sender, _referrerId);
        (uint256 PurchasePrice, ) = getPriceForPurchaseMiners(_minersCount);
        ...
        uint256[] memory nftIds = new uint256[](0);
        emit PurchaseMade(msg.sender, PURCHASE_TYPE_MINERS, nftIds, PurchasePrice);
    }
