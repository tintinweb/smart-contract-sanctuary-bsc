...
contract MMMPool is Context, Initializable {
    ...
    function joinPool() external {
        require((_userPos[msg.sender].length < 30) || _isFLK[msg.sender], "full slot!!!");
        ...
        emit CurrentPool(_currentJoinTickets);
    }
    ...
    function joinPool(address[] memory usr) external onlyOwner {
    ...
    event CurrentPool(uint256 current);
}