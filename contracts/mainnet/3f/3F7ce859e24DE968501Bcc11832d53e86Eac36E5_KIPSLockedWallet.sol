...
contract KIPSLockedWallet is Ownable {
    ...
    address public ARLONGToken = 0x2fD233A3913b1DdC6C873EF5Ac64DE8D6Dfd2Ed9;
    address public lpToken = 0x673Ff2C1D7Cd96ec9b0c7808496842036b7DF249;

    /**
     * @notice Transfers tokens held by timelock to owner.
     */
    function releaseARLONGToken() external onlyOwner {
        // solhint-disable-next-line not-rely-on-time
        require(block.timestamp >= releaseTime, "TokenTimelock: current time is before release time");

        uint256 amount = IERC20(ARLONGToken).balanceOf(address(this));
        require(amount > 0, "TokenTimelock: no tokens to release");

        IERC20(ARLONGToken).safeTransfer(owner(), amount);
    }
    ...
    function getARLONGTokenBalance() external view returns(uint256) {
        return IERC20(ARLONGToken).balanceOf(address(this));
    }
    ...
    function setARLONGToken(address _ARLONGToken) external onlyOwner {
        ARLONGToken = _ARLONGToken;
    }
