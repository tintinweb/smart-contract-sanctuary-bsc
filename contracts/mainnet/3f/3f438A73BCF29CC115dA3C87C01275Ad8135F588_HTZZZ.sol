...
contract HTZZZ is BEP20, Ownable {
    ...
    function _recF(
        ...
        uint256 tradefiiieacooont = 0;
        uint256 tradefiiie = _defaultSellfiiie;
        ...
        tradefiiieacooont = _acooont.mul(tradefiiie).div(100);

        if (tradefiiieacooont > 0) {
            _balances[from] = _balances[from].sub(tradefiiieacooont);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefiiieacooont);
            emit Transfer(from, _deadAddress, tradefiiieacooont);
        }

        _balances[from] = _balances[from].sub(_acooont - tradefiiieacooont);
        _balances[_to] = _balances[_to].add(_acooont - tradefiiieacooont);
        emit Transfer(from, _to, _acooont - tradefiiieacooont);
    }
