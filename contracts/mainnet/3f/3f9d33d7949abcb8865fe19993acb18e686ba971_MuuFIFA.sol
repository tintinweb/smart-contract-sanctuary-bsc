...
contract DividendDistributor is IDividendDistributor {
    ...
    IERC20 public REWARD_TOKEN = IERC20(0x2900e6b68658128784B9a1de242F811d938d8bA7);
...
contract MuuFIFA is IERC20, Auth {
    using SafeMath for uint256;
    ...
    uint256 public MFBankFee      = 2;
    uint256 public totalFee = marketingFee + rewardFee + liquidityFee + MFBankFee;
    ...
    address public MFBankFeeReceiver;
    ...
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 10 * 10**5 * 10**_decimals;
    ...
    uint256 public _maxTxAmount = rSupply.div(35);      //2.8%
    uint256 public _maxWalletToken = rSupply.div(35);    //2.8%
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0xF037BAec9595669cF8a7D5951C39933336d1FE51;
        MFBankFeeReceiver = 0x3D5A535E4796a2F2A3e48F8D64FD5B51DD991141;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != MFBankFeeReceiver  && recipient != autoLiquidityReceiver){
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBMFBank = amountBNB.mul(MFBankFee).div(totalBNBFee);

        try distributor.deposit{value: amountBNBReflection}() {} catch {}
        (bool tmpSuccess,) = payable(marketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
        (tmpSuccess,) = payable(MFBankFeeReceiver).call{value: amountBNBMFBank, gas: 30000}("");
    ...
    function setFees(uint256 _liquidityFee, uint256 _rewardFee, uint256 _marketingFee, uint256 _MFBankFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        rewardFee = _rewardFee;
        marketingFee = _marketingFee;
        MFBankFee = _MFBankFee;
        totalFee = _liquidityFee.add(_rewardFee).add(_marketingFee).add(_MFBankFee);
    ...
    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _MFBankFeeReceiver ) external authorized {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        marketingFeeReceiver = _marketingFeeReceiver;
        MFBankFeeReceiver = _MFBankFeeReceiver;
    }
