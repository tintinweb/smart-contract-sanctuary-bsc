...
// Copyright (c) Inverse Protocol 09.06.2022
...
contract INVERSE is ERC20Detailed, Ownable, WhitelistedRole {
    ...
    bool public autoRebase = false; // Should be enabled at launch
    bool public feesOnNormalTransfers = true; // Important to have this enabled
    bool public isLiquidityInBnb = true;

    uint256 public rewardYield = 100000; // 10.000% Yield
    uint256 public rewardYieldDenominator = 10000000000;
    uint256 public maxSellTransactionAmount = 2500000 * 10 ** 18;

    uint256 public rebaseFrequency = 900; // 15min = 900
    ...
    uint256 private constant MAX_REBASE_FREQUENCY = 900; // 15min
    uint256 private constant DECIMALS = 18;
    uint256 private constant MAX_UINT256 = ~uint256(0);
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 100 * 10**3 * 10**DECIMALS; // 100 = 100.000
    ...
    address public treasuryReceiver = 0x895b4aE3AD4534F9852778e5e19796486943A6a4; // TREASURY WALLET
    address public insuranceFundReceiver = 0xf4990DA1868C03511037846b69FaFDf9249A1E78; // INSURANCE FUND WALLET
    address public busdToken = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56; // BUSD ADDRESS
    ...
    uint256 public buyFeeInsurance = 2;
    uint256 public sellFeeTreasuryAdded = 5;
    uint256 public sellFeeInsuranceAdded = 5;
    uint256 public totalBuyFee = liquidityFee.add(treasuryFee).add(buyFeeInsurance); // 5% BUYING
    uint256 public totalSellFee = totalBuyFee.add(sellFeeTreasuryAdded).add(sellFeeInsuranceAdded); // 15% SELLING
    uint256 public feeDenominator = 100;
    uint256 public transferTax = 100; // 100% Transfer Tax if enabled
    ...
    constructor() ERC20Detailed("INVERSE", "INVERSE", uint8(DECIMALS)) {
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountToRFV = contractTokenBalance.mul(buyFeeInsurance.mul(2).add(sellFeeInsuranceAdded)).div(realTotalFee);
    ...
    function setFees(uint256 _liquidityFee, uint256 _riskFreeValue, uint256 _treasuryFee, uint256 _sellFeeTreasuryAdded, uint256 _sellFeeInsuranceAdded, uint256 _feeDenominator) external onlyOwner {
        require(
            _liquidityFee <= MAX_FEE_RATE &&
            _riskFreeValue <= MAX_FEE_RATE &&
            _treasuryFee <= MAX_FEE_RATE &&
            _sellFeeTreasuryAdded <= MAX_FEE_RATE &&
            _sellFeeInsuranceAdded <= MAX_FEE_RATE,
            "wrong"
        );

        liquidityFee = _liquidityFee;
        buyFeeInsurance = _riskFreeValue;
        treasuryFee = _treasuryFee;
        sellFeeTreasuryAdded = _sellFeeTreasuryAdded;
        sellFeeInsuranceAdded = sellFeeInsuranceAdded;
        totalBuyFee = liquidityFee.add(treasuryFee).add(buyFeeInsurance);
        totalSellFee = totalBuyFee.add(sellFeeTreasuryAdded).add(sellFeeInsuranceAdded);
