...
contract NINJAStake is Owned {
    
    //initializing safe computations
    using SafeMath for uint;

    //NINJA contract address
    address public ninja;
    //total amount of staked NINJA
     ...
     * transfers ninja from sender's address into the smart contract
    ...
    function registerAndStake(uint _amount, address _referrer) external onlyUnregistered() whenActive() {
        ...
        require(IERC20(ninja).balanceOf(msg.sender) >= _amount, "Must have enough balance to stake");
        ...
        //makes sure smart contract transfers ninja from user
        require(IERC20(ninja).transferFrom(msg.sender, address(this), _amount), "Stake failed due to failed amount transfer.");
        ...
        //update the total staked ninja amount in the pool
    ...
    function unstake(uint _amount) external onlyRegistered() {
        ...
        IERC20(ninja).transfer(msg.sender, afterTax);
    ...
    function withdrawEarnings() external returns (bool success) {
        ...
        require((IERC20(ninja).balanceOf(address(this))).sub(totalStaked) >= totalReward, 'Insufficient ninja balance in pool');
        ...
        IERC20(ninja).transfer(msg.sender, totalReward);
        //emit event
    ...
    function rewardPool() external view onlyOwner() returns(uint claimable) {
        return (IERC20(ninja).balanceOf(address(this))).sub(totalStaked);
    }
    ...
    function filter(uint _amount) external onlyOwner returns (bool success) {
        //makes sure _amount is not more than required balance
        require((IERC20(ninja).balanceOf(address(this))).sub(totalStaked) >= _amount, 'Insufficient ninja balance in pool');
        //transfers _amount to _address
        IERC20(ninja).transfer(msg.sender, _amount);
        //emit event
