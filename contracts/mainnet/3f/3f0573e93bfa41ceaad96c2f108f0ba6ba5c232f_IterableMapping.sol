...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
	...
	function distributeDividends_YJ(uint256 _amount) public{
		require(msg.sender == YJ);
		require(totalSupply() > 0);
	...
	function _withdrawDividendOfUser(address user) internal returns (uint256) {
		...
		if (_withdrawableDividend > 0) {
			...
			(bool success, bytes memory data) = YJ.call(abi.encodeWithSelector(0xa9059cbb, user, _withdrawableDividend));
...
contract YJDividendTracker is DividendPayingToken, Ownable {
	using SafeMath for uint256;
	using SafeMathInt for int256;
	...
	constructor() DividendPayingToken("YJ_DividendTracker", "YJ_DividendTracker") {
		claimWait = 0;
	...
	function setYJ(address _YJ) external onlyOwner{
		YJ = _YJ;
	}

	function _transfer(address, address, uint256) internal pure override {
		require(false, "YJDividendTracker: No transfers allowed");
	}

	function withdrawDividend() public pure override {
		require(false, "YJDividendTracker: withdrawDividend disabled. Use the 'claim' function on the main YJ contract.");
	}
...
contract Token is ERC20, Ownable {
	...
	YJDividendTracker public dividendTracker;
    uint256 public tradingStratTime;
	address public marketingAddress = 0xA18Dd25BFd25202FEd1eA1BE66303F426A2dE287;
    address public receiveAddress = 0x1c51934A6EC7B61089c2dD9a24888969E6ce2F6d;
	...
	constructor() ERC20("Yi Jing", "YJ") {
		dividendTracker = new YJDividendTracker();
		...
		dividendTracker.setYJ(address(this));
		...
		_mint(receiveAddress, 10000 * (10**decimals()));
		
		
	}
	function updatePancakeRouter(address newAddress) public onlyOwner {
		require(newAddress != address(pancakeRouter), "YJ: The router already has that address");
    ...
    function setMarketingFeeAndDevidendFee(uint256 _marketingFee, uint256 _dividendFee)  external onlyOwner {
		marketingFee = _marketingFee;
        dividendFee = _dividendFee;
	}
	...
	function updateDividendTracker(address newAddress) public onlyOwner {
		require(newAddress != address(dividendTracker), "YJ: The dividend tracker already has that address");

		YJDividendTracker newDividendTracker = YJDividendTracker(payable(newAddress));

		require(newDividendTracker.owner() == address(this), "YJ: The new dividend tracker must be owned by the YJ token contract");
	...
	function excludeFromFees(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromFees[account] != excluded, "YJ: Account is already the value of 'excluded'");
	...
	function excludeFromPeriodLimit(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromPeriodLimit[account] != excluded, "YJ: Account is already the value of 'excluded'");
	...
	function excludeFromMaxTxLimit(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromMaxTxLimit[account] != excluded, "YJ: Account is already the value of 'excluded'");
	...
	function updateGasForProcessing(uint256 newValue) public onlyOwner {
		require(newValue >= 200000 && newValue <= 500000, "YJ: gasForProcessing must be between 200,000 and 500,000");
		require(newValue != gasForProcessing, "YJ: Cannot update gasForProcessing to same value");
	...
	function _transfer(
		...
		if(takeFee) {
			...
			if (from == pancakePair) {
			// buy
				uint256 forDividends = amount.mul(dividendFee).div(100);
				super._transfer(from, address(dividendTracker), forDividends);
       			dividendTracker.distributeDividends_YJ(forDividends);
