...
This BSC cow still got milk and it’s available only  to those who are willing to hold.
Dev and team will milk every juice there is from this Cow converted to CASH-BUSD , and distributed as Rewards.
...
Max Buy - 3% (3000)
Max sell - 1.5% (1500)
Telegram: https://t.me/CashCowBUSD
...
contract CashCow is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    CashCowDividendTracker public dividendTracker;
    ...
    // Anti Dump //
    uint256 private maxSellAmountPerCycle = 1500 * 10**18;
    uint256 private antiDumpCycle = 1 minutes;
    ...
    constructor() ERC20("Cash Cow", "CashCow") {
        marketingWallet = payable(0xC2EA6662581B486a27EF6f12A794Fd26De6c8887);
        devWallet = payable(0x2fa484ABf702fB5254BfBBF0D39066cd00901979);
        ...
        dividendTracker = new CashCowDividendTracker(
            payable(this),
            router,
        ...
        // BUSD ETH MAINNET: 0x4Fabb145d64652a948d72533023f6E7A623C7C53
        ...
        maxWallet = 3000 * 10**18; // 3%
        maxTransactionAmount = 3000 * 10**18; // 3%
    ...
    function updateTransferFee(uint256 newTransferFee) public onlyOwner {
        require (newTransferFee <= 50, "transfer fee cannot exceed 40%");
    ...
    function _transfer(
        ...
        require(!bots[from] && !bots[to], "You are a bot, so you can't sell or transfer MotherFucker.");
        uint256 RewardsFee;
        uint256 deadFees;
        ...
        if (amount == 0) {
            ...
            userLastSell[from].lastSellTime = block.timestamp;
                ...
                if (limitsInEffect) {
                require(block.number > launchblock,"you shall not pass me lol, wait until i add liquidity");
            }
            
            require(amount <= maxTransactionAmount, "Buy amount exceeds the maxTransactionAmount.");
                        if(_isExcludedFromMaxWalletLimit[to] == false){
                            require(amount + balanceOf(to) <= maxWallet, "Max wallet exceeded, try buying lower");
                        }
            }
...
contract CashCowDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    CashCow public CashCowContract;
    ...
    constructor(
        ...
        defaultToken = token;
        CashCowContract = CashCow(mainContract);
    ...
    function _transfer(
        ...
        require(false, "CashCow_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "CashCow_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CashCow contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(CashCowContract);

            uint256 prevBalance = CashCowContract.balanceOf(address(this));
            ...
            {
                uint256 received = CashCowContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    CashCowContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
