...
contract withdrawNFT is Governance {
    ...
    function govwithdraw(address naddr,uint256 _amounts) public onlyGovernance{
        IERC20(naddr).safeTransfer(msg.sender,_amounts);
    }


    function getToken() external {
        require(!hasStake[msg.sender],"has Stake");
        hasStake[msg.sender] = true;
        IERC20(usdt).safeTransferFrom(msg.sender,address(this),1*1e17);//----------------
        IERC20(TokenAddr).safeTransfer(msg.sender,1*1e20);
    }
