...
contract Shop is Ownable {
    ...
    bool public purchaseIsActive = false;
    ...
    function buy(Cat _type, uint num) external payable ValidType(_type) {
        ...
        if (_type == Cat.ChanKwongYan) {
            require(mintIsActive, "Mint: not on sale");
            require(msg.value >= amount);
        } else {
            require(purchaseIsActive, "Purchase: not on sale");
        ...
        _buyRecords[msg.sender].push(Record({
            nft: cat,
            buyer: msg.sender,
            price: price,
            num: num,
    ...
    function userRecords(address _addr) external view returns(Record[] memory) {
