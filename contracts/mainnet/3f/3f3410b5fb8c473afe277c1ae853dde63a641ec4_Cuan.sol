...
contract Cuan is Initializable, ERC20Upgradeable, PausableUpgradeable, AccessControlEnumerableUpgradeable, OwnableUpgradeable {
...
/* ---------------------------------- Token --------------------------------- */
	...
	string internal constant NAME = "CuanTest6";
	string internal constant SYMBOL = "CTEST6";
...
/* ---------------------------------- EXEMPTION --------------------------------- */
    ...
    function mint(address to, uint256 amount) external onlyMinter {
        ...
        require (admin!=address(0), "admin wallet cannot be zero");
...
/* ---------------------------------- METADATA ---------------------------------- */
	...
	function name() public pure override returns(string memory) { return NAME;}
	function symbol() public pure override returns(string memory) { return SYMBOL;}
...
/* ---------------------------------- Taxes ---------------------------------- */
	...
	function setTaxBuy(uint256 _percent)external onlyOwner{
		require(_percent <= 100, "Should be lower than 90%");
		taxBuy = _percent;
	}
...
/* --------------------------------- IERC20 --------------------------------- */
	...
	function _tokenTransfer(
		...
		bool isBuyType = getTransactionType(sender, recipient) == TransactionType.BUY;
		// Remove fees for this transaction if needed.
		if (!take)
			removeAllFee();

		// Calculate all reflection magic...
		TValues memory tV = _getValues(amount,isBuyType);
		...
		_balances[recipient] += tV.tTransferAmount;
...
/* -------------------------------- Helpers ------------------------------- */
    ...
    function rescueBEP20Tokens(address tokenAddress) external onlyOwner{
        IERC20Upgradeable(tokenAddress).transfer(msg.sender, IERC20Upgradeable(tokenAddress).balanceOf(address(this)));
    }
