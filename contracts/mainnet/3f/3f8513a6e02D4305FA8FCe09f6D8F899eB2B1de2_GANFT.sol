...
contract GANFT is ERC721Enumerable, Ownable {
   
    string private _baseUri;
   
  
    constructor(string memory _name, string memory _symbol, string memory baseUri) ERC721(_name, _symbol) {
      string memory _address = Strings.toHexString(uint256(uint160( address(this))), 20);
      _baseUri = string(abi.encodePacked(baseUri, _address, "/"));
    }

    function setBaseURI( string memory _newVal) external onlyOwner{
     _baseUri = _newVal;
    }
