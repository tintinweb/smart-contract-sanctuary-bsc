...
contract MINE {
    ...
    mapping(address=>bool)public My;
    uint256 sumPower;
    constructor () public {
    }
    receive() external payable{
  }
  function buyMinerUA(address up)public{
      require(v1[msg.sender]==0);
      uint mine=getMINE(100 ether);
      ERC20(DEX).transferFrom(msg.sender,address(this),mine);
      v1[msg.sender]=mine;
      v1time[msg.sender]=block.timestamp;
      v1Value[msg.sender]=0.0000007 ether;
      sumPower+=mine;
      ERC20(DEX).transfer(0x000000000000000000000000000000000000dEaD,mine *5/100);
      if(up!=msg.sender && upAddress[msg.sender]==address(0)){
         My[msg.sender]=true;
         upAddress[msg.sender]=up;
      }
      if(upAddress[msg.sender] !=address(0)){
         ERC20(DEX).transfer(upAddress[msg.sender],mine *5/100); 
      }
      _swap(mine * 85/100);
  }
  function buyMinerUB(address up)public{
      require(v2[msg.sender]==0);
      uint mine=getMINE(500 ether);
      ERC20(DEX).transferFrom(msg.sender,address(this),mine);
      v2[msg.sender]=mine;
      v2time[msg.sender]=block.timestamp;
      v2Value[msg.sender]=0.0000016 ether;
      sumPower+=mine;
      ERC20(DEX).transfer(0x000000000000000000000000000000000000dEaD,mine *5/100);
      if(up!=msg.sender && upAddress[msg.sender]==address(0)){
         My[msg.sender]=true;
  ...
  function buyMinerUC(address up)public{
      require(v3[msg.sender]==0);
      uint mine=getMINE(1000 ether);
      ERC20(DEX).transferFrom(msg.sender,address(this),mine);
      v3[msg.sender]=mine;
      v3time[msg.sender]=block.timestamp;
      v3Value[msg.sender]=0.000004 ether;
      sumPower+=mine;
      ERC20(DEX).transfer(0x000000000000000000000000000000000000dEaD,mine *5/100);
      if(up!=msg.sender && upAddress[msg.sender]==address(0)){
         My[msg.sender]=true;
  ...
  function getUser(address addr)public view returns(bool a,uint b,uint c,uint d){
      a=My[addr];
      b=getMINE(100 ether);
      c=getMINE(500 ether);
      d=getMINE(1000 ether);
  }
  function getMINE(uint256 _usdt)public view returns(uint256){
        address[] memory path = new address[](3);
        path[0]=0x55d398326f99059fF775485246999027B3197955;
        path[1]=0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
        path[2]=0x6e0c46Ec78525c4b238929265DfB61769823Fb20;
        uint256 [] memory amount;
        amount=IPancakeRouter01(0x10ED43C718714eb63d5aA57B78B54704E256024E).getAmountsOut(_usdt,path);
         return amount[2];
    }
