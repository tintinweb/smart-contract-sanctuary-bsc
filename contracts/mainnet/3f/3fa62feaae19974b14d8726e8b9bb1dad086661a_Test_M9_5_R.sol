...
//////
 ...
 //bool public activTORSREF_REWARD;//bool activate
    ...
    address  SE_total_LevelReferrals1US_ADR;
       ...
       uint256  DEP_activatesRefer2;
       uint256  DEP_activatesRefer3;
...
if (user.REG_activatesRefer >= 1)
{
    ...
    user.DEP_activatesRefer = 2;
        ...
        function GET_DEP_activatesRefer2(address referrer) public view returns(uint256) {
                    User storage user = users[referrer];

        return  user.DEP_activatesRefer2;
    }

        function GET_DEP_activatesRefer3(address referrer) public view returns(uint256) {
                    User storage user = users[referrer];
if(user.REG_activatesRefer >=1)
{user.DEP_activatesRefer3 == 5;
}
else {user.DEP_activatesRefer3==11;}

        return  user.DEP_activatesRefer3;
    }
...
function withdraw () public {
 ...
 if (user.levels[0] >= 1   /*&& user.checkpointLVR1RefBonus >= 5 minutes*/)
