...
contract BUSDDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor() public DividendPayingToken("BUSD_Dividen_Tracker", "BUSD_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 50 * (10**4) * (10**18); //must hold 500,000+ tokens
    }
...
pragma solidity ^0.6.12;
contract Test11Finance is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public _DevWalletAddress = 0x932850589EDc2aA64E0cFCa621ae9ac0314426Ff;
    address public _FoudWalletAddress = 0x6d0963d05AdEbf468A9Cce91B02BfC84D468d208;
    address public _BurnAddress = 0x426B6E28d6F3Ad2695061151591d2E1C7723526B;

    address public immutable BUSD = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);

    uint256 public totalSupply_ = 10 * (10**8) * (10**18);
    ...
    uint256 public BUSDRewardsShare = 2;
    ...
    constructor() public ERC20("Test11Finance", "Test11Finance") {
    ...
    function setFee(
        uint256 _BUSDRewardsFee,
        uint256 _BUSDRewardsShare
    ) public onlyOwner {
        if (_BUSDRewardsFee > 10) {
            _BUSDRewardsFee = 10;
        }
        if (BUSDRewardsShare > 10) {
            BUSDRewardsShare = 10;
        }
        BUSDRewardsFee = _BUSDRewardsFee;
        BUSDRewardsShare = _BUSDRewardsShare;
    }

    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BUSD: The dividend tracker already has that address");

        BUSDDividendTracker newDividendTracker = BUSDDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BUSD: The new dividend tracker must be owned by the BUSD token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BUSD: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BUSD: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BUSD: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BUSD: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BUSD: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if (from != address(_PresaleAddress)) {
            if(liquidityLaunched && block.number <= lastSnipeTaxBlock && !isFirstLaunch){
                uint256 fees = amount.mul(BUSDRewardsFee).div(100);
        ...
        if( !isJustTransfer && canSwap &&
            ...
            uint256 tokenAmountForLP = tokenAmount.mul(LPShare).div(BUSDRewardsFee);
            ...
            uint256 BNBFee = BUSDRewardsFee.sub(LPShare);

            uint256 BNBAmountForReward = BNBAmount.mul(BUSDRewardsShare).div(BNBFee);

            uint256 BNBAmountForMarket = BNBAmount.sub(BNBAmountForReward);
            uint256 BNBAmountForBurn = BNBAmountForMarket.div(7);
            uint256 BNBAmountForRest = BNBAmountForMarket.sub(BNBAmountForBurn);
            uint256 BNBAmountForApp = BNBAmountForRest.div(2);
            uint256 BNBAmountForFoud = BNBAmountForRest.sub(BNBAmountForApp);

            payable(_DevWalletAddress).transfer(BNBAmountForApp);
            payable(_FoudWalletAddress).transfer(BNBAmountForFoud);
            payable(_BurnAddress).transfer(BNBAmountForBurn);
