...
// File: contracts/farms/CST/common/IBEP20Metadata.sol
...
// File: contracts/farms/CST/CSTPool.sol
...
contract CSTPool is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    function initialize(
        ...
        rewardDecimals = IBEP20Metadata(address(rewardToken)).decimals();
    ...
    /* CST Admin functions to setup pool */

    function CSTStartIn(uint256 _blocks) public onlyOwner {
        CSTSetStart(block.number.add(_blocks));
    }

    function CSTSetStart(uint256 _startBlock) public onlyOwner {
    ...
    function CSTSetDuration(uint256 _durationBlocks) public onlyOwner {
        bonusEndBlock = startBlock.add(_durationBlocks);
        CSTCalcRewardPerBlock();
    }

    function CSTCalcRewardPerBlock() public onlyOwner {
        uint256 rewardBal = rewardToken.balanceOf(address(this));
        rewardPerBlock = rewardBal.div(rewardDuration());
    }

    function CSTSetRewardMaxTxAmount(uint256 _maxTxAmount) public onlyOwner {
...
// File: contracts/farms/CST/CSTPoolFactory.sol


pragma solidity 0.6.12;

contract CSTPoolFactory is Ownable {
    event NewCSTPool(address indexed pool);
    ...
    function createPool(
        ...
        bytes memory bytecode = type(CSTPool).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token, reward, now));
        address pool;
        ...
        CSTPool(pool).initialize(token, reward, rewardPerBlock, startBlock, endBlock, _poolLimitPerUser, _admin);

        emit NewCSTPool(address(pool));
    }
}