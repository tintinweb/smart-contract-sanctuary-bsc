...
contract FurMax is BaseContract
{
    ...
    {
        return _available(_availableFurbotDividends(participant_) + _availableFurpoolDividends(participant_));
    }
     ...
     */
    function _availableFurbotDividends(address participant_) public view returns (uint256)
    {
        uint256 _available_;
        if(totalFurbotInvestment > 0 && totalFurbotDividends > 0)
        {
            uint256 _totalDividends_ = (totalFurbotDividends * furbotInvestment[participant_]) / totalFurbotInvestment;
            if(_totalDividends_ >= furbotDividendsClaimed[participant_]) _available_ = _totalDividends_ - furbotDividendsClaimed[participant_];
        }
        return _available_;
    }
     ...
     */
    function _availableFurpoolDividends(address participant_) public view returns (uint256)
    {
        uint256 _available_;
        if(totalFurpoolInvestment > 0 && totalFurpoolDividends > 0)
        {
            uint256 _totalDividends_ = (totalFurpoolDividends * furpoolInvestment[participant_]) / totalFurpoolInvestment;
            if(_totalDividends_ >= furpoolDividendsClaimed[participant_]) _available_ = _totalDividends_ - furpoolDividendsClaimed[participant_];
        }
        return _available_;
    }

    /**
     * Update furpool dividends.
     */
    function _updateFurpoolDividends() internal
    {
    ...
    {
        uint256 _amount_ = _available(totalFurpoolPendingInvestment);
    ...
    /**
     * Available.
     * @param originalAmount_ Original amount.
     * @return uint256 Available amount.
     */
    function _available(uint256 originalAmount_) internal view returns (uint256)
    {
        uint256 _balance_ = _usdc.balanceOf(address(this));
        if(originalAmount_ > _balance_) originalAmount_ = _balance_;
    ...
    function withdrawFurbotPendingInvestment() external onlyOwner
    {
        ...
        uint256 _amount_ = _available(totalFurbotPendingInvestment);
