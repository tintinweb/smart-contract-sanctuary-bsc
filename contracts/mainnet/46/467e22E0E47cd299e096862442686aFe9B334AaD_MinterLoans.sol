...
contract MinterLoans is IMinterLoans, Ownable {
    ...
    function _buyWithLeverage(uint256 _coin1Amount, uint256 _amountOutMin) noContracts internal {
    ...
    function borrow(uint256 _collateralAmount) noContracts override external {
    ...
    function repayBNB(uint256 _loanId) noContracts payable override external {
        Loan memory loan = loans[_loanId];
        require(!loan.closed, "Loan has been already closed");
        require(loan.borrower == msg.sender, "Sender is not a borrower of loan");
    ...
    function sellAndRepay(uint256 _loanId, uint256 _amountInMax) noContracts override external {
        Loan memory loan = loans[_loanId];
        require(!loan.closed, "Loan has been already closed");
        require(loan.borrower == msg.sender, "Sender is not a borrower of loan");
    ...
    function liquidate(uint256 _loanId) noContracts override external {
        Loan memory loan = loans[_loanId];
