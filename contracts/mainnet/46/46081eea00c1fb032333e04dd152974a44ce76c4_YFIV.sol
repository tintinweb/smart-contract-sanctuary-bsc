...
abstract contract ERC20 is IERC20, ERC20Detailed, Config {
    ...
    function _transfer(
        ...
        if (whiteList[sender] || whiteList[recipient]) {
            ...
            if (sender == address(uniswapV2Pair)) { //buy
                ...
                _tokenBurn(recipient, amount.mul(burnFee).div(10000));
                _tokenTransfer(
                    recipient,
                    tokenBonusAddress,
                ...
                if (liquiditySwitch) {
                    ...
                    _tokenTransfer(recipient, address(this), liquidityAmount);
                    if (_balances[address(this)] >= minSwapAndLiquifyLimit) {
                        swapAndLiquify(minSwapAndLiquifyLimit);
                    }
                }

                if (usdtSwitch) {
                    usdtAmount = amount.mul(usdtFeeBuy).div(10000);
                    _tokenTransfer(recipient, usdtManageAddress, usdtAmount);
                }

                _balances[recipient] = _balances[recipient].sub(amount.mul(getFeeBuy()).div(10000));
                ...
                if (usdtSwitch) {
                    ...
                    _tokenTransfer(sender, usdtManageAddress, usdtAmount);
                }
    ...
    function swapAndLiquify(uint256 val) private lockTheSwap {
        reviseBalance(address(this));
        uint256 contractTokenBalance = val;
...
contract YFIV is ERC20 {
    ...
    constructor() ERC20Detailed("YFIV", "YFIV", 18) {
		...
		address firstAddress = msg.sender;  //0xb8132C4527741F178D426903d2610e4b0d5479e9
