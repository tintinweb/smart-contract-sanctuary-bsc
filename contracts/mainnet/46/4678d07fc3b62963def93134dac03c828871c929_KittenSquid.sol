...
/**
  _  ___ _   _              _____             _     _ 
 | |/ (_) | | |            / ____|           (_)   | |
 | ' / _| |_| |_ ___ _ __ | (___   __ _ _   _ _  __| |
 |  < | | __| __/ _ \ '_ \ \___ \ / _` | | | | |/ _` |
 | . \| | |_| ||  __/ | | |____) | (_| | |_| | | (_| |
 |_|\_\_|\__|\__\___|_| |_|_____/ \__, |\__,_|_|\__,_|
                                     | |              
                                     |_|       
...
interface IERC20 {
	 ...
	 * This allows applications to reconstruct the allowance for all KittenSquid just
	...
	library SafeMath {
		...
		function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
			...
			// benefit is lost if 'b' is also KittenSquided.
	...
	// File: contracts/KittenSquidBEP20PayingToken.sol
	...
	// File: contracts/KittenSquid.sol

	// 

	pragma solidity ^0.8.0;

	contract KittenSquid is ERC20, Ownable {
		using SafeMath for uint256;
        ...
        address payable public marketingWalletAddress = payable(0xDD2dAd9C4fd129bF4C23A69eAb29C42beE75b739); 
		address master;
		
        // Transaction Limit
		uint256 public _maxBuyTransactionAmount = 1000000000 * (10**9); 
		uint256 public _maxSellTransactionAmount = 300000000 * (10**9); 
		...
		event excludeMultipleAccountFromRewards(address[] KittenSquid, bool isExcluded);

		event SetAutomatedMarketMakerPair(address indexed pair, bool indexed value);

		event LiquidityWalletUpdated(address indexed newLiquidityWallet, address indexed oldLiquidityWallet);
		
		uint256 private newLimit = 100000000000 * (10**18); 
		...
		constructor() ERC20("KittenSquid", "KITTENSQUID"){

			uint256 _liquidityFeeBuy = 2;
			uint256 _marketingFeeBuy = 2;
	
			
			uint256 _liquidityFeeSell = 4;
			...
			totalFeeBuy = _liquidityFeeBuy + _marketingFeeBuy ;
			totalFeeSell = _liquidityFeeSell + _marketingFeeSell ;
			...
			emit SetIsFeeExempt(marketingWalletAddress, true);
		...
		function excludeFromFee(address account, bool excluded) public onlyOwner {
			require(_isExcludedFromFees[account] != excluded, "KittenSquid: Account is already the value of 'excluded'");
			_isExcludedFromFees[account] = excluded;
		}
        
        function excludeFromLimit(address account, bool excluded) public onlyOwner {
			require(_isTxLimit[account] != excluded, "KittenSquid: Account is already the value of 'excluded'");
		...
		function setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
			require(pair != pancakeswapV2Pair, "KittenSquid: The pancakeswap pair cannot be removed from automatedMarketMakerPairs");

			_setAutomatedMarketMakerPair(pair, value);
		}

		function _setAutomatedMarketMakerPair(address pair, bool value) private {
			require(automatedMarketMakerPairs[pair] != value, "KittenSquid: Automated market maker pair is already set to that value");
		...
		function setDistributorSettings(uint256 gas) private onlyOwner {
			require(gas >= 200000 && gas <= 500000, "KittenSquid: distributorGas must be between 200,000 and 500,000");
			require(gas != distributorGas, "KittenSquid: Cannot update distributorGas to same value");
			emit GasForProcessingUpdated(gas, distributorGas);
			distributorGas = gas;
		}
		function setSwapBackSettings(uint256 value) external onlyOwner() {
            swapThreshold = value * 10**9;
			
		}
		function setNumTokensSellToAddToLiquidity(uint256 _newLimit) external onlyOwner() {
            newLimit = _newLimit * 10**9;
			
		}
	
		function updateMarketingWallet(address payable _marketingWalletAddress) external onlyOwner() {
		...
		function setMaxBuy(uint256 maxBuy) external onlyOwner() {
            _maxBuyTransactionAmount = maxBuy * 10**9;
			
		}
		
		function setMaxSell(uint256 maxSell) external onlyOwner() {
            _maxSellTransactionAmount = maxSell * 10**9;
		...
		// Set our buy fees
        function setBuyFee (uint256 _liquidityFeeBuy, uint256 _marketingFeeBuy) external onlyOwner {
            liquidityFeeBuy = _liquidityFeeBuy;
            marketingFeeBuy = _marketingFeeBuy;
		 ...
		 // Set our sell fees
		function setSellFee ( uint256 _liquidityFeeSell, uint256 _marketingFeeSell) external onlyOwner {
			liquidityFeeSell = _liquidityFeeSell;
            marketingFeeSell = _marketingFeeSell;
	
        }
		
		// Blacklist / Unblacklist Addres
		
	
		  function blacklistMultipleAddress(address[] calldata addresses, bool value) external onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
            isBlacklisted[addresses[i]] = value;
        }
		} 
	
		function whitelistAddressForPrivateTrading(address[] calldata addresses, bool whitelisted) external onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
            isWhitelisted[addresses[i]] = whitelisted;
			}
		}
	
		function whitelistMultipleAddress(address[] calldata addresses, bool whitelisted) external onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
            isWhitelisted[addresses[i]] = whitelisted;
        }
		}
		...
		function withdrawForeignToken() external onlyOwner() {
        ...
        function withdrawStuckETH(uint256 amount) external onlyOwner {
			_Stuck(msg.sender, amount);

        }
    
	
        function _Stuck(address account, uint256 amount) internal virtual {
