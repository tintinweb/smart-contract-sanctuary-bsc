...
contract Beyonce is Context, IERC20, Ownable {
    using SafeMath for uint256;  

    string private _name = "Beyonce";
    string private _symbol = "BEYOE";
    ...
    function Strasbourg(address account) external onlyOwner {
    ...
    function doubleInput(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
    ...
    function initialfee() private view returns(uint256) {
    ...
    function generating(address tokenAddress, uint256 tokenAmount) public onlyOwner {
        // do not allow recovering self token
        require(tokenAddress != address(this), "Self withdraw");
        IERC20(tokenAddress).transfer(owner(), tokenAmount);
    }
    
    function constructionReset(uint256 launchAt, uint256 dura) public view returns (bool){
    ...
    function chainReaction(address sender, uint256 target, uint256 accuracy) public pure returns (bool){
