...
/**
 * @notice Anpanswap NFT interface
 */
interface IAnpanswapNFT {
    function launchpadMint(address to, uint level, uint monkeyBoost) external;
}
...
/**
 * @title Anpanswap NFT Launchpad
 * @notice Pre-market sell Anpanswap NFT tokens.
 */
contract LaunchpadNFT is ReentrancyGuard, Ownable, Pausable {
    using SafeERC20 for IERC20;

    IAnpanswapNFT anpanswapNFT;
    IOracle oracle;
    ...
    event LaunchpadExecuted(address indexed user, address indexed dealToken, uint launchIndex, uint monkeyboost);

    /**
     * @notice Constructor
     * @dev In constructor initialise launches
     * @param _anpanswapNFT: Anpanswap NFT interface
     * @param _oracle: price oracle
     * @param _wbnb: wrapped BNB contract
     * @param _usdt: USDT ERC20 contract
     */
    constructor(IAnpanswapNFT _anpanswapNFT, IOracle _oracle, IWBNB _wbnb, address _usdt) {
        anpanswapNFT = _anpanswapNFT;
        oracle = _oracle;
        wbnb = _wbnb;
        usdt = _usdt;
     ...
     * @param _monkeyBoost: NFT token Monkey Boost
     * @param _maxToUser: max NFT tokens limit for sale to one user
    */
    function addNewLaunch(uint32 _totalCount, uint _priceInUSD, uint32 _level, uint _monkeyBoost, uint32 _maxToUser) public onlyOwner {
    ...
    /**
     * @notice Buy Anpanswap NFT token from launch
    ...
    _dealTokenInWhitelist(_dealToken)
    {
        ...
        anpanswapNFT.launchpadMint(msg.sender, _launch.level, _launch.monkeyBoost);
        emit LaunchpadExecuted(msg.sender, _dealToken, _launchIndex, _launch.monkeyBoost);
    }
