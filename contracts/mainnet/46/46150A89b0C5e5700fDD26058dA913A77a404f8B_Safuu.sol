...
contract Safuu is ERC20Detailed, Ownable {
    ...
    uint256 public rebaseRate = 2520169;
    uint256 public firePitFee = 10;
    uint256 public treasuryFee = 20;
    uint256 public safuuInsuranceFundFee = 30;
    ...
    uint256 private constant MAX_SUPPLY = 1 * 10**10 * 10**DECIMALS;
    ...
    constructor() ERC20Detailed("Safuu", "SAFUU", uint8(DECIMALS)) Ownable() {
        ...
        treasuryReceiver = 0x3691d2e8c99AA028789D358fFc8Ee0e5E03E4AfF; 
        autoLiquidityReceiver = 0x4B7Fd1771C8E479348ADF87a2aebE57162469FC0;
        safuuInsuranceFundReceiver = 0x8608dbaaEc0f1920100f75c719b39EAB1914801e;
    ...
    function tradingOpen(bool _flag) external onlyOwner {
        if (_flag) {
            _autoRebase = _flag;
            _tradingOpen = true;
            _lastRebasedTime = block.timestamp;
    ...
    function rebase() internal {
        ...
        if (deltaTimeFromInit >= (182 days)) {
            rebaseRate = 630125;
        } else if (deltaTimeFromInit >= (1 * 365 days)) {
            rebaseRate = 126025;
        } else if (deltaTimeFromInit >= (2 * 365 days)) {
            rebaseRate = 18881;
        } else if (deltaTimeFromInit >= (4 * 365 days)) {
            rebaseRate = 1881;
        } else if (deltaTimeFromInit >= (8 * 365 days)) {
            rebaseRate = 94;
        }
    ...
    function _transferFrom(
        ...
        if (!_tradingOpen) {
            require(whitelist[sender] || whitelist[recipient], "Trading not open yet");
        }
    ...
    function setFeeExempt(address[] calldata accounts, bool _flag) external onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++) {
            _isFeeExempt[accounts[i]] = _flag;
        }
    }

    function setWhitelist(address _address, bool _flag) external onlyOwner {
