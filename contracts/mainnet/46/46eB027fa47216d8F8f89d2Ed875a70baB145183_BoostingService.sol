...
contract BoostingService is PausableByOwner {
    ...
    function _materializeShares(Staker storage staker, address owner) private returns(uint128 compoundAmount, uint128 boostingAmount) {
        uint128 _stakerShares = staker.shares;
        uint128 _stakerInitialBoostingRate = staker.initialBoostingRate;

        compoundAmount = uint128(((uint168(historicalCompoundRate) - staker.initialCompoundRate) * _stakerShares) >> 40);
        boostingAmount = uint128(((uint168(historicalBoostingRate) - _stakerInitialBoostingRate) * _stakerShares) >> 40);
    ...
    function _stakeShares(Staker storage staker, uint128 amount, address owner) private {
        if (amount == 0) return;
