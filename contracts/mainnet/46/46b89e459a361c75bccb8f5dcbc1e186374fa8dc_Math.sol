...
contract Operator is Context, Ownable {
    ...
    function transfeChakrarator(address newOperator_) public onlyOwner {
        _transfeChakrarator(newOperator_);
    }

    function _transfeChakrarator(address newOperator_) internal {
...
contract Karma is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the 2 days genesis pools
    ...
    /**
     * @notice Constructs the Karma ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Karma", "Karma") {
        // Mints 1 Karma to contract creator for initial pool setup
    ...
    function _getKarmaPrice() internal view returns (uint256 _KarmaPrice) {
        try IOracle(KarmaOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Karma: failed to fetch Karma price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _KarmaPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_KarmaPrice >= taxTiersTwaps[tierId]) {
    ...
    function setKarmaOracle(address _KarmaOracle) public onlyOperatorOrTaxOffice {
        require(_KarmaOracle != address(0), "oracle address cannot be 0 address");
        KarmaOracle = _KarmaOracle;
    }
    ...
    /**
     * @notice Operator mints Karma to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of Karma to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentKarmaPrice = _getKarmaPrice();
            currentTaxRate = _updateTaxRate(currentKarmaPrice);
            if (currentKarmaPrice < burnThreshold) {
                burnTax = true;
            }
        }
