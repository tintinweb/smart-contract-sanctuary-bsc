...
contract H2O is Context, IERC20, Ownable {
    ...
    string private _name = "Water-H2O-Aqua";
    ...
    address payable public devWalletAddress = payable(0xaF2E5ea3be07022Ab512BF44CDBEdDFf3856a3DD);
    ...
    uint256 public numTokensSellToAddToLiquidity = 5000000000 * 10**9;
    ...
    function changeNumTokensSellToAddToLiquidity(uint256 _num) external onlyOwner
    {
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
        _taxFee = taxFee;
        require(_taxFee.add(_liquidityFee).add(_devFee) <= 25, "Fee limit reached!!");
    }
    ...
    function setMaxTxAmout (uint256 maxTx) external onlyOwner() {
        require(maxTx > 1000000 * 10**9, "Cannot set maximum transaction amount less than 1 million");
        _maxTxAmount = maxTx;
    }
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee.add(_devFee)).div(
            10**2
        );
    }
    ...
    function sendBNBToDev(uint256 amount) private { 
        ...
        devWalletAddress.transfer(amount); 
    }
    
    function _setDevWallet(address payable _devWalletAddress) external onlyOwner() {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 devShare = newBalance.mul(_devFee).div(_devFee.add(_liquidityFee));
        uint256 liquidityShare = newBalance.sub(devShare);

        // add liquidity to uniswap
        addLiquidity(otherHalfOfLiquify, liquidityShare);
        sendBNBToDev(devShare);
