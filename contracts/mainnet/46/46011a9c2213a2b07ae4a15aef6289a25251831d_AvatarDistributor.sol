...
// File contracts/TalesAvatarNFT.sol

pragma solidity 0.8.10;


contract TalesAvatarNFT is ERC721EnumerableUpgradeable, AccessControlUpgradeable {
...
// File contracts/AvatarDistributor.sol

pragma solidity 0.8.10;


contract AvatarDistributor {
    TalesAvatarNFT public immutable nft;
    bytes32 public immutable merkleRoot;
    uint256 public immutable endOfDistributionTime;

    mapping(uint256 => uint256) private claimedBitMap;

    event Claimed(uint256 index, address account, uint256 tokenId);

    constructor(address _nft, bytes32 _merkleRoot, uint256 _endOfDistributionTime) {
        nft = TalesAvatarNFT(_nft);
        merkleRoot = _merkleRoot;
        endOfDistributionTime = _endOfDistributionTime;
    }
    ...
    function claim(uint256 index, address account, uint256 tokenId, bytes32[] calldata merkleProof) external {
        require(!isClaimed(index), "AvatarDistributor: Drop already claimed.");
        require(endOfDistributionTime > block.timestamp, "AvatarDistributor: End of claiming date.");

        // Verify the merkle proof.
        bytes32 node = keccak256(abi.encodePacked(index, account, tokenId));
        require(MerkleProof.verify(merkleProof, merkleRoot, node), "AvatarDistributor: Invalid proof.");
