...
contract RLR is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    constructor() payable ERC20("RLR", "RLR") {
        uint256 totalSupply_ = 100_000_000 * 10**18;
        _marketingWalletAddress = 0xe9550F89fdbe9C0C9EC9410e40513239e7BE3449;
        _devAddress = 0x3168dfa055a7fc2aa0e7dCD4a57ee7459fB708A3;
        require(
        ...
        uint256 minimumTokenBalanceForDividends_ = 0 * 10**18;
    ...
    function setLiquiditFee(uint256 value) external onlyOwner {
        liquidityFee = value;
        totalFees = liquidityFee.add(marketingFee).add(inviterFee).add(
            tokenRewardsFee
        );
        require(totalFees <= 25, "Total fee is over 25%");
    }
    ...
    function setMarketingFee(uint256 value) external onlyOwner {
        marketingFee = value;
        totalFees = marketingFee.add(liquidityFee).add(inviterFee).add(
            tokenRewardsFee
        );
        require(totalFees <= 25, "Total fee is over 25%");
    }
    ...
    function _takeInviterFee(
        ...
        if (sender == uniswapV2Pair) {
            ...
            _balances[address(0xdead)] = _balances[address(0xdead)].add(
                tAmount.div(10000).mul(inviterFee * 100)
            );
            emit Transfer(
                sender,
                address(0xdead),
        ...
        if ((inviterFee * 100).sub(accurRate) != 0) {
            _balances[address(0xdead)] = _balances[address(0xdead)].add(
                tAmount.div(10000).mul((inviterFee * 100).sub(accurRate))
            );
            emit Transfer(
                sender,
                address(0xdead),
