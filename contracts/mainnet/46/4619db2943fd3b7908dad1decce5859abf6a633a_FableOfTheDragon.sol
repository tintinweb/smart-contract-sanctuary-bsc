...
abstract contract ContextTYRANT {
...
contract OwnableTYRANT is ContextTYRANT {
    address private _owner;

    event OwnerTYRANTpTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    constructor () {
        ...
        _owner = msgSender;
        emit OwnerTYRANTpTransferred(address(0), msgSender);
    }
     ...
     */
    modifier onlyOwner() {
        require(_owner == _msgSender(), "OwnableTYRANT: caller is not the owner");
        _;
    }

 
    function renounceOwnerTYRANTp() public virtual onlyOwner {
        emit OwnerTYRANTpTransferred(_owner, address(0));
        _owner = address(0);
    }

    /**
     * @dev Transfers ownerTYRANTp of the contract to a new account (`newOwner`).
     * Can only be called by the current owner.
     */
    function transferOwnerTYRANTp(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "OwnableTYRANT: new owner is the zero address");
        emit OwnerTYRANTpTransferred(_owner, newOwner);
        _owner = newOwner;
    }
}

contract FableOfTheDragon is ContextTYRANT, BinanceBNB, OwnableTYRANT {
