...
contract CoinManufactory is ERC20Burnable, Ownable {
    ...
    function _getCompleteTaxValue(uint256 amount)
        ...
        returns (uint256)
    {
        uint256 allTaxes = _liquidityPoolFee + _reflectionFee;
    ...
    function _takeLiquidity(
        ...
        uint256 tLiquidity = (tAmount * _liquidityPoolFee) / 100;
        if (tLiquidity > 0) {
    ...
    function removeAllFee() private {
        if (_liquidityPoolFee == 0 && _reflectionFee == 0) return;
        _previousLiquidityPoolFee = _liquidityPoolFee;
        _previousReflectionFee = _reflectionFee;
