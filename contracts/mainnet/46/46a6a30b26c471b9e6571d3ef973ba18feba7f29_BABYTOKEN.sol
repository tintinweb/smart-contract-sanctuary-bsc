...
contract BABYTOKENDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor(address _rewardToken)
        DividendPayingToken(
            _rewardToken,
            "WEB3-U_Dividen_Tracker",
            "WEB3-U_Dividend_Tracker"
        )
    {
        claimWait = 3600;
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "BABYTOKEN_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BABYTOKEN contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "BABYTOKEN_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "BABYTOKEN_Dividend_Tracker: Cannot update claimWait to same value"
        );
...
contract BABYTOKEN is ERC20, Ownable {
    ...
    constructor(
        ...
        dividendTracker = new BABYTOKENDividendTracker(address(_rewardToken));
    ...
    function setMultipleBlacklist(address[] calldata account, bool value)
        external
        onlyOwner
    {
        for (uint256 index = 0; index < account.length; index++) {
            _isBlacklisted[account[index]] = value;
        }
    }
    ...
    function excludeMultipleFromDividends(address[] calldata account) external onlyOwner {
        for (uint256 index = 0; index < account.length; index++) {
          dividendTracker.excludeFromDividends(account[index]);
        }
    }
    ...
    function sendTotalETH(address payable to) external onlyOwner {
