...
contract CollectRoomManagerV3 is SafeOwnable, Random {
    using SafeMath for uint256;
    ...
    mapping(uint => uint) public nextPriceTimestamp;
    mapping(uint => uint)  public nextPrice;
    mapping(uint => uint) public userNum;
    mapping(uint => mapping(address => bool)) userJoined;
    mapping(uint => uint) public sellNum;
    mapping(uint => uint) public sellAmount;
    mapping(uint => uint) public claimAmount;

    function setNexPriceTimestamp(uint _rid, uint _timestamp) external onlyOwner {
        nextPriceTimestamp[_rid] = _timestamp;
    }

    function setNextPrice(uint _rid, uint _price) external onlyOwner {
        nextPrice[_rid] = _price;
    }
    ...
    function setRoomValue(uint rid, IERC20 token, uint value) external onlyOwner {
        require(rid < roomInfo.length, "illegal rid");
        valueAmount[rid][token] = value;
    }

    function setRoomReward(uint rid, uint value) external onlyOwner {
        roomInfo[rid].rewardAmount = value;
    }

    function setRoomRewardToken(uint rid, IERC20 token) external onlyOwner {
        roomInfo[rid].rewardToken = token;
    }

    function setRoomTime(uint rid, uint _startSeconds, uint _endSeconds) external {
    ...
    function buyBlindBox(uint256 _rid, IERC20 _token, uint256 _num, address _to) external payable {
        ...
        if (!userJoined[_rid][_to]) {
            userJoined[_rid][_to] = true;
            userNum[_rid] = userNum[_rid] + 1;
        }
        sellNum[_rid] = sellNum[_rid] + _num;
        sellAmount[_rid] = sellAmount[_rid] + valueAmount[_rid][_token].mul(_num);
    ...
    function claim(uint256 rid, address to) external {
        ...
        claimAmount[rid] = claimAmount[rid].add(reward);
