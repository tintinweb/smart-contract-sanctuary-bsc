...
contract CR7FINANCE is Context, Ownable, IERC20 {
    ...
    string private _name = "CR7 FINANCE";
    string private _symbol = "CR7";
    uint8 private _decimals = 9;

    bool public pause_Contract = false;
    ...
    feeRatesStruct public buyFees = feeRatesStruct(
     {taxFee: 5000,
	  ...
	  liquidityFee: 0,
      charityFee: 5000, 
      swapFee: 15000, // burnFee+airdropFee+liquidityFee+charityFee
      totFees: 2
    });
    ...
    uint256 public maxSellPerDay = 50 * 10**9;
    uint256 public maxTrPerDay = 50 * 10**9;
    
    address payable public burnWallet = payable(address(0));
    address payable public marketingWallet = payable(0x3Fef1008BE0873467C33fb605310F8033aAEbB96);
    address payable public liquidityWallet = payable(0x3Fef1008BE0873467C33fb605310F8033aAEbB96);
    address payable public airdropWallet = payable(0x3Fef1008BE0873467C33fb605310F8033aAEbB96);
    ...
    uint256 private minTokensToSwap = 50 * 10**9;
    uint256 public maxTxAmount = 50 * 10**9;
    ...
    constructor () {
        ...
        _isExcludedFromFee[0x78360f4484B413d9A355e05B063541851d49f05C] = true;
   ...
   function setMarketingWallet(address payable _address) external onlyOwner returns (bool){
    ...
    function setBuyFees(uint256 taxFee, uint256 burnFee, uint256 airdropFee, uint256 charityFee, uint256 liquidityFee) external onlyOwner{
        buyFees.taxFee = taxFee;
		buyFees.burnFee = burnFee;
		buyFees.airdropFee = airdropFee;
        buyFees.charityFee = charityFee;      
        buyFees.liquidityFee= liquidityFee;
        buyFees.swapFee = charityFee+airdropFee+burnFee+liquidityFee;
        //require(buyFees.swapFee+buyFees.taxFee == 10000, "sum of all percentages should be 10000");
    }
    
    function setSellFees(uint256 sellTaxFee, uint256 sellBurnFee, uint256 sellAirdropFee, uint256 sellcharityFee, uint256 sellLiquidityFee) external onlyOwner{
        sellFees.taxFee = sellTaxFee;
		sellFees.burnFee = sellBurnFee;
		sellFees.airdropFee = sellAirdropFee; 
        sellFees.charityFee = sellcharityFee;               
        sellFees.liquidityFee = sellLiquidityFee;        
        sellFees.swapFee = sellcharityFee+sellAirdropFee+sellBurnFee+sellLiquidityFee;
    ...
    function  setPauseContract(bool _temp) external onlyOwner{
        pause_Contract = _temp;
    }
    ...
    function _transfer(
        ...
        require(!pause_Contract, "Contract is Paused.");
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");
        require(amount > 0, "Transfer amount must be greater than zero");
        require(!_isBadActor[from] && !_isBadActor[to], "You are not allowed");
