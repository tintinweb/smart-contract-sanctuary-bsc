...
contract Donate is Context, Ownable {
    ...
    constructor() {
        token = IBEP20(0x55d398326f99059fF775485246999027B3197955); //USDT

        purchase.push(50 * 10**18);
        purchase.push(100 * 10**18);
        purchase.push(200 * 10**18);
        purchase.push(300 * 10**18);
        purchase.push(400 * 10**18);
    ...
    // version 2.0
    function buy(uint256 purchaseIndex) public {
        require(_balances[msg.sender] == 0, "Cannot buy");
        require(purchaseIndex >=0  && purchaseIndex < purchase.length, "No corresponding item");
        ...
        _balances[msg.sender] = _balances[msg.sender].add(_maxAmount);
        token.transferFrom(msg.sender, address(this), _maxAmount);
        emit Buyed(msg.sender, _maxAmount);
    }

    function withdraw() public onlyOwner {        
        token.transfer(msg.sender, _totalSupply);
        emit Withdrawn(msg.sender, _totalSupply);
        _totalSupply = 0;
    }
