...
contract HealthchainToken is Context, IERC20, Ownable {
    ...
    string private _name = "Dafen Art";
    string private _symbol = "DFA";
    ...
    uint[] internal shareConfig = [30,1,1,5,5,5,5];

    uint256 public _totalFee = 120;

    uint256 public _transferMarketFee = 50;
    ...
    uint public shareCondition = 200 * 10 ** 9;

    uint public lastSendTaxTime;

    uint public sendTaxProid;
    ...
    constructor (
        ...
        lastSendTaxTime = block.timestamp / 1 days * 1 days;
        sendTaxProid = 7 days;

        initPoolAddress[_initPoolAddress] = true;
        emit Transfer(address(0), holder, _tTotal);
    }

    function setMarketAddress(address m)external onlyOwner{
        marketAddress = m;
    }
    ...
    function setSendTaxProid(uint proid)external onlyOwner {
        sendTaxProid = proid;
    }
    ...
    function _getValues(uint256 tAmount,bool takeFee,address from ,address to) private view returns (Param memory param) {
        ...
        if(takeFee){
            ...
            if( ammPairs[from] || ammPairs[to] ){
                ...
                param.tMarket = tAmount * _transferMarketFee / 1000;
            }
    ...
    function _take(uint256 tValue,uint256 currentRate,address from,address to) private {
    ...
    function _takeShare(uint tShare,uint256 currentRate,address from,address user) private {
        ...
        address[] memory farthers = getForefathers(user,7);
        ...
        for( uint i = 0; i < len; i++ ){
            ...
            _take(tv,currentRate,from,parent);
            sended += tv;
        }  
        
        if( tShare > sended ){
            _take(tShare - sended,currentRate,from,address(0));
        }
    }
    ...
    function _transfer(
        ...
        if( contractTokenBalance >= _maxTxAmount 
                && !inSwapAndLiquify 
                && !ammPairs[from] 
                && IERC20(uniswapV2Pair).totalSupply() > 10){

                inSwapAndLiquify = true;

                uint ma = marketAmount;
                if( ma <= contractTokenBalance){
                    marketAmount = 0;
                    swapTokensForEth(ma,marketAddress);
                }
                
                contractTokenBalance = balanceOf(address(this));
                uint la = liquifyAmount;
                if( la <=contractTokenBalance ){
                    liquifyAmount = 0;
                    swapAndLiquify(la);
                }
        ...
        Param memory param =  _getValues(amount,takeFee,from,to);
    ...
    function _takeFee(Param memory param,address from)private {
        ...
        if( param.tLiquidity > 0 ){
            _take(param.tLiquidity, currentRate, from, address(this));
            liquifyAmount += param.tLiquidity;
        }

        if( param.tMarket > 0 ){
            _take(param.tMarket, currentRate, from, address(this));
            marketAmount += param.tMarket;
        }

        if( param.tShare > 0 ){
            _takeShare(param.tShare, currentRate,from,param.user);
        }

        if( param.tTax > 0 ){
            taxAmount += param.tTax;

            uint time = _getCurrentZeroTime();
            if( 
                time - lastSendTaxTime >= sendTaxProid 
                &&  taxAmount > 0 ){

                uint v = taxAmount;
                taxAmount = 0;
                lastSendTaxTime = time;
    ...
    function _getCurrentZeroTime()internal view returns(uint){
