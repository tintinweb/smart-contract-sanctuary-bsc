	...
	ReentrancyGuardUpgradeable
{
	...
	function buyToken(uint256 _tokenAmount)
		...
		nonReentrant
	{
		require(phases[currentPhase].maxEntry>=_tokenAmount,"Too much tokens");
		require(
		...
		if (phases[currentPhase].timelock > 0) {
			...
			tokenLocksForSale[id].percentsToRelease = phases[currentPhase]
				.percentsToRelease;
			tokenLocksForSale[id].price=phases[currentPhase].price;
		} else {
	...
	function createPhase(
		...
		emit PhaseCreated(_isPublic,_maxEntry, _minimunEntry, _price, _endAt, _supply);
	}
