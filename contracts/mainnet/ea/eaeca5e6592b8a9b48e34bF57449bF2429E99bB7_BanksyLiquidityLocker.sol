...
// File contracts/BanksyLiquidityLocker.sol
...
contract BanksyLiquidityLocker is Ownable {
    error ClaimIsStillLocked(uint256 time);
    ...
    function claimLiquidity(IERC20 liquidity, address to) external onlyOwner {
        if (block.timestamp < unlockTime)
            revert ClaimIsStillLocked(unlockTime);
