...
contract HVSPresale is Context, ReentrancyGuard, Ownable {
    ...
    uint256 public constant PRESALE_DURATION = 50 days;
    ...
    event UserWithdrawUsdtAndBusd(uint256, uint256);
    event UserWithdrawTokensSuccess(uint256);
    event OwnerWithdrawTokens(uint256);
    event OwnerWithdrawUsdtAndBusd(uint256, uint256);
    ...
    function isContract(address account) internal view returns (bool) {
        ...
        if(block.timestamp < presale_info.presale_start)
            return 0;
        else if((block.timestamp >= presale_info.presale_start) && (block.timestamp < (presale_info.presale_start + 3 days)))
            return 3;
        else if((block.timestamp >= (presale_info.presale_start + 3 days)) && (block.timestamp < (presale_info.presale_start + 6 days)))
            return 5;
        else if((block.timestamp >= (presale_info.presale_start + 6 days)) && (block.timestamp < (presale_info.presale_start + 13 days)))
            return 8;
        else if((block.timestamp >= (presale_info.presale_start + 13 days)) && (block.timestamp < (presale_info.presale_start + PRESALE_DURATION)))
            return 40;
        return 0;
    }
    ...
    function validatePurchase(uint256 token_amount) internal view {
        ...
        require((token_amount + status.sold_amount) <= IBEP20(presale_info.sale_token).balanceOf(address(this)), "HVS Presale: Not enough HVS Tokens on the contract for purchasing");
    }
    ...
    // withdraw usdt & busd by owner on presale success
    function ownerWithdrawUsdtAndBusd () external onlyOwner {
        ...
        require((remainingUsdtBalance > 0) || (remainingBusdBalance > 0), "HVS Presale: Nothing to withdraw");

        if(remainingUsdtBalance > 0) {
            IBEP20(USDT).transfer(_msgSender(), remainingUsdtBalance);
        }

        if(remainingBusdBalance > 0) {
            IBEP20(BUSD).transfer(_msgSender(), remainingBusdBalance);
        }
        
        emit OwnerWithdrawUsdtAndBusd(remainingUsdtBalance, remainingBusdBalance);
    }

    // withdraw presale tokens by user on presale success
    function userWithdrawTokens() external nonReentrant {
        ...
        uint256 amountBought = buyers[msg.sender].sale - buyers[msg.sender].withdraw_tokens;
        require(amountBought > 0, "HVS Presale: Not allow to withdraw");
        
        TransferHelper.safeTransfer(address(presale_info.sale_token), msg.sender, amountBought);
        
        status.token_withdraw = status.token_withdraw + amountBought;
        buyers[msg.sender].withdraw_tokens = buyers[msg.sender].withdraw_tokens + amountBought;

        emit UserWithdrawTokensSuccess(amountBought);
    }

    // withdraw usdt & busd by user on presale failure
    function userWithdrawUsdtAndBusd() external nonReentrant {
        ...
        emit UserWithdrawUsdtAndBusd(buyers[msg.sender].based_usdt, buyers[msg.sender].based_busd);
    }

    // withdraw tokens by owner on presale failure
