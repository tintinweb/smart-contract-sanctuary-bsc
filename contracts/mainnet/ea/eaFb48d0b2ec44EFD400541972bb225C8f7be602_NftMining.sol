...
contract NftMining is ERC721Holder, Ownable{
    ...
    address public addressLpCtnUsdt;
    ...
    function init(address ctn_, address LpCtnUsdt_, address usdt_, address wbnb_, address addressBlackHole_) public onlyOwner{
        require(!initialize, "is started");
        addressCtn = ctn_;
        addressUsdt = usdt_;
        addressLpCtnUsdt = LpCtnUsdt_;
        addressWbnb = wbnb_;
    ...
    function costCtnToBeLeader(uint pid_, uint level_) public {
        (uint r0, uint r1,) = ICnnPair(addressLpCtnUsdt).getReserves();
        uint usdtRe;
        uint ctnRe;
        if (addressCtn > addressUsdt) {
            usdtRe = r0;
            ctnRe = r1;
        } else {
            usdtRe = r1;
        ...
        uint _brunAmount = _getAmountOut(_amountIn, usdtRe, ctnRe);
        IERC20(addressCtn).transfer(addressBlackHole, _brunAmount);
    }
    ...
    function deposit(uint pid_, IERC721 nft721_, uint nftTokenId_,address inv_) public lock {
        ...
        require(pool.status && block.timestamp > pool.startTime, "not started");
    ...
    function addNewPool(address[] memory nftList_, 
        ...
        poolInfo[listindex] = PoolInfo(true, poolName_, nftList_, outPutToken_, dailyOut_, block.timestamp, 0, 0, requireCtn_, block.timestamp, endTime_);
    ...
    function addNewTimeLimitPool(address[] memory nftList_,
        ...
        poolInfo[listindex] = PoolInfo(true, poolName_, nftList_, outPutToken_, dailyOut_, startTime_, 0, 0, requireCtn_, startTime_, endTime_);
        poolList.push(poolInfo[listindex]);
        _pid = listindex;
        listindex += 1; 
    }
    ...
    function getPool(uint pid_, address addr_) external view returns(
        ...
        poolNftaddressList = pool.nftList;
        poolName = pool.poolName;
        poolOutputToken = address(pool.outPutToken);
        poolDailyOut = pool.dailyOut;
        poolEndTime = pool.endTime;
        poolTotalPower = pool.totalPower; // TVL
        poolRequireCtn = pool.requireCtn; // 
        }
