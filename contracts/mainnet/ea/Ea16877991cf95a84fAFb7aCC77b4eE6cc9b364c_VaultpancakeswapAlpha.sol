...
// File: contracts_BSC/main/Vaultpancakeswap.sol
...
/** 
* @author Formation.Fi.
* @notice Implementation of the contract Vaultpancakeswap;
*/
...
contract Vaultpancakeswap is Ownable {
    ...
    Admin public admin;
    constructor (address _admin, address _safeHouse, address _pancake) payable {
        ...
        admin = Admin(_admin);
        pancake = _pancake;
        pancakeRouter = IPancakeRouter01(_pancake);
    ...
    BEP20(tokenA).approve(pancake,  amountADesired);
    BEP20(tokenB).approve(pancake,  amountBDesired);
   (amountA,amountB,liquidity) = pancakeRouter.addLiquidity(tokenA,tokenB,amountADesired,amountBDesired,amountAMin,amountBMin, safeHouse, block.timestamp + duration);
    }
    ...
    BEP20(token).approve(pancake, amountTokenDesired);
    require (address(this).balance >= amountETHDesired, "Formation.Fi: balance limit");
    (amountToken,amountETH,liquidity)= pancakeRouter.addLiquidityETH{value: amountETHDesired}(token,amountTokenDesired,amountTokenMin, amountETHMin, safeHouse, block.timestamp + duration);
    }
    ...
    function removeLiquidity(
       ...
       BEP20(_pair).approve(pancake, liquidity);
      (amountA,amountB)= pancakeRouter.removeLiquidity(tokenA,tokenB,liquidity,amountAMin,amountBMin, safeHouse, block.timestamp + duration);
    ...
    function removeLiquidityETH(
        ...
        BEP20(_pair).approve(pancake, liquidity);
        (amountToken,amountETH)= pancakeRouter.removeLiquidityETH(token,liquidity,amountTokenMin,amountETHMin, safeHouse, block.timestamp + duration);
...
// File: contracts_BSC/Alpha/VaultpancakeswapAlpha.sol
...
/** 
* @author Formation.Fi.
* @notice Implementation of the contract VaultpancakeswapAlpha.
*/

contract VaultpancakeswapAlpha is Vaultpancakeswap {
    constructor(address _admin, address _safeHouse, address _pancake) 
    Vaultpancakeswap ( _admin,  _safeHouse, _pancake ) {
    }
}