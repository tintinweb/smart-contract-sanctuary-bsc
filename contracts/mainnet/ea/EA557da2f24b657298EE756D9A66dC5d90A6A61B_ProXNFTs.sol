...
contract ProXNFTs is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    address public RewardToken = 0x9d9fA9DbAe391C3FB6866F43De62FF3B393133b2;
    address public FeeReceiver = 0x52D92f6edf3620C771A6a2dCB0abD854fEe57B12;
    ...
    // Constructor
    constructor(
    ) ERC721("Welcome To The Jungle", "ProXNFT") {
        // Save URI
        _baseTokenURI = "ipfs://bafybeict2g3syjdjlhiktlravy4qzgevsxtkvhy4hr4uvpv673esp3om3y/";
        // Save dev wallet
        _wallet = msg.sender;
        // Set router
    ...
    function mint(uint256 _mintAmount) public payable {
        // Checks
        require(!paused, "ProXNFTs: minting has not started yet");
        require(_mintAmount > 0, "ProXNFTs: you have to mint at least one");
        require(
            _mintAmount <= maxMintAmount,
            "ProXNFTs: you can only mint 10 at a time"
        );
        require(
            lastSupply >= _mintAmount,
            "ProXNFTs: the collection is sold out"
        );
        if (msg.sender != owner()) {
            require(
                msg.value >= cost * _mintAmount,
                "ProXNFTs: total cost isn't match"
            );
        }
        // Minting
    ...
    {
        // Checks
        require(_winner != address(0), "ProXNFTs: zero address");
        require(_amount > 0, "ProXNFTs: you have to mint at least one");
        require(lastSupply != 0, "ProXNFTs: the collection is sold out");
    ...
    function tokenURI(uint256 _id)
        ...
        returns (string memory)
    {
        require(_exists(_id), "ProXNFTs: URI query for nonexistent token");
