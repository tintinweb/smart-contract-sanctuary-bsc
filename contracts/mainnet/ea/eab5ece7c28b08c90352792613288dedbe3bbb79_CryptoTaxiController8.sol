...
// File: CryptoTaxiController8.sol
...
contract CryptoTaxiController8 is Ownable {
    ...
    function vehicleOffShelfByIndexs(uint256[] memory indexs) public onlyOwner {

        uint256 i;

        for(i = 0; i < indexs.length; i++){
            (uint256 types,,uint256 vehicleState,,,uint256 tokenId,address hold,,,,) = UTD.allVehicles(indexs[i]);

            if(vehicleState == 2){
                UTD.setVehicleState(indexs[i], 0);

                UTD.vehicleOffShelf(indexs[i]);

                UTBC.setNFTLock(tokenId, false);

                emit VehicleOffShelf(hold, indexs[i], types, tokenId);
            }
        }
    }

    function vehicleOffShelfByTokens(uint256[] memory tokens) public onlyOwner {
        ...
        for(i = 0; i < tokens.length; i++){
            ...
            index = UTD.tokenIdToVehicleIds(tokens[i]);

            (uint256 types,,uint256 vehicleState,,,uint256 tokenId,address hold,,,,) = UTD.allVehicles(index);
            ...
            if(vehicleState == 2){
                ...
                emit VehicleOffShelf(hold, index, types, tokenId);
            }
        }
    }

    function vehicleOffShelfByUsers(address[] memory users) public onlyOwner {

        uint256 i;
        uint256 j;

        for(i = 0; i < users.length; i++){
            uint256[] memory indexs = UTD.queryUserRentVehicleIndexs(users[i]);

            for(j = 0; j < indexs.length; j++){
                (uint256 types,,uint256 vehicleState,,,uint256 tokenId,,,,,) = UTD.allVehicles(indexs[j]);

                if(vehicleState == 2){
                    UTD.setVehicleState(indexs[j], 0);

                    UTD.vehicleOffShelf(indexs[j]);

                    UTBC.setNFTLock(tokenId, false);

                    emit VehicleOffShelf(users[i], indexs[j], types, tokenId);
                }
            }
        }
    }

    function vehiclesOffShelf() public onlyOwner {

        uint256[] memory indexs = UTD.queryVehicleSells();

        uint256 i;
        uint256 end;

        require(start < indexs.length,"No Vehicle on sale");

        if(start + amount < indexs.length){
            end = start + amount;
        }else{
            end = indexs.length;
        }
        ...
        if(start >= indexs.length){
            start = 0;
        }
    }

    function vehiclesOffShelfByRange(uint256 _start, uint256 _end) public onlyOwner {
        ...
        require(_start < indexs.length,"No Vehicle on sale");

        if(_end < indexs.length){
            end = _end;
        }else{
            end = indexs.length;
        }
    ...
    function vehiclesOffShelfByType(uint256 types) public onlyOwner {

        uint256[] memory indexs = UTD.queryVehiclesTypeSells(types);
        ...
        for(i = start; i < end; i++){
            ...
            if(indexs[i] > 10000){
                (uint256 typess,,uint256 vehicleState,,,uint256 tokenId,address hold,,,,) = UTD.allVehicles(indexs[i]);
                ...
                if(vehicleState == 2){
                    ...
                    emit VehicleOffShelf(hold, indexs[i], typess, tokenId);
                }
            }
        }
    }
