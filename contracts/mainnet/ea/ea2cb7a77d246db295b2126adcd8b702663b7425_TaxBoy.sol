...
If you make the biggest buy (in tokens) you become the TaxBoy, and collect 5% fees (in BNB) the same way marketing does. 
If you sell any tokens at all at any point you can never be the TaxBoy. 
If someone beats your record, they become the TaxBoy. 
If you sell while the TaxBoy, the marketing wallet becomes TaxBoy and the record is reset to 0, this way no one can buy a ton and then sell and keep collecting tax fees.
The TaxBoy is set via biggest buy in tokens.
...
The point is to reward big buys. There are no punishments other than disallowing sellers to be the TaxBoy.
*/
...
contract TaxBoy is IBEP20, Auth {
   using SafeMath for uint256;
   ...
   string constant _name = "TaxBoy";
   string constant _symbol = "TAXBOY";
