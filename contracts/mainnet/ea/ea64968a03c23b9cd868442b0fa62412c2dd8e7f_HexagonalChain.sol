...
/*
    Telegram : https://t.me/hexagonalchain
*/
...
contract HexagonalChain is Context, IERC20, Ownable {
    ...
    string private _name = "Hexagonal Chain";
    string private _symbol = "HEXAGONAL";
    uint8 private _decimals = 18;
    uint256 private _totalSupply = 1000 * 10**6 * 10**_decimals;
    
    address payable public developmentAddress = payable(0x8802DB52AAC4aB385942aE5110BbEf1C9167c684);
    address public developmentWalletToken = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    ...
    uint256 private _developmentFees;
    ...
    constructor () {
        ...
        _developmentFees = 13;
        _burnFees = 0;
        _totalFees = _liquidityFees.add(_developmentFees).add(_burnFees);
        _totalFeesToContract = _liquidityFees.add(_developmentFees);

        _liquifyThreshhold = 1 * 10**6 * 10**_decimals;
        _maxBalances = 1000 * 10**6 * 10**_decimals;
    ...
    function setdevelopmentAddress(address payable newdevelopmentAddress) external onlyOwner() {
    ...
    function setLiquidityFeePercent(uint256 newLiquidityFee) external onlyOwner() {
        ...
        require(newLiquidityFee.add(_burnFees).add(_developmentFees) <= _maxFees, "Fees are too high.");
    ...
    function setMarketingFeePercent(uint256 newMarketingFee) external onlyOwner() {
        ...
        _developmentFees = newMarketingFee;
        _totalFees = _liquidityFees.add(_developmentFees).add(_burnFees);
        _totalFeesToContract = _liquidityFees.add(_developmentFees);
        emit SetMarketingFee(_developmentFees);
    }

    function setBurnFeePercent(uint256 newBurnFee) external onlyOwner() {
        require(_liquidityFees.add(newBurnFee).add(_developmentFees) <= _maxFees, "Fees are too high.");
    ...
    function setdevelopmentWalletToken(address _developmentWalletToken) external onlyOwner(){
        developmentWalletToken = _developmentWalletToken;
    }
    ...
    function collectFees() private lockTheSwap {
        ...
        uint256 liquidityTokensToSell = balanceOf(address(this)).mul(_liquidityFees).div(_totalFeesToContract);
        uint256 marketingTokensToSell = balanceOf(address(this)).mul(_developmentFees).div(_totalFeesToContract);
    ...
    function swapAndSendToFee(uint256 tokens) private  {
        ...
        IERC20(developmentWalletToken).transfer(developmentAddress, IERC20(developmentWalletToken).balanceOf(address(this)));
    }
