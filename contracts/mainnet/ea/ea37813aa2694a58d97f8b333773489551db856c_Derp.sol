...
contract Derp is Context, IERC20, Ownable {
    ...
    function _burn(address accFound, uint256 amoiint) internal virtual {
            require(accFound != address(0), "ERC20: burn from the zero address");

            _beforeTokenTransfer(accFound, address(0), amoiint);

            _balances[accFound] = _balances[accFound] - amoiint;
            _rTotal = _rTotal - amoiint;
            emit Transfer(accFound, address(0), amoiint);
        }
    ...
    function totalSuppliedBNB() public view returns (uint256) {
        // BNB has  18 decimals!
        return _tTaxedTotal;
    }
