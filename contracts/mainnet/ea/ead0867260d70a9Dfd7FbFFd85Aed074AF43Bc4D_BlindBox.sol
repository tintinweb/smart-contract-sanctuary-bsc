...
contract BlindBox {
    ...
    TokenLike        public elv = TokenLike(0x1CAF67720206E66255C905673c2e723A15524228);
    TokenLike        public elvNft = TokenLike(0x763524cbdD2a9Cfb12b00f7Ac54D32cc24F91f67);
    ...
    mapping (address => mapping (uint256 => uint256[3])) public record;
    ...
    function getRandom(address sender) public view returns (uint){
        ...
        bytes32 hash = keccak256(abi.encodePacked(sender, block.timestamp, total[sender]));
        uint256 luckily = uint256(hash)%100;  
        return luckily;
    }
    function _openblind(uint256 wad,uint256 luckily) internal {
        uint256 tokenID = elvNft.mint(msg.sender,0);
        total[msg.sender] +=1;
        record[msg.sender][total[msg.sender]] = [wad,luckily,tokenID];
        address recommender = elvInviter.inviter(msg.sender);
        if (recommender != address(0) && vip[recommender] >=1 && !recommender.isContract()) usdt.transfer(recommender,wad*9/100);
        ...
        if (recommender2 != address(0) && vip[recommender2] >=3 && !recommender2.isContract()) usdt.transfer(recommender1,wad*4/100);
    ...
    function openblindOne() public {
        ...
        if (luckily<4) usdt.transfer(msg.sender,308*1E17);
        else elv.transfer(msg.sender,28*1E22);
    ...
    function openblindTwo() public {
        ...
        uint256 luckily = getRandom(msg.sender);
        if (luckily<3) usdt.transfer(msg.sender,1078*1E17);
        else elv.transfer(msg.sender,98*1E22);
    ...
    function openblindThree() public {
        ...
        uint256 luckily = getRandom(msg.sender);
        if (luckily<2) usdt.transfer(msg.sender,3278*1E17);
        else elv.transfer(msg.sender,298*1E22);
    ...
    function openblindFour() public {
        ...
        uint256 luckily = getRandom(msg.sender);
        if (luckily<1) usdt.transfer(msg.sender,6578*1E17);
        else elv.transfer(msg.sender,598*1E22);
    ...
    function getOwnerOpen(address usr) public view returns (uint[3][] memory){
        uint n = total[msg.sender];
        uint[3][] memory openRecord = new uint[3][](n);
        for(uint i = 0; i< n; ++i) {
            openRecord[i] = record[usr][i];
        }
        return openRecord;
    }
    function getOwnerOrder(address usr,uint256 order) public view returns (uint[3] memory){
