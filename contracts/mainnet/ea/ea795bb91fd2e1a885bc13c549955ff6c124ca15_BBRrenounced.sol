...
contract BBRrenounced is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BBRrenouncedDividendTracker public dividendTracker;
    ...
    constructor() ERC20("BBRrenounced", "BBRR") {
        marketingWallet = payable(0x9fDd83Bbe32e5959B70f743f32e7FcEB98fE28a2);
        devWallet = payable(0x2d37C5188e6B8A5E4a4e1A79B391C4BBf96c536f);
        ...
        dividendTracker = new BBRrenouncedDividendTracker(
            payable(this),
            router,
    ...
    // set max wallet, can not be lower than 0.5% of supply
...
contract BBRrenouncedDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    BBRrenounced public BBRrenouncedContract;
    ...
    constructor(
        ...
        defaultToken = token;
        BBRrenouncedContract = BBRrenounced(mainContract);
    ...
    function _transfer(
        ...
        require(false, "BBRrenounced_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "BBRrenounced_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BBRrenounced contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(BBRrenouncedContract);

            uint256 prevBalance = BBRrenouncedContract.balanceOf(address(this));
            ...
            {
                uint256 received = BBRrenouncedContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    BBRrenouncedContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
