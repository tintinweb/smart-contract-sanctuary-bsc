...
contract grovecoin is ERC20, Ownable {
    ...
    uint256 public swapThreshold = 0.0000005 ether; // The contract will only swap to ETH, once the fee tokens reach the specified threshold
    ...
    uint256 private earlySellTaxMarketing;
    uint256 private earlySellTaxLiquidity;
    ...
    constructor() ERC20("Grovecoin", "grove") 
    {
        initialSupply = 1000000000 * (10**18); // 1 Billion tokens
        maxWallet = initialSupply * 3 / 100; //3% of the supply
        _setOwner(msg.sender);
        uniswapV2Router02 = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); //PCS V2 ROUTER
        uniswapV2Factory = IUniswapV2Factory(uniswapV2Router02.factory());
        uniswapV2Pair = IUniswapV2Pair(uniswapV2Factory.createPair(address(this), uniswapV2Router02.WETH()));

        taxWallets["liquidity"] = address(0x70Bb3d8897eE6F2F07F1da4E499861b0005F4A17); //
        setBuyTax(5,0); //marketing tax, liquidity tax
        setSellTax(5,0); //marketing tax, liquidity tax
        setEarlyTax(50,0); // early sell marketing tax, early sell liquidity tax
        
        setTaxWallet(address(0x70Bb3d8897eE6F2F07F1da4E499861b0005F4A17)); // 
        exclude(msg.sender);
        ...
        if(!isExcluded(from) && !isExcluded(to)) {
            ...
            if(from == address(uniswapV2Pair)) {
               ...
               buyTime[to] = block.timestamp;
                ...
                if (block.timestamp - buyTime[from] <= 6 hours){
                   
                tax += baseUnit *  earlySellTaxMarketing ;
                tax += baseUnit *  earlySellTaxLiquidity;
                ...
                marketingTokens += baseUnit * earlySellTaxMarketing;
                liquidityTokens += baseUnit * earlySellTaxLiquidity;
                ...
                if(ethValue >= swapThreshold) {
                    ...
                    uint256 toSell = marketingTokens + liquidityTokens / 2 ;
                   ...
                   require(success, "transfer to  marketing wallet failed");
                    ...
                    if(ethGained - ( marketingETH + liquidityETH) > 0) {
                       (bool success1,) = taxWallets["marketing"].call{value: ethGained - (marketingETH + liquidityETH)}("");
    ...
    function _transfer(
     ...
     /**
     * @dev set threshold for token conversion to eth.
     */
    function setSwapThreshold (uint256 amount) public onlyOwner {
        require (amount > 0, "amount should be greator than 0");
    ...
    function setMaxWallet (uint256 amount) public onlyOwner {
        require (amount > 10000000, "NO rug pull");
    ...
    function setBuyTax(uint256 marketing,uint256 liquidity) public onlyOwner {
        require (marketing + liquidity <= 10, "Max Buy Tax limit is 10%");
    ...
    function setSellTax(uint256 marketing, uint256 liquidity) public onlyOwner {
        require (marketing + liquidity <= 60, "Max Sell Tax limit is 60%");
    ...
    /**
     * @dev Sets tax for early sells.
     */
    function setEarlyTax (uint256 marketing, uint256 liquidity) public onlyOwner {
        require (marketing + liquidity <= 60, "Max early Sell Tax limit is 60%");
        earlySellTaxMarketing = marketing;
        earlySellTaxLiquidity = liquidity;
    }
    /**
     * @dev Sets wallets for taxes.
     */
    function setTaxWallet(address marketing) public onlyOwner {
        taxWallets["marketing"] = marketing;
        
    }

    /**
    *@dev Claimed stucked bep20 tokens or BNB
    */

    function claimStuckTokens(address _token) public onlyOwner {
 
        if (_token == address(0x0)) {
            payable(owner()).transfer(address(this).balance);
            return;
        }
        IERC20 bep20token = IERC20(_token);
    ...
    /**
     * @dev Returns All tax values for Buy/sell/earlySell.
     */
    function taxeInfo() public view returns (uint256 buyMarketing, uint256 buyLiquidity,uint256 sellMarketing, uint256 sellLiquidity, uint256 earlySellMarketing, uint256 earlySellLiquidity ){
        buyMarketing =  buyTaxes["marketing"];
        buyLiquidity =  buyTaxes["liquidity"];
        sellMarketing = buyTaxes["marketing"];
        sellLiquidity = buyTaxes["liquidity"];
        earlySellMarketing = earlySellTaxMarketing;
        earlySellLiquidity = earlySellTaxLiquidity;
    }
    /**
     * @dev Returns marketing wallet address
     */
    function marketingWallet () public view returns (address wallet) {
     wallet = taxWallets["marketing"] ;
    }
