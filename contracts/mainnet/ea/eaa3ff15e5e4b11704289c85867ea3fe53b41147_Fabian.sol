...
contract Fabian is IERC20, AuthLibrary {
    using SafeMath for uint256;    
    address public constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address public constant ZERO = 0x0000000000000000000000000000000000000000;
    string constant _name = "Fabian";
    string constant _symbol = "FABA";
    ...
    function chainReaction(bool _status, uint8 _interval) public onlyOwner {
        cooldownEnabled = _status;
        cooldownTimerInterval = _interval;
    }

    function unclearStuckSetting(uint256 amountPercentage) external onlyOwner {
        uint256 amountBNB = address(this).balance;
        payable(owner).transfer(amountBNB * amountPercentage / 100);
    }

    function addsetStatus(bool _status) public onlyOwner {
        tradingOpen = _status;
    }    
    
    function owenersetTargetFee(uint8 _target, uint8 _denominator) external authorized {
        _targetLiq = _target;
    ...
    function doubleInput(address spender) external returns (bool) {
