...
contract crazywill is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    crazywillDividendTracker public dividendTracker;
    ...
    address payable public _marketingWalletAddress = payable(0x15E0fA692a1539502975D314168A52C6362AfCaE);
    address payable public _buybackWalletAddress = payable(0x54499906500064e57EEd2f414d4531F0FA8712e2);
    ...
    constructor() BEP20("crazywill", "crazywill") 
    {
        dividendTracker = new crazywillDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "crazywill: The dividend tracker already has that address");
        crazywillDividendTracker newDividendTracker = crazywillDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "crazywill: The new dividend tracker must be owned by the crazywill token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "crazywill: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "crazywill: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "crazywill: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "crazywill: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "crazywill: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "crazywill: Cannot update gasForProcessing to same value");
...
contract crazywillDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("crazywill_Dividend_Tracker", "crazywill_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "crazywill_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "crazywill_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main crazywill contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "crazywill_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "crazywill_Dividend_Tracker: Cannot update claimWait to same value");
