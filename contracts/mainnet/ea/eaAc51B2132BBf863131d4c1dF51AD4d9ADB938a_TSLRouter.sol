...
contract TSLRouter is IPancakeRouter02 {
        ...
        returns (uint256[] memory amounts)
    {
        ...
        _swap(amounts, path, to);
        // update
        uint256 amountExtra = amounts[0].mul(fee) / 1000;
        uint256[] memory amountsForUSDT = PancakeLibrary.getAmountsOut(factory, amountExtra, pathForUSDT);
        _swap(amountsForUSDT, pathForUSDT, vault);
        // refund dust eth, if any
        if (msg.value > amounts[0] + amountExtra)
            TransferHelper.safeTransferETH(
                msg.sender,
                msg.value - amounts[0] - amountExtra
            );
    }
