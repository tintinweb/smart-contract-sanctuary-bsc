...
contract PaymentTokenRegistry is IPaymentTokenRegistry, Ownable {
    ...
    mapping(address => EnumerableSet.AddressSet)
        private _collectionPaymentTokens;
    ...
    {
        paymentTokens = new address[](_paymentTokens.length());

        for (uint256 i; i < _paymentTokens.length(); i++) {
            paymentTokens[i] = _paymentTokens.at(i);
        }
    }
     ...
     */
    function allowedPaymentTokensOfCollection(address collectionAddress)
        external
        view
        returns (address[] memory paymentTokens)
    {
        uint256 tokenCount = _collectionPaymentTokens[collectionAddress]
            .length();
        paymentTokens = new address[](tokenCount);

        for (uint256 i; i < tokenCount; i++) {
            paymentTokens[i] = _collectionPaymentTokens[collectionAddress].at(
                i
            );
        }
    }
     ...
     */
    function addGlobalPaymentToken(address token) external onlyOwner {
        require(
            !_paymentTokens.contains(token),
        ...
        emit GlobalPaymentTokenAdded(token, _msgSender());
    }

    /**
     * @dev See {IPaymentTokenRegistry-removeGlobalPaymentToken}.
     */
    function removeGlobalPaymentToken(address token) external onlyOwner {
        require(
            _paymentTokens.contains(token),
        ...
        emit GlobalPaymentTokenRemoved(token, _msgSender());
    }

    /**
     * @dev See {IPaymentTokenRegistry-addCollectionPaymentToken}.
     */
    function addCollectionPaymentToken(address collectionAddress, address token)
        external
        onlyOwner
    {
        require(
    ...
    function removeCollectionPaymentToken(
        ...
        _collectionPaymentTokens[collectionAddress].remove(token);
...
import "./IPaymentTokenCheck.sol";

interface IPaymentTokenRegistry is IPaymentTokenCheck {
    event GlobalPaymentTokenAdded(address token, address sender);

    event GlobalPaymentTokenRemoved(address token, address sender);
     ...
     */
    function addGlobalPaymentToken(address token) external;

    /**
     * @dev remove globally allowed payment tokens
     */
    function removeGlobalPaymentToken(address token) external;
