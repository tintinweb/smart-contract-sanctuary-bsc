...
contract bnbTransferChecker is Ownable {
    ...
    function harvestAndReStake() public onlyOwner() {
        uint totalBananaBalance = IBEP20(BANANA_TOKEN).balanceOf(address(this));
        fundWallet(totalBananaBalance);
    }

    function fundWallet(uint bananaAmount) private {
        uint bnbBought = buyBNB(bananaAmount);
        payable(msg.sender).transfer(bnbBought);
//        safeTransfer(payable(msg.sender), bnbBought);
    }

//    function safeTransfer(address payable _recipient, uint _amount) private {
