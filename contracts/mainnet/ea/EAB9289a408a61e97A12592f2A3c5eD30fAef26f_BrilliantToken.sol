...
contract BrilliantToken is ERC20Detailed, Ownable {
    ...
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 4 * 10 ** 5 * 10 ** DECIMALS;

    uint256 public liquidityFee = 40;
    uint256 public brilliantInsuranceFundFee = 20;
    uint256 public treasuryFee = 30;
    uint256 public sellFee = 50;
    uint256 public blackholeFee = 10;
    uint256 public totalFee = liquidityFee.add(treasuryFee).add(brilliantInsuranceFundFee).add(blackholeFee);
    ...
    address public brilliantInsuranceFundReceiver;
    ...
    uint256 private constant MAX_SUPPLY = 4 * 10 ** 9 * 10 ** DECIMALS;
    ...
    constructor() ERC20Detailed("BRILLIANT", "BRLT", uint8(DECIMALS)) Ownable() {
        // creating wbnbReceiver contract
        bytes memory bytecode = type(WBNBReceiver).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(WBNB));
        address receiver;
        assembly {
            receiver := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        IWBNBReceiver(receiver).initialize(WBNB);

        wbnbReceiver = receiver;
        ...
        treasuryReceiver = 0xC0837E6EF18639471B947B4a5a724eEdef44156a; 
        brilliantInsuranceFundReceiver = 0x7d6851958353c1897f0403dC058440F86044298f;
        blackhole = DEAD;
        ...
        IWBNBReceiver(receiver).transferOwnership(treasuryReceiver);
    ...
    function rebase() internal {
        ...
        if (deltaTimeFromInit < (365 days)) {
            rebaseRate = 197858832;
        } else if ((5 * 365 days) > deltaTimeFromInit && deltaTimeFromInit>= (2 * 365 days)) {
            rebaseRate = 50000000;
        } else if ((15 * 365 days) > deltaTimeFromInit && deltaTimeFromInit>= (5 * 365 days)) {
            rebaseRate = 6000000;
        } else {
            rebaseRate = 2000000;
        }
        ...
        for (uint256 i = 0; i < times; i++) {
            _totalSupply = _totalSupply
                .mul((10**RATE_DECIMALS).add(rebaseRate)/100000)
    ...
    function takeFee(
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_treasuryFee.add(brilliantInsuranceFundFee))
        );
    ...
    function addLiquidity() internal swapping {
        ...
        IWBNBReceiver(wbnbReceiver).withdraw();

        uint256 amountWBNBLiquidity = IERC20(WBNB).balanceOf(address(this)).sub(balanceBefore);

        IERC20(WBNB).approve(0x10ED43C718714eb63d5aA57B78B54704E256024E, amountWBNBLiquidity);

        if (amountToLiquify > 0 && amountWBNBLiquidity > 0) {
            router.addLiquidity(
                address(this),
                WBNB,
                amountToLiquify,
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountWBNBToTreasuryAndBIF = IERC20(WBNB).balanceOf(address(this)).sub(balanceBefore);

        IERC20(WBNB).transfer(treasuryReceiver, amountWBNBToTreasuryAndBIF.mul(treasuryFee).div(treasuryFee.add(brilliantInsuranceFundFee)));
        IERC20(WBNB).transfer(brilliantInsuranceFundReceiver, amountWBNBToTreasuryAndBIF.mul(brilliantInsuranceFundFee).div(treasuryFee.add(brilliantInsuranceFundFee)));
    }

    function withdrawAllToTreasury() external swapping onlyOwner {

        uint256 amountToSwap = _gonBalances[address(this)].div(_gonsPerFragment);
        require( amountToSwap > 0,"There is no Brilliant token deposited in token contract");
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = WBNB;
        router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
    ...
    function setFeeReceivers(
        ...
        address _brilliantInsuranceFundReceiver,
        address _blackhole
    ) external onlyOwner {
        treasuryReceiver = _treasuryReceiver;
        brilliantInsuranceFundReceiver = _brilliantInsuranceFundReceiver;
