...
import {IFireCatNFTUpgradeable} from "../src/interfaces/IFireCatNFTUpgradeable.sol";


contract FireCatNFTUpgradeable is IFireCatNFTUpgradeable, ERC721EnumerableUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event SetLevelProxy(address levelProxy);
    event SetLevelSource(address levelSource);
    ...
    */
    ...
    /// @inheritdoc IFireCatNFTUpgradeable
    function totalSupply() public view override(ERC721EnumerableUpgradeable, IFireCatNFTUpgradeable) returns (uint256) {
        return _totalSupply;
    }
    ...
    /// @inheritdoc IFireCatNFTUpgradeable
    function tokenIdOf(address owner) public view override(IFireCatNFTUpgradeable) returns (uint256[] memory) {
    ...
    function tokenURI(uint256 tokenId)
        ...
        view
        virtual
        override(IFireCatNFTUpgradeable, ERC721Upgradeable)
    ...
    */
    ...
    /// @inheritdoc IFireCatNFTUpgradeable
    function upgradeToken(uint256 tokenId) external override(IFireCatNFTUpgradeable) nonReentrant {
        ...
        require(levelProxy != address(0) && levelSource != address(0), "NFT:E06");

        bytes memory callData = abi.encodeWithSignature("isQualified(uint256)", tokenId);
        (bool res, bytes memory returnData) = levelProxy.delegatecall(callData);
    ...
    /// @inheritdoc IFireCatNFTUpgradeable
    function setLevelProxy(address levelProxy_) external onlyOwner {
        levelProxy = levelProxy_;
        emit SetLevelProxy(levelProxy_);
    }

    /// @inheritdoc IFireCatNFTUpgradeable
    function setLevelSource(address levelSource_) external onlyOwner {
        levelSource = levelSource_;
        emit SetLevelSource(levelSource_);
    }
...
/**
* @notice IFireCatNFTUpgradeable
*/
interface IFireCatNFTUpgradeable is IERC721Upgradeable {
    ...
    */
    ...
    /**
    * @notice set the NFT upgrade logic contract of NFT.
    * @dev set to levelProxy.
    * @param levelProxy_ address.
    */
    function setLevelProxy(address levelProxy_) external;

    /**
    * @notice set the NFT upgrade condtiions contract of NFT.
    * @dev set to levelSource.
    * @param levelSource_ address.
    */
    function setLevelSource(address levelSource_) external;
