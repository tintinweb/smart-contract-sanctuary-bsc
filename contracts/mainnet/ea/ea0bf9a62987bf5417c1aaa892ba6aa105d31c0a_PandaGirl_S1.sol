...
contract PandaGirl_S1 is ERC721 {
    ...
    mapping (uint256 => uint256) private tokenTypeMaxSupply ;
    mapping (uint256 => TokenRarityType) private tokenType2Rarity ;
    mapping (uint256 => TokenRarityType) private tokenId2Rarity ;
    mapping (uint256 => uint256) private tokenTypeMinted ;
    
    constructor (string memory name, string memory symbol, string memory tokenURIPrefix, uint256 royaltyFee_, address _creator, address _operator) ERC721 (name, symbol) {
        tokenCounter = 1;
        owner = msg.sender;
        ...
        tokenTypeMaxSupply[1] = 3800;
        tokenTypeMaxSupply[2] = 3800;
        tokenTypeMaxSupply[3] = 3800; 
        tokenTypeMaxSupply[4] = 2500; 
        tokenTypeMaxSupply[5] = 2500; 
        tokenTypeMaxSupply[6] = 1300;
        tokenTypeMaxSupply[7] = 1300;
        tokenTypeMaxSupply[8] = 460;
        tokenTypeMaxSupply[9] = 460;
        tokenTypeMaxSupply[10] = 80;    
    ...
    */
    ...
    function getRarity(uint256 _tokenId) external view returns (uint256) {
        require(_tokenId <= totalSupply , "Rarity Unavailable for the given TokenId");
        return uint256 ( tokenId2Rarity[_tokenId] ) ;
    }

    /**
    * @notice A method for a stakeholder to create a stake.
    * @param tokenTypes Type of token from the defined 10 types ( [1,2,3,4...10]).
    */
    function mintNFTLoot(address from,uint256[] calldata tokenIds,uint256[] calldata tokenTypes) external onlyOperator returns (bool) {
        require((tokenTypes.length + (tokenCounter-1)) <= totalSupply, "Mint exceeding Total Loot Supply");
        for(uint i=0; i < tokenTypes.length; i++) {
            require(tokenTypes[i] <= 10, "Token Type Error");
            require(tokenTypeMinted[tokenTypes[i]] + 1 <= 20000, "Mint exceeding Total Supply");
            _safeMint(from,tokenIds[i]);
            _setTokenURI(tokenIds[i], tokenTypes[i].toString());
            tokenTypeMinted[tokenTypes[i]] += 1 ;
            tokenId2Rarity[tokenIds[i]] = tokenType2Rarity[tokenTypes[i]];            
