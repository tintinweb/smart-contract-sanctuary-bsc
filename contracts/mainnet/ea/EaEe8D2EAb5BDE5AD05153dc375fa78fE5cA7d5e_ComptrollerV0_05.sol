...
contract ComptrollerV0_05 is ComptrollerStorageOlaV0_05, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
    ...
    // OLA_ADDITIONS : This event
    /// @notice Emitted when the custom oracle for an underlying is changed
    event NewCustomPriceOracle(address indexed underlying, address indexed oldOracle, address indexed newOracle);
    ...
    /**
     * @notice Is there a custom oracle defined for this
     * @param underlying The underlying
     * @return The asset price
     */
    function hasCustomOracle(address underlying) external view returns (bool) {
        return customOracles[underlying] != address(0);
    }

    /**
     * @notice Returns the effective price used in this LeN for the underlying
     * @param underlying The underlying
     * @dev Custom oracle will be used if available, otherwise returns the ministry price
    ...
    function getUnderlyingPriceInLen(address underlying) external view returns (uint) {
        return getUnderlyingPriceInternal(underlying);
    }
        ...
        if (!markets[cToken].accountMembership[borrower])
{
    ...
    function _setCustomOracle(address underlying, address oracle) external returns (uint) {
        // Check caller is admin
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_CUSTOM_ORACLE_OWNER_CHECK);
        }

        if (oracle != address(0)) {
            // Sanity -- asset is supported by oracle
            uint oraclePrice = PriceOracle(oracle).getAssetPrice(underlying);
            require(oraclePrice != 0, "bad oracle");
        }

        // Safety -- can only have a custom oracle for supported assets
        uint ministryPrice = RegistryForComptrollerV0_05(registry).getPriceForAsset(underlying);
        require(ministryPrice != 0, "unsupported asset");

        address oldCustomOracle = customOracles[underlying];
        customOracles[underlying] = oracle;

        emit NewCustomPriceOracle(underlying, oldCustomOracle, oracle);
     ...
     */
    function getUnderlyingPriceInternal(address underlying) internal view returns (uint256) {
        address customOracle = customOracles[underlying];

        if (customOracle != address(0)) {
            return PriceOracle(customOracle).getAssetPrice(underlying);
        } else {
            return RegistryForComptrollerV0_05(registry).getPriceForAsset(underlying);
        }
    }
