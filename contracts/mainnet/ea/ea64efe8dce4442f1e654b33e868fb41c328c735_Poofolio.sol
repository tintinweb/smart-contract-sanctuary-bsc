...
contract Poofolio is IERC20 {
    ...
    string constant private _name = "Poofolio";
    string constant private _symbol = "Poofolio";
    uint8 constant private _decimals = 18;
    uint256 constant private _tTotal = startingSupply * 10**_decimals;
    uint public maxWallet = ( _tTotal * 1 / 100); // 1% Max Wallet
    uint public maxTX = ( _tTotal * 1 / 100); // 1% Max Transaction
    ...
    TaxWallets public _taxWallets = TaxWallets({
        marketing: payable(0x75a6c5f40D210b0Ea96d7B2efaBc8b8a3D845d4e),
        charity: payable(0x75a6c5f40D210b0Ea96d7B2efaBc8b8a3D845d4e),
        reserve: payable(0x75a6c5f40D210b0Ea96d7B2efaBc8b8a3D845d4e),
        development: payable(0x75a6c5f40D210b0Ea96d7B2efaBc8b8a3D845d4e)
    });
    ...
    constructor () payable {
        ...
        } else if (block.chainid == 20202021) {
            dexRouter = IRouter02(0x038B68F8827444f36863bA76ea424bC781980826);
...
//===============================================================================================================
    ...
    function transferOwner(address newOwner) external onlyOwner {
        ...
        setExcludedFromLimits(_owner, false);
        setExcludedFromLimits(newOwner, true);
...
//===============================================================================================================
    ...
    function isExcludedFromLimits(address account, bool enabled) public onlyOwner {
        _isExcludedFromLimits[account] = enabled;
    }
    ...
    function setRatios(uint16 reflection, uint16 marketing, uint16 charity, uint16 reserve, uint16 development) external onlyOwner {
        _ratios.reflection = reflection;
        _ratios.marketing = marketing;
        _ratios.charity = charity;
        _ratios.reserve = reserve;
        _ratios.development = development;
        _ratios.totalSwap = marketing + reserve + development + charity;
        uint256 total = _taxRates.buyFee + _taxRates.sellFee;
        require(_ratios.totalSwap + _ratios.reflection <= total, "Cannot exceed sum of buy and sell fees.");
    }

    function setWallets(address payable marketing, address payable reserve, address payable charity, address payable development) external onlyOwner {
        require(marketing != address(0) && development != address(0) && charity != address(0) && reserve != address(0), "Cannot be zero address.");
    ...
    function contractSwap(uint256 contractTokenBalance) internal inSwapFlag {
        ...
        uint256 marketingBalance = amtBalance - (developmentBalance + reserveBalance + charityBalance);
        if (ratios.charity > 0) {
        ...
        if (ratios.reserve > 0) {
            (success,) = _taxWallets.reserve.call{value: reserveBalance, gas: 55000}("");
        }
    ...
    function finalizeTransfer(address from, address to, uint256 tAmount, bool buy, bool sell, bool other) internal returns (bool) {
        if(!_isExcludedFromLimits[from] || !_isExcludedFromLimits[to]) {
            require(tAmount<=maxTX,"Amount Exceeds Max TX");
        }
        bool takeFee = true;
