...
contract MasRelic is Pausable, Ownable, ReentrancyGuard, IERC20 {
    ...
    string private constant _name = "MasRelic";
    string private constant _symbol = "RELIC";

    uint256 public buyTreasuryFeeBPS = 1;
    uint256 public buyLiquidityFeeBPS = 1;
    uint256 public buyRewardFeeBPS = 598;
    uint256 public buyTotalFeeBPS = 600;

    uint256 public sellTreasuryFeeBPS = 1;
    uint256 public sellLiquidityFeeBPS = 1;
    uint256 public sellRewardFeeBPS = 598;
    uint256 public sellTotalFeeBPS = 600;

    uint256 public bridgeTotalFeeBPS = 200;
    ...
    event UnBridged(address indexed user,address indexed destinationAddress, uint256 amount);
    event RelicBridged(address indexed user, uint256 amount);
    ...
    function mintBridgedTokens(address relicHolder, uint256 amount) external onlyOwner returns(bool) {
         relicUserInfo[relicHolder] = RelicHolderInfo(relicHolder, address(0), amount, true, false);
        _mint(relicHolder, amount); 
        rewardTracker.setBalance(payable(relicHolder), amount);
        emit RelicBridged(relicHolder, amount);       
        return true;
    }



    function burnBridgedTokensMultiSides(uint256 amount, address destinationAddress) external nonReentrant returns (bool) {
         require(_balances[_msgSender()] >= amount , 
                    "User does not exist or does not hold tokens.");
         
         if (_msgSender() != owner()){
            uint256 fees; 
            fees = (amount * bridgeTotalFeeBPS) / 10000;
            amount -= fees;
            _executeTransfer(_msgSender(), autoRelic, fees);
         }
         
         relicUserInfo[_msgSender()] = RelicHolderInfo(_msgSender(),destinationAddress, amount, true, true);         
         _burn(_msgSender(), relicUserInfo[_msgSender()].bridgedAmount);                 
         rewardTracker.setBalance(payable(_msgSender()), balanceOf(_msgSender()));
         emit UnBridged(_msgSender(),destinationAddress, relicUserInfo[_msgSender()].bridgedAmount);
         return true;
    }
    ...
    function _transfer(
        ...
        if (
            ...
            require(
                ...
                "Wallet holds too large amount of RELIC"
            );
        }
    ...
    function rescueETH(uint256 _amount) external onlyOwner {
        ...
        require(success, "BNB rescue failed.");
...
contract RewardTracker is Ownable, IERC20 {
    ...
    string private constant _name = "MasRelic_RewardTracker";
    string private constant _symbol = "MasRelic_RewardTracker";
        ...
        onlyOwner
    {
        require(
            ...
            "MasRelic_RewardTracker: account already set to requested state"
        );
        excludedFromRewards[account] = excluded;
        if (excluded) {
            _setBalance(account, 0);
    ...
    function _mint(address account, uint256 amount) private {
        require(
            account != address(0),
            "MasRelic_RewardTracker: mint to the zero address"
        );
        _totalSupply += amount;
        _balances[account] += amount;
        emit Transfer(address(0), account, amount);
    ...
    function _burn(address account, uint256 amount) private {
        require(
            account != address(0),
            "MasRelic_RewardTracker: burn from the zero address"
        );
        uint256 accountBalance = _balances[account];
        require(
            accountBalance >= amount,
            "MasRelic_RewardTracker: burn amount exceeds balance"
        );
        _balances[account] = accountBalance - amount;
        _totalSupply -= amount;
        emit Transfer(account, address(0), amount);
    ...
    function transfer(address, uint256) public pure override returns (bool) {
        revert("MasRelic_RewardTracker: method not implemented");
    }
