...
interface IDayOfRightsClubPackage {
    function mint(address _recipient, uint _type) external;
}
...
contract AMOPool is Ownable {
    ...
    function giveInvReward(address account, uint256 amount) internal {
        ...
        address referrer1 = user.referrers(referrer);
        if (referrer1 != address(0) && referrer1 != address(user)) {
            giveInvStock(referrer1, amount.mul(2), 2);
        } else {
            referrer1 = fundAddr;
        }
        usdt.safeTransferFrom(
            msg.sender,
            referrer1,
            amount.mul(invUsdtRate[2]).div(100)
        );

        address referrer2 = user.referrers(referrer1);
        if (referrer2 != address(0) && referrer2 != address(user)) {
            giveInvStock(referrer2, amount.mul(2), 3);
        } else {
            referrer2 = fundAddr;
        }
        usdt.safeTransferFrom(
            msg.sender,
            referrer2,
            amount.mul(invUsdtRate[3]).div(100)
        );
    }
