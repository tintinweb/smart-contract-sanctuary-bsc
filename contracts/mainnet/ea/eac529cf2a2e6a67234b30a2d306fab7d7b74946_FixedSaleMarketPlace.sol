...
contract FixedSaleMarketPlace is ReentrancyGuard, RoleManager {
    ...
    function setBrokerDetails(address payable _broker, uint256 _brokerageAmt) external isOperator
    {
        _setBrokerDetails(_broker, _brokerageAmt);
    }

    function _setBrokerDetails(address payable _broker, uint256 _brokerageAmt) internal {
        require(_brokerageAmt < 100 * decimalPrecision, "Brokerage can't be 100%");
    ...
    function getTokensForSalePerUser(address _user) external view returns (TokenDetArrayLib.TokenDet[] memory) {
    ...
    function buy(address nftContract, uint256 tokenId) external payable nonReentrant whenNotPaused saleExists(nftContract, tokenId)
    {
        ...
        uint256 royality;
        if (royalities != 0) {
            royality = royalities.mul(sales[nftContract][tokenId].price).div(100).div(decimalPrecision);
        } else {
            royality = 0;
        }
        ...
        uint256 _brokerage = price.mul(brokerage).div(100).div(decimalPrecision);
        ...
        if (creator == fundsRecipient) {
            ...
            transferBNBOrWBNB(fundsRecipient, price.sub(_brokerage).sub(royality));
        }
