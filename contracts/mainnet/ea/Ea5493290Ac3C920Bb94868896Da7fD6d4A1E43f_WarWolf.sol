...
contract WarWolf is IERC20, Ownable, BaseToken {
    ...
    constructor(
        ...
        _maxWalletToken = ( totalSupply_ * 1 ) / 100;
        ...
        _maxTxAmount = totalSupply_.mul(5).div(10**2); // 5%
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (to != address(this)  && to != address(DEAD) && to != uniswapV2Pair && to != _marketingAddress && to != _devAddress && !_isExcludedFromMaxWallet[to] && !_isExcludedFromMaxWallet[from]){
    ...
    function excludeFromMaxWallet(address _user, bool exempt) external onlyOwner() {
        _isExcludedFromMaxWallet[_user] = exempt;
    }
