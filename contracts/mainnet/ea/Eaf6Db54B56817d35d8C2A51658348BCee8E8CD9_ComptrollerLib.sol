...
/*
    ...
    (c) FNDZ Council <[email protected]>
...
/// @author FNDZ Council <[email protected]>
...
contract ComptrollerLib is IComptroller {
    ...
    event PostRedeemSharesHookFailed(
        bytes failureReturnData,
        address redeemer,
        uint256 sharesQuantity
    );
        ...
        onlyActive
    {
        ...
        } else if (_action == VaultAction.RedeemVirtualShares) {
            __vaultActionRedeemVirtualShares(_actionData);
        } else if (_action == VaultAction.RedeemAndSwapVirtualShares) {
            __vaultActionRedeemAndSwapVirtualShares(_actionData);
        } else if (_action == VaultAction.SwapAsset) {
            __vaultActionSwapAsset(_actionData);
        }
    }
        ...
        if (_caller == INTEGRATION_MANAGER) {
            ...
            require(
                _action == VaultAction.BurnShares ||
                    ...
                    _action == VaultAction.RedeemVirtualShares ||
                    _action == VaultAction.RedeemAndSwapVirtualShares ||
                    _action == VaultAction.SwapAsset,
    ...
    /// @dev Helper to swap an asset from one token to another
    function __vaultActionSwapAsset(bytes memory _actionData) private {
        (address fromAsset, address toAsset, uint256 amount) = abi.decode(
            _actionData,
            (address, address, uint256)
        );
        IVault(vaultProxy).addTrackedAsset(toAsset);
        IVault(vaultProxy).swapAsset(fromAsset, toAsset, amount);
    }

    /// @dev Helper to redeem virtual shares on behalf of a given account
    function __vaultActionRedeemVirtualShares(bytes memory _actionData) private {
        (address target, uint256 amount) = abi.decode(_actionData, (address, uint256));
        (address[] memory payoutAssets, uint256[] memory payoutAmounts) = IVault(vaultProxy)
            .redeemVirtualShares(target, amount);
        emit SharesRedeemed(target, amount, payoutAssets, payoutAmounts, true);
    }

    /// @dev Helper to redeem and swap virtual shares on behalf of a given account
    function __vaultActionRedeemAndSwapVirtualShares(bytes memory _actionData) private {
        (address target, uint256 amount) = abi.decode(_actionData, (address, uint256));
        (address[] memory payoutAssets, uint256[] memory payoutAmounts) = IVault(vaultProxy)
            .redeemAndSwapVirtualShares(target, amount);
        emit SharesRedeemed(target, amount, payoutAssets, payoutAmounts, true);
    }
    ...
    /// @param _investmentAmount The amounts of the fund's denomination asset
    ...
    /// @return sharesReceivedAmount_ The actual amounts of shares received
    ...
    function buyShares(
        ...
        locksReentrance
        allowsPermissionedVaultAction
        returns (uint256 sharesReceivedAmount_)
    {
        ...
        __buySharesSetupHook(msg.sender, _investmentAmount, gav);
        ...
        gav = gav.add(_investmentAmount);
        __buySharesCompletedHook(msg.sender, sharesReceivedAmount_, gav);
    ...
    function __buySharesSetupHook(
        ...
        IPolicyManager(POLICY_MANAGER).validatePolicies(
            ...
            abi.encode(_caller, _investmentAmount, _gav)
        );

        IFeeManager(FEE_MANAGER).invokeHook(
            IFeeManager.FeeHook.BuySharesSetup,
            abi.encode(_caller, _investmentAmount),
            _gav
        );
    }
    ...
    function __preBuySharesHook(
        ...
        IFeeManager(FEE_MANAGER).invokeHook(
            ...
            abi.encode(_buyer, _investmentAmount, _minSharesQuantity, _referrer),
            _gav
        );
    ...
    /// @param _sharesQuantity The quantity of shares to redeem. If 0, redeem all shares
    ...
    function redeemSharesDetailed(
        ...
        if (_sharesQuantity == 0) {
            _sharesQuantity = ERC20(vaultProxy).balanceOf(msg.sender);
        }
    ...
    /// @dev Helper for system actions immediately later to redeeming shares.
    function __postRedeemSharesHook(address _redeemer, uint256 _sharesQuantity)
        private
        allowsPermissionedVaultAction
    {
        try
            IFeeManager(FEE_MANAGER).invokeHook(
                IFeeManager.FeeHook.PostRedeemShares,
                abi.encode(_redeemer, _sharesQuantity),
                0
            )
         {} catch (bytes memory reason) {
            emit PostRedeemSharesHookFailed(reason, _redeemer, _sharesQuantity);
        }
    }
        ...
        returns (address[] memory payoutAssets_, uint256[] memory payoutAmounts_)
    {
        ...
        __postRedeemSharesHook(_redeemer, _sharesQuantity);

        emit SharesRedeemed(_redeemer, _sharesQuantity, payoutAssets_, payoutAmounts_, false);

        return (payoutAssets_, payoutAmounts_);
    }

    /// @notice Redeem a specified quantity of the sender's shares for a proportionate slice of
    /// the fund's assets, optionally specifying additional assets and assets to swap to the denomination asset.
    /// @param _sharesQuantity The quantity of shares to redeem
    /// @param _swapData The encoded swap data
    /// @return payoutAssets_ The assets paid out to the redeemer
    /// @return payoutAmounts_ The amount of each asset paid out to the redeemer
    /// @dev This function can only be called during optimal vault operation i.e. no tracked assets are
    /// causing the withdrawal to fail (due to being paused for example). Additional untracked assets will be
    /// forfeited. This is intended to be a convenience function for when investors wish to cash out their
    /// vault shares back out for the denomination assets. redeemSharesDetailed() is the escape hatch in case
    /// some assets are causing the share redemption to fail. There, the user may skip bugged assets and also
    /// claim additional untracked assets, if any exist.
    function redeemSharesAndSwap(uint256 _sharesQuantity, bytes calldata _swapData)
        external
        locksReentrance
        returns (address[] memory payoutAssets_, uint256[] memory payoutAmounts_)
    {
        require(_sharesQuantity > 0, "redeemSharesAndSwap: _sharesQuantity must be >0");
        (
            address[][] memory swapPaths,
            uint256[] memory minimumDestinationAmounts,
            uint256 swapDeadline
        ) = abi.decode(_swapData, (address[][], uint256[], uint256));
        require(
            swapPaths.length == minimumDestinationAmounts.length,
            "redeemSharesAndSwap: swapPaths length must be minimumDestinationAmounts length"
        );
        // // Requirement statement that puts the contract over the limit
        // // Warning: Contract code size exceeds 24576 bytes
        // // Technically not required because UniswapV2Router: EXPIRED will be thrown deeper
        // // in the call stack
        // require(
        //     swapDeadline >= block.timestamp,
        //     "redeemSharesAndSwap: swapDeadline must be in the future"
        // );
        ...
        // Only apply the sharesActionTimelock when a migration is not pending
        if (!IDispatcher(DISPATCHER).hasMigrationRequest(address(vaultProxyContract))) {
            __assertSharesActionNotTimelocked(msg.sender);
            acctToLastSharesAction[msg.sender] = block.timestamp;
        }
        ...
        if (!__fundIsPaused()) {
            ...
            // then those fee shares will be transferred from the user's balance rather
            // than reallocated from the sharesQuantity being redeemed.
            __preRedeemSharesHook(msg.sender, _sharesQuantity);
        }

        // Check the shares quantity against the user's balance after settling fees
        require(
            _sharesQuantity <= ERC20(address(vaultProxyContract)).balanceOf(msg.sender),
            "redeemSharesAndSwap: Insufficient shares"
        );

        payoutAssets_ = vaultProxyContract.getTrackedAssets();
        require(payoutAssets_.length > 0, "redeemSharesAndSwap: No payout assets");
        require(
            payoutAssets_.length == swapPaths.length,
            "redeemSharesAndSwap: payoutAssets_ length must match swapPaths"
        );

        // Destroy the shares.
        // Must get the shares supply before doing so.
        uint256 sharesSupply = ERC20(address(vaultProxyContract)).totalSupply();
        vaultProxyContract.burnShares(msg.sender, _sharesQuantity);
        ...
        for (uint256 i; i < payoutAssets_.length; i++) {
            ...
            // Transfer payout asset to redeemer
            if (payoutAmounts_[i] > 0 && swapPaths[i].length >= 2) {
                // Since we're swapping, update payout data to reflect received asset
                payoutAmounts_[i] = vaultProxyContract.swapAndWithdrawAssetTo(
                    payoutAssets_[i],
                    denominationAssetCopy,
                    msg.sender,
                    payoutAmounts_[i],
                    minimumDestinationAmounts[i],
                    swapDeadline,
                    swapPaths[i]
                );
                payoutAssets_[i] = denominationAssetCopy;
            } else if (payoutAmounts_[i] > 0) {
        ...
        __postRedeemSharesHook(msg.sender, _sharesQuantity);

        emit SharesRedeemed(msg.sender, _sharesQuantity, payoutAssets_, payoutAmounts_, false);
