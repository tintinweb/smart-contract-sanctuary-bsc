...
contract BidaMigration is ReentrancyGuard, Context, Ownable{
    ...
    function migrateToV2(uint256 v1amount) public nonReentrant(){
        ...
        require(onlyRequire[msg.sender], 'Forbidden');
        if (claimed[msg.sender] && !isExcluded[msg.sender]) {
    ...
    function onlyRequireToMigrate(address[] memory _excluded, bool status) external onlyOwner  {
        uint256 lent = _excluded.length;
        for (uint256 i; i < lent; ) {
            onlyRequire[_excluded[i]] = status;
            unchecked {
                i++;
            }
        }
