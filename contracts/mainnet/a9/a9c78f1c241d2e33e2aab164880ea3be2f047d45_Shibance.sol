...
contract Shibance is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ShibanceDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Shibance", "SHIBANCE") {
        marketingWallet = payable(0xe21d36FcF14BC7D7584ca05BC8BFA4e55f4aC7bb);
        devWallet = payable(0x7e1E7712cD8bb11338cE84bF3ACA128FDa3b795e);
        ...
        dividendTracker = new ShibanceDividendTracker(
            payable(this),
            router,
...
contract ShibanceDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    Shibance public ShibanceContract;
    ...
    constructor(
        ...
        defaultToken = token;
        ShibanceContract = Shibance(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Shibance_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Shibance_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Shibance contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(ShibanceContract);

            uint256 prevBalance = ShibanceContract.balanceOf(address(this));
            ...
            {
                uint256 received = ShibanceContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    ShibanceContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
