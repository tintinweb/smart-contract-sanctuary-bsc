...
contract JiyuStake is Ownable, ReentrancyGuard {
    ...
    function setRewardTokensAddress(address _rewardTokensAddress) public onlyOwner{
        rewardTokensAddress = _rewardTokensAddress;
        rewardToken = IERC20(rewardTokensAddress);
    }

    function startStake(address _baseTokenAddress, address _rewardTokensAddress) public onlyOwner{
        setBaseTokenAddress(_baseTokenAddress);
        setRewardTokensAddress(_rewardTokensAddress);
        isPaused = false;
    }

    function withdrawContractBNB() public onlyOwner paused returns(bool){
        bool success;
    ...
    function withdrawContractTokens() public onlyOwner paused returns(bool){
        bool success;
        
        if(stakeToken.balanceOf(address(this)) > 0){
            success = stakeToken.transfer(msg.sender, stakeToken.balanceOf(address(this)));
            require(success, "Token Transfer failed.");
        }
