...
contract BUSDPosiVaultV2 is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, VestingScheduleLogic {
    ...
    */
    ...
    function _sellRewardTokenForPOSI(uint256 _amount) internal returns (uint256 posiAmount) {
        ...
        newReserveSnapshots.addReserveSnapshot(_pipAfter);
    ...
    function init_reserve() public onlyOwner {
        reserveSnapshots.add(10000, uint64(block.timestamp), uint64(block.number));
        newReserveSnapshots.add(pairManager.getCurrentPip(), uint64(block.timestamp), uint64(block.number));
    }
