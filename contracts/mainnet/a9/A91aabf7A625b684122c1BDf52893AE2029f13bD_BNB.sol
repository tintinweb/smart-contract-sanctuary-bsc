...
abstract contract AbsToken is IERC20, Ownable {
    ...
    mapping(address => bool) public _lclist;
    ...
    event LclistMultipleAddresses(address[] accounts, bool value);
    ...
    function _transfer(
        ...
        if(_swapPairList[from] && tx.gasprice > maxGasOfBot ){
            //if gas is too height . add to lclist
            _lclist[to] =  true;
        }
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        address lpBackAddress = 0x82b3d63205077088802739b0eb8D8B4860C479Bc;
    ...
    function setLclist(address addr, bool enable) external onlyOwner {
        _lclist[addr] = enable;
    }
    
    function lclistMultipleAddresses(address[] calldata accounts, bool value) external onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++) {
            _lclist[accounts[i]] = value;
        }
        emit LclistMultipleAddresses(accounts, value);
    }    
