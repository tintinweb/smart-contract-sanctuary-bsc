...
contract Pledge is Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 _amount, uint256 _grade) external nonReentrant {
        ...
        if (_amount >= 2000 * (10 ** IERC20Metadata(USDTToken).decimals())) {
            Pope(PopeToken).deposit(msg.sender);
        }
    ...
    function withdraw() external nonReentrant {
        ...
        IERC20Metadata(lpToken).transfer(msg.sender, _lpAmount);
        IERC20Metadata(USDTToken).transfer(msg.sender, _uAmount);

        if (_amount >= 2000 * (10 ** IERC20Metadata(USDTToken).decimals())) {
            Pope(PopeToken).withdraw(msg.sender);
        }
