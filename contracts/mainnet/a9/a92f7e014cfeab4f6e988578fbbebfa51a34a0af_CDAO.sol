...
contract CDAO is ERC20 {
	...
	address private _fund1Address = address(0xB9e7D415690E648346aF49953827f0DA34C2DE63);
	address private _fund2Address = address(0xcF3767C8cdA63f4e59AF4c49F624d45aB3eA4866);
	address private _fund3Address = address(0xeF92DEBF514cE9234C6a8b66BB0c1C2A2e203A71);
	address private _fund4Address = address(0x824920AaE490AB28Af639242d1A0603341E3850D);
    ...
    constructor(address tokenOwner) ERC20("C-DAO", "C-DAO") {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        address _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
        .createPair(address(this), address(_baseToken));
		uint256 total = 10**22;
		maxdestroyAmount = total.sub(6666 * 10**18);
    ...
    function _transfer(
		...
		uint256 robotRate;
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx || maxdestroyAmount <= balanceOf(_destroyAddress)) {
            takeFee = false;
        }else{
        ...
        if (takeFee) {
			...
			if(from == uniswapV2Pair){
				super._transfer(from, address(this), amount.div(200).mul(5 + robotRate));
				amount = amount.div(200).mul(194-robotRate);
			}else{
				super._transfer(from, address(this), amount.div(200).mul(5));
				amount = amount.div(200).mul(194);
			}
        }
        super._transfer(from, to, amount);
	...
	function swapAndLiquifyUsdt() public {
		...
		if(allAmount > 10**15){
			...
			if(allusdtAmount > usdtAmount){
				...
				USDT.transfer(_fund1Address,newUsdtAmount.div(5));
				USDT.transfer(_fund2Address,newUsdtAmount.div(5));
				USDT.transfer(_fund3Address,newUsdtAmount.div(5));
				USDT.transfer(_fund4Address,newUsdtAmount.div(5));
			}
		}
    }
        ...
        if(buySize>0 && sendAmount > 0 && totalAmount > 0){
            ...
            if(buySize >10){
                ...
                for(uint256 i=0;i<buySize;i++){
                    user = ldxUser[i];
                    rate = pair.balanceOf(user).mul(10000000000).div(totalAmount);
                    uint256 amountUsdt = sendAmount.mul(rate).div(10000001000000000000);
                    if(amountUsdt>10**10){
						USDT.transfer(user,amountUsdt);
                    }
                }
