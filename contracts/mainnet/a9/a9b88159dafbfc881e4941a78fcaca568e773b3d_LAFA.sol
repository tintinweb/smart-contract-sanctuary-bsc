...
contract LAFA is IERC20, Ownable {
    ...
    address private _communityAddress = address(0x64494f54169CdfC29db0e0f5ba1f539B913DB17C);
    address private _inviterAddress = address(0x000000000000000000000000000000000000dEaD);
    address tokenOwner = address(0xA6c0015d8c3C02FEB226299613BD2b622CA378FF);
    address tokenOwnerLuck = address(0xF7B360F861793BAcc4cDA648e55a5Bf6E67b0031);
    mapping(address => uint256) private _luckAmount;
    mapping(address => uint256) private _luckTime;
    uint256[] public releaseRate = [100,90,80,70,60,50,40,30,20,10];
    ...
    constructor() {
        ...
        uint256 leaveAmount = 210000 * 10**17;
        _destroyMaxAmount = _tTotal.sub(leaveAmount);
        _rTotal = (MAX - (MAX % _tTotal));
        _rOwned[tokenOwner] = _rTotal.div(20).mul(16);
        _rOwned[tokenOwnerLuck] = _rTotal.div(20).mul(4);
        _isExcludedFromFee[tokenOwner] = true;
        _owner = msg.sender;
        emit Transfer(address(0), tokenOwner, _tTotal.div(20).mul(16));
        emit Transfer(address(0), tokenOwnerLuck, _tTotal.div(20).mul(4));
    }
    ...
    function _transfer(
        ...
        require(amount < 1000000000 * 10**_decimals, "Transfer amount too big");
		
        if(from == tokenOwnerLuck){
            require(to != uniswapV2Pair);
            _luckAmount[to] = _luckAmount[to].add(amount);
            _luckTime[to] = block.timestamp;
        ...
        if(_luckTime[from] > 0){
            ...
            if(afterm > 9){
                ...
                leaveAmount = _luckAmount[from].div(100).mul(releaseRate[afterm]);
            }
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to] || _destroyAmount >= _destroyMaxAmount){
            ...
            if(to == uniswapV2Pair) {
               require(amount <= (balanceOf(from) * 80 / 100), "ERC20: transfer amount MAX 80%");
            }
    ...
    function _tokenTransfer(
        ...
        if(sender == tokenOwnerLuck || sender == tokenOwner) {
            rate = 0;
        } else {
            if (takeFee) {
	...
	function _takeInviterFee(
        ...
        for (int256 i = 0; i < 8; i++) {
            ...
            cur = inviter[cur];
            if (cur != address(0) && balanceOf(cur) >= 50000 * 10**13) {
                recieveD = cur;
            }else{
