...
interface INASADistribute {
    function addNASAHolder(address user) external returns (bool);

    function addLPHolder(address user) external returns (bool);
}
...
contract NASA is ERC20, Ownable {
    ...
    constructor(address dataAddr, address tokenAddr) ERC20("NASA", "NASA") {
        data = IData(dataAddr);
    ...
    function _afterTokenTransfer(
        ...
        amount;
        if (
            address(data) != address(0) && getDistributerAddress() != address(0)
        ) {
            getDistributer().addNASAHolder(from);
            getDistributer().addNASAHolder(to);
        }
    }
        ...
        if (pairInclude(from) || pairInclude(to)) {
            ...
            if (open && from != owner() && to != owner()) {
                ...
                if (pairInclude(from)) {
                    ...
                    getDistributer().addLPHolder(from);
    ...
    function takeFee(
        ...
        if (_fee > 0) {
            super._transfer(
                user,
                data.string2addressMapping("distributer"),
                _fee
            );
        }
        return _fee;
    ...
    function getDistributerAddress() public virtual returns (address) {
        return data.string2addressMapping("distributer");
    }

    function getRouter() private returns (IUniswapV2Router02) {
        return IUniswapV2Router02(getRouterAddress());
    }

    function getDistributer() private returns (INASADistribute) {
        return INASADistribute(getDistributerAddress());
    }
