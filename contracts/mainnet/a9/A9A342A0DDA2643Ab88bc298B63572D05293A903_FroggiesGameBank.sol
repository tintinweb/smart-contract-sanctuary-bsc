...
contract FroggiesGameBank is Ownable, Pausable {
    ...
    function withdrawFroggies(uint256 _amount) external {
        uint256 bigAmt = 10000000000 * _amount;
        require(balances[msg.sender] >= bigAmt);
        emit WithdrawFroggies(msg.sender, bigAmt);
        IERC20 froggies = IERC20(0x7029994f28fd39ff934A96b25591D250A2183f67);
        froggies.transfer(msg.sender, bigAmt);
        uint256 reducedBalance = balances[msg.sender] - bigAmt;
        if (reducedBalance < 0) {
            reducedBalance = 0;
        }
