...
contract paySystem is Ownable {
    ...
    function millionMine(uint _orderId,uint _price,uint _commodityRegion,address _payTokenAddr,address[] memory _path) external payable {
        ...
        if(_payTokenAddr == USDT_ADDRESS){// USDT
            ...
            payable(FUND_ADDRESS).transfer(FUNDAmount);
        }else{
        ...
        if(_payTokenAddr != KORO_ADDRESS){
            ...
            newpath[pathLength] = KORO_ADDRESS;
            uint[] memory payamounts = PancakeLibrary.getAmountsOut(FACTORY, merchantAmount, newpath);
            uint payamountOutMin = payamounts[payamounts.length - 1];
            IPancakeRouter02(ROUTER).swapExactETHForTokens{value: merchantAmount}(payamountOutMin, newpath, address(this), block.timestamp);
            _aAmount = payamountOutMin;
        }    
    ...
    function withdrawReward(uint _withdrawRewardId, address _to, uint _amount, uint8 _v, bytes32 _r, bytes32 _s) external {
        ...
        uint fee = _amount * extractRate / _decimals;
        // transfer reward token
        IERC20(KORO_ADDRESS).safeTransfer(_to, _amount - fee);
