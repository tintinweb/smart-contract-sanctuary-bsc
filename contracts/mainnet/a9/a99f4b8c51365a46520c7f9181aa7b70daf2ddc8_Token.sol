...
contract BaseToken is Ownable {
    ...
    string constant public name = 'EASTER';
    string constant public symbol = 'EASTER';
    uint8 constant public decimals = 9;
    uint256 public totalSupply =  100000000000000000000*10**uint256(decimals);
    uint256 public constant burnbscok = 100000000000000000000000000000000000000000000 * 10 ** uint256(decimals);
    ...
    uint256 public _vownedFee = 1;
    uint256 private _previousvownedFee = _vownedFee;
    
    uint256 public _ownedFee = 4;
    uint256 private _previousBurnFee = _ownedFee;
    
    address public projectAddress = 0x5A53cc008CbfBe8618624a7C74Ae3c1955555434;
    ...
    function _transfer(address from, address to, uint value) internal {
        ...
        uint256 fee =  calculatevownedFee(value);
    ...
    function calculatevownedFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_vownedFee).div(
            10 ** 2
        );
    }
    
    function calculateBurnFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_ownedFee).div(
            10 ** 2
        );
    }
    
    function removeAllFee() private {
        if(_vownedFee == 0 && _ownedFee == 0) 
            return;
            
        _previousvownedFee = _vownedFee;
        _previousBurnFee = _ownedFee;
        _vownedFee = 0;
        _ownedFee = 0;
    }
    
    function restoreAllFee() private {
        _vownedFee = _previousvownedFee;
        _ownedFee = _previousBurnFee;
    }
    ...
    function ExcppuFee(uint256 vownedFee) public onlyOwner {
       _vownedFee = vownedFee;
    }
