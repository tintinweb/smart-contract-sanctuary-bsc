...
// Telegram: https://t.me/vidya_gamez
...
contract VidyaGames is ERC20, ERC20Burnable, Ownable {
    ...
    // Max wallet and TX
    uint256 public _maxBuyTxAmount; // 2%
    uint256 public _maxSellTxAmount; // 1%
    uint256 public _maxWalletToken; // 2%
    ...
    constructor() ERC20("VidyaGamez", "TGA") {
        pair = pancakeFactory.createPair(pancakeRouter.WETH(), address(this));
        isTxLimitExempt[msg.sender] = true;
        _mint(msg.sender, 1000000000 * 10 ** decimals());
        _maxBuyTxAmount = totalSupply() * 200 / 10000; // 2%
        _maxSellTxAmount = totalSupply() * 100 / 10000; // 1%
        _maxWalletToken = ( totalSupply() * 200 ) / 10000; // 2%
    }
    ...
    function checkMaxWallet(address recipient, uint256 amount) internal view {
        if (recipient != owner() && recipient != address(this)  && recipient != address(DEAD) && recipient != address(ZERO) && recipient != pair){
