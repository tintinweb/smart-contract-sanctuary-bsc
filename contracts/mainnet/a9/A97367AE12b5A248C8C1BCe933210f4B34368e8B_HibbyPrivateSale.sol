...
contract HibbyPrivateSale is Context, Ownable {
  ...
  // Tokens to sold from this contract 
  uint256 public totalTokenToSell;
  ...
  uint256 public increaseAtToken = 1000000 * 10**18;
  ...
  // How many time any referral is used
  mapping(bytes32 => uint) public referralUsed;
  ...
  constructor(
    ...
    saleEnabled = true;
    totalTokenToSell = 4 * 10 ** 6 * 10 ** 18;
    priceIncreaseEnabled = true;
    rewardPercent = 5;
    startTime = block.timestamp;
    endTime = block.timestamp + 21 days;    
  }
  ...
  */
  ...
  // Function to check how many time the referral is used
  function getReferralUsed(address _address) external view returns(uint){
    bytes32 _referral = users[_address].userToReferral;
    return referralUsed[_referral];
  } 
  ...
  function setTotalTokenToSell(uint _amount) external onlyOwner{
    totalTokenToSell = _amount;
  }
