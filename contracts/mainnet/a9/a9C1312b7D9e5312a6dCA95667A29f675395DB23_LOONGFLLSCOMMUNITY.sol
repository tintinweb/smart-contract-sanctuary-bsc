...
contract LOONGFLLSCOMMUNITY is Context, Ownable {
    ...
    address public _receiver2=address(0xd6861a7Ef6Be10d8D287592cbd189e35D4b75b);//接收人2
    ...
    function buy(uint amount,uint status) external {
        ...
        if(status==0){
            ...
            path[0]=_usdt;
            if(_isDead==true){
                IBEP20(_usdt).transferFrom(_msgSender(),address(this),amount.mul(ratio2).div(100));
                path[1]=_loong;
                uint loong=IBEP20(_loong).balanceOf(address(this));
                IBEP20(_usdt).approve(address(uniswapV2Router),2**256-1);
            ...
            //社区币部分
            IBEP20(_usdt).transferFrom(_msgSender(),_receiver2,amount.mul(100-ratio2).div(100));
        }else{
            revert("error3");
        }
    }
