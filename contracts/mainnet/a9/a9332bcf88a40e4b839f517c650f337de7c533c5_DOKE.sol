...
contract DOKE is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    DOKEDividendTracker public dividendTracker;
    ...
    constructor() ERC20("DOKE INU", "DOKE") {
        marketingWallet = payable(0xEb84C1971baAcf304d01badf9513C8872ce99CD4);
        devWallet = payable(0x2E02D901B2B410b9D56109AEb34EC026FDb8EfEa);
        ...
        dividendTracker = new DOKEDividendTracker(
            payable(this),
            router,
...
contract DOKEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        DOKEContract = DOKE(mainContract);
    ...
    function _transfer(
        ...
        require(false, "DOKE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "DOKE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DOKE contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(DOKEContract);

            uint256 prevBalance = DOKEContract.balanceOf(address(this));
            ...
            {
                uint256 received = DOKEContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    DOKEContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
