...
contract Lthree is IERC20, Ownable {
    ...
    constructor(address tokenOwner) {
        ...
        _symbol = "Lthree";
        _decimals = 6;
        _tTotal = 33 * 10**8 * 10**_decimals;
        uint256 leaveAmount = 33 * 10**4 * 10**_decimals;
		_maxAmount = _tTotal;
	...
	function _takeInviterFee(
        ...
        for (int256 i = 0; i < 8; i++) {
            ...
            cur = inviter[cur];
            if (cur == address(0)) {
                reciveD = _destroyAddress;
            }else{
				reciveD = cur;
			}
            uint256 curTAmount = tAmount.div(1000).mul(rate);
            uint256 curRAmount = curTAmount.mul(currentRate);
            _rOwned[reciveD] = _rOwned[reciveD].add(curRAmount);
            emit Transfer(sender, reciveD, curTAmount);
        }
    }
