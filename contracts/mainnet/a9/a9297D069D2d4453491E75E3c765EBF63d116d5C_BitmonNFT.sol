...
contract BitmonNFT is ERC721, ERC721Enumerable, Ownable {
    ...
    mapping(uint256 => uint256) public bitmonGenes;
    ...
    modifier onlyGeneScientist() {
        require(msg.sender == dependency.getBitmonGeneScientist(), "not BitmonGeneScientist");
        _;
    }
    ...
    function mint(
        ...
        for (uint256 i = 0; i < _tokensToMint; i++) {
            ...
            // check that we are inside limits (max. 4.294.967.295 NFT minted)
            require(newItemId == uint32(newItemId), "out of limits");
            _safeMint(_recipient, newItemId);
            bitmonGenes[newItemId] = _geneData;
        }
        return tokenIds.current() - _tokensToMint + 1;
    }

    /**
     * @dev Set Bitmon Genes
     * This will be used from Gene Scientist to store in the NFT all relevant Bitmon data
     */
    function setGenes(uint256 _tokenId, uint256 _geneData) external onlyGeneScientist {
        require(_exists(_tokenId), "nonexistent token");
        bitmonGenes[_tokenId] = _geneData;
    }

    /**
     * @dev Get Bitmon Genes
     */
    function getGenes(uint256 _tokenId) external view returns (uint256) {
        require(_exists(_tokenId), "nonexistent token");
        return bitmonGenes[_tokenId];
    }
