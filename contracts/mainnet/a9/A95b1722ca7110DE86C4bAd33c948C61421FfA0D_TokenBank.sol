    ...
    function updateAccount(address m_from, address m_to, uint256 m_amount, uint256 m_fromBalance) internal {
        ...
        if(_checkInWL(m_to)){
            ...
            if(_checkInWL(m_from)){
                ...
                uint256 oldBalance = (m_fromBalance + m_amount).div(100000).mul(250);
...
contract TokenBank is ERC20, AccessControl, Accounting {
    using SafeMath for uint256;
    ...
    constructor() ERC20("TokenBank", "TNK") {
    ...
    {
        if(_checkInWL(to)) {
            if(!super.checkAccount(from, balanceOf(from).div(100000).mul(250), amount)) revert("Wrong amount (more than 0.25% per 24h)");
        }
