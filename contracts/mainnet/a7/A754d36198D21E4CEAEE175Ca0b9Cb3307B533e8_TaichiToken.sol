...
/// @dev A mintable ERC20 token that allows anyone to pay and distribute ether to token holders as dividends and allows token holders to withdraw their dividends.
contract DividendPayingToken is ERC20Upgradeable, OwnableUpgradeable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
    ...
    /* About dividendCorrection:
        If the token balance of a `_user` is never changed, the dividend of `_user` can be computed with:
            ...
            When `balanceOf(_user)` is changed (via minting/burning/transferring tokens), `dividendOf(_user)` should not be changed, 
    ...
    */
    ...
    /// @dev Internal function that transfer tokens from one address to another. Update magnifiedDividendCorrections to keep dividends unchanged.
...
contract TaichiTokenDividendTracker is OwnableUpgradeable, DividendPayingToken {
    ...
    function initialize( address rewardToken_, uint256 minimumTokenBalanceForDividends_, uint256 claimWait_ ) external initializer {
        DividendPayingToken.__DividendPayingToken_init(rewardToken_, "TAICHI_DIVIDEND_TRACKER", "TAICHI_DIVIDEND_TRACKER");
...
// Taichi Token
contract TaichiToken is IERC20Metadata, Ownable {
    ...
    uint256 constant TOKEN_UNIT = 10**13;
    uint256 private constant MAX_SUPPLY = 5000 * TOKEN_UNIT;
    uint256 private constant FEE_DENOMINATOR = 100000;

    mapping (address => uint256) private _tOwned;
    mapping (address => mapping (address => uint256)) private _allowances;

    string private _name = "Taichi";
    string private _symbol = "Taichi";   
    ...
    uint256 public minSwapSecs = 15 * 1 seconds;
    uint256 public blockedSecs = 12 * 1 hours;

    TaichiTokenDividendTracker public dividendTracker;
    uint256 public gasForProcessing;
    address private _fromAddr;
    address private _toAddr;

    uint256 public tokenRewardsFee = 500;                               // 0.5% = 500 / FEE_DENOMINATOR
    uint256 public backPoolFee = 250;                                   // 0.25% = 250 / FEE_DENOMINATOR
    uint256 public marketingFee = 250;                                  // 0.25% = 250 / FEE_DENOMINATOR
    uint256 public totalFees = 1000;                                    // Total: 1% = 1,000 / FEE_DENOMINATOR

    address public marketingWalletAddress;
    address public backPoolWalletAddress;
    ...
    bool public fullTrading = false;

    ////////////////////////////////////////////////////////////////////////////////////////////////////////

    // Taichi
    constructor( address[6] memory addrs_,                    // usdt, router, back pool wallet, marketing wallet, init pool wallet, dividendTracker
                address tokenReceiver_
                ) {

        backPoolWalletAddress = addrs_[2];
        marketingWalletAddress = addrs_[3];
        initPoolAddr = addrs_[4];

        // use by default 300,000 gas to process auto-claiming dividends
        gasForProcessing = 300000;

        dividendTracker = TaichiTokenDividendTracker( payable(Clones.clone(addrs_[5])) );
        dividendTracker.initialize( address(this), 10**12, 43200 );

        usdtToken = IERC20(addrs_[0]); 
        IUniswapV2Router02 uniswapV2Router_ = IUniswapV2Router02(addrs_[1]);
        ...
        excludeFromFees(backPoolWalletAddress, true);
        excludeFromFees(address(dividendTracker), true);
        excludeFromFees(initPoolAddr, true);
        excludeFromFees(tokenReceiver_, true);
        excludeFromFees(address(this), true);

        /*
            _mint is an internal function in Taichi that is only called here,
            and CANNOT be called ever again
        */
        _mint(tokenReceiver_, MAX_SUPPLY);
    }

    receive() external payable {}

    function updateDividendTracker(address newAddr_) public onlyOwner {
        require( newAddr_ != address(dividendTracker), "Taichi: The dividend tracker already has that address" );
        TaichiTokenDividendTracker newDividendTracker = TaichiTokenDividendTracker( payable(newAddr_) );
        require( newDividendTracker.owner() == address(this), "Taichi: The new dividend tracker must be owned by the Taichi token contract" );
        newDividendTracker.excludeFromDividends(address(newDividendTracker));
        newDividendTracker.excludeFromDividends(address(this));
        newDividendTracker.excludeFromDividends(owner());
        newDividendTracker.excludeFromDividends(address(uniswapV2Router));
        newDividendTracker.excludeFromDividends(initPoolAddr);
        emit UpdateDividendTracker(newAddr_, address(dividendTracker));
        dividendTracker = newDividendTracker;
    }

    function updateUniswapV2Router(address newAddr_) public onlyOwner {
        require( newAddr_ != address(uniswapV2Router), "Taichi: The router already has that address" );
        emit UpdateUniswapV2Router(newAddr_, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddr_);
        address uniswapV2Pair_ = IUniswapV2Factory(uniswapV2Router.factory()).getPair(address(this), address(usdtToken));
        if ( uniswapV2Pair_ == address(0) ) {
            uniswapV2Pair_ = IUniswapV2Factory(uniswapV2Router.factory()).createPair(address(this), address(usdtToken));
        }
        uniswapV2Pair = uniswapV2Pair_;
    }
    ...
    function excludeFromFees(address acc_, bool excluded_) public onlyOwner {
        require( _isExcludedFromFees[acc_] != excluded_, "Taichi: Account is already the value of 'excluded_'");
        _isExcludedFromFees[acc_] = excluded_;
        emit ExcludeFromFees(acc_, excluded_);
    }
    ...
    function excludeMultipleAccountsFromFees( address[] calldata accounts_, bool excluded_ ) public onlyOwner {
        ...
        emit ExcludeMultipleAccountsFromFees(accounts_, excluded_);
    }

    function setFullTrading(bool open_) external onlyOwner {
        fullTrading = open_;
    }

    function setMarketingWallet(address payable wallet_) external onlyOwner {
        marketingWalletAddress = wallet_;
    }

    function setBackPoolWallet(address payable wallet_) external onlyOwner {
        backPoolWalletAddress = wallet_;
    }

    function setTokenRewardsFee(uint256 value_) external onlyOwner {
        tokenRewardsFee = value_;
        totalFees = tokenRewardsFee.add(backPoolFee).add(marketingFee);
        require(totalFees <= 3000, "Total fee is over 3%");
    }

    function setBackPoolFee(uint256 value_) external onlyOwner {
        backPoolFee = value_;
        totalFees = tokenRewardsFee.add(backPoolFee).add(marketingFee);
        require(totalFees <= 3000, "Total fee is over 3%");
    }

    function setMarketingFee(uint256 value_) external onlyOwner {
        marketingFee = value_;
        totalFees = tokenRewardsFee.add(backPoolFee).add(marketingFee);
        require(totalFees <= 3000, "Total fee is over 3%");
    }
    ...
    function setAutomatedMarketMakerPair(address pair_, bool value_) public onlyOwner {
        require( pair_ != uniswapV2Pair, "Taichi: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs" );
        _setAutomatedMarketMakerPair(pair_, value_);
    }

    function _setAutomatedMarketMakerPair(address pair_, bool value_) private {
        require( automatedMarketMakerPairs[pair_] != value_, "Taichi: Automated market maker pair is already set to that value" );
    ...
    function updateGasForProcessing(uint256 newValue_) public onlyOwner {
        require( newValue_ >= 200000 && newValue_ <= 500000, "Taichi: gasForProcessing must be between 200,000 and 500,000" );
        require( newValue_ != gasForProcessing, "Taichi: Cannot update gasForProcessing to same value" );
        emit GasForProcessingUpdated(newValue_, gasForProcessing);
    ...
    function _transfer( address from_, address to_, uint256 amount_ ) private {
        ...
        require( from_ != address(0), "Taichi: transfer from the zero address" );
        require( to_ != address(0), "Taichi: transfer to the zero address" );
        require(amount_ > 0, "Taichi: transfer the zero amount");
        ...
        // check tx conditions
        if ( automatedMarketMakerPairs[from_] ) {  // buy
            _swaplist[to_] = block.timestamp;
            bSwap_ = true;
        }
        else if ( automatedMarketMakerPairs[to_] ) {  // sell
            bSwap_ = true;
            if ( _swaplist[from_] != 0 ) {
                if (block.timestamp == _swaplist[from_] && blockedSecs != 0) {
                    _botlist.set(from_, block.timestamp + blockedSecs);
                    revert("tf2");
                }
                require((block.timestamp - _swaplist[from_]) > minSwapSecs, "tf3");
                _swaplist[from_] = 0;
            }
            if ( IERC20(to_).totalSupply() == 0 ) {
                ...
                require(_isExcludedFromFees[from_], "tf5");
            }
        }
        ...
        bool takeFee_ = true;
        if ( _isExcludedFromFees[from_] || _isExcludedFromFees[to_] ) { // not in excluded list
            takeFee_ = false;
        }
        if ( takeFee_ && bSwap_ ) {
            uint256 fees_ = amount_.mul(totalFees).div(FEE_DENOMINATOR);      // 1%
            uint256 dividends_ = fees_.mul(tokenRewardsFee).div(totalFees);    // 0.5%
            netAmount_ = amount_.sub(fees_);
            _take( from_, backPoolWalletAddress, fees_.mul(backPoolFee).div(totalFees) );
            _take( from_, marketingWalletAddress, fees_.mul(marketingFee).div(totalFees) );
    ...
    function _tokenTransfer(address from_, address to_, uint256 amount_, uint256 netAmount_) private {
        _tOwned[from_] = _tOwned[from_].sub(amount_, "Taichi: transfer amount exceeds balance");
    ...
    function transferFrom(address sender, address recipient, uint256 amount_) external override returns (bool) {
        ...
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount_, "Taichi: transfer amount exceeds allowance"));
        return true;
    }
    ...
    function decreaseAllowance(address spender_, uint256 subtractedValue_) external returns (bool) {
        _approve(_msgSender(), spender_, _allowances[_msgSender()][spender_].sub(subtractedValue_, "Taichi: decreased allowance below zero"));
        return true;
    }
    ...
    function burnFrom(address from_, uint amount_) external {
        _approve(from_, _msgSender(), _allowances[from_][_msgSender()].sub(amount_, "Taichi: burn amount exceeds balance"));
        _burn(from_, amount_);
    }

    function _approve(address owner_, address spender_, uint256 amount_) private {
        require(owner_ != address(0), "Taichi: approve owner_ the zero address");
        require(spender_ != address(0), "Taichi: approve spender_ the zero address");
