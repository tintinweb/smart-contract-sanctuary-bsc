...
contract SwiftFinance is IERC20, Context, Ownable {
    ...
    address private Swift__Marketing_Wallet = 0x26dB6638C3bb5940F69675D8067eaDa59E83D825;
    address private Swift__Team_Wallet = 0x1E58F31Fbc7Dc1E8356A24BA60FB2b63a20f91EC;
    
    string private constant _symbol = "STF";
    string private constant _name = "Swift Finance";

    uint256 public Swift__max_BuyLimit = 1000000 * 10**9;
    uint256 public Swift__max_SellLimit = 500000 * 10**9;

    uint256 public Swift__max_WalletLimit = 1000000  * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[Swift__Marketing_Wallet] = true;
        _isExcludedFromFee[Swift__Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[Swift__Marketing_Wallet] = true;
        allowedTransfer[Swift__Team_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= Swift__max_BuyLimit, "You are exceeding Swift__max_BuyLimit");
            require(balanceOf(to) + amount <= Swift__max_WalletLimit, "You are exceeding Swift__max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= Swift__max_SellLimit, "You are exceeding Swift__max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= Swift__max_WalletLimit, "You are exceeding Swift__max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(Swift__Marketing_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(Swift__Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function Swift__Update_MarketingWallet(address newWallet) external onlyOwner{
        Swift__Marketing_Wallet = newWallet;
    }
    
    function Swift__Update_TeamWallet(address newWallet) external onlyOwner{
        Swift__Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        Swift__max_BuyLimit = maxBuy * 10**decimals();
        Swift__max_SellLimit = maxSell * 10**decimals();
    }
    
    function Swift__Max_Walletlimit(uint256 amount) external onlyOwner{
        Swift__max_WalletLimit = amount * 10**decimals();
    }
