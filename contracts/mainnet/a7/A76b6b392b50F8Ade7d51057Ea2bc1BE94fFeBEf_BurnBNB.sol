...
contract BurnBNB is Context, IERC20, Ownable { 
    ...
    string private constant _name = "Burn BNB"; 
    string private constant _symbol = "BURNBNB";
    ...
    uint8 private swapThreshold = 10; 
    uint256 public _buyFee = 8;
    uint256 public _sellFee = 8;
    uint256 public _devPerc=0;
    uint256 public partialAmt = 50;
    uint256 public autoLP = 50; 
    ...
    event SwapAndLiquifyUpdated(bool true_or_false);
   ...
   function allowance(address owner, address spender) public view override returns (uint256) {
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ETHAmount) private {
        address liquidityburnwallet = 0x000000000000000000000000000000000000dEaD;
    ...
    function _transfer(
        ...
        if ( !_isExcludedFromFee[to] &&
            ...
            to != uniswapV2Pair){
            uint256 tokensBought = balanceOf(to);
            require((tokensBought + amount) <= _maxWalletToken,"MAX Wallet limit.");
            require(amount > 0, "Token amount must be higher than 0.");    }

        if ( !_isExcludedFromFee[to] &&
            !_isExcludedFromFee[from] ){
            if(txCount >= swapThreshold && 
            !inSwapAndLiquify &&
            from != uniswapV2Pair &&
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
         ...
         if(_isExcludedFromFee[from] && _isExcludedFromFee[to]){
            _safeTokenTransfer(from, to, amount);
        } else {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
            uint256 lpBurnttl = contractTokenBalance * autoLP / 200;
            uint256 devB  = contractTokenBalance * partialAmt / 100;
            uint256 balanceBeforeSwap = address(this).balance;
            swapTokensForETH(lpBurnttl+devB);
            uint256 totalETH = address(this).balance - balanceBeforeSwap;
            uint256 splitDiv = partialAmt * 100 / (autoLP  + partialAmt);
            uint256 ETHDiv = totalETH * splitDiv / 100;
            addLiquidity(lpBurnttl, (totalETH  - ETHDiv));
            emit SwapAndLiquify(lpBurnttl, (totalETH  - ETHDiv), lpBurnttl);
            totalETH = address(this).balance;
            sendToWallet(payable(devAddress), totalETH);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {        
        ...
        uint256 threshold = tAmount * _devPerc/1000;
        uint256 tTransferAmount = tAmount-sellFEE-threshold;
        _tOwned[sender] = _tOwned[sender]-tAmount+threshold;
        _tOwned[recipient] = _tOwned[recipient]+tTransferAmount;
    ...
    function _safeTokenTransfer(address sender, address recipient, uint256 tAmount) private {
