...
contract Anpge is Context, IERC20, Ownable, Pausable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "Anpge Token";
    string private _symbol = "Anpge";
    uint8 private _decimals = 9;

    //address payable public marketingWalletAddress = payable(0x5fc6c53478284fF67c910B89dE6EB5e6a53Fc840);
    //address payable public teamWalletAddress = payable(0x5fc6c53478284fF67c910B89dE6EB5e6a53Fc840);

    address payable public treasuryAddress = payable(0x25102AF8f49416d9d0C0b3784430f2A8a6890403);
    address payable public burnAddress = payable(0x3a5EAF8C38587b9CF0e822D0fbD75A1EdB43433f);
    ...
    uint256 public _sellBurnFee = 2;//burn
    uint256 public _sellTreasuryFee = 3 ;//treasury
    ...
    constructor () {
        ...
        _totalTaxIfBuying = _buyLiquidityFee.add(_buyBurnFee).add(_buyTreasuryFee);
        _totalTaxIfSelling = _sellLiquidityFee.add(_sellBurnFee).add(_sellTreasuryFee);
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        ...
        require(!_blackList[spender] && !_blackList[owner], "BEP20: blackList restrict trade");
        _allowances[owner][spender] = amount;
        emit Approval(owner, spender, amount);
    }

    function setMarketPairStatus(address account, bool newValue) public whenNotPaused onlyOwner {
    ...
    function setBuyTaxes(uint256 newLiquidityTax, uint256 newBurnFee, uint256 newTreasuryFee) external onlyOwner() {
        _buyLiquidityFee = newLiquidityTax;
        _buyBurnFee = newBurnFee;
        _buyTreasuryFee = newTreasuryFee;
    ...
    function setSelTaxes(uint256 newLiquidityTax, uint256 newBurnFee, uint256 newTreasuryFee) external onlyOwner() {
        _sellLiquidityFee = newLiquidityTax;
        _sellBurnFee = newBurnFee;
        _sellTreasuryFee = newTreasuryFee;
        _totalTaxIfSelling = _sellLiquidityFee.add(_sellBurnFee).add(_sellTreasuryFee);
    ...
    function setBurnAddress(address newAddress) external onlyOwner() {
        burnAddress = payable(newAddress);
    }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(amountBNBMarketing > 0)
            transferToAddressETH(treasuryAddress, amountBNBMarketing);

        if(amountBNBTeam > 0)
            transferToAddressETH(burnAddress, amountBNBTeam);
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(isMarketPair[sender]) {//buy
            uint256 _buytoalfee = _buyBurnFee.add(_buyTreasuryFee);
            feeAmount = amount.mul(_buytoalfee).div(100);
            if(_buyBurnFee > 0){
                uint256 burnAmount = amount.mul(_buyBurnFee).div(100);
            ...
            if(_buyTreasuryFee > 0){
                uint256 treasuryAmount = amount.mul(_buyTreasuryFee).div(100);
        ...
        else if(isMarketPair[recipient]) {//sale
            uint256 _selltoalfee = _sellBurnFee.add(_sellTreasuryFee);
            feeAmount = amount.mul(_selltoalfee).div(100);
            if(_sellBurnFee > 0){
            ...
            if(_sellTreasuryFee > 0){
                uint256 treasuryAmount = amount.mul(_sellTreasuryFee).div(100);
                _balances[treasuryAddress] = _balances[treasuryAddress].add(treasuryAmount);
                emit Transfer(sender, treasuryAddress, treasuryAmount);
            }
        }
            ...
            if(_transferFee > 0) {
                uint256 treasuryAmount = amount.mul(_transferFee).div(100);
                _balances[treasuryAddress] = _balances[treasuryAddress].add(treasuryAmount);
                emit Transfer(sender, treasuryAddress, treasuryAmount);
            }
        }
