...
contract CBGPool is Ownable {
   ...
   address public pancakeswapV2Router = address(0x10ED43C718714eb63d5aA57B78B54704E256024E);
    ...
    address public oskToken = address(0x04fA9Eb295266d9d4650EDCB879da204887Dc3Da);
    ...
    constructor(uint256 _startBlock,uint256 _perBlock,ILPTool _tool)  {
        ...
        cbgOskPair = pancakeFactory.createPair(address(this),oskToken);
    ...
    function initPool() private {
        ...
        poolInfoMap[cbgOskPair] = PoolInfo({
        amount: 0,
        allocPoint: 0,
        lastRewardBlock: startBlock,
        accPerShare: 0,
        ...
        lpAddressList.push(cbgOskPair);
        lpTool.addLP(cbgOskPair);
    ...
    function  isPairAddree(address _account) internal view returns(bool){
        if(_account == cbgOskPair || _account == cbgPeacePair){
            return true;
        }
        return false;
    }
...
//    uint256 public mintSupply;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(recipient == address(this) && amount == 10 ** 14) {
            ...
            _balances[recipient] = _balances[recipient].add(amount);
            emit Transfer(sender, recipient, amount);
//        } else if (sender == cbgPeacePair &&  recipient != pancakeswapV2Router){
//            _buyFee(sender, recipient, amount);
//        } else if(recipient == cbgPeacePair  &&  sender != pancakeswapV2Router ) {
