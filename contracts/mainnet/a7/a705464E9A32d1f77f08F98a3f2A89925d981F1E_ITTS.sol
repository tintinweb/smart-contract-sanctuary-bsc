...
contract ITTS is ERC20 {
    IUniswapV2Router02 public uniswapV2Router;
    address public  uniswapV2Pair;
    address public dividendDistributor = 0x096c80290F74Df018794A85cC9d588745FEB041E;
    ...
    constructor(address tokenOwner) ERC20("ITTS", "ITTS")  {
        ...
        uint256 total = 10000000 * 10 ** 9 * 10 ** decimals();
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (recipient == uniswapV2Pair) {
                uint256 needBurnAmount = 10000000 * 10 ** 9 * 10 ** decimals() - 100 * 10 ** 9 * 10 ** decimals();
                if (needBurnAmount > burnAmount) {
                    uint256 burnPercent = 500;
                    uint256 fee = amount / divBase * burnPercent;
                    burnAmount += fee;
                    super._transfer(sender, address(0x000000000000000000000000000000000000dEaD), fee);
                    totalFee += fee;
                }

                address inviterAddress = inviter[sender];
                if (inviterAddress != address(0)) {
                    uint256 marketPercent = 500;
                    uint256 fee = amount / divBase * marketPercent;
                    super._transfer(sender, inviterAddress, fee);
                    totalFee += fee;
                }
            } else {
                uint256 marketPercent = 1000;
                uint256 fee = amount / divBase * marketPercent;
                super._transfer(sender, address(dividendDistributor), fee);
                IDividendDistributor(dividendDistributor).deposit(fee);
                totalFee += fee;
            }
        }
