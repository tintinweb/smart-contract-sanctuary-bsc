...
*/
//Rickety Rick Token, First Project. 10% max transaction, BUSD rewards, 1 BUSD minimum 1,000,000,000 supply 3% reflection 3%liquidity 2%marketing fees
//https://t.me/RickTokenGroup  come by hang out and maybe help me spread the word
...
contract RICKETYRICK is IBEP20, RRTAuth {
    using SafeMath for uint256;
    ...
    string constant _name = "Rickety Rick Token";
    string constant _symbol = "RRT";
    uint8 constant _decimals = 18;

    uint256 _totalSupply =  1000000000; // 1 billion
    uint256 public _maxTxAmount = _totalSupply.div(10); // 10% - 100,000,000
    ...
    uint256 public marketing = 200;
    uint256 totalFee = 800;
    uint256 public feeDenominator = 10000;

    uint256 public BUSDthreshold = 250000; // 2500
    ...
    uint256 public swapThreshold = _totalSupply / 20; // 50,000,000
    bool public inSwap;
    ...
    constructor () RRTAuth(msg.sender) {
        ...
        IBEP20(BUSD).approve(dexRouter_, 1000000000);
    ...
    function shouldSwapBUSD() internal view returns (bool) {
    ...
    function swapBUSDManual() external authorizedFor(Permission.AdjustContractVariables) {
        uint256 sendBUSDAmount = IBEP20(BUSD).balanceOf(address(this));
        IBEP20(BUSD).transfer(address(distributor), sendBUSDAmount);
        distributor.depositBUSD(sendBUSDAmount);
    }
    ...
    function swapBack() internal swapping {
        ...
        try router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            ...
            uint256 amountBNchen = amountBNB.mul(marketing).div(totalBNBFee);

            try distributor.deposit{value: amountBNBReflection}() {} catch {}
            (bool MarketSuccess, ) = payable(marketingReceiver).call{value: amountBNchen, gas: 30000}("");
    ...
    function setBUSDthreshold(uint256 amount) external authorizedFor(Permission.AdjustContractVariables) {
        BUSDthreshold = amount;
    }
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketing, uint256 _feeDenominator) external authorizedFor(Permission.AdjustContractVariables) {
        liquidityFee = _liquidityFee;
        reflectionFee = _reflectionFee;
        marketing = _marketing;
        totalFee = _liquidityFee.add(_reflectionFee).add(_marketing);
        feeDenominator = _feeDenominator;
        require(totalFee <= 2500);
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingReceiver) external authorizedFor(Permission.AdjustContractVariables) {
