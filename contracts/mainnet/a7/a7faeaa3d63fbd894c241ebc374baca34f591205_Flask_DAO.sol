...
contract Flask_DAO is Base, Math, Adressable, AccessControl, With_DAORole, Terminatable, With_UpdaterRole,
                         With_FundContract, With_StackingContract, With_RoyaltyContract, With_CompetitionContract,
                         With_TradeManagerContract, With_UpdaterContract, With_PWRToken, I_Flask_DAO{
    ////
    ////
    ////
    //////////////// Public variables ////////////////
    uint256 public constant nbProposals = 59;
    ...
    modifier onlyGovernor(){
        require(users[msg.sender].isGovernor, "Flask_DAO: Only governor are allowed to call this function.");
        _;
    }
    ...
    function init(uint256[16] calldata data,
        ...
        require(_proposalLevel.length == nbProposals, "Flask_DAO: _proposalLevel is not correct length.");
        ...
        for(uint i=0; i<nbProposals; i++){
            require(_proposalLevel[i]<=2, "Flask_DAO: incorrect _proposalLevel.");
            arr[i] = _proposalLevel[i];
        }
        proposalLevel = arr;
    }

    function addUser(address addr, bool isGovernor, bool isJuniorTrader, bool isSeniorTrader) external onlyRole(DAO_ROLE){
        DAOLevel storage user = users[addr];
        user.isGovernor = isGovernor;
        
        if (isJuniorTrader){
            tradeManager.setJuniorTrader(addr);
        }
        
        if (isSeniorTrader){
            tradeManager.setSeniorTrader(addr);
        }
        
        updater.setUserGovernorStatus(addr, isGovernor);
    }
    ...
    function getVotes(address addr) public view returns (uint8){
        if (!users[addr].isGovernor){
            return 0;
        }

        uint256 balance = PWRToken.balanceOf(addr) + stacking.stackedPWR(addr);
    ...
    function getAllProposalLevels() external view returns(uint256[nbProposals] memory){
    ...
    function submitProposal(uint16 id, uint256 paramsUint256, address paramsAddress) external onlyGovernor onlyNotTerminated{
        require(openProposalIds.length < maxNbOpenProposals, "Flask_DAO: Max number of open proposals reached");
        require(id < nbProposals, "Flask_DAO: This is not a valid proposal id");
        require(getRemainingProposals(msg.sender) > 0, "Flask_DAO: No remaining proposals for this address");
        ...
        require(_checkProposalParameters(proposal), "Flask_DAO: Error in parameters");
    ...
    function vote(uint256 id, bool isYes) public onlyGovernor{
        ...
        require(nbVotes>0, "Flask_DAO: There is 0 vote for this address");


        require(id < proposals.length, "Flask_DAO: Proposal does not exist");
        Proposal storage proposal = proposals[id];

        require(!hasVoted[msg.sender][id], "Flask_DAO: address have already voted");

        require(block.number >= proposal.startBlock, "Flask_DAO: Proposal is not opened to vote");
        require(block.number < proposal.endBlock, "Flask_DAO: Proposal is closed to vote");
    ...
    function updateOpenProposalNeeded(uint256 i) public view returns (bool){
        ...
        require(i < l, "Flask_DAO: Open proposal ID not found.");
            ...
            if (id<16){
                ...
                if (id<24){
                    if (id<20){
                        if (id<18){
                            if (id==16){
                                tradeManager.setJuniorTrader(proposal.paramsAddress);
                            }else{
                                tradeManager.setSeniorTrader(proposal.paramsAddress);
                            }
                        }else{
                            if (id==18){
                                users[proposal.paramsAddress].isGovernor = true;
                                users[proposal.paramsAddress].isInvestor = true;
                                updater.setUserGovernorStatus(proposal.paramsAddress, true);
                            }else{ //id==19
                                users[proposal.paramsAddress].isGovernor = false;
                    ...
                    if (id<28){
                        ...
                        if (id<30){
                            if (id==28){
                                ...
                                I_Flask_Trade_Manager.DeployerData memory data = tradeManager.getTradeDeployer(deployerId);
                                
                                data.deployer.applyProposal(deployerProposalId, param, proposal.paramsAddress);
                            }
                        }else{
                            if (id==30){
                                I_Flask_Trade_Deployer_Abstract deployer = I_Flask_Trade_Deployer_Abstract(payable(proposal.paramsAddress));
            ...
            if (id<48){
                ...
                if (id<56){
                    if (id<52){
                        if (id<50){
                            if (id == 48){
                                ...
                                tradeManager.demoteTrader(proposal.paramsAddress);
                            }
                        }else{
                            if (id == 50){
                                users[proposal.paramsAddress].isInvestor = false;
                            }else{ //id==51
                                tradeManager.setAutoPromotionEnabled(true);
                            }
                        }
                    }else{
                        if (id<54){
                            if (id == 52){
                                tradeManager.setAutoPromotionEnabled(false);
                            }else{ //id==53
                                tradeManager.setNbBlockPerPointCycle(proposal.paramsUint256);
                            }
                        }else{
                            if (id == 54){
                                tradeManager.setMinPWRToStartPaperTrade(proposal.paramsUint256);
                            }else{ //id==55
                                tradeManager.setNbPointsToPromoteJuniorTrader(proposal.paramsUint256);
                            }
                        }
                    }
                }else{
                    if (id<60){
                        if (id<58){
                            if (id == 56){
                                tradeManager.setNbPointsToPromoteSeniorTrader(proposal.paramsUint256);
                            }else{ //id==57
                                tradeManager.setNbNegativePointsToDemoteJuniorTrader(proposal.paramsUint256);
                            }
                        }else{
                            if (id == 58){
                                tradeManager.setNbNegativePointsToDemoteSeniorTrader(proposal.paramsUint256);
                            }else{ //id==59
                    ...
                    if (id<36){
                        if (id<34){
                            if (id==32){
                                ...
                                if (   proposal.paramsAddress != address(0x0)
                                    || proposal.paramsUint256 < 1 ether){
                                    return false;
                                }
                            }
                        }else{
                    ...
                    if (id<44){
                        if (id<42){
                            ...
                            if (id==42){
                                ...
                                I_Flask_Trade_Manager.TradeData memory data = tradeManager.getTrade(proposal.paramsUint256);
                                if (!data.isActive){
                                    return false;
                                }
                            }
                    ...
                    if (id<52){
                        ...
                        if (id<54){
                            if (id == 52){
                                ...
                                if (   proposal.paramsAddress != address(0x0)
                                    || proposal.paramsUint256 < 201600
                                    || proposal.paramsUint256 > 2592000){
                                    return false;
                                }
                            }
                        }else{
                            if (id == 54){
                                ...
                                if (   proposal.paramsAddress != address(0x0)
                                    || proposal.paramsUint256 < 100 ether
                                    || proposal.paramsUint256 > 10000 ether){
                                    return false;
                                }
                            }
                        }
