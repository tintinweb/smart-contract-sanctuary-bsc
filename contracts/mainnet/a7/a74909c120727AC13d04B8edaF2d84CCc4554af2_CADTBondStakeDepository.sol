...
contract CADTBondStakeDepository is Ownable {
    ...
    address public immutable CADT; // intermediate reward token from treasury
    address public immutable sCADT; // token given as payment for bond
    ...
    struct Bond {
        uint gonsPayout; // sCADT gons remaining to be paid
        uint cadtPayout; //cadt amount at the moment of bond
    ...
    function deposit( 
        ...
        IERC20( CADT ).approve( stakingManager, payout );

        IStakingManager(stakingManager).stake( payout, address(this) );
        /* ---------------------------------------------------------- */
        
        uint stakeGons=ISCADT(sCADT).gonsForBalance(payout);

        // depositor info is stored
        _bondInfo[ _depositor ] = Bond({ 
            gonsPayout: _bondInfo[ _depositor ].gonsPayout.add( stakeGons ),
            cadtPayout: _bondInfo[ _depositor ].cadtPayout.add( payout ),
    ...
    function redeem( address _recipient, bool _stake) external returns ( uint ) {        
        ...
        uint _amount = ISCADT(sCADT).balanceForGons(info.gonsPayout);
        emit BondRedeemed( _recipient, _amount, 0 ); // emit bond data
        IERC20( sCADT ).transfer( _recipient, _amount ); // pay user everything due
        return _amount;
    }
    ...
    /**
     *  @notice return bond info with latest sCADT balance calculated from gons
     *  @param _depositor address
     *  @return payout uint
     *  @return vesting uint
    ...
    function bondInfo(address _depositor) public view returns ( uint payout,uint vesting,uint lastBlock,uint pricePaid ) {
        ...
        payout=ISCADT(sCADT).balanceForGons(info.gonsPayout);
    ...
    function pendingPayoutFor( address _depositor ) external view returns ( uint pendingPayout_ ) {
        ...
        uint payout = ISCADT(sCADT).balanceForGons(_bondInfo[ _depositor ].gonsPayout);
