...
contract KJDS is ERC20 {
	...
	address public _fundAddress = address(0x263864B2BA40A3F6c068c722fbE51Df9c951eaEC);
    ...
    constructor(address tokenOwner) ERC20("KJDS", "KJDS") {
	...
	uint256 public limitAmount = 10**20;
	function changeLimitAmount(uint256 _limitAmount) public onlyOwner {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx) {
			...
			if(from == uniswapV2Pair){
				require(amount <= limitAmount,"max amount");
				splitOtherToken();
            }else if(to == uniswapV2Pair){
				splitOtherToken();
                require(amount<=balanceOf(from).div(100).mul(90));
            }else{takeFee = false;
            }
        }
