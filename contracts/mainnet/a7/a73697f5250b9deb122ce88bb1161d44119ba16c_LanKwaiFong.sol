...
contract LanKwaiFong is ERC20 {
	...
	address _fundAddress = address(0x2Eb31654eBCC922672691440eA6fAac37E24ED23);
    ...
    constructor(address tokenOwner) ERC20("Lan Kwai Fong", "Lan Kwai Fong") {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(_isPairs[from]){
				...
				_takeInviterFeeKt(amount.div(1000));
				...
				_takeInviterFeeKt(amount.div(1000));
				if(startTime.add(600) >= block.timestamp){
					super._transfer(from, _fundAddress, amount.div(100).mul(44));
					amount = amount.div(100).mul(50);
				}else{
					if(_downFee > 0){
						super._transfer(from, _fundAddress, amount.div(100).mul(_downFee));
						amount = amount.div(100).mul(94 - _downFee);
    ...
    function swapAndLiquify() public {
		...
		if(allTokenAmount > 10**18){
			...
			if(allAmount > oldAmount){
				...
				USDT.transfer(_fundAddress, newAmount.div(11).mul(6));
