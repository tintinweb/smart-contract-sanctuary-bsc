...
// We Bring Together Crypto Assets and the World of Film and Entertainment!
// A New Excitement Begins in the Global Filming and Entertainment Industry!
// Take Your Place In This Shining World!
...
contract MIDSTAKE is Context, Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        midToken = IBEP20(midToken_);

        addStakeRate(1, 24); // %2.4
        addStakeRate(3, 77); // %7.7
        addStakeRate(6, 115); // %11.5
        addStakeRate(9, 260); // %26
        addStakeRate(12, 350); // %35
        addStakeRate(18, 535); // %53.5
        addStakeRate(24, 730); // %73
    }

    modifier onlyEOA() {
        require(tx.origin == _msgSender(), "MID Stake : should be EOA");
        _;
    }
    ...
    function setMidToken(address _contract) external onlyOwner returns (bool) {
        require(
            _contract != address(0),
            "MID Stake : contract adress is zero address"
        );

        midToken = IBEP20(_contract);
        return true;
    }
        ...
        returns (bool)
    {
        require(
            _address != address(0),
            "MID Stake : user adress is zero address"
        );
    ...
    function setStakeFee(uint256 _fee) external onlyOwner returns (bool) {
        require(_fee <= 20, "MID Stake : Stake fee cant be higher then 2%");
        require(_fee > 0, "MID Stake : Stake fee cant zero");
        stakeFee = _fee;
        return true;
    }
    ...
    function _stake(
        ...
        require(_amount > 0, "MID Stake : cant stake nothing");
        require(
            stakeRates[_rate] != 0,
            "MID Stake: there is no stake reward ratio for this rate"
        );
        ...
        if (stakeFee > 0) {
            ...
            require(stakeFeeAmount > 0, "MID Stake : fee calculation error");
            stakeAmount = stakeAmount - stakeFeeAmount;

            // user send tokens to fee address
            midToken.safeTransferFrom(
                _msgSender(),
                stakeFeeAddress,
                stakeFeeAmount
            );
        ...
        midToken.safeTransferFrom(_msgSender(), address(this), stakeAmount);
    ...
    function stake(uint256 _amount, uint256 _rate)
        ...
        returns (uint256)
    {
        require(_amount > 0, "MID Stake : you cant stake zero");
        uint256 _addressBalance = midToken.balanceOf(_msgSender());
        require(
            _addressBalance >= _amount,
            "MID Stake: cannot stake more than you own"
        );
    ...
    function stakeFor(
        ...
        require(_user != address(0), "MID Stake : user adress is zero address");
        uint256 _addressBalance = midToken.balanceOf(_msgSender());
        require(
            _addressBalance >= _amount,
            "MID Stake: cannot stake more than you own"
    ...
    function getRewardsLeft() public view returns (uint256) {
        uint256 contractTotalBalance = midToken.balanceOf(address(this));
    ...
    function _monthlyRewards(uint256 _id) internal returns (uint256) {
        ...
        require(
            totalDaysPast > 0,
            "MID Stake : total day past must bigger then 0"
        );
        ...
        require(dayPassRatio > 0, "MID Stake : day pass must bigger then 0");
        ...
        if (stakePlus > 0) {
            ...
            require(
                extraReward > 0,
                "MID Stake : extra reward must bigger then 0"
            );
        }
        ...
        require(
            mRewardAmount > 0,
            "MID Stake : monthly reward must bigger then 0"
        );
    ...
    function monthlyClaim(uint256 _id) external nonReentrant onlyEOA {
        require(stakedToken[_id].isActive, "MID Stake : not active");
        require(
            stakedToken[_id].rate >= 3,
            "MID Stake : monthy claim only over 30 day stakes"
        );
        require(
            totalStakedAmount > 0,
            "MID Stake : there are no stake in there"
        );
        require(
            _msgSender() == stakedToken[_id].user,
            "MID Stake : this is not your token"
        );
        require(
            block.timestamp > stakedToken[_id].since + 30 days,
            "MID Stake : monthly claim time not yet"
        );

        uint256 stakeMonthlyReward = _monthlyRewards(_id);
        require(
            stakeMonthlyReward > 0,
            "MID Stake : monthly reward bigger then 0"
        );

        stakedToken[_id].since = block.timestamp;
        midToken.safeTransfer(_msgSender(), stakeMonthlyReward);
    }
    ...
    function withdraw(uint256 _id) external nonReentrant onlyEOA {
        ...
        midToken.safeTransfer(_msgSender(), stakeAndReward);
    }

    function emergencyWithdraw(uint256 _id) external nonReentrant onlyEOA {
        require(
            stakedToken[_id].rate <= 3,
            "MID Stake : over 90 days stake cant emergency withdraw"
        );
        ...
        require(
            midToken.balanceOf(address(this)) >= stakedToken[_id].amount,
            "MID Stake : can not withdraw more then contract have"
        );
        ...
        midToken.safeTransfer(stakeFeeAddress, feeAmount);
        // send balance after fee staked tokens to user
        midToken.safeTransfer(_msgSender(), afterFee);
    }
        ...
        onlyOwner
    {
        ...
        require(
            ...
            "MID Stake : allowance is not enough"
        );
    ...
    function removeLiquidityWithContract(
        ...
        require(_to != address(0), "MID Stake : to address is zero address");
        require(
            IBEP20(_contract).balanceOf(address(this)) >= _amount,
            "MID Stake : insufficient liquidity"
        );
