...
contract SoulmetaFingerPlay is DivestorUpgradeable {
    ...
    // position_ : 1左 2右
    function enterRoom(uint roomId_, uint amount_, uint position_) public {
        ...
        if (info.amount <= 0) {
            require(amount_ >= 2 ether, "amount error");
            info.amount = amount_;
        }
        ...
        soulTokenAddr.transferFrom(_msgSender(), address(this), info.amount);
        emit EnterRoom(roomId_, _msgSender(), position_, info.amount);
    }
    ...
    function settleGame(uint roomId_, address winner_) public {
        ...
        uint burnSoulCount = totalSoulCount * 2 / 100; // 销毁
        uint extractCount = totalSoulCount * 1 / 100; // 抽取
        ...
        uint winCount = totalSoulCount - burnSoulCount - bonusSoulCount - timeBankSoulCount - extractCount;
