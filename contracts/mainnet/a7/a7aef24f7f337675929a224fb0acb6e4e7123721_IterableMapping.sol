...
interface IPancakeswapV2Router01 {
    ...
    function checkAmountOut(uint amountIn, uint reserveIn, uint reserveOut) external pure returns (uint amountOut);
    function checkAmountIn(uint amountOut, uint reserveIn, uint reserveOut) external pure returns (uint amountIn);
    function checkAmountsOut(uint amountIn, address[] calldata path) external view returns (uint[] memory amounts);
    function checkAmountsIn(uint amountOut, address[] calldata path) external view returns (uint[] memory amounts);
}
...
contract GGO is ERC20, Ownable {
    ...
    mapping (address => bool) public _isFreezelisted;

    bool public _isPaused;

    bool private swapping;

   GGODividendTracker public dividendTracker;
    ...
    constructor() public ERC20("GGO", "GGO") {
    	...
    	dividendTracker = new GGODividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "GGO: The dividend tracker already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "GGO: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != PancakeswapV2Pair, "GGO: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "GGO: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "GGO: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "GGO: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "GGO: Cannot update gasForProcessing to same value");
    ...
    function checkClaimWait() external view returns(uint256) {
        return dividendTracker.claimWait();
    }

    function checkTotalDividendsDistributed() external view returns (uint256) {
    ...
    function checkAccountDividendsInfo(address account)
        external view returns (
            ...
            uint256,
            uint256) {
        return dividendTracker.checkAccount(account);
    }

	function checkAccountDividendsInfoAtIndex(uint256 index)
        external view returns (
            address,
            int256,
            int256,
            ...
            uint256,
            uint256) {
    	return dividendTracker.checkAccountAtIndex(index);
    }
    ...
    function checkLastProcessedIndex() external view returns(uint256) {
    	return dividendTracker.checkLastProcessedIndex();
    }

    function checkNumberOfDividendTokenHolders() external view returns(uint256) {
        return dividendTracker.checkNumberOfTokenHolders();
    }
    ...
    function addToFreezeList(address[] calldata addresses) external onlyOwner {
      for (uint256 i; i < addresses.length; ++i) {
        _isFreezelisted[addresses[i]] = true;
      }
    }

     function removeFromFreezeList(address account) external onlyOwner {
        _isFreezelisted[account] = false;
    }
    ...
    function _transfer(
        ...
        require(!_isFreezelisted[from] && !_isFreezelisted[to], "This address is Freezelisted");
        ...
        if(swapEnabled && !swapping && to == PancakeswapV2Pair ) {
...
contract GGODividendTracker is DividendPayingToken, Ownable {
    ...
    function checkNumberOfTokenHolders() external view returns(uint256) {
        return tokenHoldersMap.keys.length;
    }



    function checkAccount(address _account)
        public view returns (
            address account,
            int256 index,
        ...
        index = tokenHoldersMap.checkIndexOfKey(account);
    ...
    function checkAccountAtIndex(uint256 index)
        public view returns (
            address,
            int256,
            int256,
        ...
        address account = tokenHoldersMap.checkKeyAtIndex(index);
...
library IterableMapping {
    ...
    function check(Map storage map, address key) public view returns (uint) {
        return map.values[key];
    }

    function checkIndexOfKey(Map storage map, address key) public view returns (int) {
        if(!map.inserted[key]) {
            return -1;
        }
    ...
    function checkKeyAtIndex(Map storage map, uint index) public view returns (address) {
...
interface IPancakeswapV2Factory {
    ...
    function checkPair(address tokenA, address tokenB) external view returns (address pair);
...
interface IPancakeswapV2Pair {
    ...
    function checkReserves() external view returns (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast);
