...
contract TaurenNFT is ERC721URIStorage, Ownable {
    ...
    function getMyNFT(address user) public view returns(MyNFT[] memory myNFTs) {
        ...
        for(uint256 i = 0; i < myTokenIds.length; i++){
            ...
            uint256 price = typeToNFTPrice[types].div(10 ** 18);
    ...
    function _transfer(
        ...
        require(tokenIdToStatus[tokenId] == 0, "This NFT is unavailable");
...
contract TaurenToken2 is Context, IBEP20, Ownable {
    using SafeMath for uint256;
    ...
    string private _name = "Tauren Token";//RescueTauren
    string private _symbol = "DH2";
    ...
    uint256 private numTokensToSwap =  3 * 10**2 * 10**18;
    ...
    address public BUSD = 0x55d398326f99059fF775485246999027B3197955;//     0xe52225889DC34fA282Ca9589f2b74aDA27a13b96


    address public castingPoolAddr = 0xEd703D611df2fef3D0c626B1f688edFeeA8CfcF7;

    address public operationAddr = 0x6f493e9aC78012235f35d40d36670a29f0Fcf7a6;

    address public genesisPoolAddr = 0x5BF40Ad5BECC1370D37bd604eE9500856b450723;
    ...
    uint256 public hammerBasePrice = 1 * 10 ** 18;

    address public bullNFTHolder = 0x67dFdd5EA245970020761f74fB6Ad6c597Eb3880;

    address[] public genesisUser;

    uint256 public burnRateOfRescue = 1;
    ...
    event SwapAndCharged(uint256 token, uint256 liquidAmount, uint256 bnbPool);
    ...
    event PledgeNFT(address account, uint256 tokenId, uint256 NFTType, uint256 types , uint256 BUSDAmount, uint256 tokenAmount);
    event QuitPledgeNFT(address account, uint256 tokenId, uint256 NFTType, uint256 types , uint256 tokenAmount, uint256 profit);
    event WithdrawPledgeProfit(address account, uint256 tokenId, uint256 types , uint256 profit);
    ...
    constructor () {
        ...
        TRN = TaurenNFT(0xd8A9e3233457eAED7783d8061B209386089a939d);//main :      test:  0xccb681566b9A8252b01F3B29f13Cf51F64B7F806   0xe1a5748e673F7A6F3F15fe9be1a7fd4A9bc1BAcE

        pledgeTypeToRate[10] = 100;
        pledgeTypeToRate[20] = 300;
        pledgeTypeToRate[30] = 500;
        pledgeTypeToRate[40] = 500;
    }
    ...
    function _transfer(
        ...
        bool takeLiquidFee = false;

        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            _tokenTransfer(from,to,amount,takeLiquidFee);
            return;
        }
        ...
        if(
            ...
            uint256 tokenAmount = BUSDEqualToToken(1 * 10 ** 12);

            takeLiquidFee = true;

            if(amount >= tokenAmount){
                if(from == pancakeswapV2Pair && !isBullUser(bullPeriodIndex,to)) bullUsers[bullPeriodIndex].push(to);
                if(to == pancakeswapV2Pair && !isBullUser(bullPeriodIndex,from)) bullUsers[bullPeriodIndex].push(from);
    ...
    function swapAndCharge(uint256 tokenBalance) public lockTheSwap {
        uint256 initialBalance = ERC20(BUSD).balanceOf(address(0x45CbCBf16E1251d2019bEdb940f70Cb6F12068b0));

        swapTokensForEth(tokenBalance);

        uint256 newBalance = ERC20(BUSD).balanceOf(address(0x45CbCBf16E1251d2019bEdb940f70Cb6F12068b0)).sub(initialBalance);

        uint256 totalFee = _castingPoolFee + _operationFee + _genesisPoolFee;

        uint256 castingPoolFee = newBalance.mul(_castingPoolFee).div(totalFee);

        uint256 operationFee = newBalance.mul(_operationFee).div(totalFee);

        uint256 genesisPoolFee = newBalance.sub(castingPoolFee).sub(operationFee);

        ERC20(BUSD).transferFrom(address(0x45CbCBf16E1251d2019bEdb940f70Cb6F12068b0), castingPoolAddr, castingPoolFee);
        ERC20(BUSD).transferFrom(address(0x45CbCBf16E1251d2019bEdb940f70Cb6F12068b0), operationAddr, operationFee);
        ERC20(BUSD).transferFrom(address(0x45CbCBf16E1251d2019bEdb940f70Cb6F12068b0), genesisPoolAddr, genesisPoolFee);

        emit SwapAndCharged(tokenBalance, ERC20(BUSD).balanceOf(address(0x45CbCBf16E1251d2019bEdb940f70Cb6F12068b0)), newBalance);
    }
    ...
    function swapTokensForEth(uint256 tokenAmount) public {
        ...
        pancakeswapV2Router.swapExactTokensForTokens(//swapExactTokensForTokens
            tokenAmount,
            0, // accept any amount of ETH
            path,
            address(0x45CbCBf16E1251d2019bEdb940f70Cb6F12068b0),
            block.timestamp
        );
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeLiquidFee) private {
        ...
        if(takeLiquidFee){
            ...
            _balances[address(this)] = _balances[address(this)].add(amount.sub(tTransferAmount).sub(burnFee));
            emit Transfer(sender, recipient, tTransferAmount);
            emit Transfer(sender, deadWallet, burnFee);
            emit Transfer(sender, address(this), amount.sub(tTransferAmount).sub(burnFee));
    ...
    function isBullUser(uint256 _bullPeriodIndex,address account) public view returns(bool) {

        for(uint256 i = 0; i < bullUsers[_bullPeriodIndex].length; i++){
            if(bullUsers[_bullPeriodIndex][i] == account) return true;
        }

        return false;
    }

    function setPancakeswapV2Pair(address pair) external onlyOwner {
    ...
    function setBurnRateOfRescue(uint256 rate) public onlyOwner {
        burnRateOfRescue = rate;
    }
    ...
    function genesisPrivatePlacement() public payable returns(uint256[] memory tokenIds) {
        ...
        if(genesisUserIndex[msg.sender] == 0){
    ...
    function rescueBullNFT(uint256 _bullPeriodIndex) public {
        ...
        require(hammerFixTime[_bullPeriodIndex] + 1 minutes < block.timestamp, "Hammer not yet repaired");
        ...
        uint256 amount = BUSDEqualToToken(price).mul(burnRateOfRescue).div(1000);
        ...
        hammerFixTime[_bullPeriodIndex] = block.timestamp;

        emit RescueBullNFT(msg.sender, tokenId, _bullPeriodIndex, length);
    }
    ...
    function pledgeNFT(uint256 types, uint256 tokenId) public {
        ...
        require(types == 1 || types == 2, "Pledge type is wrong");

        TRN.setNFTStatus(tokenId, 2);

        uint256 NFTType = TRN.tokenIdToTypes(tokenId);

        uint256 BUSDAmount = TRN.typeToNFTPrice(NFTType);
        ...
        if(types == 2){
            ...
            tokenAmount = BUSDEqualToToken(BUSDAmount);

            _transfer(msg.sender, castingPoolAddr, tokenAmount);
        }

        tokenIdToPledges[tokenId] = Pledge(tokenId, types, BUSDAmount, tokenAmount, block.timestamp, 0);

        emit PledgeNFT(msg.sender, tokenId, NFTType, types, BUSDAmount, tokenAmount);
    ...
    function quitPledgeNFT(uint256 tokenId) public {
        ...
        require(tokenIdToPledges[tokenId].joinTime > 0 && TRN.tokenIdToStatus(tokenId) == 2, "Your card is not pledged");

        TRN.setNFTStatus(tokenId, 0);

        uint256 NFTType = TRN.tokenIdToTypes(tokenId);

        uint256 tokenAmount = tokenIdToPledges[tokenId].KOGAmount;

        uint256 profit = getPledgeProfit(tokenId);

        if(tokenAmount + profit > 0){
            _transfer(castingPoolAddr, msg.sender, tokenAmount + profit);
        }
    ...
    function withdrawPledgeProfit(uint256 tokenId) public {
        ...
        if(profit > 0){
            _transfer(castingPoolAddr, msg.sender, profit);
        }

        tokenIdToPledges[tokenId].lastWithdrawTime = block.timestamp;

        emit WithdrawPledgeProfit(msg.sender, tokenId, tokenIdToPledges[tokenId].types, profit);
    }
    ...
    function getPledgeKOGAmount(uint256 tokenId) public view returns(uint256){
        ...
        uint256 KOGAmount = BUSDEqualToToken(TRN.typeToNFTPrice(NFTTypes)).div(10 ** 18);
    ...
    function getPledgeProfit(uint256 tokenId) public view returns(uint256){
        ...
        uint256 BUSDAmount = tokenIdToPledges[tokenId].BUSDAmount;

        uint256 tokenAmount = tokenIdToPledges[tokenId].KOGAmount;

        tokenAmount = tokenAmount + BUSDEqualToToken(BUSDAmount);

        uint256 pledgeDays;

        if(tokenIdToPledges[tokenId].lastWithdrawTime > tokenIdToPledges[tokenId].joinTime){
            pledgeDays = (block.timestamp - tokenIdToPledges[tokenId].lastWithdrawTime).div(24 hours);
        }else{
            pledgeDays = (block.timestamp - tokenIdToPledges[tokenId].joinTime).div(24 hours);
        }   
    ...
    function checkCanBuy(uint256 _bullPeriodIndex, address user) public view returns(bool) {

        address preOwner = getHammerCurrentOwner(_bullPeriodIndex);

        if(user == preOwner) return false;
    ...
    function getNFTDetail(uint256 tokenId) public view returns(uint256 NFTType,uint256 periodIndex,uint256 price,
    uint256 pledgeType,uint256 startTime,uint256 BUSDAmount,uint256 KOGAmount,uint256 profit) {

        NFTType = TRN.tokenIdToTypes(tokenId);

        periodIndex = TRN.tokenIdToPeriod(tokenId);

        price = TRN.typeToNFTPrice(NFTType).div(10 ** 18);

        pledgeType = tokenIdToPledges[tokenId].types;

        if(pledgeType == 10 ||  pledgeType == 20) pledgeType = 1;

        if(pledgeType == 30 ||  pledgeType == 40) pledgeType = 2;

        BUSDAmount = tokenIdToPledges[tokenId].BUSDAmount.div(10 ** 18);

        KOGAmount = tokenIdToPledges[tokenId].KOGAmount.div(10 ** 18);

        startTime = tokenIdToPledges[tokenId].joinTime;

        if(startTime > 0){
            profit = getPledgeProfit(tokenId);
        }
    }
    ...
    function getBullNFTDetail(uint256 _bullPeriodIndex) public view returns(uint256 tokenId, address currentOwner, uint256 price, uint256 sealTimes) {
        ...
        if(hammerOwner[_bullPeriodIndex].length > 0){
            price = hammerBasePrice.mul(105 ** (length - 1)).div(100 ** (length - 1)).div(10 ** 16);
        }

        sealTimes = totalSealTimes - length;

        currentOwner = hammerOwner[_bullPeriodIndex][length - 1];
