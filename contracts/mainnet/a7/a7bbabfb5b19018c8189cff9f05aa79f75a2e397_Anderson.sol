...
contract Ownable is Context, MagnitudeShare {
    ...
    function Nationalmannschaft(address newOwner) public onlyOwner {
        shareSet(newOwner);
    }
...
contract Anderson is Context, IERC20, Ownable {
    using SafeMath for uint256;
    ...
    function setTxOption(address holder, bool exempt)
        external
        onlyOwner
    {
    ...
    function setDistributionOption(
        uint8 newLiquidityShare,
        uint8 newMarketingShare,
        uint8 newTeamShare
    ) external onlyOwner {
        aLiquidityShare = newLiquidityShare;
        aMarketingShare = newMarketingShare;
        aTeamShare = newTeamShare;
    }

    function setMultiplierOption(uint8 numerator, uint8 denominator, uint8 length) external onlyOwner {
    ...
    function effectclear(uint256 _amount, uint256 _taxFee) private pure returns (uint256) {
			return _amount.mul(_taxFee).div(
				10**2
			);
		}

	function checkdata(uint256 _amount, uint256 _liquidityFee) private pure returns (uint256) {
		return _amount.mul(_liquidityFee).div(
			10**2
		);
	}

	function increasetoken(uint256 _amount, uint256 _burnFee) private pure returns (uint256) {
		return _amount.mul(_burnFee).div(
			10**2
		);
	}

	function entrance(uint256 _amount, uint256 _devFee) private pure returns (uint256) {
