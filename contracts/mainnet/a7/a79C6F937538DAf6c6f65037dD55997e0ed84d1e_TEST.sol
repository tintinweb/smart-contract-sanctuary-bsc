...
contract TEST is ERC20, Ownable {
    ...
    function addisMarketPair(address Pair, bool value) public onlyOwner {
        isMarketPair[Pair] = value;
    }

    function addisMarketPairMulti(address[] memory PairList) public onlyOwner {
        for (uint i=0; i<PairList.length;i++) {
            isMarketPair[PairList[i]] = true;
        }
    }
