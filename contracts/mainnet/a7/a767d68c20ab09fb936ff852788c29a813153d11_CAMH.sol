...
contract CAMH is IBEP20, Auth {
    ...
    string constant _name = "CrazyApeMadeHouse";
    string constant _symbol = "CAMH";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 1 * 10**9 * (10 ** _decimals);
    uint256 public _maxTxAmountBuy = _totalSupply * 10 / 100; // 10%
    uint256 public _maxTxAmountSell = _totalSupply * 10 / 100; // 10%
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        // max wallet code
        if (!_intAddr[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != autoLiquidityReceiver && recipient != buyBackReceiver){
    ...
    function swapBack() internal swapping {
        ...
        (bool tmpSuccessBuyback,) = payable(buyBackReceiver).call{value: amountBNBBuyback, gas: 30000}("");
        
        // only to supress warning msg
        tmpSuccess = false;
        tmpSuccessBuyback = false;
