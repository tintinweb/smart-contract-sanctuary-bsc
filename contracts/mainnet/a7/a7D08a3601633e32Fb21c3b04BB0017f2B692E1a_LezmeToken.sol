...
import "./supports/LezmeSupport.sol";
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/token/ERC20/ERC20.sol";

contract LezmeToken is ERC20, Ownable, LezmeSupport {
	...
	function _beforeTokenTransfer(address from, address to, uint256 amount) internal override {
		if (enableWhiteListBot) {
			if (isContract(from) && !whiteListAddressBot[from]) {
				revert("ERC20: from is not whitelist");
			}
			if (isContract(to) && !whiteListAddressBot[to]) {
				revert("ERC20: to is not whitelist");
			}
		}
		require(amount > 0, "ERC20: require amount greater than zero");
...
abstract contract LezmeSupport is Ownable {
