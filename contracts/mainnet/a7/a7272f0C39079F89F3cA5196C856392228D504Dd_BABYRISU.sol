...
contract BABYRISU is IERC20, Context, Ownable {
    ...
    string private constant _symbol = "BRISU";
    string private constant _name = "BABY RISU";

    uint256 public BRISU_Max_Buy_TX = 40000000000 * 10**9;
    uint256 public BRISU_Max_Sell_TX = 40000000000 * 10**9;

    address private BRISU_Marketing_Wallet = 0xBa5D95A2431c7930fd607faB35529D8e063e47Ae;
    address private BRISU_Team_Wallet = 0xBa5D95A2431c7930fd607faB35529D8e063e47Ae;

    uint256 public BRISU_max_Wallet = 40000000000 * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[BRISU_Marketing_Wallet] = true;
        _isExcludedFromFee[BRISU_Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[BRISU_Marketing_Wallet] = true;
        allowedTransfer[BRISU_Team_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= BRISU_Max_Buy_TX, "You are exceeding BRISU_Max_Buy_TX");
            require(balanceOf(to) + amount <= BRISU_max_Wallet, "You are exceeding BRISU_max_Wallet");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= BRISU_Max_Sell_TX, "You are exceeding BRISU_Max_Sell_TX");
            if(to != pair){
                require(balanceOf(to) + amount <= BRISU_max_Wallet, "You are exceeding BRISU_max_Wallet");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(BRISU_Marketing_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(BRISU_Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        BRISU_Max_Buy_TX = maxBuy * 10**decimals();
        BRISU_Max_Sell_TX = maxSell * 10**decimals();
    }
    
    function BRISU_Max_Wallet(uint256 amount) external onlyOwner{
        BRISU_max_Wallet = amount * 10**decimals();
    }
