...
contract VoteWin is Pausable, Ownable {
    ...
    constructor(IERC20 _voteToken, address _ceo, IRefferal _refer, ICOMM _commTreasury) {
    ...
    function vote(uint _day, uint _matchIndex, uint _teamSide, uint amount) external whenNotPaused {
        ...
        uint comm = amount * 15 / 100;
        uint process = amount - comm;
        voteToken.transferFrom(_msgSender(), address(this), process);
        if(refer.isReferrer(_msgSender())) {
            voteToken.transferFrom(_msgSender(), address(commTreasury), comm);
            commTreasury.handleComm(_msgSender(), amount, voteToken);
        } else {
            voteToken.transferFrom(_msgSender(), ceo, comm);
        }
        ...
        users[_msgSender()].matchAmount[_day][_matchIndex] += process;

        matchDetails[_day][_matchIndex].totalVote[_teamSide] += process;
