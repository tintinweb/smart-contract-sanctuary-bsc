...
contract stakereward {
    ...
    uint256[32] public _priceID = [1*1e15,1*1e15,1*1e15,1*1e15,1*1e15,1*1e15,1*1e15,1*1e15,1*1e15,1*1e15,
                                   1*1e15,1*1e15,1*1e15,1*1e15,1*1e15,1*1e15,1*1e15,1*1e15,1*1e15,1*1e15,
                                   1*1e15,1*1e15,1*1e15,1*1e15,1*1e15,1*1e15,1*1e15,1*1e15,1*1e15,1*1e15,1*1e15,1*1e15];

    uint256[32] public _rewardPerTokenStored;
    mapping(address => uint256[32]) public _userRewardPerTokenPaid;

    uint256[32] public _totalSupply;
    mapping(address => uint256[32]) public _balances;
        ...
        onlyLock
    {
        ...
        uint tprice = _priceID[_id];
        IERC20(usdt).safeTransferFrom(msg.sender,address(this),tprice);

        _totalSupply[_id] = _totalSupply[_id].add(1);
        _balances[msg.sender][_id] = _balances[msg.sender][_id].add(1);

        IERC20(usdt).safeTransfer(controller,tprice.mul(70).div(100));
        _stakeTotal[_id] = _stakeTotal[_id].add(tprice.mul(30).div(100));
    ...
    function withdraw()
        external 
    {
        IERC20(usdt).safeTransfer(msg.sender,earned(msg.sender));
        for(uint i = 0;i<32;i++)
        {
            _userRewardPerTokenPaid[msg.sender][i] = _rewardPerTokenStored[i];
        }
    }
