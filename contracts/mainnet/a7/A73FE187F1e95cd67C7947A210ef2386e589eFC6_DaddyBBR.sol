...
contract DaddyBBR is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("DaddyBBR", "DADDY") {
...
contract DaddyBBRDividendTracker is DividendPayingToken, Ownable {
    ...
    DaddyBBR public DaddyBBRContract;
    ...
    constructor(
        ...
        defaultToken = token;
        DaddyBBRContract = DaddyBBR(mainContract);
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(DaddyBBRContract);

            uint256 prevBalance = DaddyBBRContract.balanceOf(address(this));
            ...
            {
                uint256 received = DaddyBBRContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    DaddyBBRContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
