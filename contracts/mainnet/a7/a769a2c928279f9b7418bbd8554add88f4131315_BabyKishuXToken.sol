...
contract BabyKishuXToken is Context, IERC20, Ownable {
    ...
    uint256 private constant _tTotal = 17e9 * 17**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private constant _name = "Baby KishuX Token";
    string private constant _symbol = "kishuX";
    ...
    function setBuyTaxFeePercent(uint256 taxFee) external onlyOwner() {
        _buyTaxFee = taxFee;
        require(_buyTaxFee + _buyDevFee + _buyLiquidityFee + _buyMarketFee <= 120, "Total fees exceed 12%");
    }
    ...
    function setSellTaxFeePercent(uint256 taxFee) external onlyOwner() {
        _sellTaxFee = taxFee;
        require(_sellTaxFee + _sellDevFee + _sellLiquidityFee + _sellMarketFee <= 120, "Total fees exceed 12%");
    }
