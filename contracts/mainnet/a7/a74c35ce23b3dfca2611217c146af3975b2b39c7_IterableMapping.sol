...
contract DesiWeed is ERC20, Ownable {
    ...
    uint256 public presaleRate = 62500000;
    uint256 public minPurchase = 0.1 ether;
    uint256 public maxPurchase = 10 ether;

    DesiWeedDividendTracker public dividendTracker;
    ...
    address public marketingWallet = 0x118CfC38F851696691785e93D85e3B2656E2c6B4;
    address public devWallet = 0x5D62D1E955Fa786778528BE53f33cFc93964a9F4;
    address public teamWallet = 0x82A25DC4CbE6B9908b63AEc8d5007d8E7EDd2d9f;
    ...
    constructor()  ERC20("DesiWeed", "DWEED") {

    	dividendTracker = new DesiWeedDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "DesiWeed: The dividend tracker already has that address");

        DesiWeedDividendTracker newDividendTracker = DesiWeedDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "DesiWeed: The new dividend tracker must be owned by the DesiWeed contract");
    ...
    function updateRouter(address newAddress) public onlyOwner {
        require(newAddress != address(router), "DesiWeed: The router already has that address");
        emit Updaterouter(newAddress, address(router));
        router = IRouter(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "DesiWeed: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address newPair, bool value) public onlyOwner {
        require(newPair != pair, "DesiWeed: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address newPair, bool value) private {
        require(automatedMarketMakerPairs[newPair] != value, "DesiWeed: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "DesiWeed: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "DesiWeed: Cannot update gasForProcessing to same value");
...
contract DesiWeedDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()  DividendPayingToken("DesiWeed_Dividend_Tracker", "DesiWeed_Dividend_Tracker") {
    	claimWait = 3600;
    }

    function _transfer(address, address, uint256) internal pure override{
        require(false, "DesiWeed_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "DesiWeed_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DesiWeed contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DesiWeed_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DesiWeed_Dividend_Tracker: Cannot update claimWait to same value");
