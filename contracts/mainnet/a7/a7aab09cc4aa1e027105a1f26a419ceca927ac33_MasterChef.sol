...
// File contracts/interfaces/IANT.sol

pragma solidity >=0.6.0 <0.8.0;

interface IANT is IERC20 {
...
interface IVault {
  function safeANTTransfer(address _account, uint256 _amount) external;
}
...
contract MasterChef is Ownable {
  ...
  constructor(
    ...
    poolInfo.push(
      PoolInfo({ lpToken: _ANT, allocPoint: 1000, lastRewardBlock: startBlock, accANTPerShare: 0 })
    );
  ...
  function setANTPerBlock(uint256 _ANTPerBlock) external onlyOwner {
    massUpdatePools();
    ANTPerBlock = _ANTPerBlock;
  }
  ...
  function add(
    ...
    poolInfo.push(
      PoolInfo({ lpToken: _lpToken, allocPoint: _allocPoint, lastRewardBlock: lastRewardBlock, accANTPerShare: 0 })
    );
    updateStakingPool();
  }
  ...
  function updatePool(uint256 _pid) public {
    ...
    uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
    uint256 ANTReward = multiplier.mul(ANTPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ANT.mint(devaddr, ANTReward.div(10));
    ANT.mint(address(vault), ANTReward);
  ...
  function deposit(uint256 _pid, uint256 _amount) public {
    ...
    if (user.amount > 0) {
      ...
      if (pending > 0) {
        safeANTTransfer(msg.sender, pending);
      }
    }
    if (_amount > 0) {
  ...
  function safeANTTransfer(address _to, uint256 _amount) internal {
    vault.safeANTTransfer(_to, _amount);
  }
