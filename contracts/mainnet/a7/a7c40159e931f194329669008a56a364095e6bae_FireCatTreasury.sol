 ...
 */
contract FireCatTreasury is IFireCatTreasury, FireCatTransfer, Ownable {
    ...
    function addTreasury(address user, uint256 tokenId, uint256 addAmount) external onlyProxy returns (uint) {
        ...
        totalTreasuryNew = totalTreasuryNew.add(actualAddAmount);
    ...
    function swapTreasury(uint256 tokenId) external returns (uint) {
        require(swapOn, "TRS:E06");
        require(IFireCatNFT(fireCatNFT).ownerOf(tokenId) == msg.sender, "TRS:E07");
        ...
        require(totalTreasuryNew <= _totalTreasury, "TRS:E05");
    ...
    /// @inheritdoc IFireCatTreasury
    function withdrawTreasury(uint256 amount) external onlyOwner returns (uint) {
        // totalTreasury + actualAddAmount
        require(_totalTreasury - amount < _totalTreasury, "TRS:E05");
        uint totalTreasuryNew;
        uint actualSubAmount;
