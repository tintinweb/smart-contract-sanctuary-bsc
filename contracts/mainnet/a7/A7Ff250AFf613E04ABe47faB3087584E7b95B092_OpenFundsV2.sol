...
contract OpenFundsV2 is Context, Ownable , ReentrancyGuard {
    ...
    function deposit(uint256 plan , uint256 _amount) public noReentrant  {
        ...
        if (plan == 1) {
            ...
            investPlan1[msg.sender] = user_investment_details(msg.sender,totalInvestment,block.timestamp,block.timestamp + 3 days,15);
        } else if (plan == 2) {
            ...
            uint256 userCurrentInvestment = _amount;
            uint256 totalInvestment = SafeMath.add(userLastInvestment,userCurrentInvestment);
            investPlan2[msg.sender] = user_investment_details(msg.sender,totalInvestment,block.timestamp,block.timestamp + 3 days,20);
        } else if (plan == 3) {
            ...
            uint256 userCurrentInvestment = _amount;
            uint256 totalInvestment = SafeMath.add(userLastInvestment,userCurrentInvestment);
            investPlan3[msg.sender] = user_investment_details(msg.sender,totalInvestment,block.timestamp,block.timestamp + 3 days,30);
        } else {
    ...
    function withdrawal(uint256 plan) public noReentrant {
        ...
        BusdInterface.transfer(msg.sender,SafeMath.sub(userEarns, wFee) );
