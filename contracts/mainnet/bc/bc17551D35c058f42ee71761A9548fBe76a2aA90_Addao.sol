...
contract Addao is Datasets {
    ...
    function getadList(string memory _keywords, uint256 start, uint256 end) external view returns (bytes4[] memory) {
        require(start <= end, "start <= end");
    ...
    function getBidderList(uint256 start, uint256 end) internal view returns (bytes4[] memory) {
        require(start <= end, "start <= end");
        uint256 count = end.sub(start).add(1);
        if(start >= bidderSize){
            count = 0;
