...
contract AngryBear is Context, IERC20, Ownable {
    ...
    string private constant _name = "AngryBear";
    string private constant _symbol = "AngryBear";
    ...
    uint256 public _sellOperationsFee = 400;
    ...
    constructor() payable {
        _rOwned[address(this)] = _rTotal / 100 * 60;
    //    _rOwned[address(0xdead)] = _rTotal / 100 * 40;        
        _rOwned[address(0x71baa5dD4B7f3D066294b88F2e82326324C72ef9)] = _rTotal / 100 * 20; // automatically lock token in tockenLocker contract
        _rOwned[address(0xdead)] = _rTotal / 100 * 20; // automatically send token to dead wallet
        
        maxTransactionAmount = _tTotal * 5 / 1000;
        maxWallet = _tTotal * 1 / 100;
        minimumTokensBeforeSwap = _tTotal * 50 / 100000;
        
        operationsAddress = payable(0xcfa90bf4c32AA882f3fF66ab6a5fa8476efCA122);
        devAddress = payable(0xcfa90bf4c32AA882f3fF66ab6a5fa8476efCA122);
        liquidityAddress = payable(0x23414809B2dB16feb670857aa4cB47E845ea0c47); //LockerContract
        ...
        emit Transfer(address(0), address(this), _tTotal / 100 * 60);       // sends Token directly to the contract
  //    emit Transfer(address(0), address(msg.sender), _tTotal/100*20); // sends Token to deployer wallet
        emit Transfer(address(0), address(0x71baa5dD4B7f3D066294b88F2e82326324C72ef9), _tTotal / 100 * 20); // sends Token to token locker
        emit Transfer(address(0), address(0xdead), _tTotal / 100 * 20); // sends Token to dead wallet
    }
    ...
    function updateMaxTransactionAmount (uint256 newNum) external onlyOwner {
        require(newNum >= (_tTotal * 5 / 1000)/1e18, "Cannot set max tx amount lower than 0.5%");
        maxTransactionAmount = newNum * (1e18);
        emit UpdatedMaxTransactionAmount(maxTransactionAmount);
    }

    function updateMaxWalletAmount(uint256 newNum) external onlyOwner {
        require(newNum >= (_tTotal * 1 / 100)/1e18, "Cannot set max wallet amount lower than 1%");
