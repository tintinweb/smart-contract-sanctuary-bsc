...
contract GrainStats is OwnableUpgradeSafe {
    ...
    function _calculateTotalEarnedPerYear(address _pool, address _share) internal view returns (uint256) {
        return IRewardPool(_pool).getRewardPerSeconds().mul(365 days).mul(IPriceChecker(priceChecker).getTokenPriceToUsd(_share)).div(1e18);
    }

    function poolStats(address _pool) public view returns (uint256 poolLength, uint256 totalTvl, address[] memory lps, uint256[] memory allocPoints, uint256[] memory tvls, uint256[] memory aprs, uint256[] memory lpPrices) {
        ...
        for (uint i = 0; i < poolLength; i++) {
            ...
            {
                address _share = (_pool == grainPool) ? grain : bgold;
                uint256 _totalEarnedPerYear = _calculateTotalEarnedPerYear(_pool, _share);
    ...
    function tvls() external view returns (uint256 grainPoolTvl, uint256 bgoldPoolTvl, uint256 boardRoomTvl, uint256 totalTvl) {
        ...
        (, grainPoolTvl, , , , ,) = poolStats(grainPool);
