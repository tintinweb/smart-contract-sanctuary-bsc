...
contract NCOINSTANDARTCONTRACT is Context, IERC20, Ownable {
    ...
    function setBuyTaxes(uint256 newLiquidityTax, uint256 newMarketingTax) external onlyOwner() {
        require(_totalTaxIfBuying >= 0 && _totalTaxIfBuying <= 15, "ERC20: Please set buy tax to 0 - 15");
    ...
    function setSelTaxes(uint256 newLiquidityTax, uint256 newMarketingTax) external onlyOwner() {
        require(_totalTaxIfSelling >= 0 && _totalTaxIfSelling <= 15, "ERC20: Please set buy tax to 0 - 15");
    ...
    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        require(maxTxAmount >= (_totalSupply * 10**(_decimals - 2)) && maxTxAmount <= _totalSupply * (10**_decimals), "ERC20: Set Max TX between 1% - 100% of total supply");
    ...
    function setWalletLimit(uint256 newLimit) external onlyOwner {
        require(newLimit >= (_totalSupply * 10**(_decimals - 2)) && newLimit <= _totalSupply * (10**_decimals), "ERC20: Set Max Wallet between 1% - 100% of total supply");
        _walletMax  = newLimit;
    }
