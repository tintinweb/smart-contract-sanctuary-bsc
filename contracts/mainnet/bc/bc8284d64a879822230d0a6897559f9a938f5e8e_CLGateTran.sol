...
contract CLGateTran is ERC165, ReentrancyGuard, Context, IERC721TokenReceiver {
	...
	constructor(
        address _owner, address _admin, IERC721 _contract_ceekland)
    {
		contract_owner = _owner;
        contract_admin = _admin;
    ...
    function transferFromAllTokens(uint256[] memory tokenIds, address oldTokenOwnerAddress, address newTokenOwnerAddress, string memory newTokenOwnerCeekUserId) public {
        require((_msgSender()==contract_owner || _msgSender()==contract_admin), "msg.sender is not the owner nor the admin");
        ...
        for (i=0; i<tokenIds.length; i++) {
            tokenId = tokenIds[i];
            callOptionalReturn(contract_ceekland, abi.encodeWithSelector(contract_ceekland.transferFrom.selector, oldTokenOwnerAddress, newTokenOwnerAddress, tokenId));
       		Address.functionCall(address(contract_ceekland), abi.encodeWithSignature("setCeekOwnerOf(uint256,string)", tokenId, newTokenOwnerCeekUserId));
        }
    }

    	// update contract fields
    function updateAdmin(IERC721 _contract_ceekland, address _admin) public {
        require(_msgSender()==contract_owner, "msg.sender is not the owner");
        contract_ceekland = _contract_ceekland;
        contract_admin = _admin;
    }
