    ...
    function rewardPerToken(address account) public view returns (uint256) {
        if (s_totalSupply == 0) {
            ...
            return
                ...
                (block.timestamp - s_lastUpdateTime) * (s_balances[account] / 100 * rateReward) / 365 / 24 / 60 / 60;
        }
    }

    function calculateReward(address account) public view returns (uint256) {
        return (block.timestamp - s_lastUpdateTime) * (s_balances[account] / 100 * rateReward) / 365 / 24 / 60 / 60;
    }
