...
interface IUVTakeProfit {
    ...
    function withdrawToken(uint8 _id) external;
...
contract UVTakeProfit is Ownable, AccessControl, IUVTakeProfit {
    ...
    function withdrawToken(uint8 _stakeId) external {
        ...
        if (stakeInfos[_stakeId].feePercent > 0) {
            ...
            TransferHelper.safeTransfer(stakeInfos[_stakeId].tokenAddress, fundWallet, fee.div(dec));
        }

        TransferHelper.safeTransfer(stakeInfos[_stakeId].tokenAddress, msg.sender, totalToken.sub(fee).div(dec));
...
contract UVReserve is Ownable, AccessControl, IUVReserve {
        ...
        onlyRole(MANAGER_ROLE)
    {
        ...
        require(amount > 0);
        TransferHelper.safeTransfer(token, poolStake, amount);
    }
