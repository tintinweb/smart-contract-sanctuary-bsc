...
contract DividendPayingToken is DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  address public constant token = address(0x715D400F88C167884bbCc41C5FeA407ed4D2f8A0); // AXIE 0x715D400F88C167884bbCc41C5FeA407ed4D2f8A0
...
contract DividendTracker is DividendPayingToken {
    ...
    constructor() {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 3 * 1e5 * 1e18; //must hold 300,000+ tokens
    }
...
contract BabyTestAxie2 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public constant penaltyDuration = 7 days;
    
    address public constant token = address(0x715D400F88C167884bbCc41C5FeA407ed4D2f8A0); // bep-20 AXIE 0x715D400F88C167884bbCc41C5FeA407ed4D2f8A0
    ...
    uint256 public charityPenaltySellFee;
    ...
    constructor() ERC20("BabyTestAxie2", "BabyTestAxie2"){
        address newOwner = address(0xF28c816d9D6B5f25066f7dD5F5Ed346929c02559); // update to 0x036e995024aCe105D9DF801801fb7957936bc512
        ...
        marketingPenaltySellFee = 600;
        liquidityPenaltySellFee = 600;
        charityPenaltySellFee = 600;
        totalPenaltySellFees = rewardsPenaltySellFee + marketingPenaltySellFee + liquidityPenaltySellFee + charityPenaltySellFee;

        rewardsSellFee = 400;
        marketingSellFee = 400;
        liquiditySellFee = 300;
        charitySellFee = 50;
        totalSellFees = rewardsSellFee + marketingSellFee + liquiditySellFee + charitySellFee;
        
        rewardsBuyFee = 400;
        marketingBuyFee = 400;
        liquidityBuyFee = 300;
        charityBuyFee = 50;
        totalBuyFees = rewardsBuyFee + marketingBuyFee + liquidityBuyFee + charityBuyFee;
        
    	dividendTracker = new DividendTracker();
    	
    	marketingWallet = address(0xe7747a365ACc270DC423036eF90C46107eaD6617); // set as marketing wallet
        charityWallet = address(0x6bba9eeD036dAaD47FBeAF671B80E2Fb774Bf759); // set as charity wallet
        liquidityWallet = address(0xabc4611E7929d368A1c89B4d726778548BdeFc79); // set as liquidity wallet
    ...
    function updateBuyFees(uint256 _marketingFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _charityFee) external onlyOwner {
        marketingBuyFee = _marketingFee;
        rewardsBuyFee = _rewardsFee;
        liquidityBuyFee = _liquidityFee;
        charityBuyFee = _charityFee;
        totalBuyFees = marketingBuyFee + rewardsBuyFee + liquidityBuyFee + charityBuyFee;
        require(totalBuyFees <= 1500, "Must keep fees at 15% or less");
    }
    
    function updateSellFees(uint256 _marketingFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _charityFee) external onlyOwner {
        marketingSellFee = _marketingFee;
        rewardsSellFee = _rewardsFee;
        liquiditySellFee = _liquidityFee;
        charitySellFee = _charityFee;
        totalSellFees = marketingSellFee + rewardsSellFee + liquiditySellFee + charitySellFee;
        require(totalSellFees <= 2500, "Must keep fees at 25% or less");
    }

     function updatePenaltySellFees(uint256 _marketingFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _charityFee) external onlyOwner {
        marketingPenaltySellFee = _marketingFee;
        rewardsPenaltySellFee = _rewardsFee;
        liquidityPenaltySellFee = _liquidityFee;
        charityPenaltySellFee = _charityFee;
        totalPenaltySellFees = marketingPenaltySellFee + rewardsPenaltySellFee + liquidityPenaltySellFee + charityPenaltySellFee;
    ...
    function _transfer(
        ...
        if(takeFee){
            ...
            if (automatedMarketMakerPairs[to] && totalPenaltySellFees > 0 && block.timestamp < walletSellPenaltyEndTime[from]){
                ...
                tokensForCharity += fees * charityPenaltySellFee / totalPenaltySellFees;
            }
            // on sell
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForLiquidity + tokensForMarketing + tokensForRewards + tokensForCharity;
