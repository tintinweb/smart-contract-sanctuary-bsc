...
contract StakingPhenom{
    ...
    constructor(address owner, address bonusContract, address migrationCaller){
        _owner = owner;
    ...
    function delivery(
        ...
        if(_emergency){
            holders[user].push(Staking(amount, 0, 0, 0, 0, 0, true));
    ...
    function withdrawPercents(address user, uint8 token) public{
    ...
    function withdrawPercentsForPeriod(address user, uint8 token, uint256 period) public{
        for(uint i = 0; i < holders[user].length; i++){
            if(holders[user][i].staking == token && holders[user][i].StakingMonths == period){
                withdrawOneStakingPercents(user, i);
            }
        }
    }
    ...
    function withdrawOneStakingPercents(address user, uint256 stakingNum) public{
        ...
        require(!holders[user][stakingNum].frozen, "Stacking is not activated yet");
        uint256 amount = 0;
        ...
        if(amount > 0){
            if(_emergency){
                _depositToken.transfer(user, amount * 75 / 100);
        ...
        if(amountUSDT > 0){
            ...
            emit WithdrawalPercents(user, amountUSDT, 0);
        }
        if(bAmountUSDT > 0){
            bAmountUSDT = 10 ** _depositToken.decimals() * bAmountUSDT / _priceUSDT;
            if(_emergency){
                _depositToken.transfer(user, bAmountUSDT);
            }else{
            ...
            holders[user].pop();
            holderStakingCount[user]--;
            emit WithdrawalBodyDeposit(user, bAmountUSDT, 0);
        }
    ...
    function addToStaking(uint256 amount, uint256 stakingNum) public{
        ...
        if(m > 0){
            reinvestOneStakingPercents(msg.sender, stakingNum);
        }
        holders[msg.sender][stakingNum].amount += amount;
        emit AddedToTokenStaking(msg.sender, amount, 1);
    }

    function reinvestStakingPercents(address user, uint8 token) public{
        for(uint i = 0; i < holders[user].length; i++){
            if(holders[user][i].staking == token){
    ...
    function reinvestStakingPercentsForPeriod(address user, uint8 token, uint256 period) public{
        for(uint i = 0; i < holders[user].length; i++){
            if(holders[user][i].staking == token && holders[user][i].StakingMonths == period){
                reinvestOneStakingPercents(user, i);
            }
    ...
    function reinvestOneStakingPercents(address user, uint256 stakingNum) public{
        ...
        if(amount > 0){
            ...
            user,nAmount,allStakingAmount));
            require(success,"depositPools call FAIL");
            emit ReinvestedPercents(user, amount, 1);
        }
        if(amountUSDT > 0){
            holders[user][stakingNum].amount += amountUSDT;
            emit ReinvestedPercents(user, amountUSDT, 0);
        }
    }
