...
/**
 ...
 Telegram : https://t.me/RewardsBSC

 Twitter : https://twitter.com/RewardsBSC

 Website : http://rewardsbsc.tech/   rewardsbsc.tech
...
contract RewardsBSC is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    RewardsBSCDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Rewards BSC", "RewardsBSC") {
        marketingWallet = payable(0xb8e2D5601Ae113E3A41a4278c141975122695148);
        ...
        dividendTracker = new RewardsBSCDividendTracker(
            payable(this),
            router,
    ...
    // set cooldown timer, can only be between 0 and 1 second (1 second max)
    ...
    function updateFees(
        ...
        require(totalSellFees <= 5 && totalBuyFees <= 5, "total Fees cannot exceed 5% BUY 5% SELL");
...
contract RewardsBSCDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    RewardsBSC public RewardsBSCContract;
    ...
    constructor(
        ...
        defaultToken = token;
        RewardsBSCContract = RewardsBSC(mainContract);
    ...
    function _transfer(
        ...
        require(false, "RewardsBSC_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "RewardsBSC_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main RewardsBSC contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(RewardsBSCContract);

            uint256 prevBalance = RewardsBSCContract.balanceOf(address(this));
            ...
            {
                uint256 received = RewardsBSCContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    RewardsBSCContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
