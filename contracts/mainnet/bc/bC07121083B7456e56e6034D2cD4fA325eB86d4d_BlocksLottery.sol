...
contract BlocksLottery is VRFConsumerBase, Ownable {
    ...
    uint256 public amountOfTokenWin = 2400 * 10**18;
    IERC20 public immutable rewardToken;
    mapping(address => bool) blacklist;
    BlocksSpace2 public space;
    address[10] previousWinners;
    uint256 previousWinnersCounter = 0;

    event EmergencySweepWithdraw(address indexed user, IERC20 indexed token, uint256 amount);
    event WinnerChosen(address indexed user, uint256 amount, uint256 stage, uint256 block);
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        uint256 stage;
        uint256 blockSelected = (randomness % 42 * 100) + randomness % 24;
        address winner = space.getOwnerOfBlock(blockSelected);  // %42 %24
        // Select winner that is not blacklisted (3 tries)
        if(blacklist[winner] || recentlyWon(winner)){
            blockSelected = ((randomness + 69) % 42 * 100) + randomness % 24;
            winner = space.getOwnerOfBlock(blockSelected);
            stage = 1;
            if(blacklist[winner] || recentlyWon(winner)){
                blockSelected = ((randomness + 96) % 42 * 100) + randomness % 24;
                winner = space.getOwnerOfBlock(blockSelected);
                stage = 2;
            }
        }
        ...
        if (blacklist[winner] == false && !recentlyWon(winner)) {

            lotteryState.winner = winner;

            previousWinners[previousWinnersCounter] = winner;
            if(previousWinnersCounter >= 9){
                previousWinnersCounter = 0;
            }else{
                previousWinnersCounter++;
            }

            uint256 tokensWon = safeBlsTransfer(winner);
            emit WinnerChosen(winner, tokensWon, stage, blockSelected);
        }else{
            emit NoWinnerSelected(stage, randomness);
        }
    }

    function recentlyWon(address wannabeWinner) internal returns(bool){
        for (uint256 i = 0; i < 10; ++i) {
            if(wannabeWinner == previousWinners[i]){
                return true;
            }
        }
        return false;
...
contract BlocksSpace2 is Ownable {
    ...
    event DiceRolled(uint256 lastRollBlock, uint256 randomness);
    ...
    uint256 public minTimeBetweenRolls = 20 hours;
    uint256 public addedVariability = 8 hours;
    uint256 public lastRollBlock = block.number;
    function rollTheDice() public {
        if(address(lotteryContract) != address(0) && block.number > (lastRollBlock + (minTimeBetweenRolls / 3) + block.number % (addedVariability/3))){
            // Request randomness from ChainLink
            lotteryContract.requestWinningBlock();
            lastRollBlock = block.number;
            emit DiceRolled(lastRollBlock, (lastRollBlock + (minTimeBetweenRolls / 3) + block.number % (addedVariability/3)));
        }
    }
    ...
    function setMinTimeBetweenRolls(uint256 seconds_) external onlyOwner{
        minTimeBetweenRolls = seconds_;
    }

    function setAddedVariability(uint256 seconds_) external onlyOwner{
        addedVariability = seconds_;
    }
