...
contract ZefiCard is Ownable {
    ...
    */
    ...
    uint constant public MAX_FEE_ZEFI = 1000;
    uint constant public BURN_FEE_ZEFI = 800;
    uint constant public REFERAL_FEE_ZEFI = 200;
    uint constant public ALL_FEE_ZEFI = BURN_FEE_ZEFI + REFERAL_FEE_ZEFI;
    ...
    function customRouteToBusd(
        ...
        if(IERC20(zefi).allowance(address(this), _dexrouter) == 0){
            IERC20(zefi).safeApprove(_dexrouter, uint(-1));
        }

        if(IERC20(busd).allowance(address(this), _dexrouter) == 0){
            IERC20(busd).safeApprove(_dexrouter, uint(-1));
        }
        ...
        if(_feeNetwork){
            IUniswapRouter(_dexrouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(tokenBal,  uint256(0), _TokentoBusd, address(this), now.add(1800));
        }else{
            IUniswapRouter(_dexrouter).swapExactTokensForTokens(tokenBal, 0, _TokentoBusd, address(this), now.add(600));
        }
        ...
        uint256 nbusdBal = IERC20(busd).balanceOf(address(this));
        IUniswapRouter(unirouter).swapExactTokensForTokens(nbusdBal, 0, busdToZefiRoute, address(this), now.add(600));
        uint256 zefiBal = IERC20(zefi).balanceOf(address(this));


        if (_referral == dead){
            uint256 zefiBalBuyback = zefiBal.mul(ALL_FEE_ZEFI).div(MAX_FEE_ZEFI);
            IERC20(zefi).safeTransfer(dead, zefiBalBuyback);
        }else{
            uint256 zefiBalBuyback = zefiBal.mul(BURN_FEE_ZEFI).div(MAX_FEE_ZEFI);
            IERC20(zefi).safeTransfer(dead, zefiBalBuyback);            

            uint256 zefiBalReferral = zefiBal.mul(REFERAL_FEE_ZEFI).div(MAX_FEE_ZEFI);
            IERC20(zefi).safeTransfer(_referral, zefiBalReferral);
        }
