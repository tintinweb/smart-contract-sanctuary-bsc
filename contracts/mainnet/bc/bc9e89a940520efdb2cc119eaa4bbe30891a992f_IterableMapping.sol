...
/**
 ...
 .d8888b.   .d88888b.  888     888 8888888 8888888b.  888b    888  .d88888b.  8888888b.  8888888888 .d8888b.  
d88P  Y88b d88P" "Y88b 888     888   888   888  "Y88b 8888b   888 d88P" "Y88b 888  "Y88b 888       d88P  Y88b 
Y88b.      888     888 888     888   888   888    888 88888b  888 888     888 888    888 888       Y88b.      
 "Y888b.   888     888 888     888   888   888    888 888Y88b 888 888     888 888    888 8888888    "Y888b.   
    "Y88b. 888     888 888     888   888   888    888 888 Y88b888 888     888 888    888 888           "Y88b. 
      "888 888 Y8b 888 888     888   888   888    888 888  Y88888 888     888 888    888 888             "888 
Y88b  d88P Y88b.Y8b88P Y88b. .d88P   888   888  .d88P 888   Y8888 Y88b. .d88P 888  .d88P 888       Y88b  d88P 
 "Y8888P"   "Y888888"   "Y88888P"  8888888 8888888P"  888    Y888  "Y88888P"  8888888P"  8888888888 "Y8888P"  
                  Y8b                                                                                         
...
[ OFFICIAL LINKS ]
WEBSITE:  https://squidnodes.com/
WEB3APP:  https://app.squidnodes.com/
TELEGRAM: https://t.me/SquidNodes

Liquidity Tokens burned forever
...
contract NODERewardManagement {
    ...
    uint256 public globalRewardAmount = 0;
    uint256 public lastDistributionTime = block.timestamp;
    uint256 public totalNodeValue = 0;
    ...
    modifier onlySentry() {
        require(msg.sender == token || msg.sender == gateKeeper, "Denied");
        _;
    }
    ...
    private
    {
        uint256 addRewardAmount = (block.timestamp - lastDistributionTime).mul(rewardPerValue).div(claimTime);
        globalRewardAmount += addRewardAmount;
        lastDistributionTime = block.timestamp;
    }

    function createBulkNode(address[] memory accounts, string[] memory nodeNames, uint256[] memory _initValues, uint256[] memory _multipliers, uint256[] memory timeadd) external onlySentry {
        for (uint256 i = 0; i < accounts.length; i++) {
            createNodeBulk(accounts[i], nodeNames[i], _initValues[i], _multipliers[i], timeadd[i]);
        }
    }

    function createNodeBulk(address account, string memory nodeName, uint256 _nodeInitValue, uint256 _multiplier, uint256 timeadd) internal {
        require(
        ...
        _nodesOfUser[account].push(
            NodeEntity({
                ...
                lastClaimAmount: globalRewardAmount,
                rewardMult: _multiplier,
                nodeValue: _nodeInitValue,
                addValueCount: 0
        })
        );
        nodeOwners.set(account, _nodesOfUser[account].length);
        totalNodesCreated++;
        totalNodeValue += _nodeInitValue;
    }

    function createNodeManual(address account, string memory nodeName, uint256 _nodeInitValue, uint256 _multiplier) external onlySentry {
        require(
            isNameAvailable(account, nodeName),
            "CREATE NODE: Name not available"
        );
    ...
    function createNode(address account, string memory nodeName, uint256  _nodeInitialValue) external onlySentry {
        ...
        globalRewardAmount += addRewardAmount;
        lastDistributionTime = block.timestamp;
        totalNodeValue += _nodeInitialValue;
    }

    function getTotalNodeValue() public view returns (uint256) {
    ...
    public 
    {
        ...
        require(isNodeOwner(sender), "GET REWARD MULTIPLIER OF: NO NODE OWNER");
    ...
    returns (uint256)
    {
        ...
        NodeEntity storage node = _getNodeWithCreatime(nodes, _creationTime);
        uint256 rewardAvailable = globalRewardAmount - node.lastClaimAmount;
        uint256 rewardNode = rewardAvailable.mul(node.rewardMult).mul(node.nodeValue).div(100000).div(1e18);
        node.lastClaimAmount = globalRewardAmount;
    ...
    returns (uint256)
    {
        ...
        for (uint256 i = 0; i < nodesCount; i++) {
            _node = nodes[i];
            rewardAvailable = globalRewardAmount - _node.lastClaimAmount;
            rewardsTotal += rewardAvailable.mul(_node.rewardMult).mul(_node.nodeValue).div(100000).div(1e18);
            _node.lastClaimAmount = globalRewardAmount;
    ...
    returns (uint256)
    {
        ...
        NodeEntity storage node = _getNodeWithCreatime(nodes, _creationTime);
        require(
            node.lastClaimTime + claimTime < block.timestamp,
            "You need to wait before you can compound"
        );
        uint256 rewardAvailable = globalRewardAmount - node.lastClaimAmount;
        uint256 rewardNode = rewardAvailable.mul(node.rewardMult).mul(node.nodeValue).div(100000).div(1e18);
    ...
    returns (uint256)
    {
        ...
        for (uint256 i = 0; i < nodesCount; i++) {
            _node = nodes[i];
            if (_node.lastClaimTime + claimTime < block.timestamp){
                rewardAvailable = globalRewardAmount -  _node.lastClaimAmount;
                rewardsTotal += rewardAvailable.mul(_node.rewardMult).mul(_node.nodeValue).div(100000).div(1e18);
                _node.nodeValue += rewardAvailable.mul(_node.rewardMult).mul(_node.nodeValue).div(100000).div(1e18);
                ...
                if ( rewardAvailable > 0) {
                    ...
                    _node.lastClaimTime = block.timestamp;
                    totalNodeValue += rewardAvailable;
                }
            }
        }
        return rewardsTotal;
    ...
    returns (uint256)
    {
        require(isNodeOwner(account), "GET AddValueCount OF: NO NODE OWNER");
    ...
    returns (uint256)
    {
        require(isNodeOwner(account), "GET REWARD MULTIPLIER OF: NO NODE OWNER");
        uint256 nodesCount;
        uint256 valueCount = 0;
    ...
    returns (uint256)
    {
        ...
        for (uint256 i = 0; i < nodesCount; i++) {
            rewardAvailable = globalRewardAmount - nodes[i].lastClaimAmount;
            rewardCount += rewardAvailable.mul(nodes[i].rewardMult).mul(nodes[i].nodeValue).div(100000).div(1e18);
        }
    ...
    {
        return
        (globalRewardAmount - _getNodeWithCreatime(_nodesOfUser[account], creationTime)
        .lastClaimAmount).mul(_getNodeWithCreatime(_nodesOfUser[account], creationTime).rewardMult).mul(_getNodeWithCreatime(_nodesOfUser[account], creationTime).nodeValue).div(100000).div(1e18);
    }
    ...
    returns (string memory)
    {
        ...
        uint256 nodesCount = nodes.length;
        NodeEntity memory _node;
        string memory _rewardsAvailable = uint2str((globalRewardAmount -  nodes[0].lastClaimAmount).mul(nodes[0].rewardMult).mul(nodes[0].nodeValue).div(100000).div(1e18));
        string memory separator = "#";
        uint256 rewardAvailable = 0;
        ...
        for (uint256 i = 1; i < nodesCount; i++) {
            _node = nodes[i];
            rewardAvailable = globalRewardAmount - _node.lastClaimAmount;
            uint256 _totalReward = rewardAvailable.mul(_node.rewardMult).mul(_node.nodeValue).div(100000).div(1e18);
    ...
    // in case someone sends funds to the contract
    function rescueEther(uint256 weiAmount) external onlySentry{
        require(address(this).balance >= weiAmount, "insufficient ETH balance");
        payable(msg.sender).transfer(weiAmount);
    }

    function transferTokens(address _tokenAddr, address _to, uint _amount) public onlySentry {
...
contract SQUIDNODES is ERC20, Ownable, PaymentSplitter {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        address uniV2Router,
        uint256 snipeBlkAmt
    ) ERC20("SQUIDNODES", "SQN") PaymentSplitter(payees, shares) {

        treasuryPool = addresses[4];
        distributionPool = addresses[5];
        devPool = addresses[6];
        snipeBlockAmt = snipeBlkAmt;

        require(treasuryPool != address(0) && distributionPool != address(0) && devPool != address(0), "TREASURY, DEV & REWARD ADDRESS CANNOT BE ZERO");
    ...
    function updateSwapTokensAmount(uint256 newVal) external onlyOwner {
        require(swapTokensAmount >= 1, 'amount should be at least 1 >'); 
    ...
    function _transfer(
        ...
        if (from != owner() && to != uniswapV2Pair && to != address(uniswapV2Router) && to != address(this) && from != address(this)) {
            ...
            // anti whale
            if (to != treasuryPool && to != distributionPool && to != devPool && from != treasuryPool && from != distributionPool && from != devPool) {
                uint256 totalSupply = totalSupply();
                uint256 walletBalance = balanceOf(address(to));
                require(
                    amount.add(walletBalance) <= totalSupply.mul(maxTx).div(10000), 
                    "ANTI WHALE GREEN LIGHT RED LIGHT TRIGGERED RIP.");
            }
        }
    ...
    function createNodeWithTokens(string memory name, uint256  _initValue) public {
        ...
        require(
            sender != treasuryPool && sender != distributionPool && sender != devPool,
            "NODE CREATION: treasury, dev and rewardsPool cannot create node"
        );
        ...
        if (
            ...
            uint256 fdTokens = contractTokenBalance.mul(treasuryFee).div(100);
            uint256 devTokens = fdTokens.mul(devShare).div(100);
            uint256 treasuryTokens = fdTokens.sub(devTokens);
    ...
    function cashoutReward(uint256 blocktime) public {
        address sender = _msgSender();
        require(sender != address(0), "RED LIGHT: creation from the zero address");
        require(!_isBlacklisted[sender], "RED LIGHT: Blacklisted address");
        require(
            sender != treasuryPool && sender != distributionPool,
            "RED LIGHT: treasury and rewardsPool cannot cashout rewards"
        );
        ...
        require(
            rewardAmount > 0,
            "RED LIGHT: You don't have enough reward to cash out"
        );
    ...
    function cashoutAll() public {
        ...
        require(
            sender != address(0),
            "RED LIGHT: creation from the zero address"
        );
        require(!_isBlacklisted[sender], "RED LIGHT: Blacklisted address");
        require(
            sender != treasuryPool && sender != distributionPool,
    ...
    function addNodeValue(uint256 blocktime) public {
        ...
        require(
            sender != treasuryPool && sender != distributionPool && sender != devPool,
            "RED LIGHT: treasury, dev and rewardsPool cannot compound nodes"
        );
        uint256 rewardAmount = nodeRewardManager._getRewardAmountOf(
            sender,
            blocktime
        ...
        require(
            rewardAmount > 0,
            "RED LIGHT: You don't have enough reward to compound your node"
        );
    ...
    function addAllNodeValue() public {
        ...
        require(
            sender != treasuryPool && sender != distributionPool && sender != devPool,
            "RED LIGHT: treasury, dev and rewardsPool cannot cashout rewards"
        );
        uint256 rewardAmount = nodeRewardManager._getRewardAmountOf(sender);
        require(
            rewardAmount > 0,
            "RED LIGHT: You don't have enough reward to compound"
        );
        if (swapLiquify) {
            uint256 feeAmount;
            if (cashoutFee > 0) {
    ...
    // in case of swap and liquify issues
    function rescueEther(uint256 weiAmount) external onlyOwner{
        require(address(this).balance >= weiAmount, "insufficient ETH balance");
        payable(msg.sender).transfer(weiAmount);
    }

    function transferTokens(address _tokenAddr, address _to, uint _amount) public onlyOwner {
        require(_tokenAddr != address(this), "Cannot transfer out own tokens");
        IERC20(_tokenAddr).transfer(_to, _amount);
    }
    
    function updateMaxTxAmount(uint256 newVal) public onlyOwner {
        require(maxTx >= 1, 'maxTx should be at least 1 >');
        maxTx = newVal;
    }
