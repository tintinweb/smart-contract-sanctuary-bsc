...
import "./ISweeper.sol";
import "./funds/ISweepingFunds.sol";
import "./funds/SweepingFunds.sol";

contract MasterSweeper is Recoverable {
    ...
    constructor(address _wbnb, address _vault) {
    ...
    function sweep(
        ...
        _sweep(_market, _collection, _tokenId, _price);
    }
        ...
        onlyOwner
    {
        ...
        SweepingFunds funds = new SweepingFunds(WBNB, address(this), _collection);
     ...
     * @dev Create wallet for BNB to be used for sweeping `collection`.
     */
    function closeSweepingFunds(address _collection, address payable _receiver)
        external
        onlyOwner
    {
        require(_collection != address(0), "Invalid collection");
        ...
        ISweepingFunds(sweepingFunds[_collection]).closeFunds(_receiver);
    ...
    function isValidSigned(
        ...
        bytes32 msgHash = keccak256(
            abi.encodePacked(_market, _collection, _tokenId, _price, _timestamp)
        );
    ...
    function _sweep(
        ...
        IERC20(WBNB).approve(sweeper, _value);
        ISweeper(sweeper).sweep(_collection, _tokenId, _value, VAULT);
...
interface ISweeper {
    function sweep(address _collection, uint256 _tokenId, uint256 _price, address _to) external;
}
