...
import './interfaces/IAlfaPro.sol';
import './interfaces/IProCall.sol';

contract AlfaProV2 is ProMarketCreator, ProViewer, ProNoteKeeper, ProRewarder {
    using SafeERC20 for IERC20;

    event Bond(uint256 indexed id, uint256 amount, uint256 price);
    event Tuned(uint256 indexed id, uint256 oldControlVariable, uint256 newControlVariable);

    constructor(address _authority) ProMarketCreator() ProViewer() ProNoteKeeper() ProRewarder(IAlfaAuthority(_authority)) {}
        ...
        )
    {
        ...
        if ((term.fixedTerm && term.vesting == 0) || (!term.fixedTerm && term.vesting <= block.timestamp)) {
             ...
             */
            notes[_addresses[0]].push(Note({payout: payout_, created: uint48(block.timestamp), matured: uint48(expiry_), redeemed: 0, marketID: uint48(_id), token: address(market.baseToken)}));
        }
    ...
    function _tune(
        ...
        if (_time >= meta.lastTune + meta.tuneInterval) {
            ...
            uint256 capacity = market.capacityInQuote ? ((market.capacity * (10**(2 * meta.baseDecimals))) / _price) / (10**meta.quoteDecimals) : market.capacity;
...
interface IAlfaPro is IProMarketCreator, IProNoteKeeper, IProViewer {
    /**
     * @notice deposit quote tokens in exchange for a bond in a specified market
     */
    function deposit(
...
interface IProMarketCreator {
    ...
    struct Market {
        ...
        bool capacityInQuote; // capacity limit is in payment token (true) or in AFA (false, default)
...
abstract contract ProMarketCreator is IProMarketCreator {
    ...
    function create(
         ...
         */
        uint256 targetDebt = _booleans[0] ? ((_market[0] * (10**(2 * baseDecimals))) / _market[1]) / 10**quoteDecimals : _market[0];
        ...
        terms.push(Terms({fixedTerm: _booleans[1], controlVariable: controlVariable, vesting: uint48(_terms[0]), conclusion: uint48(_terms[1]), maxDebt: maxDebt}));
    }
...
import '../AlfaAccessControlled.sol';

import '../../interfaces/IERC20.sol';

abstract contract ProRewarder is AlfaAccessControlled {
    /* ========== STATE VARIABLES ========== */
    ...
    constructor(IAlfaAuthority _authority) AlfaAccessControlled(_authority) {}
...
abstract contract ProViewer is IProViewer, ProMarketCreator {
    ...
    function marketPrice(uint256 _id) public view override returns (uint256) {
        uint256 price = (currentControlVariable(_id) * currentDebt(_id)) / (10**metadata[_id].baseDecimals);
        return (price > markets[_id].minPrice) ? price : markets[_id].minPrice;
    }
    ...
    function payoutFor(uint256 _amount, uint256 _id) public view override returns (uint256) {
        Metadata memory meta = metadata[_id];
        return (_amount * 10**(2 * meta.baseDecimals)) / marketPrice(_id) / 10**meta.quoteDecimals;
    }
    ...
    function currentDebt(uint256 _id) public view override returns (uint256) {
        uint256 decay = (markets[_id].totalDebt * (block.timestamp - metadata[_id].lastDecay)) / metadata[_id].length;
    ...
    function maxIn(uint256 _id) public view returns (uint256) {
        Metadata memory meta = metadata[_id];
        return ((markets[_id].maxPayout * 10**meta.quoteDecimals * marketPrice(_id)) / 2) * (10**meta.baseDecimals);
    }
    ...
    function liveMarketsFor(
        ...
        if (_creator) {
            ...
            mkts = _base ? marketsForBase[_address] : marketsForQuote[_address];
        }
