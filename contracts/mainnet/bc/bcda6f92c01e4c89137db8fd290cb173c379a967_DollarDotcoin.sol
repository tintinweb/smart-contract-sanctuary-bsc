...
contract DollarDotcoin is Context, Ownable {

    using Math for uint256;
    address public OWNER_ADDRESS;
    bool private initialized = false;
    address BUSD = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    address public DEV_ADDRESS = 0xD4b77c5A1C7162aC4f2ac3876AB0879291bC22F5;
    address public MARKETING_ADDRESS = 0x13980b829141BC6BD0d6b3cF15d87336eebBE750;
    address public CEO_ADDRESS = 0xD4b77c5A1C7162aC4f2ac3876AB0879291bC22F5;
    address public GIVEAWAY_ADDRESS = 0x13980b829141BC6BD0d6b3cF15d87336eebBE750;
    ...
    uint136 BNB_PER_Dotcoin = 1000000000000;
    ...
    function buyDotcoin(address ref, uint256 _amount) public {
        require(initialized);
        ...
        uint256 DotcoinFrom = baker.Dotcoin;

        uint256 totalBnbFee = percentFromAmount(_amount, DEPOSIT_FEE);
        uint256 bnbValue = Math.sub(_amount, totalBnbFee);
        uint256 DotcoinBought = bnbToDotcoin(bnbValue);

        uint256 totalDotcoinBought = addDotcoin(baker.adr, DotcoinBought);
        baker.Dotcoin = totalDotcoinBought;
        ...
        if (
            ...
            if (hasInvested(baker.adr) == false) {
                uint256 refBonus = percentFromAmount(
                    bnbToDotcoin(_amount),
                    FIRST_DEPOSIT_REF_BONUS
                );
                upline.Dotcoin = addDotcoin(upline.adr, refBonus);
            }
        }
        ...
        emit EmitBoughtDotcoin(msg.sender, ref, _amount, DotcoinFrom, baker.Dotcoin);
    }
    ...
    function handleBake(bool onlyRebaking) private {
        ...
        if (onlyRebaking == true) {
            require(
                DotcoinToBnb(rewardedDotcoin(baker.adr)) >= MIN_BAKE,
                "Rewards must be equal or higher than 1 BUSD to bake"
            );
        }

        uint256 DotcoinFrom = baker.Dotcoin;
        uint256 DotcoinFromRewards = rewardedDotcoin(baker.adr);

        uint256 totalDotcoin = addDotcoin(baker.adr, DotcoinFromRewards);
        baker.Dotcoin = totalDotcoin;
        baker.bakedAt = block.timestamp;

        emit EmitBaked(msg.sender, baker.upline, DotcoinFrom, baker.Dotcoin);
    }
    ...
    function eat() public {
        ...
        uint256 DotcoinBeforeFee = rewardedDotcoin(baker.adr);
        uint256 DotcoinInBnbBeforeFee = DotcoinToBnb(DotcoinBeforeFee);

        uint256 totalBnbFee = percentFromAmount(
            DotcoinInBnbBeforeFee,
            WITHDRAWAL_FEE
        );

        uint256 bnbToEat = Math.sub(DotcoinInBnbBeforeFee, totalBnbFee);
        uint256 forGiveAway = calcGiveAwayAmount(baker.adr, bnbToEat);
        bnbToEat = addWithdrawalTaxes(baker.adr, bnbToEat);

        if (
            Math.add(DotcoinInBnbBeforeFee, baker.totalPayout) >=
            maxPayout(baker.adr)
        ) {
        ...
        emit EmitAte(msg.sender, bnbToEat, DotcoinBeforeFee);
    }
    ...
    function addWithdrawalTaxes(
        ...
        return
            percentFromAmount(
                bnbWithdrawalAmount,
                Math.sub(100, hasDotcoinTaxed(adr))
            );
    }
    ...
    function calcGiveAwayAmount(
        ...
        return (percentFromAmount(bnbWithdrawalAmount, hasDotcoinTaxed(adr)));
    }

    function hasDotcoinTaxed(address adr) public view returns (uint256) {
    ...
    function addDotcoin(
        address adr,
        uint256 DotcoinToAdd
    ) private view returns (uint256) {
        uint256 totalDotcoin = Math.add(bakers[adr].Dotcoin, DotcoinToAdd);
        uint256 maxDotcoin = bnbToDotcoin(MAX_WALLET_TVL_IN_BNB);
        if (totalDotcoin >= maxDotcoin) {
            return maxDotcoin;
        }
        return totalDotcoin;
    }
    ...
    function maxTvlReached(address adr) public view returns (bool) {
        return bakers[adr].Dotcoin >= bnbToDotcoin(MAX_WALLET_TVL_IN_BNB);
    }
    ...
    function bnbRewards(address adr) public view returns (uint256) {
        uint256 DotcoinRewarded = rewardedDotcoin(adr);
        uint256 bnbinWei = DotcoinToBnb(DotcoinRewarded);
        return bnbinWei;
    }

    function bnbTvl(address adr) public view returns (uint256) {
        uint256 bnbinWei = DotcoinToBnb(bakers[adr].Dotcoin);
        return bnbinWei;
    }

    function DotcoinToBnb(uint256 DotcoinToCalc) private view returns (uint256) {
        uint256 bnbInWei = Math.mul(DotcoinToCalc, BNB_PER_Dotcoin);
        return bnbInWei;
    }

    function bnbToDotcoin(uint256 bnbInWei) private view returns (uint256) {
        uint256 DotcoinFromBnb = Math.div(bnbInWei, BNB_PER_Dotcoin);
        return DotcoinFromBnb;
    }
    ...
    function rewardedDotcoin(address adr) private view returns (uint256) {
        uint256 secondsPassed = secondsSinceLastAction(adr);
        uint256 dailyRewardFactor = dailyReward(adr);
        uint256 DotcoinRewarded = calcDotcoinReward(
            secondsPassed,
            dailyRewardFactor,
            adr
        );

        if (DotcoinRewarded >= bnbToDotcoin(MAX_DAILY_REWARDS_IN_BNB)) {
            return bnbToDotcoin(MAX_DAILY_REWARDS_IN_BNB);
        }

        return DotcoinRewarded;
    }

    function calcDotcoinReward(
        uint256 secondsPassed,
        uint256 dailyRewardFactor,
        address adr
    ) private view returns (uint256) {
        uint256 rewardsPerDay = percentFromAmount(
            Math.mul(bakers[adr].Dotcoin, 100000000),
            dailyRewardFactor
        );
        uint256 rewardsPerSecond = Math.div(rewardsPerDay, SECONDS_PER_DAY);
        uint256 DotcoinRewarded = Math.mul(rewardsPerSecond, secondsPassed);
        DotcoinRewarded = Math.div(DotcoinRewarded, 1000000000000);
        return DotcoinRewarded;
    }
