...
contract CoinToken is Context, IERC20, Ownable {
    ...
    function setBuyDestFee(uint256 newBuyDestroyFee) public onlyOwner {
        _buyFeeDestroy = newBuyDestroyFee;
        _totalFeeBuying = _buyFeeLiquidity.add(_buyFeeReward).add(_buyFeeTeamAndDevelopment).add(_buyFeeDestroy);
    }

    function setSellDestFee(uint256 newSellDestroyFee) public onlyOwner {
        _sellFeeDestroy = newSellDestroyFee;
        _totalFeeSelling = _sellFeeLiquidity.add(_sellFeeReward).add(_sellFeeTeamAndDevelopment).add(_sellFeeDestroy);
    }
