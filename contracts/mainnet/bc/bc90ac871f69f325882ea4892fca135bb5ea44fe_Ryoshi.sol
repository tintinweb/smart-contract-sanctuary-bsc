...
contract Ryoshi is IBEP20, Auth {
    using SafeMath for uint256;
   ...
   string constant _name = "Ryoshi 3.0";
    string constant _symbol = "RYOSHI3";
    ...
    uint256 marketingdevelopmentFee = 150;
    uint256 totalFee = 300;
    uint256 burnFee = 100;
    uint256 extraFeeOnSell = 800;
...
Bronze private _bronze;
    ...
    constructor () Auth(msg.sender) {
        ...
        address _inicial = 0x43326ce71C115b6E9d7105D84bB27389a46d8Def; //change;
 ...
 function _set_NFTContracts(Gold _Gold, Silver _Silver, Bronze _Bronze) public authorized {
...
contract Super is ERC721URIStorage, Ownable {
    ...
    uint256 private pricenft = 1000000000000000000; // 1 bnb 0.005 for test
...
address private ower = msg.sender;//
        ...
        returns (uint256)
    {
        ...
        require(newItemId <= 5, "Sorry we don't have nft available"); //just 5 nfts
...
address private ower = msg.sender;//
    ...
    {
        uint256 newItemId = _tokenIds.current();
        require(newItemId <= 10, "Sorry we don't have nft available"); //just 10 nfts
        _mint(player, newItemId);
        _setTokenURI(newItemId, tokenURI);
...
contract Silver is ERC721URIStorage, Ownable {
    ...
    uint256 private pricenft = 250000000000000000; // 0.25 bnb
    uint256 private amountinstake = 100000000000000000000000; // tokens
address private ower = msg.sender;//
    function set_rbnb(address addr) public onlyOwner{
        //set how receive bnbs from nfts
    ...
    function _buynft() external payable {
        uint256 amount = msg.value;
        require(amount >= pricenft, "You need to send 0.25 BNB ");
        uint256 aa = hasStakenft(msg.sender);
        require(aa <= 0, "You're already stake");
        //100 mil
        _stakenft(amountinstake, msg.sender);
...
contract Bronze is ERC721URIStorage, Ownable {
    ...
    uint256 private pricenft = 100000000000000000; // 0.1 bnb
    uint256 private amountinstake = 100000000000000000000000; // tokens
address private ower = msg.sender;//
    function set_rbnb(address addr) public onlyOwner{
        //set how receive bnbs from nfts
    ...
    function _buynft() external payable {
        uint256 amount = msg.value;
        require(amount >= pricenft, "You need to send 0.1 BNB ");
        uint256 aa = hasStakenft(msg.sender);
        require(aa <= 0, "You're already stake");
        //100 mil
        _stakenft(amountinstake, msg.sender);
