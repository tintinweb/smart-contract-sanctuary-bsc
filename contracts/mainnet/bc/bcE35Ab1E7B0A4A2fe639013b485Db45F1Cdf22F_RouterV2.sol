...
contract RouterV2 {
    ...
    function addLiquidityBNB(
        ...
        if (msg.value > amountBNB) _safeTransferBNB(msg.sender, msg.value - amountBNB);
    }
    ...
    function removeLiquidityBNBWithPermit(
        ...
        uint value = approveMax ? type(uint).max : liquidity;
        IBaseV1Pair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
        (amountToken, amountBNB) = removeLiquidityBNB(token, stable, liquidity, amountTokenMin, amountBNBMin, to, deadline);
    }
    ...
    function swapExactBNBForTokens(uint amountOutMin, route[] calldata routes, address to, uint deadline)
    external
    payable
    ensure(deadline)
    returns (uint[] memory amounts)
    {
        require(routes[0].from == address(wBNB), 'BaseV1Router: INVALID_PATH');
        amounts = getAmountsOut(msg.value, routes);
        require(amounts[amounts.length - 1] >= amountOutMin, 'BaseV1Router: INSUFFICIENT_OUTPUT_AMOUNT');
        wBNB.deposit{value: amounts[0]}();
        assert(wBNB.transfer(pairFor(routes[0].from, routes[0].to, routes[0].stable), amounts[0]));
        _swap(amounts, routes, to);
    }

    function swapExactTokensForBNB(uint amountIn, uint amountOutMin, route[] calldata routes, address to, uint deadline)
    external
    ensure(deadline)
    returns (uint[] memory amounts)
    {
        require(routes[routes.length - 1].to == address(wBNB), 'BaseV1Router: INVALID_PATH');
        amounts = getAmountsOut(amountIn, routes);
        require(amounts[amounts.length - 1] >= amountOutMin, 'BaseV1Router: INSUFFICIENT_OUTPUT_AMOUNT');
        _safeTransferFrom(
            routes[0].from, msg.sender, pairFor(routes[0].from, routes[0].to, routes[0].stable), amounts[0]
    ...
    function _safeTransferBNB(address to, uint value) internal {
    ...
    // Experimental Extension [BNB.guru/solidly/BaseV1Router02]
    ...
    function removeLiquidityBNBWithPermitSupportingFeeOnTransferTokens(
        ...
        uint value = approveMax ? type(uint).max : liquidity;
        IBaseV1Pair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
        (amountToken, amountBNB) = removeLiquidityBNBSupportingFeeOnTransferTokens(
            token, stable, liquidity, amountTokenMin, amountBNBMin, to, deadline
        );
    }
        ...
        ensure(deadline)
    {
        ...
        assert(wBNB.transfer(pairFor(routes[0].from, routes[0].to, routes[0].stable), amountIn));
        uint balanceBefore = erc20(routes[routes.length - 1].to).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(routes, to);
        require(
            erc20(routes[routes.length - 1].to).balanceOf(to).sub(balanceBefore) >= amountOutMin,
