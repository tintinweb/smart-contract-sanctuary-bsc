...
contract HYPE is Context, IERC20, Ownable {
    ...
    bool    public  maxGweiEnabled                   = true;
    uint256 public  maxGwei                          = 7 gwei;
    uint256 public  maxSellGwei                      = 7 gwei; 
    ...
    uint256 private _tmpBuyGwei                      = 0;
    uint256 private _tmpSellGwei                     = 0;
    ...
    bool    private _tmpMaxGweiEnabled               = false;
    ...
    function _transfer(
        ...
        // MAX GWEI AGAINST FRONT RUNNERS
    ...
    // CHECKS MAX GWEI AGAINST FRONT RUNNERS
    ...
    function setMaxGwei(uint256 _buyMaxGwei, uint256 _sellMaxGwei, bool _enabled) external {
        // MULTI-SIGNATURE ID
        uint256 id = 18;
        // LOCAL REQUIREMENTS
        require(_buyMaxGwei  >= 5 && _buyMaxGwei  <= 10, "MAX_BUY_GWEI should be reasonable" );
        require(_sellMaxGwei >= 5 && _sellMaxGwei <= 10, "MAX_SELL_GWEI should be reasonable");
        // GLOBAL REQUIREMENTS   
        checkAuth(msg.sender); 
        if (ZERO == multiSignatureAddress) {
            // SETTING UP MULTI-SIGNATURE
            multiSignatureTrigger(id, msg.sender);
            _tmpBuyGwei        = _buyMaxGwei;
            _tmpSellGwei       = _sellMaxGwei;
            _tmpMaxGweiEnabled = _enabled;
        }
        else {
            // GLOBAL MULTI-SIGNATURE REQUIREMENTS
            multiSignatureRequirements(id, msg.sender, true);
            // LOCAL MULTI-SIGNATURE REQUIREMENTS
            require(_buyMaxGwei == _tmpBuyGwei 
                 && _tmpSellGwei == _sellMaxGwei 
                 && _tmpMaxGweiEnabled == _enabled, "Invalid parameters");
            // NICE JOB. YOU DID IT!
            maxGwei        = _buyMaxGwei  ** 10 * 9;
            maxSellGwei    = _sellMaxGwei  ** 10 * 9;
            maxGweiEnabled = _enabled;
            // RESET AFTER SUCCESSFULLY COMPLETING TASK
            resetMultiSignature();
        }
    }
