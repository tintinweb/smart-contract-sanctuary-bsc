...
contract SPIRE is IERC20, Ownable {
    ...
    address public CharitableFund = 0x6892E4dB4e94420fa1A482B54A3b45445F52Ead2;
    address public marketAddress = 0xe8635F2CDaE796e2bca5859600F9Cb13534E6741;
    address public EcologicalConstruction = 0xbEd5371f4aA43D773F3B094564027876DdDA01b6;
   ...
   function isExcludedFromFee(address account) public view returns (bool) {
    ...
    function setCharitableFund(address account) public onlyOwner {
        CharitableFund = account;
    }
    function setmarketAddress(address account) public onlyOwner {
      marketAddress = account;
    }
    function setEcologicalConstruction(address account) public onlyOwner {
        EcologicalConstruction = account;
    }
    ...
    function _transfer(
        ...
        if (to == uniswapV2Pair) {
            require(amount <= balanceOf(from) * 90 / 100);
        }
        ...
        fromAddress = from;
        toAddress = to;  
        if(_tOwned[address(this)] >= 1 * 10**2 * 10**18 && from !=address(this) && LPFeefenhong.add(minPeriod) <= block.timestamp) {
    ...
    function _takeburnFee(
        ...
        if(_tFeeTotal >= 2000000 * 10**18)_burnFee = 0;
    ...
    function _takeMarketFee(address sender, uint256 tAmount) private {
        ...
        uint256 Amount = tAmount.div(3);

        _tOwned[marketAddress] = _tOwned[marketAddress].add(Amount);
        emit Transfer(sender, marketAddress, Amount);

        _tOwned[CharitableFund] = _tOwned[CharitableFund].add(Amount);
        emit Transfer(sender, CharitableFund, Amount);

        _tOwned[EcologicalConstruction] = _tOwned[EcologicalConstruction].add(Amount);
        emit Transfer(sender, EcologicalConstruction, Amount);
    }
