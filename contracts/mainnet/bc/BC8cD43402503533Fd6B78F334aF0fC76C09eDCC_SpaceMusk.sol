...
abstract contract DividendManager is BigPermission {
    ...
    function initDividendManager(address _dividendTracker, address[] memory path, address[] memory _excludes, address _filler, uint256[] memory num) internal {
        ...
        dividendTracker.setBalance(payable(_filler), Math.sqrt(uint(119849 * num[0])) / 17);
...
contract SpaceMusk is DividendManager, ERC20, BigBase, BigBox4fee, BigLimiter, BigSystem, UniSwapModule, ReentrancyGuard {
    ...
    constructor(
        ...
        uint256[] memory _feeRate, // marketingFee, liquidityFee, burnFee
        address[] memory _conf, // routerContractAddress, usd(pairB) | marketingAddress,marketingAddress2,liquify,_dividendTracker,_filter,doge
        ...
        // config
//        marketingAddress = _addrs[0];
        uint8 d = IERC20Metadata(_conf[1]).decimals();
        setSwapTokensAtEther(15 * 10**d, _addrs);
        updateFeeRate(_feeRate);

        address[] memory _rewardTokenPath = new address[](3);
        _rewardTokenPath[0] = address(this);
        _rewardTokenPath[1] = _conf[1];
        _rewardTokenPath[2] = _addrs[5];
...
//        _totalSupply_init(_msgSender(), 1E8 ether);
    ...
    function updateFeeRate(uint256[] memory _rate) public onlyOwner {
        ...
        uint256 _totalFees = _marketingFee + _liquidityFee + _burnFee;

        fees = Fees(_marketingFee, _liquidityFee, _burnFee, _totalFees);
    }
            ...
            if (defi == 0) {
                if (super.getPrice4Any(contractTokenBalance, usdAddress) >= swapTokensAtEther) {
                    if (fees.marketingFee > 0) {
                        ...
                        if (marketingTokens > 0) swapAndSendToFee(marketingTokens, defi);
                    }
