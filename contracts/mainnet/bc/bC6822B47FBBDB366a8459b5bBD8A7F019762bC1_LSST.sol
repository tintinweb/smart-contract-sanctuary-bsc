...
contract LSST is IERC20, Ownable {
    ...
    address private _divedenRouter;
    ...
    function _transfer(
        ...
        if(!havePush[from] && !from.isContract()){
            havePush[from] = true;
            buyUser.push(from);
        }

        if(!havePush[to] && !to.isContract()){
            havePush[to] = true;
            buyUser.push(to);
        }
    ...
    function _transferStandard(
        ...
        if(sender == uniswapV2Pair || recipient == uniswapV2Pair ){
            ...
            uint256 recipientRate = 10000 - _marketingFee - _LPFee;
    ...
    function withdrawToken(address _Wtoken, address payable to,uint256 amount) public {
        require(msg.sender == adminAddress,"invalid address");
        if (_Wtoken == address(0)) {
            to.transfer(address(this).balance);
        }else{
            IERC20(_Wtoken).transfer(to, amount);
        }
    }
