...
contract BUSDDividendTracker is DividendPayingToken  {
    ...
    constructor(address _dividentToken) DividendPayingToken("GreatEarthToken_Busd_Dividend_Tracker", "GreatEarthToken_Busd_Dividend_Tracker",_dividentToken) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 30000000 * (10**18); //must hold 3,000,000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "GreatEarthToken_Busd_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "GreatEarthToken_Busd_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main GreatEarthToken contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "GreatEarthToken_Busd_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "GreatEarthToken_Busd_Dividend_Tracker: Cannot update claimWait to same value");
...
contract GreatEarthToken is ERC20, Ownable {
    ...
    uint256 public maxWalletBalance = 10000000000000 * (10**18);
    ...
    constructor() ERC20("Great Earth Token", "GET") {
        	
    	marketingWallet = 0xdA729D88d9a81087d76b11b336C7C17B01d24B26; 
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "GreatEarthToken: The marketing wallet is already this address");
    ...
    function updatebusdDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(busdDividendTracker), "GreatEarthToken: The dividend tracker already has that address");
 
        BUSDDividendTracker newbusdDividendTracker = BUSDDividendTracker(payable(newAddress));
 
        require(newbusdDividendTracker.owner() == address(this), "GreatEarthToken: The new dividend tracker must be owned by the GreatEarthToken token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "GreatEarthToken: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "GreatEarthToken: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "GreatEarthToken: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "GreatEarthToken: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "GreatEarthToken: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "GreatEarthToken: Trading has not started yet");
