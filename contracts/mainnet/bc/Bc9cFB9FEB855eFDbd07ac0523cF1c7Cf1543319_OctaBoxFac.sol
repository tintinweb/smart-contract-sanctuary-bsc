...
contract OctaBoxFac is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, AccessControlUpgradeable {
  ...
  struct BoxData {
    uint256 boxId;
    uint256 boxType;  // random -> 1 common, 2 rare, 3 epic, 4 legendary, 5 ultimate black / character / option
    uint256 boxOpt;
  }
  ...
  function getBoxDataById(uint256 _boxId) external view returns (uint256[2] memory _boxData) {
    _boxData[0] = boxDatas[_boxId].boxType;
    _boxData[1] = boxDatas[_boxId].boxOpt;
  }
...
interface IOctaWarrior {

  function born(address _owner, uint256 warrId, uint256[3] memory warrDet) external;
