...
// File: contracts/sacredtails/ITier.sol
...
abstract contract ITier{
    struct Tier{
        uint8 divId; // the divisionId of each Tier should be unique
        ...
        uint32 limit; // the monetized step limit associated with each tier
	}
    ...
    function getTierById(uint8 _id) public view virtual returns (Tier memory);
...
// File: contracts/sacredtails/MysteryBox.sol
...
contract MysteryBox is OwnableUpgradeable {
    ...
    function initialize(address _tier) public initializer {
        tier = ITier(_tier);
        __Ownable_init();
    }

    function updateTier(address _tier) external onlyOwner
    {
        tier = ITier(_tier);
    }
        ...
        external
    {
        ...
        require(admin[msg.sender] || owner() == msg.sender, "not whitelisted");
        ...
        box.whitelistOnly = whitelistOnly;
    ...
    function openBox(
        ...
        payable
    {
        require(msg.sender == tx.origin, "Sender Only");
        
        Box storage box = box_by_id[box_id];
        uint32 cap = tier.getTier(msg.sender).limit;
       ...
       // require((box.purchased_nft[msg.sender].length + amount) <= box.personal_limit, "exceeds personal limit");
        require((box.purchased_nft[msg.sender].length + amount) <= cap, "exceeds personal limit");
        require(!(box.canceled), "sale canceled");
        if(box.whitelistOnly)
        {
        ...
        uint256[] memory token_ids = new uint256[](amount);
        ...
        for (uint256 i = 0; i < amount; i++) {
            ...
            require(creator == IERC721(nft_address).ownerOf(token_id),"Not Owner");
            // transfer NFT
        ...
        uint256 amount = _partialFee(msg.sender,payment_token_address,total_payment,code);
        
        IERC20(payment_token_address).transferFrom(msg.sender, address(this), amount);
        
        box.payment[payment_token_index].receivable_amount += total_payment;
        
        emit OpenSuccess(box_id, msg.sender, nft_address, amount,token_ids);
    }


    function _partialFee(address from,address token,uint256 amount,string memory code) private returns (uint256) {
        return amount.sub(_calculateFeeAmount(from,token,amount,code));
    }

    function _calculateFeeAmount(address from,address payment_token_address,uint256 amount,string memory code) private returns (uint256) {
        uint256 totalFeeAmount = 0;
        // Check the balance of the smart contract before making the swap
        if(codes[code] != address(0))
        {
             totalFeeAmount  = (amount.mul(10)).div(100);
             used[code] += 1;
             IERC20(payment_token_address).transferFrom(from, codes[code],totalFeeAmount.div(2));
        }
        ...
        )
    {
        ...
        name = box.name;
        personal_limit = tier.getTier(msg.sender).limit;
    ...
    function addReferral(string memory code) external {
        require(codes[code] == address(0),"code already used");
        codes[code] = msg.sender;
    }

    function checkReferral(string memory code) external view returns(bool){
        return codes[code] != address(0);
    }
