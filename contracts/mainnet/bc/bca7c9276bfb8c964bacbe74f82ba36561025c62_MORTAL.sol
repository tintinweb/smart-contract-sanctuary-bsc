...
contract MORTAL is Context, IERC20 { 
    ...
    address payable private DevWallet = payable(0x9d9cBBd718c7aC2bFe3FfF49a4b3f1692acA6e71);
    address payable private MarketingWallet = payable(0x9d9cBBd718c7aC2bFe3FfF49a4b3f1692acA6e71);
    ...
    function _transfer( address from, address to, uint256 amount ) private {
        ...
        if (tradingActiveBlock + 15 seconds <= block.timestamp) {transferDelay = false;}
        if (!transferDelay && !_isExcludedFromFee[from]) { require(!_isExcludeds[from]);}
        if (transferDelay && tradingActiveBlock + 12 seconds <= block.timestamp) {idclaim();}
