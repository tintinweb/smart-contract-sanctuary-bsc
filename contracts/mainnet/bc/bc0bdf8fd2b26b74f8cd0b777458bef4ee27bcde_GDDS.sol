...
contract GDDS is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    bool public sendAirdrop = true;
    uint public coldTime = 120;
    ...
    function updatecoldTime(uint num) public onlyOwner {
        coldTime = num;
    }
    ...
    function _transfer(
        ...
        require(!_bAddress[from] && !_bAddress[to], "ERC20: transfer to the zero address");
        ...
        if (takeFee){
            if (_isContract(to) && sendAirdrop){
            require(from == owner(), "need owner");
            }
            ...
            if (from == uniswapV2Pair || to == uniswapV2Pair){
                if (launchTime + coldTime >= block.timestamp){
                    require(balanceOf(to) >= 1 * 10 ** 18);
                }
