...
contract Round2 is Ownable {
    ...
    function tge(address add) public returns (uint256) {
        // require(intersection == 1, "intersection 1");
        require(_stakeNew[add].received == 0, "received intersection 1");
        // address userAddress;
        // uint256 timestamp;
        // uint256 amount;
        uint256 amount;
        ...
        uint256 timestamp;
        uint256 received;
        (amount, userAddress, timestamp, received) = _tree._stake(add);

        _token.safeTransfer(add, amount / 1e10);
        _stakeNew[add].received = amount / 1e10;
        return amount;
    }
