...
// File: contracts/SynoptiLock.sol
...
/**
 * @title SynoptiVesting
 */
contract SynoptiVesting is AccessControl {
    ...
    constructor(address token, uint256 start, uint256 duration, uint256 releasesCount, bool revocable) {
        require(token != address(0), "SynoptiVesting: token is the zero address!");
        require(duration > 0, "SynoptiVesting: duration is 0!");
        require(releasesCount > 0, "SynoptiVesting: releases count is 0!");
        require(block.timestamp <= start, "SynoptiVesting: TIMESTAMP_INVALID");
  ...
  /****************************|
    ...
    */
    ...
    function createVestingSchedule(
        ...
        require(_beneficiary != address(0), "SynoptiVesting: ZERO_ADDRESS");
        require(_totalAmount > 0, "SynoptiVesting: ZERO_AMOUNT");

        VestingSchedule storage Schedule = _vestingSchedules[_beneficiary];
        require(Schedule.totalAmount == 0, "SynoptiVesting: ADDRESS_ALREADY_INITIALIZED");
     ...
     */
    function claim() public onlyIfNotRevoked(msg.sender){
        VestingSchedule storage Schedule = _vestingSchedules[msg.sender];
        require(
            msg.sender == Schedule.beneficiary,
            "SynoptiVesting: NOT_BENEFICIARY"
        );
        uint256 unreleased = _releasableAmount(msg.sender, Schedule.releasedAmount );
        require(unreleased > 0, "SynoptiVesting: NO_RELEASABLE_AMOUNT");
        uint256 balance = _token.balanceOf(address(this));
        require(balance >= unreleased, "SynoptiVesting: INSUFFICIENT_BALANCE");
    ...
    {
        require(_revocable, "SynoptiVesting: VESTING_IRREVOCABLE");
        VestingSchedule storage Schedule = _vestingSchedules[_beneficiary];
