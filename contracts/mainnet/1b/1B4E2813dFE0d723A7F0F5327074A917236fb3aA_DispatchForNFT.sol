...
contract DispatchForNFT {
    ...
    constructor(address stakedToken_, address rewardToken_) {
        ...
        stakedToken = IERC721(stakedToken_);
    }
    ...
    function stake(uint256 tokenId) external updateDispatch(msg.sender) {
        require(_balances[msg.sender] == 0, "already a node");
    ...
    function withdraw() external updateDispatch(msg.sender) {
        _totalSupply = _totalSupply.sub(1);
        _balances[msg.sender] = _balances[msg.sender].sub(1);
        uint256 tokenId = stakedTokenId[msg.sender];
        stakedTokenId[msg.sender] = 0;
        stakedToken.transferFrom(address(this), msg.sender, tokenId);
