...
uint128 constant USDT_PRICE = 0.006 * 10 ** 18; // price of 1 UTILIS in USDT in 18 decimals (1 utilis = 0.01 USDT)
uint128 constant MAX_BUY_IN_UTILIS = 500000 * 10 ** 18; // max buy per user in UTILIS
uint128 constant MIN_BUY_IN_UTILIS = 8333 * 10 ** 18; // minimum buy in utilis token
...
// These are for BSC no need to change
...
address constant UTILIS = 0xB130A92284bEAF759479C4E5eaa9252331F88567;
...
contract UtilisPreLaunch is Ownable {
    ...
    IERC20 public constant utilisToken = IERC20(UTILIS);
    IERC20 public constant usdt = IERC20(USDT);
    IERC20 public constant wbnb = IERC20(WBNB);
    IUniswapV2Router02 public constant uniswapRouter = IUniswapV2Router02(PANCAKE_ROUTER);
    IUniswapV2Pair public constant usdtWbnbPair = IUniswapV2Pair(USDT_WBNB_PAIR);
    ...
    function setStartTime(uint64 _startTime) external onlyOwner {
        require(
            startTime == 0 || (startTime > block.timestamp && _startTime > block.timestamp),
