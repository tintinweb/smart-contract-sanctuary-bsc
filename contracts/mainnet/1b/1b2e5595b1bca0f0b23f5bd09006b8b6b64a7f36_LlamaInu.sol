...
Social Media:
 ...
 Website: www.llamainu.xyz
Telegram: https://t.me/LlamaINU
 Twitter: https://twitter.com/llamainu
...
library Address{
 ...
 contract LlamaInu is Context, IERC20, Ownable {
    ...
    address public marketingWallet = 0xb441e59d4Bbf379Ce2a990c6D737e8cb3Ab87fEA;
    address public LlamaWallet = 0xb38Ba6d0b2B1ED82D2a5bF1cbB257dFeF9b19Ed5;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[LlamaWallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[marketingWallet] = true;
        allowedTransfer[LlamaWallet] = true;
    ...
    function setTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _Llama) public onlyOwner {
       taxes = Taxes(_rfi,_marketing,_liquidity,_Llama);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _Llama) public onlyOwner {
       sellTaxes = Taxes(_rfi,_marketing,_liquidity,_Llama);
        emit FeesChanged();
    }
    ...
    function _takeLlama(uint256 rLlama, uint256 tLlama) private {
        totFeesPaid.Llama +=tLlama;

        if(_isExcluded[address(this)])
        {
            _tOwned[address(this)]+=tLlama;
        }
        _rOwned[address(this)] +=rLlama;
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rLlama) = _getRValues2(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.tLlama = tAmount*temp.Llama/100;
        s.tTransferAmount = tAmount-s.tRfi-s.tMarketing-s.tLiquidity-s.tLlama;
        return s;
    }
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rMarketing, uint256 rLiquidity){
        ...
        uint256 rLlama = s.tLlama*currentRate;
        rTransferAmount =  rAmount-rRfi-rMarketing-rLiquidity-rLlama;
        return (rAmount, rTransferAmount, rRfi,rMarketing,rLiquidity);
    }
    
    function _getRValues2(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 rLlama) {

        if(!takeFee) {
          return(0);
        }

        rLlama = s.tLlama*currentRate;
        return (rLlama);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tMarketing + s.tLlama);
        }
        if(s.rMarketing > 0 || s.tMarketing > 0) _takeMarketing(s.rMarketing, s.tMarketing);
        if(s.rLlama > 0 || s.tLlama > 0) _takeLlama(s.rLlama, s.tLlama);
        emit Transfer(sender, recipient, s.tTransferAmount);
        
    }

    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.marketing + temp.Llama) * 2;
        ...
        uint256 LlamaAmt = unitBalance * 2 * temp.Llama;
        if(LlamaAmt > 0){
            payable(LlamaWallet).sendValue(LlamaAmt);
        }
    }
    ...
    function updateLlamaWallet(address newWallet) external onlyOwner{
        LlamaWallet = newWallet;
    }
