...
TG: https://t.me/Official_ShirojiINu
WEB: https://shirojiinu.com/index.html
*/
...
contract ShirojiINU is Context, IBEP20, Ownable {
    ...
    uint256 private _tTotal = 100000 * 10 ** 12;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    uint256 private maxWallet;

    string private _name = "Shiroji INU";
    string private _symbol = "SHIROJI";
    ...
    uint256 public  _liq_fee = 0; // ZERO tax for transfering tokens

    // buy fee
    uint256 public  _taxFeeBuy       = 5;
    uint256 public  _liq_feeBuy = 0; //

    uint256 public  _taxFeeSell       = 6;
    uint256 public  _liq_feeSell = 0;
    ...
    constructor () {
        ...
        pair = IUniswapV2Factory(_uniswaprouter.factory())
        ...
        _isExcludedFromAutoLiquidity[address(uniswaprouter)] = true;

        _isExcludedFromAntiWhale[owner()]                  = true;
        _isExcludedFromAntiWhale[address(this)]            = true;
        _isExcludedFromAntiWhale[pair]            = true;
        _isExcludedFromAntiWhale[address(uniswaprouter)] = true;
...
//Remove from Blacklist
    ...
    function setWalletLimit(uint256 value) external onlyOwner {
        maxWallet = value;
    }
    ...
    function setExcludedFromMaxTxLimit(address account, bool e) external onlyOwner {
    ...
    function setFeesTransfer(uint taxFee, uint liq_fee) external onlyOwner {
        _taxFee       = taxFee;
        _liq_fee = liq_fee;
    }

    function setFeesBuy(uint taxFee, uint liq_fee) external onlyOwner {
        _taxFeeBuy       = taxFee;
        _liq_feeBuy = liq_fee;
    }

    function setFeesSell(uint taxFee, uint liq_fee) external onlyOwner {
        _taxFeeSell       = taxFee;
        _liq_feeSell = liq_fee;
    }
    ...
    function setLiquidityPercentages(uint256 teamFee, uint256 marketingFeeVariable) external onlyOwner {
        _percentageOfLiquidityForTeam        = teamFee;
        _percentageOfLiquidityForMarketing  = marketingFeeVariable;
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tLiquidity = calculateFee(tAmount, _liq_fee);
    ...
    function _transfer(
        ...
        if (from == pair && to != address(uniswaprouter)) {
    ...
    function addLiquidity(uint256 tokenAmount, uint256 bnbAmount) private returns (uint256, uint256) {
        ...
        (uint amountToken, uint amountETH, ) = uniswaprouter.addLiquidityETH{value: bnbAmount}(
            address(this),
            tokenAmount,
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        uint256 previousLiquidityFee = _liq_fee;
        uint256 block_factor = block.number - cooldown[sender];

        bool isBuy  = sender == pair && recipient != address(uniswaprouter);
