...
contract NftCardMarket is ReentrancyGuard {
    ...
    function createMarketSale(
        ...
        IERC20 tokenTTC=IERC20(_cardNftContract.getTokenAddress());
        uint256 total=fee+price;

 
        //购买者余额要大于商品的价格
        require(token.balanceOf(msg.sender)>= price, "Please submit the asking price in order to complete the purchase"); // 如果要价不满足会不会产生误差
       
        //验证是否授权
        require(tokenTTC.allowance(msg.sender,address(this))>=fee,'Insufficient authorized balance For TTC');
        require(token.allowance(msg.sender,address(this))>=price,'Insufficient authorized balance For ETI');

        require(fee>0,'fee set failed') ;       
        
        require(tokenTTC.transferFrom(msg.sender,feeAddress,fee),'TTC transfer fee failed') ;
        require(token.transferFrom(msg.sender,idToMarketItem[itemId].seller,price),'ETI transfer price failed') ;
