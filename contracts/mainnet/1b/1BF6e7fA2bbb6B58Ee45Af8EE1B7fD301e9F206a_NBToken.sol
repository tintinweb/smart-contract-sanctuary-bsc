...
contract NBToken is ERC20, Ownable {
    ...
    uint256 public swapTokensAtAmount = 2 * 1e18;
    ...
    function setTokenDividendAddress(address _token) external onlyOwner {
        dividendTracker = TokenDividendTracker(_token);
    }
    ...
    function setMaxBuyAmount(uint256 _maxBuyAmount) external onlyOwner {
        maxTxBuyAmount = _maxBuyAmount;
    }

    function setMaxSellAmount(uint256 _maxSellAmount) external onlyOwner {
        maxTxSellAmount = _maxSellAmount;
    }

    function setBuyFee(uint256 _lFee, uint256 _mFee, uint256 _rFee) external onlyOwner {
        buyRewardFee = _rFee;
        buyMarketingFee = _mFee;
        buyLiquidityFee = _lFee;
    }

    function setSellFee(uint256 _lFee, uint256 _mFee, uint256 _rFee, uint256 _nFee) external onlyOwner {
        sellRewardFee = _rFee;
    ...
    function _transfer(
        ...
        if (canSwap &&
            ...
            if (AmountNftFee > swapTokensAtAmount && _nftAddress != address(0)) {
                swapAndSendToNft(AmountNftFee);
            }
            if (AmountRewardFee > swapTokensAtAmount) {
                swapAndSendDividends(AmountRewardFee);
            }
            swapping = false;
        }
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[from]) {
                if (maxTxBuyAmount > 0) {
                    require(amount <= maxTxBuyAmount, "Transfer amount exceeds the maxBuyAmount");
                }
                LFee = amount.mul(buyLiquidityFee).div(1e4);
                AmountLiquidityFee += LFee;
                MFee = amount.mul(buyMarketingFee).div(1e4);
                AmountMarketingFee += MFee;
                DFee = amount.mul(buyRewardFee).div(1e4);
                AmountRewardFee += DFee;
            }
            if (automatedMarketMakerPairs[to]) {
                if (maxTxSellAmount > 0) {
                    require(amount <= maxTxSellAmount, "Transfer amount exceeds the maxSellAmount");
                }
                LFee = amount.mul(sellLiquidityFee).div(1e4);
                AmountLiquidityFee += LFee;
                MFee = amount.mul(sellMarketingFee).div(1e4);
                AmountMarketingFee += MFee;
                DFee = amount.mul(sellRewardFee).div(1e4);
                AmountRewardFee += DFee;
                if (_nftAddress != address(0) && sellNftFee > 0) {
                    NFee = amount.mul(sellNftFee).div(1e4);
                    AmountNftFee += NFee;
                }
            }
            fees = LFee.add(MFee).add(DFee).add(NFee);
            if (fees > 0) {
    ...
    function swapAndSendToFee(uint256 tokens) private {
        super._transfer(address(this), address(_marketingWalletAddress), tokens);
        AmountMarketingFee = AmountMarketingFee - tokens;
    }

    function swapAndSendToNft(uint256 tokens) private {
        super._transfer(address(this), address(_nftAddress), tokens);
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        AmountRewardFee = AmountRewardFee - tokens;
