...
contract SAPPNftPool is ISAPPNftPool, Ownable, ReentrancyGuard {
    ISAPPNftMarketplace public SAPPMKT;
    using Address for address;

    mapping(address => mapping(address => uint256)) public amountWithdrawn;

    constructor(address _SAPPNftMarketplace, address _owner) {
        SAPPMKT = ISAPPNftMarketplace(_SAPPNftMarketplace);
    ...
    modifier onlyOwnerOrMarketplace() {
        require(
            _msgSender() == owner() || _msgSender() == address(SAPPMKT),
            "Already inprocess"
        );
        _;
    }
    ...
    function getAmountEarn(address _user, address _tokenAddress)
        ...
        returns (uint256)
    {
        return SAPPMKT.getAmountEarn(_user, _tokenAddress);
    }
    ...
    function withdraw(address _tokenAddress)
        ...
        onlyEOA
    {
        uint256 subOwnerFee = SAPPMKT.getAmountEarn(msg.sender, _tokenAddress);
        if (_tokenAddress == address(0)) {
            ...
            SAPPMKT.setDefaultAmountEarn(msg.sender, _tokenAddress);
        ...
        onlyOwner
    {
        ...
        SAPPMKT = ISAPPNftMarketplace(marketPlaceAddress);
    }
