...
contract WLM is ERC20, ReentrancyGuard, Ownable {
    ...
    constructor(address pool, uint supplyWholeTokens) ERC20("WEN LUNA MOON", "WLM") {
        donationpool = pool;
    ...
    function buyPresale() external payable nonReentrant {
        ...
        require(ERC20(address(this)).balanceOf(msg.sender) <= maxsupply / 30, "You are not allowed to presale buy more then 3% of the supply");
        require(totalSupply() + count < maxsupply, "Not enough supply left");
        require(count < maxsupply / 100, "you can only buy 1% of supply at once");
    ...
    function presaleEnd() external view returns(uint) {
        return presaleend;
    }
