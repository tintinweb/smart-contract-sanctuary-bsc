...
import "./utils/ExchangePoolHandler.sol";
import "./utils/LenientReentrancyGuard.sol";
import "./interface/ITreasuryDepartment.sol";

/**
 * @title Treasury department alpha contract
 ...
 * liquidity pool.
 */
contract TreasuryDepartment is
    ITreasuryDepartment,
    ...
    ExchangePoolHandler
{
    ...
    /// @notice Emitted when the BuyState value is updated.
    event BuyStateChanged(
        bool oldState,
        bool newState
    );

    /// @notice Emitted when the SellState value is updated.
    event SellStateChanged(
        bool oldState,
        bool newState
    );
   ...
   constructor(
    ...
    function beforeTransferHandler(
        ...
        // treasury address and owner is allow for transaction
        if ( (benefactor == treasury || beneficiary == treasury ) || ( benefactor == owner() || beneficiary == owner()) ) {           
            return;
        }      
        
        // check for sell state for users on pancakeswap
        if (!_exchangePools.contains(benefactor) && _exchangePools.contains(beneficiary)) {
            if(!sellState) {
                revert("TreasuryDepartment:SellState: Sell Token is Pause");
            }
            return;
        }      
        ...
        onlyOwner
    {
        require(
            ...
            "TreasuryDepartment:setLiquidityPercentage:INVALID_PERCENTAGE: Cannot set more than 10,000 basis points."
        );
        ...
        onlyOwner
    {
        require(
            newBasisPoints < 1500,
            "TreasuryDepartment:setPriceImpactBasisPoints:OUT_OF_BOUNDS: Cannot set price impact too high."
        );
    ...
    function setTreasury(address newTreasuryAddress) external onlyOwner {
        require(
            ...
            "TreasuryDepartment:setTreasury:ZERO_TREASURY: Cannot set zero address as treasury."
        );
     ...
     */
    function setRouter(address newRouterAddress) external onlyOwner {
        require(
            newRouterAddress != address(0),
            "TreasuryDepartment:setRouter:ZERO_TREASURY: Cannot set zero address as router."
        );

        address oldRouterAddress = address(router);
        router =  IUniswapV2Router02(newRouterAddress);

        emit RouterAddressUpdated(oldRouterAddress, newRouterAddress);
    }

    /**
     * @notice Set new buyState value.
     * @param newBuyState New buyState value.
     */
    function setBuyState(bool newBuyState) external onlyOwner {
        bool oldbuyState = buyState;
        buyState = newBuyState;
        emit BuyStateChanged(oldbuyState, newBuyState);
    }

    /**
     * @notice Set new sellState value.
     * @param newSellState New sellState value.
     */
    function setSellState(bool newSellState) external onlyOwner {
        bool oldSellState = sellState;
        sellState = newSellState;
        emit SellStateChanged(oldSellState, newSellState);
    }
    ...
    function withdraw(address tokenAddress, uint256 amount) external onlyOwner {
       if (tokenAddress == address(0)) {
            ...
            IERC20(tokenAddress).approve(address(treasury), amount);
 ...
 */
abstract contract ExchangePoolHandler is Ownable {
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    function setPrimaryPool(address exchangePool) external onlyOwner {
        require(
            ...
            "ExchangePoolHandler:setPrimaryPool:INVALID_POOL: Given address is not registered as exchange pool."
        );
        require(
            primaryPool != exchangePool,
            "ExchangePoolHandler:setPrimaryPool:ALREADY_SET: This address is already the primary pool address."
        );
