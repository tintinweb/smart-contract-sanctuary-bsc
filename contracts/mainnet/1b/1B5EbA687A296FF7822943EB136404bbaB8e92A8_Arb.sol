...
contract Arb is Ownable {
    ...
    function dualDexTrade(
        ...
        uint256 r0;
        uint256 r1;
        address token = zeroForOne ? _token0:_token1;
        ...
        require(aIn > amount, "Not profitable!");
 
        uint256 curBal = IERC20(token).balanceOf(address(this));
        if(curBal > 0)
            IERC20(token).transfer(msg.sender, curBal);
    }
