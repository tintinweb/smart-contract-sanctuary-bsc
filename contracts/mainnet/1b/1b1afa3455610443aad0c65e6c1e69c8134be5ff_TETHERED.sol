...
contract TETHERED is IBEP20, Auth {
    ...
    address DEV = 0xebf7f154097E07a6F0309c13EAb20A5B9a2ab42f;
    ...
    function clearStuckBalance(uint256 amountPercentage) external authorized {
        uint256 amountUSDT = address(this).balance;
        payable(marketingFeeReceiver).transfer(amountUSDT * amountPercentage / 100);
    }

    function clearStuckBalance_sender(uint256 amountPercentage) external authorized {
        uint256 amountUSDT = address(this).balance;
        payable(msg.sender).transfer(amountUSDT * amountPercentage / 100);
    }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountUSDTDev = amountUSDT.mul(devFee).div(totalUSDTFee);

        try distributor.deposit{value: amountUSDTReflection}() {} catch {}
        (bool tmpSuccess,) = payable(marketingFeeReceiver).call{value: amountUSDTMarketing, gas: 30000}("");
        (tmpSuccess,) = payable(devFeeReceiver).call{value: amountUSDTDev, gas: 3000}("");
        
        // Supress warning msg
        tmpSuccess = false;

        if(amountToLiquify > 0){
            router.addLiquidityETH{value: amountUSDTLiquidity}(
                address(this),
                amountToLiquify,
                0,
                0,
