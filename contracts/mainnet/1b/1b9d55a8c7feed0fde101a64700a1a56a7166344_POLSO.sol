...
contract POLSO is BEP20, BEP20Burnable, Pausable {
    constructor() BEP20("POLSO", "POLSO") {
        _mint(msg.sender, 50000000000 * 10 ** decimals()); // 50,000,000,000 initial supply
    }
    ...
    /**@dev claim stucked BNB by the owner.
     */
    ...
    /**@dev claim stucked tokens by owner, if user sent it accidently to the token contract.
     */
     function claimStuckedTokens(address _token) external onlyOwner {
        require (_token != address(0), "POLSO: Token address is not valid");
