...
contract AegisSphynx is NFTokenMetadata, Ownable {
 
  using Strings for uint256;  

  uint256 constant _startTokenId = 7166;
...
modifier safe() {
  ...
  function getCurrentPrice() public view returns (uint256) {
      if (_lastTokenId <= 500) return prices[0];
      if (_lastTokenId <= 750) return prices[1];
      return prices[2];
  }
