...
interface LooBr is IERC20 {
    ...
    function updateTaxesFees(uint256 _buyFee, uint256 _sellFee, uint256 _transferFee) external;

    function updateFees(uint256 _marketing, uint256 _developer, uint256 _reward, uint256 _burn) external;
...
contract LooBrManagement is Context, Signable {

    LooBr public token;
    IERC20 public rewardToken;


    event LooBrTokenUpdated(address indexed newTokenAddress);
        ...
        Signable(signers)
    {
        ...
        token = LooBr(payable(token_));
        rewardToken = IERC20(rewardToken_);
    }

    function updateLooBrToken(address newTokenAddress) public onlySigned {
        require(newTokenAddress != address(0), "Contract: address is zero");
        token = LooBr(payable(newTokenAddress));
        emit LooBrTokenUpdated(newTokenAddress);
    }
    ...
    function updateTaxesFees(uint256 _buyFee, uint256 _sellFee, uint256 _transferFee) external onlySigned {
        token.updateTaxesFees(_buyFee, _sellFee, _transferFee);
    }

    function updateFees(uint256 _marketing, uint256 _developer, uint256 _reward, uint256 _burn) external onlySigned {
        token.updateFees(_marketing, _developer, _reward, _burn);
    }
