...
import "../../interfaces/evry/IEvryAutoRoute.sol";
import "../../interfaces/evry/IEvryAMMRouter.sol";
import "../../interfaces/evry/IEvryAMMPair.sol";
import "../../interfaces/evry/IFarmsV2.sol";
...
contract EvryByalanLPV2 is ByalanIsland, Sailor, IByalan {
    using SafeERC20 for IERC20;
    using Address for address payable;

    // Tokens used
    address public constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address public constant VELO = 0xf486ad071f3bEE968384D2E39e2D8aF0fCf6fd46;
    address public immutable override want;
    address public immutable lpToken0;
    address public immutable lpToken1;

    // Third party contracts
    address public constant MASTERCHEF = 0xDD3e2da1d017A564b8225bc8e92f2970cfa61945;
    uint256 public immutable pid;
    address public autoRouter = 0x5bC864BBbe1350141fC5Ce124C4e2457a62D4cCb;

    // Routes
    address[] public veloToWbnbRoute = [VELO, WBNB];
    string[] public veloToWbnbRoutePoolType = ["AMM"];
    address[] public veloToWbnbRouteDMMPool = [0x0000000000000000000000000000000000000000];

    event Harvest(address indexed harvester);
    event SetAutoRouter(address autoRouter);

    address[] public veloToLp0Route;
    string[] public veloToLp0RoutePoolType;
    address[] public veloToLp0RouteDMMPool;
    address[] public veloToLp1Route;
    string[] public veloToLp1RoutePoolType;
    address[] public veloToLp1RouteDMMPool;

    struct ConstructorParam {
        address hydra;
        address izlude;
        address kswFeeRecipient;
        address treasuryFeeRecipient;
        address harvester;
        uint256 pid;
        address[] veloToLp0Route;
        string[] veloToLp0RoutePoolType;
        address[] veloToLp0RouteDMMPool;
        address[] veloToLp1Route;
        string[] veloToLp1RoutePoolType;
        address[] veloToLp1RouteDMMPool;
    }

    constructor(ConstructorParam memory param)
        ByalanIsland(
            param.hydra,
            0x296B7C203E2C7306b132F2D5aD66106Bb7665C46, // evry router
            param.izlude,
            param.kswFeeRecipient,
            param.treasuryFeeRecipient,
            param.harvester
        )
    {
        pid = param.pid;

        want = IFarmsV2(MASTERCHEF).poolInfo(param.pid).lpToken;
        lpToken0 = IEvryPair(want).token0();
        lpToken1 = IEvryPair(want).token1();

        if (lpToken0 != VELO) {
            require(param.veloToLp0Route[0] == VELO, "invalid lp 0 route");
            require(param.veloToLp0Route[param.veloToLp0Route.length - 1] == lpToken0, "invalid lp 0 route");
            require(
                IEvryAutoRoute(autoRouter).getAmountsOut(
                    1 ether,
                    param.veloToLp0Route,
                    param.veloToLp0RoutePoolType,
                    param.veloToLp0RouteDMMPool
                )[param.veloToLp0Route.length - 1] > 0,
                "invalid lp 0 route"
            );
            veloToLp0Route = param.veloToLp0Route;
            veloToLp0RoutePoolType = param.veloToLp0RoutePoolType;
            veloToLp0RouteDMMPool = param.veloToLp0RouteDMMPool;
        }

        if (lpToken1 != VELO) {
            require(param.veloToLp1Route[0] == VELO, "invalid lp 1 route");
            require(param.veloToLp1Route[param.veloToLp1Route.length - 1] == lpToken1, "invalid lp 1 route");
            require(
                IEvryAutoRoute(autoRouter).getAmountsOut(
                    1 ether,
                    param.veloToLp1Route,
                    param.veloToLp1RoutePoolType,
                    param.veloToLp1RouteDMMPool
                )[param.veloToLp1Route.length - 1] > 0,
                "invalid lp 1 route"
            );
            veloToLp1Route = param.veloToLp1Route;
            veloToLp1RoutePoolType = param.veloToLp1RoutePoolType;
            veloToLp1RouteDMMPool = param.veloToLp1RouteDMMPool;
        }

        _giveAllowances();
    }

    function setAutoRouter(address _autoRouter) external onlyOwner {
        autoRouter = _autoRouter;
        emit SetAutoRouter(_autoRouter);
    }
    ...
    function deposit() public override whenNotPaused {
        ...
        if (wantBal > 0) {
            IFarmsV2(MASTERCHEF).deposit(address(this), pid, wantBal);
        }
    }
    ...
    function withdraw(uint256 _amount) external override onlyIzlude {
        ...
        if (wantBal < _amount) {
            IFarmsV2(MASTERCHEF).withdraw(address(this), pid, _amount - wantBal);
    ...
    function _harvest() private {
        IFarmsV2(MASTERCHEF).harvest(pid);

        uint256 rewardBal = IERC20(VELO).balanceOf(address(this));
        if (rewardBal > 0) {
            chargeFees();
            addLiquidity();
            deposit();
    ...
    function chargeFees() internal {
        uint256 toBnb = (IERC20(VELO).balanceOf(address(this)) * totalFee) / MAX_FEE;
        IEvryAutoRoute(autoRouter).swapExactTokensForNative(
            toBnb,
            0,
            veloToWbnbRoute,
            veloToWbnbRoutePoolType,
            veloToWbnbRouteDMMPool,
            address(this),
            block.timestamp
        );
    ...
    function addLiquidity() internal {
        uint256 rewardHalf = IERC20(VELO).balanceOf(address(this)) / 2;

        if (lpToken0 != VELO) {
            if (veloToLp0Route[veloToLp0Route.length - 1] == WBNB) {
                uint256[] memory amounts = IEvryAutoRoute(autoRouter).swapExactTokensForNative(
                    rewardHalf,
                    0,
                    veloToLp0Route,
                    veloToLp0RoutePoolType,
                    veloToLp0RouteDMMPool,
                    address(this),
                    block.timestamp
                );
                uint256 bnbAmount = amounts[amounts.length - 1];
                IWETH(WBNB).deposit{value: bnbAmount}();
            } else {
                IEvryAutoRoute(autoRouter).swapExactTokensForTokens(
                    rewardHalf,
                    0,
                    veloToLp0Route,
                    veloToLp0RoutePoolType,
        ...
        if (lpToken1 != VELO) {
            if (veloToLp1Route[veloToLp1Route.length - 1] == WBNB) {
                uint256[] memory amounts = IEvryAutoRoute(autoRouter).swapExactTokensForNative(
                    rewardHalf,
                    0,
                    veloToLp1Route,
                    veloToLp1RoutePoolType,
                    veloToLp1RouteDMMPool,
                    address(this),
                    block.timestamp
                );
                uint256 bnbAmount = amounts[amounts.length - 1];
        ...
        IEvryRouter2(unirouter).addLiquidity(
            lpToken0,
            lpToken1,
    ...
    function balanceOfPool() public view override returns (uint256) {
        return IFarmsV2(MASTERCHEF).userInfo(pid, address(this)).amount;
    }
        ...
        returns (IERC20[] memory rewardTokens, uint256[] memory rewardAmounts)
    {
        ...
        rewardTokens[0] = IERC20(IFarmsV2(MASTERCHEF).rewardToken());
        rewardAmounts[0] =
            IFarmsV2(MASTERCHEF).pendingReward(pid, address(this)) +
            IERC20(VELO).balanceOf(address(this));
    }
    ...
    function retireStrategy() external override onlyIzlude {
        uint256 amount = IFarmsV2(MASTERCHEF).userInfo(pid, address(this)).amount;
        IFarmsV2(MASTERCHEF).withdraw(address(this), pid, amount);
    ...
    function _giveAllowances() internal {
        ...
        IERC20(VELO).safeApprove(autoRouter, type(uint256).max);
        IERC20(VELO).safeApprove(unirouter, type(uint256).max);

        // lp token 0 and 1 maybe VELO so approve 0 is needed here
    ...
    function _removeAllowances() internal {
        ...
        IERC20(VELO).safeApprove(autoRouter, 0);
        IERC20(VELO).safeApprove(unirouter, 0);
        IERC20(lpToken0).safeApprove(unirouter, 0);
        IERC20(lpToken1).safeApprove(unirouter, 0);
    }

    receive() external payable {
        require(msg.sender == unirouter || msg.sender == autoRouter, "reject");
    }
}
...
interface IFarmsV2 {
    ...
    function stakeTokens() external view returns (address);

    function rewardToken() external view returns (address);

    function rewardPerBlock() external view returns (uint256);

    function rewardTokenSupply() external view returns (uint256);
