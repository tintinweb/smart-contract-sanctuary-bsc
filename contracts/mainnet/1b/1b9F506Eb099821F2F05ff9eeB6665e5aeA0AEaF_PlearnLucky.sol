...
contract PlearnLucky is IPlearnLucky, ReentrancyGuard, Ownable {
    ...
    constructor(
        ...
        uint256 _currentLuckyId,
        uint256 _currentTicketId
    ) {
        // Initializes values
        ...
        currentLuckyId = _currentLuckyId;
    ...
    function updateLuckyInfo(
        uint256 _luckyId,
        uint256 _endTime,
        uint256 _priceTicketInPlearn,
        uint32 _maxTicketsPerRound,
        uint32 _maxTicketsPerUser,
        uint32[] calldata _countLuckyNumbersPerBracket,
        uint32 _countReservePerBracket,
        ...
        require(
            _lucks[_luckyId].status == Status.Open || _lucks[_luckyId].status == Status.Close,
            'Unable to update lucky info'
        );
        require(_endTime > _lucks[_luckyId].startTime, 'End time must > start time');

        uint256 timeDuration = _endTime - _lucks[_luckyId].startTime;
        require((timeDuration > minLengthRound) && (timeDuration < maxLengthRound), 'Round length outside of range');
        ...
        _lucks[_luckyId].endTime = _endTime;
        _lucks[_luckyId].priceTicketInPlearn = _priceTicketInPlearn;
        _lucks[_luckyId].maxTicketsPerRound = _maxTicketsPerRound;
        _lucks[_luckyId].maxTicketsPerUser = _maxTicketsPerUser;
        _lucks[_luckyId].countLuckyNumbersPerBracket = _countLuckyNumbersPerBracket;
        _lucks[_luckyId].countReservePerBracket = _countReservePerBracket;
        _lucks[_luckyId].enableReserve = _enableReserve;
    ...
    function _getTicketIdFromTicketNumber(uint256 _luckyId, uint32 _ticketNumber) internal view returns (uint256) {
        ...
        require(ticketId >= _lucks[_luckyId].firstTicketId, 'Invalid ticket number');
        require(ticketId <= _lucks[_luckyId].lastTicketId, 'Invalid ticket number');
        return ticketId;
    }
