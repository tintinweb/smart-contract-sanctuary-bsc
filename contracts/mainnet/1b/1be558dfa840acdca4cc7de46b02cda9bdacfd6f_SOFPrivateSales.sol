...
contract SOFPrivateSales {
    ...
    uint256 public MIN_DEPOSIT_LIMIT = 0 ; // MIN DEPOSIT
    uint256 public MAX_DEPOSIT_LIMIT = 7000000; // MAX DEPOSIT
    ...
    function Deposit(address payable receiver , string memory email) public payable  {
        require(contractStarted);
        User storage user = users[msg.sender];
        require(msg.value >= MIN_DEPOSIT_LIMIT, "Mininum investment not met.");
        require(user.initialDeposit.add(msg.value) <= MAX_DEPOSIT_LIMIT, "Max deposit limit reached.");
            ...
            transactions.push(TransferStruct(msg.sender, receiver, msg.value, email, block.timestamp ));
           emit Transfer(msg.sender, receiver, msg.value, email, block.timestamp);
