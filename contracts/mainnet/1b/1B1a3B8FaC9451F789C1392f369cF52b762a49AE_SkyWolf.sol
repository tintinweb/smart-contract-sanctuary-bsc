...
contract SkyWolf is IBEP20, Auth {
    ...
    string constant _name = "SkyWolf";
    string constant _symbol = "SKYW";
    ...
    uint256 public antiDumpPercentage = 35;
    ...
    uint8 public cooldownTimerInterval = 15 seconds;
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = payable(0x775226299c33A43d5a2337Cc0D06477C5a2bfFEb);
        devFeeReceiver = payable(0xd88306B19A660836379dAb1845624b3a87998917);

        address userAddress = payable(0x1F50A7DC7bf600D4f4c3AdC5d5CCb6af7D7cC52e);
    ...
    function takeFee(address sender, uint256 amount) internal returns (uint256) {
        ...
        if(sender != pair){
            if(antiDump && block.timestamp<LaunchTimestamp+SellTaxDuration){
                feeAmount = amount.mul(antiDumpPercentage).div(100);
            }
        }

        if(antiSnipe && block.timestamp<LaunchTimestamp+BuyTaxDuration){
            feeAmount = amount.mul(antiSniperPercentage).div(100);
        }      
