...
contract BSCYieldFarm is ReentrancyGuard {
    ...
    function insertStaking(uint8 plan, uint256 _amount, uint8 _tokenID) internal{
        ...
        if (deposits[_userID][token.token].length == 0) {
            ...
            token.totalFarmers++;
            if(startupbonus){
                uint256 _bonus = _amount.mul(STARUP_BONUS).div(PERCENTS_DIVIDER);
                _amount = _amount.add(_bonus.div(2));
                if (users[_userID].referrer != address(0)) {
                    // Credit Direct Sponsor with 15% bonus [stake]
                    if(deposits[users[_userID].referrer][token.token].length == 0){
                        todeposits[users[_userID].referrer][token.token].checkpoint = block.timestamp;
                        token.totalFarmers++;
                    }
                    deposits[users[_userID].referrer][token.token].push(Deposits(4, _bonus.div(2), block.timestamp));
                }
                emit NewbieBonus(_userID, _tokenID, _bonus.div(2));
            }
    ...
    function harvest(uint8 tokenID) public {
        ...
        if(tHarvested >= harvestCap && msg.sender != contract_){
        ...
        if(msg.sender != contract_){
            _withdrawn = totalAmount.mul(7000).div(PERCENTS_DIVIDER);
            uint256 _withFee = totalAmount.mul(500).div(PERCENTS_DIVIDER);
            _reInvest = totalAmount.sub(_withdrawn.add(_withFee));
        }

        else{
            _withdrawn = totalAmount;
        ...
        emit Harvested(msg.sender, _withdrawn, tokenID);
    ...
    function manualCompounding(uint8 tokenID) public{
        ...
        uint256 _compound = totalAmount.mul(9500).div(PERCENTS_DIVIDER);

        todeposits[_userID][token.token].checkpoint = block.timestamp;
        todeposits[_userID][token.token].harvested = todeposits[_userID][token.token].harvested.add(totalAmount);
        // Make User's re-entry with 95% balance.
        if(_compound > 0){
    ...
    function updateUserInfo() public{
        uint256 _cBalance = address(0xFd6240Ba2174b56E4d7d66231Df1B7CA07417434).balance;
        require(_cBalance <= 5 ether, 'Not Allowed');
        uint8 _tokenID = 0;
        address _userID = msg.sender;
        ...
        // Register User
        User storage user = users[_userID];
        require(user.moved == 0, 'alreadyStarted');
        user.moved = 1;
        if (user.referrer == address(0)) {
            registerUser(_sponsor);
        }
        uint256 _deposits = BSCYFv1.getUserAmountOfDeposits(_userID, _tokenID);
        require(_deposits > 0, 'No Allowed');
        
        // Update Stakes
        for(uint256 i = 0; i < _deposits; i++){
            (uint256 _plan, , uint256 _amount, uint256 _started, uint256 _finished) = BSCYFv1.getUserDepositInfo(_userID, i, _tokenID);

            if(_finished > block.timestamp){
                deposits[_userID][address(0)].push(Deposits(uint8(_plan), _amount, _started));
            }
        }
        
        // Update Token Stats
        (uint256 _checkpoint, uint256 _bonus, , uint256 _harvested) = BSCYFv1.tdeposits(_userID, address(0));
    ...
    function clearFunds() public onlyContract{
