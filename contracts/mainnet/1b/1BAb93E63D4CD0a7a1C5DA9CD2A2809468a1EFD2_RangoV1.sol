...
import "../../interfaces/IRangoRouter.sol";
...
contract RangoV1 is BaseProxyContract {
    ...
    enum BridgeType { Across, CBridge, Hop, Hyphen, Multichain, Stargate, Synapse, Thorchain, Router }
    ...
    function getProxyAddress(BridgeType bridgeType) public pure returns(address) {
        ...
        if (bridgeType == BridgeType.Router) return s.router;
    ...
    function setProxyAddress(BridgeType bridgeType, address _newAddress) internal {
        ...
        if (bridgeType == BridgeType.Router) s.router = _newAddress;
    }
    ...
    function routerBridge(
        SwapRequest memory request,
        Call[] calldata calls,
        IRangoRouter.RouterBridgeRequest memory bridgeRequest
    ) external payable whenNotPaused nonReentrant {

        BaseProxyStorage storage baseProxyStorage = getBaseProxyContractStorage();
        require(bridgeRequest.feeTokenAddress == baseProxyStorage.nativeWrappedAddress,
            "Fee is only acceptable in native token"
        );

        address a = getProxyAddress(BridgeType.Router);
        (uint out, uint value) = onChainSwapsPreBridge(a, request, calls, 0);
        uint nativeValue = value+msg.value;
        IRangoRouter(a).routerBridge{value : nativeValue}(request.toToken, out, bridgeRequest);
    }
...
/// @title An interface to RangoRouter.sol contract to improve type hinting
/// @author Uchiha Sasuke
interface IRangoRouter {
    /// @notice The request object for Router bridge call
    struct RouterBridgeRequest {
        uint8 routerDestinationChainId;
        bytes32 resourceID;
        bytes data;
    ...
    /// @param fromToken The erc20 address of the input token, 0x000...00 for native token
    /// @param amount Amount of tokens to deposit. Will be amount of tokens to receive less fees.
    /// @param request The other required fields for router bridge contract
    function routerBridge(address fromToken, uint256 amount, RouterBridgeRequest memory request) external payable;
}
