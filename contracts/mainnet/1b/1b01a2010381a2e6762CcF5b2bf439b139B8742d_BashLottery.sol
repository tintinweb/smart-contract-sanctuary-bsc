...
// File: contracts/interfaces/IBashLottery.sol

pragma solidity ^0.8.4;

interface IBashLottery {
    /**
...
// File: contracts/BashLottery.sol
 ...
 */
contract BashLottery is ReentrancyGuard, IBashLottery, Ownable {
    ...
    uint256 public maxPriceTicketInBpt = 50 ether;
    uint256 public minPriceTicketInBpt = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInBpt;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] bptPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInBpt;
        uint32 finalNumber;
    }
     ...
     * @param _bptTokenAddress: address of the BPT token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _bptTokenAddress, address _randomGeneratorAddress) {
        bptToken = IERC20(_bptTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of BPT to this contract
        uint256 amountBptToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInBpt,
            _ticketNumbers.length
        );

        // Transfer bpt tokens to this contract
        bptToken.safeTransferFrom(address(msg.sender), address(this), amountBptToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInBpt += amountBptToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInBptToTransfer
        uint256 rewardInBptToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInBptToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        bptToken.safeTransfer(msg.sender, rewardInBptToTransfer);

        emit TicketsClaim(msg.sender, rewardInBptToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInBpt) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in BPT for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].bptPerBracket[j] =
                ...
                // A. No BPT to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].bptPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInBpt - amountToShareToWinners);

        // Transfer BPT to treasury address
        bptToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        bptToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInBpt += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInBpt >= minPriceTicketInBpt) && (_priceTicketInBpt <= maxPriceTicketInBpt),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInBpt: _priceTicketInBpt,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            bptPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInBpt: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(bptToken), "Cannot be BPT token");
    ...
    /**
     * @notice Set BPT price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInBpt: minimum price of a ticket in BPT
     * @param _maxPriceTicketInBpt: maximum price of a ticket in BPT
     */
    function setMinAndMaxTicketPriceInBpt(uint256 _minPriceTicketInBpt, uint256 _maxPriceTicketInBpt)
        external
        onlyOwner
    {
        require(_minPriceTicketInBpt <= _maxPriceTicketInBpt, "minPrice must be < maxPrice");

        minPriceTicketInBpt = _minPriceTicketInBpt;
        maxPriceTicketInBpt = _maxPriceTicketInBpt;
    }
     ...
     * @param _priceTicket price of a ticket (in BPT)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].bptPerBracket[_bracket];
        } else {
            return 0;
        }
    }
