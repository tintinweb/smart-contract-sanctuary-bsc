...
import {IChainlinkPrice} from "../interfaces/IChainlinkPrice.sol";
...
contract ChainlinkPriceFacet is IChainlinkPrice {
    ...
    function getPriceFromChainlink(address token) external view override returns (IOraclePrice.PriceInfo memory priceInfo) {
        (uint256 price, uint8 decimals) = LibChainlinkPrice.getPriceFromChainlink(token);
        priceInfo = IOraclePrice.PriceInfo(price, decimals);
    }

    function chainlinkPriceFeeds() external view override returns (IOraclePrice.PriceFeedInfo[] memory priceFeeds) {
        ...
        priceFeeds = new IOraclePrice.PriceFeedInfo[](numFeeds);
        for (uint256 i; i < numFeeds;) {
            address token = cps.tokenAddresses[i];
            LibChainlinkPrice.PriceFeed storage pf = cps.priceFeeds[token];
            priceFeeds[i].token = token;
            priceFeeds[i].feedAddress = pf.feedAddress;
            AggregatorV3Interface oracle = AggregatorV3Interface(pf.feedAddress);
            priceFeeds[i].description = oracle.description();
            priceFeeds[i].decimals = oracle.decimals();
            unchecked {
                i++;
            }
        }
...
import {IOraclePrice} from "./IOraclePrice.sol";
...
interface IChainlinkPrice {
    ...
    function getPriceFromChainlink(address token) external view returns (IOraclePrice.PriceInfo memory priceInfo);

    function chainlinkPriceFeeds() external view returns (IOraclePrice.PriceFeedInfo[] memory priceFeeds);
}
