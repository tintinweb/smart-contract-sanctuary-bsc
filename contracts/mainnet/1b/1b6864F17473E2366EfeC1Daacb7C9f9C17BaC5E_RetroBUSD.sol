...
/**
Telegram : https://t.me/RetroBUSD
Website : https://RetroBusd.cloud
*/
...
contract RetroBUSD is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    RetroBUSDDividendTracker public dividendTracker;
    ...
    constructor() ERC20("RetroBUSD", "rBUSD") {
        marketingWallet = payable(0x5E6f23d56209300bCb6eC124674ACD895D0A3727);
        devWallet = payable(0xb8456F7E2b5455b6BDD90EBD910F8285465c839f);
        ...
        dividendTracker = new RetroBUSDDividendTracker(
            payable(this),
            router,
        ...
        maxWallet = totalTokenSupply / 30; // 5%
...
contract RetroBUSDDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    RetroBUSD public RetroBUSDContract;
    ...
    constructor(
        ...
        defaultToken = token;
        RetroBUSDContract = RetroBUSD(mainContract);
    ...
    function _transfer(
        ...
        require(false, "RetroBUSD_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "RetroBUSD_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main RetroBUSD contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(RetroBUSDContract);

            uint256 prevBalance = RetroBUSDContract.balanceOf(address(this));
            ...
            {
                uint256 received = RetroBUSDContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    RetroBUSDContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
