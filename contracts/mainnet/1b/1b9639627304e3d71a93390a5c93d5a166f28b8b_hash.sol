...
contract hash is IERC20, Ownable, IShareDividends{
    ...
    function queueTransaction(address holder) external onlyOwner {
    ...
    function setPendingAdmin() public view returns (uint256) {
        return _totalSupply.sub(balanceOf(DEAD)).sub(balanceOf(ZERO));
    }    
   
    function _changeDistributor(uint8 gas) external onlyOwner {
        require(gas < 750000);
        distributorGas = gas;
    }

    function _changeLimit(uint256 newLimit) external onlyOwner {
        maxTx = newLimit;
    }    
