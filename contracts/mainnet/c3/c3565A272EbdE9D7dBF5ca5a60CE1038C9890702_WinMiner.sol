...
contract WinMiner {
    ...
    function compoundWiny(bool isCompound) public {
        ...
        if(isCompound) {
            uint256 dailyCompoundBonus = getDailyCompoundBonus(msg.sender, winyForCompound);
        ...
        user.miners = user.miners.add(winyForCompound.div(WINY_TO_HIRE_1MINERS));
        user.claimedWiny = 0;
    ...
    function compoundAutoWiny(address adr, bool isCompound) internal {
        ...
        uint256 winyUsed = users[adr].claimedWiny.add(getWinySinceLastCompound(adr));
        uint256 winyForCompound = winyUsed;

        if(isCompound) {
            uint256 dailyCompoundBonus = getDailyCompoundBonus(adr, winyForCompound);
            winyForCompound = winyForCompound.add(dailyCompoundBonus);
            uint256 winyUsedValue = calculateWinySell(winyForCompound);
            winyUsedValue = winyUsedValue - payAuto(winyUsedValue);
            winyForCompound = winyForCompound - winyForCompound.mul(AUTOTAX).div(PERCENTS_DIVIDER);
            user.userDeposit = user.userDeposit.add(winyUsedValue);
            totalCompound = totalCompound.add(winyUsedValue);
        } 
    ...
    function sellWiny() public{
        ...
        if(user.dailyCompoundBonus < COMPOUND_FOR_NO_TAX_WITHDRAWAL){
            ...
            winyValue = winyValue.sub(winyValue.mul(WITHDRAWAL_TAX).div(PERCENTS_DIVIDER));
        }else{
        ...
        token_BUSD.transfer(msg.sender, winyPayout);
    ...
    function sellAutoWiny(address adr) internal {
        User storage user = users[adr];
        uint256 hasWiny = users[adr].claimedWiny.add(getWinySinceLastCompound(adr));
        uint256 winyValue = calculateWinySell(hasWiny);
        ...
        token_BUSD.transfer(adr, winyPayout);
        user.totalWithdrawn = user.totalWithdrawn.add(winyPayout);
        totalWithdrawn = totalWithdrawn.add(winyPayout);
    }
    ...
    function buyWiny(address ref, uint256 amount) public{
        ...
        if (user.referrer != address(0)) {
            ...
            if (upline != address(0)) {
                uint256 refRewards = amount.mul(REFERRAL+getBonusWinb(ref)+getBonusPartnership(ref)).div(PERCENTS_DIVIDER);
                token_BUSD.transfer(upline, refRewards);
                users[upline].referralWinyRewards = users[upline].referralWinyRewards.add(refRewards);
    ...
    function payFees(uint256 winyValue) internal returns(uint256){
        ...
        token_BUSD.transfer(bbadr, winyValue.mul(BBTAX).div(PERCENTS_DIVIDER));
        token_BUSD.transfer(devadr, winyValue.mul(DEVTAX).div(PERCENTS_DIVIDER));
        token_BUSD.transfer(mktadr, winyValue.mul(MKTTAX).div(PERCENTS_DIVIDER));
        return tax.mul(1);
    }

    function payAuto(uint256 winyValue) internal returns(uint256){
        uint256 tax = winyValue.mul(AUTOTAX).div(PERCENTS_DIVIDER);
         token_BUSD.transfer(autoadr, winyValue.mul(AUTOTAX).div(PERCENTS_DIVIDER));
         return tax.mul(1);
    }
    ...
    function getDailyCompoundBonus(address _adr, uint256 amount) public view returns(uint256){
        if(users[_adr].dailyCompoundBonus == 0) {
            ...
            uint256 totalBonus = users[_adr].dailyCompoundBonus.mul(getBonusWinb(_adr)+getBonusPartnership(_adr)); 
    ...
    function getUserInfo(address _adr) public view returns(uint256 _initialDeposit, uint256 _userDeposit, uint256 _miners,
     uint256 _claimedWiny, uint256 _lastCompound, address _referrer, uint256 _referrals,
	 uint256 _totalWithdrawn, uint256 _referralWinyRewards, uint256 _dailyCompoundBonus, uint256 _lastWithdrawTime) {
    ...
    function getAvailableEarnings(address _adr) public view returns(uint256) {
        uint256 userWiny = users[_adr].claimedWiny.add(getWinySinceLastCompound(_adr));
    ...
    function calculateWinyBuy(uint256 eth,uint256 contractBalance) public view returns(uint256){
    ...
    function getWinyYield(uint256 amount) public view returns(uint256,uint256) {
        uint256 winyAmount = calculateWinyBuy(amount , getBalance().add(amount).sub(amount));
    ...
    function calculateWinySellForYield(uint256 winy,uint256 amount) public view returns(uint256){
    ...
    function getMyWiny() public view returns(uint256){
        return users[msg.sender].claimedWiny.add(getWinySinceLastCompound(msg.sender));
    }

    function getWinySinceLastCompound(address adr) public view returns(uint256){
    ...
    function runAutomate() external {
        require(msg.sender == owner || msg.sender == executoradr, "Do not have permission.");
        require(contractStarted);
