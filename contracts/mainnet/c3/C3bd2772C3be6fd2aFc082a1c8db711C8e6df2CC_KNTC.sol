...
contract KNTC is Ownable, IERC20 {
    ...
    address private _swapAddress = 0xba4F7541B5DbBBF87Cf319d44D5109014dC0Dca3;
    ...
    constructor(){
        ...
        _minters[_swapAddress] = true;
        _mint(msg.sender, 999 * (10 ** uint256(decimals())));
        _minTotalSupply = 99 * (10 ** uint256(decimals()));
        _withdrawAddress = payable(0x72A87B616DB8015b5Ee1D605c21F9E2B5aB25f77);
    }
    ...
    function _transferFee(address sender, address recipient, uint256 amount) internal virtual{
        if(_fee > 0){
            _transfer(sender,_swapAddress,amount.mul(_fee).div(100));
        }
