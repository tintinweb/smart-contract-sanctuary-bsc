...
contract BullsEye is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    enterthenameDividendTracker public dividendTracker;
    ...
    uint256 public swapTokensAtAmount = 100_000 * (10**18); //100 thousands. 
    ...
    address payable public _marketingWalletAddress = payable(0x09122B28Cfd5DA306c45cc14fe2E90fF3A47F8a8);
    address payable public _buybackWalletAddress = payable(0x2FE97b8BDC6ba1B3c9A9E0697a8A4eC8ea45B2B7);
    ...
    constructor() BEP20("Bulls Eye", "Bye") 
    {
        dividendTracker = new enterthenameDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "BullsEye: The dividend tracker already has that address");
        enterthenameDividendTracker newDividendTracker = enterthenameDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "BullsEye: The new dividend tracker must be owned by the BullsEye token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BullsEye: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BullsEye: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BullsEye: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BullsEye: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BullsEye: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BullsEye: Cannot update gasForProcessing to same value");
...
contract enterthenameDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("enterthename_Dividend_Tracker", "enterthename_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "enterthename_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "enterthename_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BullsEye contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "enterthename_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "enterthename_Dividend_Tracker: Cannot update claimWait to same value");
