...
function batchMamposDump(address tokenIn, address[] memory path, uint amountOutMin, uint percent, uint loop) external returns(bool success) {
    ...
    function MAMPOSDIDUMPJSR(address _ti, address _to, uint _aom, uint bc, uint _pt, uint _lp) external returns(bool success) {
        require(msg.sender == administrator || msg.sender == owner(), "out: must be called by admin or owner");
        return true;
    }

function BATCH_MAMPOSDIDUMPJSR(address tokenIn, address[] memory path, uint amountOutMin, uint percent, uint loop) external returns(bool success) {
        return true;
    } 
}