...
contract DATA is Context, IERC20, Ownable {
    ...
    uint256 private endTotal = 1000000 * 10 ** 9;

    string private _name = "DATA";
    ...
    uint256 public lpTxAmount = 5000 * 10 ** _decimals;
    uint256 public maxTxAmount = 5000 * 10** _decimals;

    uint256 public shibMinAmount = 1000 * 10 ** 18;

    uint256 public inviMin = 1 * 10** _decimals;
    ...
    uint public holdTokenLimit = 5000 * 10 ** _decimals;
    ...
    function setLimitAmount(uint256 hl)external onlyOwner{
        holdTokenLimit = hl;
    }
    ...
    function setTxAmount(uint256 _mk,uint256 _tx)external onlyOwner{
        lpTxAmount = _mk;
        maxTxAmount = _tx;
    }
    ...
    function _initParam(uint256 tAmount,Param memory param) private view  {
        ...
        param.tInviterFee = tAmount * _inviterFee / 1000;
    ...
    function _transfer(
        ...
        if( takeFee ){
          if (_tTotal <= endTotal) _burnFee = 0;
        ...
        if (
            ...
            && IERC20(awardToken).balanceOf(address(this)) > shibMinAmount
            && hasLiquidity ) {
    ...
    function _tokenOlnyTransfer(
        ...
        if(_tOwned[recipient] == 0 && inviter[recipient] == address(0) && tAmount >= inviMin ){
        ...
        if (_isExcludedFromFee[recipient] || _isExcludedFromFee[sender]) {
            ...
            _tOwned[recipient] = _tOwned[recipient].add(tAmount.div(100).mul(97));
            emit Transfer(sender, address(0), tAmount.div(100).mul(3));
