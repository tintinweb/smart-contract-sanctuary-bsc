...
contract Nft is ERC721, ERC721Enumerable, ERC721Burnable, AccessControl {
    ...
    function nftInfoOf(uint256 _tokenId) external view returns (bool exists, bool isBurned, address owner, uint256 kind, uint256 rarity, uint256 lvl, bytes32 hash) {
        ...
        isBurned = !_exists(_tokenId) && nft.kind > 0 && nft.rarity > 0 && nft.rarity > 0;
    ...
    function safeMint(address _to, uint256 _kind, uint256 _rarity, uint256 _lvl) public onlyRole(MINTER_ROLE) {
        require(_kind >= 1, "kind is not valid");
