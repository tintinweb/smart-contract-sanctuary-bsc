...
contract GlowDistributor is UUPSUpgradeable, OwnableUpgradeable {
    ...
    bool public inRewardTransition;
    mapping (address => address) public userRewardID;
    ...
    function setShare(address shareholder_, uint256 amount_) external onlyToken {
        if(shares[shareholder_].amount > 0 && !inRewardTransition) {
    ...
    function process(uint256 gas_) external onlyToken {
        if(address(this).balance > swapBNBThreshold && !inRewardTransition) {
            processBNBSwap();
            return;
        }
        ...
        while(gasUsed < gas_ && iterations < shareholderCount) {
            ...
            if(inRewardTransition) {
                if(userRewardID[shareholders[currentIndex]] != rewardToken) {
                    shares[shareholders[currentIndex]].amount = IERC20(parentToken).balanceOf(shareholders[currentIndex]);
                    shares[shareholders[currentIndex]].totalExcluded = 0;
                    shares[shareholders[currentIndex]].totalRealised = 0;
                    userRewardID[shareholders[currentIndex]] = rewardToken;
                }
                else {
                    inRewardTransition = false;
                }
            }
            else {
    ...
    function claimDividend() external {
        if(shares[msg.sender].amount > 0 && !inRewardTransition) {
    ...
    function setDistribName(string memory distributorName_) external onlyOwner {
        distributorName = distributorName_;
    }
}