...
contract GRUUK is IERC20, Ownable, Pausable {
    ...
    address public officialPresaleAddress;
    address public constant teamWallet = 0xB29AAB1B0014039a79cc2601f6A5f063C2668cDa;
    ...
    uint256 private constant numTokensSellToAddToLiquidity = 1000000 * 10**18;
   ...
   function transferFrom(
    ...
    function _transfer(
        ...
        if (_isAMM[sender] || _isAMM[recipient]) {
            if (_isExcludedFromFees[sender] || _isExcludedFromFees[recipient]) {
                ...
                _teamAmount = amount.mul(_teamFee).div(10000);
                _liquidityAmount = amount.mul(_liquidityFee).div(10000);
            }
        } else {
            _teamAmount = 0;
            _liquidityAmount = 0;
        ...
        _balances[teamWallet] += _teamAmount;
        _balances[address(this)] += _liquidityAmount;
        _balances[recipient] += amount.sub(_teamAmount).sub(_liquidityAmount);

        if (_teamAmount > 0) {
            emit Transfer(sender, teamWallet, _teamAmount);
        }

        emit Transfer(
            sender,
            recipient,
            amount.sub(_teamAmount).sub(_liquidityAmount).sub(_burnAmt)
        );
    }
    ...
    function updateFees(
        ...
        require(
            liquidityFee.add(teamFee) <= 3000,
            "You can't set fee more than 30%"
        );
        _liquidityFee = liquidityFee;
        _teamFee = teamFee;

        emit TaxFeeUpdated(liquidityFee.add(teamFee));
    }

    function excludeOrIncludeFromFee(address Account, bool status) external onlyOwner whenNotPaused
    {
    ...
    function setAMM(address Account, bool status) external onlyOwner whenNotPaused
    {
        require(Account != address(0), "You can't exclude zero address");
        _isAMM[Account] = status;
    }
}