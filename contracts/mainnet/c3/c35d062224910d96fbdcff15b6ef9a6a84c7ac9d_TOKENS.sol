...
contract TOKENS is ERC20, Ownable {
    ...
    uint256 public sellTotalFees = ETHRewardsFee.add(marketingFee).add(sellFee);
    uint256 public _maxTxAmount = 100000000 * (10**18);
    uint256 public _maxWalletAmount = 100000000 * (10**18);
    ...
    constructor() public ERC20("SportsWorld", "SW") {
    ...
    function setSellFee(uint256 value) external onlyOwner{
        require(value<=16,"Over then 20");
        sellFee = value;
        sellTotalFees = ETHRewardsFee.add(marketingFee).add(sellFee);
    }
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            swapAndSendToFee(marketingFeeAmount);
        ...
        if(takeFee) {
            ...
            if(transferFee == sellTotalFees){
                marketingFeeAmount += fees.mul(sellFee + marketingFee).div(sellTotalFees);
            }else{
                marketingFeeAmount += fees.mul(marketingFee).div(totalFees);
            }
...
contract ETHBackDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor() public DividendPayingToken("ETHBack_Dividen_Tracker", "ETHBack_Dividend_Tracker") {
        claimWait = 600;
        minimumTokenBalanceForDividends = 50000 * (10**18); //must hold 200000+ tokens
    }
