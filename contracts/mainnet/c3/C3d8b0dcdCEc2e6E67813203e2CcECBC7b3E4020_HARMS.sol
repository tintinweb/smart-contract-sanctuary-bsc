...
abstract contract AbsToken is IERC20, Ownable {
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            if (isRemoveLP) {
                ...
                uint256 fundAmount = tAmount * (_buyFundFee + _buyFundFee2 + _buyLPDividendFee) / 10000;
                if (fundAmount > 0) {
                    feeAmount += fundAmount;
                    _takeTransfer(sender, address(this), fundAmount);
                }
            } else if (_swapPairList[recipient]) {//Sell
                uint256 fundAmount = tAmount * (_sellFundFee + _sellFundFee2 + _sellLPDividendFee) / 10000;
                if (fundAmount > 0) {
                    feeAmount += fundAmount;
                    _takeTransfer(sender, address(this), fundAmount);
                }
    ...
    function setBuyFee(
        uint256 fundFee, uint256 fundFee2, uint256 lpDividendFee
    ) external onlyOwner {
        _buyFundFee = fundFee;
    ...
    function setSellFee(
        uint256 sellFundFee, uint256 sellFundFee2, uint256 lpDividendFee
    ) external onlyOwner {
...
contract HARMS is AbsToken {
    constructor() AbsToken(
        ...
        address(0x10d1872f54444d4FdDcd9729c73c930FfF4c6AB3),

        address(0xa7eb1f90088d966C83ef9728bcE0e4bFEdD6Bc49),

        address(0x4840dFc91B25fecdF8D7A3A8d64A8e13c6075861)
    ){
