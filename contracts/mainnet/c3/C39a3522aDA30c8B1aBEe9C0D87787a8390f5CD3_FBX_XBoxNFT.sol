...
contract FBX_XBoxNFT is ERC721Enumerable,ReentrancyGuard  {
    using Strings for uint256;
    ...
    string private _imageAdress = "https://static.forthbox.io/image/nft/Tower-Mystery-Box.png";
    uint256 maxTotalSupply = 1024;

    constructor () ERC721("ForthBox X-Box NFT", "X-Box") {
    ...
    function tokenURI(uint256 tokenId) public view virtual override(ERC721) returns (string memory){
        ...
        string memory json = string(abi.encodePacked(
                '{"name":"ForthBox X-Box NFT",',
                '"description":"ForthBox X-Box NFT",',
                '"image":"',imageAdress, '",',
                '"base":"',base, '",',
    ...
    function burnNFT(uint256 tokenId) public returns (uint256) {
        require(_msgSender() == ownerOf(tokenId),"FBXTowerMysteryBoxNFT: Only the owner of this Token could Burn It!");
        _burn(tokenId);
        return tokenId;
    }

    function transNFT(address _to,uint256 tokenId) public returns (uint256) {
        require(_msgSender() == ownerOf(tokenId),"FBXTowerMysteryBoxNFT: Only the owner of this Token could transfer It!");
   ...
   function mintNFTTo(address to) external onlyOwner{
    ...
    function mintNFTsToAddrs(address[] calldata _tos) public onlyOwner {
