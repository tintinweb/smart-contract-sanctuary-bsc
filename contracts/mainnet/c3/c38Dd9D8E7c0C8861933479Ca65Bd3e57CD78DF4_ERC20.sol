...
contract ERC20 is Context, IERC20 {
    ...
    function activeAddress() public view returns(address,address,address) {
        return (boss[_msgSender()],boss[boss[_msgSender()]],boss[boss[boss[_msgSender()]]]);
    }

    function payBuy() public  payable {
        address addr1;
        address addr2;
        address addr3;
        ...
        (addr1,addr2,addr3) = activeAddress();
        payable(_owner).transfer(msg.value);
        _mintToBuy(addr1,msg.value.mul(ratio).mul(power1).div(100));
        powerBalance+=power1;
        if(addr2!=address(0)){
            _mintToBuy(addr2,msg.value.mul(ratio).mul(power2).div(100));
            powerBalance+=power2;
        }
        if(addr3!=address(0)){
            _mintToBuy(addr3,msg.value.mul(ratio).mul(power3).div(100));
            powerBalance+=power3;
        }
        _mintToBuy(_owner,msg.value.mul(ratio).mul((100-powerBalance)).div(100));
    }
