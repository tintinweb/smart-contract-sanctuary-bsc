...
contract Launchpad {
  ...
  mapping(address => BonusPackage) public bonusPackages;
  ...
  constructor(uint256 _startBlock) {
    ...
    pool = PoolInfo({
      lastRewardBlock: _startBlock.to64(),
      accRewardPerShare: 0,
      totalEmission: 0
    });
  }
  ...
  function setLastRewardBlock(uint64 _lastRewardBlock) external onlyOwner returns (bool) {
  ...
  function getSummary() external view returns (uint256 bonusProgress, uint256 totalBonus, uint256 poolMaxBnb, uint256 dailyReward, uint256 poolMaxUsd, uint256 bnbPrice, uint256 badgePrice, uint256 bdxPrice) {
    ...
    bdxPrice = convertBnbToUsd(convertBdxToBnb(1e18));
  }
  ...
  function _addLiquidityAndDeposit(address _to) private returns (bool) {
    ...
    BonusPackage storage bonusPackage = bonusPackages[msg.sender];
    bonusPackage.totalDeposit = bonusPackage.totalDeposit + msg.value;
    bonusPackage.paidBdx = bonusPackage.paidBdx + bdxAmount;
  ...
  function withdrawAndHarvest(uint256 _amount, address _to) external withdrawable returns (bool) {
    updatePool();
  ...
  function withdrawAndRemoveLiquidityByAdmin(address _to) external onlyOwner returns (bool) {
    _withdrawAndRemoveLiquidity(userInfo[_to].amount, _to, _to);
    return true;
  }

  function withdrawBadge(address _to, uint256 _amount) external onlyOwner returns (bool) {
    if(_amount > 0) {
      badge.transfer(_to, _amount);
    } else {
      badge.transfer(_to, badge.balanceOf(address(this)));
    }
    return true;
  }

  function withdrawBnb(address _to, uint256 _amount) external onlyOwner returns (bool) {
    if(_amount > 0) {
  ...
  function _harvest(address _from, address _to) private returns (bool) {
    ...
    BonusPackage storage bonusPackage = bonusPackages[_to];
    bonusPackage.paidBadge = bonusPackage.paidBadge + _pendingReward;
