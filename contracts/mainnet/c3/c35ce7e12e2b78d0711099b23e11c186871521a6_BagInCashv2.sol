...
contract BagInCashv2 is ReentrancyGuard{
    ...
    function nextHarvest() public view returns(uint256){
        return players[msg.sender]._activeStake * DAILY_PERCENT / DIVIDER;
    }
    ...
    function moveOld() public{
        ...
        if(oldPlayers > 0){
            for(uint256 i = 0; i < oldPlayers; i++){
                ...
                (uint256 _totalStaked, uint256 _totalProfit, uint256 _activeStake, uint256 _harvested, uint256 _totalCommsn, , address _sponsor) = OLD.players(_player);
                // register old user
                ...
                player._lastHarvest = block.timestamp;
