...
/*
ChickenFarm - bsc-chickenfarm.online
*/
...
contract ChickenFarm {  
    ...
    uint256 public immutable OWNER_ETHER_FEE = 3;
    uint256 public immutable PARTNER_ETHER_FEE = 3; 
    uint256 public immutable MARKET_ETHER_FEE = 2;  
    uint256 public immutable OWNER_GEM_FEE = 8;
    uint256 public immutable OWNER_WITHDRAW_FEE = 0;
    ...
    constructor() {
        developer = address(0x321FCcC3499dF72708Bcb21bdc01576C73Fee118);
        partner = address(0xd6A1eaE85df805f9754ba42cf740AE7A10B42b1F);
        market = address(0x5F8F5eB26F0Cc3B0be93e17e0449ceA859f740Ed);
    }
    ...
    function register(address neighbor) external initialized {
        ...
        uint256 gems;
        totalVillages++;
        if (villages[neighbor].sheeps[0] > 0 && neighbor != developer) {
            gems += GEM_BONUS * 2;
        } else{
            neighbor = developer;
            gems += GEM_BONUS;
    ...
    function convertEggsToGem(uint256 wool) external initialized {
        address user = msg.sender;
    ...
    function upgradeWarehouse() external initialized {
        ...
        syncVillage(user);
        require(warehouseId < 5, "Max Lvl 5 EggHouse");
    ...
    function upgradeTruck() external initialized {
        ...
        syncVillage(user);
        require(truckId < 4, "Max Lvl 4 truck");
    ...
    function payFee(uint256 etherFee, uint256 gemFee) internal {
        if(etherFee > 0){
            payable(developer).transfer((etherFee * OWNER_ETHER_FEE) / 100);
            payable(partner).transfer((etherFee * PARTNER_ETHER_FEE) / 100);
            payable(market).transfer((etherFee * MARKET_ETHER_FEE) / 100);
        }
        if(gemFee > 0){
            villages[developer].gems += (gemFee * OWNER_GEM_FEE) / 100;
        }
    }
    ...
    function topLeaderUpdate(address user, bool value) external {
        require(
            ...
            "Only Developer is allowed to modify topLeader."
        );
