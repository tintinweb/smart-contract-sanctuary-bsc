...
contract MasterChef is Ownable {
    ...
    function deposit(uint256 _pid, uint256 _amount) external {
        require(_pid != 0, "pool ID cannot be zero");
        ...
        // Transfer token from user into the contract
    ...
    function withdraw(uint256 _pid, uint256 _amount) external {
        require(_pid != 0, "pool ID cannot be zero");
        require(_amount != 0, "amount cannot be zero");
        require(userInfo[_pid][msg.sender].amount >= _amount, "Invalid amount");
    ...
    function enterStaking(uint256 _amount) external {
        ...
        corn.mint(msg.sender, _amount);
        emit Deposit(msg.sender, 0, _amount);
    }

    function leaveStaking(uint256 _amount) external {
        require(_amount != 0, "amount cannot be zero");
        require(userInfo[0][msg.sender].amount >= _amount, "Invalid amount");
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 cornReward = multiplier.mul(cornPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
