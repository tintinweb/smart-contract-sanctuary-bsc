...
contract Token is ERC20, Ownable {
    ...
    constructor(
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(address(swapRouterAddr_));
        address _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(address(this), _uniswapV2Router.WETH());

        buyDeadFee = buyFeeSetting_[0];
        buyLiquidityFee = buyFeeSetting_[1];

        sellDeadFee = sellFeeSetting_[0];
    ...
    function SetBatchEnmyAddress(address[] calldata accounts, bool value) external onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++) {
            _isEnemy[accounts[i]] = value;
        }
    }
    ...
    function _transfer(
        ...
        // `transfer` to another wallte don't take
        ...
        if(takeFee) {
            ...
            super._transfer(from, address(uniswapV2Pair), fees.sub(DFee));
        }
