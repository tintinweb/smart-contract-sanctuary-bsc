...
contract Bul is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Initial distribution for the day 2-5 BUL-WETH LP -> BUL pool
    uint256 public constant INITIAL_BUL_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the BUL ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("bul.finance", "BUL") {
        // Mints 1 BUL to contract creator for initial pool setup
    ...
    function _getBulPrice() internal view returns (uint256 _bulPrice) {
        try IOracle(bulOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Bul: failed to fetch BUL price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _bulPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_bulPrice >= taxTiersTwaps[tierId]) {
    ...
    function setBulOracle(address _bulOracle) public onlyOperatorOrTaxOffice {
        require(_bulOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints BUL to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of BUL to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentBulPrice = _getBulPrice();
            currentTaxRate = _updateTaxRate(currentBulPrice);
            if (currentBulPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_bulPool != address(0), "!_bulPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_bulPool, INITIAL_BUL_POOL_DISTRIBUTION);
