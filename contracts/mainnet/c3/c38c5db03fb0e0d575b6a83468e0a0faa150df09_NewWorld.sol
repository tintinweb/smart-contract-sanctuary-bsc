...
library Address{
 ...
 contract NewWorld is Context, IERC20, Ownable { 
    ...
    uint256 public maxBuyLimit = 100_000_000 * 10**_decimals;
    uint256 public maxSellLimit = 100_000_000 * 10**_decimals;
    uint256 public maxWalletLimit = 100_000_000 * 10**_decimals;

    uint256 public genesis_block;
    
    address public marketingWallet = 0x0f07050eB6C2ceFeC1155ec7BaB66E9cd6Bf77CD;
    address public developer = 0x1DC6B493D7c24c5e18B2907849E3E0F7D5D7BFeC;
    address public burn = 0x000000000000000000000000000000000000dEaD;

    string private constant _name = "New World";
    string private constant _symbol = "NWRL";
    ...
    Taxes public taxes = Taxes(0, 1, 1, 2, 1);
    Taxes public sellTaxes = Taxes(0, 1, 1, 2, 1);
    ...
    function setTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _developer, uint256 _burn) public onlyOwner {
       taxes = Taxes(_rfi,_marketing,_liquidity,_developer,_burn);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _developer, uint256 _burn) public onlyOwner {
       sellTaxes = Taxes(_rfi,_marketing,_liquidity,_developer,_burn);
        emit FeesChanged();
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rBurn) = _getRValues3(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.tTransferAmount = tAmount-s.tRfi-s.tMarketing-s.tLiquidity-s.tDeveloper-s.tBurn;
        return s;
    }
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rMarketing, uint256 rLiquidity){
        ...
        rTransferAmount =  rAmount-rRfi-rMarketing-rLiquidity-rDeveloper-rBurn;
    ...
    function _getRValues3(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 rBurn) {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tMarketing + s.tDeveloper + s.tBurn);
        }
        if(s.rMarketing > 0 || s.tMarketing > 0) _takeMarketing(s.rMarketing, s.tMarketing);
        if(s.rDeveloper > 0 || s.tDeveloper > 0) _takeDeveloper(s.rDeveloper, s.tDeveloper);
        if(s.rBurn > 0 || s.tBurn > 0) _takeBurn(s.tBurn);

        emit Transfer(sender, recipient, s.tTransferAmount);        
    }

    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.marketing + temp.developer + temp.burn) * 2;
        ...
        uint256 burnAmount = unitBalance * 2 * temp.burn;
        if(burnAmount > 0){
            payable(burn).sendValue(burnAmount);
        }
    }
