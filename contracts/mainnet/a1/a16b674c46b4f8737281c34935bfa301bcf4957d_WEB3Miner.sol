...
contract WEB3Miner {
	...
	uint256 constant public TOTAL_REF = 5;
	uint256 constant public MINE_FEE = 2;
    uint256 constant public REMINE_FEE = 5;
	...
	bool public started;
	address payable public defi;

	event Newbie(address user);
	event NewMining(address indexed user, uint8 plan, uint256 amount);
    event ReMining(address indexed user, uint8 plan, uint256 amount);
	...
	constructor(address payable wallet) public {
        ...
        plans.push(Plan(365, 4));
      
	}

	function hire(address referrer) public payable {
		uint8 plan = 0;
		started = true;
		...
		uint256 fee = msg.value.mul(MINE_FEE*100).div(DIVIDER*10);
		defi.transfer(fee);
		...
		if (user.referrer != address(0)) {
			...
			for (uint256 i = 0; i < 5; i++) {
				if (upline != address(0)) {
					uint256 amount = msg.value.mul(REFERRAL_PERCENTS[i]).div(DIVIDER*10);
		...
		emit NewMining(msg.sender, plan, msg.value);
	}


    function remine() public {
		uint8 plan = 0;
		...
		uint256 fee = totalAmount.mul(REMINE_FEE*10).div(DIVIDER*10);
		defi.transfer(fee);
		emit FeePayed(msg.sender, fee);
		...
		emit ReMining(msg.sender, plan, totalAmount);
	...
	function getUserDividends(address userAddress) public view returns (uint256) {
		...
		for (uint256 i = 0; i < user.deposits.length; i++) {
			...
			if (user.checkpoint < finish) {
				uint256 share = user.deposits[i].amount.mul(plans[user.deposits[i].plan].percent).div(DIVIDER*10);
	...
	function getSiteInfo() public view returns(uint256 _totalInvested, uint256 _totalBonus) {
		return(totalInvested, totalInvested.mul(TOTAL_REF).div(DIVIDER*10));
	}
