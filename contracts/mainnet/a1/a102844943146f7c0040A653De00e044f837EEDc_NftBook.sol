...
contract NftStaking is  Ownable {
  ...
  function initialize(address _addr,address _nftAddress,uint256 _rewardPerDay) public{
...
contract NftBook is Ownable {
  ...
  function withdrawRemainingToken(IBEP20 token,address _recipient) public onlyOwner {
    ...
    function deployNewPool(
        ...
        NftStaking newpool=new NftStaking();
        newpool.initialize(_earningTokenAddress,_nftAddress,_rewardPerDay);
        deployedPools[msg.sender][counter[msg.sender]]=address(newpool);
        newpool.transferOwnership(deployer);
