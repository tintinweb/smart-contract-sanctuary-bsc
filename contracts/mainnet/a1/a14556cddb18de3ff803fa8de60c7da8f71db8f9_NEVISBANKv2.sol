...
contract NEVISBANKv2  is ERC20,Ownable  {
	...
	struct User {
        ...
        uint256 team;
        uint256 reinvestseedtotal;
        uint256 withdrawseedtotal;
	}
	...
	function invest(address referrer, uint8 plan ,uint256 _amount) external payable {
        ...
        if (user.referrer == address(0)) {
                    if (users[referrer].deposits.length > 0 && referrer != msg.sender) {
                        ...
                        uint256 amount = _amount.mul(REFERRAL_PERCENTSS).div(PERCENTS_DIVIDER);
  ...
  function swapAndLiquify(uint256 contractTokenBalance) private  {
    ...
    function getUserAmountOfRoll_seed(address userAddress) public view returns(uint256) {
		return users[userAddress].reinvestseedtotal;
	}
    ...
    function getUserAvailable(address userAddress) public view returns(uint256) {
       User storage user = users[userAddress]; 
		return (user.totalBonus.sub(user.withdrawnbonus)).add(getUserDividends(userAddress).add(getcurrentseedincome(userAddress)));
	}
    ...
    function getinfo() public view returns(uint256 paid) {
		...
		for(uint256 i = 0; i < investors.length; i++){
             ...
             totalamount += getUserDividends(investors[i]).add(user.bonus.add(user.withdrawn).add(user.reinvest.add(user.withdrawseedtotal.add(user.reinvestseedtotal)))).add(getUserSeedIncome(investors[i]));
        }
          return totalamount;
	}
	...
	uint256 public REFERRAL_PERCENTSS;
    ...
    function getUserSeedDepositInfo(address userAddress, uint256 index) public view returns(address owneraddress,address downlineaddress,uint256 level, uint256 amount, uint256 _roiIncome,uint256 percentage, uint256 start,uint256 finish,uint256 seedshare) {
	    User storage user = users[userAddress];
        if(users[userAddress].seedDeposit.length > 0)
        {
            ...
            finish =  user.seedDeposit[index].start.add(SEED_DAYS.mul(1 days));
            seedshare = _roiIncome.mul(user.seedDeposit[index].perc).div(PERCENTS_DIVIDER);
    ...
    function getUserSeedIncome(address userAddress) public view returns (uint256){
        ...
        for (uint256 i = 0; i < user.seedDeposit.length; i++) 
               {
 				 uint256 finish = user.seedDeposit[i].start.add(user.seedDeposit[i].seeddays.mul(1 days));
                    ...
                    seedshare = share.mul(user.seedDeposit[i].perc).div(PERCENTS_DIVIDER);
                        ...
                        if (from < to) {
                            ...
                            FinalAmount=FinalAmount.add(seedshare.mul(daysDiff));
    ...
    function setseedpercentage(uint256 _percentage) public onlyOwner(){
        SEED_PERCENTS = _percentage;
    }

    function setseeddays(uint256 _Days) public onlyOwner(){
        SEED_DAYS = _Days;     
    }

    function setrefpercentage(uint256 _percentage) public onlyOwner(){
        REFERRAL_PERCENTSS = _percentage;     
    } 

    function withdraw_seed() public {
		User storage user = users[msg.sender];

		uint256 referralBonus = getUserReferralBonus(msg.sender);
        uint256 totalAmount=getcurrentseedincome(msg.sender);
		...
		require(totalAmount > 0, "User has no Referral income");
        user.withdrawseedtotal = user.withdrawseedtotal.add(totalAmount);
        user.withdrawnseed = user.withdrawnseed.add(getcurrentseedincome(msg.sender));
	    swaptoken(totalAmount);
        uint256 fee = totalAmount.mul(PROJECT_FEE).div(PERCENTS_DIVIDER);
        uint256 famount=totalAmount.sub(fee);
        token.transfer(msg.sender,famount);
    ...
    function reinvest_seed() public{
        ...
        require(totalAmount > 0, "User has no Referral income");
        user.reinvestseedtotal = user.reinvestseedtotal.add(totalAmount);
        user.withdrawnseed = user.withdrawnseed.add(getcurrentseedincome(msg.sender));
    ...
    function getUserAvailableDivident(address userAddress) public view returns(uint256) {
		return getUserDividends(userAddress);
	}

    function getUserAvailable_seed(address userAddress) public view returns(uint256) {
       User storage user = users[userAddress]; 
		return (user.totalBonus.sub(user.withdrawnbonus)).add(getcurrentseedincome(userAddress));
	}


    function getUserSeedIncome_byid(address userAddress,uint256 indexid) public view returns (uint256){
	
        uint256 seedshare;
        uint256 FinalAmount;
	    User storage user =users[userAddress];
       
 				 uint256 finish = user.seedDeposit[indexid].start.add(user.seedDeposit[indexid].seeddays.mul(1 days));
				     if (user.seedDeposit[indexid].start < finish) {
                    uint256 share = user.seedDeposit[indexid]._roiIncome;
					uint256 from = user.seedDeposit[indexid].start;
					uint256 to = finish < block.timestamp ? finish : block.timestamp;
                    uint daysDiff = (to - from) / 60 / 60/ 24;
                    seedshare = share.mul(user.seedDeposit[indexid].perc).div(PERCENTS_DIVIDER);
