...
contract JDB is ERC20, Ownable {
    ...
    uint256 public maxSellTransactionAmount = 15000000 * (10**9); 
    uint256 public maxBuyTransactionAmount = 15000000 * (10**9);
    uint256 public swapTokensAtAmount = 1000000 * (10**9);
    uint256 public maxWalletToken = 15000000 * (10**9);

    uint256 public liquidityFee = 1;
    uint256 public MarketingFee = 2;
    uint256 private devFee = 2;

    uint256 private totalFees = liquidityFee.add(MarketingFee).add(devFee);

    address payable public MarketingWallet = payable(0x7bfaD1a6358872de51298f4752bAea427d12dCc9);
    address payable private devWallet = payable(0x325a7544BB0c46DB9de43fB754BAbbc7f9a9338f);
    ...
    constructor() ERC20("JDB Test", "JDB") {
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "JDB Test: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "JDB Test: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "JDB Test: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "JDB Test: Automated market maker pair is already set to that value");
    ...
    function _transfer(
     ...
     function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        MarketingWallet.transfer(address(this).balance.mul(MarketingFee).div(MarketingFee.add(devFee)));
    ...
    function setFee(uint256 _liquidityFee, uint256 _MarketingFee, uint256 _devFee) public onlyOwner {
        liquidityFee = _liquidityFee;
        MarketingFee = _MarketingFee;
        devFee = _devFee;
        totalFees = liquidityFee.add(MarketingFee).add(devFee);
    }
