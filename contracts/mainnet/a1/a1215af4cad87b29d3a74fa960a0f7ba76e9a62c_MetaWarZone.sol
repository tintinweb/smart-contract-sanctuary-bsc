 ...
 * METAWARZONE
 * 
 * Website: Metawar.zone
 * Telegram: https://t.me/MetawarZoneBscOfficial
 * Dev: https://t.me/j_thedev
 *
 ...
 *  Decimals: 18
 *  Token Name: METAWARZONE
 *  Symbol: MWZone
 * .___  ___.  _______ .___________.    ___   ____    __    ____  ___      .______      ________    ______   .__   __.  _______ 
 * |   \/   | |   ____||           |   /   \  \   \  /  \  /   / /   \     |   _  \    |       /   /  __  \  |  \ |  | |   ____|
 * |  \  /  | |  |__   `---|  |----`  /  ^  \  \   \/    \/   / /  ^  \    |  |_)  |   `---/  /   |  |  |  | |   \|  | |  |__   
 * |  |\/|  | |   __|      |  |      /  /_\  \  \            / /  /_\  \   |      /       /  /    |  |  |  | |  . `  | |   __|  
 * |  |  |  | |  |____     |  |     /  _____  \  \    /\    / /  _____  \  |  |\  \----. /  /----.|  `--'  | |  |\   | |  |____ 
 * |__|  |__| |_______|    |__|    /__/     \__\  \__/  \__/ /__/     \__\ | _| `._____|/________| \______/  |__| \__| |_______|
 *                                                                                                                              
 * Buy Tax 10%
 *  Game development  2%
 *  Marketing         4%
...
/* Token contract */
contract MetaWarZone is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    // Addresses
    address GAME = 0x6d241BCACE93Aee07f005162D54ebD7B90926Df2;
    address TEAM = 0x5aB1a213Cb4182382C834Ec7c3D4855362B60394;
    address MARKETING = 0xa4ca3cdE00Cd7c42206d6B223530A0828a263c5D;
    
    address public autoLiquidityReceiver;

    // Name and symbol
    string constant _name = "METAWARZONE";
    string constant _symbol = "MWZ";
    ...
    uint256 gameFeeBuy = 200;
    uint256 teamFeeBuy = 100;
    ...
    uint256 gameFeeSell = 200;
    uint256 teamFeeSell = 100;
    ...
    uint256 public swapThreshold = _totalSupply / 500; // 0.2% of supply
    ...
    function checkMaxWallet(address sender, address recipient, uint256 amount) internal view {
        if (!authorizations[sender] && recipient != owner && recipient != address(this) 
    ...
    function swapBack() internal swapping {
        ...
        (bool successMarketing, /* bytes memory data */) = payable(MARKETING).call{value: amountBNBMarketing, gas: 30000}("");
        (bool successGame, /* bytes memory data */) = payable(GAME).call{value: amountBNBGame, gas: 30000}(""); 
        ...
        require(successGame, "Game receiver rejected ETH transfer");
        require(successTeam, "Team receiver rejected ETH transfer");

        checkLiquidity(amountToLiquify, amountBNBLiquidity);
    }

    function checkLiquidity(uint256 amountToLiquify, uint256 amountBNBLiquidity) internal{
    ...
    // Set our buy fees
    function setBuyFees(uint256 _liquidityFeeBuy, uint256 _buybackFeeBuy, uint256 _reflectionFeeBuy, uint256 _marketingFeeBuy, uint256 _gameFeeBuy, uint256 _teamFeeBuy, uint256 _feeDenominator) external authorized {
        liquidityFeeBuy = _liquidityFeeBuy;
        buybackFeeBuy = _buybackFeeBuy;
        reflectionFeeBuy = _reflectionFeeBuy;
        marketingFeeBuy = _marketingFeeBuy;
        gameFeeBuy = _gameFeeBuy;
        teamFeeBuy = _teamFeeBuy;
        uint256 totalDevelopmentFee = _gameFeeBuy.add(_teamFeeBuy);
        totalFeeBuy = _liquidityFeeBuy.add(_buybackFeeBuy).add(_reflectionFeeBuy).add(_marketingFeeBuy).add(totalDevelopmentFee);
        feeDenominator = _feeDenominator;
    }

    // Set our sell fees
    function setSellFees(uint256 _liquidityFeeSell, uint256 _buybackFeeSell, uint256 _reflectionFeeSell, uint256 _marketingFeeSell, uint256 _gameFeeSell, uint256 _teamFeeSell, uint256 _feeDenominator) external authorized {
        liquidityFeeSell = _liquidityFeeSell;
        buybackFeeSell = _buybackFeeSell;
        reflectionFeeSell = _reflectionFeeSell;
        marketingFeeSell = _marketingFeeSell;
        gameFeeSell = _gameFeeSell;
        teamFeeSell = _teamFeeSell;
        uint256 totalDevelopmentFee = _gameFeeSell.add(_teamFeeSell);
        totalFeeSell = _liquidityFeeSell.add(_buybackFeeSell).add(_reflectionFeeSell).add(_marketingFeeSell).add(totalDevelopmentFee);
        feeDenominator = _feeDenominator;
    }

    // Set the marketing and liquidity receivers
    function setFeeReceivers(address _autoLiquidityReceiver, address _MARKETING) external authorized {
