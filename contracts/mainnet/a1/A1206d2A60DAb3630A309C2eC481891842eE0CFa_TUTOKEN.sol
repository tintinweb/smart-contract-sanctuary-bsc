...
contract TUTOKEN is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    TUTOKENDividendTracker public dividendTracker;
    ...
    constructor() BEP20("TU TOKEN", "TUT") 
    {
        dividendTracker = new TUTOKENDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "TUTOKEN: The dividend tracker already has that address");
        TUTOKENDividendTracker newDividendTracker = TUTOKENDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "TUTOKEN: The new dividend tracker must be owned by the TUTOKEN token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "TUTOKEN: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "TUTOKEN: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "TUTOKEN: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "TUTOKEN: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "TUTOKEN: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "TUTOKEN: Cannot update gasForProcessing to same value");
...
contract TUTOKENDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("TUTOKEN_Dividend_Tracker", "TUTOKEN_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "TUTOKEN_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "TUTOKEN_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TUTOKEN contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "TUTOKEN_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "TUTOKEN_Dividend_Tracker: Cannot update claimWait to same value");
