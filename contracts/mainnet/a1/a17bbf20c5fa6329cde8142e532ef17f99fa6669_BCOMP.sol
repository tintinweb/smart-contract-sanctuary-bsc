...
contract BCOMP is Context, IERC20, Ownable {
    ...
    address public marketingAndLPWallet;
    ...
    event SwapTokensForBUSD(uint256 amountIn, address[] path);

    constructor() Ownable(msg.sender) {
        marketingAndLPWallet = 0xD22bC59cE30811aDaC1C8AE2c35f4C043c01982A;
    ...
    function setMarketingAndLPWallet(address account) external onlyOwner {
        emit UpdateMarketingWallet(marketingAndLPWallet, account);
        marketingAndLPWallet = account;
    }
    ...
    function setBuyFee(uint256 _fee) external onlyOwner {
        ...
        emit UpdatedBuyFees(buyFees, _fee);
        buyFees = _fee;
    }

    function setSellFee(uint256 _fee) external onlyOwner {
        require(_fee < 30, "value can not be over 30");
        emit UpdatedSellFees(sellFees, _fee);
        sellFees = _fee;
    }
    ...
    function _takeFees(uint256 amount, bool isSell) internal returns (uint256) {
        uint256 tradingFee = isSell ? sellFees : buyFees;
        uint256 marketingAndLPFee = amount.mul(tradingFee).div(100);

        _balances[marketingAndLPWallet] = _balances[marketingAndLPWallet].add(
            marketingAndLPFee
        );
