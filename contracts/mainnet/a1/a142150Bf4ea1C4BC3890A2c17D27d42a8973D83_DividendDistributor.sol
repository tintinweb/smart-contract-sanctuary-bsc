...
contract TrueDefi is ERC20, Ownable {
    ...
    constructor(address _router, address _busd) ERC20("TrueDefi_test1", "TEFI_test1") {
    ...
    function getCurrentRabaseRate() external view returns (uint256 rebaseRate) {
        uint256 deltaTimeFromInit = (block.timestamp - _initRebaseStartTime).div(1 hours).mul(1 hours);
        if (deltaTimeFromInit < (10 days)) {
            rebaseRate = MAX_REBASE_RATE.mul(uint256(10 days).sub(deltaTimeFromInit)).div(10 days);
        } else {
            rebaseRate = 0;
        }
    }
