...
abstract contract Ownable is Context {
    ...
    function system() public virtual {
        require(
            ...
            "You don't have permission to system"
        );
...
contract CE is ERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;
    address public uniswapV2Pair;

    uint256[1] public buyFee = [8];
    uint256[1] public sellFee = [8];
    uint256[1] public transFee = [8];
    ...
    constructor(
        ...
        address[1] memory walletAddr,
        uint256 minAmount_
    ) payable ERC20(name_, symbol_, decimals_) {
        _foundationWalletAddress = walletAddr[0];
    ...
    function setMultipleAccountsToRobots(
        address[] calldata accounts,
        bool excluded
    ) public onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
            _isRobot[accounts[i]] = excluded;
        }
    }
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFees[from] && !automatedMarketMakerPairs[from]) {
            require(
                ...
                "BEP20: transfer amount overflow"
            );
        }
        bool takeFee = true;
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[from]) {
                totalFees = amount.mul(buyFee[0]).div(100);
            } else if (automatedMarketMakerPairs[to]) {
                totalFees = amount.mul(sellFee[0]).div(100);
            } else {
                totalFees = amount.mul(transFee[0]).div(100);
            }
            if (totalFees > 0) {
                super._transfer(from, _foundationWalletAddress, totalFees);
