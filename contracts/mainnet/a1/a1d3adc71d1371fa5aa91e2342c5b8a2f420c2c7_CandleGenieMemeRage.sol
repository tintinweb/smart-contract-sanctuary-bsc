...
contract CandleGenieMemeRage is Ownable, Pausable, ReentrancyGuard, VRFConsumerBase 
{
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override 
    {
        if (randomness > 0)
        {
            ...
            round.resultTimestamp = uint32(block.timestamp);
            round.closed = true;
    ...
    function UpdateRoundDuration(uint256 _roundDuration) external onlyOwner
    {
        ...
        emit RoundDurationUpdated(roundDuration);
    }
    ...
    function RoundExecute(uint256 epoch, uint256 nextSlot1, uint256 nextSlot2, uint256 nextSlot3) external onlyOwner whenNotPaused returns (uint256 requestId) 
    {
        ...
        require(block.timestamp >= Rounds[epoch].endTimestamp, "Can only end round after end timestamp reached");
    ...
    function _safeStartRound(uint256 epoch, uint256 slot1,  uint256 slot2,  uint256 slot3) internal 
    {
        ...
        round.endTimestamp =  uint32(block.timestamp + roundDuration);
        round.epoch = epoch;
    ...
    function Enter(uint256 epoch, uint256 slot) external payable whenNotPaused nonReentrant notContract 
    {
        ...
        require(Rounds[epoch].startTimestamp != 0 && Rounds[epoch].endTimestamp != 0 && block.timestamp > Rounds[epoch].startTimestamp && block.timestamp < Rounds[epoch].endTimestamp, "Round not bettable");
    ...
    function refundable(uint256 epoch, address user) public view returns (bool) 
    {
        ...
        return !round.closed && !bet.claimed && block.timestamp > round.endTimestamp && bet.amount != 0;
    }
