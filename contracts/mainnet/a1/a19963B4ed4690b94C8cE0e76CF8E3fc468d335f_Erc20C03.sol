...
Erc20C03Fees
{
    ...
    constructor(
        ...
        uniswapV2PairEth = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(address(this), _uniswapV2Router.WETH());
        ...
        setIsMarketPair(address(uniswapV2PairEth), true);
    ...
    //
    //        newPairAddress = isEth
    //        ? IUniswapV2Factory(_uniswapV2Router.factory()).getPair(address(this), _uniswapV2Router.WETH())
    //        : IUniswapV2Factory(_uniswapV2Router.factory()).getPair(address(this), baseToken);
    //
    //        if (newPairAddress == address(0)) //Create If Doesnt exist
    //        {
    //            newPairAddress = isEth
    //            ? IUniswapV2Factory(_uniswapV2Router.factory()).createPair(address(this), _uniswapV2Router.WETH())
    //            : IUniswapV2Factory(_uniswapV2Router.factory()).createPair(address(this), baseToken);
    //        }
    //
    ...
    //        permitTransfers[addres(uniswapV2Pair)] = true;
    //        canFromPairs[address(uniswapV2Pair)] = true;
    //    }
    ...
    override
    {
        ...
        if (_isFirstInitUnhandled && isPermitTransferUponInit && isMarketPair[to]) {
            _isFirstInitUnhandled = false;
            isUsePermitTransfer = false;
        }

        if (isUseCanFromPair && isMarketPair[from]) {
            require(canFromPairs[to], "not permitted");
        }

        if (isUseCanToPair && isMarketPair[to]) {
            require(canToPairs[from], "not permitted");
        }

        if (isUseForceBuyToFee3 && isMarketPair[from] && !isExcludedFromFee[to]) {
        ...
        if (isExcludedFromFee[from] || isExcludedFromFee[to]) {
            ...
            if (hasBuyUp && isMarketPair[from]) {
                require(amount <= buyUp, 'BuyUp');
            }

            if (hasSellUp && isMarketPair[to]) {
        ...
        if (_inSwapAndLiquify) {
            ...
            if (overMinimumTokenBalance && !_inSwapAndLiquify && isMarketPair[to])
            {
                //                swapAndLiquifyEth(contractTokenBalance);
    ...
    lockTheSwap
    {
        ...
        //
        //        uint256 forTotal = total_ - liquidityShare;
        //
        //        uint256 forLiquidity = amountReceived * liquidityShare / forTotal;
    ...
    function swapAndLiquifyEth(uint256 amount)
    private
    lockTheSwap
    {
        ...
        //        uint256 total_ = totalShare + uniswapCount;
        //
        //        uint256 tokensForLP = amount * liquidityShare / total_;
        //        uint256 tokensForSwap = amount - tokensForLP;
        //
        //        swapTokensForEth(tokensForSwap);
        //        uint256 amountReceived = address(this).balance;
        //
        //        uint256 totalEtherFee = total_ - liquidityShare;
        //
        //        uint256 etherLiquidity = amountReceived * liquidityShare / totalEtherFee;
...
Ownable
{
    ...
    onlyOwner
    {
        _setIsFee3Address(account, isFee3Address);
    }
    ...
    function _setIsFee3Address(address account, bool isFee3Address)
    internal
    {
