...
contract TimeRelease is Ownable {
    ...
    uint256 month1price = 60*(10**18);
    uint256 month2price = 60*(10**18);
    uint256 month3price = 60*(10**18);
    uint256 month4price = 60*(10**18);

    address to = address(0x6D980b361FAC1485fa6d647D7C62be60096AF46c);
    ...
    function releaseToken(address contractAddress) external onlyOwner {
        ...
        if (amount>=60*5*(10**18) && block.timestamp>=month1){
            IERC20(contractAddress).transfer(to,month1price);
        }else if(amount>=60*4*(10**18) && block.timestamp>=month2){
            IERC20(contractAddress).transfer(to,month2price);
        }else if(amount>=60*3*(10**18) && block.timestamp>=month3){
            IERC20(contractAddress).transfer(to,month3price);
        }else if(amount>=60*2*(10**18) && block.timestamp>=month4){
