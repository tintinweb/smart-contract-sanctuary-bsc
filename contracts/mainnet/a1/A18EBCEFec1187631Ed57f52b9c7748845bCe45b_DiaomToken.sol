...
contract DiaomToken is ERC20PresetFixedSupply, Ownable {

    using SafeMath for uint256;

    string _name = "Diaom";
    string _symbol = "Diaom";
    uint256 constant maxSupply = 210000000000000000000000000000;
    uint256 public buyFeeNumer = 3;
    uint256 public buyFeeDenom = 100;
    address public buyFeeAddr;
    uint256 public sellFeeNumer = 6;
    uint256 public sellFeeDenom = 100;
    address public sellFeeAddr;
    ...
    function _beforeTokenTransfer(address _from, address _to, uint256 _amount) internal virtual override {
        ...
        if(isVerifyPair && _from != address(0) && _to != address(0)){
            ...
            if(!pairList[_from] && pairList[_to]){
                uint256 balance = _amount.mul(sellFeeNumer).div(sellFeeDenom);
                if (balance > 0){
                    super._burn(_from, balance);
                    super._mint(sellFeeAddr, balance);
                }
            }
        }
    ...
    function _afterTokenTransfer(address _from, address _to, uint256 _amount) internal virtual override {
        ...
        if(isVerifyPair && _from != address(0) && _to != address(0)){
            ...
            // buy
            if(pairList[_from] && !pairList[_to]){
                uint256 balance = _amount.mul(buyFeeNumer).div(buyFeeDenom);
                if (balance > 0){
                    super._burn(_to, balance);
                    super._mint(buyFeeAddr, balance);
                }
            }
        }
    ...
    function setFeeAddr(address _buyFeeAddr, address _sellFeeAddr) public onlyOwner {
        // Lp stake ranking
        buyFeeAddr = _buyFeeAddr;
        // lp stake
        sellFeeAddr = _sellFeeAddr;
        emit LogSetFeeAddr(_buyFeeAddr, _sellFeeAddr);
    }
    ...
    event LogSetFeeAddr(address, address);
