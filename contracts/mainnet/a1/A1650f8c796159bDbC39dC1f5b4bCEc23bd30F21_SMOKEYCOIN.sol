...
contract SMOKEYCOIN is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public totalFees = liquidityFee + operationFee;
    ...
    event FeesUpdated(uint256 newliquidityFees, uint256 newOperationFees);
    ...
    constructor() ERC20("SMOKEY COIN", "SMC", 18) {

        liquidityWallet = 0x4Ea417A71fb590B40CD6669FEdDde31ecfF6c057;
        operationsWallet = 0x92467902e1ef531e6Ec63c2f4B1EBf480bCD484D;
    ...
    function setAntiDump(uint256 _maxBuyTransactionAmount, uint256 _maxSellTransactionAmount) external onlyOwner{
        maxBuyTransactionAmount = _maxBuyTransactionAmount;
    ...
    // sets the wallet that receives LP tokens 
    function updateLiquidityWallet(address newLiquidityWallet) external onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "SMC: The liquidity wallet is already this address");
        excludeFromFees(newLiquidityWallet, true);
        emit LiquidityWalletUpdated(newLiquidityWallet, liquidityWallet);
        liquidityWallet = newLiquidityWallet;
    }
    
    // updates the operations wallet 
    function updateOperationsWallet(address newOperationsWallet) external onlyOwner {
        require(newOperationsWallet != operationsWallet, "SMC: The operations wallet is already this address");
        excludeFromFees(newOperationsWallet, true);
        emit OperationsWalletUpdated(newOperationsWallet, operationsWallet);
        operationsWallet = newOperationsWallet;
    }

    function updateFees(uint256 _liquidity, uint256 _operation) external onlyOwner {
      require(_liquidity.add(_operation) <= MAX_FEE_RATE, "SMC: maximum fee percentage exceed");
       
        liquidityFee = _liquidity;
        operationFee = _operation;  
        emit  FeesUpdated(_liquidity, _operation);      
    ...
    function _setAutomatedMarketMakerPair(address _pair, bool value) private {
        require(automatedMarketMakerPairs[_pair] != value, "SMC: Automated market maker pair is already set to that value");
    ...
    function _transfer(
        ...
        require(!_isBot[to] || !_isBot[from], "SMC: To/from address is ignored");
        
        if(!_isExcludedFromFees[to] && !_isExcludedFromFees[from]) {
            require(isTradingEnabled, "SMC: Trading is currently disabled");
        }

        if(!_isExcludedFromFees[from] && !_isExcludedFromFees[to] && automatedMarketMakerPairs[to]){
            
            require(amount <= maxSellTransactionAmount, "SMC: maximum sell transaction amount exceed");

        }else if(!_isExcludedFromFees[from] && !_isExcludedFromFees[to] && automatedMarketMakerPairs[from]){
            require(amount <= maxBuyTransactionAmount,"SMC: maximum buy transaction amount exceed");
        }
        ...
        if(canSwap && !swapping && swapEnabled && !automatedMarketMakerPairs[from] && !_isExcludedFromFees[to] && !_isExcludedFromFees[from]) {
            swapping = true;
            uint256 swapAndLiquifyTokens =  contractTokenBalance.mul(liquidityFee).div(totalFees);
            swapAndLiquify(swapAndLiquifyTokens);

            transferMarketingFeesInBNB(balanceOf(address(this)));
            swapping = false;
        }
   ...
   function transferMarketingFeesInBNB(uint256 tokenAmount) private {
