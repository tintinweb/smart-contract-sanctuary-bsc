...
// File contracts/interfaces/IMouseHauntStashing.sol
...
// File contracts/store/sigGuardian/StoreSigValidator.sol

pragma solidity ^0.8.9;

contract StoreSigValidator {
  ...
  /**
   * @dev Data structure of a web3 item
   * @param address_ The address of the web3 item
   * @param amount The amount of the web3 item
   */
  struct Web3Item {
    address address_;
    uint256 amount;
   ...
   * @param guardianAddress the address of this contract
   * @param networkDescriptor which network are we on?
   * @param playerNonce sequential number of reqs for that player
   * @param purchaseId id of the order in our backend
   * @param playerAddress address involved in this operation
   * @param currency what will the user pay with
   * @param amount how much the users has to pay in that currency
   * @param web3Items a list of Web3Item objects in json format
   */
  struct ClaimData {
    ...
    address[] web3ItemsAddresses;
    uint256[] web3ItemsAmounts;
  }

  function _doValidateClaim(
    bytes32 hashDigest,
    ...
    uint256 claimID = _getClaimID(
      ...
      claimData.web3ItemsAddresses,
      claimData.web3ItemsAmounts
    );
  ...
  function _getClaimID(
    ...
    address currency,
    uint256 amountToCharge,
    address[] calldata web3ItemsAddresses,
    uint256[] calldata web3ItemsAmounts
...
// File contracts/store/sigGuardian/StoreSigGuardian.sol

pragma solidity ^0.8.9;

contract StoreSigGuardian is StoreSigValidator {
  ...
  function validateClaim(
    ...
    _doValidateClaim(hashDigest, claimData, signatureData);
  }
}
...
// File contracts/store/MouseHauntStore.sol

pragma solidity 0.8.9;

/**
 * @title MouseHauntStore
 * @dev Sells items for an ERC20 token
 */
contract MouseHauntStore is
  Initializable,
  ...
  Blocklistable
{
  ...
  event PurchaseCancelled(int64 purchaseId, address playerAddress);
  ...
  function buy(
    ...
    for (uint256 i = 0; i < claimData.web3ItemsAddresses.length; i++) {
      _deliverWeb3Item(msg.sender, claimData.web3ItemsAddresses[i], claimData.web3ItemsAmounts[i]);
    }

    _incrementPlayerNonce(claimData.playerAddress);

    emit Web3ItemsDelivered(
      claimData.purchaseId,
      claimData.playerAddress,
      claimData.currency,
      claimData.amountToCharge
  ...
  function cancel(
    ...
    emit PurchaseCancelled(claimData.purchaseId, claimData.playerAddress);
  }

  function _deliverWeb3Item(
    address to,
    address what,
    uint256 amount
  ) internal {
    IERC20(what).transfer(to, amount);
  }
