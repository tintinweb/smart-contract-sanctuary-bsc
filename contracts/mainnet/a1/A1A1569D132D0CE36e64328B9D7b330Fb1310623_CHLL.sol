...
contract CHLL {
	...
	uint256 public PAYOUT_TIME = 3600;
	...
	constructor(address _BUSD_address) {
        ...
        info.lastPayout = block.timestamp;
	}
	function setPayoutTime(uint256 _seconds) external {
		PAYOUT_TIME = _seconds;
	}
	...
	function withdraw() external returns (uint256) {
        ...
        info.users[msg.sender].scaledPayout += int256(_nonReferralRewards * MAGNITUDE);
	...
	function restake(address _referrer) external returns (uint256) {
        ...
        info.users[msg.sender].referralEarned += info.users[msg.sender].referralEarnings;
        info.users[msg.sender].referralEarnings = 0;
		info.users[msg.sender].scaledPayout += int256(_rewards * MAGNITUDE);
	...
	function rewardsOf(address _user) public view returns (uint256) {
        uint256 _latestBlock = block.timestamp;
        uint256 _globalRewards = calculateRewards(_latestBlock);
        uint256 _currentScaledBUSDPerToken = (info.totalSupply > 0 && _globalRewards > 0 ? info.scaledBUSDPerToken + _globalRewards * MAGNITUDE / info.totalSupply : info.scaledBUSDPerToken);
		uint256 _rewards = uint256(info.users[_user].referralEarnings) + (uint256(int256(_currentScaledBUSDPerToken * balanceOf(_user)) - info.users[_user].scaledPayout) / MAGNITUDE);
        return _rewards;
	}
	...
	function allInfoFor(address _user) public view returns (uint256 contractBalance, uint256 globalRewardsPool, uint256 lastPayout, uint256 totalTokenSupply, uint256 truePrice, uint256 mintPrice, uint256 burnValue, uint256 userBUSD, uint256 userAllowance, uint256 userBalance, uint256 userReferralEarnings, uint256 userRewards, uint256 userLiquidValue) {
		contractBalance = info.busd.balanceOf(address(this));
		globalRewardsPool = uint256(info.globalRewardsPool);
		lastPayout = uint256(info.lastPayout);
		...
		userLiquidValue = calculatePrice(userBalance, false) + userRewards;
	}
	...
	function calculatePrice(uint256 _amount, bool _staking) public view returns (uint256) {
		uint256 _mintPrice;
		uint256 _burnValue;
		( , _mintPrice, _burnValue) = currentPrices();
		uint256 _increment = INCREMENT * (_staking ? (100 / (100 - STAKE_FEE)) : (100 - UNSTAKE_FEE / 100));
		if (_staking) {
				return (_sqrt((_increment + 2 * _mintPrice) * (_increment + 2 * _mintPrice) + 8 * _amount * _increment) - _increment - 2 * _mintPrice) * 1e18 / (2 * _increment);
		} else {
				return (_burnValue * _amount - (_increment * (_amount + 1e18) / 2e18) * _amount) / 1e18;
		}
	}

    function calculateRewards(uint256 _latestBlock) public view returns (uint256) {
		uint256 _timeToPay = _latestBlock - info.lastPayout;
        if (info.globalRewardsPool > 0 && _timeToPay > 0) {
			return min(info.globalRewardsPool, (info.globalRewardsPool * _timeToPay / PAYOUT_TIME));
        } else {
            return 0;
        } 
    }
	...
	function _transfer(address _from, address _to, uint256 _tokens) internal returns (bool) {
        ...
        uint256 _globalRewards = calculateRewards(_latestBlock);
        if (_globalRewards > 0) {
			info.globalRewardsPool -= _globalRewards;
            info.scaledBUSDPerToken += _globalRewards * MAGNITUDE / info.totalSupply;
        }
        info.lastPayout = _latestBlock;
    ...
    require(msg.sender != _referrer);
		...
		tokens = calculatePrice(_amount, true);
        ...
        info.lastPayout = _latestBlock;
        info.globalRewardsPool += _taxAfterReferral;
	...
	function _unstake(uint256 _tokens) internal returns (uint256 amount) {
		...
		amount = calculatePrice(_tokens, false);
