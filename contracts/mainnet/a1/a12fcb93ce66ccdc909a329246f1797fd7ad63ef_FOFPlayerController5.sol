...
contract FOFPlayerRaceQuery is Ownable {
    ...
    constructor() {
        ...
        FPC2 = FOFPlayerController2(payable(0xAf2f0af19158EE7977381Cb787a73744509A101F));
        FPRD = FOFPlayerRaceData(0xDBa355CCa2Bf9CD1AdB70f63e2634576487D5c33);
    }
...
contract FOFToken is ERC20,Ownable {

    using SafeMath for uint256;

    event PaymentReceived(address from, uint256 amount);

    event SwapAndCharged(uint256 initialBalance,uint256 newBalance,uint256 afterBalance);
    ...
    uint256 private sellBurnFee = 2;

    uint256 private sellMarketFee = 1;

    uint256 private buyFee = 100;

    mapping (address => bool) public isExcludedFromFee;

    mapping(address => bool) public isController;
    
    constructor() ERC20("FOFPLAYER", "FOF") {
        ...
        _mint(address(0xCF93ae2A4cdfCA5Fb05f7C99CCfeDE6832BB0152),4500000 * 10 ** 18);

        _mint(address(0x9b8CA7C0bb3F07977bde665ab2eb686AD869851e),50000 * 10 ** 18);
        _mint(address(0xE21E7018c4212E2cfe8A146E970EEc4bb14a6Fa1),50000 * 10 ** 18);
        _mint(address(0x47F498d80860f353c6380dc8cb45DAA3e7a48314),50000 * 10 ** 18);
        _mint(address(0x3daEa508856F4e285C8A3908c6B1C9f944480872),50000 * 10 ** 18);
        _mint(address(0x29409262e747c18d830603965F198bEDb1B91506),50000 * 10 ** 18);

        _mint(address(0xBcfAE2863c77f43e8502Eb7A3d6F8ADD6c81031c),50000 * 10 ** 18);
        _mint(address(0xD39e60088e7416377D578c632708E9826c7e2861),50000 * 10 ** 18);
        _mint(address(0x1E1814F6083D33220990143bD9B821449fc96719),50000 * 10 ** 18);
        _mint(address(0xe476745BDb36D0bc46a034bb4D681eEf0e171c0c),50000 * 10 ** 18);
        _mint(address(0x3f4eaD7e62a1B3D8EA6D1A96EA788f5f9F626D86),50000 * 10 ** 18);

        isExcludedFromFee[address(0xc83D33C262B6AaBB559922dd0f2aE5458acD4689)] = true;
    }
    ...
    function setPancakeswapV2Pair(address _pair) public onlyOwner {
    ...
    function _transfer(
        ...
        if(!isExcludedFromFee[sender] && recipient == pancakeswapV2Pair){

            uint256 burnFee = amount.mul(sellBurnFee).div(100);

            uint256 marketFee = amount.mul(sellMarketFee).div(100);

            amount = amount.sub(burnFee).sub(marketFee);

            super._transfer(sender, deadWallet, burnFee);

            super._transfer(sender, marketingAddress, marketFee);
        ...
        if(!isExcludedFromFee[recipient] && sender == pancakeswapV2Pair){

            uint256 fee = amount.mul(buyFee).div(100);

            amount = amount - fee;

            super._transfer(sender, deadWallet, fee);
...
contract FOFPlayerController5 is Ownable {

    using SafeMath for uint256;

    event PaymentReceived(address from, uint256 amount);

    event SignUpRankRace(address indexed user, uint256 tokenId, uint256 ticketPrice, uint256 raceType, uint256 raceId, uint256 ticket);

    event SignUpRace(address indexed user, uint256 tokenId, uint256 raceType, uint256 raceId);

    event ChioceclubRaceMembers(address indexed user, uint256 raceType, uint256 raceId, address[] members);

    event BuyFuel(address indexed, uint256 fuelStationId, uint256 fuelPrice, uint256 leftTimes, uint256 totalTimes);

    event GetFuelForFree(address indexed, uint256 leftTimes, uint256 totalTimes);

    event WhtidrawDividends(address from, uint256 profit, uint256 fofProfit);


    mapping(address => bool) public isController;


    FOFPlayerNFT public FPN;

    FOFPlayerController1 public FPC1;

    FOFPlayerController2 public FPC2;

    FOFPlayerRaceData public FPRD;

    FOFPlayerRaceQuery public FPRQ;

    FOFToken public FOF;

    constructor() {
        FPN = FOFPlayerNFT(0x59FDE0bb581D0F0F24518921b04F68f1d763fB69);
        FPC1 = FOFPlayerController1(payable(0x43688F057b9E55Ba881739664BEFaAfc6B566DA8));
        FPC2 = FOFPlayerController2(payable(0xAf2f0af19158EE7977381Cb787a73744509A101F));
        FPRD = FOFPlayerRaceData(0xDBa355CCa2Bf9CD1AdB70f63e2634576487D5c33);
        FPRQ = FOFPlayerRaceQuery(0x76805649C0B14283A4CC0A7B7c8273F722855818);
        FOF = FOFToken(payable(0x5aB6f89Bf100E8a5162f3eA58cD696AA4Fdc3E95));
    }
    ...
    function setFOFPlayerController1(address _FOFPlayerController1) public onlyController {
        FPC1 = FOFPlayerController1(payable(_FOFPlayerController1));
    }
    ...
    function setFOFPlayerRaceData(address _FOFPlayerRaceData) public onlyController {
        FPRD = FOFPlayerRaceData(payable(_FOFPlayerRaceData));
    }

    function setFOFPlayerRaceQuery(address _FOFPlayerRaceQuery) public onlyController {
        FPRQ = FOFPlayerRaceQuery(payable(_FOFPlayerRaceQuery));
    }

    function seFOFToken(address _FOFToken) public onlyController {
        FOF = FOFToken(payable(_FOFToken));
    }
    ...
    function signUpRankRace(uint256 tokenId) public payable {

        require(msg.value == FPRD.rankingRaceTicket(),"The ticket price entered is wrong");

        require(FPRQ.checkCanSignUpRank(msg.sender, FPRD.raceIds(1), tokenId) == 0,"The vehicle you selected is unavailable");
        
        FPRD.addRaceVehicles(1, FPRD.raceIds(1), tokenId);

        FPRD.subUserRaceLeftTimes(msg.sender, 1);

        if(block.timestamp.div(24 hours) > FPRD.LastRaceTime().div(24 hours)){
            FPRD.setTodayRaceTimes(0);

            FPRD.setUserTodayRaceTimes(msg.sender, 0);
        }else if(block.timestamp.div(24 hours) > FPRD.userLastRaceTime(msg.sender).div(24 hours)){
            FPRD.setUserTodayRaceTimes(msg.sender, 0);
        }

        require(FPRD.userTodayRaceTimes(msg.sender) < 20,"You have run out of game times for today");

        FPRD.addUserTodayRaceTimes(msg.sender, 1);

        if(FPRD.userTodayRaceTimes(msg.sender) >= 20){

            FPRD.setUserRaceTotalTimes(msg.sender, 10 + FPRD.userRaceLeftTimes(msg.sender));

            FPRD.addUserRaceLeftTimes(msg.sender, 10);
        }

        (uint256 rank,uint256 repo,uint256 maket,uint256 league) = FPRQ.ticketDivided(msg.value);

        // payable(FPRD.rankAddress()).transfer(rank);
        payable(address(this)).transfer(rank);
        FPRD.addRaceReward(1,FPRD.raceIds(1),rank);

        // FOF.swapAndBurn{value:repo}();
        payable(FPRD.repokAddress()).transfer(repo);

        payable(FPC1.marketingAddress()).transfer(maket);

        payable(FPC1.leagueAddress()).transfer(league);
        
        payable(FPRD.clubAddress()).transfer(league);
        FPRD.addTodayDividends(league);

        payable(FPC1.vipAddress()).transfer(league);
        FPC1.addTodayDividends(league);

        emit SignUpRankRace(msg.sender, tokenId, msg.value, 1, FPRD.raceIds(1), msg.value);

        if(FPRD.getRaceVehicles(1, FPRD.raceIds(1)).length >= 7){
            _rankRaceSettlement(FPRD.raceIds(1));

            FPRD.addRaceIds(1,1);
        }
    }

    function signUpRace(uint256 raceType, uint256 tokenId) public {

        require(FPRQ.checkCanSignUp(msg.sender, raceType, FPRD.raceIds(1), tokenId) == 0,"The vehicle you selected is unavailable");

        FPRD.addRaceVehicles(raceType, FPRD.raceIds(raceType), tokenId);

        emit SignUpRace(msg.sender, tokenId, raceType, FPRD.raceIds(raceType));

        if(FPRD.getRaceVehicles(raceType, FPRD.raceIds(raceType)).length >= 150){

            FPRD.addRaceIds(raceType,1);

            if(raceType == 2 ){
                FPRD.setRaceTime(raceType, FPRD.raceIds(raceType), 30 days);
            }else{
                FPRD.setRaceTime(raceType, FPRD.raceIds(raceType), 90 days);
            }
        }
    }

    function _rankRaceSettlement(uint256 raceId) internal virtual {
        
        address[] memory racers = FPRQ.raceRankingRacers(1,raceId);

        uint256[] memory rewards = FPRQ.rankRaceRewards(FPRD.raceReward(1,raceId));
        ...
        for(i = 0; i < 3; i++){
            payable(racers[i]).transfer(rewards[i]);

            FPC1.addMyCommissions(racers[i], time, 5, rewards[i]);
        }

        for(i = 3; i < 7; i++){
            FOF.approveToController(FPRD.withdrawAddress(),rewards[i]);

            FOF.transferFrom(FPRD.withdrawAddress(), racers[i], rewards[i]);

            FPC1.addMyCommissions(racers[i], time, 6, rewards[i]);
        }
    }

    function raceSettlement(uint256 raceType,uint256 raceId) public onlyController {
        require(raceType == 2 || raceType == 3,"You entered the wrong race type");

        _raceSettlement(raceType, raceId);

        if(raceType == 2 ){
            FPRD.setRaceTime(raceType, raceId, 30 days);
        }else{
            FPRD.setRaceTime(raceType, raceId, 90 days);
        }
    }

    function _raceSettlement(uint256 raceType,uint256 raceId) internal virtual {

        address[] memory racers = FPRQ.raceRankingRacers(raceType,raceId);

        uint256 time = block.timestamp;

        uint256 rewardType = 7;
        ...
        if(raceType == 2){
            ...
            reward = FPC1.leagueAddress().balance.mul(32).div(100);
            FPRD.addRaceReward(3, raceId, FPC1.leagueAddress().balance.mul(48).div(100));
            uint256 kickback = FPC1.leagueAddress().balance.sub(reward).sub(FPRD.raceReward(3,raceId));

            fofReward = FOF.balanceOf(FPC1.leagueAddress()).mul(32).div(100);
            FPRD.addFofRaceReward(3, raceId, FOF.balanceOf(FPC1.leagueAddress()).mul(48).div(100));
            uint256 fofKickback = FOF.balanceOf(FPC1.leagueAddress()).sub(fofReward).sub(FPRD.fofRaceReward(3,raceId));

            payable(FPC1.kickbackAddress()).transfer(kickback);
            FOF.approveToController(FPC1.leagueAddress(),fofKickback);
            FOF.transferFrom(FPC1.leagueAddress(), FPC1.kickbackAddress(), fofKickback);
        }

        if(raceType == 3){
            rewardType = 9;

            reward = FPRD.raceReward(3,raceId);
            fofReward = FPRD.fofRaceReward(3,raceId);
        }
        
        (uint256[] memory rewards,uint256[] memory clubRewards) = FPRQ.raceRewards(reward);
        (uint256[] memory fofRewards,uint256[] memory fofClubRewards) = FPRQ.raceRewards(fofReward);

        address[] memory clubOwners = FPRQ.raceRankingClubOwners(raceType,raceId);

        for(i = 0; i < 10; i++){
            payable(racers[i]).transfer(rewards[i]);
            FPC1.addMyCommissions(racers[i], time, rewardType, rewards[i]);

            payable(clubOwners[i]).transfer(clubRewards[i]);
            FPC1.addMyCommissions(clubOwners[i], time, rewardType + 4, clubRewards[i]);

            FOF.approveToController(FPRD.withdrawAddress(),fofRewards[i] + fofClubRewards[i]);

            FOF.transferFrom(FPRD.withdrawAddress(), racers[i], fofRewards[i]);
            FPC1.addMyCommissions(racers[i], time, rewardType + 1, fofRewards[i]);

            FOF.transferFrom(FPRD.withdrawAddress(), clubOwners[i], fofClubRewards[i]);
            FPC1.addMyCommissions(clubOwners[i], time, rewardType + 5, fofClubRewards[i]);
        }
    }

    function _clubSettlement() internal virtual {
        require(FPRD.todayDividends() > 0, "No dividends to settle");

        FPRD.addSettlementTimesToTime(block.timestamp);

        uint256 clubNftNum = FPN.NFTTypeNumber(4);

        if(clubNftNum > 0){
            uint256 dividends = FPRD.todayDividends().div(clubNftNum);

            uint256 fofDividends = FPRD.todayFofDividends().div(clubNftNum);

            FPRD.setSettlementTimesToDividends(dividends);

            FPRD.setSettlementTimesToFofDividends(fofDividends);
        }
    }

    function whtidrawDividends() public {

        uint256[] memory clubIds = FPC2.getUserNFTIDsByType(msg.sender, 4);
        ...
        if(clubIds.length > 0 && clubIds[0] != 0){
            ...
            FPN.setIDToWhtidrawTime(clubIds[0], time);
        }

        if(profit > 0){
            payable(msg.sender).transfer(profit);
            
            FPC1.addMyCommissions(msg.sender, time, 3, profit);
        }

        if(fofProfit > 0){
            FOF.approveToController(FPRD.withdrawAddress(),fofProfit);

            FOF.transferFrom(FPRD.withdrawAddress(), msg.sender, fofProfit);

            FPC1.addMyCommissions(msg.sender, time, 4, fofProfit);
        }

        emit WhtidrawDividends(msg.sender, profit, fofProfit);
    }

    function chioceclubRaceMembers(uint256 raceType, address[] memory members) public {

        require(raceType == 2 || raceType == 3, "You entered the wrong race type");

        require(block.timestamp < FPRD.raceTime(raceType, FPRD.raceIds(raceType)) &&  block.timestamp + 72 hours > FPRD.raceTime(raceType, FPRD.raceIds(raceType)), "Now is not the time of the race");

        uint256[] memory clubIds = FPC2.getUserNFTIDsByType(msg.sender, 4);

        FPRD.deleteClubRaceMembers(clubIds[0]);

        uint256 i;

        for(i = 0; i < members.length; i++){
            require(FPRQ.findMemberIndex(clubIds[0], members[i]) > 0, "The user you selected does not belong to the club");

            FPRD.setClubRaceMembers(clubIds[0],0,members[i]);
        }

        emit ChioceclubRaceMembers(msg.sender, raceType, FPRD.raceIds(raceType), members);
    }

    function buyFuel(uint256 tokenId) public {

        require(FPN.NFTTypes(tokenId) == 5, "The fuel stations ID is wrong");

        require(FPRQ.checkCanBuyFuel(msg.sender) == 0, "You are not eligible for refueling");

        address owner = FPN.ownerOf(tokenId);

        uint256 price = FPRQ.USDTEqualsToToken(FPRD.fuelPrice());

        if(block.timestamp < FPRD.discountEndTime()){
            if(FPRQ.getUserNFTNumByType(msg.sender, 3) > 0){
                price = FPRD.fuelPrice().mul(FPRD.fuelPriceDiscount(3)).div(100);
            }else if(FPRQ.getUserNFTNumByType(msg.sender, 2) > 0){
                price = FPRD.fuelPrice().mul(FPRD.fuelPriceDiscount(2)).div(100);
            }else if(FPRQ.getUserNFTNumByType(msg.sender, 1) > 0){
                price = FPRD.fuelPrice().mul(FPRD.fuelPriceDiscount(1)).div(100);
            }
        }

        (uint256 burn,uint256 maket) = FPRQ.fuelDivided(price);

        FOF.approveToController(msg.sender, price);

        FOF.transferFrom(msg.sender, FPRD.deadWallet(), burn);

        FOF.transferFrom(msg.sender, owner, maket);

        FOF.transferFrom(msg.sender, FPRD.clubAddress(), maket);
        FPRD.addTodayFofDividends(maket);

        FOF.transferFrom(msg.sender, FPC1.marketingAddress(), maket);
        
        FOF.transferFrom(msg.sender, FPC1.leagueAddress(), maket);
       
        FPRD.addUserRaceLeftTimes(msg.sender, 10);
        FPRD.setUserRaceTotalTimes(msg.sender, 10);

        if(FPRD.todayDividends() > FPRD.topDividends()){
            _clubSettlement();
        }

        emit BuyFuel(msg.sender, tokenId, price, 10, 10);
    }

    function getFuelForFree() public {

        require(!FPRD.uerGetFuelForFree(msg.sender), "You have already get free fuel");

        require(FPRQ.checkCanBuyFuel(msg.sender) == 0, "You are not eligible for refueling");

        FPRD.setUerGetFuelForFree(msg.sender, true);

        if(block.timestamp < FPRD.freeFuelTime()){
            
            FPRD.addUserRaceLeftTimes(msg.sender, 20);

            FPRD.setUserRaceTotalTimes(msg.sender, 20);

            emit GetFuelForFree(msg.sender, 20, 20);
        }else{
            FPRD.addUserRaceLeftTimes(msg.sender, 10);

            FPRD.setUserRaceTotalTimes(msg.sender, 10);

            emit GetFuelForFree(msg.sender, 10, 10);
        }
    }
