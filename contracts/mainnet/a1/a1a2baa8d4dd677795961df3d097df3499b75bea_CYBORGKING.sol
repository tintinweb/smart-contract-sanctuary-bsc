...
contract CYBORGKING is Context, IERC20, Ownable {
    ...
    address public Cyborg_Marketing_Wallet = 0x17aD993Eb9184fE08AAEb4cE28e57Cfefa3a7FCe;
    address public Cyborg_Team_Wallet = 0xf9A080110f594a25C347618cbC425930B6bF882b;
    
    uint256 public genesis_block;
    
    string private constant _name = "Cyborg King";
    string private constant _symbol = "CYBORG";

    // Supply Holder Configuration
    uint256 public Cyborg_max_WalletLimit = 20000000000 * 10**9;
    uint256 public swapTokensAtAmount = 2000000000 * 10**9;
    uint256 public Cyborg_max_BuyLimit = 10000000000 * 10**9;
    uint256 public Cyborg_max_SellLimit = 10000000000 * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[Cyborg_Marketing_Wallet] = true;
        _isExcludedFromFee[Cyborg_Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[Cyborg_Marketing_Wallet] = true;
        allowedTransfer[Cyborg_Team_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= Cyborg_max_BuyLimit, "You are exceeding Cyborg_max_BuyLimit");
            require(balanceOf(to) + amount <= Cyborg_max_WalletLimit, "You are exceeding Cyborg_max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= Cyborg_max_SellLimit, "You are exceeding Cyborg_max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= Cyborg_max_WalletLimit, "You are exceeding Cyborg_max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(Cyborg_Marketing_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(Cyborg_Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function Cyborg_Update_MarketingWallet(address newWallet) external onlyOwner{
        Cyborg_Marketing_Wallet = newWallet;
    }
    
    function Cyborg_Update_TeamWallet(address newWallet) external onlyOwner{
        Cyborg_Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        Cyborg_max_BuyLimit = maxBuy * 10**decimals();
        Cyborg_max_SellLimit = maxSell * 10**decimals();
    }
    
    function updateCyborg_Max_Walletlimit(uint256 amount) external onlyOwner{
        Cyborg_max_WalletLimit = amount * 10**decimals();
    }
