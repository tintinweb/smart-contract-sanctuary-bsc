...
// KennedyNFT
contract KennedyMinter {
    address public kennedyMinter;

    modifier onlyKennedyMinter() {
        require(msg.sender == kennedyMinter, 'Defender: minter error');
        _;
    }

    function transferKennedyMinter(address newMinter) public onlyKennedyMinter {
        if (newMinter != address(0)) {
            kennedyMinter = newMinter;
        }
    }

    function renounceMinter() public onlyKennedyMinter {
        kennedyMinter = address(0);
    }
}

contract KennedyNFT is ERC721A, Ownable, KennedyMinter {
    ...
    function mintByContract(uint16 _num, address recipient) public onlyKennedyMinter{
        require(totalSupply() + _num <= maxTotal,"Exceeds maximum supply");
        _safeMint(recipient, _num);
    }
