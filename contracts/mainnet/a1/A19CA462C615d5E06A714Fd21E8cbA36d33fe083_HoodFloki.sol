...
Telegram: https://t.me/HoodFloki_BSC
...
contract HoodFloki is Context, IBEP20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address payable private marketingAddress =
        payable(0x85E88Dc4AcBF3e3b5842D1C69bd62C5d41C4C15A); // Marketing Address

    address payable private teamAddress =
        payable(0x17cB3BE7F5837292fDF81997971354178A68234C); // Team Address

    address payable public liquidityAddress =
        payable(0x17cB3BE7F5837292fDF81997971354178A68234C); // Liquidity Address
    ...
    string private constant _name = "HoodFloki";
    ...
    constructor() {
        ...
        maxTransactionAmount = _tTotal * 40 / 1000; // 4% maxTransactionAmountTxn
    ...
    function _transfer(
     ...
     if(limitsInEffect){
        if (
            ...
            if(from != owner() && to != PancakeSwapV2Pair && block.number == tradingActiveBlock){
                boughtEarly[to] = true;
                emit BoughtEarly(to);
            }
        ...
        onlyOwner
    {
        ...
        require(_buyTaxFee + _buyLiquidityFee + _buyMarketingFee + _buyTeamFee <= 15, "Must keep taxes below 20%");
        ...
        returns (address _pair)
    {
        ...
        _pair = IPancakeSwapV2Factory(_PancakeSwapV2Router.factory()).getPair(
            address(this),
            _PancakeSwapV2Router.WETH()
        );
        if (_pair == address(0)) {
            // Pair doesn't exist
            _pair = IPancakeSwapV2Factory(_PancakeSwapV2Router.factory()).createPair(
                address(this),
                _PancakeSwapV2Router.WETH()
            );
        }
