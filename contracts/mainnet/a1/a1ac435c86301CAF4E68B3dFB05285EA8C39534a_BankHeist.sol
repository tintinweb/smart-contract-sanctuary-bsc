    ...
    ManageableUpgradeable
{
    ...
    function getAvailableRewards(address user)
        public
        view
        returns (uint256[] memory values, uint256[] memory indexes)
    {
                ...
                if (lock[i].claim < rewards_[j].timestamp) {
                    for (uint256 k = 0; k < tokens.length; k++) {
                        if (rewards_[j].token == tokens[k]) {
                            values[k] +=
                                (rewards_[j].amount * lock[i].amount) /
                                rewards_[j].totalSupply;
    ...
    function _claimRewards(address user) internal {
        ...
        ) = getAvailableRewards(user);
        for (uint256 i = 0; i < indexes.length; i++) {
            lockedTokens[_msgSender()][indexes[i]].claim = block.timestamp;
        }
        for (uint256 i = 0; i < values.length; i++) {
            IERC20Upgradeable(tokens[i]).transferFrom(BANK, user, values[i]);
        }
    }
    ...
    function stake(uint256 tierId, uint256 amount) public {
        ...
        uint256 amountOfTokens = amount * tiers[tierId].allocation;
        ...
        _mint(_msgSender(), amountOfTokens);
    }
    ...
    function unstake(uint256 lockId) public {
        ...
        tiers[lock.tier].totalStaked -= lock.amountOfLP;
        _burn(_msgSender(), lock.amountOfLP);
        LP.transferFrom(BANK, _msgSender(), lock.amountOfLP);
    }
    ...
    function increaseStake(uint256 lockId, uint256 amount) public {
        ...
        uint256 amountOfTokens = amount * tiers[lock.tier].allocation;
        lock.amountOfLP += amount;
        lock.amount += amountOfTokens;
    ...
    function moveStake(uint256 lockId, uint256 tierId) public {
        ...
        tiers[lock.tier].totalStaked -= lock.amountOfLP;
        tiers[tierId].totalStaked += lock.amountOfLP;
        stakings[_msgSender()][tierId] += 1;
        stakings[_msgSender()][lock.tier] -= 1;
        require(
            stakings[_msgSender()][tierId] <= maxStakingsPerTier,
        ...
        _claimRewards(_msgSender());
        uint256 temp = lock.amount;
        uint256 amountOfTokens = lock.amountOfLP * tiers[tierId].allocation;
        lock.tier = tierId;
        lock.amount = amountOfTokens;
        lock.start = block.timestamp;
        lock.release = block.timestamp + tiers[tierId].duration;
        lockedTokens[_msgSender()][lockId] = lock;
        _mint(_msgSender(), amountOfTokens - temp);
    }
