...
contract BIG {
    ...
    uint256 private constant feePercents = 400;  // development and maintenance fees 
    ...
    uint256 private constant dayPerCycle = 20 days; 
   ...
   // address public feeReceiver1; 
    ...
    constructor(address _usdtAddr, address _defaultRefer, address  _feeReceivers,address  _feeReceivers2) public {
    ...
    function _updateTopUser(address _user, uint256 _amount, uint256 _dayNow) private {
        ...
        bool updated;
        for(uint256 i = 0; i < 3; i++){ // calculating top 3 users
    ...
    function _deposit(address _user, uint256 _amount) private {
        ...
        }else if(user.maxDeposit < _amount){ // the deposited amount should be greater than users max deposit
            user.maxDeposit = _amount;
        }

        _distributeDeposit(_amount);

        if(user.totalDeposit == 0){ //updating daiuly lucky users, top users
            uint256 dayNow = getCurDay();
    ...
    function _distributeDeposit(uint256 _amount) private { // distributes deposted amount to fee receiver addresss, adding the amount portion to luck, star and top pool.
        uint256 fee = _amount.mul(feePercents).div(baseDivider);
        usdt.transfer(feeReceivers, fee.div(2));
        usdt.transfer(feeReceivers2, fee.div(2));
        
        uint256 luck = _amount.mul(luckPoolPercents).div(baseDivider); // 0.5
        luckPool = luckPool.add(luck);
        uint256 star = _amount.mul(starPoolPercents).div(baseDivider); //0.3
        starPool = starPool.add(star);
        uint256 top = _amount.mul(topPoolPercents).div(baseDivider); // 0.2
