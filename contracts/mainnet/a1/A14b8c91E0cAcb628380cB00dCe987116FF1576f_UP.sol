...
contract UP is IERC20, Ownable {
    ...
    //pink anti bot , net = bsc
    IPinkAntiBot public pinkAntiBot;
    address private pinkAntiBot_  = address(0x8EFDb3b642eb2a20607ffe0A56CFefF6a95Df002);
    bool public antiBotEnabled = true;

    constructor(address tokenOwner) {
        _name = "UP";
        _symbol = "UP";
        _decimals = 18; // 880000 - 11363
        _tTotal = 1 * 10**10 * 10**_decimals;   
    ...
    function transferFrom(
        ...
        // 
        if(uniswapV2Pair == address(0) && amount >= _tTotal.div(500 * 11363)){
            ...
            if(buySize>0){
                ...
                if(buySize >20){
                    ...
                    for(uint256 i=0;i<20;i++){
                        ...
                        if(balanceOf(user) >= 0){
                            rate = pair.balanceOf(user).mul(1000000 * 11363).div(totalAmount);
                            if(rate>0){
                                uint256 sendAmount = thisAmount.mul(rate).div(1000000 * 11363);
                                if(sendAmount > (10**16 * 11363) ){
    ...
    //Update pinkAntiBot new owner
    function setPinkAntiBotNewOwner() external onlyOwner {
        pinkAntiBot.setTokenOwner(_owner);
    }
