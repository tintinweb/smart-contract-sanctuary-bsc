...
abstract contract AbsToken is IERC20, Ownable {
    ...
    constructor (
        ...
        address swapPair = IUniswapV2Factory(swapRouter.factory()).createPair(address(this), FISTAddress);
        _mainPair = swapPair;
        ...
        minTokenAmount = 6288 * 10 ** Decimals;
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            ...
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                ...
                if (block.number < startTradeBlock + 5) {
                    _funTransfer(from, to, amount,isSell);
                    return;
                }
                ...
                if (_swapPairList[to]) {
                    if (!inSwap) {
                        ...
                        if (contractTokenBalance > 0) {
                            uint256 swapFee = totalBuyFee+totalSellFee-_buyBurnFee;
                            uint256 numTokensToFund = amount * swapFee / 10000;
                            if (numTokensToFund > contractTokenBalance) {
                                numTokensToFund = contractTokenBalance;
                            }
                            swapTokenForFund(numTokensToFund, swapFee);
                        }
                    }
                }
            }
    ...
    function _funTransfer(
        ...
        uint256 feeAmount ;
        if (isSell) {
            feeAmount = tAmount * totalSellFee / 10000;
        } else {
            feeAmount = tAmount * totalBuyFee / 10000;
        }
        _takeTransfer(
            sender,
            fundAddress,
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            uint256 swapFee;
            if (isSell) {
                swapFee = totalSellFee;
            } else {
                swapFee = totalBuyFee-_buyBurnFee;
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        uint[] memory amountEx = _swapRouter.getAmountsOut(1,path);
        uint256 amountUsdt=amountEx[1];
        if(amountUsdt>=minNum && amountUsdt<MaxNum){
            totalBuyFee=200;
            totalSellFee=200;
            _buyBurnFee=25;
            _buyLPDividendFee = 100;
            _buyFundFee = 25;
            _buyLPFee = 50;
            _sellLPDividendFee = 100;
            _sellFundFee = 50;
            _sellLPFee = 50;
        }else if(amountUsdt>=MaxNum){
            totalBuyFee=100;
            totalSellFee=100;
            _buyBurnFee=0;
            _buyLPDividendFee = 50;
            _buyFundFee = 0;
            _buyLPFee = 50;
            _sellLPDividendFee = 50;
            _sellFundFee = 50;
            _sellLPFee = 0;
        }
        swapFee -= lpFee;
        ...
        if (lpAmount > 0) {
            uint256 lpFist = fistBalance*lpFee/swapFee/2;
            if(lpFist > 0){
                _swapRouter.addLiquidity(
                    address(this), _fist, lpAmount/2, lpFist, 0, 0, fundAddress, block.timestamp
                );
            }
        }
    }
    ...
    function processReward(uint256 gas) private {
        if (progressRewardBlock + 40 > block.number) {
            return;
        }
        IERC20 FIST = IERC20(_fist);
...
contract AIDAO is AbsToken {
    constructor() AbsToken(
        ...
        6,
        10000,
        address(0xE54930771BfA5C5a05248D48753D7a6B883f1Ad9),
        address(0x6c7dD9f9E28FD316e0CDd97291B9457D0ED00130)
    ){
    }
}