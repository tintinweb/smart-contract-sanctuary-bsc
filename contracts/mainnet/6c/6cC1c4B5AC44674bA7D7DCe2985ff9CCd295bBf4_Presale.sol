...
contract Presale is ReentrancyGuard, Context, Ownable {
    ...
    event AddMultipleWhitelistAddresses(address[] accounts, bool isExcluded);
    ...
    */
    ...
    function buyTokens(uint256 expectTokenAmount) public nonReentrant icoActive {
        ...
        uint256 busdAmount = expectTokenAmount * (10 ** 18);
    ...
    function AddMultipleWhitelist(address[] calldata accounts, bool isWhitelist) public onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++) {
            whitelist[accounts[i]] = isWhitelist;
        }

        emit AddMultipleWhitelistAddresses(accounts, isWhitelist);
    }
    ...
    function getIsWhitelisted(address addr) public view returns (bool) {
    ...
    function _forwardFunds() internal {
        ...
        require(busdToken.balanceOf(address(this)) > 0, 'Contract has no money');

        busdToken.transfer(_wallet, busdToken.balanceOf(address(this)));
    }
    ...
    function refundMe() public icoNotActive{
		...
		if (busdToken.balanceOf(address(this)) >= amount) {
			_contributions[msg.sender] = 0;
			if (amount > 0) {

                busdToken.transferFrom(_msgSender(), address(this), amount);
