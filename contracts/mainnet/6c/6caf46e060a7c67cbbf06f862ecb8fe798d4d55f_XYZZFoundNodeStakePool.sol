 ...
 */
contract XYZZFoundNodeStakePool is Ownable {
    ...
    function getBoostMultiplier(uint256 _indexed) public view returns (uint256 muliplier) {
        ...
        if (transaction.depositDays == duration * 86400) {
            return muliplier = multipliter;
        }
    }
    ...
    function pendingReward(uint256 _pid, address _user) external view returns(uint256 totalDeservedReward,uint256 finalyPendingReward) {
        ...
        finalyPendingReward = totalDeservedReward.sub(user.claimedReward).add(myReferralReward(_pid,msg.sender));
    ...
    function deposite(uint256 _pid,uint256 _amount,uint256 _depositDays) public {
		...
		updatePool(_pid);
        require(_depositDays == duration,"depositDays amount must be met");
