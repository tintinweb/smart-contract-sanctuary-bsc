...
contract OmniverseNFTMarketPlace is Initializable, IERC721ReceiverUpgradeable, PausableUpgradeable, OwnableUpgradeable,   ReentrancyGuardUpgradeable {
    ...
    function createItemForSale(
        ...
        // not allowed to sell omnilegends
        if(nftContract == 0x55291C33d55eC877e68a7702F1ab2C5B61f78Fa3){
            require(_msgSender()==owner(),"No allowed to sell omnilegends nft");
        }

        validateItemForSale(nftContract, tokenId, paymentToken, price, duration, extra_, saleType_, newOwner);
        ...
        idMarketItem[itemId] = MarketItem(
            itemId, nftContract, tokenId, payable(_msgSender()), 
            expectedOwner, 
            paymentToken,
            price,
            extra_,
