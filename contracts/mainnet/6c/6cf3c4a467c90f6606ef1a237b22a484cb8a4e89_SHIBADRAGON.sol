...
/**
Telegram: https://t.me/shibadragonglobal
Website: https://www.shibadragonbsc.com
*/
...
contract SHIBADRAGON is IERC20, Ownable {
    ...
    string private _name = "SHIBA DRAGON";
    string private _symbol = "SHIBAD";
    uint256 private constant INITIAL_SUPPLY = 1_000_000_000_000_000 * 10**TOKEN_DECIMALS;
    ...
    TierTaxes public _buyTaxes = TierTaxes({first: 11, second: 11, third: 11});

    TierTaxes public _sellTaxes = TierTaxes({first: 11, second: 11, third: 11});
    ...
    TaxWallets public _taxWallet =
        TaxWallets({
            Team: 0xe8d364092166A8BDffE2fCc902F1acf39Ee404E4,
            Events: 0x1B80237EB2D3D6e57870e48F65C8a9fEA264cA63,
            Operations: 0x4FA87d02CacF078ac0c14886959c903494b010bc
        });
    ...
    event UpdateOperationsWallet(address newOperationsWallet);
    ...
    constructor() {
        ...
        _excluded.add(_taxWallet.Operations);
    ...
    function withdrawOperations() external authorized {
        uint256 amount = OperationsBalance;
        OperationsBalance = 0;
        _sendETH(_taxWallet.Operations, amount);
    }
    ...
    function setOperationsWallet(address payable addr) external authorized {
        address prevOperations = _taxWallet.Operations;
        _excluded.remove(prevOperations);
        _taxWallet.Operations = addr;
        _excluded.add(_taxWallet.Operations);
        emit UpdateOperationsWallet(addr);
    }
    ...
    {
        return (buybackBalance, TeamBalance, EventsBalance, OperationsBalance);
    }
    ...
    function _distributeStake(uint256 ETHAmount, bool newStakingReward)
        private
    {
        uint256 OperationsSplit = (ETHAmount * _taxRatios.Operations) /
            distributeRatio;
        ...
        OperationsBalance += OperationsSplit;
        ...
        if (stakingSplit > 0) {
            ...
            if (totalShares == 0) OperationsBalance += stakingSplit;
            else {
