...
// Telegram: https://t.me/SquidElonGametwoPortal

// Hard-coded tax limit - taxes cannot be set over 20%
// Anti-rug feature - max tx (on Buys & Sells) and max wallet cannot be set below 0.3%
...
contract SquidElonGametwo is Context, IERC20, Ownable {
        
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "SquidElonGame2.0";
    string private _symbol = "$SEG";
    uint8 private _decimals = 9;

    address payable public marketingWalletAddress = payable(0x564878154A4dB53D708eDB27eA229f02bFd8CC27);
    address payable public TeamWalletAddress = payable(0x946B699cb3b4DCd9Ac8EAc6dCE42663f43FcDe45);
    ...
    uint256 private _totalSupply = 1000 * 10**5 * 10**9;
    uint256 public _maxBuyTxAmount = 20 * 10**5 * 10**9;
    uint256 public _maxSellTxAmount = 10 * 10**5 * 10**9;
    uint256 public _walletMax = 20 * 10**5 * 10**9;
    ...
    function setBuyTaxes(uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newTeamTax) external onlyOwner() {
        ...
        _totalTaxIfBuying = _buyLiquidityFee.add(_buyMarketingFee).add(_buyTeamFee);
        require(_totalTaxIfBuying <= 100/5);
    }
    ...
    function setSellTaxes(uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newTeamTax) external onlyOwner() {
        ...
        _totalTaxIfSelling = _sellLiquidityFee.add(_sellMarketingFee).add(_sellTeamFee);
        require(_totalTaxIfSelling <= 100/5);
    ...
    function setMaxBuyTxAmount(uint256 maxBuyTxAmount) external onlyOwner() {
        require(maxBuyTxAmount >= (3 * 10**5 * 10**9));
    ...
    function setMaxSellTxAmount(uint256 maxSellTxAmount) external onlyOwner() {
        require(maxSellTxAmount >= (3 * 10**5 * 10**9));
