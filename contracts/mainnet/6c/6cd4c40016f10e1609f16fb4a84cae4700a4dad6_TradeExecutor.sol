...
contract TradeExecutor is Ownable {
    ...
    IPriceReporter private immutable priceReporter;
    IPool private immutable pool;

    address[] public feeTokens;

    constructor(address _pool, address _priceReporter) {
        require(_pool != address(0), "Invalid address");
        require(_priceReporter != address(0), "Invalid address");
        pool = IPool(_pool);
        priceReporter = IPriceReporter(_priceReporter);
    }
    ...
    function executeOrders(uint256[] calldata _orderIds, uint256[] calldata _swapOrderIds) external {
        if (_orderIds.length != 0) {
            priceReporter.postPriceAndExecuteOrders(new address[](0), new uint256[](0), _orderIds);
        }
        if (_swapOrderIds.length != 0) {
            priceReporter.executeSwapOrders(_swapOrderIds);
        }
    }
