...
contract bfnftPoolFarm {
    ...
    modifier auth {
        require(wards[msg.sender] == 1, "bfnftfarm/not-authorized");
        _;
    }
    ...
    function sub(int256 a, int256 b) internal pure returns (int256) {
        int256 c = a - b;
        require((b >= 0 && c <= a) || (b < 0 && c > a), "bffarmNFT/SignedSafeMath: subtraction overflow");

        return c;
    }
    function add(int256 a, int256 b) internal pure returns (int256) {
        int256 c = a + b;
        require((b >= 0 && c >= a) || (b < 0 && c < a), "bffarmNFT/SignedSafeMath: addition overflow");
    ...
    function harvest() public returns (uint256) {
        ...
        UserInfo storage user = userInfo[msg.sender];
        uint256 amount;
        if (user.id != 0) amount = 1;
        uint256 accumulatedlp = mul(amount,acclpPerShare) / 1e18;
    ...
    function beharvest(address usr) public view returns (uint256) {
        ...
        uint256 amount;
        if (user.id != 0) amount = 1;
        uint256 accumulatedlp = mul(amount,_acclpPerShare) / 1e18;
        uint256 _pendinglp = toUInt256(sub(int256(accumulatedlp),user.rewardDebt));
        return _pendinglp;
    }
 }