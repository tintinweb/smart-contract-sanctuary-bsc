...
abstract contract Ownable is Context {
    ...
    function check_confirmed()public view returns(bool){
         if(_owners.length > 0){
            for(uint i=0;i<_owners.length;i++){
    ...
    function setconfirmed(uint8 _confirm)public isConfirmed{
        managers[msg.sender] = _confirm;

    }
   
   
    function addManager(address manager) public onlyOwner{
       _owners.push(manager);
    }
     
    function removeManager(address manager) public isManager{
      
        for (uint i=0; i<_owners.length - 1; i++)
            if (_owners[i] == manager) {
...
abstract contract VRFv2Consumer is VRFConsumerBaseV2 {
  ...
  contract MMGNFT is ERC721, Ownable,VRFv2Consumer{
    ...
    mapping(uint256 => bool) public is_open;
        ...
        uint256 probability;
        uint256 synthesis;
        uint256 cooling_time;
        uint256 consume_number;
        uint256 life_cycle;
        uint256 total;
        string image_url;
    }
