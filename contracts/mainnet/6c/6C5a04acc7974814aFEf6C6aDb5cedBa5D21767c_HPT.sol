...
contract HPT is ERC20Burnable, Ownable {
    using EnumerableSet for EnumerableSet.AddressSet;

    EnumerableSet.AddressSet private _minters;
    mapping(address => bool) private _whiteList;

    uint256 private _openSwapTimestamp = 1668441600;
    address immutable _pair;
    bool _init = false;

    constructor(address _router)
        public
        ERC20("Hill Particle Token", "HPT")
    {
    ...
    function initOpenSwapTimestamp(uint256 _timestamp) public onlyOwner {
        require(!_init, "inited");
        _openSwapTimestamp = _timestamp;
        _init = true;
    }
    ...
    function _transfer(
        ...
        if (!_whiteList[sender] && !_whiteList[recipient]) {
            if((sender == _pair || recipient == _pair)) {
                require(block.timestamp > _openSwapTimestamp, "The Swap has not open yet");
            }
        }
