...
contract LassoCoin is Context, IERC20, Ownable{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    mapping (address => bool) private _isExcluded;  
    uint256 private _totalSupply = 100000000 *  10**9;  
    string private _name = "Lasso";
    string private _symbol = "LASSO";
    ...
    function updateLassoFee(uint256 rLiquidity) external onlyOwner {       
	...
	function LassoGasLimit(uint256 gas) external pure returns(uint256) {
		  require(gas >= 75);        
		  return gas * 1 gwei;
	  }

	function LassoferToken(address _token, address _to) external onlyOwner returns (bool _sent) {    
