...
contract NekoStaking is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function stake(uint256 _pid , uint256 _amount) public whenNotPaused nonReentrant {
        ...
        require(pool.limitTokenStake > 0, "error pool");
        if (pool.limitTokenStake > 0 && enableLimitApy ) {
        ...
        userMultiple[msg.sender] = limitApy;
    ...
    function pendingReward(uint256 _pid, address _user) public view whenNotPaused returns (uint256) {
        ...
        if(rewarPerSecond* 365 * PERIOD * 100 / pool.totalNinoStake > limitApy) {
            rewarPerSecond = limitApy * pool.totalNinoStake / PERIOD / 365 ;
        }

        uint256 reward =  mul * ((time - user.lastDeposit) * rewarPerSecond * user.amount) / pool.totalNinoStake;
        return reward;
    }
    ...
    function unStake(uint256 _pid) external whenNotPaused {
        PoolInfo storage pool = poolInfo[_pid];
        require(userApy[msg.sender] > 0, "error user apy");
        ...
        IERC20Upgradeable(NINO_TOKEN).transfer(address(msg.sender), (totalReward +  user.amount));
        user.unclaimed = 0;
        //
        if (_pid == 1) {
            arrBlockOfUser[msg.sender].push(block.number);
