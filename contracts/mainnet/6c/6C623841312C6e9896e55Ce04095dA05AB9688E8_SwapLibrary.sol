...
contract SwapLibrary {
  ...
  function getNormalizedReserves(address tokenA, address tokenB, bool stable) external view returns (uint reserveA, uint reserveB){
    (uint decimals0, uint decimals1, uint reserve0, uint reserve1,, address t0, address t1) =
    IPair(pairFor(tokenA, tokenB, stable)).metadata();
    ...
    reserveA = reserveA * (10 ** decimalsA) / 1e18;
    reserveB = reserveB * (10 ** decimalsB) / 1e18;
  }
