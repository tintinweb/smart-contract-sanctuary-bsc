...
import "./Storage.sol";
contract recovery is Storage, Ownable, AccessControl {
  ...
  function revertBalance() internal returns (uint) {
      ...
      (uint _bal0, uint _bal1) = (ERC20(iData.token0).balanceOf(address(this)),ERC20(iData.token1).balanceOf(address(this)));
  ...
  function recover() public allowAdmin {
    ...
    for (uint i=0;i < mHolders.iQueue.length;i++) {
      amountTotal += mHolders.iHolders[mHolders.iQueue[i]].amount;
    }

    uint _shareTotal;
    sendAmount[] memory _sendAmount = new sendAmount[](mHolders.iQueue.length);

    for (uint i=0;i < mHolders.iQueue.length;i++) {
      uint amt = mHolders.iHolders[mHolders.iQueue[i]].amount;
      if (amt > 0) {
        uint _amount = (_bal * ((amt*1e18)/amountTotal))/1e18;
        _shareTotal += _amount;
        _sendAmount[i] = sendAmount(mHolders.iQueue[i], _amount);
      }
    }

    if(_bal - _shareTotal < 1000) {
      for (uint i=0;i < mHolders.iQueue.length;i++) {
        address _user = mHolders.iQueue[i];
        if (_sendAmount[i].amount > 0) {
          mHolders.iHolders[_user].amount -= _sendAmount[i].amount;
          payable(_user).transfer(_sendAmount[i].amount);
          emit sendFunds_evt(_user, _sendAmount[i].amount);
        }
      }
    }
  }

  function users() public view returns(userData[] memory) {
    userData[] memory user_data = new userData[](mHolders.iQueue.length);
    for (uint i=0;i < mHolders.iQueue.length;i++) {
      user_data[i].user = mHolders.iQueue[i];
      user_data[i].data = mHolders.iHolders[mHolders.iQueue[i]];
    }
    return user_data;   
  }
}
