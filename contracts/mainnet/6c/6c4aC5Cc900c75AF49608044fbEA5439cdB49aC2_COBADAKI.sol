...
abstract contract Context {
  function _msgSender()
   internal view virtual
    returns (address) {
       return msg.sender; 
    }// wallet yang berinteraksi dengan contract
...
contract TaxPool is Ownable {
  ...
  uint256 public maxPercent = 8000;

  uint256 public devrewardPercentage = 4000;
    address public devrewardWallet = 0x3689836BAF56B892D825899B14456d31d3323E8B;
  function setDevreward (uint256 percent_, address wallet_)
    public onlyOwner { devrewardPercentage = percent_; devrewardWallet = wallet_; }
  
  uint256 public marketingPercentage = 2000;
    address public marketingWallet = 0xD7E29B8dfa43424185a80FE2B5334dA4d71B0186;
  function setMarketing (uint256 percent_, address wallet_)
    public onlyOwner { marketingPercentage = percent_; marketingWallet = wallet_; }
  
  uint256 public buybackandburnPercentage = 2000;
    address public buybackandburnWallet = 0x03a56EF9004121A8841EBfD57d53dD9C2d48F919;
  function setBuyandburn (uint256 percent_, address wallet_)
    public onlyOwner {
      buybackandburnPercentage = percent_; buybackandburnWallet = wallet_; 
    }
  ...
  function distribute() public onlyOwner swapping {
    require(devrewardPercentage + marketingPercentage + buybackandburnPercentage == maxPercent, "The sum of percentage isn't 100.");
    require(
      devrewardWallet != address(0)
    ...
    (bool sent_1, ) = payable(devrewardWallet).call
      {
        value: (amount * devrewardPercentage / maxPercent), gas: 30000
      }
      (""); require(sent_1, "Transfer wallet_1 error."); 
      balance = address(this).balance;

    (bool sent_2, ) = payable(marketingWallet).call
      {
        value: (amount * marketingPercentage / maxPercent), gas: 30000
      }
      (""); require(sent_2, "Transfer wallet_2 error."); 
      balance = address(this).balance;

    (bool sent_3, ) = payable(buybackandburnWallet).call
      {
        value: (amount * buybackandburnPercentage / maxPercent), gas: 30000
      }
      (""); require(sent_3, "Transfer wallet_3 error."); 
      balance = address(this).balance;
  }
...
interface InterfaceTaxPool {
  function setDevreward (uint256 percent_, address wallet_) external;
  ...
  , IBEP20Metadata
{
  ...
  string private _name = "COBADAKI";
  string private _symbol = "CBDK";
...
// Max wallet holding (% at launch)
  ...
  // Set the maximum wallet holding (percent of total supply)
    function set_Max_Wallet_Percent(uint256 maxWallPercent) public onlyOwner() {
...
//   address WBNB = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd;
  ...
  constructor() {
    ...
    _mint(_msgSender(), 500000000 * 10 ** uint256(_decimals));
  }
  ...
  function taxBalance() public view onlyOwner returns(uint256) 
    { 
      return iTaxPool.getBalance(); 
    }
  function distributeTax() public onlyOwner 
    { iTaxPool.distribute(); }
  function setDevrewardTax(uint256 percent_, address wallet_) public onlyOwner 
    { iTaxPool.setDevreward
      (percent_, wallet_); 
    }
  function setMarketingTax(uint256 percent_, address wallet_) public onlyOwner
    { iTaxPool.setMarketing
      (percent_, wallet_); 
    }
  function setBuyandburnTax(uint256 percent_, address wallet_) public onlyOwner 
    { iTaxPool.setBuyandburn
      (percent_, wallet_); 
    }
  function setTaxTransferOwner(address newOwner_) public onlyOwner 
    { iTaxPool.transferOwnership(newOwner_); }
  ...
  function distributeFee () private swapping 
  {
    ...
    if (_balances[address(this)] > 0) 
    {
      ...
      try _router.swapExactTokensForETHSupportingFeeOnTransferTokens(
        ...
        (bool sent, ) = payable(_taxPool).call{value: amountBNB, gas: 30000}(""); 
