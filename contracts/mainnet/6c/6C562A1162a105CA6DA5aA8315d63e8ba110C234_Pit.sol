...
contract Pit is Ownable {
    ...
    address public pitFoundationAddress;
    address public communityAddress;
    uint256 public pitMax;
    address private PitMediumAddress;
    ...
    constructor(string memory _name, string memory _symbol,address _address) {
        ...
        buySlipPoint = 6;
        sellSlipPoint = 10;
        totalDestruction = 360000000 * 10 ** decimals;
        endDestruction = 36000000 * 10 ** decimals;
        transferMaxRatio = 90;
        serviceCharge = 2;
        pitFoundationAddress = 0xEf7A24dF92737c70690a2fAA2d1bc84122845c5D;
        communityAddress = 0xB2671Ec504B20c8AC73E9E10218031357f4b6e4F;
        pitMax = 3600000 * 10 ** decimals;

        systemList[pitFoundationAddress] = true;
    ...
    function setPitMediumAddress(
        address _address
    ) public onlyOwner returns (bool) {
        PitMediumAddress = _address;
        systemList[_address] = true;
        return true;
    }
        ...
        if (totalDestruction - endDestruction > destroyNum) {
            if (isPairAddress[sender]) {
                ...
                if (!systemList[sender]) {
                    ...
                    _balances[pitFoundationAddress] += foundation_amount;
                    senderBalance = senderBalance - foundation_amount;
                    _balances[sender] = senderBalance;
                    emit Transfer(sender, pitFoundationAddress, foundation_amount);
                }
            }
        }
    ...
    function _takeFee(
        ...
        if (_types == 1) {
            ...
            _transfer(address(this), PitMediumAddress, buy_pt_num);
            uint256 usdt_profit = PitMedium(PitMediumAddress).exchangeUsdt(
                buy_pt_num
            );
