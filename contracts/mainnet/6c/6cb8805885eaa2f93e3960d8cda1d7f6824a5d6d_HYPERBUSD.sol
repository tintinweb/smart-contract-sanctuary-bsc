...
contract HYPERBUSD {
    ...
    function sellEggs() public {
        ...
        IBEP20(BUSD).transfer(ceoAddress, fee2);
        IBEP20(BUSD).transfer(ceoAddress2, fee-fee2);
        IBEP20(BUSD).transfer(address(msg.sender), SafeMath.sub(eggValue,fee));
    }
    function buyEggs(address ref, uint256 amount) public {
        require(initialized);
    
        IBEP20(BUSD).transferFrom(address(msg.sender), address(this), amount);
    ...
    function calculateEggSell(uint256 eggs) public view returns(uint256) {
        return calculateTrade(eggs,marketEggs,IBEP20(BUSD).balanceOf(address(this)));
    }
    function calculateEggBuy(uint256 eth,uint256 contractBalance) public view returns(uint256) {
        return calculateTrade(eth,contractBalance,marketEggs);
    }
    function calculateEggBuySimple(uint256 eth) public view returns(uint256){
        return calculateEggBuy(eth,IBEP20(BUSD).balanceOf(address(this)));
    }
