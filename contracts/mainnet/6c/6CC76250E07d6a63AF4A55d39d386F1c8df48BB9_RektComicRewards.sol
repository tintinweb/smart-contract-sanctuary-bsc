...
contract RektComicRewards is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  // Lists 
  uint256[200] public allTokens;
  uint256[200] public remainingIds;
  ...
  constructor(string memory name, string memory symbol, string memory baseTokenURI) ERC721(name, symbol) {
      ...
      // Save dev wallet
      _wallet = 0xe41387D8642473BC115d810dF18C16dC6b405668;
  }
  ...
  function mint(uint256 _mintAmount) public payable {
    // Checks
    require(!paused, "Rekt: minting has not started yet");
    require(_mintAmount > 0, "Rekt: you have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "Rekt: you can only mint 5 at a time");
    require(lastSupply >= _mintAmount,  "Rekt: the collection is sold out");
    if (useWhitelist && _includeToWhitelist[msg.sender]) {
      require(msg.value == 0, "Rekt: total cost needs to be 0");
      require(_mintAmount <= 1 , "Rekt: whitelisted users only get to mint one free NFT");
      require(balanceOf(msg.sender) == 0, "Rekt: It looks like you already minted. Whitelisted users can only mint one free NFT. Remove from whitelist to purchase more.");
      _randomMint(msg.sender);
      adminSetExcludeFromWhiteList(msg.sender);
      return;
    } else {
      require(msg.value >= cost * _mintAmount, "Rekt: total cost doesn't match");
    }
    // Minting
  ...
  function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
    // Checks
    require(_winner != address(0), "Rekt: zero address");
    require(_amount > 0, "Rekt: you have to mint at least one");
    require(lastSupply != 0,  "Rekt: the collection is sold out");
  ...
  function tokenURI(uint256 _id) public view virtual override returns (string memory){
    require(_exists(_id),"Rekt: URI query for nonexistent token");
