...
/*
TG: t.me/GoFreedomToken
Website: www.gofreedom.io
 ...
 * Contract Code
 */
contract GoFreedom is Context, IBEP20, Ownable {
    ...
    string private _name = "GoFreedom";
    string private _symbol = "GFT";
    uint8 private _decimals = 18;
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 111_000_000_000*(10**_decimals); // 111 B, 18 decimals
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    // AirDropper & Burn Wallet
    address public burnWallet = 0x000000000000000000000000000000000000dEaD;
    address public airDropper = 0x0000000000000000000000000000000000000000;
    // BNB distribution
    uint256 public totalLPBNB;
    uint256 public totalMarketingBNB;
    uint256 public totalDonationBNB;
    // Taxes 10 %
    uint256 public _reflectFee=2;
    uint256 private _previousReflectFee=_reflectFee;
    uint256 public _secondaryTax=8;
    ...
    uint256 private _maxWallet=5_550_000_000*(10**18); // 5.55B
    // PancakeSwap
...
// Swap and distribution
    function swapAndLiquify(uint8 _swapThreshold,bool ignoreLimits) private lockTheSwap {
        ...
        // 1% of 111 B
        uint256 maxSwapSize=(111_000_000_000*(10**_decimals))/100;
        ...
        uint256 donationBNB=remainingBNB-marketingBNB;
        totalMarketingBNB+=marketingBNB;
        totalDonationBNB+=donationBNB;
        //
...
// Transfer
    function _transfer(address from,address to,uint256 amount) private {
        ...
        if(_isExcludedFromFee[from]||_isExcludedFromFee[to]){
            ...
            require(_canTrade);
            if(isBuy) {
                require(balanceOf(to)+amount<=(_maxWalletEnabled?_maxWallet:_tTotal));
            } else if (isSell) {
...
// Owner Functions
    ...
    function ownerEnableMaxWallet(bool maxWalletEnabled) public onlyOwner {
        _maxWalletEnabled=maxWalletEnabled;
    }
    function ownerSetMaxWallet(uint256 maxWallet) public onlyOwner {
        require(maxWallet>=(_tTotal/100)); // Cannot be set lower than 1 % of total supply
        _maxWallet=maxWallet;
    }
    function ownerUpdateAMM(address AMM,bool enabled) public onlyOwner {
        _marketMakers[AMM]=enabled;
        // exclude from rewards after using this function
    }
    ...
    function ownerFixStuckBNB() public onlyOwner {
        uint256 stuckBNB=address(this).balance-(totalMarketingBNB+totalDonationBNB);
        totalMarketingBNB+=(stuckBNB>0?stuckBNB:0);
    }
    // Cannot withdraw token, only LP from contract
    ...
    function ownerUpdateSecondaryTaxes(uint8 marketingTax,uint8 liquidityTax) public onlyOwner {
        ...
        // donation tax = 100 - (liquidityTax+marketingTax)
    }
    ...
    function contractBNB() public view returns (uint256 marketingBNB,uint256 donationBNB) {
        marketingBNB=totalMarketingBNB;
        donationBNB=totalDonationBNB;
    }
    ...
    function ownerWithdrawDonationBNB(uint256 amountWei) public onlyOwner {
        require(amountWei<=totalDonationBNB);
        (bool sent,) = msg.sender.call{value: (amountWei)}("");
        require(sent);
        totalDonationBNB-=amountWei;
    }
}