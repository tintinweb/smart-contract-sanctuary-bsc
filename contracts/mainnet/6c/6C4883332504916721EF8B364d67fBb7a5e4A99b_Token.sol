...
contract Token is Ownable, ERC20 {
  ...
  constructor(string memory name, string memory symbol, uint256 supply) ERC20(name, symbol) {
    _totalSupply += supply;
    _updateBalance(_msgSender(), supply, true);
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal override {
    ...
    require(recipient != _pair || !antiBotEnabled || solvedCaptchas[sender], "Token: anti-bot captcha not solved");
    _updateBalance(sender, amount, false);

    if (recipient == _pair && !feeRemoved && _balances[_pair] != 0) { // no sell fee when providing initial liquidity
  ...
  function _verifySignature(bytes memory abiEncoded, bytes memory signature) private view {
    require(keccak256(abiEncoded).toEthSignedMessageHash().recover(signature) == signer, "Token: invalid signature");
  }
  ...
  function sendToChain(uint256 dstChain, address recipient, uint256 amount) external {
    require(amount != 0, "Token: bridging 0");
  ...
  function receiveFromChain(uint256 srcChain, uint256 nonce, address sender, uint256 amount, bytes memory signature) external {
    ...
    require(!usedNonces[srcChain][nonce], "Token: nonce already used");
