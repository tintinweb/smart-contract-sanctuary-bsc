...
contract CATGPT is ERC20, Ownable {
    ...
    constructor(
        ...
        require(
            ...
            'CATGPT: fee account from the zero address'
        );
        require(feePercent <= PRECISION, 'CATGPT: incorrect fee percent');
        require(owner != address(0), 'CATGPT: incorrect owner address');
        require(mintToAddr != address(0), 'CATGPT: incorrect mintToAddr address');
        if (_msgSender() != owner) {
    ...
    function _transfer(
        ...
        require(sender != address(0), 'CATGPT: transfer from the zero address');
        require(recipient != address(0), 'CATGPT: transfer to the zero address');
