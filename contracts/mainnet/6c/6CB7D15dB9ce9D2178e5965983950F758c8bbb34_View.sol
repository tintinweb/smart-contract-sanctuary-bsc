...
interface IUserStorage
{
    ...
    function updateUserStatus(uint userId,uint8 statusLevel) external;
...
/// DEPOSITE
    ...
    function getDepositeProfit(address acc) public override view returns(uint)
    {
        ...
        uint value = 0;
        uint daysPassed = (block.timestamp - depDate)/86400;
        if (daysPassed > 0)
        {
            value  = (((userDeposite * userPercent) / 100) / 30) * daysPassed ;
        }
        return value;
    }
///
