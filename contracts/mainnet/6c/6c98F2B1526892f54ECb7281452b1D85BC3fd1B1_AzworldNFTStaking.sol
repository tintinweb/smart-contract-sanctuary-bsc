...
abstract contract ERC721Stakeable is Ownable{
    ...
    * Rarity
    */
    uint256 internal constant RARITY_COMMON = 1;
    uint256 internal constant RARITY_RARE = 2;
    uint256 internal constant RARITY_EPIC = 3;
    uint256 internal constant RARITY_LEGENDARY = 4;
    uint256 internal constant RARITY_MYTHICAL = 5;
    uint256 internal constant RARITY_ULTIMATE = 6;
    ...
    function rewardPerToken() public view returns (uint256) {
        ...
        return
            rewardPerTokenStored + (                                         
                lastTimeRewardApplicable() - (lastUpdateTime) * (rewardRate) / (_totalWeight)
            );
    }

    function earned(address owner) internal view returns (uint256) {                   
        return _balances[owner] * (rewardPerToken() - (userRewardPerTokenPaid[owner])) + (rewards[owner]);
    }
...
contract AzworldNFTStaking is ERC721Receiver, ERC721Stakeable, ReentrancyGuard  {
    ...
    function getStakedNFT(address owner) virtual public view returns (uint256[] memory) {
    ...
    function confirmPoolAndStart(uint256 amount, uint256 duration) external stakingFinished {
        ...
        require (amount <= balance, "NFTtaking: Not enough balance to confirm");
