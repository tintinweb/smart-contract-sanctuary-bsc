...
contract colosseumstake is Ownable {
    ...
    // Colosseum token contract...
    address public Colosseum = 0x0C21AC471494dFca9a0A2406e52c6f45D3c4E0C6;
    ...
    function updateAccount(address account) private {
        ...
        uint256 conbalance = Token(Colosseum).balanceOf(address(this));
        ...
        if (pendingDivs != 0) {
            ...
            Token(Colosseum).transfer(account, pendingDivs);
    ...
    function deposit(uint256 amountToStake, uint256 _timeperiod) public {
       ...
       require(_timeperiod == 7 || _timeperiod == 15, "Invalid Time Period. it must be either 7 or 15"); 
        ...
        Token(Colosseum).transferFrom(msg.sender, address(this), amountToStake);
    ...
    function withdraw(uint256 amountToWithdraw) public {
        ...
        if (_lastClaimedTime >= MinimumWithdrawTime) {
            require(Token(Colosseum).transfer(msg.sender, amountToWithdraw), "Could not transfer tokens.");
        }
        
        if (_lastClaimedTime < MinimumWithdrawTime) {
            uint256 WithdrawFee = amountToWithdraw.div(1e3).mul(750);
            uint256 amountAfterFee = amountToWithdraw.sub(WithdrawFee);
            require(Token(Colosseum).transfer(msg.sender, amountAfterFee), "Could not transfer tokens.");
            require(Token(Colosseum).transfer(devAddress, WithdrawFee), "Could not transfer tokens.");
        }}
    ...
    function transferAnyERC20Tokens(address _tokenAddress, address _to, uint256 _amount) public onlyOwner {
        require(_tokenAddress != Colosseum, "You can't transfer Colosseum token.");
