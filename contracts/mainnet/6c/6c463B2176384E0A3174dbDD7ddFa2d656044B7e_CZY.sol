...
// File contracts/interfaces/ICZY.sol


pragma solidity ^0.8.9;

/**
 * @dev CZY token interface
 */

interface ICZY is IERC20 {
...
// File contracts/interfaces/ICZYVester.sol


pragma solidity ^0.8.9;

/**
 * @dev BOMB Stake token vester interface
 */

interface ICZYVester {
  function createVest(address user, uint256 amount) external;
}


// File contracts/interfaces/IStakeRewards.sol
...
contract CZYStake is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  function _stake(
    ...
    _amountStaked = _amountStaked == 0
      ...
      : _amountStaked;
    _amountCZYBaseEarn = _amountCZYBaseEarn == 0
      ? _amountStaked
      : _amountCZYBaseEarn;
    require(
      _amountStaked > 0 && _amountCZYBaseEarn > 0,
    ...
    stakes[_ids.current()] = Stake({
      ...
      amountCZYBaseEarn: _amountCZYBaseEarn,
  ...
  function unstake(uint256 _tokenId) public {
    ...
    if (_isUnstakingEarly) {
      ...
      if (address(stakeToken) == address(czy)) {
        czy.burn(_tokenStake.amountStaked - _earnedAmount);
      } else {
    ...
    returns (uint256)
  {
    ...
    return
      (_tokenStake.amountCZYBaseEarn * _tokenStake.apr * _secondsStaked) /
      PERCENT_DENOMENATOR /
      ONE_YEAR;
  }
  ...
  function _afterTokenTransfer(
    ...
    if (_from != address(0)) {
      ...
      rewards.setShare(_from, _tokenStake.amountCZYBaseEarn, true);
    }
    ...
    if (_to != address(0)) {
      ...
      rewards.setShare(_to, _tokenStake.amountCZYBaseEarn, false);
    }
...
// File contracts/sCZY.sol

pragma solidity ^0.8.9;

contract sCZY is CZYStake {
  constructor(
    address _czy,
    address _vester,
    address _rewards,
...
// File contracts/slCZY.sol

pragma solidity ^0.8.9;



contract slCZY is CZYStake {
  ...
  function zapAndStakeETHOnly(uint256 _lockOptIndex) external payable {
    ...
    uint256 _czyBalanceBefore = czy.balanceOf(address(this));
    IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(_uniswapRouter);

    // swap half the ETH for CZY
    ...
    _addLp(czy.balanceOf(address(this)) - _czyBalanceBefore, msg.value / 2);
    ...
    _returnExcessCZY(msg.sender, _czyBalanceBefore);

    emit ZapETHOnly(msg.sender, msg.value, _lpBalanceToStake);
  }

  function zapAndStakeETHAndCZY(uint256 _amountCZY, uint256 _lockOptIndex)
    external
    payable
  {
    require(msg.value > 0, 'need to provide ETH to zap');
    ...
    uint256 _ethBalBefore = address(this).balance - msg.value;
    uint256 _czyBalBefore = czy.balanceOf(address(this));
    czy.transferFrom(msg.sender, address(this), _amountCZY);
    uint256 _czyToProcess = czy.balanceOf(address(this)) - _czyBalBefore;

    uint256 _lpBalBefore = stakeToken.balanceOf(address(this));
    _addLp(_czyToProcess, msg.value);
    uint256 _lpBalanceToStake = stakeToken.balanceOf(address(this)) -
      _lpBalBefore;
    _stakeLp(msg.sender, _lpBalanceToStake, _lockOptIndex, false);

    _returnExcessETH(msg.sender, _ethBalBefore);
    _returnExcessCZY(msg.sender, _czyBalBefore);

    emit ZapETHAndCZY(msg.sender, msg.value, _amountCZY, _lpBalanceToStake);
  }

  function zapAndStakeCZYOnly(uint256 _amountCZY, uint256 _lockOptIndex)
    external
  {
    require(
      _aprLockOptions[_lockOptIndex].lockTime > 0,
      'cannot zap and stake CZY only without lockup period'
    );
    ...
    czy.approve(address(_uniswapV2Router), _czyToProcess / 2);
    ...
    uint256 _lpBalBefore = stakeToken.balanceOf(address(this));
    _addLp(_czyToProcess / 2, address(this).balance - _ethBalBefore);
    uint256 _lpBalanceToStake = stakeToken.balanceOf(address(this)) -
      _lpBalBefore;
    _stakeLp(msg.sender, _lpBalanceToStake, _lockOptIndex, false);
    ...
    emit ZapCZYOnly(msg.sender, _amountCZY, _lpBalanceToStake);
  }

  function _addLp(uint256 tokenAmount, uint256 ethAmount) private {
    IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(_uniswapRouter);
    czy.approve(address(_uniswapV2Router), tokenAmount);
  ...
  function _getETHToReceiveOnSellNoSlippage(uint256 _amountCZY)
    internal
    view
    returns (uint256)
  {
    ...
    (uint112 _r0, uint112 _r1, ) = pair.getReserves();
    if (pair.token0() == IUniswapV2Router02(_uniswapRouter).WETH()) {
      return (_amountCZY * _r0) / _r1;
    } else {
      return (_amountCZY * _r1) / _r0;
    }
  }
  ...
  function _stakeLp(
    ...
    uint256 czyReserves = t0 == address(czy) ? res0 : res1;
    uint256 singleSideTokenAmount = (_amountStakeToken * czyReserves) /
  ...
  function _returnExcessCZY(address _user, uint256 _initialBal) internal {
    uint256 _currentBal = czy.balanceOf(address(this));
    if (_currentBal > _initialBal) {
      czy.transfer(_user, _currentBal - _initialBal);
      require(czy.balanceOf(address(this)) >= _initialBal, 'took too much');
    }
  }
...
// File contracts/CZYVester.sol

pragma solidity ^0.8.9;


contract CZYVester is Ownable {
  ICZY private _czy;
  ...
  function withdraw(uint256 _index) external {
    ...
    _vest.withdrawsCompleted = _withdrawsAllowed;
    _czy.transfer(_user, _tokensPerWithdrawPeriod * _withdrawsToComplete);
    _czy.addToBuyTracker(
      _user,
  ...
  function getCZY() external view returns (address) {
    return address(_czy);
  }
...
// File contracts/StakeRewards.sol
...
contract StakeRewards is IStakeRewards, Ownable {
  address public czy;
  IERC721 private sCZY;
  IERC721 private slCZY;
  ...
  modifier onlyToken() {
    require(
      msg.sender == address(sCZY) || msg.sender == address(slCZY),
      'must be stake token'
    );
    _;
  }

  constructor(address _czy, address _dexRouter) {
    czy = _czy;
  ...
  function getsCZY() external view returns (address) {
    return address(sCZY);
  }

  function getslCZY() external view returns (address) {
    return address(slCZY);
  }
  ...
  function setsCZY(address _sCZY) external onlyOwner {
    sCZY = IERC721(_sCZY);
  }

  function setslCZY(address _slCZY) external onlyOwner {
    slCZY = IERC721(_slCZY);
  }
}
...
contract CZY is ICZY, ERC20, Ownable {
  ...
  sCZY private _stakedCZY;
  slCZY private _stakedCZYLP;
  CZYVester private _vester;
  ...
  modifier onlyStake() {
    require(
      address(_stakedCZY) == _msgSender() ||
        address(_stakedCZYLP) == _msgSender(),
  ...
  constructor() ERC20('CZ Yields', 'CZY') {
    ...
    _vester = new CZYVester(address(this));
    ...
    _stakedCZY = new sCZY(
      ...
      address(_vester),
      address(_rewards),
      'https://api.czyields.com/sczy/metadata/'
    );
    _stakedCZY.setPaymentAddress(msg.sender);
    _stakedCZY.setRoyaltyAddress(msg.sender);
    _stakedCZY.transferOwnership(msg.sender);

    _stakedCZYLP = new slCZY(
      uniswapV2Pair,
    ...
    _stakedCZYLP.setPaymentAddress(msg.sender);
    _stakedCZYLP.setRoyaltyAddress(msg.sender);
    _stakedCZYLP.transferOwnership(msg.sender);

    _vester.addStakingContract(address(_stakedCZY));
    _vester.addStakingContract(address(_stakedCZYLP));
    _vester.transferOwnership(msg.sender);

    _rewards.setsCZY(address(_stakedCZY));
    _rewards.setslCZY(address(_stakedCZYLP));
    ...
    isTaxExcluded[address(_stakedCZYLP)] = true; // allow zapping without taxes
  ...
  function _transfer(
    ...
    // handles transferring to a fresh wallet or wallet that hasn't bought CZY before
  ...
  function getStakedCZY() external view returns (address) {
    return address(_stakedCZY);
  }

  function getStakedCZYLP() external view returns (address) {
    return address(_stakedCZYLP);
  }
