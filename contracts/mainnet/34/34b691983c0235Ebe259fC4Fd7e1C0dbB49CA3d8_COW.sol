...
// File: SymBull_2.0/contracts/interfaces/IStakingContract.sol
...
// File: SymBull_2.0/contracts/interfaces/IPYESwapPair.sol
...
// File: SymBull_2.0/contracts/interfaces/IPYESwapFactory.sol
...
// File: SymBull_2.0/contracts/interfaces/IWETH.sol
...
// File: SymBull_2.0/contracts/interfaces/IPYE.sol
...
// File: SymBull_2.0/contracts/COW.sol
        ...
        if(tokenIndex < pairsLength) {
            ...
            if(allowanceT >= amount) {
                ...
                if(token == BUSD) {
                    ...
                    uint256 amountToSwap = marketingFeeAmount.add(liquifyFeeAmount).add(buybackFeeAmount);
                    uint256 totalFee2 = _defaultFees.marketingFee.add(_defaultFees.buybackFee).add(_defaultFees.liquifyFee);
                    swapToWBNB(amountToSwap, token);
                    uint256 fAmount = IERC20(address(WBNB)).balanceOf(address(this)).sub(balanceBefore);
                    uint256 marketingFeeAmount2 = fAmount.mul(_defaultFees.marketingFee).div(totalFee2);
                    uint256 liquifyFeeAmount2 = fAmount.mul(_defaultFees.liquifyFee).div(totalFee2);

                    IERC20(token).transfer(_defaultFees.marketingAddress, marketingFeeAmount2);
                    IERC20(token).transfer(_defaultFees.liquifyAddress, liquifyFeeAmount2);
                    ...
                    if(stakingContract != address(0)) {
                        ...
                        IERC20(BUSD).transfer(_defaultFees.liquifyAddress, reflectionBUSDFeeAmount);
                    }
                    IERC20(token).transfer(_defaultFees.liquifyAddress, liquifyFeeAmount);
                } else {
                    uint256 balanceBefore = IERC20(address(WBNB)).balanceOf(address(this));
...
//--------------------------------------BEGIN BUYBACK / BULLRUN FUNCTIONS---------|
    ...
    function setAutoLiquifySettings(bool _enabled, uint256 _period) public onlyOwner {
