...
abstract contract DFSMafiaFlags is UsingFlagsWithStorage {
    ...
    uint constant REWARD_EXEMPT = BLOCKED << 1;
...
contract DFSMafia is DFSMafiaFlags, UsingBatchTransfer, UsingERC20WithStorage, UsingPermitWithStorage, UsingUUPS {
    using AddressSet for AddressSet.Set;
    using bits for uint256;

    struct Fee {
        uint128 value;
        uint128 burnAllocation;
        uint128 liquidityAllocation;
        uint128 rewardAllocation;
    }
    ...
    function setFees(uint128 buyFee_, uint128 sellFee_, uint128 transferFee_) external requiresAdmin {
        _buyFee.value = buyFee_;
        _sellFee.value = sellFee_;
        _transferFee.value = transferFee_;
    }

    /// @notice set the fee allocation
    /// @param feeTypeName_ the name of the fee type ie "buy", "sell" or "transfer"
    ...
    function setAllocations(bytes32 feeTypeName_, uint128 liquidityAllocation_,  uint128 burnAllocation_, uint128 rewardsAllocation_) external requiresAdmin {
        Fee storage fee;
        if (feeTypeName_ == "buy") {
            fee = _buyFee;
        } else if (feeTypeName_ == "sell") {
            fee = _sellFee;
        } else if (feeTypeName_ == "transfer") {
            fee = _transferFee;
        } else {
            revert InvalidFeeTypeName();
        }
        fee.liquidityAllocation = liquidityAllocation_;
        fee.burnAllocation = burnAllocation_;
        fee.rewardAllocation = rewardsAllocation_;
    }
    ...
    function _transfer(address source_, address target_, uint256 amount_) internal override(UsingERC20, UsingBatchTransfer) returns (bool) {
        ...
        if (!flags.all(SOURCE_IS_REWARD_EXEMPT | TARGET_IS_REWARD_EXEMPT)) _rewards.process(source_, uint96(amount_), target_, uint96(_balanceOf(address(_rewards))), flags | (state != SWAP_FOR_REWARDS ? THIS_REWARD_SWAPPING_DISABLED : 0));
        unchecked {
    ...
    function _calculatePercentage(uint amount_, uint128 percentage_) internal pure returns (uint) {
