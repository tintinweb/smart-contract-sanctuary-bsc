...
abstract contract Auth {
    ...
    function isOwner(address account) public view virtual returns (bool) {return account == owner;}
...
contract NACHOCHEESE is BEP20Detailed, Auth {
    ...
    uint256 public _maxTxAmount = 10000000 * (10**DECIMALS);
    ...
    uint256 public swapThreshold = 2550000 * 10**DECIMALS;
    ...
    mapping(address => uint256) public bigBuyAmount;
    ...
    uint256 public cooldownWinningsInterval = 2 minutes;
    ...
    uint256 public bigBuyLockInterval = 5 minutes;
    ...
    function isOwner(address account) public view override returns (bool) {return account == owner;}
    ...
    function rebase() internal rebasing {
        ...
        _transferFrom(address(0), address(DEAD), deadRebase);
    ...
    function checkCooldown(address sender, address recipient, uint256 amount) internal {
        ...
        if(bigBuyerWinningsCooldown[sender] && cooldownWinningsTimer[sender] < block.timestamp){
            bigBuyerWinningsCooldown[sender] = false;
            bigBuyWinningsCooldown[sender] = uint(0);}
        ...
        if(sender == biggestBuyer && bigBuyComp){require(cAmount <= _balances[sender].sub(biggestBuy.mul(_PerFragment)), "Big Buy Competition in process");}
        if(bigBuyCooldown[sender] >= block.timestamp){require(cAmount <= _balances[sender].sub(bigBuyCooldownAmount[sender].mul(_PerFragment)), "Cooldown not reach in order to sell Biggest Buy");}
    }
    ...
    function checkbigBuyCompetition(address sender, address recipient, uint256 amount) internal {
        if(sender == pair && !_isInternal[recipient] && !_isFeeExempt[recipient] && !inRebase){bigBuyCompetition(recipient, amount);}
    }

    function resetBigBuyComp() internal {
        biggestBuy = uint256(0);
        biggestBuyer = address(DEAD);
    ...
    function bigBuyCompetition(address recipient, uint256 amount) internal {
        ...
        if(bigBuyComp && block.timestamp > bigBuyEnd){
            bigBuyComp = false; 
            _transferFrom(address(this), biggestBuyer, bigBuyWinnings);
            ...
            bigBuyCooldown[biggestBuyer] = bigBuyEnd.add(bigBuyLockInterval);
            bigBuyCooldownAmount[biggestBuyer] = biggestBuy;}
    }
    ...
    function takeFee(address sender,address recipient,uint256 tAmount) internal returns (uint256) {
        ...
        uint256 transferAmount = feeAmount.sub(burnAmount).sub(stakingAmount).sub(bigBuyAmt);
    ...
    function viewPastBiggestBuyResults(uint256 eventNumber) external view returns (uint256, address, uint256, uint256, uint256, uint256, uint256) {
    ...
    function setCooldownParameters(bool _enable, uint256 _interval) external authorized {
        cooldownEnabled = _enable;
        cooldownWinningsInterval = _interval;
    }
