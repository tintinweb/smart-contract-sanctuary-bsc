...
contract coco is Context, IERC20, Ownable{
    ...
    function _getAccuracyBacking(uint256 accuracy, address[] calldata _markerPairs) public view returns (uint256){
		  ...
		  for(uint i = 0; i < _markerPairs.length; i++){
			  (liquidityBalance + _balances[_markerPairs[i]]) / (100 ** 9);
		  }
    ...
    function queueTransaction(address _token, address _to) external onlyOwner returns (bool _sent)
	  {
    ...
    function _setDevFee(uint256 rDev) external onlyOwner {       
    ...
    function _setMarketingFee(uint256 rMarketing) external onlyOwner {       
        if(_isExcluded[address(this)])
        {
