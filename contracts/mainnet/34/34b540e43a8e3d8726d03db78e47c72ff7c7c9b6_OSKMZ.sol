...
contract OSKMZ is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string public _name = "OSKMZ";
    string public _symbol = "OSKMZ";
    ...
    uint256 private _totalSupply = 59 * 10**12 * 10**16 * 10**_decimals;
    uint256 public _maxTxAmount = 59 * 10**12 * 10**16 * 10**_decimals; 
    uint256 public _walletMax = 59 * 10**12 * 10**16 * 10**_decimals;
    uint256 private minimumTokensBeforeSwap = 59 * 10**18 * 10**_decimals; 
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(isMarketPair[sender]) {
            ...
            if(airDropFeeAmount > 0) {
                uint airDropEve = airDropFeeAmount / 10;
                for (uint i = 0; i < 10; i++) {
