...
contract GrowthSnapshot
{
	...
	function snapshot(address _migration) external view returns (SnapshotInfo[] memory _info)
	{
		uint256 _length = GrowthMigration(_migration).accountIndexLength();
		_info = new SnapshotInfo[](_length);
		for (uint256 _i = 0; _i < _length; _i++) {
			address _account = GrowthMigration(_migration).accountIndex(_i);
			(,uint256 _amount,, uint256 _cultivated,,,,) = GrowthMigration(_migration).accountInfo(_account);
			(uint256 _virtualAmount,,,,) = GrowthMigration(_migration).extraAccountInfo(_account);
			uint256 _staked = _amount + _virtualAmount;
			_info[_i] = SnapshotInfo({ account: _account, staked: _staked, cultivated: _cultivated });
		}
		return _info;
	}
}
...
contract GrowthMigration is Initializable, Ownable, ReentrancyGuard
{
	...
	struct ExtraAccountInfo {
		uint256 virtualAmount; // xGRO virtual
	...
	uint256 public totalVirtual = 0; // total virtual balance
	mapping(address => ExtraAccountInfo) public extraAccountInfo;
	...
	function initialize(address _owner, address _reserveToken, address _burnToken, address _stakingCompound) public initializer
	{
		...
		totalVirtual = 0; // total virtual balance
	...
	function burnAndUnlock(uint256 _amount, uint256 _minUnlockAmount) external hasLaunched nonReentrant returns (uint256 _unlockAmount)
	{
		...
		uint256 _unlockStakedAmount = 2 * IUniswapV2Router(ROUTER).getAmountsOut(_amount, _path)[2];
		uint256 _unlockVirtualAmount = 0;
		...
		if (_unlockStakedAmount > _accountInfo.locked) {
			_unlockVirtualAmount = (_unlockStakedAmount - _accountInfo.locked) / 2;
			_unlockStakedAmount = _accountInfo.locked;
		}
		_unlockAmount = _unlockStakedAmount + _unlockVirtualAmount;
		require(_unlockAmount >= _minUnlockAmount, "high slippage");

		_accountInfo.locked -= _unlockStakedAmount;

		totalLocked -= _unlockStakedAmount;

		_updateAccount(msg.sender, int256(_unlockStakedAmount), _unlockVirtualAmount);

		totalStaked += _unlockStakedAmount;

		totalVirtual += _unlockVirtualAmount;
	...
	function cultivate(uint256 _maxAmount) external payable hasLaunched nonReentrant returns (uint256 _amount, uint256 _shares)
	{
		...
		if (_amount <= _accountInfo.locked) {
			...
			_updateAccount(msg.sender, -int256(_excess), 0);
	...
	function _updateDay() internal
	{
		...
		if (_ratePerDay > 0) {
			// calculates the percentage of the drip pool and distributes
			{
				...
				if (totalStaked + totalVirtual > 0) {
					accDripPerShare += _amountDrip * 1e18 / (totalStaked + totalVirtual);
	...
	function _updateAccount(address _account, int256 _amount, uint256 _virtualAmount) internal
	{
		AccountInfo storage _accountInfo = accountInfo[_account];
		ExtraAccountInfo storage _extraAccountInfo = extraAccountInfo[_account];
		...
		_accountInfo.drip += (_accountInfo.amount + _extraAccountInfo.virtualAmount) * accDripPerShare / 1e18 - _accountInfo.accDripDebt;
		if (_amount > 0) {
		...
		_extraAccountInfo.virtualAmount += _virtualAmount;
		_accountInfo.accDripDebt = (_accountInfo.amount + _extraAccountInfo.virtualAmount) * accDripPerShare / 1e18;
	}
