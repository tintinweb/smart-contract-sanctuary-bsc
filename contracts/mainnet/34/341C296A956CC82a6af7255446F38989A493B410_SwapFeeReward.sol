...
interface IAnpanToken is IERC20 {
...
contract SwapFeeReward is Ownable{
    ...
    function getSwapFee(address tokenA, address tokenB) internal view returns (uint swapFee) {
        swapFee = uint(1000).sub(IANPANPair(pairFor(tokenA, tokenB)).swapFee());
    }
    ...
    function withdraw() public returns(bool){
        ...
        if (balance > 0){
            anpanToken.mint(msg.sender, balance);
    ...
    function getQuantity(address outputToken, uint256 outputAmount, address anchorToken) public view returns (uint256) {
        ...
        if (outputToken == anchorToken) {
        ...
        } else if (IANPANFactory(factory).getPair(outputToken, anchorToken) != address(0)) {
            ...
            for (uint256 index = 0; index < length; index++) {
                ...
                if (IANPANFactory(factory).getPair(outputToken, intermediate) != address(0) && IANPANFactory(factory).getPair(intermediate, anchorToken) != address(0)) {
    ...
    // Only tokens in the whitelist can be mined ANPAN
