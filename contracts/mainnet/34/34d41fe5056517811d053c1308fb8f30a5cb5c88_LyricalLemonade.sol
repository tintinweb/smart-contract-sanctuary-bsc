...
contract LyricalLemonade is IBEP20, Auth {
    ...
    string constant _name = "LyricalLemonade";
    string constant _symbol = "LL";
    ...
    uint256 marketingwallettwo = 0; 
    uint256 marketingFee = 4;
    uint256 totalFee = 6;
    uint256 feeDenominator = 100;
    uint256 public _sellMultiplier = 1;
    
    address public marketingFeeReceiver = 0xC7119315c14280b38a30B94abc839EA50ad7DdF1;
    address public marketingwallettwoReceiver = msg.sender;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBdev = amountBNB.mul(marketingwallettwo).div(totalBNBFee);
        uint256 amountBNBMarketing = amountBNB.mul(marketingFee).div(totalBNBFee);


        (bool MarketingSuccess, /* bytes memory data */) = payable(marketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
        require(MarketingSuccess, "receiver rejected ETH transfer");
        (bool devSuccess, /* bytes memory data */) = payable(marketingwallettwoReceiver).call{value: amountBNBdev, gas: 30000}("");
   ...
   function setMaxWallet(uint256 amount) external onlyOwner {
    ...
    function setFees(uint256 _liquidityFee, uint256 _marketingFee, uint256 _marketingwallettwo, uint256 _feeDenominator) external onlyOwner {
        liquidityFee = _liquidityFee;
        marketingFee = _marketingFee;
        marketingwallettwo = _marketingwallettwo;
        totalFee = _liquidityFee.add(_marketingFee).add(_marketingwallettwo);
    ...
    function setFeeReceiver(address _marketingFeeReceiver, address _marketingwallettwoReceiver) external onlyOwner {
        marketingFeeReceiver = _marketingFeeReceiver;
        marketingwallettwoReceiver = _marketingwallettwoReceiver;
    }
    ...
    function manualSend() external {
        ...
        payable(marketingwallettwoReceiver).transfer(contractETHBalance);
    }

    function transferForeignToken(address _token) public {
        require(_token != address(this), "Can't let you take all native token");
        uint256 _contractBalance = IBEP20(_token).balanceOf(address(this));
        payable(marketingwallettwoReceiver).transfer(_contractBalance);
    }
