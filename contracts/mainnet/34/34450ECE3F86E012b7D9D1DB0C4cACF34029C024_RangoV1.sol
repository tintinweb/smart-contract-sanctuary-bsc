...
contract RangoV1 is BaseProxyContract {
    ...
    enum BridgeType { Across, CBridge, Hop, Hyphen, Multichain, Stargate, Synapse, Thorchain, Wormhole }
    ...
    function getProxyAddress(BridgeType bridgeType) public pure returns(address) {
        ...
        if (bridgeType == BridgeType.Wormhole) return s.wormhole;
    ...
    function setProxyAddress(BridgeType bridgeType, address _newAddress) internal {
        ...
        if (bridgeType == BridgeType.Wormhole) s.wormhole = _newAddress;
    ...
    function wormholeSwap(
        SwapRequest memory request,
        Call[] calldata calls,
        IRangoWormhole.WormholeRequest memory wormholeRequest
    ) external payable whenNotPaused nonReentrant {
        address a = getProxyAddress(BridgeType.Wormhole);
        (uint out, uint value) = onChainSwapsPreBridge(a, request, calls, wormholeRequest._fee);
        IRangoWormhole(a).wormholeSwap{value: value}(request.toToken, out, wormholeRequest);
    }
}
