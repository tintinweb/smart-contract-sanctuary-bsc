...
contract StakingManager is Ownable, ReentrancyGuard {
    ...
    uint256 constant public MINIMUM_BALANCE = 2000000000000000000000000000;

    bool public isStakingEnabled = false;

    struct UserInfo {
        uint256 amount; //amount of prim the user has provided
    ...
    constructor(IERC20 _prim) {
        prim = _prim;
        operators[owner()] = true;
        emit OperatorUpdated(owner(), true);

        _setDefaultLevels(60 days, 350, 0, 0); // if 0 for min/max Tx amount, no limit
        _setDefaultLevels(30 days, 200, 0, 0); // if 0 for min/max Tx amount, no limit
        _setDefaultLevels(30 days, 50, 0, 0); // if 0 for min/max Tx amount, no limit
    }
    ...
    /// @notice Update the given level's data. Can only be called by an operator.
    ...
    function safePrimTransfer(address _to, uint256 _amount) internal {
        uint256 primBal = prim.balanceOf(address(this));
        if (_amount > primBal) {
            prim.transfer(_to, primBal);
        } else {
            prim.transfer(_to, _amount);
        }
    }
    ...
    function stake(uint256 _amount, uint256 _lid) external nonReentrant {
        ...
        require(prim.balanceOf(msg.sender) >= MINIMUM_BALANCE, "You need a minimum of 2 billion PRIM token to participate");
        ...
        prim.transferFrom(msg.sender, address(this), _amount);
    ...
    function harvest(uint256 _lid) external nonReentrant {
        ...
        safePrimTransfer(msg.sender, rewardAmount);
    }

    function withdraw(uint256 _amount, uint256 _lid) external nonReentrant {
        ...
        if (level.reward == 200) {
            require(
                ...
                "withdraw: staking period has not ended"
            );
        }
        uint256 lockEndtime = user.lockEndTime;
        updateUserStake(msg.sender, _lid);
        ...
        if (level.stakingDuration == 60 days) {
            if (lockEndtime > block.timestamp) {
                withdrawalAmount = withdrawalAmount.div(2); // 50% of the withdrawal amount as fine
            }
        }

        safePrimTransfer(msg.sender, withdrawalAmount);
    }
    ...
    function emergencyWithdraw(uint256 _lid) external returns (uint256) {
        UserInfo memory user = userInfo[_lid][msg.sender];
        ...
        uint256 stakedAmount = user.amount.sub(
            // take percentage of the amount based on the penalty
            user.amount.mul(_penalty).div(100)
        );
        ...
        safePrimTransfer(msg.sender, stakedAmount);
        return stakedAmount;
    }
