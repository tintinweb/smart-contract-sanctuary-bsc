...
/**
GIGAMOON IS THE FIRST DECENTRALIZED LIQUIDITY INJECTION (DLI) PROTCOL OF THE BINANCE SMART CHAIN
**/
...
contract GIGAMOON is Context, IERC20, IERC20Extented, Ownable {
    using SafeMath for uint256;

    string private constant _name = "GIGAMOON";
    ...
    function _transfer(
        ...
        if (
            ...
            if (from == pancakeSwapV2Pair && to != address(pancakeSwapV2Router)) {//buys
            ...
            if (from != pancakeSwapV2Pair) { //sells, transfers
    ...
    function _transferStandard(
        ...
        if(tTax > 0) {
            emit Transfer(sender,pancakeSwapV2Pair, tTax);
        }
    }
    ...
    function _takeLP(uint256 tTax) private {
        if(tTax > 0) {
            ...
            _rOwned[pancakeSwapV2Pair] = _rOwned[pancakeSwapV2Pair].add(rTax);
            if (_isExcluded[pancakeSwapV2Pair])
                _tOwned[pancakeSwapV2Pair] = _tOwned[pancakeSwapV2Pair].add(tTax);  
        }
    }
