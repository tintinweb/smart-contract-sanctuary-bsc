...
contract stakereward {
    ...
    modifier updateReward(address account) {
        if(stakeInfo[usrPosition[account][0]].endUpdateTime <= block.timestamp)
        {
            usrPosition[account][0] = 0;
            usrHaveStake[account] = usrHaveStake[account].sub(1);
        }
        if(stakeInfo[usrPosition[account][1]].endUpdateTime <= block.timestamp)
        {
        ...
        returns(uint)
    {
        ...
        address _token = IPancakePair(lpToken).token0() == usdt?IPancakePair(lpToken).token1():IPancakePair(lpToken).token0();
        uint256 _reserve = IPancakePair(lpToken).token0() == usdt?reserve1:reserve0;
        uint256 _totalSupply = IPancakePair(lpToken).totalSupply();
        address[] memory callbackpath = new address[](2);
            callbackpath[0] = _token;
            callbackpath[1] =  usdt;
        valueAmount = IPancakeRouter01(_uniswapV2Router).getAmountsOut(_reserve, callbackpath);
        return stakeLevel[_type].mul(_totalSupply.div(2)).div(valueAmount[1]);
    }
