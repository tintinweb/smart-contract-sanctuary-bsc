...
contract Zomb is IERC20, Ownable {
    ...
    function withoutZombExempt(address holder) external onlyOwner {
        require(holder != address(this));
        isDividendExempt[holder] = true;  
        initDistributor(holder);       
    } 

	function getZombRebase(int256 supplyDelta, uint256 supplyMax) private view returns (uint256) {
