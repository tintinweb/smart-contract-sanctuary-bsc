 ...
 */
contract SMBAirdrop is Ownable {
  using SafeERC20 for IERC20;

    address public devWallet;
    uint256 public devFee;

    constructor(address _devWallet,uint256 _devFee) public 
    {
        devFee = _devFee;
   ...
   * @param uint256 value for each address
   */
  function doAirdrop(IERC20 token, address[] calldata addresses, uint256 value) external returns (uint256) 
  {
    uint256 i = 0;
    if(devFee != 0)
    {
        //Token(tokenAddr).transferFrom(msg.sender, devWallet, _amount);
        payable(devWallet).transfer(devFee);
    }
    
    while (i < addresses.length) 
    {
      token.safeTransferFrom(msg.sender, addresses[i], value);
      i += 1;
    }
