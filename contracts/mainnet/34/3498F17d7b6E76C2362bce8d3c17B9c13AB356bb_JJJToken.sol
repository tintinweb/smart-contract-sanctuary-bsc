...
contract JJJToken is Context, IERC20, Ownable {     //合约名字
    using SafeMath for uint256;
    using Address for address;
    string private _name = "JJJ";   //全名
    string private _symbol = "J"; //符号
    ...
    uint256 private _buyLiquidityFee = 1;    //买入回流
    uint256 private _buyruinFee = 1;       //买入销毁
    uint256 private _buyTeamFee = 1;       //买入营销
    
    uint256 private _sellLiquidityFee = 1;    //卖出回流
    uint256 private _sellruinFee = 1;       //卖出销毁
    uint256 private _sellTeamFee = 1;       //卖出营销

    uint256 private _liquidityShare = 1;    //设置回流分部
    uint256 private _ruinShare = 1;        //设置销毁分部
    uint256 private _teamShare = 1;       //设置营销分部
    ...
    uint256 private _totalSupply = 2100 * 10**_decimals;      //发行数量
    uint256 public _maxTxAmount = 5 * 10**_decimals;        //单笔限购多少
    uint256 public _walletMax = 50 * 10**_decimals;          //钱包上限
    uint256 private minimumTokensBeforeSwap = 1 * 10**_decimals;     //回流设置 这里设100 营销合约有100自动卖出的时候一起卖出
    ...
    constructor () {
        ...
        teamWalletAddress = payable(address(0x53ed7bbFD7bC45D4c10cA9F2226128f17a4d0EAF));   //营销地址
        ruinWalletAddress = payable(address(0x000000000000000000000000000000000000dEaD));   //销毁地址
