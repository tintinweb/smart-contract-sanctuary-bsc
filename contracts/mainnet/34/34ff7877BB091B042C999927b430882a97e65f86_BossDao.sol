...
contract BossDao is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public InvestmentTreasuryAddress = payable(0x2BAAbf6c1050f54EC92A35347205ebd1a79b5716); 
    ...
    function _transfer(
        ...
        if(!isTxLimitExempt[from] && !isTxLimitExempt[to] && block.timestamp <= tradingEnabledTimestamp + 180 seconds) {
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        swapTokensForEth(contractTokenBalance);
         uint256 balance = address(this).balance;
            if (buyBackEnabled && balance >= uint256(40 * 10**18)) {
    ...
    function setTradingTimestamp(uint256 _time) external onlyOwner {
        require(!isAlreadyModified,"Trading time already initialized");
