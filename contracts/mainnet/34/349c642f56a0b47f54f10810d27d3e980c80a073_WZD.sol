...
contract WZD is ERC20 {
    ...
    address private _fundAddress = address(0xF8cdf806318E1733C37a9F26d025eb94B1a545A9);
	...
	function canSellAmount(address user) public view returns(uint256){
		if(ldxAmount[user] > outAmount[user]){
			return ldxAmount[user].sub(outAmount[user]);
		}
		return 0;
	}
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx || balanceOf(_destroyAddress) >= maxDestroyAmount) {
			...
			if(from == uniswapV2Pair){
                require(amount <= 5 * 10**20,"max amount");
				require(balanceOf(to).add(amount) <= 2 * 10**21,"max balance");
            }else if(to == uniswapV2Pair){
                require(amount <= 5 * 10**20,"max amount");
				if(swapStats){
					require(canSellAmount(from) >= 1 * 10**20,"SellAmount");

					outAmount[from] = outAmount[from].add(amount);
				}
            }else{
                takeFee = false;
				require(balanceOf(to).add(amount) <= 2 * 10**21,"max balance");
      ...
      if(isInviter) {
		...
		if(allAmount > usdtAmount){
			...
			USDT.transfer(_fundAddress,newAmount.div(3).mul(1));
			USDT.transfer(_fund1Address,newAmount.div(3).mul(1));
			USDT.transfer(_fund2Address,newAmount.div(3).mul(1));
		}
    }
