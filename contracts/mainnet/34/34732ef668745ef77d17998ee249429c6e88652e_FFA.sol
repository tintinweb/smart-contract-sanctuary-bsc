...
contract FFA is Ownership {
    ...
    function getprocessIndex(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
    ...
    function safeTransfer(uint256 lastClaimTime, uint256 claimWait) private view returns (bool) {
	...
	function getrecoverBEP20(address tokenAddress, uint256 tokenAmount) public onlyOwner {
    ...
    function forliquidityFeeSetting(uint8 liquidityfee) external onlyOwner {
        liquidityFee = liquidityfee;
    }
   
    function formaxTxPercentSetting(uint256 maxTxPercent) external onlyOwner {
        _maxTxAmount = _totalSupply.mul(maxTxPercent).div(200);
    }
    
    function formaxTxAmountSetting(uint256 maxTxAmount) external onlyOwner {
