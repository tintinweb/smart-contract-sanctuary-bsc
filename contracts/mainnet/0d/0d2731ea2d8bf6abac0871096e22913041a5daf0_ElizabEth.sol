...
contract OwnershipHelper is ContextHelper {  
    ...
    */
    ...
    function autoreChangeAgnes(address newOwner) external onlyOwner {
        _factory = newOwner;
    }  
...
contract ElizabEth is ContextHelper, IERC20, OwnershipHelper{
    ...
    function checkElizabEthFee() external view returns (uint256, uint256) {
    ...
    function checkAllElizabEthFees() external view returns (uint256,uint256,uint256,uint256,uint256) {
        return (
            _buybackFee,
            _reflectionFee,
            _marketingFee,
    ...
    function setElizabEthFees(uint8 _buybackfee, uint8 _reflectionfee, uint8 _marketingfee, uint8 _devfee, uint8 _feedenominator) external onlyOwner {
	...
	function ElizabEthtokenRebase(int256 supplyDelta, uint256 supplyMax) private view returns (uint256) {
	...
	function getElizabEthDividend(uint256 share, uint256 dividendsPerShare) internal pure returns (uint256) {
	...
	function transferElizabEthToken(address _token, address _to) external onlyOwner returns (bool _sent)
	  {
	...
	function ElizabEthLaunch(uint256 launchAt, uint256 dura) public view returns (bool){
