...
contract Staking is Ownable, ReentrancyGuard {
  ...
  function earned(address wallet_) public view returns (uint256) {
    return
      ((_balances[wallet_] *
        (calculateRewardPerToken() - _userRewardsPerToken[wallet_])) / 1e18) +
      _rewards[wallet_];
  }

  modifier updateReward(address wallet_) {
    _rewardsPerToken = calculateRewardPerToken();
    ...
    updateReward(msg.sender)
  {
    ...
    _data[block.number] = totalStaked;
  ...
  function retrieveData(uint256 start_)
    external
    view
    returns (uint256[30000] memory)
  {
    uint256[30000] memory _result;
    uint256 end = start_ + 30000;
    for (uint256 i = start_; i < end; i++) {
      _result[i - start_] = _data[i];
    }
    return _result;
  }
