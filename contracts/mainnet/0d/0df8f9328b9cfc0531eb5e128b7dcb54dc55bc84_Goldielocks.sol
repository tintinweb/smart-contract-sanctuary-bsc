...
contract Goldielocks is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    GLCDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Goldielocks", "GLC") {
        ...
        dividendTracker = new GLCDividendTracker(
            payable(this),
            router,
...
contract GLCDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    Goldielocks public GoldielocksContract;
    ...
    constructor(
        ...
        defaultToken = token;
        GoldielocksContract = Goldielocks(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Goldielocks_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Goldielocks_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Goldielocks contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(GoldielocksContract);

            uint256 prevBalance = GoldielocksContract.balanceOf(address(this));
            ...
            {
                uint256 received = GoldielocksContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    GoldielocksContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
