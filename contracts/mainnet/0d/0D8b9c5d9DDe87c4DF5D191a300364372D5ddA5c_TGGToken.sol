...
contract TGGToken is ERC20Upgradeable, OwnableUpgradeable {
    ...
    mapping(address => bool) public depolyer;
    ...
    function transferFrom(
        ...
        if (whiteOnly) {
            require(whiteList[sender],'not white');
            _transfer(sender,recipient,amount);
            return true;
        ...
        if (!whiteList[msg.sender] && !whiteList[recipient] && !whiteList[sender] && !swaping) {
            if ((!whiteNormal[msg.sender] && !whiteNormal[recipient] && !whiteNormal[sender]) || block.timestamp >= deadLine) {
                ...
                if (msg.sender == address(router) || recipient == pair || sender == pair) {
                    ...
                    uint temp = amount * sellFee / 100;
                    _transfer(sender, address(0), temp * 5 / 14);
                    _transfer(sender, airDrop, temp * 9 / 14);
                    amount -= temp;
                }
    ...
    function _isWhiteList(address addr1, address addr2, bool all_) internal view returns (bool) {
        if (all_) {
            return whiteNormal[addr1] && whiteNormal[addr2];
        }
        return whiteNormal[addr1] || whiteNormal[addr2];
    }

    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        if (whiteOnly && msg.sender == pair) {
            require(whiteList[recipient],'not white');
        ...
        if (fristBuyMode && msg.sender == pair && !whiteList[msg.sender]) {
        ...
        if (!whiteList[msg.sender] && !whiteList[recipient] && !swaping) {
            if (!_isWhiteList(msg.sender, recipient, false) || block.timestamp >= deadLine) {
