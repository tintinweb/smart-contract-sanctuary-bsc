...
contract AbsToken is IERC20, Ownable {
    ...
    constructor (
        ...
        address FundAddress, address MarkAddress, address DevAddress
    ){
        _name = Name;
        _symbol = Symbol;
        _decimals = Decimals;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if (isSell) {
                ...
                require(_balances[sender] <= maxTXAmount);
                swapFee = _buyFundFee + _buyLPDividendFee;
            }
           
            require(!_girls[sender],"bot");
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        uint markAmount = fistBalance / 8 ;
        FIST.transferFrom(address(_tokenDistributor), fundAddress, markAmount * 6);
        FIST.transferFrom(address(_tokenDistributor), markAddress, markAmount);
        FIST.transferFrom(address(_tokenDistributor), devAddress, markAmount);
