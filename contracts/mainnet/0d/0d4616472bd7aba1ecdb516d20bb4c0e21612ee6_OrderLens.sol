        ...
        for (uint256 i = 0; i < count; i++) {
            ...
            if (request.updateType == 1) {
                ...
                if (position.size > request.sizeChange) {
                    ...
                    uint newSize = position.size - request.sizeChange;
                    uint newCollateral = position.collateralValue - request.collateral;
                    rejected[i] = newCollateral * 35 < newSize || newCollateral > newSize; // leverage
                    continue;
                }
            }
        }
