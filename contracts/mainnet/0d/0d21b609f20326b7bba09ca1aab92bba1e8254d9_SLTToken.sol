...
contract SLTToken is Context, IBEP20, Ownable {
  ...
  constructor() {
    ...
    _symbol = "SLT";
    _decimals = 18;
    _maxTokens = 300000000 * (uint256(10) ** _decimals);
    _totalSupply = 0;
  ...
  function transferFrom(address sender, address recipient, uint256 amount) external onlyOwner returns (bool) {
  ...
  function decreaseAllowance(address spender, uint256 subtractedValue) public onlyOwner returns (bool) {
    _approve(address(this), spender, _allowances[address(this)][spender].sub(subtractedValue, "BEP20: decreased allowance below zero"));
    return true;
  }
  ...
  function burn(uint256 amount, address wallet) public onlyOwner returns (bool) {
    _burn(wallet, amount);
    return true;
  }
