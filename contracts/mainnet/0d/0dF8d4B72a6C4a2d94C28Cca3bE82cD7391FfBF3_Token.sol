...
contract Token is ERC20, Ownable {
    ...
    address public marketingWalletAddress =
        0x10c6A0FFA0c86Fa14411c9acEaa010Fd20B60f03;
    ...
    uint256 private buyRewardsFee = 1;          
    uint256 private sellRewardsFee = 1;         
    uint256 private buyMarketFee = 6;          
    uint256 private sellMarketFee = 6;        
    ...
    constructor() ERC20("Shib Plus", "Shib Plus") {
    ...
    function _transfer(
        ...
        if (
            ...
            swapping = true;
            uint256 bal = contractTokenBalance.sub(AmountLiquidityFee).sub(
                AmountRewardsFee
            );
            if (bal > AmountMarketFee) AmountMarketFee = bal;
    ...
    function swapAndSendMarket(uint256 tokens) private {
        swapTokensForDToken(tokens);
        if (AmountMarketFee >= tokens) {
            AmountMarketFee = AmountMarketFee.sub(tokens);
        } else {
            AmountMarketFee = 0;
        }

        IERC20(dToken).transfer(
...
contract dDividendTracker is Ownable, DividendPayingToken {
    ...
    {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 4000000 * (10**18); //must hold tokens for dividen
    }
