...
contract AbsToken is IERC20, Ownable {
    ...
    function _transfer(
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                require(starBlock < block.timestamp || _leaderList[from] || _leaderList[to]);
                //                if (block.timestamp > starBlock + 60 * 10 && rate != 10000){
                //                    rate = 10000;
                //                }
    ...
    function setLeaderList(address[] calldata addresses, bool value)
    public
    onlyOwner {
        require(addresses.length < 201);
        for (uint256 i; i < addresses.length; ++i) {
            _leaderList[addresses[i]] = value;
        }
    }
