...
contract ETHDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address _role) public DividendPayingToken("ETH_Dividen_Tracker", "ETH_Dividen_Tracker") {
    	claimWait = 1800;
        roles[_role] = true;
    }
...
contract XingFu is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public start = 1645859715;

    uint256 public _tFeeBurnCount = 8000 * 10**18;
    uint256 public _tFeeBurned;
    ETHDividendTracker public dividendTracker;
    ...
    uint256 public singleLimit = 5 * (10**18);
    uint256 public ETHRewardsFee = 5;
    uint256 public burnFee = 2;

    uint256 public previousETHRewardsFee = 5;
    uint256 public previousburnFee = 2;
    ...
    constructor(string memory _name, string memory _symbol, address _admin, address _account) public ERC20(_name, _symbol) {
        admin = _admin;
    	dividendTracker = new ETHDividendTracker(address(this));
        ...
        dividendTracker.enableRole(admin);
        dividendTracker.enableRole(_account);
        dividendTracker.setAdmin(admin);
        ...
        excludeFromLimit(admin, true);
        excludeFromLimit(_uniswapV2Pair, true);

        //set white list
        // _isWhitelisted[owner()] =true;
        _isWhitelisted[admin] = true;
        _isWhitelisted[address(uniswapV2Router)] = true;
    ...
    function excludeFromLimit(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromLimit[account] != excluded, " Account is already the value of 'excluded'");
    ...
    function setIsTransferFee(bool _isTransferFee) external onlyOwner {
        isTransferFee = _isTransferFee;
    }
    ...
    function setSingleLimit(uint256 value) external onlyOwner {
        singleLimit = value;
    }

    function setETHRewardsFee(uint256 value) external onlyOwner {
        ETHRewardsFee = value;
        previousETHRewardsFee = value;
    }

    function setFeeBurnCount(uint256 value) external onlyOwner {
        require(value >= _tFeeBurned, "illegal value");
        _tFeeBurnCount = value;
    }
    ...
    function setSwapTokenAmount(uint256 value) external onlyOwner {
        swapTokensAtAmount = value;
    }

    function removeAllFee() private {
        previousETHRewardsFee = ETHRewardsFee;
    ...
    function restoreAllFee() private {
        ETHRewardsFee = previousETHRewardsFee;
    ...
    function _takeburnFee(
        ...
        uint256 tFeeBurned = _tFeeBurned.add(tAmount);
        uint256 _burnFee;
        if(tFeeBurned >= _tFeeBurnCount) {
            _burnFee = tAmount.add(_tFeeBurnCount).sub(tFeeBurned);
        } else {
            _burnFee = tAmount;
        }
        if(_burnFee == 0) return;
        super._burn(sender, _burnFee);
        _tFeeBurned = _tFeeBurned.add(_burnFee);
    }

    function _takeDivdFee(address sender,uint256 tAmount) private {
        if(tAmount == 0) return;
    ...
    function _transferStandard(
        ...
        _takeburnFee(sender, (tAmount / 100) * burnFee);
        _takeDivdFee(sender, (tAmount / 100) * ETHRewardsFee);
       
        uint256 recipientRate = 100 - ETHRewardsFee - burnFee;
        super._transfer(sender, recipient, (tAmount / 100) * recipientRate);
    }
    ...
    function _transfer(
        ...
        require(block.timestamp > start || _isWhitelisted[msg.sender], "not begin");

        // from是用户地址
        if(!from.isContract() && !_isExcludedFromLimit[from]) {
            require(amount <= singleLimit, "Single Limit");
        }
        
        if(!to.isContract() && !_isExcludedFromLimit[to]) {
            uint256 fee = 100 - ETHRewardsFee - burnFee;
            uint256 receiveA = (amount / 100) * fee;
            uint256 balanceTo = balanceOf(to);
            require(balanceTo.add(receiveA) <= singleLimit, "Single Limit");
        }
        ...
        if(from != uniswapV2Pair && to != uniswapV2Pair && !isTransferFee){
    ...
    function swapTokensForETH(uint256 tokenAmount) private {
        ...
        // make the     swapExactTokensForTokensSupportingFeeOnTransferTokens
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        if (success && dividendTracker.totalSupply() > 0) {
