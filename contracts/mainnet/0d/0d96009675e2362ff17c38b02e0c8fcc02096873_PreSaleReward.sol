...
contract PreSaleReward is Ownable,ReentrancyGuard {
    ...
    uint256 public withdrawStartTime; // 开始提取时间
    uint256 public withdrawEndTime; // 结束提取时间
    ...
    function awardInfo(address addr) external view returns (uint256 pastAmount, uint256 pendingAmount, uint256 totalAmount) {
        ...
        if (block.timestamp >= withdrawStartTime && block.timestamp <= withdrawEndTime) {
    ...
    function totalPendingCashBack(address addr) external view returns (uint256) {
        if (block.timestamp > withdrawEndTime || block.timestamp < withdrawStartTime) {
            return 0;
        }
    ...
    function changeWithdrawStartTime(uint256 startTime) external onlyOwner {
        withdrawStartTime = startTime;
    }

    function changeWithdrawEndTime(uint256 endTime) external onlyOwner {
        withdrawEndTime = endTime;
    }
