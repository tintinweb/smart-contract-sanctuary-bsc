...
contract FinoToken is ERC20Detailed, Ownable {
    ...
    constructor() ERC20Detailed("FINO", "FINO", uint8(DECIMALS)) {
    ...
    function takeFee(address sender, address recipient, uint256 gonAmount) internal returns (uint256){
        uint256 _realFee = totalBuyFee.add(burnFee);
        if(automatedMarketMakerPairs[recipient]) _realFee = totalSellFee.add(burnFee);
