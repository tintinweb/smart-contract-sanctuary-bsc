...
// File: Lotto/Lotto_release_102.sol
...
contract Lotto is Context, IERC20, Ownable {
    ...
    function transferRelationship(address from, address to)
        private
        returns (address)
    {
        ...
        )
    {
        ...
        tTransferAmount = tTransferAmount.sub(burnFee, "sub burn fee err").sub(
            marketingFee,
            "sub market fee err"
        );
    }
    ...
    function _transfer(
        ...
        if (shouldInvite) {
            address inviterRef = transferRelationship(from, to);
