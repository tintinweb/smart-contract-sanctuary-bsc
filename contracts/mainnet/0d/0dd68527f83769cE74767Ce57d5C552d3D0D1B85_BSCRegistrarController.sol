...
contract BSCRegistrarController is Ownable, StakeForDistributeProfits {
    ...
    function registerFreeForDAO(string calldata name, address owner, uint duration, bytes32 secret) external {
        _registerFreeForDAOWithConfig(name, owner, duration, secret, address(0), address(0));
    }

    function _registerFreeForDAOWithConfig(string memory name, address owner, uint duration, bytes32 secret, address resolver, address addr) private {
        bytes32 commitment = makeCommitmentWithConfig(name, owner, secret, resolver, addr);
        uint cost = _consumeCommitment(name, duration, commitment);
