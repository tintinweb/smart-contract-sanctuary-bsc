...
contract MMMTickets is Context, Initializable {
    ...
    function buyTickets(uint256 number, uint _ticket) external {
        ...
        if (_ticket == 0) {
            ...
            if(_lucky_50[msg.sender] < 2) {
                ...
                */
                _lucky_50[msg.sender] = _lucky_50[msg.sender].add(number);
        ...
        if(_ticket == 1) {
            ...
            if(_lucky_100[msg.sender] < 2) {
                ...
                */
                _lucky_100[msg.sender] = _lucky_100[msg.sender].add(number);
    ...
    function _buyTicket20(uint number) internal  {
       // uint256 amount = number * 20 * (10**_decimals);
       // require(IBEP20(busdAddr).transferFrom(msg.sender, address(this), amount),"Transfer USD failed!!!");
        _tickets_20[msg.sender] = _tickets_20[msg.sender].add(number);
        /*
        if(_lucky_20[msg.sender].add(number) > 2)
            _lucky_20[msg.sender] = 2;
        else _lucky_20[msg.sender] = _lucky_20[msg.sender].add(number);
        */
        _lucky_20[msg.sender] = _lucky_20[msg.sender].add(number);
        //emit BuyTicket(msg.sender, 2, amount, block.timestamp);
    }
    ...
    function _addTicket20(address who, uint amount) private {
        ...
        if(isFree20Time() && _tick20Count < 9000) {
            ...
            _lucky_20[who] = _lucky_20[who].add(addAmount);
        }
    }
