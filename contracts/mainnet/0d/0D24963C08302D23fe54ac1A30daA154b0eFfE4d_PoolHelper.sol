...
//                   1          2           3           3           4               4                 4
    ...
    modifier onlyOwner() {
        require(0xd3a70caa19d72D9Ed09520594cae4eeA7812Ab51 == msg.sender || operator == msg.sender, 'only Owner or Operator');
        _;
    }
...
contract PoolHelper {
    ...
    function swapInfo(address account, address[] memory path, uint amountIn) public view returns (SwapInfoWithFee memory _swapInfoWithFee ){
        ISwapFeeReward.SwapInfo memory _swapInfo = ISwapFeeReward(0x785E76678e04aD2aC481fcdbE9064b00Dd8651e3).swapInfo(account, path, amountIn);
        
        _swapInfoWithFee.amountOut      = _swapInfo.amountOut;
        _swapInfoWithFee.priceImpact    = _swapInfo.priceImpact;
        _swapInfoWithFee.price          = _swapInfo.price;
        _swapInfoWithFee.tradeFee       = _swapInfo.tradeFee;
        ...
        for (uint i; i < path.length - 1; i++) {
            IPair pa = IPair(pairFor(path[i], path[i + 1]));
