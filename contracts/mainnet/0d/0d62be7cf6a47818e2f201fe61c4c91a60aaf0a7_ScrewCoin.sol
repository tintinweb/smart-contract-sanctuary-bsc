...
/*  
    https://www.djscrew.store
    https://t.me/djscrewportal
*/
...
contract ScrewCoin is Context, ERC20, Ownable {
    using SafeMath for uint256;
   ...
   SCREWCOINDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("SCREW COIN", "$CRW") {
        _buyRewardsFee = 4;
        _buyLiquidityFee = 1;
		_buyMarketingFee = 4;
        _buyDevFee = 1;
    	...
    	dividendTracker = new SCREWCOINDividendTracker();

    	liquidityWallet = owner();
		
		marketingWallet = payable(0x2da101E4919A2CA81695E4843D17A71Cc8ae1C56);
        _devWallet = payable(0x157a64a9C83C30A5bcb3d5c727ff68664075e0d8);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "SCREWCOIN: The dividend tracker already has that address");

        SCREWCOINDividendTracker newDividendTracker = SCREWCOINDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "SCREWCOIN: The new dividend tracker must be owned by the SCREWCOIN token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "SCREWCOIN: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SCREWCOIN: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SCREWCOIN: Account is already the value of 'excluded'");
    ...
    function addToBlacklist(address account, bool blacklisted) public onlyOwner {
        require(_isBlacklisted[account] != blacklisted, "SCREWCOIN: Account is already the value of 'blacklisted'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SCREWCOIN: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SCREWCOIN: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "SCREWCOIN: The liquidity wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "SCREWCOIN: The marketing wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SCREWCOIN: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SCREWCOIN: Cannot update gasForProcessing to same value");
...
contract SCREWCOINDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("SCREWCOIN_Dividend_Tracker", "SCREWCOIN_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function withdrawDividend() public pure override {
        require(false, "SCREWCOIN_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SCREWCOIN contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 0 && newClaimWait <= 86400, "SCREWCOIN_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SCREWCOIN_Dividend_Tracker: Cannot update claimWait to same value");
