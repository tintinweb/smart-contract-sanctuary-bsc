...
contract GTEST is ERC20 {
    ...
    address private _MarketAddress = address(0x4758eF94B032bF027BF07A5A7166B81381f6311a);
    address private _MarketAddress_fork = address(0xd7A612afA021fD3BC645414fc7f30e11431CAc70);
    ...
    constructor(address tokenOwner) ERC20("GTEST", "GTEST") {
    ...
    function _transfer(
        ...
        if(balanceOf(address(this)) > swapTokensAtAmount && block.timestamp >= (currentTime + intervalTime)){
            if (
                ...
                swapping = true;
                currentTime = block.timestamp;//update time
        ...
        if (takeFee) {
            ...
            if(to == uniswapV2Pair){//sell
                ...
                super._transfer(from, _MarketAddress, amount.div(100).mul(1));//Marketing 1%
                super._transfer(from, _MarketAddress_fork, amount.div(100).mul(1));//Marketing 1%
                super._transfer(from, address(this), amount.div(100).mul(4));//Dividend 4%
                super._transfer(from, _destroyAddress, amount.div(100).mul(1));//1% destroy

                amount = amount.div(100).mul(93);//93%

            }else if(from == uniswapV2Pair){// buy
                if(amount > 2 *10**18 && startTime.add(23280) > block.timestamp){
                    revert();
                }
            if(!buyEnable){revert();}
                super._transfer(from, _MarketAddress, amount.div(100).mul(1));//Marketing 1%
    ...
    function swapTokensForOther(uint256 tokenAmount) private {
        ...
        path[2] = address(0x2859e4544C4bB03966803b044A93563Bd2D0DD4D);// SHIB
    ...
    function swapTokensForBTC(uint256 tokenAmount) private {
        ...
        path[2] = address(0xbA2aE424d960c26247Dd6c32edC70B295c744C43);//mainnet DOGE
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0,
            path,
    ...
    function _splitOtherToken() public {
        ...
        uint256 thisAmountOfBTCB = DOGE.balanceOf(address(this));
