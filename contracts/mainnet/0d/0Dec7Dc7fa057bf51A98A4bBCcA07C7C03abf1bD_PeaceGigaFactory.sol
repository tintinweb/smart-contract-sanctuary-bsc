...
contract PeaceGigaFactory is Ownable,ERC1155Receiver {
    ...
    function mintNFTs (uint _id, uint256 _count) external {
        ...
        uint256 mintUSDT = _allCardInfos[_id].priceBUSD;
        ...
        _usdtToken.transferFrom (msg.sender, addressInfo.treasuryAddress, mintUSDT * royaltyInfo.treasuryPercent / royaltyInfo.totalPercent);
    ...
    function getRewardInfoByNFT(uint256 _nftID) view private returns(uint256){
        ...
        uint256 nftPrice = _allCardInfos[_getCIDFromID[_nftID]].priceBUSD;
    ...
    function claimByNFT(uint256 _tokenID) external onlyNFTOwner(_tokenID) {
        ...
        require(_usdtToken.balanceOf(address(this)) > nftReward, "no enough balance on peace");

        _usdtToken.transfer(msg.sender, nftReward * (royaltyInfo.totalPercent - royaltyInfo.tradingPercent) / royaltyInfo.totalPercent);
        _usdtToken.transfer(addressInfo.teamAddress, nftReward * royaltyInfo.tradingPercent / royaltyInfo.totalPercent);
    ...
    function claimAll() external {
        ...
        require(_usdtToken.balanceOf(address(this)) > rewards, "no enough balance on reward pool");
        
        _usdtToken.transfer(msg.sender, rewards * (royaltyInfo.totalPercent - royaltyInfo.tradingPercent) / royaltyInfo.totalPercent);
        _usdtToken.transfer(addressInfo.teamAddress, rewards * royaltyInfo.tradingPercent / royaltyInfo.totalPercent);
    ...
    event SetNFTCardInfo(address sender, uint infoID, string uri, uint256 busd, uint256 sup);
    ...
    event AddNFTCardInfo (address addr, string symbol, string uri, uint256 busd, uint256 sup);
