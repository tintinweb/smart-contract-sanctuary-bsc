...
contract SoleriumTokenPresale is OwnerWithdrawable {
    using SafeMath for uint256;
    ...
    modifier saleStarted(){
        if(preSaleStartTime != 0){
            require(block.timestamp < preSaleStartTime, "Solerium: Sale has already started. Cannot change Sale Params!");
        }
        _;
    }
    ...
    modifier saleDuration(){
        require(block.timestamp > preSaleStartTime, "Solerium: Sale hasn't started");
        require(block.timestamp < preSaleEndTime, "Solerium: Sale has already ended");
        _;
    }
    ...
    modifier saleValid(
        ...
        require(block.timestamp < _preSaleStartTime, "Solerium: Starting time is less than current timestamp!");
        require(_preSaleStartTime < _preSaleEndTime, "Solerium: Invalid PreSale Dates!");
        _;
    }
    ...
    function setSaleTokenParams(
        ...
        require(_rate != 0, "Solerium: Invalid Native Currency rate!");
        rate = _rate;
    ...
    function addWhiteListedToken(
        ...
        require(
            ...
            "Solerium: tokens & prices arrays length mismatch"
        );

        for (uint256 i = 0; i < _tokens.length; i++) {
            require(_prices[i] != 0, "Solerium: Cannot set price to 0");
    ...
    function updateTokenRate(
        ...
        for (uint256 i = 0; i < _tokens.length; i+=1){
            require(tokenWL[_tokens[i]] == true, "Solerium: Token not whitelisted");
            require(_prices[i] != 0, "Solerium: Cannot set rate as 0");
            tokenPrices[_tokens[i]] = _prices[i];
        }
    }
    ...
    function stopSale() external onlyOwner {
        require(block.timestamp > preSaleStartTime, "Solerium: Sale hasn't started yet!");
        ...
        returns (uint256)
    {
        uint256 amtOut;
        if (token != address(0)){
            require(tokenWL[token] == true, "Solerium: Token not whitelisted");
    ...
    function buyToken(address _token, uint256 _amount) external payable saleDuration
    {
        ...
        if(_token != address(0)){
            require(_amount > 0, "Solerium: Cannot buy with zero amount");
            require(tokenWL[_token] == true, "Solerium: Token not whitelisted");

            saleTokenAmt = getTokenAmount(_token, _amount);
            require((totalTokensSold + saleTokenAmt) < totalTokensforSale, "Solerium: Total Token Sale Reached!");
