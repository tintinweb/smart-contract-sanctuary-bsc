...
contract Web69Inu is Ownable, IERC20 {
    ...
    address private marketingWalletAddress = 0x4Ad24B2210a3A73875897D8B9d84641bBCB11210;
    ...
    constructor () {
        ...
        _isExcludedFrom[marketingWalletAddress] = true;
    ...
    function setNumTokensBeforeSwap(uint256 newLimit) public onlyOwner() {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (!_isExcludedFrom[sender] && !_isExcludedFrom[recipient] && recipient != address(this)) {
            ...
            if(isMarketPair[sender] || isMarketPair[recipient]){
                uint256 fees = _marketingFee.add(_liquidityFee);
    ...
    function swapAndLiquify(uint256 amount) private lockTheSwap {
        uint256 fees = _marketingFee.add(_liquidityFee);
        uint256 tokensForMarketing = amount.mul(_marketingFee).div(fees);
        uint256 initialBalanceForMarketing = address(this).balance;
        swapTokensForEth(tokensForMarketing);
        uint256 newBalanceForMarketing = address(this).balance.sub(initialBalanceForMarketing);
        if(newBalanceForMarketing > 0){
            payable(marketingWalletAddress).transfer(newBalanceForMarketing);
        }
        
        uint tokens = amount.sub(tokensForMarketing);
