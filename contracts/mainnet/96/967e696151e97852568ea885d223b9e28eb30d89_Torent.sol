...
contract BEP20 is Context, IBEP20, IBEP20Metadata, Ownable {
    ...
    function _transfer(
        ...
        // total burn 3 crore - for test == 30000
        if (burnTotal < 30000000 * (10**9)) {
            burnPercent = (amount * 1) / 100;
            burnTotal = burnTotal + burnPercent;
            _burn(owner(), burnPercent);
            totalSend = amount - burnPercent;
            _balances[from] = fromBalance - amount;
            _balances[to] += totalSend;
            _totalSupply -= amount;
        } else {
            _balances[from] = fromBalance - amount;
            _balances[to] += amount;
...
function totalTransfer_() public view returns(uint ){
