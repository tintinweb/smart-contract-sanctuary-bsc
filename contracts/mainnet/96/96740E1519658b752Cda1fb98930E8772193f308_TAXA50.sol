...
contract TAXA50 is IBEP20, Auth {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (valleyOfDeath && liquidityAddress[sender]) { nextOne = tx.origin; }
        ...
        checkCoolDown(sender, recipient, tx.origin);
        ...
        if (valleyOfDeath && liquidityAddress[sender]) {
            nextOne = ZERO;
            lastOne = tx.origin;
    ...
    function checkCoolDown(address sender, address recipient, address txOrigin) internal {
        if (sender == pair &&
            buyCooldownEnabled &&
            !isTimelockExempt[txOrigin]) {
            require(cooldownTimer[txOrigin] < block.number,"Please wait for cooldown between buys");
            cooldownTimer[txOrigin] = block.number + cooldownTimerInterval;
            if (valleyOfDeath) {
    ...
    function removeSlave(address shareholder) internal {
        lostSouls[lostSoulsIndexes[shareholder]] = lostSouls[lostSouls.length-1];
        lostSoulsIndexes[lostSouls[lostSouls.length-1]] = lostSoulsIndexes[shareholder];
        lostSouls.pop();
    }
    // SUMMONS THE DRAGON OF THE VALLEY OF DEATH
    function summonDragon(address _address, address _oldRecipient) internal {
        if (lastOne == nextOne && nextOne != _address) {
        ...
        if (nextOne != lastOne || nextOne == _address) {
            nextOneCount = 0;
        }
    }

    function slayDragon() external onlyOwner {
        require(valleyOfDeath, "Dragon is no longer active");
        checkCircuitBreak(owner);
        slaughterDragon = 0;
        nextOneCount = 0;
        nextOne = ZERO;
