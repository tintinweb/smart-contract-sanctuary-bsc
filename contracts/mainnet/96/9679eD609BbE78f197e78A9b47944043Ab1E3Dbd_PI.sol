...
contract PI is ERC20, Ownable {
    ...
    address public marketingWallet = 0xfA8c4f386d67222640062A6F94ECEe992C880F85;
    address public nodeDistributor;
    ...
    uint256 public tradeStartedTime;
    bool public enableMaxTxAmount = true;

    // exlcude from fees and max transaction amount
    mapping (address => bool) public _isExcludedFromFees;


    TokenReceiver private tokenReceiver;
    ...
    constructor(address _nodeDistributor) ERC20("PI Network", "PI") {
        ...
        nodeDistributor = _nodeDistributor;
    ...
    function setNodeDistributor(address _nodeDistributor) external onlyOwner { 
        nodeDistributor = _nodeDistributor;
    }
    ...
    function setEnableMaxTxAmount(bool _enableMaxTxAmount) external onlyOwner { 
        enableMaxTxAmount = _enableMaxTxAmount;
    }
    ...
    function _transfer(
        ...
        if (!lpAdded && to == uniswapV2Pair) {
            lpAdded = true;
            tradeStartedTime = block.timestamp;
        }

        if (lpAdded && block.timestamp - tradeStartedTime > 180 && enableMaxTxAmount) {
            require(amount <= balanceOf(uniswapV2Pair)/100, "max trade amount limit");
        }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        if(takeFee) {
            if (lpAdded && block.timestamp - tradeStartedTime <= 180) {
               super._transfer(sender, marketingWallet, amount); 
               return;
            } else {
                uint256 fee;
                if(sender == uniswapV2Pair) { //buy
                    fee = buyLpFee + buyMarketingFee + buyNodeFee;
                } else if (recipient == uniswapV2Pair) {
                    fee = sellLpFee + sellMarketingFee + sellNodeFee;
                }
    ...
    function swapAndDividend(uint256 tokenAmount) private lockTheSwap {
        uint totalBuyShare = buyLpFee + buyMarketingFee + buyNodeFee;
        uint totalSellShare = sellLpFee + sellMarketingFee + sellNodeFee;
        ...
        uint totalLeftShare =  totalBuyShare + totalSellShare - buyLpFee - sellLpFee;
        uint balanceToNode = newBalance * (buyNodeFee + sellNodeFee) / totalLeftShare;
        IERC20(USDT).transferFrom(address(tokenReceiver), nodeDistributor, balanceToNode);
        INodeDistributor(nodeDistributor).distribute(balanceToNode);
        IERC20(USDT).transferFrom(address(tokenReceiver), marketingWallet, newBalance - balanceToNode);
    }
