...
contract AddLiquidity {
    ...
    modifier onlyWOW() {
                require(wowAddress == address(msg.sender), "Ownable: caller is not the owner");
                _;
        }

    function setWOWAddress(address _wow) public onlyOwner{
        wowAddress=_wow;
    }

    function swapAndLiquify(uint256 contractTokenBalance) public onlyWOW{
        ...
        swapTokensForExactTokens(half); // <- this breaks the USDT -> HATE swap when swap+liquify is triggered
