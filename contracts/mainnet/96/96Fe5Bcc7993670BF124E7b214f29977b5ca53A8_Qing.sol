...
contract Qing is ERC20, ERC20Burnable, Ownable, ERC20Permit {
    ...
    address marketingWallet = 0x77De24736f4ac60604B3b728F7d0d8250E6745C3;
    uint96 public constant DENOMINATOR=10000;

    // Settings of Auto Swapping the tokens which are collected from taxes
    IPancakeSwapV2Router02 public PancakeSwapV2Router;
    uint96 public maxSupply = 419264000 * 10**18;
    
    uint96 public launchTime;
    ...
    uint16 public transferGas = 3000;
    uint96 public autoSwapAmount = 500000*10**18;
    uint96 public maxTokenForWallet = 2000000*10**18;

    uint32 public buyTax = 500;
    uint32 public sellTax = 500;
    uint32 public transferTax = 500;
    uint80 public liquidityShare = 4000;
    uint80 public marketingShare = 6000;
    ...
    modifier buyLimit(address sender, address recipient, uint256 amount) {
        if(walletLimit && isMarketMaker[sender] && !isWhitelisted[recipient]) {
            require(balanceOf(recipient).add(amount) <= maxTokenForWallet, "Max");
        }
        _;
    }

    constructor() ERC20("QING DYNASTY", "QING") ERC20Permit("QING") {
    ...
    // Override
    function _transfer(address sender, address recipient, uint256 amount) internal override buyLimit(sender, recipient, amount) {
    ...
    function setSwapBackSettings(bool status, uint96 _autoSwapAmount) external onlyOwner {
        autoSwap = status;
        autoSwapAmount = _autoSwapAmount;
    }

    function setTransferGas(uint16 newGas) external onlyOwner {
    ...
    function triggerSwapBack(uint256 a) external onlyOwner {
        if(a == 0) { a = balanceOf(address(this)); }
        _swapBack(a);
    ...
    function setIsWhitelisted(address[] memory accounts, bool value) external onlyOwner {
        for(uint i; i < accounts.length; i++) {
            isWhitelisted[accounts[i]] = value;
        }
    }

    function setIsBlacklisted(address[] memory accounts, bool value) external onlyOwner {
        for(uint i; i < accounts.length; i++) {
    ...
    function setWalletLimit(bool _walletLimitEnabled, uint96 _limitAmount) external onlyOwner {
        walletLimit = _walletLimitEnabled;
        maxTokenForWallet = _limitAmount;
    }

    function setTaxes(uint32 newBuyTax, uint32 newSellTax, uint32 newTransferTax) external onlyOwner {
        require(newBuyTax <= 5000 && newSellTax <= 5000 && newTransferTax <= 5000);
        buyTax = newBuyTax;
        sellTax = newSellTax;
    ...
    function setShares(uint32 newLiquidityShare, uint32 newMarketingShare) external onlyOwner {
