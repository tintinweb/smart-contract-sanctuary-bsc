...
contract AladMiner is Ownable {
        ...
        returns (uint256, uint256)
    {
        ...
        uint256 cWithdraw = acc.getTranslateRewards + acc.selfExpectedRewards.mul(5).div(10000).mul(r);
    ...
    function withdrawPrivateRewards(uint256 amount) external {
        ...
        if (amount > 0) {
            ...
            if (amount < trew) {
                accountInfo[msg.sender].getTranslateRewards = accountInfo[msg.sender]
                    .getTranslateRewards
                    .add((trew-amount).mul(1e18).div(getp2()));
            }

            token.safeTransfer(msg.sender, amount.mul(100 - r).div(100));
            if (r > 0) token.safeTransfer(msg.sender, amount.mul(r).div(100));
        }
