...
contract MeteorToken is ERC20, SafeOwnable {
    using SafeMath for uint256;
    ...
    address public NFTProfitAddr;

    constructor() ERC20("Meteor Token", "Meteor") {
        ...
        */
        _mint(0xd919a537580a349552bcfD2737913aCe2Fb51268, 200 *1e26);
        _mint(0xC3910AE8eb7a7a3bB2Fd4F3aba022650c393Ae7E, 800 *1e26);
    ...
    function setLPAddr(address _LPAddr) external onlyOwner { 
        LPAddr = _LPAddr;
    } 

    function setNFTProfitAddr(address _NFTProfitAddr) external onlyOwner { 
        NFTProfitAddr = _NFTProfitAddr;
    } 

    function setSwapPairList(address _address, bool value) external onlyOwner {
        _swapPairList[_address] = value;
    }
    ...
    function _tokenTransfer(
        ...
        if(takeFee) {
            ...
            if(feeToLP > 0) {
                uint256 feeAmount = originalAmount.mul(feeToLP).div(100);
                super._transfer(sender, LPAddr, feeAmount);
                amount -= feeAmount;                
            }

            if(feeToNFT > 0) {
                uint256 feeAmount = originalAmount.mul(feeToNFT).div(100);                
                super._transfer(sender, NFTProfitAddr, feeAmount);
                amount -= feeAmount;                
            }

            if (feeToBurn > 0) {
                uint256 feeAmount = originalAmount.mul(feeToBurn).div(100);
