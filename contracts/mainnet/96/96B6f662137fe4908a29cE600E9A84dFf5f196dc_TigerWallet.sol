...
https://t.me/tigerwalletofficial
https://tigerwallet.finance
...
contract TigerWallet is Context, IBEP20, Ownable {
    ...
    string private _name = "Tiger Wallet 2.0"; // token name
    string private _symbol = "TW"; // token ticker
    ...
    uint256 public minTokenToSwap = 1250 * 1e9; // 10000k amount will trigger the swap and add liquidity
    uint256 public maxHoldingAmount = 40000 * 1e9;
    uint256 public minBuyLimit = 20000 * 1e9;
        ...
        returns (uint256)
    {
        ...
        return tokenFromDev(_rOwned[_account]);
    }
    ...
    function DevFromToken(uint256 tAmount, bool deductTransferFee)
        public
        view
        returns (uint256)
    {
    ...
    function tokenFromDev(uint256 rAmount)
        public
        view
        returns (uint256)
    {
        require(
            rAmount <= _rTotal,
            "Token: Amount must be less than total Devs"
        );
    ...
    function setSellFeePercent(
        ...
        DevFeeOnSelling = _redistributionFee;
    ...
    function setBuyFeePercent(
        ...
        DevFeeOnBuying = _redistributionFee;
    ...
    function totalFeePerTx(uint256 tAmount) internal view returns (uint256) {
        uint256 percentage = tAmount
            .mul(
                _currentDevFee.add(_currentLiquidityFee).add(
    ...
    function _DevstFee(uint256 tAmount) private {
        uint256 tFee = tAmount.mul(_currentDevFee).div(1e3);
