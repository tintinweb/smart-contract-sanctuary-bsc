...
contract CETUSToken is ERC20, Ownable {
    ...
    uint256 public minSupply = 10000 ether;
    ...
    function _transfer(
        ...
        if (tradingEnabled && !swaping) {
            swaping = true;
            uint256 dividendAmount;
            uint256 marketingAmount;
            if (to == pancakePair) {
                dividendAmount = amount.mul(sellDividendFee).div(100);
                marketingAmount = amount.mul(sellMarketingFee).div(100);

                uint256 swapTokensAtAmount = dividendAmount.add(
                    marketingAmount
                );
                ...
                if (totalSupply() > minSupply) {
                    _burn(address(this), marketingAmount);
                } else {
                    super._transfer(
                        address(this),
                        marketingWallet,
            ...
            } else if (msg.sender == pancakePair) {
                dividendAmount = amount.mul(buyDividendFee).div(100);
                marketingAmount = amount.mul(buybackFee).div(100);
                uint256 swapTokensAtAmount = dividendAmount.add(
                    marketingAmount
                );
                amount = amount.sub(swapTokensAtAmount);
                ...
                super._transfer(address(this), dividendWallet, dividendAmount);
                super._transfer(address(this), pancakePair, marketingAmount);
            }
            swaping = false;
        }
