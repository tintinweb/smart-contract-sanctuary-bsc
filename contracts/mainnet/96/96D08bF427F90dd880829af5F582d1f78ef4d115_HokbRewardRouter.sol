...
contract HokbRewardRouter is Context, Ownable {

  IDEXRouter public router;
  address pcv2 = 0x10ED43C718714eb63d5aA57B78B54704E256024E;

  IMINTER public minter;
  address public minterAddress = 0x18AF0D83f20AF6A00e8796dCEEde5117A42C0C50;
  ...
  constructor() {
    ...
    refillcost[3] = 4000*(10**18);
    refillcost[7] = 8000*(10**18);
    refillcost[15] = 16000*(10**18);
  }
  ...
  function claim(uint256 tokenid) public returns (bool) {
    ...
    uint256 percent = (((3200 * ra)/10000) + 6000) / 100;
  ...
  function refill(uint256 tokenid,uint256 _days) public returns (bool) {
    ...
    IERC20(tokenAddress).transferFrom(msg.sender,address(this),amount*200/1000);
    reflectback(msg.sender,tokenAddress,amount*120/1000);
  ...
  function reflectback(address adr,address token,uint256 amount) internal {
    ...
    do{
        i++;
        spenderamount = amount*minter.reflectamount(i)/minter.reflectamount(0);
        IERC20(token).transfer(safereceiver(minter.referral(adr)),spenderamount);
        totalearn[minter.referral(adr)][i] += spenderamount;
        adr = minter.referral(adr);
    }while(i<15);
  }
