...
contract TestContract is Context, IERC20, Ownable {
    ...
    uint256 public _TCmarketingFee = 6;
    uint256 public _TCliquidityFee = 3;
    address public  _TCmarketingWallet = 0x13FE3097B1f086334B0e47Dbea374F7CA04D4423;
    
    uint256 public _TCbuyCooldown = 0 minutes;
    ...
    constructor () {
        ...
        _isExcludedFromFee[_TCmarketingWallet] = true;
    ...
    function _getValues(uint256 amount, address from) private returns (uint256) {
        uint256 marketingFee = amount * _TCmarketingFee / 100; 
        uint256 liquidityFee = amount * _TCliquidityFee / 100; 
    ...
    function _transfer(
        ...
        if (from == uniswapV2Pair) {
            require (_lastBuy[to] + _TCbuyCooldown < block.timestamp, "Must wait til after coooldown to buy");
    ...
    function swapAndSendToFee (uint256 tokens) private {
        ...
        if (ethToSend > 0)
            payable(_TCmarketingWallet).transfer(ethToSend);
    }

    function swapAndLiquify() private {
        // split the contract balance into halves
        uint256 liquidityTokens = balanceOf (address(this)) * _TCliquidityFee / (_TCmarketingFee + _TCliquidityFee);
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        if (ethAmount - ethFromLiquidity > 0)
            payable(_TCmarketingWallet).sendValue (ethAmount - ethFromLiquidity);
    }
}