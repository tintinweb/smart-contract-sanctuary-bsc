...
contract JOJO is Context, IERC20, Ownable {
    ...
    uint256 private minimumTokensBeforeSwap = _totalSupply.mul(2).div(10**5); // 0.002%;
    ...
    constructor(
        ...
        IPancakeRouter02 _router = IPancakeRouter02(_routerAddress);
        ...
        _totalFee = _liquidityFee.add(_marketingFee).add(_devFee);
    ...
    function _transfer(
        ...
        if (launchedAt == 0 && recipient == pairAddress)
        ...
        if (inSwapAndLiquify) {
            ...
            if (
                ...
                // send fee to dev wallet address
                uint256 devTokens = contractTokenBalance.mul(_devFee).div(
                    _totalFee
                );
                if (devTokens > 0) swapAndSendToFee(devTokens, devAddress);
                ...
                if (marketingTokens > 0)
                    swapAndSendToFee(marketingTokens, marketingAddress);

                // add liquidity
                uint256 swapTokens = contractTokenBalance.sub(devTokens).sub(
                    marketingTokens
                );
