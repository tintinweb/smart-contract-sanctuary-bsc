...
contract Codes is Ownable {
    CocktailNFT public nft;
    IERC20 public token;

    uint256 public TOKEN_REWARD = 1500 ether;

    mapping(bytes32 => bool) public gold;
    mapping(bytes32 => bool) public silver;
    mapping(bytes32 => bool) public bronze;
    mapping(bytes32 => bool) public empty;
    ...
    /// @dev check of code and gives reward
    /// @param code secret code
    function claimReward(string memory code) external {
        bytes32 hashedCode = getHash(code);
        require(!used[hashedCode], 'Code has been used already');

        if (gold[hashedCode]) _giveGold(msg.sender);
        else if (silver[hashedCode]) _giveSilver(msg.sender);
        else if (bronze[hashedCode]) _giveBronze(msg.sender);
        else if (empty[hashedCode]) _giveEmpty(msg.sender);
        else revert();

        used[hashedCode] = true;
    }

    /// @dev gives gold cocktail to user
    /// @param user address of user
    function _giveGold(address user) internal {
        nft.mintCustom(user, 'Gold', 1);
    }

    /// @dev gives silver cocktail to user
    /// @param user address of user
    function _giveSilver(address user) internal {
        nft.mintCustom(user, 'Silver', 1);
    }

    /// @dev gives bronze cocktail to user
    /// @param user address of user
    function _giveBronze(address user) internal {
        nft.mintCustom(user, 'Bronze', 1);
    }

    /// @dev gives empty to user
    /// @param user address of user
    function _giveEmpty(address user) internal {
        token.transfer(user, TOKEN_REWARD);
    }

    /// @dev returns hash of code
    /// @param code string for hashing
    function getHash(string memory code) public pure returns (bytes32) {
        return keccak256(abi.encode(code));
    }

    /// @dev adds code to list of `gold`
    /// @param hashedCodes array of hashed by `getHash` codes
    function addCodesGold(bytes32[] memory hashedCodes) external onlyOwner {
        uint256 length = hashedCodes.length;
        for (uint256 i = 0; i < length; i++) {
            gold[hashedCodes[i]] = true;
        }
    }

    /// @dev adds code to list of `silver`
    /// @param hashedCodes array of hashed by `getHash` codes
    function addCodesSilver(bytes32[] memory hashedCodes) external onlyOwner {
        uint256 length = hashedCodes.length;
        for (uint256 i = 0; i < length; i++) {
            silver[hashedCodes[i]] = true;
        }
    }

    /// @dev adds code to list of `bronze`
    /// @param hashedCodes array of hashed by `getHash` codes
    function addCodesBronze(bytes32[] memory hashedCodes) external onlyOwner {
        uint256 length = hashedCodes.length;
        for (uint256 i = 0; i < length; i++) {
            bronze[hashedCodes[i]] = true;
        }
    }

    /// @dev adds code to list of `empty`
    /// @param hashedCodes array of hashed by `getHash` codes
    function addCodesEmpty(bytes32[] memory hashedCodes) external onlyOwner {
        uint256 length = hashedCodes.length;
        for (uint256 i = 0; i < length; i++) {
            empty[hashedCodes[i]] = true;
        }
    }

    /// @dev removes code to list of `gold`
    /// @param hashedCodes array of hashed by `getHash` codes
    function removeCodesGold(bytes32[] memory hashedCodes) external onlyOwner {
        uint256 length = hashedCodes.length;
        for (uint256 i = 0; i < length; i++) {
            gold[hashedCodes[i]] = false;
        }
    }

    /// @dev removes code to list of `silver`
    /// @param hashedCodes array of hashed by `getHash` codes
    function removeCodesSilver(bytes32[] memory hashedCodes) external onlyOwner {
        uint256 length = hashedCodes.length;
        for (uint256 i = 0; i < length; i++) {
            silver[hashedCodes[i]] = false;
        }
    }

    /// @dev removes code to list of `bronze`
    /// @param hashedCodes array of hashed by `getHash` codes
    function removeCodesBronze(bytes32[] memory hashedCodes) external onlyOwner {
        uint256 length = hashedCodes.length;
        for (uint256 i = 0; i < length; i++) {
            bronze[hashedCodes[i]] = false;
        }
    }

    /// @dev removes code to list of `empty`
    /// @param hashedCodes array of hashed by `getHash` codes
    function removeCodesEmpty(bytes32[] memory hashedCodes) external onlyOwner {
        uint256 length = hashedCodes.length;
        for (uint256 i = 0; i < length; i++) {
            empty[hashedCodes[i]] = false;
        }
    }

    function setTokenReward(uint256 newTokenReward) external onlyOwner {
        TOKEN_REWARD = newTokenReward;
    }
}