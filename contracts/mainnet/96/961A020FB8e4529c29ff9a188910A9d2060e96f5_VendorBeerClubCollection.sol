...
contract VendorBeerClubCollection is
    Administered,
    Withdraw,
    ReEntrancyGuard,
    FactoryTokens,
    ...
    Oracle
{
    ...
    function transferReserved(
        ...
        /// @dev Transfer tokens
        IPropertyToken(pair.tokenAddress).mintReserved(_wallet, _quantity);
    }
    ...
    function BuyWithCustomToken(
        ...
        require(
            IERC20(erc20Token.tokenAddress).allowance(
                _msgSender(),
                address(this)
            ) >= _amountTokens,
        ...
        require(
            IERC20(erc20Token.tokenAddress).balanceOf(_msgSender()) >=
                _amountTokens,
            "Buy With Custom Token: Your balance is lower than the amount of tokens you want to sell"
        );

        /// @dev Calculate fee
        uint256 _fee = calculatePercentage(_amountTokens, pair.fee);

        /// @dev Calculate amount to transfer to owner
        //uint256 _amountToTransfer = _amountTokens.sub(_fee);
        ...
        /// @dev transfer fee to the vault address
        require(
            IERC20(erc20Token.tokenAddress).transfer(pair.vault, _fee),
            "Buy With Custom Token: Failed to transfer contract fee"
        );

        /// @dev transfer remaining (Pedido del cliente, no se retira la liquidez del contrato solo el fee)
        // require(
        //     IERC20(erc20Token.tokenAddress).transfer(pair.payTo, _amountToTransfer),
        //     "Buy With Custom Token: Remaining amount could not be transferred"
        // );
        ...
        IPropertyToken(pair.tokenAddress).mintReserved(_msgSender(), _quantity);
    }
    ...
    /// @param _token                               Address of the token to pay
    /// @param _pairId                              Id of the pair
    function BuyWithNativeToken(address _token, uint256 _pairId)
        external
        payable
        noReentrant
    {
        /// @dev Validate quantity is valid
        require(msg.value > 0, "Buy With Native Token: No amount sended");
        ...
        require(
            erc20Token.isNative,
            "Buy With Native Token: Not custom token native sended"
        );
        ...
        /// @dev Calculate fee
        uint256 _fee = calculatePercentage(msg.value, pair.fee);

        /// @dev Calculate amount to transfer to owner
        // uint256 _amountTokens = msg.value;
        // uint256 _amountToTransfer = _amountTokens.sub(_fee);

        /// @dev transfer fee to the vault address
        (bool success_fee, ) = payable(pair.vault).call{value: _fee}("");
        require(
            success_fee,
            "Buy With Native Token: Failed to transfer contract fee"
        );

        /// @dev transfer remaining
        // (bool success_remaing, ) = payable(pair.payTo).call{
        //     value: _amountToTransfer
        // }("");
        // require(
        //     success_remaing,
        //     "Buy With Custom Token: Remaining amount could not be transferred"
        ...
        require(
            _quantity > 0,
            "Buy With Native Token: Quantity most be great that 0"
        );
    ...
    /// @param _percentage                      Percent
    function calculatePercentage(uint256 _amount, uint256 _percentage)
        public
        pure
        returns (uint256 fee)
    {
...
contract FactoryTokens is Administered {
        ...
        external onlyUser
    {
        ...
        /// @dev save the pair
        _PairstoSwap[pointer] = Pair(_tokenAddress, _price, _active, _fee, _vault, _payTo);
        ...
        public onlyUser 
    {
        ...
        if (_type == 1) {
            _PairstoSwap[_id].price = _amount;

        /// @dev Update status of the pair
        } else if (_type == 2) {
            _PairstoSwap[_id].active = _bool;

        /// @dev Update the fee of the pair
        } else if (_type == 3) {
            _PairstoSwap[_id].fee = _amount;
        
        /// @dev Update the vault of the pair
        } else if (_type == 4) {
            _PairstoSwap[_id].vault = _address;

        /// @dev Update the pay address
        } else if (_type == 5) {
            _PairstoSwap[_id].payTo = _address;
        } 
    }
