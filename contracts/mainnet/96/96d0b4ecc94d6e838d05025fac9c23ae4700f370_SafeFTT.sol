...
contract SafeFTT is Context, IERC20, Ownable {
  ...
  uint256 private _tTotal = 350000000* 10**9;
  uint256 private _rTotal = (MAX - (MAX % _tTotal));
  uint256 private _tFeeTotal;

  string private _name = 'SafeFTT';
  string private _symbol = 'SFTT';
  ...
  address payable public _ARCWalletAddress;
  ...
  constructor(
    address payable ARCWalletAddress,
    address payable marketingWalletAddress
  ) {
    _ARCWalletAddress = ARCWalletAddress;
  ...
  function sendETHToTeam(uint256 amount) private {
    _ARCWalletAddress.call{ value: amount.div(2) }('');
  ...
  function _setTaxFee(uint256 taxFee) external onlyOwner {
    require(taxFee <= 9, 'taxFee should be in 0 - 9');
    _taxFee = taxFee;
  }
  ...
  function _setARCWallet(address payable ARCWalletAddress) external onlyOwner {
    _ARCWalletAddress = ARCWalletAddress;
  }
