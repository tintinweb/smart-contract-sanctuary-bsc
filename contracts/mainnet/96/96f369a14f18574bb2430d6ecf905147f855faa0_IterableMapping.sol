...
contract BaoBey is ERC20, Ownable {
    ...
    uint256 public treasuryFeeOnTransfer;
    ...
    constructor() payable ERC20("BaoBey", "BeBe") {
        ...
        liquidityFeeOnTransfer  = 1;
        treasuryFeeOnTransfer   = 2;
        rewardsFeeOnTransfer    = 2;
        burnFeeOnTransfer       = 1;

        totalTransferFee        = liquidityFeeOnTransfer + treasuryFeeOnTransfer + rewardsFeeOnTransfer + burnFeeOnTransfer;

        treasuryWallet = 0x73829CDA55E4A609b937e9A213ebd39E49798166;
        ...
        transferOwnership(0xe9e46DE3527D2681742c545F1d0ff12bd8Dd083B);
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            if(liquidityFeeOnBuy + liquidityFeeOnSell + liquidityFeeOnTransfer > 0) {
                liquidityTokens = contractTokenBalance * (liquidityFeeOnBuy + liquidityFeeOnSell + liquidityFeeOnTransfer) / 100;
                swapAndLiquify(liquidityTokens);
            }

            contractTokenBalance -= liquidityTokens;

            uint256 bnbShare = (treasuryFeeOnBuy + treasuryFeeOnSell + treasuryFeeOnTransfer) + (rewardsFeeOnBuy + rewardsFeeOnSell + rewardsFeeOnTransfer);
            ...
            if(contractTokenBalance > 0 && bnbShare > 0) {
                ...
                if((treasuryFeeOnBuy + treasuryFeeOnSell) > 0) {
                    uint256 treasuryBNB = newBalance * (treasuryFeeOnBuy + treasuryFeeOnSell + treasuryFeeOnTransfer) / bnbShare;
                    sendBNB(payable(treasuryWallet), treasuryBNB);
                    emit SendMarketing(treasuryBNB);
                }

                if((rewardsFeeOnBuy + rewardsFeeOnSell) > 0) {
                    uint256 rewardBNB = newBalance * (rewardsFeeOnBuy + rewardsFeeOnSell + rewardsFeeOnTransfer) / bnbShare;
        ...
        if(takeFee) {
            ...
            if(from == uniswapV2Pair) {
                _totalFees = totalBuyFee - burnFeeOnBuy;
                _totalBurn = amount * burnFeeOnBuy / 100;
            } else if (to == uniswapV2Pair){
                _totalFees = totalSellFee - burnFeeOnSell;
                _totalBurn = amount * burnFeeOnSell / 100;
            } else{
                _totalFees = totalTransferFee - burnFeeOnTransfer;
                _totalBurn = amount * burnFeeOnTransfer / 100;
            }
            
            uint256 fees = amount * _totalFees / 100;
            
            amount = amount - fees - _totalBurn;

            super._transfer(from, DEAD, _totalBurn);
