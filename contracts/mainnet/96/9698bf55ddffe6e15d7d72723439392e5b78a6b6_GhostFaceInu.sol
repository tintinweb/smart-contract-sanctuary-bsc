...
Token Symbol: $GHOSTFACE
Decimals: 18

Website: https://ghostfaceinu.com
Telegram: https://t.me/GhostFaceInuBSC
Twitter: https://twitter.com/GhostFace_inu
*/
...
contract GhostFaceInuDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("GhostFaceInu_Dividend_Tracker", "GhostFaceInu_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000000 * (10**18);
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "GhostFaceInu_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "GhostFaceInu_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main GhostFaceInu contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "GhostFaceInu_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "GhostFaceInu_Dividend_Tracker: Cannot update claimWait to same value");
...
contract GhostFaceInu is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    GhostFaceInuDividendTracker public dividendTracker;
    ...
    constructor() ERC20("GhostFaceInu", "$GHOSTFACE") {
        
        BNBRewardsFee = 2;
        liquidityFee = 2;
        extraFeeOnSell = 0; // extra sell fee
        marketingFee = 4;
        marketingWallet = payable(0x6e6fbC0D7Eeb1EB457C52CA0F36Ba27221f3bBb3);

    
        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new GhostFaceInuDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "GhostFaceInu: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "GhostFaceInu: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "GhostFaceInu: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "GhostFaceInu: Cannot update gasForProcessing to same value");
