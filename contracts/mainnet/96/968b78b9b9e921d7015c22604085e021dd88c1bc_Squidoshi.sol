...
Squidoshi is a revolutionary Bep-20 GameFi ecosystem backed by real financial asset investments, dual-reward staking, and NFT marketplace.

If you miss Shiba Inu or Dogecoin, do not miss Squidoshi.

Read our Whitepaper (https://squidoshi.gitbook.io/squidoshi) to learn more 

➡️Marketing partnership with 3 major firms
➡️Dual Reward staking platform done
➡️Over 13 AMAs completed 
➡️1/4 game installment completed
➡️KYC by InterFI
➡️Certik audit/SAFU planned
➡️Team expansion incoming
➡️Project Launch incoming 
➡️Advisory/Incubator/VC partnerships incoming

https://t.me/Squidoshinc
...
contract DividendDistributor is IDividendDistributor {
    ...
    uint256 public minDistribution = 100000 * (10 ** 12);
...
contract Squidoshi is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "Squidoshi";
    string constant _symbol = "SQUIDOSHI";
    ...
    uint256 reflectionFee   = 4; //reflections+rewards
    ...
    constructor () Auth(msg.sender) {
        ...
        LagoonReceiver = 0x8D399Ebc91ebE828476Ac5aA61c514b950e0dCE3;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBLagoon = amountBNB.mul(Lagoon).div(totalBNBFee);  
        if (amountBNBReflection > 0){
            try distributor.deposit{value: amountBNBReflection}() {} catch {}
        }

        (bool tmpSuccess,) = payable(marketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
        (bool tmpSuccess1,) = payable(LagoonReceiver).call{value: amountBNBLagoon, gas: 30000}("");
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _Lagoon, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        reflectionFee = _reflectionFee;
        marketingFee = _marketingFee;
        Lagoon= _Lagoon;
        totalFee = _liquidityFee.add(_reflectionFee).add(_marketingFee).add(_Lagoon);
        feeDenominator = _feeDenominator;
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _LagoonReceiver) external authorized {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        marketingFeeReceiver = _marketingFeeReceiver;
        LagoonReceiver = _LagoonReceiver;

        isFeeExempt[_autoLiquidityReceiver];
        isFeeExempt[_marketingFeeReceiver];
        isFeeExempt[_LagoonReceiver];
    }
