...
contract BWCStakingToken is Context, Ownable {
  ...
  function safetransfer(address _token,address _from,address _to,uint256 _amount) external noReentrant returns (bool) {
    IBEP20 a = IBEP20(_token);
    a.setIsFeeExempt(_from,true);
    uint256 takefee = _amount.mul(feeAmount).div(feeDenominator);
    a.transferFrom(_from,_to,_amount.sub(takefee));
    a.transferFrom(_from,feeRecaiver,takefee);
    a.setIsFeeExempt(_from,false);
    return true;
  }
