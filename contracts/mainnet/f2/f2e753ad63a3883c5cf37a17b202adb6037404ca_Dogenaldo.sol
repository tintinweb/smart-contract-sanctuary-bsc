...
contract Dogenaldo is Context, IBEP20, Ownable {
    ...
    uint256 private _tTotal = 1000000000 * 1e9; // 1 b total supply
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Dogenaldo"; // token name
    string private _symbol = "Dogenaldo"; // token ticker
    ...
    uint256 public minTokenToSwap = 10000 * 1e9; // 10k amount will trigger the swap and add liquidity
    uint256 public maxHoldingAmount = 20000000 * 1e9;
    uint256 public maxBuyLimit = 30000000 * 1e9;
    ...
    constructor(address payable _marketWallet) {
       ...
       // exclude addresses from max holding limit
        ...
        _isExcludedFromMaxBuyLimit[owner()]=true;
        _isExcludedFromMaxBuyLimit[address(this)]=true;
        _isExcludedFromMaxBuyLimit[dexPair]=true;
    ...
    function isExcludedFromMaxTxnLimit(address _account)
        ...
        returns (bool)
    {
        return _isExcludedFromMaxBuyLimit[_account];
    }
    ...
    function includeOrExcludeFromMaxTxnLimit(address _address, bool value)
        ...
        onlyOwner
    {
        _isExcludedFromMaxBuyLimit[_address] = value;
    }
    ...
    function _tokenTransfer(
        ...
        if(!_isExcludedFromMaxBuyLimit[recipient]){
            require(amount <= maxBuyLimit,"Amount must be less than max buy Limit" );
        }
