...
contract BabyPoo is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  uint256 public maxSupply = 28195;
  ...
  // Lists 
  uint256[28195] public remainingIds;
  ...
  constructor(string memory name, string memory symbol, string memory baseTokenURI) ERC721(name, symbol) {
      ...
      // Save dev wallet
      artistWallet = 0x6a5262aD0Ed89D2e4074Ed006c0A42eAc81a6577;
      projectWallet = 0x44432EE347704842794EDF96121dad7bAFCA4ef3;
      dappWallet = 0x1057a936a87cb28674956a62C43C546998912323;
      devWallet = 0x13F32c1c3F13B6b6c8Fe230c4F915eB9607E1E0C;
  }
  ...
  function mint(uint256 _mintAmount) public payable {
    // Checks
    require(!paused, "BabyPoo: minting has not started yet");
    require(_mintAmount > 0, "BabyPoo: you have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "BabyPoo: you can only mint 10 at a time");
    require(lastSupply >= _mintAmount,  "BabyPoo: the collection is sold out");
    if (msg.sender != owner()) {
      require(msg.value >= cost * _mintAmount, "BabyPoo: total cost isn't match");
    }
    // Minting
  ...
  function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
    // Checks
    require(_winner != address(0), "BabyPoo: zero address");
    require(_amount > 0, "BabyPoo: you have to mint at least one");
    require(lastSupply != 0,  "BabyPoo: the collection is sold out");
  ...
  function tokenURI(uint256 _id) public view virtual override returns (string memory){
    require(_exists(_id),"BabyPoo: URI query for nonexistent token");
  ...
  function withdraw() public payable onlyOwner {
    (bool success, ) = payable(devWallet).call{value: address(this).balance}("");
    require(success);
  }
  ...
  function handleMintReward(uint256 _amount) private {
      ...
      uint256 devShare = _amount.mul(10).div(100);
      uint256 dappShare = _amount.mul(5).div(100);
      uint256 artistShare = _amount.mul(5).div(100);
      uint256 mktShare = _amount - mintShare - devShare - dappShare - artistShare;

      // Save dividend for community
      reflectDividend(mintShare);
      // Send owner share
      payable(devWallet).transfer(devShare);
      payable(dappWallet).transfer(dappShare);
      payable(artistWallet).transfer(artistShare);
      payable(projectWallet).transfer(mktShare);
  }
