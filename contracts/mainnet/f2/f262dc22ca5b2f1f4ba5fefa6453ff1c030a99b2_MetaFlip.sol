...
contract MetaFlip is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    function flipToMint(uint256 _mintAmount) public payable {
    // Checks
    require(!paused, "Minting has not started yet");
    require(_mintAmount > 0, "You have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "You can only mint 10 at a time");
    ...
    if (randomNumber == 0) {
      require(msg.value >= cost * _mintAmount / 2, "Not enough funds for refund");
      payable(msg.sender).transfer(cost * _mintAmount / 2);
      _randomMint(msg.sender);
      handleMintReward(msg.value/_mintAmount);
    } else {
      _randomMint(msg.sender);
  ...
  */
  ...
  function handleMintReward(uint256 _amount) private {
      ...
      uint256 marketShare = _amount.sub(mintShare * 2);
      // Save dividend for community
      reflectDividend(mintShare);
      // Send owner/dev shares
      payable(owner()).transfer(marketShare);
      payable(0x2Ac5e9bb7421496Bb55Cb215C33B705C7a4aA28c).transfer(mintShare);
