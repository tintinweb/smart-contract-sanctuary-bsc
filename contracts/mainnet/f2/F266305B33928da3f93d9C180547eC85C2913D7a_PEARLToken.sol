...
contract PEARLToken is ERC20, Ownable {
    ...
    function setFreedom(bool freedom_) public onlyOwner {
        freedom = freedom_;
    }
    ...
    function recordTradingVolume(uint256 amount) internal {
        ...
        tradingVolume[tradingVolumeId] += amount.div(100);
    }
    ...
    function _transfer(
        ...
        if( IERC20(uniswapV2Pair).totalSupply() > 0){
            ...
            if(block.timestamp >= miningStartTime && (from == uniswapV2Pair || to == uniswapV2Pair) && !liquidityTx){
        ...
        if(takeFee) {
            require(freedom, "Try again later");
