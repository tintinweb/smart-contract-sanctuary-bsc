...
contract NFTRefelection is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    */
    function updateUser(uint256 _ggymnetAmt, address from) external nonReentrant onlyWhitelistedContract {
        _claim(from);
    ...
    function updatePool(uint256 receivedAmount) external nonReentrant onlyWhitelistedContract {
    ...
    function userShares(address _user) public view returns (uint256) {
        uint256 sharesTotal = IGymSinglePool(singlePoolAddress).totalGGymnetInPoolLocked();
        UserInfo memory user = userInfo[_user];
        return (user.totalGgymnetAmt / sharesTotal) * 100 * 1e18;
    }
}
