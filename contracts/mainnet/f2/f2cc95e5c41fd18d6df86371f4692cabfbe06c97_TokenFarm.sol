...
contract TokenFarm is Ownable {
    ...
    struct UserInfo {
        ...
        uint256 maturityTime; // The time after which the user can withdraw his tokens with rewards.
    }
    ...
    function stakeTokens (uint256 _pid, uint256 _amount) public {
        ...
        uint256 _maturityTime = (block.timestamp).add((poolInfo[_pid].lockDays).mul(1 days)); 
    ...
    function unstakeTokens (uint256 _pid) public {
        ...
        require (userInfo[_pid][msg.sender].maturityTime < block.timestamp , "Your maturity time is not reached. If you want you can do EmergencyWithdraw");

        address _tokenAddress = poolInfo[_pid].tokenAddress;
        IBEP20 token = IBEP20 (_tokenAddress);
        uint256 _refundValue = token.balanceOf(address(this)).mul(userInfo[_pid][msg.sender].amount).div(poolInfo[_pid].currentPoolSize);
    ...
    function emergencyWithdraw (uint256 _pid) public {
        require (_pid < poolLength(), "Invalid pool ID");
        require (userInfo[_pid][msg.sender].amount > 0 , "You don't have any staked tokens");
        require (userInfo[_pid][msg.sender].maturityTime > block.timestamp , "Your maturity time is reached. You can unstake tokens and enjoy rewards");

        uint256 _refundValue = (userInfo[_pid][msg.sender].amount)
            .sub((poolInfo[_pid].emergencyFees)
