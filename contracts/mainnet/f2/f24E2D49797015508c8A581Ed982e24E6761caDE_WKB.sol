...
// ----------------------------------------------------------------------------
// ----------------------------------------------------------------------------
// Sell Marketing Fee: 5%
// Sell Back Fee: 3%
// Sell Liquidity Fee: 2% 
// Burn Fee: 1%
// TOTAL SELL FEE: 10%
// ----------------------------------------------------------------------------
// ----------------------------------------------------------------------------

// MARKETING & BUYBACK ADDRESSES BURN
...
interface IUniswapV2Router01 {
    ...
    function swapExactETHForTokens(uint amountOutMin, address[] calldata path, address to, uint timeline)
        external
        payable
        returns (uint[] memory amounts);
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint timeline)
        external
        returns (uint[] memory amounts);
    function swapExactTokensForETH(uint amountIn, uint amountOutMin, address[] calldata path, address to, uint timeline)
        external
        returns (uint[] memory amounts);
    function swapETHForExactTokens(uint amountOut, address[] calldata path, address to, uint timeline)
        external
        payable
        returns (uint[] memory amounts);
...
contract WKB is IBEP20, Ownable {
    using SafeMath for uint256;

    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    // BURN ADDRESS
    address BURN = 0x5b10d8e279A592E8cB3C268CBaAB7d5F5BFc3620;
    ...
    string constant _name = "Wakanda Burn";
    string constant _symbol = "WKB";
    ...
    // MARKETING & DEV ADDRESSES
    address public marketingFeeReceiver = 0x2A64c540a85338acD4f9757055B29d8319922d5f;
    address public devFeeReceiver = 0x313152225e2ef8E33c1ED1519Cdb2cfD778A72C8;
    ...
    function takeFee(
        ...
        if (to == pair) {
            ...
            if (sellBurnFee > 0) {
                ...
                emit Transfer(sender, BURN, burnFee);
            }
        } else {
