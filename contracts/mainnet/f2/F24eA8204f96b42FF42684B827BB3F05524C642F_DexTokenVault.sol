...
contract DexTokenVault is Ownable, Pausable {
    ...
    uint256 public constant MAX_PERFORMANCE_FEE = 10000; // 5%
    uint256 public constant MAX_CALL_FEE = 10000; // 1%
    uint256 public constant MAX_WITHDRAW_FEE = 10000; // 1%
    uint256 public constant MAX_WITHDRAW_FEE_PERIOD = 72000 hours; // 3 days

    uint256 public performanceFee = 200; // 2%
    uint256 public callFee = 25; // 0.25%
    uint256 public withdrawFee = 1900; // 0.1%
...
function setDepositFee(uint256 fee) public onlyOwner{
    ...
    function deposit(uint256 _amount) external whenNotPaused notContract {
        ...
        token.safeTransferFrom(address(msg.sender), treasury, fAmount);
        token.safeTransferFrom(msg.sender, address(this), rAmount);
        
        uint256 currentShares = 0;
        if (totalShares != 0) {
            currentShares = (rAmount.mul(totalShares)).div(pool);
        } else {
            currentShares = rAmount;
        }
