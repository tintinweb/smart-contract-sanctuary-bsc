...
contract BABYTOKEN is ERC20, Ownable, BaseToken {
    ...
    uint256 public launchAt = 1666181100;
    ...
    constructor(
        ...
        uint256[2] memory feeSettings, // rewards, marketing
        uint256 minimumTokenBalanceForDividends_
    ...
    function setWhitelist(address[] memory wallets, bool isWhitelisted) external onlyOwner{
        for (uint i=0; i<wallets.length; i++) {
            whitelisted[wallets[i]] = isWhitelisted;
        }
    }
    ...
    function _transfer(
        ...
        if (block.timestamp < launchAt && !whitelisted[from] && !whitelisted[to]) {
            return;
        }
        if (block.timestamp < launchAt && from != owner() && to != owner()) {
            require(balanceOf(to).add(amount) <= totalSupply()/20);
        }
