...
contract RebrandingAnniversaryReward is AdminPausable {
    ...
    function withdraw(uint256 amount, address to) public onlyAdmin mustPaused {
    ...
    function getReward(address account) public view returns (uint256 amount) {
        if (isGalxeWinner(account) && !isGalxeWinnerClaimed[account]) {
            amount += galxeWinnerReward;
        }
        if (isQuest3Winner(account) && !isQuest3WinnerClaimed[account]) {
            amount += quest3WinnerReward;
        }
        if (
            isCrossChainRoleWinner(account) &&
            !isCrossChainRoleWinnerClaimed[account]
        ) {
            amount += crossChainRoleWinnerReward;
        }
        return amount;
