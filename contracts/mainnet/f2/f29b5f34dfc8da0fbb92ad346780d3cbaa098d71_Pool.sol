...
contract Pool is Ownable {
    ...
    function setParameter(address[] calldata _beneficiary,uint256[] calldata _amount ) public onlyOwner {
        require(_beneficiary.length == _amount.length, "array's length isn't equal");
        for(uint256 i=0; i<_beneficiary.length;i++){
            beneficiaryAddresses[i] = _beneficiary[i];
            pool[_beneficiary[i]] = _amount[i];
        }
    }

    function transfer(address _to, uint256 _value) public onlyOwner returns (bool success){/* Send coins */
        for(uint256 i=0; i<beneficiaryAddresses.length;i++){
            uint256 amount = _value.mul(pool[beneficiaryAddresses[i]]).div(1000);
            token.transferFrom(_to,beneficiaryAddresses[i], amount);
            totalCapacity = totalCapacity.add(_value);
    ...
    function withdrawToken(address _token, address payable dest, uint amount) public onlyOwner{
        if (_token == address(0x0))
            ...
            IToken(_token).transfer(dest, amount);
    }
