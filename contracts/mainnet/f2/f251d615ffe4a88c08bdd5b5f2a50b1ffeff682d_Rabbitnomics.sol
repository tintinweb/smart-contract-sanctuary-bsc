...
contract Rabbitnomics is Ownable, IBEP20 {
    ...
    constructor () {
        ...
        isTimelockExempt[ecosystemFeeReceiver] = true;
        ...
        marketingFeeReceiver = 0xB33B1b74d982812133A329982035BE29dbe52463;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != burnFeeReceiver && recipient != ecosystemFeeReceiver && recipient != marketingFeeReceiver && !isTxLimitExempt[recipient]){
    ...
    function swapBack() internal swapping {
        ...
        (tmpSuccess,) = payable(ecosystemFeeReceiver).call{value: amountBNBecosystem}("");
    ...
    function addantibot(bool _status) external onlyOwner {
        antibotMode = _status;
    ...
    function manage_antibot(address[] calldata addresses, bool status) public onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
    ...
    function setTax(uint256 _liquidityFee, uint256 _teamFee, uint256 _marketingFee, uint256 _ecosystemFee, uint256 _burnFee, uint256 _feeDenominator) external onlyOwner {
