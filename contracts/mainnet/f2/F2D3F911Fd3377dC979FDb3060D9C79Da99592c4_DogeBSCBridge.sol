...
contract DogeBSCBridge is ReentrancyGuard {
    ...
    function modifyMaintenanceCost(uint256 cost) public{
      require(msg.sender==owner);
      maintenanceCost=cost;
    }

    uint256 lockFee=2;
    uint256 minFee=150000000000; //Ether unit
    ...
    function DCtoBSC(uint256 amount,address requestor) public nonReentrant{
    ...
    function BSCToDC (address receiver,uint256 amount) public nonReentrant
    {   
        require(bridge==false,"Bridge disabled.");

        uint256 bridgeFee=SafeMath.mul(SafeMath.div(amount,100),lockFee);
        if(bridgeFee<minFee){
          bridgeFee=minFee;
        }
