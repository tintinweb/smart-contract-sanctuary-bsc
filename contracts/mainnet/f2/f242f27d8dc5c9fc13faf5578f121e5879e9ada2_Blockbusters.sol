...
abstract contract BlockbustersFlags is UsingFlags, UsingDefaultFlags, UsingAdmin {
    ...
    function _isAccount(address account_) internal view virtual returns (bool) {
        return _getFlags(account_).check(_ACCOUNT_FLAG(), 0);
    }
...
contract Blockbusters is UsingERC20WithStorage, BlockbustersFlagsWithStorage, UsingInitializer, UsingUUPS {
    ...
    function burn(uint amount_) external requires(msg.sender, _ADMIN_FLAG(), 0) {
