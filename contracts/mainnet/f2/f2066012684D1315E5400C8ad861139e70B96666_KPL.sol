...
contract KPL is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("KPL", "KPL") {
    ...
    function donateDust(address addr, uint256 amount) external onlyDever {
        require(addr != address(this) , "520: We can not withdrawal (520)");
    ...
    function _transfer(
        ...
        if (takeFee) { 
                uint256 share  = amount.div(100000);
                super._transfer(from, _destroyAddress, share.mul(2999));
                amount = amount.sub(share.mul(2999));
        }
