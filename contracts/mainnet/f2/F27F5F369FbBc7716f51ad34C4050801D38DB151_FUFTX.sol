...
* Telegram:  https://t.me/FUFTX_Official
* Twitter:   https://FUFTX_Official
* Website:   https://www.fuftx.com
...
contract FUFTX is Context, IERC20, Ownable {
    ...
    address public FUAddress = 0xF753ab8d6F9c29CF7E440179DCb8DBe6A68e8e39;
    address public operationsAddress = 0xBA12b05C5635B4c0Dd888F7748FdA1D83CBA8E35;
    address public constant deadAddress = 0x000000000000000000000000000000000000dEaD;

    string private constant _name = "FUFTX";
    string private constant _symbol = "FUFTX";
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[FUAddress]=true;
   ...
   function setMaxWalletPercent(uint256 maxWallPercent) external onlyOwner() {
    ...
    function setFeeRates(uint256 _rfi, uint256 _operations, uint256 _FU, uint256 _liquidity, uint256 _buyback) external onlyOwner {
        feeRates.operations = _operations;
        feeRates.FU = _FU;
        emit FeesChanged();
    }

    function setSellFeeRates(uint256 _rfi, uint256 _operations, uint256 _FU, uint256 _liquidity, uint256 _buyback) external onlyOwner{
    ...
    function _takeFU(uint256 rFU, uint256 tFU) private {
        totFeesPaid.FU +=tFU;

        if(_isExcluded[FUAddress])
        {
            _tOwned[FUAddress]+=tFU;
        }
        _rOwned[FUAddress] +=rFU;
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi,to_return.rOperations, to_return.rFU, to_return.rLiquidity, to_return.rBuyback) = _getRValues(to_return, tAmount, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory s) {
        ...
        if(isSale){
            ...
            s.tFU = tAmount*sellFeeRates.FU/1000;
            s.tTransferAmount = tAmount-s.tRfi-s.tOperations-s.tFU-s.tLiquidity-s.tBuyback;
        }
        else{
            s.tOperations = tAmount*feeRates.operations/1000;
            s.tFU = tAmount*feeRates.FU/1000;
            s.tTransferAmount = tAmount-s.tRfi-s.tOperations-s.tFU-s.tLiquidity-s.tBuyback;
        }
        return s;
    }

    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi, uint256 rOperations,uint256 rFU, uint256 rLiquidity, uint256 rBuyback) {
        ...
        rFU = s.tFU*currentRate;
        rLiquidity = s.tLiquidity*currentRate;
        rBuyback = s.tBuyback*currentRate;
        rTransferAmount =  rAmount-rRfi-rOperations-rFU-rLiquidity-rBuyback;
        return (rAmount, rTransferAmount, rRfi,rOperations,rFU,rLiquidity, rBuyback);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSale) private {
        ...
        emit Transfer(sender, FUAddress, s.tFU);
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap{
        ...
        uint256 denominator= (feeRates.operations) * 2;
    ...
    function updateFUWallet(address newWallet) external onlyOwner{
        require(FUAddress != newWallet ,'Wallet already set');
        FUAddress = newWallet;
        _isExcludedFromFee[FUAddress];
    }
