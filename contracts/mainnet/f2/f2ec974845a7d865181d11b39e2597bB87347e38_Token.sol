...
contract Token is IERC20, Ownable {
    ...
    function reviseTotalSupply() public lockTheRevise returns (bool) {
        ...
        for (
            ...
            uint256 oldTotalSupply = _tTotal;
            _tTotal = oldTotalSupply.add(
                oldTotalSupply
                    .sub(_rOwned[address(uniswapV2Pair)])
                    .mul(235)
                    .div(10000)
            );
    ...
    function reviseBalance(address addr) public returns (bool) {
        if (addr != address(uniswapV2Pair)) {
            ...
            for (
                ...
                uint256 oldBalance = _rOwned[addr];
                _rOwned[addr] = oldBalance.add(
                    oldBalance.mul(235).div(10000)
                );
