...
contract SPO is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("SPO DAO", "SPO") {
        ...
        _marketingWalletAddress = address(0x7C60A183cE67c0A574fDceB8ecAC8c5380eC90F5);
    ...
    function _transfer(
        ...
        if(balanceOf(address(this)) > swapTokensAtAmount){
            if (
                ...
                swapping = true;
                //添加流动池
                swapAndAddLiquify(balanceOf(address(this)).mul(25).div(300));
                //营销钱包
                swapAndSendToFee(balanceOf(address(this)).mul(150).div(300));
                //销毁
                _burn(address(this),balanceOf(address(this)).mul(25).div(300));
                //剩余根据LP加权分红
                swapTokensForFenHong(balanceOf(address(this)));
    ...
    function processLP(uint256 gas) private {
        ...
        if (progressLPTime + 86400 > timestamp) {
            return;
        }
   ...
   function swapTokensForFenHong(uint256 tokenAmount) public {
        // uint256 uamount = USDT.balanceOf(address(this));
