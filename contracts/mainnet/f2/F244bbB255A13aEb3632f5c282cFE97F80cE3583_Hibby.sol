...
contract Hibby is Context, IERC20, AccessControlEnumerable {
    ...
    constructor (address _teamWallet, address _developmentWallet, address _marketingWallet) {
        ...
        uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
            .createPair(address(this), 0xcA143Ce32Fe78f1f7019d7d551a6402fC5350c73);
    ...
    function getTotalDeductionFee() external view returns(uint){
        return (fees.reflectionFee + fees.teamWalletFee + fees.developmentFee + fees.marketingFee);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if ( sender != uniswapV2Pair){
            if(!inSwap){
                if ( collected.teamCollection + collected.developmentCollection + collected.marketingCollection > 0 ){

                    swapTokensForToken(collected.teamCollection, wallets.teamWallet);
                    swapTokensForToken(collected.developmentCollection, wallets.developmentWallet);
                    swapTokensForToken(collected.marketingCollection, wallets.marketingWallet);
        ...
        if( (sender == uniswapV2Pair || 
            ...
            collected.developmentCollection += ( amount * fees.developmentFee ) / 100;
            collected.marketingCollection += ( amount * fees.marketingFee ) / 100;
            _takeFee = true;
            uint256 tax = (amount * (fees.teamWalletFee + fees.developmentFee + fees.marketingFee) / 100 );
            _takeTaxes(tax);         
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount, bool _takeFee) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee) = _getValues(tAmount, _takeFee);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
    ...
    function _transferToExcluded(address sender, address recipient, uint256 tAmount, bool _takeFee) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee) = _getValues(tAmount, _takeFee);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
    ...
    function _transferFromExcluded(address sender, address recipient, uint256 tAmount, bool _takeFee) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee) = _getValues(tAmount, _takeFee);
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount, bool _takeFee) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee) = _getValues(tAmount, _takeFee);
        _tOwned[sender] = _tOwned[sender].sub(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _tOwned[recipient] = _tOwned[recipient].add(tTransferAmount);
    ...
    function _getValues(uint256 tAmount, bool _takeFee) private view returns (uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee) = _getTValues(tAmount, _takeFee);
        uint256 currentRate =  _getRate();
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, currentRate);
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee);
    }

    function _getTValues(uint256 tAmount, bool _takeFee) private view returns (uint256, uint256) {
        if (_takeFee){
            uint256 tFee = (tAmount.mul(fees.reflectionFee + fees.teamWalletFee + fees.developmentFee + fees.marketingFee)).div(100);
    ...
    function _burn(address account, uint256 tAmount) internal virtual {
        ...
        (uint256 rAmount, , uint256 rFee, ,uint256 tFee) = _getValues(tAmount, false);
    ...
    function swapTokensForToken(uint256 tokenAmount, address _account) public lockTheSwap(){
    ...
    function updateRouter(address newAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(
        ...
        address _uniswapV2Pair = IUniswapV2Factory(uniswapV2Router.factory())
            .createPair(address(this), 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7);
        uniswapV2Pair = _uniswapV2Pair;
    }

    // Function to change the owner
    function changeOwner(address _address) external onlyRole(DEFAULT_ADMIN_ROLE) {
        owner = _address;
    }
}
