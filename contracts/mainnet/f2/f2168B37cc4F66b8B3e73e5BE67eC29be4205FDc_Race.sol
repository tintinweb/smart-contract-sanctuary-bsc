...
contract Race is OwnableUpgradeable {
    ...
    function execAutopilot(address _addr) external returns (bool) {
        require(msg.sender == autopilot_executor || msg.sender == variableTaxcontrollerAddress, "not allowed");
        if(_addr != address(0)){
            ...
            if(users[_addr].deposits > 0 && users[_addr].deposits < maxRollAmount){ // check for deposits. this also covers if a user has ejected
                ...
                if( msg.sender == autopilot_executor){
    ...
    function deposit(address _upline, uint256 _amount) external {
        ...
        if (claimsAvailable(_addr) > _amount / 100 
            ...
            uint256 claimedDivs = _claim(_addr, true);
             ...
             users[_addr].rolls += taxedDivs;

            if(variableTaxcontrollerAddress != address(0)){
                // track the stats (gross amount!)
                variableTaxcontroller.increaseNitroDirect(claimedDivs, _addr);
            }
    ...
    // like a roll but taxed 19% and without the 5x rule
    function manualCompound() external {
        address _addr = msg.sender;
        _manualCompound(_addr);
    }
    ...
    function _roll(address _addr, bool _skipCheck) internal {
        ...
        if(variableTaxcontrollerAddress != address(0)){
            // track the stats (gross amount!)
            variableTaxcontroller.increaseNitroDirect(to_payout, _addr);
        }
    ...
    function payoutOf(address _addr, bool isClaimedOut) public view returns(uint256 payout, uint256 max_payout, uint256 net_payout, uint256 sustainability_fee) {
        return payoutOf(_addr, isClaimedOut, true);
    }
    //@dev Calculate the current payout and maxpayout of a given address
    function payoutOf(address _addr, bool isClaimedOut, bool useBoosterRate) public view returns(uint256 payout, uint256 max_payout, uint256 net_payout, uint256 sustainability_fee) {
    ...
    function getBoostedPayout(address _addr) public view returns(uint256 boostedPayout){
        (uint256 _slot0_percent, 
        uint256 _slot1_percent,
        uint256 _slot2_percent, 
        uint256 _slot0_endtime,
        uint256 _slot1_endtime, 
        uint256 _slot2_endtime, 
        uint256 _slot0_gastank, 
        uint256 _slot1_gastank, 
        uint256 _slot2_gastank) = variableTaxcontroller.getBoosterSlotInfo(_addr);
        uint256 last_timestamp = users[_addr].deposit_time;
        if(_slot0_endtime > block.timestamp){
            uint256 share0 = _slot0_gastank.mul(_slot0_percent.mul(1e18)); //divide the profit by payout rate
            share0 = share0.div(100e18).div(24 hours); // divide by seconds in the day
            share0 = share0.div(10); // back to normal base
            boostedPayout = boostedPayout + share0 * block.timestamp.safeSub(last_timestamp);
        }
        if(_slot1_endtime > block.timestamp){
            uint256 share1 = _slot1_gastank.mul(_slot1_percent.mul(1e18)); //divide the profit by payout rate
            share1 = share1.div(100e18).div(24 hours); // divide by seconds in the day
            share1 = share1.div(10); // back to normal base
            boostedPayout = boostedPayout + share1 * block.timestamp.safeSub(last_timestamp);
        }
        if(_slot2_endtime > block.timestamp){
            uint256 share2 = _slot2_gastank.mul(_slot2_percent.mul(1e18)); //divide the profit by payout rate
            share2 = share2.div(100e18).div(24 hours); // divide by seconds in the day
            share2 = share2.div(10); // back to normal base
            boostedPayout = boostedPayout + share2 * block.timestamp.safeSub(last_timestamp);
        }
    }
    function availableFuel(address _addr) public view returns(uint256 base_payout, uint256 boosted_payout) {
        (,,uint256 net_payout_without_booster,) = payoutOf(_addr, false, false);
        (,,uint256 net_payout_with_booster,) = payoutOf(_addr, false, true);
        base_payout = net_payout_without_booster;
        boosted_payout = net_payout_with_booster.sub(net_payout_without_booster);
    }
    ...
    function rebaseOwner() external onlyOwner {
        users[owner()].deposit_time = block.timestamp;
        users[owner()].accumulatedDiv = 0;
    }


    function fixNitroClaim(address _addr) external onlyOwner {
        users[_addr].rolls = users[_addr].payouts;
    }

    //@dev Claim and deposit in one transaction;
    function _manualCompound(address _addr) internal {

        require(users[_addr].deposits > 0, "user used eject or zero deposits");        

        uint256 to_payout = _claim(_addr, false); // 3% referral included

        //check gross amount. no tax are deducted yet
        uint256 payout_taxed = to_payout.mul(81/* receive 81% */).div(100); // 19% tax because it is handled like a claim and a deposit. claim 10 pstn = 9 pstn after tax. deposit 9 pstn is 8.1 pstn after tax. == 19% tax
        ...
        usersRealDeposits[_addr].deposits += payout_taxed;
        usersRealDeposits[_addr].deposits_BUSD += pistonTokenPriceFeed.getPrice(1).mul(payout_taxed).div(1 ether); // new cash in BUSD        
...
interface IVariableTaxController {
    ...
    function increaseNitroDirect(uint256 _nitro_amount, address _addr) external;
    ...
    function getBoosterRate( address _addr ) external view returns (uint256);
    function getBoosterSlotInfo(address _addr) external view returns (uint256 _slot0_percent, uint256 _slot1_percent, uint256 _slot2_percent, uint256 _slot0_endtime, uint256 _slot1_endtime, uint256 _slot2_endtime, uint256 _slot0_gastank, uint256 _slot1_gastank, uint256 _slot2_gastank );
}
