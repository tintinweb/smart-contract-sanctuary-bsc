...
contract EverBurn is Ownable, IERC20 {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if(rebaseEnabled) {
            currentPrice = getQuote().mul(1 * 10**5).div(quoteBase);
            ...
            if(block.timestamp > currentDate.add(86400)) {
                ...
                transferLimit = targetPrice.mul(dailyUSDLimit).mul(1 * 10**13);
            } 
            
            if(currentPrice <= targetPrice.mul(95).div(100) && block.timestamp >= lastRebase.add(rebaseCooldown)) {
