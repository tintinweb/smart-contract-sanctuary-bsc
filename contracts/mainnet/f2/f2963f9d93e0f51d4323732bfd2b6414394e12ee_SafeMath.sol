...
//*
//*Submitted by PayCash
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.10;
contract PayCash {
    using SafeMath for uint256;
    token public BUSD = token(0x13Bd311FB72D2E5Edd2536507CB81a31E016a031);
    ...
    uint256[3] public REFERRAL_PERCENTS = [8_000, 3_000, 2_000];
    ...
    function ReinvestSTake() public returns (bool) {
        ...
        for (uint256 i = 0; i < user.deposits.length; i++) {
            if (
                ...
                dividends = (user.deposits[i].amount.mul(3).div(100))
                    .mul(block.timestamp.sub(user.deposits[i].start))
                    .div(TIME_STEP);
