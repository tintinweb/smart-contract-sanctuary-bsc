...
contract SwapPoolForAirdropFee is Ownable {
    using SafeMath for uint256;
    using Address for address;
    ...
    EnumerableSet.AddressSet private swapUserList;

    IERC20 public swapToken;
    uint256 public swapPrice;
    address public blackHoleAddress = address(1);
    address public feeToken;
    uint256 public feePrice = 15 * 10 ** 18;
    address payable public feeAddress;
    mapping(address => bool) public takeFeeList;
    ...
    mapping(address => EnumerableSet.UintSet) private UserSwapTokenIdSet;
    ...
    function setFeeInfo(address _feeToken, uint256 _feePrice, address _feeAddress) external onlyOwner {
        feeToken = _feeToken;
        feePrice = _feePrice;
    ...
    function SwapToken(uint256 _tokenId) external payable {
        require(canSwap, "e001");
        uint256 allFee = feePrice;
        if (!takeFeeList[msg.sender]) {
            if (feeToken == address(0)) {
                require(msg.value == allFee, "e002");
                feeAddress.transfer(allFee);
            } else {
                IERC20(feeToken).transferFrom(msg.sender, feeAddress, allFee);
            }
            takeFeeList[msg.sender] = true;
        }
        userTokenIdList[msg.sender].push(_tokenId);
        UserSwapTokenIdSet[msg.sender].add(_tokenId);
        SwapNFT.transferFrom(msg.sender, blackHoleAddress, _tokenId);
        SwapTokenIdSet.add(_tokenId);
        if (!swapUserList.contains(msg.sender)) {
            swapUserList.add(msg.sender);
        }
        emit swapTokenEvent(msg.sender, _tokenId, block.timestamp);
    }

    function SwapTokenList(uint256[] memory _tokenIdList) external payable {
        require(canSwap, "e001");
        uint256 allFee = feePrice;
        if (!takeFeeList[msg.sender]) {
            if (feeToken == address(0)) {
        ...
        for (uint256 i = 0; i < _tokenIdList.length; i++) {
            ...
            UserSwapTokenIdSet[msg.sender].add(_tokenIdList[i]);
    ...
    function getSwapUserList() external view returns (uint256 num, address[] memory userList) {
        num = swapUserList.length();
        userList = swapUserList.values();
    }

    struct UserSwapTokenIdItem {
        uint256 num;
        uint256[] tokenIdList;
    }

    function getUserSwapTokenIdSet(address _user) public view returns (UserSwapTokenIdItem memory) {
        return UserSwapTokenIdItem(UserSwapTokenIdSet[_user].length(),UserSwapTokenIdSet[_user].values());
    }

    function getUserListSwapTokenIdSet(uint256[] memory _indexList) external view returns (UserSwapTokenIdItem[] memory UserSwapTokenIdList_) {
        UserSwapTokenIdList_ = new UserSwapTokenIdItem[](_indexList.length);
        for (uint256 i=0;i<_indexList.length;i++) {
            address _user = swapUserList.at(_indexList[i]);
            UserSwapTokenIdList_[i] = getUserSwapTokenIdSet(_user);
        }
    }

    function getAllUserListSwapTokenIdSet() external view returns (UserSwapTokenIdItem[] memory UserSwapTokenIdList_) {
        uint256 num = swapUserList.length();
        UserSwapTokenIdList_ = new UserSwapTokenIdItem[](num);
        for (uint256 i=0;i<num;i++) {
            address _user = swapUserList.at(i);
            UserSwapTokenIdList_[i] = getUserSwapTokenIdSet(_user);
        }
    }
