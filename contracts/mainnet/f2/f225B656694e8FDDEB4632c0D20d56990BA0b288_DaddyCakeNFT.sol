...
contract DaddyCakeNFT is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  // Lists 
  uint256[555] public remainingIds;
  ...
  constructor(string memory name, string memory symbol, string memory baseTokenURI) ERC721(name, symbol) {
      ...
      // Save dev wallet
      _wallet = 0xf6ec2f0069D0092fF120120d36D9EFF33a92767A;
      _artistWallet = 0x00310a8483d4517b90c8cc8137209A75DeAb4e22;
  }
  ...
  function mint(uint256 _mintAmount) public payable {
    // Checks
    require(!paused, "DC: minting has not started yet");
    require(_mintAmount > 0, "DC: you have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "DC: you can only mint 3 at a time");
    require(lastSupply >= _mintAmount,  "DC: the collection is sold out");
    if (msg.sender != owner()) {
      require(msg.value >= cost * _mintAmount, "DC: total cost isn't match");
    }
    // Minting
  ...
  function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
    // Checks
    require(_winner != address(0), "DC: zero address");
    require(_amount > 0, "DC: you have to mint at least one");
    require(lastSupply != 0,  "DC: the collection is sold out");
  ...
  function tokenURI(uint256 _id) public view virtual override returns (string memory){
    require(_exists(_id),"DC: URI query for nonexistent token");
  ...
  */
  ...
  function handleMintReward(uint256 _amount) private {
      ...
      payable(_wallet).transfer(devShare.div(100).mul(90));
      payable(_artistWallet).transfer(devShare.div(100).mul(10));
  }
