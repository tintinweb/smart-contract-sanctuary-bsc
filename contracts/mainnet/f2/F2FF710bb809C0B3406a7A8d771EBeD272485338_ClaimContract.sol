...
contract ClaimContract is AccessControl, ReentrancyGuard, Pausable {
    ...
    mapping(address => LockInfo) public lockedData;
    ...
    function getLockedData(address _user) external view returns (
        uint128,
        uint128,
        uint64,
        uint64,
        ...
        LockInfo storage _lockInfo = lockedData[_user];
        return (
            _lockInfo.amount,
    ...
    function sendLockToken(
        ...
        lockedData[_user] = lockinfo;
    }

    function claimToken(uint128 _amount) external nonReentrant whenNotPaused returns (uint256) {
