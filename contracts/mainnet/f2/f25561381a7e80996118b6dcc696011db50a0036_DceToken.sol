...
contract DceToken is ERC20, ERC20Burnable, Ownable {
    ...
    constructor(address p, address holder) ERC20("DceToken", "DCE") {
        _mint(holder, 10000000 * 10**decimals());
        projectAdd = p;
        isOpenBuy = false;
    ...
    function _beforeTokenTransfer(
        ...
        if (!VIPs[from] && DEXs[to]) {
            ...
            uint256 _cost = (amount * 10) / 100;
            require(fromBalance > amount + _cost, "invalid balance");
            transferCost(from, amount);
            if (totalSupply() > 1000000 ether) {
                uint256 _burnFee = (amount * 35) / 1000;
                _burn(from, _burnFee);
            }
            inviteBouns(from, amount, from);
        }
    }
        ...
        returns (bool)
    {
        ...
        if (DEXs[owner] && !VIPs[to]) {
            require(isOpenBuy, "forbidden action");
            transferCost(owner, amount);
            inviteBouns(owner, amount, to);
            if (totalSupply() > 1000000 ether) {
                uint256 _burnFee = (amount * 35) / 1000;
                _burn(owner, _burnFee);
                _transfer(owner, to, (amount * 90) / 100);
            } else {
                _transfer(owner, to, (amount * 935) / 1000);
            }
            return true;
        }
        if (!DEXs[owner] && !DEXs[to] && Team[to] == address(0)) {
            Team[to] = owner;
        }
        _transfer(owner, to, amount);
        return true;
    ...
    function transferCost(address from, uint256 amount) internal {
        uint256 _fee = (amount * 15) / 1000;
        _transfer(from, projectAdd, _fee);
    }

    function inviteBouns(
        address from,
        uint256 amount,
        address _start
    ) internal {
        uint8 _counter = 0;
        address _target = Team[_start];
        uint256 _amount = (amount * 5) / 1000;
        while (_counter < 10) {
            if (_target != address(0)) {
                _transfer(from, _target, _amount);
            } else {
                _transfer(from, projectAdd, _amount);
            }
            _target = Team[_target];
            _counter++;
        }
    }
}
