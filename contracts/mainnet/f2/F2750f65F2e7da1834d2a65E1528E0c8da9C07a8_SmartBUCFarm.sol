 ...
 */
contract SmartBUCFarm is BaseFallback {
    /**
...
interface INFTFarm {
	function depositNFT(uint256 _pid, uint256 _amount, address fromAddress,uint extraDot) external;
	function withdrawNFT(uint256 _pid, uint256 _amount, address fromAddress,uint extraDot) external;
    function pendingSushi(uint256 _pid, address _user) external view returns (uint256);
    function getPoolAmount(uint256 _pid) external view returns(uint256);
}

contract BUCFarm is Ownable,INFTFarm {
    ...
    function pendingSushi(uint256 _pid, address _user) override external view returns (uint256) {
	...
	function depositNFT(uint256 _pid, uint256 _amount, address fromAddress,uint extraDot) public override{
        require(msg.sender == nftAddress);
		PoolInfo storage pool = poolInfo[_pid];
		UserInfo storage user = userInfo[_pid][fromAddress];
		updatePool(_pid);
		if (user.amount > 0) {
		    uint256 pending = user.amount.mul(pool.accSushiPerShare).div(1e12).sub(user.rewardDebt);
		    if(pending > 0) {
				pending = pending.mul(user.amount + extraDot).div(user.amount);
		        safeSushiTransfer(fromAddress, pending);
		    }
		}
		if(_amount > 0) {
		    pool.amount = pool.amount.add(_amount);
		...
		user.rewardDebt = user.amount.mul(pool.accSushiPerShare).div(1e12);
		emit DepositNFT(fromAddress, _pid, _amount, 0);
	}
	
	function withdrawNFT(uint256 _pid, uint256 _amount, address fromAddress,uint extraDot) public override{
        require(msg.sender == nftAddress);
		PoolInfo storage pool = poolInfo[_pid];
		UserInfo storage user = userInfo[_pid][fromAddress];
		...
		user.rewardDebt = user.amount.mul(pool.accSushiPerShare).div(1e12);
		emit WithdrawNFT(fromAddress, _pid, _amount);
	}
	
    function getPoolAmount(uint256 _pid) override public view returns(uint256){
        PoolInfo storage pool = poolInfo[_pid];
        return pool.amount;
    }
