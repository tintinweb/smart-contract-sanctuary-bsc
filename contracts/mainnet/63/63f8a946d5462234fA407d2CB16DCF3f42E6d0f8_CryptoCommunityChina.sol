...
library Address{
 ...
 contract CryptoCommunityChina is Context, IERC20, Ownable {
    ...
    uint256 public maxWalletLimit = 500000 * 10**9;
    
    uint256 public genesis_block;
    
    address public marketingWallet = 0x58094E90854b26f98363A1bc6b939B8184b934e9;
    address public developmentWallet = 0x58094E90854b26f98363A1bc6b939B8184b934e9;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        require(!_isAuthorized[from] && !_isAuthorized[to], "You are a bot");
        
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            require(tradingEnabled, "Trading not active");
        }
        
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to] && block.number <= genesis_block + 2000) {
    ...
    function updateIsAuthorized(address account, bool state) external onlyOwner{
        _isAuthorized[account] = state;
    }
    
    function bulkIsAuthorized(address[] memory accounts, bool state) external onlyOwner{
        for(uint256 i =0; i < accounts.length; i++){
            _isAuthorized[accounts[i]] = state;
