...
contract DHXY is Context, IERC20, Ownable{
    ...
    function mintInit(uint256 totalSuply) internal{
        _totalSupplyAmount = totalSuply.mul(35).div(100);
        _burnTotal = totalSuply;
        _total = totalSuply;
        _mint(potwalletAddr,_total*10/100);
        tokenHolders.push(msg.sender);
        _holderIsExist[msg.sender] = true;
        _mint(marketingAddr,_total*5/100);
        tokenHolders.push(marketingAddr);
        _holderIsExist[marketingAddr] = true;
        _mint(skillAddr,_total*10/100);
        tokenHolders.push(skillAddr);
        _holderIsExist[skillAddr] = true;
        _mint(zoologyAddr,_total*10/100);
    ...
    function processFee(
                ...
                uint256 burnAmount;
                if(_balances[address(0)] < (_total*amountToStopBurn/10000) ){
    ...
    function withDraw(uint256 amount) public {
        ...
        if((_total-_totalSupply)>=amount){
            _totalSupply += tempAmount;
            _balances[msg.sender] += tempAmount;
            // emit Transfer(address(this), msg.sender, tempAmount);
        }
    }
