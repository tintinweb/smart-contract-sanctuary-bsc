...
contract UbtStake is OwnableUpgradeable, ERC721HolderUpgradeable {
    ...
    function initialize() initializer public {
        ...
        wallet = 0x0E01A61cC704F2f79233f1A881277214fd4ff427;
        status = true;
    }
    ...
    function calculateReward(address addr) public view returns (uint){
        ...
        if (quota >= user.quotaLeft) {
            rew = coutingToken(user.quotaLeft - user.toClaimQuota, price) + user.toClaim;
        } else {
    ...
    function _calculateReward(address addr) internal view returns (uint){
        ...
        uint quota = coutingQuota(rew, price) + user.toClaimQuota;
        if (quota >= user.quotaLeft) {
            rew = coutingToken(user.quotaLeft - user.toClaimQuota, price);
        } else {
            rew = rew;
        }
        return rew;
    ...
    function allRew(address addr,uint price) internal view returns(uint){
        ...
        uint rew = coutingToken(userInfo[addr].quotaLeft - userInfo[addr].toClaimQuota , price) + userInfo[addr].toClaim;
        return rew;
    }
        ...
        for (uint i = 0; i < 10; i++) {
            ...
            if (i < 2) {
                ...
                if (tempQuota + quota + userInfo[temp].toClaimQuota >= userInfo[temp].quotaLeft) {
                    ...
                    left += tokenAmount / 20;
                    userInfo[temp].claimed += tokenAmount;
                    ...
                    userInfo[temp].toClaimQuota += coutingQuota(rew, price);
                    left -= tempRew;
    ...
    function claimReward() checkDeadline external {
        ...
        token.transfer(wallet, _calculateReward(msg.sender) / 20);
