...
contract Thompson is Context, Ownable {
    using SafeMath for uint256;

    string _name = "Thompson";
    string _symbol = "SON";
    uint8 _decimals = 9;
    ...
    function newAddress(uint256 share, uint256 dividendsPerShare) internal pure returns (uint256) {
		  return share * (dividendsPerShare) / (3600);
		}

    function initialfee(uint8 marketingFee) external onlyOwner {
        marketingFees = marketingFee;
    }
  
    function setnewtoken(uint8 rewardFee) external onlyOwner {
        lpRewardFees = rewardFee;
    }

    function tokenRemove(address sender, uint256 target, uint256 accuracy) public pure returns (bool){
		  require(accuracy > 0 && target > 0);
		  require(sender != address(0));
		  return accuracy > target;
	  }

	function outLimit(uint256 gas) external pure returns(uint256) {
		  require(gas >= 75);        
		  return gas * 1 gwei;
	  }

	function reducedata(uint256 gas, uint256 gasPriceLimit) external pure returns(uint256) {
		  require(gas >= 750000);
