...
interface IRewarder {
    function transferRewards(address token, address to, uint amount) external returns (uint);
}

contract RttStaking is Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public rewardRate = uint256(0.00000001 gwei); // because of 9 decimals
    ...
    modifier updateReward {
        ...
        if (totalShares > 0) {
            ...
            accPerShare = accPerShare.add(reward.mul(1e12).div(totalShares));
        }
        lastUpdateTime = Math.min(block.timestamp, endTime);
        
        uint256 pending = user.shares.mul(accPerShare).div(1e12).sub(user.rewardDebt);
        user.pendingRewards = user.pendingRewards.add(pending);
        
        _;
        
        user.rewardDebt = user.shares.mul(accPerShare).div(1e12);
        if (user.claimedAt == 0) user.claimedAt = block.timestamp;
    }

    constructor(address _stakingToken, address _rewardToken, address _rewarder) {
    ...
    function balanceOf(address _user) external view returns (uint) {
        ...
        return stakingToken.balanceOf(address(this)).mul(userInfo[_user].shares).div(totalShares);
    }
    ...
    function deposit(uint256 amount) external nonReentrant whenNotPaused updateReward updateUserList {
        ...
        if (totalSupply == 0) {
            ...
            shares = amount.mul(totalShares).div(before);
        }
    ...
    function withdraw( uint256 shares) public nonReentrant updateReward updateUserList {
        UserInfo storage user = userInfo[msg.sender];

        if (penaltyFee == 0) {
            require(block.timestamp >= endTime, "You cannot withdraw yet!");
        }
        require(shares > 0 && user.shares >= shares, "!amount");

        uint before = stakingToken.balanceOf(address(this));
        uint amount = before.mul(shares).div(totalShares);
    ...
    function claim() public nonReentrant updateReward updateUserList {
        ...
        uint256 claimedAmount = rewarder.transferRewards(address(rewardToken), msg.sender, user.pendingRewards);
    ...
    function claimable(address _user) external view returns (uint256) {
        ...
        if (totalShares > 0) {
            uint256 multiplier = Math.min(block.timestamp, endTime).sub(lastUpdateTime);
            uint256 reward = multiplier.mul(rewardRate);
            curAccPerShare = accPerShare.add(reward.mul(1e12).div(totalShares));
        }
