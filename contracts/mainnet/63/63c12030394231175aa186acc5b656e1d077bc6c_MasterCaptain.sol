...
//
contract MasterCaptain is Ownable {
    ...
    // The maximum deposit fee allowed is 95%
    uint16 public MAX_DEPOSIT_FEE = 9500;
    // Deposit Fee address
    ...
    // Update the given pool's BUSD allocation point and deposit fee. Can only be called by the owner.
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 busdReward = multiplier.mul(busdPerBlock).mul(pool.allocPoint).div(totalAllocPoint);                
        // This is normally where the mint would happen, Busd has a fixed supply and does not mint any tokens.
