...
import "./IPancakeMasterChefV2.sol";
import "./IGenericPancakeMasterChef.sol";
...
contract DeltaNeutralPancakeMCV2Worker02 is
  OwnableUpgradeable,
  ...
  IWorker02
{
  ...
  event SetWhitelistedCallers(
    address indexed caller,
    address indexed whitelistUser,
    bool indexed isOk
  );
  ...
  modifier onlyEOA() {
    if (msg.sender != tx.origin)
      revert DeltaNeutralPancakeMCV2Worker02_NotEOA();
    _;
  }
  ...
  modifier onlyOperator() {
    if (msg.sender != operator)
      revert DeltaNeutralPancakeMCV2Worker02_NotOperator();
    _;
  }

  //// @dev Require that the caller must be ok reinvestor.
  modifier onlyReinvestor() {
    if (!okReinvestors[msg.sender])
      revert DeltaNeutralPancakeMCV2Worker02_NotReinvestor();
    _;
  }

  //// @dev Require that the caller must be whitelisted caller.
  modifier onlyWhitelistedCaller(address user) {
    if (!whitelistCallers[user])
      revert DeltaNeutralPancakeMCV2Worker02_NotWhitelistedCaller();
    _;
  }
  ...
  function _reinvest(
    ...
    if (_treasuryAccount == address(0))
      revert DeltaNeutralPancakeMCV2Worker02_BadTreasuryAccount();
    ...
    if (bounty > 0) {
      uint256 beneficialVaultBounty = (bounty * beneficialVaultBountyBps) /
        BASIS_POINT;
  ...
  function work(
    ...
    if (!okStrats[strat])
      revert DeltaNeutralPancakeMCV2Worker02_UnApproveStrategy();
  ...
  function setReinvestConfig(
    ...
    if (_reinvestBountyBps > maxReinvestBountyBps)
      revert DeltaNeutralPancakeMCV2Worker02_ExceedReinvestBounty();

    if (baseToken == cake) {
      if (_reinvestPath.length != 1)
        revert DeltaNeutralPancakeMCV2Worker02_InvalidReinvestPathLength();
    } else {
      if (_reinvestPath.length < 2)
        revert DeltaNeutralPancakeMCV2Worker02_InvalidReinvestPathLength();
    }
  ...
  function setMaxReinvestBountyBps(uint256 _maxReinvestBountyBps)
    ...
    onlyOwner
  {
    if (reinvestBountyBps > _maxReinvestBountyBps)
      revert DeltaNeutralPancakeMCV2Worker02_ExceedReinvestBounty();
    // _maxReinvestBountyBps should not exceeds 30%
    if (_maxReinvestBountyBps > 3000)
      revert DeltaNeutralPancakeMCV2Worker02_ExceedReinvestBps();
  ...
  /// @param isOk - Whether to approve or unapprove the given strategies.
  function setWhitelistedCallers(address[] calldata callers, bool isOk)
    external
    onlyOwner
  {
  ...
  function setTreasuryConfig(
    ...
    if (_treasuryBountyBps > maxReinvestBountyBps)
      revert DeltaNeutralPancakeMCV2Worker02_ExceedReinvestBounty();
  ...
  function setBeneficialVaultConfig(
    ...
    if (_beneficialVaultBountyBps > 10000)
      revert DeltaNeutralPancakeMCV2Worker02_ExceedReinvestBps();
