...
contract ZCRFMv is ERC777 {
    ...
    function mintManyAddress(uint256[] calldata _values, address[] calldata _recipients, uint[] calldata _ids) external {
        require(_msgSender() == owner,"Only the owner can mint tokens!");        
        require(_values.length == _recipients.length, "Data error!");        
        require(_values.length == _ids.length, "Data error!");        
        require(_recipients.length == _ids.length, "Data error!");
        
        for(uint i = 0; i < _values.length; i++) {
            uint nftid = _ids[i];
            require(nfts[nftid] == address(0) , "This NFT is minted!");
            mint(_recipients[i], _values[i], "", "");
            nfts[_ids[i]] = _recipients[i];
        }
    }  
