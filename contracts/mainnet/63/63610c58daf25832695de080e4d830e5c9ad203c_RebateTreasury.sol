...
contract RebateTreasury is Ownable {
    ...
    constructor(address ape, address apeOracle, address treasury) {
        Ape = IERC20(ape);
        ApeOracle = IOracle(apeOracle);
        Treasury = ITreasury(treasury);
    }
    
    // Bond asset for discounted Ape at bond rate

    function bond(address token, uint256 amount) external onlyAsset(token) {
        require(amount > 0, "RebateTreasury: invalid bond amount");
        uint256 apeAmount = getApeReturn(token, amount);
        require(apeAmount <= Ape.balanceOf(address(this)) - totalVested, "RebateTreasury: insufficient ape balance");

        IERC20(token).transferFrom(msg.sender, address(this), amount);
        _claimVested(msg.sender);

        VestingSchedule storage schedule = vesting[msg.sender];
        schedule.amount = schedule.amount - schedule.claimed + apeAmount;
        schedule.period = bondVesting;
        schedule.end = block.timestamp + bondVesting;
        schedule.claimed = 0;
        schedule.lastClaimed = block.timestamp;
        totalVested += apeAmount;
    }

    // Claim available Ape rewards from bonding
    ...
    function setApe(address ape) external onlyOwner {
        Ape = IERC20(ape);
    }

    // Set Ape oracle

    function setApeOracle(address oracle) external onlyOwner {
        ApeOracle = IOracle(oracle);
    }
    ...
    function _claimVested(address account) internal {
        ...
        Ape.transfer(account, claimable);
    }
    ...
    // Calculate Ape return of bonding amount of token

    function getApeReturn(address token, uint256 amount) public view onlyAsset(token) returns (uint256) {
        uint256 apePrice = getApePrice();
        uint256 tokenPrice = getTokenPrice(token);
        uint256 bondPremium = getBondPremium();
        return amount * tokenPrice * (bondPremium + DENOMINATOR) * assets[token].multiplier / (DENOMINATOR * DENOMINATOR) / apePrice;
    }
    ...
    function getBondPremium() public view returns (uint256) {
        uint256 apePrice = getApePrice();
        if (apePrice < 1e18) return 0;

        uint256 apePremium = apePrice * DENOMINATOR / 1e18 - DENOMINATOR;
        if (apePremium < bondThreshold) return 0;
        if (apePremium <= secondaryThreshold) {
            return (apePremium - bondThreshold) * bondFactor / DENOMINATOR;
        } else {
            uint256 primaryPremium = (secondaryThreshold - bondThreshold) * bondFactor / DENOMINATOR;
            return primaryPremium + (apePremium - secondaryThreshold) * secondaryFactor / DENOMINATOR;
        }
    }

    // Get APE price from Oracle

    function getApePrice() public view returns (uint256) {
        return ApeOracle.consult(address(Ape), 1e18);
    }
    ...
    // Get claimable vested Ape for account

    function claimableApe(address account) external view returns (uint256) {
