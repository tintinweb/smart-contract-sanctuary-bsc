...
contract NewETH is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    NewETHDividendTracker public dividendTracker;
    ...
    constructor() ERC20("NewETH", "TEST") {
        marketingWallet = payable(0xd4Eb6E155a122B942A7cB6FAFD43435EEe01E3da);
        devWallet = payable(0xd4Eb6E155a122B942A7cB6FAFD43435EEe01E3da);
        ...
        dividendTracker = new NewETHDividendTracker(
            payable(this),
            router,
        ...
        uint256 totalTokenSupply = (200_000_000_000_000) * (10**18);
...
contract NewETHDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        NewETHContract = NewETH(mainContract);
    ...
    function _transfer(
        ...
        require(false, "NewETH_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "NewETH_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main NewETH contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(NewETHContract);

            uint256 prevBalance = NewETHContract.balanceOf(address(this));
            ...
            {
                uint256 received = NewETHContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    NewETHContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
