...
contract SATUZI is IERC20, API, Ownable {
    ...
    function shouldSwapBack(address sender, address recipient, uint256 amount) internal view returns (bool) {
        ...
        return !swapping && swapEnabled && !isFeeExempt[sender] && aboveMin && recipient == pair && swapAmount >= uint256(2) && canSwap;
    }

    function swapBack(address sender, address recipient, uint256 amount) internal {
        if(shouldSwapBack(sender, recipient, amount)){
            swapAndLiquify(swapThreshold); swapAmount = uint256(0); emit SwapbackOccurred(block.timestamp);}
    }
