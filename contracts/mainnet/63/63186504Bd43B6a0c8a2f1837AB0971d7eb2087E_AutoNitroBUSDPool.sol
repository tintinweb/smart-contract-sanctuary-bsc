...
// File: contracts/NitroAutoBUSD.sol
...
contract AutoNitroBUSDPool is Ownable {
    ...
    uint256 public payAmount = 6 * 10 ** 18;
    uint256 public rebuyAmount = 38 * 10 ** 18;
    uint256[] public sponsorEarnAmount = [1, 2, 3, 1];
    uint256 public oneAmount = 1 * 10 ** 18;
    ...
    address public marketingWallet = address(0x8eCe26CB76A79A4B52214b2bA6b21554373Bb3E2);
    address public buybackWallet = address(0x9a3A33d6D7F9E1b9C3F60898B499BCec53532477);
    ...
    /* MAPPINGS */
    mapping(address => address[]) public paidUserToSponsor;
    mapping(uint => address) public idToAddress;
    mapping(address => uint) public addressToId;
    mapping(uint => MissedPayments) public missedPayment;
    ...
    event newMaxLevel(uint maxLevel);
    ...
    constructor() {    
        ...
        newSlot.paidCount = 0;
        newSlot.level = 0;
        newSlot.createdAt = block.timestamp;
    ...
    function checkBoughtSlots(address wallet) public view returns (uint refPayLevel)  {
        if(wallet == buybackWallet) return 0;
        uint _refPayLevel = 0;
        Users storage u = User[wallet];
    ...
    function getQueue() public view returns(uint256[] memory, uint256) {
        uint256[] memory slots;
        for(uint i=0; i<paymentQueue.length; i++) {
            slots[i] = paymentQueue[i];
        }
        return(slots, paymentQueue.length);
    }

    function getActiveSlot() public view returns(uint256) {
        return(paymentQueue[activeSlot]);
    }
    ...
    function BuySlot() public {
        ...
        uint _boughtSlots = checkBoughtSlots(_buyer);
        uint _slotToPay = paymentQueue[activeSlot];
        Users storage u = User[_buyer];
        require(_boughtSlots < buyLimit, "2 Slot Per 24 hrs Limit");

        if (u.slots == 0) {
            Users storage s = User[u.invitedBy];
            s.roundToPaidRefs[currentRound]++;
        }
        ...
        Users storage uToPay = User[slotToPay.user];
        uToPay.paid += payAmount;
        BUSD.safeTransferFrom(address(this), slotToPay.user, payAmount);
        activeSlot++;
        ...
        if (slotToPay.paidCount == 2) {
            
            slotToPay.level++;           

            if (slotToPay.level == maxLevel) {
                BUSD.safeTransferFrom(address(this), buybackWallet, rebuyAmount);
                BUSD.safeTransferFrom(address(this), dev0SupportWallet, oneAmount * 10 / 2);
                BUSD.safeTransferFrom(address(this), dev1SupportWallet, oneAmount * 10 / 2);
            } else {

                slotToPay.paidCount = 0;
                doubling = 2;

                paymentQueue.push(_slotToPay);
                paymentQueue.push(_slotToPay);

                // NEXT SLOT PAY START
                uint _next_slotToPay = paymentQueue[activeSlot];
                Slots storage next_slotToPay = Slot[_next_slotToPay];
                next_slotToPay.paidCount++;
                Users storage next_uToPay = User[next_slotToPay.user];
                // REFPAYMENTS
                    refPayment(next_uToPay.invitedBy);
                next_uToPay.paid += payAmount;
                BUSD.safeTransferFrom(address(this), next_slotToPay.user, payAmount);
                activeSlot++;
                emit paySlot(owner(), next_slotToPay.user, payAmount, totalSlots, _next_slotToPay);
                totalPaid += payAmount;
                // NEXT SLOT PAY END
        ...
        BUSD.safeTransferFrom(address(this), owner(), poolSupportAmount * doubling);  // 2
        BUSD.safeTransferFrom(address(this), marketingWallet, oneAmount * doubling); // 1
        BUSD.safeTransferFrom(address(this), buybackWallet, oneAmount * doubling); // 1
        BUSD.safeTransferFrom(address(this), dev0SupportWallet, (oneAmount / 2) * doubling); // 0.5
        BUSD.safeTransferFrom(address(this), dev1SupportWallet, (oneAmount / 2) * doubling); // 0.5
        BUSD.safeTransferFrom(address(this), vaultWallet, vaultTokenAmount * doubling); // 5
        BUSD.safeTransferFrom(address(this), leaderboardWallet, oneAmount * doubling); // 1

        totalDeposited += slotPrice;
    }

    function refPayment(address _sponsor) internal {
        for (uint8 i = 0; i < sponsorEarnAmount.length; i++) {            
            uint _transferamount = sponsorEarnAmount[i] * 10 ** 18; // [1, 2, 3, 1]
            totalRefPaid += _transferamount;
            Users storage innerSponsor = User[_sponsor];
            uint _levelPaid = 0;
            
            if (_sponsor == owner() || _sponsor == buybackWallet) _levelPaid = 2;
            else _levelPaid = checkBoughtSlots(_sponsor);
            ...
            if ((i == 0) || (i == 1)) {
                if (_levelPaid >= 1) {
                    ...
                    missedPayments++;
                    innerSponsor.missedRefPayment.push(missedPayments);
                    MissedPayments storage mPayment = missedPayment[missedPayments];
                    mPayment.id = missedPayments;
                    mPayment.user = _sponsor;
                    mPayment.amount = _transferamount;
                    mPayment.claimedBefore = block.timestamp + timeLimit;
                    missedPayment[missedPayments] = mPayment;
                }
            }
            if ((i == 2) || (i == 3)) {
                if (_levelPaid >= 2) {
    ...
    function claimAllMissed() public {
        address _claimer = msg.sender;
        uint _claimerLevel = checkBoughtSlots(_claimer);
        require(_claimerLevel >= 2, "No bought slots last 24 Hrs");
        Users storage u = User[_claimer];
        uint256[] memory _missedPayments = u.missedRefPayment;
        uint256 _toPay = 0;
        uint256 _lastPaidSlot = u.lastPaidMissed;
        uint256 lastPaidSlot_ = 0;
        for(uint i = 0; i < _missedPayments.length; i++) {
            MissedPayments memory mPayment = missedPayment[_missedPayments[i]];
            if (mPayment.id > _lastPaidSlot) {
                if (mPayment.claimedBefore > block.timestamp) {
                    _toPay += mPayment.amount;
                }
                lastPaidSlot_ = mPayment.id;
            }
        }
        if (_toPay > 0) {
            require((BUSD.balanceOf(address(this)) >= _toPay), "No BUSD");
            BUSD.safeTransferFrom(address(this), _claimer, _toPay);
            u.lastPaidMissed = lastPaidSlot_;
        }
    }

    function getclaimAllMissed() public view returns (uint256, uint256) {
        address _claimer = msg.sender;
        Users storage u = User[_claimer];
        uint256[] memory _missedPayments = u.missedRefPayment;
        uint256 _toPay = 0;
        ...
        return (_toPay, lastPaidSlot_);
    }

    function getMissedPayments() public view returns (uint256[] memory) {
        address _claimer = msg.sender;
        Users storage u = User[_claimer];
        uint256[] memory _missedPayments = u.missedRefPayment;
        return(_missedPayments);
    }
    ...
    function getPaidReferralsByRound(address _user, uint _round) public view returns(uint refCount) {
        Users storage u = User[_user];
        return u.roundToPaidRefs[_round];
    }
    ...
    function getUserSlots(address _user) public view returns(uint[] memory slots) {
        Users storage u = User[_user];
        return u.slotIds;
    }
