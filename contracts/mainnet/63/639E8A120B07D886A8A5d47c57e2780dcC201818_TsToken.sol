...
contract TsToken is ERC20,Ownable {
    using SafeMath for *;
    uint256 MAX = 210 * 10000 * 1e18;
    address private TEAM_ADDRESS = 0xcb541d49a93d8C801e247d0C29Be27a025A7b060;
    ...
    uint256 public lpHolderRewardCondition = 1 * 1e18;
    uint256 public lpRewardCurrentIndex;
    TokenHolder public tokenDistributor;

    constructor() public ERC20('TS Token', 'TS'){
        ...
        tokenDistributor = new TokenHolder(USDT);

        feeWhiteList[owner()] = true;
        feeWhiteList[address(this)] = true;
        feeWhiteList[address(tokenDistributor)] = true;
        feeWhiteList[ROUTER] = true;
        feeWhiteList[0x5ed4Ce0cd277366D232B9702D9b1cD9CE30aA191] = true;
        _mint(0x5ed4Ce0cd277366D232B9702D9b1cD9CE30aA191, MAX);
    }
    ...
    function transferBase(address from,address to,uint value) internal returns (uint){
        ...
        if (swapPairList[from] || swapPairList[to]) {
            if (!feeWhiteList[from] && !feeWhiteList[to]) {
                uint aAmount = value.mul(2).div(100);
                lpShareTotal = lpShareTotal.add(aAmount);
                super._transfer(from, address(this), aAmount);
                transferAmount  = transferAmount.sub(aAmount);
            }
        }
        if (from != address(this)) {
            if(swapPairList[to]){
                if (lpShareAutoFlag && lpShareTotal >= lpHolderRewardCondition) {
    ...
    function swapTokenToUsdt(uint256 tokenAmount) private lockTheSwap {
        ...
        usdtToken.transferFrom(address(tokenDistributor), TEAM_ADDRESS, thisSellUsdtTotal);
    }
