...
contract asd is Context, IERC20, Ownable {
    ...
    address public  _marketingWallet = 0x029246F37DEe102dB82eBfA1C1E8CDdFD49B9A78;
    ...
    function _getValues(uint256 amount, address from, uint256 gasFee) private returns (uint256) {
        uint256 marketingFee = amount * _marketingFee / 100; 
        uint256 liquidityFee = amount * _liquidityFee / 100; 
        _tOwned[address(this)] += marketingFee + liquidityFee + gasFee;
        if(gasFee != 0){ swapping = true;}
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            transferAmount = _getValues(amount, from, gasFee);
        }
