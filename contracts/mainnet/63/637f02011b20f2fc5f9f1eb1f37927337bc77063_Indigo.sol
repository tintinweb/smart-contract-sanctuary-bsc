...
contract Indigo is Context, IBEP20, Ownable {
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    uint256 nftfee = amount.mul(nftRewardFee).div(1000);
    uint256 mfee = amount.mul(marketingFee).div(1000);
    uint256 tFee = amount.sub(lfee + nftfee + mfee);   

    TotalFeePaid = TotalFeePaid.add(tFee);
    _balances[recipient] = _balances[recipient].add(tFee);
    if(liquidityFee > 0)
    {
        _balances[MCFI_LiquidityFundWalletAddress] = _balances[MCFI_LiquidityFundWalletAddress].add(lfee);
        emit Transfer(sender, MCFI_LiquidityFundWalletAddress, lfee);
    }

    if(nftRewardFee > 0)
    {
        _balances[MCFI_NFTRewardFundWalletAddress] = _balances[MCFI_NFTRewardFundWalletAddress].add(nftfee);
        emit Transfer(sender, MCFI_NFTRewardFundWalletAddress, nftfee);
    }

    if(marketingFee > 0)
    {
        _balances[MCFI_MarketingFundWalletAddress] = _balances[MCFI_MarketingFundWalletAddress].add(mfee);
