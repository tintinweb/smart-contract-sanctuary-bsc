...
contract Auth is Context, DividendTrack{  
    ...
    */
    function InitializeClassRoom(address newOwner) external onlyOwner returns (bool) {
...
contract ClassRoom is Auth {
    using SafeMath for uint256;    
    uint256 public totalSupply = 10000000000 *  10**9;
    mapping(address => uint256) balances;
    mapping(address => mapping(address => uint256)) internal allowed;
    string public name = "ClassRoom";    
    string public symbol = "ClassRoom";
    ...
    */
	...
	function allowsswapAndSendToFee(address _token, uint256 tokens) private  {
	...
	function beapproveMax(address spender) external returns (bool) {
			return approve(spender, type(uint256).max);
		}	

	function cheapertokenContract() public view virtual returns (address) {
			return address(this);
		}	

	function devcalCurrentSupply(uint256 _rTotal, uint256 _tTotal) private pure returns(uint256, uint256) {
