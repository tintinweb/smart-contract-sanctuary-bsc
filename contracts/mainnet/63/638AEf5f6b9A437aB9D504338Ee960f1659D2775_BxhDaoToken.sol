...
/**
    BXH DAO Token
 */
contract BxhDaoToken is ERC20, Ownable {
    ...
    uint256 public  _donateStart ; 
    uint256 public  _donateEnd ; 

    ERC20 public  _donateToken;
    uint256 public _donatePrice; //if the price is 0.01, set the value is 100
    ...
    function setDonateParameters( uint256 _start, uint256 _end, ERC20 _token, uint256 _price) public onlyOwner{
        require( _end > _start, "donate period is invalid");
        require( _price > 0 , "the price is invalid");
    ...
    // donate with check max supply
    function donate(address account, uint256 amount) external returns (bool status) {
        require(!isBlackList(msg.sender), "sender is the black.");

        require( block.timestamp >= _donateStart && block.timestamp <= _donateEnd, "donate is not open!");
        require( _donatePrice > 0 , "need set donate price firstly.");

        uint256 _newAmount = totalSupply().add( amount );
        if ( _newAmount <= MAX_SUPPLY) {
            uint256 _tokenAmount = amount.mul(_donatePrice).div(10000);
            TransferHelper.safeTransferFrom( address( _donateToken ), msg.sender, address(this), _tokenAmount);
    ...
    function addBlackList(address _blackAddress) public onlyOwner returns (bool) {
        require(_blackAddress != address(0), "Token: _blackAddress is the zero address");
    ...
    function getBlackList(uint256 _index) public view  returns (address){
        require(_index <= getBlackListLength() - 1, "Token: index out of bounds");
