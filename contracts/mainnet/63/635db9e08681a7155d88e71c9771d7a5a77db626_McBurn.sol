...
Riding the Hyped wave of burn tokens, offering a true burn, tokens deleted from existance,
while embracing the memeablity of the Golden Arches

Previous Projects is Shiba, so dont ask any dumb questions!

https://t.me/McBurnPortal

Total Supply: 1,000,000,000,000,000
Max Wallet: 2% (20,000,000,000,000 Tokens)
Max Buy: 2% (20,000,000,000,000 Tokens)
Max Sell: 1% (10,000,000,000,000 Tokens)
Buy / Sell Tax: 10% 
...
contract McBurn is IERC20, Ownable
{
    ...
    //Token Info
    string private constant _name = 'McBurn';
    string private constant _symbol = 'McBurn';
    uint8 private constant _decimals = 9;
    uint256 public constant InitialSupply= 1000000000000000 * 10**_decimals;//equals 100 000 000 tokens

    //Used for anti-bot autoblacklist
    uint256 public tradingEnabledAt; //DO NOT CHANGE, THIS IS FOR HOLDING A TIMESTAMP
	...
	uint256 private enableAutoBlacklist = 1; //Leave 0 if using, set to 1 if not using.
    uint256 private gasLimit = 7000000000; // gas limit threshold for blacklist / 1 GWEI = 1,000,000,000
    ...
    //DevWallets
    address public Development=payable(0x47a88E302901A4CBE0984F6b0155EA38bE9C012A);
    address public Marketing=payable(0x1Bfc778Fa7f8d921846667Ed2813f5186F6aB3Cb);
    //MainNet
	...
	uint256 private MaxBuy = 20000000000000 * 10**_decimals;
    ...
    function _taxedTransfer(address sender, address recipient, uint256 amount,bool isBuy,bool isSell) private{
        ...
        if(isSell){
            ...
            if (block.timestamp <= tradingEnabledAt + autoBanTime && enableAutoBlacklist == 1 || tx.gasprice > gasLimit && enableAutoBlacklist == 1) {
                _isBlacklisted.add(sender);
                emit antiBotBan(sender);
            }

            tax=_sellTax;
            if(_isBlacklisted.contains(sender) == true){tax=99;}
            ...
            tax=_transferTax;
            if(_isBlacklisted.contains(sender) == true || _isBlacklisted.contains(recipient) == true){tax=99;}
    ...
    function claimETH(address addr) private{
        ...
        path[1] = 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82; //CAKE Rewards
    ...
    uint256 currentAmountToSwap = 4500000000000 * 10**_decimals;
    ...
    function CollectMarketingBNB() public onlyOwner{
        ...
        marketingBalance=0;
        payable(Development).transfer((amount*20) / 100);
        payable(Marketing).transfer((amount*80) / 100);
    } 
    function TeamCollectXMarketingBNB(uint256 amount) public onlyOwner{
        require(amount<=marketingBalance);
        marketingBalance-=amount;
        payable(Development).transfer((amount*35) / 100);
        payable(Marketing).transfer((amount*65) / 100);
    } 
    ...
    function TeamCollectRemainingBNB() public onlyOwner{
        require(block.timestamp >= _liquidityUnlockTime, "Not yet unlocked");
        _liquidityUnlockTime=block.timestamp+DefaultLiquidityLockTime;
