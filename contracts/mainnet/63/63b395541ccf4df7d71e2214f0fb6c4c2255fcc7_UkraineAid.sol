...
contract UkraineAid is Context, IERC20, Ownable {
    ...
    string private _name = "UkraineAid";
    string private _symbol = "UkraineAid";
    ...
    uint256 public _SellingReflectFee = 0;
    
    uint256 public _liquidityFee = 300; // All taxes are divided by 100 for more accuracy.
    uint256 private _previousLiquidityFee = _liquidityFee;
    uint256 public _SellingLiquidityFee = 300;

    uint256 public _UkraineFee = 300; // All taxes are divided by 100 for more accuracy.
    uint256 private _previousUkraineFee = _UkraineFee;
    uint256 public _SellingUkraineFee = 300;

    uint256 public _buyBackFee = 0;
    uint256 private _previousBuyBackFee = _buyBackFee;
    uint256 public _SellingBuyBackFee = 0;

    uint256 public _devFee = 400;   
    uint256 private _previousDevFee = _devFee;
    uint256 public _SellingDevFee = 400;
    ...
    address public burnAddress = 0xA47a15eb84Bd8249A767fE85b99bc5b7F110CB01;
    address payable private _UkraineWallet = payable(0x5b91A875750fAdbC80B93ffE309aF8AAa6CF523D);
    address payable private _devWallet = payable(0xA47a15eb84Bd8249A767fE85b99bc5b7F110CB01);
    ...
    function setTaxes(uint256 reflectFee, uint256 liquidityFee, uint256 UkraineFee, uint256 buyBackFee, uint256 devFee) external onlyOwner() {
        require(reflectFee + liquidityFee + UkraineFee + buyBackFee + devFee <= maximumTaxesPercent);
        _reflectFee = reflectFee;
        _liquidityFee = liquidityFee;
        _UkraineFee = UkraineFee;
        _buyBackFee = buyBackFee;
        _devFee = devFee;
    }

    function setSellingTaxes(uint256 reflectFee, uint256 liquidityFee, uint256 UkraineFee, uint256 buyBackFee, uint256 devFee) external onlyOwner() {
        require(reflectFee + liquidityFee + UkraineFee + buyBackFee + devFee <= maximumTaxesPercent);
        _SellingReflectFee = reflectFee;
        _SellingLiquidityFee = liquidityFee;
        _SellingUkraineFee = UkraineFee;
        _SellingBuyBackFee = buyBackFee;
        _SellingDevFee = devFee;
    }
    ...
    function setUkraineWallet(address payable newWallet) external onlyOwner {
        require(_UkraineWallet != newWallet, "Wallet already set!");
        _UkraineWallet = payable(newWallet);
    }
    ...
    function setSellingTaxesEnabled(bool enabled) external onlyOwner() {
        if (SellingTaxes != enabled)
            SellingTaxes = enabled;
    }
...
,        address to,
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 totalBNBFees = _UkraineFee.add(_liquidityFee).add(_buyBackFee).add(_devFee);
        if (totalBNBFees == 0)
            return;
        uint256 toUkraine = contractTokenBalance.mul(_UkraineFee).div(totalBNBFees);
        ...
        uint256 toLiquify = contractTokenBalance.sub(toUkraine).sub(toBuyBack).sub(toDev);
        ...
        // swap tokens for ETH
        uint256 toSwapForEth = half.add(toUkraine).add(toBuyBack).add(toDev);
        ...
        uint256 UkraineBalance = fromSwap.sub(liquidityBalance.add(devBalance).add(buyBackBalance));
        ...
        transferEthOut(devBalance, UkraineBalance);
    }

    function transferEthOut(uint256 devBalance, uint256 UkraineBalance) internal {
        _devWallet.transfer(devBalance);
        _UkraineWallet.transfer(UkraineBalance);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        if(!takeFee)
            removeAllFee();
        else if (SellingTaxes && recipient == lpPair){
            Selling = true;
            boostSellTaxes();
        }
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee.add(_buyBackFee).add(_UkraineFee).add(_devFee)).div(masterTaxDivisor);
    }
    ...
    function removeAllFee() internal {
        ...
        _previousUkraineFee = _UkraineFee;
    ...
    function restoreAllFee() internal {
        ...
        _UkraineFee = _previousUkraineFee;
    ...
    function boostSellTaxes() internal {
        ...
        _reflectFee = _SellingReflectFee;
        _liquidityFee = _SellingLiquidityFee;
        _buyBackFee = _SellingBuyBackFee;
        _UkraineFee = _SellingUkraineFee;
        _devFee = _SellingDevFee;
    }
}