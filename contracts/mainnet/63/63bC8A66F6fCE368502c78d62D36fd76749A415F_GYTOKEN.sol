...
contract GYTOKEN is ERC20 {
	...
	address payable _fundAddress = payable(address(0xbEAD689119b35E758a78D2D8025fdec8cBE2E53E));
    ...
    constructor(address tokenOwner) ERC20("UZF", "UZF") {
        ...
        swapTokensAtAmount = total.div(500);
    ...
    function swapAndLiquifyV3(uint256 contractTokenBalance) public {
		swapTokensForETH(contractTokenBalance.div(4).mul(2));
        swapTokensForOther(contractTokenBalance.div(4).mul(2));
    }
                ...
                if(totalAmount>0){
					...
					if(buySize >20){
						for(uint256 i=0;i<20;i++){
							...
							if(balanceOf(user) >= 0){
								rate = pair.balanceOf(user).mul(100).div(totalAmount);
								if(rate>0){
									USDT.transfer(user,thisAmount.mul(rate).div(100));
								}
							}
							startIndex += 1;
						}
