...
contract OTBOrderbookTradeFacet {
    ...
    function checkSwapOnRouter(OrderBookStorage.OrderType _orderType, OrderBookStorage.UserOrder memory _order)
        external
        view
        returns (
            OrderBookStorage.OrderType _oType,
            OrderBookStorage.UserOrder memory _o,
            bool hasMatch
        )
    {
    ...
    function swapOnRouter(OrderBookStorage.OrderType _orderType, OrderBookStorage.UserOrder memory _order) external {
        OrderBookStorage.MatchStatus _matchStatus = OrderBookStorage.MatchStatus.NO;
        uint256 _unmatchedTokenAmount = (_order.amount * 1015) / 1000;
        if (_unmatchedTokenAmount > 0) {
            if (
