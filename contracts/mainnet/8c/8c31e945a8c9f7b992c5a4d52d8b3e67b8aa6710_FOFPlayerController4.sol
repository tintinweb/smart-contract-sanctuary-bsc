...
contract FOFPlayerController4 is Ownable {

    using SafeMath for uint256;

    event PaymentReceived(address from, uint256 amount);

    event SetNickname(address indexed user, string nickname);

    event SetCardNicknames(address indexed user, uint256 tokenId, string nickname);

    event JoinClub(address indexed user, uint256 clubId);

    event DeleteClubMember(address indexed operator, uint256 clubId, address indexed user);
    ...
    constructor() {
        ...
        FPRD = FOFPlayerRaceData(payable(0xDBa355CCa2Bf9CD1AdB70f63e2634576487D5c33));
    }
    ...
    function setNickname(string memory _nickname) public {
        FPRD.setNickname(msg.sender, _nickname);
        emit SetNickname(msg.sender, _nickname);
    }

    function setCardNicknames(uint256 tokenId, string memory _nickname) public {
        require(FPN.ownerOf(tokenId) == msg.sender, "You are not the owner");

        require(FPN.NFTTypes(tokenId) == 4 || FPN.NFTTypes(tokenId) == 5, "Only petrol stations and clubs can set nicknames");

        FPRD.setCardNicknames(tokenId, _nickname);

        emit SetCardNicknames(msg.sender, tokenId, _nickname);
    }

    function joinClub(uint256 tokenId) public {
        FPRD.joinClub(msg.sender, tokenId);
        emit JoinClub(msg.sender, tokenId);
    }

    function deleteClubMember(address member) public {

        require(FPRD.memberToClubIds(member) != 0, "The user is not in the club");

        uint256[] memory clubIds = FPC2.getUserNFTIDsByType(msg.sender, 4);

        uint256 index = findMemberIndex(clubIds[0], member);

        FPRD.deleteClubMember(clubIds[0], index, member);

        emit DeleteClubMember(msg.sender, clubIds[0], member);
    }
