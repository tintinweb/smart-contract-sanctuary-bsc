...
(UAFA) Token
Copyright UA Faith LLC All Rights Reserved, Never Waived.
...
contract UAFaith is BEP20, LockToken {
    ...
    uint256 public totalFees = BUSDRewardsFee.add(liquidityFee).add(marketingFee).add(charityFee).add(buybackFee);

    address payable public _marketingWalletAddress = payable(0xE8C65AFe72991C49549B58Cdf331d3fEB6086a1D);
    address payable public _charityWalletAddress = payable(0xAC0E5822a917e2C6a388067265274DA9735BF49c);
    address payable public _buybackWalletAddress = payable(0xF0C2e1281812300d9c530fCD03F0032eb462eCC8);
    ...
    function setBUSDRewardsFee(uint256 value) external onlyOwner{
        ...
        totalFees = BUSDRewardsFee.add(liquidityFee).add(marketingFee).add(charityFee).add(buybackFee);
        require(totalFees<=13, "To High Fee");
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal open(from, to) override {
        ...
        !automatedMarketMakerPairs[from] && from != owner()) {
            ...
            // @public marketingFee+charityFee+buybackFee+liquidityFee
    ...
    function swapAndLiquify(uint256 tokens) private {
       uint256 swapableFee = totalFees.sub(BUSDRewardsFee);
        ...
        uint256 bnbForBuyback = newBalance.sub(bnbForLiquidity).sub(bnbForMarketing).sub(bnbForCharity);
    ...
    uint256 public _walletHoldingMaxLimit = _totalSupply.div(100).mul(100);
    uint256 public _amountDueForSaleLimit = _totalSupply.div(1000000); //0.0000001%

    uint256 maxAllowedSalePercentage = 100;
...
contract UAFaithDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor() DividendPayingToken("UAFaith_Dividend_Tracker", "UAFaith_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 100_000_000_000_000 * (10**18); // Must hold 100 Trillions+ Tokens
    } // Currently the BUSD Rewards are turned off, for Dividends and Rewards, there must be tax engaged on 'LINE 730' uint256 public BUSDRewardsFee = 0;
//    // Because 'BUSDRewardsFee = 0' --> then Dividends/Reflections/Rewards are currently turned off. If they are turned on, Holders will earn in BUSD.
