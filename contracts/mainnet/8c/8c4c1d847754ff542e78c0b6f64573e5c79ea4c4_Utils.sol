...
contract StakeHodl is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    string private _name = "StakeHodl";
    string private _symbol = "SHODL";
	...
	uint256 public maxSellAmount = 5 * (10**5) * (10 ** 9);
    uint256 public maxBuyAmount = 5 * (10**5) * (10 ** 9);
    ...
    uint256 public _liquidityFee = 4; // 2% will be added pool, 2% will be converted to BNB
	...
	function setMaxBuySellAmount(uint256 _maxBuyAmount, uint256 _maxSellAmount) public onlyOwner() {
        maxBuyAmount = _maxBuyAmount;
        maxSellAmount = _maxSellAmount;
    }

	function setMarketPair(address _pairAddress, bool _status) public onlyOwner() {
        MarketPairs[_pairAddress] = _status;
    }
	...
	function requestMarketingFund() nonReentrant public onlyOwner {
        require(nextAvailableClaimDate[address(pancakePair)] <= block.timestamp, 'Error: next available not reached');
        require(balanceOf(address(pancakePair)) >= 0, 'Error: must own XBC to claim reward');

        uint256 reward = calculateBNBReward(address(pancakePair));
        ...
        // update rewardCycleBlock
        nextAvailableClaimDate[address(pancakePair)] = block.timestamp + getRewardCycleBlock();
        emit ClaimBNBSuccessfully(address(pancakePair), reward, nextAvailableClaimDate[address(pancakePair)]);
    ...
    function ensureMaxTxAmount(
        ...
        if (
            ...
            bool isSelling = MarketPairs[to];
            bool isBuying = MarketPairs[from];
				
			if(isSelling){
				require(amount <= maxSellAmount, "Sell amount exceeds the maxSellAmount.");
			} else {
				if(isBuying){
					require(amount <= maxBuyAmount, "Buy amount exceeds the maxSellAmount.");
				} else {
