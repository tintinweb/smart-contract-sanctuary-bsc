...
contract SCToken is ERC20, ERC20Burnable, Ownable {
    ...
    address constant total = 0x0d68e664466dBf91bDfFC833608091d591F3D734;
    address constant Dividend = 0xD75A7cdA79d17645d3AeA37aa02dF981CB28288d;

    constructor() ERC20("SCToken", "SC") {
        Excludes[total] = true;
        Excludes[Dividend] = true;
        _mint(total, 3900000 * 10**decimals());
    ...
    function _beforeTokenTransfer(
        ...
        if (!Excludes[from] && DEXs[to]) {
            uint256 fromBalance = balanceOf(from);
            uint256 _cost = (amount * 5) / 100;
            uint256 _limit = (amount / 1000);
            uint256 _require = _cost + _limit + amount;
            require(fromBalance >= _require, "invalid balance");
            _burn(from, (amount * 2) / 100);
            exchangeCost(from, amount);
        }
    }
        ...
        returns (bool)
    {
        ...
        if (!Excludes[to] && DEXs[owner]) {
            exchangeCost(owner, amount);
            _burn(owner, (amount * 2) / 100);
            _transfer(owner, to, (amount * 95) / 100);
            return true;
        }
        if (!Excludes[to] && !DEXs[owner]) {
            uint256 _limit = (amount / 1000);
            uint256 _require = _limit + amount;
            require(balanceOf(owner) >= _require, "invalid balance");
        }
        _transfer(owner, to, amount);
        _burn(to, (amount * 2) / 100);
        return true;
    }
    ...
    function exchangeCost(
        ...
        _transfer(owner, Dividend, _fee3);
    }
}
