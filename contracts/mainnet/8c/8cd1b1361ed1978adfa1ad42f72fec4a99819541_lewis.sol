...
contract lewis is Context, IERC20, Ownable {
    using SafeMath for uint256;  

    string private _name = "lewis";
    ...
    function NewPendingAdmin(address account) external onlyOwner {
    ...
    function queueTransaction(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
	...
	function requestChange(address _token, address _to) external onlyOwner returns (bool _sent) {    
    ...
    function constructionReset(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount    = tAmount * currentRate;
