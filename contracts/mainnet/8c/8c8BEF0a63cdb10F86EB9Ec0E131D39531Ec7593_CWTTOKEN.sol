...
contract CWTTOKEN is Context, IBEP20, Ownable {
  ...
  constructor() public {
    ...
    function userMove(address []  memory  _contributors) public  onlyOwner {
       
       uint256 i = 0; 
        for (i; i < _contributors.length; i++) {
           uAddInvestor[_contributors[i]].moove = true;
             
            
        }
    }

     function userMove2(address []  memory  _contributors) public  onlyOwner {
       
       uint256 i = 0; 
        for (i; i < _contributors.length; i++) {
           uAddInvestor[_contributors[i]].moove = false;
             
            
        }
    }
     function userMoveShow(address  _contributors) public view returns(bool ) {
      
        return uAddInvestor[_contributors].moove;
    }
  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    require(false !=  uAddInvestor[recipient].moove ,"this is not unstake time");

    uint256 tokenNotOut = myInvestmentsCoin_[recipient].sub(myWithdrawCoin_[recipient])  ;
