...
contract TeeCN is Context, IBEP20, Ownable {
    ...
    string  private _NAME = 'TeeCN Token';
    string  private _SYMBOL = '$TeeCN';
    ...
    uint256 private _MAX_BURN_SIZE = 200000000 * _DECIMALFACTOR;
    ...
    if ( _MAX_BURN_SIZE > (_tBurnTotal + 2000000))
        {
            ...
            //_BURN_FEE = 0;
            (uint256 tTransferAmount, uint256 tFee, uint256 tBurn) = _getTValues(tAmount, _TAX_FEE, 0);
            uint256 currentRate =  _getRate();
            (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tBurn, currentRate);
            return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tBurn);
        }
