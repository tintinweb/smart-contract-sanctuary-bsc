...
contract  GGGToken is IERC20, Ownable {
    ...
    uint256 public minLPDividendToken =  1e8;
    ...
    constructor(  string memory  name_,
        ...
        address adminAddress = msg.sender;//持币地址
            _name = name_; //名字
            _symbol = symbol_;//代币简写
            _decimals= decimals_;//代币精度
            _tTotal = totalSupply_* (10**uint256(_decimals)); //数量
            _LPFee = 200;//LP分红手续费，要乘以100 比如1的手续费 就是填100
   ...
   function isExcludedFromFee(address account) public view returns (bool) {
    ...
    function setFee(uint lpFee_  ) public onlyOwner {
        _LPFee =  lpFee_;
    }
