...
contract PLUSPLUSPLUSSwapper is Ownable {
    ...
    function _buyToken(address recipient, uint value, uint minOut) internal {
        ...
        // handle swap logic
        (address input, address output) = (token, phoenix);
        (address token0,) = sortTokens(input, output);
        uint amountInput;
        uint amountOutput;
        { // scope to avoid stack too deep errors
        (uint112 reserve0, uint112 reserve1,) = IPair(pair).getReserves();
        (uint112 reserveInput, uint112 reserveOutput) = input == token0 ? (reserve0, reserve1) : (reserve1, reserve0);
