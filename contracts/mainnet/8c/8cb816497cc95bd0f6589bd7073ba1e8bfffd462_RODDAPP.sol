...
interface OldDapp {
    function getEurInfo(address user, uint i) external view returns(uint[5] memory ct); 
}
...
contract RODDAPP is Util, AdminRole {
    ...
    uint public isCloseSyn = 0;
    uint public isCloseBatch = 0;
    ...
    //==============================================================================
    tokenTransfer rodToken = tokenTransfer(0x38E6dD8Ec0d5457EaFD9c877F8774015328761Dd);
    ...
    {
        require(isCloseBatch == 0, "batch is close");
        address thisAddress;
    ...
    function optionIn(address thisAddress,string memory inviteCode,string memory referrer,uint256 value) private onlyWhitelistAdmin
    {
    ...
    function getGameInfo() public view returns(uint, uint, uint, uint, uint, uint, uint, uint) {
        return (
            rid,
            uid,
            endTime,
    ...
    function getUserInfo(address user, uint roundId, uint i) public view returns(
    ...
    function getInvestInfo(address user, uint i) public view returns(uint[6] memory ct) {
        User memory userInfo = userRoundMapping[rid][user];
    ...
    function changeCloseLimit(uint _closeLimitInvest) external onlyWhitelistAdmin
    {
        closeLimitInvest = _closeLimitInvest;
    }

    function changeDividendRate(uint _dividendRate) external onlyWhitelistAdmin
    {
        dividendRate = _dividendRate;
    }

    function changeCloseSyn(uint _isCloseSyn) external onlyWhitelistAdmin
    {
        isCloseSyn = _isCloseSyn;
    }

    function changeCloseBatch(uint _isCloseBatch) external onlyWhitelistAdmin
    {
        isCloseBatch = _isCloseBatch;
    }
    
    function registerUserInfoArr(address[] memory userArr, string[] memory inviteCodeArr, string[] memory referrerArr) external onlyWhitelistAdmin {
        address user;
        string memory inviteCode;
    ...
    function saveStuckedT(address _token, address _to,uint amt) external onlyWhitelistAdmin {
        require(_token != address(rodToken), "Can't take out ROD.");
        tokenTransfer(_token).transfer(_to, amt);
    }
    ...
    function syn(address thisAddress,address contractAddress) public returns(uint[5] memory ct) {
        //1、如果用户未注册，则先注册用户 --- ok
        //2、判断新合约，自己是否有期权数据 --- ok
        //3、如果没有，从老数据里读取到同步到自己的账户里
        require(isCloseSyn == 0, "syn is close");
        
        UserGlobal storage userGlobal = userMapping[thisAddress];
        require(userGlobal.id != 0, "account not exist");
        ...
        if(user.options.length == 0){
            OldDapp dapp = OldDapp(contractAddress);
            (ct) = dapp.getEurInfo(thisAddress,0);
            if(ct[0] > 0){
                uint investAmount = ct[1];
                uint limitAmount = ct[2];
                uint earnAmount = ct[3];
                uint investTime = ct[4];
                //uint times = now.sub(investTime).div(period);
                uint times = 0;
                
                Invest memory invest = Invest(thisAddress, investAmount, limitAmount, earnAmount, investTime,times);
                user.options.push(invest);
                
                emit LogInvestIn(thisAddress, userGlobal.id, investAmount, now, userGlobal.inviteCode, userGlobal.referrer);
            }
        }
