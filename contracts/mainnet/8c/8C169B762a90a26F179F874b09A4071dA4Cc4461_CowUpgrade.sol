...
contract AdminTransfer is Ownable {
    function adminTransferToken(address token,address to,uint amount) external onlyOwner {
...
// File: my/new_cow/grade/CowUpgrade.sol
...
contract CowUpgrade is EIP712,ReentrancyGuard,AdminTransfer{
    address public signerAddress;

    function cowUpgrade(uint deadline,uint256 cowId,bytes calldata signature) view external {
        require(block.timestamp <= deadline, "expired");
        require(check(signerAddress,hashData(deadline,cowId),signature), "sign error");
    }

    constructor(address _signerAddress) EIP712("cowUpgrade", "1") {
    ...
    function hashData(uint deadline,uint256 cowId) public view returns (bytes32) {
        return _hashTypedDataV4(keccak256(abi.encode(
            keccak256("CowUpgradeRequest(uint256 deadline,uint256 cowId)"),
            deadline,
            cowId
        )));
    }
