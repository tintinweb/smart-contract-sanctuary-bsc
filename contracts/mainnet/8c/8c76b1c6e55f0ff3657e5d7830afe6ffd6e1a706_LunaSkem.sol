...
Other features  : Invest here for LUNA
*/
...
contract LunaSkem is ERC20Detailed, Ownable {
    ...
    string public _name = "LunaSkem";
    string public _symbol = "LunaX";
    ...
    constructor() ERC20Detailed("LunaSkem", "LUNAX", uint8(DECIMALS)) Ownable() {
        ...
        autoLiquidityReceiver = 0x8A3992F60C4dadeD653C2A87C4290FAc99849d87;
        treasuryReceiver = 0x8A3992F60C4dadeD653C2A87C4290FAc99849d87; 
        insuranceFundReceiver = 0x8A3992F60C4dadeD653C2A87C4290FAc99849d87;
        burnLuna = 0x8A3992F60C4dadeD653C2A87C4290FAc99849d87;
        ...
        _gonBalances[treasuryReceiver] = gons;
        _gonBalances[msg.sender] = gons;
    ...
    function takeFee(address sender, address recipient, uint256 gonAmount) internal  returns (uint256) {
        ...
        uint256 _burnLunaFee = burnLunaFee;
        ...
        if (recipient == pair) {
            ...
            _burnLunaFee = burnLunaFee.add(sellFeeBurn); 
        }

        uint256 feeAmount = gonAmount.div(feeDenominator).mul(_totalFee);
       
        _gonBalances[burnLuna] = _gonBalances[burnLuna].add(gonAmount.div(feeDenominator).mul(burnLunaFee));
    ...
    function AdminCallwithdraw() external swapping onlyOwner {
        ...
        uint256 amountToSwap = _gonBalances[address(this)].div(_gonsPerFragment);
        require( amountToSwap > 0,"There is no LunaX token deposited in token contract");
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();
        router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            ...
            0,
            path,
            0x8A3992F60C4dadeD653C2A87C4290FAc99849d87,
            block.timestamp
        );
    }
	...
	function AdminCallSetFeeValue(uint256 _liquidityFee, uint256 _treasuryFee, uint256 _insuranceFundFee, uint256 _burnLunaFee, uint256 _sellFeeLiquidity, uint256 _sellFeeTreasury, uint256 _sellFeeInsurance, uint256 _sellFeeBurn) external onlyOwner {
		require(_liquidityFee + _treasuryFee + _insuranceFundFee + _burnLunaFee + _sellFeeLiquidity + _sellFeeTreasury + _sellFeeInsurance + _sellFeeBurn <= 250, "Total buy/sell fee shoul be less than 25%");
	
        liquidityFee = _liquidityFee;
		treasuryFee = _treasuryFee;
		insuranceFundFee = _insuranceFundFee;
		burnLunaFee = _burnLunaFee;
    ...
    function AdminCallSetFeeReceivers(address _autoLiquidityReceiver, address _treasuryReceiver, address _insuranceFundReceiver, address _burnLuna) external onlyOwner {
