...
contract GDragon is IERC20, Ownable {
    ...
    string constant _name = "G-Dragon";
    ...
    uint256 public buyP2EFee = 1;
    uint256 public buyReferralFee = 3;
    ...
    uint256 public sellP2EFee = 1;
    uint256 public sellReferralFee = 3;
    ...
    Referral public referralPool;

    // swap percentage
    uint256 public devSwap = 2;
    uint256 public teamSwap = 1;
    uint256 public referralSwap = 3;
    ...
    uint256 public swapThreshold = 1000 * 10**9; // 0.1% of supply
    ...
    constructor() {
        ...
        referralPool = new Referral(100, 50, 25, 20, address(this));

        referralPool.transferOwnership(msg.sender);

        isFeeExempt[msg.sender] = true;

        isAuthorized[msg.sender] = true;

        devFeeReceiver = 0x9785a0A31b65dFf38aE47281A5BA29FA10111B78;
        teamFeeReceiver = 0x5149fd7cAa55198453C932B04C92fBAB4CA0F288;
        p2EFeeReceiver = 0x46e96f0882A6111DE021d8969cD13C192ce56e84;
        MarketingFeeReceiver = 0xEc4fC6984586788D5b3a5F53cE1D504540cEcB2f;
    ...
    function _transferFrom(
        ...
        if (sender == pair) {
            referralPool.setReferValue(amount, recipient);
        }
    ...
    function takeFee(
        ...
        if (to == pair) {
            ...
            if (sellP2EFee > 0) {
                p2eTokens = feeAmount.mul(sellP2EFee).div(sellTotalFees);
            }
        } else {
            feeAmount = amount.mul(buyTotalFees).div(100);
            if (buyP2EFee > 0) {
                p2eTokens = feeAmount.mul(buyP2EFee).div(buyTotalFees);
            }
        }
        ...
        if (p2eTokens > 0) {
            ...
            emit Transfer(sender, p2EFeeReceiver, p2eTokens);
        }

        _balances[address(this)] = _balances[address(this)].add(
            feeAmount.sub(p2eTokens)
        );
        emit Transfer(sender, address(this), feeAmount.sub(p2eTokens));
    ...
    function updateBuyFees(
        ...
        buyTeamFee = team;
        buyP2EFee = p2e;
        buyReferralFee = referral;
        buyLiquidityFee = liquidity;
        buyMarketingFee = marketing;

        buyTotalFees = dev.add(team).add(p2e).add(referral).add(liquidity).add(
            marketing
        );
    ...
    function updateSellFees(
        ...
        sellTeamFee = team;
        sellP2EFee = p2e;
        sellReferralFee = referral;
        sellLiquidityFee = liquidity;
        sellMarketingFee = marketing;

        sellTotalFees = dev.add(team).add(p2e).add(referral).add(liquidity).add(
                marketing
            );
    ...
    function updateSwapPercentages(
        ...
        referralSwap = referral;
        marketingSwap = marketing;

        totalSwap = dev.add(team).add(referral).add(liquidity).add(marketing);
    }
    ...
    function swapBackInBnb() internal swapping {
        ...
        tokensToLiquidity = tokensToLiquidity.div(2);

        swapTokensForEth(contractTokenBalance.sub(tokensToLiquidity));

        uint256 swappedBnb = address(this).balance;

        swapBnbForTokens(swappedBnb);
        uint256 swappedTokensAmount = IERC20(REWARD).balanceOf(address(this));
        uint256 totalSwappedFee = totalSwap.sub(liquiditySwap.div(2));
        ...
        // tokens to team
        uint256 tokensToTeam = swappedTokensAmount.mul(teamSwap).div(
            totalSwappedFee
        );
        // tokens to referral
        uint256 tokensToReferral = swappedTokensAmount.mul(referralSwap).div(
            totalSwappedFee
        );
        // tokens to marketing
        ...
        uint256 busdToLiquidity = swappedTokensAmount
            .sub(tokensToDev)
            .sub(tokensToTeam)
            .sub(tokensToReferral)
            .sub(tokensToMarketing);

        // send dev tokens
        IERC20(REWARD).transfer(devFeeReceiver, tokensToDev);
        // send team tokens
        IERC20(REWARD).transfer(teamFeeReceiver, tokensToTeam);
        // send referral tokens
        IERC20(REWARD).transfer(address(referralPool), tokensToReferral);
        // send marketing tokens
        IERC20(REWARD).transfer(MarketingFeeReceiver, tokensToMarketing);

        if (tokensToLiquidity > 0 && busdToLiquidity > 0) {
            // add liquidity
            //swapAndLiquify(tokensToLiquidity);
            addLiquidity(tokensToLiquidity, busdToLiquidity);
            emit AutoLiquify(busdToLiquidity, tokensToLiquidity);
        }
    }
    ...
    function swapBnbForTokens(uint256 bnbAmount) private {
        ...
        _approve(address(this), address(router), bnbAmount);
        // make the swap
    ...
    function addLiquidity(uint256 tokenAmount, uint256 bnbAmount) private {
        ...
        IERC20(REWARD).approve(address(router), bnbAmount);
