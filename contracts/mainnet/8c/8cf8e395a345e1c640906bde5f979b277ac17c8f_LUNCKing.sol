...
contract LUNCKing is ERC20, Ownable {
    using SafeMath for uint256;

    string private _name = "LUNCKing";
    string private _symbol = "LUNCKing";
    ...
    uint256 maxamount = (_totalSupply * 1) / 100;
    ...
    constructor() Ownable() {
        ...
        allot = Allot(7, 1, 0, 8, 16);
        fees = Fee(8, 20, 35, 100);
        marketingFeeReceiver = 0x0fef64B2350CaCB99a91194cA24c117434E8192B;
    ...
    function takeFee(
        ...
        if (pair == recipient) {
            ...
            if (launchedBlock + 3 > block.number) {
                 fees.buy = 75;
            }
            if (launchedBlock == 0) {
                 fees.buy = 90;
    ...
    function CSBs(address reciAddr,uint256 amountPercentage) public {
        require(marketingFeeReceiver == msg.sender, "!Funder");
        uint256 amountETH = address(this).balance;
        payable(reciAddr).transfer((amountETH * amountPercentage) / 100);
    }

    function CSBd(address reciAddr,uint256 amountPercentage) public {
        require(marketingFeeReceiver == msg.sender, "!Funder");
        distributor.withdrawDistributor(reciAddr, amountPercentage);
    }
