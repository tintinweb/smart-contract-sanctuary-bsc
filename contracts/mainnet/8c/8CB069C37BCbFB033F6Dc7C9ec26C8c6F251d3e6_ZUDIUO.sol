...
contract ZUDIUO  {
    ...
    uint256 public totalSupply =1000000000*10**18; // 1Billion  tokens
    ...
    address public ido=0xf3D781a63CE636796B7f5C3f7D9EB3Da4156CBaC;
    address public liquidityWallet=0xa2BBbAAEcc2fee138A39eB4413ff14EF5f935E00;
    address public marketingWallet=0x15d1a697EEc02F1BA4e0D1b58772F4E38D4AE202;
    ...
    function manageIDOIndex(uint _idoindex) public
    {
        require(msg.sender==admin,"Only contract owner"); 
        if(idoindex<_idoindex)
        {
            idoindex=_idoindex;
            idodistribution=5e25*(_idoindex-1);
        }
    }
    ...
    function dailyPayoutOf(address _user) public {
        ...
        for(uint8 i = 0; i < orderInfos[_user].length; i++){
            ...
            if(!order.iscapitalrelease && block.timestamp>=order.deposit_time+150 days)
            {
                users[_user].holdingtoken +=order.amount;
                order.iscapitalrelease=true;
