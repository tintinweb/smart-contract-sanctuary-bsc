...
//  _______ _     _ _______ _______ _______        _______  _____ 
...
contract Chameleo is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private constant _name = "Chameleon Coin";
    string private constant _symbol = "CAMO";
    ...
    uint public maxHoldings = 20; // max holdings per wallet

    address payable private _devwallet = payable(address(0x18db7758C31Ab973E47D042517aC5b5673Fa860c));
    address payable private _buybackWallet = payable(address(0x5dDa37e102d99009Dbb10C1D33D4287B59AB1D50));
    ...
    event UpdatedMaxHoldings(uint maxHoldings);
    ...
    constructor() {
        ...
        multisig = 0xdc580F72A14d93DE6a66e8967f023f6f33ff9eC4;
    ...
    function setSwapTreshold(uint newSwapTresholdPercentage) public onlyMultisig {
        require(newSwapTresholdPercentage <= 10);
        swapTreshold = newSwapTresholdPercentage;
        emit SwapThresholdChange(newSwapTresholdPercentage);
    }
    ...
    function getMaxHoldings() public view returns (uint256) {
        return _tTotal.mul(maxHoldings).div(100);
    }
    ...
    function updateBuybackWallet(address payable newAddress) external onlyMultisig {
        require(newAddress != address(0), "newAddress can not be zero address.");
        _devwallet = newAddress;
        _isExcludedFromFee[newAddress] = true;
        emit DevWalletUpdate(newAddress);
    ...
    function setFees(
        ...
        require(tax + marketing + liquidity + dev + buyback <= 10);

        txFees.tax = tax;
        txFees.marketing = marketing;
        txFees.liquidity = liquidity;
        txFees.dev = dev;
        txFees.buyback = buyback;
    ...
    /// Update Max Holdings
    function updateMaxHoldings(uint newMaxHoldings) external onlyMultisig {
        require(newMaxHoldings > 0, "Max Holdings Parameter should be greater than 0");

        maxHoldings = newMaxHoldings;
        
        emit UpdatedMaxHoldings(newMaxHoldings);
    }
        ...
        )
    {
        ...
        uint256 tWallet = calculateMarketingFee(tAmount) +
            calculateDevFee(tAmount) + calculateBuybackFee(tAmount);
    ...
    function setTxFee() private {
        _taxFee = txFees.tax;
        _liquidityFee = txFees.liquidity;
        _marketingFee = txFees.marketing;
        _devFee = txFees.dev;
        _buyBackFee = txFees.buyback;
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (to != uniswapV2Pair && !_isExcludedFromFee[to]) {
            // Implement max holdings to wallet addresses
            require(balanceOf(to).add(amount) < getMaxHoldings(), "Destination address cant hold more tokens");
        }
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap {
        uint256 denominator = (txFees.liquidity +
            txFees.marketing +
            txFees.dev + txFees.buyback) * 2;

        require(denominator > 0, "denominator must be greater than 0.");
        uint256 tokensToAddLiquidityWith = (tokens *
            txFees.liquidity) / denominator;
        ...
        uint256 marketingAmt = unitBalance * 2 * txFees.marketing;
        uint256 buybackAmt = unitBalance * 2 * txFees.buyback;
        uint256 devAmt = unitBalance * 2 * txFees.dev >
            address(this).balance
            ? address(this).balance
            : unitBalance * 2 * txFees.dev;
        ...
        if (buybackAmt > 0) {
            payable(_buybackWallet).transfer(buybackAmt);
        }
