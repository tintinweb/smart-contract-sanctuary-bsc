...
abstract contract AbsToken is IERC20, Ownable {
  ...
  mapping(address=>uint) public record;
   ...
   mapping(uint=>address[]) private sordRecord;
  
   uint[] public SortNum;
   
   mapping(address=>uint) public InviterNum;
 
   mapping(address=>uint) public ReleasedNum;
  
   mapping(address=>uint) public BuyUsdtNum;

   mapping(address=>uint) public NftNum;
   address[] RecordAddr;

   mapping(address=>address[]) public AddrIndex;
 
   mapping(address=>uint) public OpenNftNum;
    ...
    function setWalletLimit(uint256 _walletLimit) public onlyOwner{
     ...
     function AddIdonum(address addr,uint amount) external onlyOwner{
    //record[addr]=record[addr].add(amount);
    record[addr]=record[addr] + amount;
    // addressIndexes.push(addr);
    // sordRecord[amount].push(addr);
    // SortNum.push(amount);
  }
  function AddOpenNftNum(address addr) external onlyOwner{
      OpenNftNum[addr]=OpenNftNum[addr]+1;
  }
  function SubOpenNftNum(address addr) external onlyOwner{
      OpenNftNum[addr]=OpenNftNum[addr]-1;
  }
  function AddNftNum(address addr) external onlyOwner{
      //NftNum[addr]=NftNum[addr].add(1);
      NftNum[addr]=NftNum[addr]+1;
  }
  function subNftNum(address addr) external onlyOwner{
    //NftNum[addr]=NftNum[addr].sub(1);
    NftNum[addr]=NftNum[addr]-1;
  }
  function removeNftNum(address addr) external onlyOwner{
    NftNum[addr]=0;
  }
  function JudgeRepeat(address addr1,address addr2) view public returns(bool){
        for(uint i=0;i<AddrIndex[addr1].length;i++){
            if(AddrIndex[addr1][i]==addr2){
                return true;
            }
        }
        return false;
    }
    function addInvitNum(address addr1,address addr2) external onlyOwner{
        if(JudgeRepeat(addr1,addr2)==false){
               AddrIndex[addr1].push(addr2);
               //InviterNum[addr1]=InviterNum[addr1].add(1);
               InviterNum[addr1]=InviterNum[addr1]+1;
              // return true;
        }else{
           // return false;
        }
  ...
  function addUsdtNum(address addr,uint amount) public onlyOwner{
      //BuyUsdtNum[addr]=BuyUsdtNum[addr].add(amount);
        BuyUsdtNum[addr]=BuyUsdtNum[addr] + amount;
      // if(sordRecord[amount]!=addr){
      //       sordRecord[amount].push(addr);
      // }
     // SortNum.push(amount);
...
function ForAdd() public view returns(uint[] memory){
  uint[] memory tempAmount;
  for(uint i=0;i<RecordAddr.length;i++){
     tempAmount[i]=BuyUsdtNum[RecordAddr[i]];
  }
  return tempAmount;
}

  function removeUserUsdt(address addr) public onlyOwner{
      BuyUsdtNum[addr]=0;
  }

  function showRecord(uint amount) view public returns(address[] memory){
    return sordRecord[amount];
  }
  // function addInvitNum(address addr) public onlyOwner{
  //   InviterNum[addr]=InviterNum[addr].add(1);
  // }
  function sortAddress(uint[] memory SortAmount) public view returns(address[] memory){

 //  address[] memory SortAddr;
   address[] memory SortAddr = new address[](30);
   uint k=0;
    for(uint i=0;i<SortAmount.length;i++){
        for(uint j=0;j<sordRecord[SortAmount[i]].length;j++){
          // SortAddr.push(sordRecord[SortNum[i]][j]);  
          SortAddr[k++]=sordRecord[SortAmount[i]][j]; 
        }
    }
    return SortAddr;
  }

  function showSortAmount() public view returns(uint[] memory){
    return SortNum;
  }
...
contract ZJL is AbsToken {
    constructor() AbsToken(
        ...
        6,//Decimals
        1000000,//Supply
        address(0xB0214a57CCe1eA32DaD564b695C1eC6136420567),//FundAddress
        address(0xB0214a57CCe1eA32DaD564b695C1eC6136420567)//ReceiveAddress
    ){
