...
contract GIACS is IERC20,Ownable {
    ...
    function getpice () public  view  returns (uint256) {
        ...
        path[0] = usdt;
        path[1] = address(this);
        uint256 [] memory pice = IUniswapV2Pair(router).getAmountsOut(1*10**18,path);
        return pice[1];
    }
        ...
        if(_isuniswapV2Pair[sender] || _isuniswapV2Pair[recipient] ){
            if(_isuniswapV2Pair[sender]){
                ...
                if(_iscompany[recipient]){
                    require(_sOwned[recipient].div(64).mul(getpice()).div(1*10**17) >= tAmount);
                    _recordlogs(sender,recipient,tAmount);
                    _sOwned[recipient] = _sOwned[recipient].sub(tAmount.div(getpice()).mul(64*10**17));
