...
interface StableSwap {
    function exchange(uint256 i, uint256 j, uint256 dx, uint256 min_dy, address caller, address receiver) external returns(uint256 stablesReceived);
}
...
contract PlanetRouter {
    ...
    uint public swapFeeFactor = 9900;
    uint public constant swapFeeFactorMax = 9990;
    ...
    function swapExactTokensForTokensThenStableSwap(
        ...
        IERC20(path[lastIndex]).approve(stableSwapPool, tokensReceived); 
        stablesReceived = StableSwap(stableSwapPool).exchange(stableSwapPoolData[0], stableSwapPoolData[1], tokensReceived, stableSwapPoolData[2], address(this), to);
    ...
    function swapTokensForExactTokensThenStableSwap(
        ...
        IERC20(path[lastIndex]).approve(stableSwapPool, tokensReceived);
        stableReceived = StableSwap(stableSwapPool).exchange(stableSwapPoolData[0], stableSwapPoolData[1], tokensReceived, stableSwapPoolData[2], address(this), to);
    ...
    function updateSwapFeeFactor(uint _swapFeeFactor) external {
        ...
        require(swapFeeFactorMax >= _swapFeeFactor);
