...
import {IFireCatNFTBoxMiddleware} from "../src/interfaces/IFireCatNFTBoxMiddleware.sol";
...
contract FireCatNFTStake is IFireCatNFTStake, ModifyControl, FireCatTransfer {
    ...
    function stake(uint256 tokenId_) external beforeStake nonReentrant returns (bool) {
        ...
        fireCatNFT.approve(boxMiddleware, tokenId_);
        IFireCatNFTBoxMiddleware(boxMiddleware).stake(tokenId_);
    ...
    function claim() external beforeClaim nonReentrant returns (uint256) {
        ...
        if (fireCatNFT.ownerOf(tokenId) != address(this)) {
            IFireCatNFTBoxMiddleware(boxMiddleware).claim(tokenId);
        } 
    ...
    /**
    * @notice set FireCatNFTBoxMiddleware.
    * @param boxMiddleware_ address
    */
    function setFireCatNFTBoxMiddleware(address boxMiddleware_) external onlyRole(DATA_ADMIN) {
        boxMiddleware = boxMiddleware_;
    }

    /**
    * @notice migrate NFT to FireCatNFTBoxMiddleware contract.
    * @param numLimit uint256
    */
    function migrate(uint256 numLimit) external nonReentrant onlyRole(DATA_ADMIN) {
        uint256[] memory tokenIdList = fireCatNFT.tokenIdOf(address(this));
        uint256 lastTokenIndex = tokenIdList.length - 1;
        for (uint256 i = 0; i < tokenIdList.length; i++) {
            if (i + 1 > numLimit) {
                break;
            }

            uint256 index = lastTokenIndex - i;
            uint256 tokenId = tokenIdList[index];
            fireCatNFT.approve(boxMiddleware, tokenId);
            IFireCatNFTBoxMiddleware(boxMiddleware).stake(tokenId);
        }
    }
}
...
/**
* @notice IFireCatNFTBoxMiddleware
*/
interface IFireCatNFTBoxMiddleware {
    function totalBoxes() external view returns (uint256);
    function boxOf(uint256 index_) external view returns (address);
    function sweep(uint256 tokenId_) external;
    function claimTokens(address token, address to, uint256 amount) external ;
    function setProxyAdmin(address proxyAdmin_) external ;
    function createBox(address safeAdmin_, uint256 boxLimit_) external returns (address);
    function getUsingBox() external view returns (address);
