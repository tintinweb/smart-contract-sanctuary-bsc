...
// Heroic Ape // $HRAPE
// 
// Heroic Ape is an innovative defletionary token aiming to revolutionize Defi
// building its own Heroic Ape Blockchain, NFT assets for Metaverse,
// introducing a multi-chain swap (HrapeSwap) across Binance, Ethereum, Cronos and Polygon.
//
// 🦍 Telegram: https://t.me/heroicape
// 🦍 Website: https://heroicape.com
// 🦍 Twitter: https://twitter.com/heroicape
//
//
...
contract HeroicApeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("HeroicApe_Dividend_Tracker", "HeroicApe_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "HeroicApe_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "HeroicApe_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main HeroicApe contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "HeroicApe_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "HeroicApe_Dividend_Tracker: Cannot update claimWait to same value");
...
contract HeroicApe is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    HeroicApeDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Heroic Ape", "HRAPE") {
        BNBRewardsFee = 4;
        liquidityFee = 3;
        extraFeeOnSell = 12; // extra fee on sell
        marketingFee = 4;
        marketingWallet = payable(0x978445FE2beae99Fc8aFe2D0e9e65Ae3C36e4fD1);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new HeroicApeDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "HeroicApe: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "HeroicApe: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "HeroicApe: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "HeroicApe: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "HeroicApe: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "HeroicApe: Cannot update gasForProcessing to same value");
