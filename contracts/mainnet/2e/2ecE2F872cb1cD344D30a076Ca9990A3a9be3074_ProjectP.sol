...
/*
    Welcome to the Future of Defi, crosschain, secure, tools for all to use.
	
    Project Perseverance: Innovation comes through those who never surrender.

    Links:
    https://perseverancedefi.com/
    https://docs.perseverancedefi.com/
    https://t.me/PerseveranceBSC
    https://t.me/PREVAnnouncement

    All presales will be held thru the $Prev Presale platform.
    This code and all future released SC's are property of the Project Perseverance team.
 */
...
contract ProjectP is IBEP20, Auth {
    using SafeMath for uint256;

    address targettedReward = 0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c;
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    IBEP20 toSendReward = IBEP20(targettedReward);

    string constant _name = "Project Perseverance";
    string constant _symbol = "$PREV";
    uint8 constant _decimals = 18;
    uint256 _totalSupply = 10000000000 * 10**18; // 100,000,000,000
    uint256 public _maxTxAmount = 1200000000 * 10**18; // 1.2%
    ...
    address public stakingReflecitonReceiver;
    ...
    constructor (address stakingAddress) Auth(msg.sender) {
        // PancakeSwap
        ...
        stakingReflecitonReceiver = stakingAddress;
    ...
    function getTotalFee(bool selling) public view returns (uint256) {
        if(launchedAt + 3 >= block.number){ 
            ...
            // Reduced buy taxes for new buyers
                if(selling){
    ...
    function takeFee(address sender, address receiver, uint256 amount) internal returns (uint256) {
        uint256 feeAmount = amount.mul(getTotalFee(receiver == pair)).div(feeDenominator).sub(burnFee);
        uint256 burnAmount = amount.mul(getTotalFee(receiver == pair)).div(feeDenominator).sub(liquidityFee).sub(reflectionFee);
        uint256 totalToGive = amount.mul(getTotalFee(receiver == pair)).div(feeDenominator);
        _balances[0x000000000000000000000000000000000000dEaD] = _balances[address(this)].add(burnAmount);
    ...
    function swapBack() internal swapping {
        if(liquidityFeeAccumulator >= swapThreshold && autoLiquifyEnabled){
            ...
            payable(stakingReflecitonReceiver).call{value: balanceBefore, gas: 30000}("");
    ...
    function setReward(address newReward) external authorized {
        targettedReward = newReward;
    }
    ...
    function updateStakingAddress(address _new) external authorized{
        stakingReflecitonReceiver = _new;
    }
    ...
    function setFees(
        ...
        totalFee = reflectionFee.add(burnFee);
    ...
    function getFees() external view returns (bool,uint256,uint256,uint256) {
        return (
            feeEnabled,
            reflectionFee,
            liquidityFee,
