...
contract EvoTionToken is Context, IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string private _name = "EvoTion";
    string private _symbol = "EvoTion";
    ...
    function getEvoTionFees() public view returns (uint256) {
	...
	function BurnEvoTionFee(uint256 _amount, uint256 _burnFee) private pure returns (uint256) {
		return _amount.mul(_burnFee).div(
			10**2
		);
	}

	function AutoEvoTionDevFee(uint256 _amount, uint256 _devFee) private pure returns (uint256) {
		return _amount.mul(_devFee).div(
			10**2
		);
	}
	
	function procesEvoTionIndex(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
