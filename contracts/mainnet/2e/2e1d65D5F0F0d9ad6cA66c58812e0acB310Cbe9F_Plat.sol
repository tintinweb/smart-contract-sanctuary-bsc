...
contract Plat {
	...
	uint256 public COMPOUND_BONUS_MAX_TIMES = 12; /** 10 times / 5 days. **/
    uint256 public COMPOUND_STEP = 12 * 60 * 60; /** every 12 hours. **/

    uint256 public WITHDRAWAL_TAX = 800; //800 = 80%, 600 = 60%, 400 = 40%
    uint256 public COMPOUND_FOR_NO_TAX_WITHDRAWAL = 12; // compound for no tax withdrawal.
    ...
    uint256 goldvalue = 200000 * 10**18;
    uint256 platinumvalue = 400000 * 10**18;
    uint256 platinumhonorvalue = 800000 * 10**18;
    uint256 diamondvalue = 1300000 * 10**18;
    uint256 diamondhonorvalue = 1700000 * 10**18;

    uint256 goldbonus = 2;
    uint256 platinumbonus = 4;
    uint256 platinumhonorbonus = 8;
    uint256 diamondbonus = 13;
    uint256 diamondhonorbonus = 17;

    IBEP20 platinum = IBEP20(0x0D714A328CEF6B403A752625692fF965Bb948baA);
    ...
    constructor(address _bbadr, address _teamadr, address _mktadr, address _autoadr) {
		require(!isContract(_bbadr) && !isContract(_teamadr) && !isContract(_mktadr) && !isContract(_autoadr));
        owner = msg.sender;
        bbadr = _bbadr;
        teamadr = _teamadr;
        mktadr = _mktadr;
    ...
    function payFees(uint256 platinValue) internal returns(uint256){
        ...
        token_BUSD.transfer(teamadr, platinValue.mul(DEVTAX).div(PERCENTS_DIVIDER));
