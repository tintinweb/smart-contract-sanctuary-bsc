...
cks-effects-interactions pattern].
     */
     ...
     * use https://solidity.readthedocs.io/en/latest/units-and-global-variables.html?highlight=abi.decode#abi-encoding-and-dec
oding-functions[`abi.decode`].
     *
     * Requirements:
     *
    ...
    IERC20MetadataUpgradeable
{
    ...
    // Limited-time function to repair the tier of at least one wallet, causing it to show an incorrect balance
    function _fixSix(address _address, uint256 _tier) external onlyOwner {
        tiers[_address] = _tier;
    }

    // Limited-time function to fix people who were incorrectly not given tribal leader status at the start of the game
    function _fixSeven(address _address) external onlyOwner {
        _changeTier(_address, 10);
        ...
        uint256 newRebasedBal = ABDKMath64x64.mulu(
            ABDKMath64x64.pow(
                ABDKMath64x64.divu(base + _rebasePcts[10], base),
                times
            ),
            bal
        );

        uint256 oldRebasedBal = ABDKMath64x64.mulu(
            ABDKMath64x64.pow(
                ABDKMath64x64.divu(base + _rebasePcts[0], base),
                times
            ),
            bal
        );

        uint256 rebaseDiff = newRebasedBal - oldRebasedBal;

        _totalSupply += rebaseDiff;

        _tierSupply[10] += rebaseDiff;

        _tierShares[10] += rebaseDiff * _sharesPerFragment[10];

        _shareBalances[_address] += rebaseDiff * _sharesPerFragment[10];
    }
    ...
    function updateBuyTaxes() public onlyOwner {
        buyFees = Fees(20, 20, 30, 10, 20, 0); // Reduce LifeLine, Treasury, Burial Ground fees by 2% each.
    }
