...
//       uint32[] memory tempArray = [uint32(367),368,369,370];
...
//*************** USER FUNCTIONS ******************************* */
    ...
    function buyProduct(uint8 _productID, uint32 _itemsAmount) public whenNotPaused(){
        ...
        require(_itemsAmount >= products[_productID].minBuyAmount[0], "Not a valid buy amount");
...
//*************** ADMIN FUNCTIONS ******************************* */
   ...
   function getUserDataByID_2(uint256 _userTokenID) external view returns(uint128, uint128, uint128, uint128){
    ...
    function getProductInfo_2(uint8 _productID) external view returns(uint32, uint32, uint32[3] memory){
        require(_productID < products.length, "Product ID is out of bounds");
    ...
    function getPrice(uint8 _productID, uint32 _itemsAmount, uint256 _userTokenID) public view returns(uint128){
        return products[_productID].price[0];
   ...
   function GetMinBuyAmount(uint8 _productID, uint256 _userTokenID) public view returns(uint128){
        ...
        return products[_productID].minBuyAmount[0];
   }
...
//        uint32[] memory _tempArray;
//        for(uint i=0; i < users[_userTokenID].productIntervalIDs[0].length; i++){
...
/*    function addItems(uint256 _userTokenID, uint8 _productIndex, uint32 _itemsAmount) private{
        ...
        while (i < _itemsAmount){
            users[_userTokenID].productIDs[_productIndex].push(products[_productIndex].lastItemID);
            products[_productIndex].lastItemID++;
            i++;
        }
    }
*/
