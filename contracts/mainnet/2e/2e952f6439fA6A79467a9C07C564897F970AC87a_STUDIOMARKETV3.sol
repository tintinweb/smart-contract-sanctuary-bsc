...
contract STUDIOMARKETV3 is ReentrancyGuard, ERC721URIStorage, Ownable {
  ...
  bool public _takeFees = true;

  bool public _tokenActive = true;
    ...
    if (_royaltyAddress.length == 0) {
      ...
      for (uint256 i = 0; i < _royaltyAddress.length; i++) {
        if (_royaltyFee[i] > 0) {
          ...
          IERC20(tokenContract).transferFrom(msg.sender, _royaltyAddress[i], royalPercent);
        }
      }
  ...
  //set if to allow token to be used for token mint and sales operation
  function setTokenSaleStatus(bool _status) public {
    _tokenActive = _status;
  }

  //set if to take platform fees when txs is used for mint and sales operations
  function setTokenFeeStatus(bool _fees) public {
    _takeFees = _fees;
  }
  ...
  /// @notice Get the creator of a token in the contract
  /// @dev returns slice of data from the MarketItem type
  /// @return string

  function getCreator(uint256 marketItemId) public view returns (address) {
    return idToMarketItem[marketItemId].creator;
  }

  /// @notice Get the current owner of a token in the contract
  /// @dev returns slice of data (owner key) from the MarketItem type
  /// @return string

  function getOwner(uint256 marketItemId) public view returns (address) {
