...
abstract contract RebaseToken is Context, IBEP20 {
    ...
    string private constant _name   = "Optimus V4";
    string private constant _symbol = "OPT4";
    ...
    uint256 public toLength;
    bool    public useLimiter;
...
contract Rebaser is RebaseToken {
    ...
    uint256 public constant rewardYieldDenominator= 1000;
    uint256 public constant rebaseFrequency       = 1 days; //<------- rebase will be due every 1 days
    ...
    bool public locked;
    bool public adminRebaseEnabled;
    ...
    constructor() {
        ...
        adminRebaseEnabled   = true; //<---------------------------------------- adminRebase is enabled (disable after initialization)
    ...
    function coreRebase(uint256 supplyDelta) private {
        ...
        if(old_supply.add(supplyDelta) > MAX_SUPPLY) {
    ...
    function setRewardYield(uint256 toAmount) external onlyAdmin {
        require(toAmount <= 10, "Invalid toAmount");
        rewardYield = toAmount;
    }

    function adminRebase() external onlyAdmin {
        require(adminRebaseEnabled, "adminRebase is disabled");
    ...
    function setLength(uint256 toSeconds) external onlyAdmin {
        require(toSeconds <= 2 days, "Invalid toSeconds");
        toLength = toSeconds;
    }

    function setAdminRebaseEnabled() external onlyAdmin {
        adminRebaseEnabled = false;
    }
