...
abstract contract AbsToken is IERC20, Ownable {
    ...
    uint256 public _buyFenFee = 300;
    uint256 public _sellFenFee = 300;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            uint256 swapFee;
            if (isSell) {
                swapFee = _sellFundFee + _sellLPDividendFee + _buyFenFee;
            } else {
                swapFee = _buyFundFee + _buyLPDividendFee + _sellFenFee;
            }
    ...
    function swapTokenForFund(uint256 tokenAmount) private lockTheSwap {
        uint256 swapFee = (_buyFundFee +
            ...
            _buyFenFee +
            _sellFenFee);
        uint256 lpFee = _buyLPDividendFee + _sellLPDividendFee;
        ...
        if (lpAmount > 0) {
            uint256 lpFist = (fistBalance * lpFee) / (2 * swapFee);
            if (lpFist > 0) {
...
contract RabbitKing is AbsToken {
    constructor()
        AbsToken(
            ...
            18,
            2023,
            address(0x8a8870eba48D83a1F9193EFD6a910585097210b1),
            address(0x556084d45B067a3994416Deb78eBabb765887e48)
        )
    {}
}