...
contract Treasury is ContractGuard {
    ...
    function setAirdropSwapper(address _airdropSwapper) external onlyOperator {
        require(_airdropSwapper != address(0), "address 0");
        airdropSwapper = _airdropSwapper;
        setAddrexcludedTotalSupply(address(airdropSwapper));
    }

    function mintToAirdropSwapper(uint256 _amount) external onlyOperator {
        require(_amount >= 0, "Amount equal or smaller to 0");
        IBasisAsset(tomb).mint(airdropSwapper, _amount);
    }
