...
contract SwitchCelerBSC is SwitchBSC {
        ...
        returns (bytes32 transferId)
    {
        ...
        _emitCrossChainSwapRequest(transferArgs, transferId, returnAmount, msg.sender, SwapStatus.Succeeded);
    }
        ...
        returns (IMessageReceiverApp.ExecutionStatus)
    {
        ...
        if (m.bridgeToken == m.dstToken) {
            ...
            _emitCrosschainSwapDone(m, _amount, _amount, SwapStatus.Succeeded);
        } else {
            ...
            if (parts == 0) {
                if (IERC20(m.bridgeToken).isETH()) {
                    ...
                    _emitCrosschainSwapDone(m, _amount, 0, SwapStatus.Failed);
                } else {
                    IERC20(m.bridgeToken).universalTransfer(m.recipient, _amount);
                    _emitCrosschainSwapDone(m, _amount, 0, SwapStatus.Failed);
                }
        ...
        returns (IMessageReceiverApp.ExecutionStatus)
    {
        ...
        _emitCrosschainSwapDone(m, _amount, 0, SwapStatus.Fallback);
    ...
    {
        returnAmount = _swapInternal(distribution, amount, parts, lastNonZeroIndex, IERC20(m.bridgeToken), IERC20(m.dstToken));
        if (returnAmount > 0) {
    ...
    function _emitCrossChainSwapRequest(TransferArgsCeler calldata transferArgs, bytes32 transferId, uint256 returnAmount, address sender, SwapStatus status) internal {
        emit CrosschainSwapRequest(
            transferArgs.id,
            transferId,
            transferArgs.bridge,
    ...
    function _emitCrosschainSwapDone(CelerSwapRequest memory m, uint256 srcAmount, uint256 dstAmount, SwapStatus status) internal {
        emit CrosschainSwapDone(m.id, m.bridge, m.recipient, m.bridgeToken, m.dstToken, srcAmount, dstAmount, status);
    }
}
