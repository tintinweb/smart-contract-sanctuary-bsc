...
// File contracts/Scholarshipcoin.sol
...
contract Scholarshipcoin is IERC20, Ownable, Pausable {
    ...
    string constant public name = "Scholarshipcoin";
    string constant public symbol = "SCHO";
    uint8 constant public decimals = 18;

    uint256 private _totalSupply = 20E6 * 1E18;
    ...
    event SetGameContractAddress(address gameContractAddress);
    event TransferReward(address indexed from, address indexed to, uint256 value);
    event RewardApproval(address indexed owner, address indexed spender, uint256 value);
    ...
    function setFeePercentage(uint8 feePercentage_) external onlyOwner {
        require(feePercentage_ <= 10, "Scholarshipcoin: transaction fee percentage exceeds 10");
        require(feePercentage_ >= 0, "Scholarshipcoin: transaction fee percentage equals 0");
    ...
    function blacklist(address[] calldata addresses, bool isBlacklist) external {
        require(hasBlacklistPermission[msg.sender], "Scholarshipcoin: insufficient permission");
    ...
    function _transfer(
        ...
        require(!isBlacklisted[sender], "Scholarshipcoin: transfer from blacklisted address");
        require(!isBlacklisted[recipient], "Scholarshipcoin: transfer to blacklisted address");
        require(!isBlacklisted[tx.origin], "Scholarshipcoin: transfer called from blacklisted address");
    ...
    function setGameContractAddress(address gameContractAddress_) external onlyOwner {
        gameContractAddress = gameContractAddress_;
        emit SetGameContractAddress(gameContractAddress);
    }

    function rewards(address owner, address spender) external view returns (uint256) {
        return _rewards[owner][spender];
    }

    function approveReward(address spender,uint256 amount) public returns (bool)
    {
        require(_msgSender() == gameContractAddress, "You have no permission");
        require(spender != address(0x0));

        _rewards[_msgSender()][spender] = amount;

        emit RewardApproval(_msgSender(), spender, amount);
    ...
    function transferReward(address sender, address recipient, uint256 amount) public returns (bool)
    {
        ...
        require(amount <= _rewards[_msgSender()][recipient], "there's no allowed balance");
        ...
        emit TransferReward(sender, recipient, amount);
