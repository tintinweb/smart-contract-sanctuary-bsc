...
contract GETH is ERC20, Ownable {
    ...
    GETHDividendTracker public dividendTracker;
    ...
    uint256 public _BuyBackGETHFee = 1;
  
    uint256 public _BuyTotalFees = _BuyETHRewardsFee.add(_BuyLiquidityFee).add(_BuyMarketingFee).add(_BuyBackGETHFee);
    
    uint256 public _SellETHRewardsFee = 8;
    uint256 public _SellLiquidityFee = 2;
    uint256 public _SellMarketingFee = 2;
    uint256 public _SellBackGETHFee = 2;
    uint256 public _SellTotalFees = _SellETHRewardsFee.add(_SellLiquidityFee).add(_SellMarketingFee).add(_SellBackGETHFee);
    
    uint256 public maxWalletToken = 2 * (10**9) * (10**18); //2%

    address public _marketingWalletAddress = 0x8bed1303D63622d5Abc3Ae826A78dA397c33211D;
    address public _GETHWalletAddress = 0x21e2ABFf8418E8F7213Cc8340AB5BFD9dB705787;
    ...
    constructor() ERC20("ETH GETH", "$ETHGETH") {

    	dividendTracker = new GETHDividendTracker();
        ...
        //dividendTracker.excludeFromDividends(_GETHWalletAddress);
        ...
        excludeOrIncludeFromFees(_GETHWalletAddress, true);
        ...
        excludeOrIncludeFromMaxTx(_GETHWalletAddress, true);
        ...
        excludeOrIncludeFromMaxWallet(_GETHWalletAddress, true);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "GETH: The dividend tracker already has that address");

        GETHDividendTracker newDividendTracker = GETHDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "GETH: The new dividend tracker must be owned by the GETH token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "GETH: The router already has that address");
    ...
    function excludeOrIncludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "GETH: Account is already the value of 'excluded'");
    ...
    {
        require(_isExcludedFromMaxTx[account] != excluded, "GETH: Account is already the value of 'excluded'");
    ...
    {
        require(_isExcludedFromMaxWallet[account] != excluded, "GETH: Account is already the value of 'excluded'");
    ...
    function setGETHWallet(address payable wallet) external onlyOwner{
         require(wallet != address(0), "Invalid address");
        _GETHWalletAddress = wallet;
    }

      function setBuyFees(uint256 buyEthRewardFee, uint256 buyLiquidityFee, uint256 buyMarketingFee, uint256 buyBackGETHFee) external onlyOwner
    {
        _BuyETHRewardsFee = buyEthRewardFee;
        _BuyLiquidityFee = buyLiquidityFee;
        _BuyMarketingFee = buyMarketingFee;
        _BuyBackGETHFee = buyBackGETHFee;
        _BuyTotalFees = _BuyETHRewardsFee.add(_BuyLiquidityFee).add(_BuyMarketingFee).add(_BuyBackGETHFee);

    }

    function setSellFees(uint256 sellEthRewardFee, uint256 sellLiquidityFee, uint256 sellMarketingFee, uint256 sellBackGETHFee) external onlyOwner
    {
        _SellETHRewardsFee = sellEthRewardFee;
        _SellLiquidityFee = sellLiquidityFee;
        _SellMarketingFee = sellMarketingFee;
        _SellBackGETHFee = sellBackGETHFee;
        _SellTotalFees = _SellETHRewardsFee.add(_SellLiquidityFee).add(_SellMarketingFee).add(_SellBackGETHFee);

    }
      function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "GETH: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 800000, "GETH: gasForProcessing must be between 200,000 and 800,000");
        require(newValue != gasForProcessing, "GETH: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        
        {
            ...
            uint256 walletTokens = contractTokenBalance.mul(_SellMarketingFee.add(_SellBackGETHFee)).div(_SellTotalFees);
            uint256 contractBalance = address(this).balance;
            swapTokensForEth(walletTokens);
            uint256 newBalance = address(this).balance.sub(contractBalance);
            uint256 marketingShare = newBalance.mul(_SellMarketingFee).div(_SellBackGETHFee.add(_SellMarketingFee));
            uint256 developmentShare = newBalance.sub(marketingShare);
            payable(_marketingWalletAddress).transfer(marketingShare);
            payable(_GETHWalletAddress).transfer(developmentShare);
...
contract GETHDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("GETH_Dividend_Tracker", "GETH_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "GETH_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "GETH_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main GETH contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "GETH_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "GETH_Dividend_Tracker: Cannot update claimWait to same value");
