...
contract YF3 {
	...
	uint256 public minTime = 1 minutes;
	uint256 public addTime = 30 seconds;
	uint public maxAddTime = 45 minutes;
	uint public startTime;

    uint calTime = 1 minutes;
    ...
    function drawCoinFunction(uint amount) public {
        ...
        if (nowDay.sub(lastDay) >= 102){
            IERC20 newCoin = IERC20(contract1);
            assert(newCoin.transferFrom(msg.sender, address(0), feeAmount) == true);
	...
	function joinHelp(uint256 amount, bool doType) public {
        ...
        if (!startGiveCoin && nowDay.sub(lastDay) >= 100){
			startGiveCoin = true;
		}
    ...
    function getStartTime() public view returns (uint, uint){
        return (startTime, backStatic);
    }
    ...
    function _calLevelNow(address _myAddress) private view returns(uint256 nowLevel) {
        ...
        if(myDoing >= 4000e18){
            if(minScore >= 5000 && user.teamNum >= 10){
                nowLevel = 5;
            }else if(minScore >= 20000 && user.teamNum >= 5){
                nowLevel = 4;
            }else{
                nowLevel = 3;
            }
        } else if(myDoing >= 2000e18){
            if(minScore >= 20000 && user.teamNum >= 5){
                nowLevel = 4;
            }else{
                nowLevel = 3;
            }
	...
	function giveNewCoin(address yourAddress, uint _amount) private{
        ...
        uint backPer = minStartBack.add(nowDay.sub(lastDay).sub(100));
        if (backPer >= 10000){
            backPer = 10000;
        }
        uint giveAmount = _amount.mul(backPer).div(10000);
