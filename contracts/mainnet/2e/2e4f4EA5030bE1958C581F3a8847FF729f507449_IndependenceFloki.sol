...
contract IndependenceFloki is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "Independence Floki";
    string private _symbol = "4THOFJULY";
    ...
    uint256 public _buyBackSplit = 3;
    uint256 public _marketingSplit = 5;
    ...
    uint256 private minTokensBeforeSwap = _tTotal  / 1250; // .08% of supply
    ...
    function setFeeReceivers (address _marketing, address _liquidity, address _buyBack) external onlyOwner{
    ...
    function calculateTaxFee(uint256 _amount, bool isSell) private view returns (uint256) {
        if (isSell) {
            return _amount.mul(_taxFee).mul(totalSellFee).div(totalBuyFee).div(
                10**2
                );
    ...
    function calculateLiquidityFee(uint256 _amount, bool isSell) private view returns (uint256) {
        if (isSell) {
            return _amount.mul(_liquidityFee).mul(totalSellFee).div(totalBuyFee).div(
                10**2
                );
    ...
    function _transfer(
        ...
        if (!_isMaxWalExempt[from] && to != address(this)  && to != address(DEAD_ADDRESS) && to != uniswapV2Pair && to != marketingFeeReceiver && to != autoLiquidityReceiver){
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 amountBNBBuyBack = newBalance.mul(_buyBackSplit).div(totalBNBFee);
        
        (bool success, /* bytes memory data */) = payable(buyBackFeeReceiver).call{value: amountBNBBuyBack, gas: 30000}("");
        require(success, "receiver rejected ETH Transfer");
        (bool success2, /* bytes memory data */) = payable(marketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
