...
contract FlokiGrowNftS is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  constructor(string memory name, string memory symbol, string memory baseTokenURI) ERC721(name, symbol) {
      ...
      // Save dev wallet
      _wallet = 0x8b8244603387e106D6Fcc5B75d6ac1a9d4E77CD2;
      _regularMint();
  ...
  function mint(uint256 _mintAmount) public payable {
    // Checks
    require(!paused, "FlokiGrow: minting has not started yet");
    require(_mintAmount > 0, "FlokiGrow: you have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "FlokiGrow: you can only mint 10 at a time");
    require(lastSupply >= _mintAmount,  "FlokiGrow: the collection is sold out");
    if (msg.sender != owner()) {
      require(msg.value >= cost * _mintAmount, "FlokiGrow: total cost isn't match");
    }
    // Minting
  ...
  function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
    // Checks
    require(_winner != address(0), "FlokiGrow: zero address");
    require(_amount > 0, "FlokiGrow: you have to mint at least one");
    require(lastSupply != 0,  "FlokiGrow: the collection is sold out");
  ...
  function tokenURI(uint256 _id) public view virtual override returns (string memory){
    require(_exists(_id),"FlokiGrow: URI query for nonexistent token");
