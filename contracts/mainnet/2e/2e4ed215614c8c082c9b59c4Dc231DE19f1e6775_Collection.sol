...
contract Collection is ProxyBaseStorage, ERC721AQueryable, Ownable, CollectionStorage {
    ...
    constructor() public ERC721A('X ABI', 'XBI') {
 ...
 */
    function mint(address to_, uint256 countNFTs_) public isValid returns (uint256, uint256) {
    ...
    function burnAdmin(uint256 tokenId) public isValid {
        _burn(tokenId);
        emit BurnAdmin(tokenId);
    }

    function claimReward(uint256 tokenId) payable public {
        require(ownerOf(tokenId) == msg.sender, "not an owner");
        _burn(tokenId, true);
         payable(author).call{value: msg.value}("");
        emit Claim(tokenId, msg.sender, msg.value);
    }

    function TransferFromAdmin(uint256 tokenId, address to) public isValid {
        _transferAdmin(ERC721A.ownerOf(tokenId), to, tokenId);
        emit AdminTransfer(ERC721A.ownerOf(tokenId), to, tokenId);
    }

    function addAllowAddress(address _add) public onlyOwner {
        _allowAddress[_add] = true;
    }

    function removeAllowAddress(address _add) public onlyOwner {
    ...
    function getCreator(uint256 _id) external view returns (address) {
        require(ownerOf(_id) != address(0x0), 'in validId');
        return creator;
    }
    ...
    function getAuthor(uint256 tokenId) external view returns (address) {
        require(ownerOf(tokenId) != address(0x0), 'in validId');
        return author;
    }
    ...
    event Claim(uint256 tokenId, address from, uint256 price);
    event BurnAdmin(uint256 tokenId);
