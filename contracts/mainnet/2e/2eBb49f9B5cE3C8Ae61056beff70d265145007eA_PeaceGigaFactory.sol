...
contract PeaceGigaFactory is Ownable,ERC1155Receiver {
    ...
    function setNFTCardInfo (uint _id, string memory _symbol, string memory _uri, uint256 _usdt, uint256 _roi, uint256 _sup) external onlyOwner {
    ...
    function addNFTCardInfo (string memory _symbol, string memory _uri, uint256 _usdt, uint256 _roi, uint256 _sup) external onlyOwner {
        _allCardInfos.push (NFTCardInfo({symbol: _symbol, imgUri: _uri, priceUSDT: _usdt, nftROI: _roi, supply: _sup, soldCount: 0, state: false}));
    ...
    function getRewardInfoByNFT(uint256 _nftID) view private returns(uint256){
        ...
        uint256 wholeReturnDays = 10000 / nftROI;
        if (block.timestamp - nftCreatedTime <= ONE_DAY_TIME * wholeReturnDays) {
            rewardAmount += (block.timestamp - nftClaimedTime) * nftPrice * nftROI / 10000 / ONE_DAY_TIME;
        }
        else if (block.timestamp - nftCreatedTime <= ONE_DAY_TIME * 365) {
            if (nftCreatedTime + ONE_DAY_TIME * wholeReturnDays >= nftClaimedTime) {
                rewardAmount += (nftCreatedTime + ONE_DAY_TIME * wholeReturnDays - nftClaimedTime) * nftPrice * nftROI / 10000 / ONE_DAY_TIME;
            }
            rewardAmount += (block.timestamp - (nftCreatedTime + ONE_DAY_TIME * wholeReturnDays)) * nftPrice * royaltyInfo.OneYearROI / 10000 / ONE_DAY_TIME;
        }
        else if (block.timestamp - nftCreatedTime <= ONE_DAY_TIME * 365 * 2) {
            if (nftCreatedTime + ONE_DAY_TIME * wholeReturnDays >= nftClaimedTime) {
                rewardAmount += (nftCreatedTime + ONE_DAY_TIME * wholeReturnDays - nftClaimedTime) * nftPrice * nftROI / 10000 / ONE_DAY_TIME;
    ...
    function createSaleReal(uint256 _tokenID, uint _price) external onlyNFTOwner(_tokenID) {
        ...
        _allSaleInfo.push (SaleInfo(_tokenID, _uriFromId[_tokenID], _NFTRewardCardInfos[_tokenID].createdTime, mkNFT.creators(_tokenID), msg.sender, _price, getCurrentROIofNFT(_tokenID)));
    ...
    function setRoyalty(uint256 _ra, uint256 _ta, uint256 _tp, uint256 _tda, uint256 _oyr, uint256 _tyr) external onlyOwner {
