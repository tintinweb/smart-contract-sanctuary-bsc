...
contract MAGLC is Ownable {
    ...
    address public aToken = 0x6560D4336Cc0A180Ab4EF2aebB16C78bCC66d675;
    ...
    function getUamounts(uint _amount, address[] memory _path) internal view  returns(uint) {
        uint[] memory Uamounts = PancakeLibrary.getAmountsIn(FACTORY, _amount, _path);
        uint UamountOutMin = Uamounts[0];
        return UamountOutMin;
    }


    function getUburnAmounts(uint _amount, address[] memory _path) internal view  returns(uint) {
        uint[] memory Uamounts = PancakeLibrary.getAmountsIn(FACTORY, _amount, _path);
        uint UamountOutMin = Uamounts[0];
        return UamountOutMin;
    }
    ...
    function withdrawReward(uint _withdrawRewardId, address _to, uint _amount,uint _burnAmount, uint8 _v, bytes32 _r, bytes32 _s) external {
        ...
        bytes32 msgHash = keccak256(abi.encodePacked(_withdrawRewardId, _to, _amount,_burnAmount));
        ...
        uint UamountOutMin = getUamounts(_amount,_path);
        IPancakeRouter02(ROUTER).swapETHForExactTokens{value: UamountOutMin}(_amount, _path, _to, block.timestamp);
        
        address[] memory _burnPath = new address[](3);
        _burnPath[0] = WETH_ADDRESS;
        _burnPath[1] = USDT_ADDRESS;
        _burnPath[2] = aToken;
        uint UburnAmountMin = getUamounts(_burnAmount,_burnPath);
        IPancakeRouter02(ROUTER).swapExactETHForTokens{value: UburnAmountMin}(0, _burnPath, BURN_ADDRESS, block.timestamp);
