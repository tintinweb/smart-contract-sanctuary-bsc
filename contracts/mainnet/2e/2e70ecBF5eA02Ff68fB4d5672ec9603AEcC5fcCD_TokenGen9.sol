...
import {ERC20_Swap2_Own_Liq_TwoWal} from './ERC20_Swap2_Own_Liq_TwoWal.sol';
import './libraries/Ownable.sol';
import './interfaces/ITokenFactory.sol';

contract TokenGen9 is Ownable {
    ...
    function createToken(
        ...
        payable(TOKEN_FACTORY.getTokenFeeAddress()).transfer(MINT_FEE);
         ...
         ERC20_Swap2_Own_Liq_TwoWal newtok = new ERC20_Swap2_Own_Liq_TwoWal(name, symbol, decimals, totalSupply, owner, raddr, lbfee, lsfee, w1addr, w1bfee, w1sfee, w2addr, w2bfee, w2sfee);
...
contract ERC20_Swap2_Own_Liq_TwoWal is IERC20, Ownable {
    ...
    function setBuyFees(uint256 _liquifyFee, uint256 _wallet1Fee, uint256 _wallet2Fee) external onlyOwner {
    ...
    function setSellFees(uint256 _liquifyFee, uint256 _wallet1Fee, uint256 _wallet2Fee) external onlyOwner {
    ...
    function _getValues(uint256 tAmount) private view returns (FeeValues memory) {
        ...
        values.transferAmount = tAmount.sub(values.liquify).sub(values.wallet1).sub(values.wallet2);
        return values;
    }
    ...
    function getTotalFee(address account) public view returns (uint256) {
        ...
        return _defaultFees.liquifyFee.add(_defaultFees.wallet1Fee).add(_defaultFees.wallet2Fee);
        }
    }
