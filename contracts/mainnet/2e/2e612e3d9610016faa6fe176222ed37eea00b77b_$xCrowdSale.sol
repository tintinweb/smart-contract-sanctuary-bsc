...
contract $xCrowdSale is Ownable, Pausable {
    ...
    IBEP20 public tokenAddress = IBEP20(0x04f675Da2f295A9C62c5D53eaDa50EEF69637CBF);
	IBEP20 public acceptedCurrency = IBEP20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
	...
	function convertAmountToTokens(uint256 amount) public view returns (uint256) {
	    uint256 tokens = amount * 10**tokenAddress.decimals() / pricePerToken;
	    return tokens;
	}
	...
	function migrateTokens(address receiver, address token, uint256 tokenAmount) external onlyOwner{
       IBEP20(token).safeTransfer(receiver, tokenAmount);
       emit MigrateTokens(token, tokenAmount);
    }
