...
contract csawp{
    ...
    function swapETHForTokensWithLimit( address[] memory path,uint256 limit1,uint256 limit2,uint256 outPercent) payable public {
        ...
        require(token1.balanceOf(pair) * (10 ** token1.decimals()) > limit1 ,'limit1 is not' );  // number
        require(token2.balanceOf(pair) * (10 ** token2.decimals()) > token2.totalSupply()  * limit2 / 100 ,'limit2 is not' );  // percent
