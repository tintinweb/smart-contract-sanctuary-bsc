...
abstract contract AbsToken is IERC20, Ownable {
    ...
    constructor (
        ...
        address FundAddress,address TaxAddress, address ReceiveAddress,
        uint256 LimitAmount
    ){
        _name = Name;
        _symbol = Symbol;
        ...
        _invitorCondition = 500 * tokenUnit;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if (_swapPairList[sender]) {//Buy
                ...
                isSell = true;
                swapFeeAmount = tAmount * (_sellFundFee + _sellLPDividendFee + _sellTaxFee) / 10000;
            }
            if (swapFeeAmount > 0) {
    ...
    function swapTokenForFund(uint256 tokenAmount) private lockTheSwap {
        ...
        uint256 taxFee = _sellTaxFee;
        uint256 lpDividendFee = _sellLPDividendFee;
        uint256 totalFee = fundFee + lpDividendFee + taxFee;

        uint256 fundUsdt = usdtBalance * fundFee / totalFee;
        uint256 taxUsdt = usdtBalance * taxFee / totalFee;
        if (fundUsdt > 0) {
            USDT.transfer(fundAddress, fundUsdt);
        }
        if (taxUsdt > 0) {
            USDT.transfer(taxAddress, taxUsdt);
        }
    }
    ...
    function setBuyFee(
        uint256 fundFee, uint256 inviteFee
    ) external onlyOwner {
        _buyFundFee = fundFee;
    ...
    function setSellFee(
        uint256 lpDividendFee, uint256 fundFee, uint256 taxFee
    ) external onlyOwner {
...
contract MDE is AbsToken {
    ...
    //USDT
        ...
        16396263,
    //Fund
        address(0x4A46D6204DBF65f1AC2Ec06135164f5C731cDE32),
    //Tax_distribute
        address(0xc41E2A9a00Df18964dC2A1f14399aE011E9fD3f2),
    //Received
        address(0x56d7159305a062D6279eF13457Ad0Ed29cEA006b),
    //LimitAmount
        16396263
    ){
