...
contract Token is Context, IERC20, IERC20Metadata, Ownable {
    ...
    address public pair;
    //nft合约地址
    address public _nftAddress = 0xB7f1884C748C715942404B7374eea7C1D0Ca67a5;
    //旧币合约地址
    IERC20 public OldTokens = IERC20(0xfd8A5Bf1C296c9aeD39cCAfE14c0472e8A143814);
    ...
    function _transfer(address sender,address recipient,uint256 amount) internal virtual {
        ...
        if(pair != address(0)){
            ...
            if(sender == pair){
                ...
                _balances[recipient] += x * (100 - saleNftFee - saleHoldFee - _tarFee1 - _tarFee2 - _tarFee3);
                emit Transfer(sender, recipient, x * (100 - saleNftFee - saleHoldFee - _tarFee1 - _tarFee2 - _tarFee3));
            }else{
                require(amount <= balanceOf(sender) * 9 / 10, "amount too much");
                if(amount >= 1 * 10 ** 18){
                ...
                _balances[recipient] += x * (100 - transHoldFee - transTarFee1 - transTarFee2);
                emit Transfer(sender, recipient, x * (100 - transHoldFee - transTarFee1 - transTarFee2));
            }
        }else{
            _balances[sender] -= amount;
            _balances[recipient] += amount;
    ...
    //设置24小时变化量
    function setChangeTotal(uint256 _target) public onlyOwner {
        _changeTotal = _target;
    }
    ...
    //设置动态费率
    function setRate(uint256 _target) public onlyOwner {
        _rate = _target;
    }
    ...
    //设置旧币地址
    function setOldToken(address _pair) public onlyOwner {
        OldTokens = IERC20(_pair);
    }
    ...
    //旧币兑换新币 需要此合约预存新币，旧币需要授权
    function swapOldToken(uint256 _amount) external {
        require(_amount > 0, "amount error");
        require(balanceOf(address(this)) >= _amount, "balance error");
        OldTokens.transferFrom(msg.sender, address(this), _amount);
