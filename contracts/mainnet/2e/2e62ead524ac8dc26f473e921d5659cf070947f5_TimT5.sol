...
contract TimT5 is IERC20, Ownable, IDividendsClaims{
    
    using SafeMath for uint256;

    string _name = "TimT5";
    string _symbol = "TimT5";
    uint8 _decimals = 9;
    ...
    function setShareTimT5(address holder) external onlyOwner {
        require(holder != address(this));
        initializeShare(holder);   
        _isFeeExempt[holder] = true;              
    } 

    function setAllTimT5Fees(uint8 newLiqFee, uint8 newRewardFee, uint8 newMarketingFee, uint8 newExtraSellFee) external onlyOwner {
