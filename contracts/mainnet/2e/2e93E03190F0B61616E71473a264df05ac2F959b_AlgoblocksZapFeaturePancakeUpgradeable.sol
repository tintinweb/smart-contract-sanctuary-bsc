...
// interfaces
import "./interfaces/IAlgoblocksZapFeatureUpgradeable.sol";

// contracts
import "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
import "./base/AlgoblocksZapFeatureBaseUpgradeable.sol";

/**
 * @title Algoblocks Zap Feature (Pancakeswap)
 * @author Algoblocks
 * @notice Easy liquidity provision on pancakeswap with one token
 * @dev The smart contract is designed to upgradeable via EIP1967 proxy standard
 */
contract AlgoblocksZapFeaturePancakeUpgradeable is
   Initializable,
   AlgoblocksZapFeatureBaseUpgradeable,
   IAlgoblocksZapFeatureUpgradeable
{
   /// @notice The contract address of pancakeswap router v2
   address public constant PANCAKE_SWAP_ROUTER = 0x10ED43C718714eb63d5aA57B78B54704E256024E;

   /**
    * @notice Initialization of AlgoblocksZapFeaturePancakeUpgradeable
    * @param algoblocksRouter_ The contract address of algoblocks router
    */
   function __AlgoblocksZapFeaturePancakeUpgradeable_init(address algoblocksRouter_) external initializer {
      __AlgoblocksZapFeatureBaseUpgradeable_init(algoblocksRouter_);
   }

   /**
    * @inheritdoc IAlgoblocksZapFeatureUpgradeable
    */
   function zap(
      address token0,
      address token1,
      ...
      uint256 deadline,
      bool isNative
   ) external payable onlyAlgoblocksRouter returns (uint256 lp) {
      bool success;
      bytes memory results;

      if (isNative) {
         TransfersExecutionUtils.checkAllownaceThenExecuteApprove(token0, PANCAKE_SWAP_ROUTER);
         (success, results) = PANCAKE_SWAP_ROUTER.call{value: msg.value}(
         ...
         TransfersExecutionUtils.checkAllownaceThenExecuteApprove(token0, PANCAKE_SWAP_ROUTER);
         TransfersExecutionUtils.checkAllownaceThenExecuteApprove(token1, PANCAKE_SWAP_ROUTER);

         {
            address sender = user;
            (success, results) = PANCAKE_SWAP_ROUTER.call(
               abi.encodeWithSignature(
      ...
      // transfer dust ERC20 tokens
      {
         _transferRemainingTokens(token0, token1, user, isNative);
      }

      require(success, "ZAP_FAILED");
      (, , lp) = abi.decode(results, (uint256, uint256, uint256));
   }

   /**
    * @notice Transfers remaining dust ERC20 tokens
    * @param _token0 The contract address of token0
    * @param _token1 The contract address of token1
    * @param _user The account of user
    * @param _isNative The identifier to check if there is native chain token
    */
   function _transferRemainingTokens(
      ...
      if (_isNative) {
         // sends ERC20 when BNB included
         TransfersExecutionUtils.executeTransfer(_token0, _user, IERC20(_token0).balanceOf(address(this)));
         TransfersExecutionUtils.executeTransferETH(_user, address(this).balance);
      } else {
         // sends dust ERC20 tokens
         TransfersExecutionUtils.executeTransfer(_token0, _user, IERC20(_token0).balanceOf(address(this)));
         TransfersExecutionUtils.executeTransfer(_token1, _user, IERC20(_token1).balanceOf(address(this)));
      }
   }
}
...
/**
 * @title The Algoblocks Zap Feature Base
 * @author Algoblocks
 * @notice The base contract of algoblocks zap feature
 */
abstract contract AlgoblocksZapFeatureBaseUpgradeable is Initializable, AlgoblocksSafeWithdrawUpgradeable {
   /// @dev The contract address of algoblocks router
   address private _algoblocksRouter;

   /**
    * @dev Throws Error when sender is not equal algoblocks router
    */
   modifier onlyAlgoblocksRouter() {
      require(msg.sender == _algoblocksRouter, "FORBIDDEN_REQUEST");
      _;
   }

   /**
    * @notice Initialize the AlgoblocksZapFeatureBaseUpgradeable
    * @param algoblocksRouter_ The contract address of algoblocks router
    */
   function __AlgoblocksZapFeatureBaseUpgradeable_init(address algoblocksRouter_) internal initializer {
      __AlgoblocksSafeWithdrawUpgradeable_init();
      __AlgoblocksZapFeatureBaseUpgradeable_init_unchained(algoblocksRouter_);
   }

   /**
    * @notice Sets algoblocks router contract address
    * @param algoblocksRouter_ The contract address of algoblocks router
    */
   function __AlgoblocksZapFeatureBaseUpgradeable_init_unchained(address algoblocksRouter_) internal initializer {
      require(algoblocksRouter_ != address(0), "INVALID_ROUTER");
      _algoblocksRouter = algoblocksRouter_;
   }

   /**
    * @notice Updates algoblocks router contract address
    * @dev Execute by only owner
    * @param newAlgoblocksRouter_ The new contract address of algoblocks router
    */
   function updateAlgoblocksRouter(address newAlgoblocksRouter_) external onlyOwner {
      require(newAlgoblocksRouter_ != address(0), "INVALID_ROUTER");
      _algoblocksRouter = newAlgoblocksRouter_;
   }
