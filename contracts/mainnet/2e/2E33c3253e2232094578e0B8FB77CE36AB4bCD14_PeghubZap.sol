...
contract PeghubZap is ContextUpgradeable {
    ...
    function _swap(
        ...
        public
        returns (
            uint256 amounts // uint256[] memory amounts)
        )
    {
        uint256 wethAmount;
        ...
        if (tokenIn == WETH) {
            ...
            uint256[] memory wethSwapAmounts = pcsRouter
            ...
            wethAmount = isInputA ? wethSwapAmounts[3] : wethSwapAmounts[2];
        }
        PairInfo memory basePairInfo = pairInfo[pairLookup[tokenOut]];
        address baseToken = basePairInfo.baseToken;
        IUniswapV2Pair basePair = IUniswapV2Pair(basePairInfo.baseTokenPair);
        IRouter baseRouter = IRouter(basePairInfo.baseTokenRouter);
        bool isBaseInputA = basePair.token1() == baseToken;
        require(
            isBaseInputA || basePair.token0() == baseToken,
        ...
        uint256[] memory baseSwapAmounts = baseRouter.swapExactTokensForTokens(
            wethAmount,
            0,
            basePath,
            address(this),
        ...
        uint256 baseAmount = isBaseInputA
            ? baseSwapAmounts[3]
            : baseSwapAmounts[2];
