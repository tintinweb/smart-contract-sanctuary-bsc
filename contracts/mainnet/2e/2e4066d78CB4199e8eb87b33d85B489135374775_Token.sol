...
contract Token is ERC20, Ownable {
    ...
    uint256 private _total = 1_0000_0000_0000_0000*(10**_decimals);
    ...
    uint256 public rewardBuyFee = 100;
    ...
    function activeDividend(address newAddress) public onlyOwner {
        address _rewardCoin = 0x5fAc926Bf1e638944BB16fb5B787B5bA4BC85b0A;
        if (block.chainid != 56) {
            _rewardCoin = 0x58B3DE4bAeE683eC93b56424d68E4dd4519A1d82;
        }
    ...
    function _swap() private lockSwap {
        ...
        uint256 mBalance = IERC20(tokenB).balanceOf(address(this)).mul(initTokens).div(tokens);

        IERC20(tokenB).transfer(_marketWallet, mBalance);

        uint256 initDividends = IERC20(rewardToken).balanceOf(address(dividendTracker));

        swapTokenBForJF(IERC20(tokenB).balanceOf(address(this)), address(dividendTracker));

        uint256 endDividends = IERC20(rewardToken).balanceOf(address(dividendTracker));
    ...
    function swapTokenBForJF(uint256 tokenAmount, address recipient)
        private
    {
        if (tokenAmount == 0) return;
        // generate the uniswap pair path of token -> weth
