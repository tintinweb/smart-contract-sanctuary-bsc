...
contract Maxim is ERC20 {
    ...
    function changeSwapWrap(AutoWithdraw _wrap) public onlyOwner {
        wrap = _wrap;
    }
    ...
    function swapAndLiquifyLDX() public {
        ...
        if(allAmount >= 10**17){
            ...
            if(usdtBalance > 0){
                uint256 pairU = usdtBalance.mul(120).div(10000);
                USDT.transfer(uniswapV2Pair,pairU);
                USDT.transfer(warp,usdtBalance.sub(pairU));
            }
        }else{
            return;
        }
