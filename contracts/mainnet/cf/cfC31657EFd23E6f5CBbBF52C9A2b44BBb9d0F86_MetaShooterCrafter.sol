...
contract MetaShooterCrafter is Ownable {
    using Strings for uint256;

    string[][] public recipeItemURIs;
    uint64[][] public recipeItemQuantities;
    uint32[] public recipeOutputItemID;
    ...
    function craft(uint256 recipeId, uint256[] calldata tokenIds) public {
        require(recipeId < recipeItemURIs.length, "MetaShooterCrafter: unknown recipe");
        require(0 < recipeItemURIs[recipeId].length, "MetaShooterCrafter: empty recipe");

        string[] memory tokenURIs = new string[](tokenIds.length);
        for (uint i = 0; i < tokenIds.length; i++) {
            tokenURIs[i] = MetaShooterNFT(nftAddress).tokenURI(tokenIds[i]);
        }

        uint64 suppliedComponentAmount = 0;

        for (uint x = 0; x < recipeItemURIs[recipeId].length; x++) {
            string memory itemURI = recipeItemURIs[recipeId][x];
            uint64 suppliedAmount = 0;

            for (uint y = 0; y < tokenIds.length; y++) {
                if (keccak256(abi.encodePacked((tokenURIs[y]))) == keccak256(abi.encodePacked((itemURI)))){
                    suppliedAmount += 1;
                }
            }

            if (suppliedAmount == recipeItemQuantities[recipeId][x]) {
                suppliedComponentAmount += 1;
            }
        }

        if (suppliedComponentAmount == recipeItemURIs[recipeId].length){
            for (uint i = 0; i < tokenIds.length; i++) {
                 MetaShooterNFT(nftAddress).safeTransferFrom(msg.sender, address(this.owner()), tokenIds[i]);
            }

            MetaShooterNFTMinter(minterAddress).mintNFT(msg.sender, recipeOutputItemID[recipeId]);
        } else {
            revert("MetaShooterCrafter: wrong token amounts");
        }
    }

    function addRecipe(string[] memory itemURIs, uint64[] calldata itemQuantities, uint32 outputItemId) public onlyOwner {
        require(itemURIs.length == itemQuantities.length, "MetaShooterCrafter: wrong recipe length");

        for (uint i = 0; i < itemQuantities.length; i++) {
            require(itemQuantities[i] > 0, "MetaShooterCrafter: zero item quantity");
        }

        recipeItemURIs.push(itemURIs);
        recipeItemQuantities.push(itemQuantities);
        recipeOutputItemID.push(outputItemId);
    }

    function modifyRecipe(uint256 recipeId, string[] memory itemURIs, uint64[] calldata itemQuantities, uint32 outputItemID) public onlyOwner {
        require(itemURIs.length == itemQuantities.length, "MetaShooterCrafter: wrong recipe length");
        require(recipeId < recipeItemURIs.length, "MetaShooterCrafter: unknown recipe");
        ...
        recipeItemURIs[recipeId] = itemURIs;
        recipeItemQuantities[recipeId] = itemQuantities;
        recipeOutputItemID[recipeId] = outputItemID;
    }
