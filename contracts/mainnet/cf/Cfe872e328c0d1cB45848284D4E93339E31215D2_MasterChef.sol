...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (_amount > 0) {
            ...
            uint256 balanceNow = pool.lpToken.balanceOf(address(this));
            realAmountDeposited = balanceNow.sub(balanceBefore);
            if (pool.depositFeeBP > 0) {
                uint256 depositFee = realAmountDeposited.mul(pool.depositFeeBP).div(10000);
                pool.lpToken.safeTransfer(feeAddress, depositFee);
                user.amount = user.amount.add(realAmountDeposited).sub(depositFee);
                pool.amount = pool.amount.add(realAmountDeposited).sub(depositFee);
            } else {
                user.amount = user.amount.add(realAmountDeposited);
                pool.amount = pool.amount.add(realAmountDeposited);
            }
        }
        user.rewardDebt = user.amount.mul(pool.accXhashPerShare).div(1e12);
        emit Deposit(msg.sender, _pid, realAmountDeposited);
    }
