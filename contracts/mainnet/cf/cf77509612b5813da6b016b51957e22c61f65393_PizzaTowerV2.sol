...
contract PizzaTowerV2 {
    ...
    function getUpgradePrice(uint256 floorId, uint256 chefId) internal pure returns (uint256) {
        if (chefId == 1) return [1, 1, 1, 1, 1, 1, 1, 1][floorId];
        if (chefId == 2) return [1, 1, 1, 1, 1, 1, 1, 1][floorId];
        if (chefId == 3) return [1, 1, 1, 1, 1, 1, 1, 1][floorId];
        if (chefId == 4) return [1, 1, 1, 1, 1, 1, 1, 1][floorId];
        if (chefId == 5) return [1, 1, 1, 1, 1, 1, 1, 1][floorId];
        revert("Incorrect chefId");
    }

    function getYield(uint256 floorId, uint256 chefId) internal pure returns (uint256) {
        if (chefId == 1) return [1000000, 170, 526, 1615, 4965, 15100, 48000, 137000][floorId];
