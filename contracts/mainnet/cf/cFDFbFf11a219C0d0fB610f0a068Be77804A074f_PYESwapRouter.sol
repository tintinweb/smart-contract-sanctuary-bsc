...
library PYESwapLibrary {
    ...
    address constant factory = 0x3d79819A68Fc4AA4b73D61D2Ea6422c9b18309CF;
    ...
    // given an output amount of an asset and pair reserves, returns a required input amount of the other asset
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut, bool tokenFee, uint totalFee) internal pure returns (uint, uint) {
        require(amountOut > 0, 'PYESwapLibrary: INSUFFICIENT_OUTPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'PYESwapLibrary: INSUFFICIENT_LIQUIDITY');
        uint amountOutMultiplier = tokenFee ? 10000 - totalFee : 10000;
        uint amountOutWithFee = (amountOut / amountOutMultiplier) * 10000;
        ...
        return (amountIn, amountOutWithFee);
    }
    ...
    function getAmountsIn(uint amountOut, address[] memory path, uint totalFee) internal view returns (uint[] memory amounts) {
        ...
        for (uint i = path.length - 1; i > 0; i--) {
            ...
            (amounts[i - 1], amounts[i]) = getAmountIn(amounts[i], reserveIn, reserveOut, baseToken != address(0), totalFee);
        }
    }
