//  .----------------.  .----------------.  .----------------.  .----------------.  .----------------.  .----------------.  .----------------.  .----------------.  .----------------.  .-----------------. .----------------. 
// | .--------------. || .--------------. || .--------------. || .--------------. || .--------------. || .--------------. || .--------------. || .--------------. || .--------------. || .--------------. || .--------------. |
// | |     ______   | || |   _____      | || |  _________   | || | ____   ____  | || |  _________   | || |  _______     | || |      __      | || |    ______    | || |  _________   | || | ____  _____  | || |  _________   | |
// | |   .' ___  |  | || |  |_   _|     | || | |_   ___  |  | || ||_  _| |_  _| | || | |_   ___  |  | || | |_   __ \    | || |     /  \     | || |  .' ___  |   | || | |_   ___  |  | || ||_   \|_   _| | || | |  _   _  |  | |
// | |  / .'   \_|  | || |    | |       | || |   | |_  \_|  | || |  \ \   / /   | || |   | |_  \_|  | || |   | |__) |   | || |    / /\ \    | || | / .'   \_|   | || |   | |_  \_|  | || |  |   \ | |   | || | |_/ | | \_|  | |
// | |  | |         | || |    | |   _   | || |   |  _|  _   | || |   \ \ / /    | || |   |  _|  _   | || |   |  __ /    | || |   / ____ \   | || | | |    ____  | || |   |  _|  _   | || |  | |\ \| |   | || |     | |      | |
// | |  \ `.___.'\  | || |   _| |__/ |  | || |  _| |___/ |  | || |    \ ' /     | || |  _| |___/ |  | || |  _| |  \ \_  | || | _/ /    \ \_ | || | \ `.___]  _| | || |  _| |___/ |  | || | _| |_\   |_  | || |    _| |_     | |
// | |   `._____.'  | || |  |________|  | || | |_________|  | || |     \_/      | || | |_________|  | || | |____| |___| | || ||____|  |____|| || |  `._____.'   | || | |_________|  | || ||_____|\____| | || |   |_____|    | |
// | |              | || |              | || |              | || |              | || |              | || |              | || |              | || |              | || |              | || |              | || |              | |
// | '--------------' || '--------------' || '--------------' || '--------------' || '--------------' || '--------------' || '--------------' || '--------------' || '--------------' || '--------------' || '--------------' |
//  '----------------'  '----------------'  '----------------'  '----------------'  '----------------'  '----------------'  '----------------'  '----------------'  '----------------'  '----------------'  '----------------' 
...
contract AgentManager is Ownable, ReentrancyGuard {
    ...
    mapping (uint256 => uint256) public totalPayed;
    ...
    function _createNewAgent(uint256 _tid, uint256 _amount) internal {
        ...
        Proxy agent = new Proxy{salt: salt}(target.master);
        ...
        if(available > 0) {
            IERC20(target.depositToken).safeTransfer(address(agent), available);
    ...
    function _sendAssetToUser(uint256 _pid, uint256 _amount) internal {
        ...
        if(contractBalance < _amount) {
            ...
            pool.token.safeTransfer(msg.sender, contractBalance);
        }
        else {
    ...
    function _swap(uint256 _from, uint256 _to, uint256 _amount) internal returns (uint256) {
        ...
        for (uint256 index = 0; index < routers.length; index++) {
            uint256[] memory amounts = IUniswapV2Router02(routers[index]).getAmountsOut(_amount, path);
            if(amounts[amounts.length] > amountOut) {
                amountOut = amounts[amounts.length];
                router = IUniswapV2Router02(routers[index]);
            }
        }
    ...
    function cancelPendingWithdraw(uint256 _pid) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    function harvest(uint256 _pid) public nonReentrant {
        ...
        UserInfo storage user = userInfo[_pid][msg.sender];
        updateUser(_pid, msg.sender);
        pool.token.safeTransfer(address(msg.sender), user.pending);
        user.pending = 0;
    ...
    function harvestAll() public nonReentrant {
        for (uint256 tid = 0; tid < targetInfo.length; tid++) {
            ...
            IERC20(target.depositToken).safeTransfer(msg.sender, earned.div(20));
            earned = earned.mul(95).div(100);
            totalEarned[target.pid] = totalEarned[target.pid].add(earned);
        }
        ...
        for (uint256 pid = 0; pid < poolInfo.length; pid++) {
            ...
            for (uint256 index = 0; index < debtList.length; index++) {
                ...
                pool.token.safeTransfer(debt.owner, available);
            }
            removeZeroList(pid);
        }
    ...
    function buyTicket(address _user, uint256 _amount) public onlyLottery nonReentrant returns (uint256) {
    ...
    function addTarget(address _master, address[] memory _initAddresses, uint256[] memory _initNumbers) public onlyOwner {
        address agent = testAgent(_master, _initAddresses, _initNumbers);
        uint256 pid;
        for (uint256 index = 0; index < poolInfo.length; index++) {
            if(address(poolInfo[index].token) == IAgent(agent).depositToken())
                pid = index;
        }
        targetInfo.push(TargetInfo({
            master: _master,
            pid: pid,
            depositToken: IAgent(agent).depositToken(),
            rewardToken:  IAgent(agent).rewardToken(),
    ...
    function testAgent(address _master, address[] memory _initAddresses, uint256[] memory _initNumbers) public onlyOwner returns (address){
        bytes32 salt = keccak256(abi.encodePacked(_master, block.number));
        Proxy agent = new Proxy{salt: salt}(_master);
        IAgent(address(agent)).init(_initAddresses, _initNumbers);
    ...
    function distributeProfit() public onlyOwner {
        for (uint256 pid = 0; pid < poolInfo.length; pid++) {
            if(totalEarned[pid] > poolInfo[pid].accAmount) {
                uint256 profit = totalEarned[pid] - poolInfo[pid].accAmount;
                totalPayed[pid] = totalPayed[pid].add(profit);
                poolInfo[pid].token.safeTransfer(feeAddress, profit.mul(feePercent).div(10000));
                uint256 amountOut = _swap(pid, 0, profit.mul(10000 - feePercent).div(10000));
                poolInfo[0].token.safeTransfer(lottery, amountOut);
            }
        }
    }
    ...
    function getProtocolHealthRate() public view returns (uint256) {
        ...
        for (uint256 pid = 0; pid < poolInfo.length; pid++) {
            ...
            totalProfit = totalProfit.add(totalPayed[pid]);
        }
        for (uint256 tid = 0; tid < targetInfo.length; tid++) {
            TargetInfo memory target = targetInfo[tid];
...
contract InsureAceAgent is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    function availableDeposit(uint256) public pure returns (uint256) {
        return type(uint256).max;
    }

    function availableWithdraw(uint256) public view returns (uint256) {
        address stakersV2 = IStakingV2Controller(target).stakersPoolV2();
