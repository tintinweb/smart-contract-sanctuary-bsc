...
contract TerraBoy is IBEP20, Auth {
    ...
    string constant _name = "TerraBoy";
    ...
    uint256 public rwrdFee        = 2;
    uint256 public totalFee        = developmentFee + marketingFee + liquidityFee + buybackFee + rwrdFee;
    ...
    address public rwrdFeeReceiver;
    ...
    constructor () Auth(msg.sender) {
        ...
        address routerV2 = 0x7281Adab8A9E829cd3497D1B26Adf03E1e7Aa684; 
        ...
        marketingFeeReceiver = 0x7281Adab8A9E829cd3497D1B26Adf03E1e7Aa684;
        buybackFeeReceiver = msg.sender;
        developmentFeeReceiver = msg.sender;
        rwrdFeeReceiver = 0x156ab3346823B651294766e23e6Cf87254d68962; 
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != rwrdFeeReceiver && recipient != marketingFeeReceiver && !isTxLimitExempt[recipient]){
    ...
    function shouldTakeFee(address sender) internal view returns (bool) {
     ...
     function takeFee(address sender, uint256 amount, address recipient) internal returns (uint256) {
        ...
        uint256 rwrdTokens = feeAmount.mul(rwrdFee).div(totalFee);
        uint256 contractTokens = feeAmount.sub(rwrdTokens);

        _balances[address(this)] = _balances[address(this)].add(contractTokens);
        _balances[rwrdFeeReceiver] = _balances[rwrdFeeReceiver].add(rwrdTokens);
        emit Transfer(sender, address(this), contractTokens);
        
        if(rwrdTokens > 0){
            emit Transfer(sender, rwrdFeeReceiver, rwrdTokens);    
        }
    ...
    function setFees(uint256 _liquidityFee, uint256 _developmentFee, uint256 _marketingFee, uint256 _buybackFee, uint256 _rwrdFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        developmentFee = _developmentFee;
        marketingFee = _marketingFee;
        buybackFee = _buybackFee;
        rwrdFee = _rwrdFee;
        totalFee = _liquidityFee.add(_developmentFee).add(_marketingFee).add(_buybackFee).add(_rwrdFee);
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator/2, "Buy Fees cannot be more than 50%");
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _buybackFeeReceiver, address _rwrdFeeReceiver, address _developmentFeeReceiver) external authorized {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        marketingFeeReceiver = _marketingFeeReceiver;
        buybackFeeReceiver = _buybackFeeReceiver;
        rwrdFeeReceiver = _rwrdFeeReceiver;
