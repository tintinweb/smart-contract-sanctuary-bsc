...
contract PearlFarm is Initializable, OwnableUpgradeable, IERC721ReceiverUpgradeable, IPearlFarm {
    ...
    /// @notice reclaiming is deprecated as gem is not attached to clam when staking anymore
    ...
    IClamShop public clamShop;
    /// @notice gem spent per user
    mapping(address => uint256) public gemSpent;

    /// @notice keeps track of clams that have been staked before and have not yet produced pearl
    mapping(uint256 => bool) public clamHasBeenStaked;
    ...
    event PearlCollected(address indexed farmer, uint256 indexed clamId, uint256 indexed pearlId);
    ...
    event SetClamShop(IClamShop indexed _clamShop);
    ...
    function registerClamTransfer(uint256 clamId) external override {
    ...
    /// @notice Stake previously staked clam
    /// @param clamId Clam id
    function stakeClamAgain(uint256 clamId) external payable isPossibleToStake(clamId) {
        require(clamHasBeenStaked[clamId], "PearlFarm: This clam was not staked yet, please call 'stakeClam' instead.");
    ...
    function handleBurn(uint256 clamId, uint256 currentPearlPrice) private {
        uint256 burnAmount;
        uint256 transferredGem = gemsTransferred[msg.sender][clamId];

        if (transferredGem > 0) {
            // for legacy clams, that have gem attached at the time of staking
            gemsTransferred[msg.sender][clamId] = 0;
            gemSpent[msg.sender] += transferredGem;
            burnAmount = transferredGem.mul(burnRateProduction).div(100);
            gemToken.transfer(treasury, transferredGem.sub(burnAmount));
        } else {
            gemToken.transferFrom(msg.sender, address(this), currentPearlPrice);

            gemSpent[msg.sender] += currentPearlPrice;
            burnAmount = currentPearlPrice.mul(burnRateProduction).div(100);
            gemToken.transfer(treasury, currentPearlPrice.sub(burnAmount));
        }
    ...
    function collectPearl(uint256 clamId) external senderIsStaker(clamId) {
        ...
        clamHasBeenStaked[clamId] = false;

        IClam.ClamInfo memory clamInfo = clam.getClamData(clamId);

        uint256 currentPearlPrice = clamInfo.gemPrice == 0 ? clamShop.getUpdatedPrice("d").div(10) : clamInfo.gemPrice.mul(dnaDecoder.getPearlPriceForClamGrade(clamInfo.grade)).div(dnaDecoder.getPriceForClamGrade(clamInfo.grade));

        handleBurn(clamId, currentPearlPrice);
        ...
        IDNADecoder.Traits memory traits = clam.getTraits(clamId);

        uint256 pearlBoost;

        if (clamInfo.pearlBoostM > 0) {
            pearlBoost = clamInfo.pearlBoostM;
        } else {
            // pearlBoost of clams before pearlBoost was a property
            pearlBoost = clam.calculatePearlBoost(traits.size, traits.lifespan, traits.rarityValue);
        }
        ...
        pearl.mint(
            ...
            pearlBoost,
            clamId,
            traits.pearlBodyColorNumber,
            traits.pearlShapeNumber
        );

        emit PearlCollected(msg.sender, clamId, pearl.nextPearlId() - 1);
    }
    ...
    function setClamShop(IClamShop _clamShop) external onlyOwner {
        clamShop = _clamShop;

        emit SetClamShop(_clamShop);
    }
    ...
    function setClamHasBeenStaked(uint256 _clamId, bool _hasBeenStaked) external onlyOwner {
        clamHasBeenStaked[_clamId] = _hasBeenStaked;
    }
...
interface IClam is IERC721EnumerableUpgradeable {
    struct ClamInfo {
        ...
        uint256 pearlBoostM; // multiplier for gem boost for every pearl that is born from this clam. In million format because of decimals, divide by 1_000_000
        string grade;
        uint256 gemPrice;
    }
    ...
    function mintMaxima(address, uint256, string memory, uint256) external;
    ...
    event CommunityRewardedClam(address recipient, uint256 dna, string _grade, uint256 gemPrice);
    event ClamMaximaMinted(address recipient, uint256 dna, string _grade, uint256 gemPrice);
...
interface IPearlFarm {
    ...
    function registerClamTransfer(uint256 clamId) external;
...
interface IClamShop {
    struct UnCollectedClamInfo {
      string grade;
      uint256 gemPrice; // in wei
    }
    function getUpdatedPrice(string memory) external returns (uint256);
    function getUpdatedPearlPrice(string memory _grade) external view returns (uint256);
    function getGemPriceUsd() external view  returns (uint256);
}
...
interface IOracle {
    ...
    function getLatestBnbPrice() external view returns (uint256);
}
...
interface IDNADecoder {
    ...
    struct SharedColorProps {
        string color;
        uint8 dropRate;
        uint8 pearlBodyColorNumber;
        uint8 boost;
        uint8[3] hsv;
        uint8[3] hsvMinAdj;
        uint8[3] hsvMaxAdj;
    ...
    struct ShellShapeProps {
        string shape;
        uint8 dropRate;
        uint8 pearlBoost;
        uint8 pearlBoostShape;
    }
    ...
    function decodeTraitsFromDNA(uint256, string memory _grade, bool) external;
    ...
    function getClamGrade(string memory) external view returns (uint256[7] memory);

    function isClamGradeValid(string memory _grade) external view returns (bool);

    function getPriceForClamGrade(string memory _grade) external view returns(uint256);

    function getPearlPriceForClamGrade(string memory _grade) external view returns(uint256);

    function getShellForClamGrade(string memory _grade) external view returns(uint256);

    function getClamGradesList() external view returns (string[10] memory);

    function getClamGradesData() external view returns(uint256[7][10] memory);
}
