...
contract Seedx_Dropzone is Initializable, Ownable {
    ...
    function initialize(address _newowner,address _feeTo,uint _fee) external initializer {
        ownable(_newowner);
        feeTo = payable(_feeTo);
        fee = _fee;
    }
   
    function applyforAirdrop(address _token,uint _amount,uint _from ,uint _to ,string[] memory _socialMedia ,uint[] calldata amount,bool _isVesting, uint _vestingPeriod) external payable {
        require(_from<_to,"Invalid Date");
        require(msg.value>=10**18,"Fee mising");
        feeTo.transfer(1 ether);
	...
	function getVestingInfo(bytes32 _hash ,address _user) external view returns (ClaimWithVesting memory ) {
		return airdrops[_hash].claimWithVesting[_user];
	}

    function claimAirdrop(bytes32 _hash , uint _amount) external noReentrant payable{
        Airdrop storage airdrop = airdrops[_hash];
        ...
        if(!airdrop.isVesting){
            require(msg.value ==fee,"Fee Is Missing");
            feeTo.transfer(fee);
    ...
    function withdrawRemainTokenAfterExpire(bytes32 _hash) external onlyCreator(_hash) {
        Airdrop storage airdrop = airdrops[_hash];
        require(airdrop.to<block.timestamp,"Airdrop : Expired");
        IERC20(airdrop.token).transfer(airdrop.owner, airdrop.amount-airdrop.distbutedAmount);
        airdrop.distbutedAmount += airdrop.amount-airdrop.distbutedAmount;  
    ...
    function getSocialMediaReward(bytes32 _hash)  external view returns(string[5] memory socialMediaId,uint[5] memory _amount) {
        return (airdrops[_hash]._socialMediaId,airdrops[_hash]._socialMediaAmount);
    }
