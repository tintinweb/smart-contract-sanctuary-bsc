...
contract ConsumptionNFT is ERC1155, ERC2981, Ownable {
    using Strings for uint256;

    uint32 public constant MAX_SUPPLY = 10000000;
    ...
    function mint(uint256 id, address to, uint32 amount) external {
        require(_allowedMinter == msg.sender, "Consumption: ?");
        require(amount + _minted[id] <= MAX_SUPPLY, "Consumption: Exceed max supply");

        _minted[id] += amount;
        _mint(to, id, amount, "");
    }

    function airdrop(uint256 id, address[] memory tos, uint32[] memory amounts) external onlyOwner {
        require(tos.length == amounts.length);
        for (uint256 i = 0; i < amounts.length; i++) {
            _minted[id] += amounts[i];
            require(_minted[id] <= MAX_SUPPLY, "Consumption: Exceed max supply");
    ...
    function burn(address who, uint32 amount) external {
        require(msg.sender == _burner, "Consumption: ?");
