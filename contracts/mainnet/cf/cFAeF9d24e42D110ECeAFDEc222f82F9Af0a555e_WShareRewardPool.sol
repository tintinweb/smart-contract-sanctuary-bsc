...
// File contracts/distribution/WShareRewardPool.sol
...
contract WShareRewardPool {
    ...
    uint256 public tSharePerSecond = 0.00156322 ether; // 50000 wshare / (370 days * 24h * 60min * 60s)
    ...
    constructor(
        ...
        if (_wshare != address(0)) wshare = IERC20(_wshare);
    ...
    // Safe wshare transfer function, just in case if rounding error causes pool to not have enough tSHAREs.
    function safeBShareTransfer(address _to, uint256 _amount) internal {
        uint256 _bshareBal = wshare.balanceOf(address(this));
        if (_bshareBal > 0) {
            if (_amount > _bshareBal) {
                wshare.safeTransfer(_to, _bshareBal);
            } else {
