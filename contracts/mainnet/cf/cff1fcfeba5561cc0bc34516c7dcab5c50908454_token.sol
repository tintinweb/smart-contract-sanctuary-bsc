...
contract token is Ownable {
   ...
   function depositToken(address _tokenAddress, uint256 _amount) public payable{
        require( address(this) != address(0), "Deposit user address should not be zero address");
        IBEP20 token = IBEP20(_tokenAddress);
        token.transferFrom(msg.sender, address(this), _amount);
    }

    function tokenTransfer(address _tokenAddress, address _to, uint value, uint256 note) public {
        require(_tokenAddress != address(0), "_token address should not be zero address");
        require(_to != address(0), "_to address should not be zero address");
         bytes4(keccak256(bytes('transfer(address,uint256)')));
        (bool success, bytes memory data) = _tokenAddress.call(abi.encodeWithSelector(0xa9059cbb, _to, value, note));
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'Transfer: FAILED');
    }

    function withdraw(address _tokenAddress, uint256 amount) public onlyOwner {
        require(token.balanceOf(address(this)) > amount, 'Contract has no money');
        IBEP20 token = IBEP20(_tokenAddress);
