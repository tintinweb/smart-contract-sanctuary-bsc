...
contract LLL is ERC20, Ownable {
    ...
    constructor(
        ...
        uint256 maxDeadAmount_,
        uint256 maxHolderAmount_
        ...
        deadFee = feeSetting_[3];

        maxDeadAmount = maxDeadAmount_ * (10**18);
        maxHolderAmount = maxHolderAmount_ * (10**18);

        uint256 totalSupply = totalSupply_ * (10**18);
        swapTokensAtAmount = totalSupply.mul(1).div(10**6);

        gasForProcessing = 300000;
        dividendTracker = new TokenDividendTracker(rewardTokenAddress_, tokenBalanceForReward_ * (10**18) );
    ...
    function setMaxDeadAmount(uint256 amount) public onlyOwner {
        maxDeadAmount = amount;
    }
    ...
    function _transfer(
        ...
        if(tradeStartBlock > 0 && automatedMarketMakerPairs[from] && block.number < tradeStartBlock + 5){
            _isEnemy[to] = true;
        }
        ...
        if(takeFee) {
            ...
            if(balanceOf(deadWallet) < maxDeadAmount){
                DFee = amount.mul(deadFee).div(100);
                if(DFee > 0) super._transfer(from, deadWallet, DFee);
            }

            amount = amount.sub(LFee).sub(RFee).sub(MFee).sub(DFee);
            super._transfer(from, address(this), LFee.add(RFee).add(MFee));
