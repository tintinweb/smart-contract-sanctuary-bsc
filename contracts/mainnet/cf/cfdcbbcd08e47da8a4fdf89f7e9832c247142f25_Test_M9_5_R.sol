               ...
               if(users[referrer].referrer == upline1 && users[upline1].activeMainLevels[lvel] ==  users[referrer].activeMainLevels[lvel])
    {
				...
				if (upline1 != address(0)) {
                    ...
                    //refera = upline1;
                          ...
                          //  if(refera[msg.sender]==address(0) && refera[msg.sender]!=msg.sender){
   ...
   function LevelReferrals1_1(address useraddress) public view returns (address) {
       ...
       function LevelReferrals1_3(address referrerAddress) public view returns (uint256) {
        ...
        return idToAddress[lastUserId];
           ...
           function LevelReferrals1_7(address referrerAddress) public view returns (address) {
       			User storage user = users[referrerAddress];
...
function withdraw () public {
 ...
 if( user.DEP_REF_activatesRefer_Timer >= user.checkpointLVR1RefBonus && user.partnersCount >=1)
{
		user.checkpoint = block.timestamp;
        token.transfer(msg.sender, totalAmount);
        emit Withdrawn(msg.sender, totalAmount);
...
return totalAmount2;
        	...
        	function DEP_REF_1_1() public view returns(uint256) {
	    User storage user = users[msg.sender];
...
if (user.DEP_REF_activatesRefer_Timer >= user.checkpointLVR1RefBonus)
{totalAmount3 = 555;}
else {totalAmount3 = 123;}
return totalAmount3;
	}

            	function DEP_REF_1_1_2() public view returns(uint256) {
	    User storage user = users[msg.sender];
...
return totalAmount3;
    	...
    	function DEP_REF_1_2(address userAddress) public view returns(uint256) {
	    User storage user = users[userAddress];
