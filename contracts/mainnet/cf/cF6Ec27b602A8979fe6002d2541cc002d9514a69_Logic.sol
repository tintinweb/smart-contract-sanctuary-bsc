...
contract Logic is Ownable {
    ...
    modifier onlyStorage() {
        require(msg.sender == _storage, "You can not accept");
        _;
    }

    modifier isUsedVToken(address vToken) {
        require(usedVTokens[vToken], "vTokens is not used");
        _;
    }

    modifier isUsedSwap(address swap) {
        require(swap == apeswap || swap == pancake, "swap is not used");
        _;
    }

    modifier isUsedMaster(address swap) {
        require(
            swap == pancakeMaster || apeswapMaster == swap,
            "swapMaster is not used"
        );
        _;
    }
    ...
    function addVTokens(address token, address vToken) external onlyOwner {
        ...
        require(_isUsedVToken, "vToken is not used");
        if ((token) != address(0)) {
        ...
        returns (address[] memory path)
    {
        ...
        for (uint256 i = 0; i < length; i++) {
            if (reserves[id].path[i][length - 1] == token) {
    ...
    function repayBorrowBNBandToken(address swap, address tokenB,address VTokenA, address VTokenB, uint lpAmount)
        private
    {
        (
            uint256 amountToken,
            uint256 amountETH
        ) = IPancakeRouter01(swap)
                .removeLiquidityETH(
                    tokenB,
                    lpAmount,
                    0,
        ...
        {
            uint256 totalBorrow = IVToken(VTokenA)
                .borrowBalanceCurrent(address(this));
            if (totalBorrow >= amountETH) {
                IVToken(VTokenA).repayBorrow{
                    value: amountETH
                }();
            } else {
                IVToken(VTokenA).repayBorrow{
            ...
            totalBorrow = IVToken(VTokenB)
                .borrowBalanceCurrent(address(this));
            if (totalBorrow >= amountToken) {
                IVToken(VTokenB).repayBorrow(
                    amountToken
                );
            } else {
                IVToken(VTokenB).repayBorrow(
    ...
    function repayBorrowOnlyTokens(address swap, address tokenA, address tokenB,address VTokenA, address VTokenB, uint lpAmount)
        private
    {
        (uint256 amountA, uint256 amountB) = IPancakeRouter01(
            swap
        ).removeLiquidity(
                tokenA,
                tokenB,
    ...
    function withdrawAndRepay(ReserveLiquidity memory reserve, uint256 lpAmount)
        private
    {
        IMasterChef(reserve.swapMaster).withdraw(
            reserve.poolID,
            lpAmount
        );
        if (
            reserve.tokenA == address(0) ||
            reserve.tokenB == address(0)
        ) {
            //if tokenA is BNB
        ...
        onlyStorage
    {
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            (uint256 depositedLp, ) = IMasterChef(reserve.swapMaster)
                .userInfo(reserve.poolID, address(this)); //get how many deposited to farming 
    ...
    function addReserveReserveLiquidity(
        ReserveLiquidity memory reserveLiquidity
