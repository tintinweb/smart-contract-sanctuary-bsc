...
contract OlaLogic is Logic {
  modifier isTokenTypeAccepted(uint8 leadingTokenType) override {
    require(leadingTokenType == oTokenType, "E8");
    _;
  }
   ...
   */
  function rewardsAccrued() external returns (uint256) {
    return IDistributionOla(olaRainMaker).compAccrued(address(this));
  }
  ...
  function returnToken(uint256 amount, address token)
    ...
    onlyStorage
  {
    uint8 circlesCount = 3;
    uint256 collateralFactor;
    address cToken = XTokens[token];
    uint8 iterateCount = circlesCount + 3; // additional iteration to repay all borrowed
    uint8 avoidLiquidityFactor = 2;

    // Get information from comptroller
    (, collateralFactor, , , , ) = IComptrollerOla(olaComptroller).markets(
      cToken
    );

    // Apply avoidLiquidityFactor to collateralFactor
    collateralFactor = ((collateralFactor * 100) / (10**18)) - avoidLiquidityFactor;
    require(collateralFactor > 0, "OC10");
    ...
    for (uint256 i = 0; i < iterateCount;) {
      ...
      (, cTokenBalance, borrowBalance, exchangeRateMantissa) = IXToken(cToken)
        .getAccountSnapshot(address(this));

      // calculates of supplied balance, divided by 10^18 to safe digits correctly
      uint256 supplyBalance = (cTokenBalance * exchangeRateMantissa) / 10**18;
      ...
      if (withdrawBalance > amount) {
        IXToken(cToken).redeemUnderlying(amount);
        if (borrowBalance > 0 && borrowBalance < amount) {
          IXToken(cToken).repayBorrow(borrowBalance);  
        } else {
          IXToken(cToken).repayBorrow(amount);
        }
        IXToken(cToken).redeemUnderlying(amount);
        break;
      }
      ...
      if (borrowBalance == 0) {
        if (cTokenBalance > 0) {
          // redeem and exit
          IXToken(cToken).redeemUnderlying(supplyBalance);
          break;
        }
      }
      // if already redeemed
      ...
      // if redeem tokens
      IXToken(cToken).redeemUnderlying(withdrawBalance);
      uint256 repayAmount = IERC20Upgradeable(token).balanceOf(address(this));

      // if there is something to repay
      if (repayAmount > 0) {
        // if borrow balance more then we have on account
        if (borrowBalance > repayAmount) {
          IXToken(cToken).repayBorrow(repayAmount);
        } else {
          IXToken(cToken).repayBorrow(borrowBalance);
        }
      }
...
contract Logic is LogicUpgradeable {
  ...
  address internal olaComptroller;
  address internal olaRainMaker;
  ...
  uint8 internal constant vTokenType = 0;
  uint8 internal constant oTokenType = 1;
  ...
  modifier isUsedMaster(address swap) {
    require(
      swap == pancakeMaster || apeswapMaster == swap || biswapMaster == swap,
      "E4"
    );
    _;
  }

  modifier isTokenTypeAccepted(uint8 leadingTokenType) virtual {
    require(
  ...
  function addXTokens(
    ...
    if (leadingTokenType == vTokenType) {
      (_isUsedXToken, , ) = IComptrollerVenus(venusComptroller).markets(xToken);
    }
    ...
    onlyStorage
  {
    ...
    //     IXToken(XTokens[token]).redeemUnderlying(amount);
    //     return;
    //   }
    //   unchecked {
    //     ++i;
  ...
  {
    if (leadingTokenType == vTokenType)
      IDistributionVenus(venusComptroller).claimVenus(address(this), xTokens);
    if (leadingTokenType == oTokenType)
  ...
  //   )
  // {
  //   (amountETHDesired, amountTokenMin, amountETHMin) = IPancakeRouter01(swap)
...
//         IXTokenETH(xTokenA).repayBorrow{value: amountETH}();
//       } else {
//         IXTokenETH(xTokenA).repayBorrow{value: totalBorrow}();
//       }

//       totalBorrow = IXToken(xTokenB).borrowBalanceCurrent(address(this));
//       if (totalBorrow >= amountToken) {
//         IXToken(xTokenB).repayBorrow(amountToken);
//       } else {
//         IXToken(xTokenB).repayBorrow(totalBorrow);
//       }
//     }
//   }
...
//     );
//     {
//       uint256 totalBorrow = IXToken(xTokenA).borrowBalanceCurrent(
//         address(this)
//       );
//       if (totalBorrow >= amountA) {
//         IXToken(xTokenA).repayBorrow(amountA);
//       } else {
//         IXToken(xTokenA).repayBorrow(totalBorrow);
//       }

//       totalBorrow = IXToken(xTokenB).borrowBalanceCurrent(address(this));
//       if (totalBorrow >= amountB) {
//         IXToken(xTokenB).repayBorrow(amountB);
//       } else {
//         IXToken(xTokenB).repayBorrow(totalBorrow);
//       }
//     }
