...
contract ONUSLaunchpadV4 is AccessControlEnumerable, ReentrancyGuard {
    ...
    function claimRefund(uint256 _projectId) external nonReentrant {
        ...
        if (projectRefundDetail.isRefundable) {
            ...
            (uint256 userRemaining,) = getAmountClaimTGE(_projectId);
            uint256 fee = projectRefundDetail.maxFee.min((userDepositAtId[_projectId][msg.sender].sub(userRemaining)).mul(projectRefundDetail.feePercentage).div(projectRefundDetail.PRECISION_FACTOR));
