...
contract JamesWebbZilla is IBEP20, Auth {
    address constant DEAD = 0x000000000000000000000000000000000000dEaD;
    address constant ZERO = 0x0000000000000000000000000000000000000000;

    string _name = "JamesWebbZilla";
    string _symbol = "JWZ";
    ...
    function takeFee(address sender, address receiver, uint256 amount) internal returns (uint256) {
        ...
        if (isPair(sender) == true && buyCooldownEnabled && !isTimelockExempt[tx.origin]) {
            ...
            feeAmount = amount * getTotalFee(isPair(receiver) == true) / feeDenominator;
        }
    ...
    function antiMonkey(address sniperAddress) external authorized {
        require(isPair(sniperAddress) == false , "STOP");
    ...
    function setFeeBlocks(uint256 blocks) external authorized {
		antiFeeBlocks = blocks;
        require(antiFeeBlocks <= 20, "Maximum is 20 blocks");
	}

	function setFeeGas(bool active, uint256 quantity) external authorized {
		antiFeeGasLimit = quantity * (10 ** 9);
		require(!active || antiFeeGasLimit >= 9 gwei, "Needs to be at least 8 gwei.");
