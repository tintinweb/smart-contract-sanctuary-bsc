...
contract HonorLand is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0xfc2D40ad08f176f728A63DAdd22abC6929f544b6); // Marketing Address
    ...
    mapping (address => bool) private _blacklistFlag;
    ...
    string private _name = "HonorLand";
    string private _symbol = "HONOR";
    ...
    bool private _isAutoBuyBack = true;
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        require(!_blacklistFlag[_msgSender()] && !_blacklistFlag[recipient], "Sender or Recipent is blacklist.");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        require(!_blacklistFlag[sender] && !_blacklistFlag[recipient], "Sender or Recipent is blacklist.");
    ...
    function _transfer(
        ...
        require(!_blacklistFlag[from] && !_blacklistFlag[to], "Sender or Recipent is blacklist.");
    ...
    function addBlackList(address _address) external onlyOwner {
        require(!_blacklistFlag[_address], "This address is already blacklist");
        _blacklistFlag[_address] = true;
        _blacklists.push(_address);
    }

    function removeBlackList(address _address) external onlyOwner {
        require(_blacklistFlag[_address], "This address is not blacklist");
        for (uint256 i = 0; i < _blacklists.length; i++) {
            if (_blacklists[i] == _address) {
                _blacklists[i] = _blacklists[_blacklists.length - 1];
                _blacklistFlag[_address] = false;
                _blacklists.pop();
                break;
            }
        }
