...
contract MbaToken is ERC20, Ownable {
    ...
    uint256 public buybackFee = 167; 
    uint256 public buybackMbankFee = 167; 
    uint256 public tokenDividendFee = 291; 
    uint256 public burnFee = 250;
    uint256 public developFee = 42;
    uint256 public fundFee = 83; 
                    
    uint256 public deflationLimit  = 10000000 * (10 ** decimals());
    ...
    constructor( 
        ...
        address _mbankAddress
    ) ERC20("MetBanka", "MBA") {
        //init wallet
        usdtAddress = _usdtAddress;
        mbankAddress = _mbankAddress;
    ...
    function updatePancakeRouter(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeRouter), "MBAToken: The router already has that address");
    ...
    function setBuyFees(
        ...
        buybackMbankFee = _buybackMbankFee; 
    ...
    function setConfigAddress(address _developAddress, address _mbankAddress, address _idoAddess, address[] memory _fundAddresses) external onlyOwner {
        developAddress = _developAddress;
        mbankAddress = _mbankAddress;
        idoAddess = _idoAddess;

        fundAddresses = _fundAddresses;
    }
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MBAToken: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MBAToken: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MBAToken: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "MBAToken: This account cannot send tokens until trading is enabled");
        }

        if(!from.isContract()) {
            uint fromBalance = balanceOf(from);
            require(fromBalance > 1000000000000000000, "MBAToken: Balance lock");
    ...
    function swapMbaToTokens(uint256 tokenAmount, address receiveAddress) private {
    ...
    function swapAndDistribute(uint256 tokens) private {
        ...
        uint256 deflationAmount = super.totalSupply();
        if(deflationAmount > deflationLimit) {
            forBurn = tokens.mul(burnFee).div(1000);
            super._burn(address(this), forBurn);
        }

        uint256 mbaTokenDividendsAmount = tokens.mul(tokenDividendFee).div(1000);
        if(mbaTokenDividendsAmount > 0) {
            _approve(address(this), address(dividendTracker), mbaTokenDividendsAmount);
            dividendTracker.distributeDividends(mbaTokenDividendsAmount);
            emit SwapAndSendTo(mbaTokenDividendsAmount, "TOKENDIVIDENDS");
        }
    
        //
        uint256 buybackMbankAmount = tokens.mul(buybackMbankFee).div(1000);
        swapMbaToTokens(buybackMbankAmount, 0x000000000000000000000000000000000000dEaD);

        uint256 totalSwapAmount = tokens.sub(forBuyback.add(forBurn).add(mbaTokenDividendsAmount.add(buybackMbankAmount)));
        swapTokensToToken(totalSwapAmount, tokenReceiver);
        uint256 swapTotalUsdtAmount = IERC20(usdtAddress).balanceOf(tokenReceiver);
        uint256 fundAmount = fundFee.mul(swapTotalUsdtAmount.div(developFee.add(fundFee)));
        uint256 perUsdtAmount = fundAmount.div(fundAddresses.length);
        for(uint256 i = 0; i < fundAddresses.length; i++) {
            IERC20(usdtAddress).transferFrom(tokenReceiver, fundAddresses[i], perUsdtAmount);
        }

        uint256 developAmount = swapTotalUsdtAmount.sub(fundAmount);
