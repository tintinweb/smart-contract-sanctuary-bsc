...
contract GymMLMQualifications is OwnableUpgradeable {
    ...
    // user level from 1 to 25 (0 mark as False)
    ...
    function setUserLevel(address _address, uint256 _level) external onlyOwner {
     ...
     * @return _userLevel user MLM level
     */
    function getUserCurrentLevel(address _user) external view returns (uint256 _userLevel) {
     ...
     */
    function _getUserLevel(address _userAddress) private view returns (uint256) {
        uint256 _userLevel;
        ...
        if (userLevel[_userAddress] != 0) {
            _userLevel = userLevel[_userAddress] - 1;
        } else {
            for (uint32 i = 0; i <= 24; i++) {
                if (_usdFarm + _usdPool >= levels[i].usdAmountVault) {
                    _userLevel = i;
                } else {
                    break;
                }
        ...
        uint256 _rockstarRank = _getRockstarRank(_userAddress, _usdVault, _usdFarm, _usdFarm);
        uint256 mlmLevel = _rockstarRank > _userLevel ? _rockstarRank : _userLevel;
        //gymStreetAddress
        uint256 gymStreetLevel = INetGymStreet(0xF07cDB4eE143F3ecF34ece7c593D8DD726615484).getUserCurrentLevel(_userAddress);
        return gymStreetLevel > mlmLevel ? gymStreetLevel : _userLevel;
    }
    ...
    function _getRockstarRank(
        ...
        uint256 _usdFarm,
        uint256 _usdPool
    ) private view returns (uint256 _rockstarRank) {
        for (uint32 i = 0; i < 2; i++) {
            if (
                ...
                _rockstarRank = rockStarLevels[i].qualificationLevel;
            }
        }
    }
