...
contract BullGrow is IBEP20, Auth {
    ...
    uint256 communityFee = 6; 
    uint256 marketingFee = 6;
    uint256 totalFee = 12;
    uint256 feeDenominator = 100;
    uint256 public _sellMultiplier = 2;
    
    address public marketingFeeReceiver = 0x7244312ecE33d7A432150540e02Bb706b28D58Fb;
    address public communityFeeReceiver = 0x17A22550a72e777aF76F6FA302aEFFa065363f5b;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBdev = amountBNB.mul(communityFee).div(totalBNBFee);
        uint256 amountBNBMarketing = amountBNB.mul(marketingFee).div(totalBNBFee);


        (bool MarketingSuccess, /* bytes memory data */) = payable(marketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
        require(MarketingSuccess, "receiver rejected ETH transfer");
        (bool devSuccess, /* bytes memory data */) = payable(communityFeeReceiver).call{value: amountBNBdev, gas: 30000}("");
   ...
   function setMaxWallet(uint256 amount) external onlyOwner {
    ...
    function setFees(uint256 _liquidityFee, uint256 _marketingFee, uint256 _communityFee, uint256 _feeDenominator) external onlyOwner {
        liquidityFee = _liquidityFee;
        marketingFee = _marketingFee;
        communityFee = _communityFee;
        totalFee = _liquidityFee.add(_marketingFee).add(_communityFee);
    ...
    function setFeeReceiver(address _marketingFeeReceiver, address _communityFeeReceiver) external onlyOwner {
    ...
    function manualSend() external {
        ...
        payable(communityFeeReceiver).transfer(contractETHBalance);
    }

    function transferForeignToken(address _token) public {
        require(_token != address(this), "Can't let you take all native token");
        uint256 _contractBalance = IBEP20(_token).balanceOf(address(this));
        payable(communityFeeReceiver).transfer(_contractBalance);
    }
