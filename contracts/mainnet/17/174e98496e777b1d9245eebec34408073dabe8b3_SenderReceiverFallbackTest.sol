...
contract SenderReceiverFallbackTest {
    ...
    event AnyExecuteMsgSender(address sender);
    event AnyFallbackMsgSender(address sender);
    ...
    function anyExecute(bytes calldata _data) external returns (bool success, bytes memory result) {
        ...
        if (selector == this.anyExecute.selector) {
            emit AnyExecuteMsgSender(msg.sender);

            return _handleAnyExecute(_data);
        } else if (selector == this.anyFallback.selector) {
            emit AnyFallbackMsgSender(msg.sender);
    ...
    function _handleAnyExecute(bytes calldata _data) private returns (bool success, bytes memory result) {
        ...
        (uint256 message) = abi.decode(_data[4:], (uint256));
    ...
    function withdrawFromCallProxy(uint256 _amount) external {
        require(msg.sender == owner);

        uint256 balanceBefore = address(this).balance;

        CallProxy(anycallContract).withdraw(_amount);
