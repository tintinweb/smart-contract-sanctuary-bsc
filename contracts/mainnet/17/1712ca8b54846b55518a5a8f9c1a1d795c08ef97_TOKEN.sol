...
contract TOKEN is IERC20, MinterRole {
    ...
    address public marketingFeeReceiver = 0x2a91fE5181570617699BA873D925844a50C43824;
    uint256 public marketingFee = 5;
    uint256 public liquidityFee = 5;
    uint256 public buybackFee = 0;
    uint256 public burnFee = 2;
    uint256 public sellTaxRate = liquidityFee.add(buybackFee).add(marketingFee).add(burnFee);
    uint256 public constant MAX_TX_RATE = 1000;
    uint256 public transferTaxRate = 60;    

    event AdminTokenRecovery(address tokenAddress, uint256 tokenAmount);
    address public constant deadWallet = 0x000000000000000000000000000000000000dEaD;
    address public buyBackToken = address(0x2a91fE5181570617699BA873D925844a50C43824); 
    ...
    event CTYSWAPSwapRouterUpdated(address indexed operator, address indexed router, address indexed pair);
        ...
        if (tradingOpen || sender == owner()) {
            if (
                ...
                if (transferTaxRate == 0 || _excludedFromFees[sender] || _excludedFromFees[recipient]) {
                    ...
                    require(amount == taxAmount + sendAmount, "PXS:: transfer: Tax value invalid");
    ...
    function swapAndLiquifyBuyBack() private lockTheSwap transferTaxFree {
        ...
        if (contractTokenBalance >= minAmountToLiquify) {
            if (sellTaxRate > 0) {
                if (buybackFee > 0) {
                    uint256 buybackTokens = minAmountToLiquify.mul(buybackFee).div(sellTaxRate);
                    processBuyback(buybackTokens);                 
                }
                if (marketingFee > 0) { 
                ...
                if (burnFee > 0) {
                    uint256 burningTokens = minAmountToLiquify.mul(burnFee).div(sellTaxRate);
 ...
 function setmkt(address _marketingFeeReceiver) public onlyOwner {
    ...
    function processBuyback(uint256 tokens) private {
    ...
    function setExcludeFromFees(address _account, bool _excluded) public onlyOwner {
        require(_excludedFromFees[_account] != _excluded, "PXS: Account is already the value of 'excluded'");
    ...
    function updateCTYSWAPSwapRouter(address _router) public onlyOwner {
        router = IUniswapV2Router02(_router);
        routerPair = IUniswapV2Factory(router.factory()).getPair(address(this), router.WETH());
        require(routerPair != address(0), "PXS::updatePXSWAPSwapRouter: Invalid pair address.");
        emit CTYSWAPSwapRouterUpdated(_router, address(router), routerPair);  
    }

    function setTxFees(uint256 _transactionFee) external onlyOwner{
        transferTaxRate = _transactionFee;
        require(transferTaxRate <= MAX_TX_RATE, "PXS: Total fees can not be bigger than max value");
    }

    function setSellFees(uint256 _liquidityFee, uint256 _buybackFee, uint256 _marketingFee, uint256 _burnFee) external onlyOwner{
        liquidityFee = _liquidityFee;
        buybackFee = _buybackFee;
        marketingFee = _marketingFee;
        burnFee = _burnFee; 
        sellTaxRate = liquidityFee.add(buybackFee).add(marketingFee).add(burnFee);
        require(sellTaxRate <= MAX_TX_RATE, "PXS: Total fees can not be bigger than max value");
    }
    ...
    */
        ...
        external
    {
        ...
        require(block.timestamp <= expiry, "PXS::delegateBySig: signature expired");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying PXS (not scaled);
