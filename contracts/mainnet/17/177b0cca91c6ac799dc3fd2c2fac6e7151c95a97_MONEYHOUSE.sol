...
contract MONEYHOUSE is IBEP20, Auth {
    using SafeMath for uint256;

    address CAKE = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address FUND = 0x133f626810cF2DF3f2351A870be96e0f3E5c85B3;
    address MKT  = 0x2Db0612F586Efecc0bAAE67B1af2A8cb4bf1a0BE;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

    string constant _name = "MoneyHouse Coin";
    string constant _symbol = "MONEY";
    ...
    address public fundFeeReceiver = FUND;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBFund = amountBNB.mul(fundFee).div(totalBNBFee);

        try distributor.deposit{value: amountBNBReflection}() {} catch {}
        (bool tmpSuccess,) = payable(marketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
        (bool tmpFundSuccess,) = payable(fundFeeReceiver).call{value: amountBNBFund, gas: 30000}("");

        // only to supress warning msg
        tmpSuccess = false;
        tmpFundSuccess = false;
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _fundFee, uint256 _marketingFee,  uint256 _burnFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        reflectionFee = _reflectionFee;
        marketingFee = _marketingFee;
        fundFee = _fundFee;
        totalFee = _liquidityFee.add(_reflectionFee).add(_marketingFee).add(_fundFee).add(_burnFee);
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator);
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _fundFeeReceiver) external onlyOwner {
