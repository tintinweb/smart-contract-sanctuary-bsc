...
contract Betting is VerifySignature, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function claimCommission(uint256 poolId_) external validPool(poolId_) {
        ...
        // console.log("Transfer amount: %s", _commissionAmount);
    ...
    function _totalAmountWon(address who_, uint256 poolId_) internal view returns(uint256) {
        ...
        uint256 _teamBalance = poolBalance[poolId_][pool.winnerId];
        if (_teamBalance == 0) {
            return 0;
        }
        ...
        for (uint256 i = 0; i < _userBets.length; i++) {
            ...
            if (bet.teamId == pool.winnerId) {
                ...
                _winningAmount += (((pool.totalAmount - _teamBalance) * bet.amount)/ _teamBalance) + bet.amount;
            }
        } 
    ...
    function _totalAmountRefunded(address who_, uint256 poolId_) internal view returns(uint256) {
        ...
        for (uint256 i = 0; i < _userBets.length; i++) {
            // Get bet reference
            Bet memory bet = bets[_userBets[i]];
            Commission memory commission = poolCommission[poolId_][bet.id];
            _refundedAmount += bet.amount + commission.amount;
        } 
