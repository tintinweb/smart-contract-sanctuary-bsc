...
contract BSCYieldFarm is ReentrancyGuard {
    ...
    function joinPresales() external payable{
        ...
        address _userID = msg.sender;
        uint256 _isStaker = getUserAmountOfDeposits(_userID, 0);
        require(_isStaker > 0);
        uint256 _amount = msg.value;
    ...
    function purchaseWithActiveStakes(uint8 _allocation) public{
        require(TOKEN_PRESALE_ENDS >= block.timestamp && _allocation >= 100 && _allocation <= 10000);
