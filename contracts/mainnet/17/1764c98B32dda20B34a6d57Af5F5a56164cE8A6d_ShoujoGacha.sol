...
contract ShoujoGacha is Auth {
	...
	constructor(address hibi, address shoujo, address d, address r) Auth(msg.sender) {
		hibiki = hibi;
		cs = shoujo;
		directory = d;
		randomProvider = r;
	...
	function setPrices(uint256 rol, uint256 guaranteed, uint256 exch) external authorized {
		rollPrice = rol;
		guaranteePrice = guaranteed;
		exchangePrice = exch;
	}

	function setDirectory(address r) external authorized {
		directory = r;
	}
	...
	function getReceiverAddress() internal view returns (address) {
		...
		ShoujoDirectory dir = ShoujoDirectory(directory);
		return dir.shoujoBattle();
	}

	function takePrice(uint256 price) internal {
		IBEP20(hibiki).transferFrom(msg.sender, getReceiverAddress(), price);
	}
	...
	function forceRemoveCardFromList(uint256 cardId) external authorized {
		removeFromList(heldGirls, cardId);
	}
	...
	function availableSets() external view returns (uint256) {
		return sets.length;
	}

	function availableCards() external view returns (uint256) {
		return heldGirls.length;
	}
}