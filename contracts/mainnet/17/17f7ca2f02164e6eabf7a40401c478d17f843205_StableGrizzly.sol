...
// File contracts/Interfaces/IStableGrizzly.sol

//SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.4;

interface IStableGrizzly {
    enum Strategy {
        GRIZZLY,
        STANDARD,
        STABLECOIN
...
// File contracts/Config/StableBaseConfig.sol
...
// File contracts/Strategy/StableGrizzlyStrategy.sol
...
abstract contract StableGrizzlyStrategy is Initializable, StableBaseConfig {
    using SafeERC20Upgradeable for IERC20Upgradeable;
...
// File contracts/Strategy/StableStandardStrategy.sol
...
abstract contract StableStandardStrategy is Initializable, StableBaseConfig {
    using SafeERC20Upgradeable for IERC20Upgradeable;
...
// File contracts/Strategy/StableStableCoinStrategy.sol
...
abstract contract StableStableCoinStrategy is Initializable, StableBaseConfig {
...
// File contracts/StableGrizzly.sol
...
contract StableGrizzly is
    Initializable,
    StableBaseConfig,
    StableGrizzlyStrategy,
    StableStableCoinStrategy,
    StableStandardStrategy,
    ...
    IStableGrizzly
{
    ...
    function deposit(
        ...
        return _deposit(msg.value, referralGiver, false);
    }
    ...
    function depositFromToken(
        ...
        return _deposit(amountConverted, referralGiver, false);
    }
    ...
    function depositLp(
        ...
        return _deposit(amount, referralGiver, true);
    }
    ...
    function withdraw(
        ...
        require(deadline > block.timestamp, "DE");
        DEX.checkSlippage(fromToken, toToken, amountIn, amountOut, slippage);
        uint256 amountWithdrawn = _withdraw(amount, false);
    ...
    function withdrawToLp(
        ...
        require(deadline > block.timestamp, "DE");
        DEX.checkSlippage(fromToken, toToken, amountIn, amountOut, slippage);
        uint256 amountWithdrawn = _withdraw(amount, true);
        LPToken.transfer(msg.sender, amountWithdrawn);
        return amountWithdrawn;
    }
    ...
    function _deposit(
        ...
        if (!isLp) {
            (lpValue, unusedTokenA, unusedTokenB) = DEX.convertEthToPairLP{
                value: amount
            }(address(StableSwap));
        } else {
            lpValue = amount;
    ...
    function _withdraw(uint256 amount, bool isLp) internal returns (uint256) {
        _stakeRewards();
        uint256 withdawAmount;
        ...
        if (!isLp) {
            uint256 bnbAmount = DEX.convertPairLpToEth(
                address(StableSwap),
                amount
            );
            withdawAmount = bnbAmount;
        } else {
            withdawAmount = amount;
        }
    ...
    function stakeStandardRewards(uint256 bnbReward) internal {
        ...
        (
            ...
            uint256 unusedTokenA,
            uint256 unusedTokenB
        ) = DEX.convertEthToPairLP{value: tokenPairLpShare}(
                address(StableSwap)
            );
    ...
    function stakeStablecoinRewards(uint256 bnbReward) internal {
        ...
        (uint256 pairLpAmount, uint256 unusedTokenA, uint256 unusedTokenB) = DEX
            .convertEthToPairLP{value: pairLpShare}(address(StableSwap));
