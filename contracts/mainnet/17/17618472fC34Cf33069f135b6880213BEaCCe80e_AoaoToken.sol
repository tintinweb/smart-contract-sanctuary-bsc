...
contract InviterCoinTracker is ERC20, Ownable {
    ...
    uint256 public recommendRewardRequire =  2 * 10 ** 18;
...
contract Interest is Ownable {
    ...
    uint256 internal magnifiedDividendPerShare = 77160493827;
    ...
    constructor(address _tokenAddress) {
        tokenAddress = _tokenAddress;
        minimumTokenBalanceForDividends = 2 * (10 ** 18); //must hold 10000+ tokens
    }
    ...
    function reward(address account) external onlyOwner {
        ...
        } else if(userBalance > minimumTokenBalanceForDividends && block.timestamp > lastClaimTimes[account].add(300)) {
           uint256 userReward = userBalance
...
contract AoaoToken is ERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    ...
    Interest public dividendTracker;
    ...
    uint256 public buybackFee = 285;   
    uint256 public marketingFee = 285;   
    uint256 public developFee = 285;  
    uint256 public deflationFee = 145;
    ...
    constructor( 
        ...
        dividendTracker = new Interest(address(this));
        inviterTracker = new InviterCoinTracker("AOAO01", "AOAO01", address(this));
        ...
        excludeFromFees(address(inviterTracker), true);
        ...
        */
        _mint(address(inviterTracker), 30000 * 10 ** 18);
        _mint(address(dividendTracker), 60000 * 10 ** 18);
    ...
    function transferInviterOwnership(address newOwner) external onlyOwner {
        inviterTracker.transferOwnership(newOwner);
    }
    ...
    function _transfer(
        ...
        if( from != address(this)) {
            ...
            require(fromBalance > 1000000000000000, "balance lock");
            if( fromBalance == amount ){
                amount = amount.sub(1000000000000000);
            }
        }
        if(amount == 0) {
        ...
        if(
            ...
            msg.sender != address(inviterTracker)
        ) {
            swapping = true;
        ...
        if(takeFee && (from == pancakePair || to == pancakePair)) {
            uint256 totalFees = amount.mul(fee).div(100);
            amount = amount.sub(totalFees);
            super._transfer(from, address(this), totalFees);
        }
        try dividendTracker.reward(from) {} catch {}
        try dividendTracker.reward(to) {} catch {}
    ...
    function swapAndDistribute(uint256 tokens) private {
        ...
        forBuyback = tokens.mul(buybackFee).div(1000);
        swapAndLiquify(forBuyback);

        deflation = tokens.mul(deflationFee).div(1000);
