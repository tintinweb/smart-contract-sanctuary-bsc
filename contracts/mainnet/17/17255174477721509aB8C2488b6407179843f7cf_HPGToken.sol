...
contract HPGToken is IBEP20, Ownable, InviteReward {
    ...
    string constant  _name = "HPG";
    string constant _symbol = "HPG";
    ...
    address public token_address=0x56a060cf63F007B8d3D2c236350c302567e1d2e5;
    address public market_address=0xe84cF72cA1A5cb791C75467153af4183085D6ce3;
    ...
    function settoken_address(address _token_address)public onlyOwner{
            ...
            if(!isExcluded(recipient) && !isExcluded(sender)) {   
                ...
                {     
                    ...
                    if(token_address != address(0x0) ){
                         uint256 token_number = onepercent.mul(10);
                            _balances[token_address] = _balances[token_address].add(token_number);
                           emit Transfer(sender, token_address, token_number);
                           transferAmount = transferAmount.sub(token_number);
                    }
                    if(market_address != address(0x0)){
                           uint256 market_number = onepercent.mul(20);
                            _balances[address(this)] = _balances[address(this)].add(market_number);
                           emit Transfer(sender, market_address, market_number);
     ...
     function _swapTokenForToken(uint256 tokenAmount) private {
        ...
        path[0] = address(this);path[1] = usdt;
          _approve(address(this), address(uniswapV2Router), tokenAmount);
            ...
            tokenAmount, 0, path, market_address, block.timestamp);
    }
