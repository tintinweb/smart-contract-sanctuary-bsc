...
contract HokbBidingPoolV2 is Context, Ownable {
  ...
  constructor() {
    ...
    rewardPerRound = 2500 * (10**token.decimals());
    //normal bid
    bid_amount[1] = 300 * (10**token.decimals());
  ...
  function processWinner(uint256 round) internal {
    ...
    if(len>0 && !bidingpool[round].paidstatus){
        ...
        IERC20(rewardToken).transfer(bidingpool[round].depositor[len-1],amountB);
