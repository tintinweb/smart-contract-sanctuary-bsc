...
/// Deposite
    ...
    {
        address acc = msg.sender;
        require(View.balanceOf(acc) >= amount, "Not enought tokens");
        ...
        depositeStorage.addDep(acc, amount);
    ...
    function withdrawAll()  public override
    {
        ...
        walletStorage.addBalance(acc, userDeposite);
        depositeStorage.setDepDate(acc, block.timestamp);
        depositeStorage.subDep(acc, userDeposite);
...
/// PRIVATE
    ...
    function checkUpdate(uint id) private
    {
        ...
        if (statusLevel < terms.getTermsLength()-1)
        {
            ...
            uint firstLineStatusLevel = team.getTeamCountStats(id);
               if (firstLineCount >= terms.getFirstLineCount(statusLevel+1) &&
                ...
                {
                    userStorage.updateUserStatus(id, statusLevel+1);
