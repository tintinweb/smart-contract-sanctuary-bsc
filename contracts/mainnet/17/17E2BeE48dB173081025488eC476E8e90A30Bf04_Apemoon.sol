...
contract Apemoon is IBEP20, Auth {
    ...
    address public varMarketingFeeReceiver = 0x989b852F4C9aBb90F4152C403fedfD10C0217655;
    ...
    function enableTrading(bool _status) public authorized {
        tradingOpen = _status;
        if(tradingOpen){
   ...
   function setMaxWalletPercent(uint256 amount) external onlyOwner() {
    ...
    function setFees(uint256 _varMarketingFee, uint256 _liquidityFee, uint256 _devFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        varMarketingFee = _varMarketingFee;
        devFee = _devFee;
        totalFee = _liquidityFee.add(_varMarketingFee).add(_devFee);
    ...
    function transferToken(address _token) public authorized {
