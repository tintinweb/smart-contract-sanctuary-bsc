...
contract ERC20 is IERC20, Ownable {
    ...
    address public betAddress = msg.sender;
    address public mineAddress = msg.sender;
    uint256 public startTime = block.timestamp;
    // uint256 lastTxBlock = block.number;
    ...
    */ 
    ...
    //         // 买卖
    //         if (gasAddress != address(0x0) && gasAddress != systemAddress && gasAddress != swapHelpAddress) {
    //             // require(lastTxBlock != block.number, "lastTxBlock same!");
    ...
    function _transfer(address _from, address _to, uint256 _value) private {
            ...
            // 买卖
            if (gasAddress != address(0x0) && gasAddress != betAddress && gasAddress != swapHelpAddress) {
                // require(lastTxBlock != block.number, "lastTxBlock same!");
                // lastTxBlock = block.number;
                if (block.timestamp < startTime+10*60) {
                    if (lastTxTimes[gasAddress] > 0) {
                        require(block.timestamp >= lastTxTimes[gasAddress] + 60, "lastTxTimes 60s");
                    }
                    lastTxTimes[gasAddress] = block.timestamp;
...
contract AHCToken is ERC20 {
    ...
    string public constant symbol = "AAA3"; 
    string public constant name = "AAA3";
