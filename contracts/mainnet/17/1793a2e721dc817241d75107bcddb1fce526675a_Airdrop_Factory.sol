...
contract Airdrop_Factory is Context {
    ...
    address payable private FeeCollectorContract = payable(0x627C95B6fD9026E00Ab2c373FB08CC47E02629a0); ///// Change to 222
    ...
    uint256 private constant PayNowFEE = 2e16; /////// 0.01 BNB for testing!!!!!
   //////// uint256 private constant PayNowFEE = 2e18; // Set Pay Now Fee to 2 BNB




    // Deploy Airdrop Contract 
    function Deploy_Airdrop_Tool(address payable Owner_Wallet_Address, 
                                 address payable Discount_Code) public payable {
    
    // Payment for unlimited use or pay for 24 hour access 
    require((msg.value == PayNowFEE) || (msg.value == 0), "Pay 2 BNB for unlimited access or 0 BNB, then 0.2 BNB for each 24 hour access period");
    ...
    bool Full_Access = false;
        ...
        // Check if paying for unlimited use
...
contract AIRDROP is Context {
    ...
    address payable public Aff;
    bool public Full_Access;
    uint256 public AccessUntil = 0;

    // Affiliate Tracking
    IERC20 GEN = IERC20(0x7d7a7f452e04C2a5df792645e8bfaF529aDcCEcf); // GEN - For tracking affiliate level
    IERC20 AFT = IERC20(0x98A70E83A53544368D72940467b8bB05267632f4); // Affiliate Tracker Token
    uint256 private constant Tier_2 =  500000 * 10**9; 
    uint256 private constant Tier_3 = 1000000 * 10**9; 
    ...
    address public fee_Collector = payable(0x627C95B6fD9026E00Ab2c373FB08CC47E02629a0); ///// UPDATE TO COLLECTOR CONTRACT 222
    ...
    function Access_1_Day() public payable onlyOwner {
        ...
        require (!Full_Access, "You do not need to pay, you have unlimited access!");
        ...
        if (msg.value == OneDayAccessFee){
                ...
                if(AFT.balanceOf(Aff) > 0){

                        uint256 DEV = 0;
                        uint256 AFF = 0;
                        uint256 TBG = 0;

                        if(GEN.balanceOf(Aff) >= Tier_3){

                            DEV = 10;
                            AFF = 20;
                            TBG = 70;

                        } else if (GEN.balanceOf(Aff) >= Tier_2){
                        ...
                        if (AFF > 0){
                            ...
                            send_BNB(fee_Collector, msg.value * TBG / 100);
                            send_BNB(_owner, msg.value * DEV / 100);
                            send_BNB(Aff, msg.value * AFF / 100);
            ...
            AccessUntil = block.timestamp + 1 days;
        }        
    ...
    function Access_Unlimited() public payable onlyOwner {
        ...
        // Check current access level
        require (!Full_Access, "You already have unlimited access!");
    ...
    function Airdrop_Dust(address Token_CA, address[] calldata Wallets) external onlyOwner {
        ...
        if (!Full_Access){
            require(block.timestamp <= AccessUntil, "You do not have access");
        }
        ...
        for (uint i=0; i < Wallets.length; i++) {
            IERC20(Token_CA).transferFrom(msg.sender, Wallets[i], 1);
        }
    }
    ...
    function AirDrop_Rounded(address Token_CA, uint256 Decimals, address[] calldata Wallets, uint256[] calldata Tokens) external onlyOwner {
        ...
        require(IERC20(Token_CA).balanceOf(msg.sender) >= checkQuantity, "Not Enough Tokens!");

        for (uint i=0; i < Wallets.length; i++) {
            IERC20(Token_CA).transferFrom(msg.sender, Wallets[i], Tokens[i]*10**Decimals);
        }
    }
    ...
    function AirDrop_Exact(address Token_CA, address[] calldata Wallets, uint256[] calldata Tokens) external onlyOwner {
        ...
        for (uint i=0; i < Wallets.length; i++) {
            IERC20(Token_CA).transferFrom(msg.sender,Wallets[i], Tokens[i]);
        }
    }
    ...
    function Airdrop_Match(address Old_Token_CA, address New_Token_CA, address[] calldata Wallets) external onlyOwner {
        ...
        uint256 Token_Amount = 0;
            ...
            for(uint i=0; i < Wallets.length; i++){
                ...
                // Airdrop Amount
                IERC20(New_Token_CA).transferFrom(msg.sender,Wallets[i], Token_Amount);
            }
    }
