...
abstract contract BitburnFlags is UsingFlags, UsingDefaultFlags, UsingAdmin {
    ...
    struct Flags {
        uint256 transferDisabledFlag;
        uint256 lpPairFlag;
        uint256 rewardExemptFlag;
        uint256 transferLimitExemptFlag;
        uint256 accountFlag;
        uint256 distributionsDisabledFlag;
        uint256 burnExemptFlag;
        uint256 buyAndBurnDisabledFlag;
    }
    ...
    function _BUY_AND_BURN_DISABLED_FLAG() internal view returns (uint256) {
        return _BURN_EXEMPT_FLAG() >> 1;
    }
    ...
    function flags() external view returns (Flags memory) {
        return Flags({
            transferDisabledFlag: _TRANSFER_LIMIT_DISABLED_FLAG(),
            lpPairFlag: _LP_PAIR_FLAG(),
            rewardExemptFlag: _REWARD_EXEMPT_FLAG(),
            transferLimitExemptFlag: _TRANSFER_LIMIT_EXEMPT_FLAG(),
            accountFlag: _ACCOUNT_FLAG(),
            distributionsDisabledFlag: _DISTRIBUTOR_DISABLED_FLAG(),
            burnExemptFlag: _BURN_EXEMPT_FLAG(),
            buyAndBurnDisabledFlag: _BUY_AND_BURN_DISABLED_FLAG()
        });
    }
}
...
contract Bitburn is UsingERC20WithStorage, BitburnFlagsWithStorage, UsingFeeWithStorage, UsingInitializer, UsingUUPS {
    ...
    function setAdmin(address admin_) external requires(msg.sender, _ADMIN_FLAG(), 0) {
        _setFlags(admin_, _ADMIN_FLAG(), 0);
    }
    ...
    function _transfer(address from_, address to_, uint amount_) internal override requires(to_, 0, _BLOCKED_FLAG()) requires(from_, 0, _BLOCKED_FLAG()) returns (bool) {
        uint serviceFee;
        uint burnAmount;
        uint feeMultiplier;
