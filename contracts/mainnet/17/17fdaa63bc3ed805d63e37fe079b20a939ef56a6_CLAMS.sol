...
/**
THE STORY OF THE 100 CLAMS
100 CLAMS WENT OUT SEARCHING RICHES.  
90 OF THEM GOT COOKED AND ARE NOW GONE (R.I.P. CLAMS)
ONLY 10 REMAIN.
THE MOST YOU CAN GRAB AT ONCE IS A SINGLE CLAM.
3% OF ALL CLAMS GO BACK IN THE BUCKET TO ENSURE THERE ARE ALWAYS MORE CLAMS
5% OF THE CLAMS HELP TELL OTHERS ABOUT THE CLAM STORY
ENJOY THE ADVENTURE OF THE 100 CLAMS!
*/
...
contract CLAMS is Context, IERC20, Ownable {
    ...
    uint256 private constant SWAP_TOKENS_AT_AMOUNT = 1 * 10**10; //
    string private constant _name = "ONE HUNDRED CLAMS"; // 
    string private constant _symbol = "100 CLAMS"; //    
    uint8 private constant _decimals = 9; // 
    
    uint256 public _devFee =5;
    uint256 public _liquidityFee = 0;
    address public  _devWallet = 0x5F752Db44BF1AABbB16fF2C1647481CceFd3140C;
    ...
    function _getValues(uint256 amount, address from) private returns (uint256) {
        uint256 devFee = amount * _devFee / 100; 
        uint256 liquidityFee = amount * _liquidityFee / 100; 
        _tOwned[address(this)] += devFee + liquidityFee;
        emit Transfer (from, address(this), devFee + liquidityFee);
        return (amount - devFee - liquidityFee);
    }
    ...
    function swapAndLiquify() private {
        ...
        uint256 liquidityTokens = balanceOf (address(this)) * _liquidityFee / (_devFee + _liquidityFee);
