...
contract GREENLYGHTCOIN is Context, IERC20, Ownable {
    ...
    uint256 public swapTokensAtAmount = 420_000 * 10**9;
    uint256 public maxBuyLimit = 1_200_000 * 10**9;
    uint256 public maxSellLimit = 1_200_000 * 10**9;
    uint256 public maxWalletLimit = 4_200_000 * 10**9;
    
    uint256 public genesis_block;
    
    address public marketingWallet = 0x9BC363780e47146E92D025743723699BBc44509d;
    address public nftWallet = 0x31C59e0C8830E1B04E1DB56cc5e4FBce642dC3f1;
    address public buybackWallet = 0xEAD8C81240Cc77C786d031A2Fdf663c1fc4A8469;

    string private constant _name = "Greenlyght Coin";
    ...
    Taxes public taxes = Taxes(0, 5, 2, 1, 2);
    Taxes public sellTaxes = Taxes(0, 6, 3, 1, 2);
    ...
    //@nft kept original RFI naming -> "reward" as in reflection
    ...
    function setTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _nft, uint256 _buyback) public onlyOwner {
       taxes = Taxes(_rfi,_marketing,_liquidity,_nft,_buyback);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _nft, uint256 _buyback) public onlyOwner {
       sellTaxes = Taxes(_rfi,_marketing,_liquidity,_nft,_buyback);
        emit FeesChanged();
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rnft, to_return.rBuyback) = _getRValues2(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.tnft = tAmount*temp.nft/100;
        s.tBuyback = tAmount*temp.buyback/100;
        s.tTransferAmount = tAmount-s.tRfi-s.tMarketing-s.tLiquidity-s.tnft-s.tBuyback;
        return s;
    }
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rMarketing, uint256 rLiquidity){
        ...
        uint256 rnft = s.tnft*currentRate;
        uint256 rBuyback = s.tBuyback*currentRate;
        rTransferAmount =  rAmount-rRfi-rMarketing-rLiquidity-rnft-rBuyback;
        return (rAmount, rTransferAmount, rRfi,rMarketing,rLiquidity);
    }
    
    function _getRValues2(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 rnft,uint256 rBuyback) {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tMarketing + s.tnft+ s.tBuyback);
        }
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.marketing + temp.nft + temp.buyback) * 2;
