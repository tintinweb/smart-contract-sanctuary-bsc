...
interface AnyGamieJourney {
    /**
     * @dev Returns the Journey.
     */
    function getJourney(address addr,uint256 index)
        external
        view
        returns (
            uint256,
...
contract AnyGamieUserJourney is Context, AnyGamieJourney, Ownable {
    ...
    mapping(address => Journey[]) private _journey;
    ...
    {
        if (_journey[addr].length == 0 ) {
            return (0, addr, 0, 0, 0, 0, 0, 0);
        } else {
            Journey storage result = _journey[addr][index];
            return (
                _journey[addr].length,
                result.from,
                result.pointAdd,
                result.pointUsed,
