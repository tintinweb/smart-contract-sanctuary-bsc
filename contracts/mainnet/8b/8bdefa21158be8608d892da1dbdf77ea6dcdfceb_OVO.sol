...
contract DividendTracker is DividendPayingToken, Ownable {
    ...
    uint256 immutable public minimumTokenBalanceForDividends;
...
contract OVO is ERC20, Ownable {
    ...
    uint256 public maxTransactionAmount = 700000 * (10**18);
    uint256 public swapTokensAtAmount = 70000 * (10**18);
    uint256 public maxWalletToken = 840000 * (10**18);

    uint256 public rewardFee = 5;
    uint256 public liquidityFee = 3;
    uint256 public devFee = 2;
    uint256 public totalFees = rewardFee.add(liquidityFee).add(devFee);

    address payable public  devWallet = payable(0xf5ed73b17aB413128DA099Be800D0D4aC0B86cDb);
    ...
    constructor() ERC20("Vigor Loop", "OVO") {
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function _transfer(
        ...
        if(automatedMarketMakerPairs[from] || automatedMarketMakerPairs[to] && !excludedAccount){
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        // token share for liquidity, Dev and redistribution
        uint256 tokensForLiquidity = contractTokenBalance.mul(liquidityFee).div(totalFees);
        uint256 tokensForDev = contractTokenBalance.mul(devFee).div(totalFees);
        uint256 tokensForHolders = contractTokenBalance.sub(tokensForLiquidity.add(tokensForDev));
        ...
        swapTokensForEth(tokensForDev, address(this));
    ...
    function setMaxWalletTokend(uint256 _maxToken) external onlyOwner {
        require(_maxToken*10**18 >= totalSupply().div(1000), "wallet limit must be greater than or equal to 0.1% of total supply");
  	    maxWalletToken = _maxToken * (10**18);
  	}

    function updateFees(uint256 _rewardFee, uint256 _liquidityFee, uint256 _devFee) public onlyOwner {
        require(_rewardFee.add(_liquidityFee).add(_devFee) <= 25, "total tax must not be greater than 25%");
        rewardFee = _rewardFee;
        liquidityFee = _liquidityFee;
        devFee = _devFee;
        totalFees = rewardFee.add(liquidityFee).add(devFee);
    }

    function setMaxTxAmount(uint256 _maxTxAmount) public onlyOwner {
        require(_maxTxAmount*10**18 >= totalSupply().div(1000), "Transaction amount must be greater than or equal to 0.1% of total supply");
        maxTransactionAmount = _maxTxAmount * (10**18);
    }
