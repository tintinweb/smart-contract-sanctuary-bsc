...
contract DecubateCrowdfunding is IDecubateCrowdfunding, Ownable, ReentrancyGuard {
  ...
  /**
   *
   * @dev Mapping value of ticket price per tier
   *
   */
  mapping(uint8 => uint256) public ticketValue;
   ...
   * @param {_multi} multiplier of the user
   *
  ...
  function setAllocation(
    ...
    userAllocation[_address].tier = _tier;
    userAllocation[_address].multiplier = _multi;
    return true;
  }
  ...
  /**
   *
   * @dev set ticket values for each tier
   *
   * @param {uint8[]} tier value
   * @param {uint256[]} ticket value
   *
   * @return {bool} status of update
   *
   */

  function setTicketValues(uint8[] calldata _tiers, uint256[] calldata _ticketPrice)
    external
    onlyOwner
    returns (bool)
  {
    uint256 len = _tiers.length;
    require(len == _ticketPrice.length, "Invalid input");

    for (uint8 i = 0; i < len; i++) {
      ticketValue[_tiers[i]] = _ticketPrice[i];
    }
  ...
  function fundAgreement(uint256 _investFund) external override nonReentrant returns (bool) {
    ...
    // Allocation of user
    uint256 alloc = ticketValue[user.tier].mul(user.multiplier);
