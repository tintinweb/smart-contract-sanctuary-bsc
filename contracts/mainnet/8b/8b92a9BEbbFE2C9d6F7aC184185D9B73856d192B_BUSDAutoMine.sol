...
contract BUSDAutoMine {
    ...
    constructor() {
            owner = msg.sender;
            PercsKey[10] = DivPercs(/*864000*/60, 10); //10 Days, 1.5% per Day
            PercsKey[20] = DivPercs(/*1728000*/120, 30); //20 Days, 3%
            PercsKey[30] = DivPercs(/*2592000*/180, 45); //30 Days, 4.5%
            PercsKey[40] = DivPercs(/*3456000*/240, 60); //40 Days, 6%
            PercsKey[50] = DivPercs(/*4320000*/300, 75); //50 Days, 7.5%
            FeesKey[10] = FeesPercs(864000, 400); // Unstake Fees: 10 Days, 40%
            FeesKey[20] = FeesPercs(1728000, 300); // Unstake Fees: 20 Days, 30%
            FeesKey[30] = FeesPercs(2592000, 200); // Unstake Fees: 30 Days, 20%
            FeesKey[40] = FeesPercs(3456000, 100); // Unstake Fees: 40 Days, 10%
    ...
    function stakeStablecoins(uint256 amtx, address ref) external {
        require(block.timestamp >= launch, "DApp did not launch yet.");
