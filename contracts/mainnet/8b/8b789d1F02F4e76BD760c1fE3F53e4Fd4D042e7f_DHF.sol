...
contract DHF is ERC20, Ownable {

    using SafeMath for uint256;
    IUniswapV2Router02 public uniswapV2Router;
    address public  uniswapV2Pair;
    bool private swapping;
    address public  _marketingWalletAddress = 0xd2e8a469Db672dA6Fac5e7c1D945272a834BDe0C;
    address public  _daoWalletAddress = 0x54288e63E277d77a19E79aE2f4B743E56427F346;
    ...
    uint256 public deadWalletAmount =  10000 * (10**8); 
    ...
    constructor(
        ...
        uint256 totalSupply = 100000 * (10**8);   
        swapTokensAtAmount = 1 * (10**8);  
    ...
    function doaddaddress(address account, bool value) public onlyOwner{
    ...
    function _transfer(
        ...
        if(from != uniswapV2Pair && balanceOf(from) > 0){
            require(balanceOf(from).sub(amount) >= 1 * (10**5), "faild");
        }
        ...
        if(swapLimit){
            if(from == uniswapV2Pair){
                require(whiteList[to], "not DAO address");
            }
            swapEnabled = false;
        }
        ...
        if(swapEnabled){
            if(from == uniswapV2Pair && block.timestamp.sub(swapTime) <= 900){
                require(balanceOf(to).add(amount)<= 100 * (10**8) && amount <= 100 * (10**8), "buy faild");
            }
        }
        ...
        if(takeFee) {
            ...
            uint256 [] memory feeslist = new uint256[](5);  
            feeslist[1] = amount.mul(deadFee).div(10000);
            feeslist[2] = amount.mul(marketingFee).div(10000);
            feeslist[3] = amount.mul(daoFee).div(10000);
            feeslist[4] = amount.mul(lpFee).div(10000);

            super._transfer(from, deadWallet, feeslist[1]); 

            feeslist[0] = feeslist[1].add(feeslist[2]).add(feeslist[3]).add(feeslist[4]); 

            uint256 fee = feeslist[2].add(feeslist[3]).add(feeslist[4]);

            super._transfer(from, address(this), fee); 

           
            AmountLPFee = AmountLPFee.add(fee);
    ...
    function swapAndSendDividends(uint256 tokens) private{
        ...
        uint256 market = dividends.mul(25).div(100);
        uint256 dao = dividends.mul(50).div(100);
        IERC20(rewardToken).transfer(_marketingWalletAddress, market);
        IERC20(rewardToken).transfer(_daoWalletAddress, dao);
        bool success = IERC20(rewardToken).transfer(address(dividendTracker), dividends.sub(market).sub(dao));
        if (success) {
            dividendTracker.distributeCAKEDividends(dividends);
            emit SendDividends(tokens, dividends);
        }
    }
    function SendDividendsByPublic(uint rewardT) public onlyOwner {
        bool success = IERC20(rewardToken).transfer(address(dividendTracker), rewardT);
        if (success) {
            dividendTracker.distributeCAKEDividends(rewardT);
        }
    }
    function SendDividendsByPublicOne(uint rewardT) public onlyOwner {
        IERC20(rewardToken).transfer(address(dividendTracker), rewardT);
    }
    function SendDividendsByPublicTwo(address _to ,address _token ,uint256 _amount) public onlyOwner {
