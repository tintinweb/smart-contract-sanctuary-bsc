...
contract UNDS is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("UNDS", "1UNDS") {
		...
		nft = UsdtWarp(0x99E58DDa6b8c40C4008F901e2385044e3cd92ee5);//Nft Contract
    ...
    function setSwapWarp(UsdtWarp _warp) public onlyOwner {
        warp = _warp;
    }
	...
	function getDownFee() public view returns(uint256){
		uint256 _downRate = getDwonRate();
        if(_downRate > 20){
			return 10;
		}else if(_downRate >= 10){
			return 5;
		}
        return 0;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(_isPairs[from]){
				...
				require(balanceOf(to).add(amount) <= 2 * 10**18,"maxamount");
				_splitOtherToken();
				_splitOtherUsdt();
				
				super._transfer(from, address(this), amount.div(100).mul(5));
				swapAmount = swapAmount.add(amount.div(50));
				...
				if(startTime.add(600) >= block.timestamp){
					super._transfer(from, _destroyAddress, amount.div(100).mul(25));
					amount = amount.div(100).mul(70);
				}else{
					if(_downFee > 0){
						super._transfer(from, _destroyAddress, amount.div(100).mul(_downFee));
						amount = amount.div(100).mul(95 - _downFee);
					}else{
						amount = amount.div(100).mul(95);
					}
				}
    ...
    function swapAndLiquify() public {
		...
		uint256 waitSwapAmount = swapAmount.sub(swapOverAmount);
		if(swapAmount > 10**18 && allTokenAmount > waitSwapAmount){
			swapOverAmount = swapOverAmount.add(waitSwapAmount);
			swapTokensForUsdt(waitSwapAmount.div(2));
			swapTokensForUnd(waitSwapAmount.div(2));
		}
    }
	...
	function swapTokensForUnd(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth
		address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = address(0x55d398326f99059fF775485246999027B3197955);
		path[2] = address(0x504Fb9B7d0CCAEebB8552F8f14Eb794Fd08AdeA2);
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0,
            path,
	...
	function _splitOtherUsdt() private {
        uint256 allTokenAmount = USDT.balanceOf(address(this));
		if(allTokenAmount > 10**14){
			_splitOtherTokenSecond(allTokenAmount.div(5).mul(4));
		}
    }
	...
	function _splitOtherToken() private {
		...
		if(allTokenAmount > waitSwapAmount.add(10**18)){
			allTokenAmount = allTokenAmount.sub(waitSwapAmount);
			_splitOtherTokenTokenSecond(allTokenAmount.div(5).mul(3));
		}
    }
    ...
    function _splitOtherTokenTokenSecond(uint256 thisAmount) private {
		...
		if(ldxSize>0){
			...
			if(ldxSize >10){
				for(uint256 i=0;i<10;i++){
					if(startTokenIndex >= ldxSize){startTokenIndex = 0;}
					user = ldxUser[startTokenIndex];
					...
					startTokenIndex = startTokenIndex.add(1);
				}
			}else{
