...
contract TRUV is Context, IERC20, Ownable {
  ...
  //anti-whale
  uint256 public _maxTxAmount = 12000000 * 10**18;
  uint256 public _maxWalletSize = 60000000 * 10 ** 18;
  ...
  constructor(uint256 _snipeBlockAmt) {
    ...
    _isExcludedAntiwhale[owner()] = true;
  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if(!_isExcludedAntiwhale[sender] || !_isExcludedAntiwhale[recipient] && recipient != routerAddress){
  ...
  function _checkLiquidityAdd() public onlyOwner {
  ...
  function setMaxWalletSizePercent(uint256 maxWalletPercent) external onlyOwner() {
    _maxWalletSize = _totalSupply.mul(maxWalletPercent).div(
    10**2 );
  }

  function excludeAntiwhale(address account) public onlyOwner {
   _isExcludedAntiwhale[account] = true;
  }

  function IncludeAntiwhale(address account) public onlyOwner {
   _isExcludedAntiwhale[account] = false;
  }
