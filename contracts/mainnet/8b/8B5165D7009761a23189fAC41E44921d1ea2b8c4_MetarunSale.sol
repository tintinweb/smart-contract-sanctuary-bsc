...
contract MetarunSale is AccessControlUpgradeable {
    ...
    function pancakeRate(uint256 amountIn) public view returns (uint256 rate) {
        address[] memory path = new address[](2);
        path[0] = address(BUSD);
        path[1] = address(token);

        uint256[] memory pancake = router.getAmountsOut(amountIn, path);
        return pancake[1];
    }
    ...
    function buy(uint256 kind) external {
        ...
        uint256 price = pancakeRate(characters[kind].price);
