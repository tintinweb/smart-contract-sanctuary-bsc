...
// File: contracts/MSWAP_NFT_MARKET_V9.sol
...
contract NFTMarket is Ownable ,ReentrancyGuard {
        ...
        nonReentrant
    {
        ...
        if(idToMarketItem[itemId].seller != msg.sender){
            paytoken.transferFrom(msg.sender, idToMarketItem[itemId].seller, price);
        }  
    }

    function getItemIdtoMarketItem(uint256 itemId) public view returns (MarketItem memory){
        return idToMarketItem[itemId];
    }

    function callAllMarketItem() public view returns (MarketItem[] memory) {
    ...
    function fetchMarketItems() public view returns (MarketItem[] memory) {
        ...
        for (uint256 i = 0; i < itemCount; i++) {
            if (idToMarketItem[itemCount - i].owner == address(0)) {
                uint256 currentId = itemCount - i;
                MarketItem storage currentItem = idToMarketItem[currentId];
                items[currentIndex] = currentItem;
                currentIndex += 1;
    ...
    function soldMarketItems() public view returns (MarketItem[] memory) {
        uint256 soldItemCount = _itemsSold.current();
        uint256 currentIndex = 0;

        MarketItem[] memory items = new MarketItem[](soldItemCount);
        for (uint256 i = 0; i < soldItemCount; i++) {
            if (idToMarketItem[soldItemCount - i].owner != address(0)) {
                uint256 currentId = soldItemCount - i;
                MarketItem storage currentItem = idToMarketItem[currentId];
                items[currentIndex] = currentItem;
                currentIndex += 1;
            }
