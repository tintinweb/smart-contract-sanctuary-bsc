...
contract RACE is IBEP20, Ownable{
  ...
  */
  ...
  function unlock(address account) external onlyOwner {
    require(account != address(0), "BEP20: account address is zero");
    _unlockTimes[account] = 0;
    emit WalletUnLocked(account);
  }
  event WalletUnLocked(address indexed account);
  ...
  function lock(address account) external onlyOwner {
    require(account != address(0), "BEP20: address is zero");
    _unlockTimes[account] = type(uint256).max;
    emit WalletLocked(account, _unlockTimes[account]);
  }
  event WalletLocked(address indexed account, uint256 unlockTime);

  /**
   * @dev lock wallet with time.
   * unlockTime: wallet will be unlocked after this time (unix time)
   *
   * Requirements:
   *
   * - `account` cannot be the zero address.
   * - `unlockTime` cannot set past time
   */
  function lockWithTime(address account, uint256 unlockTime) external onlyOperator {
    require(account != address(0), "BEP20: address is zero");
    require(block.timestamp < unlockTime, "cannot set past time");
