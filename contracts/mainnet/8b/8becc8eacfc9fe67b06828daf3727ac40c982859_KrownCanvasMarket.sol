...
/** 
 *  SourceUnit: d:\GitWork\01-CourtlandWorking\KrownCanvasNFT\contracts\contracts\KrownCanvasMarket.sol
*/
...
contract KrownCanvasMarket is Ownable, ERC1155Holder {
	...
	function setDevFee(uint256 _feeDev, 
		address _devAddress) external {	
		require(devAddress == msg.sender, "caller is not the dev");			
        feeDev = _feeDev;
		devAddress = _devAddress;		
    }

    function list(uint256 _token_id, uint256 _price, address _tokenAdr, uint256 _amount) public {
    ...
    function buy(uint256 _id, uint256 _amount) external {
		...
		if (feeDev > 0) {
			require(governanceToken.transferFrom(msg.sender, devAddress, totalAmount.mul(feeDev).div(PERCENTS_DIVIDER)), "failed to transfer dev fee");
		}
		...
		uint256 ownerPercent = PERCENTS_DIVIDER.sub(feeAdmin).sub(feeDev).sub(pair.creatorFee);
