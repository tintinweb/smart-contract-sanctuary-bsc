...
contract NeverForget is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    NeverForgetDividendTracker public dividendTracker;
    ...
    constructor() ERC20("NeverForget", "9/11") {
        charityWallet = payable(0x5D32Ff1c93FA73A303512E3818163619629699df);
        marketingWallet = payable(0x02726F316721D47d76837f675C27AEE26f3Cac01);
        ...
        dividendTracker = new NeverForgetDividendTracker(
            payable(this),
            router,
        ...
        _isExcludedFromFees[address(charityWallet)] = true;
    ...
    function updateTransferFee(uint256 newTransferFee) public onlyOwner {
        require (newTransferFee <= 12, "transfer fee cannot exceed 5%");
    ...
    function updateFees(
        ...
        buymarketingFee = marketingBuy;
        sellmarketingFee = marketingSell;
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            uint256 totalFees = RewardsFee
                .add(liquidityFee + charityFees + marketingFees);
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        if (buyRewardsFee > 0) {
            ...
            .div(totalAmount)
            .mul(buyRewardsFee)
            .div(buyRewardsFee + buycharityFees + buymarketingFee);
        }
        if (sellRewardsFee > 0) {
            ...
            .div(totalAmount)
            .mul(sellRewardsFee)
            .div(sellRewardsFee + sellcharityFees + sellmarketingFee);
        }
        ...
        uint256 _completeFees = sellcharityFees.add(sellmarketingFee) +
            buycharityFees.add(buymarketingFee);
        ...
        if (charityPayout > 0) {
            (successOp1, ) = address(charityWallet).call{value: charityPayout}("");
        }
        if (marketingPayout > 0) {
            (successOp2, ) = address(marketingWallet).call{value: marketingPayout}("");
        }

        emit SendDividends(
            dividends,
            charityPayout + marketingPayout,
...
contract NeverForgetDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    NeverForget public NeverForgetContract;
    ...
    constructor(
        ...
        defaultToken = token;
        NeverForgetContract = NeverForget(mainContract);
    ...
    function _transfer(
        ...
        require(false, "NeverForget_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "NeverForget_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main NeverForget contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(NeverForgetContract);

            uint256 prevBalance = NeverForgetContract.balanceOf(address(this));
            ...
            {
                uint256 received = NeverForgetContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    NeverForgetContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
