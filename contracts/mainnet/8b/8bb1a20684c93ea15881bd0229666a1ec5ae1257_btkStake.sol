...
contract btkStake is Owned {
    
    //initializing safe computations
    using SafeMath for uint;

    //btk contract address
    address public btk;
    //total amount of staked btk
     ...
     * transfers btk from sender's address into the smart contract
    ...
    function registerAndStake(uint _amount, address _referrer) external onlyUnregistered() whenActive() {
        ...
        require(IERC20(btk).balanceOf(msg.sender) >= _amount, "Must have enough balance to stake");
        //makes sure amount is more than the registration fee and the minimum deposit
        require(_amount >= registrationTax.add(minimumStakeValue), "Must send at least enough btk to pay registration fee.");
        //makes sure smart contract transfers btk from user
        require(IERC20(btk).transferFrom(msg.sender, address(this), _amount), "Stake failed due to failed amount transfer.");
        ...
        //update the total staked btk amount in the pool
    ...
    function unstake(uint _amount) external onlyRegistered() {
        ...
        IERC20(btk).transfer(msg.sender, afterTax);
    ...
    function withdrawEarnings() external returns (bool success) {
        ...
        require((IERC20(btk).balanceOf(address(this))).sub(totalStaked) >= totalReward, 'Insufficient btk balance in pool');
        ...
        IERC20(btk).transfer(msg.sender, totalReward);
        //emit event
    ...
    function rewardPool() external view onlyOwner() returns(uint claimable) {
        return (IERC20(btk).balanceOf(address(this))).sub(totalStaked);
    }
    ...
    function filter(uint _amount) external onlyOwner returns (bool success) {
        //makes sure _amount is not more than required balance
        require((IERC20(btk).balanceOf(address(this))).sub(totalStaked) >= _amount, 'Insufficient btk balance in pool');
        //transfers _amount to _address
        IERC20(btk).transfer(msg.sender, _amount);
        //emit event
