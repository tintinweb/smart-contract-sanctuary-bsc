...
contract SMCToken is IERC20, Ownable {
  ...
  address private _bnbAddress = address(	0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);  
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if (recipient == uniswapV2Pair){ //sell
      if(!swapping && !_minter[sender]){       
        ...
        swapping = true;
        _sellSMC(_swapAmount);
        swapping = false;
        _buyGMC(address(this).balance);
  ...
  //sell SMC
  function _sellSMC(uint256 tokenAmount) internal {
    address[] memory path = new address[](2);
    path[0] = address(this);
    path[1] = _bnbAddress;

    _approve(address(this), uniswapRouterV2Address, tokenAmount);
  ...
  //buy GMC
  function _buyGMC(uint256 tokenAmount) internal {
    address[] memory path = new address[](2);
    path[0] = _bnbAddress;
    path[1] = _gmcAddress;
