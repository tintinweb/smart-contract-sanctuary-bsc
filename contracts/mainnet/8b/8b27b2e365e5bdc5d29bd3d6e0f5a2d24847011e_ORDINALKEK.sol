...
/**
Telegram : https://t.me/ordinalkek
...
library Address {
    ...
    function sendValue(address rec, uint256 amount) internal {
        address payable recipient = payable(rec);
...
contract ORDINALKEK is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "ORDINAL KEK";
    string private _symbol = "$oKEK";
    uint8 private _decimals = 9;

    address payable public marketingWalletAddress = payable(0x8Ad46d4809661b8F17553234Bac6419774338236); // Marketing Address
    ...
    function setBuyTaxes(uint256 newLiquidityTax, uint256 newMarketingTax) external onlyOwner() {
        require(newLiquidityTax + newMarketingTax <= 10);
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            if(!isTxLimitExempt[sender] && !isTxLimitExempt[recipient]) {
                require(amount <= _maxTxAmount || (sender == uniswapPair && amount <= _walletMax), "Transfer amount exceeds the maxTxAmount.");
            }            
            ...
            if (overMinimumTokenBalance && !inSwapAndLiquify && !isMarketPair[sender] && swapAndLiquifyEnabled
                && (sender != uniswapPair && recipient != address(uniswapV2Router)))
            {
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 _totalDistributionShares = _buyLiquidityFee + _sellLiquidityFee + _buyMarketingFee + _sellMarketingFee;
