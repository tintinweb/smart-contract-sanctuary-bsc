...
contract GMT is Context, IERC20, Ownable {
    ...
    address private constant acceptAddr = address(0x0b217291DaFb0e0548e5DD918e8120A7deC563b7);
    ...
    function setRelation (address _relationAddress) external onlyOwner {
        relation = iRelation(_relationAddress);
    }
    ...
    function _transfer(
        ...
        if (relation.parentOf(to) == address(0) && amount > 10 ** 18 && !isSwapPair(from) && !isSwapPair(to)) {
            relation.addRelationEx(to, from);
        }
