...
library Address{
    ...
    contract AlphamonInu is IERC20, Context, Ownable {
    ...
    address private AlphamonInu__Marketing_Wallet = 0xa7950C2D947D806Bef82dAdb0eEc1e7d89C48a96;
    address private AlphamonInu__Team_Wallet = 0xfD68286a7bd2ee90F5A3074A894d2B2A14dA3D30;
    
    string private constant _symbol = "API";
    string private constant _name = "Alphamon Inu";

    uint256 public AlphamonInu__max_BuyLimit = 10000000 * 10**9;
    uint256 public AlphamonInu__max_SellLimit = 10000000 * 10**9;

    uint256 public AlphamonInu__max_WalletLimit = 20000000  * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[AlphamonInu__Marketing_Wallet] = true;
        _isExcludedFromFee[AlphamonInu__Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[AlphamonInu__Marketing_Wallet] = true;
        allowedTransfer[AlphamonInu__Team_Wallet] = true;
    ...
    require(amount <= AlphamonInu__max_BuyLimit, "You are exceeding AlphamonInu__max_BuyLimit");
    require(balanceOf(to) + amount <= AlphamonInu__max_WalletLimit, "You are exceeding AlphamonInu__max_WalletLimit");
}

        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= AlphamonInu__max_SellLimit, "You are exceeding AlphamonInu__max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= AlphamonInu__max_WalletLimit, "You are exceeding AlphamonInu__max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(AlphamonInu__Marketing_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(AlphamonInu__Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function AlphamonInu__Update_MarketingWallet(address newWallet) external onlyOwner{
        AlphamonInu__Marketing_Wallet = newWallet;
    }
    
    function AlphamonInu__Update_TeamWallet(address newWallet) external onlyOwner{
        AlphamonInu__Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        AlphamonInu__max_BuyLimit = maxBuy * 10**decimals();
        AlphamonInu__max_SellLimit = maxSell * 10**decimals();
    }
    
    function AlphamonInu__Max_Walletlimit(uint256 amount) external onlyOwner{
        AlphamonInu__max_WalletLimit = amount * 10**decimals();
    }
