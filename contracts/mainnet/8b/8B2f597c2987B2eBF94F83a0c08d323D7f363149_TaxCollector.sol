...
contract TaxCollector is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    function _sendAmount() private onlyWhitelisted {
        ...
        if (collectedBuyTax < collectedSellTax) {
            uint256 percent = (collectedBuyTax / collectedSellTax) * 100;
            payable(buyTaxAddress).transfer((address(this).balance * percent) / 100);
            payable(sellTaxAddress).transfer(address(this).balance);
        }
        if (collectedBuyTax > collectedSellTax) {
            uint256 percent = (collectedSellTax / collectedBuyTax) * 100;
            payable(sellTaxAddress).transfer((address(this).balance * percent) / 100);
            payable(buyTaxAddress).transfer(address(this).balance);
        }
        collectedBuyTax = 0;
        collectedSellTax = 0;
    }
