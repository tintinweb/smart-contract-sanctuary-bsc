...
/**
 * @dev We need the interfaces from KazamaSwap (i.e. the KazamaFactory and KazamaRouter interfaces).
 * The interfaces and contracts are originally from Uniswap V2, forked by PCS. KazamaSwap also uses this.
 *
 * Documentation can be found at:
 * - https://docs.uniswap.org/protocol/V2/introduction
 *
 * Note: We need these interfaces because we want to add extra utilities to the token in
 * addition to staking, farming, voting and IFO's (i.e. automatically generated liquidity and automated buybacks).
 *
 * KazamaFactory:
 * - https://bscscan.com/address/0x20aff1549aa99a873937dbef56543542b54860d8#code
 *
 * KazamaRouter:
 * - https://bscscan.com/address/0x0d35916fadf5078ceeb026a928070f903d33cbe3#code
 *
 * Note: While the owner of this contract (i.e the SenshiMaster contract) will mint tokens for
 * stakers and farmers, we will also reduce the totalSupply by performing automated burns on transactions
 * and tokens obtained through the automatic buybacks made by the contract that automatically be burned.
 */
...
interface KazamaRouter {
    /**
     * @dev Returns the factory (i.e KazamaFactory) address.
     */
    function factory() 
...
contract BEP20 is IBEP20, TheZaibatsu {
    ...
    uint256 _totalSupply = 750_000_000 * (10 ** _decimals);
    uint256 LiqGeneratorFee = 3;
    uint256 BuyBackBurnFee = 3;
    ...
    *
    * Note: See line 1029 which shows:
    * - {uint256 percentValue = roundValue.mul(BurnPercentSettings).div(100000); // = 3.75%}
    *
    ...
    * InSwap modifier boolean.
    ...
    constructor (string memory name, string memory symbol) {
        _name = name;
        _symbol = symbol;
        address _KazamaRouter = 0x0d35916fADf5078CEEB026A928070f903d33CbE3;
        ...
        approve(_KazamaRouter, _totalSupply);
    ...
    * @dev If it is decided to increase or decrease the burning percentage.
    ...
    function buyKazama(uint256 amount, address to) internal swapping {
        ...
        uint256 tokensToBurn = burnPercentage(amount);
        uint256 finalBurn = amount.sub(tokensToBurn);

        _totalSupply = _totalSupply.sub(finalBurn);
    ...
    *
    * NOTE: All fees added together can never be set higher than 10% (TotalFee).
    * See {TotalFee < FeeDenominator/10}
     */
    function setFees(uint256 _LiqGeneratorFee, uint256 _BuyBackBurnFee, uint256 _TreasuryFee, uint256 _FeeDenominator) external OnlyJin {
        LiqGeneratorFee = _LiqGeneratorFee;
        BuyBackBurnFee = _BuyBackBurnFee;
        TreasuryFee = _TreasuryFee;
        TotalFee = _LiqGeneratorFee.add(_BuyBackBurnFee).add(_TreasuryFee);
