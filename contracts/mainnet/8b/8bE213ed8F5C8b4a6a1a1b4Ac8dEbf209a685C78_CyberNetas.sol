...
contract CyberNetas is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    address public RewardToken = 0x31a9975f71F53D5cDeB4BF4A48B3857E3827d487;
    address public FeeReceiver = 0x7d260A9bBe18EdCB983233bC762847faE543110C;
    ...
    function mint(uint256 _mintAmount) public payable {
        // Checks
        require(!paused, "CyberNetas: minting has not started yet");
        require(_mintAmount > 0, "CyberNetas: you have to mint at least one");
        require(
            _mintAmount <= maxMintAmount,
            "CyberNetas: you can only mint 10 at a time"
        );
        require(
            lastSupply >= _mintAmount,
            "CyberNetas: the collection is sold out"
        );
        if (msg.sender != owner()) {
            require(
                msg.value >= cost * _mintAmount,
                "CyberNetas: total cost isn't match"
            );
        }
        // Minting
    ...
    {
        // Checks
        require(_winner != address(0), "CyberNetas: zero address");
        require(_amount > 0, "CyberNetas: you have to mint at least one");
        require(lastSupply != 0, "CyberNetas: the collection is sold out");
    ...
    function tokenURI(uint256 _id)
        ...
        returns (string memory)
    {
        require(_exists(_id), "CyberNetas: URI query for nonexistent token");
