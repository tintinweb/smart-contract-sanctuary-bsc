...
contract Shim is Ownable, IERC20, IERC20Metadata {
    ...
    uint256 public constant BUSDRewardFee = 80;
    uint256 public constant sellFee = 20;

    uint256 public totalFee = liquidityFee.add(treasuryFee).add(BUSDRewardFee);
    ...
    function takeFee(
        ...
        for (uint256 i = 0; i < pairs.length; i++) {
            if (recipient == pairs[i]) {
                ...
                _BUSDRewardFee = BUSDRewardFee.add(sellFee);
                break;
            }
        }
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_BUSDRewardFee.add(treasuryFee))
        );
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBUSDForReward = amountBUSDForTreasuryAndReward.mul(
            BUSDRewardFee).div(treasuryFee.add(BUSDRewardFee));
