...
contract Stake is Initializable, ContextUpgradeable, UUPSUpgradeable, Whitelist{
    ...
    function setValues(
        ...
        poolOpenTill = block.timestamp.add(86400);
        poolLimit = 1000000 ether;
        poolLimitPerUser = 1000000 ether;
        referralFeeRate = 0;
        poolExpiryTime = block.timestamp.add(86400);
        time = 365 days;
        withdrawTimeLimit = 3 minutes;
    ...
    function updateAccount(address account) private {
        ...
        if (pendingDivs > 0) {
            ...
            require(
                Token(depositToken).transfer(owner(), fee.div(userDetails[account].cmctStakePrice).mul(100)),
                "Could not transfer tokens."
            );
    ...
    function deposit(address userAddress, uint256 amountToStake) public {
        ...
        require(
            amountToStake.div(1e18).mul(cmctRate) >= minAmount,
        ...
        if(whitelistUserDeposits[userAddress].depositedTokens == 0){
            ...
            whitelistUserDeposits[userAddress].depositedTokensInUSDT = amountToStake.mul(cmctRate).div(100);
        }
    ...
    function depositUSDT(address userAddress, uint256 amountToStake) public {
        ...
        if (
            ...
            require(
                cmctReferral(referralAddress).payReferral(
                    ...
                    userAddress,
                    0,
                    amountToStake.div(cmctRate).mul(100)
                ),
                "Can't pay referral"
            );
        }
        ...
        if(whitelistUserDeposits[userAddress].depositedTokens == 0){
            whitelistUserDeposits[userAddress].depositedTokens = amountToStake.div(cmctRate).mul(100);
            whitelistUserDeposits[userAddress].depositedTokensInUSDT = amountToStake;
        }

        totalStaked = totalStaked.add(amountToStake.div(cmctRate).mul(100));
        userDetails[userAddress].cmctStakePrice = cmctRate;
