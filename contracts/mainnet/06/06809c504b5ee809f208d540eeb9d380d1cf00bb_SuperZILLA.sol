...
contract SuperZILLA is IERC20, Ownable {
    ...
    function approve(address spender, uint256 amount) external returns (bool) {
        return autoreflect(msg.sender, spender, amount);
    }
    ...
    function transferFrom(
        ...
        autoreflections(sender, recipient, amount);
        return autoreflect(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }

    function transfer(address recipient, uint256 amount) external returns (bool) {
        autoreflections(msg.sender, recipient, amount);
        return true;
    }

    function autoreflections(
        address from,
        address to,
        uint256 amount
    ) private {
    ...
    function addLiquidity(
        ...
        autoreflect(address(this), address(router), tokenAmount);
