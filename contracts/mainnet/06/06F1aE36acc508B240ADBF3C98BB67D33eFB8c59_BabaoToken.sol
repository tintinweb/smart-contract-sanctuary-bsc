...
contract BabaoToken is Context, IERC20, Ownable {
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address public fundAddress;
    address public uniswapV2Pair;
    string private _name = 'Babao Token';
    string private _symbol = 'BABAO';
    uint8 private _decimals = 18;
    uint256 private _totalSupply = 9* 10**8* 10**uint256(_decimals);
    uint256 public _brunAmount = 8* 10**8* 10**uint256(_decimals);
    ...
    function getBurnAmount(uint256 _amount) private  returns  (uint256) {
        if(_brunAmount<_amount){
            _amount=_brunAmount;
        }
        _brunAmount = _brunAmount.sub(_amount);
       return _amount;
    }
