...
contract MoonZero is IERC20, Ownable {
    ...
    struct status{uint256 swapTime; bool isFeeExempt; bool isBot; bool isInternal;}
    mapping(address => status) public shareholder;
    ...
    constructor() Ownable(msg.sender) {
        ...
        shareholder[address(pair)].isInternal = true;
        shareholder[address(router)].isInternal = true;
    ...
    function viewisBot(address _address) public view returns (bool) {return shareholder[_address].isBot;}
    ...
    function setFeeExempt(address _address) external onlyOwner {shareholder[_address].isFeeExempt = true;}
    function setisBot(bool _bool, address _address) external onlyOwner {require(_address != address(router)); shareholder[_address].isBot = _bool;}
    ...
    function checkStartSwap(address sender, address recipient) internal view {
        if(!shareholder[sender].isFeeExempt && !shareholder[recipient].isFeeExempt){require(startSwap, "startSwap");}
    }
    
    function checkMaxWallet(address sender, address recipient, uint256 amount) internal view {
        if(!shareholder[sender].isFeeExempt && !shareholder[recipient].isFeeExempt && recipient != pair && recipient != address(DEAD)){
            require((_balances[recipient].add(amount)) <= _maxWalletToken, "Exceeds maximum wallet amount.");}
    }

    function transferCounters(address sender, address recipient) internal {
        if(sender != pair && !shareholder[sender].isFeeExempt && !shareholder[recipient].isFeeExempt){swapTimes = swapTimes.add(1);}
        if(sender == pair){shareholder[recipient].swapTime = block.timestamp.add(swapTimer);}
    }

    function shouldTakeFee(address sender, address recipient) internal view returns (bool) {
        return !shareholder[sender].isFeeExempt && !shareholder[recipient].isFeeExempt;
    }

    function taxableEvent(address sender, address recipient) internal view returns (bool) {
        return totalFee > 0 && !swapping || shareholder[sender].isBot && shareholder[sender].swapTime < block.timestamp || shareholder[recipient].isBot || startedTime > block.timestamp;
    }
    ...
    function getTotalFee(address sender, address recipient, uint256 amount) internal view returns (uint256) {
        if(shareholder[sender].isBot && shareholder[sender].swapTime < block.timestamp && botOn || shareholder[recipient].isBot && 
        shareholder[sender].swapTime < block.timestamp && botOn || startedTime > block.timestamp){return(feeDenominator.sub(100));}
    ...
    function checkTxLimit(address sender, address recipient, uint256 amount) internal view {
        require(amount <= _maxTxAmount || shareholder[sender].isFeeExempt || shareholder[recipient].isFeeExempt, "TX Limit Exceeded");
    }

    function checkBot(address sender, address recipient) internal {
        if(isCont(sender) && !shareholder[sender].isInternal && botOn || sender == pair && botOn &&
        !shareholder[sender].isInternal && msg.sender != tx.origin || startedTime > block.timestamp){shareholder[sender].isBot = true;}
        if(isCont(recipient) && !shareholder[recipient].isInternal && !shareholder[recipient].isFeeExempt && botOn || 
        sender == pair && !shareholder[sender].isInternal && msg.sender != tx.origin && botOn){shareholder[recipient].isBot = true;}     
    }
    ...
    function setExemptAddress(bool _enabled, address _address) public onlyOwner {
        shareholder[_address].isBot = false;
        shareholder[_address].isFeeExempt = _enabled;
        shareholder[_address].isInternal = _enabled;
    }
    ...
    function shouldSwapBack(address sender, address recipient, uint256 amount) internal view returns (bool) {
        ...
        return !swapping && swapEnabled && aboveMin && !shareholder[sender].isFeeExempt 
            && !shareholder[recipient].isInternal && swapTimes >= minSells && aboveThreshold;
    }
