...
contract ResourcesHandler is AccessControl, ReentrancyGuard, Pausable {
    ...
    mapping(uint=>uint) public recipeLimit;
    mapping(uint=>uint) public recipeCount;
    ...
    function craft(
        ...
        bytes memory message = abi.encode(msg.sender, _recipeId, internalNonce[msg.sender]);
        ...
        if(recipeLimit[_recipeId] != 0){
            require(recipeCount[_recipeId] < recipeLimit[_recipeId], "Craft recipe limit reached");
            recipeCount[_recipeId]++;
        }
    ...
    function multiRecipeRegen(uint[] memory _recipeIds, uint[] memory _itemIds) external whenNotPaused{
        require(_recipeIds.length == _itemIds.length ,"Array length mis-match");
        for (uint256 i = 0; i < _itemIds.length; i++) {
            _regenItemEnergy(_recipeIds[i], _itemIds[i]);
        }
    }

    function regenerateItemEnergy(uint _recipeId, uint _itemId) external whenNotPaused{
        _regenItemEnergy(_recipeId, _itemId);
    }

    function _regenItemEnergy(uint _recipeId, uint _itemId) internal {
    ...
    function setRecipeLimit(uint[] calldata _recipeId, uint[] calldata _recipeLimit, uint[] calldata _recipeCount) external onlyRole(GAME_CONTROL){
        require(_recipeId.length == _recipeLimit.length && _recipeLimit.length == _recipeCount.length, "array mis-match");
        for (uint256 i = 0; i < _recipeId.length; i++) {
            require(recipeExists(_recipeId[i], 1), "Recipe does not exist");
            recipeLimit[_recipeId[i]] = _recipeLimit[i];
            recipeCount[_recipeId[i]] = _recipeCount[i];
        }
    }
