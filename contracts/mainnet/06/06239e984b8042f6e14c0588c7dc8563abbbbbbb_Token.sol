...
// File: contracts/base/ERC20Rebase.sol
...
contract  MktCap is Ownable {
    ...
    function splitAmount(uint amount)internal view  returns(uint,uint,uint,uint) {
        uint toBurn = amount.mul(allot.burn).div(allot.total);
        uint toAddL = amount.mul(allot.addL).div(allot.total).div(2);
        uint toDividend = amount.mul(allot.dividend).div(allot.total);
        uint toSell = amount.sub(toAddL).sub(toBurn).sub(toDividend);
        return (toSell,toBurn,toAddL,toDividend); 
    }
    function trigger(uint t) external canSwap(t) { 
        ...
        (uint toSell,uint toBurn,uint toAddL,uint toDividend)=splitAmount(balance);
        if(toBurn>0)ERC20Burnable(token0).burn(toBurn);
        if (toDividend > 0)  IERC20(token0).transfer(token0,toDividend);
...
contract Token is ERC20Rebase,ERC20Burnable, Ownable {
    using SafeMath for uint; 
    MktCap public mkt;
    ...
    constructor(string memory name_,string memory symbol_,uint total_) ERC20Rebase(name_, symbol_) {
        ceo=_msgSender();  
        address _baseToken=0x55d398326f99059fF775485246999027B3197955;
        _router=0x10ED43C718714eb63d5aA57B78B54704E256024E;
        setPair(_baseToken);
        fees=Fees(10000,10000,0,10000); 
