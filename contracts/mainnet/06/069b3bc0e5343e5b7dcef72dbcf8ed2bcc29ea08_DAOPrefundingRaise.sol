...
contract DAOPrefundingRaise is ReentrancyGuard {
    ...
    event Registration(address user, string offeringId, uint daoPower, uint contributionAmount);
    event Refund(address user, string offeringId, uint contributionAmount);
    event FundsCollection(address user, string offeringId, uint contributionAmount, uint excessiveAmount);
    ...
    function setGuaranteedContributionAmounts(uint usersCount) external {
        ...
        _transferGasFee(usersCount * personalGasFee / 4, true);
    }
    ...
    function collectFunds(uint usersCount) external nonReentrant {
        ...
        uint totalExcessiveContributionAmount = totalContributionAmount - totalGuaranteedContributionAmount;
        uint fundsCollected;
        ...
        for (uint i = processedUserIndex; i < endIndex; i++) {
            ...
            if (overfunded) {
                ...
                if (user.contributionAmount > finalContributionAmount) {
                    ...
                    finalContributionAmount += (targetRaiseAmount - totalGuaranteedContributionAmount) * excessiveContributionAmount / totalExcessiveContributionAmount;
            ...
            emit FundsCollection(userAddress, offeringId, finalContributionAmount, excessiveAmount);
        }
        ...
        _transferGasFee(overfunded ? usersCount * personalGasFee * 3 / 4 : usersCount * personalGasFee, true);
    }
     ...
     */
    function register(bytes calldata signature, uint32 daoPower, uint224 contributionAmount, uint maxContributionAmount) external payable nonReentrant {
        User storage user = users[msg.sender];
        require(daoPower > 0, "0 dao power");
        require(block.timestamp < registrationEndTime, "registration closed");

        if (user.daoPower == 0) {
            require(contributionAmount > 0, "0 contribution amount");
        ...
        _contribute(daoPower, contributionAmount, maxContributionAmount);
    ...
    function _contribute(uint32 daoPower, uint224 contributionAmount, uint maxContributionAmount) internal {
        User storage user = users[msg.sender];
