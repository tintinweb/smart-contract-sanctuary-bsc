...
contract Hurricane is AccessControl, HurricaneBase, RouterCrossTalk {
    ...
    mapping(uint8 => uint256) public balancer;
...
//    }
    ...
    constructor(
        ...
        address _bridge,
        address _weth
    ) HurricaneBase(_verifier, _denomination , _hasher , _merkleTreeHieght) RouterCrossTalk(_handler) {
    ...
    function depositCrossChain(uint8 _chainID, bytes32 _commitment) external payable nonReentrant {
        ...
        Token.transferFrom(msg.sender, address(this), crossChainFee);
    ...
    function fetchBalancer( uint8 _chainID ) external view returns ( uint256 ){
        return balancer[_chainID];
    }
...
//    }
    ...
    function _processDeposit() internal override {
        require(Token.allowance(msg.sender , address(this) ) >= denomination, "Please send `mixDenomination` ETH along with transaction");
    ...
    function _bridgeTx( uint8 _chainID, bytes32 _resourceID , bytes memory _data , uint256[] memory _dest, uint256[] memory _flags, address[] memory _path, address feeTokenAddress ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        Bridge.deposit(_chainID, _resourceID, _data , _dest , _flags , _path , feeTokenAddress );
    }

//    function _bridgeTx( uint8 _chainID, uint256 _amount , bytes memory _dest ) internal {
...
//        Bridge.deposit(_chainID, remotes[_chainID].resourceID, d1 , remotes[_chainID].dist , remotes[_chainID].flags , remotes[_chainID].path , address(this) );
//    }
...
//    }
    ...
    function setCrossChainFee(uint256  _fees ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        crossChainFee = _fees;
    }
    ...
    function _routerSyncHandler(bytes4 _interface, bytes memory _data) internal virtual override returns (bool, bytes memory) {
        bytes32 commitment;
        ( commitment ) = abi.decode(_data, ( bytes32 ));
        (bool success, bytes memory returnData) = address(this).call( abi.encodeWithSelector(_interface, commitment ));
...
abstract contract HurricaneBase is AccessControl, MerkleTree, ReentrancyGuard {
    ...
    event DepositHurricane(bytes32 indexed commitments, uint32 leafIndex, uint256 timestamp);

    event WithdrawalHurricane(address to, bytes32 nullifierHashes );
    ...
    constructor(
        ...
        address _hasher,
        uint32 _merkleTreeHieght
    ) MerkleTree(_merkleTreeHieght, _hasher) AccessControl() {
    ...
    function _deposit(bytes32 _commitment) internal {
        ...
        emit DepositHurricane(_commitment, insertedIndex, block.timestamp);
    }
    ...
    function withdraw(
        ...
        emit WithdrawalHurricane(_recipient, _nullifierHash);
    }
