...
contract CEXOFFENDER is Context, ERC20, Ownable {
    using SafeMath for uint256;
   ...
   SquidCheemsDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("CEXOFFENDER", "FTXv5") {
        _buyRewardsFee = 4;
        _buyLiquidityFee = 2;
		_buyMarketingFee = 2;
        _buyDevFee = 2;
    	...
    	dividendTracker = new SquidCheemsDividendTracker();

    	liquidityWallet = owner();
		
		marketingWallet = payable(0xCE120A3136a45CD1fC6e3EaC9a6b3D06216460b7);
        _devWallet = payable(0xc59FF11bDcF26EF00E09f7323BbBF15a342C1482);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "CEXOFFENDER: The dividend tracker already has that address");

        SquidCheemsDividendTracker newDividendTracker = SquidCheemsDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "CEXOFFENDER: The new dividend tracker must be owned by the CEXOFFENDER token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "CEXOFFENDER: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "CEXOFFENDER: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "CEXOFFENDER: Account is already the value of 'excluded'");
    ...
    function addToBlacklist(address account, bool blacklisted) public onlyOwner {
        require(_isBlacklisted[account] != blacklisted, "CEXOFFENDER: Account is already the value of 'blacklisted'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "CEXOFFENDER: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "CEXOFFENDER: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "CEXOFFENDER: The liquidity wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "CEXOFFENDER: The marketing wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "CEXOFFENDER: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "CEXOFFENDER: Cannot update gasForProcessing to same value");
...
contract SquidCheemsDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("SquidCheems_Dividend_Tracker", "SquidCheems_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function withdrawDividend() public pure override {
        require(false, "SquidCheems_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CEXOFFENDER contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 0 && newClaimWait <= 86400, "SquidCheems_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SquidCheems_Dividend_Tracker: Cannot update claimWait to same value");
