...
contract NFTMinter is Ownable {
    ...
    constructor() public { 
        ...
        IERC20(gELVNAddress).approve(tierAddress,9999999999999999999999999999);
        IERC20(ELVNAddress).approve(gELVNAddress,9999999999999999999999999999);
    }
    ...
    function swapMint(uint _tierLevel) public {
        ...
        if(_gelvnBalance >= _cost){
            ...
            uint _swapAmount = _cost - _gelvnBalance;
            if(_swapAmount > 0){
                IERC20(ELVNAddress).transferFrom(msg.sender, address(this), _swapAmount);
                IGELVN(gELVNAddress).swapFromOriginal(_swapAmount);
            }
            if(_gelvnBalance > 0){
    ...
    function busdMint(uint _tierLevel) public {
        ...
        swapStableForToken(_cost - _elvnBalance - _gelvnBalance,address(this));
        if(_elvnBalance > 0){
            IERC20(ELVNAddress).transferFrom(msg.sender, address(this), _elvnBalance);
        }

        
        IGELVN(gELVNAddress).swapFromOriginal(_cost-_gelvnBalance);
        IGELVN(gELVNAddress).unvestedPayment(msg.sender, _gelvnBalance);
    ...
    function swapStableForToken(uint256 _tokenAmount, address _receiver) private {
        ...
        IERC20(tokenAddress).approve(address(uniswapV2Router), _busdBalance);
        // make the swap
