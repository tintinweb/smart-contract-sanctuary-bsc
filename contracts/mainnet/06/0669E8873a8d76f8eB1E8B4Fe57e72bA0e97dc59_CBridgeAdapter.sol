...
contract CBridgeAdapter is AdapterBase {
    ...
    struct SendArgs {
        address receiver;
        uint64 chainid;
        uint64 nonce;  // some unique value

        // there is not exact way to calculate it,
        // it's not percent/1e6 as said in docs,
        // their API set it based on internal logic.
        // In Discord their devs recommended me,
        // in case if we don't know exact tokens amount,
        // to get slippage value from HTTP API for maxInputAmount
        // it may lead to higher slippage!
        uint32 slippage;
    }
