...
contract ModClaimV2 is Ownable, ReentrancyGuard {
    ...
    struct Moderator {
        ...
        uint paymentMultiplier; // 250 for chiefs, 150 for leads.. etc
    ...
    modifier zeroAddressCheck(address _address) {
        ...
        require(Moderators[_address].modAddress != address(0) , "Mod has zero address!");
        _;
    }
    ...
    modifier timeCheck() {
        require((block.timestamp.sub(Moderators[msg.sender].startTime).div(200)) >= 1 , "Must be a mod for 1 week");
        require(block.timestamp.sub(Moderators[msg.sender].lastClaim) >= 200 , "Must wait 1 week between payments");
        _;
    }
    ...
    function getPaid() external nonReentrant isMod(msg.sender) timeCheck() {
        ...
        uint timeMultiple = (block.timestamp.sub(Moderators[msg.sender].lastClaim)).div(200);
        ...
        if (Moderators[msg.sender].applePaymentEnabled && !Moderators[msg.sender].BUSDPaymentEnabled) {
            ...
            Moderators[msg.sender].lastClaim = Moderators[msg.sender].lastClaim.add(200 * timeMultiple);
