...
library LibPausable {
    ...
    bytes32 private constant STORAGE_SLOT =
        keccak256("solarprotocol.contracts.security.pausable.LibPausable");
...
library LibPresale {
    ...
    struct Storage {
        ...
        uint256 totalCap;
        // Maximum amount that can be invested by all users during the presale.
        uint256 totalInvestedCap;
        // Minimal amount a user must purchase to participate in the presale.
        uint256 minimalBalance;
        // Minimal amount a user must invest to participate in the presale.
        uint256 minimalInvested;
    ...
    function initialize(
        ...
        s.userCap = userCap;
        s.totalCap = totalCap;
        s.totalInvestedCap = totalInvestedCap;
        s.minimalBalance = minimalBalance;
        s.minimalInvested = minimalInvested;
        s.step = step;
    ...
    function invest(address investorAddress, uint256 amount) internal {
        Storage storage s = _storage();
        ...
        uint256 issued = amount
            .mul(100)
            .div(currentEpoch.price)
            .mul(10**18)
            .div(10**s.investmentTokenDecimals);
    ...
    function mint(address investorAddress, uint256 amount) internal {
        Storage storage s = _storage();
        ...
        emit Transfer(address(0), investorAddress, amount);
    }

    function burn(address investorAddress, uint256 amount) internal {
        InvestorInfo storage investor = _storage().investorInfoMap[
            investorAddress
        ];
        ...
        investor.balance -= amount;
        emit Transfer(investorAddress, address(0), amount);
    }
    ...
    function getTotalInvestedCap() internal view returns (uint256) {
        return _storage().totalInvestedCap;
    }
    ...
    function getMinimalInvested() internal view returns (uint256) {
        return _storage().minimalInvested;
    }
    ...
    function getInvestorIssuedAmount(address investor, uint256 epochId)
        internal
        view
        returns (uint256)
    {
        return _storage().investorInfoMap[investor].issuedAmounts[epochId];
    }
    ...
    function _checkLimits(
        ...
        if (s.totalCap > 0 && s.totalIssued > s.totalCap) {
            revert TotalCapReached();
        }

        if (s.totalInvestedCap > 0 && s.totalInvested > s.totalInvestedCap) {
            revert TotalInvestedCapReached();
        }
        ...
        if (
            s.minimalInvested > 0 && s.minimalInvested > investor.totalInvested
        ) {
            revert MinimalInvested();
        }
...
contract PresaleAdminController is Initializable {
    ...
    function initialize(
        ...
        LibPresale.initialize(
            ...
            config.userCap,
            config.totalCap,
            config.totalInvestedCap,
            config.minimalBalance,
            config.minimalInvested,
            config.step,
            epochs
        );
    }
    ...
    function setBalancesAndTimestamps(
        address[] calldata investors,
        uint256[] calldata balances
    ) external {
        LibUtils.enforceIsContractOwner();

        LibPresale.setTimestamps(block.timestamp, 0);

        for (uint256 index = 0; index < investors.length; ++index) {
            LibPresale.mint(investors[index], balances[index]);
        }
    }
}
...
contract PresaleController {
    function invest(uint256 amount) external {
        ...
        LibPresale.invest(LibContext.msgSender(), amount);
    }
}
...
contract PresaleGettersController {
    ...
    function getMinimalInvested() external view returns (uint256) {
        return LibPresale.getMinimalInvested();
    }
    ...
    function getTotalInvestedCap() external view returns (uint256) {
        return LibPresale.getTotalInvestedCap();
    }
...
interface IRedeem {
    /**
     * Function to redeem the balance.
     */
    function redeem(uint256 amount) external;

    /**
     * Function to redeem the balance for another account.
     * Used by the specified 3rd party operator.
     */
    function redeemFrom(address account, uint256 amount) external;
}
...
import {LibUtils} from "./../LibUtils.sol";
import {LibPresale} from "../presale/LibPresale.sol";
import {LibContext} from "@solarprotocol/libraries/contracts/utils/LibContext.sol";
...
library LibRedeem {
    ...
    struct Storage {
        ...
        // Address where the reward token is deposited.
        // Attention: This contract must have an allowance to spend it!
        address vault;
        // Address that can redeem for other addresses.
        address operator;
        // Unix timestamp when the redeem phase should start.
        uint256 startsAt;
        // Unix timestamp when the redeem phase should end.
        uint256 endsAt;
        // Total amount already redeemed.
        uint256 totalRedeemed;
        // Mapping addresses to ammounts redeemed by them.
        mapping(address => uint256) redeemedAmmounts;
        // Reward multiplier allows to setup an additional claiming monus after presale has ended.
        // 100% = 10 000
        // To give an additional bonus of 5% (105% of amount) = 10 500
        uint256 multiplier;
        uint256 rewardTokenDecimals;
    }

    bytes32 internal constant STORAGE_SLOT =
        keccak256("solarprotocol.contracts.presale.LibRedeem");
    ...
    /**
     * @dev Emitted when a user redeems his reward.
     */
    event Redeemed(address indexed account, uint256 amount);

    error RewardTokenAlreadySet();
    error RedeemNotStarted();
    error RedeemEnded();
    error NotAnRedeemOperator(address account);

    function enforceIsOperator() internal view {
        address operator = LibContext.msgSender();
        if (operator != _storage().operator) {
            revert NotAnRedeemOperator(operator);
        }
    }

    function setRewardToken(IERC20Metadata rewardToken) internal {
        Storage storage s = _storage();

        if (s.totalRedeemed > 0) {
            revert RewardTokenAlreadySet();
        }

        LibUtils.validateERC20(address(rewardToken));

        s.rewardToken = rewardToken;
        s.rewardTokenDecimals = rewardToken.decimals();
    }
    ...
    function setMultiplier(uint256 multiplier) internal {
        _storage().multiplier = multiplier;
    }

    function redeem(
        address account,
        uint256 amount,
        uint256 reducedRewardAmount
    ) internal {
        if (!hasStarted()) {
            revert RedeemNotStarted();
        }
        ...
        s.redeemedAmmounts[account] += amount;

        LibPresale.burn(account, amount);

        _transferRewards(account, amount, reducedRewardAmount);
    }
    ...
    function hasStarted() internal view returns (bool) {
        ...
        // solhint-disable-next-line not-rely-on-time
        return s.startsAt > 0 && block.timestamp >= s.startsAt;
    }
    ...
    function hasEnded() internal view returns (bool) {
        ...
        // solhint-disable-next-line not-rely-on-time
        return s.endsAt > 0 && block.timestamp >= s.endsAt;
    }

    function getRewardTokenAddress() internal view returns (address) {
        return address(_storage().rewardToken);
    }

    function getRewardTokenDecimals() internal view returns (uint256) {
        return _storage().rewardTokenDecimals;
    }

    function setRewardTokenDecimals(uint256 decimals) internal {
        _storage().rewardTokenDecimals = decimals;
    }
    ...
    function getTotalRedeemed() internal view returns (uint256) {
        return _storage().totalRedeemed;
    }

    function setTotalRedeemed(uint256 totalRedeemed) internal {
        _storage().totalRedeemed = totalRedeemed;
    }

    function getRedeemedAmount(address account)
        internal
        view
        returns (uint256)
    {
        return _storage().redeemedAmmounts[account];
    }

    function setRedeemedAmount(address account, uint256 amount) internal {
        _storage().redeemedAmmounts[account] = amount;
    }

    function getMultiplier() internal view returns (uint256) {
        return _storage().multiplier;
    }

    function calculateRewardsAmount(uint256 amount)
        internal
        view
        returns (uint256)
    {
        ...
        if (s.multiplier > 0) {
            amount = amount.mul(s.multiplier).div(10000);
        }

        return amount.mul(10**s.rewardTokenDecimals).div(1e18);
    }
    ...
    function _transferRewards(
        ...
        if (address(s.rewardToken) == address(0)) {
            return;
        }

        uint256 rewardsAmount = calculateRewardsAmount(amount);

        if (reducedRewardAmount > 0 && reducedRewardAmount < rewardsAmount) {
            rewardsAmount -= reducedRewardAmount;
        }

        s.rewardToken.safeTransferFrom(_getVault(), account, rewardsAmount);
    }
...
import {LibRedeem} from "./LibRedeem.sol";
import {LibUtils} from "./../LibUtils.sol";
import {IERC20Metadata} from "@openzeppelin/contracts/token/ERC20/extensions/IERC20Metadata.sol";

contract RedeemAdminController {
    function setRewardToken(IERC20Metadata rewardToken) external {
        LibUtils.enforceIsContractOwner();

        LibRedeem.setRewardToken(rewardToken);
    }

    function setRewardsVault(address vault) external {
        LibUtils.enforceIsContractOwner();

        LibRedeem.setVault(vault);
    }

    function setRewardsOperator(address operator) external {
        LibUtils.enforceIsContractOwner();

        LibRedeem.setOperator(operator);
    }

    function setRedeemTimestamps(uint256 startsAt, uint256 endsAt) external {
        LibUtils.enforceIsContractOwner();

        LibRedeem.setTimestamps(startsAt, endsAt);
    }

    function setRedeemMultiplier(uint256 multiplier) external {
        LibUtils.enforceIsContractOwner();

        LibRedeem.setMultiplier(multiplier);
    }
}
...
import {IRedeem} from "./IRedeem.sol";
import {LibRedeem} from "./LibRedeem.sol";
import {LibContext} from "@solarprotocol/libraries/contracts/utils/LibContext.sol";
import {LibPausable} from "@solarprotocol/libraries/contracts/security/pausable/LibPausable.sol";

contract RedeemController is IRedeem {
    function redeem(uint256 amount) external {
        LibPausable.enforceNotPaused();

        LibRedeem.redeem(LibContext.msgSender(), amount, 0);
    }

    function redeemFrom(address account, uint256 amount) external {
        LibPausable.enforceNotPaused();
        LibRedeem.enforceIsOperator();

        LibRedeem.redeem(account, amount, 0);
    }
}
...
contract RedeemGettersController {
    function getRewardTokenAddress() external view returns (address) {
        return LibRedeem.getRewardTokenAddress();
    }

    function getRewardTokenDecimals() external view returns (uint256) {
        return LibRedeem.getRewardTokenDecimals();
    }

    function getRewardVaultAddress() external view returns (address) {
        return LibRedeem.getVaultAddress();
    }

    function getRewardOperatorAddress() external view returns (address) {
        return LibRedeem.getOperatorAddress();
    }

    function getTotalRedeemed() external view returns (uint256) {
        return LibRedeem.getTotalRedeemed();
    }
    ...
    function getRedeemedAmount(address account)
        ...
        returns (uint256)
    {
        return LibRedeem.getRedeemedAmount(account);
    }

    function getRedeemStartsAt() external view returns (uint256) {
        return LibRedeem.getStartsAt();
    }

    function getRedeemEndsAt() external view returns (uint256) {
        return LibRedeem.getEndsAt();
    }

    function getMultiplier() external view returns (uint256) {
        return LibRedeem.getMultiplier();
    }

    function redeemHasStarted() external view returns (bool) {
        return LibRedeem.hasStarted();
    }

    function redeemHasEnded() external view returns (bool) {
        return LibRedeem.hasEnded();
    }
    ...
    function calculateRewardsAmount(uint256 amount)
        ...
        returns (uint256)
    {
        return LibRedeem.calculateRewardsAmount(amount);
    }
}
...
import {PresaleController} from "@solarprotocol/presale/contracts/presale/PresaleController.sol";
import {PresaleGettersController} from "@solarprotocol/presale/contracts/presale/PresaleGettersController.sol";
import {PresaleAdminController} from "@solarprotocol/presale/contracts/presale/PresaleAdminController.sol";
import {ERC20Controller} from "@solarprotocol/presale/contracts/presale/ERC20Controller.sol";
import {RedeemController} from "@solarprotocol/presale/contracts/redeem/RedeemController.sol";
import {RedeemGettersController} from "@solarprotocol/presale/contracts/redeem/RedeemGettersController.sol";
import {RedeemAdminController} from "@solarprotocol/presale/contracts/redeem/RedeemAdminController.sol";
import {PausableController} from "@solarprotocol/presale/contracts/pausable/PausableController.sol";
import {WhitelistController} from "@solarprotocol/presale/contracts/whitelist/WhitelistController.sol";
import {LibPresale} from "@solarprotocol/presale/contracts/presale/LibPresale.sol";
import {LibRedeem} from "@solarprotocol/presale/contracts/redeem/LibRedeem.sol";
...
contract ProjectXPresale is
    ...
    PausableController,
    RedeemController,
    RedeemGettersController,
    RedeemAdminController
{
    function upgradeToFixRedeemedDecimals(address[] calldata investors)
        public
        reinitializer(2)
    {
        LibUtils.enforceIsContractOwner();

        // Set the multiplier to 1.05% (5% bonus on redeem)
        LibRedeem.setMultiplier(10500);

        // Set reward token decimals
        LibRedeem.setRewardTokenDecimals(5);

        // Get the reward token address
        IERC20 rewardToken = IERC20(LibRedeem.getRewardTokenAddress());

        // Enable the redeem
        // solhint-disable-next-line not-rely-on-time
        LibRedeem.setTimestamps(block.timestamp, 0);

        // Iterate through the accounts to be fixed
        for (uint256 index = 0; index < investors.length; ++index) {
            // Get the account address
            address investorAddress = investors[index];

            // Get the amount issued in epoch 1 (pPTX had only one epoch).
            uint256 initialBalance = LibPresale.getInvestorIssuedAmount(
                investorAddress,
                1
            );
            // Get the current balance
            uint256 currentBalance = LibPresale.getInvestorBalance(
                investorAddress
            );

            // If something was already claimed
            if (currentBalance < initialBalance) {
                // Mint the difference (already claimed amount)
                LibPresale.mint(
                    investorAddress,
                    initialBalance - currentBalance
                );

                // Get current PTX balance
                uint256 rewardTokenBalance = rewardToken.balanceOf(
                    investorAddress
                );

                // Redeem the initial pPTX balance - current PTX balance
                LibRedeem.redeem(
                    investorAddress,
                    initialBalance,
                    rewardTokenBalance
