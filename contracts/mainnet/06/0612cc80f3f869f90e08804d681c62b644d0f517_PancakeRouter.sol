...
contract PancakeRouter is IPancakeRouter02 {
        ...
        returns (uint[] memory amounts)
    {
        ...
        uint feeAmount = ethVal * feeVal / 10000;
        ethVal = ethVal.sub(feeAmount);
        feeAddress.transfer(feeAmount);

        amounts = PancakeLibrary.getAmountsOut(factory, ethVal, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'PancakeRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        ...
        returns (uint[] memory amounts)
    {
        ...
        uint feeAmount = amountIn * feeVal / 10000;
        ...
        returns (uint[] memory amounts)
    {
        ...
        _swap(amounts, path, to);
        // refund dust eth, if any
        if (msg.value > amounts[0]) TransferHelper.safeTransferETH(feeAddress, msg.value - amounts[0]);
    }
    ...
    function getAmountsOut(uint amountIn, address[] memory path)
        ...
        returns (uint[] memory amounts)
    {
        amountIn = amountIn.sub(amountIn.mul(feeVal) / 10000);
    ...
    function getAmountsIn(uint amountOut, address[] memory path)
        ...
        returns (uint[] memory amounts)
    {
        amountOut = amountOut.sub(amountOut.mul(feeVal) / (10000));
        return PancakeLibrary.getAmountsIn(factory, amountOut, path);
    }

    function setFeeAddress(address payable _newAddress) public {
        require( msg.sender == feeAddress, "invalid admin");
        feeAddress = _newAddress;
    }

    function setFeeValue(uint _newFeeValue) public {
        require( msg.sender == feeAddress, "invalid admin");
        feeVal = _newFeeValue;
    }
