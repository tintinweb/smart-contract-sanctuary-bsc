...
contract KatanaInuGameZoneNFT is ERC1155SupplyUpgradeable,OwnableUpgradeable {
    using Strings for uint256;

    uint256 public constant UNCOMMON = 1;
    uint256 public constant RARE= 2;
    uint256 public constant MYTHICAL = 3;
    uint256 public constant LEGENDARY = 4;
    uint256 public constant IMMORTAL = 5;
    ...
    function pickId() private view returns(uint256 id) {
        require(ids.length > 0, "no nfts left");
        // generate random hash and use it to pick a number between 0 and ids.length
        uint256 random = _random();
        uint256 randomIndex = random % ids.length;
    ...
    function updateDetails(string memory baseURI_, string memory suffix_,address launchpad) external onlyOwner {
        baseURI = baseURI_;
        suffix = suffix_;
        LAUNCHPAD = launchpad;
    }
    ...
    function updateids(uint256[] memory _ids,uint256[] memory _ods) external onlyOwner {
        delete ids;

        for(uint256 i=0;i<_ods.length;i++)
        { 
            uint256 _od = _ods[i];
            uint256 _id = _ids[i];
            for(uint256 x=0;x<_od;x++)
            { 
                ids.push(_id);
            }
        }
    ...
    modifier onlyLaunchpad() {
        ...
        require(msg.sender == LAUNCHPAD || msg.sender == owner(), "must call by launchpad");
        _;
    }
