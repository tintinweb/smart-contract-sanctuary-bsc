    ...
    */
    ...
    updateReward(address(0))
    {
        ...
        if (block.timestamp >= _periodFinish) {
            _rewardRate = realReward.div(_24hDuration);
        } else {
            uint256 remaining = _periodFinish.sub(block.timestamp);
            uint256 leftover = remaining.mul(_rewardRate);
            _rewardRate = realReward.add(leftover).div(_24hDuration);
        }
        _lastUpdateTime = block.timestamp;
        _periodFinish = block.timestamp.add(_24hDuration);
