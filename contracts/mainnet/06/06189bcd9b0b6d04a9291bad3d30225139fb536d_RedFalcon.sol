...
contract RedFalcon is BEP20, LockToken {
    ...
    uint256 public totalFees = USDTRewardsFee.add(liquidityFee).add(marketingFee).add(CharityFee);

    address payable public _marketingWalletAddress = payable(0xA73951c66BBb1674AFEc057365a7626475655B5a);
    address payable public _CharityWalletAddress = payable(0xD820b43F8AAF26DEB1EB54e91B42d3c61d69aa33);
    ...
    function setLiquiditFee(uint256 value) external onlyOwner{
        liquidityFee = value;
        totalFees = USDTRewardsFee.add(liquidityFee).add(marketingFee).add(CharityFee);
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "RedFalcon: gasForProcessing must be between 200,000 and 500,000");
    ...
    function swapAndSendToFee(uint256 tokens) private  
    {
        ...
        uint256 marketingShare = newBalance.div(marketingFee+CharityFee).mul(marketingFee);
        IBEP20(USDT).transfer(_marketingWalletAddress, marketingShare);
        IBEP20(USDT).transfer(_CharityWalletAddress, newBalance.sub(marketingShare));
    }
    ...
    function swapAndLiquify(uint256 tokens) private 
    {
       uint256 swapableFee = totalFees.sub(USDTRewardsFee);
        ...
        uint256 bnbForDeveloper = newBalance.mul(CharityFee).div(swapableFee);

        _marketingWalletAddress.transfer(bnbForMarketing);
        _CharityWalletAddress.transfer(bnbForDeveloper);
    }
