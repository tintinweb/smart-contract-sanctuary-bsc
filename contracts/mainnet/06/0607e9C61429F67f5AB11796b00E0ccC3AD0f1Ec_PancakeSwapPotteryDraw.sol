...
//    uint32 constant POTTERY_PERIOD = 1 weeks;
    uint32 constant POTTERY_PERIOD = 1 days;
    uint32 constant START_TIME_BUFFER = 2 weeks;
    uint32 constant DRAW_TIME_BUFFER = 5 days;
    ...
    event CreatePottery(address indexed vault, uint256 totalPrize, uint256 lockTime, uint256 drawTime, address admin);
    ...
    function generatePottery(
        ...
        uint256 prize = _totalPrize / NUM_OF_DRAW;
        require(prize > 0, "zero prize in each draw");
        require(_totalPrize % NUM_OF_DRAW == 0, "round prize has reminder");
        require(prize / NUM_OF_WINNER > 0, "zero prize in each winner");
        require(prize % NUM_OF_WINNER == 0, "winner prize has reminder");
        ...
        emit CreatePottery(vault, _totalPrize, _lockTime, _drawTime, msg.sender);
    }
