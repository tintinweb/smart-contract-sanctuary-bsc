...
contract MORPHTOKEN is IBEP20, MORPHAuth {
    using SafeMath for uint256;
    ...
    string constant _name = "MORPH TOKEN";
    ...
    event FoundationTransfer(bool status);

    constructor () MORPHAuth(msg.sender) {
    ...
    function swapBack() internal swapping {
        ...
        try router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            ...
            uint256 amountBNBFoundation = amountBNB.mul(foundationFee).div(totalBNBFee);

            try distributor.deposit{value: amountBNBReflection}() {} catch {}
            (bool marketSuccess, ) = payable(marketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
            (bool foundationSuccess, ) = payable(foundationFeeReceiver).call{value: amountBNBFoundation, gas: 30000}("");
            
            emit MarketTransfer(marketSuccess);
            emit FoundationTransfer(foundationSuccess);
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _foundationFee, uint256 _burnFee, uint256 _feeDenominator) external authorizedFor(Permission.AdjustContractVariables) {
        liquidityFee = _liquidityFee;
        reflectionFee = _reflectionFee;
        marketingFee = _marketingFee;
        foundationFee = _foundationFee;
        burnFee = _burnFee;
        totalFee = _liquidityFee.add(_reflectionFee).add(_marketingFee).add(_foundationFee).add(_burnFee);
        feeDenominator = _feeDenominator;
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _foundationFeeReciever) external authorizedFor(Permission.AdjustContractVariables) {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        marketingFeeReceiver = _marketingFeeReceiver;
        foundationFeeReceiver = _foundationFeeReciever;
    }
