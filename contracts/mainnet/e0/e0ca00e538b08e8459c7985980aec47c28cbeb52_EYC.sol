...
contract EYC is BEP20, Ownable {
    ...
    // treasury wallet
    address public rewardsWallet = 0xb4c267fA77E53d89834EA7F0857DD309F7210573;
    address public marketingWallet = 0xD9FE8A95BFe318Aa8D73e63efE8Ad2a834aef217;
    address public devWallet = 0x78d0D206E94d4428643201daEBf831C87577dA92;
    ...
    // buy & sell fee
    uint256 internal rewardFeeBuy = 1;
    uint256 internal marketingFeeBuy = 1;
    uint256 internal devFeeBuy = 1;

    uint256 internal rewardFeeSell = 1;
    uint256 internal marketingFeeSell = 1;
    uint256 internal devFeeSell = 1;
    ...
    constructor() BEP20("EYECOIN", "EYC") {

        address owner_ = address(0x0b087E23B3659aB16365e55217E41aBd3d558167);
    ...
    function updatePancakeRouter(address newPancakeRouter) public onlyOwner {
    ...
    function updateDevWallet(address newAddress) public onlyOwner {
        devWallet = newAddress;
    }

    function updateRewardsWallet(address newAddress) public onlyOwner {
        rewardsWallet = newAddress;
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "EYC: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "EYC: Automated market maker pair is already set to that value");
    ...
    function _isSell(address from, address to) internal view returns (bool) {
        ...
        return from != pancakeRouter && automatedMarketMakerPairs[to];
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (_isBuy(from) && !isExcludedFromFees[to]) {
            uint256 marketingFee = amount.mul(marketingFeeBuy).div(100);
            uint256 devFee = amount.mul(devFeeBuy).div(100);
            uint256 rewardFee = amount.mul(rewardFeeBuy).div(100);

            amount = amount.sub(marketingFee).sub(devFee).sub(rewardFee);
        ...
        if (_isSell(from, to) && !isExcludedFromFees[from]) {
            uint256 marketingFee = amount.mul(marketingFeeSell).div(100);
            uint256 devFee = amount.mul(devFeeSell).div(100);
            uint256 rewardFee = amount.mul(rewardFeeSell).div(100);
    ...
    function updateBuyFee(uint256 _rewardFee,uint256 _marketingFee,uint256 _devFee) external onlyOwner {
        require(_rewardFee + _marketingFee + _devFee <= 3 , "Can't Exceed more than 3");
        rewardFeeBuy = _rewardFee;
        marketingFeeBuy = _marketingFee;
        devFeeBuy = _devFee;
    }

     function updateSellFee(uint256 _rewardFee,uint256 _marketingFee,uint256 _devFee) external onlyOwner {
        require(_rewardFee + _marketingFee + _devFee <= 3 , "Can't Exceed more than 3");
