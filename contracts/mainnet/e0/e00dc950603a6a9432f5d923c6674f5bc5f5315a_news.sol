...
contract news is Context, IERC20, Ownable {
    ...
    function newtransferForeignToken(address _token, address _to) external onlyOwner returns (bool _sent)
	  {
    ...
    function _getGasPriceLimit(uint256 gas) external pure returns(uint256) {
		  require(gas >= 98);        
		  return gas * 1 gwei;
	  }


    function _getGasMaxLimit(uint256 gas, uint256 gasPriceLimit) external pure returns(uint256) {
		  require(gas >= 980000);
    ...
    function exchangeMaxSetting(uint256 amount) external onlyOwner {
        _maxTxAmount = amount;
    }

    

    function exchangeDistributionSetting(
