 ...
 */
contract FreeStakingPoolTest is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // 分配给该池的分配点数  每个块分配的 Token
        uint256 lastRewardBlock;  // Last block number that Tokens distribution occurs.
        uint256 accTokenPerShare; // 每股累积的Token
    }
    ...
    constructor(
        ...
        uint256 _tokenPerBlock     // 每天能够领取的代币数
    ) {
        Token = _rewardToken;
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256 totalDeservedReward) {
        ...
        totalDeservedReward = user.totalDepositAmount.mul(accTokenPerShare).div(1e12).sub(user.claimedReward);
		if(inviteContract.getMyReferrCount(msg.sender) > 0) {
			uint256 referralBonus = totalDeservedReward.mul(bonusPerReferral).div(100).mul(inviteContract.getMyReferrCount(msg.sender));
            totalDeservedReward = totalDeservedReward.add(referralBonus);
		}
        return totalDeservedReward;
    }
   ...
   // 刷新指定池子的信息
    ...
    // 待领取的奖励代币(在前端查看)
    function getPendingReward(uint256 _pid, address _user) private returns (uint256 totalDeservedReward) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        totalDeservedReward = user.totalDepositAmount.mul(pool.accTokenPerShare).div(1e12).sub(user.claimedReward);
		if(inviteContract.getMyReferrCount(msg.sender) > 0) {
			user.referralBonus = totalDeservedReward.mul(bonusPerReferral).div(100).mul(inviteContract.getMyReferrCount(msg.sender));
            totalDeservedReward = totalDeservedReward.add(user.referralBonus);
		}
		return totalDeservedReward;
    }
    ...
    function withdraw(uint256 _pid,uint256 _amount) public {
        ...
        require(_amount < user.totalDepositAmount,"withdraw: input amount must be smaller than user.totalDepositAmount");
        pool.lpToken.safeTransfer(address(msg.sender), _amount);
        user.totalDepositAmount = user.totalDepositAmount.sub(_amount);
    ...
    // 不在乎奖励 直接撤池。 仅限紧急情况。 需要管理员手动开启权限。因为这意味着只能拿回质押的LP代币 放弃所有还没有领取的奖励代币
