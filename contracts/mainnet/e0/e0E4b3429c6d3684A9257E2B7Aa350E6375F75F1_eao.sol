...
contract eao {
    ...
    uint256 private constant feePercents = 190; 
    ...
    uint256[7] private levelInvite = [0, 5000e18, 10000e18, 20000e18, 30000e18, 40000e18, 500000e18];
...
// 解冻资本或奖励
    function _unfreezeCapitalOrReward(address _userAddr, uint256 _amount) private {
        ...
        if(order.isUnfreezed == false && block.timestamp >= order.unfreeze && _amount >= order.amount){
            ...
            if(user.totalFreezed.mul(3) > user.totalRevenue){
                    uint256 leftCapital = user.totalFreezed.mul(3).sub(user.totalRevenue);
                    if(staticReward > leftCapital){
                        staticReward = leftCapital;
                    }
                }else{
                        ...
                        if(newAmount > 0 && userInfo[upline].totalFreezed.mul(3) > userInfo[upline].totalRevenue ){
                            ...
                            if(userInfo[upline].level > i ){
                                  ...
                                  if(userInfo[upline].level ==  5 && i < 7 ){
                                   ...
                                   if(userInfo[upline].level ==  6 && i < 12 ){
                                        if(i < 12){
                                            upRewards.invited = upRewards.invited.add(reward);
                                            userInfo[upline].totalRevenue = userInfo[upline].totalRevenue.add(reward);
                                        }else{
                    ...
                    if(_bal < MAXBal.mul(balFreezeDynamic[i - 1]).div(100))
                    {
                        ...
                        if(_bal < MAXBal.mul(balFreezeAll[i - 1]).div(100))
                        {
                            ...
                            if(RebackEndTime == 0){
                                ...
                                if(block.timestamp.sub(RebackEndTime)>20000){
