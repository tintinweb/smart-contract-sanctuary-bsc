...
contract AnjiUtils {
    ...
    function getTokenPairReserveInfo(address tokenAddress, address tokenPairAddress, address factory) public view virtual returns (PairReserveInfo memory) {
        ...
        (address token0, address token1) = DEXLibrary.sortTokens(tokenAddress, tokenPairAddress);
        (uint256 token_decimals, uint256 reserve_decimals) = tokenAddress == token0 ? (IERC20(token0).decimals(), IERC20(token1).decimals()) : (IERC20(token1).decimals(), IERC20(token0).decimals());
        ...
        return PairReserveInfo({
            ...
            token_pair: token0 == tokenAddress ? token1 : token0,
            factory: factory
        });
    }
}