...
import '@lovelyswap/v3-periphery/contracts/base/SelfPermit.sol';
import '@lovelyswap/v3-periphery/contracts/base/PeripheryImmutableState.sol';
...
/// @title LovelySwap V3 Swap Router
contract LovelySwapRouter02 is ISwapRouter02, V2SwapRouter, V3SwapRouter, ApproveAndCall, MulticallExtended, SelfPermit {
    constructor(
        address _factoryV2,
        address factoryV3,
...
import '@lovelyswap/v3-periphery/contracts/interfaces/ISelfPermit.sol';
...
import '@lovelyswap/v3-core/contracts/libraries/LowGasSafeMath.sol';
import '@openzeppelin/contracts/token/ERC20/IERC20.sol';
...
import './libraries/LovelySwapV2Library.sol';

/// @title LovelySwap V2 Swap Router
/// @notice Router for stateless execution of swaps against LovelySwap V2
abstract contract V2SwapRouter is IV2SwapRouter, ImmutableState, PeripheryPaymentsWithFeeExtended {
    ...
    function _swap(address[] memory path, address _to) private {
        for (uint256 i; i < path.length - 1; i++) {
            ...
            IUniswapV2Pair pair = IUniswapV2Pair(LovelySwapV2Library.pairFor(factoryV2, input, output));
            uint256 amountInput;
            ...
            // scope to avoid stack too deep errors
            {
                ...
                amountOutput = LovelySwapV2Library.getAmountOut(amountInput, reserveInput, reserveOutput);
            }
            (uint256 amount0Out, uint256 amount1Out) =
                input == token0 ? (uint256(0), amountOutput) : (amountOutput, uint256(0));
            address to = i < path.length - 2 ? LovelySwapV2Library.pairFor(factoryV2, output, path[i + 2]) : _to;
    ...
    function swapExactTokensForTokens(
        ...
        pay(
            ...
            LovelySwapV2Library.pairFor(factoryV2, path[0], path[1]),
            amountIn
        );
    ...
    function swapTokensForExactTokens(
        ...
        amountIn = LovelySwapV2Library.getAmountsIn(factoryV2, amountOut, path)[0];
        require(amountIn <= amountInMax, 'Too much requested');

        pay(path[0], msg.sender, LovelySwapV2Library.pairFor(factoryV2, path[0], path[1]), amountIn);
...
import '@lovelyswap/v3-core/contracts/libraries/SafeCast.sol';
import '@lovelyswap/v3-core/contracts/libraries/TickMath.sol';
import '@lovelyswap/v3-core/contracts/interfaces/ILovelySwapV3Pool.sol';
import '@lovelyswap/v3-periphery/contracts/libraries/Path.sol';
import '@lovelyswap/v3-periphery/contracts/libraries/PoolAddress.sol';
import '@lovelyswap/v3-periphery/contracts/libraries/CallbackValidation.sol';
import '@openzeppelin/contracts/token/ERC20/IERC20.sol';
...
/// @title LovelySwap V3 Swap Router
/// @notice Router for stateless execution of swaps against LovelySwap V3
abstract contract V3SwapRouter is IV3SwapRouter, PeripheryPaymentsWithFeeExtended, OracleSlippage {
    ...
    function getPool(
        ...
        address tokenB,
        uint24 fee
    ) private view returns (ILovelySwapV3Pool) {
        return ILovelySwapV3Pool(PoolAddress.computeAddress(factory, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }
    ...
    /// @inheritdoc ILovelySwapV3SwapCallback
    function LovelySwapV3SwapCallback(
        int256 amount0Delta,
        int256 amount1Delta,
...
import '@openzeppelin/contracts/token/ERC20/IERC20.sol';
import '@lovelyswap/v3-periphery/contracts/interfaces/INonfungiblePositionManager.sol';

import '../interfaces/IApproveAndCall.sol';
import './ImmutableState.sol';

/// @title Approve and Call
/// @notice Allows callers to approve the LovelySwap V3 position manager from this contract,
...
import '@lovelyswap/v3-periphery/contracts/base/Multicall.sol';
...
/// @title Router token swapping functionality
/// @notice Functions for swapping tokens via LovelySwap V2
...
import '@lovelyswap/v3-core/contracts/interfaces/callback/ILovelySwapV3SwapCallback.sol';

/// @title Router token swapping functionality
/// @notice Functions for swapping tokens via LovelySwap V3
interface IV3SwapRouter is ILovelySwapV3SwapCallback {
...
import '@lovelyswap/v3-periphery/contracts/interfaces/IMulticall.sol';
...
/// @title Callback for ILovelySwapV3PoolActions#swap
/// @notice Any contract that calls ILovelySwapV3PoolActions#swap must implement this interface
interface ILovelySwapV3SwapCallback {
    /// @notice Called to `msg.sender` after executing a swap via ILovelySwapV3Pool#swap.
    /// @dev In the implementation you must pay the pool tokens owed for the swap.
    /// The caller of this method must be checked to be a LovelySwapV3Pool deployed by the canonical LovelySwapV3Factory.
    /// amount0Delta and amount1Delta can both be 0 if no tokens were swapped.
    /// @param amount0Delta The amount of token0 that was sent (negative) or must be received (positive) by the pool by
    /// the end of the swap. If positive, the callback must send that amount of token0 to the pool.
    /// @param amount1Delta The amount of token1 that was sent (negative) or must be received (positive) by the pool by
    /// the end of the swap. If positive, the callback must send that amount of token1 to the pool.
    /// @param data Any data passed through by the caller via the ILovelySwapV3PoolActions#swap call
    function LovelySwapV3SwapCallback(
        int256 amount0Delta,
        int256 amount1Delta,
        bytes calldata data
...
import '@lovelyswap/v3-periphery/contracts/base/PeripheryPaymentsWithFee.sol';
...
interface IImmutableState {
    /// @return Returns the address of the LovelySwap V2 factory
    function factoryV2() external view returns (address);

    /// @return Returns the address of LovelySwap V3 NFT position manager
...
import '@lovelyswap/v3-periphery/contracts/interfaces/IPeripheryPaymentsWithFee.sol';
...
import '@lovelyswap/v3-periphery/contracts/base/PeripheryPayments.sol';
import '@lovelyswap/v3-periphery/contracts/libraries/TransferHelper.sol';
...
import '@lovelyswap/v3-periphery/contracts/interfaces/IPeripheryPayments.sol';
...
import './pool/ILovelySwapV3PoolImmutables.sol';
import './pool/ILovelySwapV3PoolState.sol';
import './pool/ILovelySwapV3PoolDerivedState.sol';
import './pool/ILovelySwapV3PoolActions.sol';
import './pool/ILovelySwapV3PoolOwnerActions.sol';
import './pool/ILovelySwapV3PoolEvents.sol';

/// @title The interface for a LovelySwap V3 Pool
/// @notice A LovelySwap pool facilitates swapping and automated market making between any two assets that strictly conform
/// to the ERC20 specification
/// @dev The pool interface is broken up into many smaller pieces
interface ILovelySwapV3Pool is
    ILovelySwapV3PoolImmutables,
    ILovelySwapV3PoolState,
    ILovelySwapV3PoolDerivedState,
    ILovelySwapV3PoolActions,
    ILovelySwapV3PoolOwnerActions,
    ILovelySwapV3PoolEvents
{
...
library PoolAddress {
    bytes32 internal constant POOL_INIT_CODE_HASH = 0x700ee48439d720c9187eb29d36b3e45e38f986c3db92378136e8b879eb25913c;
    ...
    /// @param factory The LovelySwap V3 factory contract address
...
/// @notice Provides validation for callbacks from LovelySwap V3 Pools
library CallbackValidation {
    /// @notice Returns the address of a valid LovelySwap V3 Pool
    /// @param factory The contract address of the LovelySwap V3 factory
    ...
    function verifyCallback(
        ...
        address tokenB,
        uint24 fee
    ) internal view returns (ILovelySwapV3Pool pool) {
    ...
    function verifyCallback(address factory, PoolAddress.PoolKey memory poolKey)
        internal
        view
        returns (ILovelySwapV3Pool pool)
    {
        pool = ILovelySwapV3Pool(PoolAddress.computeAddress(factory, poolKey));
...
import '@lovelyswap/v3-periphery/contracts/base/PeripheryImmutableState.sol';
import '@lovelyswap/v3-periphery/contracts/base/BlockTimestamp.sol';
import '@lovelyswap/v3-periphery/contracts/libraries/Path.sol';
import '@lovelyswap/v3-periphery/contracts/libraries/PoolAddress.sol';
import '@lovelyswap/v3-core/contracts/interfaces/ILovelySwapV3Pool.sol';
import '@lovelyswap/v3-periphery/contracts/libraries/OracleLibrary.sol';
...
abstract contract OracleSlippage is IOracleSlippage, PeripheryImmutableState, BlockTimestamp {
    ...
    function getBlockStartingAndCurrentTick(ILovelySwapV3Pool pool)
        internal
        view
    ...
    function getPoolAddress(
        ...
        address tokenB,
        uint24 fee
    ) internal view virtual returns (ILovelySwapV3Pool pool) {
        pool = ILovelySwapV3Pool(PoolAddress.computeAddress(factory, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }
        ...
        returns (int256 syntheticAverageTick, int256 syntheticCurrentTick)
    {
        ...
        for (uint256 i = 0; i < numPools; i++) {
            ...
            ILovelySwapV3Pool pool = getPoolAddress(tokenIn, tokenOut, fee);
            ...
            if (secondsAgo == 0) {
                ...
                (, currentTick, , , , , ) = ILovelySwapV3Pool(pool).slot0();
            }
...
interface ILovelySwapV3PoolImmutables {
    /// @notice The contract that deployed the pool, which must adhere to the ILovelySwapV3Factory interface
...
/// per transaction
interface ILovelySwapV3PoolState {
...
interface ILovelySwapV3PoolDerivedState {
...
interface ILovelySwapV3PoolActions {
    ...
    /// @dev The caller of this method receives a callback in the form of ILovelySwapV3MintCallback#LovelySwapV3MintCallback
    ...
    /// @dev The caller of this method receives a callback in the form of ILovelySwapV3SwapCallback#LovelySwapV3SwapCallback
    ...
    /// @dev The caller of this method receives a callback in the form of ILovelySwapV3FlashCallback#LovelySwapV3FlashCallback
...
interface ILovelySwapV3PoolOwnerActions {
...
interface ILovelySwapV3PoolEvents {
...
import '@lovelyswap/v3-core/contracts/libraries/FullMath.sol';
import '@lovelyswap/v3-core/contracts/libraries/TickMath.sol';
import '@lovelyswap/v3-core/contracts/interfaces/ILovelySwapV3Pool.sol';
...
library OracleLibrary {
    /// @notice Calculates time-weighted means of tick and liquidity for a given LovelySwap V3 pool
        ...
        returns (int24 arithmeticMeanTick, uint128 harmonicMeanLiquidity)
    {
        ...
        (int56[] memory tickCumulatives, uint160[] memory secondsPerLiquidityCumulativeX128s) =
            ILovelySwapV3Pool(pool).observe(secondsAgos);
    ...
    /// @param pool Address of LovelySwap V3 pool that we want to observe
    /// @return secondsAgo The number of seconds ago of the oldest observation stored for the pool
    function getOldestObservationSecondsAgo(address pool) internal view returns (uint32 secondsAgo) {
        (, , uint16 observationIndex, uint16 observationCardinality, , , ) = ILovelySwapV3Pool(pool).slot0();
        require(observationCardinality > 0, 'NI');

        (uint32 observationTimestamp, , , bool initialized) =
            ILovelySwapV3Pool(pool).observations((observationIndex + 1) % observationCardinality);

        // The next index might not be initialized if the cardinality is in the process of increasing
        // In this case the oldest observation is always in index 0
        if (!initialized) {
            (observationTimestamp, , , ) = ILovelySwapV3Pool(pool).observations(0);
        }
    ...
    /// @param pool Address of LovelySwap V3 pool
    /// @return The tick that the pool was in at the start of the current block
    function getBlockStartingTickAndLiquidity(address pool) internal view returns (int24, uint128) {
        (, int24 tick, uint16 observationIndex, uint16 observationCardinality, , , ) = ILovelySwapV3Pool(pool).slot0();
        ...
        (uint32 observationTimestamp, int56 tickCumulative, uint160 secondsPerLiquidityCumulativeX128, ) =
            ILovelySwapV3Pool(pool).observations(observationIndex);
        if (observationTimestamp != uint32(block.timestamp)) {
            return (tick, ILovelySwapV3Pool(pool).liquidity());
        }
        ...
        ) = ILovelySwapV3Pool(pool).observations(prevIndex);
...
/// @notice Wraps LovelySwap V3 positions in a non-fungible token interface which allows for them to be transferred
/// and authorized.
...
import '@lovelyswap/v3-periphery/contracts/base/PeripheryValidation.sol';
...
abstract contract PeripheryValidation is BlockTimestamp {
    modifier checkDeadline(uint256 deadline) {
        require(_blockTimestamp() <= deadline, 'T1');
        _;
    }
}