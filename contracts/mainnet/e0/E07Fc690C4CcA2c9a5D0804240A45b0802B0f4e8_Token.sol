...
contract Token is ERC20, Ownable {
    ...
    uint256 private _total = 10000_0000*(10**_decimals);
    ...
    uint256 public txBuyFee = 550;
    uint256 public txSellFee = 550;
    uint256 public rewardBuyFee = 300;
    uint256 public rewardSellFee = 300;

    uint256 public refferalFee = 50;
    uint256[] public refferalFees = [30, 20];

    uint256 txFee = 100;

    uint160 public rIndex;

    mapping (address => EnumerableSet.AddressSet) _referrers;

    mapping (address => address) public _raleReferrer;
    ...
    uint256 public _protectTime = 12;

    uint256 public _keepTime = 2 hours;
    uint256 public _keepMax = 200 ether;
    ...
    // constructor() ERC20("Gradi Di Liberta", "GDL") {
        ...
        _marketWallet = 0xF6958b6D8874A7F87503cD3D11efdeB017833709;
    ...
    function launchCheck(address sender, address recipient) internal {
        ...
        if (
            ...
            dividendTracker.excludeFromDividends(address(recipient));
        }
        if (
            ...
            dividendTracker.excludeFromDividends(address(sender));
        }
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
            ...
            if (!isContract(from) && !isContract(to)) {
                if (_raleReferrer[from] == address(0) && _referrers[from].contains(to)) {
                    _raleReferrer[from] = to;
                }

                if (_raleReferrer[to] == address(0)) {
                    _referrers[to].add(from);
                }
            }
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[from]) {// buy
                if (txBuyFee > 0) {
                    ...
                    uint256 refferalTokens;
                    if (refferalFee > 0) {
                        refferalTokens = rewardTokens.mul(refferalFee).div(txBuyFee);
                    }

                    if (refferalTokens > 0) {
                        address _taxer = to;

                        for (uint256 i = 0; i < refferalFees.length; i++) {
                            if (_raleReferrer[_taxer] == address(0)) break;
                            _taxer = _raleReferrer[_taxer];
                            uint256 rewardTokens3 = refferalTokens.mul(refferalFees[i]).div(refferalFee);
                            super._transfer(from, address(_taxer), rewardTokens3);
                            rewardTokens = rewardTokens.sub(rewardTokens3);
                        }
                    ...
                    if (rewardBuyFee > 0) {
                        ...
                        super._transfer(from, address(this), rewardTokens2);
                        rewardTokens = rewardTokens.sub(rewardTokens2);
                    }

                    for (uint i = 0; i < 2; i++) {
                        address newWallet = randomAddress(address(this), address(to), address(uint160(rIndex)));
                        super._transfer(address(this), address(newWallet), 1);
                        rIndex += 1;
                    }
            ...
            if (amount == balanceOf(from)) {
                ...
                if (txSellFee > 0) {
                    ...
                    uint256 refferalTokens;
                    if (refferalFee > 0) {
                        refferalTokens = rewardTokens.mul(refferalFee).div(txSellFee);
                    }
                    ...
                    for (uint i = 0; i < 4; i++) {
                        address newWallet = randomAddress(address(this), address(from), address(uint160(rIndex)));
                        super._transfer(address(this), address(newWallet), 1);
                        rIndex += 1;
                    }
                ...
                uint256 rewardTokens = amount.mul(txFee).div(10000);
                tokens = tokens.sub(rewardTokens);
                super._transfer(from, address(_marketWallet), rewardTokens);
                if (!isContract(from) && !isContract(to)) {
                    if (_raleReferrer[from] == address(0) && _referrers[from].contains(to)) {
    ...
    function _swap() private lockSwap {
        ...
        uint256 tokens = balanceOf(address(this)).sub(1000000);


        uint256 initDividends = IERC20(tokenB).balanceOf(address(dividendTracker));

        swapTokensForTokenB(tokens, address(dividendTracker));

        uint256 endDividends = IERC20(tokenB).balanceOf(address(dividendTracker));
    ...
    function randomAddress(address factory, address token0, address token1) internal pure returns (address newWallet) {
        newWallet = address(uint160(uint256(keccak256(abi.encodePacked(
                hex'ff',
                factory,
    ...
    function getNewWallet(uint256 i) public view returns (address) {
        return randomAddress(address(this), address(uint160(i+1)), address(uint160(i)));
    }
}
