...
contract SRG20 is IERC20, Context, Ownable, ReentrancyGuard {
    ...
    // token data
    string private constant _name = "baby surge";
    string private constant _symbol = "bsrg";
    ...
    function withdrawTaxBalance() external nonReentrant onlyOwner {
        uint256 oldTaxBalance = taxBalance;
        uint256 teamSend = (taxBalance * teamShare) /
            (teamShare + treasuryShare);
        bool temp1 = SRGIE.transfer(teamWallet, teamSend);
        bool temp2 = SRGIE.transfer(treasuryWallet, oldTaxBalance - teamSend);
        assert(temp1 && temp2);
        taxBalance = 0;
    }
