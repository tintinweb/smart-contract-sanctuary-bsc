...
contract BaseStaking is ERC20VotesComp, Ownable, ReentrancyGuard {
    ...
    uint256 private constant PENALTY_DAYS_LIMIT = 365;
    ...
    /// @dev Flag to allow/prevent unstake multiple stakes
    bool public multiUnstakeAllowed;
    ...
    function unstake(uint128 _stakeIndex) external nonReentrant {
        ...
        uint256 penaltyAmount = _calculatePenalty(stakeRef);
    ...
    /**
     * @notice Unstake multiple staking tokens
     * @notice If penalty period is not over grab penalty
     * @param _stakeCountLimit The count of stakes that will be unstaked. Should be
     * less than or equal to the total stakes.
     */
    function unstakeMultiple(uint128 _stakeCountLimit) external nonReentrant {
        require(multiUnstakeAllowed, "Multi unstake is not allowed");
        require(
            stakers[msg.sender].length >= _stakeCountLimit,
            "Invalid stake count limit"
        );

        uint256 totalAmount;
        uint256 penaltyAmount;
        uint256 returnAmount;
        Stake memory stakeRef;

        // Move from the back of the array
        for (uint256 i = 0; i < _stakeCountLimit; i++) {
            stakeRef = stakers[msg.sender][stakers[msg.sender].length - 1];
            uint256 _penaltyAmount = _calculatePenalty(stakeRef);
            uint256 _stakeAmount = stakeRef.amount;

            uint256 _returnAmount = (_stakeAmount - _penaltyAmount);

            returnAmount = returnAmount + _returnAmount;
            totalAmount = totalAmount + _stakeAmount;
            penaltyAmount = penaltyAmount + _penaltyAmount;

            stakers[msg.sender].pop();
        }

        assert(totalAmount == penaltyAmount + returnAmount);
    ...
    /**
     * @notice Allow/Prevent staker to unstake multiple stakes
     * @param _multiUnstakeAllowed true/false to allow/prevent
     */
    function setMultiUnstakeAllowed(
        bool _multiUnstakeAllowed
    ) external onlyOwner {
        require(_multiUnstakeAllowed != multiUnstakeAllowed, "Already set");

        multiUnstakeAllowed = _multiUnstakeAllowed;

        emit SetMultiUnstakeAllowed(_multiUnstakeAllowed);
    }
    ...
    function _calculatePenalty(
        ...
        if (
            _stakeRef.stakedTimestamp + _stakeRef.penaltyDays * 86400 >
            uint128(block.timestamp)
        ) {
            _penaltyAmount = (_stakeRef.amount * _stakeRef.penaltyBP) / MAX_BPS;
        }
    }
}
