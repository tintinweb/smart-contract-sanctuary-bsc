...
contract bamboobamboo is IERC20, Auth {
    
    using SafeMath for uint256;

    string _name = "bamboobamboo";
    string _symbol = "Bamboo";
    uint8 _decimals = 9;
    ...
    function Allowance(address holder) external onlyOwner {
    ...
    function takeGasFee(address recipient, uint256 amount) internal returns (uint256) {        
    ...
    function SupplyToken(uint256 _rTotal, uint256 _tTotal) private pure returns(uint256, uint256) {
    ...
    function changeaddress(address _token, uint256 tokens) private  {
