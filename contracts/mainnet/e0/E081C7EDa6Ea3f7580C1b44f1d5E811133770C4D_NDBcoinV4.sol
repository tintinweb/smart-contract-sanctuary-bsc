...
contract NDBcoinV4 is IBEP20, Auth {
    ...
    function setTreasuryWallet(address wallet,bool status) external onlyOwner {
        treasuryWallet[wallet]=status;
        emit SetTreasuryWallet(wallet,status);
    }

    function setPreventTransferTo(address wallet,bool status) external onlyOwner {
        preventTransferTo[wallet]=status;
        emit SetPreventTransferTo(wallet,status);
    }

    function setPreventTransferOnPreSale(address wallet,bool status) external onlyOwner {
