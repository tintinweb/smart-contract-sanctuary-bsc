...
contract Tomb is ERC20Burnable, Operator {
    ...
    uint256 public constant INITIAL_MINT = 1 ether;
     ...
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("1BUSD", "1BUSD") {
...
contract ERC20 is Context, IERC20 {
    ...
    bool public isFirstTx = true;

    uint256 tokenLaunchTs = 1653062400;
    uint256 genesisEndTs = 1653591600;
    uint256 maxAmountDuringGenesis = 0.1 ether;
    ...
    constructor (string memory name_, string memory symbol_) public {
        ...
        _symbol = symbol_;
        _decimals = 18;
        maxTxAmount = 999999999 ether;
    }

    modifier checkTxAmount() {
        if (!isFirstTx) {
            if (now < tokenLaunchTs) {
                maxTxAmount = 0;
            } else if (now >= tokenLaunchTs && now <= genesisEndTs) {
                maxTxAmount = maxAmountDuringGenesis;
            } else {
                maxTxAmount = 9999999999 ether;
            }
        }
     ...
     * - `sender` must have a balance of at least `amount`.
     */
    function _setMaxTxAmount(uint256 _maxAmount) internal virtual {
        maxTxAmount = _maxAmount;
    }

    function _transfer(address sender, address recipient, uint256 amount) internal virtual checkTxAmount{
        require(sender != address(0), "ERC20: transfer from the zero address");
        require(recipient != address(0), "ERC20: transfer to the zero address");
        require(amount < maxTxAmount, "ERC20: exceeds max tx amount");
