...
contract PancakeSwapAcl is OwnableUpgradeable, UUPSUpgradeable {
    address public safeAddress;
    address public safeModule;

    bytes32 private _checkedRole = hex"01";
    uint256 private _checkedValue = 1;
    string public constant NAME = "PancakeSwapAcl";
    uint public constant VERSION = 1;
    

    uint256 public constant SLIPPAGE_BASE = 10000;
    uint256 public constant DECIMALS_BASE = 10 ** 18;
    ...
    address public pancakeFactory;                                            // PancakeSwap: Factory v2 contract
    ...
    function initialize(
        ...
        __PancakeSwapAcl_init(_safeAddress, _safeModule);
    }

    function __PancakeSwapAcl_init(
        address _safeAddress,
        address _safeModule
    ) internal onlyInitializing {
        __Ownable_init();
        __UUPSUpgradeable_init();
        __PancakeSwapAcl_init_unchained(_safeAddress, _safeModule);
    }

    function __PancakeSwapAcl_init_unchained(
        address _safeAddress,
        address _safeModule
    ) internal onlyInitializing {
    ...
    function setPancakeFactory(address _pancakeFactory) external onlySafe returns (bool){
        require(_pancakeFactory != address(0), "_pancakeFactory not allowed");
        require(pancakeFactory != _pancakeFactory, "_pancakeFactory existed");
    ...
    // get pair addr from pancakeswap v2 factory
    function getPair(address tokenA, address tokenB) internal view returns (address pair) {
        pair = IPancakeSwapV2Factory(pancakeFactory).getPair(tokenA, tokenB);
    }
    ...
    function pairCheck(address[] calldata _path) internal {  
        for (uint i=0; i < _path.length - 1; i++) { 
            require(swapPairWhitelist[_checkedRole][getPair(_path[i], _path[i+1])], "Invalid pair");
        }
    }
    ...
    function routerCheckAcl(uint256 _tokenInAmount, uint256 _tokenOutAmount, address[] calldata _path, address _to) internal {
        address _inToken = address(_path[0]);
        address _outToken = address(_path[_path.length - 1]); 
        require(_to == safeAddress,"to not allowed");
        if(isCheckSwapToken){
    ...
    function smartRouterCheckAcl(uint256 _tokenInAmount, uint256 _tokenOutAmount, address _tokenIn, address _tokenOut) internal {
        if(isCheckSwapToken){
            swapInOutTokenCheck(_tokenIn,_tokenOut);
        }

        if(isCheckRoleSlippage){
            slippageCheckLiquidity(_tokenInAmount,_tokenOutAmount,_tokenIn,_tokenOut);
        }
    }
    ...
    function swapETHForExactTokens(uint amountOut, address[] calldata path, address to, uint deadline)
        ...
        payable
        onlySelf {
        routerCheckAcl(_checkedValue, amountOut, path, to);
    }
    ...
    function swapExactETHForTokens(uint amountOutMin, address[] calldata path, address to, uint deadline)
        ...
        payable
        onlySelf {
        routerCheckAcl(_checkedValue, amountOutMin, path, to);
    }

    function swapExactTokensForETH(uint amountIn, uint amountOutMin, address[] calldata path, address to, uint deadline)
        external
        onlySelf {
        routerCheckAcl(amountIn, amountOutMin, path, to);
    }
    ...
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        external
        onlySelf {
        routerCheckAcl(amountInMax, amountOut, path, to);
    }
    ...
    function swap(
        ...
        address _inToken = address(srcToken);
        address _outToken = address(dstToken); 

        smartRouterCheckAcl(amount, minReturn, _inToken, _outToken);
    }
    ...
    function swapMulti(
        ...
        address _inToken = address(tokens[0]);
        address _outToken = address(tokens[tokens.length - 1]); 
