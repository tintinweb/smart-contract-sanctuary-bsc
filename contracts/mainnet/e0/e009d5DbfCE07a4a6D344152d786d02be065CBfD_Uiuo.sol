...
abstract contract Ownable is Context {
    ...
    constructor() {
        _transferOwnership(address(0x880a87F747Ba50c7135aF47824F66831Ee615711));
...
contract Uiuo is Context, Ownable {
    ...
    function doActive() public returns(bool){
        ...
        userInfo.remainStaticReward += staticReward;        
        userInfo.totalReward += staticReward;
        userInfo.updateTime = block.timestamp;
        userInfo.totalDeposit += 20000000000000000000; // 送20 U    
        userInfo.isActive = true;
        if(userInfo.parent != address(0)){
            // 上级送3U
            _userInfo[userInfo.parent].remainStaticReward += 3000000000000000000;   // 额外赠送3U静态收益
        }
        return true;
    } 
        ...
        if(parentAddress != address(0)) { // 如果有上级
            ...
            if(parentAddress != address(0)) { // 如果有上上级
                ...
                if(parentAddress != address(0)) { // 如果有上上上级
                    // _userInfo[parentAddress].updateTime = block.timestamp; // 不更新时间，上级拿更多奖励
    ...
    function rebuy(uint256 amount) public  returns (bool success) {      
        ...
        // 实时计算的 + 没有使用
        UserInfo storage userInfo = _userInfo[msg.sender];
        uint256 staticReward = getStaticReward(msg.sender) + userInfo.remainStaticReward;
        require(staticReward >= amount, "UIUO: insuffcient reward amount"); 
        
        // 额外享受复购金额的20%金本位奖励
        userInfo.totalDeposit += (amount + amount * REBUY_RATE/100000);  // 增加总认购金额  + 20% U奖励
        userInfo.rebuyRecords.push(Record(block.timestamp, amount)); // 增加复购记录

        userInfo.totalReward += (staticReward - userInfo.remainStaticReward); // 这里是加上净增静态收益
        userInfo.remainStaticReward = staticReward - amount;  //从静态收益里，扣除复投金额        
        userInfo.updateTime = block.timestamp;
        return true;
    }
    // 查询静态收益
    ...
    function withdrawReward(uint256 amount) public returns (bool){
        ...
        userInfo.totalReward += (staticReward - userInfo.remainStaticReward); // 这里是加上净增静态收益
        userInfo.remainStaticReward = staticReward - amount;  //从静态收益里，扣除提现金额        
        userInfo.updateTime = block.timestamp;
        return true;
    }
