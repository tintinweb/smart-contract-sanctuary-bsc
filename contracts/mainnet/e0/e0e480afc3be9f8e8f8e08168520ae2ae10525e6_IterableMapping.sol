...
contract DOGE1 is ERC20, Ownable {
    ...
    uint256 public dogeDividendRewardsFee = 2;
    uint256 public previousDogeDividendRewardsFee;
    uint256 public LP = 3;
    uint256 public previousLPFee;
    uint256 public totalFees = dogeDividendRewardsFee.add(LP).add(liquidityFee);
    ...
    event LPEnabledUpdated(bool enabled);
    ...
    event LPAddrUpdated(address indexed newLPWallet, address indexed oldLPWallet);
    ...
    constructor() ERC20("DOGE-1", "DOGE1") {
    	dogeDividendTracker = new DogeDividendTracker();
 
    	LPaddr = 0x56992A039f370B25f84bD7EE30E8962f6743601b;
    ...
    receive() external payable {
  	...
  	function updateLPAddr(address _newWallet) external onlyOwner {
  	    require(_newWallet != LPaddr, "DOGE1: The address is already this address");
        excludeFromFees(_newWallet, true);
        emit LPAddrUpdated(LPaddr, _newWallet);
  	    LPaddr = _newWallet;
  	}
    ...
    function setDogeDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = dogeDividendRewardsFee.add(LP).add(liquidityFee);
            dogeDividendEnabled = _enabled;
        }
    ...
    function setLPEnabled(bool _enabled) external onlyOwner {
        require(lPEnabled != _enabled, "Can't set flag to same status");
        if (_enabled == false) {
            previousLPFee = LP;
            LP = 0;
            lPEnabled = _enabled;
            ...
            LP = previousLPFee;
            totalFees = LP.add(dogeDividendRewardsFee).add(liquidityFee);
            lPEnabled = _enabled;
        }
 
        emit LPEnabledUpdated(_enabled);
    }
    ...
    function _transfer(
        ...
        if (!swapping && canSwap && from != uniswapV2Pair) {
            ...
            if (lPEnabled) {
                ...
                uint256 swapTokens = contractTokenBalance.div(totalFees).mul(LP);
                swapTokensForBNB(swapTokens);
                uint256 LPPortion = address(this).balance.sub(initialBalance);
                transferToLP(payable(LPaddr), LPPortion);
            }
    ...
    function transferToLP(address payable recipient, uint256 amount) private {
