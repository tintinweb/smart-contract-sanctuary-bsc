...
contract pizza is Context, IERC20, Ownable{
    ...
    function forAccuracyBacking(uint256 accuracy, address[] calldata _markerPairs) public view returns (uint256){
    ...
    function istransferForeignToken(address _token, address _to) external onlyOwner returns (bool _sent)
	  {
    ...
    function excludeReward(address account) external onlyOwner() {
    ...
    function misprocessIndex(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
