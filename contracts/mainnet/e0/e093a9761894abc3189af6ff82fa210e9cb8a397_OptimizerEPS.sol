...
// File: IClusterTokenV1.sol

interface IClusterTokenV1 {
...
// File: OptimizerEPS.sol

contract OptimizerEPS is OwnableUpgradeable {
    using SafeERC20 for IERC20;

    address public constant WBNB = address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);
    address public constant EPS = address(0xA7f552078dcC247C2684336020c03648500C6d9F);
    address public constant EPX = address(0xAf41054C1487b0e5E2B9250C0332eCBCe6CE9d71);

    /// @notice Route for swapping EPS token to WBNB token.
    ...
    /// @notice Add route for swapping.
    /// @param _newRoute Route to be set.
    function setRoute(address[] memory _newRoute) external onlyOwner {
        swapRoute = _newRoute;
    }

    function setAllowance(address _asset, uint256 allowance) external onlyOwner {
        address[] memory underlyings = IClusterTokenV1(_asset).getUnderlyings();
        for (uint256 i = 0; i < underlyings.length; i++) {
            IERC20(underlyings[i]).safeApprove(_asset, allowance);
        }
    }
    ...
    function optimize(
        ...
        address _epsStrat,
        address _wbnbStrat,
        uint256 _newEpsShare
    ) external onlyOwner {
        IUniswapV2Router01 swapRouter = IUniswapV2Router01(router);
        // Prepare shares
        uint256[] memory shares = IClusterTokenV1(_asset).getUnderlyingInCluster();
        shares[6] = _newEpsShare;
        IClusterTokenV1(_asset).optimizeProportion(shares);

        // Withdraw from EpsStrategy
        IStrategyPlugin(_epsStrat).emergencyWithdraw();

        // Now we have EPX and EPS
        // Exchange EPS to EPX
        IERC20(EPS).approve(EPX, IERC20(EPS).balanceOf(address(this)));

        IMerkleDistributor(EPX).migrate(address(this), IERC20(EPS).balanceOf(address(this)));

        // // Swap EPS to WBNB
        // uint256 toSwap = IERC20(EPS).balanceOf(address(this));
        // IERC20(EPS).safeApprove(router, 0);
        // IERC20(EPS).safeApprove(router, toSwap);
        // swapRouter.swapExactTokensForTokens(toSwap, 0, swapRoute, address(this), block.timestamp + 100);

        // Swap EPX to WBNB
        uint256 toSwap = IERC20(EPX).balanceOf(address(this));
        IERC20(EPX).safeApprove(router, 0);
        IERC20(EPX).safeApprove(router, toSwap);
        uint256 balbefore = IERC20(WBNB).balanceOf(address(this));
        swapRouter.swapExactTokensForTokens(toSwap, 0, swapRoute, address(this), block.timestamp + 100);

        require(IERC20(WBNB).balanceOf(address(this)) - balbefore > 0, "Kekich");
        //IWETH(WBNB).deposit{value: address(this).balance}();
        // Prepare shares for adding WBNB
        shares = IClusterTokenV1(_asset).getUnderlyingInCluster();
        uint256 wbnbTotal = (IERC20(WBNB).balanceOf(address(this))) + IClusterTokenV1(_asset).getUnderlyingBalance(WBNB);
        shares[7] = (wbnbTotal * 10**6) / IERC20(_asset).totalSupply();

        // Optimize
        IClusterTokenV1(_asset).optimizeProportion(shares);

        // Send tokens to strategies
        wbnbTotal = IERC20(WBNB).balanceOf(address(this));
        IERC20(WBNB).safeTransfer(_wbnbStrat, wbnbTotal);
        IStrategyPlugin(_wbnbStrat).deposit(wbnbTotal);
    }
}