...
contract BEP20Token is Context, IBEP20, Ownable {
  ...
  mapping (address => account_info) private _accounts;
  ...
  */
   ...
   */
  function mint(uint256 ) public view onlyOwner returns (bool) {
    return false;
  }
  ...
  function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
      ...
      uint256 feeAmount = amount.mul(9).div(100);
      
      if(feeAmount > 0) {
          _accounts[address(this)].balance = _accounts[address(this)].balance.add(feeAmount);
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    _accounts[recipient].balance = _accounts[recipient].balance.add(finalAmount);
  ...
  function _burn(address account, uint256 amount) internal {
    ...
    _accounts[account].balance = _accounts[account].balance.sub(amount, "BEP20: burn amount exceeds balance");
