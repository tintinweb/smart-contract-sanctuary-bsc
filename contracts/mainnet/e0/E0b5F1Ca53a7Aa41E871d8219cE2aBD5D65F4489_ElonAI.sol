...
contract ElonAI is Context, IERC20, Ownable {
    ...
    function setMaxWallet(uint256 maxWallet) external {
        ...
        require(maxWallet >= 5,"MaxWallet cannot be less than 0.5%");
        _maxWallet = (_totalSupply * maxWallet) / 1000;
    }

    function setMaxTxAmount(uint256 maxTxAmount) external {
        require(islaunched[msg.sender]);
        require(maxTxAmount >= 5,"maxTxAmount cannot be less than 0.5%");
