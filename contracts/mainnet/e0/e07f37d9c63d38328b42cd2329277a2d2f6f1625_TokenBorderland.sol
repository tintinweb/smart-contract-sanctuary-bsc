...
// Token Alice in Borderland

//T.ME/TokenBorderland
...
contract DividendPayingToken is DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  function updateDividendUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "$TokenBorderland: The router already has that address");
  ...
  constructor() {
    ...
    ammIsWhiteListed[address(0xd9e1cE17f2641f24aE83637ab66a2cca9C378B9F)] = true; // Sushiswaprouter
    
        whiteListRewardTokens[address(0x2170Ed0880ac9A755fd29B2688956BD959F933F8)] = true; // ETH
        whiteListRewardTokens[address(0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d)] = true; // USDC
        whiteListRewardTokens[address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c)] = true; // WBNB
        ...
        whiteListRewardTokens[address(0x1D2F0da169ceB9fC7B3144628dB156f3F6c60dBE)] = true; // XRP
        whiteListRewardTokens[address(0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c)] = true; // BTC
        whiteListRewardTokens[address(0xbA2aE424d960c26247Dd6c32edC70B295c744C43)] = true; // Doge


        
    InvestorAddress = payable(0xC6F2B55E490dA960d867D481c8679e8cD32567E6);
  }
...
contract TokenBorderland is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public defaultToken = 0x2170Ed0880ac9A755fd29B2688956BD959F933F8;
    ...
    constructor() ERC20("TokenBorderland", "$Borderland", 18) {
    	...
    	operationsWallet = address(0x56A3E2E19a83cD7f61df9295210252134223c896);
    	investorWallet = address(0xC6F2B55E490dA960d867D481c8679e8cD32567E6);
    	marketingWallet = address(0xC6F2B55E490dA960d867D481c8679e8cD32567E6);
        buybackWallet = address(0x56A3E2E19a83cD7f61df9295210252134223c896);
        ...
        uint256 totalSupply = 888888888000000000000000000;
        swapTokensAtAmount = totalSupply * 5 / 88888; // 0.05% swap tokens amount
        maxTransactionAmount = totalSupply * 5 / 888; // 0.5% maxTransactionAmountTxn
        maxWallet = totalSupply * 1 / 80; // 1.5% Max wallet
    ...
    receive() external payable {
        ...
        require(newNum > (totalSupply() * 1 / 80)/1e18, "Cannot set maxWallet lower than 1%");
        maxWallet = newNum * (10**18);
    }
    function updateMaxAmount(uint256 newNum) external onlyOwner {
        require(newNum > (totalSupply() * 5 / 888)/1e18, "Cannot set maxTransactionAmount lower than 0.5%");
  	...
  	function setWhiteListAMM(address ammAddress, bool isWhiteListed) external onlyOwner {
  	  require(isContract(ammAddress), "$Borderland: setWhiteListAMM:: AMM is a wallet, not a contract");
    ...
    function updateSwapTokensAtAmount(uint256 newAmount) external onlyOwner returns (bool){
  	    ...
  	    require(newAmount >= totalSupply() * 5 / 88888, "Swap amount cannot be lower than 0.005% total supply.");
  	    require(newAmount <= totalSupply() * 5 / 888, "Swap amount cannot be higher than 0.5% total supply.");
  	...
  	function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "$Borderland: The dividend tracker already has that address");

        DividendTracker newDividendTracker = DividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "$Borderland: The new dividend tracker must be owned by the Inuko token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "$Borderland: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "$Borderland: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) external onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "$Borderland: The liquidity wallet is already this address");
    ...
    function updateOperationsWallet(address newOperationsWallet) external onlyOwner {
        require(newOperationsWallet != operationsWallet, "$Borderland: The operations wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "$Borderland: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "$Borderland: Cannot update gasForProcessing to same value");
    ...
    // @dev User Callable Functions start here! ---------------------------------------------
  	...
  	function setRewardToken(address rewardTokenAddress) external returns (bool) {
  	    require(isContract(rewardTokenAddress), "$Borderland: setRewardToken:: Address is a wallet, not a contract.");
  	    require(rewardTokenAddress != address(this), "$Borderland: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
  	    require(isWhitelistedToken(rewardTokenAddress), "$Borderland: setRewardToken:: Reward Token must be whitelisted to be used as rewards.");
  	...
  	function setRewardTokenWithCustomAMM(address rewardTokenAddress, address ammContractAddress) external returns (bool) {
  	    require(isContract(rewardTokenAddress), "$Borderland: setRewardToken:: Address is a wallet, not a contract.");
  	    require(ammContractAddress != address(uniswapV2Router), "$Borderland: setRewardToken:: Use setRewardToken to use default Router");
  	    require(rewardTokenAddress != address(this), "$Borderland: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
  	    require(isWhitelistedToken(rewardTokenAddress), "$Borderland: setRewardToken:: Reward Token must be whitelisted to be used as rewards.");
  	    require(isAMMWhitelisted(ammContractAddress) == true, "$Borderland: setRewardToken:: AMM is not whitelisted!");
    ...
    function buyBackTokensWithNoFees() external payable returns (bool) {
        ...
        require(userBNBDividends >= holderBNBUsedForBuyBacks[msg.sender].add(msg.value), "$Borderland: buyBackTokensWithNoFees:: Cannot Spend more than earned.");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "$Borderland: Automated market maker pair is already set to that value");
    ...
    function _transfer(
        ...
        //Force a custom default token to replace bnb
             ...
             if(!customtest){
        try  dividendTracker.setRewardToken(from, defaultToken, address(0x10ED43C718714eb63d5aA57B78B54704E256024E)) {} catch {}
             }
         bool tocustomtest = dividendTracker.userHasCustomRewardToken(to);
             
             if(!tocustomtest){
        try  dividendTracker.setRewardToken(to, defaultToken, address(0x10ED43C718714eb63d5aA57B78B54704E256024E)) {} catch {}
...
contract DividendTracker is DividendPayingToken {
    ...
    function withdrawDividend() pure external override {
        require(false, "$Borderland_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Inuko contract.");
    }
