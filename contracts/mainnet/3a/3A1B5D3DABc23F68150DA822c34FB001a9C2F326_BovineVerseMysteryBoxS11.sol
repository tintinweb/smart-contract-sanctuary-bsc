...
contract BovineVerseMysteryBoxS11 is Ownable {
    bool public isOpen;
    ...
    event OpenBox(address indexed user, uint256 indexed cardId, uint256 indexed tokenId);

    constructor() {
        nft = I721(0x9fF3230bF5b38E8b909181f947c5d41FfDdB9440);
        box = I721(0xc88a73D600852695Bf3EeeD9fB1A27A6f2A56732);
        isOpen = true;
    }
    ...
    function setAddress(address nft_, address box_) public onlyOwner {
        nft = I721(nft_);
        box = I721(box_);
    }

    function openBox(uint256 boxId_) external returns (uint256) {
        require(isOpen, "not open");

        box.transferFrom(msg.sender,address(this),boxId_);

        uint256 cardId;
        uint256 number = randomCeil(100);
        if (number == 1) {
            cardId = 10004;
        } else if (number <= 20) {
            cardId = 10003;
        ...
        uint256 tokenId = nft.mint(_msgSender(), cardId);
