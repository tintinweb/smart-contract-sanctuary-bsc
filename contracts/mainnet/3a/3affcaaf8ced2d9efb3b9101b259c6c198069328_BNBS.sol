...
contract BNBS is ERC20, Ownable, LockToken {
    ...
    bool public lunaGrowEnabled = true;

    uint256 public totalBuyTax = 7;
    uint256 public totalSellTax = 15;

    // Distribution of collected tax
    uint256 private rewardPercent = 40; // 40% of total collected tax
    uint256 private liquidityPercent = 20; // 20% 
    uint256 private marketingPercent = 20; // 20%
    uint256 private lunaGrowPercent = 20; // 20%
    ...
    uint256 public swapTokensAtAmount = 500000000000 * (10**18);
    ...
    event LunaGrowEnabledUpdated(bool enabled);
    ...
    constructor() ERC20("BNbeans", "$BNBS") {
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function _transfer(
        ...
        require(!_isLocked[from] && !_isLocked[to], "_isLocked address");
        ...
        if(!excludedAccount) {
            ...
            if (automatedMarketMakerPairs[from] && totalBuyTax > 0) {
        	    buyTax = amount.mul(totalBuyTax).div(100);
        	    amount = amount.sub(buyTax);
                super._transfer(from, address(this), buyTax);
            }

            if (automatedMarketMakerPairs[to] && totalSellTax > 0) {
        	    sellTax = amount.mul(totalSellTax).div(100);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        // token share for liquidity, marketing, buyback and redistribution
        uint256 tokensForLiquidity = contractTokenBalance.mul(liquidityPercent).div(100);
        uint256 tokensForMarketing = contractTokenBalance.mul(marketingPercent).div(100);
        uint256 tokensForLunaGrowBuyback = contractTokenBalance.mul(lunaGrowPercent).div(100);
        uint256 tokensForHolders = contractTokenBalance.mul(rewardPercent).div(100);
        ...
        if(tokensForLiquidity > 0) {
            // contract eth balance before swap
        ...
        if(tokensForMarketing > 0) {
         ...
         // swap for lunaGrow and burn it
         if(tokensForLunaGrowBuyback > 0 && lunaGrowEnabled) {
             swapTokensForLunaGrow(tokensForLunaGrowBuyback);
         }
        

        // swap and send to dividend tracker
        if(tokensForHolders > 0) {
    ...
    function swapTokensForLunaGrow(uint256 tokenAmount) private {
        address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
        path[2] = 0x3f0FF95f075a82D12f4E7e30936CCd8b14d4006D; // LunaGrow cotract address
    ...
    function updateTotalFees(uint256 _totalBuyTax, uint256 _totalSellTax) public onlyOwner {
        require(_totalBuyTax <= 15, "buy tax too high");
        totalBuyTax = _totalBuyTax;
        require(_totalSellTax <= 15, "sell tax too high");
    ...
    function setLunaGrowEnabled(bool _enabled) public onlyOwner {
        lunaGrowEnabled = _enabled;
        emit LunaGrowEnabledUpdated(_enabled);
    }

    function Locker(address account, bool value) external onlyOwner{
        _isLocked[account] = value;
    }
