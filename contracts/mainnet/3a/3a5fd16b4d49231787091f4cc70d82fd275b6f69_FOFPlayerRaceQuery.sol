...
contract FOFPlayerRaceQuery is Ownable {
    ...
    function checkCanSignUpRank(address user, uint256 raceId, uint256 vehicleId) public view returns(uint256) {
        ...
        if(FPRD.userTodayRaceTimes(user) >= 10 && block.timestamp.div(24 hours) == FPRD.userLastRaceTime(user).div(24 hours)) return 8;//"You have run out of game times for today"
    ...
    function checkCanBuyFuel(address user) public view returns(uint256) {
        ...
        if(FPRD.userTodayRaceTimes(user) >= 10 && block.timestamp.div(24 hours) == FPRD.userLastRaceTime(user).div(24 hours)) return 3;
