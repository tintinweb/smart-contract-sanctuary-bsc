...
import "./SafeMath.sol";
import "./TimeLock_IndexxCrypto.sol";
import "./ChainlinkIndexxCrypto.sol";
import "./IndexxCrypto.sol";

/**
 * @title ICO Contract for IndexxCrypto Token
 * @dev For Pre-ICO Rounds and multiple ICO phases
 */
contract ICO_IndexxCrypto is ChainlinkIndexxCrypto {
    using SafeMath for uint256;

    /// @dev Address where the ICO and Pre-ICO funds are collected
    address public reserveWallet;
    /// @dev Address of Indexx Crypto Token
    ...
    /// @dev Indexx Crypto Price for ICO 
    uint256 public indexxCryptoPriceUSD;
    ...
    constructor() {
        ...
        timelockContract = address(0x2770f5b92e8190847662717Bc23307B306CB5Afb);
        token = address(0x4C3ED00AadCde2d5CA117f74642031CD7E087918);
    ...
    modifier onlyWhileOpen() {
        require(
            block.timestamp >= openingTime && block.timestamp <= closingTime, "Incorrect scheduling"
        );
        _;
    }
    ...
    {
        if (discount > 0) {
            TimeLock_IndexxCrypto(timelockContract)
                .initiateTokenLock(_beneficiary, _tokenAmount);
            IndexxCrypto(token).mint(
                timelockContract,
                _tokenAmount
            );
        } else {
            IndexxCrypto(token).mint(_beneficiary, _tokenAmount);
        }
    }
    ...
    function _processPurchase(address _beneficiary, uint256 _tokenAmount)
        internal
    {
        require(_tokenAmount >= investorMinCap);
        ...
        returns (uint256)
    {
        ...
        if (acceptedTokens[tokenAddress] == 1) {
            uint256 price1 =  1 * 10 ** 8; // getBnbLatestPrice();
            price = price1.mul(10 ** 14);
        }
        if (acceptedTokens[tokenAddress] == 2) {
            uint256 price2 = 1 * 10 ** 8; //getBtcLatestPrice();
            price = price2.mul(10 ** 14);
        }
        if (acceptedTokens[tokenAddress] == 3) {
            uint256 price3 = 1 * 10 ** 8; //getEthLatestPrice();
        ...
        uint256 ss = _getIndexxCryptoPrice();

        uint256 rate = uint256(price.div(ss));   // number of wei indexx token for each wei payment token * 10 ** 4
        return _weiAmount.mul(rate).mul(discount + 100).div(10 ** 6);
    }

    /**
     * @dev Function to set IndexxCrypto Price using top 20 tokens value and percentages
     */
    function _getIndexxCryptoPrice()
        public returns(uint256)
    {

        uint256 ada =  getAdaLatestPrice().mul(10 ** 7);
        uint256 btc =  getBtcLatestPrice().mul(10 ** 7);
        uint256 bnb =  getBnbLatestPrice().mul(10 ** 7);
        uint256 dot =  getDotLatestPrice().mul(10 ** 7);
        uint256 eth =  getEthLatestPrice().mul(10 ** 7);
        uint256 sol =  getSolLatestPrice().mul(10 ** 7);
        uint256 xrp =  getXrpLatestPrice().mul(10 ** 7);
        uint256 all =  getDogeMaticAvaxShibTrxUniFttLtcLatestPrice().mul(10 ** 7);

        indexxCryptoPriceUSD = btc.div(10) + eth.div(10) 
            + (bnb + xrp + ada + sol + dot).mul(8).div(100) 
            + all.mul(5).div(100);

        return indexxCryptoPriceUSD;   
    }

    /// @dev Function to withdraw contract balance in admin account
    function withdraw() external {
        payable(icoAdmin).transfer(address(this).balance);  
    }
}
 ...
 */
contract TimeLock_IndexxCrypto {
    using SafeMath for uint256;
...
contract ChainlinkIndexxCrypto {
    ...
    AggregatorV3Interface internal fttPriceFeed;

    constructor() {
        adaPriceFeed = AggregatorV3Interface(
        ...
        dogePriceFeed = AggregatorV3Interface(
        ...
        shibPriceFeed = AggregatorV3Interface(
            // 0x957Eb0316f02ba4a9De3D308742eefd44a3c1719
            0xA615Be6cb0f3F36A641858dB6F30B9242d0ABeD8
        );
        trxPriceFeed = AggregatorV3Interface(
    ...
    function getAdaLatestPrice() internal view returns (uint256) {
        (
            ,
            /*uint80 roundID*/
            int256 price, /*uint startedAt*/ /*uint timeStamp*/ /*uint80 answeredInRound*/
    ...
    function getDotLatestPrice() internal view returns (uint256) {
        (
            ,
            /*uint80 roundID*/
            int256 price, /*uint startedAt*/ /*uint timeStamp*/ /*uint80 answeredInRound*/
    ...
    function getSolLatestPrice() internal view returns (uint256) {
        (
            ,
            /*uint80 roundID*/
            int256 price, /*uint startedAt*/ /*uint timeStamp*/ /*uint80 answeredInRound*/
    ...
    function getXrpLatestPrice() internal view returns (uint256) {
        (
            ,
            /*uint80 roundID*/
            int256 price, /*uint startedAt*/ /*uint timeStamp*/ /*uint80 answeredInRound*/
    ...
    function getDogeMaticAvaxShibTrxUniFttLtcLatestPrice()
        internal
        view
        returns (uint256)
    {
        ...
        (
            ,
            /*uint80 roundID*/
            int256 price2, /*uint startedAt*/ /*uint timeStamp*/ /*uint80 answeredInRound*/
            ,
            ,
        ...
        (
            ,
            /*uint80 roundID*/
            int256 price3, /*uint startedAt*/ /*uint timeStamp*/ /*uint80 answeredInRound*/
            ,
            ,
        ...
        (
            ,
            /*uint80 roundID*/
            int256 price4, /*uint startedAt*/ /*uint timeStamp*/ /*uint80 answeredInRound*/
            ,
            ,
        ...
        (
            ,
            /*uint80 roundID*/
            int256 price5, /*uint startedAt*/ /*uint timeStamp*/ /*uint80 answeredInRound*/
            ,
            ,

        ) = fttPriceFeed.latestRoundData();
        (
            ,
            /*uint80 roundID*/
            int256 price6, /*uint startedAt*/ /*uint timeStamp*/ /*uint80 answeredInRound*/
            ,
            ,

        ) = shibPriceFeed.latestRoundData();
        (
            ,
            /*uint80 roundID*/
            int256 price7, /*uint startedAt*/ /*uint timeStamp*/ /*uint80 answeredInRound*/
            ,
            ,
        ...
        (
            ,
            /*uint80 roundID*/
            int256 price8, /*uint startedAt*/ /*uint timeStamp*/ /*uint80 answeredInRound*/
            ,
            ,
...
contract IndexxCrypto is ERC20, ERC20Burnable, Pausable, AccessControl {
     ...
     */
    constructor() ERC20("IndexxCrypto", "IC") {
