...
contract NfterriumNet is Initializable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function processUplines(uint256 _userId, uint256 _amount) internal {
        ...
        while (row > 1 && counter < 12) {
            ...
            if ((users[uplineId].row <= 2) || (!isExpired(uplineId) && checkLevel(counter, users[uplineId].status))) {
                ...
                counter++;
            } else if (!isExpired(uplineId) && !checkLevel(counter, users[uplineId].status)) {
                counter++;
            }
            row--;
        }
    ...
    function calculateVipPrice(uint256 _userId) internal view returns (uint256) {
        ...
        if (users[_userId].terraCount == 0) {
            totalVipPrice = totalVipPrice.add(5);
        }
    ...
    function buyVip(uint8 _count) external payable whenNotPaused {
        ...
        uint256 toFightPool = vipPriceNomad.mul(_count).sub(vipPriceNomad.mul(_count).div(5).mul(10**26).div(rate));
        if (users[userId].terraCount > 0) {
            toFightPool = toFightPool.add(users[userId].terraCount.mul(vipPriceTerra.div(5)).mul(_count).mul(10**26).div(rate));
            processUplines(userId, users[userId].terraCount.mul(vipPriceTerra).mul(_count).mul(10**26).div(rate));
        }
