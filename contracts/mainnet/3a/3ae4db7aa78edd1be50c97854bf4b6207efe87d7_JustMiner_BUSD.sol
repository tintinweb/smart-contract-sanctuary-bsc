...
// |   JustMiner BUSD   |
// --------*--------
// The busd.justminer.site (BUSD) 

// Website : https://justminer.site
...
contract JustMiner_BUSD {
	using SafeMath for uint256;
	using SafeERC20 for IERC20;
    
    IERC20 public token = IERC20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); //BUSD Token Contract

	uint256 public INVEST_MIN_AMOUNT = 10 ether; // 10 BUSD
	uint256[] public REFERRAL_PERCENTS = [160, 30, 10];
	...
	constructor(address payable wallet) {
        ...
        plans.push(Plan(360, 15)); // 360 days 1.5% daily // 540%
        plans.push(Plan(180, 17)); // 180 days 1.7% daily // 306%
        plans.push(Plan(90, 26)); // 90 days 2.6% daily // 240%
    ...
    function setting(uint256 min_investment, uint256 Max_withdraw, uint256 w_limit, uint64 ref1, uint64 ref2, uint64 ref3) public owner {
                                 
        INVEST_MIN_AMOUNT = min_investment  * 1 ether;
        MAX_WITHDRAW = Max_withdraw  * 1 ether;
        limit = w_limit  * 1 ether;
	...
	function set_Bonus() external {
        require(msg.sender == devWallet, "Admin use only.");
