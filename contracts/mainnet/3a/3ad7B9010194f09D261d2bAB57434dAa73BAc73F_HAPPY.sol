...
contract HAPPY is ERC20, Ownable{
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (!swapping && swapEnabled && from != pair && taxAmt > 0) {
            handle_fees();
        }

        super._transfer(from, to, amount - taxAmt);
        if(taxAmt - treasuryAmt > 0) {
            super._transfer(from, address(this), taxAmt - treasuryAmt);
        }
        if(treasuryAmt > 0) {
    ...
    function handle_fees() private inSwap {
        ...
        if (contractBalance >= swapThreshold) {
            ...
            uint256 denominator = (sellFees.lp + sellFees.marketing + sellFees.development) * 2;
            ...
            uint256 developmentAmt = unitBalance * 2 * sellFees.development;
            if(developmentAmt > 0){
