...
contract Deagle is Initializable, UUPSUpgradeable, ERC20Upgradeable, ERC20PermitUpgradeable, ERC20VotesUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    using SafeMathUpgradeable for uint256;
    using SafeERC20Upgradeable for IERC20Upgradeable;
    IERC20Upgradeable public eagle;
    ...
    struct UserInfo {
        ...
        uint256 rewardEAGLEDebt; // Reward debt in EAGLE.
        //
        // We do some fancy math here. Basically, any point in time, the amount of EAGLE
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accEAGLEPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accEAGLEPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. EAGLEs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that EAGLEs distribution occurs.
        uint256 accEAGLEPerShare; // Accumulated EAGLEs per share, times 1e12. See below.
        uint256 lastTotalEAGLEReward; // last total rewards
        uint256 lastEAGLERewardBalance; // last EAGLE rewards tokens
        uint256 totalEAGLEReward; // total EAGLE rewards tokens
    }
    ...
    // Bonus muliplier for early EAGLE makers.
    ...
    // total EAGLE staked
    uint256 public totalEAGLEStaked;
    // total EAGLE used for purchase land
    uint256 public totalEagleUsedForPurchase;
    ...
    function initialize(        
        ...
        __ERC20_init_unchained("dEagle", "dtestEagle");
        __Pausable_init_unchained();
        ERC20PermitUpgradeable.__ERC20Permit_init("dEagle");
    ...
    function add(uint256 _allocPoint, IERC20Upgradeable _lpToken, bool _withUpdate) public onlyOwner {
        ...
        poolInfo.push(PoolInfo({
            ...
            accEAGLEPerShare: 0,
            lastTotalEAGLEReward: 0,
            lastEAGLERewardBalance: 0,
            totalEAGLEReward: 0
        }));
    }

    // Update the given pool's EAGLE allocation point. Can only be called by the owner.
    ...
    // View function to see pending EAGLEs on frontend.
    function pendingEAGLE(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accEAGLEPerShare = pool.accEAGLEPerShare;
        uint256 lpSupply = totalEAGLEStaked;
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 rewardBalance = pool.lpToken.balanceOf(address(this)).sub(totalEAGLEStaked.sub(totalEagleUsedForPurchase));
            uint256 _totalReward = rewardBalance.sub(pool.lastEAGLERewardBalance);
            accEAGLEPerShare = accEAGLEPerShare.add(_totalReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accEAGLEPerShare).div(1e12).sub(user.rewardEAGLEDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 rewardBalance = pool.lpToken.balanceOf(address(this)).sub(totalEAGLEStaked.sub(totalEagleUsedForPurchase));
        uint256 _totalReward = pool.totalEAGLEReward.add(rewardBalance.sub(pool.lastEAGLERewardBalance));
        pool.lastEAGLERewardBalance = rewardBalance;
        pool.totalEAGLEReward = _totalReward;
        
        uint256 lpSupply = totalEAGLEStaked;
        if (lpSupply == 0) {
            pool.lastRewardBlock = block.number;
            pool.accEAGLEPerShare = 0;
            pool.lastTotalEAGLEReward = 0;
            user.rewardEAGLEDebt = 0;
            pool.lastEAGLERewardBalance = 0;
            pool.totalEAGLEReward = 0;
            return;
        }
        
        uint256 reward = _totalReward.sub(pool.lastTotalEAGLEReward);
        pool.accEAGLEPerShare = pool.accEAGLEPerShare.add(reward.mul(1e12).div(lpSupply));
        pool.lastTotalEAGLEReward = _totalReward;
    }

    /**
    @notice Sorting the highest EAGLE staker in pool
    @param _pid : pool id
    @param left : left
    @param right : right
    @dev Description :
        It is used for sorting the highest EAGLE staker in pool. This function definition is marked
    ...
    */
    ...
    @dev Description :
    DAO governance will be performed by the top 50 wallets with the highest amount of staked EAGLE tokens.
    */
    ...
    /**
    @notice EAGLE staking track the Highest 50 staked users
    @param _pid : pool id
    ...
    */
    ...
    // Deposit EAGLE tokens to MasterChef.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            uint256 eagleReward = user.amount.mul(pool.accEAGLEPerShare).div(1e12).sub(user.rewardEAGLEDebt);
            pool.lpToken.safeTransfer(msg.sender, eagleReward);
            pool.lastEAGLERewardBalance = pool.lpToken.balanceOf(address(this)).sub(totalEAGLEStaked.sub(totalEagleUsedForPurchase));
        }
        pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        totalEAGLEStaked = totalEAGLEStaked.add(_amount);
        user.amount = user.amount.add(_amount);
        user.rewardEAGLEDebt = user.amount.mul(pool.accEAGLEPerShare).div(1e12);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        totalEAGLEStaked = totalEAGLEStaked.sub(_amount);
        user.rewardEAGLEDebt = user.amount.mul(pool.accEAGLEPerShare).div(1e12);
    ...
    // Earn EAGLE tokens to MasterChef.
    function claimEAGLE(uint256 _pid) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
    ...
    // Safe EAGLE transfer function to admin.
    function accessEAGLETokens(uint256 _pid, address _to, uint256 _amount) public {
        require(msg.sender == adminAddress, "sender must be admin address");
        require(totalEAGLEStaked.sub(totalEagleUsedForPurchase) >= _amount, "Amount must be less than staked EAGLE amount");
        PoolInfo storage pool = poolInfo[_pid];
        uint256 EagleBal = pool.lpToken.balanceOf(address(this));
        if (_amount > EagleBal) {
            pool.lpToken.transfer(_to, EagleBal);
            totalEagleUsedForPurchase = totalEagleUsedForPurchase.add(EagleBal);
            emit EmergencyWithdraw(_to, _pid, EagleBal);
        } else {
            pool.lpToken.transfer(_to, _amount);
            totalEagleUsedForPurchase = totalEagleUsedForPurchase.add(_amount);
