...
contract ERC20 is Context, IERC20, IERC20Metadata, ReentrancyGuard {
    ...
    string private _name = "IOXPlatform";
    string private _symbol = "IOX";
...
contract IOXPlatform is ERC20, Ownable {
    ...
    address internal constant Team_addr = 0x2E1033A0Fd2FAad2058D6DDceb006213dcbdABf8; // 
																									 
																									 
																									 
    address internal marketing_addr = 0xA4b3eD6e8703972Dc4027d0A2c1A0e0A02e456e7;
    address internal buyBack_addr = 0xfBe79C33969493f5478cD7bF2c719637e0117fE0;
    ...
    /* % from every day's lobby entry dedicated to team, marketing and buy back */
    uint256 internal constant DM_Team_percentage = 5;      
    ...
    function flushFirstDayLobbyEntry(uint256 l) external onlyOwner() nonReentrant {
        require(firstDayFlushed == false);
        firstDayFlushed = true;

        payable(Team_addr).transfer((lobbyEntry[l] * 93) /100);  
    }
    ...
    /* Gets called once a day and withdraws team's share for the privious day of lobby */ 
    function _sendDevShare() internal nonReentrant {
        require(currentDay > 0);

        // TeamPercentage = 5% of every day's lobby entry
        uint256 TeamPercentage = (lobbyEntry[currentDay - 1] * DM_Team_percentage) /100;
    
								
        payable(Team_addr).transfer(TeamPercentage);
     ...
     * @param amount Amount of IOX tokens to stake
    ...
    function EnterStake(uint256 amount, uint256 stakingDays) external {
        ...
        /* On stake IOX tokens get burned */
    ...
    /**
     * @dev Calculating a stakes Bonus IOX tokens based on stake duration and stake amount
     * @param StakeDuration The stake's days
     * @param StakeAmount The stake's IOX tokens amount
     */
