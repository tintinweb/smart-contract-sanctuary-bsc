...
contract AutomationOrderIncrease is AutomationCompatible, Ownable {
    ...
    //---------- Modifiers ----------//
    modifier onlyKeeper() {
        require(allowed.contains(_msgSender()), "AOI: forbidden");
        _;
    }
    ...
    function _validateMaxGlobalSize(
        ...
        if (_isLong) {
            uint256 maxGlobalLongSize = positionManager.maxGlobalLongSizes(
                _indexToken
            );
            if (
                maxGlobalLongSize > 0 &&
            ...
            uint256 maxGlobalShortSize = positionManager.maxGlobalShortSizes(
                _indexToken
            );
            if (
    ...
    function _validateOrder(bytes32 _orderKey) private view returns (bool) {
        (
            address _account,
            address _purchaseToken,
            uint256 _purchaseTokenAmount,
            ...
            bool _triggerAboveThreshold, // executionFee

        ) = orderBook.getIncreaseOrder(_orderKey);
        ...
        if (!_validateMaxGlobalSize(_indexToken, _isLong, _sizeDelta)) {
            return false;
        }

        if (!positionManager.shouldValidateIncreaseOrder()) {
            return isPriceValid;
        }
    ...
    function performUpkeep(
        ...
        require(_validateOrder(orderId), "Not executable");
        try
            positionManager.executeIncreaseOrder(orderId, keeperFunds)
        {} catch {
            orderBook.cancelIncreaseOrder(orderId);
        }
    }
