 ...
 */
contract XYZZFreeNodeStakePool is Ownable {
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        return user.totalDepositAmount.mul(accTokenPerShare).div(1e12).sub(user.claimedReward);
    }

    function myReferralReward(uint256 _pid,address _user) public view returns(uint256 referralBonus){
		PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 totalDeservedReward = user.totalDepositAmount.mul(pool.accTokenPerShare).div(1e12).sub(user.claimedReward);
    ...
    function claim(uint256 _pid) public {
		...
		updatePool(_pid);
        uint256 pending = user.totalDepositAmount.mul(pool.accTokenPerShare).div(1e12).sub(user.claimedReward).add(myReferralReward(_pid,msg.sender));
        require(pending > 0,"You have no reward tokens to claim");
        if(pending > 0){
            safeTokenTransfer(msg.sender, pending);
        }
        user.claimedReward = user.totalDepositAmount.mul(pool.accTokenPerShare).div(1e12).add(myReferralReward(_pid,msg.sender));
