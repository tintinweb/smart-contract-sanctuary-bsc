...
contract XMEN is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("X-MEN", "X-MEN") {
	...
	function swapAndLiquify() public {
		...
		uint256 canswap = allAmount.div(8).mul(7);
		uint256 otherAmount = allAmount.sub(canswap);
		uint256 usdtBalance = ETH.balanceOf(address(this));
		swapTokensForOther(canswap);
		uint256 allBalance = ETH.balanceOf(address(this));
		if(allBalance > usdtBalance){
			uint256 newBalance = allBalance.sub(usdtBalance);
			ETH.transfer(_fundAddress,newBalance.div(4).mul(2));
			if(newBalance.mul(otherAmount) > 10**20){
				addLiquidityUsdt(newBalance.div(4), otherAmount);
			}
		}
    }
