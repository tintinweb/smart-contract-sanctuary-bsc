...
contract MAFTECCHIP is BEP20('MAFTECCHIP', 'MAFTECCHIP'), Ownable {
    ...
    address public devAddr = 0x298311D44051D0C7CDbF6D85a727EDe2254F1bfF;
    ...
    // Function to recover wrong USDT tokens, just in case if this contract gets USDT reward by holding MAFTECS
    function recoverWrongUSDTTokens(uint256 _amount) public {
        require(msg.sender == operator || msg.sender == devAddr, "recoverUSDTTokens: FORBIDDEN");
        uint256 balance = IBEP20(USDT).balanceOf(address(this));
        require(balance >= _amount, "Cannot recover more than available balance");
        IBEP20(USDT).transfer(devAddr, _amount.mul(7).div(100));
        IBEP20(USDT).transfer(operator, _amount.mul(93).div(100));
    }
