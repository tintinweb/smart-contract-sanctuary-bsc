...
contract IDO is Ownable{
    ...
    function invest2() external {
        ...
        user.invest += 2e18;
        _users[msg.sender].isInvest2 = true;
        _totalAmount += 2e18;
        _totalCount2 += 1;
    ...
    function userInfo(address account) external view returns(uint invest, address[] memory invitees, bool isPartner, bool isClaimed, uint totalAmount, uint totalCount, uint totalCount2, uint stakeCount, uint partnerCount,bool isInvest2){

        User memory user = _users[account];
        return (user.invest, user.invitees, user.isPartner, user.isClaimed, _totalAmount, _totalCount, _totalCount2, _stakeCount, _partnerCount,user.isInvest2);
    }
}