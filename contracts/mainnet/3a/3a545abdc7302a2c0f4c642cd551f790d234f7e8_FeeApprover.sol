...
contract FeeApprover is OwnableUpgradeSafe {
    ...
    function initialize(
        ...
        address _contractVaultAddress
    ) public initializer {
        OwnableUpgradeSafe.__Ownable_init();
        contractTokenAddress = _CONTRACTAddress;
        WBNBAddress = _WBNBAddress;
        tokenBakerySwapPair = IBakerySwapFactory(address(0x01bF7C66c6BD861915CdaaE475042d3c4BaE16A7)).getPair(WBNBAddress,contractTokenAddress);
        contractVaultAddress = _contractVaultAddress;
        feePercentX100 = 20;
    ...
    function setContractVaultAddress(address _contractVaultAddress) public onlyOwner {
        contractVaultAddress = _contractVaultAddress;
    }
        ...
        )
    {
        ...
        // Dont have a fee when contractVault is sending, or infinite loop
        // And when pair is sending (buys are happening, do not tax on it)
        if(sender == contractVaultAddress || sender == tokenBakerySwapPair ) {
            console.log("Sending without fee");
            transferToFeeDistributorAmount = 0;
            transferToAmount = amount;
        } else {
            console.log("Normal fee transfer");
            transferToFeeDistributorAmount = amount.mul(feePercentX100).mul(1000);
