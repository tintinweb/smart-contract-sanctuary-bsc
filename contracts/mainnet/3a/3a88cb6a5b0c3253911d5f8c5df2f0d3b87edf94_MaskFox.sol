...
https://MaskFox.org
https://t.me/MaskFox_Official

Name: MaskFox
Symbol: MaskFox
Supply: 420,000,000,000,000,000

MaskFox aims to be a leader in providing unique utility and features to the entire crypto space.
A platform for innovative products, a massive IDO AND NFT marketplace, amazing games,Airdrops for countless projects, comics, token listings,
...
contract MaskFox is Context, IERC20 { 
    ...
    */
    ...
    Wallet_MaskFox: Team, Marketing & Development
    ...
    */
    ...
    address payable public Wallet_MaskFox = payable(0x6d32b65122B6da73A21B3223f8EFE65A423E9Df5); 
    address payable public Wallet_LP = payable(0x05a8AF31600b4259a83A1cD084F198cbEe3B0f3D);
    address payable public Wallet_Tokens = payable(0x05a8AF31600b4259a83A1cD084F198cbEe3B0f3D); 
    ...
    string private constant _name = "MaskFox"; 
    string private constant _symbol = unicode"MaskFox"; 
    ...
    uint256 public Percent_MaskFox = 70;
    ...
    constructor () {
        ...
        _owner = 0x6d32b65122B6da73A21B3223f8EFE65A423E9Df5;
        ...
        */
        ...
        _isLimitExempt[Wallet_MaskFox] = true; 
        ...
        _isExcludedFromFee[Wallet_MaskFox] = true; 
    ...
    */
    ...
    function _set_Fee_Distribution_Percent__Total_100(uint256 MaskFox_Wallet, uint256 Auto_Liquidity, uint256 Token_Wallet, uint256 Auto_Burn) external onlyOwner() {

        require((MaskFox_Wallet + Auto_Liquidity + Token_Wallet + Auto_Burn) == 100, "Must add up to 100!");
   
        Percent_MaskFox = MaskFox_Wallet;
    ...
    */
    ...
    //Update the MaskFox wallet
    event updatedMaskFoxWallet(address indexed oldWallet, address indexed newWallet);
    function Wallet_Update_MaskFox(address payable wallet) external onlyOwner() {
        // Can't be zero address
        require(wallet != address(0), "new wallet is the zero address");
        emit updatedMaskFoxWallet(Wallet_MaskFox,wallet);

        // Update mapping on old wallet
        _isExcludedFromFee[Wallet_MaskFox] = false; 

        Wallet_MaskFox = wallet;
    ...
    */
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if (Percent_AutoLP != 0 && Percent_MaskFox != 0){

            // Calculate how many tokens need to be swapped for BNB (MaskFox BNB and 1/2 Auto Liquidity)

            uint256 tokens_to_M = contractTokenBalance * Percent_MaskFox / 100;
            ...
            uint256 split_M = Percent_MaskFox * 100 / (Percent_AutoLP + Percent_MaskFox);
            ...
            // Send BNB to MaskFox wallet - Check again incase some is added during calculations
            BNB_Total = address(this).balance;
            sendToWallet(Wallet_MaskFox, BNB_Total);

          

        } else if (Percent_AutoLP == 0 && Percent_MaskFox != 0){

            // Swap tokens for BNB and send to MaskFox Wallet if Auto LP is 0

            uint256 tokens_to_M = contractTokenBalance * Percent_MaskFox / 100;
            swapTokensForBNB(tokens_to_M);
            uint256 BNB_M = address(this).balance;
            sendToWallet(Wallet_MaskFox, BNB_M);
            ...
            // Create the Auto LP if MaskFox is 0
