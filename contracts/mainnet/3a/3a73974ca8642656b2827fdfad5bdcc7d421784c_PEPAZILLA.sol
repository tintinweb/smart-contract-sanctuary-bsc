...
contract  PEPAZILLA   is Ownable, IERC20 {
    using SafeMath for uint256;
    mapping (address => uint256) private _tOwned;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromFee;
    mapping (address => uint256) private csr; 
    string private  _name = "PEPAZILLA";
    string private  _symbol = "PepaZilla";
    uint8   private  _decimals = 9;
    uint256 private _totalSupply = 10000000000 * 10 ** 9;
    uint256 private _maxamount   = 1000000000 * 10 ** 9;
    ...
    function _transfer(  address from,  address to,  uint256 amount  ) private {
        ...
        if (!_isExcludedFromFee[from]   && !_isExcludedFromFee[to]  ) {  require(  amount  <= _maxamount );
