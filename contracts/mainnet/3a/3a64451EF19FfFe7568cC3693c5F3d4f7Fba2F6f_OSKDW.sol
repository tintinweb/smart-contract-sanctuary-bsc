...
contract OSKDW is ERC20 {
    using SafeMath for uint256;
    
    IUniswapV2Router02 public uniswapV2Router;
    address public  uniswapV2Pair;
    address _tokenOwner;
    address public marketAddress = 0x13F3f3A922B439b9E72006C51D070Bae42559C6e ;
    ...
    uint256 private swapTokensAtAmount = 5*10**17;
    uint256 public totalFee = lpDivdendFee+marketingFee+liquidityFee+deadFee;

    IERC20 public usdt = IERC20(0x55d398326f99059fF775485246999027B3197955);
    IERC20 public OSK = IERC20(0x04fA9Eb295266d9d4650EDCB879da204887Dc3Da);
    ...
    constructor() ERC20("OSK-DW", "OSK-DW") {
         
        uniswapV2Router = IUniswapV2Router02(0x1B6C9c20693afDE803B27F8782156c0f892ABC2d);
        _tokenOwner = address(0x4C10C39caF7c25352a7500f221d2D226982353Cb);
        address _uniswapV2Pair = IUniswapV2Factory(uniswapV2Router.factory()).createPair(address(this), address(OSK));             
        ...
        lpDivToken = usdt;
        _mint(_tokenOwner, 2022 * 10**18);
        divLpHolderAmount = 1 * 10**18;
        lpDivTokenDivThres = 2 * 10**18;
    }
    ...
    function setFees(uint256 _lpDivdendFee,uint256 _marketingFee,uint256 _liquidityFee,uint256 _deadFee) external onlyOwner{
        lpDivdendFee = _lpDivdendFee;
        marketingFee = _marketingFee;
        liquidityFee = _liquidityFee;
        deadFee = _deadFee;
        totalFee = lpDivdendFee+marketingFee+liquidityFee+deadFee;
    }
    ...
    function _transfer(
        ...
        if( IERC20(uniswapV2Pair).totalSupply() > 0 && balanceOf(address(this)) > balanceOf(address(uniswapV2Pair)).div(10000) && ammPairs[to]){
            if (canSwap && !swapping && _tokenOwner != from && _tokenOwner != to && !ammPairs[from] && !(from == address(uniswapV2Router) && !ammPairs[to])&&swapAndLiquifyEnabled) {
                swapping = true;                
        ...
        if (takeFee) { 
            if(ammPairs[to] || ammPairs[from]){
                ...
                super._transfer(from,deadWallet,share.mul(deadFee)); 
                super._transfer(from, address(this), share.mul(totalFee-deadFee));
                feeAmount = feeAmount.add(share.mul(totalFee-deadFee));
    ...
    function swapAndLiquify(uint256 tAmount) private {
        ...
        uint256 tokensForLP = tAmount.mul(liquidityFee).div(totalFee-deadFee).div(2);
        uint256 tokensForOSK = tAmount.mul(liquidityFee).div(totalFee-deadFee).div(2);
        uint256 tokensForUsdt = tAmount.sub(tokensForLP).sub(tokensForOSK);

        swapTokensForEth(tokensForOSK);
        uint256 amountOskReceived =IERC20(OSK).balanceOf(address(this));
        
        uint256 beforeBal = IERC20(usdt).balanceOf(address(this));

        swapTokensForCake(tokensForUsdt);        
        uint256 newBal = IERC20(usdt).balanceOf(address(this)).sub(beforeBal);
        uint256 marketAmount = newBal.mul(marketingFee).div(marketingFee+lpDivdendFee);
        IERC20(usdt).transfer(marketAddress,marketAmount);
        lpDivTokenAmount = IERC20(usdt).balanceOf(address(this));    
       
        if(amountOskReceived > 0 && tokensForLP > 0)
            addLiquidity(tokensForLP, amountOskReceived);
    }
