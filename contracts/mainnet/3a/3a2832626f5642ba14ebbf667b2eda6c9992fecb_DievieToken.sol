...
contract DievieToken is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    /// @notice EIP-20 token name for this token
    string public constant name = "DIEVIE Token";
    ...
    /// @notice Max gas price allowed for Metavie transaction
    ...
    uint96 public constant MAX_SUPPLY = 36000000 ether;
    uint96 public totalSupply;
    uint96 public minted;

    /// @notice Address of Metavie Treasury
    ...
    /**
     * @notice Construct a new Metavie token
     */
    ...
    function approve(address spender, uint256 rawAmount) public returns (bool) {
        ...
        if (rawAmount == type(uint256).max) {
            ...
            amount = safe96(rawAmount, "DieVie::approve: amount exceeds 96 bits");
        }
    ...
    function transfer(address dst, uint256 rawAmount) external returns (bool) {
        uint96 amount = safe96(rawAmount, "DieVie::transfer: amount exceeds 96 bits");
    ...
    function transferFrom(
        ...
        uint96 amount = safe96(rawAmount, "DieVie::approve: amount exceeds 96 bits");

        if (spender != src && spenderAllowance != type(uint96).max) {
            uint96 newAllowance = sub96(
                spenderAllowance,
                amount,
                "DieVie::transferFrom: transfer amount exceeds spender allowance"
            );
    ...
    function getPriorVotes(address account, uint256 blockNumber) public view returns (uint96) {
        require(blockNumber < block.number, "DieVie::getPriorVotes: not yet determined");
    ...
    function burnFrom(address account, uint256 rawAmount) public {
        ...
        require(currentAllowance >= amount, "DieVie: burn amount exceeds allowance");
    ...
    function delegateBySig(
        ...
        require(signatory != address(0), "DieVie::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "DieVie::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "DieVie::delegateBySig: signature expired");
    ...
    function _burn(address account, uint96 amount) internal {
        require(account != address(0), "DieVie: burn from the zero address");
    ...
    function _transferTokens(
        ...
        require(src != address(0), "DieVie::_transferTokens: cannot transfer from the zero address");
        require(dst != address(0), "DieVie::_transferTokens: cannot transfer to the zero address");
        require(
            !_isBlackListed[src] && !_isBlackListed[dst],
            "DieVie::_transferTokens: cannot transfer to/from blacklisted account"
        );
        require(tx.gasprice < gasPriceLimit, "DieVie::_transferTokens: gasprice limit");
        ...
        if (sellLimitActive && _isSellLimited[src]) {
            ...
            uint96 totalBalance = add96(
                balances[src],
                withdrawnAmount,
                "DieVie::_transferTokens: error with total balance"
            );
            ...
            require(
                ...
                "DieVie::_transferTokens: withdraw more than 50% of balance"
            );
        ...
        if ((!isDex[dst] && !isDex[src]) || (_dexTaxExcempt[dst] || _dexTaxExcempt[src]) ) {
            ...
            balances[src] = sub96(balances[src], amount, "DieVie::_transferTokens: transfer amount exceeds balance");
            balances[dst] = add96(balances[dst], amount, "DieVie::_transferTokens: transfer amount overflows");
            emit Transfer(src, dst, amount);

            _moveDelegates(delegates[src], delegates[dst], amount);
        } else {
            require(!isTradingPaused, "DieVie::_transferTokens: You cannot trade this token. Only Transfers are availiable");
            ...
            balances[dst] = add96(
                balances[dst],
                (amount),
                "DieVie::_transferTokens: transfer amount overflows"
            );
             //   _swapComissionsReceivedMetavie(tax,taxTarget);
            emit Transfer(src, dst, (amount ));

            _moveDelegates(delegates[src], delegates[dst], (amount));
            // _swapReceivedMetavie();
    ...
    function _moveDelegates(
        ...
        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                ...
                uint96 srcRepNew = sub96(srcRepOld, amount, "DieVie::_moveVotes: vote amount underflows");
            ...
            if (dstRep != address(0)) {
                ...
                uint96 dstRepNew = add96(dstRepOld, amount, "DieVie::_moveVotes: vote amount overflows");
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(block.number, "DieVie::_writeCheckpoint: block number exceeds 32 bits");
    ...
    function manageLimitExcempt(address[] memory users, bool[] memory _isUnlimited) public onlyOwner {
        require(users.length == _isUnlimited.length, "DieVie::_adminFunctions: Array mismatch");
    ...
    function manageBlacklist(address[] memory users, bool[] memory _toBlackList) public onlyOwner {
        require(users.length == _toBlackList.length, "DieVie::_adminFunctions: Array mismatch");
    ...
    function manageSellLimitExcempt(address[] memory users, bool[] memory _toLimit) public onlyOwner {
        require(users.length == _toLimit.length, "DieVie::_adminFunctions: Array mismatch");
    ...
    function mintFor(address account, uint96 amount) public onlyOwner {
        require(minted + amount <= MAX_SUPPLY, "DieVie::_adminFunctions: Mint more tokens than allowed");
