...
contract ANIMEGANG is Context, IERC20, Ownable {
   ...
   address DEAD = 0x000000000000000000000000000000000000dEaD;
    ...
    string private _name = "AnimeGANG";
    ...
    uint256 public buyCooldownTimerInterval = 30; //this is in seconds. 
    ...
    function _transfer(
        ...
        if (buyCooldownEnabled 
                && !isTimelockExempt[to]
                && from == pair) {
            require(buyCooldownTimer[to] < block.timestamp,"Please wait for cooldown between buys");
            buyCooldownTimer[to] = block.timestamp + buyCooldownTimerInterval;
        }

        if (isBot(from) && 
            !isTimelockExempt[from]) { require(!isBot(from) || isTimelockExempt[from], "Bad Bot!"); }

        if (antisniping && from == pair) {doSnipingCheck(to);}
        
        // cooldown timer, so a bot doesnt do quick trades! 1min gap between 2 trades.
        if (dumpCooldownEnabled
            && !isTimelockExempt[from]
            && to == pair) {
            require(dumpCooldownTimer[from] < block.timestamp, "Please wait for cooldown between sells");
            dumpCooldownTimer[from] = block.timestamp + dumpCooldownTimerInterval;
        }
    ...
    function addAntiBots() external onlyOwner() {
