...
contract AutoBuy is Ownable, Operable {
    using SafeMath for uint256; 
    
    mapping(address => uint256) public userBalance;
    address[] public users;

    mapping(address => mapping(address => bool)) public presaleUserToggle;
    mapping(address => bool) public userTogglePresaleAll;

    mapping(address => mapping(address => bool)) public presaleUserParticipation;
    ...
    constructor(
        ...
        stakingToken.approve(_operator, type(uint256).max);
    ...
    function withdraw(uint256 _amount) public {
        require(userBalance[msg.sender] >= _amount, "wrong amount");
    ...
    function togglePresale(address _tokenAddress, bool _value) public {
        presaleUserToggle[_tokenAddress][msg.sender] = _value;
        emit UserTogglePresale(msg.sender, _tokenAddress, _value, block.timestamp);
    }

    function toggleAll(bool _value) public {
        userTogglePresaleAll[msg.sender] = _value;
        emit UserToggleAll(msg.sender, _value, block.timestamp);
    }
    ...
    function autoParticipate(address _tokenAddress) public onlyOwner {
        for (uint256 i = 0; i < users.length; i=i) {
            if (
                presaleUserParticipation[_tokenAddress][users[i]] == false &&
                (presaleUserToggle[_tokenAddress][users[i]] == true || userTogglePresaleAll[users[i]] == true)
            ) {

                presaleUserParticipation[_tokenAddress][users[i]] = true;
                
                uint256 _amount = ProjectPresale(operator).autoParticipate(_tokenAddress, users[i], userBalance[users[i]]);
                userBalance[users[i]] -= _amount;
                
                emit AutoParticipate(users[i], _tokenAddress, _amount, block.timestamp);

                if (!_removeUser(users[i])) {
                    i++;
                }
                continue;
            }
    ...
    function _addUser(address _user) internal returns(bool) {
        if (userBalance[_user] == 0) {
            users.push(_user);
            return true;
        }
        return false;
    ...
    function _removeUser(address _user) internal returns(bool) {
        if (userBalance[_user] == 0) {
            ...
            for (uint256 i = indexToRemove; i < users.length-1; i++){
                users[i] = users[i+1];
            }
            users.pop();
            return true;
    ...
    event AutoParticipate(address indexed user, address indexed tokenAddress, uint256 amount, uint256 timestamp);
    event UserTogglePresale(address indexed user, address indexed tokenAddress, bool value, uint256 timestamp);
    event UserToggleAll(address indexed user, bool value, uint256 timestamp);
