...
/**               
Telegram : https://t.me/ShibaSuperIntellegence
Website : https://twitter.com/ShibaSuperAI
...
contract ShibSuperAI is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ShibSuperAIDividendTracker public dividendTracker;
    ...
    constructor() ERC20("ShibSuperAI", "SHIBAI") {
        marketingWallet = payable(0xD237b912eD8Ac4A35fc633F689F853a6Fe84C6B7);
        devWallet = payable(0xf961bb5b765821Bf9dd694916Ccc0655A551c0C8);
        ...
        dividendTracker = new ShibSuperAIDividendTracker(
            payable(this),
            router,
...
contract ShibSuperAIDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    ShibSuperAI public ShibSuperAIContract;
    ...
    constructor(
        ...
        defaultToken = token;
        ShibSuperAIContract = ShibSuperAI(mainContract);
    ...
    function _transfer(
        ...
        require(false, "ShibSuperAI_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "ShibSuperAI_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ShibSuperAI contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(ShibSuperAIContract);

            uint256 prevBalance = ShibSuperAIContract.balanceOf(address(this));
            ...
            {
                uint256 received = ShibSuperAIContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    ShibSuperAIContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
