...
interface StabilizeUtilities{
    ...
    function withdrawAvailable(address _contract, uint256 _share, uint256 _total) external returns (bool);
...
// DF rewards are earned from lending/borrowing and from farming. DF is sold for USX then repaid (plus a little WBNB)

// If Health ratio is too low, strategy is forced rebalanced

contract BUSDStrategyDforceV1 is Ownable {
    using SafeMath for uint256;
    ...
    uint256 public percentTradeTrigger = 25000; // 10% amount of free tokens will trigger a rebalance
    ...
    uint256 public usxMaxBorrowAmount = 500000e18;
    uint256 public usxBorrowPercent = 80000; // Borrows up to 80% of lent BUSD
    uint256 public usxSetPrice = 1e18; // Governance controls the price of USX
    ...
    function withdrawAvailable(uint256 _share, uint256 _total) public returns (bool) {
        // Checks whether the user's staked LP is enough to cover the cost of the decreasing loan
        return StabilizeUtilities(strategyUtilitiesAddress).withdrawAvailable(address(this), _share, _total);
    }
    ...
    function deposit(bool nonContract) public onlyZSBToken {
        ...
        if(_bal > balance().mul(percentTradeTrigger).div(DIVISION_FACTOR)){
            // Free BUSD balance is greater than a percentage of the total tokens deposited, stake more
            if(nonContract == true){
    ...
    */
    ...
    function claimRewards(bool _nopayout) internal {
        ...
        // This will convert rewards (DF) to USX and some to WBNB (via DF -> USX -> BUSD -> WBNB)
        ...
        uint256 _newUSX;
        if(_bal > 0){
            // Convert all DF to USX
            _newUSX = swapViaDodoV2(DODO_DF_USX_POOL, DF_ADDRESS, USX_ADDRESS, _bal, 1);
        }

        if(_newUSX > 0 && _nopayout == false){
            // Convert a percentage to WBNB
            _newUSX = _newUSX.mul(DIVISION_FACTOR.sub(percentStakeDepositor)).div(DIVISION_FACTOR);
            uint256 busdAmount = swapViaDodoV2(DODO_USX_BUSD_POOL, USX_ADDRESS, address(tokenList[0].token), _newUSX, 1);
            ...
            IERC20(path[0]).safeApprove(address(router), busdAmount);
            router.swapExactTokensForTokens(busdAmount, 1, path, address(this), now.add(60)); // Get WBNB
        }

        repayExcessUSX();
    }

    function repayExcessUSX() internal {
        uint256 _bal = IERC20(USX_ADDRESS).balanceOf(address(this));
        if(_bal > 0){
            // Use any USX in the contract to payoff the loan slowly
            DforceToken iusx = DforceToken(DFORCE_IUSX_POOL);
            uint256 borrowAmount = iusx.borrowBalanceCurrent(address(this));
            if(borrowAmount > 0){
                if(_bal > borrowAmount){ // Repay loan fully
                    _bal = borrowAmount;
                }
                IERC20(USX_ADDRESS).safeApprove(address(iusx), 0);
                IERC20(USX_ADDRESS).safeApprove(address(iusx), _bal);
                iusx.repayBorrow(_bal);
            }
        }
    }
    ...
    function unstakeAndReducePosition(bool total, uint256 _amountBUSD) internal {
        ...
        if(total == false){
            ...
            uint256 _supplied = IERC20(DFORCE_IBUSD_POOL).balanceOf(address(this)); // Check the token balance
            if(_supplied > 0){
        ...
        uint256 posUSX = IERC20(USX_ADDRESS).balanceOf(address(this));
        {
            ...
            if(total == false){
                // Remove slightly more than the proportion
                _bal = _bal.mul(proportionNeeded).div(1e18);//.mul(DIVISION_FACTOR.add(usxSlippage.div(10))).div(DIVISION_FACTOR);
                if(_bal > farm.balanceOf(address(this))){
                    _bal = farm.balanceOf(address(this));
                    total = true;
                }
            }
            if(_bal > 0){
        ...
        if(total == true){
            // Convert unstaked USX (or everything) into BUSD
            ...
            // Restake excess USX back into the pool
            repayExcessUSX();
    ...
    function expandPositionAndStake() internal {
        ...
        {
            // If there is any BUSD left over, convert it back to USX
            uint256 busdAmount = tokenList[0].token.balanceOf(address(this));
            if(busdAmount > 0){
                swapViaDodoV2(DODO_USX_BUSD_POOL, address(tokenList[0].token), USX_ADDRESS, busdAmount, 1);
            }
        }
    ...
    function checkAndSwapToken(address _executor, bool skipRatioCheck) internal {
        lastTradeTime = now;
        ...
        if(ratio > 0 && ratio < minHealthRatio && skipRatioCheck == false) {
            // We are at risk for liquidation
            // Force rebalance and pay executor from BUSD
            claimRewards(true); // Convert all rewards to only USX
    ...
    // --------------------
    function changeStrategyConditions(uint256 _pTradeTrigger, 
                                    ...
                                    uint256 _minHealth,
                                    uint256 _usxSlip,
                                    uint256 _maxUSwap) external onlyGovernance {
    ...
    // Used to update USX Price

    function updateUSXPrice(uint256 _newPrice) external onlyGovernance {
        require(_newPrice > 0, "Can't set 0 price");
