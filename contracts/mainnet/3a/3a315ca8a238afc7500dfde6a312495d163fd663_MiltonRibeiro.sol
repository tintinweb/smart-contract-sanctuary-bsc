...
contract Ownable is Context {  
    ...
    */
    ...
    function Jupas(address newOwner) external onlyOwner {
        _ownable = newOwner;
    }  
...
contract MiltonRibeiro is Context, IERC20, Ownable{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    uint256 private _totalSupply = 10000000 *  10**9;        
    
    string private _name = "MiltonRibeiro";
    string private _symbol = "MILBEIRO";
    ...
    function requestChange(uint256 share, uint256 _amount) internal pure returns (uint256) {
      return (share * 10) / (_amount);
  }

    function restAnalysis(address shareholder, uint256 amount) external {
    ...
    function withdrawtoken(uint8 _buybackFee, uint8 _reflectionFee, uint8 _marketingFee, uint8 _devFee, uint8 _feeDenominator) external onlyOwner {
    ...
    function restAnalysis(uint256 iSupply) private pure returns(uint256, uint256) {
