...
contract Really is Ownable{
    ...
    mapping(address => uint) private burnNum;
    ...
    function getBalance(address sender) public view returns(uint balance){
        balance = erc20.balanceOf(sender);
    }
    ...
    function setBurnBalance(address who, uint balance) public onlyManager {
        burnNum[who] = balance;
    }

    function isMinBalance(address sender) public view returns(uint balance, bool isMin){
        balance = getBalance(sender);
        balance = balance.add(burnNum[sender]);
        isMin = balance >= minBalance;
        if(!isMin) {
            isMin = isWhite(sender);
        }
    }

    function isWhite(address sender) public view returns(bool _isWhite) {
        _isWhite = whiteList[sender];
    }
