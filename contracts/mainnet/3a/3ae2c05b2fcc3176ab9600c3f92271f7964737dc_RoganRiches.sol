...
contract RoganRiches is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "Rogan Riches";
    string constant _symbol = "RR";
    uint8 constant _decimals = 18;

    uint256 _totalSupply = 420000000069 * (10 ** _decimals);
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        require(!shouldLimit(sender, amount, recipient), "Sell too big");
    ...
    function shouldLimit(address sender, uint256 amount, address recipient) internal view returns (bool) {
        if (recipient == pair) {
            // selling
