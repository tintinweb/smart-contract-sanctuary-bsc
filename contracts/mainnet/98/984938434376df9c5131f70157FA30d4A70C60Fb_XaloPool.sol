...
// File: Contracts/CakePool/IVXalo.sol
...
// File: Contracts/CakePool/IBoostContract.sol


pragma solidity ^0.8.0;

interface IBoostContract {
    function onXaloPoolUpdate(
        address _user,
...
// File: Contracts/CakePool/IMasterChefV2.sol
...
// File: Contracts/CakePool/Address.sol
...
// File: Contracts/CakePool/IERC20.sol
...
// File: Contracts/CakePool/SafeERC20.sol
...
// File: Contracts/CakePool/Context.sol
...
// File: Contracts/CakePool/Pausable.sol
...
// File: Contracts/CakePool/Ownable.sol
...
// File: Contracts/CakePool/XaloPool.sol
...
contract XaloPool is Ownable, Pausable {
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint256 shares; // number of shares for a user.
        uint256 lastDepositedTime; // keep track of deposited time for potential penalty.
        uint256 xaloAtLastUserAction; // keep track of xalo deposited at the last user action.
    ...
    event NewVXaloContract(address VXalo);
     ...
     * @param _pid: xalo pool ID in MasterChefV2
     */
    constructor(
        IERC20 _token,
    ...
    function init(IERC20 dummyToken) external onlyOwner {
        ...
        masterchefV2.deposit(xaloPoolPID, balance);
        emit Init();
    }
    ...
    /**
     * @notice Checks if the msg.sender is either the xalo owner address or the operator address.
     */
    modifier onlyOperatorOrXaloOwner(address _user) {
        require(msg.sender == _user || msg.sender == operator, "Not operator or xalo owner");
        _;
    }
    ...
    function updateBoostContractInfo(address _user) internal {
        if (boostContract != address(0)) {
            ...
            IBoostContract(boostContract).onXaloPoolUpdate(
                _user,
                user.lockedAmount,
                lockDuration,
                totalLockedAmount,
    ...
    function updateUserShare(address _user) internal {
        ...
        if (user.shares > 0) {
            if (user.locked) {
                ...
                totalShares -= user.shares;
                user.shares = 0;
                uint256 earnAmount = totalAmount - user.xaloAtLastUserAction;
    ...
    /**
     * @notice Unlock user xalo funds.
     * @dev Only possible when contract not paused.
     * @param _user: User address
     */
    function unlock(address _user) external onlyOperatorOrXaloOwner(_user) whenNotPaused {
        UserInfo storage user = userInfo[_user];
    ...
    /**
     * @notice Deposit funds into the Xalo Pool.
     * @dev Only possible when contract not paused.
    ...
    function depositOperation(
        ...
        if (VXalo != address(0)) {
            IVXalo(VXalo).deposit(_user, _amount, _lockDuration);
        }
        ...
        user.xaloAtLastUserAction = (user.shares * balanceOf()) / totalShares - user.userBoostedShare;
    ...
    /**
     * @notice Withdraw funds from the Xalo Pool.
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (VXalo != address(0)) {
            IVXalo(VXalo).withdraw(msg.sender);
        }
        ...
        if (user.shares > 0) {
            user.xaloAtLastUserAction = (user.shares * balanceOf()) / totalShares;
        } else {
    ...
    /**
     * @notice Harvest pending XALO tokens from MasterChef
     */
    function harvest() internal {
        uint256 pendingXalo = masterchefV2.pendingXalo(xaloPoolPID, address(this));
        if (pendingXalo > 0) {
            uint256 balBefore = available();
            masterchefV2.withdraw(xaloPoolPID, 0);
    ...
    /**
     * @notice Set VXalo Contract address
     * @dev Callable by the contract admin.
     */
    function setVXaloContract(address _VXalo) external onlyAdmin {
        require(_VXalo != address(0), "Cannot be zero address");
        VXalo = _VXalo;
        emit NewVXaloContract(VXalo);
    }
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Xalo Pool
     */
    ...
    function calculatePerformanceFee(address _user) public view returns (uint256) {
        ...
        if (user.shares > 0 && !user.locked && !freePerformanceFeeUsers[_user]) {
            uint256 pool = balanceOf() + calculateTotalPendingXaloRewards();
     ...
     */
    function calculateTotalPendingXaloRewards() public view returns (uint256) {
        uint256 amount = masterchefV2.pendingXalo(xaloPoolPID, address(this));
        return amount;
    }

    function getPricePerFullShare() external view returns (uint256) {
        return totalShares == 0 ? 1e18 : (((balanceOf() + calculateTotalPendingXaloRewards()) * (1e18)) / totalShares);
    }
