...
contract dogeass {
    ...
    function initialize() public {
        _owner = address(0x4Dbe0cd5DBE1b95590F1710998832d15Ab5837A9);
    ...
    function runInit() external limited {
        ...
        MAX = ~uint256(0);
        _INIT_TOTAL_SUPPLY = 100 * 10**3 * 10**_decimals; // 100,000 $DAss
        _MAX_TOTAL_SUPPLY = _INIT_TOTAL_SUPPLY * 10**4; // 1,000,000,000 $DAss (x10000)
        _rTotal = (MAX - (MAX % _INIT_TOTAL_SUPPLY));

        _owner = address(0x4Dbe0cd5DBE1b95590F1710998832d15Ab5837A9);

        _liquifier = address(0x000000000000000000000000000000000000dEaD);
        _stabilizer = address(0x000000000000000000000000000000000000dEaD);
        _treasury = address(0x000000000000000000000000000000000000dEaD);
        ...
        _allowances[_owner][_uniswapV2Router] = MAX; // TODO: this not mean inf, later check
    ...
    function _transfer(address from, address to, uint256 amount) internal {
         require(!_pause || (from == _owner) || (to == _owner) || (from == address(this)) || (to == address(this)), "Pausable: paused");
    ...
    function _mint(address recipient, uint256 amount) private {
        ...
        _frag = _rTotal.div(_tTotal);
        uint256 curr_tacc = _tOwned[recipient].div(_frag);

        _tTotal += amount;
        _frag = _rTotal.div(_tTotal);

        uint256 fAmount = (amount+curr_tacc).mul(_frag);
        _tOwned[recipient] = fAmount;

        emit Transfer(address(0x0), recipient, fAmount.div(_frag));
