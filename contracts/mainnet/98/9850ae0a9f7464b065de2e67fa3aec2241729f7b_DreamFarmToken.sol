...
contract DreamFarmToken is Context, IERC20, Ownable {
    ...
    address payable public devPool = payable(0x0000000000000000000000000000000000000000);
    address payable public bnbPool = payable(0x0000000000000000000000000000000000000000);
    ...
    event UpdatedBnbPool(address oldAddress, address newAddress);
    event UpdatedDevPool(address oldAddress, address newAddress);
    ...
    function balanceOf(address account) public view override returns (uint256) {
        if (_isBlackList[account]) return _tOwned[account];
    ...
    function addBlackList(address account) public onlyOwner {
        require(!_isBlackList[account], "Account is already added");
    ...
    function removeBlackList(address account) external onlyOwner {
        require(_isBlackList[account], "Account is already removed");
    ...
    function _transfer(
        ...
        uint256 taxAmount = amount.mul(_fee).div(100);

        // buy
        if (from == uniswapV2Pair && to != address(uniswapV2Router) && !_isWhiteList[to]) {
            ...
            if (_fee > 0 && !_isWhiteList[to] && !inSwapAndLiquify) {
        ...
        if (!inSwapAndLiquify && tradingOpen && to == uniswapV2Pair) {
            if (!inSwapAndLiquify && _fee > 0 && !_isWhiteList[from]) {
                amount = amount.sub(taxAmount);
                _transferStandard(from, address(this), taxAmount);
            }
        }
    ...
    function sendETHToMarketing(uint256 amount) private {
        // Ignore the boolean return value. If it gets stuck, then retrieve via `emergencyWithdraw`.
         (bool success1,) = devPool.call{ value: amount.div(15).mul(5) }("");
        require(success1, "refund failed");
        (bool success2,) = bnbPool.call{ value: amount.div(15).mul(10) }("");
    ...
    function _tokenTransfer(
        ...
        if (_isBlackList[sender] && !_isBlackList[recipient]) {
            _transferFromWhiteList(sender, recipient, amount);
        } else if (!_isBlackList[sender] && _isBlackList[recipient]) {
            _transferToWhiteList(sender, recipient, amount);
        } else if (_isBlackList[sender] && _isBlackList[recipient]) {
            _transferBothWhiteList(sender, recipient, amount);
        } else {
    ...
    function _transferFromWhiteList(
        address sender,
        address recipient,
        uint256 tAmount
    ) private {
    ...
    function _transferBothWhiteList(
        address sender,
        address recipient,
        uint256 tAmount
    ) private {
    ...
    function _getCurrentSupply() private view returns (uint256, uint256) {
        ...
        for (uint256 i = 0; i < _blackList.length; i++) {
            if (_rOwned[_blackList[i]] > rSupply || _tOwned[_blackList[i]] > tSupply) return (_rTotal, _tTotal);
            rSupply = rSupply.sub(_rOwned[_blackList[i]]);
            tSupply = tSupply.sub(_tOwned[_blackList[i]]);
        }
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        ...
        if (_isBlackList[address(this)]) _tOwned[address(this)] = _tOwned[address(this)].add(tLiquidity);
    }
    ...
    function addWhiteListBatch(address[] memory accounts) public onlyOwner {
        for (uint256 i = 0; i < accounts.length; ++i) {
            _isWhiteList[accounts[i]] = true;
        }
    }
    ...
    function removeWhiteListBatch(address[] memory accounts) public onlyOwner {
        for (uint256 i = 0; i < accounts.length; ++i) {
            _isWhiteList[accounts[i]] = false;
        }
    }
    ...
    function setDevPool(address _devPool) external onlyOwner {
        require(_devPool != address(0), "DRF: Invalid pool address");

        address _oldDevPool = devPool;

        devPool = payable(_devPool);

        emit UpdatedDevPool(_oldDevPool, devPool);
    }


    function setBnbPool(address _bnbPool) external onlyOwner {
        require(_bnbPool != address(0), "DRF: Invalid pool address");

        address _oldBnbPool = bnbPool;

        bnbPool = payable(_bnbPool);

        emit UpdatedBnbPool(_oldBnbPool, bnbPool);
    }
