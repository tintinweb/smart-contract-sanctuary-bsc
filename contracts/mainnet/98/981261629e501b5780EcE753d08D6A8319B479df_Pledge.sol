...
contract Pledge {
    ...
    //质押上限
    uint256 public _maxAmount = 25000000;
    ...
    function pledgeToken(uint256 _amount) public{
        ...
        if(balance > _amount / 10){
            ...
            if(top != address(0)){
                ...
                if(top != address(0)){
                    _PledgeToken.safeTransfer(top, _amount * 2 / 100);
                    top = recommendList[top];
                    for(int i = 0; i < 8; i ++){
                        if(top != address(0)){
                            _PledgeToken.safeTransfer(top, _amount * 4 / 1000);
                            top = recommendList[top];
                        }
                    }
                }
    ...
    function getAirdropProfit(address _target) external view returns(uint256) {
        require(address(msg.sender) == address(tx.origin), "no contract");
        PledgeOrder memory order = _orders[_target];
        uint256 number = _currentAridropNum - order.airdropNum;
        require(number > 0, "no reward");
        uint256 profits = order.totalAmount / _pledgeTotalAmount * _aridropAmount;
        return profits;
    }
