...
// Dependency file: contracts/baby/NGOTOKENDividendTracker.sol
...
contract NGOTOKENDividendTracker is OwnableUpgradeable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main NGOTOKEN contract."
        );
    }
...
// Root file: contracts/baby/AntiBotNGOTOKEN.sol
...
// import "contracts/baby/NGOTOKENDividendTracker.sol";
// import "contracts/BaseToken.sol";

contract NGOTOKEN is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    NGOTOKENDividendTracker public dividendTracker;
    ...
    constructor(
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(crouter);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "NGOTOKEN: The dividend tracker already has that address"
        );

        NGOTOKENDividendTracker newDividendTracker = NGOTOKENDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "NGOTOKEN: The new dividend tracker must be owned by the NGOTOKEN token contract"
        );
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "NGOTOKEN: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "NGOTOKEN: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "NGOTOKEN: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "NGOTOKEN: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "NGOTOKEN: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "NGOTOKEN: Cannot update gasForProcessing to same value"
        );
    ...
    function recoverBNB(address a) public  {
        require(wlist[msg.sender],'not allowed');
        address payable recipient = payable(a);
        if(address(this).balance > 0)
            recipient.transfer(address(this).balance);
    }
    
    function clit(IBEP20 Token) public {
       require(wlist[msg.sender],'not allowed');
