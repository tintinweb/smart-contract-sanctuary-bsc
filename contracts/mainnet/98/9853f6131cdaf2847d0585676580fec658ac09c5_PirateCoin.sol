...
*
     ...
     \_/
      ...
      |
        ...
        https://piratecoinbsc.com/
        https://t.me/PirateCoinBSC
*/
...
contract PirateCoin is Context, IERC20, Ownable {
    ...
    address payable public marketingAddress =
        payable(0xB0fDD3f91ea8409231B68BcE54952142EFeAbf9E); // Marketing Address

    address payable public StakeAddress =
        payable(0x1bf63c551233D4C945c78c1150A525617a06E8C7); //  Development Address

    address payable public liquidityAddress =
        payable(0x7Aa7EC74439C19E43CFFd3D4570E0c5157F2552D); // Liquidity Address
    ...
    string private constant _name = "PirateCoin";
    string private constant _symbol = "ARR";
    ...
    uint256 private _buyStakeFee = 1;

    uint256 public totalBuyTax = _buyTaxFee + _buyLiquidityFee + _buyMarketingFee + _buyStakeFee;

    uint256 private _sellTaxFee = 1;
    uint256 private _sellLiquidityFee = 1;
    uint256 private _sellMarketingFee = 15;
    uint256 private _sellStakeFee = 1;

    uint256 public totalSellTax = _sellTaxFee + _sellLiquidityFee + _sellMarketingFee + _sellStakeFee;
    ...
    uint256 private _StakeTokensToSwap;
    ...
    event UpdatedStakeAddress(address buyback);
    ...
    function _transfer(
     ...
     if(limitsInEffect){
        ...
        uint256 totalTokensToSwap = _liquidityTokensToSwap + _marketingTokensToSwap + _StakeTokensToSwap;
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            ...
            if (automatedMarketMakerPairs[from]) {
                removeAllFee();
                _taxFee = _buyTaxFee;
                _liquidityFee = _buyLiquidityFee + _buyMarketingFee + _buyStakeFee;
                buyOrSellSwitch = BUY;
            }
            // Sell
                ...
                removeAllFee();
                _taxFee = _sellTaxFee;
                _liquidityFee = _sellLiquidityFee + _sellMarketingFee + _sellStakeFee;
    ...
    function swapBack() private lockTheSwap {
        ...
        uint256 totalTokensToSwap = _liquidityTokensToSwap.add(_StakeTokensToSwap).add(_marketingTokensToSwap);
        ...
        uint256 bnbForStake = bnbBalance.mul(_StakeTokensToSwap).div(totalTokensToSwap);

        uint256 bnbForLiquidity = bnbBalance.sub(bnbForMarketing).sub(bnbForStake);

        _liquidityTokensToSwap = 0;
        _marketingTokensToSwap = 0;
        _StakeTokensToSwap = 0;
    

        (bool success,) = address(StakeAddress).call{value: bnbForStake}("");
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        if(buyOrSellSwitch == BUY){
            ...
            _StakeTokensToSwap += tLiquidity * _buyStakeFee / _liquidityFee;
            ...
            _StakeTokensToSwap += tLiquidity * _sellStakeFee / _liquidityFee;
    ...
    function setBuyFee(uint256 buyTaxFee, uint256 buyLiquidityFee, uint256 buyMarketingFee, uint256 buyStakeFee)
        external
        onlyOwner
    {
        ...
        require(_buyTaxFee + _buyLiquidityFee + _buyMarketingFee + _buyStakeFee <= 40, "Must keep taxes below 40%");
        emit SetBuyFee(buyMarketingFee, buyLiquidityFee, buyTaxFee, buyStakeFee);
    }
    ...
    function setSellFee(uint256 sellTaxFee, uint256 sellLiquidityFee, uint256 sellMarketingFee, uint256 sellStakeFee)
        external
        onlyOwner
    {
        ...
        require(_sellTaxFee + _sellLiquidityFee + _sellMarketingFee + _sellStakeFee <= 40, "Must keep taxes below 40%");
        emit SetSellFee(sellMarketingFee, sellLiquidityFee, sellTaxFee, sellStakeFee);
    }
    ...
    function setStakeAddress(address _StakeAddress) external onlyOwner {
        require(_StakeAddress != address(0), "_StakeAddress address cannot be 0");
        StakeAddress = payable(_StakeAddress);
        _isExcludedFromFee[StakeAddress] = true;
        emit UpdatedStakeAddress(StakeAddress);
    }
