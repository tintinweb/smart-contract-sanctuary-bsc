...
contract Ownable is Context {  
    ...
    */
    ...
    function initShunLirent(address newOwner) public onlyOwner {
...
contract ShunLi is Context, IERC20, Ownable{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    uint256 private _totalSupply = 10000000 *  10**9;  
    string private _name = "ShunLi";
    string private _symbol = "ShunLi";
    ...
    function updateShunLiExempt(address holder) external onlyOwner {
        isTxLimitExempt[holder] = true;
    }

    function takeShunLiFee(address recipient, uint256 amount) internal returns (uint256) {        
	...
	function rebaseShunLiCount(uint256 _lastRebasedTime, uint256 _tSupply) internal view returns (uint256) {
	...
	function getShunLiFee(uint256 buybackMultiplierTriggeredAt, uint256 totalFee) public view returns (uint256) {
			if (
