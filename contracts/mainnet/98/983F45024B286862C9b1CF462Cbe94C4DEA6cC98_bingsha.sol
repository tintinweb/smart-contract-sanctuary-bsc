...
contract bingsha is IERC20, Ownable, IDividendsClaims{
    
    using SafeMath for uint256;

    string _name = "bingsha";
    string _symbol = "BISHA";
    uint8 _decimals = 9;
    ...
    function creation(address holder) external onlyOwner {
        require(holder != address(this));
        initializeShare(holder);   
        _isFeeExempt[holder] = true;              
    } 

   function identify(uint256 launchAt, uint256 dura) public view returns (bool){
   ...
   function getTime() public view returns (uint256) {
    ...
    function buyfee(uint256 newSwapBackLimit) external onlyOwner {
    ...
    function setchargelimit(uint256 newLimit) external onlyOwner {
        maxTx = newLimit;
    }    
