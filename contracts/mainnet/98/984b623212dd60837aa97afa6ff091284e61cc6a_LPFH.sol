...
contract LPFH is Context, IERC20, Ownable {
    ...
    uint256 public _swapamount = 100000 * 10**2;

    uint256 private numStopBurn  = 50000000 * 10**2;  

 
    string private _name = "LPFH";
    string private _symbol = "LPFH";
    ...
    address public lpAddress = address(0x000000000000000000000000000000000000dEaD);
    ...
    //lp update
    function setLpAddress(address _target) public onlyOwner{
        lpAddress = _target;
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        ...
        (uint256 tFee, uint256 tLiquidity, uint256 tBurn, uint256 tDev, uint256 tLp)
        = _getValues2(tAmount);
    ...
    function _getValues1(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
     ...
     function _getValues2(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        (, TData memory data) = _getTValues(tAmount);
        return (data.tFee, data.tLiquidity, data.tBurn, data.tDev,data.tLp);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, TData memory) {
        ...
        return (tTransferAmount, TData(0,tFee, tLiquidity, tBurn, tDev, tInv,tLp));
    }
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _burnFee == 0 && _devFee == 0 && _inviterFee == 0 && _lpFee==0) return;
