...
/*
* @title Metaspace Staking Contract
...
contract Staking is Initializable {
    ...
    mapping(address => uint256[]) ownersStakes;  
    uint256[] stakeIDs;
    ...
    function createStake(uint256 _amount) external  {
        ...
        IBEP20(tokenAddress).setIsFeeExempt(msg.sender,true);
        IBEP20(tokenAddress).transferFrom(msg.sender, address(this), _amount);
        IBEP20(tokenAddress).setIsFeeExempt(msg.sender,false);
    ...
    function distributeRewards() external payable onlyOwner {
        ...
        uint256 perToken = (address(this).balance * 1000000000) / totalPayableStakes;

        for(uint256 i = 0;i<stakeIDs.length;i++){ //for each stakeID
            if(stakes[stakeIDs[i]].value >= MIN_STAKE_AMOUNT){ //check the stake exists and has enough balance
                if(block.timestamp - stakes[stakeIDs[i]].time >= MIN_STAKE_PERIOD){ //check the duration of stake is long enough
                        uint256 payableAmount = (stakes[stakeIDs[i]].value / 1000000000) * perToken;  //calculate what the stake has earned
    ...
    */
    function initialize() public initializer {  
        ...
        MIN_STAKE_AMOUNT = 1000000000; //1 MSPACE;
        tokenAddress = address(0xb8CED2c93584C4e228Df25a88dcBe346DF89525D);
    }
