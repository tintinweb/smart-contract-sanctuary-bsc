...
contract BITBETCOIN is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    BITBETCOINDividendTracker public dividendTracker;

    address public deadWallet = 0x000000000000000000000000000000000000dEaD;

    address public immutable BTC = address(0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c); //BTC

    uint256 public _totalSupply = 250_000_000 * (10**18);
    uint256 public swapTokensAtAmount = 1_000_000 * (10**18); //1 millions. 
    ...
    uint256 public totalFees = BTCRewardsFee.add(liquidityFee).add(marketingFee).add(buybackFee);

    address payable public _marketingWalletAddress = payable(0x5894bd5fAa8Ab74626D881d2f90b89BBD9D53BE9);
    address payable public _buybackWalletAddress = payable(0xfD3D7b6adA7ab0B9D7C922D0a45c0cE3C1c11146);
    ...
    constructor() BEP20("BITBETCOIN", "BTB") 
    {
        dividendTracker = new BITBETCOINDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "BITBETCOIN: The dividend tracker already has that address");
        BITBETCOINDividendTracker newDividendTracker = BITBETCOINDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "BITBETCOIN: The new dividend tracker must be owned by the BITBETCOIN token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BITBETCOIN: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BITBETCOIN: Account is already the value of 'excluded'");
    ...
    function setBTCRewardsFee(uint256 value) external onlyOwner{
        BTCRewardsFee = value;
        totalFees = BTCRewardsFee.add(liquidityFee).add(marketingFee).add(buybackFee);
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BITBETCOIN: The PanBTCSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BITBETCOIN: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BITBETCOIN: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BITBETCOIN: Cannot update gasForProcessing to same value");
    ...
    function _transfer(address from, address to, uint256 amount) internal open(from, to) override 
    {
        ...
        !automatedMarketMakerPairs[from] && from != owner()) 
        {
            ...
            uint256 swapableFees = totalFees.sub(BTCRewardsFee); // marketingFee+buybackFee+liquidityFee.
    ...
    {
       uint256 swapableFee = totalFees.sub(BTCRewardsFee);
...
contract BITBETCOINDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("BITBETCOIN_Dividend_Tracker", "BITBETCOIN_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "BITBETCOIN_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "BITBETCOIN_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BITBETCOIN contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BITBETCOIN_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BITBETCOIN_Dividend_Tracker: Cannot update claimWait to same value");
