...
contract XLFToken is IERC20,Ownable {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if(unSwapDividendsAmount > swapTokensAtAmount){
            if (
                ...
                swapping = false;              
                emit AutoSwap(unSwapLpAmount.div(2), usdtAmount, 2);  
	...
	function swapTokensForOtherUsdt2(uint256 tokenAmount) external returns(uint256) {
		address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = address(_usdtAddress);
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
	...
	function swapAddLiquidity2(uint256 tokenAmount,uint256 usdtAmount) external  {
        uniswapV2Router.addLiquidity(
            address(this),
            _usdtAddress,
            tokenAmount,
