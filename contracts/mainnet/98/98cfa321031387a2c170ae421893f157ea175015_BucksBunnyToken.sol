...
// Bucks Bunny Token is a rewards token built on the Binance Smart Chain.
// Designed to reward Bucks Bunny Token and NFT holders with passive income paid in BUSD.


// 📱 Telegram: https://t.me/BucksBunnyToken
// 🌎 Website: https://Bucksbunnytoken.io
// 🌐 Twitter: https://twitter.com/bucksbunnytoken
...
contract BUSDDividendTracker is DividendPayingToken  {
    ...
    constructor(address _dividentToken) DividendPayingToken("Bunny_Busd_Dividend_Tracker", "Bunny_Busd_Dividend_Tracker",_dividentToken) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18);
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "Bunny_Busd_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "Bunny_Busd_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Bunny contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Bunny_Busd_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Bunny_Busd_Dividend_Tracker: Cannot update claimWait to same value");
...
contract BucksBunnyToken is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public maxWalletBalance = 50000000 * (10**18);
    ...
    constructor() ERC20("BBNY", "BucksBunnyToken") {
        	
    	marketingWallet = 0xCe88D442B37348B339Fa8aa752aF671aBd436D2F; 
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "Bunny: The marketing wallet is already this address");
    ...
    function setMarketingFeeBuy(uint256 marketingFeeBuy) external onlyOwner {
  	    _marketingFeeBuy = marketingFeeBuy;
  	}

    function setMarketingFeeSell(uint256 marketingFeeSell) external onlyOwner {
  	    _marketingFeeSell = marketingFeeSell;
  	}

    function setLpFeeBuy(uint256 LpFeeBuy) external onlyOwner {
  	    _LpFeeBuy = LpFeeBuy;
  	}

    function setLpFeeSell(uint256 LpFeeSell) external onlyOwner {
  	    _LpFeeSell = LpFeeSell;
  	}

    function setBusdDividendRewardsFeeBuy(uint256 busdDividendRewardsFeeBuy) external onlyOwner {
		_busdDividendRewardsFeeBuy = busdDividendRewardsFeeBuy;
    }

    function setBusdDividendRewardsFeeSell(uint256 busdDividendRewardsFeeSell) external onlyOwner {
    ...
    function updatebusdDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(busdDividendTracker), "Bunny: The dividend tracker already has that address");
 
        BUSDDividendTracker newbusdDividendTracker = BUSDDividendTracker(payable(newAddress));
 
        require(newbusdDividendTracker.owner() == address(this), "Bunny: The new dividend tracker must be owned by the Bunny token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "Bunny: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "Bunny: Account is already excluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Bunny: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "Bunny: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "Bunny: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "Bunny: Trading has not started yet");
