...
contract NativeFarm is Ownable, ReentrancyGuard {
  ...
  // The flag if the empty rewards withdraw possible or not
  bool public enableForceWithdraw = false;
  ...
  event UpdateEndBlock(uint256 blockNumber);
  event UpdateEnableForceWithdraw(bool enable);
  ...
  constructor(
    ...
    endBlock = startBlock + (blocksPerDay * 365 * 10); // Farm runs for about 10 year.
  }
    ...
    returns (uint256)
  {
    ...
    if (block.number > pool.lastRewardBlock && lpSupply != 0) {
      ...
      uint256 nativeReward = (multiplier * nativePerBlock * pool.allocPoint) /
        totalAllocPoint;
      accNativePerShare =
        accNativePerShare +
        ((nativeReward * 1e12) / lpSupply);
    }
    return ((user.amount * accNativePerShare) / 1e12) - user.rewardDebt;
  }
    ...
    onlyMember
  {
    ...
    updatePool(_pid);
    if (user.amount > 0) {
      uint256 pending = ((user.amount * pool.accNativePerShare) / 1e12) -
        user.rewardDebt;
      if (pending > 0) {
        require(
    ...
    user.rewardDebt = (user.amount * pool.accNativePerShare) / (1e12);
    ...
    onlyMember
  {
    ...
    if (pending > 0) {
      require(
        safeNativeTransfer(msg.sender, pending) || enableForceWithdraw,
  ...
  function safeNativeTransfer(address _to, uint256 _amount)
    internal
    returns (bool)
  {
  ...
  function removeMember(address account) external onlyOwner {
    ...
    for (uint256 pid = 0; pid < length; ++pid) {
      ...
      // If account has staked amount, revert!
      require(amount == 0, "removeMember: staked member!");
    }
  ...
  function updateEmissionRate(uint256 _nativePerBlock) external onlyOwner {
    massUpdatePools();
    nativePerBlock = _nativePerBlock;
    emit UpdateEmissionRate(msg.sender, _nativePerBlock);
  }

  function updateEndBlock(uint256 blockNum) external onlyOwner {
    require(endBlock >= block.number, "updateEndBlock: bad block number");
    endBlock = blockNum;
    emit UpdateEndBlock(blockNum);
  }

  function updateEnableForceWithdraw(bool enable) external onlyOwner {
    require(
      enableForceWithdraw != enable,
      "updateEnableForceWithdraw: already!"
    );
    enableForceWithdraw = enable;
    emit UpdateEnableForceWithdraw(enable);
  }
}