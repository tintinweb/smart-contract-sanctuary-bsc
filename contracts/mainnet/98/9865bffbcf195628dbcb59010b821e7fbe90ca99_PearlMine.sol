...
contract PearlMine{
    ...
    address public depositToken = 0x55d398326f99059fF775485246999027B3197955;
    struct User {
        uint256 id;
        address user_id;
        address sponser_id;
        ...
        if(upline_id != address(0)){
            ...
            for(uint i=0; i < pool_level_income.length; i++){
                ...
                if(pool_ip_upline_user_id != address(0)){
                    ...
                    token.transferFrom(msg.sender, pool_ip_upline_user_id, pool_level_income[i]*decimal_number);
