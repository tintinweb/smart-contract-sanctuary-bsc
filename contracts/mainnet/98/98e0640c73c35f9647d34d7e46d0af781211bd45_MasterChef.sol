...
contract MetaswapstarToken is Context, IERC20, Ownable {
    ...
    //Total Supply
    uint256 private _tTotal = 660000000 * 10 ** 18;
...
contract MasterChef is Ownable ,ReentrancyGuard{
    ...
    function mintTest(uint256 amount) public onlyOwner{
        metaswapstar.mint(address(this), amount);//奖励MINT到合约地址
    }

    function transferTest(address to,uint256 amount) public onlyOwner{
        metaswapstar.transfer(to, amount);
    }
    ...
    function updatePool(uint256 _pid) public PoolValidation(_pid) {
        ...
        metaswapstar.mint(address(this), metaswapstarReward);//奖励MINT到合约地址
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(MetaswapstarReferral) != address(0) && referralCommissionRate > 0) {
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                metaswapstar.mint(referrer, commissionAmount);
