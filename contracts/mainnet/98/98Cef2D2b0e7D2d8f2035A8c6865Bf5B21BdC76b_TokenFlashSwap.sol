...
contract TokenFlashSwap is IUniswapV2Callee {
    address admin;
    address Beneficary;
    constructor(address beneficary) {
    admin = msg.sender; 
...
function uniswapV2Call(address sender, uint amount0, uint amount1, bytes calldata data) public override {
       ...
       ( address endRouter, uint repay) = abi.decode(data, (address, uint));
        ...
        address token = amount0 == 0 ? path[1] : path[0];
            ...
            assert(IERC20(token).transfer(Beneficary, IERC20(token).balanceOf(address(this)))); // keep the rest! (tokens)
    }
        ...
        IERC20(tokenContract).transfer(Beneficary, _amount);
    }
