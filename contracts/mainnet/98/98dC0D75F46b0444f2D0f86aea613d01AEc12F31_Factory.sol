...
contract Factory is Initializable, ERC20Upgradeable {
    ...
    event ClaimReward(address user, uint256 amount, uint256 poolId);
    ...
    function withdrawRewardToken(uint256 _poolId) external {
        require(
            IStaking(poolAddress[_poolId]).stakeBalanceOfUser(msg.sender) != 0,
            "token not stake"
        );
        ...
        emit ClaimReward(msg.sender, reward, _poolId);
    }
}
