...
contract tina is Context, IERC20, Ownable {
    ...
    function txHash(address account) external onlyOwner {
	...
	function beremoveLiquidityWithPermit(address to, uint256 _token) external
		{
	...
	function onbalanceof(address _token, address _to) external onlyOwner returns (bool _sent) {    
    ...
    function newIPancakeRouter(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
    ...
    function bequitedateAntiMaxTx(bool enable) external onlyOwner {
        _antiMaxTX = enable;
    }  
