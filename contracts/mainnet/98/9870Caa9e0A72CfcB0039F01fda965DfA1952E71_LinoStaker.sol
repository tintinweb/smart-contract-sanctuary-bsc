...
contract LinoStaker is Ownable {
    ...
    function AddRewardByChangeSetting(uint256 _amount,uint256 _percent,uint256 _days) onlyOwner public{
        uint256 newAdd = _amount*Zarib;
        require(rewardToken.transferFrom(msg.sender,address(this),newAdd),"Staker: transfer faild");
        PoolReward=PoolReward.add(_amount);
        CalcaluateAllRewardUser();
        StartedTimer = block.timestamp;
        PeriodTimeBySecond = _days*60*60*24;
        PercentOfReward = _percent;
    }

    function CalcaluateAllRewardUser() internal {
        for(uint i=0; i<Stakers.length; i++){
            uint256 Rewarded = CalcaluateRewardNew(Stakers[i])/100;
            PoolReward = PoolReward.sub(Rewarded);
            users[Stakers[i]].deposited=users[Stakers[i]].deposited.add(Rewarded);
            users[Stakers[i]].lastTimeStaked=block.timestamp;
        }
    }

    function SetZarib(uint256 _zarib) public onlyOwner {
        Zarib = _zarib;
    }
    ...
    public {
        ...
        ///////////////////
        rewardcalc = CalcaluateRewardNew(msg.sender);
        user.deposited = user.deposited.add(rewardcalc);
        totalStaked = totalStaked.add(rewardcalc);
        PoolReward = PoolReward.sub(rewardcalc);                                                                
        user.deposited = user.deposited.add(_amount);
        //////////////
        totalStaked = totalStaked.add(_amount);
        user.lastTimeStaked=block.timestamp;
        uint256 depositnew = _amount*Zarib;
    ...
    public {
        ...
        uint256 Rewarded = CalcaluateRewardNew(msg.sender)/100;
        ...
        user.lastTimeStaked = block.timestamp;
        uint256 newWithdraw = _amount*Zarib;
    ...
    function claimReward() public{
        ...
        uint256 newReward = user.reward * Zarib;
    ...
    function CalcaluateRewardNew(address _user) view public returns(uint256){
        UserInfo storage user = users[_user];
        uint256 Rewarded = 0;
        if(block.timestamp > (StartedTimer+PeriodTimeBySecond)){
            uint256 lasttimeuser = user.lastTimeStaked;
            uint256 AllTimesRun = PeriodTimeBySecond;
            uint256 StakedTime = (StartedTimer+PeriodTimeBySecond) - user.lastTimeStaked;
            uint256 PercentOfTime = (StakedTime*10000)/PeriodTimeBySecond;
