...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    function changeFeeReceiver(address _newAddr) public onlyOwner {
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        uint256 userTimestamp = lastStaked[msg.sender];
        if (_amount > 0) {
            user.amount = user.amount.sub(_amount);
            if(currentTimestamp.sub(userTimestamp) <= 604800){ // 604800 is 7 days in seconds
                if(depositFee > 0){
                    uint256 _depositFee = _amount.mul(depositFee).div(10000);
                    rewardToken.safeTransfer(feeReceiver, _depositFee);
                    rewardToken.safeTransfer(address(msg.sender), _amount.sub(_depositFee));
