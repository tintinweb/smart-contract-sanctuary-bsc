...
abstract contract AbsPool is Ownable {
    ...
    string private _lockTokenSymbol;
    address private _rewardTokenAddress;
    Pool[] private _pools;

    mapping(address => Record[]) private _userRecords;
    bool private _pause;
    uint256 public _rewardExtFactor = 1;
    ...
    function lock(uint256 pid, uint256 amount) external {
        ...
        uint256 reward = amount * pool.rewardPerToken / (10 ** IERC20(_lockTokenAddress).decimals()) / _rewardExtFactor;
    ...
    function getBaseInfo() public view returns (
        ...
        address rewardToken, uint256 rewardTokenDecimals, string memory rewardTokenSymbol, uint256 rewardExtFactor
    ){
    ...
    function setLockToken(address token, string memory lockTokenSymbol) external onlyOwner {
        _lockTokenAddress = token;
        _lockTokenSymbol = lockTokenSymbol;
    }
    ...
    function setRewardExtFactor(uint256 factor) external onlyOwner {
        require(factor > 0, "min 1");
        _rewardExtFactor = factor;
    }

    constructor(
        address LockTokenAddress, string memory LockTokenSymbol, address RewardTokenAddress,
        uint256 reward15, uint256 reward30, uint256 reward60
    ){
...
contract XJRLPPool is AbsPool {
    constructor() AbsPool(
    //XJRLP
        address(0x35DC7B7802b23C87899d95DE74647512142496d1),
        "XJRLP",
    //TJ
        address(0x451D8d18759916b091a66F9B669a0FFE567D8322),
        100000, 220000, 480000
    ){
