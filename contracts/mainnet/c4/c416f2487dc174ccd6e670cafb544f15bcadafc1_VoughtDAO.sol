...
`8.`888b           ,8'  ,o888888o.     8 8888      88     ,o888888o.    8 8888        8 8888888 8888888888 
 `8.`888b         ,8'. 8888     `88.   8 8888      88    8888     `88.  8 8888        8       8 8888       
  `8.`888b       ,8',8 8888       `8b  8 8888      88 ,8 8888       `8. 8 8888        8       8 8888       
   `8.`888b     ,8' 88 8888        `8b 8 8888      88 88 8888           8 8888        8       8 8888       
    `8.`888b   ,8'  88 8888         88 8 8888      88 88 8888           8 8888        8       8 8888       
     `8.`888b ,8'   88 8888         88 8 8888      88 88 8888           8 8888        8       8 8888       
      `8.`888b8'    88 8888        ,8P 8 8888      88 88 8888   8888888 8 8888888888888       8 8888       
       `8.`888'     `8 8888       ,8P  ` 8888     ,8P `8 8888       .8' 8 8888        8       8 8888       
        `8.`8'       ` 8888     ,88'     8888   ,d8P     8888     ,88'  8 8888        8       8 8888       
         `8.`           `8888888P'        `Y88888P'       `8888888P'    8 8888        8       8 8888       */

// VOUGHT DOA 
// CONTRACT DEV @FRONTRUNDEV  
...
contract ERC20DividendToken is Context, ERC20Burnable, Ownable {
    ...
    uint256 private liquidityPending;
    uint256 private marketingPending;
    ...
    event RecoverBNB(uint256 amountRecovered);
    event RecoverERC20(uint256 amountRecovered);
    ...
    constructor(string memory name_, string memory symbol_, uint256 totalSupply_, address routerV2_, address marketingAddress_) ERC20(name_, symbol_) {
        setRewardFees(4, 4);
        setLiquidityFees(1, 1);
        setMarketingFees(5, 5);

        setMarketingWallet(marketingAddress_);
        
        setMinTokensBeforeSwap(500_000 * (10**18));

        changeMaxWalletAmount(totalSupply_ * 2 * (10**16)); // 2%
    ...
    function setRewardFees(uint256 buy, uint256 sell) public onlyOwner {
        rewardsBuyFee = buy;
        rewardsSellFee = sell;

        totalBuyFees = rewardsBuyFee.add(liquidityBuyFee).add(marketingBuyFee);
        totalSellFees = rewardsSellFee.add(liquiditySellFee).add(marketingSellFee);
        require(totalBuyFees <= 25 && totalSellFees <= 25, "Total fee is over 25%");
    }

    function setLiquidityFees(uint256 buy, uint256 sell) public onlyOwner {
        liquidityBuyFee = buy;
    ...
    function setMarketingFees(uint256 buy, uint256 sell) public onlyOwner {
        marketingBuyFee = buy;
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner() && (automatedMarketMakerPairs[to] || automatedMarketMakerPairs[from]))
        ...
        if (
            ...
            if (marketingPending > 0) {
                swapAndSendDividendsToMarketing(marketingPending);
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[from] && totalBuyFees > 0) {
                ...
                marketingPending += fees.mul(marketingBuyFee).div(totalBuyFees);
            } else if (totalSellFees > 0) {
                fees = amount.mul(totalSellFees).div(100);

                liquidityPending += fees.mul(liquiditySellFee).div(totalSellFees);
                marketingPending += fees.mul(marketingSellFee).div(totalSellFees);
            }
...
contract VoughtDAO is ERC20DividendToken {
    ...
    constructor (address _marketingAddress) ERC20DividendToken("Vought DAO", "CompoundV", _tokenSupply, _routerAddress, _marketingAddress) {
