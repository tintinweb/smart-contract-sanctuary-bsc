...
contract BKITAgent is IBKITAgent {
    ...
    address public BBFT = 0x8a9424745056Eb399FD19a0EC26A14316684e274;
...
// MAIN BBFT CONTRACT
contract BBFTKitchen2 is BKITAuth {
    using SafeMath for uint256;
    ...
    uint256 public buyThreshold = 100 * (10 ** 18);
    uint256 public minBuyAmount = 20 * (10 ** 18);
    ...
    uint256 public kitchenagentGas = 600000;
   ...
   // Set the agent gas
    function setKitchenAgentGas(uint256 _amount) external authorizedFor(Permission.AdjustContractVariables) {
        kitchenagentGas = _amount;
    }
    ...
    function setBBFTAddress(address _token) external authorizedFor(Permission.AdjustContractVariables) {
        BBFT = _token;
        kitchenagent.setAgentBBFTAddress(_token);
    }
        ...
        if(!inSwap) {
            ...
            for (uint256 i = 0; i < tokenPool.length; i++) {
                if(isSellToken[tokenPool[i]]) {
                    emit bkitError(724, tokenPool[i], tokenSellP[tokenPool[i]]);
                    ...
                    emit bkitError(731, tokenPool[i], curSellP);
                    kitchenagent.processSellRequest(tokenPool[i], curSellP);
                }
            }
        }
    }
    ...
    function manualConvertBNBtoBUSD(uint256 _amount) external authorizedFor(Permission.AdjustContractVariables) {
        if(address(this).balance > _amount) {
            ...
            emit bkitError(743, address(this), _amount);
    ...
    function emergencySellToken(address _token, uint256 _percentage) external authorizedFor(Permission.AdjustContractVariables) {
        kitchenagent.emergencySellToken(_token, _percentage);
    }
