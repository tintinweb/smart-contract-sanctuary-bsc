...
contract MSTToken is IERC20, Ownable {
    ...
    mapping(address => uint256) public _transactionT;
    mapping(address => uint256) public _transactionBuyC;
    mapping(address => uint256) public _transactionSellC;
    ...
    string private _name = "MST";
    string private _symbol = "MST";
    uint8 private _decimals = 18;

    uint256 private _tTotal = 200 * 10 ** 4 * 10 ** _decimals;

    uint256[6] priceStage = [0, 10 * 10 ** 18, 25 * 10 ** 18, 50 * 10 ** 18, 150 * 10 ** 18, 300 * 10 ** 18];
    ...
    event Agent(address indexed sender, uint256 amount);
    event BindingParents(address indexed user, address inviter);

    constructor(address _projectAddress, address _agentAddress, address _foundAddress, address _operationAddress, address _capitalAddress) {
        ...
        _parents[projectAddress] = address(this);

        //exclude owner and this contract from fee
        _isExcludedFromFee[msg.sender] = true;
        _isExcludedFromFee[_projectAddress] = true;
        _isExcludedFromFee[_agentAddress] = true;
        _isExcludedFromFee[_foundAddress] = true;
        _isExcludedFromFee[_operationAddress] = true;
        _isExcludedFromFee[_capitalAddress] = true;
        _isExcludedFromFee[address(this)] = true;
        _isExcludedFromFee[address(recv)] = true;

        _tOwned[_projectAddress] = _tTotal.mul(89).div(100);
        emit Transfer(address(0), _projectAddress, _tTotal.mul(89).div(100));

        _tOwned[_foundAddress] = _tTotal.mul(5).div(100);
        emit Transfer(address(0), _foundAddress, _tTotal.mul(5).div(100));

        uint256 monthTime = 3600 * 24 * 30;

        lockInfos.push(
            LockInfo({
                addr : _operationAddress,
                lockBalances : _tTotal.mul(3).div(100),
                releasePerSecond : _tTotal.mul(2).div(10000).div(monthTime * 5),
                lastUnlockTime : block.timestamp.add(monthTime * 3),
                hasRelease : 0
            })
        );
    ...
    function _transfer(
        ...
        // only 90% of wallet coins can be sold each time
        uint256 balance = balanceOf(from);
        if (amount >= balance * 90 / 100 && from != uniswapV2Pair) {
            amount = balance * 90 / 100;
        }

        // only hold a maximum of 1000 * 10 ** 18
        if (to != uniswapV2Pair) {
            require(_tOwned[to] + amount <= 1000 * 10 ** 18, "large amount");
        }

        _tokenTransfer(from, to, amount);

        transferLimit(from, to, amount);
    ...
    function _tokenTransfer(
        ...
        // agent
        uint256 fee = taxFeeAmount.add(extraTaxFeeAmount).mul(agentFee).div(feeDenominator);
        _tOwned[agentAddress] = _tOwned[agentAddress].add(fee);
        emit Transfer(sender, agentAddress, fee);
        if (fee != 0) emit Agent(sender, fee);

        // invite
        if (currentPrice < 30 * 10 ** 18) {
            fee = taxFeeAmount.add(extraTaxFeeAmount).mul(inviterFee).div(feeDenominator);

            address addr = _parents[sender];
            if (addr == address(0) || addr == address(this)) {
                addr = projectAddress;
            }
            _tOwned[addr] = _tOwned[addr].add(fee);
            emit Transfer(sender, addr, fee);
        }
        

        // lp + lp reward
        if (totalLp < 600000 * 10 ** _decimals) {
            fee = taxFeeAmount.add(extraTaxFeeAmount).mul(lpFee).div(feeDenominator);
            _tOwned[address(this)] = _tOwned[address(this)].add(fee);
            emit Transfer(sender, address(this), fee);
            totalLp = totalLp.add(fee);

            fee = taxFeeAmount.add(extraTaxFeeAmount).mul(lpRewardFee).div(feeDenominator);
            ...
            fee = taxFeeAmount.add(extraTaxFeeAmount).mul(lpRewardFee + lpFee).div(feeDenominator);
            _tOwned[address(dividendTrack)] = _tOwned[address(dividendTrack)].add(fee);
            emit Transfer(sender, address(dividendTrack), fee);
            dividendTrack.distributeDividends(fee);
        }
        uint256 recipientAmount = amount.sub(taxFeeAmount.add(extraTaxFeeAmount));
        _tOwned[recipient] = _tOwned[recipient].add(recipientAmount);
        emit Transfer(sender, recipient, recipientAmount);
    }

    // buy or sell can not exceed 1000 * 10 ** 18
    function transferLimit(address from ,address to, uint256 amount) private {
        if (from == uniswapV2Pair) {
            // buy
            require(amount <= 1000 * 10 ** 18, "buy bigger");
            uint256 today = block.timestamp - (block.timestamp + offset) % 1 days;
            if (today != _transactionT[to] && _transactionT[to] != 0) {
                _transactionT[to] = today;
                _transactionBuyC[to] = amount;
                _transactionSellC[to] = 0;
            } else {
                require(_transactionBuyC[to] + amount <= 1000 * 10 ** 18, "buy bigger");
                _transactionBuyC[to] += amount;
                if (_transactionT[to] == 0) {
                    _transactionT[to] = today;
                    _transactionSellC[to] = 0;
                }
            } 
            ...
            // sell
            require(amount <= 1000 * 10 ** 18, "sell bigger");
            uint256 today = block.timestamp - (block.timestamp + offset) % 1 days;
            if (today != _transactionT[from] && _transactionT[from] != 0) {
                _transactionT[from] = today;
                _transactionBuyC[from] = 0;
                _transactionSellC[from] = amount;
            } else {
                require(_transactionSellC[from] + amount <= 1000 * 10 ** 18, "sell bigger");
                _transactionSellC[from] += amount;
                if (_transactionT[from] == 0) {
                    _transactionT[from] = today;
                    _transactionBuyC[from] = 0;
                }
            }
