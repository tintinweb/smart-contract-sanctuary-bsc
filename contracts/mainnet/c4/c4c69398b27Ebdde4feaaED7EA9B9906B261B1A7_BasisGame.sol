...
contract BasisGame  is LPTokenWrapper
{
	...
	constructor(address _token,address _TokenBGM) LPTokenWrapper(IERC20(_token)) public
    {
    ...
    function RealtimeEarning(address account) public view returns (uint256 _TotalRewards)
    {
        ...
        {
            if ( RateTable[i].timestamp > EndTime)
            {
                TotalRewards += (EndTime - RateTable[i].timestamp) * RewardsPS * (StateTable[account].Balance / 10000000000) / RateTable[i].totalSupply * 10000000000;
                break;
            }
