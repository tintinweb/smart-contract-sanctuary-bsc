...
contract Midgard is Ownable, ReentrancyGuard {
    ...
    mapping (uint => uint) public vestingTime;
    ...
    function proccessClaim(
        ...
        uint256 _amount = user.remainingClaim;
        ...
        if (minDistributedTokenAmount <= tokensForDistribution) {
            ...
            uint userclaim=0;   
            if (block.timestamp > vestingTime[1] && block.timestamp < vestingTime[2])
               
               {
                require(user.claimround == 0, "ROUND1_TOKENS_CLAIMED");
                userclaim = (user.debt * 50) / 100;
                user.remainingClaim -= userclaim; 
                user.claimround = 1; 
                rewardToken.safeTransfer(_receiver, userclaim);
               } 
            else if (block.timestamp > vestingTime[2] && block.timestamp < vestingTime[3])
            
               {
                require(user.claimround <= 1, "ROUND2_TOKENS_CLAIMED");

                if (user.claimround == 0) {
                    userclaim = (user.debt * 75) / 100;   
                }
                else {
                    userclaim = (user.debt * 25) / 100;   
                } 
                user.remainingClaim -= userclaim; 
                user.claimround = 2;
                rewardToken.safeTransfer(_receiver, userclaim);
            ...
            else if (block.timestamp > vestingTime[3])    
               {
                userclaim = user.remainingClaim;
                user.remainingClaim = 0; 
                user.claimround = 3;
                rewardToken.safeTransfer(_receiver, userclaim);
               }
            else 
               {
                   revert("ICO: TIME_NOT_REACHED");
               }
                distributedTokens = distributedTokens.add(userclaim);
                emit TokensWithdrawn(_receiver,userclaim);     
           } 
    ...
    function initializeVestingDistribution(uint[] calldata _time) external onlyOwner  {

        require(!vestinginitialized, "BSCBAYPAD: Vesting Already Initialized");
         
        for (uint i=0; i<_time.length; i++) {
            vestingTime[i+1] = _time[i];
        }
        vestinginitialized=true;
        // vestingRounds = _time.length;
    } 
