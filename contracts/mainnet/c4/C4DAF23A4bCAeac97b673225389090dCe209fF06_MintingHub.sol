...
contract MintingHub is ReentrancyGuard, Ownable
{
    ...
    event BuyHub(address indexed user, uint256 indexed busdTotal,
    ...
    function buyHub(string memory hubName, uint256 totalVaseLocked, address referrer) external 
    ...
    updateWithdrawalDurationHub(hubName) updateWithdrawalDurationHubYearly(hubName)
    {
       ...
       hub.lockedVase = totalVaseLocked;
       hub.lockedVaseBalance = totalVaseLocked;
       hub.status = 1;
       hub.name = hubName;

       if(referrer != address(0))
       {
          uint256 directBonus = uint256((s_refBonus * totalVaseLocked) /(100 * 1e18));
       ...
       s_addressToLockedBal[msg.sender][hubName] = totalVaseLocked;
       s_addressToLockedVASEHUB[msg.sender] += totalVaseLocked;
       ...
       s_hubCount[hubName] += 1;
        ...
        emit BuyHub(msg.sender, msg.value, lockedTempBusd, totalVaseLocked, hub.name);
