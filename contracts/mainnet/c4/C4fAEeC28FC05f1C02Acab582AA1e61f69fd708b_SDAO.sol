...
interface OldSDAO is IERC20 {
    function parent(address account) external view returns (address);

    function recommended(address account, uint256 page, uint256 size) external view returns (uint256, address[] memory);
}
...
contract BackManager {
    ...
    uint256 public minTime = 8 minutes;
...
contract SDAO is IERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public sellInterval = 30 hours;

    string private _name = "SDAO Labs";
    string private _symbol = "SDAO";
    uint8 private _decimals = 18;
    uint256 private _tTotal = 210000000000 * 10 ** _decimals;

    uint256 public minTokenNumberToSell = 100 * 10 ** _decimals;

    uint256 public baseRate = 10000;
    uint256[4] public sellAmountRate = [1, 10, 100, baseRate];
    uint256 public decimalRate = 10 ** 17;


    Middle public middle;
    LPManager public lPManager;
    BackManager public backManager;
    ...
    constructor(address _router, IERC20 _usdt) {
        ...
        backManager = new BackManager(_uniswapV2Router, IERC20(this), usdt, msg.sender);
        _approve(address(backManager), address(_uniswapV2Router), MAX);
        ...
        _isExcludedFromFee[address(backManager)] = true;
    ...
    function setBackManager(BackManager _backManager) public onlyOwner {
        backManager = _backManager;
    }

    function setSellAmountRate(uint256[4] memory _sellAmountRate) public onlyOwner {
        sellAmountRate = _sellAmountRate;
    }

    function setDecimalRate(uint256 _decimalRate) public onlyOwner {
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (from != uniswapV2Pair && address(backManager) != address(0x0)) {
                backManager.manageBack();
            }
    ...
    function _tokenTransfer(
        ...
        uint256 burnFee = _takeBurn(sender, recipient, amount);
    ...
    function _takeBurn(
        ...
        if (sender != uniswapV2Pair && recipient != uniswapV2Pair) {
            tFee = amount.mul(300).div(baseRate);
        }
    ...
    function getSellAmount() public view returns (uint256){
        ...
        if (price > 4 * decimalRate) {
            return sellAmountRate[3];
        } else if (price > 2 * decimalRate) {
            return sellAmountRate[2];
        } else if (price > 1 * decimalRate) {
    ...
    function getUserInfo(address account) public view returns (uint256, uint256, uint256){
        return (getCurrentPrice(), getSellAmount(), lastSellTime[account]);
    }
