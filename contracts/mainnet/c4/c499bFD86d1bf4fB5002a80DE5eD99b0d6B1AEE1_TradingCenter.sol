...
contract TradingCenter is ERC1155Holder {
    ...
    function init() external {
        if (miningCenter == IMiningCenter(address(0))) {
            miningCenter = IMiningCenter(
                0xb51D7Ac845D2A427199D7d0c145ac230F4799d21
            );
            _ownerAddress = address(0x5747a7f258Bd38908A551CE6d76b8C2A428D7586);
            tradingFee = 10;
        }
    }
    ...
    function createOffer(
        ...
        miningCenter.transferNFTByUser(
            msg.sender,
            _nftType,
            address(miningCenter)
        );
    }
    ...
    function cancelOffer(uint256 _nftType, uint256 _nftNum) external {
        ...
        for (uint256 i = index; i < tradingOfferListLength[msg.sender]; i++) {
            tradingOfferList[msg.sender][i] = NftOffer(
                tradingOfferList[msg.sender][i + 1].nftType,
                tradingOfferList[msg.sender][i + 1].nftNum,
                tradingOfferList[msg.sender][i + 1].offerAmountWithBNB,
                tradingOfferList[msg.sender][i + 1].owner
            );
        }
    ...
    function transferNFTByOwner(
        address _from,
        address _to,
        uint256 _nftType,
        uint256 _nftNum
    ) external onlyOwner {
        miningCenter.transferNFTByUser(_from, _nftType, _to);
        miningCenter.updateNFTHistoryExternal(_from, _nftType, _nftNum, _to);
    }
    ...
    function manageTradingHistory(
        ...
        if (_mode == 0) {
            ...
            tradingOfferList[_owner][tradingOfferListLength[_owner]] = NftOffer(
                _nftType,
                _nftNum,
                _tradingOfferAmount,
                _owner
