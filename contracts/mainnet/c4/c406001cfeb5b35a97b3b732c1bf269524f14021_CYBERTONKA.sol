...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  function updateDividendUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "CYBERTONKA: The router already has that address");
...
contract CYBERTONKA is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    CYBERTONKADividendTracker public dividendTracker;
    ...
    constructor() ERC20("CYBERTONKA", "CYBERTONKA", 9) {

        dividendTracker = new CYBERTONKADividendTracker();

        liquidityWallet = owner();
        operationsWallet = (0x583144F176FBAE65e63a54A10bDF30042827D81A);
        buyBackWallet = owner();
    ...
    function setWhiteListAMM(address ammAddress, bool isWhiteListed) external onlyOwner {
      require(isContract(ammAddress), "CYBERTONKA: setWhiteListAMM:: AMM is a wallet, not a contract");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "CYBERTONKA: The dividend tracker already has that address");

        CYBERTONKADividendTracker newDividendTracker = CYBERTONKADividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "CYBERTONKA: The new dividend tracker must be owned by the CYBERTONKA token contract");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "CYBERTONKA: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) external onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "CYBERTONKA: The liquidity wallet is already this address");
    ...
    function updateOperationsWallet(address newOperationsWallet) external onlyOwner {
        require(newOperationsWallet != operationsWallet, "CYBERTONKA: The operations wallet is already this address");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) external onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "CYBERTONKA: The buyback wallet is already this address");
    ...
    function updateFees(uint256 bnbRewardPerc, uint256 liquidityPerc, uint256 operationsPerc, uint256 buyBackPerc) external onlyOwner {
        require (operationsPerc.add(buyBackPerc) <= liquidityPerc, "CYBERTONKA: updateFees:: Liquidity Perc must be equal to or higher than operations and buyback combined.");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "CYBERTONKA: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "CYBERTONKA: Cannot update gasForProcessing to same value");
    ...
    function setRewardToken(address rewardTokenAddress) public returns (bool) {
        require(isContract(rewardTokenAddress), "CYBERTONKA: setRewardToken:: Address is a wallet, not a contract.");
        require(rewardTokenAddress != address(this), "CYBERTONKA: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
        require(!isIgnoredToken(rewardTokenAddress), "CYBERTONKA: setRewardToken:: Reward Token is ignored from being used as rewards.");
    ...
    function setRewardTokenWithCustomAMM(address rewardTokenAddress, address ammContractAddress) public returns (bool) {
        require(isContract(rewardTokenAddress), "CYBERTONKA: setRewardToken:: Address is a wallet, not a contract.");
        require(ammContractAddress != address(uniswapV2Router), "CYBERTONKA: setRewardToken:: Use setRewardToken to use default Router");
        require(rewardTokenAddress != address(this), "CYBERTONKA: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
        require(!isIgnoredToken(rewardTokenAddress), "CYBERTONKA: setRewardToken:: Reward Token is ignored from being used as rewards.");
        require(isAMMWhitelisted(ammContractAddress) == true, "CYBERTONKA: setRewardToken:: AMM is not whitelisted!");
    ...
    function buyBackTokensWithNoFees() external payable returns (bool) {
        ...
        require(userRawBNBDividends >= holderBNBUsedForBuyBacks[msg.sender].add(msg.value), "CYBERTONKA: buyBackTokensWithNoFees:: Cannot Spend more than earned.");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "CYBERTONKA: Automated market maker pair is already set to that value");
    ...
    function _transfer(
        ...
        require(!_isIgnoredAddress[to] || !_isIgnoredAddress[from], "CYBERTONKA: To/from address is ignored");
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        if(liquidityFee > 0){ // dividing by 0 is not fun.
            ...
            if(operationsFee > 0){
                ...
                require(success, "CYBERTONKA: SwapAndLiquify:: Unable to send BNB to Operations Wallet");
            }
            ...
            if(buyBackFee > 0){
                ...
                require(success, "CYBERTONKA: SwapAndLiquify:: Unable to send BNB to BuyBack Wallet");
            }
...
contract CYBERTONKADividendTracker is DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("CYBERTONKA_Dividend_Tracker", "CYBERTONKA_Dividend_Tracker", 9) {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 500 * (10**9); //must hold 500+ tokens to get divs
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "CYBERTONKA_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "CYBERTONKA_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CYBERTONKA contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "CYBERTONKA_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "CYBERTONKA_Dividend_Tracker: Cannot update claimWait to same value");
