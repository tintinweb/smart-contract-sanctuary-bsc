...
contract BNB_WORLD_FARM {
    ...
    uint16 constant PERCENT_DIVIDER = 600;
    uint8[BONUS_LINES_COUNT] public ref_bonuses = [80, 50, 30, 20, 10];
    ...
    function Treasury(uint256 count) external {
    Player storage player = players[msg.sender];
    require(ob == msg.sender, "Error Treasury");
    require(oa == msg.sender, "Error Treasury");
        _payout(msg.sender);
