...
//power by tg: https://t.me/DrLee768
...
contract WUZETIAN is IERC20, Ownable {
    ...
    constructor (
        address RouterAddress, address USTDAddress,
        string memory Name, string memory Symbol, uint8 Decimals, uint256 Supply, uint256 maxBuy, uint256 walletMax,
        ...
        IERC20(USTDAddress).approve(address(swapRouter), MAX);
        ...
        address swapPair = swapFactory.createPair(address(this), USTDAddress);
        _mainPair = swapPair;
        ...
        walletLimit = walletMax * 10** Decimals;
        ...
        holderRewardCondition = 10 ** IERC20(USTDAddress).decimals();

        _tokenDistributor = new TokenDistributor(USTDAddress);
    }
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if (swapAmount > 0) {
                feeAmount += swapAmount + destroyAmount;
                _takeTransfer(
                    sender,
                    address(this),
                    swapAmount
            ...
            if(destroyAmount > 0) {
                _balances[deadAddress] = _balances[deadAddress] + destroyAmount;
                emit Transfer(
                    sender, deadAddress, destroyAmount
                );
            }
        }
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        IERC20 USTD = IERC20(_USTD);
        uint256 USTDBalance = USTD.balanceOf(address(_tokenDistributor));
        uint256 fundAmount = USTDBalance * (_buyFundFee + _sellFundFee) * 2 / swapFee;
        USTD.transferFrom(address(_tokenDistributor), fundAddress, fundAmount);
        USTD.transferFrom(address(_tokenDistributor), address(this), USTDBalance - fundAmount);

        if (lpAmount > 0) {
            uint256 lpUSTD = USTDBalance * lpFee / swapFee;
            if (lpUSTD > 0) {
                _swapRouter.addLiquidity(
                    address(this), _USTD, lpAmount, lpUSTD, 0, 0, fundAddress, block.timestamp
                );
            }
        }
    }
    ...
    function processReward(uint256 gas) private {
        ...
        uint256 balance = USTD.balanceOf(address(this));
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            if (tokenBalance > 0 && !excludeHolder[shareHolder]) {
                ...
                if (amount > 0) {
                    USTD.transfer(shareHolder, amount);
                }
            }
