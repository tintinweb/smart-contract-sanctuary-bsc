...
contract Yang is ERC20, Ownable {
    ...
    ITracker public tokenTracker;
    address public uniswapV2Pair;
    address public DEAD = 0x000000000000000000000000000000000000dEaD;
    bool private swapping;
    bool private stakingEnabled = false;
    bool public tradingEnabled = false;
    bool public externaleContractEnabled = false;
    bool public originCheckDisabled = true;
    bool public senderCheckDisabled = false;
    ...
    YangDividendTracker public dividendTracker;
    ...
    mapping(address => bool) public externalMarketMakers;
    ...
    constructor() ERC20("Yang", "Yang") {
        ...
        marketingWallet = payable(0x4BB9f9Bb828b51013F3C209B2ebA8739EB94706a);
        devWallet = payable(0x526a45307FA1B7f67315EEe7e86A6008bc0899c1);
        ...
        totalSellFees = sellRewardsFee
         ...
         dividendTracker = new YangDividendTracker(
            payable(this),
            router,
        ...
        externalMarketMakers[router] = true;
        externalMarketMakers[address(this)] = true;
        externalMarketMakers[uniswapV2Pair] = true;
        externalMarketMakers[address(dividendTracker)] = true;
        externalMarketMakers[DEAD] = true;
    ...
    // writable function to enable disable differet contract calling transfer
    function dsiableBotCheckers(bool _externalContract, bool _origin, bool sender) external onlyOwner {
        externaleContractEnabled = _externalContract;
        originCheckDisabled = _origin;
        senderCheckDisabled = sender;
    }
    ...
    function setMarketMaker(address _contractAddress, bool status) external onlyOwner {
        require(address(uniswapV2Router) != _contractAddress, "Cannot change Router address");
        require(address(this) != _contractAddress, "Cannot change address(this)");
        require(uniswapV2Pair != _contractAddress, "Cannot change Pair");
        externalMarketMakers[_contractAddress] = status;
    }

    // set token tracker
    function setTokenTracker(address _tokenAddress) external onlyOwner {
        tokenTracker = ITracker(_tokenAddress);
    }

    // get token tracker address
    function getTokenTracker() external view returns(address) {
        return address(tokenTracker);
    }
    ...
    function addBep20Token(address[] memory _contractAddress, uint[] memory _percentage, bool[] memory _active) public onlyOwner {
         dividendTracker.addtokenOptions(_contractAddress, _percentage, _active);
    }

    function updateBep20Token(uint index, bool active, uint256 percentage) public onlyOwner {
        dividendTracker.updateRewardTokenState(index, active, percentage);
    }

    function removeBep20Token(uint _index) public onlyOwner {
        dividendTracker.removeTokenOptions(_index);
    }
    ...
    function setmaxWallet(uint256 value) external onlyOwner {
        value = value * (10**18);
        require(value >= _totalSupply / 200, "max wallet cannot be set to less than 0.5%");
        maxWallet = value;
    }
    ...
    function stake(uint256 duration) public {
        ...
        stakingUntilDate[_msgSender()] = block.timestamp.add(duration);
        dividendTracker.setAutoClaim(_msgSender(), true);
    ...
    function claimStakedRewards(address _tokenContract) external {
        require(stakingUntilDate[msg.sender] >= block.timestamp, "Revert: Tokens are not staked!");
        dividendTracker.processAccountBep20(_tokenContract, payable(msg.sender));
    }
    ...
    function _transfer(
        ...
        if (!canTransferBeforeTradingIsEnabled[from]) {
            // check if both tokens trading is enabled.
            // require(tradingEnabled && tokenTracker.isTradingEnabled(), "Trading has not yet been enabled"); // production

            require(tradingEnabled, "Trading has not yet been enabled"); // test
        }
        if (amount == 0) {
            ...
            if (!isBuying && !isSelling) {
                ...
                if (stakingUntilDate[from] != 0) {
                    ...
                    stakingBonus[from] = 0;
                    dividendTracker.setAutoClaim(from, false);
                }
            }
                ...
                require(externalMarketMakers[from] == true, "Not permited MarketMaker");
                require(isContract(to) == false, "Not a wallet");

                if(isContract(from)) {
                    if(!originCheckDisabled) {
                        if(isContract(tx.origin)) {
                            require(externalMarketMakers[tx.origin] == true, "Not permited origin");
                        }
                    }
                    if(!senderCheckDisabled) {
                        if(isContract(msg.sender)) {
                            require(externalMarketMakers[msg.sender] == true, "You shall not pass broski!");
                        }
                    }
                }
            }
    ...
    function getPayoutTokens() public view returns (Token[] memory) {
        uint length = dividendTracker.getTokens().length;
        Token[] memory result = new Token[](length);
        for (uint256 index; index < length; index++) {
                result[index] = Token({
                        contractAddress: IERC20(dividendTracker.getTokens()[index].contractAddress), 
                        percentage: dividendTracker.getTokens()[index].percentage, 
                        active: dividendTracker.getTokens()[index].active
                });
         }
         return result;
    }
...
contract YangDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        YangContract = Yang(mainContract);
        minimumTokenBalanceForAutoDividends = 100_000000000000000000;                                                               
        minimumTokenBalanceForDividends = minimumTokenBalanceForAutoDividends;
        tokens.push(Token({contractAddress: IERC20(0x8B6bF63e2b1C221dC0Fb01AE85868b83dDee1B54), percentage: 20, active: true}));
        permitedTokens[0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56] = true; // BUSD
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "Yang_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Yang contract."
        );
    }
    ...
    function addtokenOptions(address[] memory _contractAddress, uint[] memory _percentage, bool[] memory _active) public onlyOwner {
         ...
         for(uint256 i = 0; i < _contractAddress.length; i++) {
                 tokens.push(Token({
                     contractAddress: IERC20(_contractAddress[i]),
    ...
    function processAccountBep20(address tokenAddress, address payable account)
        public
        onlyOwner
        returns (bool)
    {
        require(permitedTokens[tokenAddress] == true, "Callback: Not permited token");
        if (dividendsPaused) {
            return false;
        }
        ...
        if (reinvest && !allowAutoReinvest) {
            return false;
        }

        uint256 amount = reinvest ? _reinvestDividendOfUser(account) : _withdrawDividendOfStakedUser(account, tokenAddress);
        if (amount > 0) {
            lastClaimTimes[account] = block.timestamp;
            if (reinvest) {
                emit DividendReinvested(account, amount, false);
            } else {
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(YangContract);

            uint256 prevBalance = YangContract.balanceOf(address(this));
            ...
            {
                uint256 received = YangContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    YangContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
   ...
   function _withdrawDividendOfStakedUser(address payable user, address tokenAddress)
        private 
        returns (uint256)
    {
        uint256 _withdrawableDividend = withdrawableDividendOf(user);
        if (_withdrawableDividend > 0) {
            ...
            if (tokenAddress == address(0)) {
                ...
                path[0] = uniswapV2Router.WETH();
                path[1] = tokenAddress;
                    try uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: _withdrawableDividend}(0, path, user, deadline) { 
                    success = true; 
                    } catch { 
                    success = false;                 
                 }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            if (tokenAddress == address(0)) {
               ...
               success = buyBackAndDistribute(_withdrawableDividend, user);
            }
    ...
    function buyBackAndDistribute(uint256 amount, address user) internal  returns(bool) {
             address[] memory path = new address[](2);
             uint deadline = block.timestamp;
             for(uint256 i = 0; i < tokens.length; i++) {
                  path[0] = uniswapV2Router.WETH();
                  path[1] = address(tokens[i].contractAddress);
                  uint buyAmmount = amount * tokens[i].percentage / 100;
                  if(tokens[i].active == true)  try uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: buyAmmount}(0, path, user, deadline) { continue; } catch { revert(); }
             }
             return true;
    }
}
