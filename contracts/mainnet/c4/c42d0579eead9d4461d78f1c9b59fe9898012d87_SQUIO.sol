...
contract SQUIO is Context, IBEP20, Ownable {
    ...
    function _sendToCharity(uint256 tCharity, address sender) private {
        uint256 currentRate = _getRate();
        uint256 rCharity = tCharity.mul(currentRate);
        _rOwned[whiscAdd] = _rOwned[whiscAdd].add(rCharity);
        _tOwned[whiscAdd] = _tOwned[whiscAdd].add(tCharity);
        emit Transfer(sender, whiscAdd, tCharity);
    }
