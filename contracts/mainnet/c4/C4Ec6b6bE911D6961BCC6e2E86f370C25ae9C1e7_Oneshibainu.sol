...
One ShibaInu is a community-driven token deployed on the Binance Smart Chain through the BSC medium. It is community driven and backed by a strong team to deliver for investors with the goal of creating wealth and use case just by holding the token

Taxes
8% of every transaction is taken and will be used for enhancements, operations, and marketing of the project
2% of every transaction is transformed into liquidity. It's automatic and helps create a price floor
*/
...
contract Oneshibainu is IERC20, Ownable {
    ...
    string _name = "OneShibaInu";
    string _symbol = "1SHIB";
    ...
    function viewFees() external view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        return (liquidityFee, varMarketingFee, giveawayFee, reflectionFee, totalFee, feeDenominator);
    }
    ...
    function changeFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _varMarketingFee, uint256 _giveawayFee) external onlyOwner {
        liquidityFee = _liquidityFee;
        reflectionFee = _reflectionFee;
        varMarketingFee = _varMarketingFee;
        giveawayFee = _giveawayFee;
        totalFee = liquidityFee.add(reflectionFee).add(varMarketingFee).add(giveawayFee);
