...
OsmoX bets on a DEFI ecosystem capable of offering sustainable income to all $OsmoX Holders, be it with Mining, Staking, Farming, NFT or the incubation of OsmoX projects.
When you choose a project to diversify what is important is future sustainability, through OsmoX, we are creating a safe place where crypto investors from all over the world can generate high-yield sustainable income.

OsmoX-$OsmoX

✅ OsmoX Mining.
✅ OsmoX Renewable Energies.
✅ OsmoX Staking
✅ OsmoX Farming
✅ OsmoX Launch
✅ OsmoX Marketplace
...
Telegram: https://t.me/Osmoxbsc
Website: https://osmox.live
*/
...
contract OsmoX is IERC20, Context, Ownable {
    ...
    address private OsmoX__Marketing_Wallet = 0x12D3Fa58Ff504f59D39302088f79e923b6C340d3;
    address private OsmoX__Team_Wallet = 0x0433Fc6670F90a292381fb238ecccdfB7E10fC30;
    
    string private constant _symbol = "OsmoX";
    string private constant _name = "OsmoX";

    uint256 public OsmoX__max_BuyLimit = 5000000 * 10**9;
    uint256 public OsmoX__max_SellLimit = 5000000 * 10**9;

    uint256 public OsmoX__max_WalletLimit = 10000000  * 10**9;
    ...
    Taxes public sellTaxes = Taxes(0, 8, 8, 9);
    Taxes public taxes = Taxes(0, 3, 2, 5);
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[OsmoX__Marketing_Wallet] = true;
        _isExcludedFromFee[OsmoX__Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[OsmoX__Marketing_Wallet] = true;
        allowedTransfer[OsmoX__Team_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= OsmoX__max_BuyLimit, "You are exceeding OsmoX__max_BuyLimit");
            require(balanceOf(to) + amount <= OsmoX__max_WalletLimit, "You are exceeding OsmoX__max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= OsmoX__max_SellLimit, "You are exceeding OsmoX__max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= OsmoX__max_WalletLimit, "You are exceeding OsmoX__max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(OsmoX__Marketing_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(OsmoX__Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function OsmoX__Update_MarketingWallet(address newWallet) external onlyOwner{
        OsmoX__Marketing_Wallet = newWallet;
    }
    
    function OsmoX__Update_TeamWallet(address newWallet) external onlyOwner{
        OsmoX__Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        OsmoX__max_BuyLimit = maxBuy * 10**decimals();
        OsmoX__max_SellLimit = maxSell * 10**decimals();
    }
    
    function OsmoX__Max_Walletlimit(uint256 amount) external onlyOwner{
        OsmoX__max_WalletLimit = amount * 10**decimals();
    }
