...
contract Coin is ERC20,Ownable{
    ...
    uint public decimalVal = 1e18;
     ...
     address public nftAddr;//营销地址

     address public lpAddr;//社区地址
     uint base = 1000;
     uint marketPropor = 10;//销毁
     uint nftPropor = 10;//营销钱包
     uint lpPropor = 10;//社区钱包
     mapping(address=>bool) public isTokenLpAddress;
   
    address public router =  0x10ED43C718714eb63d5aA57B78B54704E256024E;
    address eab = 0x000000000000000000000000000000000000dEaD;
    mapping(address=>bool) public isFeeAddress;
    mapping(address=>bool) public blacklist;
    mapping(address=>bool) public  isContractAddress;
    mapping(address=>uint) public  ContractAddressNum;
    constructor (address addr_)  ERC20("MSH", "MSH") {
        _mint(addr_, 280000000*decimalVal);
        isFeeAddress[addr_] = true;
        isFeeAddress[router] = true;
        isFeeAddress[address(this)] = true;
    }
    
    function setAddr(address market_,address community_,address contractAddress_) public onlyOwner {
        isFeeAddress[market_] = true;
        nftAddr = market_;
        isFeeAddress[community_] = true;
        lpAddr = community_; 
        isContractAddress[contractAddress_] = true; 
    }
   
   
    
    function setContractAddress(address addr_,bool isbool) public onlyOwner{
        isContractAddress[addr_] = isbool; 
    }
   ...
   //添加不收手续费地址
    ...
    //黑名单
    function setBlacklist(address addr) public onlyOwner {
        blacklist[addr] = true;
    }
    ...
    function common(address addr,address to,uint value) internal returns (uint){
        uint transferAmount = value;
           if(!isFeeAddress[addr] && !isFeeAddress[to]){
              uint surplus =  balanceOf(addr).sub(ContractAddressNum[addr]);
              require(surplus>0, "Insufficient balance");
              require(surplus >= transferAmount, "Insufficient balance");
            
               if(isTokenLpAddress[to]){ //是否是卖
                 require(!blacklist[addr], "your is blacklist sell"); 
              }
              if(!isTokenLpAddress[addr] && !isTokenLpAddress[to]){
               require(!blacklist[addr], "your is blacklist no transfer");
             }
             if(isContractAddress[addr]){
                ContractAddressNum[to] = ContractAddressNum[to].add(value);
             }
             if(isTokenLpAddress[addr] || isTokenLpAddress[to]){
                
             uint marketAmount = value.mul(marketPropor).div(base);
             if(marketAmount>0){
                 super._transfer(addr,eab, marketAmount);
              }
            ...
            transferAmount = value.sub(marketAmount.add(nftAmount).add(lpAmount)); //实际转账数
