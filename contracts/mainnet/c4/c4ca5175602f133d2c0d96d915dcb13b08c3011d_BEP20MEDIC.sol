...
contract BEP20MEDIC is BEP20, Auth {
    ...
    string public constant name = "MEDI-C";
    ...
    uint256 public totalFee = marketingFee + liquidityFee + teamFee + developmentFee + charityFee + rewardFee;
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0xf5b2dA50bC29a4144e08957BB2ad06EAde558759;
        charityFeeReceiver = 0xAC81133Ac795059cBdb3EB225D2eE1A879FDC3cE;
        developmentFeeReceiver = 0xB4D3fC1C256498192d1B8FC345707c5c149D889a;
        teamFeeReceiver = 0x47131bC404586fB147d9AFb2Ca111a9B7c2F7827;
        rewardFeeReceiver = 0x708fC58267a4dBb7402C6C20BEF848829007e90d;
    ...
    function tradingStatus_launchmode(uint256 confirm) external onlyOwner {
        require(confirm == 93112341,"Accidental Press");
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBReward = (amountBNB * rewardFee) / totalBNBFee;
    ...
    function update_fees() internal {
        if(!launchMode){
            require(totalFee.mul(buyMultiplier).div(100) <= 100, "Buy tax cannot be more than 20%");
            require(totalFee.mul(sellMultiplier).div(100) <= 250, "Sell tax cannot be more than 25%");
            require(totalFee.mul(transferMultiplier).div(100) <= 200, "Transfer Tax cannot be more than 20%");
        }
    ...
    function setFees_base1000(uint256 _liquidityFee,  uint256 _marketingFee, uint256 _teamFee, uint256 _developmentFee, uint256 _charityFee, uint256 _rewardFee) external onlyOwner {
        liquidityFee = _liquidityFee;
        marketingFee = _marketingFee;
        teamFee = _teamFee;
        developmentFee = _developmentFee;
        rewardFee = _rewardFee;
        charityFee = _rewardFee;

        totalFee = _liquidityFee + _marketingFee + _teamFee + _developmentFee + _rewardFee + _charityFee;
        
        update_fees();
    }

    function setFeeReceivers(address _marketingFeeReceiver, address _teamFeeReceiver, address _developmentFeeReceiver, address _charityFeeReceiver, address _rewardFeeReceiver) external onlyOwner {
        ...
        require(_charityFeeReceiver != address(0),"Charity fee address cannot be zero address");
        require(_rewardFeeReceiver != address(0),"Reward fee address cannot be zero address");
