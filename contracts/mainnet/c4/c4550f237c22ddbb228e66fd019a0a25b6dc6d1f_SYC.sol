...
contract SYC is BEP20, Auth {

    using SafeMath for uint256;

    string constant _name = "SYC";
    ...
    function changeFees(uint256 newLiqFee, uint256 newRewardFee, uint256 newMarketingFee) external authorized {
        require(newLiqFee+(newRewardFee)+(newMarketingFee) <=20, "Buy Taxes cannot be higher than 20%");
    ...
    function changeFeesSell(uint256 newLiqFeeSell, uint256 newRewardFeeSell, uint256 newMarketingFeeSell) external authorized {
        require(newLiqFeeSell+(newRewardFeeSell)+(newMarketingFeeSell) <=20, "Sell Taxes cannot be higher than 20%");
