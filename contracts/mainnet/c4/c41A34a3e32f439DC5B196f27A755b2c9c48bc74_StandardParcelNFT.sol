    ...
    ReentrancyGuardUpgradeable
{
    ...
    function upgradeParcel(uint256 tokenId) external onlyMunicipality  {
        upgradedParcelsMapping[tokenId] = true;
        emit StandardParcelUpgraded(msg.sender, tokenId);
    }

    function upgradeParcels(uint256[] memory tokenIds) external onlyMunicipality  {
    ...
    function getParcelInfo(uint256 token)  public pure returns (uint256, uint256, uint256, uint256) { //private pure
        uint256 _token = token >> 8;
        uint256 x = _token & 65535;
        uint256 y = (_token >> 16) & 65535;
        uint256 pt = (_token >> 32) & 255;
        uint256 lt = _token >> 40;
        return (x, y, pt, lt);
    }
