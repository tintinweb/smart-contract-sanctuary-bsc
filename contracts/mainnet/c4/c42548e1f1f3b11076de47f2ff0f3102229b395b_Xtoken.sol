...
contract Xtoken is ERC20, Ownable {
    ...
    uint8  _taxFee ;
    uint8  _liquidityFee ;
    uint8  _extraFeeOnSell ;
    uint8  _marketingFee ;
    uint8  _extraSellFee;
    address private marketingWallet = payable(0x0e35cEE853b65068f5f3D650492ADecFC58080E6);
    address private liqWallet = payable(0x0e35cEE853b65068f5f3D650492ADecFC58080E6);
    address private taxFeeWallet = payable(0xbcE355f3EA0eCE28D27e9e9451eCedcfBC18FdF6);
    ...
    constructor() ERC20("Xtoken", "SVX") {
        _mint(msg.sender, 10000000 * 1 ether);
        maxWalletSize = 1000 * 1 ether;
        maxTransactionSize = 500 * 1 ether;
    }


 

    function setRewardsFee(uint8 newrewardsFee) public onlyOwner{
     _rewardsFee =  newrewardsFee;
} 

     function setTaxFee(uint8 newtaxFee) public onlyOwner{
     _taxFee =  newtaxFee;
} 
    ...
    function setLiquidityFee(uint8 newliquidityFee) public onlyOwner{
     _liquidityFee =  newliquidityFee;
        ...
        function setExtraFeeOnSell(uint8 newextraFeeOnSell) public onlyOwner{
     _extraFeeOnSell =  newextraFeeOnSell;
} 

    
     function setMarketingFee(uint8 newmarketingFee) public onlyOwner{
     _marketingFee =  newmarketingFee;
} 

     function setExtraSellFee(uint8 newextraSellFee) public onlyOwner{
     _extraSellFee =  newextraSellFee;
} 
    ...
    function setMarketingWallet (address newMarketingWallet) external onlyOwner {
     ...
     function setLiqWallet (address newLiqWallet) external onlyOwner {
        liqWallet = newLiqWallet;
        //emit SetLiqWallet(newLiqWallet);
    } 

     function setTaxFeeWallet (address newTaxFeeWallet) external onlyOwner {
        taxFeeWallet = newTaxFeeWallet;
        //emit SetTaxFeeWallet(newTaxFeeWallet);
    } 
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override(ERC20) {
        ...
        if(recipient != lpAddress) {
            if(whiteList[recipient].isRewardsFee == false) {
             super._transfer(sender, marketingWallet, amount * _marketingFee / 100);
             feeAmount += amount * _marketingFee / 100;
            }   
            if(whiteList[recipient].isMarketingFee == false) {
             super._transfer(sender, liqWallet, amount * _rewardsFee / 100);
             feeAmount += amount * _rewardsFee / 100;
            }
            if(whiteList[recipient].isTaxFee == false) {
                super._transfer(sender, taxFeeWallet, amount * _taxFee / 100);
                feeAmount += amount * _taxFee / 100;
            }
        } else {
            super._transfer(sender, marketingWallet, amount * _extraSellFee / 100);
            super._transfer(sender, liqWallet, amount * _extraSellFee / 100);
            feeAmount += amount * _extraSellFee / 100 + amount * _extraSellFee / 100;
        }
