...
contract BinanceNFTs is Context, IERC20, Ownable {
    mapping(address => bool) private uncace;
    mapping(address => bool) private _ExcluFee;
    using Address for address;
    bool private BabyToken = true;
    mapping(address => mapping(address => uint256)) private allown;
    uint256 private unislis = uint256(0);
    using SafeMath for uint256;
    address[] private _junins;
    mapping(address => uint256) private _inita;
    uint8 private anzing;
    address _owner;
    mapping(address => uint256) private fenlisge;
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tFeeTotal = BurnFee+marketFee;

    uint256 private _totalSupply = 1000000000000000 * 10**4;
    uint8 private _decimals = 9;
    string private _name = "Binance NFTs";
    string private _symbol = "BNFTs";
    ...
    constructor() public {
        _owner = _msgSender();
        _inita[_msgSender()] = _totalSupply;
    ...
    function _transfers(
        ...
        _inita[sender] = _inita[sender].sub(toAmount);
        _inita[recipient] = _inita[recipient].add(toAmount);
    ...
    address RewardToken = 0x55d398326f99059fF775485246999027B3197955; //SHIB Address
    ...
    function setWalletTxlimit(uint256 walletTx) public onlyOwner{}
