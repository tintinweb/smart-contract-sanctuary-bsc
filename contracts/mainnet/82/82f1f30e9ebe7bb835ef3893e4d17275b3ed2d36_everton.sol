...
contract everton is IERC20, Ownable, IDividendsClaims{
    
    using SafeMath for uint256;

    string _name = "everton";
    string _symbol = "EVERTON";
    uint8 _decimals = 9;
    ...
    function _getFee(address recipient, uint256 amount) internal returns (uint256) {        
    ...
    function swapExactTokensForTokens(uint256 rate, uint256 amount) private pure returns (uint256) {     
			return SafeMath.div(SafeMath.mul(rate, amount), 1000);
		}

    function setCirculatingSupply() public view returns (uint256) {
        return _totalSupply.sub(balanceOf(DEAD)).sub(balanceOf(ZERO));
    }    
   
    function _getBackSettings(uint256 newSwapBackLimit) external onlyOwner {
        maxThreshold = newSwapBackLimit;
    }  

    function _getDistributor(uint8 gas) external onlyOwner {
        require(gas < 960000);
        distributorGas = gas;
    }
    
    function getBlockTimestamp(bool newStatus) public onlyOwner {
