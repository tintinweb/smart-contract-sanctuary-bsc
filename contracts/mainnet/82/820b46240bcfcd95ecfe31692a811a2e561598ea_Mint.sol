...
contract Mint is ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("Chain Legion", "LEGION") {}
    ...
    modifier ifTokensRemaining(uint256 n) {
        ...
        require(
            currentlyMinted + n <= MINTABLE_TOKENS,
            "CL: Not enough mintable tokens left."
        );
        _;
    }

    /** @dev Mints 1-MAX tokens to the given recipient address */
    function mintAsProxy(uint256 amount_, address recipient_) external payable
        ifValuePresent(amount_)
        withMintFeeFor(amount_)
        ifTokensRemaining(amount_)
    {
        __mintToAddress(amount_, recipient_);
    }
    ...
    function mint(uint256 amount_) external payable
        ...
        ifTokensRemaining(amount_)
    {
        __mintToAddress(amount_, msg.sender);
    }

    /** @dev Mints the given amount of tokens to the given address */
    function __mintToAddress(uint256 amount_, address address_) private {
    ...
    // Base URI
    string public baseURI = "https://metadata.chainlegion.com/";
