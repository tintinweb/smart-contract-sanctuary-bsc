...
|_______ \___  >__|_|  /\____/|___|  (____  /\____ |\___  > /_______  /|__| (____  /___|  /\____ | 
        \/   \/      \/            \/     \/      \/    \/          \/           \/     \/      \/    
Lemonade Stand - BSC BUSD Lemon Crops Farmer
*/
...
contract BusdLemons {
	...
	address erctoken = 0x35bd225d16B8E8d2250EC8F410810d5067e778d7; /** LEMON Mainnet (testing only) **/
    ...
    /* Two developer wallets (covers development, marketing, hosting, end more) */
    address public dev1;
    address public dev2;

    /* Lottery wallet hold rewards to be distributed back to the players */
    ...
    constructor(address _dev1, address _dev2, address _lottery) {
		require(!isContract(_dev1) && !isContract(_dev2) && !isContract(_lottery));
        owner = msg.sender;
        dev1 = _dev1;
        dev2 = _dev2;
        lottery = _lottery;
    ...
    function buildStands(bool isCompound) public {
    ...
    function payFees(uint256 eggValue) internal returns(uint256){
        ...
        uint256 tax_dev = tax.mul(2) + tax.div(4);
        uint256 tax_lotto = tax.div(2);

        token_BUSD.transfer(dev1, tax_dev);
        token_BUSD.transfer(dev2, tax_dev);
        token_BUSD.transfer(lottery, tax_lotto);
        return tax.mul(5);
    }
    ...
    function getCompoundBonus() public view returns(uint256){
        return COMPOUND_BONUS;
    }
...
/**
Lemonade Stand: BUSD
 Build Stands, Create Lemonade, and Sell for BUSD. 
  8% daily daily Rate
