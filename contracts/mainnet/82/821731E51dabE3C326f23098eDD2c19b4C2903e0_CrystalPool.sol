...
// File: contracts/locksTokens/CrystalPool.sol
...
// File: contracts/lockTokens/CrystalPool.sol
...
contract CrystalPool is Ownable, Pausable, WithSigner {
    ...
    event Deposit(IERC20 token, uint256 amount, address from, bytes32 idempotencyKey);
    event Unlock(IERC20 token, uint256 amount, address to, bytes32 idempotencyKey);
    event Withdraw(IERC20 token, uint256 amount, address to,bytes32 idempotencyKey);
    ...
    {   
        ...
        bool isPermitValid = validateData(_signature,_idempotencyKey, _depositAmount, _sender, _blockExpiry);
        ...
        emit Deposit(_token, _depositAmount, msg.sender, _idempotencyKey);
        return true;
    }
    ...
    function unLock(
        ...
        bool used = getUsedKeys(_idempotencyKey);
        require(!used, "FACTORY: Permit already used");
        bool isPermitValid = validateData(_signature,_idempotencyKey, _unLockAmount, _sender, _blockExpiry);
        require(isPermitValid, "No signer match");
        ...
        setUsedKeys(_idempotencyKey);
        emit Unlock(_token, _unLockAmount, msg.sender, _idempotencyKey);
        return true;
    }
    ...
    function withdraw(
        ...
        bool used = getUsedKeys(_idempotencyKey);
        require(!used, "FACTORY: Permit already used");
        bool isPermitValid = validateData(_signature,_idempotencyKey, _withdrawAmount, _sender, _blockExpiry);
        require(isPermitValid, "No signer match");
        ...
        emit Withdraw(_token, _withdrawAmount, msg.sender, _idempotencyKey);
    }
