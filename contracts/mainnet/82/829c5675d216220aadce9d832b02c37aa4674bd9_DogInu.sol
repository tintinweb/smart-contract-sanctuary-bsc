...
library Address{
 ...
 contract DogInu is Context, IERC20, Ownable {
    ...
    uint256 public maxSellLimit = 100_000_000 * 10**9;
    uint256 public maxWalletLimit = 100_000_000 * 10**9;
    
    uint256 public genesis_block;
    
    address public marketingWallet = 0xE75c52F564Cb1DeFa4239B77319418d1EA1FCDdc;
    address public DogWallet = 0xE75c52F564Cb1DeFa4239B77319418d1EA1FCDdc;
    ...
    Taxes public taxes = Taxes(0, 1, 1, 1);
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[DogWallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[marketingWallet] = true;
        allowedTransfer[DogWallet] = true;
    ...
    function setTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _Dog) public onlyOwner {
       taxes = Taxes(_rfi,_marketing,_liquidity,_Dog);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _Dog) public onlyOwner {
       sellTaxes = Taxes(_rfi,_marketing,_liquidity,_Dog);
        emit FeesChanged();
    }
    ...
    function _takeDog(uint256 rDog, uint256 tDog) private {
        totFeesPaid.Dog +=tDog;

        if(_isExcluded[address(this)])
        {
            _tOwned[address(this)]+=tDog;
        }
        _rOwned[address(this)] +=rDog;
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rDog) = _getRValues2(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.tDog = tAmount*temp.Dog/100;
        s.tTransferAmount = tAmount-s.tRfi-s.tMarketing-s.tLiquidity-s.tDog;
        return s;
    }
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rMarketing, uint256 rLiquidity){
        ...
        uint256 rDog = s.tDog*currentRate;
        rTransferAmount =  rAmount-rRfi-rMarketing-rLiquidity-rDog;
        return (rAmount, rTransferAmount, rRfi,rMarketing,rLiquidity);
    }
    
    function _getRValues2(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 rDog) {

        if(!takeFee) {
          return(0);
        }

        rDog = s.tDog*currentRate;
        return (rDog);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tMarketing + s.tDog);
        }
        if(s.rMarketing > 0 || s.tMarketing > 0) _takeMarketing(s.rMarketing, s.tMarketing);
        if(s.rDog > 0 || s.tDog > 0) _takeDog(s.rDog, s.tDog);
        emit Transfer(sender, recipient, s.tTransferAmount);
        
    }

    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.marketing + temp.Dog) * 2;
        ...
        uint256 DogAmt = unitBalance * 2 * temp.Dog;
        if(DogAmt > 0){
            payable(DogWallet).sendValue(DogAmt);
        }
    }
    ...
    function updateDogWallet(address newWallet) external onlyOwner{
        DogWallet = newWallet;
    }
