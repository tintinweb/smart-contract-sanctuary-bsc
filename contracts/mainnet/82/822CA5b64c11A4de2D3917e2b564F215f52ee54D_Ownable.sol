...
contract REVCC is Context, IBEP20, Ownable {
  ...
  constructor() {
    _name = "RENEWABLE ENERGY FOR VEHICLE CHARGING COIN";
    _symbol = "REVCC";
    _decimals = 18;
  ...
  */
    ...
    if(_isExcludedFromFee[sender] || _isExcludedFromFee[recipient]){
      ...
      if(sender == uniswapV2Pair){
        if (_taxBuyFee == 0) {
          ...
          fee = SafeMath.div(SafeMath.mul(amount, SafeMath.div(_taxBuyFee,100)), 100);
        ...
        if (_taxBuyFee == 0) {
          ...
          fee = SafeMath.div(SafeMath.mul(amount, SafeMath.div(_taxSellFee,100)), 100);
          taxedValue = SafeMath.sub(amount, fee);
          //as we have a tax, we send it to the tax smartcontract
          _balances[_fundWallet] = _balances[_fundWallet].add(fee);
          emit Transfer(sender, _fundWallet, fee);
        ...
        if (_taxBuyFee == 0) {
          ...
          fee = SafeMath.div(SafeMath.mul(amount, SafeMath.div(_taxTxFee,100)), 100);
          taxedValue = SafeMath.sub(amount, fee);
          //as we have a tax, we send it to the tax smartcontract
          _balances[_fundWallet] = _balances[_fundWallet].add(fee);
          emit Transfer(sender, _fundWallet, fee);
  ...
  function getBuyTaxFee() external view returns(uint256) {
      return _taxBuyFee;
  }
  ...
  function setBuyTaxFee(uint256 taxFee) external onlyOwner() {
    ...
    function getSellTaxFee() external view returns(uint256) {
      return _taxSellFee;
  }
  ...
  function setSellTaxFee(uint256 taxFee) external onlyOwner() {
    ...
    function getTxTaxFee() external view returns(uint256) {
      return _taxTxFee;
  }

  function setTxTaxFee(uint256 taxFee) external onlyOwner() {
      require(taxFee >= 0 && taxFee <= 2000, 'taxFee should be in 0 - 2000');
      _taxTxFee = taxFee;
  }
