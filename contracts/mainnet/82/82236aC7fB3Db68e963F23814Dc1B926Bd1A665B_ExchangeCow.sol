...
contract ExchangeCow is OwnableUpgradeable {
    ...
    function checkPrice(uint packId) public view returns(uint,uint){
        uint bvtAmount = countingOut(packInfo[packId].price * rate[0] / 100, bvtPrice);
        uint tokenAmount = countingOut(packInfo[packId].price * rate[1] / 100, hoPrice);
        return(bvtAmount,tokenAmount);
    }

    function checkPriceBatch(uint[] memory packIds) public view returns(uint[] memory,uint[] memory){
        uint[] memory bvtAmounts = new uint[](packIds.length);
        uint[] memory tokenAmounts = new uint[](packIds.length);
        for(uint i = 0; i < packIds.length; i ++){
            (bvtAmounts[i],tokenAmounts[i]) = checkPrice(packIds[i]);
        }
        return(bvtAmounts,tokenAmounts);
    }

    function exchangePack(uint packId, uint amount) external onlyEOA {
        require(packInfo[packId].types != 0, 'wrong packId');
