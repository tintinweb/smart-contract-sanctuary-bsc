...
import "./interfaces/IPrivatePresaleV2.sol";
import "./libraries/UQ112x112.sol";

/// @title Private Presale contract
contract PrivatePresaleV2 is IPrivatePresaleV2, UUPSUpgradeable, ReentrancyGuardUpgradeable, ERC165 {
    ...
    function initialize(InitializeParams calldata _initializeParams) initializer external {
        require(
            _initializeParams.registry != address(0) &&
            ...
            "PrivatePresaleV2: ZERO"
        );
        require(IERC165(_initializeParams.whitelist).supportsInterface(type(IWhitelist).interfaceId), "PrivatePresaleV2: ADDRESS_NOT_SUPPORTED");
        require(_initializeParams.endTimestamp > _initializeParams.startTimestamp, "PrivatePresaleV2: INVALID_TIME");
        ...
        updateStakingPower(_buyParams.idsToUpdate)
    {
        ...
        require(IAccessControl(registry).hasRole(PRIVATE_PRESALE_BUYER, msg.sender) || stakingPower >= minStakingPower, "PrivatePresaleV2: INVALID_STAKING_POWER");
    ...
    function setPriceTokenPerBuyTokenInUQ(uint _priceTokenPerBuyTokenInUQ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_priceTokenPerBuyTokenInUQ != 0, "PrivatePresaleV2: ZERO");
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return interfaceId == type(IERC165).interfaceId || interfaceId == type(IPrivatePresaleV2).interfaceId;
    }

    modifier onlyRole(bytes32 role) {
        require(IAccessControl(registry).hasRole(role, msg.sender), "PrivatePresaleV2: FORBIDDEN");
        _;
    }
    ...
    modifier requireKYC(bytes[] calldata _signatures, address[] calldata _signers) {
        ...
        require(IWhitelist(whitelist).isAddressWhitelisted(data, _signatures, _signers), "PrivatePresaleV2: USER_SHOULD_PASS_KYC");
        _;
    }
