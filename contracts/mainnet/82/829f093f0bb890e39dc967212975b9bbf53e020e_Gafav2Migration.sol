...
Dev Notes:
This contract will assist on the migration of a token, the user needs to deposit their v1 tokens and then claim their v2 tokens.
Once all tokens are collected, the liquidity will be migrated and a new liquidity pool created.
One The liquidity pool is created, the users can claim their v2 tokens and be able to continue without any problem.
This contract will be excluded from taxes on v1 and on v2 contract to ensure the user get the exact amount.
...
contract Gafav2Migration is Ownable, ReentrancyGuard {
    ...
    address public tokenV2Pair = 0xB9bECb6Bf6EeFfAe0194b4b833fb044099374B9D;
