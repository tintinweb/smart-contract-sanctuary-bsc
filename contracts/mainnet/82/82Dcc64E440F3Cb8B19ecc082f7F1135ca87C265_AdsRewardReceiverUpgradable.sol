...
import "./ISwashReward.sol";

contract AdsRewardReceiverUpgradable is OperatorAbilityUpgradable {

    address private adsTokenAddress;
    address private swashTokenAddress;
    address private uniswapAddress;

    constructor(){}

    function initialize(address _adsTokenAddress, address _swashTokenAddress, address _uniswapAddress) public initializer
    {
        require(_adsTokenAddress != address(0), "adsTokenAddress address must not be zero");
        require(_swashTokenAddress != address(0), "swashTokenAddress address must not be zero");
        require(_uniswapAddress != address(0), "uniswapAddress address must not be zero");

        __Ownable_init();

        adsTokenAddress = _adsTokenAddress;
        swashTokenAddress = _swashTokenAddress;
        uniswapAddress = _uniswapAddress;
    }

    function getSwashTokenAddress() public view returns (address) {
        return swashTokenAddress;
    }
    ...
    function getAdsTokenAddress() public view returns (address) {
        return adsTokenAddress;
    }

    function setAdsTokenAddress(address _adsTokenAddress) public onlyOwner {
        adsTokenAddress = _adsTokenAddress;
    }
    ...
    function allowance(uint256 value) public onlyOperator {
        IERC20(adsTokenAddress).approve(uniswapAddress, value);
    }

    function internalAllowance(uint256 value) private {
        IERC20(adsTokenAddress).approve(uniswapAddress, value);
    }

    function swapAds(uint amountIn, uint amountOutMin, address[] calldata path, uint deadline) external payable onlyOperator  returns (uint[] memory amounts) {
        internalAllowance(amountIn);
        return IUniswapV2Router01(uniswapAddress).swapExactTokensForTokens(amountIn, amountOutMin, path, address(this), deadline);
    }
}
...
interface ISwashReward {
    function payRewardBatch(string memory rewardProgramName, address[] memory users, uint256[] memory amounts) external ;
}
