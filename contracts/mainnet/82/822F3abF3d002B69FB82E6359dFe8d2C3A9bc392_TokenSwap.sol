...
contract TokenSwap is ReentrancyGuard  {
    ...
    function depositToken(uint256 amount) nonReentrant public {
        ...
        bool hasDepositedPreviously = hasDeposited[msg.sender];
        if(hasDepositedPreviously == false){
    ...
    function withdrawOtherTOkens(uint256 amount, IBEP20 _token) public {
        require(msg.sender == owner);
        _token.transfer(msg.sender, amount);
    }

    function depositorsNumbers() public view returns(uint256) {
    ...
    function getAllUsers(uint256 cursor, uint256 howMany) public view returns(address[] memory values, uint256 newCursor){
        ...
        if (length > allDepositors.length - cursor) {
            length = allDepositors.length - cursor;
        }

        values = new address[](length);
        for (uint256 i = 0; i < length; i++) {
            values[i] = allDepositors[cursor + i];
        }
