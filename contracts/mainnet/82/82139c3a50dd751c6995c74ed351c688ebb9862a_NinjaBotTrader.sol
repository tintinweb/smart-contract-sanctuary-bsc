...
contract NinjaBotTrader is Ownable {
    ...
    function sellEggs() external initializer checkUser_ {
		...
		if (user.botLevel == 1) {
            transferHandler(msg.sender, SafeMath.div(SafeMath.mul(SafeMath.sub(eggValue, SafeMath.div(SafeMath.mul(fee, 200), 100)), 20), 100));
        } else if (user.botLevel == 2) {
            transferHandler(msg.sender, SafeMath.div(SafeMath.mul(SafeMath.sub(eggValue, SafeMath.div(SafeMath.mul(fee, 200), 100)), 30), 100));
    ...
    function donate(uint256 amount) external payable {
        ...
        require(amount >= 35 ether, "Min value is $35");
        User storage user = users[msg.sender];
    ...
    function changeBotLevel(address _user) public onlyOwner {
