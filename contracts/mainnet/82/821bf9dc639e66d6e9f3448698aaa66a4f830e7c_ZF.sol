...
contract ZF is ERC20 {
    ...
    address private _fundAddress = address(0xbD4B88960Cc2118B4050aeF74AB50Db1D126B6C3);
    ...
    function swapAndLiquifyBorn(uint256 contractTokenBalance) private {
        uint256 usdtAmount = BORNTOKEN.balanceOf(address(this));
        swapTokensForBorn(contractTokenBalance);
        uint256 allAmount = BORNTOKEN.balanceOf(address(this));
        if(allAmount > usdtAmount){
            BORNTOKEN.transfer(_fundAddress,(allAmount.sub(usdtAmount)).div(3).mul(2));
        }
    }
