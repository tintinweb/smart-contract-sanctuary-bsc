...
contract WastedFusion is AccessControl, EIP712, TokenWithdrawable {
    using SafeERC20 for IERC20;

    event FusionWarrior(
        uint256 originalWarrior,
        uint256 secondWarrior,
        uint256 totalPrice,
        uint256[] lockParts
    );

    IWastedWarrior public wastedWarrior =
        IWastedWarrior(0xdeE71419bC45c11D28F9106cbb4923c7038Ed594);
    IERC20 public acceptedToken =
        IERC20(0xd306c124282880858a634E7396383aE58d37c79c);
    ...
    address public receiver = 0x40cfBcFfA02B1Cae039921a604dbb5566520C03f;
    bytes32 public constant SERVER_ROLE = keccak256("SERVER_ROLE");
    bytes32 public constant CONTROLLER_ROLE = keccak256("CONTROLLER_ROLE");
    string private constant SIGNING_DOMAIN = "LazyFusion-WastedWarrior";
    string private constant SIGNATURE_VERSION = "1";

    constructor() EIP712(SIGNING_DOMAIN, SIGNATURE_VERSION) {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(CONTROLLER_ROLE, msg.sender);
        _setupRole(SERVER_ROLE, 0x000000F4Ec39bB994066B06F5eDAd09602aD3D28);
    }

    function setAcceptedToken(IERC20 _acceptedToken)
        external
    ...
    function setWastedWarriorContract(IWastedWarrior _warriorContract)
        external
        onlyRole(CONTROLLER_ROLE)
    {
        wastedWarrior = _warriorContract;
    }
    ...
    function fusionWarrior(Fusion calldata fusionInfo, bytes calldata signature)
        external
    {
        uint256[] memory warriorIds = fusionInfo.warriorIds;
        uint256 price = fusionInfo.price;

        address signer = _verify(fusionInfo, signature);
        ...
        require(warriorIds.length == 2);
        acceptedToken.safeTransferFrom(msg.sender, receiver, price);

        wastedWarrior.fusionWarrior(
            warriorIds[0],
            warriorIds[1],
            msg.sender,
            fusionInfo.lockParts
        );
    ...
    function _verify(Fusion calldata fusionInfo, bytes memory signature)
        internal
        view
        returns (address)
    {
        bytes32 digest = _hash(fusionInfo);
        return ECDSA.recover(digest, signature);
    }

    function _hash(Fusion calldata fusionInfo) internal view returns (bytes32) {
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "Fusion(uint256 price,uint256[] warriorIds,uint256[] lockParts)"
                        ),
                        fusionInfo.price,
                        keccak256(abi.encodePacked(fusionInfo.warriorIds)),
                        keccak256(abi.encodePacked(fusionInfo.lockParts))
                    )
                )
            );
    }
