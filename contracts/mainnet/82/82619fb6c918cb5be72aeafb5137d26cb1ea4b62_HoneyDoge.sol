...
contract HoneyDoge is ERC20Detailed, Ownable, WhitelistedRole {
    ...
    uint256 public rewardYield = 525842567;
    uint256 public rewardYieldDenominator = 1000000000000;
    uint256 public maxSellTransactionAmount = 2500 * 10 ** 5;
    ...
    address public marketingReceiver = 0xd5b9078aB266473B55E7C7dE2e66f435094aBB4A;
    address public honeycombReceiver = 0x91518C8dB6c08dEB17012cFda54E5934E496d7cb;
    ...
    uint256 public buyFeeHCP = 2;
    uint256 public sellFeeMarketingAdded = 1;
    uint256 public sellFeeHCPAdded = 1;
    uint256 public totalBuyFee = liquidityFee.add(marketingFee).add(buyFeeHCP);
    uint256 public totalSellFee = totalBuyFee.add(sellFeeMarketingAdded).add(sellFeeHCPAdded);
    uint256 public feeDenominator = 100;

    uint256 public transferTax = 100;
    uint256 public dbFee = 99;

    uint256 targetLiquidity = 50;
    uint256 targetLiquidityDenominator = 100;

    bool inSwap;
    uint256 public txfee = 2;
    ...
    constructor() ERC20Detailed("HoneyDoge", "HDOGE", uint8(DECIMALS)) {
        ...
        _isFeeExempt[honeycombReceiver] = true;
    ...
    function checkFragments() external view returns (uint256) {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (
            ...
            uint256 twoPercent = balanceOf(sender).mul(txfee).div(100); //Should use variable
            ...
            if( blkTime > tradeData[sender].lastTradeTime + TwentyFourhours) {
                ...
                require(tradeData[sender].tradeAmount + amount <= twoPercent, "ERR: Can't sell more than 2% in One day");
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountToHCP = contractTokenBalance.mul(buyFeeHCP.mul(2).add(sellFeeHCPAdded)).div(realTotalFee);
        uint256 amountToMarketing = contractTokenBalance.sub(amountToLiquify).sub(amountToHCP);
        ...
        if(amountToHCP > 0){
            _swapTokensForBusd(amountToHCP, honeycombReceiver);
        }

        if(amountToMarketing > 0){
            _swapTokensForBusd(amountToMarketing, marketingReceiver);
        }

        emit SwapBack(contractTokenBalance, amountToLiquify, amountToHCP, amountToMarketing);
    }

    function takeFee(address sender, address recipient, uint256 gonAmount) internal returns (uint256){
        uint256 _realFee = totalBuyFee;

        if((launchedAt + db) >= block.number){
            _realFee = dbFee;
        }
    ...
    function setFeeReceivers(address _liquidityReceiver, address _marketingReceiver, address _honeycombReceiver) external onlyOwner {
        liquidityReceiver = _liquidityReceiver;
        marketingReceiver = _marketingReceiver;
        honeycombReceiver = _honeycombReceiver;
    }

    function setFees(uint256 _liquidityFee, uint256 _honeycomb, uint256 _marketingFee, uint256 _sellFeeMarketingAdded, uint256 _sellFeeHCPAdded, uint256 _feeDenominator) external onlyOwner {
        require(
            _liquidityFee <= MAX_FEE_RATE &&
            _honeycomb <= MAX_FEE_RATE &&
            _marketingFee <= MAX_FEE_RATE &&
            _sellFeeMarketingAdded <= MAX_FEE_RATE &&
            _sellFeeHCPAdded <= MAX_FEE_RATE,
            "wrong"
        );

        liquidityFee = _liquidityFee;
        buyFeeHCP = _honeycomb;
        marketingFee = _marketingFee;
        sellFeeMarketingAdded = _sellFeeMarketingAdded;
        sellFeeHCPAdded = _sellFeeHCPAdded;
        totalBuyFee = liquidityFee.add(marketingFee).add(buyFeeHCP);
        totalSellFee = totalBuyFee.add(sellFeeMarketingAdded).add(sellFeeHCPAdded);
    ...
    event SwapBack(uint256 contractTokenBalance,uint256 amountToLiquify,uint256 amountToHCP,uint256 amountToMarketing);
