...
contract CRB is Context, Ownable, IBEP20, BEP20Detailed {
  ...
  using SafeMath for uint256;
    ...
    address payable public TAddress = payable(owner());
    ...
    constructor () BEP20Detailed("CryptoBill", "CRB", 8) {
      _owner = msg.sender ;
    ...
    function setTFeePercent(uint256 updatedTFee) external onlyOwner {
        transactionFee = updatedTFee;
    }

    function setTAddress(address payable wallet) external onlyOwner
    {
        TAddress = wallet;
    }
    ...
    function _transfer(address sender, address recipient, uint amount) internal{
        ...
        uint256 taxAmount = (amount.mul(transactionFee)).div(100);
    ...
    function totalFee() internal view returns(uint256)
    {
     ...
     function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        payable(TAddress).transfer(newBalance);
