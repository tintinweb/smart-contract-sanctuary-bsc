...
contract NFT is ERC721, Ownable {
    ...
    mapping(address => bool) public _canDoPrivateMint;
    bool public _openPublicMint = false;
    uint256 public totalSupplyLimit = 2100;
    ...
    constructor() ERC721("OptimusNFTs", "OPTT") {
        _contractOwner = msg.sender;
    }

    function setCanDoPrivateMint(address account, bool state) public onlyOwner {
        _canDoPrivateMint[account] = state;
    }

    function setOpenPublicMint(bool state) public onlyOwner {
        _openPublicMint = state;
    }

    function setTotalSupplyLimit(uint256 newTotalSupplyLimit) public onlyOwner {
    ...
    function mint(string memory _tokenURI, address _toAddress, uint _price) public returns (uint) {
        if(!_openPublicMint) {
            require(_canDoPrivateMint[msg.sender], "");
        }

        uint _tokenId = totalSupply() + 1;
        require(_tokenId <= totalSupplyLimit, "nft count is limited within totalSupplyLimit");
