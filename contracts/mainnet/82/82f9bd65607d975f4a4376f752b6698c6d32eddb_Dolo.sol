...
contract Dolo is Context, IERC20, Ownable {
    ...
    //3% marketing
    //2% burn
    address public marketingAddress = 0x4DB8302E51C9EE1BA3c25802360Ddd18fAcEa99F; // Account 4 mozilla
    address public nftAddress = 0x2707C91Fd67DeDed6Be623e7b68b682106670b8F;  // Account 3 mozilla
	address public burnAddress = 0xAf207724D5068f586431Af54a56ceaF6e9F855c3; // // Account 2 mozilla
    address public constant deadAddress = 0x000000000000000000000000000000000000dEaD;

    string private constant _name = "DOLOX";
    string private constant _symbol = "DOLOX";
   ...
   function setMaxWalletPercent(uint256 maxWallPercent) external onlyOwner() {
    ...
    function setFeeRates(uint256 _rfi, uint256 _nft, uint256 _marketing, uint256 _liquidity, uint256 _burn) external onlyOwner {
        feeRates.rfi = _rfi;
        feeRates.nft = _nft;
    ...
    function setSellFeeRates(uint256 _rfi, uint256 _nft, uint256 _marketing, uint256 _liquidity, uint256 _burn) external onlyOwner{
	...
	// Burn
    function _takeBurning(uint256 rBurn, uint256 tBurn) private {
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi,to_return.rNFT, to_return.rMarketing, to_return.rLiquidity, to_return.rBurn) = _getRValues(to_return, tAmount, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory s) {
        ...
        if(isSale){
            ...
            s.tNFT = tAmount*sellFeeRates.nft/1000; // NFT 
            s.tMarketing = tAmount*sellFeeRates.marketing/1000;
            s.tLiquidity = tAmount*sellFeeRates.liquidity/1000;
            s.tBurn = tAmount*sellFeeRates.burn/1000; // Burn 
            s.tTransferAmount = tAmount-s.tRfi-s.tNFT-s.tMarketing-s.tLiquidity-s.tBurn;
        }
        else{
    ...
    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi, uint256 rNFT,uint256 rMarketing, uint256 rLiquidity, uint256 rBurn) {
        ...
        rTransferAmount =  rAmount-rRfi-rNFT-rMarketing-rLiquidity-rBurn;
        return (rAmount, rTransferAmount, rRfi,rNFT,rMarketing,rLiquidity, rBurn);
    }
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap{
        ...
        // uint256 denominator = (feeRates.liquidity + feeRates.burn + feeRates.nft + feeRates.marketing)* 2;
        ...
        // Send BNB to nftWallet
        uint256 nftAmt = unitBalance * 2 * feeRates.nft;
        if(nftAmt > 0){
          payable(nftAddress).transfer(nftAmt);
        }
    }
