...
contract VRT is ERC20, Ownable {
    ...
    uint256 public maxBuyTxAmount     =  10**9 * 10**18;
    uint256 public swapTokensAtAmount =  10**9 * 10**18;
    uint256 public buyBackTime = 3600;
    ...
    function _transfer(
        ...
        } else if (block.timestamp - startTime < 600 && from == PancakeswapV2Pair) {
        ...
        if (takeFee){
            ...
            if (from == PancakeswapV2Pair) {
                ...
                if (!_isExcluded[to]){
                 ...
                 if (!_isExcluded[from]){
                   require((_lastBuyTime[from] == 0)||(_lastBuyTime[from] + buyBackTime < block.timestamp), "bot detected!");
                 }
    ...
    function setBuyBackTime (uint256 _time) external onlyOwner {
        buyBackTime = _time;
    }
}