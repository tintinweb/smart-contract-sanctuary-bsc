...
contract SmurfsInu is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    CatuXDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Smurfs Inu", "SMURFS") {
        marketingWallet = payable(0x117D66b286e84520d3a0Fad42Be492A5Ed1e5BF1); 
        ...
        dividendTracker = new CatuXDividendTracker(
            payable(this),
            router,
...
contract CatuXDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    SmurfsInu public CatuXContract;
    ...
    constructor(
        ...
        defaultToken = token;
        CatuXContract = SmurfsInu(mainContract);
    ...
    function _transfer(
        ...
        require(false, "CatuX_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "CatuX_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CatuX contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(CatuXContract);

            uint256 prevBalance = CatuXContract.balanceOf(address(this));
            ...
            {
                uint256 received = CatuXContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    CatuXContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
