...
// File: contracts/NitroPool.sol


// Nitro Pool V2 by XTO
// https://xoxobusd.com/dapp/nitropool.html
...
interface iNitroPool {
    function User(address _user) external view returns (
        uint id,
        address invitedBy,
        uint referrals,
        uint slots,
...
contract NitroPoolV2 is Ownable {

    address public constant NitroPoolV1 = address(0x210452D6D46bE37D1F2AeC10e85894B923271bf9);
    iNitroPool NitroContract = iNitroPool(NitroPoolV1);
    ...
    /* CONSTANTS */
    uint256 public slotPrice = 25 * 10 ** 18;
    uint256 public buyAmount = 15 * 10 ** 18;
    uint256 public poolSupportAmount = 1 * 10 ** 18;
    uint256 public sponsorEarnAmount = 4 * 10 ** 18;
    uint256 public devSupportAmount = (1 * 10 ** 18) / 2;
    uint256 public buyTokenAmount = 4 * 10 ** 18;
    ...
    address public buyTokenWallet = address(0x98A3a3ccB73eA5fcD9D35fd2982b8C97EF33bFD1);
    ...
    event SlotPaid(uint slotId, address user, uint amount);
    ...
    constructor() {    
        ...
        XTO.approve(address(uniswapV2Router), MAX_INT);
        XTO.approve(address(this), MAX_INT);

        BUSD = IERC20(busdAddress);
        BUSD.approve(address(uniswapV2Router), MAX_INT);
    ...
    function Register(address sponsorWallet) public onlyEOA {
        ...
        require(NitroContract.addressToId(wallet) == 0, "Already Registered");
        ...
        require(u.id == 0, "Already registered");
    ...
    function BuySlot() public {
        ...
        uint XTO_balanceBefore = XTO.balanceOf(address(this));
        uint _slotToPay = totalSlots;

        // ADD SLOT 
        totalSlots++;
        Slots storage s = Slot[_slotToPay];
        Slots storage newSlot = Slot[totalSlots];
        newSlot.user = _buyer;
        ...
        if ((NitroContract.addressToId(_buyer) > 0) && (u.id == 0)) {
            (uint256 _v1uid, address _v1sponsor, uint256 _v1refcount,,,) = NitroContract.User(_buyer);
            u.id =_v1uid;
            u.invitedBy =_v1sponsor;
            u.referrals = _v1refcount;
            idToAddress[_v1uid] = _buyer;
            addressToId[_buyer] = _v1uid;
        }
        ...
        // UPDATE PAID USER 
        Users storage uPaid = User[s.user];

        // UPDATE SPONSOR
        address _sponsor = u.invitedBy;
        Users storage sponsor = User[_sponsor];
        if ((NitroContract.addressToId(_sponsor) > 0) && (sponsor.id == 0)) {
            (uint256 _v1uid, address _v1sponsor, uint256 _v1refcount,,,) = NitroContract.User(_sponsor);
            sponsor.id =_v1uid;
            sponsor.invitedBy =_v1sponsor;
            sponsor.referrals = _v1refcount;
            idToAddress[_v1uid] = _sponsor;
            addressToId[_sponsor] = _v1uid;
        }
        sponsor.refPaid += sponsorEarnAmount;

        require((BUSD.balanceOf(_buyer) >= slotPrice), "Insufficient balance");
        uint256 allowance = BUSD.allowance(_buyer, address(this));
        require(allowance >= slotPrice, "Not enough allowance");
        BUSD.safeTransferFrom(_buyer, address(this), slotPrice);

        // DISTRIBUTE PROFITS
        BUSD.safeTransferFrom(address(this), _sponsor, sponsorEarnAmount);
        BUSD.safeTransferFrom(address(this), poolSupportWallet, poolSupportAmount);
        BUSD.safeTransferFrom(address(this), dev1SupportWallet, devSupportAmount);
        BUSD.safeTransferFrom(address(this), dev0SupportWallet, devSupportAmount);
        BUSD.safeTransferFrom(address(this), buyTokenWallet, buyTokenAmount);
        ...
        uint XTO_balanceAfter = XTO.balanceOf(address(this));
        totalBought += XTO_balanceAfter - XTO_balanceBefore;

        uint sendBalance = XTO_balanceAfter - XTO_balanceBefore;

        require(XTO.balanceOf(address(this)) >= sendBalance, "Not Enough XTO");
        s.amount = sendBalance;
        XTO.safeTransferFrom(address(this), s.user, sendBalance);

        totalDeposited += slotPrice;
        totalPaid += sendBalance;
        uPaid.paid += sendBalance;

        emit SlotPaid(_slotToPay, s.user, sendBalance);
