...
abstract contract AutoCompoundStrategy is Ownable, ReentrancyGuard, Pausable {
    ...
    IRouteOracle public routeOracle;
    ...
    function initialize(
        ...
        liquidityRouter = IUniswapV2Router02(_addresses[4]);
        WNATIVE = _addresses[5];
        routeOracle = IRouteOracle(_addresses[6]);
        pid = _pid;
        if (_isLpToken) {
    ...
    function deposit(uint256 _depositAmount) external virtual onlyOwner nonReentrant whenNotPaused returns (uint256) {
        ...
        if (totalStakedBefore > 0 && _sharesTotal > 0) {
            sharesAdded = (_depositAmount * _sharesTotal) / totalStakedBefore;
        }
        sharesTotal = _sharesTotal + sharesAdded;
    ...
    function earn(address _bountyHunter) external virtual onlyOwner returns (uint256 bountyReward) {
        ...
        if (address(token0) == address(0)) {
            if (stakeToken != earnToken) {
                _safeSwap(earnAmount, address(earnToken), address(stakeToken));
            }
            _farm();
            return bountyReward;
        }
        ...
        uint256 halfEarnAmount = earnAmount / 2;
        if (earnToken != token0) {
     ...
     */
    function tokenToEarn(address _token) external virtual nonReentrant whenNotPaused {
        uint256 amount = IERC20(_token).balanceOf(address(this));
        require(_token != address(earnToken) && _token != address(stakeToken), 'tokenToEarn: NOT_ALLOWED');
        if (amount > 0) {
        ...
        returns (uint256 bountyReward, uint256 platformFee)
    {
        ...
        platformFee = performanceFee - bountyReward;
