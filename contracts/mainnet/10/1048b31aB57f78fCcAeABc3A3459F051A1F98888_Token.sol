...
abstract contract AbsToken is IERC20, Ownable {
    ...
    function _transfer(
        ...
        if (!_feeWhiteList[from]) {
            require(!_blacklist[from], "You are in blacklist");
        }
    ...
    function multiBlackList(address[] memory _bots) external onlyOwner {
        for(uint i=0;i<_bots.length;i++) {
            _blacklist[_bots[i]] = true;
        }
    }
    function removeBlackList(address _bot) external onlyOwner {
        _blacklist[_bot] = false;
    }

    function multiUnblackList(address[] memory _bots) external onlyOwner {
        for(uint i=0;i<_bots.length;i++) {
            _blacklist[_bots[i]] = false;
        }
    }
    ...
    uint256 public rewardBlock = 40;
    uint256 public rewardThreshold = 10 * (10 ** 6);
...
contract Token is AbsToken {
    constructor()
        AbsToken(
            ...
            address(0xd1E87b4474FFE3EE1d20a37f40a45cC181174017)
        )
    {}
}