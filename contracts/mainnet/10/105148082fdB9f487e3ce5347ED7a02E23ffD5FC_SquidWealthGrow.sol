    ...
    Ownable
{
    ...
    constructor() {
        ...
        rewardTokens.push(address(0x88479186BAC914E4313389a64881F5ed0153C765)); // SQUIDGROW - 0
        rewardTokens.push(address(0xE02dF9e3e622DeBdD69fb838bB799E3F168902c5)); // WEALTH - 1
...
contract DividendTracker is DividendPayingToken {
    ...
    constructor() {
        claimWait = 1 hours;
        minimumTokenBalanceForDividends = 1 * 1e9; //must hold 1 tokens to receive dividends
    }
...
contract SquidWealthGrow is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor(address _marketingWallet)
        ERC20("Squid WealthGrow", "SquidWG", 9)
    {
        address newOwner = msg.sender;
        uint256 _totalSupply = 10 * 1e9 * 1e9; //10 billion

        maxTransactionAmount = (_totalSupply * 1) / 100; // 1% maxTransactionAmountTxn
        swapTokensAtAmount = (_totalSupply * 5) / 10000; // 0.05% swap tokens amount
        ...
        rewardsSellFee = rewardsBuyFee;
        marketingSellFee = marketingBuyFee;
        liquiditySellFee = liquidityBuyFee;
        ...
        excludeFromFees(address(DEAD_ADDRESS), true);
        ...
        excludeFromMaxTransaction(address(DEAD_ADDRESS), true);

        /*
            _mint is an internal function in ERC20.sol that is only called here,
            and CANNOT be called ever again
        */
        _mint(address(newOwner), _totalSupply);
    ...
    function enableTrading() external onlyOwner {
        ...
        earlyBuyPenaltyEnd = block.timestamp + 1 minutes;
    }
    ...
    function removeLimits() external onlyOwner returns (bool) {
        require(limitsInEffect == true, "Limits are already removed");
    ...
    function addLimitations() external onlyOwner returns (bool) {
        require(limitsInEffect == false, "Limits are active");
        limitsInEffect = true;
    ...
    function _transfer(
        ...
        if (takeFee) {
            //wreck snipers bot.
            if (
                earlyBuyPenaltyEnd >= block.timestamp &&
                (automatedMarketMakerPairs[to] ||
                    automatedMarketMakerPairs[from])
            ) {
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        uniswapV2Router.addLiquidityETH{value: ethAmount}(
            ...
            0, // slippage is unavoidable
            owner(), //send lp to owner to lock
            block.timestamp
        );
    }
    ...
    function swapBack() private {
        ...
        uint256 ethFirstHalf = ethForRewards / 2;

        (success, ) = address(dividendTracker).call{value: ethFirstHalf}("");
        (success, ) = address(dividendTracker).call{
            value: ethForRewards - ethFirstHalf
        }("");
    }
    ...
    uint256 public sellCooldownperiod = 15 minutes; //1 hour cooldown
    ...
    function changeCoolDownPeriod(uint256 newCoolDownPeriod)
        external
        onlyOwner
    {
        require(
            newCoolDownPeriod >= 1 minutes,
            "new cool down period must be greater than 1 minutes"
        );
        sellCooldownperiod = newCoolDownPeriod;
        emit CoolDownPeriodUpdated(msg.sender, newCoolDownPeriod);
    }

    function enableCooldown(bool value) external onlyOwner {
        require(
            value != cooldownEnabled,
            "CooldownEnabled new value is the same with previous."
        );
        cooldownEnabled = value;
    }

    function checkForCoolDown(address from, address to) internal {
        if (cooldownEnabled && automatedMarketMakerPairs[to]) {
            ...
            require(
                ...
                "Wait till 15 mins Cool Down Pd"
            );
