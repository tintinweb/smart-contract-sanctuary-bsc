...
abstract contract AbsToken is IERC20, Ownable {
    ...
    uint256 public _limitDuration = 24 hours;
    ...
    constructor (
        ...
        _limitAmount = LimitAmount * usdtUnit;
    ...
    function _transfer(
        ...
        if (!_swapPairList[to] && !_feeWhiteList[to]) {
            ...
            uint startTime = _startTradeTime;
            if (limitAmount > 0 && startTime > 0 && block.timestamp < startTime + _limitDuration) {
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            if (removeLP) {
                ...
                if (feeAmount > 0) {
                    _takeTransfer(sender, address(_tokenDistributor), feeAmount);
                }
                ...
                uint256 fundAmount = tAmount * (_buyFundFee + _buyLPDividendFee) / 10000;
                if (fundAmount > 0) {
                ...
                uint256 fundAmount = tAmount * (_sellFundFee + _sellLPDividendFee) / 10000;
                if (fundAmount > 0) {
                    feeAmount += fundAmount;
                    _takeTransfer(sender, address(this), fundAmount);
                }
    ...
    function setBuyFee(
        uint256 buyDestroyFee, uint256 buyFundFee, uint256 lpDividendFee
    ) external onlyOwner {
    ...
    function setSellFee(
        uint256 sellDestroyFee, uint256 sellFundFee, uint256 lpDividendFee
    ) external onlyOwner {
...
contract AGL is AbsToken {
    ...
    //USDT
        ...
        31000,
    //Fund
        address(0x0B14f784A8eedD9877690c88eBc33BE81733DFeE),
    //Received
        address(0xCc62A18DddA8D14428882F5e77F7Af7964ae90d2),
    //limitAmount
        500,
    //MinTotal
        9999
