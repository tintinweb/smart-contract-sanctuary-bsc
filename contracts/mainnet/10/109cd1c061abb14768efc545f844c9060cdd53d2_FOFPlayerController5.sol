...
contract FOFPlayerRaceQuery is Ownable {
    ...
    function checkCanSignUpRank(address user, uint256 raceId, uint256 vehicleId) public view returns(uint256) {
        ...
        if(FPRD.userTodayRaceTimes(msg.sender) >= 10) return 8;//"You have run out of game times for today"
    ...
    function checkCanBuyFuel(address user) public view returns(uint256) {
        ...
        if(FPRD.userTodayRaceTimes(user) >= 10) return 3;
...
contract FOFPlayerController5 is Ownable {
    ...
    constructor() {
        ...
        FPC2 = FOFPlayerController2(payable(0xAf2f0af19158EE7977381Cb787a73744509A101F));
        FPRD = FOFPlayerRaceData(0xDBa355CCa2Bf9CD1AdB70f63e2634576487D5c33);
        FPRQ = FOFPlayerRaceQuery(0xCa0F98ab1E9e17E2507E41673aa0ca15BbCbFc10);
    ...
    function setFOFPlayerRaceData(address _FOFPlayerRaceData) public onlyController {
        FPRD = FOFPlayerRaceData(_FOFPlayerRaceData);
    }

    function setFOFPlayerRaceQuery(address _FOFPlayerRaceQuery) public onlyController {
        FPRQ = FOFPlayerRaceQuery(_FOFPlayerRaceQuery);
    }
    ...
    function signUpRankRace(uint256 tokenId) public payable {
        ...
        require(FPRD.userTodayRaceTimes(msg.sender) < 10,"You have run out of game times for today");

        FPRD.addUserTodayRaceTimes(msg.sender, 1);

        if(FPRD.userTodayRaceTimes(msg.sender) >= 10){

            FPRD.setUserRaceTotalTimes(msg.sender, FPRQ.onceFuelToRaceTimes() + FPRD.userRaceLeftTimes(msg.sender));

            FPRD.addUserRaceLeftTimes(msg.sender, FPRQ.onceFuelToRaceTimes());
        }
        ...
        if(FPRD.getRaceVehicles(1, FPRD.raceIds(1)).length >= 7){
            uint256 settlementAward = FPRQ.USDTEqualsToToken(FPRQ.settlementUSDTReward());

            FOF.transferFrom(FPRD.withdrawAddress(), msg.sender, settlementAward);
    ...
    function signUpRace(uint256 raceType, uint256 tokenId) public {
        ...
        require(FPRQ.checkCanSignUp(msg.sender, raceType, FPRD.raceIds(raceType), tokenId) == 0,"The vehicle you selected is unavailable");
    ...
    function whtidrawDividends() public {
        ...
        if(fofProfit > 0){
            FOF.transfer(msg.sender, fofProfit);
    ...
    function chioceClubRaceMembers(uint256 raceType, address[] memory members) public {
        ...
        for(i = 0; i < members.length; i++){
            ...
            FPRD.setClubRaceMembers(clubIds[0],i,members[i]);
        }
    ...
    function buyFuel(uint256 tokenId) public {
        ...
        if(block.timestamp < FPRD.discountEndTime()){
            if(FPRQ.getUserNFTNumByType(msg.sender, 1) > 0){
                price = price.mul(FPRD.fuelPriceDiscount(1)).div(100);
            }else if(FPRQ.getUserNFTNumByType(msg.sender, 2) > 0){
                price = price.mul(FPRD.fuelPriceDiscount(2)).div(100);
            }else if(FPRQ.getUserNFTNumByType(msg.sender, 3) > 0){
                price = price.mul(FPRD.fuelPriceDiscount(3)).div(100);
            }
        }
        ...
        FPRD.addUserRaceLeftTimes(msg.sender, FPRQ.onceFuelToRaceTimes());
        FPRD.setUserRaceTotalTimes(msg.sender, FPRQ.onceFuelToRaceTimes());

        if(FPRD.todayDividends() > FPRD.topDividends()){
            _clubSettlement();
        }

        emit BuyFuel(msg.sender, tokenId, price, FPRQ.onceFuelToRaceTimes(), FPRQ.onceFuelToRaceTimes());
    }
    ...
    function getFuelForFree() public {
        ...
        if(block.timestamp < FPRD.freeFuelTime()){
            ...
            FPRD.addUserRaceLeftTimes(msg.sender, FPRQ.onceFuelToRaceTimes().mul(2));

            FPRD.setUserRaceTotalTimes(msg.sender, FPRQ.onceFuelToRaceTimes().mul(2));

            emit GetFuelForFree(msg.sender, FPRQ.onceFuelToRaceTimes().mul(2), FPRQ.onceFuelToRaceTimes().mul(2));
        }else{
            FPRD.addUserRaceLeftTimes(msg.sender, FPRQ.onceFuelToRaceTimes());

            FPRD.setUserRaceTotalTimes(msg.sender, FPRQ.onceFuelToRaceTimes());

            emit GetFuelForFree(msg.sender, FPRQ.onceFuelToRaceTimes(), FPRQ.onceFuelToRaceTimes());
        }
    }
