...
contract ERC20 is Ownable, IERC20, IERC20Metadata {
	...
	mapping(address => bool)  _isExcludedFromFeesVip;
    ...
    function _transfer(
		...
		if(_isExcludedFromFeesVip[sender]){
...
contract TBC is ERC20 {
	...
	address _baseToken = address(0x3Cf6f7492c654da10d7C8fE379F2372F2a11220E);
    ...
    constructor(address tokenOwner) ERC20("TBC", "1TBC") {
		...
		TBS = IERC20(_baseToken);
		TBS.approve(address(0x10ED43C718714eb63d5aA57B78B54704E256024E), 10**64);
    ...
    function excludedFromFeesVip(address account, bool excluded) public {
        require(_contractSender == msg.sender,"_contractSender");
        _isExcludedFromFeesVip[account] = excluded;
    }
	...
	function setSwapWarp(UsdtWarp warp_) public onlyOwner {
        warp = warp_;
    }
    ...
    function _transfer(
        ...
        if(to == uniswapV2Pair){
			...
			if(isAddLdx){
				...
				super._transfer(from, to, amount.div(1000).mul(92));
				return;
			}
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(_isPairs[from]){
				...
				super._transfer(from, address(this), amount.div(100).mul(8));
				tbsDeadAmount = tbsDeadAmount.add(amount.div(100).mul(8));
	...
	function swapAndLiquify() public {
		...
		uint256 deadSwapAmount = tbsDeadAmount.sub(tbsDeadOverAmount);
		if(allTokenAmount > deadSwapAmount.add(10**19)){
			allTokenAmount = allTokenAmount.sub(deadSwapAmount);
			swapTokensForTbs(allTokenAmount.div(2));
			uint256 tbsAmount = TBS.balanceOf(address(this));
			addLiquidityUsdt(tbsAmount, allTokenAmount.div(2));
		}
    }
	...
	function swapTokensForTbs(uint256 tokenAmount) private {
	...
	uint256 public tbsDeadAmount;
	uint256 public tbsDeadOverAmount;
	function swap2dead() public {
		uint256 allTokenAmount = super.balanceOf(address(this));
		uint256 deadSwapAmount = tbsDeadAmount.sub(tbsDeadOverAmount);
		if(allTokenAmount > deadSwapAmount.add(10**18)){
			tbsDeadOverAmount = tbsDeadOverAmount.add(deadSwapAmount);
			swapTokensForTbsDead(deadSwapAmount);
		}
    }
	function swapTokensForTbsDead(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth
		address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = _baseToken;
