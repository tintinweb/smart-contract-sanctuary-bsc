...
/**
 *Submitted by Vaultswap at BscScan.com on 2022-07-15
*/
...
// File: contracts/interfaces/IVaultSwapLottery.sol

pragma solidity ^0.8.4;

interface IVaultSwapLottery {
    /**
    ...
    /**
     * @notice Draw the final number, calculate reward in VAULT per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in VAULT token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInVAULT: price of a ticket in VAULT
...
// File: contracts/VaultSwapLottery.sol

pragma solidity ^0.8.4;
pragma abicoder v2;

/** @title VaultSwap Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract VaultSwapLottery is ReentrancyGuard, IVaultSwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInVAULT = 50 ether;
    uint256 public minPriceTicketInVAULT = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInVAULT;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] VAULTPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInVAULT;
        uint32 finalNumber;
    }
     ...
     * @param _VAULTTokenAddress: address of the VAULT token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _VAULTTokenAddress, address _randomGeneratorAddress) {
        ...
        nonReentrant
    {
        ...
        // Calculate number of VAULT to this contract
        uint256 amountVAULTToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInVAULT,
            _ticketNumbers.length
        );

        // Transfer vault tokens to this contract
        VAULTToken.safeTransferFrom(address(msg.sender), address(this), amountVAULTToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInVAULT += amountVAULTToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInVAULTToTransfer
        uint256 rewardInVAULTToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInVAULTToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        VAULTToken.safeTransfer(msg.sender, rewardInVAULTToTransfer);

        emit TicketsClaim(msg.sender, rewardInVAULTToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInVAULT) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in VAULT for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].VAULTPerBracket[j] =
                ...
                // A. No VAULT to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].VAULTPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInVAULT - amountToShareToWinners);

        // Transfer VAULT to treasury address
        VAULTToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        VAULTToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInVAULT += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInVAULT >= minPriceTicketInVAULT) && (_priceTicketInVAULT <= maxPriceTicketInVAULT),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInVAULT: _priceTicketInVAULT,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            VAULTPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInVAULT: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(VAULTToken), "Cannot be VAULT token");
    ...
    /**
     * @notice Set VAULT price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInVAULT: minimum price of a ticket in VAULT
     * @param _maxPriceTicketInVAULT: maximum price of a ticket in VAULT
     */
    function setMinAndMaxTicketPriceInVAULT(uint256 _minPriceTicketInVAULT, uint256 _maxPriceTicketInVAULT)
        external
        onlyOwner
    {
        require(_minPriceTicketInVAULT <= _maxPriceTicketInVAULT, "minPrice must be < maxPrice");

        minPriceTicketInVAULT = _minPriceTicketInVAULT;
        maxPriceTicketInVAULT = _maxPriceTicketInVAULT;
    }
     ...
     * @param _priceTicket price of a ticket (in VAULT)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].VAULTPerBracket[_bracket];
        } else {
            return 0;
        }
    }
