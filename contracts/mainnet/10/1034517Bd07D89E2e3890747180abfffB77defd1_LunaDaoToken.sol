...
contract LunaDaoToken is ERC20 {
    ...
    address public receiveBackLPAddress;
    mapping(address => bool) private _isExcludedFromFees;
    bool public swapAndLiquifyEnabled = true;
    uint256 public startTime;

    address constant private _destroyAddress = address(0x000000000000000000000000000000000000dEaD);

    uint256 public waitLPHolderTokenNum;  // LP持有者待分红Token数
    address public lpHolderDivideAddress;
    ...
    constructor(address tokenOwner) ERC20("LunaDao", "LunaDao", 18) {
        ...
        _approve(address(this), address(_uniswapV2Router), 10 ** 30);

        marketingAddress = tokenOwner;
        receiveBackLPAddress = tokenOwner;
        ...
        lpHolderDivideAddress = address(0);
    ...
    function setLPHolderDivideAddress(address _lpHolderDivideAddress) public onlyOwner {
        lpHolderDivideAddress = _lpHolderDivideAddress;
    }
    ...
    function setReceiveBackLPAddress(address _receiveBackLPAddress) public onlyOwner {
        receiveBackLPAddress = _receiveBackLPAddress;
    }
    ...
    function _transfer(
        ...
        if(!isSwapedUsdt && waitLPHolderTokenNum >= swapTokensAtAmount && (from == uniswapV2Pair || to == uniswapV2Pair)){
            if (lpHolderDivideAddress != address(0)) {
                super._transfer(address(this), lpHolderDivideAddress, waitLPHolderTokenNum);
        ...
        if (takeFee) {
            ...
            // 2%进入黑洞地址，当总量只剩下100万时候，2%进入到ow账户
            if(balanceOf(_destroyAddress) >= _destroyMaxAmount){
                super._transfer(from, _tokenOwner, amount * 2 / 100);
            }else{
                super._transfer(from, _destroyAddress, amount * 2 / 100);
            }
            ...
            waitBackLiquidityTokenNum += amount * 2 / 100;
    ...
    function swapAndLiquify(uint256 _backTokenAmount) private {
