...
contract ModClaim is Ownable, ReentrancyGuard {
    ...
    mapping(address => uint) lastClaim; // a block.timestamp
    mapping(address => uint) startTime; // the block.timestamp when member was made a chief or a lead

    uint constant accFactor = 1 * 10 ** 18;
    uint chiefMultiplier = 5;
    uint leadMultiplier = 3;
    ...
    modifier modCheck() {
        require(isChief[msg.sender] || isLead[msg.sender] , "Not approved!");
    ...
    function getPaid() external nonReentrant modCheck() paymentCheck timeCheck() {
        ...
        uint timeMultiple = (block.timestamp.sub(lastClaim[msg.sender])).div(200);
