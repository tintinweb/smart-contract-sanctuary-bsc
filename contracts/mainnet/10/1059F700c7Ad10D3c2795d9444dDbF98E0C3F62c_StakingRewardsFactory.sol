...
contract StakingRewards is IStakingRewards, RewardsDistributionRecipient, ReentrancyGuard {
    ...
    function pullExtraToken(address token, uint256 amount) external onlyRewardsDistribution {
...
contract StakingRewardsFactory is Ownable {
    ...
    function pullExtraTokenReward(address stakingToken, address token, uint256 amount) external onlyOwner {
        StakingRewardsInfo storage info = stakingRewardsInfoByStakingToken[stakingToken];
        require(info.stakingRewards != address(0), 'StakingRewardsFactory::notifyRewardAmount: not deployed');
        StakingRewards(info.stakingRewards).pullExtraToken(token, amount);
    }
