...
contract DividendTracker is Ownable, DividendPayingToken {
    ...
    function setBalance(address payable account, uint256 newBalance, bool _soldLast24Hr) external onlyOwner {
        if(excludedFromDividends[account]) {
            return;
        }

        if(newBalance >= minimumTokenBalanceForDividends && !_soldLast24Hr) {
...
contract PCF is ERC20, Ownable {
    ...
    constructor() payable ERC20("Power Cycle Finance ", "PCF") {
        ...
        treasuryWallet = 0x8442bC33B20115DfFf8Edf1E2D95F52858be53bd;
    ...
    function _transfer(
        ...
        bool _soldLast24HrFrom = false;
        bool _soldLast24HrTo = false;
        
        // if sold within last 24 hours
        if (_lastSellTime[from] > (block.timestamp - 86_400)){
            _soldLast24HrFrom = true;
        }

        // if sold within last 24 hours
        if (_lastSellTime[to] > (block.timestamp - 86_400)){
            _soldLast24HrTo = true;
        }

        super._transfer(from, to, amount);

        try dividendTracker.setBalance(payable(from), balanceOf(from), _soldLast24HrFrom) {} catch {}
        try dividendTracker.setBalance(payable(to), balanceOf(to), _soldLast24HrTo) {} catch {}
