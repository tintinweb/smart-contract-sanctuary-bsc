...
contract Race is OwnableUpgradeable {
    ...
    uint256 public deposit_bracket_size;     // 5% increase whale tax per 5000 tokens... 10 below cuts it at 50% since 5 * 10
    ...
    function initialize() external initializer {
        ...
        total_users = 1;
        deposit_bracket_size = 5000 ether;     // 5% increase whale tax per 5000 tokens... 
    ...
    function _claim(address _addr, bool isClaimedOut) internal returns (uint256) {
        (uint256 _gross_payout, uint256 _max_payout, uint256 _to_payout,) = payoutOf(_addr, isClaimedOut);
    ...
    function sustainabilityFeeV2(address _addr, uint256 _pendingDiv) public view returns (uint256) {
        return sustainabilityFeeV2(_addr, _pendingDiv, true);
    }

    function sustainabilityFeeV2(address _addr, uint256 _pendingDiv, bool isClaimedOut) public view returns (uint256) {
        uint256 _pending = _pendingDiv;
        if(!isClaimedOut && users[_addr].deposits >= /*max wallet = 50k / 3.65 */13700 * 1 ether){
            _pending = 0;   // if nitro and user has max wallet reached, then dont add the pending divs to the paid out amount
        }

        uint256 _bracket = users[_addr].payouts.add(_pending).div(deposit_bracket_size);
    ...
    function payoutOf(address _addr) public view returns(uint256 payout, uint256 max_payout, uint256 net_payout, uint256 sustainability_fee) {
        return payoutOf(_addr, true);
    }
    //@dev Calculate the current payout and maxpayout of a given address
    function payoutOf(address _addr, bool isClaimedOut) public view returns(uint256 payout, uint256 max_payout, uint256 net_payout, uint256 sustainability_fee) {
        ...
        if(users[_addr].payouts < max_payout) {
            ...
            uint256 _fee = sustainabilityFeeV2(_addr, payout, isClaimedOut);
...
contract PistonRaceTestnet is OwnableUpgradeable {
    ...
    uint256 public EjectTax;
    uint256 public DepositTax;
    uint256 public ClaimTax;// !!!!!!!!!! TODO: cleanup/remove before deployment
...
/*
    ...
    function updateEjectEnable(bool value) external onlyOwner {
        EJECT_ENABLED = value;
    }
    function updateAutopilotExecutor(address value) external onlyOwner {
        autopilot_executor = value;
    ...
    function HasUsedEject(address _addr) public view returns (bool) {
       if(users[_addr].userDepositsForEject.length > 0){
            //return users[_addr].userDepositsForEject[0].ejected;
            if(users[_addr].deposits == 0){ // user has deposits in array but the user.deposits are 0. so an eject happend
                return true;
            }
        }
...
contract PistonRaceTestnetOLD is OwnableUpgradeable {
    ...
    /****** Administrative Functions ******/
    function updateTaxes(uint256 _depositTax, uint256 _claimTax, uint256 _exitTax, uint256 _compoundTax) external onlyOwner {
        DepositTax = _depositTax;
        ClaimTax = _claimTax;
        ExitTax = _exitTax;
        CompoundTax = _compoundTax;
