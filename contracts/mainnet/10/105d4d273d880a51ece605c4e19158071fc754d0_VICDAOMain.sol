...
contract VICDAOMain is BEPPausable {
    ...
    Holder[] lsHolder;
    mapping(address => uint256) lsHolderIndex;
    ...
    constructor() payable{
        ...
        stopStakeAt = 0;
        uint256[] memory ls ;
        lsHolder.push(Holder(address(0), 0, false, ls, 0, ls));
    }
    ...
    function stake(uint256 amount, address tokenAddress) external whenNotPaused{
        ...
        if(lsHolderIndex[msg.sender] == 0)
        {
            uint256[] memory ls ;
            lsHolder.push(Holder(msg.sender, 0, false, ls, 0, ls));
            lsHolderIndex[msg.sender] = lsHolder.length - 1;
        }
        uint256 index = lsHolderIndex[msg.sender];
        Holder storage holder = lsHolder[index];
    ...
    function stakeAt(uint256 amount, address tokenAddress, uint256 timestamp) external whenNotPaused{
        require(!stopStake, "stake stoped");
        require(amount >= minAmount,"amount >= minAmount");
        require(tokenAddress == VIMContract || tokenAddress == NELUMContract,"Can not stake this token");
        if (!test)
        ...
        Stake memory newstake = Stake(allStake.length, msg.sender, amount, timestamp, 0, timestamp/86400*86400, false, tokenAddress);  
        allStake.push(newstake);
        if(lsHolderIndex[msg.sender] == 0)
        {
            uint256[] memory ls ;
        ...
        uint256 index = lsHolderIndex[msg.sender];
        lsHolder[index].dNULEM += amount;
        lsHolder[index].lsStakeId.push(allStake.length - 1);
    ...
    function unStakeAt(uint256 stakeId, uint256 timestamp) external whenNotPaused{
        ...
        unStakeObj.unStakeAt = timestamp;
        unStakeNum++;
        uint256 index = lsHolderIndex[msg.sender];
        lsHolder[index].dNULEM -= unStakeObj.amount;
    ...
    function updateWhiteLotus(uint256 x) external whenNotPaused{
        uint256[] memory lsMax = new uint256[](whiteLotusCount);
        uint256 min = 0;
        uint256 minIndex = 0;
        for (uint256 i = 0; i < x; i++)
        {
            if (min == 0)
            {
                min = lsMax[0];
                minIndex = 0;
                for (uint256 j = 0; j < whiteLotusCount; j++)
                {
                    if (lsMax[j] < min)
                    {
                        min = lsMax[j];
        ...
        emit fortuneDrawDone(msg.sender, lsMax[0]);
    }

    function testx(uint256[] memory x) external view returns(uint256[] memory res)
    {
        uint256[] memory lsMax = new uint256[](whiteLotusCount);
        uint256 min = 0;
        uint256 minIndex = 0;
    ...
    {
        uint256 holderIndex = lsHolderIndex[_address];
        Holder memory holder = lsHolder[holderIndex];
    ...
    {
        return (lsHolder[lsHolderIndex[_address]]);
    }
