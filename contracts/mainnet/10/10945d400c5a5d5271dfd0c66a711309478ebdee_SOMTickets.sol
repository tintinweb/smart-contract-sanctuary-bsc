...
contract SOMTickets is Ownable {
    ...
    function purchase(uint256 ticketsNum) public {
        address payer = msg.sender;
        uint256 amount = price * ticketsNum;
        busd.transferFrom(payer, address(this), amount);
        for (uint256 i = 0; i < ticketsNum; i++) {
            purchased[nextTicketId] = payer;
            nextTicketId++;
        }
    }

    uint256 private processedTicketId = 0;
    function getProcessedTicketId() public view returns (uint256) {
        return processedTicketId;
    }

    event AddTicket(address payer);
    function process() public onlyOwner {
        uint maxTicketId = processedTicketId + 100;
        while (true) {
            processedTicketId++;
            if (processedTicketId == nextTicketId || processedTicketId == maxTicketId) {
                break;
            }
            address payer = purchased[processedTicketId];
            emit AddTicket(payer);
        }
