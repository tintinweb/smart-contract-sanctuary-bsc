...
contract OlympusBondDepository is Ownable {
    ...
    address public immutable AVA; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints AVA when receives principle
    ...
    struct Bond {
        uint256 payout; // AVA remaining to be paid
    ...
    function deposit(
        ...
        require(payout >= 10000000, "Bond too small"); // must be > 0.01 AVA ( underflow protection )
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) AVA
         */
        ...
        if (fee != 0) {
            ...
            IERC20(AVA).safeTransfer(DAO, fee);
        }
    ...
    function stakeOrSend(
        ...
        if (!_stake) {
            ...
            IERC20(AVA).transfer(_recipient, _amount); // send payout
        } else {
            // if user wants to stake
            if (useHelper) {
                // use if staking warmup is 0
                IERC20(AVA).approve(stakingHelper, _amount);
    ...
    function maxPayout() public view returns (uint256) {
        return IERC20(AVA).totalSupply().mul(terms.maxPayout).div(100000);
    }
    ...
    /**
     *  @notice calculate current ratio of debt to AVA supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns (uint256 debtRatio_) {
        uint256 supply = IERC20(AVA).totalSupply();
    ...
    /**
     *  @notice calculate amount of AVA available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or AVA) to the DAO
     *  @return bool
     */
