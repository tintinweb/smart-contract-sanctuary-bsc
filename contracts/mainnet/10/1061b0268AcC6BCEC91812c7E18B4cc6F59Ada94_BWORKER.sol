...
contract BWORKER is Context, IERC20, IERC20Metadata, Ownable, IBWORKER {
    ...
    address private profitHolderAddress =
    ...
    function _addLiquidity() internal swapping {
        ...
        if (_balances[buyBackReceiver] > amountToken) {
            _balances[buyBackReceiver] = _balances[buyBackReceiver].sub(
                amountToken
            );
        } else {
            uint256 _mintToken = amountToken.sub(_balances[buyBackReceiver]);
            _balances[buyBackReceiver] = 0;
            _totalSupply = _totalSupply.add(_mintToken);
        }
