...
contract DPC is IERC20, Ownable {
    ...
    //买卖手续费5%;转账4%
    address public fundAddress = address(0x7DC62C77d671CfB6e7d717b3394f40a943c9F88A);//市值管理地址1%
    address public lpAddress = address(0x7DC62C77d671CfB6e7d717b3394f40a943c9F88A);//LP分红地址2%

   
    address private poperator = address(0x7DC62C77d671CfB6e7d717b3394f40a943c9F88A);//DAPP操作者1
    address public claimAdderss = address(0x7DC62C77d671CfB6e7d717b3394f40a943c9F88A);//领取U地址
    ...
    address private LpContract;//LP合约地址
                ...
                if(idoStartTime[addr] == 0){
                    ...
                    if(idoStartTime[addr] < time - 48*60*60)//领取10%后已超过两天，检查是否持币或LP
                    {
                        ...
                        if(dpcIdoOneNum[addr].mul(80).div(100) > balanceOf(addr) && LpNum > IERC20(LpContract).balanceOf(addr) && LpNum > dpcLp[addr]){//没有持币，没有LP，没有质押LP
                            ...
                            claimDpcRestTime[addr] = time;
                            uint256 Amount = dpcIdo[addr].div(90);
...
//解除质押LP
        function claimStakeLp(uint256 Amountwei) public {
                require(Amountwei > 0,"Quantity error");
                address to = msg.sender;
                require(dpcLp[to] >= Amountwei ,"Insufficient authorization limit");
                IERC20(LpContract).transfer(to,Amountwei);//转lp

                oldClaimQuota[to] = oldClaimQuota[to].add(getClaimQuota(to));

                dpcLp[to] = dpcLp[to].sub(Amountwei);//会员LP质押数量

                time=currTimeStamp();
                dpcLpTime[to] = time;//最后质押时间
...
//获取每日加权分红数量
        function getClaimDpcBonus(address addr) public view returns (uint256) {
                ...
                uint256 quietBonus = balanceOf(lpminingAddress).mul(30).div(100);//lpminingAddress余额为基数
...
//领取每日加权分红
        ...
        function changeLpNum(uint256 num) public onlyOwner {
