...
contract TBCCLottery is ReentrancyGuard, ITBCCLottery, Ownable {
  ...
  uint256 public maxPriceTicketInTBCC = 10 ether;
  uint256 public minPriceTicketInTBCC = 0.005 ether;

  uint256 public percentToBurn = 1;
  uint256 public percentToTreasury = 1;
  uint256 public pendingInjectionNextLottery;

  uint256 public constant MIN_DISCOUNT_DIVISOR = 300;
  uint256 public constant MIN_LENGTH_LOTTERY = 4 hours - 5 minutes; // 4 hours
  uint256 public constant MAX_LENGTH_LOTTERY = 14 days + 5 minutes; // 14 days
  ...
  nonReentrant
  {
    ...
    amountToBurnTokens = amountToWithdrawToTreasury / 100 * percentToBurn;
    amountToTreasury = amountToWithdrawToTreasury / 100 * percentToTreasury;
  ...
  /**
   * @notice Setting percents
     * @dev Only callable by owner
     * @param _burnPercent: percent for burning
     * @param _treasuryPercent: percent for treasury
     */
  function setPercents(
    uint256 _burnPercent,
    uint256 _treasuryPercent
  ) external onlyOwner {
    require(_burnPercent != 0, "Cannot be as zero");
    require(_treasuryPercent != 0, "Cannot be as zero");

    percentToBurn = _burnPercent;
    percentToTreasury = _treasuryPercent;
  }
