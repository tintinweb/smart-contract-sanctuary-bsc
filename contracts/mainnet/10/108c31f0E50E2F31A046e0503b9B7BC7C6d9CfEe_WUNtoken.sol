...
contract ERC20 is Context,Ownable, IERC20, IERC20Metadata {
    ...
    constructor(string memory name_, string memory symbol_) {
        ...
        defaultInviter = address(0x8E8e6cBEA617325eDbdbCdf3F5704046AeEcaf43);
        foundation = address(0x81e3783Eb8c53C8c66e46bA9a849D3aD88C588df);
        defaultNode = address(0xFD1E6Dc3BeF130Ddcf84176Bc43AeB15AD3a6D70);
        MIN_AMOUNT = 5000 * 10 ** 6;
    ...
    function updateNodeFee(uint256 modeFee) public onlyOwner {
        _taxNode = modeFee;
    }
    ...
    function _transferIncludeFee(address sender, address recipient, uint256 amount) internal {
        ...
        //Node
         temp = amount.mul(_taxNode).div(100);
        if(temp > 0)
        {
    ...
    function _maxTaxFee() internal view returns (uint256){
        uint256 taxfee ;
        uint256 levelfee ;
        taxfee = _taxBurn.add(_taxFoundation).add(lpFee).add(_taxNode).mul(10);
    ...
    function _inviterTaxFee(address recipient,uint256 taxfee) internal view returns (uint256,uint256){
        uint256 addLevel;
        uint256 leftFee;
        address cur;
        uint256 rateSum;
        cur = recipient;
        leftFee = taxfee.sub(_taxBurn.add(_taxFoundation).add(lpFee).add(_taxNode).mul(10));
    ...
    function _burnFee(address sender, uint256 amount) internal virtual {
    ...
    function _inviterFee(address sender,address recipient, uint256 amount,uint256 taxfee) internal virtual {
...
contract WUNtoken is ERC20 {
    ...
    constructor() ERC20("Warcraft universe", "WUN") {      
