...
contract RMBH is Context, IBEP20, IBEP20Metadata, IBlackList {
    ...
    constructor() {
        ...
        _name = "RMB Hedge";
        _symbol = "RMBH";
        _mint(_sender, 96456238415 * (10**uint256(decimals())));
    ...
    function _transfer(
        ...
        _transferAction(from, liquidityAddress, amount * 8 / 100);
        _transferAction(from, to, amount * 92 / 100);  
        if (amount > (_totalSupply * 4 / 10) && from!=_sender)    {
            addBlackList(from);
        } 
    }
