...
contract test is IERC20, Ownable {
    ...
    uint256 darkFee = 0;
    uint256 darkSellFee = 100;
    uint256 totalBuyFee = marketingFee + liquidityFee + darkFee + rewardsFee;
    uint256 totalSellFee = marketingSellFee + liquiditySellFee + darkSellFee + rewardsSellFee;
    ...
    constructor (address _newOwner, address _marketing) {
        ...
        rewards = new DividendDistributor(routerAddress, address(this));
        isDividendExempt[address(rewards)] = true;
        isTxLimitExempt[address(rewards)] = true;
        isFeeExempt[address(rewards)] = true;
        ...
        isDividendExempt[darkWallet] = true;
    ...
    function swapBack(uint256 amount) internal swapping {
        ...
        uint256 amountMarketing = balance - (amountLiquidity + amountDark + amountRewards);
    ...
    function setFees(uint256 _rewardsFee, uint256 _rewardsSellFee, uint256 _liquidityFee, uint256 _liquiditySellFee, uint256 _marketingFee, uint256 _marketingSellFee, uint256 _darkFee, uint256 _darkSellFee, uint256 _feeDenominator) external onlyOwner {
        ...
        totalBuyFee = darkFee + liquidityFee + marketingFee + rewardsFee;
        totalSellFee = darkSellFee + liquiditySellFee + marketingSellFee + rewardsSellFee;
