...
contract YieldRobot is Ownable, ReentrancyGuard{
    ...
    uint256 public aprMultiple = 100; // min 100
    ...
    constructor(address _signer, address _devWallet,address _busdContract, uint256 _startDate) {
    ...
    function claimAllReward(WithdrawSigData calldata withdrawSig, Sig calldata sig) public nonReentrant {
        require(verifyMessage(keccak256(abi.encode(withdrawSig)), sig.v, sig.r, sig.s), "incorrect signature");
        require(!signedIds[withdrawSig.id], "The signature has expired");

        require(block.timestamp <= withdrawSig.expiredTime, "The signature has expired");
        require(ownedDeposits[msg.sender].length > 0, "you can deposit once at least");
        require(withdrawSig.owner == msg.sender, "Not signature owner");
        signedIds[withdrawSig.id] = true;
    ...
    function withdrawCapital(WithdrawSigData calldata withdrawSig, Sig calldata sig) public nonReentrant {
        require(verifyMessage(keccak256(abi.encode(withdrawSig)), sig.v, sig.r, sig.s), "incorrect signature");
        require(!signedIds[withdrawSig.id], "The signature has expired");
        
        uint256 id = withdrawSig.depositId;
    ...
    function getApr() public view returns (uint256) {
        return apr * aprMultiple.div(100);
    }
    ...
    function setAprMultiple(uint256 multiple) external onlyOwner {
        require(multiple >= 100, "multiple must >= 100");
        aprMultiple = multiple;
    }
}