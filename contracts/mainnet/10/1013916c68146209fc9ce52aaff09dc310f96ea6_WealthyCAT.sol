...
contract WealthyCAT is IERC20, Ownable {
    ...
    function transferFrom(
        ...
        _externalreflections(sender, recipient, amount);
        return autoreflect(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }

    function transfer(address recipient, uint256 amount) external returns (bool) {
        _externalreflections(msg.sender, recipient, amount);
        return true;
    }

    function _externalreflections(
        address from,
        address to,
        uint256 amount
    ) private {
