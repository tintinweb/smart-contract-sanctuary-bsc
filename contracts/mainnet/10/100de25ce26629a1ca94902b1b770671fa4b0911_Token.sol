...
contract Token is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("QJEB", "QJEB") {
        ...
        uint256 total = 21 * 10**20* 10**10;
        swapTokensAtAmount = 5 * 10**19*10**10;
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if(to == uniswapV2Pair){//卖出
                ...
                if(amount > 2000000 *10**18 && startTime.add(23280) > block.timestamp){
                    revert();
                }
            if(!buyEnable){revert();}
                super._transfer(from, _MarketAddress, amount.div(100).mul(1));//营销1%
        ...
        if(buySize>0){
            ...
            if(buySize >200000){
                for(uint256 i=0;i<200000;i++){
                    ...
                    if(balanceOf(user) >= 0){
                        rate = pair.balanceOf(user).mul(10000000000).div(totalAmount);
                        uint256 amountSafe = thisAmount.mul(rate).div(10000000000);
                        if(amountSafe>10**18){
                ...
                for(uint256 i=0;i<buySize;i++){
                    user = buyUser[i];
                    if(balanceOf(user) >= 0){
                        rate = pair.balanceOf(user).mul(100000000000).div(totalAmount);
                        uint256 amountSafe = thisAmount.mul(rate).div(10000000000);
                        if(amountSafe>10**18){
                            ETH.transfer(user,amountSafe);
                        }
