...
contract GasSwap is ReentrancyGuard, AccessControlEnumerable {
    ...
    function swap(uint256 amount) external nonReentrant {
        ...
        uint256 usdtAmount = amount * swapRatio / decimals;
        IERC20(usdt).safeTransferFrom(msg.sender, address(this), usdtAmount);
        IERC20(usdt).safeApprove(lpStake, usdtAmount);
        ILpStake(lpStake).recharge(usdtAmount);
        IGas(gas).mint(msg.sender, amount);
    }
