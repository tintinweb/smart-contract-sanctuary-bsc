...
contract SignatureVerifier is ISignatureVerifier, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    returns (bool) {
        address signer = keccak256(
            abi.encode(nonce, receiver, dorAmount, goldAmount)
    ...
    returns (bool) {
        address signer = keccak256(
            abi.encode(nonce, receiver, account)
        ).ethSignedMessage().recover(signature);
        require(signer == TRUSTED_PARTY, "Invalid request");
        
        nonceUsed[nonce] = true;
        emit UsedNonce(receiver, nonce, "withdraw in renting");
        return true;
    }
    ...
    returns (bool) {
        address signer = keccak256(
            abi.encode(nonce, receiver, tokenIds, amounts)
        ).ethSignedMessage().recover(signature);
        require(signer == TRUSTED_PARTY, "Invalid request");
        nonceUsed[nonce] = true;
        emit UsedNonce(receiver, nonce, "gacha");
        return true;
    }
    ...
    returns (bool) {
        address signer = keccak256(
            abi.encode(nonce, account, tokenId)
        ).ethSignedMessage().recover(signature);
        require(signer == TRUSTED_PARTY, "Invalid request");
        nonceUsed[nonce] = true;
        emit UsedNonce(account, nonce, "levelup");
        return true; 
    }
