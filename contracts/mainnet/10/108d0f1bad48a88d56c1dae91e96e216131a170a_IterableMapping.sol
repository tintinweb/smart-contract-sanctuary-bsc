...
contract ERC20 is Context, IERC20 {
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "LCP: Transfer amount exceeds allowance"));
        return true;
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "LCP: Decreased allowance below zero"));
        return true;
    }

    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        require(sender != address(0), "LCP: Transfer from the zero address");
        require(recipient != address(0), "LCP: Transfer to the zero address");
        require(!_isBlacklisted[sender] && !_isBlacklisted[recipient], "This address is BlackListed!");
        _beforeTokenTransfer(sender, recipient, amount);

        _balances[sender] = _balances[sender].sub(amount, "LCP: Transfer amount exceeds your balance");
        _balances[recipient] = _balances[recipient].add(amount);
        emit Transfer(sender, recipient, amount);
    }

    function _mint(address account, uint256 amount) internal virtual {
        require(account != address(0), "LCP: Mint to the zero address");
    ...
    function _burn(address account, uint256 amount) internal virtual {
        require(account != address(0), "LCP: Burn it!!");

        _beforeTokenTransfer(account, address(0), amount);

        _balances[account] = _balances[account].sub(amount, "LCP: Burn amount exceeds your balance");
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  //BUSD BEP-20 version Dividend Token // Bridged from ERC-20 and equaling the same price as ERC-20
  address public immutable DividendsToken = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); //BUSD
...
contract LCP is ERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;
    address public immutable uniswapV2Pair;

    //BEP-20 BUSD dividend tracker
    address public immutable DividendsToken = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); //BUSD
 ...
 //Add multiple wallets for the blacklist //
   ...
   LCPDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("Light Company", "LCP") {
        //Reflection Rewards %
        _ETHRewardsFee = 6;
        //Liqudity Fee %
        _liquidityFee = 3;
    	...
    	dividendTracker = new LCPDividendTracker();
    	antiBotEnabled = false;

    	liquidityWallet = owner();
		
        //Expenses/Buyback/Burn Wallet Address
		marketingWallet = payable(0xa3A0a02e4738C4F606d6C7f8d15149FF290CE901);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "LCP: The dividend tracker already has that address");

        LCPDividendTracker newDividendTracker = LCPDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "LCP: The new dividend tracker must be owned by the LCP token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "LCP: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "LCP: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "LCP: The PanSHIBSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "LCP: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "LCP: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "LCP: Cannot update gasForProcessing to same value");
    ...
    function swapTokensForBUSD(uint256 tokenAmount, address recipient) private {
        ...
        // Generate the Pancakeswap pair path of weth -> LCP - BEP-20 BUSD
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // Accept any amount of LCP on zero value
            path,
            recipient,
            block.timestamp
        );
    ...
    function updateMarketingFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "LCP: Expenses tax must be between 0 and 10");
        marketingFee = newFee;
    }
    
    function updateLiquidityFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "LCP: Liquidity tax must be between 0 and 10");
        liquidityFee = newFee;
    }
    
    function updateETHRewardsFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 15, "LCP: ETH/BNB Rewards tax must be between 0 and 15");
        liquidityFee = newFee;
    }
...
contract LCPDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("LCP_Dividend_Tracker", "LCP_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "LCP_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "LCP_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main LCP contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 3600, "LCP_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "LCP_Dividend_Tracker: Cannot update claimWait to same value");
