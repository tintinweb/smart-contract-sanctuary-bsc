...
contract WRTEST is Context, Ownable {
    ...
    function deposit(address ref, uint256 amount) public checkPaused{
        ...
        if (users[msg.sender].lastTime != 0) {
            require(users[msg.sender].lastTime + 60 * 7 < block.timestamp, "err: not in time");
        }
    ...
    function getRewardsSinceLastDeposit(address adr) public view returns(uint256) {
        uint256 secondsPassed=min(420, block.timestamp - users[adr].lastTime);
        return secondsPassed * users[adr].deposit / 86400;
    }
