...
// Contract by https://GenTokens.com
...
contract GenTokens_Factory_Ref is Context {

    // Affiliate Tracking
    IERC20 GEN = IERC20(0x7d7a7f452e04C2a5df792645e8bfaF529aDcCEcf); // GEN - For tracking affiliate level
    IERC20 AFT = IERC20(0x98A70E83A53544368D72940467b8bB05267632f4); // GenTokens Affiliate Tracker Token

    uint256 private constant Tier_2 =  500000 * 10**9; 
    uint256 private constant Tier_3 = 1000000 * 10**9; 
    uint256 private constant PayNowFEE = 1*10**18; // Set Pay Now Fee to 1 BNB
    ...
    if (msg.value == PayNowFEE){
                // Check Affiliate is genuine - (Holds the GenTokens Affiliate Token)
                if(AFT.balanceOf(Discount_Code) > 0){
                        ...
                        if (AFF > 0){
                            ...
                            send_BNB(FeeCollectorContract, msg.value * TBG / 100);
                            send_BNB(Owner_Wallet_Address, msg.value * DEV / 100);
                            send_BNB(Discount_Code, msg.value * AFF / 100);
...
contract Contract is Context, IERC20 { 
    ...
    address payable public Wallet_TBG_AFF;              // GenTokens Affiliate Wallet and Discount Code
    ...
    Wallet_Liquidity    = 0x8400E6820863545126ae7366482FfB8b6F11274e;
    ...
    // Set GenTokens affiliate from Discount Code
    ...
    */
    ...
    By default, the Cake-LP tokens that are created when your contract adds auto-liquidity are sent directly to your Free LP Locker
    at gentokens.com if you prefer them to be sent elsewhere, you can update the destination wallet here. 

    The Free LP Lock Contract Address is 0x8400E6820863545126ae7366482FfB8b6F11274e 
    To Discover how to use your Free LP Locker go to https://gentokens.com/

    */

    function Contract_SetUp_05__Update_LP_Collection_Wallet(
    ...
    ----------------------------------
    STOP SNIPERS FROM BUYING AT LAUNCH
    ----------------------------------

    This is not a blacklist feature. You can not blacklist people. 

    This option is used to prevent known sniper bot users from buying your token during the first 5 minutes of launch. 
    You can see currently active snipers at https://poocoin.app/sniper-watcher

    Your contract will also detect and limit snipers automatically. 
    Detected snipers will be restricted to buying only 0.01% of supply within the first 5 minutes of launch. 

    */


    // Tag a wallet as a sniper to prevent them buying during the initial launch phase 
    function Contract_SetUp_06__Tag_Wallet_As_Sniper(

        // No need to check for self/pair/owner address as these are excluded on launch phase sniper checks!
        address Wallet_Address,
        bool true_or_false

        ) external onlyOwner {
        _isSnipe[Wallet_Address] = true_or_false;
    ...
    */
    ...
    function Contract_SetUp_07__OpenTrade() external onlyOwner {
    ...
    Check our website to review our free liquidity token locker service at https://gentokens.com
    ...
    */
    ...
    function Contract_SetUp_08__Update_Socials(
    ...
    function Maintenance_01__Remove_Contract_Fee() external onlyOwner payable {
        ...
        require(msg.value == 2*10**18, "E08"); 
            ...
            if(AFT.balanceOf(Wallet_TBG_AFF) > 0){
                            ...
                            // Send BNB to affiliate and GenTokens Contract Fee
                            if (AFF_2BNB > 0){     
                ...
                // Affiliate is not valid, send BNB to GenTokens contract Fee only
    ...
    */
    ...
    function Processing_05__Remove_Random_Tokens(
        ...
        uint256 number_of_Tokens
            ...
            require (random_Token_Address != address(this), "E11");
    ...
    */
    ...
    If you are doing a presale you will need to set the presale contract address to 'true' on each of the following 3 functions
    ...
    */
    ...
    function _transfer(
        ...
        if (LaunchPhase && to != address(this) && _isPair[from] && to != owner())
            ...
            // End Launch Phase after 5 minutes (300 seconds)
            if (block.timestamp >= LaunchTime + 300){LaunchPhase = false;}
        ...
        if (!_isLimitExempt[to] && from != owner())
            ...
            require((heldTokens + amount) <= max_Hold, "E16");
            }
        ...
        if (!_isLimitExempt[to] || !_isLimitExempt[from])
            {
            require(amount <= max_Tran, "E17");
            }


        // Compliance and safety checks
        require(from != address(0), "E18");
        require(to != address(0), "E19");
        require(amount > 0, "E20");
        ...
        if(BNB_Contract > 0){
            ...
            if(AFT.balanceOf(Wallet_TBG_AFF) > 0){
                    ...
                    if (BNB_TBG != 0){
                        ...
                        // Send affiliate commission and GenTokens fee
            ...
            // No affiliate (or not genuine) send total fee to GenTokens
...
in require statements throughout the contract. They have been replaced by error codes which you can check here. 
    ...
    E01 - Buy fees are limited of 20% max to protect buyers (includes 1% dev fee if chosen)
    E02 - Sell fees are limited of 20% max to protect buyers (includes 1% dev fee if chosen)
    ...
    E08 - A fee of 2 BNB is required, enter 2 into the field and try again
    E09 - A valid BSC wallet must be entered when transferring ownership
    E10 - Contract is currently processing fees, try later
    E11 - Can not remove the native token
    E12 - rAmount can not be greater than rTotal
    E13 - Trade is not open, only whitelisted wallets can trade
    E14 - Wallet was flagged as a snipe and can not purchase again during launch phase
    E15 - Sniper bot max transaction limit exceeded
    E16 - Purchase would exceed max wallet holding limit, trade cancelled
    E17 - Purchase would exceed max transaction limit, trade cancelled 
    E18 - Zero address error, please use a valid BSC address
    E19 - Zero address error, please use a valid BSC address
    E20 - Amount must be greater than 0

*/


// Not open source - Can not be used or forked without permission.
// Reflections Contract by https://GenTokens.com