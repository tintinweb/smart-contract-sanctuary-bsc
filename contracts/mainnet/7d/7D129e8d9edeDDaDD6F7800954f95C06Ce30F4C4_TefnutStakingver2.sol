...
// File contracts/TefnutStakingver2.sol
...
contract TefnutStakingver2 is ReentrancyGuard {
    address private owner;
    uint256 private rateReward = 25;
    IERC20 public s_stakingToken;
    IERC20 public s_rewardToken;

    uint256 public constant REWARD_RATE = 25;
    ...
    mapping(address => uint256) public s_lastUpdateTime;
    ...
    function setRateReward(uint256 _rateReward) public{
        require(msg.sender == owner);
        rateReward = _rateReward;
    }

    function getRateReward() public view returns (uint256){
        return rateReward;
    }

    function earned(address account) public view returns (uint256) {
        uint256 balanceReward = calculateReward(account);
        uint256 pastRewards = s_rewards[account];
        uint256 _earned = balanceReward + pastRewards;
        return _earned;
    }
    ...
    function rewardPerToken(address account) public view returns (uint256) {
        if (s_totalSupply == 0) {
            ...
            return
                ...
                (block.timestamp - s_lastUpdateTime[account]) * (s_balances[account] / 100 * rateReward) / 365 / 24 / 60 / 60;
        }
    }

    function calculateReward(address account) public view returns (uint256) {
        return (block.timestamp - s_lastUpdateTime[account]) * (s_balances[account] / 100 * rateReward) / 365 / 24 / 60 / 60;
    }
