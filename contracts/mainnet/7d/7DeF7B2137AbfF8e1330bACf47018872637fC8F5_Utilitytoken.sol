...
contract MythCoinFlip {
    ...
    function changeWhiteList(address _whitelist) external {
        require(msg.sender == owner, "only owner can set whitelist");
        whitelist = MythWhiteList(_whitelist);
    }

    function changeMyth(address _myth) external {
        require(msg.sender == owner, "only owner can set myth");
        mythContract = Utilitytoken(_myth);
    }
