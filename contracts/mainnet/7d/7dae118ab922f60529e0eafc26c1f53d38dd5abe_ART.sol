...
contract Setter is Ownable {
    ...
    uint256 public brFeeRate=9999; //99.99%

    bool public saSwitch=true;
    uint256 public saFeeRate=300; //3%

    address public feeAddr =0xA18b9B64c68b93DC54487F1C1ff772dd69859864;

}

contract ART is ERC20, Setter {
    using SafeMath for uint256;
    event Claim(address opt,address to, uint256 amount);
    
    constructor() payable ERC20("ART", "ART")  {
        uint256 totalSupply = 10000000*1e18;
        whiteList[owner()]=true;
        whiteList[feeAddr] =true;
    ...
    function _takeBuyFee(address _sender,uint256 _feeAmount) private {
        if (_feeAmount>0){
            super._transfer(_sender, feeAddr, _feeAmount);
        }
    }
