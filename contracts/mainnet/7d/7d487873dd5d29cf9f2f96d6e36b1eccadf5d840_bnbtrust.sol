...
contract bnbtrust{
    ...
    uint8 constant commission = 130;
    uint8 constant marketing = 20;
    ...
    constructor(address _commission_wallet,address _marketing_wallet, uint256 startDate) {
    ...
    function deposit(uint16 _package, address _referrer) external payable {
        ...
        uint256 marketingAmount = (msg.value * marketing) / percent_divide_by;
        payable(marketing_wallet).transfer(marketingAmount);
        emit CommissionPaid(msg.sender, msg.value, marketingAmount);
