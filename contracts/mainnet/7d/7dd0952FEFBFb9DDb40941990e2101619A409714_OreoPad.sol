...
contract OreoPad is Context, Ownable, IERC20 {
    ...
    uint256 constant public _totalSupply = 1_000_000 * 10**9;
    ...
    address payable private marketingAddress = payable(0x0b75f9A5Ae9CEbd110a39B7E18F88473d6Dd1d7A);
    address payable private developerFeeAddress = payable(0);

    IRouter02 public swapRouter;
    string constant private _name = "Oreo Pad";
    string constant private _symbol = "Oreo";
    ...
    function changeWallets(address marketing, address developement) external onlyOwner payable {
        require(!cannotChangeMore,"Owner has locked fees");
        marketingAddress = payable(marketing);
        developerFeeAddress = payable(developement);
    }
    ...
    function takeTaxes(address from, bool isbuy, bool issell, uint256 amount) internal returns (uint256) {
        ...
        if (feeAmount > 0) { if(burnFee > 0 && !inSwap) {
            uint256 deadAmount = feeAmount * burnFee / 100;
            balance[address(DEAD)] += deadAmount;
            emit Transfer(from, address(DEAD), deadAmount);
            feeAmount = feeAmount - deadAmount; }
    ...
    function internalSwap(uint256 contractTokenBalance) internal inSwapFlag {
        ...
        if(burnFee > 0) {
            marketingFee = marketingFee + (burnFee / 2);
            developerFee = developerFee + (burnFee / 2);
        } 

        uint256 howMuchToTransferToDeveloper = address(this).balance * developerFee / 100;
        uint256 howMuchToTransferToMarketing = address(this).balance * marketingFee / 100;

        (success,) = marketingAddress.call{value: howMuchToTransferToMarketing, gas: 35000}("");
        (success,) = developerFeeAddress.call{value: howMuchToTransferToDeveloper, gas: 35000}("");
    }
        ...
        function setTaxPercentage(uint256 newmarketingFee,uint256 newburnFee, uint256 newdeveloperFee) external onlyOwner {
            require(!cannotChangeMore,"Owner has locked fees");
            marketingFee = newmarketingFee;
            newburnFee = newburnFee;
            developerFee = newdeveloperFee;

            require(marketingFee + burnFee + developerFee == 100,"must be 100%");
        }
