...
interface IUniswapV2Pair {
    ...
    function gHODLeserves() external view returns (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast);
...
contract HODL is ERC20, Ownable {
    ...
    HODLDividendTracker public dividendTracker;

    address constant public deadWallet = 0x000000000000000000000000000000000000dEaD;

    address private immutable ETH = address(0x2170Ed0880ac9A755fd29B2688956BD959F933F8); //ETH
    
    uint256 public swapTokensAtAmount = 500000 * (10**18); 
    uint256 public maxTransactionAmount = 1000000000 * (10**18); 

    uint256 private _BuyETHRewardsFee = 0;
    uint256 public _BuyLiquidityFee = 1;
    uint256 public _BuyMarketingFee = 2;
    uint256 public _BuyBackHODLFee = 1;
  
    uint256 public _BuyTotalFees = _BuyETHRewardsFee.add(_BuyLiquidityFee).add(_BuyMarketingFee).add(_BuyBackHODLFee);
    
    uint256 private _SellETHRewardsFee = 0;
    uint256 public _SellLiquidityFee = 1;
    uint256 public _SellMarketingFee = 2;
    uint256 public _SellBackHODLFee = 3;
    uint256 public _SellTotalFees = _SellETHRewardsFee.add(_SellLiquidityFee).add(_SellMarketingFee).add(_SellBackHODLFee);
    
    uint256 public maxWalletToken = 1000000000 * (10**18); 

    address public _marketingWalletAddress = 0x301c0F44FdAcd973Be1A1dAEE339eFAf8CD468f0;
    address public _HODLWalletAddress = 0x64054e7A5cC1a1D8a65546dEfA676883507A96cB;
    ...
    constructor() ERC20("HODL Poker", "$HODL") {

    	dividendTracker = new HODLDividendTracker();
        ...
        dividendTracker.excludeFromDividends(_HODLWalletAddress);
        ...
        excludeOrIncludeFromFees(_HODLWalletAddress, true);
        ...
        excludeOrIncludeFromMaxTx(_HODLWalletAddress, true);
        ...
        excludeOrIncludeFromMaxWallet(_HODLWalletAddress, true);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "HODL: The dividend tracker already has that address");

        HODLDividendTracker newDividendTracker = HODLDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "HODL: The new dividend tracker must be owned by the HODL token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "HODL: The router already has that address");
    ...
    function excludeOrIncludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "HODL: Account is already the value of 'excluded'");
    ...
    {
        require(_isExcludedFromMaxTx[account] != excluded, "HODL: Account is already the value of 'excluded'");
    ...
    {
        require(_isExcludedFromMaxWallet[account] != excluded, "HODL: Account is already the value of 'excluded'");
    ...
    function setHODLWallet(address payable wallet) external onlyOwner{
         require(wallet != address(0), "Invalid address");
        _HODLWalletAddress = wallet;
    }

      function setBuyFees(uint256 buyEthRewardFee, uint256 buyLiquidityFee, uint256 buyMarketingFee, uint256 buyBackHODLFee) external onlyOwner
    {
        _BuyETHRewardsFee = buyEthRewardFee;
        _BuyLiquidityFee = buyLiquidityFee;
        _BuyMarketingFee = buyMarketingFee;
        _BuyBackHODLFee = buyBackHODLFee;
        _BuyTotalFees = _BuyETHRewardsFee.add(_BuyLiquidityFee).add(_BuyMarketingFee).add(_BuyBackHODLFee);

    }

    function setSellFees(uint256 sellEthRewardFee, uint256 sellLiquidityFee, uint256 sellMarketingFee, uint256 sellBackHODLFee) external onlyOwner
    {
        _SellETHRewardsFee = sellEthRewardFee;
        _SellLiquidityFee = sellLiquidityFee;
        _SellMarketingFee = sellMarketingFee;
        _SellBackHODLFee = sellBackHODLFee;
        _SellTotalFees = _SellETHRewardsFee.add(_SellLiquidityFee).add(_SellMarketingFee).add(_SellBackHODLFee);

    }
      function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "HODL: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 800000, "HODL: gasForProcessing must be between 200,000 and 800,000");
        require(newValue != gasForProcessing, "HODL: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        
        {
            ...
            uint256 walletTokens = contractTokenBalance.mul(_SellMarketingFee.add(_SellBackHODLFee)).div(_SellTotalFees);
            uint256 contractBalance = address(this).balance;
            swapTokensForEth(walletTokens);
            uint256 newBalance = address(this).balance.sub(contractBalance);
            uint256 marketingShare = newBalance.mul(_SellMarketingFee).div(_SellBackHODLFee.add(_SellMarketingFee));
            uint256 developmentShare = newBalance.sub(marketingShare);
            payable(_marketingWalletAddress).transfer(marketingShare);
            payable(_HODLWalletAddress).transfer(developmentShare);
...
contract HODLDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("HODL_Dividend_Tracker", "HODL_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 200000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "HODL_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "HODL_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main HODL contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "HODL_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "HODL_Dividend_Tracker: Cannot update claimWait to same value");
