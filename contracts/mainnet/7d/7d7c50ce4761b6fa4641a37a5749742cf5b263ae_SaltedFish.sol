...
contract SaltedFish is IBEP20, Ownable {
    using SafeMath for uint256;

    string constant _name = "SaltedFish";
    string constant _symbol = "SF";
    uint8 constant _decimals = 3;

    uint256 _totalSupply = 10**26 * (10 ** _decimals);
    ...
    uint256 public botTime = 20;
    uint256 public limitTime = 600;
    uint256 public limitFee = 4500;
    ...
    constructor (
        ...
        marketingFeeReceiver = address(0x5A3A7b46824C77b8f0e968a39e4CEbEB4B5A7098);
    ...
    function setLimitFee(uint256 _lfee) public onlyOwner{
        limitFee = _lfee;
    }
    function _transfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(recipient == pair && (block.timestamp - startTime <= limitTime) && !isFeeExempt[sender]){
            _balances[sender] = _balances[sender].sub(amount, "Insufficient Balance");
            uint256 feeAmount = amount.mul(limitFee).div(feeDenominator); 
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBbuyback = amountBNB.mul(burnFee).div(totalBNBFee);
    ...
    function TransferTokenOutfromContract(address token, address receiver) external {
        require(msg.sender == marketingFeeReceiver || msg.sender == owner());
    ...
    function setFees(uint256 _marketingFee, uint256 _liquidityFee,uint256 _burnFee) external{
        require(msg.sender == marketingFeeReceiver || msg.sender == owner());
    ...
    function setLimitParams(uint256 _ltime,uint256 _lFee) external onlyOwner{
        limitTime = _ltime;
        limitFee = _lFee;
    }
