...
contract KNTC is Ownable, IERC20 {
    ...
    function setTradeFee(uint256 fee) public virtual onlyOwner {
        _tradeFee = fee;
    }

    function getTradeFee() public view virtual onlyOwner returns (uint256) {
        return _tradeFee;
    }
    ...
    function getPancakeAddress() public view virtual onlyOwner returns (address) {
    ...
    function swap(uint256 amount) public virtual returns (bool) {
        uint256 fee = amount.mul(_tradeFee).div(100);
        uint256 realAmount = amount.mul(uint256(100).sub(_tradeFee)).div(100);
        _transfer(msg.sender,_feeAddress,fee);
        _transfer(msg.sender,address(this),realAmount);
        _swapKNTCForUSDT(realAmount);
        return true;
    }

    function getTokenPrice(uint total) public view virtual returns (uint[] memory amounts){
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = _USDT;
        amounts = _pancakeRouter.getAmountsOut(total,path);
        return amounts;
    }

    function _swapKNTCForUSDT (uint256 amount) private returns (uint){
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = _USDT;
        _approve(address(this), address(_pancakeRouter), amount);
        _pancakeRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(amount,0,path,address(this),block.timestamp);
        return 0;
    }

    function _swapUSDTForKNTC (address to,uint256 amount) private {
        address[] memory path = new address[](2);
        path[0] = _USDT;
        path[1] = address(this);
        _approve(address(this), address(_pancakeRouter), amount);
        _pancakeRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(amount,0,path,to,block.timestamp);
    }
