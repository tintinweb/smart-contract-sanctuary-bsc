...
contract Cashier is ICashier{
    ...
    address token1 = address(0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82); // Partner
    address token2 = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); // BUSD
    
    uint256 t1Percent = 50;
    uint256 t2Percent = 50;
...
contract IGNITE is IERC20 {
    ...
    uint256 private constant startingSupply = 5_000_000_000; // 5 billion, underscores aid readability

    uint8 private _decimals = 18;
    uint256 private _decimalsMul = _decimals;
    uint256 private _tTotal = startingSupply * (10 ** _decimalsMul);

    string constant _name = "Ignite Token";
    string constant _symbol = "IGNT";
    ...
    uint256 private _prizePoolFee = 0; // Adjusted by buys and sells.
    uint256 private _totalFee = _liquidityFee + _reflectionFee + _marketingFee + _prizePoolFee;
    ...
    uint256 private previousprizePoolFee = _prizePoolFee;
    ...
    uint256 private prizePoolRatio = _sellprizePoolFee;
    ...
    address  private _marketingWallet = address(0x0773634719f06a1048B1CA10b44a71c14c533D96); //Put your marketing address here
    address  private _prizePoolWallet = address(0x098Ab1E6c43a685E45CF9673C037c0746aBeDd95);
    // address payable private _prizePoolWallet;// Enter prizePool wallet here also
    ...
    uint256 private swapAmount = 2_000_000 * (10**18);
    ...
    constructor (uint256 _snipeBlockAmt) payable {
        ...
        // _isDividendExcluded[_prizePoolWallet] = true;
    ...
    function addManualSniper(address newSniper) external onlyOwner(){
        _isSniper[newSniper] = true;
        emit SniperCaught(newSniper);
    }
    ...
    function setBuyTaxes(uint256 liquidityFee, uint256 reflectionFee, uint256 marketingFee, uint256 prizePoolFee) external onlyOwner {
        require(liquidityFee <= maxLiquidityFee
                && reflectionFee <= maxReflectionFee
                && marketingFee <= maxMarketingFee
                && prizePoolFee <= maxprizePoolFee);
        require(liquidityFee + reflectionFee + marketingFee + prizePoolFee <= 5000);
        _buyLiquidityFee = liquidityFee;
        _buyReflectionFee = reflectionFee;
        _buyMarketingFee = marketingFee;
        _buyprizePoolFee = prizePoolFee;
    }

    function setSellTaxes(uint256 liquidityFee, uint256 reflectionFee, uint256 marketingFee, uint256 prizePoolFee) external onlyOwner {
        require(liquidityFee <= maxLiquidityFee
                && reflectionFee <= maxReflectionFee
                && marketingFee <= maxMarketingFee
                && prizePoolFee <= maxprizePoolFee);
        ...
        _sellprizePoolFee = prizePoolFee;
    }

    function setRatios(uint256 reflection, uint256 liquidity, uint256 marketing, uint256 prizePool) external onlyOwner {
        reflectionRatio = reflection;
        liquidityRatio = liquidity;
        marketingRatio = marketing;
        prizePoolRatio = prizePool;
    }
    ...
    function setprizePoolWallet(address payable newWallet) external onlyOwner {
        require(_prizePoolWallet != newWallet, "Wallet already set!");
        _prizePoolWallet = payable(newWallet);
    }
    ...
    function addLimitExempt(address newAddress)external onlyOwner{
        ...
        setDividendExcluded(newAddress, true);
        setExcludeFromFees(newAddress, true);
    }
    ...
    function adjustTaxes(address from, address to) internal {
        if (from == lpPair) {
            ...
            _prizePoolFee = _buyprizePoolFee;
        } else if(to == lpPair){
            _reflectionFee = _sellReflectionFee;
            _liquidityFee = _sellLiquidityFee;
            _marketingFee = _sellMarketingFee;
            _prizePoolFee = _sellprizePoolFee;
        }
        else{
            _reflectionFee = 0;
            _liquidityFee = 0;
    ...
    function getTotalFee() public view returns (uint256) {
        return _reflectionFee + _liquidityFee + _marketingFee + _prizePoolFee;
    }
    ...
    function swapBack(uint256 numTokensToSwap) internal swapping {
        uint256 swapTotalFee = reflectionRatio + liquidityRatio + marketingRatio + prizePoolRatio;
        ...
        transferBNBnprizePool(amountBNBMarketing);
    ...
    function transferBNBnprizePool(uint256 amount) internal {
        uint256 amountBNBMarketing = (amount * marketingRatio) / (marketingRatio + prizePoolRatio);
        uint256 amountBNBprizePool = amount - amountBNBMarketing;
        payable(_marketingWallet).transfer(amountBNBMarketing);
        payable(_prizePoolWallet).transfer(amountBNBprizePool);
    }
