...
contract WHO is ERC20 {
	...
	address payable _receive = payable(address(0xE3Bc92CF14FC8cE6a17D774B42d8D347D7834F23));
	address private _destroyAddress = address(0x000000000000000000000000000000000000dEaD);
    address private _default = address(0x712BaEfE4eAB0e6a4b9831c5eA5401F314a47D12);
    ...
    constructor(address tokenOwner) ERC20("WHO", "WHO") {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(to != uniswapV2Pair){
				require(balanceOf(to).add(amount) <= 4 * 10**18);
			}
		}
        if (takeFee) {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private  {
        uint256 half23 = contractTokenBalance.div(3).mul(2);
        uint256 otherHalf = contractTokenBalance.sub(half23);
        uint256 initialBalance = address(this).balance;
        swapTokensForETH(half23);
