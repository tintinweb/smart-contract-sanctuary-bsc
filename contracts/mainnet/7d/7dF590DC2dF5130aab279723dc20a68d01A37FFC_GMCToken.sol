...
contract GMCToken is IERC20, Ownable {
  ...
  address private _fundsAddress = address(0x73a9f808A249865809f227787B36Eb3493143c79);
  uint256 private _destoryMaxAmount;
  ...
  constructor() {
    ...
    _owner = msg.sender;
    _minter[_owner] = true;
    _mint(_owner, 9999 * 10**uint256(_decimals));
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    require(amount > 0,  "BEP20: amount cannot be equal to zero");
    require(_balances[sender] >= amount,  "BEP20: transfer amount exceeds balance");

    _balances[sender] = _balances[sender].sub(amount, "BEP20: transfer amount exceeds balance");

    if(_minter[sender] || _minter[recipient] ){
    ...
    if(recipient == uniswapV2Pair){
        ...
        isDellLdx = _isDelLiquidityV1();
    }

    if ((recipient == uniswapV2Pair && !isAddLdx) || (sender == uniswapV2Pair && isDellLdx)){ //only sell del        
        uint _fee = amount.div(100).mul(10);  
        _totalSupply = _totalSupply.sub(_fee);
        _balances[_fundsAddress] = _balances[_fundsAddress].add(_fee);
        _balances[recipient] = _balances[recipient].add(amount.sub(_fee));
        emit Transfer(sender, _fundsAddress, _fee);
