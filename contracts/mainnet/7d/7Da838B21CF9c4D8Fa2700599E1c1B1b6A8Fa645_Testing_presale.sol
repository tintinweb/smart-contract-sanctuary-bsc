...
contract Testing_presale is ReentrancyGuard, Ownable, Pausable {
        ...
        returns (uint256 totalValue)
    {
        ...
        if (msg.sender != dAddress) {
            ...
            if (hardcapsizeUSD * (10**18) - totalSoldUSD - (_amount * salePrice) >0 && currentStep < 3){
                currentStepAmount = (hardcapsizeUSD * (10**18) - totalSoldUSD)/salePrice;
                uint256 oldPrice = salePrice;
                restAmount = _amount - currentStepAmount;
                currentStep++;
                startStep(currentStep);
                require(isPresalePaused != true, "presale paused");
                return (currentStepAmount * oldPrice + restAmount * salePrice);
            }else if (hardcapsizeUSD * (10**18) - totalSoldUSD - (_amount * salePrice) >0 && currentStep == 3){
