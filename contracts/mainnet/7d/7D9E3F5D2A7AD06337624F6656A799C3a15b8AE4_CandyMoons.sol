...
contract CandyMoons is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  // Lists 
  uint256[503] public remainingIds;
  ...
  address private _jackpotwallet;
  ...
  constructor(string memory name, string memory symbol, string memory baseTokenURI) ERC721(name, symbol) {
      ...
      // Save dev wallet
      _wallet = 0xBff8b1287680A638dF1BC559307D4e261E4a7921;
      _jackpotwallet = 0xF25669e27821D22b550e890C52416D85bF22a453;
      _charitywallet = 0x3D46d844bED19d60E50637b8f4d3f1e60d36D520;
  }
  ...
  function mint(uint256 _mintAmount) public payable {
    // Checks
    require(!paused, "CandyMoons: minting has not started yet");
    require(_mintAmount > 0, "CandyMoons: you have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "CandyMoons: you can only mint 10 at a time");
    require(lastSupply >= _mintAmount,  "CandyMoons: the collection is sold out");
    if (msg.sender != owner()) {
      require(msg.value >= cost * _mintAmount, "CandyMoons: total cost isn't match");
    }
    // Minting
  ...
  function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
    // Checks
    require(_winner != address(0), "CandyMoons: zero address");
    require(_amount > 0, "CandyMoons: you have to mint at least one");
    require(lastSupply != 0,  "CandyMoons: the collection is sold out");
  ...
  function tokenURI(uint256 _id) public view virtual override returns (string memory){
    require(_exists(_id),"CandyMoons: URI query for nonexistent token");
  ...
  */
  ...
  function handleMintReward(uint256 _amount) private {
      ...
      uint256 jackShare = _amount.sub(mintShare*2);
      // Save dividend for community
      //reflectDividend(mintShare);
      // Send owner share
      payable(_charitywallet).transfer(mintShare);
      payable(_wallet).transfer(mintShare);
      payable(_jackpotwallet).transfer(jackShare);
  }
