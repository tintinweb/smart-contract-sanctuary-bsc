...
contract Quiz is Context, IERC20, Ownable {
    ...
    address public marketingWallet = 0xd1a864fc26eF34f30D4f4D1a0D999b42054896ed;
    address public operationWallet = 0x665c40a11cABe35c3978c895B190FA541c149E1E;
    address public devWallet = 0x4741f49BB1c6CB40Bc0202e49569f3FEa3737EFc;
    address public quizWallet = 0xadCd2071E10B3B3d24d2D7a6e24A888050549b6B;

    string private constant _name = "Elons Quiz";
    ...
    Taxes public taxes = Taxes(2, 5, 2, 2, 1, 3);
    Taxes public sellTaxes = Taxes(2, 5, 2, 2, 1, 3);
    ...
    constructor(address routerAddress) {
        ...
        _isExcludedFromFee[quizWallet] = true;
        ...
        allowedTransfer[quizWallet] = true;
    ...
    function setTaxes(
        ...
        taxes = Taxes(_rfi, _marketing, _liquidity, _operation, _dev, _quiz);
        require((_rfi + _marketing + _liquidity + _operation + _dev + _quiz) <= 20, "Must keep fees at 25% or less");
        emit FeesChanged();
    }
    ...
    function setSellTaxes(
        ...
        sellTaxes = Taxes(_rfi, _marketing, _liquidity, _operation, _dev, _quiz);
        require((_rfi + _marketing + _liquidity + _operation + _dev + _quiz) <= 20, "Must keep fees at 25% or less");
        emit FeesChanged();
    }
    ...
    function _takeQuiz(uint256 rQuiz, uint256 tQuiz) private {
        totFeesPaid.quiz += tQuiz;

        if (_isExcluded[address(this)]) {
            _tOwned[address(this)] += tQuiz;
        }
        _rOwned[address(this)] += rQuiz;
    }
    ...
    function _getValues(
        ...
        (to_return.rOperation, to_return.rDev, to_return.rQuiz) = _getRValues2(
            to_return,
            takeFee,
            _getRate()
        );
        ...
        )
    {
        ...
        uint256 rQuiz = s.tQuiz * currentRate;
        rTransferAmount =
            rAmount -
            rRfi -
            rMarketing -
        ...
        )
    {
        ...
        rQuiz = s.tQuiz * currentRate;
        return (rOperation, rDev, rQuiz);
    }
    ...
    function _tokenTransfer(
        ...
        if (s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(
                sender,
                address(this),
                s.tLiquidity + s.tMarketing + s.tOperation + s.tDev + s.tQuiz
            );
        }
        if (s.rMarketing > 0 || s.tMarketing > 0) _takeMarketing(s.rMarketing, s.tMarketing);
        if (s.rDev > 0 || s.tDev > 0) _takeDev(s.rDev, s.tDev);
        if (s.rQuiz > 0 || s.tQuiz > 0) _takeQuiz(s.rQuiz, s.tQuiz);
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap {
        ...
        uint256 quizAmt = unitBalance * 2 * temp.quiz;
        if (quizAmt > 0) {
            payable(quizWallet).sendValue(quizAmt);
        }
    }
    ...
    function updateQuizWallet(address newWallet) external onlyOwner {
        quizWallet = newWallet;
    }
