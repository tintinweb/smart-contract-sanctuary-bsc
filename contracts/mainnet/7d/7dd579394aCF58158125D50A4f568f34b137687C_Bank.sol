...
contract Bank is Context, ReentrancyGuard, AccessControlRegistry {
   ...
   * @param sender The destination address of the funds.
   */
  function deposit(address sender) public payable {
    uint256 amount = msg.value;
    emit Deposited(assignIndex(indexDeposited), sender, amount);
  }

  /**
   * @dev withdraw balance for a payee. equivalent amount of wraped coin should be burned on the bridging chain
   * @param from The address whose funds will be withdrawn
   * @param to The address of transferred to
  ...
  function depositNFT(
    ...
    emit NFTDeposited(assignIndex(indexNFTDeposited), token, sender, tokenid);
  }
...
contract AccessControlRegistry is Context {
  ...
  modifier onlyPermited(bytes32 role) {
    bytes32 adminRole = accessController[controlVersion].getRoleAdmin(role);
    require(
      accessController[controlVersion].hasRole(role, _msgSender()) ||
        accessController[controlVersion].hasRole(adminRole, _msgSender()),
      'no permission'
    );
    _;
  }
