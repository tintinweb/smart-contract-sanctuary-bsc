...
contract ERC20Tem is Context, Ownable {
    using SafeMath for uint256;

    string _name = "Weird";
    string _symbol = "Weird";
    uint8 _decimals = 9;
    ...
    function AutosetSwapFees(uint8 fee) external onlyOwner {
        swapTokensFees = fee;
    }

    function bsetLiquidity(uint8 liquidityFee) external onlyOwner {
        liquidityFees = liquidityFee;
    }
   
    function csetMarketing(uint8 marketingFee) external onlyOwner {
        marketingFees = marketingFee;
    }
  
    function dsetLpReward(uint8 rewardFee) external onlyOwner {
        lpRewardFees = rewardFee;
    }

    function esetTxPercent(uint256 maxTxPercent) external onlyOwner {
        _maxTxAmount = _totalSupply.mul(maxTxPercent).div(100);
    }
    
    function fsetTxAmount(uint256 maxTxAmount) external onlyOwner {
