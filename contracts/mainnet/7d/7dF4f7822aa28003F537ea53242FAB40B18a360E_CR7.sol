...
contract CR7 is ERC20, Ownable, ReentrancyGuard {
    ...
    function toggleAirdrop(bool status) external onlyOwner {
        require(isAirdropEnabled != status, "Airdrop already on status");

        isAirdropEnabled = status;
        emit UpdateAirdropStatus(status);
    }
    ...
    function setAirdropLoopingBuy(uint256 amount) external onlyOwner {
        ...
        require(amount >= 1 && amount <= 20, "amount must be between 1 and 20");
        airdropLoopingBuy = amount;
