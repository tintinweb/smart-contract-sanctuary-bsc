...
contract ACTIVITIES is Ownable {
    ...
    uint32 limit_calories = 2000;
    uint32 limit_distane = 16100;
    uint32 limit_workouts = 120;
    uint32 limit_sleep = 600;
    ...
    constructor() Ownable() {
        ...
        //STEPS
        daily_steps_ratio = 100;
        intensity_steps_ratio = 30;
    ...
    function nftStepsBonus (uint tokenId) public view returns(uint){
        ...
        if(_elem == 0){
            activity_bonus = generic_elem_bonus;
        }
        if(_elem == 4){
            activity_bonus = specific_elem_bonus;
        }
    ...
    function computeIntensityStepsShares (uint tokenId, uint t1, uint t2, uint t3, uint t4 ) public view returns ( uint256 ) {
        uint total_steps = (t1*100 + (t2*(100+steps_tiers_bonus[2])) + (t3*(100+steps_tiers_bonus[3])) + (t4*(100+steps_tiers_bonus[4])))/100;
    ...
    function stepsComputeShares (uint tokenId, uint steps, uint stepsRatio) public view returns ( uint256 ) {
        ...
        uint miningLvl = (steps/incr)*incr;
        uint nft_bonus = 100 + nftStepsBonus(tokenId);
        uint raw_shares = (miningLvl*stepsRatio)/100;
    ...
    function caloriesComputeShares (uint tokenId, uint _calories) public view returns ( uint256 ) {
        ...
        uint miningLvl = (getLimitedCalories(_calories)/incr)*incr;
        uint nft_bonus = 100 + nftCaloriesBonus(tokenId);
        uint raw_shares = miningLvl;
        uint bonus_shares = (raw_shares*nft_bonus)/100;
        uint final_shares = bonus_shares*getMiningLevelBonus(tokenId);
        return final_shares/100;
    }
    ...
    function getLimitedDistance(uint _count) internal view returns(uint){
        if(_count > limit_distane) return limit_distane;
        return _count;
    }
    function distanceComputeShares (uint tokenId, uint _distance_meters) public view returns ( uint256 ) {
        uint incr = limit_distane/20;
        uint miningLvl = (getLimitedDistance(_distance_meters)/incr)*incr;
    ...
    function workoutsComputeShares (uint tokenId, uint _time_minutes) public view returns ( uint256 ) {
        ...
        uint miningLvl = (getLimitedWorkouts(_time_minutes)/incr)*incr;
    ...
    function sleepComputeShares (uint tokenId, uint _time_minutes) public view returns ( uint256 ) {
        ...
        uint miningLvl = (getLimitedSleep(_time_minutes)/incr)*incr;
...
// File contracts/Mining/Mining.sol
...
contract MINING is Pausable, Ownable {
    ...
    mapping ( uint256 => shareData ) private  submitted;
    mapping ( uint256 => shareData ) private  claimed;
    mapping ( uint256 => submittedData[] ) private  submitterByNft;
    ...
    function submit (
        ...
        // DATA [ steps_t1, steps_t2, steps_t3, steps_t4, distance, calories,  workouts, sleep]
    ...
    function claim (string memory _tkn, uint256 tokenId) payable whenNotPaused public returns ( bool ) {
        ...
        for (uint256 index = 0; index < n; index++) {
            ...
            if(isValidDateToMine(_days)){
                ...
                airdropsCon.claimTokens(owner, ratio);
    ...
    function isValidSubmitter (uint tokenId, address owner, address device, uint date_days) public view returns (bool) {
        (address _owner, address _int_wallet,,,, uint32 _days) = stakeCon.ownerOf(tokenId);
        if(owner == _owner && device == _int_wallet && date_days >= _days){
            return true;
        }
        return false;        
    }
