...
contract StakingMapMilestoneForCardNFT is Ownable, Pausable {
    ...
    function setAvailableRewards(uint32[] calldata _cardType, uint32[] calldata _pointsCost) external onlyOwner {
        uint256 _inputLen = _cardType.length;
    ...
    // Array of reward IDs. E.g. input: [0,0,1,2,1] to get 2x0id + 2x1id + 1x2id rewards
    function receiveRewards(uint256[] calldata _rewardIds) external whenNotPaused {
        ...
        uint256 i;
        for (i = 0; i < _len; i++) {
            _totalCost += uint256(REWARDS[_rewardIds[i]].pointsCost);
        }
        ...
        // Mint reward NFTs for user
        for (i = 0; i < _len; i++) {
            REWARDS_CONTRACT.mint(_msgSender(), REWARDS[_rewardIds[i]].cardType);
        }
