...
contract MoneySaving is Ownable {
    ...
    function getRemainTime() public view onlyOwner returns (uint) {
        if (unlockTime > block.timestamp)
            return unlockTime.sub(block.timestamp);
        return 0;
    }

    function getCurrentTime() public view onlyOwner returns (uint) {
        return block.timestamp;
    }

    function getUnlockTime() public view onlyOwner returns (uint) {
        return unlockTime;
    }
