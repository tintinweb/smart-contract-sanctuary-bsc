...
contract Medals is ERC721, AccessControlEnumerable, ReentrancyGuard{
    ...
    address public gas = 0x97a8642Db64dc5aD2bA6F68dF7eD070Bb14B665B;
    address public collect = 0x84c9BAb06f5F01b302776F2E2A95756E2416A3E0;
    ...
    function init(address _gas, address _collect) external onlyRole(OPERATER_ROLE) {
        gas = _gas;
        collect = _collect;
    }
    ...
    function mint(uint256 _index) external nonReentrant {
        ...
        require(collect != address(0), "Medals: collect is zero address!");
    ...
    function _transfer(
        ...
        accountMedals[from][donateConfigs[tokenIdIndexs[tokenId]].medalId].remove(tokenId);
        accountMedals[to][donateConfigs[tokenIdIndexs[tokenId]].medalId].add(tokenId);
    ...
    function getMedalInfos(uint256[] memory tokenIds) external view returns (uint256[] memory, uint256[] memory) {
        uint256[] memory weights  = new uint256[](tokenIds.length);
        uint256[] memory medalIds = new uint256[](tokenIds.length);
        for (uint256 i = 0; i < tokenIds.length; i++) {
            medalIds[i] = donateConfigs[tokenIdIndexs[tokenIds[i]]].medalId;
            weights[i]  = donateConfigs[tokenIdIndexs[tokenIds[i]]].weight;
        }
