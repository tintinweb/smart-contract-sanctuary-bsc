...
contract WLSQToken is ERC20, Ownable {
    ...
    constructor(
        ...
        swapTokensMaxAmount = totalSupply.mul(2).div(10**4);
        weth = _WETH;
        ...
        IERC20(weth).approve(address(uniswapV2Router), MAX);

        _totalswapfee = liquidityFee.add(marketingFee).add(foundationFee.mul(2));
    ...
    function takeAllFee(address from,uint256 amount) private returns(uint256 amountAfter) {
        ...
        amountAfter = amountAfter.sub(totalLiFee);

        uint256 LPFee = amount.mul(lpRewardFee.sub(foundationFee)).div(100);
...
//        emit SwapAndLiquify(half, newBalance, otherHalf);
        ...
        uint256 amountReceived = IERC20(weth).balanceOf(address(this)) - initialBalance;

        uint256 totalFee2 = (_totalswapfee.mul(2) - liquidityFee);
        
        uint256 amountLiquidity = amountReceived.mul(liquidityFee).div(totalFee2);
        uint256 amountFoundation = amountReceived.mul(foundationFee).div(totalFee2).mul(3).div(2);
        uint256 amountMarketing = amountReceived.mul(marketingFee).div(totalFee2).mul(3).div(2);
        uint256 amount = amountReceived.sub(amountLiquidity).sub(amountFoundation).sub(marketingFee);
