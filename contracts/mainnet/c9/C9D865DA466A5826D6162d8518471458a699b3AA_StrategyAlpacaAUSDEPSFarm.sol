...
interface IZap {
  ...
  function add_liquidity(address pool, uint256[4] memory deposit_amounts, uint256 min_mint_amount) external view returns (uint256);
  function remove_liquidity_one_coin(address pool, uint256 burn_amount, int128 i, uint256 min_amount) external view returns (uint256);
}
...
contract StrategyAlpacaAUSDEPSFarm is BaseStrategy {
  ...
  uint256 private poolId = 25;        // AUSD-3EPS pool id of alpaca farm contract
  address private pool = 0xa74077EB97778F4E94D79eA60092D0F4831d05A6;    // AUSD-3EPS pool address on Ellipsis
  ...
  function _estimatedTotalAssets() internal override view returns (uint256) {
    ...
    uint256 collateralValue = collateral.mul(ibToken.totalToken()).div(ibToken.totalSupply());
    ...
    uint256 _assets = collateralValue.add(lpValue).add(claimableValue).sub(debt);
    return _assets;
  }
  ...
  function prepareReturn(uint256 _debtOutstanding) internal override returns (
    ...
    uint256 lpValue = IZap(zap).calc_withdraw_one_coin(pool, stakedBalance, 0);
    if (debt > lpValue) {
      _mintAndStakeAusd(debt.sub(lpValue));
    }
    ...
    uint256 assetBalance = collateralValue.add(wantBalance).add(lpValue).sub(debt);
  ...
  function _farm(uint256 amount) internal {
    ...
    uint256 collateralValue = collateral.mul(ibToken.totalToken()).div(ibToken.totalSupply());
    uint256 desiredCollateralValue = collateralValue.add(amount);
    uint256 desiredDebt = desiredCollateralValue.mul(collateralFactor).div(MAX_BPS);       // need to use price oracle later
  ...
  function _withdrawSome(uint256 _amount) internal {
    (uint256 collateral, uint256 debt, ) = _getCurrentPosition();
    uint256 collateralValue = collateral.mul(ibToken.totalToken()).div(ibToken.totalSupply());
    if (_amount > collateralValue) {
      _amount = collateralValue;
    }
    uint256 desiredCollateralValue = collateralValue.sub(_amount);
    uint256 desiredDebt = desiredCollateralValue.mul(collateralFactor).div(MAX_BPS);       // need to use price oracle later
  ...
  function _mintAndStakeAusd(uint256 amount) internal {
  ...
  function liquidatePosition(uint256 _amountNeeded) internal override returns (uint256 _amountFreed, uint256 _loss) {
    ...
    uint256 assets = collateral.mul(ibToken.totalToken()).div(ibToken.totalSupply());
