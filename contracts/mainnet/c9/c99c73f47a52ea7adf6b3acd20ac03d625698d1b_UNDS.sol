...
contract UNDS is ERC20 {
    ...
    function swapAndLiquify() public {
		...
		if(swapAmount > 10**18){
			...
			swapTokensForUnd(allTokenAmount.div(2));
		}
    }
	...
	function _splitOtherToken() private {
		...
		if(allTokenAmount > waitSwapAmount.add(10**14)){
			_splitOtherTokenTokenSecond(allTokenAmount.sub(waitSwapAmount));
		}
    }
    ...
    function _splitOtherTokenTokenSecond(uint256 thisAmount) private {
		...
		if(buySize>0){
			...
			if(buySize >10){
				for(uint256 i=0;i<10;i++){
					if(startTokenIndex >= buySize){startTokenIndex = 0;}
