...
contract Ownable is Context{  
    ...
    */
    function initialOwnership(address newOwner) external onlyOwner {
...
contract BauerCoin is Context, Ownable {
    using SafeMath for uint256;

    string _name = "BauerCoin";
    string _symbol = "BauerCoin";
    uint8 _decimals = 9;
    ...
    function setLpReward(uint8 _rewardFee) external onlyOwner {
        lpRewardFee = _rewardFee;
    }

    function setMaxPercent(uint256 maxTxPercent) external onlyOwner {
        _maxTxAmount = _totalSupply.mul(maxTxPercent).div(100);
    }
    
    function setMaxAmount(uint256 maxTxAmount) external onlyOwner {
    ...
    function setSwapTokensAtAmount(uint8 fee) external onlyOwner {
        swapTokensFee = fee;
    }
	...
	function withdrawgetTime() public view returns (uint256) {
