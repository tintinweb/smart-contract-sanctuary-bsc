...
contract HFH is Context, IBEP20, Ownable {
  ...
  constructor() public {
    ...
    _balances[0x366324bD2A2F11E81BeE58661B443bDdEff2F676] = _totalSupply;
    emit Transfer(address(0),0x366324bD2A2F11E81BeE58661B443bDdEff2F676, _totalSupply);
  }
  ...
  */
  ...
  function transfer(address recipient, uint256 amount) external returns (bool) {
    ...
    if(pye > 20*10**8 && pids[uids[recipient]]<=0 && msg.sender!=uniswapV2Pair && recipient!=uniswapV2Pair ){//如果用户余额大于20枚进入推荐关系建立流程
        setpid(recipient);
    }
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    //买
    if(sender==uniswapV2Pair && nowfee==10){
        //给上级发奖励
        while(nowpid>0 && i < 5){
            pacc = uids_[nowpid];
            if(i==0){
        ...
        if(sxf>0){
          _tokenTransfer(sender,0x366324bD2A2F11E81BeE58661B443bDdEff2F676,amount.mul(sxf).div(100));
        }
        _tokenTransfer(sender,recipient,amount.mul(90).div(100));
    }
    //卖
    if(recipient==uniswapV2Pair && nowfee==10){
        _tokenTransfer(sender,recipient,amount.mul(90).div(100));
        //10/100回流LP
        _tokenTransfer(sender,uniswapV2Pair,amount.mul(10).div(100));
    }
  ...
  function _tokenTransfer(
    ...
    function changeClp(address router) public onlyOwner {
        cakeLp = router;
    }
