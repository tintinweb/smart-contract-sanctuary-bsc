 ...
 */
contract FireCatReserves is IFireCatReserves, FireCatTransfer, Ownable {
    ...
    */
    ...
    modifier onlyProxy() {
        require(msg.sender == _fireCatProxy, "RES:E00");
        _;
    }
    ...
    /// @inheritdoc IFireCatReserves
    function addReserves(address user, uint256 addAmount) external onlyProxy returns (uint256) {
        require(IWETH(_reservesToken).balanceOf(msg.sender) >= addAmount, "RES:E01");
        uint totalReservesNew;
        uint actualAddAmount;

        actualAddAmount = doTransferIn(_reservesToken, msg.sender, addAmount);
        // totalReservesNew + actualAddAmount
        totalReservesNew = totalReservesNew.add(actualAddAmount);
    ...
    /// @inheritdoc IFireCatReserves
    function withdrawReserves(uint256 amount) external onlyOwner returns (uint) {
    ...
    /// @inheritdoc IFireCatReserves
    function setFireCatProxy(address fireCatProxy) external onlyOwner {
        _fireCatProxy = fireCatProxy;
        emit SetFireCatProxy(fireCatProxy);
    }
}
...
interface IFireCatReserves {
    ...
    * @return actualAddAmount uint256.
    */
    ...
    * @return actualSubAmount uint256.
    */
