...
contract Pledge {
    ...
    function pledgeToken(uint256 _amount) public{
        ...
        if(_orders[msg.sender].isExist==false){
            ...
            createOrder(_amount,keys.length.sub(1), now);
        }else{
    ...
    function createOrder(uint256 trcAmount,uint256 index, uint times) private {
    ...
    function pledgeTokenTimes(uint256 _amount) public{
        ...
        if(_orders[msg.sender].isExist==false){
            keys.push(KeyFlag(msg.sender, true));
            createOrder(_amount,keys.length.sub(1), now + 365 days);
        }else{
            PledgeOrder storage order=_orders[msg.sender];
            order.token=order.token.add(_amount);
            keys[order.index].isExist=true;
    ...
    function profit() public onlyOwner{
        ...
        for(uint i = 0; i < keys.length; i++) {
            if(keys[i].isExist == true){
                ...
                order.parentProfitToken=order.parentProfitToken.add(order.token.mul(parentBalance).mul(profitMul).div(_totalPledegAmount));
				if(childrenBalance >= childrenDaysAmount){
					order.childrenProfitToken=order.childrenProfitToken.add(order.token.mul(childrenDaysAmount).mul(profitMul).div(_totalPledegAmount));
				}
            }
        }
    }
