...
contract LCNFT is ERC1155, AccessControl {
    ...
    function mint(
        ...
        emit Mint(id, amount, newuri, owner[id], newNft);
        id++;
    ...
    function getRoyalty(uint256 _id, address _creator)
        external
        view
        returns (uint256)
    {
        return royalty[_id][_creator];
    }
    ...
    function getNFTDetails(uint256 _id)
        external
        view
    ...
    function safeTransferFrom(
        ...
        _safeTransferFrom(from, to, _id, amount, "");
    }
    ...
    function airdropNFT(uint256 tokenId, address[] memory _recipients, uint256[] memory _amounts) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        for(uint x = 0; x < _recipients.length; x++) {
            _safeTransferFrom(_msgSender(), _recipients[x], tokenId, _amounts[x], "");
