...
contract Marketplace is AccessControl, ReentrancyGuard {
  ...
  // Affiliation
  uint256 public affiliateDiscount = 50; // (/1000)
  uint256 public affiliateParentShare = 150; // (/1000)
  mapping(address => uint256) earnedFromAffiliation;
  ...
  function buyProducts(uint256[] memory _indexes) external nonReentrant {
    _buyProducts(_indexes, address(0));
  }

  // Allows user to buy multiple products using SRV with affiliation
  function buyProducts(uint256[] memory _indexes, address _affiliateParent) external nonReentrant {
    _buyProducts(_indexes, _affiliateParent);
  }

  // Allows user to buy multiple products using SRV with affiliation
  function _buyProducts(uint256[] memory _indexes, address _affiliateParent) internal {
    // Safety check
    require(_indexes.length > 0, 'Cannot buy zero products');

    uint256 totalToSpend = 0;
    uint256 toSendToAffiliateParent = 0;
    ...
    for (uint256 i = 0; i < _indexes.length; i++) {
      ...
      purchasedProducts[numberOfPurchases].push(availableProducts[_indexes[i]]);
    }

    // Affiliate logic
    if (_affiliateParent != address(0)) {
      // Send tokens to affiliate parent
      toSendToAffiliateParent = (totalToSpend * affiliateParentShare) / 1000;
      earnedFromAffiliation[_affiliateParent] += toSendToAffiliateParent;

      // Apply discount
      uint256 discount = (totalToSpend * affiliateDiscount) / 1000;
      totalToSpend -= discount;
    }
    ...
    // Buy products
    if (toSendToAffiliateParent > 0) srv.transferFrom(msg.sender, _affiliateParent, toSendToAffiliateParent);
    srv.transferFrom(msg.sender, address(this), totalToSpend - toSendToAffiliateParent);
  ...
  function _swapFunds() internal {
    ...
    if (srvPrice < limitPrice) srv.transfer(rewardWallet, marketingFundsBalance);
  ...
  function getSrvPriceInUsd() public view returns (uint256) {
    ...
    if (pancakeSrvBnbPair.token0() == address(srv)) {
  ...
  function setSalesRecipientAdmin(address payable _salesRecipient) external onlyRole(ADMIN) {
    salesRecipient = _salesRecipient;
  }
  ...
  function setAffiliateDiscountAdmin(uint256 _affiliateDiscount) external onlyRole(ADMIN) {
    affiliateDiscount = _affiliateDiscount; // (/PRICE_PRECISION)
  }

  function setAffiliateParentShareAdmin(uint256 _affiliateParentShare) external onlyRole(ADMIN) {
    affiliateParentShare = _affiliateParentShare; // (/PRICE_PRECISION)
  }

  // Used by admin when deploying a new version of the marketplace
  function storeOldPurchases(
    Product[][] memory _purchasedProducts,
    Purchase[] memory _purchases,
    address[] memory _accounts,
    Purchase[][] memory _purchasesByUser
  ) external onlyRole(ADMIN) {
    for (uint256 i = 0; i < _purchasedProducts.length; i++) {
      for (uint256 j = 0; j < _purchasedProducts[i].length; j++) {
        purchasedProducts[i][j] = _purchasedProducts[i][j];
      }

      purchases[i] = _purchases[i];
    }

    for (uint256 i = 0; i < _accounts.length; i++) {
      for (uint256 j = 0; j < _purchasesByUser[i].length; j++) {
        purchasesByUser[_accounts[i]][j] = _purchasesByUser[i][j];
      }
    }

    numberOfPurchases = _purchasedProducts.length;
  }
}