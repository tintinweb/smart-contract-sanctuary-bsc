...
contract Flashswap {
    ...
    function start(
        ...
        uint256 _tokenBorrowAmount, // example: BNB => 10 * 1e18
        address _tokenPay, // our profit and what we will get; example BNB
        address _sourceRouter,
        address _targetRouter,
        address pairAddress, // from source router
        address token0,
        address token1
    ) external {
    ...
    function execute(address _sender, uint256 _amount0, uint256 _amount1, bytes calldata _data) internal {
        ...
        amountToken   = _amount0 == 0 ? _amount1 : _amount0;
        ...
        otherToken = IERC20(_amount0 == 0 ? token0 : token1);
        token = IERC20(_amount0 == 0 ? token1 : token0);
        ...
        uint256 realAmount = token.balanceOf(address(this));
        token.approve(targetRouter, realAmount);

        // calculate the amount of token how much input token should be reimbursed
        uint256 amountRequired = IUniswapV2Router(sourceRouter).getAmountsIn(realAmount, path1)[0];

        require(address(token)==path[0], 'approve wrong');
