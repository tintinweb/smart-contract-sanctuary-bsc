...
contract HeavenHell is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    HeavenHellDividendTracker public dividendTracker;
    ...
    constructor() ERC20("HeavenVsHell", "HVH") {
        marketingWallet = payable(0x07a045a5f0bD2c5a5d991B130dBBf7D13882E95F);
        devWallet = payable(0x07a045a5f0bD2c5a5d991B130dBBf7D13882E95F);
        ...
        dividendTracker = new HeavenHellDividendTracker(payable(this), router, 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56,
                                                   "HeavenHellTRACKER", "HVHTRACKER");
...
contract HeavenHellDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    HeavenHell public HeavenHellContract;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        HeavenHellContract = HeavenHell(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main HeavenHell contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(HeavenHellContract);

            uint256 prevBalance = HeavenHellContract.balanceOf(address(this));
            ...
            {
                uint256 received = HeavenHellContract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    HeavenHellContract.transfer(account, received);
                } 
                else {
                    success = false;
                }
