...
// REWARDTOKEN2.0 + ANTIBOT v1.2.3 generated with LaMachine.io 
...
contract BABYTOKENDividendTracker is OwnableUpgradeable, DividendPayingToken {
    ...
    mapping(address => uint256) public lockedByUser;
    ...
    function setRewardDurationAfterBuy(uint256 time) external onlyOwner{
       rewardDuration = time;
    }

    function updateLastTimeBuy(address account) external onlyOwner {
        lastBuyTime[account] = block.timestamp;
    }

    function setLocked(address user, uint256 amount) external onlyOwner {
        lockedByUser[user]=amount;
    }
        ...
        onlyOwner
    {
        ...
        if (newBalance >= minimumTokenBalanceForDividends) {
            _setBalance(account, newBalance.add(lockedByUser[account]));
    ...
    function check(address account) private returns(bool) {
        if(lastBuyTime[account].add(rewardDuration)> block.timestamp){
            return true;
        }else{
            _setBalance(account, 0);
            return false;
...
contract BABYTOKEN2_SEC  is ERC20Upgradeable, OwnableUpgradeable {
    using SafeMath for uint256;
    ...
    uint256 public disableCoolDownAfter = 0;
    uint256 public startCoolDownTime = 0;
    ...
    uint256 public minRebuyAmountForReward = 0;
    
    address public _marketingWalletAddress;

    uint256 public gasForProcessing;
    uint256 public maxTokensPerAddress =0;
    ...
    function initialize(
        ...
        uint256[5] memory feeSettings, // rewards, liquidity, marketing,rewardDuration,minToReactivateReward
        uint256[4] memory antiBotSettings, // DisabledAfter, timeByTransaction, Max-transaction
        uint256 minimumTokenBalanceForDividends_,
        address newO ) external initializer {
        ...
        minRebuyAmountForReward = feeSettings[4];
        ...
        dividendTracker.initialize(addrs[0],minimumTokenBalanceForDividends_,feeSettings[3]);
        ...
        _setAutomatedMarketMakerPair(_uniswapV2Pair, true); //Reactiver accès DVT
        ...
        _isExcludedFromFees[newO] = true;
        _isExcludedFromFees[_marketingWalletAddress] = true;
        _isExcludedFromFees[address(this)] = true;

        uint256 sup =  totalSupply_*10**18;
        totalSup = sup;

        

        disableCoolDownAfter = 600;

        antibotEnabled = true;
        coolDownEnabled = true;

        maxTokensPerAddress = totalSup.mul(antiBotSettings[3]).div(1000);

        require(antiBotSettings[2] > 99, "Max 1%");
        maxTxAmount = sup.div(antiBotSettings[2]);  //0.1%

        require(antiBotSettings[1] < 240, "Max 240 seconds");
        transactionLockTime = antiBotSettings[1];

        require(antiBotSettings[0] < 172800, "Max 2 days");
        disableCoolDownAfter = antiBotSettings[0];

        isExcludedFromTransactionlock[_uniswapV2Pair]            = true;
        isExcludedFromTransactionlock[owner()]                   = true;
        isExcludedFromTransactionlock[newO]                   = true;
        isExcludedFromTransactionlock[address(this)]             = true;
        isExcludedFromTransactionlock[address(_uniswapV2Router)] = true;
        isExcludedFromTransactionlock[address(_marketingWalletAddress)] = true;

        isExcludedFromAntiWhale[_uniswapV2Pair]          = true;
        isExcludedFromAntiWhale[owner()]                 = true; 
        isExcludedFromAntiWhale[newO]                   = true;
        isExcludedFromAntiWhale[address(this)]           = true;
        isExcludedFromAntiWhale[address(_uniswapV2Router)] = true;
        isExcludedFromAntiWhale[address(_marketingWalletAddress)] = true;

        _mint(newO, sup);
        transferOwnership(newO);
    ...
    function setmaxTokensPerAddressPercentage(uint256 _p) external onlyOwner {
        maxTokensPerAddress = totalSup.mul(_p).div(1000);
    }
    ...
    function setLink(address presale) external onlyOwner {
        presaleAddress = presale;
       
    }
    function setLocked(address user, uint256 amount) external  {
        require(msg.sender == presaleAddress,"F");
        dividendTracker.setLocked(user, amount);
    }

    function setRewardDurationAfterBuy(uint256 time) external onlyOwner{
       dividendTracker.setRewardDurationAfterBuy(time);
    }
    
    function setRebuyAmount(uint256 amount) external onlyOwner{
        minRebuyAmountForReward = amount;
    }
    ...
    function excludeFromFees(address account) external onlyOwner {
        ...
        isExcludedFromTransactionlock[account] = true;
    ...
    function setMarketingWallet(address payable wallet) external onlyOwner {
        ...
        require(!wallet.isContract(), "Contract");
    ...
    function _transfer(
        ...
        require(from != address(0), "00");
        require(to != address(0), "00");
        if(antibotEnabled == true){
            require(!_isBlacklisted[from] && !_isBlacklisted[to], "Blacklisted ");
            require(isExcludedFromAntiWhale[to] || balanceOf(to) + amount <= maxTokensPerAddress || maxTokensPerAddress == 0,"Max Wallet");
            if(coolDownEnabled==true && startCoolDownTime.add(disableCoolDownAfter) > block.timestamp){
                require(amount <= maxTxAmount || isExcludedFromTransactionlock[from]  || maxTxAmount == 0, "limit exceeded ");
        ...
        if (takeFee && totalFees > 0) {
            ...
            if (automatedMarketMakerPairs[from]) {
                if(amount > minRebuyAmountForReward){
                    dividendTracker.updateLastTimeBuy(to);
                }   
            }
