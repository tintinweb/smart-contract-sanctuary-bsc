...
contract DRL is BEP20, Auth {
    ...
    function update_fees() internal {
        if(!launchMode){
            require(totalFee.mul(buyMultiplier).div(100) <= 100, "Buy tax cannot be more than 90%");
            require(totalFee.mul(sellMultiplier).div(100) <= 150, "Sell tax cannot be more than 90%");
