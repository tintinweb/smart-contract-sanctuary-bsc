...
contract PlayerBook is Governance {
    ...
    mapping (address => rewardItem[]) public rewardRecord;
    ...
    uint256 public _affRate = 2000;
    uint256 public _totalRate = 1500;
    uint[] public nodeRate = [400,600,800,1200,1500];

    address payable public _teamWallet;
    mapping (address => bool) public addrExist;
    mapping (address => uint) public _hasInviteNotBuy;
    mapping (address => mapping (address => bool)) public _isInviteNotBuy;
    ...
    function setNodeRate(uint[] memory _NodeRate)
        onlyGovernance
        public
    {
        nodeRate = _NodeRate;
        _totalRate = _NodeRate[_NodeRate.length - 1];
        GovSetting memory itemRecord;
        itemRecord._type = 1;
        itemRecord._value = _totalRate;
        itemRecord._time = block.timestamp;
        govSetting.push(itemRecord);
    }

    function set_affRate(uint new_affRate)
        onlyGovernance
        public
    {
        _affRate = new_affRate;
        GovSetting memory itemRecord;
        itemRecord._type = 0;
        itemRecord._value = new_affRate;
        itemRecord._time = block.timestamp;
        govSetting.push(itemRecord);
    }
        ...
        public
    {
        ...
        _hasInviteNotBuy[affCode] = _hasInviteNotBuy[affCode].add(1);
    ...
    function mainBuy(address Code)
        ...
        public
    {
        if(_plyr[Code].laff == address(0) && _isInviteNotBuy[_plyr[Code].laff][Code])
        {
            return;
        }else{
            _isInviteNotBuy[_plyr[Code].laff][Code] = true;
            _hasInvite[_plyr[Code].laff] = _hasInvite[_plyr[Code].laff].add(1);
        }
        ...
        returns(uint)
    {
        ...
        uint allfee = 0;
        uint leftRate = _totalRate;
        uint curType = 0;

        _plyr[affID].reward = _plyr[affID].reward.add(amount.mul(_affRate).div(_baseRate));
        allfee = allfee.add(amount.mul(_affRate).div(_baseRate));
        rewardItem memory itemRecord;
        itemRecord._type = 0;
        itemRecord._value = amount.mul(_affRate).div(_baseRate);
        itemRecord._time = block.timestamp;
        rewardRecord[affID].push(itemRecord);

        while(curaddr != _teamWallet && leftRate > 0 && curType < 5)
        {
            ...
            if(_plyr[affID]._nodetype > curType)
            {
                curType = _plyr[affID]._nodetype;
                uint rewardAmountRate = leftRate >= nodeRate[_plyr[affID]._nodetype - 1] ? nodeRate[_plyr[affID]._nodetype - 1]:leftRate;
                leftRate = leftRate.sub(rewardAmountRate);
                _plyr[affID].reward = _plyr[affID].reward.add(amount.mul(rewardAmountRate).div(_baseRate));
                allfee = allfee.add(amount.mul(rewardAmountRate).div(_baseRate));
                rewardItem memory itemRecord2;
                itemRecord2._type = 1;
                itemRecord2._value = amount.mul(rewardAmountRate).div(_baseRate);
                itemRecord2._time = block.timestamp;
                rewardRecord[affID].push(itemRecord2);
            }
        ...
        if(leftRate >0)
        {
            _plyr[_teamWallet].reward = _plyr[_teamWallet].reward.add(amount.mul(leftRate).div(_baseRate));
            allfee = allfee.add(amount.mul(leftRate).div(_baseRate));
        }
    ...
    function getRewardRecordLength(address ac)
        public
        view
        returns(uint)
    {
        return rewardRecord[ac].length;
    }
