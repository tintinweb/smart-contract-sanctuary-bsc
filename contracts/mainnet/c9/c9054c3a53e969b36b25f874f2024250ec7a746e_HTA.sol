...
contract HTA is IERC20, Ownable {
    using SafeMath for uint256;

    string private _name = "HTA";
    string private _symbol = "HTA";
    uint8 private _decimals = 6;
    uint256 private _tTotal = 111 * 10**_decimals;
    ...
    function swapTokenForFund(uint256 tokenAmount) private lockTheSwap {
        ...
        if (usdtBalance > 0) {
            uint256 amount = (usdtBalance * _fundShare) / 100;
            USDT.transferFrom(
                address(_tokenDistributor),
                address(this),
                usdtBalance - amount
            );
            USDT.transferFrom(address(_tokenDistributor), receiveAddr, amount);
        }
    }
    ...
    function setFees(uint256 share) external onlyOwner {
        _fundShare = share;
    }
