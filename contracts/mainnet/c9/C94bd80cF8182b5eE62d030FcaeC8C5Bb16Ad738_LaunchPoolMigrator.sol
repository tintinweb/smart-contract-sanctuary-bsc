...
contract LaunchPoolMigrator is Context, PausableByOwner, RecoverableByOwner {
  ...
  function addAllowedAddresses(address[] calldata allowedAddresses) external onlyOwner {
    for (uint256 i = 0; i < allowedAddresses.length; i++) {
      ...
      isAllowedAddress[address(allowedAddresses[i])] = true;
    }
  }
  ...
  function removeAllowedAddresses(address[] calldata allowedAddresses) external onlyOwner  {
    for (uint256 i = 0; i < allowedAddresses.length; i++) {
      ...
      isAllowedAddress[address(allowedAddresses[i])] = false;
    }
  }

  function migrate(MigratePayload calldata payload) external whenNotPaused {
    require(isAllowedAddress[payload.migrationAddress], "LaunchPoolMigrator: INVALID_MIGRATION_ADDRESS");
    ...
    IStakingService(address(payload.migrationAddress)).stakeFor(_msgSender(), payload.unstakeSignedAmount);

    emit Migrate(_msgSender(), payload.unstakeSignedAmount, 0, 0, payload.unstakeSignedAmount, 0, 0);
  }
