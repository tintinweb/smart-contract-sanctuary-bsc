...
//   uint public InterestPercent = 43200;// in Hours ;
  ...
  constructor() {  }
    ...
    function stake(uint256 token) public returns(uint256 optionId){
        ...
        options[msg.sender].push(_options(msg.sender,token,token,InterestPercent,InterestFeePercent,block.timestamp,0,0,true,false));
    ...
    function calculateReward(address user, uint256 optionId) public view returns(uint256 interestAmount) {
        if(options[user][optionId].isActive){
             uint256 TotalAmount = options[user][optionId].mainAmount;
             uint256 timeUpdate = (options[user][optionId].isCapitalWithdraw)? options[user][optionId].capitalWithdrawTime : options[user][optionId].stakeTime; 
             uint256 rewardCount = uint(block.timestamp.sub(timeUpdate)/duration);
    ...
    function withdrawCapital(uint256 optionId) public returns(bool) {
        ...
        options[msg.sender][optionId].mainAmount = calculateReward(msg.sender,optionId);
    ...
    function claimReward(uint256 optionId) public  returns(bool){
        require(options[msg.sender][optionId].isActive, "Invalid Option ID");
        require(options[msg.sender][optionId].isCapitalWithdraw, "Pool Error : You need to withdraw the capital amount first");
        uint256 reward = options[msg.sender][optionId].mainAmount.add(calculateReward(msg.sender,optionId));
        uint256 fee = reward.mul(options[msg.sender][optionId].interestFeePercent).div(100000);
        uint256 rewardToken = reward.sub(fee);
        ...
        options[msg.sender][optionId].mainAmount = rewardToken;
    ...
    function setInterestPercent(uint256 _InterestPercent) public onlyOwner {
