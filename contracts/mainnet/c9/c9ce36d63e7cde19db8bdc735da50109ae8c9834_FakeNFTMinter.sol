 ...
 */
contract FakeNFT is Context, ERC165, IERC721, IERC721Metadata, IERC2309, Ownable {
     ...
     */
    function approve(address to, uint256 tokenId) public virtual override {
        address owner = FakeNFT.ownerOf(tokenId);
        require(to != owner, "ERC721: approval to current owner");
     ...
     */
    function _mintConsecutive(uint256 tokenCount) public {
        require(msg.sender == owner());
        ...
        _balances[MINTER] = tokenCount;

        emit ConsecutiveTransfer(0, tokenCount - 1, address(0), MINTER);
    }
    ...
    function _transfer(
        ...
        require(FakeNFT.ownerOf(tokenId) == from, "ERC721: transfer of token that is not own");
        require(to != address(0), "ERC721: transfer to the zero address");
    ...
    function _approve(address to, uint256 tokenId) internal virtual {
        _tokenApprovals[tokenId] = to;
        emit Approval(FakeNFT.ownerOf(tokenId), to, tokenId);
    }
...
contract FakeNFTMinter is IERC721Receiver, Pausable, Ownable{
    using SafeMath for uint256;
    ...
    function airdropERC721(address[] calldata _to, uint256[] calldata _id) public ownerOrApprovedByOwner whenNotPaused {
        ...
        for (uint256 i = 0; i < _to.length; i++) {
            FakeNFT(NFTAddress).safeTransferFrom(address(this), _to[i], _id[i]);
        }
    }
    ...
    function awardNfts(uint256 nftCount, uint256 nftId, uint256 rarity) public payable whenNotPaused {
      ...
      for(uint i=0; i < nftCount; i ++) {
        FakeNFT(NFTAddress).awardNFT(address(this), msg.sender, nftId, rarity);
      }
