...
abstract contract BakeryAdapter {
    ...
    // Pool bits are 255-161: fee, 160: direction flag, 159-0: address
    uint256 private constant _FEE_OFFSET = 161;
    uint256 private constant _DIRECTION_FLAG = 0x0000000000000000000000010000000000000000000000000000000000000000;
    ...
    function swapOnBakery(
        ...
        _swapOnBakery(address(fromToken), fromAmount, data.weth, data.pools);
    }
    ...
    function _swapOnBakery(
        ...
        if (tokenIn == Utils.ethAddress()) {
            IWETH(weth).deposit{ value: amountIn }();
            require(IWETH(weth).transfer(address(pools[0]), amountIn), "!transfer");
        } else {
            TransferHelper.safeTransfer(tokenIn, address(pools[0]), amountIn);
            tokensBoughtEth = weth != address(0);
        }
        ...
        for (uint256 i = 0; i < pairs; ++i) {
            ...
            tokensBought = NewUniswapV2Lib.getAmountOut(tokensBought, pool, direction, p >> _FEE_OFFSET);
            (uint256 amount0Out, uint256 amount1Out) = direction
                ? (uint256(0), tokensBought)
                : (tokensBought, uint256(0));
            IBakeryPair(pool).swap(amount0Out, amount1Out, i + 1 == pairs ? address(this) : address(pools[i + 1]));
        }
