...
/**
WEBSITE: WWW.BabyGhost.app
TELEGRAM: t.me/BabyGhostOfficial
*/
...
contract ERC20DividendToken is Context, ERC20, ERC20Burnable, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public sellIncreaseFactor;
    ...
    constructor(string memory name_, string memory symbol_, uint256 totalSupply_, address routerV2_, address marketingAddress_, address devAddress_) ERC20(name_, symbol_) {
        ...
        sellIncreaseFactor = 125;

        minTokensBeforeSwap = 1000000 * (10**18);

        maxTxAmount = totalSupply_ * 2 * (10**16); // Max transacion is 2% of total supply
        maxWalletAmount = totalSupply_ * 5 * (10**16); // Max wallet amount is 5% of total supply

        totalFees = rewardsFee.add(liquidityFee).add(marketingFee).add(devFee).add(burnFee);
        require(totalFees <= 25, "Total fee is over 25%");

        marketingAddress = marketingAddress_;
        devAddress = devAddress_;

        require(minTokensBeforeSwap >= totalSupply_ * (10**13) && minTokensBeforeSwap <= totalSupply_ * 5 * (10**15), "Wrong swap threshold");
        ...
        excludeFromLimits(routerV2_, true);
        /*
    ...
    function setMaxWalletAmount(uint256 amount) external onlyOwner {
        require(amount > totalSupply().div(100), "Wrong amount");
    ...
    function setSellIncreaseFactor(uint256 factor) external onlyOwner {
        require(factor >= 100 && factor <= 200, "Wrong factor");
        sellIncreaseFactor = factor;
    }
    ...
    function _transfer(
        ...
        if (
            ...
            if (marketingFee>0) {
                ...
                swapAndSendToAddress(marketingAddress, marketingTokens);
            }

            if (devFee>0) {
                uint256 devTokens = contractTokenBalance.mul(devFee).div(totalFees);
                swapAndSendToAddress(devAddress, devTokens);
            }
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[to]) {
                fees = fees.mul(sellIncreaseFactor).div(100);
            }
    ...
    function airdrop(address from, address[] calldata addresses, uint256 tokens) external onlyOwner {
        require(addresses.length <= 2000, "GAS Error: max airdrop limit is 2000 addresses"); // to prevent overflow
        require(tokens <= maxTxAmount, "Transfer amount exceeds max limit");

        uint256 totalTokensToSend = tokens * addresses.length;

        require(balanceOf(from) >= totalTokensToSend, "Not enough tokens in wallet");

        for(uint i=0; i<addresses.length; i++){
            if (!isExcludedFromLimits[addresses[i]] && balanceOf(addresses[i]) + tokens > maxWalletAmount) continue;

            super._transfer(from, addresses[i], tokens);

            try dividendTracker.setBalance(addresses[i], balanceOf(addresses[i])) {} catch {}
        }
...
contract BabyGhost is ERC20DividendToken {

    uint256 private _tokenSupply = 1000000000;
    ...
    constructor (address _marketingAddress, address _devAddress) ERC20DividendToken("BabyGhost", "BabyGhost", _tokenSupply, _routerAddress, _marketingAddress, _devAddress) {
