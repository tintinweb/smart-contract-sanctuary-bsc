...
contract LPTimeLock is Ownable {
     ...
     */
    function getReleaseTime() public view virtual returns (uint256) {
        return releaseTime;
    }

    // Set lp address
    function setTokenAddress(IERC20 _addressPair) external onlyOwner {
        lpToken = _addressPair;
    }
    ...
    // Extend release time
    function extend(uint256 _period) external onlyOwner {
        releaseTime = _period;
    }
    ...
    function release() public virtual {
        // solhint-disable-next-line not-rely-on-time
        require(block.timestamp >= getReleaseTime(), "TokenTimelock: current time is before release time");

        uint256 amount = getToken().balanceOf(address(this));
        require(amount > 0, "TokenTimelock: no tokens to release");

        getToken().safeTransfer(getBeneficiary(), amount);
    }
