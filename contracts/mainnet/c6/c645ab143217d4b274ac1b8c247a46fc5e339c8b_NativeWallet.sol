...
contract NativeWallet is IERC20, Context, Ownable {
    ...
    address private Native__Wallet__Marketing_Wallet = 0x6ea1be6e1a78c58A77ac5f0A18083b09fe1D1CC0;
    address private Native__Wallet__Team_Wallet = 0x5AE93D46fa336276C728914BEf32c593DF6D15F6;
    
    string private constant _symbol = "NAT";
    string private constant _name = "Native Wallet";

    uint256 public Native__Wallet__max_BuyLimit = 2000000 * 10**9;
    uint256 public Native__Wallet__max_SellLimit = 500000 * 10**9;

    uint256 public Native__Wallet__max_WalletLimit = 2000000  * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[Native__Wallet__Marketing_Wallet] = true;
        _isExcludedFromFee[Native__Wallet__Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[Native__Wallet__Marketing_Wallet] = true;
        allowedTransfer[Native__Wallet__Team_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= Native__Wallet__max_BuyLimit, "You are exceeding Native__Wallet__max_BuyLimit");
            require(balanceOf(to) + amount <= Native__Wallet__max_WalletLimit, "You are exceeding Native__Wallet__max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= Native__Wallet__max_SellLimit, "You are exceeding Native__Wallet__max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= Native__Wallet__max_WalletLimit, "You are exceeding Native__Wallet__max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(Native__Wallet__Marketing_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(Native__Wallet__Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function Native__Wallet__Update_MarketingWallet(address newWallet) external onlyOwner{
        Native__Wallet__Marketing_Wallet = newWallet;
    }
    
    function Native__Wallet__Update_TeamWallet(address newWallet) external onlyOwner{
        Native__Wallet__Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        Native__Wallet__max_BuyLimit = maxBuy * 10**decimals();
        Native__Wallet__max_SellLimit = maxSell * 10**decimals();
    }
    
    function Native__Wallet__Max_Walletlimit(uint256 amount) external onlyOwner{
        Native__Wallet__max_WalletLimit = amount * 10**decimals();
    }
