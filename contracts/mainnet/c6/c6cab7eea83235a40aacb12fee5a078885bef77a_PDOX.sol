...
contract DividendPayingToken is BEP20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
 ...
 function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
    ...
    if (_withdrawableDividend > 0) {
      ...
      bool success = IBEP20(BNB).transfer(user, _withdrawableDividend);
...
contract PDOX is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    PDOXDividendTracker public dividendTracker;
    ...
    uint256 public swapTokensAtAmount = 100 * (10**18); //50 millions. 
    ...
    address payable public _marketingWalletAddress = payable(0xDE31ce854CaBc9FAB1e58F0e28E0f40d0d641eE1);
    address payable public _buybackWalletAddress = payable(0x4E17CbBf077c1f4B692a938d2C12936E0D41f2Ec);
    ...
    constructor() BEP20("PDOX", "PDOX") 
    {
        dividendTracker = new PDOXDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "PDOX: The dividend tracker already has that address");
        PDOXDividendTracker newDividendTracker = PDOXDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "PDOX: The new dividend tracker must be owned by the PDOX token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "PDOX: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "PDOX: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "PDOX: The PanBNBSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "PDOX: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "PDOX: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "PDOX: Cannot update gasForProcessing to same value");
    ...
    function _transfer(address from, address to, uint256 amount) internal open(from, to) override 
    {
        ...
        !automatedMarketMakerPairs[from] && from != owner()) 
        {
            ...
            uint256 swapableFees = totalFees.sub(BNBRewardsFee); // marketingFee+buybackFee+liquidityFee.
    ...
    {
       uint256 swapableFee = totalFees.sub(BNBRewardsFee);
    ...
    function swapAndSendDividends(uint256 tokens) private{
        ...
        uint256 dividends = IBEP20(BNB).balanceOf(address(this));
        bool success = IBEP20(BNB).transfer(address(dividendTracker), dividends);
...
contract PDOXDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("PDOX_Dividend_Tracker", "PDOX_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "PDOX_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "PDOX_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PDOX contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "PDOX_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "PDOX_Dividend_Tracker: Cannot update claimWait to same value");
