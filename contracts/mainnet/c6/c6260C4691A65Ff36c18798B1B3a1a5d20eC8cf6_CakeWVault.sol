...
// File: contracts/farms/CakeWSwap/IMasterChef.sol
...
// File: contracts/farms/CakeWSwap/CakeWVault.sol


pragma solidity 0.6.12;

contract CakeWVault is Ownable, Pausable {
    ...
    struct UserInfo {
        ...
        uint256 cakeWAtLastUserAction; // keeps track of CAKEW deposited at the last user action
        uint256 lastUserActionTime; // keeps track of the last user action time
    }

    IBEP20 public immutable token; // CAKEW token
    ...
    /**
     * @notice Constructor
     * @param _token: CAKEW token contract
    ...
    /**
     * @notice Deposits funds into the CAKEW Vault
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in CAKEW)
     */
    function deposit(uint256 _amount) external whenNotPaused notContract {
        ...
        user.cakeWAtLastUserAction = user.shares.mul(balanceOf()).div(totalShares);
    ...
    /**
     * @notice Reinvests CAKEW tokens into MasterChef
     * @dev Only possible when contract not paused.
     */
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the CakeWVault
     */
     ...
     * @return Expected reward to collect in CAKEW
     */
    function calculateHarvestCakeWRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingCakeW(poolId, address(this));
     ...
     * @return Returns total pending CAKEW rewards
     */
    function calculateTotalPendingCakeWRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingCakeW(poolId, address(this));
        amount = amount.add(available());
    ...
    /**
     * @notice Withdraws from funds from the CakeWVault
