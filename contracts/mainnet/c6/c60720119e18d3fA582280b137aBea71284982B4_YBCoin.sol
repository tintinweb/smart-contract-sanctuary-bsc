...
interface ControlAutonomy {
    function getYBC_Control_status4(address _address) external view returns (bool);

    function getYBC_Control_status1() external view returns (bool);

    function getYBC_Control_status2() external view returns (bool);
}
...
contract Recv {
    ...
    constructor (IERC20 _tokenYBC) {
        ...
        //usdt = IERC20(0x803a3e2bfC656d971bFA0dbF4353BaBD9D4D6d5e);
    }
...
contract YBCoin is ERC20Burnable {
    ...
    address public _YBC_LP_USDT_address;//YBC LP鍦板潃
    IPancakeRouter02 public _mdx_V2Router02;
    IPancakePair public _mdx_V2Pair;

    uint256 private _minSwapNumber = 1 * 10 ** 18;//鏈€鍏戞崲鏁伴噺
    uint256 private _reNumber = 1 * 10 ** 17;//鎺ㄨ崘閿佺矇鏁伴噺
    uint256 private _maxSupply = 210000000 * 10 ** 18;//鏈€澶у彂琛岄噺
    uint256 private _remainSwapNumber;//鍓╀綑鍏戞崲鏁伴噺
    uint public _minAddLiquidityNumber = 1 * 10 ** 18;

    bool private _is_not_slippage = true;//鏄惁鍏佽婊戠偣
    ...
    uint256 private _part_rate = 10;//鍒嗘垚姣斾緥
     ...
     * See {ERC20-transfer}.
     * (1) 婊戠偣:6%锛屼拱鍏ュ崠鍑洪兘鏄?6%
     * (2) 閿€姣?1%
     * (3) NFT:1%
     * (4) 搴曟睜鍥炴祦:2%
     * (5) DKF-LP 鍒嗙孩:1%锛圲SDT锛?     * (6) 鎺ㄨ崘浜烘嬁鍗佸眰:1%锛圲SDT锛?     * 澧炲姞杞竵閿佺矇鍔熻兘锛屽彧瑕佹帴鏀跺湴鍧€鐨剏bc涓洪浂灏卞紑濮嬬粦瀹氬叧绯?     */
    ...
    function _can_trade(address _user_address) public view returns (uint256) {
        ...
        if (amount > 0 && _can_trade_amount[_user_address] <= amount) {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        require(ControlAutonomy(_CA_address).getYBC_Control_status1());
        ...
        if (_special_list[from] == true || _special_list[to] == true) {
            ...
            if (ControlAutonomy(_CA_address).getYBC_Control_status2() == true) {
    ...
    function set_isSwapIncludes(address _address) public virtual {
        require(ControlAutonomy(_CA_address).getYBC_Control_status4(_msgSender()) == true, "Only YBC_Control can set isSwapIncludes");
        _isSwapInclude[_address] = true;
    }

    function set_is_not_slippage(bool _bool) public virtual {
        require(ControlAutonomy(_CA_address).getYBC_Control_status4(_msgSender()) == true, "Only YBC_Control can set is_not_slippage");
        _is_not_slippage = _bool;
    }

    function set_isSwap_LP_Includes(address _address) public virtual {
        require(ControlAutonomy(_CA_address).getYBC_Control_status4(_msgSender()) == true, "Only YBC_Control can set isSwap_LP_Includes");
        _isSwap_LP_Include[_address] = true;
    }

    function set_special_list(address _address) public virtual {
        require(ControlAutonomy(_CA_address).getYBC_Control_status4(_msgSender()) == true, "Only YBC_Control can set special_list");
        _special_list[_address] = true;
    }

    function set_startTime(uint256 _time) public virtual {
        require(ControlAutonomy(_CA_address).getYBC_Control_status4(_msgSender()) == true, "Only YBC_Control can set startTime");
        _startTime = _time;
    }

    function set_friend_address(address _address) public virtual {
        require(ControlAutonomy(_CA_address).getYBC_Control_status4(_msgSender()) == true, "Only YBC_Control can set friend_address");
        _friend_address = _address;
    }

    function set_YBC_LP_USDT_address(address _address) public virtual {
        require(ControlAutonomy(_CA_address).getYBC_Control_status4(_msgSender()) == true, "Only YBC_Control can set YBC_LP_USDT_address");
        _YBC_LP_USDT_address = _address;
    }

    function set_NFT_address(address _address) public virtual {
        require(ControlAutonomy(_CA_address).getYBC_Control_status4(_msgSender()) == true, "Only YBC_Control can set NFT_address");
        _NFT_address = _address;
    }

    function set_PledgeYBCLP_address(address _address) public virtual {
        require(ControlAutonomy(_CA_address).getYBC_Control_status4(_msgSender()) == true, "Only YBC_Control can set PledgeYBCLP_address");
    ...
    function set_part_rate(uint256 _rate) public virtual {
        require(ControlAutonomy(_CA_address).getYBC_Control_status4(_msgSender()) == true, "Only YBC_Control can set part_rate");
        _part_rate = _rate;
    }

    function set_minAddLiquidityNumber(uint256 _number) public virtual {
        require(ControlAutonomy(_CA_address).getYBC_Control_status4(_msgSender()) == true, "Only YBC_Control can set minAddLiquidityNumber");
        _minAddLiquidityNumber = _number;
    }

    function set_splitter_address(address _address) public virtual {
        require(ControlAutonomy(_CA_address).getYBC_Control_status4(_msgSender()) == true, "Only YBC_Control can set splitter_address");
    ...
    function set_reNumber(uint256 _number) public virtual {
        require(ControlAutonomy(_CA_address).getYBC_Control_status4(_msgSender()) == true, "Only YBC_Control can set reNumber");
        _reNumber = _number;
    }

    function set_YaBoWorld_address(address _address) public virtual {
        require(ControlAutonomy(_CA_address).getYBC_Control_status4(_msgSender()) == true, "Only YBC_Control can set YaBoWorld address");
        _YaBoWorld_address = _address;
    }

    function set_CA_address(address _address) public virtual {
        require(ControlAutonomy(_CA_address).getYBC_Control_status4(_msgSender()) == true, "Only YBC_Control can set CA address");
        _CA_address = _address;
    }
