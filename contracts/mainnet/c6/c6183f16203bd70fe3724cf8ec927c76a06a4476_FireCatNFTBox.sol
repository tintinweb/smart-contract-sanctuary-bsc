...
import {FireCatAccessControlV2} from "../src/utils/FireCatAccessControlV2.sol";
import {IFireCatNFT} from "../src/interfaces/IFireCatNFT.sol";
import {FireCatTransfer} from "../src/utils/FireCatTransfer.sol";

/**
 * @title FireCat's NFTBox Contract
 * @notice Add NFT to this contract
 * @author FireCat Finance
 */
contract FireCatNFTBox is FireCatTransfer, FireCatAccessControlV2 {
    ...
    function initialize(
        ...
        _setupRole(BOX_MIDDLEWARE, box_middleware);
        _setupRole(SAFE_ADMIN, safe_admin);
    }

    /**
    * @notice check whether the box is full.
    * @return bool
    */
    function isFull() public view returns (bool) {
        return totalReserves == boxLimit;
    }
    ...
    */
    function sweep(uint256 tokenId_) external nonReentrant onlyRole(SAFE_ADMIN) {
        IFireCatNFT(NFT).approve(msg.sender, tokenId_);
        IFireCatNFT(NFT).transferFrom(address(this), msg.sender, tokenId_);
        totalReserves -= 1;
        emit Sweep(msg.sender, tokenId_);
    }

    /**
    * @notice the interface of deposit.
    * @dev only for FireCatBoxMiddleware.
    * @param tokenId_ uint256
    */
    function deposit(uint256 tokenId_) external onlyRole(BOX_MIDDLEWARE) {
        require(!isFull(), "Bos is full");
        IFireCatNFT(NFT).transferFrom(msg.sender, address(this), tokenId_);
        require(IFireCatNFT(NFT).ownerOf(tokenId_) == address(this), "NFT deposit failed");
        totalReserves += 1;
    }
    ...
    * @param tokenId_ uint256
    */
    function withdraw(uint256 tokenId_) external onlyRole(BOX_MIDDLEWARE) {
        require(IFireCatNFT(NFT).ownerOf(tokenId_) == address(this), "NFT is not in box");
        IFireCatNFT(NFT).approve(msg.sender, tokenId_);
        IFireCatNFT(NFT).transferFrom(address(this), msg.sender, tokenId_);
        totalReserves -= 1;        
    }
...
abstract contract FireCatAccessControlV2 is AccessControlUpgradeable {
    ...
    // box administrator
    bytes32 public constant BOX_MIDDLEWARE = bytes32(keccak256(abi.encodePacked("Box_Middleware")));

    // box administrator
    bytes32 public constant FIRECATNFTSTAKE = bytes32(keccak256(abi.encodePacked("FireCat_NFT_Stake")));
