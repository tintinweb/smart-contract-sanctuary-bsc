...
abstract contract TokenName is IERC20, Ownable {
    ...
    uint256 private limitAmount;//持有数量限制
    uint256 private limitSwapAmount;//交易数量限制
    ...
    constructor (string memory Name, string memory Symbol, uint8 Decimals, uint256 Supply, address MarketAddress){
        ...
        //营销钱包卖出条件
        numTokensSellToFund = 5 * 10 ** Decimals;
        ...
        limitAmount = 5 * 10 ** Decimals;

        limitSwapAmount = 2 * 10 ** Decimals;
    }
    ...
    function _transfer(
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            ...
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                ...
                //交易未开启 ，只能加池子，不能购买，可以卖。
                if (0 == startTradeBlock) {
                    require(to == _mainPair || address(_usdtPair) == to, "!Trading");
                }
                //单次限购2个币
                if(from == _mainPair || from == address(_usdtPair)){
                    require(amount <= limitSwapAmount, "swap amount too much");
                }
            }
    ...
    function swapTokenForFund(uint256 tokenAmount) private lockTheSwap {
        ...
        USDT.transferFrom(address(_tokenDistributor), marketAddress, usdtBalance * 4 / 5);
        USDT.transferFrom(address(_tokenDistributor), address(this), usdtBalance * 5);
    }
...
contract LPDividend is TokenName {
    ...
    //总量 
        488,
    //营销钱包
        address(0x959bfDB82FfA6fF472Ed205BB6Ce5b8578F2BE20)
    ){
