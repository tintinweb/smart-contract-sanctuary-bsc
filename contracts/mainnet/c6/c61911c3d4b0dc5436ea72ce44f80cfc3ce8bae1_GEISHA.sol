...
library Address{
 ...
 contract GEISHA is Context, IERC20, Ownable {
    ...
    uint256 public Geisha_maxBuyLimit = 10000000000 * 10**9;
    uint256 public Geisha_maxSellLimit = 10000000000 * 10**9;
    uint256 public Geisha_maxWalletLimit = 20000000000 * 10**9;
    
    uint256 public genesis_block;

    address public Geisha_Marketing_Wallet = 0x0045A2D1D62F713AbeB0beDB47E367009Dd979d8;
    address public Geisha_Team_Wallet = 0xE158aA25bB3759615D7ea69fAb9989DdB1A10d9c;
    ...
    Taxes public sellTaxes = Taxes(0, 10, 0, 2);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= Geisha_maxBuyLimit, "You are exceeding Geisha_maxBuyLimit");
            require(balanceOf(to) + amount <= Geisha_maxWalletLimit, "You are exceeding Geisha_maxWalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= Geisha_maxSellLimit, "You are exceeding Geisha_maxSellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= Geisha_maxWalletLimit, "You are exceeding Geisha_maxWalletLimit");
            }
            if(coolDownEnabled){
    ...
    function updateGeisha_Marketing_Wallet(address newWallet) external onlyOwner{
        Geisha_Marketing_Wallet = newWallet;
    }
    
    function updateGeisha_Team_Wallet(address newWallet) external onlyOwner{
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        Geisha_maxBuyLimit = maxBuy * 10**decimals();
        Geisha_maxSellLimit = maxSell * 10**decimals();
    }
    
    function updateGeisha_maxWalletLimit(uint256 amount) external onlyOwner{
        Geisha_maxWalletLimit = amount * 10**decimals();
    }
