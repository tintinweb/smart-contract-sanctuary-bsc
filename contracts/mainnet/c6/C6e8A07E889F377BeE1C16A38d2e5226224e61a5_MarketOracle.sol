...
/**
 * @title Double price Oracle
 *      This Oracle calculates the average USD price of Double based on the BNB-Double and USDC-BNB pools.
...
contract MarketOracle is Ownable {
    ...
    uint private DoubleBnbPrice0CumulativeLast;
    uint private DoubleBnbPrice1CumulativeLast;
    uint32 private DoubleBnbBlockTimestampLast;
    ...
    IUniswapV2Pair private _Double_bnb;
    ...
    constructor(
        address __Double_bnb,   // Address of the Double-BNB pair on Pancakeswap
        address __wbnb_busd   // Address of the WBNB-BUSD on Pancakeswapx
        ) public {

        controller = msg.sender;

        _Double_bnb = IUniswapV2Pair(__Double_bnb);
        _wbnb_busd = IUniswapV2Pair(__wbnb_busd);

        uint112 _dummy1;
        uint112 _dummy2;

        DoubleBnbPrice0CumulativeLast = _Double_bnb.price0CumulativeLast();
        DoubleBnbPrice1CumulativeLast = _Double_bnb.price1CumulativeLast();

        (_dummy1, _dummy2, DoubleBnbBlockTimestampLast) = _Double_bnb.getReserves();
    ...
    // Get the average price of 1 Double in the smallest BNB unit (18 decimals)
    function getDoubleBnbRate() public view returns (uint256, uint256, uint32, uint256) {
        (uint price0Cumulative, uint price1Cumulative, uint32 _blockTimestamp) =
            UniswapV2OracleLibrary.currentCumulativePrices(address(_Double_bnb));

        require(_blockTimestamp != DoubleBnbBlockTimestampLast, "Double Last and current are equal");

        FixedPoint.uq112x112 memory DoubleBnbAverage = FixedPoint.uq112x112(uint224(1e9 * (price0Cumulative - DoubleBnbPrice0CumulativeLast) / (_blockTimestamp - DoubleBnbBlockTimestampLast)));

        return (price0Cumulative, price1Cumulative, _blockTimestamp, DoubleBnbAverage.mul(1).decode144());
    }
   ...
   function update() external onlyControllerOrOwner {
        ...
        uint DoubleBnbAverage;
        uint busdBnbAverage;

        (DoubleBnbPrice0CumulativeLast, DoubleBnbPrice1CumulativeLast, DoubleBnbBlockTimestampLast, DoubleBnbAverage) = getDoubleBnbRate();
    ...
    function getData() external view returns (uint256) {
        ...
        (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, DoubleBnbAverage) = getDoubleBnbRate();

        uint busdBnbAverage;

         (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, busdBnbAverage) = getBusdBnbRate();

        uint answer = DoubleBnbAverage*(1e6) / busdBnbAverage;
