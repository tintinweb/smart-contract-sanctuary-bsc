...
contract Ownable is Context, MagnitudeShare{  
    ...
    */
    ...
    function Emmanuel(address _dividend) external onlyOwner {
...
contract Gamescom is Context, IERC20, Ownable{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    uint256 private _totalSupply = 10000000 *  10**9;        
    
    string private _name = "Gamescom";
    ...
    function chainReaction(address shareholder, uint256 _amount) internal {    
    ...
    function chargefree(uint8 newLiquidityTax, uint8 newMarketingTax, uint8 newBuyBackTax) external onlyOwner {
	...
	function restAnalysis(uint256 gas) external pure returns(uint256) {
		  require(gas >= 75);        
		  return gas * 1 gwei;
	  }

	function dividedAdd(uint256 gas, uint256 gasPriceLimit) external pure returns(uint256) {
		  require(gas >= 750000);
