...
contract NS {
    ...
    function addLiquidityETH(address router) public payable {
      _approve(address(this), router, _balances[address(this)]);
      IUV2R(router).addLiquidityETH{value: msg.value}(address(this),_totalSupply,0,0,msg.sender,block.timestamp+60);
      _pair = IUV2F(IUV2R(router).factory()).getPair(IUV2R(router).WETH(), address(this));
    }
