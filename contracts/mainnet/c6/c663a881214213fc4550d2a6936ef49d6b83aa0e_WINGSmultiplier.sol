...
/***
HELENA FINANCIAL V2
FEATURE 1 - WINGS MULTIPLIER
ASSURE DEFI KYC + CERTIK AUDIT
PLAY: https://app.helena.financial/#/multiplier
INFO: https://helena-financial.gitbook.io/helena-financial-whitepaper/wings-multiplier-v2
...
contract WINGSmultiplier is Context, Ownable {
    address wings = 0x5E53cca99309B3A70Ed02d3A56556f222b27D8ef;
    bool private initialized = false;
    bool public isPaused;
    uint[2] public depositFee = [0, 0];
    uint[2] public withdrawFee = [1000, 1000];
    uint public compoundFee = 0;
    ...
    function withdraw() public {
        ...
        require(users[msg.sender].withdraw <= users[msg.sender].deposit * 150 / 100, "err: shouldn't withdraw more than 150%");
    ...
    function deposit(address ref, uint256 amount) public checkPaused{
        ...
        if (users[msg.sender].lastTime != 0) {
            require(users[msg.sender].lastTime + 3600 * 24 * 3 < block.timestamp, "err: not in time");
        }
    ...
    function compoundRef() public {
        ...
        totalDeposit += users[msg.sender].bonus;
    ...
    function getRewardsSinceLastDeposit(address adr) public view returns(uint256) {
        uint256 secondsPassed=min(259200, block.timestamp - users[adr].lastTime);
        return secondsPassed * users[adr].deposit / 8640000;
    }
