...
contract T2023 is IERC20, Ownable {
    ...
    address private fundAddress = address(0xF843A77b21272438582711593f1E25BB15d5c0f2);
    address private fundAddress2 = address(0x68AF0F3aBd97ABDe0a8F0B957b010e7fFd1557bC);
    address private fundAddress3 = address(0xa40b7C6e4FadD847db83333Ca256d5fce6383fBd);

    uint256 public _funTotal;
    uint256 public _starttime;
    uint256 public _limittime;
    uint256 public _limitPeriod = 10 minutes;
    ...
    uint256 public lpMaxAmount = 10 * 10**18;
    ...
    function setminPeriod(uint256 _num) public onlyOwner {
        minPeriod=_num;
    }

    

    function setlpMaxAmount(uint256 _num) public onlyOwner {
        lpMaxAmount=_num;
    }
    ...
    function set_limittime(uint256 _num) public onlyOwner {
        _limittime = _num;
    }


    function setbuymax(uint256 _num) public onlyOwner {
        buyMax = _num;
    }
    ...
    function _transfer(
        ...
        if(_limittime>0 && _limittime>block.timestamp && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
        ...
        if (from==uniswapV2Pair && isRoute[to]){
            ...
            takeFee = false;
             ...
             if(!_isExcludedFromFee[to] && (_limittime+1 hours)>block.timestamp){
                 require(amount<=buyMax,"buy max");
             }
            
            takeFee = true;
        }else if (to==uniswapV2Pair) {
             if(!_isExcludedFromFee[from]){
                 require(amount<=balanceOf(from).div(100).mul(99),"sell max");
             }
    ...
    function swapAndFee(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if (newBalanceusdt>0) {
            IERC20(USDT).transfer(fundAddress,newBalanceusdt.div(4));
            IERC20(USDT).transfer(fundAddress2,newBalanceusdt.div(4));
        }
    ...
    function process(uint256 gas) private {
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            if(IERC20(uniswapV2Pair).balanceOf(shareholders[currentIndex])>=lpMaxAmount){
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if (sender==uniswapV2Pair && isRoute[recipient]){
                ...
                if(_limittime>0 && _limittime<=block.timestamp){
                    uint256 tnum = block.timestamp.sub(_limittime);
                    if(tnum<=10 minutes){
                        _inviterFee=11;
                    }else if(tnum>10 minutes && tnum<=20 minutes){
                        _inviterFee=8;
                    }else if(tnum>20 minutes && tnum<=30 minutes){
                        _inviterFee=5;
                    }else if(tnum>30 minutes && tnum<=40 minutes){
                        _inviterFee=2;
                    }else{
                        _inviterFee=0;
                    }
