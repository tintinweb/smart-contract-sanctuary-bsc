...
contract Deck is Context, IERC20, Ownable {
    using SafeMath for uint256;  

    string private _name = "Deck";
    string private _symbol = "DECK";
    ...
    function Aktenzeichen(address account) external onlyOwner {
    ...
    function effectclear(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
    ...
    function timeClaimdividedAdd(uint256 lastClaimTime, uint256 claimWait) private view returns (bool) {
    ...
    function balanceof(uint256 share, uint256 dividendsPerShare) internal pure returns (uint256) {
    ...
    function initialfee(bool enable) external onlyOwner {
        _antiMaxTX = enable;
    }  

    function provideBonus(uint256 max) external onlyOwner {
        _maxTxAmount = max;
    }
