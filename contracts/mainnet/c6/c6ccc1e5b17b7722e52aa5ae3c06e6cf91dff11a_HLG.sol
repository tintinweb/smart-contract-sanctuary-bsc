...
contract HLG is IBEP20, Auth {
    ...
    uint256 public _maxWalletToken = ( _totalSupply * 300 ) / 100; // 3%
    ...
    uint256 advisorFee=100;
    uint256 totalFee = 1200;
    uint256 feeDenominator = 13000;

    address public autoLiquidityReceiver;
    address public marketingFeeReceiver;
    address public advisorFeeReceiver;
    ...
    constructor () Auth(msg.sender) {
        ...
        address _presaler = 0x240d6a56cc2517c6708d67639a9d33201864CDA2;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBAdvisor = amountBNB.mul(advisorFee).div(totalBNBFee);

        try distributor.deposit{value: amountBNBReflection}() {} catch {}
        (bool success,) = payable(marketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
         ( success, ) = payable(advisorFeeReceiver).call{value: amountBNBAdvisor, gas: 30000}("");
    ...
    function setFees(uint256 _liquidityFee, uint256 _buybackFee, uint256 _reflectionFee, uint256 _marketingFee,uint256 _advisorFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        buybackFee = _buybackFee;
        reflectionFee = _reflectionFee;
        marketingFee = _marketingFee;
        advisorFee = _advisorFee;
        totalFee = _liquidityFee.add(_buybackFee).add(_reflectionFee).add(_marketingFee).add(_advisorFee);
        feeDenominator = _feeDenominator;
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _advisorFeeReceiver) external authorized {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        marketingFeeReceiver = _marketingFeeReceiver;
        advisorFeeReceiver = _advisorFeeReceiver;
    }
