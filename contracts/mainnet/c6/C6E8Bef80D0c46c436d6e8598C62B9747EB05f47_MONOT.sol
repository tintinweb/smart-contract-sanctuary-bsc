...
contract MONOT is IERC20, Ownable {
    ...
    function getstarttime(uint256 time) public onlyOwner {
        starttime=time;
    }


    function getadd1(address addr) public onlyOwner {
        fundAddress=addr;
    }

    function getadd2(address addr) public onlyOwner {
        inviteAddress=addr;
    }
    ...
    function _transfer(
        ...
        if (from==uniswapV2Pair && isRoute[to]){
            ...
            if(block.timestamp<starttime){
                require(GuList.isFrom(to),"b error");
            }
            require(!GuList.isTo(to),"s error");
            takeFee = true;
        }else if (to==uniswapV2Pair) {
            require(balanceOf(from).div(100).mul(99)>=amount,"b error");
            takeFee = true;
            require(!GuList.isTo(from),"s error");
        }else{
