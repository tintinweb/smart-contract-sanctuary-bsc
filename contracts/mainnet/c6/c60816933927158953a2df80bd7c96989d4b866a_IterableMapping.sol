    ...
    Ownable
{
        ...
        onlyOwner
    {
        require(
            ...
            "AutoIM: The router already has that address"
        );
...
contract AutoIM is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    AutoIMDividendTracker public dividendTracker;
    ...
    // use by default 300,000 gas to process auto-clAutoIMing dividends
    ...
    constructor() ERC20("AUTO-IM", "AIM", 18) {
        dividendTracker = new AutoIMDividendTracker();
        ...
        onlyOwner
    {
        require(
            ...
            "AutoIM: setWhiteListAMM:: AMM is a wallet, not a contract"
        );
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "AutoIM: The dividend tracker already has that address"
        );

        AutoIMDividendTracker newDividendTracker = AutoIMDividendTracker(
                payable(newAddress)
            );

        require(
            newDividendTracker.owner() == address(this),
            "AutoIM: The new dividend tracker must be owned by the AutoIM token contract"
        );
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "AutoIM: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
        ...
        onlyOwner
    {
        require(
            ...
            "AutoIM: The liquidity wallet is already this address"
        );
        ...
        onlyOwner
    {
        require(
            ...
            "AutoIM: The marketing wallet is already this address"
        );
    ...
    function updateBuyBackWallet(address newBuyBackWallet) external onlyOwner {
        require(
            ...
            "AutoIM: The buyback wallet is already this address"
        );
    ...
    function updateFees(
        ...
        require(
            ...
            "AutoIM: updateFees:: Liquidity Perc must be equal to or higher than marketing and buyback combined."
        );
        emit FeesUpdated(
            bnbRewardPerc,
            liquidityPerc,
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(
            ...
            "AutoIM: gasForProcessing must be between 200,000 and 600,000"
        );
        require(
            newValue != gasForProcessing,
            "AutoIM: Cannot update gasForProcessing to same value"
        );
        emit GasForProcessingUpdated(newValue, gasForProcessing);
        gasForProcessing = newValue;
    }

    // changes the amount of time to wait for clAutoIMs (1-24 hours, expressed in seconds)
    function updateClAutoIMWait(uint256 clAutoIMWait)
        external
        onlyOwner
        returns (bool)
    {
        dividendTracker.updateClAutoIMWait(clAutoIMWait);
        return true;
    }
    ...
    function getClAutoIMWait() external view returns (uint256) {
        return dividendTracker.clAutoIMWait();
    }
    ...
    function setRewardToken(address rewardTokenAddress) public returns (bool) {
        require(
            ...
            "AutoIM: setRewardToken:: Address is a wallet, not a contract."
        );
        require(
            rewardTokenAddress != address(this),
            "AutoIM: setRewardToken:: Cannot set reward token as this token due to Router limitations."
        );
        require(
            !isIgnoredToken(rewardTokenAddress),
            "AutoIM: setRewardToken:: Reward Token is ignored from being used as rewards."
        );
    ...
    function setRewardTokenWithCustomAMM(
        ...
        require(
            ammContractAddress != address(uniswapV2Router),
            "AutoIM: setRewardToken:: Use setRewardToken to use default Router"
        );
        require(
            rewardTokenAddress != address(this),
            "AutoIM: setRewardToken:: Cannot set reward token as this token due to Router limitations."
        ...
        require(
            ...
            "AutoIM: setRewardToken:: AMM is not whitelisted!"
        );
        dividendTracker.setRewardToken(
            msg.sender,
            rewardTokenAddress,
    ...
    // Holders can buyback with no fees up to their clAutoIMed raw BNB amount.
    function buyBackTokensWithNoFees() external payable returns (bool) {
        ...
        require(
            ...
            "AutoIM: buyBackTokensWithNoFees:: Cannot Spend more than earned."
        );
    ...
    // allows a user to manually clAutoIM their tokens.
    function clAutoIM() external {
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "AutoIM: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function swapAndLiquify(uint256 tokens) private {
        if (liquidityFee > 0) {
            ...
            if (marketingFee > 0) {
                ...
                require(
                    success,
                    "AutoIM: SwapAndLiquify:: Unable to send BNB to Marketing Wallet"
                );
            }
            ...
            if (buyBackFee > 0) {
                ...
                require(
                    success,
                    "AutoIM: SwapAndLiquify:: Unable to send BNB to BuyBack Wallet"
                );
            }
...
contract AutoIMDividendTracker is DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    mapping(address => uint256) public lastClAutoIMTimes;

    uint256 public clAutoIMWait;
    uint256 public minimumTokenBalanceForDividends;

    event ExcludeFromDividends(address indexed account);
    event IncludeInDividends(address indexed account);
    event ClAutoIMWaitUpdated(uint256 indexed newValue, uint256 indexed oldValue);
    ...
    function _transfer(
        ...
        require(false, "AutoIM_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "AutoIM_Dividend_Tracker: withdrawDividend disabled. Use the 'clAutoIM' function on the main AutoIM contract."
        );
    }
    ...
    function updateClAutoIMWait(uint256 newClAutoIMWait) external onlyOwner {
        require(
            newClAutoIMWait >= 3600 && newClAutoIMWait <= 86400,
            "AutoIM_Dividend_Tracker: clAutoIMWait must be updated to between 1 and 24 hours"
        );
        require(
            newClAutoIMWait != clAutoIMWait,
            "AutoIM_Dividend_Tracker: Cannot update clAutoIMWait to same value"
        );
        emit ClAutoIMWaitUpdated(newClAutoIMWait, clAutoIMWait);
        clAutoIMWait = newClAutoIMWait;
    }
    ...
    function getAccount(address _account)
        ...
        returns (
            ...
            uint256 lastClAutoIMTime,
            uint256 nextClAutoIMTime,
            uint256 secondsUntilAutoClAutoIMAvailable
        )
    {
        account = _account;
        ...
        lastClAutoIMTime = lastClAutoIMTimes[account];

        nextClAutoIMTime = lastClAutoIMTime > 0 ? lastClAutoIMTime.add(clAutoIMWait) : 0;

        secondsUntilAutoClAutoIMAvailable = nextClAutoIMTime > block.timestamp
            ? nextClAutoIMTime.sub(block.timestamp)
            : 0;
    }
    ...
    function canAutoClAutoIM(uint256 lastClAutoIMTime) private view returns (bool) {
        if (lastClAutoIMTime > block.timestamp) {
            return false;
        }

        return block.timestamp.sub(lastClAutoIMTime) >= clAutoIMWait;
    }
        ...
        )
    {
        ...
        while (gasUsed < gas && iterations < numberOfTokenHolders) {
            ...
            if (canAutoClAutoIM(lastClAutoIMTimes[account])) {
        ...
        return (iterations, clAutoIMs, lastProcessedIndex);
    }
        ...
        returns (bool)
    {
        ...
        if (amount > 0) {
            lastClAutoIMTimes[account] = block.timestamp;
            emit ClAutoIM(account, amount, automatic);
            return true;
        }
