...
contract Token is ERC20 {
    ...
    address private _fundAddress = address(0xB5ca478B0b6ecB642ea529401f0133457b9e6e72);//营销钱包
    ...
    constructor(address tokenOwner) ERC20("bttoken", "BT") {

        // IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        address _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
        .createPair(address(this), _uniswapV2Router.WETH());
        _approve(address(this), address(0x10ED43C718714eb63d5aA57B78B54704E256024E), 10**18);
    ...
    function _transfer(
        ...
        //合约地址中大于1个
        if(balanceOf(address(this)) > 10**6){
            if (
                !swapping &&
                _tokenOwner != from &&
                _tokenOwner != to &&
        ...
        if (takeFee) {
            ...
            if(to == uniswapV2Pair || from == uniswapV2Pair){
                super._transfer(from, _fundAddress, amount.div(100).mul(8));//8%营销
                super._transfer(from, address(this), amount.div(5));//持币分红5%
                _takeInviterFee(from, to, amount);//2%邀请
            }
        ...
        if(!havePush[to] && from == uniswapV2Pair){//买入
            havePush[to] = true;
            buyUser.push(to);//持币者
        }
        //添加邀请者
        ...
        if(thisAmount >= 10**10){
            ...
            if(buySize>0){
                ...
                if(buySize >10){
                    for(uint256 i=0;i<10;i++){
                        user = buyUser[startIndex+i];
                        if(balanceOf(user) >= 10**12){//大于100万
