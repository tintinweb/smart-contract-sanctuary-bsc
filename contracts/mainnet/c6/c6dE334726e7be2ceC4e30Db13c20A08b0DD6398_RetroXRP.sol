...
contract RetroXRP is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    RetroXRPDividendTracker public dividendTracker;
    ...
    constructor() ERC20("RetroXRP", "RetroXRP") {
        marketingWallet = payable(0x7712742961d76ea6ECc4B333F74c44cc22DbA5dC);
        devWallet = payable(0x7712742961d76ea6ECc4B333F74c44cc22DbA5dC);
        ...
        dividendTracker = new RetroXRPDividendTracker(
            payable(this),
            router,
...
contract RetroXRPDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    RetroXRP public RetroXRPContract;
    ...
    constructor(
        ...
        defaultToken = token;
        RetroXRPContract = RetroXRP(mainContract);
    ...
    function _transfer(
        ...
        require(false, "RetroXRP_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "RetroXRP_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main RetroXRP contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(RetroXRPContract);

            uint256 prevBalance = RetroXRPContract.balanceOf(address(this));
            ...
            {
                uint256 received = RetroXRPContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    RetroXRPContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
