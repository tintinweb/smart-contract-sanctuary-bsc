...
contract StableSanta is IBEP20, Auth {
    ...
    string constant _name = "Stable Santa";
    ...
    function changeTxLimit(uint256 amountBuy, uint256 amountSell) external authorized {
        _maxTxAmountBuy = amountBuy;
        _maxTxAmountSell = amountSell;
    }
    
    // Set the maximum permitted wallet holding (percent of total supply)
    function changeMaxWalletPercent(uint256 maxWallPercent) external authorized() {
        _maxWalletToken = (_totalSupply * maxWallPercent ) / 1000;
    }
    
    // Whitelist a holder from fees
    function changeFeeExempt(address holder, bool exempt) external authorized {
        isFeeExempt[holder] = exempt;
    }
    
    
    // Whitelist a holder from transaction limits
    function changeTxLimitExempt(address holder, bool exempt) external authorized {
        isTxLimitExempt[holder] = exempt;
    }

    // Whitelist a holder from timelocks
    function changeTimelockExempt(address holder, bool exempt) external authorized {
        isTimelockExempt[holder] = exempt;
    }

    function BlackList(address addr, bool value) external authorized {
    ...
    function changeFees(uint256 _liquidityFee, uint256 _burnFee, uint256 _marketingFee, uint256 _devFee, uint256 _feeDenominator) external authorized {
    ...
    function changeFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _devFeeReceiver) external authorized {
