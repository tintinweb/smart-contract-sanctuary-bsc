...
contract ULE_BUY is Ownable {
    ...
    payable
    {
        ...
        uint256[] memory returnValues = swapExactETHForTokens(half,address(ULE));
    ...
    function swapExactETHForTokens(uint256 value, address token)
    public
    payable 
    returns (uint[] memory amounts)  
    {
    ...
    function UpdateSwapandLiquifyCondition(uint256 SwapandLiquifyCount_)
    public
    onlyOwner
    {SwapandLiquifyCount = SwapandLiquifyCount_;}

    function changeLPaddress(address _lpreceiver)
    public
    onlyOwner
    {LpReceiver = _lpreceiver;}
    
    function withdrawBNB(address to)
    public
    onlyOwner
    {payable(to).transfer(address(this).balance);}

    function withdrawToken(address to)
    public
    onlyOwner
    {ULE.transfer(to, ULE.balanceOf(address(this)));}
