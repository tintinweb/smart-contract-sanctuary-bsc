...
contract ETCsToken is ERC20, Ownable {
    ...
    constructor(
        ...
        receiveAddress = 0x5C7E713000f04eE224C59361cF8EAd3999Ee119D;

        // marketingWalletAddress = 0xe02D80ce0E3EEE79833Ce6a9581B23Ca9B7969Bb; 
        marketingWalletAddress = 0x6951fe9a4D0F847837259BAd80A84EC252Ac3E0c; 
        
        etcAddress = 0x55d398326f99059fF775485246999027B3197955;

        lpWalletAddress = 0x57B5e317E42FeB79bafe777715D29e00A3E894a0;
        ...
        excludeFromFees(lpWalletAddress, true); 
    ...
    function setLpWalletAddress(address addr) public onlyOwner {
        lpWalletAddress = addr;
    }
    ...
    function takeAllFee(address from, address to, uint256 amount) internal returns (uint256 amountAfter) {
        ...
        if (from == uniswapV2Pair) { 
            ...
            uint256 amount1 = totalLP;
            totalLP = 0;  
            _swapLp(lpAmount.add(amount1));
             
 
            uint256 marketingAmount = amount.mul(sellMarketingRate).div(maxRate);
            super._transfer(from, address(this), marketingAmount);
             uint256 amount2 = totalMarketing;
            totalMarketing=0;
            _swapMarketing( marketingAmount.add(amount2)); 
    ...
    function _swapLp(uint256 amount ) internal {
        if(amount > 0){
            ...
            path[0] = address(this);
            path[1] = 0x55d398326f99059fF775485246999027B3197955; 
            swapTokensForToken(amount, lpWalletAddress, path);
        } 
    }
    ...
    function lpDividendProcess(uint256 dividendAmount) internal {
        ...
        for (uint256 i = 0; i < lpHolderSet.length(); i++) {
            ...
            if(amount > 0){ 
                IERC20(etcAddress).transferFrom(lpWalletAddress,holder, amount ); 
