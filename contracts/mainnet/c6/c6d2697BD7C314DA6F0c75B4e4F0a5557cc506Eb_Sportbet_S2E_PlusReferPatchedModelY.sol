...
contract Sportbet_S2E_PlusReferPatchedModelY is Ownable {
    ...
    function drawLottery(uint _leagueId, uint _gamblingId, uint _ballot) external onlyOwner onlyNotFinished(_leagueId, _gamblingId) {
        ...
        uint losingStakesTotal = 0 - leagues[_leagueId].gamblings[_gamblingId].stakes[_ballot] + leagues[_leagueId].gamblings[_gamblingId].stakes[0] + leagues[_leagueId].gamblings[_gamblingId].stakes[1] + leagues[_leagueId].gamblings[_gamblingId].stakes[2]; /////////USING 9 DECIMALS IN THIS CONTRACT/////////
