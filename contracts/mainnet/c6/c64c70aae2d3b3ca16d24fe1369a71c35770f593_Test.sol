...
contract Test is Context, IERC20, Ownable {
    ...
    address payable public marketingAddress =
        payable(0x6e0D8e5ce438f3f47Cfee14DEA9120eC32C39993); // Marketing Address
    address payable public miniGameAddress =
        payable(0x37B317642182Af5fFC1F2f241178B7ca90296Add); // Mini Game Address
    ...
    uint256 public _maxTxAmount = _tTotal.mul(1).div(100); // 3% of supply
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        (bool tmpSuccess, ) = payable(miniGameAddress).call{
            value: transferredBalance.div(_marketingMiniGameFee).mul(
                miniGameDivisor
            )
        }("");
        (bool tmpSuccess2, ) = payable(marketingAddress).call{
            value: transferredBalance.div(_marketingMiniGameFee).mul(
                marketingDivisor
            )
        }("");
    ...
    {
        uint256 tFee = tAmount.mul(_reflectionFee).div(10**2);
        uint256 tLiquidity = tAmount.mul(_marketingMiniGameFee).div(10**2);
    ...
    function setCanTransferBeforeTrade(address holder, bool can)
        external
        onlyOwner
    {
    ...
    function revertAntisnipe() external onlyOwner {
        require(antiSnipe, "Already disabled.");
        antiSnipe = false;
