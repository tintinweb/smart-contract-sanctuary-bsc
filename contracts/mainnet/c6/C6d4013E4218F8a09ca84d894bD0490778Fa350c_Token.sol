...
contract Token is ERC20, Ownable {
    ...
    uint256 private _total = 10_0000_0000 ether;

    uint256 public _maxHold = 100 ether;
    ...
    uint256 public txMarketFee = 1200;
    uint256 public txLPFee = 400;
    ...
    function set1() public onlyOwner {
        setTxMarketFee(1200);
        setTxLPFee(400);
        // _maxHold = 100 ether;
        burnFee = 400;
    ...
    function set2() public onlyOwner {
        setTxMarketFee(600);
        setTxLPFee(200);
        // _maxHold = 100 ether;
        burnFee = 200;
    ...
    function set3() public onlyOwner {
        setTxMarketFee(300);
        setTxLPFee(100);
        // _maxHold = _total;
        burnFee = 100;
    ...
    function _transfer(
        ...
        if (takeFee) {
            if (launched()) {
                ...
                } else if (block.timestamp < launchedAtTime + 2 hours) {
                    if (txMarketFee != 600) {
                        txMarketFee = 600;
                        txLPFee = 200;
                        burnFee = 200;
    ...
    function _swap() private lockSwap {
        uint256 tokens = balanceOf(address(this)).sub(10000);
        uint256 lpTokens = tokens.mul(txLPFee).div(txFee);
        // uint256 halfTokens = lpTokens.div(2);
        // uint256 otherHalf = lpTokens.sub(halfTokens);

        uint256 marketTokens = tokens.sub(lpTokens);

        uint256 swapTokens = tokens;//.sub(otherHalf);
        ...
        IERC20(tokenB).transfer(_marketWallet, bBalance.mul(marketTokens).div(swapTokens));

        // addLiquidityForTokenB(balanceOf(address(this)), IERC20(tokenB).balanceOf(address(this)));

        uint256 dividends = IERC20(tokenB).balanceOf(address(this));
        IERC20(tokenB).transfer(address(dividendTracker), dividends);
