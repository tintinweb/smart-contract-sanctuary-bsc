...
// uint256 constant SECONDS_17_MONTHS = 4.468e7;
...
contract AutofarmVenusDistributor {
	...
	constructor(bytes32 _root, address payable _owner, address[] memory _assets) {
		...
		owner = _owner;
		assets = _assets;
		// distributionEnd = block.timestamp + SECONDS_17_MONTHS;
	}

	function claim(address claimant, address asset, uint96 amount, bytes32[] calldata proof) public {
		bytes32 leaf = keccak256(abi.encodePacked(claimant, asset, amount));
		...
		claimed[leaf] = true;
		ERC20(asset).safeTransfer(claimant, amount);
	}

	function stopDistribution() public {
		// require(block.timestamp >= distributionEnd);
