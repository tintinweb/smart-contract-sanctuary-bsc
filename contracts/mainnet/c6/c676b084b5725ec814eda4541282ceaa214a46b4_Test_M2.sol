...
//////
    ...
    constructor(address payable wallet) public {
        ...
        plans.push(Plan(0.10 ether, 1 minutes));
        plans.push(Plan(0.09 ether, 1 minutes));
         plans.push(Plan(0.09 ether, 1 minutes));
        plans.push(Plan(0.09 ether, 1 minutes));
        plans.push(Plan(0.09 ether, 1 minutes));
    ...
    function invest(address referrer, uint8 plan) public payable{
			...
			address upline2 = user.secondLevelReferrals;
				if (upline2 != address(0)) {
					...
					upline2 = users[upline2].firstLevelReferrals;
            			...
            			address upline3 = user.thirdLevelReferrals;
		...
		if (user.deposits.length == 0) {
            ...
            user.checkpointRefBonus2 = block.timestamp + (plans[plan].time);
  ...
  function withdraw() public {
    ...
    //  sfr =  user.checkpointRefBonus+(user.deposits[i].finish);
     //require(block.timestamp >= (user.checkpointRefBonus+(user.deposits[i].finish)));
       //user.checkpointRefBonus+(user.deposits[i].finish);

    

    require(block.timestamp >= user.checkpointRefBonus2);
...
//str2 = sfr.add(1);
    ...
    function getUsercheckpointRefBonus2(address userAddress)
        public
        view
        returns (uint256)
    {

        return users[userAddress].checkpointRefBonus2;
    }
    ...
    function getUsercheckpointRefBonusMinutes22222(address userAddress)
        public
        view
        returns (uint256)
    {
