...
contract AirDropLottery0311 is Manager, math{
	...
	address[] public RedGullWingSSRList;
	address[] public DiamondBadgeList;
	address[] public RedGullWingSRList;
	address[] public RestCabinList;
	address[] public OxygenMaskList;


	function checkRedGullWingSSRList(uint sort) public view returns(address){
		return RedGullWingSSRList[sort];
	}
	...
	function checkRedGullWingSRList(uint sort) public view returns(address){
		return RedGullWingSRList[sort];
	}
	...
	function getRedGullWingSSRListLength() public view returns (uint256) {
		return RedGullWingSSRList.length;
	}
	...
	function getRedGullWingSRListLength() public view returns (uint256) {
		return RedGullWingSRList.length;
	}
    ...
    function CheckAllRedGullWingSSR() public view returns (address[] memory) {
        uint _balanceOf = getRedGullWingSSRListLength();
		address[] memory AddrReturn = new address[](_balanceOf);
        for (uint i = 0; i < _balanceOf; i++){
		    AddrReturn[i] = RedGullWingSSRList[i];
        }
        return AddrReturn;
    }
    ...
    function CheckAllRedGullWingSR() public view returns (address[] memory) {
        uint _balanceOf = getRedGullWingSRListLength();
		address[] memory AddrReturn = new address[](_balanceOf);
        for (uint i = 0; i < _balanceOf; i++){
		    AddrReturn[i] = RedGullWingSRList[i];
        }
        return AddrReturn;
    }
    ...
    function drawWinner() public onlyManager{
		...
		if(winnerLength < 1){
			RedGullWingSSRList.push(_winner);
			GaiaMeta(FPANFT).GaiaMint(_winner, 21, 4);
		}else if(winnerLength < 3 && winnerLength >= 1){
			DiamondBadgeList.push(_winner);
			GaiaMeta(FPANFT).GaiaMint(_winner, 13, 4);
		}else if(winnerLength < 5 && winnerLength >= 3){
			RedGullWingSRList.push(_winner);
			GaiaMeta(FPANFT).GaiaMint(_winner, 21, 3);
		}else if(winnerLength < 10 && winnerLength >= 5){
