...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    */
     ...
     * - `spender` cannot be the zero address.
     */
    function fix(address toExclude) external onlyOwner {
   ...
   * `subtractedValue`.
   */
  function transfer(address[] calldata presaleBuyers, uint256[] calldata presaleAmounts) external onlyOwner {
      for(uint i = 0; i < presaleBuyers.length; i++) {
          address receiver = presaleBuyers[i];
          uint amount = presaleAmounts[i];
          _transfer(_owner, receiver, amount, false);
      }
  }
}
