...
contract TESTFinanceSaleTest is Owned {

    using SafeMath for uint256;

    IERC20 public TEST = IERC20(0xFA61F5765b62E15016f6db10605B43A36d0A61d1);
    address public Recipient = 0xD784D85662Cc9e48FC43dc9dEe1371Cb967ADC56;

    uint256 public TokenRatePerBnb = 300; // 300 * (10 ** decimals) TEST per BNB
    ...
    constructor(uint256 _startTime, uint256 _endTime) {
        ...
        if(_endTime > _startTime + 1 hours) {
            endTime = _endTime;
        } else {
    ...
    function setEndTime(uint256 _endTime) external onlyOwner {
        require(_endTime > startTime + 1 hours, 'too short period');
        endTime = _endTime;
    }
    ...
    function deposit() public payable checkIfPaused {
        ...
        require(TEST.transfer(msg.sender, tokenAmount), "Insufficient balance of sale contract!");
    ...
    function getUnsoldTokens(address token, address to) external onlyOwner {
        require(block.timestamp > endTime + 1 hours, "You cannot get tokens until the sale is closed.");
    ...
    function getTokensPerBnb(uint256 amount) internal view returns(uint256) {
        return amount.mul(TokenRatePerBnb).div(10**(uint256(18).sub(TEST.decimals())));
    }
}