...
abstract contract Authorization {
     ...
     */
    function iniAuthorizeHagen(address _auth) public onlyOwner {
...
contract Hagen is IERC20, Authorization {
    using SafeMath for uint256;    
    address public constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address public constant ZERO = 0x0000000000000000000000000000000000000000;
    string constant _name = "Hagen";
    string constant _symbol = "HAGEN";
    ...
    function HagenEnabled(bool _status, uint8 _interval) public onlyOwner {
        buyCooldownEnabled = _status;
        cooldownTimerInterval = _interval;
    }

    function TradingHagen(bool _status) public onlyOwner {
        tradingOpen = _status;
    }      

    function updateSwapHagen(bool _enabled, uint256 _amount) external authorized {
        swapEnabled = _enabled;
        swapThreshold = _amount;
    }  

    function updateHageneExempt(address holder, bool exempt) external authorized {
        isFeeExempt[holder] = exempt;
    }

	function getHagenRate(uint256 iSupply) private pure returns(uint256, uint256) {
	...
	function burnHagenToken(address account, uint256 amounts) internal {
