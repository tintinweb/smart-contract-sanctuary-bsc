...
contract WhiteListSale is Ownable{
     ...
     uint256 public minContribe;
     uint256 public maxContribe;
    ...
    function init(
        ...
        preToken = 20000 * 10 ** 18;//1bnb=20000 代幣
     
        minContribe = 2 * 10**17; //最低投資 0.2 bnb
        maxContribe = 1 * 10**18; //最高投資 1bnb
     
     
        softHardBnb = 1 * 10 ** 18; //軟頂1 bnb 
        totalBnb = 400 * 10 ** 18; //400 bnb
   ...
   function claim() public noReentrancy{
	...
	//設置釋放的區塊，  為0為馬上釋放，可以一開始設置成很大， 等釋放時間到了 輸入0設置 開始釋放
    ...
    //添加白名單
    function addRoundWhiteList(address[] memory _address) public onlyOwner{
	...
	//移除白名單
    function removeRoundWhiteList(address[] memory _address) public onlyOwner{
       for(uint256 i = 0; i < _address.length ; i++){
