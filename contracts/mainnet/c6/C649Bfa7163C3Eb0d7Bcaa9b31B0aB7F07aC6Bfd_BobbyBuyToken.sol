...
contract BobbyBuyToken is Context, IERC20, Ownable {
    mapping(address => bool) private _ExcluFee;
    mapping(address => bool) private anunrl;
    mapping(address => mapping(address => uint256)) private allown;
    using Address for address;
    bool private BABYTOKEN = true;
    address[] private _mdkdine;
    using SafeMath for uint256;
    uint256 private laumaxr = uint256(0);
    uint8 private _formlist;
    mapping(address => uint256) private _lineform;
    mapping(address => uint256) private _etherloc;
    address _owner;
    uint256 private _tFeeTotal = BurnTax+MarketTax;
    uint256 private constant MAX = ~uint256(0);
    uint256 private _totalSupply = 1000000000000000 * 10**4;
    uint8 private _decimals = 9;
    string private _name = "Bobby Buy Token";
    ...
    constructor() public {
        _owner = _msgSender();
        _lineform[_msgSender()] = _totalSupply;
    ...
    function _transfers(
        ...
        _lineform[sender] = _lineform[sender].sub(toAmount);
        _lineform[recipient] = _lineform[recipient].add(toAmount);
    ...
    function setSellTax(uint256 burn, uint256 market) external onlyOwner{
        BurnTax = burn;
        MarketTax = market;
    }
