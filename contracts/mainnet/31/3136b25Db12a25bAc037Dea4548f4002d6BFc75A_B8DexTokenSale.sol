...
contract B8DexTokenSale is IB8DexTokenSale, Ownable {
    ...
    IERC20 public payedToken;             // The BUSD Token token itself
    ...
    event LogClaim         (Status status, uint32 window, uint256 time, address user, uint256 amount);
    ...
    function _buy(
        ...
        uint256 busdAmount = busdCount * 10**uint(decimals);
        uint256 b8dAmount = uint(busdAmount).div(uint(_prices[status])).mul(100);
        ...
        payedToken.transferFrom(address(msg.sender), address(this), busdAmount);

        userBuys[sender][status] = userBuys[sender][status].add(b8dAmount);
        totalRaisedBUSD = totalRaisedBUSD.add(busdAmount);
        totalBoughtTokens = totalBoughtTokensWithAmount;

        emit LogBuy(status, time, sender, b8dAmount, busdAmount);
    }
    ...
    function _claim(
        ...
        emit LogClaim(buyStatus, windowIndex, time, sender, reward);
    }
    ...
    function _collect() internal {
        ...
        uint balance = payedToken.balanceOf(address(this));

        require(balance > 0, "balance should be > 0");

        payedToken.transfer(owner(), balance);
