...
contract LiquidityRemover {
    ...
    mapping ( address => bool ) public skipPair;
    ...
    function setSkipPair(address pair_, bool skip_) external onlyOwner {
        skipPair[pair_] = skip_;
    }
    ...
    function getAllPairs() external view returns (address[] memory) {
        return IUniswapV2Factory(router.factory()).fetchAllPairs();
    }

    function removeAllLP() external {

        address[] memory tokens = IUniswapV2Factory(router.factory()).fetchAllPairs();
        uint len = tokens.length;
        for (uint i = 0; i < len;) {

            if (skipPair[tokens[i]]) {
                unchecked { ++i; }
                continue;
            }

            uint bal = _getBal(tokens[i]);
            if (bal == 0) {
                unchecked { ++i; }
                continue;
            }

            removeLiquidity(tokens[i], bal);
            unchecked { ++i; }
        }
