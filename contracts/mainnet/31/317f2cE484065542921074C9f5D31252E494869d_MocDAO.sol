...
contract MocDAO is IERC20, IERC20Metadata, Ownable {
    ...
    address public mocAddress;

    address public root;

    constructor(address _root) {
        _name = "macroeconomics  DAO";
        _symbol = "MOCDAO";
        _mint(_root, 1000000 * 10**decimals());
        root = _root;
    }
    ...
    function _transfer(
        ...
        uint256 fromOldBalance = _balances[from];
        require(
            fromOldBalance >= amount,
            "ERC20: transfer amount exceeds balance"
        );
        require(mocAddress != address(0), "no set moc");
        IMoc moc = IMoc(mocAddress);
        uint256 fromMocBalance = moc.balanceOf(from);
        uint256 toMocBalance = moc.balanceOf(to);
        uint256 newFromBalance;
        unchecked {
            newFromBalance = fromOldBalance - amount;
            _balances[from] = newFromBalance;
        }
        uint256 toOldBalance = _balances[to];
        uint256 toNewBalance = toOldBalance + amount;
        _balances[to] = toNewBalance;

        moc.updateDAOAccount(
            from,
            fromOldBalance,
            newFromBalance,
            fromMocBalance
        );
        moc.updateDAOAccount(to, toOldBalance, toNewBalance, toMocBalance);
    ...
    function _afterTokenTransfer(
        ...
        if (
            ...
            if (from.isContract()) {
                ...
                address p = from == address(0) ? root : from;
                parents[to] = p;
                addChild(to, p);
                emit Bind(to, p);
            }
    ...
    function mint(uint256 amount) external onlyOwner {
        require(amount >= 1 * 10**decimals(), "at least 1");
    ...
    function setMoc(address moc) external onlyOwner {
        mocAddress = moc;
    }
}
...
interface IMoc{
    function balanceOf(address account) external view returns (uint256);
    function updateDAOAccount(address user,uint256 oldBalance,uint256 newBalance,uint256 mocBalance) external;
}