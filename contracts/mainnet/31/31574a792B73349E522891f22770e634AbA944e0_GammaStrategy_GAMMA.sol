...
abstract contract StratX2 is Ownable, ReentrancyGuard {
    ...
    uint256 public immutable instantWithdrawFeeFactorMax = 1000; //10%
    ...
    */
    ...
    function deposit(uint256 _wantAmt) external virtual returns (uint256) {
        return deposit(_wantAmt, true);
    }

    // Receives new deposits from depositAuthorised
    function depositAuthorized(uint256 _wantAmt) external virtual returns (uint256) {
        return deposit(_wantAmt, false);
    } 

    // Executes deposit based on whether or not to charge fee
    function deposit(uint256 _wantAmt, bool _chargeFee) internal virtual nonReentrant returns (uint256) {
