...
/*
    ...
    / ____/ ____/_  __/  /_  __/___  / /_____  ____ 
   / /   / __/   / /      / / / __ \/ //_/ _ \/ __ \
  / /___/ /___  / /      / / / /_/ / ,< /  __/ / / /
  \____/_____/ /_/      /_/  \____/_/|_|\___/_/ /_/ 
                                                                                                                                           
                                                
  Website:  https://www.cakeeatertoken.com/
  Telegram: http://t.me/CakeeaterBsc/19
*/
...
contract BUSDDividendTracker is DividendPayingToken  {
    ...
    function process(uint256 gas) external returns (uint256, uint256, uint256) {
    	...
    	while(gasUsed < gas && iterations < numberOfTokenHolders) {
            ...
            bool claimTest = canAutoClaim(lastClaimTimes[account]);
...
contract CET is ERC20, Ownable {
    ...
    event OpenTrading(uint256 time);
    ...
    constructor() ERC20("Cake Eater", "CET") {
    	marketingWallet = 0x8B570407C0A19F849cD6E750aedceEfbB56748da;  
        ...
        transferOwnership(0x8B570407C0A19F849cD6E750aedceEfbB56748da);

        /*
            _mint is an internal function in ERC20.sol that is only called here,
            and CANNOT be called ever again
        */
        _mint(owner(), 1_0000_0000 * (10**18));
    }
    ...
    receive() external payable {
  	...
  	function setMaxSellTransaction(uint256 _maxTxn) external onlyOwner {
        require(_maxTxn >= 10_000, "CET: The maxSellTransactionAmount cannot be lower than 10,000 tokens" );
  	...
  	function setSellTransactionMultiplier(uint256 _multiplier) external onlyOwner {
        require(_multiplier <= 200, "Sell Multiplier Must Be Less than 100% of the fees (20% tax max)");
    ...
    function getNumberOfBusdDividendTokenHolders() external view returns(uint256) {
     ...
     function getAccumulativeDividendOf(address account) external view returns(uint256) {
        return busdDividendTracker.accumulativeDividendOf(account);
    }
    ...
    function _transfer(
        ...
        if(swapping) {super._transfer(from, to, amount); return;}
        ...
        if (!swapping && canSwap && from != uniswapV2Pair) {
            ...
            if (busdDividendEnabled) {
                ...
                swapTokensForDividendToken(busdTokens, busdDividendToken);
        ...
        if(takeFee) {
            ...
            // if sell, multiply by 1.n (max tax is 20%)
            ...
            super._transfer(from, marketingWallet, tmpMarketingRewardPercent);
        }
    ...
    function swapTokensForDividendToken(uint256 _tokenAmount, address _dividendAddress) private {
        // Contract token balance sanity check
        uint256 tokenBalance = balanceOf(address(this));
        if(_tokenAmount > tokenBalance) _tokenAmount = tokenBalance;
        // capture the vault contract current BUSD balance.
		// this is so that we can capture exactly the amount 
		// of BUSD that the we are depositing. 
		uint256 balanceBefore = IERC20(busdDividendToken).balanceOf(address(busdDividendTracker));
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            _tokenAmount,
            1, // accept any amount of dividend token
            path,
            address(busdDividendTracker),
            block.timestamp
        );

        // how much BUSD did we just deposited?
        uint256 balanceAfter = IERC20(busdDividendToken).balanceOf(address(busdDividendTracker));
		uint256 deposited = balanceAfter.sub(balanceBefore);

        busdDividendTracker.distributeDividends(deposited);
        emit SendDividends(deposited);
    }
