...
import "../reflexion/ReflectionSupportStaking.sol";
import "../timelock/StaticFixedTimeLockStaking.sol";
import "../whitelist/WhitelistedStaking.sol";

import "../reflexion/ReflectVault.sol";
import "@openzeppelin/contracts/proxy/Clones.sol";

contract ReflectWhitelistedStaticTimelockVault is
    WhitelistedStaking,
    ReflectionSupportStaking,
    StaticFixedTimeLockStaking
{
    function initialize(
        ...
        __ReflectionSupportStaking_init_unchained(_reflectionToken);
        __FixedTimeLockStaking_init_unchained(_lockTime, _earlyUnlockPenalty);
    }

    function stake(uint256 _amount)
        public
        override(BaseStaking, ReflectionSupportStaking, FixedTimeLockStaking)
        canStake(_amount)
    ...
    function _stake(uint256 _amount)
        internal
        virtual
        override(BaseStaking, ReflectionSupportStaking, StaticFixedTimeLockStaking)
    {
        StaticFixedTimeLockStaking._stake(_amount);
        TaxedStaking._stake(_amount);
        ReflectionSupportStaking._stake(_amount);
    }
    ...
    function _withdraw(uint256 _amount)
        ...
        override(BaseStaking, ReflectionSupportStaking, FixedTimeLockStaking)
    {
        ReflectionSupportStaking._withdraw(_amount);
        FixedTimeLockStaking._withdraw(_amount);
    }

    function _claim() internal virtual override(BaseStaking, ReflectionSupportStaking) {
        ReflectionSupportStaking._claim();
    }

    function _canWithdraw(address account, uint256 amount) internal view override(BaseStaking, FixedTimeLockStaking) {
        FixedTimeLockStaking._canWithdraw(account, amount);
    }

    function _canStake(address account, uint256 amount)
        internal
        view
        virtual
        override(BaseStaking, WhitelistedStaking)
    {
        WhitelistedStaking._canStake(account, amount);
    }
}
...
import "./ReflectVault.sol";

contract ReflectionSupportStaking is BaseStaking {
    using SafeERC20 for IERC20;

    address constant MASTER_ADDREESS = 0x50720E10f47F21e59eB5C7a13Bd31f10A5b0F92B;
    address implementation;
    mapping(address => address) public deposits;

    IERC20 public reflectionToken;

    function __ReflectionSupportStaking_init(StakingUtils.StakingConfiguration memory config, address _reflectionToken)
        public
        onlyInitializing
    {
        __BaseStaking_init(config);
    ...
    function __ReflectionSupportStaking_init_unchained(address _reflectionToken) public onlyInitializing {
        reflectionToken = ERC20(_reflectionToken);
        implementation = address(new ReflectVault());
    }
    ...
    function _stake(uint256 _amount) internal virtual override {
        if (deposits[msg.sender] == address(0)) {
            deposits[msg.sender] = createVault(msg.sender);
        }
        configuration.stakingToken.increaseAllowance(deposits[msg.sender], _amount);
        ReflectVault(deposits[msg.sender]).deposit(_amount);
    }

    function _claim() internal virtual override {
        ReflectVault(deposits[msg.sender]).claimReflection(msg.sender);
        BaseStaking._claim();
    }

    function _withdraw(uint256 _amount) internal virtual override {
        ReflectVault(deposits[msg.sender]).release(_amount);
    }

    function createVault(address owner) internal returns (address) {
        address clone = Clones.clone(implementation);

        ReflectVault(clone).initialize(
            owner,
            MASTER_ADDREESS,
            address(configuration.stakingToken),
            address(reflectionToken)
        );
        return clone;
    }

    function getReflections(address account) public view returns (uint256) {
        if (deposits[account] == address(0)) {
            return 0;
        }
        return ReflectVault(deposits[account]).earnedFromReflection();
    }
...
contract WhitelistedStaking is BaseStaking {
    using SafeERC20 for IERC20;
    ...
    function __WhitelistedStaking_init(StakingUtils.StakingConfiguration memory config) public onlyInitializing {
        __BaseStaking_init(config);
    }

    function addAddressToWhitelist(address[] calldata addresses) public virtual onlyRole(MANAGER_ROLE) {
        for (uint256 index = 0; index < addresses.length; index++) {
            whitelist.add(addresses[index]);
        }
    }

    function removeAddressesFromWhitelist(address[] calldata addresses) public virtual onlyRole(MANAGER_ROLE) {
        for (uint256 index = 0; index < addresses.length; index++) {
            whitelist.remove(addresses[index]);
        }
    }
    ...
    function setOpenForAll(bool status) public virtual onlyRole(MANAGER_ROLE) {
        openForAll = status;
    }

    function _canStake(address account, uint256 amount) internal view virtual override {
        super._canStake(account, amount);
...
contract ReflectVault {
    using SafeERC20 for IERC20;
    event ClaimedReflection(address indexed account, uint256 amount);
    ...
    modifier onlyManager() {
        require(msg.sender == manager || msg.sender == master, "Access denied");
        _;
    }
    ...
    function initialize(
        ...
        require(!init, "Allready intialized");
        init = true;
        owner = _owner;
        master = _master;
        manager = msg.sender;
    ...
    function release(uint256 _amount) public onlyManager {
    ...
    function claimReflection(address receiver) public onlyManager {
        uint256 earned = earnedFromReflection();
        if (earned > 0) {
            reflectionToken.safeTransfer(receiver, earned);
            emit ClaimedReflection(receiver, earned);
        }
    }

    function earnedFromReflection() public view returns (uint256) {
        uint256 reflectionBalance = reflectionToken.balanceOf(address(this));
        if (address(reflectionToken) == address(token)) {
            if (reflectionBalance < balance) {
                return 0;
            }
...
contract BaseStaking is Initializable, AccessControlUpgradeable {
    ...
    function topUpRewards(uint256 amount) public {
    ...
    function earned(address account) public view returns (uint256) {
        return ((_balances[account] * (rewardPerToken() - userRewardPerTokenPaid[account])) / 1e36) + rewards[account];
    }
...
contract FixedTimeLockStaking is TaxedStaking {
    ...
    function lockEnded(address account) public view returns (bool) {
        return block.timestamp >= locks[account] || _rewardSupply == 0;
    }
