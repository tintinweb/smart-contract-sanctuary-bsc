...
contract BnbLoot is ReentrancyGuard {
    // Structs
    struct User {
        uint id;
    ...
    // Constants
    uint public constant registrationPrice = 0.03 ether;
    ...
    uint[] public levelPrice = [
        ...
        4.4 ether,  // Level 14
        6.5 ether,  // Level 15
        1 ether, // level 16(special level)
        8 ether,    // Level 17
        10 ether,   // Level 18
        12.5 ether // Level 19
    ];
    ...
    constructor() public {
        ...
        minTotalUsersForLevel[16] = 100;
        minTotalUsersForLevel[17] = 100;
        minTotalUsersForLevel[18] = 200;
        minTotalUsersForLevel[19] = 300;
    ...
    function buyLevel(uint8 level) public payable nonReentrant {
        ...
        require(!isContract(msg.sender), "Can not be a contract");
        if(level==16){
            require(users[msg.sender].levels[level].activationTimes==0, "This level can be activated only once");
        }
        if (!users[msg.sender].levels[level].active && level!=16){
            for(uint8 l = 1; l < level; l++) {
                require(users[msg.sender].levels[l].activationTimes>0, "All previous levels must be at least once activated");
            }
        }
        ...
        if (!users[msg.sender].levels[level].active) {
            ...
            if(level==16){
                users[msg.sender].levels[level].maxPayouts += 1000;
            }
