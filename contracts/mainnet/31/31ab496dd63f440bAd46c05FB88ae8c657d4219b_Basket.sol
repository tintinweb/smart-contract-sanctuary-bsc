...
contract Basket is Ownable {
  ...
  function deposit1(address account, address beefyzap, address vault, address router, address[] calldata path, uint256 iscoin) public payable {
    IWETH(path[0]).deposit{value: msg.value}();
  }

  function deposit2(address account, address beefyzap, address vault, address router, address[] calldata path, uint256 iscoin) public payable {
    IUniswapRouterETH(router).swapExactTokensForTokens(10000_0000_0000_0000, 0, path, address(this), block.timestamp);
  }

  function deposit3(address account, address beefyzap, address vault, address router, address[] calldata path, uint256 iscoin) public payable {
    uint256 tokenbalance = IERC20(path[1]).balanceOf(address(this));
    _approveTokenIfNeeded(path[1], beefyzap);
