...
contract DOTC is Configurable {
    ...
    function make(SMake memory make_,bool isPrivate) virtual external returns(uint makeID) { 
    ...
    function take(uint makeID, uint volume,string memory link) virtual external returns (uint takeID, uint vol) {
        require(makes[makeID].maker != address(0), 'Nonexistent make order');
        require(makes[makeID].remain > 0, 'make.remain should > 0');
    ...
    function price() public view returns(uint p1, uint p2) {
        p1 = _feeBuf().mul(1e18).div(config[_rewardOfSpan_]);
    ...
    function _payFee(uint takeID, address asset, uint vol) internal returns(uint fee) {
        ...
        uint feeBuf = _feeBuf();
        vol  = amt1.mul(config[_rewardOfSpan_]).div(feeBuf);
