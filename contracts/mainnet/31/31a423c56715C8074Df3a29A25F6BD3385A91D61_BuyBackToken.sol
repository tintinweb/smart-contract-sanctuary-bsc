...
contract BuyBackToken is Context, IERC20, Ownable {
    ...
    uint256 public _maxTax;
    uint256 public _minTax;
    ...
    constructor (string memory _n, string memory _s, uint8 _d, uint256 _ts, uint256 _tax, uint256 _bb, uint256 _mkt, address _ma, uint256 _maxTx, uint _minTx) {
    ...
    function _transfer(
        ...
        require(amount >= _minTax * 10 ** _decimals, "Transfer amount is smaller than the minimum tax amount.");
    ...
    function calculateTaxFee(uint256 _amount) private view returns (uint256) {
        uint256 taxAmount = _amount.mul(_taxFee).div(10**3);

        if (_taxFee != 0) {
            if (taxAmount > (_maxTax * 10 ** _decimals)) 
                taxAmount = _maxTax * 10 ** _decimals;

            if (taxAmount < (_minTax * 10 ** _decimals)) 
                taxAmount = _minTax * 10 ** _decimals;
        }
    ...
    function setMaxTax(uint256 maxTax) external onlyOwner() {
        _maxTax = maxTax;
    }

    function setMinTax(uint256 minTax) external onlyOwner() {
        _minTax = minTax;
    }
