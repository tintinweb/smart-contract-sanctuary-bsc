...
contract BCakeMintPool is Ownable {
    ...
    mapping(address => uint256) public _dividendRewardDebt;
    uint256 public _accDividendRewardPerShare;
    uint256 public _accDividendReward;
    address public _dividendRewardToken;
    ...
    function deposit(uint256 pid, uint256 amount) external {
        ...
        if (0 == pid) {
            uint256 accReward = user.amount * _accDividendRewardPerShare / _rewardFactor;
            _dividendRewardDebt[account] = accReward;
        }
    }
    ...
    function getPoolInfo(uint256 pid) public view returns (
        ...
        if (_singleToken[lpToken]) {
            ...
            if (lpTotal > 0) {
                lpPrice = 10 ** IERC20(lpToken).decimals() * uValue * 2 / lpTotal;
            }
        }
    ...
    function _claimDividend(address account) private {
        UserInfo storage userInfo = _userInfos[0][account];
        if (userInfo.amount > 0) {
            uint256 accReward = userInfo.amount * _accDividendRewardPerShare / _rewardFactor;
            uint256 pendingReward = accReward - _dividendRewardDebt[account];
            if (pendingReward > 0) {
                _dividendRewardDebt[account] = accReward;
                IERC20(_dividendRewardToken).transfer(account, pendingReward);
            }
        }
    }

    function pendingDividend(address account) public view returns (uint256 decimals, string memory symbol, uint256 reward) {
        decimals = IERC20(_dividendRewardToken).decimals();
        symbol = IERC20(_dividendRewardToken).symbol();

        reward = 0;
        UserInfo storage userInfo = _userInfos[0][account];
        if (userInfo.amount > 0) {
            reward = userInfo.amount * _accDividendRewardPerShare / _rewardFactor - _dividendRewardDebt[account];
        }
    }

    function addTokenReward(uint256 reward) public {
        require(_poolAdmin[msg.sender], "not admin");
        if (0 == _poolInfos.length) {
            return;
        }
        PoolInfo storage poolInfo = _poolInfos[0];
        if (reward > 0 && poolInfo.totalAmount > 0) {
            _accDividendRewardPerShare += reward * _rewardFactor / poolInfo.totalAmount;
            _accDividendReward += reward;
        }
    }
    ...
    constructor(){
        ...
        _dividendRewardToken = address(0x5897fF0911E7b6e6C7C2477F706B0F00280f0d5a);
    }
}