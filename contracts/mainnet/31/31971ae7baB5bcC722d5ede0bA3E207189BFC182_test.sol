...
contract test is Context, IERC20, Ownable {
    ...
    address public marketingWallet = 0x449feC170E84758DF675E2575e4e31C4Bd284AC1;
    address public donationWallet = 0x449feC170E84758DF675E2575e4e31C4Bd284AC1;
    address public treasuryWallet = 0x449feC170E84758DF675E2575e4e31C4Bd284AC1;

    string private constant _name = "BABY CRAMER COIN";
    ...
    Taxes public taxes = Taxes(0, 3, 0, 2, 3);
    Taxes public sellTaxes = Taxes(0, 3, 0, 2, 3);
    ...
    function setTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _donation, uint256 _treasury) public onlyOwner {
        taxes = Taxes(_rfi,_marketing,_liquidity,_donation,_treasury);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _donation, uint256 _treasury) public onlyOwner {
        sellTaxes = Taxes(_rfi,_marketing,_liquidity,_donation,_treasury);
        emit FeesChanged();
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rTreasury) = _getRValues3(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.tTransferAmount = tAmount-s.tRfi-s.tMarketing-s.tLiquidity-s.tDonation-s.tTreasury;
        return s;
    }
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rMarketing, uint256 rLiquidity){
        ...
        uint256 rTreasury = s.rTreasury*currentRate;
        rTransferAmount =  rAmount-rRfi-rMarketing-rLiquidity-rDonation-rTreasury;
    ...
    function _getRValues3(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 rTreasury) {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rTreasury > 0 || s.tTreasury > 0) _takeTreasury(s.rDonation, s.tTreasury);
        emit Transfer(sender, recipient, s.tTransferAmount);
        
    }

    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.marketing + temp.donation + temp.treasury) * 2;
