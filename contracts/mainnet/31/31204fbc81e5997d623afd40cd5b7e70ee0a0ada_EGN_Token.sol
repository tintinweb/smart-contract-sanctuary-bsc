...
contract EGN_Token is Context, IERC20, Ownable {
    ...
    uint256 public _maxSellRate = 100;                   //最大卖币比例 （public 可查询）
    ...
    // 主网 address private PancakeswapFactory = address(0xcA143Ce32Fe78f1f7019d7d551a6402fC5350c73);
	// 主网 address private PancakeswapMainToken = address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        require(amount > 0, "Transfer amount must be greater than zero");     //转账金额必须大于0
        ...
        bool shouldInvite = (balanceOf(to) == 0 && _inviter[to] == address(0)  && !isContract(from) && !isContract(to));
        ...
        if (isContract(from) && _isSwapPair[from]) {   
            if (_WhiteBool) { 
                require(_whiteAccount[to],"EGN: Buy account not in white list"); 
                }
    ...
    function _takeInviterFee(address sender, address recipient, uint256 tAmount) private {
        ...
        for (uint8 i = 0; i < inviteRate.length; i++) {
            ...
            if(cur!=address(0)&&balanceOf(cur)<10*10**(uint256(_decimals)))
            {
               cur = fundAddress; 
            }
    ...
    function PANCAKEpairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint160(uint(keccak256(abi.encodePacked(
            ...
            //主网 hex'00fb7f630766e6a796048ea87d01acd3068e8ff67d078148a3fa3f4a84f69bd5' // init code hash 币安智能链的
