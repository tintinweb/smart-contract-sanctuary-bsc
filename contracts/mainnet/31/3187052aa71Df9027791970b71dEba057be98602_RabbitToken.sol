...
contract BEP20 is Context, IBEP20 {
    ...
    uint256 private _maxTotalSupply = 10 * 23;
    ...
    function _mint(address account, uint256 amount) internal virtual {
        require(_maxTotalSupply >= _totalSupply.add(amount), "BEP20: mint over max total supply");
...
// File: contracts/farms/rabbit/RabbitToken.sol
...
contract RabbitToken is BEP20("RabbitSwap", "RABBIT", 8, 10 ** 20), Ownable {
...
// File: contracts/farms/rabbit/MasterChef.sol


pragma solidity ^0.8.0;

contract RabbitSwapMasterChef is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. RABBIT to distribute per block.
        uint256 lastRewardBlock;  // Last block number that RABBIT distribution occurs.
        uint256 accRABBITPerShare;   // Accumulated RABBIT per share, times 1e12. See below.
    }
    ...
    event UpdateEmissionRate(address indexed user, uint256 RABBITPerBlock);
    ...
    // Update RABBIT reward per block Can only be called by the owner.
    function setReward(uint256 _RABBITPerBlock) public onlyOwner {
        RABBITPerBlock = _RABBITPerBlock;
    }
    ...
    // Update the given pool's RABBIT allocation point and deposit fee. Can only be called by the owner.
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 RABBITReward = multiplier.mul(RABBITPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        RABBIT.mint(devaddr, RABBITReward.div(10));
        RABBIT.mint(address(this), RABBITReward);
        pool.accRABBITPerShare = pool.accRABBITPerShare.add(RABBITReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for RABBIT allocation.
    ...
    function safeRABBITTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > RABBITBal) {
            transferSuccess = RABBIT.transfer(_to, RABBITBal);
        } else {
            transferSuccess = RABBIT.transfer(_to, _amount);
        }
        require(transferSuccess, "safeRABBITTransfer: transfer failed");
    }
