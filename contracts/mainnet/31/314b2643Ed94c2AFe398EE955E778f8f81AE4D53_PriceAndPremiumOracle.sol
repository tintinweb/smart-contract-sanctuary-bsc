...
contract PriceAndPremiumOracle {
    ...
    function getPremium() public view returns(uint256) {
        return (getRewardUsd()+ebContract.premiumCollected()) * 2 / 3 / ebContract.totalShareCount() / 1e12 * 1e12; // 33% of reward is used for auto re-investment. Don't care about the decimals corresponding to after .yy
    }
}