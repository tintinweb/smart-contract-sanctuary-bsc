...
contract DrawManager is VRFConsumerBase, ReentrancyGuard, Ownable {
    ...
    uint256 public minimumBuyAmount = 1000000000000000000;
    ...
    function calculateTicketCount(uint256 _amount) public view returns (uint256) {
        (uint256 reserveA, uint256 reserveB, ) = pancakeswapBusdPair.getReserves();
        (uint256 xoowReserve, uint256 busdReserve) = address(xoow) < address(BUSD)
            ? (reserveA, reserveB)
            : (reserveB, reserveA);
    ...
    function draw() external {
        ...
        for (uint256 i = 0; i < roundWinnerCount; i++) {
            ...
            xoow.transfer(winners[i], award);

            emit AwardUser(roundCount, winnerTicketNumbers[i], winners[i], award);
        }

        xoow.transfer(_protocolFeeTo, _currentAward.mul(10).div(100));
    ...
    function setProtocolFeeTo(address _address) external onlyOwner {
...
contract Xoow is Context, IBEP20 {
    ...
    string private _name = 'XoowTest';
    string private _symbol = 'XOOWT';
