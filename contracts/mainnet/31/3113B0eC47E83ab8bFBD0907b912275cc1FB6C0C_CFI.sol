...
contract CFI is IBEP20{
    ...
    //mapping(address=>mapping(uint=>UserDeposit)) public map_user_deposits;
    ...
    {
        uint noOfTokens = buyPrice()*amount; //Not dividing by 10**18 because both tokens have 18 decimal points which cancel each other
    ...
    function WithdrawHolding(uint tokenAmount) external
    {
        ...
        uint amountOfCake = tokenAmount/sellPrice(userAddress);
    ...
    // function getTotalROI(address userAddress) internal view returns(uint)
    // {
    //     uint TotalROI = 0;
    //     for(uint i=1; i<=map_users[userAddress].DepositsCount; i++)
    //     {
    //         uint depositAmount = map_user_deposits[userAddress][i].Amount;
    //         uint timestamp = map_user_deposits[userAddress][i].Timestamp;

    //         uint timeElapsed = block.timestamp-timestamp;

    //         TotalROI += (timeElapsed*(depositAmount*65*2)/(100*100*30*24*60*60)); // Calculated on 65% of deposit, 2% monthly
    //     }
    //     return TotalROI;
    // }
