...
contract MonstaPartyP2E is OwnableUpgradeable {
  ...
  function upgrade() public onlyOwner {
    ADDRESS_SWAP_ROUTER = 0xcF0feBd3f17CEf5b47b0cD257aCf6025c5BFf3b7;
  }
  ...
  function attemptSteal(uint256 attackerTokenId, uint256 victimTokenId) external {
    ...
    if (
      ...
      currentStealWinning[victimTokenId] = 88888;
  ...
  function getStealProgress(uint256 victimTokenId) public view returns (uint256) {
    ...
    if (currentStealWinning[victimTokenId] == 88888) {
      return 2; // Wait for result
    }

    return 3; // Steal finished
  }

  function steal(uint256 attackerTokenId, uint256 victimTokenId) external {
    require(attackerTokenId > 0, 'TokenID 0 not allowed');

    require(getStealProgress(victimTokenId) == 3, 'Stealing not available');
  ...
  function canSteal(
    ...
    if (
      getStealProgress(victimTokenId) > 1 &&
      currentSteals[victimTokenId] >= block.timestamp - 30 minutes
    ) {
  ...
  // Get players
  function getRamPlayers() public view returns (uint256[] memory, uint256[] memory) {
    uint256 size = currentStealParticipants[lastVictimTokenId].length;
    uint256[] memory players = new uint256[](size);
    uint256[] memory pxp = new uint256[](size);

    for (uint i = 0; i < size; i++) {
      players[i] = currentStealParticipants[lastVictimTokenId][i];
      pxp[i] = infos[players[i]].xp;
    }
  ...
  // Set winner
  function finishGame(uint256 winnerTokenId) external {
    require(_msgSender() == 0x2607C669B7b8B4189083165FE07913ddb3b3a7AD, 'Unauthorized call');
    require(currentSteals[lastVictimTokenId] > block.timestamp - 30 minutes, 'Too late');
    require(block.timestamp >= currentSteals[lastVictimTokenId] + 15 minutes, 'Too early');
    require(currentStealWinning[lastVictimTokenId] == 88888, 'Winner already set');

    currentSteals[lastVictimTokenId] = block.timestamp - 15 minutes;
    currentStealWinning[lastVictimTokenId] = winnerTokenId;
  }
  ...
  function getGameState(
    uint256 attackerTokenId,
    uint256 victimTokenId
  ) external view returns (GameState memory state) {
  ...
  function _share(uint256 a, uint256 b, uint256 c) private pure returns (uint256) {
    return
