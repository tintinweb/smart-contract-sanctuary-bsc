...
contract Platinum is ERC20, Ownable {
    ...
    PlatinumDividendTracker public dividendTracker;

    address public constant deadWallet = 0x000000000000000000000000000000000000dEaD;
    address public marketingWallet = 0x216B9DDF343456EDb8195a5528c96E9Fb642d794;
    address public teamWallet = 0x7D7b16822b92b4106FccEBB0aBd1c00F280367ec;
    address public devWallet = 0x16A87866Cc8137157a188B6c7AeB998Ff435a2EB;

    uint256 public swapTokensAtAmount = 150_000 * 10**9;
    uint256 public maxWalletBalance = 1_000_000 * 10**9;
    uint256 public maxBuyAmount = 1_000_000 * 10**9;
    ...
    Taxes public buyTaxes = Taxes(3, 6, 1, 1, 2);
    Taxes public sellTaxes = Taxes(3, 6, 1, 1, 2);
    ...
    constructor() ERC20("Platinum Finance", "PLATINUM") {
        dividendTracker = new PlatinumDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        PlatinumDividendTracker newDividendTracker = PlatinumDividendTracker(
            payable(newAddress)
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "Platinum: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
    ...
    function setBuyTaxes(uint256 _rewards, uint256 _marketing, uint256 _team, uint256 _dev, uint256 _liquidity) external onlyOwner{
        require((_rewards + _marketing + _team +  _dev +_liquidity ) <= 25, "Must keep fees at 25% or less");
        buyTaxes = Taxes(_rewards, _marketing, _team,  _dev, _liquidity);
    }

    function setSellTaxes(uint256 _rewards, uint256 _marketing, uint256 _team, uint256 _dev, uint256 _liquidity) external onlyOwner{
        require((_rewards + _marketing + _team + _dev + _liquidity ) <= 25, "Must keep fees at 25% or less");
        require(_dev > 0, "Dev tax should be greater than 0");
        sellTaxes = Taxes(_rewards, _marketing, _team, _dev, _liquidity);
    }

    function setMaxBuyLimits(uint256 maxBuy) external onlyOwner{
        require(maxBuy >= 50000, "Cannot set max buy amount lower than 0.05%");
        maxBuyAmount = maxBuy * 10**decimals();
    }

    function setMaxSellLimits(uint256 maxSell) external onlyOwner{
        require(maxSell >= 50000, "Cannot set max sell amount lower than 0.05%");
        maxSellAmount = maxSell * 10**decimals();
        
    }

    function setMaxWallet(uint256 maxWallet) external onlyOwner {
        require(maxWallet >= 50000, "Cannot set max wallet amount lower than 0.05%");
    ...
    function _setAutomatedMarketMakerPair(address newPair, bool value) private {
        require(
            ...
            "Platinum: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[newPair] = value;
    ...
    function setGasForProcessing(uint256 newValue) external onlyOwner {
        require(
            ...
            "Platinum: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "Platinum: Cannot update gasForProcessing to same value"
        );
    ...
    function swapAndLiquify(uint256 tokens, uint256 swapTax) private {
        ...
        uint256 teamWalletAmt = unitBalance * 2 * sellTaxes.team;
        if (teamWalletAmt > 0) {
...
contract PlatinumDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor()
        DividendPayingToken("Platinum_Dividen_Tracker", "Platinum_Dividend_Tracker")
    {
        claimWait = 3600;
    ...
    function _transfer(
        ...
        require(false, "Platinum_Dividend_Tracker: No transfers allowed");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "Platinum_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "Platinum_Dividend_Tracker: Cannot update claimWait to same value"
        );
