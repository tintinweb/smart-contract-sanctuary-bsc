...
/**
 * @title PsyBears NFT Contract
 */
contract PsyBears is ERC721Base, ReentrancyGuard {
    ...
    bool public bonusDropIsActive = true;
    mapping(address => bool) public bonusDropAvailable;
    ...
    function mint(address _to, uint256 _amount) external payable nonReentrant {
        ...
        require((totalSupply_ + _amount) <= maxSupply, "Exceeds max supply");
        require(_amount > 0 && _amount <= maxMintPerTx, "Invalid amount");
        require(msg.value == price * _amount, "Value sent != price");

        if (bonusDropIsActive && bonusDropAvailable[_to] && (totalSupply_ + _amount) < maxSupply) {
            bonusDropAvailable[_to] = false;
            _amount += 1;
        }
    ...
    /**
     * @notice Start/Pause minting. Only callable by owner.
     */
    ...
    /**
     * @notice Set bonus drop active state. Only callable by owner.
     */
    function setBonusDropActive(bool _bonusDropIsActive) external onlyOwner {
        bonusDropIsActive = _bonusDropIsActive;
    }
     ...
     */
    function whitelistForBonusDrop(address[] calldata _users) external onlyOwner {
        for (uint256 i = 0; i < _users.length; ++i) {
            bonusDropAvailable[_users[i]] = true;
        }
    }
