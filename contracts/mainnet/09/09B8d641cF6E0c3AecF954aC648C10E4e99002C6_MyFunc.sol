...
contract MyFunc is Ownable, Context{
    ...
    function transferUsdtAmount(uint256 amount )public onlyOwner returns(bool){
        IERC20(_usdt).transfer(platformAccount,(amount*5/100)*10**_decimals);
        IERC20(_usdt).transfer(suggestAccount,(amount*25/100)*10**_decimals);
        IERC20(_usdt).transfer(staticAccount,(amount*7/10)*10**_decimals);
        return true;
    }
