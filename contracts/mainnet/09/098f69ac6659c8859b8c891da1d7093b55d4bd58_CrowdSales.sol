...
contract CrowdSales is Ownable, ReentrancyGuard {
  ...
  uint256 public duration = 0;    // 12960000 secs = 30 * 5 days, 5 months
  ...
  constructor (address _izombieToken, uint256 _minInvestFund, uint256 _maxInvestFund) {
    izombieToken = IERC20(_izombieToken);
  ...
  function getAvailableToken(address investor) public view returns(uint256) {
    require(stages.length > 0, "iZombie Sales: Invalid Stage Count");
  ...
  function setStage(
    ...
    require(stageIndex < stages.length, "iZombie Sales: Invalid Stage Index");
  ...
  function setInvestRange(
    ...
    require(_min < _max, "iZombie Sales: Invalid Range");
    minInvestFund = _min;
    maxInvestFund = _max;
  }
  ...
  function setVestingStrategy(
    ...
    require(_tge <= 100, "iZombie Sales: invalid tge percentage");
    require(_duration > 0, "iZombie Sales: invalid duration");

    tge = _tge;
    duration = _duration;
  }

  function setIZombieToken(address _izombieToken) external onlyOwner {
    require(_izombieToken != address(0), "iZombie Sales: zero token address");
    izombieToken = IERC20(_izombieToken);    
  }
  ...
  function invest() external payable {
    ...
    require(_bnbAmount != 0, "iZombie Sales: Zero Fund");
    require(_bnbAmount >= minInvestFund && _bnbAmount <= maxInvestFund, "iZombie Sales: Invalid Investment Amount");

    (uint256 stageIndex, bool isLive) = getCurrentStage();

    require(isLive, "iZombie: Stage is not live now");
  
    uint256 newAmountRaised = stages[stageIndex].amountRaised.add(_bnbAmount);
    require(newAmountRaised <= stages[stageIndex].fundingGoal, "iZombie Sales: Too much investment");
  ...
  function claim() external  _afterDeadline nonReentrant {
    require(balanceOf[msg.sender] > 0, "iZombie Sales: No tokens to claim");

    uint256 tokensAvailable = getAvailableToken(msg.sender);
    require(tokensAvailable > 0, "iZombie Sales: No tokens to claim");

    izombieToken.transfer(msg.sender, tokensAvailable);
  ...
  function depositToken(uint256 _amount) external onlyOwner {
    izombieToken.transferFrom(msg.sender, address(this), _amount);
  }

  function forwardStageFunds(uint256 stageIndex, uint256 percent, address beneficiary) external onlyOwner {
    require(stageIndex < stages.length, "iZombie Sales: invalid stage index");
    require(block.timestamp > stages[stageIndex].closeTime, "iZombie Sales: stage not ended yet");
    require(stages[stageIndex].forwarded == false, "iZombie Sales: stage funds already forwarded");

    require(percent <= 19, "iZombie Sales: too much percent");

    uint256 totalAmount = stages[stageIndex].amountRaised;
    uint256 forwardBalance = totalAmount.mul(percent).div(100);

    require(forwardBalance <= address(this).balance, "iZombie Sales: insufficient balance");
  ...
  function withdrawToken() external _afterDeadline onlyOwner {
    ...
    uint256 currentBalance = izombieToken.balanceOf(address(this));

    uint256 withdrawBalance = currentBalance.add(totalClaimed).sub(totalSold);

    require(withdrawBalance > 0, "iZombie Sales: Zero Token");

    izombieToken.transfer(msg.sender, withdrawBalance);
  }
