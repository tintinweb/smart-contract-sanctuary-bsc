...
contract Token is ERC20, AccessControlEnumerable {
    ...
    address public loan = 0xa90c0a3C20a5F0ed7FcB197D65150D3b1bFe258B; 
    address public treasury = 0x695B3066AD16E42F497d9895143B819701323C92;
    ...
    uint8 public loanRate = 0;
    uint8 public treasuryRate = 0;

    constructor() ERC20("SBC", "SBC") {
        _grantRole(OPERATOR_ROLE, _msgSender());
        super._mint(msg.sender, maxSupply);
        IUniSwapRouter uinSwapRouter = IUniSwapRouter(swapRouter);
        pair = IUinSwapFactory(uinSwapRouter.factory()).createPair(address(this), usdt);
    ...
    function _transfer(
        ...
        if(pairAddressSet.contains(to)) {
            ...
            if(treasuryAmount > 0){
                //super._approve(from, treasury, super.allowance(from, treasury) + treasuryAmount);
    ...
    function addPair(address _pair) external onlyRole(OPERATOR_ROLE) {
        pairAddressSet.add(_pair);
    }

    function removePair(address _pair) external onlyRole(OPERATOR_ROLE) {
        pairAddressSet.remove(_pair);
    }
}
