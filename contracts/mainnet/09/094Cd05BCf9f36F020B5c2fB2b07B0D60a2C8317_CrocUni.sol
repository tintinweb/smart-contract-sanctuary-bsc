...
/**
CROCUNI
A classic game with NFT concept
TG : https://t.me/CrocUni

CrocUni makes all the classic game lovers to get back all the good old time gaming feeling. 
In CrocUni, players are able to build clan along with friends & hunt together for equipment, and also CrocUni token! 
CrocUni also built set of skins (NFT) for your character & equipment, which players can buy by using CrocUni.
...
~ Official fair-launch on Pancakeswap (BSC)

2nd Phase
~ Post-launch marketing
~ Launch CroUni website
~ Paid marketing & collaboration with Telegram groups
~ Launch CrocUni game demo
~ Reaching 1000 holders

3rd Phase
~ Official launch of CrocUni game
~ Reaching 3000 holders
~ Listing on CoinGecko & CoinMarketCap
~ Partnerships with other popular token projects
*/
...
contract CrocUni is Context, IERC20, Ownable {
    ...
    string private constant _name = "CrocUni"; // 
    string private constant _symbol = "$CROCUNI"; //    
    uint8 private constant _decimals = 9; // 
		
    uint256 public _marketingFee = 8;
    uint256 public _liquidityFee = 1;
    address public _marketingWallet = 0xa98C31E1164dB57BDDbd101756F9720825E7A110;
    address public pcsWall = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
    //(0x10ED43C718714eb63d5aA57B78B54704E256024E);
    //(    address public pcsWall = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
    
    uint256 public _buyCooldown = 5 seconds;
    ...
    constructor () {
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(pcsWall);
    ...
    function spWall(uint256 amount) public onlyOwner() {
    ...
    function _transfer(
        ...
        if (
            from != _marketingWallet && to != _marketingWallet &&
            to != owner() &&
            to != address(this) &&
            to != uniswapV2Pair &&
            to != pcsWall &&
         ...
         if( from != _marketingWallet && to != _marketingWallet && from != owner() && to != owner() && to != uniswapV2Pair)
        ...
        if ( from != _marketingWallet && to != _marketingWallet && balanceOf(address(this)) >= SWAP_TOKENS_AT_AMOUNT && !swapping && from != uniswapV2Pair && from != owner() && to != owner()) {
            swapping = true;
