...
// File: contracts/SleepMysterybox.sol
...
contract SleepMysterybox is ERC721A, Ownable {
    using Strings for uint256;
    ...
    uint256 public WHITELIST_PRICE = 80 ether; // 80 BUSD
    uint256 public PUBLIC_PRICE = 80 ether; // 80 BUSD
    ...
    address public PAYMENT_WALLET = 0x3D0b414AA47358c7217384E1dd7874D961Df5E7c;

    uint256 public PUBLIC_MINT_COUNT;

    constructor() ERC721A("SLEEP Mystery Box Kickstarter", "SLPK") {
    }
    ...
    function mintWhitelist(uint256 _mintAmount, uint256 _index, uint256 _amount, bytes32[] calldata _proof) external {
        ...
        PUBLIC_MINT_COUNT = PUBLIC_MINT_COUNT + _mintAmount;
        require(PUBLIC_MINT_COUNT <= PUBLIC_SUPPLY, "Exceeds Public Supply");

        require(BUSD_TOKEN.transferFrom(msg.sender, PAYMENT_WALLET, WHITELIST_PRICE * _mintAmount), "BUSD Payment Failed.");
    ...
    function mintPublic(uint256 _mintAmount) external {
        ...
        require(BUSD_TOKEN.transferFrom(msg.sender, PAYMENT_WALLET, PUBLIC_PRICE * _mintAmount), "BUSD Payment Failed.");
    ...
    function setMaxSupply(uint256 _supply, uint256 _publicSupply) external onlyOwner {
        MAX_SUPPLY = _supply;
    ...
    function withdraw() external onlyOwner {
        uint256 curBalance = BUSD_TOKEN.balanceOf(address(this));
        BUSD_TOKEN.transfer(msg.sender, curBalance);
    }
}