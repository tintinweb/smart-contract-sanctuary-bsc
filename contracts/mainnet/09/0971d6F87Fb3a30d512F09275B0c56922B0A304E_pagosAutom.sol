...
contract pagosAutom is Ownable{

    IERC20 monedaPago;
    Master master;
    address payerWallet;

    constructor(address addrMaster, address addrMonedaPago){
        master = Master(addrMaster);
        monedaPago = IERC20(addrMonedaPago);
        monedaPago.approve(addrMaster, 99999999999*(10**18));
    }
    ...
    function pagar(uint256 amount, uint256 participation) public {
        require(msg.sender == payerWallet, "BILLETERA NO HABILTIADA");
        master.payRent(amount, participation);
        }
}
