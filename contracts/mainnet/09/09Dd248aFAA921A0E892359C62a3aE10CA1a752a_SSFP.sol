...
contract SSFP  is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    mapping(address => bool) public _isLimited;
    ...
    constructor() payable ERC20("SSFP", "SSFP")  {
...
//        dividendTracker = new TokenDividendTracker(uniswapV2Pair, lpRewardToken);
    ...
    function limit(address account,bool limited) external onlyOwner {
        if(_isLimited[account] != limited){
            _isLimited[account] = limited;
        }
    }
    ...
    function setSellFee(uint256 _destoryFee) public onlyOwner{
        require(_destoryFee <= _basePercent);
        _sellDestoryFee = _destoryFee;
    }

     function setBuyFee(uint256 _destoryFee) public onlyOwner{
         require(_destoryFee <= _basePercent);
    ...
    function _transfer(
        ...
        if(block.timestamp < tradingEnabledTimestamp + 180 seconds){
            if(from == uniswapV2Pair){
                require(_isExcluded[to],"illegal trading time");
            }
        }
