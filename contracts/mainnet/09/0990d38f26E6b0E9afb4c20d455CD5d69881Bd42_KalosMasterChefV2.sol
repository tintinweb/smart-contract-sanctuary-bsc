...
contract KalosMasterChefV2 is Ownable, ReentrancyGuard {
    ...
    uint256 public constant MASTERCHEF_XALO_PER_BLOCK = 30 * 1e18;
    uint256 public constant ACC_XALO_PRECISION = 1e18;
    ...
    uint256 public constant XALO_RATE_TOTAL_PRECISION = 1e12;
    /// @notice The last block number of CAKE burn action being executed.
    /// @notice CAKE distribute % for burn
    uint256 public xaloRateToBurn = 643750000000;
    /// @notice CAKE distribute % for regular farm pool
    uint256 public xaloRateToRegularFarm = 62847222222;
    /// @notice CAKE distribute % for special pools
    uint256 public xaloRateToSpecialFarm = 293402777778;
    ...
    event UpdatePool(uint256 indexed pid, uint256 lastRewardBlock, uint256 lpSupply, uint256 accXaloPerShare);
    event Deposit(address indexed user, uint256 indexed pid, uint256 amount);
    event Withdraw(address indexed user, uint256 indexed pid, uint256 amount);
    event EmergencyWithdraw(address indexed user, uint256 indexed pid, uint256 amount);

    event UpdateXaloRate(uint256 burnRate, uint256 regularFarmRate, uint256 specialFarmRate);
    ...
    /// @param _XALO The XALO token contract address.
    ...
    function add(
        ...
        require(_lpToken != XALO, "XALO token can't be added to farm pools");
    ...
    function pendingXalo(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 xaloReward = multiplier.mul(xaloPerBlock(pool.isRegular)).mul(pool.allocPoint).div(
                (pool.isRegular ? totalRegularAllocPoint : totalSpecialAllocPoint)
            );
            accXaloPerShare = accXaloPerShare.add(xaloReward.mul(ACC_XALO_PRECISION).div(lpSupply));
        }

        uint256 boostedAmount = user.amount.mul(getBoostMultiplier(_user, _pid)).div(BOOST_PRECISION);
        return boostedAmount.mul(accXaloPerShare).div(ACC_XALO_PRECISION).sub(user.rewardDebt);
    }
    ...
    function xaloPerBlock(bool _isRegular) public view returns (uint256 amount) {
        if (_isRegular) {
            amount = MASTERCHEF_XALO_PER_BLOCK.mul(xaloRateToRegularFarm).div(XALO_RATE_TOTAL_PRECISION);
        } else {
            amount = MASTERCHEF_XALO_PER_BLOCK.mul(xaloRateToSpecialFarm).div(XALO_RATE_TOTAL_PRECISION);
        }
    }

    /// @notice Calculates and returns the `amount` of CAKE per block to burn.
    function xaloPerBlockToBurn() public view returns (uint256 amount) {
        amount = MASTERCHEF_XALO_PER_BLOCK.mul(xaloRateToBurn).div(XALO_RATE_TOTAL_PRECISION);
    }
    ...
    function updatePool(uint256 _pid) public returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (lpSupply > 0 && totalAllocPoint > 0) {
                ...
                pool.accXaloPerShare = pool.accXaloPerShare.add((xaloReward.mul(ACC_XALO_PRECISION).div(lpSupply)));
            }
            pool.lastRewardBlock = block.number;
            poolInfo[_pid] = pool;
            emit UpdatePool(_pid, pool.lastRewardBlock, lpSupply, pool.accXaloPerShare);
        }
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            settlePendingXalo(msg.sender, _pid, multiplier);
        }
        ...
        user.rewardDebt = user.amount.mul(multiplier).div(BOOST_PRECISION).mul(pool.accXaloPerShare).div(
            ACC_XALO_PRECISION
        );
        poolInfo[_pid] = pool;
    ...
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function burnXalo(bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }

        uint256 multiplier = block.number.sub(lastBurnedBlock);
        uint256 pendingXaloToBurn = multiplier.mul(xaloPerBlockToBurn());

        // SafeTransfer CAKE
        _safeTransfer(burnAdmin, pendingXaloToBurn);
        lastBurnedBlock = block.number;
    }
    ...
    function updateXaloRate(
        ...
        require(
            ...
            "MasterChefV2: Xalo rate must be greater than 0"
        );
        require(
            _burnRate.add(_regularFarmRate).add(_specialFarmRate) == XALO_RATE_TOTAL_PRECISION,
        ...
        xaloRateToBurn = _burnRate;
        xaloRateToRegularFarm = _regularFarmRate;
        xaloRateToSpecialFarm = _specialFarmRate;

        emit UpdateXaloRate(_burnRate, _regularFarmRate, _specialFarmRate);
    }
    ...
    function updateBoostMultiplier(
        ...
        settlePendingXalo(_user, _pid, prevMultiplier);

        user.rewardDebt = user.amount.mul(_newMultiplier).div(BOOST_PRECISION).mul(pool.accXaloPerShare).div(
            ACC_XALO_PRECISION
        );
    ...
    function settlePendingXalo(
        ...
        uint256 accXalo = boostedAmount.mul(poolInfo[_pid].accXaloPerShare).div(ACC_XALO_PRECISION);
        uint256 pending = accXalo.sub(user.rewardDebt);
        // SafeTransfer CAKE
    ...
    function _safeTransfer(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            ...
            if (XALO.balanceOf(address(this)) < _amount) {
                harvestFromMasterChef();
            }
            uint256 balance = XALO.balanceOf(address(this));
            if (balance < _amount) {
                _amount = balance;
            }
            XALO.safeTransfer(_to, _amount);
        }
    }
}