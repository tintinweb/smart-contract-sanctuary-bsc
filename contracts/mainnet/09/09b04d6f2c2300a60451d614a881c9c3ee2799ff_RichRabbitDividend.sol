...
contract RichRabbitDividend is Ownable{
    ...
    uint256 public genesisSupply = 600;
    mapping (uint256 => Tokens) public accounts;
    mapping (uint256 => Passes) public accountsGenesis;
    struct Tokens{
        uint256 amount;
    }
    struct Passes{
   ...
   //redeem Dividends
    ...
    function setGenesisSupply(uint256 genesisSupply_) external onlyOwner returns (bool) {
        genesisSupply = genesisSupply_;
        return true;
    }

    function setTotalSupply(uint256 totalSupply_) external onlyOwner returns (bool) {
    ...
    function disqualifyGenesis(uint256 tokenId_) external onlyOwner returns(bool) {
    ...
    function qualifyGenesis(uint256 tokenId_) external onlyOwner returns(bool) {
        require(!accountsGenesis[tokenId_].dead);
