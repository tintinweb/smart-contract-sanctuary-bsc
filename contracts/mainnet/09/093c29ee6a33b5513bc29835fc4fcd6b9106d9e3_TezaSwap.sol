...
contract TezaSwap is Ownable {
    ...
    uint256 public exchangeRate=277777*10**16;
    event Bought(uint256 amount);
    event Sold(uint256 amount);
    event TransferSent(address _from, address _destAddr, uint _amount);
    IBEP20 private immutable token;
    IBEP20 private immutable usdt;
    ...
    constructor(address _BEP20Address, address _USDTAddress) {
        token = IBEP20(_BEP20Address);
        usdt = IBEP20(_USDTAddress);
    }

    function buy(uint256 usdtAmount) public {
        uint256 decimalAmount = usdtAmount*10**18;
        uint256 amountTobuy = (usdtAmount*exchangeRate);
        uint256 allowance = usdt.allowance(msg.sender, address(this));
        uint256 dexBalance = token.balanceOf(address(this));
        require(allowance >= usdtAmount, "Check USDT allowance");
        require(amountTobuy > 0, "You need to send some USDT");
        require(amountTobuy <= dexBalance, "Not enough tokens in the reserve");
        usdt.transferFrom(msg.sender, owner(), decimalAmount);
