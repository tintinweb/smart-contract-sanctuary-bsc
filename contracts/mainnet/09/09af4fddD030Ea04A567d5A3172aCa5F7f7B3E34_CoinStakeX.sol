...
contract CoinStakeX {
    ...
    string public name = "CoinStakeX";
    
    // create 2 state variables
    address public CoinWars = 0xf6aD4943dD43EE24f14E56d03C4055Bd89b331D1;
    uint public reward_tax = 75;
    ...
    constructor() {
        owner = msg.sender;
        is_farmable[CoinWars] = true; // CHANGE ON MAINNET

        set_multiplier(7 days, 260);
        set_multiplier(15 days, 288);
        set_multiplier(30 days, 301);

        set_time_allowed(7 days, true, false, 260);
        set_time_allowed(15 days, true, false, 288);
        set_time_allowed(30 days, true, false, 301);

        coin_reward = IERC20(CoinWars);
    }
    ...
    function approveTokens() public {
        bool approved = IERC20(CoinWars).approve(address(this), 2**256 - 1);
    ...
    function farmTokens(uint _amount, uint locking) public {
        require(is_farmable[CoinWars], "Farming not supported");
        if (is_fixed_locking) {
            require(time_allowed[locking].enabled, "Locking time not allowed");
        } else {
            require(locking >= 1 days, "Locking time not allowed");
        }
        require(IERC20(CoinWars).allowance(msg.sender, address(this)) >= _amount, "Allowance?");

        if (time_allowed[locking].check_balance) {
            uint min_amount = IERC20(CoinWars).balanceOf(msg.sender) * time_allowed[locking].percentage_to_check / 100;
            require(_amount >= min_amount, "Pool not allowed for this amount of tokens.");
        }

        // Trasnfer farmable tokens to contract for farming
        bool transferred = IERC20(CoinWars).transferFrom(msg.sender, address(this), _amount);
        ...
        farming_unit[msg.sender][id].token = CoinWars;
        token_pool[CoinWars].total_balance += _amount;

        // Add user to farms array if they haven't farmd already
        if(token_pool[CoinWars].has_farmed[msg.sender]) {
            token_pool[CoinWars].has_farmed[msg.sender] = true;
        }

        // Update farming status to track
        token_pool[CoinWars].is_farming[msg.sender]++;
    ...
    function unfarmTokens(uint id) public safe cooldown {
        ...
        if (has_revenue) {  
          ...
          // transfer CoinWars tokens out of this contract to the msg.sender
          coin_reward.transfer(msg.sender, balance);
          coin_reward.transfer(msg.sender, farming_unit[msg.sender][id].balance);
        } else {
          // send tax
          uint b = farming_unit[msg.sender][id].balance * reward_tax;
          uint balance = b / 100;
          coin_reward.transfer(msg.sender, balance);
        }
  ...
  function get_pool() public view returns (uint) {
      require(is_farmable[CoinWars], "Not active");
      return(token_pool[CoinWars].total_balance);
  }
  ...
  function _calculate_rewards(uint id, address addy) public view returns (uint) {
    // get the users farming balance in CoinWars
      ...
      uint updated_reward_rate = reward_rate + lock_multiplier[CoinWars][locking_time] + time_allowed[locking_time].multiplier;
    ...
    function set_reward_tax(uint reward) public authorized {
        reward_tax = reward;
    }

    function set_farming_state(bool status) public authorized {
        is_farmable[CoinWars] = status;
    }

    function get_farming_state() public view returns (bool) {
        return is_farmable[CoinWars];
    }
    ...
    function get_reward_rate_timed(uint time) public view returns (uint) {
        uint reward_timed = reward_rate+lock_multiplier[CoinWars][time];
        return reward_timed;
    }
  ...
  function set_reward_rate(uint rate) public authorized {
    ...
    function set_reward_token(address token) public authorized {
        CoinWars = token;
        coin_reward = IERC20(CoinWars);
    }  

    function set_multiplier(uint time, uint multiplier) public authorized {
        lock_multiplier[CoinWars][time] = multiplier;
    }
  ...
  function set_is_fixed_locking(bool fixed_locking) public authorized {
    ...
    function get_multiplier(uint time) public view returns(uint) {
        return lock_multiplier[CoinWars][time];
    }
