...
contract Treasury{
    ...
    function swapForTokenDirect(address to, uint amount, uint minOut) external{
        require(msg.sender==token);
        Stablecoin.approve(address(router),amount);
        address[] memory path = new address[](2);
        path[1] = token;
...
contract Yieldify is IBEP20, Ownable {
    ...
    string public constant symbol = "YIFY";
    uint8 public constant decimals = 9;
    uint256 public constant InitialSupply=10**6 * 10**decimals;
    ...
    IBEP20 private Stablecoin=
     IBEP20(0x55d398326f99059fF775485246999027B3197955);//Mainnet
    ...
    function _transferTaxed(
        ...
        if(timeSinceLaunch<=2 minutes)
            tax=999-(999-taxes)*timeSinceLaunch/(2 minutes);
        else tax=taxes;
