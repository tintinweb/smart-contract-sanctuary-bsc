    ...
    ECDSAOffsetRecovery
{
    ...
    mapping(uint128 => uint256) public feeFractionOfBlockchain;
    mapping(uint128 => uint256) public feeNativeOfBlockchain;
    ...
    constructor(
        ...
        if(_maxTokenAmount == 0) 
            maxTokenAmount = type(uint256).max;
        else
    ...
    function transferToOtherBlockchain(
        ...
        require(
            amount >= minTokenAmount && amount <= maxTokenAmount,
    ...
    function transferToUserWithFee(
        ...
        require(
            amountETH == feeNativeOfBlockchain[numOfThisBlockchain],
            "swapContract: Wrong fee amount!"
        );
        require(
        ...
        require(
            concatSignatures.length / signatureLength >=
        ...
        uint256 fee = (amountWithFee *
            feeFractionOfBlockchain[numOfThisBlockchain]) / 100;
        ...
        if (amountETH > 0)
            TransferHelper.safeTransferETH(feeAddress, amountETH);
    ...
    /**
     * @dev Transfers useless tokens to admin
     * @param tokenAddressToReceive Address of token will be transfer
     */

    function getTokens(IERC20 tokenAddressToReceive) external onlyOwner {
        require(
            address(tokenAddressToReceive) != address(0) &&
                tokenAddressToReceive != tokenAddress,
            "swapContract: 0x00..."
        );
        address admin = _msgSender();
        uint256 amount = tokenAddressToReceive.balanceOf(address(this));
        if (amount > 0)
            TransferHelper.safeTransfer(
        ...
        if (amount > 0) TransferHelper.safeTransferETH(admin, amount);
    }
     ...
     */
    function setFeesOfBlockchain(
        uint128 blockchainNum,
        uint256 fraction,
        uint256 nativeFee
    ) external onlyOwnerAndManager {
        require(fraction <= 100, "swapContract: Fee fraction is greater 100%");
        feeFractionOfBlockchain[blockchainNum] = fraction;
        feeNativeOfBlockchain[blockchainNum] = nativeFee;
    }
