...
contract MRV is IERC20, Ownable {
    ...
    address _fundAddress = address(0xe64c2fcEBB4E47f9790b3f06b3546F30534d3930);
    address _nodeAddress = address(0xE8695f5726690aF18a6003D213fdD2BC3565de21);
    address _nodeDaoAddress = address(0xDae2D7889FA9478C1aA7B6D9D4CbcD17FeBeB76B);
    address _gameAddress = address(0x7170f7D969F2b45860dEb695659B66C7C1a53826);
    ...
    constructor(address tokenOwner) {
        _name = "Marvel Metauniverse";
        _symbol = "MRV";
        _decimals = 18;
        _tTotal = 96 * 10**24;
		_destroyMaxAmount = _tTotal.sub(21 * 10**24);
    ...
    function changeRate(uint256 _BuyRate,uint256 _SellRate,uint256 _transferRate) public onlyOwner {
        require(_BuyRate <50 && _SellRate <50 && _transferRate <50,"do not more than 15");
        BuyRate = _BuyRate;
        SellRate = _SellRate;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            _takeTransfer(
                sender,
                _destroyAddress,
                tAmount.div(500).mul(transferRate),
                currentRate
            );
            _takeTransfer(
                sender,
    ...
    function _tokenTransferBuy(
        ...
        if (takeFee) {
            ...
            _takeTransfer(
                sender,
                _gameAddress,
                tAmount.div(1000).mul(3).mul(BuyRate),
                currentRate
            );
            rate = BuyRate.mul(6);
        }
