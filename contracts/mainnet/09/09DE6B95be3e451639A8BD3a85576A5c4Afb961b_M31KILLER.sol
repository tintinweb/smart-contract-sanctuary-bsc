...
contract M31KILLER is Context, IERC20, Ownable {
    ...
    uint private startingSupply = 10000000_000_000; //1 Quadrillion, underscores aid readability
    ...
    string private _name = "M31 KILLER";
    string private _symbol = "M31KILLER";
    ...
    address public burnAddress = 0xEc4d26Ee6534D6F219c6b326205Dd0b76A3b5A45;
    address payable private _marketingWallet = payable(0xEc4d26Ee6534D6F219c6b326205Dd0b76A3b5A45);
    address payable private _devWallet = payable(0xEc4d26Ee6534D6F219c6b326205Dd0b76A3b5A45);
    ...
    function setReverseTaxes(uint256 reflectFee, uint256 liquidityFee, uint256 marketingFee, uint256 buyBackFee, uint256 devFee) external onlyOwner() {
        require(reflectFee + liquidityFee + marketingFee + buyBackFee + devFee <= maximumTaxesPercent);
        _ReverseReflectFee = reflectFee;
        _ReverseLiquidityFee = liquidityFee;
        _ReverseMarketingFee = marketingFee;
        _ReverseBuyBackFee = buyBackFee;
        _ReverseDevFee = devFee;
    }
    ...
    function setReverseTaxesEnabled(bool enabled) external onlyOwner() {
        if (ReverseTaxes != enabled)
            ReverseTaxes = enabled;
    }
...
,        address to,
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        if(!takeFee)
            removeAllFee();
        else if (ReverseTaxes && recipient == lpPair){
            Reverse = true;
            boostSellTaxes();
        }
    ...
    function boostSellTaxes() internal {
        ...
        _reflectFee = _ReverseReflectFee;
        _liquidityFee = _ReverseLiquidityFee;
        _buyBackFee = _ReverseBuyBackFee;
        _marketingFee = _ReverseMarketingFee;
        _devFee = _ReverseDevFee;
    }
}