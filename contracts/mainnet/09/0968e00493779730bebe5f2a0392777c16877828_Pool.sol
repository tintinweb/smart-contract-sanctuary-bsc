...
contract PausableAssets {
    ...
    error FUN_ASSET_ALREADY_PAUSED();
    error FUN_ASSET_NOT_PAUSED();

    function requireAssetNotPaused(address asset) internal view {
        if (_pausedAssets[asset]) revert FUN_ASSET_ALREADY_PAUSED();
    }

    function requireAssetPaused(address asset) internal view {
        if (!_pausedAssets[asset]) revert FUN_ASSET_NOT_PAUSED();
    }
    ...
    FunCurveV1
{
    ...
    event SetMasterFun(address addr);
    ...
    error FUN_ASSET_NOT_EXISTS();
    error FUN_ASSET_ALREADY_EXIST();

    error FUN_ZERO_ADDRESS();
    error FUN_ZERO_AMOUNT();
    error FUN_ZERO_LIQUIDITY();
    error FUN_INVALID_VALUE();
    error FUN_SAME_ADDRESS();
    error FUN_AMOUNT_TOO_LOW();
    error FUN_CASH_NOT_ENOUGH();
    error FUN_INTERPOOL_SWAP_NOT_SUPPORTED();
    ...
    function addAsset(address token, address asset) external onlyOwner nonReentrant {
        ...
        if (_containsAsset(token)) revert FUN_ASSET_ALREADY_EXIST();
    ...
    function removeAsset(address token) external onlyOwner {
        if (!_containsAsset(token)) revert FUN_ASSET_NOT_EXISTS();
    ...
    function deposit(
        ...
        if (amount == 0) revert FUN_ZERO_AMOUNT();
        _checkAddress(to);
        _ensure(deadline);
        ...
        _checkAddress(address(masterChef));
        liquidity = _deposit(asset, amount.toWad(asset.underlyingTokenDecimals()), minimumLiquidity, address(this));
        asset.approve(address(masterChef), liquidity);
        masterChef.depositFor(masterChef.getAssetPid(address(asset)), liquidity, to);
    ...
    function withdraw(
        ...
        masterChef.withdrawFor(masterChef.getAssetPid(address(asset)), liquidity, to);
    ...
    function _quoteFrom(
        ...
        if (toCash < idealToAmount) revert FUN_CASH_NOT_ENOUGH();
    ...
    function _swap(
        ...
        if (uint256(toAsset.cash()).wdiv(toAsset.liability()) < WAD / 100) revert FUN_FORBIDDEN();
    }
    ...
    function swap(
        ...
        if (fromAmount == 0) revert FUN_ZERO_AMOUNT();
        _checkAddress(to);
        _ensure(deadline);
