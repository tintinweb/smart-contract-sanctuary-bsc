...
abstract contract MintpassMintable is Ownable {
     ...
     * @dev onlyOwner
     */
    function mintUnclaimedToVault(address _vault) external onlyOwner {
        require(_vault != address(0), "Invalid vault");
        require(address(vault) == address(0), "Already minted unclaimed");
        _mintpassesTotalFrozen = mintpass.totalSupply();

        uint256 unused = mintpassesUnused();
        vault = IMintpassVault(_vault);
        _mintTo(_vault, unused);
    }
    ...
    function _mintpassMintsOpen() internal view returns (uint256) {
        return address(vault) == address(0) ? mintpassesUnused() : 0; 
    }
