...
contract Contribution is Ownable {
    ...
    function getContributionMinByPhase(uint8 phase, uint8 tier) public view returns (uint256) {
    ...
    // Looking inside the internals for the owner
    function getContributionMaxByPhase(uint8 phase, uint8 tier) public view returns (uint256) {
        require((tier > 0) && (tier <= 5));
        if (tier == 1) {
    ...
    // Looking inside the internals for the owner
    function getFactorByPhase(uint8 phase, uint8 tier) public view returns (uint256) {
        require((tier > 0) && (tier <= 5));
        if (tier == 1) {
