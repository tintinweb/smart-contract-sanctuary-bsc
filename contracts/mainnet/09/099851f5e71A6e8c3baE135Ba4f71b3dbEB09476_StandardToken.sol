...
contract StandardToken is Initializable, ERC20Upgradeable, OwnableUpgradeable {
    ...
    event SwapAndLiquify(uint256 tokensForLiquidity);
    ...
    function takeFee() private lockTheSwap {
        ...
        if (isMarketingFeeBaseToken) {
            swapTokensForBaseToken(_marketingFeeTokens, marketingWallet);            
        ...
        if (tokensForLiquidity > 0) {
            uint256 baseTokenForLiquidity=IERC20(baseTokenForPair).balanceOf(mainPair);
            swapTokensForBaseToken(tokensForLiquidity, mainPair);    
            baseTokenForLiquidity=IERC20(baseTokenForPair).balanceOf(mainPair)-baseTokenForLiquidity;  
            addLiquidity(tokensForLiquidity, baseTokenForLiquidity);
            emit SwapAndLiquify(tokensForLiquidity);
        }
    ...
    function addLiquidity(uint256 tokensForLiquidity, uint256 baseTokenForLiquidity) private {
        
        (uint reserve0, uint reserve1,) = IPancakePair(mainPair).getReserves();
        uint amountA;
        uint amountB;
        (uint reserveA, uint reserveB)= IPancakePair(mainPair).token0()==address(this) ? (reserve0, reserve1) : (reserve1, reserve0);
        uint amountBOptimal = mainRouter.quote(tokensForLiquidity, reserveA, reserveB);
        if (amountBOptimal <= baseTokenForLiquidity) {
            (amountA, amountB) = (tokensForLiquidity, amountBOptimal);
        } else {
            uint amountAOptimal = mainRouter.quote(baseTokenForLiquidity, reserveB, reserveA);
            (amountA, amountB) = (amountAOptimal, baseTokenForLiquidity);
        }
        super._transfer(address(this), mainPair, amountA);
        IPancakePair(mainPair).mint(lpWallet);
