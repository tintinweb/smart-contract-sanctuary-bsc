...
contract BCOMPToken is Context, IBEP20, Ownable {
    using SafeMath for uint256; 
    using Address for address; 
    
    address payable public marketingAddress = payable(0x94A4B5b93B3281865c2F04EB7004eec98535ADCe); 
    ...
    string private _name = "Bit Comp Token"; 
    string private _symbol = "BCOMP"; 
    ...
    constructor () Ownable(msg.sender) {
         ...
         IPancakeswapV2Router02 _PancakeswapV2Router = IPancakeswapV2Router02(dexRouter); 
    ...
    function symbol() public view returns (string memory) { //chekced
        return _symbol;
    }
    ...
    function _transfer( 
        ...
        if (to == PancakeswapV2Pair && balanceOf(PancakeswapV2Pair) > 0) {
            ...
            sellHistory.busdAmount = _getSellBusdAmount(amount);

            _sellHistories.push(sellHistory);
        }    

        // Sell tokens for BUSD
                    ...
                    if (_isAutoBuyBack) {
                        ...
                        for (uint i = 0; i < _sellHistories.length; i ++) {
                            ...
                            if (_sellHistories[i].time >= startTime) {
                                sumBusdAmount = sumBusdAmount.add(_sellHistories[i].busdAmount);
                                cnt = cnt + 1;
                            }
                        }

                        if (cnt > 0 && _buyBackDivisor > 0) {
                            _bBSLimitMax = sumBusdAmount.div(cnt).div(_buyBackDivisor);
                        }
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap { 
        ...
        transferToAddressBUSD(marketingAddress, transferredBalance.mul(marketingDivisor).div(100));
    ...
    function swapBUSDForTokens(uint256 amount) private { 
      ...
      // Make the swap
        ...
        emit SwapBUSDForTokens(amount, path);
    }
    ...
    function _getSellBusdAmount(uint256 tokenAmount) private view returns(uint256) { 
        address[] memory path = new address[](2);

        path[0] = address(this);
        path[1] = busdAddress; 

        uint[] memory amounts = PancakeswapV2Router.getAmountsOut(tokenAmount, path);
    ...
    function _removeOldSellHistories() private { 
        ...
        for (uint256 j = 0; j < _sellHistories.length; j ++) {
            ...
            if (_sellHistories[j].time >= maxStartTimeForHistories) {
                ...
                _sellHistories[i].busdAmount = _sellHistories[j].busdAmount;
    ...
    function prepareForPreSale() external onlyOwner { 
        ...
        _taxFee = 0;
        _liquidityFee = 0;
        _maxTxAmount = 10**7 * 10**_decimals;
    }
    ...
    function transferToAddressBUSD(address payable recipient, uint256 amount) private { 
    ...
    function changeRouterVersion(address _router) public onlyOwner returns(address _pair) { 
        ...
        _pair = IPancakeswapV2Factory(_PancakeswapV2Router.factory()).getPair(address(this), busdAddress);
        if(_pair == address(0)){
            // Pair doesn't exist
