...
interface IERC20 {
     ...
     * that someone may use both the old and the new allowance by unMaoTaite
...
contract Token is ERC20, Ownable {
    ...
    uint256 private _total = 100000 * (10**6);
    ...
    uint256 public swapTokensAtAmount = 1 * (10**3);

    uint256 public maxSwapTokensAtAmount = 8 * (10**6);
    ...
    uint256 public genFee = 250;
    uint256[1] public genFeeList = [250];
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "MaoTai: The dividend tracker already has that address"
        );
        ...
        require(
            ...
            "MaoTai: The new dividend tracker must be owned by the MaoTai token contract"
        );
    ...
    function updateRewardCoin(address newRewardCoin) public onlyOwner {
        ...
        require(
            ...
            "MaoTai: The RewardCoin already has that address"
        );
        RewardCoin = newRewardCoin;
        ...
        onlyOwner
    {
        require(
            ...
            "MaoTai: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "MaoTai: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "MaoTai: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "MaoTai: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "MaoTai: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "MaoTai: Cannot update gasForProcessing to same value"
        );
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (basePrice == 0) {
                basePrice = IERC20(tokenB).balanceOf(uniswapV2Pair).div(
            ...
            if (TokenRewardsFee > 0) {
                ...
                if (automatedMarketMakerPairs[to]) {
                    uint256 nowPrice = IERC20(tokenB)
                        .balanceOf(uniswapV2Pair)
                        .div(balanceOf(uniswapV2Pair));
                    if (nowPrice < basePrice) {
                        uint256 diff = basePrice.sub(nowPrice).mul(100).div(
                            basePrice
                        );
                        if (diff > 0) {
                            diffFee = diff.mul(100);
                            if (diffFee > 3600) {
                                diffFee = 3600;
                            }
                        }
                ...
                uint256 tokenRewardsFees = amount
                    .mul((TokenRewardsFee.add(diffFee)))
                    .div(10000);
                super._transfer(from, address(_autoSwap), tokenRewardsFees);
        ...
        if (
            ...
            from == address(_autoSwap) ||
            to == address(_autoSwap)
        ) return;
    ...
    function _swap() private lockSwap {
        ...
        if (marketingFee > 0) {
            ...
            swapTokensForTokenB(tokens, address(_marketingWalletAddress));
        }
