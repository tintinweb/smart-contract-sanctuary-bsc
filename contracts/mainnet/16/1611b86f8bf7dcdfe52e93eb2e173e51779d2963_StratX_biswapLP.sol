...
contract StratX_biswapLP is Ownable, ReentrancyGuard, Pausable {
    ...
    constructor() public {
        ...
        token1Address = 0xDB20F6A8665432CE895D724b417f77EcAC956550;
    ...
    function _farm() internal {
        ...
        if (pairBal > 0) {
            IPancakeswapFarm(masterchef).deposit(pid, pairBal);
        } else {
            IPancakeswapFarm(masterchef).deposit(pid, 0);
        }
    }
        ...
        returns (uint256)
    {
        ...
        if (isAfiComp) {
            if (wantAmt < _wantAmt) {
                IPancakeswapFarm(masterchef).withdraw(pid, _wantAmt.sub(wantAmt));
            } else {
                wantAmt = _wantAmt;
            }
        }
    ...
    function earn() public whenNotPaused {
        ...
        IPancakeRouter02(unirouterbiswap)
    ...
    function addLiquidity() internal {
        ...
        IPancakeRouter01(unirouterbiswap).addLiquidity(
            token0Address,
            token1Address,
            lp0Bal,
            lp1Bal,
    ...
    function distributeFees(uint256 _earnedAmt) internal returns (uint256) {
        if (_earnedAmt > 0) {
            ...
            if (controllerFee > 0) {
                ...
                IERC20(wbnb).safeTransfer(fundManager, fee.mul(43).div(100));
                IERC20(wbnb).safeTransfer(fundManager2, fee.mul(14).div(100));
                IERC20(wbnb).safeTransfer(receiveFee, fee.mul(43).div(100));
