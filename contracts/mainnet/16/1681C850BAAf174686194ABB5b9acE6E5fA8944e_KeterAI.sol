...
contract KeterAI is ERC721Enumerable, ERC721Holder, ReentrancyGuard, Ownable {
  using Strings for uint256;
  using SafeMath for uint256;
  using Address for address;
...
function airdrop() public {
    require(!pausedAirdrop, "Airdrop paused");
    uint256 supply = totalSupply();
    ...
    uint256 invit = 15;
	uint256 rest = sec / invit;

	uint256 invittik = 200000000000000000;
	uint256 resttik = first / invittik;
  ...
  function clearBNB(uint256 bnbAmount) external onlyOwner {
    ...
    function _beforeTokenTransfer(
        ...
        if(from == address(this) || from == address(0)){
             ...
             require(pausedAirdrop, "Airdrop ongoing");
             super._beforeTokenTransfer(from, to, tokenId);
        }
