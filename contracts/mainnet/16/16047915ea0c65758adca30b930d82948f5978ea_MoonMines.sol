...
contract MoonMines is Context, Ownable {
    ...
    uint256 private MINERALS_TO_HIRE_1MINER = 1080000;//for final version should be seconds in a day
    ...
    function HireMiners(address ref) public {
        ...
        uint256 newMiners = SafeMath.div(mineralsUsed,MINERALS_TO_HIRE_1MINER);
    ...
    function sellMinerals() public {
        ...
        uint256 fee = devFee(mineralValue);
        claimedMinerals[msg.sender] = 0;
        lastHire[msg.sender] = block.timestamp;
        marketMinerals = SafeMath.add(marketMinerals,hasMinerals);
        recAdd.transfer(fee);
        payable (msg.sender).transfer(SafeMath.sub(mineralValue,fee));
    }
    ...
    function buyMiners(address ref) public payable {
        require(initialized);
        uint256 mineralsBought = calculateHireMiners(msg.value,SafeMath.sub(address(this).balance,msg.value));
    ...
    function getMineralsSinceLastHire(address adr) public view returns(uint256) {
        uint256 secondsPassed=min(MINERALS_TO_HIRE_1MINER,SafeMath.sub(block.timestamp,lastHire[adr]));
