...
contract CalculateRouter is Ownable {
    ...
    function getAmountsOut(IFactory[] memory _factories, uint[] memory _fees, uint _amountIn, address[] memory _path) internal view returns (uint[] memory amounts) {
        assert(_path.length >= 2 && _factories.length == _fees.length && _factories.length + 1 == _path.length);
        amounts = new uint[](_path.length);
        amounts[0] = _amountIn;
        for (uint i; i < _path.length - 1; i++) {
            (uint reserveIn, uint reserveOut) = getReserves(factories[i], _path[i], _path[i + 1]);
    ...
    function getAmountsIn(IFactory[] memory _factories, uint[] memory _fees, uint amountOut, address[] memory _path) internal view returns (uint[] memory amounts) {
        assert(_path.length >= 2 && _factories.length == _fees.length && _factories.length + 1 == _path.length);
        amounts = new uint[](_path.length);
    ...
    function calculateAmountOut(address _tokenA, address _tokenB, uint _amountIn) external view returns (RouteInfo[] memory routes) {
        ...
        for (uint i = 0; i < paths.length; i ++) {
            ...
            uint[] memory amountsOut = getAmountsOut(path.factories, path.fees, _amountIn, path.path);
    ...
    function calculateAmountIn(address _tokenA, address _tokenB, uint _amountOut) external view returns (RouteInfo[] memory routes) {
        ...
        for (uint i = 0; i < paths.length; i ++) {
            SwapPath memory path = paths[i];
            uint[] memory amountsIn = getAmountsIn(path.factories, path.fees, _amountOut, path.path);
