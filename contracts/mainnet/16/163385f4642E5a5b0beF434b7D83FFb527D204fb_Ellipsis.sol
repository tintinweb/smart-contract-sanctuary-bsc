...
contract Ellipsis is Ownable, ReentrancyGuard {
    ...
    function totalValueLocked() public view returns (uint256) {
        ...
        if(receiveToken != token0) {
            path[0] = token0;
            path[1] = receiveToken;
            uint256[] memory amounts = IUniswapV2Router01(sellRouter).getAmountsOut(lockedAmount, path);
        ...
        path[1] = IUniswapV2Router01(sellRouter).WETH();
        path[2] = receiveToken;
        uint256[] memory amounts = IUniswapV2Router01(sellRouter).getAmountsOut(reward, path);
        depositedAmount += amounts[amounts.length - 1];
