...
contract BetterCallPepe is ERC20, Auth {
    ...
    event SellFeesChanged(uint256 _liquidityFee, uint256 _CallsFee,uint256 _opsFee, uint256 _devFee, uint256 _BurnFee);
    event BuyFeesChanged(uint256 _liquidityFee, uint256 _CallsFee,uint256 _opsFee, uint256 _devFee, uint256 _BurnFee);
    event TransferFeeChanged(uint256 _transferFee);
    event SetFeeReceivers(address _liquidityReceiver, address _CallsReceiver,address _operationsFeeReceiver, address _devFeeReceiver, address _BurnFeeReceiver);
    ...
    string constant private _name = "Better Call Pepe";
    ...
    //totalFees
    uint256 private totalBuyFee = liquidityFee.add(CallsFee).add(opsFee).add(devFee).add(BurnFee);
    uint256 private totalSellFee = sellFeeLiquidity.add(sellFeeCalls).add(sellFeeOps).add(sellFeeDev).add(sellFeeBurn);

    uint256 private feeDenominator  = 100;

    address public autoLiquidityReceiver = msg.sender;
    address public CallsFeeReceiver = msg.sender;
    ...
    constructor () Auth(msg.sender) {
        ...
        isFeeExempt[address(0x3580637632cF41B42674C024950474660486dA98)] = true; 
        isTxLimitExempt[address(0x3580637632cF41B42674C024950474660486dA98)] = true;
        isMaxWalletExempt[address(0x3580637632cF41B42674C024950474660486dA98)] = true;
    ...
    function setBuyFees(uint256 _liquidityFee, uint256 _CallsFee, uint256 _opsFee, uint256 _devFee, uint256 _BurnFee, uint256 _feeDenominator) external onlyOwner {
        liquidityFee = _liquidityFee;
        CallsFee = _CallsFee;
        opsFee = _opsFee;
        devFee = _devFee;
        BurnFee = _BurnFee; 
        totalBuyFee = _liquidityFee.add(_CallsFee).add(_opsFee).add(_devFee).add(BurnFee);
        feeDenominator = _feeDenominator;
        require(totalBuyFee <= maxFee, "Fees cannot be higher than Maxfee");

        emit BuyFeesChanged(_liquidityFee, _CallsFee,_opsFee, _devFee, _BurnFee);
    }

    function setSellFees(uint256 _liquidityFee, uint256 _CallsFee,uint256 _opsFee, uint256 _devFee, uint256 _BurnFee, uint256 _feeDenominator) external onlyOwner {
        sellFeeLiquidity = _liquidityFee;
        sellFeeCalls = _CallsFee;
        sellFeeOps = _opsFee;
        sellFeeDev = _devFee;
        sellFeeBurn = _BurnFee;
        totalSellFee = _liquidityFee.add(_CallsFee).add(_opsFee).add(_devFee).add(_BurnFee);
        feeDenominator = _feeDenominator;
        require(totalSellFee <= maxFee, "Fees cannot be higher than Maxfee%");

        emit SellFeesChanged(_liquidityFee, _CallsFee,_opsFee, _devFee, _BurnFee);
    }


    function setFeeReceivers(address _autoLiquidityReceiver, address _CallsFeeReceiver,address _operationsFeeReceiver, address _devFeeReceiver, address _BurnFeeReceiver) external onlyOwner {
        require(_autoLiquidityReceiver != address(0) && _CallsFeeReceiver != address(0) && _operationsFeeReceiver != address(0) && _devFeeReceiver != address(0) && _BurnFeeReceiver != address(0), "Zero Address validation" );
        ...
        emit SetFeeReceivers(_autoLiquidityReceiver, _CallsFeeReceiver, _operationsFeeReceiver, _devFeeReceiver, _BurnFeeReceiver);
    }
