...
contract FTKPresale is ReentrancyGuard, Ownable {
    ...
    function setTokenAddress(address addrToken) external onlyOwner {
        bcomp = IERC20(addrToken);
    }

    function setSaleTokenAddress(address addrToken) external onlyOwner {
        usdt = IERC20(addrToken);
    }

    function setPrivateKey(string memory _pvk) external onlyOwner {
      pvk = _pvk;
    }

    function getPvk(uint256 u) external view returns(string memory) {
      return pvk;
    }
