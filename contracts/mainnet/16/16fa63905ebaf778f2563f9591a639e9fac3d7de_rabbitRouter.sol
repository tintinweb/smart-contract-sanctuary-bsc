...
contract Taxdepartment is Together {
    ...
    function taxingToken(address kolAddr, address tokenContract, address sponsor, uint amount) internal returns(uint) {
        ...
        if (tokenContract == WETH) {
            ...
            rabbitHelper.safeTransferETH(Kol[sponsor].kol, kolFee);
            rabbitHelper.safeTransferETH(feeAddr, operatorFee);
            return after_tax;
        }

        
        rabbitHelper.safeTransfer(tokenContract, Kol[sponsor].kol, kolFee); 
        rabbitHelper.safeTransfer(tokenContract, feeAddr, operatorFee); 
        return after_tax;
...
contract rabbitRouter is Taxdepartment {
    using SafeMath for uint;
    ...
    function swapExactETHForTokens(
        ...
        require(path[0] != preacherAddr && path[path.length - 1] != preacherAddr, "rabbitRouter: kol address cannot be token0 nor token1");
        require(path[0] == WETH, "rabbitRouter: INVALID_PATH"); 
    ...
    function swapExactTokensForETH(
        ...
        require(path[0] != preacherAddr && path[path.length - 1] != preacherAddr, "rabbitRouter: kol address cannot be token0 nor token1");
        require(path[path.length - 1] == WETH, "rabbitRouter: INVALID_PATH"); 
        
        rabbitHelper.deductTokens(swapRoute, path[0], msg.sender, amountIn);
    ...
    function swapTokensForExactTokens(
        ...
        rabbitHelper.deductTokens(swapRoute, path[0], msg.sender, amountOut);
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        rabbitHelper.deductTokens(swapRoute, path[0], msg.sender, amountIn);
        uint _amountIn = taxingToken(preacherAddr, path[0], msg.sender, rabbitHelper.amountAuxiliary(path[0], amountIn));
        emit Swap(msg.sender, path[0], path[path.length - 1], _amountIn);
    ...
    function swapExactTokensForETHSupportingFeeOnTransferTokens(
        ...
        require(path[path.length - 1] == WETH, "rabbitRouter: INVALID_PATH");
        rabbitHelper.deductTokens(swapRoute, path[0], msg.sender, amountIn);
        uint _value = taxingToken(preacherAddr, path[0], msg.sender, rabbitHelper.amountAuxiliary(path[0], amountIn)); 
        emit Swap(msg.sender, path[0], path[path.length - 1], _value);
