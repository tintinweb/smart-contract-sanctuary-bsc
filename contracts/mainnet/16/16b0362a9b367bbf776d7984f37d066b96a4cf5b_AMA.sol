...
contract AMA is IERC20, Ownable {
    ...
    address private buyFundAddress = address(0x6bDecd9DdDe8e47f9ca111bCcEEa33a9B74d91FB);
	address private sellFundAddress = address(0x6bDecd9DdDe8e47f9ca111bCcEEa33a9B74d91FB);
    address private fundAddress = address(0x7D11314a3D61419D2262bbb64E5f59314a813340);
	...
	uint256 public buyLpRate = 3;
	uint256 public buyFundRate = 2;
	uint256 public sellLpRate = 3;
	uint256 public sellFundRate = 2;
	function changeRate(uint256 _buyLpRate,uint256 _buyFundRate,uint256 _sellLpRate,uint256 _sellFundRate) public onlyOwner {
    ...
    function _tokenTransferBuy(
        ...
        if (takeFee) {
            _takeTransfer(
                sender,
                buyFundAddress,
                tAmount.div(100).mul(buyFundRate),
                currentRate
            );
            _takeTransfer(
                sender,
                address(this),
                tAmount.div(100).mul(buyLpRate),
                currentRate
            );
            rate = buyFundRate + buyLpRate;
        }
        uint256 recipientRate = 100 - rate;
        _rOwned[recipient] = _rOwned[recipient].add(rAmount.div(100).mul(recipientRate));
        emit Transfer(sender, recipient, tAmount.div(100).mul(recipientRate));
    ...
    function _tokenTransferSell(
        ...
        if (takeFee) {
            _takeTransfer(
                sender,
                sellFundAddress,
                tAmount.div(100).mul(sellFundRate),
                currentRate
            );
            _takeTransfer(
                sender,
                address(this),
                tAmount.div(100).mul(sellLpRate),
                currentRate
            );
            rate = sellFundRate + sellLpRate;
        }
        uint256 recipientRate = 100 - rate;
        _rOwned[recipient] = _rOwned[recipient].add(rAmount.div(100).mul(recipientRate));
        emit Transfer(sender, recipient, tAmount.div(100).mul(recipientRate));
