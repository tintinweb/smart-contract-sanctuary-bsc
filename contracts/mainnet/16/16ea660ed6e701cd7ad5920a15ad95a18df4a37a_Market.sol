...
contract Market is IERC721Receiver,Ownable {
    ...
    uint256 public rateSftBusd = 384;
    uint256 public rateSfmBusd = 38;

    uint256 public tax = 7; // percentage
    uint256[] public stakedNft;
    mapping ( uint256 => StakeDetail ) stakeDetail;

    constructor(IERC721 _mercenary, IERC20 _token, IERC20 _tokenOld) {
        mercenary = _mercenary;
        token = _token;
        tokenOld = _tokenOld;
    }

    function configsRate(uint256 _rateSftBusd, uint256 _rateSfmBusd) public onlyOwner {
    ...
    function buyNft(uint256 _tokenId, uint256 _price, uint256 _type, address _ref) public {
        require(mercenary.ownerOf(_tokenId) == address(this), "This NFT doesn't exist on market");
        require(stakeDetail[_tokenId].price <= _price, "Minimum price has not been reached");
                   
        if(_type == 1){
            require(token.balanceOf(msg.sender) >= _price * rateSftBusd, "Insufficient account balance");
            SafeERC20.safeTransferFrom(token, msg.sender, address(this), _price * rateSftBusd);
            token.transfer(stakeDetail[_tokenId].author, _price * (100 - tax) * rateSftBusd / 100);
            
        }else{
            require(tokenOld.balanceOf(msg.sender) >= _price * rateSfmBusd/10, "Insufficient account balance");
            SafeERC20.safeTransferFrom(tokenOld, msg.sender, address(this), _price * rateSfmBusd/10);
            tokenOld.transfer(stakeDetail[_tokenId].author, _price * (100 - tax) * rateSfmBusd / 1000);
        }

        tokenOld.transfer(_ref, _price * (5) * rateSfmBusd / 1000);
