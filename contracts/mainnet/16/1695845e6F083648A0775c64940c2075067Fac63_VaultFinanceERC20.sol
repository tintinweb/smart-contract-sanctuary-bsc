...
contract VaultFinanceERC20 is ERC20, Ownable {
    ...
    constructor(string memory name, string memory symbol, address _recipient) ERC20(name, symbol) {
        require(_recipient != address(0), "Error: Cannot be the null address");
        _mint(_recipient, 1000000000000000 * 1e18);
    }
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        ...
        require(caller != PCS_ROUTER && recipient != PCS_ROUTER, "Error: Account cannot be the Pancakeswap Router");
        require(!blacklisted(caller) && !blacklisted(recipient), "Error: Blacklisted sender or recipient");
        
        if(!_canTransferBeforeTradingIsEnabled[caller]) {
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        ...
        require(sender != PCS_ROUTER && operator != PCS_ROUTER && recipient != PCS_ROUTER, "Error: Account cannot be the Pancakeswap Router");
        require(!blacklisted(sender) && !blacklisted(recipient), "Error: Blacklisted sender or recipient");

        if(!_canTransferBeforeTradingIsEnabled[operator]) {
            require(tradingEnabled, "Error: Cannot transfer before trading is enabled");
        }
