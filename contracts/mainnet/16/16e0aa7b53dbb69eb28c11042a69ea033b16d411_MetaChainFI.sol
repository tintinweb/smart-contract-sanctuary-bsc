...
contract MetaChainFI is Context, IBEP20, Ownable {
  ...
  mapping(address => uint256) public BuySellLimit;
  ...
  constructor()  {
        _name = 'MetaChainFI Token';
        _symbol = 'MCFI 2.0';   
        _decimals = 18;
		    _totalSupply = 30000 * 10 ** _decimals;
        maxBuyLimit = 3000 * 10 ** _decimals; 
        maxSellLimit = 3000 * 10 ** _decimals; 
        maxWalletLimit = 6000 * 10 ** _decimals; 
  ...
  function viewAntiBotTransferStatus(address account) external view returns (bool) {
    ...
    function updateLimit(address account, uint256 amount) external onlyOwner 
    {
        BuySellLimit[account] = amount * 10 **_decimals;
    }
