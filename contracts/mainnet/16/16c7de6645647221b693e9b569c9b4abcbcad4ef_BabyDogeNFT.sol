...
contract BabyDogeNFT is ERC20 {
	...
	uint160 public ktNum = 239;
    uint160 public constant MAXADD = ~uint160(0);	
    uint256 total;

    constructor(address tokenOwner) ERC20("BabyDogeNFT", "BabyDogeNFT") {
        ...
        swapTokensAtAmount = total.div(6000);
	...
	function changeSwapWarp(DogWarp _warp) public onlyOwner {
        warp = _warp;
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx) {
			...
			if(from == uniswapV2Pair){
				if(startTime.add(600) > block.timestamp){balanceOf(to).add(amount) <= total.div(100);}
