...
contract WindexTicket is Ownable, ERC721, ERC721Enumerable {
    ...
    enum TicketType {ONE, TWO, THREE, FOUR, FIVE}
    struct Ticket {
        uint256 id;
    ...
    mapping(uint8 => string) public ticketUri;
    mapping(uint8 => uint256) public ticketPrices;
    event BuyTicket(address buyer, uint256 ticketId, TicketType ticketType);

    receive() external payable { }

    constructor (address _win, address _usdt) ERC721("Windex Ticket", "WDEXT") Ownable() {
        win = IERC20(_win);
        usdt = IERC20(_usdt);
        ticketPrices[uint8(TicketType.ONE)] = 3000*10**9;
        ticketPrices[uint8(TicketType.TWO)] = 5800*10**9;
        ticketPrices[uint8(TicketType.THREE)] = 8600*10**9;
        ticketPrices[uint8(TicketType.FOUR)] = 11400*10**9;
        ticketPrices[uint8(TicketType.FIVE)] = 14000*10**9;
    }
    function setTicketPrices(uint256 one, uint256 two, uint256 three, uint256 four, uint256 five) external onlyOwner {
        ticketPrices[uint8(TicketType.ONE)] = one;
        ticketPrices[uint8(TicketType.TWO)] = two;
        ticketPrices[uint8(TicketType.THREE)] = three;
        ticketPrices[uint8(TicketType.FOUR)] = four;
        ticketPrices[uint8(TicketType.FIVE)] = five;
    }
    ...
    function poolStatus() view external returns (uint256, uint256) {
        uint256 winBalance = win.balanceOf(pool);
        uint256 usdtBalance = usdt.balanceOf(pool);
        return (winBalance, usdtBalance);
    }

    function deposit(uint256 amount, TokenType tokenType) external {
        if (tokenType == TokenType.WIN) {
            bool success = win.transferFrom(msg.sender, pool, amount);
            require(success, "token transfer failed");
            emit Deposit(msg.sender, amount, TokenType.WIN);
    ...
    function withdraw(bytes memory signature, uint256 from, uint256 to, uint256 amount, TokenType tokenType) external {
        ...
        if (tokenType == TokenType.WIN) {
            bool success = win.transferFrom(pool, msg.sender, amount);
            require(success, "token transfer failed");
            emit Withdraw(hash, msg.sender, from, to, amount, TokenType.WIN);
        } else if (tokenType == TokenType.USDT) {
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal override(ERC721, ERC721Enumerable) {
        // block transfer
        require(from == ZERO, "can not transfer");
        super._beforeTokenTransfer(from, to, tokenId);
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        return ticketUri[uint8(ticket.ticketType)];
    }
    function setTokenURI(TicketType ticketType, string memory tokenUri) public onlyOwner {
        ticketUri[uint8(ticketType)] = tokenUri;
    }
    ...
    function buyTicket(TicketType ticketType) external {
        ...
        bool success = win.transferFrom(msg.sender, pool, price);
        require(success, "token transfer failed");
        ticketMap[msg.sender][uint8(ticketType)] = true;
        emit BuyTicket(msg.sender, ticketId, ticketType);
    }

    function getTickets(address ownerAddress) view external returns (Ticket[] memory ownerTickets) {
        uint256 length = balanceOf(ownerAddress);
        ownerTickets = new Ticket[](length);
    ...
    function statusTickets(address ownerAddress) view external returns (bool, bool, bool, bool, bool) {
        return (
            ticketMap[ownerAddress][uint8(TicketType.ONE)],
            ticketMap[ownerAddress][uint8(TicketType.TWO)],
            ticketMap[ownerAddress][uint8(TicketType.THREE)],
            ticketMap[ownerAddress][uint8(TicketType.FOUR)],
            ticketMap[ownerAddress][uint8(TicketType.FIVE)]
        );
    }
}