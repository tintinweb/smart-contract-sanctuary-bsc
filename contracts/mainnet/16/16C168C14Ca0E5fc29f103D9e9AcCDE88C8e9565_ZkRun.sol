...
contract ZkRun is Context, IBEP20, Ownable {
    ...
    constructor() {
        ...
        _minBalance = 10**9 * 10**18; 
        _maxSoldAmount = 2 * 10**9 * 10**18;

        _treasuryWallet = payable(0x3A748bB6f950EcF070c7d7770a7775A3caDAe749);
        _marketingWallet = payable(0x5fc1bEE63D37EC9cCa06bD5F5B80C9b49bAbd52D);
    ...
    function _transfer(
        ...
        if (!inSwap && swapEnabled && to == uniswapV2Pair) {
            ...
            // transfer tokens
            uint256 zruBalance = balanceOf(address(this));
            if (zruBalance > _minBalance) {
                transferTokens(zruBalance);
            }
    ...
    /**
     * @dev Swap tokens from zru to bnb
     */
    ...
    /**
     * @dev Add zru token and bnb as same ratio on pancakeswap router
     */
    ...
    /**
     * @dev the Owner can swap regarding the zru token's amount of contract balance
    ...
    /**
     * @dev the Owner can send regarding the zru token's amount of contract balance
     * this is for manual function
    ...
    function _setMaxSoldAmount(uint256 maxvalue) external onlyOwner {
        require(maxvalue >= 10**9 * 10**18, "_maxSoldAmount must be greater than or equal to 0.1% total supply");
