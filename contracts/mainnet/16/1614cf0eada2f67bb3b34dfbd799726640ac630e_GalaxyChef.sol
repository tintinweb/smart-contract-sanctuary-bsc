...
interface IC9EChef {
    ...
    function getInvestmentLength(uint256 _pid, address _user) external view returns(uint256);


    function getInvestmentInfo(uint256 _pid, address _user, uint256 index) external view returns(DepositAmount memory);
}
...
contract GalaxyChef is Ownable, ReentrancyGuard {
    ...
    IC9EChef public c9eChef = IC9EChef(0xfbcd7eDB373D6DDa0bf9f417f5F503Cd1eA29c9e);
    ...
    mapping(uint256 => mapping(address => bool)) public c9eMigrated;

    mapping(uint256 => mapping(address => bool)) public arrayAdded;
    mapping(uint256 => mapping(address => bool)) public notRewardReduction;
    ...
    function getTokenPrice() public view returns (uint256) {
        return IBEP20(busdAddress).balanceOf(tokenLPAddress).mul(1e4).div(galaxyToken.balanceOf(tokenLPAddress));
    }
    ...
    function getLPPrice(uint256 _pid) public view returns (uint256) {
        ...
        if(pool.tokenType == 1)
            return IBEP20(busdAddress).balanceOf(pool.token1BusdLPaddress).mul(1e4).div(IBEP20(pool.lpToken).balanceOf(pool.token1BusdLPaddress));
        if(pool.tokenType == 2)
            return IBEP20(busdAddress).balanceOf(address(pool.lpToken)).mul(2e4).div(pool.lpToken.totalSupply());
        ...
        return IBEP20(pool.token1).balanceOf(address(pool.lpToken)).mul(token1Price).mul(2e4).div(pool.lpToken.totalSupply()).div(1e8);
    }
        ...
        returns (uint256)
    {
        ...
        if (pool.lockingPeriod >= 15552000 && !notRewardReduction[_pid][_user]) {
        ...
        uint256 pending = multiplier.mul(_userAmount).mul(pool.fixedApr).mul(getLPPrice(_pid)).div(getTokenPrice()).div(SECONDS_PER_YEAR.mul(100));
        ...
        if (_amount > 0) {
            ...
            if (address(glxReferral) != address(0)) {
                ...
                if (referrer != address(0)) {
                    uint256 totalFund = _amount.mul(getLPPrice(_pid)).div(1e4);
    ...
    function migrate(uint256 _pid, address _user, uint256 _amount, uint256[] memory _investments_amount , uint256[] memory _investments_lock_until) external{
        ...
        if(!arrayAdded[_pid][_user]){
            userArr.push(UserPool({
                pid: _pid,
                user: _user
            }));
            arrayAdded[_pid][_user] = true;
        }
    ...
    function c9eMigrate(uint256 _pid, address _user) internal {
        require(!c9eMigrated[_pid][_user], "user has been migrated");
        ...
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        IC9EChef.UserInfo memory c9eUser = c9eChef.userInfo(_pid, _user);
        uint256 _amount = c9eUser.amount;
        uint256 _lastHarvest =  c9eUser.lastHarvest;
        uint256 _startInvestmentPosition = c9eUser.startInvestmentPosition;

        if(_amount > 0){
            user.amount = user.amount.add(_amount);
            uint256 _investmentLength = c9eChef.getInvestmentLength(_pid, _user);
            for(uint256 i=_startInvestmentPosition;i<_investmentLength;i++){
                IC9EChef.DepositAmount memory _investment = c9eChef.getInvestmentInfo(_pid, _user, i);
                user.investments.push(DepositAmount({
                    amount: _investment.amount,
                    lockUntil: _investment.lockUntil
                }));
            }
            user.lastHarvest = _lastHarvest;
            pool.totalAmount = pool.totalAmount.add(_amount);
        }

        c9eMigrated[_pid][_user] = true;
    }

    function c9eMigrateArr(uint256[] memory _pid, address[] memory _user) external onlyOwner{
        require(_pid.length == _user.length, "not valid");
        for(uint256 i=0;i<_pid.length;i++){
            c9eMigrate(_pid[i], _user[i]);
        }
    }

    function setNotRewardReduction(uint256[] memory _pid, address[] memory _user, bool isNotReduction) external onlyOwner{
        require(_pid.length == _user.length, "not valid");
        for(uint256 i=0;i<_pid.length;i++){
            notRewardReduction[_pid[i]][_user[i]] = isNotReduction;
        }
    }
    ...
    function payDirectCommission(uint256 _pid,uint256 _amount, address referrer) internal {
        ...
        glxReferral.recordReferralCommission(referrer,directCommissionAmount.mul(getTokenPrice()).div(1e4));
    }
        ...
        if (_amount > 0) {
            ...
            if (
                ...
                glxReferral.reduceTotalFund(
                    ...
                    _amount.mul(getLPPrice(_pid)).div(1e4),
                    0
                );
            }
        }
    ...
    function withdrawInvestment(uint256 _pid) public nonReentrant {
        ...
        if(_totalWithdrawalAmount > 0 && _totalWithdrawalAmount <= user.amount){
            ...
            if (
                ...
                glxReferral.reduceTotalFund(
                    glxReferral.getReferrer(msg.sender),
                    _totalWithdrawalAmount.mul(getLPPrice(_pid)).div(1e4),
                    0
                );
            }
        }
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
        ...
        if (
            ...
            glxReferral.reduceTotalFund(
                glxReferral.getReferrer(msg.sender),
                amount.mul(getLPPrice(_pid)).div(1e4),
                0
            );
        }
    ...
    function payOrLockupPendingToken(uint256 _pid) internal {
        ...
        uint256 _userAmount = 0;
        if (pool.lockingPeriod >= 15552000 && !notRewardReduction[_pid][msg.sender]) {
            for(uint i=user.startInvestmentPosition; i<user.investments.length;i++){
                if(user.investments[i].lockUntil.sub(pool.lockingPeriod) <= block.timestamp.sub(15552000)){
                    _userAmount = _userAmount.add(user.investments[i].amount.div(2));
                } else {
            ...
            if (address(glxReferral) != address(0)) {
                ...
                if (commissionAmount > 0 && _referrer!=address(0)) {
                    ...
                    glxReferral.recordReferralCommission(
                        _referrer,
                        commissionAmount.mul(getTokenPrice()).div(1e4)
                    );
