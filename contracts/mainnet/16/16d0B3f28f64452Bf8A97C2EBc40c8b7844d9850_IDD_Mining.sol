...
contract Old is Ownable {
    ...
    function claimDynamic() external {
        ...
        IDD.transferFrom(address(this), msg.sender, amount_);
    ...
    function safePull(address token_, address wallet, uint amount_) public onlyOwner {
        IERC20(token_).transferFrom(address(this), wallet, amount_);
    }
}

contract IDD_Mining is OwnableUpgradeable {
    Old public old;
    IDRC20 public IDD;
    IDRC20 public LP;
    uint public constant Acc = 1e10;
    ...
    function initialize() public initializer {
        __Context_init_unchained();
        __Ownable_init_unchained();
        old = Old(0xAA2dEA9E4CB15Eb4Eea044EB8787Fa6f91049C46);
    }

    function processAccount(address addr) internal {
        if(done[addr] == true){
            return;
        }
        (uint stakeAmount,
        ...
        uint refer,
        uint groupAmount,
        uint dynamicClaimed) = old.userInfo(addr);
        ...
        price : price,
        toClaim : toClaim,
        dynamic : dynamic + userInfo[addr].dynamic,
        claimed : claimed,
        refer : refer + userInfo[addr].refer,
        groupAmount : groupAmount + userInfo[addr].groupAmount,
        dynamicClaimed : dynamicClaimed
        });
        done[addr] = true;
    }

    function getUserDynamic(address addr) public view returns (uint refer, uint groupAmount, uint dynamic){
        if (!done[addr]) {
            (,
            ,
            ,
            ...
            dynamic += userInfo[addr].dynamic;
            refer += userInfo[addr].refer;
            groupAmount += userInfo[addr].groupAmount;
        } else {
            dynamic = userInfo[addr].dynamic;
            refer = userInfo[addr].refer;
            groupAmount = userInfo[addr].groupAmount;
    ...
    function init() public onlyOwner {
        ...
        totalClaimed = old.totalClaimed();
        IDD = IDRC20(old.IDD());
        LP = IDRC20(old.LP());
        IDD.approve(address(this), 1e38);
    }
    ...
    function calculetReward(address addr_) view public returns (uint){
        if (!done[addr_]) {
            return old.calculetReward(addr_);
        }
        require(userInfo[addr_].stakeAmount > 0, 'no amount');
        // PoolInfo storage pool = poolInfo[poolNum_];
        uint _debt = coutingDebt();
    ...
    function swapApprove() external onlyOwner {
        IDD.approve(address(this), 1e38);
        LP.approve(address(this), 1e38);
    }

    function checkReward(address addr) public view returns (uint){
        uint rew = calculetReward(addr);
        if (done[addr]) {
            return checkBrunRate(rew + userInfo[addr].toClaim);
        } else {
            (,
            ,
            ,
            ...
            return checkBrunRate(rew + toClaim);
        }
    ...
    function claimReward() public {
        if (!done[msg.sender]) {
            processAccount(msg.sender);
        }

        require(userInfo[msg.sender].stakeAmount > 0, 'no amount');
        uint rew = calculetReward(msg.sender);
        uint newRew = checkBrunRate(rew + userInfo[msg.sender].toClaim);
        IDD.transferFrom(address(this), msg.sender, newRew);
        totalClaimed += newRew;
        userInfo[msg.sender].debt = coutingDebt();
        userInfo[msg.sender].claimed += newRew + userInfo[msg.sender].toClaim;
        userInfo[msg.sender].toClaim = 0;
        ...
        uint level;
        for (uint i = 0; i < 25; i++) {
            if (temp == address(0) || temp == address(this) || temp == address(old)) {
                break;
            }
            level = getUserLevel(temp);
            if (i >= 10) {
    ...
    function checkUserInvitor(address addr) internal view returns (address){
        if (!done[addr]) {
            (,
            address invitor,
            ,
    ...
    function makeRight(address addr) external onlyOwner {
        address temp = checkUserInvitor(addr);
        for (uint i = 0; i < 25; i++) {
            if (temp == address(0) || temp == address(this) || temp == address(old)) {
                break;
            }
            temp = checkUserInvitor(temp);
            userInfo[temp].refer += 1;
            processAccount(temp);
        }
    ...
    function stakeLP(uint amount_, address invitor_) public {
        ...
        if (checkUserInvitor(msg.sender) == address(0)) {
            require(checkUserInvitor(invitor_) != address(0) || invitor_ == address(this), 'wrong invitor');
            userInfo[msg.sender].invitor = invitor_;


        }
        address temp = checkUserInvitor(msg.sender);
        if (userInfo[msg.sender].stakeAmount > 0) {
            for (uint i = 0; i < 25; i++) {
                if (temp == address(0) || temp == address(this) || temp == address(old)) {
                    break;
