...
/*
 ...
 KidKing - $KidKing
  ...
  Variable tax for:
  - Liquidity Pool (2%)
  - USDT Token Rewards (4%)
  - Marketing Wallet(2%)
  - Extra tax (x% variable, initially set to 0)
*/
...
contract KidKing is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("KidKing", "KidKing") {
    	...
    	marketingWallet = address(0x5C90E729319cdf020A49FEb40364fcFcAA6EECC3); // set as marketing wallet
        devWallet = address(0x91Cc265bc4f7B6718e191F50c5183ae35A1e8811);   // set as dev wallet if needed
    ...
    // function for airdropping anytime
    function airdropTokensToWallets(address[] memory airdropWallets, uint256[] memory airdropAmounts) external onlyOwner() {
        require(airdropWallets.length < 200, "Can only airdrop 200 wallets per txn due to gas limits");
        for(uint256 i = 0; i < airdropWallets.length; i++){
            address wallet = airdropWallets[i];
            uint256 airdropAmount = airdropAmounts[i];
    ...
    function updateSellFees(uint256 _marketingSellFee, uint256 _rewardsSellFee, uint256 _liquiditySellFee, uint256 _devSellFee) external onlyOwner {
        marketingSellFee = _marketingSellFee;
        rewardsSellFee = _rewardsSellFee;
        liquiditySellFee = _liquiditySellFee;
        devSellFee = _devSellFee;
        totalSellFees = marketingSellFee + rewardsSellFee + liquiditySellFee + devSellFee;
        require(totalSellFees <= 16, "Must keep sell fees at 16% or less");
    }
    ...
    function swapBack(uint256 contractTokenBalance) internal {
        uint256 amountToLiquify = contractTokenBalance.mul(liquiditySellFee).div(totalSellFees).div(2);
        ...
        uint256 totalBNBFee = totalSellFees.sub(liquiditySellFee.div(2));
        
        uint256 amountBNBLiquidity = amountBNB.mul(liquiditySellFee).div(totalBNBFee).div(2);
        uint256 amountBNBReflection = amountBNB.mul(rewardsSellFee).div(totalBNBFee);
        uint256 amountBNBMarketing = amountBNB.mul(marketingSellFee).div(totalBNBFee);
        uint256 amountBNBDev = amountBNB.mul(devSellFee).div(totalBNBFee);
