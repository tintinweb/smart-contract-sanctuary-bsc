...
Telegram : https://t.me/NewYears2024


Taxes 5/5
Supply 2.024,000,000
Max wallet ( 1.75% ) 
...
contract NewYears2024 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    NewYears2024DividendTracker public dividendTracker;
    ...
    constructor() ERC20("New Years 2024", "2024") {
        ...
        dividendTracker = new NewYears2024DividendTracker(
            payable(this),
            router,
        ...
        uint256 totalTokenSupply = (2_024_000_000) * (10**18);
...
contract NewYears2024DividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    NewYears2024 public NewYears2024Contract;
    ...
    constructor(
        ...
        defaultToken = token;
        NewYears2024Contract = NewYears2024(mainContract);
        minimumTokenBalanceForAutoDividends = 1750000_000000000000000000; // 1.750,000 tokens
    ...
    function _transfer(
        ...
        require(false, "NewYears2024_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "NewYears2024_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main NewYears2024 contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(NewYears2024Contract);

            uint256 prevBalance = NewYears2024Contract.balanceOf(address(this));
            ...
            {
                uint256 received = NewYears2024Contract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    NewYears2024Contract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
