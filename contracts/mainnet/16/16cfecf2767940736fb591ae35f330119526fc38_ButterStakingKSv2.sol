...
contract ButterStakingKSv2 {
    ...
    address MainPoolAddress = address(0xE50cb76A71b0c52Ab091860cD61b9BA2FA407414);
    address MAIN_TOKEN_ADDRESS = address(0xD23811058Eb6e7967D9a00dc3886E75610c4AbBa); 
    ...
    uint public depositAllowed = 1;
    PoolInfo public PoolData;
    uint256 public teamFee = 40;
    uint256 public MainPoolPid = 3;
    uint256 public MainPoolActivated = 0;
    ...
    address[] public syrupAddresses;

    constructor() public {
        teamAddress = 0xFCD677d4Bbee1fB1B5fB5C195E902c4e5F98a090;
        teamAddress2 = 0xE5009C5ac2d84196484fa0848a3fBa96460B71e9;
        // CHANGE THIS ON CHANGE OF EXCHANGE
        uniswapV2Router = IUniswapV2Router02(address(0x05E61E0cDcD2170a76F9568a110CEe3AFdD6c46f)); //SWAP CONTRACT
    ...
    function currentPool() external view returns (ISmartChef) {
        return PoolData.current;
    }
        
    function tvl() external view returns (uint256) {
        return PoolData.stakedAmount;
    }
    ...
    function WithdrawAndRecalculateRewards(uint8 cancelswap) private {        
        PoolInfo memory pool = PoolData;
        if (MainPoolActivated == 0) {
            IBEP20 rewardToken = IBEP20(PoolData.current.rewardToken());
            pool.current.withdraw(pool.stakedAmount);
            uint256 afterRewardBalance = rewardToken.balanceOf(address(this));
            uint256 beforeMainBalance = MAIN_TOKEN.balanceOf(address(this));            
            if (afterRewardBalance > 0 && cancelswap == 0) {
                swapTokensForMain(rewardToken, afterRewardBalance);
                uint256 afterMainBalance = MAIN_TOKEN.balanceOf(address(this));
