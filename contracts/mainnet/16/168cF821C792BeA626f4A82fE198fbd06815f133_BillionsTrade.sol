...
contract BillionsTrade is Context, Ownable {
    ...
    uint256 private constant PERCENT_WHITELIST = 3000;
    uint256 private constant PERCENT_FIRSTLEVEL = 300;
    uint256 private constant PERCENT_SECONDLEVEL = 250;
    uint256 private constant PERCENT_THIRDLEVEL = 150;
    ...
    function createDeposit(uint256 amountDeposit, address adr) private {
        ...
        uint256 limitPay = amountDeposit.mul(percent).div(BASE_PERCENT);
        ...
        user.amountDeposits = user.amountDeposits.add(amountDeposit);
        user.limit = user.limit.add(limitPay);
