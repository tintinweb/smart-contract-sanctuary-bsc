...
contract DebloxNFT is ERC721Enumerable, Ownable {
    ...
    string private _tokenBaseURI = "https://meta.deblox.io/";
    ...
    */
    ...
    function reveal() public onlyOwner {
        require(!saleLive, "Cannot change status during live sale");
        revealed = true;
    }
  
    function setPrice(uint256 _newCost) public onlyOwner {
        require(!saleLive, "Cannot change price during live sale");
        price = _newCost;
    }
  
    function setNotRevealedURI(string memory _notRevealedURI) public onlyOwner {
        require(!saleLive, "Cannot change revealed status during live sale");
    ...
    function setMaxMintAmount(uint256 _limit) public onlyOwner {
        require(!saleLive, "Cannot change max mint amount during live sale");
        maxMintAmount = _limit;
    }
    
    function setMaxSupply(uint256 _limit) public onlyOwner {
        require(!saleLive, "Cannot change max supply amount during live sale");
        maxSupply = _limit;
    }
     ...
     * Rules: 
     * - Sale must be live
     * - The mint amount must be 1 or more
     * - Tht mint amount must less than maxMintAmount per session
     * - Must pay sufficent BNB
     * - Cannot exceed the maxSupply
     * - Per address limit cannot be exceeded
     * - (Optional) If only open to whitelisted addresses:
     *   - msg.sender must be in whitelisted list
     */
    function mint(uint256 _mintAmount) external payable {
        require(saleLive, "Sale must be active to mint");
        require(_mintAmount > 0, "need to mint at least 1 NFT");
        require(_mintAmount <= maxMintAmount, "max mint amount per session exceeded");
        require(price * _mintAmount <= msg.value, "INSUFFICIENT_BNB");
