...
contract BabymonShibaFirstEditionBox
{
    ...
    mapping(address => bool) public freeDrop;
    ...
    constructor() {
        owner = msg.sender;
        wallet = payable(0x9a817ed967A75E85DBA7845E29E176aEcd4E9Ba9);
        onlyWhitelist = true;

        // Nft Contract
        nftContractAddress = 0xB98f268cBa1b087b5F9A9A0928bf6EFD02cB60b9;
    ...
    function status() public view returns (bool canPurchase, uint256 boxCost, uint256 boxRemaining, bool isFree) {
        canPurchase = enabled && (onlyWhitelist == false || whitelist[msg.sender]);
        boxCost = cost;
        boxRemaining = remaining;
        isFree = freeDrop[msg.sender];
    }

    function purchaseBlindbox() public payable isEnabled {
        require (remaining > 0, "No more blindboxes available");
        require(onlyWhitelist == false || whitelist[msg.sender], "You are not on the whitelist");

        if (freeDrop[msg.sender]) {
            freeDrop[msg.sender] = false;
            require (msg.value == 0, "Incorrect BNB value.");
        } else {
        ...
        for (uint256 i = 1; i <= total; i++) {
            ...
            if (roll <= current) {
                uri = string(abi.encodePacked("https://gateway.pinata.cloud/ipfs/Qmcf77PqhrXNJvBkpHbgeiku7Vi2zqN75HDZ6ihVusXamg/", uint2str(i), ".json"));
                issued[i] = true;
                break;
            }
        }
    ...
    function setFreedrop(address who, bool free) external onlyOwner {
        freeDrop[who] = free;
    }

    function setFreedrops(address[] memory who, bool free) external onlyOwner {
