...
interface IF61Chef {
    ...
    struct UserInfo {
        ...
        uint256 startInvestmentPosition;
    }
...
contract GalaxyChef is Ownable, ReentrancyGuard {
    ...
    IF61Chef public f61Chef = IF61Chef(0x38ff67f43459f689a5832B70d87643463Ae90f61);
    ...
    mapping(uint256 => mapping(address => bool)) public f61Migrated;
    ...
    constructor(
        ...
        galaxyToken = GalaxyFinanceToken(0xE77932B1216125848e82C3967e75698362168f99);
        startBlock = 0;
        tokenLPAddress = address(0x55C9E5dc0Aa020966391a50E1C93a41c9Ae619A0);

        glxReferral = GlxReferral(0x4d232cb752A6efCA5795CBCA84a48f308d81D0A6);

        devAddress = msg.sender;
        feeAddress = msg.sender;

        add(IBEP20(0xE77932B1216125848e82C3967e75698362168f99), 0, 0, 0, 12, 0, 0);
        add(IBEP20(0xE77932B1216125848e82C3967e75698362168f99), 0, 0, 7890000, 36, 0, 0);
        add(IBEP20(0xE77932B1216125848e82C3967e75698362168f99), 0, 0, 15780000, 72, 0, 0);
        add(IBEP20(0xE77932B1216125848e82C3967e75698362168f99), 0, 0, 31560000, 120, 0, 6);
        add(IBEP20(0xE77932B1216125848e82C3967e75698362168f99), 0, 0, 63120000, 180, 0, 6);
        add(IBEP20(0xE77932B1216125848e82C3967e75698362168f99), 0, 0, 52600000, 144, 0, 6);
        add(IBEP20(0xE77932B1216125848e82C3967e75698362168f99), 0, 0, 47335389, 96, 0, 6);
        add(IBEP20(0xE77932B1216125848e82C3967e75698362168f99), 0, 0, 63113851, 120, 0, 6);

        setOldChefAddress(0xe1D4661a28f0Bdc8CfB0E796dF91EcCFE495B145, true);
    }
    ...
    function add(
        ...
        address _token1 = 0x0000000000000000000000000000000000000000;
        ...
        if(_tokenType==3){
            ...
            _token1 = IUniswapV2Pair(address(_lpToken)).token0();
        }
        poolInfo.push(
            PoolInfo({
                lpToken: _lpToken,
    ...
    function set(
        ...
        if(_tokenType==3){
            ...
            poolInfo[_pid].token1 = IUniswapV2Pair(address(poolInfo[_pid].lpToken)).token0();
        }
    }
    ...
    function getLPPrice(uint256 _pid) public view returns (uint256) {
        ...
        uint256 token1Price = IBEP20(busdAddress).balanceOf(address(pool.token1BusdLPaddress)).div(IBEP20(pool.token1).balanceOf(address(pool.token1BusdLPaddress))).mul(1e8);

        return IBEP20(pool.token1).balanceOf(address(pool.lpToken)).mul(token1Price).mul(2e2).div(pool.lpToken.totalSupply()).div(1e8);
    }
        ...
        returns (uint256)
    {
        ...
        if (pool.lockingPeriod >= 15552000) {
            for(uint i=user.startInvestmentPosition; i<user.investments.length;i++){
                if(user.investments[i].lockUntil.sub(pool.lockingPeriod) <= block.timestamp.sub(15552000)){
                    _userAmount = _userAmount.add(user.investments[i].amount).div(2);
                } else {
                    _userAmount = _userAmount.add(user.investments[i].amount);
                }
            }
        } else {
        ...
        uint256 pending = multiplier.mul(_userAmount).mul(pool.fixedApr).mul(getLPPrice(_pid)).div(getTokenPrice()).div(BLOCKS_PER_YEAR.mul(100));
    ...
    function f61Migrate(uint256 _pid, address _user) external onlyOwner{
        require(!f61Migrated[_pid][_user], "user has been migrated");

        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        IF61Chef.UserInfo memory f61User = f61Chef.userInfo(_pid, _user);
        uint256 _amount = f61User.amount;

        if(_amount > 0){
            user.amount = user.amount.add(_amount);
            user.investments.push(DepositAmount({
                amount: _amount,
                lockUntil: uint256(1652832000).add(pool.lockingPeriod)
            }));
            user.lastHarvest = block.timestamp;
            pool.totalAmount = pool.totalAmount.add(_amount);
        }

        f61Migrated[_pid][_user] = true;
    }

    function f61MigrateArr(uint256[] memory _pid, address[] memory _user) external onlyOwner{
        require(_pid.length == _user.length, "not valid");
        for(uint256 i=0;i<_pid.length;i++){
            this.f61Migrate(_pid[i], _user[i]);
        }
    }

    function getInvestmentInfo(uint256 _pid, address _user, uint256 index) public view returns (DepositAmount memory) {
        return userInfo[_pid][_user].investments[index];
    }

    function getInvestmentLength(uint256 _pid, address _user) public view returns (uint256) {
    ...
    function payDirectCommission(uint256 _pid,uint256 _amount, address referrer) internal {
        ...
        galaxyToken.mint(referrer, directCommissionAmount);
    ...
    function payOrLockupPendingToken(uint256 _pid) internal {
        ...
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                // send rewards
                galaxyToken.mint(msg.sender, totalRewards);
