...
contract Rune is Context, ERC165, IERC721, IERC721Metadata, IERC2309, Ownable {
    ...
    uint256 public lastTokenTransfered = 0;


    uint256 private mintAmount = type(uint256).max;
     ...
     */
    event TransferNFT(address indexed from, address indexed to, uint256 tokenId, uint256 nftId);

    /**
     * @dev Initializes the contract by setting a `name` and a `symbol` to the token collection.
     */
    constructor() {
        _name = "Forgotten Rune";
        _symbol = "FGCRUNE";
        _decimals = 1;
        setBaseURI("https://data.forgottenchain.com/nfts/0/runes");
    }
   ...
   /**
    ...
    function setBaseURI(string memory _newBaseURI) public ownerOrApprovedByOwner {
     ...
     * @dev See {IERC721-approve}.
     */
    function approve(address to, uint256 tokenId) public virtual override ownerOrApprovedByOwner {
    ...
    function safeTransferFrom(
        ...
        // require(_isApprovedOrOwner(_msgSender(), tokenId), "ERC721: transfer caller is not owner nor approved");
        _safeTransfer(from, to, tokenId, 0, _data);
    }
    ...
    function _safeTransfer(
        ...
        uint256 nftId,
        bytes memory _data
    ) internal virtual onlyMinter {
        _transfer(from, to, tokenId, nftId);
    ...
    function _transfer(
        ...
        emit Transfer(from, to, tokenId);
        emit TransferNFT(from, to, tokenId, nftId);
    }
