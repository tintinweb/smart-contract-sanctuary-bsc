...
contract Cooky is BEP20, Auth {
    ...
    string private constant TOKEN_NAME = "Cooky";
    string private constant TOKEN_SYMBOL = "COOKY";
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        require(from != ADDR_ZERO, "COOKY: transfer-from-zero");
        require(to != ADDR_ZERO, "COOKY: transfer-to-zero");
    ...
    function _checkTxLimit(address sender, uint256 amount) internal view {
        require (amount <= maxTxAmount || _maxTxExempt[sender] || isAuthorized(sender), "COOKY: max-tx-size-exceeded");
    }
    ...
    function _checkMaxWallet(address sender, address recipient, uint256 amount) internal view {
        ...
        require ((heldTokens + amount) <= maxWalletSize || _maxWalletExempt[recipient] || amms[recipient] || isAuthorized(sender), "COOKY: max-wallet-exceeded");
    }
    // #endregion
    ...
    function setSwapEnabled(bool enabled) external authorized {
        require (swapEnabled != enabled, "COOKY: value-already-set");

        swapEnabled = enabled;
    }

    function setSwapThreshold(uint256 nextThreshold) external authorized {
        require(swapThreshold != nextThreshold, "COOKY: value-already-set");
    ...
    function setFeeExempt(address holder, bool exempt) external authorized {
        require(_feeExempt[holder] != exempt, "COOKY: already-set");
    ...
    function setMaxWalletExempt(address holder, bool exempt) external authorized {
        require(_maxWalletExempt[holder] != exempt, "COOKY: already-set");
    ...
    function setMaxTxExempt(address holder, bool exempt) external authorized {
        require(_maxTxExempt[holder] != exempt, "COOKY: already-set");
    ...
    function setAmm(address amm, bool isMaker) external authorized {
        require(amms[amm] != isMaker, "COOKY: already-set");
    ...
    function setMarketingWallet(address nextMarketingWallet) external authorized {
        require(nextMarketingWallet != marketingWallet, "COOKY: value-already-set");
    ...
    function setDevelopmentWallet(address nextDevelopmentWallet) external authorized {
        require(nextDevelopmentWallet != developmentWallet, "COOKY: value-already-set");
    ...
    function setStakingWallet(address nextStakingWallet) external authorized {
        require(nextStakingWallet != stakingWallet, "COOKY: value-already-set");
    ...
    function setLiquidityReceiver(address nextLiquidityReceiver) external authorized {
        require(nextLiquidityReceiver != liquidityReceiver, "COOKY: value-already-set");
    ...
    function setBuyFees(uint256 nextMarketingFee, uint256 nextDevelopmentFee, uint256 nextLiquidityFee, uint256 nextStakingFee) external authorized {
        require((nextMarketingFee + nextDevelopmentFee + nextLiquidityFee + nextStakingFee) <= 2000, "COOKY: total-fees-exceed-20-percent");
        require(nextMarketingFee <= 1000 && nextDevelopmentFee <= 1000 && nextLiquidityFee <= 1000 && nextStakingFee <= 1000, "COOKY: single-fee-exceeds-10-percent");
    ...
    function rescueBalance(IBEP20 token, uint256 percentage) external authorized {
        require(percentage >= 0 && percentage <= 100, "COOKY: value-not-between-0-and-100");

        uint256 balance = token.balanceOf(address(this));

        require(balance > 0, "COOKY: contract-has-no-balance");
