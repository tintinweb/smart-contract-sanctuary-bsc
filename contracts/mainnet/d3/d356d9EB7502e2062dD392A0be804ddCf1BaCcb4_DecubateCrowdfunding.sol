...
contract DecubateCrowdfunding is IDecubateCrowdfunding, Ownable, ReentrancyGuard {
  ...
  uint8[] public weights = [1, 3, 6, 12, 24, 48];
   ...
   *
   */
  function registerForAllocation() external override returns (bool) {
  ...
  function fundAgreement(uint256 _investFund) external override nonReentrant returns (bool) {
    ...
    // Allocation of user
    uint256 alloc = getUserAllocation(msg.sender);
   ...
   *
   */
  function getAllocationForTier(uint8 _tier, uint8 _multi) external view returns (uint256) {
...
interface IDecubateCrowdfunding {
  ...
  function registerForAllocation() external returns (bool);
