...
contract AnTonyETH is Context, IERC20, Ownable{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    mapping (address => bool) private _isExcluded;  
    uint256 private _totalSupply = 10000000 *  10**9;  
    string private _name = "AnTony";
    string private _symbol = "AnTony";
    ...
    function AntonyFromReward(address account) external onlyOwner() {
    ...
    function setAntonyLiquidityFee(uint256 rLiquidity) external onlyOwner {       
    ...
    function setAntonyETHDevFee(uint256 rDev) external onlyOwner {       
        if(_isExcluded[address(this)])
        {
            totalFees.dev +=rDev;
        }        
	...
	function getAntonyDividend(uint256 share, uint256 dividendsPerShare) internal pure returns (uint256) {
