...
contract buyBackAndBurnMainstreetChecker is Ownable{
    ...
    function getAmountsOut() public returns(uint mainstreetAmountOut) {
        address[] memory path = new address[](2);
        path[0] = BANANA_TOKEN;
        path[1] = MAINST_TOKEN;
        uint[] memory mainstAmountsOut = IApeSwapRouter(APESWAP_ROUTER).getAmountsOut(IBEP20(BANANA_TOKEN).balanceOf(address(this)), path);
        return mainstAmountsOut[1];
    }

    function calculateSlippage(uint mainstreetAmount, uint slippagePercentage) public view returns(uint slippageAmount) {
        uint slippage = (mainstreetAmount / 100) * slippagePercentage;
        return slippage;
    }

    function buyBackAndBurnMainstreet(uint bananaAmount, uint mainstreetAmount) public returns(uint mainstBurned) {
        address[] memory path = new address[](2);
        path[0] = BANANA_TOKEN;
        path[1] = MAINST_TOKEN;
