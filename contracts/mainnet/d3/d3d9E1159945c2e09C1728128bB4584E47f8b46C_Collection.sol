...
contract Collection is Strings, Ownable {
    ...
    function addCollecterTo(address addr) external {
        require(!mapCollecter[addr], "collecter exists");
        require(addr != owner, "don't add owner as collecter");
        mapCollecter[addr] = true;
        mapCollecterList[++collecterCount] = CollecterData(addr, true);
    }
    ...
    function getCollecter(uint32 index) external view returns (bool res, address addr, bool isActivated) {
        if(mapCollecterList[index].collecter != address(0)) {
            res = true;
            addr = mapCollecterList[index].collecter;
            isActivated = mapCollecterList[index].isCollecter;
        }
    }
    ...
    function getWithdrawer(uint32 index) external view returns (bool res, address addr, bool isActivated) {
        if(mapWithdrawerList[index].withdrawer != address(0)) {
            res = true;
            addr = mapWithdrawerList[index].withdrawer;
            isActivated = mapWithdrawerList[index].isWithdrawer;
        }
    }
