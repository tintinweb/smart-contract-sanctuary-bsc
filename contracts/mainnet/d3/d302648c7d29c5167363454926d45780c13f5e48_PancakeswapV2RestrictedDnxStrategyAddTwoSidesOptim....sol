...
contract PancakeswapV2RestrictedDnxStrategyAddTwoSidesOptimal is
  OwnableUpgradeSafe,
  ...
  IStrategy
{
  ...
  modifier onlyWhitelistedWorkers() {
    require(
      ...
      "PancakeswapV2RestrictedDnxStrategyAddTwoSidesOptimal::onlyWhitelistedWorkers:: bad worker"
    );
    _;
  }
  ...
  function initialize(IPancakeRouter02 _router, IVault _vault)
    external
    initializer
  {
  ...
  function execute(
    ...
    (uint256 farmingTokenAmount, uint256 minLPAmount) = abi.decode(
      data,
      (uint256, uint256)
    );
    ...
    bool isReversed;
    {
      ...
      (uint256 baseTokenReserve, uint256 farmingTokenReserve) = lpToken
        .token0() == baseToken
        ? (r0, r1)
        : (r1, r0);
      ...
      (path[0], path[1]) = isReversed
        ? (farmingToken, baseToken)
        : (baseToken, farmingToken);
    ...
    require(
      ...
      "PancakeswapV2RestrictedDnxStrategyAddTwoSidesOptimal::execute:: insufficient LP tokens received"
    );
    require(
      lpToken.transfer(msg.sender, lpToken.balanceOf(address(this))),
      "PancakeswapV2RestrictedDnxStrategyAddTwoSidesOptimal::execute:: failed to transfer LP token to msg.sender"
    );
  ...
  function setWorkersOk(address[] calldata workers, bool isOk)
    external
    onlyOwner
  {
