...
{
    function Resolve(uint256 id, uint256 amount, uint256 maxRewardMultiplier) external virtual; 
}
...
contract CandleGenieKillTheBoss is Ownable, ReentrancyGuard 
{
   ...
   receive() external payable {}
    ...
    {
        require(msg.sender == address(refereeContract), "Only CG contracts allowed");
        _;
    }
    ...
    function Impact(uint256 id, uint256 reward) external onlyCGContracts
    {
    ...
    function _safePunch(address user, uint256 amount) internal
    {
        ...
        // Referee
        refereeContract.Resolve(punch.id, punch.punchAmount, maxRewardMultiplier); 
