...
library poolUtil {
    ...
    function calcUnits(stData storage _self, stHolders storage _holders, address _user, bool _liquidate) internal view returns (uint _units) {
        ...
        if (!_liquidate) {            
            ...
            for (uint d = 0; d < _holders.dHolders.length; d++) {     
                ...
                uint _dTime = _holders.dHolders[d].timestamp - _self.lastProcess; //difference between deposit and last harvest user NOT in pool
                uint _tAmt;
                uint _cAmt;
                if (_dTime > 0) {
    ...
    function getUserInfo(stData storage _self, stHolders storage _holders,address chefContract, address _user) public view returns (uint _amount, uint _depositDate, uint _units, uint _accumulatedRewards) {
    ...
    function returnValue(uint value) external pure returns (uint) {
        return value;
    }
}
