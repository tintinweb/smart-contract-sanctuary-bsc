...
contract CRAYONCHAIN is Context, IERC20, Ownable {
    ...
    string private constant _name = "Crayon Chain";
    string private constant _symbol = "CRAYON";

    address public Crayon__Wallet = 0xCB42e6F9b534a1AF7C465ee2082225217F92E6e1;
    address public Crayon__XChain_Wallet = 0xCB42e6F9b534a1AF7C465ee2082225217F92E6e1;

    uint256 public Crayon__Max_Wallet_Size = 10000000000 * 10**9;
    uint256 public swapTokensAtAmount = 2000000000 * 10**9;
    uint256 public Crayon_Max_Buy_Size = 10000000000 * 10**9;
    uint256 public Crayon_Max_Sell_Size = 4000000000 * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[Crayon__Wallet] = true;
        _isExcludedFromFee[Crayon__XChain_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[Crayon__Wallet] = true;
        allowedTransfer[Crayon__XChain_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= Crayon_Max_Buy_Size, "You are exceeding Crayon_Max_Buy_Size");
            require(balanceOf(to) + amount <= Crayon__Max_Wallet_Size, "You are exceeding Crayon__Max_Wallet_Size");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= Crayon_Max_Sell_Size, "You are exceeding Crayon_Max_Sell_Size");
            if(to != pair){
                require(balanceOf(to) + amount <= Crayon__Max_Wallet_Size, "You are exceeding Crayon__Max_Wallet_Size");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(Crayon__Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(Crayon__XChain_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function Crayon_Update_MarketingWallet(address newWallet) external onlyOwner{
        Crayon__Wallet = newWallet;
    }
    
    function Crayon_Update_TeamWallet(address newWallet) external onlyOwner{
        Crayon__XChain_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        Crayon_Max_Buy_Size = maxBuy * 10**decimals();
        Crayon_Max_Sell_Size = maxSell * 10**decimals();
    }
    
    function updateCrayon__Max_Wallet_Size(uint256 amount) external onlyOwner{
        Crayon__Max_Wallet_Size = amount * 10**decimals();
    }
