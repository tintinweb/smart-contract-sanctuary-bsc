...
// File: contracts/bet/interfaces/game/IRange.sol
...
contract Range is BaseFunction, IRange {
    ...
    function setWhichGamesResultNumber(uint256 round, uint256 which, uint256 number, uint256[] calldata ver) public virtual override onlyOwner {
        ...
        require(number == (ver.length + 1), "Range: setWhichGamesResultNumber number is not == ver.length + 1");

        _gameRoundInfo.roundGameMap[round].gameMap[which].resultNum = number;
        for (uint256 i = 0; i < number; ++i) {
            if (i == 0) {
                _rangeScoreRoundInfo.roundRangeScoreMap[round].rangeScoreMap[which].rangeScore[i].start = 0;
            } else {
                _rangeScoreRoundInfo.roundRangeScoreMap[round].rangeScoreMap[which].rangeScore[i].start = ver[i - 1];
            }

            if (i == ver.length) {
                _rangeScoreRoundInfo.roundRangeScoreMap[round].rangeScoreMap[which].rangeScore[i].end = UINT256_MAX;
            } else {
                _rangeScoreRoundInfo.roundRangeScoreMap[round].rangeScoreMap[which].rangeScore[i].end = ver[i];
            }
        }
