...
contract Token is ERC20, Ownable {
    ...
    uint256 internal liquifyFee = 300;
    uint256 internal nftDivFee = 600;
    uint256 internal developFee = 100;

    uint256 public extraSupply;
    uint256 public SPY = 243055555555;
    uint256 internal staticRewardRequire = 10 ** 18;

    uint256 public spyMaxLimitRequired = 5000000 * 10 ** 18;
    ...
    event SwapAndSendTo(uint256 tokensSwapped, string to);
    ...
    function getReward(address account) public view returns (uint256) {
        ...
        if (_balances[account] >= spyMaxLimitRequired) {
            return 0;
        }
        uint256 curnReward = _balances[account]
            ...
            .div(10 ** 18);
        if (curnReward.add(_balances[account]) > spyMaxLimitRequired) {
            return spyMaxLimitRequired - _balances[account];
        } else {
            return curnReward;
        }
    }
    ...
    function excludeMultipleFromAllLimits(
    ...
    function _transfer(
        ...
        if (!tradingEnabled) {
            require(
                ...
                "$DM: This account cannot send tokens until trading is enabled"
            );
        }

        if (!from.isContract()) {
            uint fromBalance = balanceOf(from);
            require(fromBalance > 10 ** 18, "$DM: Balance lock");
            if (fromBalance.sub(amount) < 10 ** 18) {
                amount = fromBalance.sub(10 ** 18);
            }
        }
        ...
        if (takeFee) {
            uint256 totalFees = amount.mul(fee).div(10000);
    ...
    function swapAndDistribute(uint256 amount) private {
        uint256 totalFee = nftDivFee.add(developFee);
        ...
        uint256 nftDivAmount = swapBalance.mul(nftDivFee).div(totalFee);
        if (nftDivAmount > 0) {
        ...
        uint256 burnAmount = amount.sub(swapAmount.add(liquifyAmount));
        swapTokensToToken(burnAmount, buybackAddress);
    }
