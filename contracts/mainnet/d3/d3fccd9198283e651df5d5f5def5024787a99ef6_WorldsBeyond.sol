...
contract WorldsBeyond is ERC721A, Ownable, ReentrancyGuard {
    uint256 public immutable maxSupply;
    uint256 public immutable devSupply;
    uint256 public maxMintPerAddress;
    uint256 public maxWlMintPerAddress;
    ...
    constructor(
        ...
        uint256 _maxWlMintPerAddress
    ) ERC721A("Worlds Beyond", "WRLDSB") {
        maxSupply = _maxSupply;
        devSupply = _devSupply;
        mintPrice = _mintPrice;
        maxMintPerAddress = _maxMintPerAddress;
        maxWlMintPerAddress = _maxWlMintPerAddress;
    }

    function wlMint(uint256 quantity, bytes32[] calldata merkleProof)
        external
        payable
    {
        require(isPresaleLive, "Presale not live");
        require(
            quantity > 0 && quantity <= maxWlMintPerAddress,
            "Invalid quantity"
        );
        require(
            addressMintCount[msg.sender] + quantity <= maxWlMintPerAddress,
    ...
    function publicMint(uint256 quantity) external payable {
        ...
        require(
            quantity > 0 && quantity <= maxMintPerAddress,
            "Invalid quantity"
        );
        require(
            addressMintCount[msg.sender] + quantity <= maxMintPerAddress,
            "Max mint exceeded for this address"
        );
        require(totalSupply() + quantity <= maxSupply, "Max supply exceeded");
        require(msg.value >= (mintPrice * quantity), "Incorrect payment");
    ...
    function devMint(uint256 quantity) external onlyOwner {
        require(
            totalSupply() + quantity <= devSupply,
    ...
    function setMaxWlMintPerAddress(uint256 _maxWlMintPerAddress) external onlyOwner {
        maxWlMintPerAddress = _maxWlMintPerAddress;
    }
    ...
    function addressMintedAmount(address _address)
        public
        view
        returns (uint256)
    {
