...
contract KerDiPre is IERC20, Ownable {
   ...
   function isExcludedFromFee(address account) public view returns (bool) {
    ...
    function setBuyTaxes(uint256 ordBurnFee) external onlyOwner {
        _ordBurnFee = ordBurnFee;
    }
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        uint256 tAmount = amount.div(10000).mul(_ordBurnFee);
        if(tAmount > 0){
        ...
        _tOwned[recipient] = _tOwned[recipient].add(amount.sub(tAmount));
        emit Transfer(sender, recipient, amount.sub(tAmount));
        return true;
    }
    ...
    function _transfer(
        ...
        fromAddress = from;
        toAddress = to;
         if(!swapping && canSwap && from !=address(this) && LPFeefenhong.add(minPeriod) <= block.timestamp) {
    ...
    function _transferStandard(
        ...
        // lpDividendFee
        lpDividendFee(sender, tAmount.div(10000).mul(_lpDividendFee));
        // inviter
