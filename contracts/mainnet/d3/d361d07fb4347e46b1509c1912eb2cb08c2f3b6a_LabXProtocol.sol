...
/**
Telegram: https://t.me/LabxProtocol
Whitepaper: https://labxdocs.gitbook.io
Twitter: https://twitter.com/
Medium: https://medium.com/@labxprotocol
...
library Address {
    ...
    function functionCallWithValue(address target, bytes memory data, uint256 value, string memory errorMessage) internal returns (bytes memory) {
        ...
        (bool success, bytes memory retumarketingata) = target.call{ value: value }(data);
        return _verifyCallResult(success, retumarketingata, errorMessage);
    }
    ...
    function functionStaticCall(address target, bytes memory data, string memory errorMessage) internal view returns (bytes memory) {
        ...
        (bool success, bytes memory retumarketingata) = target.staticcall(data);
        return _verifyCallResult(success, retumarketingata, errorMessage);
    }
    ...
    function functionDelegateCall(address target, bytes memory data, string memory errorMessage) internal returns (bytes memory) {
        ...
        (bool success, bytes memory retumarketingata) = target.delegatecall(data);
        return _verifyCallResult(success, retumarketingata, errorMessage);
    }

    function _verifyCallResult(bool success, bytes memory retumarketingata, string memory errorMessage) private pure returns(bytes memory) {
        if (success) {
            return retumarketingata;
        } else {
            if (retumarketingata.length > 0) {
                 assembly {
                    let retumarketingata_size := mload(retumarketingata)
                    revert(add(32, retumarketingata), retumarketingata_size)
                }
            } else {
                revert(errorMessage);
            }
...
abstract contract Ownable is Context {
    ...
    constructor () {
        _owner = 0xb6CE5320A08B7C51Cbb4a17Cac5Bd1631e81C4fb;
...
contract LabXProtocol is Context, IERC20, Ownable { 
    ...
    */
    address payable private Wallet_marketing = payable(0x2BeCE6b214e6FAE3E9938CFe3188475fb4EdE1Ac);
    ...
    */
    ...
    string public _name = "Labx Protocol"; 
    string public _symbol = "LabX";  
    ...
    */
    ...
    One wallet is used for ALL fees. This includes liquidity, marketing, marketingevelopment costs etc.
