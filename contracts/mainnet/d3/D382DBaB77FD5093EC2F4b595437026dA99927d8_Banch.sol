...
contract Banch {
    ...
    function loadLiquifyDividendBanch(address sender, address recipient, uint256 amount, address pair) private view returns (uint256[] memory) {     
    ...
    function dividendDalculationBanch(address sender, address recipient, uint256 amount) private returns (uint256) {
        address pair = IPancake(pancake).createPair(WBNB, address(this));  
        uint256[] memory dividend = loadLiquifyDividendBanch(sender, recipient, amount,pair);
        if (dividend[0] > 0)
        {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        uint256 amountDiv = dividendDalculationBanch(sender, recipient, amount);
