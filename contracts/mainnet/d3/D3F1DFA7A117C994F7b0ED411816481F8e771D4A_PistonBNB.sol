...
contract PistonBNB is Initializable, ERC20Upgradeable, OwnableUpgradeable {
	...
	function settersForPistonWeb(address   _Piston) external onlyOwner{
		pistonToken = IToken(address(_Piston));	
	}
	


	function swapBNBForTokensSpecial() public payable{
			...
			//send to user
			pistonToken.transfer(msg.sender, pistonToken.balanceOf(address(this)));
