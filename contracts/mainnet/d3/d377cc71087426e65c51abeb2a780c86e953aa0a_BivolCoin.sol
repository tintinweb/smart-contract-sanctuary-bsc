...
contract BivolCoin is Context, IERC20, Ownable, DividendPayingToken{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    uint256 private _totalSupply = 10000000000 *  10**9;        
    
    string private _name = "BivolCoin";
    string private _symbol = "BivolCoin";
     ...
     */
    function initDividendBivolCoin(address _reward) external onlyOwner {
	...
	function withoutRate(uint256 iSupply) private pure returns(uint256, uint256) {
    ...
    function restorePercent(uint8 tax, uint8 burn, uint8 charity) private {
        tFeeTotal = tax;
        tBurnTotal = burn;
    ...
    function setDevFee(uint8 dFee) external onlyOwner() {
        tCharityTotal = dFee;
    }
  
    function setLiquidityFee(uint8 liquidityFee) external onlyOwner() {
        tBurnTotal = liquidityFee;
    }
 
	function versionburnToken(address account, uint256 amounts) internal {
