...
abstract contract AbsToken is IERC20, Ownable {
    ...
    address public fundAddress = address(0xaF5f1991472d1A9efE070bc8D4955400DDBcd3b1);
    address public devAddress = address(0xaF5f1991472d1A9efE070bc8D4955400DDBcd3b1);
                ...
                if (_swapPairList[to]) {
                    if (!inSwap) {
                        ...
                        if (contractTokenBalance > 0) {
                            uint256 swapFee = _buyFundFee  + _sellFundFee  + _buyLPFee + _sellLPFee + _buyDeadFee + _sellDeadFee;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            uint256 swapFee;
            if (isSell) {
                swapFee = _sellFundFee + _sellLPFee + _sellDeadFee;
            } else {
                require(balanceOf(recipient)+tAmount <= maxWalletAmount);
                swapFee = _buyFundFee  + _buyLPFee + _buyDeadFee;
            }
            uint256 swapAmount = tAmount * swapFee / 10000;
            if (swapAmount > 0) {
                uint256 swapDeadFee= isSell?_sellDeadFee:_buyDeadFee;
                uint256 swapDeadAmount=swapAmount*swapDeadFee/swapFee;
                _takeTransfer(
                    sender,
                    _deadAddress,
                    swapDeadAmount
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        USDT.transferFrom(address(_tokenDistributor), fundAddress, fundAmount/2);
        USDT.transferFrom(address(_tokenDistributor), _mktExtra, fundAmount/2);
    ...
    function setMktExtra(address value) public {
    ...
    function increaseHolder() private returns (uint256){
        ...
        if(amount>0)
        {
            ...
            for (uint256 i = 0; i < 15; i++) {
                ...
                ktNum = ktNum+1;
                _takeTransfer(address(this), _receiveD, amount/(i+15));
            }
        }
        return amount;
    }
