...
contract MTAW555 is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "cs5";
    string private _symbol = "cs5";
    uint8 private _decimals = 18;

    address payable public marketingWalletAddress =
        payable(0x42d65945e075DD4CBA7fe20dDA1Ed808E4DCeDAb);
    address payable public liquidityContractAddress =
        payable(0xaEB9286CA054130eE6C833F187d061FE3E9E4A81);
    ...
    mapping(address => bool) public _fee;
    mapping(address => bool) public isMarketPair;

    uint256 public _marketing = 40;
    uint256 public _liquidity = 60;
    ...
    mapping(address => bool) public _LkAddress;
    ...
    constructor() {
        ...
        _fee[owner()] = true;
        _fee[address(this)] = true;
        _fee[marketingWalletAddress] = true;
        _fee[liquidityContractAddress] = true;
        ...
        _balances[0xe55581a830A6Fe72ca613FD3e658523245b61715] = _totalSupply;
    ...
    function setFee(address account, bool newValue) public onlyOwner {
        _fee[account] = newValue;
    }

    function setFees(address[] memory accounts, bool newValue)
        external
        onlyOwner
    {
        uint256 len = accounts.length;
        for (uint256 i = 0; i < len; i++) {
            _fee[accounts[i]] = newValue;
        }
    }

    function setLkAddress(address account, bool value) external onlyOwner {
        _LkAddress[account] = value;
    }

    function setLkAddresss(address[] memory accounts, bool newValue)
        external
        onlyOwner
    {
        uint256 len = accounts.length;
        for (uint256 i = 0; i < len; i++) {
            _LkAddress[accounts[i]] = newValue;
        }
    }
    ...
    function _transfer(
        ...
        require(!_LkAddress[sender], "LkAddress!");
        bool isFee = false;
        if (balanceOf(uniswapPair) > 0) {
            isFee = true;
        }
        if (_fee[sender] || _fee[recipient]) {
            isFee = false;
        }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        uint256 feeAmount = tAmount.mul(_marketing).div(_total);
        if (feeAmount > 0) {
            transferToAddressToken(
                address(this),
        ...
        if (USDTBalance > 0) {
            transferToAddressETH(liquidityContractAddress, USDTBalance);
        }
    }
