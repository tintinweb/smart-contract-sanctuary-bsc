...
contract CryptoAquaticFarmer is Ownable, RequestToken {
    ...
    function buyGasFrom(address wallet) public onlyMain_ {
        ...
        createBuy(amount.sub(fee), wallet);
    ...
    function userData(address wallet_) public view returns (UserData memory) {
        User memory user = players[wallet_];
        (uint256 kmsAv_, uint256 kmVal, uint256 kms_) = calculateKms(wallet_);
