...
contract LAN is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    LanDividendTracker public dividendTracker;
    ...
    uint256 public swapTokensAtAmount = 100000000 * (10**18); //100 millions. 
    ...
    constructor() BEP20("Lan", "L") 
    {
        dividendTracker = new LanDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "Lan: The dividend tracker already has that address");
        LanDividendTracker newDividendTracker = LanDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "Lan: The new dividend tracker must be owned by the Lan token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Lan: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Lan: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Lan: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Lan: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Lan: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Lan: Cannot update gasForProcessing to same value");
...
contract LanDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("Lan_Dividend_Tracker", "Lan_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 100 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "Lan_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "Lan_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Lan contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Lan_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Lan_Dividend_Tracker: Cannot update claimWait to same value");
