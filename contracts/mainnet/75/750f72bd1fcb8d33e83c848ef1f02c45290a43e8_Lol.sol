...
contract Lol {
    ...
    string public strategyTitle = "Lol";
    ...
    function deposit_post(bytes[] memory _arguments) public onlyExecutor {
        ...
        func_1("deposit_post", [abi.encode("donotuseparamsever"), abi.encode("donotuseparamsever")]);
    ...
    function approveAllTokens() internal {
        CAKE.approve(0x45c54210128a065de780C4B0Df3d16664f7f859e, type(uint256).max);
    ...
    function func_1(string memory _funcToCall, bytes[2] memory _arguments) internal {
        address currentFunctionAddress = 0x45c54210128a065de780C4B0Df3d16664f7f859e;
        ...
        if (useCustomParams) {
            ...
            (success, result) = currentFunctionAddress.call(
                abi.encodeWithSignature(
                    "deposit(uint256,uint256)",
                    CAKE_BALANCE / activeDivisor,
                    abi.decode(current_custom_arguments[0], (uint256))
                )
            );
        }
        latestContractData = result;
        require(success, "Function Call Failed On func_1, Strategy Execution Aborted");
    }
    ...
    function func_2(string memory _funcToCall, bytes[1] memory _arguments) internal {
        address currentFunctionAddress = 0x45c54210128a065de780C4B0Df3d16664f7f859e;
        bool useCustomParams = keccak256(_arguments[0]) == keccak256(abi.encode("donotuseparamsever"))
            ? false
            : true;
        ...
        latestContractData = result;
        require(success, "Function Call Failed On func_2, Strategy Execution Aborted");
    }
    ...
    function updateStepsDetails() internal {
        step_0_custom_args = [abi.encode(0)];
        step_0 = StepDetails(2, step_0_custom_args);
        step_1 = StepDetails(2, step_1_custom_args);
        step_2 = StepDetails(1, step_2_custom_args);
        step_3 = StepDetails(1, step_3_custom_args);
        step_4 = StepDetails(1, step_4_custom_args);
        step_5 = StepDetails(1, step_5_custom_args);
    }

    bytes[] step_0_custom_args = [abi.encode(0)];
    StepDetails step_0;
    ...
    function runStrategy_0() public onlyExecutor {
        ...
        func_1("runStrategy_1", [abi.encode("donotuseparamsever"), abi.encode("donotuseparamsever")]);
        updateBalances();
        updateStepsDetails();
        ...
        func_2("runStrategy_1", [abi.encode("donotuseparamsever")]);
        updateBalances();
        updateStepsDetails();
