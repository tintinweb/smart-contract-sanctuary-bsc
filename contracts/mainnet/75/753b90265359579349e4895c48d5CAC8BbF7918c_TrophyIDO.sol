...
contract TrophyIDO is Ownable, ReentrancyGuard {
    ...
    uint256 public publicPhaseTime;
    ...
    constructor(
        ...
        publicPhaseTime = _publicPhaseTime;
    ...
    function setVesting(uint256[] memory _vestingTimes, uint256[] memory _vestingPercents) external onlyOwner {
        vestingTimes = _vestingTimes;
        vestingPercents = _vestingPercents;
    }

    function setPrice(uint256 _price, uint256 _listingPrice) external onlyOwner {
        price = _price;
        listingPrice = _listingPrice;
    }

    function setLPPercent(uint256 _lpPercent) external onlyOwner {
        lpPercent = _lpPercent;
    }
    ...
    function addWhitelistOverwrite(address[] memory accounts) public onlyOwner {
        for (uint256 i = 0; i < whitelistedList.length; i++) {
            whitelisteds[whitelistedList[i]] = false;
        }
    ...
    function isDoneVesting(address purchaser) public view returns (bool) {
        uint256 purchasedTokenAmount = calcPurchasedTokenAmount(purchaser);
        return
            purchaseDetails[purchaser].amount > 0 &&
            purchasedTokenAmount == purchaseDetails[purchaser].tokenAmountClaimed;
    }

    function getVestingTimes() external view returns (uint256[] memory) {
        return vestingTimes;
    }

    function getVestingPercents() external view returns (uint256[] memory) {
        return vestingPercents;
    }
    ...
    function getSalePhase() public view returns (SalePhase) {
        if (block.timestamp > publicPhaseTime) {
            return SalePhase.PUBLIC;
        }
        return SalePhase.WHITELIST;
    }

    function purchase() external payable occurring validAmount(msg.value) {
        SalePhase phase = getSalePhase();
    ...
    function calcClaimableTokenAmount(address purchaser) public view returns (uint256) {
        ...
        for (uint8 i = 0; i < vestingTimes.length; i++) {
            if (block.timestamp >= vestingTimes[i]) {
                if (i == vestingTimes.length - 1) {
                    claimableTokens = totalTokens.sub(purchaseDetails[purchaser].tokenAmountClaimed);
                } else {
                    claimableTokens = claimableTokens.add(
                        totalTokens.mul(vestingPercents[i]).div(RATE_PRECISION_FACTOR)
                    );
                }
            }
        }
    ...
    function finalize(address _lpTo, address _raiseTo) external onlyOwner {
        ...
        ROUTER.addLiquidityETH{value: ethLiqAmount}(address(token), tokenLiqAmount, 0, 0, _lpTo, block.timestamp);

        // for project
        payable(_raiseTo).transfer(currentCap.sub(ethLiqAmount));
    ...
    function claim() external whenFinalized {
        ...
        purchaseDetails[msg.sender].tokenAmountClaimed = purchaseDetail.tokenAmountClaimed.add(claimableAmount);
