...
contract LOVECOIN is ERC20, Ownable {
    ...
    // to lp holder
    uint256 public buyRewardLpFee = 2;
    uint256 public sellRewardLpFee = 2;
    ...
    // dev
    // address public usdtAddress= 0xcEeECf9d1E188C83B1e472df05Ad05b921164d9D;
    ...
    constructor() ERC20("LOVE", "LOVE")  {
        ...
        uint256 totalSupply = 9999 * (10**18);
    ...
    function distributeEth() private {
        ...
        // to market and lp holders
        uint256 toMarket = balance.div(sellMarketingFee1.add(sellRewardLpFee)) * sellMarketingFee1;
        IERC20(usdtAddress).transfer(marketingWalletAddress1, toMarket);
        //console.log("start distributeEth   %d ", balance);
        //console.log(" distributeEth to  market  %d ", toMarket);
        processLPReward();
    }
    ...
    function processLPReward() private {
        ...
        uint256 balance =  IERC20(usdtAddress).balanceOf(address(this)); 
        if (balance <= 0.00000001 * 10 ** 18 ) {
            return;
        }
        balance = balance.sub(0.00000001 * 10 ** 18);
                ...
                if (tokenBalance > 0 && !excludeHolder[shareHolder]) {
                    ...
                    if (amount >= 0.9 * 10 ** 18) {
                       tmpRewrdAmount =  balance.mul(amount).div(totalBigLP);
                        ...
                        IERC20(usdtAddress).transfer(shareHolder, tmpRewrdAmount);
