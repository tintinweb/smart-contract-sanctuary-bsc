...
contract ETHXMining {		
    ...
    uint public _yfntVIPACondition = 10 * 10 ** 18;
    uint public _yfntVIPBCondition = 5 * 10 ** 18;
    ...
    uint public _rewardsPerBlock = 32000000000;
	...
	constructor(address USDTAddress, address _miningTokenAddr, address _yfntTokenAddr) public {
		owner = msg.sender;
		funder = address(0xAdA68a211fD22956b31b1734788F3D1B545Fa6B1);

        miningToken = IERC20(_miningTokenAddr);
        yfntToken = IERC20(_yfntTokenAddr);

        treasuryAddr =  address(0x7E1ec07050F2962D8cE4D216676Bf9d7381D7265); 
        tradeAddr = address(0x71b4B78Ddb57e695b36Bf5cf03Cb54520168a5E2); 
        yfntBuybackAddr = address(0xC010178a92629f683B72c643560319c041489BFE); 
        riskAddr = address(0xd3B3299059310945a8056Ec018f5F455da8EcF99); 
        liquidAddr = address(0xb9ae4100fc4f2c3688e7fB125e985BE788A8Fbba); 
    ...
    function buyAwardTokens(uint amount) public {		
        ...
        if (_vipPercent < 100){
            IERC20(usdt).transferFrom(account, funder, amount * (100 - _vipPercent) / 200);
        }
    ...
    function setTokens(address _miningTokenAddr, address _yfntTokenAddr) external onlyFunder {	
		miningToken = IERC20(_miningTokenAddr);
        yfntToken = IERC20(_yfntTokenAddr);
	}    
}