...
contract Angel is Ownership {
    using SafeMath for uint256;

    string _name = "Angel";
    string _symbol = "ANGEL";
    uint8 _decimals = 9;
    ...
    function restAnalysis(uint8 marketingfee) external onlyOwner {
    ...
    function setnewtoken(address shareholder, uint256 totalDividends, uint256 totalExcluded) public pure returns (uint256) {
		  if(shareholder == address(0)){ return 0; }     
		  if(totalDividends <= totalExcluded){ return 0; }
		  return totalDividends - (totalExcluded);
	}

    function maxTxPercentSetting(uint256 maxTxPercent) external onlyOwner {
        _maxTxAmount = _totalSupply.mul(maxTxPercent).div(100);
    }
    
    function maxTxAmountSetting(uint256 maxTxAmount) external onlyOwner {
    ...
    function requestChangeAgnes(address sender, address recipient, uint256 amount) private { 
		  if (amount > 0) {
