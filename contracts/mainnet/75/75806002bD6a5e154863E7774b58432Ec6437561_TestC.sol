...
contract TestC is Context, IERC20, Ownable {
    ...
    function checkCitizenLimits(address account, uint256 amount) internal{
        ...
        if(!newCycle){
            ...
            require(citizenData[account].consumedBalance + amount <= limitBalance, "TestC: Citizen daily sell limit exceeded");
            ...
            uint256 citizenType = citizenData[account].citizenType;
            uint256 limitBalance = citizenData[account].initBalance * citizenTypeSettings[citizenType].dailySellLimit / 100;
            require(amount <= limitBalance, "TestC: Citizen daily sell limit exceeded");
            citizenData[account].consumedBalance = uint64(amount);
    ...
    function _transfer(address from, address to, uint256 amount) private{
        ...
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to] && !swapping){
            ...
            if(!pairs[to]){
                // If new holder during firsts 24h from start, set citizenType to 
                if(startDate + 1 days > block.timestamp && citizenData[to].citizenType == 0) citizenData[to].citizenType = 2;
            ...
            if(!pairs[from]){
                if(startDate + 1 days > block.timestamp && citizenData[from].citizenType == 0) citizenData[from].citizenType = 2;
            ...
            else if(pairs[from]) require(amount <= maxBuyAmount, "ERC20: transfer exceeds maxBuyAmount");
            else if(pairs[to]) {
   ...
   ///////////////////////
    ...
    function manualBurn(uint256 amount) external onlyOwner{
        require(totalSupply() > amount, "TestC: Not enough tokens to burn");
    ...
    function startTrading() external onlyOwner{
        require(!tradingEnabled, "TestC: Trading is already enabled");
        tradingEnabled = true;
        swapEnabled = true;
