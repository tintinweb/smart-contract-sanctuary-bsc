...
contract stakereward {
    ...
    uint256[32] public _priceID = [1*1e8,1*1e8,1*1e8,1*1e8,1*1e8,1*1e8,1*1e8,1*1e8,1*1e8,1*1e8,
                                   1*1e8,1*1e8,1*1e8,1*1e8,1*1e8,1*1e8,1*1e8,1*1e8,1*1e8,1*1e8,
                                   1*1e8,1*1e8,1*1e8,1*1e8,1*1e8,1*1e8,1*1e8,1*1e8,1*1e8,1*1e8,1*1e8,1*1e8];
    ...
    uint public increasePrice = 1*1e8;
    ...
    mapping(address => uint) public whiteList2;
    mapping(address => uint) public hasRewardwhiteList2;

    bool public _couuldStake=true;
        ...
        updateReward(msg.sender)
    {
        ...
        require(_id<_priceID.length, "_id err!");
        ...
        totalFee = sendPrice.mul(inviteFee.add(inviteFee).add(winnerFee).add(team1Fee).add(team2Fee)).div(BASE);
        ...
        if(_totalSupply[_id].mod(countsToReward) == 0)
        {
    ...
    function withdrawWhiteList2()
        external 
    {
        uint rtnAmt = whiteList2[msg.sender];
        require(rtnAmt > 0,"amount zero");
        whiteList2[msg.sender] = 0;
        hasRewardwhiteList2[msg.sender] = hasRewardwhiteList2[msg.sender].add(rtnAmt);
        IERC20(stakeToken).safeTransfer(msg.sender, rtnAmt);
    }
    ...
    function setwhiteList2(address[] memory accountAddr,uint[] memory _amounts)
        external onlyOwner
    {
        require( _amounts.length >= accountAddr.length, "len err");
        ...
        for(uint i=0;i<accountAddr.length;i++)
        {
            whiteList2[accountAddr[i]] = whiteList2[accountAddr[i]].add(_amounts[i]);
        }
    }
