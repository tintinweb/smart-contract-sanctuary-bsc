...
// File: contracts/fast-farm/MasterChef.sol
...
contract MasterChef is Ownable {
    ...
    constructor() public {
        fast = IERC20(0x2322AfAaC81697E770c19a58df587d8739777536);
        startTime = 1665410400;
        endTime = startTime.add(perDays);
    }
    ...
    function changeReward(uint256 _pid, uint256 newAmount) public onlyOwner {
        ...
        require(
            fast.balanceOf(address(this)) >=
                totalAmountFastTokens + newAmount - oldAmount,
            "add: not enough balance on contract"
        );

        pool.amountFastTokens = newAmount;
        totalAmountFastTokens = totalAmountFastTokens.sub(oldAmount).add(
            newAmount
        );
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (fastReward > 0) {
            pool.accFastPerShare = pool.accFastPerShare.add(
                fastReward.mul(1e18).div(lpSupply)
            );
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        uint256 pending = user.amount.mul(pool.accFastPerShare).div(1e18).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safeFastTransfer(msg.sender, pending);
    ...
    function emergencyFastWithdraw(uint256 _amount) public onlyOwner {
        require(
            _amount > 0,
            "emergencyFastWithdraw: amount must be greater than zero"
        );
