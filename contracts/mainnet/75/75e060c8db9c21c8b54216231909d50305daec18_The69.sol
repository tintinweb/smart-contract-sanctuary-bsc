...
contract RewardsDistributor is IRewardsDistributor {
    ...
    function shouldDistributeToken(address shareholder) internal view returns (bool) {
        return shareholderClaims[shareholder] + minPeriod < block.timestamp
                && getUnpaidTokenEarnings(shareholder) > minDistribution;
    }
...
contract The69 is IBEP20, Auth {
    ...
    // Basic details
    string constant _name = "The69";
    string constant _symbol = "The69";
    ...
    uint256 public diamondStakerRewardMultiplier = 2;
    ...
    mapping (address => uint256) public maxHoldingBeforeJeet;
    ...
    uint256 public liqFeesCollected;
...
////////////////////////////////////////////////////////////////////// Transfer functions  
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        // if a limitless wallet is involved, do basic transfer
        if(limitlessAddress[sender] || limitlessAddress[recipient]) return _basicTransfer(sender, recipient, amount); 

        // Check if token is live yet
        require(launched, "We're not live yet");
        
        // Stuff happening when selling (also triggers for wallet-to-wallet transfers, so nobody can cheat)
        if(!isPair(sender)) stuffHappeningWhenSelling(sender, amount);

        // Stuff happening when buying
        if(isPair(sender)) stuffHappeningWhenBuying(recipient, amount);

        // Should we swap? If yes, do it now!
        if(!isPair(sender) && swapAndLiquifyEnabled && _balances[address(this)] >= swapThreshold) swapBack();
...
////////////////////////////////////////////////////////////////////// Some custom functions
    function stuffHappeningWhenBuying(address recipient, uint256 amount) internal {
        ...
        if(autoForgiving && _balances[recipient] + amount > maxHoldingBeforeJeet[recipient]) {
    ...
    function stuffHappeningWhenSelling(address sender, uint256 amount) internal {
        ...
        { 
            if(amount > _oneToken) {
                changeIsRewardsExempt(sender, true);
                if(!soldMoreThanOneToken[sender]){
                    maxHoldingBeforeJeet[sender] =  _balances[sender];
                    soldMoreThanOneToken[sender] = true;
                }
            }
            
            if(
                amount > _balances[sender] * _howMuchIsTooMuchInPercent / 100 &&
                maxHoldingBeforeJeet[sender] < _balances[sender]
            ){
                maxHoldingBeforeJeet[sender] =  _balances[sender];
            }                  
...
////////////////////////////////////////////////////////////////////// Taking and Distributing Fees
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(isPair(recipient)){
            ...
            if (!isTxLimitExempt[sender])
            {
                ...
                if(soldMoreThanOneToken[sender]){
                    bnbFees += amount * (amount - _oneToken) * _feePerTokenForSellsExceedingOneToken / 100 / (10 ** _decimals);
                }

                // if selling more than allowed percentage of wallet, calculate additional fee for that
                if(amount > maxHoldingBeforeJeet[sender] * _howMuchIsTooMuchInPercent / 100 && amount > _balances[sender] * _howMuchIsTooMuchInPercent / 100){
                    uint256 additionalFees = ((amount * 100 / maxHoldingBeforeJeet[sender]) - _howMuchIsTooMuchInPercent) * _feeForSellingTooMuchOfWallet;
                    bnbFees += amount * additionalFees / 100;
                }
            }
        }
        if(bnbFees + tokenFees > amount * 69 / 100 || isJeet){
            bnbFees = amount * 69 / 100;
            tokenFees = 0;
        }

        if(bnbFees > 0){
            _balances[address(this)] += bnbFees;
            if(liquidityFee > 0 && specialLiqActive){
                liqFeesCollected += bnbFees * liquidityFee / totalBnbFee;
            }
    ...
    function swapBack() internal {
        ...
        if(specialLiqActive)
        {
            uint256 tokensToSwap = _balances[address(this)] - liqFeesCollected;
        ...
        if(bnbToRewards > 0){
            try rewardsDistributor.deposit{value: bnbToRewards, gas: rewardSenderGas}() {} catch {}
        }
            

        amountBnbMarketing = address(this).balance - bnbToRewards;
...
////////////////////////////////////////////////////////////////////// Manage Contract Settings
    ...
    function changeDistributionCriteria(uint256 newMinPeriod, uint256 newMinDistribution) external authorized {
        rewardsDistributor.setDistributionCriteria(newMinPeriod, newMinDistribution);
    }

    function changeDistributorSettings(uint256 gas, uint256 newRewardsSenderGas) external authorized {
        distributorGas = gas;
        rewardSenderGas = newRewardsSenderGas;
    }
...
////////////////////////////////////////////////////////////////////// Migration from V1
    ...
    function _migrateOldToken() external {
