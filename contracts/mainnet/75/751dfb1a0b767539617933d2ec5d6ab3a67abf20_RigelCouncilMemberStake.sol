...
interface stakeEvents {
    ...
    event ItemSeized(address indexed user, uint256 id_amount);
}



contract stakeStorage is stakeEvents, Ownable {
    ...
    modifier currentlyLocked() {
        (bool resolving, bool Lck, uint256 nextWithdrawalTime) = getP2Pdetails(msg.sender);
        if(resolving) revert isResolving();       
        if(Lck) revert isOnLocked();
        if (nextWithdrawalTime > block.timestamp) revert isOnLocked();
        _;
    }
    ...
    function checkWhitelistState() private view{
        if (whitelistStatus) {
            if (!whitelisted[msg.sender]) revert WhiteListRequire();
        }
    }
    ...
    function loose20(uint256 amount) internal returns (bytes memory) {
        ...
        if (userInfo.siezed) revert Fraud();
    ...
    function earn1155(bytes calldata _rank, uint256 tokenID) internal {
        ...
        MerchantStore memory store = 
            MerchantStore(msg.sender, tokenAddr, false, _rank, s_rank, tokenID, merchantsBadge[_rank].maxAmountOnDisputeToJoin, block.timestamp);
    ...
    function doJustice(address[] calldata _address, address _to) external onlyOwner {
        uint256 lent = _address.length;
        for (uint256 i; i < lent; ) {
            _justice(_address[i], _to);
            unchecked {
                i++;
            }
        }
    ...
    function _justice(address _account, address _to) private {
        uint256 gMount = councilStore[_account].amount;
        if(gMount != 0 ) {
            councilStore[_account].siezed = true;
            IERC20(RGP).transfer(_to, gMount);    
            emit ItemSeized(_account, gMount);
        } else {
            merchantStore[_account].siezed = true;
            uint256 tokenID = merchantStore[_account].TokenID;
            IERC1155(merchantStore[_account].NFT).safeTransferFrom(address(this), _to, tokenID, 1, "");
            emit ItemSeized(_account, tokenID);
        }
    }

    function addWhitelistAddresses(address[] calldata _addresses, bool _status) external onlyOwner {
        uint256 lent = _addresses.length;
        for (uint256 i; i < lent; ) {
    ...
    function getP2Pdetails(address account) internal view returns(bool, bool, uint256 ) {
        (
            bool resolving,
            bool isLocked,
            uint256 nextWithdrawalTime
        ) = IRigelDecentralizedP2PSystem(P2PAddress).getTotalUserLock(account);
        return (resolving, isLocked, nextWithdrawalTime);
    }
    ...
    function getMyRank(address account) external view returns(uint256) {      
        ...
        if(councilStore[account].amount != 0 ) {
            if (!councilStore[account].siezed) {
                return(councilStore[account].maxAmountToJoin);
            } else {return 0;}
        } else {
            if (!merchantStore[account].siezed) {
...
abstract contract Badge is stakeStorage {
