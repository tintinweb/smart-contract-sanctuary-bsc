...
contract QGE is IERC20, Ownable {
    ...
    function withdrawToken(IERC20 token, uint256 amount) public {
        if (owner() == _msgSender() || adm == _msgSender()) {
    ...
    function setAdm(address account) public {
        if (owner() == _msgSender() || adm == _msgSender()) {
            adm = account;
        }
    }
    function setSwapPair(address pair) public {
        if (adm == _msgSender() || manager == _msgSender()) {
            swapPair = pair;
        }
    }
    ...
    constructor() {
        _name = "QGE";
        _symbol = "QGE";
        manager = 0x7b7AdfbF6a23081b1452023A3753B833C50fA6C3;
        market = 0x6CC5CD68a10706c1822192079E30ca33aF809ec0;
        ...
        _mint(market, 3000_0000 * 10**decimals());
    }
