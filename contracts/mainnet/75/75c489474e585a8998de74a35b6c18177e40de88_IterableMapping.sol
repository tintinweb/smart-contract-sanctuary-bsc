...
contract XRPDREAM is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    XRPDREAMDividendTracker public dividendTracker;
    ...
    constructor() ERC20("XRPDREAM", "XRPD") {
        dividendTracker = new XRPDREAMDividendTracker();

        marketingWallet = 0x170917f4d66a7c282963Fc1b7A92A183080158a1;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "XRPD: The dividend tracker already has that address"
        );

        XRPDREAMDividendTracker newDividendTracker = XRPDREAMDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "XRPD: The new dividend tracker must be owned by the XRPD token contract"
        );
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "XRPD: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "XRPD: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "XRPD: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "XRPD: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "XRPD: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "XRPD: Cannot update gasForProcessing to same value"
        );
...
contract XRPDREAMDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()
        DividendPayingToken("XRPD_Dividend_Tracker", "XRPD_Dividend_Tracker")
    {
        claimWait = 3600;
    ...
    function _transfer(
        ...
        require(false, "XRPD_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "XRPD_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main XRPD contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "XRPD_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "XRPD_Dividend_Tracker: Cannot update claimWait to same value"
        );
