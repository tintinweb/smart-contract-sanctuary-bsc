...
function withdraw () public {
 ...
 if (user.activTORSREF_REWARD == true  /*&& user.checkpointLVR1RefBonus >= 5 minutes*/)
...
//user.checkpoint
//require (users[msg.sender].checkpoint <= user.checkpointLVR1RefBonus.add(10 minutes));

if(users[msg.sender].levels[0] >=1 && users[msg.sender].checkpoint <= user.checkpointLVR1RefBonus.add(10 minutes))
{
    user.activTORSREF_REWARD == true;
}
else 
{user.activTORSREF_REWARD == false;}

		//return user.activTORSREF_REWARD;
	}
