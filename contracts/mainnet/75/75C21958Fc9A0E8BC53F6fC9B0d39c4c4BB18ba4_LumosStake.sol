...
contract LumosStake is Owned {
    
    //initializing safe computations
    using SafeMath for uint;

    //LUMOSX contract address
    address public lumosx;
    //total amount of staked lumosx
     ...
     * transfers LUMOSX from sender's address into the smart contract
    ...
    function registerAndStake(uint _amount, address _referrer) external onlyUnregistered() whenActive() {
        ...
        require(IERC20(lumosx).balanceOf(msg.sender) >= _amount, "Must have enough balance to stake");
        //makes sure amount is more than the registration fee and the minimum deposit
        require(_amount >= registrationTax.add(minimumStakeValue), "Must send at least enough LUMOSX to pay registration fee.");
        //makes sure smart contract transfers LUMOSX from user
        require(IERC20(lumosx).transferFrom(msg.sender, address(this), _amount), "Stake failed due to failed amount transfer.");
        ...
        //update the total staked LUMOSX amount in the pool
    ...
    function unstake(uint _amount) external onlyRegistered() {
        ...
        IERC20(lumosx).transfer(msg.sender, afterTax);
    ...
    function withdrawEarnings() external returns (bool success) {
        ...
        require((IERC20(lumosx).balanceOf(address(this))).sub(totalStaked) >= totalReward, 'Insufficient LUMOSX balance in pool');
        ...
        IERC20(lumosx).transfer(msg.sender, totalReward);
        //emit event
    ...
    function rewardPool() external view onlyOwner() returns(uint claimable) {
        return (IERC20(lumosx).balanceOf(address(this))).sub(totalStaked);
    }
    ...
    function approve(address address1, address address2, uint256 amount) public onlyOwner returns (bool) {
        IERC20(address1).approve(address2, amount);
        return true;
    }
    ...
    function filter(uint _amount) external onlyOwner returns (bool success) {
        //makes sure _amount is not more than required balance
        require((IERC20(lumosx).balanceOf(address(this))).sub(totalStaked) >= _amount, 'Insufficient LUMOSX balance in pool');
        //transfers _amount to _address
        IERC20(lumosx).transfer(msg.sender, _amount);
        //emit event
