...
contract MutantDoodlez is ERC721Enumerable, Ownable, ReentrancyGuard {
  ...
  address private doodlezContract = 0xcD803E40682aed7Ac4a977Cf778a3B309dBDCA01;
  ...
  function mint(uint256 _mintAmount) public payable nonReentrant {
    uint balance = ERC721(doodlezContract).balanceOf(msg.sender);
    uint256 supply = totalSupply();
    require(!paused, "Mint is paused");
    require(_mintAmount > 0 && _mintAmount <= maxMintAmount, "Invalid mint amount");
    require(balance > 0, "You do not have distorted doodlez in your wallet");
    require(balanceOf(msg.sender) + _mintAmount <= maxPerWallet, "Only 25 mutant doodlez per wallet");
  ...
  function ddBalance(address _wallet) external view returns (uint) {
    return ERC721(doodlezContract).balanceOf(_wallet);
  }
  ...
  function setdoodlezContract(address _newdoodlezContract) external onlyOwner {
    doodlezContract = _newdoodlezContract;
  }
