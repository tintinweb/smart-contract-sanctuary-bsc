...
contract MainToken is ERC20, Ownable {
    ...
    constructor() ERC20("Mpo-main-t", "m-444") {
        buringSwitch = true;
    ...
    function setMarketingWallet(address market_) external onlyOwner {
        MarketingWallet = market_;
    }

    function setRdf(address rdf_) external onlyOwner {
        RDF = rdf_;
    }

    function setInvite(address invite_) external onlyOwner {
        INVITE = invite_;
    }
    ...
    function setLiquidityWallet(address pool_) external onlyOwner {
    ...
    // set uint
    function setFeeToInit() external onlyOwner {
            ...
            if (
                ...
                if (!transferW[msg.sender] && !transferW[recipient]) {
                    ...
                    if (
                        ...
                        // Finance
                        if (debtMarketing >= swapUToMarketingLimit && !_b) {
                            processFinanceMar();
                            _b = true;
                        }
                        if (debtRdf >= swapUToRdfLimit && !_b) {
                            processFinanceRdf();
                            _b = true;
                        }
