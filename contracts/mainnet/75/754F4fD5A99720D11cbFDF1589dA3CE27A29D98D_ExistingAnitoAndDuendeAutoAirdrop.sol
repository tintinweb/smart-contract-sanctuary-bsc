...
contract ExistingAnitoAndDuendeAutoAirdrop is
    Pausable,
    Ownable,
    ReentrancyGuard,
    ERC721Holder,
    ERC1155Holder
{
    ...
    mapping(uint256 => uint256) anitoRarities;
    ...
    constructor(
        ...
        anito = IAnito(_anito);
        anitoNew = IAnito(_anitoNew);
        duende = IDuende(_duende);
        duendeNew = IDuende(_duendeNew);
        operationsAddresses[msg.sender] = true;
        operationsAddresses[0x5BBbb8136f90E72603Af7c96af8c0137983831d4] = true;
    }

    function setAnitoRarities(
        uint256[] memory _anitoRarities
    ) public onlyOperators {
        for (uint256 i = 0; i < _anitoRarities.length; i++) {
            anitoRarities[i + 1] = _anitoRarities[i];
        }
    }

    function airdropAnito(
        uint256 fromTokenId,
        uint256 maxQty
    ) public onlyOperators {
        for (uint256 i = 0; i < maxQty; i++) {
            uint256 currentTokenId = fromTokenId + i;
            anitoNew.safeMintSingle(
                address(this),
                anitoRarities[currentTokenId]
            );
            ERC721(address(anitoNew)).safeTransferFrom(
                address(this),
                anito.ownerOf(currentTokenId),
                currentTokenId
            );
        }
    }

    function airdropDuende(
        uint256 fromTokenId,
        uint256 maxQty
    ) public onlyOperators {
        for (uint256 i = 0; i < maxQty; i++) {
            uint256 currentTokenId = fromTokenId + i;
            duendeNew.safeMintSingle(address(this));
            ERC721(address(duendeNew)).safeTransferFrom(
                address(this),
                duende.ownerOf(currentTokenId),
                currentTokenId
            );
        }
    }
