...
abstract contract Auth {
    ...
    0x3377Ca6eF5e57c3A450139d0f2B0a872C6b36691)
        ] = true;
...
contract RanShaoInu is IBEP20, Auth {
    using SafeMath for uint256;
    
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;
    string constant _name = "RanShao Inu";
    string constant _symbol = "RSHAO";
    ...
    mapping (address => bool) public isBlacklisted;//new

    uint256 liquidity = 15;
    uint256 marketing = 75;
    uint256 burn = 10;
    ...
    bool public tradingOpen = true; // new
    uint256 AB1 = 7 * 1 gwei; // new
    bool setFreeze = true;
    ...
    bool startSwap = true;
    uint256 acquireFactor = 3;
    uint256 transferFactor = 3;
    bool swapEnabled = true;
    uint256 swapThreshold = _totalSupply * 150 / 100000;
    uint256 public launchedAt; // new
    IDEXRouter router;
    address LPReceiver;
    address Distributor;
    address Dividend;
    ...
    function totalSupply() external view override returns (uint256) {return _totalSupply.sub(balanceOf(DEAD)).sub(balanceOf(ZERO));} // new
    ...
    // Set the launchedAt to token launch // new
    function launch() internal {
        launchedAt = block.number;
    }
    // Blacklist/unblacklist an address // new
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool){
        require(tradingOpen,"Trading not enabled yet");//new
        require(!isBlacklisted[recipient] && !isBlacklisted[sender], 'Address is blacklisted');//new
