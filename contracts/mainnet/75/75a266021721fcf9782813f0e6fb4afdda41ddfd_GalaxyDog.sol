...
contract GalaxyDog is ERC20 {
	...
	address public _nodeAddress = address(0xBB83AE9064876f4f33E642a9A4FC94A9c0B0494D);
    address public _fund10Address = address(0x51896599dB734F5A8D49a962b5825e1237B1E875);
	address public _fund051Address = address(0x5e7063B5F3fc73Be3084FD41c05EDef36959fc82);
	address public _fund052Address = address(0xAc7B911C1eDBce16F505F391834487796f440c86);
    ...
    constructor(address tokenOwner) ERC20("GalaxyDog", "GalaxyDog") {
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] ) {
			...
			if(from == uniswapV2Pair){
				if(startTime.add(600) > block.timestamp){
					require(balanceOf(to).add(amount) <= 10**23);
				}
				splitOtherToken();
	...
	function swapAndLiquify() public {
		...
		if(allAmount > usdtAmount){
			...
			USDT.transfer(_nodeAddress,newAmount.div(4));
			USDT.transfer(_fund10Address,newAmount.div(4));
			USDT.transfer(_fund051Address,newAmount.div(8));
			USDT.transfer(_fund052Address,newAmount.div(8));
		}
    }
