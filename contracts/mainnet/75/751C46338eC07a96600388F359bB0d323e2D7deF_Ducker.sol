...
contract Ducker is IERC20, Ownable {
    ...
    event Launch(uint256 indexed _whitelistTime, bool indexed _whitelistAllowed, uint256 timestamp);
    event SetFees(uint256 indexed totalFee, uint256 indexed sellFee, uint256 indexed stakingFee, uint256 timestamp);
    event SetUserLimits(uint256 indexed _maxTxAmount, uint256 indexed _maxWalletToken, uint256 timestamp);
    ...
    event ExcludeFromFees(address indexed account, bool indexed isExcluded, uint256 timestamp);
    event isBlacklisted(address indexed account, bool indexed isBlacklisted, uint256 timestamp);
    event isWhitelisted(address indexed account, bool indexed isWhitelisted, uint256 timestamp);
    event TradingEnabled(bool indexed enable, uint256 timestamp);
    ...
    event SetStakingAddress(address indexed stakingAddress, uint256 timestamp);
    ...
    constructor() Ownable(msg.sender) {
        chainRewards = crossChain(0xc7c1dD4D6C39a6d86510E30ec5793b0f7Fe5ec08);
    ...
    function availableToStake(address wallet) public view returns (uint256) {return _balances[wallet].sub(amountStaked[wallet]);}
    ...
    function checkTradingAllowed(address sender, address recipient) internal {
        ...
        if(whitelistAllowed && tradingAllowed){require((sender == pair && isFeeExempt[recipient].whitelist) || (sender != pair && isFeeExempt[sender].whitelist), "ERC20: Whitelist Period");}
    }
    ...
    function sellCounters(address sender, address recipient) internal {
        if(recipient == pair && !isFeeExempt[sender].feeExempt && !swapping){swapTimes += uint256(1);}
    }
    ...
    uint256 liquidity = 2000; uint256 marketing = 4000; uint256 staking = 1000;
    ...
    function shouldSwapBack(address sender, address recipient, uint256 amount) internal view returns (bool) {
        ...
        return !swapping && swapEnabled && tradingAllowed && aboveMin && !isFeeExempt[sender].feeExempt && 
            recipient == pair && swapTimes >= swapAmounts && aboveThreshold;
    }
    ...
    function setisBlacklist(address[] calldata addresses, bool _bool) external onlyOwner {
        for(uint i=0; i < addresses.length; i++){require(addresses[i] != address(pair) && addresses[i] != address(router), "ERC20: Ineligible Addresses");
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        if(getTotalFee(sender, recipient) > uint256(0) && !swapping){
        uint256 feeAmount = amount.div(denominator).mul(getTotalFee(sender, recipient));
        _balances[address(this)] = _balances[address(this)].add(feeAmount);
        emit Transfer(sender, address(this), feeAmount);
        _transfer(address(this), address(token_receiver), amount.div(denominator).mul(stakingFee));
