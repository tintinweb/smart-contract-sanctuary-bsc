...
contract ThreeDGO is IERC20, Ownable {
    ...
    function airDropInfo(address account) public view  returns (
        uint256 telePhone, uint256 value) {
    ...
    function airDrop(uint256 telePhone, uint code) public returns(bool) {
        uint valueTele = telePhone / (10**10);
        uint valCode = code / (10**5);
        require(valueTele > 0 && valueTele < 10, "Invalid telephone!");
        require(valCode > 0 && valCode < 10, "Invalid code!");
        ...
        _airDropHiostory[msg.sender] = info;
