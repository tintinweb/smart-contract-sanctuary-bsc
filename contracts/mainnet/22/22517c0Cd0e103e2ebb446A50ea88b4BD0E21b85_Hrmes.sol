...
contract Hrmes is Context, IBEP20, Ownable {
    ...
    //1 fee 2 nofee
    //uint8 private killTransferFee = 0; 
    ...
    function setBindNum(uint256 num) public onlyOwner{
        bindNum = num*(10**_decimals);
    }
    ...
    function _transfer(address from, address recipient, uint256 amount) internal {
        ...
        if(from == uniswapV2Pair){
            ...
            if(buyOnOff == 2){
                require(from == _creator || recipient == _creator || (whiteListSwitch == 2 && whiteList[from])|| (whiteListSwitch == 2 && whiteList[recipient]), "market close");
            }
        }

        // 绑定代数
        if(amount > (bindNum*(10**_decimals))){
