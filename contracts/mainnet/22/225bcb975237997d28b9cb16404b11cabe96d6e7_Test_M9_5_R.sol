...
function withdraw () public {
 ...
 if (activTORSREF_REWARD == true  /*&& user.checkpointLVR1RefBonus >= 5 minutes*/)
...
if(user.checkpoint <= user.checkpointLVR1RefBonus  )
{
    activTORSREF_REWARD2 == true;
}
else 
{activTORSREF_REWARD2 == false;}

		return activTORSREF_REWARD2;
	}

     	function getUserDiv3(address userAddress) public view returns (bool) {
		User storage user = users[userAddress];

if(user.checkpointLVR1RefBonus >= 1  )
{
    activTORSREF_REWARD3 == true;
}
else {activTORSREF_REWARD3 == false;}

return activTORSREF_REWARD3;
	}

     	function getUserDiv4(address userAddress) public view returns (bool) {
		User storage user = users[userAddress];
