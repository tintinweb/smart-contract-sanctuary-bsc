...
import "./IPCSCStaking.sol";
import "./Ownable.sol";
import "./IERC20.sol";
import "./SafeERC20.sol";

/// @title Staking Platform with fixed APY and lockup
contract PCSCStaking is IPCSCStaking, Ownable {
    ...
    uint8 public treasuryFee = 2;

    address treasuryAddress = 0x42573692B468B9C33E48fE9bf9b6ee91DC6FdA7D;
    address devFeeAddress = 0x42573692B468B9C33E48fE9bf9b6ee91DC6FdA7D;
    ...
    function withdrawResidualBalance() external onlyOwner {
        ...
        //    "Withdraw 1 day after endPeriod"
        //);
    ...
    /**
     * @notice set a new treasury fee (up to 25%)
     * @dev Only Owner can do it
     */
    function setTreasuryFees(uint8 newFee) external onlyOwner {
        require(newFee <= 25, "Treasury Fees cant be higher then 25%");
        treasuryFee = newFee;
    }

    /**
     * @notice set a new dev address
     * @dev Only actual dev can do it
     */
    function setDevAddress(address newDevAddress) external {
        require(devFeeAddress == _msgSender(), "Only actual dev address can do that.");
        devFeeAddress = newDevAddress;
    }

    /**
     * @notice set a new dev fee (up to 10%)
     * @dev Only actual dev can do it
     */
    function setDevFees(uint8 newDevFee) external {
        require(devFeeAddress == _msgSender(), "Only actual dev address can do that.");
        require(newDevFee <= 10, "Dev Fees cant be higher then 10%");
        devFee = newDevFee;
    }
    ...
    function _claimRewards() private {
        ...
        uint rewardFeeTreasury = (rewardsToClaim / 100 * treasuryFee);
        rewardsToClaim = rewardsToClaim - rewardFeeTreasury;

        uint rewardFeeDev = (rewardsToClaim / 100 * devFee);
        rewardsToClaim = rewardsToClaim - rewardFeeDev;

        token.safeTransfer(_msgSender(), rewardsToClaim);

        token.safeTransfer(treasuryAddress, rewardFeeTreasury);

        token.safeTransfer(devFeeAddress, rewardFeeDev);
