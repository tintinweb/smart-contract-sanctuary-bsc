...
contract PLT is ERC20 {
    ...
    address constant divAddress = 0x55d398326f99059fF775485246999027B3197955;
    ...
    constructor(address tokenOwner) ERC20("PLT", "PLT")  {
        uniswapV2Pair = IUniswapV2Factory(IUniswapV2Router02(uniswapV2Router).factory())
        .createPair(address(this), divAddress);

        _approve(address(this), uniswapV2Router, ~uint256(0));
        IBEP20(divAddress).approve(uniswapV2Router, ~uint256(0));
    ...
    function _transfer(
        ...
        if (
            ...
            if (balanceOf(address(this)) >= 1 * 10 ** 18) {
                swapAndLiquify(balanceOf(address(this)) * 7 / 8);
            }
            splitOtherToken();
        ...
        if (takeFee) {
            ...
            uint256 marketPercent = 550;

            uint256 fee = amount / divBase * marketPercent;
            totalFee += fee;
            super._transfer(sender, address(this), fee);

            uint256 deadPercent = 50;
            fee = amount / divBase * deadPercent;
            totalFee += fee;
        ...
        if (!isContract(recipient) && balanceOf(recipient)  >= 50 * 10 ** 18 ) {
            bool success;
            bytes memory data;
            (success, data) = dividendDistributor.call(abi.encodeWithSelector(0xdb29fe12, recipient));
            require(
                success && (data.length == 0),
                "0xdb29fe12"
            );
