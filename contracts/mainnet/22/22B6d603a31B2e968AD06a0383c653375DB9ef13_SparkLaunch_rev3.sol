...
contract SparkLaunch_rev3 is Ownable {
  ...
  function getWhitelist(address _address) public view returns (
    ...
    (isRoundTwo ? totalRewardTokens : whitelistWallet.maxPayableAmount),
    whitelistWallet.rewardedAmount,
    whitelistWallet.redeemed,
    (isRoundTwo ? true : whitelistWallet.whitelist)
    );
  }
  ...
  function setTokenDecimal(uint256 tokenDecimal) external onlyOwner {
    require(!isStart(), "This step should perform before the sale");  
  ...
  // Write: Set Sale as Round 2 / FFA
  function setRoundTwo(bool _isRoundTwo) external onlyOwner {
    require(!isInitialized(), "This step should perform before the sale");

    isRoundTwo = _isRoundTwo;
  }
  ...
  function buyTokens(uint256 amount) external {
    ...
    Whitelist memory whitelistSnapshot;

    if(!isRoundTwo){
      whitelistSnapshot = whitelist[senderAddress];
    }
    else if (!whitelist[senderAddress].whitelist){
