...
contract Staking is Ownable, ReentrancyGuard {
  ...
  {
    require(msg.value >= 0.01 ether, "not enough fees");
    (bool sent, bytes memory __) = _treasury.call{ value: msg.value }("");
  ...
  function claimRewards() external payable updateReward(msg.sender) {
    uint256 reward = _rewards[msg.sender];
    _rewards[msg.sender] = 0;
    IERC20(token).transfer(msg.sender, reward);
  }

  function retrieveData(uint256 block_) external view returns (uint256) {
    return _data[block_];
  }
