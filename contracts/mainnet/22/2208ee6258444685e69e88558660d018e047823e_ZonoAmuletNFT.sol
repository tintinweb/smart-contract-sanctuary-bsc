...
contract ZonoAmuletNFT is ERC721A, Ownable {
    using Strings for uint256;

    IERC20 public PAYMENT_TOKEN =
        IERC20(0xA8De787E6196A991a35E46a5B8d5505e15B486ad);
    ...
    uint256 public COST = 450000 ether;
    uint256 public WHITELIST_COST = 450000 ether;

    bool public PAUSED = true;

    mapping(address => uint256) private MAPPING_WHITELIST_MINT;

    constructor() ERC721A("Zono Amulet Govern", "ZANG") {}
    ...
    function isWhiteListed(bytes32 _leafNode, bytes32[] memory _proof)
        public
        view
        returns (bool)
    {
    ...
    function mintWhitelist(
        ...
        require(
            isWhiteListed(toLeaf(msg.sender, _index, _amount), _proof),
            "Invalid proof"
        );

        require(
            (MAPPING_WHITELIST_MINT[msg.sender] + _mintAmount) <=
                MAX_MINT_LIMIT_PER_WHITELIST,
        ...
        PAYMENT_TOKEN.transferFrom(
            msg.sender,
            address(this),
            WHITELIST_COST * _mintAmount
        );

        //Mint
        _mintLoop(msg.sender, _mintAmount);
        MAPPING_WHITELIST_MINT[msg.sender] =
            MAPPING_WHITELIST_MINT[msg.sender] +
            _mintAmount;
    }
    ...
    function mint(uint256 _mintAmount) public {
        if (msg.sender != owner()) {
            ...
            require(
                (ownerTokenCount + _mintAmount) <= MAX_MINT_LIMIT_PER_WALLET,
    ...
    function getHoldList(address _owner)
        external
        view
    ...
    function withdraw() public onlyOwner {
        PAYMENT_TOKEN.transferFrom(
            address(this),
            msg.sender,
            PAYMENT_TOKEN.balanceOf(address(this))
        );
    }
}