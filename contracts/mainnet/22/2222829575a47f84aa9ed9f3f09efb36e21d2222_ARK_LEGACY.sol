...
contract ARK_LEGACY is ERC721, ERC721Enumerable {
    ...
    ILEGACYOLD public constant OLD = ILEGACYOLD(0xfFFFCE5720d24765f0277222a21288444D27fFFF);
    ...
    constructor() ERC721("ARK Legacy NFT", "ALN") {
...
///////////////// NFT Rewards Pool
    ...
    function airdropEveryone() external onlyCEO {
        ...
        for(uint256 i = 0; i < minted; i++){
            owner = OLD.ownerOf(i);
            OLD.claimRewardsFor(owner);
            mintToWallet(owner,OLD.levelOfNft(i));
            vault.addSparkPlayer(owner);
            locked[i] = true;
        }
    }
}