...
// main contract
contract CASINOCHEEMS is IERC20Extended, Auth {
    using SafeMath for uint256;

    string private constant _name = "CASINO CHEEMS";
    string private constant _symbol = "$CC";
    uint8 private constant _decimals = 9;
    uint256 private constant _totalSupply =
        1_000_000_000_000 * 10**_decimals;

    address public rewardToken = 0xdFE6891ce8E5a5c7Cf54fFdE406A6C2C54145F71;
    ...
    function swapBack() internal swapping {
        uint256 amountTokenStaking = swapThreshold.mul(auctionFee).div(
            totalFee
        );
        ...
        payable(devFeeReceiver).transfer(amountBNBMarketing);
        _balances[auctionPool] = _balances[auctionPool].add(
            amountTokenStaking
        );
        emit Transfer(address(this), auctionPool, amountTokenStaking);
    ...
    function setBuyFee(uint256 _fee) public authorized {
        totalBuyFee = _fee;
        require(
    ...
    function setSellFee(uint256 _fee) public authorized {
        totalSellFee = _fee;
        require(
            totalSellFee < feeDenominator / 4,
            "Total fee should not be greater than 1/4 of fee denominator"
        );
    }
