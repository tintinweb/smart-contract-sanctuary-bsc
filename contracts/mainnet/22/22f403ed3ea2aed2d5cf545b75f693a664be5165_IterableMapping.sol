...
contract ELC is ERC20, Ownable {
    ...
    uint256 private buybackBNB;

    mapping (address => uint256) private traded;
    ...
    constructor() payable ERC20("Elancing", "ELC") {
        ...
        totalFee         = marketingFee + rewardsFee + teamFee + validatorFee + buyBackFee;

        marketingWallet = 0x862cD7bC48eBaB97436eE29953c62A8aaD2fBCDC;
        teamWallet = 0xffa1cF4835b3D41A02b7e4b565186ece8eF55Be8;
        validatorWallet = 0xF88f32a808ceA003963869685d467B1DaF59abB4;

        dividendTracker = new DividendTracker(500_000, rewardToken);
    ...
    function updateCooldownTime (uint256 _newCooldown) external onlyOwner {
        require (_newCooldown <= 30, "can't set more than 30 seconds");
        cooldown = _newCooldown;
    }

    function updatePriceImpact(uint256 _newPriceImpact) external onlyOwner {
        require (_newPriceImpact < 50, "can't set below 50 percent");
        priceImpactPercent = _newPriceImpact;
    }

    function updateBuybackThreshold (uint256 _amount) external onlyOwner {
        require (_amount > 1_000, "amount should be greator than minimum");
        buybackThreshold = _amount;
    }

    function setBuyback (bool _value) external onlyOwner {
        buybackEnabled = _value;
    }
    ...
    function updateFees(uint256 _marketingFee, uint256 _rewardsFee, uint256 _teamFee, uint256 _validatorFee, uint256 _buyBackFee) external onlyOwner {
        ...
        require(totalFee <= 10, "Buy fee cannot be more than 10%");
    }
    ...
    function changeTeamWallet(address _teamWallet) external onlyOwner {
        ...
        require(_teamWallet != DEAD, "Marketing wallet cannot be the zero address");
        
        teamWallet = _teamWallet;
    }

    function changeValidatorWallet(address _validatorWallet) external onlyOwner {
        require(_validatorWallet != validatorWallet, "Marketing wallet is already that address");
        require(!isContract(_validatorWallet), "Marketing wallet cannot be a contract");
        require(_validatorWallet != DEAD, "Marketing wallet cannot be the zero address");
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            uint256 price = getPriceOfToken(1 * 10 ** decimals());

            uint256 bnbShare = marketingFee + rewardsFee + teamFee + validatorFee + buyBackFee;
            ...
            if(contractTokenBalance > 0 && bnbShare > 0) {
                ...
                if(marketingFee > 0) {
                    uint256 marketingBNB = newBalance * marketingFee / bnbShare;
                    sendBNB(payable(marketingWallet), marketingBNB);
                }

                if(teamFee > 0) {
                    uint256 teamBNB = newBalance * teamFee / bnbShare;
                    sendBNB(payable(teamWallet), teamBNB);
                }

                if(validatorFee > 0) {
                    uint256 validatorBNB = newBalance * validatorFee / bnbShare;
                    sendBNB(payable(validatorWallet), validatorBNB);
                }

                if(buyBackFee > 0) {
                    buybackBNB += newBalance * buyBackFee / bnbShare;

                    if (buybackBNB > buybackThreshold && buybackEnabled && (price <= allTimeHigh * (100 - priceImpactPercent) / 100)){
                        buyBackTokens(buybackBNB);
                        buybackBNB = 0;
                    }
                }

                if(rewardsFee > 0) {
                    uint256 rewardBNB = newBalance * rewardsFee / bnbShare;
        ...
        if(takeFee) {
            ...
            if(from == uniswapV2Pair) {
                ...
                _totalFees = totalFee;
                require (block.timestamp - traded[from] >= cooldown, "Wait before selling. Cooldown enabled.");
