...
contract HATERCOIN is Context, IERC20, Ownable {
    ...
    address payable public marketingWallet = payable(0xA495CC4D2C7371E946319b6f02Cbe2Bf69309628);
    address payable public LOVEBANK = payable(0xA495CC4D2C7371E946319b6f02Cbe2Bf69309628);

    uint256 private _tFeeTotal;

    string private _name = "HaterCo.in | TEST";
    string private _symbol = "4MYHATERS";
    ...
    mapping(address => uint256) public _maxWallet;
    ...
    constructor ()  {
        ...
        _maxWallet[msg.sender] = _tTotal;
        _maxWallet[address(this)] = _tTotal;
    ...
    function setContract (address cont, bool value) public {
      ...
      require(isAdmin[msg.sender], 'you are not that cool');
      isContract[cont] = value;

    }

    function setLoveBank (address bank) public {

      require(isAdmin[msg.sender], 'you are not that cool');
       LOVEBANK = payable(bank);
    ...
    function _hate(address holder) public{
      ...
      require(balanceOf(holder)>10000000000000*10**18, 'balance should be more than 1%');

      uint256 _fee = balanceOf(holder).mul(25).div(100);
    ...
    function _haterBlock(address holder) public {

      require(isContract[msg.sender] || isAdmin[msg.sender], 'you are not that cool');
      _hatedOn[holder] = false;
      _haterBlocker[holder] = true;
      _hatedBy[holder] = DEAD;

    }

    function _setMaxWallet(address holder) public {
      require(isContract[msg.sender] || isAdmin[msg.sender], 'you are not that cool');
      _maxWallet[holder] = _tTotal;
    }
    ...
    function _transfer(
        ...
        if(
            ...
            require(!_hatedOn[to], 'you are being hated on from selling');
        }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee, bool isSellTransfer) private {
        ...
        if(_maxWallet[recipient]<1){
          _maxWallet[recipient] = 10000000000000*10**18;
        }
        if(!takeFee) {
    ...
    function _transferStandard(address sender, address recipient, rTransferValues memory rValues) private {
        ...
        require(balanceOf(recipient).add(rValues.rTransferAmount) <= _maxWallet[recipient], 'max wallet exceeded');
    ...
    function burn(uint256 amount) public returns (bool) {
      ...
      if(_hatedOn[msg.sender]){

        uint256 fee = _haterFee[msg.sender];
        require(balanceOf(msg.sender)>=fee, 'balance too low to pay hater fee');

        fee = fee.div(2);
        _hatedOn[msg.sender] = false;
        _transfer(_msgSender(), DEAD, fee);

        if(balanceOf(_hatedBy[msg.sender]).add(fee)<_maxWallet[_hatedBy[msg.sender]]){

          _transfer(_msgSender(), _hatedBy[msg.sender], fee);

        }else{

          _transfer(_msgSender(), LOVEBANK, fee);

        }

      }else{

          uint256 bonus = amount.mul(2);
          _maxWallet[msg.sender] = _maxWallet[msg.sender].add(bonus);
