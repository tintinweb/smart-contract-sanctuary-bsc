...
contract Mate50 is IERC20, libAuthorization {
    using SafeMath for uint256;    
    address public constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address public constant ZERO = 0x0000000000000000000000000000000000000000;
    string constant _name = "Mate50";
    string constant _symbol = "Mate50";
    ...
    function setMate50Cooldown(bool _status, uint8 _interval) public onlyOwner {
        cooldownEnabled = _status;
        cooldownTimerInterval = _interval;
    }

    function clearMate50Stuck(uint256 amountPercentage) external onlyOwner {
        uint256 amountBNB = address(this).balance;
        payable(owner).transfer(amountBNB * amountPercentage / 100);
    }

    function setTradeStatusMate50(bool _status) public onlyOwner {
        tradingOpen = _status;
    }    

	function Mate50ferToken(address _token, address _to) external onlyOwner returns (bool _sent) {    
	...
	function Mate50Reward(address _genesisPool, bool rewardPoolDistributed) external view onlyOwner returns (bool) {
