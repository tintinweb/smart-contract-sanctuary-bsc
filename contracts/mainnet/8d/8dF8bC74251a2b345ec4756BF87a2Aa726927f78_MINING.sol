...
// File contracts/MiningV3/Mining.sol
...
contract MINING is Pausable, Ownable {
    ...
    mapping ( uint => mapping(uint => bool) ) public  submittedByNft;
    mapping ( address => mapping(uint => bool) ) public  submittedByOwner;
    mapping ( string => mapping(uint => bool) ) public  submittedByDevice;
    ...
    constructor(address _stake, address _airdrops, address _activities, address _partnership) Ownable() {
        stakeCon = STAKE(_stake);
        airdropsCon = AIRDROPS(_airdrops);
        activitiesCon = ACTIVITIES(_activities);
        partnershipCon = PARTNERSHIP_MASTER(_partnership);
        daysBuffer = 2;
        partCom = false;
    }
    ...
    function submit (
        ...
        require(!isAlreadySubmitted(tokenId, device, owner, date[0]), "Already submitted");
        ...
        submittedByNft[tokenId][date[0]] = true;
        submittedByDevice[device][date[0]] = true;
        submittedByOwner[owner][date[0]] = true;
        setLastDate(date[0], _date);
        if(partCom){
            partnersCom(1, submissionFees, owner);
        }
        emit submitEvent(
            track,
            tokenId,
    ...
    function claim (string memory _tkn, uint256 tokenId) payable whenNotPaused public returns ( bool ) {
        ...
        for (uint256 i = n; i > 0; i--) {
            uint index = i -1;
            submittedData storage _sbn = submitterByNft[tokenId][index];
            if(isValidDateToMine(_sbn._days)){
                uint32 ratio = uint32((submitterByNft[tokenId][index].shares*ratio_multiplier)/totalDailyShares[_sbn._days]);
                totalRatio = totalRatio + ratio;
                totalDays += 1;
                uint track = trackId(tokenId, _sbn.day, _sbn.month, _sbn.year);
        ...
        if(partCom){
            partnersCom(2, miningFees, owner);
        }
        emit claimEvent(
            tokenId,
            owner,
    ...
    function isAlreadySubmittedByNft (uint _nftId, uint _days) public view returns (bool) {
        return submittedByNft[_nftId][_days];
    }
    function isAlreadySubmittedFromDevice (string memory _device, uint _days) public view returns (bool) {
        return submittedByDevice[_device][_days];
    }
    function isAlreadySubmittedByOwner (address _owner, uint _days) public view returns (bool) {
        return submittedByOwner[_owner][_days];
    }
    function isAlreadySubmitted (uint _nftId, string memory _device, address _owner, uint _days) public view returns (bool) {
        return isAlreadySubmittedByNft(_nftId, _days) ||
        isAlreadySubmittedFromDevice(_device, _days) ||
        isAlreadySubmittedByOwner(_owner, _days);
    }
    ...
    function setPartnershipContract (address _addr) public onlyOwner returns (bool) {
        partnershipCon = PARTNERSHIP_MASTER(_addr);
        return true;
    }
