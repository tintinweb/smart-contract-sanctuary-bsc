...
contract SimpleAirdropper {
    ...
    uint256 private constant RECIPIENT_LIMIT = 600;

    event Airdropped(uint256 total, IERC20 token, address sender);


    event CurrenyAirdropped(uint256 total, address sender);
    ...
    )
    {
        ...
        if (len > RECIPIENT_LIMIT) return (false, "Maximum 600 recipients", 0);
    ...
    function tranferToContract() external payable {
    ...
    function airdropCurreny(
        ...
        for (uint i = 0; i < len; i++) {
            ...
            address(uint160(_recipients[i])).transfer(amount);
            total = total.add(amount);
        }

        emit CurrenyAirdropped(total, msg.sender);
    }
     ...
     * @return totalAmount - total units of dropped token
     */
    function checkCurrencyAirdropValidity(
        address[] calldata _recipients,
        uint256[] calldata _amounts
    )
    external
    ...
    function emergencyWithdrawCurreny(address payable _to) public onlyOwner {
