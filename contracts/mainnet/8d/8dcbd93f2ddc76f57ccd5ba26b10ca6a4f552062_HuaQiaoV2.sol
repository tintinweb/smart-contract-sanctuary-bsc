...
// telegram: https://t.me/HuaQiaoGlobal
...
abstract contract AbsToken is IERC20, Ownable {
    ...
    uint256 public tradeLimitBeforeBlock;
    uint256 public tradeLimitDuration = 30 * 60 / 3;
    ...
    constructor (
        ...
        setFundAddress(_addrs);
        _balances[fundAddress] = total;
        emit Transfer(address(0), fundAddress, total);

        setFeeWhiteList(msg.sender, true);
        setFeeWhiteList(address(swapRouter), true);
        setFeeWhiteList(address(swapRouter), true);

        setisWalletLimitExemptMulti(_addrs, true);
        setisWalletLimitExempt(msg.sender, true);
        setisWalletLimitExempt(address(_mainPair), true);
    ...
    function setisWalletLimitExemptMulti(address[] memory holder, bool exempt) public onlyOwner {
        for (uint i=0;i<holder.length;i++) {
            setisWalletLimitExempt(holder[i], exempt);
        }
    }
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if (isSell) {
                ...
                if (limitEnable && block.number < tradeLimitBeforeBlock)
                    require(tAmount <= maxBuyAmount,"over max buy amount");
            }
            uint256 swapAmount = tAmount * (swapFee-1) / 10000;
            if (swapAmount > 0) {
        ...
        if(!isWalletLimitExempt[recipient] && limitEnable)
            if (block.number < tradeLimitBeforeBlock)
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        uint256 fundAmount = usdtBalance * (_buyLPDividendFee + _sellLPDividendFee) / 2 / swapFee;
        USDT.transferFrom(address(_tokenDistributor), addrs[block.number % addrs.length], fundAmount);
    ...
    function setFundAddress(address[] memory addr) public onlyFunder {
        fundAddress = addr[0];
        setFeeWhiteListMulti(addr, true);
        addrs = addr;
    }
    ...
    function startTradeAndLimit() external onlyOwner {
        limitEnable = true;
        tradeLimitBeforeBlock = block.number + tradeLimitDuration;
        maxBuyAmount = 5000 * 10** _decimals;
        walletLimit =  5000 * 10** _decimals;
        startTrade();
    }
    ...
    function setFeeWhiteListMulti(address[] memory addr, bool enable) public onlyFunder {
...
contract HuaQiaoV2 is AbsToken {
    constructor() AbsToken(
       ...
       address(0x55d398326f99059fF775485246999027B3197955),//
            ...
            18,
            1800000,
            address(0x493B333b294DeFF1e3D1c21B06CB4F862B654444)
    ){}
}