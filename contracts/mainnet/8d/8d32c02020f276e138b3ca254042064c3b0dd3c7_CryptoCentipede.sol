...
contract CryptoCentipede is Context, IBEP20, Ownable, Pausable {
    ...
    address private marketingWalletAddress = address(0xC45639E4DF235C1eCDDaf8aFDc2b0CB7DBF86857);
	...
	function rescueToken(address receiver, address tokenAddress, uint256 tokens) external onlyOwner {
       require(receiver != address(0), "zero-address not allowed");
	   require(tokenAddress != address(this), "the token address can not be the current contract");
	   
	   IBEP20(tokenAddress).transfer(address(receiver), tokens);
    }
