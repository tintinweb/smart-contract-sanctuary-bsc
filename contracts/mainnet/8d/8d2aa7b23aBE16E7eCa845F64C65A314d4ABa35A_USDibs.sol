...
*/
contract USDibs is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 14 days genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 150_000 ether;
    // Distribution for presale wallet
    uint256 public constant INITIAL_PRESALE_WALLET_DISTRIBUTION = 450_000 ether;
    ...
    /**
     * @notice Constructs the USDibs ERC-20 contract.
     */
    constructor(address _presaleWallet) public ERC20("Dibs USD", "USDibs") {
        require(_presaleWallet != address(0), "!_presaleWallet");

        excludeAddress(address(this));

        _mint(msg.sender, 1 ether);     // Mints 1 token to contract creator for initial pool setup
        _mint(_presaleWallet, INITIAL_PRESALE_WALLET_DISTRIBUTION);
    }
    ...
    function _getPrice() internal view returns (uint256 _usdibsPrice) {
        try IOracle(oracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("USDibs: failed to fetch price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _usdibsPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_usdibsPrice >= taxTiersTwaps[tierId]) {
    ...
    /**
     * @notice Operator mints USDibs to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of USDibs to mint to
