...
// twitter.com/FOTDBSC
//t.me/tyrantfotdbsc
//OwnerShip will be sent to Vitalik Buterin Address
...
contract TYRANT is IERC20, Context, Ownable {
    ...
    string private constant _symbol = "Fable Of The Dragon";
    string private constant _name = "TYRANT";

    uint256 public FABLE_max_BuyLimit = 10000000 * 10**9;
    uint256 public FABLE_max_SellLimit = 10000000 * 10**9;

    address public FABLE_iniINU_Wallet = msg.sender;
    address public FABLE_Team_Wallet = msg.sender;

    uint256 public FABLE_max_WalletLimit = 20000000 * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[FABLE_iniINU_Wallet] = true;
        _isExcludedFromFee[FABLE_Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[FABLE_iniINU_Wallet] = true;
        allowedTransfer[FABLE_Team_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= FABLE_max_BuyLimit, "You are exceeding FABLE_max_BuyLimit");
            require(balanceOf(to) + amount <= FABLE_max_WalletLimit, "You are exceeding FABLE_max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= FABLE_max_SellLimit, "You are exceeding FABLE_max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= FABLE_max_WalletLimit, "You are exceeding FABLE_max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(FABLE_iniINU_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(FABLE_Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function FABLE_Update_MarketingWallet(address newWallet) external onlyOwner{
        FABLE_iniINU_Wallet = newWallet;
    }
    
    function FABLE_Update_TeamWallet(address newWallet) external onlyOwner{
        FABLE_Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        FABLE_max_BuyLimit = maxBuy * 10**decimals();
        FABLE_max_SellLimit = maxSell * 10**decimals();
    }
    
    function FABLE_Max_Walletlimit(uint256 amount) external onlyOwner{
        FABLE_max_WalletLimit = amount * 10**decimals();
    }
