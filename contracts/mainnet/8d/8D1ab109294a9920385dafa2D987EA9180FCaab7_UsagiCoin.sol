...
/**
 * Allows for contract ownership for multiple adressess or non at all
 */
...
/* Contract */
contract UsagiCoin is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    // Adresses
    address payable public marketingWallet = payable(0xa09351E18C0e07b69f55846a08Dd7a82BE5Db097); 
    address payable public devWallet = payable(0x342FC134524d74B48D92c5eaBd5f9DB0dF99146c); 
    address payable public liquidityWallet = payable(0x342FC134524d74B48D92c5eaBd5f9DB0dF99146c); 
    ...
    // Name and symbol
    string private _name = "Usagi Coin";
    string private _symbol = "USAGI";
    uint8 private constant _decimals = 18;

    // Total Supply
    uint256 private constant _tTotal = 100000000 * (10 ** _decimals); // 30
    uint256 private constant MAX = ~uint256(0);
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    // Max wallet and TX
    uint256 public maxTransactionAmount = _tTotal * 20 / 10000; // 0.2% 
    uint256 public maxWalletAmount = _tTotal * 100 / 10000; // 1% 
    ...
    uint256 private quickScope = 2;
    uint256 private matchDuration = 72 hours;
    ...
    event UpdatedMarketingWallet(address marketing);
    event UpdatedLiquidityWallet(address liquidity);
    event UpdatedDevWallet(address buyback);
    ...
    function opentheGates() external onlyOwner {
        ...
        earlyBuyPenaltyEnd = block.timestamp + matchDuration;
    }
    ...
    function addPresaleAddressForExclusions(address _presaleAddress) external onlyOwner {
        ...
        _isExcludedMaxWalletAmount[_presaleAddress] = true;
    }
    ...
    function swapBack() private lockTheSwap {
        ...
        (bool success,) = address(devWallet).call{value: bnbForTeam}("");
        ...
        onlyOwner
    {
        ...
        require(_buyTaxFee + _buyLiquidityFee + _buyMarketingFee + _buyDevFee <= 90, "Must keep taxes below 90%");
        ...
        onlyOwner
    {
        ...
        require(_sellTaxFee + _sellLiquidityFee + _sellMarketingFee + _sellDevFee <= 90, "Must keep taxes below 90%");
    ...
    function setLiquidityWallet(address _liquidityWallet) external onlyOwner {
        require(_liquidityWallet != address(0), "_liquidityWallet address cannot be 0");
        liquidityWallet = payable(_liquidityWallet);
        _isExcludedFromFee[liquidityWallet] = true;
        emit UpdatedLiquidityWallet(_liquidityWallet);
    }
