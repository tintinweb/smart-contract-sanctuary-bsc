...
contract KylonToken is ERC20, ERC20Burnable, Ownable, ERC20Permit {
    ...
    uint256 public marketingShare = 7500;
    uint256 public charityShare = 2500;
    ...
    constructor(string memory _name, string memory _ticker, uint256 _initalSupply) ERC20(_name, _ticker) ERC20Permit(_name) {
        _mint(address(msg.sender), _initalSupply * 10**18);
    }
    ...
    function _swapBack() private {
        ...
        if(liquidityShare > 0) {
           liquidityTokens = netAmount.mul(liquidityShare).div(DENOMINATOR).div(2);
        }
    ...
    function setTaxes(uint256 newBuyTax, uint256 newSellTax, uint256 newTransferTax) external onlyOwner {
        require(newBuyTax <= 20000 && newSellTax <= 20000 && newTransferTax <= 20000);
        buyTax = newBuyTax;
        sellTax = newSellTax;
    ...
    function setShares(uint256 newLiquidityShare, uint256 newMarketingShare, uint256 newCharityShare, uint256 newBurnShare) external onlyOwner {
        uint256 total = newLiquidityShare.add(newMarketingShare).add(newCharityShare).add(newBurnShare);
        require(total == DENOMINATOR, "Exceeded limit");
