...
contract ShortsTrackerTimelock {
    ...
    event SignalSetExternalHandler(
        address target,
        address handler,
        bool active
    );
    event SetExternalHandler(address target, address handler, bool active);
    ...
    function signalSetExternalHandler(
        address _shortsTracker,
        address _handler,
        bool _isActive
    ) external onlyAdmin {
        require(
            _handler != address(0),
            "ShortsTrackerTimelock: invalid hander"
        );

        bytes32 action = keccak256(
            abi.encodePacked("setHandler", _shortsTracker, _handler, _isActive)
        );
        _setPendingAction(action);

        emit SignalSetExternalHandler(_shortsTracker, _handler, _isActive);
    }

    function setExternalHandler(
        address _shortsTracker,
        address _handler,
        bool _isActive
    ) external onlyAdmin {
        ...
        IShortsTracker(_shortsTracker).setHandler(_handler, _isActive);

        emit SetExternalHandler(_shortsTracker, _handler, _isActive);
    }
