...
contract MyDiamondTeamV2 {
    ...
    mapping(address => uint8) user_migrate_state; // 0 =  no migration,    1 = migrated but inactive,  2 = migrated and active
    ...
    uint256 public MAX_POOL_BALANCE = 25 ether; //25 BNB
    ...
    constructor(address payable ownerAddress, address payable projectAddress, address payable marketingAddress) {
        ...
        v1Contract = MDTv1(address(0x24436520c319695E8c435Ea32Aa275f55E2a3F07));
    ...
    function _refPayout(address _addr, uint256 _amount) private {
        ...
        for(uint8 i = 0; i < ref_depth; i++) {
            ...
            if(users[up].referrals >= i.add(1)) {
                ...
                //@TODO: check if upline address is a migration user and if he is already here. if not, skip to the next level
    ...
    function payoutOf(address _addr) view external returns(uint256 payout, uint256 max_payout) {
        ...
        max_payout = ((_addr == project || _addr == owner)  
    ...
    function payoutToReinvest(address _addr) view external returns(uint256 payout) {
        ...
        uint256 max_payout = ((_addr == project || _addr == owner)  
                ? this.maxPayoutP(users[_addr].deposit_amount) 
                : this.maxPayoutOf(users[_addr].deposit_amount));
   ...
   function changeTeamName(uint256 teamId, string memory name) external {
    ...
    /** Migration **/
    function ImportUsers(uint256 noOfUsers) external {
        //require(msg.sender == owner, "Admin use only");
        for(uint256 i; i < noOfUsers; i++){
            address userAddress = v1Contract.id2Address(i);
            MDTv1.User memory v1usr = v1Contract.users(userAddress);

            //users[userAddress].upline = v1usr.upline;
            _setUpline(userAddress, v1usr.upline); // set the upline and creates the team structure
            user_migrate_state[userAddress] = 1; // migrated but still inactive
            id2Address[total_users] = userAddress;
            total_users++;
...
interface MDTv1 {
    ...
    function users(address value) external returns (User memory);
    function id2Address(uint256 value) external returns (address);
    function teams(uint256 value) external returns (User memory);  
}
