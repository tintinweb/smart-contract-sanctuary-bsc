...
contract Tencent is ERC20Detailed, Ownable, MinterRole {
    ...
    uint256 public updatedTokenPrice ;
    ...
    ) ERC20Detailed("Tencent", "TENCENT", uint8(DECIMALS)) {
    ...
    function coreRebase(uint256 epoch, int256 supplyDelta) private returns (uint256) {
        if(_lastTokenPrice != 0)
            {
                (uint112 reserve0, uint112 reserve1,) =  InterfaceLP(pair).getReserves();
