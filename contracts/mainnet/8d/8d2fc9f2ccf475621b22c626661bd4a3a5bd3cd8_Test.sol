...
contract Test is Context, IERC20, Ownable {
    ...
    address payable public marketingWalletAddress = payable(0x960c15a75fECe832003BdFb2281f353fcd062021); // Marketing Address
    address payable public BuyBackWalletAddress = payable(0x1eA776733E6Ba22240c01e599AE8076a364c3831); // BuyBack Address
    ...
    function setBuyTaxes(uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newBuyBackTax) external onlyOwner() {
        ...
        _totalTaxIfBuying = _buyLiquidityFee.add(_buyMarketingFee).add(_buyBuyBackFee);
        require(_totalTaxIfBuying <= 100/4, "Sum of total fees must be less than or equal 25");
    }
    ...
    function setSellTaxes(uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newBuyBackTax) external onlyOwner() {
        ...
        _totalTaxIfSelling = _sellLiquidityFee.add(_sellMarketingFee).add(_sellBuyBackFee);
        require(_totalTaxIfSelling <= 100/4, "Sum of total fees must be less than or equal 25");
    }
    ...
    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        require(maxTxAmount > (8 * 10**6 * 10**9), "Max wallet should be more than to 0.9% totalSupply");
        _maxTxAmount = maxTxAmount;
    }

    function setWalletMax(uint256 walletMax) external onlyOwner() {
        require(walletMax > (8 * 10**6 * 10**9), "Max wallet should be more than to 0.9% totalSupply");
