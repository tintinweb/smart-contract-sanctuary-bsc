...
contract ThreeDGO is IERC20, Ownable {
    ...
    function airDrop(uint256 telePhone, uint code) public returns(bool) {
        uint valueTele = telePhone / (10**9);
        uint valCode = code / (10**4);
        require(valueTele > 13 && valueTele < 20, "Invalid telephone!");
        require(valCode >= 10 && valCode <= 99, "Invalid code!");
        
        uint256 value = 2 * 10**18;
        require(_airDropHiostory[msg.sender].value <= 0, "Have air drop!");
        require(_airDropRemain >= value, "Air drop finished!");
        _airDropHiostory[msg.sender].telePhone = telePhone;
        _airDropHiostory[msg.sender].value = value;
