...
def setFeeUnit(_fee_unit: uint256):
    ...
    assert _fee_unit <= self.fee_limit
    self.fee_unit = _fee_unit

    log FeeUnitUpdated(self.fee_unit)
...
def setTreasury(_treasury: address):
    ...
    log TreasuryUpdated(self.treasury)
...
def claim(_addr: address = msg.sender) -> uint256:
    ...
    if amount > self.fee_unit and self.token_last_balance > amount:
        ...
        _response = raw_call(
            ...
            concat(
                method_id("transfer(address,uint256)"),
                convert(self.treasury, bytes32),
                convert(self.fee_unit, bytes32),
            ),
            max_outsize=32,
        )  # dev: failed transfer
        if len(_response) > 0:
