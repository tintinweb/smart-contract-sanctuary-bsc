...
contract MMACBridge is Ownable{
   ...
   uint256 public BSCtoAVAXBridgeRequestCount;
   
   mapping ( uint256 => bool ) public AVAXBridgeRequests;
   uint256 public AVAXtoBSCBridgeRequestCount;
   ...
   event BridgedAVAXtoBSC ( uint256 _request, address indexed _user, uint256 _amount, uint256 _blocknumber );
    ...
    function BridgeMMAC ( uint256 _mmac ) public  payable  {
       ...
       BSCtoAVAXBridgeRequestCount++;
       IERC20 _token = IERC20 ( MMACADDRESS ) ;
       _token.transferFrom ( msg.sender, address(this) , _mmac);
       Bridge.transfer(msg.value);
       deposits += _mmac;
       bridgeRequests[BSCtoAVAXBridgeRequestCount].user =  msg.sender;
       bridgeRequests[BSCtoAVAXBridgeRequestCount].balance = _mmac;
       bridgeRequests[BSCtoAVAXBridgeRequestCount].blockNumber = block.number;
       
       emit BridgedBSCtoAVAX ( BSCtoAVAXBridgeRequestCount, msg.sender, _mmac, block.number );
    ...
    function transferBridgedMMAC (  uint256 _request, address _address, uint256 _amount ) public onlyBridge {
        require (!AVAXBridgeRequests[_request], "Already Processed" );
        AVAXtoBSCBridgeRequestCount++;
        AVAXBridgeRequests[ AVAXtoBSCBridgeRequestCount ] = true;
        IERC20 _token = IERC20 ( MMACADDRESS ) ;
        _token.transfer ( _address , _amount );
        deposits -= _amount;
        
        emit BridgedAVAXtoBSC ( AVAXtoBSCBridgeRequestCount, _address, _amount, block.number );
