// SPDX-License-Identifier: MIT
    ...
    contract OdinMarketplace is ReentrancyGuard, Ownable {
        ...
        constructor(
            ...
            payoutAddressMap["ODIN"] = bcPaymentAddress_;
            tokenAddressMap["ODIN"] = bcTokenAddress_;
        ...
        function openTrade(
            ...
            ODINNFT(nftContract).transferFrom(
                msg.sender,
                address(this),
                _id
            );
        ...
        function closeTrade(uint256 _id) external {
            ...
            ODINNFT(nftContract).transferFrom(address(this), msg.sender, _id);
            listedMap[_id] = false;
        ...
        function giveaway(
            ...
            if (listedMap[_id] == false) {
                ODINNFT(nftContract).transferFrom(msg.sender, _to, _id);
            } else {
                require(ownerMap[_id] == msg.sender, "sender is not owner");
                ODINNFT(nftContract).transferFrom(address(this), _to, _id);
                listedMap[_id] == false;
            }
            ownerMap[_id] = _to;
        ...
        function mint(uint256 number, uint256[] memory _level) public payable{
            ...
            for (uint256 i = 0; i < number; i++) {
                ...
                ODINNFT(nftContract).mint(_level[i], msg.sender);
                _tokenIds.increment();
        ...
        function buy(uint256 _id, uint256 _price, string memory paymentType) external payable {
            ...
            require(
                msg.sender != ODINNFT(nftContract).ownerOf(_id),
        ...
        {
            require(
                msg.sender == ODINNFT(nftContract).ownerOf(_tokenId),
                "Error, you are not the owner"
            );
            ...
            onlyCreatorOrWinner(tokenId)
        {
            ...
            if (winner == address(0)) {
                ODINNFT(nftContract).transferFrom(address(this), Creator, tokenId);
                ownerMap[tokenId] = Creator;
            } else {
                ODINNFT(nftContract).transferFrom(address(this), winner, tokenId);
                if (
                    keccak256(abi.encodePacked((paymentType))) ==
                    keccak256(abi.encodePacked(("BNB")))
                ) {
