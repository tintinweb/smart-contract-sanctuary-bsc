...
contract Dusdt is ERC20 {
	...
	address payable _fundAddress = payable(address(0xF9740D2EdF7b388b4c8D7532cf2f1E5963C4212c));
	address private _destroyAddress = address(0x000000000000000000000000000000000000dEaD);
    address _defout = address(0xF9740D2EdF7b388b4c8D7532cf2f1E5963C4212c);
    ...
    constructor(address tokenOwner,address root) ERC20("Dusdt", "Dusdt") {
    ...
    function _transfer(
        ...
        if(address(this) == from || address(this) == to){
            super._transferr(from, to, amount);
            return;
        }
		
		if(balanceOf(address(this)) > 10**10){
            if (
                !swapping &&
                _tokenOwner != from &&
                _tokenOwner != to &&
    ...
    function _splitOtherToken(uint256 thisAmount) public {
        if(thisAmount >= 0){
            thisAmount = thisAmount.mul(3).div(5);
    ...
    function _takeInviterFee(
        ...
        for (int256 i = 0; i < 2; i++) {
            ...
            super._transfer(sender, receiveD, amount.div(100).mul(rate));
        }
