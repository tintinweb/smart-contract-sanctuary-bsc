...
contract magicpump is IERC20, Auth {
    
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    string constant _name = "magicpump";
    string constant _symbol = "$MCP";
    ...
    uint256 private rewardsFee = 1;
    uint256 private rewardsFeeSell = 1;

    uint256 public BNBRewardFee = 6;
    uint256 public BNBRewardFeeSell = 10;

    uint256 public totalFee = 13;
    uint256 public totalFeeSell = 22;

    address public autoLiquidityReceiver = address(this);
    address private marketingWallet = 0xCbA2aA3F6D446032A8ECcdf48da846C249882618;
    address private BNBRewardWallet = 0xCbA2aA3F6D446032A8ECcdf48da846C249882618;
    ...
    constructor () Auth(msg.sender) {
        ...
        isFeeExempt[BNBRewardWallet] = true;
        ...
        isDividendExempt[BNBRewardWallet] = true;
    ...
    function changeFees(uint256 newLiqFee, uint256 newRewardFee, uint256 newMarketingFee, uint256 newBNBRewardFee) external authorized {

        require(newLiqFee <= 10, "LP Tax on buys can not be higher than 10%");
        require(newMarketingFee <= 80, "Marketing Tax on buys can not be higher than 10%");
        require(newBNBRewardFee >= 0, "Team Tax on buys can not be lower than 0%");
        require(newRewardFee >= 2, "Rewards Tax on buys can not be lower than 2%");

        liquidityFee = newLiqFee;
        rewardsFee = newRewardFee;
        marketingFee = newMarketingFee;
        BNBRewardFee = newBNBRewardFee;

        totalFee = liquidityFee.add(marketingFee).add(BNBRewardFee).add(rewardsFee);
    }

    function changeFeesSell(uint256 newLiqFeeSell, uint256 newRewardFeeSell, uint256 newMarketingFeeSell, uint256 newBNBRewardFeeSell) external authorized {
        require(newLiqFeeSell <= 10, "LP Tax on sells can not be higher than 20%");
        require(newMarketingFeeSell <= 70, "Marketing Tax on sells can not be higher than 30%");
        require(newBNBRewardFeeSell >= 0, "Team Tax on sells can not be lower than 0%");
        ...
        BNBRewardFeeSell = newBNBRewardFeeSell;

        totalFeeSell = liquidityFeeSell.add(marketingFeeSell).add(BNBRewardFeeSell).add(rewardsFeeSell);
    }

    function changeFeeReceivers(address newLiquidityReceiver, address newMarketingWallet, address newBNBRewardWallet) external authorized {
        autoLiquidityReceiver = newLiquidityReceiver;
        marketingWallet = newMarketingWallet;
        BNBRewardWallet = newBNBRewardWallet;
    }
    ...
    function swapBack() internal lockTheSwap {
        ...
        uint256 teamShare = amountCRO.mul(BNBRewardFee).div(totalCROFee);
        ...
        (bool tmpSuccess1,) = payable(BNBRewardWallet).call{value: teamShare, gas: 30000}("");
