    ...
    IDividendPayingTokenOptional
{
    ...
    address public adminAddress = 0x168873531F8Ae8E7267A0DD159c01265789dC365; //contract wallet
...
contract Eliza is ERC20, Ownable {
    ...
    constructor() ERC20("Eliza Inu", "ELIZA") {
        a80DividendTracker = new A80DividendTracker();
        b20DividendTracker = new B20DividendTracker();

        contractWallet = 0x168873531F8Ae8E7267A0DD159c01265789dC365;
        marketingWallet = 0xb480C371F132b76aDeb208F0399801e86348648c;
        a80DividendToken = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56; //BUSD
        b20DividendToken = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c; //WBNB
        ...
        */
        _mint(deadAddress, 9 * (10**14) * (10**18));
        _mint(contractWallet, 1 * (10**14) * (10**18));
    }
    ...
    function updateMinA80BeforeSendDividend(uint256 _newAmount)
        external
        onlyOwner
    {
    ...
    function updateMinB20BeforeSendDividend(uint256 _newAmount)
        external
        onlyOwner
    {
    ...
    function afterPreSale() external onlyOwner {
        a80DividendRewardsBuyFee = 5;
        b20DividendRewardsBuyFee = 1;
        marketingBuyFee = 1;
        buyBackAndLiquidityBuyFee = 1;

        a80DividendRewardsSellFee = 7;
        ...
        maxSellTransactionAmount = 5000000000000 * (10**18);
    ...
    function _updateTotalFee() internal {
        totalBuyFees = buyBackAndLiquidityBuyFee
            .add(marketingBuyFee)
            .add(a80DividendRewardsBuyFee)
            .add(b20DividendRewardsBuyFee);
        totalSellFees = buyBackAndLiquiditySellFee
            .add(marketingSellFee)
            .add(a80DividendRewardsSellFee)
            .add(b20DividendRewardsSellFee);
    }
    ...
    function updateA80DividendTracker(address newAddress) external onlyOwner {
        A80DividendTracker newA80DividendTracker = A80DividendTracker(
            payable(newAddress)
        );

        require(
            newA80DividendTracker.owner() == address(this),
            "must be owned by Eliza Inu"
        );

        newA80DividendTracker.excludeFromDividends(
            address(newA80DividendTracker)
        );
    ...
    function updateB20DividendTracker(address newAddress) external onlyOwner {
        B20DividendTracker newB20DividendTracker = B20DividendTracker(
            payable(newAddress)
        );

        require(
            newB20DividendTracker.owner() == address(this),
            "must be owned by Eliza Inu"
        );

        newB20DividendTracker.excludeFromDividends(
            address(newB20DividendTracker)
        );
    ...
    function updateA80DividendRewardSellFee(uint8 newSellFee)
        external
        onlyOwner
    {
    ...
    function updateB20DividendRewardSellFee(uint8 newSellFee)
        external
        onlyOwner
    {
        require(newSellFee <= MAX_FEE_RATE, "wrong");
    ...
    function updateBuyBackAndLiquidityBuyFee(uint8 newBuyFee)
        external
        onlyOwner
    {
        require(newBuyFee <= MAX_FEE_RATE, "wrong");
    ...
    function updateBuyBackAndLiquiditySellFee(uint8 newSellFee)
        external
        onlyOwner
    {
        require(newSellFee <= MAX_FEE_RATE, "wrong");
    ...
    function updateMinimumBalanceForDividends(uint256 newMinimumBalance)
        ...
        onlyOwner
    {
        a80DividendTracker.updateMinimumTokenBalanceForDividends(
            newMinimumBalance
        );
        b20DividendTracker.updateMinimumTokenBalanceForDividends(
            newMinimumBalance
        );
    }
    ...
    function withdrawableA80DividendOf(address account)
        external
        view
        returns (uint256)
    {
    ...
    function withdrawableB20DividendOf(address account)
        external
        view
        returns (uint256)
    {
    ...
    function a80DividendTokenBalanceOf(address account)
        external
        view
        returns (uint256)
    {
    ...
    function b20DividendTokenBalanceOf(address account)
        external
        view
        returns (uint256)
    {
    ...
    function processDividendTracker(uint256 gas) external onlyOwner {
        (
            uint256 aIterations,
            uint256 aClaims,
            uint256 aLastProcessedIndex
        ) = a80DividendTracker.process(gas);
        emit ProcessedA80DividendTracker(
            aIterations,
            aClaims,
            aLastProcessedIndex,
            false,
        ...
        (
            uint256 bIterations,
            uint256 bClaims,
            uint256 bLastProcessedIndex
        ) = b20DividendTracker.process(gas);
        emit ProcessedB20DividendTracker(
            bIterations,
            bClaims,
            bLastProcessedIndex,
            false,
    ...
    function getLastA80DividendProcessedIndex()
        external
        view
        returns (uint256)
    {
    ...
    function getLastB20DividendProcessedIndex()
        external
        view
        returns (uint256)
    {
    ...
    function getNumberOfA80DividendTokenHolders()
        external
        view
        returns (uint256)
    {
    ...
    function getNumberOfB20DividendTokenHolders()
        external
        view
        returns (uint256)
    {
    ...
    function _transfer(
        ...
        require(!isBlacklisted(from), "Eliza Inu: sender blacklisted");
        require(!isBlacklisted(to), "Eliza Inu: recipient blacklisted");
        require(!isBlacklisted(tx.origin), "Eliza Inu: sender blacklisted");
            ...
            if (!swapping && contractTokenBalance >= swapTokensAtAmount) {
                ...
                if (marketingEnabled) {
                    uint256 swapTokens = contractTokenBalance
                        .mul(marketingSellFee)
                        .div(totalSellFees);

                    uint256 beforeAmount = address(this).balance;
                    swapTokensForBNB(swapTokens);
                    uint256 increaseAmount = address(this).balance.sub(
                        beforeAmount
                    );

                    if (increaseAmount > 0) {
                        uint256 teamPortion = increaseAmount.mul(66).div(10**2);
                        uint256 marketingPortion = increaseAmount.sub(
                            teamPortion
                        );
                        transferToWallet(
                    ...
                    if (buyBackMode) {
                        swapTokensForBNB(
                            contractTokenBalance
                                .mul(buyBackAndLiquiditySellFee)
                                .div(totalSellFees)
                        );
                    } else {
                        swapAndLiquify(
        ...
        try
            a80DividendTracker.setBalance(payable(from), balanceOf(from))
        {} catch {}
        try
            b20DividendTracker.setBalance(payable(from), balanceOf(from))
        {} catch {}
        try
            a80DividendTracker.setBalance(payable(to), balanceOf(to))
        {} catch {}
        try
            b20DividendTracker.setBalance(payable(to), balanceOf(to))
        {} catch {}
        ...
        if (!swapping && to != deadAddress) {
            ...
            if (rand() <= a80DividendPriority) {
                if (a80DividendEnabled && sendA80InTx) {
                    try a80DividendTracker.process(gas) returns (
                        uint256 iterations,
                        uint256 claims,
                        uint256 lastProcessedIndex
                    ) {
                ...
                if (b20DividendEnabled && sendB20InTx) {
                    try b20DividendTracker.process(gas) returns (
                        uint256 iterations,
                        uint256 claims,
                        uint256 lastProcessedIndex
                    ) {
    ...
    function enableBlacklist(address account) public onlyOwner {
        require(
            !blacklist[account],
            "Eliza Inu: Account is already blacklisted"
        );
    ...
    function disableBlacklist(address account) public onlyOwner {
        require(blacklist[account], "Eliza Inu: Account is not blacklisted");
    ...
    function swapAndSendA80Dividends(uint256 tokens) private {
        uint256 beforeAmount = IERC20(a80DividendToken).balanceOf(
            address(a80DividendTracker)
        );
        ...
        uint256 a80Dividends = IERC20(a80DividendToken)
            .balanceOf(address(a80DividendTracker))
            .sub(beforeAmount);
    ...
    function swapAndSendB20Dividends(uint256 tokens) private {
        uint256 beforeAmount = IERC20(b20DividendToken).balanceOf(
            address(b20DividendTracker)
        );
        ...
        uint256 b20Dividends = IERC20(b20DividendToken)
            .balanceOf(address(b20DividendTracker))
            .sub(beforeAmount);
