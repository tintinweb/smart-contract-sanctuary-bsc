...
contract PetraRubarNFTToken is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    PetraRubarNFTTokenDividendTracker public dividendTracker;
    ...
    address payable public _marketingWalletAddress = payable(0xDf2ED456147faC54dDfF78010617E4aec925f463);
    ...
    constructor() BEP20("Petra Rubar NFT Token", "RUBAR") 
    {
        dividendTracker = new PetraRubarNFTTokenDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "PetraRubarNFTToken: The dividend tracker already has that address");
        PetraRubarNFTTokenDividendTracker newDividendTracker = PetraRubarNFTTokenDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "PetraRubarNFTToken: The new dividend tracker must be owned by the PetraRubarNFTToken token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "PetraRubarNFTToken: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "PetraRubarNFTToken: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "PetraRubarNFTToken: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "PetraRubarNFTToken: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "PetraRubarNFTToken: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "PetraRubarNFTToken: Cannot update gasForProcessing to same value");
...
contract PetraRubarNFTTokenDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("PetraRubarNFTToken_Dividend_Tracker", "PetraRubarNFTToken_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "PetraRubarNFTToken_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "PetraRubarNFTToken_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PetraRubarNFTToken contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "PetraRubarNFTToken_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "PetraRubarNFTToken_Dividend_Tracker: Cannot update claimWait to same value");
