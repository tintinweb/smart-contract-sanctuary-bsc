...
contract Nanottery {
    ...
    function startLottery() external onlyOwner {
        ...
        uint nonce = 0;
        uint randomNumber = random(nonce);
    ...
    function random(uint nonce) private view returns (uint) {
        return uint(keccak256(abi.encodePacked(msg.sender, block.number, block.timestamp, nonce))) % playerList.length;
    }
