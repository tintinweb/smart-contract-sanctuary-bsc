...
contract PlanetRouter is ISolidlyRouter{
    ...
    function swapSolidlyToGamma(
        ...
        TransferHelper.safeTransferFrom(
            IERC20(route[0].from), msg.sender, address(this), amountIn
        );
        ...
        ISolidlyRouter2(solidlyRouterAddress).swapExactTokensForTokens(amountIn, amounts[len], route, address(this), block.timestamp + 300);
        ...
        len = amounts.length - 1;

        require(amounts[len] >= amountOutMin, 'PlanetRouter: INSUFFICIENT_OUTPUT_AMOUNT');
