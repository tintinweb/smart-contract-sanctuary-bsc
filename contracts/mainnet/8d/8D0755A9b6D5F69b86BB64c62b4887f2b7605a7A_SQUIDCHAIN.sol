...
contract SQUIDCHAIN is ERC20, Ownable {
    ...
    address public WBNB;
    IERC20 public constant RWRD = IERC20(0xAf98e6c2D3b520F4e9b3D7ea22ad7AeFFFDbc2fd);
    ...
    uint256 public jackpotMinBuy = 0.05 ether;
    uint256 public jackpotBlockLimit = 200;
    ...
    uint256 public jackpotShare = 300;
    ...
    uint256 public maxTxAmount = (10_000_000 * 10 ** 18 * 5) / 1000;
    uint256 public maxHoldAmount = (10_000_000 * 10 ** 18 * 2) / 100;
    ...
    constructor() ERC20("SQUIDCHAIN", "SQUIDCHAIN") {
        WBNB = ROUTER.WETH();
        pair = IDexFactory(ROUTER.factory()).createPair(
            address(this),
            WBNB
        );
        marketingWallet = 0x5CA7B305e57e778b658Dd0682B5aeb351a76e042;
        devWallet = 0x1adEC7dce44923B318d90ED65212dB0dEb3b548B;
    ...
    function _transfer(
        ...
        if (!isWhitelisted[sender] && !isWhitelisted[recipient]) {
            require(tradingEnabled,"Wait for launch");
            require(amount <= maxTxAmount, "Max Txn limit exceeds");
            if(block.timestamp < launchTime + 12 seconds){
        ...
        if (!isMaxWalletExempt[recipient]) {
            require(balanceOf(recipient)+(amount) <= maxHoldAmount,"Max Wallet limit exceeds");
        }

        // Jackpot
        if (
            lastBuyBlock != 0 &&
            block.number - lastBuyBlock >= jackpotBlockLimit
        ) {
            _handleJackpot();
        }
        ...
        if (_shouldSwapBack(recipient)) {
            uint256 swapAmount = swapWholeStorage
        ...
        // Tax
        uint256 amountReceived = isTaxExempt[sender] || isTaxExempt[recipient] 
            ? amount
            : _takeTax(sender, recipient, amount);
    ...
    function _takeTax(
        ...
        uint256 taxAmount = (amount * _getTotalTax(sender, recipient)) /
            TAX_DENOMINATOR;
        if (taxAmount > 0) {
    ...
    function _getTotalTax(address sender, address recipient)
        private
        view
        returns (uint256)
    {
    ...
    function _swapBack(uint256 tokenAmount) private swapping {
        ...
        uint256 liquidityTokens = (tokenAmount * liquidityShare) /
            totalShares /
            2;
        ...
        uint256 amountBNBLiquidity = (amountBNB * liquidityShare) /
            totalBNBShares /
            2;
        uint256 amountBNBMarketing = (amountBNB * marketingShare) /
            totalBNBShares;
        uint256 amountBNBDev = (amountBNB * devShare) / totalBNBShares;
        uint256 amountBNBJackpot = (amountBNB * jackpotShare) /
            totalBNBShares;

        payable(marketingWallet).transfer(amountBNBMarketing);
        payable(devWallet).transfer(amountBNBDev);
        deposit(amountBNBJackpot);
    ...
    function _handleJackpot() private {
        uint256 jackpot = (RWRD.balanceOf(address(this)) * (jackpotPercent)) / (100);
    ...
    function recoverBnb(address payable recipient) external onlyOwner {
    ...
    function setIsMaxWalletExempt(address account, bool exempt) external onlyOwner {
    ...
    function setTaxes(
        ...
        require(
            newBuyTax <= 2500 && newSellTax <= 2500 && newTransferTax <= 2500,
            "Too high taxes"
        );
        buyTax = newBuyTax;
        sellTax = newSellTax;
    ...
    function setTaxShares(
        ...
        totalShares = liquidityShare + marketingShare + devShare + jackpotShare;
        require(totalShares > 0, "totalShares must be positive number");
    }

    function setMaxLimits(uint256 maxTxn, uint256 maxWallet) external onlyOwner {
        maxTxAmount = maxTxn;
        maxHoldAmount = maxWallet;
    }
    ...
    function setJackpotSettings(
        ...
        jackpotPercent = percent;
    }

    function setFeeRecieverWallets(address marketing, address dev)
        external
        onlyOwner
    {
