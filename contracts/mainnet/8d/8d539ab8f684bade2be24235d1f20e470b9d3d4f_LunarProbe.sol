...
contract LunarProbe is ERC20 {
	...
	address _fundAddress = address(0xe52E99B04c211bBF3315f1286ce81Ec3207a30D3);
    ...
    constructor(address tokenOwner) ERC20("Lunar Probe", "Lunar Probe") {
		...
		nftContract = UsdtWarp(0x7f75b62E6A9bcf7ACDB6A78BBEC41049D3B9ce5C);//nft;
    ...
    function _transfer(
        ...
        if (takeFee) {
			...
			if(from == uniswapV2Pair){
				...
				if(amount >= 28 * 10**18){
					_takeInviterGpFee(from,amount.div(50));
					buyUser.push(to);
				}else{
					buyUser.push(_fundAddress);
					super._transfer(from, _fundAddress, amount.div(50));
				}
            }else{
				super._transfer(from, address(this), amount.div(100).mul(4));
				nftRewardAmount = nftRewardAmount.add(amount.div(50));
			}
    ...
    function swapTokensForFund(uint256 tokenAmount) private {
		...
		if(allamount > uamount){
			addLiquidityUsdt((allamount - uamount), tokenAmount);
		}
    }
	...
	function _takeInviterFeeKt(
        ...
        if(balanceOf(address(this)) > 10**18){
            ...
            for (uint256 i = 0; i < 2; i++) {
                ...
                ktNum = ktNum+1;
                super._transfer(address(this), _receiveD, (i + ktNum).mul(10**11));
            }
        }
    }
    ...
    function swapAndLiquify() private {
		...
		if(thisAmount > nftRemain && nftRemain > 10**19){
			nftRewardOverAmount = nftRewardOverAmount.add(nftRemain);
			swapTokensForOther(nftRemain);
		}
		
		thisAmount = super.balanceOf(address(this));
		if(thisAmount > 10**19){
			nftRewardOverAmount = nftRewardOverAmount.add(nftRemain);
			swapTokensForFund(thisAmount.div(2));
		}
    }
	...
	function _takeInviterGpFee(
		...
		if(size >= 5){
			for (uint256 i = 0; i < 5; i++) {
				super._transfer(sender, buyUser[size - 1 - i], amount.div(5));
			}
		}else{
			for (uint256 i = 0; i < size; i++) {
				super._transfer(sender, buyUser[i], amount.div(5));
			}
			super._transfer(sender, _fundAddress, amount.div(5).mul(5 - size));
		}
    }
    ...
    function splitOtherTokenNft() private {
        ...
        if(thisAmount >= 5 * 10**18){
			_splitOtherNftSecond(5 * 10**18);
        }
    }
}