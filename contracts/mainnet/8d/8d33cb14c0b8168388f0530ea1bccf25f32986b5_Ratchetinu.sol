...
contract Ratchetinu is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    RatchetinuDividendTracker public dividendTracker;
    ...
    address payable public _marketingWalletAddress = payable(0xfFDC39071EC986e77f8cFC2C6d3B7c2C737257Be);
    address payable public _buybackWalletAddress = payable(0x8f54f48dEF06d945998487e292CcE87D3545c758);
    ...
    constructor() BEP20("Ratchet Inu", "RATCHET") 
    {
        dividendTracker = new RatchetinuDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "Ratchetinu: The dividend tracker already has that address");
        RatchetinuDividendTracker newDividendTracker = RatchetinuDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "Ratchetinu: The new dividend tracker must be owned by the Ratchetinu token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Ratchetinu: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Ratchetinu: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Ratchetinu: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Ratchetinu: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Ratchetinu: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Ratchetinu: Cannot update gasForProcessing to same value");
...
contract RatchetinuDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("Ratchetinu_Dividend_Tracker", "Ratchetinu_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "Ratchetinu_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "Ratchetinu_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Ratchetinu contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Ratchetinu_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Ratchetinu_Dividend_Tracker: Cannot update claimWait to same value");
