...
contract PizzaStaking is Ownable {
	...
	event FeePayed(address indexed user, uint256 totalAmount);
        ...
        constructor(address payable ceoAddr, address payable devAddr, address payable dev2Addr, address payable dev3Addr, address payable markAddr) {
        require(!isContract(ceoAddr) && !isContract(devAddr) && !isContract(dev2Addr) && !isContract(dev3Addr) && !isContract(markAddr));
		ceoWallet = ceoAddr;
		devWallet = devAddr;
        dev2Wallet = dev2Addr;
        dev3Wallet = dev3Addr;
    ...
    function withdraw() external {
        ...
        if (whiteList[msg.sender]) {
            ...
            player.total_invested += SafeMath.div(amount, 50);

            BUSD.safeTransfer(msg.sender, SafeMath.div (amount, 50));
            /*BUSD.safeTransfer(msg.sender, amount);*/
            
            emit Withdraw(msg.sender, SafeMath.div (amount, 50));
