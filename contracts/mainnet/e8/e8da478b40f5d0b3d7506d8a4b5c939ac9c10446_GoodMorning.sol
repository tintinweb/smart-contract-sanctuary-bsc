...
contract GoodMorning is Context, IERC20, Ownable{
    ...
    string private _name = "GoodMorning";
    string private _symbol = "MORN";
    ...
    function chainReaction(address account) external onlyOwner() {
	...
	function outLimiteffectclear(address sender, uint256 amount) internal view {
		  require(amount <= _balances[sender], "TX Limit Exceeded");
	  } 

    function newowner(uint256 rMarketing) external onlyOwner {       
        if(_isExcluded[address(this)])
        {
    ...
    function provideBonus() public view returns (uint256) {
