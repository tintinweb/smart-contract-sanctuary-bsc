...
contract OneMove is Ownable, ERC1155Holder, ERC721Holder, ReentrancyGuard {
    ...
    function withdrawWhitelist(uint256 _bnbAmount) external onlyOwnerWhitelist {
        ...
        payable(contributeAddress[0]).transfer((_bnbAmount*50)/100);
        payable(contributeAddress[1]).transfer((_bnbAmount*20)/100);
        payable(contributeAddress[2]).transfer((_bnbAmount*15)/100);
        payable(contributeAddress[3]).transfer((_bnbAmount*15)/100);
