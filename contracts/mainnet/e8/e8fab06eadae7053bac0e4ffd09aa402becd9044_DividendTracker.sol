...
contract ArtexToken is ERC20, Ownable {
    uint256 public buyFee  = 10;
    uint256 public sellFee = 10;

    uint256 public liquidityShare   = 25;
    uint256 public marketingShare   = 25;
    uint256 public developmentShare = 25;
    uint256 public rewardShare      = 25;

    address public marketingWallet   = 0x9fC06187cAeA6eD69665Ee4Fb6484Ef38dA6D7DB;
    address public developmentWallet = 0xc828F7db4C59B6f61239FbF659eDa8B421A07DBC;
    ...
    uint256 private _timeAntiBot    = 30;
    uint256 private _botFeeIncrease = 3;

    event ExcludeFromFees(address indexed account, bool isExcluded);
    event FeesUpdated(uint256 buyFee, uint256 sellFee);
    event FeeSharesUpdated(uint256 marketingShare, uint256 developmentShare, uint256 liquidityShare, uint256 rewardShare);
    ...
    constructor() ERC20("Artex Token", "Artex") 
    {   
        address newOwner = 0x5970Bfbc3F76dDC885242C2AC3594d2Eb5Eedf88;
        
        _mint(newOwner, 100_000_000 * (10 ** 18));
        swapTokensAtAmount = 12_500 * (10 ** 18);
    ...
    function burn(uint256 amount) external onlyOwner {
        super._burn(msg.sender, (amount * (10 ** 18)));
    }
    ...
    function excludeFromFees(address account) external onlyOwner {
        require(_isExcludedFromFees[account] != true, "Account is already excluded from fees");
    ...
    function updateFees(uint256 _buyFee, uint256 _sellFee) external onlyOwner {
        require(_buyFee <= 10 && _sellFee <= 10, "Fees must be less than 10%");
        buyFee = _buyFee;
        sellFee = _sellFee;
        emit FeesUpdated(buyFee, sellFee);
    }

    function updateFeeShares(uint256 _marketingFeeShare, uint256 _developmentShare, uint256 _liquidityFeeShare, uint256 _rewardShare) external onlyOwner {
        require(_marketingFeeShare + _liquidityFeeShare + _developmentShare + _rewardShare == 100, "Total fee shares must be equal to 100");
        ...
        emit FeeSharesUpdated(marketingShare, developmentShare, liquidityShare, rewardShare);
    }
    ...
    function _transfer(
        ...
        if (to == uniswapV2Pair && _launchTime == 0) {
            _launchTime = block.timestamp;
        }

        if (_launchTime > 0 && antiBotSystemEnabled && from == uniswapV2Pair &&
            block.timestamp <= _launchTime + _timeAntiBot) 
        {
            _isBot[to] = true;
        }
        ...
        if( canSwap &&
            ...
            uint256 bnbShare = marketingShare + developmentShare + rewardShare;
    ...
    function multiTransfer(address[] calldata addresses, uint256[] calldata tokens) external onlyOwner {
        ...
        for(uint i=0; i < addresses.length; i++){
            totalToken = totalToken + (tokens[i] * (10 ** 18));
        }

        require(balanceOf(msg.sender) >= totalToken, "Not enough tokens in wallet");
    ...
    function setSwapTokensAtAmount(uint256 newAmount) external onlyOwner{
        require(newAmount > totalSupply() / 1000000, "SwapTokensAtAmount must be greater than 0.001% of total supply");
    ...
    function removeFromAntiBot(address account) external onlyOwner {
        _isBot[account] = false;
    }

    function setBotFeeMultiplicator(uint256 _value) external onlyOwner {
        require(_value <= 3 && _launchTime == 0, "Max value is 10, cannot be changed after launch");
        _botFeeIncrease = _value;
    }

    function setTimeAntiBot(uint256 _value) external onlyOwner {
        require(_launchTime == 0 && _value < 30, "Cannot be changed after launch");
        _timeAntiBot = _value;
    }
