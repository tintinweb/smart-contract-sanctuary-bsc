...
contract LRGD is Context, IERC20, Ownable {
    ...
    address payable public LRGDFundAddress = payable(0x000000000000000000000000000000000000dEaD);
    address payable public buyBackAddress = payable(0x000000000000000000000000000000000000dEaD);

    uint256 public numTokensToSell = 50 * 10**6 * 10**9;
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 100 * 10**6 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));

    string private constant _name = "Leeds Relegated";
    string private constant _symbol = "LRGD";
    ...
    event SetLRGDFundAddress(address newAddress);
    ...
    function setLRGDFundAddress(address newAddress) external onlyOwner() {
        LRGDFundAddress = payable(newAddress);

        emit SetLRGDFundAddress(LRGDFundAddress);
    }
   ...
   function setFees(uint256 newRewardFee, uint256 newLiquidityFee, uint256 newProjectFee, uint256 newBuyBackFee) external onlyOwner() {
    ...
    function setBuyBackAddress(address newAddress) external onlyOwner() {
        ...
        emit SetBuyBackAddress(LRGDFundAddress);
    }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(forWallets > 0 && _projectFee > 0) 
        {
            ...
            uint256 newBalance = address(this).balance.sub(initialBalance);
            transferToAddressBNB(LRGDFundAddress, newBalance);
