...
contract BNBBeatsV3 is Ownable {
    ...
    function userData(address user_)
        external
        view
        returns (UserResponse memory)
    {
        UserV3 memory user = users[user_];
        (, uint256 beatValue, uint256 beats) = calculateMyBeats(user_);
        UserResponse memory userResp;
        userResp.lastHatch_ = user.lastHatch;
        userResp.referrals_ = user.referrals;
        userResp.rewards_ = beats;
        userResp.amountAvailableReinvest_ = SafeMath.sub(beats, beatValue);
        userResp.availableWithdraw_ = beatValue;
        userResp.beatsMiners_ = getBeatsSinceLastHatch(user_);
        userResp.referrer = user.referrer;
        userResp.checkpoint = user.checkpoint;
        userResp.referrerBNB = user.amountBNBReferrer;
        userResp.referrerBEATS = user.amountBEATSReferrer;
        return userResp;
    }
