...
contract BUSDHOURMINER {
    using SafeMath for uint256;
    uint256 constant launch = 1663156800;     //WED SEP 14th 12.00 PM UTC
  	uint256 constant hardDays = 3600;
    uint256 constant percentdiv = 1000;
    uint256 constant min_invest = 10 ether;
    ...
    constructor() {
            owner = msg.sender;
            PercsKey[10] = DivPercs(36000, 1);
            PercsKey[20] = DivPercs(72000, 2);
            PercsKey[30] = DivPercs(108000, 3);
            PercsKey[40] = DivPercs(144000, 4);
            PercsKey[50] = DivPercs(180000, 5);
            
            FeesKey[10] = FeesPercs(36000, 150);
            FeesKey[20] = FeesPercs(72000, 100);
            FeesKey[30] = FeesPercs(108000, 50);
            FeesKey[40] = FeesPercs(144000, 30);
            FeesKey[50] = FeesPercs(180000, 30);
    ...
    function withdrawInitial(uint256 keyy) public {
        ...
        if (currDays < FeesKey[10].daysInSeconds){ // LESS THAN 10 hours STAKED
            uint256 minusAmt = initialAmt.mul(FeesKey[10].feePercentage).div(percentdiv); //5% fee
            ...
            uint256 minusAmt = initialAmt.mul(FeesKey[20].feePercentage).div(percentdiv); //10% fee
            ...
            uint256 minusAmt = initialAmt.mul(FeesKey[40].feePercentage).div(percentdiv); //20% fee
            ...
            uint256 minusAmt = initialAmt.mul(FeesKey[50].feePercentage).div(percentdiv); //12% fee
