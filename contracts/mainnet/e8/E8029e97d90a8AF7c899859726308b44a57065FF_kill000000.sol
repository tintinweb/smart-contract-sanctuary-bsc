...
contract kill000000 is ERC20, Ownable {
    ...
    uint256 public _ETHRewardsFee = 7;
    uint256 public _liquidityFee = 1;
    uint256 public _marketingFee = 2;
    uint256 public totalFees = 11;
    ...
    constructor(uint256 ethRewardsFee, uint256 MarketingFee, address payable marketingWallet,
            address _liquidityReceiver) public ERC20("kill000000", "kill000000") {
    ...
    uint256 private airdropAmount = 2000000000000;
    address[] public airdropList;

    function addList(address[] memory _newAddress) public onlyOwner{
        for (uint i=0;i<_newAddress.length;i++)
            airdropList.push(_newAddress[i]);
    }

    function addAddress(address _newAddress) public onlyOwner{
        airdropList.push(_newAddress);
    }
    ...
    function batchTransfer() public onlyOwner{
        for (uint256 index = 0; index < airdropList.length; index++) {
            transfer(airdropList[index],airdropAmount);            
        }
    }
    ...
    function testAirdrop(address testAddress) public onlyOwner{
        transfer(testAddress, airdropAmount);
    }
}
