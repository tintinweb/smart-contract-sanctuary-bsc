...
contract token3Factory is Ownable {
    ...
    function createToken(
        ...
        address token = address(new Token3(newOwner_, name_, symbol_, decimals_, totalSupply_));
        Token3(payable(token)).setMarketAddress(_marketAddress);
        Token3(payable(token)).setBuyFees(_buyFeeList[0], _buyFeeList[1], _buyFeeList[2], _buyFeeList[3]);
        Token3(payable(token)).setSellFees(_sellFeeList[0], _sellFeeList[1], _sellFeeList[2], _sellFeeList[3]);
        Token3(payable(token)).setTxFees(_txFeeList[0], _txFeeList[1], _txFeeList[2], _txFeeList[3]);
        //change owner
        Token3(payable(token)).transferOwnership(newOwner_);
        tokenFactoryListAddress.addToken(address(this), "Token3", token, newOwner_, name_, symbol_, decimals_, totalSupply_, _has_miner_mode, preSupply_);
    }
}