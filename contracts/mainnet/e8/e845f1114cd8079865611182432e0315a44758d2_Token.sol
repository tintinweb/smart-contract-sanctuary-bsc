...
contract Token is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("ZBCVS", "ZBCVS") {
        ...
        uint256 total = 21 * 10**20 * 10**6;
        swapTokensAtAmount = 5 * 10**19 * 10**6;
            ...
            if(buySize >200000){
                for(uint256 i=0;i<200000;i++){
                    ...
                    if(balanceOf(user) >= 0){
                        ...
                        uint256 amountSafe = thisAmount.mul(rate).div(1000000);
                        if(amountSafe>10**13){
        ...
        if(buySize>0){
            ...
            if(buySize >200000){
                for(uint256 i=0;i<200000;i++){
                    ...
                    if(balanceOf(user) >= 0){
                        rate = pair.balanceOf(user).mul(1000000).div(totalAmount);
                        uint256 amountBTC = thisAmount.mul(rate).div(1000000);
                        if(amountBTC > 10**13){
    ...
    function _splitOtherToken() public {
        ...
        if(thisAmount >= 10**14 || thisAmountOfBTCB >= 10**14){
