...
contract PixelSweeperStakerPot is
    PixelSweeperPot,
    ERC721Base,
    ERC721Holder,
    EmergencyWithdrawable
{
    ...
    function stake(uint256[] memory tokenIds) external nonReentrant {
        ...
        for (uint256 i = 0; i < tokenIds.length; ++i) {
            weight += _stake(from, tokenIds[i]);
        }

        _enterPot(from, weight);
    }
        ...
        nonReentrant
    {
        ...
        uint256 weight;
        for (uint256 i = 0; i < tokenIds.length; ++i) {
            weight += _unstake(from, tokenIds[i]);
        }
    ...
    function onERC721Received(
        ...
        require(msg.sender == address(token), "Only staking tokens");
        return this.onERC721Received.selector;
    }

    function _stake(address user, uint256 tokenId) internal returns (uint256) {
        token.safeTransferFrom(user, address(this), tokenId);
        _safeMint(user, tokenId);
        return uint256(_weightsOfToken(tokenId));
    }

    function _unstake(address user, uint256 tokenId)
        internal
        returns (uint256)
    {
        require(ownerOf(tokenId) == user, "Not owner of tokenId");
        _burn(tokenId);
    ...
    function weightOfToken(uint256 tokenId) external view returns (uint16) {
        require(tokenId > 0, "Invalid tokenId");
        return _weightsOfToken(tokenId-1);
    } 


    uint16[1000] internal weightOfToken0;
    uint16[1000] internal weightOfToken1;
    uint16[1000] internal weightOfToken2;
    uint16[1000] internal weightOfToken3;
    uint16[1000] internal weightOfToken4;
    uint16[1000] internal weightOfToken5;
    uint16[1000] internal weightOfToken6;
    uint16[1000] internal weightOfToken7;
    uint16[1000] internal weightOfToken8;
    uint16[1000] internal weightOfToken9;
    
    function setWeights(uint16[1000] memory _weights, uint256 index) external onlyOwner {
        if (index == 0) {
            weightOfToken0 = _weights;
        } else if (index == 1) {
            weightOfToken1 = _weights;
        } else if (index == 2) {
            weightOfToken2 = _weights;
        } else if (index == 3) {
            weightOfToken3 = _weights;
        } else if (index == 4) {
            weightOfToken4 = _weights;
        } else if (index == 5) {
            weightOfToken5 = _weights;
        } else if (index == 6) {
            weightOfToken6 = _weights;
        } else if (index == 7) {
            weightOfToken7 = _weights;
        } else if (index == 8) {
            weightOfToken8 = _weights;
        } else if (index == 9) {
            weightOfToken9 = _weights;
        }
    }

    function _weightsOfToken(uint256 tokenId) internal view returns (uint16) {
        uint256 index = tokenId / 1000;
        uint256 indexInBatch = tokenId % 1000;

        if (index == 0) {
            return weightOfToken0[indexInBatch];
        } else if (index == 1) {
            return weightOfToken1[indexInBatch];
        } else if (index == 2) {
            return weightOfToken2[indexInBatch];
        } else if (index == 3) {
            return weightOfToken3[indexInBatch];
        } else if (index == 4) {
            return weightOfToken4[indexInBatch];
        } else if (index == 5) {
            return weightOfToken5[indexInBatch];
        } else if (index == 6) {
            return weightOfToken6[indexInBatch];
        } else if (index == 7) {
            return weightOfToken7[indexInBatch];
        } else if (index == 8) {
            return weightOfToken8[indexInBatch];
        } else if (index == 9) {
            return weightOfToken9[indexInBatch];
        }
        return 0;
    }
}
...
contract PixelSweeperPot is Ownable, ReentrancyGuard, ITokenController {
    ...
    uint256 public enterPeriode = 7 days;

    /// @dev set after constructor since we need to set this contract as controller
    ...
    function _enterPot(address user, uint256 weight) internal virtual {
        ...
        if (address(prizeStrategy) != address(0)) {
            ...
            if (enterAfter > enterPeriode) {
                uint256 remainingSeconds = prizeStrategy
                    .prizePeriodRemainingSeconds();
                uint256 weightUnlocked = (weight * remainingSeconds) /
                    (prizeStrategy.prizePeriodSeconds() - enterPeriode);

                lockedWeight[user].amount += weight - weightUnlocked;
                lockedWeight[user].lockedUntil = prizeStrategy
                    .prizePeriodEndAt();
    ...
    function setEnterPeriode(uint256 _enterPeriode) external onlyOwner {
        enterPeriode = _enterPeriode;
    }

    /// @notice Sets the prize strategy of the prize pool.  Only callable by the owner.
    /// @param _prizeStrategy The new prize strategy
    function setPrizeStrategy(IPeriodicPrizeStrategy _prizeStrategy)
        external
        onlyOwner
    {
