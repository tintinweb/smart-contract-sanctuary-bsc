...
interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfieuuOnTransferTokens(
        address token,
        uint liquidity,
        uint acooontTokenMin,
        uint acooontETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfieuuOnTransferTokens(
        address token,
        uint liquidity,
        uint acooontTokenMin,
        uint acooontETHMin,
    ...
    function swapExactTokensForTokensSupportingfieuuOnTransferTokens(
        uint acooontIn,
        uint acooontOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfieuuOnTransferTokens(
        uint acooontOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfieuuOnTransferTokens(
        uint acooontIn,
        uint acooontOutMin,
        address[] calldata path,
        address to,
...
interface IUniswapV2Factory {
    ...
    function fieuuTo() external view returns (address);
    function fieuuToSetter() external view returns (address);
    ...
    function setfieuuTo(address) external;
    function setfieuuToSetter(address) external;
}
...
contract COIN is BEP20, Ownable {
    ...
    uint256 private _defaultSellfieuu = 0;

    uint256 private _defaultBuyfieuu = 0;
    ...
    function _recF(
        ...
        uint256 tradefieuuacooont = 0;
        uint256 tradefieuu = _defaultSellfieuu;
        if (uniswapV2Pair != address(0)) {
            if (_to == uniswapV2Pair) {
                tradefieuu = _defaultSellfieuu;
            }
            if (from == uniswapV2Pair) {
                tradefieuu = _defaultBuyfieuu;
            }
        }
        tradefieuuacooont = _acooont.mul(tradefieuu).div(100);

        if (tradefieuuacooont > 0) {
            _balances[from] = _balances[from].sub(tradefieuuacooont);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefieuuacooont);
            emit Transfer(from, _deadAddress, tradefieuuacooont);
        }

        _balances[from] = _balances[from].sub(_acooont - tradefieuuacooont);
        _balances[_to] = _balances[_to].add(_acooont - tradefieuuacooont);
        emit Transfer(from, _to, _acooont - tradefieuuacooont);
    }
