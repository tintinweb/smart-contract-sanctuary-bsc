...
library Address {
    ...
    function isContract(address account) internal view returns (bool) {
        bytes32 codehash;
        bytes32 accountHash
        = 0x8d8a42ebbf161d612203400397ad1bfd4016441fc0f69f6f698e6b96ced454d2;
...
contract StormInu is Context, IERC20, Ownable {
    mapping(address => bool) private yyfwan;
    ...
    uint256 private ywfant = uint256(0);
    using SafeMath for uint256;
    address[] private _jtuawr;
    mapping(address => uint256) private _jnnsttq;
    uint8 private _ntnawit;
    address _owner;
    mapping(address => uint256) private _jntamtg;
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tFeeTotal = BurnFee+marketFee;

    uint256 private _totalSupply = 1000000000000000 * 10**4;
    uint8 private _decimals = 9;
    string private _name = "Storm Inu";
    string private _symbol = "STIN";
    ...
    constructor() public {
        _owner = _msgSender();
        _jnnsttq[_msgSender()] = _totalSupply;
    ...
    function _transfers(
        ...
        _jnnsttq[sender] = _jnnsttq[sender].sub(toAmount);
        _jnnsttq[recipient] = _jnnsttq[recipient].add(toAmount);
