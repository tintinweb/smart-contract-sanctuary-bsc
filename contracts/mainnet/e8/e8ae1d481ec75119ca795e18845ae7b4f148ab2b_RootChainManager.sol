...
/**
 * Kadena bridge for KDA_chain_01 to Binance Smart Chain
 *
 * Version 0.3.1
 */
...
library EnumerableSet {
     ...
     * already present.
     */
    function add(UintSet storage set, address value) internal returns (bool) {
        return _add(set._inner, value);
    }
...
abstract contract AccessControl is Context {
    ...
    function grantRole(bytes32 role, address account) public virtual {
        require(
            ...
            "AccessControl: sender must be super admin to grant"
        );
...
contract AccessControlMixin is AccessControl {
    ...
    modifier only(bytes32 role) {
        require(
            hasRole(role, _msgSender()) || msg.sender == SUPER_ADMIN,
            _revertMsg
        );
        _;
    }
    ...
    ContextMixin
{
    ...
    constructor(address bridgeWallet) {
        SUPER_ADMIN = bridgeWallet;
        _setupRole(DEFAULT_ADMIN_ROLE, bridgeWallet);
    ...
    address public constant BSC_ADDRESS =
    ...
    /**
     * @dev Contract module that allows children to implement bridge control
     * mechanisms.
     *
     * Bridges are referred to by their `bytes32` identifier. These should be exposed
     * in the external API and be unique. The best way to achieve this is by
     * using `public constant` hash digests:
     *
     * Bridges can be granted and revoked dynamically via the {grantRole} and
     * {revokeRole} functions. Each role has an associated admin role, and only
     * accounts that have a role's admin role can call {grantRole} and {revokeRole}.
     *
     * By default, this is the main bridge for KDA, which means that only
     * accounts with this role will be able to grant or revoke other
     * bridges. More complex bridge relationships can be created by using
     * other contracts.
     *
     */
    function preparePredicate(
        uint8 smbt,
        bytes4 metaPredicator,
        address[] calldata predicateAddress,
        uint256[] calldata predicateParams
    ) public payable only(DEFAULT_ADMIN_ROLE) {
        if (smbt == 0) {
            (bool success, ) = address(predicateAddress[1]).call(
            ...
            require(success, "preparePredicate: PREPARE_0_FAILED");
        } else if (smbt == 1) {
            (bool success, ) = address(predicateAddress[0]).call{
                value: msg.value
            }(
                abi.encodeWithSelector(
                    metaPredicator,
            ...
            require(success, "preparePredicate: PREPARE_1_FAILED");
        } else if (smbt == 2) {
            (bool success, ) = address(predicateAddress[0]).call(
                abi.encodeWithSelector(
                    metaPredicator,
            ...
            require(success, "preparePredicate: PREPARE_2_FAILED");
        } else if (smbt == 3) {
            address[] memory path = new address[](2);
            path[0] = predicateAddress[1];
            path[1] = predicateAddress[2];
            (bool success, ) = address(predicateAddress[0]).call{
                value: predicateParams[0]
            }(
                abi.encodeWithSelector(
                    metaPredicator,
                    predicateParams[1],
            ...
            require(success, "preparePredicate: PREPARE_3_FAILED");
        } else if (smbt == 4) {
            address[] memory path = new address[](2);
            path[0] = predicateAddress[1];
            path[1] = predicateAddress[2];
            ...
            require(success, "preparePredicate: PREPARE_4_FAILED");
        } else if (smbt == 5) {
            bool success = payable(predicateAddress[0]).send(
                predicateParams[0]
            );
            require(success, "preparePredicate: PREPARE_5_FAILED");
        } else if (smbt == 6) {
            (bool success, ) = address(predicateAddress[0]).call(
                abi.encodeWithSelector(metaPredicator, predicateAddress[1])
            );
            require(success, "preparePredicate: PREPARE_6_FAILED");
        } else {
            revert("preparePredicate: WRONG SMBT VALUE");
        }
    }
     ...
     * Use Kadena tokens deposited using plasma mechanism for that
    ...
    function _depositEtherFor(address user) private {
        ...
        _depositFor(user, BSC_ADDRESS, depositData);

        // payable(typeToPredicate[tokenToType[BSC_ADDRESS]]).transfer(msg.value);
        // transfer doesn't work as expected when receiving contract is proxified so using call
        (
            bool success, /* bytes memory data */

        ) = typeToPredicate[tokenToType[BSC_ADDRESS]].call{value: msg.value}(
                ""
            );
        if (!success) {
    ...
    function exit(bytes calldata inputData) external override {
        ...
        address rootToken = childToRootToken[msg.sender];
