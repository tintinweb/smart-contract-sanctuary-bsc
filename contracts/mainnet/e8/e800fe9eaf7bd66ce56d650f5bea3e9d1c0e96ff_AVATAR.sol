...
contract AVATAR is ERC20 {
    ...
    address public marketAddress = address(0x971cD130657c60193c0491363935469A88888888);

    uint256 public lpFee = 60;
    ...
    string internal tokenName = "AVATAR";
    string internal tokenSymbol = "AVATAR";
    uint256 constant total  = 100000 * 10**18;
    uint256 constant mintotal  = 10000 * 10**18;
    ...
    function _transfer(
        ...
        if(balanceOf(address(this)) > swapTokensAtAmount && block.timestamp >= (currentTime.add(lpIntervalTime))){
            if (
                !swapping &&
                _tokenOwner != from &&
                _tokenOwner != to &&
        ...
        if (takeFee) {
            ...
            if(to == uniswapV2Pair || from == uniswapV2Pair ){
                ...
                if(_currentSupply > mintotal){
                    ...
                    super._transfer(from, uniswapV2Pair, amount.div(1000).mul(liquidFee));

                }else{
                    super._transfer(from, uniswapV2Pair, amount.div(1000).mul((liquidFee + destroyFee)));
                }

                super._transfer(from, address(this), amount.div(1000).mul((lpFee + marketFee)));
                amount = amount.div(1000).mul((1000 - (lpFee + liquidFee + destroyFee + marketFee)));
