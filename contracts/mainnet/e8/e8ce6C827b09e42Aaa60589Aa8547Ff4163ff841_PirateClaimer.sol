...
contract PirateClaimer is Pausable, ReentrancyGuard {
  address public _nftContract;
  PirateWarAccessControl public _accessControl;

  // whitelisted address address => allowedAmount
  mapping(address => uint256) public _allowedAddresses;

  constructor(address nftContract, address accessControl) {
  ...
  function claimCharacter() external whenNotPaused nonReentrant {
    // check if address is whitelisted
    require(_allowedAddresses[msg.sender] > 0, 'No more characters to claim');

    IPirateWarCharacter(_nftContract).summonNewPirates(msg.sender, 1, 20);

    // remove from whitelist
    _allowedAddresses[msg.sender] = 0;
  }
  ...
  /// @dev whitelist multiple address
  function whitelists(address[] calldata _addresses) external onlyAdmin {
    for (uint256 i = 0; i < _addresses.length; i++) {
      _allowedAddresses[_addresses[i]] = 1;
    }
  }
