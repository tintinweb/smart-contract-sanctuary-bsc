...
contract Mdate is NFToken {
    ...
    internal  validNFToken(_tokenId)  {
        all[_tokenId] =  starall(_random,amounts,_uri);
    }
    function totalSupply() external view returns (uint256) {
    ...
    function  getAllType(uint _tid) public view returns (bool ret_random,uint256 ret_amounts){
        ret_random = all[_tid].random;
        ret_amounts = all[_tid]._value;
    }
}
...
contract zNFTCONTRACT is Mdate,Ownable{
        ...
        onlyManager
    {
        ...
        super._mint(_to, _tokenId);
        super._setTokenstarall(_tokenId,_random,amounts,_uri);
    }
