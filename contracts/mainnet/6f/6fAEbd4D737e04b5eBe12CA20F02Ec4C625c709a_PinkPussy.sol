...
/**
TG: pinkpussytg
Project Name: PINK PUSSY

PINK PUSSY 

This for sure is one of very few community meme token set to take BSC by CUM storm.
You’ll know the power of the PUSSY…You’re about to experience the power of the $PINKPUSSY 

💦Live Sex Cam 
💦Staking for Pornhub membership 
💦PINKPUSSY NFTs 
💦Porn Star Partnership
...
contract PinkPussy is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    PinkPussyDividendTracker public dividendTracker;
    ...
    // Anti Dump //
    uint256 public maxSellAmountPerCycle = 5000 * 10**18;
    uint256 public antiDumpCycle = 10 minutes;
    ...
    constructor() ERC20("PinkPussy", "PPussy") {
        marketingWallet = payable(0x310a96682295509195A04f1E5D7AA0bf1daCF11D);
        devWallet = payable(0x464Ad2232005Ba11001E0137dEba973F1C05CC20);
        address router = 0x10ED43C718714eb63d5aA57B78B54704E256024E; // testnet router: 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3 
        ...
        dividendTracker = new PinkPussyDividendTracker(
            payable(this),
            router,
...
contract PinkPussyDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    PinkPussy public PinkPussyContract;
    ...
    constructor(
        ...
        defaultToken = token;
        PinkPussyContract = PinkPussy(mainContract);
    ...
    function _transfer(
        ...
        require(false, "PinkPussy_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "PinkPussy_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PinkPussy contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(PinkPussyContract);

            uint256 prevBalance = PinkPussyContract.balanceOf(address(this));
            ...
            {
                uint256 received = PinkPussyContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    PinkPussyContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
