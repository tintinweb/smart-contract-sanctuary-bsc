...
contract GNFT is ITRC721, ITRC165, ITRC721Metadata, ITRC721Enumerable {
    using EnumerableSet for EnumerableSet.UintSet;
    // using EnumerableSetMarket for EnumerableSetMarket.MarketLotSet;

    string internal _name = "GNFT";
    string internal _symbol = "Golang Chain NFT";
    IERC20 public FON = IERC20(0x12a055D95855b4Ec2cd70C1A5EaDb1ED43eaeF65);
    uint256 internal totalMinted = 0;
    uint256 internal totalMigrated = 0;

    uint256 public constant MINTING_LIMIT = 8000;

    uint256 public mintingPrice = 49e18;
    uint256 public minimumMarketPrice = 10000000000e18;

    uint256 internal devComissionNom = 1;
    uint256 internal devComissionDenom = 100;
    uint256 public sum_tokenId=1;
    mapping (address => uint ) public is_two;
    ...
    address admin;
    string data_url = "https://arweave.net/0m1QAQ13yeTQgAS3KcEnUQRGmGfd36d2VFCRFaoCtd8";
    // Market
    struct MarketLot {
        uint256 tokenId;
        bool isForSale;
    ...
    function mint() external payable  {
        require(FON.balanceOf(msg.sender) > 49e18,"small 50");
        require(FON.allowance(msg.sender,address(this))>0,"allow");
        require(sum_tokenId < 201,'ID>200');
        require(is_two[msg.sender]<2,">2");
        FON.transferFrom(msg.sender,address(0x3699bD200904179e9d71ABB722b3623E4a1D5DF3),50e18);
        // uint num = msg.value/49e6;
        // for(uint i=0;i<num;i++){
            // _mint(msg.sender);
        _addToken(msg.sender, sum_tokenId);
        URI[sum_tokenId] = data_url;
            // emit Mint(sum_tokenId, msg.sender,msg.sender);
        emit Transfer(addressZero, msg.sender, sum_tokenId);
        sum_tokenId++;
        is_two[msg.sender] += 1;

        // }
        //TPor6mxouKuF2rtukPLhTk95C8b3HkVbJT
    ...
    modifier  is_admin(){
        require(msg.sender == address(0x3699bD200904179e9d71ABB722b3623E4a1D5DF3),"no admin");
        _;
    }
    function mint_admin() external is_admin  {
