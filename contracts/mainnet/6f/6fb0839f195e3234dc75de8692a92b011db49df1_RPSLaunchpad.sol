...
interface IRpsNFT {
    function launchpadMint(address to, bool freeze)
        external
        returns (uint256 id);
}

contract RPSLaunchpad is
    Initializable,
    PausableUpgradeable,
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize(IRpsNFT _RpsNFT, address _treasuryAddress)
        public
        initializer
    {
        __Pausable_init();
        ...
        launches.push(
            Launchpad({
                price: 40 ether,
                status: true,
                dealToken: 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56,
                amount: 2000,
                sold: 0,
                isMulti: false,
                multiTimes: 0
    ...
    function updateLaunch(
        ...
        launches[_index].isMulti = _isMulti;
        launches[_index].multiTimes = _multiTimes;
    }

    function gacha(uint256 _launchIndex) external nonReentrant whenNotPaused {
        require(_launchIndex < launches.length, "Invalid launchpad");
        require(!launches[_launchIndex].isMulti, "Invalid launchpad");
        ...
        IERC20(launches[_launchIndex].dealToken).safeTransferFrom(
            msg.sender,
            treasuryAddress,
            launches[_launchIndex].price
        );

        launches[_launchIndex].sold++;

        uint256 nftId = RpsNFT.launchpadMint(msg.sender, false);

        emit Gacha(msg.sender, nftId, _launchIndex, block.timestamp);
    }

    function multiGacha(uint256 _launchIndex)
        external
        nonReentrant
        whenNotPaused
    // returns (uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < launches[_launchIndex].multiTimes; i++) {
            uint256 nftId = RpsNFT.launchpadMint(msg.sender, false);
            emit MultiGacha(msg.sender, nftId, _launchIndex, block.timestamp);
        }
    }
}