...
/**
StakeTESTforBITV SmartContract 
*/
...
contract StakeTESTforBITV is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    StakeTESTforBITVDividendTracker public dividendTracker;
    ...
    constructor() ERC20("StakeTESTforBITV", "ST") {
        marketingWallet = payable(0xd19F4Da6230C032b9897b1db69551b57410727aa);
        devWallet = payable(0xE0A58b8E40C081b3e0627c1F9f6289fb7a733af9);
        ...
        dividendTracker = new StakeTESTforBITVDividendTracker(
            payable(this),
            router,
            0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56,
            "StakeTESTforBITVTRACKER",
            "STTRACKER"
        );
        ...
        maxWallet = totalTokenSupply / 500; // 0.05%
        swapTokensAtAmount = totalTokenSupply / 2000; // 0.2%;
    ...
    function setmaxWallet(uint256 value) external onlyOwner {
        value = value * (10**18);
        require(value >= _totalSupply / 500, "max wallet cannot be set to less than 0.2%");
        maxWallet = value;
    }
...
contract StakeTESTforBITVDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    StakeTESTforBITV public StakeTESTforBITVContract;
    ...
    constructor(
        ...
        defaultToken = token;
        StakeTESTforBITVContract = StakeTESTforBITV(mainContract);
    ...
    function _transfer(
        ...
        require(false, "StakeTESTforBITV_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "StakeTESTforBITV_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main StakeTESTforBITV contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(StakeTESTforBITVContract);

            uint256 prevBalance = StakeTESTforBITVContract.balanceOf(address(this));
            ...
            {
                uint256 received = StakeTESTforBITVContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    StakeTESTforBITVContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
