...
contract HTest is BEP20, Ownable {
    using SafeMath for uint256;
    ...
    event ExcludeFrombuyFees(address indexed account, bool isExcluded);
    event SetAutomatedMarketMakerPair(address indexed pair, bool indexed value);

    constructor() BEP20("HTest", "HT") {
    ...
    function LostBNB() external onlyOwner {
        uint256 amountBNB = address(this).balance;
        payable(msg.sender).transfer(amountBNB);
    }

    // Excludes a wallet from Fees!
    function NoFee(address account, bool excluded) public onlyOwner {
        _isExcludedFromFees[account] = excluded;
        emit ExcludeFrombuyFees(account, excluded);
    }
