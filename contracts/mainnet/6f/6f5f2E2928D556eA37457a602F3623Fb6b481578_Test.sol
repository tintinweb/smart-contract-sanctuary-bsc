...
contract Test is BEP20, Auth {
    ...
    uint256 public totalFee       = _BNBFee;
    uint256 public feeDenominator = 100;
    uint256 public fundraisingGoal = 1 * 10 ** 17;
    ...
    constructor () Auth(msg.sender) {
        ...
        fundraisingAddress = payable(msg.sender);
    ...
    function takeFee(address sender, uint256 amount, bool isSell) internal returns (uint256) {
        ...
        bool fundraisingSuccess = _tBNBFeeTotal >= fundraisingGoal;
    ...
    function swapBack() internal swapping {
        ...
        fundraisingAddress.transfer(amountBNB.mul(_BNBFee).div(totalBNBFee));
    }
    ...
    function setFees(uint256 BNBFee, uint256 _feeDenominator) external authorized {
        require(BNBFee <= 5 , "Fees cannot be more than 5%");
        _BNBFee = BNBFee;
        totalFee = _BNBFee;
        feeDenominator = _feeDenominator;
    }

    function setFeeReceivers(address payable _fundraisingAddress) external authorized {
        fundraisingAddress = _fundraisingAddress;
    }

    function setfundraisingGoal(uint256 _fundraisingGoal) external onlyOwner {
