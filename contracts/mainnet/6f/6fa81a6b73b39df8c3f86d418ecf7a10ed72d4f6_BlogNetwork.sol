...
contract BlogNetwork is IERC20, Ownable {
    ...
    constructor(address tokenOwner,
        ...
        _tTotalFeeMax = 1800 * 10**_decimals;
    ...
    function _transfer(
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to] || destroyAmount >= _tTotalFeeMax){
            ...
            require(amount <= 2 * 10**18,"Every time is less than 3");//每笔最多交易
            if(from == uniswapV2Pair){
                require(balanceOf(to).add(amount) <= 5 * 10**18,"total is less than 10");//持币量最大量
                _tokenTransfer(from, to, amount, true);
            }else if(to == uniswapV2Pair){
                _tokenTransferSell(from, to, amount, true);
            }else{
                require(balanceOf(to).add(amount) <= 3 * 10**18,"total is less than 10");//持币量最大量
                _tokenTransfer(from, to, amount, false);
            }
        }
    }
