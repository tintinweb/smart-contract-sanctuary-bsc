...
contract swapHelper is  Ownable {
    ...
    function autoBuyHXJ(uint256 _usdtAmount,uint256 _minSwapOut) external {
        USDT.transferFrom(msg.sender,address(this),_usdtAmount);
        address[] memory path = new address[](2);
        path[0] = address(USDT);
        path[1] = address(HXJ);
        swapRouter.swapExactTokensForTokens(_usdtAmount, _minSwapOut, path, msg.sender, block.timestamp);
    }

    function autoAddLiquidity(uint256 _usdtAmount,uint256 _minSwapOut) external {
        USDT.transferFrom(msg.sender,address(this),_usdtAmount);
        address[] memory path = new address[](2);
        path[0] = address(USDT);
        path[1] = address(HXJ);
        ...
        (uint256[] memory amounts) = swapRouter.swapExactTokensForTokens(half, _minSwapOut, path, address(this), block.timestamp);
