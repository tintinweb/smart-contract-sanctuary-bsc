...
contract cantryDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("cantry_Dividend_Tracker", "cantry_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 1000 * (10**18); //must hold 1000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "cantry_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "cantry_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main cantry contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "cantry_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");   
        require(newClaimWait != claimWait, "cantry_Dividend_Tracker: Cannot update claimWait to same value");
...
contract cantry is BEP20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    cantryDividendTracker public dividendTracker;
    ...
    constructor() BEP20("cantry", "NST") 
    {
        ...
        dividendTracker = new cantryDividendTracker();
    ...
    {
        require(newAddress != address(uniswapV2Router), "cantry: The router already has that address");
    ...
    {
        require(_isExcludedFromFees[account] != excluded, "cantry: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "cantry: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "cantry: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "cantry: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "cantry: Cannot update gasForProcessing to same value");
