...
contract Stake is Ownable{
    ...
    address public sera = address(0);

    address public era = address(0);
    ...
    constructor(address _usdt, address _sera, address _era) public {
        ...
        poolInfo[0].token1 = sera;
        poolInfo[0].awardToken = era;
        poolInfo[0].awardPerDay = 288800 * 1e18;
        poolInfo[0].minLimit = 130*1e18;
        poolInfo[0].awardRefer = 8*1e18;
    }
    ...
    function doAward(uint8 pid, address addr) internal {
        PoolInfo storage pool = poolInfo[pid];
        for(uint8 g=0; g<10; g++) {
            ...
            if(userNode.referrer != address(0) && upNode.count > g && power_left >= pool.awardRefer) {
                address[] memory path = new address[](2);
                path[0] = address(pool.awardToken);
                path[1] = address(usdt);
                uint[] memory amounts_in = uniswapV2Router.getAmountsIn(pool.awardRefer, path);
                
                pool.debtSum += pool.awardRefer;
                upline.award += amounts_in[0];
                upline.award_in_usdt += pool.awardRefer;

                TransferHelper.safeTransfer(pool.awardToken, userNode.referrer, amounts_in[0]);
                emit Award(pid, addr, userNode.referrer, pool.awardRefer, amounts_in[0], g);
            }else{
                //销毁
                TransferHelper.safeTransfer(pool.awardToken, destroyAddress, pool.awardRefer);
            }
