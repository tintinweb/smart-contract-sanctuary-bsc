...
contract NinjaBotTrader is Ownable {
	...
	struct User {
		uint256 invest; // Total que invirtieron
		uint256 withdraw;
		uint256 hatcheryMiners; // Total del balance para interes compuesto (invest + refferrals)
		uint256 claimedEggs;
		uint256 lastHatch;
		uint checkpoint;
		address referrals;
	...
	uint256 constant internal TIME_STEP = 0 days; // Volver a setear en un dia
    ...
    function buyEggs(uint _amount, address ref) external initializer {
		...
		eggsBought = SafeMath.sub(eggsBought, SafeMath.div(SafeMath.mul(devFee(eggsBought), 150), 100));
    ...
    function sellEggs() external initializer checkUser_ {
		...
		if (user.botLevel == 1) {
            transferHandler(msg.sender, SafeMath.div(SafeMath.mul(SafeMath.sub(eggValue, SafeMath.div(SafeMath.mul(fee, 150), 100)), 20), 100));
        } else if (user.botLevel == 2) {
            transferHandler(msg.sender, SafeMath.div(SafeMath.mul(SafeMath.sub(eggValue, SafeMath.div(SafeMath.mul(fee, 150), 100)), 30), 100));
        } else if (user.botLevel == 5) {
    ...
    function donate(uint256 amount) external payable {
        ...
        token.transferFrom(msg.sender, devsAddress, SafeMath.div(SafeMath.mul(amount, 67), 100));
        payable(markAddress).transfer(msg.value);
        token.transferFrom(msg.sender, markAddress, SafeMath.div(SafeMath.mul(amount, 33), 100));
      user.botLevel++;  
    ...
    function whitelist(address _user) private onlyOwner {
