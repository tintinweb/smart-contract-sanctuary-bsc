...
library ConfigUtils {
  ...
  // keccak256("bool");
  bytes32 internal constant BOOL_HASH = 0xc1053bdab4a5cf55238b667c39826bbb11a58be126010e7db397c1b67c24271b;

  error ConfigNotValid();
  function _checkConfig(IConfig config) internal view {
    if(config.version() == 0 || !config.supportsInterface(type(IConfig).interfaceId)){
      revert ConfigNotValid();
    }
  }
  ...
  error ConfigKVTypeErr(bytes32 typeID, bytes32 want);
  function _checkTypeID(bytes32 typeID, bytes32 want) pure private{
    if(typeID != want){
      revert ConfigKVTypeErr(typeID, want);
    }
  }
  function _bytesToAddress(bytes32 typeID, bytes memory data) internal pure returns (address addr) {
    _checkTypeID(typeID, ADDRESS_HASH);
...
library ProxyConfigUtils{
  ...
  function _getRawValue(bytes32 key) internal view returns(bytes32 typeID, bytes memory){
    return _getConfig().getRawValue(key);
  }
}
...
abstract contract DABase{
  ...
  function hasRole2(bytes32 role, address account) internal view returns(bool has){
    return ProxyConfigUtils._getConfig().hasRole2(role, account);
  }
  ...
  error NotAdminErr();
  modifier onlyAdmin(){
    if(!isAdmin(msg.sender) && !hasRole(Registry.contractAdmin(address(this)), msg.sender)){
      revert NotAdminErr();
    }
    _;
  }
...
contract Ucon is DABase, ERC20Burnable {
  using EnumerableSet for EnumerableSet.AddressSet;

  //////////////////// constant
  // 1 million
  uint256 constant private TOTALAMOUNT = 1000000e18;
  ...
  function mint(uint256 amount) public virtual {
    ...
    require(realMintAmount <= TOTALAMOUNT, "mint exceeds");

    _mint(ProxyConfigUtils._getAddress(MINER_TARGET_ADDR), amount);
  }
