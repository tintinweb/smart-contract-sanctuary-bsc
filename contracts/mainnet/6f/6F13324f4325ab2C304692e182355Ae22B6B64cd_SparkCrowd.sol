...
contract SparkCrowd is AccessControlEnumerable {
    ...
    address public fee = 0x849377ae3893518235bd87CCE0cda018C12CaAb3;
    address public donate = 0xf5286627D5eFFF8834Ee6d6a713876e958167d68;
    address public crowdAccount = 0x74eDea1889346a5d1Ab31be7298C07C4Cb4F3A2f;
    uint16[] public bonus = [0, 0, 10, 20, 30, 40];
    uint16[] public defaultParams = [150, 20, 750, 30, 0, 600, 500, 10, 0, 10, 100, 3, 5, 10];
    uint16 public fundInterval = 30;
    uint256 public leverageThreshold = 30000e18;
    ...
    function setLeverageFlag(bool flag) external {
        require(ICrowdAccount(crowdAccount).accountVips(msg.sender) >= 4, "level limited");
        require(fundAmounts[msg.sender] >= leverageThreshold, "amount limited");
    ...
    function setFundAmount(address addr, uint256 amount) external onlyRole(OPERATOR_ROLE) {
        fundAmounts[addr] = amount;
    }
    ...
    function safeApprove(address spender, uint256 amount) external onlyRole(RESCUE_ROLE) {
    ...
    function setFundParams(uint16 interval, uint256 threshold) external onlyRole(OPERATOR_ROLE) {
    ...
    function setRegionInitParams(
        ...
        regions[regionIndex].poolType = poolType;
    }
    ...
    function _addProject(uint256 regionIndex) internal {
        ...
        if (pl == 1) {
            ...
            if (regions[regionIndex].poolType == 1) {
    ...
    function fund(uint256 regionIndex, uint256 amount) external {
        ...
        fundAmounts[msg.sender] += fundAmount;
    ...
    function processDonate(uint256 amount) internal {
        IERC20(USDT).safeTransfer(donate, amount);
    }
    ...
    function getLeverageAmount(
        ...
        return leverageMap[addr] == true ? amount - (amount * regions[regionIndex].params[2]) / 1000 : amount;
    }
}
