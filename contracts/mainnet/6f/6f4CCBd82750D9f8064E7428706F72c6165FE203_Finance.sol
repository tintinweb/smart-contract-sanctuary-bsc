...
contract Finance is Ownable, ReentrancyGuard {
    ...
    function pendingToken(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && tokenSupply != 0) {
            ...
            tokenReward = totalReward + tokenReward * 115 / 100 > maxReward ? maxReward - totalReward : tokenReward;
    ...
    function update() public {
        if (block.number > lastRewardBlock) {
            ...
            if (tokenSupply > 0) {
                ...
                uint256 tokenReward = multiplier * allocReward;
                tokenReward = totalReward + tokenReward * 115 / 100 > maxReward ? maxReward - totalReward : tokenReward;
                totalReward += tokenReward * 115 / 100;
    ...
    function deposit(uint256 _amount, uint256 _grade) external nonReentrant {
        ...
        require(_amount > minAmount, "Finance:Quantity cannot be 0");
        require(_grade > 0, "Finance:grade cannot be 0");

        UserInfo storage user = userInfo[msg.sender];

        require(user.amount == 0, "Finance:Can not be re-staking");

        update();

        IERC20Metadata(lpToken).safeTransferFrom(msg.sender, address(this), _amount);
        ...
        user.grade = _grade;
        user.rewardDebt = _amount * _grade * tokenPerShare / ACC_TOKEN_PRECISION;
        user.depositTime = block.timestamp;

        emit Deposit(msg.sender, _amount, _grade, 0, 0, block.timestamp);
    }
    ...
    function withdraw() external nonReentrant {
        ...
        require(grade * 7 * 86400 + user.depositTime <= block.timestamp, "Finance:The pledge time has not expired");

        update();

        uint256 _amount = user.amount;

        require(_amount > 0, "Finance:withdraw: Insufficient");

        uint256 accToken = _amount * grade * tokenPerShare / ACC_TOKEN_PRECISION;
        ...
        if (pending > 0) {
            ...
            if (superior != address(0) && superior != address(1) && userInfo[superior].amount > 0) {
                fee1 = pending * 10 / 100;
                if (fee1 > 0) {
                    if (userInfo[superior].amount > 0) {
                        GSS(GSSToken).transfer(superior, fee1);
                    ...
                    if (twoSuperior != address(0) && twoSuperior != address(1) && userInfo[twoSuperior].amount > 0) {
                        fee2 = pending * 5 / 100;
                        if (fee2 > 0) {
                            GSS(GSSToken).transfer(twoSuperior, fee2);
        ...
        user.amount = 0;
        user.pending = 0;
        user.rewardDebt = accToken;

        totalBoostedShare = totalBoostedShare - _amount * grade;

        IERC20Metadata(lpToken).transfer(msg.sender, _amount);

        emit Withdraw(msg.sender, _amount, grade, 0, 0, pending, block.timestamp);
    }
    ...
    function transfer(address _token, address _to) external onlyOwner {
        ...
        IERC20Metadata(_token).transfer(_to, IERC20Metadata(_token).balanceOf(address(this)));
    }
...
interface IGSS {
    function inviter(address user) external view returns (address superior);
}


contract GSS is ERC20Basics, Ownable {
    ...
    constructor(address _old, address _old2, address _USDTToken, address _Pledge, address _Constellation, address _Pope, address _GSSHoldToken, address _marketing, address _community, address _lp, address _uniswapV2RouterAddress)
    ERC20Basics("Gold Saints", "GSS", 18)
    {
        ...
        _mint(address(this), 4000000 * (10 ** _decimals));

        _mint(_GSSHoldToken, 200000 * (10 ** _decimals));

        _mint(_Pledge, 1000000 * (10 ** _decimals));

        _mint(_Constellation, 100000 * (10 ** _decimals));

        _mint(_Pope, 200000 * (10 ** _decimals));

        _mint(_marketing, 200000 * (10 ** _decimals));

        _mint(_community, 300000 * (10 ** _decimals));

        _mint(_lp, 4000000 * (10 ** _decimals));

        IUniswapV2Router02 _uniswapV2Router;
        address _uniswapV2Pair;
        {// scope to avoid stack too deep errors
            address uToken = _USDTToken;
            _uniswapV2Router = IUniswapV2Router02(_uniswapV2RouterAddress);
            _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(address(this), uToken);
    ...
    function swap(address oldAddress, uint256 _amount) public returns (bool) {
        require(oldAddress == old || oldAddress == old2, "GSS swap:oldAddress Err");

        address from = _msgSender();
        uint256 amount = ERC20Basics(oldAddress).balanceOf(from);
        require(_amount <= amount, "balance Err");

        ERC20Basics(old).transferFrom(from, _destroyAddress, _amount);

        _transfer(address(this), from, _amount);
    ...
    function destroy() public onlyOwner returns (bool) {
        ...
        _transfer(address(this), _destroyAddress, 4000000 * (10 ** decimals()) - swapAmount);
    ...
    function _tokenTransfer(address from, address to, uint256 amount) private {
        ...
        if (from == _uniswapV2Pair || to == _uniswapV2Pair) {
            ...
            if (fee > 0) {
                ...
                if (_destroyFee > 0) {
                    _transfer(from, _destroyAddress, _destroyFee);
                }
                uint256 _inviterFee = fee * _inviterFeeRate / 100;
                if (_inviterFee > 0 && inviter[_user] != address(0)) {
                    _transfer(from, inviter[_user], _inviterFee);
                }
                uint256 _dividendFee = fee * _dividendFeeRate / 100;
                if (_dividendFee > 0) {
                    _transfer(from, PopeFeeDividendToken, _dividendFee);
    ...
    function _migration_invite(address user) internal {
        if (inviter[user] != address(0)) {
            return;
        }

        address superior = IGSS(old2).inviter(user);
        if (superior == address(0)) {
            return;
        }
        _invite(user, superior);

        address twoSuperior = IGSS(old2).inviter(superior);
        if (twoSuperior == address(0)) {
            return;
        }
        _invite(superior, twoSuperior);
    }

    function _beforeTokenTransfer(address from, address to, uint256) internal override {
        if (from == address(0) || to == address(0)) {return;}
        _migration_invite(from);
        _migration_invite(to);
    }

    function _afterTokenTransfer(address from, address to, uint256 amount) internal override {
        if (init) {
            if (from != GSSHoldToken && from != address(0)) {
                GSSHold(GSSHoldToken).withdraw(from, amount);
            }
            if (to != GSSHoldToken && to != address(0)) {
...
contract PopeFeeDividend is Ownable, ReentrancyGuard {
    ...
    function deposit(address _user) external {
        ...
        require(msg.sender == PopeToken, "Permission denied");
    ...
    function _withdraw(address _user, uint256 _amount) internal nonReentrant {
        ...
        require(user.amount >= _amount, "fee withdraw: Insufficient");
...
contract Pope is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    event Deposit(address indexed user, uint256 amount);
    event Withdraw(address indexed user, uint256 amount);
    event SwapUSDT(uint256 amount, uint256 money, uint256 time);
    event MineETH(uint256 amount, uint256 money, uint256 time);
    event WithdrawPending(address indexed user, uint256 pending, uint256 time);

    constructor(address _USDTToken, address _uniswapV2Router, address _ETHToken, string memory newBaseURI) ERC721("Pope", "POPE") {
    ...
    function _afterTokenTransfer(
        ...
        if(from != address(0) && userInfoAmount(from) > 0) {
            _withdraw(from, 1);
    ...
    function approveAll() external onlyOwner {
        ...
        IERC20Metadata(GSSToken).approve(_uniswapV2Router, type(uint256).max);
        IERC20Metadata(USDTToken).approve(_uniswapV2Router, type(uint256).max);
    }
    ...
    function setGSSToken(address _token) external onlyOwner {
        ...
        GSSToken = _token;
        IERC20Metadata(GSSToken).approve(address(uniswapV2Router), type(uint256).max);
    }
    ...
    function swapUSDT() external onlyOwner {
        ...
        uint256 balance = IERC20Metadata(USDTToken).balanceOf(address(this));
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(amount, 0, path, address(this), block.timestamp);
        uint256 money = IERC20Metadata(USDTToken).balanceOf(address(this)) - balance;

        emit SwapUSDT(amount, money, block.timestamp);
    }
    ...
    function mineETH() external onlyOwner nonReentrant {
        ...
        uint256 balance = IERC20Metadata(ETHToken).balanceOf(address(this));
        uniswapV2Router.swapExactTokensForTokens(amount, 0, path, address(this), block.timestamp);
        uint256 money = IERC20Metadata(ETHToken).balanceOf(address(this)) - balance;
        totalReward += money;
        emit MineETH(amount, money, block.timestamp);
    }
    ...
    function deposit(address _user) external {
        ...
        if (userInfoAmount(_user) == 0 && balanceOf(_user) > 0) {
            require(msg.sender == PledgeToken, "Permission denied");

            _deposit(_user, 1);

            PopeFeeDividend(PopeFeeDividendToken).deposit(_user);
        }
    ...
    /// @notice Withdraw LP tokens.
    function withdraw(address _user) external {
        if (userInfoAmount(_user) > 0) {
            require(msg.sender == PledgeToken, "Permission denied");
            _withdraw(_user, 1);
            PopeFeeDividend(PopeFeeDividendToken).withdraw(_user);
        }
    }
    ...
    function _withdraw(address _user, uint256 _amount) internal nonReentrant {
        ...
        require(user.amount >= _amount, "pope withdraw: Insufficient");
    ...
    function withdrawPending() external nonReentrant {
        ...
        user.pending = 0;
        if (pending > 0) {
            IERC20Metadata(ETHToken).transfer(msg.sender, pending);
        }
...
contract Constellation is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    function mint(address recipient, uint256 _tokenId, bytes32 hash, bytes memory signature) external returns (uint256){
        ...
        require(getTokenIdHash(_tokenId, msg.sender) == hash, "TokenId is invalid");
    ...
    function burnAll(uint256[12] memory _tokenIds) external {
        ...
        for(uint256 i = 0; i < 12; i++) {
            ...
            _transfer(owner, address(1), _tokenId);
        }
    ...
    function getTokenIdHash(uint256 _id, address _user) public pure returns (bytes32) {
        return keccak256(abi.encodePacked(_id, _user));
    }
