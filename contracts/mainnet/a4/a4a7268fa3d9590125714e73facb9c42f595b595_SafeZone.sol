...
contract SafeZone is ERC20, Ownable {
    ...
    uint256 public totalBuyBackTokens;
    ...
    uint256 private rewardsBuyFee = 2;
    uint256 private rewardsSellFee = 2;
    uint256 private rewardsTransferFee = 2;
    ...
    uint256 private buybackBuyFee = 2;
    uint256 private buybackSellFee = 2;
    uint256 private buybackTransferFee = 2;

    uint256 private marketingBuyFee = 2;
    uint256 private marketingSellFee = 2;
    uint256 private marketingTransferFee = 2;

    uint256 private devBuyFee = 1;
    uint256 private devSellFee = 1;
    uint256 private devTransferFee = 1;

    uint256 private reserveBuyFee = 0;
    uint256 private reserveSellFee = 5;
    uint256 private reserveTransferFee = 3;
    
    uint256 public totalBuyFees = rewardsBuyFee.add(liquidityBuyFee).add(burnBuyFee).add(marketingBuyFee).add(devBuyFee).add(reserveBuyFee).add(buybackBuyFee);
    uint256 public totalSellFees = rewardsSellFee.add(liquiditySellFee).add(burnSellFee).add(marketingSellFee).add(devSellFee).add(reserveSellFee).add(buybackSellFee);
    uint256 public totalTransferFees = rewardsTransferFee.add(liquidityTransferFee).add(burnTransferFee).add(marketingTransferFee).add(devTransferFee).add(reserveTransferFee).add(buybackTransferFee);
    ...
    constructor() ERC20("SafeZone", "SafeZone") {
        ...
        marketingWallet = 0x764Ef0962198d529c324b8b9d5Fb644397D7B21C;
        devWallet = 0x221D01bf9AC0323D9AEd09aE599e8df0AFaefC41;
        reserveWallet = 0x7D3B00d25db5c1b66A0F68999e8124bdDB9B9a89;
        liquidityWallet = owner();

        dividendTracker.setDividendTokenAddress(address(this));
        ...
        excludeFromDividend(_uniswapV2Pair);
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        ...
        require(newDividendTracker.owner() == address(this), "must be owned by Main Token Contract");
    ...
    function processDividendTracker(uint256 gas) external onlyOwner {
        ...
        (uint256 rumIterations, uint256 rumClaims, uint256 rumLastProcessedIndex) = dividendTracker.process(gas);
        emit ProcessedDividendTracker(rumIterations, rumClaims, rumLastProcessedIndex, false, gas, tx.origin);
    }
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            if (canSwap && !automatedMarketMakerPairs[from]) {
                ...
                swapTokensForBuyback(countBuybackFee);

                transferDividends(countRewardsFee);


                swapTokensForDevBNB(countDevFees);

                swapTokensForReserveBNB(countReserveFee);

                swapTokensForMarketingBNB(balanceOf(address(this)));
                 ...
                 uint256 buyBackBalanceBnb = address(this).balance;
                    ...
                    buyBackTokens(buyBackBalanceBnb);
            ...
            uint256 fees = marketingFeeAmount + devFeeAmount + liquidityFeeAmount + rewardsFeeAmount + burnFeeAmount + buybackFeeAmount + reserveFeeAmount;
    ...
    function executeBurn(uint256 amount) private {
    ...
    function swapTokensForDevBNB(uint256 tokenAmount) private {
    ...
    function swapTokensForReserveBNB(uint256 tokenAmount) private {
        if(tokenAmount <= 0){
            return;
        }
    ...
    function swapTokensForMarketingBNB(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
    ...
    function swapTokensForBuyback(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
    ...
    function buyBackTokens(uint256 amount) private {
        ...
        uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: amount}(
            ...
            path,
            marketingWallet, // receiver address
            block.timestamp.add(300)
        );

        uint256 swappedBalance = balanceOf(marketingWallet).sub(initialBalance);
        totalBuyBackTokens = totalBuyBackTokens.add(swappedBalance); 
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private {
        ...
        countLiquidityFees -= contractTokenBalance;
    ...
    function transferDividends(uint256 tokens) private {
