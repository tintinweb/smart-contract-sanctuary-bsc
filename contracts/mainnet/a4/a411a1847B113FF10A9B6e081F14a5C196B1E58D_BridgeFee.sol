...
contract BridgeFee is Ownable, IBridgeFee {
    ...
    function transfer(
        ...
        token.transferFrom(_msgSender(), dcrmAddress, bridgeAmount);
        token.transferFrom(_msgSender(), feeAddress, feeAmount);
        emit BridgeDone(_msgSender(), dcrmAddress, tokenAddress, bridgeAmount);
    }
    ...
    function transfer(address dcrmAddress) external payable {
        ...
        payable(dcrmAddress).transfer(bridgeAmount);
        payable(feeAddress).transfer(feeAmount);
        emit BridgeDone(_msgSender(), dcrmAddress, address(0), bridgeAmount);
    }
