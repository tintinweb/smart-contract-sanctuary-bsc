...
contract NftFactoryUpgradeable is Initializable, OwnableUpgradeable {
    struct FactoryInfo {
        ...
        uint32 mintCount; //how many times production has been completed for this factory
    }
    ...
    uint256 public boostPercentPerShard; // 1000 = 10.00%
    uint256 public maxBoosts; //total boost points pre production
        ...
        returns (uint256[] memory, uint32[] memory, uint8[] memory)
    {
        ...
        for (uint256 x; x < tokenIds.length; ++x) {
            ...
            boosts[x] = nftToBoosts[tokenIds[x]];
        }
    ...
    function startOrBoostFactory(
        ...
        uint8 points = powershardPoints(powershardId);
        if (points == 0) {
        ...
        nftToBoosts[tokenId] = uint8(
            nftToBoosts[tokenId] + points > maxBoosts
                ? maxBoosts
                : nftToBoosts[tokenId] + points
        );
    ...
    function _performClaim(
        ...
        emit PerformClaim(owner, tokenId, nftTypeToMint);
    }
}
