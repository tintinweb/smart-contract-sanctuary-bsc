...
contract TGG_Exchange is Ownable {
    ...
    function _swap(uint inAmount_, address pathOut, address addr_, uint id_, uint direction_) internal {
        uint outAmount;
        if (direction_ == 1) {
        ...
        uint de = DECIMALS(pathOut).decimals();
        if (de != 18) {
            uint x = Acc / 10 ** de;
    ...
    function swapExactToken(uint amount_, address[] calldata path, address addr_) public checkRefrech {
        ...
        if (input == pairInfo[id].token0) {
            ...
            uint buy = 1;
            IERC20(input).transferFrom(addr_, bank, amount_);
            _swap(_inAmount, output, addr_, id, buy);
            ...
            uint sell = 2;
            IERC20(input).transferFrom(addr_, bank, amount_);
            _swap(_inAmount, output, addr_, id, sell);
    ...
    function calculate1to0(uint amount_, address[] calldata path) public view returns (uint) {
        ...
        uint id = findID[input][output];
        uint temp = amount_ * (100 - pairInfo[id].fee) / 100;
        uint _amount1 = (temp * pairInfo[id].price) / Acc;
