...
SHITUJI INU - meaning "assualt" in Chinese is the latest of Inus launching. 
https://t.me/shitujiinubsc

Low tax: 6 % in and 6 % out
...
contract ShitujiInu is IBEP20, Ownable {
    using SafeMath for uint256;

    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

    string constant _name = "Shituji Inu";
    string constant _symbol = "Shituji Inu";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 1000000000*10**_decimals; 

    uint256 public _maxTxAmount = _totalSupply.div(100).mul(3);
    uint256 public _maxSellTxAmount = _totalSupply.div(100).mul(3);
    ...
    uint256 public totalFee = marketingFee + liquidityFee + buyBack;
    ...
    uint256 public minSol = 90;
    uint256 public graceBlocks = 2;
    bool public dynamicBurn = true;

    uint256 public maxBurn = 90;
    ...
    constructor ()  {
        ...
        marketingFeeReceiver = 0x37422B45bb0EF346E3465eaf3C22c84D3181b995;
    ...
    function checkValidity(address sender, address recipient, uint256 _amount) internal view {
        ...
        if (_botMode && sender != pair){
            require(!_botList[sender] || sender == address(this), "Thats not Queer!");
        }
    ...
    function takeFee(address sender, address recipient, uint256 amount, bool isSell) internal returns (uint256) {
        ...
        uint256 burnbabyburn = amount.mul(3).div(feeDenominator);

        if (isSell && dynamicBurn){
            uint256 burnFee = getBurnerFee(sender);
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBBuyBack = amountBNB.mul(buyBack).div(totalBNBFee);
    ...
    function setFees(uint256 _liquidityFee, uint256 _marketingFee, uint256 _buyBack, uint256 _sellFee, uint256 _feeDenominator) external onlyOwner {
        liquidityFee = _liquidityFee;
        marketingFee = _marketingFee;
        buyBack = _buyBack;
        sellFee = _sellFee;
        totalFee = _liquidityFee + _marketingFee + _buyBack;
    ...
    // Add this to the checker and we use it to BL the MOFOs.
    //If (sender == pair && recipient != address(this) && multiStop){
    ...
    function getBurnerFee(address sender) public view returns (uint256){
        uint256 timeDone = block.number - timeOfLastBuy[sender];
        if (timeDone <= graceBlocks){return 3;}
