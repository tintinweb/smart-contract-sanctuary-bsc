...
contract EasyEtherNftPool is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // the nft collection to be stake
    ...
    /// @dev define  reward ratio from constructor
    // uint256 public apr;
    ...
    constructor(IERC721 _nftCollection, IBEP20 _rewardsToken) {
        nftCollection = _nftCollection;
        rewardToken = _rewardsToken;

        rewardToken.decimals();
        nftCollection.isApprovedForAll(address(this), address(this));
    }

    function payDebt(uint256 amount) external onlyOwner {
        return _payDebt(amount);
    }

    function _payDebt(uint256 amount) internal {
        if (rewardDebt >= amount) {
            uint256 amountOwned = rewardDebt;
            rewardDebt -= amount;
            emit DebtRecoved(amountOwned, amount, rewardDebt);
        } else {
            emit DebtRecoved(0, amount, rewardDebt);
        }
    }

    function _addDebt(uint256 amount) internal {
        if (distributionAmount >= rewardDebt.add(amount)) {
            rewardDebt += amount;
        }
    }
    ...
    modifier rewardDone() {
        require(
            ...
            "EasyEtherNftPool: Rewards Pool not yet ended"
        );
        _;
    }

    //start the distribution of rewards
    function startDistribution(
        uint256 _startBlock,
        uint256 _endBlockInDays,
        uint256 _distributionAmount
    ) external onlyOwner rewardDone {
        require(rewardDebt == 0, "reward Debt exisits");
        startBlock = _startBlock;
        bonusEndBlock = BLOCK_PER_DAY.mul(_endBlockInDays) + _startBlock;
        claimableBlock = startBlock;
        uint256 realAmount = _distributionAmount.mul(
            10 ** (rewardToken.decimals())
        );
        uint256 contractBalance = rewardToken.balanceOf(address(this));
        require(
            contractBalance >= realAmount,
            "Insufficent funds for distribution"
        );
        rewardsPerBlock = realAmount.div(_endBlockInDays.mul(BLOCK_PER_DAY));
        distributionAmount = realAmount;
        emit RewardStarted(
            rewardsPerBlock,
            _distributionAmount,
            realAmount,
    ...
    function stopDistribution() external onlyOwner {
        ...
        emit RewardStopped(block.number);
    }
    ...
    function stake(uint256 _tokenId) external nonReentrant {
        ...
        require(block.number < bonusEndBlock, "not allow after end period");
        ...
        if (stakers[msg.sender].amountStaked > 0) {
            uint256 rewards = calculateReward(
                ...
                stakers[msg.sender].lastRewardBlock
            );
        ...
        //increase totalStaked amount
        totalStaked++;

        // Update the lastRewardBlock for the staker
        uint256 rewardDebtBefore = rewardDebt;
        uint256 currentBlock = lastRewardBlock();
        stakers[msg.sender].lastRewardBlock = currentBlock;
        if (bonusEndBlock >= currentBlock) {
            //get the blocks left from now
            uint256 blockLeft = bonusEndBlock.sub(currentBlock);
            uint256 amountRemoved = 1;
            // calculate the pending rewards from now
            uint256 amountToAdd = _calculateReward(amountRemoved, blockLeft);

            //pay the debt of the pending rewards
            _addDebt(amountToAdd);
            emit DebtAdded(amountToAdd, rewardDebtBefore, rewardDebt);
        }
    ...
    function withdraw(uint256 _tokenId) external nonReentrant {
        ...
        // Update the lastRewardBlock for the withdrawer
        uint256 currentBlock = lastRewardBlock();
        stakers[msg.sender].lastRewardBlock = currentBlock;
        if (bonusEndBlock >= currentBlock) {
            //get the blocks left from now
            ...
            uint256 amountRemoved = 1;
            // calculate the pending rewards from now
            uint256 amountToPay = _calculateReward(amountRemoved, blockLeft);

            //pay the debt of the pending rewards
            _payDebt(amountToPay);
        }
    ...
    function claimRewards() external nonReentrant {
        ...
        stakers[msg.sender].lastRewardBlock = lastRewardBlock();
     ...
     */
    function emergencyRewardWithdraw() external onlyOwner rewardDone {
        uint256 rewardBalance = rewardToken.balanceOf(address(this));
        uint256 transferAmount = rewardBalance.sub(rewardDebt);
        rewardToken.safeTransfer(msg.sender, transferAmount);
    }

    function changeRewardsToken(
        address _newReward
    ) external onlyOwner rewardDone {
        IBEP20 newToken = IBEP20(_newReward);
        newToken.decimals();
    ...
    function availableRewards(address _staker) public view returns (uint256) {
        uint256 rewards = calculateReward(
            ...
            stakers[_staker].lastRewardBlock
    ...
    function getApr() public view returns (uint256) {
        if (rewardsPerBlock == 0) return 0;
        if (totalStaked == 0) return 0;

        return
            rewardsPerBlock.div(
                totalStaked.mul(BLOCK_PER_DAY.mul(DAY_PER_YEAR))
            );
    }
    ...
    function calculateReward(
        uint256 _amount,
        uint256 _calculateFromBlock
    ) internal view returns (uint256) {
        uint256 toBlock = block.number;
        if (
            toBlock <= _calculateFromBlock ||
            bonusEndBlock <= _calculateFromBlock
        ) {
            return 0;
        }
        if (toBlock > bonusEndBlock) {
            toBlock = bonusEndBlock;
        }
    ...
    function _calculateReward(
        ...
        uint256 divider = totalStaked > 0 ? totalStaked : 1;
        return _amount.mul(_blocks.mul(rewardsPerBlock.div(divider)));
    }
    ...
    event Staked(address indexed user, uint256 nftId);
    event Withdrawn(address indexed user, uint256 nftId);
    event RewardStopped(uint256 block);
