...
contract MultiCallSacha{
    ...
    function get_all_pairs_full(address[] memory factorys) external view returns(address[] memory,address[] memory,address[] memory,string[] memory,string[] memory){//return : Lp, token0 address, token1 address, token0 symbol, token1 symbol
        uint len = 0;
        uint[] memory lens = new uint[](factorys.length);
        for(uint i = 0; i<factorys.length; i ++ ){
            lens[i] = UniswapFunctions(factorys[i]).allPairsLength();
            len = lens[i] + len;
        }
        ...
        for(uint i = 0; i < len; i++){
            ...
            for(uint j = 0; j<factorys.length; j++){//cette boucle va juste me trouver la bonne factory
                if(j==0 && i<lens[j] ){//si on est dans la première 
                    factory = factorys[j];
                }else if(j>0 && i>=lens[j] && i <lens[j]){
                    factory = factorys[j];
                }
            }
            //on met comme contract le factory du broker donné
