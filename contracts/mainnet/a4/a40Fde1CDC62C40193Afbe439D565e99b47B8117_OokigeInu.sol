...
library Address{
 ...
 contract OokigeInu is Context, IERC20, Ownable {
    ...
    uint256 public maxBuyLimit = 30000000 * 10**9;
    uint256 public maxSellLimit = 30000000 * 10**9;
    uint256 public maxWalletLimit = 50000000 * 10**9;
    
    uint256 public genesis_block;
    
    address public marketingWallet = 0xc48C1F29F75d80DA464852dee3b7BD6F471345bf;
    address public reward = 0xc48C1F29F75d80DA464852dee3b7BD6F471345bf;

    string private constant _name = "OokigeInu";
    string private constant _symbol = "Ookige";
    ...
    Taxes public sellTaxes = Taxes(0, 5, 3, 0);
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[reward] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[marketingWallet] = true;
        allowedTransfer[reward] = true;
    ...
    function setTaxes(uint256 _reward, uint256 _marketing, uint256 _liquidity, uint256 _development) public onlyOwner {
       taxes = Taxes(_reward,_marketing,_liquidity,_development);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _reward, uint256 _marketing, uint256 _liquidity, uint256 _development) public onlyOwner {
       sellTaxes = Taxes(_reward,_marketing,_liquidity,_development);
        emit FeesChanged();
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rreward, to_return.rMarketing, to_return.rLiquidity) = _getRValues1(to_return, tAmount, takeFee, _getRate());
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.tTransferAmount = tAmount-s.treward-s.tMarketing-s.tLiquidity-s.tdevelopment;
        return s;
    }

    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rreward,uint256 rMarketing, uint256 rLiquidity){
        ...
        rTransferAmount =  rAmount-rreward-rMarketing-rLiquidity-rdevelopment;
        return (rAmount, rTransferAmount, rreward,rMarketing,rLiquidity);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rreward > 0 || s.treward > 0) _reflectreward(s.rreward, s.treward);
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(developmentAmt > 0){
            payable(reward).sendValue(developmentAmt);
        }
    }
    ...
    function updatereward(address newWallet) external onlyOwner{
        reward = newWallet;
    }
