...
contract BuyandSell is Context, ReentrancyGuard {
    ...
    external
    nonReentrant {
        uint256 tokenamount = TokenAmount * 10 ** _token.decimals();
        require(_token.balanceOf(address(this)) >= tokenamount, "Not enough token in contract.");
        require(tokenamount >= minimumBuyAmount, "Amount cannot be zero.");
        require(tokenamount <= maximumBuyAmount, "Amount should be less than maximum buy amount.");
    ...
    function sell(uint256 SellTokenAmount) external nonReentrant {
        ...
        require(sellAmount >= minimumsellAmount, "Minimum token need to be sold.");
        require(sellAmount <= maximumsellAmount, "Maximum token need to be sold.");
        ...
        _token.transferFrom(address(this), msg.sender, sellAmount);
