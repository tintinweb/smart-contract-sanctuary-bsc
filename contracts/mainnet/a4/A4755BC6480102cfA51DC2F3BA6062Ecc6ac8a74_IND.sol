...
contract IND is ERC20Permit, VaultOwned {
    ...
    address public bondReward;
    address public stakeReward;
    address public pair;
    uint256 public fee = 20;
    uint256 public bondFee = 30;
    uint256 public stakeFee = 30;
    ...
    constructor() ERC20("Infinity DAO", "IND", 9) {
        ...
        feeAddress = 0xA067054d7Ef642d4c0Da0fd7D2D6ab37DbC2A4FD;
        bondReward = 0x5Cc30C39f2817c266908c6456295383d4714D8b4;
        stakeReward = 0x5Cc30C39f2817c266908c6456295383d4714D8b4;
        _mint(msg.sender,11744949395830000);
        swaped = true;
    }
    ...
    function setBondReward(address _address) external onlyOwner(){
        bondReward = _address;
    }

    function setStakeReward(address _address) external onlyOwner(){
        stakeReward = _address;
    }
    ...
    function setSwaped() external onlyOwner() {
        swaped = !swaped;
    }

    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
      if(swaped == true && whiteListed[recipient] == false && msg.sender == pair)
      revert("not swap");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        if(swaped == true && whiteListed[recipient] == false && sender == pair)
        revert("not swap");
        uint256 tAmount = amount;
    ...
    returns (uint256)
    {
        ...
        uint256 bondAmount = amount.mul(bondFee).div(feeDenominator);
        _transfer(sender, bondReward, bondAmount);
        uint256 stakeAmount = amount.mul(stakeFee).div(feeDenominator);
        _transfer(sender, stakeReward, stakeAmount);


        return amount.sub(feeAmount).sub(bondAmount).sub(stakeAmount);
    }
