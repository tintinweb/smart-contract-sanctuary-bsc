...
// SPDX-License-Identifier: Unlicensed
    ...
    contract CYFpreminterclaims is ReentrancyGuard {
        ...
        BSCYFieldv5 internal immutable BSCYFv5 = BSCYFieldv5(0x1bE690adeF6EE315B3906cc0724Bd3A6E6598db4);
        ...
        function claimMinterPack() public{
            uint256 allocation_private;
            uint256 allocation_presale;
            uint256 allocation_public;
            address _user = msg.sender;
            require(msg.sender != address(0x493323C1f45caC0ceC5C151347681CA969De71AE), 'Banned!');
            require(!minters[_user].claimed_private || !minters[_user].claimed_presale || !minters[_user].claimed_public, 'NotAllowed');
            // Get allocation from privateSales
            if(!minters[_user].claimed_private){
                minters[_user].claimed_private = true;
                if(PrivateSaleBuyers[_user] > 0){
                    allocation_private = ( PrivateSaleBuyers[_user] / (TOKEN_START_PRICE_USD / 5) ) * 10 ** 18;
                }
            }
            // Get allocation from presale
            if(!minters[_user].claimed_presale){
                minters[_user].claimed_presale = true;
                allocation_presale = isPresaleBuyer(_user);
            }
            // Get Allocation from Public Sales
            if(!minters[_user].claimed_public){
                minters[_user].claimed_public = true;
                allocation_public = isPublicsaleBuyer(_user);
            }
            minters[_user].dateClaimed = block.timestamp;
            minters[_user].allocation += allocation_public + allocation_presale + allocation_private;
        }
        ...
        function claimCommunityReward() public {
            // Every bscyf staker get a one time special free [ 1m CYF minter packs ]
            address _user = msg.sender;
            require(msg.sender != address(0x493323C1f45caC0ceC5C151347681CA969De71AE), 'Banned!');
            bool _isStaker = isStaker(_user);
            require(_isStaker, 'NotAllowed');
            require(!minters[_user].claimed_one, 'NotAllowed');
            minters[_user].claimed_one = true;
            minters[_user].dateClaimed = block.timestamp;
            minters[_user].allocation += 1000000 * 10 ** 18;
        }

        function claimFromStake() public {
            address _user = msg.sender;
            uint256 allocation_;
            bool _isStaker = isStaker(_user);
            if(!minters[_user].claimed_farm && _isStaker){
                ...
                if(_deposit > 0){
                    ...
                    if(totalAmount > 0){
                        allocation_ = (totalAmount / TOKEN_START_PRICE_BNB) * 10 ** 18;
                    }
                }
            }
            // Claim from BSCYFv5
            uint256 claimed = minters[_user].allocation_farm;
            uint256 sallocation_ = isSpecialBuyer(_user);
            uint256 fallocation_ = sallocation_ - claimed; // shows any additional token
            if(sallocation_ > 0 && fallocation_ > 0){
                allocation_ += fallocation_;
                minters[_user].allocation_farm += fallocation_;
            }
            minters[_user].dateClaimed = block.timestamp;
            minters[_user].allocation += allocation_;
        }
        ...
        // Get Presale Sales Allocation
        function isSpecialBuyer(address _user) private view returns(uint256 _allocation){
            _allocation = BSCYFv5._tokensPurchased(_user);
        }
