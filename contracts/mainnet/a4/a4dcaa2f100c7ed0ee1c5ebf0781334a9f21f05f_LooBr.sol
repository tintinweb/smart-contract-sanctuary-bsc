...
contract DividendDistributor is IDividendDistributor {
    ...
    constructor(address rewardToken) {
        require(rewardToken != address(0), "reward token is 0 address");
        _token = msg.sender;
...
contract LooBr is IERC20, Ownable, ReentrancyGuard{
    using SafeMath for uint256;
    ...
    bool public enableantibot = true;
    ...
    event ClearedStuckBalance(uint256 amount);
    event SwapTokenChanged(address newToken);
    event TaxesUpdated(uint256 buyFee, uint256 sellFee, uint256 transferFee);
    event UpdatedDistributionFees(
        uint256 marketingFee,
        uint256 developerFee,
        uint256 rewardfee,
        uint256 burnFee
    );
    event UpdateDistributorGas(uint256 newGas);
    ...
    constructor(address _pinkandtibot) {
        pinkAntiBot = IPinkAntiBot(_pinkandtibot);
    ...
    function clearStuckBalance(uint256 amountPercentage) external onlyOwner {
        ...
        emit ClearedStuckBalance(amountBNB);
    }
    ...
    function changeSwapToken(address token) external onlyOwner {
        ...
        SWAPTOKEN = token;
        emit SwapTokenChanged(token);
    }

    function updateTaxesFees(uint256 _buyFee, uint256 _sellFee, uint256 _transferFee) external onlyOwner {
        buyFee = _buyFee;
        sellFee = _sellFee;
        transferFee = _transferFee;
        require(buyFee <= 20 && sellFee <= 20 && transferFee <= 20, "max fee exceeds");
        emit TaxesUpdated(_buyFee, _sellFee, _transferFee);
    }

    /**
     * @dev total amount of accumulated token from buy and sell fees
     * will be distributed among these topics  
     */
    function updateFees(
        ...
        uint256 totalFee = marketingFee.add(developerFee).add(rewardFee).add(burnFee);
        ...
        burnFee = _burnFee;
        emit UpdatedDistributionFees(marketingFee, developerFee, rewardFee, burnFee);
    }
    ...
    function switchToken(address rewardToken, bool isIncludeHolders) external onlyOwner {
        ...
        if (isIncludeHolders) {
            ...
            for (uint256 i = 0; i < currentHolders.length; i++) {
                dividendDistributor.setShare(currentHolders[i],_balances[currentHolders[i]]);
            }
        }
    ...
    function swapBackInBnb() internal swapping {
        ...
        uint256 tokensToSwap = contractTokenBalance.sub(tokensToReward).sub(tokensToBurn);
        // swap the tokens
        if(tokensToSwap > 0) {
        ...
        if(tokensToBurn > 0) {
            ...
            _balances[DEAD] = _balances[DEAD].add(tokensToBurn);
            emit Transfer(address(this), DEAD, tokensToBurn);
        }

        if (marketingFeeBnb > 0) {
            payable(marketingFeeReceiver).transfer(marketingFeeBnb);
        }

        if (devFeeBnb > 0) {
            payable(devFeeReceiver).transfer(devFeeBnb);
        }
    }
    ...
    function swapBackInTokens() internal swapping {
        ...
        uint256 tokensToBurn = burnFee != 0 ? contractTokenBalance.mul(burnFee).div(100) : 0;
        uint256 tokensForFee = contractTokenBalance.sub(rewardTokens).sub(tokensToBurn);
    ...
    event UpdateSwapBackSettings(bool _enabled, uint256 _amount);
    ...
    event DistributionCritearea(uint256 _minPeriod, uint256 _minDistribution);
    function setDistributionCriteria( uint256 _minPeriod, uint256 _minDistribution) external onlyOwner {
        dividendDistributor.setDistributionCriteria(
            _minPeriod,
            _minDistribution
        );
        emit DistributionCritearea(_minPeriod, _minDistribution);
    }

    function setDistributorSettings(uint256 gas) external onlyOwner {
        require(gas < 750000);
        distributorGas = gas;
        emit UpdateDistributorGas(gas);
    }

    event EnableAntibot(bool _enable);
    function enableAntibot(bool _enable) external onlyOwner {
        enableantibot = _enable;
        emit EnableAntibot(_enable);
    }
