...
version 1.3
contract name: TresLechesCakeTokenStakingV2
*/
...
// will be transferred to a governance smart contract once TresLechesCake is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract TresLechesCakeTokenStakingV2 is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    // Info of each user.
    struct UserInfo {
        uint256 amount;         // How many TresLechesCake tokens the user has provided.
        uint256 rewardDebt;     // Reward debt. See explanation below.
        uint256 rewardLockedUp;  // Reward locked up.
        uint256 lastDepositTime; // when user deposited

        //
        // We do some fancy math here. Basically, any point in time, the amount of TresLechesCake
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accTokenPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws TresLechesCake tokens to a pool. Here's what happens:
    ...
    // Info of each pool.
    struct PoolInfo {
        uint256 allocPoint;         // How many allocation points assigned to this pool. TresLechesCake to distribute per block.
        uint256 lastRewardBlock;    // Last block number that TresLechesCake distribution occurs.
        uint256 accTokenPerShare;   // Accumulated TresLechesCake per share, times 1e12. See below.
    ...
    // The TresLechesCake TOKEN!
    IBEP20 public token;
    // TresLechesCake tokens created per block.
    uint256 public tokenPerBlock;
    // Info of each pool.
    PoolInfo[] public poolInfo;
    // Info of each user that stakes TresLechesCake tokens.
    ...
    constructor() {
        token = IBEP20(0x97B846DB4E521A07185d3da4408a12897316b618); //Tres Leches Cake Production
    ...
    // Update the given pool's TresLechesCake allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending TresLechesCake on frontend.
    ...
    // View function to see if user can harvest TresLechesCake.
    ...
    // Withdraw TresLechesCake tokens from Farm.
    ...
    // Pay or lockup pending TresLechesCake.
