...
contract HTCTEST is Context, Ownable, IBEP20, BEP20Detailed {
  ...
  using SafeMath for uint256;
    ...
    uint256 public minimumLotteryAmount = 10000 * (10**8);
    uint256 public NumberOfLotteryEligibles;
    uint256 public lastDrawTime = block.timestamp;
    uint256 public drawLapse = 604800;
   // address public BNB = address(0xc778417E063141139Fce010982780140Aa0cD5Ab); //BNB testnet
   //address public BNB = address(0xB8c77482e45F1F44dE1745F52C74426C631bDD52); //BNB mainnet
     address public BNB = address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c); //WBNB mainnet
     //address public BNB = address(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd); //WBNB testnet
    uint256 public count;
    uint256 public rand;
    address public winner;

    address payable public devAddress = payable(0x508a82DE9c52167fb02E384DE78C85522Ed5977a); 
    address payable public lotteryAddress = payable(0x0c174d92ECa5b811eaeC9932954960d31401FB77);  
     

    bool inSwapAndLiquify;
    bool public swapAndLiquifyEnabled = true;

    uint256 public numTokensSellToAddToLiquidity = 1000 * (10**8);
    ...
    constructor () BEP20Detailed("HASHTAG TOKEN", "HTC", 8) {
      _owner = msg.sender ;
	...
	_balances[_owner] = _totalSupply;
	 ...
	 IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        ...
        _isExcludedFromFee[lotteryAddress] = true;

        _isExcludedFromLottery[owner()] = true;
        _isExcludedFromLottery[address(this)] = true;
        _isExcludedFromLottery[devAddress] = true;
        _isExcludedFromLottery[lotteryAddress] = true;
        _isExcludedFromLottery[uniswapV2Pair] = true;
        _isExcludedFromLottery[address(uniswapV2Router)] = true;
    ...
    function setDevFeePercent(uint256 updatedDevFee) external onlyOwner {
        devFee = updatedDevFee;
    }
    ...
    {
        devAddress = wallet;
        _isExcludedFromLottery[wallet] = true;
    }

    function setLotteryAddress(address payable wallet) external onlyOwner
    {
    ...
    function changeDrawLapse(uint256 lapse) external onlyOwner
    {
        drawLapse = lapse;
    }

    function changeMinimumLotteryAmount(uint256 amount) external onlyOwner
    {
        minimumLotteryAmount = amount;
    }
    ...
    function _transfer(address sender, address recipient, uint amount) internal{
        ...
        require(recipient != address(0), "BEP20: transfer to the zero address");
         ...
         uint256 taxAmount = (amount.mul(devFee + lotteryFee + liquidityFee)).div(100);
       ...
       if(!_isExcludedFromLottery[recipient])
        {
        if(_balances[recipient] + amount >= minimumLotteryAmount)
        {
    ...
    function showEligibles() external onlyOwner view returns(address[] memory)
    {
        return lotteryEligibles;
    }


    function random(uint256 _NumberOfLotteryEligibles) public view returns(uint256){
    return uint(keccak256(abi.encodePacked(block.timestamp, block.difficulty, lotteryEligibles))) % _NumberOfLotteryEligibles;
}

    function totalFees() internal view returns(uint256)
    {
        return devFee.add(lotteryFee).add(liquidityFee);
    }
    ...
    function swapAndLiquify(uint256 tokensToLiquify) private lockTheSwap {
        ...
        uint256 ethFeeFactor = totalFees().sub((liquidityFee).div(2));

        uint256 ethForLiquidity = ethBalance.mul(liquidityFee).div(ethFeeFactor).div(2);
        uint256 ethForDev = ethBalance.mul(devFee).div(ethFeeFactor);
     //   uint256 ethForLottery = ethBalance.sub(ethForDev.add(ethForLiquidity));
     
        addLiquidity(tokensToLP, ethForLiquidity);

        payable(devAddress).transfer(ethForDev);

         if(block.timestamp > lastDrawTime + drawLapse)
        { 
            rand = random(count);
