...
contract MSystemstake is Ownable {
    ...
    // Fees address...
    address public devAddress = 0x4d38B18002B0c83D99eD9DBe65e37404db9B9ACC;
    
    // MSystem token contract...
    address public MSystem = 0x7FAE8B4Df1A2b09f6Ee3AAcc4b621203FBeCf977;
    
    // reward rate 100 % per year
    uint256 public rewardRate = 300e2;
    ...
    function updateAccount(address account) private {
        ...
        uint256 conbalance = Token(MSystem).balanceOf(address(this));
        ...
        if (pendingDivs != 0) {
            ...
            Token(MSystem).transfer(account, pendingDivs);
    ...
    function deposit(uint256 amountToStake, uint256 _timeperiod) public {
       ...
       require(_timeperiod == 7 || _timeperiod == 15, "Invalid Time Period. it must be either 7 or 15"); 
        ...
        Token(MSystem).transferFrom(msg.sender, address(this), amountToStake);
    ...
    function withdraw(uint256 amountToWithdraw) public {
        ...
        if (_lastClaimedTime >= MinimumWithdrawTime) {
            require(Token(MSystem).transfer(msg.sender, amountToWithdraw), "Could not transfer tokens.");
        }
        
        if (_lastClaimedTime < MinimumWithdrawTime) {
            uint256 WithdrawFee = amountToWithdraw.div(1e3).mul(750);
            uint256 amountAfterFee = amountToWithdraw.sub(WithdrawFee);
            require(Token(MSystem).transfer(msg.sender, amountAfterFee), "Could not transfer tokens.");
            require(Token(MSystem).transfer(devAddress, WithdrawFee), "Could not transfer tokens.");
        }}
    ...
    function transferAnyERC20Tokens(address _tokenAddress, address _to, uint256 _amount) public onlyOwner {
        require(_tokenAddress != MSystem, "You can't transfer MSystem token.");
