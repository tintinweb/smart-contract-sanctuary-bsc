...
//PREDICTIONS
contract WickWizardV01 is Ownable, Pausable, ReentrancyGuard, ExtraModifiers, EtherTransfer {
    struct Round {
        uint256 epoch;
        uint256 bullAmount;
        uint256 bearAmount;
    ...
    uint256 public rewardRate = 975;                 // Percents
    ...
    function _CheckPlayRequirements(uint epoch) internal {
        ...
        require(_playable(epoch), "Round not playtable. You might be too early/too late");
        require(msg.value >= minPlayAmount, "Play amount must be greater than minPlayAmount");
        require(msg.value <= maxPlayAmount, "Play amount must be less than maxPlayAmount");
