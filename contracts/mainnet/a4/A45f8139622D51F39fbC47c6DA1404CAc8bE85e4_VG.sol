...
contract SmartVault {
    ...
    IERC20 public vgToken;

    function initialize(IERC20 _usdtToken, IERC20 _vgToken) external {
        require(address(usdtToken) == address(0), "has been initialized");
        usdtToken = _usdtToken;
        vgToken = _vgToken;

        usdtToken.approve(address(vgToken), uint256(-1));
    }
...
contract VG is ERC20, SafeOwnable {
    using SafeMath for uint256;
    ...
    uint256 public fundFeePercent = (6 * BASE_RATIO) / 100;
    ...
    function setFund2(address _fund2) external onlyOwner {
        fund2 = _fund2;
    }
    ...
    function calculateFee(
        ...
        if (
            ...
            uint256 half_smartBalance = smartVaultBalance.div(2);
            uint256 another_half = smartVaultBalance - half_smartBalance;
