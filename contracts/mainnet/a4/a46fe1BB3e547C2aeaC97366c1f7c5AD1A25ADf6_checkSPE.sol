...
contract Pledge is Ownable {
    ...
    uint public rate;
    uint[5] public  cycle = [7 * 60, 15 * 60, 30 * 60, 60 * 60, 90 * 60];
    ...
    function claimStatic(uint slot_, uint mode_) isStart checkTime checkHolder internal returns (uint reward) {
    ...
    function unStake(uint slot_, uint mode_) isStart checkTime checkHolder public {
        ...
        if (mode_ == 0) {
            ...
            if (temp != 0) {
                SpeToken.transfer(msg.sender, temp * 89 / 100);
                userInfo[msg.sender].Claimed += temp;
                _burn(temp / 100);
                // userInfo[msg.sender].lockSpe += temp / 10;
                emit ClaimToWallet(msg.sender, temp);
            }
        }
        else if (mode_ == 1) {
    ...
    function setStatus(bool stake_, bool claimStatic_, bool exchange_, bool stock_, bool comm, bool node) public onlyOwner {
        status = Status({
        stake : stake_,
    ...
    function setAddress(address addr_, address token_) public onlyOwner {
        air = AirDrop(addr_);
    ...
    function _burn(uint amount_) internal {
    ...
    function buyAmount(uint amount_) external {
        ...
        SpeToken.transferFrom(msg.sender, address(0), amount_);
        burnTotal += amount_;
...
contract checkSPE {
    Pledge public main = Pledge(0xb51E2e90cab115DE59Fec19c153CA717fc2fbA48);
    function checkUserSpeSlot(address addr) public view returns(uint[] memory slot_,uint[] memory stakeTime,uint[] memory endTime,uint[] memory stakeAmount){
        uint temp;
         for (uint i = 0; i < 10; i++) {
             (bool status,,,,,) = main.userSlotSPE(addr,i);
            if (status) {
                temp += 1;
            }
        }
        slot_ = new uint[](temp);
        stakeTime = new uint[](temp);
        endTime = new uint[](temp);
        stakeAmount = new uint[](temp);
        uint index;
        for (uint i = 0; i < 10; i++) {
             (bool status_,uint stakeAmount_,,uint stakeTime_,,uint endTime_) = main.userSlotSPE(addr,i);
            if (status_) {
                slot_[index] = i;
                stakeTime[index] = stakeTime_;
                endTime[index] = endTime_;
                stakeAmount[index] = stakeAmount_;
                index ++;
            }
        }
    }
    function checkUserNBTCSlot(address addr) public view returns(uint[] memory slot_,uint[] memory stakeTime,uint[] memory endTime,uint[] memory stakeAmount){
        uint temp;
         for (uint i = 0; i < 10; i++) {
             (bool status,,,,,) = main.userSlotNBTC(addr,i);
            if (status) {
                temp += 1;
            }
        }
        ...
        uint index;
        for (uint i = 0; i < 10; i++) {
             (bool status_,uint stakeAmount_,,uint stakeTime_,,uint endTime_) = main.userSlotNBTC(addr,i);
            if (status_) {
                slot_[index] = i;
                stakeTime[index] = stakeTime_;
                endTime[index] = endTime_;
