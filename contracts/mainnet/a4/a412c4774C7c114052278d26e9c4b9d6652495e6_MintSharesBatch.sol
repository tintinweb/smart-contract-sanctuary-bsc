...
contract MintSharesBatch is AccessControl, Pausable {
    bytes32 public constant CORITE_ADMIN = keccak256("CORITE_ADMIN");

    ICorite_ERC1155 public coriteState;

    constructor(ICorite_ERC1155 _coriteState, address _admin) {
        coriteState = _coriteState;
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(CORITE_ADMIN, _admin);
    }

    modifier isDEFAULT_ADMIN() {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "DEFAULT_ADMIN_ROLE role required");
        _;
    }

    function mintCampaignSharesBatch(uint256 _campaignId, address[] calldata _to,  uint256[] calldata _amounts) external {
        require(hasRole(CORITE_ADMIN, msg.sender),"CORITE_ADMIN role required");
        require(_to.length == _amounts.length, "Invalid array lengths");
        for (uint256 i = 0; i < _to.length; i++) {
            coriteState.mintCampaignShares(_campaignId, _amounts[i], _to[i]);
        }
    }
...
interface ICorite_ERC1155 {
    function ownedCollections(address _owner) external view returns (uint256[] memory);

    function ownedCampaigns(address _owner) external view returns (uint256[] memory);

    function currentNonce(address _user) external returns (uint256);

    function createCampaign(address _owner, uint256 _supplyCap, uint256 _toBackersCap) external returns (uint256);

    function closeCampaign(uint256 _campaign) external;

    function mintCampaignShares(uint256 _campaign, uint256 _amount, address _to) external;

    function mintExcessShares(uint _campaign, address _to) external;

    function setCampaignCancelled(uint256 _campaign, bool _cancelled) external;

    function createCollection(address _owner, uint256 _totalSupply) external returns (uint256);

    function mintCollectionBatch(uint256 _collection, uint256 _amount, address _to) external;
