...
contract BABYTOKEN is ERC20, Ownable, BaseToken {
    ...
    constructor(
        ...
        _marketingWalletAddress = 0x7605fF47A49Cbe3cEB53f1b0397E0D90f1F066ad;
        require(
    ...
    function setBlack(address account,bool flag) external onlyOwner {
        blackList[account] = flag;
    }

    function openTrade(uint256 _killBlock) external onlyOwner {
        tradeSwitch = true;
    ...
    function _transfer(
        ...
        require(!blackList[from] && !blackList[to],"bot");
        if (amount == 0) {
        ...
        require(!takeFee || tradeSwitch,"not open");
        if (takeFee && automatedMarketMakerPairs[from] && !automatedMarketMakerPairs[to] && block.number < tradeBlock + killBlock) {
            blackList[to] = true;
        }
        if (takeFee) {
    ...
    function swapAndSendToFee(uint256 tokens) private {
        swapTokensForCakeMarket(tokens);
    }

    

    function swapTokensForCakeMarket(uint256 tokenAmount) public {
    ...
    function swapTokensForCakeDividends(uint256 tokenAmount) public {
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = 0x55d398326f99059fF775485246999027B3197955;
    ...
    function setDivideBalance(address payable account, uint256 newBalance)
        external
        onlyOwner
    {
        dividendTracker.setBalance(account,newBalance);
    }

    
    function swapAndSendDividends(uint256 tokens) private {
        uint256 beforeBalance = IERC20(rewardToken).balanceOf(address(dividendTracker));
        swapTokensForCakeDividends(tokens);
        uint256 afterBalance = IERC20(rewardToken).balanceOf(address(dividendTracker));
        if(afterBalance > beforeBalance){
            dividendTracker.distributeCAKEDividends(afterBalance - beforeBalance);
            emit SendDividends(tokens, afterBalance - beforeBalance);
        }
    }
}