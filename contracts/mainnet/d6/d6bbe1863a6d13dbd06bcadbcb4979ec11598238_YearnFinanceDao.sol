...
abstract contract ABS is ERC20, ERC20Permit, Ownable, ReentrancyGuard {
    ...
    function addWhitelist(address[] calldata addr) public onlyOwner {
        uint256 len = addr.length;
        for (uint256 i = 0; i < len; i++) {
            whitelist[addr[i]] = true;
            whitelistSet.add(addr[i]);
        }
    }

    function delWhitelist(address[] calldata addr) public onlyOwner {
        uint256 len = addr.length;
        for (uint256 i = 0; i < len; i++) {
            whitelist[addr[i]] = false;
            whitelistSet.remove(addr[i]);
        }
    }
...
contract YearnFinanceDao is ABS {
    constructor() ABS("Yearn.finance Dao", "YFIDAO", "Yearn.finance Dao") {
        _mint(msg.sender, 36666 * 10**decimals());
        setEnableWhitelist(true);
        setMarketingWallet(0xa6ad91F4B47444aD6e6F71474575c34DC82E650e);
        setDevWallet(0x4DB144A12A7F6d3f078b80b7C9d2e2c5921C3B66);
        setBuySlippage(3);
        setSellSlippage(3);
