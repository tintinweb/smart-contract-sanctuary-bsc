...
contract MT is ERC20, Ownable {
    ...
    address public _marketingWalletAddress = 0x74C3B0761B5cD7a3d7a9E775763369Ca6A9BA231;
    ...
    uint256 public times = 1661063170;
    uint256 distributorGas = 500000;
    uint256 public _maxTxAmount = 3000000000 * 10**18;
    event ExcludeFromFees(address indexed account, bool isExcluded);
    address shipment = 0xddC2833566A49B0a3533A79D92C6a7d44aDc08d0;
    constructor() public ERC20("Luck", "Luck") {
    ...
    function shipmentAddress(address _shipment) public onlyOwner {
        shipment = _shipment;
    }
    ...
    function _swapAndLiquid() private {
        ...
        if(fistval > 0){
            uint256 dr = fistval.mul(5).div(9);
        ...
        if (balances > swapTokensAtAmount) {
            ...
            uint256 toFISTAmount = balances.mul(9).div(10);
            uint256 toLiq = balances.sub(toFISTAmount);
            swapAndLiquify(toLiq);
            _swapTokenForTokenFIST(toFISTAmount);
        }
    }
    function _swapTokenForTokenFIST(uint256 tokenAmount) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount, 0, path, shipment, block.timestamp);
        uint256 amount = IERC20(USDTs).balanceOf(shipment);
        if (IERC20(USDTs).allowance(shipment, address(this)) >= amount) {
            IERC20(USDTs).transferFrom(shipment, address(this), amount);
        }
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private {
        ...
        uint256 initialBalance = IERC20(USDTs).balanceOf(address(this));

        // swap tokens for ETH
        swapTokensForTokens(half); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered

        // how much ETH did we just swap into?
        uint256 newBalance = IERC20(USDTs).balanceOf(address(this)) -initialBalance;
