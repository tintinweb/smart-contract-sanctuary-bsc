...
contract DipDetectorBot is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    DipDetectorBotDividendTracker public dividendTracker;
    ...
    uint256 public swapTokensAtAmount = 1_000 * (10**18); //1 thousand. 
    ...
    address payable public _marketingWalletAddress = payable(0xE1395CBDb4D792827002263A02499FCD96AA8fd8);
    address payable public _buybackWalletAddress = payable(0xE1395CBDb4D792827002263A02499FCD96AA8fd8);
    ...
    constructor() BEP20("DipDetectorBot", "DIP") 
    {
        dividendTracker = new DipDetectorBotDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "DipDetectorBot: The dividend tracker already has that address");
        DipDetectorBotDividendTracker newDividendTracker = DipDetectorBotDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "DipDetectorBot: The new dividend tracker must be owned by the DipDetectorBot token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "DipDetectorBot: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "DipDetectorBot: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DipDetectorBot: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "DipDetectorBot: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "DipDetectorBot: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "DipDetectorBot: Cannot update gasForProcessing to same value");
...
contract DipDetectorBotDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("DipDetectorBot_Dividend_Tracker", "DipDetectorBot_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "DipDetectorBot_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "DipDetectorBot_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DipDetectorBot contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DipDetectorBot_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DipDetectorBot_Dividend_Tracker: Cannot update claimWait to same value");
