...
contract StakingVolatile is Initializable, Ownable, ReentrancyGuard
{
	...
	uint256 constant DEFAULT_CLAIM_FEE = 50e16; // 50%
	...
	// updates the claim tax receiver
	...
	function claimReward() external nonReentrant returns (uint256 _rewardAmount)
	{
		...
		if (_rewardAmount > 0) {
			...
			uint256 _feeAmount = _rewardAmount * claimFee / 100e16;
			uint256 _netAmount = _rewardAmount - _feeAmount;
	...
	function claimDrip() external nonReentrant returns (uint256 _dripAmount)
	{
		...
		if (_dripAmount > 0) {
			...
			uint256 _feeAmount = _dripAmount * claimFee / 100e16;
			uint256 _netAmount = _dripAmount - _feeAmount;
			IERC20(reserveToken).safeTransfer(bankroll, _feeAmount);
			IERC20(reserveToken).safeTransfer(msg.sender, _netAmount);
		}
	...
	function claimBoost() external nonReentrant returns (uint256 _boostAmount)
	{
		...
		if (_boostAmount > 0) {
			...
			uint256 _feeAmount = _boostAmount * claimFee / 100e16;
			uint256 _netAmount = _boostAmount - _feeAmount;
			IERC20(reserveToken).safeTransfer(bankroll, _feeAmount);
			IERC20(reserveToken).safeTransfer(msg.sender, _netAmount);
		}
	...
	function claimAll() external nonReentrant returns (uint256 _rewardAmount, uint256 _dripAmount, uint256 _boostAmount)
	{
		...
		if (_rewardPlusDripPlusBoostAmount > 0) {
			uint256 _feeAmount = _rewardPlusDripPlusBoostAmount * claimFee / 100e16;
			uint256 _netAmount = _rewardPlusDripPlusBoostAmount - _feeAmount;
			IERC20(reserveToken).safeTransfer(bankroll, _feeAmount);
			IERC20(reserveToken).safeTransfer(msg.sender, _netAmount);
		}
