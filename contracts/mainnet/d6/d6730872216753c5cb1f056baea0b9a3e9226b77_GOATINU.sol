...
contract GOATINU is Context, IERC20, Ownable {
    ...
    uint256 public maxBuyLimit = 4_200_000_000_000_000 * 10**9;
    uint256 public maxSellLimit = 4_200_000_000_000_000 * 10**9;
    uint256 public maxWalletLimit = 8_400_000_000_000_000 * 10**9;
    
    uint256 public genesis_block;
    uint256 private deadline;
    
    address public marketingWallet = 0x434eDeFe1E01566e74f96f1a9205f863b5B2d9bd;
    address public developmentWallet = 0x8d91EE7c53d970c2465A3eAAE8735AFED8DfB830;
    address public goatdreamWallet = 0x7EB402257dDf8B9aB8d7bBE127E179b2e005aa87;
    ...
    Taxes public taxes = Taxes(2, 4, 2, 1, 0);
    Taxes public sellTaxes = Taxes(2, 4, 2, 1, 0);
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[goatdreamWallet] = true;
        ...
        allowedTransfer[goatdreamWallet] = true;
    ...
    function setTradingStatus(
        ...
        swapEnabled = state;
        deadline = _deadline;
        launchtax = Taxes(rfi, marketing, liquidity, development, goatdream);
    ...
    function setTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _development, uint256 _goatdream) public onlyOwner {
       taxes = Taxes(_rfi,_marketing,_liquidity,_development,_goatdream);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _development, uint256 _goatdream) public onlyOwner {
       sellTaxes = Taxes(_rfi,_marketing,_liquidity,_development,_goatdream);
        emit FeesChanged();
    }
    ...
    function _takegoatdream(uint256 rgoatdream, uint256 tgoatdream) private {
        totFeesPaid.goatdream +=tgoatdream;

        if(_isExcluded[address(this)])
        {
            _tOwned[address(this)]+=tgoatdream;
        }
        _rOwned[address(this)] +=rgoatdream;
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell, bool useLaunchTax) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rdevelopment, to_return.rgoatdream) = _getRValues2(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell, bool useLaunchTax) private view returns (valuesFromGetValues memory s) {
        ...
        s.tgoatdream = tAmount*temp.goatdream/100;
        s.tTransferAmount = tAmount-s.tRfi-s.tMarketing-s.tLiquidity-s.tdevelopment-s.tgoatdream;
        return s;
    }
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rMarketing, uint256 rLiquidity){
        ...
        uint256 rgoatdream = s.tgoatdream*currentRate;
        rTransferAmount =  rAmount-rRfi-rMarketing-rLiquidity-rdevelopment-rgoatdream;
        return (rAmount, rTransferAmount, rRfi,rMarketing,rLiquidity);
    }
    
    function _getRValues2(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 rdevelopment,uint256 rgoatdream) {
        ...
        rgoatdream = s.tgoatdream*currentRate;
        return (rdevelopment,rgoatdream);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tMarketing + s.tdevelopment+ s.tgoatdream);
        }
        if(s.rMarketing > 0 || s.tMarketing > 0) _takeMarketing(s.rMarketing, s.tMarketing);
        if(s.rgoatdream > 0 || s.tgoatdream > 0) _takegoatdream(s.rgoatdream, s.tgoatdream);
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.marketing + temp.development + temp.goatdream) * 2;
        ...
        uint256 buybackAmt = unitBalance * 2 * temp.goatdream;
        if(buybackAmt > 0){
            payable(goatdreamWallet).sendValue(buybackAmt);
        }
    ...
    function updategoatdreamWallet(address newWallet) external onlyOwner{
        goatdreamWallet = newWallet;
    }
