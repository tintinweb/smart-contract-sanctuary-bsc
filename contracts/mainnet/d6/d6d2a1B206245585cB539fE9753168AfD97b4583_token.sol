...
contract token is ERC20 {
    ...
    uint256 public bliquidFee = 5;

    uint256 public sburnFee = 5;
    uint256 public sliquidFee = 5;
    
    uint256 public transferFee = 300;

    address public burnAddress = address(0xdead);
    address public liquidAddress = 0x1fa27D3956717092a44626468Cf3FeDCc065a1fd;

    uint256 public liquidCount;

    string internal tokenName = "339";
    string internal tokenSymbol = "339";
    uint256 constant total  =  3399 * 10**18;
    ...
    function setBuyFee( uint256 _liquidFee, uint256 _burnFee) public onlyOwner {
    ...
    function setSellFee(uint256 _liquidFee, uint256 _burnFee) public onlyOwner {
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if(from == uniswapV2Pair ){
                ...
                uint256 liquidFee = amount.div(1000).mul(bliquidFee);
                uint256 burnFee = amount.div(1000).mul(bburnFee);
                super._transfer(from, burnAddress, burnFee);
                super._transfer(from, address(this), liquidFee);
                liquidCount = liquidCount.add(liquidFee);
                amount = amount.div(1000).mul(1000 - (bliquidFee + bburnFee));
                
            }else if(to == uniswapV2Pair){
                uint256 liquidFee = amount.div(1000).mul(sliquidFee);
                uint256 burnFee = amount.div(1000).mul(sburnFee);
                super._transfer(from, burnAddress, burnFee);
                super._transfer(from, address(this), liquidFee);
                liquidCount =liquidCount.add(liquidFee);
                amount = amount.div(1000).mul(1000 - (sliquidFee + sburnFee));
