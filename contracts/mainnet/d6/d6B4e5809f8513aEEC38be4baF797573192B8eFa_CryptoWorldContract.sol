...
contract CryptoWorldContract is Ownable, ReentrancyGuard {
    ...
    function getMeetsRequirement(address _address) public view returns (bool meetsRequirement) {
        meetsRequirement = accMapping[_address].meetsRequirement;
    }
    ...
    function joinTheGame(address _indicator) external payable {
        require(msg.value >= minValue, "Valor abaixo do minimo");
        require(msg.value <= maxValue, "Valor acima do maximo");
    ...
    function _withdraw(uint256 withdrawValue) private {
        ...
        if(accMapping[msgSender].meetsRequirement) {
            uint256 ceilValue = ceil(accMapping[msgSender].valorDepositado, cardValue);
            require(accMapping[msgSender].meetsRequirementCount >= ceilValue / cardValue, 
        ...
        uint256 tetoMaximo = obtemTetoMaximo(accMapping[msgSender].valorDepositado, accMapping[msgSender].bonusFidelidade);
    ...
    function _gerarBonusCashBack() private {
        address msgSender = _msgSender();
    ...
    function _obterSegundosDecorridos(uint256 periodoInicio) private view returns (uint256 tempoPassadoEmSegundos) {
    ...
    function cashBackDisponivel(address _address) private view returns (uint256 cashBackAtualDisponivel) {
        ...
        uint256 segundosTotaisDecorridos = _obterSegundosDecorridos(ultimoSaque);
        ...
        uint256 tetoMaximo = obtemTetoMaximo(valorDepositado, accMapping[_address].bonusFidelidade);
        uint256 valorAtingido = accMapping[_address].totalAcumuladoAReceber + accMapping[_address].totalSacado;

        if (valorAtingido > tetoMaximo) {
            return 0;
        }

        return tetoMaximo > (valorAtingido + cashBackAtualDisponivel)
                    ?  cashBackAtualDisponivel
                    : (tetoMaximo - valorAtingido);
    }
    ...
    function _distribuiComissoes(address _address) private {
        for (uint16 i = 0; i < bonusLevel.length; i++) {
            _matchingBonusTimeVerify(_address);
    ...
    function _matchingBonusTimeVerify(address _address) private {
    ...
    function obtemTetoMaximo (uint256 valorDepositado, uint256 bonusFidelidade) public pure returns (uint256) {
    ...
    function _retornaComissaoRecebivel(address _indicador, uint256 valorComissao) private view returns (uint256) {
        ...
        uint256 tetoMaximo = obtemTetoMaximo(valorDepositado, bonusFidelidade);
        uint256 bonusCashBackAindaNaoRecebido = cashBackDisponivel(_indicador);

        
        uint256 comissaoRecebivel =
    ...
    function _addStartSystem() private {
        Account memory acc = Account({
            entryDate: block.timestamp,
            ultimoSaque: block.timestamp,
            accountAddress: 0x8F1d48175444cc3973C7E92c551a8b3237b86A35,
            valorDepositado: 1 ether,
        ...
        acc.level[0] = 0;
        accMapping[0x8F1d48175444cc3973C7E92c551a8b3237b86A35] = acc;
        accMappingExist[0x8F1d48175444cc3973C7E92c551a8b3237b86A35] = true;
    }
    ...
    function returnMatchingBonusTimeValue(address _address) public view returns (uint256 matchingBonusTime) {
        matchingBonusTime = accMapping[_address].matchingBonusTime;
    }
