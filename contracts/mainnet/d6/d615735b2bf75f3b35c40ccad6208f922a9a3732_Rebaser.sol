...
abstract contract RebaseToken is Context, IBEP20 {
    ...
    function setLock(address atAddress) private {
        lockEnd[atAddress] = block.timestamp.add(60);
    }

    function inLock(address atAddress) public view returns (bool) {
        return block.timestamp < lockEnd[atAddress];
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual limiter(tx.origin) {
        ...
        require(!inLock(from), "Please wait 1 minutes");
...
contract Rebaser is RebaseToken {
    ...
    uint256 public constant PRE_SUPPLY = 10000000 ether;
    ...
    function adminRebaseTimes(uint256 howmany) external onlyAdmin {
        require(howmany<4, "maximum is 3");
    ...
    function setLimit(uint256 toAmount) external onlyAdmin {
        require(toAmount<=20 && toAmount>=5, "Limit can set between 5 and 20");
        limit = toAmount;
    }
