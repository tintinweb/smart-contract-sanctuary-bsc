...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function _forStake(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: For stake from the zero address");
        uint256 accountBalance = _balances[account];
        require(accountBalance >= amount, "ERC20: For stake amount exceeds balance");
        unchecked {
...
contract TestY3 is ERC20, Ownable {
    ...
    constructor() ERC20("Test Y3", "TY3") {
        ...
        buyDevFee = 0;
        buyStakingFee = 0;
        buyTotalFees = buyOperationsFee + buyLiquidityFee + buyDevFee + buyStakingFee;

        sellOperationsFee = 20;
        sellLiquidityFee = 0;
        sellDevFee = 0;
        sellStakingFee = 0;
        sellTotalFees = sellOperationsFee + sellLiquidityFee + sellDevFee + sellStakingFee;
    ...
    function excludeFromMaxTransaction(address updAds, bool isEx) external onlyOwner {
        if(!isEx){
            require(updAds != lpPair, "Cannot remove pair from max txn");
        }
    ...
    function updateBuyFees(uint256 _operationsFee, uint256 _liquidityFee, uint256 _devFee, uint256 _stakingFee) external onlyOwner {
    ...
    function updateSellFees(uint256 _operationsFee, uint256 _liquidityFee, uint256 _devFee, uint256 _stakingFee) external onlyOwner {
    ...
    function swapBack() private {
        ...
        if(tokensForStake > 0 && balanceOf(address(this)) >= tokensForStake) {
            _transfer(address(this), address(stakingAddress), tokensForStake);
        }
        tokensForStake = 0;
