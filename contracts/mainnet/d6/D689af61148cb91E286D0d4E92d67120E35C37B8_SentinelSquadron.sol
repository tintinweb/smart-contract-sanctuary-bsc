...
contract SentinelSquadron is Context, ERC165, IERC721, IERC721Metadata, Ownable {

    using Address for address;

    // Token name
    string private constant _name = "Sentinel Squadron";

    // Token symbol
    string private constant _symbol = "EMP-Squad";
    ...
    uint256 public costPhoenix = 150 * 10**18;
    ...
    function setCostPhoenix(uint256 newCost) external onlyOwner {
        costPhoenix = newCost;
    }
     ...
     */
    function mint(address token, uint256 numberOfMints) external {
        require(
            mintingEnabled,
            'Minting Not Enabled'
        );
        ...
        require(
            token == BUSD || token == Phoenix,
            'Invalid Token'
        );

        // determine cost
        uint cost = token == BUSD ? costBUSD : costPhoenix;

        // transfer in tokens
        _transferIn(token, numberOfMints * cost);
