...
contract DOGE is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        uint256[4] memory feeSettings, // rewards, liquidity, marketing,brun
        uint256 minimumTokenBalanceForDividends_
        ...
        brunFee = feeSettings[3];
        totalFees = tokenRewardsFee.add(liquidityFee).add(marketingFee).add(brunFee);
    ...
    function setBrunFee(uint256 value) external onlyOwner {
        brunFee = value;
        totalFees = tokenRewardsFee.add(liquidityFee).add(marketingFee).add(brunFee);
        require(totalFees <= 25, "Total fee is over 25%");
    }
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            uint256 burnAmount = amount.mul(brunFee).div(100);
