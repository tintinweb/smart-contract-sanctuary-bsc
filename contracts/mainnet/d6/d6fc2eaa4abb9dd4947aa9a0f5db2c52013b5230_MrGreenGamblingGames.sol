...
contract MrGreenGamblingGames {
    ...
    event WeHaveAWinner(string response);
    event PlayerPlayed(string response);    
...
////////////////////////////////ChainLink Section ///////////////////////////
    ...
    function praiseTheWinner(address winner, uint256 bnbAmount) internal {
        string memory response = string(
            abi.encodePacked(
                "The wallet ", 
                getSlice(1,4,addressToString(winner)),
                "..",
                getSlice(39,42,addressToString(winner)),
                " just won 0.",
                uintToString(bnbAmount / 10**15),
                " BNB! Congratulations to the lucky winner!"
            )
        );
        emit WeHaveAWinner(response);
    }

    function praiseThePlayer(address winner, uint256 bnbAmount) internal {
        string memory response = string(
            abi.encodePacked(
                "The wallet ", 
                getSlice(1,4,addressToString(winner)),
