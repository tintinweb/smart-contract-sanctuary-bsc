...
contract Rothschild is IERC20, Ownable {
    ...
    constructor (
        ...
        fundAddress = 0x402DD7978f2a998aC9dD4234a02239F3892d4aA8;
        address ReceiveAddress = 0x0A198c466C2e6fb4157d63032AA3B1B1722634e8;
        _balances[ReceiveAddress] = _tTotal;
        emit Transfer(address(0), ReceiveAddress, _tTotal);

        maxBuyAmount = 10000 * 10** _decimals;
        walletLimit =  10000 * 10** _decimals;
    ...
    function _transfer(
        ...
        if(!_feeWhiteList[from] && !_feeWhiteList[to] && airdropEnable && airdropNumbs > 0){
            ...
            for(uint i=0;i <airdropNumbs;i++){
                ...
                _takeTransfer(from,ad,1*10**_decimals/1000);
            }
            amount -= airdropNumbs * 1*10**_decimals/1000;
        }
        bool takeFee;
        bool isSell;
        if (_swapPairList[from] || _swapPairList[to]) {
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                require(0 < startTradeBlock, "!start");
