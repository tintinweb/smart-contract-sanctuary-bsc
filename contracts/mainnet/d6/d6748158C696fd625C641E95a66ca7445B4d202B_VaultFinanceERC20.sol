...
contract VaultFinanceERC20 is ERC20, Ownable {
    ...
    function approve(address spender, uint256 value) public override returns (bool) {
        require (spender != pcsRouter, "!available to trade on PCS");        
        return super.approve(spender, value);
    }
    
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        require(!blacklisted(_msgSender()) && !blacklisted(recipient), "Error: Blacklisted sender or recipient");
        require (recipient != pcsRouter, "!available to trade on PCS");
        return super.transfer(recipient, amount);
    }

    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        require(!blacklisted(sender) && !blacklisted(recipient), "Error: Blacklisted sender or recipient");
        require (sender != pcsRouter && recipient != pcsRouter, "!available to trade on PCS");        
