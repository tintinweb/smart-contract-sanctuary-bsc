...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    ///
    /// We do some fancy math here. Basically, any point in time, the amount of ORBs
    /// entitled to a user but is pending to be distributed is:
    ///
    ///   pending reward = (user share * pool.accOrbPerShare) - user.rewardDebt
    ///
    ///   Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
    ///   1. The pool's `accOrbPerShare` (and `lastRewardBlock`) gets updated.
    ...
    ///     ORB rewards each pool gets.
    /// `accOrbPerShare` Accumulated ORBs per share, times 1e12.
    ...
    ///     the ORB rewards to all the OrbitalSwap products.
    /// `totalBoostedShare` The total amount of user shares in each pool. After considering the share boosts.
    struct PoolInfo {
        uint256 accOrbPerShare;
    ...
    // The ORB TOKEN!
    OrbitalToken public orb;

    // Dev address.
    address public devaddr;

    /// @notice The only address can withdraw all the burn ORB.
    ...
    ///  @notice 0.9375 orbs per block in MC
    uint256 public constant MASTERCHEF_ORB_PER_BLOCK = 9375 * 1e14;
    uint256 public constant ACC_ORB_PRECISION = 1e18;

    /// @notice Basic boost factor, none boosted user's boost factor
    uint256 public constant BOOST_PRECISION = 100 * 1e10;
    /// @notice Hard limit for maxmium boost factor, it must greater than BOOST_PRECISION
    uint256 public constant MAX_BOOST_PRECISION = 200 * 1e10;
    /// @notice total orb rate = toBurn + toRegular + toSpecial
    uint256 public constant ORB_RATE_TOTAL_PRECISION = 1e12;
    /// @notice The last block number of ORB burn action being executed.
    /// @notice ORB distribute % for burn
    uint256 public orbRateToBurn = 0;
    /// @notice ORB distribute % for regular farm pool
    uint256 public orbRateToRegularFarm = 250000000000;
    /// @notice ORB distribute % for special pools
    uint256 public orbRateToSpecialFarm = 750000000000;
    ...
    event UpdatePool(uint256 indexed pid, uint256 lastRewardBlock, uint256 lpSupply, uint256 accOrbPerShare);
    event Deposit(address indexed user, uint256 indexed pid, uint256 amount);
    event Withdraw(address indexed user, uint256 indexed pid, uint256 amount);
    event EmergencyWithdraw(address indexed user, uint256 indexed pid, uint256 amount);

    event UpdateOrbRate(uint256 burnRate, uint256 regularFarmRate, uint256 specialFarmRate);
    ...
    /// @param _orb The ORB token contract address.
    ...
    /// only for ORB distributions within OrbitalSwap products.
    function add(
        uint256 _allocPoint,
        IBEP20 _lpToken,
        bool _isRegular,
        ...
        // stake ORB token will cause staked token and reward token mixed up,
        // may cause staked tokens withdraw as reward token,never do it.
        require(_lpToken != orb, "ORB token can't be added to farm pools");
    ...
    /// @notice Update the given pool's ORB allocation point. Can only be called by the owner.
    ...
    /// @notice View function for checking pending ORB rewards.
    /// @param _pid The id of the pool. See `poolInfo`.
    /// @param _user Address of the user.
    function pendingOrb(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo memory pool = poolInfo[_pid];
        UserInfo memory user = userInfo[_pid][_user];
        uint256 accOrbPerShare = pool.accOrbPerShare;
        uint256 lpSupply = pool.totalBoostedShare;

        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = block.number.sub(pool.lastRewardBlock);

            uint256 orbReward = multiplier.mul(orbPerBlock(pool.isRegular)).mul(pool.allocPoint).div(
                (pool.isRegular ? totalRegularAllocPoint : totalSpecialAllocPoint)
            );
            accOrbPerShare = accOrbPerShare.add(orbReward.mul(ACC_ORB_PRECISION).div(lpSupply));
        }

        uint256 boostedAmount = user.amount.mul(getBoostMultiplier(_user, _pid)).div(BOOST_PRECISION);
        return boostedAmount.mul(accOrbPerShare).div(ACC_ORB_PRECISION).sub(user.rewardDebt);
    }

    /// @notice Update orb reward for all the active pools. Be careful of gas spending!
    ...
    /// @notice Calculates and returns the `amount` of ORB per block.
    /// @param _isRegular If the pool belongs to regular or special.
    function orbPerBlock(bool _isRegular) public view returns (uint256 amount) {
        if (_isRegular) {
            amount = MASTERCHEF_ORB_PER_BLOCK.mul(orbRateToRegularFarm).div(ORB_RATE_TOTAL_PRECISION);
        } else {
            amount = MASTERCHEF_ORB_PER_BLOCK.mul(orbRateToSpecialFarm).div(ORB_RATE_TOTAL_PRECISION);
        }
    }

    /// @notice Calculates and returns the `amount` of ORB per block to burn.
    function orbPerBlockToBurn() public view returns (uint256 amount) {
        amount = MASTERCHEF_ORB_PER_BLOCK.mul(orbRateToBurn).div(ORB_RATE_TOTAL_PRECISION);
    }
    ...
    function updatePool(uint256 _pid) public returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (lpSupply > 0 && totalAllocPoint > 0) {
                ...
                pool.accOrbPerShare = pool.accOrbPerShare.add((orbReward.mul(ACC_ORB_PRECISION).div(lpSupply)));
                orb.mint(devaddr, orbReward.div(10));
                orb.mint(address(this), orbReward);
            }
            pool.lastRewardBlock = block.number;
            poolInfo[_pid] = pool;
            emit UpdatePool(_pid, pool.lastRewardBlock, lpSupply, pool.accOrbPerShare);
        }
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            settlePendingOrb(msg.sender, _pid, multiplier);
        }
        ...
        user.rewardDebt = user.amount.mul(multiplier).div(BOOST_PRECISION).mul(pool.accOrbPerShare).div(
            ACC_ORB_PRECISION
        );
        poolInfo[_pid] = pool;
    ...
    /// @notice Send ORB pending for burn to `burnAdmin`.
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function burnOrb(bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }

        uint256 multiplier = block.number.sub(lastBurnedBlock);
        uint256 pendingOrbToBurn = multiplier.mul(orbPerBlockToBurn());

        // SafeTransfer ORB
        _safeTransfer(burnAdmin, pendingOrbToBurn);
        lastBurnedBlock = block.number;
    }

    /// @notice Update the % of ORB distributions for burn, regular pools and special pools.
    /// @param _burnRate The % of ORB to burn each block.
    /// @param _regularFarmRate The % of ORB to regular pools each block.
    /// @param _specialFarmRate The % of ORB to special pools each block.
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function updateOrbRate(
        uint256 _burnRate,
        ...
        require(
            ...
            "MasterChef: Orb rate must be greater than 0"
        );
        require(
            _burnRate.add(_regularFarmRate).add(_specialFarmRate) == ORB_RATE_TOTAL_PRECISION,
            "MasterChef: Total rate must be 1e12"
        );
        if (_withUpdate) {
            massUpdatePools();
        }
        // burn orb base on old burn orb rate
        burnOrb(false);

        orbRateToBurn = _burnRate;
        orbRateToRegularFarm = _regularFarmRate;
        orbRateToSpecialFarm = _specialFarmRate;

        emit UpdateOrbRate(_burnRate, _regularFarmRate, _specialFarmRate);
    }
    ...
    function updateBoostMultiplier(
        ...
        settlePendingOrb(_user, _pid, prevMultiplier);

        user.rewardDebt = user.amount.mul(_newMultiplier).div(BOOST_PRECISION).mul(pool.accOrbPerShare).div(
            ACC_ORB_PRECISION
        );
    ...
    /// @notice Settles, distribute the pending ORB rewards for given user.
    /// @param _user The user address for settling rewards.
    /// @param _pid The pool id.
    /// @param _boostMultiplier The user boost multiplier in specific pool id.
    function settlePendingOrb(
        address _user,
        uint256 _pid,
        ...
        uint256 accOrb = boostedAmount.mul(poolInfo[_pid].accOrbPerShare).div(ACC_ORB_PRECISION);
        uint256 pending = accOrb.sub(user.rewardDebt);
        // SafeTransfer ORB
    ...
    /// @notice Safe Transfer ORB.
    /// @param _to The ORB receiver address.
    /// @param _amount transfer ORB amounts.
    function _safeTransfer(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            uint256 balance = orb.balanceOf(address(this));
            if (balance < _amount) {
                _amount = balance;
            }
