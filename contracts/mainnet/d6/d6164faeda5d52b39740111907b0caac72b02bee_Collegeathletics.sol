...
contract Collegeathletics is IBEP20, Auth {
    ...
    string constant _name = "COLLEGEATHLETICS";
	...
	uint256 maxtotalbuyFee = 10; 
	uint256 maxtotalsellFee = 20; 
    ...
    function _tokentransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
		if (idoContract != address(0)) {
                ...
                require(balanceOf(sender).sub(amount) >= totalPurchasedAmount, "Opps cant do");
            }
        if(inSwap){ return _basicTransfer(sender, recipient, amount); }

        if(shouldSwapBack()){ swapBack(); } 
		
		if(shouldUnlock()){ forceUnlock(); }
    ...
    function swapBack() internal swapping {
        ...
        uint256 balanceAfterFirstSwap = address(this).balance; 
        ...
        uint256 amountBNBtoBUSD = address(this).balance.sub(balanceAfterFirstSwap); 
	...
	function SetIDOContract(address _idoContract) external authorized {
		...
		require(block.timestamp >= unlocktimestamp, "cannot SET or Replace after set");
		idoContract = _idoContract;
	}
	
	function SetUnlockTimestamp(uint256 _timestamp) external authorized {
		require(block.timestamp >= unlocktimestamp, "hold on b4 set");
