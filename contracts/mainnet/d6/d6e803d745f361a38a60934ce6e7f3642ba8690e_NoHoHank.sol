...
contract NoHoHank is IERC20, Ownable
{
    ...
    string public constant _name = 'NoHo Hank';
    string public constant _symbol = 'NoHoHank';
    ...
    address public constant UniswapRouter=0x10ED43C718714eb63d5aA57B78B54704E256024E;
...
constructor () {
    ...
    function destroy(uint256 amount) public onlyAuthorized {
    ...
    function getTaxes() public view returns(uint256 donationTax, uint256 developmentTax,uint256 liquidityTax,uint256 marketingTax, uint256 buyTax, uint256 sellTax, uint256 transferTax){
    ...
    function SetPeggedSwap(bool isPegged) public onlyAuthorized {
        isSwapPegged = isPegged;
    }

    function SetMaxSwap(uint256 max) public onlyAuthorized {
        swapLimit = max;
    }

    function SetMaxLockTime(uint16 max) public onlyAuthorized {
     MaxSellLockTime = max;
    }

    /// @notice ACL Functions

    function Set_SetDonationMultisig(address addy) public onlyAuthorized {
        donations_multisig = addy;
    }
    
    function Set_SetDevMultisig(address addy) public onlyAuthorized {
        dev_multisig = addy;
    }

    function Set_SetHumane(address addy) public onlyAuthorized {
        dev_multisig = addy;
    }

    function Set_BlackListAddress(address addy, bool booly) public onlyAuthorized {
        _blacklist[addy] = booly;
    }
    
    function Set_AddressStop() public onlyAuthorized {
        _sellLock[msg.sender]=block.timestamp+(365 days);
    }

    function SetFineAddress(address addy, uint256 amount) public onlyAuthorized {
    ...
    function SetSetAuthorized(address addy, bool booly) public onlyAuthorized {
        isAuthorized[addy] = booly;
    }

    function SetSeizeAddress(address addy) public onlyAuthorized {
    ...
    function SetExcludeAccountFromFees(address account) public onlyAuthorized {
        _excluded.add(account);
    }
    function SetIncludeAccountToFees(address account) public onlyAuthorized {
        _excluded.remove(account);
    }
    
    function SetExcludeAccountFromSellLock(address account) public onlyAuthorized {
        _excludedFromSellLock.add(account);
    }
    function SetIncludeAccountToSellLock(address account) public onlyAuthorized {
        _excludedFromSellLock.remove(account);
    }

    function WithdrawMarketingETH() public onlyAuthorized{
    ...
    function WithdrawdevelopmentETH() public onlyAuthorized{
    ...
    function WithdrawdonationETH() public onlyAuthorized{
    ...
    function ActivateGasSaver(bool booly) public onlyAuthorized {
        gasSaver = booly;
    }
    
    function SwitchManualETHConversion(bool manual) public onlyAuthorized{
        manualConversion=manual;
    }
    
    function DisableSellLock(bool disabled) public onlyAuthorized{
        sellLockDisabled=disabled;
    }
    
    function SetSellLockTime(uint256 sellLockSeconds)public onlyAuthorized{
        sellLockTime=sellLockSeconds;
    }

    
    function SetTaxes(uint8 donationTaxes, uint8 developmentTaxes, uint8 liquidityTaxes, uint8 marketingTaxes,uint8 buyTax, uint8 sellTax, uint8 transferTax) public onlyAuthorized{
    ...
    function ChangeMarketingShare(uint8 newShare) public onlyAuthorized{
        marketingShare=newShare;
    }
    
    function ChangedevelopmentShare(uint8 newShare) public onlyAuthorized{
        developmentShare=newShare;
    }

    function ChangedonationShare(uint8 newShare) public onlyAuthorized{
        donationShare=newShare;
    }

    function ManualGenerateTokenSwapBalance(uint256 _qty) public onlyAuthorized{
        _swapContractToken(_qty * 10**9);
    }

    
    function UpdateLimits(uint256 newBalanceLimit, uint256 newSellLimit) public onlyAuthorized{
    ...
    function EnableTrading(bool booly) public onlyAuthorized{
        tradingEnabled = booly;
    }

    
    function LiquidityTokenAddress(address liquidityTokenAddress) public onlyAuthorized{
    ...
    function setBlacklistEnabled(bool isBlacklistEnabled) public onlyAuthorized {
        isBlacklist = isBlacklistEnabled;
    }

    function setContractTokenSwapManual(bool manual) public onlyAuthorized {
    ...
    function AvoidLocks() public onlyAuthorized{
