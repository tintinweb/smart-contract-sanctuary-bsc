...
contract DOGF is ERC20, Ownable, Payable {
    ...
    mapping(uint256 => uint256) public dailyProfit;

    constructor() public ERC20("DOGF", "DOGF") {
        _mint(msg.sender, 10000 * 10000 * 10 ** uint256(decimals()));
    }
    ...
    function _customTransfer(address sender, address recipient, uint256 amount) private returns (uint256) {
        if ((swapPairs[sender] && !excludedFee[recipient]) || (swapPairs[recipient] && !excludedFee[sender])) {
            ...
            _transfer(sender, address(this), profit);
            (uint year, uint month, uint day) = Datetime.timestampToDate(now + 8 * 60 * 60);
            uint256 date = year*10000+month*100+day;
            dailyProfit[date] = dailyProfit[date].add(profit);
