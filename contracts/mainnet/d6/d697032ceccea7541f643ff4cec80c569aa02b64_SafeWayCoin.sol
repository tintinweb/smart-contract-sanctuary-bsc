...
contract SafeWayCoin is Context, IERC20, Ownable{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    mapping (address => bool) private _isExcluded;  
    uint256 private _totalSupply = 10000000 *  10**9;  
    string private _name = "SafeWay";
    string private _symbol = "SWAY";
    ...
    function hsetLiquidityFee(uint256 rLiquidity) external onlyOwner {       
    ...
    function jsetDevFee(uint256 rDev) external onlyOwner {       
        if(_isExcluded[address(this)])
        {
            totalFees.dev +=rDev;
        }        
    ...
    function ksetMarketingFee(uint256 rMarketing) external onlyOwner {       
        if(_isExcluded[address(this)])
        {
	...
	function transferSafeWayToken(address _token, address _to) external onlyOwner returns (bool _sent)
	  {
	...
	function getSafeWayTime() public view returns (uint256) {
