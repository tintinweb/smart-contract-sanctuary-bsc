...
contract StakeVELLPPool is LPTokenWrapper, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public everyDayReward = 1 * 10 ** 9;


    uint256[] period = [30, 90, 180, 270, 360];
    uint256[] periodRewardRate = [22, 75, 168, 279, 408];
    ...
    modifier checkPeriod(uint256 _period) {
        ...
        require(isPeriod, "StakeVELLPPool: Wrong cycle");
        _;
    }
    ...
    constructor(
        ...
        for(uint256 i = 0; i < period.length; i++) {
            periodReward[period[i]] = everyDayReward.div(period[i]).mul(periodRewardRate[i]).div(8640000);
        }
    }
    ...
    function stakeLp(uint256 _amount, uint256 _period) public checkPeriod(_period) {
        require(_amount > 0, 'StakeVELLPPool: Cannot stake 0');
        super.stake(_amount);
        StakeMap memory stakeMap = StakeMap({
            amount: _amount,
            marketAmount: calculateVELAmount(_amount),
            period: _period,
    ...
    function withdrawLP(uint256 index) public {
        ...
        require(index < userStakes.length, "StakeVELLPPool: you have no stake");
    ...
    function getReward() public  {
        StakeMap[] storage userStakes = userStakeMaps[msg.sender];
        require(userStakes.length > 0, "StakeVELLPPool: You have no staked");
    ...
    function remove(address staker, uint256 index) internal {
        ...
        for (uint256 k = index; k < userStakes.length - 1; k++) {
            userStakes[k] = userStakes[k + 1];
        }
        userStakes.pop();
    ...
    function calculateVELAmount(uint256 lpAmount) private view returns(uint256) {
        uint256 totalLP = IERC20(address(lpt)).totalSupply();
        uint256 lpVelAmount = IERC20(velTokenAddress).balanceOf(address(lpt));
        uint256 marketAmount = lpVelAmount.mul(lpAmount).div(totalLP);
        return marketAmount;
    }
