 ...
 * Implemented and modified by CSTSwap teams.
 */
contract CSTZapV1 is Ownable, ReentrancyGuard {
    ...
    // CSTRouter interface
    ICSTRouter02 public CSTRouter;

    // Maximum integer (used for managing allowance)
    uint256 public constant MAX_INT = 2**256 - 1;

    // Minimum amount for a swap (derived from CSTSwap)
    ...
    // Address CSTRouter
    address private CSTRouterAddress;
     ...
     * @param _CSTRouter: address of the CSTRouter
    ...
    constructor(
        ...
        CSTRouterAddress = _CSTRouter;
        CSTRouter = ICSTRouter02(_CSTRouter);
    ...
    {
        address token0 = ICSTPair(_lpToken).token0();
        address token1 = ICSTPair(_lpToken).token1();

        require(_tokenToZap == token0 || _tokenToZap == token1, "Zap: Wrong tokens");

        // Convert to uint256 (from uint112)
        (uint256 reserveA, uint256 reserveB, ) = ICSTPair(_lpToken).getReserves();

        if (token0 == _tokenToZap) {
            swapTokenOut = token1;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveA, reserveB);
            swapAmountOut = CSTRouter.getAmountOut(swapAmountIn, reserveA, reserveB);
        } else {
            swapTokenOut = token0;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveB, reserveA);
            swapAmountOut = CSTRouter.getAmountOut(swapAmountIn, reserveB, reserveA);
        }
    ...
    {
        require(
            _token0ToZap == ICSTPair(_lpToken).token0() || _token0ToZap == ICSTPair(_lpToken).token1(),
            "Zap: Wrong token0"
        );
        require(
            _token1ToZap == ICSTPair(_lpToken).token0() || _token1ToZap == ICSTPair(_lpToken).token1(),
            "Zap: Wrong token1"
        );
        ...
        if (_token0ToZap == ICSTPair(_lpToken).token0()) {
        ...
        )
    {
        ...
        if (token1 == _tokenToReceive) {
            // sell token0
            uint256 tokenAmountIn = (_lpTokenAmount * reserveA) / ICSTPair(_lpToken).totalSupply();
            ...
            // sell token1
            uint256 tokenAmountIn = (_lpTokenAmount * reserveB) / ICSTPair(_lpToken).totalSupply();
    ...
    function _zapIn(
        ...
        uint256[] memory swapedAmounts = CSTRouter.swapExactTokensForTokens(
            swapAmountIn,
            _tokenAmountOutMin,
            path,
            address(this),
        ...
        (, , lpTokenReceived) = CSTRouter.addLiquidity(
            path[0],
            path[1],
    ...
    function _zapOut(
        ...
        (uint256 amount0, uint256 amount1) = ICSTPair(_lpToken).burn(address(this));

        require(amount0 >= MINIMUM_AMOUNT, "CSTRouter: INSUFFICIENT_A_AMOUNT");
        require(amount1 >= MINIMUM_AMOUNT, "CSTRouter: INSUFFICIENT_B_AMOUNT");
        ...
        // Swap tokens
        CSTRouter.swapExactTokensForTokens(swapAmountIn, _tokenAmountOutMin, path, address(this), block.timestamp);
    ...
    function _approveTokenIfNeeded(address _token, uint256 _swapAmountIn) private {
        if (IERC20(_token).allowance(address(this), CSTRouterAddress) < _swapAmountIn) {
            // Reset to 0
            IERC20(_token).safeApprove(CSTRouterAddress, 0);
            // Re-approve
            IERC20(_token).safeApprove(CSTRouterAddress, MAX_INT);
        }
    }
    ...
    function _calculateAmountToSwap(
        ...
        uint256 nominator = CSTRouter.getAmountOut(halfToken0Amount, _reserve0, _reserve1);
        uint256 denominator = CSTRouter.quote(
            halfToken0Amount,
    ...
    function _calculateAmountToSwapForRebalancing(
        ...
        if (sellToken0) {
            ...
            uint256 nominator = CSTRouter.getAmountOut(token0AmountToSell, _reserve0, _reserve1);
            uint256 denominator = CSTRouter.quote(
                token0AmountToSell,
            ...
            uint256 nominator = CSTRouter.getAmountOut(token1AmountToSell, _reserve1, _reserve0);
