...
contract Verifier {
    function verify(uint256 gasPrice, uint256 gasLimit, bytes memory value, uint256 nonce, bytes memory data, address to, address from, uint8 v, bytes32 r, bytes32 s) external pure returns(address, bool) {
    ...
    function generateRaw(uint256 gasPrice, uint256 gasLimit, bytes memory value, uint256 nonce, bytes memory data, address to) public pure returns(bytes memory) {
        ...
        encoded[4] = RLPEncoder.encodeBytes(value);
