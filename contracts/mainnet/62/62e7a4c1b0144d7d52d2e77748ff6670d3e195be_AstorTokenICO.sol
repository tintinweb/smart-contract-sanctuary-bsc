...
contract AstorTokenICO is Ownable {
	...
	function updateStage3Time(uint256 _stage3) external onlyOwner{
		stage3Time = _stage3;
	}


	function getAmount(address token) external view returns(uint256 amount){
	...
	function buyToken(address token, uint256 amount, address user) external payable {
		...
		require(usdAmount >= (firstBuyAmount - 10000000), "Cannot buy below $50");
