...
contract MdexRouter is IMdexRouter, Ownable {
    ...
    function setUnrestricted(address[] calldata users, bool opt) external onlyOwner {
                ...
                if (tx.origin == msg.sender) {
                    ...
                    if (restrict[pair]) {
                        if (unrestricted[msg.sender]) {
                            (uint112 reserve0, uint112 reserve1, ) = IMdexPair(pair).getReserves();
                            uint256 pairThreshold = getThreshold(pair);
                            if (
                                amount0Out.mul(1e8).div(reserve0) <= pairThreshold &&
                                amount1Out.mul(1e8).div(reserve1) <= pairThreshold
                            ) {
                                ISwapMining(swapMining).swap(msg.sender, input, output, amountOut);
                            }
                        }
