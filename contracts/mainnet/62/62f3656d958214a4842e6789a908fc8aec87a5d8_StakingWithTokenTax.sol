...
contract StakingWithTokenTax {
    ...
    bool public extendLockWhenStaking;
    ...
    constructor (address stakingToken, address _rewardToken, address tokenOwner, uint256 lockTime, bool _extendLockWhenStaking) {
        token = stakingToken;
        rewardToken = _rewardToken;
        admin = tokenOwner;
        lockDuration = lockTime;
        extendLockWhenStaking = _extendLockWhenStaking;
    ...
    function stakeSomeForSomeone(uint amount, address staker) external {
        require(amount > 0);
        amount = amount * (10 ** _decimals);
        IBEP20(token).transferFrom(msg.sender, address(this), amount);
    ...
    function stakeSomeForSomeoneWithCustomLockTime(uint amount, address staker, uint256 _days) external {
        require(amount > 0);
        require(_days * 1 days >= lockDuration, "Can't lock for shorter than the public");
        amount = amount * (10 ** _decimals);
        IBEP20(token).transferFrom(msg.sender, address(this), amount);
        uint256 normalLock = lockDuration;
        lockDuration = _days * 1 days;
        _stake(staker, amount);
        lockDuration = normalLock;
    }
    ...
    function _stake(address staker, uint256 stakedAmount) internal {
        ...
        _realise(staker);
        if(extendLockWhenStaking || stakes[staker].lockedUntil == 0) stakes[staker].lockedUntil = block.timestamp + lockDuration;
