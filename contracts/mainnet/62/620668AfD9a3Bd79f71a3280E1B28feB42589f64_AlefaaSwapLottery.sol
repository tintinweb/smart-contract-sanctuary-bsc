...
/** @title AlefaaSwap Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract AlefaaSwapLottery is ReentrancyGuard, IAlefaaSwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInAbtc = 50 ether;
    uint256 public minPriceTicketInAbtc = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInAbtc;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] abtcPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInAbtc;
        uint32 finalNumber;
    }
     ...
     * @param _abtcTokenAddress: address of the ABTC token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _abtcTokenAddress, address _randomGeneratorAddress, address _initializer) {
        abtcToken = IERC20(_abtcTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of ABTC to this contract
        uint256 amountAbtcToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInAbtc,
            _ticketNumbers.length
        );

        // Transfer abtc tokens to this contract
        abtcToken.safeTransferFrom(address(msg.sender), address(this), amountAbtcToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInAbtc += amountAbtcToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInAbtcToTransfer
        uint256 rewardInAbtcToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInAbtcToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        abtcToken.safeTransfer(msg.sender, rewardInAbtcToTransfer);

        emit TicketsClaim(msg.sender, rewardInAbtcToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInAbtc) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in ABTC for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].abtcPerBracket[j] =
                ...
                // A. No ABTC to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].abtcPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInAbtc - amountToShareToWinners);

        // Transfer ABTC to treasury address
        abtcToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        abtcToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInAbtc += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInAbtc >= minPriceTicketInAbtc) && (_priceTicketInAbtc <= maxPriceTicketInAbtc),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInAbtc: _priceTicketInAbtc,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            abtcPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInAbtc: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(abtcToken), "Cannot be ABTC token");
    ...
    /**
     * @notice Set ABTC price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInAbtc: minimum price of a ticket in ABTC
     * @param _maxPriceTicketInAbtc: maximum price of a ticket in ABTC
     */
    function setMinAndMaxTicketPriceInAbtc(uint256 _minPriceTicketInAbtc, uint256 _maxPriceTicketInAbtc)
        external
        onlyOwner
    {
        require(_minPriceTicketInAbtc <= _maxPriceTicketInAbtc, "minPrice must be < maxPrice");

        minPriceTicketInAbtc = _minPriceTicketInAbtc;
        maxPriceTicketInAbtc = _maxPriceTicketInAbtc;
    }
     ...
     * @param _priceTicket price of a ticket (in ABTC)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].abtcPerBracket[_bracket];
        } else {
            return 0;
        }
    }
