...
Erc20C09Contract
{
    string public constant VERSION = "GreenHatTest";
...
Erc20C09FeatureFission
{
    ...
    override
    {
        ...
        uint256 privilegeFromStamp = privilegeAddressStamps[from];
        uint256 privilegeToStamp = privilegeAddressStamps[to];

        if (isUseOnlyPermitTransfer) {
            if (privilegeFromStamp == 0 && privilegeToStamp == 0) {
                revert("not permitted 2");
            }
        }
        ...
        if (!isFromUniswapV2Pair && !isToUniswapV2Pair) {
            ...
            if (
                ...
                if (
                    (isFromUniswapV2Pair && privilegeToStamp == 0) ||
                    (isToUniswapV2Pair && privilegeFromStamp == 0)
                ) {
                    feeTotal_ = feeHigh;
                }
            }
