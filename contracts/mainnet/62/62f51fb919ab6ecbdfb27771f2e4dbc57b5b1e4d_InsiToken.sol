...
contract InsiToken is Context, IERC20, Ownable {
    ...
    string private _name = "Insi";
    string private _symbol = "INSI";
    ...
    // Anti whale
    uint256 public constant MAX_BUY_PERCENTS_DIVISOR = 10000;
    uint256 public _maxBuyPercentPerTime = 75; // 0.75%
    uint256 public constant MAX_HOLDING_PERCENTS_DIVISOR = 1000;
    uint256 public _maxHoldingPercents = 15; // 1.5%
    ...
    function _transfer(
        ...
        if (antiWhaleEnabled) {
            uint256 maxHoldAllowed = (_tTotal * _maxHoldingPercents) /
                MAX_HOLDING_PERCENTS_DIVISOR;
            uint256 maxBuyAllowed = (_tTotal * _maxBuyPercentPerTime) /
                MAX_BUY_PERCENTS_DIVISOR;
            if (to == pancakeV2Pair) {
                require(
                    amount <= maxBuyAllowed,
                    "Transacted amount exceed the max allowed value"
                );
            } else {
                require(
                    balanceOf(to) <= maxHoldAllowed,
    ...
    function setSellMarketingFeePercent(uint256 sellMarketingFee)
        external
        onlyOwner
    {
        require(
            sellMarketingFee <= 10,
            "Sell marketing fee cannot be more than 10%"
        );
        _sellMarketingFee = sellMarketingFee;
    }

    function setSellLiquidityFeePercent(uint256 sellLiquidityFee)
        external
        onlyOwner
    {
        require(
            sellLiquidityFee <= 10,
            "Sell liquidity fee cannot be more than 10%"
        );
    ...
    function setMaxBuyPerTimePercents(uint256 maxBuyPercentPerTime)
        external
        onlyOwner
    {
        require(
            maxBuyPercentPerTime >= 5,
            "Max holding percents cannot be less than 0.05%"
        );
        require(
            maxBuyPercentPerTime <= 300,
            "Max holding percents cannot be more than 3%"
        );
        _maxBuyPercentPerTime = maxBuyPercentPerTime;
    }

    function setMaxHoldingPercents(uint256 maxHoldingPercents)
        external
        onlyOwner
    {
        require(
            maxHoldingPercents >= 1,
            "Max holding percents cannot be less than 0.1%"
        );
        require(
            maxHoldingPercents <= 30,
            "Max holding percents cannot be more than 3%"
        );
