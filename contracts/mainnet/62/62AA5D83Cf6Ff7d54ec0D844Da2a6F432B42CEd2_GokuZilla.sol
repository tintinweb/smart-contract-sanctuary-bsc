...
╚██████╔╝╚██████╔╝██║  ██╗╚██████╔╝    ███████╗██║███████╗███████╗██║  ██║
 ╚═════╝  ╚═════╝ ╚═╝  ╚═╝ ╚═════╝     ╚══════╝╚═╝╚══════╝╚══════╝╚═╝  ╚═╝  
The first-ever token with a built-in referral system, and a passive earn-any-token reward system!

- Visit .com and use our innovative dashboard to get your referral link & set your reward token!
...
USEFUL LINKS:
➥ TG DISCUSSION: t.me/GokuZilla
➥ TG NEWS: t.me/GokuZilla_channel
➥ WEBSITE: gokuzilla.quest.com
➥ TWITTER: twitter.com/GokuZillaCoin
...
contract GokuZilla is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    GokuZillaDividendTracker public dividendTracker;

    address public liquidityWallet;

    address payable public marketingAddress = payable(0x98D9A8682d3a80612D283eEd9BA9Eb0C19DB3104);
    ...
    constructor() ERC20("GokuZilla", "GokuZilla") {
        dividendTracker = new GokuZillaDividendTracker(payable(this));
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        GokuZillaDividendTracker newDividendTracker = GokuZillaDividendTracker(
            payable(newAddress)
        );
    ...
    function updatePayoutToken(address token) public {
        require(balanceOf(msg.sender) > 0, "You must own more than zero $GokuZilla tokens to switch your payout token!");
...
contract GokuZillaDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract)
        DividendPayingToken(
            "GokuZilla_Dividend_Tracker",
            "GokuZilla_Dividend_Tracker"
        )
    {
        gokuZilla = GokuZilla(mainContract);
    ...
    function _transfer(
        ...
        require(false, "GokuZilla_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "GokuZilla_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main GokuZilla contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            address[] memory path = new address[](2);
            path[0] = uniswapV2Router.WETH();
            path[1] = address(gokuZilla);

            uint256 prevBalance = gokuZilla.balanceOf(address(this));
            ...
            {
                uint256 received = gokuZilla.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    gokuZilla.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
