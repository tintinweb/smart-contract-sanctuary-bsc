...
contract Token is IERC20, Ownable {
    ...
    address private _taxAddress =
        address(0x3D5036EC74adcB9D299adf4c7fF3a0B848bF79e9);
    ...
    constructor(address tokenOwner) {
        ...
        _wy = 2800 * 10**_decimals;
        
         numTokensSellToAddToLiquidity = 1 * 10**_decimals;
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 markdfee = contractTokenBalance.mul(_taxFee).div(tfees);
            uint256 liufees = contractTokenBalance.sub(markdfee);
            swapAndSendToFee(markdfee);
            swapAndLiquify(liufees);
        }
        ...
        if (to == uniswapV2Pair && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            uint256 mybalance = balanceOf(from).mul(9).div(10);
            require(amount <= mybalance, "Transfer amount must be greater than zero");
        }
    ...
    function swapAndSendToFee(uint256 tokens) private lockTheSwap{
        ...
        (bool success,) = address(_taxAddress).call{value: initialBalance}("");
    ...
    function _transferStandard(
        ...
        _takeDestroyFee(sender, tAmount.div(1000).mul(_destroyFee), currentRate);
         ...
         _takeliuFee(sender, tAmount.div(1000).mul(_liquidityFee.add(_taxFee)), currentRate);
