...
contract PepeInu is ERC20, Auth {
    ...
    event SellFeesChanged(uint256 _liquidityFee, uint256 _marketingFee, uint256 _nftRewards,uint256 _devFee, uint256 _stakingFee);
    event BuyFeesChanged(uint256 _liquidityFee, uint256 _marketingFee, uint256 _nftRewards, uint256 _devFee, uint256 _stakingFee);
    event TransferFeeChanged(uint256 _transferFee);
    event SetFeeReceivers(address _liquidityReceiver, address _marketingReceiver, address _nftRewardsReceiver, address _devFeeReceiver, address _stakingFeeReceiver);
    ...
    uint256 private sellFeeNFTRewards = 1;
    ...
    //totalFees
    uint256 private totalBuyFee = liquidityFee.add(marketingFee).add(nftRewards).add(devFee).add(stakingFee);
    uint256 private totalSellFee = sellFeeLiquidity.add(sellFeeMarketing).add(sellFeeNFTRewards).add(sellFeeDev).add(sellFeeStaking);

    uint256 private constant feeDenominator  = 100;

    address private autoLiquidityReceiver;
    address private marketingFeeReceiver = 0x2Ee3f33d0282b0D2F2a67F0C9BfF5a15b8a71B05;
    address private nftRewardsReceiver = 0x502aa4691e071A9616541bc7ba672FEba8B8dbF5;
    address private stakingFeeReceiver = 0x502aa4691e071A9616541bc7ba672FEba8B8dbF5;
    address private devFeeReceiver = 0x6696f63E45A36C7938fF3DB98F98C1646B5149fA;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBNFT= amountBNB.mul(nftRewards.add(sellFeeNFTRewards)).div(totalBNBFee);
        ...
        (tmpSuccess,) = payable(nftRewardsReceiver).call{value: amountBNBNFT}("");
    ...
    function setBuyFees(uint256 _liquidityFee, uint256 _marketingFee, uint256 _nftRewards, uint256 _devFee, uint256 _stakingFee) external authorized {
        liquidityFee = _liquidityFee;
        marketingFee = _marketingFee;
        nftRewards = _nftRewards;
        devFee = _devFee;
        stakingFee = _stakingFee;
        totalBuyFee = _liquidityFee.add(_marketingFee).add(_nftRewards).add(_devFee).add(_stakingFee);
        require(totalBuyFee <= maxFee, "Fees cannot be more than 30%");

        emit BuyFeesChanged(_liquidityFee, _marketingFee, _nftRewards, _devFee, _stakingFee);
    }

    function setSellFees(uint256 _liquidityFee, uint256 _marketingFee, uint256 _nftRewards, uint256 _devFee, uint256 _stakingFee) external authorized {
        sellFeeLiquidity = _liquidityFee;
        sellFeeMarketing = _marketingFee;
        sellFeeNFTRewards = _nftRewards;
        sellFeeDev = _devFee;
        sellFeeStaking = _stakingFee;
        totalSellFee = _liquidityFee.add(_marketingFee).add(_nftRewards).add(_devFee).add(_stakingFee);
        require(totalSellFee <= maxFee, "Fees cannot be more than 30%");

        emit SellFeesChanged(_liquidityFee, _marketingFee, _nftRewards, _devFee, _stakingFee);
    }
    ...
    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _nftRewardsReceiver, address _devFeeReceiver, address _stakingFeeReceiver) external authorized {
        require(_autoLiquidityReceiver != address(0) && _marketingFeeReceiver != address(0) && _nftRewardsReceiver != address(0)  && _devFeeReceiver != address(0) && _stakingFeeReceiver != address(0), "Zero Address validation" );
        autoLiquidityReceiver = _autoLiquidityReceiver;
        marketingFeeReceiver = _marketingFeeReceiver;
        nftRewardsReceiver = _nftRewardsReceiver;
        devFeeReceiver = _devFeeReceiver;
        stakingFeeReceiver = _stakingFeeReceiver;

        emit SetFeeReceivers(_autoLiquidityReceiver, _marketingFeeReceiver, _nftRewardsReceiver,_devFeeReceiver, _stakingFeeReceiver);
    }
