...
contract TesterOven is ERC20, Ownable {                                                                                          
    ...
    uint256 public maxTx = (TOTAL_SUPPLY * 10) / 1000;               // 1%  of total supply                                           
    uint256 public maxWallet = (TOTAL_SUPPLY * 20) / 1000;          // 2.0%  of total supply
    ...
    function checkValidTrade(address from, address to, uint256 amount) internal {
        ...
        require(amount <= maxTx|| isExcludedFromFees[from] || isExcludedFromFees[to], "TX Limit Exceeded");
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if(shouldSwap(from)){swapTokens(swapTokensAtAmount); }
    ...
    function shouldSwap(address from) internal view returns (bool){
        ...
        !isPair[from] && !isExcludedFromFees[from];
    }    

    function performSwap(uint256 tokens) external onlyAuthorized {
        swapTokens(tokens);
    }
