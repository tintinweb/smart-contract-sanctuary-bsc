...
contract HODL is Context, IBEP20, Ownable, ReentrancyGuard, VRFConsumerBaseV2 {
    ...
    */
    ...
    function startStacking() external {
        ...
        transferTokensTaxfree(msg.sender, stackingWallet, balance);
    ...
    */
    function stopStackingAndClaim(uint256 perc) external nonReentrant {
        ...
        transferTokensTaxfree(stackingWallet, msg.sender, amount);

        HODLStruct.stacking memory tmpStack;
        rewardStacking[msg.sender] = tmpStack;
    }

    function getRewards(uint256 reward, uint256 perc, bool stacking) private {

        uint256 rewardBNB;
        uint256 rewardreinvest;

        if (perc == 100) {
            require(stacking || reward > claimBNBLimit, "Reward below gas fee");
            rewardBNB = reward;
        } else if (perc == 0) {     
        ...
        if (perc < 100) {
            ...
            require(stacking || reward > reinvestLimit, "Reward below gas fee");
            ...
            transferTokensTaxfree(address(this), msg.sender, expectedtoken);
        }
    ...
    /* @dev Intern function to transfer tokens from a intern wallet to
    *   a recipient. Recipient could be excluded. 
    */
    function transferTokensTaxfree(address _sender, address _recipient, uint256 _amount) private {
        uint256 rAmount = _amount * _getRate();
        if (_isExcluded[_recipient]) { 
            _rOwned[_recipient] += rAmount;
            _tOwned[_recipient] += _amount;
            _rOwned[_sender] -= rAmount;
        } else {
            _rOwned[_recipient] += rAmount;
            _rOwned[_sender] -= rAmount;
        }
