...
contract StrategyGalaxyFlashBoost is StrategyBase {
    ...
    uint256 public borrowBasis;
     ...
     constructor (
        ...
        ctoken = newCtoken;
        want = ICompoundToken(ctoken).underlying();
    ...
    function _callback(
        ...
        if (details.minting) {
            ICompoundToken(details.cToken).mint(mintOrRedeemAmount);
            ICompoundToken(details.cToken).borrow(borrowedAmount);
        } else {
            ICompoundToken(details.cToken).repayBorrow(borrowedAmount);
            ICompoundToken(details.cToken).redeemUnderlying(mintOrRedeemAmount);
        }
    ...
    function deposit() public whenNotPaused {
        ...
        if (wantBal > 0) {
            ...
            _doFlashSwap(true, borrowPool, ctoken, want, borrowAmount, wantBal);
            updateBalance();
        }
    }
    ...
    function withdraw(uint256 amount) external {
        ...
        if (wantBal < amount) {
            ...
            _doFlashSwap(false, borrowPool, ctoken, want, borrowAmount, amount);
    ...
    function updateBalance() public {
        uint256 supplyBal = ICompoundToken(ctoken).balanceOfUnderlying(address(this));
    ...
    function supply(uint256 amount) public onlyManager {
        ICompoundToken(ctoken).mint(amount);
    }

    function redeem(uint256 shares) public onlyManager {
        ICompoundToken(ctoken).redeem(shares);
    }

    function redeemUnderlying(uint256 amount) public onlyManager {
        ICompoundToken(ctoken).redeemUnderlying(amount);
    }
     ...
     */
     ...
     function retireStrategy() external {
        ...
        ICompoundUnitroller(unitroller).exitMarket(ctoken);
    }
    ...
    function panic() public onlyManager {
        ...
        uint256 supplyBal = ICompoundToken(ctoken).balanceOfUnderlying(address(this));
        uint256 borrowBal = ICompoundToken(ctoken).borrowBalanceStored(address(this));
        _doFlashSwap(false, borrowPool, ctoken, want, borrowBal, supplyBal);
    }
