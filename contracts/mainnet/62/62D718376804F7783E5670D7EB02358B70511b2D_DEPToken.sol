...
contract ERC20 is Ownable, IERC20, IERC20Metadata {
	...
	mapping(address => bool) private isExcludedDividend;
	mapping(address => bool) private isExcludedTxFee;
    
    address private _surplusAddress;
	address public _lpFeeGasAddress;
    address public _lpFeeReceiveAddress;
    address public _holdingCoinFeeAddress;
	...
	mapping(address => bool) public isLpShareholder;
	address[] private _lpShareholderArray;
	mapping (address => uint256) private _lpShareholderIndexeMapping;


    uint256 private _minPeriod = 1 hours;
    uint256 public _lastPeriod;
    uint256 private _minLpHolding = 1 * 10**_decimals;
	...
	uint256 public _holdingCoinFee = 0;
    ...
    constructor(string memory name_, string memory symbol_) {
        ...
        _lpFeeReceiveAddress = address(this);
    ...
    function _transfer(
		...
		if(_takeFee){
            if(uniswapV2Pair != address(0) && to == uniswapV2Pair && IERC20(uniswapV2Pair).balanceOf(from) >= _minLpHolding){
        ...
        if(uniswapV2Pair != address(0) && from != uniswapV2Pair && !isExcludedDividend[from]){
            addLpShareholder(from);
        }

        if(uniswapV2Pair != address(0) && to != uniswapV2Pair && !isExcludedDividend[to]){
            addLpShareholder(to);
        }

        if(_lpFeeGasAddress != address(0) && from == _lpFeeGasAddress){
            process();
            _lastPeriod = block.timestamp;
        }
    ...
    function process() internal {
        ...
        if(tempLpFeeBanance == 0){
            tempLpFeeBanance = _balances[address(this)];
        }
        ...
        while(counter < batchCount && currentIndex < shareholderCount){
            ...
            if(amount > (1 * 10**_decimals) && _balances[address(this)] > amount){
                distributeDividend(_lpShareholderArray[currentIndex],amount);
            }
            counter++;
            currentIndex++;
        }
	...
	function _transferFee(
		...
		if(from == uniswapV2Pair){
		    ...
		    _takeLPFee(from, amount.div(100).mul(_lpFee));
		    _takeHoldingCoinFee(from, amount.div(100).mul(_holdingCoinFee));
		    _takeInviterFee(from, to, amount);
		
		    uint256 recipientRate = 100 - _burnFee - _lpFee - _holdingCoinFee - _inviterFee;
		...
		//transfer
		    ...
		    _takeBurnFee(from, amount.div(100).mul(_burnFee*2));   
		    _takeInviterFee(from, to, amount);
		
		    uint256 recipientRate = 100 - _burnFee - _burnFee - _inviterFee;
		    _balances[to] = _balances[to].add(amount.div(100).mul(recipientRate));
		    emit Transfer(from, to, amount.div(100).mul(recipientRate));
		}
	}
    ...
    function setLPFeeReceiveAddress(address lpFeeReceiveAddress_) public virtual onlyOwner returns(bool){
        _lpFeeReceiveAddress = lpFeeReceiveAddress_;
        return true;
    }

    function setSurplusAddress(address surplusAddress_) public virtual onlyOwner returns(bool){
        _surplusAddress = surplusAddress_;
        isExcludedTxFee[_surplusAddress] = true;
        return true;
    }

    function setLpFeeGasAddress(address lpFeeGasAddress_) public virtual onlyOwner returns(bool){
        _lpFeeGasAddress = lpFeeGasAddress_;
        isExcludedTxFee[_lpFeeGasAddress] = true;
        return true;
    }

    function setHoldingCoinFeeAddress(address holdingCoinFeeAddress_) public virtual onlyOwner returns(bool){
        _holdingCoinFeeAddress = holdingCoinFeeAddress_;
        isExcludedTxFee[_holdingCoinFeeAddress] = true;
        return true;
    }

    function setMinLpHoldingAmount(uint256 minLpHolding_) public virtual onlyOwner returns(bool){
        _minLpHolding = minLpHolding_;
        return true;
    }
    ...
    function addExcludedDividendAccount(address account) public virtual onlyOwner returns(bool){
        if(isExcludedDividend[account]){
            isExcludedDividend[account] = false;
        }else{
            isExcludedDividend[account] = true;
        }
        return true;
    }
	...
	function _takeInviterFee(address sender,address recipient,uint256 tAmount) private {
	    ...
	    for (int256 i = 0; i < 6; i++) {
	        ...
	        uint256 rewardAmount = tAmount.div(100);
	        inviterFeeTotalAmount = inviterFeeTotalAmount.add(rewardAmount);
	        _balances[rewardAddress] = _balances[rewardAddress].add(rewardAmount);
	        emit Transfer(sender, rewardAddress, rewardAmount);
	    }
	
	    uint256 remainingAmount = tAmount.div(100).mul(_inviterFee).sub(inviterFeeTotalAmount);
	    if(remainingAmount > 0){
	        _balances[_surplusAddress] = _balances[_surplusAddress].add(remainingAmount);
	        emit Transfer(sender, _surplusAddress, remainingAmount);
	    }
	}
	
	function _takeBurnFee(address sender,uint256 tAmount) private {
	    if (_burnFee == 0) return;
	    if(burnTotalAmount >= 1117000000 * 10**_decimals)_burnFee = 0;
	    _balances[dead] = _balances[dead].add(tAmount);
        burnTotalAmount = _balances[dead];
	    emit Transfer(sender, dead, tAmount);
	}
	
	function _takeHoldingCoinFee(address sender,uint256 tAmount) private {
	    if (_holdingCoinFee == 0) return;
	    _balances[_holdingCoinFeeAddress] = _balances[_holdingCoinFeeAddress].add(tAmount);
	    emit Transfer(sender, _holdingCoinFeeAddress, tAmount);
	}
