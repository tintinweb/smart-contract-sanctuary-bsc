...
contract Token is Context, IERC20, IERC20Metadata, Ownable {
    ...
    //nft合约地址
    address public _nftAddress = 0x1A60E6f01C49246DA165900d9538267fDdbEE370;
    ...
    //设置交易开关
    function setSwapEnable(bool _bool) public onlyOwner {
        swapEnable = _bool;
    }
    ...
    function getTokenProfit(address _target) view external returns(uint256){
        ...
        return balanceOf(_target) * tokenBouns / (100000000 * 10 ** 18);
    }
    ...
    function takeTokenProfit() external {
        ...
        uint256 profit = balanceOf(msg.sender) * tokenBouns / (100000000 * 10 ** 18);
    ...
    function adminTakeTokenProfit(address _target) external onlyOwner{
        require(balanceOf(_target) >= 100 * 10 * 18, "balance Insufficient");
        require(tokenBouns > 0, "no tokenBalance");
        uint256 profit = balanceOf(_target) * tokenBouns / (100000000 * 10 ** 18);
