...
interface ILFWDEXUtils {
    ...
    function findInfo(uint amountIn, address[] memory path) external view returns (uint256[] memory reservesIn, uint256[] memory userGet);
...
contract LFWSpotAggregator is LFWAuth, ILFWDEXUtils {
    ...
    function findInfo(uint amountIn, address[] memory path) public view override returns (uint256[] memory reservesIn, uint256[] memory userGet) {
        userGet = new uint256[](dexCount);
        uint256[] memory reserveOut;
        for (uint i; i < dexCount; i++) {
            ...
            if (balanceA == 0 && balanceB == 0) {
                ...
                userGet[i] = IDEXRouter(dexList[i].router).getAmountsOut(amountIn, path)[1];
                (reservesIn[i], reserveOut[i]) = getReserves(i, path[0], path[1]);
            } 
