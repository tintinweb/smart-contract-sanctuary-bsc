...
contract LanKwaiFong is ERC20 {
	...
	address _fundAddress = address(0x1F95Fc3C089255AAb8e5A3Dc58de107460C31852);
    address _fund1Address = address(0x853080aD3Cc8CD915a74487e30a9ADC093D439e5);
    ...
    constructor(address tokenOwner) ERC20("Lan Kwai Fong", "LKF") {
		...
		nft = UsdtWarp(0x469609083e8e4DE445a2A702bE0Ce69E34D86c37);//Nft Contract
    ...
    function _transfer(
		...
		uint256 _downFee;
        if(startTime > 0 && balanceOf(uniswapV2Pair) > 0){
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(_isPairs[from]){
				...
				require(amount <= 2 * 10**23,"maxamount");
				...
				if(startTime.add(600) >= block.timestamp){
					super._transfer(from, _fundAddress, amount.div(100).mul(14));
				...
				_splitOtherToken();
				if(startTime.add(600) >= block.timestamp){
					super._transfer(from, _fundAddress, amount.div(100).mul(24));
					amount = amount.div(100).mul(70);
				}else{
					if(_downFee > 0){
						super._transfer(from, _fundAddress, amount.div(100).mul(_downFee));
						amount = amount.div(100).mul(94 - _downFee);
					}else{
						amount = amount.div(100).mul(94);
					}
				}
    ...
    function swapAndLiquify() public {
		...
		if(allTokenAmount > 10**18){
			...
			swapTokensForUsdt(allTokenAmount.div(12).mul(11));
			uint256 allAmount = USDT.balanceOf(address(this));
			if(allAmount > oldAmount){
				uint256 newAmount = allAmount - oldAmount;
				USDT.transfer(_fundAddress, newAmount.div(11).mul(3));
                USDT.transfer(_fund1Address, newAmount.div(11).mul(3));
				addLiquidityUsdt(newAmount.div(11), allTokenAmount.div(12));
			}
		}
    }
