...
interface IZap {
  function calc_withdraw_one_coin(address pool, uint256 token_amount, uint128 index) external view returns (uint256);
...
contract StrategyAlpacaAUSDEPSFarm is BaseStrategy {
  ...
  function convertLockTokenAndDraw(uint256 amount, uint256 stablecoinAmount, bool flag) internal {
    ...
    if (flag) {
      if (positionId == 0) {
        ...
        _data = abi.encodeWithSignature(
          "convertAndLockToken(address,address,address,address,address,uint256,uint256,uint256,bytes)",
          ibToken,
          positionManager,
          stabilityFeeCollector,
          tokenAdapter,
      ...
      if (stablecoinAmount == uint256(-1)) {
        ...
        _data = abi.encodeWithSignature(
          "wipeUnlockTokenAndConvert(address,address,address,address,uint256,uint256,uit256,bytes)",
          ibToken,
          positionManager,
          tokenAdapter,
          positionId,
  ...
  function liquidatePosition(uint256 _amountNeeded) internal override returns (uint256 _amountFreed, uint256 _loss) {
    ...
    uint256 amountToWithdraw = _min(assets, _amountNeeded);

    _withdrawSome(amountToWithdraw);
