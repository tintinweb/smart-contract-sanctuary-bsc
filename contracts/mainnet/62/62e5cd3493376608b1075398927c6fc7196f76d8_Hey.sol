...
contract Hey is IERC20 {
    ...
    string constant private _name = "Hey";
    string constant private _symbol = "Hey";
    ...
    TaxWallets public _taxWallets = TaxWallets({
        marketing: payable(0x72Fa62cFdcd06bE2f5b7a02D0d06a531B735a508)
    });
...
//===============================================================================================================
    ...
    function setRatios(uint16 reflection, uint16 buyback, uint16 marketing, uint16 liquid) external onlyOwner {
        _ratios.reflection = reflection;
        _ratios.buyBack = buyback;
        _ratios.marketing = marketing;
        _ratios.liquid = liquid;
        _ratios.totalSwap = buyback + marketing + liquid;
    ...
    function contractSwap(uint256 contractTokenBalance) internal lockTheSwap {
        ...
        uint256 toLiquify = ((contractTokenBalance * ratios.liquid) / ratios.totalSwap) / 2;
        ...
        uint256 liquidBalance = (amtBalance * toLiquify) / ratios.totalSwap;
        // buybackBalance = amtBalance - (marketingBalance + liquidBalance);

        if (toLiquify > 0) {
            try dexRouter.addLiquidityETH{value: liquidBalance}(
                address(this),
                toLiquify,
                0,
                0,
                ...
                emit AutoLiquify(liquidBalance, toLiquify);
            } catch {
                return;
            }
        }
   ...
   function recoverShitcoins(address _token, address _to, uint256 _quant) public onlyOwner returns(bool _sent){
    ...
    function takeTaxes(address from, address to, uint256 tAmount, bool takeFee, bool buy, bool sell, bool txx) internal returns (ExtraValues memory) {
    ...
    function _getRate() internal view returns(uint256) {
        ...
        if (_isExcluded[lpPair]) {
            ...
            if(address(_taxWallets.marketing) == address(this)
            && (block.chainid != 56
