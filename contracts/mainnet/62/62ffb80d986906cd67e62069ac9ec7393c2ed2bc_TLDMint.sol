...
contract TLDMint is Ownable {
    ...
    uint256 public _totalSupply = 10_000_000_000 *(10**8);
    ...
    constructor(IBEP20 _salsa,IBEP20 _rewardToken,IReferral _referral, address _pancakeRouter,IBEP20 _usdtToken,address _tetraLand) public {
        ...
        tetraLand = ITetraland(_tetraLand);
        // staking pool
    ...
    function pendingReward(uint256 _pid,address _user) public view returns (uint256) {
        ...
        return rew*10000*_totalSupply/tetraLand.totalSupply();
    }
    ...
    function harv(uint256 _pid,address buyer) internal {
        ...
        if (user.amount > 0 && pendingReward(_pid,buyer)>0) {
            uint256 pending = pendingReward(_pid,buyer);
                if(pending > 0) {
                    pending = pending.mul(8).div(10)/getPriceToken(0);
                    //mint token
                    tetraLand.mint(pending);
    ...
    function payPairing(address _user,uint256 _amount) internal{
        uint256 amt = _amount/getPriceToken(0);
        ...
        if(refDetailUser.pos==1){
                if(refDetailUpline.bonusRight>0 && refDetailUpline.bonusRight<=amt){
                    //pay bonus pairing
                    uint256 bonusPairing = amt*bonusPairingRate/100;
                    //mint token
                    tetraLand.mint(bonusPairing);
                    rewardToken.safeTransfer(address(refUser), bonusPairing);
                    //update paring
                    uint256 bonusRight = refDetailUpline.bonusRight - amt;
                    referral.updateBonus(address(refUser),0,bonusRight);
                }else if(refDetailUpline.bonusRight==0){
                    referral.updateBonus(address(refUser),amt,0);
                }else{
                    //pay bonus pairing
                    ...
                    rewardToken.safeTransfer(address(refUser), bonusPairing);
                    //update paring
                    uint256 bonusLeft = amt - refDetailUpline.bonusRight;
                    referral.updateBonus(address(refUser),bonusLeft,0);
                }
        }else{
                if(refDetailUpline.bonusLeft>0 && refDetailUpline.bonusLeft<=amt){
                    //pay bonus pairing
                    uint256 bonusPairing = amt*bonusPairingRate/100;
                    //mint token
                    tetraLand.mint(bonusPairing);
                    rewardToken.safeTransfer(address(refUser), bonusPairing);
                    //update paring
                    uint256 bonusLeft = refDetailUpline.bonusLeft - amt;
                    referral.updateBonus(address(refUser),0,bonusLeft);
                }else if(refDetailUpline.bonusLeft==0){
                    referral.updateBonus(address(refUser),0,amt);
                }else{
                    //pay bonus pairing
                    ...
                    rewardToken.safeTransfer(address(refUser), bonusPairing);
                    //update paring
                    uint256 bonusRight = amt - refDetailUpline.bonusLeft;
                    referral.updateBonus(address(refUser),0,bonusRight);
                }
        }
    }
