...
contract RABBOT is IERC20, Ownable {
    using SafeMath for uint256;
    string private constant _name = 'RABBOT';
    string private constant _symbol = 'RABBOT';
    ...
    address internal constant development_receiver = 0x044AE3CB61Bc645d00d69d76278Be5AeBa157f20; 
    address public constant marketing_receiver = 0x044AE3CB61Bc645d00d69d76278Be5AeBa157f20;
    ...
    function AutomaticTaxLoweringSystem() internal {
        if(launchTime.add(2 minutes) <= block.timestamp){totalFee = uint256(1200); sellFee =uint256(1600);}
        if(launchTime.add(3 minutes) <= block.timestamp){totalFee = uint256(1000); sellFee =uint256(1600);}
        if(launchTime.add(4 minutes) <= block.timestamp){totalFee = uint256(800); sellFee =uint256(1600);}
        if(launchTime.add(5 minutes) <= block.timestamp){totalFee = uint256(400); sellFee =uint256(800);}
        if(launchTime.add(6 minutes) <= block.timestamp){totalFee = uint256(400); sellFee =uint256(400);
            ATLSCompletion = true;}
    }
