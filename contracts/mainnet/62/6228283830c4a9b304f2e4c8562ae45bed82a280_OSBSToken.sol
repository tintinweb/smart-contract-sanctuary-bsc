...
contract OSBSToken is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    
    string private _name = "One Stop Block Shop";
    string private _symbol = "$OSBS";
    uint8 private _decimals = 18;

    address payable public OwnerWallet = payable(0x6901AB8DacD0684135B30c4e63cd319EE5AE1E68);
    
    address public immutable deadAddress = 0x000000000000000000000000000000000000dEaD;
    address public immutable zeroAddress = 0x0000000000000000000000000000000000000000;

    address public USDT = 0x55d398326f99059fF775485246999027B3197955;  //Mainnet
    // address public USDT = 0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684; //Testnet
    ...
    uint256 public minimumTokensBeforeSwap = _totalSupply.mul(1).div(1e5);   //0.0001%
    ...
    constructor () {
        ...
        _allowances[address(this)][address(usdtpair)] = ~uint256(0);   //busd pair

        isExcludedFromFee[OwnerWallet] = true;
        isExcludedFromFee[address(this)] = true;

        isMarketPair[address(uniswapPair)] = true;
        isMarketPair[address(usdtpair)] = true;

        IERC20(USDT).approve(address(uniswapV2Router), ~uint256(0));
        IERC20(USDT).approve(address(uniswapPair), ~uint256(0));
        IERC20(USDT).approve(address(usdtpair), ~uint256(0));
        IERC20(USDT).approve(address(this), ~uint256(0));

        transferOwnership(OwnerWallet);

        _balances[OwnerWallet] = _totalSupply;
    ...
    function setMarketPair(address _pair, bool status) public onlyOwner {
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
       if(tAmount == 0) return;
        _swapTokensForBusd(tAmount,OwnerWallet);
    }
