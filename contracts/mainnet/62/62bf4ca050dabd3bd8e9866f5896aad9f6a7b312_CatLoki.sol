...
contract CatLoki is ICatLoki, ContextUpgradeable, OwnableUpgradeable {
    ...
    address private _initialiserAccount;
    address public _burnAddress;

    bool inSwapAndLiquify;
    bool public swapAndLiquifyEnabled;
    bool public initializedd;
    ...
    modifier lockUpgrade {
        require(!_upgraded, "CatLoki: Already upgraded");
        _;
        _upgraded = true;
    }

    modifier checkTierIndex(uint256 _index) {
        require(feeTiers.length > _index, "CatLoki: Invalid tier index");
        _;
    }
    ...
    function initialize(address _router) public {
        __Context_init_unchained();
        __Ownable_init_unchained();
        __CatLoki_v2_init_unchained(_router);
    }

    function __CatLoki_v2_init_unchained(address _router) internal {
        require(!initializedd, "initialized already");
        _name = "Cat Loki";
        _symbol = "LO";
        _decimals = 18;

        _tTotal = 10 * 10**6 * 10**18;
        ...
        numTokensSellToAddToLiquidity = 500 * 10**6 * 10**18;

        _burnAddress = 0x000000000000000000000000000000000000dEaD;
        _initialiserAccount = _msgSender();

        _rOwned[_initialiserAccount] = _rTotal;
        ...
        __CatLoki_tiers_init();
        initializedd = true;
        emit Transfer(address(0), _msgSender(), _tTotal);
    }

    function __CatLoki_tiers_init() internal {
    ...
    preventBlacklisted(_account, "CatLoki: Selected account is in blacklist")
    {
        require(_account != address(0), "CatLoki: Invalid address");
        _accountsTier[_account] = _tierIndex;
    }

    function excludeWhitelistedAddress(address _account) public onlyOwner() {
        require(_account != address(0), "CatLoki: Invalid address");
        require(_accountsTier[_account] > 0, "CatLoki: Account is not in whitelist");
    ...
    function checkFees(FeeTier memory _tier) internal view returns (FeeTier memory) {
        ...
        require(_fees <= _maxFee, "CatLoki: Fees exceeded max limitation");
    ...
    function setEcoSystemFeeAddress(uint256 _tierIndex, address _ecoSystem) external onlyOwner() checkTierIndex(_tierIndex) {
        require(_ecoSystem != address(0), "CatLoki: Address Zero is not allowed");
    ...
    function setOwnerFeeAddress(uint256 _tierIndex, address _owner) external onlyOwner() checkTierIndex(_tierIndex) {
        require(_owner != address(0), "CatLoki: Address Zero is not allowed");
    ...
    private
    preventBlacklisted(owner, "CatLoki: Owner address is blacklisted")
    preventBlacklisted(spender, "CatLoki: Spender address is blacklisted")
    {
    ...
    private
    preventBlacklisted(_msgSender(), "CatLoki: Address is blacklisted")
    preventBlacklisted(from, "CatLoki: From address is blacklisted")
    preventBlacklisted(to, "CatLoki: To address is blacklisted")
    isRouter(_msgSender())
    {
    ...
    external
    preventBlacklisted(account, "CatLoki: Migrated account is blacklisted")
    override
    {
        require(migration != address(0), "CatLoki: Migration is not started");
        require(_msgSender() == migration, "CatLoki: Not Allowed");
    ...
    function _migrate(address account, uint256 amount) private {
        ...
        _tokenTransfer(_initialiserAccount, account, amount, 0, false);
    }
