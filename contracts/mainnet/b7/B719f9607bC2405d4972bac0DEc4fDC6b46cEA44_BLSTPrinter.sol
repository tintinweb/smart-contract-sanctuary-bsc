...
/*
     ____  _                 _     _                     ____       _       _            
    | __ )| | ___   ___   __| |___| |_ ___  _ __   ___  |  _ \ _ __(_)_ __ | |_ ___ _ __ 
    |  _ \| |/ _ \ / _ \ / _` / __| __/ _ \| '_ \ / _ \ | |_) | '__| | '_ \| __/ _ \ '__|
    | |_) | | (_) | (_) | (_| \__ \ || (_) | | | |  __/ |  __/| |  | | | | | ||  __/ |   
    |____/|_|\___/ \___/ \__,_|___/\__\___/|_| |_|\___| |_|   |_|  |_|_| |_|\__\___|_|   
                                                                                      
    In honor of the awesome game, Crypto Legions, we are proud to present you with BloodstonePrinter!
    
    $BLSTp is a dividend paying token that pays out $BLST rewards.

    This accomplishes several things:
        - Creates constant buy pressure for the $BLST token.
        - Provides users with a token that they can actually use in game.
        - Expands the community and brings more folks to Crypto Legions.

    Fees:
        - 8% $BLST Rewards
        - 6% Liquidity
        - 4% Marketing/Development Expensees

    Set slippage to ~20% for best results.

    Telegram: https://t.me/bloodstoneprinter
...
// Dependency file: contracts/baby/BLSTPrinterDividendTracker.sol
    ...
    DividendPayingTokenOptionalInterface
{
    ...
    function distributeBLSTDividends(uint256 amount) public onlyOwner {
...
contract BLSTPrinterDividendTracker is OwnableUpgradeable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BLSTPrinter contract."
        );
    }
...
// Root file: contracts/baby/BLSTPrinter.sol
...
// import "contracts/interfaces/IPancakeFactory.sol";
// import "contracts/interfaces/IPancakeRouter02.sol";
// import "contracts/baby/BLSTPrinterDividendTracker.sol";
// import "contracts/BaseToken.sol";

contract BLSTPrinter is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    BLSTPrinterDividendTracker public dividendTracker;
    ...
    constructor(
        ...
        dividendTracker = BLSTPrinterDividendTracker(
        ...
        IPancakeRouter02 _pancakeRouter = IPancakeRouter02(addrs[1]);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "BLSTPrinter: The dividend tracker already has that address"
        );

        BLSTPrinterDividendTracker newDividendTracker = BLSTPrinterDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "BLSTPrinter: The new dividend tracker must be owned by the BLSTPrinter token contract"
        );
    ...
    function updatePancakeRouter(address newAddress) public onlyOwner {
        require(
            ...
            "BLSTPrinter: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "BLSTPrinter: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != pancakePair,
            "BLSTPrinter: The PanBLSTSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "BLSTPrinter: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "BLSTPrinter: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "BLSTPrinter: Cannot update gasForProcessing to same value"
        );
    ...
    function swapAndSendToFee(uint256 tokens) private {
        uint256 initialBLSTBalance = IERC20(rewardToken).balanceOf(
            address(this)
        );

        swapTokensForBLST(tokens);
    ...
    function swapTokensForBLST(uint256 tokenAmount) private {
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        if (success) {
            dividendTracker.distributeBLSTDividends(dividends);
            emit SendDividends(tokens, dividends);
        }
    }

    function recoverStuckERC20(address _token) public onlyOwner {
        IERC20 token = IERC20(_token);
        token.transfer(msg.sender, token.balanceOf(address(this)));
    }

    function recoverStuckNative() public onlyOwner {
