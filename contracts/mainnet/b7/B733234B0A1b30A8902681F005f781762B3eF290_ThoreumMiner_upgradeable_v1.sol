...
contract ThoreumMiner_upgradeable_v1 is Initializable, UUPSUpgradeable, AuthUpgradeable, ReentrancyGuardUpgradeable {
    using SafeMath for uint256;

    IAutoMiner public AUTO_CONTRACT;
    //    IBEP20 public token_MAIN = IBEP20(0x22D954CA5540caB869AdA9bd9d339CDE3a9313b3);
    ...
    function initialize() public initializer {
        ...
        treasury = 0x0019d3590Fa4745910e171E24A2Ab477e9997B0b;
        bankWallet = treasury;
        AUTO_CONTRACT = IAutoMiner(0x0Ae3B16566f32AB57EaCDDA4e735B09C37fbE65d);
        token_MAIN = IERC20Upgradeable(0x12875583FAF82119B8A6A351689Bf38079085cc9);
        EGGS_TO_HIRE_1MINERS = 2592000 * 11 / 10;// 18% 479520; 3% 2592000; 8% 1080000;
        ...
        MIN_INVEST_LIMIT = 0; /** change to 10 * 1e18 = 10 BUSD  **/
        WALLET_DEPOSIT_LIMIT = 50000 * 1e18; /** 50000 LP  **/
        ...
        COMPOUND_STEP = 2 * 60; /** todo: change to 24 * 60 *60 so every 24 hours. **/
        WITHDRAW_STEP = 7 * COMPOUND_STEP;
        CUTOFF_STEP = 1 * COMPOUND_STEP;
        ZERO_STEP = 3 * COMPOUND_STEP/2; //36 hours

        WITHDRAW_TAX = 200; // 20%
        WITHDRAW_LIMIT = 2000 * 1e18;

        TAX_ACCELERATOR = 2000;

        allFee = 70; // 7% to treasury
        AUTO_TAX = 50;
        AUTO_ENABLED = false;
        AUTO_MIN_DEPOSIT = 1000 * 10**18;
        WHALE_TAX_MULTIPLIER=5;


        BIGGEST_BUYER_PERCENTAGE = 10; // 1%
        BUYING_POWER_STEP_PERDAY = 1; // 0.1%
        isRewardBiggestBuyer = true;
        biggestBuyerPeriod = COMPOUND_STEP; // to do: change this to 24*3600;
    ...
    function _resetMiner(address addr) internal returns(uint256 _decreased){
    }
    ...
    function SET_MARKET_EGGS_DIVISOR(uint256 _MARKET_EGGS_DIVISOR,uint256 _MARKET_EGGS_DIVISOR_SELL) external onlyOwner{
    }
    ...
    event PayBiggestBuyer(address account, uint256 period, uint256 amount);
