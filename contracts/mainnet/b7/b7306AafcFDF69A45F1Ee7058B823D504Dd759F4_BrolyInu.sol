...
interface ITokenManager {
    ...
    function isBotAddress(address _address) external view returns (bool);
    function isBurnAddress(address _address) external view returns (bool);
    function isBurnEnable() external view  returns(bool);
    function checkBurnAddress(address sender, address recipient) external view returns(bool);
...
interface IReward {
    function claimReward(address _userAddress,uint256 amount) external;
}


contract BrolyInu  is Ownable, ERC20 , IReward {
    using SafeMath for uint256;
    ...
    modifier isOwner() {
        bool check;
        address _address;
        (check,_address) =tkControl.isOwner(msg.sender);
        require(
            check,
