...
contract DefiLaunchPool {
    ...
    function setLockupDuration(uint256 _lockupDuration) public {
        PoolInfo storage pool = poolInfo[0];
...
contract IFO is ReentrancyGuard {
    ...
    function deposit(uint256 _amount) external onlyActiveIFO {
        ...
        require(userInfo[msg.sender].amount < limitPerUser(msg.sender) || limitPerUser(msg.sender) == 0, "user deposit limit is reached");
        require(totalAmount < raisingAmount || overflow == true, "100% raised");
        
        // trim _amount if higher than limit per user
        if (limitPerUser(msg.sender) > 0) {
            // if deposited amount plus new amount is higher than the available limit
            if (userInfo[msg.sender].amount + _amount > limitPerUser(msg.sender)) {
                _amount = limitPerUser(msg.sender) - userInfo[msg.sender].amount;
            }
        }
    ...
    function limitPerUser(address _user) public view returns (uint256) {
        uint256 limit = 500 * (10 ** uint256(stakeToken.decimals()));
        for(uint256 i = 0; i < poolAddresses.length; i++) {
            (uint256 stakedAmount, , ) = DefiLaunchPool(poolAddresses[i]).userInfo(_user);
            if (stakedAmount > 0) {
                limit += stakedAmount * 4 * (10 ** uint256(stakeToken.decimals())) / (10 ** uint256(nativeToken.decimals()));
            }
        }
        return limit;
    }
