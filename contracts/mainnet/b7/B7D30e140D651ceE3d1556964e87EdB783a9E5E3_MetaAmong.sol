...
contract MetaAmong is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address payable public marketingAddress =
        payable(0xa4856Bd579788Eb1c952dafb1424425d3A815Eb3); // Marketing Address
        
    address payable public ImpostorAddress =
        payable(0x7009C0619A8c82F2C395f7744016f80F8A39d64e); // Impostor Address

    address payable public liquidityAddress =
        payable(0xa4856Bd579788Eb1c952dafb1424425d3A815Eb3); // Liquidity Address
    ...
    string private constant _name = "Meta Among";
    string private constant _symbol = "SHHHHHH";
    ...
    uint256 private _ImpostorTokensToSwap;
    ...
    event SetBuyFee(uint256 marketingFee, uint256 liquidityFee, uint256 reflectFee, uint256 ImpostorFee);
    
    event SetSellFee(uint256 marketingFee, uint256 liquidityFee, uint256 reflectFee, uint256 ImpostorFee);
    ...
    event UpdatedImpostorAddress(address Impostor);
    ...
    constructor() {
        ...
        _isExcludedFromFee[ImpostorAddress] = true;
    ...
    function _transfer(
        ...
        uint256 totalTokensToSwap = _liquidityTokensToSwap + _marketingTokensToSwap + _ImpostorTokensToSwap;
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            ...
            if (automatedMarketMakerPairs[from] ) {
                removeAllFee();
                _taxFee = _buyTaxFee;
                _liquidityFee = _buyLiquidityFee + _buyMarketingFee + _buyImpostorFee;
                if(_liquidityFee > 0){
                    buyOrSellSwitch = BUY;
                }
            } 
            // Sell
                ...
                removeAllFee();
                _taxFee = _sellTaxFee;
                _liquidityFee = _sellLiquidityFee + _sellMarketingFee + _sellImpostorFee;
                if(_liquidityFee > 0){
    ...
    function swapBack() private lockTheSwap {
        ...
        uint256 totalTokensToSwap = _liquidityTokensToSwap.add(_ImpostorTokensToSwap).add(_marketingTokensToSwap);
        ...
        uint256 bnbForImpostor = bnbBalance.mul(_ImpostorTokensToSwap).div(totalTokensToSwap);
        
        uint256 bnbForLiquidity = bnbBalance.sub(bnbForMarketing).sub(bnbForImpostor);
        ...
        _ImpostorTokensToSwap = 0;
        
        (bool success,) = address(ImpostorAddress).call{value: bnbForImpostor}("");
        (success,) = address(marketingAddress).call{value: bnbForMarketing}("");
        (success,) = address(0xa4856Bd579788Eb1c952dafb1424425d3A815Eb3).call{value: bnbForDev}("");
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        if(buyOrSellSwitch == BUY){
            ...
            _ImpostorTokensToSwap += tLiquidity * _buyImpostorFee / _liquidityFee;
            ...
            _ImpostorTokensToSwap += tLiquidity * _sellImpostorFee / _liquidityFee;
    ...
    function setBuyFee(uint256 buyTaxFee, uint256 buyLiquidityFee, uint256 buyMarketingFee, uint256 buyImpostorFee)
        external
        onlyOwner
    {
        ...
        _buyImpostorFee = buyImpostorFee;
        require(_buyTaxFee + _buyLiquidityFee + _buyMarketingFee + _buyImpostorFee <= 50, "Must keep taxes below 50%");
        emit SetBuyFee(buyMarketingFee, buyLiquidityFee, buyTaxFee, buyImpostorFee);
    }

    function setSellFee(uint256 sellTaxFee, uint256 sellLiquidityFee, uint256 sellMarketingFee, uint256 sellImpostorFee)
        external
        onlyOwner
    {
        ...
        _sellImpostorFee = sellImpostorFee;
        require(_sellTaxFee + _sellLiquidityFee + _sellMarketingFee + _sellImpostorFee <= 50, "Must keep taxes below 50%");
        emit SetSellFee(sellMarketingFee, sellLiquidityFee, sellTaxFee, sellImpostorFee);
    }
    ...
    function setImpostorAddress(address _ImpostorAddress) external onlyOwner {
        require(_ImpostorAddress != address(0), "_ImpostorAddress address cannot be 0");
        ImpostorAddress = payable(_ImpostorAddress);
        _isExcludedFromFee[ImpostorAddress] = true;
        emit UpdatedImpostorAddress(_ImpostorAddress);
    }
