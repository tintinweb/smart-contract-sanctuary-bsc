...
contract TOKEN is ERC20, Ownable {
    ...
    FeeInfo[] public buyRateList;
    FeeInfo[] public sellRateList;
    mapping(address => uint256) public lastBuyTime;
    uint256 inviteRewardLimit = 20 ether;
    ...
    constructor() ERC20("CEER", "CEER") {
        address mainCol = 0x5902554e6bAb49e37a7282F4F153075711d4C5A6;
        _mint(mainCol, 1000000000000000 * 1e18);
        _router = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
        
        address lpWallet = 0xBD9b1057304CA4127f4e723c0944d0e303711cF7; // 
        address marketingWalletLess = 0xF49F5D1c95f1Af765BBabf966fA53E0EB2703953;
        address marketingWallet = 0xE65a8FB6591d5f556003276c638F00Cce5EDDAD8;
        fireWallet = 0xA8964991EC9dB1D26834Bb08A2E90a53650b5843;
        ...
        buyRateList.push(FeeInfo(marketingWallet,9));
        buyRateList.push(FeeInfo(marketingWalletLess,1));

     

        sellRateList.push(FeeInfo(marketingWallet,27));
        sellRateList.push(FeeInfo(marketingWalletLess,3));
    ...
    function setFireWallet(address addr) public onlyOwner{
        fireWallet = addr;
    }
    ...
    function setInviteRewardLimit(uint256 limit) public onlyOwner{
        inviteRewardLimit = limit;
    }
    ...
    function setBuyRateList(uint8 index,address addr) public onlyOwner{
        FeeInfo storage info = buyRateList[index];
        info.addr = addr;
    }


    function setSellRateList(uint8 index,address addr) public onlyOwner{
    ...
    function getPriceByU(uint256 amount) view public returns(uint256 amounts){ 
        address[] memory routePath = new address[](2);
        routePath[0] = uAddr;
        routePath[1] = address(this);
        return uniswapV2Router.getAmountsOut(amount,routePath)[1];
    }       
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            ...
            if (from == address(uniswapV2Pair) && to == _router){
                ...
                uint256 totalIFee;
                uint256 curLimit = getPriceByU(inviteRewardLimit);
                for(uint i = 0;i<_rate.length;i++){
                    address father = getFather(tmpAddr);
                    address sendTo;
                    if(father != address(0)){
                        tmpAddr = father;
                        if(balanceOf(father)>=curLimit){
                            sendTo = father;
                        }else{
                            sendTo = fireWallet;
                        }
                ...
                if (to == address(uniswapV2Pair)){ // sell
                    ...
                    if(lastBuyTime[to] != 0){
                        uint256 diff = DateTime.diffDays(lastBuyTime[to],block.timestamp);
