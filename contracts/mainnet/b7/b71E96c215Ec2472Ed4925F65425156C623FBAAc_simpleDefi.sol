...
contract simpleDefi is Storage, Ownable, AccessControl {
    ...
    event sdLiquidated(address _user, uint256 _amount);
    event sdHarveterAdd(address _harvester);
    event sdFeeSent(uint amount,uint total);
    event sdGasRecovery(uint amount,uint total,uint finalReward);
    ...
    modifier allowHarvest() {
        if (!(hasRole(HARVESTER,msg.sender) || owner() == msg.sender)) revert sdFunctionLocked();
        _;
    }

    modifier allowAdmin() {
        bool role = hasRole(DEFAULT_ADMIN_ROLE,msg.sender);
    ...
    function liquidate() external {
        performLiquidation(msg.sender,true);
    }
    ...
    function performLiquidation(address _user, bool _sendfunds) public lockFunction returns (uint) {    
        if (poolUtil.getLastDepositDate(mHolders, _user) + DEPOSIT_HOLD > block.timestamp) revert sdLiquidationTooSoon();

        uint _units = poolUtil.calcUnits(iData, mHolders,_user);
        uint total;
        ...
        if (_units > 0) {
            ...
            if (minDepositTime > 0 && mHolders.iHolders[_user].depositDate < block.timestamp - minDepositTime) {
                ...
                emit sdFeeSent(feeAmount,total);
            }

            if (_sendfunds) {
                payable(address(_user)).transfer(total);
                emit sdLiquidated(_user, total);
            }
        }
        return total;
    }
    ...
    function system_liquidate() external allowAdmin lockFunction {
        ...
        for (uint i = mHolders.iQueue.length; i > 0; i--) {
            ...
            emit sdLiquidated(_user,_refund);
        }
        iData.poolTotal = 0;
    ...
    function addFunds(uint inValue, address _user) private {
        ...
        if (_user != address(0)) {
            poolUtil.addFunds(iData, mHolders, inValue,_user);
        }
    }
    ...
    function swap(uint amountIn, address _token0, address _token1) internal returns (uint){
        ...
        path[0] = _token0;
        path[1] = _token1;
        return poolUtil.swap(exchangeInfo, amountIn,path);
    }
    ...
    function do_harvest(uint revert_trans) private returns (uint) {
        ...
        emit sdGasRecovery(gasRecovery,pendingCake,finalReward);
    ...
    function userInfo(address _user) external view returns (uint amount, uint lastDeposit, uint units, uint _pendingReward) {
        (amount,lastDeposit,units) = poolUtil.getUserInfo(iData, mHolders,_user);
    ...
    function pendingRewardUser(address _user, uint _units) public view returns (uint) {
        ...
        if (_units == 0) _units = poolUtil.calcUnits(iData, mHolders,_user);
