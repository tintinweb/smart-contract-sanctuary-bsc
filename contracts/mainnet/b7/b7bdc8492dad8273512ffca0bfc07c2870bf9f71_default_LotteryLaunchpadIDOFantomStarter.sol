...
**/
contract default_LotteryLaunchpadIDOFantomStarter is AccessControl {
...
* @dev Get total tokens bought, and total tokens spent
    **/
    function getTotalIdoTokensBought() public view returns (uint256 idoTokens) {
        uint256 _receiptTotalAmountBoughtIdoTokens = 0;
        ...
        return _receiptTotalAmountBoughtIdoTokens;
    }
    ...
    **/
    function fakeBuy(address[] memory _wallets, uint256 _amountInPayToken) external isAdmin isFundingNotCanceledByAdmin isLotteryPlayedAndAllocationCalculated onlyWinners {
        require(_amountInPayToken > 0, "Amount has to be positive");

        for (uint256 i = 0; i < _wallets.length; i++) {
            require(_amountInPayToken <= calculateMaxPaymentToken(lotteryContract.getWinnerTokenAllocation(_wallets[i])), Strings.append("You cannot buy more tokens than is allowed according to your lottery allocation calculation", Strings.toString(i), Strings.addressToAsciiString(_wallets[i]), "", ""));
            require(spendPayTokensMapping[_wallets[i]].add(_amountInPayToken) <= calculateMaxPaymentToken(lotteryContract.getWinnerTokenAllocation(_wallets[i])), Strings.append("You cannot buy more tokens than is allowed according to your lottery allocation calculation", Strings.toString(i), Strings.addressToAsciiString(_wallets[i]), "", ""));
        }
        ...
        for (uint256 i = 0; i < _wallets.length; i++) {
            ...
            if (spendPayTokensMapping[_wallets[i]] == 0) {
            ...
            spendPayTokensMapping[_wallets[i]] = spendPayTokensMapping[_wallets[i]].add(_amountInPayToken);
            idoTokensToGetMapping[_wallets[i]] = idoTokensToGetMapping[_wallets[i]].add(_idoTokensToBuy);

            if (!isBuyer(_wallets[i])) {
                buyersList.push(_wallets[i]);
            }

            emit BoughtIDOTokens(_wallets[i], _amountInPayToken, _idoTokensToBuy, block.timestamp);
        }
    }
