...
contract AngleInvestor is Ownable, Pausable {
	...
	returns (uint256)
	{
		...
		if (block.timestamp > timeTGE) {
			...
			.div(100)
			.mul(block.timestamp.sub(timeTGE).div(timePeriod));
			if (unlockAmount > 0) {
				if (unlockAmount >= userLockDetail[_wallet]) {
					totalLock = totalLock.sub(userLockDetail[_wallet]);
				} else {
	...
	function setUserLockDetail(address _wallet, uint256 _lockDetail)
	public
	onlyOwner
	{
		userLockDetail[_wallet] = _lockDetail;
	}

	function setTimeTGE(uint256 _newTime) public onlyOwner {
		timeTGE = _newTime;
	}
