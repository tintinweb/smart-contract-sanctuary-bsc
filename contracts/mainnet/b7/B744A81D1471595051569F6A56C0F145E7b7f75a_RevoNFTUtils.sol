...
interface IRevoNFTUtilsBurnLimit{
    ...
    function burnDollarAmount() external returns(uint256);
    function setburnDollarAmount(uint256) external;
    function burnDollarTs() external returns(uint256);
    function setburnDollarTs(uint256) external;
}
...
contract RevoNFTUtils is Ownable {
    ...
    uint256 public burnLimitMinTime = 604800;
    uint256 public burnLimitDollarTime = 86400;
    uint256 public maxDollarSpendable = 1000;
    ...
    constructor(address _revoLibAddress, address _revoNFT, address _revoTier) {
        ...
        editBurnType(0, "Common Burn", "BURN_COMMON_SAME_REVO", 30, true, 1, 170);
        editBurnType(1, "Rare Burn", "BURN_RARE_SAME_REVO", 12, true, 2, 110);
        editBurnType(2, "Legendary Burn", "BURN_LEGENDARY_DIFFERENT_REVO", 1, false, 3, 120);
        editBurnType(3, "Shiny Burn", "BURN_SHINY_DIFFERENT_REVO", 1, false, 4, 110);
    }
    ...
    */
    ...
    function burnNFT(uint256 _burnIndex, uint256[] memory _tokenIds) public {
        ...
        if(block.timestamp - burnLimit.burnDollarTs() < burnLimitDollarTime){
            require(burnLimit.burnDollarAmount().add(burnType[_burnIndex].rewardDollar) <= maxDollarSpendable, "Burn limit reached");
            burnLimit.setburnDollarAmount(burnLimit.burnDollarAmount() + burnType[_burnIndex].rewardDollar);
        }else{
            burnLimit.setburnDollarTs(block.timestamp);
            burnLimit.setburnDollarAmount(burnType[_burnIndex].rewardDollar);
        }

        if(block.timestamp - burnLimit.getFirstBuyTs(_burnIndex, msg.sender) < burnLimitMinTime){
            require(burnLimit.getCount(_burnIndex, msg.sender) < burnLimit.burnLimitValue(), "Burn amount limit reached");
    ...
    */
    ...
    function setBurnLimitMinTime(uint256 _value) public onlyOwner {
        burnLimitMinTime = _value;
    }

    function setburnLimitDollarTime(uint256 _value) public onlyOwner {
        burnLimitDollarTime = _value;
    }

    function setMaxDollarSpendable(uint256 _value) public onlyOwner {
        maxDollarSpendable = _value;
    }
}