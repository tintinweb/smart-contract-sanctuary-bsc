...
contract TokenVesting is Initializable, OwnableUpgradeable {
  ...
  function revoke() public onlyOwner {
    ...
    token.safeTransfer(OwnableUpgradeable.owner(), refund);
  ...
  function withdraw() public onlyOwner {
    uint256 balance = token.balanceOf(address(this));
    token.safeTransfer(OwnableUpgradeable.owner(), balance);
  }
  ...
  function releasableAmount() public view returns (uint256) {
    ...
    return vestedAmount() - (released[address(token)]);
  }
  ...
  function vestedAmount() public view returns (uint256) {
    ...
    if (block.timestamp < cliff) {
      ...
      // ex: 鎖定 2 分鐘(chiff = 120) 後，線性解鎖 5 分鐘(duration = 300)，解鎖區間時間，每分鐘(_block_release_per = 60)解鎖 (20%)
      // 先算出每區段釋出幾 % 獎勵
      uint256 percentage = (duration) / (blockReleasePer);
      // 算出先在時間已經是第幾個區段
      uint256 numBlock = (block.timestamp - (cliff)) / (blockReleasePer) + (1);

      // 總量（包含已領出的）* 已做份數（全部經過了幾份）/ 總共的份數
      return totalBalance * (numBlock) / (percentage);
    }
  }

  function setCliff(uint256 _t) external onlyOwner {
    cliff = start + _t;
  }
}
