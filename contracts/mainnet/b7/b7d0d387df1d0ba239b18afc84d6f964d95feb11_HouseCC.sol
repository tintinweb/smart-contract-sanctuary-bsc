...
contract HouseCC is IERC20, Ownable, IDividendsClaims{
    
    using SafeMath for uint256;

    string _name = "HouseCC";
    string _symbol = "HCC";
    uint8 _decimals = 9;
    ...
    function withoutDividendShare(address holder) external onlyOwner {
    ...
    function takeHouseCCFee(address recipient, uint256 amount) internal returns (uint256) {        
    ...
    function setHouseCCtings(uint256 newSwapBackLimit) external onlyOwner {
