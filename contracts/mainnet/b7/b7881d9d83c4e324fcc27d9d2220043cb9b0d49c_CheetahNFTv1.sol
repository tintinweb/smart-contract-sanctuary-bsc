...
contract CheetahNFTv1 is Initializable, ERC721URIStorageUpgradeable, OwnableUpgradeable {
    ...
    function mintCheetah(string memory _uri) external payable returns(uint) {
        _tokenIds.increment();

        require((_tokenIds.current() + 1) < maxSupply, "Cannot mint more");
        require(msg.value > 0, "Nft not free");
    ...
    // Fetch total cheetahs minted
    function totalCheetahs() external view returns(uint) {
        return _tokenIds.current();
    }

    // Change fee address
    function changeFeeAddress(address addr) external onlyOwner {
