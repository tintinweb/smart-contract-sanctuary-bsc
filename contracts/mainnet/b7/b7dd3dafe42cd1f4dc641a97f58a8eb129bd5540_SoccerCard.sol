...
// Telegram : https://t.me/SoccerCardPortal
// Website : https://soccercard.fans/
...
library Address{
 ...
 contract SoccerCard is IERC20, Context, Ownable {
    ...
    string private constant _name = "SoccerCard";

    uint256 public SOCA_max_BuyLimit = 30000000 * 10**9;
    uint256 public SOCA_max_SellLimit = 30000000 * 10**9;

    address private SOCA_Wallet = 0xB4aFbE21340Dbf6694Ca1A63fe63bCb569f5766C;
    address private SOCA_Team_Wallet = 0x6ff6e5e1EC201960f28D24159f4b8E586DC59C17;

    uint256 public SOCA_max_WalletLimit = 30000000 * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[SOCA_Wallet] = true;
        _isExcludedFromFee[SOCA_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[SOCA_Wallet] = true;
        allowedTransfer[SOCA_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= SOCA_max_BuyLimit, "You are exceeding SOCA_max_BuyLimit");
            require(balanceOf(to) + amount <= SOCA_max_WalletLimit, "You are exceeding SOCA_max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= SOCA_max_SellLimit, "You are exceeding SOCA_max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= SOCA_max_WalletLimit, "You are exceeding SOCA_max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(SOCA_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(SOCA_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function SOCA_Update_MarketingWallet(address newWallet) external onlyOwner{
        SOCA_Wallet = newWallet;
    }
    
    function SOCA_Update_TeamWallet(address newWallet) external onlyOwner{
        SOCA_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        SOCA_max_BuyLimit = maxBuy * 10**decimals();
        SOCA_max_SellLimit = maxSell * 10**decimals();
    }
    
    function SOCA_Max_Walletlimit(uint256 amount) external onlyOwner{
        SOCA_max_WalletLimit = amount * 10**decimals();
    }
