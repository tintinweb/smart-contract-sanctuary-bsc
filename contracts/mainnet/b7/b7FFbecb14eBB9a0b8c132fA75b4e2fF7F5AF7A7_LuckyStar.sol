...
contract LuckyStar is Ownable {
    ...
    function archive(
        ...
        string calldata _rnHash,
        address _addr,
        string calldata _code
    ) public onlyOwner {
        require(bytes(_period).length != 0, "Parameter period is null");
        require(!orders[_period].isUsed, "Record was archived.");
        require(bytes(_rnHash).length != 0, "Invalid rnHash value");
        require(bytes(_code).length != 0, "Parameter luckyCode is null");
        orders[_period] = Record({
            period: _period,
            rnHash: _rnHash,
            winner: _addr,
            luckyCode: _code,
