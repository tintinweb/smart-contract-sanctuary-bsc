...
contract WK is ERC20Burnable, Ownable{
    ...
    uint256 public lpRenewAmount = 10**28;
    uint256 public nftRenewAmount = 10**28;
    uint256 public totalLPAmount;
    uint256 public totalNFTAmount;
    uint256 public nftStartTime;
    uint256 public startTime;
    address public taskData = 0x1fd72426128920253765344E0822faaAC31DAe4b;
    ...
    // address public constant _operationAddress = 0xD76DCdb617b7CD60C6502b3BB6737A14d179A936;
    // address public constant _communityAddress = 0x9b42713DBEc3bbB7C13B3E34f3850930b874fFF6;
    address public constant _operationAddress = 0x9d9e3BCc8F6242EAB6a250A91eE57Fea4fa229a3;
    ...
    constructor(string memory name_, string memory symbol_,address nft_, uint256 markTime_,uint256 nftStartTime_,uint256 startTime_,address relation_)
        ERC20(name_, symbol_)
    {
        ...
        _isExcludedFromFees[_fund0Address] = true;
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair) {
            ...
            if(checkOpen){
            require(block.timestamp >= nftStartTime,"NFT Not Start");
            uint256 ilvl = IWKNFT(wknft).getNFTlvl(to);
            uint256 WKCurrentPrice = getWKPrice();
            uint256 value = (balanceOf(to)+amount) * WKCurrentPrice/10 ** 18;
            if(ilvl ==1){
            require(value <= 400 * 10 ** 18,"Over Size");
            }
            else if(ilvl ==2){
            require(value <= 200 * 10 ** 18,"Over Size");
            }
            else if(ilvl ==3){
            require(value <= 100 * 10 ** 18,"Over Size");
            } 
            else if(ilvl ==4){
            require(value <= 50 * 10 ** 18,"Over Size");
            }            
            else{
        ...
        if(to == uniswapV2Pair) {
            ...
            if(ITask(taskData).taskStats(cur)){
    ...
    function setLpAddress(address value) external onlyOwner {
        _lpAddress = value;
        emit LpAddressSet(value);
    }
    function setNftAddress(address value) external onlyOwner {
        _nftAddress = value;
        emit NftAddressSet(value);
    }

    function setRelation(address value) external onlyOwner {
        Relation = value;
    }
    ...
    function setNftRenewAmount(uint256 value) external onlyOwner {
        nftRenewAmount = value;
    }
    ...
    function setTask(address value) external onlyOwner {
        taskData = value;
    }


    event LpAddressSet(address value);
    event NftAddressSet(address value);
